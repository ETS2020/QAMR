// Benchmark "FAU" written by ABC on Thu Jun 25 17:31:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n124_, new_n126_, new_n127_;
  inv1   g00(.a(x02), .O(new_n46_));
  nor2   g01(.a(x01), .b(x00), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g03(.a(x17), .O(new_n49_));
  inv1   g04(.a(x18), .O(new_n50_));
  inv1   g05(.a(x19), .O(new_n51_));
  nand4  g06(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x09), .O(new_n52_));
  nor2   g07(.a(new_n52_), .b(new_n48_), .O(z02));
  nor2   g08(.a(x19), .b(new_n50_), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  nand3  g10(.a(new_n47_), .b(new_n49_), .c(new_n46_), .O(new_n56_));
  nand2  g11(.a(x02), .b(x00), .O(new_n57_));
  inv1   g12(.a(x09), .O(new_n58_));
  nand2  g13(.a(x10), .b(new_n58_), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(x12), .c(x11), .O(new_n61_));
  oai22  g16(.a(new_n61_), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(z03));
  nor4   g17(.a(new_n59_), .b(new_n57_), .c(x12), .d(x11), .O(z04));
  nand4  g18(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(z05));
  inv1   g20(.a(x11), .O(new_n66_));
  nor3   g21(.a(new_n59_), .b(new_n57_), .c(new_n66_), .O(z06));
  nor2   g22(.a(x12), .b(new_n66_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n60_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(x01), .c(new_n57_), .O(z07));
  inv1   g25(.a(x03), .O(new_n71_));
  nor2   g26(.a(x06), .b(x05), .O(new_n72_));
  nor2   g27(.a(x08), .b(x07), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(x04), .d(new_n71_), .O(new_n74_));
  nand2  g29(.a(new_n47_), .b(x02), .O(new_n75_));
  nand3  g30(.a(x19), .b(new_n50_), .c(x17), .O(new_n76_));
  nor3   g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(z08));
  inv1   g32(.a(x15), .O(new_n78_));
  inv1   g33(.a(x16), .O(new_n79_));
  nand3  g34(.a(x20), .b(new_n79_), .c(new_n78_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(new_n81_));
  inv1   g36(.a(x01), .O(new_n82_));
  nand2  g37(.a(x02), .b(new_n82_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  inv1   g39(.a(x13), .O(new_n85_));
  inv1   g40(.a(x14), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n85_), .c(x12), .d(new_n66_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n84_), .c(new_n81_), .O(new_n89_));
  nor2   g44(.a(x20), .b(x19), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(x18), .c(x01), .O(new_n91_));
  inv1   g46(.a(x00), .O(new_n92_));
  inv1   g47(.a(x21), .O(new_n93_));
  inv1   g48(.a(x22), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  aoi21  g50(.a(new_n91_), .b(new_n89_), .c(new_n95_), .O(z09));
  nor2   g51(.a(x20), .b(new_n82_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n54_), .c(x22), .d(x21), .O(new_n98_));
  nand3  g53(.a(new_n85_), .b(x12), .c(new_n66_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(new_n83_), .O(new_n100_));
  nor2   g55(.a(new_n78_), .b(x14), .O(new_n101_));
  nand4  g56(.a(new_n94_), .b(new_n93_), .c(x20), .d(x16), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n98_), .c(x00), .O(z10));
  nand4  g60(.a(new_n90_), .b(x21), .c(x18), .d(x01), .O(new_n106_));
  nand3  g61(.a(new_n93_), .b(x20), .c(new_n79_), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n101_), .c(new_n100_), .O(new_n109_));
  nand2  g64(.a(new_n94_), .b(new_n92_), .O(new_n110_));
  aoi21  g65(.a(new_n109_), .b(new_n106_), .c(new_n110_), .O(z11));
  inv1   g66(.a(x10), .O(new_n112_));
  nor2   g67(.a(new_n112_), .b(x02), .O(new_n113_));
  nor2   g68(.a(x10), .b(new_n82_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n113_), .c(x00), .O(new_n115_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n116_));
  nand3  g71(.a(new_n51_), .b(x17), .c(new_n46_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n47_), .O(new_n119_));
  inv1   g74(.a(x24), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x09), .O(new_n121_));
  aoi21  g76(.a(new_n119_), .b(new_n115_), .c(new_n121_), .O(z12));
  nor3   g77(.a(new_n48_), .b(x19), .c(new_n49_), .O(z13));
  nand4  g78(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n58_), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(new_n48_), .O(z14));
  oai21  g80(.a(new_n114_), .b(new_n46_), .c(x00), .O(new_n126_));
  nand3  g81(.a(x19), .b(new_n46_), .c(new_n82_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n126_), .O(z15));
  nor2   g83(.a(new_n82_), .b(x00), .O(z16));
  inv1   g84(.a(new_n75_), .O(z17));
  zero   g85(.O(z00));
  zero   g86(.O(z01));
endmodule


