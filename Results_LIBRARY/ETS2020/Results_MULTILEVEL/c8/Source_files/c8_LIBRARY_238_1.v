// Benchmark "FAU" written by ABC on Fri Jul 24 00:50:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x27), .O(new_n48_));
  inv1   g01(.a(x20), .O(new_n49_));
  nand2  g02(.a(new_n48_), .b(new_n49_), .O(new_n50_));
  oai21  g03(.a(new_n48_), .b(x09), .c(new_n50_), .O(z01));
  inv1   g04(.a(x11), .O(new_n53_));
  nand2  g05(.a(x27), .b(new_n53_), .O(new_n54_));
  oai21  g06(.a(x27), .b(x22), .c(new_n54_), .O(z03));
  inv1   g07(.a(x12), .O(new_n56_));
  nand2  g08(.a(x27), .b(new_n56_), .O(new_n57_));
  oai21  g09(.a(x27), .b(x23), .c(new_n57_), .O(z04));
  inv1   g10(.a(x24), .O(new_n59_));
  nand2  g11(.a(new_n48_), .b(new_n59_), .O(new_n60_));
  oai21  g12(.a(new_n48_), .b(x13), .c(new_n60_), .O(z05));
  inv1   g13(.a(x15), .O(new_n63_));
  nand2  g14(.a(x27), .b(new_n63_), .O(new_n64_));
  oai21  g15(.a(x27), .b(x26), .c(new_n64_), .O(z07));
  inv1   g16(.a(x17), .O(new_n67_));
  inv1   g17(.a(x19), .O(new_n68_));
  nor2   g18(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nor2   g19(.a(x19), .b(x17), .O(new_n70_));
  oai21  g20(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  inv1   g21(.a(x08), .O(new_n72_));
  nor2   g22(.a(x18), .b(new_n72_), .O(new_n73_));
  aoi21  g23(.a(x18), .b(x00), .c(new_n73_), .O(new_n74_));
  oai21  g24(.a(new_n74_), .b(x16), .c(new_n71_), .O(z09));
  inv1   g25(.a(x09), .O(new_n76_));
  nor2   g26(.a(x18), .b(new_n76_), .O(new_n77_));
  aoi21  g27(.a(x18), .b(x01), .c(new_n77_), .O(new_n78_));
  nor2   g28(.a(new_n70_), .b(new_n49_), .O(new_n79_));
  nor3   g29(.a(x20), .b(x19), .c(x17), .O(new_n80_));
  oai21  g30(.a(new_n80_), .b(new_n79_), .c(x16), .O(new_n81_));
  oai21  g31(.a(new_n78_), .b(x16), .c(new_n81_), .O(z10));
  inv1   g32(.a(x16), .O(new_n84_));
  nand2  g33(.a(x18), .b(x03), .O(new_n85_));
  oai21  g34(.a(x18), .b(new_n53_), .c(new_n85_), .O(new_n86_));
  nand2  g35(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nor2   g36(.a(x21), .b(x20), .O(new_n88_));
  nand2  g37(.a(new_n88_), .b(new_n70_), .O(new_n89_));
  nor3   g38(.a(x22), .b(x21), .c(x20), .O(new_n90_));
  aoi22  g39(.a(new_n90_), .b(new_n70_), .c(new_n89_), .d(x22), .O(new_n91_));
  oai21  g40(.a(new_n91_), .b(new_n84_), .c(new_n87_), .O(z12));
  nand2  g41(.a(x18), .b(x04), .O(new_n93_));
  oai21  g42(.a(x18), .b(new_n56_), .c(new_n93_), .O(new_n94_));
  nand2  g43(.a(new_n94_), .b(new_n84_), .O(new_n95_));
  nor2   g44(.a(x22), .b(x21), .O(new_n96_));
  nand3  g45(.a(new_n96_), .b(new_n70_), .c(new_n49_), .O(new_n97_));
  nor3   g46(.a(x23), .b(x22), .c(x21), .O(new_n98_));
  aoi22  g47(.a(new_n98_), .b(new_n80_), .c(new_n97_), .d(x23), .O(new_n99_));
  oai21  g48(.a(new_n99_), .b(new_n84_), .c(new_n95_), .O(z13));
  inv1   g49(.a(x13), .O(new_n101_));
  nand2  g50(.a(x18), .b(x05), .O(new_n102_));
  oai21  g51(.a(x18), .b(new_n101_), .c(new_n102_), .O(new_n103_));
  nand2  g52(.a(new_n103_), .b(new_n84_), .O(new_n104_));
  aoi21  g53(.a(new_n98_), .b(new_n80_), .c(new_n59_), .O(new_n105_));
  nor2   g54(.a(x20), .b(x19), .O(new_n106_));
  nor2   g55(.a(x24), .b(x23), .O(new_n107_));
  nand4  g56(.a(new_n107_), .b(new_n96_), .c(new_n106_), .d(new_n67_), .O(new_n108_));
  inv1   g57(.a(new_n108_), .O(new_n109_));
  oai21  g58(.a(new_n109_), .b(new_n105_), .c(x16), .O(new_n110_));
  nand2  g59(.a(new_n110_), .b(new_n104_), .O(z14));
  inv1   g60(.a(x14), .O(new_n112_));
  nand2  g61(.a(x18), .b(x06), .O(new_n113_));
  oai21  g62(.a(x18), .b(new_n112_), .c(new_n113_), .O(new_n114_));
  nand2  g63(.a(new_n114_), .b(new_n84_), .O(new_n115_));
  nand2  g64(.a(new_n108_), .b(x25), .O(new_n116_));
  nor2   g65(.a(x23), .b(x22), .O(new_n117_));
  nor2   g66(.a(x25), .b(x24), .O(new_n118_));
  nand4  g67(.a(new_n118_), .b(new_n117_), .c(new_n88_), .d(new_n70_), .O(new_n119_));
  nand2  g68(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand2  g69(.a(new_n120_), .b(x16), .O(new_n121_));
  nand2  g70(.a(new_n121_), .b(new_n115_), .O(z15));
  nand2  g71(.a(x18), .b(x07), .O(new_n123_));
  oai21  g72(.a(x18), .b(new_n63_), .c(new_n123_), .O(new_n124_));
  nand2  g73(.a(new_n124_), .b(new_n84_), .O(new_n125_));
  nand2  g74(.a(new_n119_), .b(x26), .O(new_n126_));
  nor3   g75(.a(x26), .b(x25), .c(x24), .O(new_n127_));
  nand4  g76(.a(new_n127_), .b(new_n117_), .c(new_n88_), .d(new_n70_), .O(new_n128_));
  nand2  g77(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand2  g78(.a(new_n129_), .b(x16), .O(new_n130_));
  nand2  g79(.a(new_n130_), .b(new_n125_), .O(z16));
  nor2   g80(.a(new_n68_), .b(x17), .O(new_n132_));
  nand4  g81(.a(new_n132_), .b(new_n127_), .c(new_n117_), .d(new_n88_), .O(new_n133_));
  nand2  g82(.a(x27), .b(x17), .O(new_n134_));
  aoi21  g83(.a(new_n134_), .b(new_n133_), .c(new_n84_), .O(z17));
  zero   g84(.O(z00));
  zero   g85(.O(z02));
  zero   g86(.O(z06));
  zero   g87(.O(z08));
  zero   g88(.O(z11));
endmodule


