// Benchmark "FAU" written by ABC on Fri Jul 24 00:50:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x27), .O(new_n49_));
  inv1   g01(.a(x21), .O(new_n50_));
  nand2  g02(.a(new_n49_), .b(new_n50_), .O(new_n51_));
  oai21  g03(.a(new_n49_), .b(x10), .c(new_n51_), .O(z02));
  inv1   g04(.a(x22), .O(new_n53_));
  nand2  g05(.a(new_n49_), .b(new_n53_), .O(new_n54_));
  oai21  g06(.a(new_n49_), .b(x11), .c(new_n54_), .O(z03));
  inv1   g07(.a(x15), .O(new_n59_));
  nand2  g08(.a(x27), .b(new_n59_), .O(new_n60_));
  oai21  g09(.a(x27), .b(x26), .c(new_n60_), .O(z07));
  and2   g10(.a(x19), .b(x17), .O(new_n62_));
  nor2   g11(.a(x19), .b(x17), .O(new_n63_));
  oai21  g12(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g13(.a(x08), .O(new_n65_));
  nor2   g14(.a(x18), .b(new_n65_), .O(new_n66_));
  aoi21  g15(.a(x18), .b(x00), .c(new_n66_), .O(new_n67_));
  oai21  g16(.a(new_n67_), .b(x16), .c(new_n64_), .O(z09));
  inv1   g17(.a(x09), .O(new_n69_));
  nor2   g18(.a(x18), .b(new_n69_), .O(new_n70_));
  aoi21  g19(.a(x18), .b(x01), .c(new_n70_), .O(new_n71_));
  inv1   g20(.a(x20), .O(new_n72_));
  nor2   g21(.a(new_n63_), .b(new_n72_), .O(new_n73_));
  nor3   g22(.a(x20), .b(x19), .c(x17), .O(new_n74_));
  oai21  g23(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  oai21  g24(.a(new_n71_), .b(x16), .c(new_n75_), .O(z10));
  inv1   g25(.a(x16), .O(new_n77_));
  inv1   g26(.a(x10), .O(new_n78_));
  nand2  g27(.a(x18), .b(x02), .O(new_n79_));
  oai21  g28(.a(x18), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nand2  g29(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nor2   g30(.a(new_n74_), .b(new_n50_), .O(new_n82_));
  nor2   g31(.a(x21), .b(x20), .O(new_n83_));
  nand2  g32(.a(new_n83_), .b(new_n63_), .O(new_n84_));
  inv1   g33(.a(new_n84_), .O(new_n85_));
  oai21  g34(.a(new_n85_), .b(new_n82_), .c(x16), .O(new_n86_));
  nand2  g35(.a(new_n86_), .b(new_n81_), .O(z11));
  inv1   g36(.a(x11), .O(new_n88_));
  nand2  g37(.a(x18), .b(x03), .O(new_n89_));
  oai21  g38(.a(x18), .b(new_n88_), .c(new_n89_), .O(new_n90_));
  nand2  g39(.a(new_n90_), .b(new_n77_), .O(new_n91_));
  nor3   g40(.a(x22), .b(x21), .c(x20), .O(new_n92_));
  aoi22  g41(.a(new_n92_), .b(new_n63_), .c(new_n84_), .d(x22), .O(new_n93_));
  oai21  g42(.a(new_n93_), .b(new_n77_), .c(new_n91_), .O(z12));
  inv1   g43(.a(x12), .O(new_n95_));
  nand2  g44(.a(x18), .b(x04), .O(new_n96_));
  oai21  g45(.a(x18), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  nand2  g46(.a(new_n97_), .b(new_n77_), .O(new_n98_));
  nand4  g47(.a(new_n63_), .b(new_n53_), .c(new_n50_), .d(new_n72_), .O(new_n99_));
  nor3   g48(.a(x23), .b(x22), .c(x21), .O(new_n100_));
  aoi22  g49(.a(new_n100_), .b(new_n74_), .c(new_n99_), .d(x23), .O(new_n101_));
  oai21  g50(.a(new_n101_), .b(new_n77_), .c(new_n98_), .O(z13));
  nand2  g51(.a(x18), .b(x07), .O(new_n105_));
  oai21  g52(.a(x18), .b(new_n59_), .c(new_n105_), .O(new_n106_));
  nand2  g53(.a(new_n106_), .b(new_n77_), .O(new_n107_));
  nor2   g54(.a(x23), .b(x22), .O(new_n108_));
  nor2   g55(.a(x25), .b(x24), .O(new_n109_));
  nand4  g56(.a(new_n109_), .b(new_n108_), .c(new_n83_), .d(new_n63_), .O(new_n110_));
  nand2  g57(.a(new_n110_), .b(x26), .O(new_n111_));
  nor3   g58(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nand4  g59(.a(new_n112_), .b(new_n108_), .c(new_n83_), .d(new_n63_), .O(new_n113_));
  nand2  g60(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g61(.a(new_n114_), .b(x16), .O(new_n115_));
  nand2  g62(.a(new_n115_), .b(new_n107_), .O(z16));
  inv1   g63(.a(x19), .O(new_n117_));
  nor2   g64(.a(new_n117_), .b(x17), .O(new_n118_));
  nand4  g65(.a(new_n118_), .b(new_n112_), .c(new_n108_), .d(new_n83_), .O(new_n119_));
  nand2  g66(.a(x27), .b(x17), .O(new_n120_));
  aoi21  g67(.a(new_n120_), .b(new_n119_), .c(new_n77_), .O(z17));
  zero   g68(.O(z00));
  zero   g69(.O(z01));
  zero   g70(.O(z04));
  zero   g71(.O(z05));
  zero   g72(.O(z06));
  zero   g73(.O(z14));
  zero   g74(.O(z15));
  buf    g75(.a(x27), .O(z08));
endmodule


