// Benchmark "FAU" written by ABC on Fri Jul 24 21:55:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g06(.a(x04), .b(x02), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n30_), .c(x03), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n27_), .c(x00), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  nand2  g11(.a(new_n31_), .b(new_n35_), .O(new_n36_));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  nand4  g13(.a(new_n37_), .b(new_n36_), .c(new_n30_), .d(new_n34_), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(z1));
  aoi22  g15(.a(x12), .b(new_n34_), .c(x11), .d(x07), .O(z3));
  nand2  g16(.a(x08), .b(x00), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(new_n43_));
  inv1   g18(.a(x09), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x07), .O(new_n45_));
  aoi22  g20(.a(new_n45_), .b(new_n43_), .c(x11), .d(x07), .O(z4));
  inv1   g21(.a(x12), .O(new_n47_));
  nor2   g22(.a(x10), .b(x07), .O(new_n48_));
  nand4  g23(.a(new_n48_), .b(x13), .c(new_n47_), .d(new_n34_), .O(z5));
  nor2   g24(.a(x02), .b(x01), .O(new_n50_));
  nand2  g25(.a(new_n44_), .b(x03), .O(new_n51_));
  nand2  g26(.a(x09), .b(new_n35_), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand4  g28(.a(x14), .b(x02), .c(x01), .d(new_n34_), .O(new_n54_));
  inv1   g29(.a(x07), .O(new_n55_));
  inv1   g30(.a(x10), .O(new_n56_));
  nand3  g31(.a(new_n42_), .b(new_n56_), .c(new_n55_), .O(new_n57_));
  inv1   g32(.a(new_n57_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n54_), .c(new_n53_), .O(new_n59_));
  inv1   g34(.a(new_n59_), .O(z6));
  inv1   g35(.a(x08), .O(new_n61_));
  aoi21  g36(.a(new_n50_), .b(x03), .c(new_n61_), .O(new_n62_));
  nand3  g37(.a(new_n50_), .b(new_n44_), .c(x03), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  oai21  g39(.a(new_n64_), .b(new_n62_), .c(new_n48_), .O(z7));
  nand2  g40(.a(new_n50_), .b(x03), .O(new_n66_));
  nand2  g41(.a(new_n47_), .b(x09), .O(new_n67_));
  nand2  g42(.a(new_n44_), .b(x00), .O(new_n68_));
  oai21  g43(.a(new_n67_), .b(new_n66_), .c(new_n68_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n48_), .O(z8));
  zero   g45(.O(z2));
endmodule


