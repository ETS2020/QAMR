// Benchmark "FAU" written by ABC on Fri Jul 24 21:56:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_;
  inv1   g00(.a(x27), .O(new_n49_));
  inv1   g01(.a(x10), .O(new_n50_));
  nor2   g02(.a(new_n49_), .b(new_n50_), .O(new_n51_));
  aoi21  g03(.a(new_n49_), .b(x21), .c(new_n51_), .O(z02));
  inv1   g04(.a(x11), .O(new_n53_));
  nor2   g05(.a(new_n49_), .b(new_n53_), .O(new_n54_));
  aoi21  g06(.a(new_n49_), .b(x22), .c(new_n54_), .O(z03));
  inv1   g07(.a(x12), .O(new_n56_));
  nor2   g08(.a(new_n49_), .b(new_n56_), .O(new_n57_));
  aoi21  g09(.a(new_n49_), .b(x23), .c(new_n57_), .O(z04));
  inv1   g10(.a(x13), .O(new_n59_));
  nor2   g11(.a(new_n49_), .b(new_n59_), .O(new_n60_));
  aoi21  g12(.a(new_n49_), .b(x24), .c(new_n60_), .O(z05));
  inv1   g13(.a(x15), .O(new_n63_));
  nor2   g14(.a(new_n49_), .b(new_n63_), .O(new_n64_));
  aoi21  g15(.a(new_n49_), .b(x26), .c(new_n64_), .O(z07));
  inv1   g16(.a(x16), .O(new_n67_));
  inv1   g17(.a(x20), .O(new_n68_));
  nor2   g18(.a(x19), .b(x17), .O(new_n69_));
  xor2a  g19(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g20(.a(x18), .O(new_n71_));
  nor2   g21(.a(new_n71_), .b(x01), .O(new_n72_));
  oai21  g22(.a(x18), .b(x09), .c(new_n67_), .O(new_n73_));
  oai22  g23(.a(new_n73_), .b(new_n72_), .c(new_n70_), .d(new_n67_), .O(z10));
  inv1   g24(.a(x21), .O(new_n75_));
  aoi21  g25(.a(new_n69_), .b(new_n68_), .c(new_n75_), .O(new_n76_));
  nor2   g26(.a(x21), .b(x20), .O(new_n77_));
  nand2  g27(.a(new_n77_), .b(new_n69_), .O(new_n78_));
  inv1   g28(.a(new_n78_), .O(new_n79_));
  oai21  g29(.a(new_n79_), .b(new_n76_), .c(x16), .O(new_n80_));
  aoi21  g30(.a(new_n71_), .b(new_n50_), .c(x16), .O(new_n81_));
  oai21  g31(.a(new_n71_), .b(x02), .c(new_n81_), .O(new_n82_));
  nand2  g32(.a(new_n82_), .b(new_n80_), .O(z11));
  xor2a  g33(.a(new_n78_), .b(x22), .O(new_n84_));
  aoi21  g34(.a(new_n71_), .b(new_n53_), .c(x16), .O(new_n85_));
  oai21  g35(.a(new_n71_), .b(x03), .c(new_n85_), .O(new_n86_));
  oai21  g36(.a(new_n84_), .b(new_n67_), .c(new_n86_), .O(z12));
  inv1   g37(.a(x22), .O(new_n88_));
  nand3  g38(.a(new_n77_), .b(new_n69_), .c(new_n88_), .O(new_n89_));
  nand2  g39(.a(new_n89_), .b(x23), .O(new_n90_));
  nor2   g40(.a(x23), .b(x22), .O(new_n91_));
  nand3  g41(.a(new_n91_), .b(new_n77_), .c(new_n69_), .O(new_n92_));
  nand2  g42(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g43(.a(new_n93_), .b(x16), .O(new_n94_));
  aoi21  g44(.a(new_n71_), .b(new_n56_), .c(x16), .O(new_n95_));
  oai21  g45(.a(new_n71_), .b(x04), .c(new_n95_), .O(new_n96_));
  nand2  g46(.a(new_n96_), .b(new_n94_), .O(z13));
  nand2  g47(.a(new_n92_), .b(x24), .O(new_n98_));
  nor2   g48(.a(x24), .b(x21), .O(new_n99_));
  nand4  g49(.a(new_n99_), .b(new_n91_), .c(new_n69_), .d(new_n68_), .O(new_n100_));
  nand2  g50(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g51(.a(new_n101_), .b(x16), .O(new_n102_));
  aoi21  g52(.a(new_n71_), .b(new_n59_), .c(x16), .O(new_n103_));
  oai21  g53(.a(new_n71_), .b(x05), .c(new_n103_), .O(new_n104_));
  nand2  g54(.a(new_n104_), .b(new_n102_), .O(z14));
  nand2  g55(.a(new_n100_), .b(x25), .O(new_n106_));
  nor2   g56(.a(x25), .b(x24), .O(new_n107_));
  nand4  g57(.a(new_n107_), .b(new_n91_), .c(new_n77_), .d(new_n69_), .O(new_n108_));
  nand2  g58(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand2  g59(.a(new_n109_), .b(x16), .O(new_n110_));
  inv1   g60(.a(x14), .O(new_n111_));
  aoi21  g61(.a(new_n71_), .b(new_n111_), .c(x16), .O(new_n112_));
  oai21  g62(.a(new_n71_), .b(x06), .c(new_n112_), .O(new_n113_));
  nand2  g63(.a(new_n113_), .b(new_n110_), .O(z15));
  or2    g64(.a(new_n108_), .b(x26), .O(new_n115_));
  aoi21  g65(.a(new_n108_), .b(x26), .c(new_n67_), .O(new_n116_));
  oai21  g66(.a(x18), .b(new_n63_), .c(new_n67_), .O(new_n117_));
  aoi21  g67(.a(x18), .b(x07), .c(new_n117_), .O(new_n118_));
  aoi21  g68(.a(new_n116_), .b(new_n115_), .c(new_n118_), .O(z16));
  zero   g69(.O(z00));
  zero   g70(.O(z01));
  zero   g71(.O(z06));
  zero   g72(.O(z09));
  zero   g73(.O(z17));
  buf    g74(.a(x27), .O(z08));
endmodule


