// Benchmark "FAU" written by ABC on Fri Jul 24 21:59:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x16), .O(new_n52_));
  xor2a  g01(.a(x19), .b(x17), .O(new_n53_));
  inv1   g02(.a(x15), .O(new_n54_));
  aoi21  g03(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g04(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g05(.a(x20), .O(new_n57_));
  nor2   g06(.a(x19), .b(x17), .O(new_n58_));
  nor2   g07(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g08(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g09(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g10(.a(x14), .O(new_n62_));
  aoi21  g11(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g12(.a(new_n63_), .b(new_n61_), .O(z01));
  nand2  g13(.a(x22), .b(x21), .O(new_n66_));
  nor2   g14(.a(x22), .b(x21), .O(new_n67_));
  nor2   g15(.a(x20), .b(x17), .O(new_n68_));
  nand2  g16(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g17(.a(new_n69_), .b(new_n66_), .c(x19), .O(new_n70_));
  inv1   g18(.a(x22), .O(new_n71_));
  nor2   g19(.a(new_n60_), .b(new_n71_), .O(new_n72_));
  oai21  g20(.a(new_n72_), .b(new_n70_), .c(x16), .O(new_n73_));
  inv1   g21(.a(x12), .O(new_n74_));
  aoi21  g22(.a(new_n52_), .b(new_n74_), .c(x18), .O(new_n75_));
  nand2  g23(.a(new_n75_), .b(new_n73_), .O(z03));
  inv1   g24(.a(x23), .O(new_n77_));
  aoi21  g25(.a(new_n67_), .b(new_n60_), .c(new_n77_), .O(new_n78_));
  nand4  g26(.a(new_n67_), .b(new_n58_), .c(new_n77_), .d(new_n57_), .O(new_n79_));
  inv1   g27(.a(new_n79_), .O(new_n80_));
  oai21  g28(.a(new_n80_), .b(new_n78_), .c(x16), .O(new_n81_));
  inv1   g29(.a(x11), .O(new_n82_));
  aoi21  g30(.a(new_n52_), .b(new_n82_), .c(x18), .O(new_n83_));
  nand2  g31(.a(new_n83_), .b(new_n81_), .O(z04));
  xor2a  g32(.a(new_n79_), .b(x24), .O(new_n85_));
  inv1   g33(.a(x10), .O(new_n86_));
  aoi21  g34(.a(new_n52_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g35(.a(new_n85_), .b(new_n52_), .c(new_n87_), .O(z05));
  inv1   g36(.a(x24), .O(new_n89_));
  nand2  g37(.a(new_n80_), .b(new_n89_), .O(new_n90_));
  nor3   g38(.a(x23), .b(x22), .c(x21), .O(new_n91_));
  nor2   g39(.a(x25), .b(x24), .O(new_n92_));
  nand3  g40(.a(new_n92_), .b(new_n91_), .c(new_n60_), .O(new_n93_));
  inv1   g41(.a(new_n93_), .O(new_n94_));
  aoi21  g42(.a(new_n90_), .b(x25), .c(new_n94_), .O(new_n95_));
  inv1   g43(.a(x09), .O(new_n96_));
  aoi21  g44(.a(new_n52_), .b(new_n96_), .c(x18), .O(new_n97_));
  oai21  g45(.a(new_n95_), .b(new_n52_), .c(new_n97_), .O(z06));
  xor2a  g46(.a(new_n93_), .b(x26), .O(new_n99_));
  inv1   g47(.a(x08), .O(new_n100_));
  aoi21  g48(.a(new_n52_), .b(new_n100_), .c(x18), .O(new_n101_));
  oai21  g49(.a(new_n99_), .b(new_n52_), .c(new_n101_), .O(z07));
  inv1   g50(.a(x26), .O(new_n103_));
  nand4  g51(.a(new_n92_), .b(new_n91_), .c(new_n60_), .d(new_n103_), .O(new_n104_));
  nor2   g52(.a(x27), .b(x26), .O(new_n105_));
  aoi22  g53(.a(new_n105_), .b(new_n94_), .c(new_n104_), .d(x27), .O(new_n106_));
  inv1   g54(.a(x07), .O(new_n107_));
  aoi21  g55(.a(new_n52_), .b(new_n107_), .c(x18), .O(new_n108_));
  oai21  g56(.a(new_n106_), .b(new_n52_), .c(new_n108_), .O(z08));
  nor3   g57(.a(x28), .b(x27), .c(x26), .O(new_n111_));
  nand3  g58(.a(new_n111_), .b(new_n94_), .c(x29), .O(new_n112_));
  inv1   g59(.a(x29), .O(new_n113_));
  inv1   g60(.a(new_n111_), .O(new_n114_));
  oai21  g61(.a(new_n114_), .b(new_n93_), .c(new_n113_), .O(new_n115_));
  nand3  g62(.a(new_n115_), .b(new_n112_), .c(x16), .O(new_n116_));
  inv1   g63(.a(x05), .O(new_n117_));
  aoi21  g64(.a(new_n52_), .b(new_n117_), .c(x18), .O(new_n118_));
  nand2  g65(.a(new_n118_), .b(new_n116_), .O(z10));
  nor2   g66(.a(x30), .b(x29), .O(new_n121_));
  nand2  g67(.a(new_n121_), .b(new_n111_), .O(new_n122_));
  oai21  g68(.a(new_n122_), .b(new_n93_), .c(x31), .O(new_n123_));
  inv1   g69(.a(x28), .O(new_n124_));
  inv1   g70(.a(x31), .O(new_n125_));
  nand4  g71(.a(new_n121_), .b(new_n105_), .c(new_n125_), .d(new_n124_), .O(new_n126_));
  inv1   g72(.a(new_n126_), .O(new_n127_));
  nand3  g73(.a(new_n127_), .b(new_n92_), .c(new_n80_), .O(new_n128_));
  nand2  g74(.a(new_n128_), .b(new_n123_), .O(new_n129_));
  nand2  g75(.a(new_n129_), .b(x16), .O(new_n130_));
  inv1   g76(.a(x03), .O(new_n131_));
  aoi21  g77(.a(new_n52_), .b(new_n131_), .c(x18), .O(new_n132_));
  nand2  g78(.a(new_n132_), .b(new_n130_), .O(z12));
  nand3  g79(.a(new_n127_), .b(new_n94_), .c(x32), .O(new_n134_));
  inv1   g80(.a(x32), .O(new_n135_));
  nand2  g81(.a(new_n128_), .b(new_n135_), .O(new_n136_));
  nand3  g82(.a(new_n136_), .b(new_n134_), .c(x16), .O(new_n137_));
  inv1   g83(.a(x02), .O(new_n138_));
  aoi21  g84(.a(new_n52_), .b(new_n138_), .c(x18), .O(new_n139_));
  nand2  g85(.a(new_n139_), .b(new_n137_), .O(z13));
  zero   g86(.O(z02));
  zero   g87(.O(z09));
  zero   g88(.O(z11));
  zero   g89(.O(z14));
  zero   g90(.O(z15));
endmodule


