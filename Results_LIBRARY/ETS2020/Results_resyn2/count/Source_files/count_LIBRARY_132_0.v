// Benchmark "FAU" written by ABC on Fri Jul 24 21:59:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g009(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g010(.a(x14), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n61_), .O(z01));
  xnor2a g013(.a(new_n60_), .b(x21), .O(new_n65_));
  inv1   g014(.a(x13), .O(new_n66_));
  aoi21  g015(.a(new_n52_), .b(new_n66_), .c(x18), .O(new_n67_));
  oai21  g016(.a(new_n65_), .b(new_n52_), .c(new_n67_), .O(z02));
  nand2  g017(.a(x22), .b(x21), .O(new_n69_));
  nor2   g018(.a(x22), .b(x21), .O(new_n70_));
  nor2   g019(.a(x20), .b(x17), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g021(.a(new_n72_), .b(new_n69_), .c(x19), .O(new_n73_));
  inv1   g022(.a(x22), .O(new_n74_));
  nor2   g023(.a(new_n60_), .b(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(new_n73_), .c(x16), .O(new_n76_));
  inv1   g025(.a(x12), .O(new_n77_));
  aoi21  g026(.a(new_n52_), .b(new_n77_), .c(x18), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n76_), .O(z03));
  nand3  g028(.a(new_n70_), .b(new_n58_), .c(new_n57_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(x23), .O(new_n81_));
  inv1   g030(.a(x23), .O(new_n82_));
  nand3  g031(.a(new_n70_), .b(new_n60_), .c(new_n82_), .O(new_n83_));
  and2   g032(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  inv1   g033(.a(x11), .O(new_n85_));
  aoi21  g034(.a(new_n52_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n52_), .c(new_n86_), .O(z04));
  and2   g036(.a(new_n83_), .b(x24), .O(new_n88_));
  nor2   g037(.a(x24), .b(x23), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n70_), .c(new_n60_), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n88_), .c(x16), .O(new_n92_));
  inv1   g041(.a(x10), .O(new_n93_));
  aoi21  g042(.a(new_n52_), .b(new_n93_), .c(x18), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n92_), .O(z05));
  inv1   g044(.a(x25), .O(new_n96_));
  nand2  g045(.a(new_n89_), .b(new_n96_), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n80_), .O(new_n98_));
  aoi21  g047(.a(new_n90_), .b(x25), .c(new_n98_), .O(new_n99_));
  inv1   g048(.a(x09), .O(new_n100_));
  aoi21  g049(.a(new_n52_), .b(new_n100_), .c(x18), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n52_), .c(new_n101_), .O(z06));
  inv1   g051(.a(x26), .O(new_n103_));
  nor2   g052(.a(new_n98_), .b(new_n103_), .O(new_n104_));
  nor3   g053(.a(x25), .b(x24), .c(x23), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n70_), .c(new_n60_), .d(new_n103_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(new_n104_), .c(x16), .O(new_n108_));
  inv1   g057(.a(x08), .O(new_n109_));
  aoi21  g058(.a(new_n52_), .b(new_n109_), .c(x18), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n108_), .O(z07));
  nor2   g060(.a(x27), .b(x26), .O(new_n112_));
  aoi22  g061(.a(new_n112_), .b(new_n98_), .c(new_n106_), .d(x27), .O(new_n113_));
  inv1   g062(.a(x07), .O(new_n114_));
  aoi21  g063(.a(new_n52_), .b(new_n114_), .c(x18), .O(new_n115_));
  oai21  g064(.a(new_n113_), .b(new_n52_), .c(new_n115_), .O(z08));
  nand2  g065(.a(new_n112_), .b(new_n98_), .O(new_n117_));
  nor3   g066(.a(x28), .b(x27), .c(x26), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n105_), .c(new_n70_), .d(new_n60_), .O(new_n119_));
  inv1   g068(.a(new_n119_), .O(new_n120_));
  aoi21  g069(.a(new_n117_), .b(x28), .c(new_n120_), .O(new_n121_));
  inv1   g070(.a(x06), .O(new_n122_));
  aoi21  g071(.a(new_n52_), .b(new_n122_), .c(x18), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n52_), .c(new_n123_), .O(z09));
  xor2a  g073(.a(new_n119_), .b(x29), .O(new_n125_));
  inv1   g074(.a(x05), .O(new_n126_));
  aoi21  g075(.a(new_n52_), .b(new_n126_), .c(x18), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n52_), .c(new_n127_), .O(z10));
  oai21  g077(.a(new_n119_), .b(x29), .c(x30), .O(new_n129_));
  nor2   g078(.a(x30), .b(x29), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n118_), .c(new_n98_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(x16), .O(new_n133_));
  inv1   g082(.a(x04), .O(new_n134_));
  aoi21  g083(.a(new_n52_), .b(new_n134_), .c(x18), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n133_), .O(z11));
  nand3  g085(.a(new_n130_), .b(new_n120_), .c(x31), .O(new_n137_));
  inv1   g086(.a(x31), .O(new_n138_));
  nand2  g087(.a(new_n131_), .b(new_n138_), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n137_), .c(x16), .O(new_n140_));
  inv1   g089(.a(x03), .O(new_n141_));
  aoi21  g090(.a(new_n52_), .b(new_n141_), .c(x18), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n140_), .O(z12));
  inv1   g092(.a(x32), .O(new_n145_));
  nor2   g093(.a(x28), .b(x27), .O(new_n146_));
  nand4  g094(.a(new_n130_), .b(new_n146_), .c(new_n145_), .d(new_n138_), .O(new_n147_));
  oai21  g095(.a(new_n147_), .b(new_n106_), .c(x33), .O(new_n148_));
  inv1   g096(.a(x33), .O(new_n149_));
  inv1   g097(.a(new_n147_), .O(new_n150_));
  nand4  g098(.a(new_n150_), .b(new_n98_), .c(new_n149_), .d(new_n103_), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(new_n148_), .O(new_n152_));
  nand2  g100(.a(new_n152_), .b(x16), .O(new_n153_));
  inv1   g101(.a(x01), .O(new_n154_));
  aoi21  g102(.a(new_n52_), .b(new_n154_), .c(x18), .O(new_n155_));
  nand2  g103(.a(new_n155_), .b(new_n153_), .O(z14));
  nand4  g104(.a(new_n150_), .b(new_n107_), .c(x34), .d(new_n149_), .O(new_n157_));
  inv1   g105(.a(x34), .O(new_n158_));
  nand2  g106(.a(new_n151_), .b(new_n158_), .O(new_n159_));
  nand3  g107(.a(new_n159_), .b(new_n157_), .c(x16), .O(new_n160_));
  inv1   g108(.a(x00), .O(new_n161_));
  aoi21  g109(.a(new_n52_), .b(new_n161_), .c(x18), .O(new_n162_));
  nand2  g110(.a(new_n162_), .b(new_n160_), .O(z15));
  zero   g111(.O(z13));
endmodule


