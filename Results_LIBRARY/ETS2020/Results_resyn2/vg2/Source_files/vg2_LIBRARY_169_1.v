// Benchmark "FAU" written by ABC on Fri Jul 24 21:16:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand4  g007(.a(new_n36_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  nor2   g012(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  nor2   g013(.a(x03), .b(x01), .O(new_n47_));
  inv1   g014(.a(new_n47_), .O(new_n48_));
  inv1   g015(.a(x06), .O(new_n49_));
  inv1   g016(.a(x11), .O(new_n50_));
  inv1   g017(.a(x14), .O(new_n51_));
  inv1   g018(.a(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(new_n48_), .c(new_n46_), .O(z0));
  nand3  g021(.a(x11), .b(x09), .c(x08), .O(new_n55_));
  nand4  g022(.a(x06), .b(x04), .c(x03), .d(x01), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  inv1   g024(.a(x15), .O(new_n58_));
  nand3  g025(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n59_));
  nand2  g026(.a(x13), .b(x05), .O(new_n60_));
  oai21  g027(.a(new_n60_), .b(new_n58_), .c(new_n59_), .O(new_n61_));
  nand4  g028(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n62_));
  nand3  g029(.a(x16), .b(x14), .c(x12), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g031(.a(new_n64_), .b(new_n61_), .c(new_n57_), .O(new_n65_));
  inv1   g032(.a(x17), .O(new_n66_));
  inv1   g033(.a(x22), .O(new_n67_));
  inv1   g034(.a(x23), .O(new_n68_));
  nand3  g035(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  inv1   g036(.a(x01), .O(new_n70_));
  inv1   g037(.a(x03), .O(new_n71_));
  inv1   g038(.a(x04), .O(new_n72_));
  inv1   g039(.a(x16), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nor2   g041(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  inv1   g042(.a(x19), .O(new_n76_));
  nand3  g043(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n77_));
  oai21  g044(.a(new_n60_), .b(new_n76_), .c(new_n77_), .O(new_n78_));
  inv1   g045(.a(x08), .O(new_n79_));
  inv1   g046(.a(x09), .O(new_n80_));
  inv1   g047(.a(x12), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(new_n53_), .O(new_n83_));
  nand3  g050(.a(new_n83_), .b(new_n78_), .c(new_n75_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n65_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n36_), .O(new_n86_));
  nor3   g053(.a(x23), .b(x22), .c(x17), .O(new_n87_));
  inv1   g054(.a(new_n74_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand4  g056(.a(x18), .b(x16), .c(x14), .d(x12), .O(new_n90_));
  nor2   g057(.a(new_n90_), .b(new_n62_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n57_), .O(new_n92_));
  nor2   g059(.a(x11), .b(x06), .O(new_n93_));
  nor2   g060(.a(x20), .b(x14), .O(new_n94_));
  nor2   g061(.a(x09), .b(x08), .O(new_n95_));
  inv1   g062(.a(x07), .O(new_n96_));
  nor2   g063(.a(x12), .b(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n95_), .c(new_n94_), .d(new_n93_), .O(new_n98_));
  oai21  g065(.a(new_n98_), .b(new_n89_), .c(new_n92_), .O(new_n99_));
  nor2   g066(.a(new_n42_), .b(new_n36_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand4  g068(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n102_));
  nor2   g069(.a(new_n102_), .b(new_n62_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n57_), .O(new_n104_));
  nor2   g071(.a(new_n76_), .b(x12), .O(new_n105_));
  nand4  g072(.a(new_n105_), .b(new_n95_), .c(new_n94_), .d(new_n93_), .O(new_n106_));
  oai21  g073(.a(new_n106_), .b(new_n89_), .c(new_n104_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n42_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n101_), .c(new_n86_), .O(z1));
  nand2  g076(.a(x24), .b(x18), .O(new_n110_));
  nand3  g077(.a(new_n36_), .b(x15), .c(x13), .O(new_n111_));
  aoi21  g078(.a(new_n111_), .b(new_n110_), .c(new_n34_), .O(new_n112_));
  nand4  g079(.a(new_n36_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n113_));
  nand3  g080(.a(x24), .b(x18), .c(x13), .O(new_n114_));
  nand2  g081(.a(new_n42_), .b(x15), .O(new_n115_));
  nand3  g082(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  nor2   g083(.a(new_n116_), .b(new_n112_), .O(new_n117_));
  nor2   g084(.a(new_n71_), .b(new_n70_), .O(new_n118_));
  nor2   g085(.a(new_n52_), .b(new_n51_), .O(new_n119_));
  nand4  g086(.a(new_n119_), .b(new_n118_), .c(x11), .d(x06), .O(new_n120_));
  nor2   g087(.a(new_n120_), .b(new_n117_), .O(z2));
  aoi21  g088(.a(new_n68_), .b(x04), .c(x17), .O(new_n123_));
  oai21  g089(.a(new_n123_), .b(x22), .c(new_n80_), .O(new_n124_));
  aoi21  g090(.a(new_n124_), .b(new_n73_), .c(x08), .O(new_n125_));
  aoi21  g091(.a(x23), .b(new_n72_), .c(new_n66_), .O(new_n126_));
  oai21  g092(.a(new_n126_), .b(new_n67_), .c(x09), .O(new_n127_));
  aoi21  g093(.a(new_n127_), .b(x16), .c(new_n79_), .O(new_n128_));
  oai22  g094(.a(new_n128_), .b(new_n117_), .c(new_n125_), .d(new_n46_), .O(z4));
  inv1   g095(.a(new_n46_), .O(z5));
  oai21  g096(.a(new_n52_), .b(x14), .c(new_n49_), .O(new_n131_));
  aoi21  g097(.a(new_n131_), .b(new_n50_), .c(x03), .O(new_n132_));
  oai21  g098(.a(x20), .b(new_n51_), .c(x06), .O(new_n133_));
  aoi21  g099(.a(new_n133_), .b(x11), .c(new_n71_), .O(new_n134_));
  oai22  g100(.a(new_n134_), .b(new_n117_), .c(new_n132_), .d(new_n46_), .O(z6));
  inv1   g101(.a(new_n117_), .O(z7));
  zero   g102(.O(z3));
endmodule


