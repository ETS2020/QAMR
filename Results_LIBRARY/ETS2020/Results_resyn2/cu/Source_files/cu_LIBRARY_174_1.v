// Benchmark "FAU" written by ABC on Fri Jul 24 21:26:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n63_,
    new_n64_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  oai21  g02(.a(x05), .b(new_n27_), .c(x04), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  oai21  g05(.a(new_n30_), .b(x02), .c(new_n29_), .O(new_n31_));
  nand3  g06(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(z00));
  inv1   g07(.a(z00), .O(z01));
  nor2   g08(.a(new_n30_), .b(x02), .O(new_n38_));
  inv1   g09(.a(x00), .O(new_n39_));
  inv1   g10(.a(x10), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x01), .O(new_n41_));
  inv1   g12(.a(x01), .O(new_n42_));
  inv1   g13(.a(x08), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(new_n41_), .c(new_n39_), .O(new_n45_));
  inv1   g16(.a(x11), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x01), .O(new_n47_));
  inv1   g18(.a(x09), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n42_), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(new_n47_), .c(x00), .O(new_n50_));
  inv1   g21(.a(x07), .O(new_n51_));
  inv1   g22(.a(x12), .O(new_n52_));
  nand3  g23(.a(x13), .b(new_n52_), .c(new_n51_), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(new_n50_), .c(new_n45_), .d(new_n38_), .O(new_n55_));
  inv1   g26(.a(x13), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n30_), .c(x02), .O(new_n57_));
  nand2  g28(.a(x04), .b(new_n26_), .O(new_n58_));
  aoi21  g29(.a(new_n57_), .b(new_n55_), .c(new_n58_), .O(z06));
  and2   g30(.a(x13), .b(x06), .O(z09));
  nand2  g31(.a(x05), .b(x02), .O(new_n63_));
  nand3  g32(.a(new_n63_), .b(x06), .c(new_n26_), .O(new_n64_));
  inv1   g33(.a(new_n64_), .O(z10));
  zero   g34(.O(z02));
  zero   g35(.O(z03));
  zero   g36(.O(z04));
  zero   g37(.O(z05));
  zero   g38(.O(z07));
  zero   g39(.O(z08));
endmodule


