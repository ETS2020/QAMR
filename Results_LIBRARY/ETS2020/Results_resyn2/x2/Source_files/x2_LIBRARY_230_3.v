// Benchmark "FAU" written by ABC on Fri Jul 24 21:06:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  nand2  g02(.a(new_n18_), .b(x7), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  oai21  g04(.a(x9), .b(x8), .c(new_n21_), .O(new_n22_));
  nand2  g05(.a(x9), .b(x8), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n20_), .O(z1));
  inv1   g07(.a(x8), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n21_), .O(new_n26_));
  nor2   g09(.a(new_n26_), .b(x9), .O(z2));
  nand3  g10(.a(new_n23_), .b(x7), .c(x6), .O(z4));
  inv1   g11(.a(x0), .O(new_n30_));
  inv1   g12(.a(x1), .O(new_n31_));
  oai21  g13(.a(new_n23_), .b(new_n21_), .c(new_n26_), .O(new_n32_));
  nand4  g14(.a(new_n32_), .b(x2), .c(new_n31_), .d(new_n30_), .O(new_n33_));
  inv1   g15(.a(x4), .O(new_n34_));
  nand3  g16(.a(x7), .b(new_n34_), .c(x3), .O(new_n35_));
  nand2  g17(.a(new_n35_), .b(x8), .O(new_n36_));
  nand2  g18(.a(new_n36_), .b(new_n18_), .O(new_n37_));
  inv1   g19(.a(x6), .O(new_n38_));
  nand3  g20(.a(x9), .b(x8), .c(x5), .O(new_n39_));
  inv1   g21(.a(new_n39_), .O(new_n40_));
  aoi21  g22(.a(new_n40_), .b(new_n21_), .c(new_n38_), .O(new_n41_));
  nand3  g23(.a(new_n41_), .b(new_n37_), .c(new_n33_), .O(z5));
  inv1   g24(.a(x2), .O(new_n43_));
  nand3  g25(.a(new_n43_), .b(new_n31_), .c(new_n30_), .O(new_n44_));
  aoi21  g26(.a(new_n44_), .b(x8), .c(new_n18_), .O(new_n45_));
  nand4  g27(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n46_));
  inv1   g28(.a(new_n46_), .O(new_n47_));
  oai21  g29(.a(new_n47_), .b(new_n45_), .c(x7), .O(new_n48_));
  oai21  g30(.a(x9), .b(x8), .c(new_n39_), .O(new_n49_));
  aoi21  g31(.a(new_n49_), .b(new_n21_), .c(new_n38_), .O(new_n50_));
  nand2  g32(.a(new_n50_), .b(new_n48_), .O(z6));
  zero   g33(.O(z3));
endmodule


