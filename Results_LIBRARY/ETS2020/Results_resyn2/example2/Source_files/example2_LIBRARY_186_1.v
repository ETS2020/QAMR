// Benchmark "FAU" written by ABC on Sat Jul 25 01:27:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n174_, new_n175_, new_n177_,
    new_n178_, new_n183_, new_n184_, new_n187_, new_n188_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n196_, new_n197_, new_n200_,
    new_n201_, new_n203_, new_n204_, new_n209_, new_n210_, new_n211_,
    new_n212_, new_n213_, new_n214_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n225_, new_n229_, new_n231_, new_n233_, new_n235_, new_n237_,
    new_n240_, new_n241_, new_n242_, new_n244_, new_n250_, new_n252_,
    new_n255_, new_n260_, new_n261_, new_n262_, new_n263_, new_n264_,
    new_n265_, new_n266_, new_n267_, new_n268_, new_n269_, new_n270_,
    new_n271_, new_n272_, new_n273_, new_n275_, new_n276_, new_n277_,
    new_n278_, new_n279_, new_n280_, new_n283_, new_n284_, new_n285_,
    new_n286_, new_n287_, new_n288_, new_n291_, new_n292_, new_n293_,
    new_n294_, new_n295_, new_n296_, new_n298_, new_n299_, new_n300_,
    new_n301_, new_n302_, new_n303_, new_n307_, new_n308_, new_n311_,
    new_n312_, new_n313_, new_n314_, new_n315_, new_n316_, new_n317_,
    new_n318_, new_n319_, new_n320_, new_n321_, new_n322_, new_n323_,
    new_n325_, new_n326_, new_n327_, new_n328_, new_n329_, new_n335_,
    new_n336_, new_n337_, new_n339_, new_n340_;
  nor2   g000(.a(x40), .b(x06), .O(new_n152_));
  inv1   g001(.a(x01), .O(new_n153_));
  nand2  g002(.a(x78), .b(x77), .O(new_n154_));
  oai21  g003(.a(new_n154_), .b(x79), .c(new_n153_), .O(new_n155_));
  inv1   g004(.a(x40), .O(new_n156_));
  nor2   g005(.a(x52), .b(new_n156_), .O(new_n157_));
  aoi21  g006(.a(new_n157_), .b(new_n155_), .c(new_n152_), .O(z00));
  inv1   g007(.a(x77), .O(new_n159_));
  inv1   g008(.a(x78), .O(new_n160_));
  nand2  g009(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand2  g010(.a(new_n161_), .b(new_n154_), .O(new_n162_));
  inv1   g011(.a(new_n162_), .O(new_n163_));
  aoi21  g012(.a(new_n163_), .b(x79), .c(x01), .O(z01));
  inv1   g013(.a(new_n155_), .O(z04));
  inv1   g014(.a(x65), .O(new_n168_));
  nor2   g015(.a(x40), .b(x23), .O(new_n169_));
  aoi21  g016(.a(new_n168_), .b(x40), .c(new_n169_), .O(z05));
  inv1   g017(.a(x64), .O(new_n171_));
  nor2   g018(.a(x40), .b(x24), .O(new_n172_));
  aoi21  g019(.a(new_n171_), .b(x40), .c(new_n172_), .O(z06));
  inv1   g020(.a(x63), .O(new_n174_));
  nor2   g021(.a(x40), .b(x25), .O(new_n175_));
  aoi21  g022(.a(new_n174_), .b(x40), .c(new_n175_), .O(z07));
  inv1   g023(.a(x62), .O(new_n177_));
  nor2   g024(.a(x40), .b(x26), .O(new_n178_));
  aoi21  g025(.a(new_n177_), .b(x40), .c(new_n178_), .O(z08));
  inv1   g026(.a(x58), .O(new_n183_));
  nor2   g027(.a(x40), .b(x30), .O(new_n184_));
  aoi21  g028(.a(new_n183_), .b(x40), .c(new_n184_), .O(z12));
  inv1   g029(.a(x51), .O(new_n187_));
  nor2   g030(.a(x40), .b(x32), .O(new_n188_));
  aoi21  g031(.a(new_n187_), .b(x40), .c(new_n188_), .O(z14));
  inv1   g032(.a(x50), .O(new_n190_));
  nor2   g033(.a(x40), .b(x33), .O(new_n191_));
  aoi21  g034(.a(new_n190_), .b(x40), .c(new_n191_), .O(z15));
  inv1   g035(.a(x49), .O(new_n193_));
  nor2   g036(.a(x40), .b(x34), .O(new_n194_));
  aoi21  g037(.a(new_n193_), .b(x40), .c(new_n194_), .O(z16));
  inv1   g038(.a(x48), .O(new_n196_));
  nor2   g039(.a(x40), .b(x35), .O(new_n197_));
  aoi21  g040(.a(new_n196_), .b(x40), .c(new_n197_), .O(z17));
  inv1   g041(.a(x46), .O(new_n200_));
  nor2   g042(.a(x40), .b(x37), .O(new_n201_));
  aoi21  g043(.a(new_n200_), .b(x40), .c(new_n201_), .O(z19));
  inv1   g044(.a(x45), .O(new_n203_));
  nor2   g045(.a(x40), .b(x38), .O(new_n204_));
  aoi21  g046(.a(new_n203_), .b(x40), .c(new_n204_), .O(z20));
  inv1   g047(.a(x79), .O(new_n209_));
  inv1   g048(.a(new_n154_), .O(new_n210_));
  nor2   g049(.a(new_n210_), .b(new_n209_), .O(new_n211_));
  inv1   g050(.a(x04), .O(new_n212_));
  inv1   g051(.a(x43), .O(new_n213_));
  nand4  g052(.a(new_n213_), .b(x05), .c(new_n212_), .d(new_n153_), .O(new_n214_));
  nor2   g053(.a(new_n214_), .b(new_n211_), .O(z24));
  inv1   g054(.a(x42), .O(new_n216_));
  xor2a  g055(.a(x84), .b(x82), .O(new_n217_));
  nand2  g056(.a(new_n217_), .b(x81), .O(new_n218_));
  inv1   g057(.a(x81), .O(new_n219_));
  xnor2a g058(.a(x84), .b(x82), .O(new_n220_));
  nand2  g059(.a(new_n220_), .b(new_n219_), .O(new_n221_));
  nand2  g060(.a(new_n221_), .b(new_n218_), .O(new_n222_));
  inv1   g061(.a(new_n222_), .O(new_n223_));
  nor4   g062(.a(new_n154_), .b(new_n209_), .c(x04), .d(x01), .O(new_n224_));
  nand4  g063(.a(new_n224_), .b(new_n223_), .c(new_n216_), .d(x05), .O(new_n225_));
  inv1   g064(.a(new_n225_), .O(z25));
  nand4  g065(.a(new_n224_), .b(new_n223_), .c(x46), .d(new_n216_), .O(new_n229_));
  inv1   g066(.a(new_n229_), .O(z28));
  nand4  g067(.a(new_n224_), .b(new_n223_), .c(x47), .d(new_n216_), .O(new_n231_));
  inv1   g068(.a(new_n231_), .O(z29));
  nand4  g069(.a(new_n224_), .b(new_n223_), .c(x48), .d(new_n216_), .O(new_n233_));
  inv1   g070(.a(new_n233_), .O(z30));
  nand4  g071(.a(new_n224_), .b(new_n223_), .c(x49), .d(new_n216_), .O(new_n235_));
  inv1   g072(.a(new_n235_), .O(z31));
  nand4  g073(.a(new_n224_), .b(new_n223_), .c(x50), .d(new_n216_), .O(new_n237_));
  inv1   g074(.a(new_n237_), .O(z32));
  nand2  g075(.a(x83), .b(x42), .O(new_n240_));
  xor2a  g076(.a(new_n240_), .b(new_n222_), .O(new_n241_));
  nand3  g077(.a(new_n241_), .b(new_n224_), .c(x52), .O(new_n242_));
  inv1   g078(.a(new_n242_), .O(z34));
  nand3  g079(.a(new_n241_), .b(new_n224_), .c(x53), .O(new_n244_));
  inv1   g080(.a(new_n244_), .O(z35));
  nand3  g081(.a(new_n241_), .b(new_n224_), .c(x58), .O(new_n250_));
  inv1   g082(.a(new_n250_), .O(z40));
  nand3  g083(.a(new_n241_), .b(new_n224_), .c(x59), .O(new_n252_));
  inv1   g084(.a(new_n252_), .O(z41));
  nand3  g085(.a(new_n241_), .b(new_n224_), .c(x61), .O(new_n255_));
  inv1   g086(.a(new_n255_), .O(z43));
  or2    g087(.a(x75), .b(x67), .O(new_n260_));
  xor2a  g088(.a(x84), .b(x81), .O(new_n261_));
  nor4   g089(.a(new_n261_), .b(new_n209_), .c(x78), .d(new_n159_), .O(new_n262_));
  nand2  g090(.a(new_n262_), .b(new_n260_), .O(new_n263_));
  nor2   g091(.a(new_n160_), .b(new_n212_), .O(new_n264_));
  nor2   g092(.a(x79), .b(x77), .O(new_n265_));
  nand2  g093(.a(new_n265_), .b(new_n264_), .O(new_n266_));
  inv1   g094(.a(new_n266_), .O(new_n267_));
  inv1   g095(.a(x07), .O(new_n268_));
  inv1   g096(.a(x52), .O(new_n269_));
  nand2  g097(.a(new_n269_), .b(new_n268_), .O(new_n270_));
  inv1   g098(.a(x15), .O(new_n271_));
  nand2  g099(.a(x52), .b(new_n271_), .O(new_n272_));
  nand3  g100(.a(new_n272_), .b(new_n270_), .c(new_n267_), .O(new_n273_));
  aoi21  g101(.a(new_n273_), .b(new_n263_), .c(x01), .O(z47));
  nand2  g102(.a(new_n262_), .b(x68), .O(new_n275_));
  inv1   g103(.a(x08), .O(new_n276_));
  nand2  g104(.a(new_n269_), .b(new_n276_), .O(new_n277_));
  inv1   g105(.a(x16), .O(new_n278_));
  nand2  g106(.a(x52), .b(new_n278_), .O(new_n279_));
  nand3  g107(.a(new_n279_), .b(new_n277_), .c(new_n267_), .O(new_n280_));
  aoi21  g108(.a(new_n280_), .b(new_n275_), .c(x01), .O(z48));
  nand2  g109(.a(new_n262_), .b(x70), .O(new_n283_));
  inv1   g110(.a(x10), .O(new_n284_));
  nand2  g111(.a(new_n269_), .b(new_n284_), .O(new_n285_));
  inv1   g112(.a(x18), .O(new_n286_));
  nand2  g113(.a(x52), .b(new_n286_), .O(new_n287_));
  nand3  g114(.a(new_n287_), .b(new_n285_), .c(new_n267_), .O(new_n288_));
  aoi21  g115(.a(new_n288_), .b(new_n283_), .c(x01), .O(z50));
  nand2  g116(.a(new_n262_), .b(x72), .O(new_n291_));
  inv1   g117(.a(x12), .O(new_n292_));
  nand2  g118(.a(new_n269_), .b(new_n292_), .O(new_n293_));
  inv1   g119(.a(x20), .O(new_n294_));
  nand2  g120(.a(x52), .b(new_n294_), .O(new_n295_));
  nand3  g121(.a(new_n295_), .b(new_n293_), .c(new_n267_), .O(new_n296_));
  aoi21  g122(.a(new_n296_), .b(new_n291_), .c(x01), .O(z52));
  nand2  g123(.a(new_n262_), .b(x73), .O(new_n298_));
  inv1   g124(.a(x13), .O(new_n299_));
  nand2  g125(.a(new_n269_), .b(new_n299_), .O(new_n300_));
  inv1   g126(.a(x21), .O(new_n301_));
  nand2  g127(.a(x52), .b(new_n301_), .O(new_n302_));
  nand3  g128(.a(new_n302_), .b(new_n300_), .c(new_n267_), .O(new_n303_));
  aoi21  g129(.a(new_n303_), .b(new_n298_), .c(x01), .O(z53));
  oai21  g130(.a(new_n261_), .b(x76), .c(new_n211_), .O(new_n307_));
  aoi21  g131(.a(new_n160_), .b(new_n159_), .c(x01), .O(new_n308_));
  nand3  g132(.a(new_n308_), .b(new_n307_), .c(x00), .O(z56));
  and2   g133(.a(x84), .b(x81), .O(new_n311_));
  nand2  g134(.a(x82), .b(x80), .O(new_n312_));
  nor2   g135(.a(new_n312_), .b(x83), .O(new_n313_));
  nor2   g136(.a(x74), .b(new_n213_), .O(new_n314_));
  nand3  g137(.a(new_n314_), .b(new_n313_), .c(new_n311_), .O(new_n315_));
  nand2  g138(.a(x42), .b(x40), .O(new_n316_));
  nand3  g139(.a(new_n316_), .b(new_n264_), .c(x79), .O(new_n317_));
  aoi21  g140(.a(new_n315_), .b(new_n216_), .c(new_n317_), .O(new_n318_));
  nand4  g141(.a(new_n209_), .b(new_n160_), .c(new_n216_), .d(x40), .O(new_n319_));
  inv1   g142(.a(new_n319_), .O(new_n320_));
  oai21  g143(.a(new_n320_), .b(new_n318_), .c(x77), .O(new_n321_));
  oai21  g144(.a(new_n160_), .b(x77), .c(x04), .O(new_n322_));
  nand2  g145(.a(new_n322_), .b(new_n209_), .O(new_n323_));
  aoi21  g146(.a(new_n323_), .b(new_n321_), .c(x01), .O(z58));
  nand2  g147(.a(new_n315_), .b(new_n216_), .O(new_n325_));
  nand2  g148(.a(new_n325_), .b(new_n156_), .O(new_n326_));
  nand2  g149(.a(new_n326_), .b(new_n264_), .O(new_n327_));
  aoi21  g150(.a(new_n160_), .b(new_n156_), .c(new_n159_), .O(new_n328_));
  oai21  g151(.a(new_n328_), .b(new_n212_), .c(new_n153_), .O(new_n329_));
  aoi21  g152(.a(new_n327_), .b(x79), .c(new_n329_), .O(z59));
  nand2  g153(.a(new_n210_), .b(new_n212_), .O(new_n335_));
  oai21  g154(.a(new_n261_), .b(new_n162_), .c(new_n335_), .O(new_n336_));
  nand3  g155(.a(new_n336_), .b(x83), .c(x79), .O(new_n337_));
  aoi21  g156(.a(new_n337_), .b(new_n266_), .c(x01), .O(z64));
  nand2  g157(.a(new_n154_), .b(x81), .O(new_n339_));
  nand3  g158(.a(new_n308_), .b(x84), .c(x79), .O(new_n340_));
  aoi21  g159(.a(new_n339_), .b(new_n335_), .c(new_n340_), .O(z65));
  zero   g160(.O(z02));
  zero   g161(.O(z03));
  zero   g162(.O(z09));
  zero   g163(.O(z10));
  zero   g164(.O(z11));
  zero   g165(.O(z13));
  zero   g166(.O(z18));
  zero   g167(.O(z21));
  zero   g168(.O(z22));
  zero   g169(.O(z23));
  zero   g170(.O(z26));
  zero   g171(.O(z27));
  zero   g172(.O(z33));
  zero   g173(.O(z36));
  zero   g174(.O(z37));
  zero   g175(.O(z38));
  zero   g176(.O(z39));
  zero   g177(.O(z42));
  zero   g178(.O(z44));
  zero   g179(.O(z45));
  zero   g180(.O(z46));
  zero   g181(.O(z49));
  zero   g182(.O(z51));
  zero   g183(.O(z54));
  zero   g184(.O(z55));
  zero   g185(.O(z57));
  zero   g186(.O(z60));
  zero   g187(.O(z61));
  zero   g188(.O(z62));
  zero   g189(.O(z63));
endmodule


