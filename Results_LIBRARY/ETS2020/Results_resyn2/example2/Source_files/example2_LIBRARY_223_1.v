// Benchmark "FAU" written by ABC on Sat Jul 25 01:27:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n168_, new_n171_, new_n172_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n184_, new_n185_, new_n188_, new_n189_,
    new_n191_, new_n192_, new_n195_, new_n196_, new_n198_, new_n199_,
    new_n202_, new_n203_, new_n208_, new_n209_, new_n210_, new_n211_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n221_, new_n223_, new_n225_, new_n227_, new_n229_,
    new_n234_, new_n235_, new_n236_, new_n237_, new_n239_, new_n245_,
    new_n247_, new_n250_, new_n255_, new_n256_, new_n257_, new_n258_,
    new_n259_, new_n260_, new_n261_, new_n262_, new_n263_, new_n264_,
    new_n265_, new_n266_, new_n267_, new_n268_, new_n269_, new_n270_,
    new_n272_, new_n273_, new_n274_, new_n275_, new_n276_, new_n277_,
    new_n279_, new_n280_, new_n281_, new_n282_, new_n283_, new_n284_,
    new_n286_, new_n287_, new_n288_, new_n289_, new_n290_, new_n291_,
    new_n294_, new_n295_, new_n296_, new_n297_, new_n298_, new_n299_,
    new_n301_, new_n302_, new_n303_, new_n304_, new_n305_, new_n306_,
    new_n308_, new_n309_, new_n312_, new_n313_, new_n314_, new_n317_,
    new_n318_, new_n319_, new_n320_, new_n321_, new_n322_, new_n323_,
    new_n324_, new_n325_, new_n326_, new_n327_, new_n328_, new_n330_,
    new_n331_, new_n332_, new_n333_, new_n337_, new_n338_, new_n339_,
    new_n340_, new_n341_, new_n342_, new_n343_, new_n344_, new_n347_,
    new_n348_, new_n349_, new_n351_, new_n352_;
  nor2   g000(.a(x40), .b(x06), .O(new_n152_));
  inv1   g001(.a(x01), .O(new_n153_));
  inv1   g002(.a(x77), .O(new_n154_));
  inv1   g003(.a(x79), .O(new_n155_));
  nand2  g004(.a(new_n155_), .b(x78), .O(new_n156_));
  oai21  g005(.a(new_n156_), .b(new_n154_), .c(new_n153_), .O(new_n157_));
  inv1   g006(.a(x40), .O(new_n158_));
  nor2   g007(.a(x52), .b(new_n158_), .O(new_n159_));
  aoi21  g008(.a(new_n159_), .b(new_n157_), .c(new_n152_), .O(z00));
  nor2   g009(.a(x78), .b(new_n154_), .O(new_n161_));
  inv1   g010(.a(x78), .O(new_n162_));
  nor2   g011(.a(new_n162_), .b(x77), .O(new_n163_));
  nor2   g012(.a(new_n163_), .b(new_n161_), .O(new_n164_));
  inv1   g013(.a(new_n164_), .O(new_n165_));
  aoi21  g014(.a(new_n165_), .b(x79), .c(x01), .O(z01));
  nand4  g015(.a(new_n155_), .b(x78), .c(x52), .d(new_n153_), .O(new_n168_));
  inv1   g016(.a(new_n168_), .O(z03));
  inv1   g017(.a(new_n157_), .O(z04));
  inv1   g018(.a(x65), .O(new_n171_));
  nor2   g019(.a(x40), .b(x23), .O(new_n172_));
  aoi21  g020(.a(new_n171_), .b(x40), .c(new_n172_), .O(z05));
  inv1   g021(.a(x64), .O(new_n174_));
  nor2   g022(.a(x40), .b(x24), .O(new_n175_));
  aoi21  g023(.a(new_n174_), .b(x40), .c(new_n175_), .O(z06));
  inv1   g024(.a(x63), .O(new_n177_));
  nor2   g025(.a(x40), .b(x25), .O(new_n178_));
  aoi21  g026(.a(new_n177_), .b(x40), .c(new_n178_), .O(z07));
  inv1   g027(.a(x58), .O(new_n184_));
  nor2   g028(.a(x40), .b(x30), .O(new_n185_));
  aoi21  g029(.a(new_n184_), .b(x40), .c(new_n185_), .O(z12));
  inv1   g030(.a(x51), .O(new_n188_));
  nor2   g031(.a(x40), .b(x32), .O(new_n189_));
  aoi21  g032(.a(new_n188_), .b(x40), .c(new_n189_), .O(z14));
  inv1   g033(.a(x50), .O(new_n191_));
  nor2   g034(.a(x40), .b(x33), .O(new_n192_));
  aoi21  g035(.a(new_n191_), .b(x40), .c(new_n192_), .O(z15));
  inv1   g036(.a(x48), .O(new_n195_));
  nor2   g037(.a(x40), .b(x35), .O(new_n196_));
  aoi21  g038(.a(new_n195_), .b(x40), .c(new_n196_), .O(z17));
  inv1   g039(.a(x47), .O(new_n198_));
  nor2   g040(.a(x40), .b(x36), .O(new_n199_));
  aoi21  g041(.a(new_n198_), .b(x40), .c(new_n199_), .O(z18));
  inv1   g042(.a(x45), .O(new_n202_));
  nor2   g043(.a(x40), .b(x38), .O(new_n203_));
  aoi21  g044(.a(new_n202_), .b(x40), .c(new_n203_), .O(z20));
  nand2  g045(.a(x78), .b(x77), .O(new_n208_));
  inv1   g046(.a(x43), .O(new_n209_));
  nor2   g047(.a(x04), .b(x01), .O(new_n210_));
  nand3  g048(.a(new_n210_), .b(new_n209_), .c(x05), .O(new_n211_));
  aoi21  g049(.a(new_n208_), .b(x79), .c(new_n211_), .O(z24));
  inv1   g050(.a(x42), .O(new_n213_));
  xnor2a g051(.a(x84), .b(x82), .O(new_n214_));
  xor2a  g052(.a(new_n214_), .b(x81), .O(new_n215_));
  inv1   g053(.a(new_n208_), .O(new_n216_));
  nand3  g054(.a(new_n210_), .b(new_n216_), .c(x79), .O(new_n217_));
  nor2   g055(.a(new_n217_), .b(new_n215_), .O(new_n218_));
  nand3  g056(.a(new_n218_), .b(new_n213_), .c(x05), .O(new_n219_));
  inv1   g057(.a(new_n219_), .O(z25));
  nand3  g058(.a(new_n218_), .b(x44), .c(new_n213_), .O(new_n221_));
  inv1   g059(.a(new_n221_), .O(z26));
  nand3  g060(.a(new_n218_), .b(x45), .c(new_n213_), .O(new_n223_));
  inv1   g061(.a(new_n223_), .O(z27));
  nand3  g062(.a(new_n218_), .b(x46), .c(new_n213_), .O(new_n225_));
  inv1   g063(.a(new_n225_), .O(z28));
  nand3  g064(.a(new_n218_), .b(x47), .c(new_n213_), .O(new_n227_));
  inv1   g065(.a(new_n227_), .O(z29));
  nand3  g066(.a(new_n218_), .b(x48), .c(new_n213_), .O(new_n229_));
  inv1   g067(.a(new_n229_), .O(z30));
  inv1   g068(.a(new_n217_), .O(new_n234_));
  nand2  g069(.a(x83), .b(x42), .O(new_n235_));
  xor2a  g070(.a(new_n235_), .b(new_n215_), .O(new_n236_));
  nand3  g071(.a(new_n236_), .b(new_n234_), .c(x52), .O(new_n237_));
  inv1   g072(.a(new_n237_), .O(z34));
  nand3  g073(.a(new_n236_), .b(new_n234_), .c(x53), .O(new_n239_));
  inv1   g074(.a(new_n239_), .O(z35));
  nand3  g075(.a(new_n236_), .b(new_n234_), .c(x58), .O(new_n245_));
  inv1   g076(.a(new_n245_), .O(z40));
  nand3  g077(.a(new_n236_), .b(new_n234_), .c(x59), .O(new_n247_));
  inv1   g078(.a(new_n247_), .O(z41));
  nand3  g079(.a(new_n236_), .b(new_n234_), .c(x61), .O(new_n250_));
  inv1   g080(.a(new_n250_), .O(z43));
  and2   g081(.a(x84), .b(x81), .O(new_n255_));
  nor2   g082(.a(x84), .b(x81), .O(new_n256_));
  nor2   g083(.a(new_n256_), .b(new_n255_), .O(new_n257_));
  nand2  g084(.a(new_n161_), .b(x79), .O(new_n258_));
  nor2   g085(.a(new_n258_), .b(new_n257_), .O(new_n259_));
  oai21  g086(.a(x75), .b(x67), .c(new_n259_), .O(new_n260_));
  inv1   g087(.a(x04), .O(new_n261_));
  nor2   g088(.a(x79), .b(new_n261_), .O(new_n262_));
  nand2  g089(.a(new_n262_), .b(new_n163_), .O(new_n263_));
  inv1   g090(.a(new_n263_), .O(new_n264_));
  inv1   g091(.a(x07), .O(new_n265_));
  inv1   g092(.a(x52), .O(new_n266_));
  nand2  g093(.a(new_n266_), .b(new_n265_), .O(new_n267_));
  inv1   g094(.a(x15), .O(new_n268_));
  nand2  g095(.a(x52), .b(new_n268_), .O(new_n269_));
  nand3  g096(.a(new_n269_), .b(new_n267_), .c(new_n264_), .O(new_n270_));
  aoi21  g097(.a(new_n270_), .b(new_n260_), .c(x01), .O(z47));
  nand2  g098(.a(new_n259_), .b(x68), .O(new_n272_));
  inv1   g099(.a(x08), .O(new_n273_));
  nand2  g100(.a(new_n266_), .b(new_n273_), .O(new_n274_));
  inv1   g101(.a(x16), .O(new_n275_));
  nand2  g102(.a(x52), .b(new_n275_), .O(new_n276_));
  nand3  g103(.a(new_n276_), .b(new_n274_), .c(new_n264_), .O(new_n277_));
  aoi21  g104(.a(new_n277_), .b(new_n272_), .c(x01), .O(z48));
  nand2  g105(.a(new_n259_), .b(x69), .O(new_n279_));
  inv1   g106(.a(x09), .O(new_n280_));
  nand2  g107(.a(new_n266_), .b(new_n280_), .O(new_n281_));
  inv1   g108(.a(x17), .O(new_n282_));
  nand2  g109(.a(x52), .b(new_n282_), .O(new_n283_));
  nand3  g110(.a(new_n283_), .b(new_n281_), .c(new_n264_), .O(new_n284_));
  aoi21  g111(.a(new_n284_), .b(new_n279_), .c(x01), .O(z49));
  nand2  g112(.a(new_n259_), .b(x70), .O(new_n286_));
  inv1   g113(.a(x10), .O(new_n287_));
  nand2  g114(.a(new_n266_), .b(new_n287_), .O(new_n288_));
  inv1   g115(.a(x18), .O(new_n289_));
  nand2  g116(.a(x52), .b(new_n289_), .O(new_n290_));
  nand3  g117(.a(new_n290_), .b(new_n288_), .c(new_n264_), .O(new_n291_));
  aoi21  g118(.a(new_n291_), .b(new_n286_), .c(x01), .O(z50));
  nand2  g119(.a(new_n259_), .b(x72), .O(new_n294_));
  inv1   g120(.a(x12), .O(new_n295_));
  nand2  g121(.a(new_n266_), .b(new_n295_), .O(new_n296_));
  inv1   g122(.a(x20), .O(new_n297_));
  nand2  g123(.a(x52), .b(new_n297_), .O(new_n298_));
  nand3  g124(.a(new_n298_), .b(new_n296_), .c(new_n264_), .O(new_n299_));
  aoi21  g125(.a(new_n299_), .b(new_n294_), .c(x01), .O(z52));
  nand2  g126(.a(new_n259_), .b(x73), .O(new_n301_));
  inv1   g127(.a(x13), .O(new_n302_));
  nand2  g128(.a(new_n266_), .b(new_n302_), .O(new_n303_));
  inv1   g129(.a(x21), .O(new_n304_));
  nand2  g130(.a(x52), .b(new_n304_), .O(new_n305_));
  nand3  g131(.a(new_n305_), .b(new_n303_), .c(new_n264_), .O(new_n306_));
  aoi21  g132(.a(new_n306_), .b(new_n301_), .c(x01), .O(z53));
  nor2   g133(.a(x52), .b(x14), .O(new_n308_));
  oai21  g134(.a(new_n266_), .b(x22), .c(new_n153_), .O(new_n309_));
  nor3   g135(.a(new_n309_), .b(new_n308_), .c(new_n263_), .O(z54));
  aoi22  g136(.a(new_n257_), .b(new_n165_), .c(new_n208_), .d(x76), .O(new_n312_));
  nand2  g137(.a(new_n153_), .b(x00), .O(new_n313_));
  aoi21  g138(.a(new_n162_), .b(new_n154_), .c(new_n313_), .O(new_n314_));
  oai21  g139(.a(new_n312_), .b(new_n155_), .c(new_n314_), .O(z56));
  oai21  g140(.a(new_n163_), .b(new_n261_), .c(new_n155_), .O(new_n317_));
  inv1   g141(.a(x83), .O(new_n318_));
  nand3  g142(.a(new_n318_), .b(x82), .c(x80), .O(new_n319_));
  inv1   g143(.a(x74), .O(new_n320_));
  nand4  g144(.a(x84), .b(x81), .c(new_n320_), .d(x43), .O(new_n321_));
  oai21  g145(.a(new_n321_), .b(new_n319_), .c(new_n213_), .O(new_n322_));
  nor2   g146(.a(new_n162_), .b(new_n261_), .O(new_n323_));
  nand3  g147(.a(new_n323_), .b(new_n322_), .c(x79), .O(new_n324_));
  nand3  g148(.a(new_n155_), .b(new_n162_), .c(x40), .O(new_n325_));
  nand2  g149(.a(new_n325_), .b(new_n324_), .O(new_n326_));
  aoi21  g150(.a(x42), .b(x40), .c(new_n154_), .O(new_n327_));
  nand2  g151(.a(new_n327_), .b(new_n326_), .O(new_n328_));
  aoi21  g152(.a(new_n328_), .b(new_n317_), .c(x01), .O(z58));
  nand2  g153(.a(new_n322_), .b(new_n158_), .O(new_n330_));
  nand2  g154(.a(new_n330_), .b(new_n323_), .O(new_n331_));
  aoi21  g155(.a(new_n162_), .b(new_n158_), .c(new_n154_), .O(new_n332_));
  oai21  g156(.a(new_n332_), .b(new_n261_), .c(new_n153_), .O(new_n333_));
  aoi21  g157(.a(new_n331_), .b(x79), .c(new_n333_), .O(z59));
  nand3  g158(.a(new_n255_), .b(new_n161_), .c(x79), .O(new_n337_));
  aoi21  g159(.a(new_n322_), .b(x79), .c(new_n261_), .O(new_n338_));
  nand3  g160(.a(x81), .b(x79), .c(new_n261_), .O(new_n339_));
  nand2  g161(.a(new_n339_), .b(x77), .O(new_n340_));
  nand2  g162(.a(new_n255_), .b(x79), .O(new_n341_));
  nor2   g163(.a(new_n262_), .b(x77), .O(new_n342_));
  aoi21  g164(.a(new_n342_), .b(new_n341_), .c(new_n162_), .O(new_n343_));
  oai21  g165(.a(new_n340_), .b(new_n338_), .c(new_n343_), .O(new_n344_));
  aoi21  g166(.a(new_n344_), .b(new_n337_), .c(x01), .O(z62));
  nand2  g167(.a(new_n216_), .b(new_n261_), .O(new_n347_));
  oai21  g168(.a(new_n257_), .b(new_n164_), .c(new_n347_), .O(new_n348_));
  nand3  g169(.a(new_n348_), .b(x83), .c(x79), .O(new_n349_));
  aoi21  g170(.a(new_n349_), .b(new_n263_), .c(x01), .O(z64));
  nand2  g171(.a(new_n165_), .b(x81), .O(new_n351_));
  nand3  g172(.a(x84), .b(x79), .c(new_n153_), .O(new_n352_));
  aoi21  g173(.a(new_n351_), .b(new_n347_), .c(new_n352_), .O(z65));
  zero   g174(.O(z02));
  zero   g175(.O(z08));
  zero   g176(.O(z09));
  zero   g177(.O(z10));
  zero   g178(.O(z11));
  zero   g179(.O(z13));
  zero   g180(.O(z16));
  zero   g181(.O(z19));
  zero   g182(.O(z21));
  zero   g183(.O(z22));
  zero   g184(.O(z23));
  zero   g185(.O(z31));
  zero   g186(.O(z32));
  zero   g187(.O(z33));
  zero   g188(.O(z36));
  zero   g189(.O(z37));
  zero   g190(.O(z38));
  zero   g191(.O(z39));
  zero   g192(.O(z42));
  zero   g193(.O(z44));
  zero   g194(.O(z45));
  zero   g195(.O(z46));
  zero   g196(.O(z51));
  zero   g197(.O(z55));
  zero   g198(.O(z57));
  zero   g199(.O(z60));
  zero   g200(.O(z61));
  zero   g201(.O(z63));
endmodule


