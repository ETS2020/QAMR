// Benchmark "FAU" written by ABC on Fri Jul 24 21:09:37 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  nand3  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  inv1   g05(.a(new_n22_), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n18_), .c(x4), .O(new_n24_));
  inv1   g07(.a(x4), .O(new_n25_));
  inv1   g08(.a(x1), .O(new_n26_));
  nand2  g09(.a(x3), .b(x2), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(new_n19_), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(x6), .c(x5), .d(new_n25_), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n24_), .O(z0));
  aoi21  g13(.a(new_n28_), .b(x6), .c(x4), .O(new_n31_));
  oai21  g14(.a(new_n31_), .b(new_n23_), .c(x5), .O(new_n32_));
  nor2   g15(.a(new_n20_), .b(new_n25_), .O(new_n33_));
  nor2   g16(.a(x2), .b(x1), .O(new_n34_));
  nor2   g17(.a(x3), .b(x2), .O(new_n35_));
  nand2  g18(.a(x4), .b(x1), .O(new_n36_));
  oai22  g19(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(new_n20_), .O(new_n37_));
  aoi21  g20(.a(new_n37_), .b(x0), .c(new_n33_), .O(new_n38_));
  oai21  g21(.a(new_n38_), .b(x5), .c(new_n32_), .O(z1));
  nand2  g22(.a(x5), .b(x0), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n26_), .O(new_n41_));
  nand3  g24(.a(x5), .b(x3), .c(x2), .O(new_n42_));
  oai21  g25(.a(x3), .b(x2), .c(x0), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n41_), .c(x4), .O(new_n45_));
  nand2  g28(.a(new_n44_), .b(new_n41_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n25_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n45_), .c(new_n20_), .O(new_n48_));
  oai21  g31(.a(new_n34_), .b(new_n19_), .c(new_n18_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n28_), .c(new_n25_), .O(new_n50_));
  nand2  g33(.a(new_n18_), .b(new_n19_), .O(new_n51_));
  oai21  g34(.a(x3), .b(x0), .c(x2), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(new_n40_), .c(new_n26_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x4), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n50_), .c(x6), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n48_), .O(z2));
  nand2  g40(.a(x5), .b(x3), .O(new_n58_));
  nand3  g41(.a(new_n52_), .b(new_n58_), .c(new_n20_), .O(new_n59_));
  nor2   g42(.a(new_n52_), .b(new_n20_), .O(new_n60_));
  aoi21  g43(.a(new_n59_), .b(x1), .c(new_n60_), .O(new_n61_));
  and2   g44(.a(new_n51_), .b(new_n40_), .O(new_n62_));
  inv1   g45(.a(x3), .O(z8));
  oai21  g46(.a(x6), .b(x1), .c(x2), .O(new_n64_));
  aoi21  g47(.a(x5), .b(z8), .c(new_n64_), .O(new_n65_));
  oai21  g48(.a(x5), .b(z8), .c(new_n20_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(x1), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n62_), .O(new_n68_));
  oai22  g51(.a(new_n68_), .b(new_n65_), .c(new_n62_), .d(new_n61_), .O(z3));
  nand2  g52(.a(new_n52_), .b(new_n20_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(new_n26_), .O(new_n71_));
  nand2  g54(.a(new_n27_), .b(new_n19_), .O(new_n72_));
  nor2   g55(.a(new_n35_), .b(x6), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g57(.a(new_n43_), .b(new_n27_), .c(x6), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n74_), .c(x1), .O(new_n76_));
  oai21  g59(.a(new_n71_), .b(new_n60_), .c(new_n76_), .O(z4));
  aoi21  g60(.a(x3), .b(x1), .c(x2), .O(new_n78_));
  inv1   g61(.a(new_n27_), .O(new_n79_));
  oai21  g62(.a(new_n78_), .b(new_n79_), .c(x0), .O(new_n80_));
  oai21  g63(.a(new_n78_), .b(new_n72_), .c(new_n80_), .O(z5));
  zero   g64(.O(z6));
  zero   g65(.O(z7));
  zero   g66(.O(z9));
endmodule


