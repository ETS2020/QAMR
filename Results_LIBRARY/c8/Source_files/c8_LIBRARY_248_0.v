// Benchmark "FAU" written by ABC on Thu Jun 25 19:35:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nand2  g002(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  oai21  g003(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g004(.a(x20), .O(new_n51_));
  nand2  g005(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  oai21  g006(.a(new_n47_), .b(x09), .c(new_n52_), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  nand2  g008(.a(x27), .b(new_n54_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  inv1   g010(.a(x22), .O(new_n57_));
  nand2  g011(.a(new_n47_), .b(new_n57_), .O(new_n58_));
  oai21  g012(.a(new_n47_), .b(x11), .c(new_n58_), .O(z03));
  inv1   g013(.a(x23), .O(new_n60_));
  nand2  g014(.a(new_n47_), .b(new_n60_), .O(new_n61_));
  oai21  g015(.a(new_n47_), .b(x12), .c(new_n61_), .O(z04));
  inv1   g016(.a(x13), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  oai21  g018(.a(x27), .b(x24), .c(new_n64_), .O(z05));
  inv1   g019(.a(x25), .O(new_n66_));
  nand2  g020(.a(new_n47_), .b(new_n66_), .O(new_n67_));
  oai21  g021(.a(new_n47_), .b(x14), .c(new_n67_), .O(z06));
  inv1   g022(.a(x26), .O(new_n69_));
  nand2  g023(.a(new_n47_), .b(new_n69_), .O(new_n70_));
  oai21  g024(.a(new_n47_), .b(x15), .c(new_n70_), .O(z07));
  inv1   g025(.a(x17), .O(new_n72_));
  nor2   g026(.a(new_n48_), .b(new_n72_), .O(new_n73_));
  nor2   g027(.a(x19), .b(x17), .O(new_n74_));
  oai21  g028(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  inv1   g029(.a(x16), .O(new_n76_));
  inv1   g030(.a(x08), .O(new_n77_));
  nand2  g031(.a(x18), .b(x00), .O(new_n78_));
  oai21  g032(.a(x18), .b(new_n77_), .c(new_n78_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n75_), .O(z09));
  inv1   g035(.a(x09), .O(new_n82_));
  nand2  g036(.a(x18), .b(x01), .O(new_n83_));
  oai21  g037(.a(x18), .b(new_n82_), .c(new_n83_), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n76_), .O(new_n85_));
  nand2  g039(.a(new_n48_), .b(new_n72_), .O(new_n86_));
  nand3  g040(.a(new_n86_), .b(x20), .c(x16), .O(new_n87_));
  nor2   g041(.a(new_n86_), .b(x20), .O(new_n88_));
  inv1   g042(.a(new_n88_), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z10));
  xor2a  g044(.a(new_n74_), .b(x21), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n51_), .O(new_n92_));
  nand2  g046(.a(x18), .b(x02), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n54_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n76_), .O(new_n95_));
  nand3  g049(.a(x21), .b(x20), .c(x16), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n95_), .c(new_n92_), .O(z11));
  nor2   g051(.a(x21), .b(x20), .O(new_n98_));
  nor2   g052(.a(new_n98_), .b(new_n76_), .O(new_n99_));
  nand2  g053(.a(new_n86_), .b(new_n51_), .O(new_n100_));
  inv1   g054(.a(new_n100_), .O(new_n101_));
  oai21  g055(.a(new_n101_), .b(new_n99_), .c(x22), .O(new_n102_));
  inv1   g056(.a(x11), .O(new_n103_));
  nand2  g057(.a(x18), .b(x03), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n76_), .O(new_n106_));
  nor3   g060(.a(x22), .b(x21), .c(x20), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n74_), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n106_), .c(new_n102_), .O(z12));
  nor2   g063(.a(new_n107_), .b(new_n76_), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n101_), .c(x23), .O(new_n111_));
  inv1   g065(.a(x12), .O(new_n112_));
  nand2  g066(.a(x18), .b(x04), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n112_), .c(new_n113_), .O(new_n114_));
  nor3   g068(.a(x23), .b(x22), .c(x21), .O(new_n115_));
  aoi22  g069(.a(new_n115_), .b(new_n88_), .c(new_n114_), .d(new_n76_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n111_), .O(z13));
  nor2   g071(.a(x23), .b(x22), .O(new_n118_));
  aoi21  g072(.a(new_n118_), .b(new_n98_), .c(new_n76_), .O(new_n119_));
  oai21  g073(.a(new_n119_), .b(new_n101_), .c(x24), .O(new_n120_));
  nand2  g074(.a(x18), .b(x05), .O(new_n121_));
  oai21  g075(.a(x18), .b(new_n63_), .c(new_n121_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n76_), .O(new_n123_));
  nor2   g077(.a(x22), .b(x21), .O(new_n124_));
  nor2   g078(.a(x24), .b(x23), .O(new_n125_));
  nand3  g079(.a(new_n125_), .b(new_n124_), .c(new_n88_), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n123_), .c(new_n120_), .O(z14));
  nand3  g081(.a(new_n125_), .b(new_n98_), .c(new_n57_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(x16), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n100_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(x25), .O(new_n131_));
  nand2  g085(.a(x18), .b(x06), .O(new_n132_));
  inv1   g086(.a(x18), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x14), .O(new_n134_));
  aoi21  g088(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  nand2  g089(.a(new_n98_), .b(new_n74_), .O(new_n136_));
  nor2   g090(.a(x25), .b(x24), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n118_), .O(new_n138_));
  nor2   g092(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nor2   g093(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n131_), .O(z15));
  nor3   g095(.a(x25), .b(x24), .c(x23), .O(new_n142_));
  aoi21  g096(.a(new_n142_), .b(new_n107_), .c(new_n76_), .O(new_n143_));
  oai21  g097(.a(new_n143_), .b(new_n101_), .c(x26), .O(new_n144_));
  nand2  g098(.a(x18), .b(x07), .O(new_n145_));
  nand2  g099(.a(new_n133_), .b(x15), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n145_), .c(x16), .O(new_n147_));
  nand3  g101(.a(new_n137_), .b(new_n118_), .c(new_n69_), .O(new_n148_));
  nor2   g102(.a(new_n148_), .b(new_n136_), .O(new_n149_));
  nor2   g103(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n144_), .O(z16));
  nand3  g105(.a(x27), .b(x17), .c(x16), .O(new_n152_));
  nand3  g106(.a(new_n98_), .b(x19), .c(new_n72_), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(new_n148_), .c(new_n152_), .O(z17));
  buf    g108(.a(x27), .O(z08));
endmodule


