// Benchmark "FAU" written by ABC on Thu Jun 25 16:34:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n53_, new_n54_, new_n57_, new_n59_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x09), .O(new_n31_));
  oai21  g02(.a(x10), .b(new_n31_), .c(x11), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(new_n32_), .c(new_n30_), .O(z00));
  nand2  g06(.a(new_n32_), .b(x12), .O(z01));
  inv1   g07(.a(x12), .O(new_n37_));
  nor2   g08(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  nand2  g09(.a(x11), .b(new_n31_), .O(new_n39_));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z02));
  nand3  g13(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(z03));
  inv1   g14(.a(x14), .O(z04));
  inv1   g15(.a(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  aoi21  g19(.a(new_n47_), .b(new_n34_), .c(new_n48_), .O(z06));
  inv1   g20(.a(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(z08));
  xor2a  g23(.a(x12), .b(x11), .O(new_n53_));
  nand2  g24(.a(new_n51_), .b(x00), .O(new_n54_));
  nor2   g25(.a(new_n54_), .b(new_n53_), .O(z09));
  nand3  g26(.a(new_n37_), .b(x11), .c(new_n30_), .O(new_n57_));
  nor2   g27(.a(new_n57_), .b(new_n54_), .O(z11));
  nand4  g28(.a(x12), .b(x11), .c(new_n51_), .d(x00), .O(new_n59_));
  inv1   g29(.a(new_n59_), .O(z12));
  zero   g30(.O(z10));
endmodule


