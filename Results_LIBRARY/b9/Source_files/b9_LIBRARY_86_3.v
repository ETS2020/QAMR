// Benchmark "FAU" written by ABC on Thu Jun 25 20:41:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n138_, new_n140_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_;
  inv1   g00(.a(x10), .O(new_n63_));
  oai21  g01(.a(x36), .b(x35), .c(x27), .O(new_n64_));
  nand2  g02(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g03(.a(x04), .O(new_n66_));
  nand3  g04(.a(x40), .b(x39), .c(new_n66_), .O(new_n67_));
  inv1   g05(.a(new_n67_), .O(new_n68_));
  inv1   g06(.a(x27), .O(new_n69_));
  inv1   g07(.a(x37), .O(z06));
  nor2   g08(.a(z06), .b(new_n69_), .O(new_n71_));
  aoi21  g09(.a(new_n68_), .b(new_n65_), .c(new_n71_), .O(new_n72_));
  oai21  g10(.a(new_n72_), .b(x15), .c(x16), .O(z00));
  nor2   g11(.a(x32), .b(x30), .O(new_n74_));
  nand2  g12(.a(new_n74_), .b(new_n64_), .O(new_n75_));
  nand2  g13(.a(new_n75_), .b(x04), .O(z01));
  nor2   g14(.a(x27), .b(x08), .O(new_n77_));
  inv1   g15(.a(x28), .O(new_n78_));
  nand2  g16(.a(x35), .b(new_n78_), .O(new_n79_));
  oai21  g17(.a(new_n79_), .b(new_n77_), .c(x04), .O(new_n80_));
  aoi21  g18(.a(x29), .b(x08), .c(x02), .O(new_n81_));
  nand2  g19(.a(x40), .b(x39), .O(new_n82_));
  nor2   g20(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g21(.a(new_n83_), .b(new_n80_), .O(z02));
  nand3  g22(.a(x37), .b(new_n69_), .c(x21), .O(z03));
  inv1   g23(.a(x21), .O(new_n86_));
  nand3  g24(.a(x37), .b(new_n69_), .c(new_n86_), .O(z04));
  inv1   g25(.a(x03), .O(new_n88_));
  inv1   g26(.a(x00), .O(new_n89_));
  oai21  g27(.a(x25), .b(new_n89_), .c(x38), .O(new_n90_));
  inv1   g28(.a(x15), .O(new_n91_));
  nand2  g29(.a(x17), .b(new_n91_), .O(new_n92_));
  nor2   g30(.a(x33), .b(x31), .O(new_n93_));
  nand3  g31(.a(new_n93_), .b(new_n92_), .c(new_n90_), .O(new_n94_));
  inv1   g32(.a(x14), .O(new_n95_));
  inv1   g33(.a(x25), .O(new_n96_));
  nand3  g34(.a(x38), .b(new_n96_), .c(new_n89_), .O(new_n97_));
  nand2  g35(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  aoi21  g36(.a(new_n98_), .b(new_n94_), .c(new_n88_), .O(z07));
  inv1   g37(.a(new_n82_), .O(z08));
  nand4  g38(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n101_));
  inv1   g39(.a(new_n101_), .O(z09));
  nand2  g40(.a(new_n82_), .b(x07), .O(new_n103_));
  nand3  g41(.a(x40), .b(x39), .c(x05), .O(new_n104_));
  aoi21  g42(.a(new_n104_), .b(new_n103_), .c(new_n64_), .O(new_n105_));
  inv1   g43(.a(x05), .O(new_n106_));
  nor3   g44(.a(new_n82_), .b(new_n74_), .c(new_n106_), .O(new_n107_));
  oai21  g45(.a(new_n107_), .b(new_n105_), .c(new_n66_), .O(new_n108_));
  nand2  g46(.a(new_n71_), .b(x06), .O(new_n109_));
  nand2  g47(.a(new_n109_), .b(new_n108_), .O(z10));
  inv1   g48(.a(x08), .O(new_n111_));
  nand3  g49(.a(x35), .b(x27), .c(new_n66_), .O(new_n112_));
  nand2  g50(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g51(.a(x40), .b(x39), .c(x29), .O(new_n114_));
  nor2   g52(.a(new_n69_), .b(new_n66_), .O(new_n115_));
  oai21  g53(.a(new_n115_), .b(new_n79_), .c(new_n114_), .O(new_n116_));
  inv1   g54(.a(x09), .O(new_n117_));
  inv1   g55(.a(x30), .O(new_n118_));
  nand2  g56(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  aoi21  g57(.a(new_n116_), .b(new_n113_), .c(new_n119_), .O(z11));
  nor2   g58(.a(new_n82_), .b(x04), .O(new_n121_));
  nor2   g59(.a(x36), .b(x35), .O(new_n122_));
  aoi21  g60(.a(new_n122_), .b(z06), .c(new_n69_), .O(new_n123_));
  oai22  g61(.a(new_n123_), .b(x10), .c(new_n121_), .d(new_n71_), .O(z12));
  inv1   g62(.a(x18), .O(new_n125_));
  inv1   g63(.a(x19), .O(new_n126_));
  nand4  g64(.a(x37), .b(x20), .c(new_n126_), .d(new_n125_), .O(new_n127_));
  inv1   g65(.a(x13), .O(new_n128_));
  nand3  g66(.a(z08), .b(new_n128_), .c(new_n66_), .O(new_n129_));
  nand2  g67(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g68(.a(new_n130_), .b(x27), .O(new_n131_));
  inv1   g69(.a(new_n74_), .O(new_n132_));
  nand4  g70(.a(z08), .b(new_n132_), .c(new_n128_), .d(new_n66_), .O(new_n133_));
  nand2  g71(.a(new_n133_), .b(new_n131_), .O(z13));
  nand4  g72(.a(new_n71_), .b(x20), .c(new_n126_), .d(new_n125_), .O(new_n135_));
  nand2  g73(.a(new_n135_), .b(new_n129_), .O(new_n136_));
  oai21  g74(.a(new_n132_), .b(x27), .c(new_n136_), .O(z14));
  nand4  g75(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n138_));
  inv1   g76(.a(new_n138_), .O(z15));
  nand2  g77(.a(x22), .b(x01), .O(new_n140_));
  nor2   g78(.a(new_n140_), .b(x23), .O(z16));
  inv1   g79(.a(x24), .O(new_n142_));
  nand4  g80(.a(new_n142_), .b(x23), .c(x22), .d(x01), .O(new_n143_));
  inv1   g81(.a(new_n143_), .O(z17));
  oai21  g82(.a(new_n79_), .b(x27), .c(new_n114_), .O(new_n145_));
  nand2  g83(.a(new_n145_), .b(x08), .O(new_n146_));
  nand4  g84(.a(new_n146_), .b(new_n112_), .c(new_n118_), .d(new_n117_), .O(z18));
  nand2  g85(.a(x35), .b(x27), .O(new_n148_));
  nand2  g86(.a(new_n148_), .b(new_n111_), .O(new_n149_));
  nand2  g87(.a(new_n114_), .b(new_n79_), .O(new_n150_));
  aoi21  g88(.a(new_n150_), .b(new_n149_), .c(new_n119_), .O(z20));
  buf    g89(.a(x37), .O(z05));
  aoi21  g90(.a(new_n116_), .b(new_n113_), .c(new_n119_), .O(z19));
endmodule


