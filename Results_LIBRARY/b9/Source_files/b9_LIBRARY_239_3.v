// Benchmark "FAU" written by ABC on Thu Jun 25 20:42:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n146_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_;
  nor2   g00(.a(x36), .b(x35), .O(new_n63_));
  inv1   g01(.a(x04), .O(new_n64_));
  nand3  g02(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g03(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g04(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand2  g05(.a(x40), .b(x39), .O(new_n68_));
  inv1   g06(.a(new_n68_), .O(z08));
  nand3  g07(.a(z08), .b(x10), .c(new_n64_), .O(new_n70_));
  inv1   g08(.a(new_n70_), .O(new_n71_));
  aoi21  g09(.a(new_n67_), .b(x27), .c(new_n71_), .O(new_n72_));
  oai21  g10(.a(new_n72_), .b(x15), .c(x16), .O(z00));
  inv1   g11(.a(x27), .O(new_n74_));
  inv1   g12(.a(x36), .O(new_n75_));
  inv1   g13(.a(x28), .O(new_n76_));
  nand2  g14(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g15(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  nor2   g16(.a(x32), .b(x30), .O(new_n79_));
  inv1   g17(.a(new_n79_), .O(new_n80_));
  oai21  g18(.a(new_n80_), .b(new_n78_), .c(x04), .O(z01));
  aoi21  g19(.a(x29), .b(x08), .c(x02), .O(new_n82_));
  aoi21  g20(.a(new_n77_), .b(x04), .c(new_n82_), .O(new_n83_));
  inv1   g21(.a(x08), .O(new_n84_));
  nand3  g22(.a(new_n74_), .b(new_n84_), .c(x04), .O(new_n85_));
  nand4  g23(.a(new_n85_), .b(new_n83_), .c(x40), .d(x39), .O(z02));
  inv1   g24(.a(x35), .O(new_n87_));
  oai21  g25(.a(new_n87_), .b(new_n76_), .c(x27), .O(new_n88_));
  inv1   g26(.a(x37), .O(new_n89_));
  nand2  g27(.a(new_n89_), .b(new_n74_), .O(new_n90_));
  nand3  g28(.a(new_n90_), .b(new_n88_), .c(x21), .O(z03));
  inv1   g29(.a(x21), .O(new_n92_));
  nand3  g30(.a(new_n90_), .b(new_n88_), .c(new_n92_), .O(z04));
  aoi21  g31(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g32(.a(z06), .O(z05));
  inv1   g33(.a(x03), .O(new_n96_));
  inv1   g34(.a(x00), .O(new_n97_));
  oai21  g35(.a(x25), .b(new_n97_), .c(x38), .O(new_n98_));
  inv1   g36(.a(x15), .O(new_n99_));
  nand2  g37(.a(x17), .b(new_n99_), .O(new_n100_));
  nor2   g38(.a(x33), .b(x31), .O(new_n101_));
  nand3  g39(.a(new_n101_), .b(new_n100_), .c(new_n98_), .O(new_n102_));
  inv1   g40(.a(x14), .O(new_n103_));
  inv1   g41(.a(x25), .O(new_n104_));
  nand3  g42(.a(x38), .b(new_n104_), .c(new_n97_), .O(new_n105_));
  nand2  g43(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  aoi21  g44(.a(new_n106_), .b(new_n102_), .c(new_n96_), .O(z07));
  nand4  g45(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n108_));
  inv1   g46(.a(new_n108_), .O(z09));
  inv1   g47(.a(x05), .O(new_n110_));
  nand2  g48(.a(new_n68_), .b(x07), .O(new_n111_));
  oai21  g49(.a(new_n68_), .b(new_n110_), .c(new_n111_), .O(new_n112_));
  nor2   g50(.a(new_n79_), .b(new_n68_), .O(new_n113_));
  aoi22  g51(.a(new_n113_), .b(x05), .c(new_n112_), .d(new_n78_), .O(new_n114_));
  nand3  g52(.a(x37), .b(x27), .c(x06), .O(new_n115_));
  oai21  g53(.a(new_n114_), .b(x04), .c(new_n115_), .O(z10));
  nand2  g54(.a(z08), .b(x29), .O(new_n117_));
  nor2   g55(.a(new_n74_), .b(new_n64_), .O(new_n118_));
  oai21  g56(.a(new_n118_), .b(new_n77_), .c(new_n117_), .O(new_n119_));
  nand4  g57(.a(x35), .b(new_n76_), .c(x27), .d(new_n64_), .O(new_n120_));
  nand2  g58(.a(new_n120_), .b(new_n84_), .O(new_n121_));
  nor2   g59(.a(x30), .b(x09), .O(new_n122_));
  inv1   g60(.a(new_n122_), .O(new_n123_));
  aoi21  g61(.a(new_n121_), .b(new_n119_), .c(new_n123_), .O(z11));
  aoi22  g62(.a(new_n63_), .b(new_n89_), .c(x40), .d(new_n74_), .O(new_n125_));
  oai22  g63(.a(new_n68_), .b(x04), .c(new_n66_), .d(new_n74_), .O(new_n126_));
  oai21  g64(.a(new_n125_), .b(x10), .c(new_n126_), .O(z12));
  nand2  g65(.a(x36), .b(x35), .O(new_n128_));
  aoi21  g66(.a(new_n128_), .b(x28), .c(new_n65_), .O(new_n129_));
  inv1   g67(.a(x18), .O(new_n130_));
  inv1   g68(.a(x19), .O(new_n131_));
  nand3  g69(.a(x20), .b(new_n131_), .c(new_n130_), .O(new_n132_));
  nor2   g70(.a(new_n132_), .b(new_n66_), .O(new_n133_));
  oai21  g71(.a(new_n133_), .b(new_n129_), .c(x27), .O(new_n134_));
  nand2  g72(.a(new_n113_), .b(new_n64_), .O(new_n135_));
  nand2  g73(.a(new_n135_), .b(new_n134_), .O(z13));
  nand4  g74(.a(x27), .b(x20), .c(new_n131_), .d(new_n130_), .O(new_n137_));
  oai22  g75(.a(new_n137_), .b(new_n66_), .c(new_n68_), .d(x04), .O(new_n138_));
  nor2   g76(.a(x37), .b(x35), .O(new_n139_));
  aoi21  g77(.a(new_n132_), .b(new_n128_), .c(new_n139_), .O(new_n140_));
  oai21  g78(.a(new_n140_), .b(new_n76_), .c(x27), .O(new_n141_));
  nand2  g79(.a(new_n141_), .b(new_n79_), .O(new_n142_));
  nand2  g80(.a(new_n142_), .b(new_n138_), .O(z14));
  nand4  g81(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n144_));
  inv1   g82(.a(new_n144_), .O(z15));
  nand2  g83(.a(x22), .b(x01), .O(new_n146_));
  nor2   g84(.a(new_n146_), .b(x23), .O(z16));
  inv1   g85(.a(x24), .O(new_n148_));
  nand4  g86(.a(new_n148_), .b(x23), .c(x22), .d(x01), .O(new_n149_));
  inv1   g87(.a(new_n149_), .O(z17));
  oai21  g88(.a(new_n77_), .b(x27), .c(new_n117_), .O(new_n151_));
  nand2  g89(.a(new_n151_), .b(x08), .O(new_n152_));
  nand3  g90(.a(new_n152_), .b(new_n122_), .c(new_n120_), .O(z18));
  oai21  g91(.a(new_n87_), .b(new_n74_), .c(new_n84_), .O(new_n154_));
  oai21  g92(.a(new_n76_), .b(x27), .c(x35), .O(new_n155_));
  nand2  g93(.a(new_n155_), .b(new_n117_), .O(new_n156_));
  aoi21  g94(.a(new_n156_), .b(new_n154_), .c(new_n123_), .O(z20));
  aoi21  g95(.a(new_n121_), .b(new_n119_), .c(new_n123_), .O(z19));
endmodule


