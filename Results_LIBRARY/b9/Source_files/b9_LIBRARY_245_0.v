// Benchmark "FAU" written by ABC on Thu Jun 25 20:42:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n151_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n161_, new_n162_, new_n163_,
    new_n164_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(x15), .c(x16), .O(z00));
  inv1   g009(.a(x36), .O(new_n72_));
  inv1   g010(.a(x28), .O(new_n73_));
  nand2  g011(.a(x35), .b(new_n73_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n75_), .c(x04), .O(z01));
  nor2   g017(.a(x27), .b(x08), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n74_), .c(x04), .O(new_n81_));
  inv1   g019(.a(x02), .O(new_n82_));
  nand2  g020(.a(x40), .b(x39), .O(new_n83_));
  nand2  g021(.a(x29), .b(x08), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(new_n82_), .c(new_n83_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n81_), .O(z02));
  inv1   g024(.a(x35), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n73_), .c(x27), .O(new_n88_));
  inv1   g026(.a(x27), .O(new_n89_));
  inv1   g027(.a(x37), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g029(.a(new_n91_), .b(new_n88_), .c(x21), .O(z03));
  inv1   g030(.a(x21), .O(new_n93_));
  nand3  g031(.a(new_n91_), .b(new_n88_), .c(new_n93_), .O(z04));
  aoi21  g032(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g033(.a(z06), .O(z05));
  inv1   g034(.a(x03), .O(new_n97_));
  inv1   g035(.a(x00), .O(new_n98_));
  oai21  g036(.a(x25), .b(new_n98_), .c(x38), .O(new_n99_));
  inv1   g037(.a(x15), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n100_), .O(new_n101_));
  nor2   g039(.a(x33), .b(x31), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n101_), .c(new_n99_), .O(new_n103_));
  inv1   g041(.a(x14), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand3  g043(.a(x38), .b(new_n105_), .c(new_n98_), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n103_), .c(new_n97_), .O(z07));
  inv1   g046(.a(new_n83_), .O(z08));
  nand4  g047(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(z09));
  nand3  g049(.a(new_n83_), .b(x27), .c(x07), .O(new_n112_));
  nand3  g050(.a(x40), .b(x39), .c(x05), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  aoi22  g054(.a(new_n116_), .b(new_n78_), .c(new_n114_), .d(new_n75_), .O(new_n117_));
  nand3  g055(.a(x37), .b(x27), .c(x06), .O(new_n118_));
  oai21  g056(.a(new_n117_), .b(x04), .c(new_n118_), .O(z10));
  inv1   g057(.a(new_n80_), .O(new_n120_));
  nor2   g058(.a(new_n87_), .b(x28), .O(new_n121_));
  nand2  g059(.a(x27), .b(x04), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  oai21  g061(.a(new_n84_), .b(new_n83_), .c(new_n123_), .O(new_n124_));
  inv1   g062(.a(x09), .O(new_n125_));
  nand2  g063(.a(new_n76_), .b(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(new_n120_), .c(new_n126_), .O(z11));
  aoi21  g065(.a(new_n63_), .b(new_n90_), .c(new_n89_), .O(new_n128_));
  nor2   g066(.a(new_n66_), .b(new_n89_), .O(new_n129_));
  nor2   g067(.a(new_n83_), .b(x04), .O(new_n130_));
  oai22  g068(.a(new_n130_), .b(new_n129_), .c(new_n128_), .d(x10), .O(z12));
  inv1   g069(.a(new_n129_), .O(new_n132_));
  nor2   g070(.a(x19), .b(x18), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x20), .O(new_n134_));
  inv1   g072(.a(x13), .O(new_n135_));
  inv1   g073(.a(x39), .O(new_n136_));
  nor2   g074(.a(new_n136_), .b(x04), .O(new_n137_));
  nor2   g075(.a(x30), .b(new_n73_), .O(new_n138_));
  aoi21  g076(.a(x36), .b(x35), .c(x32), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand4  g078(.a(new_n140_), .b(new_n137_), .c(x40), .d(new_n135_), .O(new_n141_));
  oai21  g079(.a(new_n134_), .b(new_n132_), .c(new_n141_), .O(z13));
  nand4  g080(.a(new_n140_), .b(new_n122_), .c(z08), .d(new_n135_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n134_), .O(new_n144_));
  nand3  g082(.a(z08), .b(new_n135_), .c(new_n64_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n132_), .O(new_n146_));
  nand4  g084(.a(new_n138_), .b(new_n90_), .c(new_n87_), .d(new_n77_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n146_), .c(new_n144_), .O(z14));
  nand4  g086(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(z15));
  nand2  g088(.a(x22), .b(x01), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(x23), .O(z16));
  inv1   g090(.a(x24), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x23), .c(x22), .d(x01), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(z17));
  inv1   g093(.a(x08), .O(new_n156_));
  aoi22  g094(.a(new_n121_), .b(new_n89_), .c(z08), .d(x29), .O(new_n157_));
  nor2   g095(.a(new_n89_), .b(x04), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n121_), .c(new_n126_), .O(new_n159_));
  oai21  g097(.a(new_n157_), .b(new_n156_), .c(new_n159_), .O(z18));
  oai21  g098(.a(new_n87_), .b(new_n89_), .c(new_n156_), .O(new_n161_));
  nand2  g099(.a(z08), .b(x29), .O(new_n162_));
  oai21  g100(.a(new_n73_), .b(x27), .c(x35), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n161_), .c(new_n126_), .O(z20));
  aoi21  g103(.a(new_n124_), .b(new_n120_), .c(new_n126_), .O(z19));
endmodule


