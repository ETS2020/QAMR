// Benchmark "FAU" written by ABC on Thu Jun 25 20:40:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n154_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n162_, new_n163_,
    new_n164_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x10), .O(new_n64_));
  nand2  g002(.a(x35), .b(x27), .O(new_n65_));
  inv1   g003(.a(x04), .O(new_n66_));
  nand3  g004(.a(x40), .b(x39), .c(new_n66_), .O(new_n67_));
  aoi21  g005(.a(new_n65_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  nor2   g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  oai21  g009(.a(new_n71_), .b(new_n68_), .c(new_n63_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(x16), .O(z00));
  inv1   g011(.a(x28), .O(new_n74_));
  nor2   g012(.a(x36), .b(new_n74_), .O(new_n75_));
  nor2   g013(.a(x32), .b(x30), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(new_n65_), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x04), .O(z01));
  nor2   g016(.a(x27), .b(x08), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n74_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n79_), .c(x04), .O(new_n81_));
  aoi21  g019(.a(x29), .b(x08), .c(x02), .O(new_n82_));
  nand2  g020(.a(x40), .b(x39), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n81_), .O(z02));
  inv1   g023(.a(x35), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n74_), .c(x27), .O(new_n87_));
  inv1   g025(.a(x37), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n69_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n87_), .c(x21), .O(z03));
  inv1   g028(.a(x21), .O(new_n91_));
  nand3  g029(.a(new_n89_), .b(new_n87_), .c(new_n91_), .O(z04));
  aoi21  g030(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g031(.a(z06), .O(z05));
  inv1   g032(.a(x03), .O(new_n95_));
  inv1   g033(.a(x00), .O(new_n96_));
  oai21  g034(.a(x25), .b(new_n96_), .c(x38), .O(new_n97_));
  nand2  g035(.a(x17), .b(new_n63_), .O(new_n98_));
  nor2   g036(.a(x33), .b(x31), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x25), .O(new_n102_));
  nand3  g040(.a(x38), .b(new_n102_), .c(new_n96_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n100_), .c(new_n95_), .O(z07));
  inv1   g043(.a(new_n83_), .O(z08));
  nand4  g044(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z09));
  inv1   g046(.a(x05), .O(new_n109_));
  nand2  g047(.a(new_n83_), .b(x07), .O(new_n110_));
  oai21  g048(.a(new_n83_), .b(new_n109_), .c(new_n110_), .O(new_n111_));
  inv1   g049(.a(x36), .O(new_n112_));
  aoi21  g050(.a(new_n80_), .b(new_n112_), .c(new_n69_), .O(new_n113_));
  nor3   g051(.a(new_n83_), .b(new_n76_), .c(new_n109_), .O(new_n114_));
  aoi21  g052(.a(new_n113_), .b(new_n111_), .c(new_n114_), .O(new_n115_));
  nand3  g053(.a(x37), .b(x27), .c(x06), .O(new_n116_));
  oai21  g054(.a(new_n115_), .b(x04), .c(new_n116_), .O(z10));
  nand2  g055(.a(z08), .b(x29), .O(new_n118_));
  nor2   g056(.a(new_n69_), .b(new_n66_), .O(new_n119_));
  oai21  g057(.a(new_n119_), .b(new_n80_), .c(new_n118_), .O(new_n120_));
  inv1   g058(.a(x08), .O(new_n121_));
  nand4  g059(.a(x35), .b(new_n74_), .c(x27), .d(new_n66_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nor2   g061(.a(x30), .b(x09), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(new_n120_), .c(new_n125_), .O(z11));
  oai21  g064(.a(x37), .b(x35), .c(x27), .O(new_n127_));
  nand2  g065(.a(x10), .b(new_n66_), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n83_), .c(new_n127_), .O(new_n129_));
  nor2   g067(.a(x37), .b(x28), .O(new_n130_));
  oai21  g068(.a(new_n83_), .b(x04), .c(new_n130_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(z12));
  oai21  g070(.a(x32), .b(x30), .c(z08), .O(new_n133_));
  inv1   g071(.a(x13), .O(new_n134_));
  nand4  g072(.a(x40), .b(x39), .c(new_n134_), .d(new_n66_), .O(new_n135_));
  nor2   g073(.a(x19), .b(x18), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x20), .O(new_n137_));
  oai22  g075(.a(new_n137_), .b(new_n70_), .c(new_n135_), .d(new_n75_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x27), .O(new_n139_));
  nor2   g077(.a(x13), .b(x04), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n133_), .c(new_n139_), .O(z13));
  nand2  g080(.a(new_n140_), .b(z08), .O(new_n143_));
  oai21  g081(.a(new_n70_), .b(new_n69_), .c(new_n143_), .O(new_n144_));
  nand2  g082(.a(new_n76_), .b(new_n75_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n143_), .c(new_n137_), .O(new_n147_));
  nand3  g085(.a(new_n88_), .b(new_n86_), .c(x28), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n76_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n147_), .c(new_n144_), .O(z14));
  nand4  g089(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(z15));
  nand2  g091(.a(x22), .b(x01), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(x23), .O(z16));
  inv1   g093(.a(x24), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x23), .c(x22), .d(x01), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z17));
  oai21  g096(.a(new_n80_), .b(x27), .c(new_n118_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x08), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n124_), .c(new_n122_), .O(z18));
  nand2  g099(.a(new_n65_), .b(new_n121_), .O(new_n162_));
  oai21  g100(.a(new_n74_), .b(x27), .c(x35), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n118_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n162_), .c(new_n125_), .O(z20));
  aoi21  g103(.a(new_n123_), .b(new_n120_), .c(new_n125_), .O(z19));
endmodule


