// Benchmark "FAU" written by ABC on Thu Jun 25 19:36:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(x01), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  oai21  g05(.a(x02), .b(new_n25_), .c(new_n29_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  nor2   g07(.a(new_n27_), .b(new_n25_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n26_), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n31_), .c(x00), .O(z0));
  inv1   g10(.a(x03), .O(new_n35_));
  aoi21  g11(.a(new_n26_), .b(new_n35_), .c(x01), .O(new_n36_));
  aoi22  g12(.a(x06), .b(x05), .c(new_n26_), .d(new_n35_), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  nand3  g14(.a(x03), .b(new_n27_), .c(x01), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n38_), .c(x00), .O(z1));
  inv1   g16(.a(x07), .O(new_n41_));
  inv1   g17(.a(x10), .O(new_n42_));
  aoi22  g18(.a(new_n42_), .b(new_n41_), .c(new_n26_), .d(x03), .O(new_n43_));
  nor2   g19(.a(x04), .b(x01), .O(new_n44_));
  oai21  g20(.a(new_n43_), .b(x01), .c(new_n44_), .O(new_n45_));
  nand2  g21(.a(new_n42_), .b(new_n41_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n35_), .O(new_n47_));
  nand2  g23(.a(new_n42_), .b(x09), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n47_), .c(x01), .O(new_n49_));
  aoi21  g25(.a(new_n45_), .b(x02), .c(new_n49_), .O(new_n50_));
  inv1   g26(.a(x08), .O(new_n51_));
  nand3  g27(.a(x10), .b(new_n41_), .c(new_n35_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n48_), .c(x01), .O(new_n53_));
  nor2   g29(.a(x07), .b(new_n27_), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(new_n55_));
  oai21  g31(.a(new_n50_), .b(x00), .c(new_n55_), .O(z2));
  inv1   g32(.a(x00), .O(new_n57_));
  nor2   g33(.a(x09), .b(new_n57_), .O(new_n58_));
  nor2   g34(.a(x12), .b(x00), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n58_), .c(new_n41_), .O(new_n60_));
  inv1   g36(.a(x09), .O(new_n61_));
  nand2  g37(.a(x11), .b(new_n61_), .O(new_n62_));
  nor2   g38(.a(x12), .b(x11), .O(new_n63_));
  aoi21  g39(.a(new_n62_), .b(x00), .c(new_n63_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n60_), .O(z3));
  nand2  g41(.a(x08), .b(x00), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(new_n58_), .c(new_n41_), .O(new_n67_));
  oai21  g43(.a(x11), .b(new_n41_), .c(new_n67_), .O(z4));
  inv1   g44(.a(new_n46_), .O(new_n69_));
  oai21  g45(.a(x09), .b(new_n41_), .c(x00), .O(new_n70_));
  nand3  g46(.a(x12), .b(new_n41_), .c(new_n57_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(x13), .O(z5));
  nor2   g48(.a(x14), .b(new_n25_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n28_), .c(new_n57_), .O(new_n74_));
  oai21  g50(.a(x09), .b(new_n35_), .c(new_n25_), .O(new_n75_));
  nand2  g51(.a(new_n27_), .b(new_n57_), .O(new_n76_));
  nand2  g52(.a(new_n51_), .b(x00), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nand2  g55(.a(new_n28_), .b(new_n51_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n79_), .c(new_n74_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n41_), .O(new_n82_));
  nand4  g58(.a(new_n66_), .b(x09), .c(new_n35_), .d(new_n25_), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(new_n82_), .c(x10), .O(z6));
  nand3  g60(.a(x09), .b(x03), .c(new_n27_), .O(new_n85_));
  nand3  g61(.a(new_n54_), .b(new_n42_), .c(new_n51_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(new_n25_), .O(new_n88_));
  nand2  g64(.a(x03), .b(new_n25_), .O(new_n89_));
  aoi21  g65(.a(new_n89_), .b(new_n51_), .c(new_n46_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n88_), .O(z7));
  inv1   g67(.a(x12), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(new_n27_), .O(new_n93_));
  oai21  g69(.a(new_n93_), .b(new_n89_), .c(x09), .O(new_n94_));
  aoi21  g70(.a(new_n61_), .b(new_n57_), .c(new_n46_), .O(new_n95_));
  nand2  g71(.a(new_n95_), .b(new_n94_), .O(z8));
endmodule


