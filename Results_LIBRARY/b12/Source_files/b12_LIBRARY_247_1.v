// Benchmark "FAU" written by ABC on Thu Jun 25 19:37:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  nand3  g09(.a(x06), .b(x05), .c(x01), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  inv1   g11(.a(x04), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n34_), .c(x02), .O(new_n38_));
  nor2   g14(.a(new_n35_), .b(x02), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(x01), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n38_), .c(x00), .O(z1));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  inv1   g18(.a(new_n42_), .O(new_n43_));
  nand4  g19(.a(new_n43_), .b(new_n31_), .c(new_n35_), .d(new_n25_), .O(new_n44_));
  nand2  g20(.a(x10), .b(new_n35_), .O(new_n45_));
  nand2  g21(.a(new_n42_), .b(x09), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n45_), .c(x01), .O(new_n47_));
  aoi21  g23(.a(new_n44_), .b(x02), .c(new_n47_), .O(new_n48_));
  inv1   g24(.a(x08), .O(new_n49_));
  nor2   g25(.a(x07), .b(new_n27_), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n47_), .c(new_n49_), .O(new_n51_));
  oai21  g27(.a(new_n48_), .b(x00), .c(new_n51_), .O(z2));
  inv1   g28(.a(x00), .O(new_n53_));
  aoi22  g29(.a(x12), .b(new_n53_), .c(x11), .d(x07), .O(z3));
  inv1   g30(.a(x07), .O(new_n55_));
  inv1   g31(.a(x09), .O(new_n56_));
  nand2  g32(.a(x08), .b(x00), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  oai21  g34(.a(x11), .b(new_n55_), .c(new_n58_), .O(z4));
  inv1   g35(.a(x10), .O(new_n60_));
  nor2   g36(.a(x12), .b(x00), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(x13), .c(new_n60_), .d(new_n55_), .O(z5));
  oai21  g38(.a(x09), .b(new_n35_), .c(new_n27_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(x00), .c(new_n26_), .O(new_n64_));
  nand3  g40(.a(new_n56_), .b(x03), .c(new_n27_), .O(new_n65_));
  nand2  g41(.a(x14), .b(x02), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(x01), .O(new_n67_));
  nand2  g43(.a(x02), .b(new_n25_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n67_), .c(new_n65_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  oai21  g46(.a(new_n64_), .b(x08), .c(new_n70_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n55_), .O(new_n72_));
  nand4  g48(.a(new_n57_), .b(x09), .c(new_n35_), .d(new_n25_), .O(new_n73_));
  aoi21  g49(.a(new_n73_), .b(new_n72_), .c(x10), .O(z6));
  nand3  g50(.a(x09), .b(x03), .c(new_n25_), .O(new_n75_));
  nand4  g51(.a(new_n60_), .b(new_n49_), .c(new_n55_), .d(x01), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n27_), .O(new_n78_));
  oai21  g54(.a(x07), .b(new_n27_), .c(x03), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n49_), .c(new_n43_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n78_), .O(z7));
  nand2  g57(.a(new_n30_), .b(x03), .O(new_n82_));
  nand2  g58(.a(new_n42_), .b(new_n25_), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(new_n82_), .c(new_n27_), .O(new_n84_));
  nand2  g60(.a(new_n42_), .b(new_n56_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(x02), .c(x03), .O(new_n86_));
  oai21  g62(.a(new_n86_), .b(new_n84_), .c(new_n53_), .O(new_n87_));
  nand3  g63(.a(x05), .b(x02), .c(new_n53_), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(new_n56_), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(x01), .O(new_n90_));
  inv1   g66(.a(x12), .O(new_n91_));
  nand2  g67(.a(new_n39_), .b(new_n91_), .O(new_n92_));
  aoi21  g68(.a(new_n92_), .b(x09), .c(new_n43_), .O(new_n93_));
  nand3  g69(.a(new_n93_), .b(new_n90_), .c(new_n87_), .O(z8));
endmodule


