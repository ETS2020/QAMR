// Benchmark "FAU" written by ABC on Thu Jun 25 17:03:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nor2   g002(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g003(.a(x19), .b(x13), .O(new_n37_));
  nor2   g004(.a(new_n37_), .b(x24), .O(new_n38_));
  oai21  g005(.a(new_n38_), .b(new_n36_), .c(x05), .O(new_n39_));
  nor2   g006(.a(x13), .b(x05), .O(new_n40_));
  aoi22  g007(.a(new_n40_), .b(x19), .c(new_n36_), .d(x13), .O(new_n41_));
  inv1   g008(.a(x06), .O(new_n42_));
  inv1   g009(.a(x11), .O(new_n43_));
  nor2   g010(.a(x03), .b(x01), .O(new_n44_));
  nor2   g011(.a(x20), .b(x14), .O(new_n45_));
  nand4  g012(.a(new_n45_), .b(new_n44_), .c(new_n43_), .d(new_n42_), .O(new_n46_));
  aoi21  g013(.a(new_n41_), .b(new_n39_), .c(new_n46_), .O(z0));
  nand3  g014(.a(x11), .b(x09), .c(x01), .O(new_n48_));
  inv1   g015(.a(new_n48_), .O(new_n49_));
  nand4  g016(.a(x08), .b(x06), .c(x04), .d(x03), .O(new_n50_));
  inv1   g017(.a(new_n50_), .O(new_n51_));
  nand4  g018(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n52_));
  nand4  g019(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g021(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(new_n55_));
  nor2   g022(.a(x09), .b(x08), .O(new_n56_));
  nor2   g023(.a(x12), .b(x11), .O(new_n57_));
  nor2   g024(.a(x23), .b(x22), .O(new_n58_));
  nor2   g025(.a(new_n34_), .b(x06), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n60_));
  inv1   g027(.a(x04), .O(new_n61_));
  nor2   g028(.a(x17), .b(x16), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(new_n45_), .c(new_n44_), .d(new_n61_), .O(new_n63_));
  oai21  g030(.a(new_n63_), .b(new_n60_), .c(new_n55_), .O(new_n64_));
  nor2   g031(.a(new_n40_), .b(new_n35_), .O(new_n65_));
  nand2  g032(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand4  g033(.a(x20), .b(x17), .c(x16), .d(x15), .O(new_n67_));
  nand4  g034(.a(x23), .b(x22), .c(x14), .d(x12), .O(new_n68_));
  nor2   g035(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n51_), .c(new_n49_), .O(new_n70_));
  nor2   g037(.a(x08), .b(x06), .O(new_n71_));
  nor2   g038(.a(x11), .b(x09), .O(new_n72_));
  inv1   g039(.a(x19), .O(new_n73_));
  nor2   g040(.a(x20), .b(new_n73_), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n72_), .c(new_n71_), .d(new_n58_), .O(new_n75_));
  nor2   g042(.a(x14), .b(x12), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n62_), .c(new_n44_), .d(new_n61_), .O(new_n77_));
  oai21  g044(.a(new_n77_), .b(new_n75_), .c(new_n70_), .O(new_n78_));
  inv1   g045(.a(new_n40_), .O(new_n79_));
  nand3  g046(.a(new_n35_), .b(x13), .c(x05), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  inv1   g049(.a(x01), .O(new_n83_));
  nor2   g050(.a(x02), .b(new_n83_), .O(new_n84_));
  inv1   g051(.a(x22), .O(new_n85_));
  inv1   g052(.a(x23), .O(new_n86_));
  nor3   g053(.a(x24), .b(new_n86_), .c(new_n85_), .O(new_n87_));
  nand4  g054(.a(x21), .b(x17), .c(x16), .d(x14), .O(new_n88_));
  inv1   g055(.a(x10), .O(new_n89_));
  nand4  g056(.a(x12), .b(x11), .c(new_n89_), .d(x09), .O(new_n90_));
  nor2   g057(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n87_), .c(new_n84_), .d(new_n51_), .O(new_n92_));
  nand3  g059(.a(new_n92_), .b(new_n82_), .c(new_n66_), .O(z1));
  nand2  g060(.a(x24), .b(x18), .O(new_n94_));
  nand2  g061(.a(x15), .b(x13), .O(new_n95_));
  oai21  g062(.a(new_n95_), .b(x24), .c(new_n94_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(x05), .O(new_n97_));
  nand3  g064(.a(x24), .b(x18), .c(x13), .O(new_n98_));
  inv1   g065(.a(new_n98_), .O(new_n99_));
  aoi21  g066(.a(new_n40_), .b(x15), .c(new_n99_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n97_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(x20), .O(new_n102_));
  inv1   g069(.a(x02), .O(new_n103_));
  inv1   g070(.a(x21), .O(new_n104_));
  nor2   g071(.a(x24), .b(new_n104_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n89_), .c(new_n103_), .O(new_n106_));
  inv1   g073(.a(x03), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(new_n83_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(x14), .c(x11), .d(x06), .O(new_n109_));
  aoi21  g076(.a(new_n106_), .b(new_n102_), .c(new_n109_), .O(z2));
  inv1   g077(.a(x08), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n42_), .O(new_n112_));
  nand2  g079(.a(new_n108_), .b(new_n112_), .O(new_n113_));
  inv1   g080(.a(x14), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n43_), .O(new_n115_));
  nand3  g082(.a(new_n115_), .b(x20), .c(x18), .O(new_n116_));
  nand3  g083(.a(new_n45_), .b(new_n43_), .c(new_n111_), .O(new_n117_));
  nand2  g084(.a(new_n59_), .b(new_n44_), .O(new_n118_));
  oai22  g085(.a(new_n118_), .b(new_n117_), .c(new_n116_), .d(new_n113_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n65_), .O(new_n120_));
  nand3  g087(.a(new_n115_), .b(x20), .c(x15), .O(new_n121_));
  nand3  g088(.a(new_n44_), .b(new_n114_), .c(new_n43_), .O(new_n122_));
  nand2  g089(.a(new_n74_), .b(new_n71_), .O(new_n123_));
  oai22  g090(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n113_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n81_), .O(new_n125_));
  nand3  g092(.a(new_n115_), .b(new_n89_), .c(x03), .O(new_n126_));
  inv1   g093(.a(new_n126_), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(new_n105_), .c(new_n84_), .d(new_n112_), .O(new_n128_));
  nand3  g095(.a(new_n128_), .b(new_n125_), .c(new_n120_), .O(z3));
  inv1   g096(.a(x17), .O(new_n130_));
  oai21  g097(.a(x23), .b(new_n61_), .c(new_n130_), .O(new_n131_));
  aoi21  g098(.a(new_n131_), .b(new_n85_), .c(x09), .O(new_n132_));
  oai21  g099(.a(new_n132_), .b(x16), .c(new_n111_), .O(new_n133_));
  nand4  g100(.a(new_n35_), .b(new_n89_), .c(new_n103_), .d(x00), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n41_), .c(new_n39_), .O(z5));
  nand2  g102(.a(z5), .b(new_n133_), .O(new_n136_));
  inv1   g103(.a(x16), .O(new_n137_));
  inv1   g104(.a(x09), .O(new_n138_));
  oai21  g105(.a(new_n86_), .b(x04), .c(x17), .O(new_n139_));
  aoi21  g106(.a(new_n139_), .b(x22), .c(new_n138_), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(new_n137_), .c(x08), .O(new_n141_));
  nand3  g108(.a(new_n106_), .b(new_n100_), .c(new_n97_), .O(z7));
  nand2  g109(.a(z7), .b(new_n141_), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n136_), .O(z4));
  inv1   g111(.a(x20), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(x14), .c(new_n42_), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(new_n43_), .c(x03), .O(new_n147_));
  nand3  g114(.a(new_n115_), .b(new_n145_), .c(x18), .O(new_n148_));
  oai21  g115(.a(new_n147_), .b(new_n34_), .c(new_n148_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n65_), .O(new_n150_));
  nand3  g117(.a(new_n115_), .b(new_n145_), .c(x15), .O(new_n151_));
  oai21  g118(.a(new_n147_), .b(new_n73_), .c(new_n151_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n81_), .O(new_n153_));
  oai21  g120(.a(new_n43_), .b(x06), .c(x03), .O(new_n154_));
  oai21  g121(.a(new_n114_), .b(x06), .c(new_n43_), .O(new_n155_));
  aoi21  g122(.a(new_n155_), .b(new_n107_), .c(new_n134_), .O(new_n156_));
  aoi21  g123(.a(new_n154_), .b(z7), .c(new_n156_), .O(new_n157_));
  nand3  g124(.a(new_n157_), .b(new_n153_), .c(new_n150_), .O(z6));
endmodule


