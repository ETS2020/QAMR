// Benchmark "FAU" written by ABC on Thu Jun 25 17:02:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n170_, new_n171_, new_n172_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nor2   g005(.a(x13), .b(x05), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(x19), .O(new_n40_));
  nand3  g007(.a(x24), .b(x13), .c(x07), .O(new_n41_));
  inv1   g008(.a(x02), .O(new_n42_));
  inv1   g009(.a(x10), .O(new_n43_));
  nand4  g010(.a(new_n36_), .b(new_n43_), .c(new_n42_), .d(x00), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n41_), .c(new_n40_), .O(new_n45_));
  nor2   g012(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  inv1   g013(.a(x06), .O(new_n47_));
  inv1   g014(.a(x11), .O(new_n48_));
  nor2   g015(.a(x03), .b(x01), .O(new_n49_));
  nor2   g016(.a(x20), .b(x14), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(new_n46_), .O(z0));
  inv1   g019(.a(x15), .O(new_n53_));
  nor2   g020(.a(x10), .b(x02), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(x21), .O(new_n55_));
  oai21  g022(.a(new_n53_), .b(new_n34_), .c(new_n55_), .O(new_n56_));
  and2   g023(.a(x03), .b(x01), .O(new_n57_));
  nand4  g024(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand4  g026(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n60_));
  nand4  g027(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(new_n59_), .c(new_n57_), .d(new_n56_), .O(new_n63_));
  inv1   g030(.a(x19), .O(new_n64_));
  nand2  g031(.a(new_n54_), .b(x00), .O(new_n65_));
  nand2  g032(.a(x13), .b(x05), .O(new_n66_));
  oai21  g033(.a(new_n66_), .b(new_n64_), .c(new_n65_), .O(new_n67_));
  inv1   g034(.a(x04), .O(new_n68_));
  nor2   g035(.a(x09), .b(x08), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(new_n49_), .c(new_n47_), .d(new_n68_), .O(new_n70_));
  inv1   g037(.a(x16), .O(new_n71_));
  inv1   g038(.a(x17), .O(new_n72_));
  inv1   g039(.a(x22), .O(new_n73_));
  inv1   g040(.a(x23), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nor2   g042(.a(x12), .b(x11), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n50_), .O(new_n77_));
  nor3   g044(.a(new_n77_), .b(new_n75_), .c(new_n70_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n63_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n36_), .O(new_n81_));
  nor2   g048(.a(new_n39_), .b(new_n36_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n73_), .c(x07), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n40_), .O(new_n84_));
  nand3  g051(.a(new_n74_), .b(new_n72_), .c(new_n71_), .O(new_n85_));
  nor3   g052(.a(new_n85_), .b(new_n77_), .c(new_n70_), .O(new_n86_));
  nand4  g053(.a(x12), .b(x11), .c(x09), .d(x08), .O(new_n87_));
  nand3  g054(.a(new_n57_), .b(x06), .c(x04), .O(new_n88_));
  nor2   g055(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand4  g056(.a(x24), .b(x23), .c(x22), .d(x18), .O(new_n90_));
  nand4  g057(.a(x20), .b(x17), .c(x16), .d(x14), .O(new_n91_));
  nor2   g058(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  aoi22  g059(.a(new_n92_), .b(new_n89_), .c(new_n86_), .d(new_n84_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n81_), .O(z1));
  nand2  g061(.a(x24), .b(x18), .O(new_n95_));
  nand3  g062(.a(new_n36_), .b(x15), .c(x13), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(x05), .O(new_n98_));
  nand3  g065(.a(new_n54_), .b(new_n36_), .c(x21), .O(new_n99_));
  nand3  g066(.a(x24), .b(x18), .c(x13), .O(new_n100_));
  nand2  g067(.a(new_n39_), .b(x15), .O(new_n101_));
  nand3  g068(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  inv1   g069(.a(new_n102_), .O(new_n103_));
  nor2   g070(.a(new_n48_), .b(new_n47_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n57_), .c(x20), .d(x14), .O(new_n105_));
  aoi21  g072(.a(new_n103_), .b(new_n98_), .c(new_n105_), .O(z2));
  oai21  g073(.a(new_n66_), .b(new_n53_), .c(new_n55_), .O(new_n107_));
  nand3  g074(.a(x20), .b(x14), .c(x08), .O(new_n108_));
  inv1   g075(.a(new_n108_), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(new_n107_), .c(new_n104_), .d(new_n57_), .O(new_n110_));
  inv1   g077(.a(x20), .O(new_n111_));
  nor2   g078(.a(x14), .b(x08), .O(new_n112_));
  nand3  g079(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n113_));
  inv1   g080(.a(new_n113_), .O(new_n114_));
  nand4  g081(.a(new_n114_), .b(new_n112_), .c(new_n67_), .d(new_n111_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n110_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n36_), .O(new_n117_));
  nand3  g084(.a(new_n57_), .b(x08), .c(x06), .O(new_n118_));
  nand4  g085(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n119_));
  nand3  g086(.a(new_n112_), .b(new_n111_), .c(x07), .O(new_n120_));
  oai22  g087(.a(new_n120_), .b(new_n113_), .c(new_n119_), .d(new_n118_), .O(new_n121_));
  nand4  g088(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n122_));
  nand3  g089(.a(new_n112_), .b(new_n111_), .c(x19), .O(new_n123_));
  oai22  g090(.a(new_n123_), .b(new_n113_), .c(new_n122_), .d(new_n118_), .O(new_n124_));
  aoi22  g091(.a(new_n124_), .b(new_n39_), .c(new_n121_), .d(new_n82_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n117_), .O(z3));
  aoi21  g093(.a(new_n74_), .b(x04), .c(x17), .O(new_n127_));
  nand2  g094(.a(new_n44_), .b(new_n41_), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(new_n38_), .c(new_n73_), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(new_n40_), .c(new_n127_), .O(new_n130_));
  inv1   g097(.a(x09), .O(new_n131_));
  nand2  g098(.a(new_n82_), .b(x07), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(new_n44_), .c(new_n131_), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n130_), .c(new_n71_), .O(new_n134_));
  inv1   g101(.a(new_n99_), .O(new_n135_));
  nand2  g102(.a(x23), .b(new_n68_), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(x17), .c(new_n73_), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(new_n131_), .c(new_n135_), .O(new_n138_));
  nand3  g105(.a(new_n36_), .b(x15), .c(x05), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n95_), .O(new_n140_));
  inv1   g107(.a(x13), .O(new_n141_));
  nor2   g108(.a(new_n141_), .b(x09), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(new_n137_), .c(new_n140_), .O(new_n143_));
  oai21  g110(.a(new_n95_), .b(new_n34_), .c(new_n101_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n131_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n143_), .c(new_n138_), .O(new_n146_));
  inv1   g113(.a(new_n39_), .O(new_n147_));
  oai21  g114(.a(new_n66_), .b(x24), .c(new_n147_), .O(new_n148_));
  aoi21  g115(.a(new_n71_), .b(x09), .c(x08), .O(new_n149_));
  oai22  g116(.a(new_n149_), .b(new_n64_), .c(new_n53_), .d(x08), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  inv1   g118(.a(x21), .O(new_n152_));
  nand2  g119(.a(x08), .b(x00), .O(new_n153_));
  oai21  g120(.a(new_n152_), .b(x08), .c(new_n153_), .O(new_n154_));
  nor3   g121(.a(x24), .b(x10), .c(x02), .O(new_n155_));
  inv1   g122(.a(x18), .O(new_n156_));
  nand2  g123(.a(x08), .b(x07), .O(new_n157_));
  oai21  g124(.a(new_n156_), .b(x08), .c(new_n157_), .O(new_n158_));
  aoi22  g125(.a(new_n158_), .b(new_n82_), .c(new_n155_), .d(new_n154_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n151_), .O(new_n160_));
  aoi21  g127(.a(new_n146_), .b(x16), .c(new_n160_), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n134_), .O(z4));
  inv1   g129(.a(new_n46_), .O(z5));
  inv1   g130(.a(x14), .O(new_n164_));
  nand2  g131(.a(x20), .b(new_n164_), .O(new_n165_));
  aoi21  g132(.a(new_n165_), .b(new_n47_), .c(x11), .O(new_n166_));
  oai21  g133(.a(new_n166_), .b(x03), .c(z5), .O(new_n167_));
  inv1   g134(.a(x03), .O(new_n168_));
  nand2  g135(.a(new_n103_), .b(new_n98_), .O(z7));
  nand2  g136(.a(new_n111_), .b(x14), .O(new_n170_));
  aoi21  g137(.a(new_n170_), .b(x06), .c(new_n48_), .O(new_n171_));
  oai21  g138(.a(new_n171_), .b(new_n168_), .c(z7), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(new_n167_), .O(z6));
endmodule


