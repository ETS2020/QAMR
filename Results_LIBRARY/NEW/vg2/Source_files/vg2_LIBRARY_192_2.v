// Benchmark "FAU" written by ABC on Thu Jun 25 17:02:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  oai21  g004(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x05), .O(new_n39_));
  nor2   g006(.a(new_n36_), .b(new_n35_), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  aoi22  g008(.a(new_n41_), .b(x19), .c(new_n40_), .d(x13), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n39_), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(new_n34_), .O(new_n44_));
  inv1   g011(.a(x02), .O(new_n45_));
  nor2   g012(.a(x24), .b(x10), .O(new_n46_));
  nand3  g013(.a(new_n46_), .b(new_n45_), .c(x00), .O(new_n47_));
  inv1   g014(.a(x20), .O(new_n48_));
  nor2   g015(.a(x03), .b(x01), .O(new_n49_));
  nor2   g016(.a(x14), .b(x11), .O(new_n50_));
  nand3  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  aoi21  g018(.a(new_n47_), .b(new_n44_), .c(new_n51_), .O(z0));
  nand3  g019(.a(x11), .b(x09), .c(x04), .O(new_n53_));
  inv1   g020(.a(new_n53_), .O(new_n54_));
  nand4  g021(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n55_));
  inv1   g022(.a(new_n55_), .O(new_n56_));
  nand4  g023(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n57_));
  nand4  g024(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g026(.a(new_n59_), .b(new_n56_), .c(new_n54_), .O(new_n60_));
  nor2   g027(.a(x23), .b(x22), .O(new_n61_));
  nor2   g028(.a(x09), .b(x08), .O(new_n62_));
  nor2   g029(.a(new_n35_), .b(x06), .O(new_n63_));
  nor2   g030(.a(x20), .b(x17), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n61_), .O(new_n65_));
  nor3   g032(.a(x04), .b(x03), .c(x01), .O(new_n66_));
  inv1   g033(.a(x12), .O(new_n67_));
  inv1   g034(.a(x16), .O(new_n68_));
  nand3  g035(.a(new_n50_), .b(new_n68_), .c(new_n67_), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nand2  g037(.a(new_n70_), .b(new_n66_), .O(new_n71_));
  oai21  g038(.a(new_n71_), .b(new_n65_), .c(new_n60_), .O(new_n72_));
  nor2   g039(.a(new_n41_), .b(new_n36_), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g041(.a(x20), .b(x17), .c(x16), .d(x15), .O(new_n75_));
  nand4  g042(.a(x23), .b(x22), .c(x14), .d(x12), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g044(.a(new_n77_), .b(new_n56_), .c(new_n54_), .O(new_n78_));
  nor2   g045(.a(x08), .b(x06), .O(new_n79_));
  nor2   g046(.a(x11), .b(x09), .O(new_n80_));
  inv1   g047(.a(x19), .O(new_n81_));
  nor2   g048(.a(x20), .b(new_n81_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n80_), .c(new_n79_), .d(new_n61_), .O(new_n83_));
  inv1   g050(.a(x14), .O(new_n84_));
  nor2   g051(.a(x17), .b(x16), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n66_), .c(new_n84_), .d(new_n67_), .O(new_n86_));
  oai21  g053(.a(new_n86_), .b(new_n83_), .c(new_n78_), .O(new_n87_));
  nand2  g054(.a(x13), .b(x05), .O(new_n88_));
  nor2   g055(.a(new_n88_), .b(x24), .O(new_n89_));
  or2    g056(.a(new_n89_), .b(new_n41_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  inv1   g058(.a(x00), .O(new_n92_));
  nor2   g059(.a(x01), .b(new_n92_), .O(new_n93_));
  nor2   g060(.a(x03), .b(x02), .O(new_n94_));
  nand2  g061(.a(new_n61_), .b(new_n36_), .O(new_n95_));
  inv1   g062(.a(x04), .O(new_n96_));
  inv1   g063(.a(x08), .O(new_n97_));
  inv1   g064(.a(x09), .O(new_n98_));
  inv1   g065(.a(x10), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n96_), .O(new_n100_));
  nor3   g067(.a(new_n100_), .b(new_n95_), .c(new_n69_), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n94_), .c(new_n93_), .d(new_n64_), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n91_), .c(new_n74_), .O(z1));
  inv1   g070(.a(x18), .O(new_n104_));
  nor2   g071(.a(new_n36_), .b(new_n104_), .O(new_n105_));
  nand2  g072(.a(x15), .b(x13), .O(new_n106_));
  nor2   g073(.a(new_n106_), .b(x24), .O(new_n107_));
  oai21  g074(.a(new_n107_), .b(new_n105_), .c(x05), .O(new_n108_));
  aoi22  g075(.a(new_n105_), .b(x13), .c(new_n41_), .d(x15), .O(new_n109_));
  inv1   g076(.a(x11), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(new_n34_), .O(new_n111_));
  nor2   g078(.a(new_n48_), .b(new_n84_), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(new_n111_), .c(x03), .d(x01), .O(new_n113_));
  aoi21  g080(.a(new_n109_), .b(new_n108_), .c(new_n113_), .O(z2));
  nand4  g081(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n115_));
  nand4  g082(.a(new_n48_), .b(new_n84_), .c(new_n110_), .d(new_n97_), .O(new_n116_));
  nand2  g083(.a(new_n63_), .b(new_n49_), .O(new_n117_));
  oai22  g084(.a(new_n117_), .b(new_n116_), .c(new_n115_), .d(new_n55_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n73_), .O(new_n119_));
  nand4  g086(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n120_));
  nand2  g087(.a(new_n50_), .b(new_n49_), .O(new_n121_));
  nand2  g088(.a(new_n82_), .b(new_n79_), .O(new_n122_));
  oai22  g089(.a(new_n122_), .b(new_n121_), .c(new_n120_), .d(new_n55_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n90_), .O(new_n124_));
  inv1   g091(.a(new_n116_), .O(new_n125_));
  nand4  g092(.a(new_n125_), .b(new_n94_), .c(new_n93_), .d(new_n46_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(new_n124_), .c(new_n119_), .O(z3));
  inv1   g094(.a(x22), .O(new_n128_));
  inv1   g095(.a(x17), .O(new_n129_));
  oai21  g096(.a(x23), .b(new_n96_), .c(new_n129_), .O(new_n130_));
  aoi21  g097(.a(new_n130_), .b(new_n128_), .c(x09), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(x16), .c(new_n97_), .O(new_n132_));
  nand3  g099(.a(new_n47_), .b(new_n42_), .c(new_n39_), .O(z5));
  nand2  g100(.a(z5), .b(new_n132_), .O(new_n134_));
  inv1   g101(.a(x23), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(x04), .c(x17), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(x22), .c(new_n98_), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(new_n68_), .c(x08), .O(new_n138_));
  nor2   g105(.a(x10), .b(x02), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n36_), .c(x21), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n109_), .c(new_n108_), .O(z7));
  nand2  g108(.a(z7), .b(new_n138_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n134_), .O(z4));
  nand2  g110(.a(new_n110_), .b(x03), .O(new_n144_));
  nand3  g111(.a(new_n144_), .b(new_n139_), .c(x21), .O(new_n145_));
  aoi21  g112(.a(new_n48_), .b(x14), .c(new_n34_), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(new_n110_), .c(x03), .O(new_n147_));
  nand3  g114(.a(x19), .b(new_n110_), .c(x06), .O(new_n148_));
  inv1   g115(.a(new_n148_), .O(new_n149_));
  aoi21  g116(.a(new_n147_), .b(x15), .c(new_n149_), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(new_n88_), .c(new_n145_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n36_), .O(new_n152_));
  inv1   g119(.a(new_n150_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n41_), .O(new_n154_));
  inv1   g121(.a(x03), .O(new_n155_));
  nand2  g122(.a(new_n50_), .b(x20), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nand2  g124(.a(new_n147_), .b(x18), .O(new_n158_));
  nand3  g125(.a(new_n110_), .b(x07), .c(x06), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  aoi22  g127(.a(new_n160_), .b(new_n73_), .c(new_n157_), .d(z5), .O(new_n161_));
  nand3  g128(.a(new_n161_), .b(new_n154_), .c(new_n152_), .O(z6));
endmodule


