// Benchmark "FAU" written by ABC on Mon Jul 27 18:47:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n72_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n96_, new_n98_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x23), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x18), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n51_), .b(x17), .c(new_n55_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n48_), .c(new_n49_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(x07), .O(new_n61_));
  oai21  g16(.a(new_n58_), .b(x25), .c(new_n61_), .O(z01));
  nor2   g17(.a(new_n58_), .b(x25), .O(z03));
  xor2a  g18(.a(x09), .b(x00), .O(new_n64_));
  xor2a  g19(.a(x10), .b(x01), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  xor2a  g22(.a(x12), .b(x03), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n66_), .c(x08), .O(z04));
  nor2   g25(.a(x13), .b(x08), .O(z05));
  inv1   g26(.a(x14), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(x08), .O(z06));
  nand2  g28(.a(new_n46_), .b(x06), .O(z07));
  inv1   g29(.a(x17), .O(new_n75_));
  nand3  g30(.a(x24), .b(x22), .c(x21), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n53_), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(x25), .c(new_n75_), .O(new_n78_));
  inv1   g33(.a(new_n76_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x25), .c(x20), .O(new_n80_));
  oai21  g35(.a(x25), .b(x24), .c(x23), .O(new_n81_));
  inv1   g36(.a(x22), .O(new_n82_));
  inv1   g37(.a(x21), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(x19), .c(x18), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  and2   g40(.a(x18), .b(x17), .O(new_n86_));
  nor2   g41(.a(x20), .b(new_n52_), .O(new_n87_));
  nor2   g42(.a(x22), .b(x21), .O(new_n88_));
  nor2   g43(.a(x24), .b(x23), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n85_), .c(new_n49_), .d(new_n82_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(x25), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n81_), .c(new_n80_), .d(new_n78_), .O(z08));
  inv1   g48(.a(x07), .O(new_n94_));
  inv1   g49(.a(x15), .O(new_n95_));
  nand3  g50(.a(new_n60_), .b(new_n95_), .c(new_n94_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z09));
  nand4  g52(.a(new_n59_), .b(new_n75_), .c(new_n95_), .d(new_n94_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z10));
  xor2a  g54(.a(x18), .b(x17), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n59_), .c(new_n95_), .d(new_n94_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z11));
  nand3  g57(.a(new_n52_), .b(x18), .c(x17), .O(new_n103_));
  oai21  g58(.a(new_n86_), .b(new_n52_), .c(new_n103_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n59_), .c(new_n95_), .d(new_n94_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(z12));
  nand2  g61(.a(new_n87_), .b(new_n86_), .O(new_n107_));
  oai21  g62(.a(new_n52_), .b(new_n75_), .c(x20), .O(new_n108_));
  nor2   g63(.a(x15), .b(x07), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n108_), .c(new_n107_), .d(new_n59_), .O(z13));
  nand2  g65(.a(new_n107_), .b(x21), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n86_), .c(x19), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(new_n109_), .d(new_n59_), .O(z14));
  nand2  g69(.a(new_n113_), .b(x22), .O(new_n115_));
  nand3  g70(.a(x19), .b(x18), .c(x17), .O(new_n116_));
  inv1   g71(.a(new_n116_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n88_), .c(new_n50_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n115_), .c(new_n109_), .d(new_n59_), .O(z15));
  nand2  g74(.a(new_n118_), .b(x23), .O(new_n120_));
  nand4  g75(.a(new_n117_), .b(new_n112_), .c(new_n48_), .d(new_n82_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n120_), .c(new_n109_), .d(new_n59_), .O(z16));
  nand3  g77(.a(new_n89_), .b(new_n82_), .c(new_n83_), .O(new_n123_));
  nor2   g78(.a(new_n123_), .b(x20), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(x19), .c(x18), .d(x17), .O(new_n125_));
  nand3  g80(.a(new_n59_), .b(new_n95_), .c(new_n94_), .O(new_n126_));
  aoi21  g81(.a(new_n121_), .b(x24), .c(new_n126_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n125_), .O(z17));
  nand2  g83(.a(new_n90_), .b(x25), .O(new_n129_));
  nor3   g84(.a(x25), .b(x24), .c(x23), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n109_), .d(new_n59_), .O(z18));
  buf1   g87(.a(x16), .O(z02));
endmodule


