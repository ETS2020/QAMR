// Benchmark "FAU" written by ABC on Mon Jul 27 18:46:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n69_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n122_,
    new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x07), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  oai21  g06(.a(x19), .b(x17), .c(new_n51_), .O(new_n52_));
  inv1   g07(.a(x21), .O(new_n53_));
  inv1   g08(.a(x22), .O(new_n54_));
  nor2   g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n52_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n49_), .c(new_n50_), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n48_), .O(z01));
  inv1   g14(.a(new_n57_), .O(z03));
  xor2a  g15(.a(x09), .b(x00), .O(new_n61_));
  xor2a  g16(.a(x10), .b(x01), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  xor2a  g18(.a(x11), .b(x02), .O(new_n64_));
  xor2a  g19(.a(x12), .b(x03), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n63_), .c(x08), .O(z04));
  nor2   g22(.a(x13), .b(x08), .O(z05));
  inv1   g23(.a(x14), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(x08), .O(z06));
  nand2  g25(.a(new_n46_), .b(x06), .O(z07));
  inv1   g26(.a(x17), .O(new_n72_));
  inv1   g27(.a(x19), .O(new_n73_));
  nand4  g28(.a(x24), .b(x22), .c(x21), .d(new_n73_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n50_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand3  g31(.a(x24), .b(x22), .c(x21), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n50_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x20), .O(new_n79_));
  oai21  g34(.a(x25), .b(x24), .c(x23), .O(new_n80_));
  inv1   g35(.a(x18), .O(new_n81_));
  nor2   g36(.a(new_n73_), .b(new_n81_), .O(new_n82_));
  nand3  g37(.a(x19), .b(x18), .c(x17), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  nor2   g39(.a(x21), .b(x20), .O(new_n85_));
  nor2   g40(.a(x23), .b(x22), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n82_), .c(new_n54_), .d(new_n53_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(x25), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n80_), .c(new_n79_), .d(new_n76_), .O(z08));
  inv1   g45(.a(x15), .O(new_n91_));
  nand4  g46(.a(new_n91_), .b(new_n48_), .c(x05), .d(x04), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand4  g48(.a(new_n58_), .b(new_n72_), .c(new_n91_), .d(new_n48_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n58_), .c(new_n91_), .d(new_n48_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z11));
  nor2   g53(.a(new_n81_), .b(new_n72_), .O(new_n99_));
  nand3  g54(.a(new_n73_), .b(x18), .c(x17), .O(new_n100_));
  oai21  g55(.a(new_n99_), .b(new_n73_), .c(new_n100_), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n58_), .c(new_n91_), .d(new_n48_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z12));
  nand2  g58(.a(new_n83_), .b(x20), .O(new_n104_));
  nor2   g59(.a(x15), .b(x07), .O(new_n105_));
  nand3  g60(.a(new_n99_), .b(new_n51_), .c(x19), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n105_), .c(new_n104_), .d(new_n58_), .O(z13));
  nand2  g62(.a(new_n106_), .b(x21), .O(new_n108_));
  nand3  g63(.a(new_n99_), .b(new_n85_), .c(x19), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n108_), .c(new_n105_), .d(new_n58_), .O(z14));
  nand2  g65(.a(new_n109_), .b(x22), .O(new_n111_));
  nor2   g66(.a(x22), .b(x21), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n84_), .c(new_n51_), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(new_n105_), .d(new_n58_), .O(z15));
  nand3  g69(.a(new_n86_), .b(new_n53_), .c(new_n51_), .O(new_n115_));
  inv1   g70(.a(new_n115_), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(x19), .c(x18), .d(x17), .O(new_n117_));
  nand3  g72(.a(new_n58_), .b(new_n91_), .c(new_n48_), .O(new_n118_));
  aoi21  g73(.a(new_n113_), .b(x23), .c(new_n118_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n117_), .O(z16));
  nand4  g75(.a(new_n117_), .b(new_n105_), .c(new_n58_), .d(new_n49_), .O(z17));
  nand2  g76(.a(new_n87_), .b(x25), .O(new_n122_));
  inv1   g77(.a(new_n106_), .O(new_n123_));
  nor2   g78(.a(x25), .b(x23), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n112_), .c(new_n123_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n122_), .c(new_n105_), .d(new_n58_), .O(z18));
  buf1   g81(.a(x16), .O(z02));
endmodule


