// Benchmark "FAU" written by ABC on Mon Jul 27 18:46:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n71_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x25), .O(new_n48_));
  inv1   g03(.a(x20), .O(new_n49_));
  inv1   g04(.a(x17), .O(new_n50_));
  inv1   g05(.a(x18), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  aoi21  g09(.a(new_n53_), .b(new_n49_), .c(new_n54_), .O(new_n55_));
  oai21  g10(.a(new_n55_), .b(x23), .c(x24), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n48_), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n57_), .O(z01));
  inv1   g16(.a(new_n57_), .O(z03));
  xor2a  g17(.a(x09), .b(x00), .O(new_n63_));
  xor2a  g18(.a(x10), .b(x01), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  xor2a  g20(.a(x11), .b(x02), .O(new_n66_));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n65_), .c(x08), .O(z04));
  nor2   g24(.a(x13), .b(x08), .O(z05));
  inv1   g25(.a(x14), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(x08), .O(z06));
  nand2  g27(.a(new_n46_), .b(x06), .O(z07));
  nand2  g28(.a(new_n52_), .b(new_n51_), .O(new_n74_));
  nand3  g29(.a(x24), .b(x22), .c(x21), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(new_n74_), .c(new_n48_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n50_), .O(new_n77_));
  inv1   g32(.a(x24), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(new_n49_), .c(new_n48_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x21), .O(new_n80_));
  oai21  g35(.a(x25), .b(x24), .c(x23), .O(new_n81_));
  inv1   g36(.a(x22), .O(new_n82_));
  nor2   g37(.a(new_n52_), .b(new_n51_), .O(new_n83_));
  nand3  g38(.a(x19), .b(x18), .c(x17), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  nor2   g40(.a(x22), .b(x21), .O(new_n86_));
  nor2   g41(.a(x24), .b(x23), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n83_), .c(new_n78_), .d(new_n82_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(x25), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n81_), .c(new_n80_), .d(new_n77_), .O(z08));
  inv1   g46(.a(x07), .O(new_n92_));
  inv1   g47(.a(x15), .O(new_n93_));
  nand3  g48(.a(new_n59_), .b(new_n93_), .c(new_n92_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z09));
  nand4  g50(.a(new_n58_), .b(new_n50_), .c(new_n93_), .d(new_n92_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z10));
  xor2a  g52(.a(x18), .b(x17), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n58_), .c(new_n93_), .d(new_n92_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z11));
  nor2   g55(.a(new_n51_), .b(new_n50_), .O(new_n101_));
  nand3  g56(.a(new_n52_), .b(x18), .c(x17), .O(new_n102_));
  oai21  g57(.a(new_n101_), .b(new_n52_), .c(new_n102_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n58_), .c(new_n93_), .d(new_n92_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(z12));
  nand2  g60(.a(new_n84_), .b(x20), .O(new_n106_));
  nor2   g61(.a(x15), .b(x07), .O(new_n107_));
  nand3  g62(.a(new_n101_), .b(new_n49_), .c(x19), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n107_), .c(new_n106_), .d(new_n58_), .O(z13));
  nand2  g64(.a(new_n108_), .b(x21), .O(new_n110_));
  inv1   g65(.a(x21), .O(new_n111_));
  nand4  g66(.a(new_n101_), .b(new_n111_), .c(new_n49_), .d(x19), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n110_), .c(new_n107_), .d(new_n58_), .O(z14));
  nand3  g68(.a(new_n101_), .b(new_n111_), .c(x19), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x22), .O(new_n115_));
  nand3  g70(.a(new_n101_), .b(new_n86_), .c(x19), .O(new_n116_));
  and2   g71(.a(new_n116_), .b(new_n49_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n115_), .c(new_n107_), .d(new_n58_), .O(z15));
  nand2  g73(.a(new_n116_), .b(x23), .O(new_n119_));
  nor2   g74(.a(x23), .b(x22), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n85_), .c(new_n111_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n119_), .c(new_n107_), .d(new_n58_), .O(z16));
  nand3  g77(.a(new_n87_), .b(new_n82_), .c(new_n111_), .O(new_n123_));
  inv1   g78(.a(new_n123_), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(x19), .c(x18), .d(x17), .O(new_n125_));
  nand3  g80(.a(new_n58_), .b(new_n93_), .c(new_n92_), .O(new_n126_));
  aoi21  g81(.a(new_n121_), .b(x24), .c(new_n126_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n125_), .O(z17));
  nand2  g83(.a(new_n88_), .b(x25), .O(new_n129_));
  inv1   g84(.a(new_n114_), .O(new_n130_));
  nand4  g85(.a(new_n120_), .b(new_n130_), .c(new_n48_), .d(new_n78_), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n129_), .c(new_n107_), .d(new_n58_), .O(z18));
  buf1   g87(.a(x16), .O(z02));
endmodule


