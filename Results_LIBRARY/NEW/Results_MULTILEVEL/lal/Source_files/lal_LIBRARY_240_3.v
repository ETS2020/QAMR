// Benchmark "FAU" written by ABC on Mon Jul 27 18:47:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n65_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n48_));
  oai21  g03(.a(new_n48_), .b(x20), .c(x21), .O(new_n49_));
  oai21  g04(.a(x23), .b(x22), .c(x24), .O(new_n50_));
  aoi21  g05(.a(new_n49_), .b(x22), .c(new_n50_), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  nor2   g08(.a(new_n53_), .b(x07), .O(new_n54_));
  oai21  g09(.a(new_n51_), .b(x25), .c(new_n54_), .O(z01));
  nor2   g10(.a(new_n51_), .b(x25), .O(z03));
  xor2a  g11(.a(x09), .b(x00), .O(new_n57_));
  xor2a  g12(.a(x10), .b(x01), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  xor2a  g14(.a(x11), .b(x02), .O(new_n60_));
  xor2a  g15(.a(x12), .b(x03), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n59_), .c(x08), .O(z04));
  nor2   g18(.a(x13), .b(x08), .O(z05));
  inv1   g19(.a(x14), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(x08), .O(z06));
  nand2  g21(.a(new_n46_), .b(x06), .O(z07));
  inv1   g22(.a(x17), .O(new_n68_));
  nand3  g23(.a(x24), .b(x22), .c(x21), .O(new_n69_));
  nor3   g24(.a(new_n69_), .b(x19), .c(x18), .O(new_n70_));
  oai21  g25(.a(new_n70_), .b(x25), .c(new_n68_), .O(new_n71_));
  inv1   g26(.a(new_n69_), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(x25), .c(x20), .O(new_n73_));
  oai21  g28(.a(x25), .b(x24), .c(x23), .O(new_n74_));
  inv1   g29(.a(x22), .O(new_n75_));
  inv1   g30(.a(x24), .O(new_n76_));
  inv1   g31(.a(x21), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(x19), .c(x18), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(new_n79_));
  and2   g34(.a(x18), .b(x17), .O(new_n80_));
  inv1   g35(.a(x19), .O(new_n81_));
  nor2   g36(.a(x20), .b(new_n81_), .O(new_n82_));
  nor2   g37(.a(x22), .b(x21), .O(new_n83_));
  nor2   g38(.a(x24), .b(x23), .O(new_n84_));
  nand4  g39(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(new_n80_), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(new_n79_), .c(new_n76_), .d(new_n75_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x25), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n74_), .c(new_n73_), .d(new_n71_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  inv1   g44(.a(x15), .O(new_n90_));
  nand3  g45(.a(new_n53_), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z09));
  nand4  g47(.a(new_n52_), .b(new_n68_), .c(new_n90_), .d(new_n89_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  xor2a  g49(.a(x18), .b(x17), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n52_), .c(new_n90_), .d(new_n89_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z11));
  nand3  g52(.a(new_n81_), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n80_), .b(new_n81_), .c(new_n98_), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n52_), .c(new_n90_), .d(new_n89_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z12));
  nand2  g56(.a(new_n82_), .b(new_n80_), .O(new_n102_));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x20), .O(new_n104_));
  nor2   g59(.a(x15), .b(x07), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n104_), .c(new_n102_), .d(new_n52_), .O(z13));
  nand2  g61(.a(new_n102_), .b(x21), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n80_), .c(x19), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n107_), .c(new_n105_), .d(new_n52_), .O(z14));
  inv1   g65(.a(x20), .O(new_n111_));
  inv1   g66(.a(x23), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(x22), .c(x21), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n111_), .c(x19), .d(x18), .O(new_n114_));
  nand3  g69(.a(new_n52_), .b(new_n90_), .c(new_n89_), .O(new_n115_));
  aoi21  g70(.a(new_n109_), .b(x22), .c(new_n115_), .O(new_n116_));
  oai21  g71(.a(new_n114_), .b(new_n68_), .c(new_n116_), .O(z15));
  nand2  g72(.a(new_n109_), .b(x23), .O(new_n118_));
  inv1   g73(.a(new_n103_), .O(new_n119_));
  nand4  g74(.a(new_n108_), .b(new_n119_), .c(new_n112_), .d(new_n75_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n118_), .c(new_n105_), .d(new_n52_), .O(z16));
  nand3  g76(.a(new_n84_), .b(new_n75_), .c(new_n77_), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(x20), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(x19), .c(x18), .d(x17), .O(new_n124_));
  aoi21  g79(.a(new_n120_), .b(x24), .c(new_n115_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n124_), .O(z17));
  nand2  g81(.a(new_n85_), .b(x25), .O(new_n127_));
  nor3   g82(.a(x25), .b(x24), .c(x23), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n83_), .c(new_n82_), .d(new_n80_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n127_), .c(new_n105_), .d(new_n52_), .O(z18));
  buf1   g85(.a(x16), .O(z02));
endmodule


