// Benchmark "FAU" written by ABC on Mon Jul 27 18:46:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n69_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x23), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x21), .O(new_n50_));
  inv1   g05(.a(x22), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(new_n50_), .c(x24), .O(new_n52_));
  nor3   g07(.a(x19), .b(x18), .c(x17), .O(new_n53_));
  oai21  g08(.a(new_n53_), .b(x20), .c(new_n52_), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n48_), .c(new_n49_), .O(new_n55_));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(x07), .O(new_n58_));
  oai21  g13(.a(new_n55_), .b(x25), .c(new_n58_), .O(z01));
  nor2   g14(.a(new_n55_), .b(x25), .O(z03));
  xor2a  g15(.a(x09), .b(x00), .O(new_n61_));
  xor2a  g16(.a(x10), .b(x01), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  xor2a  g18(.a(x11), .b(x02), .O(new_n64_));
  xor2a  g19(.a(x12), .b(x03), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n63_), .c(x08), .O(z04));
  nor2   g22(.a(x13), .b(x08), .O(z05));
  inv1   g23(.a(x14), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(x08), .O(z06));
  nand2  g25(.a(new_n46_), .b(x06), .O(z07));
  inv1   g26(.a(x17), .O(new_n72_));
  nand3  g27(.a(x24), .b(x22), .c(x21), .O(new_n73_));
  nor3   g28(.a(new_n73_), .b(x19), .c(x18), .O(new_n74_));
  oai21  g29(.a(new_n74_), .b(x25), .c(new_n72_), .O(new_n75_));
  inv1   g30(.a(new_n73_), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(x25), .c(x20), .O(new_n77_));
  oai21  g32(.a(x25), .b(x24), .c(x23), .O(new_n78_));
  nand3  g33(.a(new_n50_), .b(x19), .c(x18), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(new_n80_));
  and2   g35(.a(x18), .b(x17), .O(new_n81_));
  inv1   g36(.a(x19), .O(new_n82_));
  nor2   g37(.a(x20), .b(new_n82_), .O(new_n83_));
  nor2   g38(.a(x22), .b(x21), .O(new_n84_));
  nor2   g39(.a(x24), .b(x23), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n81_), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n80_), .c(new_n49_), .d(new_n51_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(x25), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n78_), .c(new_n77_), .d(new_n75_), .O(z08));
  inv1   g44(.a(x07), .O(new_n90_));
  inv1   g45(.a(x15), .O(new_n91_));
  nand3  g46(.a(new_n57_), .b(new_n91_), .c(new_n90_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand4  g48(.a(new_n56_), .b(new_n72_), .c(new_n91_), .d(new_n90_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  xor2a  g50(.a(x18), .b(x17), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n56_), .c(new_n91_), .d(new_n90_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z11));
  nand3  g53(.a(new_n82_), .b(x18), .c(x17), .O(new_n99_));
  oai21  g54(.a(new_n81_), .b(new_n82_), .c(new_n99_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n56_), .c(new_n91_), .d(new_n90_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z12));
  nand2  g57(.a(new_n83_), .b(new_n81_), .O(new_n103_));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x20), .O(new_n105_));
  nor2   g60(.a(x15), .b(x07), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n105_), .c(new_n103_), .d(new_n56_), .O(z13));
  nand2  g62(.a(new_n103_), .b(x21), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n81_), .c(x19), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n108_), .c(new_n106_), .d(new_n56_), .O(z14));
  nand2  g66(.a(new_n110_), .b(x22), .O(new_n112_));
  inv1   g67(.a(x20), .O(new_n113_));
  inv1   g68(.a(new_n104_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n84_), .c(new_n113_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n112_), .c(new_n106_), .d(new_n56_), .O(z15));
  nand2  g71(.a(new_n115_), .b(x23), .O(new_n117_));
  nand4  g72(.a(new_n109_), .b(new_n114_), .c(new_n48_), .d(new_n51_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n117_), .c(new_n106_), .d(new_n56_), .O(z16));
  nand3  g74(.a(new_n85_), .b(new_n51_), .c(new_n50_), .O(new_n120_));
  nor2   g75(.a(new_n120_), .b(x20), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(x19), .c(x18), .d(x17), .O(new_n122_));
  nand3  g77(.a(new_n56_), .b(new_n91_), .c(new_n90_), .O(new_n123_));
  aoi21  g78(.a(new_n118_), .b(x24), .c(new_n123_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n122_), .O(z17));
  nand2  g80(.a(new_n86_), .b(x25), .O(new_n126_));
  nor3   g81(.a(x25), .b(x24), .c(x23), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n84_), .c(new_n83_), .d(new_n81_), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n126_), .c(new_n106_), .d(new_n56_), .O(z18));
  buf1   g84(.a(x16), .O(z02));
endmodule


