// Benchmark "FAU" written by ABC on Mon Jul 27 18:14:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x13), .O(new_n30_));
  inv1   g02(.a(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand4  g05(.a(x12), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n34_));
  nor3   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  and2   g09(.a(x08), .b(x00), .O(z1));
  nand2  g10(.a(x08), .b(x01), .O(new_n39_));
  inv1   g11(.a(x12), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n39_), .O(z2));
  xor2a  g14(.a(x13), .b(x12), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n44_));
  nand2  g16(.a(x08), .b(x02), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(z3));
  nand2  g18(.a(x13), .b(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x14), .O(new_n48_));
  nand3  g20(.a(new_n31_), .b(x13), .c(x12), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand4  g22(.a(new_n50_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n51_));
  nand2  g23(.a(x08), .b(x03), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n51_), .O(z4));
  nand3  g25(.a(x14), .b(x13), .c(x12), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x15), .O(new_n55_));
  inv1   g27(.a(x15), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x14), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(new_n47_), .c(new_n55_), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n59_));
  nand2  g31(.a(x08), .b(x04), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(z5));
  nand2  g33(.a(x15), .b(x14), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(new_n47_), .c(x16), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(x15), .c(x14), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n47_), .c(new_n63_), .O(new_n66_));
  nand4  g38(.a(new_n66_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n67_));
  nand2  g39(.a(x08), .b(x05), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(z6));
  nand3  g41(.a(x16), .b(x15), .c(x14), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n47_), .c(x17), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(x16), .c(x15), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n54_), .c(new_n71_), .O(new_n74_));
  nand4  g46(.a(new_n74_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n75_));
  nand2  g47(.a(x08), .b(x06), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n75_), .O(z7));
  nand3  g49(.a(x17), .b(x16), .c(x15), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n54_), .c(x18), .O(new_n79_));
  nand4  g51(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n54_), .c(new_n79_), .O(new_n81_));
  nand4  g53(.a(new_n81_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x07), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z8));
endmodule


