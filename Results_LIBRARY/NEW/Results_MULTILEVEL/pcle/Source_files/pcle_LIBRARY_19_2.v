// Benchmark "FAU" written by ABC on Mon Jul 27 18:13:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  nand2  g09(.a(x08), .b(x00), .O(new_n38_));
  inv1   g10(.a(x11), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(x09), .c(new_n30_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n38_), .O(z1));
  inv1   g13(.a(x12), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(x11), .c(new_n31_), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(x11), .c(new_n43_), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(x09), .c(new_n30_), .O(new_n45_));
  nand2  g17(.a(x08), .b(x01), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(z2));
  nand2  g19(.a(x13), .b(new_n42_), .O(new_n48_));
  inv1   g20(.a(x13), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(x12), .c(x11), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n48_), .c(x10), .O(new_n51_));
  nor2   g23(.a(new_n49_), .b(x11), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n51_), .c(x09), .O(new_n53_));
  nand2  g25(.a(x08), .b(x02), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(x08), .c(new_n54_), .O(z3));
  nand2  g27(.a(x13), .b(x12), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(x13), .c(x12), .d(x11), .O(new_n59_));
  aoi21  g31(.a(new_n59_), .b(new_n57_), .c(x10), .O(new_n60_));
  nor2   g32(.a(new_n58_), .b(x11), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n60_), .c(x09), .O(new_n62_));
  nand2  g34(.a(x08), .b(x03), .O(new_n63_));
  oai21  g35(.a(new_n62_), .b(x08), .c(new_n63_), .O(z4));
  nand2  g36(.a(x12), .b(x11), .O(new_n65_));
  nand3  g37(.a(x14), .b(x13), .c(x12), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x15), .O(new_n67_));
  inv1   g39(.a(x15), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(x14), .c(x13), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(new_n65_), .c(new_n67_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n31_), .O(new_n71_));
  nand2  g43(.a(x15), .b(new_n39_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(x09), .c(new_n30_), .O(new_n74_));
  nand2  g46(.a(x08), .b(x04), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(z5));
  nand2  g48(.a(x15), .b(x14), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n56_), .c(x16), .O(new_n78_));
  nand3  g50(.a(x13), .b(x12), .c(x11), .O(new_n79_));
  inv1   g51(.a(x16), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(x15), .c(x14), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n79_), .c(new_n78_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n31_), .O(new_n83_));
  nand2  g55(.a(x16), .b(new_n39_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(x09), .c(new_n30_), .O(new_n86_));
  nand2  g58(.a(x08), .b(x05), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n86_), .O(z6));
  nand3  g60(.a(x16), .b(x15), .c(x14), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n56_), .c(x17), .O(new_n90_));
  inv1   g62(.a(x17), .O(new_n91_));
  inv1   g63(.a(new_n77_), .O(new_n92_));
  inv1   g64(.a(new_n79_), .O(new_n93_));
  nand4  g65(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(x16), .O(new_n94_));
  aoi21  g66(.a(new_n94_), .b(new_n90_), .c(x10), .O(new_n95_));
  nor2   g67(.a(new_n91_), .b(x11), .O(new_n96_));
  oai21  g68(.a(new_n96_), .b(new_n95_), .c(x09), .O(new_n97_));
  nand2  g69(.a(x08), .b(x06), .O(new_n98_));
  oai21  g70(.a(new_n97_), .b(x08), .c(new_n98_), .O(z7));
  nand3  g71(.a(x17), .b(x16), .c(x15), .O(new_n100_));
  oai21  g72(.a(new_n100_), .b(new_n66_), .c(x18), .O(new_n101_));
  nor2   g73(.a(new_n80_), .b(new_n68_), .O(new_n102_));
  nand4  g74(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n103_));
  inv1   g75(.a(new_n103_), .O(new_n104_));
  nor2   g76(.a(x18), .b(new_n91_), .O(new_n105_));
  nand3  g77(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  aoi21  g78(.a(new_n106_), .b(new_n101_), .c(x10), .O(new_n107_));
  nor2   g79(.a(new_n29_), .b(x11), .O(new_n108_));
  oai21  g80(.a(new_n108_), .b(new_n107_), .c(x09), .O(new_n109_));
  nand2  g81(.a(x08), .b(x07), .O(new_n110_));
  oai21  g82(.a(new_n109_), .b(x08), .c(new_n110_), .O(z8));
endmodule


