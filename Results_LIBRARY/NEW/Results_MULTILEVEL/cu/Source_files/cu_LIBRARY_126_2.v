// Benchmark "FAU" written by ABC on Mon Jul 27 17:55:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n64_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g04(.a(x05), .b(x02), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  nand2  g06(.a(x04), .b(new_n31_), .O(new_n32_));
  nand4  g07(.a(new_n32_), .b(new_n30_), .c(new_n29_), .d(new_n26_), .O(z00));
  nand3  g08(.a(x05), .b(new_n27_), .c(new_n31_), .O(new_n34_));
  nand3  g09(.a(new_n28_), .b(x04), .c(x02), .O(new_n35_));
  aoi21  g10(.a(new_n35_), .b(new_n34_), .c(x03), .O(z01));
  nor2   g11(.a(x01), .b(x00), .O(new_n37_));
  nand4  g12(.a(new_n37_), .b(x05), .c(new_n26_), .d(new_n31_), .O(new_n38_));
  nor2   g13(.a(new_n38_), .b(x13), .O(z02));
  inv1   g14(.a(x01), .O(new_n40_));
  nand4  g15(.a(new_n26_), .b(new_n31_), .c(new_n40_), .d(x00), .O(new_n41_));
  nor3   g16(.a(new_n41_), .b(x13), .c(new_n28_), .O(z03));
  inv1   g17(.a(x00), .O(new_n43_));
  nand4  g18(.a(new_n26_), .b(new_n31_), .c(x01), .d(new_n43_), .O(new_n44_));
  nor3   g19(.a(new_n44_), .b(x13), .c(new_n28_), .O(z04));
  nand4  g20(.a(new_n26_), .b(new_n31_), .c(x01), .d(x00), .O(new_n46_));
  nor3   g21(.a(new_n46_), .b(x13), .c(new_n28_), .O(z05));
  inv1   g22(.a(x07), .O(new_n48_));
  nor2   g23(.a(x09), .b(x01), .O(new_n49_));
  nor2   g24(.a(x11), .b(new_n40_), .O(new_n50_));
  oai21  g25(.a(new_n50_), .b(new_n49_), .c(x00), .O(new_n51_));
  nor2   g26(.a(x08), .b(x01), .O(new_n52_));
  nor2   g27(.a(x10), .b(new_n40_), .O(new_n53_));
  oai21  g28(.a(new_n53_), .b(new_n52_), .c(new_n43_), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(new_n51_), .c(x12), .O(new_n55_));
  nand4  g30(.a(new_n55_), .b(new_n48_), .c(x05), .d(x04), .O(new_n56_));
  nor3   g31(.a(new_n56_), .b(x03), .c(x02), .O(z06));
  inv1   g32(.a(x13), .O(new_n58_));
  nand4  g33(.a(new_n58_), .b(x05), .c(new_n26_), .d(new_n31_), .O(new_n59_));
  inv1   g34(.a(new_n59_), .O(z07));
  nand4  g35(.a(x05), .b(x04), .c(new_n26_), .d(new_n31_), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(x12), .O(z08));
  and2   g37(.a(x13), .b(x06), .O(z09));
  nand3  g38(.a(new_n30_), .b(x06), .c(new_n26_), .O(new_n64_));
  inv1   g39(.a(new_n64_), .O(z10));
endmodule


