// Benchmark "FAU" written by ABC on Mon Jul 27 17:55:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n72_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g04(.a(x05), .b(x02), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  nand2  g06(.a(x04), .b(new_n31_), .O(new_n32_));
  nand4  g07(.a(new_n32_), .b(new_n30_), .c(new_n29_), .d(new_n26_), .O(z00));
  nand3  g08(.a(x05), .b(new_n27_), .c(new_n31_), .O(new_n34_));
  inv1   g09(.a(x13), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n27_), .O(new_n36_));
  nand3  g11(.a(new_n36_), .b(new_n28_), .c(x02), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n34_), .c(x03), .O(z01));
  nor2   g13(.a(x01), .b(x00), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(new_n27_), .c(new_n26_), .d(new_n31_), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(new_n28_), .O(z02));
  inv1   g16(.a(x01), .O(new_n42_));
  nand4  g17(.a(new_n26_), .b(new_n31_), .c(new_n42_), .d(x00), .O(new_n43_));
  nor3   g18(.a(new_n43_), .b(new_n28_), .c(x04), .O(z03));
  inv1   g19(.a(x00), .O(new_n45_));
  nand4  g20(.a(new_n26_), .b(new_n31_), .c(x01), .d(new_n45_), .O(new_n46_));
  nor3   g21(.a(new_n46_), .b(new_n28_), .c(x04), .O(z04));
  nand4  g22(.a(new_n26_), .b(new_n31_), .c(x01), .d(x00), .O(new_n48_));
  nor3   g23(.a(new_n48_), .b(new_n28_), .c(x04), .O(z05));
  inv1   g24(.a(x07), .O(new_n50_));
  inv1   g25(.a(x09), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(new_n42_), .O(new_n52_));
  inv1   g27(.a(x11), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x01), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(new_n52_), .c(new_n45_), .O(new_n55_));
  inv1   g30(.a(x08), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n42_), .O(new_n57_));
  inv1   g32(.a(x10), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(x01), .O(new_n59_));
  aoi21  g34(.a(new_n59_), .b(new_n57_), .c(x00), .O(new_n60_));
  oai21  g35(.a(new_n60_), .b(new_n55_), .c(x13), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(x12), .O(new_n62_));
  nand4  g37(.a(new_n62_), .b(new_n50_), .c(x05), .d(new_n31_), .O(new_n63_));
  nand4  g38(.a(new_n35_), .b(new_n28_), .c(x04), .d(x02), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n63_), .c(x03), .O(z06));
  nand4  g40(.a(x05), .b(new_n27_), .c(new_n26_), .d(new_n31_), .O(new_n66_));
  inv1   g41(.a(new_n66_), .O(z07));
  inv1   g42(.a(x12), .O(new_n68_));
  nand4  g43(.a(x13), .b(new_n68_), .c(x05), .d(new_n31_), .O(new_n69_));
  aoi21  g44(.a(new_n69_), .b(new_n64_), .c(x03), .O(z08));
  and2   g45(.a(x13), .b(x06), .O(z09));
  nand3  g46(.a(new_n30_), .b(x06), .c(new_n26_), .O(new_n72_));
  inv1   g47(.a(new_n72_), .O(z10));
endmodule


