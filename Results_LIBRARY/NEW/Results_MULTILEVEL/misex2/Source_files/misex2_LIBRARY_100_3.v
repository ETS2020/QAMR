// Benchmark "FAU" written by ABC on Mon Jul 27 17:30:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n123_,
    new_n125_, new_n126_, new_n129_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x19), .O(z00));
  nand2  g08(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z02));
  inv1   g15(.a(new_n47_), .O(new_n59_));
  nor4   g16(.a(new_n59_), .b(x19), .c(new_n45_), .d(x17), .O(z03));
  inv1   g17(.a(x12), .O(new_n61_));
  nand3  g18(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n61_), .c(x10), .d(new_n46_), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(z04));
  nor3   g22(.a(new_n62_), .b(new_n56_), .c(new_n46_), .O(z05));
  inv1   g23(.a(x01), .O(new_n68_));
  nand3  g24(.a(x02), .b(new_n68_), .c(x00), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z07));
  inv1   g26(.a(x19), .O(new_n71_));
  inv1   g27(.a(x08), .O(new_n72_));
  inv1   g28(.a(x05), .O(new_n73_));
  inv1   g29(.a(x06), .O(new_n74_));
  inv1   g30(.a(x00), .O(new_n75_));
  inv1   g31(.a(x03), .O(new_n76_));
  nand4  g32(.a(new_n76_), .b(x02), .c(new_n68_), .d(new_n75_), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand4  g34(.a(new_n78_), .b(new_n74_), .c(new_n73_), .d(x04), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(x07), .O(new_n80_));
  nand4  g36(.a(new_n80_), .b(new_n45_), .c(x17), .d(new_n72_), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n71_), .O(z08));
  inv1   g38(.a(x21), .O(new_n83_));
  inv1   g39(.a(x22), .O(new_n84_));
  inv1   g40(.a(x11), .O(new_n85_));
  inv1   g41(.a(x02), .O(new_n86_));
  nor2   g42(.a(new_n86_), .b(x01), .O(new_n87_));
  nand3  g43(.a(new_n87_), .b(x12), .c(new_n85_), .O(new_n88_));
  inv1   g44(.a(x15), .O(new_n89_));
  inv1   g45(.a(x16), .O(new_n90_));
  nor2   g46(.a(x14), .b(x13), .O(new_n91_));
  nand4  g47(.a(new_n91_), .b(x20), .c(new_n90_), .d(new_n89_), .O(new_n92_));
  inv1   g48(.a(x20), .O(new_n93_));
  nor2   g49(.a(new_n45_), .b(new_n68_), .O(new_n94_));
  nand3  g50(.a(new_n94_), .b(new_n93_), .c(new_n71_), .O(new_n95_));
  oai21  g51(.a(new_n92_), .b(new_n88_), .c(new_n95_), .O(new_n96_));
  nand4  g52(.a(new_n96_), .b(new_n84_), .c(new_n83_), .d(new_n75_), .O(new_n97_));
  inv1   g53(.a(new_n97_), .O(z09));
  nor3   g54(.a(new_n84_), .b(new_n83_), .c(x20), .O(new_n99_));
  nand4  g55(.a(new_n99_), .b(new_n71_), .c(x18), .d(x01), .O(new_n100_));
  nor2   g56(.a(x13), .b(new_n61_), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n87_), .c(new_n85_), .O(new_n102_));
  nor3   g58(.a(new_n90_), .b(new_n89_), .c(x14), .O(new_n103_));
  nand4  g59(.a(new_n103_), .b(new_n84_), .c(new_n83_), .d(x20), .O(new_n104_));
  or2    g60(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n100_), .c(x00), .O(z10));
  nand4  g62(.a(new_n94_), .b(x21), .c(new_n93_), .d(new_n71_), .O(new_n107_));
  nor2   g63(.a(new_n89_), .b(x14), .O(new_n108_));
  nand4  g64(.a(new_n108_), .b(new_n83_), .c(x20), .d(new_n90_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n102_), .c(new_n107_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n84_), .c(new_n75_), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(z11));
  nand2  g68(.a(x10), .b(x02), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(x01), .c(x00), .O(new_n114_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n115_));
  nand3  g71(.a(new_n71_), .b(x17), .c(new_n86_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n68_), .c(new_n75_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n114_), .c(x24), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(x09), .O(new_n120_));
  inv1   g76(.a(new_n120_), .O(z12));
  nor3   g77(.a(new_n59_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g78(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n123_));
  nor2   g79(.a(new_n123_), .b(x19), .O(z14));
  aoi21  g80(.a(new_n56_), .b(x01), .c(new_n86_), .O(new_n125_));
  nand3  g81(.a(x19), .b(new_n86_), .c(new_n68_), .O(new_n126_));
  oai21  g82(.a(new_n125_), .b(new_n75_), .c(new_n126_), .O(z15));
  nor2   g83(.a(new_n68_), .b(x00), .O(z16));
  nand3  g84(.a(x02), .b(new_n68_), .c(new_n75_), .O(new_n129_));
  inv1   g85(.a(new_n129_), .O(z17));
  zero   g86(.O(z06));
endmodule


