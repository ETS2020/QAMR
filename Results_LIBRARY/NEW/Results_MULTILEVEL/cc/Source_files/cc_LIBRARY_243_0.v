// Benchmark "FAU" written by ABC on Mon Jul 27 17:23:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x14), .O(new_n45_));
  inv1   g04(.a(x10), .O(new_n46_));
  nand2  g05(.a(x12), .b(new_n46_), .O(new_n47_));
  nand4  g06(.a(new_n47_), .b(x15), .c(new_n45_), .d(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n45_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z02));
  nand3  g13(.a(x12), .b(x10), .c(x08), .O(new_n55_));
  nor3   g14(.a(new_n55_), .b(new_n50_), .c(x14), .O(z03));
  inv1   g15(.a(x18), .O(z04));
  nand2  g16(.a(x09), .b(x08), .O(z10));
  inv1   g17(.a(z10), .O(z09));
  nand2  g18(.a(new_n50_), .b(x08), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(x13), .O(new_n61_));
  nand4  g20(.a(new_n50_), .b(x10), .c(x08), .d(x00), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(x12), .O(new_n64_));
  nand2  g23(.a(x13), .b(new_n46_), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n64_), .O(z12));
  aoi21  g25(.a(x10), .b(x08), .c(x14), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n50_), .O(new_n68_));
  aoi21  g27(.a(x08), .b(x01), .c(new_n45_), .O(new_n69_));
  oai21  g28(.a(new_n69_), .b(new_n68_), .c(x12), .O(new_n70_));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n70_), .O(z13));
  nand4  g31(.a(new_n50_), .b(x10), .c(x08), .d(x02), .O(new_n73_));
  oai21  g32(.a(new_n50_), .b(x08), .c(new_n73_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x12), .O(new_n75_));
  nand2  g34(.a(x15), .b(new_n46_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(z14));
  nand3  g36(.a(new_n60_), .b(x16), .c(new_n45_), .O(new_n78_));
  nand4  g37(.a(new_n50_), .b(x10), .c(x08), .d(x03), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x12), .O(new_n81_));
  nand3  g40(.a(x16), .b(new_n45_), .c(new_n46_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n81_), .O(z15));
  nand2  g42(.a(new_n60_), .b(x17), .O(new_n84_));
  nand4  g43(.a(new_n50_), .b(x10), .c(x08), .d(x04), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x12), .O(new_n87_));
  nand2  g46(.a(x17), .b(new_n46_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(z16));
  inv1   g48(.a(x08), .O(new_n90_));
  nor2   g49(.a(x15), .b(new_n90_), .O(new_n91_));
  nand4  g50(.a(new_n50_), .b(x10), .c(x08), .d(x05), .O(new_n92_));
  oai21  g51(.a(new_n91_), .b(z04), .c(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x12), .O(new_n94_));
  nand2  g53(.a(x18), .b(new_n46_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(z17));
  nand2  g55(.a(new_n60_), .b(x19), .O(new_n97_));
  nand4  g56(.a(new_n50_), .b(x10), .c(x08), .d(x06), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x12), .O(new_n100_));
  nand2  g59(.a(x19), .b(new_n46_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(z18));
  nand4  g61(.a(new_n50_), .b(x10), .c(x08), .d(x07), .O(new_n103_));
  oai21  g62(.a(new_n91_), .b(new_n43_), .c(new_n103_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(x12), .O(new_n105_));
  nand2  g64(.a(x20), .b(new_n46_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(z19));
  buf    g66(.a(x19), .O(z05));
  buf    g67(.a(x15), .O(z06));
  buf    g68(.a(x17), .O(z07));
  buf    g69(.a(x16), .O(z08));
  buf    g70(.a(x14), .O(z11));
endmodule


