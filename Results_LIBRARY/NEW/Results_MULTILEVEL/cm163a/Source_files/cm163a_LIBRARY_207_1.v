// Benchmark "FAU" written by ABC on Mon Jul 27 17:20:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_;
  inv1   g00(.a(x04), .O(new_n22_));
  inv1   g01(.a(x03), .O(new_n23_));
  inv1   g02(.a(x11), .O(new_n24_));
  aoi21  g03(.a(new_n24_), .b(new_n23_), .c(x09), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  aoi22  g05(.a(new_n26_), .b(x09), .c(new_n25_), .d(x02), .O(new_n27_));
  inv1   g06(.a(x00), .O(new_n28_));
  inv1   g07(.a(x05), .O(new_n29_));
  aoi21  g08(.a(new_n22_), .b(new_n28_), .c(new_n29_), .O(new_n30_));
  oai21  g09(.a(new_n27_), .b(new_n22_), .c(new_n30_), .O(z0));
  inv1   g10(.a(x02), .O(new_n32_));
  oai21  g11(.a(x09), .b(new_n32_), .c(x11), .O(new_n33_));
  inv1   g12(.a(x09), .O(new_n34_));
  nand4  g13(.a(new_n24_), .b(new_n34_), .c(x03), .d(x02), .O(new_n35_));
  aoi21  g14(.a(new_n35_), .b(new_n33_), .c(new_n22_), .O(new_n36_));
  oai21  g15(.a(x04), .b(x01), .c(x05), .O(new_n37_));
  or2    g16(.a(new_n37_), .b(new_n36_), .O(z1));
  inv1   g17(.a(new_n26_), .O(new_n39_));
  nor3   g18(.a(x12), .b(x11), .c(x09), .O(new_n40_));
  aoi22  g19(.a(new_n40_), .b(new_n39_), .c(new_n35_), .d(x12), .O(new_n41_));
  inv1   g20(.a(x06), .O(new_n42_));
  aoi21  g21(.a(new_n42_), .b(new_n22_), .c(new_n29_), .O(new_n43_));
  oai21  g22(.a(new_n41_), .b(new_n22_), .c(new_n43_), .O(z2));
  nor2   g23(.a(x12), .b(x11), .O(new_n45_));
  nand4  g24(.a(new_n45_), .b(new_n34_), .c(x03), .d(x02), .O(new_n46_));
  nand3  g25(.a(new_n34_), .b(x03), .c(x02), .O(new_n47_));
  inv1   g26(.a(new_n47_), .O(new_n48_));
  nor3   g27(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  aoi22  g28(.a(new_n49_), .b(new_n48_), .c(new_n46_), .d(x13), .O(new_n50_));
  inv1   g29(.a(x07), .O(new_n51_));
  aoi21  g30(.a(new_n51_), .b(new_n22_), .c(new_n29_), .O(new_n52_));
  oai21  g31(.a(new_n50_), .b(new_n22_), .c(new_n52_), .O(z3));
  inv1   g32(.a(x15), .O(new_n54_));
  nand4  g33(.a(x14), .b(x10), .c(x08), .d(x03), .O(new_n55_));
  nor2   g34(.a(new_n55_), .b(new_n54_), .O(z4));
endmodule


