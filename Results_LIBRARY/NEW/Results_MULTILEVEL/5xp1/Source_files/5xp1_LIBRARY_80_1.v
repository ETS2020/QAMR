// Benchmark "FAU" written by ABC on Mon Jul 27 17:30:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_;
  nand2  g00(.a(x1), .b(x0), .O(new_n18_));
  nor2   g01(.a(new_n18_), .b(x6), .O(new_n19_));
  aoi21  g02(.a(new_n19_), .b(new_n18_), .c(x5), .O(new_n20_));
  inv1   g03(.a(x0), .O(new_n21_));
  inv1   g04(.a(x6), .O(new_n22_));
  nand2  g05(.a(x3), .b(x1), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(new_n24_));
  inv1   g07(.a(new_n24_), .O(new_n25_));
  oai21  g08(.a(new_n25_), .b(new_n20_), .c(x4), .O(new_n26_));
  inv1   g09(.a(x4), .O(new_n27_));
  inv1   g10(.a(x1), .O(new_n28_));
  inv1   g11(.a(x3), .O(z8));
  nand3  g12(.a(z8), .b(new_n28_), .c(new_n21_), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(x6), .c(x5), .d(new_n27_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n26_), .O(z0));
  oai21  g15(.a(x4), .b(x3), .c(x6), .O(new_n33_));
  aoi22  g16(.a(new_n33_), .b(new_n28_), .c(new_n22_), .d(z8), .O(new_n34_));
  nand2  g17(.a(new_n23_), .b(new_n21_), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(new_n22_), .c(new_n27_), .O(new_n36_));
  oai21  g19(.a(new_n34_), .b(x0), .c(new_n36_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x5), .O(new_n38_));
  inv1   g21(.a(x5), .O(new_n39_));
  aoi21  g22(.a(x4), .b(x2), .c(x6), .O(new_n40_));
  nand2  g23(.a(x6), .b(x2), .O(new_n41_));
  oai21  g24(.a(new_n40_), .b(new_n28_), .c(new_n41_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x0), .O(new_n43_));
  nand2  g26(.a(x6), .b(x4), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n39_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n38_), .O(z1));
  nor2   g30(.a(x6), .b(x4), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x1), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  and2   g33(.a(x2), .b(x0), .O(new_n51_));
  nor2   g34(.a(new_n39_), .b(z8), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nor2   g36(.a(x5), .b(x2), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x1), .O(new_n55_));
  nand2  g38(.a(x6), .b(x5), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n55_), .c(new_n21_), .O(new_n57_));
  aoi21  g40(.a(x5), .b(x0), .c(x1), .O(new_n58_));
  aoi21  g41(.a(x5), .b(x3), .c(x0), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n58_), .c(new_n22_), .O(new_n60_));
  nand3  g43(.a(x6), .b(x5), .c(x1), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n57_), .c(x4), .O(new_n63_));
  oai21  g46(.a(x3), .b(x1), .c(x5), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n21_), .O(new_n65_));
  nand2  g48(.a(new_n54_), .b(new_n28_), .O(new_n66_));
  aoi21  g49(.a(new_n66_), .b(new_n65_), .c(new_n22_), .O(new_n67_));
  nand3  g50(.a(new_n22_), .b(x5), .c(x0), .O(new_n68_));
  inv1   g51(.a(new_n68_), .O(new_n69_));
  oai21  g52(.a(new_n69_), .b(new_n67_), .c(new_n27_), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n63_), .c(new_n53_), .O(z2));
  nand3  g54(.a(x5), .b(x2), .c(x0), .O(new_n72_));
  nand3  g55(.a(new_n39_), .b(x3), .c(new_n21_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai21  g57(.a(x6), .b(x1), .c(new_n74_), .O(new_n75_));
  nand3  g58(.a(x5), .b(z8), .c(new_n28_), .O(new_n76_));
  xor2a  g59(.a(x5), .b(x0), .O(new_n77_));
  oai22  g60(.a(new_n77_), .b(new_n28_), .c(new_n76_), .d(x0), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(x6), .O(new_n79_));
  nand2  g62(.a(x2), .b(x1), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n39_), .c(x0), .O(new_n81_));
  nand3  g64(.a(new_n23_), .b(x5), .c(new_n21_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g66(.a(x1), .b(new_n21_), .O(new_n84_));
  aoi22  g67(.a(new_n84_), .b(new_n54_), .c(new_n83_), .d(new_n22_), .O(new_n85_));
  nand3  g68(.a(new_n85_), .b(new_n79_), .c(new_n75_), .O(z3));
  xnor2a g69(.a(x6), .b(x1), .O(new_n87_));
  oai21  g70(.a(new_n51_), .b(x3), .c(new_n87_), .O(new_n88_));
  xor2a  g71(.a(x6), .b(x1), .O(new_n89_));
  oai21  g72(.a(x3), .b(x0), .c(x2), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(new_n88_), .O(z4));
  nand2  g75(.a(z8), .b(x2), .O(new_n93_));
  xor2a  g76(.a(new_n93_), .b(new_n21_), .O(z5));
  nand2  g77(.a(new_n22_), .b(z8), .O(new_n95_));
  nor2   g78(.a(x5), .b(x0), .O(new_n96_));
  nor2   g79(.a(new_n39_), .b(new_n27_), .O(new_n97_));
  oai21  g80(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(new_n98_));
  nand2  g81(.a(x4), .b(x2), .O(new_n99_));
  nand2  g82(.a(new_n99_), .b(new_n22_), .O(new_n100_));
  nand2  g83(.a(new_n100_), .b(new_n39_), .O(new_n101_));
  oai21  g84(.a(new_n48_), .b(x5), .c(x2), .O(new_n102_));
  nand2  g85(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g86(.a(new_n22_), .b(z8), .O(new_n104_));
  nand3  g87(.a(new_n104_), .b(x5), .c(new_n27_), .O(new_n105_));
  nand2  g88(.a(new_n90_), .b(new_n22_), .O(new_n106_));
  nand2  g89(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g90(.a(new_n103_), .b(x0), .c(new_n107_), .O(new_n108_));
  aoi21  g91(.a(new_n108_), .b(new_n98_), .c(new_n28_), .O(z6));
  inv1   g92(.a(new_n93_), .O(z7));
  nand2  g93(.a(new_n95_), .b(x1), .O(new_n111_));
  nand2  g94(.a(new_n30_), .b(z8), .O(new_n112_));
  nand2  g95(.a(new_n112_), .b(x6), .O(new_n113_));
  nand3  g96(.a(new_n113_), .b(new_n111_), .c(new_n21_), .O(new_n114_));
  nand3  g97(.a(new_n114_), .b(x5), .c(x4), .O(new_n115_));
  inv1   g98(.a(new_n115_), .O(z9));
endmodule


