// Benchmark "FAU" written by ABC on Mon Jul 27 17:25:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  aoi21  g03(.a(new_n22_), .b(x08), .c(new_n21_), .O(new_n23_));
  inv1   g04(.a(x08), .O(new_n24_));
  inv1   g05(.a(x02), .O(new_n25_));
  nor2   g06(.a(new_n21_), .b(new_n25_), .O(new_n26_));
  nor2   g07(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  aoi21  g08(.a(new_n23_), .b(x02), .c(new_n27_), .O(new_n28_));
  inv1   g09(.a(x00), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n20_), .b(new_n29_), .c(new_n30_), .O(new_n31_));
  oai21  g12(.a(new_n28_), .b(new_n20_), .c(new_n31_), .O(z0));
  inv1   g13(.a(x10), .O(new_n33_));
  inv1   g14(.a(x11), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(new_n33_), .c(new_n24_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n22_), .c(new_n20_), .O(new_n36_));
  nand3  g17(.a(x11), .b(new_n33_), .c(new_n24_), .O(new_n37_));
  inv1   g18(.a(new_n37_), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n36_), .c(x04), .O(new_n39_));
  nand3  g20(.a(new_n24_), .b(x04), .c(x02), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(x10), .c(x03), .O(new_n41_));
  inv1   g22(.a(new_n41_), .O(new_n42_));
  oai21  g23(.a(x03), .b(x01), .c(x05), .O(new_n43_));
  nor2   g24(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g25(.a(new_n39_), .b(new_n25_), .c(new_n44_), .O(z1));
  nand2  g26(.a(new_n35_), .b(new_n22_), .O(new_n46_));
  nand4  g27(.a(new_n46_), .b(x04), .c(x03), .d(x02), .O(new_n47_));
  nor2   g28(.a(x10), .b(x08), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n26_), .O(new_n49_));
  oai21  g30(.a(x06), .b(x03), .c(x05), .O(new_n50_));
  aoi21  g31(.a(new_n49_), .b(x11), .c(new_n50_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n47_), .O(z2));
  inv1   g33(.a(x12), .O(new_n53_));
  nand4  g34(.a(new_n53_), .b(new_n34_), .c(new_n33_), .d(new_n24_), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n22_), .c(new_n21_), .O(new_n55_));
  aoi21  g36(.a(new_n48_), .b(new_n26_), .c(new_n53_), .O(new_n56_));
  aoi21  g37(.a(new_n55_), .b(x02), .c(new_n56_), .O(new_n57_));
  inv1   g38(.a(x07), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n20_), .O(new_n59_));
  nand2  g40(.a(x12), .b(x11), .O(new_n60_));
  nand3  g41(.a(new_n60_), .b(new_n59_), .c(x05), .O(new_n61_));
  inv1   g42(.a(new_n61_), .O(new_n62_));
  oai21  g43(.a(new_n57_), .b(new_n20_), .c(new_n62_), .O(z3));
  nand3  g44(.a(x13), .b(x09), .c(x04), .O(new_n64_));
  inv1   g45(.a(new_n64_), .O(z4));
endmodule


