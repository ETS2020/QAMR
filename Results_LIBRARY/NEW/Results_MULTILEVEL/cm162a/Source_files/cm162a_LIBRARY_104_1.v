// Benchmark "FAU" written by ABC on Mon Jul 27 17:25:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x03), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x04), .c(x02), .O(new_n24_));
  and2   g05(.a(x04), .b(x02), .O(new_n25_));
  nor2   g06(.a(new_n25_), .b(new_n20_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand3  g12(.a(new_n31_), .b(new_n27_), .c(new_n24_), .O(z0));
  inv1   g13(.a(x10), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(new_n20_), .c(x03), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n22_), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  nand3  g17(.a(new_n20_), .b(x04), .c(x02), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(x10), .c(x03), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  aoi21  g20(.a(new_n29_), .b(new_n39_), .c(new_n30_), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(new_n38_), .c(new_n36_), .O(z1));
  nor2   g22(.a(x11), .b(x10), .O(new_n42_));
  nand3  g23(.a(new_n42_), .b(new_n20_), .c(x03), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n22_), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(x04), .c(x02), .O(new_n45_));
  nand3  g26(.a(new_n25_), .b(new_n33_), .c(new_n20_), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(x11), .c(x03), .O(new_n47_));
  inv1   g28(.a(x06), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n29_), .c(new_n30_), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n47_), .c(new_n45_), .O(z2));
  inv1   g31(.a(x11), .O(new_n51_));
  inv1   g32(.a(x12), .O(new_n52_));
  nand3  g33(.a(new_n52_), .b(new_n51_), .c(new_n33_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n21_), .c(new_n22_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(x04), .c(x02), .O(new_n55_));
  nand3  g36(.a(new_n42_), .b(new_n25_), .c(new_n20_), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(x12), .c(x03), .O(new_n57_));
  inv1   g38(.a(x07), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(new_n29_), .c(new_n30_), .O(new_n59_));
  nand3  g40(.a(new_n59_), .b(new_n57_), .c(new_n55_), .O(z3));
  nand3  g41(.a(x13), .b(x09), .c(x04), .O(new_n61_));
  inv1   g42(.a(new_n61_), .O(z4));
endmodule


