// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n170_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_;
  oai21  g000(.a(x36), .b(x35), .c(x39), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  oai21  g002(.a(new_n63_), .b(x04), .c(new_n64_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x39), .O(new_n67_));
  nor3   g005(.a(new_n67_), .b(new_n66_), .c(x04), .O(new_n68_));
  aoi21  g006(.a(new_n65_), .b(x27), .c(new_n68_), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x15), .c(x16), .O(z00));
  inv1   g008(.a(x30), .O(new_n71_));
  inv1   g009(.a(x32), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  inv1   g011(.a(x28), .O(new_n74_));
  nand2  g012(.a(x35), .b(new_n74_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x27), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(new_n72_), .c(new_n71_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x04), .O(z01));
  aoi21  g017(.a(x29), .b(x08), .c(x02), .O(new_n80_));
  aoi21  g018(.a(new_n75_), .b(x04), .c(new_n80_), .O(new_n81_));
  inv1   g019(.a(x08), .O(new_n82_));
  inv1   g020(.a(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  nand4  g022(.a(new_n84_), .b(new_n81_), .c(x40), .d(x39), .O(z02));
  nand2  g023(.a(x35), .b(x28), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  inv1   g025(.a(x37), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n83_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n87_), .c(x21), .O(z03));
  inv1   g028(.a(x21), .O(new_n91_));
  nand3  g029(.a(new_n89_), .b(new_n87_), .c(new_n91_), .O(z04));
  aoi21  g030(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g031(.a(z06), .O(z05));
  inv1   g032(.a(x03), .O(new_n95_));
  inv1   g033(.a(x31), .O(new_n96_));
  inv1   g034(.a(x33), .O(new_n97_));
  inv1   g035(.a(x15), .O(new_n98_));
  nand2  g036(.a(x17), .b(new_n98_), .O(new_n99_));
  inv1   g037(.a(x00), .O(new_n100_));
  oai21  g038(.a(x25), .b(new_n100_), .c(x38), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n99_), .c(new_n97_), .d(new_n96_), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand3  g042(.a(x38), .b(new_n104_), .c(new_n100_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n102_), .c(new_n95_), .O(z07));
  nand2  g045(.a(x40), .b(x39), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(z08));
  nand4  g047(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(z09));
  inv1   g049(.a(x04), .O(new_n112_));
  nand2  g050(.a(new_n108_), .b(x07), .O(new_n113_));
  nand3  g051(.a(x40), .b(x39), .c(x05), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n76_), .c(x27), .O(new_n116_));
  oai21  g054(.a(x32), .b(x30), .c(x40), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n67_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x05), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n112_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(z10));
  inv1   g061(.a(x09), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(x29), .O(new_n125_));
  nor2   g063(.a(new_n83_), .b(new_n112_), .O(new_n126_));
  oai21  g064(.a(new_n126_), .b(new_n75_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(x40), .b(x04), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(x35), .c(new_n74_), .d(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n82_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n71_), .c(new_n124_), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(z11));
  nor3   g071(.a(x37), .b(x36), .c(x35), .O(new_n134_));
  inv1   g072(.a(x40), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(x27), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n134_), .c(new_n66_), .O(new_n137_));
  nand2  g075(.a(z08), .b(new_n112_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n86_), .c(new_n88_), .O(new_n139_));
  nand3  g077(.a(x40), .b(x39), .c(new_n112_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n83_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n139_), .c(new_n137_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  nand2  g081(.a(x36), .b(x35), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x28), .c(new_n135_), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(x39), .c(new_n143_), .d(new_n112_), .O(new_n146_));
  inv1   g084(.a(x18), .O(new_n147_));
  inv1   g085(.a(x19), .O(new_n148_));
  inv1   g086(.a(new_n64_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(x20), .c(new_n148_), .d(new_n147_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n146_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x27), .O(new_n152_));
  nand3  g090(.a(new_n118_), .b(new_n143_), .c(new_n112_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n152_), .O(z13));
  nand3  g092(.a(x20), .b(new_n148_), .c(new_n147_), .O(new_n155_));
  nand4  g093(.a(new_n144_), .b(new_n72_), .c(new_n71_), .d(x28), .O(new_n156_));
  nand2  g094(.a(new_n143_), .b(new_n112_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x40), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n156_), .c(z08), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  inv1   g098(.a(x35), .O(new_n161_));
  nand3  g099(.a(new_n88_), .b(new_n161_), .c(x28), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x27), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n72_), .c(new_n71_), .O(new_n164_));
  oai21  g102(.a(new_n136_), .b(new_n64_), .c(new_n157_), .O(new_n165_));
  oai21  g103(.a(new_n64_), .b(new_n83_), .c(new_n108_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n165_), .c(new_n164_), .d(new_n160_), .O(z14));
  nand4  g105(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z15));
  nand2  g107(.a(x22), .b(x01), .O(new_n170_));
  nor2   g108(.a(new_n170_), .b(x23), .O(z16));
  inv1   g109(.a(x23), .O(new_n172_));
  nor3   g110(.a(new_n170_), .b(x24), .c(new_n172_), .O(z17));
  oai21  g111(.a(new_n75_), .b(x27), .c(new_n125_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x08), .O(new_n175_));
  inv1   g113(.a(new_n75_), .O(new_n176_));
  nor2   g114(.a(new_n83_), .b(x04), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n176_), .c(x30), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n175_), .c(new_n124_), .O(z18));
  inv1   g117(.a(new_n125_), .O(new_n180_));
  oai21  g118(.a(new_n161_), .b(new_n83_), .c(new_n82_), .O(new_n181_));
  aoi21  g119(.a(x28), .b(new_n83_), .c(new_n161_), .O(new_n182_));
  oai21  g120(.a(new_n182_), .b(new_n180_), .c(new_n181_), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n71_), .c(new_n124_), .O(new_n184_));
  inv1   g122(.a(new_n184_), .O(z20));
  inv1   g123(.a(new_n132_), .O(z19));
endmodule


