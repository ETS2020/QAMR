// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n90_, new_n91_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n157_, new_n159_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  inv1   g002(.a(x10), .O(new_n65_));
  oai21  g003(.a(x36), .b(x35), .c(x27), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g005(.a(new_n67_), .b(x40), .c(x39), .d(new_n64_), .O(new_n68_));
  nand2  g006(.a(x37), .b(x27), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n63_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(x16), .O(z00));
  inv1   g010(.a(x30), .O(new_n73_));
  inv1   g011(.a(x32), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x27), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n74_), .c(new_n73_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x04), .O(z01));
  inv1   g019(.a(x02), .O(new_n82_));
  nand2  g020(.a(x29), .b(x08), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n77_), .c(x04), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  inv1   g025(.a(new_n87_), .O(z08));
  nand3  g026(.a(z08), .b(new_n86_), .c(new_n84_), .O(z02));
  inv1   g027(.a(x35), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n76_), .c(x27), .O(new_n91_));
  nand3  g029(.a(new_n91_), .b(x37), .c(x21), .O(z03));
  inv1   g030(.a(x37), .O(z06));
  nor2   g031(.a(z06), .b(x21), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n91_), .O(z04));
  inv1   g033(.a(x03), .O(new_n96_));
  inv1   g034(.a(x31), .O(new_n97_));
  inv1   g035(.a(x33), .O(new_n98_));
  nand2  g036(.a(x17), .b(new_n63_), .O(new_n99_));
  inv1   g037(.a(x00), .O(new_n100_));
  oai21  g038(.a(x25), .b(new_n100_), .c(x38), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand3  g042(.a(x38), .b(new_n104_), .c(new_n100_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n102_), .c(new_n96_), .O(z07));
  nand4  g045(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(z09));
  nand2  g047(.a(new_n87_), .b(x07), .O(new_n110_));
  nand3  g048(.a(x40), .b(x39), .c(x05), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n78_), .c(x27), .O(new_n113_));
  inv1   g051(.a(x40), .O(new_n114_));
  nor2   g052(.a(x32), .b(x30), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(x39), .c(x05), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n64_), .O(new_n119_));
  inv1   g057(.a(new_n69_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(x06), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n119_), .O(z10));
  inv1   g060(.a(x09), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x29), .O(new_n124_));
  inv1   g062(.a(x27), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n64_), .O(new_n126_));
  oai21  g064(.a(new_n126_), .b(new_n77_), .c(new_n124_), .O(new_n127_));
  inv1   g065(.a(x08), .O(new_n128_));
  nand4  g066(.a(x35), .b(new_n76_), .c(x27), .d(new_n64_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n73_), .c(new_n123_), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(z11));
  oai21  g071(.a(new_n87_), .b(x04), .c(new_n69_), .O(new_n134_));
  nor2   g072(.a(x37), .b(x36), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n90_), .c(new_n125_), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(x10), .c(new_n134_), .O(z12));
  inv1   g075(.a(x13), .O(new_n138_));
  nand2  g076(.a(x36), .b(x35), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x28), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x27), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(new_n115_), .c(new_n114_), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(x39), .c(new_n138_), .d(new_n64_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand4  g083(.a(new_n120_), .b(x20), .c(new_n145_), .d(new_n144_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n143_), .O(z13));
  nand3  g085(.a(x20), .b(new_n145_), .c(new_n144_), .O(new_n148_));
  nand4  g086(.a(new_n139_), .b(new_n74_), .c(new_n73_), .d(x28), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(new_n150_));
  nand3  g088(.a(z08), .b(new_n138_), .c(new_n64_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n148_), .O(new_n152_));
  aoi22  g090(.a(new_n151_), .b(new_n69_), .c(new_n115_), .d(new_n125_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n152_), .O(z14));
  nand4  g092(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(z15));
  nand2  g094(.a(x22), .b(x01), .O(new_n157_));
  nor2   g095(.a(new_n157_), .b(x23), .O(z16));
  inv1   g096(.a(x23), .O(new_n159_));
  nor3   g097(.a(new_n157_), .b(x24), .c(new_n159_), .O(z17));
  oai21  g098(.a(new_n77_), .b(x27), .c(new_n124_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x08), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(new_n129_), .c(new_n73_), .d(new_n123_), .O(z18));
  inv1   g101(.a(new_n124_), .O(new_n164_));
  oai21  g102(.a(new_n90_), .b(new_n125_), .c(new_n128_), .O(new_n165_));
  aoi21  g103(.a(x28), .b(new_n125_), .c(new_n90_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n164_), .c(new_n165_), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n73_), .c(new_n123_), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z20));
  buf    g107(.a(x37), .O(z05));
  inv1   g108(.a(new_n132_), .O(z19));
endmodule


