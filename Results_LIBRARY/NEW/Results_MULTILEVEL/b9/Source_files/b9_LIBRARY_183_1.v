// Benchmark "FAU" written by ABC on Mon Jul 27 18:20:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n170_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n183_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  inv1   g002(.a(x10), .O(new_n65_));
  nand2  g003(.a(x36), .b(x27), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g005(.a(new_n67_), .b(x40), .c(x39), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(x27), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n63_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(x16), .O(z00));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(x36), .c(x27), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n77_), .c(new_n76_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x04), .O(z01));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(x40), .c(new_n84_), .O(new_n86_));
  nand2  g024(.a(new_n78_), .b(x27), .O(new_n87_));
  inv1   g025(.a(x39), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(x04), .c(x28), .O(new_n89_));
  nand4  g027(.a(new_n89_), .b(new_n87_), .c(new_n86_), .d(x40), .O(z02));
  nand2  g028(.a(new_n70_), .b(x27), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  nand2  g030(.a(new_n69_), .b(new_n92_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n91_), .c(new_n95_), .O(z04));
  aoi21  g034(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g035(.a(z06), .O(z05));
  inv1   g036(.a(x03), .O(new_n99_));
  inv1   g037(.a(x31), .O(new_n100_));
  inv1   g038(.a(x33), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n63_), .O(new_n102_));
  inv1   g040(.a(x00), .O(new_n103_));
  oai21  g041(.a(x25), .b(new_n103_), .c(x38), .O(new_n104_));
  nand4  g042(.a(new_n104_), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n103_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n105_), .c(new_n99_), .O(z07));
  nand2  g048(.a(x40), .b(x39), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z08));
  nand4  g050(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z09));
  nand3  g052(.a(new_n66_), .b(new_n77_), .c(new_n76_), .O(new_n115_));
  nand4  g053(.a(new_n115_), .b(x40), .c(x39), .d(x05), .O(new_n116_));
  nand2  g054(.a(new_n111_), .b(x36), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n79_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(x27), .c(x07), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n64_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(z10));
  inv1   g061(.a(x09), .O(new_n124_));
  inv1   g062(.a(x08), .O(new_n125_));
  nand2  g063(.a(new_n87_), .b(new_n125_), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x28), .O(new_n128_));
  inv1   g066(.a(x40), .O(new_n129_));
  oai21  g067(.a(new_n92_), .b(new_n64_), .c(x35), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n128_), .c(new_n126_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n76_), .c(new_n124_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z11));
  inv1   g072(.a(x35), .O(new_n135_));
  nand2  g073(.a(new_n69_), .b(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x27), .O(new_n137_));
  oai21  g075(.a(new_n111_), .b(x04), .c(new_n137_), .O(new_n138_));
  nor3   g076(.a(x37), .b(x36), .c(x35), .O(new_n139_));
  nor2   g077(.a(new_n129_), .b(x27), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n139_), .c(new_n65_), .O(new_n141_));
  nor2   g079(.a(x37), .b(x28), .O(new_n142_));
  inv1   g080(.a(new_n142_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n141_), .c(new_n138_), .O(z12));
  inv1   g082(.a(x18), .O(new_n145_));
  inv1   g083(.a(x19), .O(new_n146_));
  nand4  g084(.a(new_n71_), .b(x20), .c(new_n146_), .d(new_n145_), .O(new_n147_));
  nor2   g085(.a(new_n135_), .b(x13), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(z08), .c(x36), .d(new_n64_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  inv1   g089(.a(x13), .O(new_n152_));
  aoi21  g090(.a(new_n77_), .b(new_n76_), .c(new_n129_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x39), .c(new_n152_), .d(new_n64_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n151_), .O(z13));
  nand3  g093(.a(x20), .b(new_n146_), .c(new_n145_), .O(new_n156_));
  nor2   g094(.a(new_n88_), .b(x04), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n77_), .c(new_n76_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n157_), .c(x40), .d(new_n152_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n156_), .O(new_n161_));
  nand2  g099(.a(new_n77_), .b(new_n76_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(z08), .c(new_n64_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n137_), .O(new_n164_));
  nor2   g102(.a(x37), .b(x35), .O(new_n165_));
  oai21  g103(.a(new_n140_), .b(new_n165_), .c(x13), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n164_), .c(new_n161_), .d(new_n143_), .O(z14));
  nand4  g105(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z15));
  nand2  g107(.a(x22), .b(x01), .O(new_n170_));
  nor2   g108(.a(new_n170_), .b(x23), .O(z16));
  inv1   g109(.a(x23), .O(new_n172_));
  nor3   g110(.a(new_n170_), .b(x24), .c(new_n172_), .O(z17));
  oai21  g111(.a(new_n79_), .b(x27), .c(new_n127_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x08), .O(new_n175_));
  nor2   g113(.a(new_n92_), .b(x04), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n80_), .c(x30), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n175_), .c(new_n124_), .O(z18));
  inv1   g116(.a(new_n127_), .O(new_n179_));
  oai21  g117(.a(new_n135_), .b(new_n92_), .c(new_n125_), .O(new_n180_));
  aoi21  g118(.a(x28), .b(new_n92_), .c(new_n135_), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n179_), .c(new_n180_), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n76_), .c(new_n124_), .O(new_n183_));
  inv1   g121(.a(new_n183_), .O(z20));
  inv1   g122(.a(new_n133_), .O(z19));
endmodule


