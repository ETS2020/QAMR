// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n166_,
    new_n168_, new_n170_, new_n171_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(z08));
  nand3  g011(.a(z08), .b(x10), .c(new_n65_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n71_), .c(new_n63_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x16), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n67_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n79_), .c(new_n78_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x04), .O(z01));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  aoi21  g024(.a(new_n81_), .b(x04), .c(new_n86_), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  nand3  g026(.a(new_n64_), .b(new_n88_), .c(x04), .O(new_n89_));
  nand4  g027(.a(new_n89_), .b(new_n87_), .c(x40), .d(x39), .O(z02));
  nand2  g028(.a(x35), .b(x28), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g036(.a(z06), .O(z05));
  inv1   g037(.a(x03), .O(new_n100_));
  inv1   g038(.a(x31), .O(new_n101_));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n63_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand4  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n104_), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  aoi21  g048(.a(new_n110_), .b(new_n106_), .c(new_n100_), .O(z07));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z09));
  nand2  g051(.a(new_n72_), .b(x07), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n82_), .c(x27), .O(new_n117_));
  inv1   g055(.a(x39), .O(new_n118_));
  oai21  g056(.a(x32), .b(x30), .c(x40), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(x05), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n65_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(z10));
  inv1   g063(.a(x09), .O(new_n126_));
  nand2  g064(.a(z08), .b(x29), .O(new_n127_));
  nor2   g065(.a(new_n64_), .b(new_n65_), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n81_), .c(new_n127_), .O(new_n129_));
  nand4  g067(.a(x35), .b(new_n80_), .c(x27), .d(new_n65_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n88_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n78_), .c(new_n126_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z11));
  nor2   g072(.a(x37), .b(x36), .O(new_n135_));
  aoi22  g073(.a(new_n135_), .b(new_n66_), .c(x40), .d(new_n64_), .O(new_n136_));
  oai22  g074(.a(new_n72_), .b(x04), .c(new_n70_), .d(new_n64_), .O(new_n137_));
  oai21  g075(.a(new_n136_), .b(x10), .c(new_n137_), .O(z12));
  inv1   g076(.a(x13), .O(new_n139_));
  inv1   g077(.a(x40), .O(new_n140_));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(x28), .c(new_n140_), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(x39), .c(new_n139_), .d(new_n65_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  inv1   g083(.a(x20), .O(new_n146_));
  nor2   g084(.a(new_n70_), .b(new_n146_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n145_), .c(new_n144_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n143_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  nand3  g088(.a(new_n120_), .b(new_n139_), .c(new_n65_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(z13));
  nand4  g090(.a(new_n141_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  nand3  g092(.a(z08), .b(new_n139_), .c(new_n65_), .O(new_n155_));
  oai22  g093(.a(new_n155_), .b(new_n154_), .c(x19), .d(x18), .O(new_n156_));
  nand3  g094(.a(new_n93_), .b(new_n66_), .c(x28), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n79_), .c(new_n78_), .O(new_n159_));
  nand3  g097(.a(new_n155_), .b(new_n91_), .c(new_n93_), .O(new_n160_));
  nand4  g098(.a(x40), .b(x39), .c(new_n139_), .d(new_n65_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n64_), .c(new_n146_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(new_n160_), .c(new_n159_), .d(new_n156_), .O(z14));
  nand4  g101(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(x23), .O(z16));
  inv1   g105(.a(x23), .O(new_n168_));
  nor3   g106(.a(new_n166_), .b(x24), .c(new_n168_), .O(z17));
  oai21  g107(.a(new_n81_), .b(x27), .c(new_n127_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x08), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(new_n130_), .c(new_n78_), .d(new_n126_), .O(z18));
  inv1   g110(.a(new_n127_), .O(new_n173_));
  oai21  g111(.a(new_n66_), .b(new_n64_), .c(new_n88_), .O(new_n174_));
  aoi21  g112(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n173_), .c(new_n174_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n78_), .c(new_n126_), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(z20));
  inv1   g116(.a(new_n133_), .O(z19));
endmodule


