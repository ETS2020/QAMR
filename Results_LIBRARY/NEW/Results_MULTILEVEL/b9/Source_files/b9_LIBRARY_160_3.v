// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n98_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n153_, new_n155_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  aoi21  g004(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  nand4  g006(.a(x40), .b(x39), .c(x10), .d(new_n68_), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n67_), .c(new_n63_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(x16), .O(z00));
  inv1   g010(.a(x30), .O(new_n73_));
  inv1   g011(.a(x32), .O(new_n74_));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x27), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n74_), .c(new_n73_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x04), .O(z01));
  inv1   g018(.a(x02), .O(new_n81_));
  inv1   g019(.a(x08), .O(new_n82_));
  inv1   g020(.a(x29), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n82_), .c(x04), .O(new_n87_));
  inv1   g025(.a(x39), .O(new_n88_));
  nor2   g026(.a(x40), .b(x27), .O(new_n89_));
  aoi21  g027(.a(x40), .b(new_n88_), .c(new_n89_), .O(new_n90_));
  nand4  g028(.a(new_n90_), .b(new_n87_), .c(new_n84_), .d(new_n64_), .O(z02));
  nand2  g029(.a(new_n66_), .b(x27), .O(new_n92_));
  nand2  g030(.a(new_n65_), .b(new_n64_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n92_), .c(new_n95_), .O(z04));
  oai21  g034(.a(new_n85_), .b(new_n64_), .c(new_n65_), .O(z05));
  inv1   g035(.a(x40), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n85_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(x27), .c(x37), .O(z06));
  inv1   g038(.a(x03), .O(new_n101_));
  inv1   g039(.a(x31), .O(new_n102_));
  inv1   g040(.a(x33), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n63_), .O(new_n104_));
  inv1   g042(.a(x00), .O(new_n105_));
  oai21  g043(.a(x25), .b(new_n105_), .c(x38), .O(new_n106_));
  nand4  g044(.a(new_n106_), .b(new_n104_), .c(new_n103_), .d(new_n102_), .O(new_n107_));
  inv1   g045(.a(x14), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand3  g047(.a(x38), .b(new_n109_), .c(new_n105_), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(new_n107_), .c(new_n101_), .O(z07));
  nor2   g050(.a(new_n98_), .b(new_n88_), .O(z08));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z09));
  nand2  g053(.a(new_n74_), .b(new_n73_), .O(new_n116_));
  nand4  g054(.a(new_n116_), .b(x40), .c(x39), .d(x05), .O(new_n117_));
  nand3  g055(.a(new_n77_), .b(x27), .c(x07), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n68_), .O(new_n120_));
  nand3  g058(.a(x37), .b(x27), .c(x06), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(z10));
  nand2  g060(.a(new_n83_), .b(x28), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(x08), .c(x27), .O(new_n124_));
  oai21  g062(.a(new_n98_), .b(new_n88_), .c(x28), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n75_), .O(new_n127_));
  nand2  g065(.a(x27), .b(x04), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n125_), .O(new_n129_));
  oai21  g067(.a(new_n129_), .b(new_n124_), .c(new_n73_), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(x09), .O(z11));
  oai21  g069(.a(x35), .b(new_n64_), .c(new_n99_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n65_), .O(new_n133_));
  nand3  g071(.a(x39), .b(x10), .c(new_n68_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(x40), .c(new_n89_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n133_), .O(z12));
  inv1   g074(.a(x13), .O(new_n137_));
  nand2  g075(.a(new_n116_), .b(x40), .O(new_n138_));
  nor2   g076(.a(new_n138_), .b(new_n88_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n137_), .c(new_n68_), .O(new_n140_));
  inv1   g078(.a(x18), .O(new_n141_));
  inv1   g079(.a(x19), .O(new_n142_));
  nand4  g080(.a(new_n67_), .b(x20), .c(new_n142_), .d(new_n141_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n140_), .O(z13));
  nand2  g082(.a(new_n138_), .b(new_n64_), .O(new_n145_));
  nand3  g083(.a(x39), .b(new_n137_), .c(new_n68_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x40), .O(new_n147_));
  nand3  g085(.a(x20), .b(new_n142_), .c(new_n141_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n98_), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(new_n147_), .c(new_n145_), .d(new_n133_), .O(z14));
  nand4  g088(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(z15));
  nand2  g090(.a(x22), .b(x01), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(x23), .O(z16));
  inv1   g092(.a(x23), .O(new_n155_));
  nor3   g093(.a(new_n153_), .b(x24), .c(new_n155_), .O(z17));
  inv1   g094(.a(x09), .O(new_n157_));
  oai21  g095(.a(new_n86_), .b(x27), .c(new_n126_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x08), .O(new_n159_));
  inv1   g097(.a(new_n86_), .O(new_n160_));
  nor2   g098(.a(new_n64_), .b(x04), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n160_), .c(x30), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n159_), .c(new_n157_), .O(z18));
  oai21  g101(.a(new_n85_), .b(x27), .c(x35), .O(new_n164_));
  nand2  g102(.a(x40), .b(x29), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  nand2  g104(.a(x40), .b(x28), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x35), .O(new_n168_));
  nor2   g106(.a(x27), .b(x08), .O(new_n169_));
  aoi21  g107(.a(new_n168_), .b(new_n88_), .c(new_n169_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n166_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n73_), .c(new_n157_), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(z20));
  nor2   g111(.a(new_n130_), .b(x09), .O(z19));
endmodule


