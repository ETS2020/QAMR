// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n161_, new_n163_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_;
  inv1   g000(.a(x37), .O(new_n63_));
  nand2  g001(.a(x35), .b(x28), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(x27), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(x15), .c(x16), .O(z00));
  inv1   g005(.a(x30), .O(new_n68_));
  inv1   g006(.a(x32), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  inv1   g008(.a(x28), .O(new_n71_));
  nand2  g009(.a(x35), .b(new_n71_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(x27), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(new_n69_), .c(new_n68_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(x04), .O(z01));
  inv1   g014(.a(x02), .O(new_n77_));
  nand2  g015(.a(x29), .b(x08), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g017(.a(x27), .b(x08), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n72_), .c(x04), .O(new_n81_));
  nand4  g019(.a(new_n81_), .b(new_n79_), .c(x40), .d(x39), .O(z02));
  and2   g020(.a(x40), .b(x16), .O(new_n83_));
  nor2   g021(.a(x37), .b(x27), .O(new_n84_));
  aoi21  g022(.a(new_n64_), .b(x27), .c(new_n84_), .O(new_n85_));
  oai21  g023(.a(new_n83_), .b(x21), .c(new_n85_), .O(z03));
  inv1   g024(.a(x21), .O(new_n87_));
  oai21  g025(.a(new_n83_), .b(new_n87_), .c(new_n85_), .O(z04));
  aoi21  g026(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g027(.a(z06), .O(z05));
  inv1   g028(.a(x03), .O(new_n91_));
  inv1   g029(.a(x31), .O(new_n92_));
  inv1   g030(.a(x33), .O(new_n93_));
  inv1   g031(.a(x15), .O(new_n94_));
  nand2  g032(.a(x17), .b(new_n94_), .O(new_n95_));
  inv1   g033(.a(x00), .O(new_n96_));
  oai21  g034(.a(x25), .b(new_n96_), .c(x38), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n95_), .c(new_n93_), .d(new_n92_), .O(new_n98_));
  inv1   g036(.a(x14), .O(new_n99_));
  inv1   g037(.a(x25), .O(new_n100_));
  nand3  g038(.a(x38), .b(new_n100_), .c(new_n96_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n98_), .c(new_n91_), .O(z07));
  nand2  g041(.a(x40), .b(x39), .O(new_n104_));
  inv1   g042(.a(new_n104_), .O(z08));
  nand4  g043(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z09));
  inv1   g045(.a(x04), .O(new_n108_));
  nand2  g046(.a(new_n104_), .b(x07), .O(new_n109_));
  nand3  g047(.a(x40), .b(x39), .c(x05), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n73_), .c(x27), .O(new_n112_));
  inv1   g050(.a(x39), .O(new_n113_));
  oai21  g051(.a(x32), .b(x30), .c(x40), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x05), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n112_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n108_), .O(new_n118_));
  nand3  g056(.a(x37), .b(x27), .c(x06), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(z10));
  inv1   g058(.a(x09), .O(new_n121_));
  nand3  g059(.a(x40), .b(x39), .c(x29), .O(new_n122_));
  inv1   g060(.a(x27), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n108_), .O(new_n124_));
  oai21  g062(.a(new_n124_), .b(new_n72_), .c(new_n122_), .O(new_n125_));
  inv1   g063(.a(x08), .O(new_n126_));
  nand4  g064(.a(x35), .b(new_n71_), .c(x27), .d(new_n108_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n68_), .c(new_n121_), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(z11));
  inv1   g069(.a(x35), .O(new_n132_));
  nor2   g070(.a(x37), .b(x36), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n132_), .c(new_n123_), .O(new_n134_));
  oai21  g072(.a(new_n104_), .b(x04), .c(new_n66_), .O(new_n135_));
  oai21  g073(.a(new_n134_), .b(x10), .c(new_n135_), .O(z12));
  inv1   g074(.a(x13), .O(new_n137_));
  inv1   g075(.a(x40), .O(new_n138_));
  nand2  g076(.a(x36), .b(x35), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(x28), .c(new_n138_), .O(new_n140_));
  nand4  g078(.a(new_n140_), .b(x39), .c(new_n137_), .d(new_n108_), .O(new_n141_));
  inv1   g079(.a(x18), .O(new_n142_));
  inv1   g080(.a(x19), .O(new_n143_));
  nand4  g081(.a(new_n65_), .b(x20), .c(new_n143_), .d(new_n142_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x27), .O(new_n146_));
  nand3  g084(.a(new_n115_), .b(new_n137_), .c(new_n108_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(z13));
  nand3  g086(.a(x20), .b(new_n143_), .c(new_n142_), .O(new_n149_));
  nand4  g087(.a(new_n139_), .b(new_n69_), .c(new_n68_), .d(x28), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  nand3  g089(.a(z08), .b(new_n137_), .c(new_n108_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n151_), .c(new_n149_), .O(new_n153_));
  nand3  g091(.a(new_n63_), .b(new_n132_), .c(x28), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x27), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n69_), .c(new_n68_), .O(new_n156_));
  nand2  g094(.a(new_n152_), .b(new_n66_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n156_), .c(new_n153_), .O(z14));
  nand4  g096(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(z15));
  nand2  g098(.a(x22), .b(x01), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(x23), .O(z16));
  inv1   g100(.a(x23), .O(new_n163_));
  nor3   g101(.a(new_n161_), .b(x24), .c(new_n163_), .O(z17));
  oai21  g102(.a(new_n72_), .b(x27), .c(new_n122_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x08), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n127_), .c(new_n68_), .d(new_n121_), .O(z18));
  inv1   g105(.a(new_n122_), .O(new_n168_));
  oai21  g106(.a(new_n132_), .b(new_n123_), .c(new_n126_), .O(new_n169_));
  aoi21  g107(.a(x28), .b(new_n123_), .c(new_n132_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n168_), .c(new_n169_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n68_), .c(new_n121_), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(z20));
  inv1   g111(.a(new_n130_), .O(z19));
endmodule


