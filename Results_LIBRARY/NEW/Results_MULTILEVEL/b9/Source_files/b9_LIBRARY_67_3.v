// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n147_, new_n149_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  inv1   g002(.a(x10), .O(new_n65_));
  oai21  g003(.a(x36), .b(x35), .c(x27), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g005(.a(new_n67_), .b(x40), .c(x39), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  inv1   g007(.a(x37), .O(z06));
  nor2   g008(.a(z06), .b(new_n69_), .O(new_n71_));
  inv1   g009(.a(new_n71_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n63_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(x16), .O(z00));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  nand3  g015(.a(new_n66_), .b(new_n77_), .c(new_n76_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x04), .O(z01));
  inv1   g017(.a(x02), .O(new_n80_));
  nand2  g018(.a(x29), .b(x08), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n83_), .c(x04), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  inv1   g025(.a(new_n87_), .O(z08));
  nand3  g026(.a(z08), .b(new_n86_), .c(new_n82_), .O(z02));
  nand3  g027(.a(x37), .b(new_n69_), .c(x21), .O(z03));
  inv1   g028(.a(x21), .O(new_n91_));
  nand3  g029(.a(x37), .b(new_n69_), .c(new_n91_), .O(z04));
  inv1   g030(.a(x03), .O(new_n93_));
  inv1   g031(.a(x31), .O(new_n94_));
  inv1   g032(.a(x33), .O(new_n95_));
  nand2  g033(.a(x17), .b(new_n63_), .O(new_n96_));
  inv1   g034(.a(x00), .O(new_n97_));
  oai21  g035(.a(x25), .b(new_n97_), .c(x38), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n99_));
  inv1   g037(.a(x14), .O(new_n100_));
  inv1   g038(.a(x25), .O(new_n101_));
  nand3  g039(.a(x38), .b(new_n101_), .c(new_n97_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n99_), .c(new_n93_), .O(z07));
  nand4  g042(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(z09));
  inv1   g044(.a(x35), .O(new_n107_));
  inv1   g045(.a(x36), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g047(.a(new_n87_), .b(x07), .O(new_n110_));
  nand3  g048(.a(x40), .b(x39), .c(x05), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n109_), .c(x27), .O(new_n113_));
  nand2  g051(.a(new_n77_), .b(new_n76_), .O(new_n114_));
  nand4  g052(.a(new_n114_), .b(x40), .c(x39), .d(x05), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n64_), .O(new_n117_));
  nand2  g055(.a(new_n71_), .b(x06), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(z10));
  inv1   g057(.a(x09), .O(new_n120_));
  nand2  g058(.a(x35), .b(x27), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(x04), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(x29), .O(new_n123_));
  nand2  g061(.a(x27), .b(x04), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(x35), .c(new_n84_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  oai21  g064(.a(new_n122_), .b(x08), .c(new_n126_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n76_), .c(new_n120_), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(z11));
  nor2   g067(.a(new_n87_), .b(x04), .O(new_n130_));
  nor2   g068(.a(x37), .b(x36), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n107_), .c(new_n69_), .O(new_n132_));
  oai22  g070(.a(new_n132_), .b(x10), .c(new_n130_), .d(new_n71_), .O(z12));
  inv1   g071(.a(x18), .O(new_n134_));
  inv1   g072(.a(x19), .O(new_n135_));
  nand4  g073(.a(x37), .b(x20), .c(new_n135_), .d(new_n134_), .O(new_n136_));
  inv1   g074(.a(x13), .O(new_n137_));
  nand3  g075(.a(z08), .b(new_n137_), .c(new_n64_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x27), .O(new_n140_));
  and2   g078(.a(new_n114_), .b(x40), .O(new_n141_));
  nand4  g079(.a(new_n141_), .b(x39), .c(new_n137_), .d(new_n64_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n140_), .O(z13));
  nand4  g081(.a(new_n71_), .b(x20), .c(new_n135_), .d(new_n134_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n138_), .O(new_n145_));
  oai21  g083(.a(new_n114_), .b(x27), .c(new_n145_), .O(z14));
  nand4  g084(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(z15));
  nand2  g086(.a(x22), .b(x01), .O(new_n149_));
  nor2   g087(.a(new_n149_), .b(x23), .O(z16));
  inv1   g088(.a(x23), .O(new_n151_));
  nor3   g089(.a(new_n149_), .b(x24), .c(new_n151_), .O(z17));
  oai21  g090(.a(new_n85_), .b(x27), .c(new_n123_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x08), .O(new_n154_));
  nor2   g092(.a(new_n122_), .b(x30), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n154_), .c(new_n120_), .O(z18));
  inv1   g094(.a(x08), .O(new_n157_));
  nand2  g095(.a(new_n121_), .b(new_n157_), .O(new_n158_));
  nand2  g096(.a(new_n123_), .b(new_n85_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n76_), .c(new_n120_), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z20));
  buf    g100(.a(x37), .O(z05));
  inv1   g101(.a(new_n128_), .O(z19));
endmodule


