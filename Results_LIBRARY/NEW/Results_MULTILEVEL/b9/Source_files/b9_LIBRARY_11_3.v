// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n153_, new_n155_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x28), .O(new_n64_));
  inv1   g002(.a(x35), .O(new_n65_));
  nand2  g003(.a(x40), .b(x39), .O(new_n66_));
  aoi21  g004(.a(new_n66_), .b(new_n64_), .c(new_n65_), .O(new_n67_));
  inv1   g005(.a(x36), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  oai21  g007(.a(new_n66_), .b(new_n68_), .c(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n67_), .c(x27), .O(new_n71_));
  inv1   g009(.a(new_n66_), .O(z08));
  nand2  g010(.a(z08), .b(x10), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n63_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(x16), .O(z00));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n64_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n68_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x27), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n78_), .c(new_n77_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x04), .O(z01));
  nand2  g021(.a(x29), .b(x08), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(x02), .c(z08), .O(z02));
  nand2  g024(.a(x35), .b(x28), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x27), .O(new_n88_));
  inv1   g026(.a(x27), .O(new_n89_));
  nand2  g027(.a(new_n69_), .b(new_n89_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n88_), .c(x21), .O(z03));
  inv1   g029(.a(x21), .O(new_n92_));
  nand3  g030(.a(new_n90_), .b(new_n88_), .c(new_n92_), .O(z04));
  aoi21  g031(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g032(.a(z06), .O(z05));
  inv1   g033(.a(x03), .O(new_n96_));
  inv1   g034(.a(x31), .O(new_n97_));
  inv1   g035(.a(x33), .O(new_n98_));
  nand2  g036(.a(x17), .b(new_n63_), .O(new_n99_));
  inv1   g037(.a(x00), .O(new_n100_));
  oai21  g038(.a(x25), .b(new_n100_), .c(x38), .O(new_n101_));
  nand4  g039(.a(new_n101_), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand3  g042(.a(x38), .b(new_n104_), .c(new_n100_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n102_), .c(new_n96_), .O(z07));
  nand4  g045(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(z09));
  inv1   g047(.a(x07), .O(new_n110_));
  inv1   g048(.a(x39), .O(new_n111_));
  nor2   g049(.a(x40), .b(x04), .O(new_n112_));
  aoi21  g050(.a(x40), .b(new_n111_), .c(new_n112_), .O(new_n113_));
  nand2  g051(.a(z08), .b(x05), .O(new_n114_));
  oai21  g052(.a(new_n113_), .b(new_n110_), .c(new_n114_), .O(new_n115_));
  and2   g053(.a(x37), .b(x06), .O(new_n116_));
  aoi21  g054(.a(new_n115_), .b(new_n80_), .c(new_n116_), .O(new_n117_));
  nand2  g055(.a(new_n78_), .b(new_n77_), .O(new_n118_));
  nand4  g056(.a(new_n118_), .b(x40), .c(x39), .d(x05), .O(new_n119_));
  oai21  g057(.a(new_n117_), .b(new_n89_), .c(new_n119_), .O(z10));
  inv1   g058(.a(x09), .O(new_n121_));
  oai21  g059(.a(new_n84_), .b(new_n66_), .c(new_n79_), .O(new_n122_));
  nor2   g060(.a(x27), .b(x08), .O(new_n123_));
  aoi21  g061(.a(x27), .b(x04), .c(new_n123_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n77_), .c(new_n121_), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(z11));
  nor2   g065(.a(x37), .b(x36), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n65_), .c(new_n89_), .O(new_n129_));
  nand2  g067(.a(new_n87_), .b(new_n69_), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n89_), .c(new_n66_), .O(new_n132_));
  oai21  g070(.a(new_n129_), .b(x10), .c(new_n132_), .O(z12));
  nand3  g071(.a(new_n118_), .b(x40), .c(x39), .O(new_n134_));
  nand2  g072(.a(x36), .b(x35), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x28), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x40), .c(x39), .O(new_n137_));
  inv1   g075(.a(x18), .O(new_n138_));
  inv1   g076(.a(x19), .O(new_n139_));
  nand4  g077(.a(new_n130_), .b(x20), .c(new_n139_), .d(new_n138_), .O(new_n140_));
  oai21  g078(.a(new_n137_), .b(x13), .c(new_n140_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x27), .O(new_n142_));
  oai21  g080(.a(new_n134_), .b(x13), .c(new_n142_), .O(z13));
  nand4  g081(.a(x27), .b(x20), .c(new_n139_), .d(new_n138_), .O(new_n144_));
  oai22  g082(.a(new_n144_), .b(new_n131_), .c(new_n66_), .d(x13), .O(new_n145_));
  nand3  g083(.a(x20), .b(new_n139_), .c(new_n138_), .O(new_n146_));
  aoi22  g084(.a(new_n146_), .b(new_n135_), .c(new_n69_), .d(new_n65_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n64_), .c(x27), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n78_), .c(new_n77_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n145_), .O(z14));
  nand4  g088(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(z15));
  nand2  g090(.a(x22), .b(x01), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(x23), .O(z16));
  inv1   g092(.a(x23), .O(new_n155_));
  nor3   g093(.a(new_n153_), .b(x24), .c(new_n155_), .O(z17));
  nand2  g094(.a(z08), .b(x29), .O(new_n157_));
  oai21  g095(.a(new_n79_), .b(x27), .c(new_n157_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x08), .O(new_n159_));
  inv1   g097(.a(new_n79_), .O(new_n160_));
  nor2   g098(.a(new_n89_), .b(x04), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n160_), .c(x30), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n159_), .c(new_n121_), .O(z18));
  inv1   g101(.a(new_n157_), .O(new_n164_));
  inv1   g102(.a(x08), .O(new_n165_));
  oai21  g103(.a(new_n65_), .b(new_n89_), .c(new_n165_), .O(new_n166_));
  aoi21  g104(.a(x28), .b(new_n89_), .c(new_n65_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n164_), .c(new_n166_), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n77_), .c(new_n121_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z20));
  inv1   g108(.a(new_n126_), .O(z19));
endmodule


