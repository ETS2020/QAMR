// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n90_, new_n91_, new_n93_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_;
  inv1   g000(.a(x04), .O(new_n63_));
  inv1   g001(.a(x39), .O(new_n64_));
  oai21  g002(.a(x27), .b(x10), .c(x40), .O(new_n65_));
  nor2   g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n66_), .b(new_n63_), .c(new_n70_), .O(new_n71_));
  oai21  g009(.a(new_n71_), .b(x15), .c(x16), .O(z00));
  inv1   g010(.a(x30), .O(new_n73_));
  inv1   g011(.a(x32), .O(new_n74_));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x27), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n74_), .c(new_n73_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x04), .O(z01));
  aoi21  g018(.a(x29), .b(x08), .c(x02), .O(new_n81_));
  nor2   g019(.a(new_n75_), .b(x28), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n63_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n81_), .c(x40), .O(new_n84_));
  inv1   g022(.a(x08), .O(new_n85_));
  nand3  g023(.a(new_n67_), .b(new_n85_), .c(x04), .O(new_n86_));
  inv1   g024(.a(x40), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n64_), .O(z08));
  nand3  g026(.a(z08), .b(new_n86_), .c(new_n84_), .O(z02));
  nand2  g027(.a(new_n69_), .b(x27), .O(new_n90_));
  nand2  g028(.a(new_n68_), .b(new_n67_), .O(new_n91_));
  nand3  g029(.a(new_n91_), .b(new_n90_), .c(x21), .O(z03));
  inv1   g030(.a(x21), .O(new_n93_));
  nand3  g031(.a(new_n91_), .b(new_n90_), .c(new_n93_), .O(z04));
  aoi21  g032(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g033(.a(z06), .O(z05));
  inv1   g034(.a(x03), .O(new_n97_));
  inv1   g035(.a(x31), .O(new_n98_));
  inv1   g036(.a(x33), .O(new_n99_));
  inv1   g037(.a(x15), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n100_), .O(new_n101_));
  inv1   g039(.a(x00), .O(new_n102_));
  oai21  g040(.a(x25), .b(new_n102_), .c(x38), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n101_), .c(new_n99_), .d(new_n98_), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand3  g044(.a(x38), .b(new_n106_), .c(new_n102_), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n104_), .c(new_n97_), .O(z07));
  nand4  g047(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(z09));
  inv1   g049(.a(x07), .O(new_n112_));
  nand2  g050(.a(x39), .b(x05), .O(new_n113_));
  oai21  g051(.a(x39), .b(new_n112_), .c(new_n113_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x40), .O(new_n115_));
  nand3  g053(.a(new_n77_), .b(new_n87_), .c(x07), .O(new_n116_));
  aoi21  g054(.a(new_n116_), .b(new_n115_), .c(new_n67_), .O(new_n117_));
  inv1   g055(.a(x05), .O(new_n118_));
  nor2   g056(.a(x32), .b(x30), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n87_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(x39), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  oai21  g060(.a(new_n122_), .b(new_n117_), .c(new_n63_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(z10));
  inv1   g063(.a(x09), .O(new_n126_));
  nand2  g064(.a(z08), .b(x29), .O(new_n127_));
  nand2  g065(.a(x27), .b(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n82_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand3  g068(.a(new_n82_), .b(x27), .c(new_n63_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n85_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n73_), .c(new_n126_), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(z11));
  nor3   g073(.a(new_n87_), .b(new_n64_), .c(x04), .O(new_n136_));
  nand2  g074(.a(x40), .b(new_n67_), .O(new_n137_));
  oai22  g075(.a(new_n137_), .b(x10), .c(new_n136_), .d(new_n70_), .O(z12));
  inv1   g076(.a(x13), .O(new_n139_));
  aoi21  g077(.a(new_n75_), .b(x28), .c(new_n87_), .O(new_n140_));
  nand4  g078(.a(new_n140_), .b(x39), .c(new_n139_), .d(new_n63_), .O(new_n141_));
  inv1   g079(.a(x19), .O(new_n142_));
  nand2  g080(.a(new_n69_), .b(new_n68_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(x20), .c(new_n142_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(x18), .c(new_n141_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x27), .O(new_n146_));
  nand4  g084(.a(new_n120_), .b(x39), .c(new_n139_), .d(new_n63_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(z13));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(x37), .c(x20), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n74_), .c(new_n73_), .d(x28), .O(new_n151_));
  nand3  g089(.a(z08), .b(new_n139_), .c(new_n63_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n68_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n75_), .O(new_n155_));
  inv1   g093(.a(x28), .O(new_n156_));
  nand2  g094(.a(new_n68_), .b(new_n156_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n149_), .c(x20), .O(new_n158_));
  nor2   g096(.a(new_n119_), .b(x04), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x40), .c(x39), .d(new_n139_), .O(new_n160_));
  aoi22  g098(.a(new_n160_), .b(new_n67_), .c(new_n158_), .d(new_n152_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n155_), .O(z14));
  nand4  g100(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z15));
  nand2  g102(.a(x22), .b(x01), .O(new_n165_));
  nor2   g103(.a(new_n165_), .b(x23), .O(z16));
  inv1   g104(.a(x23), .O(new_n167_));
  nor3   g105(.a(new_n165_), .b(x24), .c(new_n167_), .O(z17));
  nand2  g106(.a(new_n82_), .b(new_n67_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n127_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x08), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(new_n131_), .c(new_n73_), .d(new_n126_), .O(z18));
  inv1   g110(.a(new_n127_), .O(new_n173_));
  oai21  g111(.a(new_n75_), .b(new_n67_), .c(new_n85_), .O(new_n174_));
  aoi21  g112(.a(x28), .b(new_n67_), .c(new_n75_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n173_), .c(new_n174_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n73_), .c(new_n126_), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(z20));
  inv1   g116(.a(new_n134_), .O(z19));
endmodule


