// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n96_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n155_, new_n157_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(z08));
  nand3  g014(.a(z08), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x16), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n67_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x27), .c(x32), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n81_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x04), .O(z01));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  aoi21  g026(.a(x28), .b(x04), .c(new_n88_), .O(new_n89_));
  oai21  g027(.a(x27), .b(x08), .c(x35), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x04), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n89_), .c(x40), .d(x39), .O(z02));
  nand2  g030(.a(new_n71_), .b(x27), .O(new_n93_));
  nand2  g031(.a(new_n70_), .b(new_n64_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n93_), .c(new_n96_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g036(.a(z06), .O(z05));
  inv1   g037(.a(x03), .O(new_n100_));
  inv1   g038(.a(x31), .O(new_n101_));
  inv1   g039(.a(x33), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n63_), .O(new_n103_));
  inv1   g041(.a(x00), .O(new_n104_));
  oai21  g042(.a(x25), .b(new_n104_), .c(x38), .O(new_n105_));
  nand4  g043(.a(new_n105_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand3  g046(.a(x38), .b(new_n108_), .c(new_n104_), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  aoi21  g048(.a(new_n110_), .b(new_n106_), .c(new_n100_), .O(z07));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z09));
  nand4  g051(.a(new_n84_), .b(new_n75_), .c(x27), .d(x07), .O(new_n114_));
  nand2  g052(.a(z08), .b(x05), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n65_), .O(new_n117_));
  nand3  g055(.a(x37), .b(x27), .c(x06), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(z10));
  inv1   g057(.a(x09), .O(new_n120_));
  nand2  g058(.a(z08), .b(x29), .O(new_n121_));
  nor2   g059(.a(new_n64_), .b(new_n65_), .O(new_n122_));
  oai21  g060(.a(new_n122_), .b(new_n83_), .c(new_n121_), .O(new_n123_));
  inv1   g061(.a(x08), .O(new_n124_));
  nand2  g062(.a(x40), .b(x04), .O(new_n125_));
  nand4  g063(.a(new_n125_), .b(x35), .c(new_n82_), .d(x27), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n81_), .c(new_n120_), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(z11));
  inv1   g068(.a(x10), .O(new_n131_));
  nor3   g069(.a(x37), .b(x36), .c(x35), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n64_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(z08), .b(new_n65_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n71_), .c(new_n70_), .O(new_n135_));
  nand3  g073(.a(x40), .b(x39), .c(new_n65_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n64_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n135_), .c(new_n133_), .O(z12));
  inv1   g076(.a(x19), .O(new_n139_));
  nand4  g077(.a(new_n72_), .b(x27), .c(x20), .d(new_n139_), .O(new_n140_));
  nor2   g078(.a(x13), .b(x04), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(z08), .O(new_n142_));
  oai21  g080(.a(new_n140_), .b(x18), .c(new_n142_), .O(z13));
  inv1   g081(.a(x18), .O(new_n144_));
  nand3  g082(.a(x20), .b(new_n139_), .c(new_n144_), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x40), .c(new_n73_), .O(new_n146_));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(new_n72_), .c(x27), .d(x20), .O(new_n148_));
  inv1   g086(.a(x13), .O(new_n149_));
  aoi21  g087(.a(new_n125_), .b(new_n149_), .c(x27), .O(new_n150_));
  aoi21  g088(.a(new_n148_), .b(new_n75_), .c(new_n150_), .O(new_n151_));
  oai21  g089(.a(new_n146_), .b(new_n141_), .c(new_n151_), .O(z14));
  nand4  g090(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(z15));
  nand2  g092(.a(x22), .b(x01), .O(new_n155_));
  nor2   g093(.a(new_n155_), .b(x23), .O(z16));
  inv1   g094(.a(x23), .O(new_n157_));
  nor3   g095(.a(new_n155_), .b(x24), .c(new_n157_), .O(z17));
  oai21  g096(.a(new_n83_), .b(x27), .c(new_n121_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x08), .O(new_n160_));
  inv1   g098(.a(new_n83_), .O(new_n161_));
  nor2   g099(.a(new_n64_), .b(x04), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n161_), .c(x30), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n160_), .c(new_n120_), .O(z18));
  inv1   g102(.a(new_n121_), .O(new_n165_));
  oai21  g103(.a(new_n66_), .b(new_n64_), .c(new_n124_), .O(new_n166_));
  aoi21  g104(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n165_), .c(new_n166_), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n81_), .c(new_n120_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z20));
  inv1   g108(.a(new_n129_), .O(z19));
endmodule


