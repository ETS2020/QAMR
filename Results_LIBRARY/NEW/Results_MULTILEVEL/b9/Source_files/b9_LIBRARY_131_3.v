// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n110_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n172_, new_n174_,
    new_n176_, new_n177_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n69_), .c(new_n64_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(z08));
  nand3  g014(.a(z08), .b(x10), .c(new_n65_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n63_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x16), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n82_), .c(new_n81_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x04), .O(z01));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  inv1   g029(.a(x18), .O(new_n92_));
  inv1   g030(.a(x19), .O(new_n93_));
  nor2   g031(.a(new_n73_), .b(new_n64_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(x20), .c(new_n93_), .d(new_n92_), .O(new_n95_));
  inv1   g033(.a(x20), .O(new_n96_));
  nand2  g034(.a(new_n75_), .b(new_n96_), .O(new_n97_));
  oai21  g035(.a(x08), .b(new_n65_), .c(x40), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n64_), .O(new_n99_));
  nand2  g037(.a(new_n84_), .b(x04), .O(new_n100_));
  inv1   g038(.a(x40), .O(new_n101_));
  nand3  g039(.a(new_n72_), .b(new_n93_), .c(new_n92_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand4  g041(.a(new_n103_), .b(new_n100_), .c(new_n99_), .d(new_n97_), .O(new_n104_));
  inv1   g042(.a(new_n104_), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n95_), .c(new_n91_), .O(z02));
  nand2  g044(.a(new_n71_), .b(x27), .O(new_n107_));
  nand2  g045(.a(new_n70_), .b(new_n64_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x21), .O(z03));
  inv1   g047(.a(x21), .O(new_n110_));
  nand3  g048(.a(new_n108_), .b(new_n107_), .c(new_n110_), .O(z04));
  aoi21  g049(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g050(.a(z06), .O(z05));
  inv1   g051(.a(x03), .O(new_n114_));
  inv1   g052(.a(x31), .O(new_n115_));
  inv1   g053(.a(x33), .O(new_n116_));
  nand2  g054(.a(x17), .b(new_n63_), .O(new_n117_));
  inv1   g055(.a(x00), .O(new_n118_));
  oai21  g056(.a(x25), .b(new_n118_), .c(x38), .O(new_n119_));
  nand4  g057(.a(new_n119_), .b(new_n117_), .c(new_n116_), .d(new_n115_), .O(new_n120_));
  inv1   g058(.a(x14), .O(new_n121_));
  inv1   g059(.a(x25), .O(new_n122_));
  nand3  g060(.a(x38), .b(new_n122_), .c(new_n118_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n120_), .c(new_n114_), .O(z07));
  nand4  g063(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(z09));
  nand2  g065(.a(new_n75_), .b(x07), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x05), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n85_), .c(x27), .O(new_n131_));
  inv1   g069(.a(x39), .O(new_n132_));
  oai21  g070(.a(x32), .b(x30), .c(x40), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x05), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n65_), .O(new_n137_));
  nand3  g075(.a(x37), .b(x27), .c(x06), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(z10));
  inv1   g077(.a(x09), .O(new_n140_));
  nand2  g078(.a(z08), .b(x29), .O(new_n141_));
  nor2   g079(.a(new_n64_), .b(new_n65_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n84_), .c(new_n141_), .O(new_n143_));
  inv1   g081(.a(x08), .O(new_n144_));
  nor2   g082(.a(new_n66_), .b(x28), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(x27), .c(new_n65_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n81_), .c(new_n140_), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(z11));
  nor2   g088(.a(x37), .b(x36), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n66_), .c(new_n64_), .O(new_n152_));
  nor2   g090(.a(new_n75_), .b(x04), .O(new_n153_));
  oai22  g091(.a(new_n153_), .b(new_n94_), .c(new_n152_), .d(x10), .O(z12));
  inv1   g092(.a(x13), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x28), .c(new_n101_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(x39), .c(new_n155_), .d(new_n65_), .O(new_n158_));
  nand4  g096(.a(new_n72_), .b(x20), .c(new_n93_), .d(new_n92_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nand3  g099(.a(new_n134_), .b(new_n155_), .c(new_n65_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n161_), .O(z13));
  nand2  g101(.a(new_n133_), .b(new_n64_), .O(new_n164_));
  oai22  g102(.a(new_n101_), .b(x35), .c(x36), .d(x20), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(new_n82_), .c(new_n81_), .d(x28), .O(new_n166_));
  aoi21  g104(.a(new_n155_), .b(new_n65_), .c(new_n101_), .O(new_n167_));
  aoi21  g105(.a(new_n102_), .b(new_n101_), .c(new_n167_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n166_), .c(new_n164_), .d(new_n97_), .O(z14));
  nand4  g107(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z15));
  nand2  g109(.a(x22), .b(x01), .O(new_n172_));
  nor2   g110(.a(new_n172_), .b(x23), .O(z16));
  inv1   g111(.a(x23), .O(new_n174_));
  nor3   g112(.a(new_n172_), .b(x24), .c(new_n174_), .O(z17));
  oai21  g113(.a(new_n84_), .b(x27), .c(new_n141_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x08), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n146_), .c(new_n81_), .d(new_n140_), .O(z18));
  inv1   g116(.a(new_n141_), .O(new_n179_));
  oai21  g117(.a(new_n66_), .b(new_n64_), .c(new_n144_), .O(new_n180_));
  aoi21  g118(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n179_), .c(new_n180_), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n81_), .c(new_n140_), .O(new_n183_));
  inv1   g121(.a(new_n183_), .O(z20));
  inv1   g122(.a(new_n149_), .O(z19));
endmodule


