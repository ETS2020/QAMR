// Benchmark "FAU" written by ABC on Mon Jul 27 18:45:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x20), .O(new_n46_));
  inv1   g02(.a(x24), .O(new_n47_));
  inv1   g03(.a(x25), .O(new_n48_));
  oai21  g04(.a(x15), .b(x13), .c(x26), .O(new_n49_));
  nor3   g05(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand4  g06(.a(new_n50_), .b(x23), .c(x22), .d(x21), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(x19), .c(new_n45_), .d(x09), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(x08), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  inv1   g11(.a(x08), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z02));
  nand2  g15(.a(x08), .b(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  inv1   g17(.a(x03), .O(new_n62_));
  nor2   g18(.a(new_n56_), .b(new_n62_), .O(z04));
  inv1   g19(.a(x04), .O(new_n64_));
  nor2   g20(.a(new_n56_), .b(new_n64_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  inv1   g23(.a(x06), .O(new_n68_));
  nor2   g24(.a(new_n56_), .b(new_n68_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  and2   g27(.a(x22), .b(x21), .O(new_n72_));
  nand3  g28(.a(new_n72_), .b(x20), .c(x11), .O(new_n73_));
  inv1   g29(.a(x23), .O(new_n74_));
  nor2   g30(.a(new_n47_), .b(new_n74_), .O(new_n75_));
  inv1   g31(.a(x26), .O(new_n76_));
  nor2   g32(.a(new_n76_), .b(new_n48_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n73_), .c(x19), .O(new_n79_));
  nand4  g35(.a(new_n79_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n80_));
  oai21  g36(.a(new_n56_), .b(new_n55_), .c(new_n80_), .O(z09));
  nand2  g37(.a(new_n72_), .b(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n78_), .c(x19), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(x20), .O(new_n84_));
  nand2  g40(.a(new_n46_), .b(x19), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g42(.a(new_n86_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n58_), .O(z10));
  nand2  g44(.a(new_n72_), .b(x13), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n78_), .c(x21), .O(new_n90_));
  nand3  g46(.a(new_n90_), .b(x20), .c(x19), .O(new_n91_));
  inv1   g47(.a(x19), .O(new_n92_));
  nor2   g48(.a(new_n46_), .b(new_n92_), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(x21), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand4  g52(.a(new_n96_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(new_n60_), .O(z11));
  nand2  g54(.a(x23), .b(x14), .O(new_n99_));
  nand3  g55(.a(x26), .b(x25), .c(x24), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n99_), .c(x21), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n94_), .c(x22), .O(new_n102_));
  inv1   g58(.a(x22), .O(new_n103_));
  nand3  g59(.a(new_n93_), .b(new_n103_), .c(x21), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand4  g61(.a(new_n105_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n106_));
  oai21  g62(.a(new_n56_), .b(new_n62_), .c(new_n106_), .O(z12));
  inv1   g63(.a(x15), .O(new_n108_));
  oai21  g64(.a(new_n100_), .b(new_n108_), .c(x23), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(x22), .c(x21), .d(x20), .O(new_n110_));
  nand2  g66(.a(new_n93_), .b(new_n72_), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  oai22  g68(.a(new_n112_), .b(new_n74_), .c(new_n110_), .d(new_n92_), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n114_));
  oai21  g70(.a(new_n56_), .b(new_n64_), .c(new_n114_), .O(z13));
  nand3  g71(.a(x21), .b(x20), .c(x19), .O(new_n116_));
  nand2  g72(.a(new_n77_), .b(x16), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(x23), .c(x22), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n116_), .c(x24), .O(new_n119_));
  inv1   g75(.a(new_n116_), .O(new_n120_));
  nand4  g76(.a(new_n120_), .b(new_n47_), .c(x23), .d(x22), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand4  g78(.a(new_n122_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n66_), .O(z14));
  nor2   g80(.a(new_n74_), .b(new_n103_), .O(new_n125_));
  nand2  g81(.a(x26), .b(x17), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n125_), .c(x24), .O(new_n127_));
  oai21  g83(.a(new_n127_), .b(new_n116_), .c(x25), .O(new_n128_));
  nand4  g84(.a(new_n125_), .b(new_n120_), .c(new_n48_), .d(x24), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand4  g86(.a(new_n130_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n131_));
  oai21  g87(.a(new_n56_), .b(new_n68_), .c(new_n131_), .O(z15));
  inv1   g88(.a(x18), .O(new_n133_));
  nand4  g89(.a(x21), .b(x20), .c(x19), .d(new_n133_), .O(new_n134_));
  nand3  g90(.a(new_n125_), .b(x25), .c(x24), .O(new_n135_));
  oai21  g91(.a(new_n135_), .b(new_n134_), .c(x26), .O(new_n136_));
  nand4  g92(.a(new_n112_), .b(new_n75_), .c(new_n76_), .d(x25), .O(new_n137_));
  nand2  g93(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand4  g94(.a(new_n138_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n139_));
  nand2  g95(.a(new_n139_), .b(new_n70_), .O(z16));
endmodule


