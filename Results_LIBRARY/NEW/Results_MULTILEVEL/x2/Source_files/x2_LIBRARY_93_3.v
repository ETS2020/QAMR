// Benchmark "FAU" written by ABC on Mon Jul 27 19:01:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n27_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_;
  inv1   g00(.a(x7), .O(new_n18_));
  nor2   g01(.a(x8), .b(new_n18_), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(new_n18_), .O(z0));
  inv1   g03(.a(x8), .O(new_n21_));
  inv1   g04(.a(x9), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x7), .O(new_n23_));
  nand2  g06(.a(x9), .b(new_n18_), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n21_), .O(z1));
  nor3   g08(.a(x9), .b(x8), .c(x7), .O(z2));
  nor3   g09(.a(x2), .b(x1), .c(x0), .O(new_n27_));
  nand4  g10(.a(new_n27_), .b(new_n22_), .c(x8), .d(new_n18_), .O(z3));
  nand3  g11(.a(new_n21_), .b(x7), .c(x6), .O(z4));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  nand3  g14(.a(x2), .b(new_n31_), .c(new_n30_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(x9), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n21_), .c(new_n18_), .O(new_n34_));
  nand3  g17(.a(x9), .b(x8), .c(x5), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(new_n23_), .c(x6), .O(new_n36_));
  inv1   g19(.a(new_n36_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n34_), .O(z5));
  aoi21  g21(.a(x8), .b(x5), .c(x7), .O(new_n39_));
  inv1   g22(.a(x6), .O(new_n40_));
  nor2   g23(.a(x9), .b(x8), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n18_), .c(new_n40_), .O(new_n42_));
  oai21  g25(.a(new_n39_), .b(new_n22_), .c(new_n42_), .O(z6));
endmodule


