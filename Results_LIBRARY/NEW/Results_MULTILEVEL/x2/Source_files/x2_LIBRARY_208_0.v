// Benchmark "FAU" written by ABC on Mon Jul 27 19:02:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n26_, new_n27_,
    new_n28_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  nor2   g02(.a(x9), .b(x8), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nor2   g04(.a(new_n18_), .b(new_n21_), .O(new_n22_));
  aoi21  g05(.a(new_n18_), .b(x7), .c(new_n22_), .O(new_n23_));
  oai21  g06(.a(new_n20_), .b(x7), .c(new_n23_), .O(z1));
  nor3   g07(.a(x9), .b(x8), .c(x7), .O(z2));
  inv1   g08(.a(x0), .O(new_n26_));
  inv1   g09(.a(x1), .O(new_n27_));
  inv1   g10(.a(x2), .O(new_n28_));
  nand4  g11(.a(new_n23_), .b(new_n28_), .c(new_n27_), .d(new_n26_), .O(z3));
  inv1   g12(.a(x7), .O(new_n30_));
  oai21  g13(.a(x9), .b(new_n30_), .c(x8), .O(new_n31_));
  oai21  g14(.a(x9), .b(new_n21_), .c(new_n30_), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(new_n31_), .c(x6), .O(z4));
  inv1   g16(.a(new_n20_), .O(new_n34_));
  nand4  g17(.a(x9), .b(x2), .c(new_n27_), .d(new_n26_), .O(new_n35_));
  inv1   g18(.a(x4), .O(new_n36_));
  nand3  g19(.a(new_n18_), .b(new_n36_), .c(x3), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x7), .O(new_n39_));
  nand3  g22(.a(new_n22_), .b(new_n30_), .c(x5), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(new_n34_), .d(x6), .O(z5));
  nand3  g24(.a(x9), .b(x8), .c(x5), .O(new_n42_));
  inv1   g25(.a(new_n42_), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n20_), .c(new_n30_), .O(new_n44_));
  nand3  g27(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(x8), .c(new_n18_), .O(new_n46_));
  nand4  g29(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n47_));
  inv1   g30(.a(new_n47_), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n46_), .c(x7), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n44_), .c(x6), .O(z6));
endmodule


