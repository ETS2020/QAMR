// Benchmark "FAU" written by ABC on Mon Jul 27 17:10:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand3  g01(.a(x2), .b(new_n16_), .c(x0), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(z0));
  inv1   g03(.a(x0), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  inv1   g06(.a(x2), .O(new_n22_));
  inv1   g07(.a(x5), .O(new_n23_));
  nor3   g08(.a(new_n23_), .b(new_n22_), .c(x0), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n16_), .O(new_n25_));
  oai21  g10(.a(x4), .b(new_n20_), .c(x1), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n20_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n26_), .c(x2), .O(new_n28_));
  nor2   g13(.a(new_n20_), .b(new_n16_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n28_), .c(new_n19_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n25_), .O(z1));
  nand2  g16(.a(x4), .b(new_n22_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n20_), .c(new_n16_), .O(new_n33_));
  nand2  g18(.a(new_n23_), .b(x2), .O(new_n34_));
  inv1   g19(.a(x6), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n20_), .c(new_n22_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n34_), .c(x1), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n33_), .c(new_n19_), .O(new_n38_));
  nand3  g23(.a(x3), .b(new_n16_), .c(x0), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n38_), .O(z2));
  oai21  g25(.a(x4), .b(x3), .c(x7), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n22_), .c(x1), .O(new_n42_));
  oai21  g27(.a(new_n34_), .b(x1), .c(new_n42_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n19_), .O(new_n44_));
  nand3  g29(.a(x2), .b(new_n16_), .c(x0), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z3));
  aoi21  g31(.a(new_n20_), .b(new_n22_), .c(new_n19_), .O(new_n47_));
  aoi21  g32(.a(new_n36_), .b(new_n22_), .c(x0), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(new_n47_), .c(new_n16_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n30_), .O(z4));
  oai21  g35(.a(new_n47_), .b(new_n24_), .c(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n30_), .O(z5));
  inv1   g37(.a(new_n34_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n19_), .c(new_n47_), .O(new_n54_));
  oai21  g39(.a(x4), .b(x2), .c(new_n20_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(x1), .c(new_n19_), .O(new_n56_));
  oai21  g41(.a(new_n54_), .b(x1), .c(new_n56_), .O(z6));
endmodule


