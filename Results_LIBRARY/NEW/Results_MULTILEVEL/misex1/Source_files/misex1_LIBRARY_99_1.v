// Benchmark "FAU" written by ABC on Mon Jul 27 17:10:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  nand2  g07(.a(x6), .b(new_n20_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n19_), .O(new_n24_));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(x1), .O(new_n26_));
  aoi21  g11(.a(new_n24_), .b(new_n16_), .c(new_n26_), .O(new_n27_));
  nand4  g12(.a(x3), .b(new_n16_), .c(new_n19_), .d(x0), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(x0), .c(new_n28_), .O(z1));
  nand2  g14(.a(x4), .b(new_n16_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n20_), .c(new_n19_), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nor2   g17(.a(x6), .b(x2), .O(new_n33_));
  aoi21  g18(.a(new_n32_), .b(x2), .c(new_n33_), .O(new_n34_));
  nor2   g19(.a(new_n34_), .b(x3), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n19_), .c(new_n31_), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(x0), .c(new_n28_), .O(z2));
  nand2  g22(.a(new_n19_), .b(x0), .O(new_n38_));
  nor2   g23(.a(x5), .b(x3), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(x2), .c(new_n19_), .O(new_n40_));
  nor2   g25(.a(x7), .b(x2), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x1), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n20_), .b(x2), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n38_), .c(new_n44_), .O(z3));
  nand2  g31(.a(x3), .b(new_n16_), .O(new_n47_));
  aoi21  g32(.a(new_n45_), .b(new_n47_), .c(new_n17_), .O(new_n48_));
  oai21  g33(.a(new_n34_), .b(x3), .c(new_n25_), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(new_n17_), .c(new_n48_), .O(new_n50_));
  oai21  g35(.a(new_n41_), .b(x3), .c(x1), .O(new_n51_));
  oai21  g36(.a(new_n23_), .b(x2), .c(new_n51_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n17_), .O(new_n53_));
  oai21  g38(.a(new_n50_), .b(x1), .c(new_n53_), .O(z4));
  nor2   g39(.a(new_n25_), .b(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n48_), .c(new_n19_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n53_), .O(z5));
  nand3  g42(.a(new_n39_), .b(x2), .c(new_n17_), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n48_), .c(new_n19_), .O(new_n60_));
  oai21  g45(.a(x4), .b(x2), .c(new_n20_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(x1), .c(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z6));
endmodule


