// Benchmark "FAU" written by ABC on Mon Jul 27 17:10:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand3  g06(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n21_), .c(new_n16_), .O(z0));
  oai21  g08(.a(x4), .b(x3), .c(x1), .O(new_n24_));
  inv1   g09(.a(x4), .O(new_n25_));
  oai21  g10(.a(x6), .b(new_n25_), .c(new_n19_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n24_), .c(x2), .O(new_n27_));
  oai21  g12(.a(x5), .b(x3), .c(x2), .O(new_n28_));
  nor2   g13(.a(new_n28_), .b(x1), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n27_), .c(new_n17_), .O(new_n30_));
  nand4  g15(.a(x3), .b(new_n16_), .c(new_n18_), .d(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(z1));
  nand2  g17(.a(x3), .b(new_n16_), .O(new_n33_));
  nor2   g18(.a(x5), .b(x3), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(x2), .c(new_n17_), .O(new_n35_));
  oai21  g20(.a(new_n33_), .b(new_n17_), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n18_), .O(new_n37_));
  nor2   g22(.a(x6), .b(new_n25_), .O(new_n38_));
  oai21  g23(.a(new_n25_), .b(x2), .c(new_n19_), .O(new_n39_));
  nor2   g24(.a(x3), .b(x2), .O(new_n40_));
  aoi22  g25(.a(new_n40_), .b(new_n38_), .c(new_n39_), .d(x1), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x0), .c(new_n37_), .O(z2));
  nand2  g27(.a(new_n25_), .b(new_n16_), .O(new_n43_));
  inv1   g28(.a(x5), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(x2), .c(new_n18_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n43_), .c(x0), .O(new_n46_));
  nand3  g31(.a(x2), .b(new_n18_), .c(x0), .O(new_n47_));
  inv1   g32(.a(new_n47_), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(new_n46_), .c(new_n19_), .O(new_n49_));
  inv1   g34(.a(x7), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(z3));
  nand2  g37(.a(new_n19_), .b(x2), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n33_), .c(new_n17_), .O(new_n54_));
  nor2   g39(.a(new_n16_), .b(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n18_), .O(new_n56_));
  nor2   g41(.a(x3), .b(x2), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n20_), .c(new_n17_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n56_), .O(z4));
  nor2   g44(.a(new_n28_), .b(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n54_), .c(new_n18_), .O(new_n61_));
  aoi21  g46(.a(x4), .b(new_n16_), .c(x3), .O(new_n62_));
  nor2   g47(.a(new_n62_), .b(new_n18_), .O(new_n63_));
  nor2   g48(.a(new_n26_), .b(x2), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n63_), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n61_), .O(z5));
  inv1   g51(.a(new_n35_), .O(new_n67_));
  oai21  g52(.a(new_n54_), .b(new_n67_), .c(new_n18_), .O(new_n68_));
  nor3   g53(.a(x4), .b(x3), .c(x2), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n20_), .c(new_n17_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n68_), .O(z6));
endmodule


