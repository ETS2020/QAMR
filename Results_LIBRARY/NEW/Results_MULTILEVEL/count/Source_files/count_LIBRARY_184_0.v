// Benchmark "FAU" written by ABC on Mon Jul 27 18:27:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_;
  inv1   g00(.a(x17), .O(new_n52_));
  inv1   g01(.a(x19), .O(new_n53_));
  nor2   g02(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g03(.a(x19), .b(x17), .O(new_n55_));
  oai21  g04(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g05(.a(x15), .O(new_n57_));
  inv1   g06(.a(x16), .O(new_n58_));
  aoi21  g07(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g08(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g09(.a(x20), .O(new_n61_));
  nor2   g10(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g11(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g12(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g13(.a(x14), .O(new_n65_));
  aoi21  g14(.a(new_n58_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g15(.a(new_n66_), .b(new_n64_), .O(z01));
  nand3  g16(.a(new_n61_), .b(new_n53_), .c(new_n52_), .O(new_n68_));
  nor2   g17(.a(x21), .b(x20), .O(new_n69_));
  aoi22  g18(.a(new_n69_), .b(new_n55_), .c(new_n68_), .d(x21), .O(new_n70_));
  inv1   g19(.a(x13), .O(new_n71_));
  aoi21  g20(.a(new_n58_), .b(new_n71_), .c(x18), .O(new_n72_));
  oai21  g21(.a(new_n70_), .b(new_n58_), .c(new_n72_), .O(z02));
  nand2  g22(.a(new_n69_), .b(new_n55_), .O(new_n74_));
  nand2  g23(.a(new_n74_), .b(x22), .O(new_n75_));
  nor2   g24(.a(x22), .b(x21), .O(new_n76_));
  nand3  g25(.a(new_n76_), .b(new_n55_), .c(new_n61_), .O(new_n77_));
  and2   g26(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  inv1   g27(.a(x12), .O(new_n79_));
  aoi21  g28(.a(new_n58_), .b(new_n79_), .c(x18), .O(new_n80_));
  oai21  g29(.a(new_n78_), .b(new_n58_), .c(new_n80_), .O(z03));
  nor3   g30(.a(x23), .b(x22), .c(x21), .O(new_n82_));
  aoi22  g31(.a(new_n82_), .b(new_n63_), .c(new_n77_), .d(x23), .O(new_n83_));
  inv1   g32(.a(x11), .O(new_n84_));
  aoi21  g33(.a(new_n58_), .b(new_n84_), .c(x18), .O(new_n85_));
  oai21  g34(.a(new_n83_), .b(new_n58_), .c(new_n85_), .O(z04));
  inv1   g35(.a(x24), .O(new_n87_));
  aoi21  g36(.a(new_n82_), .b(new_n63_), .c(new_n87_), .O(new_n88_));
  nor2   g37(.a(x24), .b(x23), .O(new_n89_));
  nand3  g38(.a(new_n89_), .b(new_n76_), .c(new_n63_), .O(new_n90_));
  inv1   g39(.a(new_n90_), .O(new_n91_));
  oai21  g40(.a(new_n91_), .b(new_n88_), .c(x16), .O(new_n92_));
  inv1   g41(.a(x10), .O(new_n93_));
  aoi21  g42(.a(new_n58_), .b(new_n93_), .c(x18), .O(new_n94_));
  nand2  g43(.a(new_n94_), .b(new_n92_), .O(z05));
  inv1   g44(.a(x25), .O(new_n96_));
  nor2   g45(.a(x23), .b(x22), .O(new_n97_));
  nand3  g46(.a(new_n97_), .b(new_n96_), .c(new_n87_), .O(new_n98_));
  nor2   g47(.a(new_n98_), .b(new_n74_), .O(new_n99_));
  aoi21  g48(.a(new_n90_), .b(x25), .c(new_n99_), .O(new_n100_));
  inv1   g49(.a(x09), .O(new_n101_));
  aoi21  g50(.a(new_n58_), .b(new_n101_), .c(x18), .O(new_n102_));
  oai21  g51(.a(new_n100_), .b(new_n58_), .c(new_n102_), .O(z06));
  oai21  g52(.a(new_n98_), .b(new_n74_), .c(x26), .O(new_n104_));
  nor3   g53(.a(x26), .b(x25), .c(x24), .O(new_n105_));
  nand4  g54(.a(new_n105_), .b(new_n97_), .c(new_n69_), .d(new_n55_), .O(new_n106_));
  nand2  g55(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g56(.a(new_n107_), .b(x16), .O(new_n108_));
  inv1   g57(.a(x08), .O(new_n109_));
  aoi21  g58(.a(new_n58_), .b(new_n109_), .c(x18), .O(new_n110_));
  nand2  g59(.a(new_n110_), .b(new_n108_), .O(z07));
  nor2   g60(.a(x27), .b(x26), .O(new_n112_));
  nand3  g61(.a(new_n112_), .b(new_n89_), .c(new_n96_), .O(new_n113_));
  nor2   g62(.a(new_n113_), .b(new_n77_), .O(new_n114_));
  aoi21  g63(.a(new_n106_), .b(x27), .c(new_n114_), .O(new_n115_));
  inv1   g64(.a(x07), .O(new_n116_));
  aoi21  g65(.a(new_n58_), .b(new_n116_), .c(x18), .O(new_n117_));
  oai21  g66(.a(new_n115_), .b(new_n58_), .c(new_n117_), .O(z08));
  oai21  g67(.a(new_n113_), .b(new_n77_), .c(x28), .O(new_n119_));
  inv1   g68(.a(x06), .O(new_n120_));
  aoi21  g69(.a(new_n58_), .b(new_n120_), .c(x18), .O(new_n121_));
  oai21  g70(.a(new_n119_), .b(new_n58_), .c(new_n121_), .O(z09));
  inv1   g71(.a(x18), .O(new_n123_));
  nand2  g72(.a(x29), .b(x16), .O(new_n124_));
  inv1   g73(.a(x05), .O(new_n125_));
  nand2  g74(.a(new_n58_), .b(new_n125_), .O(new_n126_));
  nand3  g75(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(z10));
  nand2  g76(.a(x30), .b(x16), .O(new_n128_));
  inv1   g77(.a(x04), .O(new_n129_));
  nand2  g78(.a(new_n58_), .b(new_n129_), .O(new_n130_));
  nand3  g79(.a(new_n130_), .b(new_n128_), .c(new_n123_), .O(z11));
  nand2  g80(.a(x31), .b(x16), .O(new_n132_));
  inv1   g81(.a(x03), .O(new_n133_));
  nand2  g82(.a(new_n58_), .b(new_n133_), .O(new_n134_));
  nand3  g83(.a(new_n134_), .b(new_n132_), .c(new_n123_), .O(z12));
  nand2  g84(.a(x32), .b(x16), .O(new_n136_));
  inv1   g85(.a(x02), .O(new_n137_));
  nand2  g86(.a(new_n58_), .b(new_n137_), .O(new_n138_));
  nand3  g87(.a(new_n138_), .b(new_n136_), .c(new_n123_), .O(z13));
  nand2  g88(.a(x33), .b(x16), .O(new_n140_));
  inv1   g89(.a(x01), .O(new_n141_));
  nand2  g90(.a(new_n58_), .b(new_n141_), .O(new_n142_));
  nand3  g91(.a(new_n142_), .b(new_n140_), .c(new_n123_), .O(z14));
  nand2  g92(.a(x34), .b(x16), .O(new_n144_));
  inv1   g93(.a(x00), .O(new_n145_));
  nand2  g94(.a(new_n58_), .b(new_n145_), .O(new_n146_));
  nand3  g95(.a(new_n146_), .b(new_n144_), .c(new_n123_), .O(z15));
endmodule


