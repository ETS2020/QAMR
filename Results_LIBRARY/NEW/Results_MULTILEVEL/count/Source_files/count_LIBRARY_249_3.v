// Benchmark "FAU" written by ABC on Mon Jul 27 18:27:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g008(.a(x17), .O(new_n60_));
  nor2   g009(.a(x20), .b(x19), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g011(.a(new_n62_), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n59_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n52_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  aoi22  g017(.a(new_n68_), .b(new_n58_), .c(new_n62_), .d(x21), .O(new_n69_));
  inv1   g018(.a(x13), .O(new_n70_));
  aoi21  g019(.a(new_n52_), .b(new_n70_), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n52_), .c(new_n71_), .O(z02));
  nand2  g021(.a(x22), .b(x21), .O(new_n73_));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n61_), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(new_n73_), .c(x17), .O(new_n76_));
  inv1   g025(.a(x22), .O(new_n77_));
  nor2   g026(.a(new_n63_), .b(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n76_), .c(x16), .O(new_n79_));
  inv1   g028(.a(x12), .O(new_n80_));
  aoi21  g029(.a(new_n52_), .b(new_n80_), .c(x18), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n79_), .O(z03));
  nand3  g031(.a(new_n74_), .b(new_n61_), .c(new_n60_), .O(new_n83_));
  nor2   g032(.a(x23), .b(x22), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(x21), .O(new_n86_));
  aoi22  g035(.a(new_n86_), .b(new_n63_), .c(new_n83_), .d(x23), .O(new_n87_));
  inv1   g036(.a(x11), .O(new_n88_));
  aoi21  g037(.a(new_n52_), .b(new_n88_), .c(x18), .O(new_n89_));
  oai21  g038(.a(new_n87_), .b(new_n52_), .c(new_n89_), .O(z04));
  nand3  g039(.a(new_n86_), .b(new_n61_), .c(new_n60_), .O(new_n91_));
  nor2   g040(.a(x24), .b(x23), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n74_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n62_), .O(new_n94_));
  aoi21  g043(.a(new_n91_), .b(x24), .c(new_n94_), .O(new_n95_));
  inv1   g044(.a(x10), .O(new_n96_));
  aoi21  g045(.a(new_n52_), .b(new_n96_), .c(x18), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n52_), .c(new_n97_), .O(z05));
  inv1   g047(.a(new_n93_), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n61_), .c(new_n60_), .O(new_n100_));
  nand2  g049(.a(new_n68_), .b(new_n58_), .O(new_n101_));
  nor2   g050(.a(x25), .b(x24), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n84_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g053(.a(new_n100_), .b(x25), .c(new_n104_), .O(new_n105_));
  inv1   g054(.a(x09), .O(new_n106_));
  aoi21  g055(.a(new_n52_), .b(new_n106_), .c(x18), .O(new_n107_));
  oai21  g056(.a(new_n105_), .b(new_n52_), .c(new_n107_), .O(z06));
  inv1   g057(.a(new_n74_), .O(new_n109_));
  inv1   g058(.a(x23), .O(new_n110_));
  nand2  g059(.a(new_n102_), .b(new_n110_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n61_), .c(new_n60_), .O(new_n113_));
  nor2   g062(.a(x26), .b(x25), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(x24), .O(new_n116_));
  inv1   g065(.a(new_n116_), .O(new_n117_));
  nor3   g066(.a(new_n117_), .b(new_n85_), .c(new_n101_), .O(new_n118_));
  aoi21  g067(.a(new_n113_), .b(x26), .c(new_n118_), .O(new_n119_));
  inv1   g068(.a(x08), .O(new_n120_));
  aoi21  g069(.a(new_n52_), .b(new_n120_), .c(x18), .O(new_n121_));
  oai21  g070(.a(new_n119_), .b(new_n52_), .c(new_n121_), .O(z07));
  nand4  g071(.a(new_n116_), .b(new_n86_), .c(new_n61_), .d(new_n60_), .O(new_n123_));
  nand3  g072(.a(new_n74_), .b(new_n58_), .c(new_n57_), .O(new_n124_));
  inv1   g073(.a(x25), .O(new_n125_));
  nor2   g074(.a(x27), .b(x26), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nor4   g076(.a(new_n127_), .b(new_n124_), .c(x24), .d(x23), .O(new_n128_));
  aoi21  g077(.a(new_n123_), .b(x27), .c(new_n128_), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  aoi21  g079(.a(new_n52_), .b(new_n130_), .c(x18), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n52_), .c(new_n131_), .O(z08));
  nand2  g081(.a(new_n126_), .b(new_n102_), .O(new_n133_));
  inv1   g082(.a(new_n133_), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n86_), .c(new_n61_), .d(new_n60_), .O(new_n135_));
  inv1   g084(.a(x26), .O(new_n136_));
  nor2   g085(.a(x28), .b(x27), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nor3   g087(.a(new_n138_), .b(new_n124_), .c(new_n111_), .O(new_n139_));
  aoi21  g088(.a(new_n135_), .b(x28), .c(new_n139_), .O(new_n140_));
  inv1   g089(.a(x06), .O(new_n141_));
  aoi21  g090(.a(new_n52_), .b(new_n141_), .c(x18), .O(new_n142_));
  oai21  g091(.a(new_n140_), .b(new_n52_), .c(new_n142_), .O(z09));
  nand2  g092(.a(new_n137_), .b(new_n114_), .O(new_n144_));
  inv1   g093(.a(new_n144_), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n99_), .c(new_n61_), .d(new_n60_), .O(new_n146_));
  nand2  g095(.a(new_n86_), .b(new_n63_), .O(new_n147_));
  nor2   g096(.a(x29), .b(x28), .O(new_n148_));
  inv1   g097(.a(new_n148_), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(x27), .O(new_n150_));
  inv1   g099(.a(new_n150_), .O(new_n151_));
  nor3   g100(.a(new_n151_), .b(new_n117_), .c(new_n147_), .O(new_n152_));
  aoi21  g101(.a(new_n146_), .b(x29), .c(new_n152_), .O(new_n153_));
  inv1   g102(.a(x05), .O(new_n154_));
  aoi21  g103(.a(new_n52_), .b(new_n154_), .c(x18), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n52_), .c(new_n155_), .O(z10));
  nor2   g105(.a(new_n151_), .b(new_n115_), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n99_), .c(new_n61_), .d(new_n60_), .O(new_n158_));
  inv1   g107(.a(new_n137_), .O(new_n159_));
  inv1   g108(.a(x29), .O(new_n160_));
  inv1   g109(.a(x30), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  nor4   g111(.a(new_n162_), .b(new_n159_), .c(new_n117_), .d(new_n147_), .O(new_n163_));
  aoi21  g112(.a(new_n158_), .b(x30), .c(new_n163_), .O(new_n164_));
  inv1   g113(.a(x04), .O(new_n165_));
  aoi21  g114(.a(new_n52_), .b(new_n165_), .c(x18), .O(new_n166_));
  oai21  g115(.a(new_n164_), .b(new_n52_), .c(new_n166_), .O(z11));
  inv1   g116(.a(new_n126_), .O(new_n168_));
  nor3   g117(.a(new_n162_), .b(new_n168_), .c(x28), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n112_), .c(new_n61_), .d(new_n60_), .O(new_n170_));
  nor2   g119(.a(x31), .b(x30), .O(new_n171_));
  inv1   g120(.a(new_n171_), .O(new_n172_));
  nor3   g121(.a(new_n172_), .b(new_n149_), .c(new_n127_), .O(new_n173_));
  aoi22  g122(.a(new_n173_), .b(new_n94_), .c(new_n170_), .d(x31), .O(new_n174_));
  inv1   g123(.a(x03), .O(new_n175_));
  aoi21  g124(.a(new_n52_), .b(new_n175_), .c(x18), .O(new_n176_));
  oai21  g125(.a(new_n174_), .b(new_n52_), .c(new_n176_), .O(z12));
  nor3   g126(.a(new_n172_), .b(new_n138_), .c(x29), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n112_), .c(new_n61_), .d(new_n60_), .O(new_n179_));
  nor2   g128(.a(x32), .b(x31), .O(new_n180_));
  inv1   g129(.a(new_n180_), .O(new_n181_));
  nor3   g130(.a(new_n181_), .b(new_n162_), .c(new_n144_), .O(new_n182_));
  aoi22  g131(.a(new_n182_), .b(new_n94_), .c(new_n179_), .d(x32), .O(new_n183_));
  inv1   g132(.a(x02), .O(new_n184_));
  aoi21  g133(.a(new_n52_), .b(new_n184_), .c(x18), .O(new_n185_));
  oai21  g134(.a(new_n183_), .b(new_n52_), .c(new_n185_), .O(z13));
  inv1   g135(.a(x21), .O(new_n187_));
  inv1   g136(.a(x27), .O(new_n188_));
  inv1   g137(.a(x31), .O(new_n189_));
  inv1   g138(.a(x32), .O(new_n190_));
  inv1   g139(.a(x34), .O(new_n191_));
  aoi21  g140(.a(new_n191_), .b(x19), .c(x33), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n190_), .c(new_n189_), .d(new_n161_), .O(new_n193_));
  nor3   g142(.a(new_n193_), .b(x29), .c(x28), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n188_), .c(new_n136_), .d(new_n125_), .O(new_n195_));
  nor2   g144(.a(new_n195_), .b(x24), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n110_), .c(new_n77_), .d(new_n187_), .O(new_n197_));
  nand2  g146(.a(new_n116_), .b(new_n86_), .O(new_n198_));
  nand3  g147(.a(new_n180_), .b(new_n150_), .c(new_n161_), .O(new_n199_));
  oai21  g148(.a(new_n199_), .b(new_n198_), .c(x33), .O(new_n200_));
  oai21  g149(.a(new_n197_), .b(x20), .c(new_n200_), .O(new_n201_));
  inv1   g150(.a(x33), .O(new_n202_));
  nor2   g151(.a(new_n63_), .b(new_n202_), .O(new_n203_));
  aoi21  g152(.a(new_n201_), .b(new_n60_), .c(new_n203_), .O(new_n204_));
  inv1   g153(.a(x01), .O(new_n205_));
  aoi21  g154(.a(new_n52_), .b(new_n205_), .c(x18), .O(new_n206_));
  oai21  g155(.a(new_n204_), .b(new_n52_), .c(new_n206_), .O(z14));
  nand4  g156(.a(new_n102_), .b(new_n84_), .c(new_n68_), .d(new_n60_), .O(new_n208_));
  nand2  g157(.a(new_n148_), .b(new_n126_), .O(new_n209_));
  inv1   g158(.a(new_n209_), .O(new_n210_));
  nand4  g159(.a(new_n210_), .b(new_n171_), .c(new_n202_), .d(new_n190_), .O(new_n211_));
  oai21  g160(.a(new_n211_), .b(new_n208_), .c(x34), .O(new_n212_));
  nor3   g161(.a(x34), .b(x33), .c(x32), .O(new_n213_));
  nand4  g162(.a(new_n213_), .b(new_n210_), .c(new_n171_), .d(new_n104_), .O(new_n214_));
  nand2  g163(.a(new_n214_), .b(new_n212_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(x16), .O(new_n216_));
  inv1   g165(.a(x00), .O(new_n217_));
  aoi21  g166(.a(new_n52_), .b(new_n217_), .c(x18), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n216_), .O(z15));
endmodule


