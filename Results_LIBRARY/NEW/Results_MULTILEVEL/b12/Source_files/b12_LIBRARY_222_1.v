// Benchmark "FAU" written by ABC on Mon Jul 27 17:22:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n74_, new_n75_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(x02), .c(new_n27_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  nand2  g11(.a(new_n28_), .b(x03), .O(new_n36_));
  nand2  g12(.a(x06), .b(x04), .O(new_n37_));
  oai21  g13(.a(new_n36_), .b(new_n25_), .c(new_n37_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n27_), .O(new_n39_));
  inv1   g15(.a(x03), .O(new_n40_));
  nand2  g16(.a(x06), .b(new_n32_), .O(new_n41_));
  inv1   g17(.a(x06), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(x01), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  nand2  g20(.a(new_n42_), .b(x02), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n41_), .c(new_n28_), .O(new_n46_));
  nor2   g22(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n39_), .c(x00), .O(z1));
  inv1   g24(.a(x00), .O(new_n49_));
  nand2  g25(.a(new_n33_), .b(x01), .O(new_n50_));
  inv1   g26(.a(x07), .O(new_n51_));
  inv1   g27(.a(x10), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n36_), .O(new_n54_));
  nor2   g30(.a(x06), .b(new_n28_), .O(new_n55_));
  aoi21  g31(.a(new_n54_), .b(new_n27_), .c(new_n55_), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n50_), .c(new_n25_), .O(new_n57_));
  nand2  g33(.a(new_n53_), .b(new_n40_), .O(new_n58_));
  nand3  g34(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n58_), .c(new_n37_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n27_), .O(new_n61_));
  nand3  g37(.a(x06), .b(new_n32_), .c(x04), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n57_), .c(new_n49_), .O(new_n64_));
  inv1   g40(.a(x08), .O(new_n65_));
  inv1   g41(.a(x09), .O(new_n66_));
  nor2   g42(.a(x10), .b(new_n66_), .O(new_n67_));
  nor2   g43(.a(new_n52_), .b(x03), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(new_n67_), .c(new_n27_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n25_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n65_), .c(new_n51_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n64_), .O(z2));
  aoi22  g48(.a(x12), .b(new_n49_), .c(x11), .d(x07), .O(z3));
  nand3  g49(.a(x09), .b(x08), .c(x00), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n51_), .O(new_n75_));
  oai21  g51(.a(x11), .b(new_n51_), .c(new_n75_), .O(z4));
  nor2   g52(.a(x12), .b(x00), .O(new_n77_));
  nand4  g53(.a(new_n77_), .b(x13), .c(new_n52_), .d(new_n51_), .O(z5));
  nand2  g54(.a(new_n66_), .b(x03), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n25_), .c(new_n49_), .O(new_n80_));
  nand3  g56(.a(x09), .b(new_n40_), .c(new_n27_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n26_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n80_), .c(new_n65_), .O(new_n83_));
  oai21  g59(.a(new_n66_), .b(x03), .c(new_n25_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n27_), .O(new_n85_));
  oai21  g61(.a(x09), .b(new_n40_), .c(new_n27_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n25_), .O(new_n87_));
  inv1   g63(.a(x14), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(x02), .O(new_n89_));
  nand3  g65(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n49_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n83_), .O(new_n92_));
  nand3  g68(.a(new_n92_), .b(new_n52_), .c(new_n51_), .O(new_n93_));
  inv1   g69(.a(new_n93_), .O(z6));
  nand3  g70(.a(x09), .b(x03), .c(new_n27_), .O(new_n95_));
  nand4  g71(.a(new_n52_), .b(new_n65_), .c(new_n51_), .d(x01), .O(new_n96_));
  nand2  g72(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g73(.a(new_n97_), .b(new_n25_), .O(new_n98_));
  oai21  g74(.a(x07), .b(new_n25_), .c(x03), .O(new_n99_));
  aoi21  g75(.a(new_n99_), .b(new_n65_), .c(new_n53_), .O(new_n100_));
  nand2  g76(.a(new_n100_), .b(new_n98_), .O(z7));
  inv1   g77(.a(x12), .O(new_n102_));
  nand4  g78(.a(new_n102_), .b(x03), .c(new_n25_), .d(new_n27_), .O(new_n103_));
  nand2  g79(.a(new_n103_), .b(x09), .O(new_n104_));
  aoi21  g80(.a(new_n66_), .b(new_n49_), .c(x10), .O(new_n105_));
  nand3  g81(.a(new_n105_), .b(new_n104_), .c(new_n51_), .O(z8));
endmodule


