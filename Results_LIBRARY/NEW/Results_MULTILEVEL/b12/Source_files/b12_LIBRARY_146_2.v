// Benchmark "FAU" written by ABC on Mon Jul 27 17:22:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  nand3  g03(.a(new_n27_), .b(x02), .c(new_n25_), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  oai21  g05(.a(new_n29_), .b(new_n26_), .c(x03), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x04), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(x01), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g10(.a(x03), .O(new_n35_));
  aoi21  g11(.a(new_n27_), .b(new_n35_), .c(x01), .O(new_n36_));
  aoi22  g12(.a(x06), .b(x05), .c(new_n27_), .d(new_n35_), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  inv1   g14(.a(x02), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n39_), .c(x01), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n38_), .c(x00), .O(z1));
  inv1   g17(.a(x00), .O(new_n42_));
  nor3   g18(.a(x04), .b(x03), .c(x01), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n35_), .c(new_n25_), .O(new_n47_));
  oai21  g23(.a(new_n43_), .b(new_n39_), .c(new_n47_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n42_), .O(new_n49_));
  nand3  g25(.a(x10), .b(new_n45_), .c(new_n35_), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x09), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n50_), .c(x01), .O(new_n53_));
  nor2   g29(.a(x08), .b(new_n39_), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n53_), .c(new_n44_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n49_), .O(z2));
  inv1   g32(.a(x11), .O(new_n57_));
  inv1   g33(.a(x12), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(x00), .c(new_n57_), .O(new_n59_));
  oai21  g35(.a(x10), .b(new_n45_), .c(x00), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(x12), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n44_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n59_), .O(z3));
  oai21  g39(.a(x02), .b(x00), .c(new_n45_), .O(new_n64_));
  inv1   g40(.a(x09), .O(new_n65_));
  nor2   g41(.a(new_n65_), .b(x01), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(new_n26_), .c(new_n51_), .O(new_n67_));
  aoi21  g43(.a(x10), .b(new_n42_), .c(new_n65_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n67_), .c(new_n64_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n44_), .O(new_n70_));
  nand2  g46(.a(new_n57_), .b(x07), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n70_), .O(z4));
  inv1   g48(.a(x13), .O(new_n73_));
  aoi21  g49(.a(x10), .b(new_n42_), .c(new_n73_), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n60_), .c(new_n58_), .d(new_n44_), .O(z5));
  nand2  g51(.a(x14), .b(x02), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x01), .O(new_n77_));
  oai21  g53(.a(new_n65_), .b(x03), .c(new_n39_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n25_), .O(new_n79_));
  nand2  g55(.a(x02), .b(x00), .O(new_n80_));
  nand3  g56(.a(new_n65_), .b(x03), .c(new_n39_), .O(new_n81_));
  nand4  g57(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(new_n77_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n51_), .c(new_n44_), .O(new_n83_));
  inv1   g59(.a(new_n83_), .O(z6));
  nor2   g60(.a(new_n54_), .b(new_n35_), .O(new_n85_));
  nand4  g61(.a(new_n85_), .b(new_n67_), .c(new_n51_), .d(new_n44_), .O(z7));
  oai21  g62(.a(new_n51_), .b(x07), .c(x09), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(new_n42_), .O(new_n88_));
  inv1   g64(.a(new_n52_), .O(new_n89_));
  nor2   g65(.a(x03), .b(x01), .O(new_n90_));
  aoi22  g66(.a(new_n90_), .b(new_n89_), .c(x10), .d(x00), .O(new_n91_));
  nand3  g67(.a(new_n58_), .b(new_n39_), .c(new_n25_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(x09), .O(new_n93_));
  nand4  g69(.a(new_n93_), .b(new_n91_), .c(new_n88_), .d(new_n44_), .O(z8));
endmodule


