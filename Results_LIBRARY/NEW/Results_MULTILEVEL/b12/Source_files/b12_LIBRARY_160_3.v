// Benchmark "FAU" written by ABC on Mon Jul 27 17:22:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n59_, new_n60_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  aoi21  g02(.a(new_n26_), .b(x04), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x03), .O(new_n29_));
  inv1   g05(.a(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n27_), .c(x02), .O(new_n31_));
  nor2   g07(.a(new_n31_), .b(x00), .O(z0));
  inv1   g08(.a(x00), .O(new_n33_));
  nand3  g09(.a(x06), .b(x05), .c(x01), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n29_), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(x02), .c(new_n33_), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(z1));
  nand2  g14(.a(new_n26_), .b(x04), .O(new_n39_));
  inv1   g15(.a(x07), .O(new_n40_));
  inv1   g16(.a(x10), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n40_), .c(x04), .O(new_n42_));
  nand4  g18(.a(new_n42_), .b(new_n29_), .c(new_n39_), .d(new_n25_), .O(new_n43_));
  inv1   g19(.a(x03), .O(new_n44_));
  oai21  g20(.a(x10), .b(x07), .c(new_n44_), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  nor2   g22(.a(x10), .b(new_n46_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n40_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  aoi22  g25(.a(new_n49_), .b(new_n25_), .c(new_n43_), .d(x02), .O(new_n50_));
  inv1   g26(.a(x08), .O(new_n51_));
  inv1   g27(.a(x02), .O(new_n52_));
  nor2   g28(.a(new_n41_), .b(x03), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n47_), .c(new_n25_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n51_), .c(new_n40_), .O(new_n56_));
  oai21  g32(.a(new_n50_), .b(x00), .c(new_n56_), .O(z2));
  aoi22  g33(.a(x12), .b(new_n33_), .c(x11), .d(x07), .O(z3));
  nand3  g34(.a(x09), .b(x08), .c(x00), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n40_), .O(new_n60_));
  oai21  g36(.a(x11), .b(new_n40_), .c(new_n60_), .O(z4));
  nor2   g37(.a(x12), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(x13), .c(new_n41_), .d(new_n40_), .O(z5));
  oai21  g39(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand3  g40(.a(x09), .b(new_n44_), .c(new_n25_), .O(new_n65_));
  nand2  g41(.a(new_n46_), .b(x03), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n51_), .O(new_n68_));
  nand2  g44(.a(x14), .b(x02), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x01), .O(new_n70_));
  oai21  g46(.a(new_n46_), .b(x03), .c(new_n52_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n25_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n33_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n68_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n41_), .c(new_n40_), .O(new_n76_));
  inv1   g52(.a(new_n76_), .O(z6));
  oai21  g53(.a(x07), .b(new_n52_), .c(x03), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n51_), .O(new_n79_));
  nand3  g55(.a(x09), .b(x03), .c(new_n52_), .O(new_n80_));
  nand4  g56(.a(new_n80_), .b(new_n79_), .c(new_n41_), .d(new_n40_), .O(z7));
  inv1   g57(.a(x12), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(x03), .c(new_n52_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(x09), .O(new_n84_));
  aoi21  g60(.a(new_n46_), .b(new_n33_), .c(x10), .O(new_n85_));
  nand3  g61(.a(new_n85_), .b(new_n84_), .c(new_n40_), .O(z8));
endmodule


