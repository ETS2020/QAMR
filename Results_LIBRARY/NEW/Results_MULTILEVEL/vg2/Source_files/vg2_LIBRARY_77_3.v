// Benchmark "FAU" written by ABC on Mon Jul 27 19:38:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_;
  inv1   g000(.a(x11), .O(new_n34_));
  inv1   g001(.a(x14), .O(new_n35_));
  inv1   g002(.a(x20), .O(new_n36_));
  inv1   g003(.a(x07), .O(new_n37_));
  inv1   g004(.a(x24), .O(new_n38_));
  nor2   g005(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g006(.a(new_n39_), .O(new_n40_));
  nand3  g007(.a(new_n38_), .b(x19), .c(x13), .O(new_n41_));
  oai21  g008(.a(new_n40_), .b(x06), .c(new_n41_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x05), .O(new_n43_));
  inv1   g010(.a(x06), .O(new_n44_));
  nand3  g011(.a(x24), .b(x13), .c(x07), .O(new_n45_));
  inv1   g012(.a(x02), .O(new_n46_));
  inv1   g013(.a(x10), .O(new_n47_));
  nand4  g014(.a(new_n38_), .b(new_n47_), .c(new_n46_), .d(x00), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand2  g016(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  inv1   g017(.a(x05), .O(new_n51_));
  inv1   g018(.a(x13), .O(new_n52_));
  nand3  g019(.a(x19), .b(new_n52_), .c(new_n51_), .O(new_n53_));
  nand3  g020(.a(new_n53_), .b(new_n50_), .c(new_n43_), .O(new_n54_));
  nand4  g021(.a(new_n54_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n55_));
  nor3   g022(.a(new_n55_), .b(x03), .c(x01), .O(z0));
  inv1   g023(.a(x12), .O(new_n57_));
  nand3  g024(.a(x15), .b(x13), .c(x05), .O(new_n58_));
  nand3  g025(.a(x21), .b(new_n47_), .c(new_n46_), .O(new_n59_));
  nand2  g026(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand4  g027(.a(new_n60_), .b(x23), .c(x22), .d(x20), .O(new_n61_));
  inv1   g028(.a(new_n61_), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(x17), .c(x16), .d(x14), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(x11), .c(x09), .d(x08), .O(new_n65_));
  nor2   g032(.a(new_n65_), .b(new_n44_), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(x04), .c(x03), .d(x01), .O(new_n67_));
  inv1   g034(.a(x01), .O(new_n68_));
  inv1   g035(.a(x03), .O(new_n69_));
  inv1   g036(.a(x04), .O(new_n70_));
  inv1   g037(.a(x09), .O(new_n71_));
  inv1   g038(.a(x17), .O(new_n72_));
  inv1   g039(.a(x22), .O(new_n73_));
  nand4  g040(.a(new_n47_), .b(new_n44_), .c(new_n46_), .d(x00), .O(new_n74_));
  nand3  g041(.a(x19), .b(x13), .c(x05), .O(new_n75_));
  aoi21  g042(.a(new_n75_), .b(new_n74_), .c(x23), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n73_), .c(new_n36_), .d(new_n72_), .O(new_n77_));
  nor3   g044(.a(new_n77_), .b(x16), .c(x14), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n57_), .c(new_n34_), .d(new_n71_), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(x08), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n67_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n38_), .O(new_n83_));
  nor2   g050(.a(x13), .b(x05), .O(new_n84_));
  nand4  g051(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n85_));
  nor4   g052(.a(new_n85_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n86_));
  nand4  g053(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  inv1   g055(.a(x18), .O(new_n89_));
  nor2   g056(.a(new_n36_), .b(new_n89_), .O(new_n90_));
  inv1   g057(.a(x23), .O(new_n91_));
  nor2   g058(.a(new_n91_), .b(new_n73_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n90_), .c(new_n88_), .d(new_n86_), .O(new_n93_));
  nor3   g060(.a(x04), .b(x03), .c(x01), .O(new_n94_));
  inv1   g061(.a(x08), .O(new_n95_));
  nand4  g062(.a(new_n71_), .b(new_n95_), .c(x07), .d(new_n44_), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  inv1   g064(.a(x16), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n35_), .c(new_n57_), .d(new_n34_), .O(new_n99_));
  nand4  g066(.a(new_n91_), .b(new_n73_), .c(new_n36_), .d(new_n72_), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g068(.a(new_n101_), .b(new_n97_), .c(new_n94_), .O(new_n102_));
  aoi21  g069(.a(new_n102_), .b(new_n93_), .c(new_n84_), .O(new_n103_));
  nand4  g070(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n104_));
  nand3  g071(.a(new_n92_), .b(x20), .c(x17), .O(new_n105_));
  nor2   g072(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n86_), .O(new_n107_));
  nand4  g074(.a(new_n57_), .b(new_n34_), .c(new_n71_), .d(new_n95_), .O(new_n108_));
  inv1   g075(.a(new_n108_), .O(new_n109_));
  nor3   g076(.a(x17), .b(x16), .c(x14), .O(new_n110_));
  nand4  g077(.a(new_n91_), .b(new_n73_), .c(new_n36_), .d(x19), .O(new_n111_));
  inv1   g078(.a(new_n111_), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(new_n110_), .c(new_n109_), .d(new_n94_), .O(new_n113_));
  aoi21  g080(.a(new_n113_), .b(new_n107_), .c(x13), .O(new_n114_));
  aoi22  g081(.a(new_n114_), .b(new_n51_), .c(new_n103_), .d(x24), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n83_), .O(z1));
  nand2  g083(.a(x24), .b(x18), .O(new_n117_));
  nand3  g084(.a(new_n38_), .b(x15), .c(x13), .O(new_n118_));
  aoi21  g085(.a(new_n118_), .b(new_n117_), .c(new_n51_), .O(new_n119_));
  nand3  g086(.a(x24), .b(x18), .c(x13), .O(new_n120_));
  nand3  g087(.a(x15), .b(new_n52_), .c(new_n51_), .O(new_n121_));
  nand4  g088(.a(new_n38_), .b(x21), .c(new_n47_), .d(new_n46_), .O(new_n122_));
  nand3  g089(.a(new_n122_), .b(new_n121_), .c(new_n120_), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nor3   g091(.a(new_n124_), .b(new_n36_), .c(new_n35_), .O(new_n125_));
  nand4  g092(.a(new_n125_), .b(x11), .c(x06), .d(x03), .O(new_n126_));
  nor2   g093(.a(new_n126_), .b(new_n68_), .O(z2));
  nand4  g094(.a(new_n60_), .b(x20), .c(x14), .d(x11), .O(new_n128_));
  inv1   g095(.a(new_n128_), .O(new_n129_));
  nand4  g096(.a(new_n129_), .b(x08), .c(x06), .d(x03), .O(new_n130_));
  nor2   g097(.a(new_n130_), .b(new_n68_), .O(new_n131_));
  nand2  g098(.a(new_n75_), .b(new_n74_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n133_));
  nor4   g100(.a(new_n133_), .b(x08), .c(x03), .d(x01), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n131_), .c(new_n38_), .O(new_n135_));
  inv1   g102(.a(new_n84_), .O(new_n136_));
  nand4  g103(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n137_));
  nand3  g104(.a(new_n90_), .b(x14), .c(x11), .O(new_n138_));
  nand4  g105(.a(x07), .b(new_n44_), .c(new_n69_), .d(new_n68_), .O(new_n139_));
  nand4  g106(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(new_n95_), .O(new_n140_));
  oai22  g107(.a(new_n140_), .b(new_n139_), .c(new_n138_), .d(new_n137_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n136_), .c(x24), .O(new_n142_));
  nand4  g109(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n143_));
  nand3  g110(.a(new_n95_), .b(new_n69_), .c(new_n68_), .O(new_n144_));
  nand4  g111(.a(new_n36_), .b(x19), .c(new_n35_), .d(new_n34_), .O(new_n145_));
  oai22  g112(.a(new_n145_), .b(new_n144_), .c(new_n143_), .d(new_n137_), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n52_), .c(new_n51_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n142_), .c(new_n135_), .O(z3));
  aoi21  g115(.a(new_n91_), .b(x04), .c(x17), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(x22), .c(new_n71_), .O(new_n150_));
  aoi21  g117(.a(new_n150_), .b(new_n98_), .c(x08), .O(new_n151_));
  aoi21  g118(.a(new_n41_), .b(new_n40_), .c(new_n51_), .O(new_n152_));
  nand3  g119(.a(new_n53_), .b(new_n48_), .c(new_n45_), .O(new_n153_));
  nor2   g120(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  aoi21  g121(.a(x23), .b(new_n70_), .c(new_n72_), .O(new_n155_));
  oai21  g122(.a(new_n155_), .b(new_n73_), .c(x09), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(x16), .c(new_n95_), .O(new_n157_));
  oai22  g124(.a(new_n157_), .b(new_n124_), .c(new_n154_), .d(new_n151_), .O(z4));
  inv1   g125(.a(new_n154_), .O(z5));
  aoi21  g126(.a(x20), .b(new_n35_), .c(x06), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(x11), .c(new_n69_), .O(new_n161_));
  nand2  g128(.a(new_n136_), .b(x24), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(new_n37_), .c(new_n48_), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n161_), .O(new_n164_));
  nand3  g131(.a(new_n38_), .b(x13), .c(x05), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(new_n136_), .O(new_n166_));
  aoi21  g133(.a(new_n36_), .b(x14), .c(new_n44_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(new_n34_), .c(x03), .O(new_n168_));
  nand2  g135(.a(new_n168_), .b(x15), .O(new_n169_));
  nand3  g136(.a(x20), .b(new_n35_), .c(new_n34_), .O(new_n170_));
  nand2  g137(.a(new_n170_), .b(new_n69_), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(x19), .O(new_n172_));
  nand2  g139(.a(new_n172_), .b(new_n169_), .O(new_n173_));
  nand2  g140(.a(new_n173_), .b(new_n166_), .O(new_n174_));
  oai21  g141(.a(new_n162_), .b(new_n89_), .c(new_n122_), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(new_n168_), .O(new_n176_));
  nand3  g143(.a(new_n176_), .b(new_n174_), .c(new_n164_), .O(z6));
  inv1   g144(.a(new_n124_), .O(z7));
endmodule


