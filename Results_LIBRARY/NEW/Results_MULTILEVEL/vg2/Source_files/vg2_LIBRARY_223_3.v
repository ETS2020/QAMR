// Benchmark "FAU" written by ABC on Mon Jul 27 19:38:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x05), .O(new_n37_));
  nand2  g004(.a(x24), .b(x07), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(x19), .c(x13), .O(new_n40_));
  aoi21  g007(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  inv1   g009(.a(x02), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  nand4  g011(.a(new_n39_), .b(new_n44_), .c(new_n43_), .d(x00), .O(new_n45_));
  inv1   g012(.a(x13), .O(new_n46_));
  nand3  g013(.a(x19), .b(new_n46_), .c(new_n37_), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nor3   g016(.a(new_n49_), .b(x20), .c(x14), .O(new_n50_));
  nand4  g017(.a(new_n50_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(x01), .O(z0));
  inv1   g019(.a(x04), .O(new_n53_));
  inv1   g020(.a(x09), .O(new_n54_));
  inv1   g021(.a(x12), .O(new_n55_));
  inv1   g022(.a(x17), .O(new_n56_));
  inv1   g023(.a(x20), .O(new_n57_));
  inv1   g024(.a(x22), .O(new_n58_));
  nand3  g025(.a(new_n44_), .b(new_n43_), .c(x00), .O(new_n59_));
  nand3  g026(.a(x19), .b(x13), .c(x05), .O(new_n60_));
  aoi21  g027(.a(new_n60_), .b(new_n59_), .c(x23), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n62_));
  nor3   g029(.a(new_n62_), .b(x16), .c(x14), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n55_), .c(new_n36_), .d(new_n54_), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(x08), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n35_), .c(new_n53_), .d(new_n34_), .O(new_n66_));
  nand4  g033(.a(x04), .b(x03), .c(new_n43_), .d(x01), .O(new_n67_));
  inv1   g034(.a(x08), .O(new_n68_));
  nor2   g035(.a(new_n68_), .b(new_n35_), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n44_), .c(x09), .O(new_n70_));
  nor2   g037(.a(new_n70_), .b(new_n67_), .O(new_n71_));
  nand4  g038(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n72_));
  inv1   g039(.a(new_n72_), .O(new_n73_));
  nand3  g040(.a(x23), .b(x22), .c(x21), .O(new_n74_));
  nor3   g041(.a(new_n74_), .b(new_n57_), .c(new_n56_), .O(new_n75_));
  nand3  g042(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(new_n76_));
  oai21  g043(.a(new_n66_), .b(x01), .c(new_n76_), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n39_), .O(new_n78_));
  nor2   g045(.a(x13), .b(x05), .O(new_n79_));
  nand3  g046(.a(x04), .b(x03), .c(x01), .O(new_n80_));
  inv1   g047(.a(new_n80_), .O(new_n81_));
  nor2   g048(.a(new_n36_), .b(new_n54_), .O(new_n82_));
  nand4  g049(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n83_));
  nand4  g050(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n82_), .c(new_n81_), .d(new_n69_), .O(new_n86_));
  nor3   g053(.a(x04), .b(x03), .c(x01), .O(new_n87_));
  nand4  g054(.a(new_n54_), .b(new_n68_), .c(x07), .d(new_n35_), .O(new_n88_));
  inv1   g055(.a(new_n88_), .O(new_n89_));
  inv1   g056(.a(x14), .O(new_n90_));
  inv1   g057(.a(x16), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n90_), .c(new_n55_), .d(new_n36_), .O(new_n92_));
  nor2   g059(.a(x23), .b(x22), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n57_), .c(new_n56_), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(new_n89_), .c(new_n87_), .O(new_n96_));
  aoi21  g063(.a(new_n96_), .b(new_n86_), .c(new_n79_), .O(new_n97_));
  nor2   g064(.a(x03), .b(x01), .O(new_n98_));
  nor2   g065(.a(x05), .b(x04), .O(new_n99_));
  nor2   g066(.a(x08), .b(x06), .O(new_n100_));
  nor2   g067(.a(x11), .b(x09), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n100_), .c(new_n99_), .d(new_n98_), .O(new_n102_));
  nand4  g069(.a(new_n91_), .b(new_n90_), .c(new_n46_), .d(new_n55_), .O(new_n103_));
  nand4  g070(.a(new_n93_), .b(new_n57_), .c(x19), .d(new_n56_), .O(new_n104_));
  nor3   g071(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  aoi21  g072(.a(new_n97_), .b(x24), .c(new_n105_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n78_), .O(z1));
  inv1   g074(.a(x01), .O(new_n108_));
  nand2  g075(.a(x24), .b(x18), .O(new_n109_));
  nand3  g076(.a(new_n39_), .b(x15), .c(x13), .O(new_n110_));
  aoi21  g077(.a(new_n110_), .b(new_n109_), .c(new_n37_), .O(new_n111_));
  nand3  g078(.a(x24), .b(x18), .c(x13), .O(new_n112_));
  nand3  g079(.a(x15), .b(new_n46_), .c(new_n37_), .O(new_n113_));
  nand4  g080(.a(new_n39_), .b(x21), .c(new_n44_), .d(new_n43_), .O(new_n114_));
  nand3  g081(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(new_n115_));
  nor2   g082(.a(new_n115_), .b(new_n111_), .O(new_n116_));
  nor3   g083(.a(new_n116_), .b(new_n57_), .c(new_n90_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(x11), .c(x06), .d(x03), .O(new_n118_));
  nor2   g085(.a(new_n118_), .b(new_n108_), .O(z2));
  nand3  g086(.a(x15), .b(x13), .c(x05), .O(new_n120_));
  nand3  g087(.a(x21), .b(new_n44_), .c(new_n43_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(x20), .c(x14), .d(x11), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n68_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(x06), .c(x03), .d(x01), .O(new_n125_));
  nand2  g092(.a(new_n60_), .b(new_n59_), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(new_n57_), .c(new_n90_), .d(new_n36_), .O(new_n127_));
  nor2   g094(.a(new_n127_), .b(x08), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n35_), .c(new_n34_), .d(new_n108_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n39_), .O(new_n131_));
  inv1   g098(.a(new_n79_), .O(new_n132_));
  nand3  g099(.a(new_n69_), .b(x03), .c(x01), .O(new_n133_));
  nand4  g100(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n134_));
  nand3  g101(.a(new_n98_), .b(x07), .c(new_n35_), .O(new_n135_));
  nand4  g102(.a(new_n57_), .b(new_n90_), .c(new_n36_), .d(new_n68_), .O(new_n136_));
  oai22  g103(.a(new_n136_), .b(new_n135_), .c(new_n134_), .d(new_n133_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n132_), .c(x24), .O(new_n138_));
  nand4  g105(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n139_));
  nand2  g106(.a(new_n100_), .b(new_n98_), .O(new_n140_));
  nand4  g107(.a(new_n57_), .b(x19), .c(new_n90_), .d(new_n36_), .O(new_n141_));
  oai22  g108(.a(new_n141_), .b(new_n140_), .c(new_n139_), .d(new_n133_), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n46_), .c(new_n37_), .O(new_n143_));
  nand3  g110(.a(new_n143_), .b(new_n138_), .c(new_n131_), .O(z3));
  inv1   g111(.a(x07), .O(new_n145_));
  nand2  g112(.a(x23), .b(new_n53_), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(x17), .c(new_n58_), .O(new_n147_));
  inv1   g114(.a(new_n147_), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(x09), .c(new_n91_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(new_n68_), .c(x18), .O(new_n150_));
  inv1   g117(.a(x23), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(x04), .c(x17), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(x22), .c(new_n54_), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(new_n91_), .c(x08), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(new_n145_), .c(new_n150_), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n132_), .c(x24), .O(new_n156_));
  inv1   g123(.a(new_n126_), .O(new_n157_));
  nor2   g124(.a(new_n154_), .b(new_n157_), .O(new_n158_));
  aoi21  g125(.a(x16), .b(new_n54_), .c(new_n68_), .O(new_n159_));
  inv1   g126(.a(new_n159_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n122_), .O(new_n161_));
  nand4  g128(.a(new_n147_), .b(x21), .c(x16), .d(new_n44_), .O(new_n162_));
  oai21  g129(.a(new_n162_), .b(x02), .c(new_n161_), .O(new_n163_));
  oai21  g130(.a(new_n163_), .b(new_n158_), .c(new_n39_), .O(new_n164_));
  inv1   g131(.a(x19), .O(new_n165_));
  nand2  g132(.a(new_n160_), .b(x15), .O(new_n166_));
  oai21  g133(.a(new_n154_), .b(new_n165_), .c(new_n166_), .O(new_n167_));
  nand3  g134(.a(new_n167_), .b(new_n46_), .c(new_n37_), .O(new_n168_));
  nand3  g135(.a(new_n168_), .b(new_n164_), .c(new_n156_), .O(z4));
  inv1   g136(.a(new_n49_), .O(z5));
  nand2  g137(.a(x20), .b(new_n90_), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n35_), .O(new_n172_));
  aoi21  g139(.a(new_n172_), .b(new_n36_), .c(x03), .O(new_n173_));
  nand2  g140(.a(new_n57_), .b(x14), .O(new_n174_));
  nand2  g141(.a(new_n174_), .b(x06), .O(new_n175_));
  aoi21  g142(.a(new_n175_), .b(x11), .c(new_n34_), .O(new_n176_));
  oai22  g143(.a(new_n176_), .b(new_n116_), .c(new_n173_), .d(new_n49_), .O(z6));
  inv1   g144(.a(new_n116_), .O(z7));
endmodule


