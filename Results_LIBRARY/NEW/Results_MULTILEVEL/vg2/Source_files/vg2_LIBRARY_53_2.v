// Benchmark "FAU" written by ABC on Mon Jul 27 19:38:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n177_, new_n178_, new_n179_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x07), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(x19), .c(x13), .O(new_n40_));
  oai21  g007(.a(new_n39_), .b(new_n38_), .c(new_n40_), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x05), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n39_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  inv1   g013(.a(x05), .O(new_n47_));
  inv1   g014(.a(x13), .O(new_n48_));
  nand3  g015(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n46_), .c(new_n43_), .d(new_n42_), .O(z5));
  nand2  g017(.a(z5), .b(new_n37_), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(x14), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(x01), .O(z0));
  inv1   g021(.a(x04), .O(new_n55_));
  inv1   g022(.a(x09), .O(new_n56_));
  inv1   g023(.a(x12), .O(new_n57_));
  inv1   g024(.a(x17), .O(new_n58_));
  inv1   g025(.a(x22), .O(new_n59_));
  nand3  g026(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n60_));
  nand3  g027(.a(x19), .b(x13), .c(x05), .O(new_n61_));
  aoi21  g028(.a(new_n61_), .b(new_n60_), .c(x23), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(new_n59_), .c(new_n37_), .d(new_n58_), .O(new_n63_));
  nor3   g030(.a(new_n63_), .b(x16), .c(x14), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n57_), .c(new_n36_), .d(new_n56_), .O(new_n65_));
  nor2   g032(.a(new_n65_), .b(x08), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n35_), .c(new_n55_), .d(new_n34_), .O(new_n67_));
  inv1   g034(.a(x01), .O(new_n68_));
  nor2   g035(.a(x02), .b(new_n68_), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(x04), .c(x03), .O(new_n70_));
  inv1   g037(.a(x08), .O(new_n71_));
  nor2   g038(.a(new_n71_), .b(new_n35_), .O(new_n72_));
  nand3  g039(.a(new_n72_), .b(new_n45_), .c(x09), .O(new_n73_));
  nor2   g040(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  nor2   g041(.a(new_n57_), .b(new_n36_), .O(new_n75_));
  inv1   g042(.a(x14), .O(new_n76_));
  inv1   g043(.a(x16), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  inv1   g045(.a(x23), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n59_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(x21), .O(new_n81_));
  nor3   g048(.a(new_n81_), .b(new_n37_), .c(new_n58_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n78_), .c(new_n75_), .d(new_n74_), .O(new_n83_));
  oai21  g050(.a(new_n67_), .b(x01), .c(new_n83_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n39_), .O(new_n85_));
  nand3  g052(.a(new_n72_), .b(x11), .c(x09), .O(new_n86_));
  nor4   g053(.a(new_n86_), .b(new_n55_), .c(new_n34_), .d(new_n68_), .O(new_n87_));
  nand4  g054(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n88_));
  nand3  g055(.a(new_n80_), .b(x20), .c(x17), .O(new_n89_));
  nor2   g056(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nor3   g058(.a(x04), .b(x03), .c(x01), .O(new_n92_));
  nor2   g059(.a(x08), .b(x06), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n92_), .c(new_n36_), .d(new_n56_), .O(new_n94_));
  nor2   g061(.a(x14), .b(x12), .O(new_n95_));
  nor2   g062(.a(x17), .b(x16), .O(new_n96_));
  inv1   g063(.a(x19), .O(new_n97_));
  nor2   g064(.a(x20), .b(new_n97_), .O(new_n98_));
  nor2   g065(.a(x23), .b(x22), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n98_), .c(new_n96_), .d(new_n95_), .O(new_n100_));
  oai21  g067(.a(new_n100_), .b(new_n94_), .c(new_n91_), .O(new_n101_));
  nand3  g068(.a(new_n101_), .b(new_n48_), .c(new_n47_), .O(new_n102_));
  nand2  g069(.a(new_n48_), .b(new_n47_), .O(new_n103_));
  inv1   g070(.a(new_n89_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n87_), .c(new_n78_), .d(x12), .O(new_n105_));
  nor2   g072(.a(new_n38_), .b(x06), .O(new_n106_));
  nand4  g073(.a(new_n106_), .b(new_n92_), .c(new_n56_), .d(new_n71_), .O(new_n107_));
  nand4  g074(.a(new_n77_), .b(new_n76_), .c(new_n57_), .d(new_n36_), .O(new_n108_));
  inv1   g075(.a(new_n108_), .O(new_n109_));
  nand4  g076(.a(new_n109_), .b(new_n99_), .c(new_n37_), .d(new_n58_), .O(new_n110_));
  oai21  g077(.a(new_n110_), .b(new_n107_), .c(new_n105_), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n103_), .c(x24), .O(new_n112_));
  nor2   g079(.a(new_n34_), .b(new_n68_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(x05), .c(x04), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n86_), .O(new_n115_));
  nand4  g082(.a(x15), .b(x14), .c(x13), .d(x12), .O(new_n116_));
  inv1   g083(.a(new_n116_), .O(new_n117_));
  nand2  g084(.a(new_n80_), .b(x20), .O(new_n118_));
  nor3   g085(.a(new_n118_), .b(new_n58_), .c(new_n77_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(new_n117_), .c(new_n115_), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(new_n112_), .c(new_n102_), .O(new_n121_));
  inv1   g088(.a(new_n121_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n85_), .O(z1));
  nand2  g090(.a(x24), .b(x13), .O(new_n124_));
  nand2  g091(.a(x15), .b(x13), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n39_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(x05), .O(new_n127_));
  nand3  g094(.a(x15), .b(new_n48_), .c(new_n47_), .O(new_n128_));
  nand4  g095(.a(new_n39_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  inv1   g097(.a(new_n130_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n127_), .c(new_n124_), .O(new_n132_));
  nand4  g099(.a(new_n132_), .b(x20), .c(x14), .d(x11), .O(new_n133_));
  inv1   g100(.a(new_n133_), .O(new_n134_));
  nand4  g101(.a(new_n134_), .b(x06), .c(x03), .d(x01), .O(new_n135_));
  inv1   g102(.a(new_n135_), .O(z2));
  nand2  g103(.a(new_n61_), .b(new_n60_), .O(new_n137_));
  nand4  g104(.a(new_n137_), .b(new_n37_), .c(new_n76_), .d(new_n36_), .O(new_n138_));
  nor2   g105(.a(new_n138_), .b(x08), .O(new_n139_));
  nand4  g106(.a(new_n139_), .b(new_n35_), .c(new_n34_), .d(new_n68_), .O(new_n140_));
  nand3  g107(.a(x21), .b(x20), .c(x14), .O(new_n141_));
  nor3   g108(.a(new_n141_), .b(new_n36_), .c(x10), .O(new_n142_));
  nand4  g109(.a(new_n142_), .b(new_n72_), .c(new_n69_), .d(x03), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n39_), .O(new_n145_));
  nand2  g112(.a(new_n113_), .b(new_n72_), .O(new_n146_));
  nand4  g113(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n147_));
  nand3  g114(.a(new_n93_), .b(new_n34_), .c(new_n68_), .O(new_n148_));
  nand3  g115(.a(new_n98_), .b(new_n76_), .c(new_n36_), .O(new_n149_));
  oai22  g116(.a(new_n149_), .b(new_n148_), .c(new_n147_), .d(new_n146_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n48_), .c(new_n47_), .O(new_n151_));
  nand3  g118(.a(x06), .b(x03), .c(x01), .O(new_n152_));
  nand4  g119(.a(x20), .b(x14), .c(x11), .d(x08), .O(new_n153_));
  nand3  g120(.a(new_n106_), .b(new_n34_), .c(new_n68_), .O(new_n154_));
  nand4  g121(.a(new_n37_), .b(new_n76_), .c(new_n36_), .d(new_n71_), .O(new_n155_));
  oai22  g122(.a(new_n155_), .b(new_n154_), .c(new_n153_), .d(new_n152_), .O(new_n156_));
  nand3  g123(.a(new_n156_), .b(new_n103_), .c(x24), .O(new_n157_));
  nand3  g124(.a(x20), .b(x15), .c(x14), .O(new_n158_));
  nor3   g125(.a(new_n158_), .b(new_n48_), .c(new_n36_), .O(new_n159_));
  nand4  g126(.a(new_n159_), .b(new_n113_), .c(new_n72_), .d(x05), .O(new_n160_));
  nand4  g127(.a(new_n160_), .b(new_n157_), .c(new_n151_), .d(new_n145_), .O(z3));
  oai21  g128(.a(x23), .b(new_n55_), .c(new_n58_), .O(new_n162_));
  aoi21  g129(.a(new_n162_), .b(new_n59_), .c(x09), .O(new_n163_));
  oai21  g130(.a(new_n163_), .b(x16), .c(new_n71_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(z5), .O(new_n165_));
  oai21  g132(.a(new_n79_), .b(x04), .c(x17), .O(new_n166_));
  aoi21  g133(.a(new_n166_), .b(x22), .c(new_n56_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(new_n77_), .c(x08), .O(new_n168_));
  inv1   g135(.a(x18), .O(new_n169_));
  nand3  g136(.a(new_n39_), .b(x15), .c(x13), .O(new_n170_));
  oai21  g137(.a(new_n39_), .b(new_n169_), .c(new_n170_), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(x05), .O(new_n172_));
  nand3  g139(.a(x24), .b(x18), .c(x13), .O(new_n173_));
  nand3  g140(.a(new_n173_), .b(new_n172_), .c(new_n131_), .O(z7));
  nand2  g141(.a(z7), .b(new_n168_), .O(new_n175_));
  nand2  g142(.a(new_n175_), .b(new_n165_), .O(z4));
  inv1   g143(.a(new_n46_), .O(new_n177_));
  nand2  g144(.a(x20), .b(new_n76_), .O(new_n178_));
  aoi21  g145(.a(new_n178_), .b(new_n35_), .c(x11), .O(new_n179_));
  nand3  g146(.a(new_n39_), .b(x13), .c(x05), .O(new_n180_));
  aoi21  g147(.a(new_n180_), .b(new_n103_), .c(new_n97_), .O(new_n181_));
  oai22  g148(.a(new_n181_), .b(new_n177_), .c(new_n179_), .d(x03), .O(new_n182_));
  nor2   g149(.a(new_n169_), .b(x06), .O(new_n183_));
  nor2   g150(.a(x20), .b(new_n76_), .O(new_n184_));
  oai21  g151(.a(new_n184_), .b(new_n183_), .c(x11), .O(new_n185_));
  nor2   g152(.a(x20), .b(x06), .O(new_n186_));
  oai21  g153(.a(new_n186_), .b(x11), .c(new_n34_), .O(new_n187_));
  nand2  g154(.a(new_n187_), .b(x07), .O(new_n188_));
  nand2  g155(.a(x18), .b(new_n34_), .O(new_n189_));
  nand3  g156(.a(new_n189_), .b(new_n188_), .c(new_n185_), .O(new_n190_));
  nand3  g157(.a(new_n190_), .b(new_n103_), .c(x24), .O(new_n191_));
  inv1   g158(.a(new_n184_), .O(new_n192_));
  aoi21  g159(.a(new_n192_), .b(x06), .c(new_n36_), .O(new_n193_));
  oai21  g160(.a(new_n193_), .b(new_n34_), .c(new_n130_), .O(new_n194_));
  aoi21  g161(.a(new_n39_), .b(new_n35_), .c(new_n184_), .O(new_n195_));
  oai22  g162(.a(new_n195_), .b(new_n36_), .c(x24), .d(x03), .O(new_n196_));
  nand4  g163(.a(new_n196_), .b(x15), .c(x13), .d(x05), .O(new_n197_));
  nand4  g164(.a(new_n197_), .b(new_n194_), .c(new_n191_), .d(new_n182_), .O(z6));
endmodule


