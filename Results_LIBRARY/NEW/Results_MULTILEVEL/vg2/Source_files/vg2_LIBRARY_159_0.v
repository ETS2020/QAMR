// Benchmark "FAU" written by ABC on Mon Jul 27 19:38:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x03), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x11), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  inv1   g010(.a(x13), .O(new_n44_));
  nand3  g011(.a(x19), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  oai21  g013(.a(new_n46_), .b(new_n42_), .c(new_n37_), .O(new_n47_));
  nor2   g014(.a(new_n47_), .b(x14), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(x01), .O(z0));
  inv1   g017(.a(x01), .O(new_n51_));
  inv1   g018(.a(x04), .O(new_n52_));
  nand4  g019(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n53_));
  nor4   g020(.a(new_n53_), .b(new_n52_), .c(new_n34_), .d(new_n51_), .O(new_n54_));
  nand4  g021(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n55_));
  nand4  g022(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g024(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  inv1   g025(.a(x09), .O(new_n59_));
  nor3   g026(.a(x04), .b(x03), .c(x01), .O(new_n60_));
  nor2   g027(.a(x08), .b(x06), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n60_), .c(new_n36_), .d(new_n59_), .O(new_n62_));
  nor2   g029(.a(x14), .b(x12), .O(new_n63_));
  nor2   g030(.a(x17), .b(x16), .O(new_n64_));
  inv1   g031(.a(x19), .O(new_n65_));
  nor2   g032(.a(x20), .b(new_n65_), .O(new_n66_));
  nor2   g033(.a(x23), .b(x22), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n66_), .c(new_n64_), .d(new_n63_), .O(new_n68_));
  oai21  g035(.a(new_n68_), .b(new_n62_), .c(new_n58_), .O(new_n69_));
  nor2   g036(.a(x13), .b(x05), .O(new_n70_));
  inv1   g037(.a(new_n70_), .O(new_n71_));
  nand3  g038(.a(new_n40_), .b(x13), .c(x05), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  nand4  g041(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n75_));
  nand4  g042(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n54_), .O(new_n78_));
  nor2   g045(.a(x09), .b(x08), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n60_), .c(x07), .d(new_n35_), .O(new_n80_));
  nor2   g047(.a(x12), .b(x11), .O(new_n81_));
  nor2   g048(.a(x16), .b(x14), .O(new_n82_));
  nor2   g049(.a(x20), .b(x17), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n82_), .c(new_n81_), .d(new_n67_), .O(new_n84_));
  oai21  g051(.a(new_n84_), .b(new_n80_), .c(new_n78_), .O(new_n85_));
  nand3  g052(.a(new_n85_), .b(new_n71_), .c(x24), .O(new_n86_));
  inv1   g053(.a(new_n75_), .O(new_n87_));
  nor2   g054(.a(x02), .b(new_n51_), .O(new_n88_));
  nand3  g055(.a(new_n88_), .b(x04), .c(x03), .O(new_n89_));
  inv1   g056(.a(x10), .O(new_n90_));
  nand4  g057(.a(x11), .b(new_n90_), .c(x09), .d(x08), .O(new_n91_));
  nor2   g058(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  inv1   g059(.a(x22), .O(new_n93_));
  inv1   g060(.a(x23), .O(new_n94_));
  nand2  g061(.a(x21), .b(x20), .O(new_n95_));
  nor4   g062(.a(new_n95_), .b(x24), .c(new_n94_), .d(new_n93_), .O(new_n96_));
  nand3  g063(.a(new_n96_), .b(new_n92_), .c(new_n87_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n86_), .c(new_n74_), .O(z1));
  nand2  g065(.a(x24), .b(x18), .O(new_n99_));
  nand3  g066(.a(new_n40_), .b(x15), .c(x13), .O(new_n100_));
  aoi21  g067(.a(new_n100_), .b(new_n99_), .c(new_n38_), .O(new_n101_));
  nand3  g068(.a(x24), .b(x18), .c(x13), .O(new_n102_));
  nand3  g069(.a(x15), .b(new_n44_), .c(new_n38_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n101_), .c(x06), .O(new_n105_));
  inv1   g072(.a(x02), .O(new_n106_));
  nand4  g073(.a(new_n40_), .b(x21), .c(new_n90_), .d(new_n106_), .O(new_n107_));
  aoi21  g074(.a(new_n107_), .b(new_n105_), .c(new_n37_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(x14), .c(x11), .d(x03), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n51_), .O(z2));
  nand4  g077(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n111_));
  inv1   g078(.a(x14), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n36_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(x20), .c(x15), .O(new_n114_));
  nand3  g081(.a(new_n61_), .b(new_n34_), .c(new_n51_), .O(new_n115_));
  nand3  g082(.a(new_n66_), .b(new_n112_), .c(new_n36_), .O(new_n116_));
  oai22  g083(.a(new_n116_), .b(new_n115_), .c(new_n114_), .d(new_n111_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n73_), .O(new_n118_));
  nand3  g085(.a(new_n113_), .b(x20), .c(x18), .O(new_n119_));
  nand4  g086(.a(x07), .b(new_n35_), .c(new_n34_), .d(new_n51_), .O(new_n120_));
  inv1   g087(.a(x08), .O(new_n121_));
  nand4  g088(.a(new_n37_), .b(new_n112_), .c(new_n36_), .d(new_n121_), .O(new_n122_));
  oai22  g089(.a(new_n122_), .b(new_n120_), .c(new_n119_), .d(new_n111_), .O(new_n123_));
  nand3  g090(.a(new_n123_), .b(new_n71_), .c(x24), .O(new_n124_));
  nand3  g091(.a(new_n90_), .b(x08), .c(x03), .O(new_n125_));
  inv1   g092(.a(new_n125_), .O(new_n126_));
  nand3  g093(.a(new_n40_), .b(x21), .c(x20), .O(new_n127_));
  inv1   g094(.a(new_n127_), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n126_), .c(new_n113_), .d(new_n88_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(new_n124_), .c(new_n118_), .O(z3));
  inv1   g097(.a(x16), .O(new_n131_));
  aoi21  g098(.a(new_n94_), .b(x04), .c(x17), .O(new_n132_));
  oai21  g099(.a(new_n132_), .b(x22), .c(new_n59_), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n131_), .c(x08), .O(new_n134_));
  nand4  g101(.a(new_n40_), .b(new_n90_), .c(new_n106_), .d(x00), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n45_), .c(new_n43_), .O(new_n136_));
  nor2   g103(.a(new_n136_), .b(new_n42_), .O(new_n137_));
  inv1   g104(.a(x17), .O(new_n138_));
  aoi21  g105(.a(x23), .b(new_n52_), .c(new_n138_), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(new_n93_), .c(x09), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(x16), .c(new_n121_), .O(new_n141_));
  nand3  g108(.a(new_n107_), .b(new_n103_), .c(new_n102_), .O(new_n142_));
  nor2   g109(.a(new_n142_), .b(new_n101_), .O(new_n143_));
  oai22  g110(.a(new_n143_), .b(new_n141_), .c(new_n137_), .d(new_n134_), .O(z4));
  inv1   g111(.a(new_n137_), .O(z5));
  oai21  g112(.a(new_n36_), .b(x03), .c(x00), .O(new_n146_));
  nand3  g113(.a(new_n37_), .b(x14), .c(x11), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(x03), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(x21), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n146_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n90_), .c(new_n106_), .O(new_n151_));
  inv1   g118(.a(x15), .O(new_n152_));
  oai21  g119(.a(x20), .b(new_n112_), .c(x06), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(x11), .c(new_n34_), .O(new_n154_));
  oai21  g121(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n155_));
  aoi21  g122(.a(new_n155_), .b(new_n36_), .c(x03), .O(new_n156_));
  oai22  g123(.a(new_n156_), .b(new_n65_), .c(new_n154_), .d(new_n152_), .O(new_n157_));
  nand3  g124(.a(new_n157_), .b(x13), .c(x05), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n151_), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n40_), .O(new_n160_));
  nand2  g127(.a(new_n153_), .b(x11), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(x03), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(x18), .O(new_n163_));
  nand2  g130(.a(new_n155_), .b(new_n36_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(new_n34_), .O(new_n165_));
  nand2  g132(.a(new_n165_), .b(x07), .O(new_n166_));
  aoi21  g133(.a(new_n166_), .b(new_n163_), .c(new_n70_), .O(new_n167_));
  nand2  g134(.a(new_n162_), .b(x15), .O(new_n168_));
  nand2  g135(.a(new_n165_), .b(x19), .O(new_n169_));
  aoi21  g136(.a(new_n169_), .b(new_n168_), .c(x13), .O(new_n170_));
  aoi22  g137(.a(new_n170_), .b(new_n38_), .c(new_n167_), .d(x24), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n160_), .O(z6));
  inv1   g139(.a(new_n143_), .O(z7));
endmodule


