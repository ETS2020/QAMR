// Benchmark "FAU" written by ABC on Mon Jul 27 19:29:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  oai21  g02(.a(x14), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(x05), .b(new_n40_), .O(new_n41_));
  oai21  g07(.a(new_n39_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g08(.a(x16), .O(new_n43_));
  nand2  g09(.a(x07), .b(x03), .O(new_n44_));
  aoi21  g10(.a(new_n44_), .b(x02), .c(new_n43_), .O(new_n45_));
  nor3   g11(.a(new_n45_), .b(x06), .c(new_n40_), .O(z02));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x07), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(x06), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  inv1   g17(.a(x06), .O(new_n52_));
  nor2   g18(.a(x07), .b(new_n52_), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n51_), .c(x02), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n49_), .c(new_n43_), .O(new_n55_));
  aoi21  g21(.a(new_n55_), .b(new_n54_), .c(new_n40_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n57_), .b(x06), .c(new_n58_), .O(new_n59_));
  oai21  g25(.a(x03), .b(new_n35_), .c(x16), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g27(.a(x16), .b(new_n35_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x08), .c(new_n48_), .O(new_n63_));
  aoi21  g29(.a(new_n63_), .b(new_n61_), .c(new_n40_), .O(z04));
  oai21  g30(.a(new_n40_), .b(x02), .c(new_n47_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x16), .O(new_n66_));
  nand3  g32(.a(x08), .b(x07), .c(x06), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n68_), .c(new_n66_), .d(x04), .O(z05));
  nand2  g37(.a(new_n70_), .b(x10), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(x08), .c(x07), .d(x06), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n72_), .c(new_n66_), .d(x04), .O(z06));
  nand2  g41(.a(new_n74_), .b(x11), .O(new_n76_));
  inv1   g42(.a(x10), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  inv1   g44(.a(new_n67_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(new_n77_), .d(new_n69_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n76_), .c(new_n66_), .d(x04), .O(z07));
  nand2  g47(.a(new_n80_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand4  g49(.a(new_n73_), .b(new_n79_), .c(new_n83_), .d(new_n78_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n82_), .c(new_n66_), .d(x04), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  nand4  g52(.a(new_n60_), .b(new_n86_), .c(new_n83_), .d(new_n78_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x10), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n69_), .c(x08), .d(x07), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n52_), .c(new_n62_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x04), .O(new_n91_));
  oai21  g57(.a(new_n43_), .b(new_n47_), .c(x04), .O(new_n92_));
  aoi21  g58(.a(new_n84_), .b(x13), .c(new_n92_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n91_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand4  g61(.a(new_n86_), .b(new_n83_), .c(new_n78_), .d(new_n77_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n60_), .c(new_n69_), .d(x08), .O(new_n98_));
  inv1   g64(.a(new_n98_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(x07), .c(x06), .O(new_n100_));
  inv1   g66(.a(new_n60_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x14), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n100_), .c(new_n40_), .O(z10));
  aoi21  g69(.a(new_n62_), .b(new_n47_), .c(new_n40_), .O(z12));
  and2   g70(.a(x17), .b(x04), .O(z14));
  buf1   g71(.a(x02), .O(z11));
  buf1   g72(.a(x04), .O(z13));
endmodule


