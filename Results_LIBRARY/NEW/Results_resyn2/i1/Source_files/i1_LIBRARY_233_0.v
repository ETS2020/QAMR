// Benchmark "FAU" written by ABC on Mon Jul 27 18:18:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n58_,
    new_n63_, new_n64_, new_n66_, new_n68_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x19), .O(new_n42_));
  inv1   g01(.a(x07), .O(new_n43_));
  nor2   g02(.a(x06), .b(x05), .O(new_n44_));
  nor2   g03(.a(x02), .b(x01), .O(new_n45_));
  nor2   g04(.a(x04), .b(x03), .O(new_n46_));
  nand4  g05(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n47_));
  xor2a  g06(.a(x09), .b(x08), .O(new_n48_));
  oai22  g07(.a(new_n48_), .b(new_n47_), .c(new_n42_), .d(x00), .O(z01));
  nor3   g08(.a(x07), .b(x06), .c(x05), .O(new_n50_));
  inv1   g09(.a(x09), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x08), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n46_), .c(new_n45_), .d(new_n50_), .O(new_n53_));
  nand2  g12(.a(x19), .b(x00), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  and2   g14(.a(new_n55_), .b(new_n53_), .O(z02));
  or2    g15(.a(x21), .b(x20), .O(z04));
  inv1   g16(.a(x10), .O(new_n58_));
  oai21  g17(.a(new_n47_), .b(x08), .c(new_n58_), .O(z05));
  nand2  g18(.a(new_n47_), .b(new_n42_), .O(z06));
  and2   g19(.a(x24), .b(x18), .O(z07));
  and2   g20(.a(x24), .b(x11), .O(z09));
  inv1   g21(.a(x14), .O(new_n63_));
  inv1   g22(.a(x22), .O(new_n64_));
  nor3   g23(.a(x24), .b(new_n64_), .c(new_n63_), .O(z10));
  inv1   g24(.a(x17), .O(new_n66_));
  nor3   g25(.a(x24), .b(new_n64_), .c(new_n66_), .O(z11));
  inv1   g26(.a(x23), .O(new_n68_));
  nor3   g27(.a(x24), .b(new_n68_), .c(new_n63_), .O(z12));
  nor3   g28(.a(x24), .b(new_n68_), .c(new_n66_), .O(z13));
  inv1   g29(.a(x16), .O(new_n71_));
  nor2   g30(.a(x24), .b(new_n71_), .O(z14));
  nor2   g31(.a(x13), .b(x12), .O(new_n73_));
  nor2   g32(.a(x15), .b(x14), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n73_), .O(z15));
  buf    g34(.a(x00), .O(z00));
  buf    g35(.a(x20), .O(z03));
  buf    g36(.a(x11), .O(z08));
endmodule


