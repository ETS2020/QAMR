// Benchmark "FAU" written by ABC on Mon Jul 27 21:45:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x08), .O(new_n48_));
  nor2   g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  aoi21  g03(.a(new_n47_), .b(x19), .c(new_n49_), .O(z00));
  and2   g04(.a(x27), .b(x09), .O(new_n51_));
  aoi21  g05(.a(new_n47_), .b(x20), .c(new_n51_), .O(z01));
  inv1   g06(.a(x10), .O(new_n53_));
  nor2   g07(.a(new_n47_), .b(new_n53_), .O(new_n54_));
  aoi21  g08(.a(new_n47_), .b(x21), .c(new_n54_), .O(z02));
  inv1   g09(.a(x11), .O(new_n56_));
  nor2   g10(.a(new_n47_), .b(new_n56_), .O(new_n57_));
  aoi21  g11(.a(new_n47_), .b(x22), .c(new_n57_), .O(z03));
  inv1   g12(.a(x12), .O(new_n59_));
  nor2   g13(.a(new_n47_), .b(new_n59_), .O(new_n60_));
  aoi21  g14(.a(new_n47_), .b(x23), .c(new_n60_), .O(z04));
  inv1   g15(.a(x13), .O(new_n62_));
  nor2   g16(.a(new_n47_), .b(new_n62_), .O(new_n63_));
  aoi21  g17(.a(new_n47_), .b(x24), .c(new_n63_), .O(z05));
  inv1   g18(.a(x14), .O(new_n65_));
  nor2   g19(.a(new_n47_), .b(new_n65_), .O(new_n66_));
  aoi21  g20(.a(new_n47_), .b(x25), .c(new_n66_), .O(z06));
  inv1   g21(.a(x15), .O(new_n68_));
  nor2   g22(.a(new_n47_), .b(new_n68_), .O(new_n69_));
  aoi21  g23(.a(new_n47_), .b(x26), .c(new_n69_), .O(z07));
  xnor2a g24(.a(x19), .b(x17), .O(new_n71_));
  nand2  g25(.a(new_n71_), .b(x16), .O(new_n72_));
  inv1   g26(.a(x16), .O(new_n73_));
  inv1   g27(.a(x00), .O(new_n74_));
  nand2  g28(.a(x18), .b(new_n74_), .O(new_n75_));
  inv1   g29(.a(x18), .O(new_n76_));
  nand2  g30(.a(new_n76_), .b(new_n48_), .O(new_n77_));
  nand3  g31(.a(new_n77_), .b(new_n75_), .c(new_n73_), .O(new_n78_));
  nand2  g32(.a(new_n78_), .b(new_n72_), .O(z09));
  oai21  g33(.a(x19), .b(x17), .c(x20), .O(new_n80_));
  nand2  g34(.a(x18), .b(x01), .O(new_n81_));
  aoi21  g35(.a(new_n76_), .b(x09), .c(x16), .O(new_n82_));
  aoi22  g36(.a(new_n82_), .b(new_n81_), .c(new_n80_), .d(x16), .O(z10));
  nand2  g37(.a(new_n76_), .b(new_n53_), .O(new_n84_));
  inv1   g38(.a(x02), .O(new_n85_));
  nand2  g39(.a(x18), .b(new_n85_), .O(new_n86_));
  nand2  g40(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nor2   g41(.a(x21), .b(new_n73_), .O(new_n88_));
  aoi21  g42(.a(new_n87_), .b(new_n73_), .c(new_n88_), .O(z11));
  nand2  g43(.a(new_n76_), .b(new_n56_), .O(new_n90_));
  inv1   g44(.a(x03), .O(new_n91_));
  nand2  g45(.a(x18), .b(new_n91_), .O(new_n92_));
  nand2  g46(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nor2   g47(.a(x22), .b(new_n73_), .O(new_n94_));
  aoi21  g48(.a(new_n93_), .b(new_n73_), .c(new_n94_), .O(z12));
  nand2  g49(.a(new_n76_), .b(new_n59_), .O(new_n96_));
  inv1   g50(.a(x04), .O(new_n97_));
  nand2  g51(.a(x18), .b(new_n97_), .O(new_n98_));
  nand2  g52(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nor2   g53(.a(x23), .b(new_n73_), .O(new_n100_));
  aoi21  g54(.a(new_n99_), .b(new_n73_), .c(new_n100_), .O(z13));
  nand2  g55(.a(new_n76_), .b(new_n62_), .O(new_n102_));
  inv1   g56(.a(x05), .O(new_n103_));
  nand2  g57(.a(x18), .b(new_n103_), .O(new_n104_));
  nand2  g58(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nor2   g59(.a(x24), .b(new_n73_), .O(new_n106_));
  aoi21  g60(.a(new_n105_), .b(new_n73_), .c(new_n106_), .O(z14));
  nand2  g61(.a(new_n76_), .b(new_n65_), .O(new_n108_));
  inv1   g62(.a(x06), .O(new_n109_));
  nand2  g63(.a(x18), .b(new_n109_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nor2   g65(.a(x25), .b(new_n73_), .O(new_n112_));
  aoi21  g66(.a(new_n111_), .b(new_n73_), .c(new_n112_), .O(z15));
  nand2  g67(.a(new_n76_), .b(new_n68_), .O(new_n114_));
  inv1   g68(.a(x07), .O(new_n115_));
  nand2  g69(.a(x18), .b(new_n115_), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nor2   g71(.a(x26), .b(new_n73_), .O(new_n118_));
  aoi21  g72(.a(new_n117_), .b(new_n73_), .c(new_n118_), .O(z16));
  nand2  g73(.a(x27), .b(x17), .O(new_n120_));
  nor2   g74(.a(x24), .b(x23), .O(new_n121_));
  nor2   g75(.a(x26), .b(x25), .O(new_n122_));
  nor2   g76(.a(x20), .b(x17), .O(new_n123_));
  nor2   g77(.a(x22), .b(x21), .O(new_n124_));
  nand4  g78(.a(new_n124_), .b(new_n123_), .c(new_n122_), .d(new_n121_), .O(new_n125_));
  aoi21  g79(.a(new_n125_), .b(new_n120_), .c(new_n73_), .O(z17));
  buf    g80(.a(x27), .O(z08));
endmodule


