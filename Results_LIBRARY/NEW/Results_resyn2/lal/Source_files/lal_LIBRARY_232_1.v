// Benchmark "FAU" written by ABC on Mon Jul 27 19:17:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n74_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x23), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x21), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n50_), .c(new_n51_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n49_), .c(x22), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n48_), .O(new_n58_));
  inv1   g13(.a(x25), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n49_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n60_), .c(new_n58_), .O(z01));
  nand2  g19(.a(new_n60_), .b(new_n58_), .O(z03));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(x08), .O(z04));
  nor2   g27(.a(x13), .b(x08), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(x08), .O(z06));
  nand2  g30(.a(new_n46_), .b(x06), .O(z07));
  aoi21  g31(.a(new_n56_), .b(x22), .c(x23), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(new_n49_), .c(new_n59_), .O(z08));
  inv1   g33(.a(x07), .O(new_n79_));
  inv1   g34(.a(x15), .O(new_n80_));
  nand3  g35(.a(new_n62_), .b(new_n80_), .c(new_n79_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  nand3  g37(.a(new_n61_), .b(new_n80_), .c(new_n79_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n52_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z10));
  nor2   g41(.a(x18), .b(x17), .O(new_n87_));
  nor2   g42(.a(new_n53_), .b(new_n52_), .O(new_n88_));
  nor3   g43(.a(new_n88_), .b(new_n83_), .c(new_n87_), .O(z11));
  nand2  g44(.a(new_n88_), .b(x19), .O(new_n90_));
  oai21  g45(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n90_), .c(new_n84_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z12));
  nor2   g48(.a(new_n54_), .b(new_n53_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(x20), .O(new_n96_));
  nand3  g51(.a(new_n94_), .b(new_n50_), .c(x17), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n84_), .O(z13));
  nand2  g53(.a(new_n97_), .b(x21), .O(new_n99_));
  inv1   g54(.a(new_n97_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n51_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n99_), .c(new_n84_), .O(z14));
  oai21  g57(.a(new_n97_), .b(x21), .c(x22), .O(new_n103_));
  nor2   g58(.a(x22), .b(x21), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n100_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n84_), .O(z15));
  inv1   g61(.a(new_n104_), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(new_n97_), .c(x23), .O(new_n108_));
  nand3  g63(.a(new_n104_), .b(new_n48_), .c(new_n50_), .O(new_n109_));
  or2    g64(.a(new_n109_), .b(new_n95_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n84_), .O(z16));
  oai21  g66(.a(new_n109_), .b(new_n95_), .c(new_n49_), .O(new_n112_));
  nand3  g67(.a(new_n48_), .b(new_n50_), .c(x17), .O(new_n113_));
  inv1   g68(.a(new_n113_), .O(new_n114_));
  nand2  g69(.a(x24), .b(x19), .O(new_n115_));
  aoi21  g70(.a(x21), .b(x18), .c(new_n115_), .O(new_n116_));
  aoi22  g71(.a(x22), .b(new_n51_), .c(x19), .d(new_n53_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n114_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n112_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n84_), .O(z17));
  nand4  g75(.a(new_n114_), .b(new_n104_), .c(new_n94_), .d(new_n49_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n84_), .c(new_n59_), .O(z18));
  buf    g77(.a(x16), .O(z02));
endmodule


