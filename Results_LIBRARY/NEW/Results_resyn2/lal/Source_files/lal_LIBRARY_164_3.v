// Benchmark "FAU" written by ABC on Mon Jul 27 19:16:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x07), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g06(.a(x23), .O(new_n52_));
  inv1   g07(.a(x21), .O(new_n53_));
  inv1   g08(.a(x22), .O(new_n54_));
  nor2   g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  oai21  g11(.a(x19), .b(x17), .c(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n55_), .c(x25), .O(new_n58_));
  inv1   g13(.a(x17), .O(new_n59_));
  nand4  g14(.a(x24), .b(new_n56_), .c(x18), .d(new_n59_), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n58_), .c(new_n52_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n62_), .c(new_n51_), .d(new_n48_), .O(z01));
  nand2  g19(.a(new_n62_), .b(new_n51_), .O(z03));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(x08), .O(z04));
  nor2   g27(.a(x13), .b(x08), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(x08), .O(z06));
  nand2  g30(.a(new_n46_), .b(x06), .O(z07));
  inv1   g31(.a(new_n55_), .O(new_n77_));
  inv1   g32(.a(x18), .O(new_n78_));
  inv1   g33(.a(x19), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n78_), .c(new_n59_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n56_), .c(new_n77_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(x23), .c(x24), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n50_), .O(z08));
  inv1   g38(.a(x15), .O(new_n84_));
  nand4  g39(.a(new_n84_), .b(new_n48_), .c(x05), .d(x04), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  nand3  g41(.a(new_n63_), .b(new_n84_), .c(new_n48_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n59_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nand2  g45(.a(x18), .b(new_n59_), .O(new_n91_));
  nand2  g46(.a(new_n78_), .b(x17), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n91_), .c(new_n87_), .O(z11));
  aoi21  g48(.a(x18), .b(x17), .c(x19), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n94_), .c(new_n87_), .O(z12));
  nand2  g52(.a(new_n95_), .b(x20), .O(new_n98_));
  nand2  g53(.a(new_n96_), .b(new_n56_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n88_), .O(z13));
  nand2  g55(.a(new_n99_), .b(x21), .O(new_n101_));
  nor2   g56(.a(new_n99_), .b(x21), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n88_), .O(z14));
  nand2  g59(.a(new_n102_), .b(new_n54_), .O(new_n105_));
  nand2  g60(.a(new_n103_), .b(x22), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n88_), .O(z15));
  nand3  g62(.a(new_n102_), .b(new_n52_), .c(new_n54_), .O(new_n108_));
  nand2  g63(.a(new_n105_), .b(x23), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n88_), .O(z16));
  nand2  g65(.a(x19), .b(x18), .O(new_n111_));
  nand3  g66(.a(new_n50_), .b(x24), .c(new_n79_), .O(new_n112_));
  nand3  g67(.a(new_n49_), .b(new_n54_), .c(new_n53_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n111_), .c(new_n112_), .O(new_n114_));
  nand3  g69(.a(new_n50_), .b(x21), .c(x19), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n91_), .c(new_n49_), .O(new_n116_));
  aoi21  g71(.a(new_n114_), .b(x17), .c(new_n116_), .O(new_n117_));
  nand4  g72(.a(new_n50_), .b(x24), .c(x22), .d(new_n53_), .O(new_n118_));
  oai21  g73(.a(new_n117_), .b(x20), .c(new_n118_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n52_), .O(new_n120_));
  nand3  g75(.a(new_n52_), .b(new_n56_), .c(x18), .O(new_n121_));
  aoi21  g76(.a(new_n121_), .b(x24), .c(new_n87_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n120_), .O(z17));
  nand2  g78(.a(new_n88_), .b(new_n50_), .O(new_n124_));
  inv1   g79(.a(new_n124_), .O(new_n125_));
  oai21  g80(.a(new_n108_), .b(x24), .c(new_n125_), .O(z18));
  buf    g81(.a(x16), .O(z02));
endmodule


