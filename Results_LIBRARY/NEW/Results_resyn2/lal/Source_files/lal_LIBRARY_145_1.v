// Benchmark "FAU" written by ABC on Mon Jul 27 19:16:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n73_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n114_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x25), .O(new_n48_));
  inv1   g03(.a(x20), .O(new_n49_));
  inv1   g04(.a(x17), .O(new_n50_));
  inv1   g05(.a(x18), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  inv1   g09(.a(x22), .O(new_n55_));
  nand2  g10(.a(x24), .b(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n54_), .c(x23), .O(new_n57_));
  nand2  g12(.a(x24), .b(x21), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n57_), .c(new_n48_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(new_n59_), .O(z03));
  xor2a  g19(.a(x12), .b(x03), .O(new_n65_));
  xor2a  g20(.a(x11), .b(x02), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  xor2a  g22(.a(x09), .b(x00), .O(new_n68_));
  xor2a  g23(.a(x10), .b(x01), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n67_), .c(x08), .O(z04));
  nor2   g26(.a(x13), .b(x08), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(x08), .O(z06));
  nand2  g29(.a(new_n46_), .b(x06), .O(z07));
  inv1   g30(.a(new_n58_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n54_), .c(x22), .O(new_n77_));
  aoi21  g32(.a(x24), .b(x23), .c(x25), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n77_), .O(z08));
  inv1   g34(.a(x07), .O(new_n80_));
  inv1   g35(.a(x15), .O(new_n81_));
  nand3  g36(.a(new_n61_), .b(new_n81_), .c(new_n80_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  nand3  g38(.a(new_n60_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n50_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z10));
  nor2   g42(.a(x18), .b(x17), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nor3   g45(.a(new_n90_), .b(new_n84_), .c(new_n88_), .O(z11));
  nand3  g46(.a(x19), .b(x18), .c(x17), .O(new_n92_));
  nand2  g47(.a(new_n89_), .b(new_n52_), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n92_), .c(new_n85_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z12));
  nor2   g50(.a(new_n92_), .b(x20), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n92_), .b(x20), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n85_), .O(z13));
  inv1   g54(.a(x21), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(new_n100_), .O(new_n101_));
  nand2  g56(.a(new_n97_), .b(x21), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n85_), .O(z14));
  nand2  g58(.a(new_n101_), .b(x22), .O(new_n104_));
  nand3  g59(.a(new_n96_), .b(new_n55_), .c(new_n100_), .O(new_n105_));
  and2   g60(.a(new_n105_), .b(new_n85_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n104_), .O(z15));
  inv1   g62(.a(x23), .O(new_n108_));
  nand2  g63(.a(new_n106_), .b(new_n108_), .O(z16));
  nand2  g64(.a(new_n105_), .b(x24), .O(new_n110_));
  nor2   g65(.a(x24), .b(x22), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n96_), .c(new_n100_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n85_), .O(z17));
  aoi21  g68(.a(new_n112_), .b(x25), .c(new_n84_), .O(new_n114_));
  oai21  g69(.a(new_n112_), .b(x25), .c(new_n114_), .O(z18));
  buf    g70(.a(x16), .O(z02));
endmodule


