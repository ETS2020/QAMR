// Benchmark "FAU" written by ABC on Mon Jul 27 18:27:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand2  g03(.a(x3), .b(new_n18_), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(x1), .c(x2), .O(new_n21_));
  aoi21  g06(.a(new_n19_), .b(new_n17_), .c(new_n21_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  inv1   g08(.a(x4), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  nand3  g10(.a(x3), .b(new_n16_), .c(x0), .O(new_n26_));
  oai21  g11(.a(new_n25_), .b(new_n16_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n23_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n16_), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n20_), .c(new_n23_), .O(new_n32_));
  aoi22  g17(.a(new_n32_), .b(new_n16_), .c(new_n30_), .d(new_n23_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x0), .c(new_n28_), .O(z1));
  nand3  g19(.a(new_n24_), .b(new_n20_), .c(new_n23_), .O(new_n35_));
  aoi21  g20(.a(new_n20_), .b(x2), .c(new_n16_), .O(new_n36_));
  aoi21  g21(.a(x6), .b(new_n23_), .c(x3), .O(new_n37_));
  aoi21  g22(.a(x5), .b(x2), .c(x1), .O(new_n38_));
  aoi22  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n39_));
  nand4  g24(.a(x3), .b(new_n23_), .c(new_n16_), .d(x0), .O(new_n40_));
  oai21  g25(.a(new_n39_), .b(x0), .c(new_n40_), .O(z2));
  oai21  g26(.a(x7), .b(x0), .c(new_n25_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n23_), .c(x1), .O(new_n43_));
  nand2  g28(.a(x5), .b(new_n18_), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n20_), .c(x2), .d(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z3));
  nand2  g31(.a(x6), .b(new_n23_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n20_), .c(new_n18_), .O(new_n48_));
  xor2a  g33(.a(x3), .b(x2), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(x0), .O(new_n50_));
  nand2  g35(.a(new_n31_), .b(new_n20_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(x2), .c(new_n18_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n50_), .c(new_n48_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n16_), .O(new_n54_));
  nand2  g39(.a(new_n20_), .b(x2), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n35_), .c(x1), .O(new_n56_));
  nand3  g41(.a(x6), .b(new_n20_), .c(new_n23_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n18_), .O(new_n59_));
  nand4  g44(.a(new_n24_), .b(new_n20_), .c(new_n23_), .d(x1), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n59_), .c(new_n54_), .O(z4));
  nand2  g46(.a(new_n52_), .b(new_n50_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n16_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n60_), .c(new_n59_), .O(z5));
  nor2   g49(.a(x5), .b(x3), .O(new_n65_));
  aoi22  g50(.a(new_n49_), .b(x0), .c(new_n65_), .d(x2), .O(new_n66_));
  nand2  g51(.a(new_n35_), .b(new_n19_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(x1), .O(new_n68_));
  oai21  g53(.a(new_n66_), .b(x1), .c(new_n68_), .O(z6));
endmodule


