// Benchmark "FAU" written by ABC on Mon Jul 27 18:27:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x3), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n18_), .b(new_n16_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  aoi21  g07(.a(new_n20_), .b(new_n17_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  inv1   g09(.a(x5), .O(new_n25_));
  aoi21  g10(.a(new_n17_), .b(new_n25_), .c(new_n24_), .O(new_n26_));
  nand3  g11(.a(x3), .b(new_n24_), .c(x0), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n26_), .c(new_n18_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n18_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n24_), .c(new_n16_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(z1));
  nand2  g18(.a(new_n25_), .b(x2), .O(new_n34_));
  nand2  g19(.a(new_n30_), .b(new_n24_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(new_n20_), .O(new_n36_));
  nand2  g21(.a(x4), .b(new_n24_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n19_), .c(new_n18_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n36_), .c(new_n16_), .O(new_n39_));
  nand4  g24(.a(x3), .b(new_n24_), .c(new_n18_), .d(x0), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(z2));
  nand4  g26(.a(new_n19_), .b(x2), .c(new_n18_), .d(x0), .O(new_n42_));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n24_), .c(new_n18_), .O(new_n44_));
  nand3  g29(.a(new_n25_), .b(new_n19_), .c(x2), .O(new_n45_));
  inv1   g30(.a(new_n45_), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(x1), .c(new_n16_), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n44_), .c(new_n42_), .O(z3));
  nand2  g33(.a(x3), .b(new_n24_), .O(new_n49_));
  nand2  g34(.a(new_n19_), .b(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai21  g36(.a(new_n25_), .b(new_n19_), .c(x0), .O(new_n52_));
  nor2   g37(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g38(.a(new_n19_), .b(x2), .c(x0), .O(new_n54_));
  oai22  g39(.a(new_n54_), .b(new_n18_), .c(new_n49_), .d(new_n21_), .O(new_n55_));
  nor2   g40(.a(new_n55_), .b(new_n53_), .O(z4));
  aoi21  g41(.a(new_n50_), .b(new_n49_), .c(new_n16_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n26_), .c(new_n18_), .O(new_n58_));
  nand2  g43(.a(new_n54_), .b(new_n31_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n58_), .O(z5));
  oai21  g45(.a(new_n57_), .b(new_n46_), .c(new_n18_), .O(new_n61_));
  oai21  g46(.a(x4), .b(x2), .c(new_n19_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(x1), .c(new_n16_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(z6));
endmodule


