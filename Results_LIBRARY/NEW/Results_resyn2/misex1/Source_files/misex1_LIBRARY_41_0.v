// Benchmark "FAU" written by ABC on Mon Jul 27 18:27:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n16_), .b(x0), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  nor2   g08(.a(x5), .b(x3), .O(new_n24_));
  aoi21  g09(.a(x5), .b(new_n23_), .c(new_n24_), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(new_n16_), .O(new_n27_));
  aoi22  g12(.a(new_n27_), .b(new_n23_), .c(new_n25_), .d(new_n16_), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n23_), .O(new_n29_));
  or2    g14(.a(new_n29_), .b(new_n19_), .O(new_n30_));
  oai21  g15(.a(new_n28_), .b(x0), .c(new_n30_), .O(z1));
  oai21  g16(.a(x6), .b(x2), .c(x5), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n18_), .c(x1), .O(new_n33_));
  inv1   g18(.a(x0), .O(new_n34_));
  inv1   g19(.a(x4), .O(new_n35_));
  nor2   g20(.a(new_n35_), .b(x2), .O(new_n36_));
  nand2  g21(.a(new_n18_), .b(x1), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n36_), .c(new_n34_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n33_), .c(new_n30_), .O(z2));
  nand2  g24(.a(new_n18_), .b(x2), .O(new_n40_));
  nor3   g25(.a(x5), .b(x3), .c(x1), .O(new_n41_));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nor2   g27(.a(x2), .b(new_n16_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(new_n44_));
  oai22  g29(.a(new_n44_), .b(x0), .c(new_n40_), .d(new_n19_), .O(z3));
  aoi21  g30(.a(new_n40_), .b(new_n29_), .c(new_n34_), .O(new_n46_));
  inv1   g31(.a(x5), .O(new_n47_));
  nor2   g32(.a(new_n47_), .b(x2), .O(new_n48_));
  nand2  g33(.a(new_n32_), .b(new_n18_), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(new_n48_), .c(x0), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n46_), .c(new_n16_), .O(new_n51_));
  nand3  g36(.a(new_n40_), .b(new_n27_), .c(new_n34_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(z4));
  nand2  g38(.a(new_n47_), .b(new_n18_), .O(new_n54_));
  nand2  g39(.a(x5), .b(new_n23_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n34_), .O(new_n56_));
  inv1   g41(.a(new_n56_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n46_), .c(new_n16_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n52_), .O(z5));
  oai21  g44(.a(x4), .b(x2), .c(new_n18_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(x1), .c(new_n41_), .O(new_n61_));
  nand2  g46(.a(new_n46_), .b(new_n16_), .O(new_n62_));
  oai21  g47(.a(new_n61_), .b(x0), .c(new_n62_), .O(z6));
endmodule


