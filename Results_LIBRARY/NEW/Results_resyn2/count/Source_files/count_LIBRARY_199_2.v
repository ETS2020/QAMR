// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  inv1   g016(.a(x18), .O(new_n68_));
  nand2  g017(.a(new_n58_), .b(x13), .O(new_n69_));
  inv1   g018(.a(x21), .O(new_n70_));
  nand2  g019(.a(new_n63_), .b(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n55_), .b(new_n61_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x21), .O(new_n73_));
  nand3  g022(.a(new_n73_), .b(new_n71_), .c(x16), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n68_), .O(z02));
  xor2a  g025(.a(new_n71_), .b(x22), .O(new_n77_));
  inv1   g026(.a(x12), .O(new_n78_));
  aoi21  g027(.a(new_n58_), .b(new_n78_), .c(x18), .O(new_n79_));
  oai21  g028(.a(new_n77_), .b(new_n58_), .c(new_n79_), .O(z03));
  inv1   g029(.a(x22), .O(new_n81_));
  nand3  g030(.a(new_n63_), .b(new_n81_), .c(new_n70_), .O(new_n82_));
  nor3   g031(.a(x23), .b(x22), .c(x21), .O(new_n83_));
  aoi22  g032(.a(new_n83_), .b(new_n63_), .c(new_n82_), .d(x23), .O(new_n84_));
  inv1   g033(.a(x11), .O(new_n85_));
  aoi21  g034(.a(new_n58_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n58_), .c(new_n86_), .O(z04));
  inv1   g036(.a(x24), .O(new_n88_));
  aoi21  g037(.a(new_n83_), .b(new_n63_), .c(new_n88_), .O(new_n89_));
  inv1   g038(.a(x23), .O(new_n90_));
  nand4  g039(.a(new_n88_), .b(new_n90_), .c(new_n81_), .d(new_n70_), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n72_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n89_), .c(x16), .O(new_n93_));
  inv1   g042(.a(x10), .O(new_n94_));
  aoi21  g043(.a(new_n58_), .b(new_n94_), .c(x18), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n93_), .O(z05));
  nand2  g045(.a(new_n58_), .b(x09), .O(new_n97_));
  inv1   g046(.a(x25), .O(new_n98_));
  nand2  g047(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  nor2   g048(.a(x23), .b(x22), .O(new_n100_));
  nor2   g049(.a(x24), .b(x21), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n100_), .c(new_n63_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(x25), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n99_), .c(x16), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n97_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n68_), .O(z06));
  nor2   g055(.a(x26), .b(x25), .O(new_n107_));
  aoi22  g056(.a(new_n107_), .b(new_n92_), .c(new_n99_), .d(x26), .O(new_n108_));
  inv1   g057(.a(x08), .O(new_n109_));
  aoi21  g058(.a(new_n58_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n58_), .c(new_n110_), .O(z07));
  inv1   g060(.a(x27), .O(new_n112_));
  aoi21  g061(.a(new_n107_), .b(new_n92_), .c(new_n112_), .O(new_n113_));
  inv1   g062(.a(x26), .O(new_n114_));
  nand3  g063(.a(new_n112_), .b(new_n114_), .c(new_n98_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n102_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n113_), .c(x16), .O(new_n117_));
  inv1   g066(.a(x07), .O(new_n118_));
  aoi21  g067(.a(new_n58_), .b(new_n118_), .c(x18), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n117_), .O(z08));
  nand2  g069(.a(new_n58_), .b(x06), .O(new_n121_));
  nor3   g070(.a(x28), .b(x27), .c(x26), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n92_), .c(new_n98_), .O(new_n123_));
  oai21  g072(.a(new_n115_), .b(new_n102_), .c(x28), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n123_), .c(x16), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n68_), .O(z09));
  nor2   g076(.a(x29), .b(x28), .O(new_n128_));
  aoi22  g077(.a(new_n128_), .b(new_n116_), .c(new_n123_), .d(x29), .O(new_n129_));
  inv1   g078(.a(x05), .O(new_n130_));
  aoi21  g079(.a(new_n58_), .b(new_n130_), .c(x18), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n58_), .c(new_n131_), .O(z10));
  aoi21  g081(.a(x30), .b(x16), .c(new_n53_), .O(new_n133_));
  inv1   g082(.a(new_n91_), .O(new_n134_));
  nor3   g083(.a(x27), .b(x26), .c(x25), .O(new_n135_));
  nand4  g084(.a(new_n128_), .b(new_n135_), .c(new_n134_), .d(new_n63_), .O(new_n136_));
  inv1   g085(.a(x29), .O(new_n137_));
  inv1   g086(.a(x30), .O(new_n138_));
  nand4  g087(.a(new_n101_), .b(new_n100_), .c(new_n138_), .d(new_n137_), .O(new_n139_));
  inv1   g088(.a(x28), .O(new_n140_));
  nand4  g089(.a(new_n135_), .b(new_n140_), .c(new_n61_), .d(new_n52_), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  aoi21  g091(.a(new_n136_), .b(x30), .c(new_n142_), .O(new_n143_));
  inv1   g092(.a(x04), .O(new_n144_));
  aoi21  g093(.a(new_n58_), .b(new_n144_), .c(x18), .O(new_n145_));
  oai21  g094(.a(new_n143_), .b(new_n133_), .c(new_n145_), .O(z11));
  inv1   g095(.a(x31), .O(new_n147_));
  nand3  g096(.a(new_n128_), .b(new_n101_), .c(new_n100_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n115_), .c(new_n52_), .O(new_n149_));
  nand2  g098(.a(new_n63_), .b(new_n138_), .O(new_n150_));
  inv1   g099(.a(new_n150_), .O(new_n151_));
  aoi21  g100(.a(new_n151_), .b(new_n149_), .c(new_n147_), .O(new_n152_));
  nand2  g101(.a(new_n147_), .b(new_n138_), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n136_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n152_), .c(x16), .O(new_n155_));
  inv1   g104(.a(x03), .O(new_n156_));
  aoi21  g105(.a(new_n58_), .b(new_n156_), .c(x18), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n155_), .O(z12));
  nand4  g107(.a(new_n122_), .b(new_n63_), .c(new_n138_), .d(new_n137_), .O(new_n159_));
  nand4  g108(.a(new_n101_), .b(new_n100_), .c(new_n147_), .d(new_n98_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n159_), .c(x32), .O(new_n161_));
  nor2   g110(.a(x32), .b(x31), .O(new_n162_));
  nand3  g111(.a(new_n140_), .b(new_n112_), .c(new_n114_), .O(new_n163_));
  nand2  g112(.a(new_n138_), .b(new_n137_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n162_), .c(new_n92_), .d(new_n98_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n161_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(x16), .O(new_n168_));
  inv1   g117(.a(x02), .O(new_n169_));
  aoi21  g118(.a(new_n58_), .b(new_n169_), .c(x18), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n168_), .O(z13));
  nand4  g120(.a(new_n162_), .b(new_n101_), .c(new_n100_), .d(new_n98_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n159_), .c(x33), .O(new_n173_));
  nor3   g122(.a(x33), .b(x32), .c(x31), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n165_), .c(new_n92_), .d(new_n98_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n173_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  inv1   g126(.a(x01), .O(new_n178_));
  aoi21  g127(.a(new_n58_), .b(new_n178_), .c(x18), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z14));
  inv1   g129(.a(x34), .O(new_n181_));
  nand2  g130(.a(new_n175_), .b(new_n181_), .O(new_n182_));
  nor2   g131(.a(new_n148_), .b(new_n115_), .O(new_n183_));
  nand4  g132(.a(new_n174_), .b(new_n151_), .c(new_n183_), .d(x34), .O(new_n184_));
  nand3  g133(.a(new_n184_), .b(new_n182_), .c(x16), .O(new_n185_));
  inv1   g134(.a(x00), .O(new_n186_));
  aoi21  g135(.a(new_n58_), .b(new_n186_), .c(x18), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n185_), .O(z15));
endmodule


