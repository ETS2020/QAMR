// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n54_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n53_), .b(new_n59_), .O(new_n61_));
  inv1   g010(.a(new_n61_), .O(new_n62_));
  oai21  g011(.a(new_n62_), .b(new_n60_), .c(x16), .O(new_n63_));
  inv1   g012(.a(x14), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(new_n64_), .c(x18), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n63_), .O(z01));
  xor2a  g015(.a(new_n61_), .b(x21), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n56_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n56_), .c(new_n69_), .O(z02));
  inv1   g019(.a(x21), .O(new_n71_));
  inv1   g020(.a(x22), .O(new_n72_));
  aoi21  g021(.a(new_n62_), .b(new_n71_), .c(new_n72_), .O(new_n73_));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(new_n61_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(new_n73_), .c(x16), .O(new_n77_));
  inv1   g026(.a(x12), .O(new_n78_));
  aoi21  g027(.a(new_n56_), .b(new_n78_), .c(x18), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n77_), .O(z03));
  inv1   g029(.a(x18), .O(new_n81_));
  nand2  g030(.a(new_n56_), .b(x11), .O(new_n82_));
  inv1   g031(.a(x23), .O(new_n83_));
  nand2  g032(.a(new_n76_), .b(new_n83_), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  oai21  g034(.a(new_n76_), .b(new_n83_), .c(x16), .O(new_n86_));
  oai21  g035(.a(new_n86_), .b(new_n85_), .c(new_n82_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n81_), .O(z04));
  nor2   g037(.a(x24), .b(x23), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n74_), .c(new_n53_), .d(new_n59_), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  aoi21  g040(.a(new_n84_), .b(x24), .c(new_n91_), .O(new_n92_));
  inv1   g041(.a(x10), .O(new_n93_));
  aoi21  g042(.a(new_n56_), .b(new_n93_), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n56_), .c(new_n94_), .O(z05));
  nand2  g044(.a(new_n56_), .b(x09), .O(new_n96_));
  nor2   g045(.a(new_n90_), .b(x25), .O(new_n97_));
  inv1   g046(.a(x25), .O(new_n98_));
  oai21  g047(.a(new_n91_), .b(new_n98_), .c(x16), .O(new_n99_));
  oai21  g048(.a(new_n99_), .b(new_n97_), .c(new_n96_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n81_), .O(z06));
  inv1   g050(.a(x26), .O(new_n102_));
  nor2   g051(.a(x20), .b(x17), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n74_), .c(new_n83_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  nor3   g054(.a(x25), .b(x24), .c(x19), .O(new_n106_));
  aoi21  g055(.a(new_n106_), .b(new_n105_), .c(new_n102_), .O(new_n107_));
  nor2   g056(.a(x26), .b(x25), .O(new_n108_));
  inv1   g057(.a(new_n108_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n90_), .O(new_n110_));
  oai21  g059(.a(new_n110_), .b(new_n107_), .c(x16), .O(new_n111_));
  inv1   g060(.a(x08), .O(new_n112_));
  aoi21  g061(.a(new_n56_), .b(new_n112_), .c(x18), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n111_), .O(z07));
  inv1   g063(.a(new_n110_), .O(new_n115_));
  nor3   g064(.a(new_n109_), .b(new_n90_), .c(x27), .O(new_n116_));
  aoi21  g065(.a(new_n115_), .b(x27), .c(new_n116_), .O(new_n117_));
  inv1   g066(.a(x07), .O(new_n118_));
  aoi21  g067(.a(new_n56_), .b(new_n118_), .c(x18), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n56_), .c(new_n119_), .O(z08));
  inv1   g069(.a(x28), .O(new_n121_));
  inv1   g070(.a(x27), .O(new_n122_));
  nand3  g071(.a(new_n121_), .b(new_n122_), .c(new_n102_), .O(new_n123_));
  inv1   g072(.a(new_n123_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n106_), .c(new_n105_), .O(new_n125_));
  oai21  g074(.a(new_n116_), .b(new_n121_), .c(new_n125_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(x16), .O(new_n127_));
  inv1   g076(.a(x06), .O(new_n128_));
  aoi21  g077(.a(new_n56_), .b(new_n128_), .c(x18), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n127_), .O(z09));
  nand2  g079(.a(new_n124_), .b(new_n97_), .O(new_n131_));
  inv1   g080(.a(x29), .O(new_n132_));
  nand4  g081(.a(new_n108_), .b(new_n132_), .c(new_n121_), .d(new_n122_), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n90_), .O(new_n134_));
  aoi21  g083(.a(new_n131_), .b(x29), .c(new_n134_), .O(new_n135_));
  inv1   g084(.a(x05), .O(new_n136_));
  aoi21  g085(.a(new_n56_), .b(new_n136_), .c(x18), .O(new_n137_));
  oai21  g086(.a(new_n135_), .b(new_n56_), .c(new_n137_), .O(z10));
  inv1   g087(.a(x19), .O(new_n139_));
  inv1   g088(.a(x24), .O(new_n140_));
  nand4  g089(.a(new_n103_), .b(new_n74_), .c(new_n140_), .d(new_n83_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n133_), .c(x30), .O(new_n142_));
  inv1   g091(.a(x30), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n132_), .O(new_n144_));
  inv1   g093(.a(new_n144_), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n89_), .c(new_n121_), .O(new_n146_));
  nor2   g095(.a(new_n109_), .b(x27), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n103_), .c(new_n74_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n146_), .c(new_n142_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n139_), .O(new_n150_));
  nand3  g099(.a(x30), .b(x19), .c(x16), .O(new_n151_));
  inv1   g100(.a(x04), .O(new_n152_));
  aoi21  g101(.a(new_n56_), .b(new_n152_), .c(x18), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n151_), .c(new_n150_), .O(z11));
  nand4  g103(.a(new_n145_), .b(new_n124_), .c(new_n106_), .d(new_n105_), .O(new_n155_));
  nor2   g104(.a(new_n144_), .b(x31), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n121_), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  aoi22  g107(.a(new_n158_), .b(new_n116_), .c(new_n155_), .d(x31), .O(new_n159_));
  inv1   g108(.a(x03), .O(new_n160_));
  aoi21  g109(.a(new_n56_), .b(new_n160_), .c(x18), .O(new_n161_));
  oai21  g110(.a(new_n159_), .b(new_n56_), .c(new_n161_), .O(z12));
  nor3   g111(.a(new_n123_), .b(new_n90_), .c(x25), .O(new_n163_));
  nand4  g112(.a(new_n156_), .b(new_n124_), .c(new_n106_), .d(new_n105_), .O(new_n164_));
  nor2   g113(.a(x32), .b(x31), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n145_), .O(new_n166_));
  inv1   g115(.a(new_n166_), .O(new_n167_));
  aoi22  g116(.a(new_n167_), .b(new_n163_), .c(new_n164_), .d(x32), .O(new_n168_));
  inv1   g117(.a(x02), .O(new_n169_));
  aoi21  g118(.a(new_n56_), .b(new_n169_), .c(x18), .O(new_n170_));
  oai21  g119(.a(new_n168_), .b(new_n56_), .c(new_n170_), .O(z13));
  inv1   g120(.a(x33), .O(new_n172_));
  nor3   g121(.a(x32), .b(x31), .c(x30), .O(new_n173_));
  aoi21  g122(.a(new_n173_), .b(new_n134_), .c(new_n172_), .O(new_n174_));
  nand2  g123(.a(new_n173_), .b(new_n172_), .O(new_n175_));
  nor3   g124(.a(new_n175_), .b(new_n133_), .c(new_n90_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n174_), .c(x16), .O(new_n177_));
  inv1   g126(.a(x01), .O(new_n178_));
  aoi21  g127(.a(new_n56_), .b(new_n178_), .c(x18), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z14));
  nor2   g129(.a(new_n176_), .b(x34), .O(new_n181_));
  nor3   g130(.a(x29), .b(x28), .c(x27), .O(new_n182_));
  nand4  g131(.a(new_n173_), .b(new_n182_), .c(x34), .d(new_n172_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n115_), .c(x16), .O(new_n184_));
  inv1   g133(.a(x00), .O(new_n185_));
  aoi21  g134(.a(new_n56_), .b(new_n185_), .c(x18), .O(new_n186_));
  oai21  g135(.a(new_n184_), .b(new_n181_), .c(new_n186_), .O(z15));
endmodule


