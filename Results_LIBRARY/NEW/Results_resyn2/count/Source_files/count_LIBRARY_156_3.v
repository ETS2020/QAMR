// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  xor2a  g017(.a(new_n63_), .b(new_n68_), .O(new_n69_));
  inv1   g018(.a(x13), .O(new_n70_));
  aoi21  g019(.a(new_n58_), .b(new_n70_), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n58_), .c(new_n71_), .O(z02));
  nand2  g021(.a(new_n63_), .b(new_n68_), .O(new_n73_));
  xor2a  g022(.a(new_n73_), .b(x22), .O(new_n74_));
  inv1   g023(.a(x12), .O(new_n75_));
  aoi21  g024(.a(new_n58_), .b(new_n75_), .c(x18), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n58_), .c(new_n76_), .O(z03));
  inv1   g026(.a(x22), .O(new_n78_));
  nand3  g027(.a(new_n63_), .b(new_n78_), .c(new_n68_), .O(new_n79_));
  inv1   g028(.a(x23), .O(new_n80_));
  nor2   g029(.a(x22), .b(x21), .O(new_n81_));
  nand4  g030(.a(new_n81_), .b(new_n55_), .c(new_n80_), .d(new_n61_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  aoi21  g032(.a(new_n79_), .b(x23), .c(new_n83_), .O(new_n84_));
  inv1   g033(.a(x11), .O(new_n85_));
  aoi21  g034(.a(new_n58_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n58_), .c(new_n86_), .O(z04));
  xor2a  g036(.a(new_n82_), .b(x24), .O(new_n88_));
  inv1   g037(.a(x10), .O(new_n89_));
  aoi21  g038(.a(new_n58_), .b(new_n89_), .c(x18), .O(new_n90_));
  oai21  g039(.a(new_n88_), .b(new_n58_), .c(new_n90_), .O(z05));
  inv1   g040(.a(x24), .O(new_n92_));
  nand2  g041(.a(new_n83_), .b(new_n92_), .O(new_n93_));
  nor2   g042(.a(x25), .b(x24), .O(new_n94_));
  inv1   g043(.a(new_n94_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n82_), .O(new_n96_));
  aoi21  g045(.a(new_n93_), .b(x25), .c(new_n96_), .O(new_n97_));
  inv1   g046(.a(x09), .O(new_n98_));
  aoi21  g047(.a(new_n58_), .b(new_n98_), .c(x18), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n58_), .c(new_n99_), .O(z06));
  nor3   g049(.a(x23), .b(x22), .c(x21), .O(new_n101_));
  nand3  g050(.a(new_n94_), .b(new_n101_), .c(new_n63_), .O(new_n102_));
  xor2a  g051(.a(new_n102_), .b(x26), .O(new_n103_));
  inv1   g052(.a(x08), .O(new_n104_));
  aoi21  g053(.a(new_n58_), .b(new_n104_), .c(x18), .O(new_n105_));
  oai21  g054(.a(new_n103_), .b(new_n58_), .c(new_n105_), .O(z07));
  nand2  g055(.a(new_n81_), .b(new_n80_), .O(new_n107_));
  inv1   g056(.a(x26), .O(new_n108_));
  nand4  g057(.a(new_n94_), .b(new_n108_), .c(new_n61_), .d(new_n52_), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n107_), .c(x27), .O(new_n110_));
  nor2   g059(.a(x27), .b(x26), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n94_), .c(new_n101_), .d(new_n63_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  inv1   g063(.a(x18), .O(new_n115_));
  oai21  g064(.a(x16), .b(x07), .c(new_n115_), .O(new_n116_));
  aoi21  g065(.a(x27), .b(x19), .c(new_n116_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n114_), .O(z08));
  nor3   g067(.a(x28), .b(x27), .c(x26), .O(new_n119_));
  aoi22  g068(.a(new_n119_), .b(new_n96_), .c(new_n112_), .d(x28), .O(new_n120_));
  inv1   g069(.a(x06), .O(new_n121_));
  aoi21  g070(.a(new_n58_), .b(new_n121_), .c(x18), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n58_), .c(new_n122_), .O(z09));
  inv1   g072(.a(x29), .O(new_n124_));
  nand4  g073(.a(new_n111_), .b(new_n94_), .c(new_n81_), .d(new_n80_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n52_), .O(new_n126_));
  inv1   g075(.a(x28), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n61_), .c(new_n53_), .d(new_n52_), .O(new_n128_));
  inv1   g077(.a(new_n128_), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n126_), .c(new_n124_), .O(new_n130_));
  nand4  g079(.a(new_n94_), .b(new_n101_), .c(new_n63_), .d(new_n108_), .O(new_n131_));
  inv1   g080(.a(x27), .O(new_n132_));
  nand3  g081(.a(new_n124_), .b(new_n127_), .c(new_n132_), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n130_), .c(x16), .O(new_n135_));
  inv1   g084(.a(x05), .O(new_n136_));
  aoi21  g085(.a(new_n58_), .b(new_n136_), .c(x18), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n135_), .O(z10));
  inv1   g087(.a(x30), .O(new_n139_));
  nand2  g088(.a(new_n55_), .b(new_n61_), .O(new_n140_));
  nand2  g089(.a(new_n124_), .b(new_n127_), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n126_), .c(new_n139_), .O(new_n143_));
  nand4  g092(.a(new_n139_), .b(new_n124_), .c(new_n127_), .d(new_n132_), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n131_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n143_), .c(x16), .O(new_n146_));
  inv1   g095(.a(x04), .O(new_n147_));
  aoi21  g096(.a(new_n58_), .b(new_n147_), .c(x18), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n146_), .O(z11));
  inv1   g098(.a(x31), .O(new_n150_));
  nand2  g099(.a(new_n139_), .b(new_n124_), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n128_), .O(new_n152_));
  aoi21  g101(.a(new_n152_), .b(new_n126_), .c(new_n150_), .O(new_n153_));
  nand3  g102(.a(new_n150_), .b(new_n139_), .c(new_n124_), .O(new_n154_));
  nor3   g103(.a(new_n154_), .b(new_n128_), .c(new_n125_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n153_), .c(x16), .O(new_n156_));
  inv1   g105(.a(x03), .O(new_n157_));
  aoi21  g106(.a(new_n58_), .b(new_n157_), .c(x18), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n156_), .O(z12));
  inv1   g108(.a(x32), .O(new_n160_));
  nor2   g109(.a(new_n154_), .b(new_n128_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n126_), .c(new_n160_), .O(new_n162_));
  nand4  g111(.a(new_n119_), .b(new_n94_), .c(new_n101_), .d(new_n63_), .O(new_n163_));
  nand4  g112(.a(new_n160_), .b(new_n150_), .c(new_n139_), .d(new_n124_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n162_), .c(x16), .O(new_n166_));
  inv1   g115(.a(x02), .O(new_n167_));
  aoi21  g116(.a(new_n58_), .b(new_n167_), .c(x18), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(z13));
  inv1   g118(.a(x33), .O(new_n170_));
  nor2   g119(.a(new_n164_), .b(new_n128_), .O(new_n171_));
  aoi21  g120(.a(new_n171_), .b(new_n126_), .c(new_n170_), .O(new_n172_));
  nor3   g121(.a(x31), .b(x30), .c(x29), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n119_), .c(new_n170_), .d(new_n160_), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n102_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n172_), .c(x16), .O(new_n176_));
  inv1   g125(.a(x01), .O(new_n177_));
  aoi21  g126(.a(new_n58_), .b(new_n177_), .c(x18), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n176_), .O(z14));
  inv1   g128(.a(x34), .O(new_n180_));
  oai21  g129(.a(new_n174_), .b(new_n102_), .c(new_n180_), .O(new_n181_));
  nor3   g130(.a(new_n154_), .b(x33), .c(x32), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n129_), .c(new_n126_), .d(x34), .O(new_n183_));
  nand3  g132(.a(new_n183_), .b(new_n181_), .c(x16), .O(new_n184_));
  inv1   g133(.a(x00), .O(new_n185_));
  aoi21  g134(.a(new_n58_), .b(new_n185_), .c(x18), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n184_), .O(z15));
endmodule


