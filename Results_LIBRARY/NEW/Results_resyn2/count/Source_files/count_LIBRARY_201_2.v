// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  nor2   g000(.a(x19), .b(x17), .O(new_n52_));
  and2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n54_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(new_n52_), .b(new_n59_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g011(.a(x14), .O(new_n63_));
  aoi21  g012(.a(new_n56_), .b(new_n63_), .c(x18), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n62_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  xor2a  g015(.a(new_n61_), .b(new_n66_), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n56_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n56_), .c(new_n69_), .O(z02));
  nand2  g019(.a(new_n61_), .b(new_n66_), .O(new_n71_));
  nor3   g020(.a(x21), .b(x19), .c(x17), .O(new_n72_));
  nor2   g021(.a(x22), .b(x20), .O(new_n73_));
  aoi22  g022(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x22), .O(new_n74_));
  inv1   g023(.a(x12), .O(new_n75_));
  aoi21  g024(.a(new_n56_), .b(new_n75_), .c(x18), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n56_), .c(new_n76_), .O(z03));
  nand2  g026(.a(new_n73_), .b(new_n72_), .O(new_n78_));
  nor2   g027(.a(x23), .b(x22), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n72_), .c(new_n59_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  aoi21  g030(.a(new_n78_), .b(x23), .c(new_n81_), .O(new_n82_));
  inv1   g031(.a(x11), .O(new_n83_));
  aoi21  g032(.a(new_n56_), .b(new_n83_), .c(x18), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n56_), .c(new_n84_), .O(z04));
  nor2   g034(.a(x24), .b(x21), .O(new_n86_));
  nand4  g035(.a(new_n86_), .b(new_n79_), .c(new_n52_), .d(new_n59_), .O(new_n87_));
  inv1   g036(.a(new_n87_), .O(new_n88_));
  aoi21  g037(.a(new_n80_), .b(x24), .c(new_n88_), .O(new_n89_));
  inv1   g038(.a(x10), .O(new_n90_));
  aoi21  g039(.a(new_n56_), .b(new_n90_), .c(x18), .O(new_n91_));
  oai21  g040(.a(new_n89_), .b(new_n56_), .c(new_n91_), .O(z05));
  inv1   g041(.a(x25), .O(new_n93_));
  nor2   g042(.a(x25), .b(x24), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n79_), .c(new_n72_), .d(new_n59_), .O(new_n95_));
  oai21  g044(.a(new_n88_), .b(new_n93_), .c(new_n95_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x16), .O(new_n97_));
  inv1   g046(.a(x09), .O(new_n98_));
  aoi21  g047(.a(new_n56_), .b(new_n98_), .c(x18), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n97_), .O(z06));
  nor3   g049(.a(x26), .b(x25), .c(x24), .O(new_n101_));
  aoi22  g050(.a(new_n101_), .b(new_n81_), .c(new_n95_), .d(x26), .O(new_n102_));
  inv1   g051(.a(x08), .O(new_n103_));
  aoi21  g052(.a(new_n56_), .b(new_n103_), .c(x18), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n56_), .c(new_n104_), .O(z07));
  nand4  g054(.a(new_n101_), .b(new_n79_), .c(new_n72_), .d(new_n59_), .O(new_n106_));
  nand3  g055(.a(new_n86_), .b(new_n79_), .c(new_n52_), .O(new_n107_));
  inv1   g056(.a(x27), .O(new_n108_));
  nor2   g057(.a(x26), .b(x25), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n107_), .O(new_n111_));
  aoi22  g060(.a(new_n111_), .b(new_n59_), .c(new_n106_), .d(x27), .O(new_n112_));
  inv1   g061(.a(x07), .O(new_n113_));
  aoi21  g062(.a(new_n56_), .b(new_n113_), .c(x18), .O(new_n114_));
  oai21  g063(.a(new_n112_), .b(new_n56_), .c(new_n114_), .O(z08));
  inv1   g064(.a(new_n107_), .O(new_n116_));
  inv1   g065(.a(new_n110_), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n116_), .c(new_n59_), .O(new_n118_));
  nand4  g067(.a(new_n94_), .b(new_n79_), .c(new_n52_), .d(new_n66_), .O(new_n119_));
  inv1   g068(.a(x28), .O(new_n120_));
  nor2   g069(.a(x27), .b(x26), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n119_), .O(new_n123_));
  aoi22  g072(.a(new_n123_), .b(new_n59_), .c(new_n118_), .d(x28), .O(new_n124_));
  inv1   g073(.a(x06), .O(new_n125_));
  aoi21  g074(.a(new_n56_), .b(new_n125_), .c(x18), .O(new_n126_));
  oai21  g075(.a(new_n124_), .b(new_n56_), .c(new_n126_), .O(z09));
  nor3   g076(.a(x28), .b(x27), .c(x26), .O(new_n128_));
  nand4  g077(.a(new_n128_), .b(new_n94_), .c(new_n79_), .d(new_n72_), .O(new_n129_));
  nand2  g078(.a(new_n86_), .b(new_n79_), .O(new_n130_));
  inv1   g079(.a(x29), .O(new_n131_));
  nand4  g080(.a(new_n109_), .b(new_n131_), .c(new_n120_), .d(new_n108_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n130_), .c(new_n59_), .O(new_n133_));
  aoi22  g082(.a(new_n133_), .b(new_n52_), .c(new_n129_), .d(x29), .O(new_n134_));
  inv1   g083(.a(x05), .O(new_n135_));
  aoi21  g084(.a(new_n56_), .b(new_n135_), .c(x18), .O(new_n136_));
  oai21  g085(.a(new_n134_), .b(new_n56_), .c(new_n136_), .O(z10));
  nor2   g086(.a(x30), .b(x29), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n117_), .c(new_n116_), .d(new_n120_), .O(new_n139_));
  inv1   g088(.a(x24), .O(new_n140_));
  nand3  g089(.a(new_n79_), .b(new_n72_), .c(new_n140_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n132_), .c(x30), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  inv1   g093(.a(x04), .O(new_n145_));
  aoi21  g094(.a(new_n56_), .b(new_n145_), .c(x18), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n144_), .O(z11));
  inv1   g096(.a(x31), .O(new_n148_));
  nand3  g097(.a(new_n138_), .b(new_n148_), .c(new_n120_), .O(new_n149_));
  nor3   g098(.a(new_n149_), .b(new_n110_), .c(new_n107_), .O(new_n150_));
  aoi21  g099(.a(new_n139_), .b(x31), .c(new_n150_), .O(new_n151_));
  inv1   g100(.a(x03), .O(new_n152_));
  aoi21  g101(.a(new_n56_), .b(new_n152_), .c(x18), .O(new_n153_));
  oai21  g102(.a(new_n151_), .b(new_n56_), .c(new_n153_), .O(z12));
  inv1   g103(.a(new_n149_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n111_), .O(new_n156_));
  nor2   g105(.a(x32), .b(x31), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n138_), .c(new_n121_), .d(new_n120_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n119_), .O(new_n159_));
  aoi21  g108(.a(new_n156_), .b(x32), .c(new_n159_), .O(new_n160_));
  inv1   g109(.a(x02), .O(new_n161_));
  aoi21  g110(.a(new_n56_), .b(new_n161_), .c(x18), .O(new_n162_));
  oai21  g111(.a(new_n160_), .b(new_n56_), .c(new_n162_), .O(z13));
  oai21  g112(.a(new_n158_), .b(new_n119_), .c(x33), .O(new_n164_));
  inv1   g113(.a(new_n119_), .O(new_n165_));
  nor2   g114(.a(x33), .b(x32), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n121_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n149_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n164_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x16), .O(new_n171_));
  inv1   g120(.a(x01), .O(new_n172_));
  aoi21  g121(.a(new_n56_), .b(new_n172_), .c(x18), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z14));
  inv1   g123(.a(x34), .O(new_n175_));
  aoi21  g124(.a(new_n168_), .b(new_n165_), .c(new_n175_), .O(new_n176_));
  inv1   g125(.a(x33), .O(new_n177_));
  nand3  g126(.a(new_n175_), .b(new_n177_), .c(new_n93_), .O(new_n178_));
  nor3   g127(.a(new_n178_), .b(new_n158_), .c(new_n107_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n176_), .c(x16), .O(new_n180_));
  inv1   g129(.a(x00), .O(new_n181_));
  aoi21  g130(.a(new_n56_), .b(new_n181_), .c(x18), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n180_), .O(z15));
endmodule


