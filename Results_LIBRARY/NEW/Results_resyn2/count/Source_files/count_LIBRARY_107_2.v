// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n54_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  xor2a  g008(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  inv1   g009(.a(x14), .O(new_n61_));
  aoi21  g010(.a(new_n56_), .b(new_n61_), .c(x18), .O(new_n62_));
  oai21  g011(.a(new_n60_), .b(new_n56_), .c(new_n62_), .O(z01));
  nand2  g012(.a(new_n53_), .b(new_n59_), .O(new_n64_));
  nor2   g013(.a(x21), .b(x20), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n53_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  aoi21  g016(.a(new_n64_), .b(x21), .c(new_n67_), .O(new_n68_));
  inv1   g017(.a(x13), .O(new_n69_));
  aoi21  g018(.a(new_n56_), .b(new_n69_), .c(x18), .O(new_n70_));
  oai21  g019(.a(new_n68_), .b(new_n56_), .c(new_n70_), .O(z02));
  xor2a  g020(.a(new_n66_), .b(x22), .O(new_n72_));
  inv1   g021(.a(x12), .O(new_n73_));
  aoi21  g022(.a(new_n56_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n56_), .c(new_n74_), .O(z03));
  inv1   g024(.a(x22), .O(new_n76_));
  nand2  g025(.a(new_n67_), .b(new_n76_), .O(new_n77_));
  nor2   g026(.a(x23), .b(x22), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n65_), .c(new_n53_), .O(new_n79_));
  inv1   g028(.a(new_n79_), .O(new_n80_));
  aoi21  g029(.a(new_n77_), .b(x23), .c(new_n80_), .O(new_n81_));
  inv1   g030(.a(x11), .O(new_n82_));
  aoi21  g031(.a(new_n56_), .b(new_n82_), .c(x18), .O(new_n83_));
  oai21  g032(.a(new_n81_), .b(new_n56_), .c(new_n83_), .O(z04));
  inv1   g033(.a(x18), .O(new_n85_));
  nand2  g034(.a(new_n56_), .b(x10), .O(new_n86_));
  inv1   g035(.a(x24), .O(new_n87_));
  nand4  g036(.a(new_n78_), .b(new_n65_), .c(new_n53_), .d(new_n87_), .O(new_n88_));
  nand2  g037(.a(new_n79_), .b(x24), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n88_), .c(x16), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n86_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n85_), .O(z05));
  nor2   g041(.a(x24), .b(x17), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n78_), .c(new_n65_), .O(new_n94_));
  inv1   g043(.a(new_n94_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(x25), .c(x19), .O(new_n96_));
  oai21  g045(.a(new_n95_), .b(x25), .c(new_n96_), .O(new_n97_));
  nand3  g046(.a(x25), .b(x19), .c(x16), .O(new_n98_));
  inv1   g047(.a(x09), .O(new_n99_));
  aoi21  g048(.a(new_n56_), .b(new_n99_), .c(x18), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(z06));
  oai21  g050(.a(new_n88_), .b(x25), .c(x26), .O(new_n102_));
  inv1   g051(.a(x25), .O(new_n103_));
  inv1   g052(.a(x26), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n103_), .c(new_n87_), .O(new_n105_));
  inv1   g054(.a(new_n105_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n80_), .O(new_n107_));
  aoi21  g056(.a(new_n107_), .b(new_n102_), .c(new_n56_), .O(new_n108_));
  oai21  g057(.a(x16), .b(x08), .c(new_n85_), .O(new_n109_));
  or2    g058(.a(new_n109_), .b(new_n108_), .O(z07));
  inv1   g059(.a(x27), .O(new_n111_));
  nor2   g060(.a(new_n105_), .b(new_n79_), .O(new_n112_));
  xor2a  g061(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  inv1   g062(.a(x07), .O(new_n114_));
  aoi21  g063(.a(new_n56_), .b(new_n114_), .c(x18), .O(new_n115_));
  oai21  g064(.a(new_n113_), .b(new_n56_), .c(new_n115_), .O(z08));
  inv1   g065(.a(x28), .O(new_n117_));
  aoi21  g066(.a(new_n112_), .b(new_n111_), .c(new_n117_), .O(new_n118_));
  nor2   g067(.a(x28), .b(x27), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n112_), .O(new_n120_));
  inv1   g069(.a(new_n120_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n118_), .c(x16), .O(new_n122_));
  inv1   g071(.a(x06), .O(new_n123_));
  aoi21  g072(.a(new_n56_), .b(new_n123_), .c(x18), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n122_), .O(z09));
  inv1   g074(.a(x29), .O(new_n126_));
  nand2  g075(.a(new_n119_), .b(new_n126_), .O(new_n127_));
  nor3   g076(.a(new_n127_), .b(new_n105_), .c(new_n79_), .O(new_n128_));
  aoi21  g077(.a(new_n120_), .b(x29), .c(new_n128_), .O(new_n129_));
  inv1   g078(.a(x05), .O(new_n130_));
  aoi21  g079(.a(new_n56_), .b(new_n130_), .c(x18), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n56_), .c(new_n131_), .O(z10));
  nand2  g081(.a(new_n128_), .b(x30), .O(new_n133_));
  inv1   g082(.a(x30), .O(new_n134_));
  oai21  g083(.a(new_n127_), .b(new_n107_), .c(new_n134_), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n133_), .c(x16), .O(new_n136_));
  inv1   g085(.a(x04), .O(new_n137_));
  aoi21  g086(.a(new_n56_), .b(new_n137_), .c(x18), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n136_), .O(z11));
  nor2   g088(.a(x30), .b(x29), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n119_), .c(new_n106_), .d(new_n80_), .O(new_n141_));
  nor2   g090(.a(x31), .b(x30), .O(new_n142_));
  aoi22  g091(.a(new_n142_), .b(new_n128_), .c(new_n141_), .d(x31), .O(new_n143_));
  inv1   g092(.a(x03), .O(new_n144_));
  aoi21  g093(.a(new_n56_), .b(new_n144_), .c(x18), .O(new_n145_));
  oai21  g094(.a(new_n143_), .b(new_n56_), .c(new_n145_), .O(z12));
  inv1   g095(.a(x31), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n134_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(x29), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n119_), .c(new_n112_), .d(x32), .O(new_n150_));
  inv1   g099(.a(x32), .O(new_n151_));
  nand3  g100(.a(new_n149_), .b(new_n119_), .c(new_n112_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n150_), .c(x16), .O(new_n154_));
  inv1   g103(.a(x02), .O(new_n155_));
  aoi21  g104(.a(new_n56_), .b(new_n155_), .c(x18), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n154_), .O(z13));
  inv1   g106(.a(new_n127_), .O(new_n158_));
  nor2   g107(.a(new_n148_), .b(x32), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n158_), .c(new_n106_), .d(new_n80_), .O(new_n160_));
  inv1   g109(.a(x33), .O(new_n161_));
  nand3  g110(.a(new_n142_), .b(new_n161_), .c(new_n151_), .O(new_n162_));
  inv1   g111(.a(new_n162_), .O(new_n163_));
  aoi22  g112(.a(new_n163_), .b(new_n128_), .c(new_n160_), .d(x33), .O(new_n164_));
  inv1   g113(.a(x01), .O(new_n165_));
  aoi21  g114(.a(new_n56_), .b(new_n165_), .c(x18), .O(new_n166_));
  oai21  g115(.a(new_n164_), .b(new_n56_), .c(new_n166_), .O(z14));
  nand3  g116(.a(new_n163_), .b(new_n128_), .c(x34), .O(new_n168_));
  inv1   g117(.a(x34), .O(new_n169_));
  nand3  g118(.a(new_n163_), .b(new_n158_), .c(new_n112_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n168_), .c(x16), .O(new_n172_));
  inv1   g121(.a(x00), .O(new_n173_));
  aoi21  g122(.a(new_n56_), .b(new_n173_), .c(x18), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n172_), .O(z15));
endmodule


