// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x16), .O(new_n52_));
  xor2a  g01(.a(x19), .b(x17), .O(new_n53_));
  inv1   g02(.a(x15), .O(new_n54_));
  aoi21  g03(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g04(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g05(.a(x20), .O(new_n57_));
  nor2   g06(.a(x19), .b(x17), .O(new_n58_));
  nor2   g07(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g08(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g09(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g10(.a(x14), .O(new_n62_));
  aoi21  g11(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g12(.a(new_n63_), .b(new_n61_), .O(z01));
  inv1   g13(.a(x21), .O(new_n65_));
  xor2a  g14(.a(new_n60_), .b(new_n65_), .O(new_n66_));
  inv1   g15(.a(x13), .O(new_n67_));
  aoi21  g16(.a(new_n52_), .b(new_n67_), .c(x18), .O(new_n68_));
  oai21  g17(.a(new_n66_), .b(new_n52_), .c(new_n68_), .O(z02));
  nand3  g18(.a(new_n58_), .b(new_n65_), .c(new_n57_), .O(new_n70_));
  nor2   g19(.a(x22), .b(x21), .O(new_n71_));
  aoi22  g20(.a(new_n71_), .b(new_n60_), .c(new_n70_), .d(x22), .O(new_n72_));
  inv1   g21(.a(x12), .O(new_n73_));
  aoi21  g22(.a(new_n52_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g23(.a(new_n72_), .b(new_n52_), .c(new_n74_), .O(z03));
  nand3  g24(.a(new_n71_), .b(new_n58_), .c(new_n57_), .O(new_n76_));
  nor3   g25(.a(x23), .b(x22), .c(x21), .O(new_n77_));
  aoi22  g26(.a(new_n77_), .b(new_n60_), .c(new_n76_), .d(x23), .O(new_n78_));
  inv1   g27(.a(x11), .O(new_n79_));
  aoi21  g28(.a(new_n52_), .b(new_n79_), .c(x18), .O(new_n80_));
  oai21  g29(.a(new_n78_), .b(new_n52_), .c(new_n80_), .O(z04));
  inv1   g30(.a(x23), .O(new_n82_));
  nand4  g31(.a(new_n71_), .b(new_n58_), .c(new_n82_), .d(new_n57_), .O(new_n83_));
  xor2a  g32(.a(new_n83_), .b(x24), .O(new_n84_));
  inv1   g33(.a(x10), .O(new_n85_));
  aoi21  g34(.a(new_n52_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g35(.a(new_n84_), .b(new_n52_), .c(new_n86_), .O(z05));
  nor2   g36(.a(new_n83_), .b(x24), .O(new_n88_));
  oai21  g37(.a(new_n58_), .b(x25), .c(x16), .O(new_n89_));
  inv1   g38(.a(x09), .O(new_n90_));
  aoi21  g39(.a(new_n52_), .b(new_n90_), .c(x18), .O(new_n91_));
  oai21  g40(.a(new_n89_), .b(new_n88_), .c(new_n91_), .O(z06));
  inv1   g41(.a(x18), .O(new_n93_));
  nand2  g42(.a(x26), .b(x16), .O(new_n94_));
  inv1   g43(.a(x08), .O(new_n95_));
  nand2  g44(.a(new_n52_), .b(new_n95_), .O(new_n96_));
  nand3  g45(.a(new_n96_), .b(new_n94_), .c(new_n93_), .O(z07));
  nand2  g46(.a(x27), .b(x16), .O(new_n98_));
  inv1   g47(.a(x07), .O(new_n99_));
  nand2  g48(.a(new_n52_), .b(new_n99_), .O(new_n100_));
  nand3  g49(.a(new_n100_), .b(new_n98_), .c(new_n93_), .O(z08));
  nand2  g50(.a(x28), .b(x16), .O(new_n102_));
  inv1   g51(.a(x06), .O(new_n103_));
  nand2  g52(.a(new_n52_), .b(new_n103_), .O(new_n104_));
  nand3  g53(.a(new_n104_), .b(new_n102_), .c(new_n93_), .O(z09));
  nand2  g54(.a(x29), .b(x16), .O(new_n106_));
  inv1   g55(.a(x05), .O(new_n107_));
  nand2  g56(.a(new_n52_), .b(new_n107_), .O(new_n108_));
  nand3  g57(.a(new_n108_), .b(new_n106_), .c(new_n93_), .O(z10));
  nand2  g58(.a(x30), .b(x16), .O(new_n110_));
  inv1   g59(.a(x04), .O(new_n111_));
  nand2  g60(.a(new_n52_), .b(new_n111_), .O(new_n112_));
  nand3  g61(.a(new_n112_), .b(new_n110_), .c(new_n93_), .O(z11));
  nand2  g62(.a(x31), .b(x16), .O(new_n114_));
  inv1   g63(.a(x03), .O(new_n115_));
  nand2  g64(.a(new_n52_), .b(new_n115_), .O(new_n116_));
  nand3  g65(.a(new_n116_), .b(new_n114_), .c(new_n93_), .O(z12));
  nand2  g66(.a(x32), .b(x16), .O(new_n118_));
  inv1   g67(.a(x02), .O(new_n119_));
  nand2  g68(.a(new_n52_), .b(new_n119_), .O(new_n120_));
  nand3  g69(.a(new_n120_), .b(new_n118_), .c(new_n93_), .O(z13));
  nand2  g70(.a(x33), .b(x16), .O(new_n122_));
  inv1   g71(.a(x01), .O(new_n123_));
  nand2  g72(.a(new_n52_), .b(new_n123_), .O(new_n124_));
  nand3  g73(.a(new_n124_), .b(new_n122_), .c(new_n93_), .O(z14));
  nand2  g74(.a(x34), .b(x16), .O(new_n126_));
  inv1   g75(.a(x00), .O(new_n127_));
  nand2  g76(.a(new_n52_), .b(new_n127_), .O(new_n128_));
  nand3  g77(.a(new_n128_), .b(new_n126_), .c(new_n93_), .O(z15));
endmodule


