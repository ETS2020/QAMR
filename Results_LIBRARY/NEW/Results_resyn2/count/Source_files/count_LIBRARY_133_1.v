// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g009(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g010(.a(x14), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n61_), .O(z01));
  inv1   g013(.a(x21), .O(new_n65_));
  nor2   g014(.a(new_n60_), .b(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n60_), .b(new_n65_), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n66_), .c(x16), .O(new_n69_));
  inv1   g018(.a(x13), .O(new_n70_));
  aoi21  g019(.a(new_n52_), .b(new_n70_), .c(x18), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n69_), .O(z02));
  oai21  g021(.a(new_n68_), .b(x22), .c(x16), .O(new_n73_));
  inv1   g022(.a(x12), .O(new_n74_));
  aoi21  g023(.a(new_n52_), .b(new_n74_), .c(x18), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n73_), .O(z03));
  nor2   g025(.a(x23), .b(x21), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n58_), .c(new_n57_), .O(new_n78_));
  inv1   g027(.a(new_n78_), .O(new_n79_));
  aoi21  g028(.a(new_n67_), .b(x23), .c(new_n79_), .O(new_n80_));
  inv1   g029(.a(x11), .O(new_n81_));
  aoi21  g030(.a(new_n52_), .b(new_n81_), .c(x18), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n52_), .c(new_n82_), .O(z04));
  inv1   g032(.a(x18), .O(new_n84_));
  nand2  g033(.a(new_n52_), .b(x10), .O(new_n85_));
  inv1   g034(.a(x24), .O(new_n86_));
  nand2  g035(.a(new_n79_), .b(new_n86_), .O(new_n87_));
  aoi21  g036(.a(new_n78_), .b(x24), .c(new_n52_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n85_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n84_), .O(z05));
  xor2a  g040(.a(new_n87_), .b(x25), .O(new_n92_));
  inv1   g041(.a(x09), .O(new_n93_));
  aoi21  g042(.a(new_n52_), .b(new_n93_), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n52_), .c(new_n94_), .O(z06));
  inv1   g044(.a(x25), .O(new_n96_));
  nand3  g045(.a(new_n79_), .b(new_n96_), .c(new_n86_), .O(new_n97_));
  nor3   g046(.a(x26), .b(x25), .c(x24), .O(new_n98_));
  aoi22  g047(.a(new_n98_), .b(new_n79_), .c(new_n97_), .d(x26), .O(new_n99_));
  inv1   g048(.a(x08), .O(new_n100_));
  aoi21  g049(.a(new_n52_), .b(new_n100_), .c(x18), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n52_), .c(new_n101_), .O(z07));
  nand2  g051(.a(new_n98_), .b(new_n79_), .O(new_n103_));
  inv1   g052(.a(x26), .O(new_n104_));
  inv1   g053(.a(x27), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n104_), .c(new_n96_), .d(new_n86_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n78_), .O(new_n107_));
  aoi21  g056(.a(new_n103_), .b(x27), .c(new_n107_), .O(new_n108_));
  inv1   g057(.a(x07), .O(new_n109_));
  aoi21  g058(.a(new_n52_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n52_), .c(new_n110_), .O(z08));
  inv1   g060(.a(new_n107_), .O(new_n112_));
  nor2   g061(.a(x28), .b(x27), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n98_), .c(new_n77_), .d(new_n60_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  aoi21  g064(.a(new_n112_), .b(x28), .c(new_n115_), .O(new_n116_));
  inv1   g065(.a(x06), .O(new_n117_));
  aoi21  g066(.a(new_n52_), .b(new_n117_), .c(x18), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n52_), .c(new_n118_), .O(z09));
  nor3   g068(.a(x29), .b(x28), .c(x27), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n98_), .c(new_n77_), .d(new_n60_), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  aoi21  g071(.a(new_n114_), .b(x29), .c(new_n122_), .O(new_n123_));
  inv1   g072(.a(x05), .O(new_n124_));
  aoi21  g073(.a(new_n52_), .b(new_n124_), .c(x18), .O(new_n125_));
  oai21  g074(.a(new_n123_), .b(new_n52_), .c(new_n125_), .O(z10));
  inv1   g075(.a(x30), .O(new_n127_));
  nor2   g076(.a(x29), .b(x28), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n107_), .c(new_n127_), .O(new_n129_));
  nor3   g078(.a(x30), .b(x29), .c(x28), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n107_), .O(new_n131_));
  inv1   g080(.a(new_n131_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n129_), .c(x16), .O(new_n133_));
  inv1   g082(.a(x04), .O(new_n134_));
  aoi21  g083(.a(new_n52_), .b(new_n134_), .c(x18), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n133_), .O(z11));
  xor2a  g085(.a(new_n131_), .b(x31), .O(new_n137_));
  inv1   g086(.a(x03), .O(new_n138_));
  aoi21  g087(.a(new_n52_), .b(new_n138_), .c(x18), .O(new_n139_));
  oai21  g088(.a(new_n137_), .b(new_n52_), .c(new_n139_), .O(z12));
  inv1   g089(.a(x31), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n127_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n121_), .c(x32), .O(new_n143_));
  inv1   g092(.a(x29), .O(new_n144_));
  nor3   g093(.a(x32), .b(x31), .c(x30), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n115_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n143_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  inv1   g099(.a(x02), .O(new_n151_));
  aoi21  g100(.a(new_n52_), .b(new_n151_), .c(x18), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(z13));
  inv1   g102(.a(new_n145_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n121_), .c(x33), .O(new_n155_));
  nor3   g104(.a(x33), .b(x32), .c(x31), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n130_), .c(new_n107_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x16), .O(new_n159_));
  inv1   g108(.a(x01), .O(new_n160_));
  aoi21  g109(.a(new_n52_), .b(new_n160_), .c(x18), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n159_), .O(z14));
  nor2   g111(.a(new_n146_), .b(new_n114_), .O(new_n163_));
  nor2   g112(.a(x34), .b(x33), .O(new_n164_));
  aoi22  g113(.a(new_n164_), .b(new_n163_), .c(new_n157_), .d(x34), .O(new_n165_));
  inv1   g114(.a(x00), .O(new_n166_));
  aoi21  g115(.a(new_n52_), .b(new_n166_), .c(x18), .O(new_n167_));
  oai21  g116(.a(new_n165_), .b(new_n52_), .c(new_n167_), .O(z15));
endmodule


