// Benchmark "FAU" written by ABC on Mon Jul 27 22:44:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  xor2a  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g008(.a(x14), .O(new_n60_));
  aoi21  g009(.a(new_n52_), .b(new_n60_), .c(x18), .O(new_n61_));
  oai21  g010(.a(new_n59_), .b(new_n52_), .c(new_n61_), .O(z01));
  nand2  g011(.a(new_n58_), .b(new_n57_), .O(new_n63_));
  nor2   g012(.a(x21), .b(x20), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n58_), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  aoi21  g015(.a(new_n63_), .b(x21), .c(new_n66_), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n52_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n52_), .c(new_n69_), .O(z02));
  xor2a  g019(.a(new_n65_), .b(x22), .O(new_n71_));
  inv1   g020(.a(x12), .O(new_n72_));
  aoi21  g021(.a(new_n52_), .b(new_n72_), .c(x18), .O(new_n73_));
  oai21  g022(.a(new_n71_), .b(new_n52_), .c(new_n73_), .O(z03));
  inv1   g023(.a(x22), .O(new_n75_));
  nand2  g024(.a(new_n66_), .b(new_n75_), .O(new_n76_));
  nor2   g025(.a(x23), .b(x22), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n64_), .c(new_n58_), .O(new_n78_));
  inv1   g027(.a(new_n78_), .O(new_n79_));
  aoi21  g028(.a(new_n76_), .b(x23), .c(new_n79_), .O(new_n80_));
  inv1   g029(.a(x11), .O(new_n81_));
  aoi21  g030(.a(new_n52_), .b(new_n81_), .c(x18), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n52_), .c(new_n82_), .O(z04));
  inv1   g032(.a(x24), .O(new_n84_));
  nor2   g033(.a(new_n79_), .b(new_n84_), .O(new_n85_));
  nand4  g034(.a(new_n77_), .b(new_n64_), .c(new_n58_), .d(new_n84_), .O(new_n86_));
  inv1   g035(.a(new_n86_), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(new_n85_), .c(x16), .O(new_n88_));
  inv1   g037(.a(x10), .O(new_n89_));
  aoi21  g038(.a(new_n52_), .b(new_n89_), .c(x18), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n88_), .O(z05));
  inv1   g040(.a(x25), .O(new_n92_));
  nor2   g041(.a(new_n87_), .b(new_n92_), .O(new_n93_));
  nor2   g042(.a(new_n86_), .b(x25), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n93_), .c(x16), .O(new_n95_));
  inv1   g044(.a(x09), .O(new_n96_));
  aoi21  g045(.a(new_n52_), .b(new_n96_), .c(x18), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n95_), .O(z06));
  inv1   g047(.a(x26), .O(new_n99_));
  nor2   g048(.a(new_n94_), .b(new_n99_), .O(new_n100_));
  nand3  g049(.a(new_n99_), .b(new_n92_), .c(new_n84_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n78_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n100_), .c(x16), .O(new_n103_));
  inv1   g052(.a(x08), .O(new_n104_));
  aoi21  g053(.a(new_n52_), .b(new_n104_), .c(x18), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n103_), .O(z07));
  inv1   g055(.a(x27), .O(new_n107_));
  xor2a  g056(.a(new_n102_), .b(new_n107_), .O(new_n108_));
  inv1   g057(.a(x07), .O(new_n109_));
  aoi21  g058(.a(new_n52_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n52_), .c(new_n110_), .O(z08));
  inv1   g060(.a(x28), .O(new_n112_));
  aoi21  g061(.a(new_n102_), .b(new_n107_), .c(new_n112_), .O(new_n113_));
  nor2   g062(.a(x28), .b(x27), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  nor3   g064(.a(new_n115_), .b(new_n101_), .c(new_n78_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n113_), .c(x16), .O(new_n117_));
  inv1   g066(.a(x06), .O(new_n118_));
  aoi21  g067(.a(new_n52_), .b(new_n118_), .c(x18), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n117_), .O(z09));
  inv1   g069(.a(x29), .O(new_n121_));
  xor2a  g070(.a(new_n116_), .b(new_n121_), .O(new_n122_));
  inv1   g071(.a(x05), .O(new_n123_));
  aoi21  g072(.a(new_n52_), .b(new_n123_), .c(x18), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n52_), .c(new_n124_), .O(z10));
  inv1   g074(.a(new_n101_), .O(new_n126_));
  nand4  g075(.a(new_n114_), .b(new_n126_), .c(new_n79_), .d(new_n121_), .O(new_n127_));
  nor2   g076(.a(x30), .b(x29), .O(new_n128_));
  aoi22  g077(.a(new_n128_), .b(new_n116_), .c(new_n127_), .d(x30), .O(new_n129_));
  inv1   g078(.a(x04), .O(new_n130_));
  aoi21  g079(.a(new_n52_), .b(new_n130_), .c(x18), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n52_), .c(new_n131_), .O(z11));
  nand3  g081(.a(new_n128_), .b(new_n114_), .c(new_n102_), .O(new_n133_));
  inv1   g082(.a(x31), .O(new_n134_));
  nand3  g083(.a(new_n128_), .b(new_n114_), .c(new_n134_), .O(new_n135_));
  nor3   g084(.a(new_n135_), .b(new_n101_), .c(new_n78_), .O(new_n136_));
  aoi21  g085(.a(new_n133_), .b(x31), .c(new_n136_), .O(new_n137_));
  inv1   g086(.a(x03), .O(new_n138_));
  aoi21  g087(.a(new_n52_), .b(new_n138_), .c(x18), .O(new_n139_));
  oai21  g088(.a(new_n137_), .b(new_n52_), .c(new_n139_), .O(z12));
  inv1   g089(.a(new_n135_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n102_), .O(new_n142_));
  nand2  g091(.a(new_n128_), .b(new_n114_), .O(new_n143_));
  inv1   g092(.a(x32), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n134_), .c(new_n99_), .d(new_n92_), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  aoi22  g095(.a(new_n146_), .b(new_n87_), .c(new_n142_), .d(x32), .O(new_n147_));
  inv1   g096(.a(x02), .O(new_n148_));
  aoi21  g097(.a(new_n52_), .b(new_n148_), .c(x18), .O(new_n149_));
  oai21  g098(.a(new_n147_), .b(new_n52_), .c(new_n149_), .O(z13));
  nand2  g099(.a(new_n146_), .b(new_n87_), .O(new_n151_));
  nor2   g100(.a(new_n79_), .b(x33), .O(new_n152_));
  and2   g101(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  oai21  g102(.a(new_n152_), .b(new_n151_), .c(x16), .O(new_n154_));
  inv1   g103(.a(x01), .O(new_n155_));
  aoi21  g104(.a(new_n52_), .b(new_n155_), .c(x18), .O(new_n156_));
  oai21  g105(.a(new_n154_), .b(new_n153_), .c(new_n156_), .O(z14));
  nand2  g106(.a(x34), .b(x16), .O(new_n158_));
  inv1   g107(.a(x00), .O(new_n159_));
  aoi21  g108(.a(new_n52_), .b(new_n159_), .c(x18), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n158_), .O(z15));
endmodule


