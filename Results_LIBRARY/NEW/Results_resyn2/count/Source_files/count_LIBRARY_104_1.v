// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_;
  inv1   g000(.a(x16), .O(new_n52_));
  inv1   g001(.a(x15), .O(new_n53_));
  aoi21  g002(.a(new_n52_), .b(new_n53_), .c(x18), .O(new_n54_));
  oai21  g003(.a(x17), .b(new_n52_), .c(new_n54_), .O(z00));
  xor2a  g004(.a(x20), .b(x17), .O(new_n56_));
  inv1   g005(.a(x14), .O(new_n57_));
  aoi21  g006(.a(new_n52_), .b(new_n57_), .c(x18), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n52_), .c(new_n58_), .O(z01));
  inv1   g008(.a(x21), .O(new_n60_));
  nor2   g009(.a(x20), .b(x17), .O(new_n61_));
  nor2   g010(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nor3   g011(.a(x21), .b(x20), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x13), .O(new_n65_));
  aoi21  g014(.a(new_n52_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z02));
  nand2  g016(.a(new_n61_), .b(new_n60_), .O(new_n68_));
  nor2   g017(.a(x22), .b(x21), .O(new_n69_));
  aoi22  g018(.a(new_n69_), .b(new_n61_), .c(new_n68_), .d(x22), .O(new_n70_));
  inv1   g019(.a(x12), .O(new_n71_));
  aoi21  g020(.a(new_n52_), .b(new_n71_), .c(x18), .O(new_n72_));
  oai21  g021(.a(new_n70_), .b(new_n52_), .c(new_n72_), .O(z03));
  inv1   g022(.a(x23), .O(new_n74_));
  aoi21  g023(.a(new_n69_), .b(new_n61_), .c(new_n74_), .O(new_n75_));
  nand3  g024(.a(new_n69_), .b(new_n61_), .c(new_n74_), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(new_n75_), .c(x16), .O(new_n78_));
  inv1   g027(.a(x11), .O(new_n79_));
  aoi21  g028(.a(new_n52_), .b(new_n79_), .c(x18), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n78_), .O(z04));
  nor2   g030(.a(x24), .b(x23), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n69_), .c(new_n61_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi21  g033(.a(new_n76_), .b(x24), .c(new_n84_), .O(new_n85_));
  inv1   g034(.a(x10), .O(new_n86_));
  aoi21  g035(.a(new_n52_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n52_), .c(new_n87_), .O(z05));
  nor2   g037(.a(x25), .b(x22), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n82_), .c(new_n61_), .d(new_n60_), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  aoi21  g040(.a(new_n83_), .b(x25), .c(new_n91_), .O(new_n92_));
  inv1   g041(.a(x09), .O(new_n93_));
  aoi21  g042(.a(new_n52_), .b(new_n93_), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n52_), .c(new_n94_), .O(z06));
  nor2   g044(.a(x26), .b(x25), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n82_), .c(new_n69_), .d(new_n61_), .O(new_n97_));
  inv1   g046(.a(new_n97_), .O(new_n98_));
  aoi21  g047(.a(new_n90_), .b(x26), .c(new_n98_), .O(new_n99_));
  inv1   g048(.a(x08), .O(new_n100_));
  aoi21  g049(.a(new_n52_), .b(new_n100_), .c(x18), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n52_), .c(new_n101_), .O(z07));
  nand2  g051(.a(new_n97_), .b(x27), .O(new_n103_));
  inv1   g052(.a(x27), .O(new_n104_));
  nand3  g053(.a(new_n96_), .b(new_n84_), .c(new_n104_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(x16), .O(new_n107_));
  inv1   g056(.a(x07), .O(new_n108_));
  aoi21  g057(.a(new_n52_), .b(new_n108_), .c(x18), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n107_), .O(z08));
  inv1   g059(.a(x26), .O(new_n111_));
  nor2   g060(.a(x28), .b(x27), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n90_), .O(new_n114_));
  aoi21  g063(.a(new_n105_), .b(x28), .c(new_n114_), .O(new_n115_));
  inv1   g064(.a(x06), .O(new_n116_));
  aoi21  g065(.a(new_n52_), .b(new_n116_), .c(x18), .O(new_n117_));
  oai21  g066(.a(new_n115_), .b(new_n52_), .c(new_n117_), .O(z09));
  nand2  g067(.a(new_n89_), .b(new_n82_), .O(new_n119_));
  inv1   g068(.a(new_n119_), .O(new_n120_));
  nor3   g069(.a(x28), .b(x27), .c(x26), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n120_), .c(new_n63_), .O(new_n122_));
  inv1   g071(.a(x24), .O(new_n123_));
  inv1   g072(.a(x25), .O(new_n124_));
  nor2   g073(.a(x27), .b(x26), .O(new_n125_));
  nor2   g074(.a(x29), .b(x28), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n123_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n76_), .O(new_n128_));
  aoi21  g077(.a(new_n122_), .b(x29), .c(new_n128_), .O(new_n129_));
  inv1   g078(.a(x05), .O(new_n130_));
  aoi21  g079(.a(new_n52_), .b(new_n130_), .c(x18), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n52_), .c(new_n131_), .O(z10));
  oai21  g081(.a(new_n127_), .b(new_n76_), .c(x30), .O(new_n133_));
  nor3   g082(.a(x27), .b(x26), .c(x25), .O(new_n134_));
  nor3   g083(.a(x30), .b(x29), .c(x28), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n134_), .c(new_n84_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  inv1   g087(.a(x04), .O(new_n139_));
  aoi21  g088(.a(new_n52_), .b(new_n139_), .c(x18), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n138_), .O(z11));
  inv1   g090(.a(x30), .O(new_n142_));
  nand4  g091(.a(new_n126_), .b(new_n125_), .c(new_n142_), .d(new_n124_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n83_), .c(x31), .O(new_n144_));
  nor2   g093(.a(x31), .b(x30), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n126_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n134_), .c(new_n84_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  inv1   g099(.a(x03), .O(new_n151_));
  aoi21  g100(.a(new_n52_), .b(new_n151_), .c(x18), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(z12));
  nand4  g102(.a(new_n145_), .b(new_n126_), .c(new_n125_), .d(new_n124_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n83_), .c(x32), .O(new_n155_));
  inv1   g104(.a(x29), .O(new_n156_));
  inv1   g105(.a(x31), .O(new_n157_));
  inv1   g106(.a(x32), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n157_), .c(new_n142_), .d(new_n156_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n121_), .c(new_n120_), .d(new_n63_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n155_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(x16), .O(new_n163_));
  inv1   g112(.a(x02), .O(new_n164_));
  aoi21  g113(.a(new_n52_), .b(new_n164_), .c(x18), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n163_), .O(z13));
  nor2   g115(.a(x32), .b(x29), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n145_), .c(new_n112_), .d(new_n111_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n90_), .c(x33), .O(new_n169_));
  nor2   g118(.a(x33), .b(x32), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n125_), .O(new_n171_));
  inv1   g120(.a(new_n171_), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n147_), .c(new_n120_), .d(new_n63_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n169_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x16), .O(new_n175_));
  inv1   g124(.a(x01), .O(new_n176_));
  aoi21  g125(.a(new_n52_), .b(new_n176_), .c(x18), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n175_), .O(z14));
  nand4  g127(.a(new_n170_), .b(new_n145_), .c(new_n126_), .d(new_n125_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n90_), .c(x34), .O(new_n180_));
  inv1   g129(.a(x28), .O(new_n181_));
  inv1   g130(.a(x33), .O(new_n182_));
  inv1   g131(.a(x34), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n182_), .c(new_n181_), .d(new_n104_), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n159_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n98_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n180_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(x16), .O(new_n188_));
  inv1   g137(.a(x00), .O(new_n189_));
  aoi21  g138(.a(new_n52_), .b(new_n189_), .c(x18), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n188_), .O(z15));
endmodule


