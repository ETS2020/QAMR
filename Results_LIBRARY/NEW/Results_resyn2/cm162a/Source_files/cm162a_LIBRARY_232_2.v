// Benchmark "FAU" written by ABC on Mon Jul 27 20:49:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  inv1   g04(.a(new_n23_), .O(new_n24_));
  oai21  g05(.a(new_n22_), .b(new_n20_), .c(new_n24_), .O(new_n25_));
  nand3  g06(.a(new_n23_), .b(x08), .c(x03), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x03), .O(new_n28_));
  inv1   g09(.a(x05), .O(new_n29_));
  aoi21  g10(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand3  g11(.a(new_n30_), .b(new_n26_), .c(new_n25_), .O(z0));
  inv1   g12(.a(x10), .O(new_n32_));
  nor2   g13(.a(new_n32_), .b(new_n28_), .O(new_n33_));
  oai21  g14(.a(new_n23_), .b(x08), .c(new_n33_), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n22_), .c(new_n24_), .O(new_n36_));
  inv1   g17(.a(x01), .O(new_n37_));
  aoi21  g18(.a(new_n28_), .b(new_n37_), .c(new_n29_), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n36_), .c(new_n34_), .O(z1));
  inv1   g20(.a(x11), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(new_n32_), .c(new_n20_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n21_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n24_), .O(new_n43_));
  nand3  g24(.a(new_n35_), .b(x04), .c(x02), .O(new_n44_));
  nor2   g25(.a(new_n40_), .b(new_n28_), .O(new_n45_));
  oai21  g26(.a(x06), .b(x03), .c(x05), .O(new_n46_));
  aoi21  g27(.a(new_n45_), .b(new_n44_), .c(new_n46_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n43_), .O(z2));
  nand2  g29(.a(new_n21_), .b(x12), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n42_), .c(new_n24_), .O(new_n50_));
  and2   g31(.a(x12), .b(x03), .O(new_n51_));
  oai21  g32(.a(new_n41_), .b(new_n23_), .c(new_n51_), .O(new_n52_));
  inv1   g33(.a(x07), .O(new_n53_));
  aoi21  g34(.a(new_n53_), .b(new_n28_), .c(new_n29_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n52_), .c(new_n50_), .O(z3));
  nand3  g36(.a(x13), .b(x09), .c(x04), .O(new_n56_));
  inv1   g37(.a(new_n56_), .O(z4));
endmodule


