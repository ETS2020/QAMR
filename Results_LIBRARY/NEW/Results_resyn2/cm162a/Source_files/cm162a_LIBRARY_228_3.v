// Benchmark "FAU" written by ABC on Mon Jul 27 20:49:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  inv1   g02(.a(x00), .O(new_n22_));
  inv1   g03(.a(x05), .O(new_n23_));
  aoi21  g04(.a(new_n20_), .b(new_n22_), .c(new_n23_), .O(new_n24_));
  oai21  g05(.a(new_n21_), .b(new_n20_), .c(new_n24_), .O(z0));
  nor2   g06(.a(new_n21_), .b(x10), .O(new_n26_));
  and2   g07(.a(x04), .b(x02), .O(new_n27_));
  nand2  g08(.a(x13), .b(x09), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  aoi21  g10(.a(new_n29_), .b(x10), .c(new_n26_), .O(new_n30_));
  inv1   g11(.a(x01), .O(new_n31_));
  aoi21  g12(.a(new_n20_), .b(new_n31_), .c(new_n23_), .O(new_n32_));
  oai21  g13(.a(new_n30_), .b(new_n20_), .c(new_n32_), .O(z1));
  inv1   g14(.a(x10), .O(new_n34_));
  inv1   g15(.a(x11), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n28_), .c(new_n21_), .O(new_n37_));
  aoi21  g18(.a(new_n27_), .b(new_n34_), .c(new_n35_), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n37_), .c(x03), .O(new_n39_));
  inv1   g20(.a(x06), .O(new_n40_));
  aoi21  g21(.a(new_n40_), .b(new_n20_), .c(new_n23_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n39_), .O(z2));
  nand2  g23(.a(new_n36_), .b(new_n28_), .O(new_n43_));
  aoi21  g24(.a(new_n43_), .b(new_n27_), .c(x12), .O(new_n44_));
  nand3  g25(.a(x12), .b(new_n35_), .c(new_n34_), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n29_), .c(x03), .O(new_n46_));
  inv1   g27(.a(x07), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n20_), .c(new_n23_), .O(new_n48_));
  oai21  g29(.a(new_n46_), .b(new_n44_), .c(new_n48_), .O(z3));
  nand3  g30(.a(x13), .b(x09), .c(x04), .O(new_n50_));
  inv1   g31(.a(new_n50_), .O(z4));
endmodule


