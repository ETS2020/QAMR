// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_;
  nand2  g000(.a(x24), .b(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  nand2  g003(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x05), .O(new_n38_));
  nor2   g005(.a(x10), .b(x02), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n35_), .c(x00), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  inv1   g009(.a(new_n42_), .O(new_n43_));
  aoi21  g010(.a(new_n41_), .b(x19), .c(new_n43_), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n40_), .c(new_n38_), .O(z5));
  inv1   g012(.a(x11), .O(new_n46_));
  inv1   g013(.a(x14), .O(new_n47_));
  inv1   g014(.a(x20), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g016(.a(x06), .O(new_n50_));
  nor2   g017(.a(x03), .b(x01), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  and2   g020(.a(new_n53_), .b(z5), .O(z0));
  inv1   g021(.a(x08), .O(new_n55_));
  nand2  g022(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  nor3   g024(.a(x17), .b(x16), .c(x12), .O(new_n58_));
  nor2   g025(.a(x23), .b(x22), .O(new_n59_));
  nor2   g026(.a(x09), .b(x04), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n59_), .c(new_n51_), .O(new_n61_));
  inv1   g028(.a(new_n61_), .O(new_n62_));
  nand2  g029(.a(new_n39_), .b(x00), .O(new_n63_));
  nand3  g030(.a(x19), .b(x13), .c(x05), .O(new_n64_));
  nand2  g031(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n62_), .c(new_n58_), .d(new_n57_), .O(new_n66_));
  nand4  g033(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n67_));
  nand2  g034(.a(x09), .b(x04), .O(new_n68_));
  nor2   g035(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g036(.a(x15), .b(x13), .c(x05), .O(new_n70_));
  oai21  g037(.a(x10), .b(x02), .c(new_n70_), .O(new_n71_));
  nand4  g038(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n72_));
  nand4  g039(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n73_));
  nor2   g040(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g041(.a(new_n74_), .b(new_n71_), .c(new_n69_), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n66_), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n35_), .O(new_n77_));
  nor2   g044(.a(new_n41_), .b(new_n35_), .O(new_n78_));
  nand4  g045(.a(new_n62_), .b(new_n58_), .c(new_n57_), .d(x07), .O(new_n79_));
  nor2   g046(.a(new_n72_), .b(new_n47_), .O(new_n80_));
  nand3  g047(.a(x18), .b(x16), .c(x12), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n80_), .c(new_n69_), .d(x11), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n78_), .O(new_n85_));
  nand3  g052(.a(x16), .b(x15), .c(x12), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n80_), .c(new_n69_), .d(x11), .O(new_n88_));
  nand4  g055(.a(new_n62_), .b(new_n58_), .c(new_n57_), .d(x19), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n41_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n85_), .c(new_n77_), .O(z1));
  inv1   g059(.a(x18), .O(new_n93_));
  nand3  g060(.a(new_n35_), .b(x15), .c(x13), .O(new_n94_));
  oai21  g061(.a(new_n35_), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(x05), .O(new_n96_));
  nand3  g063(.a(new_n39_), .b(new_n35_), .c(x21), .O(new_n97_));
  nand3  g064(.a(x24), .b(x18), .c(x13), .O(new_n98_));
  nand2  g065(.a(new_n41_), .b(x15), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n96_), .O(z7));
  nand3  g069(.a(x06), .b(x03), .c(x01), .O(new_n103_));
  nand3  g070(.a(x20), .b(x14), .c(x11), .O(new_n104_));
  nor2   g071(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  and2   g072(.a(new_n105_), .b(z7), .O(z2));
  inv1   g073(.a(new_n67_), .O(new_n107_));
  inv1   g074(.a(new_n104_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n93_), .O(new_n110_));
  nor3   g077(.a(x20), .b(x14), .c(x11), .O(new_n111_));
  nor2   g078(.a(x08), .b(x06), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g080(.a(new_n51_), .b(x07), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n110_), .c(new_n78_), .O(new_n116_));
  inv1   g083(.a(x15), .O(new_n117_));
  nor2   g084(.a(new_n109_), .b(new_n117_), .O(new_n118_));
  nand2  g085(.a(new_n51_), .b(x19), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n113_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n118_), .c(new_n41_), .O(new_n121_));
  nand2  g088(.a(new_n39_), .b(x21), .O(new_n122_));
  aoi21  g089(.a(new_n122_), .b(new_n70_), .c(new_n109_), .O(new_n123_));
  nand3  g090(.a(new_n112_), .b(new_n51_), .c(new_n111_), .O(new_n124_));
  aoi21  g091(.a(new_n64_), .b(new_n63_), .c(new_n124_), .O(new_n125_));
  oai21  g092(.a(new_n125_), .b(new_n123_), .c(new_n35_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(new_n121_), .c(new_n116_), .O(z3));
  inv1   g094(.a(x16), .O(new_n128_));
  inv1   g095(.a(x09), .O(new_n129_));
  inv1   g096(.a(x23), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(x04), .c(x17), .O(new_n131_));
  aoi21  g098(.a(new_n131_), .b(x22), .c(new_n129_), .O(new_n132_));
  oai21  g099(.a(new_n132_), .b(new_n128_), .c(x08), .O(new_n133_));
  inv1   g100(.a(x05), .O(new_n134_));
  inv1   g101(.a(x13), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g103(.a(new_n35_), .b(x13), .c(x05), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(x15), .O(new_n139_));
  nand2  g106(.a(new_n78_), .b(x18), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n139_), .c(new_n97_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n133_), .O(new_n142_));
  aoi21  g109(.a(new_n59_), .b(x04), .c(x09), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(x16), .c(new_n55_), .O(new_n144_));
  inv1   g111(.a(x22), .O(new_n145_));
  inv1   g112(.a(x17), .O(new_n146_));
  nor2   g113(.a(new_n146_), .b(x16), .O(new_n147_));
  nand4  g114(.a(new_n147_), .b(new_n78_), .c(new_n145_), .d(x07), .O(new_n148_));
  inv1   g115(.a(x19), .O(new_n149_));
  nor2   g116(.a(x22), .b(new_n149_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n147_), .c(new_n138_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n148_), .O(new_n152_));
  aoi21  g119(.a(new_n144_), .b(z5), .c(new_n152_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n142_), .O(z4));
  nand2  g121(.a(x20), .b(new_n47_), .O(new_n155_));
  aoi21  g122(.a(new_n155_), .b(new_n50_), .c(x11), .O(new_n156_));
  oai21  g123(.a(new_n156_), .b(x03), .c(z5), .O(new_n157_));
  aoi21  g124(.a(new_n48_), .b(x14), .c(new_n50_), .O(new_n158_));
  oai21  g125(.a(new_n158_), .b(new_n46_), .c(x03), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(z7), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n157_), .O(z6));
endmodule


