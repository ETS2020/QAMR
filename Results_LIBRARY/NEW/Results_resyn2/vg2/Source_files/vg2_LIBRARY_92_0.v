// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand4  g007(.a(new_n36_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  inv1   g008(.a(x13), .O(new_n42_));
  inv1   g009(.a(x19), .O(new_n43_));
  nor2   g010(.a(new_n43_), .b(x05), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  inv1   g012(.a(new_n35_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(x13), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n45_), .c(new_n41_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n38_), .O(new_n49_));
  inv1   g016(.a(x11), .O(new_n50_));
  inv1   g017(.a(x14), .O(new_n51_));
  inv1   g018(.a(x20), .O(new_n52_));
  nor3   g019(.a(x06), .b(x03), .c(x01), .O(new_n53_));
  nand4  g020(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n49_), .O(z0));
  nand3  g022(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n56_));
  nand2  g023(.a(x19), .b(x05), .O(new_n57_));
  nand2  g024(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  inv1   g025(.a(x16), .O(new_n59_));
  inv1   g026(.a(x17), .O(new_n60_));
  inv1   g027(.a(x22), .O(new_n61_));
  inv1   g028(.a(x23), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n63_));
  inv1   g030(.a(x04), .O(new_n64_));
  inv1   g031(.a(x09), .O(new_n65_));
  inv1   g032(.a(x12), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n50_), .c(new_n65_), .d(new_n64_), .O(new_n67_));
  nor2   g034(.a(new_n67_), .b(new_n63_), .O(new_n68_));
  inv1   g035(.a(x01), .O(new_n69_));
  inv1   g036(.a(x03), .O(new_n70_));
  inv1   g037(.a(x06), .O(new_n71_));
  nand3  g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  inv1   g039(.a(x08), .O(new_n73_));
  nand3  g040(.a(new_n52_), .b(new_n51_), .c(new_n73_), .O(new_n74_));
  nor2   g041(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand3  g042(.a(new_n75_), .b(new_n68_), .c(new_n58_), .O(new_n76_));
  nand3  g043(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n77_));
  nand3  g044(.a(x15), .b(x13), .c(x05), .O(new_n78_));
  aoi21  g045(.a(new_n78_), .b(new_n77_), .c(new_n73_), .O(new_n79_));
  nand2  g046(.a(x11), .b(x06), .O(new_n80_));
  nand4  g047(.a(x09), .b(x04), .c(x03), .d(x01), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g049(.a(x23), .b(x22), .c(x20), .O(new_n83_));
  nand4  g050(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g052(.a(new_n85_), .b(new_n82_), .c(new_n79_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n76_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n36_), .O(new_n88_));
  nor2   g055(.a(x17), .b(x16), .O(new_n89_));
  nor2   g056(.a(x23), .b(x22), .O(new_n90_));
  nor2   g057(.a(x09), .b(x04), .O(new_n91_));
  nor2   g058(.a(x12), .b(x11), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n89_), .O(new_n93_));
  nor3   g060(.a(x20), .b(x14), .c(x08), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n53_), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  and2   g063(.a(new_n85_), .b(new_n82_), .O(new_n97_));
  nand2  g064(.a(x24), .b(x18), .O(new_n98_));
  inv1   g065(.a(new_n98_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(x13), .O(new_n100_));
  nand2  g067(.a(new_n98_), .b(x05), .O(new_n101_));
  inv1   g068(.a(x15), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n34_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n101_), .c(new_n42_), .O(new_n104_));
  oai21  g071(.a(new_n100_), .b(new_n73_), .c(new_n104_), .O(new_n105_));
  aoi22  g072(.a(new_n105_), .b(new_n97_), .c(new_n96_), .d(new_n46_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n88_), .O(z1));
  nand4  g074(.a(x20), .b(x14), .c(x03), .d(x01), .O(new_n108_));
  nor2   g075(.a(new_n108_), .b(new_n80_), .O(new_n109_));
  inv1   g076(.a(new_n109_), .O(new_n110_));
  nand2  g077(.a(x15), .b(x05), .O(new_n111_));
  oai21  g078(.a(new_n111_), .b(x24), .c(new_n98_), .O(new_n112_));
  nand2  g079(.a(new_n42_), .b(x08), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g081(.a(new_n36_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n115_));
  and2   g082(.a(new_n115_), .b(new_n104_), .O(new_n116_));
  aoi21  g083(.a(new_n116_), .b(new_n114_), .c(new_n110_), .O(z2));
  nand2  g084(.a(new_n109_), .b(new_n79_), .O(new_n118_));
  nand4  g085(.a(new_n50_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n119_));
  nor2   g086(.a(new_n74_), .b(new_n119_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n58_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n36_), .O(new_n123_));
  aoi22  g090(.a(new_n120_), .b(new_n46_), .c(new_n109_), .d(new_n105_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n123_), .O(z3));
  oai21  g092(.a(new_n62_), .b(x04), .c(x17), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(x22), .O(new_n127_));
  aoi21  g094(.a(new_n127_), .b(x09), .c(new_n59_), .O(new_n128_));
  nand3  g095(.a(new_n113_), .b(x15), .c(x05), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(new_n77_), .c(x24), .O(new_n130_));
  oai21  g097(.a(new_n128_), .b(new_n73_), .c(new_n130_), .O(new_n131_));
  oai21  g098(.a(new_n57_), .b(x24), .c(new_n35_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(x13), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n41_), .O(new_n134_));
  oai21  g101(.a(x23), .b(new_n64_), .c(new_n60_), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n61_), .c(x09), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(x16), .c(new_n73_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nor2   g105(.a(x13), .b(x05), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(x15), .O(new_n140_));
  oai21  g107(.a(new_n139_), .b(new_n98_), .c(new_n140_), .O(new_n141_));
  nand3  g108(.a(x24), .b(x07), .c(x05), .O(new_n142_));
  inv1   g109(.a(new_n142_), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(new_n44_), .c(new_n42_), .O(new_n144_));
  nand2  g111(.a(new_n99_), .b(new_n73_), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  aoi21  g113(.a(new_n141_), .b(new_n128_), .c(new_n146_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n138_), .c(new_n131_), .O(z4));
  inv1   g115(.a(new_n48_), .O(new_n149_));
  nand2  g116(.a(new_n37_), .b(x13), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n38_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n149_), .O(z5));
  nand2  g119(.a(x20), .b(new_n51_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n71_), .O(new_n154_));
  aoi21  g121(.a(new_n154_), .b(new_n50_), .c(x03), .O(new_n155_));
  nand2  g122(.a(new_n52_), .b(x14), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(x06), .c(new_n50_), .O(new_n157_));
  nand3  g124(.a(new_n36_), .b(x15), .c(x13), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(new_n98_), .c(new_n34_), .O(new_n159_));
  nand3  g126(.a(new_n140_), .b(new_n115_), .c(new_n100_), .O(new_n160_));
  oai22  g127(.a(new_n160_), .b(new_n159_), .c(new_n157_), .d(new_n70_), .O(new_n161_));
  oai21  g128(.a(new_n155_), .b(new_n49_), .c(new_n161_), .O(z6));
  or2    g129(.a(new_n160_), .b(new_n159_), .O(z7));
endmodule


