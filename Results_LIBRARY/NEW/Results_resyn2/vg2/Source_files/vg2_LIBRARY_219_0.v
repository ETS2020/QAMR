// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nor2   g002(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g003(.a(new_n35_), .b(x19), .c(x13), .O(new_n37_));
  inv1   g004(.a(new_n37_), .O(new_n38_));
  oai21  g005(.a(new_n38_), .b(new_n36_), .c(x05), .O(new_n39_));
  nor2   g006(.a(x10), .b(x02), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(new_n35_), .c(x00), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  nand2  g010(.a(new_n36_), .b(x13), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g012(.a(new_n45_), .O(new_n46_));
  inv1   g013(.a(x14), .O(new_n47_));
  inv1   g014(.a(x20), .O(new_n48_));
  inv1   g015(.a(x11), .O(new_n49_));
  nor3   g016(.a(x06), .b(x03), .c(x01), .O(new_n50_));
  nand2  g017(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g018(.a(new_n51_), .O(new_n52_));
  nand3  g019(.a(new_n52_), .b(new_n48_), .c(new_n47_), .O(new_n53_));
  aoi21  g020(.a(new_n46_), .b(new_n39_), .c(new_n53_), .O(z0));
  nand2  g021(.a(new_n40_), .b(x21), .O(new_n55_));
  nand2  g022(.a(x13), .b(x05), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand2  g024(.a(new_n57_), .b(x15), .O(new_n58_));
  nand2  g025(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  nand3  g027(.a(x06), .b(x03), .c(x01), .O(new_n61_));
  inv1   g028(.a(new_n61_), .O(new_n62_));
  nand2  g029(.a(new_n62_), .b(x08), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  nand3  g031(.a(x20), .b(x14), .c(x11), .O(new_n65_));
  inv1   g032(.a(new_n65_), .O(new_n66_));
  nand3  g033(.a(x23), .b(x22), .c(x17), .O(new_n67_));
  inv1   g034(.a(new_n67_), .O(new_n68_));
  nand4  g035(.a(x16), .b(x12), .c(x09), .d(x04), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n68_), .c(new_n66_), .d(new_n64_), .O(new_n71_));
  nand2  g038(.a(new_n40_), .b(x00), .O(new_n72_));
  inv1   g039(.a(new_n72_), .O(new_n73_));
  nor2   g040(.a(new_n57_), .b(new_n73_), .O(new_n74_));
  inv1   g041(.a(x16), .O(new_n75_));
  inv1   g042(.a(x17), .O(new_n76_));
  inv1   g043(.a(x22), .O(new_n77_));
  inv1   g044(.a(x23), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  inv1   g046(.a(x04), .O(new_n80_));
  inv1   g047(.a(x09), .O(new_n81_));
  inv1   g048(.a(x12), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n49_), .c(new_n81_), .d(new_n80_), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  nor3   g051(.a(x20), .b(x14), .c(x08), .O(new_n85_));
  nand3  g052(.a(new_n85_), .b(new_n84_), .c(new_n50_), .O(new_n86_));
  oai22  g053(.a(new_n86_), .b(new_n74_), .c(new_n71_), .d(new_n60_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n35_), .O(new_n88_));
  inv1   g055(.a(new_n71_), .O(new_n89_));
  inv1   g056(.a(x18), .O(new_n90_));
  nand2  g057(.a(new_n42_), .b(x15), .O(new_n91_));
  oai21  g058(.a(x13), .b(x05), .c(x24), .O(new_n92_));
  oai21  g059(.a(new_n92_), .b(new_n90_), .c(new_n91_), .O(new_n93_));
  inv1   g060(.a(new_n36_), .O(new_n94_));
  inv1   g061(.a(new_n42_), .O(new_n95_));
  aoi21  g062(.a(new_n95_), .b(new_n94_), .c(new_n86_), .O(new_n96_));
  aoi21  g063(.a(new_n93_), .b(new_n89_), .c(new_n96_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n88_), .O(z1));
  nand3  g065(.a(new_n35_), .b(x15), .c(x13), .O(new_n99_));
  oai21  g066(.a(new_n35_), .b(new_n90_), .c(new_n99_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(x05), .O(new_n101_));
  nand3  g068(.a(new_n40_), .b(new_n35_), .c(x21), .O(new_n102_));
  nand3  g069(.a(x24), .b(x18), .c(x13), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n102_), .c(new_n91_), .O(new_n104_));
  inv1   g071(.a(new_n104_), .O(new_n105_));
  nand2  g072(.a(new_n66_), .b(new_n62_), .O(new_n106_));
  aoi21  g073(.a(new_n105_), .b(new_n101_), .c(new_n106_), .O(z2));
  inv1   g074(.a(new_n92_), .O(new_n108_));
  nand2  g075(.a(new_n85_), .b(new_n52_), .O(new_n109_));
  nand3  g076(.a(new_n66_), .b(new_n64_), .c(x18), .O(new_n110_));
  oai21  g077(.a(new_n109_), .b(new_n34_), .c(new_n110_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(new_n108_), .O(new_n112_));
  inv1   g079(.a(x19), .O(new_n113_));
  nand3  g080(.a(new_n66_), .b(new_n64_), .c(x15), .O(new_n114_));
  oai21  g081(.a(new_n109_), .b(new_n113_), .c(new_n114_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n42_), .O(new_n116_));
  aoi21  g083(.a(new_n57_), .b(x19), .c(new_n73_), .O(new_n117_));
  nand3  g084(.a(new_n66_), .b(new_n64_), .c(new_n59_), .O(new_n118_));
  oai21  g085(.a(new_n117_), .b(new_n109_), .c(new_n118_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n35_), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(new_n116_), .c(new_n112_), .O(z3));
  inv1   g088(.a(x21), .O(new_n122_));
  inv1   g089(.a(x08), .O(new_n123_));
  aoi21  g090(.a(x23), .b(new_n80_), .c(new_n76_), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(new_n77_), .c(x09), .O(new_n125_));
  aoi21  g092(.a(new_n125_), .b(x16), .c(new_n123_), .O(new_n126_));
  nand2  g093(.a(x08), .b(x00), .O(new_n127_));
  oai21  g094(.a(new_n126_), .b(new_n122_), .c(new_n127_), .O(new_n128_));
  nor3   g095(.a(x24), .b(x10), .c(x02), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g097(.a(x08), .b(x07), .O(new_n131_));
  oai21  g098(.a(new_n126_), .b(new_n90_), .c(new_n131_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n108_), .O(new_n133_));
  oai21  g100(.a(new_n56_), .b(x24), .c(new_n95_), .O(new_n134_));
  aoi21  g101(.a(new_n124_), .b(x09), .c(new_n75_), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(new_n123_), .c(x15), .O(new_n136_));
  nor2   g103(.a(new_n113_), .b(new_n123_), .O(new_n137_));
  aoi21  g104(.a(new_n78_), .b(x04), .c(x17), .O(new_n138_));
  nand2  g105(.a(x19), .b(x09), .O(new_n139_));
  oai21  g106(.a(new_n138_), .b(x22), .c(new_n139_), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(new_n75_), .c(new_n137_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n136_), .O(new_n142_));
  inv1   g109(.a(new_n41_), .O(new_n143_));
  nand2  g110(.a(new_n78_), .b(x04), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(new_n76_), .c(x22), .O(new_n145_));
  oai21  g112(.a(new_n143_), .b(new_n36_), .c(new_n145_), .O(new_n146_));
  nor2   g113(.a(new_n92_), .b(new_n34_), .O(new_n147_));
  oai21  g114(.a(new_n147_), .b(new_n143_), .c(x09), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(new_n146_), .c(x16), .O(new_n149_));
  aoi21  g116(.a(new_n142_), .b(new_n134_), .c(new_n149_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n133_), .c(new_n130_), .O(z4));
  nand2  g118(.a(new_n46_), .b(new_n39_), .O(z5));
  inv1   g119(.a(x06), .O(new_n153_));
  nand2  g120(.a(x20), .b(new_n47_), .O(new_n154_));
  aoi21  g121(.a(new_n154_), .b(new_n153_), .c(x11), .O(new_n155_));
  oai21  g122(.a(new_n155_), .b(x03), .c(z5), .O(new_n156_));
  nand2  g123(.a(new_n105_), .b(new_n101_), .O(z7));
  aoi21  g124(.a(new_n48_), .b(x14), .c(new_n153_), .O(new_n158_));
  oai21  g125(.a(new_n158_), .b(new_n49_), .c(x03), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(z7), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n156_), .O(z6));
endmodule


