// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n157_;
  nor2   g000(.a(x13), .b(x05), .O(new_n34_));
  nand2  g001(.a(x13), .b(x05), .O(new_n35_));
  nor2   g002(.a(new_n35_), .b(x24), .O(new_n36_));
  oai21  g003(.a(new_n36_), .b(new_n34_), .c(x19), .O(new_n37_));
  inv1   g004(.a(x13), .O(new_n38_));
  nor2   g005(.a(new_n38_), .b(x07), .O(new_n39_));
  nor2   g006(.a(new_n39_), .b(new_n34_), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(x24), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  inv1   g009(.a(x02), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n43_), .c(x00), .O(new_n45_));
  inv1   g012(.a(new_n45_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n42_), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n41_), .c(new_n37_), .O(z5));
  inv1   g015(.a(x20), .O(new_n49_));
  nor2   g016(.a(x14), .b(x11), .O(new_n50_));
  nand2  g017(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g018(.a(x06), .O(new_n52_));
  nor2   g019(.a(x03), .b(x01), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  and2   g022(.a(new_n55_), .b(z5), .O(z0));
  inv1   g023(.a(x04), .O(new_n57_));
  inv1   g024(.a(x09), .O(new_n58_));
  nor2   g025(.a(x16), .b(x12), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(new_n50_), .c(new_n58_), .d(new_n57_), .O(new_n60_));
  inv1   g027(.a(new_n60_), .O(new_n61_));
  inv1   g028(.a(x01), .O(new_n62_));
  inv1   g029(.a(x03), .O(new_n63_));
  inv1   g030(.a(x08), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n52_), .c(new_n63_), .d(new_n62_), .O(new_n65_));
  inv1   g032(.a(x17), .O(new_n66_));
  inv1   g033(.a(x22), .O(new_n67_));
  inv1   g034(.a(x23), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n49_), .d(new_n66_), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n65_), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(new_n61_), .c(new_n40_), .O(new_n71_));
  nand3  g038(.a(x08), .b(x06), .c(x04), .O(new_n72_));
  nand2  g039(.a(x16), .b(x14), .O(new_n73_));
  nand2  g040(.a(x03), .b(x01), .O(new_n74_));
  nor3   g041(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(new_n75_));
  nand4  g042(.a(x20), .b(x17), .c(x12), .d(x11), .O(new_n76_));
  nand3  g043(.a(x23), .b(x22), .c(x09), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  inv1   g045(.a(x18), .O(new_n79_));
  nor2   g046(.a(new_n34_), .b(new_n79_), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n78_), .c(new_n75_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(x24), .O(new_n83_));
  inv1   g050(.a(x15), .O(new_n84_));
  nand3  g051(.a(x21), .b(new_n44_), .c(new_n43_), .O(new_n85_));
  oai21  g052(.a(new_n35_), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n78_), .c(new_n75_), .O(new_n87_));
  inv1   g054(.a(x19), .O(new_n88_));
  oai21  g055(.a(new_n35_), .b(new_n88_), .c(new_n45_), .O(new_n89_));
  nand3  g056(.a(new_n89_), .b(new_n70_), .c(new_n61_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n42_), .O(new_n92_));
  nand3  g059(.a(new_n70_), .b(new_n61_), .c(x19), .O(new_n93_));
  nand4  g060(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n94_));
  inv1   g061(.a(new_n94_), .O(new_n95_));
  nand4  g062(.a(x16), .b(x15), .c(x14), .d(x04), .O(new_n96_));
  nor3   g063(.a(new_n96_), .b(new_n77_), .c(new_n76_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n93_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n34_), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n92_), .c(new_n83_), .O(z1));
  nand2  g068(.a(x24), .b(x18), .O(new_n102_));
  inv1   g069(.a(new_n102_), .O(new_n103_));
  nand3  g070(.a(new_n42_), .b(x15), .c(x13), .O(new_n104_));
  inv1   g071(.a(new_n104_), .O(new_n105_));
  oai21  g072(.a(new_n105_), .b(new_n103_), .c(x05), .O(new_n106_));
  nand2  g073(.a(new_n34_), .b(x15), .O(new_n107_));
  nand4  g074(.a(new_n42_), .b(x21), .c(new_n44_), .d(new_n43_), .O(new_n108_));
  and2   g075(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g076(.a(new_n103_), .b(x13), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  inv1   g078(.a(new_n111_), .O(new_n112_));
  nand3  g079(.a(x20), .b(x14), .c(x11), .O(new_n113_));
  inv1   g080(.a(new_n113_), .O(new_n114_));
  nand4  g081(.a(new_n114_), .b(x06), .c(x03), .d(x01), .O(new_n115_));
  aoi21  g082(.a(new_n112_), .b(new_n106_), .c(new_n115_), .O(z2));
  nor2   g083(.a(x08), .b(x06), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(new_n53_), .c(new_n50_), .d(new_n49_), .O(new_n118_));
  nor3   g085(.a(new_n118_), .b(new_n39_), .c(new_n34_), .O(new_n119_));
  nor2   g086(.a(new_n113_), .b(new_n94_), .O(new_n120_));
  and2   g087(.a(new_n120_), .b(new_n80_), .O(new_n121_));
  oai21  g088(.a(new_n121_), .b(new_n119_), .c(x24), .O(new_n122_));
  nand2  g089(.a(new_n120_), .b(new_n86_), .O(new_n123_));
  inv1   g090(.a(new_n118_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n89_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n42_), .O(new_n127_));
  nor3   g094(.a(new_n113_), .b(new_n94_), .c(new_n84_), .O(new_n128_));
  nor2   g095(.a(new_n118_), .b(new_n88_), .O(new_n129_));
  oai21  g096(.a(new_n129_), .b(new_n128_), .c(new_n34_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n127_), .c(new_n122_), .O(z3));
  inv1   g098(.a(x16), .O(new_n132_));
  oai21  g099(.a(new_n68_), .b(x04), .c(x17), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(x22), .c(new_n58_), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n132_), .c(x08), .O(new_n135_));
  nand3  g102(.a(new_n42_), .b(x15), .c(x05), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n102_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(x13), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n109_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  oai21  g107(.a(x23), .b(new_n57_), .c(new_n66_), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(new_n67_), .c(x09), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(x16), .c(new_n64_), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(z5), .O(new_n144_));
  nor2   g111(.a(new_n102_), .b(x13), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(x05), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n144_), .c(new_n140_), .O(z4));
  inv1   g114(.a(x11), .O(new_n148_));
  aoi21  g115(.a(new_n49_), .b(x14), .c(new_n52_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(new_n148_), .c(x03), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n139_), .O(new_n151_));
  inv1   g118(.a(x14), .O(new_n152_));
  aoi21  g119(.a(x20), .b(new_n152_), .c(x06), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(x11), .c(new_n63_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(z5), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n151_), .c(new_n146_), .O(z6));
  oai21  g123(.a(new_n145_), .b(new_n105_), .c(x05), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n112_), .O(z7));
endmodule


