// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n41_));
  inv1   g008(.a(new_n41_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n36_), .O(new_n43_));
  inv1   g010(.a(x13), .O(new_n44_));
  nand3  g011(.a(x19), .b(new_n44_), .c(new_n34_), .O(new_n45_));
  inv1   g012(.a(new_n35_), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(x13), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n45_), .c(new_n43_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n38_), .O(new_n49_));
  inv1   g016(.a(x01), .O(new_n50_));
  inv1   g017(.a(x03), .O(new_n51_));
  inv1   g018(.a(x11), .O(new_n52_));
  nor3   g019(.a(x20), .b(x14), .c(x06), .O(new_n53_));
  nand4  g020(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n49_), .O(z0));
  inv1   g022(.a(x15), .O(new_n56_));
  nand2  g023(.a(x13), .b(x05), .O(new_n57_));
  nand3  g024(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n58_));
  oai21  g025(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand2  g026(.a(x03), .b(x01), .O(new_n60_));
  nand4  g027(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g029(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n63_));
  nand4  g030(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g032(.a(new_n65_), .b(new_n62_), .c(new_n59_), .O(new_n66_));
  inv1   g033(.a(x19), .O(new_n67_));
  oai21  g034(.a(new_n57_), .b(new_n67_), .c(new_n41_), .O(new_n68_));
  inv1   g035(.a(x16), .O(new_n69_));
  inv1   g036(.a(x17), .O(new_n70_));
  inv1   g037(.a(x22), .O(new_n71_));
  inv1   g038(.a(x23), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  inv1   g040(.a(x04), .O(new_n74_));
  inv1   g041(.a(x09), .O(new_n75_));
  inv1   g042(.a(x12), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n52_), .c(new_n75_), .d(new_n74_), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  inv1   g045(.a(x06), .O(new_n79_));
  inv1   g046(.a(x14), .O(new_n80_));
  inv1   g047(.a(x20), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  inv1   g049(.a(x08), .O(new_n83_));
  nand3  g050(.a(new_n83_), .b(new_n51_), .c(new_n50_), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand3  g052(.a(new_n85_), .b(new_n78_), .c(new_n68_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n66_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n36_), .O(new_n88_));
  nand2  g055(.a(new_n65_), .b(new_n62_), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  nor2   g057(.a(x17), .b(x16), .O(new_n91_));
  nor2   g058(.a(x23), .b(x22), .O(new_n92_));
  nor2   g059(.a(x09), .b(x04), .O(new_n93_));
  nor2   g060(.a(x12), .b(x11), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(new_n91_), .O(new_n95_));
  nor3   g062(.a(x08), .b(x03), .c(x01), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n53_), .O(new_n97_));
  nor2   g064(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g065(.a(new_n44_), .b(new_n34_), .O(new_n99_));
  nand2  g066(.a(x24), .b(x18), .O(new_n100_));
  oai21  g067(.a(new_n99_), .b(new_n56_), .c(new_n100_), .O(new_n101_));
  nand2  g068(.a(new_n99_), .b(new_n46_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n45_), .O(new_n103_));
  aoi22  g070(.a(new_n103_), .b(new_n98_), .c(new_n101_), .d(new_n90_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n88_), .O(z1));
  nand4  g072(.a(x20), .b(x14), .c(x11), .d(x06), .O(new_n106_));
  nor2   g073(.a(new_n106_), .b(new_n60_), .O(new_n107_));
  oai21  g074(.a(new_n57_), .b(x24), .c(new_n99_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(x15), .O(new_n109_));
  inv1   g076(.a(new_n58_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n36_), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n109_), .c(new_n100_), .O(z7));
  and2   g079(.a(z7), .b(new_n107_), .O(z2));
  nor3   g080(.a(new_n106_), .b(new_n60_), .c(new_n83_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n59_), .O(new_n115_));
  nand4  g082(.a(new_n81_), .b(new_n80_), .c(new_n52_), .d(new_n79_), .O(new_n116_));
  nor2   g083(.a(new_n84_), .b(new_n116_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n68_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n115_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n36_), .O(new_n120_));
  aoi22  g087(.a(new_n117_), .b(new_n103_), .c(new_n114_), .d(new_n101_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(z3));
  nand2  g089(.a(new_n108_), .b(x19), .O(new_n123_));
  nand2  g090(.a(new_n46_), .b(new_n83_), .O(new_n124_));
  nand3  g091(.a(new_n124_), .b(new_n123_), .c(new_n43_), .O(new_n125_));
  oai21  g092(.a(x23), .b(new_n74_), .c(new_n70_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n71_), .O(new_n127_));
  aoi21  g094(.a(new_n127_), .b(new_n75_), .c(x16), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  nand3  g096(.a(new_n123_), .b(new_n102_), .c(new_n43_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(x08), .O(new_n131_));
  oai21  g098(.a(new_n72_), .b(x04), .c(x17), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(x22), .c(new_n75_), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n69_), .c(x08), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(z7), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n131_), .c(new_n129_), .O(z4));
  inv1   g103(.a(new_n49_), .O(z5));
  nand2  g104(.a(new_n46_), .b(new_n51_), .O(new_n138_));
  nand3  g105(.a(new_n138_), .b(new_n123_), .c(new_n43_), .O(new_n139_));
  nand2  g106(.a(x20), .b(new_n80_), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(new_n79_), .c(x11), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  oai21  g109(.a(new_n48_), .b(new_n38_), .c(x03), .O(new_n143_));
  aoi21  g110(.a(new_n81_), .b(x14), .c(new_n79_), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(new_n52_), .c(x03), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(z7), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n143_), .c(new_n142_), .O(z6));
endmodule


