// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nand3  g005(.a(x24), .b(x13), .c(x07), .O(new_n39_));
  nor2   g006(.a(x13), .b(x05), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(x19), .O(new_n41_));
  inv1   g008(.a(x02), .O(new_n42_));
  inv1   g009(.a(x10), .O(new_n43_));
  nand4  g010(.a(new_n36_), .b(new_n43_), .c(new_n42_), .d(x00), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n41_), .c(new_n39_), .O(new_n45_));
  nor2   g012(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  inv1   g013(.a(x20), .O(new_n47_));
  nor2   g014(.a(x14), .b(x11), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g016(.a(x06), .O(new_n50_));
  nor2   g017(.a(x03), .b(x01), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor3   g019(.a(new_n52_), .b(new_n49_), .c(new_n46_), .O(z0));
  nand4  g020(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  nand3  g022(.a(x11), .b(x09), .c(x04), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand4  g026(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n60_));
  inv1   g027(.a(new_n60_), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n59_), .c(new_n57_), .d(new_n55_), .O(new_n62_));
  nor2   g029(.a(x16), .b(x12), .O(new_n63_));
  nor2   g030(.a(x23), .b(x22), .O(new_n64_));
  nor2   g031(.a(x09), .b(x08), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n48_), .O(new_n66_));
  nor3   g033(.a(x20), .b(x17), .c(x04), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n51_), .c(x07), .d(new_n50_), .O(new_n68_));
  oai21  g035(.a(new_n68_), .b(new_n66_), .c(new_n62_), .O(new_n69_));
  nor2   g036(.a(new_n40_), .b(new_n36_), .O(new_n70_));
  nand2  g037(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g038(.a(x13), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n34_), .O(new_n73_));
  nand3  g040(.a(new_n36_), .b(x13), .c(x05), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g042(.a(x20), .b(x17), .O(new_n76_));
  nor2   g043(.a(x09), .b(x04), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n76_), .c(new_n64_), .d(new_n51_), .O(new_n78_));
  nor2   g045(.a(x08), .b(x06), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n63_), .c(new_n48_), .d(x19), .O(new_n80_));
  nand4  g047(.a(x23), .b(x22), .c(x20), .d(x15), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n59_), .c(new_n57_), .d(new_n55_), .O(new_n83_));
  oai21  g050(.a(new_n80_), .b(new_n78_), .c(new_n83_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n75_), .O(new_n85_));
  inv1   g052(.a(new_n66_), .O(new_n86_));
  nor2   g053(.a(new_n52_), .b(new_n44_), .O(new_n87_));
  nand3  g054(.a(new_n87_), .b(new_n67_), .c(new_n86_), .O(new_n88_));
  nand3  g055(.a(new_n88_), .b(new_n85_), .c(new_n71_), .O(z1));
  nand2  g056(.a(x24), .b(x18), .O(new_n90_));
  nand3  g057(.a(new_n36_), .b(x15), .c(x13), .O(new_n91_));
  aoi21  g058(.a(new_n91_), .b(new_n90_), .c(new_n34_), .O(new_n92_));
  inv1   g059(.a(x15), .O(new_n93_));
  oai22  g060(.a(new_n90_), .b(new_n72_), .c(new_n73_), .d(new_n93_), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  inv1   g062(.a(x11), .O(new_n96_));
  inv1   g063(.a(x14), .O(new_n97_));
  nor2   g064(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor2   g065(.a(new_n47_), .b(new_n50_), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n98_), .c(x03), .d(x01), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(new_n95_), .O(z2));
  nand2  g068(.a(x20), .b(x18), .O(new_n102_));
  nand2  g069(.a(new_n98_), .b(new_n55_), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand3  g071(.a(new_n51_), .b(new_n48_), .c(new_n47_), .O(new_n105_));
  inv1   g072(.a(x08), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(x07), .c(new_n50_), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  oai21  g075(.a(new_n108_), .b(new_n104_), .c(new_n70_), .O(new_n109_));
  nand2  g076(.a(x20), .b(x15), .O(new_n110_));
  nor2   g077(.a(new_n103_), .b(new_n110_), .O(new_n111_));
  nand2  g078(.a(new_n79_), .b(x19), .O(new_n112_));
  nor2   g079(.a(new_n105_), .b(new_n112_), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(new_n111_), .c(new_n75_), .O(new_n114_));
  nor2   g081(.a(new_n52_), .b(new_n49_), .O(new_n115_));
  nor2   g082(.a(new_n44_), .b(x08), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g084(.a(new_n117_), .b(new_n114_), .c(new_n109_), .O(z3));
  inv1   g085(.a(x16), .O(new_n119_));
  inv1   g086(.a(x09), .O(new_n120_));
  inv1   g087(.a(x23), .O(new_n121_));
  aoi21  g088(.a(new_n121_), .b(x04), .c(x17), .O(new_n122_));
  oai21  g089(.a(new_n122_), .b(x22), .c(new_n120_), .O(new_n123_));
  aoi21  g090(.a(new_n123_), .b(new_n119_), .c(x08), .O(new_n124_));
  inv1   g091(.a(x22), .O(new_n125_));
  inv1   g092(.a(x04), .O(new_n126_));
  inv1   g093(.a(x17), .O(new_n127_));
  aoi21  g094(.a(x23), .b(new_n126_), .c(new_n127_), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(new_n125_), .c(x09), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(x16), .c(new_n106_), .O(new_n130_));
  oai22  g097(.a(new_n130_), .b(new_n95_), .c(new_n124_), .d(new_n46_), .O(z4));
  inv1   g098(.a(new_n46_), .O(z5));
  oai21  g099(.a(new_n47_), .b(x14), .c(new_n50_), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n96_), .c(x03), .O(new_n134_));
  inv1   g101(.a(x03), .O(new_n135_));
  oai21  g102(.a(x20), .b(new_n97_), .c(x06), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(x11), .c(new_n135_), .O(new_n137_));
  oai22  g104(.a(new_n137_), .b(new_n95_), .c(new_n134_), .d(new_n46_), .O(z6));
  inv1   g105(.a(new_n95_), .O(z7));
endmodule


