// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x24), .O(new_n34_));
  nor2   g001(.a(x10), .b(x02), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(new_n34_), .c(x00), .O(new_n36_));
  nand2  g003(.a(x24), .b(x07), .O(new_n37_));
  nand3  g004(.a(new_n34_), .b(x19), .c(x13), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(x05), .O(new_n40_));
  nand3  g007(.a(x24), .b(x13), .c(x07), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  inv1   g011(.a(new_n44_), .O(new_n45_));
  inv1   g012(.a(x06), .O(new_n46_));
  inv1   g013(.a(x20), .O(new_n47_));
  nor2   g014(.a(x14), .b(x11), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g016(.a(new_n49_), .O(new_n50_));
  nor2   g017(.a(x03), .b(x01), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g019(.a(new_n45_), .b(new_n36_), .c(new_n52_), .O(z0));
  inv1   g020(.a(x17), .O(new_n54_));
  inv1   g021(.a(x22), .O(new_n55_));
  inv1   g022(.a(x23), .O(new_n56_));
  nand3  g023(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  inv1   g024(.a(x01), .O(new_n58_));
  inv1   g025(.a(x03), .O(new_n59_));
  inv1   g026(.a(x04), .O(new_n60_));
  inv1   g027(.a(x16), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n60_), .c(new_n59_), .d(new_n58_), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  inv1   g030(.a(x12), .O(new_n64_));
  nand3  g031(.a(new_n48_), .b(new_n47_), .c(new_n64_), .O(new_n65_));
  inv1   g032(.a(new_n65_), .O(new_n66_));
  inv1   g033(.a(x02), .O(new_n67_));
  inv1   g034(.a(x10), .O(new_n68_));
  nand3  g035(.a(new_n68_), .b(new_n67_), .c(x00), .O(new_n69_));
  inv1   g036(.a(x08), .O(new_n70_));
  inv1   g037(.a(x09), .O(new_n71_));
  nand3  g038(.a(new_n71_), .b(new_n70_), .c(new_n46_), .O(new_n72_));
  nand3  g039(.a(x19), .b(x13), .c(x05), .O(new_n73_));
  aoi21  g040(.a(new_n73_), .b(new_n69_), .c(new_n72_), .O(new_n74_));
  nand3  g041(.a(new_n74_), .b(new_n66_), .c(new_n63_), .O(new_n75_));
  nand3  g042(.a(x15), .b(x13), .c(x05), .O(new_n76_));
  oai21  g043(.a(x10), .b(x02), .c(new_n76_), .O(new_n77_));
  nand2  g044(.a(x08), .b(x06), .O(new_n78_));
  nand4  g045(.a(x09), .b(x04), .c(x03), .d(x01), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand4  g047(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n81_));
  nand4  g048(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g050(.a(new_n83_), .b(new_n80_), .c(new_n77_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n75_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n34_), .O(new_n86_));
  nor2   g053(.a(new_n42_), .b(new_n34_), .O(new_n87_));
  nor2   g054(.a(new_n81_), .b(new_n64_), .O(new_n88_));
  nand3  g055(.a(x18), .b(x16), .c(x14), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n88_), .c(new_n80_), .d(x11), .O(new_n91_));
  nand4  g058(.a(new_n71_), .b(new_n70_), .c(x07), .d(new_n46_), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n66_), .c(new_n63_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n87_), .O(new_n96_));
  nand3  g063(.a(x16), .b(x15), .c(x14), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n88_), .c(new_n80_), .d(x11), .O(new_n99_));
  nand4  g066(.a(new_n47_), .b(x19), .c(new_n70_), .d(new_n46_), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand3  g068(.a(new_n48_), .b(new_n64_), .c(new_n71_), .O(new_n102_));
  inv1   g069(.a(new_n102_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n101_), .c(new_n63_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n99_), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n42_), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n96_), .c(new_n86_), .O(z1));
  nand2  g074(.a(x24), .b(x18), .O(new_n108_));
  nand3  g075(.a(new_n34_), .b(x15), .c(x13), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(x05), .O(new_n111_));
  nand2  g078(.a(new_n35_), .b(new_n34_), .O(new_n112_));
  nand2  g079(.a(new_n42_), .b(x15), .O(new_n113_));
  nand3  g080(.a(x24), .b(x18), .c(x13), .O(new_n114_));
  nand3  g081(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  inv1   g083(.a(x11), .O(new_n117_));
  inv1   g084(.a(x14), .O(new_n118_));
  nand3  g085(.a(x20), .b(x03), .c(x01), .O(new_n119_));
  nor3   g086(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(x06), .O(new_n121_));
  aoi21  g088(.a(new_n116_), .b(new_n111_), .c(new_n121_), .O(z2));
  inv1   g089(.a(new_n78_), .O(new_n123_));
  nand3  g090(.a(new_n120_), .b(new_n123_), .c(x18), .O(new_n124_));
  nor3   g091(.a(x08), .b(x03), .c(x01), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(new_n50_), .c(x07), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n87_), .O(new_n128_));
  nand3  g095(.a(new_n120_), .b(new_n123_), .c(x15), .O(new_n129_));
  nand3  g096(.a(new_n101_), .b(new_n51_), .c(new_n48_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n42_), .O(new_n132_));
  nand3  g099(.a(new_n120_), .b(new_n123_), .c(new_n77_), .O(new_n133_));
  nand2  g100(.a(new_n73_), .b(new_n69_), .O(new_n134_));
  nand3  g101(.a(new_n125_), .b(new_n134_), .c(new_n50_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n34_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n132_), .c(new_n128_), .O(z3));
  oai21  g105(.a(x23), .b(new_n60_), .c(new_n54_), .O(new_n139_));
  aoi21  g106(.a(new_n139_), .b(new_n55_), .c(x09), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(x16), .c(new_n70_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n44_), .O(new_n142_));
  nand2  g109(.a(new_n116_), .b(new_n111_), .O(z7));
  oai21  g110(.a(new_n56_), .b(x04), .c(x17), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(x22), .c(new_n71_), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(new_n61_), .c(x08), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(z7), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n142_), .c(new_n36_), .O(z4));
  nand2  g115(.a(new_n45_), .b(new_n36_), .O(z5));
  aoi21  g116(.a(x20), .b(new_n118_), .c(x06), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(x11), .c(new_n59_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n44_), .O(new_n152_));
  aoi21  g119(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(new_n117_), .c(x03), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(z7), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n152_), .c(new_n36_), .O(z6));
endmodule


