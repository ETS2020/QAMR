// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_;
  nor2   g000(.a(x14), .b(x11), .O(new_n34_));
  nor2   g001(.a(x03), .b(x01), .O(new_n35_));
  nor2   g002(.a(x20), .b(x06), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  inv1   g009(.a(x02), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  nand4  g011(.a(new_n40_), .b(new_n44_), .c(new_n43_), .d(x00), .O(new_n45_));
  nor2   g012(.a(x13), .b(x05), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(x19), .O(new_n47_));
  nand3  g014(.a(x24), .b(x13), .c(x07), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n37_), .O(z0));
  nand4  g018(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n52_));
  nand4  g019(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g021(.a(x21), .b(new_n44_), .c(new_n43_), .O(new_n55_));
  inv1   g022(.a(new_n55_), .O(new_n56_));
  nand2  g023(.a(x09), .b(x04), .O(new_n57_));
  nand4  g024(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g026(.a(new_n59_), .b(new_n56_), .c(new_n54_), .O(new_n60_));
  nor2   g027(.a(x06), .b(x04), .O(new_n61_));
  nand3  g028(.a(new_n61_), .b(new_n35_), .c(new_n34_), .O(new_n62_));
  inv1   g029(.a(new_n62_), .O(new_n63_));
  inv1   g030(.a(x20), .O(new_n64_));
  inv1   g031(.a(x22), .O(new_n65_));
  inv1   g032(.a(x23), .O(new_n66_));
  nand3  g033(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  inv1   g034(.a(x08), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  inv1   g036(.a(x16), .O(new_n70_));
  inv1   g037(.a(x17), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nor2   g039(.a(new_n72_), .b(new_n67_), .O(new_n73_));
  nand3  g040(.a(new_n44_), .b(new_n43_), .c(x00), .O(new_n74_));
  nand3  g041(.a(x19), .b(x13), .c(x05), .O(new_n75_));
  oai21  g042(.a(new_n74_), .b(x12), .c(new_n75_), .O(new_n76_));
  nand3  g043(.a(new_n76_), .b(new_n73_), .c(new_n63_), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n60_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n40_), .O(new_n79_));
  nor3   g046(.a(new_n72_), .b(new_n67_), .c(new_n62_), .O(new_n80_));
  inv1   g047(.a(x12), .O(new_n81_));
  aoi21  g048(.a(new_n81_), .b(x05), .c(x13), .O(new_n82_));
  oai22  g049(.a(new_n82_), .b(new_n39_), .c(new_n47_), .d(x12), .O(new_n83_));
  inv1   g050(.a(x15), .O(new_n84_));
  nand3  g051(.a(x24), .b(x18), .c(x05), .O(new_n85_));
  oai21  g052(.a(new_n84_), .b(x05), .c(new_n85_), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n59_), .c(new_n54_), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  aoi21  g055(.a(new_n83_), .b(new_n80_), .c(new_n88_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n79_), .O(z1));
  nand2  g057(.a(x24), .b(x18), .O(new_n91_));
  nand3  g058(.a(new_n40_), .b(x15), .c(x13), .O(new_n92_));
  aoi21  g059(.a(new_n92_), .b(new_n91_), .c(new_n38_), .O(new_n93_));
  nand4  g060(.a(new_n40_), .b(x21), .c(new_n44_), .d(new_n43_), .O(new_n94_));
  inv1   g061(.a(x13), .O(new_n95_));
  nand3  g062(.a(x15), .b(new_n95_), .c(new_n38_), .O(new_n96_));
  nand3  g063(.a(x24), .b(x18), .c(x13), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n96_), .c(new_n94_), .O(new_n98_));
  nor2   g065(.a(new_n98_), .b(new_n93_), .O(new_n99_));
  nand3  g066(.a(x20), .b(x14), .c(x11), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(x06), .c(x03), .d(x01), .O(new_n102_));
  nor2   g069(.a(new_n102_), .b(new_n99_), .O(z2));
  nand3  g070(.a(x15), .b(x13), .c(x05), .O(new_n104_));
  inv1   g071(.a(new_n52_), .O(new_n105_));
  nand2  g072(.a(new_n101_), .b(new_n105_), .O(new_n106_));
  aoi21  g073(.a(new_n104_), .b(new_n55_), .c(new_n106_), .O(new_n107_));
  nand4  g074(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(new_n68_), .O(new_n108_));
  aoi21  g075(.a(new_n75_), .b(new_n74_), .c(new_n108_), .O(new_n109_));
  oai21  g076(.a(new_n109_), .b(new_n107_), .c(new_n40_), .O(new_n110_));
  inv1   g077(.a(x18), .O(new_n111_));
  nor2   g078(.a(new_n106_), .b(new_n111_), .O(new_n112_));
  nand2  g079(.a(new_n68_), .b(x07), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n37_), .O(new_n114_));
  nor2   g081(.a(new_n46_), .b(new_n40_), .O(new_n115_));
  oai21  g082(.a(new_n114_), .b(new_n112_), .c(new_n115_), .O(new_n116_));
  nor2   g083(.a(new_n106_), .b(new_n84_), .O(new_n117_));
  nand2  g084(.a(x19), .b(new_n68_), .O(new_n118_));
  nor2   g085(.a(new_n118_), .b(new_n37_), .O(new_n119_));
  oai21  g086(.a(new_n119_), .b(new_n117_), .c(new_n46_), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(new_n116_), .c(new_n110_), .O(z3));
  aoi21  g088(.a(new_n66_), .b(x04), .c(x17), .O(new_n122_));
  oai21  g089(.a(new_n122_), .b(x22), .c(new_n69_), .O(new_n123_));
  aoi21  g090(.a(new_n123_), .b(new_n70_), .c(x08), .O(new_n124_));
  inv1   g091(.a(x04), .O(new_n125_));
  aoi21  g092(.a(x23), .b(new_n125_), .c(new_n71_), .O(new_n126_));
  oai21  g093(.a(new_n126_), .b(new_n65_), .c(x09), .O(new_n127_));
  aoi21  g094(.a(new_n127_), .b(x16), .c(new_n68_), .O(new_n128_));
  oai22  g095(.a(new_n128_), .b(new_n99_), .c(new_n124_), .d(new_n50_), .O(z4));
  inv1   g096(.a(new_n50_), .O(z5));
  inv1   g097(.a(x11), .O(new_n131_));
  inv1   g098(.a(x06), .O(new_n132_));
  oai21  g099(.a(new_n64_), .b(x14), .c(new_n132_), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n131_), .c(x03), .O(new_n134_));
  inv1   g101(.a(x03), .O(new_n135_));
  nand2  g102(.a(new_n64_), .b(x14), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(x06), .O(new_n137_));
  aoi21  g104(.a(new_n137_), .b(x11), .c(new_n135_), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n99_), .c(new_n134_), .d(new_n50_), .O(z6));
  inv1   g106(.a(new_n99_), .O(z7));
endmodule


