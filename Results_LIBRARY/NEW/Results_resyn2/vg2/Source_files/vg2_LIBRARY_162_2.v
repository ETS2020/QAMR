// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n142_, new_n143_, new_n144_;
  nor2   g000(.a(x14), .b(x11), .O(new_n34_));
  nor3   g001(.a(x20), .b(x06), .c(x01), .O(new_n35_));
  nand2  g002(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  inv1   g003(.a(x05), .O(new_n37_));
  nand2  g004(.a(x24), .b(x07), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(x19), .c(x13), .O(new_n40_));
  aoi21  g007(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  inv1   g008(.a(x02), .O(new_n42_));
  inv1   g009(.a(x10), .O(new_n43_));
  nand4  g010(.a(new_n39_), .b(new_n43_), .c(new_n42_), .d(x00), .O(new_n44_));
  nor2   g011(.a(x13), .b(x05), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(x19), .O(new_n46_));
  nand3  g013(.a(x24), .b(x13), .c(x07), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n36_), .O(z0));
  nand4  g017(.a(x23), .b(x22), .c(x16), .d(x12), .O(new_n51_));
  nand4  g018(.a(x06), .b(x04), .c(x03), .d(x01), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g020(.a(x15), .O(new_n54_));
  nand3  g021(.a(x21), .b(new_n43_), .c(new_n42_), .O(new_n55_));
  nand2  g022(.a(x13), .b(x05), .O(new_n56_));
  oai21  g023(.a(new_n56_), .b(new_n54_), .c(new_n55_), .O(new_n57_));
  inv1   g024(.a(x11), .O(new_n58_));
  inv1   g025(.a(x20), .O(new_n59_));
  nand3  g026(.a(x17), .b(x09), .c(x08), .O(new_n60_));
  nor3   g027(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand3  g028(.a(new_n61_), .b(new_n57_), .c(new_n53_), .O(new_n62_));
  inv1   g029(.a(x19), .O(new_n63_));
  nand3  g030(.a(new_n43_), .b(new_n42_), .c(x00), .O(new_n64_));
  oai21  g031(.a(new_n56_), .b(new_n63_), .c(new_n64_), .O(new_n65_));
  inv1   g032(.a(x01), .O(new_n66_));
  inv1   g033(.a(x06), .O(new_n67_));
  nand3  g034(.a(new_n59_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  inv1   g035(.a(x04), .O(new_n69_));
  inv1   g036(.a(x16), .O(new_n70_));
  inv1   g037(.a(x17), .O(new_n71_));
  nand3  g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  nor2   g039(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  inv1   g040(.a(x08), .O(new_n74_));
  inv1   g041(.a(x14), .O(new_n75_));
  nand3  g042(.a(new_n75_), .b(new_n58_), .c(new_n74_), .O(new_n76_));
  inv1   g043(.a(x09), .O(new_n77_));
  inv1   g044(.a(x12), .O(new_n78_));
  inv1   g045(.a(x22), .O(new_n79_));
  inv1   g046(.a(x23), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n77_), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n76_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n73_), .c(new_n65_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n62_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n39_), .O(new_n85_));
  nor2   g052(.a(x17), .b(x16), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n35_), .c(new_n69_), .O(new_n87_));
  nor2   g054(.a(x12), .b(x09), .O(new_n88_));
  nor2   g055(.a(x23), .b(x22), .O(new_n89_));
  inv1   g056(.a(x07), .O(new_n90_));
  nor2   g057(.a(x08), .b(new_n90_), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n89_), .c(new_n88_), .d(new_n34_), .O(new_n92_));
  nand4  g059(.a(x17), .b(x11), .c(x09), .d(x08), .O(new_n93_));
  inv1   g060(.a(new_n93_), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n53_), .c(x20), .d(x18), .O(new_n95_));
  oai21  g062(.a(new_n92_), .b(new_n87_), .c(new_n95_), .O(new_n96_));
  nor2   g063(.a(new_n45_), .b(new_n39_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand4  g065(.a(new_n94_), .b(new_n53_), .c(x20), .d(x15), .O(new_n99_));
  nand3  g066(.a(new_n35_), .b(new_n34_), .c(new_n74_), .O(new_n100_));
  nor2   g067(.a(new_n63_), .b(x04), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n89_), .c(new_n88_), .d(new_n86_), .O(new_n102_));
  oai21  g069(.a(new_n102_), .b(new_n100_), .c(new_n99_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n45_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n98_), .c(new_n85_), .O(z1));
  nand2  g072(.a(x24), .b(x18), .O(new_n106_));
  nand3  g073(.a(new_n39_), .b(x15), .c(x13), .O(new_n107_));
  aoi21  g074(.a(new_n107_), .b(new_n106_), .c(new_n37_), .O(new_n108_));
  nand4  g075(.a(new_n39_), .b(x21), .c(new_n43_), .d(new_n42_), .O(new_n109_));
  nand3  g076(.a(x24), .b(x18), .c(x13), .O(new_n110_));
  nand2  g077(.a(new_n45_), .b(x15), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n108_), .O(new_n113_));
  inv1   g080(.a(x03), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n66_), .O(new_n115_));
  nand4  g082(.a(new_n115_), .b(x20), .c(x11), .d(x06), .O(new_n116_));
  nor2   g083(.a(new_n116_), .b(new_n113_), .O(z2));
  nor2   g084(.a(new_n76_), .b(new_n68_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n65_), .O(new_n119_));
  nand2  g086(.a(x20), .b(x06), .O(new_n120_));
  nand4  g087(.a(x11), .b(x08), .c(x03), .d(x01), .O(new_n121_));
  nor2   g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n57_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n39_), .O(new_n125_));
  nand2  g092(.a(new_n122_), .b(x15), .O(new_n126_));
  oai21  g093(.a(new_n100_), .b(new_n63_), .c(new_n126_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n45_), .O(new_n128_));
  and2   g095(.a(new_n122_), .b(x18), .O(new_n129_));
  inv1   g096(.a(new_n91_), .O(new_n130_));
  nor2   g097(.a(new_n130_), .b(new_n36_), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(new_n129_), .c(new_n97_), .O(new_n132_));
  nand3  g099(.a(new_n132_), .b(new_n128_), .c(new_n125_), .O(z3));
  aoi21  g100(.a(new_n80_), .b(x04), .c(x17), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(x22), .c(new_n77_), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n70_), .c(x08), .O(new_n136_));
  aoi21  g103(.a(x23), .b(new_n69_), .c(new_n71_), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(new_n79_), .c(x09), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(x16), .c(new_n74_), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(new_n113_), .c(new_n136_), .d(new_n49_), .O(z4));
  inv1   g107(.a(new_n49_), .O(z5));
  oai21  g108(.a(new_n59_), .b(x14), .c(new_n67_), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(new_n58_), .c(x03), .O(new_n143_));
  aoi21  g110(.a(new_n120_), .b(x11), .c(new_n114_), .O(new_n144_));
  oai22  g111(.a(new_n144_), .b(new_n113_), .c(new_n143_), .d(new_n49_), .O(z6));
  inv1   g112(.a(new_n113_), .O(z7));
endmodule


