// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_;
  nor2   g000(.a(x03), .b(x01), .O(new_n34_));
  nor2   g001(.a(x20), .b(x14), .O(new_n35_));
  nor2   g002(.a(x11), .b(x06), .O(new_n36_));
  nand2  g003(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g004(.a(new_n37_), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nor2   g007(.a(x10), .b(x02), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(new_n40_), .c(x00), .O(new_n42_));
  inv1   g009(.a(new_n42_), .O(new_n43_));
  inv1   g010(.a(x05), .O(new_n44_));
  nand2  g011(.a(x19), .b(new_n44_), .O(new_n45_));
  nand2  g012(.a(x24), .b(x07), .O(new_n46_));
  inv1   g013(.a(new_n46_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(x05), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n39_), .O(z0));
  nand2  g018(.a(x11), .b(x08), .O(new_n52_));
  nand4  g019(.a(x09), .b(x04), .c(x03), .d(x01), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g021(.a(x23), .b(x22), .c(x20), .O(new_n55_));
  nand4  g022(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand3  g024(.a(x15), .b(x13), .c(x05), .O(new_n58_));
  nand3  g025(.a(new_n41_), .b(x21), .c(x06), .O(new_n59_));
  nand2  g026(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n57_), .c(new_n54_), .O(new_n61_));
  nor2   g028(.a(x09), .b(x08), .O(new_n62_));
  nor2   g029(.a(x17), .b(x16), .O(new_n63_));
  nor2   g030(.a(x23), .b(x22), .O(new_n64_));
  nand3  g031(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  inv1   g032(.a(new_n65_), .O(new_n66_));
  inv1   g033(.a(x04), .O(new_n67_));
  inv1   g034(.a(x06), .O(new_n68_));
  nand3  g035(.a(new_n35_), .b(new_n68_), .c(new_n67_), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  inv1   g037(.a(x01), .O(new_n71_));
  inv1   g038(.a(x02), .O(new_n72_));
  inv1   g039(.a(x03), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x00), .O(new_n74_));
  inv1   g041(.a(x10), .O(new_n75_));
  inv1   g042(.a(x11), .O(new_n76_));
  inv1   g043(.a(x12), .O(new_n77_));
  nand3  g044(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  nor2   g045(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  nand3  g046(.a(new_n79_), .b(new_n70_), .c(new_n66_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n61_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n40_), .O(new_n82_));
  inv1   g049(.a(x13), .O(new_n83_));
  nand3  g050(.a(x15), .b(new_n83_), .c(new_n44_), .O(new_n84_));
  nand3  g051(.a(x24), .b(x18), .c(x05), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(x06), .O(new_n87_));
  nand2  g054(.a(x24), .b(x18), .O(new_n88_));
  inv1   g055(.a(new_n88_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(x13), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n57_), .c(new_n54_), .O(new_n92_));
  nand3  g059(.a(new_n34_), .b(new_n77_), .c(new_n76_), .O(new_n93_));
  nor2   g060(.a(new_n93_), .b(new_n69_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n66_), .c(new_n49_), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(new_n92_), .c(new_n82_), .O(z1));
  nand3  g063(.a(new_n40_), .b(x15), .c(x13), .O(new_n97_));
  aoi21  g064(.a(new_n97_), .b(new_n88_), .c(new_n44_), .O(new_n98_));
  nand2  g065(.a(new_n97_), .b(new_n68_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  inv1   g067(.a(new_n90_), .O(new_n101_));
  nand3  g068(.a(new_n41_), .b(new_n40_), .c(x21), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n84_), .O(new_n103_));
  aoi21  g070(.a(new_n103_), .b(x06), .c(new_n101_), .O(new_n104_));
  nor2   g071(.a(new_n73_), .b(new_n71_), .O(new_n105_));
  inv1   g072(.a(x14), .O(new_n106_));
  inv1   g073(.a(x20), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n105_), .c(x11), .O(new_n109_));
  aoi21  g076(.a(new_n104_), .b(new_n100_), .c(new_n109_), .O(z2));
  inv1   g077(.a(new_n52_), .O(new_n111_));
  nand4  g078(.a(new_n108_), .b(new_n91_), .c(new_n105_), .d(new_n111_), .O(new_n112_));
  nand3  g079(.a(new_n108_), .b(new_n105_), .c(new_n111_), .O(new_n113_));
  aoi21  g080(.a(new_n59_), .b(new_n58_), .c(new_n113_), .O(new_n114_));
  inv1   g081(.a(x08), .O(new_n115_));
  nand2  g082(.a(new_n75_), .b(new_n115_), .O(new_n116_));
  nor3   g083(.a(new_n116_), .b(new_n74_), .c(new_n37_), .O(new_n117_));
  oai21  g084(.a(new_n117_), .b(new_n114_), .c(new_n40_), .O(new_n118_));
  nand4  g085(.a(new_n49_), .b(new_n38_), .c(new_n34_), .d(new_n115_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(new_n118_), .c(new_n112_), .O(z3));
  inv1   g087(.a(x22), .O(new_n121_));
  inv1   g088(.a(x17), .O(new_n122_));
  oai21  g089(.a(x23), .b(new_n67_), .c(new_n122_), .O(new_n123_));
  aoi21  g090(.a(new_n123_), .b(new_n121_), .c(x09), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(x16), .c(new_n115_), .O(new_n125_));
  nand3  g092(.a(new_n40_), .b(x19), .c(x13), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n46_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(x05), .O(new_n128_));
  nand2  g095(.a(new_n47_), .b(x13), .O(new_n129_));
  nand3  g096(.a(x19), .b(new_n83_), .c(new_n44_), .O(new_n130_));
  nand4  g097(.a(new_n130_), .b(new_n129_), .c(new_n128_), .d(new_n42_), .O(z5));
  nand2  g098(.a(z5), .b(new_n125_), .O(new_n132_));
  inv1   g099(.a(x16), .O(new_n133_));
  inv1   g100(.a(x09), .O(new_n134_));
  inv1   g101(.a(x23), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(x04), .c(x17), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(x22), .c(new_n134_), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(new_n133_), .c(x08), .O(new_n138_));
  inv1   g105(.a(new_n98_), .O(new_n139_));
  inv1   g106(.a(new_n103_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n139_), .c(new_n90_), .O(z7));
  nand2  g108(.a(z7), .b(new_n138_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n132_), .O(z4));
  nand3  g110(.a(new_n40_), .b(x19), .c(x05), .O(new_n144_));
  aoi22  g111(.a(new_n144_), .b(new_n46_), .c(x11), .d(new_n73_), .O(new_n145_));
  nand3  g112(.a(new_n40_), .b(x15), .c(x05), .O(new_n146_));
  nand3  g113(.a(new_n107_), .b(x14), .c(x11), .O(new_n147_));
  aoi22  g114(.a(new_n147_), .b(x03), .c(new_n146_), .d(new_n88_), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(new_n145_), .c(x13), .O(new_n149_));
  inv1   g116(.a(new_n85_), .O(new_n150_));
  nand2  g117(.a(new_n147_), .b(x03), .O(new_n151_));
  oai21  g118(.a(new_n103_), .b(new_n150_), .c(new_n151_), .O(new_n152_));
  aoi21  g119(.a(x20), .b(new_n106_), .c(x06), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(x11), .c(new_n73_), .O(new_n154_));
  nand3  g121(.a(new_n130_), .b(new_n48_), .c(new_n42_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand2  g123(.a(x15), .b(new_n44_), .O(new_n157_));
  nand3  g124(.a(new_n157_), .b(new_n102_), .c(new_n85_), .O(new_n158_));
  nand3  g125(.a(new_n158_), .b(x11), .c(new_n68_), .O(new_n159_));
  nand4  g126(.a(new_n159_), .b(new_n156_), .c(new_n152_), .d(new_n149_), .O(z6));
endmodule


