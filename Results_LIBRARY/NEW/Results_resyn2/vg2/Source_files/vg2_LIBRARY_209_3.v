// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nor2   g005(.a(x10), .b(x02), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n36_), .c(x00), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x19), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  nor2   g011(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  nor3   g012(.a(x20), .b(x14), .c(x06), .O(new_n46_));
  nor3   g013(.a(x11), .b(x03), .c(x01), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n45_), .O(z0));
  nand4  g016(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n50_));
  nand3  g017(.a(x11), .b(x09), .c(x04), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand4  g019(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n53_));
  nand4  g020(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g022(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nor2   g023(.a(x03), .b(x01), .O(new_n57_));
  nor2   g024(.a(x23), .b(x22), .O(new_n58_));
  nor2   g025(.a(x12), .b(x11), .O(new_n59_));
  inv1   g026(.a(x07), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(x06), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(new_n62_));
  inv1   g029(.a(x04), .O(new_n63_));
  nor2   g030(.a(x20), .b(x14), .O(new_n64_));
  nor2   g031(.a(x09), .b(x08), .O(new_n65_));
  nor2   g032(.a(x17), .b(x16), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  oai21  g034(.a(new_n67_), .b(new_n62_), .c(new_n56_), .O(new_n68_));
  nor2   g035(.a(new_n41_), .b(new_n36_), .O(new_n69_));
  nand2  g036(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nor2   g037(.a(x14), .b(x06), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n66_), .c(new_n65_), .d(new_n59_), .O(new_n72_));
  inv1   g039(.a(x00), .O(new_n73_));
  nor2   g040(.a(x24), .b(new_n73_), .O(new_n74_));
  nor2   g041(.a(x04), .b(x03), .O(new_n75_));
  nor2   g042(.a(x20), .b(x01), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n58_), .d(new_n74_), .O(new_n77_));
  nand4  g044(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n78_));
  nor2   g045(.a(new_n78_), .b(new_n53_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n52_), .O(new_n80_));
  oai21  g047(.a(new_n77_), .b(new_n72_), .c(new_n80_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n39_), .O(new_n82_));
  nand3  g049(.a(new_n36_), .b(x13), .c(x05), .O(new_n83_));
  oai21  g050(.a(x13), .b(x05), .c(new_n83_), .O(new_n84_));
  nand4  g051(.a(x23), .b(x22), .c(x20), .d(x15), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(new_n53_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n52_), .O(new_n87_));
  nand4  g054(.a(new_n76_), .b(new_n75_), .c(new_n58_), .d(x19), .O(new_n88_));
  oai21  g055(.a(new_n88_), .b(new_n72_), .c(new_n87_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n84_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(new_n82_), .c(new_n70_), .O(z1));
  nand2  g058(.a(x24), .b(x18), .O(new_n92_));
  nand3  g059(.a(new_n36_), .b(x15), .c(x13), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand3  g061(.a(x24), .b(x18), .c(x13), .O(new_n95_));
  nand2  g062(.a(new_n41_), .b(x15), .O(new_n96_));
  nand2  g063(.a(new_n39_), .b(x21), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(new_n98_));
  aoi21  g065(.a(new_n94_), .b(x05), .c(new_n98_), .O(new_n99_));
  inv1   g066(.a(x11), .O(new_n100_));
  inv1   g067(.a(x14), .O(new_n101_));
  nor2   g068(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  inv1   g069(.a(x06), .O(new_n103_));
  inv1   g070(.a(x20), .O(new_n104_));
  nor2   g071(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g072(.a(new_n105_), .b(new_n102_), .c(x03), .d(x01), .O(new_n106_));
  nor2   g073(.a(new_n106_), .b(new_n99_), .O(z2));
  nand2  g074(.a(x20), .b(x18), .O(new_n108_));
  inv1   g075(.a(new_n50_), .O(new_n109_));
  nand2  g076(.a(new_n102_), .b(new_n109_), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  inv1   g078(.a(x08), .O(new_n112_));
  nand3  g079(.a(new_n64_), .b(new_n100_), .c(new_n112_), .O(new_n113_));
  nand2  g080(.a(new_n61_), .b(new_n57_), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n111_), .c(new_n69_), .O(new_n116_));
  nand2  g083(.a(x21), .b(x20), .O(new_n117_));
  nor2   g084(.a(new_n110_), .b(new_n117_), .O(new_n118_));
  nand3  g085(.a(new_n57_), .b(new_n74_), .c(new_n103_), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n113_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n118_), .c(new_n39_), .O(new_n121_));
  nand2  g088(.a(x20), .b(x15), .O(new_n122_));
  nand4  g089(.a(new_n47_), .b(new_n46_), .c(x19), .d(new_n112_), .O(new_n123_));
  oai21  g090(.a(new_n110_), .b(new_n122_), .c(new_n123_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n84_), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(new_n121_), .c(new_n116_), .O(z3));
  inv1   g093(.a(x16), .O(new_n127_));
  inv1   g094(.a(x09), .O(new_n128_));
  inv1   g095(.a(x23), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(x04), .c(x17), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(x22), .c(new_n128_), .O(new_n131_));
  aoi21  g098(.a(new_n131_), .b(new_n127_), .c(x08), .O(new_n132_));
  inv1   g099(.a(x22), .O(new_n133_));
  inv1   g100(.a(x17), .O(new_n134_));
  aoi21  g101(.a(x23), .b(new_n63_), .c(new_n134_), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(new_n133_), .c(x09), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(x16), .c(new_n112_), .O(new_n137_));
  oai22  g104(.a(new_n137_), .b(new_n99_), .c(new_n132_), .d(new_n45_), .O(z4));
  inv1   g105(.a(new_n45_), .O(z5));
  oai21  g106(.a(new_n104_), .b(x14), .c(new_n103_), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(new_n100_), .c(x03), .O(new_n141_));
  inv1   g108(.a(x03), .O(new_n142_));
  oai21  g109(.a(x20), .b(new_n101_), .c(x06), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(x11), .c(new_n142_), .O(new_n144_));
  oai22  g111(.a(new_n144_), .b(new_n99_), .c(new_n141_), .d(new_n45_), .O(z6));
  inv1   g112(.a(new_n99_), .O(z7));
endmodule


