// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x14), .O(new_n34_));
  inv1   g001(.a(x05), .O(new_n35_));
  inv1   g002(.a(x13), .O(new_n36_));
  nand3  g003(.a(x19), .b(new_n36_), .c(new_n35_), .O(new_n37_));
  inv1   g004(.a(x02), .O(new_n38_));
  inv1   g005(.a(x10), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand4  g007(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(x00), .O(new_n41_));
  nand2  g008(.a(x24), .b(x07), .O(new_n42_));
  inv1   g009(.a(new_n42_), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x05), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n41_), .c(new_n37_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  nand3  g013(.a(new_n40_), .b(x19), .c(x05), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n42_), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(x13), .O(new_n49_));
  inv1   g016(.a(x11), .O(new_n50_));
  nor2   g017(.a(x20), .b(x06), .O(new_n51_));
  nor2   g018(.a(x03), .b(x01), .O(new_n52_));
  nand3  g019(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  aoi21  g020(.a(new_n49_), .b(new_n46_), .c(new_n53_), .O(z0));
  nor2   g021(.a(x06), .b(x04), .O(new_n55_));
  nor2   g022(.a(x11), .b(x09), .O(new_n56_));
  nor2   g023(.a(x16), .b(x12), .O(new_n57_));
  nand3  g024(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  inv1   g026(.a(x01), .O(new_n60_));
  inv1   g027(.a(x03), .O(new_n61_));
  inv1   g028(.a(x08), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  inv1   g030(.a(x17), .O(new_n64_));
  inv1   g031(.a(x20), .O(new_n65_));
  inv1   g032(.a(x22), .O(new_n66_));
  inv1   g033(.a(x23), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n68_));
  nor2   g035(.a(new_n68_), .b(new_n63_), .O(new_n69_));
  nand3  g036(.a(x19), .b(x13), .c(x05), .O(new_n70_));
  nand4  g037(.a(new_n34_), .b(new_n39_), .c(new_n38_), .d(x00), .O(new_n71_));
  nand2  g038(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g039(.a(new_n72_), .b(new_n69_), .c(new_n59_), .O(new_n73_));
  inv1   g040(.a(x09), .O(new_n74_));
  nand3  g041(.a(x23), .b(x22), .c(x20), .O(new_n75_));
  nor3   g042(.a(new_n75_), .b(new_n64_), .c(new_n74_), .O(new_n76_));
  nand2  g043(.a(x03), .b(x01), .O(new_n77_));
  nand3  g044(.a(x21), .b(new_n39_), .c(x04), .O(new_n78_));
  nor2   g045(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand4  g046(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n80_));
  nand3  g047(.a(x08), .b(x06), .c(new_n38_), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n79_), .c(new_n76_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n73_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n40_), .O(new_n85_));
  nor2   g052(.a(x20), .b(x17), .O(new_n86_));
  nor2   g053(.a(x23), .b(x22), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n86_), .c(new_n52_), .d(new_n62_), .O(new_n88_));
  nor2   g055(.a(new_n88_), .b(new_n58_), .O(new_n89_));
  aoi21  g056(.a(new_n34_), .b(x05), .c(x13), .O(new_n90_));
  oai22  g057(.a(new_n90_), .b(new_n42_), .c(new_n37_), .d(x14), .O(new_n91_));
  nand2  g058(.a(x15), .b(new_n35_), .O(new_n92_));
  and2   g059(.a(x24), .b(x18), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(x05), .O(new_n94_));
  nand4  g061(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n95_));
  aoi21  g062(.a(new_n94_), .b(new_n92_), .c(new_n95_), .O(new_n96_));
  inv1   g063(.a(new_n75_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(x17), .c(x09), .O(new_n98_));
  inv1   g065(.a(new_n80_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(x04), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  aoi22  g068(.a(new_n101_), .b(new_n96_), .c(new_n91_), .d(new_n89_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n85_), .O(z1));
  nand4  g070(.a(new_n40_), .b(x21), .c(new_n39_), .d(new_n38_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n94_), .c(new_n92_), .O(new_n105_));
  nand3  g072(.a(x20), .b(x14), .c(x11), .O(new_n106_));
  nor2   g073(.a(new_n106_), .b(new_n77_), .O(new_n107_));
  and2   g074(.a(new_n107_), .b(x06), .O(new_n108_));
  and2   g075(.a(new_n108_), .b(new_n105_), .O(z2));
  nand4  g076(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n62_), .O(new_n110_));
  aoi21  g077(.a(new_n71_), .b(new_n70_), .c(new_n110_), .O(new_n111_));
  nand2  g078(.a(x08), .b(x06), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(x02), .O(new_n113_));
  nand4  g080(.a(new_n107_), .b(new_n113_), .c(x21), .d(new_n39_), .O(new_n114_));
  inv1   g081(.a(new_n114_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n111_), .c(new_n40_), .O(new_n116_));
  inv1   g083(.a(new_n106_), .O(new_n117_));
  inv1   g084(.a(new_n110_), .O(new_n118_));
  aoi22  g085(.a(new_n118_), .b(new_n91_), .c(new_n117_), .d(new_n96_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n116_), .O(z3));
  inv1   g087(.a(x04), .O(new_n121_));
  oai21  g088(.a(x23), .b(new_n121_), .c(new_n64_), .O(new_n122_));
  aoi21  g089(.a(new_n122_), .b(new_n66_), .c(x09), .O(new_n123_));
  oai21  g090(.a(new_n123_), .b(x16), .c(new_n62_), .O(new_n124_));
  nand3  g091(.a(new_n40_), .b(x19), .c(x13), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n42_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(x05), .O(new_n127_));
  nand2  g094(.a(new_n43_), .b(x13), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n127_), .c(new_n41_), .d(new_n37_), .O(z5));
  nand2  g096(.a(z5), .b(new_n124_), .O(new_n130_));
  inv1   g097(.a(x16), .O(new_n131_));
  oai21  g098(.a(new_n67_), .b(x04), .c(x17), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(x22), .c(new_n74_), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n131_), .c(x08), .O(new_n134_));
  nand3  g101(.a(new_n40_), .b(x15), .c(x13), .O(new_n135_));
  inv1   g102(.a(new_n135_), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(new_n93_), .c(x05), .O(new_n137_));
  nand2  g104(.a(new_n93_), .b(x13), .O(new_n138_));
  nand3  g105(.a(x15), .b(new_n36_), .c(new_n35_), .O(new_n139_));
  and2   g106(.a(new_n139_), .b(new_n104_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n138_), .c(new_n137_), .O(z7));
  nand2  g108(.a(z7), .b(new_n134_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n130_), .O(z4));
  oai21  g110(.a(new_n51_), .b(x11), .c(new_n61_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n48_), .O(new_n145_));
  nand3  g112(.a(new_n40_), .b(x15), .c(x05), .O(new_n146_));
  inv1   g113(.a(new_n146_), .O(new_n147_));
  oai21  g114(.a(new_n50_), .b(x06), .c(x03), .O(new_n148_));
  oai21  g115(.a(new_n147_), .b(new_n93_), .c(new_n148_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n145_), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(x13), .O(new_n151_));
  nand4  g118(.a(new_n105_), .b(new_n65_), .c(x14), .d(x11), .O(new_n152_));
  aoi21  g119(.a(x20), .b(new_n34_), .c(x06), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(x11), .c(new_n61_), .O(new_n154_));
  nand3  g121(.a(new_n139_), .b(new_n104_), .c(new_n94_), .O(new_n155_));
  aoi22  g122(.a(new_n155_), .b(new_n148_), .c(new_n154_), .d(new_n45_), .O(new_n156_));
  nand3  g123(.a(new_n156_), .b(new_n152_), .c(new_n151_), .O(z6));
endmodule


