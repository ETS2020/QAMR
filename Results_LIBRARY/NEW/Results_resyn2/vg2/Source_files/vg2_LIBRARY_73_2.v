// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nor2   g005(.a(x10), .b(x02), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n36_), .c(x00), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x19), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  nor2   g011(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  inv1   g012(.a(x06), .O(new_n46_));
  inv1   g013(.a(x20), .O(new_n47_));
  nor2   g014(.a(x14), .b(x11), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g016(.a(x01), .O(new_n50_));
  inv1   g017(.a(x03), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor3   g019(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(z0));
  inv1   g020(.a(x17), .O(new_n54_));
  inv1   g021(.a(x22), .O(new_n55_));
  inv1   g022(.a(x23), .O(new_n56_));
  nand3  g023(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  inv1   g024(.a(x04), .O(new_n58_));
  inv1   g025(.a(x16), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(new_n58_), .c(new_n51_), .d(new_n50_), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  inv1   g028(.a(x12), .O(new_n62_));
  nand3  g029(.a(new_n48_), .b(new_n47_), .c(new_n62_), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  inv1   g031(.a(x02), .O(new_n65_));
  inv1   g032(.a(x10), .O(new_n66_));
  nand3  g033(.a(new_n66_), .b(new_n65_), .c(x00), .O(new_n67_));
  inv1   g034(.a(x08), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n68_), .c(new_n46_), .O(new_n70_));
  nand3  g037(.a(x19), .b(x13), .c(x05), .O(new_n71_));
  aoi21  g038(.a(new_n71_), .b(new_n67_), .c(new_n70_), .O(new_n72_));
  nand3  g039(.a(new_n72_), .b(new_n64_), .c(new_n61_), .O(new_n73_));
  nand3  g040(.a(x15), .b(x13), .c(x05), .O(new_n74_));
  nand2  g041(.a(new_n39_), .b(x21), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g043(.a(x08), .b(x06), .O(new_n77_));
  nand4  g044(.a(x09), .b(x04), .c(x03), .d(x01), .O(new_n78_));
  nor2   g045(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand4  g046(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n80_));
  nand4  g047(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n79_), .c(new_n76_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n73_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n36_), .O(new_n85_));
  nor2   g052(.a(new_n41_), .b(new_n36_), .O(new_n86_));
  nor2   g053(.a(new_n80_), .b(new_n62_), .O(new_n87_));
  nand3  g054(.a(x18), .b(x16), .c(x14), .O(new_n88_));
  inv1   g055(.a(new_n88_), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n87_), .c(new_n79_), .d(x11), .O(new_n90_));
  nand4  g057(.a(new_n69_), .b(new_n68_), .c(x07), .d(new_n46_), .O(new_n91_));
  inv1   g058(.a(new_n91_), .O(new_n92_));
  nand3  g059(.a(new_n92_), .b(new_n64_), .c(new_n61_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n86_), .O(new_n95_));
  nand3  g062(.a(x16), .b(x15), .c(x14), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n87_), .c(new_n79_), .d(x11), .O(new_n98_));
  nand4  g065(.a(new_n47_), .b(x19), .c(new_n68_), .d(new_n46_), .O(new_n99_));
  inv1   g066(.a(new_n99_), .O(new_n100_));
  nand3  g067(.a(new_n48_), .b(new_n62_), .c(new_n69_), .O(new_n101_));
  inv1   g068(.a(new_n101_), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n100_), .c(new_n61_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n41_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n95_), .c(new_n85_), .O(z1));
  nand2  g073(.a(x24), .b(x18), .O(new_n107_));
  nand3  g074(.a(new_n36_), .b(x15), .c(x13), .O(new_n108_));
  aoi21  g075(.a(new_n108_), .b(new_n107_), .c(new_n34_), .O(new_n109_));
  nand3  g076(.a(new_n39_), .b(new_n36_), .c(x21), .O(new_n110_));
  inv1   g077(.a(new_n107_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(x13), .O(new_n112_));
  nand2  g079(.a(new_n41_), .b(x15), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n112_), .c(new_n110_), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n109_), .O(new_n115_));
  inv1   g082(.a(x11), .O(new_n116_));
  inv1   g083(.a(x14), .O(new_n117_));
  nand3  g084(.a(x20), .b(x03), .c(x01), .O(new_n118_));
  nor3   g085(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(x06), .O(new_n120_));
  nor2   g087(.a(new_n120_), .b(new_n115_), .O(z2));
  inv1   g088(.a(new_n77_), .O(new_n122_));
  nand3  g089(.a(new_n119_), .b(new_n122_), .c(x18), .O(new_n123_));
  inv1   g090(.a(new_n49_), .O(new_n124_));
  nor2   g091(.a(new_n52_), .b(x08), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(new_n124_), .c(x07), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n86_), .O(new_n128_));
  nand3  g095(.a(new_n119_), .b(new_n122_), .c(x15), .O(new_n129_));
  nand4  g096(.a(new_n100_), .b(new_n48_), .c(new_n51_), .d(new_n50_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n41_), .O(new_n132_));
  nand3  g099(.a(new_n119_), .b(new_n122_), .c(new_n76_), .O(new_n133_));
  nand2  g100(.a(new_n71_), .b(new_n67_), .O(new_n134_));
  nand3  g101(.a(new_n125_), .b(new_n134_), .c(new_n124_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n36_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n132_), .c(new_n128_), .O(z3));
  aoi21  g105(.a(new_n56_), .b(x04), .c(x17), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(x22), .c(new_n69_), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(new_n59_), .c(x08), .O(new_n141_));
  aoi21  g108(.a(x23), .b(new_n58_), .c(new_n54_), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(new_n55_), .c(x09), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(x16), .c(new_n68_), .O(new_n144_));
  oai22  g111(.a(new_n144_), .b(new_n115_), .c(new_n141_), .d(new_n45_), .O(z4));
  inv1   g112(.a(new_n45_), .O(z5));
  nand2  g113(.a(new_n47_), .b(x14), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n74_), .c(x06), .O(new_n148_));
  nand3  g115(.a(new_n148_), .b(new_n76_), .c(new_n36_), .O(new_n149_));
  oai21  g116(.a(x13), .b(x05), .c(new_n46_), .O(new_n150_));
  nand4  g117(.a(new_n47_), .b(x19), .c(x14), .d(x03), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n111_), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n149_), .c(new_n113_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(x11), .O(new_n155_));
  nor2   g122(.a(new_n47_), .b(x14), .O(new_n156_));
  nor2   g123(.a(new_n156_), .b(x06), .O(new_n157_));
  nor2   g124(.a(new_n157_), .b(new_n40_), .O(new_n158_));
  inv1   g125(.a(new_n41_), .O(new_n159_));
  nand2  g126(.a(new_n156_), .b(new_n159_), .O(new_n160_));
  aoi21  g127(.a(new_n160_), .b(new_n46_), .c(new_n35_), .O(new_n161_));
  oai21  g128(.a(new_n161_), .b(new_n158_), .c(new_n116_), .O(new_n162_));
  nand3  g129(.a(new_n36_), .b(x13), .c(x05), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(new_n159_), .O(new_n164_));
  nand2  g131(.a(x19), .b(x03), .O(new_n165_));
  nand3  g132(.a(x20), .b(x19), .c(new_n117_), .O(new_n166_));
  nand2  g133(.a(new_n166_), .b(new_n46_), .O(new_n167_));
  nand2  g134(.a(new_n167_), .b(new_n116_), .O(new_n168_));
  nand2  g135(.a(x15), .b(new_n51_), .O(new_n169_));
  nand3  g136(.a(new_n169_), .b(new_n168_), .c(new_n165_), .O(new_n170_));
  inv1   g137(.a(x21), .O(new_n171_));
  nand2  g138(.a(new_n171_), .b(new_n51_), .O(new_n172_));
  inv1   g139(.a(x00), .O(new_n173_));
  nand2  g140(.a(x03), .b(new_n173_), .O(new_n174_));
  nand4  g141(.a(new_n174_), .b(new_n172_), .c(new_n39_), .d(new_n36_), .O(new_n175_));
  inv1   g142(.a(x18), .O(new_n176_));
  nand2  g143(.a(new_n176_), .b(new_n51_), .O(new_n177_));
  inv1   g144(.a(x07), .O(new_n178_));
  nand2  g145(.a(new_n178_), .b(x03), .O(new_n179_));
  nand3  g146(.a(new_n179_), .b(new_n177_), .c(new_n86_), .O(new_n180_));
  nand2  g147(.a(new_n180_), .b(new_n175_), .O(new_n181_));
  aoi21  g148(.a(new_n170_), .b(new_n164_), .c(new_n181_), .O(new_n182_));
  nand3  g149(.a(new_n182_), .b(new_n162_), .c(new_n155_), .O(z6));
  inv1   g150(.a(new_n115_), .O(z7));
endmodule


