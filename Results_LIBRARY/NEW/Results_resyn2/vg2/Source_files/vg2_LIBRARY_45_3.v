// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand4  g007(.a(new_n36_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  inv1   g010(.a(new_n35_), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(x13), .O(new_n45_));
  nand3  g012(.a(new_n45_), .b(new_n43_), .c(new_n41_), .O(new_n46_));
  nor2   g013(.a(new_n46_), .b(new_n38_), .O(new_n47_));
  nor3   g014(.a(x06), .b(x03), .c(x01), .O(new_n48_));
  nor3   g015(.a(x20), .b(x14), .c(x11), .O(new_n49_));
  nand2  g016(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n47_), .O(z0));
  nand4  g018(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n52_));
  nand2  g019(.a(x09), .b(x04), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand4  g021(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n55_));
  nand4  g022(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand3  g024(.a(new_n57_), .b(new_n54_), .c(x15), .O(new_n58_));
  inv1   g025(.a(x08), .O(new_n59_));
  nand4  g026(.a(new_n49_), .b(new_n48_), .c(x19), .d(new_n59_), .O(new_n60_));
  inv1   g027(.a(x12), .O(new_n61_));
  nor2   g028(.a(x23), .b(x22), .O(new_n62_));
  nor2   g029(.a(x17), .b(x16), .O(new_n63_));
  nor2   g030(.a(x09), .b(x04), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n61_), .O(new_n65_));
  oai21  g032(.a(new_n65_), .b(new_n60_), .c(new_n58_), .O(new_n66_));
  nand2  g033(.a(new_n66_), .b(new_n42_), .O(new_n67_));
  inv1   g034(.a(x01), .O(new_n68_));
  inv1   g035(.a(x03), .O(new_n69_));
  inv1   g036(.a(x06), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  inv1   g038(.a(x04), .O(new_n72_));
  inv1   g039(.a(x09), .O(new_n73_));
  nand3  g040(.a(new_n73_), .b(new_n59_), .c(new_n72_), .O(new_n74_));
  nor2   g041(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand3  g042(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n76_));
  nand3  g043(.a(x19), .b(x13), .c(x05), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  inv1   g045(.a(x11), .O(new_n79_));
  inv1   g046(.a(x20), .O(new_n80_));
  inv1   g047(.a(x22), .O(new_n81_));
  inv1   g048(.a(x23), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n81_), .c(new_n80_), .d(new_n79_), .O(new_n83_));
  inv1   g050(.a(x14), .O(new_n84_));
  inv1   g051(.a(x16), .O(new_n85_));
  inv1   g052(.a(x17), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n61_), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(new_n83_), .O(new_n88_));
  nand3  g055(.a(new_n88_), .b(new_n78_), .c(new_n75_), .O(new_n89_));
  inv1   g056(.a(x15), .O(new_n90_));
  nand2  g057(.a(x13), .b(x05), .O(new_n91_));
  nand3  g058(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n92_));
  oai21  g059(.a(new_n91_), .b(new_n90_), .c(new_n92_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n57_), .c(new_n54_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n89_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n36_), .O(new_n96_));
  nand2  g063(.a(new_n63_), .b(new_n62_), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  nand3  g065(.a(new_n44_), .b(new_n80_), .c(new_n79_), .O(new_n99_));
  inv1   g066(.a(new_n99_), .O(new_n100_));
  nor3   g067(.a(new_n42_), .b(x14), .c(x12), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n100_), .c(new_n75_), .d(new_n98_), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n96_), .c(new_n67_), .O(z1));
  inv1   g070(.a(x13), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n34_), .O(new_n105_));
  oai21  g072(.a(new_n91_), .b(x24), .c(new_n105_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(x15), .O(new_n107_));
  inv1   g074(.a(new_n92_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n36_), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n107_), .O(z7));
  nand3  g077(.a(x20), .b(x14), .c(x11), .O(new_n111_));
  nor4   g078(.a(new_n111_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n112_));
  and2   g079(.a(new_n112_), .b(z7), .O(z2));
  nor2   g080(.a(new_n111_), .b(new_n52_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(x15), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n60_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n42_), .O(new_n117_));
  nor2   g084(.a(new_n50_), .b(x08), .O(new_n118_));
  nand3  g085(.a(new_n118_), .b(new_n105_), .c(new_n44_), .O(new_n119_));
  nand2  g086(.a(new_n114_), .b(new_n93_), .O(new_n120_));
  nand4  g087(.a(new_n78_), .b(new_n49_), .c(new_n48_), .d(new_n59_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n36_), .O(new_n123_));
  nand3  g090(.a(new_n123_), .b(new_n119_), .c(new_n117_), .O(z3));
  oai21  g091(.a(new_n82_), .b(x04), .c(x17), .O(new_n125_));
  aoi21  g092(.a(new_n125_), .b(x22), .c(new_n73_), .O(new_n126_));
  oai21  g093(.a(new_n126_), .b(new_n85_), .c(x08), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(z7), .O(new_n128_));
  oai21  g095(.a(x23), .b(new_n72_), .c(new_n86_), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(new_n81_), .c(x09), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(x16), .c(new_n59_), .O(new_n131_));
  nand2  g098(.a(new_n105_), .b(new_n44_), .O(new_n132_));
  nand2  g099(.a(new_n106_), .b(x19), .O(new_n133_));
  nand3  g100(.a(new_n133_), .b(new_n132_), .c(new_n41_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n128_), .O(z4));
  inv1   g103(.a(new_n47_), .O(z5));
  nand2  g104(.a(x20), .b(new_n84_), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(new_n70_), .c(x11), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(x03), .c(new_n46_), .d(new_n38_), .O(new_n140_));
  aoi21  g107(.a(new_n80_), .b(x14), .c(new_n70_), .O(new_n141_));
  oai21  g108(.a(new_n141_), .b(new_n79_), .c(x03), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(z7), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n140_), .O(z6));
endmodule


