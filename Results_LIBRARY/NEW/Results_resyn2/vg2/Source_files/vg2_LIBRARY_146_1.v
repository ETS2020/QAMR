// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n143_, new_n144_,
    new_n145_, new_n146_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x20), .O(new_n35_));
  nor2   g002(.a(x14), .b(x11), .O(new_n36_));
  nor2   g003(.a(x03), .b(x01), .O(new_n37_));
  nand4  g004(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n38_));
  inv1   g005(.a(x05), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  inv1   g007(.a(x24), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(x19), .c(x13), .O(new_n42_));
  aoi21  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  inv1   g010(.a(x02), .O(new_n44_));
  inv1   g011(.a(x10), .O(new_n45_));
  nand4  g012(.a(new_n41_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n46_));
  nor2   g013(.a(x13), .b(x05), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(x19), .O(new_n48_));
  nand3  g015(.a(x24), .b(x13), .c(x07), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(new_n38_), .O(z0));
  nand4  g019(.a(x23), .b(x22), .c(x16), .d(x14), .O(new_n53_));
  nand4  g020(.a(x08), .b(x04), .c(x03), .d(x01), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g022(.a(x15), .O(new_n56_));
  nand3  g023(.a(x21), .b(new_n45_), .c(new_n44_), .O(new_n57_));
  nand2  g024(.a(x13), .b(x05), .O(new_n58_));
  oai21  g025(.a(new_n58_), .b(new_n56_), .c(new_n57_), .O(new_n59_));
  inv1   g026(.a(x17), .O(new_n60_));
  nand3  g027(.a(x12), .b(x11), .c(x09), .O(new_n61_));
  nor3   g028(.a(new_n61_), .b(new_n35_), .c(new_n60_), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n59_), .c(new_n55_), .O(new_n63_));
  inv1   g030(.a(x01), .O(new_n64_));
  inv1   g031(.a(x03), .O(new_n65_));
  inv1   g032(.a(x04), .O(new_n66_));
  nand3  g033(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  inv1   g034(.a(x11), .O(new_n68_));
  inv1   g035(.a(x12), .O(new_n69_));
  inv1   g036(.a(x14), .O(new_n70_));
  nand4  g037(.a(new_n35_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n71_));
  nor2   g038(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  inv1   g039(.a(x19), .O(new_n73_));
  nand3  g040(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n74_));
  oai21  g041(.a(new_n58_), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  nor2   g042(.a(x23), .b(x22), .O(new_n76_));
  nor2   g043(.a(x16), .b(x09), .O(new_n77_));
  nor2   g044(.a(x08), .b(x06), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(new_n79_));
  inv1   g046(.a(new_n79_), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n75_), .c(new_n72_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n63_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n41_), .O(new_n83_));
  nor2   g050(.a(new_n35_), .b(new_n60_), .O(new_n84_));
  inv1   g051(.a(new_n61_), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n84_), .c(new_n55_), .d(x18), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  nor2   g054(.a(x20), .b(x12), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n37_), .c(new_n36_), .d(new_n66_), .O(new_n89_));
  nand2  g056(.a(new_n77_), .b(new_n76_), .O(new_n90_));
  nand2  g057(.a(new_n78_), .b(x07), .O(new_n91_));
  nor3   g058(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(new_n92_));
  nor2   g059(.a(new_n47_), .b(new_n41_), .O(new_n93_));
  oai21  g060(.a(new_n92_), .b(new_n87_), .c(new_n93_), .O(new_n94_));
  nand4  g061(.a(new_n85_), .b(new_n84_), .c(new_n55_), .d(x15), .O(new_n95_));
  nand4  g062(.a(new_n37_), .b(new_n36_), .c(new_n69_), .d(new_n66_), .O(new_n96_));
  nor2   g063(.a(x20), .b(new_n73_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n78_), .c(new_n77_), .d(new_n76_), .O(new_n98_));
  oai21  g065(.a(new_n98_), .b(new_n96_), .c(new_n95_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n47_), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n94_), .c(new_n83_), .O(z1));
  nand2  g068(.a(x24), .b(x18), .O(new_n102_));
  nand3  g069(.a(new_n41_), .b(x15), .c(x13), .O(new_n103_));
  aoi21  g070(.a(new_n103_), .b(new_n102_), .c(new_n39_), .O(new_n104_));
  nand4  g071(.a(new_n41_), .b(x21), .c(new_n45_), .d(new_n44_), .O(new_n105_));
  nand3  g072(.a(x24), .b(x18), .c(x13), .O(new_n106_));
  nand2  g073(.a(new_n47_), .b(x15), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  nor2   g075(.a(new_n108_), .b(new_n104_), .O(new_n109_));
  nand2  g076(.a(x14), .b(x11), .O(new_n110_));
  nand4  g077(.a(x20), .b(x06), .c(x03), .d(x01), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  inv1   g079(.a(new_n112_), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n109_), .O(z2));
  nand3  g081(.a(new_n112_), .b(new_n59_), .c(x08), .O(new_n115_));
  inv1   g082(.a(x08), .O(new_n116_));
  inv1   g083(.a(new_n38_), .O(new_n117_));
  nand3  g084(.a(new_n75_), .b(new_n117_), .c(new_n116_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n115_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n41_), .O(new_n120_));
  nand4  g087(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n121_));
  nand4  g088(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n122_));
  nand3  g089(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n123_));
  oai22  g090(.a(new_n123_), .b(new_n91_), .c(new_n122_), .d(new_n121_), .O(new_n124_));
  nand2  g091(.a(new_n37_), .b(new_n36_), .O(new_n125_));
  nand3  g092(.a(new_n78_), .b(new_n35_), .c(x19), .O(new_n126_));
  nand4  g093(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n127_));
  oai22  g094(.a(new_n127_), .b(new_n121_), .c(new_n126_), .d(new_n125_), .O(new_n128_));
  aoi22  g095(.a(new_n128_), .b(new_n47_), .c(new_n124_), .d(new_n93_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n120_), .O(z3));
  inv1   g097(.a(x16), .O(new_n131_));
  inv1   g098(.a(x09), .O(new_n132_));
  inv1   g099(.a(x23), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(x04), .c(x17), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(x22), .c(new_n132_), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n131_), .c(x08), .O(new_n136_));
  inv1   g103(.a(x22), .O(new_n137_));
  aoi21  g104(.a(x23), .b(new_n66_), .c(new_n60_), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(new_n137_), .c(x09), .O(new_n139_));
  aoi21  g106(.a(new_n139_), .b(x16), .c(new_n116_), .O(new_n140_));
  oai22  g107(.a(new_n140_), .b(new_n109_), .c(new_n136_), .d(new_n51_), .O(z4));
  inv1   g108(.a(new_n51_), .O(z5));
  oai21  g109(.a(new_n35_), .b(x14), .c(new_n34_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n68_), .c(x03), .O(new_n144_));
  oai21  g111(.a(x20), .b(new_n70_), .c(x06), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(x11), .c(new_n65_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n109_), .c(new_n144_), .d(new_n51_), .O(z6));
  inv1   g114(.a(new_n109_), .O(z7));
endmodule


