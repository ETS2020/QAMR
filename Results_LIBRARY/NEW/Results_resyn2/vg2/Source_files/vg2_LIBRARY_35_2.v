// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x05), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand4  g007(.a(new_n35_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g012(.a(new_n45_), .O(new_n46_));
  nor2   g013(.a(x14), .b(x11), .O(new_n47_));
  nor2   g014(.a(x03), .b(x01), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g016(.a(new_n49_), .O(new_n50_));
  nor2   g017(.a(x20), .b(x06), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g019(.a(new_n46_), .b(new_n38_), .c(new_n52_), .O(z0));
  nand4  g020(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n54_));
  nand3  g021(.a(x11), .b(x09), .c(x04), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  inv1   g023(.a(x15), .O(new_n57_));
  nand3  g024(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n58_));
  nand2  g025(.a(x13), .b(x05), .O(new_n59_));
  oai21  g026(.a(new_n59_), .b(new_n57_), .c(new_n58_), .O(new_n60_));
  nand2  g027(.a(x20), .b(x16), .O(new_n61_));
  nand4  g028(.a(x23), .b(x22), .c(x14), .d(x12), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g030(.a(new_n63_), .b(new_n60_), .c(new_n56_), .O(new_n64_));
  inv1   g031(.a(x17), .O(new_n65_));
  inv1   g032(.a(x22), .O(new_n66_));
  inv1   g033(.a(x23), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  inv1   g035(.a(x01), .O(new_n69_));
  inv1   g036(.a(x03), .O(new_n70_));
  inv1   g037(.a(x04), .O(new_n71_));
  nand3  g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  nor2   g039(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  inv1   g040(.a(x19), .O(new_n74_));
  nand3  g041(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n75_));
  oai21  g042(.a(new_n59_), .b(new_n74_), .c(new_n75_), .O(new_n76_));
  inv1   g043(.a(x11), .O(new_n77_));
  inv1   g044(.a(x12), .O(new_n78_));
  inv1   g045(.a(x14), .O(new_n79_));
  inv1   g046(.a(x20), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n77_), .O(new_n81_));
  inv1   g048(.a(x06), .O(new_n82_));
  inv1   g049(.a(x08), .O(new_n83_));
  inv1   g050(.a(x09), .O(new_n84_));
  nand3  g051(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(new_n81_), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n76_), .c(new_n73_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n64_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n35_), .O(new_n89_));
  inv1   g056(.a(new_n61_), .O(new_n90_));
  inv1   g057(.a(new_n62_), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n90_), .c(new_n56_), .d(x18), .O(new_n92_));
  inv1   g059(.a(new_n81_), .O(new_n93_));
  nand4  g060(.a(new_n84_), .b(new_n83_), .c(x07), .d(new_n82_), .O(new_n94_));
  inv1   g061(.a(new_n94_), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(new_n93_), .c(new_n73_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n92_), .O(new_n97_));
  nor2   g064(.a(new_n42_), .b(new_n35_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g066(.a(x22), .b(x17), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n48_), .c(new_n67_), .d(new_n71_), .O(new_n101_));
  nand4  g068(.a(new_n91_), .b(new_n90_), .c(new_n56_), .d(x15), .O(new_n102_));
  nor2   g069(.a(x09), .b(x08), .O(new_n103_));
  nor2   g070(.a(new_n74_), .b(x12), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n103_), .c(new_n51_), .d(new_n47_), .O(new_n105_));
  oai21  g072(.a(new_n105_), .b(new_n101_), .c(new_n102_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n42_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n99_), .c(new_n89_), .O(z1));
  inv1   g075(.a(x18), .O(new_n109_));
  nand3  g076(.a(new_n35_), .b(x15), .c(x13), .O(new_n110_));
  oai21  g077(.a(new_n35_), .b(new_n109_), .c(new_n110_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(x05), .O(new_n112_));
  nand4  g079(.a(new_n35_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n113_));
  nand3  g080(.a(x24), .b(x18), .c(x13), .O(new_n114_));
  nand2  g081(.a(new_n42_), .b(x15), .O(new_n115_));
  nand3  g082(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  inv1   g083(.a(new_n116_), .O(new_n117_));
  nand3  g084(.a(x20), .b(x14), .c(x11), .O(new_n118_));
  inv1   g085(.a(new_n118_), .O(new_n119_));
  nand4  g086(.a(new_n119_), .b(x06), .c(x03), .d(x01), .O(new_n120_));
  aoi21  g087(.a(new_n117_), .b(new_n112_), .c(new_n120_), .O(z2));
  inv1   g088(.a(new_n54_), .O(new_n122_));
  nand3  g089(.a(new_n119_), .b(new_n60_), .c(new_n122_), .O(new_n123_));
  nor3   g090(.a(x20), .b(x08), .c(x06), .O(new_n124_));
  nand3  g091(.a(new_n124_), .b(new_n76_), .c(new_n50_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n35_), .O(new_n127_));
  nand2  g094(.a(new_n119_), .b(new_n122_), .O(new_n128_));
  nor2   g095(.a(new_n128_), .b(new_n109_), .O(new_n129_));
  nand3  g096(.a(new_n124_), .b(new_n48_), .c(new_n47_), .O(new_n130_));
  nor2   g097(.a(new_n130_), .b(new_n34_), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(new_n129_), .c(new_n98_), .O(new_n132_));
  nor2   g099(.a(new_n128_), .b(new_n57_), .O(new_n133_));
  nor2   g100(.a(new_n130_), .b(new_n74_), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n133_), .c(new_n42_), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n132_), .c(new_n127_), .O(z3));
  nand2  g103(.a(new_n117_), .b(new_n112_), .O(z7));
  nand2  g104(.a(x23), .b(x22), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(x04), .c(x09), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(x16), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(x08), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(z7), .O(new_n142_));
  nand2  g109(.a(new_n46_), .b(new_n38_), .O(z5));
  inv1   g110(.a(x16), .O(new_n144_));
  oai21  g111(.a(x22), .b(new_n65_), .c(new_n84_), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(new_n144_), .c(x08), .O(new_n146_));
  oai21  g113(.a(new_n68_), .b(new_n71_), .c(new_n146_), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(z5), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n142_), .O(z4));
  aoi21  g116(.a(x20), .b(new_n79_), .c(x06), .O(new_n150_));
  oai21  g117(.a(new_n150_), .b(x11), .c(new_n70_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(z5), .O(new_n152_));
  aoi21  g119(.a(new_n80_), .b(x14), .c(new_n82_), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(new_n77_), .c(x03), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(z7), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n152_), .O(z6));
endmodule


