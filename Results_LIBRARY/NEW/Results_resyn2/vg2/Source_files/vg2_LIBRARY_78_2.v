// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(new_n38_), .O(new_n39_));
  inv1   g006(.a(x02), .O(new_n40_));
  inv1   g007(.a(x10), .O(new_n41_));
  nand4  g008(.a(new_n36_), .b(new_n41_), .c(new_n40_), .d(x00), .O(new_n42_));
  nor2   g009(.a(x13), .b(x05), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x19), .O(new_n44_));
  inv1   g011(.a(new_n35_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(x13), .O(new_n46_));
  nand3  g013(.a(new_n46_), .b(new_n44_), .c(new_n42_), .O(new_n47_));
  inv1   g014(.a(new_n47_), .O(new_n48_));
  nor2   g015(.a(x03), .b(x01), .O(new_n49_));
  nor2   g016(.a(x14), .b(x11), .O(new_n50_));
  nand2  g017(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g018(.a(new_n51_), .O(new_n52_));
  nor2   g019(.a(x20), .b(x06), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g021(.a(new_n48_), .b(new_n39_), .c(new_n54_), .O(z0));
  inv1   g022(.a(new_n43_), .O(new_n56_));
  nand4  g023(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n57_));
  inv1   g024(.a(new_n57_), .O(new_n58_));
  nand3  g025(.a(x11), .b(x09), .c(x04), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  nand4  g027(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n61_));
  inv1   g028(.a(new_n61_), .O(new_n62_));
  nand4  g029(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n62_), .c(new_n60_), .d(new_n58_), .O(new_n65_));
  nor2   g032(.a(x14), .b(x12), .O(new_n66_));
  nor2   g033(.a(x17), .b(x16), .O(new_n67_));
  nor2   g034(.a(x11), .b(x09), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n49_), .O(new_n69_));
  nor2   g036(.a(x22), .b(x04), .O(new_n70_));
  nor2   g037(.a(x23), .b(x08), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n53_), .d(new_n45_), .O(new_n72_));
  oai21  g039(.a(new_n72_), .b(new_n69_), .c(new_n65_), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n56_), .O(new_n74_));
  nand4  g041(.a(x23), .b(x22), .c(x20), .d(x15), .O(new_n75_));
  inv1   g042(.a(new_n75_), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n62_), .c(new_n60_), .d(new_n58_), .O(new_n77_));
  nand2  g044(.a(new_n67_), .b(new_n66_), .O(new_n78_));
  inv1   g045(.a(new_n78_), .O(new_n79_));
  nand2  g046(.a(new_n68_), .b(new_n49_), .O(new_n80_));
  inv1   g047(.a(new_n80_), .O(new_n81_));
  inv1   g048(.a(x06), .O(new_n82_));
  inv1   g049(.a(x08), .O(new_n83_));
  inv1   g050(.a(x20), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(x19), .c(new_n83_), .d(new_n82_), .O(new_n85_));
  inv1   g052(.a(new_n85_), .O(new_n86_));
  nor3   g053(.a(x23), .b(x22), .c(x04), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n86_), .c(new_n81_), .d(new_n79_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n77_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n43_), .O(new_n90_));
  nand3  g057(.a(new_n41_), .b(new_n40_), .c(x00), .O(new_n91_));
  nand3  g058(.a(x19), .b(x13), .c(x05), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  inv1   g060(.a(x04), .O(new_n94_));
  nand4  g061(.a(new_n71_), .b(new_n53_), .c(new_n36_), .d(new_n94_), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(new_n69_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n90_), .c(new_n74_), .O(z1));
  nand2  g065(.a(x24), .b(x18), .O(new_n99_));
  nand3  g066(.a(new_n36_), .b(x15), .c(x13), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(x05), .O(new_n102_));
  nand3  g069(.a(x21), .b(new_n41_), .c(new_n40_), .O(new_n103_));
  inv1   g070(.a(new_n103_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n36_), .O(new_n105_));
  inv1   g072(.a(x13), .O(new_n106_));
  nand3  g073(.a(x15), .b(new_n106_), .c(new_n34_), .O(new_n107_));
  nand3  g074(.a(x24), .b(x18), .c(x13), .O(new_n108_));
  and2   g075(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g076(.a(new_n109_), .b(new_n105_), .c(new_n102_), .O(z7));
  nand3  g077(.a(x06), .b(x03), .c(x01), .O(new_n111_));
  nand3  g078(.a(x20), .b(x14), .c(x11), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  and2   g080(.a(new_n113_), .b(z7), .O(z2));
  inv1   g081(.a(x18), .O(new_n115_));
  inv1   g082(.a(new_n112_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n58_), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  inv1   g085(.a(x07), .O(new_n119_));
  nand2  g086(.a(new_n53_), .b(new_n83_), .O(new_n120_));
  nor3   g087(.a(new_n120_), .b(new_n51_), .c(new_n119_), .O(new_n121_));
  nor2   g088(.a(new_n43_), .b(new_n36_), .O(new_n122_));
  oai21  g089(.a(new_n121_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  inv1   g090(.a(x15), .O(new_n124_));
  nor2   g091(.a(new_n117_), .b(new_n124_), .O(new_n125_));
  nor2   g092(.a(new_n85_), .b(new_n51_), .O(new_n126_));
  oai21  g093(.a(new_n126_), .b(new_n125_), .c(new_n43_), .O(new_n127_));
  nand3  g094(.a(x15), .b(x13), .c(x05), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n103_), .O(new_n129_));
  nand3  g096(.a(new_n129_), .b(new_n116_), .c(new_n58_), .O(new_n130_));
  inv1   g097(.a(new_n120_), .O(new_n131_));
  nand3  g098(.a(new_n93_), .b(new_n131_), .c(new_n52_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n36_), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n127_), .c(new_n123_), .O(z3));
  inv1   g102(.a(x22), .O(new_n136_));
  inv1   g103(.a(x17), .O(new_n137_));
  oai21  g104(.a(x23), .b(new_n94_), .c(new_n137_), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(new_n136_), .c(x09), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(x16), .c(new_n83_), .O(new_n140_));
  oai21  g107(.a(new_n43_), .b(new_n35_), .c(new_n44_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  inv1   g109(.a(x09), .O(new_n143_));
  inv1   g110(.a(new_n138_), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(new_n143_), .c(x16), .O(new_n145_));
  aoi21  g112(.a(new_n92_), .b(new_n91_), .c(x24), .O(new_n146_));
  oai21  g113(.a(new_n145_), .b(x08), .c(new_n146_), .O(new_n147_));
  inv1   g114(.a(x16), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(x09), .c(x08), .O(new_n149_));
  oai21  g116(.a(x13), .b(x05), .c(x18), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n107_), .O(new_n151_));
  inv1   g118(.a(x23), .O(new_n152_));
  oai21  g119(.a(new_n152_), .b(x04), .c(x17), .O(new_n153_));
  nor2   g120(.a(new_n136_), .b(new_n148_), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n153_), .c(new_n151_), .O(new_n155_));
  inv1   g122(.a(new_n155_), .O(new_n156_));
  aoi21  g123(.a(new_n149_), .b(z7), .c(new_n156_), .O(new_n157_));
  nand3  g124(.a(new_n157_), .b(new_n147_), .c(new_n142_), .O(z4));
  nand2  g125(.a(new_n48_), .b(new_n39_), .O(z5));
  inv1   g126(.a(x14), .O(new_n160_));
  nand2  g127(.a(x20), .b(new_n160_), .O(new_n161_));
  aoi21  g128(.a(new_n161_), .b(new_n82_), .c(x11), .O(new_n162_));
  oai22  g129(.a(new_n162_), .b(x03), .c(new_n47_), .d(new_n38_), .O(new_n163_));
  inv1   g130(.a(x03), .O(new_n164_));
  inv1   g131(.a(x11), .O(new_n165_));
  nand2  g132(.a(new_n84_), .b(x14), .O(new_n166_));
  aoi21  g133(.a(new_n166_), .b(x06), .c(new_n165_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(new_n164_), .c(z7), .O(new_n168_));
  nand2  g135(.a(new_n168_), .b(new_n163_), .O(z6));
endmodule


