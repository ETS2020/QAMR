// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x05), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n41_));
  inv1   g008(.a(new_n41_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  nor2   g010(.a(x13), .b(x05), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(x19), .O(new_n45_));
  nand3  g012(.a(x24), .b(x13), .c(x07), .O(new_n46_));
  nand3  g013(.a(new_n46_), .b(new_n45_), .c(new_n43_), .O(new_n47_));
  inv1   g014(.a(new_n47_), .O(new_n48_));
  nor3   g015(.a(x06), .b(x03), .c(x01), .O(new_n49_));
  nor3   g016(.a(x20), .b(x14), .c(x11), .O(new_n50_));
  nand2  g017(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  aoi21  g018(.a(new_n48_), .b(new_n38_), .c(new_n51_), .O(z0));
  nor2   g019(.a(new_n44_), .b(new_n35_), .O(new_n53_));
  nor3   g020(.a(x23), .b(x22), .c(x17), .O(new_n54_));
  inv1   g021(.a(x09), .O(new_n55_));
  inv1   g022(.a(x12), .O(new_n56_));
  inv1   g023(.a(x14), .O(new_n57_));
  inv1   g024(.a(x16), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(new_n59_));
  inv1   g026(.a(x04), .O(new_n60_));
  inv1   g027(.a(x08), .O(new_n61_));
  inv1   g028(.a(x11), .O(new_n62_));
  inv1   g029(.a(x20), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  nand2  g032(.a(new_n65_), .b(new_n54_), .O(new_n66_));
  nand2  g033(.a(new_n49_), .b(x07), .O(new_n67_));
  nand4  g034(.a(x14), .b(x11), .c(x09), .d(x04), .O(new_n68_));
  nand4  g035(.a(x20), .b(x12), .c(x08), .d(x06), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g037(.a(x03), .b(x01), .O(new_n71_));
  nand2  g038(.a(x23), .b(x22), .O(new_n72_));
  nand2  g039(.a(x17), .b(x16), .O(new_n73_));
  nor3   g040(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(new_n74_));
  nand3  g041(.a(new_n74_), .b(new_n70_), .c(x18), .O(new_n75_));
  oai21  g042(.a(new_n67_), .b(new_n66_), .c(new_n75_), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n53_), .O(new_n77_));
  inv1   g044(.a(new_n68_), .O(new_n78_));
  nor4   g045(.a(new_n73_), .b(new_n72_), .c(new_n63_), .d(new_n56_), .O(new_n79_));
  inv1   g046(.a(x15), .O(new_n80_));
  nand3  g047(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n81_));
  nand2  g048(.a(x13), .b(x05), .O(new_n82_));
  oai21  g049(.a(new_n82_), .b(new_n80_), .c(new_n81_), .O(new_n83_));
  nand3  g050(.a(x06), .b(x03), .c(x01), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(x08), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n83_), .c(new_n79_), .d(new_n78_), .O(new_n88_));
  nand4  g055(.a(new_n65_), .b(new_n54_), .c(new_n49_), .d(new_n42_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n35_), .O(new_n91_));
  nand2  g058(.a(new_n44_), .b(x15), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n74_), .c(new_n70_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n91_), .c(new_n77_), .O(z1));
  inv1   g062(.a(x18), .O(new_n96_));
  nand3  g063(.a(new_n35_), .b(x15), .c(x13), .O(new_n97_));
  oai21  g064(.a(new_n35_), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(x05), .O(new_n99_));
  inv1   g066(.a(new_n81_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n35_), .O(new_n101_));
  nand3  g068(.a(x24), .b(x18), .c(x13), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n101_), .c(new_n92_), .O(new_n103_));
  inv1   g070(.a(new_n103_), .O(new_n104_));
  nand3  g071(.a(x20), .b(x14), .c(x11), .O(new_n105_));
  inv1   g072(.a(new_n105_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n85_), .O(new_n107_));
  aoi21  g074(.a(new_n104_), .b(new_n99_), .c(new_n107_), .O(z2));
  nand2  g075(.a(new_n106_), .b(new_n87_), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n96_), .O(new_n110_));
  nand2  g077(.a(new_n50_), .b(new_n61_), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n67_), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n110_), .c(new_n53_), .O(new_n113_));
  inv1   g080(.a(x19), .O(new_n114_));
  nand3  g081(.a(new_n50_), .b(new_n49_), .c(new_n61_), .O(new_n115_));
  oai22  g082(.a(new_n115_), .b(new_n114_), .c(new_n109_), .d(new_n80_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n44_), .O(new_n117_));
  inv1   g084(.a(new_n83_), .O(new_n118_));
  oai21  g085(.a(new_n82_), .b(new_n114_), .c(new_n41_), .O(new_n119_));
  inv1   g086(.a(new_n119_), .O(new_n120_));
  oai22  g087(.a(new_n120_), .b(new_n115_), .c(new_n109_), .d(new_n118_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n35_), .O(new_n122_));
  nand3  g089(.a(new_n122_), .b(new_n117_), .c(new_n113_), .O(z3));
  oai21  g090(.a(x16), .b(new_n55_), .c(new_n61_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(x19), .O(new_n125_));
  inv1   g092(.a(x22), .O(new_n126_));
  inv1   g093(.a(x17), .O(new_n127_));
  aoi21  g094(.a(x23), .b(new_n60_), .c(new_n127_), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(new_n126_), .c(x09), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(x16), .c(new_n61_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n80_), .c(new_n125_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n44_), .O(new_n132_));
  nand2  g099(.a(new_n129_), .b(x16), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(x08), .c(new_n96_), .O(new_n134_));
  inv1   g101(.a(x23), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(x04), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(new_n127_), .c(x22), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(x09), .c(new_n58_), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(new_n61_), .c(new_n34_), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(new_n134_), .c(new_n53_), .O(new_n140_));
  nand4  g107(.a(new_n58_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n141_));
  inv1   g108(.a(new_n141_), .O(new_n142_));
  aoi22  g109(.a(new_n142_), .b(new_n137_), .c(new_n124_), .d(new_n119_), .O(new_n143_));
  oai21  g110(.a(new_n130_), .b(new_n118_), .c(new_n143_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n35_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n140_), .c(new_n132_), .O(z4));
  nand2  g113(.a(new_n48_), .b(new_n38_), .O(z5));
  inv1   g114(.a(x03), .O(new_n148_));
  aoi21  g115(.a(x20), .b(new_n57_), .c(x06), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(x11), .c(new_n148_), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(z5), .O(new_n151_));
  nand2  g118(.a(new_n104_), .b(new_n99_), .O(z7));
  inv1   g119(.a(x06), .O(new_n153_));
  aoi21  g120(.a(new_n63_), .b(x14), .c(new_n153_), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(new_n62_), .c(x03), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(z7), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n151_), .O(z6));
endmodule


