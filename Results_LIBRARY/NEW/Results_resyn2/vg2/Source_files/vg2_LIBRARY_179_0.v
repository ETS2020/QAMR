// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_;
  nor3   g000(.a(x20), .b(x14), .c(x06), .O(new_n34_));
  nor3   g001(.a(x11), .b(x03), .c(x01), .O(new_n35_));
  nand2  g002(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  inv1   g003(.a(x05), .O(new_n37_));
  nand2  g004(.a(x24), .b(x07), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(x19), .c(x13), .O(new_n40_));
  aoi21  g007(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  inv1   g008(.a(x02), .O(new_n42_));
  inv1   g009(.a(x10), .O(new_n43_));
  nand4  g010(.a(new_n39_), .b(new_n43_), .c(new_n42_), .d(x00), .O(new_n44_));
  nor2   g011(.a(x13), .b(x05), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(x19), .O(new_n46_));
  inv1   g013(.a(new_n38_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(x13), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n46_), .c(new_n44_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n41_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n36_), .O(z0));
  nand4  g018(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n52_));
  inv1   g019(.a(new_n52_), .O(new_n53_));
  inv1   g020(.a(x04), .O(new_n54_));
  inv1   g021(.a(x09), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g023(.a(x21), .b(new_n43_), .c(new_n42_), .O(new_n57_));
  nand3  g024(.a(x15), .b(x13), .c(x05), .O(new_n58_));
  nand2  g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand4  g026(.a(x22), .b(x20), .c(x14), .d(x11), .O(new_n60_));
  nand4  g027(.a(x23), .b(x17), .c(x16), .d(x12), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(new_n59_), .c(new_n56_), .d(new_n53_), .O(new_n63_));
  nand3  g030(.a(new_n43_), .b(new_n42_), .c(x00), .O(new_n64_));
  nand2  g031(.a(x19), .b(x05), .O(new_n65_));
  nand2  g032(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g033(.a(x11), .O(new_n67_));
  inv1   g034(.a(x12), .O(new_n68_));
  inv1   g035(.a(x16), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n55_), .O(new_n70_));
  inv1   g037(.a(x01), .O(new_n71_));
  inv1   g038(.a(x03), .O(new_n72_));
  inv1   g039(.a(x08), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n54_), .c(new_n72_), .d(new_n71_), .O(new_n74_));
  nor2   g041(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  inv1   g042(.a(x06), .O(new_n76_));
  inv1   g043(.a(x14), .O(new_n77_));
  inv1   g044(.a(x20), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(new_n79_));
  inv1   g046(.a(x17), .O(new_n80_));
  inv1   g047(.a(x22), .O(new_n81_));
  inv1   g048(.a(x23), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  nand3  g051(.a(new_n84_), .b(new_n75_), .c(new_n66_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n63_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n39_), .O(new_n87_));
  nor2   g054(.a(x03), .b(x01), .O(new_n88_));
  nor2   g055(.a(x11), .b(x09), .O(new_n89_));
  nor2   g056(.a(x16), .b(x12), .O(new_n90_));
  nor2   g057(.a(x08), .b(x04), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(new_n88_), .O(new_n92_));
  nor3   g059(.a(x23), .b(x22), .c(x17), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n34_), .O(new_n94_));
  nor2   g061(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g062(.a(x24), .b(x18), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(x13), .c(x12), .O(new_n98_));
  inv1   g065(.a(x13), .O(new_n99_));
  inv1   g066(.a(x15), .O(new_n100_));
  nand3  g067(.a(x24), .b(x18), .c(x05), .O(new_n101_));
  oai21  g068(.a(new_n100_), .b(x05), .c(new_n101_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n99_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  nand2  g071(.a(new_n56_), .b(new_n53_), .O(new_n105_));
  inv1   g072(.a(new_n60_), .O(new_n106_));
  nor2   g073(.a(new_n80_), .b(new_n69_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n106_), .c(x23), .O(new_n108_));
  nor2   g075(.a(new_n108_), .b(new_n105_), .O(new_n109_));
  aoi22  g076(.a(new_n109_), .b(new_n104_), .c(new_n95_), .d(new_n47_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n87_), .O(z1));
  nand3  g078(.a(x06), .b(x03), .c(x01), .O(new_n112_));
  nand3  g079(.a(x20), .b(x14), .c(x11), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  inv1   g081(.a(new_n114_), .O(new_n115_));
  nand3  g082(.a(new_n39_), .b(x15), .c(x13), .O(new_n116_));
  aoi21  g083(.a(new_n116_), .b(new_n96_), .c(new_n37_), .O(new_n117_));
  nand2  g084(.a(new_n97_), .b(x13), .O(new_n118_));
  nand4  g085(.a(new_n39_), .b(x21), .c(new_n43_), .d(new_n42_), .O(new_n119_));
  nand2  g086(.a(new_n45_), .b(x15), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nor2   g088(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nor2   g089(.a(new_n122_), .b(new_n115_), .O(z2));
  nand3  g090(.a(new_n114_), .b(new_n59_), .c(x08), .O(new_n124_));
  nand3  g091(.a(x19), .b(x13), .c(x05), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n64_), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(new_n35_), .c(new_n34_), .d(new_n73_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n39_), .O(new_n129_));
  nand4  g096(.a(new_n35_), .b(new_n34_), .c(new_n73_), .d(x07), .O(new_n130_));
  inv1   g097(.a(new_n113_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n53_), .c(x18), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nor2   g100(.a(new_n45_), .b(new_n39_), .O(new_n134_));
  nand4  g101(.a(new_n35_), .b(new_n34_), .c(x19), .d(new_n73_), .O(new_n135_));
  nand3  g102(.a(new_n131_), .b(new_n53_), .c(x15), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  aoi22  g104(.a(new_n137_), .b(new_n45_), .c(new_n134_), .d(new_n133_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n129_), .O(z3));
  aoi21  g106(.a(new_n82_), .b(x04), .c(x17), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(x22), .c(new_n55_), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(new_n69_), .c(x08), .O(new_n142_));
  aoi21  g109(.a(x23), .b(new_n54_), .c(new_n80_), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(new_n81_), .c(x09), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(x16), .c(new_n73_), .O(new_n145_));
  oai22  g112(.a(new_n145_), .b(new_n122_), .c(new_n142_), .d(new_n50_), .O(z4));
  inv1   g113(.a(new_n50_), .O(z5));
  nand2  g114(.a(x20), .b(new_n77_), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n76_), .O(new_n149_));
  aoi21  g116(.a(new_n149_), .b(new_n67_), .c(x03), .O(new_n150_));
  nand2  g117(.a(new_n78_), .b(x14), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(x06), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(x11), .c(new_n72_), .O(new_n153_));
  oai22  g120(.a(new_n153_), .b(new_n122_), .c(new_n150_), .d(new_n50_), .O(z6));
  inv1   g121(.a(new_n122_), .O(z7));
endmodule


