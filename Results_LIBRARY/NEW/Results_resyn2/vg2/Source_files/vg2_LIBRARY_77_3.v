// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nor2   g005(.a(x10), .b(x02), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n36_), .c(x00), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x19), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  nor2   g011(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  inv1   g012(.a(x06), .O(new_n46_));
  inv1   g013(.a(x20), .O(new_n47_));
  nor2   g014(.a(x14), .b(x11), .O(new_n48_));
  nor2   g015(.a(x03), .b(x01), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n45_), .O(z0));
  nand2  g018(.a(new_n37_), .b(new_n35_), .O(new_n52_));
  nand2  g019(.a(new_n35_), .b(x12), .O(new_n53_));
  nand3  g020(.a(new_n53_), .b(new_n52_), .c(x05), .O(new_n54_));
  inv1   g021(.a(x12), .O(new_n55_));
  nand3  g022(.a(new_n41_), .b(x19), .c(new_n55_), .O(new_n56_));
  nand3  g023(.a(new_n56_), .b(new_n54_), .c(new_n43_), .O(new_n57_));
  nor2   g024(.a(x08), .b(x06), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n59_));
  inv1   g026(.a(x22), .O(new_n60_));
  inv1   g027(.a(x23), .O(new_n61_));
  nor2   g028(.a(x17), .b(x16), .O(new_n62_));
  nor2   g029(.a(x09), .b(x04), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  nand2  g032(.a(new_n65_), .b(new_n57_), .O(new_n66_));
  nand4  g033(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n67_));
  nand4  g034(.a(x12), .b(x11), .c(x09), .d(x04), .O(new_n68_));
  nor2   g035(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g036(.a(x23), .b(x22), .c(x21), .O(new_n70_));
  nand4  g037(.a(x20), .b(x17), .c(x16), .d(x14), .O(new_n71_));
  nor2   g038(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  nand4  g040(.a(new_n63_), .b(new_n62_), .c(new_n58_), .d(new_n48_), .O(new_n74_));
  nor2   g041(.a(x22), .b(x20), .O(new_n75_));
  nor2   g042(.a(x24), .b(x23), .O(new_n76_));
  inv1   g043(.a(x00), .O(new_n77_));
  nor2   g044(.a(x01), .b(new_n77_), .O(new_n78_));
  nor2   g045(.a(x12), .b(x03), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n78_), .c(new_n76_), .d(new_n75_), .O(new_n80_));
  oai21  g047(.a(new_n80_), .b(new_n74_), .c(new_n73_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n39_), .O(new_n82_));
  and2   g049(.a(x13), .b(x05), .O(new_n83_));
  or2    g050(.a(new_n83_), .b(new_n41_), .O(new_n84_));
  nand4  g051(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n85_));
  nor4   g052(.a(new_n85_), .b(new_n61_), .c(new_n60_), .d(new_n47_), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n84_), .c(new_n69_), .O(new_n87_));
  nand3  g054(.a(new_n87_), .b(new_n82_), .c(new_n66_), .O(z1));
  nand2  g055(.a(x24), .b(x18), .O(new_n89_));
  nand3  g056(.a(new_n36_), .b(x15), .c(x13), .O(new_n90_));
  aoi21  g057(.a(new_n90_), .b(new_n89_), .c(new_n34_), .O(new_n91_));
  nand3  g058(.a(new_n39_), .b(new_n36_), .c(x21), .O(new_n92_));
  nand3  g059(.a(x24), .b(x18), .c(x13), .O(new_n93_));
  nand2  g060(.a(new_n41_), .b(x15), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand3  g063(.a(x20), .b(x14), .c(x11), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(x06), .c(x03), .d(x01), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n96_), .O(z2));
  inv1   g067(.a(x18), .O(new_n101_));
  inv1   g068(.a(new_n67_), .O(new_n102_));
  nand2  g069(.a(new_n98_), .b(new_n102_), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  inv1   g071(.a(x07), .O(new_n105_));
  nor2   g072(.a(new_n59_), .b(new_n105_), .O(new_n106_));
  nor2   g073(.a(new_n41_), .b(new_n36_), .O(new_n107_));
  oai21  g074(.a(new_n106_), .b(new_n104_), .c(new_n107_), .O(new_n108_));
  inv1   g075(.a(x15), .O(new_n109_));
  nor2   g076(.a(new_n103_), .b(new_n109_), .O(new_n110_));
  inv1   g077(.a(x19), .O(new_n111_));
  nor2   g078(.a(new_n59_), .b(new_n111_), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n110_), .c(new_n41_), .O(new_n113_));
  nand2  g080(.a(new_n39_), .b(x21), .O(new_n114_));
  nand2  g081(.a(new_n83_), .b(x15), .O(new_n115_));
  aoi21  g082(.a(new_n115_), .b(new_n114_), .c(new_n103_), .O(new_n116_));
  nand2  g083(.a(new_n39_), .b(x00), .O(new_n117_));
  nand2  g084(.a(new_n83_), .b(x19), .O(new_n118_));
  aoi21  g085(.a(new_n118_), .b(new_n117_), .c(new_n59_), .O(new_n119_));
  oai21  g086(.a(new_n119_), .b(new_n116_), .c(new_n36_), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(new_n113_), .c(new_n108_), .O(z3));
  inv1   g088(.a(x16), .O(new_n122_));
  inv1   g089(.a(x09), .O(new_n123_));
  aoi21  g090(.a(new_n61_), .b(x04), .c(x17), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(x22), .c(new_n123_), .O(new_n125_));
  aoi21  g092(.a(new_n125_), .b(new_n122_), .c(x08), .O(new_n126_));
  inv1   g093(.a(x08), .O(new_n127_));
  inv1   g094(.a(x04), .O(new_n128_));
  inv1   g095(.a(x17), .O(new_n129_));
  aoi21  g096(.a(x23), .b(new_n128_), .c(new_n129_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n60_), .c(x09), .O(new_n131_));
  aoi21  g098(.a(new_n131_), .b(x16), .c(new_n127_), .O(new_n132_));
  oai22  g099(.a(new_n132_), .b(new_n96_), .c(new_n126_), .d(new_n45_), .O(z4));
  inv1   g100(.a(new_n45_), .O(z5));
  inv1   g101(.a(x11), .O(new_n135_));
  oai21  g102(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(new_n135_), .c(x03), .O(new_n137_));
  inv1   g104(.a(x03), .O(new_n138_));
  inv1   g105(.a(x14), .O(new_n139_));
  oai21  g106(.a(x20), .b(new_n139_), .c(x06), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(x11), .c(new_n138_), .O(new_n141_));
  oai22  g108(.a(new_n141_), .b(new_n96_), .c(new_n137_), .d(new_n45_), .O(z6));
  inv1   g109(.a(new_n96_), .O(z7));
endmodule


