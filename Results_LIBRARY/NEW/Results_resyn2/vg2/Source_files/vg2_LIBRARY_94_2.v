// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_;
  nor2   g000(.a(x06), .b(x03), .O(new_n34_));
  nor3   g001(.a(x20), .b(x14), .c(x11), .O(new_n35_));
  nand2  g002(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  inv1   g003(.a(x01), .O(new_n37_));
  inv1   g004(.a(x02), .O(new_n38_));
  inv1   g005(.a(x10), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand4  g007(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(x00), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(new_n37_), .O(new_n44_));
  nand2  g011(.a(x24), .b(x07), .O(new_n45_));
  nand4  g012(.a(new_n40_), .b(x19), .c(x13), .d(new_n37_), .O(new_n46_));
  oai21  g013(.a(new_n45_), .b(x13), .c(new_n46_), .O(new_n47_));
  nor2   g014(.a(x13), .b(x05), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(x19), .O(new_n49_));
  inv1   g016(.a(new_n49_), .O(new_n50_));
  aoi21  g017(.a(new_n47_), .b(x05), .c(new_n50_), .O(new_n51_));
  aoi21  g018(.a(new_n51_), .b(new_n44_), .c(new_n36_), .O(z0));
  nand2  g019(.a(x15), .b(x05), .O(new_n53_));
  nand3  g020(.a(x21), .b(new_n39_), .c(new_n38_), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand4  g022(.a(x16), .b(x12), .c(x11), .d(x09), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(x20), .b(x14), .c(x08), .d(x04), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand3  g026(.a(x06), .b(x03), .c(x01), .O(new_n60_));
  nand3  g027(.a(x23), .b(x22), .c(x17), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(new_n59_), .c(new_n57_), .d(new_n55_), .O(new_n63_));
  nor2   g030(.a(x09), .b(x04), .O(new_n64_));
  nor2   g031(.a(x16), .b(x12), .O(new_n65_));
  nor2   g032(.a(x22), .b(x20), .O(new_n66_));
  nand3  g033(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  inv1   g034(.a(new_n67_), .O(new_n68_));
  nand3  g035(.a(new_n39_), .b(new_n38_), .c(x00), .O(new_n69_));
  nand3  g036(.a(x19), .b(x13), .c(x05), .O(new_n70_));
  nand2  g037(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g038(.a(x11), .O(new_n72_));
  inv1   g039(.a(x14), .O(new_n73_));
  inv1   g040(.a(x17), .O(new_n74_));
  inv1   g041(.a(x23), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  inv1   g043(.a(x03), .O(new_n77_));
  inv1   g044(.a(x06), .O(new_n78_));
  inv1   g045(.a(x08), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n78_), .c(new_n77_), .d(new_n37_), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(new_n76_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n71_), .c(new_n68_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n63_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n40_), .O(new_n84_));
  nand2  g051(.a(x24), .b(x18), .O(new_n85_));
  inv1   g052(.a(new_n85_), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n62_), .c(new_n59_), .d(new_n57_), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  nand2  g055(.a(x13), .b(x01), .O(new_n89_));
  nor2   g056(.a(new_n48_), .b(new_n45_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n49_), .O(new_n92_));
  nand2  g059(.a(new_n34_), .b(new_n79_), .O(new_n93_));
  nor3   g060(.a(new_n93_), .b(new_n76_), .c(new_n67_), .O(new_n94_));
  aoi21  g061(.a(new_n94_), .b(new_n92_), .c(new_n88_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n84_), .O(z1));
  nand2  g063(.a(new_n55_), .b(new_n40_), .O(new_n97_));
  inv1   g064(.a(new_n60_), .O(new_n98_));
  nand3  g065(.a(x20), .b(x14), .c(x11), .O(new_n99_));
  inv1   g066(.a(new_n99_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  aoi21  g068(.a(new_n97_), .b(new_n85_), .c(new_n101_), .O(z2));
  nand4  g069(.a(new_n100_), .b(new_n98_), .c(new_n55_), .d(x08), .O(new_n103_));
  inv1   g070(.a(new_n80_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n71_), .c(new_n35_), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n40_), .O(new_n107_));
  nor2   g074(.a(new_n101_), .b(new_n79_), .O(new_n108_));
  nand3  g075(.a(new_n35_), .b(new_n34_), .c(new_n79_), .O(new_n109_));
  inv1   g076(.a(new_n109_), .O(new_n110_));
  aoi22  g077(.a(new_n110_), .b(new_n92_), .c(new_n108_), .d(new_n86_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(new_n107_), .O(z3));
  inv1   g079(.a(x16), .O(new_n113_));
  inv1   g080(.a(x09), .O(new_n114_));
  aoi21  g081(.a(new_n75_), .b(x04), .c(x17), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(x22), .c(new_n114_), .O(new_n116_));
  aoi21  g083(.a(new_n116_), .b(new_n113_), .c(x08), .O(new_n117_));
  inv1   g084(.a(x05), .O(new_n118_));
  nand3  g085(.a(new_n40_), .b(x19), .c(x13), .O(new_n119_));
  aoi21  g086(.a(new_n119_), .b(new_n45_), .c(new_n118_), .O(new_n120_));
  nand3  g087(.a(new_n49_), .b(new_n42_), .c(new_n41_), .O(new_n121_));
  nor2   g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  inv1   g089(.a(x22), .O(new_n123_));
  inv1   g090(.a(x04), .O(new_n124_));
  aoi21  g091(.a(x23), .b(new_n124_), .c(new_n74_), .O(new_n125_));
  oai21  g092(.a(new_n125_), .b(new_n123_), .c(x09), .O(new_n126_));
  aoi21  g093(.a(new_n126_), .b(x16), .c(new_n79_), .O(new_n127_));
  nand3  g094(.a(new_n40_), .b(x15), .c(x13), .O(new_n128_));
  aoi21  g095(.a(new_n128_), .b(new_n85_), .c(new_n118_), .O(new_n129_));
  nand4  g096(.a(new_n40_), .b(x21), .c(new_n39_), .d(new_n38_), .O(new_n130_));
  nand2  g097(.a(new_n86_), .b(x13), .O(new_n131_));
  nand2  g098(.a(new_n48_), .b(x15), .O(new_n132_));
  nand3  g099(.a(new_n132_), .b(new_n131_), .c(new_n130_), .O(new_n133_));
  nor2   g100(.a(new_n133_), .b(new_n129_), .O(new_n134_));
  oai22  g101(.a(new_n134_), .b(new_n127_), .c(new_n122_), .d(new_n117_), .O(z4));
  inv1   g102(.a(new_n122_), .O(z5));
  nand2  g103(.a(x20), .b(new_n73_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n78_), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(new_n72_), .c(x03), .O(new_n139_));
  inv1   g106(.a(x20), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(x14), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(x06), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(x11), .c(new_n77_), .O(new_n143_));
  oai22  g110(.a(new_n143_), .b(new_n134_), .c(new_n139_), .d(new_n122_), .O(z6));
  inv1   g111(.a(new_n134_), .O(z7));
endmodule


