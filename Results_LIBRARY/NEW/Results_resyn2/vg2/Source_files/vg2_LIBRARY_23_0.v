// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  inv1   g004(.a(x02), .O(new_n38_));
  inv1   g005(.a(x10), .O(new_n39_));
  nand4  g006(.a(new_n35_), .b(new_n39_), .c(new_n38_), .d(x00), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x19), .O(new_n42_));
  nor2   g009(.a(new_n35_), .b(new_n34_), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x13), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n42_), .c(new_n40_), .O(new_n45_));
  aoi21  g012(.a(new_n37_), .b(x05), .c(new_n45_), .O(new_n46_));
  inv1   g013(.a(x11), .O(new_n47_));
  inv1   g014(.a(x14), .O(new_n48_));
  inv1   g015(.a(x20), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  inv1   g017(.a(new_n50_), .O(new_n51_));
  nor3   g018(.a(x06), .b(x03), .c(x01), .O(new_n52_));
  nand2  g019(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n46_), .O(z0));
  inv1   g021(.a(x06), .O(new_n55_));
  inv1   g022(.a(x08), .O(new_n56_));
  nand2  g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  nor3   g025(.a(x23), .b(x22), .c(x17), .O(new_n59_));
  inv1   g026(.a(x04), .O(new_n60_));
  inv1   g027(.a(x09), .O(new_n61_));
  nor2   g028(.a(x03), .b(x01), .O(new_n62_));
  nor2   g029(.a(x16), .b(x12), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n64_));
  inv1   g031(.a(new_n64_), .O(new_n65_));
  nand3  g032(.a(new_n39_), .b(new_n38_), .c(x00), .O(new_n66_));
  nand3  g033(.a(x19), .b(x13), .c(x05), .O(new_n67_));
  nand2  g034(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n65_), .c(new_n59_), .d(new_n58_), .O(new_n69_));
  nand4  g036(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n70_));
  nor3   g037(.a(new_n70_), .b(new_n61_), .c(new_n60_), .O(new_n71_));
  inv1   g038(.a(x15), .O(new_n72_));
  nand2  g039(.a(x13), .b(x05), .O(new_n73_));
  nor2   g040(.a(x10), .b(x02), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(x21), .O(new_n75_));
  oai21  g042(.a(new_n73_), .b(new_n72_), .c(new_n75_), .O(new_n76_));
  nand4  g043(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n77_));
  nand4  g044(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n78_));
  nor2   g045(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g046(.a(new_n79_), .b(new_n76_), .c(new_n71_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n69_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n35_), .O(new_n82_));
  nor2   g049(.a(new_n41_), .b(new_n35_), .O(new_n83_));
  nand4  g050(.a(new_n65_), .b(new_n59_), .c(new_n58_), .d(x07), .O(new_n84_));
  nor2   g051(.a(new_n77_), .b(new_n48_), .O(new_n85_));
  nand3  g052(.a(x18), .b(x16), .c(x12), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n85_), .c(new_n71_), .d(x11), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n84_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n83_), .O(new_n90_));
  nand3  g057(.a(x16), .b(x15), .c(x12), .O(new_n91_));
  inv1   g058(.a(new_n91_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n85_), .c(new_n71_), .d(x11), .O(new_n93_));
  nand4  g060(.a(new_n65_), .b(new_n59_), .c(new_n58_), .d(x19), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n41_), .O(new_n96_));
  nand3  g063(.a(new_n96_), .b(new_n90_), .c(new_n82_), .O(z1));
  nand3  g064(.a(x06), .b(x03), .c(x01), .O(new_n98_));
  inv1   g065(.a(x18), .O(new_n99_));
  nand3  g066(.a(new_n35_), .b(x15), .c(x13), .O(new_n100_));
  oai21  g067(.a(new_n35_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  nand3  g068(.a(new_n74_), .b(new_n35_), .c(x21), .O(new_n102_));
  nand3  g069(.a(x24), .b(x18), .c(x13), .O(new_n103_));
  nand2  g070(.a(new_n41_), .b(x15), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  aoi21  g072(.a(new_n101_), .b(x05), .c(new_n105_), .O(new_n106_));
  nand3  g073(.a(x20), .b(x14), .c(x11), .O(new_n107_));
  nor3   g074(.a(new_n107_), .b(new_n106_), .c(new_n98_), .O(z2));
  nor2   g075(.a(new_n107_), .b(new_n70_), .O(new_n109_));
  inv1   g076(.a(new_n109_), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(new_n99_), .O(new_n111_));
  nand3  g078(.a(new_n51_), .b(new_n56_), .c(new_n55_), .O(new_n112_));
  nand2  g079(.a(new_n62_), .b(x07), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  oai21  g081(.a(new_n114_), .b(new_n111_), .c(new_n83_), .O(new_n115_));
  nor2   g082(.a(new_n110_), .b(new_n72_), .O(new_n116_));
  nand2  g083(.a(new_n62_), .b(x19), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n112_), .O(new_n118_));
  oai21  g085(.a(new_n118_), .b(new_n116_), .c(new_n41_), .O(new_n119_));
  nand2  g086(.a(new_n109_), .b(new_n76_), .O(new_n120_));
  nand4  g087(.a(new_n68_), .b(new_n52_), .c(new_n51_), .d(new_n56_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n35_), .O(new_n123_));
  nand3  g090(.a(new_n123_), .b(new_n119_), .c(new_n115_), .O(z3));
  inv1   g091(.a(x16), .O(new_n125_));
  inv1   g092(.a(x23), .O(new_n126_));
  aoi21  g093(.a(new_n126_), .b(x04), .c(x17), .O(new_n127_));
  oai21  g094(.a(new_n127_), .b(x22), .c(new_n61_), .O(new_n128_));
  aoi21  g095(.a(new_n128_), .b(new_n125_), .c(x08), .O(new_n129_));
  inv1   g096(.a(x22), .O(new_n130_));
  inv1   g097(.a(x17), .O(new_n131_));
  aoi21  g098(.a(x23), .b(new_n60_), .c(new_n131_), .O(new_n132_));
  oai21  g099(.a(new_n132_), .b(new_n130_), .c(x09), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(x16), .c(new_n56_), .O(new_n134_));
  oai22  g101(.a(new_n134_), .b(new_n106_), .c(new_n129_), .d(new_n46_), .O(z4));
  inv1   g102(.a(new_n46_), .O(z5));
  nand2  g103(.a(x11), .b(new_n55_), .O(new_n137_));
  inv1   g104(.a(new_n41_), .O(new_n138_));
  nand3  g105(.a(new_n49_), .b(x14), .c(x11), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(x03), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(new_n137_), .c(new_n99_), .O(new_n142_));
  inv1   g109(.a(x03), .O(new_n143_));
  nor3   g110(.a(new_n41_), .b(new_n34_), .c(new_n143_), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(new_n142_), .c(x24), .O(new_n145_));
  inv1   g112(.a(x21), .O(new_n146_));
  oai21  g113(.a(x20), .b(new_n48_), .c(x06), .O(new_n147_));
  aoi21  g114(.a(new_n147_), .b(x11), .c(new_n143_), .O(new_n148_));
  nand2  g115(.a(x03), .b(x00), .O(new_n149_));
  oai21  g116(.a(new_n148_), .b(new_n146_), .c(new_n149_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n74_), .c(new_n35_), .O(new_n151_));
  nand2  g118(.a(x20), .b(new_n48_), .O(new_n152_));
  nand3  g119(.a(new_n152_), .b(new_n67_), .c(new_n55_), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n68_), .c(new_n35_), .O(new_n154_));
  oai21  g121(.a(x13), .b(x05), .c(x06), .O(new_n155_));
  nand2  g122(.a(x15), .b(new_n143_), .O(new_n156_));
  oai21  g123(.a(new_n156_), .b(new_n152_), .c(new_n155_), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n43_), .O(new_n158_));
  nand3  g125(.a(new_n158_), .b(new_n154_), .c(new_n42_), .O(new_n159_));
  nor2   g126(.a(new_n73_), .b(x24), .O(new_n160_));
  nor2   g127(.a(new_n160_), .b(new_n41_), .O(new_n161_));
  nand3  g128(.a(new_n49_), .b(x15), .c(x14), .O(new_n162_));
  nand2  g129(.a(new_n162_), .b(x06), .O(new_n163_));
  nand2  g130(.a(new_n163_), .b(x11), .O(new_n164_));
  nand2  g131(.a(x19), .b(x03), .O(new_n165_));
  and2   g132(.a(new_n165_), .b(new_n156_), .O(new_n166_));
  aoi21  g133(.a(new_n166_), .b(new_n164_), .c(new_n161_), .O(new_n167_));
  aoi21  g134(.a(new_n159_), .b(new_n47_), .c(new_n167_), .O(new_n168_));
  nand3  g135(.a(new_n168_), .b(new_n151_), .c(new_n145_), .O(z6));
  inv1   g136(.a(new_n106_), .O(z7));
endmodule


