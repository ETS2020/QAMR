// Benchmark "FAU" written by ABC on Mon Jul 27 20:49:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand3  g01(.a(new_n22_), .b(x03), .c(x02), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nand3  g03(.a(new_n24_), .b(x09), .c(x04), .O(new_n25_));
  inv1   g04(.a(x00), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand4  g07(.a(new_n28_), .b(new_n25_), .c(new_n23_), .d(x05), .O(z0));
  nand3  g08(.a(new_n23_), .b(x11), .c(x04), .O(new_n30_));
  inv1   g09(.a(x11), .O(new_n31_));
  nand4  g10(.a(new_n31_), .b(new_n22_), .c(x03), .d(x02), .O(new_n32_));
  inv1   g11(.a(x01), .O(new_n33_));
  inv1   g12(.a(x05), .O(new_n34_));
  aoi21  g13(.a(new_n27_), .b(new_n33_), .c(new_n34_), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(new_n32_), .c(new_n30_), .O(z1));
  nor2   g15(.a(new_n24_), .b(x09), .O(new_n37_));
  nor2   g16(.a(x12), .b(x11), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g18(.a(new_n32_), .b(x12), .c(x04), .O(new_n40_));
  inv1   g19(.a(x06), .O(new_n41_));
  aoi21  g20(.a(new_n41_), .b(new_n27_), .c(new_n34_), .O(new_n42_));
  nand3  g21(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(z2));
  inv1   g22(.a(x12), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n31_), .O(new_n45_));
  and2   g24(.a(x13), .b(x04), .O(new_n46_));
  oai21  g25(.a(new_n45_), .b(new_n23_), .c(new_n46_), .O(new_n47_));
  inv1   g26(.a(x13), .O(new_n48_));
  nand3  g27(.a(new_n38_), .b(new_n37_), .c(new_n48_), .O(new_n49_));
  inv1   g28(.a(x07), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n27_), .c(new_n34_), .O(new_n51_));
  nand3  g30(.a(new_n51_), .b(new_n49_), .c(new_n47_), .O(z3));
  nand2  g31(.a(x08), .b(x03), .O(new_n53_));
  nand3  g32(.a(x15), .b(x14), .c(x10), .O(new_n54_));
  nor2   g33(.a(new_n54_), .b(new_n53_), .O(z4));
endmodule


