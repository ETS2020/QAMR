// Benchmark "FAU" written by ABC on Mon Jul 27 18:38:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  nand2  g03(.a(x23), .b(x22), .O(new_n48_));
  nand3  g04(.a(x21), .b(x20), .c(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n47_), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  nor2   g11(.a(new_n45_), .b(new_n55_), .O(z01));
  inv1   g12(.a(x01), .O(new_n57_));
  nor2   g13(.a(new_n45_), .b(new_n57_), .O(z02));
  nand2  g14(.a(x08), .b(x02), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z03));
  inv1   g16(.a(x03), .O(new_n61_));
  nor2   g17(.a(new_n45_), .b(new_n61_), .O(z04));
  nand2  g18(.a(x08), .b(x04), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z05));
  nand2  g20(.a(x08), .b(x05), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z06));
  nand2  g22(.a(x08), .b(x06), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z07));
  inv1   g24(.a(x07), .O(new_n69_));
  nor2   g25(.a(new_n45_), .b(new_n69_), .O(z08));
  inv1   g26(.a(x19), .O(new_n71_));
  inv1   g27(.a(x22), .O(new_n72_));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g30(.a(x21), .b(x20), .c(x11), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  aoi21  g32(.a(new_n76_), .b(new_n74_), .c(new_n71_), .O(new_n77_));
  oai22  g33(.a(new_n77_), .b(new_n47_), .c(new_n45_), .d(new_n55_), .O(z09));
  and2   g34(.a(x21), .b(x12), .O(new_n79_));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  aoi21  g36(.a(new_n79_), .b(new_n74_), .c(new_n80_), .O(new_n81_));
  inv1   g37(.a(new_n47_), .O(new_n82_));
  oai21  g38(.a(x20), .b(x19), .c(new_n82_), .O(new_n83_));
  oai22  g39(.a(new_n83_), .b(new_n81_), .c(new_n45_), .d(new_n57_), .O(z10));
  and2   g40(.a(x21), .b(x20), .O(new_n85_));
  nand3  g41(.a(x23), .b(x22), .c(x13), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n51_), .c(new_n85_), .O(new_n87_));
  or2    g43(.a(x21), .b(x20), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n87_), .c(new_n71_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n47_), .c(new_n59_), .O(z11));
  inv1   g46(.a(new_n73_), .O(new_n91_));
  nand4  g47(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n92_));
  aoi21  g48(.a(new_n91_), .b(x14), .c(new_n92_), .O(new_n93_));
  nand2  g49(.a(new_n49_), .b(new_n72_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n82_), .O(new_n95_));
  oai22  g51(.a(new_n95_), .b(new_n93_), .c(new_n45_), .d(new_n61_), .O(z12));
  nand2  g52(.a(new_n52_), .b(x15), .O(new_n97_));
  inv1   g53(.a(x23), .O(new_n98_));
  nor2   g54(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  aoi21  g56(.a(new_n92_), .b(new_n98_), .c(new_n47_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n63_), .O(z13));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n104_));
  inv1   g60(.a(new_n104_), .O(new_n105_));
  inv1   g61(.a(x24), .O(new_n106_));
  oai21  g62(.a(new_n49_), .b(new_n48_), .c(new_n106_), .O(new_n107_));
  nand2  g63(.a(new_n50_), .b(x24), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n107_), .c(new_n105_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n47_), .c(new_n65_), .O(z14));
  inv1   g66(.a(x25), .O(new_n111_));
  aoi21  g67(.a(x26), .b(x17), .c(new_n111_), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n50_), .c(x24), .O(new_n113_));
  nand2  g69(.a(new_n108_), .b(new_n111_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n113_), .c(new_n82_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n67_), .O(z15));
  inv1   g72(.a(x26), .O(new_n117_));
  nand3  g73(.a(x25), .b(x24), .c(x23), .O(new_n118_));
  oai21  g74(.a(new_n92_), .b(new_n118_), .c(new_n117_), .O(new_n119_));
  inv1   g75(.a(x18), .O(new_n120_));
  nand3  g76(.a(new_n52_), .b(new_n50_), .c(new_n120_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n119_), .c(new_n82_), .O(new_n122_));
  oai21  g78(.a(new_n45_), .b(new_n69_), .c(new_n122_), .O(z16));
endmodule


