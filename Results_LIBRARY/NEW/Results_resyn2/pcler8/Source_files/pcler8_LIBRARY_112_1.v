// Benchmark "FAU" written by ABC on Mon Jul 27 18:37:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_;
  and2   g00(.a(x22), .b(x21), .O(new_n45_));
  nand2  g01(.a(x20), .b(x19), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(new_n45_), .c(x23), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  nor3   g08(.a(new_n52_), .b(new_n49_), .c(new_n48_), .O(z00));
  nand2  g09(.a(x08), .b(x00), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z01));
  nand2  g11(.a(x08), .b(x01), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z02));
  nand2  g13(.a(x08), .b(x02), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z03));
  nand2  g15(.a(x08), .b(x03), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z04));
  nand2  g17(.a(x08), .b(x04), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z05));
  nand2  g19(.a(x08), .b(x05), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z06));
  nand2  g21(.a(x08), .b(x06), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z07));
  nand2  g23(.a(x08), .b(x07), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z08));
  inv1   g25(.a(x19), .O(new_n70_));
  nand2  g26(.a(x22), .b(x21), .O(new_n71_));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  nor2   g28(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  and2   g29(.a(x20), .b(x11), .O(new_n74_));
  aoi21  g30(.a(new_n74_), .b(new_n73_), .c(new_n70_), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n52_), .c(new_n54_), .O(z09));
  aoi21  g32(.a(new_n73_), .b(x12), .c(new_n46_), .O(new_n77_));
  inv1   g33(.a(new_n52_), .O(new_n78_));
  oai21  g34(.a(x20), .b(x19), .c(new_n78_), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n77_), .c(new_n56_), .O(z10));
  and2   g36(.a(x24), .b(x23), .O(new_n81_));
  and2   g37(.a(x26), .b(x25), .O(new_n82_));
  nand4  g38(.a(new_n82_), .b(new_n81_), .c(new_n45_), .d(x13), .O(new_n83_));
  inv1   g39(.a(x21), .O(new_n84_));
  nand2  g40(.a(new_n46_), .b(new_n84_), .O(new_n85_));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n85_), .c(new_n50_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n83_), .O(new_n88_));
  inv1   g44(.a(x09), .O(new_n89_));
  nor2   g45(.a(x10), .b(new_n89_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n58_), .O(z11));
  inv1   g48(.a(new_n72_), .O(new_n93_));
  inv1   g49(.a(new_n86_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(x22), .O(new_n95_));
  aoi21  g51(.a(new_n93_), .b(x14), .c(new_n95_), .O(new_n96_));
  oai21  g52(.a(new_n94_), .b(x22), .c(new_n78_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n96_), .c(new_n60_), .O(z12));
  inv1   g54(.a(x23), .O(new_n99_));
  nand4  g55(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n100_));
  nor2   g56(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  inv1   g57(.a(new_n49_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(x15), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g60(.a(new_n100_), .b(new_n99_), .c(new_n52_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n62_), .O(z13));
  inv1   g63(.a(x24), .O(new_n108_));
  nand2  g64(.a(new_n48_), .b(new_n108_), .O(new_n109_));
  nand2  g65(.a(new_n82_), .b(x16), .O(new_n110_));
  nand2  g66(.a(x24), .b(x23), .O(new_n111_));
  nor2   g67(.a(new_n111_), .b(new_n100_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n109_), .c(new_n78_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n64_), .O(z14));
  inv1   g71(.a(x25), .O(new_n116_));
  aoi21  g72(.a(x26), .b(x17), .c(new_n116_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n112_), .O(new_n118_));
  oai21  g74(.a(new_n111_), .b(new_n100_), .c(new_n116_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n118_), .c(new_n78_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n66_), .O(z15));
  aoi21  g77(.a(new_n112_), .b(x25), .c(x26), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand2  g79(.a(new_n102_), .b(new_n123_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n48_), .c(new_n78_), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n122_), .c(new_n68_), .O(z16));
endmodule


