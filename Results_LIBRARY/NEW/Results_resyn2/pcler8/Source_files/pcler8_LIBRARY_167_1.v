// Benchmark "FAU" written by ABC on Mon Jul 27 18:37:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  nand3  g03(.a(x21), .b(x20), .c(x19), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  nand2  g06(.a(x23), .b(x22), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n47_), .O(z00));
  nand2  g10(.a(x08), .b(x00), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z01));
  inv1   g12(.a(x01), .O(new_n57_));
  nor2   g13(.a(new_n45_), .b(new_n57_), .O(z02));
  inv1   g14(.a(x02), .O(new_n59_));
  nor2   g15(.a(new_n45_), .b(new_n59_), .O(z03));
  nand2  g16(.a(x08), .b(x03), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z04));
  nand2  g18(.a(x08), .b(x04), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z05));
  nand2  g20(.a(x08), .b(x05), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z06));
  nand2  g22(.a(x08), .b(x06), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z07));
  nand2  g24(.a(x08), .b(x07), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z08));
  inv1   g26(.a(x19), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n45_), .O(new_n72_));
  inv1   g28(.a(new_n50_), .O(new_n73_));
  inv1   g29(.a(new_n51_), .O(new_n74_));
  nand4  g30(.a(new_n74_), .b(new_n73_), .c(new_n49_), .d(x11), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n46_), .c(x09), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n55_), .O(z09));
  nand2  g34(.a(x20), .b(x19), .O(new_n79_));
  nand4  g35(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand3  g37(.a(x26), .b(x25), .c(x12), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n81_), .c(new_n79_), .O(new_n84_));
  inv1   g40(.a(new_n47_), .O(new_n85_));
  oai21  g41(.a(x20), .b(x19), .c(new_n85_), .O(new_n86_));
  oai22  g42(.a(new_n86_), .b(new_n84_), .c(new_n45_), .d(new_n57_), .O(z10));
  aoi21  g43(.a(new_n52_), .b(x13), .c(new_n48_), .O(new_n88_));
  and2   g44(.a(x20), .b(x19), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(x21), .c(new_n85_), .O(new_n90_));
  oai22  g46(.a(new_n90_), .b(new_n88_), .c(new_n45_), .d(new_n59_), .O(z11));
  and2   g47(.a(x24), .b(x23), .O(new_n92_));
  nand3  g48(.a(x26), .b(x25), .c(x14), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  nand2  g50(.a(new_n49_), .b(x22), .O(new_n95_));
  aoi21  g51(.a(new_n94_), .b(new_n92_), .c(new_n95_), .O(new_n96_));
  oai21  g52(.a(new_n49_), .b(x22), .c(new_n85_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n96_), .c(new_n61_), .O(z12));
  nand2  g54(.a(new_n73_), .b(x15), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n74_), .c(new_n49_), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  and2   g57(.a(x22), .b(x21), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n89_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n100_), .c(new_n85_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n63_), .O(z13));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n51_), .b(new_n48_), .c(new_n109_), .O(new_n110_));
  nand2  g66(.a(new_n81_), .b(new_n89_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n110_), .c(new_n108_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n47_), .c(new_n65_), .O(z14));
  nand3  g69(.a(x26), .b(x25), .c(x17), .O(new_n114_));
  inv1   g70(.a(new_n114_), .O(new_n115_));
  nand4  g71(.a(new_n92_), .b(new_n102_), .c(new_n89_), .d(x25), .O(new_n116_));
  inv1   g72(.a(x25), .O(new_n117_));
  oai21  g73(.a(new_n80_), .b(new_n79_), .c(new_n117_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n47_), .c(new_n67_), .O(z15));
  inv1   g76(.a(x18), .O(new_n121_));
  nand3  g77(.a(new_n52_), .b(new_n49_), .c(new_n121_), .O(new_n122_));
  inv1   g78(.a(x26), .O(new_n123_));
  nand2  g79(.a(new_n116_), .b(new_n123_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n122_), .c(new_n85_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n69_), .O(z16));
endmodule


