// Benchmark "FAU" written by ABC on Mon Jul 27 18:37:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_,
    new_n55_, new_n57_, new_n59_, new_n61_, new_n63_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  nand3  g01(.a(x22), .b(x21), .c(x19), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  nor3   g05(.a(new_n49_), .b(new_n46_), .c(new_n45_), .O(z00));
  nand2  g06(.a(x08), .b(x00), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(z01));
  inv1   g08(.a(x01), .O(new_n53_));
  nor2   g09(.a(new_n47_), .b(new_n53_), .O(z02));
  inv1   g10(.a(x02), .O(new_n55_));
  nor2   g11(.a(new_n47_), .b(new_n55_), .O(z03));
  inv1   g12(.a(x03), .O(new_n57_));
  nor2   g13(.a(new_n47_), .b(new_n57_), .O(z04));
  nand2  g14(.a(x08), .b(x04), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z05));
  nand2  g16(.a(x08), .b(x05), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z06));
  nand2  g18(.a(x08), .b(x06), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z07));
  nand2  g20(.a(x08), .b(x07), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z08));
  inv1   g22(.a(new_n49_), .O(new_n67_));
  nand3  g23(.a(x22), .b(x21), .c(x11), .O(new_n68_));
  oai21  g24(.a(new_n68_), .b(new_n45_), .c(x19), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n51_), .O(z09));
  inv1   g27(.a(new_n45_), .O(new_n72_));
  nand2  g28(.a(x20), .b(x19), .O(new_n73_));
  inv1   g29(.a(x19), .O(new_n74_));
  inv1   g30(.a(x20), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  aoi22  g34(.a(new_n78_), .b(new_n72_), .c(new_n76_), .d(new_n73_), .O(new_n79_));
  oai22  g35(.a(new_n79_), .b(new_n49_), .c(new_n47_), .d(new_n53_), .O(z10));
  nand3  g36(.a(x21), .b(x20), .c(x19), .O(new_n81_));
  nand3  g37(.a(x26), .b(x25), .c(x24), .O(new_n82_));
  nand3  g38(.a(x23), .b(x22), .c(x13), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  inv1   g41(.a(x21), .O(new_n86_));
  nand2  g42(.a(new_n73_), .b(new_n86_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n67_), .O(new_n88_));
  oai22  g44(.a(new_n88_), .b(new_n85_), .c(new_n47_), .d(new_n55_), .O(z11));
  nand4  g45(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n90_));
  aoi21  g46(.a(new_n72_), .b(x14), .c(new_n90_), .O(new_n91_));
  inv1   g47(.a(new_n81_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(x22), .c(new_n67_), .O(new_n93_));
  oai22  g49(.a(new_n93_), .b(new_n91_), .c(new_n47_), .d(new_n57_), .O(z12));
  inv1   g50(.a(x15), .O(new_n95_));
  inv1   g51(.a(x23), .O(new_n96_));
  nor2   g52(.a(new_n90_), .b(new_n96_), .O(new_n97_));
  oai21  g53(.a(new_n82_), .b(new_n95_), .c(new_n97_), .O(new_n98_));
  aoi21  g54(.a(new_n90_), .b(new_n96_), .c(new_n49_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n59_), .O(z13));
  nand3  g57(.a(x26), .b(x25), .c(x16), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n97_), .c(x24), .O(new_n103_));
  inv1   g59(.a(x24), .O(new_n104_));
  oai21  g60(.a(new_n90_), .b(new_n96_), .c(new_n104_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n103_), .c(new_n67_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n61_), .O(z14));
  inv1   g63(.a(new_n46_), .O(new_n108_));
  nand2  g64(.a(x26), .b(x17), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(new_n108_), .c(x24), .d(x23), .O(new_n110_));
  inv1   g66(.a(x25), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(x24), .c(x23), .O(new_n112_));
  nor2   g68(.a(new_n112_), .b(new_n90_), .O(new_n113_));
  aoi21  g69(.a(new_n110_), .b(x25), .c(new_n113_), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n49_), .c(new_n63_), .O(z15));
  nand3  g71(.a(x25), .b(x24), .c(x21), .O(new_n116_));
  inv1   g72(.a(new_n116_), .O(new_n117_));
  nor2   g73(.a(new_n74_), .b(x18), .O(new_n118_));
  nand4  g74(.a(new_n118_), .b(new_n117_), .c(x23), .d(x22), .O(new_n119_));
  inv1   g75(.a(x26), .O(new_n120_));
  nand4  g76(.a(new_n120_), .b(x25), .c(x24), .d(x23), .O(new_n121_));
  nor2   g77(.a(new_n121_), .b(new_n46_), .O(new_n122_));
  aoi21  g78(.a(new_n119_), .b(x26), .c(new_n122_), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n49_), .c(new_n65_), .O(z16));
endmodule


