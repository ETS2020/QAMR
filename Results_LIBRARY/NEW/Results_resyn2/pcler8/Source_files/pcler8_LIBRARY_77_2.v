// Benchmark "FAU" written by ABC on Mon Jul 27 18:37:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x22), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand3  g03(.a(x21), .b(x20), .c(x19), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand2  g05(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  nor3   g10(.a(new_n54_), .b(new_n51_), .c(new_n50_), .O(z00));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z02));
  nand2  g15(.a(x08), .b(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  inv1   g23(.a(x06), .O(new_n68_));
  nor2   g24(.a(new_n52_), .b(new_n68_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  inv1   g27(.a(new_n54_), .O(new_n72_));
  inv1   g28(.a(new_n51_), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(x23), .O(new_n74_));
  nand4  g30(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n74_), .c(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n56_), .O(z09));
  inv1   g34(.a(x19), .O(new_n79_));
  inv1   g35(.a(x20), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n74_), .c(new_n81_), .O(new_n83_));
  aoi21  g39(.a(new_n80_), .b(new_n79_), .c(new_n54_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n58_), .O(z10));
  nand2  g42(.a(new_n81_), .b(x21), .O(new_n87_));
  nand3  g43(.a(x24), .b(x23), .c(x22), .O(new_n88_));
  nand3  g44(.a(x26), .b(x25), .c(x13), .O(new_n89_));
  nor2   g45(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g46(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  oai21  g47(.a(new_n81_), .b(x21), .c(new_n72_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n91_), .c(new_n60_), .O(z11));
  nand2  g49(.a(new_n87_), .b(new_n45_), .O(new_n94_));
  nand4  g50(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  inv1   g51(.a(new_n95_), .O(new_n96_));
  nand4  g52(.a(x26), .b(x24), .c(x23), .d(x14), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n96_), .c(new_n54_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n94_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n62_), .O(z12));
  nand2  g56(.a(new_n73_), .b(x15), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n96_), .c(x23), .O(new_n102_));
  aoi21  g58(.a(new_n95_), .b(new_n46_), .c(new_n54_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n64_), .O(z13));
  nand3  g61(.a(x26), .b(x25), .c(x16), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(new_n49_), .c(new_n47_), .d(x24), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  nand2  g64(.a(new_n50_), .b(new_n108_), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n107_), .c(new_n72_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n66_), .O(z14));
  nor2   g67(.a(new_n88_), .b(new_n48_), .O(new_n112_));
  inv1   g68(.a(x25), .O(new_n113_));
  aoi21  g69(.a(x26), .b(x17), .c(new_n113_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  oai21  g71(.a(new_n88_), .b(new_n48_), .c(new_n113_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n115_), .c(new_n72_), .O(new_n117_));
  oai21  g73(.a(new_n52_), .b(new_n68_), .c(new_n117_), .O(z15));
  inv1   g74(.a(x26), .O(new_n119_));
  nor2   g75(.a(new_n113_), .b(x18), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n112_), .c(new_n119_), .O(new_n121_));
  nand4  g77(.a(new_n119_), .b(x25), .c(x24), .d(x23), .O(new_n122_));
  nor2   g78(.a(new_n122_), .b(new_n95_), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n121_), .c(new_n72_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n70_), .O(z16));
endmodule


