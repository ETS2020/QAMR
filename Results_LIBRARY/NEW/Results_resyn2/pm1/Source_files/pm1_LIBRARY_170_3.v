// Benchmark "FAU" written by ABC on Mon Jul 27 18:08:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  oai21  g04(.a(x13), .b(new_n31_), .c(x12), .O(z01));
  inv1   g05(.a(x13), .O(z05));
  nand2  g06(.a(x06), .b(x05), .O(new_n36_));
  nand3  g07(.a(x11), .b(x08), .c(x07), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(z05), .O(new_n38_));
  and2   g09(.a(x12), .b(x09), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n38_), .O(z02));
  oai21  g11(.a(new_n37_), .b(new_n36_), .c(x11), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(z05), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n39_), .O(z03));
  inv1   g14(.a(x14), .O(z04));
  nand2  g15(.a(x12), .b(new_n31_), .O(new_n45_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  aoi21  g19(.a(new_n47_), .b(new_n45_), .c(new_n48_), .O(z06));
  inv1   g20(.a(x15), .O(z07));
  nand4  g21(.a(new_n47_), .b(x12), .c(x11), .d(x09), .O(new_n51_));
  oai21  g22(.a(z05), .b(new_n32_), .c(new_n31_), .O(new_n52_));
  inv1   g23(.a(x00), .O(new_n53_));
  nor2   g24(.a(x10), .b(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n52_), .c(new_n51_), .O(z08));
  nand2  g26(.a(x12), .b(x11), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x00), .O(new_n58_));
  nand2  g29(.a(new_n32_), .b(new_n31_), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(new_n56_), .c(new_n58_), .O(z09));
  nand2  g31(.a(new_n46_), .b(x12), .O(new_n61_));
  nand3  g32(.a(new_n54_), .b(x11), .c(x09), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n61_), .O(z10));
  aoi21  g34(.a(new_n46_), .b(x12), .c(new_n30_), .O(new_n64_));
  nand2  g35(.a(new_n48_), .b(x12), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(new_n54_), .c(x11), .O(new_n66_));
  nor2   g37(.a(new_n66_), .b(new_n64_), .O(z11));
  nor3   g38(.a(new_n58_), .b(new_n56_), .c(x09), .O(z12));
endmodule


