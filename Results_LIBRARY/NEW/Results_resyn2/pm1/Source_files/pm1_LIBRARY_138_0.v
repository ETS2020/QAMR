// Benchmark "FAU" written by ABC on Mon Jul 27 18:08:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x01), .O(new_n30_));
  nor2   g01(.a(x12), .b(x11), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand3  g07(.a(x12), .b(x11), .c(x09), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n36_), .O(z02));
  nand2  g10(.a(new_n38_), .b(new_n35_), .O(z03));
  inv1   g11(.a(x14), .O(z04));
  inv1   g12(.a(x13), .O(z05));
  nand2  g13(.a(x09), .b(x01), .O(new_n43_));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  aoi21  g16(.a(new_n45_), .b(z01), .c(new_n43_), .O(z06));
  inv1   g17(.a(x15), .O(z07));
  nand2  g18(.a(new_n45_), .b(new_n38_), .O(new_n48_));
  inv1   g19(.a(x10), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x00), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(new_n48_), .c(x11), .O(z08));
  inv1   g23(.a(x02), .O(new_n53_));
  nand3  g24(.a(x11), .b(x09), .c(new_n53_), .O(new_n54_));
  nand2  g25(.a(x12), .b(x11), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(new_n56_));
  nor2   g27(.a(new_n56_), .b(new_n31_), .O(new_n57_));
  aoi21  g28(.a(new_n57_), .b(new_n54_), .c(new_n50_), .O(z09));
  nand2  g29(.a(x04), .b(x03), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(x12), .O(new_n60_));
  nand3  g31(.a(new_n51_), .b(x11), .c(x09), .O(new_n61_));
  aoi21  g32(.a(new_n60_), .b(x02), .c(new_n61_), .O(z10));
  nand2  g33(.a(new_n51_), .b(x11), .O(new_n63_));
  inv1   g34(.a(new_n43_), .O(new_n64_));
  nand2  g35(.a(new_n60_), .b(x02), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g37(.a(x12), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n30_), .O(new_n68_));
  aoi21  g39(.a(new_n68_), .b(new_n66_), .c(new_n63_), .O(z11));
  nor3   g40(.a(new_n55_), .b(new_n50_), .c(x09), .O(z12));
endmodule


