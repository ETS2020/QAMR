// Benchmark "FAU" written by ABC on Mon Jul 27 20:51:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n41_, new_n43_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n62_, new_n63_;
  inv1   g00(.a(x02), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand2  g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g03(.a(x05), .b(new_n26_), .c(new_n28_), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n27_), .c(x03), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n29_), .O(z00));
  inv1   g07(.a(z00), .O(z01));
  inv1   g08(.a(x00), .O(new_n34_));
  inv1   g09(.a(x03), .O(new_n35_));
  inv1   g10(.a(x13), .O(new_n36_));
  nand3  g11(.a(new_n36_), .b(x05), .c(new_n35_), .O(new_n37_));
  nor2   g12(.a(new_n37_), .b(new_n28_), .O(z07));
  nand2  g13(.a(z07), .b(new_n34_), .O(new_n39_));
  nor2   g14(.a(new_n39_), .b(x01), .O(z02));
  nand2  g15(.a(z07), .b(x00), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(x01), .O(z03));
  inv1   g17(.a(x01), .O(new_n43_));
  nor2   g18(.a(new_n39_), .b(new_n43_), .O(z04));
  nor2   g19(.a(new_n41_), .b(new_n43_), .O(z05));
  inv1   g20(.a(x09), .O(new_n46_));
  inv1   g21(.a(x11), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x01), .c(new_n46_), .O(new_n48_));
  and2   g23(.a(x10), .b(x01), .O(new_n49_));
  inv1   g24(.a(x08), .O(new_n50_));
  oai21  g25(.a(new_n50_), .b(x01), .c(new_n34_), .O(new_n51_));
  oai22  g26(.a(new_n51_), .b(new_n49_), .c(new_n48_), .d(new_n34_), .O(new_n52_));
  inv1   g27(.a(x12), .O(new_n53_));
  nand4  g28(.a(x13), .b(new_n53_), .c(x05), .d(new_n26_), .O(new_n54_));
  nor2   g29(.a(new_n54_), .b(x07), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand3  g31(.a(new_n36_), .b(new_n30_), .c(x02), .O(new_n57_));
  nand2  g32(.a(x04), .b(new_n35_), .O(new_n58_));
  aoi21  g33(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(z06));
  aoi21  g34(.a(new_n57_), .b(new_n54_), .c(new_n58_), .O(z08));
  and2   g35(.a(x13), .b(x06), .O(z09));
  nand2  g36(.a(x05), .b(x02), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(x06), .c(new_n35_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(z10));
endmodule


