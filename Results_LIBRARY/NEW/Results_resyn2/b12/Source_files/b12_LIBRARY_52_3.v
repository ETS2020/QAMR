// Benchmark "FAU" written by ABC on Mon Jul 27 21:36:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n57_, new_n58_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n75_;
  inv1   g00(.a(x03), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  oai21  g02(.a(x05), .b(new_n26_), .c(x02), .O(new_n27_));
  inv1   g03(.a(x00), .O(new_n28_));
  nand2  g04(.a(x01), .b(new_n28_), .O(new_n29_));
  aoi21  g05(.a(new_n27_), .b(new_n25_), .c(new_n29_), .O(z0));
  nand2  g06(.a(x03), .b(x01), .O(new_n31_));
  nand3  g07(.a(x06), .b(x05), .c(x01), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x04), .c(x02), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n31_), .c(x00), .O(z1));
  oai21  g10(.a(x10), .b(x07), .c(new_n25_), .O(new_n35_));
  inv1   g11(.a(x07), .O(new_n36_));
  inv1   g12(.a(x10), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(x09), .c(new_n36_), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n35_), .c(x01), .O(new_n39_));
  inv1   g15(.a(x02), .O(new_n40_));
  nor2   g16(.a(x10), .b(x07), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(new_n42_));
  inv1   g18(.a(x05), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(x04), .c(x01), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n42_), .c(new_n40_), .O(new_n45_));
  oai21  g21(.a(new_n45_), .b(new_n39_), .c(new_n28_), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  inv1   g23(.a(x01), .O(new_n48_));
  nand2  g24(.a(x10), .b(x03), .O(new_n49_));
  inv1   g25(.a(x09), .O(new_n50_));
  nand2  g26(.a(new_n37_), .b(new_n50_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n40_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n47_), .c(new_n36_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n46_), .O(z2));
  aoi22  g31(.a(x12), .b(new_n28_), .c(x11), .d(x07), .O(z3));
  nor2   g32(.a(new_n50_), .b(x07), .O(new_n57_));
  nor2   g33(.a(new_n47_), .b(new_n28_), .O(new_n58_));
  aoi22  g34(.a(new_n58_), .b(new_n57_), .c(x11), .d(x07), .O(z4));
  nor2   g35(.a(x12), .b(x00), .O(new_n60_));
  nand3  g36(.a(new_n60_), .b(new_n41_), .c(x13), .O(z5));
  inv1   g37(.a(x14), .O(new_n62_));
  oai21  g38(.a(new_n29_), .b(new_n62_), .c(x02), .O(new_n63_));
  nand3  g39(.a(x09), .b(new_n25_), .c(new_n48_), .O(new_n64_));
  nand2  g40(.a(new_n40_), .b(x01), .O(new_n65_));
  nand2  g41(.a(new_n50_), .b(x03), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  inv1   g43(.a(new_n67_), .O(new_n68_));
  oai21  g44(.a(new_n47_), .b(new_n28_), .c(new_n41_), .O(new_n69_));
  aoi21  g45(.a(new_n68_), .b(new_n63_), .c(new_n69_), .O(z6));
  nand2  g46(.a(new_n65_), .b(x03), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n47_), .O(new_n72_));
  nand3  g48(.a(x09), .b(x03), .c(new_n48_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n72_), .c(new_n41_), .O(z7));
  oai22  g50(.a(new_n73_), .b(x12), .c(x09), .d(new_n28_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n41_), .O(z8));
endmodule


