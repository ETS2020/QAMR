// Benchmark "FAU" written by ABC on Mon Jul 27 21:36:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n56_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n70_;
  inv1   g00(.a(x05), .O(new_n25_));
  nor2   g01(.a(x03), .b(x01), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x04), .c(new_n25_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x02), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand3  g05(.a(x03), .b(new_n29_), .c(x01), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(x00), .O(z0));
  inv1   g07(.a(x06), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x05), .O(new_n33_));
  nand2  g09(.a(x05), .b(x02), .O(new_n34_));
  oai21  g10(.a(x02), .b(x01), .c(new_n34_), .O(new_n35_));
  inv1   g11(.a(x00), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  nand2  g13(.a(x04), .b(x02), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n36_), .O(new_n40_));
  aoi21  g16(.a(new_n35_), .b(new_n33_), .c(new_n40_), .O(z1));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x08), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n42_), .c(new_n36_), .O(new_n44_));
  inv1   g20(.a(x01), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n45_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n29_), .c(new_n44_), .O(z2));
  aoi22  g27(.a(x12), .b(new_n36_), .c(x11), .d(x07), .O(z3));
  nor2   g28(.a(new_n43_), .b(new_n36_), .O(new_n53_));
  nor2   g29(.a(new_n46_), .b(x07), .O(new_n54_));
  aoi22  g30(.a(new_n54_), .b(new_n53_), .c(x11), .d(x07), .O(z4));
  nor2   g31(.a(x12), .b(x00), .O(new_n56_));
  nand3  g32(.a(new_n56_), .b(new_n47_), .c(x13), .O(z5));
  inv1   g33(.a(new_n53_), .O(new_n58_));
  nor2   g34(.a(x02), .b(x01), .O(new_n59_));
  nand2  g35(.a(new_n46_), .b(x03), .O(new_n60_));
  nand2  g36(.a(x09), .b(new_n37_), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand4  g38(.a(x14), .b(x02), .c(x01), .d(new_n36_), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(new_n58_), .d(new_n47_), .O(new_n64_));
  inv1   g40(.a(new_n64_), .O(z6));
  nand3  g41(.a(new_n59_), .b(x09), .c(x03), .O(new_n66_));
  nand2  g42(.a(new_n59_), .b(x03), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n43_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n66_), .c(new_n47_), .O(z7));
  oai22  g45(.a(new_n66_), .b(x12), .c(x09), .d(new_n36_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n47_), .O(z8));
endmodule


