// Benchmark "FAU" written by ABC on Mon Jul 27 21:36:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n57_, new_n58_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  inv1   g02(.a(new_n26_), .O(new_n27_));
  oai21  g03(.a(x05), .b(new_n25_), .c(new_n27_), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  nor2   g06(.a(x02), .b(x01), .O(new_n31_));
  nor2   g07(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g08(.a(new_n25_), .b(new_n29_), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n28_), .c(x00), .O(z0));
  nand3  g10(.a(new_n27_), .b(x06), .c(x05), .O(new_n35_));
  inv1   g11(.a(x01), .O(new_n36_));
  oai21  g12(.a(new_n30_), .b(new_n36_), .c(new_n29_), .O(new_n37_));
  aoi21  g13(.a(new_n25_), .b(new_n30_), .c(x00), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(z1));
  inv1   g16(.a(x00), .O(new_n41_));
  nand2  g17(.a(x07), .b(new_n30_), .O(new_n42_));
  inv1   g18(.a(x07), .O(new_n43_));
  nand2  g19(.a(x09), .b(new_n43_), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n42_), .c(x01), .O(new_n45_));
  oai21  g21(.a(new_n45_), .b(x02), .c(new_n41_), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  nand2  g23(.a(x10), .b(x03), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  inv1   g25(.a(x10), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n48_), .c(new_n36_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n29_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n47_), .c(new_n43_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n46_), .O(z2));
  aoi22  g31(.a(x12), .b(new_n41_), .c(x11), .d(x07), .O(z3));
  inv1   g32(.a(new_n44_), .O(new_n57_));
  nor2   g33(.a(new_n47_), .b(new_n41_), .O(new_n58_));
  aoi22  g34(.a(new_n58_), .b(new_n57_), .c(x11), .d(x07), .O(z4));
  inv1   g35(.a(x12), .O(new_n60_));
  nand4  g36(.a(x13), .b(new_n60_), .c(new_n43_), .d(new_n41_), .O(z5));
  inv1   g37(.a(x14), .O(new_n62_));
  oai21  g38(.a(new_n26_), .b(new_n62_), .c(new_n41_), .O(new_n63_));
  nand3  g39(.a(new_n50_), .b(new_n47_), .c(x00), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n30_), .O(new_n66_));
  nand2  g42(.a(new_n49_), .b(x03), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n31_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n65_), .c(new_n43_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(z6));
  nor2   g46(.a(x10), .b(x07), .O(new_n71_));
  aoi21  g47(.a(new_n31_), .b(x03), .c(new_n47_), .O(new_n72_));
  nand3  g48(.a(new_n31_), .b(new_n49_), .c(x03), .O(new_n73_));
  inv1   g49(.a(new_n73_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n72_), .c(new_n71_), .O(z7));
  nand3  g51(.a(new_n31_), .b(new_n60_), .c(x03), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x09), .O(new_n77_));
  nand2  g53(.a(new_n49_), .b(new_n41_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n77_), .c(new_n71_), .O(z8));
endmodule


