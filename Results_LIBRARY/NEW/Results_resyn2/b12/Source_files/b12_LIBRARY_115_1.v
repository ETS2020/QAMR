// Benchmark "FAU" written by ABC on Mon Jul 27 21:37:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x02), .O(new_n25_));
  nor2   g01(.a(x04), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  nor2   g03(.a(x02), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nor2   g06(.a(new_n25_), .b(new_n27_), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n30_), .c(new_n31_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  nand2  g09(.a(new_n28_), .b(x03), .O(new_n34_));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  nand2  g12(.a(new_n30_), .b(new_n36_), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n35_), .c(x02), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n34_), .c(x00), .O(z1));
  inv1   g15(.a(x00), .O(new_n40_));
  nor2   g16(.a(x08), .b(x07), .O(new_n41_));
  nor2   g17(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  oai21  g18(.a(x10), .b(x07), .c(x03), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  nor2   g20(.a(x10), .b(x07), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n43_), .c(new_n27_), .O(new_n47_));
  nor2   g23(.a(x08), .b(x02), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n47_), .c(new_n42_), .O(z2));
  inv1   g25(.a(x07), .O(new_n50_));
  nand2  g26(.a(x09), .b(new_n25_), .O(new_n51_));
  nand2  g27(.a(x08), .b(x00), .O(new_n52_));
  oai21  g28(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  aoi22  g29(.a(new_n53_), .b(x11), .c(x12), .d(new_n40_), .O(z3));
  inv1   g30(.a(new_n52_), .O(new_n55_));
  nor2   g31(.a(new_n44_), .b(x07), .O(new_n56_));
  aoi22  g32(.a(new_n56_), .b(new_n55_), .c(x11), .d(x07), .O(z4));
  nand2  g33(.a(x12), .b(new_n40_), .O(new_n58_));
  inv1   g34(.a(x08), .O(new_n59_));
  oai21  g35(.a(new_n51_), .b(new_n59_), .c(x00), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(new_n58_), .c(new_n45_), .d(x13), .O(z5));
  nand3  g37(.a(x14), .b(x01), .c(new_n40_), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n52_), .c(new_n25_), .O(new_n63_));
  xor2a  g39(.a(x09), .b(x03), .O(new_n64_));
  nand2  g40(.a(new_n25_), .b(new_n27_), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(new_n64_), .c(new_n45_), .O(new_n66_));
  nor2   g42(.a(new_n66_), .b(new_n63_), .O(z6));
  inv1   g43(.a(x10), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(x09), .c(new_n27_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n25_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n41_), .O(new_n71_));
  nand2  g47(.a(x03), .b(new_n27_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n25_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n71_), .c(new_n45_), .O(z7));
  inv1   g50(.a(x12), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(x09), .c(new_n25_), .O(new_n76_));
  oai22  g52(.a(new_n76_), .b(new_n72_), .c(x09), .d(new_n40_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n45_), .O(z8));
endmodule


