// Benchmark "FAU" written by ABC on Mon Jul 27 21:38:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  nor2   g04(.a(x02), .b(x01), .O(new_n29_));
  inv1   g05(.a(new_n29_), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(new_n28_), .c(x03), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n27_), .c(x00), .O(z0));
  nand4  g08(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n33_));
  inv1   g09(.a(x01), .O(new_n34_));
  inv1   g10(.a(x02), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n34_), .c(new_n35_), .O(new_n37_));
  inv1   g13(.a(x04), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n36_), .c(x00), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n37_), .c(new_n33_), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(z1));
  inv1   g17(.a(x00), .O(new_n42_));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x08), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n34_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n35_), .c(new_n45_), .O(z2));
  inv1   g27(.a(x12), .O(new_n52_));
  nor2   g28(.a(new_n52_), .b(x00), .O(new_n53_));
  inv1   g29(.a(x11), .O(new_n54_));
  nand3  g30(.a(x09), .b(new_n35_), .c(x00), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n43_), .c(new_n54_), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n53_), .O(z3));
  nand4  g33(.a(x09), .b(x08), .c(new_n43_), .d(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  nor2   g35(.a(new_n59_), .b(new_n56_), .O(z4));
  aoi21  g36(.a(x09), .b(new_n35_), .c(x07), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(new_n54_), .c(x00), .O(new_n62_));
  nand2  g38(.a(new_n47_), .b(x13), .O(new_n63_));
  nor2   g39(.a(new_n63_), .b(new_n53_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n62_), .O(z5));
  nand2  g41(.a(new_n46_), .b(x03), .O(new_n66_));
  nand2  g42(.a(x09), .b(new_n36_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n29_), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n42_), .O(new_n69_));
  inv1   g45(.a(x10), .O(new_n70_));
  nand2  g46(.a(x08), .b(x00), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n43_), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n69_), .c(new_n68_), .O(new_n74_));
  inv1   g50(.a(new_n74_), .O(z6));
  aoi21  g51(.a(new_n29_), .b(x03), .c(new_n44_), .O(new_n76_));
  nand3  g52(.a(new_n29_), .b(new_n46_), .c(x03), .O(new_n77_));
  inv1   g53(.a(new_n77_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n76_), .c(new_n47_), .O(z7));
  nand2  g55(.a(x09), .b(new_n35_), .O(new_n80_));
  nand3  g56(.a(new_n52_), .b(x03), .c(new_n34_), .O(new_n81_));
  oai22  g57(.a(new_n81_), .b(new_n80_), .c(x09), .d(new_n42_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n47_), .O(z8));
endmodule


