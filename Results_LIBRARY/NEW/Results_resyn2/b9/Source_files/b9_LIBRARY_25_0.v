// Benchmark "FAU" written by ABC on Mon Jul 27 22:59:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n141_,
    new_n143_, new_n145_, new_n146_, new_n149_;
  inv1   g00(.a(x10), .O(new_n63_));
  aoi22  g01(.a(x40), .b(x36), .c(x35), .d(new_n63_), .O(new_n64_));
  inv1   g02(.a(x04), .O(new_n65_));
  nand2  g03(.a(x39), .b(new_n65_), .O(new_n66_));
  aoi21  g04(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g05(.a(new_n66_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  nand2  g06(.a(x40), .b(x39), .O(new_n69_));
  nor2   g07(.a(new_n69_), .b(x04), .O(new_n70_));
  nand2  g08(.a(new_n70_), .b(x10), .O(new_n71_));
  inv1   g09(.a(new_n71_), .O(new_n72_));
  aoi21  g10(.a(new_n68_), .b(x27), .c(new_n72_), .O(new_n73_));
  oai21  g11(.a(new_n73_), .b(x15), .c(x16), .O(z00));
  inv1   g12(.a(x35), .O(new_n75_));
  nor2   g13(.a(new_n75_), .b(x28), .O(new_n76_));
  oai21  g14(.a(new_n76_), .b(x36), .c(x27), .O(new_n77_));
  nor2   g15(.a(x32), .b(x30), .O(new_n78_));
  nand2  g16(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g17(.a(new_n79_), .b(x04), .O(z01));
  inv1   g18(.a(x28), .O(new_n81_));
  nand2  g19(.a(x35), .b(new_n81_), .O(new_n82_));
  nor2   g20(.a(x27), .b(x08), .O(new_n83_));
  oai21  g21(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  aoi21  g22(.a(x29), .b(x08), .c(x02), .O(new_n85_));
  nor2   g23(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  nand2  g24(.a(new_n86_), .b(new_n84_), .O(z02));
  nand2  g25(.a(x35), .b(x28), .O(new_n88_));
  nand2  g26(.a(new_n88_), .b(x27), .O(new_n89_));
  inv1   g27(.a(x27), .O(new_n90_));
  inv1   g28(.a(x37), .O(new_n91_));
  nand2  g29(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g30(.a(new_n92_), .b(new_n89_), .c(x21), .O(z03));
  inv1   g31(.a(x21), .O(new_n94_));
  nand3  g32(.a(new_n92_), .b(new_n89_), .c(new_n94_), .O(z04));
  aoi21  g33(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g34(.a(z06), .O(z05));
  inv1   g35(.a(x25), .O(new_n98_));
  nand2  g36(.a(new_n98_), .b(x00), .O(new_n99_));
  inv1   g37(.a(x14), .O(new_n100_));
  nand2  g38(.a(x25), .b(new_n100_), .O(new_n101_));
  nand3  g39(.a(new_n101_), .b(new_n99_), .c(x38), .O(new_n102_));
  inv1   g40(.a(x17), .O(new_n103_));
  nor2   g41(.a(new_n103_), .b(x15), .O(new_n104_));
  or2    g42(.a(x33), .b(x31), .O(new_n105_));
  oai21  g43(.a(new_n105_), .b(new_n104_), .c(x14), .O(new_n106_));
  nand3  g44(.a(new_n106_), .b(new_n102_), .c(x03), .O(new_n107_));
  inv1   g45(.a(new_n107_), .O(z07));
  inv1   g46(.a(new_n69_), .O(z08));
  nand4  g47(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n110_));
  inv1   g48(.a(new_n110_), .O(z09));
  inv1   g49(.a(x36), .O(new_n112_));
  aoi21  g50(.a(new_n82_), .b(new_n112_), .c(new_n90_), .O(new_n113_));
  aoi21  g51(.a(new_n113_), .b(x07), .c(z08), .O(new_n114_));
  inv1   g52(.a(x05), .O(new_n115_));
  aoi21  g53(.a(z08), .b(new_n115_), .c(x04), .O(new_n116_));
  nand2  g54(.a(new_n116_), .b(new_n79_), .O(new_n117_));
  nand3  g55(.a(x37), .b(x27), .c(x06), .O(new_n118_));
  oai21  g56(.a(new_n117_), .b(new_n114_), .c(new_n118_), .O(z10));
  inv1   g57(.a(x08), .O(new_n120_));
  nand2  g58(.a(z08), .b(x29), .O(new_n121_));
  aoi21  g59(.a(new_n121_), .b(new_n82_), .c(new_n120_), .O(new_n122_));
  nand2  g60(.a(new_n121_), .b(x27), .O(new_n123_));
  nand3  g61(.a(new_n76_), .b(x27), .c(new_n65_), .O(new_n124_));
  nor2   g62(.a(x30), .b(x09), .O(new_n125_));
  nand2  g63(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  aoi21  g64(.a(new_n123_), .b(new_n122_), .c(new_n126_), .O(z11));
  oai21  g65(.a(x36), .b(x35), .c(new_n70_), .O(new_n128_));
  nand2  g66(.a(new_n128_), .b(new_n67_), .O(new_n129_));
  aoi21  g67(.a(new_n129_), .b(x27), .c(new_n72_), .O(z12));
  aoi21  g68(.a(x36), .b(x35), .c(new_n81_), .O(new_n131_));
  inv1   g69(.a(x13), .O(new_n132_));
  nand2  g70(.a(new_n70_), .b(new_n132_), .O(new_n133_));
  nand2  g71(.a(new_n88_), .b(new_n91_), .O(new_n134_));
  nor2   g72(.a(x19), .b(x18), .O(new_n135_));
  nand3  g73(.a(new_n135_), .b(new_n134_), .c(x20), .O(new_n136_));
  oai21  g74(.a(new_n133_), .b(new_n131_), .c(new_n136_), .O(new_n137_));
  nor2   g75(.a(new_n133_), .b(new_n78_), .O(new_n138_));
  aoi21  g76(.a(new_n137_), .b(x27), .c(new_n138_), .O(z14));
  inv1   g77(.a(z14), .O(z13));
  nand4  g78(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n141_));
  inv1   g79(.a(new_n141_), .O(z15));
  nand2  g80(.a(x22), .b(x01), .O(new_n143_));
  nor2   g81(.a(new_n143_), .b(x23), .O(z16));
  inv1   g82(.a(x24), .O(new_n145_));
  nand4  g83(.a(new_n145_), .b(x23), .c(x22), .d(x01), .O(new_n146_));
  inv1   g84(.a(new_n146_), .O(z17));
  inv1   g85(.a(z11), .O(z18));
  oai21  g86(.a(new_n75_), .b(new_n90_), .c(new_n125_), .O(new_n149_));
  nor2   g87(.a(new_n149_), .b(new_n122_), .O(z20));
  aoi21  g88(.a(new_n123_), .b(new_n122_), .c(new_n126_), .O(z19));
endmodule


