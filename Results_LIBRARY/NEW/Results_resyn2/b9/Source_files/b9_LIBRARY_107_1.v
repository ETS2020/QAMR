// Benchmark "FAU" written by ABC on Mon Jul 27 23:00:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n146_, new_n148_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x27), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  nand2  g004(.a(x36), .b(x27), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(x40), .b(x39), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x04), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n68_), .c(new_n65_), .O(new_n71_));
  oai21  g009(.a(new_n71_), .b(x15), .c(x16), .O(z00));
  inv1   g010(.a(x36), .O(new_n73_));
  inv1   g011(.a(x28), .O(new_n74_));
  nand2  g012(.a(x35), .b(new_n74_), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n63_), .O(new_n76_));
  nor2   g014(.a(x32), .b(x30), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n76_), .c(x04), .O(z01));
  inv1   g017(.a(x08), .O(new_n80_));
  inv1   g018(.a(x29), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x02), .c(new_n70_), .O(z02));
  inv1   g021(.a(x35), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n74_), .c(x27), .O(new_n85_));
  inv1   g023(.a(x37), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n63_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n85_), .c(x21), .O(z03));
  inv1   g026(.a(x21), .O(new_n89_));
  nand3  g027(.a(new_n87_), .b(new_n85_), .c(new_n89_), .O(z04));
  aoi21  g028(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g029(.a(z06), .O(z05));
  inv1   g030(.a(x25), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x00), .O(new_n94_));
  inv1   g032(.a(x14), .O(new_n95_));
  nand2  g033(.a(x25), .b(new_n95_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x38), .O(new_n97_));
  inv1   g035(.a(x17), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(x15), .O(new_n99_));
  or2    g037(.a(x33), .b(x31), .O(new_n100_));
  oai21  g038(.a(new_n100_), .b(new_n99_), .c(x14), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n97_), .c(x03), .O(new_n102_));
  inv1   g040(.a(new_n102_), .O(z07));
  and2   g041(.a(x40), .b(x39), .O(z08));
  nand4  g042(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(z09));
  inv1   g044(.a(x07), .O(new_n107_));
  aoi21  g045(.a(new_n75_), .b(z08), .c(new_n107_), .O(new_n108_));
  nand3  g046(.a(x40), .b(x39), .c(x05), .O(new_n109_));
  aoi21  g047(.a(new_n77_), .b(new_n67_), .c(new_n109_), .O(new_n110_));
  aoi21  g048(.a(new_n108_), .b(new_n76_), .c(new_n110_), .O(new_n111_));
  nand3  g049(.a(x37), .b(x27), .c(x06), .O(new_n112_));
  oai21  g050(.a(new_n111_), .b(x04), .c(new_n112_), .O(z10));
  or2    g051(.a(x30), .b(x09), .O(new_n114_));
  aoi21  g052(.a(x40), .b(x29), .c(new_n74_), .O(new_n115_));
  oai22  g053(.a(new_n115_), .b(new_n80_), .c(x28), .d(new_n63_), .O(new_n116_));
  nor2   g054(.a(x28), .b(new_n63_), .O(new_n117_));
  inv1   g055(.a(x40), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x35), .O(new_n119_));
  aoi22  g057(.a(new_n119_), .b(new_n69_), .c(new_n117_), .d(x04), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n116_), .c(new_n114_), .O(z11));
  inv1   g059(.a(new_n70_), .O(new_n122_));
  nand2  g060(.a(new_n86_), .b(new_n84_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(x27), .O(new_n124_));
  aoi22  g062(.a(new_n124_), .b(new_n122_), .c(new_n86_), .d(new_n74_), .O(new_n125_));
  nor2   g063(.a(new_n118_), .b(x27), .O(new_n126_));
  nor2   g064(.a(new_n123_), .b(x36), .O(new_n127_));
  oai21  g065(.a(new_n127_), .b(new_n126_), .c(new_n66_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n125_), .O(z12));
  nor2   g067(.a(x19), .b(x18), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x20), .O(new_n131_));
  nand2  g069(.a(x36), .b(x35), .O(new_n132_));
  inv1   g070(.a(x04), .O(new_n133_));
  inv1   g071(.a(x13), .O(new_n134_));
  nand4  g072(.a(x40), .b(x39), .c(new_n134_), .d(new_n133_), .O(new_n135_));
  oai22  g073(.a(new_n135_), .b(new_n132_), .c(new_n131_), .d(new_n64_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x27), .O(new_n137_));
  nand3  g075(.a(new_n78_), .b(new_n70_), .c(new_n134_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(z13));
  nand2  g077(.a(new_n132_), .b(new_n77_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n135_), .c(new_n131_), .O(new_n142_));
  aoi21  g080(.a(new_n123_), .b(new_n118_), .c(new_n134_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n77_), .c(new_n124_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n142_), .c(new_n125_), .O(z14));
  nand4  g083(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n146_));
  inv1   g084(.a(new_n146_), .O(z15));
  nand2  g085(.a(x22), .b(x01), .O(new_n148_));
  nor2   g086(.a(new_n148_), .b(x23), .O(z16));
  inv1   g087(.a(x24), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x23), .c(x22), .d(x01), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(z17));
  inv1   g090(.a(new_n75_), .O(new_n153_));
  aoi22  g091(.a(new_n153_), .b(new_n63_), .c(z08), .d(x29), .O(new_n154_));
  nor2   g092(.a(new_n63_), .b(x04), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n153_), .c(new_n114_), .O(new_n156_));
  oai21  g094(.a(new_n154_), .b(new_n80_), .c(new_n156_), .O(z18));
  inv1   g095(.a(x39), .O(new_n158_));
  oai21  g096(.a(new_n126_), .b(new_n84_), .c(new_n158_), .O(new_n159_));
  aoi22  g097(.a(x40), .b(x29), .c(x35), .d(new_n74_), .O(new_n160_));
  oai22  g098(.a(new_n160_), .b(new_n80_), .c(new_n84_), .d(new_n63_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n159_), .c(new_n114_), .O(z20));
  aoi21  g100(.a(new_n120_), .b(new_n116_), .c(new_n114_), .O(z19));
endmodule


