// Benchmark "FAU" written by ABC on Mon Jul 27 23:00:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n92_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n152_, new_n154_, new_n155_,
    new_n157_, new_n158_, new_n159_, new_n161_, new_n162_;
  inv1   g000(.a(x37), .O(new_n63_));
  nand2  g001(.a(x35), .b(x28), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  inv1   g006(.a(x36), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(x40), .b(x39), .O(new_n71_));
  inv1   g009(.a(new_n71_), .O(z08));
  nand3  g010(.a(z08), .b(new_n70_), .c(new_n67_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n66_), .O(new_n74_));
  nor2   g012(.a(new_n71_), .b(x04), .O(new_n75_));
  aoi22  g013(.a(new_n75_), .b(x10), .c(new_n74_), .d(x27), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(x15), .c(x16), .O(z00));
  oai21  g015(.a(new_n68_), .b(x28), .c(new_n69_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x27), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x04), .O(z01));
  aoi21  g020(.a(x29), .b(x08), .c(x02), .O(new_n83_));
  aoi21  g021(.a(new_n68_), .b(x04), .c(new_n83_), .O(new_n84_));
  inv1   g022(.a(x08), .O(new_n85_));
  inv1   g023(.a(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n85_), .c(x04), .O(new_n87_));
  nand4  g025(.a(new_n87_), .b(new_n84_), .c(x40), .d(x39), .O(z02));
  nand2  g026(.a(new_n64_), .b(x27), .O(new_n89_));
  nand2  g027(.a(new_n63_), .b(new_n86_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n89_), .c(x21), .O(z03));
  inv1   g029(.a(x21), .O(new_n92_));
  nand3  g030(.a(new_n90_), .b(new_n89_), .c(new_n92_), .O(z04));
  aoi21  g031(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g032(.a(z06), .O(z05));
  inv1   g033(.a(x25), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x00), .O(new_n97_));
  inv1   g035(.a(x14), .O(new_n98_));
  nand2  g036(.a(x25), .b(new_n98_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n97_), .c(x38), .O(new_n100_));
  inv1   g038(.a(x17), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(x15), .O(new_n102_));
  or2    g040(.a(x33), .b(x31), .O(new_n103_));
  oai21  g041(.a(new_n103_), .b(new_n102_), .c(x14), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n100_), .c(x03), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(z07));
  nand4  g044(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z09));
  inv1   g046(.a(x07), .O(new_n109_));
  aoi21  g047(.a(x40), .b(x39), .c(new_n109_), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n78_), .O(new_n111_));
  nand3  g049(.a(z08), .b(new_n70_), .c(x05), .O(new_n112_));
  aoi21  g050(.a(new_n112_), .b(new_n111_), .c(new_n86_), .O(new_n113_));
  inv1   g051(.a(x05), .O(new_n114_));
  nor3   g052(.a(new_n80_), .b(new_n71_), .c(new_n114_), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n113_), .c(new_n67_), .O(new_n116_));
  nand3  g054(.a(x37), .b(x27), .c(x06), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n116_), .O(z10));
  nor2   g056(.a(x30), .b(x09), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  nand3  g058(.a(x35), .b(x27), .c(new_n67_), .O(new_n121_));
  nand3  g059(.a(x40), .b(x39), .c(x29), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x27), .O(new_n123_));
  aoi21  g061(.a(new_n122_), .b(new_n68_), .c(new_n85_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n121_), .c(x28), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n120_), .O(z11));
  inv1   g065(.a(x10), .O(new_n128_));
  inv1   g066(.a(x39), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(x27), .O(new_n130_));
  nor2   g068(.a(new_n70_), .b(x37), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n130_), .c(new_n128_), .O(new_n132_));
  oai21  g070(.a(x37), .b(x28), .c(x27), .O(new_n133_));
  oai21  g071(.a(new_n71_), .b(x04), .c(new_n133_), .O(new_n134_));
  nor3   g072(.a(x39), .b(x37), .c(x35), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n134_), .c(new_n132_), .O(z12));
  nor3   g075(.a(x32), .b(x30), .c(x27), .O(new_n138_));
  inv1   g076(.a(x13), .O(new_n139_));
  nand2  g077(.a(new_n75_), .b(new_n139_), .O(new_n140_));
  nor2   g078(.a(x19), .b(x18), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x20), .O(new_n142_));
  nand2  g080(.a(new_n65_), .b(x27), .O(new_n143_));
  oai22  g081(.a(new_n143_), .b(new_n142_), .c(new_n140_), .d(new_n138_), .O(z13));
  nand2  g082(.a(new_n142_), .b(new_n140_), .O(new_n145_));
  nand2  g083(.a(x37), .b(x27), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(x39), .c(x13), .O(new_n147_));
  nor2   g085(.a(new_n138_), .b(new_n135_), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(new_n147_), .c(new_n145_), .d(new_n134_), .O(z14));
  nand4  g087(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(z15));
  nand2  g089(.a(x22), .b(x01), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(x23), .O(z16));
  inv1   g091(.a(x24), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x23), .c(x22), .d(x01), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(z17));
  oai21  g094(.a(new_n68_), .b(x28), .c(new_n122_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n123_), .c(x08), .O(new_n158_));
  or2    g096(.a(new_n121_), .b(x28), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n158_), .c(new_n119_), .O(z18));
  nand2  g098(.a(x28), .b(new_n86_), .O(new_n161_));
  oai21  g099(.a(new_n68_), .b(new_n86_), .c(new_n119_), .O(new_n162_));
  aoi21  g100(.a(new_n161_), .b(new_n124_), .c(new_n162_), .O(z20));
  nor2   g101(.a(new_n126_), .b(new_n120_), .O(z19));
endmodule


