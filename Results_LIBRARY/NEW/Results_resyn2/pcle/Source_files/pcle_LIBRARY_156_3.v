// Benchmark "FAU" written by ABC on Mon Jul 27 18:25:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(new_n30_), .b(x17), .c(x16), .d(x15), .O(new_n31_));
  inv1   g03(.a(x09), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(x08), .O(new_n33_));
  nand2  g05(.a(new_n33_), .b(x18), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n31_), .O(z0));
  nand2  g07(.a(x08), .b(x00), .O(new_n36_));
  inv1   g08(.a(x10), .O(new_n37_));
  nand2  g09(.a(new_n33_), .b(new_n37_), .O(new_n38_));
  oai21  g10(.a(new_n38_), .b(x11), .c(new_n36_), .O(z1));
  nand2  g11(.a(x08), .b(x01), .O(new_n40_));
  xnor2a g12(.a(x12), .b(x11), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(new_n38_), .c(new_n40_), .O(z2));
  nand2  g14(.a(x08), .b(x02), .O(new_n43_));
  aoi21  g15(.a(x12), .b(x11), .c(x13), .O(new_n44_));
  nand3  g16(.a(x13), .b(x12), .c(x11), .O(new_n45_));
  inv1   g17(.a(new_n38_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g19(.a(new_n47_), .b(new_n44_), .c(new_n43_), .O(z3));
  nand2  g20(.a(x08), .b(x03), .O(new_n49_));
  inv1   g21(.a(new_n45_), .O(new_n50_));
  nor2   g22(.a(new_n50_), .b(x14), .O(new_n51_));
  nand2  g23(.a(new_n46_), .b(new_n29_), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(z4));
  nand2  g25(.a(x08), .b(x04), .O(new_n54_));
  nor2   g26(.a(new_n30_), .b(x15), .O(new_n55_));
  nand3  g27(.a(new_n50_), .b(x15), .c(x14), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n46_), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z5));
  nand2  g30(.a(x15), .b(x14), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(new_n45_), .c(x16), .O(new_n60_));
  inv1   g32(.a(x16), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n37_), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(new_n56_), .c(new_n60_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n33_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x05), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(z6));
  nand2  g38(.a(x08), .b(x06), .O(new_n67_));
  inv1   g39(.a(x17), .O(new_n68_));
  aoi21  g40(.a(new_n62_), .b(new_n60_), .c(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n68_), .b(x16), .c(x15), .O(new_n70_));
  nor2   g42(.a(new_n70_), .b(new_n29_), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n69_), .c(new_n33_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n67_), .O(z7));
  nand2  g45(.a(x08), .b(x07), .O(new_n74_));
  inv1   g46(.a(x18), .O(new_n75_));
  nand2  g47(.a(new_n31_), .b(new_n75_), .O(new_n76_));
  nand2  g48(.a(x17), .b(x16), .O(new_n77_));
  aoi21  g49(.a(new_n77_), .b(new_n37_), .c(new_n75_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n60_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n76_), .c(new_n33_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n74_), .O(z8));
endmodule


