// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand2  g02(.a(x16), .b(x15), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  and2   g04(.a(x17), .b(x14), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(new_n32_), .c(new_n30_), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(x08), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(new_n35_), .c(x09), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n34_), .O(z0));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x08), .O(new_n41_));
  nand3  g13(.a(new_n35_), .b(x09), .c(new_n41_), .O(new_n42_));
  oai21  g14(.a(new_n42_), .b(x11), .c(new_n40_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n42_), .c(new_n44_), .O(z2));
  nand2  g18(.a(x08), .b(x02), .O(new_n47_));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  inv1   g20(.a(new_n42_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n29_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n48_), .c(new_n47_), .O(z3));
  nand2  g23(.a(x08), .b(x03), .O(new_n52_));
  nor2   g24(.a(new_n30_), .b(x14), .O(new_n53_));
  nand4  g25(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n53_), .c(new_n52_), .O(z4));
  nand2  g28(.a(x08), .b(x04), .O(new_n57_));
  inv1   g29(.a(x15), .O(new_n58_));
  aoi21  g30(.a(new_n54_), .b(new_n58_), .c(new_n42_), .O(new_n59_));
  oai21  g31(.a(new_n54_), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n57_), .O(z5));
  nand2  g33(.a(x08), .b(x05), .O(new_n62_));
  inv1   g34(.a(new_n54_), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(x15), .c(x16), .O(new_n64_));
  nand2  g36(.a(new_n63_), .b(new_n32_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n49_), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n64_), .c(new_n62_), .O(z6));
  nand2  g39(.a(x08), .b(x06), .O(new_n68_));
  nor2   g40(.a(new_n54_), .b(new_n31_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(x17), .O(new_n70_));
  inv1   g42(.a(x17), .O(new_n71_));
  nand2  g43(.a(new_n65_), .b(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n70_), .c(new_n49_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n68_), .O(z7));
  nand2  g46(.a(new_n35_), .b(x09), .O(new_n75_));
  nor2   g47(.a(x18), .b(new_n71_), .O(new_n76_));
  aoi22  g48(.a(new_n76_), .b(new_n69_), .c(new_n37_), .d(new_n34_), .O(new_n77_));
  nand2  g49(.a(x08), .b(x07), .O(new_n78_));
  oai21  g50(.a(new_n77_), .b(new_n75_), .c(new_n78_), .O(z8));
endmodule


