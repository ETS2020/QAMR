// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_;
  nand3  g00(.a(x14), .b(x13), .c(x12), .O(new_n29_));
  nand3  g01(.a(x17), .b(x16), .c(x15), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  nand2  g05(.a(x18), .b(x11), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n33_), .c(new_n30_), .d(new_n29_), .O(z0));
  nand2  g07(.a(x08), .b(x00), .O(new_n36_));
  oai21  g08(.a(new_n33_), .b(x11), .c(new_n36_), .O(z1));
  nand2  g09(.a(x08), .b(x01), .O(new_n38_));
  xnor2a g10(.a(x12), .b(x11), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n33_), .c(new_n38_), .O(z2));
  nand2  g12(.a(x08), .b(x02), .O(new_n41_));
  and2   g13(.a(x12), .b(x11), .O(new_n42_));
  nor2   g14(.a(new_n42_), .b(x13), .O(new_n43_));
  inv1   g15(.a(new_n33_), .O(new_n44_));
  nand2  g16(.a(new_n42_), .b(x13), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n43_), .c(new_n41_), .O(z3));
  nand2  g19(.a(x08), .b(x03), .O(new_n48_));
  nand4  g20(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n49_));
  inv1   g21(.a(x14), .O(new_n50_));
  nand2  g22(.a(new_n45_), .b(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n49_), .c(new_n44_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n48_), .O(z4));
  nand2  g25(.a(x08), .b(x04), .O(new_n54_));
  inv1   g26(.a(x15), .O(new_n55_));
  nor2   g27(.a(new_n49_), .b(new_n55_), .O(new_n56_));
  nand2  g28(.a(new_n49_), .b(new_n55_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n44_), .O(new_n58_));
  oai21  g30(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(z5));
  nand2  g31(.a(x08), .b(x05), .O(new_n60_));
  nand2  g32(.a(new_n56_), .b(x16), .O(new_n61_));
  or2    g33(.a(new_n56_), .b(x16), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n61_), .c(new_n44_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n60_), .O(z6));
  nand2  g36(.a(x08), .b(x06), .O(new_n65_));
  nand3  g37(.a(new_n56_), .b(x17), .c(x16), .O(new_n66_));
  inv1   g38(.a(x17), .O(new_n67_));
  nand2  g39(.a(new_n61_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n44_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n65_), .O(z7));
  oai21  g42(.a(new_n30_), .b(new_n29_), .c(x18), .O(new_n71_));
  inv1   g43(.a(x18), .O(new_n72_));
  nand4  g44(.a(new_n72_), .b(x17), .c(x16), .d(x15), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n49_), .c(new_n71_), .O(new_n74_));
  nor2   g46(.a(new_n72_), .b(x11), .O(new_n75_));
  aoi21  g47(.a(new_n74_), .b(new_n31_), .c(new_n75_), .O(new_n76_));
  nand2  g48(.a(new_n32_), .b(x09), .O(new_n77_));
  nand2  g49(.a(x08), .b(x07), .O(new_n78_));
  oai21  g50(.a(new_n77_), .b(new_n76_), .c(new_n78_), .O(z8));
endmodule


