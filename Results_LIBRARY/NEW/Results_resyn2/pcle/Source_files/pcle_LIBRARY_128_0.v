// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x15), .c(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x16), .O(new_n35_));
  nor3   g07(.a(new_n35_), .b(new_n34_), .c(new_n31_), .O(z0));
  nand2  g08(.a(x08), .b(x00), .O(new_n37_));
  inv1   g09(.a(x11), .O(new_n38_));
  inv1   g10(.a(x15), .O(new_n39_));
  inv1   g11(.a(x18), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n38_), .O(new_n42_));
  oai21  g14(.a(new_n42_), .b(new_n34_), .c(new_n37_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n34_), .c(new_n44_), .O(z2));
  nand2  g18(.a(x08), .b(x02), .O(new_n47_));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  inv1   g20(.a(new_n34_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n29_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n48_), .c(new_n47_), .O(z3));
  nand2  g23(.a(x08), .b(x03), .O(new_n52_));
  nand2  g24(.a(x13), .b(x12), .O(new_n53_));
  nand2  g25(.a(x14), .b(x11), .O(new_n54_));
  aoi21  g26(.a(new_n41_), .b(new_n53_), .c(new_n54_), .O(new_n55_));
  oai21  g27(.a(new_n30_), .b(x14), .c(new_n49_), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(new_n55_), .c(new_n52_), .O(z4));
  nand2  g29(.a(x08), .b(x04), .O(new_n58_));
  nand4  g30(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n39_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n49_), .c(new_n31_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n58_), .O(z5));
  nand2  g34(.a(x08), .b(x05), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  nand2  g36(.a(new_n31_), .b(new_n64_), .O(new_n65_));
  nand3  g37(.a(x16), .b(x15), .c(x14), .O(new_n66_));
  inv1   g38(.a(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n30_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n65_), .c(new_n49_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n63_), .O(z6));
  nand2  g42(.a(x08), .b(x06), .O(new_n71_));
  nand3  g43(.a(new_n67_), .b(new_n30_), .c(x17), .O(new_n72_));
  inv1   g44(.a(x17), .O(new_n73_));
  nand2  g45(.a(new_n68_), .b(new_n73_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n72_), .c(new_n49_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n71_), .O(z7));
  nand2  g48(.a(x08), .b(x07), .O(new_n77_));
  nand3  g49(.a(new_n40_), .b(x17), .c(x16), .O(new_n78_));
  aoi21  g50(.a(new_n78_), .b(x15), .c(new_n59_), .O(new_n79_));
  nand3  g51(.a(x17), .b(x16), .c(x14), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n30_), .c(new_n40_), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n79_), .c(new_n49_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n77_), .O(z8));
endmodule


