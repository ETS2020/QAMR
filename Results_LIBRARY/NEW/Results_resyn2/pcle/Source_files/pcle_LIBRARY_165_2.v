// Benchmark "FAU" written by ABC on Mon Jul 27 18:25:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  nand4  g04(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n33_));
  nor3   g05(.a(new_n33_), .b(new_n32_), .c(new_n29_), .O(z0));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  oai21  g07(.a(new_n32_), .b(x11), .c(new_n35_), .O(z1));
  nand2  g08(.a(x08), .b(x01), .O(new_n37_));
  xnor2a g09(.a(x12), .b(x11), .O(new_n38_));
  oai21  g10(.a(new_n38_), .b(new_n32_), .c(new_n37_), .O(z2));
  nand2  g11(.a(x08), .b(x02), .O(new_n40_));
  aoi21  g12(.a(x12), .b(x11), .c(x13), .O(new_n41_));
  nand3  g13(.a(x13), .b(x12), .c(x11), .O(new_n42_));
  inv1   g14(.a(new_n32_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n41_), .c(new_n40_), .O(z3));
  nand2  g17(.a(x08), .b(x03), .O(new_n46_));
  inv1   g18(.a(new_n42_), .O(new_n47_));
  nor2   g19(.a(new_n47_), .b(x14), .O(new_n48_));
  nand2  g20(.a(new_n43_), .b(new_n29_), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(z4));
  nand2  g22(.a(x08), .b(x04), .O(new_n51_));
  inv1   g23(.a(x15), .O(new_n52_));
  nor2   g24(.a(new_n29_), .b(new_n52_), .O(new_n53_));
  nand2  g25(.a(new_n29_), .b(new_n52_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n53_), .c(new_n51_), .O(z5));
  nand2  g28(.a(x08), .b(x05), .O(new_n57_));
  nand2  g29(.a(new_n53_), .b(x16), .O(new_n58_));
  inv1   g30(.a(x16), .O(new_n59_));
  oai21  g31(.a(new_n29_), .b(new_n52_), .c(new_n59_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n58_), .c(new_n43_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n57_), .O(z6));
  nand2  g34(.a(x08), .b(x06), .O(new_n63_));
  nand3  g35(.a(x16), .b(x15), .c(x14), .O(new_n64_));
  nor2   g36(.a(new_n64_), .b(new_n42_), .O(new_n65_));
  nor2   g37(.a(new_n65_), .b(x17), .O(new_n66_));
  inv1   g38(.a(new_n64_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n47_), .c(x17), .O(new_n68_));
  nand2  g40(.a(x17), .b(x10), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(x09), .c(new_n30_), .O(new_n70_));
  inv1   g42(.a(new_n70_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n66_), .c(new_n63_), .O(z7));
  nand2  g45(.a(x08), .b(x07), .O(new_n74_));
  nand3  g46(.a(new_n65_), .b(x18), .c(x17), .O(new_n75_));
  inv1   g47(.a(x18), .O(new_n76_));
  nand2  g48(.a(new_n68_), .b(new_n76_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n75_), .c(new_n71_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n74_), .O(z8));
endmodule


