// Benchmark "FAU" written by ABC on Mon Jul 27 18:25:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x15), .b(x14), .c(x13), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  nand3  g03(.a(x17), .b(x16), .c(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(new_n31_), .c(x12), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n34_), .c(new_n29_), .O(z0));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  oai21  g12(.a(x18), .b(x12), .c(new_n40_), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(new_n37_), .c(new_n39_), .O(z1));
  nand2  g14(.a(x08), .b(x01), .O(new_n43_));
  xnor2a g15(.a(x12), .b(x11), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n37_), .c(new_n43_), .O(z2));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  and2   g18(.a(x12), .b(x11), .O(new_n47_));
  nor2   g19(.a(new_n47_), .b(x13), .O(new_n48_));
  inv1   g20(.a(new_n37_), .O(new_n49_));
  nand2  g21(.a(new_n47_), .b(x13), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai21  g23(.a(new_n51_), .b(new_n48_), .c(new_n46_), .O(z3));
  nand2  g24(.a(x08), .b(x03), .O(new_n53_));
  aoi21  g25(.a(new_n47_), .b(x13), .c(x14), .O(new_n54_));
  nand4  g26(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n49_), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(z4));
  nand2  g29(.a(x08), .b(x04), .O(new_n58_));
  inv1   g30(.a(x15), .O(new_n59_));
  nor2   g31(.a(new_n55_), .b(new_n59_), .O(new_n60_));
  nand2  g32(.a(new_n55_), .b(new_n59_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n49_), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(z5));
  nand2  g35(.a(x08), .b(x05), .O(new_n64_));
  and2   g36(.a(new_n60_), .b(x16), .O(new_n65_));
  oai21  g37(.a(new_n60_), .b(x16), .c(new_n49_), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(z6));
  nand2  g39(.a(x08), .b(x06), .O(new_n68_));
  aoi21  g40(.a(new_n60_), .b(x16), .c(x17), .O(new_n69_));
  nand2  g41(.a(new_n49_), .b(new_n34_), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(z7));
  nand3  g43(.a(new_n29_), .b(x17), .c(x16), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n30_), .c(x12), .O(new_n73_));
  aoi21  g45(.a(new_n33_), .b(new_n31_), .c(new_n29_), .O(new_n74_));
  aoi21  g46(.a(new_n73_), .b(x11), .c(new_n74_), .O(new_n75_));
  nand2  g47(.a(x08), .b(x07), .O(new_n76_));
  oai21  g48(.a(new_n75_), .b(new_n37_), .c(new_n76_), .O(z8));
endmodule


