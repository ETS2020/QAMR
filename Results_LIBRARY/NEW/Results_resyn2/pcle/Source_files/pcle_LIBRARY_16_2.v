// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  nand4  g04(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n33_));
  nor3   g05(.a(new_n33_), .b(new_n32_), .c(new_n29_), .O(z0));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  nand2  g07(.a(x17), .b(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n30_), .O(new_n37_));
  oai21  g09(.a(new_n37_), .b(x11), .c(new_n35_), .O(z1));
  nand2  g10(.a(x08), .b(x01), .O(new_n39_));
  xnor2a g11(.a(x12), .b(x11), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n32_), .c(new_n39_), .O(z2));
  nand2  g13(.a(x08), .b(x02), .O(new_n42_));
  aoi21  g14(.a(x12), .b(x11), .c(x13), .O(new_n43_));
  nand3  g15(.a(x13), .b(x12), .c(x11), .O(new_n44_));
  inv1   g16(.a(new_n32_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n43_), .c(new_n42_), .O(z3));
  nand2  g19(.a(x08), .b(x03), .O(new_n48_));
  inv1   g20(.a(new_n44_), .O(new_n49_));
  nor2   g21(.a(new_n49_), .b(x14), .O(new_n50_));
  nand2  g22(.a(new_n45_), .b(new_n29_), .O(new_n51_));
  oai21  g23(.a(new_n51_), .b(new_n50_), .c(new_n48_), .O(z4));
  nand2  g24(.a(x08), .b(x04), .O(new_n53_));
  inv1   g25(.a(x15), .O(new_n54_));
  nor2   g26(.a(new_n29_), .b(new_n54_), .O(new_n55_));
  nand2  g27(.a(new_n29_), .b(new_n54_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n45_), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(new_n55_), .c(new_n53_), .O(z5));
  nand2  g30(.a(x08), .b(x05), .O(new_n59_));
  nand2  g31(.a(new_n55_), .b(x16), .O(new_n60_));
  inv1   g32(.a(x16), .O(new_n61_));
  oai21  g33(.a(new_n29_), .b(new_n54_), .c(new_n61_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n60_), .c(new_n45_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n59_), .O(z6));
  nand2  g36(.a(x08), .b(x06), .O(new_n65_));
  inv1   g37(.a(new_n37_), .O(new_n66_));
  nand3  g38(.a(x16), .b(x15), .c(x14), .O(new_n67_));
  inv1   g39(.a(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n49_), .c(x17), .O(new_n69_));
  inv1   g41(.a(x17), .O(new_n70_));
  oai21  g42(.a(new_n67_), .b(new_n44_), .c(new_n70_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n69_), .c(new_n66_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n65_), .O(z7));
  nand2  g45(.a(x08), .b(x07), .O(new_n74_));
  nand4  g46(.a(new_n68_), .b(new_n49_), .c(x18), .d(x17), .O(new_n75_));
  inv1   g47(.a(x18), .O(new_n76_));
  nand2  g48(.a(new_n69_), .b(new_n76_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n75_), .c(new_n66_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n74_), .O(z8));
endmodule


