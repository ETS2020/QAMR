// Benchmark "FAU" written by ABC on Mon Jul 27 18:25:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  nand4  g01(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n30_));
  and2   g02(.a(x18), .b(x09), .O(new_n31_));
  nor2   g03(.a(x10), .b(x08), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nor3   g05(.a(new_n33_), .b(new_n30_), .c(new_n29_), .O(z0));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(x09), .O(new_n37_));
  inv1   g09(.a(x11), .O(new_n38_));
  nand2  g10(.a(new_n32_), .b(new_n38_), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z1));
  nand2  g12(.a(x08), .b(x01), .O(new_n41_));
  nand2  g13(.a(new_n32_), .b(x09), .O(new_n42_));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(z2));
  nand2  g16(.a(x08), .b(x02), .O(new_n45_));
  aoi21  g17(.a(x12), .b(x11), .c(x13), .O(new_n46_));
  inv1   g18(.a(new_n42_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n29_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(z3));
  nand2  g21(.a(x08), .b(x03), .O(new_n50_));
  inv1   g22(.a(x14), .O(new_n51_));
  nand2  g23(.a(new_n29_), .b(new_n51_), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n52_), .c(new_n47_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n50_), .O(z4));
  nand2  g27(.a(x08), .b(x04), .O(new_n56_));
  inv1   g28(.a(x15), .O(new_n57_));
  nor2   g29(.a(new_n53_), .b(new_n57_), .O(new_n58_));
  inv1   g30(.a(new_n53_), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(x15), .c(new_n47_), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(z5));
  nand2  g33(.a(x08), .b(x05), .O(new_n62_));
  nor2   g34(.a(new_n58_), .b(x16), .O(new_n63_));
  and2   g35(.a(x16), .b(x15), .O(new_n64_));
  nand2  g36(.a(new_n59_), .b(new_n64_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n47_), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n63_), .c(new_n62_), .O(z6));
  nand2  g39(.a(x08), .b(x06), .O(new_n68_));
  nand3  g40(.a(new_n59_), .b(new_n64_), .c(x17), .O(new_n69_));
  inv1   g41(.a(x17), .O(new_n70_));
  nand2  g42(.a(new_n65_), .b(new_n70_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n69_), .c(new_n47_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n68_), .O(z7));
  oai21  g45(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n74_));
  nor2   g46(.a(x18), .b(new_n70_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n59_), .c(new_n64_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n32_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x07), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n78_), .O(z8));
endmodule


