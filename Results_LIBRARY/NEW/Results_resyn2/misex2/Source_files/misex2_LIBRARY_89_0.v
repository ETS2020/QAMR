// Benchmark "FAU" written by ABC on Mon Jul 27 18:45:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n121_, new_n122_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  nand3  g13(.a(x02), .b(x01), .c(x00), .O(new_n57_));
  nand3  g14(.a(x11), .b(x10), .c(new_n44_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n57_), .O(z06));
  nand2  g16(.a(z06), .b(x12), .O(new_n60_));
  inv1   g17(.a(x02), .O(new_n61_));
  nand3  g18(.a(new_n49_), .b(new_n47_), .c(new_n61_), .O(new_n62_));
  nor2   g19(.a(x01), .b(x00), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x18), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x10), .d(new_n44_), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n57_), .O(z04));
  nand2  g26(.a(x10), .b(x02), .O(new_n70_));
  inv1   g27(.a(x00), .O(new_n71_));
  inv1   g28(.a(x01), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(x09), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n70_), .O(z05));
  inv1   g32(.a(new_n58_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n67_), .O(new_n77_));
  nand2  g34(.a(x02), .b(x00), .O(new_n78_));
  aoi21  g35(.a(new_n77_), .b(x01), .c(new_n78_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand2  g40(.a(new_n63_), .b(x02), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n48_), .c(x17), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(z08));
  inv1   g43(.a(x20), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n49_), .c(x18), .d(x01), .O(new_n88_));
  nand2  g45(.a(x02), .b(new_n72_), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(x12), .c(new_n66_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nor2   g49(.a(x15), .b(x14), .O(new_n93_));
  nor2   g50(.a(new_n87_), .b(x16), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(new_n71_), .O(new_n98_));
  aoi21  g55(.a(new_n95_), .b(new_n88_), .c(new_n98_), .O(z09));
  inv1   g56(.a(new_n88_), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(x22), .c(x21), .O(new_n101_));
  inv1   g58(.a(x14), .O(new_n102_));
  nand4  g59(.a(new_n96_), .b(x20), .c(x15), .d(new_n102_), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  inv1   g61(.a(x16), .O(new_n105_));
  nor2   g62(.a(x22), .b(new_n105_), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n104_), .c(new_n92_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n101_), .c(x00), .O(z10));
  nand2  g65(.a(new_n97_), .b(new_n71_), .O(new_n109_));
  nand2  g66(.a(new_n100_), .b(x21), .O(new_n110_));
  nand3  g67(.a(new_n104_), .b(new_n92_), .c(new_n105_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(z11));
  nand2  g69(.a(new_n70_), .b(new_n73_), .O(new_n113_));
  nand2  g70(.a(new_n62_), .b(new_n63_), .O(new_n114_));
  inv1   g71(.a(x24), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x09), .O(new_n116_));
  aoi21  g73(.a(new_n114_), .b(new_n113_), .c(new_n116_), .O(z12));
  nor2   g74(.a(x19), .b(new_n47_), .O(new_n118_));
  and2   g75(.a(new_n118_), .b(new_n45_), .O(z13));
  nor2   g76(.a(new_n55_), .b(new_n46_), .O(z14));
  aoi21  g77(.a(new_n54_), .b(x01), .c(new_n61_), .O(new_n121_));
  nand3  g78(.a(x19), .b(new_n61_), .c(new_n72_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n71_), .c(new_n122_), .O(z15));
  nor2   g80(.a(new_n72_), .b(x00), .O(z16));
  inv1   g81(.a(new_n84_), .O(z17));
endmodule


