// Benchmark "FAU" written by ABC on Mon Jul 27 18:45:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n120_, new_n121_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  nand3  g13(.a(x02), .b(x01), .c(x00), .O(new_n57_));
  nand3  g14(.a(x11), .b(x10), .c(new_n44_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n57_), .O(z06));
  nand2  g16(.a(z06), .b(x12), .O(new_n60_));
  nand2  g17(.a(new_n45_), .b(new_n49_), .O(new_n61_));
  nand2  g18(.a(x18), .b(new_n47_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(z03));
  nand2  g20(.a(x10), .b(new_n44_), .O(new_n64_));
  nor4   g21(.a(new_n64_), .b(new_n57_), .c(x12), .d(x11), .O(z04));
  nand2  g22(.a(x10), .b(x02), .O(new_n66_));
  inv1   g23(.a(x00), .O(new_n67_));
  inv1   g24(.a(x01), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(x09), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n66_), .O(z05));
  inv1   g28(.a(x12), .O(new_n72_));
  inv1   g29(.a(new_n58_), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  aoi21  g32(.a(new_n74_), .b(x01), .c(new_n75_), .O(z07));
  inv1   g33(.a(x03), .O(new_n77_));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(new_n78_), .c(x04), .d(new_n77_), .O(new_n80_));
  nor2   g37(.a(x01), .b(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(x02), .O(new_n82_));
  nand3  g39(.a(x19), .b(new_n48_), .c(x17), .O(new_n83_));
  nor3   g40(.a(new_n83_), .b(new_n82_), .c(new_n80_), .O(z08));
  nor2   g41(.a(new_n72_), .b(x11), .O(new_n85_));
  nor2   g42(.a(x14), .b(x13), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(x02), .c(new_n68_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  inv1   g45(.a(x15), .O(new_n89_));
  inv1   g46(.a(x16), .O(new_n90_));
  nand3  g47(.a(x20), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n88_), .c(new_n85_), .O(new_n93_));
  inv1   g50(.a(x20), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n49_), .c(x18), .d(x01), .O(new_n95_));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(new_n67_), .O(new_n98_));
  aoi21  g55(.a(new_n95_), .b(new_n93_), .c(new_n98_), .O(z09));
  nand4  g56(.a(new_n86_), .b(new_n85_), .c(x16), .d(x15), .O(new_n100_));
  nand3  g57(.a(new_n97_), .b(new_n96_), .c(x20), .O(new_n101_));
  nor3   g58(.a(new_n101_), .b(new_n100_), .c(new_n82_), .O(z10));
  inv1   g59(.a(new_n95_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n104_));
  nand2  g61(.a(new_n90_), .b(x15), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(new_n101_), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n88_), .c(new_n85_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n104_), .c(x00), .O(z11));
  nand2  g65(.a(new_n66_), .b(new_n69_), .O(new_n109_));
  oai21  g66(.a(x19), .b(x02), .c(x23), .O(new_n110_));
  inv1   g67(.a(x02), .O(new_n111_));
  nand3  g68(.a(new_n49_), .b(x17), .c(new_n111_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n81_), .O(new_n114_));
  inv1   g71(.a(x24), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x09), .O(new_n116_));
  aoi21  g73(.a(new_n114_), .b(new_n109_), .c(new_n116_), .O(z12));
  nor2   g74(.a(new_n61_), .b(new_n47_), .O(z13));
  nor2   g75(.a(new_n55_), .b(new_n46_), .O(z14));
  aoi21  g76(.a(new_n54_), .b(x01), .c(new_n111_), .O(new_n120_));
  nand3  g77(.a(x19), .b(new_n111_), .c(new_n68_), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n67_), .c(new_n121_), .O(z15));
  nor2   g79(.a(new_n68_), .b(x00), .O(z16));
  inv1   g80(.a(new_n82_), .O(z17));
endmodule


