// Benchmark "FAU" written by ABC on Mon Jul 27 18:45:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n113_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor2   g01(.a(x02), .b(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x01), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g16(.a(x11), .b(x10), .c(new_n44_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n59_), .c(x12), .d(x02), .O(new_n62_));
  nand4  g19(.a(new_n45_), .b(new_n49_), .c(x18), .d(new_n47_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n62_), .O(z03));
  nand2  g21(.a(new_n59_), .b(x02), .O(new_n65_));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x10), .d(new_n44_), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n65_), .O(z04));
  inv1   g26(.a(new_n59_), .O(new_n70_));
  nand2  g27(.a(x10), .b(x02), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n70_), .c(new_n44_), .O(z05));
  nor2   g29(.a(new_n60_), .b(new_n65_), .O(z06));
  nand2  g30(.a(new_n61_), .b(new_n67_), .O(new_n74_));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  aoi21  g32(.a(new_n74_), .b(x01), .c(new_n75_), .O(z07));
  inv1   g33(.a(x03), .O(new_n77_));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(new_n78_), .c(x04), .d(new_n77_), .O(new_n80_));
  nand3  g37(.a(x02), .b(new_n58_), .c(new_n57_), .O(new_n81_));
  nand3  g38(.a(x19), .b(new_n48_), .c(x17), .O(new_n82_));
  nor3   g39(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(z08));
  inv1   g40(.a(x16), .O(new_n84_));
  inv1   g41(.a(new_n81_), .O(z17));
  inv1   g42(.a(x21), .O(new_n86_));
  inv1   g43(.a(x22), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n88_));
  nor2   g45(.a(x14), .b(x13), .O(new_n89_));
  nand3  g46(.a(new_n89_), .b(x12), .c(new_n66_), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(z17), .c(new_n84_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(x15), .O(z09));
  nand4  g50(.a(new_n91_), .b(z17), .c(x16), .d(x15), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(z10));
  inv1   g52(.a(x15), .O(new_n96_));
  nor2   g53(.a(new_n92_), .b(new_n96_), .O(z11));
  nand2  g54(.a(new_n71_), .b(new_n59_), .O(new_n98_));
  nand3  g55(.a(x23), .b(x19), .c(new_n58_), .O(new_n99_));
  inv1   g56(.a(x23), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(x02), .O(new_n101_));
  inv1   g58(.a(x02), .O(new_n102_));
  nand2  g59(.a(new_n47_), .b(new_n102_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n101_), .c(new_n49_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n99_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n57_), .O(new_n106_));
  inv1   g63(.a(x24), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(x09), .O(new_n108_));
  aoi21  g65(.a(new_n106_), .b(new_n98_), .c(new_n108_), .O(z12));
  nand3  g66(.a(new_n45_), .b(new_n49_), .c(x17), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(z13));
  nor2   g68(.a(new_n55_), .b(new_n46_), .O(z14));
  aoi21  g69(.a(x19), .b(new_n58_), .c(x00), .O(new_n113_));
  oai22  g70(.a(new_n113_), .b(x02), .c(new_n70_), .d(x10), .O(z15));
  nor2   g71(.a(new_n58_), .b(x00), .O(z16));
endmodule


