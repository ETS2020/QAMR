// Benchmark "FAU" written by ABC on Mon Jul 27 21:34:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n82_, new_n84_, new_n85_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  nand2  g02(.a(x2), .b(x1), .O(new_n20_));
  nor2   g03(.a(x6), .b(x0), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  inv1   g05(.a(x0), .O(new_n23_));
  inv1   g06(.a(x1), .O(new_n24_));
  nand2  g07(.a(x3), .b(x2), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n24_), .c(new_n23_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x5), .O(new_n27_));
  nand2  g10(.a(x6), .b(new_n18_), .O(new_n28_));
  oai22  g11(.a(new_n28_), .b(new_n27_), .c(new_n22_), .d(new_n18_), .O(z0));
  inv1   g12(.a(x6), .O(new_n30_));
  nor2   g13(.a(new_n30_), .b(x5), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x0), .O(new_n32_));
  nor2   g15(.a(x6), .b(x4), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(x5), .c(x1), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x2), .O(new_n36_));
  inv1   g19(.a(x2), .O(new_n37_));
  nand2  g20(.a(new_n30_), .b(new_n37_), .O(new_n38_));
  aoi21  g21(.a(new_n38_), .b(x1), .c(x0), .O(new_n39_));
  inv1   g22(.a(new_n25_), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(x4), .c(x6), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n39_), .c(x5), .O(new_n42_));
  nand2  g25(.a(new_n31_), .b(x4), .O(new_n43_));
  oai21  g26(.a(x5), .b(x1), .c(x0), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  xor2a  g28(.a(new_n33_), .b(new_n19_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand4  g30(.a(new_n47_), .b(new_n43_), .c(new_n42_), .d(new_n36_), .O(z1));
  oai21  g31(.a(new_n30_), .b(x2), .c(x3), .O(new_n49_));
  nor2   g32(.a(new_n19_), .b(x0), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g34(.a(new_n31_), .b(new_n37_), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n51_), .c(x1), .O(new_n53_));
  nand3  g36(.a(x5), .b(x2), .c(x1), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n44_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n30_), .O(new_n56_));
  nand2  g39(.a(new_n31_), .b(new_n23_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n53_), .c(new_n18_), .O(new_n59_));
  oai21  g42(.a(x2), .b(x1), .c(x0), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n27_), .c(x6), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n56_), .c(x4), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n59_), .O(z2));
  aoi21  g46(.a(x6), .b(x2), .c(x1), .O(new_n64_));
  or2    g47(.a(new_n64_), .b(new_n23_), .O(new_n65_));
  nand2  g48(.a(new_n40_), .b(x6), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n39_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  xor2a  g51(.a(new_n68_), .b(new_n19_), .O(z3));
  inv1   g52(.a(x3), .O(z8));
  oai21  g53(.a(z8), .b(x1), .c(new_n39_), .O(new_n71_));
  nand2  g54(.a(new_n20_), .b(z8), .O(new_n72_));
  nand3  g55(.a(new_n72_), .b(new_n49_), .c(x0), .O(new_n73_));
  inv1   g56(.a(new_n66_), .O(new_n74_));
  aoi22  g57(.a(new_n74_), .b(x1), .c(new_n64_), .d(new_n38_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(z4));
  nand2  g59(.a(z8), .b(x2), .O(new_n77_));
  nand2  g60(.a(x3), .b(new_n37_), .O(new_n78_));
  oai21  g61(.a(new_n78_), .b(new_n24_), .c(new_n77_), .O(new_n79_));
  xor2a  g62(.a(new_n79_), .b(x0), .O(z5));
  xor2a  g63(.a(new_n78_), .b(new_n24_), .O(z6));
  nor2   g64(.a(new_n30_), .b(x2), .O(new_n82_));
  aoi21  g65(.a(new_n82_), .b(z8), .c(new_n40_), .O(z7));
  aoi21  g66(.a(new_n49_), .b(new_n24_), .c(new_n67_), .O(new_n84_));
  nand2  g67(.a(x5), .b(x4), .O(new_n85_));
  nor2   g68(.a(new_n85_), .b(new_n84_), .O(z9));
endmodule


