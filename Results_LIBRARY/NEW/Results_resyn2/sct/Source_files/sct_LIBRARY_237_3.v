// Benchmark "FAU" written by ABC on Mon Jul 27 17:50:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_;
  inv1   g00(.a(x14), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x01), .O(new_n36_));
  aoi21  g02(.a(x02), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(x05), .b(new_n40_), .O(new_n41_));
  oai21  g07(.a(new_n39_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g08(.a(x06), .O(new_n43_));
  inv1   g09(.a(x16), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(x04), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z02));
  nor2   g12(.a(x07), .b(x06), .O(new_n47_));
  nor2   g13(.a(x16), .b(new_n40_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  and2   g15(.a(x07), .b(x06), .O(new_n50_));
  nor3   g16(.a(new_n50_), .b(new_n49_), .c(new_n47_), .O(z03));
  nor2   g17(.a(new_n50_), .b(x08), .O(new_n52_));
  nand2  g18(.a(new_n50_), .b(x08), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n52_), .O(z04));
  nand2  g21(.a(new_n53_), .b(x09), .O(new_n56_));
  inv1   g22(.a(x09), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(x08), .c(x07), .d(x06), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(new_n48_), .O(z05));
  nand2  g25(.a(new_n58_), .b(x10), .O(new_n60_));
  inv1   g26(.a(x10), .O(new_n61_));
  inv1   g27(.a(new_n58_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n60_), .c(new_n48_), .O(z06));
  inv1   g30(.a(x11), .O(new_n65_));
  inv1   g31(.a(x08), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n66_), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n50_), .c(new_n65_), .d(new_n61_), .O(new_n68_));
  oai21  g34(.a(new_n58_), .b(x10), .c(x11), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n48_), .O(z07));
  nand2  g36(.a(new_n68_), .b(x12), .O(new_n71_));
  nor2   g37(.a(x12), .b(x11), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n67_), .c(new_n50_), .d(new_n61_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n48_), .O(z08));
  nand2  g40(.a(new_n73_), .b(x13), .O(new_n75_));
  inv1   g41(.a(x12), .O(new_n76_));
  inv1   g42(.a(x13), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n76_), .c(new_n65_), .d(new_n61_), .O(new_n78_));
  inv1   g44(.a(new_n78_), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n62_), .c(new_n49_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n75_), .O(z09));
  inv1   g47(.a(x00), .O(new_n82_));
  nand2  g48(.a(new_n78_), .b(new_n82_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n62_), .c(new_n44_), .O(new_n84_));
  nand2  g50(.a(x16), .b(x14), .O(new_n85_));
  aoi21  g51(.a(new_n85_), .b(new_n84_), .c(new_n40_), .O(z10));
  inv1   g52(.a(x03), .O(new_n87_));
  aoi21  g53(.a(new_n44_), .b(new_n87_), .c(new_n40_), .O(z12));
  and2   g54(.a(x17), .b(x04), .O(z14));
  buf    g55(.a(x02), .O(z11));
  buf    g56(.a(x04), .O(z13));
endmodule


