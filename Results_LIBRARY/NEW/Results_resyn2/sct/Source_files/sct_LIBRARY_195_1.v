// Benchmark "FAU" written by ABC on Mon Jul 27 17:50:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_;
  inv1   g00(.a(x14), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x01), .O(new_n36_));
  aoi21  g02(.a(x02), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(x05), .b(new_n40_), .O(new_n41_));
  oai21  g07(.a(new_n39_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g08(.a(x06), .O(new_n43_));
  inv1   g09(.a(x03), .O(new_n44_));
  inv1   g10(.a(x16), .O(new_n45_));
  aoi21  g11(.a(new_n44_), .b(x02), .c(new_n45_), .O(new_n46_));
  nor2   g12(.a(new_n46_), .b(new_n40_), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(new_n43_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  nand2  g15(.a(x07), .b(x06), .O(new_n50_));
  inv1   g16(.a(x07), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n43_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n50_), .c(new_n47_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  nand3  g20(.a(x08), .b(x07), .c(x06), .O(new_n55_));
  inv1   g21(.a(x08), .O(new_n56_));
  nand2  g22(.a(new_n50_), .b(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  inv1   g25(.a(x09), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(x08), .c(x06), .O(new_n61_));
  nor2   g27(.a(x10), .b(x07), .O(new_n62_));
  or2    g28(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g29(.a(new_n55_), .b(x09), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n63_), .c(new_n47_), .O(z05));
  inv1   g31(.a(x10), .O(new_n66_));
  nand4  g32(.a(new_n60_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand2  g35(.a(new_n61_), .b(x10), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n47_), .O(z06));
  inv1   g37(.a(x11), .O(new_n72_));
  nand3  g38(.a(new_n68_), .b(new_n72_), .c(new_n66_), .O(new_n73_));
  nand2  g39(.a(new_n69_), .b(x11), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n47_), .O(z07));
  nand2  g41(.a(new_n73_), .b(x12), .O(new_n76_));
  nor2   g42(.a(x12), .b(x11), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n68_), .c(new_n66_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n76_), .c(new_n47_), .O(z08));
  nor2   g45(.a(new_n45_), .b(x02), .O(new_n80_));
  nor2   g46(.a(new_n67_), .b(new_n46_), .O(new_n81_));
  inv1   g47(.a(x13), .O(new_n82_));
  nand3  g48(.a(new_n77_), .b(new_n82_), .c(new_n66_), .O(new_n83_));
  inv1   g49(.a(new_n83_), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n81_), .c(new_n80_), .O(new_n85_));
  nand2  g51(.a(new_n78_), .b(x13), .O(new_n86_));
  nand2  g52(.a(x16), .b(x03), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(x04), .O(z09));
  inv1   g54(.a(x00), .O(new_n89_));
  nand2  g55(.a(new_n83_), .b(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n81_), .O(new_n91_));
  nand2  g57(.a(new_n46_), .b(x14), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n91_), .c(new_n40_), .O(z10));
  nor2   g59(.a(new_n80_), .b(x03), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n40_), .O(z12));
  and2   g61(.a(x17), .b(x04), .O(z14));
  buf    g62(.a(x02), .O(z11));
  buf    g63(.a(x04), .O(z13));
endmodule


