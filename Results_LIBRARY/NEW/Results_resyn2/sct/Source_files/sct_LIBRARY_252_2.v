// Benchmark "FAU" written by ABC on Mon Jul 27 17:50:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n94_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(z00));
  inv1   g04(.a(x18), .O(new_n39_));
  nor2   g05(.a(new_n39_), .b(x15), .O(new_n40_));
  inv1   g06(.a(x04), .O(new_n41_));
  nand2  g07(.a(x05), .b(new_n41_), .O(new_n42_));
  oai21  g08(.a(new_n40_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n36_), .c(x16), .O(new_n45_));
  and2   g11(.a(new_n45_), .b(x04), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  nand2  g14(.a(x07), .b(x06), .O(new_n49_));
  inv1   g15(.a(x07), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n44_), .c(new_n41_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n49_), .c(new_n45_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  inv1   g19(.a(new_n49_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x08), .O(new_n55_));
  inv1   g21(.a(x08), .O(new_n56_));
  nand2  g22(.a(new_n49_), .b(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n55_), .c(new_n46_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  inv1   g25(.a(x09), .O(new_n60_));
  nand3  g26(.a(new_n54_), .b(new_n60_), .c(x08), .O(new_n61_));
  nand2  g27(.a(new_n55_), .b(x09), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(new_n46_), .O(z05));
  nand2  g29(.a(new_n61_), .b(x10), .O(new_n64_));
  inv1   g30(.a(x10), .O(new_n65_));
  inv1   g31(.a(new_n61_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n64_), .c(new_n46_), .O(z06));
  nand2  g34(.a(new_n67_), .b(x11), .O(new_n69_));
  nor2   g35(.a(x11), .b(x10), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n66_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n69_), .c(new_n46_), .O(z07));
  inv1   g38(.a(x12), .O(new_n73_));
  nand3  g39(.a(new_n70_), .b(new_n66_), .c(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n71_), .b(x12), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n46_), .O(z08));
  nand2  g42(.a(new_n49_), .b(new_n45_), .O(new_n77_));
  nand2  g43(.a(new_n70_), .b(new_n73_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n54_), .c(new_n60_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g46(.a(x16), .b(new_n36_), .O(new_n81_));
  nand2  g47(.a(x13), .b(x09), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g49(.a(new_n80_), .b(x08), .c(new_n83_), .O(new_n84_));
  nand2  g50(.a(x16), .b(x03), .O(new_n85_));
  nand2  g51(.a(x13), .b(new_n56_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(x04), .O(z09));
  nor2   g53(.a(new_n56_), .b(new_n50_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n60_), .c(x06), .d(x00), .O(new_n89_));
  oai21  g55(.a(new_n45_), .b(x14), .c(x04), .O(new_n90_));
  aoi21  g56(.a(new_n89_), .b(new_n45_), .c(new_n90_), .O(z10));
  inv1   g57(.a(x03), .O(new_n92_));
  aoi21  g58(.a(new_n81_), .b(new_n92_), .c(new_n41_), .O(z12));
  nor3   g59(.a(new_n83_), .b(new_n55_), .c(x03), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n61_), .c(new_n41_), .O(z13));
  and2   g61(.a(x17), .b(x04), .O(z14));
  buf    g62(.a(x02), .O(z11));
endmodule


