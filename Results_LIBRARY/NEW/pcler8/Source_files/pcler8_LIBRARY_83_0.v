// Benchmark "FAU" written by ABC on Thu Jun 25 17:17:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_;
  inv1   g00(.a(x24), .O(new_n45_));
  inv1   g01(.a(x25), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g03(.a(new_n47_), .b(x26), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x09), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x08), .O(new_n53_));
  inv1   g09(.a(x19), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(x10), .O(new_n55_));
  nand4  g11(.a(new_n55_), .b(new_n53_), .c(new_n51_), .d(new_n49_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z00));
  inv1   g13(.a(x00), .O(new_n58_));
  inv1   g14(.a(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z01));
  inv1   g16(.a(x01), .O(new_n61_));
  nor2   g17(.a(new_n59_), .b(new_n61_), .O(z02));
  inv1   g18(.a(x02), .O(new_n63_));
  nor2   g19(.a(new_n59_), .b(new_n63_), .O(z03));
  inv1   g20(.a(x03), .O(new_n65_));
  nor2   g21(.a(new_n59_), .b(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z05));
  inv1   g24(.a(x05), .O(new_n69_));
  nor2   g25(.a(new_n59_), .b(new_n69_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z08));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  and2   g31(.a(x22), .b(x21), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(x20), .c(x11), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n54_), .O(new_n79_));
  inv1   g35(.a(x10), .O(new_n80_));
  nand2  g36(.a(new_n53_), .b(new_n80_), .O(new_n81_));
  oai22  g37(.a(new_n81_), .b(new_n79_), .c(new_n59_), .d(new_n58_), .O(z09));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n75_), .c(x19), .O(new_n84_));
  nor2   g40(.a(x20), .b(new_n54_), .O(new_n85_));
  aoi21  g41(.a(new_n84_), .b(x20), .c(new_n85_), .O(new_n86_));
  oai22  g42(.a(new_n86_), .b(new_n81_), .c(new_n59_), .d(new_n61_), .O(z10));
  nand3  g43(.a(x26), .b(x25), .c(x24), .O(new_n88_));
  nand3  g44(.a(x23), .b(x22), .c(x13), .O(new_n89_));
  nand2  g45(.a(x20), .b(x19), .O(new_n90_));
  inv1   g46(.a(new_n90_), .O(new_n91_));
  oai21  g47(.a(new_n89_), .b(new_n88_), .c(new_n91_), .O(new_n92_));
  nor2   g48(.a(new_n90_), .b(x21), .O(new_n93_));
  aoi21  g49(.a(new_n92_), .b(x21), .c(new_n93_), .O(new_n94_));
  oai22  g50(.a(new_n94_), .b(new_n81_), .c(new_n59_), .d(new_n63_), .O(z11));
  nand2  g51(.a(x23), .b(x14), .O(new_n96_));
  nand3  g52(.a(x21), .b(x20), .c(x19), .O(new_n97_));
  inv1   g53(.a(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n96_), .b(new_n88_), .c(new_n98_), .O(new_n99_));
  inv1   g55(.a(x22), .O(new_n100_));
  nand3  g56(.a(new_n91_), .b(new_n100_), .c(x21), .O(new_n101_));
  inv1   g57(.a(new_n101_), .O(new_n102_));
  aoi21  g58(.a(new_n99_), .b(x22), .c(new_n102_), .O(new_n103_));
  oai22  g59(.a(new_n103_), .b(new_n81_), .c(new_n59_), .d(new_n65_), .O(z12));
  inv1   g60(.a(x23), .O(new_n105_));
  oai21  g61(.a(new_n45_), .b(new_n59_), .c(new_n100_), .O(new_n106_));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand4  g63(.a(new_n107_), .b(x21), .c(x20), .d(x19), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n59_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n106_), .c(new_n105_), .O(new_n110_));
  nand4  g66(.a(new_n105_), .b(x22), .c(x21), .d(new_n59_), .O(new_n111_));
  nor2   g67(.a(new_n111_), .b(new_n90_), .O(new_n112_));
  nor2   g68(.a(x10), .b(new_n52_), .O(new_n113_));
  oai21  g69(.a(new_n112_), .b(new_n110_), .c(new_n113_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n67_), .O(z13));
  nand2  g71(.a(x24), .b(new_n100_), .O(new_n116_));
  nand3  g72(.a(new_n91_), .b(new_n76_), .c(new_n45_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand3  g74(.a(x26), .b(x25), .c(x16), .O(new_n119_));
  nand4  g75(.a(x23), .b(x21), .c(x20), .d(x19), .O(new_n120_));
  inv1   g76(.a(new_n120_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n119_), .c(new_n45_), .O(new_n122_));
  aoi21  g78(.a(new_n118_), .b(x23), .c(new_n122_), .O(new_n123_));
  oai22  g79(.a(new_n123_), .b(new_n81_), .c(new_n59_), .d(new_n69_), .O(z14));
  nand3  g80(.a(x21), .b(x20), .c(x19), .O(new_n125_));
  inv1   g81(.a(new_n125_), .O(new_n126_));
  nor2   g82(.a(new_n105_), .b(new_n100_), .O(new_n127_));
  nand2  g83(.a(x26), .b(x17), .O(new_n128_));
  nand4  g84(.a(new_n128_), .b(new_n126_), .c(new_n127_), .d(x24), .O(new_n129_));
  nand3  g85(.a(new_n127_), .b(new_n46_), .c(x24), .O(new_n130_));
  inv1   g86(.a(new_n130_), .O(new_n131_));
  aoi22  g87(.a(new_n131_), .b(new_n126_), .c(new_n129_), .d(x25), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n81_), .c(new_n71_), .O(z15));
  nor2   g89(.a(new_n54_), .b(x18), .O(new_n134_));
  nand3  g90(.a(new_n134_), .b(new_n51_), .c(new_n47_), .O(new_n135_));
  inv1   g91(.a(x26), .O(new_n136_));
  nand3  g92(.a(new_n76_), .b(new_n136_), .c(x25), .O(new_n137_));
  nand3  g93(.a(new_n91_), .b(x24), .c(x23), .O(new_n138_));
  nor2   g94(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  aoi21  g95(.a(new_n135_), .b(x26), .c(new_n139_), .O(new_n140_));
  oai21  g96(.a(new_n140_), .b(new_n81_), .c(new_n73_), .O(z16));
endmodule


