// Benchmark "FAU" written by ABC on Thu Jun 25 17:17:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_;
  nand4  g00(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand4  g06(.a(new_n50_), .b(x26), .c(x25), .d(x24), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(z00));
  inv1   g08(.a(x00), .O(new_n53_));
  inv1   g09(.a(x08), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n53_), .O(z01));
  inv1   g11(.a(x01), .O(new_n56_));
  nor2   g12(.a(new_n54_), .b(new_n56_), .O(z02));
  inv1   g13(.a(x02), .O(new_n58_));
  nor2   g14(.a(new_n54_), .b(new_n58_), .O(z03));
  nand2  g15(.a(x08), .b(x03), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z04));
  nand2  g17(.a(x08), .b(x04), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z05));
  nand2  g19(.a(x08), .b(x05), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z06));
  inv1   g21(.a(x06), .O(new_n66_));
  nor2   g22(.a(new_n54_), .b(new_n66_), .O(z07));
  inv1   g23(.a(x07), .O(new_n68_));
  nor2   g24(.a(new_n54_), .b(new_n68_), .O(z08));
  nand4  g25(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n70_));
  and2   g26(.a(x22), .b(x21), .O(new_n71_));
  nand3  g27(.a(new_n71_), .b(x20), .c(x11), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n70_), .c(x19), .O(new_n73_));
  nand2  g29(.a(new_n48_), .b(new_n46_), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  oai21  g32(.a(new_n54_), .b(new_n53_), .c(new_n76_), .O(z09));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n70_), .c(x19), .O(new_n79_));
  inv1   g35(.a(x19), .O(new_n80_));
  nor2   g36(.a(x20), .b(new_n80_), .O(new_n81_));
  aoi21  g37(.a(new_n79_), .b(x20), .c(new_n81_), .O(new_n82_));
  oai22  g38(.a(new_n82_), .b(new_n74_), .c(new_n54_), .d(new_n56_), .O(z10));
  nand3  g39(.a(x26), .b(x25), .c(x24), .O(new_n84_));
  nand3  g40(.a(x23), .b(x22), .c(x13), .O(new_n85_));
  nand2  g41(.a(x20), .b(x19), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  oai21  g43(.a(new_n85_), .b(new_n84_), .c(new_n87_), .O(new_n88_));
  nor2   g44(.a(new_n86_), .b(x21), .O(new_n89_));
  aoi21  g45(.a(new_n88_), .b(x21), .c(new_n89_), .O(new_n90_));
  oai22  g46(.a(new_n90_), .b(new_n74_), .c(new_n54_), .d(new_n58_), .O(z11));
  inv1   g47(.a(new_n84_), .O(new_n92_));
  nand3  g48(.a(new_n92_), .b(x23), .c(x14), .O(new_n93_));
  nand3  g49(.a(new_n93_), .b(new_n87_), .c(x21), .O(new_n94_));
  inv1   g50(.a(x22), .O(new_n95_));
  nand3  g51(.a(new_n87_), .b(new_n95_), .c(x21), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(new_n97_));
  aoi21  g53(.a(new_n94_), .b(x22), .c(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n74_), .c(new_n60_), .O(z12));
  inv1   g55(.a(x23), .O(new_n100_));
  nand4  g56(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n101_));
  nand4  g57(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  nand2  g60(.a(new_n71_), .b(new_n100_), .O(new_n105_));
  nor2   g61(.a(new_n105_), .b(new_n86_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n104_), .c(new_n75_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n62_), .O(z13));
  inv1   g64(.a(x24), .O(new_n109_));
  aoi21  g65(.a(x22), .b(x21), .c(x08), .O(new_n110_));
  nand3  g66(.a(x26), .b(x25), .c(x16), .O(new_n111_));
  nand4  g67(.a(new_n111_), .b(x23), .c(x20), .d(x19), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(x21), .c(new_n110_), .O(new_n113_));
  nor2   g69(.a(new_n100_), .b(new_n95_), .O(new_n114_));
  nand3  g70(.a(x21), .b(x20), .c(x19), .O(new_n115_));
  inv1   g71(.a(new_n115_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n114_), .c(new_n109_), .O(new_n117_));
  oai21  g73(.a(new_n113_), .b(new_n109_), .c(new_n117_), .O(new_n118_));
  nor2   g74(.a(x10), .b(new_n47_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n64_), .O(z14));
  nand2  g77(.a(x26), .b(x17), .O(new_n122_));
  nand4  g78(.a(new_n122_), .b(new_n116_), .c(new_n114_), .d(x24), .O(new_n123_));
  inv1   g79(.a(x25), .O(new_n124_));
  nand4  g80(.a(new_n124_), .b(x24), .c(x23), .d(x22), .O(new_n125_));
  nor2   g81(.a(new_n125_), .b(new_n115_), .O(new_n126_));
  aoi21  g82(.a(new_n123_), .b(x25), .c(new_n126_), .O(new_n127_));
  oai22  g83(.a(new_n127_), .b(new_n74_), .c(new_n54_), .d(new_n66_), .O(z15));
  inv1   g84(.a(x18), .O(new_n129_));
  nand4  g85(.a(x25), .b(x24), .c(x19), .d(new_n129_), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n45_), .c(x26), .O(new_n131_));
  nor2   g87(.a(new_n109_), .b(new_n100_), .O(new_n132_));
  nor2   g88(.a(x26), .b(new_n124_), .O(new_n133_));
  nand4  g89(.a(new_n133_), .b(new_n87_), .c(new_n71_), .d(new_n132_), .O(new_n134_));
  nand2  g90(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand2  g91(.a(new_n135_), .b(new_n75_), .O(new_n136_));
  oai21  g92(.a(new_n54_), .b(new_n68_), .c(new_n136_), .O(z16));
endmodule


