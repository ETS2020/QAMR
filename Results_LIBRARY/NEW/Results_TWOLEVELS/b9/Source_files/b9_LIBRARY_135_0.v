// Benchmark "FAU" written by ABC on Thu Jun 25 20:41:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n153_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n161_, new_n162_, new_n163_;
  inv1   g000(.a(x15), .O(new_n63_));
  nor2   g001(.a(x27), .b(x10), .O(new_n64_));
  inv1   g002(.a(x39), .O(new_n65_));
  nor2   g003(.a(new_n65_), .b(x04), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(x40), .O(new_n67_));
  inv1   g005(.a(x28), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x27), .O(new_n71_));
  oai21  g009(.a(new_n67_), .b(new_n64_), .c(new_n71_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n63_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(x16), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n68_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n78_), .c(x04), .O(z01));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n77_), .c(x04), .O(new_n83_));
  aoi21  g021(.a(x29), .b(x08), .c(x02), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n83_), .O(z02));
  inv1   g025(.a(x35), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n68_), .c(x27), .O(new_n89_));
  nand2  g027(.a(new_n69_), .b(new_n75_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n89_), .c(x21), .O(z03));
  inv1   g029(.a(x21), .O(new_n92_));
  nand3  g030(.a(new_n90_), .b(new_n89_), .c(new_n92_), .O(z04));
  aoi21  g031(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g032(.a(z06), .O(z05));
  inv1   g033(.a(x03), .O(new_n96_));
  inv1   g034(.a(x00), .O(new_n97_));
  oai21  g035(.a(x25), .b(new_n97_), .c(x38), .O(new_n98_));
  nand2  g036(.a(x17), .b(new_n63_), .O(new_n99_));
  nor2   g037(.a(x33), .b(x31), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x25), .O(new_n103_));
  nand3  g041(.a(x38), .b(new_n103_), .c(new_n97_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n101_), .c(new_n96_), .O(z07));
  inv1   g044(.a(new_n85_), .O(z08));
  nand4  g045(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(z09));
  inv1   g047(.a(x05), .O(new_n110_));
  nand2  g048(.a(new_n85_), .b(x07), .O(new_n111_));
  oai21  g049(.a(new_n85_), .b(new_n110_), .c(new_n111_), .O(new_n112_));
  nor3   g050(.a(new_n85_), .b(new_n79_), .c(new_n110_), .O(new_n113_));
  aoi21  g051(.a(new_n112_), .b(new_n78_), .c(new_n113_), .O(new_n114_));
  nand3  g052(.a(x37), .b(x27), .c(x06), .O(new_n115_));
  oai21  g053(.a(new_n114_), .b(x04), .c(new_n115_), .O(z10));
  nand2  g054(.a(z08), .b(x29), .O(new_n117_));
  inv1   g055(.a(x04), .O(new_n118_));
  nor2   g056(.a(new_n75_), .b(new_n118_), .O(new_n119_));
  oai21  g057(.a(new_n119_), .b(new_n77_), .c(new_n117_), .O(new_n120_));
  inv1   g058(.a(x08), .O(new_n121_));
  nand4  g059(.a(x35), .b(new_n68_), .c(x27), .d(new_n118_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nor2   g061(.a(x30), .b(x09), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(new_n120_), .c(new_n125_), .O(z11));
  oai21  g064(.a(new_n85_), .b(x04), .c(new_n71_), .O(new_n127_));
  oai21  g065(.a(x27), .b(x10), .c(new_n127_), .O(z12));
  inv1   g066(.a(x13), .O(new_n129_));
  nand2  g067(.a(x36), .b(x35), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x28), .O(new_n131_));
  nand4  g069(.a(new_n131_), .b(new_n66_), .c(x40), .d(new_n129_), .O(new_n132_));
  inv1   g070(.a(x18), .O(new_n133_));
  inv1   g071(.a(x19), .O(new_n134_));
  nand3  g072(.a(x20), .b(new_n134_), .c(new_n133_), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n70_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n132_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x27), .O(new_n139_));
  nor2   g077(.a(x13), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(z08), .c(new_n80_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(z13));
  inv1   g080(.a(x30), .O(new_n143_));
  inv1   g081(.a(x32), .O(new_n144_));
  nand4  g082(.a(new_n130_), .b(new_n144_), .c(new_n143_), .d(x28), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  nand2  g084(.a(new_n140_), .b(z08), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n146_), .c(new_n135_), .O(new_n148_));
  aoi22  g086(.a(new_n147_), .b(new_n71_), .c(new_n79_), .d(new_n75_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(z14));
  nand4  g088(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(z15));
  nand2  g090(.a(x22), .b(x01), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(x23), .O(z16));
  inv1   g092(.a(x24), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x23), .c(x22), .d(x01), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(z17));
  oai21  g095(.a(new_n77_), .b(x27), .c(new_n117_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x08), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n124_), .c(new_n122_), .O(z18));
  oai21  g098(.a(new_n88_), .b(new_n75_), .c(new_n121_), .O(new_n161_));
  oai21  g099(.a(new_n68_), .b(x27), .c(x35), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n117_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n161_), .c(new_n125_), .O(z20));
  aoi21  g102(.a(new_n123_), .b(new_n120_), .c(new_n125_), .O(z19));
endmodule


