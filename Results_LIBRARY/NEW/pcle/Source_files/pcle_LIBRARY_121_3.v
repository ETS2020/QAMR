// Benchmark "FAU" written by ABC on Thu Jun 25 16:30:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  nand4  g00(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  nand2  g02(.a(x09), .b(new_n30_), .O(new_n31_));
  nand2  g03(.a(x12), .b(x11), .O(new_n32_));
  nand2  g04(.a(x14), .b(x13), .O(new_n33_));
  nor4   g05(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n29_), .O(z0));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  inv1   g08(.a(x11), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  oai21  g10(.a(new_n38_), .b(new_n31_), .c(new_n35_), .O(z1));
  xnor2a g11(.a(x12), .b(x11), .O(new_n40_));
  inv1   g12(.a(new_n31_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  nand2  g14(.a(x08), .b(x01), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(new_n40_), .c(new_n43_), .O(z2));
  nand2  g16(.a(new_n32_), .b(x13), .O(new_n45_));
  inv1   g17(.a(x13), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(x12), .c(x11), .d(new_n36_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x02), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z3));
  xnor2a g23(.a(new_n32_), .b(x14), .O(new_n52_));
  nor2   g24(.a(x13), .b(x10), .O(new_n53_));
  aoi22  g25(.a(new_n53_), .b(x14), .c(new_n52_), .d(x13), .O(new_n54_));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n31_), .c(new_n55_), .O(z4));
  inv1   g28(.a(x15), .O(new_n57_));
  nand2  g29(.a(new_n33_), .b(new_n36_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n45_), .c(new_n57_), .O(new_n59_));
  nor3   g31(.a(new_n33_), .b(new_n32_), .c(x15), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n59_), .c(new_n41_), .O(new_n61_));
  nand2  g33(.a(x08), .b(x04), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z5));
  inv1   g35(.a(x16), .O(new_n64_));
  nand2  g36(.a(x15), .b(x14), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n46_), .c(new_n36_), .O(new_n66_));
  aoi21  g38(.a(new_n66_), .b(new_n45_), .c(new_n64_), .O(new_n67_));
  nand3  g39(.a(x13), .b(x12), .c(x11), .O(new_n68_));
  nor3   g40(.a(new_n68_), .b(new_n65_), .c(x16), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(new_n67_), .c(new_n41_), .O(new_n70_));
  nand2  g42(.a(x08), .b(x05), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z6));
  inv1   g44(.a(x17), .O(new_n73_));
  nand4  g45(.a(x16), .b(x15), .c(x14), .d(x13), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n36_), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n45_), .c(new_n73_), .O(new_n76_));
  nand2  g48(.a(new_n73_), .b(x16), .O(new_n77_));
  nor3   g49(.a(new_n77_), .b(new_n68_), .c(new_n65_), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n76_), .c(new_n41_), .O(new_n79_));
  nand2  g51(.a(x08), .b(x06), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n79_), .O(z7));
  inv1   g53(.a(x18), .O(new_n82_));
  nand3  g54(.a(x17), .b(x16), .c(x15), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n33_), .c(new_n36_), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n45_), .c(new_n82_), .O(new_n85_));
  nand2  g57(.a(new_n82_), .b(x17), .O(new_n86_));
  nor3   g58(.a(new_n86_), .b(new_n74_), .c(new_n32_), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n85_), .c(new_n41_), .O(new_n88_));
  nand2  g60(.a(x08), .b(x07), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n88_), .O(z8));
endmodule


