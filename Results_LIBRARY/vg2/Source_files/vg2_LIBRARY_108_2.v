// Benchmark "FAU" written by ABC on Thu Jun 25 17:02:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nor2   g004(.a(x13), .b(x05), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x19), .O(new_n39_));
  nand3  g006(.a(x24), .b(x13), .c(x07), .O(new_n40_));
  inv1   g007(.a(x02), .O(new_n41_));
  nand3  g008(.a(new_n35_), .b(new_n41_), .c(x00), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  aoi21  g010(.a(new_n37_), .b(x05), .c(new_n43_), .O(new_n44_));
  inv1   g011(.a(x06), .O(new_n45_));
  inv1   g012(.a(x11), .O(new_n46_));
  nor2   g013(.a(x03), .b(x01), .O(new_n47_));
  nor2   g014(.a(x20), .b(x14), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n44_), .O(z0));
  inv1   g017(.a(x15), .O(new_n51_));
  inv1   g018(.a(x21), .O(new_n52_));
  nand2  g019(.a(x13), .b(x05), .O(new_n53_));
  oai22  g020(.a(new_n53_), .b(new_n51_), .c(new_n52_), .d(x02), .O(new_n54_));
  nand2  g021(.a(x03), .b(x01), .O(new_n55_));
  nand4  g022(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g024(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n58_));
  nand4  g025(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n59_));
  nor2   g026(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n57_), .c(new_n54_), .O(new_n61_));
  inv1   g028(.a(x00), .O(new_n62_));
  inv1   g029(.a(x19), .O(new_n63_));
  oai22  g030(.a(new_n53_), .b(new_n63_), .c(x02), .d(new_n62_), .O(new_n64_));
  nor2   g031(.a(x06), .b(x04), .O(new_n65_));
  nor2   g032(.a(x09), .b(x08), .O(new_n66_));
  nand3  g033(.a(new_n66_), .b(new_n65_), .c(new_n47_), .O(new_n67_));
  inv1   g034(.a(new_n67_), .O(new_n68_));
  inv1   g035(.a(x16), .O(new_n69_));
  inv1   g036(.a(x17), .O(new_n70_));
  inv1   g037(.a(x22), .O(new_n71_));
  inv1   g038(.a(x23), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  inv1   g040(.a(x12), .O(new_n74_));
  inv1   g041(.a(x14), .O(new_n75_));
  inv1   g042(.a(x20), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n46_), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n68_), .c(new_n64_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n61_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n35_), .O(new_n81_));
  nand3  g048(.a(x11), .b(x09), .c(x04), .O(new_n82_));
  inv1   g049(.a(new_n82_), .O(new_n83_));
  nand4  g050(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  nand4  g052(.a(x23), .b(x22), .c(x17), .d(x16), .O(new_n86_));
  nand4  g053(.a(x20), .b(x18), .c(x14), .d(x12), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g055(.a(new_n88_), .b(new_n85_), .c(new_n83_), .O(new_n89_));
  nor2   g056(.a(x12), .b(x11), .O(new_n90_));
  nor2   g057(.a(new_n34_), .b(x06), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n90_), .c(new_n66_), .d(new_n48_), .O(new_n92_));
  inv1   g059(.a(new_n73_), .O(new_n93_));
  nor3   g060(.a(x04), .b(x03), .c(x01), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  oai21  g062(.a(new_n95_), .b(new_n92_), .c(new_n89_), .O(new_n96_));
  nor2   g063(.a(new_n38_), .b(new_n35_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand4  g065(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n58_), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n85_), .c(new_n83_), .O(new_n101_));
  inv1   g068(.a(x09), .O(new_n102_));
  nand4  g069(.a(new_n75_), .b(new_n74_), .c(new_n46_), .d(new_n102_), .O(new_n103_));
  inv1   g070(.a(new_n103_), .O(new_n104_));
  inv1   g071(.a(x08), .O(new_n105_));
  nand4  g072(.a(new_n76_), .b(x19), .c(new_n105_), .d(new_n45_), .O(new_n106_));
  inv1   g073(.a(new_n106_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  oai21  g075(.a(new_n108_), .b(new_n95_), .c(new_n101_), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n38_), .O(new_n110_));
  nand3  g077(.a(new_n110_), .b(new_n98_), .c(new_n81_), .O(z1));
  nand2  g078(.a(x24), .b(x18), .O(new_n112_));
  nand2  g079(.a(x15), .b(x13), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(x24), .c(new_n112_), .O(new_n114_));
  nand3  g081(.a(new_n35_), .b(x21), .c(new_n41_), .O(new_n115_));
  nand3  g082(.a(x24), .b(x18), .c(x13), .O(new_n116_));
  nand2  g083(.a(new_n38_), .b(x15), .O(new_n117_));
  nand3  g084(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(new_n118_));
  aoi21  g085(.a(new_n114_), .b(x05), .c(new_n118_), .O(new_n119_));
  nand4  g086(.a(x20), .b(x14), .c(x11), .d(x06), .O(new_n120_));
  nor3   g087(.a(new_n120_), .b(new_n119_), .c(new_n55_), .O(z2));
  nor2   g088(.a(new_n75_), .b(new_n46_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(new_n85_), .c(new_n54_), .d(x20), .O(new_n123_));
  nor2   g090(.a(x08), .b(x06), .O(new_n124_));
  nor3   g091(.a(x20), .b(x14), .c(x11), .O(new_n125_));
  nand4  g092(.a(new_n125_), .b(new_n124_), .c(new_n64_), .d(new_n47_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n35_), .O(new_n128_));
  nand3  g095(.a(new_n122_), .b(x20), .c(x18), .O(new_n129_));
  nand3  g096(.a(new_n48_), .b(new_n46_), .c(new_n105_), .O(new_n130_));
  nand2  g097(.a(new_n91_), .b(new_n47_), .O(new_n131_));
  oai22  g098(.a(new_n131_), .b(new_n130_), .c(new_n129_), .d(new_n84_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n97_), .O(new_n133_));
  nand3  g100(.a(new_n122_), .b(x20), .c(x15), .O(new_n134_));
  nand3  g101(.a(new_n47_), .b(new_n75_), .c(new_n46_), .O(new_n135_));
  oai22  g102(.a(new_n135_), .b(new_n106_), .c(new_n134_), .d(new_n84_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n38_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n133_), .c(new_n128_), .O(z3));
  aoi21  g105(.a(new_n72_), .b(x04), .c(x17), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(x22), .c(new_n102_), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(new_n69_), .c(x08), .O(new_n141_));
  inv1   g108(.a(x04), .O(new_n142_));
  aoi21  g109(.a(x23), .b(new_n142_), .c(new_n70_), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(new_n71_), .c(x09), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(x16), .c(new_n105_), .O(new_n145_));
  oai22  g112(.a(new_n145_), .b(new_n119_), .c(new_n141_), .d(new_n44_), .O(z4));
  inv1   g113(.a(new_n44_), .O(z5));
  oai21  g114(.a(new_n76_), .b(x14), .c(new_n45_), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(new_n46_), .c(x03), .O(new_n149_));
  inv1   g116(.a(x03), .O(new_n150_));
  oai21  g117(.a(x20), .b(new_n75_), .c(x06), .O(new_n151_));
  aoi21  g118(.a(new_n151_), .b(x11), .c(new_n150_), .O(new_n152_));
  oai22  g119(.a(new_n152_), .b(new_n119_), .c(new_n149_), .d(new_n44_), .O(z6));
  inv1   g120(.a(new_n119_), .O(z7));
endmodule


