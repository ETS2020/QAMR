// Benchmark "FAU" written by ABC on Thu Jun 25 17:02:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nor2   g004(.a(x13), .b(x05), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x19), .O(new_n39_));
  nand3  g006(.a(x24), .b(x13), .c(x07), .O(new_n40_));
  inv1   g007(.a(x02), .O(new_n41_));
  inv1   g008(.a(x10), .O(new_n42_));
  nand4  g009(.a(new_n35_), .b(new_n42_), .c(new_n41_), .d(x00), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n40_), .c(new_n39_), .O(new_n44_));
  aoi21  g011(.a(new_n37_), .b(x05), .c(new_n44_), .O(new_n45_));
  nor2   g012(.a(x03), .b(x01), .O(new_n46_));
  nor2   g013(.a(x11), .b(x06), .O(new_n47_));
  nor2   g014(.a(x20), .b(x14), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n45_), .O(z0));
  nand3  g017(.a(x15), .b(x13), .c(x05), .O(new_n51_));
  nor2   g018(.a(x10), .b(x02), .O(new_n52_));
  nand2  g019(.a(new_n52_), .b(x21), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g021(.a(x03), .b(x01), .O(new_n55_));
  nand4  g022(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g024(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n58_));
  nand4  g025(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n59_));
  nor2   g026(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n57_), .c(new_n54_), .O(new_n61_));
  nand2  g028(.a(new_n52_), .b(x00), .O(new_n62_));
  nand3  g029(.a(x19), .b(x13), .c(x05), .O(new_n63_));
  nand2  g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  inv1   g031(.a(x08), .O(new_n65_));
  inv1   g032(.a(x09), .O(new_n66_));
  nor2   g033(.a(x06), .b(x04), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n46_), .c(new_n66_), .d(new_n65_), .O(new_n68_));
  inv1   g035(.a(new_n68_), .O(new_n69_));
  inv1   g036(.a(x16), .O(new_n70_));
  inv1   g037(.a(x17), .O(new_n71_));
  inv1   g038(.a(x22), .O(new_n72_));
  inv1   g039(.a(x23), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nor2   g041(.a(x12), .b(x11), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n48_), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand3  g044(.a(new_n77_), .b(new_n69_), .c(new_n64_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n61_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n35_), .O(new_n80_));
  nand4  g047(.a(x12), .b(x11), .c(x09), .d(x04), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand4  g049(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n83_));
  inv1   g050(.a(new_n83_), .O(new_n84_));
  nand3  g051(.a(x23), .b(x22), .c(x20), .O(new_n85_));
  inv1   g052(.a(new_n85_), .O(new_n86_));
  nand4  g053(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n86_), .c(new_n84_), .d(new_n82_), .O(new_n89_));
  nor2   g056(.a(x09), .b(x04), .O(new_n90_));
  nor2   g057(.a(x08), .b(x06), .O(new_n91_));
  inv1   g058(.a(x19), .O(new_n92_));
  nor2   g059(.a(x20), .b(new_n92_), .O(new_n93_));
  nand4  g060(.a(new_n93_), .b(new_n91_), .c(new_n90_), .d(new_n46_), .O(new_n94_));
  nor2   g061(.a(x22), .b(x17), .O(new_n95_));
  nor2   g062(.a(x16), .b(x14), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n95_), .c(new_n75_), .d(new_n73_), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n94_), .c(new_n89_), .O(new_n98_));
  nor2   g065(.a(x08), .b(new_n34_), .O(new_n99_));
  nor2   g066(.a(x11), .b(x09), .O(new_n100_));
  nand4  g067(.a(new_n100_), .b(new_n99_), .c(new_n67_), .d(new_n46_), .O(new_n101_));
  nor2   g068(.a(x17), .b(x16), .O(new_n102_));
  inv1   g069(.a(x05), .O(new_n103_));
  inv1   g070(.a(x13), .O(new_n104_));
  aoi21  g071(.a(new_n104_), .b(new_n103_), .c(new_n35_), .O(new_n105_));
  nor3   g072(.a(x23), .b(x22), .c(x20), .O(new_n106_));
  nor2   g073(.a(x14), .b(x12), .O(new_n107_));
  nand4  g074(.a(new_n107_), .b(new_n106_), .c(new_n105_), .d(new_n102_), .O(new_n108_));
  nor2   g075(.a(new_n108_), .b(new_n101_), .O(new_n109_));
  aoi21  g076(.a(new_n98_), .b(new_n38_), .c(new_n109_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n80_), .O(z1));
  nand2  g078(.a(new_n104_), .b(new_n103_), .O(new_n112_));
  nand3  g079(.a(new_n35_), .b(x13), .c(x05), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(x15), .O(new_n115_));
  nand3  g082(.a(new_n52_), .b(new_n35_), .c(x21), .O(new_n116_));
  inv1   g083(.a(new_n55_), .O(new_n117_));
  inv1   g084(.a(x14), .O(new_n118_));
  inv1   g085(.a(x20), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(new_n117_), .c(x11), .d(x06), .O(new_n121_));
  aoi21  g088(.a(new_n116_), .b(new_n115_), .c(new_n121_), .O(z2));
  inv1   g089(.a(x11), .O(new_n123_));
  nor2   g090(.a(new_n118_), .b(new_n123_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n84_), .c(new_n54_), .d(x20), .O(new_n125_));
  nor3   g092(.a(x20), .b(x14), .c(x11), .O(new_n126_));
  nand2  g093(.a(new_n91_), .b(new_n46_), .O(new_n127_));
  inv1   g094(.a(new_n127_), .O(new_n128_));
  nand3  g095(.a(new_n128_), .b(new_n126_), .c(new_n64_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n35_), .O(new_n131_));
  nand2  g098(.a(new_n93_), .b(new_n91_), .O(new_n132_));
  nand3  g099(.a(new_n124_), .b(x20), .c(x15), .O(new_n133_));
  nand3  g100(.a(new_n46_), .b(new_n118_), .c(new_n123_), .O(new_n134_));
  oai22  g101(.a(new_n134_), .b(new_n132_), .c(new_n133_), .d(new_n83_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n38_), .O(new_n136_));
  nor2   g103(.a(x11), .b(new_n34_), .O(new_n137_));
  nand4  g104(.a(new_n137_), .b(new_n128_), .c(new_n105_), .d(new_n48_), .O(new_n138_));
  nand3  g105(.a(new_n138_), .b(new_n136_), .c(new_n131_), .O(z3));
  aoi21  g106(.a(new_n73_), .b(x04), .c(x17), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(x22), .c(new_n66_), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(new_n70_), .c(x08), .O(new_n142_));
  inv1   g109(.a(x04), .O(new_n143_));
  aoi21  g110(.a(x23), .b(new_n143_), .c(new_n71_), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(new_n72_), .c(x09), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(x16), .c(new_n65_), .O(new_n146_));
  nand2  g113(.a(x24), .b(x18), .O(new_n147_));
  nand3  g114(.a(new_n35_), .b(x15), .c(x13), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(new_n147_), .c(new_n103_), .O(new_n149_));
  nand2  g116(.a(new_n38_), .b(x15), .O(new_n150_));
  nand3  g117(.a(x24), .b(x18), .c(x13), .O(new_n151_));
  nand3  g118(.a(new_n151_), .b(new_n150_), .c(new_n116_), .O(new_n152_));
  nor2   g119(.a(new_n152_), .b(new_n149_), .O(new_n153_));
  oai22  g120(.a(new_n153_), .b(new_n146_), .c(new_n142_), .d(new_n45_), .O(z4));
  nand2  g121(.a(new_n37_), .b(x05), .O(new_n155_));
  inv1   g122(.a(new_n44_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n155_), .O(z5));
  inv1   g124(.a(x03), .O(new_n158_));
  nand2  g125(.a(new_n116_), .b(new_n115_), .O(new_n159_));
  nand2  g126(.a(new_n119_), .b(x14), .O(new_n160_));
  aoi21  g127(.a(new_n160_), .b(x06), .c(new_n123_), .O(new_n161_));
  oai21  g128(.a(new_n161_), .b(new_n158_), .c(new_n159_), .O(new_n162_));
  aoi21  g129(.a(x20), .b(new_n118_), .c(x06), .O(new_n163_));
  oai21  g130(.a(new_n163_), .b(x11), .c(new_n158_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(z5), .O(new_n165_));
  aoi21  g132(.a(new_n118_), .b(x06), .c(new_n123_), .O(new_n166_));
  nor2   g133(.a(new_n147_), .b(new_n38_), .O(new_n167_));
  oai21  g134(.a(new_n166_), .b(new_n158_), .c(new_n167_), .O(new_n168_));
  nand3  g135(.a(new_n168_), .b(new_n165_), .c(new_n162_), .O(z6));
  inv1   g136(.a(new_n153_), .O(z7));
endmodule


