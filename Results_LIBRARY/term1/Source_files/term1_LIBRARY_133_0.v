// Benchmark "FAU" written by ABC on Thu Jun 25 19:58:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x32), .O(z0));
  nor2   g001(.a(x32), .b(x03), .O(new_n46_));
  inv1   g002(.a(x03), .O(new_n47_));
  nor2   g003(.a(x33), .b(new_n47_), .O(new_n48_));
  oai21  g004(.a(new_n48_), .b(new_n46_), .c(x02), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  nor2   g006(.a(x32), .b(new_n47_), .O(new_n51_));
  nor2   g007(.a(x33), .b(x03), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(new_n49_), .O(z1));
  xnor2a g010(.a(x07), .b(x04), .O(new_n55_));
  inv1   g011(.a(x01), .O(new_n56_));
  nand2  g012(.a(x03), .b(x02), .O(new_n57_));
  nand2  g013(.a(new_n47_), .b(new_n50_), .O(new_n58_));
  aoi21  g014(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n47_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x06), .c(new_n50_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n50_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x05), .c(new_n47_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n61_), .c(x01), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n59_), .c(new_n55_), .O(new_n65_));
  xor2a  g021(.a(x07), .b(x04), .O(new_n66_));
  xnor2a g022(.a(x03), .b(x02), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n56_), .O(new_n68_));
  nand2  g024(.a(new_n62_), .b(new_n60_), .O(new_n69_));
  nand4  g025(.a(x06), .b(x05), .c(new_n47_), .d(new_n50_), .O(new_n70_));
  aoi21  g026(.a(new_n70_), .b(new_n69_), .c(x01), .O(new_n71_));
  oai21  g027(.a(new_n71_), .b(new_n68_), .c(new_n66_), .O(new_n72_));
  inv1   g028(.a(x09), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(x08), .O(new_n74_));
  aoi21  g030(.a(new_n72_), .b(new_n65_), .c(new_n74_), .O(z2));
  oai22  g031(.a(x22), .b(x17), .c(x20), .d(x15), .O(new_n76_));
  oai22  g032(.a(x24), .b(x19), .c(x23), .d(x18), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n76_), .c(new_n57_), .O(new_n78_));
  inv1   g034(.a(x26), .O(new_n79_));
  inv1   g035(.a(x27), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n79_), .c(x25), .d(x01), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n78_), .O(z3));
  xor2a  g038(.a(x28), .b(x27), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n79_), .c(x25), .d(x01), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n78_), .O(z4));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n80_), .O(new_n87_));
  xor2a  g043(.a(new_n87_), .b(x29), .O(new_n88_));
  nor2   g044(.a(new_n77_), .b(new_n76_), .O(new_n89_));
  nand4  g045(.a(new_n57_), .b(new_n79_), .c(x25), .d(x01), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z5));
  inv1   g049(.a(x30), .O(new_n94_));
  nand2  g050(.a(new_n87_), .b(x29), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(new_n96_));
  nand3  g052(.a(new_n87_), .b(new_n94_), .c(x29), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(new_n94_), .c(new_n97_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n91_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z6));
  inv1   g056(.a(x29), .O(new_n101_));
  nor2   g057(.a(new_n94_), .b(new_n101_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n87_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(x31), .O(new_n104_));
  inv1   g060(.a(x31), .O(new_n105_));
  nand3  g061(.a(new_n96_), .b(new_n105_), .c(x30), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n104_), .c(new_n91_), .O(z7));
  inv1   g063(.a(x17), .O(new_n108_));
  inv1   g064(.a(x18), .O(new_n109_));
  nand3  g065(.a(x19), .b(new_n109_), .c(x13), .O(new_n110_));
  inv1   g066(.a(x19), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(x18), .c(x14), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n110_), .c(new_n108_), .O(new_n113_));
  nand4  g069(.a(x19), .b(x18), .c(new_n108_), .d(x12), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n113_), .c(x15), .O(new_n116_));
  inv1   g072(.a(x15), .O(new_n117_));
  nor2   g073(.a(new_n111_), .b(new_n109_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(x17), .c(new_n117_), .d(x10), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand2  g076(.a(new_n86_), .b(new_n80_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n94_), .c(x29), .O(new_n122_));
  nand2  g078(.a(new_n121_), .b(x29), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n94_), .c(new_n122_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n120_), .c(z0), .d(x31), .O(new_n126_));
  nand2  g082(.a(new_n118_), .b(x12), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n108_), .O(new_n128_));
  nand3  g084(.a(new_n118_), .b(x17), .c(x10), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n117_), .O(new_n130_));
  nand2  g086(.a(new_n94_), .b(new_n101_), .O(new_n131_));
  inv1   g087(.a(x14), .O(new_n132_));
  aoi21  g088(.a(new_n111_), .b(new_n132_), .c(new_n105_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n131_), .c(new_n130_), .d(new_n128_), .O(new_n134_));
  nand2  g090(.a(new_n102_), .b(x27), .O(new_n135_));
  oai21  g091(.a(x30), .b(x27), .c(new_n135_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n86_), .O(new_n137_));
  nand2  g093(.a(new_n102_), .b(x28), .O(new_n138_));
  aoi21  g094(.a(x19), .b(x13), .c(x18), .O(new_n139_));
  nor2   g095(.a(new_n108_), .b(new_n117_), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n118_), .c(new_n139_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n138_), .c(new_n137_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n134_), .c(x32), .O(new_n143_));
  nand2  g099(.a(new_n79_), .b(x00), .O(new_n144_));
  aoi21  g100(.a(new_n143_), .b(new_n126_), .c(new_n144_), .O(z8));
  inv1   g101(.a(x33), .O(new_n146_));
  nand2  g102(.a(new_n120_), .b(new_n146_), .O(new_n147_));
  nand2  g103(.a(new_n105_), .b(new_n94_), .O(new_n148_));
  nand2  g104(.a(x33), .b(x31), .O(new_n149_));
  oai21  g105(.a(new_n148_), .b(new_n147_), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n123_), .O(new_n151_));
  nor2   g107(.a(new_n105_), .b(new_n94_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n120_), .c(new_n146_), .O(new_n153_));
  nand2  g109(.a(x33), .b(new_n105_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  aoi21  g111(.a(x18), .b(x12), .c(x17), .O(new_n156_));
  nand2  g112(.a(new_n105_), .b(x30), .O(new_n157_));
  nand2  g113(.a(x31), .b(new_n94_), .O(new_n158_));
  inv1   g114(.a(x13), .O(new_n159_));
  nand2  g115(.a(new_n109_), .b(new_n159_), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n158_), .c(new_n157_), .O(new_n161_));
  inv1   g117(.a(x10), .O(new_n162_));
  nand2  g118(.a(x18), .b(x17), .O(new_n163_));
  oai21  g119(.a(new_n163_), .b(new_n162_), .c(new_n117_), .O(new_n164_));
  nand2  g120(.a(new_n111_), .b(x14), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(new_n140_), .c(x18), .O(new_n166_));
  oai21  g122(.a(new_n163_), .b(new_n117_), .c(new_n111_), .O(new_n167_));
  nand3  g123(.a(new_n167_), .b(new_n166_), .c(new_n164_), .O(new_n168_));
  nor3   g124(.a(new_n168_), .b(new_n161_), .c(new_n156_), .O(new_n169_));
  nor2   g125(.a(new_n169_), .b(new_n146_), .O(new_n170_));
  aoi21  g126(.a(new_n155_), .b(new_n124_), .c(new_n170_), .O(new_n171_));
  aoi21  g127(.a(new_n171_), .b(new_n151_), .c(new_n144_), .O(z9));
endmodule


