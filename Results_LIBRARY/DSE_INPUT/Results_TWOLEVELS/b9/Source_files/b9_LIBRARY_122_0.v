// Benchmark "FAU" written by ABC on Thu Jun 25 20:41:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n152_, new_n154_, new_n155_,
    new_n157_, new_n158_, new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x15), .O(new_n63_));
  aoi21  g001(.a(x36), .b(x27), .c(x10), .O(new_n64_));
  inv1   g002(.a(x39), .O(new_n65_));
  nor2   g003(.a(new_n65_), .b(x04), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(x40), .O(new_n67_));
  nor2   g005(.a(x37), .b(x35), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x27), .O(new_n70_));
  oai21  g008(.a(new_n67_), .b(new_n64_), .c(new_n70_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n63_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(x16), .O(z00));
  inv1   g011(.a(x27), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n78_), .c(x04), .O(z01));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n77_), .c(x04), .O(new_n83_));
  aoi21  g021(.a(x29), .b(x08), .c(x02), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n83_), .O(z02));
  inv1   g025(.a(x35), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n76_), .c(x27), .O(new_n89_));
  nor2   g027(.a(x37), .b(x27), .O(z06));
  inv1   g028(.a(z06), .O(z05));
  nand3  g029(.a(z05), .b(new_n89_), .c(x21), .O(z03));
  inv1   g030(.a(x21), .O(new_n93_));
  nand3  g031(.a(z05), .b(new_n89_), .c(new_n93_), .O(z04));
  inv1   g032(.a(x03), .O(new_n95_));
  inv1   g033(.a(x00), .O(new_n96_));
  oai21  g034(.a(x25), .b(new_n96_), .c(x38), .O(new_n97_));
  nand2  g035(.a(x17), .b(new_n63_), .O(new_n98_));
  nor2   g036(.a(x33), .b(x31), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x25), .O(new_n102_));
  nand3  g040(.a(x38), .b(new_n102_), .c(new_n96_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n100_), .c(new_n95_), .O(z07));
  inv1   g043(.a(new_n85_), .O(z08));
  nand4  g044(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z09));
  inv1   g046(.a(x05), .O(new_n109_));
  nand2  g047(.a(new_n85_), .b(x07), .O(new_n110_));
  oai21  g048(.a(new_n85_), .b(new_n109_), .c(new_n110_), .O(new_n111_));
  nor3   g049(.a(new_n85_), .b(new_n79_), .c(new_n109_), .O(new_n112_));
  aoi21  g050(.a(new_n111_), .b(new_n78_), .c(new_n112_), .O(new_n113_));
  nand3  g051(.a(x37), .b(x27), .c(x06), .O(new_n114_));
  oai21  g052(.a(new_n113_), .b(x04), .c(new_n114_), .O(z10));
  nand2  g053(.a(z08), .b(x29), .O(new_n116_));
  inv1   g054(.a(x04), .O(new_n117_));
  nor2   g055(.a(new_n74_), .b(new_n117_), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(new_n77_), .c(new_n116_), .O(new_n119_));
  inv1   g057(.a(x08), .O(new_n120_));
  nand4  g058(.a(x35), .b(new_n76_), .c(x27), .d(new_n117_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nor2   g060(.a(x30), .b(x09), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n119_), .c(new_n124_), .O(z11));
  oai21  g063(.a(new_n85_), .b(x04), .c(new_n70_), .O(new_n126_));
  aoi21  g064(.a(new_n68_), .b(new_n75_), .c(new_n74_), .O(new_n127_));
  oai21  g065(.a(new_n127_), .b(x10), .c(new_n126_), .O(z12));
  inv1   g066(.a(x13), .O(new_n129_));
  nand2  g067(.a(x36), .b(x35), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x28), .O(new_n131_));
  nand4  g069(.a(new_n131_), .b(new_n66_), .c(x40), .d(new_n129_), .O(new_n132_));
  inv1   g070(.a(x18), .O(new_n133_));
  inv1   g071(.a(x19), .O(new_n134_));
  nand3  g072(.a(x20), .b(new_n134_), .c(new_n133_), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n69_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n132_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x27), .O(new_n139_));
  nor2   g077(.a(x13), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(z08), .c(new_n80_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(z13));
  inv1   g080(.a(x30), .O(new_n143_));
  inv1   g081(.a(x32), .O(new_n144_));
  nand4  g082(.a(new_n130_), .b(new_n144_), .c(new_n143_), .d(x28), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n140_), .c(z08), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n135_), .O(new_n147_));
  nand2  g085(.a(new_n141_), .b(new_n70_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(z14));
  nand4  g087(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(z15));
  nand2  g089(.a(x22), .b(x01), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(x23), .O(z16));
  inv1   g091(.a(x24), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(x23), .c(x22), .d(x01), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(z17));
  oai21  g094(.a(new_n77_), .b(x27), .c(new_n116_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x08), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n123_), .c(new_n121_), .O(z18));
  oai21  g097(.a(new_n88_), .b(new_n74_), .c(new_n120_), .O(new_n160_));
  oai21  g098(.a(new_n76_), .b(x27), .c(x35), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n116_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n160_), .c(new_n124_), .O(z20));
  aoi21  g101(.a(new_n122_), .b(new_n119_), .c(new_n124_), .O(z19));
endmodule


