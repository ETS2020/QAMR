// Benchmark "FAU" written by ABC on Thu Jun 25 17:16:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x24), .O(new_n45_));
  inv1   g01(.a(x25), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g03(.a(new_n47_), .b(x26), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x09), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x08), .O(new_n53_));
  inv1   g09(.a(x19), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(x10), .O(new_n55_));
  nand4  g11(.a(new_n55_), .b(new_n53_), .c(new_n51_), .d(new_n49_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  inv1   g15(.a(x01), .O(new_n60_));
  inv1   g16(.a(x08), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n60_), .O(z02));
  inv1   g18(.a(x02), .O(new_n63_));
  nor2   g19(.a(new_n61_), .b(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  inv1   g26(.a(x06), .O(new_n71_));
  nor2   g27(.a(new_n61_), .b(new_n71_), .O(z07));
  inv1   g28(.a(x07), .O(new_n73_));
  nor2   g29(.a(new_n61_), .b(new_n73_), .O(z08));
  nor2   g30(.a(x24), .b(x21), .O(new_n75_));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  nand4  g32(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n77_));
  oai22  g33(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(x19), .O(new_n78_));
  inv1   g34(.a(x10), .O(new_n79_));
  nand2  g35(.a(new_n53_), .b(new_n79_), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n58_), .O(z09));
  nor3   g39(.a(new_n75_), .b(x20), .c(new_n54_), .O(new_n84_));
  nand3  g40(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n76_), .c(x19), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(x20), .c(new_n84_), .O(new_n87_));
  oai22  g43(.a(new_n87_), .b(new_n80_), .c(new_n61_), .d(new_n60_), .O(z10));
  xor2a  g44(.a(x21), .b(x20), .O(new_n89_));
  nand3  g45(.a(x26), .b(x25), .c(x24), .O(new_n90_));
  nand3  g46(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n90_), .c(x19), .O(new_n92_));
  aoi22  g48(.a(new_n92_), .b(x21), .c(new_n89_), .d(x19), .O(new_n93_));
  oai22  g49(.a(new_n93_), .b(new_n80_), .c(new_n61_), .d(new_n63_), .O(z11));
  nand2  g50(.a(x20), .b(x19), .O(new_n95_));
  inv1   g51(.a(new_n95_), .O(new_n96_));
  inv1   g52(.a(new_n90_), .O(new_n97_));
  nand3  g53(.a(new_n97_), .b(x23), .c(x14), .O(new_n98_));
  nand3  g54(.a(new_n96_), .b(new_n98_), .c(x21), .O(new_n99_));
  inv1   g55(.a(x21), .O(new_n100_));
  nor2   g56(.a(x22), .b(new_n100_), .O(new_n101_));
  aoi22  g57(.a(new_n101_), .b(new_n96_), .c(new_n99_), .d(x22), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n80_), .c(new_n65_), .O(z12));
  inv1   g59(.a(x23), .O(new_n104_));
  nand4  g60(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n105_));
  nand4  g61(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  nand3  g64(.a(new_n104_), .b(x22), .c(x21), .O(new_n109_));
  nor2   g65(.a(new_n109_), .b(new_n95_), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n108_), .c(new_n81_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n67_), .O(z13));
  nand2  g68(.a(x23), .b(x22), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(x24), .c(x21), .O(new_n114_));
  nor2   g70(.a(new_n45_), .b(x20), .O(new_n115_));
  aoi21  g71(.a(new_n114_), .b(x20), .c(new_n115_), .O(new_n116_));
  nand3  g72(.a(x26), .b(x25), .c(x16), .O(new_n117_));
  nand4  g73(.a(new_n117_), .b(x23), .c(x22), .d(x19), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(x24), .O(new_n119_));
  oai21  g75(.a(new_n116_), .b(new_n54_), .c(new_n119_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n81_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n69_), .O(z14));
  inv1   g78(.a(new_n113_), .O(new_n123_));
  nand3  g79(.a(x21), .b(x20), .c(x19), .O(new_n124_));
  inv1   g80(.a(new_n124_), .O(new_n125_));
  nand2  g81(.a(x26), .b(x17), .O(new_n126_));
  nand4  g82(.a(new_n126_), .b(new_n125_), .c(new_n123_), .d(x24), .O(new_n127_));
  nand4  g83(.a(new_n46_), .b(x24), .c(x23), .d(x22), .O(new_n128_));
  nor2   g84(.a(new_n128_), .b(new_n124_), .O(new_n129_));
  aoi21  g85(.a(new_n127_), .b(x25), .c(new_n129_), .O(new_n130_));
  oai22  g86(.a(new_n130_), .b(new_n80_), .c(new_n61_), .d(new_n71_), .O(z15));
  nor2   g87(.a(new_n54_), .b(x18), .O(new_n132_));
  nand3  g88(.a(new_n132_), .b(new_n51_), .c(new_n47_), .O(new_n133_));
  inv1   g89(.a(x26), .O(new_n134_));
  nand4  g90(.a(new_n134_), .b(x25), .c(x22), .d(x21), .O(new_n135_));
  nand4  g91(.a(x24), .b(x23), .c(x20), .d(x19), .O(new_n136_));
  nor2   g92(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  aoi21  g93(.a(new_n133_), .b(x26), .c(new_n137_), .O(new_n138_));
  oai22  g94(.a(new_n138_), .b(new_n80_), .c(new_n61_), .d(new_n73_), .O(z16));
endmodule


