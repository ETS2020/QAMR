// Benchmark "FAU" written by ABC on Thu Jun 25 17:26:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n70_, new_n71_, new_n73_, new_n75_, new_n78_, new_n80_, new_n81_;
  inv1   g00(.a(x19), .O(new_n42_));
  xnor2a g01(.a(x09), .b(x08), .O(new_n43_));
  nor2   g02(.a(x02), .b(x01), .O(new_n44_));
  nor2   g03(.a(x04), .b(x03), .O(new_n45_));
  nor2   g04(.a(x06), .b(x05), .O(new_n46_));
  nand4  g05(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n47_));
  aoi21  g06(.a(new_n47_), .b(x00), .c(new_n42_), .O(z01));
  nand3  g07(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(x00), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  nor2   g10(.a(x09), .b(new_n51_), .O(new_n52_));
  nor3   g11(.a(x07), .b(x06), .c(x05), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n52_), .c(new_n45_), .d(new_n44_), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(new_n50_), .c(new_n42_), .O(z02));
  or2    g14(.a(x21), .b(x20), .O(z04));
  inv1   g15(.a(x10), .O(new_n57_));
  nand2  g16(.a(new_n45_), .b(new_n44_), .O(new_n58_));
  nor2   g17(.a(x08), .b(x07), .O(new_n59_));
  nand3  g18(.a(new_n59_), .b(new_n46_), .c(x19), .O(new_n60_));
  oai21  g19(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(z05));
  inv1   g20(.a(x09), .O(new_n62_));
  oai21  g21(.a(new_n62_), .b(new_n51_), .c(x07), .O(new_n63_));
  nand4  g22(.a(new_n63_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n64_));
  nand4  g23(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x00), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(new_n66_));
  aoi21  g25(.a(new_n66_), .b(new_n64_), .c(new_n42_), .O(z06));
  and2   g26(.a(x24), .b(x18), .O(z07));
  and2   g27(.a(x24), .b(x11), .O(z09));
  inv1   g28(.a(x14), .O(new_n70_));
  inv1   g29(.a(x22), .O(new_n71_));
  nor3   g30(.a(x24), .b(new_n71_), .c(new_n70_), .O(z10));
  inv1   g31(.a(x17), .O(new_n73_));
  nor3   g32(.a(x24), .b(new_n71_), .c(new_n73_), .O(z11));
  inv1   g33(.a(x23), .O(new_n75_));
  nor3   g34(.a(x24), .b(new_n75_), .c(new_n70_), .O(z12));
  nor3   g35(.a(x24), .b(new_n75_), .c(new_n73_), .O(z13));
  inv1   g36(.a(x16), .O(new_n78_));
  nor2   g37(.a(x24), .b(new_n78_), .O(z14));
  nor2   g38(.a(x13), .b(x12), .O(new_n80_));
  nor2   g39(.a(x15), .b(x14), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n80_), .O(z15));
  buf    g41(.a(x00), .O(z00));
  buf    g42(.a(x20), .O(z03));
  buf    g43(.a(x11), .O(z08));
endmodule


