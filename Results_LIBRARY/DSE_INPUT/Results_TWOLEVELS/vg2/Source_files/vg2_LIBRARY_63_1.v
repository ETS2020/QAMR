// Benchmark "FAU" written by ABC on Thu Jun 25 17:02:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_;
  inv1   g000(.a(x20), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  nand2  g002(.a(x19), .b(x13), .O(new_n36_));
  oai21  g003(.a(new_n36_), .b(x24), .c(new_n35_), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x05), .O(new_n38_));
  nand3  g005(.a(x24), .b(x13), .c(x07), .O(new_n39_));
  inv1   g006(.a(new_n39_), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(x19), .c(new_n40_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(new_n34_), .O(new_n44_));
  inv1   g011(.a(x02), .O(new_n45_));
  nor2   g012(.a(x24), .b(x10), .O(new_n46_));
  nand3  g013(.a(new_n46_), .b(new_n45_), .c(x00), .O(new_n47_));
  inv1   g014(.a(x06), .O(new_n48_));
  inv1   g015(.a(x11), .O(new_n49_));
  inv1   g016(.a(x14), .O(new_n50_));
  nor2   g017(.a(x03), .b(x01), .O(new_n51_));
  nand4  g018(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  aoi21  g019(.a(new_n47_), .b(new_n44_), .c(new_n52_), .O(z0));
  nand3  g020(.a(x11), .b(x09), .c(x04), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  nand4  g022(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n58_));
  nand4  g025(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n59_));
  nor2   g026(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n57_), .c(new_n55_), .O(new_n61_));
  nor2   g028(.a(x23), .b(x22), .O(new_n62_));
  nor2   g029(.a(x12), .b(x11), .O(new_n63_));
  inv1   g030(.a(x07), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(x06), .O(new_n65_));
  nor2   g032(.a(x09), .b(x08), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n65_), .c(new_n63_), .d(new_n62_), .O(new_n67_));
  inv1   g034(.a(x04), .O(new_n68_));
  nor2   g035(.a(x16), .b(x14), .O(new_n69_));
  nor2   g036(.a(x20), .b(x17), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n51_), .d(new_n68_), .O(new_n71_));
  oai21  g038(.a(new_n71_), .b(new_n67_), .c(new_n61_), .O(new_n72_));
  inv1   g039(.a(x24), .O(new_n73_));
  nor2   g040(.a(new_n41_), .b(new_n73_), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand4  g042(.a(x20), .b(x17), .c(x16), .d(x15), .O(new_n76_));
  nand4  g043(.a(x23), .b(x22), .c(x14), .d(x12), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n57_), .c(new_n55_), .O(new_n79_));
  nor2   g046(.a(x14), .b(x12), .O(new_n80_));
  nor2   g047(.a(x17), .b(x16), .O(new_n81_));
  inv1   g048(.a(x19), .O(new_n82_));
  nor2   g049(.a(x20), .b(new_n82_), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n81_), .c(new_n80_), .d(new_n62_), .O(new_n84_));
  nor2   g051(.a(x08), .b(x06), .O(new_n85_));
  nor2   g052(.a(x11), .b(x09), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n85_), .c(new_n51_), .d(new_n68_), .O(new_n87_));
  oai21  g054(.a(new_n87_), .b(new_n84_), .c(new_n79_), .O(new_n88_));
  nand3  g055(.a(new_n73_), .b(x13), .c(x05), .O(new_n89_));
  oai21  g056(.a(x13), .b(x05), .c(new_n89_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  inv1   g058(.a(x00), .O(new_n92_));
  nor2   g059(.a(x01), .b(new_n92_), .O(new_n93_));
  nand2  g060(.a(new_n62_), .b(new_n49_), .O(new_n94_));
  nand3  g061(.a(new_n46_), .b(new_n48_), .c(new_n68_), .O(new_n95_));
  inv1   g062(.a(x03), .O(new_n96_));
  nand3  g063(.a(new_n80_), .b(new_n96_), .c(new_n45_), .O(new_n97_));
  nor3   g064(.a(new_n97_), .b(new_n95_), .c(new_n94_), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n93_), .c(new_n81_), .d(new_n66_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n91_), .c(new_n75_), .O(z1));
  inv1   g067(.a(x18), .O(new_n101_));
  nor2   g068(.a(new_n73_), .b(new_n101_), .O(new_n102_));
  nand2  g069(.a(x15), .b(x13), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(x24), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n102_), .c(x05), .O(new_n105_));
  aoi22  g072(.a(new_n102_), .b(x13), .c(new_n41_), .d(x15), .O(new_n106_));
  nor2   g073(.a(new_n49_), .b(new_n48_), .O(new_n107_));
  nor2   g074(.a(new_n34_), .b(new_n50_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(new_n107_), .c(x03), .d(x01), .O(new_n109_));
  aoi21  g076(.a(new_n106_), .b(new_n105_), .c(new_n109_), .O(z2));
  nand4  g077(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n111_));
  inv1   g078(.a(x08), .O(new_n112_));
  nand4  g079(.a(new_n34_), .b(new_n50_), .c(new_n49_), .d(new_n112_), .O(new_n113_));
  nand2  g080(.a(new_n65_), .b(new_n51_), .O(new_n114_));
  oai22  g081(.a(new_n114_), .b(new_n113_), .c(new_n111_), .d(new_n56_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n74_), .O(new_n116_));
  nand4  g083(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n117_));
  nand2  g084(.a(new_n85_), .b(new_n51_), .O(new_n118_));
  nor2   g085(.a(x14), .b(x11), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n83_), .O(new_n120_));
  oai22  g087(.a(new_n120_), .b(new_n118_), .c(new_n117_), .d(new_n56_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n90_), .O(new_n122_));
  nand3  g089(.a(new_n85_), .b(new_n96_), .c(new_n45_), .O(new_n123_));
  inv1   g090(.a(new_n123_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n119_), .c(new_n93_), .d(new_n46_), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(new_n122_), .c(new_n116_), .O(z3));
  inv1   g093(.a(x22), .O(new_n127_));
  inv1   g094(.a(x17), .O(new_n128_));
  oai21  g095(.a(x23), .b(new_n68_), .c(new_n128_), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(new_n127_), .c(x09), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(x16), .c(new_n112_), .O(new_n131_));
  nand3  g098(.a(new_n47_), .b(new_n42_), .c(new_n38_), .O(z5));
  nand2  g099(.a(z5), .b(new_n131_), .O(new_n133_));
  inv1   g100(.a(x16), .O(new_n134_));
  inv1   g101(.a(x09), .O(new_n135_));
  inv1   g102(.a(x23), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(x04), .c(x17), .O(new_n137_));
  aoi21  g104(.a(new_n137_), .b(x22), .c(new_n135_), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(new_n134_), .c(x08), .O(new_n139_));
  inv1   g106(.a(x10), .O(new_n140_));
  nand4  g107(.a(new_n73_), .b(x21), .c(new_n140_), .d(new_n45_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n106_), .c(new_n105_), .O(z7));
  nand2  g109(.a(z7), .b(new_n139_), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n133_), .O(z4));
  inv1   g111(.a(x15), .O(new_n145_));
  oai21  g112(.a(x20), .b(new_n50_), .c(x06), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(x11), .c(new_n96_), .O(new_n147_));
  nand3  g114(.a(new_n119_), .b(x20), .c(x19), .O(new_n148_));
  oai21  g115(.a(new_n147_), .b(new_n145_), .c(new_n148_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n90_), .O(new_n150_));
  nand4  g117(.a(x20), .b(new_n50_), .c(new_n49_), .d(x07), .O(new_n151_));
  oai21  g118(.a(new_n147_), .b(new_n101_), .c(new_n151_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n74_), .O(new_n153_));
  oai21  g120(.a(x11), .b(new_n48_), .c(new_n96_), .O(new_n154_));
  oai21  g121(.a(x14), .b(new_n48_), .c(x11), .O(new_n155_));
  aoi21  g122(.a(new_n155_), .b(x03), .c(new_n141_), .O(new_n156_));
  aoi21  g123(.a(new_n154_), .b(z5), .c(new_n156_), .O(new_n157_));
  nand3  g124(.a(new_n157_), .b(new_n153_), .c(new_n150_), .O(z6));
endmodule


