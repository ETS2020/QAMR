// Benchmark "FAU" written by ABC on Mon Jul 27 21:33:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n85_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  nand2  g02(.a(x3), .b(x2), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand3  g04(.a(new_n21_), .b(x6), .c(x5), .O(new_n22_));
  nor2   g05(.a(x6), .b(x0), .O(new_n23_));
  nand3  g06(.a(x3), .b(x2), .c(x1), .O(new_n24_));
  nand2  g07(.a(x5), .b(x4), .O(new_n25_));
  aoi21  g08(.a(new_n24_), .b(new_n23_), .c(new_n25_), .O(z9));
  aoi21  g09(.a(new_n22_), .b(new_n18_), .c(z9), .O(z0));
  inv1   g10(.a(x5), .O(new_n28_));
  inv1   g11(.a(x0), .O(new_n29_));
  oai21  g12(.a(x3), .b(x2), .c(x1), .O(new_n30_));
  nor2   g13(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  oai21  g14(.a(new_n31_), .b(x6), .c(x4), .O(new_n32_));
  aoi21  g15(.a(x3), .b(x2), .c(x4), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n23_), .c(new_n19_), .O(new_n34_));
  inv1   g17(.a(x6), .O(new_n35_));
  nand3  g18(.a(new_n20_), .b(new_n35_), .c(new_n29_), .O(new_n36_));
  oai21  g19(.a(new_n35_), .b(x0), .c(new_n18_), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(new_n36_), .c(x5), .O(new_n38_));
  inv1   g21(.a(new_n38_), .O(new_n39_));
  aoi22  g22(.a(new_n39_), .b(new_n34_), .c(new_n32_), .d(new_n28_), .O(z1));
  inv1   g23(.a(new_n30_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x0), .O(new_n42_));
  oai21  g25(.a(new_n24_), .b(x6), .c(new_n29_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x5), .O(new_n44_));
  aoi21  g27(.a(x3), .b(x2), .c(x1), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n28_), .c(x6), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n44_), .c(new_n42_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n18_), .O(new_n48_));
  oai21  g31(.a(new_n35_), .b(x0), .c(new_n28_), .O(new_n49_));
  aoi21  g32(.a(new_n41_), .b(x0), .c(new_n49_), .O(new_n50_));
  nand2  g33(.a(new_n24_), .b(new_n23_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n22_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n50_), .c(x4), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n48_), .O(z2));
  inv1   g37(.a(x2), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n19_), .c(x0), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n24_), .c(new_n35_), .O(new_n57_));
  nand2  g40(.a(new_n45_), .b(x6), .O(new_n58_));
  nand2  g41(.a(x3), .b(x1), .O(new_n59_));
  inv1   g42(.a(new_n59_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(x0), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n58_), .c(new_n57_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(x5), .O(new_n63_));
  nand3  g46(.a(x3), .b(x2), .c(new_n29_), .O(new_n64_));
  aoi21  g47(.a(new_n64_), .b(new_n35_), .c(new_n19_), .O(new_n65_));
  nand2  g48(.a(new_n30_), .b(x0), .O(new_n66_));
  nand3  g49(.a(x6), .b(x3), .c(x2), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  oai21  g51(.a(new_n68_), .b(new_n65_), .c(new_n28_), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(new_n63_), .O(z3));
  nor2   g53(.a(x2), .b(x1), .O(new_n71_));
  nand2  g54(.a(new_n20_), .b(new_n35_), .O(new_n72_));
  aoi21  g55(.a(new_n67_), .b(new_n72_), .c(x1), .O(new_n73_));
  inv1   g56(.a(new_n36_), .O(new_n74_));
  nand2  g57(.a(new_n67_), .b(x1), .O(new_n75_));
  nor2   g58(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  oai22  g59(.a(new_n76_), .b(new_n73_), .c(new_n71_), .d(new_n66_), .O(z4));
  inv1   g60(.a(new_n20_), .O(new_n78_));
  aoi21  g61(.a(new_n59_), .b(new_n55_), .c(new_n78_), .O(new_n79_));
  oai21  g62(.a(new_n35_), .b(x2), .c(x0), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  oai21  g64(.a(new_n79_), .b(new_n29_), .c(new_n81_), .O(z5));
  nand2  g65(.a(x3), .b(new_n55_), .O(new_n83_));
  aoi22  g66(.a(new_n83_), .b(new_n19_), .c(new_n60_), .d(new_n55_), .O(z6));
  nor2   g67(.a(x3), .b(x2), .O(new_n85_));
  nor2   g68(.a(new_n85_), .b(new_n78_), .O(z7));
  inv1   g69(.a(x3), .O(z8));
endmodule


