// Benchmark "FAU" written by ABC on Mon Jul 27 18:37:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  nand2  g00(.a(x20), .b(x19), .O(new_n45_));
  nand3  g01(.a(x23), .b(x22), .c(x21), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  nor4   g06(.a(new_n50_), .b(new_n49_), .c(new_n46_), .d(new_n45_), .O(z00));
  inv1   g07(.a(x00), .O(new_n52_));
  nor2   g08(.a(new_n47_), .b(new_n52_), .O(z01));
  inv1   g09(.a(x01), .O(new_n54_));
  nor2   g10(.a(new_n47_), .b(new_n54_), .O(z02));
  inv1   g11(.a(x02), .O(new_n56_));
  nor2   g12(.a(new_n47_), .b(new_n56_), .O(z03));
  inv1   g13(.a(x03), .O(new_n58_));
  nor2   g14(.a(new_n47_), .b(new_n58_), .O(z04));
  nand2  g15(.a(x08), .b(x04), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z05));
  nand2  g17(.a(x08), .b(x05), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z06));
  nand2  g19(.a(x08), .b(x06), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z07));
  inv1   g21(.a(x07), .O(new_n66_));
  nor2   g22(.a(new_n47_), .b(new_n66_), .O(z08));
  inv1   g23(.a(new_n49_), .O(new_n68_));
  nand4  g24(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n69_));
  and2   g25(.a(x22), .b(x21), .O(new_n70_));
  nand3  g26(.a(new_n70_), .b(x20), .c(x11), .O(new_n71_));
  oai21  g27(.a(new_n71_), .b(new_n69_), .c(x19), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  oai21  g29(.a(new_n47_), .b(new_n52_), .c(new_n73_), .O(z09));
  inv1   g30(.a(new_n45_), .O(new_n75_));
  inv1   g31(.a(new_n69_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n70_), .c(x12), .O(new_n77_));
  and2   g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  oai21  g34(.a(x20), .b(x19), .c(new_n68_), .O(new_n79_));
  oai22  g35(.a(new_n79_), .b(new_n78_), .c(new_n47_), .d(new_n54_), .O(z10));
  nand3  g36(.a(x21), .b(x20), .c(x19), .O(new_n81_));
  and2   g37(.a(x22), .b(x13), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n76_), .c(new_n81_), .O(new_n83_));
  oai21  g39(.a(new_n75_), .b(x21), .c(new_n68_), .O(new_n84_));
  oai22  g40(.a(new_n84_), .b(new_n83_), .c(new_n47_), .d(new_n56_), .O(z11));
  nor2   g41(.a(x10), .b(x08), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  inv1   g43(.a(x22), .O(new_n88_));
  inv1   g44(.a(new_n81_), .O(new_n89_));
  inv1   g45(.a(x14), .O(new_n90_));
  oai21  g46(.a(new_n69_), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  inv1   g47(.a(x09), .O(new_n92_));
  nor2   g48(.a(new_n88_), .b(new_n92_), .O(new_n93_));
  aoi22  g49(.a(new_n93_), .b(new_n91_), .c(new_n89_), .d(new_n88_), .O(new_n94_));
  oai22  g50(.a(new_n94_), .b(new_n87_), .c(new_n47_), .d(new_n58_), .O(z12));
  inv1   g51(.a(x23), .O(new_n96_));
  nor2   g52(.a(new_n96_), .b(x22), .O(new_n97_));
  nand4  g53(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n98_));
  nand4  g54(.a(new_n98_), .b(x21), .c(x20), .d(x19), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(x23), .O(new_n100_));
  nand3  g56(.a(new_n89_), .b(new_n96_), .c(x22), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n100_), .c(new_n92_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n97_), .c(new_n86_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n60_), .O(z13));
  inv1   g60(.a(x24), .O(new_n105_));
  nor2   g61(.a(new_n105_), .b(x22), .O(new_n106_));
  oai21  g62(.a(new_n46_), .b(new_n45_), .c(new_n105_), .O(new_n107_));
  nand3  g63(.a(x26), .b(x25), .c(x16), .O(new_n108_));
  nand4  g64(.a(new_n108_), .b(new_n89_), .c(x24), .d(x23), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n107_), .c(x09), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n106_), .c(new_n86_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n62_), .O(z14));
  nand2  g69(.a(x26), .b(x17), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(x24), .c(x23), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n81_), .c(x25), .O(new_n116_));
  inv1   g72(.a(x25), .O(new_n117_));
  inv1   g73(.a(new_n46_), .O(new_n118_));
  nand4  g74(.a(new_n118_), .b(new_n75_), .c(new_n117_), .d(x24), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n116_), .c(new_n92_), .O(new_n120_));
  nor2   g76(.a(new_n117_), .b(x22), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n120_), .c(new_n86_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n64_), .O(z15));
  nand4  g79(.a(x24), .b(x23), .c(x20), .d(x19), .O(new_n124_));
  inv1   g80(.a(new_n124_), .O(new_n125_));
  inv1   g81(.a(x26), .O(new_n126_));
  nor2   g82(.a(new_n126_), .b(x18), .O(new_n127_));
  nand4  g83(.a(new_n127_), .b(new_n125_), .c(x25), .d(x21), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(x09), .c(new_n88_), .O(new_n129_));
  nand3  g85(.a(x25), .b(x22), .c(x21), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n124_), .c(new_n126_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n86_), .O(new_n132_));
  oai22  g88(.a(new_n132_), .b(new_n129_), .c(new_n47_), .d(new_n66_), .O(z16));
endmodule


