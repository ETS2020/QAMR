// Benchmark "FAU" written by ABC on Mon Jul 27 20:52:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n40_, new_n42_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n68_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  nand2  g02(.a(x04), .b(new_n27_), .O(new_n28_));
  nand2  g03(.a(x05), .b(x02), .O(new_n29_));
  nand3  g04(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(z00));
  inv1   g05(.a(z00), .O(z01));
  inv1   g06(.a(x00), .O(new_n32_));
  inv1   g07(.a(x01), .O(new_n33_));
  inv1   g08(.a(x04), .O(new_n34_));
  inv1   g09(.a(x13), .O(new_n35_));
  nand4  g10(.a(new_n35_), .b(new_n34_), .c(new_n26_), .d(new_n27_), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(z07));
  nand3  g12(.a(z07), .b(new_n33_), .c(new_n32_), .O(new_n38_));
  inv1   g13(.a(new_n38_), .O(z02));
  nand3  g14(.a(z07), .b(new_n33_), .c(x00), .O(new_n40_));
  inv1   g15(.a(new_n40_), .O(z03));
  nand3  g16(.a(z07), .b(x01), .c(new_n32_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(z04));
  nand3  g18(.a(z07), .b(x01), .c(x00), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(z05));
  inv1   g20(.a(x07), .O(new_n46_));
  inv1   g21(.a(x12), .O(new_n47_));
  nand3  g22(.a(x13), .b(new_n47_), .c(x05), .O(new_n48_));
  nor2   g23(.a(new_n48_), .b(new_n28_), .O(new_n49_));
  inv1   g24(.a(x11), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n33_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(x00), .O(new_n54_));
  inv1   g29(.a(x10), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x08), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n33_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(new_n32_), .O(new_n59_));
  nand4  g34(.a(new_n59_), .b(new_n54_), .c(new_n49_), .d(new_n46_), .O(new_n60_));
  inv1   g35(.a(x05), .O(new_n61_));
  nand3  g36(.a(new_n35_), .b(new_n61_), .c(x02), .O(new_n62_));
  aoi21  g37(.a(new_n62_), .b(new_n60_), .c(x03), .O(z06));
  inv1   g38(.a(new_n62_), .O(new_n64_));
  nor2   g39(.a(new_n64_), .b(new_n49_), .O(new_n65_));
  nor2   g40(.a(new_n65_), .b(x03), .O(z08));
  and2   g41(.a(x13), .b(x06), .O(z09));
  nand3  g42(.a(new_n29_), .b(x06), .c(new_n26_), .O(new_n68_));
  inv1   g43(.a(new_n68_), .O(z10));
endmodule


