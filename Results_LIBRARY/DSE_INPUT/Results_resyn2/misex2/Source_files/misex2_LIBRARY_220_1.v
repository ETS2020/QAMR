// Benchmark "FAU" written by ABC on Mon Jul 27 18:46:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n111_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor2   g01(.a(x02), .b(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  nand3  g13(.a(x11), .b(x10), .c(new_n44_), .O(new_n57_));
  inv1   g14(.a(new_n57_), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(x12), .c(x02), .d(x01), .O(new_n59_));
  nand4  g16(.a(new_n45_), .b(new_n49_), .c(x18), .d(new_n47_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n59_), .O(z03));
  inv1   g18(.a(x02), .O(new_n62_));
  inv1   g19(.a(x12), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x10), .c(new_n44_), .d(x01), .O(new_n64_));
  nor3   g21(.a(new_n64_), .b(x11), .c(new_n62_), .O(z04));
  nand4  g22(.a(x10), .b(x09), .c(x02), .d(x01), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(z05));
  inv1   g24(.a(x01), .O(new_n68_));
  nor3   g25(.a(new_n57_), .b(new_n62_), .c(new_n68_), .O(z06));
  nand2  g26(.a(new_n68_), .b(x00), .O(new_n70_));
  inv1   g27(.a(new_n64_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(x11), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(new_n70_), .c(new_n62_), .O(z07));
  nand3  g30(.a(x19), .b(new_n48_), .c(x17), .O(new_n74_));
  nor2   g31(.a(x06), .b(x05), .O(new_n75_));
  nor2   g32(.a(x08), .b(x07), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  inv1   g34(.a(x03), .O(new_n78_));
  nor2   g35(.a(new_n62_), .b(x00), .O(z17));
  nand3  g36(.a(z17), .b(x04), .c(new_n78_), .O(new_n80_));
  nor3   g37(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(z08));
  inv1   g38(.a(x16), .O(new_n82_));
  inv1   g39(.a(x21), .O(new_n83_));
  inv1   g40(.a(x22), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(new_n83_), .c(x20), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  inv1   g43(.a(x11), .O(new_n87_));
  inv1   g44(.a(x13), .O(new_n88_));
  inv1   g45(.a(x14), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n88_), .c(x12), .d(new_n87_), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n86_), .c(z17), .d(new_n82_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(x15), .O(z09));
  inv1   g50(.a(x15), .O(new_n94_));
  nor2   g51(.a(new_n82_), .b(new_n94_), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n91_), .c(new_n86_), .d(z17), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(z10));
  nor2   g54(.a(new_n92_), .b(new_n94_), .O(z11));
  oai21  g55(.a(new_n54_), .b(new_n62_), .c(x01), .O(new_n99_));
  inv1   g56(.a(x00), .O(new_n100_));
  oai21  g57(.a(x19), .b(x02), .c(x23), .O(new_n101_));
  nand3  g58(.a(new_n49_), .b(x17), .c(new_n62_), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  inv1   g61(.a(x24), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(x09), .O(new_n106_));
  aoi21  g63(.a(new_n104_), .b(new_n99_), .c(new_n106_), .O(z12));
  nand3  g64(.a(new_n45_), .b(new_n49_), .c(x17), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(z13));
  nor2   g66(.a(new_n55_), .b(new_n46_), .O(z14));
  nor2   g67(.a(x19), .b(x00), .O(new_n111_));
  oai22  g68(.a(new_n111_), .b(x02), .c(x10), .d(new_n68_), .O(z15));
  zero   g69(.O(z16));
endmodule


