// Benchmark "FAU" written by ABC on Mon Jul 27 18:45:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n122_, new_n123_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  nand3  g13(.a(x02), .b(x01), .c(x00), .O(new_n57_));
  nand3  g14(.a(x11), .b(x10), .c(new_n44_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n57_), .O(z06));
  nand2  g16(.a(z06), .b(x12), .O(new_n60_));
  nand2  g17(.a(new_n45_), .b(new_n49_), .O(new_n61_));
  nand2  g18(.a(x18), .b(new_n47_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(z03));
  inv1   g20(.a(x11), .O(new_n64_));
  inv1   g21(.a(x12), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(x10), .d(new_n44_), .O(new_n66_));
  nor2   g23(.a(new_n66_), .b(new_n57_), .O(z04));
  nand2  g24(.a(x10), .b(x02), .O(new_n68_));
  inv1   g25(.a(x00), .O(new_n69_));
  inv1   g26(.a(x01), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(x09), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n68_), .O(z05));
  inv1   g30(.a(new_n58_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n65_), .O(new_n75_));
  nand2  g32(.a(x02), .b(x00), .O(new_n76_));
  aoi21  g33(.a(new_n75_), .b(x01), .c(new_n76_), .O(z07));
  inv1   g34(.a(x03), .O(new_n78_));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n79_), .c(x04), .d(new_n78_), .O(new_n81_));
  nor2   g38(.a(x01), .b(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x02), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n48_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n83_), .c(new_n81_), .O(z08));
  nand2  g42(.a(x02), .b(new_n70_), .O(new_n86_));
  inv1   g43(.a(x13), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(x12), .c(new_n64_), .O(new_n88_));
  inv1   g45(.a(x22), .O(new_n89_));
  inv1   g46(.a(x20), .O(new_n90_));
  nor2   g47(.a(x21), .b(new_n90_), .O(new_n91_));
  nor2   g48(.a(x14), .b(x00), .O(new_n92_));
  nor2   g49(.a(x16), .b(x15), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n89_), .O(new_n94_));
  nor3   g51(.a(new_n94_), .b(new_n88_), .c(new_n86_), .O(z09));
  nand4  g52(.a(new_n90_), .b(new_n49_), .c(x18), .d(x01), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(x22), .O(new_n98_));
  nor2   g55(.a(new_n88_), .b(new_n86_), .O(new_n99_));
  inv1   g56(.a(x14), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(x20), .c(x15), .d(new_n100_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  inv1   g60(.a(x16), .O(new_n104_));
  nor2   g61(.a(x22), .b(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n103_), .c(new_n99_), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n98_), .c(x00), .O(z10));
  nand3  g64(.a(new_n103_), .b(new_n99_), .c(new_n104_), .O(new_n108_));
  nand2  g65(.a(new_n89_), .b(new_n69_), .O(new_n109_));
  aoi21  g66(.a(new_n108_), .b(new_n96_), .c(new_n109_), .O(z11));
  nand2  g67(.a(new_n68_), .b(new_n71_), .O(new_n111_));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  inv1   g69(.a(x02), .O(new_n113_));
  nand3  g70(.a(new_n49_), .b(x17), .c(new_n113_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n82_), .O(new_n116_));
  inv1   g73(.a(x24), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x09), .O(new_n118_));
  aoi21  g75(.a(new_n116_), .b(new_n111_), .c(new_n118_), .O(z12));
  nor2   g76(.a(new_n61_), .b(new_n47_), .O(z13));
  nor2   g77(.a(new_n55_), .b(new_n46_), .O(z14));
  aoi21  g78(.a(new_n54_), .b(x01), .c(new_n113_), .O(new_n122_));
  nand3  g79(.a(x19), .b(new_n113_), .c(new_n70_), .O(new_n123_));
  oai21  g80(.a(new_n122_), .b(new_n69_), .c(new_n123_), .O(z15));
  nor2   g81(.a(new_n70_), .b(x00), .O(z16));
  inv1   g82(.a(new_n83_), .O(z17));
endmodule


