// Benchmark "FAU" written by ABC on Mon Jul 27 18:45:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n117_, new_n120_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(x10), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n46_), .O(z00));
  nand2  g07(.a(new_n45_), .b(x09), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n49_), .O(z01));
  inv1   g09(.a(x10), .O(new_n53_));
  nand3  g10(.a(new_n48_), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n51_), .O(z02));
  nand3  g12(.a(x02), .b(x01), .c(x00), .O(new_n56_));
  nand3  g13(.a(x11), .b(x10), .c(new_n44_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n56_), .O(z06));
  nand2  g15(.a(z06), .b(x12), .O(new_n59_));
  nand3  g16(.a(new_n45_), .b(x18), .c(new_n47_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n59_), .O(z03));
  inv1   g18(.a(x11), .O(new_n62_));
  inv1   g19(.a(x12), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n62_), .c(x10), .d(new_n44_), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n56_), .O(z04));
  nand2  g22(.a(x10), .b(x02), .O(new_n66_));
  inv1   g23(.a(x00), .O(new_n67_));
  inv1   g24(.a(x01), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(x09), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n66_), .O(z05));
  inv1   g28(.a(new_n57_), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n63_), .O(new_n73_));
  nand2  g30(.a(x02), .b(x00), .O(new_n74_));
  aoi21  g31(.a(new_n73_), .b(x01), .c(new_n74_), .O(z07));
  inv1   g32(.a(x19), .O(new_n77_));
  inv1   g33(.a(x20), .O(new_n78_));
  nand4  g34(.a(new_n78_), .b(new_n77_), .c(x18), .d(x01), .O(new_n79_));
  nand2  g35(.a(x02), .b(new_n68_), .O(new_n80_));
  inv1   g36(.a(x13), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(x12), .c(new_n62_), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nor2   g39(.a(x15), .b(x14), .O(new_n84_));
  nor2   g40(.a(new_n78_), .b(x16), .O(new_n85_));
  nand3  g41(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(new_n86_));
  inv1   g42(.a(x21), .O(new_n87_));
  inv1   g43(.a(x22), .O(new_n88_));
  nand3  g44(.a(new_n88_), .b(new_n87_), .c(new_n67_), .O(new_n89_));
  aoi21  g45(.a(new_n86_), .b(new_n79_), .c(new_n89_), .O(z09));
  inv1   g46(.a(new_n79_), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(x22), .c(x21), .O(new_n92_));
  inv1   g48(.a(x14), .O(new_n93_));
  nand4  g49(.a(new_n87_), .b(x20), .c(x15), .d(new_n93_), .O(new_n94_));
  inv1   g50(.a(new_n94_), .O(new_n95_));
  inv1   g51(.a(x16), .O(new_n96_));
  nor2   g52(.a(x22), .b(new_n96_), .O(new_n97_));
  nand3  g53(.a(new_n97_), .b(new_n95_), .c(new_n83_), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(new_n92_), .c(x00), .O(z10));
  nand2  g55(.a(new_n88_), .b(new_n67_), .O(new_n100_));
  nand2  g56(.a(new_n91_), .b(x21), .O(new_n101_));
  nand3  g57(.a(new_n95_), .b(new_n83_), .c(new_n96_), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(z11));
  nand2  g59(.a(new_n66_), .b(new_n69_), .O(new_n104_));
  nor2   g60(.a(x01), .b(x00), .O(new_n105_));
  inv1   g61(.a(x23), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(x02), .O(new_n107_));
  inv1   g63(.a(x02), .O(new_n108_));
  nand2  g64(.a(new_n47_), .b(new_n108_), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n107_), .c(new_n105_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(x09), .O(new_n112_));
  aoi21  g68(.a(new_n110_), .b(new_n104_), .c(new_n112_), .O(z12));
  nand2  g69(.a(new_n45_), .b(x17), .O(new_n114_));
  inv1   g70(.a(new_n114_), .O(z13));
  nor2   g71(.a(new_n54_), .b(new_n46_), .O(z14));
  nand2  g72(.a(new_n53_), .b(x01), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(x02), .c(new_n67_), .O(z15));
  nor2   g74(.a(new_n68_), .b(x00), .O(z16));
  nand2  g75(.a(new_n105_), .b(x02), .O(new_n120_));
  inv1   g76(.a(new_n120_), .O(z17));
  zero   g77(.O(z08));
endmodule


