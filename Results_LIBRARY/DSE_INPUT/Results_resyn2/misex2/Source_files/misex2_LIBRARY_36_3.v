// Benchmark "FAU" written by ABC on Mon Jul 27 18:45:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n121_, new_n124_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x01), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(x02), .O(new_n60_));
  nand3  g17(.a(x11), .b(x10), .c(new_n44_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n60_), .O(z06));
  nand2  g19(.a(z06), .b(x12), .O(new_n63_));
  nand4  g20(.a(new_n45_), .b(new_n49_), .c(x18), .d(new_n47_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n63_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x10), .d(new_n44_), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n60_), .O(z04));
  inv1   g26(.a(new_n59_), .O(new_n70_));
  nand2  g27(.a(x10), .b(x02), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n70_), .c(new_n44_), .O(z05));
  inv1   g29(.a(new_n61_), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(new_n67_), .O(new_n74_));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  aoi21  g32(.a(new_n74_), .b(x01), .c(new_n75_), .O(z07));
  nand3  g33(.a(x19), .b(new_n48_), .c(x17), .O(new_n77_));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  inv1   g37(.a(x03), .O(new_n81_));
  nand4  g38(.a(x04), .b(new_n81_), .c(x02), .d(new_n57_), .O(new_n82_));
  nor3   g39(.a(new_n82_), .b(new_n80_), .c(new_n77_), .O(z08));
  inv1   g40(.a(x20), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x18), .c(x01), .O(new_n85_));
  inv1   g42(.a(x16), .O(new_n86_));
  nand2  g43(.a(x02), .b(new_n58_), .O(new_n87_));
  inv1   g44(.a(x13), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(x12), .c(new_n66_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nor2   g47(.a(x15), .b(x14), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(x20), .d(new_n86_), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(new_n57_), .O(new_n95_));
  aoi21  g52(.a(new_n92_), .b(new_n85_), .c(new_n95_), .O(z09));
  nand4  g53(.a(x21), .b(new_n84_), .c(x18), .d(x01), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(x22), .O(new_n99_));
  inv1   g56(.a(x14), .O(new_n100_));
  nand4  g57(.a(new_n93_), .b(x20), .c(x15), .d(new_n100_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n90_), .c(new_n94_), .d(x16), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(new_n99_), .c(x00), .O(z10));
  nand2  g61(.a(new_n94_), .b(new_n57_), .O(new_n105_));
  nand3  g62(.a(new_n102_), .b(new_n90_), .c(new_n86_), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n97_), .c(new_n105_), .O(z11));
  nand2  g64(.a(new_n71_), .b(new_n59_), .O(new_n108_));
  nand2  g65(.a(x23), .b(x19), .O(new_n109_));
  aoi21  g66(.a(new_n49_), .b(x17), .c(x02), .O(new_n110_));
  inv1   g67(.a(x02), .O(new_n111_));
  oai21  g68(.a(x23), .b(new_n111_), .c(new_n58_), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n57_), .O(new_n114_));
  inv1   g71(.a(x24), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x09), .O(new_n116_));
  aoi21  g73(.a(new_n114_), .b(new_n108_), .c(new_n116_), .O(z12));
  nand2  g74(.a(new_n45_), .b(new_n49_), .O(new_n118_));
  nor2   g75(.a(new_n118_), .b(new_n47_), .O(z13));
  nor2   g76(.a(new_n55_), .b(new_n46_), .O(z14));
  oai21  g77(.a(x19), .b(x00), .c(new_n111_), .O(new_n121_));
  oai21  g78(.a(new_n70_), .b(x10), .c(new_n121_), .O(z15));
  nor2   g79(.a(new_n58_), .b(x00), .O(z16));
  nand3  g80(.a(x02), .b(new_n58_), .c(new_n57_), .O(new_n124_));
  inv1   g81(.a(new_n124_), .O(z17));
endmodule


