// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x15), .O(new_n34_));
  inv1   g06(.a(x16), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n33_), .c(x18), .d(x17), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n31_), .O(z0));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  oai21  g12(.a(x17), .b(x12), .c(new_n40_), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(new_n31_), .c(new_n39_), .O(z1));
  nand2  g14(.a(x08), .b(x01), .O(new_n43_));
  xnor2a g15(.a(x12), .b(x11), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n31_), .c(new_n43_), .O(z2));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  and2   g18(.a(x12), .b(x11), .O(new_n47_));
  nor2   g19(.a(new_n47_), .b(x13), .O(new_n48_));
  inv1   g20(.a(new_n31_), .O(new_n49_));
  nand2  g21(.a(new_n47_), .b(x13), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai21  g23(.a(new_n51_), .b(new_n48_), .c(new_n46_), .O(z3));
  nand2  g24(.a(x08), .b(x03), .O(new_n53_));
  aoi21  g25(.a(new_n47_), .b(x13), .c(x14), .O(new_n54_));
  nand2  g26(.a(new_n32_), .b(new_n49_), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(z4));
  nand2  g28(.a(x08), .b(x04), .O(new_n57_));
  nand2  g29(.a(new_n33_), .b(x15), .O(new_n58_));
  nand2  g30(.a(new_n32_), .b(new_n34_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n58_), .c(new_n49_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n57_), .O(z5));
  nand2  g33(.a(x08), .b(x05), .O(new_n62_));
  nand2  g34(.a(new_n58_), .b(new_n35_), .O(new_n63_));
  aoi21  g35(.a(new_n36_), .b(new_n33_), .c(new_n31_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n62_), .O(z6));
  nand2  g38(.a(x08), .b(x06), .O(new_n67_));
  nand2  g39(.a(x14), .b(x13), .O(new_n68_));
  inv1   g40(.a(x17), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(x16), .c(x15), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n68_), .c(x12), .O(new_n71_));
  inv1   g43(.a(new_n68_), .O(new_n72_));
  nand3  g44(.a(x16), .b(x15), .c(x11), .O(new_n73_));
  inv1   g45(.a(new_n73_), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n72_), .c(new_n69_), .O(new_n75_));
  aoi21  g47(.a(new_n71_), .b(x11), .c(new_n75_), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n31_), .c(new_n67_), .O(z7));
  nand2  g49(.a(x08), .b(x07), .O(new_n78_));
  inv1   g50(.a(x18), .O(new_n79_));
  nand3  g51(.a(new_n36_), .b(new_n33_), .c(x17), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n37_), .c(new_n49_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n78_), .O(z8));
endmodule


