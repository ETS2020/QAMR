// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand2  g02(.a(new_n30_), .b(x15), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x18), .c(x17), .d(x16), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  nand2  g09(.a(x08), .b(x00), .O(new_n38_));
  inv1   g10(.a(x11), .O(new_n39_));
  oai21  g11(.a(x14), .b(x13), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n34_), .c(new_n38_), .O(z1));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  inv1   g15(.a(x12), .O(new_n44_));
  oai21  g16(.a(x14), .b(x13), .c(x11), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n35_), .c(new_n43_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n42_), .O(z2));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand2  g23(.a(new_n43_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n50_), .c(new_n35_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n49_), .O(z3));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  aoi21  g27(.a(x12), .b(x11), .c(x14), .O(new_n56_));
  nand2  g28(.a(new_n35_), .b(new_n29_), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z4));
  nand2  g30(.a(x08), .b(x04), .O(new_n59_));
  inv1   g31(.a(x15), .O(new_n60_));
  nand2  g32(.a(new_n29_), .b(new_n60_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n35_), .c(new_n31_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n59_), .O(z5));
  nand2  g35(.a(x08), .b(x05), .O(new_n64_));
  nand3  g36(.a(new_n30_), .b(x16), .c(x15), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n31_), .b(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n65_), .c(new_n35_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n64_), .O(z6));
  nand2  g41(.a(x08), .b(x06), .O(new_n70_));
  nor2   g42(.a(new_n66_), .b(new_n60_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n30_), .c(x17), .O(new_n72_));
  inv1   g44(.a(x17), .O(new_n73_));
  nand2  g45(.a(new_n71_), .b(new_n30_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n72_), .c(new_n35_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n70_), .O(z7));
  nand4  g49(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n50_), .c(x18), .O(new_n79_));
  nor2   g51(.a(x18), .b(new_n73_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n71_), .c(new_n30_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n35_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x07), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(z8));
endmodule


