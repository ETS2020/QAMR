// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  nand4  g04(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n33_));
  nor3   g05(.a(new_n33_), .b(new_n32_), .c(new_n29_), .O(z0));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  inv1   g07(.a(x11), .O(new_n36_));
  inv1   g08(.a(x09), .O(new_n37_));
  nand2  g09(.a(new_n31_), .b(new_n30_), .O(new_n38_));
  aoi21  g10(.a(x17), .b(new_n37_), .c(new_n38_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n36_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n35_), .O(z1));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(new_n32_), .c(new_n42_), .O(z2));
  nand2  g16(.a(x08), .b(x02), .O(new_n45_));
  aoi21  g17(.a(x12), .b(x11), .c(x13), .O(new_n46_));
  nand3  g18(.a(x13), .b(x12), .c(x11), .O(new_n47_));
  inv1   g19(.a(new_n32_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n46_), .c(new_n45_), .O(z3));
  nand2  g22(.a(x08), .b(x03), .O(new_n51_));
  inv1   g23(.a(new_n47_), .O(new_n52_));
  nor2   g24(.a(new_n52_), .b(x14), .O(new_n53_));
  nand2  g25(.a(new_n48_), .b(new_n29_), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(z4));
  nand2  g27(.a(x08), .b(x04), .O(new_n56_));
  inv1   g28(.a(x15), .O(new_n57_));
  nor2   g29(.a(new_n29_), .b(new_n57_), .O(new_n58_));
  nand2  g30(.a(new_n29_), .b(new_n57_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n48_), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(z5));
  nand2  g33(.a(x08), .b(x05), .O(new_n62_));
  nand2  g34(.a(new_n58_), .b(x16), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  oai21  g36(.a(new_n29_), .b(new_n57_), .c(new_n64_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n63_), .c(new_n48_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n62_), .O(z6));
  nand2  g39(.a(x08), .b(x06), .O(new_n68_));
  nand3  g40(.a(x16), .b(x15), .c(x14), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n52_), .c(x17), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  oai21  g44(.a(new_n69_), .b(new_n47_), .c(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n71_), .c(new_n39_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n68_), .O(z7));
  nand2  g47(.a(x08), .b(x07), .O(new_n76_));
  nand4  g48(.a(new_n70_), .b(new_n52_), .c(x18), .d(x17), .O(new_n77_));
  inv1   g49(.a(x18), .O(new_n78_));
  nand2  g50(.a(new_n71_), .b(new_n78_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n77_), .c(new_n39_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n76_), .O(z8));
endmodule


