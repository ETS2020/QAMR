// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  nand3  g00(.a(x17), .b(x16), .c(x15), .O(new_n29_));
  nand4  g01(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n30_));
  nor2   g02(.a(x10), .b(x08), .O(new_n31_));
  nand2  g03(.a(new_n31_), .b(x18), .O(new_n32_));
  nor3   g04(.a(new_n32_), .b(new_n30_), .c(new_n29_), .O(z0));
  nand2  g05(.a(x08), .b(x00), .O(new_n34_));
  nand2  g06(.a(new_n31_), .b(x09), .O(new_n35_));
  oai21  g07(.a(new_n35_), .b(x11), .c(new_n34_), .O(z1));
  nand2  g08(.a(x08), .b(x01), .O(new_n37_));
  xnor2a g09(.a(x12), .b(x11), .O(new_n38_));
  oai21  g10(.a(new_n38_), .b(new_n35_), .c(new_n37_), .O(z2));
  nand2  g11(.a(x12), .b(x11), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x13), .O(new_n41_));
  inv1   g13(.a(x13), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(x09), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(new_n40_), .c(new_n41_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n31_), .O(new_n45_));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(z3));
  nand2  g19(.a(x08), .b(x03), .O(new_n48_));
  inv1   g20(.a(x14), .O(new_n49_));
  aoi21  g21(.a(new_n43_), .b(new_n41_), .c(new_n49_), .O(new_n50_));
  nand4  g22(.a(new_n49_), .b(x13), .c(x12), .d(x11), .O(new_n51_));
  inv1   g23(.a(new_n51_), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n50_), .c(new_n31_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n48_), .O(z4));
  nand2  g26(.a(x08), .b(x04), .O(new_n55_));
  nand2  g27(.a(x14), .b(x13), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x09), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n41_), .c(x15), .O(new_n58_));
  inv1   g30(.a(x15), .O(new_n59_));
  nand2  g31(.a(new_n30_), .b(new_n59_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n58_), .c(new_n31_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n55_), .O(z5));
  nand2  g34(.a(x08), .b(x05), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  oai21  g36(.a(new_n56_), .b(new_n59_), .c(x09), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n41_), .c(new_n64_), .O(new_n66_));
  nor3   g38(.a(new_n30_), .b(x16), .c(new_n59_), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n66_), .c(new_n31_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n63_), .O(z6));
  inv1   g41(.a(x17), .O(new_n70_));
  nand4  g42(.a(x16), .b(x15), .c(x14), .d(x13), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(x09), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(new_n41_), .c(new_n70_), .O(new_n73_));
  nor3   g45(.a(new_n71_), .b(new_n40_), .c(x17), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n73_), .c(new_n31_), .O(new_n75_));
  nand2  g47(.a(x08), .b(x06), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n75_), .O(z7));
  inv1   g49(.a(x18), .O(new_n78_));
  oai21  g50(.a(new_n56_), .b(new_n29_), .c(x09), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(new_n41_), .c(new_n78_), .O(new_n80_));
  nand2  g52(.a(new_n78_), .b(x17), .O(new_n81_));
  nor3   g53(.a(new_n81_), .b(new_n71_), .c(new_n40_), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n80_), .c(new_n31_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x07), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(z8));
endmodule


