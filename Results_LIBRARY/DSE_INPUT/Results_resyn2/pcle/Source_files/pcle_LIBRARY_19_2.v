// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand3  g02(.a(x17), .b(x16), .c(x15), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  inv1   g06(.a(x09), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(x08), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x18), .c(new_n34_), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n33_), .O(z0));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x08), .O(new_n40_));
  nand2  g12(.a(x14), .b(x10), .O(new_n41_));
  nand3  g13(.a(new_n41_), .b(x09), .c(new_n40_), .O(new_n42_));
  oai21  g14(.a(new_n42_), .b(x11), .c(new_n39_), .O(z1));
  nand2  g15(.a(new_n36_), .b(new_n34_), .O(new_n44_));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n44_), .c(new_n45_), .O(z2));
  nand2  g19(.a(x08), .b(x02), .O(new_n48_));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  nand3  g22(.a(new_n36_), .b(new_n50_), .c(new_n34_), .O(new_n51_));
  oai21  g23(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(z3));
  nand2  g24(.a(x08), .b(x03), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  and2   g26(.a(new_n50_), .b(new_n54_), .O(new_n55_));
  nand3  g27(.a(new_n41_), .b(new_n36_), .c(new_n29_), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(z4));
  nand2  g29(.a(x08), .b(x04), .O(new_n58_));
  nand4  g30(.a(x15), .b(x13), .c(x12), .d(x11), .O(new_n59_));
  aoi21  g31(.a(new_n59_), .b(new_n34_), .c(new_n54_), .O(new_n60_));
  oai21  g32(.a(new_n30_), .b(x15), .c(new_n36_), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(z5));
  nand2  g34(.a(x08), .b(x05), .O(new_n63_));
  inv1   g35(.a(new_n42_), .O(new_n64_));
  inv1   g36(.a(x16), .O(new_n65_));
  oai21  g37(.a(new_n59_), .b(new_n54_), .c(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n30_), .b(x16), .c(x15), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n63_), .O(z6));
  nand2  g41(.a(x08), .b(x06), .O(new_n70_));
  inv1   g42(.a(x17), .O(new_n71_));
  nand2  g43(.a(x16), .b(x15), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n29_), .c(new_n71_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n64_), .c(new_n33_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n70_), .O(z7));
  nand2  g47(.a(x08), .b(x07), .O(new_n76_));
  nand3  g48(.a(new_n32_), .b(new_n30_), .c(x18), .O(new_n77_));
  inv1   g49(.a(x18), .O(new_n78_));
  oai21  g50(.a(new_n31_), .b(new_n29_), .c(new_n78_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n77_), .c(new_n64_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n76_), .O(z8));
endmodule


