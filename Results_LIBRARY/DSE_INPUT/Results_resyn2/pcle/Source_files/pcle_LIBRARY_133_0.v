// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_;
  nand2  g00(.a(x14), .b(x13), .O(new_n29_));
  nand3  g01(.a(x15), .b(x12), .c(x11), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x17), .c(x16), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(x18), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n32_), .O(z0));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  nor2   g11(.a(x18), .b(x16), .O(new_n40_));
  or2    g12(.a(new_n40_), .b(x11), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(new_n35_), .c(new_n39_), .O(z1));
  nand2  g14(.a(x08), .b(x01), .O(new_n43_));
  xnor2a g15(.a(x12), .b(x11), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n35_), .c(new_n43_), .O(z2));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  and2   g18(.a(x12), .b(x11), .O(new_n47_));
  nor2   g19(.a(new_n47_), .b(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n36_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n48_), .c(new_n46_), .O(z3));
  nand2  g23(.a(x08), .b(x03), .O(new_n52_));
  inv1   g24(.a(new_n49_), .O(new_n53_));
  nor2   g25(.a(new_n53_), .b(x14), .O(new_n54_));
  inv1   g26(.a(new_n29_), .O(new_n55_));
  nand2  g27(.a(new_n47_), .b(new_n55_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n36_), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(new_n54_), .c(new_n52_), .O(z4));
  nand2  g30(.a(x08), .b(x04), .O(new_n59_));
  inv1   g31(.a(x15), .O(new_n60_));
  nand2  g32(.a(new_n56_), .b(new_n60_), .O(new_n61_));
  inv1   g33(.a(new_n30_), .O(new_n62_));
  oai21  g34(.a(new_n40_), .b(new_n55_), .c(new_n62_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n61_), .c(new_n36_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n59_), .O(z5));
  nand2  g37(.a(x08), .b(x05), .O(new_n66_));
  nand2  g38(.a(new_n31_), .b(x16), .O(new_n67_));
  inv1   g39(.a(x16), .O(new_n68_));
  oai21  g40(.a(new_n30_), .b(new_n29_), .c(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n67_), .c(new_n36_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n66_), .O(z6));
  nand2  g43(.a(x08), .b(x06), .O(new_n72_));
  aoi21  g44(.a(new_n31_), .b(x16), .c(x17), .O(new_n73_));
  nand2  g45(.a(new_n36_), .b(new_n32_), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(z7));
  nand2  g47(.a(x08), .b(x07), .O(new_n76_));
  nand3  g48(.a(new_n62_), .b(new_n55_), .c(x17), .O(new_n77_));
  inv1   g49(.a(x17), .O(new_n78_));
  oai21  g50(.a(x18), .b(new_n78_), .c(x16), .O(new_n79_));
  aoi22  g51(.a(new_n79_), .b(new_n31_), .c(new_n77_), .d(x18), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n35_), .c(new_n76_), .O(z8));
endmodule


