// Benchmark "FAU" written by ABC on Mon Jul 27 18:24:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand3  g02(.a(x17), .b(x16), .c(x15), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nor2   g05(.a(x10), .b(x08), .O(new_n34_));
  nand2  g06(.a(new_n34_), .b(x18), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n33_), .O(z0));
  nand2  g08(.a(x08), .b(x00), .O(new_n37_));
  nand2  g09(.a(new_n34_), .b(x09), .O(new_n38_));
  oai21  g10(.a(new_n38_), .b(x11), .c(new_n37_), .O(z1));
  nand2  g11(.a(x08), .b(x01), .O(new_n40_));
  xnor2a g12(.a(x12), .b(x11), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(new_n38_), .c(new_n40_), .O(z2));
  nand2  g14(.a(x08), .b(x02), .O(new_n43_));
  and2   g15(.a(x12), .b(x11), .O(new_n44_));
  nor2   g16(.a(new_n44_), .b(x13), .O(new_n45_));
  inv1   g17(.a(new_n38_), .O(new_n46_));
  nand2  g18(.a(new_n44_), .b(x13), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n45_), .c(new_n43_), .O(z3));
  nand2  g21(.a(x08), .b(x03), .O(new_n50_));
  aoi21  g22(.a(new_n44_), .b(x13), .c(x14), .O(new_n51_));
  nand2  g23(.a(new_n46_), .b(new_n29_), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(z4));
  nand2  g25(.a(new_n29_), .b(x15), .O(new_n54_));
  inv1   g26(.a(x15), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(x09), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(new_n29_), .c(new_n54_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n34_), .O(new_n58_));
  nand2  g30(.a(x08), .b(x04), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n58_), .O(z5));
  nand2  g32(.a(x08), .b(x05), .O(new_n61_));
  aoi21  g33(.a(new_n30_), .b(x15), .c(x16), .O(new_n62_));
  nand3  g34(.a(new_n56_), .b(new_n54_), .c(x16), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n34_), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(z6));
  inv1   g37(.a(x17), .O(new_n66_));
  nand2  g38(.a(x16), .b(x15), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(x09), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n54_), .c(new_n66_), .O(new_n69_));
  nand2  g41(.a(new_n66_), .b(x16), .O(new_n70_));
  nor3   g42(.a(new_n70_), .b(new_n29_), .c(new_n55_), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n69_), .c(new_n34_), .O(new_n72_));
  nand2  g44(.a(x08), .b(x06), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(z7));
  nand2  g46(.a(x08), .b(x07), .O(new_n75_));
  nand2  g47(.a(new_n31_), .b(x09), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n54_), .c(x18), .O(new_n77_));
  inv1   g49(.a(x18), .O(new_n78_));
  nand2  g50(.a(new_n33_), .b(new_n78_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n77_), .c(new_n34_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n75_), .O(z8));
endmodule


