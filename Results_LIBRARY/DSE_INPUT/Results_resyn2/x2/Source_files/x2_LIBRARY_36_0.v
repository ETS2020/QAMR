// Benchmark "FAU" written by ABC on Mon Jul 27 18:02:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n25_, new_n27_, new_n28_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  xnor2a g03(.a(x9), .b(x7), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(z1));
  nor3   g05(.a(x9), .b(x8), .c(x7), .O(z2));
  nand2  g06(.a(x9), .b(x8), .O(new_n25_));
  nand3  g07(.a(new_n25_), .b(x7), .c(x6), .O(z4));
  inv1   g08(.a(x4), .O(new_n27_));
  nand3  g09(.a(x7), .b(new_n27_), .c(x3), .O(new_n28_));
  nand2  g10(.a(new_n28_), .b(x8), .O(new_n29_));
  nand2  g11(.a(new_n29_), .b(new_n18_), .O(new_n30_));
  inv1   g12(.a(x6), .O(new_n31_));
  inv1   g13(.a(x7), .O(new_n32_));
  nand3  g14(.a(x9), .b(x8), .c(x5), .O(new_n33_));
  inv1   g15(.a(new_n33_), .O(new_n34_));
  aoi21  g16(.a(new_n34_), .b(new_n32_), .c(new_n31_), .O(new_n35_));
  nand2  g17(.a(new_n35_), .b(new_n30_), .O(z5));
  nand2  g18(.a(x4), .b(x3), .O(new_n37_));
  nand2  g19(.a(new_n37_), .b(new_n18_), .O(new_n38_));
  nand2  g20(.a(new_n18_), .b(new_n20_), .O(new_n39_));
  nand4  g21(.a(new_n39_), .b(new_n38_), .c(new_n25_), .d(x7), .O(new_n40_));
  nand2  g22(.a(new_n39_), .b(new_n33_), .O(new_n41_));
  nand2  g23(.a(new_n41_), .b(new_n32_), .O(new_n42_));
  nand3  g24(.a(new_n42_), .b(new_n40_), .c(x6), .O(z6));
  one    g25(.O(z3));
endmodule


