// Benchmark "FAU" written by ABC on Mon Jul 27 20:48:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  nand4  g00(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n21_), .O(new_n24_));
  nand2  g05(.a(new_n22_), .b(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(new_n24_), .c(new_n20_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n27_), .O(z0));
  nand2  g13(.a(x13), .b(x09), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n21_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n33_), .c(new_n22_), .O(new_n36_));
  aoi21  g17(.a(new_n23_), .b(new_n21_), .c(new_n34_), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n36_), .c(x03), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  aoi21  g20(.a(new_n29_), .b(new_n39_), .c(new_n30_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n38_), .O(z1));
  nand2  g22(.a(new_n35_), .b(new_n33_), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n23_), .c(x11), .O(new_n43_));
  nand2  g24(.a(new_n23_), .b(x11), .O(new_n44_));
  nand3  g25(.a(new_n33_), .b(new_n34_), .c(new_n21_), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n44_), .c(x03), .O(new_n46_));
  inv1   g27(.a(x06), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n29_), .c(new_n30_), .O(new_n48_));
  oai21  g29(.a(new_n46_), .b(new_n43_), .c(new_n48_), .O(z2));
  inv1   g30(.a(x12), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(x04), .c(new_n20_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(x03), .O(new_n52_));
  inv1   g33(.a(x07), .O(new_n53_));
  inv1   g34(.a(x11), .O(new_n54_));
  nand4  g35(.a(new_n54_), .b(new_n34_), .c(new_n21_), .d(x02), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x12), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x03), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  inv1   g39(.a(new_n55_), .O(new_n59_));
  inv1   g40(.a(x04), .O(new_n60_));
  nor2   g41(.a(x12), .b(new_n60_), .O(new_n61_));
  aoi21  g42(.a(new_n61_), .b(new_n59_), .c(new_n30_), .O(new_n62_));
  nand3  g43(.a(new_n62_), .b(new_n58_), .c(new_n52_), .O(z3));
  nand3  g44(.a(x13), .b(x09), .c(x04), .O(new_n64_));
  inv1   g45(.a(new_n64_), .O(z4));
endmodule


