// Benchmark "FAU" written by ABC on Mon Jul 27 20:48:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_;
  nand2  g00(.a(x13), .b(x09), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x08), .O(new_n21_));
  nand4  g02(.a(new_n21_), .b(x04), .c(x03), .d(x02), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  inv1   g05(.a(x00), .O(new_n25_));
  inv1   g06(.a(x03), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand4  g08(.a(new_n27_), .b(new_n24_), .c(new_n22_), .d(x05), .O(z0));
  nand3  g09(.a(new_n20_), .b(x04), .c(x02), .O(new_n29_));
  nor2   g10(.a(new_n23_), .b(x10), .O(new_n30_));
  aoi22  g11(.a(new_n30_), .b(new_n21_), .c(new_n29_), .d(x10), .O(new_n31_));
  oai21  g12(.a(x03), .b(x01), .c(x05), .O(new_n32_));
  aoi21  g13(.a(x10), .b(x08), .c(new_n32_), .O(new_n33_));
  oai21  g14(.a(new_n31_), .b(new_n26_), .c(new_n33_), .O(z1));
  inv1   g15(.a(x08), .O(new_n35_));
  inv1   g16(.a(x10), .O(new_n36_));
  inv1   g17(.a(x11), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(new_n20_), .c(new_n23_), .O(new_n39_));
  nor2   g20(.a(new_n30_), .b(new_n37_), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n39_), .c(x03), .O(new_n41_));
  oai21  g22(.a(x06), .b(x03), .c(x05), .O(new_n42_));
  aoi21  g23(.a(x11), .b(x08), .c(new_n42_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n41_), .O(z2));
  inv1   g25(.a(x12), .O(new_n45_));
  nand4  g26(.a(new_n45_), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n20_), .c(new_n23_), .O(new_n47_));
  aoi21  g28(.a(new_n30_), .b(new_n37_), .c(new_n45_), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n47_), .c(x03), .O(new_n49_));
  oai21  g30(.a(x07), .b(x03), .c(x05), .O(new_n50_));
  aoi21  g31(.a(x12), .b(x08), .c(new_n50_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n49_), .O(z3));
  nand3  g33(.a(x13), .b(x09), .c(x04), .O(new_n53_));
  inv1   g34(.a(new_n53_), .O(z4));
endmodule


