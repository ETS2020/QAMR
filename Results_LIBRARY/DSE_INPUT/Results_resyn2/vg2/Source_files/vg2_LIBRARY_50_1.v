// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x19), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand2  g003(.a(new_n36_), .b(x13), .O(new_n37_));
  oai22  g004(.a(new_n37_), .b(new_n35_), .c(new_n36_), .d(new_n34_), .O(new_n38_));
  nor2   g005(.a(x10), .b(x02), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(x00), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x19), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  aoi21  g011(.a(new_n38_), .b(x05), .c(new_n44_), .O(new_n45_));
  inv1   g012(.a(x06), .O(new_n46_));
  inv1   g013(.a(x20), .O(new_n47_));
  nor2   g014(.a(x14), .b(x11), .O(new_n48_));
  nor2   g015(.a(x03), .b(x01), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n45_), .O(z0));
  nand4  g018(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n52_));
  nand3  g019(.a(x11), .b(x09), .c(x04), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand4  g021(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n55_));
  nand4  g022(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n56_));
  nor2   g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g024(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  nor2   g025(.a(x17), .b(x16), .O(new_n59_));
  nor2   g026(.a(x23), .b(x22), .O(new_n60_));
  nor2   g027(.a(x20), .b(x12), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n60_), .c(new_n59_), .d(new_n48_), .O(new_n62_));
  inv1   g029(.a(x04), .O(new_n63_));
  nor2   g030(.a(new_n34_), .b(x06), .O(new_n64_));
  nor2   g031(.a(x09), .b(x08), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n49_), .d(new_n63_), .O(new_n66_));
  oai21  g033(.a(new_n66_), .b(new_n62_), .c(new_n58_), .O(new_n67_));
  nor2   g034(.a(new_n41_), .b(new_n36_), .O(new_n68_));
  nand2  g035(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g036(.a(x00), .O(new_n70_));
  nor2   g037(.a(x06), .b(new_n70_), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n65_), .c(new_n49_), .d(new_n63_), .O(new_n72_));
  nand4  g039(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n73_));
  nor2   g040(.a(new_n73_), .b(new_n55_), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n54_), .O(new_n75_));
  oai21  g042(.a(new_n72_), .b(new_n62_), .c(new_n75_), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n39_), .O(new_n77_));
  nand3  g044(.a(new_n36_), .b(x13), .c(x05), .O(new_n78_));
  oai21  g045(.a(x13), .b(x05), .c(new_n78_), .O(new_n79_));
  nand4  g046(.a(x23), .b(x22), .c(x20), .d(x15), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(new_n55_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n54_), .O(new_n82_));
  nor2   g049(.a(x08), .b(x06), .O(new_n83_));
  nor2   g050(.a(x20), .b(new_n35_), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n83_), .c(new_n49_), .d(new_n48_), .O(new_n85_));
  nor3   g052(.a(x12), .b(x09), .c(x04), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n60_), .c(new_n59_), .O(new_n87_));
  oai21  g054(.a(new_n87_), .b(new_n85_), .c(new_n82_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n79_), .O(new_n89_));
  nand3  g056(.a(new_n89_), .b(new_n77_), .c(new_n69_), .O(z1));
  inv1   g057(.a(x15), .O(new_n91_));
  nand2  g058(.a(x24), .b(x18), .O(new_n92_));
  oai21  g059(.a(new_n37_), .b(new_n91_), .c(new_n92_), .O(new_n93_));
  nand3  g060(.a(x24), .b(x18), .c(x13), .O(new_n94_));
  nand2  g061(.a(new_n41_), .b(x15), .O(new_n95_));
  nand2  g062(.a(new_n39_), .b(x21), .O(new_n96_));
  nand3  g063(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  aoi21  g064(.a(new_n93_), .b(x05), .c(new_n97_), .O(new_n98_));
  inv1   g065(.a(x11), .O(new_n99_));
  inv1   g066(.a(x14), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nor2   g068(.a(new_n47_), .b(new_n46_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n101_), .c(x03), .d(x01), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n98_), .O(z2));
  nand2  g071(.a(new_n64_), .b(new_n49_), .O(new_n105_));
  inv1   g072(.a(x08), .O(new_n106_));
  nand3  g073(.a(new_n48_), .b(new_n47_), .c(new_n106_), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g075(.a(x20), .b(x18), .O(new_n109_));
  inv1   g076(.a(new_n52_), .O(new_n110_));
  nand2  g077(.a(new_n101_), .b(new_n110_), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n108_), .c(new_n68_), .O(new_n113_));
  nand2  g080(.a(x21), .b(x20), .O(new_n114_));
  nor2   g081(.a(new_n111_), .b(new_n114_), .O(new_n115_));
  nand2  g082(.a(new_n71_), .b(new_n49_), .O(new_n116_));
  nor2   g083(.a(new_n116_), .b(new_n107_), .O(new_n117_));
  oai21  g084(.a(new_n117_), .b(new_n115_), .c(new_n39_), .O(new_n118_));
  nand2  g085(.a(x20), .b(x15), .O(new_n119_));
  oai21  g086(.a(new_n111_), .b(new_n119_), .c(new_n85_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n79_), .O(new_n121_));
  nand3  g088(.a(new_n121_), .b(new_n118_), .c(new_n113_), .O(z3));
  inv1   g089(.a(x16), .O(new_n123_));
  inv1   g090(.a(x09), .O(new_n124_));
  inv1   g091(.a(x23), .O(new_n125_));
  aoi21  g092(.a(new_n125_), .b(x04), .c(x17), .O(new_n126_));
  oai21  g093(.a(new_n126_), .b(x22), .c(new_n124_), .O(new_n127_));
  aoi21  g094(.a(new_n127_), .b(new_n123_), .c(x08), .O(new_n128_));
  inv1   g095(.a(x22), .O(new_n129_));
  inv1   g096(.a(x17), .O(new_n130_));
  aoi21  g097(.a(x23), .b(new_n63_), .c(new_n130_), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(new_n129_), .c(x09), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(x16), .c(new_n106_), .O(new_n133_));
  oai22  g100(.a(new_n133_), .b(new_n98_), .c(new_n128_), .d(new_n45_), .O(z4));
  inv1   g101(.a(new_n45_), .O(z5));
  oai21  g102(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(new_n99_), .c(x03), .O(new_n137_));
  inv1   g104(.a(x03), .O(new_n138_));
  oai21  g105(.a(x20), .b(new_n100_), .c(x06), .O(new_n139_));
  aoi21  g106(.a(new_n139_), .b(x11), .c(new_n138_), .O(new_n140_));
  oai22  g107(.a(new_n140_), .b(new_n98_), .c(new_n137_), .d(new_n45_), .O(z6));
  inv1   g108(.a(new_n98_), .O(z7));
endmodule


