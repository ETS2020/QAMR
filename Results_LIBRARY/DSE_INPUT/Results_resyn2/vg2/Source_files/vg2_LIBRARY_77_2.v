// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand4  g007(.a(new_n36_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  inv1   g010(.a(new_n35_), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(x13), .O(new_n45_));
  nand3  g012(.a(new_n45_), .b(new_n43_), .c(new_n41_), .O(new_n46_));
  nor2   g013(.a(new_n46_), .b(new_n38_), .O(new_n47_));
  nor2   g014(.a(x03), .b(x01), .O(new_n48_));
  nor2   g015(.a(x14), .b(x11), .O(new_n49_));
  nor2   g016(.a(x20), .b(x06), .O(new_n50_));
  nand3  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(new_n47_), .O(z0));
  inv1   g019(.a(new_n42_), .O(new_n53_));
  nand4  g020(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  nand3  g022(.a(x11), .b(x09), .c(x04), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  nand4  g024(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  nand4  g026(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n60_));
  inv1   g027(.a(new_n60_), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n59_), .c(new_n57_), .d(new_n55_), .O(new_n62_));
  nor2   g029(.a(x11), .b(x09), .O(new_n63_));
  nor2   g030(.a(x17), .b(x16), .O(new_n64_));
  nor2   g031(.a(x14), .b(x12), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n48_), .O(new_n66_));
  nor2   g033(.a(x22), .b(x04), .O(new_n67_));
  nor2   g034(.a(x23), .b(x08), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n50_), .d(new_n44_), .O(new_n69_));
  oai21  g036(.a(new_n69_), .b(new_n66_), .c(new_n62_), .O(new_n70_));
  nand2  g037(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  nand4  g038(.a(x23), .b(x22), .c(x20), .d(x15), .O(new_n72_));
  inv1   g039(.a(new_n72_), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n59_), .c(new_n57_), .d(new_n55_), .O(new_n74_));
  inv1   g041(.a(x23), .O(new_n75_));
  inv1   g042(.a(x19), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(x08), .O(new_n77_));
  nand4  g044(.a(new_n77_), .b(new_n67_), .c(new_n50_), .d(new_n75_), .O(new_n78_));
  oai21  g045(.a(new_n78_), .b(new_n66_), .c(new_n74_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n42_), .O(new_n80_));
  nand2  g047(.a(new_n67_), .b(new_n50_), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand3  g049(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n83_));
  nand3  g050(.a(x19), .b(x13), .c(x05), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g052(.a(new_n64_), .b(new_n63_), .O(new_n86_));
  inv1   g053(.a(x08), .O(new_n87_));
  inv1   g054(.a(x14), .O(new_n88_));
  nand4  g055(.a(new_n36_), .b(new_n75_), .c(new_n88_), .d(new_n87_), .O(new_n89_));
  nor2   g056(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n85_), .c(new_n82_), .d(new_n48_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n80_), .c(new_n71_), .O(z1));
  nand2  g059(.a(x24), .b(x18), .O(new_n93_));
  nand3  g060(.a(new_n36_), .b(x15), .c(x13), .O(new_n94_));
  aoi21  g061(.a(new_n94_), .b(new_n93_), .c(new_n34_), .O(new_n95_));
  nand4  g062(.a(new_n36_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n96_));
  nand3  g063(.a(x24), .b(x18), .c(x13), .O(new_n97_));
  nand2  g064(.a(new_n42_), .b(x15), .O(new_n98_));
  nand3  g065(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n95_), .O(new_n100_));
  nand3  g067(.a(x20), .b(x14), .c(x11), .O(new_n101_));
  inv1   g068(.a(new_n101_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(x06), .c(x03), .d(x01), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n100_), .O(z2));
  inv1   g071(.a(x18), .O(new_n105_));
  nand2  g072(.a(new_n102_), .b(new_n55_), .O(new_n106_));
  nor2   g073(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g074(.a(x07), .O(new_n108_));
  nand4  g075(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n87_), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nor2   g077(.a(new_n42_), .b(new_n36_), .O(new_n111_));
  oai21  g078(.a(new_n110_), .b(new_n107_), .c(new_n111_), .O(new_n112_));
  inv1   g079(.a(x15), .O(new_n113_));
  nor2   g080(.a(new_n106_), .b(new_n113_), .O(new_n114_));
  nor2   g081(.a(new_n109_), .b(new_n76_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n114_), .c(new_n42_), .O(new_n116_));
  nand3  g083(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n117_));
  nand3  g084(.a(x15), .b(x13), .c(x05), .O(new_n118_));
  aoi21  g085(.a(new_n118_), .b(new_n117_), .c(new_n106_), .O(new_n119_));
  aoi21  g086(.a(new_n84_), .b(new_n83_), .c(new_n109_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n119_), .c(new_n36_), .O(new_n121_));
  nand3  g088(.a(new_n121_), .b(new_n116_), .c(new_n112_), .O(z3));
  inv1   g089(.a(x16), .O(new_n123_));
  inv1   g090(.a(x09), .O(new_n124_));
  aoi21  g091(.a(new_n75_), .b(x04), .c(x17), .O(new_n125_));
  oai21  g092(.a(new_n125_), .b(x22), .c(new_n124_), .O(new_n126_));
  aoi21  g093(.a(new_n126_), .b(new_n123_), .c(x08), .O(new_n127_));
  inv1   g094(.a(x22), .O(new_n128_));
  inv1   g095(.a(x04), .O(new_n129_));
  inv1   g096(.a(x17), .O(new_n130_));
  aoi21  g097(.a(x23), .b(new_n129_), .c(new_n130_), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(new_n128_), .c(x09), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(x16), .c(new_n87_), .O(new_n133_));
  oai22  g100(.a(new_n133_), .b(new_n100_), .c(new_n127_), .d(new_n47_), .O(z4));
  inv1   g101(.a(new_n47_), .O(z5));
  inv1   g102(.a(x11), .O(new_n136_));
  inv1   g103(.a(x06), .O(new_n137_));
  inv1   g104(.a(x20), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(x14), .c(new_n137_), .O(new_n139_));
  aoi21  g106(.a(new_n139_), .b(new_n136_), .c(x03), .O(new_n140_));
  inv1   g107(.a(x03), .O(new_n141_));
  oai21  g108(.a(x20), .b(new_n88_), .c(x06), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(x11), .c(new_n141_), .O(new_n143_));
  oai22  g110(.a(new_n143_), .b(new_n100_), .c(new_n140_), .d(new_n47_), .O(z6));
  inv1   g111(.a(new_n100_), .O(z7));
endmodule


