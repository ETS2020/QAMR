// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_;
  inv1   g000(.a(x05), .O(new_n34_));
  inv1   g001(.a(x13), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  aoi21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x07), .O(new_n38_));
  nand2  g005(.a(x19), .b(new_n35_), .O(new_n39_));
  nor2   g006(.a(x10), .b(x02), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(new_n36_), .c(x00), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z5));
  inv1   g009(.a(z5), .O(new_n43_));
  inv1   g010(.a(x06), .O(new_n44_));
  inv1   g011(.a(x14), .O(new_n45_));
  inv1   g012(.a(x20), .O(new_n46_));
  nor3   g013(.a(x11), .b(x03), .c(x01), .O(new_n47_));
  nand4  g014(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n43_), .O(z0));
  inv1   g016(.a(x01), .O(new_n50_));
  nand2  g017(.a(new_n50_), .b(x00), .O(new_n51_));
  inv1   g018(.a(x04), .O(new_n52_));
  nand3  g019(.a(new_n46_), .b(new_n44_), .c(new_n52_), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  inv1   g021(.a(x08), .O(new_n55_));
  inv1   g022(.a(x09), .O(new_n56_));
  inv1   g023(.a(x12), .O(new_n57_));
  nand4  g024(.a(new_n45_), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(new_n58_));
  inv1   g025(.a(new_n58_), .O(new_n59_));
  inv1   g026(.a(x02), .O(new_n60_));
  inv1   g027(.a(x03), .O(new_n61_));
  inv1   g028(.a(x10), .O(new_n62_));
  inv1   g029(.a(x11), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n64_));
  inv1   g031(.a(x16), .O(new_n65_));
  inv1   g032(.a(x17), .O(new_n66_));
  inv1   g033(.a(x22), .O(new_n67_));
  inv1   g034(.a(x23), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(new_n59_), .c(new_n54_), .O(new_n71_));
  nand2  g038(.a(new_n40_), .b(x21), .O(new_n72_));
  nand3  g039(.a(x15), .b(x13), .c(x05), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g041(.a(x16), .b(x14), .c(x12), .d(x04), .O(new_n75_));
  nand4  g042(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g044(.a(x11), .b(x09), .O(new_n78_));
  nand4  g045(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n36_), .O(new_n83_));
  inv1   g050(.a(new_n69_), .O(new_n84_));
  nor3   g051(.a(x04), .b(x03), .c(x01), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g053(.a(new_n46_), .b(new_n63_), .c(x07), .d(new_n44_), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n59_), .O(new_n89_));
  nor2   g056(.a(new_n46_), .b(new_n66_), .O(new_n90_));
  inv1   g057(.a(new_n78_), .O(new_n91_));
  inv1   g058(.a(new_n79_), .O(new_n92_));
  nand3  g059(.a(x23), .b(x22), .c(x18), .O(new_n93_));
  nor2   g060(.a(new_n93_), .b(new_n75_), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n92_), .c(new_n91_), .d(new_n90_), .O(new_n95_));
  oai21  g062(.a(new_n89_), .b(new_n86_), .c(new_n95_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n37_), .O(new_n97_));
  nand4  g064(.a(new_n46_), .b(x19), .c(new_n55_), .d(new_n44_), .O(new_n98_));
  inv1   g065(.a(new_n98_), .O(new_n99_));
  nand4  g066(.a(new_n45_), .b(new_n57_), .c(new_n63_), .d(new_n56_), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  inv1   g069(.a(x15), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(x05), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n92_), .c(new_n91_), .d(new_n77_), .O(new_n105_));
  oai21  g072(.a(new_n102_), .b(new_n86_), .c(new_n105_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n35_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n97_), .c(new_n83_), .O(z1));
  nand2  g075(.a(x15), .b(x13), .O(new_n109_));
  nand2  g076(.a(x24), .b(x18), .O(new_n110_));
  oai21  g077(.a(new_n109_), .b(x24), .c(new_n110_), .O(new_n111_));
  nand3  g078(.a(new_n40_), .b(new_n36_), .c(x21), .O(new_n112_));
  nand3  g079(.a(x24), .b(x18), .c(x13), .O(new_n113_));
  nand3  g080(.a(x15), .b(new_n35_), .c(new_n34_), .O(new_n114_));
  nand3  g081(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(new_n115_));
  aoi21  g082(.a(new_n111_), .b(x05), .c(new_n115_), .O(new_n116_));
  nand3  g083(.a(x20), .b(x14), .c(x11), .O(new_n117_));
  nand3  g084(.a(x06), .b(x03), .c(x01), .O(new_n118_));
  nor3   g085(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(z2));
  nor2   g086(.a(new_n117_), .b(new_n79_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(x18), .O(new_n121_));
  nor3   g088(.a(x20), .b(x14), .c(x08), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(new_n47_), .c(x07), .d(new_n44_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n37_), .O(new_n125_));
  nand2  g092(.a(new_n120_), .b(new_n74_), .O(new_n126_));
  inv1   g093(.a(new_n51_), .O(new_n127_));
  inv1   g094(.a(new_n64_), .O(new_n128_));
  nand4  g095(.a(new_n122_), .b(new_n128_), .c(new_n127_), .d(new_n44_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n36_), .O(new_n131_));
  nand2  g098(.a(new_n120_), .b(new_n104_), .O(new_n132_));
  nand3  g099(.a(new_n99_), .b(new_n47_), .c(new_n45_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n35_), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n131_), .c(new_n125_), .O(z3));
  aoi21  g103(.a(new_n68_), .b(x04), .c(x17), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(x22), .c(new_n56_), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(new_n65_), .c(x08), .O(new_n139_));
  aoi21  g106(.a(x23), .b(new_n52_), .c(new_n66_), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(new_n67_), .c(x09), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(x16), .c(new_n55_), .O(new_n142_));
  oai22  g109(.a(new_n142_), .b(new_n116_), .c(new_n139_), .d(new_n43_), .O(z4));
  aoi21  g110(.a(x20), .b(new_n45_), .c(x06), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(x11), .c(new_n61_), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(z5), .O(new_n146_));
  oai21  g113(.a(x20), .b(new_n45_), .c(x06), .O(new_n147_));
  aoi21  g114(.a(new_n147_), .b(x11), .c(new_n61_), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(new_n116_), .c(new_n146_), .O(z6));
  inv1   g116(.a(new_n116_), .O(z7));
endmodule


