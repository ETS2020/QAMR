// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_;
  nand2  g000(.a(x24), .b(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  nand2  g003(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nor2   g004(.a(x10), .b(x02), .O(new_n38_));
  nand3  g005(.a(new_n38_), .b(new_n35_), .c(x00), .O(new_n39_));
  nor2   g006(.a(x13), .b(x05), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(x19), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(new_n41_), .c(new_n39_), .O(new_n43_));
  aoi21  g010(.a(new_n37_), .b(x05), .c(new_n43_), .O(new_n44_));
  inv1   g011(.a(x01), .O(new_n45_));
  inv1   g012(.a(x03), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g014(.a(x06), .O(new_n48_));
  inv1   g015(.a(x20), .O(new_n49_));
  nor2   g016(.a(x14), .b(x11), .O(new_n50_));
  nand3  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nor3   g018(.a(new_n51_), .b(new_n47_), .c(new_n44_), .O(z0));
  inv1   g019(.a(x17), .O(new_n53_));
  inv1   g020(.a(x22), .O(new_n54_));
  inv1   g021(.a(x23), .O(new_n55_));
  nand3  g022(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  inv1   g023(.a(x04), .O(new_n57_));
  inv1   g024(.a(x16), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n57_), .c(new_n46_), .d(new_n45_), .O(new_n59_));
  nor2   g026(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  inv1   g027(.a(x12), .O(new_n61_));
  nand3  g028(.a(new_n50_), .b(new_n49_), .c(new_n61_), .O(new_n62_));
  inv1   g029(.a(new_n62_), .O(new_n63_));
  inv1   g030(.a(x08), .O(new_n64_));
  inv1   g031(.a(x09), .O(new_n65_));
  nand3  g032(.a(new_n65_), .b(new_n64_), .c(new_n48_), .O(new_n66_));
  inv1   g033(.a(x02), .O(new_n67_));
  inv1   g034(.a(x10), .O(new_n68_));
  nand3  g035(.a(new_n68_), .b(new_n67_), .c(x00), .O(new_n69_));
  nand3  g036(.a(x19), .b(x13), .c(x05), .O(new_n70_));
  aoi21  g037(.a(new_n70_), .b(new_n69_), .c(new_n66_), .O(new_n71_));
  nand3  g038(.a(new_n71_), .b(new_n63_), .c(new_n60_), .O(new_n72_));
  nand2  g039(.a(new_n38_), .b(x21), .O(new_n73_));
  nand3  g040(.a(x15), .b(x13), .c(x05), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g042(.a(x08), .b(x06), .O(new_n76_));
  nand4  g043(.a(x09), .b(x04), .c(x03), .d(x01), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand4  g045(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n79_));
  nand4  g046(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n78_), .c(new_n75_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n72_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n35_), .O(new_n84_));
  nor2   g051(.a(new_n40_), .b(new_n35_), .O(new_n85_));
  nor2   g052(.a(new_n79_), .b(new_n61_), .O(new_n86_));
  nand3  g053(.a(x18), .b(x16), .c(x14), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(new_n86_), .c(new_n78_), .d(x11), .O(new_n89_));
  nand4  g056(.a(new_n65_), .b(new_n64_), .c(x07), .d(new_n48_), .O(new_n90_));
  inv1   g057(.a(new_n90_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n63_), .c(new_n60_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n85_), .O(new_n94_));
  nand3  g061(.a(x16), .b(x15), .c(x14), .O(new_n95_));
  inv1   g062(.a(new_n95_), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n86_), .c(new_n78_), .d(x11), .O(new_n97_));
  nand4  g064(.a(new_n49_), .b(x19), .c(new_n64_), .d(new_n48_), .O(new_n98_));
  inv1   g065(.a(new_n98_), .O(new_n99_));
  nand3  g066(.a(new_n50_), .b(new_n61_), .c(new_n65_), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  nand3  g068(.a(new_n101_), .b(new_n99_), .c(new_n60_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n97_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n40_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n94_), .c(new_n84_), .O(z1));
  inv1   g072(.a(x18), .O(new_n106_));
  nand3  g073(.a(new_n35_), .b(x15), .c(x13), .O(new_n107_));
  oai21  g074(.a(new_n35_), .b(new_n106_), .c(new_n107_), .O(new_n108_));
  nand3  g075(.a(new_n38_), .b(new_n35_), .c(x21), .O(new_n109_));
  nand3  g076(.a(x24), .b(x18), .c(x13), .O(new_n110_));
  nand2  g077(.a(new_n40_), .b(x15), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  aoi21  g079(.a(new_n108_), .b(x05), .c(new_n112_), .O(new_n113_));
  inv1   g080(.a(x11), .O(new_n114_));
  inv1   g081(.a(x14), .O(new_n115_));
  nand3  g082(.a(x20), .b(x03), .c(x01), .O(new_n116_));
  nor3   g083(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(x06), .O(new_n118_));
  nor2   g085(.a(new_n118_), .b(new_n113_), .O(z2));
  inv1   g086(.a(new_n76_), .O(new_n120_));
  nand3  g087(.a(new_n117_), .b(new_n120_), .c(x18), .O(new_n121_));
  inv1   g088(.a(new_n51_), .O(new_n122_));
  nor2   g089(.a(new_n47_), .b(x08), .O(new_n123_));
  nand3  g090(.a(new_n123_), .b(new_n122_), .c(x07), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n85_), .O(new_n126_));
  nand3  g093(.a(new_n117_), .b(new_n120_), .c(x15), .O(new_n127_));
  nand4  g094(.a(new_n99_), .b(new_n50_), .c(new_n46_), .d(new_n45_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n40_), .O(new_n130_));
  nand3  g097(.a(new_n117_), .b(new_n120_), .c(new_n75_), .O(new_n131_));
  nand2  g098(.a(new_n70_), .b(new_n69_), .O(new_n132_));
  nand3  g099(.a(new_n123_), .b(new_n132_), .c(new_n122_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n35_), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n130_), .c(new_n126_), .O(z3));
  aoi21  g103(.a(new_n55_), .b(x04), .c(x17), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(x22), .c(new_n65_), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(new_n58_), .c(x08), .O(new_n139_));
  aoi21  g106(.a(x23), .b(new_n57_), .c(new_n53_), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(new_n54_), .c(x09), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(x16), .c(new_n64_), .O(new_n142_));
  oai22  g109(.a(new_n142_), .b(new_n113_), .c(new_n139_), .d(new_n44_), .O(z4));
  nand2  g110(.a(new_n37_), .b(x05), .O(new_n144_));
  inv1   g111(.a(new_n42_), .O(new_n145_));
  aoi21  g112(.a(new_n40_), .b(x19), .c(new_n145_), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n39_), .c(new_n144_), .O(z5));
  oai21  g114(.a(x20), .b(new_n115_), .c(x06), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(x11), .c(new_n46_), .O(new_n149_));
  nor2   g116(.a(x11), .b(new_n48_), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(x07), .O(new_n151_));
  oai21  g118(.a(new_n149_), .b(new_n106_), .c(new_n151_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n85_), .O(new_n153_));
  nand3  g120(.a(new_n35_), .b(x13), .c(x05), .O(new_n154_));
  oai21  g121(.a(x13), .b(x05), .c(new_n154_), .O(new_n155_));
  inv1   g122(.a(x15), .O(new_n156_));
  nand2  g123(.a(new_n150_), .b(x19), .O(new_n157_));
  oai21  g124(.a(new_n149_), .b(new_n156_), .c(new_n157_), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  nand2  g126(.a(new_n50_), .b(x20), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n46_), .O(new_n161_));
  nand2  g128(.a(new_n38_), .b(new_n35_), .O(new_n162_));
  inv1   g129(.a(new_n150_), .O(new_n163_));
  oai21  g130(.a(x11), .b(new_n46_), .c(x21), .O(new_n164_));
  aoi21  g131(.a(new_n164_), .b(new_n163_), .c(new_n162_), .O(new_n165_));
  aoi21  g132(.a(new_n161_), .b(z5), .c(new_n165_), .O(new_n166_));
  nand3  g133(.a(new_n166_), .b(new_n159_), .c(new_n153_), .O(z6));
  inv1   g134(.a(new_n113_), .O(z7));
endmodule


