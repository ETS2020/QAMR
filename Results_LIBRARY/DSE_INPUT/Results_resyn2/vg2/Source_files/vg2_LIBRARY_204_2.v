// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  nor2   g005(.a(x10), .b(x02), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n36_), .c(x00), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x19), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  nor2   g011(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  nor3   g012(.a(x20), .b(x14), .c(x06), .O(new_n46_));
  nor3   g013(.a(x11), .b(x03), .c(x01), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n45_), .O(z0));
  nand2  g016(.a(x13), .b(x05), .O(new_n50_));
  nand2  g017(.a(new_n39_), .b(x21), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g019(.a(x03), .b(x01), .O(new_n53_));
  nand4  g020(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand4  g022(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n56_));
  nand4  g023(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g025(.a(new_n58_), .b(new_n55_), .c(new_n52_), .O(new_n59_));
  nand2  g026(.a(new_n39_), .b(x00), .O(new_n60_));
  nand3  g027(.a(x19), .b(x13), .c(x05), .O(new_n61_));
  nand2  g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g029(.a(x09), .O(new_n63_));
  inv1   g030(.a(x11), .O(new_n64_));
  inv1   g031(.a(x12), .O(new_n65_));
  inv1   g032(.a(x16), .O(new_n66_));
  nand4  g033(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  inv1   g034(.a(x01), .O(new_n68_));
  inv1   g035(.a(x03), .O(new_n69_));
  inv1   g036(.a(x04), .O(new_n70_));
  inv1   g037(.a(x08), .O(new_n71_));
  nand4  g038(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nor2   g039(.a(new_n72_), .b(new_n67_), .O(new_n73_));
  inv1   g040(.a(x06), .O(new_n74_));
  inv1   g041(.a(x14), .O(new_n75_));
  inv1   g042(.a(x20), .O(new_n76_));
  nand3  g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(new_n77_));
  inv1   g044(.a(x17), .O(new_n78_));
  inv1   g045(.a(x22), .O(new_n79_));
  inv1   g046(.a(x23), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n77_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n73_), .c(new_n62_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n59_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n36_), .O(new_n85_));
  nor2   g052(.a(x03), .b(x01), .O(new_n86_));
  nor2   g053(.a(x11), .b(x09), .O(new_n87_));
  nor2   g054(.a(x16), .b(x12), .O(new_n88_));
  nor2   g055(.a(x08), .b(x04), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n90_));
  nor3   g057(.a(new_n81_), .b(new_n90_), .c(new_n77_), .O(new_n91_));
  oai21  g058(.a(new_n41_), .b(new_n35_), .c(new_n42_), .O(new_n92_));
  inv1   g059(.a(x13), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n34_), .O(new_n94_));
  nand2  g061(.a(x24), .b(x18), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g063(.a(new_n96_), .b(new_n58_), .c(new_n55_), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  aoi21  g065(.a(new_n92_), .b(new_n91_), .c(new_n98_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n85_), .O(z1));
  aoi21  g067(.a(new_n52_), .b(new_n36_), .c(new_n96_), .O(new_n101_));
  nand4  g068(.a(x20), .b(x14), .c(x11), .d(x06), .O(new_n102_));
  nor3   g069(.a(new_n102_), .b(new_n101_), .c(new_n53_), .O(z2));
  nor3   g070(.a(new_n102_), .b(new_n53_), .c(new_n71_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n52_), .O(new_n105_));
  nand4  g072(.a(new_n62_), .b(new_n47_), .c(new_n46_), .d(new_n71_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n36_), .O(new_n108_));
  nor2   g075(.a(new_n48_), .b(x08), .O(new_n109_));
  aoi22  g076(.a(new_n109_), .b(new_n92_), .c(new_n104_), .d(new_n96_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n108_), .O(z3));
  aoi21  g078(.a(new_n80_), .b(x04), .c(x17), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(x22), .c(new_n63_), .O(new_n113_));
  aoi21  g080(.a(new_n113_), .b(new_n66_), .c(x08), .O(new_n114_));
  aoi21  g081(.a(x23), .b(new_n70_), .c(new_n78_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n79_), .c(x09), .O(new_n116_));
  aoi21  g083(.a(new_n116_), .b(x16), .c(new_n71_), .O(new_n117_));
  nand3  g084(.a(new_n36_), .b(x15), .c(x13), .O(new_n118_));
  aoi21  g085(.a(new_n118_), .b(new_n95_), .c(new_n34_), .O(new_n119_));
  nand3  g086(.a(new_n39_), .b(new_n36_), .c(x21), .O(new_n120_));
  nand2  g087(.a(new_n41_), .b(x15), .O(new_n121_));
  nand3  g088(.a(x24), .b(x18), .c(x13), .O(new_n122_));
  nand3  g089(.a(new_n122_), .b(new_n121_), .c(new_n120_), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  oai22  g091(.a(new_n124_), .b(new_n117_), .c(new_n114_), .d(new_n45_), .O(z4));
  inv1   g092(.a(new_n45_), .O(z5));
  inv1   g093(.a(x21), .O(new_n127_));
  oai21  g094(.a(x20), .b(new_n75_), .c(x06), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(x11), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(x03), .c(new_n127_), .O(new_n130_));
  and2   g097(.a(x03), .b(x00), .O(new_n131_));
  nor3   g098(.a(x24), .b(x10), .c(x02), .O(new_n132_));
  oai21  g099(.a(new_n131_), .b(new_n130_), .c(new_n132_), .O(new_n133_));
  nand2  g100(.a(x20), .b(new_n75_), .O(new_n134_));
  aoi21  g101(.a(new_n134_), .b(new_n74_), .c(x11), .O(new_n135_));
  oai21  g102(.a(new_n44_), .b(new_n38_), .c(new_n135_), .O(new_n136_));
  aoi21  g103(.a(new_n76_), .b(x14), .c(new_n74_), .O(new_n137_));
  nand3  g104(.a(x18), .b(x15), .c(x11), .O(new_n138_));
  nor2   g105(.a(x07), .b(new_n69_), .O(new_n139_));
  oai22  g106(.a(x18), .b(x03), .c(x13), .d(x05), .O(new_n140_));
  oai22  g107(.a(new_n140_), .b(new_n139_), .c(new_n138_), .d(new_n137_), .O(new_n141_));
  nor2   g108(.a(x19), .b(new_n69_), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n129_), .O(new_n143_));
  inv1   g110(.a(x15), .O(new_n144_));
  nand3  g111(.a(new_n36_), .b(x13), .c(x05), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(new_n94_), .c(new_n144_), .O(new_n146_));
  aoi22  g113(.a(new_n146_), .b(new_n143_), .c(new_n141_), .d(x24), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n136_), .c(new_n133_), .O(z6));
  aoi21  g115(.a(new_n94_), .b(new_n69_), .c(x15), .O(new_n149_));
  inv1   g116(.a(new_n120_), .O(new_n150_));
  nor2   g117(.a(new_n146_), .b(new_n150_), .O(new_n151_));
  oai21  g118(.a(new_n149_), .b(new_n95_), .c(new_n151_), .O(z7));
endmodule


