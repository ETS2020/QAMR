// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_;
  nor2   g000(.a(x03), .b(x01), .O(new_n34_));
  nor2   g001(.a(x14), .b(x11), .O(new_n35_));
  nor2   g002(.a(x20), .b(x06), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  inv1   g009(.a(x02), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  nand4  g011(.a(new_n40_), .b(new_n44_), .c(new_n43_), .d(x00), .O(new_n45_));
  nor2   g012(.a(x13), .b(x05), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(x19), .O(new_n47_));
  nand3  g014(.a(x24), .b(x13), .c(x07), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n37_), .O(z0));
  nand3  g018(.a(new_n44_), .b(new_n43_), .c(x00), .O(new_n52_));
  nand3  g019(.a(x19), .b(x13), .c(x05), .O(new_n53_));
  oai21  g020(.a(new_n52_), .b(x12), .c(new_n53_), .O(new_n54_));
  inv1   g021(.a(x01), .O(new_n55_));
  inv1   g022(.a(x03), .O(new_n56_));
  inv1   g023(.a(x11), .O(new_n57_));
  inv1   g024(.a(x14), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(new_n59_));
  inv1   g026(.a(x16), .O(new_n60_));
  inv1   g027(.a(x17), .O(new_n61_));
  inv1   g028(.a(x22), .O(new_n62_));
  inv1   g029(.a(x23), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  inv1   g032(.a(x20), .O(new_n66_));
  nor2   g033(.a(x08), .b(x06), .O(new_n67_));
  nor2   g034(.a(x09), .b(x04), .O(new_n68_));
  nand3  g035(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(new_n65_), .c(new_n54_), .O(new_n71_));
  nand3  g038(.a(x21), .b(new_n44_), .c(new_n43_), .O(new_n72_));
  nand3  g039(.a(x15), .b(x13), .c(x05), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g041(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n75_));
  nand3  g042(.a(x11), .b(x09), .c(x04), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand4  g044(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n78_));
  nand3  g045(.a(x16), .b(x14), .c(x12), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n40_), .O(new_n83_));
  inv1   g050(.a(new_n59_), .O(new_n84_));
  inv1   g051(.a(new_n64_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g053(.a(x18), .b(x16), .c(x14), .d(x12), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(new_n78_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n77_), .O(new_n89_));
  inv1   g056(.a(x07), .O(new_n90_));
  nor2   g057(.a(x12), .b(new_n90_), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n92_));
  oai21  g059(.a(new_n92_), .b(new_n86_), .c(new_n89_), .O(new_n93_));
  nor2   g060(.a(new_n46_), .b(new_n40_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand4  g062(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(new_n78_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n77_), .O(new_n98_));
  inv1   g065(.a(x08), .O(new_n99_));
  nand4  g066(.a(new_n68_), .b(new_n36_), .c(x19), .d(new_n99_), .O(new_n100_));
  oai21  g067(.a(new_n100_), .b(new_n86_), .c(new_n98_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n46_), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n95_), .c(new_n83_), .O(z1));
  nand2  g070(.a(x24), .b(x18), .O(new_n104_));
  nand3  g071(.a(new_n40_), .b(x15), .c(x13), .O(new_n105_));
  aoi21  g072(.a(new_n105_), .b(new_n104_), .c(new_n38_), .O(new_n106_));
  nand4  g073(.a(new_n40_), .b(x21), .c(new_n44_), .d(new_n43_), .O(new_n107_));
  nand3  g074(.a(x24), .b(x18), .c(x13), .O(new_n108_));
  nand2  g075(.a(new_n46_), .b(x15), .O(new_n109_));
  nand3  g076(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(new_n106_), .O(new_n111_));
  nand3  g078(.a(x20), .b(x14), .c(x11), .O(new_n112_));
  inv1   g079(.a(new_n112_), .O(new_n113_));
  nand4  g080(.a(new_n113_), .b(x06), .c(x03), .d(x01), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n111_), .O(z2));
  inv1   g082(.a(new_n75_), .O(new_n116_));
  nand2  g083(.a(new_n113_), .b(new_n116_), .O(new_n117_));
  aoi21  g084(.a(new_n73_), .b(new_n72_), .c(new_n117_), .O(new_n118_));
  nand4  g085(.a(new_n67_), .b(new_n35_), .c(new_n34_), .d(new_n66_), .O(new_n119_));
  aoi21  g086(.a(new_n53_), .b(new_n52_), .c(new_n119_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n118_), .c(new_n40_), .O(new_n121_));
  inv1   g088(.a(x18), .O(new_n122_));
  nor2   g089(.a(new_n117_), .b(new_n122_), .O(new_n123_));
  nor2   g090(.a(new_n119_), .b(new_n90_), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(new_n123_), .c(new_n94_), .O(new_n125_));
  inv1   g092(.a(x15), .O(new_n126_));
  nor2   g093(.a(new_n117_), .b(new_n126_), .O(new_n127_));
  nand2  g094(.a(x19), .b(new_n99_), .O(new_n128_));
  nor2   g095(.a(new_n128_), .b(new_n37_), .O(new_n129_));
  oai21  g096(.a(new_n129_), .b(new_n127_), .c(new_n46_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n125_), .c(new_n121_), .O(z3));
  inv1   g098(.a(x09), .O(new_n132_));
  aoi21  g099(.a(new_n63_), .b(x04), .c(x17), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(x22), .c(new_n132_), .O(new_n134_));
  aoi21  g101(.a(new_n134_), .b(new_n60_), .c(x08), .O(new_n135_));
  inv1   g102(.a(x04), .O(new_n136_));
  aoi21  g103(.a(x23), .b(new_n136_), .c(new_n61_), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(new_n62_), .c(x09), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(x16), .c(new_n99_), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(new_n111_), .c(new_n135_), .d(new_n50_), .O(z4));
  inv1   g107(.a(new_n50_), .O(z5));
  inv1   g108(.a(x06), .O(new_n142_));
  oai21  g109(.a(new_n66_), .b(x14), .c(new_n142_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n57_), .c(x03), .O(new_n144_));
  oai21  g111(.a(x20), .b(new_n58_), .c(x06), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(x11), .c(new_n56_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n111_), .c(new_n144_), .d(new_n50_), .O(z6));
  inv1   g114(.a(new_n111_), .O(z7));
endmodule


