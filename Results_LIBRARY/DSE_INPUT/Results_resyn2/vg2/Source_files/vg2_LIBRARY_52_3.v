// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x05), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand4  g007(.a(new_n35_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g012(.a(new_n45_), .O(new_n46_));
  nor2   g013(.a(x14), .b(x11), .O(new_n47_));
  nor2   g014(.a(x03), .b(x01), .O(new_n48_));
  nor2   g015(.a(x20), .b(x06), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  aoi21  g017(.a(new_n46_), .b(new_n38_), .c(new_n50_), .O(z0));
  nand4  g018(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n52_));
  nand3  g019(.a(x11), .b(x09), .c(x04), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g021(.a(x21), .O(new_n55_));
  nand3  g022(.a(x15), .b(x13), .c(x05), .O(new_n56_));
  oai21  g023(.a(new_n55_), .b(x02), .c(new_n56_), .O(new_n57_));
  nand4  g024(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n58_));
  nand3  g025(.a(x16), .b(x14), .c(x12), .O(new_n59_));
  nor2   g026(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n57_), .c(new_n54_), .O(new_n61_));
  nand3  g028(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n62_));
  nand3  g029(.a(x19), .b(x13), .c(x05), .O(new_n63_));
  nand2  g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  inv1   g031(.a(x17), .O(new_n65_));
  inv1   g032(.a(x22), .O(new_n66_));
  inv1   g033(.a(x23), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  inv1   g035(.a(x01), .O(new_n69_));
  inv1   g036(.a(x03), .O(new_n70_));
  inv1   g037(.a(x04), .O(new_n71_));
  inv1   g038(.a(x16), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nor2   g040(.a(new_n73_), .b(new_n68_), .O(new_n74_));
  inv1   g041(.a(x11), .O(new_n75_));
  inv1   g042(.a(x12), .O(new_n76_));
  inv1   g043(.a(x14), .O(new_n77_));
  inv1   g044(.a(x20), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  inv1   g046(.a(x06), .O(new_n80_));
  inv1   g047(.a(x08), .O(new_n81_));
  inv1   g048(.a(x09), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  nand3  g051(.a(new_n84_), .b(new_n74_), .c(new_n64_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n61_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n35_), .O(new_n87_));
  nand4  g054(.a(x18), .b(x16), .c(x14), .d(x12), .O(new_n88_));
  nor2   g055(.a(new_n88_), .b(new_n58_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n54_), .O(new_n90_));
  inv1   g057(.a(new_n79_), .O(new_n91_));
  nand4  g058(.a(new_n82_), .b(new_n81_), .c(x07), .d(new_n80_), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n91_), .c(new_n74_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n90_), .O(new_n95_));
  nor2   g062(.a(new_n42_), .b(new_n35_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  inv1   g064(.a(new_n68_), .O(new_n98_));
  inv1   g065(.a(new_n73_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand4  g067(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n101_));
  nor2   g068(.a(new_n101_), .b(new_n58_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n54_), .O(new_n103_));
  nor2   g070(.a(x09), .b(x08), .O(new_n104_));
  inv1   g071(.a(x19), .O(new_n105_));
  nor2   g072(.a(new_n105_), .b(x12), .O(new_n106_));
  nand4  g073(.a(new_n106_), .b(new_n104_), .c(new_n49_), .d(new_n47_), .O(new_n107_));
  oai21  g074(.a(new_n107_), .b(new_n100_), .c(new_n103_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n42_), .O(new_n109_));
  nand3  g076(.a(new_n109_), .b(new_n97_), .c(new_n87_), .O(z1));
  inv1   g077(.a(x05), .O(new_n111_));
  nand2  g078(.a(x24), .b(x18), .O(new_n112_));
  nand3  g079(.a(new_n35_), .b(x15), .c(x13), .O(new_n113_));
  aoi21  g080(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  nand3  g081(.a(x24), .b(x18), .c(x13), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  aoi21  g083(.a(new_n42_), .b(x15), .c(new_n116_), .O(new_n117_));
  nand4  g084(.a(new_n35_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n114_), .O(new_n120_));
  nand3  g087(.a(x20), .b(x14), .c(x11), .O(new_n121_));
  inv1   g088(.a(new_n121_), .O(new_n122_));
  nand4  g089(.a(new_n122_), .b(x06), .c(x03), .d(x01), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n120_), .O(z2));
  inv1   g091(.a(new_n52_), .O(new_n125_));
  nand2  g092(.a(new_n122_), .b(new_n125_), .O(new_n126_));
  nand3  g093(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n127_));
  aoi21  g094(.a(new_n127_), .b(new_n56_), .c(new_n126_), .O(new_n128_));
  nand4  g095(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n81_), .O(new_n129_));
  aoi21  g096(.a(new_n63_), .b(new_n62_), .c(new_n129_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n128_), .c(new_n35_), .O(new_n131_));
  inv1   g098(.a(x18), .O(new_n132_));
  nor2   g099(.a(new_n126_), .b(new_n132_), .O(new_n133_));
  nor2   g100(.a(new_n129_), .b(new_n34_), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n133_), .c(new_n96_), .O(new_n135_));
  inv1   g102(.a(x15), .O(new_n136_));
  nor2   g103(.a(new_n126_), .b(new_n136_), .O(new_n137_));
  nor2   g104(.a(new_n129_), .b(new_n105_), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(new_n137_), .c(new_n42_), .O(new_n139_));
  nand3  g106(.a(new_n139_), .b(new_n135_), .c(new_n131_), .O(z3));
  nand2  g107(.a(new_n46_), .b(new_n38_), .O(z5));
  oai21  g108(.a(x23), .b(new_n71_), .c(new_n65_), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(new_n66_), .c(x09), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(x16), .c(new_n81_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(z5), .O(new_n145_));
  nand2  g112(.a(x22), .b(new_n65_), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(x09), .c(new_n72_), .O(new_n147_));
  oai22  g114(.a(new_n147_), .b(new_n81_), .c(new_n119_), .d(new_n114_), .O(new_n148_));
  nand3  g115(.a(new_n35_), .b(x21), .c(new_n39_), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n117_), .O(new_n150_));
  nand4  g117(.a(x23), .b(x22), .c(x16), .d(new_n71_), .O(new_n151_));
  inv1   g118(.a(new_n151_), .O(new_n152_));
  oai21  g119(.a(new_n150_), .b(new_n114_), .c(new_n152_), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n148_), .c(new_n145_), .O(z4));
  aoi21  g121(.a(x20), .b(new_n77_), .c(x06), .O(new_n155_));
  oai21  g122(.a(new_n155_), .b(x11), .c(new_n70_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(z5), .O(new_n157_));
  nand2  g124(.a(new_n78_), .b(x14), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(x06), .c(new_n75_), .O(new_n159_));
  oai22  g126(.a(new_n159_), .b(new_n70_), .c(new_n119_), .d(new_n114_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n157_), .O(z6));
  inv1   g128(.a(new_n120_), .O(z7));
endmodule


