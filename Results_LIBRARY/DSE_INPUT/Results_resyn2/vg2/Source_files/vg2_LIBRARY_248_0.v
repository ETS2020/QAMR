// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x11), .O(new_n34_));
  inv1   g001(.a(x14), .O(new_n35_));
  inv1   g002(.a(x20), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  inv1   g004(.a(new_n37_), .O(new_n38_));
  nor3   g005(.a(x06), .b(x03), .c(x01), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  and2   g007(.a(x24), .b(x07), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(x19), .c(x13), .O(new_n43_));
  inv1   g010(.a(new_n43_), .O(new_n44_));
  oai21  g011(.a(new_n44_), .b(new_n41_), .c(x05), .O(new_n45_));
  inv1   g012(.a(x02), .O(new_n46_));
  inv1   g013(.a(x10), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n46_), .c(x00), .O(new_n48_));
  inv1   g015(.a(new_n48_), .O(new_n49_));
  nand2  g016(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nor2   g017(.a(x13), .b(x05), .O(new_n51_));
  aoi22  g018(.a(new_n51_), .b(x19), .c(new_n41_), .d(x13), .O(new_n52_));
  nand2  g019(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  inv1   g020(.a(new_n53_), .O(new_n54_));
  aoi21  g021(.a(new_n54_), .b(new_n45_), .c(new_n40_), .O(z0));
  inv1   g022(.a(x17), .O(new_n56_));
  inv1   g023(.a(x22), .O(new_n57_));
  inv1   g024(.a(x23), .O(new_n58_));
  nand3  g025(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  inv1   g026(.a(x01), .O(new_n60_));
  inv1   g027(.a(x03), .O(new_n61_));
  inv1   g028(.a(x04), .O(new_n62_));
  inv1   g029(.a(x16), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  inv1   g032(.a(x06), .O(new_n66_));
  inv1   g033(.a(x08), .O(new_n67_));
  inv1   g034(.a(x09), .O(new_n68_));
  inv1   g035(.a(x12), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nor2   g037(.a(new_n70_), .b(new_n37_), .O(new_n71_));
  nand3  g038(.a(x19), .b(x13), .c(x05), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n48_), .O(new_n73_));
  nand3  g040(.a(new_n73_), .b(new_n71_), .c(new_n65_), .O(new_n74_));
  nand3  g041(.a(x21), .b(new_n47_), .c(new_n46_), .O(new_n75_));
  nand3  g042(.a(x15), .b(x13), .c(x05), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g044(.a(x08), .b(x06), .O(new_n78_));
  nand4  g045(.a(x09), .b(x04), .c(x03), .d(x01), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand4  g047(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n81_));
  nand4  g048(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g050(.a(new_n83_), .b(new_n80_), .c(new_n77_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n74_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n42_), .O(new_n86_));
  nand3  g053(.a(new_n71_), .b(new_n65_), .c(x07), .O(new_n87_));
  nor2   g054(.a(new_n81_), .b(new_n69_), .O(new_n88_));
  nand3  g055(.a(x18), .b(x16), .c(x14), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n88_), .c(new_n80_), .d(x11), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n87_), .O(new_n92_));
  nor2   g059(.a(new_n51_), .b(new_n42_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand3  g061(.a(x16), .b(x15), .c(x14), .O(new_n95_));
  inv1   g062(.a(new_n95_), .O(new_n96_));
  nand4  g063(.a(new_n96_), .b(new_n88_), .c(new_n80_), .d(x11), .O(new_n97_));
  nor2   g064(.a(x12), .b(x09), .O(new_n98_));
  nand3  g065(.a(x19), .b(new_n67_), .c(new_n66_), .O(new_n99_));
  nor2   g066(.a(new_n99_), .b(new_n37_), .O(new_n100_));
  nand3  g067(.a(new_n100_), .b(new_n98_), .c(new_n65_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n97_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n51_), .O(new_n103_));
  nand3  g070(.a(new_n103_), .b(new_n94_), .c(new_n86_), .O(z1));
  inv1   g071(.a(x18), .O(new_n105_));
  nor2   g072(.a(new_n42_), .b(new_n105_), .O(new_n106_));
  nand3  g073(.a(new_n42_), .b(x15), .c(x13), .O(new_n107_));
  inv1   g074(.a(new_n107_), .O(new_n108_));
  oai21  g075(.a(new_n108_), .b(new_n106_), .c(x05), .O(new_n109_));
  inv1   g076(.a(new_n75_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n42_), .O(new_n111_));
  aoi22  g078(.a(new_n106_), .b(x13), .c(new_n51_), .d(x15), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  inv1   g080(.a(new_n113_), .O(new_n114_));
  nand3  g081(.a(x20), .b(x03), .c(x01), .O(new_n115_));
  nor3   g082(.a(new_n115_), .b(new_n35_), .c(new_n34_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(x06), .O(new_n117_));
  aoi21  g084(.a(new_n114_), .b(new_n109_), .c(new_n117_), .O(z2));
  inv1   g085(.a(new_n78_), .O(new_n119_));
  nand3  g086(.a(new_n116_), .b(new_n119_), .c(x18), .O(new_n120_));
  nand4  g087(.a(new_n39_), .b(new_n38_), .c(new_n67_), .d(x07), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n93_), .O(new_n123_));
  nand3  g090(.a(new_n116_), .b(new_n119_), .c(x15), .O(new_n124_));
  nand3  g091(.a(new_n100_), .b(new_n61_), .c(new_n60_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n51_), .O(new_n127_));
  nand3  g094(.a(new_n116_), .b(new_n119_), .c(new_n77_), .O(new_n128_));
  nand4  g095(.a(new_n73_), .b(new_n39_), .c(new_n38_), .d(new_n67_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n42_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n127_), .c(new_n123_), .O(z3));
  nand3  g099(.a(new_n42_), .b(x13), .c(x05), .O(new_n133_));
  oai21  g100(.a(x13), .b(x05), .c(new_n133_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(x19), .O(new_n135_));
  nand3  g102(.a(new_n41_), .b(new_n105_), .c(new_n67_), .O(new_n136_));
  nand3  g103(.a(new_n136_), .b(new_n135_), .c(new_n50_), .O(new_n137_));
  oai21  g104(.a(x23), .b(new_n62_), .c(new_n56_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n57_), .O(new_n139_));
  aoi21  g106(.a(new_n139_), .b(new_n68_), .c(x16), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g108(.a(new_n93_), .b(x07), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n135_), .c(new_n50_), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(x08), .O(new_n144_));
  oai21  g111(.a(new_n58_), .b(x04), .c(x17), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(x22), .c(new_n68_), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(new_n63_), .c(x08), .O(new_n147_));
  nand2  g114(.a(new_n134_), .b(x15), .O(new_n148_));
  nand2  g115(.a(new_n93_), .b(x18), .O(new_n149_));
  nand3  g116(.a(new_n149_), .b(new_n148_), .c(new_n111_), .O(new_n150_));
  nand2  g117(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  nand3  g118(.a(new_n151_), .b(new_n144_), .c(new_n141_), .O(z4));
  nand2  g119(.a(new_n54_), .b(new_n45_), .O(z5));
  nand3  g120(.a(new_n41_), .b(new_n105_), .c(new_n61_), .O(new_n154_));
  nand3  g121(.a(new_n154_), .b(new_n135_), .c(new_n50_), .O(new_n155_));
  nand2  g122(.a(x20), .b(new_n35_), .O(new_n156_));
  aoi21  g123(.a(new_n156_), .b(new_n66_), .c(x11), .O(new_n157_));
  nand2  g124(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  nand2  g125(.a(new_n143_), .b(x03), .O(new_n159_));
  aoi21  g126(.a(new_n36_), .b(x14), .c(new_n66_), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(new_n34_), .c(x03), .O(new_n161_));
  nand2  g128(.a(new_n161_), .b(new_n150_), .O(new_n162_));
  nand3  g129(.a(new_n162_), .b(new_n159_), .c(new_n158_), .O(z6));
  nand2  g130(.a(new_n114_), .b(new_n109_), .O(z7));
endmodule


