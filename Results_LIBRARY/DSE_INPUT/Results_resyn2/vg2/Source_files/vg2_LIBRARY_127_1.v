// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n143_, new_n144_,
    new_n145_, new_n146_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand4  g007(.a(new_n36_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  nor2   g012(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  inv1   g013(.a(x06), .O(new_n47_));
  nor3   g014(.a(x20), .b(x14), .c(x11), .O(new_n48_));
  nor2   g015(.a(x03), .b(x01), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n46_), .O(z0));
  nand4  g018(.a(x23), .b(x22), .c(x16), .d(x14), .O(new_n52_));
  nand4  g019(.a(x08), .b(x04), .c(x03), .d(x01), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g021(.a(x15), .O(new_n55_));
  nand3  g022(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n56_));
  nand2  g023(.a(x13), .b(x05), .O(new_n57_));
  oai21  g024(.a(new_n57_), .b(new_n55_), .c(new_n56_), .O(new_n58_));
  inv1   g025(.a(x17), .O(new_n59_));
  inv1   g026(.a(x20), .O(new_n60_));
  nand3  g027(.a(x12), .b(x11), .c(x09), .O(new_n61_));
  nor3   g028(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n58_), .c(new_n54_), .O(new_n63_));
  inv1   g030(.a(x19), .O(new_n64_));
  nand3  g031(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n65_));
  oai21  g032(.a(new_n57_), .b(new_n64_), .c(new_n65_), .O(new_n66_));
  inv1   g033(.a(x09), .O(new_n67_));
  inv1   g034(.a(x14), .O(new_n68_));
  nand2  g035(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g036(.a(x04), .O(new_n70_));
  inv1   g037(.a(x16), .O(new_n71_));
  nand3  g038(.a(new_n59_), .b(new_n71_), .c(new_n70_), .O(new_n72_));
  nor2   g039(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  inv1   g040(.a(x01), .O(new_n74_));
  inv1   g041(.a(x03), .O(new_n75_));
  inv1   g042(.a(x08), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n47_), .c(new_n75_), .d(new_n74_), .O(new_n77_));
  inv1   g044(.a(x11), .O(new_n78_));
  inv1   g045(.a(x12), .O(new_n79_));
  inv1   g046(.a(x22), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n60_), .c(new_n79_), .d(new_n78_), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n77_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n73_), .c(new_n66_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n63_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n36_), .O(new_n85_));
  inv1   g052(.a(new_n81_), .O(new_n86_));
  nor3   g053(.a(x04), .b(x03), .c(x01), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g055(.a(x17), .b(x16), .O(new_n89_));
  nor2   g056(.a(x08), .b(x06), .O(new_n90_));
  inv1   g057(.a(x07), .O(new_n91_));
  nor2   g058(.a(x09), .b(new_n91_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n90_), .c(new_n89_), .d(new_n68_), .O(new_n93_));
  nor2   g060(.a(new_n60_), .b(new_n59_), .O(new_n94_));
  inv1   g061(.a(new_n61_), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n94_), .c(new_n54_), .d(x18), .O(new_n96_));
  oai21  g063(.a(new_n93_), .b(new_n88_), .c(new_n96_), .O(new_n97_));
  nor2   g064(.a(new_n42_), .b(new_n36_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g066(.a(x14), .b(x09), .O(new_n100_));
  nand4  g067(.a(new_n90_), .b(new_n89_), .c(new_n100_), .d(x19), .O(new_n101_));
  nand4  g068(.a(new_n95_), .b(new_n94_), .c(new_n54_), .d(x15), .O(new_n102_));
  oai21  g069(.a(new_n101_), .b(new_n88_), .c(new_n102_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n42_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n99_), .c(new_n85_), .O(z1));
  nand2  g072(.a(x24), .b(x18), .O(new_n106_));
  nand3  g073(.a(new_n36_), .b(x15), .c(x13), .O(new_n107_));
  aoi21  g074(.a(new_n107_), .b(new_n106_), .c(new_n34_), .O(new_n108_));
  nand4  g075(.a(new_n36_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n109_));
  nand3  g076(.a(x24), .b(x18), .c(x13), .O(new_n110_));
  nand2  g077(.a(new_n42_), .b(x15), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n108_), .O(new_n113_));
  nand2  g080(.a(x14), .b(x11), .O(new_n114_));
  nand4  g081(.a(x20), .b(x06), .c(x03), .d(x01), .O(new_n115_));
  nor2   g082(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  inv1   g083(.a(new_n116_), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n113_), .O(z2));
  inv1   g085(.a(new_n77_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(new_n66_), .c(new_n48_), .O(new_n120_));
  nand3  g087(.a(new_n116_), .b(new_n58_), .c(x08), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n36_), .O(new_n123_));
  nand3  g090(.a(new_n90_), .b(new_n49_), .c(new_n48_), .O(new_n124_));
  nand4  g091(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n125_));
  inv1   g092(.a(new_n114_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(x20), .c(x18), .O(new_n127_));
  oai22  g094(.a(new_n127_), .b(new_n125_), .c(new_n124_), .d(new_n91_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n98_), .O(new_n129_));
  nand3  g096(.a(new_n126_), .b(x20), .c(x15), .O(new_n130_));
  oai22  g097(.a(new_n130_), .b(new_n125_), .c(new_n124_), .d(new_n64_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n42_), .O(new_n132_));
  nand3  g099(.a(new_n132_), .b(new_n129_), .c(new_n123_), .O(z3));
  inv1   g100(.a(x23), .O(new_n134_));
  aoi21  g101(.a(new_n134_), .b(x04), .c(x17), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(x22), .c(new_n67_), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(new_n71_), .c(x08), .O(new_n137_));
  aoi21  g104(.a(x23), .b(new_n70_), .c(new_n59_), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(new_n80_), .c(x09), .O(new_n139_));
  aoi21  g106(.a(new_n139_), .b(x16), .c(new_n76_), .O(new_n140_));
  oai22  g107(.a(new_n140_), .b(new_n113_), .c(new_n137_), .d(new_n46_), .O(z4));
  inv1   g108(.a(new_n46_), .O(z5));
  oai21  g109(.a(new_n60_), .b(x14), .c(new_n47_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n78_), .c(x03), .O(new_n144_));
  oai21  g111(.a(x20), .b(new_n68_), .c(x06), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(x11), .c(new_n75_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n113_), .c(new_n144_), .d(new_n46_), .O(z6));
  inv1   g114(.a(new_n113_), .O(z7));
endmodule


