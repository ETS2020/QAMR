// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  inv1   g004(.a(x02), .O(new_n38_));
  inv1   g005(.a(x10), .O(new_n39_));
  nand4  g006(.a(new_n35_), .b(new_n39_), .c(new_n38_), .d(x00), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x19), .O(new_n42_));
  nand3  g009(.a(x24), .b(x13), .c(x07), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  aoi21  g011(.a(new_n37_), .b(x05), .c(new_n44_), .O(new_n45_));
  inv1   g012(.a(x06), .O(new_n46_));
  inv1   g013(.a(x20), .O(new_n47_));
  nor2   g014(.a(x14), .b(x11), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g016(.a(new_n49_), .O(new_n50_));
  nor2   g017(.a(x03), .b(x01), .O(new_n51_));
  nand3  g018(.a(new_n51_), .b(new_n50_), .c(new_n46_), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(new_n45_), .O(z0));
  nand4  g020(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n54_));
  nand3  g021(.a(x11), .b(x09), .c(x04), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  inv1   g023(.a(x15), .O(new_n57_));
  nand3  g024(.a(x21), .b(new_n39_), .c(new_n38_), .O(new_n58_));
  nand2  g025(.a(x13), .b(x05), .O(new_n59_));
  oai21  g026(.a(new_n59_), .b(new_n57_), .c(new_n58_), .O(new_n60_));
  nand2  g027(.a(x20), .b(x17), .O(new_n61_));
  nand4  g028(.a(x23), .b(x22), .c(x14), .d(x12), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g030(.a(new_n63_), .b(new_n60_), .c(new_n56_), .O(new_n64_));
  inv1   g031(.a(x19), .O(new_n65_));
  nand3  g032(.a(new_n39_), .b(new_n38_), .c(x00), .O(new_n66_));
  oai21  g033(.a(new_n59_), .b(new_n65_), .c(new_n66_), .O(new_n67_));
  inv1   g034(.a(x04), .O(new_n68_));
  nor2   g035(.a(x08), .b(x06), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n51_), .c(new_n68_), .O(new_n70_));
  inv1   g037(.a(new_n70_), .O(new_n71_));
  inv1   g038(.a(x09), .O(new_n72_));
  inv1   g039(.a(x16), .O(new_n73_));
  inv1   g040(.a(x22), .O(new_n74_));
  inv1   g041(.a(x23), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  inv1   g043(.a(x11), .O(new_n77_));
  inv1   g044(.a(x12), .O(new_n78_));
  inv1   g045(.a(x14), .O(new_n79_));
  nand4  g046(.a(new_n47_), .b(new_n79_), .c(new_n78_), .d(new_n77_), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(new_n76_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n71_), .c(new_n67_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n64_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n35_), .O(new_n84_));
  inv1   g051(.a(new_n56_), .O(new_n85_));
  inv1   g052(.a(new_n61_), .O(new_n86_));
  inv1   g053(.a(new_n62_), .O(new_n87_));
  nand3  g054(.a(new_n87_), .b(new_n86_), .c(x18), .O(new_n88_));
  nor2   g055(.a(x23), .b(x22), .O(new_n89_));
  nor2   g056(.a(x16), .b(x09), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n89_), .c(new_n51_), .d(new_n68_), .O(new_n91_));
  nor2   g058(.a(x20), .b(x12), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n69_), .c(new_n48_), .d(x07), .O(new_n93_));
  oai22  g060(.a(new_n93_), .b(new_n91_), .c(new_n88_), .d(new_n85_), .O(new_n94_));
  nor2   g061(.a(new_n41_), .b(new_n35_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor2   g063(.a(x20), .b(new_n65_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n69_), .c(new_n48_), .d(new_n78_), .O(new_n98_));
  nand3  g065(.a(new_n87_), .b(new_n86_), .c(x15), .O(new_n99_));
  oai22  g066(.a(new_n99_), .b(new_n85_), .c(new_n98_), .d(new_n91_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n41_), .O(new_n101_));
  nand3  g068(.a(new_n101_), .b(new_n96_), .c(new_n84_), .O(z1));
  inv1   g069(.a(x18), .O(new_n103_));
  nand3  g070(.a(new_n35_), .b(x15), .c(x13), .O(new_n104_));
  oai21  g071(.a(new_n35_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand4  g072(.a(new_n35_), .b(x21), .c(new_n39_), .d(new_n38_), .O(new_n106_));
  nand3  g073(.a(x24), .b(x18), .c(x13), .O(new_n107_));
  nand2  g074(.a(new_n41_), .b(x15), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n107_), .c(new_n106_), .O(new_n109_));
  aoi21  g076(.a(new_n105_), .b(x05), .c(new_n109_), .O(new_n110_));
  nand3  g077(.a(x20), .b(x14), .c(x11), .O(new_n111_));
  inv1   g078(.a(new_n111_), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(x06), .c(x03), .d(x01), .O(new_n113_));
  nor2   g080(.a(new_n113_), .b(new_n110_), .O(z2));
  inv1   g081(.a(new_n54_), .O(new_n115_));
  nand3  g082(.a(new_n112_), .b(new_n60_), .c(new_n115_), .O(new_n116_));
  nand2  g083(.a(new_n69_), .b(new_n51_), .O(new_n117_));
  inv1   g084(.a(new_n117_), .O(new_n118_));
  nand3  g085(.a(new_n118_), .b(new_n67_), .c(new_n50_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n35_), .O(new_n121_));
  nand2  g088(.a(new_n112_), .b(new_n115_), .O(new_n122_));
  nor2   g089(.a(new_n122_), .b(new_n103_), .O(new_n123_));
  nor3   g090(.a(new_n117_), .b(new_n49_), .c(new_n34_), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(new_n123_), .c(new_n95_), .O(new_n125_));
  nor2   g092(.a(new_n122_), .b(new_n57_), .O(new_n126_));
  nand2  g093(.a(new_n97_), .b(new_n69_), .O(new_n127_));
  nand2  g094(.a(new_n51_), .b(new_n48_), .O(new_n128_));
  nor2   g095(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  oai21  g096(.a(new_n129_), .b(new_n126_), .c(new_n41_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n125_), .c(new_n121_), .O(z3));
  nand2  g098(.a(new_n89_), .b(x04), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n72_), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n73_), .c(x08), .O(new_n134_));
  oai21  g101(.a(new_n74_), .b(x17), .c(x09), .O(new_n135_));
  nand4  g102(.a(x23), .b(x22), .c(x17), .d(new_n68_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(x08), .O(new_n137_));
  aoi21  g104(.a(new_n135_), .b(x16), .c(new_n137_), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n110_), .c(new_n134_), .d(new_n45_), .O(z4));
  inv1   g106(.a(new_n45_), .O(z5));
  oai21  g107(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(new_n77_), .c(x03), .O(new_n142_));
  inv1   g109(.a(x03), .O(new_n143_));
  oai21  g110(.a(x20), .b(new_n79_), .c(x06), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(x11), .c(new_n143_), .O(new_n145_));
  oai22  g112(.a(new_n145_), .b(new_n110_), .c(new_n142_), .d(new_n45_), .O(z6));
  inv1   g113(.a(new_n110_), .O(z7));
endmodule


