// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand4  g007(.a(new_n36_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  nor2   g012(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  nor3   g013(.a(x20), .b(x14), .c(x11), .O(new_n47_));
  nor3   g014(.a(x06), .b(x03), .c(x01), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n46_), .O(z0));
  inv1   g017(.a(x11), .O(new_n51_));
  inv1   g018(.a(x14), .O(new_n52_));
  inv1   g019(.a(x20), .O(new_n53_));
  nand3  g020(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  inv1   g021(.a(x06), .O(new_n55_));
  inv1   g022(.a(x08), .O(new_n56_));
  nand2  g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  nor3   g025(.a(x23), .b(x22), .c(x17), .O(new_n59_));
  nor2   g026(.a(x03), .b(x01), .O(new_n60_));
  nor2   g027(.a(x09), .b(x04), .O(new_n61_));
  nor2   g028(.a(x16), .b(x12), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  nand2  g031(.a(new_n39_), .b(x00), .O(new_n65_));
  nand3  g032(.a(x19), .b(x13), .c(x05), .O(new_n66_));
  nand2  g033(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n64_), .c(new_n59_), .d(new_n58_), .O(new_n68_));
  nand4  g035(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n69_));
  nand2  g036(.a(x09), .b(x04), .O(new_n70_));
  nor2   g037(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g038(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n72_));
  nand3  g039(.a(x15), .b(x13), .c(x05), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g041(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n75_));
  nand4  g042(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g044(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n68_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n36_), .O(new_n80_));
  nor2   g047(.a(new_n42_), .b(new_n36_), .O(new_n81_));
  nand4  g048(.a(new_n64_), .b(new_n59_), .c(new_n58_), .d(x07), .O(new_n82_));
  nor2   g049(.a(new_n75_), .b(new_n52_), .O(new_n83_));
  nand3  g050(.a(x18), .b(x16), .c(x12), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  nand4  g052(.a(new_n85_), .b(new_n83_), .c(new_n71_), .d(x11), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n82_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n81_), .O(new_n88_));
  nand3  g055(.a(x16), .b(x15), .c(x12), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n83_), .c(new_n71_), .d(x11), .O(new_n91_));
  nand4  g058(.a(new_n64_), .b(new_n59_), .c(new_n58_), .d(x19), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n42_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n88_), .c(new_n80_), .O(z1));
  nand2  g062(.a(x24), .b(x18), .O(new_n96_));
  nand3  g063(.a(new_n36_), .b(x15), .c(x13), .O(new_n97_));
  aoi21  g064(.a(new_n97_), .b(new_n96_), .c(new_n34_), .O(new_n98_));
  inv1   g065(.a(new_n98_), .O(new_n99_));
  nand4  g066(.a(new_n36_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n100_));
  nand3  g067(.a(x24), .b(x18), .c(x13), .O(new_n101_));
  nand2  g068(.a(new_n42_), .b(x15), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  inv1   g070(.a(new_n103_), .O(new_n104_));
  nand3  g071(.a(x20), .b(x14), .c(x11), .O(new_n105_));
  inv1   g072(.a(new_n105_), .O(new_n106_));
  nand4  g073(.a(new_n106_), .b(x06), .c(x03), .d(x01), .O(new_n107_));
  aoi21  g074(.a(new_n104_), .b(new_n99_), .c(new_n107_), .O(z2));
  inv1   g075(.a(x18), .O(new_n109_));
  inv1   g076(.a(new_n69_), .O(new_n110_));
  nand2  g077(.a(new_n106_), .b(new_n110_), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand3  g079(.a(new_n47_), .b(new_n56_), .c(new_n55_), .O(new_n113_));
  nand2  g080(.a(new_n60_), .b(x07), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(new_n112_), .c(new_n81_), .O(new_n116_));
  inv1   g083(.a(x15), .O(new_n117_));
  nor2   g084(.a(new_n111_), .b(new_n117_), .O(new_n118_));
  nand2  g085(.a(new_n60_), .b(x19), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n113_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n118_), .c(new_n42_), .O(new_n121_));
  nand3  g088(.a(new_n106_), .b(new_n74_), .c(new_n110_), .O(new_n122_));
  nand3  g089(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n123_));
  nand2  g090(.a(new_n66_), .b(new_n123_), .O(new_n124_));
  nand4  g091(.a(new_n124_), .b(new_n48_), .c(new_n47_), .d(new_n56_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n36_), .O(new_n127_));
  nand3  g094(.a(new_n127_), .b(new_n121_), .c(new_n116_), .O(z3));
  inv1   g095(.a(x09), .O(new_n129_));
  oai21  g096(.a(x16), .b(new_n129_), .c(new_n56_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n124_), .O(new_n131_));
  aoi21  g098(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n132_));
  inv1   g099(.a(x17), .O(new_n133_));
  inv1   g100(.a(x23), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(x04), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n133_), .c(x22), .O(new_n136_));
  nand3  g103(.a(new_n135_), .b(new_n66_), .c(x10), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n136_), .c(new_n132_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n131_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n36_), .O(new_n140_));
  inv1   g107(.a(x16), .O(new_n141_));
  oai21  g108(.a(new_n134_), .b(x04), .c(x17), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(x22), .c(new_n129_), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(new_n141_), .c(x08), .O(new_n144_));
  nand2  g111(.a(new_n74_), .b(new_n36_), .O(new_n145_));
  or2    g112(.a(new_n96_), .b(new_n42_), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n145_), .c(new_n102_), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  inv1   g115(.a(x22), .O(new_n149_));
  inv1   g116(.a(x04), .O(new_n150_));
  oai21  g117(.a(x23), .b(new_n150_), .c(new_n133_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  aoi21  g119(.a(new_n152_), .b(new_n129_), .c(x16), .O(new_n153_));
  oai21  g120(.a(new_n42_), .b(new_n35_), .c(new_n43_), .O(new_n154_));
  oai21  g121(.a(new_n153_), .b(x08), .c(new_n154_), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n148_), .c(new_n140_), .O(z4));
  inv1   g123(.a(new_n46_), .O(z5));
  nand2  g124(.a(x20), .b(new_n52_), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(new_n55_), .c(x11), .O(new_n159_));
  nor2   g126(.a(new_n159_), .b(x03), .O(new_n160_));
  inv1   g127(.a(x03), .O(new_n161_));
  nand2  g128(.a(new_n53_), .b(x14), .O(new_n162_));
  aoi21  g129(.a(new_n162_), .b(x06), .c(new_n51_), .O(new_n163_));
  oai22  g130(.a(new_n163_), .b(new_n161_), .c(new_n103_), .d(new_n98_), .O(new_n164_));
  oai21  g131(.a(new_n160_), .b(new_n46_), .c(new_n164_), .O(z6));
  nand2  g132(.a(new_n104_), .b(new_n99_), .O(z7));
endmodule


