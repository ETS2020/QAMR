// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nor2   g004(.a(x10), .b(x02), .O(new_n38_));
  nand3  g005(.a(new_n38_), .b(new_n35_), .c(x00), .O(new_n39_));
  nand3  g006(.a(x24), .b(x13), .c(x07), .O(new_n40_));
  nor2   g007(.a(x13), .b(x05), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(x19), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  aoi21  g010(.a(new_n37_), .b(x05), .c(new_n43_), .O(new_n44_));
  nor2   g011(.a(x14), .b(x11), .O(new_n45_));
  nor2   g012(.a(x03), .b(x01), .O(new_n46_));
  nor2   g013(.a(x20), .b(x06), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  nor2   g015(.a(new_n48_), .b(new_n44_), .O(z0));
  nand3  g016(.a(x20), .b(x11), .c(x09), .O(new_n50_));
  nand4  g017(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g019(.a(new_n38_), .b(x21), .c(x03), .O(new_n53_));
  nand3  g020(.a(x15), .b(x13), .c(x05), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand4  g022(.a(x08), .b(x06), .c(x04), .d(x01), .O(new_n56_));
  nand2  g023(.a(x23), .b(x22), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g025(.a(new_n58_), .b(new_n55_), .c(new_n52_), .O(new_n59_));
  nand2  g026(.a(new_n38_), .b(x00), .O(new_n60_));
  nand3  g027(.a(x19), .b(x13), .c(x05), .O(new_n61_));
  nand2  g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g029(.a(x17), .O(new_n63_));
  inv1   g030(.a(x22), .O(new_n64_));
  inv1   g031(.a(x23), .O(new_n65_));
  nand3  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  inv1   g033(.a(x01), .O(new_n67_));
  inv1   g034(.a(x03), .O(new_n68_));
  inv1   g035(.a(x04), .O(new_n69_));
  inv1   g036(.a(x16), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nor2   g038(.a(new_n71_), .b(new_n66_), .O(new_n72_));
  inv1   g039(.a(x11), .O(new_n73_));
  inv1   g040(.a(x12), .O(new_n74_));
  inv1   g041(.a(x14), .O(new_n75_));
  inv1   g042(.a(x20), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  inv1   g044(.a(x06), .O(new_n78_));
  inv1   g045(.a(x08), .O(new_n79_));
  inv1   g046(.a(x09), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n77_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n72_), .c(new_n62_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n59_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n35_), .O(new_n85_));
  nor2   g052(.a(new_n41_), .b(new_n35_), .O(new_n86_));
  inv1   g053(.a(new_n66_), .O(new_n87_));
  inv1   g054(.a(new_n71_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g056(.a(x20), .b(x12), .O(new_n90_));
  nor2   g057(.a(x08), .b(x06), .O(new_n91_));
  nor2   g058(.a(x09), .b(new_n34_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(new_n45_), .O(new_n93_));
  inv1   g060(.a(new_n56_), .O(new_n94_));
  inv1   g061(.a(new_n57_), .O(new_n95_));
  inv1   g062(.a(x18), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(new_n68_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n95_), .c(new_n94_), .d(new_n52_), .O(new_n98_));
  oai21  g065(.a(new_n93_), .b(new_n89_), .c(new_n98_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n86_), .O(new_n100_));
  inv1   g067(.a(x19), .O(new_n101_));
  nor2   g068(.a(x20), .b(new_n101_), .O(new_n102_));
  nor2   g069(.a(x12), .b(x09), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n102_), .c(new_n91_), .d(new_n45_), .O(new_n104_));
  nand4  g071(.a(new_n95_), .b(new_n94_), .c(new_n52_), .d(x15), .O(new_n105_));
  oai21  g072(.a(new_n104_), .b(new_n89_), .c(new_n105_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n41_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n100_), .c(new_n85_), .O(z1));
  nand2  g075(.a(x06), .b(x01), .O(new_n109_));
  nand3  g076(.a(x20), .b(x14), .c(x11), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  inv1   g078(.a(new_n111_), .O(new_n112_));
  inv1   g079(.a(x05), .O(new_n113_));
  nand2  g080(.a(x24), .b(x18), .O(new_n114_));
  nand3  g081(.a(new_n35_), .b(x15), .c(x13), .O(new_n115_));
  aoi21  g082(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  nand2  g083(.a(new_n115_), .b(new_n68_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g085(.a(new_n41_), .b(x15), .O(new_n119_));
  inv1   g086(.a(new_n119_), .O(new_n120_));
  nand3  g087(.a(x24), .b(x18), .c(x13), .O(new_n121_));
  nand3  g088(.a(new_n38_), .b(new_n35_), .c(x21), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  aoi21  g090(.a(new_n123_), .b(x03), .c(new_n120_), .O(new_n124_));
  aoi21  g091(.a(new_n124_), .b(new_n118_), .c(new_n112_), .O(z2));
  nand3  g092(.a(new_n111_), .b(new_n55_), .c(x08), .O(new_n126_));
  inv1   g093(.a(new_n48_), .O(new_n127_));
  nand3  g094(.a(new_n62_), .b(new_n127_), .c(new_n79_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n35_), .O(new_n130_));
  nand2  g097(.a(new_n97_), .b(x08), .O(new_n131_));
  nor2   g098(.a(new_n131_), .b(new_n112_), .O(new_n132_));
  nor3   g099(.a(new_n48_), .b(x08), .c(new_n34_), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n132_), .c(new_n86_), .O(new_n134_));
  nand2  g101(.a(x15), .b(x08), .O(new_n135_));
  nor2   g102(.a(new_n135_), .b(new_n112_), .O(new_n136_));
  nand2  g103(.a(new_n46_), .b(new_n45_), .O(new_n137_));
  nand2  g104(.a(new_n102_), .b(new_n91_), .O(new_n138_));
  nor2   g105(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(new_n136_), .c(new_n41_), .O(new_n140_));
  nand3  g107(.a(new_n140_), .b(new_n134_), .c(new_n130_), .O(z3));
  aoi21  g108(.a(new_n65_), .b(x04), .c(x17), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(x22), .c(new_n80_), .O(new_n143_));
  aoi21  g110(.a(new_n143_), .b(new_n70_), .c(x08), .O(new_n144_));
  aoi21  g111(.a(x23), .b(new_n69_), .c(new_n63_), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(new_n64_), .c(x09), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(x16), .c(new_n79_), .O(new_n147_));
  nand3  g114(.a(new_n122_), .b(new_n121_), .c(new_n119_), .O(new_n148_));
  nor2   g115(.a(new_n148_), .b(new_n116_), .O(new_n149_));
  oai22  g116(.a(new_n149_), .b(new_n147_), .c(new_n144_), .d(new_n44_), .O(z4));
  nand2  g117(.a(new_n37_), .b(x05), .O(new_n151_));
  inv1   g118(.a(new_n43_), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n151_), .O(z5));
  aoi21  g120(.a(x20), .b(new_n75_), .c(x06), .O(new_n154_));
  oai21  g121(.a(new_n154_), .b(x11), .c(new_n68_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(z5), .O(new_n156_));
  nand2  g123(.a(new_n76_), .b(x14), .O(new_n157_));
  aoi21  g124(.a(new_n157_), .b(x06), .c(new_n73_), .O(new_n158_));
  oai21  g125(.a(new_n148_), .b(new_n116_), .c(new_n158_), .O(new_n159_));
  oai21  g126(.a(new_n114_), .b(new_n41_), .c(new_n122_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n68_), .O(new_n161_));
  nand3  g128(.a(new_n161_), .b(new_n159_), .c(new_n156_), .O(z6));
  inv1   g129(.a(new_n149_), .O(z7));
endmodule


