// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nor2   g002(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g003(.a(new_n35_), .b(x19), .c(x13), .O(new_n37_));
  inv1   g004(.a(new_n37_), .O(new_n38_));
  oai21  g005(.a(new_n38_), .b(new_n36_), .c(x05), .O(new_n39_));
  nor2   g006(.a(x10), .b(x02), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(x00), .O(new_n41_));
  inv1   g008(.a(new_n41_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  nand2  g010(.a(new_n36_), .b(x13), .O(new_n44_));
  nor2   g011(.a(x13), .b(x05), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(x19), .O(new_n46_));
  nand4  g013(.a(new_n46_), .b(new_n44_), .c(new_n43_), .d(new_n39_), .O(z5));
  inv1   g014(.a(x11), .O(new_n48_));
  inv1   g015(.a(x06), .O(new_n49_));
  inv1   g016(.a(x14), .O(new_n50_));
  inv1   g017(.a(x20), .O(new_n51_));
  nand3  g018(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nor3   g019(.a(new_n52_), .b(x03), .c(x01), .O(new_n53_));
  nand2  g020(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(new_n55_));
  and2   g022(.a(new_n55_), .b(z5), .O(z0));
  inv1   g023(.a(x17), .O(new_n57_));
  inv1   g024(.a(x22), .O(new_n58_));
  inv1   g025(.a(x23), .O(new_n59_));
  nand3  g026(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  inv1   g027(.a(x04), .O(new_n61_));
  inv1   g028(.a(x08), .O(new_n62_));
  nand2  g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g030(.a(x09), .O(new_n64_));
  nor2   g031(.a(x16), .b(x12), .O(new_n65_));
  nand3  g032(.a(new_n65_), .b(new_n48_), .c(new_n64_), .O(new_n66_));
  nor3   g033(.a(new_n66_), .b(new_n63_), .c(new_n60_), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n53_), .c(x19), .O(new_n68_));
  nand3  g035(.a(x06), .b(x03), .c(x01), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nand2  g037(.a(new_n70_), .b(x08), .O(new_n71_));
  nand2  g038(.a(x09), .b(x04), .O(new_n72_));
  nor2   g039(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand4  g040(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n74_));
  nand4  g041(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n75_));
  nor2   g042(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g043(.a(new_n76_), .b(new_n73_), .c(x15), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n68_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n45_), .O(new_n79_));
  inv1   g046(.a(new_n45_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n36_), .O(new_n81_));
  inv1   g048(.a(new_n81_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n67_), .c(new_n53_), .O(new_n83_));
  inv1   g050(.a(x19), .O(new_n84_));
  nand2  g051(.a(x13), .b(x05), .O(new_n85_));
  oai21  g052(.a(new_n85_), .b(new_n84_), .c(new_n41_), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n67_), .c(new_n53_), .O(new_n87_));
  inv1   g054(.a(x15), .O(new_n88_));
  nand2  g055(.a(new_n40_), .b(x21), .O(new_n89_));
  oai21  g056(.a(new_n85_), .b(new_n88_), .c(new_n89_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(new_n76_), .c(new_n73_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n87_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n35_), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n83_), .c(new_n79_), .O(z1));
  nand2  g061(.a(x24), .b(x18), .O(new_n95_));
  nand3  g062(.a(new_n35_), .b(x15), .c(x13), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(x05), .O(new_n98_));
  nand3  g065(.a(new_n40_), .b(new_n35_), .c(x21), .O(new_n99_));
  nand2  g066(.a(new_n45_), .b(x15), .O(new_n100_));
  nand3  g067(.a(x24), .b(x18), .c(x13), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n100_), .c(new_n99_), .d(new_n98_), .O(z7));
  nand3  g069(.a(x20), .b(x14), .c(x11), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n69_), .O(new_n104_));
  and2   g071(.a(new_n104_), .b(z7), .O(z2));
  inv1   g072(.a(new_n86_), .O(new_n106_));
  nor2   g073(.a(new_n103_), .b(new_n71_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n90_), .O(new_n108_));
  nand3  g075(.a(new_n53_), .b(new_n48_), .c(new_n62_), .O(new_n109_));
  oai21  g076(.a(new_n109_), .b(new_n106_), .c(new_n108_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n35_), .O(new_n111_));
  nand2  g078(.a(new_n107_), .b(x18), .O(new_n112_));
  oai21  g079(.a(new_n109_), .b(new_n34_), .c(new_n112_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n80_), .c(x24), .O(new_n114_));
  nand2  g081(.a(new_n107_), .b(x15), .O(new_n115_));
  oai21  g082(.a(new_n109_), .b(new_n84_), .c(new_n115_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n45_), .O(new_n117_));
  nand3  g084(.a(new_n117_), .b(new_n114_), .c(new_n111_), .O(z3));
  inv1   g085(.a(x21), .O(new_n119_));
  aoi21  g086(.a(x23), .b(new_n61_), .c(new_n57_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n58_), .c(x09), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(x16), .O(new_n122_));
  aoi21  g089(.a(new_n122_), .b(x08), .c(new_n119_), .O(new_n123_));
  inv1   g090(.a(x00), .O(new_n124_));
  inv1   g091(.a(x16), .O(new_n125_));
  aoi21  g092(.a(new_n59_), .b(x04), .c(x17), .O(new_n126_));
  oai21  g093(.a(new_n126_), .b(x22), .c(new_n64_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  aoi21  g095(.a(new_n128_), .b(new_n62_), .c(new_n124_), .O(new_n129_));
  nor3   g096(.a(x24), .b(x10), .c(x02), .O(new_n130_));
  oai21  g097(.a(new_n129_), .b(new_n123_), .c(new_n130_), .O(new_n131_));
  nand2  g098(.a(new_n122_), .b(x08), .O(new_n132_));
  inv1   g099(.a(new_n85_), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n35_), .c(new_n45_), .O(new_n134_));
  nor2   g101(.a(new_n134_), .b(new_n88_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  nand2  g103(.a(new_n128_), .b(new_n62_), .O(new_n137_));
  oai21  g104(.a(new_n134_), .b(new_n84_), .c(new_n81_), .O(new_n138_));
  aoi21  g105(.a(x16), .b(new_n64_), .c(new_n62_), .O(new_n139_));
  nand3  g106(.a(new_n80_), .b(x24), .c(x18), .O(new_n140_));
  nor2   g107(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  aoi21  g108(.a(new_n138_), .b(new_n137_), .c(new_n141_), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n136_), .c(new_n131_), .O(z4));
  nand2  g110(.a(x20), .b(new_n50_), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(new_n49_), .c(x11), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(x03), .c(z5), .O(new_n146_));
  aoi21  g113(.a(new_n51_), .b(x14), .c(new_n49_), .O(new_n147_));
  oai21  g114(.a(new_n147_), .b(new_n48_), .c(x03), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(z7), .O(new_n149_));
  nand2  g116(.a(new_n149_), .b(new_n146_), .O(z6));
endmodule


