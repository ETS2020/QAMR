// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n167_, new_n168_, new_n169_;
  nand3  g000(.a(x24), .b(x13), .c(x07), .O(new_n34_));
  inv1   g001(.a(x02), .O(new_n35_));
  inv1   g002(.a(x10), .O(new_n36_));
  inv1   g003(.a(x24), .O(new_n37_));
  nand4  g004(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(x00), .O(new_n38_));
  nor2   g005(.a(x13), .b(x05), .O(new_n39_));
  nand2  g006(.a(new_n39_), .b(x19), .O(new_n40_));
  nand2  g007(.a(x24), .b(x07), .O(new_n41_));
  nand3  g008(.a(new_n37_), .b(x19), .c(x13), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x05), .O(new_n44_));
  nand4  g011(.a(new_n44_), .b(new_n40_), .c(new_n38_), .d(new_n34_), .O(z5));
  inv1   g012(.a(x06), .O(new_n46_));
  inv1   g013(.a(x20), .O(new_n47_));
  nor2   g014(.a(x14), .b(x11), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g016(.a(x01), .O(new_n50_));
  inv1   g017(.a(x03), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor2   g019(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  and2   g020(.a(new_n53_), .b(z5), .O(z0));
  nand3  g021(.a(new_n36_), .b(new_n35_), .c(x00), .O(new_n55_));
  nand3  g022(.a(x19), .b(x13), .c(x05), .O(new_n56_));
  oai21  g023(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  inv1   g024(.a(x11), .O(new_n58_));
  inv1   g025(.a(x12), .O(new_n59_));
  inv1   g026(.a(x14), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  inv1   g028(.a(x22), .O(new_n62_));
  inv1   g029(.a(x23), .O(new_n63_));
  nand3  g030(.a(new_n63_), .b(new_n62_), .c(new_n47_), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  inv1   g032(.a(x04), .O(new_n66_));
  nand3  g033(.a(new_n66_), .b(new_n51_), .c(new_n50_), .O(new_n67_));
  inv1   g034(.a(x08), .O(new_n68_));
  inv1   g035(.a(x09), .O(new_n69_));
  inv1   g036(.a(x17), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n46_), .O(new_n71_));
  nor2   g038(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand3  g039(.a(new_n72_), .b(new_n65_), .c(new_n57_), .O(new_n73_));
  inv1   g040(.a(x15), .O(new_n74_));
  nand2  g041(.a(x13), .b(x05), .O(new_n75_));
  nand3  g042(.a(x21), .b(new_n36_), .c(new_n35_), .O(new_n76_));
  oai21  g043(.a(new_n75_), .b(new_n74_), .c(new_n76_), .O(new_n77_));
  nand2  g044(.a(x08), .b(x06), .O(new_n78_));
  nand4  g045(.a(x09), .b(x04), .c(x03), .d(x01), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand4  g047(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n81_));
  nand4  g048(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n82_));
  nor2   g049(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g050(.a(new_n83_), .b(new_n80_), .c(new_n77_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n73_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n37_), .O(new_n86_));
  nor2   g053(.a(x23), .b(x22), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n48_), .c(new_n47_), .d(new_n59_), .O(new_n88_));
  nand2  g055(.a(new_n80_), .b(x11), .O(new_n89_));
  inv1   g056(.a(new_n81_), .O(new_n90_));
  inv1   g057(.a(x16), .O(new_n91_));
  nor2   g058(.a(new_n91_), .b(new_n60_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(new_n90_), .c(x18), .d(x12), .O(new_n93_));
  nor3   g060(.a(x04), .b(x03), .c(x01), .O(new_n94_));
  nor2   g061(.a(x09), .b(x08), .O(new_n95_));
  inv1   g062(.a(x07), .O(new_n96_));
  nor2   g063(.a(new_n96_), .b(x06), .O(new_n97_));
  nor2   g064(.a(x17), .b(x16), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n97_), .c(new_n95_), .d(new_n94_), .O(new_n99_));
  oai22  g066(.a(new_n99_), .b(new_n88_), .c(new_n93_), .d(new_n89_), .O(new_n100_));
  nor2   g067(.a(new_n39_), .b(new_n37_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand4  g069(.a(new_n92_), .b(new_n90_), .c(x15), .d(x12), .O(new_n103_));
  nand4  g070(.a(new_n94_), .b(new_n87_), .c(new_n70_), .d(new_n69_), .O(new_n104_));
  nor2   g071(.a(x08), .b(x06), .O(new_n105_));
  inv1   g072(.a(x19), .O(new_n106_));
  nor2   g073(.a(x20), .b(new_n106_), .O(new_n107_));
  nand4  g074(.a(new_n107_), .b(new_n105_), .c(new_n48_), .d(new_n59_), .O(new_n108_));
  oai22  g075(.a(new_n108_), .b(new_n104_), .c(new_n103_), .d(new_n89_), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n39_), .O(new_n110_));
  nand3  g077(.a(new_n110_), .b(new_n102_), .c(new_n86_), .O(z1));
  nand2  g078(.a(x24), .b(x18), .O(new_n112_));
  nand2  g079(.a(x15), .b(x13), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(x24), .c(new_n112_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(x05), .O(new_n115_));
  nand4  g082(.a(new_n37_), .b(x21), .c(new_n36_), .d(new_n35_), .O(new_n116_));
  nand3  g083(.a(x24), .b(x18), .c(x13), .O(new_n117_));
  nand2  g084(.a(new_n39_), .b(x15), .O(new_n118_));
  nand3  g085(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(new_n119_));
  inv1   g086(.a(new_n119_), .O(new_n120_));
  nand3  g087(.a(x20), .b(x03), .c(x01), .O(new_n121_));
  nor3   g088(.a(new_n121_), .b(new_n60_), .c(new_n58_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(x06), .O(new_n123_));
  aoi21  g090(.a(new_n120_), .b(new_n115_), .c(new_n123_), .O(z2));
  inv1   g091(.a(new_n78_), .O(new_n125_));
  nand3  g092(.a(new_n122_), .b(new_n125_), .c(x18), .O(new_n126_));
  nor2   g093(.a(new_n52_), .b(x08), .O(new_n127_));
  nand4  g094(.a(new_n127_), .b(new_n97_), .c(new_n48_), .d(new_n47_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n101_), .O(new_n130_));
  nand3  g097(.a(new_n122_), .b(new_n125_), .c(x15), .O(new_n131_));
  inv1   g098(.a(new_n52_), .O(new_n132_));
  nand4  g099(.a(new_n107_), .b(new_n105_), .c(new_n132_), .d(new_n48_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n39_), .O(new_n135_));
  nand3  g102(.a(new_n122_), .b(new_n125_), .c(new_n77_), .O(new_n136_));
  inv1   g103(.a(new_n49_), .O(new_n137_));
  nand2  g104(.a(new_n56_), .b(new_n55_), .O(new_n138_));
  nand3  g105(.a(new_n138_), .b(new_n127_), .c(new_n137_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n37_), .O(new_n141_));
  nand3  g108(.a(new_n141_), .b(new_n135_), .c(new_n130_), .O(z3));
  aoi21  g109(.a(new_n63_), .b(x04), .c(x17), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(x22), .c(new_n69_), .O(new_n144_));
  aoi21  g111(.a(new_n38_), .b(new_n34_), .c(x16), .O(new_n145_));
  nand2  g112(.a(new_n42_), .b(x16), .O(new_n146_));
  nand3  g113(.a(new_n146_), .b(new_n43_), .c(x05), .O(new_n147_));
  nand2  g114(.a(new_n147_), .b(new_n40_), .O(new_n148_));
  oai21  g115(.a(new_n148_), .b(new_n145_), .c(new_n144_), .O(new_n149_));
  oai21  g116(.a(new_n63_), .b(x04), .c(x17), .O(new_n150_));
  aoi21  g117(.a(new_n150_), .b(x22), .c(new_n69_), .O(new_n151_));
  oai21  g118(.a(new_n151_), .b(new_n91_), .c(x08), .O(new_n152_));
  nor2   g119(.a(new_n75_), .b(x24), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(new_n39_), .c(x15), .O(new_n154_));
  nand2  g121(.a(new_n101_), .b(x18), .O(new_n155_));
  nand3  g122(.a(new_n155_), .b(new_n154_), .c(new_n116_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n152_), .O(new_n157_));
  aoi21  g124(.a(new_n56_), .b(new_n55_), .c(x24), .O(new_n158_));
  nand2  g125(.a(new_n101_), .b(x07), .O(new_n159_));
  nand2  g126(.a(new_n159_), .b(new_n40_), .O(new_n160_));
  oai21  g127(.a(new_n160_), .b(new_n158_), .c(x08), .O(new_n161_));
  nand3  g128(.a(new_n161_), .b(new_n157_), .c(new_n149_), .O(z4));
  aoi21  g129(.a(x20), .b(new_n60_), .c(x06), .O(new_n163_));
  oai21  g130(.a(new_n163_), .b(x11), .c(new_n51_), .O(new_n164_));
  nand2  g131(.a(new_n164_), .b(z5), .O(new_n165_));
  nand2  g132(.a(new_n120_), .b(new_n115_), .O(z7));
  aoi21  g133(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n167_));
  oai21  g134(.a(new_n167_), .b(new_n58_), .c(x03), .O(new_n168_));
  nand2  g135(.a(new_n168_), .b(z7), .O(new_n169_));
  nand2  g136(.a(new_n169_), .b(new_n165_), .O(z6));
endmodule


