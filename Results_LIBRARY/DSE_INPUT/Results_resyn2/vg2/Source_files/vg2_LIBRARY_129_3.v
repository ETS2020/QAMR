// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_;
  inv1   g000(.a(x05), .O(new_n34_));
  inv1   g001(.a(x13), .O(new_n35_));
  nand2  g002(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g003(.a(x13), .b(x05), .O(new_n37_));
  oai21  g004(.a(new_n37_), .b(x24), .c(new_n36_), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x19), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  inv1   g007(.a(x02), .O(new_n41_));
  inv1   g008(.a(x10), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(new_n41_), .c(x00), .O(new_n43_));
  inv1   g010(.a(new_n43_), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(new_n40_), .O(new_n45_));
  inv1   g012(.a(x06), .O(new_n46_));
  nor3   g013(.a(x20), .b(x14), .c(x11), .O(new_n47_));
  nor2   g014(.a(x03), .b(x01), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  aoi21  g016(.a(new_n45_), .b(new_n39_), .c(new_n49_), .O(z0));
  inv1   g017(.a(x15), .O(new_n51_));
  nand3  g018(.a(x21), .b(new_n42_), .c(new_n41_), .O(new_n52_));
  oai21  g019(.a(new_n37_), .b(new_n51_), .c(new_n52_), .O(new_n53_));
  nand4  g020(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n54_));
  nand2  g021(.a(x09), .b(x04), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand4  g023(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n57_));
  nand4  g024(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g026(.a(new_n59_), .b(new_n56_), .c(new_n53_), .O(new_n60_));
  inv1   g027(.a(x12), .O(new_n61_));
  inv1   g028(.a(x16), .O(new_n62_));
  nand2  g029(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g030(.a(x17), .O(new_n64_));
  inv1   g031(.a(x22), .O(new_n65_));
  inv1   g032(.a(x23), .O(new_n66_));
  nand3  g033(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  nor2   g034(.a(new_n67_), .b(new_n63_), .O(new_n68_));
  nor2   g035(.a(x08), .b(x06), .O(new_n69_));
  nor2   g036(.a(x09), .b(x04), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(new_n69_), .c(new_n48_), .O(new_n71_));
  inv1   g038(.a(new_n71_), .O(new_n72_));
  nand3  g039(.a(x19), .b(x13), .c(x05), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n43_), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n72_), .c(new_n68_), .d(new_n47_), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n60_), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n40_), .O(new_n77_));
  nor2   g044(.a(x13), .b(x05), .O(new_n78_));
  nor3   g045(.a(x23), .b(x22), .c(x17), .O(new_n79_));
  nand4  g046(.a(new_n79_), .b(new_n47_), .c(new_n62_), .d(new_n61_), .O(new_n80_));
  nand3  g047(.a(new_n59_), .b(new_n56_), .c(x15), .O(new_n81_));
  nand4  g048(.a(new_n70_), .b(new_n69_), .c(new_n48_), .d(x19), .O(new_n82_));
  oai21  g049(.a(new_n82_), .b(new_n80_), .c(new_n81_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n78_), .O(new_n84_));
  nand2  g051(.a(x24), .b(x18), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(new_n78_), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n59_), .c(new_n56_), .O(new_n87_));
  nand3  g054(.a(new_n87_), .b(new_n84_), .c(new_n77_), .O(z1));
  nand3  g055(.a(x06), .b(x03), .c(x01), .O(new_n89_));
  nand3  g056(.a(new_n40_), .b(x15), .c(x13), .O(new_n90_));
  aoi21  g057(.a(new_n90_), .b(new_n85_), .c(new_n34_), .O(new_n91_));
  nand4  g058(.a(new_n40_), .b(x21), .c(new_n42_), .d(new_n41_), .O(new_n92_));
  nand3  g059(.a(x24), .b(x18), .c(x13), .O(new_n93_));
  nand2  g060(.a(new_n78_), .b(x15), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand3  g063(.a(x20), .b(x14), .c(x11), .O(new_n97_));
  nor3   g064(.a(new_n97_), .b(new_n96_), .c(new_n89_), .O(z2));
  nor3   g065(.a(new_n97_), .b(new_n54_), .c(new_n51_), .O(new_n99_));
  inv1   g066(.a(x19), .O(new_n100_));
  nand3  g067(.a(new_n69_), .b(new_n48_), .c(new_n47_), .O(new_n101_));
  nor2   g068(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  oai21  g069(.a(new_n102_), .b(new_n99_), .c(new_n78_), .O(new_n103_));
  nor2   g070(.a(new_n97_), .b(new_n54_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n86_), .O(new_n105_));
  nand2  g072(.a(new_n104_), .b(new_n53_), .O(new_n106_));
  nand4  g073(.a(new_n74_), .b(new_n69_), .c(new_n48_), .d(new_n47_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n40_), .O(new_n109_));
  nand3  g076(.a(new_n109_), .b(new_n105_), .c(new_n103_), .O(z3));
  nand2  g077(.a(new_n45_), .b(new_n39_), .O(z5));
  inv1   g078(.a(x08), .O(new_n112_));
  inv1   g079(.a(x04), .O(new_n113_));
  oai21  g080(.a(x23), .b(new_n113_), .c(new_n64_), .O(new_n114_));
  aoi21  g081(.a(new_n114_), .b(new_n65_), .c(x09), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(x16), .c(new_n112_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(z5), .O(new_n117_));
  inv1   g084(.a(x09), .O(new_n118_));
  oai21  g085(.a(new_n66_), .b(x04), .c(x17), .O(new_n119_));
  aoi21  g086(.a(new_n119_), .b(x22), .c(new_n118_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n62_), .c(x08), .O(new_n121_));
  nand2  g088(.a(new_n38_), .b(x15), .O(new_n122_));
  inv1   g089(.a(new_n92_), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n86_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n117_), .O(z4));
  inv1   g094(.a(x03), .O(new_n128_));
  inv1   g095(.a(x14), .O(new_n129_));
  aoi21  g096(.a(x20), .b(new_n129_), .c(x06), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(x11), .c(new_n128_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(z5), .O(new_n132_));
  inv1   g099(.a(x11), .O(new_n133_));
  inv1   g100(.a(x20), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(x14), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(x06), .c(new_n133_), .O(new_n136_));
  oai22  g103(.a(new_n136_), .b(new_n128_), .c(new_n95_), .d(new_n91_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n132_), .O(z6));
  inv1   g105(.a(new_n96_), .O(z7));
endmodule


