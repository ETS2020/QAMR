// Benchmark "FAU" written by ABC on Mon Jul 27 18:40:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_;
  nor2   g000(.a(x03), .b(x01), .O(new_n34_));
  nor2   g001(.a(x11), .b(x06), .O(new_n35_));
  nor2   g002(.a(x20), .b(x14), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  inv1   g004(.a(x05), .O(new_n38_));
  nand2  g005(.a(x24), .b(x07), .O(new_n39_));
  inv1   g006(.a(x24), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(x19), .c(x13), .O(new_n41_));
  aoi21  g008(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  inv1   g009(.a(x02), .O(new_n43_));
  inv1   g010(.a(x10), .O(new_n44_));
  nand4  g011(.a(new_n40_), .b(new_n44_), .c(new_n43_), .d(x00), .O(new_n45_));
  nor2   g012(.a(x13), .b(x05), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(x19), .O(new_n47_));
  nand3  g014(.a(x24), .b(x13), .c(x07), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n37_), .O(z0));
  nand4  g018(.a(x23), .b(x22), .c(x16), .d(x14), .O(new_n52_));
  nand4  g019(.a(x08), .b(x04), .c(x03), .d(x01), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g021(.a(x15), .O(new_n55_));
  nand3  g022(.a(x21), .b(new_n44_), .c(new_n43_), .O(new_n56_));
  nand2  g023(.a(x13), .b(x05), .O(new_n57_));
  oai21  g024(.a(new_n57_), .b(new_n55_), .c(new_n56_), .O(new_n58_));
  inv1   g025(.a(x17), .O(new_n59_));
  inv1   g026(.a(x20), .O(new_n60_));
  nand3  g027(.a(x12), .b(x11), .c(x09), .O(new_n61_));
  nor3   g028(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand3  g029(.a(new_n62_), .b(new_n58_), .c(new_n54_), .O(new_n63_));
  inv1   g030(.a(x19), .O(new_n64_));
  nand3  g031(.a(new_n44_), .b(new_n43_), .c(x00), .O(new_n65_));
  oai21  g032(.a(new_n57_), .b(new_n64_), .c(new_n65_), .O(new_n66_));
  nor2   g033(.a(x09), .b(x08), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n35_), .c(new_n34_), .O(new_n68_));
  inv1   g035(.a(new_n68_), .O(new_n69_));
  inv1   g036(.a(x16), .O(new_n70_));
  inv1   g037(.a(x22), .O(new_n71_));
  inv1   g038(.a(x23), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n59_), .d(new_n70_), .O(new_n73_));
  inv1   g040(.a(x12), .O(new_n74_));
  inv1   g041(.a(x14), .O(new_n75_));
  nand3  g042(.a(new_n60_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nand3  g044(.a(new_n77_), .b(new_n69_), .c(new_n66_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n63_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n40_), .O(new_n80_));
  nor2   g047(.a(new_n60_), .b(new_n59_), .O(new_n81_));
  inv1   g048(.a(new_n61_), .O(new_n82_));
  nand4  g049(.a(new_n82_), .b(new_n81_), .c(new_n54_), .d(x18), .O(new_n83_));
  inv1   g050(.a(new_n83_), .O(new_n84_));
  nor2   g051(.a(x17), .b(x16), .O(new_n85_));
  nor2   g052(.a(x23), .b(x22), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n85_), .c(new_n35_), .d(new_n34_), .O(new_n87_));
  nand2  g054(.a(new_n67_), .b(x07), .O(new_n88_));
  nor3   g055(.a(new_n88_), .b(new_n87_), .c(new_n76_), .O(new_n89_));
  nor2   g056(.a(new_n46_), .b(new_n40_), .O(new_n90_));
  oai21  g057(.a(new_n89_), .b(new_n84_), .c(new_n90_), .O(new_n91_));
  nand4  g058(.a(new_n82_), .b(new_n81_), .c(new_n54_), .d(x15), .O(new_n92_));
  inv1   g059(.a(new_n92_), .O(new_n93_));
  inv1   g060(.a(x08), .O(new_n94_));
  nand2  g061(.a(x19), .b(new_n94_), .O(new_n95_));
  inv1   g062(.a(x09), .O(new_n96_));
  nand3  g063(.a(new_n36_), .b(new_n74_), .c(new_n96_), .O(new_n97_));
  nor3   g064(.a(new_n97_), .b(new_n95_), .c(new_n87_), .O(new_n98_));
  oai21  g065(.a(new_n98_), .b(new_n93_), .c(new_n46_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n91_), .c(new_n80_), .O(z1));
  nand2  g067(.a(x24), .b(x18), .O(new_n101_));
  nand3  g068(.a(new_n40_), .b(x15), .c(x13), .O(new_n102_));
  aoi21  g069(.a(new_n102_), .b(new_n101_), .c(new_n38_), .O(new_n103_));
  nand4  g070(.a(new_n40_), .b(x21), .c(new_n44_), .d(new_n43_), .O(new_n104_));
  nand3  g071(.a(x24), .b(x18), .c(x13), .O(new_n105_));
  nand2  g072(.a(new_n46_), .b(x15), .O(new_n106_));
  nand3  g073(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(new_n103_), .O(new_n108_));
  nand2  g075(.a(x14), .b(x11), .O(new_n109_));
  nand4  g076(.a(x20), .b(x06), .c(x03), .d(x01), .O(new_n110_));
  nor2   g077(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  inv1   g078(.a(new_n111_), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n108_), .O(z2));
  nand3  g080(.a(new_n111_), .b(new_n58_), .c(x08), .O(new_n114_));
  inv1   g081(.a(new_n37_), .O(new_n115_));
  nand3  g082(.a(new_n66_), .b(new_n115_), .c(new_n94_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n40_), .O(new_n118_));
  nand2  g085(.a(new_n94_), .b(x07), .O(new_n119_));
  nand4  g086(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n120_));
  nand4  g087(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n121_));
  oai22  g088(.a(new_n121_), .b(new_n120_), .c(new_n119_), .d(new_n37_), .O(new_n122_));
  nand4  g089(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n123_));
  oai22  g090(.a(new_n123_), .b(new_n120_), .c(new_n95_), .d(new_n37_), .O(new_n124_));
  aoi22  g091(.a(new_n124_), .b(new_n46_), .c(new_n122_), .d(new_n90_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n118_), .O(z3));
  aoi21  g093(.a(new_n72_), .b(x04), .c(x17), .O(new_n127_));
  oai21  g094(.a(new_n127_), .b(x22), .c(new_n96_), .O(new_n128_));
  aoi21  g095(.a(new_n128_), .b(new_n70_), .c(x08), .O(new_n129_));
  inv1   g096(.a(x04), .O(new_n130_));
  aoi21  g097(.a(x23), .b(new_n130_), .c(new_n59_), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(new_n71_), .c(x09), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(x16), .c(new_n94_), .O(new_n133_));
  oai22  g100(.a(new_n133_), .b(new_n108_), .c(new_n129_), .d(new_n50_), .O(z4));
  inv1   g101(.a(new_n50_), .O(z5));
  inv1   g102(.a(x11), .O(new_n136_));
  inv1   g103(.a(x06), .O(new_n137_));
  oai21  g104(.a(new_n60_), .b(x14), .c(new_n137_), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(new_n136_), .c(x03), .O(new_n139_));
  inv1   g106(.a(x03), .O(new_n140_));
  oai21  g107(.a(x20), .b(new_n75_), .c(x06), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(x11), .c(new_n140_), .O(new_n142_));
  oai22  g109(.a(new_n142_), .b(new_n108_), .c(new_n139_), .d(new_n50_), .O(z6));
  inv1   g110(.a(new_n108_), .O(z7));
endmodule


