// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x05), .O(new_n34_));
  nand2  g01(.a(x24), .b(x07), .O(new_n35_));
  inv1   g02(.a(x24), .O(new_n36_));
  nand3  g03(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g04(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g05(.a(x02), .O(new_n39_));
  inv1   g06(.a(x10), .O(new_n40_));
  nand4  g07(.a(new_n36_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  nor2   g08(.a(x13), .b(x05), .O(new_n42_));
  nand2  g09(.a(new_n42_), .b(x19), .O(new_n43_));
  nand3  g10(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  nand3  g11(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  nor2   g12(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  inv1   g13(.a(x06), .O(new_n47_));
  inv1   g14(.a(x11), .O(new_n48_));
  inv1   g15(.a(x01), .O(new_n49_));
  inv1   g16(.a(x03), .O(new_n50_));
  inv1   g17(.a(x14), .O(new_n51_));
  inv1   g18(.a(x20), .O(new_n52_));
  nand4  g19(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  inv1   g20(.a(new_n53_), .O(new_n54_));
  nand3  g21(.a(new_n54_), .b(new_n48_), .c(new_n47_), .O(new_n55_));
  nor2   g22(.a(new_n55_), .b(new_n46_), .O(z0));
  nor2   g23(.a(x23), .b(x22), .O(new_n57_));
  nor2   g24(.a(x06), .b(x04), .O(new_n58_));
  nor2   g25(.a(x09), .b(x08), .O(new_n59_));
  nand3  g26(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  inv1   g27(.a(new_n60_), .O(new_n61_));
  nor2   g28(.a(x12), .b(x11), .O(new_n62_));
  nor2   g29(.a(x17), .b(x16), .O(new_n63_));
  nand4  g30(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n54_), .O(new_n64_));
  nor2   g31(.a(new_n64_), .b(new_n46_), .O(z1));
  nand2  g32(.a(x24), .b(x18), .O(new_n66_));
  nand3  g33(.a(new_n36_), .b(x15), .c(x13), .O(new_n67_));
  aoi21  g34(.a(new_n67_), .b(new_n66_), .c(new_n34_), .O(new_n68_));
  nand4  g35(.a(new_n36_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n69_));
  nand3  g36(.a(x24), .b(x18), .c(x13), .O(new_n70_));
  nand2  g37(.a(new_n42_), .b(x15), .O(new_n71_));
  nand3  g38(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  nor2   g39(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  nand4  g40(.a(x11), .b(x06), .c(x03), .d(x01), .O(new_n74_));
  inv1   g41(.a(new_n74_), .O(new_n75_));
  nand3  g42(.a(new_n75_), .b(x20), .c(x14), .O(new_n76_));
  nor2   g43(.a(new_n76_), .b(new_n73_), .O(z2));
  nand3  g44(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n78_));
  nand3  g45(.a(x15), .b(x13), .c(x05), .O(new_n79_));
  nand2  g46(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g47(.a(x20), .b(x14), .c(x08), .O(new_n81_));
  nor2   g48(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  nand2  g49(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand3  g50(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n84_));
  nand3  g51(.a(x19), .b(x13), .c(x05), .O(new_n85_));
  nand2  g52(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g53(.a(new_n48_), .b(new_n47_), .c(new_n50_), .d(new_n49_), .O(new_n87_));
  inv1   g54(.a(new_n87_), .O(new_n88_));
  nor3   g55(.a(x20), .b(x14), .c(x08), .O(new_n89_));
  nand3  g56(.a(new_n89_), .b(new_n88_), .c(new_n86_), .O(new_n90_));
  nand2  g57(.a(new_n90_), .b(new_n83_), .O(new_n91_));
  nand2  g58(.a(new_n91_), .b(new_n36_), .O(new_n92_));
  nand4  g59(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n93_));
  nand4  g60(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n94_));
  inv1   g61(.a(x08), .O(new_n95_));
  nand4  g62(.a(new_n52_), .b(new_n51_), .c(new_n95_), .d(x07), .O(new_n96_));
  oai22  g63(.a(new_n96_), .b(new_n87_), .c(new_n94_), .d(new_n93_), .O(new_n97_));
  nor2   g64(.a(new_n42_), .b(new_n36_), .O(new_n98_));
  nand4  g65(.a(x20), .b(x15), .c(x14), .d(x11), .O(new_n99_));
  nand4  g66(.a(new_n52_), .b(x19), .c(new_n51_), .d(new_n95_), .O(new_n100_));
  oai22  g67(.a(new_n100_), .b(new_n87_), .c(new_n99_), .d(new_n93_), .O(new_n101_));
  aoi22  g68(.a(new_n101_), .b(new_n42_), .c(new_n98_), .d(new_n97_), .O(new_n102_));
  nand2  g69(.a(new_n102_), .b(new_n92_), .O(z3));
  inv1   g70(.a(x22), .O(new_n104_));
  nand2  g71(.a(new_n104_), .b(x09), .O(new_n105_));
  aoi21  g72(.a(new_n105_), .b(x16), .c(new_n95_), .O(new_n106_));
  inv1   g73(.a(x16), .O(new_n107_));
  nand2  g74(.a(new_n57_), .b(x04), .O(new_n108_));
  nor2   g75(.a(x17), .b(x09), .O(new_n109_));
  nand2  g76(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g77(.a(new_n110_), .b(new_n107_), .c(x08), .O(new_n111_));
  oai22  g78(.a(new_n111_), .b(new_n46_), .c(new_n106_), .d(new_n73_), .O(z4));
  inv1   g79(.a(new_n46_), .O(z5));
  oai21  g80(.a(new_n52_), .b(x14), .c(new_n47_), .O(new_n114_));
  aoi21  g81(.a(new_n114_), .b(new_n48_), .c(x03), .O(new_n115_));
  oai21  g82(.a(x20), .b(new_n51_), .c(x06), .O(new_n116_));
  aoi21  g83(.a(new_n116_), .b(x11), .c(new_n50_), .O(new_n117_));
  oai22  g84(.a(new_n117_), .b(new_n73_), .c(new_n115_), .d(new_n46_), .O(z6));
  inv1   g85(.a(new_n73_), .O(z7));
endmodule


