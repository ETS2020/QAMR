// Benchmark "FAU" written by ABC on Mon Jul 27 18:39:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  nor2   g004(.a(x10), .b(x02), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x00), .O(new_n39_));
  nor2   g006(.a(x13), .b(x05), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(x19), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(new_n41_), .c(new_n39_), .O(new_n43_));
  aoi21  g010(.a(new_n37_), .b(x05), .c(new_n43_), .O(new_n44_));
  inv1   g011(.a(x06), .O(new_n45_));
  inv1   g012(.a(x20), .O(new_n46_));
  nor2   g013(.a(x14), .b(x11), .O(new_n47_));
  nor2   g014(.a(x03), .b(x01), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  nor2   g016(.a(new_n49_), .b(new_n44_), .O(z0));
  nand4  g017(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n51_));
  inv1   g018(.a(new_n51_), .O(new_n52_));
  nand4  g019(.a(x23), .b(x22), .c(x14), .d(x12), .O(new_n53_));
  inv1   g020(.a(new_n53_), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand3  g022(.a(x11), .b(x09), .c(x04), .O(new_n56_));
  inv1   g023(.a(new_n56_), .O(new_n57_));
  inv1   g024(.a(x21), .O(new_n58_));
  nor2   g025(.a(x24), .b(new_n58_), .O(new_n59_));
  nand3  g026(.a(x20), .b(x17), .c(x16), .O(new_n60_));
  inv1   g027(.a(new_n60_), .O(new_n61_));
  nand3  g028(.a(new_n61_), .b(new_n59_), .c(new_n57_), .O(new_n62_));
  nor2   g029(.a(x17), .b(x16), .O(new_n63_));
  nor2   g030(.a(x23), .b(x22), .O(new_n64_));
  nor2   g031(.a(x20), .b(x12), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n47_), .O(new_n66_));
  nor3   g033(.a(x04), .b(x03), .c(x01), .O(new_n67_));
  inv1   g034(.a(x00), .O(new_n68_));
  nor2   g035(.a(x06), .b(new_n68_), .O(new_n69_));
  nor2   g036(.a(x09), .b(x08), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(new_n69_), .c(new_n67_), .O(new_n71_));
  oai22  g038(.a(new_n71_), .b(new_n66_), .c(new_n62_), .d(new_n55_), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n38_), .O(new_n73_));
  nand3  g040(.a(new_n35_), .b(x13), .c(x05), .O(new_n74_));
  oai21  g041(.a(x13), .b(x05), .c(new_n74_), .O(new_n75_));
  nor2   g042(.a(x08), .b(x06), .O(new_n76_));
  inv1   g043(.a(x19), .O(new_n77_));
  nor2   g044(.a(x20), .b(new_n77_), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n76_), .c(new_n48_), .d(new_n47_), .O(new_n79_));
  nor3   g046(.a(x12), .b(x09), .c(x04), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n64_), .c(new_n63_), .O(new_n81_));
  nor2   g048(.a(new_n53_), .b(new_n51_), .O(new_n82_));
  nand4  g049(.a(x20), .b(x17), .c(x16), .d(x15), .O(new_n83_));
  nor2   g050(.a(new_n83_), .b(new_n56_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  oai21  g052(.a(new_n81_), .b(new_n79_), .c(new_n85_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n75_), .O(new_n87_));
  inv1   g054(.a(x04), .O(new_n88_));
  nor2   g055(.a(new_n34_), .b(x06), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n70_), .c(new_n48_), .d(new_n88_), .O(new_n90_));
  nand4  g057(.a(x20), .b(x18), .c(x17), .d(x16), .O(new_n91_));
  nor2   g058(.a(new_n91_), .b(new_n56_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n82_), .O(new_n93_));
  oai21  g060(.a(new_n90_), .b(new_n66_), .c(new_n93_), .O(new_n94_));
  nor2   g061(.a(new_n40_), .b(new_n35_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g063(.a(new_n96_), .b(new_n87_), .c(new_n73_), .O(z1));
  inv1   g064(.a(x05), .O(new_n98_));
  nand2  g065(.a(x24), .b(x18), .O(new_n99_));
  nand3  g066(.a(new_n35_), .b(x15), .c(x13), .O(new_n100_));
  aoi21  g067(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(new_n101_));
  nand3  g068(.a(x24), .b(x18), .c(x13), .O(new_n102_));
  nand2  g069(.a(new_n40_), .b(x15), .O(new_n103_));
  nand3  g070(.a(new_n38_), .b(new_n35_), .c(x21), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  nor2   g072(.a(new_n105_), .b(new_n101_), .O(new_n106_));
  nand3  g073(.a(x20), .b(x14), .c(x11), .O(new_n107_));
  inv1   g074(.a(new_n107_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(x06), .c(x03), .d(x01), .O(new_n109_));
  nor2   g076(.a(new_n109_), .b(new_n106_), .O(z2));
  inv1   g077(.a(x15), .O(new_n111_));
  nand2  g078(.a(new_n108_), .b(new_n52_), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n111_), .c(new_n79_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n75_), .O(new_n114_));
  inv1   g081(.a(x18), .O(new_n115_));
  nor2   g082(.a(new_n112_), .b(new_n115_), .O(new_n116_));
  nand2  g083(.a(new_n89_), .b(new_n48_), .O(new_n117_));
  inv1   g084(.a(x08), .O(new_n118_));
  nand3  g085(.a(new_n47_), .b(new_n46_), .c(new_n118_), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n116_), .c(new_n95_), .O(new_n121_));
  nand2  g088(.a(new_n69_), .b(new_n48_), .O(new_n122_));
  nand3  g089(.a(new_n108_), .b(new_n59_), .c(new_n52_), .O(new_n123_));
  oai21  g090(.a(new_n122_), .b(new_n119_), .c(new_n123_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n38_), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(new_n121_), .c(new_n114_), .O(z3));
  inv1   g093(.a(x16), .O(new_n127_));
  inv1   g094(.a(x09), .O(new_n128_));
  inv1   g095(.a(x23), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(x04), .c(x17), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(x22), .c(new_n128_), .O(new_n131_));
  aoi21  g098(.a(new_n131_), .b(new_n127_), .c(x08), .O(new_n132_));
  inv1   g099(.a(x22), .O(new_n133_));
  inv1   g100(.a(x17), .O(new_n134_));
  aoi21  g101(.a(x23), .b(new_n88_), .c(new_n134_), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(new_n133_), .c(x09), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(x16), .c(new_n118_), .O(new_n137_));
  oai22  g104(.a(new_n137_), .b(new_n106_), .c(new_n132_), .d(new_n44_), .O(z4));
  inv1   g105(.a(new_n44_), .O(z5));
  inv1   g106(.a(x11), .O(new_n140_));
  oai21  g107(.a(new_n46_), .b(x14), .c(new_n45_), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(new_n140_), .c(x03), .O(new_n142_));
  inv1   g109(.a(x03), .O(new_n143_));
  inv1   g110(.a(x14), .O(new_n144_));
  oai21  g111(.a(x20), .b(new_n144_), .c(x06), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(x11), .c(new_n143_), .O(new_n146_));
  oai22  g113(.a(new_n146_), .b(new_n106_), .c(new_n142_), .d(new_n44_), .O(z6));
  inv1   g114(.a(new_n106_), .O(z7));
endmodule


