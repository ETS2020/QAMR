// Benchmark "FAU" written by ABC on Mon Jul 27 18:08:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n41_, new_n42_,
    new_n43_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x11), .O(new_n31_));
  nand2  g01(.a(x12), .b(new_n31_), .O(z01));
  nand4  g02(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n33_));
  inv1   g03(.a(new_n33_), .O(new_n34_));
  nand3  g04(.a(x12), .b(x11), .c(x09), .O(new_n35_));
  inv1   g05(.a(new_n35_), .O(new_n36_));
  nand2  g06(.a(new_n36_), .b(new_n34_), .O(z02));
  nand2  g07(.a(new_n36_), .b(new_n33_), .O(z03));
  inv1   g08(.a(x14), .O(z04));
  inv1   g09(.a(x13), .O(z05));
  inv1   g10(.a(x09), .O(new_n41_));
  nand3  g11(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  nand2  g12(.a(new_n42_), .b(x01), .O(new_n43_));
  aoi21  g13(.a(new_n43_), .b(z01), .c(new_n41_), .O(z06));
  inv1   g14(.a(x15), .O(z07));
  inv1   g15(.a(new_n42_), .O(new_n46_));
  nand3  g16(.a(new_n46_), .b(x12), .c(x09), .O(new_n47_));
  inv1   g17(.a(x00), .O(new_n48_));
  nor2   g18(.a(x10), .b(new_n48_), .O(new_n49_));
  nand3  g19(.a(new_n49_), .b(new_n47_), .c(x11), .O(z08));
  nand2  g20(.a(x12), .b(x11), .O(new_n51_));
  inv1   g21(.a(x10), .O(new_n52_));
  nand2  g22(.a(new_n52_), .b(x00), .O(new_n53_));
  inv1   g23(.a(x12), .O(new_n54_));
  nand2  g24(.a(new_n54_), .b(new_n31_), .O(new_n55_));
  aoi21  g25(.a(new_n55_), .b(new_n51_), .c(new_n53_), .O(z09));
  nand3  g26(.a(new_n49_), .b(new_n42_), .c(new_n36_), .O(new_n57_));
  inv1   g27(.a(new_n57_), .O(z10));
  inv1   g28(.a(x01), .O(new_n59_));
  nand2  g29(.a(new_n54_), .b(new_n59_), .O(new_n60_));
  nand3  g30(.a(new_n42_), .b(new_n36_), .c(x01), .O(new_n61_));
  aoi21  g31(.a(new_n61_), .b(new_n60_), .c(new_n53_), .O(z11));
  nor3   g32(.a(new_n53_), .b(new_n51_), .c(x09), .O(z12));
  one    g33(.O(z00));
endmodule


