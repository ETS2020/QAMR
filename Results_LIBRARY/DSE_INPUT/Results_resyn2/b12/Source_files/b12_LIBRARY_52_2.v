// Benchmark "FAU" written by ABC on Mon Jul 27 21:36:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_,
    new_n50_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_;
  inv1   g00(.a(x03), .O(new_n25_));
  nor2   g01(.a(new_n25_), .b(x02), .O(new_n26_));
  and2   g02(.a(x05), .b(x02), .O(new_n27_));
  oai21  g03(.a(new_n27_), .b(new_n26_), .c(x01), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x02), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(x00), .O(z0));
  nand2  g07(.a(new_n26_), .b(x01), .O(new_n32_));
  nand3  g08(.a(x06), .b(x05), .c(x01), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x02), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n32_), .c(x00), .O(z1));
  inv1   g11(.a(x09), .O(new_n36_));
  nor2   g12(.a(x10), .b(x07), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  aoi21  g14(.a(x10), .b(x03), .c(x01), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  inv1   g16(.a(x07), .O(new_n41_));
  nand2  g17(.a(x08), .b(x00), .O(new_n42_));
  inv1   g18(.a(x02), .O(new_n43_));
  aoi21  g19(.a(x03), .b(new_n43_), .c(x00), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n41_), .c(new_n42_), .O(new_n45_));
  nor2   g21(.a(new_n45_), .b(new_n40_), .O(z2));
  inv1   g22(.a(x00), .O(new_n47_));
  aoi22  g23(.a(x12), .b(new_n47_), .c(x11), .d(x07), .O(z3));
  inv1   g24(.a(new_n42_), .O(new_n49_));
  nor2   g25(.a(new_n36_), .b(x07), .O(new_n50_));
  aoi22  g26(.a(new_n50_), .b(new_n49_), .c(x11), .d(x07), .O(z4));
  inv1   g27(.a(x12), .O(new_n52_));
  nand4  g28(.a(new_n37_), .b(x13), .c(new_n52_), .d(new_n47_), .O(z5));
  nor2   g29(.a(x02), .b(x01), .O(new_n54_));
  nand2  g30(.a(x09), .b(new_n25_), .O(new_n55_));
  nand2  g31(.a(new_n36_), .b(x03), .O(new_n56_));
  nand3  g32(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  and2   g33(.a(x14), .b(x02), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(new_n55_), .c(x01), .d(new_n47_), .O(new_n59_));
  nand2  g35(.a(new_n42_), .b(new_n37_), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n59_), .c(new_n57_), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(z6));
  inv1   g39(.a(x08), .O(new_n64_));
  aoi21  g40(.a(new_n54_), .b(x03), .c(new_n64_), .O(new_n65_));
  nand3  g41(.a(new_n54_), .b(new_n36_), .c(x03), .O(new_n66_));
  inv1   g42(.a(new_n66_), .O(new_n67_));
  oai21  g43(.a(new_n67_), .b(new_n65_), .c(new_n37_), .O(z7));
  nand2  g44(.a(new_n54_), .b(x03), .O(new_n69_));
  nand2  g45(.a(new_n52_), .b(x09), .O(new_n70_));
  oai22  g46(.a(new_n70_), .b(new_n69_), .c(x09), .d(new_n47_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n37_), .O(z8));
endmodule


