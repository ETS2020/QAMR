// Benchmark "FAU" written by ABC on Mon Jul 27 21:36:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n71_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  nand2  g09(.a(new_n26_), .b(x03), .O(new_n34_));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  nor2   g11(.a(x04), .b(x03), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n34_), .c(x00), .O(z1));
  inv1   g15(.a(x00), .O(new_n40_));
  aoi21  g16(.a(new_n36_), .b(new_n25_), .c(new_n27_), .O(new_n41_));
  inv1   g17(.a(x03), .O(new_n42_));
  oai21  g18(.a(x10), .b(x07), .c(new_n42_), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(x09), .c(new_n44_), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n43_), .c(x01), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(new_n41_), .c(new_n40_), .O(new_n48_));
  inv1   g24(.a(x08), .O(new_n49_));
  nand2  g25(.a(x10), .b(x03), .O(new_n50_));
  inv1   g26(.a(x09), .O(new_n51_));
  nand2  g27(.a(new_n45_), .b(new_n51_), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n50_), .c(new_n25_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n27_), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n49_), .c(new_n44_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n48_), .O(z2));
  aoi22  g32(.a(x12), .b(new_n40_), .c(x11), .d(x07), .O(z3));
  nor2   g33(.a(new_n51_), .b(x07), .O(new_n58_));
  nor2   g34(.a(new_n49_), .b(new_n40_), .O(new_n59_));
  aoi22  g35(.a(new_n59_), .b(new_n58_), .c(x11), .d(x07), .O(z4));
  nor2   g36(.a(x10), .b(x07), .O(new_n61_));
  nor2   g37(.a(x12), .b(x00), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n61_), .c(x13), .O(z5));
  nand2  g39(.a(x09), .b(new_n42_), .O(new_n64_));
  aoi21  g40(.a(new_n51_), .b(x03), .c(x01), .O(new_n65_));
  oai21  g41(.a(new_n49_), .b(new_n40_), .c(new_n61_), .O(new_n66_));
  aoi21  g42(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(z6));
  nand3  g43(.a(x09), .b(x03), .c(new_n25_), .O(new_n68_));
  oai21  g44(.a(new_n42_), .b(x01), .c(new_n49_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n61_), .O(z7));
  oai22  g46(.a(new_n68_), .b(x12), .c(x09), .d(new_n40_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n61_), .O(z8));
endmodule


