// Benchmark "FAU" written by ABC on Mon Jul 27 21:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n28_), .c(x03), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n27_), .c(x00), .O(z0));
  nand2  g09(.a(x06), .b(x02), .O(new_n34_));
  nand3  g10(.a(new_n34_), .b(new_n31_), .c(x03), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  inv1   g13(.a(x06), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n36_), .c(x02), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n35_), .c(x00), .O(z1));
  inv1   g17(.a(x00), .O(new_n42_));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x08), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n43_), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n29_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n30_), .c(new_n45_), .O(z2));
  aoi22  g27(.a(x12), .b(new_n42_), .c(x11), .d(x07), .O(z3));
  nand2  g28(.a(x08), .b(x00), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(new_n54_));
  nor2   g30(.a(new_n46_), .b(x07), .O(new_n55_));
  aoi22  g31(.a(new_n55_), .b(new_n54_), .c(x11), .d(x07), .O(z4));
  inv1   g32(.a(x12), .O(new_n57_));
  nor2   g33(.a(x10), .b(x07), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(x13), .c(new_n57_), .d(new_n42_), .O(z5));
  xor2a  g35(.a(x09), .b(x03), .O(new_n60_));
  nor2   g36(.a(new_n60_), .b(new_n31_), .O(new_n61_));
  nand4  g37(.a(x14), .b(x02), .c(x01), .d(new_n42_), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n53_), .c(new_n58_), .O(new_n63_));
  nor2   g39(.a(new_n63_), .b(new_n61_), .O(z6));
  nor2   g40(.a(x02), .b(x01), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(x09), .c(x03), .O(new_n66_));
  nand2  g42(.a(new_n65_), .b(x03), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n44_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n66_), .c(new_n58_), .O(z7));
  nand2  g45(.a(new_n46_), .b(x00), .O(new_n70_));
  nand4  g46(.a(new_n65_), .b(new_n57_), .c(x09), .d(x03), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n58_), .O(z8));
endmodule


