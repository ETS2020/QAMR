// Benchmark "FAU" written by ABC on Mon Jul 27 21:37:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x03), .O(new_n31_));
  aoi21  g07(.a(new_n28_), .b(new_n30_), .c(new_n31_), .O(new_n32_));
  oai21  g08(.a(new_n29_), .b(new_n28_), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n27_), .c(x00), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  nand2  g11(.a(new_n29_), .b(new_n31_), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n35_), .c(x02), .O(new_n37_));
  nand3  g13(.a(x03), .b(new_n28_), .c(x01), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n37_), .c(x00), .O(z1));
  inv1   g15(.a(x00), .O(new_n40_));
  inv1   g16(.a(x07), .O(new_n41_));
  nand2  g17(.a(x08), .b(new_n28_), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(x09), .c(new_n41_), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n45_), .c(new_n30_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n28_), .c(new_n43_), .O(z2));
  aoi22  g26(.a(x12), .b(new_n40_), .c(x11), .d(x07), .O(z3));
  inv1   g27(.a(x09), .O(new_n52_));
  nor2   g28(.a(new_n52_), .b(x07), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  nor2   g30(.a(new_n54_), .b(new_n40_), .O(new_n55_));
  aoi22  g31(.a(new_n55_), .b(new_n53_), .c(x11), .d(x07), .O(z4));
  inv1   g32(.a(x12), .O(new_n57_));
  nand4  g33(.a(new_n44_), .b(x13), .c(new_n57_), .d(new_n40_), .O(z5));
  inv1   g34(.a(new_n55_), .O(new_n59_));
  nand2  g35(.a(x09), .b(new_n31_), .O(new_n60_));
  nand2  g36(.a(new_n52_), .b(x03), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n60_), .c(new_n30_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  nand2  g39(.a(x01), .b(new_n40_), .O(new_n64_));
  nand3  g40(.a(x14), .b(new_n54_), .c(x02), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(new_n64_), .c(new_n44_), .O(new_n66_));
  aoi21  g42(.a(new_n63_), .b(new_n28_), .c(new_n66_), .O(z6));
  oai21  g43(.a(new_n31_), .b(x01), .c(new_n28_), .O(new_n68_));
  nand4  g44(.a(new_n25_), .b(new_n31_), .c(x01), .d(new_n40_), .O(new_n69_));
  aoi21  g45(.a(new_n69_), .b(new_n68_), .c(new_n54_), .O(new_n70_));
  nand4  g46(.a(new_n52_), .b(x03), .c(new_n28_), .d(new_n30_), .O(new_n71_));
  inv1   g47(.a(new_n71_), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n70_), .c(new_n44_), .O(z7));
  nand2  g49(.a(x03), .b(new_n28_), .O(new_n74_));
  nand3  g50(.a(new_n57_), .b(x09), .c(new_n30_), .O(new_n75_));
  oai22  g51(.a(new_n75_), .b(new_n74_), .c(x09), .d(new_n40_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n44_), .O(z8));
endmodule


