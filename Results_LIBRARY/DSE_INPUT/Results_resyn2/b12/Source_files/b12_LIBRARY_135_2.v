// Benchmark "FAU" written by ABC on Mon Jul 27 21:37:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n59_, new_n60_, new_n61_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  aoi21  g06(.a(new_n27_), .b(new_n26_), .c(new_n30_), .O(new_n31_));
  oai21  g07(.a(new_n25_), .b(new_n27_), .c(new_n31_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  nor2   g09(.a(new_n30_), .b(x02), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(x01), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  nand2  g12(.a(new_n25_), .b(new_n30_), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n36_), .c(x02), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n35_), .c(x00), .O(z1));
  inv1   g15(.a(x00), .O(new_n40_));
  oai21  g16(.a(new_n37_), .b(x01), .c(new_n40_), .O(new_n41_));
  inv1   g17(.a(x07), .O(new_n42_));
  oai21  g18(.a(x10), .b(x01), .c(x08), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n41_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(x02), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x09), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(x02), .c(x03), .O(new_n49_));
  inv1   g25(.a(x08), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(x00), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n48_), .c(x07), .O(new_n52_));
  nand2  g28(.a(x10), .b(x00), .O(new_n53_));
  nor2   g29(.a(x10), .b(x07), .O(new_n54_));
  nor2   g30(.a(new_n54_), .b(x03), .O(new_n55_));
  aoi22  g31(.a(new_n55_), .b(new_n53_), .c(new_n52_), .d(new_n49_), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(x01), .c(new_n46_), .O(z2));
  aoi22  g33(.a(x12), .b(new_n40_), .c(x11), .d(x07), .O(z3));
  nor2   g34(.a(x07), .b(new_n40_), .O(new_n59_));
  inv1   g35(.a(x09), .O(new_n60_));
  nor2   g36(.a(new_n60_), .b(new_n50_), .O(new_n61_));
  aoi22  g37(.a(new_n61_), .b(new_n59_), .c(x11), .d(x07), .O(z4));
  inv1   g38(.a(x12), .O(new_n63_));
  nand3  g39(.a(new_n54_), .b(x13), .c(new_n63_), .O(z5));
  inv1   g40(.a(new_n54_), .O(new_n65_));
  oai21  g41(.a(new_n60_), .b(x03), .c(new_n27_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n26_), .O(new_n67_));
  aoi21  g43(.a(x14), .b(x02), .c(new_n26_), .O(new_n68_));
  aoi21  g44(.a(new_n34_), .b(new_n60_), .c(new_n68_), .O(new_n69_));
  aoi21  g45(.a(new_n69_), .b(new_n67_), .c(new_n65_), .O(z6));
  nand3  g46(.a(x03), .b(new_n27_), .c(new_n26_), .O(new_n71_));
  aoi21  g47(.a(new_n71_), .b(new_n50_), .c(new_n65_), .O(new_n72_));
  oai21  g48(.a(new_n71_), .b(new_n48_), .c(new_n72_), .O(z7));
  oai21  g49(.a(x05), .b(new_n25_), .c(new_n26_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(x02), .c(new_n40_), .O(new_n75_));
  nand4  g51(.a(new_n63_), .b(new_n47_), .c(x09), .d(new_n42_), .O(new_n76_));
  aoi21  g52(.a(new_n27_), .b(x01), .c(new_n76_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n75_), .c(new_n67_), .O(z8));
endmodule


