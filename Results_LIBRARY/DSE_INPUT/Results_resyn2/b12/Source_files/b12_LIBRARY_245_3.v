// Benchmark "FAU" written by ABC on Mon Jul 27 21:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  nor2   g06(.a(x02), .b(x01), .O(new_n31_));
  nor2   g07(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g08(.a(new_n25_), .b(new_n27_), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n29_), .c(x00), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n33_), .c(x00), .O(z1));
  inv1   g13(.a(x00), .O(new_n38_));
  nand4  g14(.a(x06), .b(x05), .c(x04), .d(new_n26_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(x02), .O(new_n40_));
  nor2   g16(.a(x10), .b(x07), .O(new_n41_));
  nor2   g17(.a(new_n41_), .b(new_n30_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  nor3   g19(.a(x10), .b(x09), .c(x07), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(x01), .c(new_n27_), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n43_), .c(new_n38_), .O(new_n46_));
  nand2  g22(.a(x10), .b(x03), .O(new_n47_));
  inv1   g23(.a(x09), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n47_), .c(new_n26_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n27_), .O(new_n52_));
  nor2   g28(.a(x08), .b(x07), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n46_), .O(z2));
  aoi22  g31(.a(x12), .b(new_n38_), .c(x11), .d(x07), .O(z3));
  inv1   g32(.a(x08), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(new_n38_), .O(new_n58_));
  nor2   g34(.a(new_n48_), .b(x07), .O(new_n59_));
  aoi22  g35(.a(new_n59_), .b(new_n58_), .c(x11), .d(x07), .O(z4));
  inv1   g36(.a(x12), .O(new_n61_));
  nand4  g37(.a(new_n41_), .b(x13), .c(new_n61_), .d(new_n38_), .O(z5));
  xnor2a g38(.a(x09), .b(x03), .O(new_n63_));
  inv1   g39(.a(new_n58_), .O(new_n64_));
  nand3  g40(.a(new_n28_), .b(x14), .c(new_n38_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n41_), .O(new_n66_));
  aoi21  g42(.a(new_n63_), .b(new_n31_), .c(new_n66_), .O(z6));
  nand2  g43(.a(new_n31_), .b(x03), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x09), .O(new_n70_));
  nand2  g46(.a(new_n68_), .b(new_n57_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n41_), .O(z7));
  nand2  g48(.a(new_n48_), .b(x00), .O(new_n73_));
  nand3  g49(.a(new_n69_), .b(new_n61_), .c(x09), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n41_), .O(z8));
endmodule


