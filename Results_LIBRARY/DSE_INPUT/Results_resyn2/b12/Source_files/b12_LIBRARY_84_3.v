// Benchmark "FAU" written by ABC on Mon Jul 27 21:37:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  inv1   g06(.a(new_n30_), .O(new_n31_));
  nand2  g07(.a(x04), .b(x02), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(new_n31_), .c(x03), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  nand2  g12(.a(new_n32_), .b(new_n36_), .O(new_n37_));
  nand3  g13(.a(new_n28_), .b(x06), .c(x05), .O(new_n38_));
  nand4  g14(.a(new_n38_), .b(new_n37_), .c(new_n31_), .d(new_n35_), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(z1));
  oai21  g16(.a(x10), .b(x07), .c(new_n36_), .O(new_n41_));
  inv1   g17(.a(x10), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(x09), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n41_), .c(x01), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(x02), .c(new_n35_), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  nand3  g23(.a(x10), .b(new_n47_), .c(new_n36_), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n43_), .c(x01), .O(new_n49_));
  nor2   g25(.a(x07), .b(new_n27_), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n49_), .c(new_n46_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n45_), .O(z2));
  inv1   g28(.a(x09), .O(new_n53_));
  inv1   g29(.a(x12), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(x00), .c(new_n55_), .O(new_n56_));
  oai21  g32(.a(new_n53_), .b(new_n35_), .c(new_n56_), .O(z3));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  nor2   g35(.a(new_n53_), .b(x07), .O(new_n60_));
  aoi22  g36(.a(new_n60_), .b(new_n59_), .c(x11), .d(x07), .O(z4));
  nor2   g37(.a(x10), .b(x07), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(x13), .c(new_n54_), .d(new_n35_), .O(z5));
  oai21  g39(.a(x09), .b(new_n36_), .c(new_n30_), .O(new_n64_));
  nand3  g40(.a(new_n28_), .b(x14), .c(new_n35_), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n64_), .c(new_n58_), .d(new_n47_), .O(new_n66_));
  nand4  g42(.a(new_n58_), .b(x09), .c(new_n36_), .d(new_n26_), .O(new_n67_));
  aoi21  g43(.a(new_n67_), .b(new_n66_), .c(x10), .O(z6));
  nand2  g44(.a(new_n30_), .b(x03), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x09), .O(new_n71_));
  nand2  g47(.a(new_n69_), .b(new_n46_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n62_), .O(z7));
  nand2  g49(.a(new_n53_), .b(x00), .O(new_n74_));
  nand3  g50(.a(new_n70_), .b(new_n54_), .c(x09), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n62_), .O(z8));
endmodule


