// Benchmark "FAU" written by ABC on Mon Jul 27 21:36:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_,
    new_n59_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x03), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x04), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(x02), .c(x01), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n27_), .c(x00), .O(z0));
  inv1   g07(.a(x03), .O(new_n32_));
  nand2  g08(.a(x04), .b(x02), .O(new_n33_));
  oai21  g09(.a(x04), .b(new_n32_), .c(new_n33_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n25_), .O(new_n35_));
  nand2  g11(.a(x06), .b(x05), .O(new_n36_));
  nand2  g12(.a(new_n33_), .b(new_n32_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n35_), .c(x00), .O(z1));
  inv1   g15(.a(x02), .O(new_n40_));
  inv1   g16(.a(x09), .O(new_n41_));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  inv1   g20(.a(x00), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  aoi21  g24(.a(new_n44_), .b(new_n40_), .c(new_n48_), .O(z2));
  aoi22  g25(.a(x12), .b(new_n45_), .c(x11), .d(x07), .O(z3));
  nand2  g26(.a(x08), .b(x00), .O(new_n51_));
  inv1   g27(.a(new_n51_), .O(new_n52_));
  nor2   g28(.a(new_n41_), .b(x07), .O(new_n53_));
  aoi22  g29(.a(new_n53_), .b(new_n52_), .c(x11), .d(x07), .O(z4));
  nor2   g30(.a(x12), .b(x00), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n42_), .c(x13), .O(z5));
  nand3  g32(.a(new_n41_), .b(new_n40_), .c(new_n25_), .O(new_n57_));
  nand4  g33(.a(x14), .b(x02), .c(x01), .d(new_n45_), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(new_n57_), .c(new_n51_), .d(new_n42_), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(z6));
  oai21  g36(.a(x08), .b(x07), .c(new_n42_), .O(z7));
  nand3  g37(.a(new_n42_), .b(new_n41_), .c(x00), .O(z8));
endmodule


