// Benchmark "FAU" written by ABC on Mon Jul 27 21:36:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(new_n26_), .c(x01), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand2  g07(.a(x05), .b(new_n28_), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(new_n31_), .c(x02), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  nand3  g10(.a(x06), .b(x02), .c(x01), .O(new_n35_));
  oai21  g11(.a(x02), .b(x01), .c(new_n35_), .O(new_n36_));
  inv1   g12(.a(x00), .O(new_n37_));
  nand2  g13(.a(x04), .b(x02), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n28_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  aoi21  g16(.a(new_n36_), .b(x05), .c(new_n40_), .O(z1));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x08), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n42_), .c(new_n37_), .O(new_n44_));
  inv1   g20(.a(x01), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n42_), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n45_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n27_), .c(new_n44_), .O(z2));
  aoi22  g27(.a(x12), .b(new_n37_), .c(x11), .d(x07), .O(z3));
  nand2  g28(.a(x08), .b(x00), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(new_n54_));
  nor2   g30(.a(new_n46_), .b(x07), .O(new_n55_));
  aoi22  g31(.a(new_n55_), .b(new_n54_), .c(x11), .d(x07), .O(z4));
  nor2   g32(.a(x10), .b(x07), .O(new_n57_));
  nor2   g33(.a(x12), .b(x00), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n57_), .c(x13), .O(z5));
  nor2   g35(.a(x02), .b(x01), .O(new_n60_));
  nand2  g36(.a(new_n46_), .b(x03), .O(new_n61_));
  nand2  g37(.a(x09), .b(new_n28_), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nand4  g39(.a(x14), .b(x02), .c(x01), .d(new_n37_), .O(new_n64_));
  nand3  g40(.a(new_n53_), .b(new_n47_), .c(new_n42_), .O(new_n65_));
  inv1   g41(.a(new_n65_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  inv1   g43(.a(new_n67_), .O(z6));
  aoi21  g44(.a(new_n60_), .b(x03), .c(new_n43_), .O(new_n69_));
  nand3  g45(.a(new_n60_), .b(new_n46_), .c(x03), .O(new_n70_));
  inv1   g46(.a(new_n70_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n69_), .c(new_n57_), .O(z7));
  nand2  g48(.a(new_n60_), .b(x03), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(x12), .c(x09), .O(new_n74_));
  inv1   g50(.a(new_n57_), .O(new_n75_));
  aoi21  g51(.a(new_n46_), .b(new_n37_), .c(new_n75_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n74_), .O(z8));
endmodule


