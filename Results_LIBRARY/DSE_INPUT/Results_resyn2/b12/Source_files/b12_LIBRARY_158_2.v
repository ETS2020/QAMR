// Benchmark "FAU" written by ABC on Mon Jul 27 21:37:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n58_, new_n59_, new_n60_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n72_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  aoi21  g03(.a(new_n27_), .b(new_n25_), .c(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x03), .c(x02), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(x00), .O(z0));
  nand3  g08(.a(x06), .b(x05), .c(x01), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x04), .c(x02), .O(new_n34_));
  nand2  g10(.a(x04), .b(x02), .O(new_n35_));
  nor2   g11(.a(x02), .b(x01), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n35_), .c(x03), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n34_), .c(x00), .O(z1));
  inv1   g15(.a(x00), .O(new_n40_));
  nor3   g16(.a(x05), .b(x04), .c(x03), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n25_), .c(x01), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x09), .O(new_n44_));
  oai21  g20(.a(x10), .b(x07), .c(new_n27_), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n44_), .c(new_n25_), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n42_), .c(new_n40_), .O(new_n47_));
  nand2  g23(.a(x10), .b(x03), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  inv1   g25(.a(x10), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n48_), .c(new_n26_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n25_), .O(new_n53_));
  nor2   g29(.a(x08), .b(x07), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n47_), .O(z2));
  aoi22  g32(.a(x12), .b(new_n40_), .c(x11), .d(x07), .O(z3));
  inv1   g33(.a(x08), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(new_n40_), .O(new_n59_));
  nor2   g35(.a(new_n49_), .b(x07), .O(new_n60_));
  aoi22  g36(.a(new_n60_), .b(new_n59_), .c(x11), .d(x07), .O(z4));
  nor2   g37(.a(x12), .b(x00), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n43_), .c(x13), .O(z5));
  xnor2a g39(.a(x09), .b(x03), .O(new_n64_));
  inv1   g40(.a(new_n59_), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n40_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n43_), .O(new_n67_));
  aoi21  g43(.a(new_n64_), .b(new_n36_), .c(new_n67_), .O(z6));
  nand3  g44(.a(new_n36_), .b(x09), .c(x03), .O(new_n69_));
  oai21  g45(.a(new_n37_), .b(new_n27_), .c(new_n58_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n43_), .O(z7));
  oai22  g47(.a(new_n69_), .b(x12), .c(x09), .d(new_n40_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n43_), .O(z8));
endmodule


