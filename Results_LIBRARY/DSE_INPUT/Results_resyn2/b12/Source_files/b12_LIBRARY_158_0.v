// Benchmark "FAU" written by ABC on Mon Jul 27 21:37:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n53_, new_n54_, new_n56_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_;
  inv1   g00(.a(x03), .O(new_n25_));
  nor2   g01(.a(new_n25_), .b(x02), .O(new_n26_));
  and2   g02(.a(x05), .b(x02), .O(new_n27_));
  oai21  g03(.a(new_n27_), .b(new_n26_), .c(x01), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x02), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(x00), .O(z0));
  inv1   g07(.a(x01), .O(new_n32_));
  inv1   g08(.a(x02), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  oai21  g10(.a(new_n26_), .b(new_n32_), .c(new_n34_), .O(new_n35_));
  nand2  g11(.a(new_n29_), .b(new_n25_), .O(new_n36_));
  nand2  g12(.a(x04), .b(new_n33_), .O(new_n37_));
  nand2  g13(.a(x06), .b(x05), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n35_), .c(x00), .O(z1));
  inv1   g16(.a(x09), .O(new_n41_));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  aoi21  g19(.a(x10), .b(x03), .c(x01), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(x02), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  nand2  g22(.a(x08), .b(x00), .O(new_n47_));
  aoi21  g23(.a(x03), .b(new_n33_), .c(x00), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n46_), .c(new_n47_), .O(new_n49_));
  nor2   g25(.a(new_n49_), .b(new_n45_), .O(z2));
  inv1   g26(.a(x00), .O(new_n51_));
  aoi22  g27(.a(x12), .b(new_n51_), .c(x11), .d(x07), .O(z3));
  inv1   g28(.a(new_n47_), .O(new_n53_));
  nor2   g29(.a(new_n41_), .b(x07), .O(new_n54_));
  aoi22  g30(.a(new_n54_), .b(new_n53_), .c(x11), .d(x07), .O(z4));
  nor2   g31(.a(x12), .b(x00), .O(new_n56_));
  nand3  g32(.a(new_n56_), .b(new_n42_), .c(x13), .O(z5));
  nor2   g33(.a(x02), .b(x01), .O(new_n58_));
  xnor2a g34(.a(x09), .b(x03), .O(new_n59_));
  nand4  g35(.a(x14), .b(x02), .c(x01), .d(new_n51_), .O(new_n60_));
  nand3  g36(.a(new_n60_), .b(new_n47_), .c(new_n42_), .O(new_n61_));
  aoi21  g37(.a(new_n59_), .b(new_n58_), .c(new_n61_), .O(z6));
  nand3  g38(.a(new_n58_), .b(x09), .c(x03), .O(new_n63_));
  inv1   g39(.a(x08), .O(new_n64_));
  nand2  g40(.a(new_n58_), .b(x03), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n63_), .c(new_n42_), .O(z7));
  oai21  g43(.a(new_n65_), .b(x12), .c(x09), .O(new_n68_));
  nand2  g44(.a(new_n41_), .b(new_n51_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n42_), .O(z8));
endmodule


