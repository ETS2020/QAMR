// Benchmark "FAU" written by ABC on Mon Jul 27 21:37:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nor2   g06(.a(new_n27_), .b(new_n25_), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n30_), .c(new_n31_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  nand3  g09(.a(x06), .b(x05), .c(x01), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  nand2  g11(.a(new_n30_), .b(new_n35_), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n34_), .c(x02), .O(new_n37_));
  nor2   g13(.a(new_n35_), .b(x02), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(x01), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n37_), .c(x00), .O(z1));
  inv1   g16(.a(x00), .O(new_n41_));
  inv1   g17(.a(x07), .O(new_n42_));
  nand2  g18(.a(x09), .b(new_n42_), .O(new_n43_));
  nand2  g19(.a(x07), .b(new_n35_), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(x01), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n27_), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(new_n45_), .c(new_n41_), .O(new_n48_));
  inv1   g24(.a(x08), .O(new_n49_));
  nand2  g25(.a(x09), .b(new_n25_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n27_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n49_), .c(new_n42_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n48_), .O(z2));
  aoi22  g29(.a(x12), .b(new_n41_), .c(x11), .d(x07), .O(z3));
  inv1   g30(.a(new_n43_), .O(new_n55_));
  nor2   g31(.a(new_n49_), .b(new_n41_), .O(new_n56_));
  aoi22  g32(.a(new_n56_), .b(new_n55_), .c(x11), .d(x07), .O(z4));
  inv1   g33(.a(x12), .O(new_n58_));
  nor2   g34(.a(x10), .b(x07), .O(new_n59_));
  nand4  g35(.a(new_n59_), .b(x13), .c(new_n58_), .d(new_n41_), .O(z5));
  nand3  g36(.a(x14), .b(x01), .c(new_n41_), .O(new_n61_));
  aoi21  g37(.a(new_n61_), .b(new_n46_), .c(new_n27_), .O(new_n62_));
  xor2a  g38(.a(x09), .b(x03), .O(new_n63_));
  nand2  g39(.a(new_n27_), .b(new_n25_), .O(new_n64_));
  aoi21  g40(.a(x08), .b(x00), .c(x07), .O(new_n65_));
  oai21  g41(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(new_n66_));
  nor2   g42(.a(new_n66_), .b(new_n62_), .O(z6));
  oai21  g43(.a(x08), .b(x07), .c(x01), .O(new_n68_));
  inv1   g44(.a(x09), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n35_), .c(new_n25_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n68_), .c(new_n27_), .O(new_n71_));
  oai21  g47(.a(new_n35_), .b(x02), .c(new_n49_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n59_), .O(z7));
  nand2  g49(.a(new_n69_), .b(x00), .O(new_n74_));
  nand2  g50(.a(new_n38_), .b(new_n58_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(new_n50_), .c(new_n74_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n59_), .O(z8));
endmodule


