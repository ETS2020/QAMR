// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n55_), .b(new_n61_), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n62_), .c(x16), .O(new_n65_));
  inv1   g014(.a(x14), .O(new_n66_));
  aoi21  g015(.a(new_n58_), .b(new_n66_), .c(x18), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n65_), .O(z01));
  inv1   g017(.a(x18), .O(new_n69_));
  nand2  g018(.a(new_n58_), .b(x13), .O(new_n70_));
  inv1   g019(.a(x21), .O(new_n71_));
  nand2  g020(.a(new_n64_), .b(new_n71_), .O(new_n72_));
  aoi21  g021(.a(new_n63_), .b(x21), .c(new_n58_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n69_), .O(z02));
  xor2a  g025(.a(new_n72_), .b(x22), .O(new_n77_));
  inv1   g026(.a(x12), .O(new_n78_));
  aoi21  g027(.a(new_n58_), .b(new_n78_), .c(x18), .O(new_n79_));
  oai21  g028(.a(new_n77_), .b(new_n58_), .c(new_n79_), .O(z03));
  inv1   g029(.a(x22), .O(new_n81_));
  nand4  g030(.a(new_n55_), .b(new_n81_), .c(new_n71_), .d(new_n61_), .O(new_n82_));
  nor3   g031(.a(x23), .b(x22), .c(x21), .O(new_n83_));
  aoi22  g032(.a(new_n83_), .b(new_n64_), .c(new_n82_), .d(x23), .O(new_n84_));
  inv1   g033(.a(x11), .O(new_n85_));
  aoi21  g034(.a(new_n58_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n58_), .c(new_n86_), .O(z04));
  inv1   g036(.a(x24), .O(new_n88_));
  aoi21  g037(.a(new_n83_), .b(new_n64_), .c(new_n88_), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nor2   g039(.a(x24), .b(x21), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n63_), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n89_), .c(x16), .O(new_n94_));
  inv1   g043(.a(x10), .O(new_n95_));
  aoi21  g044(.a(new_n58_), .b(new_n95_), .c(x18), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n94_), .O(z05));
  nand2  g046(.a(new_n92_), .b(x25), .O(new_n98_));
  nor2   g047(.a(x26), .b(new_n53_), .O(new_n99_));
  inv1   g048(.a(x25), .O(new_n100_));
  nand4  g049(.a(new_n91_), .b(new_n90_), .c(new_n100_), .d(new_n61_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n99_), .c(new_n98_), .O(new_n102_));
  nor2   g051(.a(new_n64_), .b(new_n100_), .O(new_n103_));
  aoi21  g052(.a(new_n102_), .b(new_n52_), .c(new_n103_), .O(new_n104_));
  inv1   g053(.a(x09), .O(new_n105_));
  aoi21  g054(.a(new_n58_), .b(new_n105_), .c(x18), .O(new_n106_));
  oai21  g055(.a(new_n104_), .b(new_n58_), .c(new_n106_), .O(z06));
  inv1   g056(.a(x26), .O(new_n108_));
  inv1   g057(.a(new_n101_), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n52_), .c(new_n108_), .O(new_n110_));
  nand3  g059(.a(new_n91_), .b(new_n90_), .c(new_n100_), .O(new_n111_));
  nand3  g060(.a(new_n55_), .b(new_n108_), .c(new_n61_), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(new_n110_), .c(x16), .O(new_n114_));
  inv1   g063(.a(x08), .O(new_n115_));
  aoi21  g064(.a(new_n58_), .b(new_n115_), .c(x18), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n114_), .O(z07));
  inv1   g066(.a(x27), .O(new_n118_));
  inv1   g067(.a(new_n112_), .O(new_n119_));
  nand2  g068(.a(new_n111_), .b(new_n52_), .O(new_n120_));
  aoi21  g069(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nor2   g070(.a(x27), .b(x25), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n91_), .c(new_n90_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n112_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n121_), .c(x16), .O(new_n125_));
  inv1   g074(.a(x07), .O(new_n126_));
  aoi21  g075(.a(new_n58_), .b(new_n126_), .c(x18), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n125_), .O(z08));
  inv1   g077(.a(x28), .O(new_n129_));
  nor2   g078(.a(x24), .b(x23), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n122_), .c(new_n108_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n82_), .c(new_n129_), .O(new_n132_));
  nand2  g081(.a(new_n123_), .b(new_n52_), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n119_), .c(x28), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  inv1   g084(.a(x06), .O(new_n136_));
  aoi21  g085(.a(new_n58_), .b(new_n136_), .c(x18), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n135_), .O(z09));
  inv1   g087(.a(x29), .O(new_n139_));
  nand4  g088(.a(new_n122_), .b(new_n91_), .c(new_n90_), .d(new_n129_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n52_), .O(new_n141_));
  aoi21  g090(.a(new_n141_), .b(new_n119_), .c(new_n139_), .O(new_n142_));
  nor2   g091(.a(x29), .b(x28), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n118_), .O(new_n144_));
  nor3   g093(.a(new_n144_), .b(new_n112_), .c(new_n111_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n142_), .c(x16), .O(new_n146_));
  inv1   g095(.a(x05), .O(new_n147_));
  aoi21  g096(.a(new_n58_), .b(new_n147_), .c(x18), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n146_), .O(z10));
  nor2   g098(.a(new_n145_), .b(x30), .O(new_n150_));
  oai21  g099(.a(new_n144_), .b(new_n111_), .c(new_n52_), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n119_), .c(x30), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  inv1   g102(.a(x04), .O(new_n154_));
  aoi21  g103(.a(new_n58_), .b(new_n154_), .c(x18), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n150_), .c(new_n155_), .O(z11));
  inv1   g105(.a(x31), .O(new_n157_));
  inv1   g106(.a(x30), .O(new_n158_));
  nand2  g107(.a(new_n143_), .b(new_n158_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n123_), .c(new_n52_), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n119_), .c(new_n157_), .O(new_n161_));
  nor2   g110(.a(x31), .b(x30), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n143_), .O(new_n163_));
  nor3   g112(.a(new_n163_), .b(new_n123_), .c(new_n112_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n161_), .c(x16), .O(new_n165_));
  inv1   g114(.a(x03), .O(new_n166_));
  aoi21  g115(.a(new_n58_), .b(new_n166_), .c(x18), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n165_), .O(z12));
  inv1   g117(.a(x32), .O(new_n169_));
  nand3  g118(.a(new_n162_), .b(new_n143_), .c(new_n118_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n111_), .c(new_n52_), .O(new_n171_));
  aoi21  g120(.a(new_n171_), .b(new_n119_), .c(new_n169_), .O(new_n172_));
  nor2   g121(.a(x32), .b(x27), .O(new_n173_));
  nand3  g122(.a(new_n173_), .b(new_n162_), .c(new_n143_), .O(new_n174_));
  nor3   g123(.a(new_n174_), .b(new_n112_), .c(new_n111_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n172_), .c(x16), .O(new_n176_));
  inv1   g125(.a(x02), .O(new_n177_));
  aoi21  g126(.a(new_n58_), .b(new_n177_), .c(x18), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n176_), .O(z13));
  inv1   g128(.a(x33), .O(new_n180_));
  oai21  g129(.a(new_n174_), .b(new_n111_), .c(new_n52_), .O(new_n181_));
  aoi21  g130(.a(new_n181_), .b(new_n119_), .c(new_n180_), .O(new_n182_));
  nand4  g131(.a(new_n162_), .b(new_n143_), .c(new_n180_), .d(new_n169_), .O(new_n183_));
  nor3   g132(.a(new_n183_), .b(new_n131_), .c(new_n82_), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n182_), .c(x16), .O(new_n185_));
  inv1   g134(.a(x01), .O(new_n186_));
  aoi21  g135(.a(new_n58_), .b(new_n186_), .c(x18), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n185_), .O(z14));
  nor2   g137(.a(new_n184_), .b(x34), .O(new_n189_));
  oai21  g138(.a(new_n183_), .b(new_n123_), .c(new_n52_), .O(new_n190_));
  nand3  g139(.a(new_n190_), .b(new_n119_), .c(x34), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(x16), .O(new_n192_));
  inv1   g141(.a(x00), .O(new_n193_));
  aoi21  g142(.a(new_n58_), .b(new_n193_), .c(x18), .O(new_n194_));
  oai21  g143(.a(new_n192_), .b(new_n189_), .c(new_n194_), .O(z15));
endmodule


