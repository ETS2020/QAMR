// Benchmark "FAU" written by ABC on Mon Jul 27 22:44:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g009(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g010(.a(x14), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n61_), .O(z01));
  inv1   g013(.a(x18), .O(new_n65_));
  nand2  g014(.a(new_n52_), .b(x13), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nand2  g016(.a(new_n60_), .b(new_n67_), .O(new_n68_));
  nand2  g017(.a(new_n58_), .b(new_n57_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x21), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n68_), .c(x16), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n66_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n65_), .O(z02));
  nand2  g022(.a(new_n68_), .b(x22), .O(new_n74_));
  nor2   g023(.a(x20), .b(x17), .O(new_n75_));
  nor2   g024(.a(x22), .b(x19), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n75_), .c(new_n67_), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n74_), .c(new_n52_), .O(new_n78_));
  oai21  g027(.a(x16), .b(x12), .c(new_n65_), .O(new_n79_));
  or2    g028(.a(new_n79_), .b(new_n78_), .O(z03));
  inv1   g029(.a(x23), .O(new_n81_));
  aoi21  g030(.a(new_n77_), .b(new_n81_), .c(new_n52_), .O(new_n82_));
  oai21  g031(.a(new_n77_), .b(new_n81_), .c(new_n82_), .O(new_n83_));
  inv1   g032(.a(x11), .O(new_n84_));
  aoi21  g033(.a(new_n52_), .b(new_n84_), .c(x18), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n83_), .O(z04));
  oai21  g035(.a(new_n77_), .b(x23), .c(x24), .O(new_n87_));
  nor2   g036(.a(x24), .b(x23), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n76_), .c(new_n75_), .d(new_n67_), .O(new_n89_));
  aoi21  g038(.a(new_n89_), .b(new_n87_), .c(new_n52_), .O(new_n90_));
  oai21  g039(.a(x16), .b(x10), .c(new_n65_), .O(new_n91_));
  or2    g040(.a(new_n91_), .b(new_n90_), .O(z05));
  xor2a  g041(.a(new_n89_), .b(x25), .O(new_n93_));
  inv1   g042(.a(x09), .O(new_n94_));
  aoi21  g043(.a(new_n52_), .b(new_n94_), .c(x18), .O(new_n95_));
  oai21  g044(.a(new_n93_), .b(new_n52_), .c(new_n95_), .O(z06));
  nor2   g045(.a(x25), .b(x21), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n88_), .c(new_n76_), .d(new_n75_), .O(new_n98_));
  xor2a  g047(.a(new_n98_), .b(x26), .O(new_n99_));
  inv1   g048(.a(x08), .O(new_n100_));
  aoi21  g049(.a(new_n52_), .b(new_n100_), .c(x18), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n52_), .c(new_n101_), .O(z07));
  inv1   g051(.a(x27), .O(new_n103_));
  inv1   g052(.a(x25), .O(new_n104_));
  inv1   g053(.a(new_n89_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nor2   g055(.a(new_n98_), .b(x26), .O(new_n107_));
  nor2   g056(.a(x27), .b(x26), .O(new_n108_));
  inv1   g057(.a(new_n108_), .O(new_n109_));
  oai22  g058(.a(new_n109_), .b(new_n106_), .c(new_n107_), .d(new_n103_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x16), .O(new_n111_));
  inv1   g060(.a(x07), .O(new_n112_));
  aoi21  g061(.a(new_n52_), .b(new_n112_), .c(x18), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n111_), .O(z08));
  inv1   g063(.a(x28), .O(new_n115_));
  inv1   g064(.a(x26), .O(new_n116_));
  nand3  g065(.a(new_n115_), .b(new_n103_), .c(new_n116_), .O(new_n117_));
  nor2   g066(.a(new_n109_), .b(new_n98_), .O(new_n118_));
  oai22  g067(.a(new_n118_), .b(new_n115_), .c(new_n117_), .d(new_n106_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x16), .O(new_n120_));
  inv1   g069(.a(x06), .O(new_n121_));
  aoi21  g070(.a(new_n52_), .b(new_n121_), .c(x18), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n120_), .O(z09));
  nand3  g072(.a(new_n107_), .b(new_n115_), .c(new_n103_), .O(new_n124_));
  nor2   g073(.a(x29), .b(x28), .O(new_n125_));
  aoi22  g074(.a(new_n125_), .b(new_n118_), .c(new_n124_), .d(x29), .O(new_n126_));
  inv1   g075(.a(x05), .O(new_n127_));
  aoi21  g076(.a(new_n52_), .b(new_n127_), .c(x18), .O(new_n128_));
  oai21  g077(.a(new_n126_), .b(new_n52_), .c(new_n128_), .O(z10));
  inv1   g078(.a(x22), .O(new_n130_));
  nor2   g079(.a(x26), .b(x25), .O(new_n131_));
  nor2   g080(.a(x30), .b(x27), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n131_), .c(new_n125_), .d(new_n88_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nor3   g083(.a(x21), .b(x20), .c(x17), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g085(.a(new_n97_), .b(new_n88_), .O(new_n137_));
  nand2  g086(.a(new_n125_), .b(new_n108_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n137_), .c(x30), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n136_), .c(x19), .O(new_n140_));
  inv1   g089(.a(x30), .O(new_n141_));
  nor2   g090(.a(new_n60_), .b(new_n141_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n140_), .c(x16), .O(new_n143_));
  inv1   g092(.a(x04), .O(new_n144_));
  aoi21  g093(.a(new_n52_), .b(new_n144_), .c(x18), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n143_), .O(z11));
  inv1   g095(.a(x31), .O(new_n147_));
  nand4  g096(.a(new_n125_), .b(new_n108_), .c(new_n147_), .d(new_n141_), .O(new_n148_));
  nand4  g097(.a(new_n97_), .b(new_n88_), .c(new_n58_), .d(new_n57_), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  inv1   g099(.a(new_n138_), .O(new_n151_));
  inv1   g100(.a(new_n149_), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n151_), .c(new_n141_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(x31), .c(new_n150_), .O(new_n154_));
  inv1   g103(.a(x03), .O(new_n155_));
  aoi21  g104(.a(new_n52_), .b(new_n155_), .c(x18), .O(new_n156_));
  oai21  g105(.a(new_n154_), .b(new_n52_), .c(new_n156_), .O(z12));
  inv1   g106(.a(x32), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n147_), .O(new_n159_));
  oai22  g108(.a(new_n159_), .b(new_n153_), .c(new_n150_), .d(new_n158_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(x16), .O(new_n161_));
  inv1   g110(.a(x02), .O(new_n162_));
  aoi21  g111(.a(new_n52_), .b(new_n162_), .c(x18), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n161_), .O(z13));
  nor3   g113(.a(x32), .b(x31), .c(x30), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n152_), .c(new_n151_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(x33), .O(new_n167_));
  inv1   g116(.a(x33), .O(new_n168_));
  nand4  g117(.a(new_n165_), .b(new_n152_), .c(new_n151_), .d(new_n168_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n167_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x16), .O(new_n171_));
  inv1   g120(.a(x01), .O(new_n172_));
  aoi21  g121(.a(new_n52_), .b(new_n172_), .c(x18), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z14));
  inv1   g123(.a(x34), .O(new_n175_));
  nor2   g124(.a(x33), .b(x32), .O(new_n176_));
  aoi21  g125(.a(new_n176_), .b(new_n150_), .c(new_n175_), .O(new_n177_));
  nand3  g126(.a(new_n176_), .b(new_n175_), .c(new_n147_), .O(new_n178_));
  nor3   g127(.a(new_n178_), .b(new_n133_), .c(new_n68_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n177_), .c(x16), .O(new_n180_));
  inv1   g129(.a(x00), .O(new_n181_));
  aoi21  g130(.a(new_n52_), .b(new_n181_), .c(x18), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n180_), .O(z15));
endmodule


