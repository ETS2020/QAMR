// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_;
  nor2   g000(.a(x19), .b(x17), .O(new_n52_));
  and2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n54_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  xor2a  g008(.a(new_n52_), .b(new_n59_), .O(new_n60_));
  inv1   g009(.a(x14), .O(new_n61_));
  aoi21  g010(.a(new_n56_), .b(new_n61_), .c(x18), .O(new_n62_));
  oai21  g011(.a(new_n60_), .b(new_n56_), .c(new_n62_), .O(z01));
  inv1   g012(.a(x21), .O(new_n64_));
  aoi21  g013(.a(new_n52_), .b(new_n59_), .c(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n52_), .b(new_n64_), .c(new_n59_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n65_), .c(x16), .O(new_n68_));
  inv1   g017(.a(x13), .O(new_n69_));
  aoi21  g018(.a(new_n56_), .b(new_n69_), .c(x18), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z02));
  xor2a  g020(.a(new_n66_), .b(x22), .O(new_n72_));
  inv1   g021(.a(x12), .O(new_n73_));
  aoi21  g022(.a(new_n56_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n56_), .c(new_n74_), .O(z03));
  inv1   g024(.a(x22), .O(new_n76_));
  nand2  g025(.a(new_n67_), .b(new_n76_), .O(new_n77_));
  nor2   g026(.a(x23), .b(x22), .O(new_n78_));
  aoi22  g027(.a(new_n78_), .b(new_n67_), .c(new_n77_), .d(x23), .O(new_n79_));
  inv1   g028(.a(x11), .O(new_n80_));
  aoi21  g029(.a(new_n56_), .b(new_n80_), .c(x18), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n56_), .c(new_n81_), .O(z04));
  nand4  g031(.a(new_n78_), .b(new_n52_), .c(new_n64_), .d(new_n59_), .O(new_n83_));
  nor2   g032(.a(x24), .b(x23), .O(new_n84_));
  nor2   g033(.a(x22), .b(x21), .O(new_n85_));
  nand4  g034(.a(new_n85_), .b(new_n84_), .c(new_n52_), .d(new_n59_), .O(new_n86_));
  inv1   g035(.a(new_n86_), .O(new_n87_));
  aoi21  g036(.a(new_n83_), .b(x24), .c(new_n87_), .O(new_n88_));
  inv1   g037(.a(x10), .O(new_n89_));
  aoi21  g038(.a(new_n56_), .b(new_n89_), .c(x18), .O(new_n90_));
  oai21  g039(.a(new_n88_), .b(new_n56_), .c(new_n90_), .O(z05));
  nor2   g040(.a(x25), .b(x22), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n84_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n66_), .O(new_n94_));
  aoi21  g043(.a(new_n86_), .b(x25), .c(new_n94_), .O(new_n95_));
  inv1   g044(.a(x09), .O(new_n96_));
  aoi21  g045(.a(new_n56_), .b(new_n96_), .c(x18), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n56_), .c(new_n97_), .O(z06));
  inv1   g047(.a(x26), .O(new_n99_));
  nor3   g048(.a(x26), .b(x25), .c(x24), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n78_), .c(new_n67_), .O(new_n101_));
  oai21  g050(.a(new_n94_), .b(new_n99_), .c(new_n101_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(x16), .O(new_n103_));
  inv1   g052(.a(x08), .O(new_n104_));
  aoi21  g053(.a(new_n56_), .b(new_n104_), .c(x18), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n103_), .O(z07));
  nor3   g055(.a(x27), .b(x26), .c(x25), .O(new_n107_));
  aoi22  g056(.a(new_n107_), .b(new_n87_), .c(new_n101_), .d(x27), .O(new_n108_));
  inv1   g057(.a(x07), .O(new_n109_));
  aoi21  g058(.a(new_n56_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n56_), .c(new_n110_), .O(z08));
  inv1   g060(.a(new_n107_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n86_), .c(x28), .O(new_n113_));
  nor3   g062(.a(x28), .b(x27), .c(x26), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n94_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(x16), .O(new_n117_));
  inv1   g066(.a(x06), .O(new_n118_));
  aoi21  g067(.a(new_n56_), .b(new_n118_), .c(x18), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n117_), .O(z09));
  inv1   g069(.a(x24), .O(new_n121_));
  inv1   g070(.a(x29), .O(new_n122_));
  nor2   g071(.a(x26), .b(x25), .O(new_n123_));
  nor2   g072(.a(x28), .b(x27), .O(new_n124_));
  nand4  g073(.a(new_n124_), .b(new_n123_), .c(new_n122_), .d(new_n121_), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n83_), .O(new_n126_));
  aoi21  g075(.a(new_n115_), .b(x29), .c(new_n126_), .O(new_n127_));
  inv1   g076(.a(x05), .O(new_n128_));
  aoi21  g077(.a(new_n56_), .b(new_n128_), .c(x18), .O(new_n129_));
  oai21  g078(.a(new_n127_), .b(new_n56_), .c(new_n129_), .O(z10));
  oai21  g079(.a(new_n125_), .b(new_n83_), .c(x30), .O(new_n131_));
  inv1   g080(.a(x30), .O(new_n132_));
  nand3  g081(.a(new_n124_), .b(new_n132_), .c(new_n122_), .O(new_n133_));
  inv1   g082(.a(new_n133_), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n100_), .c(new_n78_), .d(new_n67_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(x16), .O(new_n137_));
  inv1   g086(.a(x04), .O(new_n138_));
  aoi21  g087(.a(new_n56_), .b(new_n138_), .c(x18), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n137_), .O(z11));
  nor2   g089(.a(x30), .b(x29), .O(new_n141_));
  nor2   g090(.a(x31), .b(x28), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n141_), .c(new_n107_), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n86_), .O(new_n144_));
  aoi21  g093(.a(new_n135_), .b(x31), .c(new_n144_), .O(new_n145_));
  inv1   g094(.a(x03), .O(new_n146_));
  aoi21  g095(.a(new_n56_), .b(new_n146_), .c(x18), .O(new_n147_));
  oai21  g096(.a(new_n145_), .b(new_n56_), .c(new_n147_), .O(z12));
  nor2   g097(.a(x32), .b(x31), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n78_), .c(new_n132_), .d(new_n64_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n125_), .c(new_n59_), .O(new_n151_));
  nand3  g100(.a(new_n85_), .b(new_n84_), .c(new_n52_), .O(new_n152_));
  inv1   g101(.a(new_n152_), .O(new_n153_));
  nand2  g102(.a(new_n142_), .b(new_n141_), .O(new_n154_));
  inv1   g103(.a(new_n154_), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n107_), .c(new_n153_), .O(new_n156_));
  aoi22  g105(.a(new_n156_), .b(x32), .c(new_n151_), .d(new_n52_), .O(new_n157_));
  inv1   g106(.a(x02), .O(new_n158_));
  aoi21  g107(.a(new_n56_), .b(new_n158_), .c(x18), .O(new_n159_));
  oai21  g108(.a(new_n157_), .b(new_n56_), .c(new_n159_), .O(z13));
  nor3   g109(.a(x21), .b(x19), .c(x17), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n92_), .c(new_n84_), .O(new_n162_));
  nand3  g111(.a(new_n149_), .b(new_n141_), .c(new_n114_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n162_), .c(x33), .O(new_n164_));
  inv1   g113(.a(new_n93_), .O(new_n165_));
  nor2   g114(.a(x27), .b(x26), .O(new_n166_));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  inv1   g117(.a(new_n168_), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n161_), .c(new_n155_), .d(new_n165_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n164_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x16), .O(new_n172_));
  inv1   g121(.a(x01), .O(new_n173_));
  aoi21  g122(.a(new_n56_), .b(new_n173_), .c(x18), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n172_), .O(z14));
  nand4  g124(.a(new_n167_), .b(new_n142_), .c(new_n141_), .d(new_n166_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n162_), .c(x34), .O(new_n177_));
  nor2   g126(.a(x34), .b(x33), .O(new_n178_));
  nand3  g127(.a(new_n178_), .b(new_n149_), .c(new_n123_), .O(new_n179_));
  inv1   g128(.a(new_n179_), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n134_), .c(new_n153_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n177_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(x16), .O(new_n183_));
  inv1   g132(.a(x00), .O(new_n184_));
  aoi21  g133(.a(new_n56_), .b(new_n184_), .c(x18), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n183_), .O(z15));
endmodule


