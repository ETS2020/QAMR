// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n54_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g011(.a(x14), .O(new_n63_));
  aoi21  g012(.a(new_n56_), .b(new_n63_), .c(x18), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n62_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  xor2a  g015(.a(new_n61_), .b(new_n66_), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n56_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n56_), .c(new_n69_), .O(z02));
  nand2  g019(.a(new_n61_), .b(new_n66_), .O(new_n71_));
  nor2   g020(.a(x22), .b(x21), .O(new_n72_));
  aoi22  g021(.a(new_n72_), .b(new_n61_), .c(new_n71_), .d(x22), .O(new_n73_));
  inv1   g022(.a(x12), .O(new_n74_));
  aoi21  g023(.a(new_n56_), .b(new_n74_), .c(x18), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(new_n56_), .c(new_n75_), .O(z03));
  inv1   g025(.a(x23), .O(new_n77_));
  aoi21  g026(.a(new_n72_), .b(new_n61_), .c(new_n77_), .O(new_n78_));
  nand2  g027(.a(new_n53_), .b(new_n59_), .O(new_n79_));
  nand2  g028(.a(new_n72_), .b(new_n77_), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n78_), .c(x16), .O(new_n82_));
  inv1   g031(.a(x11), .O(new_n83_));
  aoi21  g032(.a(new_n56_), .b(new_n83_), .c(x18), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n82_), .O(z04));
  inv1   g034(.a(x24), .O(new_n86_));
  nor2   g035(.a(new_n81_), .b(new_n86_), .O(new_n87_));
  nor3   g036(.a(new_n80_), .b(new_n79_), .c(x24), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(new_n87_), .c(x16), .O(new_n89_));
  inv1   g038(.a(x10), .O(new_n90_));
  aoi21  g039(.a(new_n56_), .b(new_n90_), .c(x18), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n89_), .O(z05));
  inv1   g041(.a(x25), .O(new_n93_));
  xor2a  g042(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  inv1   g043(.a(x09), .O(new_n95_));
  aoi21  g044(.a(new_n56_), .b(new_n95_), .c(x18), .O(new_n96_));
  oai21  g045(.a(new_n94_), .b(new_n56_), .c(new_n96_), .O(z06));
  nor3   g046(.a(x23), .b(x22), .c(x21), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n61_), .c(new_n93_), .d(new_n86_), .O(new_n99_));
  nor3   g048(.a(x26), .b(x25), .c(x24), .O(new_n100_));
  aoi22  g049(.a(new_n100_), .b(new_n81_), .c(new_n99_), .d(x26), .O(new_n101_));
  inv1   g050(.a(x08), .O(new_n102_));
  aoi21  g051(.a(new_n56_), .b(new_n102_), .c(x18), .O(new_n103_));
  oai21  g052(.a(new_n101_), .b(new_n56_), .c(new_n103_), .O(z07));
  inv1   g053(.a(x19), .O(new_n105_));
  inv1   g054(.a(x27), .O(new_n106_));
  nor2   g055(.a(x20), .b(x17), .O(new_n107_));
  nand3  g056(.a(new_n107_), .b(new_n100_), .c(new_n98_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  or2    g058(.a(new_n108_), .b(new_n106_), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n109_), .c(new_n105_), .O(new_n111_));
  nand3  g060(.a(x27), .b(x19), .c(x16), .O(new_n112_));
  inv1   g061(.a(x07), .O(new_n113_));
  aoi21  g062(.a(new_n56_), .b(new_n113_), .c(x18), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(z08));
  inv1   g064(.a(x26), .O(new_n116_));
  nand2  g065(.a(new_n106_), .b(new_n116_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n99_), .c(x28), .O(new_n118_));
  nor2   g067(.a(new_n117_), .b(x28), .O(new_n119_));
  nand4  g068(.a(new_n119_), .b(new_n81_), .c(new_n93_), .d(new_n86_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x16), .O(new_n122_));
  inv1   g071(.a(x06), .O(new_n123_));
  aoi21  g072(.a(new_n56_), .b(new_n123_), .c(x18), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n122_), .O(z09));
  nor3   g074(.a(x29), .b(x28), .c(x27), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n100_), .c(new_n98_), .d(new_n61_), .O(new_n127_));
  inv1   g076(.a(new_n127_), .O(new_n128_));
  aoi21  g077(.a(new_n120_), .b(x29), .c(new_n128_), .O(new_n129_));
  inv1   g078(.a(x05), .O(new_n130_));
  aoi21  g079(.a(new_n56_), .b(new_n130_), .c(x18), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n56_), .c(new_n131_), .O(z10));
  xor2a  g081(.a(new_n127_), .b(x30), .O(new_n133_));
  inv1   g082(.a(x04), .O(new_n134_));
  aoi21  g083(.a(new_n56_), .b(new_n134_), .c(x18), .O(new_n135_));
  oai21  g084(.a(new_n133_), .b(new_n56_), .c(new_n135_), .O(z11));
  oai21  g085(.a(new_n127_), .b(x30), .c(x31), .O(new_n137_));
  inv1   g086(.a(x30), .O(new_n138_));
  inv1   g087(.a(x31), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  inv1   g089(.a(x28), .O(new_n141_));
  inv1   g090(.a(x29), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n141_), .c(new_n106_), .d(new_n116_), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n81_), .c(new_n93_), .d(new_n86_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n137_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(x16), .O(new_n147_));
  inv1   g096(.a(x03), .O(new_n148_));
  aoi21  g097(.a(new_n56_), .b(new_n148_), .c(x18), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n147_), .O(z12));
  inv1   g099(.a(x32), .O(new_n151_));
  nand2  g100(.a(new_n145_), .b(new_n151_), .O(new_n152_));
  inv1   g101(.a(new_n99_), .O(new_n153_));
  nand3  g102(.a(new_n144_), .b(new_n153_), .c(x32), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(x16), .O(new_n155_));
  inv1   g104(.a(x02), .O(new_n156_));
  aoi21  g105(.a(new_n56_), .b(new_n156_), .c(x18), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n155_), .O(z13));
  nand3  g107(.a(new_n151_), .b(new_n139_), .c(new_n138_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n127_), .c(x33), .O(new_n160_));
  inv1   g109(.a(x33), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n151_), .c(new_n139_), .d(new_n138_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n143_), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n81_), .c(new_n93_), .d(new_n86_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n160_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(x16), .O(new_n166_));
  inv1   g115(.a(x01), .O(new_n167_));
  aoi21  g116(.a(new_n56_), .b(new_n167_), .c(x18), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(z14));
  inv1   g118(.a(x34), .O(new_n170_));
  nand2  g119(.a(new_n164_), .b(new_n170_), .O(new_n171_));
  nor2   g120(.a(new_n162_), .b(new_n170_), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n128_), .c(new_n56_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(new_n174_));
  inv1   g123(.a(x00), .O(new_n175_));
  aoi21  g124(.a(new_n56_), .b(new_n175_), .c(x18), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n174_), .O(z15));
endmodule


