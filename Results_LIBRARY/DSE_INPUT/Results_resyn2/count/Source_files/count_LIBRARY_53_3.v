// Benchmark "FAU" written by ABC on Mon Jul 27 22:44:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  inv1   g016(.a(x18), .O(new_n68_));
  nand2  g017(.a(new_n58_), .b(x13), .O(new_n69_));
  inv1   g018(.a(x21), .O(new_n70_));
  nor2   g019(.a(new_n63_), .b(new_n70_), .O(new_n71_));
  nor3   g020(.a(x20), .b(x19), .c(x17), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(x16), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(new_n71_), .c(new_n69_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n68_), .O(z02));
  nand2  g025(.a(x22), .b(x21), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n61_), .c(new_n53_), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(x17), .O(new_n80_));
  inv1   g029(.a(x22), .O(new_n81_));
  aoi21  g030(.a(new_n61_), .b(new_n52_), .c(new_n81_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  oai21  g032(.a(x16), .b(x12), .c(new_n68_), .O(new_n84_));
  aoi21  g033(.a(x22), .b(x19), .c(new_n84_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n83_), .O(z03));
  inv1   g035(.a(x23), .O(new_n87_));
  nand3  g036(.a(new_n61_), .b(new_n53_), .c(new_n52_), .O(new_n88_));
  nand2  g037(.a(new_n81_), .b(new_n70_), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand4  g039(.a(new_n78_), .b(new_n55_), .c(new_n87_), .d(new_n61_), .O(new_n91_));
  oai21  g040(.a(new_n90_), .b(new_n87_), .c(new_n91_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  inv1   g042(.a(x11), .O(new_n94_));
  aoi21  g043(.a(new_n58_), .b(new_n94_), .c(x18), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n93_), .O(z04));
  inv1   g045(.a(x24), .O(new_n97_));
  aoi21  g046(.a(new_n90_), .b(new_n87_), .c(new_n97_), .O(new_n98_));
  nor2   g047(.a(x24), .b(x23), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n78_), .c(new_n63_), .O(new_n100_));
  inv1   g049(.a(new_n100_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n98_), .c(x16), .O(new_n102_));
  inv1   g051(.a(x10), .O(new_n103_));
  aoi21  g052(.a(new_n58_), .b(new_n103_), .c(x18), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n102_), .O(z05));
  inv1   g054(.a(x25), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n97_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n91_), .O(new_n108_));
  aoi21  g057(.a(new_n100_), .b(x25), .c(new_n108_), .O(new_n109_));
  inv1   g058(.a(x09), .O(new_n110_));
  aoi21  g059(.a(new_n58_), .b(new_n110_), .c(x18), .O(new_n111_));
  oai21  g060(.a(new_n109_), .b(new_n58_), .c(new_n111_), .O(z06));
  nand3  g061(.a(new_n106_), .b(new_n97_), .c(new_n87_), .O(new_n113_));
  inv1   g062(.a(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n90_), .O(new_n115_));
  inv1   g064(.a(x26), .O(new_n116_));
  nor2   g065(.a(x25), .b(x24), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n91_), .O(new_n119_));
  aoi21  g068(.a(new_n115_), .b(x26), .c(new_n119_), .O(new_n120_));
  inv1   g069(.a(x08), .O(new_n121_));
  aoi21  g070(.a(new_n58_), .b(new_n121_), .c(x18), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n58_), .c(new_n122_), .O(z07));
  nand4  g072(.a(new_n114_), .b(new_n90_), .c(x27), .d(new_n116_), .O(new_n124_));
  inv1   g073(.a(x27), .O(new_n125_));
  oai21  g074(.a(new_n115_), .b(x26), .c(new_n125_), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n124_), .c(x16), .O(new_n127_));
  inv1   g076(.a(x07), .O(new_n128_));
  aoi21  g077(.a(new_n58_), .b(new_n128_), .c(x18), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n127_), .O(z08));
  inv1   g079(.a(x28), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n125_), .c(new_n116_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n113_), .O(new_n133_));
  nand4  g082(.a(new_n87_), .b(new_n61_), .c(new_n53_), .d(new_n52_), .O(new_n134_));
  inv1   g083(.a(new_n134_), .O(new_n135_));
  nor2   g084(.a(x27), .b(x26), .O(new_n136_));
  nand2  g085(.a(new_n89_), .b(new_n52_), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n136_), .c(new_n117_), .d(new_n135_), .O(new_n138_));
  aoi22  g087(.a(new_n138_), .b(x28), .c(new_n133_), .d(new_n90_), .O(new_n139_));
  inv1   g088(.a(x06), .O(new_n140_));
  aoi21  g089(.a(new_n58_), .b(new_n140_), .c(x18), .O(new_n141_));
  oai21  g090(.a(new_n139_), .b(new_n58_), .c(new_n141_), .O(z09));
  inv1   g091(.a(new_n132_), .O(new_n143_));
  nand4  g092(.a(new_n137_), .b(new_n143_), .c(new_n117_), .d(new_n135_), .O(new_n144_));
  nor3   g093(.a(x29), .b(x28), .c(x27), .O(new_n145_));
  aoi22  g094(.a(new_n145_), .b(new_n119_), .c(new_n144_), .d(x29), .O(new_n146_));
  inv1   g095(.a(x05), .O(new_n147_));
  aoi21  g096(.a(new_n58_), .b(new_n147_), .c(x18), .O(new_n148_));
  oai21  g097(.a(new_n146_), .b(new_n58_), .c(new_n148_), .O(z10));
  inv1   g098(.a(x30), .O(new_n150_));
  nor2   g099(.a(x29), .b(x28), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n125_), .O(new_n152_));
  aoi21  g101(.a(new_n89_), .b(new_n52_), .c(new_n152_), .O(new_n153_));
  nor2   g102(.a(new_n118_), .b(new_n134_), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n153_), .c(new_n150_), .O(new_n155_));
  nor3   g104(.a(x26), .b(x25), .c(x24), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n78_), .c(new_n72_), .d(new_n87_), .O(new_n157_));
  nand2  g106(.a(new_n145_), .b(new_n150_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n155_), .c(x16), .O(new_n160_));
  inv1   g109(.a(x04), .O(new_n161_));
  aoi21  g110(.a(new_n58_), .b(new_n161_), .c(x18), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n160_), .O(z11));
  inv1   g112(.a(x31), .O(new_n164_));
  nand4  g113(.a(new_n151_), .b(new_n136_), .c(new_n150_), .d(new_n106_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n100_), .c(new_n164_), .O(new_n166_));
  inv1   g115(.a(new_n165_), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n101_), .c(x31), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n166_), .c(x16), .O(new_n169_));
  inv1   g118(.a(x03), .O(new_n170_));
  aoi21  g119(.a(new_n58_), .b(new_n170_), .c(x18), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n169_), .O(z12));
  nor3   g121(.a(x31), .b(x30), .c(x29), .O(new_n173_));
  nand3  g122(.a(new_n173_), .b(new_n133_), .c(new_n90_), .O(new_n174_));
  inv1   g123(.a(x29), .O(new_n175_));
  inv1   g124(.a(x32), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n164_), .c(new_n150_), .d(new_n175_), .O(new_n177_));
  nand3  g126(.a(new_n136_), .b(new_n131_), .c(new_n106_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n177_), .O(new_n179_));
  aoi22  g128(.a(new_n179_), .b(new_n101_), .c(new_n174_), .d(x32), .O(new_n180_));
  inv1   g129(.a(x02), .O(new_n181_));
  aoi21  g130(.a(new_n58_), .b(new_n181_), .c(x18), .O(new_n182_));
  oai21  g131(.a(new_n180_), .b(new_n58_), .c(new_n182_), .O(z13));
  inv1   g132(.a(new_n177_), .O(new_n184_));
  nand3  g133(.a(new_n184_), .b(new_n133_), .c(new_n90_), .O(new_n185_));
  inv1   g134(.a(x33), .O(new_n186_));
  nand4  g135(.a(new_n186_), .b(new_n176_), .c(new_n164_), .d(new_n150_), .O(new_n187_));
  nand3  g136(.a(new_n136_), .b(new_n175_), .c(new_n131_), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  aoi22  g138(.a(new_n189_), .b(new_n108_), .c(new_n185_), .d(x33), .O(new_n190_));
  inv1   g139(.a(x01), .O(new_n191_));
  aoi21  g140(.a(new_n58_), .b(new_n191_), .c(x18), .O(new_n192_));
  oai21  g141(.a(new_n190_), .b(new_n58_), .c(new_n192_), .O(z14));
  aoi21  g142(.a(new_n189_), .b(new_n108_), .c(x34), .O(new_n194_));
  nand2  g143(.a(x34), .b(new_n116_), .O(new_n195_));
  nor2   g144(.a(new_n195_), .b(new_n88_), .O(new_n196_));
  nor2   g145(.a(new_n187_), .b(new_n113_), .O(new_n197_));
  nand3  g146(.a(new_n197_), .b(new_n196_), .c(new_n153_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(x16), .O(new_n199_));
  inv1   g148(.a(x00), .O(new_n200_));
  aoi21  g149(.a(new_n58_), .b(new_n200_), .c(x18), .O(new_n201_));
  oai21  g150(.a(new_n199_), .b(new_n194_), .c(new_n201_), .O(z15));
endmodule


