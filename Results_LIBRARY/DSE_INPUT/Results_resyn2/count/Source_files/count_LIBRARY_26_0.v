// Benchmark "FAU" written by ABC on Mon Jul 27 22:44:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x16), .O(new_n52_));
  xor2a  g01(.a(x19), .b(x17), .O(new_n53_));
  inv1   g02(.a(x15), .O(new_n54_));
  aoi21  g03(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g04(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g05(.a(x20), .O(new_n57_));
  nor2   g06(.a(x19), .b(x17), .O(new_n58_));
  nor2   g07(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g08(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g09(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g10(.a(x14), .O(new_n62_));
  aoi21  g11(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g12(.a(new_n63_), .b(new_n61_), .O(z01));
  inv1   g13(.a(x21), .O(new_n65_));
  xor2a  g14(.a(new_n60_), .b(new_n65_), .O(new_n66_));
  inv1   g15(.a(x13), .O(new_n67_));
  aoi21  g16(.a(new_n52_), .b(new_n67_), .c(x18), .O(new_n68_));
  oai21  g17(.a(new_n66_), .b(new_n52_), .c(new_n68_), .O(z02));
  nand3  g18(.a(new_n58_), .b(new_n65_), .c(new_n57_), .O(new_n70_));
  nor2   g19(.a(x22), .b(x21), .O(new_n71_));
  aoi22  g20(.a(new_n71_), .b(new_n60_), .c(new_n70_), .d(x22), .O(new_n72_));
  inv1   g21(.a(x12), .O(new_n73_));
  aoi21  g22(.a(new_n52_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g23(.a(new_n72_), .b(new_n52_), .c(new_n74_), .O(z03));
  nand3  g24(.a(new_n71_), .b(new_n58_), .c(new_n57_), .O(new_n76_));
  nor3   g25(.a(x23), .b(x22), .c(x21), .O(new_n77_));
  aoi22  g26(.a(new_n77_), .b(new_n60_), .c(new_n76_), .d(x23), .O(new_n78_));
  inv1   g27(.a(x11), .O(new_n79_));
  aoi21  g28(.a(new_n52_), .b(new_n79_), .c(x18), .O(new_n80_));
  oai21  g29(.a(new_n78_), .b(new_n52_), .c(new_n80_), .O(z04));
  inv1   g30(.a(x23), .O(new_n82_));
  nand4  g31(.a(new_n71_), .b(new_n58_), .c(new_n82_), .d(new_n57_), .O(new_n83_));
  xor2a  g32(.a(new_n83_), .b(x24), .O(new_n84_));
  inv1   g33(.a(x10), .O(new_n85_));
  aoi21  g34(.a(new_n52_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g35(.a(new_n84_), .b(new_n52_), .c(new_n86_), .O(z05));
  inv1   g36(.a(x24), .O(new_n88_));
  nand3  g37(.a(new_n77_), .b(new_n60_), .c(new_n88_), .O(new_n89_));
  inv1   g38(.a(x25), .O(new_n90_));
  nand2  g39(.a(new_n83_), .b(new_n90_), .O(new_n91_));
  nand3  g40(.a(new_n91_), .b(new_n89_), .c(x16), .O(new_n92_));
  inv1   g41(.a(x09), .O(new_n93_));
  aoi21  g42(.a(new_n52_), .b(new_n93_), .c(x18), .O(new_n94_));
  nand2  g43(.a(new_n94_), .b(new_n92_), .O(z06));
  inv1   g44(.a(x18), .O(new_n96_));
  nand2  g45(.a(x26), .b(x16), .O(new_n97_));
  inv1   g46(.a(x08), .O(new_n98_));
  nand2  g47(.a(new_n52_), .b(new_n98_), .O(new_n99_));
  nand3  g48(.a(new_n99_), .b(new_n97_), .c(new_n96_), .O(z07));
  nand2  g49(.a(x27), .b(x16), .O(new_n101_));
  inv1   g50(.a(x07), .O(new_n102_));
  nand2  g51(.a(new_n52_), .b(new_n102_), .O(new_n103_));
  nand3  g52(.a(new_n103_), .b(new_n101_), .c(new_n96_), .O(z08));
  nand2  g53(.a(x28), .b(x16), .O(new_n105_));
  inv1   g54(.a(x06), .O(new_n106_));
  nand2  g55(.a(new_n52_), .b(new_n106_), .O(new_n107_));
  nand3  g56(.a(new_n107_), .b(new_n105_), .c(new_n96_), .O(z09));
  nand2  g57(.a(x29), .b(x16), .O(new_n109_));
  inv1   g58(.a(x05), .O(new_n110_));
  nand2  g59(.a(new_n52_), .b(new_n110_), .O(new_n111_));
  nand3  g60(.a(new_n111_), .b(new_n109_), .c(new_n96_), .O(z10));
  nand2  g61(.a(x30), .b(x16), .O(new_n113_));
  inv1   g62(.a(x04), .O(new_n114_));
  nand2  g63(.a(new_n52_), .b(new_n114_), .O(new_n115_));
  nand3  g64(.a(new_n115_), .b(new_n113_), .c(new_n96_), .O(z11));
  nand2  g65(.a(x31), .b(x16), .O(new_n117_));
  inv1   g66(.a(x03), .O(new_n118_));
  nand2  g67(.a(new_n52_), .b(new_n118_), .O(new_n119_));
  nand3  g68(.a(new_n119_), .b(new_n117_), .c(new_n96_), .O(z12));
  nand2  g69(.a(x32), .b(x16), .O(new_n121_));
  inv1   g70(.a(x02), .O(new_n122_));
  nand2  g71(.a(new_n52_), .b(new_n122_), .O(new_n123_));
  nand3  g72(.a(new_n123_), .b(new_n121_), .c(new_n96_), .O(z13));
  nand2  g73(.a(x33), .b(x16), .O(new_n125_));
  inv1   g74(.a(x01), .O(new_n126_));
  nand2  g75(.a(new_n52_), .b(new_n126_), .O(new_n127_));
  nand3  g76(.a(new_n127_), .b(new_n125_), .c(new_n96_), .O(z14));
  nand2  g77(.a(x34), .b(x16), .O(new_n129_));
  inv1   g78(.a(x00), .O(new_n130_));
  nand2  g79(.a(new_n52_), .b(new_n130_), .O(new_n131_));
  nand3  g80(.a(new_n131_), .b(new_n129_), .c(new_n96_), .O(z15));
endmodule


