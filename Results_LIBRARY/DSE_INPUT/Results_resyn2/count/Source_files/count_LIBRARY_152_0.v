// Benchmark "FAU" written by ABC on Mon Jul 27 22:45:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g008(.a(new_n58_), .b(new_n57_), .O(new_n60_));
  inv1   g009(.a(new_n60_), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n59_), .c(x16), .O(new_n62_));
  inv1   g011(.a(x14), .O(new_n63_));
  aoi21  g012(.a(new_n52_), .b(new_n63_), .c(x18), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n62_), .O(z01));
  nor2   g014(.a(x21), .b(x20), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n58_), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  aoi21  g017(.a(new_n60_), .b(x21), .c(new_n68_), .O(new_n69_));
  inv1   g018(.a(x13), .O(new_n70_));
  aoi21  g019(.a(new_n52_), .b(new_n70_), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n52_), .c(new_n71_), .O(z02));
  xor2a  g021(.a(new_n67_), .b(x22), .O(new_n73_));
  inv1   g022(.a(x12), .O(new_n74_));
  aoi21  g023(.a(new_n52_), .b(new_n74_), .c(x18), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(new_n52_), .c(new_n75_), .O(z03));
  inv1   g025(.a(x22), .O(new_n77_));
  nand2  g026(.a(new_n68_), .b(new_n77_), .O(new_n78_));
  nor2   g027(.a(x23), .b(x22), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n66_), .c(new_n58_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  aoi21  g030(.a(new_n78_), .b(x23), .c(new_n81_), .O(new_n82_));
  inv1   g031(.a(x11), .O(new_n83_));
  aoi21  g032(.a(new_n52_), .b(new_n83_), .c(x18), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n52_), .c(new_n84_), .O(z04));
  inv1   g034(.a(x24), .O(new_n86_));
  nor2   g035(.a(new_n81_), .b(new_n86_), .O(new_n87_));
  nand4  g036(.a(new_n79_), .b(new_n66_), .c(new_n58_), .d(new_n86_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n87_), .c(x16), .O(new_n90_));
  inv1   g039(.a(x10), .O(new_n91_));
  aoi21  g040(.a(new_n52_), .b(new_n91_), .c(x18), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n90_), .O(z05));
  inv1   g042(.a(x25), .O(new_n94_));
  nor2   g043(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  nor2   g044(.a(new_n88_), .b(x25), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n95_), .c(x16), .O(new_n97_));
  inv1   g046(.a(x09), .O(new_n98_));
  aoi21  g047(.a(new_n52_), .b(new_n98_), .c(x18), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n97_), .O(z06));
  inv1   g049(.a(x26), .O(new_n101_));
  nor2   g050(.a(new_n96_), .b(new_n101_), .O(new_n102_));
  nand3  g051(.a(new_n101_), .b(new_n94_), .c(new_n86_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n80_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n102_), .c(x16), .O(new_n105_));
  inv1   g054(.a(x08), .O(new_n106_));
  aoi21  g055(.a(new_n52_), .b(new_n106_), .c(x18), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n105_), .O(z07));
  inv1   g057(.a(x27), .O(new_n109_));
  xor2a  g058(.a(new_n104_), .b(new_n109_), .O(new_n110_));
  inv1   g059(.a(x07), .O(new_n111_));
  aoi21  g060(.a(new_n52_), .b(new_n111_), .c(x18), .O(new_n112_));
  oai21  g061(.a(new_n110_), .b(new_n52_), .c(new_n112_), .O(z08));
  inv1   g062(.a(x28), .O(new_n114_));
  aoi21  g063(.a(new_n104_), .b(new_n109_), .c(new_n114_), .O(new_n115_));
  nor2   g064(.a(x28), .b(x27), .O(new_n116_));
  inv1   g065(.a(new_n116_), .O(new_n117_));
  nor3   g066(.a(new_n117_), .b(new_n103_), .c(new_n80_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n115_), .c(x16), .O(new_n119_));
  inv1   g068(.a(x06), .O(new_n120_));
  aoi21  g069(.a(new_n52_), .b(new_n120_), .c(x18), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n119_), .O(z09));
  inv1   g071(.a(x29), .O(new_n123_));
  xor2a  g072(.a(new_n118_), .b(new_n123_), .O(new_n124_));
  inv1   g073(.a(x05), .O(new_n125_));
  aoi21  g074(.a(new_n52_), .b(new_n125_), .c(x18), .O(new_n126_));
  oai21  g075(.a(new_n124_), .b(new_n52_), .c(new_n126_), .O(z10));
  inv1   g076(.a(new_n103_), .O(new_n128_));
  nand4  g077(.a(new_n116_), .b(new_n128_), .c(new_n81_), .d(new_n123_), .O(new_n129_));
  nor2   g078(.a(x30), .b(x29), .O(new_n130_));
  aoi22  g079(.a(new_n130_), .b(new_n118_), .c(new_n129_), .d(x30), .O(new_n131_));
  inv1   g080(.a(x04), .O(new_n132_));
  aoi21  g081(.a(new_n52_), .b(new_n132_), .c(x18), .O(new_n133_));
  oai21  g082(.a(new_n131_), .b(new_n52_), .c(new_n133_), .O(z11));
  nand3  g083(.a(new_n130_), .b(new_n116_), .c(new_n104_), .O(new_n135_));
  inv1   g084(.a(x31), .O(new_n136_));
  nand3  g085(.a(new_n130_), .b(new_n116_), .c(new_n136_), .O(new_n137_));
  nor3   g086(.a(new_n137_), .b(new_n103_), .c(new_n80_), .O(new_n138_));
  aoi21  g087(.a(new_n135_), .b(x31), .c(new_n138_), .O(new_n139_));
  inv1   g088(.a(x03), .O(new_n140_));
  aoi21  g089(.a(new_n52_), .b(new_n140_), .c(x18), .O(new_n141_));
  oai21  g090(.a(new_n139_), .b(new_n52_), .c(new_n141_), .O(z12));
  inv1   g091(.a(new_n137_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n104_), .O(new_n144_));
  nand2  g093(.a(new_n101_), .b(new_n94_), .O(new_n145_));
  nor3   g094(.a(new_n137_), .b(new_n145_), .c(x32), .O(new_n146_));
  aoi22  g095(.a(new_n146_), .b(new_n89_), .c(new_n144_), .d(x32), .O(new_n147_));
  inv1   g096(.a(x02), .O(new_n148_));
  aoi21  g097(.a(new_n52_), .b(new_n148_), .c(x18), .O(new_n149_));
  oai21  g098(.a(new_n147_), .b(new_n52_), .c(new_n149_), .O(z13));
  nor2   g099(.a(x24), .b(x21), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n79_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n60_), .O(new_n153_));
  and2   g102(.a(new_n153_), .b(new_n146_), .O(new_n154_));
  oai21  g103(.a(new_n61_), .b(x33), .c(x16), .O(new_n155_));
  inv1   g104(.a(x01), .O(new_n156_));
  aoi21  g105(.a(new_n52_), .b(new_n156_), .c(x18), .O(new_n157_));
  oai21  g106(.a(new_n155_), .b(new_n154_), .c(new_n157_), .O(z14));
  nand2  g107(.a(x34), .b(x16), .O(new_n159_));
  inv1   g108(.a(x00), .O(new_n160_));
  aoi21  g109(.a(new_n52_), .b(new_n160_), .c(x18), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n159_), .O(z15));
endmodule


