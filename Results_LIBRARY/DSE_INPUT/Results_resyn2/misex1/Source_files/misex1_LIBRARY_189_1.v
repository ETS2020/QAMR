// Benchmark "FAU" written by ABC on Mon Jul 27 18:27:40 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x1), .b(new_n16_), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x3), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n16_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n18_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  inv1   g08(.a(x2), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x3), .c(new_n23_), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n19_), .c(new_n24_), .O(new_n28_));
  aoi22  g13(.a(new_n28_), .b(new_n23_), .c(new_n26_), .d(new_n24_), .O(new_n29_));
  nand4  g14(.a(x3), .b(new_n24_), .c(new_n23_), .d(x0), .O(new_n30_));
  oai21  g15(.a(new_n29_), .b(x0), .c(new_n30_), .O(z1));
  aoi21  g16(.a(x6), .b(new_n24_), .c(x3), .O(new_n32_));
  nand2  g17(.a(x5), .b(x2), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n32_), .c(x1), .O(new_n34_));
  inv1   g19(.a(x4), .O(new_n35_));
  nor2   g20(.a(new_n35_), .b(x2), .O(new_n36_));
  nand2  g21(.a(new_n19_), .b(x1), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n36_), .c(new_n16_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n34_), .c(new_n30_), .O(z2));
  nor2   g24(.a(x7), .b(x2), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(x1), .c(new_n16_), .O(new_n41_));
  nor2   g26(.a(new_n27_), .b(x0), .O(new_n42_));
  nand3  g27(.a(new_n19_), .b(x2), .c(new_n23_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(z3));
  oai21  g29(.a(new_n40_), .b(x3), .c(x1), .O(new_n45_));
  nand3  g30(.a(x6), .b(new_n19_), .c(new_n24_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n16_), .O(new_n48_));
  nand2  g33(.a(x6), .b(new_n24_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n19_), .c(new_n16_), .O(new_n50_));
  xor2a  g35(.a(x3), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x0), .O(new_n52_));
  nand2  g37(.a(new_n27_), .b(new_n19_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(x2), .c(new_n16_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n52_), .c(new_n50_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n23_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n48_), .O(z4));
  nand2  g42(.a(new_n54_), .b(new_n52_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n23_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n48_), .O(z5));
  nor2   g45(.a(x5), .b(x3), .O(new_n61_));
  aoi22  g46(.a(new_n51_), .b(x0), .c(new_n61_), .d(x2), .O(new_n62_));
  aoi21  g47(.a(new_n35_), .b(new_n24_), .c(x3), .O(new_n63_));
  oai22  g48(.a(new_n63_), .b(new_n17_), .c(new_n62_), .d(x1), .O(z6));
endmodule


