// Benchmark "FAU" written by ABC on Mon Jul 27 19:17:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n68_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x20), .O(new_n48_));
  inv1   g03(.a(x17), .O(new_n49_));
  inv1   g04(.a(x18), .O(new_n50_));
  inv1   g05(.a(x19), .O(new_n51_));
  nand3  g06(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nand3  g07(.a(x24), .b(x22), .c(x21), .O(new_n53_));
  aoi21  g08(.a(new_n52_), .b(new_n48_), .c(new_n53_), .O(new_n54_));
  nand2  g09(.a(x05), .b(x04), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(x07), .O(new_n57_));
  oai21  g12(.a(new_n54_), .b(x25), .c(new_n57_), .O(z01));
  nor2   g13(.a(new_n54_), .b(x25), .O(z03));
  xor2a  g14(.a(x12), .b(x03), .O(new_n60_));
  xor2a  g15(.a(x11), .b(x02), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  xor2a  g17(.a(x09), .b(x00), .O(new_n63_));
  xor2a  g18(.a(x10), .b(x01), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n62_), .c(x08), .O(z04));
  nor2   g21(.a(x13), .b(x08), .O(z05));
  inv1   g22(.a(x14), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(x08), .O(z06));
  nand2  g24(.a(new_n46_), .b(x06), .O(z07));
  inv1   g25(.a(x25), .O(new_n71_));
  nand2  g26(.a(new_n52_), .b(new_n48_), .O(new_n72_));
  inv1   g27(.a(new_n53_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g29(.a(x19), .b(x18), .c(x17), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(new_n76_));
  nor2   g31(.a(x22), .b(x21), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(x23), .d(new_n48_), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n74_), .c(new_n71_), .O(z08));
  inv1   g34(.a(x07), .O(new_n80_));
  inv1   g35(.a(x15), .O(new_n81_));
  nand3  g36(.a(new_n56_), .b(new_n81_), .c(new_n80_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  nand3  g38(.a(new_n55_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n49_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z10));
  nor2   g42(.a(x18), .b(x17), .O(new_n88_));
  nor2   g43(.a(new_n50_), .b(new_n49_), .O(new_n89_));
  nor3   g44(.a(new_n84_), .b(new_n89_), .c(new_n88_), .O(z11));
  nor2   g45(.a(new_n89_), .b(x19), .O(new_n91_));
  nand2  g46(.a(new_n85_), .b(new_n75_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(new_n91_), .O(z12));
  nand2  g48(.a(new_n75_), .b(x20), .O(new_n94_));
  nand4  g49(.a(new_n48_), .b(x19), .c(x18), .d(x17), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n85_), .O(z13));
  aoi21  g51(.a(new_n95_), .b(x21), .c(new_n84_), .O(new_n97_));
  oai21  g52(.a(new_n95_), .b(x21), .c(new_n97_), .O(z14));
  oai21  g53(.a(new_n95_), .b(x21), .c(x22), .O(new_n99_));
  inv1   g54(.a(new_n95_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n77_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n99_), .c(new_n85_), .O(z15));
  inv1   g57(.a(x23), .O(new_n103_));
  nand4  g58(.a(new_n77_), .b(new_n76_), .c(new_n103_), .d(new_n48_), .O(new_n104_));
  inv1   g59(.a(new_n77_), .O(new_n105_));
  oai21  g60(.a(new_n95_), .b(new_n105_), .c(x23), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n85_), .O(z16));
  nand2  g62(.a(new_n104_), .b(x24), .O(new_n108_));
  aoi21  g63(.a(x25), .b(x23), .c(x24), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n100_), .c(new_n77_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n85_), .O(z17));
  inv1   g66(.a(x24), .O(new_n112_));
  nand4  g67(.a(new_n77_), .b(new_n76_), .c(new_n112_), .d(new_n48_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x25), .O(new_n114_));
  oai21  g69(.a(x25), .b(x24), .c(new_n103_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n100_), .c(new_n77_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n85_), .O(z18));
  buf    g72(.a(x16), .O(z02));
endmodule


