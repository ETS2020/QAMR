// Benchmark "FAU" written by ABC on Mon Jul 27 19:16:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n66_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n100_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x07), .O(new_n48_));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x23), .O(new_n50_));
  nand2  g05(.a(x22), .b(x21), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(x24), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(z08));
  nand2  g09(.a(x05), .b(x04), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(z08), .c(new_n48_), .O(z01));
  inv1   g11(.a(z08), .O(z03));
  xor2a  g12(.a(x11), .b(x02), .O(new_n58_));
  xor2a  g13(.a(x12), .b(x03), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  xor2a  g15(.a(x09), .b(x00), .O(new_n61_));
  xor2a  g16(.a(x10), .b(x01), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n60_), .c(x08), .O(z04));
  nor2   g19(.a(x13), .b(x08), .O(z05));
  inv1   g20(.a(x14), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(x08), .O(z06));
  nand2  g22(.a(new_n46_), .b(x06), .O(z07));
  nor2   g23(.a(x15), .b(x07), .O(new_n69_));
  nand3  g24(.a(new_n69_), .b(x05), .c(x04), .O(new_n70_));
  inv1   g25(.a(new_n70_), .O(z09));
  inv1   g26(.a(x17), .O(new_n72_));
  nand2  g27(.a(new_n69_), .b(new_n55_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z10));
  and2   g31(.a(x18), .b(x17), .O(new_n77_));
  nor2   g32(.a(x18), .b(x17), .O(new_n78_));
  nor3   g33(.a(new_n78_), .b(new_n77_), .c(new_n73_), .O(z11));
  nor2   g34(.a(new_n77_), .b(x19), .O(new_n80_));
  nand3  g35(.a(x19), .b(x18), .c(x17), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n69_), .c(new_n55_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n80_), .O(z12));
  nand2  g38(.a(new_n81_), .b(x20), .O(new_n84_));
  inv1   g39(.a(x20), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(x19), .c(x18), .d(x17), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n84_), .c(new_n74_), .O(z13));
  nand2  g42(.a(new_n86_), .b(x21), .O(new_n88_));
  inv1   g43(.a(x21), .O(new_n89_));
  inv1   g44(.a(new_n86_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n88_), .c(new_n74_), .O(z14));
  oai21  g47(.a(new_n86_), .b(x21), .c(x22), .O(new_n93_));
  nor2   g48(.a(x22), .b(x21), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n77_), .c(new_n85_), .d(x19), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n93_), .c(new_n74_), .O(z15));
  nand2  g51(.a(new_n86_), .b(new_n50_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n74_), .O(z16));
  inv1   g54(.a(x24), .O(new_n100_));
  nand2  g55(.a(new_n74_), .b(new_n100_), .O(z17));
  nand2  g56(.a(new_n74_), .b(new_n49_), .O(z18));
  buf    g57(.a(x16), .O(z02));
endmodule


