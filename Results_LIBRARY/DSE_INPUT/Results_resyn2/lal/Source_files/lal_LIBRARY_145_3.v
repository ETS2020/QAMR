// Benchmark "FAU" written by ABC on Mon Jul 27 19:16:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n72_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x25), .O(new_n48_));
  inv1   g03(.a(x23), .O(new_n49_));
  nand2  g04(.a(x22), .b(x21), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(x24), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  nor2   g08(.a(x19), .b(x18), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  nand2  g10(.a(x21), .b(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n54_), .b(new_n53_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n52_), .c(new_n48_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(x07), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n58_), .O(z01));
  inv1   g17(.a(new_n58_), .O(z03));
  xor2a  g18(.a(x12), .b(x03), .O(new_n64_));
  xor2a  g19(.a(x11), .b(x02), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  xor2a  g21(.a(x09), .b(x00), .O(new_n67_));
  xor2a  g22(.a(x10), .b(x01), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n66_), .c(x08), .O(z04));
  nor2   g25(.a(x13), .b(x08), .O(z05));
  inv1   g26(.a(x14), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(x08), .O(z06));
  nand2  g28(.a(new_n46_), .b(x06), .O(z07));
  nand2  g29(.a(new_n49_), .b(new_n55_), .O(new_n75_));
  nand3  g30(.a(x19), .b(x18), .c(x17), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  inv1   g32(.a(x24), .O(new_n78_));
  nor2   g33(.a(x22), .b(x21), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n78_), .c(x19), .d(x18), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n77_), .c(x25), .O(new_n81_));
  inv1   g36(.a(new_n54_), .O(new_n82_));
  nand3  g37(.a(x24), .b(x22), .c(x21), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n82_), .c(new_n48_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n53_), .O(new_n85_));
  nand2  g40(.a(new_n52_), .b(new_n48_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n75_), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n85_), .c(new_n81_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  inv1   g44(.a(x15), .O(new_n90_));
  nand3  g45(.a(new_n60_), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z09));
  nand3  g47(.a(new_n59_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n53_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z10));
  and2   g51(.a(x18), .b(x17), .O(new_n97_));
  nor2   g52(.a(x18), .b(x17), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n93_), .c(new_n97_), .O(z11));
  inv1   g54(.a(new_n76_), .O(new_n100_));
  nor2   g55(.a(new_n97_), .b(x19), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n93_), .c(new_n100_), .O(z12));
  nand2  g57(.a(new_n76_), .b(x20), .O(new_n103_));
  inv1   g58(.a(x19), .O(new_n104_));
  nor2   g59(.a(x20), .b(new_n104_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n97_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n103_), .c(new_n94_), .O(z13));
  nand2  g62(.a(new_n106_), .b(x21), .O(new_n108_));
  inv1   g63(.a(x21), .O(new_n109_));
  nand3  g64(.a(new_n105_), .b(new_n97_), .c(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n94_), .O(z14));
  inv1   g66(.a(x22), .O(new_n112_));
  nand2  g67(.a(new_n49_), .b(x21), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n105_), .c(new_n97_), .d(new_n112_), .O(new_n114_));
  nand2  g69(.a(new_n110_), .b(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n94_), .O(z15));
  oai21  g71(.a(new_n106_), .b(x22), .c(x23), .O(new_n117_));
  inv1   g72(.a(new_n75_), .O(new_n118_));
  nand3  g73(.a(new_n79_), .b(new_n100_), .c(new_n118_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n94_), .O(z16));
  nand2  g75(.a(new_n119_), .b(x24), .O(new_n121_));
  nor3   g76(.a(x24), .b(x22), .c(x21), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n100_), .c(new_n118_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n94_), .O(z17));
  nand3  g79(.a(new_n122_), .b(new_n77_), .c(new_n48_), .O(new_n125_));
  nand2  g80(.a(new_n123_), .b(x25), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n94_), .O(z18));
  buf    g82(.a(x16), .O(z02));
endmodule


