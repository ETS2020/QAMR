// Benchmark "FAU" written by ABC on Mon Jul 27 19:16:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n70_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n100_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n117_, new_n119_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x23), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  oai21  g05(.a(x19), .b(x18), .c(new_n50_), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  aoi21  g08(.a(new_n52_), .b(x17), .c(new_n53_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n48_), .c(new_n49_), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(x07), .O(new_n59_));
  oai21  g14(.a(new_n56_), .b(x25), .c(new_n59_), .O(z01));
  nor2   g15(.a(new_n56_), .b(x25), .O(z03));
  xor2a  g16(.a(x12), .b(x03), .O(new_n62_));
  xor2a  g17(.a(x11), .b(x02), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  xor2a  g19(.a(x09), .b(x00), .O(new_n65_));
  xor2a  g20(.a(x10), .b(x01), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(new_n64_), .c(x08), .O(z04));
  nor2   g23(.a(x13), .b(x08), .O(z05));
  inv1   g24(.a(x14), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(x08), .O(z06));
  nand2  g26(.a(new_n46_), .b(x06), .O(z07));
  nor3   g27(.a(x24), .b(x22), .c(x21), .O(new_n73_));
  nor2   g28(.a(x23), .b(x20), .O(new_n74_));
  nand3  g29(.a(x19), .b(x18), .c(x17), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n74_), .c(new_n73_), .O(new_n77_));
  inv1   g32(.a(x19), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(new_n52_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n77_), .c(new_n73_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x25), .O(new_n81_));
  inv1   g36(.a(x17), .O(new_n82_));
  nand3  g37(.a(x24), .b(new_n78_), .c(new_n52_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n53_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x25), .c(new_n82_), .O(new_n85_));
  aoi21  g40(.a(new_n53_), .b(new_n48_), .c(new_n49_), .O(new_n86_));
  oai22  g41(.a(new_n86_), .b(x25), .c(x23), .d(x20), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n85_), .c(new_n81_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  inv1   g44(.a(x15), .O(new_n90_));
  nand3  g45(.a(new_n58_), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z09));
  nand3  g47(.a(new_n57_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n82_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z10));
  nand2  g51(.a(new_n52_), .b(x17), .O(new_n97_));
  nand2  g52(.a(x18), .b(new_n82_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(new_n97_), .c(new_n93_), .O(z11));
  aoi21  g54(.a(x18), .b(x17), .c(x19), .O(new_n100_));
  nor3   g55(.a(new_n100_), .b(new_n93_), .c(new_n76_), .O(z12));
  nor2   g56(.a(new_n75_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  oai21  g58(.a(new_n78_), .b(new_n82_), .c(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n94_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  inv1   g61(.a(x21), .O(new_n107_));
  nand2  g62(.a(new_n102_), .b(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n94_), .O(z14));
  nand2  g64(.a(new_n108_), .b(x22), .O(new_n110_));
  nor2   g65(.a(x22), .b(x21), .O(new_n111_));
  nand2  g66(.a(new_n102_), .b(new_n111_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n94_), .O(z15));
  nand2  g68(.a(new_n112_), .b(x23), .O(new_n114_));
  nand3  g69(.a(new_n102_), .b(new_n111_), .c(new_n48_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n94_), .O(z16));
  nand2  g71(.a(new_n115_), .b(x24), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n94_), .c(new_n77_), .O(z17));
  aoi21  g73(.a(new_n77_), .b(x25), .c(new_n93_), .O(new_n119_));
  oai21  g74(.a(new_n77_), .b(x25), .c(new_n119_), .O(z18));
  buf    g75(.a(x16), .O(z02));
endmodule


