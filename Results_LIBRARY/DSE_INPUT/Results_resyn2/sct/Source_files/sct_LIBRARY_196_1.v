// Benchmark "FAU" written by ABC on Mon Jul 27 17:50:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_;
  inv1   g00(.a(x14), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x01), .O(new_n36_));
  aoi21  g02(.a(x02), .b(x01), .c(new_n36_), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(x05), .b(new_n40_), .O(new_n41_));
  oai21  g07(.a(new_n39_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g08(.a(x06), .O(new_n43_));
  inv1   g09(.a(x03), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(x02), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x16), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n43_), .c(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  nand2  g14(.a(x07), .b(x06), .O(new_n49_));
  inv1   g15(.a(x07), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  nand4  g17(.a(new_n51_), .b(new_n49_), .c(new_n46_), .d(x04), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z03));
  nand3  g19(.a(x08), .b(x07), .c(x06), .O(new_n54_));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(new_n49_), .b(new_n55_), .O(new_n56_));
  nand4  g22(.a(new_n56_), .b(new_n54_), .c(new_n46_), .d(x04), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z04));
  inv1   g24(.a(x16), .O(new_n59_));
  aoi21  g25(.a(new_n44_), .b(x02), .c(new_n59_), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n40_), .O(new_n61_));
  nand2  g27(.a(new_n54_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  inv1   g29(.a(new_n54_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n62_), .c(new_n61_), .O(z05));
  nand2  g32(.a(new_n65_), .b(x10), .O(new_n67_));
  nor2   g33(.a(x10), .b(x09), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n64_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(new_n61_), .O(z06));
  inv1   g36(.a(x11), .O(new_n71_));
  nand3  g37(.a(new_n68_), .b(new_n64_), .c(new_n71_), .O(new_n72_));
  nand2  g38(.a(new_n69_), .b(x11), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n61_), .O(z07));
  nand2  g40(.a(new_n72_), .b(x12), .O(new_n75_));
  nor2   g41(.a(x12), .b(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n68_), .c(new_n64_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(new_n61_), .O(z08));
  nand2  g44(.a(new_n77_), .b(x13), .O(new_n79_));
  nor2   g45(.a(new_n54_), .b(x09), .O(new_n80_));
  nor2   g46(.a(x13), .b(x10), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n76_), .c(new_n80_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n79_), .c(new_n61_), .O(z09));
  nor2   g49(.a(new_n80_), .b(new_n60_), .O(new_n84_));
  nand3  g50(.a(new_n45_), .b(x16), .c(new_n35_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x04), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(new_n84_), .O(z10));
  inv1   g53(.a(x02), .O(new_n88_));
  nand2  g54(.a(x16), .b(new_n88_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n44_), .c(new_n40_), .O(z12));
  and2   g56(.a(x17), .b(x04), .O(z14));
  buf    g57(.a(x02), .O(z11));
  buf    g58(.a(x04), .O(z13));
endmodule


