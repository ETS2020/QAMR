// Benchmark "FAU" written by ABC on Mon Jul 27 23:00:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n153_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n161_, new_n162_, new_n163_;
  inv1   g000(.a(x15), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand2  g003(.a(x39), .b(new_n65_), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n66_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  nand3  g007(.a(x39), .b(x10), .c(new_n65_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g009(.a(x40), .O(new_n72_));
  nand2  g010(.a(new_n69_), .b(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(new_n71_), .c(new_n63_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(x16), .O(z00));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x36), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n80_), .c(x04), .O(z01));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(new_n79_), .O(new_n86_));
  inv1   g024(.a(x39), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor3   g026(.a(new_n88_), .b(new_n72_), .c(new_n87_), .O(new_n89_));
  oai21  g027(.a(new_n86_), .b(new_n65_), .c(new_n89_), .O(z02));
  inv1   g028(.a(x35), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n78_), .c(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n76_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g036(.a(z06), .O(z05));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z07));
  nor2   g048(.a(new_n72_), .b(new_n87_), .O(z08));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z09));
  nand3  g051(.a(z08), .b(new_n83_), .c(x05), .O(new_n114_));
  inv1   g052(.a(x05), .O(new_n115_));
  nand2  g053(.a(x39), .b(new_n115_), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  nand2  g055(.a(new_n87_), .b(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n116_), .c(new_n80_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n114_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n65_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(z10));
  aoi21  g061(.a(x39), .b(x29), .c(new_n76_), .O(new_n124_));
  nand2  g062(.a(z08), .b(x29), .O(new_n125_));
  oai21  g063(.a(new_n124_), .b(new_n79_), .c(new_n125_), .O(new_n126_));
  inv1   g064(.a(x09), .O(new_n127_));
  nand4  g065(.a(x35), .b(new_n78_), .c(x27), .d(new_n65_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n81_), .c(new_n127_), .O(new_n129_));
  aoi21  g067(.a(new_n126_), .b(x08), .c(new_n129_), .O(z11));
  nand2  g068(.a(new_n71_), .b(x40), .O(z12));
  nand2  g069(.a(z08), .b(new_n83_), .O(new_n132_));
  inv1   g070(.a(x13), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n65_), .O(new_n134_));
  inv1   g072(.a(x18), .O(new_n135_));
  inv1   g073(.a(x19), .O(new_n136_));
  nand3  g074(.a(x20), .b(new_n136_), .c(new_n135_), .O(new_n137_));
  nor2   g075(.a(new_n137_), .b(new_n67_), .O(new_n138_));
  aoi21  g076(.a(x36), .b(x35), .c(new_n78_), .O(new_n139_));
  nand3  g077(.a(x39), .b(new_n133_), .c(new_n65_), .O(new_n140_));
  nor2   g078(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n138_), .c(x27), .O(new_n142_));
  oai21  g080(.a(new_n134_), .b(new_n132_), .c(new_n142_), .O(z13));
  inv1   g081(.a(new_n137_), .O(new_n144_));
  nand2  g082(.a(new_n93_), .b(new_n91_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n139_), .c(new_n76_), .O(new_n147_));
  nand2  g085(.a(new_n138_), .b(x27), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n140_), .c(new_n72_), .O(new_n149_));
  oai21  g087(.a(new_n147_), .b(new_n83_), .c(new_n149_), .O(z14));
  nand4  g088(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(z15));
  nand2  g090(.a(x22), .b(x01), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(x23), .O(z16));
  inv1   g092(.a(x24), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x23), .c(x22), .d(x01), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(z17));
  oai21  g095(.a(new_n79_), .b(x27), .c(new_n125_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x08), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n128_), .c(new_n81_), .d(new_n127_), .O(z18));
  nand2  g098(.a(x35), .b(x27), .O(new_n161_));
  nand3  g099(.a(x39), .b(x29), .c(x08), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n161_), .c(new_n72_), .O(new_n163_));
  nor4   g101(.a(new_n163_), .b(new_n86_), .c(x30), .d(x09), .O(z20));
  aoi21  g102(.a(new_n126_), .b(x08), .c(new_n129_), .O(z19));
endmodule


