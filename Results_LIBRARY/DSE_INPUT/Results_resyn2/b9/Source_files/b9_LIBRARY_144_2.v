// Benchmark "FAU" written by ABC on Mon Jul 27 23:00:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n161_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n170_, new_n171_, new_n172_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  aoi21  g002(.a(x35), .b(x28), .c(x37), .O(new_n65_));
  or2    g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g004(.a(x40), .b(x10), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  inv1   g006(.a(x36), .O(new_n69_));
  inv1   g007(.a(x40), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(x27), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(new_n67_), .c(new_n66_), .O(new_n73_));
  inv1   g011(.a(x39), .O(new_n74_));
  oai21  g012(.a(new_n74_), .b(x04), .c(new_n66_), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(new_n73_), .c(new_n63_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x16), .O(z00));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n69_), .c(new_n64_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n80_), .c(x04), .O(z01));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n79_), .c(x04), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nand2  g025(.a(x40), .b(x39), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n86_), .O(z02));
  oai21  g028(.a(new_n68_), .b(new_n78_), .c(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n64_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n91_), .c(new_n95_), .O(z04));
  aoi21  g034(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g035(.a(z06), .O(z05));
  inv1   g036(.a(x25), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(x00), .O(new_n100_));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x25), .b(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n100_), .c(x38), .O(new_n103_));
  inv1   g041(.a(x17), .O(new_n104_));
  nor2   g042(.a(new_n104_), .b(x15), .O(new_n105_));
  or2    g043(.a(x33), .b(x31), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(new_n105_), .c(x14), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n103_), .c(x03), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(z07));
  and2   g047(.a(x40), .b(x39), .O(z08));
  nand4  g048(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z09));
  nor2   g050(.a(new_n92_), .b(new_n64_), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x06), .O(new_n114_));
  nand2  g052(.a(z08), .b(new_n83_), .O(new_n115_));
  aoi22  g053(.a(x40), .b(x36), .c(x35), .d(new_n78_), .O(new_n116_));
  nand2  g054(.a(x39), .b(x27), .O(new_n117_));
  oai21  g055(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(new_n118_));
  nand3  g056(.a(new_n74_), .b(x35), .c(new_n78_), .O(new_n119_));
  nand3  g057(.a(new_n88_), .b(x27), .c(x07), .O(new_n120_));
  aoi21  g058(.a(new_n119_), .b(new_n69_), .c(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n118_), .b(x05), .c(new_n121_), .O(new_n122_));
  oai21  g060(.a(new_n122_), .b(x04), .c(new_n114_), .O(z10));
  nor2   g061(.a(x30), .b(x09), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nand2  g063(.a(x39), .b(x29), .O(new_n126_));
  nand3  g064(.a(x35), .b(new_n78_), .c(new_n64_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x08), .O(new_n129_));
  inv1   g067(.a(x04), .O(new_n130_));
  nand4  g068(.a(x35), .b(new_n78_), .c(x27), .d(new_n130_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(x40), .c(new_n125_), .O(z11));
  inv1   g071(.a(x10), .O(new_n134_));
  oai21  g072(.a(x36), .b(x35), .c(x27), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n134_), .c(new_n70_), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n113_), .c(new_n75_), .O(z12));
  inv1   g075(.a(x13), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n130_), .O(new_n139_));
  inv1   g077(.a(x18), .O(new_n140_));
  inv1   g078(.a(x19), .O(new_n141_));
  nand3  g079(.a(x20), .b(new_n141_), .c(new_n140_), .O(new_n142_));
  nor2   g080(.a(new_n142_), .b(new_n65_), .O(new_n143_));
  nand3  g081(.a(x39), .b(new_n138_), .c(new_n130_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n143_), .c(x27), .O(new_n146_));
  oai22  g084(.a(new_n70_), .b(x28), .c(new_n69_), .d(new_n68_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  oai22  g086(.a(new_n148_), .b(new_n146_), .c(new_n139_), .d(new_n115_), .O(z13));
  oai21  g087(.a(new_n139_), .b(new_n88_), .c(new_n146_), .O(new_n150_));
  inv1   g088(.a(new_n83_), .O(new_n151_));
  nand3  g089(.a(new_n142_), .b(new_n69_), .c(x28), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  nand2  g092(.a(new_n68_), .b(x28), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n83_), .c(x40), .O(new_n156_));
  oai21  g094(.a(new_n142_), .b(new_n92_), .c(new_n156_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n154_), .c(new_n150_), .O(z14));
  nand4  g096(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(z15));
  nand2  g098(.a(x22), .b(x01), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(x23), .O(z16));
  inv1   g100(.a(x24), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x23), .c(x22), .d(x01), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z17));
  inv1   g103(.a(x29), .O(new_n166_));
  oai21  g104(.a(new_n88_), .b(new_n166_), .c(new_n127_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x08), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n131_), .c(new_n124_), .O(z18));
  inv1   g107(.a(x08), .O(new_n170_));
  aoi21  g108(.a(new_n78_), .b(x08), .c(x27), .O(new_n171_));
  oai22  g109(.a(new_n171_), .b(new_n68_), .c(new_n126_), .d(new_n170_), .O(new_n172_));
  aoi21  g110(.a(new_n172_), .b(x40), .c(new_n125_), .O(z20));
  aoi21  g111(.a(new_n132_), .b(x40), .c(new_n125_), .O(z19));
endmodule


