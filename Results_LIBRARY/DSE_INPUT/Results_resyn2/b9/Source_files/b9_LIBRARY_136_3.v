// Benchmark "FAU" written by ABC on Mon Jul 27 23:00:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n91_, new_n93_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n144_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_;
  inv1   g00(.a(x37), .O(new_n63_));
  nand2  g01(.a(x35), .b(x28), .O(new_n64_));
  nand2  g02(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g03(.a(x10), .O(new_n66_));
  nor2   g04(.a(new_n66_), .b(x04), .O(new_n67_));
  nand2  g05(.a(x40), .b(x39), .O(new_n68_));
  inv1   g06(.a(new_n68_), .O(z08));
  aoi22  g07(.a(z08), .b(new_n67_), .c(new_n65_), .d(x27), .O(new_n70_));
  oai21  g08(.a(new_n70_), .b(x15), .c(x16), .O(z00));
  inv1   g09(.a(x35), .O(new_n72_));
  inv1   g10(.a(x36), .O(new_n73_));
  oai21  g11(.a(new_n72_), .b(x28), .c(new_n73_), .O(new_n74_));
  and2   g12(.a(new_n74_), .b(x27), .O(new_n75_));
  inv1   g13(.a(x30), .O(new_n76_));
  inv1   g14(.a(x32), .O(new_n77_));
  nand2  g15(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  oai21  g16(.a(new_n78_), .b(new_n75_), .c(x04), .O(z01));
  inv1   g17(.a(x27), .O(new_n80_));
  nor2   g18(.a(new_n72_), .b(x28), .O(new_n81_));
  nand2  g19(.a(new_n81_), .b(x08), .O(new_n82_));
  nand2  g20(.a(new_n82_), .b(x04), .O(new_n83_));
  inv1   g21(.a(x02), .O(new_n84_));
  nand2  g22(.a(x29), .b(x08), .O(new_n85_));
  nand2  g23(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g24(.a(new_n86_), .b(new_n83_), .c(z08), .d(new_n80_), .O(z02));
  nand2  g25(.a(new_n64_), .b(x27), .O(new_n88_));
  nand2  g26(.a(new_n63_), .b(new_n80_), .O(new_n89_));
  nand3  g27(.a(new_n89_), .b(new_n88_), .c(x21), .O(z03));
  inv1   g28(.a(x21), .O(new_n91_));
  nand3  g29(.a(new_n89_), .b(new_n88_), .c(new_n91_), .O(z04));
  inv1   g30(.a(x28), .O(new_n93_));
  oai21  g31(.a(new_n93_), .b(new_n80_), .c(new_n63_), .O(z05));
  inv1   g32(.a(x40), .O(new_n95_));
  nand2  g33(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  aoi21  g34(.a(new_n96_), .b(x27), .c(x37), .O(z06));
  inv1   g35(.a(x25), .O(new_n98_));
  nand2  g36(.a(new_n98_), .b(x00), .O(new_n99_));
  inv1   g37(.a(x14), .O(new_n100_));
  nand2  g38(.a(x25), .b(new_n100_), .O(new_n101_));
  nand3  g39(.a(new_n101_), .b(new_n99_), .c(x38), .O(new_n102_));
  inv1   g40(.a(x17), .O(new_n103_));
  nor2   g41(.a(new_n103_), .b(x15), .O(new_n104_));
  or2    g42(.a(x33), .b(x31), .O(new_n105_));
  oai21  g43(.a(new_n105_), .b(new_n104_), .c(x14), .O(new_n106_));
  nand3  g44(.a(new_n106_), .b(new_n102_), .c(x03), .O(new_n107_));
  inv1   g45(.a(new_n107_), .O(z07));
  nand4  g46(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n109_));
  inv1   g47(.a(new_n109_), .O(z09));
  inv1   g48(.a(x04), .O(new_n111_));
  nand3  g49(.a(new_n78_), .b(z08), .c(x05), .O(new_n112_));
  nand3  g50(.a(new_n74_), .b(x27), .c(x07), .O(new_n113_));
  nand2  g51(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g52(.a(new_n114_), .b(new_n111_), .O(new_n115_));
  nand3  g53(.a(x37), .b(x27), .c(x06), .O(new_n116_));
  nand2  g54(.a(new_n116_), .b(new_n115_), .O(z10));
  oai21  g55(.a(x28), .b(x04), .c(x27), .O(new_n118_));
  inv1   g56(.a(x08), .O(new_n119_));
  oai21  g57(.a(new_n93_), .b(x27), .c(x35), .O(new_n120_));
  nand2  g58(.a(z08), .b(x29), .O(new_n121_));
  aoi22  g59(.a(new_n121_), .b(new_n120_), .c(new_n80_), .d(new_n119_), .O(new_n122_));
  inv1   g60(.a(x09), .O(new_n123_));
  nand2  g61(.a(new_n76_), .b(new_n123_), .O(new_n124_));
  aoi21  g62(.a(new_n122_), .b(new_n118_), .c(new_n124_), .O(z11));
  oai21  g63(.a(x35), .b(new_n80_), .c(new_n96_), .O(new_n126_));
  nand2  g64(.a(new_n126_), .b(new_n63_), .O(new_n127_));
  nor2   g65(.a(new_n67_), .b(new_n95_), .O(new_n128_));
  aoi21  g66(.a(new_n68_), .b(new_n80_), .c(new_n128_), .O(new_n129_));
  nand2  g67(.a(new_n129_), .b(new_n127_), .O(z12));
  nand2  g68(.a(new_n65_), .b(x27), .O(new_n131_));
  nand2  g69(.a(new_n78_), .b(z08), .O(new_n132_));
  inv1   g70(.a(x13), .O(new_n133_));
  nand2  g71(.a(new_n133_), .b(new_n111_), .O(new_n134_));
  nor2   g72(.a(x19), .b(x18), .O(new_n135_));
  nand2  g73(.a(new_n135_), .b(x20), .O(new_n136_));
  oai22  g74(.a(new_n136_), .b(new_n131_), .c(new_n134_), .d(new_n132_), .O(z13));
  nand2  g75(.a(new_n134_), .b(x40), .O(new_n138_));
  nand2  g76(.a(new_n132_), .b(new_n80_), .O(new_n139_));
  nand2  g77(.a(new_n136_), .b(new_n95_), .O(new_n140_));
  nand4  g78(.a(new_n140_), .b(new_n139_), .c(new_n138_), .d(new_n127_), .O(z14));
  nand4  g79(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n142_));
  inv1   g80(.a(new_n142_), .O(z15));
  nand2  g81(.a(x22), .b(x01), .O(new_n144_));
  nor2   g82(.a(new_n144_), .b(x23), .O(z16));
  inv1   g83(.a(x24), .O(new_n146_));
  nand4  g84(.a(new_n146_), .b(x23), .c(x22), .d(x01), .O(new_n147_));
  inv1   g85(.a(new_n147_), .O(z17));
  aoi22  g86(.a(new_n81_), .b(new_n80_), .c(z08), .d(x29), .O(new_n149_));
  nor2   g87(.a(new_n80_), .b(x04), .O(new_n150_));
  aoi21  g88(.a(new_n150_), .b(new_n81_), .c(new_n124_), .O(new_n151_));
  oai21  g89(.a(new_n149_), .b(new_n119_), .c(new_n151_), .O(z18));
  nor2   g90(.a(new_n124_), .b(new_n122_), .O(z20));
  aoi21  g91(.a(new_n122_), .b(new_n118_), .c(new_n124_), .O(z19));
endmodule


