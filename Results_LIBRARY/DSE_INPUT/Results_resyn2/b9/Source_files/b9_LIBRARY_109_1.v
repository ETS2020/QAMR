// Benchmark "FAU" written by ABC on Mon Jul 27 23:00:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n154_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n164_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  aoi21  g005(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(new_n68_));
  inv1   g006(.a(x39), .O(new_n69_));
  inv1   g007(.a(x40), .O(new_n70_));
  nor2   g008(.a(new_n70_), .b(new_n69_), .O(z08));
  nand3  g009(.a(z08), .b(x10), .c(new_n65_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  oai21  g011(.a(new_n73_), .b(new_n68_), .c(new_n63_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(x16), .O(z00));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n64_), .O(new_n79_));
  or2    g017(.a(x32), .b(x30), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n79_), .c(x04), .O(z01));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x28), .c(x04), .O(new_n83_));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n83_), .c(z08), .O(z02));
  inv1   g025(.a(x35), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n77_), .c(x27), .O(new_n89_));
  inv1   g027(.a(x37), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n64_), .O(new_n91_));
  nand3  g029(.a(new_n91_), .b(new_n89_), .c(x21), .O(z03));
  inv1   g030(.a(x21), .O(new_n93_));
  nand3  g031(.a(new_n91_), .b(new_n89_), .c(new_n93_), .O(z04));
  aoi21  g032(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g033(.a(z06), .O(z05));
  inv1   g034(.a(x25), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(x00), .O(new_n98_));
  inv1   g036(.a(x14), .O(new_n99_));
  nand2  g037(.a(x25), .b(new_n99_), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n98_), .c(x38), .O(new_n101_));
  inv1   g039(.a(x17), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(x15), .O(new_n103_));
  or2    g041(.a(x33), .b(x31), .O(new_n104_));
  oai21  g042(.a(new_n104_), .b(new_n103_), .c(x14), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n101_), .c(x03), .O(new_n106_));
  inv1   g044(.a(new_n106_), .O(z07));
  nand4  g045(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(z09));
  oai21  g047(.a(x36), .b(x35), .c(x07), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n70_), .O(new_n111_));
  inv1   g049(.a(x05), .O(new_n112_));
  nand3  g050(.a(x40), .b(x39), .c(new_n112_), .O(new_n113_));
  nand2  g051(.a(new_n76_), .b(x28), .O(new_n114_));
  inv1   g052(.a(x07), .O(new_n115_));
  aoi21  g053(.a(new_n69_), .b(new_n115_), .c(new_n64_), .O(new_n116_));
  nand4  g054(.a(new_n116_), .b(new_n114_), .c(new_n113_), .d(new_n111_), .O(new_n117_));
  nand3  g055(.a(new_n80_), .b(z08), .c(x05), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n65_), .O(new_n120_));
  nand3  g058(.a(x37), .b(x27), .c(x06), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(z10));
  nor2   g060(.a(x30), .b(x09), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n77_), .b(x27), .c(new_n65_), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(new_n127_));
  oai21  g065(.a(new_n127_), .b(new_n77_), .c(x08), .O(new_n128_));
  nor2   g066(.a(new_n127_), .b(new_n64_), .O(new_n129_));
  oai21  g067(.a(new_n129_), .b(new_n128_), .c(new_n125_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(x35), .c(new_n124_), .O(z11));
  nand2  g069(.a(x40), .b(new_n64_), .O(new_n132_));
  nand2  g070(.a(new_n90_), .b(new_n77_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x27), .O(new_n134_));
  aoi22  g072(.a(new_n134_), .b(new_n66_), .c(new_n90_), .d(new_n88_), .O(new_n135_));
  oai21  g073(.a(new_n132_), .b(x10), .c(new_n135_), .O(z12));
  inv1   g074(.a(x13), .O(new_n137_));
  nand4  g075(.a(x40), .b(x39), .c(new_n137_), .d(new_n65_), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(new_n139_));
  nor2   g077(.a(x19), .b(x18), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x20), .O(new_n141_));
  nor2   g079(.a(new_n141_), .b(new_n67_), .O(new_n142_));
  aoi21  g080(.a(new_n139_), .b(new_n114_), .c(new_n142_), .O(new_n143_));
  nand2  g081(.a(new_n139_), .b(new_n80_), .O(new_n144_));
  oai21  g082(.a(new_n143_), .b(new_n64_), .c(new_n144_), .O(z13));
  oai21  g083(.a(new_n114_), .b(new_n80_), .c(new_n139_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n141_), .O(new_n147_));
  aoi21  g085(.a(new_n133_), .b(new_n132_), .c(new_n137_), .O(new_n148_));
  nor2   g086(.a(new_n80_), .b(x27), .O(new_n149_));
  nor2   g087(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n147_), .c(new_n135_), .O(z14));
  nand4  g089(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(z15));
  nand2  g091(.a(x22), .b(x01), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(x23), .O(z16));
  inv1   g093(.a(x24), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x23), .c(x22), .d(x01), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z17));
  inv1   g096(.a(new_n129_), .O(new_n159_));
  nand2  g097(.a(new_n126_), .b(new_n78_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n159_), .c(x08), .O(new_n161_));
  nand4  g099(.a(x35), .b(new_n77_), .c(x27), .d(new_n65_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n161_), .c(new_n123_), .O(z18));
  nand2  g101(.a(new_n128_), .b(new_n64_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(x35), .c(new_n124_), .O(z20));
  aoi21  g103(.a(new_n130_), .b(x35), .c(new_n124_), .O(z19));
endmodule


