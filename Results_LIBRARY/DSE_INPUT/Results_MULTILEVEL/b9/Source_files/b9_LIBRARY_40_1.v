// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n157_, new_n159_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x36), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n65_), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  and2   g009(.a(x40), .b(x39), .O(z08));
  nand3  g010(.a(z08), .b(x10), .c(new_n65_), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  oai21  g012(.a(new_n74_), .b(new_n71_), .c(new_n63_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(x16), .O(z00));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n67_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x27), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n78_), .c(new_n77_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x04), .O(z01));
  inv1   g022(.a(x02), .O(new_n85_));
  nand2  g023(.a(x29), .b(x08), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n80_), .c(x04), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n87_), .c(z08), .O(z02));
  oai21  g028(.a(new_n66_), .b(new_n79_), .c(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n64_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n91_), .c(new_n95_), .O(z04));
  aoi21  g034(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g035(.a(z06), .O(z05));
  inv1   g036(.a(x03), .O(new_n99_));
  inv1   g037(.a(x31), .O(new_n100_));
  inv1   g038(.a(x33), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n63_), .O(new_n102_));
  inv1   g040(.a(x00), .O(new_n103_));
  oai21  g041(.a(x25), .b(new_n103_), .c(x38), .O(new_n104_));
  nand4  g042(.a(new_n104_), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n103_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n105_), .c(new_n99_), .O(z07));
  nand4  g048(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z09));
  nand2  g050(.a(x40), .b(x39), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x07), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n81_), .c(x27), .O(new_n117_));
  inv1   g055(.a(x40), .O(new_n118_));
  nor2   g056(.a(x32), .b(x30), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(x39), .c(x05), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n65_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(z10));
  inv1   g063(.a(x09), .O(new_n126_));
  nand2  g064(.a(z08), .b(x29), .O(new_n127_));
  nor2   g065(.a(new_n64_), .b(new_n65_), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n80_), .c(new_n127_), .O(new_n129_));
  inv1   g067(.a(x08), .O(new_n130_));
  nand4  g068(.a(x35), .b(new_n79_), .c(x27), .d(new_n65_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n77_), .c(new_n126_), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(z11));
  nor2   g073(.a(x37), .b(x36), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n66_), .c(new_n64_), .O(new_n137_));
  nand2  g075(.a(z08), .b(new_n65_), .O(new_n138_));
  oai21  g076(.a(new_n70_), .b(new_n64_), .c(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n137_), .b(x10), .c(new_n139_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  oai21  g079(.a(new_n67_), .b(new_n66_), .c(x28), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x27), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n119_), .c(new_n118_), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(x39), .c(new_n141_), .d(new_n65_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(x37), .c(x27), .d(x20), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(z13));
  nand3  g086(.a(new_n143_), .b(new_n78_), .c(new_n77_), .O(new_n149_));
  nand2  g087(.a(new_n92_), .b(new_n79_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n146_), .c(x27), .d(x20), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n138_), .O(new_n152_));
  nand2  g090(.a(new_n147_), .b(x13), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n152_), .c(new_n149_), .O(z14));
  nand4  g092(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(z15));
  nand2  g094(.a(x22), .b(x01), .O(new_n157_));
  nor2   g095(.a(new_n157_), .b(x23), .O(z16));
  inv1   g096(.a(x23), .O(new_n159_));
  nor3   g097(.a(new_n157_), .b(x24), .c(new_n159_), .O(z17));
  oai21  g098(.a(new_n80_), .b(x27), .c(new_n127_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x08), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(new_n131_), .c(new_n77_), .d(new_n126_), .O(z18));
  inv1   g101(.a(new_n127_), .O(new_n164_));
  oai21  g102(.a(new_n66_), .b(new_n64_), .c(new_n130_), .O(new_n165_));
  aoi21  g103(.a(x28), .b(new_n64_), .c(new_n66_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n164_), .c(new_n165_), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n77_), .c(new_n126_), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z20));
  inv1   g107(.a(new_n134_), .O(z19));
endmodule


