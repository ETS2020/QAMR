// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n172_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n183_, new_n184_, new_n185_, new_n186_, new_n187_,
    new_n188_;
  oai21  g000(.a(x36), .b(x35), .c(x39), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  oai21  g002(.a(new_n63_), .b(x04), .c(new_n64_), .O(new_n65_));
  inv1   g003(.a(x04), .O(new_n66_));
  nand4  g004(.a(x40), .b(x39), .c(x10), .d(new_n66_), .O(new_n67_));
  inv1   g005(.a(new_n67_), .O(new_n68_));
  aoi21  g006(.a(new_n65_), .b(x27), .c(new_n68_), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x15), .c(x16), .O(z00));
  inv1   g008(.a(x30), .O(new_n71_));
  inv1   g009(.a(x32), .O(new_n72_));
  inv1   g010(.a(x35), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  oai21  g012(.a(new_n73_), .b(x28), .c(new_n74_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(x27), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n72_), .c(new_n71_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x04), .O(z01));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(x29), .b(x08), .c(x02), .O(new_n81_));
  aoi21  g019(.a(new_n80_), .b(x04), .c(new_n81_), .O(new_n82_));
  inv1   g020(.a(x08), .O(new_n83_));
  inv1   g021(.a(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  nand4  g023(.a(new_n85_), .b(new_n82_), .c(x40), .d(x39), .O(z02));
  nand2  g024(.a(x35), .b(x28), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x27), .O(new_n88_));
  inv1   g026(.a(x37), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n84_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n88_), .c(x21), .O(z03));
  inv1   g029(.a(x21), .O(new_n92_));
  nand3  g030(.a(new_n90_), .b(new_n88_), .c(new_n92_), .O(z04));
  aoi21  g031(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g032(.a(z06), .O(z05));
  inv1   g033(.a(x03), .O(new_n96_));
  inv1   g034(.a(x31), .O(new_n97_));
  inv1   g035(.a(x33), .O(new_n98_));
  inv1   g036(.a(x15), .O(new_n99_));
  nand2  g037(.a(x17), .b(new_n99_), .O(new_n100_));
  inv1   g038(.a(x00), .O(new_n101_));
  oai21  g039(.a(x25), .b(new_n101_), .c(x38), .O(new_n102_));
  nand4  g040(.a(new_n102_), .b(new_n100_), .c(new_n98_), .d(new_n97_), .O(new_n103_));
  inv1   g041(.a(x14), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand3  g043(.a(x38), .b(new_n105_), .c(new_n101_), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n103_), .c(new_n96_), .O(z07));
  nand2  g046(.a(x40), .b(x39), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z08));
  nand4  g048(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z09));
  inv1   g050(.a(x07), .O(new_n113_));
  nand2  g051(.a(x39), .b(x05), .O(new_n114_));
  oai21  g052(.a(x39), .b(new_n113_), .c(new_n114_), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n75_), .c(x27), .O(new_n116_));
  inv1   g054(.a(x40), .O(new_n117_));
  aoi21  g055(.a(new_n72_), .b(new_n71_), .c(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(x39), .c(x05), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n66_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(z10));
  inv1   g061(.a(x09), .O(new_n124_));
  nand2  g062(.a(x39), .b(x29), .O(new_n125_));
  nand2  g063(.a(x27), .b(x04), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(x35), .c(new_n79_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  oai21  g066(.a(new_n117_), .b(new_n83_), .c(new_n80_), .O(new_n129_));
  oai21  g067(.a(new_n117_), .b(new_n66_), .c(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n83_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n129_), .c(new_n128_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n71_), .c(new_n124_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z11));
  inv1   g072(.a(x10), .O(new_n135_));
  nor3   g073(.a(x37), .b(x36), .c(x35), .O(new_n136_));
  nor2   g074(.a(new_n117_), .b(x27), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n136_), .c(new_n135_), .O(new_n138_));
  oai21  g076(.a(new_n117_), .b(new_n66_), .c(x39), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n84_), .O(new_n140_));
  nand2  g078(.a(x39), .b(new_n66_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n87_), .c(new_n89_), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(new_n140_), .c(new_n138_), .d(x40), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x28), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(x39), .c(new_n144_), .d(new_n66_), .O(new_n147_));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand2  g087(.a(new_n87_), .b(new_n89_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x20), .c(new_n149_), .d(new_n148_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand4  g091(.a(new_n118_), .b(x39), .c(new_n144_), .d(new_n66_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  nand3  g093(.a(x39), .b(new_n144_), .c(new_n66_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n87_), .O(new_n157_));
  nand4  g095(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x28), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n89_), .O(new_n160_));
  nand3  g098(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n161_));
  nand4  g099(.a(new_n145_), .b(new_n72_), .c(new_n71_), .d(x28), .O(new_n162_));
  oai21  g100(.a(x13), .b(x04), .c(x40), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n162_), .c(x39), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(new_n165_));
  nand2  g103(.a(new_n72_), .b(new_n71_), .O(new_n166_));
  nand3  g104(.a(new_n163_), .b(new_n166_), .c(x39), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n84_), .c(new_n117_), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n165_), .c(new_n160_), .O(z14));
  nand4  g107(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z15));
  nand2  g109(.a(x22), .b(x01), .O(new_n172_));
  nor2   g110(.a(new_n172_), .b(x23), .O(z16));
  inv1   g111(.a(x23), .O(new_n174_));
  nor3   g112(.a(new_n172_), .b(x24), .c(new_n174_), .O(z17));
  inv1   g113(.a(x29), .O(new_n176_));
  oai22  g114(.a(new_n80_), .b(x27), .c(new_n109_), .d(new_n176_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x08), .O(new_n178_));
  inv1   g116(.a(new_n80_), .O(new_n179_));
  nor2   g117(.a(new_n84_), .b(x04), .O(new_n180_));
  aoi21  g118(.a(new_n180_), .b(new_n179_), .c(x30), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n178_), .c(new_n124_), .O(z18));
  oai21  g120(.a(new_n73_), .b(new_n84_), .c(new_n83_), .O(new_n183_));
  oai21  g121(.a(new_n79_), .b(x27), .c(x35), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n125_), .O(new_n185_));
  nand2  g123(.a(new_n80_), .b(new_n117_), .O(new_n186_));
  nand3  g124(.a(new_n186_), .b(new_n185_), .c(new_n183_), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n71_), .c(new_n124_), .O(new_n188_));
  inv1   g126(.a(new_n188_), .O(z20));
  inv1   g127(.a(new_n133_), .O(z19));
endmodule


