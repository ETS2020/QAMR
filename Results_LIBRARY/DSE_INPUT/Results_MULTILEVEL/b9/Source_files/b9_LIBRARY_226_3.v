// Benchmark "FAU" written by ABC on Mon Jul 27 18:20:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n92_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n150_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n160_, new_n161_;
  inv1   g000(.a(x27), .O(new_n63_));
  inv1   g001(.a(x37), .O(new_n64_));
  nand2  g002(.a(x35), .b(x28), .O(new_n65_));
  aoi21  g003(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  inv1   g004(.a(x10), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(x04), .O(new_n68_));
  nand2  g006(.a(x40), .b(x39), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(z08));
  aoi21  g008(.a(z08), .b(new_n68_), .c(new_n66_), .O(new_n71_));
  oai21  g009(.a(new_n71_), .b(x15), .c(x16), .O(z00));
  inv1   g010(.a(x30), .O(new_n73_));
  inv1   g011(.a(x32), .O(new_n74_));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x27), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n74_), .c(new_n73_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x04), .O(z01));
  inv1   g018(.a(x02), .O(new_n81_));
  nand2  g019(.a(x29), .b(x08), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g021(.a(x08), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n84_), .c(x04), .O(new_n87_));
  nand4  g025(.a(new_n87_), .b(new_n83_), .c(z08), .d(new_n63_), .O(z02));
  nand2  g026(.a(new_n65_), .b(x27), .O(new_n89_));
  nand2  g027(.a(new_n64_), .b(new_n63_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n89_), .c(x21), .O(z03));
  inv1   g029(.a(x21), .O(new_n92_));
  nand3  g030(.a(new_n90_), .b(new_n89_), .c(new_n92_), .O(z04));
  oai21  g031(.a(new_n85_), .b(new_n63_), .c(new_n64_), .O(z05));
  inv1   g032(.a(x40), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n85_), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x27), .c(x37), .O(z06));
  inv1   g035(.a(x03), .O(new_n98_));
  inv1   g036(.a(x31), .O(new_n99_));
  inv1   g037(.a(x33), .O(new_n100_));
  inv1   g038(.a(x15), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n101_), .O(new_n102_));
  inv1   g040(.a(x00), .O(new_n103_));
  oai21  g041(.a(x25), .b(new_n103_), .c(x38), .O(new_n104_));
  nand4  g042(.a(new_n104_), .b(new_n102_), .c(new_n100_), .d(new_n99_), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand3  g045(.a(x38), .b(new_n107_), .c(new_n103_), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n105_), .c(new_n98_), .O(z07));
  nand4  g048(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z09));
  inv1   g050(.a(x04), .O(new_n113_));
  nand2  g051(.a(new_n74_), .b(new_n73_), .O(new_n114_));
  nand4  g052(.a(new_n114_), .b(x40), .c(x39), .d(x05), .O(new_n115_));
  nand3  g053(.a(new_n77_), .b(x27), .c(x07), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  nand3  g056(.a(x37), .b(x27), .c(x06), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(z10));
  inv1   g058(.a(x09), .O(new_n121_));
  oai21  g059(.a(x28), .b(x04), .c(x27), .O(new_n122_));
  oai21  g060(.a(new_n85_), .b(x27), .c(x35), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x29), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g063(.a(new_n63_), .b(new_n84_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n122_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n73_), .c(new_n121_), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(z11));
  nand2  g067(.a(new_n69_), .b(new_n63_), .O(new_n130_));
  oai21  g068(.a(x35), .b(new_n63_), .c(new_n96_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n64_), .O(new_n132_));
  oai21  g070(.a(new_n67_), .b(x04), .c(x40), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(new_n130_), .O(z12));
  inv1   g072(.a(x13), .O(new_n135_));
  nor2   g073(.a(x32), .b(x30), .O(new_n136_));
  nor2   g074(.a(new_n136_), .b(new_n95_), .O(new_n137_));
  nand4  g075(.a(new_n137_), .b(x39), .c(new_n135_), .d(new_n113_), .O(new_n138_));
  inv1   g076(.a(x18), .O(new_n139_));
  inv1   g077(.a(x19), .O(new_n140_));
  nand4  g078(.a(new_n66_), .b(x20), .c(new_n140_), .d(new_n139_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n138_), .O(z13));
  oai21  g080(.a(x13), .b(x04), .c(x40), .O(new_n143_));
  oai21  g081(.a(new_n136_), .b(new_n69_), .c(new_n63_), .O(new_n144_));
  nand3  g082(.a(x20), .b(new_n140_), .c(new_n139_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n95_), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(new_n144_), .c(new_n143_), .d(new_n132_), .O(z14));
  nand4  g085(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(z15));
  nand2  g087(.a(x22), .b(x01), .O(new_n150_));
  nor2   g088(.a(new_n150_), .b(x23), .O(z16));
  inv1   g089(.a(x23), .O(new_n152_));
  nor3   g090(.a(new_n150_), .b(x24), .c(new_n152_), .O(z17));
  oai21  g091(.a(new_n86_), .b(x27), .c(new_n124_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x08), .O(new_n155_));
  inv1   g093(.a(new_n86_), .O(new_n156_));
  nor2   g094(.a(new_n63_), .b(x04), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n156_), .c(x30), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n155_), .c(new_n121_), .O(z18));
  nand2  g097(.a(new_n126_), .b(new_n125_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n73_), .c(new_n121_), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z20));
  inv1   g100(.a(new_n128_), .O(z19));
endmodule


