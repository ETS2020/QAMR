// Benchmark "FAU" written by ABC on Mon Jul 27 18:19:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n167_, new_n169_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n177_, new_n178_, new_n179_, new_n180_, new_n181_;
  inv1   g000(.a(x04), .O(new_n63_));
  inv1   g001(.a(x39), .O(new_n64_));
  oai21  g002(.a(x27), .b(x10), .c(x40), .O(new_n65_));
  nor2   g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi22  g007(.a(new_n69_), .b(x27), .c(new_n66_), .d(new_n63_), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(x15), .c(x16), .O(z00));
  inv1   g009(.a(x30), .O(new_n72_));
  inv1   g010(.a(x32), .O(new_n73_));
  inv1   g011(.a(x35), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(x28), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(x36), .c(x27), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n73_), .c(new_n72_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x04), .O(z01));
  aoi21  g016(.a(x29), .b(x08), .c(x02), .O(new_n79_));
  aoi21  g017(.a(x28), .b(x04), .c(new_n79_), .O(new_n80_));
  oai21  g018(.a(x27), .b(x08), .c(x35), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x04), .O(new_n82_));
  nand4  g020(.a(new_n82_), .b(new_n80_), .c(x40), .d(x39), .O(z02));
  nand2  g021(.a(new_n68_), .b(x27), .O(new_n84_));
  inv1   g022(.a(x27), .O(new_n85_));
  nand2  g023(.a(new_n67_), .b(new_n85_), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n84_), .c(x21), .O(z03));
  inv1   g025(.a(x21), .O(new_n88_));
  nand3  g026(.a(new_n86_), .b(new_n84_), .c(new_n88_), .O(z04));
  aoi21  g027(.a(x28), .b(x27), .c(x37), .O(z06));
  inv1   g028(.a(z06), .O(z05));
  inv1   g029(.a(x03), .O(new_n92_));
  inv1   g030(.a(x31), .O(new_n93_));
  inv1   g031(.a(x33), .O(new_n94_));
  inv1   g032(.a(x15), .O(new_n95_));
  nand2  g033(.a(x17), .b(new_n95_), .O(new_n96_));
  inv1   g034(.a(x00), .O(new_n97_));
  oai21  g035(.a(x25), .b(new_n97_), .c(x38), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n96_), .c(new_n94_), .d(new_n93_), .O(new_n99_));
  inv1   g037(.a(x14), .O(new_n100_));
  inv1   g038(.a(x25), .O(new_n101_));
  nand3  g039(.a(x38), .b(new_n101_), .c(new_n97_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n99_), .c(new_n92_), .O(z07));
  nand2  g042(.a(x40), .b(x39), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(z08));
  nand4  g044(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n107_));
  inv1   g045(.a(new_n107_), .O(z09));
  inv1   g046(.a(x07), .O(new_n109_));
  nand2  g047(.a(new_n105_), .b(x36), .O(new_n110_));
  inv1   g048(.a(x28), .O(new_n111_));
  inv1   g049(.a(x40), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(x35), .c(new_n111_), .O(new_n113_));
  aoi21  g051(.a(new_n113_), .b(new_n110_), .c(new_n109_), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(new_n114_), .c(x27), .O(new_n117_));
  nor2   g055(.a(x32), .b(x30), .O(new_n118_));
  nor3   g056(.a(new_n118_), .b(new_n112_), .c(new_n64_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x05), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n117_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n63_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(z10));
  inv1   g062(.a(x09), .O(new_n125_));
  nand2  g063(.a(z08), .b(x29), .O(new_n126_));
  oai21  g064(.a(new_n85_), .b(new_n63_), .c(new_n75_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(x40), .b(x04), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(x35), .c(new_n111_), .d(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n72_), .c(new_n125_), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(z11));
  nand4  g073(.a(x40), .b(x39), .c(x10), .d(new_n63_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n85_), .O(new_n137_));
  nand2  g075(.a(z08), .b(new_n63_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n68_), .c(new_n67_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n137_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  aoi21  g079(.a(new_n74_), .b(x28), .c(new_n112_), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(x39), .c(new_n141_), .d(new_n63_), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand3  g082(.a(new_n69_), .b(x20), .c(new_n144_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(x18), .c(new_n143_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  nand3  g085(.a(new_n119_), .b(new_n141_), .c(new_n63_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(z13));
  nor2   g087(.a(x19), .b(x18), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(x37), .c(x20), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(new_n73_), .c(new_n72_), .d(x28), .O(new_n152_));
  nand2  g090(.a(new_n141_), .b(new_n63_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n105_), .c(new_n67_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n74_), .O(new_n156_));
  and2   g094(.a(x27), .b(x20), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n150_), .c(new_n112_), .O(new_n158_));
  nor2   g096(.a(x37), .b(x28), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n158_), .c(new_n153_), .O(new_n160_));
  nor2   g098(.a(new_n159_), .b(new_n85_), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n150_), .c(x20), .O(new_n162_));
  aoi22  g100(.a(new_n162_), .b(new_n105_), .c(new_n118_), .d(new_n85_), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n160_), .c(new_n156_), .O(z14));
  nand4  g102(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z15));
  nand2  g104(.a(x22), .b(x01), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(x23), .O(z16));
  inv1   g106(.a(x23), .O(new_n169_));
  nor3   g107(.a(new_n167_), .b(x24), .c(new_n169_), .O(z17));
  nand2  g108(.a(new_n75_), .b(new_n85_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n126_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x08), .O(new_n173_));
  nor2   g111(.a(new_n85_), .b(x04), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n75_), .c(x30), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n173_), .c(new_n125_), .O(z18));
  inv1   g114(.a(new_n126_), .O(new_n177_));
  oai21  g115(.a(new_n74_), .b(new_n85_), .c(new_n129_), .O(new_n178_));
  aoi21  g116(.a(x28), .b(new_n85_), .c(new_n74_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n177_), .c(new_n178_), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n72_), .c(new_n125_), .O(new_n181_));
  inv1   g119(.a(new_n181_), .O(z20));
  inv1   g120(.a(new_n134_), .O(z19));
endmodule


