// Benchmark "FAU" written by ABC on Mon Jul 27 17:30:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n107_, new_n108_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x2), .O(new_n21_));
  inv1   g04(.a(x3), .O(z8));
  nand2  g05(.a(z8), .b(new_n21_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(x1), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n20_), .c(x6), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n19_), .O(new_n26_));
  inv1   g09(.a(x6), .O(new_n27_));
  nand3  g10(.a(x3), .b(x2), .c(x1), .O(new_n28_));
  nand3  g11(.a(new_n28_), .b(x5), .c(new_n20_), .O(new_n29_));
  and2   g12(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(new_n26_), .c(new_n18_), .O(z0));
  nand3  g14(.a(x5), .b(new_n18_), .c(x3), .O(new_n32_));
  nor2   g15(.a(x5), .b(new_n18_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(x0), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(new_n32_), .c(new_n21_), .O(new_n35_));
  nand2  g18(.a(new_n33_), .b(x3), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(new_n27_), .c(new_n20_), .O(new_n37_));
  oai21  g20(.a(new_n37_), .b(new_n35_), .c(x1), .O(new_n38_));
  nand2  g21(.a(x6), .b(x2), .O(new_n39_));
  oai21  g22(.a(new_n19_), .b(x4), .c(new_n39_), .O(new_n40_));
  oai21  g23(.a(new_n27_), .b(new_n18_), .c(new_n29_), .O(new_n41_));
  aoi21  g24(.a(new_n40_), .b(x0), .c(new_n41_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n38_), .O(z1));
  inv1   g26(.a(x1), .O(new_n44_));
  nand3  g27(.a(new_n23_), .b(new_n27_), .c(x0), .O(new_n45_));
  nand3  g28(.a(x5), .b(x3), .c(x2), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  nor2   g30(.a(x2), .b(x1), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n20_), .c(x6), .O(new_n49_));
  oai21  g32(.a(new_n19_), .b(new_n20_), .c(new_n49_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n47_), .c(new_n18_), .O(new_n51_));
  nor3   g34(.a(new_n48_), .b(new_n27_), .c(new_n20_), .O(new_n52_));
  nor2   g35(.a(new_n19_), .b(x0), .O(new_n53_));
  nor2   g36(.a(x6), .b(x5), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n53_), .c(new_n44_), .O(new_n55_));
  nor3   g38(.a(x6), .b(x5), .c(x3), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n53_), .c(new_n21_), .O(new_n57_));
  nor2   g40(.a(new_n19_), .b(x3), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n54_), .c(new_n20_), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n57_), .c(new_n55_), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(new_n52_), .c(x4), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n51_), .O(z2));
  nand3  g45(.a(x5), .b(x2), .c(x1), .O(new_n63_));
  nand3  g46(.a(new_n54_), .b(new_n21_), .c(x0), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(z8), .O(new_n66_));
  nand3  g49(.a(new_n19_), .b(x3), .c(x2), .O(new_n67_));
  aoi21  g50(.a(new_n67_), .b(new_n27_), .c(new_n44_), .O(new_n68_));
  oai21  g51(.a(new_n21_), .b(new_n44_), .c(x5), .O(new_n69_));
  nand3  g52(.a(x6), .b(x3), .c(x2), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  oai21  g54(.a(new_n71_), .b(new_n68_), .c(new_n20_), .O(new_n72_));
  nand3  g55(.a(new_n39_), .b(new_n19_), .c(new_n44_), .O(new_n73_));
  nand3  g56(.a(x5), .b(x3), .c(x1), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(x0), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n72_), .c(new_n66_), .O(z3));
  nand2  g60(.a(x3), .b(x2), .O(new_n78_));
  nand2  g61(.a(new_n23_), .b(x0), .O(new_n79_));
  aoi21  g62(.a(new_n79_), .b(new_n78_), .c(new_n27_), .O(new_n80_));
  nand2  g63(.a(new_n78_), .b(new_n20_), .O(new_n81_));
  aoi21  g64(.a(new_n81_), .b(new_n23_), .c(x6), .O(new_n82_));
  oai21  g65(.a(new_n82_), .b(new_n80_), .c(x1), .O(new_n83_));
  nor2   g66(.a(x3), .b(x0), .O(new_n84_));
  nor3   g67(.a(new_n84_), .b(x6), .c(new_n21_), .O(new_n85_));
  nor2   g68(.a(new_n84_), .b(new_n21_), .O(new_n86_));
  nor2   g69(.a(new_n86_), .b(new_n27_), .O(new_n87_));
  oai21  g70(.a(new_n87_), .b(new_n85_), .c(new_n44_), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n83_), .O(z4));
  nand2  g72(.a(x3), .b(new_n21_), .O(new_n90_));
  nand2  g73(.a(z8), .b(x2), .O(new_n91_));
  oai21  g74(.a(new_n90_), .b(new_n44_), .c(new_n91_), .O(new_n92_));
  nand2  g75(.a(new_n92_), .b(new_n20_), .O(new_n93_));
  inv1   g76(.a(new_n78_), .O(new_n94_));
  aoi21  g77(.a(x3), .b(x1), .c(x2), .O(new_n95_));
  oai21  g78(.a(new_n95_), .b(new_n94_), .c(x0), .O(new_n96_));
  nand2  g79(.a(new_n96_), .b(new_n93_), .O(z5));
  nor2   g80(.a(x6), .b(x4), .O(new_n98_));
  aoi21  g81(.a(new_n19_), .b(x4), .c(new_n98_), .O(new_n99_));
  nand3  g82(.a(new_n27_), .b(new_n19_), .c(x0), .O(new_n100_));
  nand2  g83(.a(new_n100_), .b(x3), .O(new_n101_));
  oai21  g84(.a(new_n99_), .b(new_n20_), .c(new_n101_), .O(new_n102_));
  aoi21  g85(.a(new_n102_), .b(x2), .c(z8), .O(new_n103_));
  nand3  g86(.a(x3), .b(new_n21_), .c(new_n44_), .O(new_n104_));
  oai21  g87(.a(new_n103_), .b(new_n44_), .c(new_n104_), .O(z6));
  nand2  g88(.a(new_n91_), .b(new_n90_), .O(z7));
  nand2  g89(.a(new_n28_), .b(new_n20_), .O(new_n107_));
  nand3  g90(.a(new_n107_), .b(x5), .c(x4), .O(new_n108_));
  inv1   g91(.a(new_n108_), .O(z9));
endmodule


