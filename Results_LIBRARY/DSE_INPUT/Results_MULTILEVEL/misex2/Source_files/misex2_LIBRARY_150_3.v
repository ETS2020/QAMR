// Benchmark "FAU" written by ABC on Mon Jul 27 17:31:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n121_, new_n123_, new_n124_, new_n127_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  or2    g06(.a(new_n49_), .b(x09), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nor2   g10(.a(x02), .b(x01), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n44_), .c(x10), .d(x09), .O(new_n55_));
  nor3   g12(.a(new_n55_), .b(x19), .c(x18), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n44_), .c(new_n57_), .d(x09), .O(new_n58_));
  nor3   g15(.a(new_n58_), .b(x19), .c(x18), .O(z02));
  nand2  g16(.a(new_n54_), .b(new_n46_), .O(new_n60_));
  inv1   g17(.a(x19), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(x18), .c(new_n44_), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nand3  g20(.a(x12), .b(x11), .c(x10), .O(new_n64_));
  oai22  g21(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n60_), .O(z03));
  nand4  g22(.a(x10), .b(x02), .c(x01), .d(x00), .O(new_n66_));
  nor3   g23(.a(new_n66_), .b(x12), .c(x11), .O(z04));
  nand4  g24(.a(x11), .b(x10), .c(x02), .d(x01), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n46_), .O(z06));
  inv1   g26(.a(x12), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(x11), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n57_), .c(x01), .O(new_n73_));
  nand3  g29(.a(new_n73_), .b(x02), .c(x00), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(z07));
  inv1   g31(.a(x08), .O(new_n76_));
  inv1   g32(.a(x05), .O(new_n77_));
  inv1   g33(.a(x06), .O(new_n78_));
  inv1   g34(.a(x03), .O(new_n79_));
  nand4  g35(.a(new_n79_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand4  g37(.a(new_n81_), .b(new_n78_), .c(new_n77_), .d(x04), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(x07), .O(new_n83_));
  nand4  g39(.a(new_n83_), .b(new_n45_), .c(x17), .d(new_n76_), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n61_), .O(z08));
  inv1   g41(.a(x21), .O(new_n86_));
  inv1   g42(.a(x22), .O(new_n87_));
  inv1   g43(.a(x11), .O(new_n88_));
  nor2   g44(.a(new_n48_), .b(x01), .O(new_n89_));
  nand3  g45(.a(new_n89_), .b(x12), .c(new_n88_), .O(new_n90_));
  inv1   g46(.a(x15), .O(new_n91_));
  inv1   g47(.a(x16), .O(new_n92_));
  nor2   g48(.a(x14), .b(x13), .O(new_n93_));
  nand4  g49(.a(new_n93_), .b(x20), .c(new_n92_), .d(new_n91_), .O(new_n94_));
  inv1   g50(.a(x20), .O(new_n95_));
  nor2   g51(.a(new_n45_), .b(new_n47_), .O(new_n96_));
  nand3  g52(.a(new_n96_), .b(new_n95_), .c(new_n61_), .O(new_n97_));
  oai21  g53(.a(new_n94_), .b(new_n90_), .c(new_n97_), .O(new_n98_));
  nand4  g54(.a(new_n98_), .b(new_n87_), .c(new_n86_), .d(new_n46_), .O(new_n99_));
  inv1   g55(.a(new_n99_), .O(z09));
  nor3   g56(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n101_));
  nand4  g57(.a(new_n101_), .b(new_n61_), .c(x18), .d(x01), .O(new_n102_));
  nor2   g58(.a(x13), .b(new_n71_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n89_), .c(new_n88_), .O(new_n104_));
  nor3   g60(.a(new_n92_), .b(new_n91_), .c(x14), .O(new_n105_));
  nand4  g61(.a(new_n105_), .b(new_n87_), .c(new_n86_), .d(x20), .O(new_n106_));
  or2    g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n102_), .c(x00), .O(z10));
  nand4  g64(.a(new_n96_), .b(x21), .c(new_n95_), .d(new_n61_), .O(new_n109_));
  nor2   g65(.a(new_n91_), .b(x14), .O(new_n110_));
  nand4  g66(.a(new_n110_), .b(new_n86_), .c(x20), .d(new_n92_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n104_), .c(new_n109_), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n87_), .c(new_n46_), .O(new_n113_));
  inv1   g69(.a(new_n113_), .O(z11));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n115_));
  nand3  g71(.a(new_n61_), .b(x17), .c(new_n48_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n115_), .c(x24), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(x09), .c(new_n47_), .O(new_n118_));
  inv1   g74(.a(new_n118_), .O(z12));
  nor3   g75(.a(new_n49_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g76(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n121_));
  nor2   g77(.a(new_n121_), .b(x19), .O(z14));
  aoi21  g78(.a(new_n57_), .b(x01), .c(new_n48_), .O(new_n123_));
  nand3  g79(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n124_));
  oai21  g80(.a(new_n123_), .b(new_n46_), .c(new_n124_), .O(z15));
  nor2   g81(.a(new_n47_), .b(x00), .O(z16));
  nand3  g82(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n127_));
  inv1   g83(.a(new_n127_), .O(z17));
  zero   g84(.O(z05));
endmodule


