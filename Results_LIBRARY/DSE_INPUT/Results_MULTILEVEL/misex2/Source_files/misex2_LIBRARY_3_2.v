// Benchmark "FAU" written by ABC on Mon Jul 27 17:30:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n131_, new_n133_, new_n134_, new_n137_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  nor3   g02(.a(x09), .b(x02), .c(x01), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x19), .O(z00));
  nor2   g05(.a(x02), .b(x01), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n44_), .c(x10), .d(x09), .O(new_n50_));
  nor3   g07(.a(new_n50_), .b(x19), .c(x18), .O(z01));
  inv1   g08(.a(x10), .O(new_n52_));
  nand4  g09(.a(new_n49_), .b(new_n44_), .c(new_n52_), .d(x09), .O(new_n53_));
  nor3   g10(.a(new_n53_), .b(x19), .c(x18), .O(z02));
  inv1   g11(.a(x00), .O(new_n55_));
  nand2  g12(.a(new_n49_), .b(new_n55_), .O(new_n56_));
  inv1   g13(.a(x19), .O(new_n57_));
  nand3  g14(.a(new_n57_), .b(x18), .c(new_n44_), .O(new_n58_));
  nand3  g15(.a(x02), .b(x01), .c(x00), .O(new_n59_));
  inv1   g16(.a(x09), .O(new_n60_));
  nand4  g17(.a(x12), .b(x11), .c(x10), .d(new_n60_), .O(new_n61_));
  oai22  g18(.a(new_n61_), .b(new_n59_), .c(new_n58_), .d(new_n56_), .O(z03));
  inv1   g19(.a(x11), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n63_), .c(x10), .d(new_n60_), .O(new_n66_));
  nor2   g23(.a(new_n66_), .b(x12), .O(z04));
  nor3   g24(.a(new_n64_), .b(new_n52_), .c(new_n60_), .O(z05));
  inv1   g25(.a(x01), .O(new_n69_));
  nand4  g26(.a(x11), .b(x10), .c(new_n60_), .d(x02), .O(new_n70_));
  nor3   g27(.a(new_n70_), .b(new_n69_), .c(new_n55_), .O(z06));
  inv1   g28(.a(x12), .O(new_n72_));
  nand3  g29(.a(new_n72_), .b(x11), .c(x10), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(x09), .c(x01), .O(new_n74_));
  nand3  g31(.a(new_n74_), .b(x02), .c(x00), .O(new_n75_));
  inv1   g32(.a(new_n75_), .O(z07));
  inv1   g33(.a(x08), .O(new_n77_));
  inv1   g34(.a(x05), .O(new_n78_));
  inv1   g35(.a(x06), .O(new_n79_));
  inv1   g36(.a(x07), .O(new_n80_));
  inv1   g37(.a(x03), .O(new_n81_));
  nand4  g38(.a(x04), .b(new_n81_), .c(x02), .d(new_n69_), .O(new_n82_));
  inv1   g39(.a(new_n82_), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n80_), .c(new_n79_), .d(new_n78_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n45_), .c(x17), .d(new_n77_), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(new_n57_), .O(z08));
  inv1   g44(.a(x21), .O(new_n88_));
  inv1   g45(.a(x22), .O(new_n89_));
  inv1   g46(.a(x02), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(x01), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n63_), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  nor2   g51(.a(x14), .b(x13), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(x20), .c(new_n94_), .d(new_n93_), .O(new_n96_));
  inv1   g53(.a(x20), .O(new_n97_));
  nor2   g54(.a(new_n45_), .b(new_n69_), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n57_), .O(new_n99_));
  oai21  g56(.a(new_n96_), .b(new_n92_), .c(new_n99_), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n89_), .c(new_n88_), .d(new_n55_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(z09));
  nor3   g59(.a(new_n89_), .b(new_n88_), .c(x20), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n57_), .c(x18), .d(x01), .O(new_n104_));
  inv1   g61(.a(x13), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(x12), .c(new_n63_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nor3   g64(.a(new_n94_), .b(new_n93_), .c(x14), .O(new_n108_));
  nand3  g65(.a(new_n89_), .b(new_n88_), .c(x20), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n108_), .c(new_n107_), .d(new_n91_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n104_), .c(x00), .O(z10));
  nand2  g69(.a(new_n107_), .b(new_n91_), .O(new_n113_));
  nand4  g70(.a(new_n98_), .b(x21), .c(new_n97_), .d(new_n57_), .O(new_n114_));
  nor2   g71(.a(new_n93_), .b(x14), .O(new_n115_));
  nand4  g72(.a(new_n115_), .b(new_n88_), .c(x20), .d(new_n94_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n113_), .c(new_n114_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n89_), .c(new_n55_), .O(new_n118_));
  inv1   g75(.a(new_n118_), .O(z11));
  nand2  g76(.a(x10), .b(x02), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(x01), .c(x00), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(x23), .O(new_n122_));
  nand3  g79(.a(new_n57_), .b(x17), .c(new_n90_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(new_n69_), .c(new_n55_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n121_), .c(x24), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x09), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(z12));
  nand4  g85(.a(x17), .b(new_n90_), .c(new_n69_), .d(new_n55_), .O(new_n129_));
  nor2   g86(.a(new_n129_), .b(x19), .O(z13));
  nand4  g87(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n52_), .O(new_n131_));
  nor2   g88(.a(new_n131_), .b(x19), .O(z14));
  aoi21  g89(.a(new_n52_), .b(x01), .c(new_n90_), .O(new_n133_));
  nand3  g90(.a(x19), .b(new_n90_), .c(new_n69_), .O(new_n134_));
  oai21  g91(.a(new_n133_), .b(new_n55_), .c(new_n134_), .O(z15));
  nor2   g92(.a(new_n69_), .b(x00), .O(z16));
  nand3  g93(.a(x02), .b(new_n69_), .c(new_n55_), .O(new_n137_));
  inv1   g94(.a(new_n137_), .O(z17));
endmodule


