// Benchmark "FAU" written by ABC on Mon Jul 27 17:25:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_;
  inv1   g00(.a(x02), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  aoi21  g02(.a(new_n21_), .b(x08), .c(new_n20_), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nor2   g04(.a(new_n23_), .b(x02), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n22_), .c(x04), .O(new_n25_));
  inv1   g06(.a(x00), .O(new_n26_));
  inv1   g07(.a(x03), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g09(.a(x04), .O(new_n29_));
  nand3  g10(.a(x08), .b(new_n29_), .c(x03), .O(new_n30_));
  nand4  g11(.a(new_n30_), .b(new_n28_), .c(new_n25_), .d(x05), .O(z0));
  oai21  g12(.a(x10), .b(x08), .c(new_n21_), .O(new_n32_));
  nand2  g13(.a(new_n23_), .b(x02), .O(new_n33_));
  aoi22  g14(.a(new_n33_), .b(x10), .c(new_n32_), .d(x02), .O(new_n34_));
  inv1   g15(.a(x01), .O(new_n35_));
  nand2  g16(.a(new_n27_), .b(new_n35_), .O(new_n36_));
  nand3  g17(.a(x10), .b(new_n29_), .c(x03), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(new_n36_), .c(x05), .O(new_n38_));
  inv1   g19(.a(new_n38_), .O(new_n39_));
  oai21  g20(.a(new_n34_), .b(new_n29_), .c(new_n39_), .O(z1));
  inv1   g21(.a(x10), .O(new_n41_));
  inv1   g22(.a(x11), .O(new_n42_));
  nand3  g23(.a(new_n42_), .b(new_n41_), .c(new_n23_), .O(new_n43_));
  aoi21  g24(.a(new_n43_), .b(new_n21_), .c(new_n20_), .O(new_n44_));
  nor2   g25(.a(x10), .b(x08), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(x02), .c(new_n42_), .O(new_n46_));
  oai21  g27(.a(new_n46_), .b(new_n44_), .c(x04), .O(new_n47_));
  inv1   g28(.a(x06), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n27_), .O(new_n49_));
  nand3  g30(.a(x11), .b(new_n29_), .c(x03), .O(new_n50_));
  nand3  g31(.a(new_n50_), .b(new_n49_), .c(x05), .O(new_n51_));
  inv1   g32(.a(new_n51_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n47_), .O(z2));
  inv1   g34(.a(x12), .O(new_n54_));
  nand4  g35(.a(new_n54_), .b(new_n42_), .c(new_n41_), .d(new_n23_), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n21_), .c(new_n20_), .O(new_n56_));
  nor2   g37(.a(x08), .b(new_n20_), .O(new_n57_));
  nor2   g38(.a(x11), .b(x10), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n56_), .c(x04), .O(new_n60_));
  inv1   g41(.a(x07), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n27_), .O(new_n62_));
  nand3  g43(.a(x12), .b(new_n29_), .c(x03), .O(new_n63_));
  nand3  g44(.a(new_n63_), .b(new_n62_), .c(x05), .O(new_n64_));
  inv1   g45(.a(new_n64_), .O(new_n65_));
  nand2  g46(.a(new_n65_), .b(new_n60_), .O(z3));
  nand3  g47(.a(x13), .b(x09), .c(x04), .O(new_n67_));
  inv1   g48(.a(new_n67_), .O(z4));
endmodule


