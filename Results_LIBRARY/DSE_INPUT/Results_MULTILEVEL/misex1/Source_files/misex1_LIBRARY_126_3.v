// Benchmark "FAU" written by ABC on Mon Jul 27 17:10:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n17_), .b(new_n18_), .O(new_n19_));
  aoi21  g04(.a(new_n17_), .b(x0), .c(new_n19_), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n16_), .O(z0));
  inv1   g06(.a(x0), .O(new_n22_));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(new_n16_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n18_), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n17_), .c(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n22_), .O(new_n28_));
  aoi21  g13(.a(x3), .b(x0), .c(x1), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x2), .c(new_n28_), .O(z1));
  inv1   g15(.a(x5), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x2), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  nand4  g20(.a(new_n35_), .b(new_n17_), .c(new_n18_), .d(new_n22_), .O(new_n36_));
  inv1   g21(.a(x4), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x2), .c(new_n17_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x1), .O(new_n39_));
  nand3  g24(.a(x3), .b(new_n16_), .c(x0), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n16_), .c(x1), .O(new_n43_));
  oai21  g28(.a(x5), .b(x1), .c(new_n22_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n17_), .c(x2), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z3));
  xor2a  g31(.a(x3), .b(x2), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x0), .O(new_n48_));
  oai21  g33(.a(x3), .b(new_n16_), .c(x1), .O(new_n49_));
  nor2   g34(.a(new_n33_), .b(x2), .O(new_n50_));
  aoi21  g35(.a(new_n35_), .b(new_n18_), .c(new_n50_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(x3), .c(new_n25_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n22_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n49_), .c(new_n48_), .O(z4));
  nand3  g39(.a(new_n49_), .b(new_n48_), .c(new_n28_), .O(z5));
  nand3  g40(.a(new_n23_), .b(new_n18_), .c(new_n22_), .O(new_n56_));
  inv1   g41(.a(new_n56_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n19_), .c(x2), .O(new_n58_));
  nand2  g43(.a(x4), .b(new_n17_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n16_), .c(x1), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n58_), .c(new_n48_), .O(z6));
endmodule


