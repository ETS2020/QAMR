// Benchmark "FAU" written by ABC on Mon Jul 27 19:38:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_;
  inv1   g000(.a(x01), .O(new_n34_));
  inv1   g001(.a(x03), .O(new_n35_));
  inv1   g002(.a(x06), .O(new_n36_));
  inv1   g003(.a(x11), .O(new_n37_));
  inv1   g004(.a(x14), .O(new_n38_));
  inv1   g005(.a(x20), .O(new_n39_));
  nand2  g006(.a(x19), .b(x05), .O(new_n40_));
  nand2  g007(.a(x24), .b(x07), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x13), .O(new_n43_));
  inv1   g010(.a(x05), .O(new_n44_));
  inv1   g011(.a(x13), .O(new_n45_));
  nand3  g012(.a(x19), .b(new_n45_), .c(new_n44_), .O(new_n46_));
  inv1   g013(.a(x02), .O(new_n47_));
  inv1   g014(.a(x10), .O(new_n48_));
  inv1   g015(.a(x24), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x00), .O(new_n50_));
  nand3  g017(.a(new_n50_), .b(new_n46_), .c(new_n43_), .O(z5));
  nand4  g018(.a(z5), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n52_));
  inv1   g019(.a(new_n52_), .O(new_n53_));
  nand4  g020(.a(new_n53_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n54_));
  inv1   g021(.a(new_n54_), .O(z0));
  nand3  g022(.a(x04), .b(x03), .c(x01), .O(new_n56_));
  nand4  g023(.a(x11), .b(x09), .c(x08), .d(x06), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand4  g025(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  and2   g027(.a(x21), .b(x20), .O(new_n61_));
  inv1   g028(.a(x22), .O(new_n62_));
  inv1   g029(.a(x23), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand4  g031(.a(new_n64_), .b(new_n61_), .c(new_n60_), .d(new_n58_), .O(new_n65_));
  nor2   g032(.a(x03), .b(x01), .O(new_n66_));
  nor2   g033(.a(x06), .b(x04), .O(new_n67_));
  nor2   g034(.a(x09), .b(x08), .O(new_n68_));
  nand4  g035(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(x00), .O(new_n69_));
  nor2   g036(.a(x12), .b(x11), .O(new_n70_));
  nor2   g037(.a(x16), .b(x14), .O(new_n71_));
  nor2   g038(.a(x20), .b(x17), .O(new_n72_));
  nor2   g039(.a(x23), .b(x22), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  oai21  g041(.a(new_n74_), .b(new_n69_), .c(new_n65_), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n76_));
  nand4  g043(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n77_));
  inv1   g044(.a(new_n77_), .O(new_n78_));
  inv1   g045(.a(x17), .O(new_n79_));
  nor2   g046(.a(new_n39_), .b(new_n79_), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n78_), .c(new_n64_), .d(new_n58_), .O(new_n81_));
  inv1   g048(.a(x09), .O(new_n82_));
  nor3   g049(.a(x04), .b(x03), .c(x01), .O(new_n83_));
  nor2   g050(.a(x08), .b(x06), .O(new_n84_));
  nand4  g051(.a(new_n84_), .b(new_n83_), .c(new_n37_), .d(new_n82_), .O(new_n85_));
  nor2   g052(.a(x14), .b(x12), .O(new_n86_));
  nor2   g053(.a(x17), .b(x16), .O(new_n87_));
  inv1   g054(.a(x19), .O(new_n88_));
  nor2   g055(.a(x20), .b(new_n88_), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n87_), .c(new_n86_), .d(new_n73_), .O(new_n90_));
  oai21  g057(.a(new_n90_), .b(new_n85_), .c(new_n81_), .O(new_n91_));
  xnor2a g058(.a(x13), .b(x05), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  and2   g060(.a(x20), .b(x18), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n64_), .c(new_n60_), .d(new_n58_), .O(new_n95_));
  nand4  g062(.a(new_n83_), .b(new_n68_), .c(x07), .d(new_n36_), .O(new_n96_));
  oai21  g063(.a(new_n96_), .b(new_n74_), .c(new_n95_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(x24), .c(x13), .O(new_n98_));
  nand3  g065(.a(new_n98_), .b(new_n93_), .c(new_n76_), .O(z1));
  nand2  g066(.a(x15), .b(x05), .O(new_n100_));
  nand2  g067(.a(x24), .b(x18), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(x13), .O(new_n103_));
  nand3  g070(.a(x15), .b(new_n45_), .c(new_n44_), .O(new_n104_));
  nand4  g071(.a(new_n49_), .b(x21), .c(new_n48_), .d(new_n47_), .O(new_n105_));
  nand3  g072(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(z7));
  nand4  g073(.a(z7), .b(x20), .c(x14), .d(x11), .O(new_n107_));
  inv1   g074(.a(new_n107_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(x06), .c(x03), .d(x01), .O(new_n109_));
  inv1   g076(.a(new_n109_), .O(z2));
  nand4  g077(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(x00), .O(new_n111_));
  inv1   g078(.a(x08), .O(new_n112_));
  nand4  g079(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(new_n112_), .O(new_n113_));
  nand4  g080(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n114_));
  nor2   g081(.a(new_n38_), .b(new_n37_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n61_), .O(new_n116_));
  oai22  g083(.a(new_n116_), .b(new_n114_), .c(new_n113_), .d(new_n111_), .O(new_n117_));
  nand4  g084(.a(new_n117_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n118_));
  nand3  g085(.a(new_n115_), .b(x20), .c(x15), .O(new_n119_));
  nand2  g086(.a(new_n84_), .b(new_n66_), .O(new_n120_));
  nand3  g087(.a(new_n89_), .b(new_n38_), .c(new_n37_), .O(new_n121_));
  oai22  g088(.a(new_n121_), .b(new_n120_), .c(new_n119_), .d(new_n114_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n92_), .O(new_n123_));
  nand2  g090(.a(new_n115_), .b(new_n94_), .O(new_n124_));
  nand3  g091(.a(new_n66_), .b(x07), .c(new_n36_), .O(new_n125_));
  oai22  g092(.a(new_n125_), .b(new_n113_), .c(new_n124_), .d(new_n114_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(x24), .c(x13), .O(new_n127_));
  nand3  g094(.a(new_n127_), .b(new_n123_), .c(new_n118_), .O(z3));
  inv1   g095(.a(z5), .O(new_n129_));
  inv1   g096(.a(z7), .O(new_n130_));
  inv1   g097(.a(x16), .O(new_n131_));
  aoi21  g098(.a(new_n63_), .b(x04), .c(x17), .O(new_n132_));
  oai21  g099(.a(new_n132_), .b(x22), .c(new_n82_), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n131_), .c(x08), .O(new_n134_));
  inv1   g101(.a(x04), .O(new_n135_));
  aoi21  g102(.a(x23), .b(new_n135_), .c(new_n79_), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(new_n62_), .c(x09), .O(new_n137_));
  aoi21  g104(.a(new_n137_), .b(x16), .c(new_n112_), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n130_), .c(new_n134_), .d(new_n129_), .O(z4));
  aoi21  g106(.a(x20), .b(new_n38_), .c(x06), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(x11), .c(new_n35_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(z5), .O(new_n142_));
  aoi21  g109(.a(new_n39_), .b(x14), .c(new_n36_), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(new_n37_), .c(x03), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(z7), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(new_n142_), .O(z6));
endmodule


