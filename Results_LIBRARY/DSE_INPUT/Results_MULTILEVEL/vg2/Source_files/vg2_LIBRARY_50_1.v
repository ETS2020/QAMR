// Benchmark "FAU" written by ABC on Mon Jul 27 19:38:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_;
  inv1   g000(.a(x06), .O(new_n34_));
  inv1   g001(.a(x11), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x24), .O(new_n37_));
  nand3  g004(.a(new_n37_), .b(x13), .c(x05), .O(new_n38_));
  oai21  g005(.a(x13), .b(x05), .c(new_n38_), .O(new_n39_));
  inv1   g006(.a(x02), .O(new_n40_));
  inv1   g007(.a(x10), .O(new_n41_));
  nand4  g008(.a(new_n37_), .b(new_n41_), .c(new_n40_), .d(x00), .O(new_n42_));
  nand2  g009(.a(x24), .b(x07), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  aoi21  g011(.a(new_n39_), .b(x19), .c(new_n44_), .O(new_n45_));
  nor2   g012(.a(new_n45_), .b(x20), .O(new_n46_));
  nand4  g013(.a(new_n46_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n47_));
  nor3   g014(.a(new_n47_), .b(x03), .c(x01), .O(z0));
  inv1   g015(.a(x12), .O(new_n49_));
  nand3  g016(.a(x15), .b(x13), .c(x05), .O(new_n50_));
  nand3  g017(.a(x21), .b(new_n41_), .c(new_n40_), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(x23), .c(x22), .d(x20), .O(new_n53_));
  inv1   g020(.a(new_n53_), .O(new_n54_));
  nand4  g021(.a(new_n54_), .b(x17), .c(x16), .d(x14), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n49_), .O(new_n56_));
  nand4  g023(.a(new_n56_), .b(x11), .c(x09), .d(x08), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n34_), .O(new_n58_));
  nand4  g025(.a(new_n58_), .b(x04), .c(x03), .d(x01), .O(new_n59_));
  inv1   g026(.a(x01), .O(new_n60_));
  inv1   g027(.a(x03), .O(new_n61_));
  inv1   g028(.a(x04), .O(new_n62_));
  inv1   g029(.a(x08), .O(new_n63_));
  inv1   g030(.a(x09), .O(new_n64_));
  inv1   g031(.a(x16), .O(new_n65_));
  inv1   g032(.a(x17), .O(new_n66_));
  inv1   g033(.a(x20), .O(new_n67_));
  inv1   g034(.a(x22), .O(new_n68_));
  inv1   g035(.a(x23), .O(new_n69_));
  nand3  g036(.a(new_n41_), .b(new_n40_), .c(x00), .O(new_n70_));
  nand3  g037(.a(x19), .b(x13), .c(x05), .O(new_n71_));
  nand2  g038(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n73_));
  inv1   g040(.a(new_n73_), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n66_), .c(new_n65_), .d(new_n36_), .O(new_n75_));
  nor2   g042(.a(new_n75_), .b(x12), .O(new_n76_));
  nand4  g043(.a(new_n76_), .b(new_n35_), .c(new_n64_), .d(new_n63_), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(x06), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n59_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n37_), .O(new_n81_));
  inv1   g048(.a(x05), .O(new_n82_));
  inv1   g049(.a(x13), .O(new_n83_));
  nand3  g050(.a(x15), .b(new_n83_), .c(new_n82_), .O(new_n84_));
  nand2  g051(.a(x24), .b(x18), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(x23), .c(x22), .d(x20), .O(new_n87_));
  inv1   g054(.a(new_n87_), .O(new_n88_));
  nand4  g055(.a(new_n88_), .b(x17), .c(x16), .d(x14), .O(new_n89_));
  nor2   g056(.a(new_n89_), .b(new_n49_), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(x11), .c(x09), .d(x08), .O(new_n91_));
  nor2   g058(.a(new_n91_), .b(new_n34_), .O(new_n92_));
  nand4  g059(.a(new_n92_), .b(x04), .c(x03), .d(x01), .O(new_n93_));
  nand3  g060(.a(x19), .b(new_n83_), .c(new_n82_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n43_), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n66_), .c(new_n65_), .d(new_n36_), .O(new_n98_));
  nor2   g065(.a(new_n98_), .b(x12), .O(new_n99_));
  nand4  g066(.a(new_n99_), .b(new_n35_), .c(new_n64_), .d(new_n63_), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(x06), .O(new_n101_));
  nand4  g068(.a(new_n101_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n102_));
  and2   g069(.a(new_n102_), .b(new_n93_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n81_), .O(z1));
  nand4  g071(.a(new_n37_), .b(x21), .c(new_n41_), .d(new_n40_), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n85_), .O(new_n106_));
  aoi21  g073(.a(new_n39_), .b(x15), .c(new_n106_), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(new_n67_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(x14), .c(x11), .d(x06), .O(new_n109_));
  nor3   g076(.a(new_n109_), .b(new_n61_), .c(new_n60_), .O(z2));
  nand4  g077(.a(new_n52_), .b(x20), .c(x14), .d(x11), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n63_), .O(new_n112_));
  nand4  g079(.a(new_n112_), .b(x06), .c(x03), .d(x01), .O(new_n113_));
  nand4  g080(.a(new_n72_), .b(new_n67_), .c(new_n36_), .d(new_n35_), .O(new_n114_));
  nor2   g081(.a(new_n114_), .b(x08), .O(new_n115_));
  nand4  g082(.a(new_n115_), .b(new_n34_), .c(new_n61_), .d(new_n60_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n37_), .O(new_n118_));
  nand4  g085(.a(new_n86_), .b(x20), .c(x14), .d(x11), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n63_), .O(new_n120_));
  nand4  g087(.a(new_n120_), .b(x06), .c(x03), .d(x01), .O(new_n121_));
  nand4  g088(.a(new_n95_), .b(new_n67_), .c(new_n36_), .d(new_n35_), .O(new_n122_));
  nor2   g089(.a(new_n122_), .b(x08), .O(new_n123_));
  nand4  g090(.a(new_n123_), .b(new_n34_), .c(new_n61_), .d(new_n60_), .O(new_n124_));
  nand3  g091(.a(new_n124_), .b(new_n121_), .c(new_n118_), .O(z3));
  aoi21  g092(.a(new_n69_), .b(x04), .c(x17), .O(new_n126_));
  oai21  g093(.a(new_n126_), .b(x22), .c(new_n64_), .O(new_n127_));
  aoi21  g094(.a(new_n127_), .b(new_n65_), .c(x08), .O(new_n128_));
  aoi21  g095(.a(x23), .b(new_n62_), .c(new_n66_), .O(new_n129_));
  oai21  g096(.a(new_n129_), .b(new_n68_), .c(x09), .O(new_n130_));
  aoi21  g097(.a(new_n130_), .b(x16), .c(new_n63_), .O(new_n131_));
  oai22  g098(.a(new_n131_), .b(new_n107_), .c(new_n128_), .d(new_n45_), .O(z4));
  inv1   g099(.a(new_n45_), .O(z5));
  nand2  g100(.a(x20), .b(new_n36_), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n34_), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(new_n35_), .c(x03), .O(new_n136_));
  nand2  g103(.a(new_n67_), .b(x14), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(x06), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(x11), .c(new_n61_), .O(new_n139_));
  oai22  g106(.a(new_n139_), .b(new_n107_), .c(new_n136_), .d(new_n45_), .O(z6));
  inv1   g107(.a(new_n107_), .O(z7));
endmodule


