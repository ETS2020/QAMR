// Benchmark "FAU" written by ABC on Mon Jul 27 18:37:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x01), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n32_), .O(z00));
  nand2  g06(.a(new_n32_), .b(x12), .O(z01));
  inv1   g07(.a(new_n30_), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(x08), .c(x07), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g11(.a(new_n37_), .b(x11), .c(x08), .d(x07), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(x12), .c(x11), .d(x09), .O(z03));
  inv1   g13(.a(x14), .O(z04));
  inv1   g14(.a(x13), .O(z05));
  inv1   g15(.a(x09), .O(new_n45_));
  nand2  g16(.a(x12), .b(new_n33_), .O(new_n46_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x01), .O(new_n48_));
  aoi21  g19(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(z06));
  inv1   g20(.a(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  inv1   g22(.a(x12), .O(new_n52_));
  nand3  g23(.a(x12), .b(x11), .c(x09), .O(new_n53_));
  nor2   g24(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  aoi21  g25(.a(new_n52_), .b(new_n33_), .c(new_n54_), .O(new_n55_));
  nand4  g26(.a(new_n55_), .b(new_n34_), .c(new_n51_), .d(x00), .O(z08));
  nand2  g27(.a(new_n52_), .b(new_n33_), .O(new_n57_));
  and2   g28(.a(x03), .b(x02), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(new_n47_), .c(x09), .d(x04), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(x12), .c(x11), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n51_), .c(x00), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(z09));
  inv1   g34(.a(x00), .O(new_n64_));
  nand4  g35(.a(new_n47_), .b(x12), .c(x11), .d(new_n51_), .O(new_n65_));
  nor3   g36(.a(new_n65_), .b(new_n45_), .c(new_n64_), .O(z10));
  nand3  g37(.a(new_n47_), .b(x12), .c(x09), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x01), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x11), .c(new_n51_), .d(x00), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(z11));
  nand4  g41(.a(x11), .b(new_n51_), .c(new_n45_), .d(x00), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n52_), .O(z12));
endmodule


