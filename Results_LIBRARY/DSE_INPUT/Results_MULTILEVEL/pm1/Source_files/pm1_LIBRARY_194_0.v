// Benchmark "FAU" written by ABC on Mon Jul 27 18:37:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n51_, new_n52_, new_n53_, new_n54_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  inv1   g02(.a(x10), .O(new_n32_));
  nand4  g03(.a(new_n31_), .b(new_n32_), .c(new_n30_), .d(x00), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n32_), .c(x00), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g07(.a(x08), .O(new_n37_));
  and2   g08(.a(x06), .b(x05), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n37_), .c(x07), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x11), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x12), .c(x08), .O(z01));
  nand3  g12(.a(x07), .b(x06), .c(x05), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x11), .O(new_n43_));
  inv1   g14(.a(x11), .O(new_n44_));
  nor2   g15(.a(new_n31_), .b(new_n44_), .O(new_n45_));
  nand4  g16(.a(new_n45_), .b(new_n43_), .c(x09), .d(x08), .O(z02));
  nand4  g17(.a(new_n38_), .b(x11), .c(x08), .d(x07), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(new_n45_), .c(x09), .O(z03));
  inv1   g19(.a(x14), .O(z04));
  inv1   g20(.a(x13), .O(z05));
  and2   g21(.a(x03), .b(x02), .O(new_n51_));
  nand2  g22(.a(x12), .b(new_n44_), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(new_n51_), .c(x04), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(x09), .c(x01), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(z06));
  inv1   g26(.a(x15), .O(z07));
  and2   g27(.a(x09), .b(x04), .O(new_n57_));
  nor2   g28(.a(new_n31_), .b(x10), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(new_n57_), .c(new_n51_), .d(x00), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(x11), .c(new_n32_), .d(x00), .O(z08));
  nand3  g31(.a(x04), .b(x03), .c(x02), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n51_), .c(x09), .d(x04), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(x12), .c(x11), .O(new_n63_));
  nand2  g34(.a(new_n31_), .b(new_n44_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(new_n32_), .c(x00), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(z09));
  inv1   g38(.a(x00), .O(new_n68_));
  inv1   g39(.a(x09), .O(new_n69_));
  nand4  g40(.a(new_n61_), .b(x12), .c(x11), .d(new_n32_), .O(new_n70_));
  nor3   g41(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(z10));
  nand4  g42(.a(new_n61_), .b(x12), .c(x09), .d(x01), .O(new_n72_));
  oai21  g43(.a(x12), .b(x01), .c(new_n72_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x11), .c(new_n32_), .d(x00), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(z11));
  nand4  g46(.a(x11), .b(new_n32_), .c(new_n69_), .d(x00), .O(new_n76_));
  nor2   g47(.a(new_n76_), .b(new_n31_), .O(z12));
endmodule


