// Benchmark "FAU" written by ABC on Mon Jul 27 18:36:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  nor2   g03(.a(x12), .b(x01), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n32_), .O(z00));
  nand2  g05(.a(new_n32_), .b(x12), .O(z01));
  nand2  g06(.a(x08), .b(x07), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n30_), .c(x11), .O(new_n37_));
  inv1   g08(.a(x11), .O(new_n38_));
  inv1   g09(.a(x12), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n37_), .c(x09), .O(z02));
  inv1   g12(.a(new_n30_), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(x11), .c(x08), .d(x07), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n40_), .c(x09), .O(z03));
  inv1   g15(.a(x14), .O(z04));
  inv1   g16(.a(x13), .O(z05));
  and2   g17(.a(x03), .b(x02), .O(new_n47_));
  nand2  g18(.a(x12), .b(new_n38_), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(new_n47_), .c(x04), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(z06));
  inv1   g22(.a(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand4  g24(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  nand4  g26(.a(new_n55_), .b(new_n40_), .c(new_n53_), .d(x09), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(x11), .c(new_n53_), .d(x00), .O(z08));
  nand3  g28(.a(x04), .b(x03), .c(x02), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(new_n47_), .c(x09), .d(x04), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(x12), .c(x11), .O(new_n60_));
  nand2  g31(.a(new_n39_), .b(new_n38_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(new_n53_), .c(x00), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(z09));
  inv1   g35(.a(x00), .O(new_n65_));
  inv1   g36(.a(x09), .O(new_n66_));
  nand4  g37(.a(new_n58_), .b(x12), .c(x11), .d(new_n53_), .O(new_n67_));
  nor3   g38(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(z10));
  nand4  g39(.a(new_n58_), .b(x12), .c(x09), .d(x01), .O(new_n69_));
  oai21  g40(.a(x12), .b(x01), .c(new_n69_), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(x11), .c(new_n53_), .d(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z11));
  nand4  g43(.a(x11), .b(new_n53_), .c(new_n66_), .d(x00), .O(new_n73_));
  nor2   g44(.a(new_n73_), .b(new_n39_), .O(z12));
endmodule


