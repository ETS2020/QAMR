// Benchmark "FAU" written by ABC on Mon Jul 27 18:37:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n47_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  aoi21  g04(.a(x12), .b(new_n33_), .c(x01), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n32_), .O(z00));
  nand2  g06(.a(new_n32_), .b(x12), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand3  g10(.a(x11), .b(x08), .c(x07), .O(new_n40_));
  or2    g11(.a(new_n40_), .b(new_n30_), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(x12), .c(x11), .d(x09), .O(z03));
  inv1   g13(.a(x14), .O(z04));
  inv1   g14(.a(x13), .O(z05));
  and2   g15(.a(x09), .b(x01), .O(z06));
  inv1   g16(.a(x15), .O(z07));
  inv1   g17(.a(x10), .O(new_n47_));
  nand3  g18(.a(x11), .b(new_n47_), .c(x00), .O(z08));
  xnor2a g19(.a(x12), .b(x11), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(new_n47_), .c(x00), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(z09));
  inv1   g22(.a(x12), .O(new_n52_));
  nand4  g23(.a(x11), .b(new_n47_), .c(x09), .d(x00), .O(new_n53_));
  nor2   g24(.a(new_n53_), .b(new_n52_), .O(z10));
  nand3  g25(.a(x12), .b(x09), .c(x01), .O(new_n55_));
  oai21  g26(.a(x12), .b(x01), .c(new_n55_), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(x11), .c(new_n47_), .d(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(z11));
  inv1   g29(.a(x09), .O(new_n59_));
  nand4  g30(.a(x11), .b(new_n47_), .c(new_n59_), .d(x00), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n52_), .O(z12));
endmodule


