// Benchmark "FAU" written by ABC on Mon Jul 27 18:27:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_;
  nor2   g000(.a(x19), .b(x17), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n52_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(new_n52_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  aoi21  g011(.a(new_n61_), .b(x20), .c(new_n62_), .O(new_n63_));
  inv1   g012(.a(x14), .O(new_n64_));
  aoi21  g013(.a(new_n58_), .b(new_n64_), .c(x18), .O(new_n65_));
  oai21  g014(.a(new_n63_), .b(new_n58_), .c(new_n65_), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  inv1   g016(.a(x20), .O(new_n68_));
  nand4  g017(.a(new_n67_), .b(new_n68_), .c(new_n54_), .d(new_n53_), .O(new_n69_));
  oai21  g018(.a(new_n62_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x16), .O(new_n71_));
  inv1   g020(.a(x13), .O(new_n72_));
  aoi21  g021(.a(new_n58_), .b(new_n72_), .c(x18), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n71_), .O(z02));
  nor3   g023(.a(x22), .b(x21), .c(x20), .O(new_n75_));
  nand3  g024(.a(new_n75_), .b(new_n54_), .c(new_n53_), .O(new_n76_));
  nand3  g025(.a(new_n52_), .b(new_n67_), .c(new_n68_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(x22), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n76_), .c(new_n58_), .O(new_n79_));
  inv1   g028(.a(x18), .O(new_n80_));
  oai21  g029(.a(x16), .b(x12), .c(new_n80_), .O(new_n81_));
  or2    g030(.a(new_n81_), .b(new_n79_), .O(z03));
  inv1   g031(.a(x23), .O(new_n83_));
  nor3   g032(.a(x23), .b(x22), .c(x21), .O(new_n84_));
  nand4  g033(.a(new_n84_), .b(new_n68_), .c(new_n54_), .d(new_n53_), .O(new_n85_));
  nand2  g034(.a(new_n75_), .b(new_n52_), .O(new_n86_));
  inv1   g035(.a(new_n86_), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(new_n83_), .c(new_n85_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(x16), .O(new_n89_));
  inv1   g038(.a(x11), .O(new_n90_));
  aoi21  g039(.a(new_n58_), .b(new_n90_), .c(x18), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n89_), .O(z04));
  inv1   g041(.a(x22), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n93_), .c(new_n67_), .d(new_n68_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(x19), .O(new_n96_));
  inv1   g045(.a(x24), .O(new_n97_));
  aoi21  g046(.a(new_n84_), .b(new_n62_), .c(new_n97_), .O(new_n98_));
  aoi21  g047(.a(new_n96_), .b(new_n53_), .c(new_n98_), .O(new_n99_));
  inv1   g048(.a(x10), .O(new_n100_));
  aoi21  g049(.a(new_n58_), .b(new_n100_), .c(x18), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n58_), .c(new_n101_), .O(z05));
  nor3   g051(.a(x25), .b(x24), .c(x23), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n93_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(x21), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n68_), .c(new_n54_), .d(new_n53_), .O(new_n106_));
  nand4  g055(.a(new_n94_), .b(new_n62_), .c(new_n93_), .d(new_n67_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(x25), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(x16), .O(new_n110_));
  inv1   g059(.a(x09), .O(new_n111_));
  aoi21  g060(.a(new_n58_), .b(new_n111_), .c(x18), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n110_), .O(z06));
  nor2   g062(.a(x26), .b(x25), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n97_), .c(new_n83_), .d(new_n93_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(x21), .O(new_n116_));
  nand4  g065(.a(new_n116_), .b(new_n68_), .c(new_n54_), .d(new_n53_), .O(new_n117_));
  inv1   g066(.a(x25), .O(new_n118_));
  nor2   g067(.a(x23), .b(x22), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n118_), .c(new_n97_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n77_), .c(x26), .O(new_n121_));
  aoi21  g070(.a(new_n121_), .b(new_n117_), .c(new_n58_), .O(new_n122_));
  oai21  g071(.a(x16), .b(x08), .c(new_n80_), .O(new_n123_));
  or2    g072(.a(new_n123_), .b(new_n122_), .O(z07));
  nor2   g073(.a(x27), .b(x26), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n118_), .c(new_n97_), .d(new_n83_), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(x22), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n67_), .c(new_n68_), .d(new_n54_), .O(new_n128_));
  nand3  g077(.a(new_n114_), .b(new_n119_), .c(new_n97_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n77_), .c(x27), .O(new_n130_));
  oai21  g079(.a(new_n128_), .b(x17), .c(new_n130_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(x16), .O(new_n132_));
  inv1   g081(.a(x07), .O(new_n133_));
  aoi21  g082(.a(new_n58_), .b(new_n133_), .c(x18), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(z08));
  nand3  g084(.a(new_n125_), .b(new_n94_), .c(new_n118_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n86_), .c(x28), .O(new_n137_));
  nor3   g086(.a(x28), .b(x27), .c(x26), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n103_), .c(new_n87_), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n137_), .c(new_n58_), .O(new_n140_));
  oai21  g089(.a(x16), .b(x06), .c(new_n80_), .O(new_n141_));
  or2    g090(.a(new_n141_), .b(new_n140_), .O(z09));
  inv1   g091(.a(x26), .O(new_n143_));
  nor2   g092(.a(x28), .b(x27), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n143_), .c(new_n118_), .d(new_n97_), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(x23), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n93_), .c(new_n67_), .d(new_n68_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n54_), .c(new_n53_), .O(new_n148_));
  nand2  g097(.a(new_n61_), .b(x29), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  inv1   g100(.a(x05), .O(new_n152_));
  aoi21  g101(.a(new_n58_), .b(new_n152_), .c(x18), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n151_), .O(z10));
  nand2  g103(.a(x30), .b(x16), .O(new_n155_));
  inv1   g104(.a(x04), .O(new_n156_));
  nand2  g105(.a(new_n58_), .b(new_n156_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n155_), .c(new_n80_), .O(z11));
  nand2  g107(.a(x31), .b(x16), .O(new_n159_));
  inv1   g108(.a(x03), .O(new_n160_));
  nand2  g109(.a(new_n58_), .b(new_n160_), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n159_), .c(new_n80_), .O(z12));
  nand2  g111(.a(x32), .b(x16), .O(new_n163_));
  inv1   g112(.a(x02), .O(new_n164_));
  nand2  g113(.a(new_n58_), .b(new_n164_), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n163_), .c(new_n80_), .O(z13));
  nand2  g115(.a(x33), .b(x16), .O(new_n167_));
  inv1   g116(.a(x01), .O(new_n168_));
  nand2  g117(.a(new_n58_), .b(new_n168_), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n167_), .c(new_n80_), .O(z14));
  nand2  g119(.a(x34), .b(x16), .O(new_n171_));
  inv1   g120(.a(x00), .O(new_n172_));
  nand2  g121(.a(new_n58_), .b(new_n172_), .O(new_n173_));
  nand3  g122(.a(new_n173_), .b(new_n171_), .c(new_n80_), .O(z15));
endmodule


