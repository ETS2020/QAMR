// Benchmark "FAU" written by ABC on Mon Jul 27 18:26:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(new_n55_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  aoi21  g011(.a(new_n61_), .b(x20), .c(new_n62_), .O(new_n63_));
  inv1   g012(.a(x14), .O(new_n64_));
  aoi21  g013(.a(new_n58_), .b(new_n64_), .c(x18), .O(new_n65_));
  oai21  g014(.a(new_n63_), .b(new_n58_), .c(new_n65_), .O(z01));
  inv1   g015(.a(x20), .O(new_n67_));
  nand3  g016(.a(new_n67_), .b(new_n53_), .c(new_n52_), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  aoi22  g018(.a(new_n69_), .b(new_n55_), .c(new_n68_), .d(x21), .O(new_n70_));
  inv1   g019(.a(x13), .O(new_n71_));
  aoi21  g020(.a(new_n58_), .b(new_n71_), .c(x18), .O(new_n72_));
  oai21  g021(.a(new_n70_), .b(new_n58_), .c(new_n72_), .O(z02));
  inv1   g022(.a(x21), .O(new_n74_));
  inv1   g023(.a(x23), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x22), .O(new_n76_));
  nand4  g025(.a(new_n76_), .b(new_n74_), .c(new_n67_), .d(new_n52_), .O(new_n77_));
  nand2  g026(.a(x22), .b(x21), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  inv1   g028(.a(x22), .O(new_n80_));
  nor2   g029(.a(new_n62_), .b(new_n80_), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n79_), .c(x16), .O(new_n82_));
  inv1   g031(.a(x12), .O(new_n83_));
  aoi21  g032(.a(new_n58_), .b(new_n83_), .c(x18), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n82_), .O(z03));
  nand2  g034(.a(x23), .b(x21), .O(new_n86_));
  nor2   g035(.a(x23), .b(x22), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n74_), .c(new_n67_), .d(new_n52_), .O(new_n88_));
  aoi21  g037(.a(new_n88_), .b(new_n86_), .c(x19), .O(new_n89_));
  nor2   g038(.a(new_n62_), .b(new_n75_), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  inv1   g040(.a(x11), .O(new_n92_));
  aoi21  g041(.a(new_n58_), .b(new_n92_), .c(x18), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n91_), .O(z04));
  nor2   g043(.a(x22), .b(x21), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(x19), .O(new_n96_));
  nor2   g045(.a(x23), .b(x20), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n55_), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(new_n96_), .c(x24), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n95_), .c(new_n62_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(x16), .O(new_n103_));
  inv1   g052(.a(x10), .O(new_n104_));
  aoi21  g053(.a(new_n58_), .b(new_n104_), .c(x18), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n103_), .O(z05));
  inv1   g055(.a(new_n96_), .O(new_n107_));
  nand2  g056(.a(new_n100_), .b(new_n67_), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n61_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nor2   g059(.a(x25), .b(x24), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n87_), .c(new_n69_), .d(new_n55_), .O(new_n112_));
  inv1   g061(.a(new_n112_), .O(new_n113_));
  aoi21  g062(.a(new_n110_), .b(x25), .c(new_n113_), .O(new_n114_));
  inv1   g063(.a(x09), .O(new_n115_));
  aoi21  g064(.a(new_n58_), .b(new_n115_), .c(x18), .O(new_n116_));
  oai21  g065(.a(new_n114_), .b(new_n58_), .c(new_n116_), .O(z06));
  nand2  g066(.a(new_n111_), .b(new_n75_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n68_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n107_), .O(new_n120_));
  nand2  g069(.a(new_n69_), .b(new_n55_), .O(new_n121_));
  inv1   g070(.a(x24), .O(new_n122_));
  nor2   g071(.a(x26), .b(x25), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n87_), .c(new_n122_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  aoi21  g074(.a(new_n120_), .b(x26), .c(new_n125_), .O(new_n126_));
  inv1   g075(.a(x08), .O(new_n127_));
  aoi21  g076(.a(new_n58_), .b(new_n127_), .c(x18), .O(new_n128_));
  oai21  g077(.a(new_n126_), .b(new_n58_), .c(new_n128_), .O(z07));
  nand2  g078(.a(new_n123_), .b(new_n100_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n68_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n107_), .O(new_n132_));
  nand3  g081(.a(new_n95_), .b(new_n55_), .c(new_n67_), .O(new_n133_));
  inv1   g082(.a(x25), .O(new_n134_));
  nor2   g083(.a(x27), .b(x26), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n100_), .c(new_n134_), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  aoi21  g086(.a(new_n132_), .b(x27), .c(new_n137_), .O(new_n138_));
  inv1   g087(.a(x07), .O(new_n139_));
  aoi21  g088(.a(new_n58_), .b(new_n139_), .c(x18), .O(new_n140_));
  oai21  g089(.a(new_n138_), .b(new_n58_), .c(new_n140_), .O(z08));
  nand4  g090(.a(new_n135_), .b(new_n111_), .c(new_n97_), .d(new_n55_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n96_), .c(x28), .O(new_n143_));
  inv1   g092(.a(x26), .O(new_n144_));
  nor2   g093(.a(x28), .b(x27), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n111_), .c(new_n75_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n133_), .c(new_n143_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  inv1   g099(.a(x06), .O(new_n151_));
  aoi21  g100(.a(new_n58_), .b(new_n151_), .c(x18), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(z09));
  inv1   g102(.a(new_n98_), .O(new_n154_));
  nand4  g103(.a(new_n147_), .b(new_n111_), .c(new_n154_), .d(new_n107_), .O(new_n155_));
  nand3  g104(.a(new_n87_), .b(new_n62_), .c(new_n74_), .O(new_n156_));
  inv1   g105(.a(x27), .O(new_n157_));
  nor2   g106(.a(x29), .b(x28), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n123_), .c(new_n157_), .d(new_n122_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  aoi21  g109(.a(new_n155_), .b(x29), .c(new_n160_), .O(new_n161_));
  inv1   g110(.a(x05), .O(new_n162_));
  aoi21  g111(.a(new_n58_), .b(new_n162_), .c(x18), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n58_), .c(new_n163_), .O(z10));
  inv1   g113(.a(new_n123_), .O(new_n165_));
  nand2  g114(.a(new_n158_), .b(new_n157_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n109_), .c(new_n107_), .O(new_n168_));
  nor2   g117(.a(x30), .b(x29), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n145_), .c(new_n123_), .d(new_n122_), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n156_), .O(new_n171_));
  aoi21  g120(.a(new_n168_), .b(x30), .c(new_n171_), .O(new_n172_));
  inv1   g121(.a(x04), .O(new_n173_));
  aoi21  g122(.a(new_n58_), .b(new_n173_), .c(x18), .O(new_n174_));
  oai21  g123(.a(new_n172_), .b(new_n58_), .c(new_n174_), .O(z11));
  nand2  g124(.a(new_n135_), .b(new_n134_), .O(new_n176_));
  inv1   g125(.a(x28), .O(new_n177_));
  nand2  g126(.a(new_n169_), .b(new_n177_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n176_), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n109_), .c(new_n107_), .O(new_n180_));
  nor2   g129(.a(x31), .b(x30), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n158_), .c(new_n135_), .d(new_n134_), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n101_), .O(new_n183_));
  aoi21  g132(.a(new_n180_), .b(x31), .c(new_n183_), .O(new_n184_));
  inv1   g133(.a(x03), .O(new_n185_));
  aoi21  g134(.a(new_n58_), .b(new_n185_), .c(x18), .O(new_n186_));
  oai21  g135(.a(new_n184_), .b(new_n58_), .c(new_n186_), .O(z12));
  inv1   g136(.a(x29), .O(new_n188_));
  nand2  g137(.a(new_n181_), .b(new_n188_), .O(new_n189_));
  nor2   g138(.a(new_n189_), .b(new_n146_), .O(new_n190_));
  nand3  g139(.a(new_n190_), .b(new_n119_), .c(new_n107_), .O(new_n191_));
  nor2   g140(.a(x32), .b(x31), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n169_), .c(new_n145_), .d(new_n123_), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n101_), .O(new_n194_));
  aoi21  g143(.a(new_n191_), .b(x32), .c(new_n194_), .O(new_n195_));
  inv1   g144(.a(x02), .O(new_n196_));
  aoi21  g145(.a(new_n58_), .b(new_n196_), .c(x18), .O(new_n197_));
  oai21  g146(.a(new_n195_), .b(new_n58_), .c(new_n197_), .O(z13));
  nand2  g147(.a(new_n192_), .b(new_n169_), .O(new_n199_));
  nor2   g148(.a(new_n199_), .b(new_n146_), .O(new_n200_));
  nand3  g149(.a(new_n200_), .b(new_n119_), .c(new_n107_), .O(new_n201_));
  nand2  g150(.a(new_n158_), .b(new_n135_), .O(new_n202_));
  nor2   g151(.a(x33), .b(x32), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n181_), .O(new_n204_));
  nor3   g153(.a(new_n204_), .b(new_n202_), .c(new_n112_), .O(new_n205_));
  aoi21  g154(.a(new_n201_), .b(x33), .c(new_n205_), .O(new_n206_));
  inv1   g155(.a(x01), .O(new_n207_));
  aoi21  g156(.a(new_n58_), .b(new_n207_), .c(x18), .O(new_n208_));
  oai21  g157(.a(new_n206_), .b(new_n58_), .c(new_n208_), .O(z14));
  nor2   g158(.a(new_n204_), .b(new_n166_), .O(new_n210_));
  nand3  g159(.a(new_n210_), .b(new_n131_), .c(new_n107_), .O(new_n211_));
  inv1   g160(.a(x32), .O(new_n212_));
  nor2   g161(.a(x34), .b(x33), .O(new_n213_));
  nand3  g162(.a(new_n213_), .b(new_n181_), .c(new_n212_), .O(new_n214_));
  nor3   g163(.a(new_n214_), .b(new_n202_), .c(new_n112_), .O(new_n215_));
  aoi21  g164(.a(new_n211_), .b(x34), .c(new_n215_), .O(new_n216_));
  inv1   g165(.a(x00), .O(new_n217_));
  aoi21  g166(.a(new_n58_), .b(new_n217_), .c(x18), .O(new_n218_));
  oai21  g167(.a(new_n216_), .b(new_n58_), .c(new_n218_), .O(z15));
endmodule


