// Benchmark "FAU" written by ABC on Mon Jul 27 17:45:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n79_, new_n80_, new_n82_, new_n84_, new_n87_,
    new_n89_, new_n90_;
  inv1   g00(.a(x19), .O(new_n42_));
  inv1   g01(.a(x01), .O(new_n43_));
  inv1   g02(.a(x02), .O(new_n44_));
  inv1   g03(.a(x03), .O(new_n45_));
  inv1   g04(.a(x04), .O(new_n46_));
  nand4  g05(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nor2   g07(.a(x06), .b(x05), .O(new_n49_));
  nor3   g08(.a(x09), .b(x08), .c(x07), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  aoi21  g10(.a(new_n51_), .b(x00), .c(new_n42_), .O(z01));
  inv1   g11(.a(x05), .O(new_n53_));
  nor3   g12(.a(x03), .b(x02), .c(x01), .O(new_n54_));
  nor2   g13(.a(x07), .b(x06), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(new_n54_), .c(new_n53_), .d(new_n46_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g16(.a(x07), .O(new_n58_));
  nand4  g17(.a(new_n49_), .b(new_n48_), .c(x08), .d(new_n58_), .O(new_n59_));
  aoi21  g18(.a(new_n59_), .b(new_n57_), .c(new_n42_), .O(z02));
  or2    g19(.a(x21), .b(x20), .O(z04));
  inv1   g20(.a(x10), .O(new_n62_));
  inv1   g21(.a(x06), .O(new_n63_));
  inv1   g22(.a(x09), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(x08), .O(new_n65_));
  nand4  g24(.a(new_n65_), .b(x19), .c(new_n58_), .d(new_n63_), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(x05), .O(new_n67_));
  nand4  g26(.a(new_n67_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(x01), .c(new_n62_), .O(z05));
  nor2   g28(.a(x07), .b(x06), .O(new_n70_));
  nand4  g29(.a(new_n70_), .b(new_n53_), .c(new_n46_), .d(new_n45_), .O(new_n71_));
  inv1   g30(.a(new_n71_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(new_n44_), .c(new_n43_), .O(new_n73_));
  inv1   g32(.a(x00), .O(new_n74_));
  nor2   g33(.a(new_n56_), .b(new_n74_), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n73_), .c(new_n42_), .O(z06));
  and2   g35(.a(x24), .b(x18), .O(z07));
  and2   g36(.a(x24), .b(x11), .O(z09));
  inv1   g37(.a(x14), .O(new_n79_));
  inv1   g38(.a(x22), .O(new_n80_));
  nor3   g39(.a(x24), .b(new_n80_), .c(new_n79_), .O(z10));
  inv1   g40(.a(x17), .O(new_n82_));
  nor3   g41(.a(x24), .b(new_n80_), .c(new_n82_), .O(z11));
  inv1   g42(.a(x23), .O(new_n84_));
  nor3   g43(.a(x24), .b(new_n84_), .c(new_n79_), .O(z12));
  nor3   g44(.a(x24), .b(new_n84_), .c(new_n82_), .O(z13));
  inv1   g45(.a(x16), .O(new_n87_));
  nor2   g46(.a(x24), .b(new_n87_), .O(z14));
  nor2   g47(.a(x15), .b(x14), .O(new_n89_));
  nor2   g48(.a(x13), .b(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(z15));
  buf1   g50(.a(x00), .O(z00));
  buf1   g51(.a(x20), .O(z03));
  buf1   g52(.a(x11), .O(z08));
endmodule


