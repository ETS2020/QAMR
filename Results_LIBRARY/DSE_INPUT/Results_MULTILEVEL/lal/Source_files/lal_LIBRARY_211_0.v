// Benchmark "FAU" written by ABC on Mon Jul 27 18:47:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n63_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x07), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(new_n51_), .c(new_n48_), .O(z01));
  inv1   g08(.a(new_n51_), .O(z03));
  xor2a  g09(.a(x09), .b(x00), .O(new_n55_));
  xor2a  g10(.a(x10), .b(x01), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  xor2a  g12(.a(x11), .b(x02), .O(new_n58_));
  xor2a  g13(.a(x12), .b(x03), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x08), .O(z04));
  nor2   g16(.a(x13), .b(x08), .O(z05));
  inv1   g17(.a(x14), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x08), .O(z06));
  nand2  g19(.a(new_n46_), .b(x06), .O(z07));
  inv1   g20(.a(x20), .O(new_n66_));
  inv1   g21(.a(x21), .O(new_n67_));
  inv1   g22(.a(x22), .O(new_n68_));
  inv1   g23(.a(x23), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  inv1   g25(.a(new_n70_), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(x19), .c(x18), .d(x17), .O(new_n72_));
  nand2  g27(.a(new_n51_), .b(x23), .O(new_n73_));
  nand3  g28(.a(x19), .b(x18), .c(x17), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(new_n75_));
  nor2   g30(.a(x22), .b(x21), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n75_), .c(new_n66_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x25), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n73_), .c(new_n72_), .O(z08));
  inv1   g34(.a(x15), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n48_), .c(x05), .d(x04), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  inv1   g37(.a(x17), .O(new_n83_));
  nand4  g38(.a(new_n52_), .b(new_n83_), .c(new_n80_), .d(new_n48_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z10));
  xor2a  g40(.a(x18), .b(x17), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n52_), .c(new_n80_), .d(new_n48_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z11));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(x19), .O(new_n90_));
  inv1   g45(.a(x19), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(x18), .c(x17), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n52_), .c(new_n80_), .d(new_n48_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z12));
  nand2  g50(.a(new_n74_), .b(x20), .O(new_n96_));
  nor2   g51(.a(x15), .b(x07), .O(new_n97_));
  nand4  g52(.a(new_n66_), .b(x19), .c(x18), .d(x17), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(new_n52_), .O(z13));
  nand2  g54(.a(new_n98_), .b(x21), .O(new_n100_));
  nor2   g55(.a(x21), .b(x20), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(x19), .c(x18), .d(x17), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n100_), .c(new_n97_), .d(new_n52_), .O(z14));
  nand2  g58(.a(new_n102_), .b(x22), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n97_), .c(new_n77_), .d(new_n52_), .O(z15));
  nand3  g60(.a(new_n52_), .b(new_n80_), .c(new_n48_), .O(new_n106_));
  aoi21  g61(.a(new_n77_), .b(x23), .c(new_n106_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n72_), .O(z16));
  nor2   g63(.a(x23), .b(x22), .O(new_n109_));
  nand3  g64(.a(new_n101_), .b(new_n75_), .c(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x24), .O(new_n111_));
  inv1   g66(.a(new_n98_), .O(new_n112_));
  nor2   g67(.a(x24), .b(x23), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n76_), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n111_), .c(new_n97_), .d(new_n52_), .O(z17));
  nand2  g70(.a(new_n110_), .b(x25), .O(new_n116_));
  nor2   g71(.a(new_n49_), .b(x23), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n112_), .c(new_n76_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n116_), .c(new_n97_), .d(new_n52_), .O(z18));
  buf1   g74(.a(x16), .O(z02));
endmodule


