// Benchmark "FAU" written by ABC on Mon Jul 27 18:46:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x20), .O(new_n46_));
  inv1   g02(.a(x24), .O(new_n47_));
  inv1   g03(.a(x25), .O(new_n48_));
  oai21  g04(.a(x17), .b(x15), .c(x26), .O(new_n49_));
  nor3   g05(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand4  g06(.a(new_n50_), .b(x23), .c(x22), .d(x21), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(x19), .c(new_n45_), .d(x09), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(x08), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  inv1   g11(.a(x08), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z02));
  inv1   g15(.a(x02), .O(new_n60_));
  nor2   g16(.a(new_n56_), .b(new_n60_), .O(z03));
  inv1   g17(.a(x03), .O(new_n62_));
  nor2   g18(.a(new_n56_), .b(new_n62_), .O(z04));
  inv1   g19(.a(x04), .O(new_n64_));
  nor2   g20(.a(new_n56_), .b(new_n64_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  inv1   g27(.a(x21), .O(new_n72_));
  inv1   g28(.a(x22), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g30(.a(new_n74_), .b(x20), .c(x11), .O(new_n75_));
  inv1   g31(.a(x23), .O(new_n76_));
  nor2   g32(.a(new_n47_), .b(new_n76_), .O(new_n77_));
  inv1   g33(.a(x26), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n48_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n75_), .c(x19), .O(new_n81_));
  nand4  g37(.a(new_n81_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n82_));
  oai21  g38(.a(new_n56_), .b(new_n55_), .c(new_n82_), .O(z09));
  nand2  g39(.a(new_n74_), .b(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n80_), .c(x19), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(x20), .O(new_n86_));
  nand2  g42(.a(new_n46_), .b(x19), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand4  g44(.a(new_n88_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n58_), .O(z10));
  inv1   g46(.a(x19), .O(new_n91_));
  nand3  g47(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  nand3  g48(.a(x26), .b(x25), .c(x24), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n92_), .c(x20), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n91_), .c(x21), .O(new_n95_));
  nand3  g51(.a(new_n72_), .b(x20), .c(x19), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n98_));
  oai21  g54(.a(new_n56_), .b(new_n60_), .c(new_n98_), .O(z11));
  nor2   g55(.a(new_n46_), .b(new_n91_), .O(new_n100_));
  inv1   g56(.a(new_n100_), .O(new_n101_));
  nand2  g57(.a(x23), .b(x14), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n93_), .c(x21), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n101_), .c(x22), .O(new_n104_));
  nand3  g60(.a(new_n100_), .b(new_n73_), .c(x21), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n107_));
  oai21  g63(.a(new_n56_), .b(new_n62_), .c(new_n107_), .O(z12));
  inv1   g64(.a(x15), .O(new_n109_));
  oai21  g65(.a(new_n93_), .b(new_n109_), .c(x23), .O(new_n110_));
  nand4  g66(.a(new_n110_), .b(x22), .c(x21), .d(x20), .O(new_n111_));
  nand2  g67(.a(new_n100_), .b(new_n74_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(x23), .O(new_n113_));
  oai21  g69(.a(new_n111_), .b(new_n91_), .c(new_n113_), .O(new_n114_));
  nand4  g70(.a(new_n114_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n115_));
  oai21  g71(.a(new_n56_), .b(new_n64_), .c(new_n115_), .O(z13));
  nand3  g72(.a(x21), .b(x20), .c(x19), .O(new_n117_));
  nand2  g73(.a(new_n79_), .b(x16), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(x23), .c(x22), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n117_), .c(x24), .O(new_n120_));
  inv1   g76(.a(new_n117_), .O(new_n121_));
  nand4  g77(.a(new_n121_), .b(new_n47_), .c(x23), .d(x22), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand4  g79(.a(new_n123_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n66_), .O(z14));
  nand3  g81(.a(x26), .b(x25), .c(x17), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(x25), .O(new_n127_));
  nand4  g83(.a(new_n127_), .b(x24), .c(x23), .d(x22), .O(new_n128_));
  inv1   g84(.a(new_n128_), .O(new_n129_));
  nand4  g85(.a(new_n129_), .b(x21), .c(x20), .d(x19), .O(new_n130_));
  nand3  g86(.a(new_n121_), .b(new_n77_), .c(x22), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(x25), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand4  g89(.a(new_n133_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n134_));
  nand2  g90(.a(new_n134_), .b(new_n68_), .O(z15));
  inv1   g91(.a(x18), .O(new_n136_));
  nand4  g92(.a(x21), .b(x20), .c(x19), .d(new_n136_), .O(new_n137_));
  nand4  g93(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n138_));
  oai21  g94(.a(new_n138_), .b(new_n137_), .c(x26), .O(new_n139_));
  nand3  g95(.a(new_n77_), .b(new_n78_), .c(x25), .O(new_n140_));
  oai21  g96(.a(new_n140_), .b(new_n112_), .c(new_n139_), .O(new_n141_));
  nand4  g97(.a(new_n141_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n142_));
  nand2  g98(.a(new_n142_), .b(new_n70_), .O(z16));
endmodule


