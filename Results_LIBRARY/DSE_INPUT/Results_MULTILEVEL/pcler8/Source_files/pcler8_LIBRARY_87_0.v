// Benchmark "FAU" written by ABC on Mon Jul 27 18:45:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x20), .O(new_n46_));
  inv1   g02(.a(x24), .O(new_n47_));
  inv1   g03(.a(x25), .O(new_n48_));
  oai21  g04(.a(x18), .b(x14), .c(x26), .O(new_n49_));
  nor3   g05(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand4  g06(.a(new_n50_), .b(x23), .c(x22), .d(x21), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(x19), .c(new_n45_), .d(x09), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(x08), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  inv1   g11(.a(x08), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z02));
  inv1   g15(.a(x02), .O(new_n60_));
  nor2   g16(.a(new_n56_), .b(new_n60_), .O(z03));
  inv1   g17(.a(x03), .O(new_n62_));
  nor2   g18(.a(new_n56_), .b(new_n62_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  inv1   g23(.a(x06), .O(new_n68_));
  nor2   g24(.a(new_n56_), .b(new_n68_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  nand4  g27(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n72_));
  and2   g28(.a(x26), .b(x25), .O(new_n73_));
  nand3  g29(.a(new_n73_), .b(x24), .c(x23), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n72_), .c(x19), .O(new_n75_));
  nand4  g31(.a(new_n75_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n76_));
  oai21  g32(.a(new_n56_), .b(new_n55_), .c(new_n76_), .O(z09));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n74_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(x20), .O(new_n80_));
  nand2  g36(.a(new_n46_), .b(x19), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand4  g38(.a(new_n82_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n58_), .O(z10));
  inv1   g40(.a(x19), .O(new_n85_));
  nand3  g41(.a(x23), .b(x22), .c(x13), .O(new_n86_));
  nand3  g42(.a(x26), .b(x25), .c(x24), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n85_), .c(x21), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand3  g46(.a(new_n90_), .b(x20), .c(x19), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand4  g48(.a(new_n92_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n93_));
  oai21  g49(.a(new_n56_), .b(new_n60_), .c(new_n93_), .O(z11));
  inv1   g50(.a(x22), .O(new_n95_));
  nand3  g51(.a(x23), .b(x22), .c(x14), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n87_), .c(x22), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(x21), .c(x20), .d(x19), .O(new_n98_));
  nand3  g54(.a(x21), .b(x20), .c(x19), .O(new_n99_));
  inv1   g55(.a(new_n99_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n95_), .c(new_n98_), .O(new_n101_));
  nand4  g57(.a(new_n101_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n102_));
  oai21  g58(.a(new_n56_), .b(new_n62_), .c(new_n102_), .O(z12));
  nand2  g59(.a(x20), .b(x19), .O(new_n104_));
  nand3  g60(.a(new_n73_), .b(x24), .c(x15), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(x22), .c(x21), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n104_), .c(x23), .O(new_n107_));
  inv1   g63(.a(x23), .O(new_n108_));
  inv1   g64(.a(new_n104_), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(new_n108_), .c(x22), .d(x21), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n107_), .O(new_n111_));
  nand4  g67(.a(new_n111_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n64_), .O(z13));
  nand2  g69(.a(new_n73_), .b(x16), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(x23), .c(x22), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n99_), .c(x24), .O(new_n116_));
  nand4  g72(.a(new_n100_), .b(new_n47_), .c(x23), .d(x22), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand4  g74(.a(new_n118_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n66_), .O(z14));
  nor2   g76(.a(new_n108_), .b(new_n95_), .O(new_n121_));
  nand2  g77(.a(x26), .b(x17), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n121_), .c(x24), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n99_), .c(x25), .O(new_n124_));
  nand4  g80(.a(new_n100_), .b(new_n121_), .c(new_n48_), .d(x24), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g82(.a(new_n126_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n127_));
  oai21  g83(.a(new_n56_), .b(new_n68_), .c(new_n127_), .O(z15));
  inv1   g84(.a(x18), .O(new_n129_));
  nand2  g85(.a(x26), .b(new_n129_), .O(new_n130_));
  nand4  g86(.a(new_n130_), .b(x25), .c(x24), .d(x23), .O(new_n131_));
  nor2   g87(.a(new_n131_), .b(new_n95_), .O(new_n132_));
  nand4  g88(.a(new_n132_), .b(x21), .c(x20), .d(x19), .O(new_n133_));
  nand3  g89(.a(new_n121_), .b(x25), .c(x24), .O(new_n134_));
  oai21  g90(.a(new_n134_), .b(new_n99_), .c(x26), .O(new_n135_));
  nand2  g91(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand4  g92(.a(new_n136_), .b(new_n45_), .c(x09), .d(new_n56_), .O(new_n137_));
  nand2  g93(.a(new_n137_), .b(new_n70_), .O(z16));
endmodule


