// Benchmark "FAU" written by ABC on Mon Jul 27 17:21:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n57_, new_n58_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_;
  inv1   g00(.a(x03), .O(new_n25_));
  nand2  g01(.a(x05), .b(x02), .O(new_n26_));
  oai21  g02(.a(new_n25_), .b(x02), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x02), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(x00), .O(z0));
  nand3  g07(.a(x06), .b(x05), .c(x01), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x02), .O(new_n33_));
  inv1   g09(.a(x02), .O(new_n34_));
  nand3  g10(.a(x03), .b(new_n34_), .c(x01), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n33_), .c(x00), .O(z1));
  inv1   g12(.a(x01), .O(new_n37_));
  inv1   g13(.a(x08), .O(new_n38_));
  inv1   g14(.a(x10), .O(new_n39_));
  nor2   g15(.a(new_n39_), .b(x03), .O(new_n40_));
  inv1   g16(.a(x09), .O(new_n41_));
  nor2   g17(.a(x10), .b(new_n41_), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(new_n40_), .c(new_n38_), .O(new_n43_));
  inv1   g19(.a(x00), .O(new_n44_));
  nand2  g20(.a(new_n42_), .b(new_n44_), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n43_), .c(x07), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  nand2  g23(.a(new_n39_), .b(new_n47_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n25_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n34_), .c(x00), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n46_), .c(new_n37_), .O(new_n51_));
  aoi21  g27(.a(x05), .b(new_n37_), .c(x00), .O(new_n52_));
  nor2   g28(.a(x08), .b(x07), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n52_), .c(x02), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n51_), .O(z2));
  aoi22  g31(.a(x12), .b(new_n44_), .c(x11), .d(x07), .O(z3));
  nand3  g32(.a(x09), .b(x08), .c(x00), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n47_), .O(new_n58_));
  oai21  g34(.a(x11), .b(new_n47_), .c(new_n58_), .O(z4));
  nor2   g35(.a(x12), .b(x00), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(x13), .c(new_n39_), .d(new_n47_), .O(z5));
  nand2  g37(.a(new_n41_), .b(x03), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n34_), .c(new_n44_), .O(new_n63_));
  nand3  g39(.a(x09), .b(new_n25_), .c(new_n37_), .O(new_n64_));
  oai21  g40(.a(x02), .b(new_n37_), .c(new_n64_), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(new_n63_), .c(new_n38_), .O(new_n66_));
  nand2  g42(.a(x14), .b(x02), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(x01), .O(new_n68_));
  oai21  g44(.a(new_n41_), .b(x03), .c(new_n34_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n37_), .O(new_n70_));
  nand3  g46(.a(new_n41_), .b(x03), .c(new_n34_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n68_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n44_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n66_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n39_), .c(new_n47_), .O(new_n75_));
  inv1   g51(.a(new_n75_), .O(z6));
  nand3  g52(.a(x09), .b(x03), .c(new_n37_), .O(new_n77_));
  nand4  g53(.a(new_n39_), .b(new_n38_), .c(new_n47_), .d(x01), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n34_), .O(new_n80_));
  oai21  g56(.a(x07), .b(new_n34_), .c(x03), .O(new_n81_));
  aoi21  g57(.a(new_n81_), .b(new_n38_), .c(new_n48_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n80_), .O(z7));
  inv1   g59(.a(x12), .O(new_n84_));
  nand4  g60(.a(new_n84_), .b(x03), .c(new_n34_), .d(new_n37_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(x09), .O(new_n86_));
  aoi21  g62(.a(new_n41_), .b(new_n44_), .c(x10), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n86_), .c(new_n47_), .O(z8));
endmodule


