// Benchmark "FAU" written by ABC on Mon Jul 27 17:21:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  oai21  g09(.a(new_n30_), .b(new_n25_), .c(x06), .O(new_n34_));
  inv1   g10(.a(x06), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x04), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(x02), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n29_), .c(x00), .O(z1));
  nand2  g15(.a(new_n31_), .b(x01), .O(new_n40_));
  inv1   g16(.a(x07), .O(new_n41_));
  inv1   g17(.a(x10), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n35_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  inv1   g21(.a(x04), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(x03), .O(new_n47_));
  nand2  g23(.a(x06), .b(new_n30_), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n36_), .O(new_n49_));
  inv1   g25(.a(new_n49_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n45_), .c(new_n40_), .O(new_n51_));
  inv1   g27(.a(x03), .O(new_n52_));
  nand2  g28(.a(new_n43_), .b(new_n52_), .O(new_n53_));
  inv1   g29(.a(x09), .O(new_n54_));
  nor2   g30(.a(x10), .b(new_n54_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n41_), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n53_), .c(x01), .O(new_n57_));
  aoi21  g33(.a(new_n51_), .b(x02), .c(new_n57_), .O(new_n58_));
  inv1   g34(.a(x08), .O(new_n59_));
  nor2   g35(.a(new_n42_), .b(x03), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n55_), .c(new_n25_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n27_), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n59_), .c(new_n41_), .O(new_n63_));
  oai21  g39(.a(new_n58_), .b(x00), .c(new_n63_), .O(z2));
  inv1   g40(.a(x00), .O(new_n65_));
  aoi22  g41(.a(x12), .b(new_n65_), .c(x11), .d(x07), .O(z3));
  nand3  g42(.a(x09), .b(x08), .c(x00), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n41_), .O(new_n68_));
  oai21  g44(.a(x11), .b(new_n41_), .c(new_n68_), .O(z4));
  nor2   g45(.a(x12), .b(x00), .O(new_n70_));
  nand4  g46(.a(new_n70_), .b(x13), .c(new_n42_), .d(new_n41_), .O(z5));
  oai21  g47(.a(x09), .b(new_n52_), .c(new_n25_), .O(new_n72_));
  nor2   g48(.a(x02), .b(x00), .O(new_n73_));
  nor2   g49(.a(x08), .b(new_n65_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(new_n75_));
  oai21  g51(.a(new_n54_), .b(x03), .c(new_n27_), .O(new_n76_));
  nand2  g52(.a(x08), .b(x00), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n76_), .c(new_n25_), .O(new_n78_));
  inv1   g54(.a(x14), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(x01), .c(new_n65_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n78_), .c(new_n75_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n42_), .c(new_n41_), .O(new_n82_));
  inv1   g58(.a(new_n82_), .O(z6));
  nand3  g59(.a(x09), .b(x03), .c(new_n27_), .O(new_n84_));
  nand4  g60(.a(new_n42_), .b(new_n59_), .c(new_n41_), .d(x02), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n25_), .O(new_n87_));
  nand2  g63(.a(x03), .b(new_n25_), .O(new_n88_));
  aoi21  g64(.a(new_n88_), .b(new_n59_), .c(new_n43_), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n87_), .O(z7));
  inv1   g66(.a(x12), .O(new_n91_));
  nand4  g67(.a(new_n91_), .b(x03), .c(new_n27_), .d(new_n25_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(x09), .O(new_n93_));
  aoi21  g69(.a(new_n54_), .b(new_n65_), .c(x10), .O(new_n94_));
  nand3  g70(.a(new_n94_), .b(new_n93_), .c(new_n41_), .O(z8));
endmodule


