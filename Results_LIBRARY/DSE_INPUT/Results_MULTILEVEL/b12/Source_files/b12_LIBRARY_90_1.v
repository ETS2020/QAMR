// Benchmark "FAU" written by ABC on Mon Jul 27 17:21:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(new_n25_), .O(new_n27_));
  oai21  g03(.a(x02), .b(new_n25_), .c(new_n27_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x03), .O(new_n34_));
  aoi21  g10(.a(new_n26_), .b(new_n34_), .c(x01), .O(new_n35_));
  aoi22  g11(.a(x06), .b(x05), .c(new_n26_), .d(new_n34_), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n35_), .c(x02), .O(new_n37_));
  inv1   g13(.a(x02), .O(new_n38_));
  nand3  g14(.a(x03), .b(new_n38_), .c(x01), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n37_), .c(x00), .O(z1));
  nand2  g16(.a(new_n31_), .b(x01), .O(new_n41_));
  oai21  g17(.a(new_n30_), .b(new_n25_), .c(x04), .O(new_n42_));
  nor2   g18(.a(x04), .b(new_n34_), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n43_), .c(new_n25_), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n42_), .c(new_n41_), .O(new_n46_));
  oai21  g22(.a(x10), .b(x07), .c(new_n34_), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  nor2   g25(.a(x10), .b(new_n49_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n47_), .c(x01), .O(new_n52_));
  aoi21  g28(.a(new_n46_), .b(x02), .c(new_n52_), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  inv1   g30(.a(x10), .O(new_n55_));
  nor2   g31(.a(new_n55_), .b(x03), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n50_), .c(new_n25_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n38_), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n54_), .c(new_n48_), .O(new_n59_));
  oai21  g35(.a(new_n53_), .b(x00), .c(new_n59_), .O(z2));
  inv1   g36(.a(x11), .O(new_n61_));
  aoi21  g37(.a(x10), .b(new_n48_), .c(new_n61_), .O(new_n62_));
  inv1   g38(.a(x12), .O(new_n63_));
  nor2   g39(.a(new_n63_), .b(x00), .O(new_n64_));
  nor2   g40(.a(new_n64_), .b(new_n62_), .O(z3));
  aoi21  g41(.a(x09), .b(x08), .c(x07), .O(new_n66_));
  aoi21  g42(.a(new_n61_), .b(x07), .c(new_n66_), .O(new_n67_));
  oai21  g43(.a(new_n62_), .b(x00), .c(new_n67_), .O(z4));
  nor2   g44(.a(x07), .b(x00), .O(new_n69_));
  inv1   g45(.a(x13), .O(new_n70_));
  nor2   g46(.a(new_n70_), .b(x12), .O(new_n71_));
  nand3  g47(.a(new_n63_), .b(x10), .c(new_n48_), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n71_), .c(new_n69_), .d(new_n61_), .O(z5));
  nor2   g49(.a(x01), .b(x00), .O(new_n74_));
  inv1   g50(.a(x00), .O(new_n75_));
  nor2   g51(.a(x08), .b(new_n75_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n74_), .c(x02), .O(new_n77_));
  nand2  g53(.a(x08), .b(x00), .O(new_n78_));
  aoi21  g54(.a(new_n49_), .b(x03), .c(x01), .O(new_n79_));
  nand3  g55(.a(x09), .b(new_n34_), .c(new_n25_), .O(new_n80_));
  oai21  g56(.a(new_n79_), .b(x02), .c(new_n80_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  inv1   g58(.a(x14), .O(new_n83_));
  nand3  g59(.a(new_n83_), .b(x01), .c(new_n75_), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(new_n82_), .c(new_n77_), .O(new_n85_));
  nand3  g61(.a(new_n85_), .b(new_n55_), .c(new_n48_), .O(new_n86_));
  inv1   g62(.a(new_n86_), .O(z6));
  nand3  g63(.a(x09), .b(x03), .c(new_n25_), .O(new_n88_));
  nand4  g64(.a(new_n55_), .b(new_n54_), .c(new_n48_), .d(x01), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n38_), .O(new_n91_));
  oai21  g67(.a(x07), .b(new_n38_), .c(x03), .O(new_n92_));
  nand3  g68(.a(new_n61_), .b(new_n55_), .c(new_n48_), .O(new_n93_));
  aoi21  g69(.a(new_n92_), .b(new_n54_), .c(new_n93_), .O(new_n94_));
  nand2  g70(.a(new_n94_), .b(new_n91_), .O(z7));
  inv1   g71(.a(new_n64_), .O(new_n96_));
  nand3  g72(.a(new_n96_), .b(x10), .c(new_n48_), .O(new_n97_));
  nand4  g73(.a(new_n63_), .b(x03), .c(new_n38_), .d(new_n25_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(x09), .O(new_n99_));
  nand2  g75(.a(new_n61_), .b(new_n48_), .O(new_n100_));
  aoi21  g76(.a(new_n49_), .b(new_n75_), .c(new_n100_), .O(new_n101_));
  nand3  g77(.a(new_n101_), .b(new_n99_), .c(new_n97_), .O(z8));
endmodule


