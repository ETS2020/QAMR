// Benchmark "FAU" written by ABC on Thu Jun 25 17:17:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_;
  nand2  g00(.a(x23), .b(x22), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x21), .c(x20), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  inv1   g04(.a(x09), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x19), .c(new_n48_), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(x26), .c(x25), .d(x24), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(z00));
  nand2  g10(.a(x08), .b(x00), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z01));
  inv1   g12(.a(x01), .O(new_n57_));
  inv1   g13(.a(x08), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n57_), .O(z02));
  inv1   g15(.a(x02), .O(new_n60_));
  nor2   g16(.a(new_n58_), .b(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  inv1   g23(.a(x06), .O(new_n68_));
  nor2   g24(.a(new_n58_), .b(new_n68_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  nor2   g27(.a(x26), .b(x23), .O(new_n72_));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai22  g30(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(x19), .O(new_n75_));
  nand2  g31(.a(new_n50_), .b(new_n48_), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n55_), .O(z09));
  nand3  g35(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n73_), .c(x19), .O(new_n81_));
  inv1   g37(.a(x19), .O(new_n82_));
  nor2   g38(.a(x20), .b(new_n82_), .O(new_n83_));
  aoi21  g39(.a(new_n81_), .b(x20), .c(new_n83_), .O(new_n84_));
  oai22  g40(.a(new_n84_), .b(new_n76_), .c(new_n58_), .d(new_n57_), .O(z10));
  nand3  g41(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  nand3  g42(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  nand2  g43(.a(x20), .b(x19), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  oai21  g45(.a(new_n87_), .b(new_n86_), .c(new_n89_), .O(new_n90_));
  nor2   g46(.a(new_n88_), .b(x21), .O(new_n91_));
  aoi21  g47(.a(new_n90_), .b(x21), .c(new_n91_), .O(new_n92_));
  oai22  g48(.a(new_n92_), .b(new_n76_), .c(new_n58_), .d(new_n60_), .O(z11));
  inv1   g49(.a(new_n86_), .O(new_n94_));
  nand3  g50(.a(new_n94_), .b(x23), .c(x14), .O(new_n95_));
  nand2  g51(.a(x21), .b(x20), .O(new_n96_));
  oai21  g52(.a(x26), .b(x23), .c(new_n96_), .O(new_n97_));
  nand3  g53(.a(new_n97_), .b(new_n95_), .c(x19), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  nand3  g55(.a(new_n89_), .b(new_n99_), .c(x21), .O(new_n100_));
  inv1   g56(.a(new_n100_), .O(new_n101_));
  aoi21  g57(.a(new_n98_), .b(x22), .c(new_n101_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n76_), .c(new_n62_), .O(z12));
  inv1   g59(.a(x23), .O(new_n104_));
  inv1   g60(.a(x25), .O(new_n105_));
  inv1   g61(.a(x26), .O(new_n106_));
  nor2   g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(x24), .c(x15), .O(new_n108_));
  nand4  g64(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n108_), .c(new_n104_), .O(new_n111_));
  nand3  g67(.a(new_n104_), .b(x22), .c(x21), .O(new_n112_));
  nor2   g68(.a(new_n112_), .b(new_n88_), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n111_), .c(new_n77_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n64_), .O(z13));
  nand2  g71(.a(new_n107_), .b(x16), .O(new_n116_));
  nand3  g72(.a(x21), .b(x20), .c(x19), .O(new_n117_));
  inv1   g73(.a(new_n117_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n116_), .c(new_n46_), .O(new_n119_));
  nor3   g75(.a(new_n117_), .b(new_n45_), .c(x24), .O(new_n120_));
  aoi21  g76(.a(new_n119_), .b(x24), .c(new_n120_), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n76_), .c(new_n66_), .O(z14));
  nand2  g78(.a(x26), .b(x17), .O(new_n123_));
  nand4  g79(.a(new_n123_), .b(new_n118_), .c(new_n46_), .d(x24), .O(new_n124_));
  nand4  g80(.a(new_n105_), .b(x24), .c(x23), .d(x22), .O(new_n125_));
  nor2   g81(.a(new_n125_), .b(new_n117_), .O(new_n126_));
  aoi21  g82(.a(new_n124_), .b(x25), .c(new_n126_), .O(new_n127_));
  oai22  g83(.a(new_n127_), .b(new_n76_), .c(new_n58_), .d(new_n68_), .O(z15));
  nand2  g84(.a(x25), .b(x24), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(x26), .c(x23), .O(new_n130_));
  nand3  g86(.a(x21), .b(x20), .c(x19), .O(new_n131_));
  inv1   g87(.a(new_n131_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g89(.a(new_n96_), .b(x26), .O(new_n134_));
  aoi21  g90(.a(new_n134_), .b(new_n133_), .c(new_n99_), .O(new_n135_));
  nand3  g91(.a(x25), .b(x24), .c(x22), .O(new_n136_));
  nor3   g92(.a(new_n136_), .b(new_n82_), .c(x18), .O(new_n137_));
  nor2   g93(.a(new_n137_), .b(new_n106_), .O(new_n138_));
  oai21  g94(.a(new_n138_), .b(new_n135_), .c(new_n77_), .O(new_n139_));
  nand2  g95(.a(new_n139_), .b(new_n70_), .O(z16));
endmodule


