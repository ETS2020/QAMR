// Benchmark "FAU" written by ABC on Thu Jun 25 17:16:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  and2   g00(.a(x25), .b(x24), .O(new_n45_));
  nand2  g01(.a(x23), .b(x22), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x21), .c(x20), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(x08), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x19), .c(new_n49_), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(new_n45_), .c(x26), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z00));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  inv1   g14(.a(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  inv1   g20(.a(x04), .O(new_n65_));
  nor2   g21(.a(new_n59_), .b(new_n65_), .O(z05));
  inv1   g22(.a(x05), .O(new_n67_));
  nor2   g23(.a(new_n59_), .b(new_n67_), .O(z06));
  inv1   g24(.a(x06), .O(new_n69_));
  nor2   g25(.a(new_n59_), .b(new_n69_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  nand4  g29(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n74_));
  nand2  g30(.a(x22), .b(x21), .O(new_n75_));
  nand2  g31(.a(x20), .b(x11), .O(new_n76_));
  nor3   g32(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand2  g34(.a(new_n51_), .b(new_n49_), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n78_), .c(new_n56_), .O(z09));
  nand3  g36(.a(x22), .b(x21), .c(x12), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n74_), .c(x19), .O(new_n82_));
  nor2   g38(.a(x20), .b(new_n73_), .O(new_n83_));
  aoi21  g39(.a(new_n82_), .b(x20), .c(new_n83_), .O(new_n84_));
  oai22  g40(.a(new_n84_), .b(new_n79_), .c(new_n59_), .d(new_n58_), .O(z10));
  nand3  g41(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  nand3  g42(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  nand2  g43(.a(x20), .b(x19), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  oai21  g45(.a(new_n87_), .b(new_n86_), .c(new_n89_), .O(new_n90_));
  nor2   g46(.a(new_n88_), .b(x21), .O(new_n91_));
  aoi21  g47(.a(new_n90_), .b(x21), .c(new_n91_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n79_), .c(new_n61_), .O(z11));
  nor2   g49(.a(x26), .b(x23), .O(new_n94_));
  nand3  g50(.a(x21), .b(x20), .c(x19), .O(new_n95_));
  inv1   g51(.a(new_n95_), .O(new_n96_));
  nand2  g52(.a(x23), .b(x14), .O(new_n97_));
  oai22  g53(.a(new_n97_), .b(new_n86_), .c(new_n96_), .d(new_n94_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  nand3  g55(.a(new_n89_), .b(new_n99_), .c(x21), .O(new_n100_));
  inv1   g56(.a(new_n100_), .O(new_n101_));
  aoi21  g57(.a(new_n98_), .b(x22), .c(new_n101_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n79_), .c(new_n63_), .O(z12));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n96_), .c(x22), .O(new_n105_));
  nor3   g61(.a(new_n88_), .b(new_n75_), .c(x23), .O(new_n106_));
  aoi21  g62(.a(new_n105_), .b(x23), .c(new_n106_), .O(new_n107_));
  oai22  g63(.a(new_n107_), .b(new_n79_), .c(new_n59_), .d(new_n65_), .O(z13));
  nand3  g64(.a(x26), .b(x25), .c(x16), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n96_), .c(new_n47_), .O(new_n110_));
  nor3   g66(.a(new_n95_), .b(new_n46_), .c(x24), .O(new_n111_));
  aoi21  g67(.a(new_n110_), .b(x24), .c(new_n111_), .O(new_n112_));
  oai22  g68(.a(new_n112_), .b(new_n79_), .c(new_n59_), .d(new_n67_), .O(z14));
  nand2  g69(.a(x26), .b(x17), .O(new_n114_));
  nand3  g70(.a(x24), .b(x23), .c(x22), .O(new_n115_));
  inv1   g71(.a(new_n115_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n114_), .c(new_n96_), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  nand4  g74(.a(new_n118_), .b(x24), .c(x23), .d(x22), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  aoi22  g76(.a(new_n120_), .b(new_n96_), .c(new_n117_), .d(x25), .O(new_n121_));
  oai22  g77(.a(new_n121_), .b(new_n79_), .c(new_n59_), .d(new_n69_), .O(z15));
  inv1   g78(.a(x26), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(x25), .c(x22), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n95_), .c(x23), .O(new_n125_));
  inv1   g81(.a(x18), .O(new_n126_));
  nand4  g82(.a(new_n96_), .b(new_n45_), .c(x22), .d(new_n126_), .O(new_n127_));
  aoi22  g83(.a(new_n127_), .b(x26), .c(new_n125_), .d(x24), .O(new_n128_));
  oai21  g84(.a(new_n128_), .b(new_n79_), .c(new_n71_), .O(z16));
endmodule


