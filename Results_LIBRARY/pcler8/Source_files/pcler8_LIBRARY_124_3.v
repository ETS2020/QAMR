// Benchmark "FAU" written by ABC on Thu Jun 25 17:17:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_,
    new_n55_, new_n57_, new_n59_, new_n61_, new_n63_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  nand2  g02(.a(x09), .b(new_n46_), .O(new_n47_));
  nand2  g03(.a(x20), .b(x19), .O(new_n48_));
  nand2  g04(.a(x22), .b(x21), .O(new_n49_));
  nor4   g05(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n45_), .O(z00));
  inv1   g06(.a(x00), .O(new_n51_));
  nor2   g07(.a(new_n46_), .b(new_n51_), .O(z01));
  nand2  g08(.a(x08), .b(x01), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(z02));
  nand2  g10(.a(x08), .b(x02), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z03));
  nand2  g12(.a(x08), .b(x03), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z04));
  nand2  g14(.a(x08), .b(x04), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z05));
  nand2  g16(.a(x08), .b(x05), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z06));
  nand2  g18(.a(x08), .b(x06), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z07));
  nand2  g20(.a(x08), .b(x07), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z08));
  inv1   g22(.a(new_n47_), .O(new_n67_));
  inv1   g23(.a(new_n49_), .O(new_n68_));
  nand3  g24(.a(new_n68_), .b(x20), .c(x11), .O(new_n69_));
  oai22  g25(.a(new_n69_), .b(new_n45_), .c(x19), .d(x10), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n67_), .O(new_n71_));
  oai21  g27(.a(new_n46_), .b(new_n51_), .c(new_n71_), .O(z09));
  inv1   g28(.a(x10), .O(new_n73_));
  xor2a  g29(.a(x20), .b(x19), .O(new_n74_));
  nand2  g30(.a(x20), .b(x12), .O(new_n75_));
  nor3   g31(.a(new_n75_), .b(new_n49_), .c(new_n45_), .O(new_n76_));
  aoi21  g32(.a(new_n74_), .b(new_n73_), .c(new_n76_), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n47_), .c(new_n53_), .O(z10));
  inv1   g34(.a(new_n48_), .O(new_n79_));
  nand3  g35(.a(x26), .b(x25), .c(x24), .O(new_n80_));
  nand2  g36(.a(x23), .b(x22), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(x13), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  inv1   g40(.a(x21), .O(new_n85_));
  nand4  g41(.a(new_n85_), .b(x20), .c(x19), .d(new_n73_), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n84_), .b(x21), .c(new_n87_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n47_), .c(new_n55_), .O(z11));
  nand2  g45(.a(x23), .b(x14), .O(new_n90_));
  nor2   g46(.a(new_n90_), .b(new_n80_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n48_), .c(x22), .O(new_n92_));
  inv1   g48(.a(x22), .O(new_n93_));
  nand2  g49(.a(new_n79_), .b(new_n93_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n92_), .c(new_n85_), .O(new_n95_));
  nand3  g51(.a(x22), .b(new_n85_), .c(new_n73_), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n95_), .c(new_n67_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n57_), .O(z12));
  inv1   g55(.a(x23), .O(new_n100_));
  nand2  g56(.a(new_n49_), .b(new_n73_), .O(new_n101_));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n79_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(x21), .O(new_n104_));
  aoi21  g60(.a(new_n104_), .b(new_n101_), .c(new_n100_), .O(new_n105_));
  nor3   g61(.a(new_n49_), .b(new_n48_), .c(x23), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n105_), .c(new_n67_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n59_), .O(z13));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n81_), .b(new_n85_), .c(new_n73_), .O(new_n110_));
  nand3  g66(.a(x26), .b(x25), .c(x16), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n48_), .c(x21), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n110_), .c(new_n109_), .O(new_n114_));
  nand2  g70(.a(new_n79_), .b(x21), .O(new_n115_));
  nor3   g71(.a(new_n115_), .b(new_n81_), .c(x24), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n114_), .c(new_n67_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n61_), .O(z14));
  inv1   g74(.a(x25), .O(new_n119_));
  and2   g75(.a(x26), .b(x17), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n48_), .c(x21), .O(new_n121_));
  nand4  g77(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n73_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n121_), .c(new_n119_), .O(new_n124_));
  nand3  g80(.a(new_n82_), .b(new_n119_), .c(x24), .O(new_n125_));
  nor2   g81(.a(new_n125_), .b(new_n115_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n124_), .c(new_n67_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n63_), .O(z15));
  inv1   g84(.a(x26), .O(new_n129_));
  nand2  g85(.a(new_n48_), .b(x21), .O(new_n130_));
  inv1   g86(.a(x18), .O(new_n131_));
  nand4  g87(.a(x25), .b(x24), .c(x23), .d(new_n131_), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n49_), .c(new_n73_), .O(new_n133_));
  aoi21  g89(.a(new_n133_), .b(new_n130_), .c(new_n129_), .O(new_n134_));
  nor4   g90(.a(new_n122_), .b(new_n48_), .c(x26), .d(new_n119_), .O(new_n135_));
  oai21  g91(.a(new_n135_), .b(new_n134_), .c(new_n67_), .O(new_n136_));
  nand2  g92(.a(new_n136_), .b(new_n65_), .O(z16));
endmodule


