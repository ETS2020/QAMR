// Benchmark "FAU" written by ABC on Tue Aug 11 19:07:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n35_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n45_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x09), .O(new_n34_));
  inv1   g01(.a(x10), .O(new_n35_));
  nand2  g02(.a(new_n35_), .b(new_n34_), .O(z00));
  inv1   g03(.a(x11), .O(new_n37_));
  nand2  g04(.a(x10), .b(new_n34_), .O(new_n38_));
  nand2  g05(.a(new_n38_), .b(new_n37_), .O(z02));
  inv1   g06(.a(x12), .O(new_n40_));
  nor2   g07(.a(new_n35_), .b(x09), .O(new_n41_));
  nor2   g08(.a(new_n41_), .b(new_n40_), .O(z03));
  inv1   g09(.a(x13), .O(new_n43_));
  nor2   g10(.a(new_n41_), .b(new_n43_), .O(z04));
  inv1   g11(.a(x14), .O(new_n45_));
  nor2   g12(.a(new_n41_), .b(new_n45_), .O(z05));
  inv1   g13(.a(x15), .O(new_n47_));
  nand2  g14(.a(new_n38_), .b(new_n47_), .O(z06));
  inv1   g15(.a(x16), .O(new_n49_));
  nor2   g16(.a(new_n41_), .b(new_n49_), .O(z07));
  nand2  g17(.a(x08), .b(x00), .O(new_n51_));
  nand2  g18(.a(x09), .b(x08), .O(new_n52_));
  nand2  g19(.a(new_n52_), .b(z00), .O(new_n53_));
  nand2  g20(.a(new_n53_), .b(new_n51_), .O(z08));
  nand2  g21(.a(new_n52_), .b(x10), .O(new_n55_));
  nand2  g22(.a(x08), .b(x01), .O(new_n56_));
  nand2  g23(.a(new_n56_), .b(new_n55_), .O(z09));
  inv1   g24(.a(x02), .O(new_n58_));
  nand2  g25(.a(x08), .b(new_n58_), .O(new_n59_));
  inv1   g26(.a(x08), .O(new_n60_));
  nand2  g27(.a(new_n37_), .b(new_n60_), .O(new_n61_));
  nand3  g28(.a(new_n61_), .b(new_n59_), .c(new_n38_), .O(new_n62_));
  inv1   g29(.a(new_n62_), .O(z10));
  inv1   g30(.a(x03), .O(new_n64_));
  nand2  g31(.a(x08), .b(new_n64_), .O(new_n65_));
  nand2  g32(.a(new_n40_), .b(new_n60_), .O(new_n66_));
  nand3  g33(.a(new_n66_), .b(new_n65_), .c(new_n38_), .O(new_n67_));
  inv1   g34(.a(new_n67_), .O(z11));
  inv1   g35(.a(x04), .O(new_n69_));
  nand2  g36(.a(x08), .b(new_n69_), .O(new_n70_));
  nand2  g37(.a(new_n43_), .b(new_n60_), .O(new_n71_));
  nand3  g38(.a(new_n71_), .b(new_n70_), .c(new_n38_), .O(new_n72_));
  inv1   g39(.a(new_n72_), .O(z12));
  inv1   g40(.a(x05), .O(new_n74_));
  nand2  g41(.a(x08), .b(new_n74_), .O(new_n75_));
  nand2  g42(.a(new_n45_), .b(new_n60_), .O(new_n76_));
  nand3  g43(.a(new_n76_), .b(new_n75_), .c(new_n38_), .O(new_n77_));
  inv1   g44(.a(new_n77_), .O(z13));
  nand2  g45(.a(x08), .b(x06), .O(new_n79_));
  nand2  g46(.a(x15), .b(new_n60_), .O(new_n80_));
  nand3  g47(.a(new_n80_), .b(new_n79_), .c(new_n38_), .O(z14));
  inv1   g48(.a(x07), .O(new_n82_));
  nand2  g49(.a(x08), .b(new_n82_), .O(new_n83_));
  nand2  g50(.a(new_n49_), .b(new_n60_), .O(new_n84_));
  nand3  g51(.a(new_n84_), .b(new_n83_), .c(new_n38_), .O(new_n85_));
  inv1   g52(.a(new_n85_), .O(z15));
  buf    g53(.a(x10), .O(z01));
endmodule


