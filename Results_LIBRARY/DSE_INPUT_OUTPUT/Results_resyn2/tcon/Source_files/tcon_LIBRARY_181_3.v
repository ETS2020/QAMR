// Benchmark "FAU" written by ABC on Tue Aug 11 19:09:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n35_, new_n37_, new_n39_, new_n41_, new_n43_, new_n46_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_;
  inv1   g00(.a(x09), .O(new_n34_));
  nand2  g01(.a(x16), .b(x15), .O(new_n35_));
  nand2  g02(.a(new_n35_), .b(new_n34_), .O(z00));
  inv1   g03(.a(x10), .O(new_n37_));
  nand2  g04(.a(new_n35_), .b(new_n37_), .O(z01));
  inv1   g05(.a(x11), .O(new_n39_));
  nand2  g06(.a(new_n35_), .b(new_n39_), .O(z02));
  inv1   g07(.a(x12), .O(new_n41_));
  nand2  g08(.a(new_n35_), .b(new_n41_), .O(z03));
  inv1   g09(.a(x13), .O(new_n43_));
  aoi21  g10(.a(x16), .b(x15), .c(new_n43_), .O(z04));
  and2   g11(.a(new_n35_), .b(x14), .O(z05));
  inv1   g12(.a(x15), .O(new_n46_));
  nor2   g13(.a(x16), .b(new_n46_), .O(z06));
  inv1   g14(.a(x16), .O(new_n48_));
  nor2   g15(.a(new_n48_), .b(x15), .O(z07));
  inv1   g16(.a(x00), .O(new_n50_));
  nand2  g17(.a(x08), .b(new_n50_), .O(new_n51_));
  inv1   g18(.a(x08), .O(new_n52_));
  nand2  g19(.a(new_n34_), .b(new_n52_), .O(new_n53_));
  nand3  g20(.a(new_n53_), .b(new_n51_), .c(new_n35_), .O(new_n54_));
  inv1   g21(.a(new_n54_), .O(z08));
  inv1   g22(.a(x01), .O(new_n56_));
  nand2  g23(.a(x08), .b(new_n56_), .O(new_n57_));
  nand2  g24(.a(new_n37_), .b(new_n52_), .O(new_n58_));
  nand3  g25(.a(new_n58_), .b(new_n57_), .c(new_n35_), .O(new_n59_));
  inv1   g26(.a(new_n59_), .O(z09));
  nand2  g27(.a(x08), .b(x02), .O(new_n61_));
  aoi22  g28(.a(x16), .b(x15), .c(x11), .d(new_n52_), .O(new_n62_));
  nand2  g29(.a(new_n62_), .b(new_n61_), .O(z10));
  nand2  g30(.a(x08), .b(x03), .O(new_n64_));
  aoi22  g31(.a(x16), .b(x15), .c(x12), .d(new_n52_), .O(new_n65_));
  nand2  g32(.a(new_n65_), .b(new_n64_), .O(z11));
  inv1   g33(.a(x04), .O(new_n67_));
  nand2  g34(.a(x08), .b(new_n67_), .O(new_n68_));
  nand2  g35(.a(new_n43_), .b(new_n52_), .O(new_n69_));
  nand3  g36(.a(new_n69_), .b(new_n68_), .c(new_n35_), .O(new_n70_));
  inv1   g37(.a(new_n70_), .O(z12));
  nand2  g38(.a(x08), .b(x05), .O(new_n72_));
  nand2  g39(.a(x14), .b(new_n52_), .O(new_n73_));
  nand3  g40(.a(new_n73_), .b(new_n72_), .c(new_n35_), .O(z13));
  inv1   g41(.a(x06), .O(new_n75_));
  nand2  g42(.a(new_n35_), .b(x08), .O(new_n76_));
  nand3  g43(.a(new_n48_), .b(x15), .c(new_n52_), .O(new_n77_));
  oai21  g44(.a(new_n76_), .b(new_n75_), .c(new_n77_), .O(z14));
  inv1   g45(.a(x07), .O(new_n79_));
  nand3  g46(.a(x16), .b(new_n46_), .c(new_n52_), .O(new_n80_));
  oai21  g47(.a(new_n76_), .b(new_n79_), .c(new_n80_), .O(z15));
endmodule


