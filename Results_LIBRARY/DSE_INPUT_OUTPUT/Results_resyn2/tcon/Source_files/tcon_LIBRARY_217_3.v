// Benchmark "FAU" written by ABC on Tue Aug 11 19:09:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n35_, new_n37_, new_n39_, new_n41_, new_n43_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x09), .O(new_n34_));
  nand2  g01(.a(x15), .b(x11), .O(new_n35_));
  nand2  g02(.a(new_n35_), .b(new_n34_), .O(z00));
  inv1   g03(.a(x10), .O(new_n37_));
  nand2  g04(.a(new_n35_), .b(new_n37_), .O(z01));
  inv1   g05(.a(x12), .O(new_n39_));
  nand2  g06(.a(new_n35_), .b(new_n39_), .O(z03));
  inv1   g07(.a(x13), .O(new_n41_));
  aoi21  g08(.a(x15), .b(x11), .c(new_n41_), .O(z04));
  inv1   g09(.a(x14), .O(new_n43_));
  nand2  g10(.a(new_n35_), .b(new_n43_), .O(z05));
  and2   g11(.a(new_n35_), .b(x16), .O(z07));
  nand2  g12(.a(x08), .b(x00), .O(new_n46_));
  inv1   g13(.a(x08), .O(new_n47_));
  aoi22  g14(.a(x15), .b(x11), .c(x09), .d(new_n47_), .O(new_n48_));
  nand2  g15(.a(new_n48_), .b(new_n46_), .O(z08));
  inv1   g16(.a(x01), .O(new_n50_));
  nand2  g17(.a(x08), .b(new_n50_), .O(new_n51_));
  nand2  g18(.a(new_n37_), .b(new_n47_), .O(new_n52_));
  nand3  g19(.a(new_n52_), .b(new_n51_), .c(new_n35_), .O(new_n53_));
  inv1   g20(.a(new_n53_), .O(z09));
  inv1   g21(.a(x02), .O(new_n55_));
  nand2  g22(.a(x08), .b(new_n55_), .O(new_n56_));
  inv1   g23(.a(x11), .O(new_n57_));
  nand2  g24(.a(new_n57_), .b(new_n47_), .O(new_n58_));
  nand3  g25(.a(new_n58_), .b(new_n56_), .c(new_n35_), .O(new_n59_));
  inv1   g26(.a(new_n59_), .O(z10));
  inv1   g27(.a(x03), .O(new_n61_));
  nand2  g28(.a(x08), .b(new_n61_), .O(new_n62_));
  nand2  g29(.a(new_n39_), .b(new_n47_), .O(new_n63_));
  nand3  g30(.a(new_n63_), .b(new_n62_), .c(new_n35_), .O(new_n64_));
  inv1   g31(.a(new_n64_), .O(z11));
  inv1   g32(.a(x04), .O(new_n66_));
  nand2  g33(.a(x08), .b(new_n66_), .O(new_n67_));
  nand2  g34(.a(new_n41_), .b(new_n47_), .O(new_n68_));
  nand3  g35(.a(new_n68_), .b(new_n67_), .c(new_n35_), .O(new_n69_));
  inv1   g36(.a(new_n69_), .O(z12));
  nand2  g37(.a(x08), .b(x05), .O(new_n71_));
  aoi22  g38(.a(x15), .b(x11), .c(x14), .d(new_n47_), .O(new_n72_));
  nand2  g39(.a(new_n72_), .b(new_n71_), .O(z13));
  inv1   g40(.a(x06), .O(new_n74_));
  nand2  g41(.a(new_n35_), .b(x08), .O(new_n75_));
  nand3  g42(.a(x15), .b(new_n57_), .c(new_n47_), .O(new_n76_));
  oai21  g43(.a(new_n75_), .b(new_n74_), .c(new_n76_), .O(z14));
  inv1   g44(.a(x07), .O(new_n78_));
  nand3  g45(.a(new_n35_), .b(x16), .c(new_n47_), .O(new_n79_));
  oai21  g46(.a(new_n75_), .b(new_n78_), .c(new_n79_), .O(z15));
  buf    g47(.a(x11), .O(z02));
  buf    g48(.a(x15), .O(z06));
endmodule


