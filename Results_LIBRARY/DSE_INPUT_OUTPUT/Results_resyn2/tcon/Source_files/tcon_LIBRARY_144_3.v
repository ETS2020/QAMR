// Benchmark "FAU" written by ABC on Tue Aug 11 19:08:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n36_, new_n38_, new_n39_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n48_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x09), .O(new_n34_));
  nor2   g01(.a(x10), .b(new_n34_), .O(z00));
  nand2  g02(.a(x10), .b(new_n34_), .O(new_n36_));
  inv1   g03(.a(new_n36_), .O(z01));
  inv1   g04(.a(x11), .O(new_n38_));
  nand2  g05(.a(x10), .b(x09), .O(new_n39_));
  nand2  g06(.a(new_n39_), .b(new_n38_), .O(z02));
  inv1   g07(.a(x12), .O(new_n41_));
  nand2  g08(.a(new_n39_), .b(new_n41_), .O(z03));
  inv1   g09(.a(x13), .O(new_n43_));
  inv1   g10(.a(new_n39_), .O(new_n44_));
  nor2   g11(.a(new_n44_), .b(new_n43_), .O(z04));
  inv1   g12(.a(x14), .O(new_n46_));
  nand2  g13(.a(new_n39_), .b(new_n46_), .O(z05));
  inv1   g14(.a(x15), .O(new_n48_));
  nor2   g15(.a(new_n44_), .b(new_n48_), .O(z06));
  inv1   g16(.a(x16), .O(new_n50_));
  nor2   g17(.a(new_n44_), .b(new_n50_), .O(z07));
  inv1   g18(.a(x10), .O(new_n52_));
  nand2  g19(.a(new_n52_), .b(x08), .O(new_n53_));
  nand2  g20(.a(new_n53_), .b(x09), .O(new_n54_));
  nand2  g21(.a(x08), .b(x00), .O(new_n55_));
  nand2  g22(.a(new_n55_), .b(new_n54_), .O(z08));
  inv1   g23(.a(x08), .O(new_n57_));
  nor2   g24(.a(new_n57_), .b(x01), .O(new_n58_));
  aoi21  g25(.a(new_n53_), .b(new_n36_), .c(new_n58_), .O(z09));
  inv1   g26(.a(x02), .O(new_n60_));
  nand2  g27(.a(x08), .b(new_n60_), .O(new_n61_));
  nand2  g28(.a(new_n38_), .b(new_n57_), .O(new_n62_));
  nand3  g29(.a(new_n62_), .b(new_n61_), .c(new_n39_), .O(new_n63_));
  inv1   g30(.a(new_n63_), .O(z10));
  inv1   g31(.a(x03), .O(new_n65_));
  nand2  g32(.a(x08), .b(new_n65_), .O(new_n66_));
  nand2  g33(.a(new_n41_), .b(new_n57_), .O(new_n67_));
  nand3  g34(.a(new_n67_), .b(new_n66_), .c(new_n39_), .O(new_n68_));
  inv1   g35(.a(new_n68_), .O(z11));
  inv1   g36(.a(x04), .O(new_n70_));
  nand2  g37(.a(x08), .b(new_n70_), .O(new_n71_));
  nand2  g38(.a(new_n43_), .b(new_n57_), .O(new_n72_));
  nand3  g39(.a(new_n72_), .b(new_n71_), .c(new_n39_), .O(new_n73_));
  inv1   g40(.a(new_n73_), .O(z12));
  inv1   g41(.a(x05), .O(new_n75_));
  nand2  g42(.a(x08), .b(new_n75_), .O(new_n76_));
  nand2  g43(.a(new_n46_), .b(new_n57_), .O(new_n77_));
  nand3  g44(.a(new_n77_), .b(new_n76_), .c(new_n39_), .O(new_n78_));
  inv1   g45(.a(new_n78_), .O(z13));
  inv1   g46(.a(x06), .O(new_n80_));
  nand2  g47(.a(x08), .b(new_n80_), .O(new_n81_));
  nand2  g48(.a(new_n48_), .b(new_n57_), .O(new_n82_));
  nand3  g49(.a(new_n82_), .b(new_n81_), .c(new_n39_), .O(new_n83_));
  inv1   g50(.a(new_n83_), .O(z14));
  inv1   g51(.a(x07), .O(new_n85_));
  nand2  g52(.a(x08), .b(new_n85_), .O(new_n86_));
  nand2  g53(.a(new_n50_), .b(new_n57_), .O(new_n87_));
  nand3  g54(.a(new_n87_), .b(new_n86_), .c(new_n39_), .O(new_n88_));
  inv1   g55(.a(new_n88_), .O(z15));
endmodule


