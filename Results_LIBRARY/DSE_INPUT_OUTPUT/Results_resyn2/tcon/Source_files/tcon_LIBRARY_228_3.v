// Benchmark "FAU" written by ABC on Tue Aug 11 19:09:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n35_, new_n37_, new_n40_, new_n41_, new_n44_, new_n46_,
    new_n47_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x09), .O(new_n34_));
  nand2  g01(.a(x13), .b(x12), .O(new_n35_));
  nand2  g02(.a(new_n35_), .b(new_n34_), .O(z00));
  inv1   g03(.a(x10), .O(new_n37_));
  nand2  g04(.a(new_n35_), .b(new_n37_), .O(z01));
  and2   g05(.a(new_n35_), .b(x11), .O(z02));
  inv1   g06(.a(x13), .O(new_n40_));
  nand2  g07(.a(new_n40_), .b(x12), .O(new_n41_));
  inv1   g08(.a(new_n41_), .O(z03));
  nor2   g09(.a(new_n40_), .b(x12), .O(z04));
  inv1   g10(.a(x14), .O(new_n44_));
  nand2  g11(.a(new_n35_), .b(new_n44_), .O(z05));
  inv1   g12(.a(x15), .O(new_n46_));
  inv1   g13(.a(new_n35_), .O(new_n47_));
  nor2   g14(.a(new_n47_), .b(new_n46_), .O(z06));
  inv1   g15(.a(x16), .O(new_n49_));
  nor2   g16(.a(new_n47_), .b(new_n49_), .O(z07));
  inv1   g17(.a(x00), .O(new_n51_));
  nand2  g18(.a(x08), .b(new_n51_), .O(new_n52_));
  inv1   g19(.a(x08), .O(new_n53_));
  nand2  g20(.a(new_n34_), .b(new_n53_), .O(new_n54_));
  nand3  g21(.a(new_n54_), .b(new_n52_), .c(new_n35_), .O(new_n55_));
  inv1   g22(.a(new_n55_), .O(z08));
  nand2  g23(.a(x08), .b(x01), .O(new_n57_));
  aoi22  g24(.a(x13), .b(x12), .c(x10), .d(new_n53_), .O(new_n58_));
  nand2  g25(.a(new_n58_), .b(new_n57_), .O(z09));
  nand3  g26(.a(new_n35_), .b(x08), .c(x02), .O(new_n60_));
  nand3  g27(.a(new_n35_), .b(x11), .c(new_n53_), .O(new_n61_));
  nand2  g28(.a(new_n61_), .b(new_n60_), .O(z10));
  inv1   g29(.a(x12), .O(new_n63_));
  nand2  g30(.a(new_n63_), .b(x08), .O(new_n64_));
  nor2   g31(.a(new_n53_), .b(x03), .O(new_n65_));
  aoi21  g32(.a(new_n64_), .b(new_n41_), .c(new_n65_), .O(z11));
  nand2  g33(.a(new_n64_), .b(x13), .O(new_n67_));
  nand2  g34(.a(x08), .b(x04), .O(new_n68_));
  nand2  g35(.a(new_n68_), .b(new_n67_), .O(z12));
  nand2  g36(.a(x08), .b(x05), .O(new_n70_));
  nand2  g37(.a(x14), .b(new_n53_), .O(new_n71_));
  nand3  g38(.a(new_n71_), .b(new_n70_), .c(new_n35_), .O(z13));
  inv1   g39(.a(x06), .O(new_n73_));
  nand2  g40(.a(x08), .b(new_n73_), .O(new_n74_));
  nand2  g41(.a(new_n46_), .b(new_n53_), .O(new_n75_));
  nand3  g42(.a(new_n75_), .b(new_n74_), .c(new_n35_), .O(new_n76_));
  inv1   g43(.a(new_n76_), .O(z14));
  inv1   g44(.a(x07), .O(new_n78_));
  nand2  g45(.a(x08), .b(new_n78_), .O(new_n79_));
  nand2  g46(.a(new_n49_), .b(new_n53_), .O(new_n80_));
  nand3  g47(.a(new_n80_), .b(new_n79_), .c(new_n35_), .O(new_n81_));
  inv1   g48(.a(new_n81_), .O(z15));
endmodule


