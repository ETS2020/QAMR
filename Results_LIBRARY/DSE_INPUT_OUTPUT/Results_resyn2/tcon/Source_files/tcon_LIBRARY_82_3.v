// Benchmark "FAU" written by ABC on Tue Aug 11 19:08:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n36_, new_n37_, new_n39_, new_n41_, new_n43_, new_n45_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_;
  inv1   g00(.a(x09), .O(new_n34_));
  nor2   g01(.a(x15), .b(new_n34_), .O(z00));
  inv1   g02(.a(x10), .O(new_n36_));
  nand2  g03(.a(x15), .b(x09), .O(new_n37_));
  nand2  g04(.a(new_n37_), .b(new_n36_), .O(z01));
  inv1   g05(.a(x11), .O(new_n39_));
  nand2  g06(.a(new_n37_), .b(new_n39_), .O(z02));
  inv1   g07(.a(x12), .O(new_n41_));
  nand2  g08(.a(new_n37_), .b(new_n41_), .O(z03));
  inv1   g09(.a(x13), .O(new_n43_));
  nand2  g10(.a(new_n37_), .b(new_n43_), .O(z04));
  inv1   g11(.a(x14), .O(new_n45_));
  aoi21  g12(.a(x15), .b(x09), .c(new_n45_), .O(z05));
  and2   g13(.a(new_n37_), .b(x16), .O(z07));
  inv1   g14(.a(x08), .O(new_n48_));
  nand2  g15(.a(new_n34_), .b(new_n48_), .O(new_n49_));
  inv1   g16(.a(x00), .O(new_n50_));
  nand2  g17(.a(x08), .b(new_n50_), .O(new_n51_));
  nand3  g18(.a(new_n51_), .b(new_n49_), .c(new_n37_), .O(new_n52_));
  inv1   g19(.a(new_n52_), .O(z08));
  nand2  g20(.a(x08), .b(x01), .O(new_n54_));
  nand2  g21(.a(x10), .b(new_n48_), .O(new_n55_));
  nand3  g22(.a(new_n55_), .b(new_n54_), .c(new_n37_), .O(z09));
  inv1   g23(.a(x02), .O(new_n57_));
  nand2  g24(.a(x08), .b(new_n57_), .O(new_n58_));
  nand2  g25(.a(new_n39_), .b(new_n48_), .O(new_n59_));
  nand3  g26(.a(new_n59_), .b(new_n58_), .c(new_n37_), .O(new_n60_));
  inv1   g27(.a(new_n60_), .O(z10));
  inv1   g28(.a(x03), .O(new_n62_));
  nand2  g29(.a(x08), .b(new_n62_), .O(new_n63_));
  nand2  g30(.a(new_n41_), .b(new_n48_), .O(new_n64_));
  nand3  g31(.a(new_n64_), .b(new_n63_), .c(new_n37_), .O(new_n65_));
  inv1   g32(.a(new_n65_), .O(z11));
  nand2  g33(.a(x08), .b(x04), .O(new_n67_));
  aoi22  g34(.a(x15), .b(x09), .c(x13), .d(new_n48_), .O(new_n68_));
  nand2  g35(.a(new_n68_), .b(new_n67_), .O(z12));
  inv1   g36(.a(x05), .O(new_n70_));
  nand2  g37(.a(x08), .b(new_n70_), .O(new_n71_));
  nand2  g38(.a(new_n45_), .b(new_n48_), .O(new_n72_));
  nand3  g39(.a(new_n72_), .b(new_n71_), .c(new_n37_), .O(new_n73_));
  inv1   g40(.a(new_n73_), .O(z13));
  inv1   g41(.a(x06), .O(new_n75_));
  nand2  g42(.a(new_n37_), .b(x08), .O(new_n76_));
  nand3  g43(.a(x15), .b(new_n34_), .c(new_n48_), .O(new_n77_));
  oai21  g44(.a(new_n76_), .b(new_n75_), .c(new_n77_), .O(z14));
  inv1   g45(.a(x07), .O(new_n79_));
  nand3  g46(.a(new_n37_), .b(x16), .c(new_n48_), .O(new_n80_));
  oai21  g47(.a(new_n76_), .b(new_n79_), .c(new_n80_), .O(z15));
  buf    g48(.a(x15), .O(z06));
endmodule


