// Benchmark "FAU" written by ABC on Tue Aug 11 19:08:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n34_, new_n35_, new_n36_, new_n38_, new_n40_, new_n42_, new_n44_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_;
  inv1   g00(.a(x09), .O(new_n34_));
  nand2  g01(.a(x14), .b(x11), .O(new_n35_));
  inv1   g02(.a(new_n35_), .O(new_n36_));
  nor2   g03(.a(new_n36_), .b(new_n34_), .O(z00));
  inv1   g04(.a(x10), .O(new_n38_));
  nand2  g05(.a(new_n35_), .b(new_n38_), .O(z01));
  inv1   g06(.a(x12), .O(new_n40_));
  nand2  g07(.a(new_n35_), .b(new_n40_), .O(z03));
  inv1   g08(.a(x13), .O(new_n42_));
  nand2  g09(.a(new_n35_), .b(new_n42_), .O(z04));
  inv1   g10(.a(x15), .O(new_n44_));
  nor2   g11(.a(new_n36_), .b(new_n44_), .O(z06));
  inv1   g12(.a(x16), .O(new_n46_));
  nor2   g13(.a(new_n36_), .b(new_n46_), .O(z07));
  inv1   g14(.a(x00), .O(new_n48_));
  nand2  g15(.a(x08), .b(new_n48_), .O(new_n49_));
  inv1   g16(.a(x08), .O(new_n50_));
  nand2  g17(.a(new_n34_), .b(new_n50_), .O(new_n51_));
  nand3  g18(.a(new_n51_), .b(new_n49_), .c(new_n35_), .O(new_n52_));
  inv1   g19(.a(new_n52_), .O(z08));
  nand2  g20(.a(x08), .b(x01), .O(new_n54_));
  aoi22  g21(.a(x14), .b(x11), .c(x10), .d(new_n50_), .O(new_n55_));
  nand2  g22(.a(new_n55_), .b(new_n54_), .O(z09));
  inv1   g23(.a(x11), .O(new_n57_));
  nand2  g24(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  inv1   g25(.a(x02), .O(new_n59_));
  nand2  g26(.a(x08), .b(new_n59_), .O(new_n60_));
  nand3  g27(.a(new_n60_), .b(new_n58_), .c(new_n35_), .O(new_n61_));
  inv1   g28(.a(new_n61_), .O(z10));
  inv1   g29(.a(x03), .O(new_n63_));
  nand2  g30(.a(x08), .b(new_n63_), .O(new_n64_));
  nand2  g31(.a(new_n40_), .b(new_n50_), .O(new_n65_));
  nand3  g32(.a(new_n65_), .b(new_n64_), .c(new_n35_), .O(new_n66_));
  inv1   g33(.a(new_n66_), .O(z11));
  nand2  g34(.a(x08), .b(x04), .O(new_n68_));
  aoi22  g35(.a(x14), .b(x11), .c(x13), .d(new_n50_), .O(new_n69_));
  nand2  g36(.a(new_n69_), .b(new_n68_), .O(z12));
  inv1   g37(.a(x14), .O(new_n71_));
  nand3  g38(.a(new_n35_), .b(x08), .c(x05), .O(new_n72_));
  oai21  g39(.a(new_n58_), .b(new_n71_), .c(new_n72_), .O(z13));
  inv1   g40(.a(x06), .O(new_n74_));
  nand2  g41(.a(x08), .b(new_n74_), .O(new_n75_));
  nand2  g42(.a(new_n44_), .b(new_n50_), .O(new_n76_));
  nand3  g43(.a(new_n76_), .b(new_n75_), .c(new_n35_), .O(new_n77_));
  inv1   g44(.a(new_n77_), .O(z14));
  inv1   g45(.a(x07), .O(new_n79_));
  nand2  g46(.a(x08), .b(new_n79_), .O(new_n80_));
  nand2  g47(.a(new_n46_), .b(new_n50_), .O(new_n81_));
  nand3  g48(.a(new_n81_), .b(new_n80_), .c(new_n35_), .O(new_n82_));
  inv1   g49(.a(new_n82_), .O(z15));
  buf    g50(.a(x11), .O(z02));
  buf    g51(.a(x14), .O(z05));
endmodule


