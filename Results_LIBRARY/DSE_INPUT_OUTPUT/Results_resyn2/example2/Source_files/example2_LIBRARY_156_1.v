// Benchmark "FAU" written by ABC on Tue Aug 11 22:52:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n161_, new_n162_, new_n164_, new_n166_, new_n167_, new_n169_,
    new_n170_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n196_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n202_, new_n203_, new_n205_, new_n206_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n212_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n220_, new_n221_, new_n223_,
    new_n224_, new_n225_, new_n226_, new_n227_, new_n229_, new_n230_,
    new_n231_, new_n232_, new_n233_, new_n235_, new_n236_, new_n237_,
    new_n238_, new_n239_, new_n241_, new_n242_, new_n244_, new_n246_,
    new_n247_, new_n248_, new_n250_, new_n261_, new_n262_, new_n263_,
    new_n264_, new_n265_, new_n266_, new_n267_, new_n268_, new_n270_,
    new_n271_, new_n272_, new_n274_, new_n275_, new_n276_, new_n278_,
    new_n279_, new_n280_, new_n281_, new_n283_, new_n284_, new_n285_,
    new_n286_, new_n287_, new_n288_, new_n290_, new_n291_, new_n292_,
    new_n294_, new_n295_, new_n296_, new_n298_, new_n299_, new_n300_,
    new_n301_, new_n302_, new_n304_, new_n305_, new_n307_, new_n308_,
    new_n310_, new_n311_, new_n312_, new_n313_, new_n315_, new_n316_;
  inv1   g000(.a(x40), .O(new_n152_));
  nand2  g001(.a(x78), .b(x77), .O(new_n153_));
  nor2   g002(.a(x79), .b(x01), .O(new_n154_));
  inv1   g003(.a(x01), .O(z01));
  nand2  g004(.a(x79), .b(z01), .O(new_n156_));
  aoi22  g005(.a(new_n156_), .b(x52), .c(new_n154_), .d(new_n153_), .O(new_n157_));
  nand3  g006(.a(new_n156_), .b(new_n152_), .c(x06), .O(new_n158_));
  oai21  g007(.a(new_n157_), .b(new_n152_), .c(new_n158_), .O(z00));
  inv1   g008(.a(new_n156_), .O(z02));
  inv1   g009(.a(x79), .O(new_n161_));
  nand2  g010(.a(x78), .b(x52), .O(new_n162_));
  aoi21  g011(.a(new_n162_), .b(new_n161_), .c(x01), .O(z03));
  nor2   g012(.a(new_n153_), .b(x79), .O(new_n164_));
  nor2   g013(.a(new_n164_), .b(x01), .O(z04));
  nand2  g014(.a(x65), .b(x40), .O(new_n166_));
  nand2  g015(.a(new_n152_), .b(x23), .O(new_n167_));
  nand3  g016(.a(new_n167_), .b(new_n166_), .c(new_n156_), .O(z05));
  nand2  g017(.a(x64), .b(x40), .O(new_n169_));
  nand2  g018(.a(new_n152_), .b(x24), .O(new_n170_));
  nand3  g019(.a(new_n170_), .b(new_n169_), .c(new_n156_), .O(z06));
  nand2  g020(.a(x63), .b(x40), .O(new_n172_));
  nand2  g021(.a(new_n152_), .b(x25), .O(new_n173_));
  nand3  g022(.a(new_n173_), .b(new_n172_), .c(new_n156_), .O(z07));
  inv1   g023(.a(x62), .O(new_n175_));
  nand2  g024(.a(new_n175_), .b(x40), .O(new_n176_));
  inv1   g025(.a(x26), .O(new_n177_));
  nand2  g026(.a(new_n152_), .b(new_n177_), .O(new_n178_));
  nand3  g027(.a(new_n178_), .b(new_n176_), .c(new_n156_), .O(new_n179_));
  inv1   g028(.a(new_n179_), .O(z08));
  inv1   g029(.a(x61), .O(new_n181_));
  nand2  g030(.a(new_n181_), .b(x40), .O(new_n182_));
  inv1   g031(.a(x27), .O(new_n183_));
  nand2  g032(.a(new_n152_), .b(new_n183_), .O(new_n184_));
  nand3  g033(.a(new_n184_), .b(new_n182_), .c(new_n156_), .O(new_n185_));
  inv1   g034(.a(new_n185_), .O(z09));
  inv1   g035(.a(x60), .O(new_n187_));
  nand2  g036(.a(new_n187_), .b(x40), .O(new_n188_));
  inv1   g037(.a(x28), .O(new_n189_));
  nand2  g038(.a(new_n152_), .b(new_n189_), .O(new_n190_));
  nand3  g039(.a(new_n190_), .b(new_n188_), .c(new_n156_), .O(new_n191_));
  inv1   g040(.a(new_n191_), .O(z10));
  nand2  g041(.a(x59), .b(x40), .O(new_n193_));
  nand2  g042(.a(new_n152_), .b(x29), .O(new_n194_));
  nand3  g043(.a(new_n194_), .b(new_n193_), .c(new_n156_), .O(z11));
  inv1   g044(.a(x58), .O(new_n196_));
  nand2  g045(.a(new_n196_), .b(x40), .O(new_n197_));
  inv1   g046(.a(x30), .O(new_n198_));
  nand2  g047(.a(new_n152_), .b(new_n198_), .O(new_n199_));
  nand3  g048(.a(new_n199_), .b(new_n197_), .c(new_n156_), .O(new_n200_));
  inv1   g049(.a(new_n200_), .O(z12));
  nand2  g050(.a(x57), .b(x40), .O(new_n202_));
  nand2  g051(.a(new_n152_), .b(x31), .O(new_n203_));
  nand3  g052(.a(new_n203_), .b(new_n202_), .c(new_n156_), .O(z13));
  nand2  g053(.a(x51), .b(x40), .O(new_n205_));
  nand2  g054(.a(new_n152_), .b(x32), .O(new_n206_));
  nand3  g055(.a(new_n206_), .b(new_n205_), .c(new_n156_), .O(z14));
  inv1   g056(.a(x50), .O(new_n208_));
  nand2  g057(.a(new_n208_), .b(x40), .O(new_n209_));
  inv1   g058(.a(x33), .O(new_n210_));
  nand2  g059(.a(new_n152_), .b(new_n210_), .O(new_n211_));
  nand3  g060(.a(new_n211_), .b(new_n209_), .c(new_n156_), .O(new_n212_));
  inv1   g061(.a(new_n212_), .O(z15));
  inv1   g062(.a(x49), .O(new_n214_));
  nand2  g063(.a(new_n214_), .b(x40), .O(new_n215_));
  inv1   g064(.a(x34), .O(new_n216_));
  nand2  g065(.a(new_n152_), .b(new_n216_), .O(new_n217_));
  nand3  g066(.a(new_n217_), .b(new_n215_), .c(new_n156_), .O(new_n218_));
  inv1   g067(.a(new_n218_), .O(z16));
  nand2  g068(.a(x48), .b(x40), .O(new_n220_));
  nand2  g069(.a(new_n152_), .b(x35), .O(new_n221_));
  nand3  g070(.a(new_n221_), .b(new_n220_), .c(new_n156_), .O(z17));
  inv1   g071(.a(x47), .O(new_n223_));
  nand2  g072(.a(new_n223_), .b(x40), .O(new_n224_));
  inv1   g073(.a(x36), .O(new_n225_));
  nand2  g074(.a(new_n152_), .b(new_n225_), .O(new_n226_));
  nand3  g075(.a(new_n226_), .b(new_n224_), .c(new_n156_), .O(new_n227_));
  inv1   g076(.a(new_n227_), .O(z18));
  inv1   g077(.a(x46), .O(new_n229_));
  nand2  g078(.a(new_n229_), .b(x40), .O(new_n230_));
  inv1   g079(.a(x37), .O(new_n231_));
  nand2  g080(.a(new_n152_), .b(new_n231_), .O(new_n232_));
  nand3  g081(.a(new_n232_), .b(new_n230_), .c(new_n156_), .O(new_n233_));
  inv1   g082(.a(new_n233_), .O(z19));
  inv1   g083(.a(x45), .O(new_n235_));
  nand2  g084(.a(new_n235_), .b(x40), .O(new_n236_));
  inv1   g085(.a(x38), .O(new_n237_));
  nand2  g086(.a(new_n152_), .b(new_n237_), .O(new_n238_));
  nand3  g087(.a(new_n238_), .b(new_n236_), .c(new_n156_), .O(new_n239_));
  inv1   g088(.a(new_n239_), .O(z20));
  nand2  g089(.a(x44), .b(x40), .O(new_n241_));
  nand2  g090(.a(new_n152_), .b(x39), .O(new_n242_));
  nand3  g091(.a(new_n242_), .b(new_n241_), .c(new_n156_), .O(z21));
  nand2  g092(.a(x78), .b(x04), .O(new_n244_));
  aoi21  g093(.a(new_n244_), .b(new_n161_), .c(x01), .O(z22));
  inv1   g094(.a(x04), .O(new_n246_));
  nand2  g095(.a(x05), .b(new_n246_), .O(new_n247_));
  and2   g096(.a(new_n247_), .b(x00), .O(new_n248_));
  oai21  g097(.a(new_n248_), .b(x79), .c(z01), .O(z23));
  inv1   g098(.a(new_n154_), .O(new_n250_));
  nor3   g099(.a(new_n247_), .b(new_n250_), .c(x43), .O(z24));
  inv1   g100(.a(x77), .O(new_n261_));
  nand3  g101(.a(x78), .b(new_n261_), .c(x04), .O(new_n262_));
  inv1   g102(.a(new_n262_), .O(new_n263_));
  nand2  g103(.a(new_n263_), .b(new_n154_), .O(new_n264_));
  inv1   g104(.a(x52), .O(new_n265_));
  inv1   g105(.a(x07), .O(new_n266_));
  nand2  g106(.a(new_n265_), .b(new_n266_), .O(new_n267_));
  oai21  g107(.a(new_n265_), .b(x15), .c(new_n267_), .O(new_n268_));
  nor2   g108(.a(new_n268_), .b(new_n264_), .O(z47));
  inv1   g109(.a(x08), .O(new_n270_));
  nand2  g110(.a(new_n265_), .b(new_n270_), .O(new_n271_));
  oai21  g111(.a(new_n265_), .b(x16), .c(new_n271_), .O(new_n272_));
  nor2   g112(.a(new_n272_), .b(new_n264_), .O(z48));
  inv1   g113(.a(x09), .O(new_n274_));
  nand2  g114(.a(new_n265_), .b(new_n274_), .O(new_n275_));
  oai21  g115(.a(new_n265_), .b(x17), .c(new_n275_), .O(new_n276_));
  nor2   g116(.a(new_n276_), .b(new_n264_), .O(z49));
  inv1   g117(.a(x10), .O(new_n278_));
  nand2  g118(.a(x52), .b(x18), .O(new_n279_));
  oai21  g119(.a(x52), .b(new_n278_), .c(new_n279_), .O(new_n280_));
  nand2  g120(.a(new_n280_), .b(new_n263_), .O(new_n281_));
  aoi21  g121(.a(new_n281_), .b(new_n161_), .c(x01), .O(z50));
  nor2   g122(.a(new_n244_), .b(x77), .O(new_n283_));
  inv1   g123(.a(x19), .O(new_n284_));
  nand2  g124(.a(x52), .b(new_n284_), .O(new_n285_));
  inv1   g125(.a(x11), .O(new_n286_));
  nand2  g126(.a(new_n265_), .b(new_n286_), .O(new_n287_));
  nand3  g127(.a(new_n287_), .b(new_n285_), .c(new_n283_), .O(new_n288_));
  aoi21  g128(.a(new_n288_), .b(new_n161_), .c(x01), .O(z51));
  inv1   g129(.a(x12), .O(new_n290_));
  nand2  g130(.a(new_n265_), .b(new_n290_), .O(new_n291_));
  oai21  g131(.a(new_n265_), .b(x20), .c(new_n291_), .O(new_n292_));
  nor2   g132(.a(new_n292_), .b(new_n264_), .O(z52));
  inv1   g133(.a(x13), .O(new_n294_));
  nand2  g134(.a(new_n265_), .b(new_n294_), .O(new_n295_));
  oai21  g135(.a(new_n265_), .b(x21), .c(new_n295_), .O(new_n296_));
  nor2   g136(.a(new_n296_), .b(new_n264_), .O(z53));
  inv1   g137(.a(x22), .O(new_n298_));
  nand2  g138(.a(x52), .b(new_n298_), .O(new_n299_));
  inv1   g139(.a(x14), .O(new_n300_));
  nand2  g140(.a(new_n265_), .b(new_n300_), .O(new_n301_));
  nand3  g141(.a(new_n301_), .b(new_n299_), .c(new_n283_), .O(new_n302_));
  aoi21  g142(.a(new_n302_), .b(new_n161_), .c(x01), .O(z54));
  oai21  g143(.a(x78), .b(x77), .c(x00), .O(new_n304_));
  nand2  g144(.a(new_n304_), .b(new_n161_), .O(new_n305_));
  nand2  g145(.a(new_n305_), .b(z01), .O(z56));
  inv1   g146(.a(x02), .O(new_n307_));
  nand3  g147(.a(x03), .b(new_n307_), .c(x00), .O(new_n308_));
  nor2   g148(.a(new_n308_), .b(new_n250_), .O(z57));
  inv1   g149(.a(x42), .O(new_n310_));
  inv1   g150(.a(x78), .O(new_n311_));
  nand4  g151(.a(new_n311_), .b(x77), .c(new_n310_), .d(x40), .O(new_n312_));
  aoi21  g152(.a(x78), .b(new_n261_), .c(new_n246_), .O(new_n313_));
  aoi21  g153(.a(new_n313_), .b(new_n312_), .c(new_n250_), .O(z58));
  oai21  g154(.a(x78), .b(x40), .c(x77), .O(new_n315_));
  nor2   g155(.a(x79), .b(new_n246_), .O(new_n316_));
  aoi21  g156(.a(new_n316_), .b(new_n315_), .c(x01), .O(z59));
  aoi21  g157(.a(new_n316_), .b(new_n311_), .c(x01), .O(z60));
  aoi21  g158(.a(new_n262_), .b(new_n161_), .c(x01), .O(z64));
  zero   g159(.O(z25));
  zero   g160(.O(z26));
  zero   g161(.O(z31));
  zero   g162(.O(z33));
  zero   g163(.O(z35));
  zero   g164(.O(z37));
  zero   g165(.O(z38));
  zero   g166(.O(z43));
  zero   g167(.O(z44));
  zero   g168(.O(z61));
  zero   g169(.O(z65));
  inv1   g170(.a(new_n156_), .O(z27));
  inv1   g171(.a(new_n156_), .O(z28));
  inv1   g172(.a(new_n156_), .O(z29));
  inv1   g173(.a(new_n156_), .O(z30));
  inv1   g174(.a(new_n156_), .O(z32));
  inv1   g175(.a(new_n156_), .O(z34));
  inv1   g176(.a(new_n156_), .O(z36));
  inv1   g177(.a(new_n156_), .O(z39));
  inv1   g178(.a(new_n156_), .O(z40));
  inv1   g179(.a(new_n156_), .O(z41));
  inv1   g180(.a(new_n156_), .O(z42));
  inv1   g181(.a(new_n156_), .O(z45));
  inv1   g182(.a(new_n156_), .O(z46));
  inv1   g183(.a(new_n156_), .O(z55));
  aoi21  g184(.a(new_n244_), .b(new_n161_), .c(x01), .O(z62));
  inv1   g185(.a(new_n156_), .O(z63));
endmodule


