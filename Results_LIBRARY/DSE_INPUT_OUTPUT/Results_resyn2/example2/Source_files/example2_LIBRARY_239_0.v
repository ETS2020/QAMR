// Benchmark "FAU" written by ABC on Tue Aug 11 22:53:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n168_, new_n169_, new_n170_, new_n171_, new_n174_,
    new_n175_, new_n177_, new_n178_, new_n180_, new_n181_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n198_, new_n199_, new_n201_,
    new_n202_, new_n204_, new_n205_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n213_, new_n214_, new_n216_, new_n217_, new_n219_,
    new_n220_, new_n222_, new_n223_, new_n225_, new_n226_, new_n227_,
    new_n228_, new_n229_, new_n230_, new_n231_, new_n232_, new_n233_,
    new_n234_, new_n235_, new_n236_, new_n237_, new_n240_, new_n241_,
    new_n242_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n251_, new_n253_, new_n255_, new_n257_, new_n259_,
    new_n261_, new_n263_, new_n265_, new_n266_, new_n267_, new_n268_,
    new_n269_, new_n270_, new_n271_, new_n272_, new_n273_, new_n274_,
    new_n276_, new_n277_, new_n278_, new_n279_, new_n280_, new_n281_,
    new_n282_, new_n283_, new_n284_, new_n285_, new_n287_, new_n288_,
    new_n289_, new_n291_, new_n293_, new_n294_, new_n295_, new_n297_,
    new_n298_, new_n299_, new_n301_, new_n302_, new_n303_, new_n305_,
    new_n307_, new_n308_, new_n309_, new_n311_, new_n312_, new_n313_,
    new_n315_, new_n316_, new_n317_, new_n319_, new_n320_, new_n321_,
    new_n323_, new_n325_, new_n327_, new_n328_, new_n330_, new_n332_,
    new_n334_, new_n335_, new_n337_, new_n339_, new_n341_, new_n344_,
    new_n345_, new_n346_, new_n347_, new_n349_, new_n350_, new_n351_,
    new_n353_, new_n354_, new_n356_, new_n357_, new_n358_, new_n360_,
    new_n361_, new_n363_, new_n365_, new_n366_, new_n368_, new_n369_,
    new_n370_, new_n371_, new_n372_, new_n373_, new_n374_, new_n375_,
    new_n376_, new_n378_, new_n379_, new_n381_, new_n383_, new_n384_;
  inv1   g000(.a(x01), .O(new_n152_));
  inv1   g001(.a(x40), .O(new_n153_));
  nor3   g002(.a(x52), .b(new_n153_), .c(new_n152_), .O(new_n154_));
  inv1   g003(.a(x79), .O(new_n155_));
  nand2  g004(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  oai21  g005(.a(x40), .b(x06), .c(new_n156_), .O(new_n157_));
  nor2   g006(.a(new_n157_), .b(new_n154_), .O(z00));
  nand2  g007(.a(x79), .b(new_n152_), .O(new_n159_));
  nand2  g008(.a(x78), .b(x77), .O(new_n160_));
  inv1   g009(.a(x77), .O(new_n161_));
  inv1   g010(.a(x78), .O(new_n162_));
  nand2  g011(.a(new_n162_), .b(new_n161_), .O(new_n163_));
  nand2  g012(.a(new_n163_), .b(new_n160_), .O(new_n164_));
  inv1   g013(.a(new_n164_), .O(new_n165_));
  nor2   g014(.a(new_n165_), .b(new_n159_), .O(z01));
  inv1   g015(.a(new_n159_), .O(z04));
  inv1   g016(.a(x66), .O(new_n168_));
  nand2  g017(.a(new_n162_), .b(x77), .O(new_n169_));
  nand3  g018(.a(x78), .b(new_n161_), .c(x75), .O(new_n170_));
  oai21  g019(.a(new_n169_), .b(new_n168_), .c(new_n170_), .O(new_n171_));
  and2   g020(.a(new_n171_), .b(z04), .O(z02));
  nor2   g021(.a(x65), .b(new_n153_), .O(new_n174_));
  oai21  g022(.a(x40), .b(x23), .c(new_n156_), .O(new_n175_));
  nor2   g023(.a(new_n175_), .b(new_n174_), .O(z05));
  nand2  g024(.a(x64), .b(x40), .O(new_n177_));
  nand2  g025(.a(new_n153_), .b(x24), .O(new_n178_));
  nand3  g026(.a(new_n178_), .b(new_n177_), .c(new_n156_), .O(z06));
  nand2  g027(.a(x63), .b(x40), .O(new_n180_));
  nand2  g028(.a(new_n153_), .b(x25), .O(new_n181_));
  nand3  g029(.a(new_n181_), .b(new_n180_), .c(new_n156_), .O(z07));
  nor2   g030(.a(x62), .b(new_n153_), .O(new_n183_));
  oai21  g031(.a(x40), .b(x26), .c(new_n156_), .O(new_n184_));
  nor2   g032(.a(new_n184_), .b(new_n183_), .O(z08));
  nand2  g033(.a(x61), .b(x40), .O(new_n186_));
  nand2  g034(.a(new_n153_), .b(x27), .O(new_n187_));
  nand3  g035(.a(new_n187_), .b(new_n186_), .c(new_n156_), .O(z09));
  nand2  g036(.a(x60), .b(x40), .O(new_n189_));
  nand2  g037(.a(new_n153_), .b(x28), .O(new_n190_));
  nand3  g038(.a(new_n190_), .b(new_n189_), .c(new_n156_), .O(z10));
  nor2   g039(.a(x59), .b(new_n153_), .O(new_n192_));
  oai21  g040(.a(x40), .b(x29), .c(new_n156_), .O(new_n193_));
  nor2   g041(.a(new_n193_), .b(new_n192_), .O(z11));
  nand2  g042(.a(x58), .b(x40), .O(new_n195_));
  nand2  g043(.a(new_n153_), .b(x30), .O(new_n196_));
  nand3  g044(.a(new_n196_), .b(new_n195_), .c(new_n156_), .O(z12));
  nand2  g045(.a(x57), .b(x40), .O(new_n198_));
  nand2  g046(.a(new_n153_), .b(x31), .O(new_n199_));
  nand3  g047(.a(new_n199_), .b(new_n198_), .c(new_n156_), .O(z13));
  nor2   g048(.a(x51), .b(new_n153_), .O(new_n201_));
  oai21  g049(.a(x40), .b(x32), .c(new_n156_), .O(new_n202_));
  nor2   g050(.a(new_n202_), .b(new_n201_), .O(z14));
  nand2  g051(.a(x50), .b(x40), .O(new_n204_));
  nand2  g052(.a(new_n153_), .b(x33), .O(new_n205_));
  nand3  g053(.a(new_n205_), .b(new_n204_), .c(new_n156_), .O(z15));
  nor2   g054(.a(x49), .b(new_n153_), .O(new_n207_));
  oai21  g055(.a(x40), .b(x34), .c(new_n156_), .O(new_n208_));
  nor2   g056(.a(new_n208_), .b(new_n207_), .O(z16));
  nor2   g057(.a(x48), .b(new_n153_), .O(new_n210_));
  oai21  g058(.a(x40), .b(x35), .c(new_n156_), .O(new_n211_));
  nor2   g059(.a(new_n211_), .b(new_n210_), .O(z17));
  nand2  g060(.a(x47), .b(x40), .O(new_n213_));
  nand2  g061(.a(new_n153_), .b(x36), .O(new_n214_));
  nand3  g062(.a(new_n214_), .b(new_n213_), .c(new_n156_), .O(z18));
  nor2   g063(.a(x46), .b(new_n153_), .O(new_n216_));
  oai21  g064(.a(x40), .b(x37), .c(new_n156_), .O(new_n217_));
  nor2   g065(.a(new_n217_), .b(new_n216_), .O(z19));
  nand2  g066(.a(x45), .b(x40), .O(new_n219_));
  nand2  g067(.a(new_n153_), .b(x38), .O(new_n220_));
  nand3  g068(.a(new_n220_), .b(new_n219_), .c(new_n156_), .O(z20));
  nand2  g069(.a(x44), .b(x40), .O(new_n222_));
  nand2  g070(.a(new_n153_), .b(x39), .O(new_n223_));
  nand3  g071(.a(new_n223_), .b(new_n222_), .c(new_n156_), .O(z21));
  inv1   g072(.a(x42), .O(new_n225_));
  nand3  g073(.a(x78), .b(x77), .c(x04), .O(new_n226_));
  inv1   g074(.a(new_n226_), .O(new_n227_));
  inv1   g075(.a(x43), .O(new_n228_));
  nor2   g076(.a(x74), .b(new_n228_), .O(new_n229_));
  and2   g077(.a(x82), .b(x80), .O(new_n230_));
  inv1   g078(.a(x84), .O(new_n231_));
  nor2   g079(.a(new_n231_), .b(x83), .O(new_n232_));
  nand4  g080(.a(new_n232_), .b(new_n230_), .c(new_n229_), .d(x81), .O(new_n233_));
  nand3  g081(.a(new_n233_), .b(new_n227_), .c(new_n225_), .O(new_n234_));
  xor2a  g082(.a(x84), .b(x81), .O(new_n235_));
  nor2   g083(.a(new_n235_), .b(x41), .O(new_n236_));
  aoi21  g084(.a(new_n236_), .b(new_n171_), .c(new_n155_), .O(new_n237_));
  aoi21  g085(.a(new_n237_), .b(new_n234_), .c(x01), .O(z22));
  oai21  g086(.a(new_n155_), .b(x00), .c(new_n152_), .O(z23));
  nor2   g087(.a(new_n160_), .b(x04), .O(new_n240_));
  inv1   g088(.a(new_n240_), .O(new_n241_));
  nand2  g089(.a(new_n228_), .b(x05), .O(new_n242_));
  nor3   g090(.a(new_n242_), .b(new_n241_), .c(new_n159_), .O(z24));
  inv1   g091(.a(x81), .O(new_n244_));
  xor2a  g092(.a(x84), .b(x82), .O(new_n245_));
  xor2a  g093(.a(new_n245_), .b(new_n244_), .O(new_n246_));
  nor2   g094(.a(new_n246_), .b(new_n160_), .O(new_n247_));
  nor2   g095(.a(x42), .b(x04), .O(new_n248_));
  nand4  g096(.a(new_n248_), .b(new_n247_), .c(z04), .d(x05), .O(new_n249_));
  inv1   g097(.a(new_n249_), .O(z25));
  nand3  g098(.a(new_n248_), .b(new_n247_), .c(x44), .O(new_n251_));
  aoi21  g099(.a(new_n251_), .b(x79), .c(x01), .O(z26));
  nand3  g100(.a(new_n248_), .b(new_n247_), .c(x45), .O(new_n253_));
  aoi21  g101(.a(new_n253_), .b(x79), .c(x01), .O(z27));
  nand4  g102(.a(new_n248_), .b(new_n247_), .c(z04), .d(x46), .O(new_n255_));
  inv1   g103(.a(new_n255_), .O(z28));
  nand3  g104(.a(new_n248_), .b(new_n247_), .c(x47), .O(new_n257_));
  aoi21  g105(.a(new_n257_), .b(x79), .c(x01), .O(z29));
  nand4  g106(.a(new_n248_), .b(new_n247_), .c(z04), .d(x48), .O(new_n259_));
  inv1   g107(.a(new_n259_), .O(z30));
  nand4  g108(.a(new_n248_), .b(new_n247_), .c(z04), .d(x49), .O(new_n261_));
  inv1   g109(.a(new_n261_), .O(z31));
  nand3  g110(.a(new_n248_), .b(new_n247_), .c(x50), .O(new_n263_));
  aoi21  g111(.a(new_n263_), .b(x79), .c(x01), .O(z32));
  nand2  g112(.a(x42), .b(x05), .O(new_n265_));
  xor2a  g113(.a(x83), .b(x81), .O(new_n266_));
  or2    g114(.a(new_n266_), .b(new_n265_), .O(new_n267_));
  nand3  g115(.a(new_n244_), .b(x51), .c(new_n225_), .O(new_n268_));
  nand3  g116(.a(new_n268_), .b(new_n267_), .c(new_n245_), .O(new_n269_));
  inv1   g117(.a(new_n245_), .O(new_n270_));
  nand3  g118(.a(new_n266_), .b(x42), .c(x05), .O(new_n271_));
  nand3  g119(.a(x81), .b(x51), .c(new_n225_), .O(new_n272_));
  nand3  g120(.a(new_n272_), .b(new_n271_), .c(new_n270_), .O(new_n273_));
  nand4  g121(.a(new_n273_), .b(new_n269_), .c(new_n240_), .d(z04), .O(new_n274_));
  inv1   g122(.a(new_n274_), .O(z33));
  xor2a  g123(.a(new_n245_), .b(x81), .O(new_n276_));
  nand3  g124(.a(new_n276_), .b(x83), .c(x42), .O(new_n277_));
  inv1   g125(.a(x83), .O(new_n278_));
  oai21  g126(.a(new_n278_), .b(new_n225_), .c(new_n246_), .O(new_n279_));
  nand3  g127(.a(x79), .b(x78), .c(x77), .O(new_n280_));
  inv1   g128(.a(new_n280_), .O(new_n281_));
  nor2   g129(.a(x04), .b(x01), .O(new_n282_));
  nand2  g130(.a(new_n282_), .b(x52), .O(new_n283_));
  inv1   g131(.a(new_n283_), .O(new_n284_));
  nand4  g132(.a(new_n284_), .b(new_n281_), .c(new_n279_), .d(new_n277_), .O(new_n285_));
  inv1   g133(.a(new_n285_), .O(z34));
  nand2  g134(.a(new_n282_), .b(x53), .O(new_n287_));
  inv1   g135(.a(new_n287_), .O(new_n288_));
  nand4  g136(.a(new_n288_), .b(new_n281_), .c(new_n279_), .d(new_n277_), .O(new_n289_));
  inv1   g137(.a(new_n289_), .O(z35));
  nand4  g138(.a(new_n279_), .b(new_n277_), .c(new_n240_), .d(x54), .O(new_n291_));
  aoi21  g139(.a(new_n291_), .b(x79), .c(x01), .O(z36));
  nand2  g140(.a(new_n282_), .b(x55), .O(new_n293_));
  inv1   g141(.a(new_n293_), .O(new_n294_));
  nand4  g142(.a(new_n294_), .b(new_n281_), .c(new_n279_), .d(new_n277_), .O(new_n295_));
  inv1   g143(.a(new_n295_), .O(z37));
  nand2  g144(.a(new_n282_), .b(x56), .O(new_n297_));
  inv1   g145(.a(new_n297_), .O(new_n298_));
  nand4  g146(.a(new_n298_), .b(new_n281_), .c(new_n279_), .d(new_n277_), .O(new_n299_));
  inv1   g147(.a(new_n299_), .O(z38));
  nand2  g148(.a(new_n282_), .b(x57), .O(new_n301_));
  inv1   g149(.a(new_n301_), .O(new_n302_));
  nand4  g150(.a(new_n302_), .b(new_n281_), .c(new_n279_), .d(new_n277_), .O(new_n303_));
  inv1   g151(.a(new_n303_), .O(z39));
  nand4  g152(.a(new_n279_), .b(new_n277_), .c(new_n240_), .d(x58), .O(new_n305_));
  aoi21  g153(.a(new_n305_), .b(x79), .c(x01), .O(z40));
  nand2  g154(.a(new_n282_), .b(x59), .O(new_n307_));
  inv1   g155(.a(new_n307_), .O(new_n308_));
  nand4  g156(.a(new_n308_), .b(new_n281_), .c(new_n279_), .d(new_n277_), .O(new_n309_));
  inv1   g157(.a(new_n309_), .O(z41));
  nand2  g158(.a(new_n282_), .b(x60), .O(new_n311_));
  inv1   g159(.a(new_n311_), .O(new_n312_));
  nand4  g160(.a(new_n312_), .b(new_n281_), .c(new_n279_), .d(new_n277_), .O(new_n313_));
  inv1   g161(.a(new_n313_), .O(z42));
  nand2  g162(.a(new_n282_), .b(x61), .O(new_n315_));
  inv1   g163(.a(new_n315_), .O(new_n316_));
  nand4  g164(.a(new_n316_), .b(new_n281_), .c(new_n279_), .d(new_n277_), .O(new_n317_));
  inv1   g165(.a(new_n317_), .O(z43));
  nand2  g166(.a(new_n282_), .b(x62), .O(new_n319_));
  inv1   g167(.a(new_n319_), .O(new_n320_));
  nand4  g168(.a(new_n320_), .b(new_n281_), .c(new_n279_), .d(new_n277_), .O(new_n321_));
  inv1   g169(.a(new_n321_), .O(z44));
  nand4  g170(.a(new_n279_), .b(new_n277_), .c(new_n240_), .d(x63), .O(new_n323_));
  aoi21  g171(.a(new_n323_), .b(x79), .c(x01), .O(z45));
  nand4  g172(.a(new_n279_), .b(new_n277_), .c(new_n240_), .d(x64), .O(new_n325_));
  aoi21  g173(.a(new_n325_), .b(x79), .c(x01), .O(z46));
  nor3   g174(.a(new_n235_), .b(new_n169_), .c(new_n159_), .O(new_n327_));
  oai21  g175(.a(x75), .b(x67), .c(new_n327_), .O(new_n328_));
  inv1   g176(.a(new_n328_), .O(z47));
  nand2  g177(.a(new_n327_), .b(x68), .O(new_n330_));
  inv1   g178(.a(new_n330_), .O(z48));
  nand2  g179(.a(new_n327_), .b(x69), .O(new_n332_));
  inv1   g180(.a(new_n332_), .O(z49));
  nor2   g181(.a(new_n235_), .b(new_n169_), .O(new_n334_));
  nand2  g182(.a(new_n334_), .b(x70), .O(new_n335_));
  aoi21  g183(.a(new_n335_), .b(x79), .c(x01), .O(z50));
  nand2  g184(.a(new_n334_), .b(x71), .O(new_n337_));
  aoi21  g185(.a(new_n337_), .b(x79), .c(x01), .O(z51));
  nand2  g186(.a(new_n327_), .b(x72), .O(new_n339_));
  inv1   g187(.a(new_n339_), .O(z52));
  nand2  g188(.a(new_n334_), .b(x73), .O(new_n341_));
  aoi21  g189(.a(new_n341_), .b(x79), .c(x01), .O(z53));
  inv1   g190(.a(x80), .O(new_n344_));
  nor2   g191(.a(new_n231_), .b(x82), .O(new_n345_));
  nor2   g192(.a(new_n278_), .b(x81), .O(new_n346_));
  nand4  g193(.a(new_n346_), .b(new_n345_), .c(new_n240_), .d(new_n344_), .O(new_n347_));
  aoi21  g194(.a(new_n347_), .b(x79), .c(x01), .O(z55));
  or2    g195(.a(new_n235_), .b(x76), .O(new_n349_));
  nand2  g196(.a(new_n163_), .b(x00), .O(new_n350_));
  aoi21  g197(.a(new_n349_), .b(new_n160_), .c(new_n350_), .O(new_n351_));
  oai21  g198(.a(new_n351_), .b(new_n155_), .c(new_n152_), .O(z56));
  inv1   g199(.a(x02), .O(new_n353_));
  nand3  g200(.a(x03), .b(new_n353_), .c(x00), .O(new_n354_));
  nor2   g201(.a(new_n354_), .b(new_n159_), .O(z57));
  nand2  g202(.a(new_n233_), .b(new_n225_), .O(new_n356_));
  aoi21  g203(.a(x42), .b(x40), .c(new_n226_), .O(new_n357_));
  nand2  g204(.a(new_n357_), .b(new_n356_), .O(new_n358_));
  aoi21  g205(.a(new_n358_), .b(x79), .c(x01), .O(z58));
  nand2  g206(.a(new_n356_), .b(new_n153_), .O(new_n360_));
  nand2  g207(.a(new_n360_), .b(new_n227_), .O(new_n361_));
  aoi21  g208(.a(new_n361_), .b(x79), .c(x01), .O(z59));
  nand2  g209(.a(new_n235_), .b(new_n165_), .O(new_n363_));
  aoi21  g210(.a(new_n363_), .b(new_n234_), .c(new_n159_), .O(z60));
  oai21  g211(.a(new_n235_), .b(new_n164_), .c(new_n241_), .O(new_n365_));
  nand2  g212(.a(new_n365_), .b(x80), .O(new_n366_));
  aoi21  g213(.a(new_n366_), .b(x79), .c(x01), .O(z61));
  nor2   g214(.a(new_n164_), .b(new_n231_), .O(new_n368_));
  inv1   g215(.a(x74), .O(new_n369_));
  nand2  g216(.a(new_n369_), .b(x43), .O(new_n370_));
  nand4  g217(.a(x84), .b(new_n278_), .c(x82), .d(x80), .O(new_n371_));
  oai21  g218(.a(new_n371_), .b(new_n370_), .c(new_n225_), .O(new_n372_));
  aoi21  g219(.a(new_n372_), .b(x04), .c(new_n160_), .O(new_n373_));
  oai21  g220(.a(new_n373_), .b(new_n368_), .c(x81), .O(new_n374_));
  nor2   g221(.a(x81), .b(x42), .O(new_n375_));
  aoi21  g222(.a(new_n375_), .b(new_n227_), .c(new_n155_), .O(new_n376_));
  aoi21  g223(.a(new_n376_), .b(new_n374_), .c(x01), .O(z62));
  nand2  g224(.a(z04), .b(x82), .O(new_n378_));
  inv1   g225(.a(new_n378_), .O(new_n379_));
  and2   g226(.a(new_n379_), .b(new_n365_), .O(z63));
  nand2  g227(.a(new_n365_), .b(x83), .O(new_n381_));
  aoi21  g228(.a(new_n381_), .b(x79), .c(x01), .O(z64));
  oai21  g229(.a(new_n164_), .b(new_n244_), .c(new_n241_), .O(new_n383_));
  nand2  g230(.a(new_n383_), .b(x84), .O(new_n384_));
  aoi21  g231(.a(new_n384_), .b(x79), .c(x01), .O(z65));
  zero   g232(.O(z03));
  zero   g233(.O(z54));
endmodule


