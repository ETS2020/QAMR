// Benchmark "FAU" written by ABC on Tue Aug 11 22:52:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n166_,
    new_n169_, new_n170_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n178_, new_n179_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n196_, new_n197_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n205_, new_n206_,
    new_n208_, new_n209_, new_n211_, new_n212_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n220_, new_n221_, new_n223_,
    new_n224_, new_n226_, new_n227_, new_n228_, new_n229_, new_n230_,
    new_n232_, new_n233_, new_n235_, new_n236_, new_n237_, new_n238_,
    new_n239_, new_n241_, new_n242_, new_n243_, new_n245_, new_n246_,
    new_n247_, new_n249_, new_n250_, new_n263_, new_n264_, new_n265_,
    new_n266_, new_n267_, new_n269_, new_n270_, new_n271_, new_n272_,
    new_n274_, new_n275_, new_n276_, new_n277_, new_n278_, new_n280_,
    new_n281_, new_n282_, new_n283_, new_n284_, new_n286_, new_n287_,
    new_n288_, new_n289_, new_n290_, new_n292_, new_n293_, new_n294_,
    new_n296_, new_n297_, new_n298_, new_n299_, new_n300_, new_n302_,
    new_n303_, new_n304_, new_n305_, new_n306_, new_n308_, new_n310_,
    new_n311_, new_n312_, new_n314_, new_n315_, new_n316_, new_n317_,
    new_n318_, new_n320_, new_n324_;
  inv1   g000(.a(x40), .O(new_n152_));
  inv1   g001(.a(x77), .O(new_n153_));
  inv1   g002(.a(x78), .O(new_n154_));
  nor2   g003(.a(x79), .b(x01), .O(new_n155_));
  oai21  g004(.a(new_n154_), .b(new_n153_), .c(new_n155_), .O(new_n156_));
  inv1   g005(.a(x52), .O(new_n157_));
  nand2  g006(.a(new_n157_), .b(x40), .O(new_n158_));
  inv1   g007(.a(x01), .O(z01));
  nand2  g008(.a(x79), .b(z01), .O(new_n160_));
  inv1   g009(.a(x06), .O(new_n161_));
  nand2  g010(.a(new_n152_), .b(new_n161_), .O(new_n162_));
  nand3  g011(.a(new_n162_), .b(new_n160_), .c(new_n158_), .O(new_n163_));
  oai21  g012(.a(new_n156_), .b(new_n152_), .c(new_n163_), .O(z00));
  inv1   g013(.a(new_n160_), .O(z02));
  nand3  g014(.a(new_n155_), .b(x78), .c(x52), .O(new_n166_));
  inv1   g015(.a(new_n166_), .O(z03));
  inv1   g016(.a(new_n156_), .O(z04));
  nand2  g017(.a(x65), .b(x40), .O(new_n169_));
  nand2  g018(.a(new_n152_), .b(x23), .O(new_n170_));
  nand3  g019(.a(new_n170_), .b(new_n169_), .c(new_n160_), .O(z05));
  inv1   g020(.a(x64), .O(new_n172_));
  nand2  g021(.a(new_n172_), .b(x40), .O(new_n173_));
  inv1   g022(.a(x24), .O(new_n174_));
  nand2  g023(.a(new_n152_), .b(new_n174_), .O(new_n175_));
  nand3  g024(.a(new_n175_), .b(new_n173_), .c(new_n160_), .O(new_n176_));
  inv1   g025(.a(new_n176_), .O(z06));
  nand2  g026(.a(x63), .b(x40), .O(new_n178_));
  nand2  g027(.a(new_n152_), .b(x25), .O(new_n179_));
  nand3  g028(.a(new_n179_), .b(new_n178_), .c(new_n160_), .O(z07));
  inv1   g029(.a(x62), .O(new_n181_));
  nand2  g030(.a(new_n181_), .b(x40), .O(new_n182_));
  inv1   g031(.a(x26), .O(new_n183_));
  nand2  g032(.a(new_n152_), .b(new_n183_), .O(new_n184_));
  nand3  g033(.a(new_n184_), .b(new_n182_), .c(new_n160_), .O(new_n185_));
  inv1   g034(.a(new_n185_), .O(z08));
  nand2  g035(.a(x61), .b(x40), .O(new_n187_));
  nand2  g036(.a(new_n152_), .b(x27), .O(new_n188_));
  nand3  g037(.a(new_n188_), .b(new_n187_), .c(new_n160_), .O(z09));
  inv1   g038(.a(x60), .O(new_n190_));
  nand2  g039(.a(new_n190_), .b(x40), .O(new_n191_));
  inv1   g040(.a(x28), .O(new_n192_));
  nand2  g041(.a(new_n152_), .b(new_n192_), .O(new_n193_));
  nand3  g042(.a(new_n193_), .b(new_n191_), .c(new_n160_), .O(new_n194_));
  inv1   g043(.a(new_n194_), .O(z10));
  nand2  g044(.a(x59), .b(x40), .O(new_n196_));
  nand2  g045(.a(new_n152_), .b(x29), .O(new_n197_));
  nand3  g046(.a(new_n197_), .b(new_n196_), .c(new_n160_), .O(z11));
  inv1   g047(.a(x58), .O(new_n199_));
  nand2  g048(.a(new_n199_), .b(x40), .O(new_n200_));
  inv1   g049(.a(x30), .O(new_n201_));
  nand2  g050(.a(new_n152_), .b(new_n201_), .O(new_n202_));
  nand3  g051(.a(new_n202_), .b(new_n200_), .c(new_n160_), .O(new_n203_));
  inv1   g052(.a(new_n203_), .O(z12));
  nand2  g053(.a(x57), .b(x40), .O(new_n205_));
  nand2  g054(.a(new_n152_), .b(x31), .O(new_n206_));
  nand3  g055(.a(new_n206_), .b(new_n205_), .c(new_n160_), .O(z13));
  nand2  g056(.a(x51), .b(x40), .O(new_n208_));
  nand2  g057(.a(new_n152_), .b(x32), .O(new_n209_));
  nand3  g058(.a(new_n209_), .b(new_n208_), .c(new_n160_), .O(z14));
  nand2  g059(.a(x50), .b(x40), .O(new_n211_));
  nand2  g060(.a(new_n152_), .b(x33), .O(new_n212_));
  nand3  g061(.a(new_n212_), .b(new_n211_), .c(new_n160_), .O(z15));
  inv1   g062(.a(x49), .O(new_n214_));
  nand2  g063(.a(new_n214_), .b(x40), .O(new_n215_));
  inv1   g064(.a(x34), .O(new_n216_));
  nand2  g065(.a(new_n152_), .b(new_n216_), .O(new_n217_));
  nand3  g066(.a(new_n217_), .b(new_n215_), .c(new_n160_), .O(new_n218_));
  inv1   g067(.a(new_n218_), .O(z16));
  nand2  g068(.a(x48), .b(x40), .O(new_n220_));
  nand2  g069(.a(new_n152_), .b(x35), .O(new_n221_));
  nand3  g070(.a(new_n221_), .b(new_n220_), .c(new_n160_), .O(z17));
  nand2  g071(.a(x47), .b(x40), .O(new_n223_));
  nand2  g072(.a(new_n152_), .b(x36), .O(new_n224_));
  nand3  g073(.a(new_n224_), .b(new_n223_), .c(new_n160_), .O(z18));
  inv1   g074(.a(x46), .O(new_n226_));
  nand2  g075(.a(new_n226_), .b(x40), .O(new_n227_));
  inv1   g076(.a(x37), .O(new_n228_));
  nand2  g077(.a(new_n152_), .b(new_n228_), .O(new_n229_));
  nand3  g078(.a(new_n229_), .b(new_n227_), .c(new_n160_), .O(new_n230_));
  inv1   g079(.a(new_n230_), .O(z19));
  nand2  g080(.a(x45), .b(x40), .O(new_n232_));
  nand2  g081(.a(new_n152_), .b(x38), .O(new_n233_));
  nand3  g082(.a(new_n233_), .b(new_n232_), .c(new_n160_), .O(z20));
  inv1   g083(.a(x44), .O(new_n235_));
  nand2  g084(.a(new_n235_), .b(x40), .O(new_n236_));
  inv1   g085(.a(x39), .O(new_n237_));
  nand2  g086(.a(new_n152_), .b(new_n237_), .O(new_n238_));
  nand3  g087(.a(new_n238_), .b(new_n236_), .c(new_n160_), .O(new_n239_));
  inv1   g088(.a(new_n239_), .O(z21));
  inv1   g089(.a(x79), .O(new_n241_));
  and2   g090(.a(x78), .b(x04), .O(new_n242_));
  inv1   g091(.a(new_n242_), .O(new_n243_));
  aoi21  g092(.a(new_n243_), .b(new_n241_), .c(x01), .O(z22));
  inv1   g093(.a(x00), .O(new_n245_));
  inv1   g094(.a(x04), .O(new_n246_));
  aoi21  g095(.a(x05), .b(new_n246_), .c(new_n245_), .O(new_n247_));
  oai21  g096(.a(new_n247_), .b(x79), .c(z01), .O(z23));
  inv1   g097(.a(x43), .O(new_n249_));
  nand3  g098(.a(new_n249_), .b(x05), .c(new_n246_), .O(new_n250_));
  aoi21  g099(.a(new_n250_), .b(new_n241_), .c(x01), .O(z24));
  inv1   g100(.a(x07), .O(new_n263_));
  nand2  g101(.a(new_n157_), .b(new_n263_), .O(new_n264_));
  inv1   g102(.a(x15), .O(new_n265_));
  nand2  g103(.a(x52), .b(new_n265_), .O(new_n266_));
  nand4  g104(.a(new_n266_), .b(new_n264_), .c(new_n242_), .d(new_n153_), .O(new_n267_));
  aoi21  g105(.a(new_n267_), .b(new_n241_), .c(x01), .O(z47));
  nand3  g106(.a(new_n242_), .b(new_n155_), .c(new_n153_), .O(new_n269_));
  inv1   g107(.a(x08), .O(new_n270_));
  nand2  g108(.a(new_n157_), .b(new_n270_), .O(new_n271_));
  oai21  g109(.a(new_n157_), .b(x16), .c(new_n271_), .O(new_n272_));
  nor2   g110(.a(new_n272_), .b(new_n269_), .O(z48));
  inv1   g111(.a(x17), .O(new_n274_));
  nand2  g112(.a(x52), .b(new_n274_), .O(new_n275_));
  inv1   g113(.a(x09), .O(new_n276_));
  nand2  g114(.a(new_n157_), .b(new_n276_), .O(new_n277_));
  nand4  g115(.a(new_n277_), .b(new_n275_), .c(new_n242_), .d(new_n153_), .O(new_n278_));
  aoi21  g116(.a(new_n278_), .b(new_n241_), .c(x01), .O(z49));
  inv1   g117(.a(x18), .O(new_n280_));
  nand2  g118(.a(x52), .b(new_n280_), .O(new_n281_));
  inv1   g119(.a(x10), .O(new_n282_));
  nand2  g120(.a(new_n157_), .b(new_n282_), .O(new_n283_));
  nand4  g121(.a(new_n283_), .b(new_n281_), .c(new_n242_), .d(new_n153_), .O(new_n284_));
  aoi21  g122(.a(new_n284_), .b(new_n241_), .c(x01), .O(z50));
  inv1   g123(.a(x19), .O(new_n286_));
  nand2  g124(.a(x52), .b(new_n286_), .O(new_n287_));
  inv1   g125(.a(x11), .O(new_n288_));
  nand2  g126(.a(new_n157_), .b(new_n288_), .O(new_n289_));
  nand4  g127(.a(new_n289_), .b(new_n287_), .c(new_n242_), .d(new_n153_), .O(new_n290_));
  aoi21  g128(.a(new_n290_), .b(new_n241_), .c(x01), .O(z51));
  inv1   g129(.a(x12), .O(new_n292_));
  nand2  g130(.a(new_n157_), .b(new_n292_), .O(new_n293_));
  oai21  g131(.a(new_n157_), .b(x20), .c(new_n293_), .O(new_n294_));
  nor2   g132(.a(new_n294_), .b(new_n269_), .O(z52));
  inv1   g133(.a(x21), .O(new_n296_));
  nand2  g134(.a(x52), .b(new_n296_), .O(new_n297_));
  inv1   g135(.a(x13), .O(new_n298_));
  nand2  g136(.a(new_n157_), .b(new_n298_), .O(new_n299_));
  nand4  g137(.a(new_n299_), .b(new_n297_), .c(new_n242_), .d(new_n153_), .O(new_n300_));
  aoi21  g138(.a(new_n300_), .b(new_n241_), .c(x01), .O(z53));
  inv1   g139(.a(x22), .O(new_n302_));
  nand2  g140(.a(x52), .b(new_n302_), .O(new_n303_));
  inv1   g141(.a(x14), .O(new_n304_));
  nand2  g142(.a(new_n157_), .b(new_n304_), .O(new_n305_));
  nand4  g143(.a(new_n305_), .b(new_n303_), .c(new_n242_), .d(new_n153_), .O(new_n306_));
  aoi21  g144(.a(new_n306_), .b(new_n241_), .c(x01), .O(z54));
  aoi21  g145(.a(new_n154_), .b(new_n153_), .c(new_n245_), .O(new_n308_));
  oai21  g146(.a(new_n308_), .b(x79), .c(z01), .O(z56));
  inv1   g147(.a(new_n155_), .O(new_n310_));
  inv1   g148(.a(x02), .O(new_n311_));
  nand3  g149(.a(x03), .b(new_n311_), .c(x00), .O(new_n312_));
  nor2   g150(.a(new_n312_), .b(new_n310_), .O(z57));
  nor2   g151(.a(x79), .b(new_n246_), .O(new_n314_));
  nand2  g152(.a(x77), .b(x40), .O(new_n315_));
  nand2  g153(.a(new_n315_), .b(new_n154_), .O(new_n316_));
  oai21  g154(.a(x78), .b(x42), .c(x77), .O(new_n317_));
  nand2  g155(.a(new_n317_), .b(new_n316_), .O(new_n318_));
  aoi21  g156(.a(new_n318_), .b(new_n314_), .c(x01), .O(z58));
  oai21  g157(.a(x78), .b(x40), .c(x77), .O(new_n320_));
  aoi21  g158(.a(new_n320_), .b(new_n314_), .c(x01), .O(z59));
  aoi21  g159(.a(new_n154_), .b(x04), .c(new_n310_), .O(z60));
  nand2  g160(.a(new_n242_), .b(new_n153_), .O(new_n324_));
  aoi21  g161(.a(new_n324_), .b(new_n241_), .c(x01), .O(z64));
  zero   g162(.O(z25));
  zero   g163(.O(z27));
  zero   g164(.O(z28));
  zero   g165(.O(z30));
  zero   g166(.O(z31));
  zero   g167(.O(z32));
  zero   g168(.O(z33));
  zero   g169(.O(z35));
  zero   g170(.O(z36));
  zero   g171(.O(z41));
  zero   g172(.O(z44));
  zero   g173(.O(z61));
  inv1   g174(.a(new_n160_), .O(z26));
  inv1   g175(.a(new_n160_), .O(z29));
  inv1   g176(.a(new_n160_), .O(z34));
  inv1   g177(.a(new_n160_), .O(z37));
  inv1   g178(.a(new_n160_), .O(z38));
  inv1   g179(.a(new_n160_), .O(z39));
  inv1   g180(.a(new_n160_), .O(z40));
  inv1   g181(.a(new_n160_), .O(z42));
  inv1   g182(.a(new_n160_), .O(z43));
  inv1   g183(.a(new_n160_), .O(z45));
  inv1   g184(.a(new_n160_), .O(z46));
  inv1   g185(.a(new_n160_), .O(z55));
  aoi21  g186(.a(new_n243_), .b(new_n241_), .c(x01), .O(z62));
  inv1   g187(.a(new_n160_), .O(z63));
  inv1   g188(.a(new_n160_), .O(z65));
endmodule


