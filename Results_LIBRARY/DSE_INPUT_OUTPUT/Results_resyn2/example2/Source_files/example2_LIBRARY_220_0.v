// Benchmark "FAU" written by ABC on Tue Aug 11 22:53:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n173_, new_n174_,
    new_n176_, new_n177_, new_n179_, new_n180_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n200_, new_n201_,
    new_n203_, new_n204_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n212_, new_n213_, new_n215_, new_n216_, new_n218_, new_n219_,
    new_n221_, new_n222_, new_n224_, new_n225_, new_n226_, new_n227_,
    new_n228_, new_n229_, new_n230_, new_n231_, new_n232_, new_n233_,
    new_n234_, new_n235_, new_n236_, new_n237_, new_n238_, new_n239_,
    new_n240_, new_n243_, new_n244_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n250_, new_n251_, new_n253_, new_n255_, new_n256_,
    new_n257_, new_n259_, new_n261_, new_n263_, new_n265_, new_n267_,
    new_n268_, new_n269_, new_n271_, new_n272_, new_n273_, new_n274_,
    new_n275_, new_n276_, new_n278_, new_n279_, new_n280_, new_n281_,
    new_n282_, new_n283_, new_n284_, new_n285_, new_n286_, new_n287_,
    new_n288_, new_n289_, new_n291_, new_n293_, new_n294_, new_n295_,
    new_n297_, new_n299_, new_n301_, new_n302_, new_n303_, new_n305_,
    new_n307_, new_n309_, new_n310_, new_n311_, new_n313_, new_n315_,
    new_n317_, new_n318_, new_n319_, new_n321_, new_n322_, new_n323_,
    new_n325_, new_n326_, new_n328_, new_n330_, new_n331_, new_n333_,
    new_n335_, new_n337_, new_n339_, new_n341_, new_n342_, new_n343_,
    new_n344_, new_n345_, new_n346_, new_n348_, new_n349_, new_n351_,
    new_n352_, new_n354_, new_n355_, new_n356_, new_n358_, new_n360_,
    new_n361_, new_n363_, new_n364_, new_n366_, new_n367_, new_n369_,
    new_n371_, new_n373_, new_n374_;
  inv1   g000(.a(x01), .O(z04));
  inv1   g001(.a(x79), .O(new_n153_));
  oai21  g002(.a(new_n153_), .b(x40), .c(z04), .O(new_n154_));
  inv1   g003(.a(x40), .O(new_n155_));
  nand2  g004(.a(new_n155_), .b(x06), .O(new_n156_));
  nand2  g005(.a(x52), .b(x40), .O(new_n157_));
  nand3  g006(.a(new_n157_), .b(new_n156_), .c(new_n154_), .O(z00));
  nor2   g007(.a(new_n153_), .b(x01), .O(new_n159_));
  inv1   g008(.a(new_n159_), .O(new_n160_));
  nand2  g009(.a(x78), .b(x77), .O(new_n161_));
  inv1   g010(.a(new_n161_), .O(new_n162_));
  nor2   g011(.a(x78), .b(x77), .O(new_n163_));
  nor2   g012(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nor2   g013(.a(new_n164_), .b(new_n160_), .O(z01));
  inv1   g014(.a(x78), .O(new_n166_));
  nand3  g015(.a(new_n166_), .b(x77), .c(x66), .O(new_n167_));
  inv1   g016(.a(x77), .O(new_n168_));
  nand3  g017(.a(x78), .b(new_n168_), .c(x75), .O(new_n169_));
  aoi21  g018(.a(new_n169_), .b(new_n167_), .c(new_n160_), .O(z02));
  nand2  g019(.a(new_n153_), .b(z04), .O(new_n171_));
  inv1   g020(.a(new_n171_), .O(z03));
  nor2   g021(.a(x65), .b(new_n155_), .O(new_n173_));
  oai21  g022(.a(x40), .b(x23), .c(new_n171_), .O(new_n174_));
  nor2   g023(.a(new_n174_), .b(new_n173_), .O(z05));
  nor2   g024(.a(x64), .b(new_n155_), .O(new_n176_));
  oai21  g025(.a(x40), .b(x24), .c(new_n171_), .O(new_n177_));
  nor2   g026(.a(new_n177_), .b(new_n176_), .O(z06));
  nor2   g027(.a(x63), .b(new_n155_), .O(new_n179_));
  oai21  g028(.a(x40), .b(x25), .c(new_n171_), .O(new_n180_));
  nor2   g029(.a(new_n180_), .b(new_n179_), .O(z07));
  nand2  g030(.a(x62), .b(x40), .O(new_n182_));
  nand2  g031(.a(new_n155_), .b(x26), .O(new_n183_));
  nand3  g032(.a(new_n183_), .b(new_n182_), .c(new_n171_), .O(z08));
  nor2   g033(.a(x61), .b(new_n155_), .O(new_n185_));
  oai21  g034(.a(x40), .b(x27), .c(new_n171_), .O(new_n186_));
  nor2   g035(.a(new_n186_), .b(new_n185_), .O(z09));
  nor2   g036(.a(x60), .b(new_n155_), .O(new_n188_));
  oai21  g037(.a(x40), .b(x28), .c(new_n171_), .O(new_n189_));
  nor2   g038(.a(new_n189_), .b(new_n188_), .O(z10));
  nand2  g039(.a(x59), .b(x40), .O(new_n191_));
  nand2  g040(.a(new_n155_), .b(x29), .O(new_n192_));
  nand3  g041(.a(new_n192_), .b(new_n191_), .c(new_n171_), .O(z11));
  nor2   g042(.a(x58), .b(new_n155_), .O(new_n194_));
  oai21  g043(.a(x40), .b(x30), .c(new_n171_), .O(new_n195_));
  nor2   g044(.a(new_n195_), .b(new_n194_), .O(z12));
  nand2  g045(.a(x57), .b(x40), .O(new_n197_));
  nand2  g046(.a(new_n155_), .b(x31), .O(new_n198_));
  nand3  g047(.a(new_n198_), .b(new_n197_), .c(new_n171_), .O(z13));
  nor2   g048(.a(x51), .b(new_n155_), .O(new_n200_));
  oai21  g049(.a(x40), .b(x32), .c(new_n171_), .O(new_n201_));
  nor2   g050(.a(new_n201_), .b(new_n200_), .O(z14));
  nand2  g051(.a(x50), .b(x40), .O(new_n203_));
  nand2  g052(.a(new_n155_), .b(x33), .O(new_n204_));
  nand3  g053(.a(new_n204_), .b(new_n203_), .c(new_n171_), .O(z15));
  nand2  g054(.a(x49), .b(x40), .O(new_n206_));
  nand2  g055(.a(new_n155_), .b(x34), .O(new_n207_));
  nand3  g056(.a(new_n207_), .b(new_n206_), .c(new_n171_), .O(z16));
  nor2   g057(.a(x48), .b(new_n155_), .O(new_n209_));
  oai21  g058(.a(x40), .b(x35), .c(new_n171_), .O(new_n210_));
  nor2   g059(.a(new_n210_), .b(new_n209_), .O(z17));
  nor2   g060(.a(x47), .b(new_n155_), .O(new_n212_));
  oai21  g061(.a(x40), .b(x36), .c(new_n171_), .O(new_n213_));
  nor2   g062(.a(new_n213_), .b(new_n212_), .O(z18));
  nor2   g063(.a(x46), .b(new_n155_), .O(new_n215_));
  oai21  g064(.a(x40), .b(x37), .c(new_n171_), .O(new_n216_));
  nor2   g065(.a(new_n216_), .b(new_n215_), .O(z19));
  nor2   g066(.a(x45), .b(new_n155_), .O(new_n218_));
  oai21  g067(.a(x40), .b(x38), .c(new_n171_), .O(new_n219_));
  nor2   g068(.a(new_n219_), .b(new_n218_), .O(z20));
  nor2   g069(.a(x44), .b(new_n155_), .O(new_n221_));
  oai21  g070(.a(x40), .b(x39), .c(new_n171_), .O(new_n222_));
  nor2   g071(.a(new_n222_), .b(new_n221_), .O(z21));
  inv1   g072(.a(x42), .O(new_n224_));
  nand3  g073(.a(x78), .b(x77), .c(x04), .O(new_n225_));
  inv1   g074(.a(new_n225_), .O(new_n226_));
  nand3  g075(.a(x84), .b(x82), .c(x80), .O(new_n227_));
  inv1   g076(.a(new_n227_), .O(new_n228_));
  inv1   g077(.a(x81), .O(new_n229_));
  nor2   g078(.a(x83), .b(new_n229_), .O(new_n230_));
  inv1   g079(.a(x43), .O(new_n231_));
  nor2   g080(.a(x74), .b(new_n231_), .O(new_n232_));
  nand3  g081(.a(new_n232_), .b(new_n230_), .c(new_n228_), .O(new_n233_));
  nand3  g082(.a(new_n233_), .b(new_n226_), .c(new_n224_), .O(new_n234_));
  inv1   g083(.a(x41), .O(new_n235_));
  nand2  g084(.a(new_n169_), .b(new_n167_), .O(new_n236_));
  xor2a  g085(.a(x84), .b(x81), .O(new_n237_));
  inv1   g086(.a(new_n237_), .O(new_n238_));
  nand3  g087(.a(new_n238_), .b(new_n236_), .c(new_n235_), .O(new_n239_));
  nand3  g088(.a(new_n239_), .b(new_n234_), .c(x79), .O(new_n240_));
  and2   g089(.a(new_n240_), .b(z04), .O(z22));
  oai21  g090(.a(new_n153_), .b(x00), .c(z04), .O(z23));
  nor2   g091(.a(new_n161_), .b(x04), .O(new_n243_));
  nand3  g092(.a(new_n243_), .b(new_n231_), .c(x05), .O(new_n244_));
  aoi21  g093(.a(new_n244_), .b(x79), .c(x01), .O(z24));
  xor2a  g094(.a(x84), .b(x82), .O(new_n246_));
  xor2a  g095(.a(new_n246_), .b(x81), .O(new_n247_));
  nor2   g096(.a(x42), .b(x04), .O(new_n248_));
  nand2  g097(.a(new_n248_), .b(x05), .O(new_n249_));
  inv1   g098(.a(new_n249_), .O(new_n250_));
  nand4  g099(.a(new_n250_), .b(new_n247_), .c(new_n162_), .d(new_n159_), .O(new_n251_));
  inv1   g100(.a(new_n251_), .O(z25));
  nand4  g101(.a(new_n248_), .b(new_n247_), .c(new_n162_), .d(x44), .O(new_n253_));
  aoi21  g102(.a(new_n253_), .b(x79), .c(x01), .O(z26));
  nand2  g103(.a(new_n248_), .b(x45), .O(new_n255_));
  inv1   g104(.a(new_n255_), .O(new_n256_));
  nand4  g105(.a(new_n256_), .b(new_n247_), .c(new_n162_), .d(new_n159_), .O(new_n257_));
  inv1   g106(.a(new_n257_), .O(z27));
  nand4  g107(.a(new_n248_), .b(new_n247_), .c(new_n162_), .d(x46), .O(new_n259_));
  aoi21  g108(.a(new_n259_), .b(x79), .c(x01), .O(z28));
  nand4  g109(.a(new_n248_), .b(new_n247_), .c(new_n162_), .d(x47), .O(new_n261_));
  aoi21  g110(.a(new_n261_), .b(x79), .c(x01), .O(z29));
  nand4  g111(.a(new_n248_), .b(new_n247_), .c(new_n162_), .d(x48), .O(new_n263_));
  aoi21  g112(.a(new_n263_), .b(x79), .c(x01), .O(z30));
  nand4  g113(.a(new_n248_), .b(new_n247_), .c(new_n162_), .d(x49), .O(new_n265_));
  aoi21  g114(.a(new_n265_), .b(x79), .c(x01), .O(z31));
  nand2  g115(.a(new_n248_), .b(x50), .O(new_n267_));
  inv1   g116(.a(new_n267_), .O(new_n268_));
  nand4  g117(.a(new_n268_), .b(new_n247_), .c(new_n162_), .d(new_n159_), .O(new_n269_));
  inv1   g118(.a(new_n269_), .O(z32));
  xor2a  g119(.a(x83), .b(x81), .O(new_n271_));
  nand2  g120(.a(x42), .b(x05), .O(new_n272_));
  aoi21  g121(.a(new_n271_), .b(new_n246_), .c(new_n272_), .O(new_n273_));
  oai21  g122(.a(new_n271_), .b(new_n246_), .c(new_n273_), .O(new_n274_));
  nand3  g123(.a(new_n247_), .b(x51), .c(new_n224_), .O(new_n275_));
  nand2  g124(.a(new_n243_), .b(new_n159_), .O(new_n276_));
  aoi21  g125(.a(new_n275_), .b(new_n274_), .c(new_n276_), .O(z33));
  inv1   g126(.a(new_n246_), .O(new_n278_));
  nand2  g127(.a(x83), .b(x42), .O(new_n279_));
  xor2a  g128(.a(new_n279_), .b(x81), .O(new_n280_));
  nand2  g129(.a(new_n280_), .b(new_n278_), .O(new_n281_));
  xor2a  g130(.a(new_n279_), .b(new_n229_), .O(new_n282_));
  nand2  g131(.a(new_n282_), .b(new_n246_), .O(new_n283_));
  nand2  g132(.a(new_n162_), .b(x79), .O(new_n284_));
  inv1   g133(.a(new_n284_), .O(new_n285_));
  nor2   g134(.a(x04), .b(x01), .O(new_n286_));
  nand2  g135(.a(new_n286_), .b(x52), .O(new_n287_));
  inv1   g136(.a(new_n287_), .O(new_n288_));
  nand4  g137(.a(new_n288_), .b(new_n285_), .c(new_n283_), .d(new_n281_), .O(new_n289_));
  inv1   g138(.a(new_n289_), .O(z34));
  nand4  g139(.a(new_n283_), .b(new_n281_), .c(new_n243_), .d(x53), .O(new_n291_));
  aoi21  g140(.a(new_n291_), .b(x79), .c(x01), .O(z35));
  nand2  g141(.a(new_n286_), .b(x54), .O(new_n293_));
  inv1   g142(.a(new_n293_), .O(new_n294_));
  nand4  g143(.a(new_n294_), .b(new_n285_), .c(new_n283_), .d(new_n281_), .O(new_n295_));
  inv1   g144(.a(new_n295_), .O(z36));
  nand4  g145(.a(new_n283_), .b(new_n281_), .c(new_n243_), .d(x55), .O(new_n297_));
  aoi21  g146(.a(new_n297_), .b(x79), .c(x01), .O(z37));
  nand4  g147(.a(new_n283_), .b(new_n281_), .c(new_n243_), .d(x56), .O(new_n299_));
  aoi21  g148(.a(new_n299_), .b(x79), .c(x01), .O(z38));
  nand2  g149(.a(new_n286_), .b(x57), .O(new_n301_));
  inv1   g150(.a(new_n301_), .O(new_n302_));
  nand4  g151(.a(new_n302_), .b(new_n285_), .c(new_n283_), .d(new_n281_), .O(new_n303_));
  inv1   g152(.a(new_n303_), .O(z39));
  nand4  g153(.a(new_n283_), .b(new_n281_), .c(new_n243_), .d(x58), .O(new_n305_));
  aoi21  g154(.a(new_n305_), .b(x79), .c(x01), .O(z40));
  nand4  g155(.a(new_n283_), .b(new_n281_), .c(new_n243_), .d(x59), .O(new_n307_));
  aoi21  g156(.a(new_n307_), .b(x79), .c(x01), .O(z41));
  nand2  g157(.a(new_n286_), .b(x60), .O(new_n309_));
  inv1   g158(.a(new_n309_), .O(new_n310_));
  nand4  g159(.a(new_n310_), .b(new_n285_), .c(new_n283_), .d(new_n281_), .O(new_n311_));
  inv1   g160(.a(new_n311_), .O(z42));
  nand4  g161(.a(new_n283_), .b(new_n281_), .c(new_n243_), .d(x61), .O(new_n313_));
  aoi21  g162(.a(new_n313_), .b(x79), .c(x01), .O(z43));
  nand4  g163(.a(new_n283_), .b(new_n281_), .c(new_n243_), .d(x62), .O(new_n315_));
  aoi21  g164(.a(new_n315_), .b(x79), .c(x01), .O(z44));
  nand2  g165(.a(new_n286_), .b(x63), .O(new_n317_));
  inv1   g166(.a(new_n317_), .O(new_n318_));
  nand4  g167(.a(new_n318_), .b(new_n285_), .c(new_n283_), .d(new_n281_), .O(new_n319_));
  inv1   g168(.a(new_n319_), .O(z45));
  nand2  g169(.a(new_n286_), .b(x64), .O(new_n321_));
  inv1   g170(.a(new_n321_), .O(new_n322_));
  nand4  g171(.a(new_n322_), .b(new_n285_), .c(new_n283_), .d(new_n281_), .O(new_n323_));
  inv1   g172(.a(new_n323_), .O(z46));
  nor3   g173(.a(new_n237_), .b(x78), .c(new_n168_), .O(new_n325_));
  oai21  g174(.a(x75), .b(x67), .c(new_n325_), .O(new_n326_));
  aoi21  g175(.a(new_n326_), .b(x79), .c(x01), .O(z47));
  nand2  g176(.a(new_n325_), .b(x68), .O(new_n328_));
  aoi21  g177(.a(new_n328_), .b(x79), .c(x01), .O(z48));
  inv1   g178(.a(new_n325_), .O(new_n330_));
  nand2  g179(.a(new_n159_), .b(x69), .O(new_n331_));
  nor2   g180(.a(new_n331_), .b(new_n330_), .O(z49));
  nand2  g181(.a(new_n159_), .b(x70), .O(new_n333_));
  nor2   g182(.a(new_n333_), .b(new_n330_), .O(z50));
  nand2  g183(.a(new_n325_), .b(x71), .O(new_n335_));
  aoi21  g184(.a(new_n335_), .b(x79), .c(x01), .O(z51));
  nand2  g185(.a(new_n325_), .b(x72), .O(new_n337_));
  aoi21  g186(.a(new_n337_), .b(x79), .c(x01), .O(z52));
  nand2  g187(.a(new_n159_), .b(x73), .O(new_n339_));
  nor2   g188(.a(new_n339_), .b(new_n330_), .O(z53));
  inv1   g189(.a(x80), .O(new_n341_));
  inv1   g190(.a(x84), .O(new_n342_));
  nor2   g191(.a(new_n342_), .b(x82), .O(new_n343_));
  inv1   g192(.a(x83), .O(new_n344_));
  nor2   g193(.a(new_n344_), .b(x81), .O(new_n345_));
  nand4  g194(.a(new_n345_), .b(new_n343_), .c(new_n243_), .d(new_n341_), .O(new_n346_));
  aoi21  g195(.a(new_n346_), .b(x79), .c(x01), .O(z55));
  nand2  g196(.a(new_n166_), .b(new_n168_), .O(new_n348_));
  oai21  g197(.a(new_n237_), .b(x76), .c(new_n161_), .O(new_n349_));
  nand4  g198(.a(new_n349_), .b(new_n348_), .c(new_n159_), .d(x00), .O(z56));
  inv1   g199(.a(x02), .O(new_n351_));
  nand3  g200(.a(x03), .b(new_n351_), .c(x00), .O(new_n352_));
  aoi21  g201(.a(new_n352_), .b(x79), .c(x01), .O(z57));
  nand2  g202(.a(new_n233_), .b(new_n224_), .O(new_n354_));
  aoi21  g203(.a(x42), .b(x40), .c(new_n225_), .O(new_n355_));
  nand2  g204(.a(new_n355_), .b(new_n354_), .O(new_n356_));
  aoi21  g205(.a(new_n356_), .b(x79), .c(x01), .O(z58));
  nand2  g206(.a(new_n226_), .b(new_n159_), .O(new_n358_));
  aoi21  g207(.a(new_n354_), .b(new_n155_), .c(new_n358_), .O(z59));
  nand2  g208(.a(new_n237_), .b(new_n164_), .O(new_n360_));
  nand3  g209(.a(new_n360_), .b(new_n234_), .c(x79), .O(new_n361_));
  and2   g210(.a(new_n361_), .b(z04), .O(z60));
  oai22  g211(.a(new_n163_), .b(new_n162_), .c(new_n166_), .d(x04), .O(new_n363_));
  nand4  g212(.a(new_n363_), .b(new_n360_), .c(new_n159_), .d(x80), .O(new_n364_));
  inv1   g213(.a(new_n364_), .O(z61));
  nand2  g214(.a(new_n164_), .b(new_n342_), .O(new_n366_));
  nand3  g215(.a(new_n366_), .b(new_n363_), .c(x81), .O(new_n367_));
  aoi21  g216(.a(new_n367_), .b(new_n234_), .c(new_n160_), .O(z62));
  nand3  g217(.a(new_n363_), .b(new_n360_), .c(x82), .O(new_n369_));
  aoi21  g218(.a(new_n369_), .b(x79), .c(x01), .O(z63));
  nand4  g219(.a(new_n363_), .b(new_n360_), .c(new_n159_), .d(x83), .O(new_n371_));
  inv1   g220(.a(new_n371_), .O(z64));
  nand2  g221(.a(new_n164_), .b(new_n229_), .O(new_n373_));
  nand4  g222(.a(new_n373_), .b(new_n363_), .c(new_n159_), .d(x84), .O(new_n374_));
  inv1   g223(.a(new_n374_), .O(z65));
  inv1   g224(.a(new_n171_), .O(z54));
endmodule


