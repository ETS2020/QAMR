// Benchmark "FAU" written by ABC on Tue Aug 11 22:53:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n161_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n180_, new_n181_, new_n183_, new_n184_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n198_, new_n199_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n211_, new_n213_, new_n214_,
    new_n216_, new_n217_, new_n219_, new_n220_, new_n222_, new_n223_,
    new_n225_, new_n226_, new_n227_, new_n228_, new_n229_, new_n231_,
    new_n232_, new_n233_, new_n234_, new_n235_, new_n237_, new_n238_,
    new_n239_, new_n240_, new_n241_, new_n243_, new_n244_, new_n246_,
    new_n247_, new_n248_, new_n250_, new_n251_, new_n263_, new_n264_,
    new_n265_, new_n266_, new_n267_, new_n268_, new_n269_, new_n271_,
    new_n272_, new_n273_, new_n274_, new_n275_, new_n276_, new_n277_,
    new_n279_, new_n280_, new_n281_, new_n282_, new_n283_, new_n285_,
    new_n286_, new_n287_, new_n288_, new_n289_, new_n291_, new_n292_,
    new_n293_, new_n294_, new_n295_, new_n297_, new_n298_, new_n299_,
    new_n300_, new_n301_, new_n303_, new_n304_, new_n305_, new_n306_,
    new_n307_, new_n309_, new_n310_, new_n312_, new_n313_, new_n315_,
    new_n316_, new_n318_, new_n319_, new_n320_, new_n321_, new_n323_,
    new_n324_, new_n326_;
  inv1   g000(.a(x40), .O(new_n152_));
  nand2  g001(.a(x78), .b(x77), .O(new_n153_));
  nor2   g002(.a(x79), .b(x01), .O(new_n154_));
  inv1   g003(.a(x01), .O(z01));
  nand2  g004(.a(x79), .b(z01), .O(new_n156_));
  aoi22  g005(.a(new_n156_), .b(x52), .c(new_n154_), .d(new_n153_), .O(new_n157_));
  nand3  g006(.a(new_n156_), .b(new_n152_), .c(x06), .O(new_n158_));
  oai21  g007(.a(new_n157_), .b(new_n152_), .c(new_n158_), .O(z00));
  inv1   g008(.a(new_n156_), .O(z02));
  nand3  g009(.a(new_n154_), .b(x78), .c(x52), .O(new_n161_));
  inv1   g010(.a(new_n161_), .O(z03));
  nor2   g011(.a(new_n153_), .b(x79), .O(new_n163_));
  nor2   g012(.a(new_n163_), .b(x01), .O(z04));
  inv1   g013(.a(x65), .O(new_n165_));
  nand2  g014(.a(new_n165_), .b(x40), .O(new_n166_));
  inv1   g015(.a(x23), .O(new_n167_));
  nand2  g016(.a(new_n152_), .b(new_n167_), .O(new_n168_));
  nand3  g017(.a(new_n168_), .b(new_n166_), .c(new_n156_), .O(new_n169_));
  inv1   g018(.a(new_n169_), .O(z05));
  nand2  g019(.a(x64), .b(x40), .O(new_n171_));
  nand2  g020(.a(new_n152_), .b(x24), .O(new_n172_));
  nand3  g021(.a(new_n172_), .b(new_n171_), .c(new_n156_), .O(z06));
  inv1   g022(.a(x63), .O(new_n174_));
  nand2  g023(.a(new_n174_), .b(x40), .O(new_n175_));
  inv1   g024(.a(x25), .O(new_n176_));
  nand2  g025(.a(new_n152_), .b(new_n176_), .O(new_n177_));
  nand3  g026(.a(new_n177_), .b(new_n175_), .c(new_n156_), .O(new_n178_));
  inv1   g027(.a(new_n178_), .O(z07));
  nand2  g028(.a(x62), .b(x40), .O(new_n180_));
  nand2  g029(.a(new_n152_), .b(x26), .O(new_n181_));
  nand3  g030(.a(new_n181_), .b(new_n180_), .c(new_n156_), .O(z08));
  nand2  g031(.a(x61), .b(x40), .O(new_n183_));
  nand2  g032(.a(new_n152_), .b(x27), .O(new_n184_));
  nand3  g033(.a(new_n184_), .b(new_n183_), .c(new_n156_), .O(z09));
  inv1   g034(.a(x60), .O(new_n186_));
  nand2  g035(.a(new_n186_), .b(x40), .O(new_n187_));
  inv1   g036(.a(x28), .O(new_n188_));
  nand2  g037(.a(new_n152_), .b(new_n188_), .O(new_n189_));
  nand3  g038(.a(new_n189_), .b(new_n187_), .c(new_n156_), .O(new_n190_));
  inv1   g039(.a(new_n190_), .O(z10));
  inv1   g040(.a(x59), .O(new_n192_));
  nand2  g041(.a(new_n192_), .b(x40), .O(new_n193_));
  inv1   g042(.a(x29), .O(new_n194_));
  nand2  g043(.a(new_n152_), .b(new_n194_), .O(new_n195_));
  nand3  g044(.a(new_n195_), .b(new_n193_), .c(new_n156_), .O(new_n196_));
  inv1   g045(.a(new_n196_), .O(z11));
  nand2  g046(.a(x58), .b(x40), .O(new_n198_));
  nand2  g047(.a(new_n152_), .b(x30), .O(new_n199_));
  nand3  g048(.a(new_n199_), .b(new_n198_), .c(new_n156_), .O(z12));
  inv1   g049(.a(x57), .O(new_n201_));
  nand2  g050(.a(new_n201_), .b(x40), .O(new_n202_));
  inv1   g051(.a(x31), .O(new_n203_));
  nand2  g052(.a(new_n152_), .b(new_n203_), .O(new_n204_));
  nand3  g053(.a(new_n204_), .b(new_n202_), .c(new_n156_), .O(new_n205_));
  inv1   g054(.a(new_n205_), .O(z13));
  inv1   g055(.a(x51), .O(new_n207_));
  nand2  g056(.a(new_n207_), .b(x40), .O(new_n208_));
  inv1   g057(.a(x32), .O(new_n209_));
  nand2  g058(.a(new_n152_), .b(new_n209_), .O(new_n210_));
  nand3  g059(.a(new_n210_), .b(new_n208_), .c(new_n156_), .O(new_n211_));
  inv1   g060(.a(new_n211_), .O(z14));
  nand2  g061(.a(x50), .b(x40), .O(new_n213_));
  nand2  g062(.a(new_n152_), .b(x33), .O(new_n214_));
  nand3  g063(.a(new_n214_), .b(new_n213_), .c(new_n156_), .O(z15));
  nand2  g064(.a(x49), .b(x40), .O(new_n216_));
  nand2  g065(.a(new_n152_), .b(x34), .O(new_n217_));
  nand3  g066(.a(new_n217_), .b(new_n216_), .c(new_n156_), .O(z16));
  nand2  g067(.a(x48), .b(x40), .O(new_n219_));
  nand2  g068(.a(new_n152_), .b(x35), .O(new_n220_));
  nand3  g069(.a(new_n220_), .b(new_n219_), .c(new_n156_), .O(z17));
  nand2  g070(.a(x47), .b(x40), .O(new_n222_));
  nand2  g071(.a(new_n152_), .b(x36), .O(new_n223_));
  nand3  g072(.a(new_n223_), .b(new_n222_), .c(new_n156_), .O(z18));
  inv1   g073(.a(x46), .O(new_n225_));
  nand2  g074(.a(new_n225_), .b(x40), .O(new_n226_));
  inv1   g075(.a(x37), .O(new_n227_));
  nand2  g076(.a(new_n152_), .b(new_n227_), .O(new_n228_));
  nand3  g077(.a(new_n228_), .b(new_n226_), .c(new_n156_), .O(new_n229_));
  inv1   g078(.a(new_n229_), .O(z19));
  inv1   g079(.a(x45), .O(new_n231_));
  nand2  g080(.a(new_n231_), .b(x40), .O(new_n232_));
  inv1   g081(.a(x38), .O(new_n233_));
  nand2  g082(.a(new_n152_), .b(new_n233_), .O(new_n234_));
  nand3  g083(.a(new_n234_), .b(new_n232_), .c(new_n156_), .O(new_n235_));
  inv1   g084(.a(new_n235_), .O(z20));
  inv1   g085(.a(x44), .O(new_n237_));
  nand2  g086(.a(new_n237_), .b(x40), .O(new_n238_));
  inv1   g087(.a(x39), .O(new_n239_));
  nand2  g088(.a(new_n152_), .b(new_n239_), .O(new_n240_));
  nand3  g089(.a(new_n240_), .b(new_n238_), .c(new_n156_), .O(new_n241_));
  inv1   g090(.a(new_n241_), .O(z21));
  inv1   g091(.a(x79), .O(new_n243_));
  nand2  g092(.a(x78), .b(x04), .O(new_n244_));
  aoi21  g093(.a(new_n244_), .b(new_n243_), .c(x01), .O(z22));
  inv1   g094(.a(x00), .O(new_n246_));
  inv1   g095(.a(x04), .O(new_n247_));
  aoi21  g096(.a(x05), .b(new_n247_), .c(new_n246_), .O(new_n248_));
  oai21  g097(.a(new_n248_), .b(x79), .c(z01), .O(z23));
  inv1   g098(.a(x43), .O(new_n250_));
  nand3  g099(.a(new_n250_), .b(x05), .c(new_n247_), .O(new_n251_));
  aoi21  g100(.a(new_n251_), .b(new_n243_), .c(x01), .O(z24));
  inv1   g101(.a(x77), .O(new_n263_));
  nand3  g102(.a(x78), .b(new_n263_), .c(x04), .O(new_n264_));
  inv1   g103(.a(new_n264_), .O(new_n265_));
  inv1   g104(.a(x07), .O(new_n266_));
  nand2  g105(.a(x52), .b(x15), .O(new_n267_));
  oai21  g106(.a(x52), .b(new_n266_), .c(new_n267_), .O(new_n268_));
  nand2  g107(.a(new_n268_), .b(new_n265_), .O(new_n269_));
  aoi21  g108(.a(new_n269_), .b(new_n243_), .c(x01), .O(z47));
  nor2   g109(.a(new_n244_), .b(x77), .O(new_n271_));
  inv1   g110(.a(x16), .O(new_n272_));
  nand2  g111(.a(x52), .b(new_n272_), .O(new_n273_));
  inv1   g112(.a(x08), .O(new_n274_));
  inv1   g113(.a(x52), .O(new_n275_));
  nand2  g114(.a(new_n275_), .b(new_n274_), .O(new_n276_));
  nand3  g115(.a(new_n276_), .b(new_n273_), .c(new_n271_), .O(new_n277_));
  aoi21  g116(.a(new_n277_), .b(new_n243_), .c(x01), .O(z48));
  inv1   g117(.a(x17), .O(new_n279_));
  nand2  g118(.a(x52), .b(new_n279_), .O(new_n280_));
  inv1   g119(.a(x09), .O(new_n281_));
  nand2  g120(.a(new_n275_), .b(new_n281_), .O(new_n282_));
  nand3  g121(.a(new_n282_), .b(new_n280_), .c(new_n271_), .O(new_n283_));
  aoi21  g122(.a(new_n283_), .b(new_n243_), .c(x01), .O(z49));
  inv1   g123(.a(x18), .O(new_n285_));
  nand2  g124(.a(x52), .b(new_n285_), .O(new_n286_));
  inv1   g125(.a(x10), .O(new_n287_));
  nand2  g126(.a(new_n275_), .b(new_n287_), .O(new_n288_));
  nand3  g127(.a(new_n288_), .b(new_n286_), .c(new_n271_), .O(new_n289_));
  aoi21  g128(.a(new_n289_), .b(new_n243_), .c(x01), .O(z50));
  inv1   g129(.a(x19), .O(new_n291_));
  nand2  g130(.a(x52), .b(new_n291_), .O(new_n292_));
  inv1   g131(.a(x11), .O(new_n293_));
  nand2  g132(.a(new_n275_), .b(new_n293_), .O(new_n294_));
  nand3  g133(.a(new_n294_), .b(new_n292_), .c(new_n271_), .O(new_n295_));
  aoi21  g134(.a(new_n295_), .b(new_n243_), .c(x01), .O(z51));
  inv1   g135(.a(x20), .O(new_n297_));
  nand2  g136(.a(x52), .b(new_n297_), .O(new_n298_));
  inv1   g137(.a(x12), .O(new_n299_));
  nand2  g138(.a(new_n275_), .b(new_n299_), .O(new_n300_));
  nand3  g139(.a(new_n300_), .b(new_n298_), .c(new_n271_), .O(new_n301_));
  aoi21  g140(.a(new_n301_), .b(new_n243_), .c(x01), .O(z52));
  inv1   g141(.a(x21), .O(new_n303_));
  nand2  g142(.a(x52), .b(new_n303_), .O(new_n304_));
  inv1   g143(.a(x13), .O(new_n305_));
  nand2  g144(.a(new_n275_), .b(new_n305_), .O(new_n306_));
  nand3  g145(.a(new_n306_), .b(new_n304_), .c(new_n271_), .O(new_n307_));
  aoi21  g146(.a(new_n307_), .b(new_n243_), .c(x01), .O(z53));
  nor2   g147(.a(x52), .b(x14), .O(new_n309_));
  oai21  g148(.a(new_n275_), .b(x22), .c(new_n154_), .O(new_n310_));
  nor3   g149(.a(new_n310_), .b(new_n309_), .c(new_n264_), .O(z54));
  inv1   g150(.a(x78), .O(new_n312_));
  nand2  g151(.a(new_n312_), .b(new_n263_), .O(new_n313_));
  nand3  g152(.a(new_n313_), .b(new_n154_), .c(x00), .O(z56));
  inv1   g153(.a(x02), .O(new_n315_));
  nand3  g154(.a(x03), .b(new_n315_), .c(x00), .O(new_n316_));
  aoi21  g155(.a(new_n316_), .b(new_n243_), .c(x01), .O(z57));
  inv1   g156(.a(x42), .O(new_n318_));
  nand4  g157(.a(new_n312_), .b(x77), .c(new_n318_), .d(x40), .O(new_n319_));
  nand2  g158(.a(new_n243_), .b(x04), .O(new_n320_));
  aoi21  g159(.a(x78), .b(new_n263_), .c(new_n320_), .O(new_n321_));
  aoi21  g160(.a(new_n321_), .b(new_n319_), .c(x01), .O(z58));
  inv1   g161(.a(new_n320_), .O(new_n323_));
  oai21  g162(.a(x78), .b(x40), .c(x77), .O(new_n324_));
  aoi21  g163(.a(new_n324_), .b(new_n323_), .c(x01), .O(z59));
  inv1   g164(.a(new_n154_), .O(new_n326_));
  aoi21  g165(.a(new_n312_), .b(x04), .c(new_n326_), .O(z60));
  aoi21  g166(.a(new_n264_), .b(new_n243_), .c(x01), .O(z64));
  zero   g167(.O(z26));
  zero   g168(.O(z27));
  zero   g169(.O(z29));
  zero   g170(.O(z30));
  zero   g171(.O(z33));
  zero   g172(.O(z37));
  zero   g173(.O(z38));
  zero   g174(.O(z42));
  zero   g175(.O(z43));
  zero   g176(.O(z45));
  zero   g177(.O(z61));
  zero   g178(.O(z63));
  inv1   g179(.a(new_n156_), .O(z25));
  inv1   g180(.a(new_n156_), .O(z28));
  inv1   g181(.a(new_n156_), .O(z31));
  inv1   g182(.a(new_n156_), .O(z32));
  inv1   g183(.a(new_n156_), .O(z34));
  inv1   g184(.a(new_n156_), .O(z35));
  inv1   g185(.a(new_n156_), .O(z36));
  inv1   g186(.a(new_n156_), .O(z39));
  inv1   g187(.a(new_n156_), .O(z40));
  inv1   g188(.a(new_n156_), .O(z41));
  inv1   g189(.a(new_n156_), .O(z44));
  inv1   g190(.a(new_n156_), .O(z46));
  inv1   g191(.a(new_n156_), .O(z55));
  aoi21  g192(.a(new_n244_), .b(new_n243_), .c(x01), .O(z62));
  inv1   g193(.a(new_n156_), .O(z65));
endmodule


