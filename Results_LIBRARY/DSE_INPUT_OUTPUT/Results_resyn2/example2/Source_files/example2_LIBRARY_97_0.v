// Benchmark "FAU" written by ABC on Tue Aug 11 22:52:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n173_, new_n174_,
    new_n176_, new_n177_, new_n179_, new_n180_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n200_, new_n201_,
    new_n203_, new_n204_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n212_, new_n213_, new_n215_, new_n216_, new_n218_, new_n219_,
    new_n221_, new_n222_, new_n224_, new_n225_, new_n226_, new_n227_,
    new_n228_, new_n229_, new_n230_, new_n231_, new_n232_, new_n233_,
    new_n234_, new_n235_, new_n236_, new_n237_, new_n240_, new_n241_,
    new_n242_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n251_, new_n253_, new_n255_, new_n257_, new_n259_,
    new_n261_, new_n263_, new_n265_, new_n266_, new_n267_, new_n268_,
    new_n269_, new_n270_, new_n271_, new_n272_, new_n274_, new_n275_,
    new_n276_, new_n277_, new_n278_, new_n279_, new_n280_, new_n281_,
    new_n282_, new_n284_, new_n286_, new_n287_, new_n288_, new_n290_,
    new_n291_, new_n292_, new_n294_, new_n295_, new_n296_, new_n298_,
    new_n299_, new_n300_, new_n302_, new_n304_, new_n306_, new_n307_,
    new_n308_, new_n310_, new_n311_, new_n312_, new_n314_, new_n315_,
    new_n316_, new_n318_, new_n319_, new_n320_, new_n322_, new_n324_,
    new_n325_, new_n326_, new_n328_, new_n330_, new_n332_, new_n334_,
    new_n335_, new_n337_, new_n339_, new_n342_, new_n343_, new_n344_,
    new_n346_, new_n348_, new_n349_, new_n351_, new_n352_, new_n353_,
    new_n356_, new_n357_, new_n358_, new_n359_, new_n360_, new_n361_,
    new_n362_, new_n364_, new_n365_, new_n367_, new_n368_, new_n369_,
    new_n370_, new_n371_, new_n372_, new_n373_, new_n375_, new_n378_,
    new_n379_;
  inv1   g000(.a(x01), .O(new_n152_));
  inv1   g001(.a(x40), .O(new_n153_));
  nor3   g002(.a(x52), .b(new_n153_), .c(new_n152_), .O(new_n154_));
  nor2   g003(.a(x79), .b(x01), .O(z03));
  inv1   g004(.a(z03), .O(new_n156_));
  oai21  g005(.a(x40), .b(x06), .c(new_n156_), .O(new_n157_));
  nor2   g006(.a(new_n157_), .b(new_n154_), .O(z00));
  nand2  g007(.a(x78), .b(x77), .O(new_n159_));
  inv1   g008(.a(x79), .O(new_n160_));
  nor2   g009(.a(x78), .b(x77), .O(new_n161_));
  nor2   g010(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  aoi21  g011(.a(new_n162_), .b(new_n159_), .c(x01), .O(z01));
  inv1   g012(.a(x77), .O(new_n164_));
  nor2   g013(.a(x78), .b(new_n164_), .O(new_n165_));
  nand2  g014(.a(new_n165_), .b(x66), .O(new_n166_));
  nand3  g015(.a(x78), .b(new_n164_), .c(x75), .O(new_n167_));
  nand2  g016(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  inv1   g017(.a(new_n168_), .O(new_n169_));
  aoi21  g018(.a(new_n169_), .b(x79), .c(x01), .O(z02));
  nand2  g019(.a(x79), .b(new_n152_), .O(new_n171_));
  inv1   g020(.a(new_n171_), .O(z04));
  nand2  g021(.a(x65), .b(x40), .O(new_n173_));
  nand2  g022(.a(new_n153_), .b(x23), .O(new_n174_));
  nand3  g023(.a(new_n174_), .b(new_n173_), .c(new_n156_), .O(z05));
  nand2  g024(.a(x64), .b(x40), .O(new_n176_));
  nand2  g025(.a(new_n153_), .b(x24), .O(new_n177_));
  nand3  g026(.a(new_n177_), .b(new_n176_), .c(new_n156_), .O(z06));
  nor2   g027(.a(x63), .b(new_n153_), .O(new_n179_));
  nor2   g028(.a(x40), .b(x25), .O(new_n180_));
  nor3   g029(.a(new_n180_), .b(new_n179_), .c(z03), .O(z07));
  nand2  g030(.a(x62), .b(x40), .O(new_n182_));
  nand2  g031(.a(new_n153_), .b(x26), .O(new_n183_));
  nand3  g032(.a(new_n183_), .b(new_n182_), .c(new_n156_), .O(z08));
  nor2   g033(.a(x61), .b(new_n153_), .O(new_n185_));
  nor2   g034(.a(x40), .b(x27), .O(new_n186_));
  nor3   g035(.a(new_n186_), .b(new_n185_), .c(z03), .O(z09));
  nand2  g036(.a(x60), .b(x40), .O(new_n188_));
  nand2  g037(.a(new_n153_), .b(x28), .O(new_n189_));
  nand3  g038(.a(new_n189_), .b(new_n188_), .c(new_n156_), .O(z10));
  nand2  g039(.a(x59), .b(x40), .O(new_n191_));
  nand2  g040(.a(new_n153_), .b(x29), .O(new_n192_));
  nand3  g041(.a(new_n192_), .b(new_n191_), .c(new_n156_), .O(z11));
  nand2  g042(.a(x58), .b(x40), .O(new_n194_));
  nand2  g043(.a(new_n153_), .b(x30), .O(new_n195_));
  nand3  g044(.a(new_n195_), .b(new_n194_), .c(new_n156_), .O(z12));
  nor2   g045(.a(x57), .b(new_n153_), .O(new_n197_));
  nor2   g046(.a(x40), .b(x31), .O(new_n198_));
  nor3   g047(.a(new_n198_), .b(new_n197_), .c(z03), .O(z13));
  nor2   g048(.a(x51), .b(new_n153_), .O(new_n200_));
  nor2   g049(.a(x40), .b(x32), .O(new_n201_));
  nor3   g050(.a(new_n201_), .b(new_n200_), .c(z03), .O(z14));
  nor2   g051(.a(x50), .b(new_n153_), .O(new_n203_));
  nor2   g052(.a(x40), .b(x33), .O(new_n204_));
  nor3   g053(.a(new_n204_), .b(new_n203_), .c(z03), .O(z15));
  nand2  g054(.a(x49), .b(x40), .O(new_n206_));
  nand2  g055(.a(new_n153_), .b(x34), .O(new_n207_));
  nand3  g056(.a(new_n207_), .b(new_n206_), .c(new_n156_), .O(z16));
  nor2   g057(.a(x48), .b(new_n153_), .O(new_n209_));
  nor2   g058(.a(x40), .b(x35), .O(new_n210_));
  nor3   g059(.a(new_n210_), .b(new_n209_), .c(z03), .O(z17));
  nor2   g060(.a(x47), .b(new_n153_), .O(new_n212_));
  nor2   g061(.a(x40), .b(x36), .O(new_n213_));
  nor3   g062(.a(new_n213_), .b(new_n212_), .c(z03), .O(z18));
  nor2   g063(.a(x46), .b(new_n153_), .O(new_n215_));
  nor2   g064(.a(x40), .b(x37), .O(new_n216_));
  nor3   g065(.a(new_n216_), .b(new_n215_), .c(z03), .O(z19));
  nand2  g066(.a(x45), .b(x40), .O(new_n218_));
  nand2  g067(.a(new_n153_), .b(x38), .O(new_n219_));
  nand3  g068(.a(new_n219_), .b(new_n218_), .c(new_n156_), .O(z20));
  nor2   g069(.a(x44), .b(new_n153_), .O(new_n221_));
  nor2   g070(.a(x40), .b(x39), .O(new_n222_));
  nor3   g071(.a(new_n222_), .b(new_n221_), .c(z03), .O(z21));
  inv1   g072(.a(x42), .O(new_n224_));
  inv1   g073(.a(new_n159_), .O(new_n225_));
  nand2  g074(.a(new_n225_), .b(x04), .O(new_n226_));
  inv1   g075(.a(new_n226_), .O(new_n227_));
  inv1   g076(.a(x74), .O(new_n228_));
  nand2  g077(.a(new_n228_), .b(x43), .O(new_n229_));
  inv1   g078(.a(x83), .O(new_n230_));
  nand4  g079(.a(x84), .b(new_n230_), .c(x82), .d(x80), .O(new_n231_));
  nor2   g080(.a(new_n231_), .b(new_n229_), .O(new_n232_));
  nand2  g081(.a(new_n232_), .b(x81), .O(new_n233_));
  nand3  g082(.a(new_n233_), .b(new_n227_), .c(new_n224_), .O(new_n234_));
  xor2a  g083(.a(x84), .b(x81), .O(new_n235_));
  nor2   g084(.a(new_n235_), .b(x41), .O(new_n236_));
  aoi21  g085(.a(new_n236_), .b(new_n168_), .c(new_n160_), .O(new_n237_));
  aoi21  g086(.a(new_n237_), .b(new_n234_), .c(x01), .O(z22));
  nand3  g087(.a(x79), .b(new_n152_), .c(x00), .O(z23));
  inv1   g088(.a(x05), .O(new_n240_));
  nor2   g089(.a(new_n159_), .b(x04), .O(new_n241_));
  inv1   g090(.a(new_n241_), .O(new_n242_));
  nor4   g091(.a(new_n242_), .b(new_n171_), .c(x43), .d(new_n240_), .O(z24));
  inv1   g092(.a(x81), .O(new_n244_));
  xor2a  g093(.a(x84), .b(x82), .O(new_n245_));
  xor2a  g094(.a(new_n245_), .b(new_n244_), .O(new_n246_));
  nor2   g095(.a(new_n246_), .b(new_n159_), .O(new_n247_));
  nor2   g096(.a(x42), .b(x04), .O(new_n248_));
  nand4  g097(.a(new_n248_), .b(new_n247_), .c(z04), .d(x05), .O(new_n249_));
  inv1   g098(.a(new_n249_), .O(z25));
  nand3  g099(.a(new_n248_), .b(new_n247_), .c(x44), .O(new_n251_));
  aoi21  g100(.a(new_n251_), .b(x79), .c(x01), .O(z26));
  nand4  g101(.a(new_n248_), .b(new_n247_), .c(z04), .d(x45), .O(new_n253_));
  inv1   g102(.a(new_n253_), .O(z27));
  nand4  g103(.a(new_n248_), .b(new_n247_), .c(z04), .d(x46), .O(new_n255_));
  inv1   g104(.a(new_n255_), .O(z28));
  nand4  g105(.a(new_n248_), .b(new_n247_), .c(z04), .d(x47), .O(new_n257_));
  inv1   g106(.a(new_n257_), .O(z29));
  nand4  g107(.a(new_n248_), .b(new_n247_), .c(z04), .d(x48), .O(new_n259_));
  inv1   g108(.a(new_n259_), .O(z30));
  nand3  g109(.a(new_n248_), .b(new_n247_), .c(x49), .O(new_n261_));
  aoi21  g110(.a(new_n261_), .b(x79), .c(x01), .O(z31));
  nand3  g111(.a(new_n248_), .b(new_n247_), .c(x50), .O(new_n263_));
  aoi21  g112(.a(new_n263_), .b(x79), .c(x01), .O(z32));
  xnor2a g113(.a(x84), .b(x82), .O(new_n265_));
  xnor2a g114(.a(x83), .b(x81), .O(new_n266_));
  nor2   g115(.a(new_n224_), .b(new_n240_), .O(new_n267_));
  oai21  g116(.a(new_n266_), .b(new_n265_), .c(new_n267_), .O(new_n268_));
  aoi21  g117(.a(new_n266_), .b(new_n265_), .c(new_n268_), .O(new_n269_));
  nand2  g118(.a(x51), .b(new_n224_), .O(new_n270_));
  nor2   g119(.a(new_n270_), .b(new_n246_), .O(new_n271_));
  oai21  g120(.a(new_n271_), .b(new_n269_), .c(new_n241_), .O(new_n272_));
  aoi21  g121(.a(new_n272_), .b(x79), .c(x01), .O(z33));
  nand2  g122(.a(new_n225_), .b(x79), .O(new_n274_));
  inv1   g123(.a(new_n274_), .O(new_n275_));
  xor2a  g124(.a(new_n245_), .b(x81), .O(new_n276_));
  nand3  g125(.a(new_n276_), .b(x83), .c(x42), .O(new_n277_));
  oai21  g126(.a(new_n230_), .b(new_n224_), .c(new_n246_), .O(new_n278_));
  nor2   g127(.a(x04), .b(x01), .O(new_n279_));
  nand2  g128(.a(new_n279_), .b(x52), .O(new_n280_));
  inv1   g129(.a(new_n280_), .O(new_n281_));
  nand4  g130(.a(new_n281_), .b(new_n278_), .c(new_n277_), .d(new_n275_), .O(new_n282_));
  inv1   g131(.a(new_n282_), .O(z34));
  nand4  g132(.a(new_n278_), .b(new_n277_), .c(new_n241_), .d(x53), .O(new_n284_));
  aoi21  g133(.a(new_n284_), .b(x79), .c(x01), .O(z35));
  nand2  g134(.a(new_n279_), .b(x54), .O(new_n286_));
  inv1   g135(.a(new_n286_), .O(new_n287_));
  nand4  g136(.a(new_n287_), .b(new_n278_), .c(new_n277_), .d(new_n275_), .O(new_n288_));
  inv1   g137(.a(new_n288_), .O(z36));
  nand2  g138(.a(new_n279_), .b(x55), .O(new_n290_));
  inv1   g139(.a(new_n290_), .O(new_n291_));
  nand4  g140(.a(new_n291_), .b(new_n278_), .c(new_n277_), .d(new_n275_), .O(new_n292_));
  inv1   g141(.a(new_n292_), .O(z37));
  nand2  g142(.a(new_n279_), .b(x56), .O(new_n294_));
  inv1   g143(.a(new_n294_), .O(new_n295_));
  nand4  g144(.a(new_n295_), .b(new_n278_), .c(new_n277_), .d(new_n275_), .O(new_n296_));
  inv1   g145(.a(new_n296_), .O(z38));
  nand2  g146(.a(new_n279_), .b(x57), .O(new_n298_));
  inv1   g147(.a(new_n298_), .O(new_n299_));
  nand4  g148(.a(new_n299_), .b(new_n278_), .c(new_n277_), .d(new_n275_), .O(new_n300_));
  inv1   g149(.a(new_n300_), .O(z39));
  nand4  g150(.a(new_n278_), .b(new_n277_), .c(new_n241_), .d(x58), .O(new_n302_));
  aoi21  g151(.a(new_n302_), .b(x79), .c(x01), .O(z40));
  nand4  g152(.a(new_n278_), .b(new_n277_), .c(new_n241_), .d(x59), .O(new_n304_));
  aoi21  g153(.a(new_n304_), .b(x79), .c(x01), .O(z41));
  nand2  g154(.a(new_n279_), .b(x60), .O(new_n306_));
  inv1   g155(.a(new_n306_), .O(new_n307_));
  nand4  g156(.a(new_n307_), .b(new_n278_), .c(new_n277_), .d(new_n275_), .O(new_n308_));
  inv1   g157(.a(new_n308_), .O(z42));
  nand2  g158(.a(new_n279_), .b(x61), .O(new_n310_));
  inv1   g159(.a(new_n310_), .O(new_n311_));
  nand4  g160(.a(new_n311_), .b(new_n278_), .c(new_n277_), .d(new_n275_), .O(new_n312_));
  inv1   g161(.a(new_n312_), .O(z43));
  nand2  g162(.a(new_n279_), .b(x62), .O(new_n314_));
  inv1   g163(.a(new_n314_), .O(new_n315_));
  nand4  g164(.a(new_n315_), .b(new_n278_), .c(new_n277_), .d(new_n275_), .O(new_n316_));
  inv1   g165(.a(new_n316_), .O(z44));
  nand2  g166(.a(new_n279_), .b(x63), .O(new_n318_));
  inv1   g167(.a(new_n318_), .O(new_n319_));
  nand4  g168(.a(new_n319_), .b(new_n278_), .c(new_n277_), .d(new_n275_), .O(new_n320_));
  inv1   g169(.a(new_n320_), .O(z45));
  nand4  g170(.a(new_n278_), .b(new_n277_), .c(new_n241_), .d(x64), .O(new_n322_));
  aoi21  g171(.a(new_n322_), .b(x79), .c(x01), .O(z46));
  inv1   g172(.a(new_n165_), .O(new_n324_));
  nor3   g173(.a(new_n235_), .b(new_n171_), .c(new_n324_), .O(new_n325_));
  oai21  g174(.a(x75), .b(x67), .c(new_n325_), .O(new_n326_));
  inv1   g175(.a(new_n326_), .O(z47));
  nand2  g176(.a(new_n325_), .b(x68), .O(new_n328_));
  inv1   g177(.a(new_n328_), .O(z48));
  nand2  g178(.a(new_n325_), .b(x69), .O(new_n330_));
  inv1   g179(.a(new_n330_), .O(z49));
  nand2  g180(.a(new_n325_), .b(x70), .O(new_n332_));
  inv1   g181(.a(new_n332_), .O(z50));
  nor2   g182(.a(new_n235_), .b(new_n324_), .O(new_n334_));
  nand2  g183(.a(new_n334_), .b(x71), .O(new_n335_));
  aoi21  g184(.a(new_n335_), .b(x79), .c(x01), .O(z51));
  nand2  g185(.a(new_n325_), .b(x72), .O(new_n337_));
  inv1   g186(.a(new_n337_), .O(z52));
  nand2  g187(.a(new_n334_), .b(x73), .O(new_n339_));
  aoi21  g188(.a(new_n339_), .b(x79), .c(x01), .O(z53));
  inv1   g189(.a(x82), .O(new_n342_));
  nand2  g190(.a(x84), .b(new_n342_), .O(new_n343_));
  nand3  g191(.a(new_n279_), .b(x83), .c(new_n244_), .O(new_n344_));
  nor4   g192(.a(new_n344_), .b(new_n274_), .c(new_n343_), .d(x80), .O(z55));
  oai21  g193(.a(new_n235_), .b(x76), .c(new_n159_), .O(new_n346_));
  nand4  g194(.a(new_n346_), .b(new_n162_), .c(new_n152_), .d(x00), .O(z56));
  inv1   g195(.a(x02), .O(new_n348_));
  nand3  g196(.a(x03), .b(new_n348_), .c(x00), .O(new_n349_));
  aoi21  g197(.a(new_n349_), .b(x79), .c(x01), .O(z57));
  nand2  g198(.a(new_n233_), .b(new_n224_), .O(new_n351_));
  nand2  g199(.a(new_n227_), .b(z04), .O(new_n352_));
  aoi21  g200(.a(x42), .b(x40), .c(new_n352_), .O(new_n353_));
  and2   g201(.a(new_n353_), .b(new_n351_), .O(z58));
  aoi21  g202(.a(new_n351_), .b(new_n153_), .c(new_n352_), .O(z59));
  inv1   g203(.a(x04), .O(new_n356_));
  nor2   g204(.a(x42), .b(new_n356_), .O(new_n357_));
  nand3  g205(.a(new_n357_), .b(new_n275_), .c(new_n233_), .O(new_n358_));
  inv1   g206(.a(new_n161_), .O(new_n359_));
  nand2  g207(.a(new_n359_), .b(new_n159_), .O(new_n360_));
  inv1   g208(.a(new_n360_), .O(new_n361_));
  aoi21  g209(.a(new_n361_), .b(new_n235_), .c(new_n160_), .O(new_n362_));
  aoi21  g210(.a(new_n362_), .b(new_n358_), .c(x01), .O(z60));
  nor2   g211(.a(new_n360_), .b(new_n235_), .O(new_n364_));
  oai21  g212(.a(new_n364_), .b(new_n241_), .c(x80), .O(new_n365_));
  aoi21  g213(.a(new_n365_), .b(x79), .c(x01), .O(z61));
  inv1   g214(.a(x84), .O(new_n367_));
  nor2   g215(.a(new_n360_), .b(new_n367_), .O(new_n368_));
  oai21  g216(.a(new_n231_), .b(new_n229_), .c(new_n224_), .O(new_n369_));
  aoi21  g217(.a(new_n369_), .b(x04), .c(new_n159_), .O(new_n370_));
  oai21  g218(.a(new_n370_), .b(new_n368_), .c(x81), .O(new_n371_));
  nor2   g219(.a(new_n159_), .b(x81), .O(new_n372_));
  aoi21  g220(.a(new_n372_), .b(new_n357_), .c(new_n160_), .O(new_n373_));
  aoi21  g221(.a(new_n373_), .b(new_n371_), .c(x01), .O(z62));
  nor2   g222(.a(new_n364_), .b(new_n241_), .O(new_n375_));
  nor3   g223(.a(new_n375_), .b(new_n171_), .c(new_n342_), .O(z63));
  nor3   g224(.a(new_n375_), .b(new_n171_), .c(new_n230_), .O(z64));
  oai21  g225(.a(new_n360_), .b(new_n244_), .c(new_n242_), .O(new_n378_));
  nand2  g226(.a(new_n378_), .b(x84), .O(new_n379_));
  aoi21  g227(.a(new_n379_), .b(x79), .c(x01), .O(z65));
  zero   g228(.O(z54));
endmodule


