// Benchmark "FAU" written by ABC on Tue Aug 11 22:51:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55,
    x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69,
    x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82, x83,
    x84,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54,
    x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68,
    x69, x70, x71, x72, x73, x74, x75, x76, x77, x78, x79, x80, x81, x82,
    x83, x84;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35, z36, z37, z38, z39, z40, z41,
    z42, z43, z44, z45, z46, z47, z48, z49, z50, z51, z52, z53, z54, z55,
    z56, z57, z58, z59, z60, z61, z62, z63, z64, z65;
  wire new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n174_,
    new_n175_, new_n177_, new_n178_, new_n180_, new_n181_, new_n183_,
    new_n184_, new_n186_, new_n187_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n198_, new_n199_, new_n201_,
    new_n202_, new_n204_, new_n205_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n213_, new_n214_, new_n216_, new_n217_, new_n219_,
    new_n220_, new_n222_, new_n223_, new_n225_, new_n226_, new_n227_,
    new_n228_, new_n229_, new_n230_, new_n231_, new_n232_, new_n233_,
    new_n234_, new_n235_, new_n236_, new_n239_, new_n240_, new_n241_,
    new_n242_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n250_, new_n252_, new_n254_, new_n256_, new_n258_,
    new_n260_, new_n261_, new_n263_, new_n265_, new_n267_, new_n268_,
    new_n269_, new_n270_, new_n271_, new_n272_, new_n273_, new_n274_,
    new_n275_, new_n276_, new_n277_, new_n278_, new_n279_, new_n280_,
    new_n281_, new_n283_, new_n284_, new_n285_, new_n286_, new_n287_,
    new_n288_, new_n289_, new_n290_, new_n292_, new_n293_, new_n295_,
    new_n296_, new_n297_, new_n299_, new_n301_, new_n303_, new_n305_,
    new_n306_, new_n308_, new_n309_, new_n310_, new_n312_, new_n313_,
    new_n315_, new_n317_, new_n318_, new_n320_, new_n321_, new_n322_,
    new_n324_, new_n325_, new_n327_, new_n328_, new_n330_, new_n332_,
    new_n333_, new_n335_, new_n337_, new_n339_, new_n341_, new_n343_,
    new_n344_, new_n346_, new_n348_, new_n349_, new_n351_, new_n352_,
    new_n353_, new_n355_, new_n356_, new_n358_, new_n359_, new_n361_,
    new_n362_, new_n364_, new_n365_, new_n366_, new_n368_, new_n370_,
    new_n372_, new_n373_;
  inv1   g000(.a(x01), .O(new_n152_));
  inv1   g001(.a(x79), .O(new_n153_));
  oai21  g002(.a(new_n153_), .b(x40), .c(new_n152_), .O(new_n154_));
  inv1   g003(.a(x40), .O(new_n155_));
  nand2  g004(.a(new_n155_), .b(x06), .O(new_n156_));
  nand2  g005(.a(x52), .b(x40), .O(new_n157_));
  nand3  g006(.a(new_n157_), .b(new_n156_), .c(new_n154_), .O(z00));
  nand2  g007(.a(x78), .b(x77), .O(new_n159_));
  nor2   g008(.a(x78), .b(x77), .O(new_n160_));
  nor2   g009(.a(new_n160_), .b(new_n153_), .O(new_n161_));
  aoi21  g010(.a(new_n161_), .b(new_n159_), .c(x01), .O(z01));
  nand2  g011(.a(x79), .b(new_n152_), .O(new_n163_));
  inv1   g012(.a(new_n163_), .O(z04));
  inv1   g013(.a(x66), .O(new_n165_));
  inv1   g014(.a(x78), .O(new_n166_));
  nand2  g015(.a(new_n166_), .b(x77), .O(new_n167_));
  inv1   g016(.a(x77), .O(new_n168_));
  nand3  g017(.a(x78), .b(new_n168_), .c(x75), .O(new_n169_));
  oai21  g018(.a(new_n167_), .b(new_n165_), .c(new_n169_), .O(new_n170_));
  and2   g019(.a(new_n170_), .b(z04), .O(z02));
  nand2  g020(.a(new_n153_), .b(new_n152_), .O(new_n172_));
  inv1   g021(.a(new_n172_), .O(z03));
  nand2  g022(.a(x65), .b(x40), .O(new_n174_));
  nand2  g023(.a(new_n155_), .b(x23), .O(new_n175_));
  nand3  g024(.a(new_n175_), .b(new_n174_), .c(new_n172_), .O(z05));
  inv1   g025(.a(x64), .O(new_n177_));
  oai21  g026(.a(x40), .b(x24), .c(new_n172_), .O(new_n178_));
  aoi21  g027(.a(new_n177_), .b(x40), .c(new_n178_), .O(z06));
  nand2  g028(.a(x63), .b(x40), .O(new_n180_));
  nand2  g029(.a(new_n155_), .b(x25), .O(new_n181_));
  nand3  g030(.a(new_n181_), .b(new_n180_), .c(new_n172_), .O(z07));
  inv1   g031(.a(x62), .O(new_n183_));
  oai21  g032(.a(x40), .b(x26), .c(new_n172_), .O(new_n184_));
  aoi21  g033(.a(new_n183_), .b(x40), .c(new_n184_), .O(z08));
  nand2  g034(.a(x61), .b(x40), .O(new_n186_));
  nand2  g035(.a(new_n155_), .b(x27), .O(new_n187_));
  nand3  g036(.a(new_n187_), .b(new_n186_), .c(new_n172_), .O(z09));
  inv1   g037(.a(x60), .O(new_n189_));
  oai21  g038(.a(x40), .b(x28), .c(new_n172_), .O(new_n190_));
  aoi21  g039(.a(new_n189_), .b(x40), .c(new_n190_), .O(z10));
  nand2  g040(.a(x59), .b(x40), .O(new_n192_));
  nand2  g041(.a(new_n155_), .b(x29), .O(new_n193_));
  nand3  g042(.a(new_n193_), .b(new_n192_), .c(new_n172_), .O(z11));
  inv1   g043(.a(x58), .O(new_n195_));
  oai21  g044(.a(x40), .b(x30), .c(new_n172_), .O(new_n196_));
  aoi21  g045(.a(new_n195_), .b(x40), .c(new_n196_), .O(z12));
  nor2   g046(.a(x57), .b(new_n155_), .O(new_n198_));
  oai21  g047(.a(x40), .b(x31), .c(new_n172_), .O(new_n199_));
  nor2   g048(.a(new_n199_), .b(new_n198_), .O(z13));
  nand2  g049(.a(x51), .b(x40), .O(new_n201_));
  nand2  g050(.a(new_n155_), .b(x32), .O(new_n202_));
  nand3  g051(.a(new_n202_), .b(new_n201_), .c(new_n172_), .O(z14));
  nand2  g052(.a(x50), .b(x40), .O(new_n204_));
  nand2  g053(.a(new_n155_), .b(x33), .O(new_n205_));
  nand3  g054(.a(new_n205_), .b(new_n204_), .c(new_n172_), .O(z15));
  nand2  g055(.a(x49), .b(x40), .O(new_n207_));
  nand2  g056(.a(new_n155_), .b(x34), .O(new_n208_));
  nand3  g057(.a(new_n208_), .b(new_n207_), .c(new_n172_), .O(z16));
  nor2   g058(.a(x48), .b(new_n155_), .O(new_n210_));
  oai21  g059(.a(x40), .b(x35), .c(new_n172_), .O(new_n211_));
  nor2   g060(.a(new_n211_), .b(new_n210_), .O(z17));
  nand2  g061(.a(x47), .b(x40), .O(new_n213_));
  nand2  g062(.a(new_n155_), .b(x36), .O(new_n214_));
  nand3  g063(.a(new_n214_), .b(new_n213_), .c(new_n172_), .O(z18));
  nand2  g064(.a(x46), .b(x40), .O(new_n216_));
  nand2  g065(.a(new_n155_), .b(x37), .O(new_n217_));
  nand3  g066(.a(new_n217_), .b(new_n216_), .c(new_n172_), .O(z19));
  nor2   g067(.a(x45), .b(new_n155_), .O(new_n219_));
  oai21  g068(.a(x40), .b(x38), .c(new_n172_), .O(new_n220_));
  nor2   g069(.a(new_n220_), .b(new_n219_), .O(z20));
  nand2  g070(.a(x44), .b(x40), .O(new_n222_));
  nand2  g071(.a(new_n155_), .b(x39), .O(new_n223_));
  nand3  g072(.a(new_n223_), .b(new_n222_), .c(new_n172_), .O(z21));
  inv1   g073(.a(x42), .O(new_n225_));
  nand3  g074(.a(x78), .b(x77), .c(x04), .O(new_n226_));
  inv1   g075(.a(new_n226_), .O(new_n227_));
  nand3  g076(.a(x84), .b(x82), .c(x80), .O(new_n228_));
  inv1   g077(.a(x74), .O(new_n229_));
  inv1   g078(.a(x83), .O(new_n230_));
  nand4  g079(.a(new_n230_), .b(x81), .c(new_n229_), .d(x43), .O(new_n231_));
  or2    g080(.a(new_n231_), .b(new_n228_), .O(new_n232_));
  nand3  g081(.a(new_n232_), .b(new_n227_), .c(new_n225_), .O(new_n233_));
  xor2a  g082(.a(x84), .b(x81), .O(new_n234_));
  nor2   g083(.a(new_n234_), .b(x41), .O(new_n235_));
  nand2  g084(.a(new_n235_), .b(new_n170_), .O(new_n236_));
  aoi21  g085(.a(new_n236_), .b(new_n233_), .c(new_n163_), .O(z22));
  oai21  g086(.a(new_n153_), .b(x00), .c(new_n152_), .O(z23));
  inv1   g087(.a(x04), .O(new_n239_));
  inv1   g088(.a(x43), .O(new_n240_));
  inv1   g089(.a(new_n159_), .O(new_n241_));
  nand4  g090(.a(new_n241_), .b(new_n240_), .c(x05), .d(new_n239_), .O(new_n242_));
  aoi21  g091(.a(new_n242_), .b(x79), .c(x01), .O(z24));
  inv1   g092(.a(x05), .O(new_n244_));
  inv1   g093(.a(x81), .O(new_n245_));
  xnor2a g094(.a(x84), .b(x82), .O(new_n246_));
  nand2  g095(.a(new_n246_), .b(new_n245_), .O(new_n247_));
  xor2a  g096(.a(x84), .b(x82), .O(new_n248_));
  aoi21  g097(.a(new_n248_), .b(x81), .c(new_n159_), .O(new_n249_));
  nand4  g098(.a(new_n249_), .b(new_n247_), .c(z04), .d(new_n225_), .O(new_n250_));
  nor3   g099(.a(new_n250_), .b(new_n244_), .c(x04), .O(z25));
  nand2  g100(.a(x44), .b(new_n239_), .O(new_n252_));
  nor2   g101(.a(new_n252_), .b(new_n250_), .O(z26));
  nand2  g102(.a(x45), .b(new_n239_), .O(new_n254_));
  nor2   g103(.a(new_n254_), .b(new_n250_), .O(z27));
  nand2  g104(.a(x46), .b(new_n239_), .O(new_n256_));
  nor2   g105(.a(new_n256_), .b(new_n250_), .O(z28));
  nand2  g106(.a(x47), .b(new_n239_), .O(new_n258_));
  nor2   g107(.a(new_n258_), .b(new_n250_), .O(z29));
  nor2   g108(.a(x42), .b(x04), .O(new_n260_));
  nand4  g109(.a(new_n260_), .b(new_n249_), .c(new_n247_), .d(x48), .O(new_n261_));
  aoi21  g110(.a(new_n261_), .b(x79), .c(x01), .O(z30));
  nand4  g111(.a(new_n260_), .b(new_n249_), .c(new_n247_), .d(x49), .O(new_n263_));
  aoi21  g112(.a(new_n263_), .b(x79), .c(x01), .O(z31));
  nand2  g113(.a(x50), .b(new_n239_), .O(new_n265_));
  nor2   g114(.a(new_n265_), .b(new_n250_), .O(z32));
  nand2  g115(.a(new_n230_), .b(x81), .O(new_n267_));
  nor2   g116(.a(new_n225_), .b(new_n244_), .O(new_n268_));
  nand2  g117(.a(x83), .b(new_n245_), .O(new_n269_));
  nand3  g118(.a(new_n269_), .b(new_n268_), .c(new_n267_), .O(new_n270_));
  inv1   g119(.a(x51), .O(new_n271_));
  nor2   g120(.a(new_n271_), .b(x42), .O(new_n272_));
  nand2  g121(.a(new_n272_), .b(new_n245_), .O(new_n273_));
  nand3  g122(.a(new_n273_), .b(new_n270_), .c(new_n248_), .O(new_n274_));
  nand2  g123(.a(new_n269_), .b(new_n267_), .O(new_n275_));
  nand2  g124(.a(new_n275_), .b(new_n268_), .O(new_n276_));
  aoi21  g125(.a(new_n272_), .b(x81), .c(new_n248_), .O(new_n277_));
  nor2   g126(.a(x04), .b(x01), .O(new_n278_));
  nand3  g127(.a(new_n278_), .b(new_n241_), .c(x79), .O(new_n279_));
  aoi21  g128(.a(new_n277_), .b(new_n276_), .c(new_n279_), .O(new_n280_));
  nand2  g129(.a(new_n280_), .b(new_n274_), .O(new_n281_));
  inv1   g130(.a(new_n281_), .O(z33));
  nand2  g131(.a(x83), .b(x42), .O(new_n283_));
  xor2a  g132(.a(new_n283_), .b(x81), .O(new_n284_));
  nand2  g133(.a(new_n284_), .b(new_n246_), .O(new_n285_));
  xor2a  g134(.a(new_n283_), .b(new_n245_), .O(new_n286_));
  nand2  g135(.a(new_n286_), .b(new_n248_), .O(new_n287_));
  inv1   g136(.a(x52), .O(new_n288_));
  nor2   g137(.a(new_n288_), .b(x04), .O(new_n289_));
  nand4  g138(.a(new_n289_), .b(new_n287_), .c(new_n285_), .d(new_n241_), .O(new_n290_));
  aoi21  g139(.a(new_n290_), .b(x79), .c(x01), .O(z34));
  nand4  g140(.a(new_n287_), .b(new_n285_), .c(new_n241_), .d(x79), .O(new_n292_));
  nand2  g141(.a(new_n278_), .b(x53), .O(new_n293_));
  nor2   g142(.a(new_n293_), .b(new_n292_), .O(z35));
  inv1   g143(.a(x54), .O(new_n295_));
  nor2   g144(.a(new_n295_), .b(x04), .O(new_n296_));
  nand4  g145(.a(new_n296_), .b(new_n287_), .c(new_n285_), .d(new_n241_), .O(new_n297_));
  aoi21  g146(.a(new_n297_), .b(x79), .c(x01), .O(z36));
  nand2  g147(.a(new_n278_), .b(x55), .O(new_n299_));
  nor2   g148(.a(new_n299_), .b(new_n292_), .O(z37));
  nand2  g149(.a(new_n278_), .b(x56), .O(new_n301_));
  nor2   g150(.a(new_n301_), .b(new_n292_), .O(z38));
  nand2  g151(.a(new_n278_), .b(x57), .O(new_n303_));
  nor2   g152(.a(new_n303_), .b(new_n292_), .O(z39));
  nor2   g153(.a(new_n195_), .b(x04), .O(new_n305_));
  nand4  g154(.a(new_n305_), .b(new_n287_), .c(new_n285_), .d(new_n241_), .O(new_n306_));
  aoi21  g155(.a(new_n306_), .b(x79), .c(x01), .O(z40));
  inv1   g156(.a(x59), .O(new_n308_));
  nor2   g157(.a(new_n308_), .b(x04), .O(new_n309_));
  nand4  g158(.a(new_n309_), .b(new_n287_), .c(new_n285_), .d(new_n241_), .O(new_n310_));
  aoi21  g159(.a(new_n310_), .b(x79), .c(x01), .O(z41));
  nor2   g160(.a(new_n189_), .b(x04), .O(new_n312_));
  nand4  g161(.a(new_n312_), .b(new_n287_), .c(new_n285_), .d(new_n241_), .O(new_n313_));
  aoi21  g162(.a(new_n313_), .b(x79), .c(x01), .O(z42));
  nand2  g163(.a(new_n278_), .b(x61), .O(new_n315_));
  nor2   g164(.a(new_n315_), .b(new_n292_), .O(z43));
  nor2   g165(.a(new_n183_), .b(x04), .O(new_n317_));
  nand4  g166(.a(new_n317_), .b(new_n287_), .c(new_n285_), .d(new_n241_), .O(new_n318_));
  aoi21  g167(.a(new_n318_), .b(x79), .c(x01), .O(z44));
  inv1   g168(.a(x63), .O(new_n320_));
  nor2   g169(.a(new_n320_), .b(x04), .O(new_n321_));
  nand4  g170(.a(new_n321_), .b(new_n287_), .c(new_n285_), .d(new_n241_), .O(new_n322_));
  aoi21  g171(.a(new_n322_), .b(x79), .c(x01), .O(z45));
  nor2   g172(.a(new_n177_), .b(x04), .O(new_n324_));
  nand4  g173(.a(new_n324_), .b(new_n287_), .c(new_n285_), .d(new_n241_), .O(new_n325_));
  aoi21  g174(.a(new_n325_), .b(x79), .c(x01), .O(z46));
  nor3   g175(.a(new_n234_), .b(new_n167_), .c(new_n163_), .O(new_n327_));
  oai21  g176(.a(x75), .b(x67), .c(new_n327_), .O(new_n328_));
  inv1   g177(.a(new_n328_), .O(z47));
  nand2  g178(.a(new_n327_), .b(x68), .O(new_n330_));
  inv1   g179(.a(new_n330_), .O(z48));
  nor2   g180(.a(new_n234_), .b(new_n167_), .O(new_n332_));
  nand2  g181(.a(new_n332_), .b(x69), .O(new_n333_));
  aoi21  g182(.a(new_n333_), .b(x79), .c(x01), .O(z49));
  nand2  g183(.a(new_n332_), .b(x70), .O(new_n335_));
  aoi21  g184(.a(new_n335_), .b(x79), .c(x01), .O(z50));
  nand2  g185(.a(new_n332_), .b(x71), .O(new_n337_));
  aoi21  g186(.a(new_n337_), .b(x79), .c(x01), .O(z51));
  nand2  g187(.a(new_n327_), .b(x72), .O(new_n339_));
  inv1   g188(.a(new_n339_), .O(z52));
  nand2  g189(.a(new_n327_), .b(x73), .O(new_n341_));
  inv1   g190(.a(new_n341_), .O(z53));
  inv1   g191(.a(x82), .O(new_n343_));
  nand2  g192(.a(x84), .b(new_n343_), .O(new_n344_));
  nor4   g193(.a(new_n279_), .b(new_n269_), .c(new_n344_), .d(x80), .O(z55));
  oai21  g194(.a(new_n234_), .b(x76), .c(new_n159_), .O(new_n346_));
  nand4  g195(.a(new_n346_), .b(new_n161_), .c(new_n152_), .d(x00), .O(z56));
  inv1   g196(.a(x03), .O(new_n348_));
  nand2  g197(.a(new_n152_), .b(x00), .O(new_n349_));
  nor4   g198(.a(new_n349_), .b(new_n153_), .c(new_n348_), .d(x02), .O(z57));
  oai21  g199(.a(new_n231_), .b(new_n228_), .c(new_n225_), .O(new_n351_));
  aoi21  g200(.a(x42), .b(x40), .c(new_n226_), .O(new_n352_));
  nand2  g201(.a(new_n352_), .b(new_n351_), .O(new_n353_));
  aoi21  g202(.a(new_n353_), .b(x79), .c(x01), .O(z58));
  nand2  g203(.a(new_n351_), .b(new_n155_), .O(new_n355_));
  nand2  g204(.a(new_n355_), .b(new_n227_), .O(new_n356_));
  aoi21  g205(.a(new_n356_), .b(x79), .c(x01), .O(z59));
  nor2   g206(.a(new_n160_), .b(new_n241_), .O(new_n358_));
  nand2  g207(.a(new_n358_), .b(new_n234_), .O(new_n359_));
  aoi21  g208(.a(new_n359_), .b(new_n233_), .c(new_n163_), .O(z60));
  oai22  g209(.a(new_n160_), .b(new_n241_), .c(new_n166_), .d(x04), .O(new_n361_));
  nand3  g210(.a(new_n361_), .b(new_n359_), .c(x80), .O(new_n362_));
  aoi21  g211(.a(new_n362_), .b(x79), .c(x01), .O(z61));
  inv1   g212(.a(x84), .O(new_n364_));
  nand2  g213(.a(new_n358_), .b(new_n364_), .O(new_n365_));
  nand3  g214(.a(new_n365_), .b(new_n361_), .c(x81), .O(new_n366_));
  aoi21  g215(.a(new_n366_), .b(new_n233_), .c(new_n163_), .O(z62));
  nand4  g216(.a(new_n361_), .b(new_n359_), .c(z04), .d(x82), .O(new_n368_));
  inv1   g217(.a(new_n368_), .O(z63));
  nand4  g218(.a(new_n361_), .b(new_n359_), .c(z04), .d(x83), .O(new_n370_));
  inv1   g219(.a(new_n370_), .O(z64));
  nand2  g220(.a(new_n358_), .b(new_n245_), .O(new_n372_));
  nand4  g221(.a(new_n372_), .b(new_n361_), .c(z04), .d(x84), .O(new_n373_));
  inv1   g222(.a(new_n373_), .O(z65));
  inv1   g223(.a(new_n172_), .O(z54));
endmodule


