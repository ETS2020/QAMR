// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x8), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x7), .O(z0));
  nand2  g05(.a(x7), .b(new_n18_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(x9), .O(new_n24_));
  aoi21  g07(.a(new_n19_), .b(x7), .c(x8), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n24_), .O(z1));
  nor2   g09(.a(x9), .b(x8), .O(new_n27_));
  nor2   g10(.a(new_n27_), .b(x7), .O(new_n28_));
  aoi21  g11(.a(x7), .b(new_n18_), .c(new_n28_), .O(z2));
  inv1   g12(.a(x7), .O(new_n30_));
  inv1   g13(.a(x0), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  inv1   g15(.a(x2), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n20_), .c(new_n30_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n23_), .O(z3));
  nand2  g19(.a(x9), .b(x8), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(x7), .c(x6), .d(new_n18_), .O(z4));
  nor2   g21(.a(x1), .b(x0), .O(new_n39_));
  nand2  g22(.a(new_n37_), .b(x7), .O(new_n40_));
  oai21  g23(.a(new_n30_), .b(x4), .c(x8), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n40_), .c(new_n39_), .d(x2), .O(new_n42_));
  nand3  g25(.a(x8), .b(new_n30_), .c(x5), .O(new_n43_));
  inv1   g26(.a(new_n43_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x9), .O(new_n45_));
  nand4  g28(.a(new_n19_), .b(x7), .c(new_n18_), .d(x3), .O(new_n46_));
  nand2  g29(.a(x7), .b(x4), .O(new_n47_));
  aoi21  g30(.a(x7), .b(x4), .c(x6), .O(new_n48_));
  aoi21  g31(.a(new_n47_), .b(new_n27_), .c(new_n48_), .O(new_n49_));
  nand4  g32(.a(new_n49_), .b(new_n46_), .c(new_n45_), .d(new_n42_), .O(z5));
  oai21  g33(.a(new_n34_), .b(new_n23_), .c(new_n43_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x9), .O(new_n52_));
  aoi21  g35(.a(new_n25_), .b(new_n24_), .c(new_n48_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n52_), .O(z6));
endmodule


