// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x9), .O(new_n18_));
  nor2   g01(.a(new_n18_), .b(x2), .O(new_n19_));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nor2   g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(new_n18_), .c(new_n19_), .O(z0));
  inv1   g06(.a(new_n19_), .O(new_n24_));
  nand3  g07(.a(x9), .b(new_n21_), .c(x7), .O(new_n25_));
  nor2   g08(.a(x8), .b(x7), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n18_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n25_), .c(new_n24_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z1));
  inv1   g12(.a(new_n27_), .O(z2));
  nor2   g13(.a(x1), .b(x0), .O(new_n31_));
  nor2   g14(.a(x9), .b(x2), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n31_), .c(x8), .d(new_n20_), .O(z3));
  nand2  g16(.a(x9), .b(x8), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x7), .O(new_n35_));
  inv1   g18(.a(new_n35_), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(x6), .c(new_n19_), .O(z4));
  nand2  g20(.a(x8), .b(new_n20_), .O(new_n38_));
  nand3  g21(.a(new_n35_), .b(new_n38_), .c(new_n31_), .O(new_n39_));
  nand4  g22(.a(x9), .b(x8), .c(new_n20_), .d(x5), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n39_), .c(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x2), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(x8), .c(x6), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n42_), .O(z5));
  nand4  g30(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n48_));
  inv1   g31(.a(new_n48_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n26_), .c(new_n18_), .O(new_n50_));
  inv1   g33(.a(x6), .O(new_n51_));
  nand2  g34(.a(new_n21_), .b(x7), .O(new_n52_));
  nand3  g35(.a(x8), .b(new_n20_), .c(x5), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(x9), .c(new_n51_), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n50_), .c(new_n19_), .O(z6));
endmodule


