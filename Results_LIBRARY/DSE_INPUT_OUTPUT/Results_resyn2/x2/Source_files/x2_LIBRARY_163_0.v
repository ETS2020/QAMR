// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n25_, new_n26_, new_n27_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  nand2  g04(.a(x9), .b(new_n21_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n20_), .O(z1));
  nor2   g06(.a(x9), .b(x8), .O(z2));
  nor3   g07(.a(x2), .b(x1), .c(x0), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n21_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x8), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n18_), .O(z3));
  or2    g11(.a(z2), .b(x6), .O(new_n29_));
  oai21  g12(.a(x9), .b(new_n21_), .c(x8), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n29_), .c(new_n22_), .O(z4));
  xor2a  g14(.a(x8), .b(x7), .O(new_n32_));
  inv1   g15(.a(x0), .O(new_n33_));
  inv1   g16(.a(x1), .O(new_n34_));
  nand3  g17(.a(x2), .b(new_n34_), .c(new_n33_), .O(new_n35_));
  nor2   g18(.a(new_n35_), .b(new_n32_), .O(new_n36_));
  nand3  g19(.a(x8), .b(new_n21_), .c(x5), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x6), .O(new_n38_));
  oai21  g21(.a(new_n38_), .b(new_n36_), .c(x9), .O(new_n39_));
  inv1   g22(.a(x4), .O(new_n40_));
  nand4  g23(.a(new_n18_), .b(x7), .c(new_n40_), .d(x3), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x6), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x8), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n39_), .O(z5));
  nor2   g27(.a(new_n18_), .b(new_n21_), .O(new_n45_));
  oai21  g28(.a(new_n25_), .b(new_n20_), .c(new_n45_), .O(new_n46_));
  nand4  g29(.a(new_n18_), .b(x7), .c(x4), .d(x3), .O(new_n47_));
  nand3  g30(.a(x9), .b(new_n21_), .c(x5), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x8), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n46_), .c(new_n29_), .O(z6));
endmodule


