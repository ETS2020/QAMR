// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_;
  inv1   g00(.a(x9), .O(new_n18_));
  nor2   g01(.a(new_n18_), .b(x4), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  nor2   g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x7), .c(new_n19_), .O(z0));
  inv1   g05(.a(x7), .O(new_n23_));
  nand2  g06(.a(new_n20_), .b(new_n23_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n18_), .O(new_n25_));
  nand2  g08(.a(x9), .b(x4), .O(new_n26_));
  nor2   g09(.a(x8), .b(new_n23_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(z1));
  and2   g11(.a(new_n26_), .b(new_n25_), .O(z2));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  inv1   g14(.a(x2), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  nand2  g16(.a(new_n21_), .b(new_n23_), .O(new_n34_));
  or2    g17(.a(new_n34_), .b(new_n33_), .O(z3));
  nand2  g18(.a(x9), .b(x8), .O(new_n36_));
  and2   g19(.a(x7), .b(x6), .O(new_n37_));
  aoi21  g20(.a(new_n37_), .b(new_n36_), .c(new_n19_), .O(z4));
  oai21  g21(.a(new_n36_), .b(new_n23_), .c(new_n24_), .O(new_n39_));
  nand4  g22(.a(new_n39_), .b(x2), .c(new_n31_), .d(new_n30_), .O(new_n40_));
  inv1   g23(.a(x4), .O(new_n41_));
  nand2  g24(.a(x7), .b(x3), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n18_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n41_), .O(new_n44_));
  inv1   g27(.a(new_n36_), .O(new_n45_));
  inv1   g28(.a(x5), .O(new_n46_));
  nor2   g29(.a(x7), .b(new_n46_), .O(new_n47_));
  nand2  g30(.a(new_n18_), .b(new_n20_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x6), .O(new_n49_));
  aoi21  g32(.a(new_n47_), .b(new_n45_), .c(new_n49_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n44_), .c(new_n40_), .O(z5));
  nand2  g34(.a(new_n33_), .b(x8), .O(new_n52_));
  nand3  g35(.a(new_n18_), .b(x8), .c(x3), .O(new_n53_));
  inv1   g36(.a(new_n53_), .O(new_n54_));
  aoi21  g37(.a(new_n52_), .b(x9), .c(new_n54_), .O(new_n55_));
  nand2  g38(.a(x7), .b(x4), .O(new_n56_));
  aoi21  g39(.a(x9), .b(new_n41_), .c(x6), .O(new_n57_));
  nand4  g40(.a(x9), .b(x8), .c(x5), .d(x4), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n48_), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n23_), .c(new_n57_), .O(new_n60_));
  oai21  g43(.a(new_n56_), .b(new_n55_), .c(new_n60_), .O(z6));
endmodule


