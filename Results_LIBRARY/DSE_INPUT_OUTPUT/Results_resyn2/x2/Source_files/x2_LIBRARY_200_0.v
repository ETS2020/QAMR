// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  nor2   g03(.a(new_n18_), .b(x8), .O(new_n21_));
  inv1   g04(.a(x1), .O(new_n22_));
  nor2   g05(.a(x7), .b(new_n22_), .O(new_n23_));
  inv1   g06(.a(x8), .O(new_n24_));
  nand2  g07(.a(new_n18_), .b(new_n24_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  oai21  g09(.a(new_n21_), .b(new_n20_), .c(new_n26_), .O(z1));
  nand3  g10(.a(new_n23_), .b(new_n18_), .c(new_n24_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z2));
  nand2  g12(.a(new_n20_), .b(new_n22_), .O(z3));
  nand2  g13(.a(x9), .b(x8), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(x7), .c(x6), .O(z4));
  inv1   g15(.a(x4), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(x3), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(x8), .c(x9), .O(new_n35_));
  inv1   g18(.a(x0), .O(new_n36_));
  nand3  g19(.a(x2), .b(new_n22_), .c(new_n36_), .O(new_n37_));
  nor2   g20(.a(new_n37_), .b(new_n31_), .O(new_n38_));
  oai21  g21(.a(new_n38_), .b(new_n35_), .c(x7), .O(new_n39_));
  inv1   g22(.a(x6), .O(new_n40_));
  nand3  g23(.a(x9), .b(x8), .c(x5), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n25_), .O(new_n42_));
  aoi22  g25(.a(new_n42_), .b(new_n23_), .c(z3), .d(new_n40_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n39_), .O(z5));
  inv1   g27(.a(x2), .O(new_n45_));
  nand3  g28(.a(x9), .b(new_n45_), .c(new_n36_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x7), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n22_), .O(new_n48_));
  nand4  g31(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n21_), .c(x7), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n48_), .c(new_n43_), .O(z6));
endmodule


