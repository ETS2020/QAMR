// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nor2   g05(.a(new_n22_), .b(x9), .O(z2));
  inv1   g06(.a(z2), .O(z1));
  inv1   g07(.a(x0), .O(new_n25_));
  inv1   g08(.a(x1), .O(new_n26_));
  inv1   g09(.a(x2), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(new_n28_));
  nand2  g11(.a(new_n18_), .b(new_n20_), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(new_n28_), .c(x8), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n22_), .O(z3));
  inv1   g14(.a(x6), .O(new_n32_));
  oai21  g15(.a(x9), .b(new_n32_), .c(x8), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(x7), .O(z4));
  oai21  g17(.a(new_n18_), .b(new_n20_), .c(x8), .O(new_n35_));
  nand3  g18(.a(x2), .b(new_n26_), .c(new_n25_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x9), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  inv1   g21(.a(x4), .O(new_n39_));
  nand3  g22(.a(new_n18_), .b(new_n39_), .c(x3), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x8), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x7), .O(new_n42_));
  nor2   g25(.a(new_n18_), .b(x7), .O(new_n43_));
  and2   g26(.a(x8), .b(x5), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(new_n43_), .c(new_n32_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n42_), .c(new_n38_), .O(z5));
  nand2  g29(.a(new_n28_), .b(x9), .O(new_n47_));
  nand2  g30(.a(x4), .b(x3), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n18_), .c(new_n20_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  oai21  g33(.a(new_n18_), .b(x7), .c(new_n21_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n50_), .c(new_n45_), .O(z6));
endmodule


