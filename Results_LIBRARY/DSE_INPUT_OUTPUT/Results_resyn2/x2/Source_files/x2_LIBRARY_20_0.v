// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x2), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x2), .c(x9), .O(z2));
  nor2   g06(.a(x8), .b(new_n20_), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(x9), .c(z2), .O(z1));
  inv1   g08(.a(x2), .O(new_n27_));
  aoi21  g09(.a(x9), .b(x8), .c(new_n20_), .O(new_n28_));
  aoi22  g10(.a(new_n28_), .b(x6), .c(new_n18_), .d(new_n27_), .O(z4));
  inv1   g11(.a(x0), .O(new_n30_));
  inv1   g12(.a(x1), .O(new_n31_));
  nand3  g13(.a(x9), .b(x8), .c(x7), .O(new_n32_));
  nand2  g14(.a(new_n32_), .b(new_n22_), .O(new_n33_));
  nand4  g15(.a(new_n33_), .b(x2), .c(new_n31_), .d(new_n30_), .O(new_n34_));
  inv1   g16(.a(x4), .O(new_n35_));
  nand3  g17(.a(x7), .b(new_n35_), .c(x3), .O(new_n36_));
  nand3  g18(.a(new_n36_), .b(x8), .c(x2), .O(new_n37_));
  nand2  g19(.a(new_n37_), .b(new_n18_), .O(new_n38_));
  inv1   g20(.a(x6), .O(new_n39_));
  nand3  g21(.a(x9), .b(x8), .c(x5), .O(new_n40_));
  inv1   g22(.a(new_n40_), .O(new_n41_));
  aoi21  g23(.a(new_n41_), .b(new_n20_), .c(new_n39_), .O(new_n42_));
  nand3  g24(.a(new_n42_), .b(new_n38_), .c(new_n34_), .O(z5));
  nand3  g25(.a(x7), .b(new_n31_), .c(new_n30_), .O(new_n44_));
  nand2  g26(.a(new_n44_), .b(x9), .O(new_n45_));
  nand2  g27(.a(new_n45_), .b(new_n27_), .O(new_n46_));
  nand2  g28(.a(new_n18_), .b(new_n21_), .O(new_n47_));
  nand2  g29(.a(new_n47_), .b(new_n40_), .O(new_n48_));
  nand2  g30(.a(new_n48_), .b(new_n20_), .O(new_n49_));
  nand2  g31(.a(x4), .b(x3), .O(new_n50_));
  nand2  g32(.a(new_n50_), .b(new_n18_), .O(new_n51_));
  nand3  g33(.a(new_n51_), .b(new_n47_), .c(new_n28_), .O(new_n52_));
  nand4  g34(.a(new_n52_), .b(new_n49_), .c(new_n46_), .d(x6), .O(z6));
  one    g35(.O(z3));
endmodule


