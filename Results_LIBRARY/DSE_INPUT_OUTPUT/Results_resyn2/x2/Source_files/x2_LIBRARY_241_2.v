// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_, new_n27_,
    new_n28_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x1), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nand2  g04(.a(new_n18_), .b(new_n21_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(new_n20_), .c(x7), .O(z2));
  nand2  g06(.a(x9), .b(x7), .O(new_n24_));
  nor2   g07(.a(new_n24_), .b(x8), .O(new_n25_));
  nor2   g08(.a(new_n25_), .b(z2), .O(z1));
  nor2   g09(.a(x2), .b(x0), .O(new_n27_));
  nor2   g10(.a(x9), .b(x7), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(new_n27_), .c(x8), .d(new_n20_), .O(z3));
  nand2  g12(.a(x9), .b(x8), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(x7), .c(x6), .O(z4));
  inv1   g14(.a(x7), .O(new_n32_));
  nand3  g15(.a(x9), .b(x8), .c(x5), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n22_), .c(new_n20_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  inv1   g18(.a(x0), .O(new_n36_));
  nand2  g19(.a(new_n24_), .b(x8), .O(new_n37_));
  oai21  g20(.a(new_n21_), .b(x1), .c(x7), .O(new_n38_));
  nand4  g21(.a(new_n38_), .b(new_n37_), .c(x2), .d(new_n36_), .O(new_n39_));
  inv1   g22(.a(x3), .O(new_n40_));
  oai21  g23(.a(x4), .b(new_n40_), .c(x8), .O(new_n41_));
  nor2   g24(.a(x9), .b(new_n32_), .O(new_n42_));
  aoi21  g25(.a(new_n32_), .b(x1), .c(x6), .O(new_n43_));
  aoi21  g26(.a(new_n42_), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n39_), .c(new_n35_), .O(z5));
  aoi21  g28(.a(new_n33_), .b(new_n22_), .c(x7), .O(new_n46_));
  nand3  g29(.a(new_n27_), .b(x9), .c(x7), .O(new_n47_));
  inv1   g30(.a(new_n47_), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n46_), .c(new_n20_), .O(new_n49_));
  nand2  g32(.a(x4), .b(x3), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(x9), .c(x8), .O(new_n51_));
  aoi21  g34(.a(new_n18_), .b(new_n21_), .c(new_n32_), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n51_), .c(new_n43_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n49_), .O(z6));
endmodule


