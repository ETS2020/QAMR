// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n28_, new_n29_, new_n30_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  nand3  g03(.a(x9), .b(new_n20_), .c(x7), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  nand3  g05(.a(new_n18_), .b(new_n20_), .c(new_n22_), .O(new_n23_));
  nand2  g06(.a(x9), .b(x0), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n21_), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(z1));
  inv1   g09(.a(new_n23_), .O(z2));
  nor2   g10(.a(x2), .b(x1), .O(new_n28_));
  nor2   g11(.a(x7), .b(x0), .O(new_n29_));
  nand4  g12(.a(new_n29_), .b(new_n28_), .c(new_n18_), .d(x8), .O(new_n30_));
  and2   g13(.a(new_n30_), .b(new_n24_), .O(z3));
  oai21  g14(.a(x8), .b(x0), .c(x9), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(x7), .c(x6), .O(z4));
  inv1   g16(.a(x0), .O(new_n34_));
  nand2  g17(.a(new_n20_), .b(new_n22_), .O(new_n35_));
  nand3  g18(.a(x9), .b(x8), .c(x7), .O(new_n36_));
  inv1   g19(.a(x1), .O(new_n37_));
  nand2  g20(.a(x2), .b(new_n37_), .O(new_n38_));
  aoi21  g21(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  nand4  g22(.a(x9), .b(x8), .c(new_n22_), .d(x5), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x6), .O(new_n41_));
  oai21  g24(.a(new_n41_), .b(new_n39_), .c(new_n34_), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(x8), .c(x6), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n42_), .O(z5));
  nand4  g30(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n48_));
  oai21  g31(.a(x2), .b(x1), .c(x8), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(x9), .c(new_n34_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x7), .O(new_n52_));
  inv1   g35(.a(x6), .O(new_n53_));
  nand3  g36(.a(x8), .b(x5), .c(new_n34_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x9), .O(new_n55_));
  aoi21  g38(.a(new_n18_), .b(x8), .c(x7), .O(new_n56_));
  aoi22  g39(.a(new_n56_), .b(new_n55_), .c(new_n24_), .d(new_n53_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n52_), .O(z6));
endmodule


