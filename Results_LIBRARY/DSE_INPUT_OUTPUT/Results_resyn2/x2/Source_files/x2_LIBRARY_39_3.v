// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n19_, new_n20_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_;
  inv1   g00(.a(x8), .O(new_n19_));
  xnor2a g01(.a(x9), .b(x7), .O(new_n20_));
  nand2  g02(.a(new_n20_), .b(new_n19_), .O(z1));
  nor3   g03(.a(x9), .b(x8), .c(x7), .O(z2));
  inv1   g04(.a(x7), .O(new_n23_));
  inv1   g05(.a(x0), .O(new_n24_));
  inv1   g06(.a(x1), .O(new_n25_));
  inv1   g07(.a(x2), .O(new_n26_));
  inv1   g08(.a(x9), .O(new_n27_));
  nand4  g09(.a(new_n27_), .b(new_n26_), .c(new_n25_), .d(new_n24_), .O(new_n28_));
  nand2  g10(.a(new_n28_), .b(new_n23_), .O(new_n29_));
  nand2  g11(.a(new_n29_), .b(x8), .O(z3));
  oai21  g12(.a(x8), .b(x6), .c(x7), .O(z4));
  nor2   g13(.a(x8), .b(x7), .O(new_n32_));
  nand3  g14(.a(x2), .b(new_n25_), .c(new_n24_), .O(new_n33_));
  nand2  g15(.a(new_n33_), .b(x9), .O(new_n34_));
  nand2  g16(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  nand2  g17(.a(x9), .b(new_n19_), .O(new_n36_));
  nand3  g18(.a(x9), .b(x8), .c(x5), .O(new_n37_));
  nand2  g19(.a(new_n37_), .b(x6), .O(new_n38_));
  aoi21  g20(.a(new_n36_), .b(x7), .c(new_n38_), .O(new_n39_));
  nand2  g21(.a(new_n39_), .b(new_n35_), .O(z5));
  oai21  g22(.a(x9), .b(x8), .c(x7), .O(new_n41_));
  nand3  g23(.a(new_n27_), .b(new_n19_), .c(new_n23_), .O(new_n42_));
  nand4  g24(.a(new_n42_), .b(new_n41_), .c(new_n37_), .d(x6), .O(z6));
  one    g25(.O(z0));
endmodule


