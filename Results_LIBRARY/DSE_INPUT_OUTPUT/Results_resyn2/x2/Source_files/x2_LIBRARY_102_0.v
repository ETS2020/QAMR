// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  nor2   g00(.a(x5), .b(x0), .O(new_n18_));
  inv1   g01(.a(x7), .O(new_n19_));
  nor2   g02(.a(x9), .b(new_n19_), .O(new_n20_));
  aoi21  g03(.a(new_n20_), .b(x8), .c(new_n18_), .O(z0));
  nand2  g04(.a(x9), .b(new_n19_), .O(new_n22_));
  nor2   g05(.a(new_n20_), .b(x8), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(new_n22_), .c(new_n18_), .O(z1));
  inv1   g07(.a(x8), .O(new_n25_));
  inv1   g08(.a(x9), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n25_), .c(new_n19_), .O(new_n27_));
  nor2   g10(.a(new_n27_), .b(new_n18_), .O(z2));
  inv1   g11(.a(x0), .O(new_n29_));
  nand2  g12(.a(x8), .b(new_n19_), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  inv1   g14(.a(x2), .O(new_n32_));
  nand3  g15(.a(new_n26_), .b(new_n32_), .c(new_n31_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n30_), .c(x5), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n29_), .O(z3));
  inv1   g18(.a(new_n18_), .O(new_n36_));
  aoi21  g19(.a(x9), .b(x8), .c(new_n19_), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(new_n36_), .c(x6), .O(z4));
  inv1   g21(.a(x6), .O(new_n39_));
  inv1   g22(.a(x4), .O(new_n40_));
  nand3  g23(.a(x7), .b(new_n40_), .c(x3), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(x8), .c(x9), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n39_), .c(new_n36_), .O(new_n43_));
  and2   g26(.a(x9), .b(x5), .O(new_n44_));
  nor2   g27(.a(x1), .b(x0), .O(new_n45_));
  nand2  g28(.a(new_n25_), .b(x7), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n45_), .c(x2), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n30_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n43_), .O(z5));
  nand3  g33(.a(x8), .b(x4), .c(x3), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n26_), .O(new_n52_));
  nand2  g35(.a(new_n27_), .b(x6), .O(new_n53_));
  aoi21  g36(.a(new_n52_), .b(new_n37_), .c(new_n53_), .O(new_n54_));
  nand2  g37(.a(new_n25_), .b(new_n19_), .O(new_n55_));
  nand2  g38(.a(new_n45_), .b(new_n32_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(x7), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n44_), .c(new_n55_), .O(new_n58_));
  oai21  g41(.a(new_n54_), .b(new_n18_), .c(new_n58_), .O(z6));
endmodule


