// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:43 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  nor2   g02(.a(new_n18_), .b(x5), .O(new_n20_));
  nand2  g03(.a(x9), .b(x7), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  nand2  g05(.a(new_n18_), .b(new_n22_), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(new_n21_), .c(x8), .O(new_n24_));
  nor2   g07(.a(new_n24_), .b(new_n20_), .O(z1));
  oai22  g08(.a(new_n23_), .b(x8), .c(new_n18_), .d(x5), .O(z2));
  inv1   g09(.a(x0), .O(new_n27_));
  inv1   g10(.a(x1), .O(new_n28_));
  inv1   g11(.a(x2), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n30_));
  nand2  g13(.a(x8), .b(new_n22_), .O(new_n31_));
  nor3   g14(.a(new_n31_), .b(new_n30_), .c(x9), .O(new_n32_));
  nor2   g15(.a(new_n32_), .b(new_n20_), .O(z3));
  inv1   g16(.a(x5), .O(new_n34_));
  oai21  g17(.a(x8), .b(new_n34_), .c(x9), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(x7), .c(x6), .O(z4));
  inv1   g19(.a(x6), .O(new_n37_));
  nor2   g20(.a(x8), .b(new_n22_), .O(new_n38_));
  nand3  g21(.a(x2), .b(new_n28_), .c(new_n27_), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(new_n38_), .c(new_n31_), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(x9), .c(new_n37_), .O(new_n41_));
  inv1   g24(.a(x4), .O(new_n42_));
  nand3  g25(.a(x7), .b(new_n42_), .c(x3), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(x8), .c(x6), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n18_), .O(new_n45_));
  oai21  g28(.a(new_n41_), .b(new_n34_), .c(new_n45_), .O(z5));
  nand4  g29(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n47_));
  oai21  g30(.a(new_n30_), .b(new_n18_), .c(new_n47_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x7), .O(new_n49_));
  oai21  g32(.a(new_n31_), .b(new_n18_), .c(x6), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x5), .O(new_n51_));
  nand2  g34(.a(new_n18_), .b(new_n37_), .O(new_n52_));
  nand4  g35(.a(new_n52_), .b(new_n51_), .c(new_n49_), .d(z1), .O(z6));
endmodule


