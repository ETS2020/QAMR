// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  nand2  g00(.a(x9), .b(x4), .O(new_n18_));
  inv1   g01(.a(new_n18_), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  nor2   g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x7), .c(new_n19_), .O(z0));
  inv1   g05(.a(x9), .O(new_n23_));
  oai21  g06(.a(x8), .b(x7), .c(new_n23_), .O(new_n24_));
  inv1   g07(.a(x4), .O(new_n25_));
  nand2  g08(.a(x9), .b(new_n25_), .O(new_n26_));
  inv1   g09(.a(x7), .O(new_n27_));
  nor2   g10(.a(x8), .b(new_n27_), .O(new_n28_));
  oai21  g11(.a(new_n28_), .b(new_n26_), .c(new_n24_), .O(z1));
  and2   g12(.a(new_n26_), .b(new_n24_), .O(z2));
  inv1   g13(.a(x0), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  inv1   g15(.a(x2), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(new_n34_));
  nand2  g17(.a(x8), .b(new_n27_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(new_n34_), .c(new_n23_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n26_), .O(z3));
  nand2  g20(.a(x9), .b(x8), .O(new_n38_));
  inv1   g21(.a(x6), .O(new_n39_));
  nor2   g22(.a(new_n27_), .b(new_n39_), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(new_n38_), .c(new_n19_), .O(z4));
  nand3  g24(.a(x2), .b(new_n32_), .c(new_n31_), .O(new_n42_));
  nand3  g25(.a(new_n23_), .b(new_n25_), .c(x3), .O(new_n43_));
  oai21  g26(.a(new_n42_), .b(new_n38_), .c(new_n43_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x7), .O(new_n45_));
  nand3  g28(.a(x9), .b(x8), .c(x5), .O(new_n46_));
  oai21  g29(.a(new_n42_), .b(x8), .c(new_n46_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n27_), .O(new_n48_));
  nand2  g31(.a(new_n18_), .b(x6), .O(new_n49_));
  aoi21  g32(.a(new_n23_), .b(new_n20_), .c(new_n49_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n48_), .c(new_n45_), .O(z5));
  aoi21  g34(.a(new_n34_), .b(x8), .c(new_n26_), .O(new_n52_));
  nand4  g35(.a(new_n23_), .b(x8), .c(x4), .d(x3), .O(new_n53_));
  inv1   g36(.a(new_n53_), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n52_), .c(x7), .O(new_n55_));
  oai22  g38(.a(new_n46_), .b(x4), .c(x9), .d(x8), .O(new_n56_));
  aoi22  g39(.a(new_n56_), .b(new_n27_), .c(new_n18_), .d(new_n39_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n55_), .O(z6));
endmodule


