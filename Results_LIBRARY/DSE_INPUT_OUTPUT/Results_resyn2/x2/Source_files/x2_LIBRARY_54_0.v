// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x8), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x7), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x3), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n20_), .O(z0));
  inv1   g06(.a(x8), .O(new_n24_));
  oai21  g07(.a(x9), .b(x8), .c(x3), .O(new_n25_));
  nand2  g08(.a(x9), .b(x7), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(new_n27_));
  aoi22  g10(.a(new_n27_), .b(new_n24_), .c(new_n25_), .d(new_n21_), .O(z1));
  nand4  g11(.a(new_n18_), .b(new_n24_), .c(new_n21_), .d(x3), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(z2));
  inv1   g13(.a(x2), .O(new_n31_));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n19_), .c(x3), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n21_), .O(z3));
  nand2  g18(.a(new_n27_), .b(x8), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  nand2  g20(.a(x7), .b(new_n37_), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n36_), .c(new_n22_), .O(z4));
  nand3  g22(.a(x9), .b(x8), .c(x5), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x3), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n21_), .c(new_n37_), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x8), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nand2  g29(.a(new_n26_), .b(x8), .O(new_n47_));
  nand2  g30(.a(new_n24_), .b(x7), .O(new_n48_));
  nand4  g31(.a(new_n48_), .b(new_n47_), .c(new_n32_), .d(x2), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n46_), .c(new_n42_), .O(z5));
  nand2  g33(.a(x9), .b(x5), .O(new_n51_));
  nand3  g34(.a(x7), .b(x4), .c(x3), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n26_), .c(x8), .O(new_n54_));
  nor3   g37(.a(x2), .b(x1), .c(x0), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n24_), .c(new_n27_), .O(new_n56_));
  aoi21  g39(.a(new_n25_), .b(new_n21_), .c(new_n37_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n56_), .c(new_n54_), .O(z6));
endmodule


