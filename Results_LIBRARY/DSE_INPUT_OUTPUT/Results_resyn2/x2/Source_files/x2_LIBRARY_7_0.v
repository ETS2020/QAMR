// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  nor2   g00(.a(x6), .b(x1), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  nor2   g02(.a(x9), .b(new_n19_), .O(new_n20_));
  aoi21  g03(.a(new_n20_), .b(x7), .c(new_n18_), .O(z0));
  inv1   g04(.a(new_n18_), .O(new_n22_));
  xnor2a g05(.a(x9), .b(x7), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n19_), .O(z1));
  inv1   g07(.a(x7), .O(new_n25_));
  inv1   g08(.a(x9), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n19_), .c(new_n25_), .O(new_n27_));
  nor2   g10(.a(new_n27_), .b(new_n18_), .O(z2));
  inv1   g11(.a(x1), .O(new_n29_));
  nor2   g12(.a(x2), .b(x0), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n20_), .c(new_n25_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x6), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n29_), .O(z3));
  nand2  g16(.a(x9), .b(x8), .O(new_n34_));
  inv1   g17(.a(x6), .O(new_n35_));
  nor2   g18(.a(new_n25_), .b(new_n35_), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(new_n34_), .c(new_n18_), .O(z4));
  nand2  g20(.a(new_n35_), .b(x1), .O(new_n38_));
  nand2  g21(.a(new_n19_), .b(new_n25_), .O(new_n39_));
  oai21  g22(.a(new_n34_), .b(new_n25_), .c(new_n39_), .O(new_n40_));
  nor2   g23(.a(x1), .b(x0), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n40_), .c(x2), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x8), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n26_), .O(new_n46_));
  nand4  g29(.a(x9), .b(x8), .c(new_n25_), .d(x5), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n46_), .c(new_n42_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x6), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n38_), .O(z5));
  aoi21  g33(.a(new_n30_), .b(new_n29_), .c(new_n19_), .O(new_n51_));
  nand4  g34(.a(new_n26_), .b(x8), .c(x4), .d(x3), .O(new_n52_));
  oai21  g35(.a(new_n51_), .b(new_n26_), .c(new_n52_), .O(new_n53_));
  nand2  g36(.a(new_n47_), .b(new_n27_), .O(new_n54_));
  aoi21  g37(.a(new_n53_), .b(x7), .c(new_n54_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n35_), .c(new_n38_), .O(z6));
endmodule


