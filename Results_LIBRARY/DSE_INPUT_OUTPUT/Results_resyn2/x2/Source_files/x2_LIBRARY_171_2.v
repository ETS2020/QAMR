// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:27 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n30_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_;
  inv1   g00(.a(x7), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  nor2   g02(.a(x9), .b(new_n19_), .O(new_n20_));
  nor2   g03(.a(x7), .b(x4), .O(new_n21_));
  inv1   g04(.a(new_n21_), .O(new_n22_));
  oai21  g05(.a(new_n20_), .b(new_n18_), .c(new_n22_), .O(z0));
  inv1   g06(.a(x9), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(x8), .c(x7), .O(new_n25_));
  nor2   g08(.a(x9), .b(x8), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(new_n22_), .c(new_n25_), .O(z1));
  nand2  g10(.a(new_n26_), .b(new_n21_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z2));
  nor3   g12(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n21_), .c(new_n20_), .O(z3));
  nand3  g14(.a(x9), .b(x8), .c(x7), .O(new_n32_));
  inv1   g15(.a(x6), .O(new_n33_));
  nand2  g16(.a(x7), .b(new_n33_), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n32_), .c(new_n22_), .O(z4));
  nor2   g18(.a(x1), .b(x0), .O(new_n36_));
  oai21  g19(.a(x8), .b(x7), .c(new_n32_), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(new_n36_), .c(x2), .O(new_n38_));
  inv1   g21(.a(x4), .O(new_n39_));
  nand2  g22(.a(x8), .b(x5), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n24_), .c(new_n39_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n18_), .O(new_n42_));
  nand3  g25(.a(x7), .b(new_n39_), .c(x3), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x8), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n24_), .O(new_n45_));
  nand4  g28(.a(new_n45_), .b(new_n42_), .c(new_n38_), .d(x6), .O(z5));
  nand3  g29(.a(new_n24_), .b(x8), .c(x3), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x7), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x4), .O(new_n49_));
  nand2  g32(.a(x8), .b(x7), .O(new_n50_));
  aoi21  g33(.a(new_n40_), .b(new_n18_), .c(new_n24_), .O(new_n51_));
  oai21  g34(.a(new_n50_), .b(new_n30_), .c(new_n51_), .O(new_n52_));
  aoi21  g35(.a(new_n26_), .b(new_n18_), .c(new_n33_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n52_), .c(new_n49_), .O(z6));
endmodule


