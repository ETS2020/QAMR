// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  nor2   g00(.a(x9), .b(x7), .O(new_n18_));
  inv1   g01(.a(new_n18_), .O(new_n19_));
  nand2  g02(.a(x9), .b(x4), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  inv1   g04(.a(x9), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n20_), .c(new_n19_), .O(z0));
  oai21  g07(.a(x8), .b(x7), .c(new_n22_), .O(new_n25_));
  inv1   g08(.a(x7), .O(new_n26_));
  inv1   g09(.a(new_n20_), .O(new_n27_));
  oai21  g10(.a(x8), .b(new_n26_), .c(new_n27_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n25_), .O(z1));
  and2   g12(.a(new_n25_), .b(new_n20_), .O(z2));
  inv1   g13(.a(x0), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  inv1   g15(.a(x2), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n21_), .c(new_n18_), .O(new_n35_));
  inv1   g18(.a(x4), .O(new_n36_));
  nand2  g19(.a(x9), .b(new_n36_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x7), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n35_), .c(new_n28_), .O(z3));
  inv1   g22(.a(x6), .O(new_n40_));
  nand3  g23(.a(new_n37_), .b(x7), .c(new_n40_), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n28_), .c(new_n19_), .O(z4));
  nand3  g25(.a(x9), .b(x8), .c(x7), .O(new_n43_));
  oai21  g26(.a(x8), .b(x7), .c(new_n43_), .O(new_n44_));
  nand4  g27(.a(new_n44_), .b(x2), .c(new_n32_), .d(new_n31_), .O(new_n45_));
  inv1   g28(.a(x3), .O(new_n46_));
  oai21  g29(.a(new_n26_), .b(new_n46_), .c(new_n22_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n36_), .O(new_n48_));
  inv1   g31(.a(x5), .O(new_n49_));
  nor2   g32(.a(new_n21_), .b(new_n49_), .O(new_n50_));
  nor2   g33(.a(new_n22_), .b(x7), .O(new_n51_));
  nand2  g34(.a(new_n23_), .b(x6), .O(new_n52_));
  aoi21  g35(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n48_), .c(new_n45_), .O(z5));
  aoi21  g37(.a(new_n34_), .b(x8), .c(new_n22_), .O(new_n55_));
  nor3   g38(.a(x9), .b(new_n21_), .c(new_n46_), .O(new_n56_));
  nor2   g39(.a(new_n26_), .b(new_n36_), .O(new_n57_));
  oai21  g40(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  aoi21  g41(.a(x9), .b(new_n36_), .c(x6), .O(new_n59_));
  oai21  g42(.a(new_n21_), .b(new_n49_), .c(new_n23_), .O(new_n60_));
  aoi21  g43(.a(new_n23_), .b(new_n20_), .c(x7), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n58_), .O(z6));
endmodule


