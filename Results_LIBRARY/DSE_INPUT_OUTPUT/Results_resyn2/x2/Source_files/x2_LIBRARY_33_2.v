// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_;
  inv1   g00(.a(x8), .O(new_n19_));
  inv1   g01(.a(x9), .O(new_n20_));
  nand2  g02(.a(new_n20_), .b(x7), .O(new_n21_));
  inv1   g03(.a(x7), .O(new_n22_));
  nand2  g04(.a(x9), .b(new_n22_), .O(new_n23_));
  nand3  g05(.a(new_n23_), .b(new_n21_), .c(new_n19_), .O(z1));
  nor3   g06(.a(x9), .b(x8), .c(x7), .O(z2));
  inv1   g07(.a(x0), .O(new_n26_));
  inv1   g08(.a(x1), .O(new_n27_));
  inv1   g09(.a(x2), .O(new_n28_));
  nand3  g10(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  oai21  g11(.a(new_n29_), .b(new_n19_), .c(new_n22_), .O(new_n30_));
  nand2  g12(.a(new_n30_), .b(new_n20_), .O(z3));
  inv1   g13(.a(x6), .O(new_n32_));
  oai21  g14(.a(x8), .b(new_n32_), .c(x9), .O(new_n33_));
  nand2  g15(.a(new_n33_), .b(x7), .O(z4));
  nor2   g16(.a(x1), .b(x0), .O(new_n35_));
  nand3  g17(.a(x9), .b(x8), .c(x7), .O(new_n36_));
  oai21  g18(.a(x8), .b(x7), .c(new_n36_), .O(new_n37_));
  nand3  g19(.a(new_n37_), .b(new_n35_), .c(x2), .O(new_n38_));
  nand2  g20(.a(new_n21_), .b(new_n32_), .O(new_n39_));
  nand2  g21(.a(x8), .b(x5), .O(new_n40_));
  nand2  g22(.a(new_n40_), .b(x9), .O(new_n41_));
  nand2  g23(.a(new_n20_), .b(x8), .O(new_n42_));
  nand3  g24(.a(new_n42_), .b(new_n41_), .c(new_n22_), .O(new_n43_));
  nand3  g25(.a(new_n43_), .b(new_n39_), .c(new_n38_), .O(z5));
  and2   g26(.a(x8), .b(x7), .O(new_n45_));
  nand2  g27(.a(new_n45_), .b(new_n29_), .O(new_n46_));
  aoi21  g28(.a(new_n40_), .b(new_n22_), .c(new_n20_), .O(new_n47_));
  nand2  g29(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  aoi21  g30(.a(new_n21_), .b(new_n32_), .c(z2), .O(new_n49_));
  nand2  g31(.a(new_n49_), .b(new_n48_), .O(z6));
  one    g32(.O(z0));
endmodule


