// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n26_,
    new_n28_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(x8), .b(x7), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x0), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n18_), .O(z0));
  inv1   g04(.a(x7), .O(new_n22_));
  inv1   g05(.a(x8), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(x0), .c(x9), .O(z2));
  nor2   g08(.a(x8), .b(new_n22_), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(x9), .c(z2), .O(z1));
  inv1   g10(.a(x0), .O(new_n28_));
  nand2  g11(.a(new_n18_), .b(new_n28_), .O(z3));
  nand2  g12(.a(x9), .b(x8), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(z3), .c(x7), .d(x6), .O(z4));
  inv1   g14(.a(x1), .O(new_n32_));
  nand3  g15(.a(x2), .b(new_n32_), .c(new_n28_), .O(new_n33_));
  aoi21  g16(.a(new_n24_), .b(new_n19_), .c(new_n33_), .O(new_n34_));
  nand3  g17(.a(x8), .b(new_n22_), .c(x5), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x6), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n34_), .c(x9), .O(new_n37_));
  inv1   g20(.a(x6), .O(new_n38_));
  inv1   g21(.a(x4), .O(new_n39_));
  nand3  g22(.a(x7), .b(new_n39_), .c(x3), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(x8), .c(x9), .O(new_n41_));
  oai21  g24(.a(new_n41_), .b(new_n38_), .c(x0), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n37_), .O(z5));
  inv1   g26(.a(x2), .O(new_n44_));
  nand3  g27(.a(x7), .b(new_n44_), .c(new_n32_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x9), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n28_), .c(new_n38_), .O(new_n47_));
  inv1   g30(.a(x5), .O(new_n48_));
  nand2  g31(.a(new_n18_), .b(new_n23_), .O(new_n49_));
  oai21  g32(.a(new_n30_), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n22_), .O(new_n51_));
  nand2  g34(.a(x4), .b(x3), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(x9), .c(x8), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n49_), .c(x7), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n51_), .c(new_n47_), .O(z6));
endmodule


