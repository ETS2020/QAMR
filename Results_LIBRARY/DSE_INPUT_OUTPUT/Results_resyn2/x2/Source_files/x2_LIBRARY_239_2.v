// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:43 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  inv1   g00(.a(x9), .O(new_n18_));
  inv1   g01(.a(x3), .O(new_n19_));
  nand2  g02(.a(x8), .b(x7), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n18_), .O(z0));
  inv1   g05(.a(x7), .O(new_n23_));
  inv1   g06(.a(x8), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  aoi21  g08(.a(new_n25_), .b(new_n19_), .c(x9), .O(new_n26_));
  nor2   g09(.a(new_n18_), .b(x8), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(x7), .c(new_n26_), .O(z1));
  nor2   g11(.a(x9), .b(x3), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(new_n24_), .c(new_n23_), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(z2));
  inv1   g14(.a(x0), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  inv1   g16(.a(x2), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  nand3  g18(.a(new_n29_), .b(x8), .c(new_n23_), .O(new_n36_));
  or2    g19(.a(new_n36_), .b(new_n35_), .O(z3));
  inv1   g20(.a(x6), .O(new_n38_));
  nor2   g21(.a(new_n23_), .b(new_n38_), .O(new_n39_));
  oai21  g22(.a(new_n29_), .b(new_n27_), .c(new_n39_), .O(z4));
  nand3  g23(.a(x2), .b(new_n33_), .c(new_n32_), .O(new_n41_));
  aoi21  g24(.a(new_n25_), .b(new_n20_), .c(new_n41_), .O(new_n42_));
  nand3  g25(.a(x8), .b(new_n23_), .c(x5), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x6), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n42_), .c(x9), .O(new_n45_));
  oai21  g28(.a(x9), .b(x8), .c(x6), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n19_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n45_), .O(z5));
  inv1   g31(.a(new_n43_), .O(new_n49_));
  aoi21  g32(.a(new_n35_), .b(x8), .c(new_n23_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n49_), .c(x9), .O(new_n51_));
  nor2   g34(.a(new_n26_), .b(new_n38_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n51_), .O(z6));
endmodule


