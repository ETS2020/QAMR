// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_;
  inv1   g00(.a(x7), .O(new_n18_));
  nor2   g01(.a(x9), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(new_n19_), .O(z0));
  nor2   g03(.a(x9), .b(x8), .O(new_n21_));
  nand2  g04(.a(x9), .b(x8), .O(new_n22_));
  oai21  g05(.a(new_n21_), .b(x7), .c(new_n22_), .O(z1));
  aoi21  g06(.a(x8), .b(new_n18_), .c(x9), .O(z2));
  inv1   g07(.a(x9), .O(new_n25_));
  inv1   g08(.a(x8), .O(new_n26_));
  inv1   g09(.a(x0), .O(new_n27_));
  inv1   g10(.a(x1), .O(new_n28_));
  inv1   g11(.a(x2), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n30_));
  oai21  g13(.a(new_n30_), .b(new_n26_), .c(new_n18_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n25_), .O(z3));
  inv1   g15(.a(x6), .O(new_n33_));
  oai21  g16(.a(x8), .b(new_n33_), .c(x9), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x7), .O(z4));
  nand3  g18(.a(x9), .b(x8), .c(x5), .O(new_n36_));
  oai21  g19(.a(x9), .b(x8), .c(new_n36_), .O(new_n37_));
  aoi21  g20(.a(new_n37_), .b(new_n18_), .c(new_n33_), .O(new_n38_));
  nand2  g21(.a(new_n22_), .b(x7), .O(new_n39_));
  aoi21  g22(.a(x8), .b(new_n18_), .c(new_n29_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(new_n28_), .d(new_n27_), .O(new_n41_));
  oai21  g24(.a(new_n38_), .b(new_n19_), .c(new_n41_), .O(z5));
  aoi21  g25(.a(new_n30_), .b(x8), .c(new_n18_), .O(new_n43_));
  nand3  g26(.a(x8), .b(new_n18_), .c(x5), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n43_), .c(x9), .O(new_n46_));
  oai21  g29(.a(new_n21_), .b(new_n33_), .c(z0), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n46_), .O(z6));
endmodule


