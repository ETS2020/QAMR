// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x0), .O(new_n18_));
  nand2  g01(.a(x9), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(new_n19_), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nor2   g04(.a(x9), .b(new_n21_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x7), .c(new_n20_), .O(z0));
  inv1   g06(.a(x9), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n18_), .c(x7), .O(new_n25_));
  inv1   g08(.a(x7), .O(new_n26_));
  aoi21  g09(.a(x9), .b(new_n26_), .c(x8), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n25_), .O(z1));
  nand3  g11(.a(new_n24_), .b(new_n21_), .c(new_n26_), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(z2));
  nor2   g13(.a(x1), .b(x0), .O(new_n31_));
  nor2   g14(.a(x7), .b(x2), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(new_n31_), .c(new_n22_), .O(z3));
  aoi21  g16(.a(x9), .b(x8), .c(new_n26_), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(x6), .c(new_n20_), .O(z4));
  nand4  g18(.a(x9), .b(x8), .c(new_n26_), .d(x5), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x6), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x0), .O(new_n38_));
  inv1   g21(.a(x4), .O(new_n39_));
  nand3  g22(.a(x7), .b(new_n39_), .c(x3), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(x8), .c(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n24_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n38_), .O(z5));
  nand3  g26(.a(x8), .b(x4), .c(x3), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n24_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  inv1   g29(.a(x6), .O(new_n47_));
  nand2  g30(.a(new_n24_), .b(new_n47_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n29_), .c(new_n19_), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n46_), .c(new_n38_), .O(z6));
endmodule


