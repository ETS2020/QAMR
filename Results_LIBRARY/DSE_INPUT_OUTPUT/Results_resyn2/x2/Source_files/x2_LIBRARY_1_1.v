// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  nand2  g00(.a(x9), .b(x2), .O(new_n18_));
  nor2   g01(.a(x9), .b(x8), .O(new_n19_));
  nor2   g02(.a(x9), .b(x7), .O(new_n20_));
  nor2   g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n18_), .O(z0));
  inv1   g05(.a(x8), .O(new_n23_));
  nand3  g06(.a(x9), .b(x7), .c(x2), .O(new_n24_));
  inv1   g07(.a(new_n24_), .O(new_n25_));
  oai21  g08(.a(new_n25_), .b(new_n20_), .c(new_n23_), .O(z1));
  inv1   g09(.a(x9), .O(new_n27_));
  inv1   g10(.a(x7), .O(new_n28_));
  nand2  g11(.a(new_n19_), .b(new_n28_), .O(new_n29_));
  oai21  g12(.a(new_n27_), .b(x2), .c(new_n29_), .O(z2));
  nor2   g13(.a(x1), .b(x0), .O(new_n31_));
  nor2   g14(.a(x9), .b(x2), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n31_), .c(x8), .d(new_n28_), .O(z3));
  nor2   g16(.a(new_n27_), .b(x2), .O(new_n34_));
  aoi21  g17(.a(x9), .b(x8), .c(new_n28_), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(x6), .c(new_n34_), .O(z4));
  inv1   g19(.a(x4), .O(new_n37_));
  nand3  g20(.a(x7), .b(new_n37_), .c(x3), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x8), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n27_), .O(new_n40_));
  nand3  g23(.a(x8), .b(new_n28_), .c(x5), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x2), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x9), .O(new_n43_));
  nand2  g26(.a(x8), .b(new_n28_), .O(new_n44_));
  oai21  g27(.a(new_n27_), .b(new_n23_), .c(x7), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n31_), .c(new_n44_), .O(new_n46_));
  nand4  g29(.a(new_n46_), .b(new_n43_), .c(new_n40_), .d(x6), .O(z5));
  nand3  g30(.a(x8), .b(x4), .c(x3), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n27_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n35_), .O(new_n50_));
  nand3  g33(.a(x9), .b(x8), .c(x5), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n19_), .c(new_n28_), .O(new_n53_));
  inv1   g36(.a(x6), .O(new_n54_));
  nor2   g37(.a(new_n34_), .b(new_n54_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n53_), .c(new_n50_), .O(z6));
endmodule


