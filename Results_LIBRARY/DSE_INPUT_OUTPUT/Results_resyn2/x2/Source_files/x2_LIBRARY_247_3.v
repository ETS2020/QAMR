// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x3), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  oai21  g02(.a(x9), .b(new_n19_), .c(new_n18_), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x7), .O(z0));
  nand2  g04(.a(x7), .b(new_n18_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x9), .O(new_n23_));
  inv1   g06(.a(x9), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(x7), .c(x8), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(z1));
  nand2  g09(.a(x7), .b(x3), .O(new_n27_));
  inv1   g10(.a(x7), .O(new_n28_));
  nor2   g11(.a(x9), .b(x8), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n27_), .O(z2));
  inv1   g14(.a(x2), .O(new_n32_));
  nor2   g15(.a(x1), .b(x0), .O(new_n33_));
  nor2   g16(.a(new_n19_), .b(x7), .O(new_n34_));
  nand4  g17(.a(new_n34_), .b(new_n33_), .c(new_n24_), .d(new_n32_), .O(z3));
  nand2  g18(.a(x9), .b(x8), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x6), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n18_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x7), .O(z4));
  inv1   g22(.a(new_n34_), .O(new_n40_));
  nand2  g23(.a(new_n36_), .b(x7), .O(new_n41_));
  aoi21  g24(.a(x7), .b(x3), .c(new_n32_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(new_n41_), .c(new_n40_), .d(new_n33_), .O(new_n43_));
  nand3  g26(.a(x8), .b(new_n28_), .c(x5), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x9), .O(new_n46_));
  aoi21  g29(.a(x7), .b(x3), .c(x6), .O(new_n47_));
  aoi21  g30(.a(new_n29_), .b(new_n27_), .c(new_n47_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(z5));
  inv1   g32(.a(x0), .O(new_n50_));
  inv1   g33(.a(x1), .O(new_n51_));
  nand3  g34(.a(new_n32_), .b(new_n51_), .c(new_n50_), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(x8), .c(new_n22_), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n45_), .c(x9), .O(new_n54_));
  aoi21  g37(.a(new_n29_), .b(new_n28_), .c(new_n47_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n54_), .O(z6));
endmodule


