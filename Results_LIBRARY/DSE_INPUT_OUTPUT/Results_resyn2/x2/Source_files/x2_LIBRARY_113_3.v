// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  nand2  g00(.a(x6), .b(x3), .O(new_n18_));
  inv1   g01(.a(new_n18_), .O(new_n19_));
  inv1   g02(.a(x7), .O(new_n20_));
  nor2   g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x8), .c(new_n19_), .O(z0));
  nand2  g05(.a(x9), .b(new_n20_), .O(new_n23_));
  nor2   g06(.a(new_n21_), .b(x8), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(new_n23_), .c(new_n19_), .O(z1));
  nor3   g08(.a(x9), .b(x8), .c(x7), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n18_), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(z2));
  nor2   g11(.a(x9), .b(x2), .O(new_n29_));
  nand2  g12(.a(x8), .b(new_n20_), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(new_n31_));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n31_), .c(new_n29_), .d(new_n18_), .O(z3));
  nand2  g16(.a(x9), .b(x8), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x7), .O(new_n35_));
  inv1   g18(.a(new_n35_), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(x3), .c(x6), .O(z4));
  nand4  g20(.a(new_n35_), .b(new_n32_), .c(new_n30_), .d(x2), .O(new_n38_));
  nand3  g21(.a(x8), .b(new_n20_), .c(x5), .O(new_n39_));
  inv1   g22(.a(new_n39_), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x9), .O(new_n41_));
  inv1   g24(.a(x6), .O(new_n42_));
  nor2   g25(.a(x9), .b(x8), .O(new_n43_));
  nor3   g26(.a(new_n43_), .b(new_n42_), .c(x3), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n41_), .c(new_n38_), .O(z5));
  inv1   g28(.a(x2), .O(new_n46_));
  inv1   g29(.a(x8), .O(new_n47_));
  aoi21  g30(.a(new_n32_), .b(new_n46_), .c(new_n47_), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n20_), .c(new_n39_), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(x9), .c(new_n26_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(x3), .c(x6), .O(z6));
endmodule


