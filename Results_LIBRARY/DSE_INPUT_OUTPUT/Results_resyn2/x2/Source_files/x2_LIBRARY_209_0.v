// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x7), .O(new_n19_));
  nor2   g02(.a(x9), .b(new_n19_), .O(new_n20_));
  aoi22  g03(.a(new_n20_), .b(x8), .c(new_n19_), .d(new_n18_), .O(z0));
  oai21  g04(.a(x9), .b(new_n18_), .c(new_n19_), .O(new_n22_));
  nor2   g05(.a(new_n20_), .b(x8), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(z1));
  inv1   g07(.a(x8), .O(new_n25_));
  inv1   g08(.a(x9), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(x4), .c(x7), .O(z2));
  inv1   g11(.a(x2), .O(new_n29_));
  nor2   g12(.a(x1), .b(x0), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g14(.a(new_n26_), .b(x8), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n31_), .c(x4), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n19_), .O(z3));
  nand2  g17(.a(x9), .b(x8), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(x7), .c(x6), .O(z4));
  nand3  g19(.a(x9), .b(x8), .c(x5), .O(new_n37_));
  aoi21  g20(.a(new_n37_), .b(new_n27_), .c(x7), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x4), .O(new_n39_));
  oai21  g22(.a(new_n26_), .b(new_n19_), .c(x8), .O(new_n40_));
  oai21  g23(.a(x7), .b(new_n18_), .c(new_n25_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n40_), .c(new_n30_), .d(x2), .O(new_n42_));
  inv1   g25(.a(x3), .O(new_n43_));
  oai21  g26(.a(x4), .b(new_n43_), .c(x8), .O(new_n44_));
  aoi21  g27(.a(new_n19_), .b(new_n18_), .c(x6), .O(new_n45_));
  aoi21  g28(.a(new_n44_), .b(new_n20_), .c(new_n45_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n42_), .c(new_n39_), .O(z5));
  nand4  g30(.a(new_n26_), .b(x8), .c(x7), .d(x3), .O(new_n48_));
  inv1   g31(.a(new_n48_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n38_), .c(x4), .O(new_n50_));
  nor2   g33(.a(new_n26_), .b(new_n19_), .O(new_n51_));
  nand2  g34(.a(new_n31_), .b(x8), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n51_), .c(new_n45_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n50_), .O(z6));
endmodule


