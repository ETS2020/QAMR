// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:58 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n27_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  nor2   g02(.a(x9), .b(x7), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x7), .O(new_n22_));
  inv1   g05(.a(new_n22_), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(x9), .c(new_n20_), .O(z1));
  inv1   g07(.a(new_n20_), .O(z3));
  nand2  g08(.a(x9), .b(x8), .O(new_n27_));
  nand3  g09(.a(new_n27_), .b(x7), .c(x6), .O(z4));
  inv1   g10(.a(x7), .O(new_n29_));
  oai21  g11(.a(new_n18_), .b(new_n29_), .c(x8), .O(new_n30_));
  nor2   g12(.a(x1), .b(x0), .O(new_n31_));
  nand4  g13(.a(new_n31_), .b(new_n30_), .c(new_n22_), .d(x2), .O(new_n32_));
  inv1   g14(.a(x3), .O(new_n33_));
  oai21  g15(.a(x4), .b(new_n33_), .c(x8), .O(new_n34_));
  nand2  g16(.a(new_n34_), .b(new_n18_), .O(new_n35_));
  inv1   g17(.a(x6), .O(new_n36_));
  nand2  g18(.a(x8), .b(x5), .O(new_n37_));
  nand2  g19(.a(new_n37_), .b(x9), .O(new_n38_));
  aoi21  g20(.a(new_n38_), .b(new_n29_), .c(new_n36_), .O(new_n39_));
  nand3  g21(.a(new_n39_), .b(new_n35_), .c(new_n32_), .O(z5));
  nor2   g22(.a(new_n18_), .b(new_n29_), .O(new_n41_));
  nor3   g23(.a(x2), .b(x1), .c(x0), .O(new_n42_));
  oai21  g24(.a(new_n42_), .b(new_n21_), .c(new_n41_), .O(new_n43_));
  nand4  g25(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n44_));
  nand3  g26(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(z6));
  zero   g27(.O(z2));
endmodule


