// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  nand2  g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n18_), .b(x7), .c(x8), .O(new_n22_));
  aoi22  g05(.a(new_n22_), .b(new_n21_), .c(x9), .d(x0), .O(z1));
  nand2  g06(.a(x9), .b(x0), .O(new_n24_));
  inv1   g07(.a(x8), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n20_), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(x9), .c(new_n24_), .O(z2));
  nor2   g10(.a(x2), .b(x1), .O(new_n28_));
  nor2   g11(.a(x7), .b(x0), .O(new_n29_));
  nand4  g12(.a(new_n29_), .b(new_n28_), .c(new_n18_), .d(x8), .O(new_n30_));
  and2   g13(.a(new_n30_), .b(new_n24_), .O(z3));
  oai21  g14(.a(x8), .b(x0), .c(x9), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(x7), .c(x6), .O(z4));
  inv1   g16(.a(x0), .O(new_n34_));
  nand3  g17(.a(x9), .b(x8), .c(x7), .O(new_n35_));
  inv1   g18(.a(x1), .O(new_n36_));
  nand2  g19(.a(x2), .b(new_n36_), .O(new_n37_));
  aoi21  g20(.a(new_n35_), .b(new_n26_), .c(new_n37_), .O(new_n38_));
  nand2  g21(.a(x8), .b(x5), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(new_n21_), .c(x6), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n38_), .c(new_n34_), .O(new_n41_));
  inv1   g24(.a(x4), .O(new_n42_));
  nand3  g25(.a(x7), .b(new_n42_), .c(x3), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(x8), .c(x6), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n18_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n41_), .O(z5));
  inv1   g29(.a(x6), .O(new_n47_));
  nand2  g30(.a(new_n18_), .b(new_n47_), .O(new_n48_));
  nand2  g31(.a(new_n40_), .b(new_n34_), .O(new_n49_));
  nand2  g32(.a(new_n28_), .b(x9), .O(new_n50_));
  nand4  g33(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x7), .O(new_n53_));
  nand4  g36(.a(new_n53_), .b(new_n49_), .c(new_n48_), .d(z1), .O(z6));
endmodule


