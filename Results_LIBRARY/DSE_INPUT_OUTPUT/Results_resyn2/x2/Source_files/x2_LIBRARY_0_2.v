// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x1), .O(new_n18_));
  nor2   g01(.a(x5), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  nor2   g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x7), .c(new_n19_), .O(z0));
  inv1   g05(.a(new_n19_), .O(new_n23_));
  xnor2a g06(.a(x9), .b(x7), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n20_), .O(z1));
  inv1   g08(.a(x7), .O(new_n26_));
  inv1   g09(.a(x9), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n20_), .c(new_n26_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n23_), .O(z2));
  nor2   g12(.a(x2), .b(x0), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n21_), .c(new_n26_), .d(new_n18_), .O(new_n31_));
  and2   g14(.a(new_n31_), .b(new_n23_), .O(z3));
  oai21  g15(.a(new_n27_), .b(new_n20_), .c(x7), .O(new_n33_));
  inv1   g16(.a(new_n33_), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(x6), .c(new_n19_), .O(z4));
  inv1   g18(.a(x4), .O(new_n36_));
  nand3  g19(.a(x7), .b(new_n36_), .c(x3), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x8), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n27_), .O(new_n39_));
  inv1   g22(.a(x0), .O(new_n40_));
  nand2  g23(.a(x8), .b(new_n26_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n33_), .c(x2), .d(new_n40_), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(new_n39_), .c(x6), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n18_), .O(new_n44_));
  nand3  g27(.a(x9), .b(x8), .c(new_n26_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n39_), .c(x6), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x5), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n44_), .O(z5));
  inv1   g31(.a(x5), .O(new_n49_));
  nand3  g32(.a(new_n30_), .b(x7), .c(new_n18_), .O(new_n50_));
  oai21  g33(.a(new_n41_), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x9), .O(new_n52_));
  nand3  g35(.a(x8), .b(x4), .c(x3), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n27_), .O(new_n54_));
  nand2  g37(.a(new_n28_), .b(x6), .O(new_n55_));
  aoi21  g38(.a(new_n54_), .b(new_n34_), .c(new_n55_), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n19_), .c(new_n52_), .O(z6));
endmodule


