// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:31 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(x4), .c(x9), .O(new_n21_));
  aoi21  g04(.a(new_n18_), .b(x7), .c(x8), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(z1));
  nand2  g06(.a(x9), .b(x4), .O(new_n24_));
  inv1   g07(.a(x8), .O(new_n25_));
  nand2  g08(.a(new_n18_), .b(new_n25_), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(x7), .c(new_n24_), .O(z2));
  inv1   g10(.a(x2), .O(new_n28_));
  nor2   g11(.a(x1), .b(x0), .O(new_n29_));
  nor2   g12(.a(x9), .b(new_n25_), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n29_), .c(new_n20_), .d(new_n28_), .O(z3));
  oai21  g14(.a(x8), .b(x4), .c(x9), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(x7), .c(x6), .O(z4));
  inv1   g16(.a(x4), .O(new_n34_));
  nand2  g17(.a(x9), .b(x8), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x7), .O(new_n36_));
  nand2  g19(.a(x8), .b(new_n20_), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(new_n36_), .c(new_n29_), .d(x2), .O(new_n38_));
  nand4  g21(.a(x9), .b(x8), .c(new_n20_), .d(x5), .O(new_n39_));
  nand3  g22(.a(new_n18_), .b(x7), .c(x3), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n34_), .O(new_n42_));
  inv1   g25(.a(x6), .O(new_n43_));
  oai21  g26(.a(new_n25_), .b(new_n43_), .c(new_n18_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n42_), .O(z5));
  nand2  g28(.a(new_n29_), .b(new_n28_), .O(new_n46_));
  nand2  g29(.a(x9), .b(new_n34_), .O(new_n47_));
  aoi21  g30(.a(new_n46_), .b(x8), .c(new_n47_), .O(new_n48_));
  nand4  g31(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n48_), .c(x7), .O(new_n51_));
  nand2  g34(.a(x8), .b(x5), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n47_), .c(new_n26_), .O(new_n53_));
  aoi22  g36(.a(new_n53_), .b(new_n20_), .c(new_n24_), .d(new_n43_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n51_), .O(z6));
endmodule


