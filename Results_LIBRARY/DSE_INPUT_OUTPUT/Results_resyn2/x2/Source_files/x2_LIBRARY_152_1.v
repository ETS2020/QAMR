// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  nand2  g00(.a(x9), .b(x6), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  inv1   g02(.a(x9), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  nand2  g05(.a(new_n20_), .b(new_n22_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n21_), .c(new_n18_), .O(z0));
  oai21  g07(.a(new_n18_), .b(new_n22_), .c(new_n23_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n19_), .O(z1));
  nor2   g09(.a(new_n23_), .b(x8), .O(z2));
  inv1   g10(.a(x0), .O(new_n28_));
  inv1   g11(.a(x1), .O(new_n29_));
  inv1   g12(.a(x2), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n29_), .c(new_n28_), .O(new_n31_));
  nand2  g14(.a(x8), .b(new_n22_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n31_), .c(new_n20_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n18_), .O(z3));
  nand2  g17(.a(x9), .b(x8), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(x7), .c(x6), .O(z4));
  nor2   g19(.a(x1), .b(x0), .O(new_n37_));
  nand2  g20(.a(new_n35_), .b(x7), .O(new_n38_));
  nand4  g21(.a(new_n38_), .b(new_n32_), .c(new_n37_), .d(x2), .O(new_n39_));
  nand4  g22(.a(x9), .b(x8), .c(new_n22_), .d(x5), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x6), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(x8), .c(x6), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n20_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n42_), .O(z5));
  aoi21  g30(.a(new_n31_), .b(x8), .c(new_n18_), .O(new_n48_));
  nand4  g31(.a(new_n20_), .b(x8), .c(x4), .d(x3), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n48_), .c(x7), .O(new_n51_));
  nor2   g34(.a(x9), .b(x6), .O(new_n52_));
  nand2  g35(.a(x6), .b(x5), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n35_), .c(new_n21_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(new_n22_), .c(new_n52_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n51_), .O(z6));
endmodule


