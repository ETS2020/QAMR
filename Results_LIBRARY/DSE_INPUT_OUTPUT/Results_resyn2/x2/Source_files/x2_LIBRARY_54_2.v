// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x8), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x7), .O(new_n20_));
  inv1   g03(.a(x3), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n20_), .O(z0));
  oai21  g07(.a(new_n18_), .b(x8), .c(x7), .O(new_n25_));
  nor2   g08(.a(x9), .b(x8), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(new_n23_), .c(new_n25_), .O(z1));
  nand3  g10(.a(new_n26_), .b(new_n22_), .c(new_n21_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z2));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  inv1   g14(.a(x2), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n19_), .c(new_n21_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n22_), .O(z3));
  nand3  g18(.a(x9), .b(x8), .c(x7), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  nand2  g20(.a(x7), .b(new_n37_), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n36_), .c(new_n23_), .O(z4));
  inv1   g22(.a(x8), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n22_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(x2), .c(new_n31_), .d(new_n30_), .O(new_n43_));
  oai21  g26(.a(x9), .b(x4), .c(x7), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x3), .O(new_n45_));
  nand4  g28(.a(x9), .b(x8), .c(new_n22_), .d(x5), .O(new_n46_));
  nor2   g29(.a(new_n26_), .b(new_n37_), .O(new_n47_));
  nand4  g30(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n43_), .O(z5));
  aoi21  g31(.a(new_n33_), .b(x8), .c(new_n22_), .O(new_n49_));
  nand3  g32(.a(x8), .b(new_n22_), .c(x5), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n49_), .c(x9), .O(new_n52_));
  inv1   g35(.a(x4), .O(new_n53_));
  oai21  g36(.a(new_n19_), .b(new_n53_), .c(x7), .O(new_n54_));
  oai21  g37(.a(new_n41_), .b(x9), .c(x6), .O(new_n55_));
  aoi21  g38(.a(new_n54_), .b(x3), .c(new_n55_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n52_), .O(z6));
endmodule


