// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n19_, new_n20_, new_n21_, new_n26_, new_n27_, new_n28_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_;
  inv1   g00(.a(x9), .O(new_n19_));
  inv1   g01(.a(x8), .O(new_n20_));
  nand2  g02(.a(new_n20_), .b(x7), .O(new_n21_));
  xor2a  g03(.a(new_n21_), .b(new_n19_), .O(z1));
  nor3   g04(.a(x9), .b(x8), .c(x7), .O(z2));
  nand3  g05(.a(new_n20_), .b(x7), .c(x6), .O(z4));
  inv1   g06(.a(x0), .O(new_n26_));
  inv1   g07(.a(x1), .O(new_n27_));
  nand3  g08(.a(x9), .b(x8), .c(x7), .O(new_n28_));
  oai21  g09(.a(x8), .b(x7), .c(new_n28_), .O(new_n29_));
  nand4  g10(.a(new_n29_), .b(x2), .c(new_n27_), .d(new_n26_), .O(new_n30_));
  inv1   g11(.a(x6), .O(new_n31_));
  inv1   g12(.a(x7), .O(new_n32_));
  nand3  g13(.a(x8), .b(new_n32_), .c(x5), .O(new_n33_));
  inv1   g14(.a(new_n33_), .O(new_n34_));
  oai21  g15(.a(new_n34_), .b(new_n31_), .c(x9), .O(new_n35_));
  nand2  g16(.a(new_n19_), .b(new_n20_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n35_), .c(new_n30_), .O(z5));
  inv1   g18(.a(x2), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n27_), .c(new_n26_), .O(new_n39_));
  aoi21  g20(.a(new_n39_), .b(x8), .c(new_n32_), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n34_), .c(x9), .O(new_n41_));
  aoi21  g22(.a(new_n21_), .b(new_n19_), .c(new_n31_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n41_), .O(z6));
  one    g24(.O(z0));
  one    g25(.O(z3));
endmodule


