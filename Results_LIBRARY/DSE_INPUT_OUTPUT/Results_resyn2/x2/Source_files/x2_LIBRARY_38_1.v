// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_;
  nand2  g00(.a(x9), .b(x3), .O(new_n18_));
  nor2   g01(.a(x9), .b(x8), .O(new_n19_));
  nor2   g02(.a(x9), .b(x7), .O(new_n20_));
  nor2   g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n18_), .O(z0));
  inv1   g05(.a(x9), .O(new_n23_));
  oai21  g06(.a(x8), .b(x7), .c(new_n23_), .O(new_n24_));
  inv1   g07(.a(x7), .O(new_n25_));
  nor2   g08(.a(x8), .b(new_n25_), .O(new_n26_));
  aoi22  g09(.a(new_n26_), .b(x9), .c(new_n24_), .d(new_n18_), .O(z1));
  and2   g10(.a(new_n24_), .b(new_n18_), .O(z2));
  inv1   g11(.a(x2), .O(new_n29_));
  nor2   g12(.a(x1), .b(x0), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n20_), .c(x8), .d(new_n29_), .O(z3));
  nor2   g14(.a(new_n23_), .b(x3), .O(new_n32_));
  nand2  g15(.a(x9), .b(x8), .O(new_n33_));
  inv1   g16(.a(x6), .O(new_n34_));
  nor2   g17(.a(new_n25_), .b(new_n34_), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(new_n33_), .c(new_n32_), .O(z4));
  inv1   g19(.a(x0), .O(new_n37_));
  inv1   g20(.a(x1), .O(new_n38_));
  nand3  g21(.a(x2), .b(new_n38_), .c(new_n37_), .O(new_n39_));
  inv1   g22(.a(x4), .O(new_n40_));
  nand3  g23(.a(new_n23_), .b(new_n40_), .c(x3), .O(new_n41_));
  oai21  g24(.a(new_n39_), .b(new_n33_), .c(new_n41_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x7), .O(new_n43_));
  nand3  g26(.a(x9), .b(x8), .c(x5), .O(new_n44_));
  oai21  g27(.a(new_n39_), .b(x8), .c(new_n44_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n25_), .O(new_n46_));
  oai21  g29(.a(x9), .b(x8), .c(x6), .O(new_n47_));
  nor2   g30(.a(new_n47_), .b(new_n32_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(z5));
  nand4  g32(.a(x9), .b(x8), .c(x5), .d(x3), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n19_), .c(new_n25_), .O(new_n52_));
  oai21  g35(.a(new_n23_), .b(x3), .c(new_n34_), .O(new_n53_));
  nand2  g36(.a(new_n23_), .b(new_n40_), .O(new_n54_));
  nand2  g37(.a(new_n30_), .b(new_n29_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(x9), .c(x8), .O(new_n56_));
  nand2  g39(.a(x7), .b(x3), .O(new_n57_));
  nor2   g40(.a(new_n57_), .b(new_n19_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n53_), .c(new_n52_), .O(z6));
endmodule


