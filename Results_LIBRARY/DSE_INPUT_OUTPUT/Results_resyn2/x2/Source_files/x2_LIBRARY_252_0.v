// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n25_, new_n26_, new_n27_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x9), .O(new_n18_));
  inv1   g01(.a(x7), .O(new_n19_));
  nand2  g02(.a(x8), .b(new_n19_), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n18_), .O(z0));
  inv1   g04(.a(x8), .O(new_n22_));
  oai21  g05(.a(new_n18_), .b(x7), .c(new_n22_), .O(z1));
  inv1   g06(.a(x2), .O(new_n25_));
  inv1   g07(.a(new_n20_), .O(new_n26_));
  nor2   g08(.a(x1), .b(x0), .O(new_n27_));
  nand4  g09(.a(new_n27_), .b(new_n26_), .c(new_n18_), .d(new_n25_), .O(z3));
  nand2  g10(.a(x9), .b(x8), .O(new_n29_));
  nand2  g11(.a(new_n18_), .b(new_n22_), .O(new_n30_));
  nand4  g12(.a(new_n30_), .b(new_n29_), .c(x7), .d(x6), .O(z4));
  nand2  g13(.a(new_n29_), .b(x7), .O(new_n32_));
  nand4  g14(.a(new_n32_), .b(new_n27_), .c(new_n20_), .d(x2), .O(new_n33_));
  inv1   g15(.a(x4), .O(new_n34_));
  nand3  g16(.a(x7), .b(new_n34_), .c(x3), .O(new_n35_));
  nand2  g17(.a(new_n35_), .b(x8), .O(new_n36_));
  nand2  g18(.a(new_n36_), .b(new_n18_), .O(new_n37_));
  inv1   g19(.a(x6), .O(new_n38_));
  inv1   g20(.a(new_n29_), .O(new_n39_));
  inv1   g21(.a(x5), .O(new_n40_));
  nor2   g22(.a(x7), .b(new_n40_), .O(new_n41_));
  aoi21  g23(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g24(.a(new_n42_), .b(new_n37_), .c(new_n33_), .O(z5));
  inv1   g25(.a(x0), .O(new_n44_));
  inv1   g26(.a(x1), .O(new_n45_));
  nand3  g27(.a(new_n25_), .b(new_n45_), .c(new_n44_), .O(new_n46_));
  nand2  g28(.a(new_n46_), .b(x9), .O(new_n47_));
  nand2  g29(.a(x4), .b(x3), .O(new_n48_));
  aoi21  g30(.a(new_n48_), .b(new_n18_), .c(new_n19_), .O(new_n49_));
  nand2  g31(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand3  g32(.a(new_n50_), .b(new_n42_), .c(z1), .O(z6));
  zero   g33(.O(z2));
endmodule


