// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_, new_n27_,
    new_n28_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  nand3  g03(.a(x9), .b(new_n20_), .c(x7), .O(new_n21_));
  nor3   g04(.a(x9), .b(x8), .c(x7), .O(z2));
  nor2   g05(.a(new_n18_), .b(x2), .O(new_n23_));
  nor2   g06(.a(new_n23_), .b(z2), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n21_), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(z1));
  nor2   g09(.a(x1), .b(x0), .O(new_n27_));
  nor2   g10(.a(x7), .b(x2), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(new_n27_), .c(new_n18_), .d(x8), .O(z3));
  inv1   g12(.a(x2), .O(new_n30_));
  oai21  g13(.a(x8), .b(new_n30_), .c(x9), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(x7), .c(x6), .O(z4));
  nor2   g15(.a(x8), .b(x7), .O(new_n33_));
  nand3  g16(.a(x9), .b(x8), .c(x7), .O(new_n34_));
  inv1   g17(.a(new_n34_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(new_n33_), .c(new_n27_), .O(new_n36_));
  inv1   g19(.a(x7), .O(new_n37_));
  aoi21  g20(.a(x9), .b(new_n37_), .c(x6), .O(new_n38_));
  nor2   g21(.a(new_n38_), .b(new_n23_), .O(new_n39_));
  nand4  g22(.a(x9), .b(x8), .c(new_n37_), .d(x5), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x2), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x8), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nand4  g29(.a(new_n46_), .b(new_n42_), .c(new_n39_), .d(new_n36_), .O(z5));
  nand4  g30(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n48_));
  inv1   g31(.a(new_n48_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n33_), .c(new_n18_), .O(new_n50_));
  inv1   g33(.a(x6), .O(new_n51_));
  oai21  g34(.a(new_n18_), .b(x2), .c(new_n51_), .O(new_n52_));
  nand2  g35(.a(x8), .b(x7), .O(new_n53_));
  nand2  g36(.a(x8), .b(x5), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n37_), .O(new_n55_));
  nand4  g38(.a(new_n55_), .b(new_n53_), .c(x9), .d(x2), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n52_), .c(new_n50_), .O(z6));
endmodule


