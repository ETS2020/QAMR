// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:21 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n22_, new_n23_, new_n24_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x4), .O(z0));
  aoi21  g02(.a(new_n18_), .b(x7), .c(x8), .O(new_n20_));
  oai21  g03(.a(new_n18_), .b(x7), .c(new_n20_), .O(z1));
  inv1   g04(.a(x7), .O(new_n22_));
  inv1   g05(.a(x8), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nor2   g07(.a(new_n24_), .b(x9), .O(z2));
  inv1   g08(.a(x2), .O(new_n26_));
  nor2   g09(.a(x1), .b(x0), .O(new_n27_));
  nand4  g10(.a(new_n27_), .b(new_n18_), .c(new_n22_), .d(new_n26_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(x4), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(x8), .O(z3));
  inv1   g13(.a(x4), .O(new_n31_));
  nand2  g14(.a(x9), .b(x8), .O(new_n32_));
  and2   g15(.a(x7), .b(x6), .O(new_n33_));
  aoi22  g16(.a(new_n33_), .b(new_n32_), .c(x8), .d(new_n31_), .O(z4));
  nand3  g17(.a(x9), .b(new_n22_), .c(x5), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x4), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x8), .O(new_n37_));
  oai21  g20(.a(new_n32_), .b(new_n22_), .c(new_n24_), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n27_), .c(x2), .O(new_n39_));
  nand2  g22(.a(new_n18_), .b(new_n23_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(new_n37_), .d(x6), .O(z5));
  inv1   g24(.a(x0), .O(new_n42_));
  inv1   g25(.a(x1), .O(new_n43_));
  nand4  g26(.a(x9), .b(new_n26_), .c(new_n43_), .d(new_n42_), .O(new_n44_));
  nand3  g27(.a(new_n18_), .b(x8), .c(x3), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n44_), .c(new_n31_), .O(new_n46_));
  nor2   g29(.a(new_n18_), .b(x8), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n46_), .c(x7), .O(new_n48_));
  aoi21  g31(.a(x8), .b(new_n31_), .c(x6), .O(new_n49_));
  nand2  g32(.a(x5), .b(x4), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n32_), .c(new_n40_), .O(new_n51_));
  aoi21  g34(.a(new_n51_), .b(new_n22_), .c(new_n49_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n48_), .O(z6));
endmodule


