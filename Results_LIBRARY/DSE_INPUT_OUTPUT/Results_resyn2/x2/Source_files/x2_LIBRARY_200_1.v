// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(x7), .b(x1), .O(new_n19_));
  inv1   g02(.a(new_n19_), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(new_n18_), .c(x8), .O(z0));
  inv1   g04(.a(x8), .O(new_n22_));
  nand2  g05(.a(x9), .b(new_n22_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(x1), .O(new_n24_));
  nor3   g07(.a(x9), .b(x8), .c(x7), .O(z2));
  aoi21  g08(.a(new_n24_), .b(x7), .c(z2), .O(z1));
  inv1   g09(.a(x1), .O(new_n27_));
  inv1   g10(.a(x7), .O(new_n28_));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x2), .O(new_n30_));
  nand4  g13(.a(new_n18_), .b(x8), .c(new_n30_), .d(new_n29_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n27_), .O(z3));
  nand2  g16(.a(x9), .b(x8), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n20_), .c(x6), .O(z4));
  nand3  g18(.a(x9), .b(x8), .c(x5), .O(new_n36_));
  nand4  g19(.a(new_n22_), .b(x2), .c(new_n27_), .d(new_n29_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n28_), .O(new_n39_));
  nand2  g22(.a(x7), .b(new_n27_), .O(new_n40_));
  oai21  g23(.a(x9), .b(x8), .c(x6), .O(new_n41_));
  inv1   g24(.a(x3), .O(new_n42_));
  nor2   g25(.a(x9), .b(new_n42_), .O(new_n43_));
  nor2   g26(.a(new_n19_), .b(x4), .O(new_n44_));
  aoi22  g27(.a(new_n44_), .b(new_n43_), .c(new_n41_), .d(new_n40_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n39_), .O(z5));
  oai21  g29(.a(x9), .b(x8), .c(new_n36_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n28_), .O(new_n48_));
  nand4  g31(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n23_), .c(x1), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x7), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n48_), .c(x6), .O(z6));
endmodule


