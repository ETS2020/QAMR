// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n29_, new_n31_, new_n32_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x3), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nor2   g02(.a(x9), .b(x8), .O(new_n20_));
  nor2   g03(.a(x9), .b(x7), .O(new_n21_));
  nor2   g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  oai21  g05(.a(new_n19_), .b(new_n18_), .c(new_n22_), .O(z0));
  inv1   g06(.a(x8), .O(new_n24_));
  nand3  g07(.a(x9), .b(x7), .c(x3), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(new_n21_), .c(new_n24_), .O(z1));
  inv1   g10(.a(x7), .O(new_n28_));
  nand2  g11(.a(new_n24_), .b(new_n28_), .O(new_n29_));
  nor2   g12(.a(new_n29_), .b(x9), .O(z2));
  inv1   g13(.a(x2), .O(new_n31_));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n21_), .c(x8), .d(new_n31_), .O(z3));
  oai21  g16(.a(x8), .b(new_n18_), .c(x9), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(x7), .c(x6), .O(z4));
  nand3  g18(.a(x9), .b(x8), .c(x7), .O(new_n36_));
  inv1   g19(.a(x0), .O(new_n37_));
  inv1   g20(.a(x1), .O(new_n38_));
  nand3  g21(.a(x2), .b(new_n38_), .c(new_n37_), .O(new_n39_));
  aoi21  g22(.a(new_n36_), .b(new_n29_), .c(new_n39_), .O(new_n40_));
  nand4  g23(.a(x9), .b(x8), .c(new_n28_), .d(x5), .O(new_n41_));
  inv1   g24(.a(x4), .O(new_n42_));
  nand3  g25(.a(new_n19_), .b(x7), .c(new_n42_), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n41_), .c(x6), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n40_), .c(x3), .O(new_n45_));
  inv1   g28(.a(x6), .O(new_n46_));
  oai21  g29(.a(new_n24_), .b(new_n46_), .c(new_n19_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n45_), .O(z5));
  nand2  g31(.a(new_n19_), .b(new_n42_), .O(new_n49_));
  nand2  g32(.a(new_n32_), .b(new_n31_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(x9), .c(x8), .O(new_n51_));
  nor3   g34(.a(new_n20_), .b(new_n28_), .c(new_n18_), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(new_n53_));
  oai21  g36(.a(new_n19_), .b(x3), .c(new_n46_), .O(new_n54_));
  nand4  g37(.a(x9), .b(x8), .c(x5), .d(x3), .O(new_n55_));
  inv1   g38(.a(new_n55_), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n20_), .c(new_n28_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n54_), .c(new_n53_), .O(z6));
endmodule


