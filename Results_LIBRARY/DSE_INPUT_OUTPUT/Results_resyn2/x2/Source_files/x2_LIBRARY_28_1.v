// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x2), .O(new_n18_));
  nand2  g01(.a(x4), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(new_n19_), .O(new_n20_));
  inv1   g03(.a(x9), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x8), .O(new_n22_));
  inv1   g05(.a(new_n22_), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(x7), .c(new_n20_), .O(z0));
  inv1   g07(.a(x7), .O(new_n25_));
  nand2  g08(.a(x9), .b(new_n25_), .O(new_n26_));
  aoi21  g09(.a(new_n21_), .b(x7), .c(x8), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(new_n26_), .c(new_n20_), .O(z1));
  inv1   g11(.a(x8), .O(new_n29_));
  nand3  g12(.a(new_n21_), .b(new_n29_), .c(new_n25_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n19_), .O(z2));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  inv1   g15(.a(new_n32_), .O(new_n33_));
  nor3   g16(.a(new_n33_), .b(new_n22_), .c(x7), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(x4), .c(new_n18_), .O(z3));
  nand2  g18(.a(x9), .b(x8), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x7), .O(new_n37_));
  inv1   g20(.a(new_n37_), .O(new_n38_));
  aoi21  g21(.a(new_n38_), .b(x6), .c(new_n20_), .O(z4));
  nand2  g22(.a(x8), .b(new_n25_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n37_), .c(new_n32_), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n30_), .c(x6), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x2), .O(new_n43_));
  nand2  g26(.a(new_n21_), .b(new_n29_), .O(new_n44_));
  nand3  g27(.a(x9), .b(x8), .c(x5), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n44_), .c(x7), .O(new_n46_));
  nand2  g29(.a(new_n45_), .b(x4), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g31(.a(new_n21_), .b(x7), .c(x3), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(x6), .c(x4), .O(new_n50_));
  nand2  g33(.a(new_n29_), .b(new_n25_), .O(new_n51_));
  oai21  g34(.a(x9), .b(x8), .c(x6), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  nor2   g37(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n48_), .c(new_n43_), .O(z5));
  nand3  g39(.a(x9), .b(new_n29_), .c(x7), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x6), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n46_), .c(new_n19_), .O(new_n59_));
  inv1   g42(.a(x4), .O(new_n60_));
  nand3  g43(.a(x9), .b(new_n60_), .c(new_n18_), .O(new_n61_));
  nand3  g44(.a(x4), .b(x3), .c(x2), .O(new_n62_));
  oai22  g45(.a(new_n62_), .b(new_n22_), .c(new_n61_), .d(new_n33_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(x7), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n59_), .O(z6));
endmodule


