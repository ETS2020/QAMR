// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  nor2   g00(.a(x7), .b(x4), .O(new_n18_));
  inv1   g01(.a(new_n18_), .O(new_n19_));
  inv1   g02(.a(x9), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x8), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x7), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n19_), .O(z0));
  inv1   g06(.a(x7), .O(new_n24_));
  aoi21  g07(.a(x9), .b(new_n24_), .c(x8), .O(new_n25_));
  oai21  g08(.a(new_n18_), .b(x9), .c(new_n25_), .O(z1));
  inv1   g09(.a(x4), .O(new_n27_));
  inv1   g10(.a(x8), .O(new_n28_));
  nand2  g11(.a(new_n20_), .b(new_n28_), .O(new_n29_));
  aoi21  g12(.a(new_n29_), .b(new_n27_), .c(x7), .O(z2));
  inv1   g13(.a(x0), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  inv1   g15(.a(x2), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n21_), .c(new_n27_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n24_), .O(z3));
  nand2  g19(.a(x9), .b(x8), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(x7), .c(x6), .O(z4));
  nor2   g21(.a(x1), .b(x0), .O(new_n39_));
  nand2  g22(.a(new_n37_), .b(new_n19_), .O(new_n40_));
  nand2  g23(.a(x8), .b(new_n24_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n40_), .c(new_n39_), .d(x2), .O(new_n42_));
  aoi21  g25(.a(new_n24_), .b(x4), .c(x8), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n27_), .c(x3), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n43_), .c(new_n20_), .O(new_n46_));
  nand3  g29(.a(x9), .b(x8), .c(x5), .O(new_n47_));
  inv1   g30(.a(new_n47_), .O(new_n48_));
  aoi21  g31(.a(new_n24_), .b(x4), .c(x6), .O(new_n49_));
  aoi21  g32(.a(new_n48_), .b(new_n18_), .c(new_n49_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n46_), .c(new_n42_), .O(z5));
  aoi21  g34(.a(new_n34_), .b(x8), .c(new_n20_), .O(new_n52_));
  nand4  g35(.a(new_n20_), .b(x8), .c(x4), .d(x3), .O(new_n53_));
  inv1   g36(.a(new_n53_), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n52_), .c(x7), .O(new_n55_));
  nand2  g38(.a(new_n47_), .b(new_n29_), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n18_), .c(new_n49_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n55_), .O(z6));
endmodule


