// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_;
  inv1   g00(.a(x8), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x7), .O(z0));
  nand2  g04(.a(x9), .b(x7), .O(new_n22_));
  oai21  g05(.a(new_n20_), .b(x7), .c(new_n22_), .O(z2));
  inv1   g06(.a(z2), .O(z1));
  inv1   g07(.a(x0), .O(new_n25_));
  inv1   g08(.a(x1), .O(new_n26_));
  nor2   g09(.a(x9), .b(new_n18_), .O(new_n27_));
  nor2   g10(.a(x7), .b(x2), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(new_n27_), .c(new_n26_), .d(new_n25_), .O(z3));
  oai21  g12(.a(x9), .b(x6), .c(x7), .O(z4));
  nand3  g13(.a(x2), .b(new_n26_), .c(new_n25_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x9), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n18_), .O(new_n33_));
  inv1   g16(.a(x7), .O(new_n34_));
  nand2  g17(.a(x8), .b(x5), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x9), .O(new_n37_));
  inv1   g20(.a(x6), .O(new_n38_));
  inv1   g21(.a(x3), .O(new_n39_));
  nor2   g22(.a(x4), .b(new_n39_), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(x7), .c(new_n38_), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n37_), .c(new_n33_), .O(z5));
  nand4  g25(.a(new_n27_), .b(x7), .c(x4), .d(x3), .O(new_n43_));
  nand2  g26(.a(new_n22_), .b(new_n38_), .O(new_n44_));
  nand2  g27(.a(new_n19_), .b(x8), .O(new_n45_));
  nand2  g28(.a(new_n35_), .b(x9), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n45_), .c(new_n34_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n44_), .c(new_n43_), .O(z6));
endmodule


