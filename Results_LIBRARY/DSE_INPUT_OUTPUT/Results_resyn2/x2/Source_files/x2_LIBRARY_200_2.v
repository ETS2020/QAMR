// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  oai21  g04(.a(x9), .b(x8), .c(new_n21_), .O(new_n22_));
  inv1   g05(.a(x8), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(x7), .O(new_n24_));
  inv1   g07(.a(new_n24_), .O(new_n25_));
  aoi22  g08(.a(new_n25_), .b(x9), .c(new_n22_), .d(new_n20_), .O(z1));
  nand4  g09(.a(new_n18_), .b(new_n23_), .c(new_n20_), .d(new_n21_), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(z2));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x2), .O(new_n30_));
  nand4  g13(.a(new_n18_), .b(x8), .c(new_n30_), .d(new_n29_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n21_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n20_), .O(z3));
  nand2  g16(.a(x9), .b(x8), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(x7), .c(x6), .O(z4));
  inv1   g18(.a(x4), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(x3), .c(new_n23_), .O(new_n37_));
  oai21  g20(.a(new_n37_), .b(x9), .c(x6), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x7), .O(new_n39_));
  nor2   g22(.a(x9), .b(x8), .O(new_n40_));
  nand3  g23(.a(x9), .b(x8), .c(x5), .O(new_n41_));
  inv1   g24(.a(new_n41_), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n40_), .c(new_n20_), .O(new_n43_));
  nand2  g26(.a(x9), .b(x7), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x8), .O(new_n45_));
  nand4  g28(.a(new_n45_), .b(new_n24_), .c(x2), .d(new_n29_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n43_), .c(x6), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n21_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n39_), .O(z5));
  inv1   g32(.a(x6), .O(new_n50_));
  nand2  g33(.a(new_n20_), .b(x5), .O(new_n51_));
  nand3  g34(.a(new_n18_), .b(x4), .c(x3), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n51_), .c(new_n23_), .O(new_n53_));
  oai22  g36(.a(new_n53_), .b(new_n50_), .c(x9), .d(x7), .O(new_n54_));
  nand2  g37(.a(new_n30_), .b(new_n29_), .O(new_n55_));
  oai21  g38(.a(new_n44_), .b(new_n55_), .c(x6), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n21_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n54_), .c(z1), .O(z6));
endmodule


