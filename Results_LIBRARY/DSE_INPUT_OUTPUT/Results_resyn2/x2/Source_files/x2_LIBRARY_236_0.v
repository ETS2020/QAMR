// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x4), .O(z0));
  nand2  g02(.a(new_n18_), .b(x4), .O(new_n20_));
  inv1   g03(.a(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n18_), .b(x7), .c(x8), .O(new_n22_));
  oai21  g05(.a(new_n21_), .b(x7), .c(new_n22_), .O(z1));
  inv1   g06(.a(x7), .O(new_n24_));
  inv1   g07(.a(x8), .O(new_n25_));
  nand3  g08(.a(new_n21_), .b(new_n25_), .c(new_n24_), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(z2));
  nand2  g10(.a(x8), .b(new_n24_), .O(new_n28_));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x1), .O(new_n30_));
  inv1   g13(.a(x2), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n28_), .c(x4), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n18_), .O(z3));
  inv1   g17(.a(x4), .O(new_n35_));
  nand2  g18(.a(new_n18_), .b(new_n35_), .O(new_n36_));
  nand2  g19(.a(x9), .b(x8), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(new_n36_), .c(x7), .d(x6), .O(z4));
  nand2  g21(.a(new_n25_), .b(new_n24_), .O(new_n39_));
  oai21  g22(.a(new_n37_), .b(new_n24_), .c(new_n39_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(x2), .c(new_n30_), .d(new_n29_), .O(new_n41_));
  nand3  g24(.a(x8), .b(new_n24_), .c(x5), .O(new_n42_));
  inv1   g25(.a(new_n42_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x9), .O(new_n44_));
  inv1   g27(.a(x6), .O(new_n45_));
  nand2  g28(.a(x8), .b(x4), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n18_), .c(new_n45_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n44_), .c(new_n41_), .O(z5));
  aoi21  g31(.a(new_n32_), .b(x8), .c(new_n24_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n43_), .c(x9), .O(new_n50_));
  aoi21  g33(.a(new_n18_), .b(new_n35_), .c(x6), .O(new_n51_));
  nand2  g34(.a(x8), .b(x3), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x7), .O(new_n53_));
  aoi21  g36(.a(x8), .b(new_n24_), .c(new_n20_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n50_), .O(z6));
endmodule


