// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n28_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x2), .O(new_n18_));
  nor2   g01(.a(x9), .b(new_n18_), .O(new_n19_));
  nand3  g02(.a(new_n19_), .b(x8), .c(x7), .O(z0));
  inv1   g03(.a(x7), .O(new_n21_));
  oai21  g04(.a(x8), .b(new_n21_), .c(x9), .O(new_n22_));
  oai21  g05(.a(x8), .b(x7), .c(new_n19_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(z1));
  inv1   g07(.a(x8), .O(new_n25_));
  nand3  g08(.a(new_n19_), .b(new_n25_), .c(new_n21_), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(z2));
  nor2   g10(.a(x9), .b(x2), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z3));
  and2   g12(.a(x7), .b(x6), .O(new_n30_));
  nand2  g13(.a(x9), .b(x8), .O(new_n31_));
  oai21  g14(.a(new_n30_), .b(new_n28_), .c(new_n31_), .O(z4));
  inv1   g15(.a(x5), .O(new_n33_));
  nand2  g16(.a(x8), .b(new_n21_), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n33_), .c(x6), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x9), .O(new_n36_));
  inv1   g19(.a(x4), .O(new_n37_));
  nand3  g20(.a(x7), .b(new_n37_), .c(x3), .O(new_n38_));
  aoi21  g21(.a(new_n38_), .b(x8), .c(x9), .O(new_n39_));
  nand2  g22(.a(new_n31_), .b(x7), .O(new_n40_));
  nor2   g23(.a(x1), .b(x0), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n40_), .c(new_n34_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x6), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n39_), .c(x2), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n36_), .O(z5));
  nand2  g28(.a(new_n41_), .b(x7), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x9), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n18_), .O(new_n48_));
  inv1   g31(.a(x9), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n25_), .O(new_n50_));
  oai21  g33(.a(new_n31_), .b(new_n33_), .c(new_n50_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n21_), .O(new_n52_));
  nand2  g35(.a(x4), .b(x3), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(x9), .c(x8), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n50_), .c(x7), .O(new_n55_));
  nand4  g38(.a(new_n55_), .b(new_n52_), .c(new_n48_), .d(x6), .O(z6));
endmodule


