// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x7), .O(new_n19_));
  nand2  g02(.a(x8), .b(x5), .O(new_n20_));
  or2    g03(.a(new_n20_), .b(new_n19_), .O(z0));
  inv1   g04(.a(x8), .O(new_n22_));
  inv1   g05(.a(x5), .O(new_n23_));
  inv1   g06(.a(x7), .O(new_n24_));
  oai21  g07(.a(x9), .b(new_n23_), .c(new_n24_), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n19_), .c(new_n22_), .O(z1));
  nor2   g09(.a(x7), .b(new_n23_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n18_), .c(new_n22_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z2));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  inv1   g14(.a(x2), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  nand2  g16(.a(x8), .b(new_n24_), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n33_), .c(x5), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n18_), .O(z3));
  oai21  g19(.a(new_n18_), .b(new_n22_), .c(x7), .O(new_n37_));
  inv1   g20(.a(new_n37_), .O(new_n38_));
  aoi22  g21(.a(new_n38_), .b(x6), .c(new_n18_), .d(new_n23_), .O(z4));
  nor2   g22(.a(x1), .b(x0), .O(new_n40_));
  nand4  g23(.a(new_n37_), .b(new_n34_), .c(new_n40_), .d(x2), .O(new_n41_));
  inv1   g24(.a(x4), .O(new_n42_));
  nand3  g25(.a(x7), .b(new_n42_), .c(x3), .O(new_n43_));
  inv1   g26(.a(new_n43_), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n20_), .c(new_n18_), .O(new_n45_));
  nand3  g28(.a(new_n27_), .b(x9), .c(x8), .O(new_n46_));
  nand4  g29(.a(new_n46_), .b(new_n45_), .c(new_n41_), .d(x6), .O(z5));
  xor2a  g30(.a(x9), .b(x8), .O(new_n48_));
  nand3  g31(.a(x8), .b(x4), .c(x3), .O(new_n49_));
  oai22  g32(.a(new_n49_), .b(new_n19_), .c(new_n48_), .d(x7), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x5), .O(new_n51_));
  aoi21  g34(.a(new_n18_), .b(new_n23_), .c(x6), .O(new_n52_));
  nand2  g35(.a(new_n33_), .b(x8), .O(new_n53_));
  nor2   g36(.a(new_n18_), .b(new_n24_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n51_), .O(z6));
endmodule


