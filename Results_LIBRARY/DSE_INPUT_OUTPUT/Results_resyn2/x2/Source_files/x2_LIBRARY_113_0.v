// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  nor2   g00(.a(x6), .b(x3), .O(new_n18_));
  inv1   g01(.a(new_n18_), .O(new_n19_));
  inv1   g02(.a(x9), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(x8), .c(x7), .O(new_n21_));
  and2   g04(.a(new_n21_), .b(new_n19_), .O(z0));
  inv1   g05(.a(x8), .O(new_n23_));
  nand2  g06(.a(x9), .b(x7), .O(new_n24_));
  nor2   g07(.a(x9), .b(x7), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(new_n23_), .c(new_n18_), .O(z1));
  nand2  g11(.a(new_n25_), .b(new_n23_), .O(new_n29_));
  nor2   g12(.a(new_n29_), .b(new_n18_), .O(z2));
  nor2   g13(.a(new_n23_), .b(x2), .O(new_n31_));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n31_), .c(new_n25_), .d(new_n19_), .O(z3));
  inv1   g16(.a(x3), .O(new_n34_));
  oai21  g17(.a(new_n20_), .b(new_n23_), .c(x7), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x6), .O(new_n36_));
  oai21  g19(.a(x6), .b(new_n34_), .c(new_n36_), .O(z4));
  inv1   g20(.a(x4), .O(new_n38_));
  nand3  g21(.a(x7), .b(new_n38_), .c(x3), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x8), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n20_), .O(new_n41_));
  inv1   g24(.a(x7), .O(new_n42_));
  nand2  g25(.a(x8), .b(new_n42_), .O(new_n43_));
  nand4  g26(.a(new_n43_), .b(new_n35_), .c(new_n32_), .d(x2), .O(new_n44_));
  nand4  g27(.a(x9), .b(x8), .c(new_n42_), .d(x5), .O(new_n45_));
  nand4  g28(.a(new_n45_), .b(new_n44_), .c(new_n41_), .d(x6), .O(z5));
  oai21  g29(.a(new_n21_), .b(new_n38_), .c(x6), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x3), .O(new_n48_));
  inv1   g31(.a(x0), .O(new_n49_));
  inv1   g32(.a(x1), .O(new_n50_));
  inv1   g33(.a(x2), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(x8), .c(new_n24_), .O(new_n53_));
  nand2  g36(.a(new_n45_), .b(new_n29_), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n53_), .c(x6), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n48_), .O(z6));
endmodule


