// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:27 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  nand3  g03(.a(x9), .b(new_n20_), .c(x7), .O(z1));
  nor2   g04(.a(x9), .b(x8), .O(z2));
  inv1   g05(.a(x0), .O(new_n23_));
  inv1   g06(.a(x1), .O(new_n24_));
  inv1   g07(.a(x2), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n24_), .c(new_n23_), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(x7), .c(x8), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n18_), .O(z3));
  xor2a  g11(.a(x9), .b(x8), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(x7), .c(x6), .O(z4));
  nor2   g13(.a(x1), .b(x0), .O(new_n31_));
  oai21  g14(.a(new_n18_), .b(new_n20_), .c(x7), .O(new_n32_));
  inv1   g15(.a(x7), .O(new_n33_));
  nand2  g16(.a(x8), .b(new_n33_), .O(new_n34_));
  nand4  g17(.a(new_n34_), .b(new_n32_), .c(new_n31_), .d(x2), .O(new_n35_));
  inv1   g18(.a(x4), .O(new_n36_));
  nand3  g19(.a(x7), .b(new_n36_), .c(x3), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x8), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n18_), .O(new_n39_));
  inv1   g22(.a(x6), .O(new_n40_));
  nor2   g23(.a(new_n18_), .b(new_n20_), .O(new_n41_));
  inv1   g24(.a(x5), .O(new_n42_));
  nor2   g25(.a(x7), .b(new_n42_), .O(new_n43_));
  aoi21  g26(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n39_), .c(new_n35_), .O(z5));
  nand2  g28(.a(new_n26_), .b(x9), .O(new_n46_));
  nand2  g29(.a(x4), .b(x3), .O(new_n47_));
  aoi21  g30(.a(new_n47_), .b(new_n18_), .c(new_n33_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  oai21  g32(.a(new_n18_), .b(x7), .c(new_n20_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n49_), .c(new_n44_), .O(z6));
endmodule


