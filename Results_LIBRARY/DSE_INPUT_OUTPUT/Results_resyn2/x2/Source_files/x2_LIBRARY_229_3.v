// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  nand2  g00(.a(x9), .b(x4), .O(new_n18_));
  inv1   g01(.a(new_n18_), .O(new_n19_));
  inv1   g02(.a(x9), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x8), .O(new_n21_));
  inv1   g04(.a(new_n21_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x7), .c(new_n19_), .O(z0));
  nand2  g06(.a(new_n20_), .b(x7), .O(new_n24_));
  inv1   g07(.a(x4), .O(new_n25_));
  nand2  g08(.a(x9), .b(new_n25_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n24_), .c(new_n21_), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(z2));
  inv1   g11(.a(x7), .O(new_n29_));
  nor2   g12(.a(x8), .b(new_n29_), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(x9), .c(z2), .O(z1));
  inv1   g14(.a(x0), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  inv1   g16(.a(x2), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  nand2  g18(.a(x8), .b(new_n29_), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n35_), .c(new_n20_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n26_), .O(z3));
  oai21  g21(.a(x8), .b(x4), .c(x9), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(x7), .c(x6), .O(z4));
  nand3  g23(.a(x2), .b(new_n33_), .c(new_n32_), .O(new_n41_));
  nand2  g24(.a(x9), .b(x8), .O(new_n42_));
  nand3  g25(.a(new_n20_), .b(new_n25_), .c(x3), .O(new_n43_));
  oai21  g26(.a(new_n42_), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x7), .O(new_n45_));
  nand3  g28(.a(x9), .b(x8), .c(x5), .O(new_n46_));
  oai21  g29(.a(new_n41_), .b(x8), .c(new_n46_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n29_), .O(new_n48_));
  inv1   g31(.a(x6), .O(new_n49_));
  nor2   g32(.a(x9), .b(x8), .O(new_n50_));
  nor3   g33(.a(new_n50_), .b(new_n19_), .c(new_n49_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n48_), .c(new_n45_), .O(z5));
  aoi21  g35(.a(new_n35_), .b(x8), .c(new_n26_), .O(new_n53_));
  nand4  g36(.a(new_n20_), .b(x8), .c(x4), .d(x3), .O(new_n54_));
  inv1   g37(.a(new_n54_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n53_), .c(x7), .O(new_n56_));
  oai22  g39(.a(new_n46_), .b(x4), .c(x9), .d(x8), .O(new_n57_));
  aoi22  g40(.a(new_n57_), .b(new_n29_), .c(new_n18_), .d(new_n49_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n56_), .O(z6));
endmodule


