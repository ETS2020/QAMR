// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:40 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x1), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x1), .c(x9), .O(z2));
  nor2   g06(.a(x8), .b(new_n20_), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(x9), .c(z2), .O(z1));
  inv1   g08(.a(x1), .O(new_n27_));
  nor2   g09(.a(new_n18_), .b(new_n21_), .O(new_n28_));
  inv1   g10(.a(new_n28_), .O(new_n29_));
  inv1   g11(.a(x6), .O(new_n30_));
  nor2   g12(.a(new_n20_), .b(new_n30_), .O(new_n31_));
  aoi22  g13(.a(new_n31_), .b(new_n29_), .c(new_n18_), .d(new_n27_), .O(z4));
  nand2  g14(.a(x8), .b(x7), .O(new_n33_));
  inv1   g15(.a(x0), .O(new_n34_));
  nand2  g16(.a(x2), .b(new_n34_), .O(new_n35_));
  aoi21  g17(.a(new_n22_), .b(new_n33_), .c(new_n35_), .O(new_n36_));
  oai21  g18(.a(new_n36_), .b(new_n18_), .c(new_n27_), .O(new_n37_));
  inv1   g19(.a(x4), .O(new_n38_));
  nand3  g20(.a(x7), .b(new_n38_), .c(x3), .O(new_n39_));
  nand2  g21(.a(new_n39_), .b(x8), .O(new_n40_));
  nand2  g22(.a(new_n40_), .b(new_n18_), .O(new_n41_));
  inv1   g23(.a(x5), .O(new_n42_));
  nor2   g24(.a(x7), .b(new_n42_), .O(new_n43_));
  aoi21  g25(.a(new_n43_), .b(new_n28_), .c(new_n30_), .O(new_n44_));
  nand3  g26(.a(new_n44_), .b(new_n41_), .c(new_n37_), .O(z5));
  inv1   g27(.a(x2), .O(new_n46_));
  nand3  g28(.a(new_n46_), .b(new_n27_), .c(new_n34_), .O(new_n47_));
  aoi21  g29(.a(new_n47_), .b(x8), .c(new_n20_), .O(new_n48_));
  nand3  g30(.a(x8), .b(new_n20_), .c(x5), .O(new_n49_));
  inv1   g31(.a(new_n49_), .O(new_n50_));
  oai21  g32(.a(new_n50_), .b(new_n48_), .c(x9), .O(new_n51_));
  nand2  g33(.a(new_n18_), .b(new_n27_), .O(new_n52_));
  nand4  g34(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n53_));
  aoi21  g35(.a(new_n53_), .b(new_n22_), .c(x9), .O(new_n54_));
  oai21  g36(.a(new_n54_), .b(new_n30_), .c(new_n52_), .O(new_n55_));
  nand2  g37(.a(new_n55_), .b(new_n51_), .O(z6));
  one    g38(.O(z3));
endmodule


