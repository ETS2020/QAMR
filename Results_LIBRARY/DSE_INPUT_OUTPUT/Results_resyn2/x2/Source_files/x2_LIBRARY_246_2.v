// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n26_, new_n27_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_;
  inv1   g00(.a(x8), .O(new_n19_));
  inv1   g01(.a(x9), .O(new_n20_));
  nand2  g02(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand2  g03(.a(x9), .b(x8), .O(new_n22_));
  nand2  g04(.a(new_n22_), .b(x7), .O(new_n23_));
  and2   g05(.a(new_n23_), .b(new_n21_), .O(z1));
  nor3   g06(.a(x9), .b(x8), .c(x7), .O(z2));
  inv1   g07(.a(x7), .O(new_n26_));
  nor3   g08(.a(x2), .b(x1), .c(x0), .O(new_n27_));
  nand4  g09(.a(new_n27_), .b(new_n20_), .c(x8), .d(new_n26_), .O(z3));
  inv1   g10(.a(x6), .O(new_n29_));
  oai21  g11(.a(x8), .b(new_n29_), .c(x9), .O(new_n30_));
  nand2  g12(.a(new_n30_), .b(x7), .O(z4));
  oai21  g13(.a(new_n20_), .b(x6), .c(x7), .O(new_n32_));
  nand3  g14(.a(x9), .b(x8), .c(x5), .O(new_n33_));
  nand3  g15(.a(new_n33_), .b(new_n21_), .c(x6), .O(new_n34_));
  nand2  g16(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  nor2   g17(.a(x1), .b(x0), .O(new_n36_));
  nand2  g18(.a(x8), .b(new_n26_), .O(new_n37_));
  nand4  g19(.a(new_n37_), .b(new_n36_), .c(new_n23_), .d(x2), .O(new_n38_));
  nand2  g20(.a(new_n38_), .b(new_n35_), .O(z5));
  oai21  g21(.a(new_n27_), .b(new_n22_), .c(x7), .O(new_n40_));
  nand2  g22(.a(new_n40_), .b(new_n35_), .O(z6));
  one    g23(.O(z0));
endmodule


