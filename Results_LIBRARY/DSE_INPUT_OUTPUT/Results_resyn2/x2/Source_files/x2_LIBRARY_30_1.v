// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_;
  nand2  g00(.a(x9), .b(x2), .O(new_n18_));
  inv1   g01(.a(x7), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  inv1   g03(.a(x9), .O(new_n21_));
  oai21  g04(.a(new_n20_), .b(new_n19_), .c(new_n21_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n18_), .O(z0));
  nand2  g06(.a(new_n18_), .b(x7), .O(new_n24_));
  nand2  g07(.a(x9), .b(new_n19_), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n24_), .c(new_n20_), .O(z1));
  nand2  g09(.a(new_n20_), .b(new_n19_), .O(new_n27_));
  nor2   g10(.a(new_n27_), .b(x9), .O(z2));
  inv1   g11(.a(x2), .O(new_n29_));
  nor2   g12(.a(x1), .b(x0), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(x8), .c(new_n19_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n21_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n29_), .O(z3));
  oai21  g16(.a(x8), .b(new_n29_), .c(x9), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(x7), .c(x6), .O(z4));
  nand4  g18(.a(x9), .b(x8), .c(new_n19_), .d(x5), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x6), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x2), .O(new_n38_));
  inv1   g21(.a(x4), .O(new_n39_));
  nand3  g22(.a(x7), .b(new_n39_), .c(x3), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x8), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n21_), .O(new_n42_));
  nand2  g25(.a(x8), .b(new_n19_), .O(new_n43_));
  oai21  g26(.a(new_n21_), .b(new_n20_), .c(x7), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n43_), .c(new_n30_), .O(new_n45_));
  inv1   g28(.a(x6), .O(new_n46_));
  aoi22  g29(.a(new_n25_), .b(new_n46_), .c(x9), .d(new_n29_), .O(new_n47_));
  nand4  g30(.a(new_n47_), .b(new_n45_), .c(new_n42_), .d(new_n38_), .O(z5));
  nand4  g31(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n27_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n21_), .O(new_n51_));
  oai21  g34(.a(new_n21_), .b(x2), .c(new_n46_), .O(new_n52_));
  inv1   g35(.a(x5), .O(new_n53_));
  oai21  g36(.a(x7), .b(new_n53_), .c(x8), .O(new_n54_));
  nand4  g37(.a(new_n54_), .b(new_n27_), .c(x9), .d(x2), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n52_), .c(new_n51_), .O(z6));
endmodule


