// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  nor2   g00(.a(x9), .b(x4), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x9), .O(new_n20_));
  aoi21  g03(.a(new_n20_), .b(x7), .c(x8), .O(new_n21_));
  oai21  g04(.a(new_n18_), .b(x7), .c(new_n21_), .O(z1));
  inv1   g05(.a(x7), .O(new_n23_));
  inv1   g06(.a(x8), .O(new_n24_));
  nand3  g07(.a(new_n18_), .b(new_n24_), .c(new_n23_), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(z2));
  inv1   g09(.a(x0), .O(new_n27_));
  inv1   g10(.a(x1), .O(new_n28_));
  inv1   g11(.a(x2), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(new_n31_));
  nand4  g14(.a(new_n31_), .b(new_n18_), .c(x8), .d(new_n23_), .O(z3));
  inv1   g15(.a(x4), .O(new_n33_));
  inv1   g16(.a(x6), .O(new_n34_));
  oai22  g17(.a(x9), .b(new_n33_), .c(new_n23_), .d(new_n34_), .O(new_n35_));
  oai21  g18(.a(new_n20_), .b(new_n24_), .c(new_n35_), .O(z4));
  nand2  g19(.a(x8), .b(x7), .O(new_n37_));
  nand2  g20(.a(new_n24_), .b(new_n23_), .O(new_n38_));
  nand3  g21(.a(x2), .b(new_n28_), .c(new_n27_), .O(new_n39_));
  aoi21  g22(.a(new_n38_), .b(new_n37_), .c(new_n39_), .O(new_n40_));
  nand3  g23(.a(x8), .b(new_n23_), .c(x5), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x6), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n40_), .c(x9), .O(new_n43_));
  nand2  g26(.a(x7), .b(x3), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(x8), .c(x9), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n34_), .c(new_n33_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n43_), .O(z5));
  inv1   g30(.a(new_n41_), .O(new_n48_));
  aoi21  g31(.a(new_n30_), .b(x8), .c(new_n23_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n48_), .c(x9), .O(new_n50_));
  nand2  g33(.a(new_n38_), .b(new_n33_), .O(new_n51_));
  aoi21  g34(.a(new_n51_), .b(new_n20_), .c(new_n34_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n50_), .O(z6));
endmodule


