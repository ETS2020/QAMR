// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x8), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g03(.a(x9), .b(x5), .O(new_n21_));
  nor2   g04(.a(x9), .b(x7), .O(new_n22_));
  inv1   g05(.a(new_n22_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n21_), .c(new_n20_), .O(z0));
  nand3  g07(.a(x9), .b(x7), .c(x5), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(new_n22_), .c(new_n18_), .O(z1));
  nor2   g10(.a(new_n19_), .b(x5), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(new_n29_));
  oai21  g12(.a(new_n23_), .b(x8), .c(new_n29_), .O(z2));
  inv1   g13(.a(x0), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  inv1   g15(.a(x2), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(new_n34_));
  inv1   g17(.a(x7), .O(new_n35_));
  nand2  g18(.a(x8), .b(new_n35_), .O(new_n36_));
  nor3   g19(.a(new_n36_), .b(new_n34_), .c(x9), .O(new_n37_));
  nor2   g20(.a(new_n37_), .b(new_n28_), .O(z3));
  nand2  g21(.a(x9), .b(x8), .O(new_n39_));
  inv1   g22(.a(x6), .O(new_n40_));
  nor2   g23(.a(new_n35_), .b(new_n40_), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n39_), .c(new_n28_), .O(z4));
  inv1   g25(.a(x5), .O(new_n43_));
  nor2   g26(.a(x8), .b(new_n35_), .O(new_n44_));
  nand3  g27(.a(x2), .b(new_n32_), .c(new_n31_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n44_), .c(new_n36_), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(x9), .c(new_n40_), .O(new_n47_));
  inv1   g30(.a(x4), .O(new_n48_));
  nand3  g31(.a(x7), .b(new_n48_), .c(x3), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(x8), .c(x6), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n19_), .O(new_n51_));
  oai21  g34(.a(new_n47_), .b(new_n43_), .c(new_n51_), .O(z5));
  aoi21  g35(.a(new_n34_), .b(x8), .c(new_n19_), .O(new_n53_));
  nand4  g36(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n54_));
  inv1   g37(.a(new_n54_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n53_), .c(x7), .O(new_n56_));
  nand2  g39(.a(new_n39_), .b(new_n20_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n35_), .O(new_n58_));
  nand4  g41(.a(new_n58_), .b(new_n56_), .c(new_n29_), .d(x6), .O(z6));
endmodule


