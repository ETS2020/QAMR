// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  oai21  g02(.a(x8), .b(x7), .c(new_n18_), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x9), .b(new_n21_), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  nor2   g06(.a(x8), .b(new_n23_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n22_), .c(new_n20_), .O(z1));
  and2   g08(.a(new_n22_), .b(new_n20_), .O(z2));
  nor2   g09(.a(new_n18_), .b(new_n21_), .O(new_n27_));
  nand2  g10(.a(new_n18_), .b(x8), .O(new_n28_));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x2), .O(new_n30_));
  nand4  g13(.a(new_n23_), .b(new_n30_), .c(new_n21_), .d(new_n29_), .O(new_n31_));
  nor2   g14(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  nor2   g15(.a(new_n32_), .b(new_n27_), .O(z3));
  oai21  g16(.a(x8), .b(x1), .c(x9), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(x7), .c(x6), .O(z4));
  nand3  g18(.a(x9), .b(x8), .c(x7), .O(new_n36_));
  oai21  g19(.a(x8), .b(x7), .c(new_n36_), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(x2), .c(new_n29_), .O(new_n38_));
  aoi21  g21(.a(x9), .b(new_n23_), .c(x6), .O(new_n39_));
  nor2   g22(.a(new_n39_), .b(new_n27_), .O(new_n40_));
  nand4  g23(.a(x9), .b(x8), .c(new_n23_), .d(x5), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x6), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n21_), .O(new_n43_));
  inv1   g26(.a(x4), .O(new_n44_));
  nand3  g27(.a(x7), .b(new_n44_), .c(x3), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x8), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n18_), .O(new_n47_));
  nand4  g30(.a(new_n47_), .b(new_n43_), .c(new_n40_), .d(new_n38_), .O(z5));
  oai21  g31(.a(x2), .b(x0), .c(x8), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(x9), .c(new_n21_), .O(new_n50_));
  nand4  g33(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x7), .O(new_n53_));
  aoi21  g36(.a(x9), .b(x1), .c(x6), .O(new_n54_));
  nand2  g37(.a(new_n22_), .b(x8), .O(new_n55_));
  nand2  g38(.a(x8), .b(x5), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(x9), .c(x7), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n53_), .O(z6));
endmodule


