// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(x8), .b(x7), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x3), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n18_), .O(z0));
  inv1   g04(.a(x8), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  nor2   g06(.a(new_n18_), .b(new_n23_), .O(new_n24_));
  nand3  g07(.a(new_n18_), .b(new_n23_), .c(x3), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(new_n24_), .c(new_n22_), .O(z1));
  nand2  g10(.a(new_n26_), .b(new_n22_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z2));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  inv1   g14(.a(x2), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  inv1   g16(.a(new_n33_), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n26_), .c(x8), .O(z3));
  nand2  g18(.a(x9), .b(x8), .O(new_n36_));
  inv1   g19(.a(x3), .O(new_n37_));
  nand2  g20(.a(new_n18_), .b(new_n37_), .O(new_n38_));
  nand4  g21(.a(new_n38_), .b(new_n36_), .c(x7), .d(x6), .O(z4));
  nand2  g22(.a(new_n22_), .b(new_n23_), .O(new_n40_));
  nand3  g23(.a(x2), .b(new_n31_), .c(new_n30_), .O(new_n41_));
  aoi21  g24(.a(new_n40_), .b(new_n19_), .c(new_n41_), .O(new_n42_));
  nand3  g25(.a(x8), .b(new_n23_), .c(x5), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x6), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n42_), .c(x9), .O(new_n45_));
  inv1   g28(.a(x4), .O(new_n46_));
  aoi21  g29(.a(x7), .b(new_n46_), .c(new_n22_), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(x9), .c(x6), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x3), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n45_), .O(z5));
  inv1   g33(.a(new_n43_), .O(new_n51_));
  aoi21  g34(.a(new_n33_), .b(x8), .c(new_n23_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n51_), .c(x9), .O(new_n53_));
  inv1   g36(.a(x6), .O(new_n54_));
  nand3  g37(.a(x8), .b(x7), .c(x4), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n40_), .c(x9), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n54_), .c(new_n38_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n53_), .O(z6));
endmodule


