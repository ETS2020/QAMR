// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:12 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n27_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  oai21  g02(.a(x8), .b(x7), .c(new_n18_), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  nor2   g04(.a(x8), .b(new_n21_), .O(new_n22_));
  inv1   g05(.a(x3), .O(new_n23_));
  nand2  g06(.a(x9), .b(new_n23_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n22_), .c(new_n20_), .O(z1));
  nor3   g08(.a(x9), .b(x8), .c(x7), .O(z2));
  nor3   g09(.a(x2), .b(x1), .c(x0), .O(new_n27_));
  nand4  g10(.a(new_n27_), .b(new_n18_), .c(x8), .d(new_n21_), .O(z3));
  oai21  g11(.a(x8), .b(x3), .c(x9), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(x7), .c(x6), .O(z4));
  nor2   g13(.a(x1), .b(x0), .O(new_n31_));
  nand3  g14(.a(x9), .b(x8), .c(x7), .O(new_n32_));
  oai21  g15(.a(x8), .b(x7), .c(new_n32_), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n31_), .c(x2), .O(new_n34_));
  oai21  g17(.a(new_n21_), .b(x4), .c(new_n18_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x3), .O(new_n36_));
  nand4  g19(.a(x9), .b(x8), .c(new_n21_), .d(x5), .O(new_n37_));
  inv1   g20(.a(x6), .O(new_n38_));
  inv1   g21(.a(x8), .O(new_n39_));
  aoi21  g22(.a(new_n18_), .b(new_n39_), .c(new_n38_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n37_), .c(new_n36_), .d(new_n34_), .O(z5));
  nand3  g24(.a(x9), .b(new_n21_), .c(x5), .O(new_n42_));
  nand3  g25(.a(x7), .b(x4), .c(x3), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x8), .O(new_n45_));
  nor2   g28(.a(new_n18_), .b(new_n21_), .O(new_n46_));
  oai21  g29(.a(new_n27_), .b(new_n39_), .c(new_n46_), .O(new_n47_));
  aoi21  g30(.a(new_n24_), .b(new_n20_), .c(new_n38_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(z6));
endmodule


