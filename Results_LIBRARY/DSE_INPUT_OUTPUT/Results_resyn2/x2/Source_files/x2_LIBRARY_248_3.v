// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  nand2  g00(.a(x9), .b(x5), .O(new_n18_));
  inv1   g01(.a(new_n18_), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  nor2   g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x7), .c(new_n19_), .O(z0));
  inv1   g05(.a(x9), .O(new_n23_));
  oai21  g06(.a(x8), .b(x7), .c(new_n23_), .O(new_n24_));
  inv1   g07(.a(x5), .O(new_n25_));
  nand2  g08(.a(x9), .b(new_n25_), .O(new_n26_));
  nand2  g09(.a(new_n20_), .b(x7), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(new_n28_));
  oai21  g11(.a(new_n28_), .b(new_n26_), .c(new_n24_), .O(z1));
  and2   g12(.a(new_n26_), .b(new_n24_), .O(z2));
  inv1   g13(.a(x2), .O(new_n31_));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  nor2   g15(.a(x9), .b(x7), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(new_n32_), .c(x8), .d(new_n31_), .O(new_n34_));
  and2   g17(.a(new_n34_), .b(new_n18_), .O(z3));
  nand2  g18(.a(x9), .b(x8), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  inv1   g20(.a(x7), .O(new_n38_));
  nor2   g21(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g22(.a(new_n39_), .b(new_n36_), .c(new_n19_), .O(z4));
  oai21  g23(.a(new_n23_), .b(new_n38_), .c(x8), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n32_), .c(new_n27_), .d(x2), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x8), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n23_), .O(new_n46_));
  nand2  g29(.a(new_n18_), .b(x6), .O(new_n47_));
  inv1   g30(.a(new_n47_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(new_n42_), .O(z5));
  inv1   g32(.a(x0), .O(new_n50_));
  inv1   g33(.a(x1), .O(new_n51_));
  nand4  g34(.a(x9), .b(new_n31_), .c(new_n51_), .d(new_n50_), .O(new_n52_));
  nand4  g35(.a(new_n23_), .b(x8), .c(x4), .d(x3), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x7), .O(new_n55_));
  xnor2a g38(.a(x9), .b(x7), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n20_), .c(new_n47_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n55_), .O(z6));
endmodule


