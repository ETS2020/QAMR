// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand3  g02(.a(new_n19_), .b(x8), .c(x7), .O(new_n20_));
  or2    g03(.a(new_n20_), .b(new_n18_), .O(z0));
  inv1   g04(.a(x8), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  nor2   g06(.a(new_n19_), .b(new_n23_), .O(new_n24_));
  nor2   g07(.a(x9), .b(new_n18_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n24_), .c(new_n22_), .O(z1));
  nand2  g11(.a(new_n22_), .b(new_n23_), .O(new_n29_));
  aoi21  g12(.a(new_n29_), .b(x5), .c(x9), .O(z2));
  inv1   g13(.a(x0), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  inv1   g15(.a(x2), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(new_n34_));
  inv1   g17(.a(new_n34_), .O(new_n35_));
  nand4  g18(.a(new_n35_), .b(new_n25_), .c(x8), .d(new_n23_), .O(z3));
  nand2  g19(.a(x9), .b(x8), .O(new_n37_));
  inv1   g20(.a(x6), .O(new_n38_));
  nor2   g21(.a(new_n23_), .b(new_n38_), .O(new_n39_));
  aoi22  g22(.a(new_n39_), .b(new_n37_), .c(new_n19_), .d(new_n18_), .O(z4));
  nand2  g23(.a(x8), .b(x7), .O(new_n41_));
  nand3  g24(.a(x2), .b(new_n32_), .c(new_n31_), .O(new_n42_));
  aoi21  g25(.a(new_n29_), .b(new_n41_), .c(new_n42_), .O(new_n43_));
  nand3  g26(.a(x8), .b(new_n23_), .c(x5), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x6), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n43_), .c(x9), .O(new_n46_));
  inv1   g29(.a(x4), .O(new_n47_));
  nand3  g30(.a(x7), .b(new_n47_), .c(x3), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(x8), .c(x9), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n38_), .c(x5), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n46_), .O(z5));
  xor2a  g34(.a(x9), .b(x8), .O(new_n52_));
  nand2  g35(.a(x4), .b(x3), .O(new_n53_));
  oai22  g36(.a(new_n53_), .b(new_n20_), .c(new_n52_), .d(x7), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x5), .O(new_n55_));
  aoi21  g38(.a(new_n19_), .b(new_n18_), .c(x6), .O(new_n56_));
  nand2  g39(.a(new_n34_), .b(x8), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n24_), .c(new_n56_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n55_), .O(z6));
endmodule


