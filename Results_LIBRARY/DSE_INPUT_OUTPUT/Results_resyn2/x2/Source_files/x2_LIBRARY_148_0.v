// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n24_, new_n26_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x0), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x0), .c(x9), .O(z2));
  nor2   g06(.a(x8), .b(new_n20_), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(x9), .c(z2), .O(z1));
  inv1   g08(.a(x0), .O(new_n26_));
  nand2  g09(.a(new_n18_), .b(new_n26_), .O(z3));
  nand2  g10(.a(x9), .b(x8), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(z3), .c(x7), .d(x6), .O(z4));
  nand2  g12(.a(x8), .b(x7), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  nand3  g14(.a(x2), .b(new_n31_), .c(new_n26_), .O(new_n32_));
  aoi21  g15(.a(new_n22_), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  nand3  g16(.a(x8), .b(new_n20_), .c(x5), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x6), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(new_n33_), .c(x9), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  inv1   g20(.a(x4), .O(new_n38_));
  nand3  g21(.a(x7), .b(new_n38_), .c(x3), .O(new_n39_));
  aoi21  g22(.a(new_n39_), .b(x8), .c(x9), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n37_), .c(x0), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n36_), .O(z5));
  inv1   g25(.a(x2), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(new_n31_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x9), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n26_), .c(new_n37_), .O(new_n46_));
  inv1   g29(.a(x5), .O(new_n47_));
  nand2  g30(.a(new_n18_), .b(new_n21_), .O(new_n48_));
  oai21  g31(.a(new_n28_), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n20_), .O(new_n50_));
  nand2  g33(.a(x4), .b(x3), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(x9), .c(x8), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(new_n48_), .c(x7), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n50_), .c(new_n46_), .O(z6));
endmodule


