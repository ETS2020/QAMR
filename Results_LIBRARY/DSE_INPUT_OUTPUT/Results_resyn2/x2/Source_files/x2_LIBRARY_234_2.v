// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  oai21  g02(.a(new_n18_), .b(x8), .c(x7), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g06(.a(x8), .O(new_n24_));
  nand2  g07(.a(new_n18_), .b(new_n24_), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(new_n23_), .c(new_n20_), .O(z1));
  nand3  g10(.a(new_n26_), .b(new_n22_), .c(new_n21_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z2));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x2), .O(new_n31_));
  nand4  g14(.a(new_n18_), .b(x8), .c(new_n31_), .d(new_n30_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n21_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n22_), .O(z3));
  nand2  g17(.a(x9), .b(x8), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x6), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x7), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n23_), .O(z4));
  nand3  g21(.a(x9), .b(x8), .c(x5), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n25_), .c(new_n21_), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n22_), .O(new_n41_));
  nand2  g24(.a(new_n35_), .b(x7), .O(new_n42_));
  oai21  g25(.a(new_n22_), .b(x1), .c(x8), .O(new_n43_));
  nand4  g26(.a(new_n43_), .b(new_n42_), .c(x2), .d(new_n30_), .O(new_n44_));
  inv1   g27(.a(x3), .O(new_n45_));
  oai21  g28(.a(x4), .b(new_n45_), .c(x8), .O(new_n46_));
  nor2   g29(.a(x9), .b(new_n22_), .O(new_n47_));
  aoi21  g30(.a(new_n22_), .b(x1), .c(x6), .O(new_n48_));
  aoi21  g31(.a(new_n47_), .b(new_n46_), .c(new_n48_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n44_), .c(new_n41_), .O(z5));
  aoi21  g33(.a(new_n39_), .b(new_n25_), .c(x7), .O(new_n51_));
  nand4  g34(.a(x9), .b(x7), .c(new_n31_), .d(new_n30_), .O(new_n52_));
  inv1   g35(.a(new_n52_), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n51_), .c(new_n21_), .O(new_n54_));
  nand3  g37(.a(x8), .b(x4), .c(x3), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n18_), .c(new_n42_), .O(new_n56_));
  nor2   g39(.a(new_n56_), .b(new_n48_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n54_), .O(z6));
endmodule


