// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_;
  inv1   g00(.a(x9), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n18_), .O(z0));
  xor2a  g06(.a(x9), .b(x7), .O(new_n24_));
  nor2   g07(.a(new_n24_), .b(x8), .O(new_n25_));
  aoi21  g08(.a(new_n18_), .b(x5), .c(new_n25_), .O(z1));
  nand2  g09(.a(new_n21_), .b(new_n20_), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(new_n19_), .c(x9), .O(z2));
  nand2  g11(.a(x8), .b(new_n20_), .O(new_n29_));
  inv1   g12(.a(x2), .O(new_n30_));
  nor2   g13(.a(x1), .b(x0), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n29_), .c(new_n19_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n18_), .O(z3));
  inv1   g17(.a(x6), .O(new_n35_));
  oai22  g18(.a(x9), .b(new_n19_), .c(new_n20_), .d(new_n35_), .O(new_n36_));
  nand2  g19(.a(x9), .b(x8), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n36_), .O(z4));
  nand2  g21(.a(new_n37_), .b(x7), .O(new_n39_));
  nand4  g22(.a(new_n39_), .b(new_n31_), .c(new_n29_), .d(x2), .O(new_n40_));
  inv1   g23(.a(x4), .O(new_n41_));
  nand3  g24(.a(x7), .b(new_n41_), .c(x3), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x8), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n18_), .O(new_n44_));
  oai21  g27(.a(new_n21_), .b(x7), .c(x9), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(x5), .c(new_n35_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n44_), .c(new_n40_), .O(z5));
  inv1   g30(.a(new_n25_), .O(new_n48_));
  inv1   g31(.a(x0), .O(new_n49_));
  inv1   g32(.a(x1), .O(new_n50_));
  nand4  g33(.a(x9), .b(new_n30_), .c(new_n50_), .d(new_n49_), .O(new_n51_));
  nand4  g34(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(x7), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n46_), .c(new_n48_), .O(z6));
endmodule


