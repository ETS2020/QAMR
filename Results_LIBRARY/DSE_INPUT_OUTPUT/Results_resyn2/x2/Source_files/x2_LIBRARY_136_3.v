// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n24_, new_n25_, new_n27_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  xnor2a g03(.a(x9), .b(x7), .O(new_n21_));
  aoi22  g04(.a(new_n21_), .b(new_n20_), .c(x9), .d(x2), .O(z1));
  nor3   g05(.a(x9), .b(x8), .c(x7), .O(z2));
  nor2   g06(.a(x1), .b(x0), .O(new_n24_));
  nor2   g07(.a(x7), .b(x2), .O(new_n25_));
  nand4  g08(.a(new_n25_), .b(new_n24_), .c(new_n18_), .d(x8), .O(z3));
  oai21  g09(.a(x8), .b(x2), .c(x9), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(x7), .c(x6), .O(z4));
  inv1   g11(.a(x2), .O(new_n29_));
  inv1   g12(.a(x7), .O(new_n30_));
  nand4  g13(.a(x9), .b(x8), .c(new_n30_), .d(x5), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x6), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  inv1   g16(.a(x4), .O(new_n34_));
  nand3  g17(.a(x7), .b(new_n34_), .c(x3), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(x8), .c(x6), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n18_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n33_), .O(z5));
  inv1   g21(.a(x0), .O(new_n39_));
  inv1   g22(.a(x1), .O(new_n40_));
  nand3  g23(.a(x9), .b(new_n40_), .c(new_n39_), .O(new_n41_));
  nand4  g24(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x7), .O(new_n44_));
  nand2  g27(.a(x9), .b(x2), .O(new_n45_));
  oai21  g28(.a(x9), .b(x6), .c(new_n45_), .O(new_n46_));
  aoi21  g29(.a(new_n21_), .b(new_n20_), .c(new_n46_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n44_), .c(new_n33_), .O(z6));
endmodule


