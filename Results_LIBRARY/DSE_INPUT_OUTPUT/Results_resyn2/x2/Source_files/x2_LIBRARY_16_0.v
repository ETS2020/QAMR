// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n25_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x1), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  oai21  g02(.a(x9), .b(new_n19_), .c(x7), .O(new_n20_));
  oai21  g03(.a(x7), .b(new_n18_), .c(new_n20_), .O(z0));
  inv1   g04(.a(x9), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n19_), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(x1), .c(x7), .O(z2));
  nor2   g07(.a(new_n22_), .b(x8), .O(new_n25_));
  aoi21  g08(.a(new_n25_), .b(x7), .c(z2), .O(z1));
  nand2  g09(.a(x9), .b(x8), .O(new_n28_));
  nand3  g10(.a(new_n28_), .b(x7), .c(x6), .O(z4));
  inv1   g11(.a(x4), .O(new_n30_));
  nand2  g12(.a(new_n30_), .b(x3), .O(new_n31_));
  aoi21  g13(.a(new_n31_), .b(x8), .c(x9), .O(new_n32_));
  inv1   g14(.a(x0), .O(new_n33_));
  nand3  g15(.a(x2), .b(new_n18_), .c(new_n33_), .O(new_n34_));
  nor2   g16(.a(new_n34_), .b(new_n28_), .O(new_n35_));
  oai21  g17(.a(new_n35_), .b(new_n32_), .c(x7), .O(new_n36_));
  nor2   g18(.a(x7), .b(new_n18_), .O(new_n37_));
  inv1   g19(.a(x7), .O(new_n38_));
  aoi21  g20(.a(new_n38_), .b(new_n18_), .c(x6), .O(new_n39_));
  nand3  g21(.a(x9), .b(x8), .c(x5), .O(new_n40_));
  nand2  g22(.a(new_n40_), .b(new_n23_), .O(new_n41_));
  aoi21  g23(.a(new_n41_), .b(new_n37_), .c(new_n39_), .O(new_n42_));
  nand2  g24(.a(new_n42_), .b(new_n36_), .O(z5));
  inv1   g25(.a(x2), .O(new_n44_));
  nand3  g26(.a(x9), .b(new_n44_), .c(new_n33_), .O(new_n45_));
  nand2  g27(.a(new_n45_), .b(x7), .O(new_n46_));
  nand2  g28(.a(new_n46_), .b(new_n18_), .O(new_n47_));
  nand4  g29(.a(new_n22_), .b(x8), .c(x4), .d(x3), .O(new_n48_));
  inv1   g30(.a(new_n48_), .O(new_n49_));
  oai21  g31(.a(new_n49_), .b(new_n25_), .c(x7), .O(new_n50_));
  nand3  g32(.a(new_n50_), .b(new_n47_), .c(new_n42_), .O(z6));
  one    g33(.O(z3));
endmodule


