// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  oai21  g02(.a(x8), .b(x7), .c(new_n18_), .O(new_n20_));
  inv1   g03(.a(x4), .O(new_n21_));
  nand2  g04(.a(x9), .b(new_n21_), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  nor2   g06(.a(x8), .b(new_n23_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n22_), .c(new_n20_), .O(z1));
  and2   g08(.a(new_n22_), .b(new_n20_), .O(z2));
  inv1   g09(.a(x2), .O(new_n27_));
  nor2   g10(.a(x1), .b(x0), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g12(.a(x8), .b(new_n23_), .O(new_n30_));
  oai21  g13(.a(new_n30_), .b(new_n29_), .c(new_n18_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n22_), .O(z3));
  nand2  g15(.a(x9), .b(x8), .O(new_n33_));
  inv1   g16(.a(x6), .O(new_n34_));
  nor2   g17(.a(new_n23_), .b(new_n34_), .O(new_n35_));
  aoi22  g18(.a(new_n35_), .b(new_n33_), .c(x9), .d(x4), .O(z4));
  nand2  g19(.a(new_n33_), .b(x7), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(new_n30_), .c(new_n28_), .d(x2), .O(new_n38_));
  nand3  g21(.a(new_n18_), .b(x7), .c(x3), .O(new_n39_));
  nand4  g22(.a(x9), .b(x8), .c(new_n23_), .d(x5), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n21_), .O(new_n42_));
  inv1   g25(.a(x8), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n34_), .c(new_n18_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n42_), .O(z5));
  aoi21  g28(.a(new_n29_), .b(x8), .c(new_n22_), .O(new_n46_));
  nand4  g29(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n47_));
  inv1   g30(.a(new_n47_), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n46_), .c(x7), .O(new_n49_));
  aoi21  g32(.a(x9), .b(x4), .c(x6), .O(new_n50_));
  inv1   g33(.a(x5), .O(new_n51_));
  oai21  g34(.a(new_n22_), .b(new_n51_), .c(x8), .O(new_n52_));
  aoi21  g35(.a(x9), .b(new_n43_), .c(x7), .O(new_n53_));
  aoi21  g36(.a(new_n53_), .b(new_n52_), .c(new_n50_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n49_), .O(z6));
endmodule


