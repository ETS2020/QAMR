// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n19_, new_n20_, new_n22_, new_n23_, new_n25_, new_n26_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_;
  inv1   g00(.a(x7), .O(new_n19_));
  aoi21  g01(.a(x9), .b(new_n19_), .c(x8), .O(new_n20_));
  oai21  g02(.a(x9), .b(new_n19_), .c(new_n20_), .O(z1));
  nand2  g03(.a(x8), .b(new_n19_), .O(new_n22_));
  inv1   g04(.a(new_n22_), .O(new_n23_));
  nor2   g05(.a(new_n23_), .b(x9), .O(z2));
  inv1   g06(.a(x9), .O(new_n25_));
  nor3   g07(.a(x2), .b(x1), .c(x0), .O(new_n26_));
  nand3  g08(.a(new_n26_), .b(new_n23_), .c(new_n25_), .O(z3));
  inv1   g09(.a(x8), .O(new_n28_));
  nand4  g10(.a(x9), .b(new_n28_), .c(x7), .d(x6), .O(z4));
  inv1   g11(.a(x6), .O(new_n30_));
  oai21  g12(.a(x9), .b(new_n19_), .c(new_n30_), .O(new_n31_));
  nand3  g13(.a(x9), .b(x8), .c(x5), .O(new_n32_));
  oai21  g14(.a(x9), .b(x8), .c(new_n32_), .O(new_n33_));
  nand2  g15(.a(new_n33_), .b(new_n19_), .O(new_n34_));
  nor2   g16(.a(x1), .b(x0), .O(new_n35_));
  nand2  g17(.a(x9), .b(x8), .O(new_n36_));
  nand2  g18(.a(new_n36_), .b(x7), .O(new_n37_));
  nand4  g19(.a(new_n37_), .b(new_n35_), .c(new_n22_), .d(x2), .O(new_n38_));
  nand3  g20(.a(new_n38_), .b(new_n34_), .c(new_n31_), .O(z5));
  oai21  g21(.a(new_n36_), .b(new_n26_), .c(x7), .O(new_n40_));
  nand3  g22(.a(new_n40_), .b(new_n34_), .c(new_n31_), .O(z6));
  one    g23(.O(z0));
endmodule


