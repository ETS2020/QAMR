// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x8), .O(new_n18_));
  oai21  g01(.a(x9), .b(new_n18_), .c(x7), .O(new_n19_));
  inv1   g02(.a(x7), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x3), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n19_), .O(z0));
  nand2  g05(.a(x9), .b(x7), .O(new_n23_));
  oai21  g06(.a(new_n21_), .b(x9), .c(new_n23_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n18_), .O(z1));
  inv1   g08(.a(x9), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n18_), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(x3), .c(x7), .O(z2));
  inv1   g11(.a(new_n21_), .O(new_n29_));
  nor3   g12(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n29_), .c(new_n26_), .d(x8), .O(z3));
  nand3  g14(.a(x9), .b(x8), .c(x7), .O(new_n32_));
  inv1   g15(.a(x6), .O(new_n33_));
  nand2  g16(.a(x7), .b(new_n33_), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n32_), .c(new_n21_), .O(z4));
  oai21  g18(.a(new_n21_), .b(x8), .c(new_n32_), .O(new_n36_));
  nor2   g19(.a(x1), .b(x0), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x2), .O(new_n38_));
  inv1   g21(.a(new_n38_), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n36_), .O(new_n40_));
  inv1   g23(.a(x3), .O(new_n41_));
  aoi21  g24(.a(new_n20_), .b(new_n41_), .c(x8), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n42_), .c(new_n26_), .O(new_n46_));
  aoi21  g29(.a(new_n20_), .b(new_n41_), .c(x6), .O(new_n47_));
  nand3  g30(.a(x9), .b(x8), .c(x5), .O(new_n48_));
  inv1   g31(.a(new_n48_), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(new_n29_), .c(new_n47_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n46_), .c(new_n40_), .O(z5));
  inv1   g34(.a(new_n47_), .O(new_n52_));
  nand3  g35(.a(new_n48_), .b(new_n27_), .c(new_n20_), .O(new_n53_));
  aoi21  g36(.a(x7), .b(new_n43_), .c(new_n41_), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n53_), .c(new_n19_), .O(new_n55_));
  inv1   g38(.a(new_n23_), .O(new_n56_));
  oai21  g39(.a(new_n30_), .b(new_n18_), .c(new_n56_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n55_), .c(new_n52_), .O(z6));
endmodule


