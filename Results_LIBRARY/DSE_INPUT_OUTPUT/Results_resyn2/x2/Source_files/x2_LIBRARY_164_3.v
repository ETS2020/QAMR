// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n27_, new_n29_, new_n30_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x9), .b(new_n21_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x7), .O(new_n23_));
  inv1   g06(.a(x7), .O(new_n24_));
  nand2  g07(.a(x9), .b(new_n24_), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n23_), .c(new_n20_), .O(z1));
  aoi21  g09(.a(new_n20_), .b(new_n24_), .c(x9), .O(new_n27_));
  aoi21  g10(.a(x9), .b(new_n21_), .c(new_n27_), .O(z2));
  nor2   g11(.a(x2), .b(x0), .O(new_n29_));
  nor2   g12(.a(x7), .b(x1), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n29_), .c(new_n18_), .d(x8), .O(z3));
  oai21  g14(.a(x8), .b(x1), .c(x9), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(x7), .c(x6), .O(z4));
  inv1   g16(.a(x0), .O(new_n34_));
  nand3  g17(.a(x9), .b(x8), .c(x7), .O(new_n35_));
  oai21  g18(.a(x8), .b(x7), .c(new_n35_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(x2), .c(new_n34_), .O(new_n37_));
  inv1   g20(.a(x6), .O(new_n38_));
  aoi22  g21(.a(new_n25_), .b(new_n38_), .c(x9), .d(x1), .O(new_n39_));
  nand4  g22(.a(x9), .b(x8), .c(new_n24_), .d(x5), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n21_), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x8), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nand4  g29(.a(new_n46_), .b(new_n42_), .c(new_n39_), .d(new_n37_), .O(z5));
  oai21  g30(.a(x2), .b(x0), .c(x8), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(x9), .c(new_n21_), .O(new_n49_));
  nand4  g32(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x7), .O(new_n52_));
  aoi21  g35(.a(x9), .b(x1), .c(x6), .O(new_n53_));
  nand2  g36(.a(new_n22_), .b(x8), .O(new_n54_));
  nand2  g37(.a(x8), .b(x5), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(x9), .c(x7), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n52_), .O(z6));
endmodule


