// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  nor2   g00(.a(x9), .b(x0), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x7), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x9), .O(new_n22_));
  oai21  g05(.a(x8), .b(x7), .c(new_n18_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(z1));
  inv1   g07(.a(x7), .O(new_n25_));
  nand3  g08(.a(new_n18_), .b(new_n20_), .c(new_n25_), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(z2));
  inv1   g10(.a(x9), .O(new_n28_));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x1), .O(new_n30_));
  inv1   g13(.a(x2), .O(new_n31_));
  nand4  g14(.a(x8), .b(new_n25_), .c(new_n31_), .d(new_n30_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n28_), .O(z3));
  nor2   g17(.a(new_n28_), .b(x8), .O(new_n35_));
  and2   g18(.a(x7), .b(x6), .O(new_n36_));
  oai21  g19(.a(new_n35_), .b(new_n18_), .c(new_n36_), .O(z4));
  inv1   g20(.a(x4), .O(new_n38_));
  nand3  g21(.a(x7), .b(new_n38_), .c(x3), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x8), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n28_), .O(new_n41_));
  oai21  g24(.a(new_n28_), .b(new_n25_), .c(x8), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(new_n21_), .c(x2), .d(new_n30_), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n41_), .c(x6), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n29_), .O(new_n45_));
  nand3  g28(.a(x8), .b(new_n25_), .c(x5), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x6), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x9), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n45_), .O(z5));
  nand3  g32(.a(x9), .b(new_n31_), .c(new_n30_), .O(new_n50_));
  nand4  g33(.a(new_n28_), .b(x8), .c(x4), .d(x3), .O(new_n51_));
  aoi21  g34(.a(new_n51_), .b(new_n50_), .c(x0), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n35_), .c(x7), .O(new_n53_));
  aoi21  g36(.a(new_n28_), .b(x0), .c(x6), .O(new_n54_));
  nand2  g37(.a(x8), .b(x5), .O(new_n55_));
  oai21  g38(.a(x9), .b(x0), .c(new_n55_), .O(new_n56_));
  aoi21  g39(.a(new_n28_), .b(x8), .c(x7), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n56_), .c(new_n54_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n53_), .O(z6));
endmodule


