// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_;
  inv1   g00(.a(x8), .O(new_n18_));
  nor2   g01(.a(x9), .b(new_n18_), .O(new_n19_));
  aoi22  g02(.a(new_n19_), .b(x7), .c(x9), .d(x5), .O(z0));
  nand2  g03(.a(x9), .b(x5), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  inv1   g05(.a(x9), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n18_), .c(new_n22_), .O(new_n24_));
  nand3  g07(.a(x9), .b(new_n18_), .c(x7), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n24_), .c(new_n21_), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(z1));
  nand2  g10(.a(new_n24_), .b(new_n21_), .O(z2));
  inv1   g11(.a(x5), .O(new_n29_));
  nand2  g12(.a(x9), .b(new_n29_), .O(new_n30_));
  inv1   g13(.a(x0), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  inv1   g15(.a(x2), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(new_n34_));
  nand2  g17(.a(x8), .b(new_n22_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(new_n34_), .c(new_n23_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n30_), .O(z3));
  oai21  g20(.a(x8), .b(x5), .c(x9), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(x7), .c(x6), .O(z4));
  inv1   g22(.a(x4), .O(new_n40_));
  nand3  g23(.a(x7), .b(new_n40_), .c(x3), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x8), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n23_), .O(new_n43_));
  nand3  g26(.a(x9), .b(x8), .c(x7), .O(new_n44_));
  oai21  g27(.a(x8), .b(x7), .c(new_n44_), .O(new_n45_));
  nand4  g28(.a(new_n45_), .b(x2), .c(new_n32_), .d(new_n31_), .O(new_n46_));
  nand4  g29(.a(new_n46_), .b(new_n43_), .c(new_n21_), .d(x6), .O(z5));
  aoi21  g30(.a(new_n34_), .b(x8), .c(new_n30_), .O(new_n48_));
  nand4  g31(.a(new_n23_), .b(x8), .c(x4), .d(x3), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n48_), .c(x7), .O(new_n51_));
  nand2  g34(.a(new_n24_), .b(x6), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n51_), .O(z6));
endmodule


