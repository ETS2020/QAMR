// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n27_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x9), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  nor2   g02(.a(x6), .b(new_n19_), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  nor2   g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(new_n18_), .c(new_n20_), .O(z0));
  nor2   g07(.a(new_n20_), .b(x8), .O(new_n25_));
  nor2   g08(.a(new_n18_), .b(new_n21_), .O(new_n26_));
  nor2   g09(.a(x9), .b(x7), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(z1));
  and2   g11(.a(new_n27_), .b(new_n25_), .O(z2));
  nor2   g12(.a(x2), .b(x0), .O(new_n30_));
  nand4  g13(.a(new_n18_), .b(x8), .c(new_n21_), .d(new_n19_), .O(new_n31_));
  inv1   g14(.a(new_n31_), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(new_n30_), .c(new_n20_), .O(z3));
  nand2  g16(.a(x9), .b(x8), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x7), .O(new_n35_));
  inv1   g18(.a(new_n35_), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(x6), .c(new_n20_), .O(z4));
  inv1   g20(.a(x0), .O(new_n38_));
  nand2  g21(.a(x8), .b(new_n21_), .O(new_n39_));
  nand4  g22(.a(new_n35_), .b(new_n39_), .c(x2), .d(new_n38_), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n19_), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(x8), .c(x9), .O(new_n45_));
  nand3  g28(.a(x9), .b(x8), .c(x5), .O(new_n46_));
  nor2   g29(.a(new_n46_), .b(x7), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n45_), .c(x6), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n42_), .O(z5));
  nand2  g32(.a(new_n30_), .b(new_n26_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x6), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n19_), .O(new_n52_));
  nand2  g35(.a(new_n18_), .b(new_n22_), .O(new_n53_));
  aoi21  g36(.a(new_n53_), .b(new_n46_), .c(x7), .O(new_n54_));
  nand3  g37(.a(x8), .b(x4), .c(x3), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n18_), .c(new_n35_), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n54_), .c(x6), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n52_), .O(z6));
endmodule


