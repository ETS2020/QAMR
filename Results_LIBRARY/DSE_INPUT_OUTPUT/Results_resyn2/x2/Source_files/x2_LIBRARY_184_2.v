// Benchmark "FAU" written by ABC on Tue Aug 11 16:40:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  nor2   g00(.a(x9), .b(x4), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  inv1   g04(.a(x9), .O(new_n22_));
  nor2   g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nor3   g06(.a(x9), .b(x7), .c(x4), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n23_), .c(new_n20_), .O(z1));
  inv1   g08(.a(x4), .O(new_n26_));
  nand2  g09(.a(new_n20_), .b(new_n21_), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(new_n26_), .c(x9), .O(z2));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x1), .O(new_n30_));
  inv1   g13(.a(x2), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(new_n32_));
  nand2  g15(.a(new_n24_), .b(x8), .O(new_n33_));
  or2    g16(.a(new_n33_), .b(new_n32_), .O(z3));
  nor2   g17(.a(new_n22_), .b(x8), .O(new_n35_));
  inv1   g18(.a(x6), .O(new_n36_));
  nor2   g19(.a(new_n21_), .b(new_n36_), .O(new_n37_));
  oai21  g20(.a(new_n35_), .b(new_n18_), .c(new_n37_), .O(z4));
  nand3  g21(.a(x8), .b(new_n21_), .c(x5), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x6), .O(new_n40_));
  nand2  g23(.a(x8), .b(x7), .O(new_n41_));
  nand3  g24(.a(x2), .b(new_n30_), .c(new_n29_), .O(new_n42_));
  aoi21  g25(.a(new_n27_), .b(new_n41_), .c(new_n42_), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n40_), .c(x9), .O(new_n44_));
  nand2  g27(.a(x7), .b(x3), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(x8), .c(x9), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n36_), .c(new_n26_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n44_), .O(z5));
  oai21  g31(.a(new_n27_), .b(x9), .c(x6), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n26_), .O(new_n50_));
  aoi21  g33(.a(new_n32_), .b(x8), .c(new_n21_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n40_), .c(x9), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n50_), .O(z6));
endmodule


