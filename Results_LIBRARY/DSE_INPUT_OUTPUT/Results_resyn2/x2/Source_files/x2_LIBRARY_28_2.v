// Benchmark "FAU" written by ABC on Tue Aug 11 16:39:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x2), .O(new_n18_));
  nor2   g01(.a(x4), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  nor2   g03(.a(x9), .b(new_n20_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x7), .c(new_n19_), .O(z0));
  inv1   g05(.a(x7), .O(new_n23_));
  nand2  g06(.a(x9), .b(new_n23_), .O(new_n24_));
  inv1   g07(.a(x9), .O(new_n25_));
  aoi21  g08(.a(new_n25_), .b(x7), .c(x8), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(new_n24_), .c(new_n19_), .O(z1));
  inv1   g10(.a(new_n19_), .O(new_n28_));
  nand2  g11(.a(new_n25_), .b(new_n20_), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(x7), .c(new_n28_), .O(z2));
  inv1   g13(.a(x0), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nor2   g16(.a(new_n33_), .b(x2), .O(new_n34_));
  nand2  g17(.a(new_n21_), .b(new_n23_), .O(new_n35_));
  inv1   g18(.a(new_n35_), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(new_n34_), .c(new_n19_), .O(z3));
  nand2  g20(.a(x9), .b(x8), .O(new_n38_));
  and2   g21(.a(x7), .b(x6), .O(new_n39_));
  aoi21  g22(.a(new_n39_), .b(new_n38_), .c(new_n19_), .O(z4));
  inv1   g23(.a(x4), .O(new_n41_));
  nand2  g24(.a(new_n20_), .b(new_n23_), .O(new_n42_));
  nand3  g25(.a(x9), .b(x8), .c(x7), .O(new_n43_));
  aoi21  g26(.a(new_n43_), .b(new_n42_), .c(new_n33_), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n41_), .c(x2), .O(new_n45_));
  oai21  g28(.a(new_n29_), .b(x7), .c(x6), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n28_), .O(new_n47_));
  nand2  g30(.a(new_n25_), .b(x7), .O(new_n48_));
  nand2  g31(.a(new_n41_), .b(x3), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(x8), .c(new_n48_), .O(new_n50_));
  nor2   g33(.a(x8), .b(x7), .O(new_n51_));
  nand3  g34(.a(x9), .b(x8), .c(x5), .O(new_n52_));
  oai22  g35(.a(new_n52_), .b(x7), .c(new_n51_), .d(x6), .O(new_n53_));
  nor2   g36(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n47_), .c(new_n45_), .O(z5));
  aoi21  g38(.a(new_n52_), .b(new_n29_), .c(x7), .O(new_n56_));
  nand3  g39(.a(x9), .b(new_n20_), .c(x7), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x6), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n56_), .c(new_n28_), .O(new_n59_));
  nand4  g42(.a(x9), .b(new_n18_), .c(new_n32_), .d(new_n31_), .O(new_n60_));
  nand4  g43(.a(new_n25_), .b(x8), .c(x4), .d(x3), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(x7), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n59_), .O(z6));
endmodule


