// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n92_, new_n94_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(x17), .b(x16), .O(new_n37_));
  aoi21  g03(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(new_n37_), .O(new_n45_));
  inv1   g11(.a(x02), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n46_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  oai21  g14(.a(new_n48_), .b(x06), .c(new_n45_), .O(z02));
  xnor2a g15(.a(x07), .b(x06), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n48_), .c(new_n45_), .O(z03));
  nor2   g17(.a(new_n37_), .b(new_n42_), .O(z13));
  and2   g18(.a(z13), .b(new_n47_), .O(new_n53_));
  nand3  g19(.a(x08), .b(x07), .c(x06), .O(new_n54_));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n54_), .c(new_n53_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  nand2  g25(.a(new_n54_), .b(x09), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n60_), .c(new_n47_), .d(x04), .O(new_n63_));
  and2   g29(.a(new_n63_), .b(new_n45_), .O(z05));
  nand2  g30(.a(new_n62_), .b(x10), .O(new_n65_));
  nor3   g31(.a(new_n54_), .b(x10), .c(x09), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n53_), .O(z06));
  inv1   g34(.a(x11), .O(new_n69_));
  nand2  g35(.a(new_n66_), .b(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n67_), .b(x11), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n53_), .O(z07));
  nand2  g38(.a(new_n70_), .b(x12), .O(new_n73_));
  nor2   g39(.a(x12), .b(x11), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n66_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n53_), .O(z08));
  nand2  g42(.a(new_n75_), .b(x13), .O(new_n77_));
  nor2   g43(.a(x13), .b(x10), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(new_n62_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n48_), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n77_), .c(new_n37_), .O(z09));
  inv1   g48(.a(x00), .O(new_n83_));
  nand2  g49(.a(new_n79_), .b(new_n83_), .O(new_n84_));
  inv1   g50(.a(x03), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x02), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(x16), .c(new_n62_), .O(new_n87_));
  nor2   g53(.a(new_n47_), .b(new_n36_), .O(new_n88_));
  aoi21  g54(.a(new_n87_), .b(new_n84_), .c(new_n88_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n42_), .c(new_n45_), .O(z10));
  nor2   g56(.a(new_n37_), .b(new_n46_), .O(z11));
  nand2  g57(.a(x17), .b(x03), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n47_), .c(new_n42_), .O(z12));
  nand2  g59(.a(x17), .b(x04), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n45_), .O(z14));
endmodule


