// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n111_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x06), .b(x00), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x00), .O(new_n46_));
  nor2   g12(.a(x06), .b(new_n46_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g21(.a(x06), .b(new_n46_), .c(x07), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n50_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  oai22  g25(.a(new_n37_), .b(new_n59_), .c(new_n54_), .d(new_n53_), .O(new_n60_));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n60_), .c(new_n50_), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  inv1   g29(.a(new_n37_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  inv1   g31(.a(new_n61_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g33(.a(new_n61_), .b(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n67_), .c(new_n48_), .d(x04), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(new_n64_), .O(z05));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(x08), .b(x07), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(x09), .O(new_n73_));
  nor2   g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(x08), .c(x07), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand2  g43(.a(new_n47_), .b(new_n71_), .O(new_n78_));
  oai21  g44(.a(new_n77_), .b(new_n74_), .c(new_n78_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n50_), .O(z06));
  oai21  g46(.a(x11), .b(new_n46_), .c(new_n53_), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  nand3  g48(.a(new_n75_), .b(new_n66_), .c(new_n82_), .O(new_n83_));
  nand2  g49(.a(new_n76_), .b(x11), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n83_), .c(new_n81_), .d(new_n50_), .O(z07));
  nor2   g51(.a(x12), .b(x11), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n75_), .c(x08), .d(x07), .O(new_n87_));
  or2    g53(.a(new_n87_), .b(new_n53_), .O(new_n88_));
  nand2  g54(.a(new_n75_), .b(new_n82_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n72_), .c(x12), .O(new_n90_));
  oai21  g56(.a(x12), .b(new_n46_), .c(new_n53_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n88_), .d(new_n50_), .O(z08));
  inv1   g58(.a(x12), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n93_), .c(new_n82_), .d(new_n71_), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  aoi22  g62(.a(new_n96_), .b(new_n73_), .c(new_n87_), .d(x13), .O(new_n97_));
  nor2   g63(.a(new_n94_), .b(x06), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n49_), .c(new_n64_), .O(new_n99_));
  oai21  g65(.a(new_n97_), .b(new_n53_), .c(new_n99_), .O(z09));
  nand2  g66(.a(new_n95_), .b(new_n46_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n66_), .c(new_n48_), .d(new_n65_), .O(new_n102_));
  inv1   g68(.a(x03), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x02), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n64_), .c(x16), .d(x14), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n102_), .c(new_n43_), .O(z10));
  nor2   g72(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n36_), .c(x03), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n43_), .c(new_n64_), .O(z12));
  nor2   g75(.a(new_n37_), .b(new_n43_), .O(z13));
  nand2  g76(.a(x17), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n64_), .O(z14));
endmodule


