// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n106_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(x06), .b(new_n35_), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  nor2   g04(.a(x15), .b(x05), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x18), .O(new_n40_));
  inv1   g06(.a(x06), .O(new_n41_));
  nor2   g07(.a(x14), .b(new_n41_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n40_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n43_), .O(z02));
  xnor2a g16(.a(x07), .b(x06), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n49_), .c(new_n43_), .O(z03));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  xor2a  g19(.a(new_n53_), .b(x08), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n49_), .c(new_n43_), .O(z04));
  inv1   g21(.a(new_n49_), .O(new_n56_));
  nand4  g22(.a(x14), .b(x08), .c(x07), .d(x06), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(x09), .O(new_n58_));
  nand2  g24(.a(x08), .b(x07), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(x09), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n36_), .c(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n58_), .c(new_n56_), .O(z05));
  nand2  g28(.a(new_n61_), .b(x10), .O(new_n63_));
  inv1   g29(.a(x10), .O(new_n64_));
  nand4  g30(.a(new_n60_), .b(x14), .c(new_n64_), .d(x06), .O(new_n65_));
  aoi21  g31(.a(new_n48_), .b(x04), .c(new_n42_), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n63_), .O(z06));
  inv1   g34(.a(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(x08), .c(x07), .O(new_n70_));
  inv1   g36(.a(x11), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n64_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n70_), .c(x14), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(new_n65_), .b(x11), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n56_), .O(z07));
  nand2  g42(.a(new_n74_), .b(x12), .O(new_n77_));
  inv1   g43(.a(x12), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n71_), .c(new_n64_), .d(new_n69_), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(new_n57_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n66_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n77_), .O(z08));
  nor3   g48(.a(x12), .b(x11), .c(x10), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n60_), .c(new_n36_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n41_), .c(x13), .O(new_n85_));
  inv1   g51(.a(x13), .O(new_n86_));
  aoi21  g52(.a(new_n80_), .b(new_n86_), .c(new_n66_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n85_), .O(z09));
  inv1   g54(.a(x04), .O(new_n89_));
  inv1   g55(.a(x00), .O(new_n90_));
  nand3  g56(.a(new_n78_), .b(new_n71_), .c(new_n64_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(x13), .c(new_n90_), .O(new_n92_));
  inv1   g58(.a(x08), .O(new_n93_));
  nor3   g59(.a(new_n53_), .b(x09), .c(new_n93_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  inv1   g61(.a(x03), .O(new_n96_));
  inv1   g62(.a(x16), .O(new_n97_));
  aoi21  g63(.a(new_n96_), .b(x02), .c(new_n97_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(x14), .c(new_n42_), .O(new_n99_));
  aoi22  g65(.a(new_n99_), .b(new_n95_), .c(new_n43_), .d(new_n89_), .O(z10));
  nor2   g66(.a(new_n42_), .b(new_n47_), .O(z11));
  aoi21  g67(.a(x16), .b(new_n47_), .c(x03), .O(new_n102_));
  nand2  g68(.a(new_n43_), .b(x04), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n102_), .O(z12));
  nand2  g70(.a(new_n43_), .b(new_n89_), .O(z13));
  inv1   g71(.a(x17), .O(new_n106_));
  nor2   g72(.a(new_n103_), .b(new_n106_), .O(z14));
endmodule


