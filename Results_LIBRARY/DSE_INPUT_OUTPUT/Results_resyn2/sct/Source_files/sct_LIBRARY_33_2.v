// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n109_, new_n112_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x00), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n44_), .c(new_n37_), .O(z01));
  nor2   g13(.a(x06), .b(x00), .O(new_n48_));
  inv1   g14(.a(x02), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n49_), .c(x16), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n36_), .O(new_n56_));
  oai21  g22(.a(x06), .b(x00), .c(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n52_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  aoi22  g25(.a(new_n37_), .b(x08), .c(x07), .d(x06), .O(new_n60_));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(new_n62_));
  nor3   g28(.a(new_n62_), .b(new_n60_), .c(new_n51_), .O(z04));
  inv1   g29(.a(x09), .O(new_n64_));
  nand2  g30(.a(new_n62_), .b(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n61_), .b(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n65_), .c(new_n50_), .d(x04), .O(new_n67_));
  and2   g33(.a(new_n67_), .b(new_n37_), .O(z05));
  inv1   g34(.a(x10), .O(new_n69_));
  nand2  g35(.a(x08), .b(x07), .O(new_n70_));
  nor2   g36(.a(new_n70_), .b(x09), .O(new_n71_));
  nor2   g37(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  inv1   g38(.a(new_n70_), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand2  g42(.a(new_n48_), .b(new_n69_), .O(new_n77_));
  oai21  g43(.a(new_n76_), .b(new_n72_), .c(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n52_), .O(z06));
  oai21  g45(.a(x11), .b(x00), .c(new_n36_), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nand3  g47(.a(new_n74_), .b(new_n62_), .c(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n75_), .b(x11), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n82_), .c(new_n80_), .d(new_n52_), .O(z07));
  nor2   g50(.a(x12), .b(x11), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n74_), .c(new_n73_), .d(x06), .O(new_n86_));
  nand3  g52(.a(new_n74_), .b(new_n73_), .c(new_n81_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x12), .O(new_n88_));
  oai21  g54(.a(x12), .b(x00), .c(new_n36_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(new_n52_), .O(z08));
  nand3  g56(.a(new_n85_), .b(new_n74_), .c(new_n73_), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n92_), .c(new_n81_), .d(new_n69_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  aoi22  g61(.a(new_n95_), .b(new_n71_), .c(new_n91_), .d(x13), .O(new_n96_));
  nor2   g62(.a(new_n93_), .b(x06), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n51_), .c(new_n37_), .O(new_n98_));
  oai21  g64(.a(new_n96_), .b(new_n36_), .c(new_n98_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n94_), .b(new_n100_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n62_), .c(new_n50_), .d(new_n64_), .O(new_n102_));
  inv1   g68(.a(x03), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(x02), .O(new_n104_));
  nand4  g70(.a(new_n104_), .b(new_n37_), .c(x16), .d(x14), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n102_), .c(new_n45_), .O(z10));
  inv1   g72(.a(new_n37_), .O(new_n107_));
  nor2   g73(.a(new_n107_), .b(new_n49_), .O(z11));
  aoi21  g74(.a(x16), .b(new_n49_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n45_), .c(new_n37_), .O(z12));
  nor2   g76(.a(new_n107_), .b(new_n45_), .O(z13));
  nand2  g77(.a(x17), .b(x04), .O(new_n112_));
  nand2  g78(.a(new_n112_), .b(new_n37_), .O(z14));
endmodule


