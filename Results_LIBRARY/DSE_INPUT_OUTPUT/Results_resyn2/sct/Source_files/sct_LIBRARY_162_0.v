// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x12), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x12), .c(x06), .O(z02));
  inv1   g14(.a(x06), .O(new_n49_));
  nand2  g15(.a(x12), .b(x07), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  nand4  g18(.a(new_n52_), .b(new_n51_), .c(new_n46_), .d(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  xor2a  g20(.a(new_n52_), .b(x08), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n47_), .c(new_n38_), .O(z04));
  nand2  g22(.a(new_n47_), .b(new_n38_), .O(new_n57_));
  nand3  g23(.a(x12), .b(x09), .c(new_n49_), .O(new_n58_));
  inv1   g24(.a(x09), .O(new_n59_));
  aoi21  g25(.a(x08), .b(x07), .c(new_n59_), .O(new_n60_));
  nand2  g26(.a(x08), .b(x07), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(x09), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n60_), .c(x06), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n58_), .c(new_n57_), .O(z05));
  nand3  g30(.a(new_n59_), .b(x08), .c(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x10), .O(new_n66_));
  inv1   g32(.a(x10), .O(new_n67_));
  nand2  g33(.a(new_n62_), .b(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  nand3  g36(.a(x12), .b(x10), .c(new_n49_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n57_), .O(z06));
  inv1   g38(.a(x11), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n67_), .O(new_n74_));
  nor2   g40(.a(new_n74_), .b(new_n65_), .O(new_n75_));
  aoi21  g41(.a(new_n68_), .b(x11), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(x11), .b(new_n49_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n46_), .c(x04), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n38_), .O(new_n79_));
  oai21  g45(.a(new_n76_), .b(new_n49_), .c(new_n79_), .O(z07));
  inv1   g46(.a(x12), .O(new_n81_));
  nand3  g47(.a(new_n75_), .b(new_n81_), .c(x06), .O(new_n82_));
  nand2  g48(.a(new_n75_), .b(x06), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x12), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n82_), .c(new_n57_), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  nor2   g52(.a(x11), .b(x10), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n62_), .c(new_n86_), .O(new_n88_));
  nand3  g54(.a(new_n87_), .b(new_n86_), .c(new_n81_), .O(new_n89_));
  nor2   g55(.a(new_n89_), .b(new_n65_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n88_), .c(x06), .O(new_n91_));
  aoi22  g57(.a(new_n47_), .b(new_n38_), .c(x13), .d(x12), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n91_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n62_), .c(new_n46_), .d(x06), .O(new_n96_));
  inv1   g62(.a(x03), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x02), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n38_), .c(x16), .d(x14), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n96_), .c(new_n43_), .O(z10));
  nand2  g66(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g67(.a(x16), .b(new_n35_), .c(x03), .O(new_n102_));
  nand2  g68(.a(new_n38_), .b(x04), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n102_), .O(z12));
  inv1   g70(.a(new_n103_), .O(z13));
  inv1   g71(.a(x17), .O(new_n106_));
  nor2   g72(.a(new_n103_), .b(new_n106_), .O(z14));
endmodule


