// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  inv1   g03(.a(x08), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g09(.a(x15), .b(x05), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(x18), .O(new_n45_));
  nand2  g11(.a(x05), .b(x04), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n45_), .c(new_n39_), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z01));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n37_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n39_), .O(z02));
  inv1   g21(.a(x07), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(new_n58_));
  nand2  g24(.a(new_n56_), .b(new_n49_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n58_), .c(new_n53_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z03));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n58_), .b(new_n38_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n62_), .c(new_n53_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  inv1   g31(.a(x09), .O(new_n66_));
  aoi21  g32(.a(new_n57_), .b(new_n66_), .c(new_n37_), .O(new_n67_));
  aoi21  g33(.a(new_n62_), .b(x09), .c(new_n52_), .O(new_n68_));
  oai22  g34(.a(new_n68_), .b(new_n37_), .c(new_n67_), .d(new_n38_), .O(z05));
  nand4  g35(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nand4  g38(.a(new_n57_), .b(new_n72_), .c(new_n66_), .d(x08), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n53_), .O(z06));
  nor3   g40(.a(x11), .b(x10), .c(x09), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n57_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x04), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x08), .O(new_n78_));
  aoi21  g44(.a(new_n73_), .b(x11), .c(new_n52_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n37_), .c(new_n78_), .O(z07));
  inv1   g46(.a(x11), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n81_), .c(new_n72_), .d(new_n66_), .O(new_n83_));
  inv1   g49(.a(new_n83_), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n57_), .c(new_n37_), .O(new_n85_));
  inv1   g51(.a(new_n62_), .O(new_n86_));
  nand2  g52(.a(new_n75_), .b(new_n86_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(x12), .c(new_n52_), .O(new_n88_));
  oai22  g54(.a(new_n88_), .b(new_n37_), .c(new_n85_), .d(new_n38_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n84_), .b(new_n86_), .c(new_n90_), .O(new_n91_));
  oai21  g57(.a(new_n83_), .b(new_n62_), .c(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n53_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nand4  g60(.a(new_n90_), .b(new_n82_), .c(new_n81_), .d(new_n72_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor2   g62(.a(new_n70_), .b(new_n52_), .O(new_n97_));
  oai21  g63(.a(x03), .b(new_n35_), .c(x16), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n41_), .O(new_n99_));
  aoi21  g65(.a(new_n97_), .b(new_n96_), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n37_), .c(new_n39_), .O(z10));
  nand2  g67(.a(new_n39_), .b(new_n35_), .O(z11));
  nand2  g68(.a(x16), .b(new_n35_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n50_), .c(new_n37_), .O(z12));
  nand2  g70(.a(x08), .b(new_n37_), .O(z13));
  and2   g71(.a(x17), .b(x04), .O(z14));
endmodule


