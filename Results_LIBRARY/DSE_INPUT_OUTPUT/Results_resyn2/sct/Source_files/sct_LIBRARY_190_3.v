// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n99_, new_n102_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  or2    g06(.a(x14), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n36_), .O(z00));
  oai21  g08(.a(x15), .b(x05), .c(new_n37_), .O(new_n43_));
  aoi22  g09(.a(new_n43_), .b(x18), .c(x05), .d(x04), .O(z01));
  inv1   g10(.a(x03), .O(new_n45_));
  inv1   g11(.a(x16), .O(new_n46_));
  aoi21  g12(.a(new_n45_), .b(x02), .c(new_n46_), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n40_), .O(z02));
  xnor2a g16(.a(x07), .b(x06), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n49_), .c(new_n40_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g21(.a(x08), .b(x07), .c(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n40_), .O(new_n57_));
  nor2   g23(.a(new_n57_), .b(new_n49_), .O(z04));
  aoi21  g24(.a(x08), .b(x07), .c(x18), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n37_), .c(x09), .O(new_n60_));
  nand2  g26(.a(new_n49_), .b(new_n40_), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n38_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n61_), .c(new_n60_), .O(z05));
  nand3  g32(.a(new_n62_), .b(x08), .c(x07), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(x18), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand4  g37(.a(new_n68_), .b(new_n38_), .c(new_n71_), .d(x06), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n61_), .O(z06));
  inv1   g39(.a(new_n49_), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n71_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n67_), .c(new_n38_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x06), .O(new_n78_));
  nand2  g44(.a(new_n72_), .b(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n74_), .O(z07));
  nand2  g46(.a(new_n78_), .b(x12), .O(new_n81_));
  nor2   g47(.a(new_n76_), .b(x12), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n64_), .c(new_n38_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n61_), .O(z08));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(x13), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n75_), .d(new_n71_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n67_), .c(new_n38_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x06), .O(new_n89_));
  nand2  g55(.a(new_n83_), .b(x13), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n74_), .O(z09));
  inv1   g57(.a(x04), .O(new_n92_));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n87_), .b(new_n93_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n64_), .c(new_n48_), .O(new_n95_));
  aoi21  g61(.a(new_n47_), .b(x14), .c(new_n39_), .O(new_n96_));
  aoi22  g62(.a(new_n96_), .b(new_n95_), .c(new_n40_), .d(new_n92_), .O(z10));
  nor2   g63(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g64(.a(x16), .b(new_n35_), .c(x03), .O(new_n99_));
  nor3   g65(.a(new_n99_), .b(new_n39_), .c(new_n92_), .O(z12));
  nand2  g66(.a(new_n40_), .b(new_n92_), .O(z13));
  nand3  g67(.a(new_n40_), .b(x17), .c(x04), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(z14));
endmodule


