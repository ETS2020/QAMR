// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n100_, new_n102_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x13), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x09), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nor2   g14(.a(new_n38_), .b(new_n44_), .O(z13));
  nand3  g15(.a(z13), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n52_), .c(z13), .d(new_n48_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n52_), .b(new_n58_), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n57_), .c(z13), .d(new_n48_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(x09), .O(new_n62_));
  inv1   g28(.a(new_n57_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x13), .c(new_n62_), .O(new_n64_));
  nand2  g30(.a(new_n57_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x03), .O(new_n66_));
  inv1   g32(.a(x16), .O(new_n67_));
  aoi21  g33(.a(new_n66_), .b(x02), .c(new_n67_), .O(new_n68_));
  nor2   g34(.a(new_n68_), .b(new_n44_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n65_), .c(new_n64_), .O(z05));
  nand2  g36(.a(new_n64_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nand2  g38(.a(new_n37_), .b(new_n62_), .O(new_n73_));
  nor2   g39(.a(new_n73_), .b(new_n57_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  oai21  g41(.a(new_n68_), .b(new_n44_), .c(new_n39_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n71_), .O(z06));
  oai21  g43(.a(new_n57_), .b(x10), .c(new_n37_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n62_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x11), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nor2   g47(.a(x13), .b(x09), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n63_), .c(new_n81_), .d(new_n72_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n80_), .c(new_n76_), .O(z07));
  nand2  g50(.a(new_n83_), .b(x12), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n81_), .c(new_n72_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n57_), .c(new_n37_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n62_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n85_), .c(new_n69_), .O(z08));
  nor2   g56(.a(new_n87_), .b(new_n57_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n62_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n69_), .c(new_n37_), .O(z09));
  nand3  g59(.a(new_n68_), .b(new_n39_), .c(x14), .O(new_n94_));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n87_), .b(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n74_), .c(new_n48_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n94_), .c(new_n44_), .O(z10));
  nor2   g64(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n35_), .c(x03), .O(new_n100_));
  nor3   g66(.a(new_n100_), .b(new_n38_), .c(new_n44_), .O(z12));
  nand2  g67(.a(x17), .b(x04), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n39_), .O(z14));
endmodule


