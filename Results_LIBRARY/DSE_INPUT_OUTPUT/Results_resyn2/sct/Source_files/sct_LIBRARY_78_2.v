// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n106_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x07), .O(new_n37_));
  nor2   g03(.a(x18), .b(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(new_n38_), .O(new_n42_));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  nor2   g15(.a(new_n38_), .b(new_n49_), .O(z13));
  oai21  g16(.a(x03), .b(new_n36_), .c(x16), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(z13), .c(new_n48_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x03), .O(new_n54_));
  inv1   g20(.a(x16), .O(new_n55_));
  aoi21  g21(.a(new_n54_), .b(x02), .c(new_n55_), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  nand2  g23(.a(new_n37_), .b(new_n48_), .O(new_n58_));
  nand2  g24(.a(x07), .b(x06), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n42_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z03));
  inv1   g27(.a(new_n57_), .O(new_n62_));
  xor2a  g28(.a(new_n59_), .b(x08), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n62_), .c(new_n42_), .O(z04));
  inv1   g30(.a(x18), .O(new_n65_));
  aoi21  g31(.a(x08), .b(x06), .c(new_n65_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n37_), .c(x09), .O(new_n67_));
  nand2  g33(.a(new_n62_), .b(new_n42_), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x18), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n68_), .c(new_n67_), .O(z05));
  nand3  g39(.a(new_n69_), .b(x08), .c(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x18), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x07), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x10), .O(new_n77_));
  inv1   g43(.a(x10), .O(new_n78_));
  nand3  g44(.a(new_n71_), .b(x18), .c(new_n78_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n68_), .O(z06));
  nand2  g46(.a(new_n79_), .b(x11), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n69_), .c(x08), .d(x06), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x18), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x07), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n81_), .c(new_n57_), .O(z07));
  nand2  g52(.a(new_n85_), .b(x12), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nand4  g54(.a(new_n82_), .b(new_n71_), .c(x18), .d(new_n88_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n87_), .c(new_n68_), .O(z08));
  inv1   g56(.a(x13), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n83_), .c(x18), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x07), .O(new_n94_));
  nand2  g60(.a(new_n89_), .b(x13), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n94_), .c(new_n57_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand3  g63(.a(new_n82_), .b(new_n91_), .c(new_n88_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g65(.a(new_n70_), .b(new_n56_), .O(new_n100_));
  inv1   g66(.a(x14), .O(new_n101_));
  nor2   g67(.a(new_n51_), .b(new_n101_), .O(new_n102_));
  aoi21  g68(.a(new_n100_), .b(new_n99_), .c(new_n102_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n49_), .c(new_n42_), .O(z10));
  nor2   g70(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n36_), .c(x03), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n49_), .c(new_n42_), .O(z12));
  and2   g73(.a(z13), .b(x17), .O(z14));
endmodule


