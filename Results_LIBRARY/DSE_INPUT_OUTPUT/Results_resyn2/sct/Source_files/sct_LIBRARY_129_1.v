// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n105_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(x07), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x07), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  aoi21  g20(.a(new_n54_), .b(new_n48_), .c(x06), .O(z02));
  nand3  g21(.a(new_n53_), .b(new_n48_), .c(x06), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  nand2  g23(.a(x08), .b(x07), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n48_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n53_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n38_), .O(z04));
  inv1   g28(.a(x09), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x08), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x07), .O(new_n66_));
  nand2  g32(.a(new_n58_), .b(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n53_), .O(z05));
  aoi21  g34(.a(new_n65_), .b(x07), .c(x10), .O(new_n69_));
  nand3  g35(.a(new_n63_), .b(x08), .c(x07), .O(new_n70_));
  nand2  g36(.a(x10), .b(x06), .O(new_n71_));
  nor2   g37(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n69_), .c(new_n53_), .O(z06));
  oai21  g39(.a(new_n64_), .b(x10), .c(x11), .O(new_n74_));
  inv1   g40(.a(new_n70_), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  nand2  g45(.a(new_n54_), .b(new_n38_), .O(new_n80_));
  nand2  g46(.a(x11), .b(new_n48_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(z07));
  nand2  g48(.a(new_n77_), .b(x12), .O(new_n83_));
  nor3   g49(.a(x12), .b(x11), .c(x10), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n75_), .O(new_n85_));
  oai21  g51(.a(x12), .b(x07), .c(new_n37_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n83_), .d(new_n53_), .O(z08));
  inv1   g53(.a(x13), .O(new_n88_));
  nand2  g54(.a(new_n85_), .b(new_n88_), .O(new_n89_));
  nand4  g55(.a(new_n84_), .b(x13), .c(new_n63_), .d(x08), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(x06), .O(new_n91_));
  nand2  g57(.a(x13), .b(new_n48_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n80_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nor2   g60(.a(x13), .b(x12), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n76_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nor2   g63(.a(new_n70_), .b(new_n52_), .O(new_n98_));
  oai21  g64(.a(x03), .b(new_n35_), .c(x16), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n40_), .O(new_n100_));
  aoi21  g66(.a(new_n98_), .b(new_n97_), .c(new_n100_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n49_), .c(new_n38_), .O(z10));
  nand2  g68(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n35_), .c(x03), .O(new_n104_));
  nand2  g70(.a(new_n38_), .b(x04), .O(new_n105_));
  nor2   g71(.a(new_n105_), .b(new_n104_), .O(z12));
  inv1   g72(.a(new_n105_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  nor2   g74(.a(new_n105_), .b(new_n108_), .O(z14));
endmodule


