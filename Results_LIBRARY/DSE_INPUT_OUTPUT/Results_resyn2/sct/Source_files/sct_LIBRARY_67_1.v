// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n103_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x13), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n37_), .c(x06), .O(z02));
  xnor2a g15(.a(x07), .b(x06), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n48_), .c(new_n39_), .O(z03));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  xor2a  g18(.a(new_n52_), .b(x08), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n48_), .c(new_n39_), .O(z04));
  inv1   g20(.a(x09), .O(new_n55_));
  nand4  g21(.a(new_n55_), .b(x08), .c(x07), .d(x06), .O(new_n56_));
  inv1   g22(.a(x08), .O(new_n57_));
  oai21  g23(.a(new_n52_), .b(new_n57_), .c(x09), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n56_), .c(new_n47_), .d(x04), .O(new_n59_));
  and2   g25(.a(new_n59_), .b(new_n39_), .O(z05));
  nand2  g26(.a(x08), .b(x07), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(x09), .c(x10), .O(new_n62_));
  inv1   g28(.a(x10), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(new_n55_), .c(x08), .d(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  inv1   g32(.a(x06), .O(new_n67_));
  nand2  g33(.a(x10), .b(new_n67_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n47_), .c(x04), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  aoi21  g36(.a(new_n70_), .b(new_n66_), .c(new_n38_), .O(z06));
  nor2   g37(.a(new_n61_), .b(x09), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  aoi22  g39(.a(new_n73_), .b(new_n72_), .c(new_n64_), .d(x11), .O(new_n74_));
  nand2  g40(.a(x11), .b(new_n67_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n47_), .c(x04), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n39_), .O(new_n77_));
  oai21  g43(.a(new_n74_), .b(new_n67_), .c(new_n77_), .O(z07));
  inv1   g44(.a(new_n48_), .O(new_n79_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x12), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(x06), .O(new_n84_));
  or2    g50(.a(new_n84_), .b(new_n64_), .O(new_n85_));
  oai21  g51(.a(x13), .b(x12), .c(new_n67_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n81_), .d(new_n79_), .O(z08));
  oai21  g53(.a(new_n84_), .b(new_n64_), .c(x13), .O(new_n88_));
  nand3  g54(.a(new_n73_), .b(new_n37_), .c(new_n83_), .O(new_n89_));
  or2    g55(.a(new_n89_), .b(new_n56_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n88_), .c(new_n79_), .O(z09));
  inv1   g57(.a(x00), .O(new_n92_));
  nand2  g58(.a(new_n89_), .b(new_n92_), .O(new_n93_));
  inv1   g59(.a(x03), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x02), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(x16), .c(new_n67_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n93_), .c(new_n72_), .O(new_n97_));
  nand4  g63(.a(new_n95_), .b(new_n39_), .c(x16), .d(x14), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n97_), .c(new_n44_), .O(z10));
  nor2   g65(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n35_), .c(x03), .O(new_n101_));
  nor2   g67(.a(new_n38_), .b(new_n44_), .O(z13));
  inv1   g68(.a(z13), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n101_), .O(z12));
  and2   g70(.a(z13), .b(x17), .O(z14));
endmodule


