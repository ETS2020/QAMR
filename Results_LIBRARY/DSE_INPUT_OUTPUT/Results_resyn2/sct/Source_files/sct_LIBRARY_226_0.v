// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x12), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(x12), .c(new_n46_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(new_n50_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n38_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  oai21  g24(.a(new_n37_), .b(new_n58_), .c(new_n53_), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n59_), .c(new_n50_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  oai21  g28(.a(x03), .b(new_n35_), .c(x16), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  inv1   g30(.a(new_n60_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g32(.a(new_n60_), .b(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n66_), .c(new_n63_), .d(x04), .O(new_n68_));
  and2   g34(.a(new_n68_), .b(new_n38_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nor2   g36(.a(new_n60_), .b(x09), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g38(.a(new_n60_), .b(x09), .c(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n63_), .c(x04), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n72_), .c(new_n37_), .O(z06));
  nand2  g42(.a(x08), .b(x07), .O(new_n77_));
  nor2   g43(.a(new_n77_), .b(x09), .O(new_n78_));
  nand4  g44(.a(new_n70_), .b(new_n64_), .c(x08), .d(x07), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  aoi22  g46(.a(new_n80_), .b(new_n78_), .c(new_n79_), .d(x11), .O(new_n81_));
  nand2  g47(.a(x11), .b(new_n46_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n63_), .c(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n38_), .O(new_n84_));
  oai21  g50(.a(new_n81_), .b(new_n46_), .c(new_n84_), .O(z07));
  inv1   g51(.a(x12), .O(new_n86_));
  and2   g52(.a(x08), .b(x07), .O(new_n87_));
  nand4  g53(.a(new_n80_), .b(new_n87_), .c(new_n86_), .d(new_n64_), .O(new_n88_));
  nand2  g54(.a(new_n80_), .b(new_n78_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x12), .O(new_n90_));
  nor2   g56(.a(new_n46_), .b(new_n43_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n88_), .d(new_n63_), .O(z08));
  xor2a  g58(.a(new_n88_), .b(x13), .O(new_n93_));
  oai21  g59(.a(x13), .b(new_n86_), .c(new_n46_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n50_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nor2   g62(.a(x13), .b(x12), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n80_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n71_), .c(new_n63_), .O(new_n100_));
  nand3  g66(.a(new_n49_), .b(new_n38_), .c(x14), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n100_), .c(new_n43_), .O(z10));
  nor2   g68(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n35_), .c(x03), .O(new_n104_));
  nor3   g70(.a(new_n104_), .b(new_n37_), .c(new_n43_), .O(z12));
  nand2  g71(.a(new_n38_), .b(new_n43_), .O(z13));
  inv1   g72(.a(x17), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(new_n43_), .c(new_n38_), .O(z14));
endmodule


