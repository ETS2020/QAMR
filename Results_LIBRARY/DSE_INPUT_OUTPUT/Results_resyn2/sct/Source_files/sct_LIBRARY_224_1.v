// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n99_, new_n101_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  inv1   g02(.a(x18), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x12), .O(new_n38_));
  aoi21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(x02), .b(new_n35_), .c(new_n39_), .O(z00));
  inv1   g06(.a(x04), .O(new_n41_));
  nor2   g07(.a(new_n38_), .b(new_n41_), .O(z13));
  inv1   g08(.a(x12), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x05), .O(new_n44_));
  nor2   g10(.a(new_n37_), .b(x15), .O(new_n45_));
  aoi22  g11(.a(new_n45_), .b(new_n44_), .c(z13), .d(x05), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor3   g16(.a(new_n50_), .b(new_n38_), .c(new_n41_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n54_), .b(new_n47_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n51_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  inv1   g25(.a(new_n38_), .O(new_n60_));
  inv1   g26(.a(x02), .O(new_n61_));
  oai21  g27(.a(x03), .b(new_n61_), .c(x16), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x04), .O(new_n63_));
  nand2  g29(.a(new_n55_), .b(x08), .O(new_n64_));
  inv1   g30(.a(x08), .O(new_n65_));
  nand2  g31(.a(new_n56_), .b(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n63_), .c(new_n60_), .O(z04));
  nand2  g34(.a(new_n64_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(x08), .c(x07), .d(x06), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n69_), .c(new_n51_), .O(z05));
  nand2  g38(.a(new_n71_), .b(x10), .O(new_n73_));
  nor2   g39(.a(new_n71_), .b(x10), .O(new_n74_));
  nor2   g40(.a(new_n74_), .b(new_n63_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n73_), .c(new_n38_), .O(z06));
  oai21  g42(.a(new_n71_), .b(x10), .c(x11), .O(new_n77_));
  nor2   g43(.a(x09), .b(new_n65_), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n55_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n77_), .c(new_n51_), .O(z07));
  nor2   g47(.a(x18), .b(x12), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n79_), .c(new_n78_), .d(new_n55_), .O(new_n83_));
  aoi21  g49(.a(new_n80_), .b(x12), .c(new_n63_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n38_), .c(new_n83_), .O(z08));
  oai21  g51(.a(new_n80_), .b(x13), .c(new_n37_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n43_), .O(new_n87_));
  aoi21  g53(.a(new_n83_), .b(x13), .c(new_n63_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n87_), .O(z09));
  inv1   g55(.a(x00), .O(new_n90_));
  nor2   g56(.a(x13), .b(x12), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n79_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nor2   g59(.a(new_n71_), .b(new_n50_), .O(new_n94_));
  nor2   g60(.a(new_n62_), .b(new_n36_), .O(new_n95_));
  aoi21  g61(.a(new_n94_), .b(new_n93_), .c(new_n95_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n41_), .c(new_n60_), .O(z10));
  nand2  g63(.a(new_n60_), .b(new_n61_), .O(z11));
  aoi21  g64(.a(x16), .b(new_n61_), .c(x03), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n41_), .c(new_n60_), .O(z12));
  nand2  g66(.a(x17), .b(x04), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n60_), .O(z14));
endmodule


