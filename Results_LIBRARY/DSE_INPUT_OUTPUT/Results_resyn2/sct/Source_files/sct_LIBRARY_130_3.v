// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x06), .b(x00), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n42_), .c(new_n37_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  oai21  g20(.a(new_n46_), .b(x00), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(new_n50_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n37_), .O(new_n59_));
  oai21  g25(.a(x03), .b(new_n35_), .c(x16), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(x04), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n59_), .O(z04));
  oai21  g30(.a(new_n58_), .b(x00), .c(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand2  g32(.a(new_n59_), .b(new_n66_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n50_), .O(z05));
  inv1   g34(.a(x10), .O(new_n69_));
  nand2  g35(.a(new_n37_), .b(new_n69_), .O(new_n70_));
  inv1   g36(.a(x00), .O(new_n71_));
  nor2   g37(.a(new_n58_), .b(x09), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  inv1   g40(.a(new_n58_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n69_), .c(new_n66_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n50_), .O(z06));
  nand4  g43(.a(new_n72_), .b(x11), .c(new_n69_), .d(new_n71_), .O(new_n78_));
  nand2  g44(.a(new_n69_), .b(new_n66_), .O(new_n79_));
  aoi21  g45(.a(x06), .b(x00), .c(x11), .O(new_n80_));
  oai21  g46(.a(new_n79_), .b(new_n58_), .c(new_n80_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n50_), .O(z07));
  inv1   g49(.a(x11), .O(new_n84_));
  nand4  g50(.a(new_n75_), .b(new_n84_), .c(new_n69_), .d(new_n66_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x12), .O(new_n86_));
  nor2   g52(.a(x12), .b(x11), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n75_), .c(new_n69_), .d(new_n66_), .O(new_n88_));
  oai21  g54(.a(x12), .b(x06), .c(x00), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(new_n50_), .O(z08));
  inv1   g56(.a(x12), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n84_), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n69_), .c(new_n71_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n72_), .c(new_n60_), .d(x04), .O(new_n96_));
  nand2  g62(.a(new_n88_), .b(x13), .O(new_n97_));
  oai21  g63(.a(x13), .b(x06), .c(x00), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n60_), .c(x04), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n97_), .c(new_n96_), .O(z09));
  nor3   g67(.a(x13), .b(x10), .c(x00), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n87_), .c(new_n72_), .d(new_n60_), .O(new_n103_));
  nand3  g69(.a(new_n49_), .b(new_n37_), .c(x14), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n103_), .c(new_n43_), .O(z10));
  aoi21  g71(.a(x06), .b(x00), .c(new_n35_), .O(z11));
  nand2  g72(.a(x16), .b(new_n35_), .O(new_n107_));
  nand2  g73(.a(new_n37_), .b(x04), .O(new_n108_));
  aoi21  g74(.a(new_n107_), .b(new_n47_), .c(new_n108_), .O(z12));
  nand2  g75(.a(new_n37_), .b(new_n43_), .O(z13));
  inv1   g76(.a(x17), .O(new_n111_));
  oai21  g77(.a(new_n111_), .b(new_n43_), .c(new_n37_), .O(z14));
endmodule


