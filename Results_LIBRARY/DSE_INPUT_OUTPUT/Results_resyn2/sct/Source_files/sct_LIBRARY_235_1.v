// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand2  g02(.a(x06), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n36_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(z02));
  inv1   g19(.a(new_n50_), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n36_), .c(new_n47_), .O(new_n56_));
  nor2   g22(.a(new_n55_), .b(new_n47_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n37_), .O(z03));
  nand2  g26(.a(new_n57_), .b(x08), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n58_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n51_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  nand2  g31(.a(new_n61_), .b(x09), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n62_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n57_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n51_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nand3  g36(.a(new_n67_), .b(new_n57_), .c(new_n70_), .O(new_n71_));
  nand2  g37(.a(new_n68_), .b(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n51_), .O(z06));
  oai21  g39(.a(x11), .b(new_n36_), .c(new_n47_), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  nand4  g41(.a(new_n67_), .b(new_n57_), .c(new_n75_), .d(new_n70_), .O(new_n76_));
  inv1   g42(.a(x09), .O(new_n77_));
  nand4  g43(.a(new_n70_), .b(new_n77_), .c(x08), .d(x07), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n76_), .c(new_n54_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n74_), .O(z07));
  nand2  g48(.a(new_n76_), .b(x12), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n75_), .O(new_n85_));
  or2    g51(.a(new_n85_), .b(new_n71_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n83_), .c(new_n51_), .O(z08));
  oai21  g53(.a(x13), .b(new_n36_), .c(new_n47_), .O(new_n88_));
  oai21  g54(.a(new_n85_), .b(new_n78_), .c(x13), .O(new_n89_));
  nor2   g55(.a(x12), .b(x11), .O(new_n90_));
  nor2   g56(.a(x13), .b(x10), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n67_), .d(new_n57_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n89_), .c(new_n54_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x04), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n88_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n91_), .b(new_n90_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g64(.a(new_n48_), .b(x02), .O(new_n99_));
  nand3  g65(.a(new_n77_), .b(x08), .c(x07), .O(new_n100_));
  aoi21  g66(.a(new_n99_), .b(x16), .c(new_n100_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n98_), .c(new_n36_), .O(new_n102_));
  nand3  g68(.a(new_n50_), .b(x14), .c(x04), .O(new_n103_));
  oai21  g69(.a(new_n102_), .b(new_n47_), .c(new_n103_), .O(z10));
  inv1   g70(.a(x02), .O(new_n105_));
  aoi21  g71(.a(x06), .b(new_n36_), .c(new_n105_), .O(z11));
  nand2  g72(.a(x16), .b(new_n105_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n48_), .c(new_n36_), .O(z12));
  and2   g74(.a(x17), .b(x04), .O(z14));
  buf    g75(.a(x04), .O(z13));
endmodule


