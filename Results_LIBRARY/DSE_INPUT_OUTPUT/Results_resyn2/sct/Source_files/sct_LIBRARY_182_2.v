// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x18), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x15), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  nand2  g08(.a(new_n37_), .b(x04), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nor2   g10(.a(x15), .b(x05), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(x18), .c(new_n44_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x16), .c(new_n48_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n47_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n37_), .O(z03));
  inv1   g24(.a(x16), .O(new_n59_));
  aoi21  g25(.a(new_n49_), .b(x02), .c(new_n59_), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n43_), .O(new_n61_));
  inv1   g27(.a(new_n54_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x08), .O(new_n63_));
  inv1   g29(.a(x08), .O(new_n64_));
  nand2  g30(.a(new_n54_), .b(new_n64_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n61_), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(z04));
  nand2  g33(.a(new_n63_), .b(x09), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n61_), .O(z05));
  nand2  g37(.a(new_n70_), .b(x10), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand4  g39(.a(new_n62_), .b(new_n73_), .c(new_n69_), .d(x08), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n72_), .c(new_n51_), .O(new_n75_));
  and2   g41(.a(new_n75_), .b(new_n37_), .O(z06));
  inv1   g42(.a(x11), .O(new_n77_));
  inv1   g43(.a(new_n70_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n73_), .O(new_n79_));
  nand2  g45(.a(new_n74_), .b(x11), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n61_), .O(z07));
  nand2  g47(.a(new_n79_), .b(x12), .O(new_n82_));
  nor2   g48(.a(x12), .b(x11), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n78_), .c(new_n73_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n82_), .c(new_n61_), .O(z08));
  nand2  g51(.a(new_n84_), .b(x13), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand3  g53(.a(new_n83_), .b(new_n87_), .c(new_n73_), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n78_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n86_), .c(new_n61_), .O(z09));
  inv1   g57(.a(x00), .O(new_n92_));
  nand2  g58(.a(new_n88_), .b(new_n92_), .O(new_n93_));
  nor2   g59(.a(new_n70_), .b(new_n60_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g61(.a(new_n60_), .b(x14), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n95_), .c(new_n43_), .O(z10));
  inv1   g63(.a(x02), .O(new_n98_));
  nand2  g64(.a(new_n37_), .b(new_n98_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n98_), .c(x03), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(new_n43_), .O(z12));
  nand2  g67(.a(new_n37_), .b(new_n48_), .O(z13));
  inv1   g68(.a(x17), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n48_), .c(new_n37_), .O(z14));
endmodule


