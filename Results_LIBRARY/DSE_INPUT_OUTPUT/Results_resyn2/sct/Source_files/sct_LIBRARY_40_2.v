// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x17), .b(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(new_n38_), .O(new_n42_));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  nand2  g13(.a(x17), .b(x06), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n36_), .c(x16), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x04), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(new_n37_), .c(new_n49_), .O(z02));
  inv1   g18(.a(new_n51_), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n48_), .b(new_n54_), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n56_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n60_), .c(new_n42_), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n51_), .O(z04));
  nand2  g29(.a(new_n51_), .b(new_n42_), .O(new_n64_));
  inv1   g30(.a(x17), .O(new_n65_));
  nand2  g31(.a(x08), .b(x07), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n65_), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand3  g36(.a(new_n67_), .b(new_n49_), .c(new_n70_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n69_), .c(new_n64_), .O(z05));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n67_), .c(new_n65_), .O(new_n74_));
  aoi21  g40(.a(new_n71_), .b(x10), .c(new_n51_), .O(new_n75_));
  oai21  g41(.a(new_n74_), .b(new_n37_), .c(new_n75_), .O(z06));
  oai21  g42(.a(new_n74_), .b(new_n37_), .c(x11), .O(new_n77_));
  nand2  g43(.a(new_n73_), .b(new_n67_), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand2  g45(.a(new_n49_), .b(new_n79_), .O(new_n80_));
  or2    g46(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n77_), .c(new_n64_), .O(z07));
  nor2   g48(.a(x12), .b(x11), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n73_), .c(x08), .d(x07), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x17), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  oai21  g52(.a(new_n80_), .b(new_n78_), .c(x12), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n53_), .O(z08));
  nand2  g54(.a(new_n86_), .b(x13), .O(new_n89_));
  nor3   g55(.a(new_n66_), .b(new_n48_), .c(x09), .O(new_n90_));
  inv1   g56(.a(x10), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand3  g58(.a(new_n83_), .b(new_n92_), .c(new_n91_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  aoi22  g60(.a(new_n94_), .b(new_n90_), .c(new_n51_), .d(new_n42_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n89_), .O(z09));
  inv1   g62(.a(x04), .O(new_n97_));
  inv1   g63(.a(x03), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x02), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n42_), .c(x16), .d(x14), .O(new_n100_));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n93_), .b(new_n101_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n90_), .c(new_n50_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n100_), .c(new_n97_), .O(z10));
  nor2   g70(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n36_), .c(x03), .O(new_n106_));
  nor3   g72(.a(new_n106_), .b(new_n38_), .c(new_n97_), .O(z12));
  nand2  g73(.a(new_n42_), .b(new_n97_), .O(z13));
  nor2   g74(.a(new_n65_), .b(new_n97_), .O(z14));
endmodule


