// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n39_), .c(new_n36_), .O(z00));
  oai21  g09(.a(new_n38_), .b(x05), .c(new_n37_), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  nor2   g11(.a(new_n45_), .b(x15), .O(new_n46_));
  oai21  g12(.a(new_n46_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x04), .c(x06), .O(z02));
  oai21  g17(.a(x03), .b(new_n35_), .c(x16), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  oai21  g19(.a(new_n38_), .b(new_n37_), .c(new_n53_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n54_), .c(new_n52_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n39_), .O(z03));
  nor2   g23(.a(x08), .b(x07), .O(new_n58_));
  nand2  g24(.a(x08), .b(x07), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(x06), .c(x04), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  aoi21  g27(.a(x08), .b(new_n38_), .c(new_n61_), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n50_), .c(new_n39_), .O(z04));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(x08), .c(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  inv1   g33(.a(new_n59_), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(x06), .c(new_n64_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n50_), .c(x04), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n67_), .O(z05));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x04), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  nand2  g42(.a(new_n52_), .b(new_n76_), .O(new_n77_));
  nand4  g43(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n50_), .O(new_n79_));
  inv1   g45(.a(new_n79_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n77_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n75_), .O(z06));
  oai21  g48(.a(new_n73_), .b(new_n38_), .c(x11), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand4  g50(.a(new_n72_), .b(new_n68_), .c(new_n84_), .d(x06), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n83_), .c(new_n52_), .d(x04), .O(z07));
  nor2   g52(.a(x12), .b(x11), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n72_), .c(new_n68_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x04), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x06), .O(new_n90_));
  aoi21  g56(.a(new_n85_), .b(x12), .c(new_n50_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n37_), .c(new_n90_), .O(z08));
  nor2   g58(.a(x13), .b(x10), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n87_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n65_), .c(x04), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x06), .O(new_n96_));
  nand4  g62(.a(new_n87_), .b(new_n72_), .c(new_n68_), .d(x06), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(x13), .c(new_n50_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n37_), .c(new_n96_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n94_), .b(new_n100_), .O(new_n101_));
  nor2   g67(.a(new_n52_), .b(new_n41_), .O(new_n102_));
  aoi21  g68(.a(new_n101_), .b(new_n79_), .c(new_n102_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n37_), .c(new_n39_), .O(z10));
  nand2  g70(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n35_), .c(x03), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n37_), .c(new_n39_), .O(z12));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n39_), .O(z14));
  buf    g75(.a(x04), .O(z13));
endmodule


