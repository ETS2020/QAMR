// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n100_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x17), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x16), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  oai21  g14(.a(new_n48_), .b(x06), .c(new_n39_), .O(z02));
  inv1   g15(.a(x04), .O(new_n50_));
  inv1   g16(.a(x03), .O(new_n51_));
  inv1   g17(.a(x16), .O(new_n52_));
  aoi21  g18(.a(new_n51_), .b(x02), .c(new_n52_), .O(new_n53_));
  nor2   g19(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  or2    g20(.a(x07), .b(x06), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand4  g22(.a(new_n56_), .b(new_n55_), .c(new_n54_), .d(new_n39_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  xor2a  g24(.a(new_n56_), .b(x08), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n48_), .c(new_n39_), .O(z04));
  inv1   g26(.a(x09), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n62_), .c(new_n54_), .d(new_n39_), .O(z05));
  inv1   g31(.a(x10), .O(new_n66_));
  inv1   g32(.a(new_n62_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g34(.a(new_n62_), .b(x10), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n68_), .c(new_n54_), .d(new_n39_), .O(z06));
  oai21  g36(.a(new_n62_), .b(x10), .c(x11), .O(new_n71_));
  inv1   g37(.a(x11), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n66_), .O(new_n73_));
  nor2   g39(.a(new_n73_), .b(new_n62_), .O(new_n74_));
  nor2   g40(.a(new_n74_), .b(new_n48_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n71_), .c(new_n38_), .O(z07));
  oai21  g42(.a(new_n73_), .b(new_n62_), .c(x12), .O(new_n77_));
  inv1   g43(.a(x12), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n72_), .c(new_n66_), .d(new_n61_), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(new_n63_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n48_), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n77_), .c(new_n38_), .O(z08));
  inv1   g48(.a(x13), .O(new_n83_));
  nand3  g49(.a(new_n74_), .b(new_n83_), .c(new_n78_), .O(new_n84_));
  inv1   g50(.a(new_n63_), .O(new_n85_));
  nor2   g51(.a(x11), .b(x10), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n78_), .d(new_n61_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(x13), .c(new_n48_), .O(new_n88_));
  aoi21  g54(.a(new_n88_), .b(new_n84_), .c(new_n38_), .O(z09));
  inv1   g55(.a(x00), .O(new_n90_));
  nand3  g56(.a(new_n86_), .b(new_n83_), .c(new_n78_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n67_), .c(new_n47_), .O(new_n93_));
  aoi21  g59(.a(new_n53_), .b(x14), .c(new_n38_), .O(new_n94_));
  aoi22  g60(.a(new_n94_), .b(new_n93_), .c(new_n39_), .d(new_n50_), .O(z10));
  nand2  g61(.a(new_n39_), .b(new_n35_), .O(z11));
  nand2  g62(.a(new_n37_), .b(x03), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n47_), .c(new_n50_), .O(z12));
  nand2  g64(.a(new_n39_), .b(new_n50_), .O(z13));
  nand3  g65(.a(x17), .b(x16), .c(x04), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(z14));
endmodule


