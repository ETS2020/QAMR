// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x00), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g05(.a(x14), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  oai21  g06(.a(new_n36_), .b(new_n35_), .c(new_n40_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n38_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  oai21  g21(.a(new_n38_), .b(x00), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(new_n39_), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n54_), .b(new_n62_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n51_), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n61_), .O(z04));
  inv1   g31(.a(x09), .O(new_n66_));
  nand2  g32(.a(new_n61_), .b(new_n66_), .O(new_n67_));
  nand4  g33(.a(x08), .b(x07), .c(x06), .d(new_n37_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(new_n51_), .O(z05));
  nor2   g36(.a(new_n60_), .b(x09), .O(new_n71_));
  nor3   g37(.a(new_n71_), .b(new_n39_), .c(x10), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nor3   g39(.a(new_n68_), .b(new_n73_), .c(x09), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n72_), .c(new_n51_), .O(z06));
  nand4  g41(.a(new_n73_), .b(new_n66_), .c(x08), .d(x07), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x11), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  and2   g44(.a(x08), .b(x07), .O(new_n79_));
  nor2   g45(.a(x10), .b(x09), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(x06), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n77_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n37_), .O(new_n83_));
  oai21  g49(.a(x03), .b(new_n36_), .c(x16), .O(new_n84_));
  nand2  g50(.a(x11), .b(new_n38_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(x04), .O(new_n86_));
  inv1   g52(.a(new_n86_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n83_), .c(new_n39_), .O(z07));
  oai22  g54(.a(new_n81_), .b(x00), .c(new_n39_), .d(x12), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  inv1   g56(.a(new_n76_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n78_), .d(x06), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n89_), .c(new_n51_), .O(z08));
  nor2   g59(.a(x10), .b(x00), .O(new_n94_));
  nor3   g60(.a(x13), .b(x12), .c(x11), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(new_n71_), .d(new_n84_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x04), .O(new_n98_));
  nand2  g64(.a(new_n92_), .b(x13), .O(new_n99_));
  oai21  g65(.a(x13), .b(x06), .c(x00), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n51_), .O(new_n101_));
  inv1   g67(.a(new_n101_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n99_), .c(new_n98_), .O(z09));
  nand3  g69(.a(new_n50_), .b(new_n59_), .c(x14), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n96_), .c(new_n45_), .O(z10));
  nand2  g71(.a(new_n59_), .b(new_n36_), .O(z11));
  nand2  g72(.a(x16), .b(new_n36_), .O(new_n107_));
  nor2   g73(.a(new_n39_), .b(x03), .O(new_n108_));
  aoi22  g74(.a(new_n108_), .b(new_n107_), .c(new_n59_), .d(new_n45_), .O(z12));
  nand2  g75(.a(new_n59_), .b(new_n45_), .O(z13));
  nand2  g76(.a(x17), .b(x04), .O(new_n111_));
  nor2   g77(.a(new_n111_), .b(new_n39_), .O(z14));
endmodule


