// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n108_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x03), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x02), .O(new_n48_));
  nand2  g14(.a(x16), .b(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n47_), .c(x06), .O(z02));
  inv1   g17(.a(x16), .O(new_n52_));
  nand2  g18(.a(x06), .b(x03), .O(new_n53_));
  aoi21  g19(.a(new_n53_), .b(x02), .c(new_n52_), .O(new_n54_));
  inv1   g20(.a(x04), .O(new_n55_));
  aoi21  g21(.a(x07), .b(x06), .c(new_n55_), .O(new_n56_));
  oai21  g22(.a(x07), .b(x06), .c(new_n56_), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n54_), .c(new_n37_), .O(z03));
  oai21  g24(.a(x03), .b(new_n48_), .c(x16), .O(new_n59_));
  xor2a  g25(.a(x08), .b(x07), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(new_n59_), .c(new_n36_), .O(new_n61_));
  nand2  g27(.a(new_n37_), .b(new_n55_), .O(z13));
  inv1   g28(.a(x08), .O(new_n63_));
  aoi21  g29(.a(x16), .b(new_n48_), .c(new_n63_), .O(new_n64_));
  nand2  g30(.a(new_n36_), .b(new_n47_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n64_), .c(z13), .O(new_n66_));
  nor2   g32(.a(new_n66_), .b(new_n61_), .O(z04));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n59_), .b(new_n68_), .c(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n47_), .O(new_n70_));
  nand3  g36(.a(x08), .b(x07), .c(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n68_), .b(x08), .c(x07), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n72_), .c(new_n59_), .d(x04), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n70_), .b(new_n36_), .c(new_n75_), .O(z05));
  oai21  g42(.a(new_n71_), .b(x09), .c(x10), .O(new_n77_));
  aoi21  g43(.a(new_n47_), .b(x02), .c(new_n52_), .O(new_n78_));
  nand2  g44(.a(new_n37_), .b(x04), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  inv1   g46(.a(x10), .O(new_n81_));
  nor2   g47(.a(new_n71_), .b(x09), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n80_), .c(new_n77_), .O(z06));
  nand2  g50(.a(new_n83_), .b(x11), .O(new_n85_));
  nor2   g51(.a(x11), .b(x10), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n82_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n85_), .c(new_n80_), .O(z07));
  inv1   g54(.a(x12), .O(new_n89_));
  nand3  g55(.a(new_n86_), .b(new_n82_), .c(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n87_), .b(x12), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n90_), .c(new_n80_), .O(z08));
  nand2  g58(.a(new_n90_), .b(x13), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand3  g60(.a(new_n86_), .b(new_n94_), .c(new_n89_), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n82_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n93_), .c(new_n80_), .O(z09));
  inv1   g64(.a(z13), .O(new_n99_));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n95_), .b(new_n100_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n82_), .c(new_n59_), .O(new_n102_));
  inv1   g68(.a(new_n37_), .O(new_n103_));
  aoi21  g69(.a(new_n78_), .b(x14), .c(new_n103_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n102_), .c(new_n99_), .O(z10));
  nor2   g71(.a(new_n103_), .b(new_n48_), .O(z11));
  aoi21  g72(.a(new_n49_), .b(new_n47_), .c(new_n79_), .O(z12));
  inv1   g73(.a(x17), .O(new_n108_));
  nor2   g74(.a(new_n79_), .b(new_n108_), .O(z14));
endmodule


