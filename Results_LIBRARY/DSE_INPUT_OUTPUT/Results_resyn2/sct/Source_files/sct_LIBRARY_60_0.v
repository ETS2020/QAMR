// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n113_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n36_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n37_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nor2   g24(.a(new_n37_), .b(x04), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(new_n50_), .O(new_n60_));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n54_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n38_), .O(z04));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x08), .c(x07), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x04), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x06), .O(new_n69_));
  aoi21  g35(.a(new_n61_), .b(x09), .c(new_n50_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n36_), .c(new_n69_), .O(z05));
  nand4  g37(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  inv1   g40(.a(new_n72_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n73_), .c(new_n51_), .O(z06));
  nand2  g43(.a(new_n76_), .b(x11), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n51_), .O(z07));
  inv1   g47(.a(x02), .O(new_n82_));
  oai21  g48(.a(x03), .b(new_n82_), .c(x16), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n74_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n72_), .c(x12), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x04), .O(new_n88_));
  nand4  g54(.a(new_n79_), .b(new_n66_), .c(x08), .d(x07), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(x12), .c(x04), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n88_), .O(z08));
  inv1   g58(.a(x12), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n89_), .c(x04), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x06), .O(new_n97_));
  nand3  g63(.a(new_n79_), .b(new_n93_), .c(new_n66_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n61_), .c(x13), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n83_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x04), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n97_), .O(z09));
  inv1   g68(.a(x00), .O(new_n103_));
  oai21  g69(.a(new_n95_), .b(new_n85_), .c(new_n103_), .O(new_n104_));
  nor2   g70(.a(new_n72_), .b(new_n50_), .O(new_n105_));
  inv1   g71(.a(x14), .O(new_n106_));
  nor2   g72(.a(new_n83_), .b(new_n106_), .O(new_n107_));
  aoi21  g73(.a(new_n105_), .b(new_n104_), .c(new_n107_), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n36_), .c(new_n38_), .O(z10));
  nand2  g75(.a(new_n38_), .b(new_n82_), .O(z11));
  nand2  g76(.a(x16), .b(new_n82_), .O(new_n111_));
  aoi21  g77(.a(new_n111_), .b(new_n48_), .c(new_n36_), .O(z12));
  nor2   g78(.a(x17), .b(new_n36_), .O(new_n113_));
  nor2   g79(.a(new_n113_), .b(new_n59_), .O(z14));
  buf    g80(.a(x04), .O(z13));
endmodule


