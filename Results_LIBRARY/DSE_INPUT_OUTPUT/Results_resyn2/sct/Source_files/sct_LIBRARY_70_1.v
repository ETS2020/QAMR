// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  nand2  g03(.a(x06), .b(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  oai21  g06(.a(x06), .b(x05), .c(new_n37_), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g10(.a(x03), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x02), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x16), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  inv1   g14(.a(x06), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n48_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n50_), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n38_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  aoi21  g23(.a(new_n53_), .b(new_n50_), .c(new_n57_), .O(new_n58_));
  nor3   g24(.a(x08), .b(new_n52_), .c(new_n49_), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n58_), .c(new_n47_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n38_), .O(z04));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nor2   g30(.a(new_n48_), .b(new_n37_), .O(new_n65_));
  nand2  g31(.a(new_n62_), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(z05));
  nand2  g33(.a(new_n64_), .b(x10), .O(new_n68_));
  inv1   g34(.a(new_n62_), .O(new_n69_));
  nor2   g35(.a(x10), .b(x09), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z06));
  nand2  g38(.a(new_n71_), .b(x11), .O(new_n73_));
  inv1   g39(.a(x11), .O(new_n74_));
  nand3  g40(.a(new_n70_), .b(new_n69_), .c(new_n74_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n65_), .O(z07));
  oai21  g42(.a(x12), .b(new_n37_), .c(new_n49_), .O(new_n77_));
  inv1   g43(.a(x10), .O(new_n78_));
  nor2   g44(.a(x12), .b(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n63_), .c(new_n78_), .O(new_n80_));
  nand2  g46(.a(x08), .b(x07), .O(new_n81_));
  inv1   g47(.a(x09), .O(new_n82_));
  nand3  g48(.a(new_n74_), .b(new_n78_), .c(new_n82_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n81_), .c(x12), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n80_), .c(new_n47_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x04), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n77_), .O(z08));
  oai21  g53(.a(x13), .b(new_n37_), .c(new_n49_), .O(new_n88_));
  nor2   g54(.a(x13), .b(x10), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n79_), .c(new_n63_), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n74_), .c(new_n78_), .O(new_n92_));
  nand3  g58(.a(new_n82_), .b(x08), .c(x07), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n92_), .c(x13), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n90_), .c(new_n47_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x04), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n88_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n89_), .b(new_n79_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  aoi21  g66(.a(new_n46_), .b(x16), .c(new_n93_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n100_), .c(new_n37_), .O(new_n102_));
  nand3  g68(.a(new_n48_), .b(x14), .c(x04), .O(new_n103_));
  oai21  g69(.a(new_n102_), .b(new_n49_), .c(new_n103_), .O(z10));
  aoi21  g70(.a(x06), .b(new_n37_), .c(new_n35_), .O(z11));
  nand2  g71(.a(x16), .b(new_n35_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n45_), .c(new_n37_), .O(z12));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n38_), .O(z14));
  buf    g75(.a(x04), .O(z13));
endmodule


