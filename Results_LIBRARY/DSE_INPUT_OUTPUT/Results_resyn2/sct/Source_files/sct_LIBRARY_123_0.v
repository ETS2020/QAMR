// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n38_), .c(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  aoi21  g15(.a(x06), .b(x04), .c(x07), .O(new_n50_));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n50_), .c(new_n39_), .O(z03));
  nand2  g19(.a(x08), .b(new_n38_), .O(new_n54_));
  nor2   g20(.a(x08), .b(x07), .O(new_n55_));
  nand2  g21(.a(x08), .b(x07), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(x06), .c(x04), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n39_), .O(z04));
  inv1   g26(.a(x09), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x08), .c(x07), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n37_), .c(x06), .O(new_n64_));
  inv1   g30(.a(x03), .O(new_n65_));
  inv1   g31(.a(x16), .O(new_n66_));
  aoi21  g32(.a(new_n65_), .b(x02), .c(new_n66_), .O(new_n67_));
  and2   g33(.a(x08), .b(x07), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(x06), .c(new_n61_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n67_), .c(x04), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n64_), .O(z05));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x04), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand3  g41(.a(new_n63_), .b(new_n47_), .c(x06), .O(new_n76_));
  inv1   g42(.a(x10), .O(new_n77_));
  aoi21  g43(.a(new_n47_), .b(new_n77_), .c(new_n37_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n75_), .O(z06));
  oai21  g46(.a(new_n73_), .b(new_n38_), .c(x11), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  nand4  g48(.a(new_n72_), .b(new_n68_), .c(new_n82_), .d(x06), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n81_), .c(new_n47_), .d(x04), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n82_), .c(new_n77_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n62_), .c(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x06), .O(new_n88_));
  aoi21  g54(.a(new_n83_), .b(x12), .c(new_n67_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n37_), .c(new_n88_), .O(z08));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n85_), .c(new_n82_), .d(new_n77_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n62_), .c(x04), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x06), .O(new_n94_));
  nor2   g60(.a(x12), .b(x11), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n72_), .c(new_n68_), .d(x06), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(x13), .c(new_n67_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n37_), .c(new_n94_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n92_), .b(new_n99_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n63_), .c(new_n47_), .d(x06), .O(new_n101_));
  nand2  g67(.a(new_n67_), .b(x14), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n37_), .O(z10));
  aoi21  g69(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z11));
  nand2  g70(.a(x16), .b(new_n35_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n65_), .c(new_n37_), .O(z12));
  and2   g72(.a(x17), .b(x04), .O(z14));
  buf    g73(.a(x04), .O(z13));
endmodule


