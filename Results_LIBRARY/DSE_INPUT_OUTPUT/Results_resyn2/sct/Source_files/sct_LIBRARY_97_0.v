// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n101_,
    new_n104_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(x09), .b(x06), .O(new_n37_));
  aoi21  g03(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x03), .O(new_n45_));
  inv1   g11(.a(x16), .O(new_n46_));
  aoi21  g12(.a(new_n45_), .b(x02), .c(new_n46_), .O(new_n47_));
  nor2   g13(.a(new_n47_), .b(new_n42_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x09), .c(x06), .O(z02));
  inv1   g16(.a(new_n37_), .O(new_n51_));
  xnor2a g17(.a(x07), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n49_), .c(new_n51_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n37_), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n48_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  inv1   g25(.a(x09), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(x08), .c(x07), .O(new_n61_));
  inv1   g27(.a(x06), .O(new_n62_));
  nand2  g28(.a(x08), .b(x07), .O(new_n63_));
  aoi21  g29(.a(new_n63_), .b(x09), .c(new_n62_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n61_), .c(new_n48_), .O(z05));
  nand2  g31(.a(new_n63_), .b(x10), .O(new_n66_));
  inv1   g32(.a(x10), .O(new_n67_));
  inv1   g33(.a(new_n61_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  nand2  g37(.a(new_n49_), .b(new_n51_), .O(new_n72_));
  nand2  g38(.a(x10), .b(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(z06));
  oai22  g40(.a(new_n69_), .b(new_n62_), .c(new_n37_), .d(x11), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n68_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(new_n48_), .O(z07));
  xor2a  g44(.a(new_n77_), .b(x12), .O(new_n79_));
  oai21  g45(.a(x12), .b(new_n60_), .c(new_n62_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n48_), .O(z08));
  inv1   g47(.a(x13), .O(new_n82_));
  nor2   g48(.a(new_n63_), .b(x12), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n76_), .c(new_n82_), .O(new_n84_));
  nor2   g50(.a(x13), .b(x12), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n76_), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(new_n61_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n84_), .c(x06), .O(new_n88_));
  nand2  g54(.a(x13), .b(x09), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n88_), .c(new_n72_), .O(z09));
  inv1   g56(.a(x00), .O(new_n91_));
  nand2  g57(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nor2   g58(.a(new_n61_), .b(new_n47_), .O(new_n93_));
  inv1   g59(.a(x02), .O(new_n94_));
  oai21  g60(.a(x03), .b(new_n94_), .c(x16), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n36_), .O(new_n96_));
  aoi21  g62(.a(new_n93_), .b(new_n92_), .c(new_n96_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n42_), .c(new_n51_), .O(z10));
  nor2   g64(.a(new_n37_), .b(new_n94_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n94_), .c(x03), .O(new_n100_));
  nand2  g66(.a(new_n51_), .b(x04), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n100_), .O(z12));
  inv1   g68(.a(new_n101_), .O(z13));
  nand2  g69(.a(x17), .b(x04), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n51_), .O(z14));
endmodule


