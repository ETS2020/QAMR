// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n107_, new_n110_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  inv1   g02(.a(x17), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  nand2  g06(.a(x14), .b(new_n40_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(z00));
  nor2   g09(.a(x15), .b(x05), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n39_), .O(new_n46_));
  aoi21  g12(.a(new_n44_), .b(x18), .c(new_n46_), .O(z01));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n36_), .c(x04), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n39_), .O(z02));
  nand2  g17(.a(new_n49_), .b(x04), .O(new_n52_));
  xnor2a g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n52_), .c(new_n39_), .O(z03));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  xor2a  g21(.a(new_n55_), .b(x08), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n52_), .c(new_n39_), .O(z04));
  nand2  g23(.a(new_n52_), .b(new_n39_), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n39_), .c(x09), .O(new_n60_));
  and2   g26(.a(x08), .b(x07), .O(new_n61_));
  nor2   g27(.a(x09), .b(new_n36_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(new_n37_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n60_), .c(new_n58_), .O(z05));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x08), .c(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n37_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x10), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  nand2  g36(.a(x08), .b(x07), .O(new_n71_));
  nand3  g37(.a(new_n37_), .b(new_n65_), .c(x06), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n69_), .c(new_n58_), .O(z06));
  inv1   g41(.a(new_n52_), .O(new_n76_));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  inv1   g43(.a(new_n77_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n66_), .c(new_n37_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x06), .O(new_n80_));
  oai21  g46(.a(new_n63_), .b(x10), .c(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n76_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  nand4  g49(.a(new_n77_), .b(new_n61_), .c(new_n83_), .d(new_n65_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n37_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand4  g52(.a(new_n77_), .b(new_n62_), .c(new_n61_), .d(new_n37_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(x12), .c(new_n52_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n86_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n77_), .b(new_n90_), .c(new_n83_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n66_), .c(new_n37_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x06), .O(new_n93_));
  nand2  g59(.a(new_n77_), .b(new_n83_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n63_), .c(x13), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n93_), .c(new_n76_), .O(z09));
  inv1   g62(.a(new_n49_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x14), .O(new_n98_));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n91_), .b(new_n99_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n73_), .c(new_n49_), .O(new_n101_));
  nand2  g67(.a(new_n39_), .b(x04), .O(new_n102_));
  aoi21  g68(.a(new_n101_), .b(new_n98_), .c(new_n102_), .O(z10));
  nor2   g69(.a(new_n38_), .b(new_n48_), .O(z11));
  inv1   g70(.a(x04), .O(new_n105_));
  nand2  g71(.a(x16), .b(new_n48_), .O(new_n106_));
  nor2   g72(.a(new_n38_), .b(x03), .O(new_n107_));
  aoi22  g73(.a(new_n107_), .b(new_n106_), .c(new_n39_), .d(new_n105_), .O(z12));
  nand2  g74(.a(new_n39_), .b(new_n105_), .O(z13));
  nand3  g75(.a(x17), .b(new_n36_), .c(x04), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z14));
endmodule


