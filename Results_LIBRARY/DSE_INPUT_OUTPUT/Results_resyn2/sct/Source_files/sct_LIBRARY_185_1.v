// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x13), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n37_), .c(x06), .O(z02));
  xnor2a g15(.a(x07), .b(x06), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n48_), .c(new_n39_), .O(z03));
  inv1   g17(.a(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nor2   g19(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(x08), .O(new_n55_));
  nand2  g21(.a(new_n54_), .b(x08), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n39_), .O(new_n57_));
  nor3   g23(.a(new_n57_), .b(new_n55_), .c(new_n48_), .O(z04));
  inv1   g24(.a(new_n48_), .O(new_n59_));
  oai21  g25(.a(new_n38_), .b(x09), .c(new_n56_), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  inv1   g27(.a(new_n56_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n60_), .c(new_n59_), .O(z05));
  inv1   g30(.a(x10), .O(new_n65_));
  nand3  g31(.a(new_n62_), .b(new_n65_), .c(new_n61_), .O(new_n66_));
  nand2  g32(.a(x08), .b(x07), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(x09), .c(x10), .O(new_n68_));
  oai21  g34(.a(x13), .b(x10), .c(new_n52_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n68_), .c(new_n66_), .d(new_n59_), .O(z06));
  inv1   g36(.a(x11), .O(new_n71_));
  nand4  g37(.a(new_n65_), .b(new_n61_), .c(x08), .d(x07), .O(new_n72_));
  xor2a  g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(x11), .b(new_n52_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n47_), .c(x04), .O(new_n76_));
  inv1   g42(.a(new_n76_), .O(new_n77_));
  aoi21  g43(.a(new_n77_), .b(new_n74_), .c(new_n38_), .O(z07));
  inv1   g44(.a(new_n72_), .O(new_n79_));
  nor2   g45(.a(x10), .b(x09), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n71_), .c(x08), .d(x07), .O(new_n81_));
  nor2   g47(.a(x12), .b(x11), .O(new_n82_));
  aoi22  g48(.a(new_n82_), .b(new_n79_), .c(new_n81_), .d(x12), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(x06), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n48_), .c(new_n39_), .O(new_n86_));
  oai21  g52(.a(new_n83_), .b(new_n52_), .c(new_n86_), .O(z08));
  nand2  g53(.a(new_n82_), .b(new_n79_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x13), .O(new_n89_));
  nor2   g55(.a(new_n67_), .b(x09), .O(new_n90_));
  nand3  g56(.a(new_n82_), .b(new_n37_), .c(new_n65_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n90_), .c(new_n48_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(new_n89_), .c(z02), .O(z09));
  inv1   g60(.a(x04), .O(new_n95_));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n90_), .c(new_n47_), .d(x06), .O(new_n98_));
  inv1   g64(.a(new_n47_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n39_), .c(x14), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n98_), .c(new_n95_), .O(z10));
  nor2   g67(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n35_), .c(x03), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n95_), .c(new_n39_), .O(z12));
  nor2   g70(.a(new_n38_), .b(new_n95_), .O(z13));
  and2   g71(.a(z13), .b(x17), .O(z14));
endmodule


