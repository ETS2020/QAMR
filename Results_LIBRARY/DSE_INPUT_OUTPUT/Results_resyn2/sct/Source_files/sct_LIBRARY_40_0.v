// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x17), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x17), .c(x06), .O(z02));
  inv1   g17(.a(x04), .O(new_n52_));
  aoi21  g18(.a(new_n48_), .b(x16), .c(new_n52_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x06), .O(new_n55_));
  inv1   g21(.a(x07), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(new_n54_), .c(new_n53_), .d(new_n41_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n54_), .b(new_n61_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nor3   g29(.a(new_n63_), .b(new_n50_), .c(new_n37_), .O(z04));
  nor2   g30(.a(new_n60_), .b(x09), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n60_), .b(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n53_), .O(new_n68_));
  and2   g34(.a(new_n68_), .b(new_n41_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nand2  g36(.a(new_n65_), .b(new_n70_), .O(new_n71_));
  oai21  g37(.a(new_n37_), .b(x10), .c(new_n66_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n53_), .O(z06));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n65_), .O(new_n75_));
  inv1   g41(.a(x09), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(x08), .c(x07), .O(new_n77_));
  nand2  g43(.a(new_n70_), .b(x06), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n77_), .c(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n75_), .c(new_n53_), .O(new_n80_));
  and2   g46(.a(new_n80_), .b(new_n41_), .O(z07));
  nand2  g47(.a(new_n75_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand3  g49(.a(new_n74_), .b(new_n65_), .c(new_n83_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n82_), .c(new_n53_), .d(new_n41_), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(x06), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n50_), .c(new_n41_), .O(new_n88_));
  nand2  g54(.a(new_n74_), .b(new_n83_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n77_), .c(new_n86_), .O(new_n90_));
  inv1   g56(.a(new_n77_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n74_), .c(x13), .d(new_n83_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n90_), .c(x06), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n88_), .O(z09));
  nand4  g60(.a(new_n48_), .b(new_n41_), .c(x16), .d(x14), .O(new_n95_));
  inv1   g61(.a(x00), .O(new_n96_));
  nand3  g62(.a(new_n74_), .b(new_n86_), .c(new_n83_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n91_), .c(new_n49_), .d(x06), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n95_), .c(new_n52_), .O(z10));
  nor2   g66(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g67(.a(x16), .b(new_n36_), .c(x03), .O(new_n102_));
  nor3   g68(.a(new_n102_), .b(new_n37_), .c(new_n52_), .O(z12));
  nand2  g69(.a(new_n41_), .b(new_n52_), .O(z13));
  and2   g70(.a(x17), .b(x04), .O(z14));
endmodule


