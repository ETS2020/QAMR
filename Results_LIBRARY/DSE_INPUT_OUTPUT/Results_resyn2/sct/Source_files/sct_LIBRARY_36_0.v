// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  oai21  g08(.a(new_n37_), .b(x05), .c(new_n36_), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n36_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(x02), .O(new_n53_));
  oai21  g19(.a(x03), .b(new_n53_), .c(x16), .O(new_n54_));
  oai21  g20(.a(new_n37_), .b(x04), .c(new_n54_), .O(new_n55_));
  xnor2a g21(.a(x07), .b(x06), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n55_), .c(new_n38_), .O(z03));
  and2   g23(.a(x07), .b(x06), .O(new_n58_));
  xnor2a g24(.a(new_n58_), .b(x08), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n55_), .c(new_n38_), .O(z04));
  inv1   g26(.a(x09), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x08), .c(x07), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x04), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x06), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  aoi21  g31(.a(new_n65_), .b(x09), .c(new_n49_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n36_), .c(new_n64_), .O(z05));
  nand4  g33(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x10), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  inv1   g36(.a(new_n68_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n69_), .c(new_n50_), .O(z06));
  nand2  g39(.a(new_n72_), .b(x11), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n58_), .c(new_n61_), .d(x08), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n50_), .O(z07));
  inv1   g43(.a(x11), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n70_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n62_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  aoi21  g48(.a(new_n76_), .b(x12), .c(new_n49_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n36_), .c(new_n82_), .O(z08));
  inv1   g50(.a(x13), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n79_), .c(new_n78_), .d(new_n70_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n62_), .c(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x06), .O(new_n88_));
  inv1   g54(.a(new_n65_), .O(new_n89_));
  nor2   g55(.a(x12), .b(x09), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n75_), .c(new_n89_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(x13), .c(new_n49_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n36_), .c(new_n88_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n86_), .b(new_n94_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n71_), .c(new_n54_), .O(new_n96_));
  nand2  g62(.a(new_n49_), .b(x14), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n96_), .c(new_n36_), .O(z10));
  nand2  g64(.a(new_n38_), .b(new_n53_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n53_), .c(x03), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n36_), .c(new_n38_), .O(z12));
  and2   g67(.a(x17), .b(x04), .O(z14));
  buf    g68(.a(x04), .O(z13));
endmodule


