// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x08), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(new_n38_), .O(new_n42_));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n37_), .c(new_n48_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z02));
  inv1   g21(.a(new_n53_), .O(new_n56_));
  xnor2a g22(.a(x07), .b(x06), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n56_), .c(new_n42_), .O(z03));
  inv1   g24(.a(x07), .O(new_n59_));
  nand2  g25(.a(new_n37_), .b(new_n59_), .O(new_n60_));
  nand2  g26(.a(x08), .b(x07), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(new_n60_), .c(new_n53_), .d(x06), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  xor2a  g29(.a(new_n61_), .b(x09), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(new_n53_), .c(new_n38_), .O(z05));
  nand2  g31(.a(new_n56_), .b(new_n42_), .O(new_n66_));
  inv1   g32(.a(x10), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(x08), .c(x07), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n67_), .c(new_n38_), .O(new_n70_));
  oai21  g36(.a(new_n69_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n66_), .O(z06));
  nand2  g38(.a(new_n68_), .b(x07), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(x10), .c(x11), .O(new_n74_));
  inv1   g40(.a(new_n69_), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  nand2  g45(.a(x11), .b(new_n37_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n66_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand3  g48(.a(new_n76_), .b(new_n75_), .c(new_n82_), .O(new_n83_));
  inv1   g49(.a(new_n76_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n73_), .c(x12), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x06), .O(new_n87_));
  nand2  g53(.a(x12), .b(new_n37_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n66_), .O(z08));
  nand2  g55(.a(new_n83_), .b(x13), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n76_), .b(new_n75_), .c(new_n91_), .d(new_n82_), .O(new_n92_));
  oai21  g58(.a(x13), .b(x08), .c(new_n48_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n92_), .c(new_n90_), .d(new_n53_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand3  g61(.a(new_n76_), .b(new_n91_), .c(new_n82_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nor2   g63(.a(new_n69_), .b(new_n52_), .O(new_n98_));
  inv1   g64(.a(x14), .O(new_n99_));
  oai21  g65(.a(x03), .b(new_n36_), .c(x16), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  aoi21  g67(.a(new_n98_), .b(new_n97_), .c(new_n101_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n49_), .c(new_n42_), .O(z10));
  nor2   g69(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n36_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n49_), .c(new_n42_), .O(z12));
  nor2   g72(.a(new_n38_), .b(new_n49_), .O(z13));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n42_), .O(z14));
endmodule


