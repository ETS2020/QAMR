// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n96_, new_n97_, new_n98_, new_n100_;
  or2    g00(.a(x14), .b(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x01), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x04), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  nand2  g07(.a(x06), .b(new_n41_), .O(new_n42_));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  aoi22  g09(.a(new_n43_), .b(x18), .c(new_n42_), .d(x04), .O(z01));
  inv1   g10(.a(x07), .O(new_n46_));
  inv1   g11(.a(x03), .O(new_n47_));
  inv1   g12(.a(x16), .O(new_n48_));
  aoi21  g13(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nand2  g14(.a(x06), .b(x04), .O(new_n50_));
  nor2   g15(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g16(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  inv1   g17(.a(new_n52_), .O(z03));
  nand2  g18(.a(x08), .b(x07), .O(new_n54_));
  inv1   g19(.a(x08), .O(new_n55_));
  nand2  g20(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  nand3  g21(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  inv1   g22(.a(new_n57_), .O(z04));
  oai21  g23(.a(x03), .b(new_n36_), .c(x16), .O(new_n59_));
  nand2  g24(.a(new_n54_), .b(x09), .O(new_n60_));
  inv1   g25(.a(x09), .O(new_n61_));
  nand3  g26(.a(new_n61_), .b(x08), .c(x07), .O(new_n62_));
  nand3  g27(.a(new_n62_), .b(new_n60_), .c(new_n59_), .O(new_n63_));
  nand2  g28(.a(new_n63_), .b(x06), .O(new_n64_));
  nand2  g29(.a(new_n64_), .b(x04), .O(z05));
  inv1   g30(.a(x10), .O(new_n66_));
  nor2   g31(.a(new_n54_), .b(x09), .O(new_n67_));
  nand2  g32(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g33(.a(new_n62_), .b(x10), .O(new_n69_));
  nand3  g34(.a(new_n69_), .b(new_n68_), .c(new_n51_), .O(z06));
  nand2  g35(.a(new_n68_), .b(x11), .O(new_n71_));
  nor2   g36(.a(x11), .b(x10), .O(new_n72_));
  nand2  g37(.a(new_n72_), .b(new_n67_), .O(new_n73_));
  nand3  g38(.a(new_n73_), .b(new_n71_), .c(new_n51_), .O(z07));
  inv1   g39(.a(x11), .O(new_n75_));
  nand2  g40(.a(new_n75_), .b(new_n66_), .O(new_n76_));
  oai21  g41(.a(new_n76_), .b(new_n62_), .c(x12), .O(new_n77_));
  nor3   g42(.a(x12), .b(x11), .c(x10), .O(new_n78_));
  nand2  g43(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  nand3  g44(.a(new_n79_), .b(new_n77_), .c(new_n59_), .O(new_n80_));
  nand2  g45(.a(new_n80_), .b(x06), .O(new_n81_));
  nand2  g46(.a(new_n81_), .b(x04), .O(z08));
  inv1   g47(.a(x13), .O(new_n83_));
  aoi21  g48(.a(new_n78_), .b(new_n67_), .c(new_n83_), .O(new_n84_));
  nor2   g49(.a(x13), .b(x12), .O(new_n85_));
  nand2  g50(.a(new_n85_), .b(new_n72_), .O(new_n86_));
  oai21  g51(.a(new_n86_), .b(new_n62_), .c(new_n59_), .O(new_n87_));
  oai21  g52(.a(new_n87_), .b(new_n84_), .c(x06), .O(new_n88_));
  nand2  g53(.a(new_n88_), .b(x04), .O(z09));
  nand2  g54(.a(new_n49_), .b(x14), .O(new_n90_));
  inv1   g55(.a(x00), .O(new_n91_));
  nand2  g56(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nand3  g57(.a(new_n92_), .b(new_n67_), .c(new_n59_), .O(new_n93_));
  aoi21  g58(.a(new_n93_), .b(new_n90_), .c(new_n50_), .O(z10));
  nand2  g59(.a(new_n39_), .b(new_n36_), .O(z11));
  inv1   g60(.a(x04), .O(new_n96_));
  nand2  g61(.a(x16), .b(new_n36_), .O(new_n97_));
  nor2   g62(.a(new_n38_), .b(x03), .O(new_n98_));
  aoi21  g63(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(z12));
  nand3  g64(.a(x17), .b(x06), .c(x04), .O(new_n100_));
  inv1   g65(.a(new_n100_), .O(z14));
  zero   g66(.O(z02));
  buf    g67(.a(x04), .O(z13));
endmodule


