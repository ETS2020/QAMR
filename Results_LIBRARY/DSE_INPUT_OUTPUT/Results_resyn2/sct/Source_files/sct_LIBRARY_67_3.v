// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n99_, new_n101_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  inv1   g03(.a(x13), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  or2    g06(.a(x14), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n40_), .O(z02));
  xnor2a g16(.a(x07), .b(x06), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n49_), .c(new_n40_), .O(z03));
  aoi21  g18(.a(x07), .b(x06), .c(x08), .O(new_n53_));
  nand3  g19(.a(x08), .b(x07), .c(x06), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(new_n55_));
  or2    g21(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n49_), .c(new_n40_), .O(z04));
  inv1   g23(.a(x09), .O(new_n58_));
  nand2  g24(.a(new_n55_), .b(new_n58_), .O(new_n59_));
  nand2  g25(.a(new_n54_), .b(x09), .O(new_n60_));
  nand4  g26(.a(new_n60_), .b(new_n59_), .c(new_n48_), .d(x04), .O(new_n61_));
  and2   g27(.a(new_n61_), .b(new_n40_), .O(z05));
  nand2  g28(.a(x08), .b(x07), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(x09), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(x13), .c(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x10), .O(new_n66_));
  inv1   g32(.a(x10), .O(new_n67_));
  nor2   g33(.a(x13), .b(new_n37_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n64_), .c(new_n67_), .O(new_n69_));
  nand2  g35(.a(new_n49_), .b(new_n40_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n66_), .O(z06));
  nand4  g37(.a(new_n67_), .b(new_n58_), .c(x08), .d(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n38_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x11), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  nand4  g42(.a(new_n68_), .b(new_n64_), .c(new_n76_), .d(new_n67_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(new_n70_), .O(z07));
  inv1   g44(.a(new_n49_), .O(new_n79_));
  nand2  g45(.a(new_n77_), .b(x12), .O(new_n80_));
  nand3  g46(.a(new_n58_), .b(x08), .c(x07), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nor2   g48(.a(x11), .b(x10), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n81_), .c(new_n38_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n80_), .c(new_n79_), .O(z08));
  nand4  g53(.a(new_n83_), .b(new_n55_), .c(new_n82_), .d(new_n58_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n79_), .c(new_n38_), .O(z09));
  nor2   g55(.a(new_n39_), .b(new_n45_), .O(z13));
  inv1   g56(.a(z13), .O(new_n91_));
  inv1   g57(.a(new_n48_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x14), .O(new_n93_));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n84_), .b(new_n94_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n68_), .c(new_n64_), .d(new_n48_), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n93_), .c(new_n91_), .O(z10));
  nor2   g63(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g64(.a(x16), .b(new_n35_), .c(x03), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n91_), .O(z12));
  nand2  g66(.a(z13), .b(x17), .O(new_n101_));
  inv1   g67(.a(new_n101_), .O(z14));
endmodule


