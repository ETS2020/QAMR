// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x04), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x04), .O(new_n42_));
  oai21  g08(.a(x06), .b(x05), .c(new_n42_), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n39_), .O(z02));
  oai21  g18(.a(x03), .b(new_n35_), .c(x16), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n42_), .c(new_n37_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n39_), .O(z03));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n54_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n50_), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand2  g31(.a(new_n64_), .b(x04), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(new_n59_), .c(new_n49_), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(new_n65_), .c(new_n38_), .O(z05));
  oai21  g34(.a(x10), .b(new_n42_), .c(new_n37_), .O(new_n69_));
  nor2   g35(.a(new_n65_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(x08), .b(x07), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(x09), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(new_n71_), .c(new_n53_), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n70_), .c(x04), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n69_), .O(z06));
  oai21  g42(.a(x11), .b(new_n42_), .c(new_n37_), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n73_), .c(x06), .O(new_n79_));
  nand2  g45(.a(new_n71_), .b(new_n64_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n72_), .c(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n79_), .c(new_n53_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n77_), .O(z07));
  oai21  g50(.a(x12), .b(new_n42_), .c(new_n37_), .O(new_n85_));
  nand3  g51(.a(new_n64_), .b(x08), .c(x07), .O(new_n86_));
  inv1   g52(.a(x11), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n71_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n86_), .c(x12), .O(new_n89_));
  inv1   g55(.a(new_n59_), .O(new_n90_));
  nor2   g56(.a(x12), .b(x11), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n71_), .d(new_n64_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n89_), .c(new_n53_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x04), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n85_), .O(z08));
  nand2  g61(.a(new_n92_), .b(x13), .O(new_n96_));
  inv1   g62(.a(new_n65_), .O(new_n97_));
  inv1   g63(.a(x13), .O(new_n98_));
  nand3  g64(.a(new_n91_), .b(new_n98_), .c(new_n71_), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n97_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n96_), .c(new_n50_), .O(z09));
  nand2  g68(.a(new_n49_), .b(x14), .O(new_n103_));
  inv1   g69(.a(x00), .O(new_n104_));
  nand2  g70(.a(new_n99_), .b(new_n104_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n97_), .c(new_n53_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n103_), .c(new_n42_), .O(z10));
  nor2   g73(.a(new_n38_), .b(new_n35_), .O(z11));
  nand2  g74(.a(x16), .b(new_n35_), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(new_n47_), .c(new_n42_), .O(z12));
  nand2  g76(.a(new_n37_), .b(new_n42_), .O(z13));
  and2   g77(.a(x17), .b(x04), .O(z14));
endmodule


