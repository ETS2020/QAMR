// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n93_, new_n94_,
    new_n97_, new_n99_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(x12), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n37_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x12), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n36_), .O(new_n49_));
  inv1   g15(.a(x02), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n50_), .c(x16), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(x04), .O(new_n52_));
  aoi21  g18(.a(new_n52_), .b(new_n36_), .c(new_n49_), .O(z02));
  inv1   g19(.a(x03), .O(new_n54_));
  inv1   g20(.a(x16), .O(new_n55_));
  aoi21  g21(.a(new_n54_), .b(x02), .c(new_n55_), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(new_n45_), .O(new_n57_));
  inv1   g23(.a(x07), .O(new_n58_));
  oai21  g24(.a(new_n48_), .b(new_n36_), .c(new_n58_), .O(new_n59_));
  nand2  g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z03));
  inv1   g28(.a(x08), .O(new_n63_));
  nand2  g29(.a(new_n60_), .b(new_n63_), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n64_), .c(new_n38_), .O(new_n66_));
  nor2   g32(.a(new_n66_), .b(new_n52_), .O(z04));
  and2   g33(.a(x08), .b(x07), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n48_), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  nand2  g36(.a(new_n52_), .b(new_n38_), .O(new_n71_));
  nand2  g37(.a(x08), .b(x07), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n49_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n71_), .c(new_n70_), .O(z05));
  oai21  g41(.a(new_n73_), .b(new_n48_), .c(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x10), .O(new_n77_));
  inv1   g43(.a(x10), .O(new_n78_));
  nand3  g44(.a(new_n73_), .b(new_n49_), .c(new_n78_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n71_), .O(z06));
  nor2   g46(.a(x10), .b(x09), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n68_), .c(new_n48_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n36_), .c(x11), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand4  g50(.a(new_n81_), .b(new_n68_), .c(new_n49_), .d(new_n84_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n83_), .c(new_n71_), .O(z07));
  nand2  g52(.a(new_n48_), .b(new_n36_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n57_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nor2   g56(.a(new_n37_), .b(new_n45_), .O(z13));
  nand3  g57(.a(z13), .b(new_n51_), .c(new_n90_), .O(z09));
  nand4  g58(.a(new_n73_), .b(new_n51_), .c(new_n49_), .d(x00), .O(new_n93_));
  nand3  g59(.a(new_n56_), .b(new_n38_), .c(x14), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n93_), .c(new_n45_), .O(z10));
  nor2   g61(.a(new_n37_), .b(new_n50_), .O(z11));
  aoi21  g62(.a(x16), .b(new_n50_), .c(x03), .O(new_n97_));
  nor3   g63(.a(new_n97_), .b(new_n37_), .c(new_n45_), .O(z12));
  nand2  g64(.a(x17), .b(x04), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n38_), .O(z14));
endmodule


