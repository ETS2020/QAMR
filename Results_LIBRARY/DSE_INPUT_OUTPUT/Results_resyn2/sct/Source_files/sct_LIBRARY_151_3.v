// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n75_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x06), .b(x04), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x06), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x04), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x05), .O(new_n44_));
  inv1   g10(.a(x05), .O(new_n45_));
  inv1   g11(.a(x18), .O(new_n46_));
  oai21  g12(.a(new_n46_), .b(x15), .c(new_n45_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n44_), .c(new_n37_), .O(z01));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n43_), .O(z02));
  inv1   g18(.a(x04), .O(new_n53_));
  oai21  g19(.a(x03), .b(new_n35_), .c(x16), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x07), .O(new_n55_));
  aoi21  g21(.a(new_n55_), .b(new_n42_), .c(new_n53_), .O(z03));
  nand2  g22(.a(new_n54_), .b(x08), .O(new_n57_));
  aoi21  g23(.a(new_n57_), .b(new_n42_), .c(new_n53_), .O(z04));
  oai21  g24(.a(new_n51_), .b(x09), .c(new_n42_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x04), .O(z05));
  inv1   g26(.a(x10), .O(new_n61_));
  nand2  g27(.a(z02), .b(new_n61_), .O(z06));
  inv1   g28(.a(x11), .O(new_n63_));
  nand2  g29(.a(z02), .b(new_n63_), .O(z07));
  oai21  g30(.a(new_n51_), .b(x12), .c(new_n42_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x04), .O(z08));
  oai21  g32(.a(new_n51_), .b(x13), .c(new_n42_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x04), .O(z09));
  nor3   g34(.a(new_n54_), .b(new_n43_), .c(new_n39_), .O(z10));
  aoi21  g35(.a(x06), .b(x04), .c(new_n35_), .O(z11));
  nand2  g36(.a(x16), .b(new_n35_), .O(new_n71_));
  nor2   g37(.a(x06), .b(x03), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(new_n71_), .c(new_n53_), .O(z12));
  inv1   g39(.a(new_n43_), .O(z13));
  nand2  g40(.a(z13), .b(x17), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(z14));
endmodule


