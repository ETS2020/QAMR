// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x13), .b(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n38_), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n37_), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand4  g22(.a(new_n56_), .b(new_n55_), .c(new_n50_), .d(new_n53_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  oai21  g24(.a(x03), .b(new_n36_), .c(x16), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x04), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  inv1   g28(.a(new_n56_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x08), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n60_), .c(new_n53_), .O(z04));
  nand3  g32(.a(new_n63_), .b(x13), .c(x08), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  inv1   g34(.a(x13), .O(new_n69_));
  nand2  g35(.a(x08), .b(x07), .O(new_n70_));
  nor2   g36(.a(new_n70_), .b(x09), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n69_), .c(x06), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n68_), .c(new_n50_), .O(z05));
  nand2  g39(.a(new_n72_), .b(x10), .O(new_n74_));
  nand2  g40(.a(new_n60_), .b(new_n53_), .O(new_n75_));
  inv1   g41(.a(new_n67_), .O(new_n76_));
  nor2   g42(.a(x10), .b(x09), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n75_), .c(new_n74_), .O(z06));
  inv1   g45(.a(new_n70_), .O(new_n80_));
  aoi21  g46(.a(new_n77_), .b(new_n80_), .c(new_n69_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n37_), .c(x11), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand3  g49(.a(new_n77_), .b(new_n76_), .c(new_n83_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n82_), .c(new_n75_), .O(z07));
  nor2   g51(.a(x11), .b(x10), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n71_), .c(new_n69_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n37_), .c(x12), .O(new_n88_));
  inv1   g54(.a(x09), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nand4  g56(.a(x13), .b(new_n90_), .c(new_n89_), .d(x08), .O(new_n91_));
  nand3  g57(.a(new_n86_), .b(x07), .c(x06), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  aoi21  g59(.a(new_n60_), .b(new_n53_), .c(new_n93_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n88_), .O(z08));
  nand3  g61(.a(new_n86_), .b(new_n90_), .c(new_n89_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n64_), .c(x13), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n75_), .O(z09));
  nand3  g64(.a(x13), .b(x06), .c(x00), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n71_), .c(new_n59_), .O(new_n101_));
  nand3  g67(.a(new_n49_), .b(new_n53_), .c(x14), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n44_), .O(z10));
  nor2   g69(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n36_), .c(x03), .O(new_n105_));
  nor3   g71(.a(new_n105_), .b(new_n38_), .c(new_n44_), .O(z12));
  nand2  g72(.a(new_n53_), .b(new_n44_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n44_), .c(new_n53_), .O(z14));
endmodule


