// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n112_, new_n115_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x08), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  nor2   g20(.a(x07), .b(x06), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n55_), .c(new_n37_), .O(z03));
  nand3  g24(.a(new_n56_), .b(new_n52_), .c(x08), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  inv1   g26(.a(x09), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(x07), .O(new_n62_));
  nand3  g28(.a(new_n61_), .b(x07), .c(x06), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n62_), .c(x08), .O(new_n65_));
  nand2  g31(.a(x09), .b(new_n47_), .O(new_n66_));
  oai21  g32(.a(new_n51_), .b(new_n48_), .c(new_n37_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(z05));
  inv1   g34(.a(x07), .O(new_n69_));
  oai21  g35(.a(x09), .b(new_n69_), .c(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(new_n64_), .b(new_n71_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x08), .O(new_n74_));
  nand2  g40(.a(x10), .b(new_n47_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n67_), .O(z06));
  inv1   g42(.a(x11), .O(new_n77_));
  nand2  g43(.a(new_n72_), .b(new_n77_), .O(new_n78_));
  nor2   g44(.a(x09), .b(new_n69_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(x11), .c(new_n71_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(x08), .O(new_n81_));
  nand2  g47(.a(x11), .b(new_n47_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n67_), .O(z07));
  nor2   g49(.a(x11), .b(x10), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n79_), .O(new_n87_));
  nand2  g53(.a(new_n84_), .b(new_n79_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n87_), .c(x08), .O(new_n90_));
  nand2  g56(.a(x12), .b(new_n47_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n90_), .c(new_n67_), .O(z08));
  inv1   g58(.a(x13), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n85_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n88_), .c(x08), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x06), .O(new_n96_));
  nand3  g62(.a(new_n85_), .b(new_n77_), .c(x08), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n72_), .c(x13), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n96_), .c(new_n52_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand3  g66(.a(new_n84_), .b(new_n93_), .c(new_n85_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nor2   g68(.a(new_n63_), .b(new_n51_), .O(new_n103_));
  inv1   g69(.a(x14), .O(new_n104_));
  inv1   g70(.a(x02), .O(new_n105_));
  oai21  g71(.a(x03), .b(new_n105_), .c(x16), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g73(.a(new_n103_), .b(new_n102_), .c(new_n107_), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n48_), .c(new_n37_), .O(z10));
  inv1   g75(.a(new_n37_), .O(new_n110_));
  nor2   g76(.a(new_n110_), .b(new_n105_), .O(z11));
  aoi21  g77(.a(x16), .b(new_n105_), .c(x03), .O(new_n112_));
  oai21  g78(.a(new_n112_), .b(new_n48_), .c(new_n37_), .O(z12));
  nor2   g79(.a(new_n110_), .b(new_n48_), .O(z13));
  nand2  g80(.a(x17), .b(x04), .O(new_n115_));
  nand2  g81(.a(new_n115_), .b(new_n37_), .O(z14));
endmodule


