// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n114_,
    new_n115_, new_n118_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  inv1   g03(.a(x17), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g05(.a(x14), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  oai21  g06(.a(new_n36_), .b(new_n35_), .c(new_n40_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor3   g16(.a(new_n50_), .b(x06), .c(new_n45_), .O(z02));
  inv1   g17(.a(new_n39_), .O(new_n52_));
  oai21  g18(.a(x03), .b(new_n36_), .c(x16), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x04), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n37_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand2  g23(.a(new_n55_), .b(new_n37_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n54_), .c(new_n52_), .O(z03));
  nand2  g26(.a(new_n56_), .b(x08), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n57_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n52_), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n54_), .O(z04));
  aoi21  g31(.a(new_n53_), .b(x04), .c(new_n39_), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nor2   g35(.a(new_n69_), .b(x17), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  aoi21  g37(.a(x08), .b(x07), .c(x17), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n37_), .c(x09), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n67_), .O(z05));
  nand3  g40(.a(new_n68_), .b(x08), .c(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n38_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x10), .O(new_n78_));
  inv1   g44(.a(x10), .O(new_n79_));
  inv1   g45(.a(new_n61_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n38_), .c(new_n79_), .d(new_n68_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n78_), .c(new_n67_), .O(z06));
  oai21  g48(.a(new_n75_), .b(x10), .c(new_n38_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x11), .O(new_n85_));
  nor2   g51(.a(x11), .b(x10), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n70_), .c(new_n66_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n85_), .O(z07));
  inv1   g54(.a(new_n86_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n75_), .c(new_n38_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x12), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  nand2  g59(.a(new_n86_), .b(new_n93_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n70_), .c(new_n66_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n92_), .O(z08));
  oai21  g63(.a(new_n94_), .b(new_n75_), .c(new_n38_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x06), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x13), .O(new_n100_));
  inv1   g66(.a(x13), .O(new_n101_));
  nand3  g67(.a(new_n86_), .b(new_n101_), .c(new_n93_), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n70_), .c(new_n66_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n100_), .O(z09));
  inv1   g71(.a(x00), .O(new_n106_));
  nand2  g72(.a(new_n102_), .b(new_n106_), .O(new_n107_));
  nor2   g73(.a(new_n69_), .b(new_n50_), .O(new_n108_));
  inv1   g74(.a(x14), .O(new_n109_));
  nor2   g75(.a(new_n53_), .b(new_n109_), .O(new_n110_));
  aoi21  g76(.a(new_n108_), .b(new_n107_), .c(new_n110_), .O(new_n111_));
  oai21  g77(.a(new_n111_), .b(new_n45_), .c(new_n52_), .O(z10));
  nor2   g78(.a(new_n39_), .b(new_n36_), .O(z11));
  aoi21  g79(.a(x16), .b(new_n36_), .c(x03), .O(new_n114_));
  nand2  g80(.a(new_n52_), .b(x04), .O(new_n115_));
  nor2   g81(.a(new_n115_), .b(new_n114_), .O(z12));
  inv1   g82(.a(new_n115_), .O(z13));
  nand3  g83(.a(x17), .b(new_n37_), .c(x04), .O(new_n118_));
  inv1   g84(.a(new_n118_), .O(z14));
endmodule


