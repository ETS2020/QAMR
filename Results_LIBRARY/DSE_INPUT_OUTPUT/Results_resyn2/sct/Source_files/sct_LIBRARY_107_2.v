// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n76_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n86_, new_n87_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x10), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x07), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  aoi21  g13(.a(new_n37_), .b(x07), .c(new_n47_), .O(z13));
  oai21  g14(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(z13), .c(new_n46_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  oai21  g19(.a(new_n37_), .b(x06), .c(x07), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n53_), .c(new_n49_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  oai21  g23(.a(new_n52_), .b(new_n46_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n58_), .c(new_n49_), .d(x04), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n38_), .O(z04));
  inv1   g27(.a(x03), .O(new_n62_));
  inv1   g28(.a(x16), .O(new_n63_));
  aoi21  g29(.a(new_n62_), .b(x02), .c(new_n63_), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n47_), .c(new_n38_), .O(new_n65_));
  aoi21  g31(.a(x08), .b(x06), .c(new_n37_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n52_), .c(x09), .O(new_n67_));
  inv1   g33(.a(new_n59_), .O(new_n68_));
  nor2   g34(.a(new_n37_), .b(x09), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n67_), .c(new_n65_), .O(z05));
  oai21  g37(.a(new_n59_), .b(x09), .c(x10), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n65_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  nand3  g40(.a(new_n49_), .b(z13), .c(new_n74_), .O(z07));
  nand2  g41(.a(new_n38_), .b(x12), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n65_), .O(z08));
  inv1   g43(.a(x13), .O(new_n78_));
  nand3  g44(.a(new_n49_), .b(z13), .c(new_n78_), .O(z09));
  nand3  g45(.a(new_n64_), .b(new_n38_), .c(x14), .O(new_n80_));
  nand4  g46(.a(x08), .b(x07), .c(x06), .d(x00), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n69_), .c(new_n49_), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n80_), .c(new_n47_), .O(z10));
  aoi21  g50(.a(new_n37_), .b(x07), .c(new_n35_), .O(z11));
  inv1   g51(.a(z13), .O(new_n86_));
  aoi21  g52(.a(x16), .b(new_n35_), .c(x03), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(new_n86_), .O(z12));
  and2   g54(.a(z13), .b(x17), .O(z14));
endmodule


