// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x08), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(new_n38_), .O(new_n42_));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n36_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n37_), .c(new_n48_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  xnor2a g19(.a(x07), .b(x06), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n50_), .c(new_n42_), .O(z03));
  inv1   g21(.a(x07), .O(new_n56_));
  nand2  g22(.a(new_n37_), .b(new_n56_), .O(new_n57_));
  nand2  g23(.a(x08), .b(x07), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n57_), .c(new_n51_), .d(x06), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  xor2a  g26(.a(new_n58_), .b(x09), .O(new_n61_));
  aoi21  g27(.a(new_n61_), .b(new_n51_), .c(new_n38_), .O(z05));
  oai21  g28(.a(x09), .b(new_n56_), .c(x10), .O(new_n63_));
  inv1   g29(.a(x10), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x08), .c(x07), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n63_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  nand2  g36(.a(new_n50_), .b(new_n42_), .O(new_n71_));
  nand2  g37(.a(x10), .b(new_n37_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(z06));
  oai22  g39(.a(new_n68_), .b(new_n48_), .c(new_n38_), .d(x11), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n67_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n51_), .O(z07));
  xor2a  g43(.a(new_n76_), .b(x12), .O(new_n78_));
  oai21  g44(.a(x12), .b(x08), .c(new_n48_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n51_), .O(z08));
  inv1   g46(.a(x12), .O(new_n81_));
  nand4  g47(.a(new_n75_), .b(new_n81_), .c(new_n65_), .d(x07), .O(new_n82_));
  inv1   g48(.a(x13), .O(new_n83_));
  nand3  g49(.a(new_n75_), .b(new_n83_), .c(new_n81_), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  aoi22  g51(.a(new_n85_), .b(new_n67_), .c(new_n82_), .d(x13), .O(new_n86_));
  aoi22  g52(.a(new_n50_), .b(new_n42_), .c(x13), .d(new_n37_), .O(new_n87_));
  oai21  g53(.a(new_n86_), .b(new_n48_), .c(new_n87_), .O(z09));
  inv1   g54(.a(x04), .O(new_n89_));
  inv1   g55(.a(x00), .O(new_n90_));
  nand2  g56(.a(new_n84_), .b(new_n90_), .O(new_n91_));
  inv1   g57(.a(x03), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x02), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(x16), .c(new_n66_), .O(new_n94_));
  inv1   g60(.a(x14), .O(new_n95_));
  nor2   g61(.a(new_n49_), .b(new_n95_), .O(new_n96_));
  aoi21  g62(.a(new_n94_), .b(new_n91_), .c(new_n96_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n89_), .c(new_n42_), .O(z10));
  nand2  g64(.a(new_n42_), .b(new_n36_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n36_), .c(x03), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n89_), .c(new_n42_), .O(z12));
  nor2   g67(.a(new_n38_), .b(new_n89_), .O(z13));
  and2   g68(.a(z13), .b(x17), .O(z14));
endmodule


