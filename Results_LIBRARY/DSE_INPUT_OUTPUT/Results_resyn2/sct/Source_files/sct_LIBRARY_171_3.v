// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n105_, new_n108_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  inv1   g02(.a(x12), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  nand2  g06(.a(x14), .b(new_n40_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(z00));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  aoi21  g12(.a(x05), .b(new_n46_), .c(new_n38_), .O(new_n47_));
  oai21  g13(.a(new_n45_), .b(x05), .c(new_n47_), .O(z01));
  inv1   g14(.a(x02), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n49_), .c(x16), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x04), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(x06), .c(new_n39_), .O(z02));
  inv1   g18(.a(x03), .O(new_n53_));
  inv1   g19(.a(x16), .O(new_n54_));
  aoi21  g20(.a(new_n53_), .b(x02), .c(new_n54_), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  inv1   g22(.a(x07), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n36_), .O(new_n58_));
  oai21  g24(.a(x12), .b(x07), .c(x06), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n58_), .c(new_n56_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z03));
  inv1   g27(.a(x08), .O(new_n62_));
  oai21  g28(.a(new_n57_), .b(new_n36_), .c(new_n62_), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n51_), .c(new_n39_), .O(z04));
  oai21  g32(.a(new_n64_), .b(x12), .c(x09), .O(new_n67_));
  nand2  g33(.a(x08), .b(x07), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(x09), .c(new_n37_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n67_), .c(new_n56_), .O(z05));
  nand2  g37(.a(new_n70_), .b(x10), .O(new_n72_));
  nand2  g38(.a(new_n51_), .b(new_n39_), .O(new_n73_));
  nor2   g39(.a(new_n64_), .b(x12), .O(new_n74_));
  nor2   g40(.a(x10), .b(x09), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n73_), .c(new_n72_), .O(z06));
  inv1   g43(.a(x11), .O(new_n78_));
  nand4  g44(.a(new_n75_), .b(new_n78_), .c(x08), .d(x07), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n37_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  nand2  g47(.a(new_n76_), .b(x11), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n56_), .O(z07));
  nor2   g49(.a(new_n68_), .b(x11), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n75_), .c(x06), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n56_), .c(new_n37_), .O(z08));
  nand3  g52(.a(new_n75_), .b(new_n74_), .c(new_n78_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x13), .O(new_n88_));
  inv1   g54(.a(x09), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(x08), .c(x07), .O(new_n90_));
  inv1   g56(.a(x10), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n78_), .c(new_n91_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n90_), .c(new_n37_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x06), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n88_), .c(new_n56_), .O(z09));
  nand2  g62(.a(new_n55_), .b(x14), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n74_), .c(new_n50_), .d(new_n89_), .O(new_n100_));
  nand2  g66(.a(new_n39_), .b(x04), .O(new_n101_));
  aoi21  g67(.a(new_n100_), .b(new_n97_), .c(new_n101_), .O(z10));
  nor2   g68(.a(new_n38_), .b(new_n49_), .O(z11));
  nand2  g69(.a(x16), .b(new_n49_), .O(new_n104_));
  nor2   g70(.a(new_n38_), .b(x03), .O(new_n105_));
  aoi22  g71(.a(new_n105_), .b(new_n104_), .c(new_n39_), .d(new_n46_), .O(z12));
  nand2  g72(.a(new_n39_), .b(new_n46_), .O(z13));
  nand3  g73(.a(new_n39_), .b(x17), .c(x04), .O(new_n108_));
  inv1   g74(.a(new_n108_), .O(z14));
endmodule


