// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n107_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x15), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x15), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x05), .c(x06), .O(new_n43_));
  aoi22  g09(.a(new_n43_), .b(new_n41_), .c(x05), .d(x04), .O(z01));
  inv1   g10(.a(x04), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x02), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x16), .c(new_n45_), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(x06), .O(z02));
  inv1   g16(.a(new_n37_), .O(new_n51_));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(new_n48_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n51_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  oai21  g24(.a(new_n37_), .b(new_n58_), .c(new_n52_), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n59_), .c(new_n48_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n64_));
  nand2  g30(.a(new_n60_), .b(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n64_), .c(new_n48_), .O(new_n66_));
  and2   g32(.a(new_n66_), .b(new_n51_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nand2  g34(.a(x08), .b(x07), .O(new_n69_));
  nor2   g35(.a(new_n69_), .b(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(x06), .O(new_n71_));
  nand2  g37(.a(new_n64_), .b(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n48_), .O(new_n73_));
  and2   g39(.a(new_n73_), .b(new_n51_), .O(z06));
  nand4  g40(.a(new_n68_), .b(new_n63_), .c(x08), .d(x07), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  aoi22  g42(.a(new_n76_), .b(new_n70_), .c(new_n75_), .d(x11), .O(new_n77_));
  oai21  g43(.a(x03), .b(new_n36_), .c(x16), .O(new_n78_));
  nand2  g44(.a(x11), .b(new_n53_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n51_), .O(new_n81_));
  oai21  g47(.a(new_n77_), .b(new_n53_), .c(new_n81_), .O(z07));
  nand4  g48(.a(new_n76_), .b(new_n63_), .c(x08), .d(x07), .O(new_n83_));
  xor2a  g49(.a(new_n83_), .b(x12), .O(new_n84_));
  nand2  g50(.a(x12), .b(new_n53_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n78_), .c(x04), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n51_), .O(new_n87_));
  oai21  g53(.a(new_n84_), .b(new_n53_), .c(new_n87_), .O(z08));
  oai21  g54(.a(new_n83_), .b(x12), .c(x13), .O(new_n89_));
  inv1   g55(.a(new_n64_), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand3  g58(.a(new_n76_), .b(new_n92_), .c(new_n91_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n90_), .O(new_n95_));
  oai21  g61(.a(new_n41_), .b(x13), .c(new_n53_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n95_), .c(new_n89_), .d(new_n48_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n90_), .c(new_n78_), .O(new_n100_));
  nand4  g66(.a(new_n47_), .b(new_n51_), .c(x16), .d(x14), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n100_), .c(new_n45_), .O(z10));
  nor2   g68(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n36_), .c(x03), .O(new_n104_));
  nor3   g70(.a(new_n104_), .b(new_n37_), .c(new_n45_), .O(z12));
  nor2   g71(.a(new_n37_), .b(new_n45_), .O(z13));
  nand2  g72(.a(x17), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n51_), .O(z14));
endmodule


