// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_,
    new_n78_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n89_,
    new_n91_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n37_), .O(z02));
  inv1   g16(.a(x06), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  oai21  g19(.a(new_n36_), .b(x07), .c(x06), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n53_), .c(new_n48_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  oai21  g24(.a(new_n52_), .b(new_n51_), .c(new_n58_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n49_), .c(new_n37_), .O(z04));
  inv1   g27(.a(x04), .O(new_n62_));
  inv1   g28(.a(x03), .O(new_n63_));
  inv1   g29(.a(x16), .O(new_n64_));
  aoi21  g30(.a(new_n63_), .b(x02), .c(new_n64_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n62_), .c(new_n37_), .O(new_n66_));
  aoi21  g32(.a(x08), .b(x07), .c(new_n36_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n51_), .c(x09), .O(new_n68_));
  inv1   g34(.a(new_n57_), .O(new_n69_));
  nor2   g35(.a(new_n36_), .b(x09), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n66_), .O(z05));
  oai21  g38(.a(new_n57_), .b(x09), .c(x10), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n66_), .O(z06));
  nand2  g40(.a(new_n37_), .b(x11), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n66_), .O(z07));
  inv1   g42(.a(x12), .O(new_n77_));
  nor2   g43(.a(x10), .b(new_n51_), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n62_), .O(z13));
  nand3  g45(.a(z13), .b(new_n48_), .c(new_n77_), .O(z08));
  inv1   g46(.a(x13), .O(new_n81_));
  nand3  g47(.a(z13), .b(new_n48_), .c(new_n81_), .O(z09));
  nand3  g48(.a(new_n65_), .b(new_n37_), .c(x14), .O(new_n83_));
  nand4  g49(.a(x08), .b(x07), .c(x06), .d(x00), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n70_), .c(new_n48_), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n83_), .c(new_n62_), .O(z10));
  nor2   g53(.a(new_n78_), .b(new_n47_), .O(z11));
  aoi21  g54(.a(x16), .b(new_n47_), .c(x03), .O(new_n89_));
  nor3   g55(.a(new_n89_), .b(new_n78_), .c(new_n62_), .O(z12));
  nand2  g56(.a(x17), .b(x04), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n37_), .O(z14));
endmodule


