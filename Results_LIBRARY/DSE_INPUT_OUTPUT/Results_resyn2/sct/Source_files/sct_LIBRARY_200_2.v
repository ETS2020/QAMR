// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x08), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n47_), .c(x04), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x08), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x06), .O(new_n55_));
  inv1   g21(.a(new_n53_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(x07), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n55_), .O(z03));
  nor2   g24(.a(x08), .b(x06), .O(new_n59_));
  aoi21  g25(.a(new_n53_), .b(x08), .c(new_n59_), .O(z04));
  xor2a  g26(.a(new_n52_), .b(x09), .O(new_n61_));
  aoi21  g27(.a(new_n61_), .b(new_n49_), .c(new_n38_), .O(z05));
  inv1   g28(.a(x10), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nand3  g32(.a(new_n63_), .b(new_n64_), .c(x07), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x08), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(x06), .c(new_n48_), .O(new_n69_));
  oai21  g35(.a(new_n66_), .b(new_n63_), .c(new_n69_), .O(z06));
  oai21  g36(.a(new_n65_), .b(x10), .c(x11), .O(new_n71_));
  nor2   g37(.a(x11), .b(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n64_), .c(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x08), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n71_), .c(new_n49_), .O(z07));
  nand2  g42(.a(new_n72_), .b(new_n66_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x12), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nand4  g45(.a(new_n72_), .b(new_n79_), .c(new_n64_), .d(x07), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x08), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n78_), .c(new_n49_), .O(z08));
  inv1   g49(.a(x08), .O(new_n84_));
  nor2   g50(.a(new_n52_), .b(x09), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n79_), .c(new_n86_), .d(new_n63_), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  aoi22  g55(.a(new_n89_), .b(new_n85_), .c(new_n80_), .d(x13), .O(new_n90_));
  nor2   g56(.a(new_n87_), .b(x06), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n48_), .c(new_n39_), .O(new_n92_));
  oai21  g58(.a(new_n90_), .b(new_n84_), .c(new_n92_), .O(z09));
  inv1   g59(.a(x04), .O(new_n94_));
  inv1   g60(.a(x03), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x02), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n39_), .c(x16), .d(x14), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n88_), .b(new_n98_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n66_), .c(new_n47_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n97_), .c(new_n94_), .O(z10));
  nand2  g67(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n35_), .c(x03), .O(new_n103_));
  nor3   g69(.a(new_n103_), .b(new_n38_), .c(new_n94_), .O(z12));
  nor2   g70(.a(new_n38_), .b(new_n94_), .O(z13));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n39_), .O(z14));
endmodule


