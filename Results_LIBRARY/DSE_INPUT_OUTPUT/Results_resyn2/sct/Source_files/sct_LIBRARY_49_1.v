// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_, new_n110_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(x13), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x15), .c(new_n43_), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nand2  g12(.a(x05), .b(new_n46_), .O(new_n47_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(new_n38_), .O(z01));
  oai21  g14(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nor2   g17(.a(x13), .b(x06), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  xnor2a g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n50_), .c(new_n38_), .O(z03));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  xor2a  g23(.a(new_n57_), .b(x08), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n50_), .c(new_n38_), .O(z04));
  nand2  g25(.a(new_n50_), .b(new_n38_), .O(new_n60_));
  nand2  g26(.a(new_n52_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  aoi21  g28(.a(x08), .b(x07), .c(new_n62_), .O(new_n63_));
  nand2  g29(.a(x08), .b(x07), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(x09), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n63_), .c(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n61_), .c(new_n60_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nor2   g34(.a(new_n65_), .b(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n65_), .b(new_n68_), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n69_), .c(x06), .O(new_n72_));
  nand2  g38(.a(new_n52_), .b(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n60_), .O(z06));
  nand2  g40(.a(new_n70_), .b(x11), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand4  g42(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n77_));
  inv1   g43(.a(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  inv1   g46(.a(x13), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n37_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n79_), .c(new_n75_), .d(new_n51_), .O(z07));
  nand2  g50(.a(new_n76_), .b(new_n65_), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n68_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x11), .O(new_n88_));
  aoi22  g54(.a(new_n88_), .b(new_n65_), .c(new_n85_), .d(x12), .O(new_n89_));
  nor2   g55(.a(new_n86_), .b(x06), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n50_), .c(new_n38_), .O(new_n91_));
  oai21  g57(.a(new_n89_), .b(new_n37_), .c(new_n91_), .O(z08));
  aoi21  g58(.a(new_n88_), .b(new_n65_), .c(x13), .O(new_n93_));
  nand3  g59(.a(new_n88_), .b(new_n65_), .c(x13), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x06), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n93_), .c(new_n60_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  oai21  g63(.a(new_n87_), .b(new_n82_), .c(new_n97_), .O(new_n98_));
  inv1   g64(.a(x03), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x02), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x16), .c(new_n77_), .O(new_n101_));
  nor2   g67(.a(new_n49_), .b(new_n40_), .O(new_n102_));
  aoi21  g68(.a(new_n101_), .b(new_n98_), .c(new_n102_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n46_), .c(new_n38_), .O(z10));
  inv1   g70(.a(new_n38_), .O(new_n105_));
  nor2   g71(.a(new_n105_), .b(new_n35_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n35_), .c(x03), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(new_n46_), .c(new_n38_), .O(z12));
  nor2   g74(.a(new_n105_), .b(new_n46_), .O(z13));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n38_), .O(z14));
endmodule


