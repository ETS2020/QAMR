// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n107_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  inv1   g02(.a(x09), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x09), .c(x06), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(new_n38_), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n50_), .O(z03));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n54_), .b(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n38_), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n50_), .O(z04));
  nand2  g27(.a(new_n50_), .b(new_n38_), .O(new_n62_));
  xor2a  g28(.a(new_n57_), .b(x09), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n62_), .O(z05));
  nand2  g30(.a(x08), .b(x07), .O(new_n65_));
  nor2   g31(.a(new_n65_), .b(x10), .O(new_n66_));
  oai22  g32(.a(new_n66_), .b(new_n36_), .c(x10), .d(new_n37_), .O(new_n67_));
  nand3  g33(.a(new_n37_), .b(x08), .c(x07), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x10), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n49_), .d(x04), .O(z06));
  inv1   g36(.a(x11), .O(new_n71_));
  inv1   g37(.a(new_n65_), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n37_), .O(new_n74_));
  oai21  g40(.a(new_n66_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand2  g42(.a(x11), .b(x09), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n62_), .O(z07));
  inv1   g44(.a(x12), .O(new_n79_));
  nand2  g45(.a(new_n74_), .b(new_n79_), .O(new_n80_));
  nand3  g46(.a(new_n73_), .b(new_n72_), .c(x12), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(x06), .O(new_n82_));
  nand2  g48(.a(x12), .b(x09), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n62_), .O(z08));
  nand3  g50(.a(new_n73_), .b(new_n72_), .c(new_n79_), .O(new_n85_));
  nor2   g51(.a(x13), .b(x12), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n73_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(new_n68_), .O(new_n88_));
  aoi21  g54(.a(new_n85_), .b(x13), .c(new_n88_), .O(new_n89_));
  aoi22  g55(.a(new_n50_), .b(new_n38_), .c(x13), .d(x09), .O(new_n90_));
  oai21  g56(.a(new_n89_), .b(new_n36_), .c(new_n90_), .O(z09));
  inv1   g57(.a(x04), .O(new_n92_));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n87_), .b(new_n93_), .O(new_n94_));
  inv1   g60(.a(x03), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x02), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(x16), .c(new_n68_), .O(new_n97_));
  inv1   g63(.a(x14), .O(new_n98_));
  nor2   g64(.a(new_n49_), .b(new_n98_), .O(new_n99_));
  aoi21  g65(.a(new_n97_), .b(new_n94_), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n92_), .c(new_n38_), .O(z10));
  inv1   g67(.a(new_n38_), .O(new_n102_));
  nor2   g68(.a(new_n102_), .b(new_n48_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n48_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n92_), .c(new_n38_), .O(z12));
  nor2   g71(.a(new_n102_), .b(new_n92_), .O(z13));
  nand2  g72(.a(x17), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n38_), .O(z14));
endmodule


