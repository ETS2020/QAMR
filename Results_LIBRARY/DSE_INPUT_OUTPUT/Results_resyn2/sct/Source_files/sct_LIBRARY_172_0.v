// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n113_, new_n115_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n38_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n38_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(new_n50_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n50_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  nor2   g30(.a(new_n63_), .b(x09), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n50_), .O(z05));
  nor2   g33(.a(x10), .b(x09), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(x08), .c(x07), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x04), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  oai21  g37(.a(x03), .b(new_n35_), .c(x16), .O(new_n72_));
  nand2  g38(.a(new_n65_), .b(new_n72_), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand2  g40(.a(new_n72_), .b(new_n74_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(x04), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n71_), .O(z06));
  nand2  g43(.a(x08), .b(x07), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand2  g45(.a(new_n68_), .b(new_n79_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n78_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  inv1   g48(.a(new_n58_), .O(new_n83_));
  aoi21  g49(.a(new_n68_), .b(new_n83_), .c(new_n79_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n49_), .c(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n82_), .O(z07));
  inv1   g52(.a(new_n63_), .O(new_n87_));
  nand3  g53(.a(new_n68_), .b(new_n87_), .c(new_n79_), .O(new_n88_));
  aoi21  g54(.a(new_n88_), .b(x12), .c(new_n49_), .O(new_n89_));
  inv1   g55(.a(x09), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(x08), .c(x07), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n79_), .c(new_n74_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n91_), .c(x04), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x06), .O(new_n95_));
  oai21  g61(.a(new_n89_), .b(new_n37_), .c(new_n95_), .O(z08));
  inv1   g62(.a(x13), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n92_), .c(new_n79_), .d(new_n74_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n91_), .c(x04), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x06), .O(new_n100_));
  nor2   g66(.a(x12), .b(x11), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n68_), .c(new_n87_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(x13), .c(new_n49_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n37_), .c(new_n100_), .O(z09));
  inv1   g70(.a(x00), .O(new_n105_));
  nand2  g71(.a(new_n98_), .b(new_n105_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n65_), .c(new_n72_), .O(new_n107_));
  nand2  g73(.a(new_n49_), .b(x14), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n107_), .c(new_n37_), .O(z10));
  nand2  g75(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g76(.a(x16), .b(new_n35_), .c(x03), .O(new_n111_));
  oai21  g77(.a(new_n111_), .b(new_n37_), .c(new_n39_), .O(z12));
  nor2   g78(.a(new_n38_), .b(x04), .O(new_n113_));
  inv1   g79(.a(new_n113_), .O(z13));
  nor2   g80(.a(x17), .b(new_n37_), .O(new_n115_));
  nor2   g81(.a(new_n115_), .b(new_n113_), .O(z14));
endmodule


