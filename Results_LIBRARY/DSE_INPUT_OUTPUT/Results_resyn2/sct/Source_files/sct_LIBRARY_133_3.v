// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n95_,
    new_n96_, new_n99_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x13), .b(x12), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  inv1   g08(.a(new_n37_), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n37_), .c(new_n46_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand4  g21(.a(new_n55_), .b(new_n53_), .c(new_n50_), .d(new_n37_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n50_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n37_), .O(z04));
  nor2   g28(.a(new_n58_), .b(x09), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nand2  g30(.a(new_n58_), .b(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n64_), .c(new_n50_), .O(new_n66_));
  and2   g32(.a(new_n66_), .b(new_n37_), .O(z05));
  nand2  g33(.a(new_n50_), .b(new_n37_), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n64_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(new_n63_), .b(new_n71_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n69_), .O(z06));
  nand2  g39(.a(new_n72_), .b(x11), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n63_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n69_), .O(z07));
  nand2  g43(.a(new_n76_), .b(x12), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nand3  g45(.a(new_n75_), .b(new_n63_), .c(new_n79_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n69_), .O(z08));
  nand2  g47(.a(new_n80_), .b(x13), .O(new_n82_));
  nor2   g48(.a(x13), .b(x12), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n75_), .c(new_n63_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n82_), .c(new_n50_), .O(z09));
  nand2  g51(.a(new_n49_), .b(x14), .O(new_n86_));
  oai21  g52(.a(x03), .b(new_n35_), .c(x16), .O(new_n87_));
  nand3  g53(.a(new_n63_), .b(new_n87_), .c(x00), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n37_), .O(new_n90_));
  inv1   g56(.a(new_n84_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n87_), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n90_), .c(new_n42_), .O(z10));
  nor2   g59(.a(new_n43_), .b(new_n35_), .O(z11));
  nand2  g60(.a(x16), .b(new_n35_), .O(new_n95_));
  nor2   g61(.a(new_n43_), .b(x03), .O(new_n96_));
  aoi22  g62(.a(new_n96_), .b(new_n95_), .c(new_n37_), .d(new_n42_), .O(z12));
  nand2  g63(.a(new_n37_), .b(new_n42_), .O(z13));
  inv1   g64(.a(x17), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n42_), .c(new_n37_), .O(z14));
endmodule


