// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n73_, new_n75_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x07), .b(x06), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  nor2   g05(.a(x15), .b(x05), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  aoi21  g08(.a(new_n40_), .b(x18), .c(new_n42_), .O(z01));
  oai21  g09(.a(x03), .b(new_n35_), .c(x16), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(x04), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x06), .c(new_n37_), .O(z02));
  or2    g12(.a(x07), .b(x06), .O(new_n47_));
  nand2  g13(.a(new_n37_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z13));
  nand3  g15(.a(z13), .b(new_n47_), .c(new_n44_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z03));
  inv1   g17(.a(x08), .O(new_n52_));
  oai21  g18(.a(new_n45_), .b(new_n52_), .c(new_n37_), .O(z04));
  inv1   g19(.a(x09), .O(new_n54_));
  inv1   g20(.a(x03), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(x02), .O(new_n56_));
  aoi21  g22(.a(new_n56_), .b(x16), .c(new_n48_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n54_), .O(z05));
  inv1   g24(.a(x10), .O(new_n59_));
  nand2  g25(.a(new_n57_), .b(new_n59_), .O(z06));
  inv1   g26(.a(x11), .O(new_n61_));
  inv1   g27(.a(new_n37_), .O(new_n62_));
  inv1   g28(.a(x04), .O(new_n63_));
  aoi21  g29(.a(new_n56_), .b(x16), .c(new_n63_), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(new_n61_), .c(new_n62_), .O(z07));
  inv1   g31(.a(x12), .O(new_n66_));
  nand2  g32(.a(new_n57_), .b(new_n66_), .O(z08));
  inv1   g33(.a(x13), .O(new_n68_));
  aoi21  g34(.a(new_n64_), .b(new_n68_), .c(new_n62_), .O(z09));
  nand2  g35(.a(x14), .b(x04), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n44_), .c(new_n37_), .O(z10));
  nand2  g37(.a(new_n37_), .b(new_n35_), .O(z11));
  nand2  g38(.a(x16), .b(new_n35_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n55_), .c(new_n48_), .O(z12));
  nand2  g40(.a(z13), .b(x17), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(z14));
endmodule


