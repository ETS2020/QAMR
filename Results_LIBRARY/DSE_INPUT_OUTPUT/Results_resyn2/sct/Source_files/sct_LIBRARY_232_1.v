// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_;
  inv1   g00(.a(x14), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x04), .O(new_n37_));
  aoi21  g03(.a(x02), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n35_), .b(x01), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x04), .O(new_n41_));
  oai21  g07(.a(x06), .b(x05), .c(new_n41_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n36_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n36_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(new_n52_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(x08), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n52_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n49_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nand2  g28(.a(new_n58_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n57_), .b(new_n64_), .c(x08), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n49_), .O(z05));
  inv1   g32(.a(x10), .O(new_n67_));
  inv1   g33(.a(new_n65_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g35(.a(new_n67_), .b(x04), .O(new_n70_));
  aoi21  g36(.a(new_n70_), .b(new_n65_), .c(new_n48_), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n69_), .c(new_n37_), .O(z06));
  oai21  g38(.a(x11), .b(new_n41_), .c(new_n36_), .O(new_n73_));
  inv1   g39(.a(new_n48_), .O(new_n74_));
  nand3  g40(.a(new_n64_), .b(x08), .c(x07), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(x10), .c(x11), .O(new_n76_));
  nor2   g42(.a(x09), .b(new_n59_), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n57_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n76_), .c(new_n74_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n73_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  nand3  g49(.a(new_n78_), .b(new_n68_), .c(new_n83_), .O(new_n84_));
  nand2  g50(.a(new_n79_), .b(x12), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n49_), .O(z08));
  oai21  g52(.a(x13), .b(new_n41_), .c(new_n36_), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  nand3  g54(.a(new_n83_), .b(new_n88_), .c(new_n67_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n75_), .c(x13), .O(new_n90_));
  nor2   g56(.a(x13), .b(x12), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n78_), .c(new_n77_), .d(new_n57_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n90_), .c(new_n74_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x04), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n87_), .O(z09));
  nand2  g61(.a(new_n48_), .b(x14), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n91_), .b(new_n78_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n68_), .c(new_n74_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n96_), .c(new_n41_), .O(z10));
  inv1   g67(.a(x02), .O(new_n102_));
  nor2   g68(.a(new_n37_), .b(new_n102_), .O(z11));
  nand2  g69(.a(x16), .b(new_n102_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n46_), .c(new_n41_), .O(z12));
  and2   g71(.a(x17), .b(x04), .O(z14));
  buf    g72(.a(x04), .O(z13));
endmodule


