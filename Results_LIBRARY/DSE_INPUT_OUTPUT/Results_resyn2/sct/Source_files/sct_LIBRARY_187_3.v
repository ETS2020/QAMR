// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n99_, new_n100_, new_n101_,
    new_n104_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x12), .b(x06), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  nor2   g05(.a(x15), .b(x05), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  aoi21  g08(.a(new_n40_), .b(x18), .c(new_n42_), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x04), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  xnor2a g15(.a(x07), .b(x06), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n46_), .c(new_n37_), .O(z03));
  aoi21  g17(.a(x07), .b(x06), .c(x08), .O(new_n52_));
  nand3  g18(.a(x08), .b(x07), .c(x06), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  or2    g20(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n46_), .c(new_n37_), .O(z04));
  inv1   g22(.a(x09), .O(new_n57_));
  nand2  g23(.a(new_n54_), .b(new_n57_), .O(new_n58_));
  nand2  g24(.a(new_n53_), .b(x09), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n58_), .c(new_n45_), .d(x04), .O(new_n60_));
  and2   g26(.a(new_n60_), .b(new_n37_), .O(z05));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(x09), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x12), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x10), .O(new_n65_));
  inv1   g31(.a(x10), .O(new_n66_));
  nor2   g32(.a(x12), .b(new_n44_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n63_), .c(new_n66_), .O(new_n68_));
  nand2  g34(.a(new_n46_), .b(new_n37_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n65_), .O(z06));
  inv1   g36(.a(x12), .O(new_n71_));
  nand3  g37(.a(new_n57_), .b(x08), .c(x07), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n72_), .c(new_n71_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand2  g42(.a(new_n68_), .b(x11), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n47_), .O(z07));
  and2   g44(.a(x08), .b(x07), .O(new_n79_));
  nand4  g45(.a(new_n73_), .b(new_n79_), .c(new_n57_), .d(x06), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n47_), .c(new_n71_), .O(z08));
  inv1   g47(.a(x13), .O(new_n82_));
  nand2  g48(.a(new_n80_), .b(new_n82_), .O(new_n83_));
  nand3  g49(.a(new_n73_), .b(new_n63_), .c(x13), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n71_), .O(new_n85_));
  nor2   g51(.a(new_n82_), .b(x06), .O(new_n86_));
  aoi21  g52(.a(new_n46_), .b(new_n37_), .c(new_n86_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n85_), .O(z09));
  inv1   g54(.a(x03), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x02), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(x16), .c(x14), .O(new_n91_));
  inv1   g57(.a(x00), .O(new_n92_));
  nand2  g58(.a(new_n73_), .b(new_n82_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n67_), .c(new_n63_), .d(new_n45_), .O(new_n95_));
  nand2  g61(.a(new_n37_), .b(x04), .O(new_n96_));
  aoi21  g62(.a(new_n95_), .b(new_n91_), .c(new_n96_), .O(z10));
  nand2  g63(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g64(.a(x04), .O(new_n99_));
  nand2  g65(.a(x16), .b(new_n35_), .O(new_n100_));
  aoi21  g66(.a(x12), .b(x06), .c(x03), .O(new_n101_));
  aoi22  g67(.a(new_n101_), .b(new_n100_), .c(new_n37_), .d(new_n99_), .O(z12));
  nand2  g68(.a(new_n37_), .b(new_n99_), .O(z13));
  inv1   g69(.a(x17), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n99_), .c(new_n37_), .O(z14));
endmodule


