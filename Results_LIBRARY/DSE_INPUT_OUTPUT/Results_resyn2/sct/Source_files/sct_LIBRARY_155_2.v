// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n83_, new_n84_, new_n87_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(x10), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n37_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n36_), .c(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nand2  g17(.a(new_n49_), .b(x04), .O(new_n52_));
  xnor2a g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n52_), .c(new_n38_), .O(z03));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  xor2a  g21(.a(new_n55_), .b(x08), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n52_), .c(new_n38_), .O(z04));
  inv1   g23(.a(x03), .O(new_n58_));
  inv1   g24(.a(x16), .O(new_n59_));
  aoi21  g25(.a(new_n58_), .b(x02), .c(new_n59_), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n45_), .O(new_n61_));
  inv1   g27(.a(x10), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n62_), .c(x09), .O(new_n64_));
  nand2  g30(.a(x08), .b(x07), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x09), .c(x10), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n64_), .c(new_n61_), .O(z05));
  oai21  g34(.a(new_n63_), .b(x09), .c(x10), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n61_), .c(new_n37_), .O(z06));
  inv1   g36(.a(x11), .O(new_n71_));
  aoi21  g37(.a(new_n61_), .b(new_n71_), .c(new_n37_), .O(z07));
  inv1   g38(.a(x12), .O(new_n73_));
  aoi21  g39(.a(new_n61_), .b(new_n73_), .c(new_n37_), .O(z08));
  inv1   g40(.a(x13), .O(new_n75_));
  aoi21  g41(.a(new_n61_), .b(new_n75_), .c(new_n37_), .O(z09));
  nor2   g42(.a(new_n37_), .b(x04), .O(new_n77_));
  nor2   g43(.a(new_n63_), .b(x09), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n49_), .c(x00), .O(new_n79_));
  aoi21  g45(.a(new_n60_), .b(x14), .c(new_n37_), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n79_), .c(new_n77_), .O(z10));
  nand2  g47(.a(new_n38_), .b(new_n48_), .O(z11));
  aoi21  g48(.a(x16), .b(new_n48_), .c(x03), .O(new_n83_));
  nand2  g49(.a(new_n38_), .b(x04), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n83_), .O(z12));
  inv1   g51(.a(new_n77_), .O(z13));
  inv1   g52(.a(x17), .O(new_n87_));
  nor2   g53(.a(new_n84_), .b(new_n87_), .O(z14));
endmodule


