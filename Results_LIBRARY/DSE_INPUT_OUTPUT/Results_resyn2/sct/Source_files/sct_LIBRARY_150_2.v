// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n107_, new_n110_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x09), .O(new_n37_));
  nor2   g03(.a(x14), .b(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(new_n38_), .O(new_n42_));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n42_), .O(z02));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(x07), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n48_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n42_), .O(z03));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n53_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n42_), .O(z04));
  oai21  g31(.a(x03), .b(new_n36_), .c(x16), .O(new_n66_));
  inv1   g32(.a(new_n61_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n37_), .O(new_n68_));
  nand2  g34(.a(new_n61_), .b(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n68_), .c(new_n66_), .d(x04), .O(new_n70_));
  and2   g36(.a(new_n70_), .b(new_n42_), .O(z05));
  oai21  g37(.a(new_n61_), .b(x09), .c(x10), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand3  g39(.a(new_n67_), .b(new_n73_), .c(new_n37_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n72_), .c(new_n66_), .d(x04), .O(new_n75_));
  and2   g41(.a(new_n75_), .b(new_n42_), .O(z06));
  inv1   g42(.a(x11), .O(new_n77_));
  nor2   g43(.a(new_n61_), .b(x09), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n73_), .O(new_n79_));
  oai21  g45(.a(new_n38_), .b(x11), .c(new_n74_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n53_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nor2   g48(.a(x11), .b(x10), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n67_), .c(new_n82_), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n37_), .O(new_n86_));
  nand2  g52(.a(new_n83_), .b(new_n67_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x12), .O(new_n88_));
  inv1   g54(.a(x14), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(x12), .c(x09), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n88_), .c(new_n86_), .d(new_n53_), .O(z08));
  nand2  g57(.a(new_n84_), .b(x13), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand3  g59(.a(new_n83_), .b(new_n93_), .c(new_n82_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n78_), .O(new_n96_));
  oai21  g62(.a(new_n89_), .b(x13), .c(x09), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n96_), .c(new_n92_), .d(new_n53_), .O(z09));
  nor2   g64(.a(new_n38_), .b(x04), .O(new_n99_));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n94_), .b(new_n100_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n78_), .c(new_n66_), .O(new_n102_));
  aoi21  g68(.a(new_n52_), .b(x14), .c(new_n38_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n99_), .O(z10));
  nand2  g70(.a(new_n42_), .b(new_n36_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n36_), .c(x03), .O(new_n106_));
  nand2  g72(.a(new_n42_), .b(x04), .O(new_n107_));
  nor2   g73(.a(new_n107_), .b(new_n106_), .O(z12));
  inv1   g74(.a(new_n99_), .O(z13));
  inv1   g75(.a(x17), .O(new_n110_));
  nor2   g76(.a(new_n107_), .b(new_n110_), .O(z14));
endmodule


