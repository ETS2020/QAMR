// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n109_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  inv1   g02(.a(x09), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  nand2  g06(.a(x14), .b(new_n40_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(z00));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  aoi21  g12(.a(x05), .b(new_n46_), .c(new_n38_), .O(new_n47_));
  oai21  g13(.a(new_n45_), .b(x05), .c(new_n47_), .O(z01));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  nand2  g20(.a(new_n49_), .b(x02), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(x16), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(x04), .O(new_n57_));
  xnor2a g23(.a(x07), .b(x06), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n57_), .c(new_n39_), .O(z03));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(new_n61_));
  aoi21  g27(.a(x07), .b(x06), .c(x08), .O(new_n62_));
  or2    g28(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n57_), .c(new_n39_), .O(z04));
  nand3  g30(.a(new_n60_), .b(new_n52_), .c(new_n37_), .O(z05));
  inv1   g31(.a(x10), .O(new_n66_));
  and2   g32(.a(x08), .b(x07), .O(new_n67_));
  nor2   g33(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nor2   g34(.a(new_n60_), .b(x10), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n68_), .c(new_n37_), .O(new_n70_));
  nand2  g36(.a(new_n57_), .b(new_n39_), .O(new_n71_));
  nand2  g37(.a(x10), .b(new_n36_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  oai21  g40(.a(new_n60_), .b(x10), .c(new_n74_), .O(new_n75_));
  nand3  g41(.a(new_n67_), .b(x11), .c(new_n66_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n37_), .O(new_n77_));
  nand2  g43(.a(x11), .b(new_n36_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n71_), .O(z07));
  inv1   g45(.a(x12), .O(new_n80_));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n67_), .c(new_n80_), .O(new_n82_));
  nand3  g48(.a(new_n80_), .b(new_n74_), .c(new_n66_), .O(new_n83_));
  nor2   g49(.a(new_n83_), .b(new_n60_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n82_), .c(new_n37_), .O(new_n85_));
  nand2  g51(.a(x12), .b(new_n36_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n71_), .O(z08));
  nand3  g53(.a(new_n81_), .b(new_n61_), .c(new_n80_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(x09), .c(x13), .O(new_n89_));
  nand3  g55(.a(new_n66_), .b(x08), .c(x07), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n80_), .c(new_n74_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n90_), .c(new_n37_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x06), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n89_), .c(new_n52_), .O(z09));
  nand2  g61(.a(new_n39_), .b(x04), .O(new_n96_));
  nand2  g62(.a(new_n51_), .b(x14), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand3  g64(.a(new_n81_), .b(new_n91_), .c(new_n80_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  aoi21  g66(.a(new_n55_), .b(x16), .c(x09), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n100_), .c(new_n61_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n97_), .c(new_n96_), .O(z10));
  inv1   g69(.a(x02), .O(new_n104_));
  nor2   g70(.a(new_n38_), .b(new_n104_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n104_), .c(x03), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n96_), .O(z12));
  nand2  g73(.a(new_n39_), .b(new_n46_), .O(z13));
  nand3  g74(.a(new_n39_), .b(x17), .c(x04), .O(new_n109_));
  inv1   g75(.a(new_n109_), .O(z14));
endmodule


