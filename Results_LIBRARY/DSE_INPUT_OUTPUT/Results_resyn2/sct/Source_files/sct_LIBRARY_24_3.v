// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n98_, new_n101_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  nand2  g01(.a(x09), .b(x06), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  nand2  g03(.a(x14), .b(new_n37_), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(x05), .b(x04), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n36_), .O(new_n43_));
  aoi21  g09(.a(new_n41_), .b(x18), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  nor2   g11(.a(x09), .b(new_n45_), .O(new_n46_));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(new_n45_), .c(new_n46_), .O(z02));
  xnor2a g16(.a(x07), .b(x06), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n49_), .c(new_n36_), .O(z03));
  nand3  g18(.a(x08), .b(x07), .c(x06), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  aoi21  g20(.a(x07), .b(x06), .c(x08), .O(new_n55_));
  or2    g21(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n49_), .c(new_n36_), .O(z04));
  inv1   g23(.a(x09), .O(new_n58_));
  inv1   g24(.a(new_n49_), .O(new_n59_));
  nand3  g25(.a(new_n53_), .b(new_n59_), .c(new_n58_), .O(z05));
  inv1   g26(.a(x10), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(x08), .c(x07), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x06), .O(new_n64_));
  oai21  g30(.a(new_n53_), .b(x09), .c(x10), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n64_), .c(new_n59_), .O(z06));
  aoi21  g32(.a(new_n63_), .b(x06), .c(x11), .O(new_n67_));
  nand2  g33(.a(new_n46_), .b(x11), .O(new_n68_));
  nor2   g34(.a(new_n68_), .b(new_n62_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n67_), .c(new_n59_), .O(z07));
  nor2   g36(.a(x11), .b(x10), .O(new_n71_));
  nand2  g37(.a(x08), .b(x07), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(x12), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n58_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nor2   g42(.a(new_n53_), .b(x09), .O(new_n77_));
  nand2  g43(.a(new_n71_), .b(new_n77_), .O(new_n78_));
  aoi21  g44(.a(new_n78_), .b(x12), .c(new_n49_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n76_), .O(z08));
  inv1   g46(.a(x13), .O(new_n81_));
  aoi21  g47(.a(new_n73_), .b(new_n71_), .c(new_n81_), .O(new_n82_));
  nor2   g48(.a(x13), .b(x12), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n71_), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n53_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n82_), .c(new_n58_), .O(new_n86_));
  nor2   g52(.a(new_n81_), .b(x06), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n49_), .c(new_n36_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n86_), .O(z09));
  nand2  g55(.a(new_n36_), .b(x04), .O(new_n90_));
  inv1   g56(.a(new_n48_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x14), .O(new_n92_));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n84_), .b(new_n93_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n77_), .c(new_n48_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n92_), .c(new_n90_), .O(z10));
  nand2  g62(.a(new_n36_), .b(new_n47_), .O(z11));
  aoi21  g63(.a(x16), .b(new_n47_), .c(x03), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n90_), .O(z12));
  inv1   g65(.a(new_n90_), .O(z13));
  nand2  g66(.a(z13), .b(x17), .O(new_n101_));
  inv1   g67(.a(new_n101_), .O(z14));
endmodule


