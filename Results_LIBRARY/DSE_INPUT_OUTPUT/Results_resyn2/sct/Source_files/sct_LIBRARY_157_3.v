// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n102_,
    new_n104_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x05), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  aoi22  g08(.a(new_n42_), .b(x18), .c(new_n41_), .d(x04), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n39_), .c(new_n36_), .O(z02));
  inv1   g13(.a(new_n45_), .O(new_n48_));
  nand2  g14(.a(x04), .b(new_n39_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g16(.a(x07), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n44_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  nand2  g19(.a(new_n51_), .b(new_n44_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n50_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand2  g27(.a(new_n57_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(new_n50_), .O(z05));
  nand2  g31(.a(new_n64_), .b(x10), .O(new_n66_));
  inv1   g32(.a(x10), .O(new_n67_));
  nor2   g33(.a(x09), .b(new_n58_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n52_), .c(new_n67_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n66_), .c(new_n45_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n39_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x04), .O(z06));
  nand2  g38(.a(new_n69_), .b(x11), .O(new_n73_));
  inv1   g39(.a(new_n64_), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n73_), .c(new_n50_), .O(z07));
  inv1   g43(.a(x11), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n64_), .c(x12), .O(new_n80_));
  inv1   g46(.a(x12), .O(new_n81_));
  inv1   g47(.a(new_n57_), .O(new_n82_));
  nand4  g48(.a(new_n75_), .b(new_n82_), .c(new_n81_), .d(new_n63_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n80_), .c(new_n45_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n39_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x04), .O(z08));
  nand4  g52(.a(new_n81_), .b(new_n78_), .c(new_n67_), .d(new_n63_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n57_), .c(x13), .O(new_n88_));
  nor2   g54(.a(x13), .b(x12), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n75_), .c(new_n68_), .d(new_n52_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n88_), .c(new_n45_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n39_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x04), .O(z09));
  nand2  g59(.a(new_n48_), .b(x14), .O(new_n94_));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n89_), .b(new_n75_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n74_), .c(new_n45_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n94_), .c(new_n49_), .O(z10));
  aoi21  g65(.a(x04), .b(x01), .c(new_n35_), .O(z11));
  nand2  g66(.a(x16), .b(new_n35_), .O(new_n101_));
  nor2   g67(.a(x03), .b(x01), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n36_), .O(z12));
  inv1   g69(.a(x17), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n39_), .c(new_n36_), .O(z14));
  buf    g71(.a(x04), .O(z13));
endmodule


