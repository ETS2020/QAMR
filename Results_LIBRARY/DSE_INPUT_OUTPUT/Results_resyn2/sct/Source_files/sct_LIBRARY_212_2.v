// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n97_, new_n98_, new_n100_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  nor2   g03(.a(x06), .b(new_n37_), .O(z02));
  inv1   g04(.a(z02), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x15), .O(new_n43_));
  nand2  g09(.a(x18), .b(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(z02), .c(new_n42_), .O(new_n45_));
  inv1   g11(.a(x06), .O(new_n46_));
  nor2   g12(.a(new_n46_), .b(new_n37_), .O(z13));
  oai21  g13(.a(z13), .b(new_n42_), .c(new_n45_), .O(z01));
  inv1   g14(.a(x07), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n35_), .c(x16), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(x06), .c(new_n37_), .O(z03));
  nand2  g18(.a(x08), .b(x07), .O(new_n53_));
  inv1   g19(.a(x08), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(new_n50_), .O(new_n56_));
  aoi21  g22(.a(new_n56_), .b(x06), .c(new_n37_), .O(z04));
  nand2  g23(.a(new_n53_), .b(x09), .O(new_n58_));
  inv1   g24(.a(x09), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(x08), .c(x07), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n50_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x04), .O(z05));
  inv1   g29(.a(x10), .O(new_n64_));
  nor2   g30(.a(new_n53_), .b(x09), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g32(.a(new_n60_), .b(x10), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n50_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x04), .O(z06));
  nand2  g36(.a(new_n66_), .b(x11), .O(new_n71_));
  nor2   g37(.a(x11), .b(x10), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n65_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n71_), .c(new_n50_), .d(z13), .O(z07));
  inv1   g40(.a(x11), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n64_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n60_), .c(x12), .O(new_n77_));
  nor3   g43(.a(x12), .b(x11), .c(x10), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n65_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n50_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x04), .O(z08));
  inv1   g48(.a(x13), .O(new_n83_));
  aoi21  g49(.a(new_n78_), .b(new_n65_), .c(new_n83_), .O(new_n84_));
  nor2   g50(.a(x13), .b(x12), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n72_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n60_), .c(new_n50_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n84_), .c(x06), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x04), .O(z09));
  inv1   g55(.a(x00), .O(new_n90_));
  nand2  g56(.a(new_n86_), .b(new_n90_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n65_), .c(new_n50_), .O(new_n92_));
  inv1   g58(.a(new_n50_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(x14), .c(new_n46_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n92_), .c(new_n37_), .O(z10));
  nand2  g61(.a(new_n39_), .b(new_n35_), .O(z11));
  nand2  g62(.a(x16), .b(new_n35_), .O(new_n97_));
  nor2   g63(.a(new_n46_), .b(x03), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n97_), .c(new_n37_), .O(z12));
  inv1   g65(.a(x17), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x06), .c(new_n37_), .O(z14));
endmodule


