// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x13), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(x13), .c(new_n46_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  oai21  g19(.a(x03), .b(new_n35_), .c(x16), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x04), .O(new_n55_));
  nor2   g21(.a(x07), .b(x06), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n38_), .O(new_n58_));
  nor3   g24(.a(new_n58_), .b(new_n56_), .c(new_n55_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n57_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(new_n38_), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n55_), .O(z04));
  oai21  g30(.a(new_n37_), .b(x09), .c(new_n62_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  inv1   g32(.a(new_n62_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n65_), .c(new_n51_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nand3  g36(.a(new_n67_), .b(new_n70_), .c(new_n66_), .O(new_n71_));
  nand2  g37(.a(x08), .b(x07), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(x09), .c(x10), .O(new_n73_));
  inv1   g39(.a(x13), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(x10), .c(new_n46_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n73_), .c(new_n71_), .d(new_n51_), .O(z06));
  nor2   g42(.a(new_n72_), .b(x09), .O(new_n77_));
  nand4  g43(.a(new_n70_), .b(new_n66_), .c(x08), .d(x07), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  aoi22  g45(.a(new_n79_), .b(new_n77_), .c(new_n78_), .d(x11), .O(new_n80_));
  nand2  g46(.a(x11), .b(new_n46_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n54_), .c(x04), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n38_), .O(new_n83_));
  oai21  g49(.a(new_n80_), .b(new_n46_), .c(new_n83_), .O(z07));
  nand4  g50(.a(new_n79_), .b(new_n66_), .c(x08), .d(x07), .O(new_n85_));
  xor2a  g51(.a(new_n85_), .b(x12), .O(new_n86_));
  nand2  g52(.a(x12), .b(new_n46_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n54_), .c(x04), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n38_), .O(new_n89_));
  oai21  g55(.a(new_n86_), .b(new_n46_), .c(new_n89_), .O(z08));
  oai21  g56(.a(new_n85_), .b(x12), .c(x13), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  nand4  g58(.a(new_n79_), .b(new_n77_), .c(new_n74_), .d(new_n92_), .O(new_n93_));
  nand2  g59(.a(new_n48_), .b(x02), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(x16), .c(new_n46_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n93_), .c(new_n91_), .d(x04), .O(z09));
  nand3  g62(.a(new_n50_), .b(new_n38_), .c(x14), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand3  g64(.a(new_n79_), .b(new_n74_), .c(new_n92_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n95_), .c(new_n77_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n97_), .c(new_n47_), .O(z10));
  nand2  g68(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n35_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n47_), .c(new_n38_), .O(z12));
  nor2   g71(.a(new_n37_), .b(new_n47_), .O(z13));
  nand2  g72(.a(x17), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n38_), .O(z14));
endmodule


