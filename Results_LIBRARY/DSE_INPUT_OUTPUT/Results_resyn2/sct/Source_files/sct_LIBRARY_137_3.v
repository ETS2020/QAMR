// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  inv1   g03(.a(x17), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g05(.a(x14), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  oai21  g06(.a(new_n36_), .b(new_n35_), .c(new_n40_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n39_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n49_), .c(new_n53_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n54_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n49_), .c(new_n53_), .O(z04));
  nor3   g29(.a(new_n61_), .b(x17), .c(x09), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n49_), .b(new_n53_), .O(new_n66_));
  nor2   g32(.a(new_n59_), .b(new_n55_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(x17), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n66_), .c(new_n65_), .O(z05));
  nand2  g36(.a(new_n65_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n72_), .c(x08), .d(x07), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(x17), .c(x06), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n71_), .c(new_n50_), .O(z06));
  nor2   g43(.a(x10), .b(x09), .O(new_n78_));
  aoi21  g44(.a(new_n78_), .b(new_n67_), .c(x17), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n37_), .c(x11), .O(new_n80_));
  nor2   g46(.a(x11), .b(new_n37_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n75_), .c(new_n38_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(new_n66_), .O(z07));
  nor2   g49(.a(x12), .b(x11), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n78_), .c(x08), .d(x07), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n38_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x06), .O(new_n87_));
  nand2  g53(.a(new_n82_), .b(x12), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n50_), .O(z08));
  nand2  g55(.a(new_n87_), .b(x13), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand3  g57(.a(new_n84_), .b(new_n91_), .c(new_n73_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  aoi22  g59(.a(new_n93_), .b(new_n64_), .c(new_n49_), .d(new_n53_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n90_), .O(z09));
  nor2   g61(.a(new_n61_), .b(x09), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n96_), .c(new_n48_), .O(new_n99_));
  inv1   g65(.a(x03), .O(new_n100_));
  inv1   g66(.a(x16), .O(new_n101_));
  aoi21  g67(.a(new_n100_), .b(x02), .c(new_n101_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(x14), .c(new_n39_), .O(new_n103_));
  aoi22  g69(.a(new_n103_), .b(new_n99_), .c(new_n53_), .d(new_n45_), .O(z10));
  nor2   g70(.a(new_n39_), .b(new_n36_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n36_), .c(x03), .O(new_n106_));
  nor3   g72(.a(new_n106_), .b(new_n39_), .c(new_n45_), .O(z12));
  nand2  g73(.a(new_n53_), .b(new_n45_), .O(z13));
  aoi21  g74(.a(new_n37_), .b(new_n45_), .c(new_n38_), .O(z14));
endmodule


