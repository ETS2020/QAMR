// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n104_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  nand2  g01(.a(x17), .b(x06), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  nand2  g03(.a(x14), .b(new_n37_), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(x05), .b(x04), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n36_), .O(new_n43_));
  aoi21  g09(.a(new_n41_), .b(x18), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x02), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n46_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  xnor2a g17(.a(x07), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n48_), .c(new_n36_), .O(z03));
  nand3  g19(.a(x08), .b(x07), .c(x06), .O(new_n54_));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n54_), .c(new_n36_), .O(new_n58_));
  nor2   g24(.a(new_n58_), .b(new_n48_), .O(z04));
  nor2   g25(.a(new_n54_), .b(x17), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x17), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(x08), .c(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n62_), .c(new_n49_), .O(z05));
  nand2  g34(.a(new_n67_), .b(x10), .O(new_n69_));
  nand2  g35(.a(new_n48_), .b(new_n36_), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand3  g37(.a(new_n60_), .b(new_n71_), .c(new_n64_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n69_), .O(z06));
  nand4  g39(.a(new_n71_), .b(new_n64_), .c(x08), .d(x07), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n63_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x11), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  nand4  g44(.a(new_n60_), .b(new_n78_), .c(new_n71_), .d(new_n64_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n70_), .O(z07));
  nand2  g46(.a(new_n79_), .b(x12), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n78_), .c(new_n71_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n65_), .c(new_n63_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x06), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n81_), .c(new_n49_), .O(z08));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n82_), .c(new_n78_), .d(new_n71_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n65_), .c(new_n63_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x06), .O(new_n90_));
  nand4  g56(.a(new_n82_), .b(new_n78_), .c(new_n71_), .d(new_n64_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n61_), .c(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n90_), .c(new_n49_), .O(z09));
  nand2  g59(.a(new_n36_), .b(x04), .O(new_n94_));
  inv1   g60(.a(x03), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x02), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(x16), .c(x14), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n88_), .b(new_n98_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n60_), .c(new_n47_), .d(new_n64_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n97_), .c(new_n94_), .O(z10));
  nand2  g67(.a(new_n36_), .b(new_n46_), .O(z11));
  inv1   g68(.a(x04), .O(new_n103_));
  aoi21  g69(.a(x16), .b(new_n46_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n103_), .c(new_n36_), .O(z12));
  inv1   g71(.a(new_n94_), .O(z13));
  aoi21  g72(.a(new_n45_), .b(new_n103_), .c(new_n63_), .O(z14));
endmodule


