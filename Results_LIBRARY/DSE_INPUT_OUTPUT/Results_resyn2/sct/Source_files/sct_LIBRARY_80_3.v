// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(new_n35_), .c(x01), .O(new_n37_));
  oai21  g03(.a(x14), .b(x01), .c(new_n37_), .O(z00));
  inv1   g04(.a(x18), .O(new_n39_));
  nor2   g05(.a(new_n39_), .b(x15), .O(new_n40_));
  inv1   g06(.a(x04), .O(new_n41_));
  nand2  g07(.a(x06), .b(x01), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n40_), .b(x05), .c(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  oai21  g13(.a(new_n47_), .b(x06), .c(new_n42_), .O(z02));
  xnor2a g14(.a(x07), .b(x06), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(new_n47_), .c(new_n42_), .O(z03));
  nand3  g16(.a(x08), .b(x07), .c(x06), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  aoi21  g18(.a(x07), .b(x06), .c(x08), .O(new_n53_));
  or2    g19(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n47_), .c(new_n42_), .O(z04));
  inv1   g21(.a(new_n47_), .O(new_n56_));
  oai21  g22(.a(new_n51_), .b(x01), .c(x09), .O(new_n57_));
  inv1   g23(.a(x09), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(x08), .c(x07), .d(x06), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n57_), .c(new_n56_), .d(new_n42_), .O(z05));
  nor2   g26(.a(new_n51_), .b(x01), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x10), .O(new_n63_));
  inv1   g29(.a(x10), .O(new_n64_));
  inv1   g30(.a(new_n59_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n43_), .c(new_n64_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n63_), .c(new_n56_), .O(z06));
  nand4  g33(.a(new_n61_), .b(x11), .c(new_n64_), .d(new_n58_), .O(new_n68_));
  nor2   g34(.a(new_n59_), .b(x10), .O(new_n69_));
  inv1   g35(.a(x11), .O(new_n70_));
  nand2  g36(.a(new_n42_), .b(new_n70_), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n69_), .c(new_n68_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n56_), .O(z07));
  inv1   g39(.a(x01), .O(new_n74_));
  inv1   g40(.a(x12), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n52_), .c(new_n75_), .d(new_n58_), .O(new_n77_));
  nand2  g43(.a(new_n70_), .b(new_n64_), .O(new_n78_));
  nand3  g44(.a(new_n58_), .b(x08), .c(x07), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n78_), .c(x12), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  nor2   g48(.a(new_n75_), .b(x06), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n47_), .c(new_n42_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n82_), .O(z08));
  nand2  g51(.a(new_n77_), .b(x13), .O(new_n86_));
  nor2   g52(.a(x13), .b(x12), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n76_), .c(new_n52_), .d(new_n58_), .O(new_n88_));
  oai21  g54(.a(x13), .b(x06), .c(x01), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(new_n56_), .O(z09));
  nor2   g56(.a(new_n43_), .b(x04), .O(new_n91_));
  inv1   g57(.a(x00), .O(new_n92_));
  nand2  g58(.a(new_n87_), .b(new_n76_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n65_), .c(new_n46_), .O(new_n95_));
  inv1   g61(.a(x03), .O(new_n96_));
  inv1   g62(.a(x16), .O(new_n97_));
  aoi21  g63(.a(new_n96_), .b(x02), .c(new_n97_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(x14), .c(new_n43_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n95_), .c(new_n91_), .O(z10));
  nand2  g66(.a(new_n42_), .b(new_n35_), .O(z11));
  nand2  g67(.a(x16), .b(new_n35_), .O(new_n102_));
  nor2   g68(.a(new_n43_), .b(x03), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n91_), .O(z12));
  inv1   g70(.a(new_n91_), .O(z13));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n43_), .O(z14));
endmodule


