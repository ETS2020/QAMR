// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n113_, new_n114_,
    new_n117_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x00), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x00), .O(new_n47_));
  nor2   g13(.a(new_n37_), .b(new_n47_), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n37_), .c(new_n48_), .O(z02));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  oai21  g19(.a(new_n37_), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n52_), .c(new_n49_), .d(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n52_), .b(new_n58_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n50_), .c(new_n39_), .O(z04));
  oai21  g27(.a(new_n58_), .b(new_n53_), .c(x09), .O(new_n62_));
  inv1   g28(.a(new_n52_), .O(new_n63_));
  nor2   g29(.a(x09), .b(new_n58_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x00), .O(new_n67_));
  nand2  g33(.a(new_n50_), .b(new_n39_), .O(new_n68_));
  nand2  g34(.a(x09), .b(new_n37_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(z05));
  inv1   g36(.a(x10), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(x08), .c(x07), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  inv1   g40(.a(new_n57_), .O(new_n75_));
  nor2   g41(.a(x10), .b(x09), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  oai21  g43(.a(new_n74_), .b(new_n71_), .c(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x00), .O(new_n79_));
  nand2  g45(.a(x10), .b(new_n37_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n68_), .O(z06));
  nand3  g47(.a(new_n76_), .b(x08), .c(x07), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x11), .O(new_n83_));
  inv1   g49(.a(x11), .O(new_n84_));
  nand3  g50(.a(new_n76_), .b(new_n75_), .c(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x00), .O(new_n87_));
  nand2  g53(.a(x11), .b(new_n37_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n68_), .O(z07));
  inv1   g55(.a(x12), .O(new_n90_));
  nand2  g56(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  inv1   g57(.a(new_n82_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(x12), .c(new_n84_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n91_), .c(x00), .O(new_n94_));
  nand2  g60(.a(x12), .b(new_n37_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n94_), .c(new_n68_), .O(z08));
  nand3  g62(.a(new_n90_), .b(new_n84_), .c(new_n71_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n73_), .c(x13), .O(new_n98_));
  inv1   g64(.a(x13), .O(new_n99_));
  nor3   g65(.a(x12), .b(x11), .c(x10), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n64_), .c(new_n63_), .d(new_n99_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x00), .O(new_n103_));
  aoi22  g69(.a(new_n50_), .b(new_n39_), .c(x13), .d(new_n37_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n103_), .O(z09));
  inv1   g71(.a(x04), .O(new_n106_));
  inv1   g72(.a(x03), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(x02), .O(new_n108_));
  nand4  g74(.a(new_n108_), .b(new_n39_), .c(x16), .d(x14), .O(new_n109_));
  nand3  g75(.a(new_n74_), .b(new_n49_), .c(new_n48_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n109_), .c(new_n106_), .O(z10));
  nor2   g77(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g78(.a(x16), .b(new_n35_), .c(x03), .O(new_n113_));
  nand2  g79(.a(new_n39_), .b(x04), .O(new_n114_));
  nor2   g80(.a(new_n114_), .b(new_n113_), .O(z12));
  inv1   g81(.a(new_n114_), .O(z13));
  inv1   g82(.a(x17), .O(new_n117_));
  nor2   g83(.a(new_n114_), .b(new_n117_), .O(z14));
endmodule


