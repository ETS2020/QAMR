// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n103_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(x14), .b(x06), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  nor2   g05(.a(x15), .b(x05), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  aoi21  g08(.a(new_n40_), .b(x18), .c(new_n42_), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(x04), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z02));
  nand2  g13(.a(new_n45_), .b(x04), .O(new_n48_));
  xnor2a g14(.a(x07), .b(x06), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(new_n48_), .c(new_n36_), .O(z03));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  xor2a  g17(.a(new_n51_), .b(x08), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n48_), .c(new_n36_), .O(z04));
  aoi21  g19(.a(x08), .b(x07), .c(x14), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n44_), .c(x09), .O(new_n55_));
  nand2  g21(.a(new_n48_), .b(new_n36_), .O(new_n56_));
  inv1   g22(.a(x14), .O(new_n57_));
  inv1   g23(.a(x09), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(x08), .c(x07), .d(x06), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n56_), .c(new_n55_), .O(z05));
  inv1   g28(.a(x10), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(new_n58_), .c(x08), .d(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n57_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  nand2  g32(.a(new_n61_), .b(x10), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n66_), .c(new_n45_), .d(x04), .O(z06));
  nand2  g34(.a(new_n66_), .b(x11), .O(new_n69_));
  inv1   g35(.a(x11), .O(new_n70_));
  nand3  g36(.a(new_n57_), .b(new_n70_), .c(x06), .O(new_n71_));
  or2    g37(.a(new_n71_), .b(new_n64_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n69_), .c(new_n56_), .O(z07));
  nand2  g39(.a(x08), .b(x07), .O(new_n74_));
  nor2   g40(.a(new_n74_), .b(x09), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n75_), .c(x14), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n44_), .c(x12), .O(new_n78_));
  inv1   g44(.a(new_n51_), .O(new_n79_));
  inv1   g45(.a(x08), .O(new_n80_));
  nor2   g46(.a(x09), .b(new_n80_), .O(new_n81_));
  nor3   g47(.a(x12), .b(x11), .c(x10), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n81_), .c(new_n79_), .d(new_n57_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n78_), .c(new_n56_), .O(z08));
  nor2   g50(.a(x13), .b(x12), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n76_), .c(new_n75_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n57_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x06), .O(new_n88_));
  aoi21  g54(.a(new_n83_), .b(x13), .c(new_n48_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n88_), .O(z09));
  inv1   g56(.a(x04), .O(new_n91_));
  nand2  g57(.a(new_n45_), .b(new_n44_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x14), .O(new_n93_));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n85_), .b(new_n76_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n60_), .c(new_n45_), .O(new_n97_));
  aoi22  g63(.a(new_n97_), .b(new_n93_), .c(new_n36_), .d(new_n91_), .O(z10));
  nor2   g64(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n35_), .c(x03), .O(new_n100_));
  nor3   g66(.a(new_n100_), .b(new_n37_), .c(new_n91_), .O(z12));
  nand2  g67(.a(new_n36_), .b(new_n91_), .O(z13));
  inv1   g68(.a(x17), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n91_), .c(new_n36_), .O(z14));
endmodule


