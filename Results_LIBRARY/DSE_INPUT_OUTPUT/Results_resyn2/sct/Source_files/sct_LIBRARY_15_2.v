// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n109_, new_n112_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x05), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  inv1   g09(.a(x05), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  aoi22  g11(.a(new_n45_), .b(x06), .c(new_n42_), .d(x18), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n36_), .c(new_n44_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n36_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(new_n50_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n37_), .O(z03));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n50_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n37_), .O(z04));
  inv1   g28(.a(x02), .O(new_n63_));
  oai21  g29(.a(x03), .b(new_n63_), .c(x16), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand2  g32(.a(new_n58_), .b(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n66_), .c(new_n64_), .d(x04), .O(new_n68_));
  and2   g34(.a(new_n68_), .b(new_n37_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  inv1   g36(.a(new_n66_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g38(.a(x08), .b(x07), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(x09), .c(x10), .O(new_n74_));
  oai21  g40(.a(x10), .b(x05), .c(new_n36_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n74_), .c(new_n72_), .d(new_n50_), .O(z06));
  nor2   g42(.a(new_n73_), .b(x09), .O(new_n77_));
  nand4  g43(.a(new_n70_), .b(new_n65_), .c(x08), .d(x07), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  aoi22  g45(.a(new_n79_), .b(new_n77_), .c(new_n78_), .d(x11), .O(new_n80_));
  nand2  g46(.a(x11), .b(new_n36_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n64_), .c(x04), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n37_), .O(new_n83_));
  oai21  g49(.a(new_n80_), .b(new_n36_), .c(new_n83_), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  nand4  g51(.a(new_n79_), .b(new_n77_), .c(new_n85_), .d(x06), .O(new_n86_));
  nand2  g52(.a(new_n79_), .b(new_n77_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n36_), .c(x12), .O(new_n88_));
  nand3  g54(.a(new_n64_), .b(new_n37_), .c(x04), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n88_), .c(new_n86_), .O(z08));
  oai21  g57(.a(new_n87_), .b(x12), .c(x13), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand3  g59(.a(new_n79_), .b(new_n93_), .c(new_n85_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  oai21  g61(.a(x13), .b(x05), .c(new_n36_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n64_), .c(x04), .O(new_n97_));
  aoi21  g63(.a(new_n95_), .b(new_n71_), .c(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n92_), .O(z09));
  inv1   g65(.a(new_n37_), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(x04), .O(new_n101_));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n94_), .b(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n71_), .c(new_n64_), .O(new_n104_));
  aoi21  g70(.a(new_n49_), .b(x14), .c(new_n100_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(new_n101_), .O(z10));
  nor2   g72(.a(new_n100_), .b(new_n63_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n63_), .c(x03), .O(new_n108_));
  nand2  g74(.a(new_n37_), .b(x04), .O(new_n109_));
  nor2   g75(.a(new_n109_), .b(new_n108_), .O(z12));
  inv1   g76(.a(new_n101_), .O(z13));
  inv1   g77(.a(x17), .O(new_n112_));
  nor2   g78(.a(new_n109_), .b(new_n112_), .O(z14));
endmodule


