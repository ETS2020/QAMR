// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n102_,
    new_n105_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x15), .b(x08), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x15), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x05), .c(x08), .O(new_n43_));
  aoi22  g09(.a(new_n43_), .b(new_n41_), .c(x05), .d(x04), .O(z01));
  inv1   g10(.a(new_n37_), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n36_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  oai21  g13(.a(new_n47_), .b(x06), .c(new_n45_), .O(z02));
  xnor2a g14(.a(x07), .b(x06), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(new_n47_), .c(new_n45_), .O(z03));
  inv1   g16(.a(x08), .O(new_n51_));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n41_), .c(new_n51_), .O(new_n53_));
  oai21  g19(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n47_), .O(z04));
  inv1   g21(.a(x04), .O(new_n56_));
  inv1   g22(.a(x03), .O(new_n57_));
  inv1   g23(.a(x16), .O(new_n58_));
  aoi21  g24(.a(new_n57_), .b(x02), .c(new_n58_), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  oai21  g26(.a(new_n52_), .b(new_n51_), .c(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(new_n64_));
  and2   g30(.a(new_n64_), .b(new_n45_), .O(z05));
  oai21  g31(.a(new_n41_), .b(x10), .c(new_n51_), .O(new_n66_));
  oai21  g32(.a(new_n52_), .b(x09), .c(x10), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  inv1   g34(.a(new_n63_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n67_), .c(new_n66_), .d(new_n60_), .O(z06));
  inv1   g37(.a(x11), .O(new_n72_));
  nor2   g38(.a(new_n63_), .b(x10), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai21  g40(.a(new_n37_), .b(x11), .c(new_n70_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n60_), .O(z07));
  inv1   g42(.a(x12), .O(new_n77_));
  nand3  g43(.a(new_n73_), .b(new_n77_), .c(new_n72_), .O(new_n78_));
  nand2  g44(.a(new_n45_), .b(new_n77_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n74_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n60_), .O(z08));
  inv1   g47(.a(x13), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(x08), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n47_), .c(new_n45_), .O(new_n84_));
  nor2   g50(.a(new_n52_), .b(x09), .O(new_n85_));
  nor2   g51(.a(x12), .b(x11), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n68_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(new_n82_), .O(new_n88_));
  nand2  g54(.a(new_n87_), .b(new_n82_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x08), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n88_), .c(new_n84_), .O(z09));
  inv1   g57(.a(x00), .O(new_n92_));
  nand3  g58(.a(new_n86_), .b(new_n82_), .c(new_n68_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g60(.a(new_n63_), .b(new_n59_), .O(new_n95_));
  inv1   g61(.a(x14), .O(new_n96_));
  nor2   g62(.a(new_n46_), .b(new_n96_), .O(new_n97_));
  aoi21  g63(.a(new_n95_), .b(new_n94_), .c(new_n97_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n56_), .c(new_n45_), .O(z10));
  nand2  g65(.a(new_n45_), .b(new_n36_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n36_), .c(x03), .O(new_n101_));
  nand2  g67(.a(new_n45_), .b(x04), .O(new_n102_));
  nor2   g68(.a(new_n102_), .b(new_n101_), .O(z12));
  inv1   g69(.a(new_n102_), .O(z13));
  inv1   g70(.a(x17), .O(new_n105_));
  nor2   g71(.a(new_n102_), .b(new_n105_), .O(z14));
endmodule


