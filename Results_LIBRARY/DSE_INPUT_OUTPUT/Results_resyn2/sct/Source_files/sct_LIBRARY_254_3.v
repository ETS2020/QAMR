// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x15), .b(x06), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  nor2   g05(.a(x15), .b(x05), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  aoi21  g08(.a(new_n40_), .b(x18), .c(new_n42_), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x04), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  nand2  g15(.a(x07), .b(x06), .O(new_n50_));
  inv1   g16(.a(x07), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n44_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n46_), .c(new_n37_), .O(z03));
  xor2a  g20(.a(new_n50_), .b(x08), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n46_), .c(new_n37_), .O(z04));
  inv1   g22(.a(x09), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(x08), .c(x07), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x15), .c(x06), .O(new_n60_));
  inv1   g26(.a(x15), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n60_), .c(new_n47_), .O(z05));
  inv1   g30(.a(x10), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n57_), .c(x08), .d(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  oai21  g34(.a(new_n62_), .b(x09), .c(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n47_), .O(z06));
  inv1   g36(.a(x11), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n58_), .c(new_n61_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(new_n61_), .b(x06), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n66_), .c(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n47_), .O(z07));
  nand2  g43(.a(new_n74_), .b(x12), .O(new_n78_));
  nor2   g44(.a(new_n72_), .b(new_n58_), .O(new_n79_));
  inv1   g45(.a(x12), .O(new_n80_));
  nand3  g46(.a(new_n61_), .b(new_n80_), .c(x06), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  aoi22  g48(.a(new_n82_), .b(new_n79_), .c(new_n46_), .d(new_n37_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n78_), .O(z08));
  inv1   g50(.a(x13), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n80_), .c(new_n71_), .d(new_n65_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n58_), .c(new_n61_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x06), .O(new_n88_));
  nand2  g54(.a(new_n82_), .b(new_n79_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x13), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n88_), .c(new_n47_), .O(z09));
  nand2  g57(.a(new_n37_), .b(x04), .O(new_n92_));
  inv1   g58(.a(x03), .O(new_n93_));
  inv1   g59(.a(x16), .O(new_n94_));
  aoi21  g60(.a(new_n93_), .b(x02), .c(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x14), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n86_), .b(new_n97_), .O(new_n98_));
  nor2   g64(.a(new_n75_), .b(new_n95_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n98_), .c(new_n59_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n96_), .c(new_n92_), .O(z10));
  nand2  g67(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g68(.a(x04), .O(new_n103_));
  aoi21  g69(.a(x16), .b(new_n35_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n103_), .c(new_n37_), .O(z12));
  inv1   g71(.a(new_n92_), .O(z13));
  nand2  g72(.a(z13), .b(x17), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(z14));
endmodule


