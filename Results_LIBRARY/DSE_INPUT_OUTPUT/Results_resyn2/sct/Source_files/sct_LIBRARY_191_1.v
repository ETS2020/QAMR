// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand2  g02(.a(x06), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n36_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n54_), .b(new_n47_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n51_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n36_), .c(new_n56_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n49_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n37_), .O(z04));
  oai21  g30(.a(x09), .b(new_n36_), .c(new_n60_), .O(new_n65_));
  nor2   g31(.a(x09), .b(new_n61_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n55_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n49_), .O(new_n68_));
  and2   g34(.a(new_n68_), .b(new_n37_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  inv1   g36(.a(new_n67_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g38(.a(new_n67_), .b(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n51_), .O(z06));
  oai21  g40(.a(x11), .b(new_n36_), .c(new_n47_), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  nand4  g42(.a(new_n66_), .b(new_n55_), .c(new_n76_), .d(new_n70_), .O(new_n77_));
  inv1   g43(.a(x09), .O(new_n78_));
  nand4  g44(.a(new_n70_), .b(new_n78_), .c(x08), .d(x07), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x11), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n77_), .c(new_n49_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x04), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n75_), .O(z07));
  nand2  g49(.a(new_n77_), .b(x12), .O(new_n84_));
  nor2   g50(.a(x12), .b(x11), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n71_), .c(new_n70_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n51_), .O(z08));
  oai21  g53(.a(x13), .b(new_n36_), .c(new_n47_), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n76_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n79_), .c(x13), .O(new_n91_));
  nor2   g57(.a(x13), .b(x10), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n85_), .c(new_n66_), .d(new_n55_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n91_), .c(new_n49_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x04), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n88_), .O(z09));
  nand2  g62(.a(new_n50_), .b(x14), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n92_), .b(new_n85_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n71_), .c(new_n49_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n97_), .c(new_n36_), .O(z10));
  aoi21  g68(.a(x06), .b(new_n36_), .c(new_n48_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n48_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n36_), .c(new_n37_), .O(z12));
  and2   g71(.a(x17), .b(x04), .O(z14));
  buf    g72(.a(x04), .O(z13));
endmodule


