// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x12), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(x12), .c(new_n46_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n37_), .O(new_n52_));
  xnor2a g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n48_), .c(new_n52_), .O(z03));
  inv1   g20(.a(x07), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(x08), .O(new_n57_));
  nand2  g23(.a(new_n56_), .b(x08), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n52_), .O(new_n59_));
  nor3   g25(.a(new_n59_), .b(new_n57_), .c(new_n48_), .O(z04));
  nand2  g26(.a(new_n48_), .b(new_n52_), .O(new_n61_));
  nand3  g27(.a(x12), .b(x09), .c(new_n46_), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  aoi21  g29(.a(x08), .b(x07), .c(new_n63_), .O(new_n64_));
  nand2  g30(.a(x08), .b(x07), .O(new_n65_));
  nor2   g31(.a(new_n65_), .b(x09), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n64_), .c(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n62_), .c(new_n61_), .O(z05));
  nand3  g34(.a(new_n56_), .b(new_n63_), .c(x08), .O(new_n69_));
  or2    g35(.a(new_n69_), .b(x10), .O(new_n70_));
  oai21  g36(.a(new_n37_), .b(x10), .c(new_n69_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n49_), .O(z06));
  inv1   g38(.a(x10), .O(new_n73_));
  nand2  g39(.a(new_n66_), .b(new_n73_), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  aoi22  g41(.a(new_n75_), .b(new_n66_), .c(new_n74_), .d(x11), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  nor2   g43(.a(new_n77_), .b(x06), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n48_), .c(new_n52_), .O(new_n79_));
  oai21  g45(.a(new_n76_), .b(new_n46_), .c(new_n79_), .O(z07));
  inv1   g46(.a(x12), .O(new_n81_));
  inv1   g47(.a(new_n75_), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(new_n69_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  oai21  g50(.a(new_n82_), .b(new_n69_), .c(x12), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n61_), .O(z08));
  inv1   g52(.a(x13), .O(new_n87_));
  aoi21  g53(.a(new_n75_), .b(new_n66_), .c(new_n87_), .O(new_n88_));
  nand3  g54(.a(new_n63_), .b(x08), .c(x07), .O(new_n89_));
  nand3  g55(.a(new_n75_), .b(new_n87_), .c(new_n81_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n88_), .c(x06), .O(new_n92_));
  nand2  g58(.a(x13), .b(x12), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n61_), .O(z09));
  nand2  g60(.a(x06), .b(x00), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n90_), .O(new_n96_));
  inv1   g62(.a(x03), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x02), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(x16), .c(new_n89_), .O(new_n99_));
  inv1   g65(.a(x14), .O(new_n100_));
  nor2   g66(.a(new_n47_), .b(new_n100_), .O(new_n101_));
  aoi21  g67(.a(new_n99_), .b(new_n96_), .c(new_n101_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n43_), .c(new_n52_), .O(z10));
  nor2   g69(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n36_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n43_), .c(new_n52_), .O(z12));
  nor2   g72(.a(new_n37_), .b(new_n43_), .O(z13));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n52_), .O(z14));
endmodule


