// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x07), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x06), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n38_), .O(z02));
  inv1   g16(.a(x06), .O(new_n51_));
  inv1   g17(.a(new_n49_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(x07), .c(new_n51_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z03));
  xnor2a g20(.a(x08), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n49_), .c(new_n38_), .O(z04));
  inv1   g22(.a(x08), .O(new_n57_));
  nor2   g23(.a(x09), .b(new_n57_), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n37_), .c(x06), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n52_), .O(z05));
  inv1   g28(.a(x09), .O(new_n63_));
  inv1   g29(.a(x10), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n63_), .c(x08), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n37_), .c(x06), .O(new_n67_));
  inv1   g33(.a(new_n60_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n63_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x10), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n67_), .c(new_n52_), .O(z06));
  inv1   g37(.a(x11), .O(new_n72_));
  nor2   g38(.a(new_n66_), .b(new_n72_), .O(new_n73_));
  nand2  g39(.a(new_n72_), .b(new_n64_), .O(new_n74_));
  nand3  g40(.a(new_n63_), .b(x08), .c(x06), .O(new_n75_));
  nor2   g41(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n73_), .c(x07), .O(new_n77_));
  nand2  g43(.a(new_n49_), .b(new_n38_), .O(new_n78_));
  nand2  g44(.a(x11), .b(new_n51_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(z07));
  inv1   g46(.a(x12), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n58_), .c(new_n81_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x07), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x06), .O(new_n85_));
  oai21  g51(.a(new_n74_), .b(new_n69_), .c(x12), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n52_), .O(z08));
  nor2   g53(.a(x13), .b(x12), .O(new_n88_));
  aoi22  g54(.a(new_n88_), .b(new_n76_), .c(new_n83_), .d(x13), .O(new_n89_));
  aoi22  g55(.a(new_n49_), .b(new_n38_), .c(x13), .d(new_n51_), .O(new_n90_));
  oai21  g56(.a(new_n89_), .b(new_n37_), .c(new_n90_), .O(z09));
  inv1   g57(.a(x04), .O(new_n92_));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n88_), .b(new_n82_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  inv1   g61(.a(x03), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x02), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(x16), .c(new_n75_), .O(new_n98_));
  nor2   g64(.a(new_n48_), .b(new_n40_), .O(new_n99_));
  aoi21  g65(.a(new_n98_), .b(new_n95_), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n92_), .c(new_n38_), .O(z10));
  nand2  g67(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n35_), .c(x03), .O(new_n103_));
  nand2  g69(.a(new_n38_), .b(x04), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n103_), .O(z12));
  inv1   g71(.a(new_n104_), .O(z13));
  inv1   g72(.a(x17), .O(new_n107_));
  nor2   g73(.a(new_n104_), .b(new_n107_), .O(z14));
endmodule


