// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x12), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  inv1   g14(.a(x12), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(x06), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xnor2a g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n47_), .c(new_n38_), .O(z03));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  xor2a  g21(.a(new_n55_), .b(x08), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n47_), .c(new_n38_), .O(z04));
  nand2  g23(.a(new_n47_), .b(new_n38_), .O(new_n58_));
  nand2  g24(.a(new_n50_), .b(x09), .O(new_n59_));
  inv1   g25(.a(x09), .O(new_n60_));
  aoi21  g26(.a(x08), .b(x07), .c(new_n60_), .O(new_n61_));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(x09), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n61_), .c(x06), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n59_), .c(new_n58_), .O(z05));
  inv1   g31(.a(x10), .O(new_n66_));
  nand3  g32(.a(new_n60_), .b(x08), .c(x07), .O(new_n67_));
  inv1   g33(.a(x06), .O(new_n68_));
  aoi21  g34(.a(new_n67_), .b(new_n66_), .c(new_n68_), .O(new_n69_));
  oai21  g35(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n50_), .b(x10), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n58_), .O(z06));
  inv1   g38(.a(new_n55_), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(x08), .O(new_n75_));
  or2    g41(.a(new_n75_), .b(x11), .O(new_n76_));
  oai21  g42(.a(new_n37_), .b(x11), .c(new_n75_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n48_), .O(z07));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n63_), .c(new_n49_), .O(new_n80_));
  nand2  g46(.a(new_n79_), .b(new_n63_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x03), .O(new_n83_));
  inv1   g49(.a(x16), .O(new_n84_));
  aoi21  g50(.a(new_n83_), .b(x02), .c(new_n84_), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(new_n68_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n82_), .c(new_n80_), .d(x04), .O(z08));
  inv1   g53(.a(x13), .O(new_n88_));
  aoi21  g54(.a(new_n79_), .b(new_n63_), .c(new_n88_), .O(new_n89_));
  nand3  g55(.a(new_n79_), .b(new_n88_), .c(new_n49_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n67_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n89_), .c(x06), .O(new_n92_));
  aoi22  g58(.a(new_n47_), .b(new_n38_), .c(x13), .d(x12), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n92_), .O(z09));
  inv1   g60(.a(x04), .O(new_n95_));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n90_), .b(new_n96_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n86_), .c(new_n63_), .O(new_n98_));
  nand3  g64(.a(new_n85_), .b(new_n38_), .c(x14), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(new_n95_), .O(z10));
  nand2  g66(.a(new_n38_), .b(new_n35_), .O(z11));
  nand2  g67(.a(x16), .b(new_n35_), .O(new_n102_));
  nor2   g68(.a(new_n37_), .b(x03), .O(new_n103_));
  aoi22  g69(.a(new_n103_), .b(new_n102_), .c(new_n38_), .d(new_n95_), .O(z12));
  nand2  g70(.a(new_n38_), .b(new_n95_), .O(z13));
  inv1   g71(.a(x17), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n95_), .c(new_n38_), .O(z14));
endmodule


