// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n106_, new_n108_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x08), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x05), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x15), .O(new_n43_));
  nand3  g09(.a(x18), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  oai21  g11(.a(new_n42_), .b(x04), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  nor2   g14(.a(new_n38_), .b(new_n48_), .O(z13));
  oai21  g15(.a(x03), .b(new_n36_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(z13), .c(new_n47_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n38_), .O(new_n53_));
  nand2  g19(.a(new_n50_), .b(x04), .O(new_n54_));
  xnor2a g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(z03));
  inv1   g22(.a(new_n54_), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  nand2  g24(.a(x07), .b(x06), .O(new_n59_));
  oai21  g25(.a(new_n37_), .b(new_n42_), .c(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nand2  g28(.a(new_n58_), .b(x09), .O(new_n63_));
  oai21  g29(.a(x09), .b(x08), .c(new_n42_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n64_), .c(new_n63_), .d(new_n57_), .O(z05));
  oai22  g33(.a(new_n66_), .b(new_n42_), .c(new_n38_), .d(x10), .O(new_n68_));
  inv1   g34(.a(new_n58_), .O(new_n69_));
  nor2   g35(.a(x10), .b(x09), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n57_), .O(z06));
  inv1   g38(.a(x11), .O(new_n73_));
  nand3  g39(.a(new_n71_), .b(new_n53_), .c(new_n73_), .O(new_n74_));
  inv1   g40(.a(x10), .O(new_n75_));
  inv1   g41(.a(new_n66_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(x11), .c(new_n75_), .d(x05), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n57_), .O(z07));
  inv1   g45(.a(x12), .O(new_n80_));
  nor2   g46(.a(new_n59_), .b(x11), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n70_), .c(new_n80_), .O(new_n82_));
  nor2   g48(.a(x12), .b(x11), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n70_), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n58_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n82_), .c(x05), .O(new_n86_));
  aoi22  g52(.a(new_n54_), .b(new_n53_), .c(x12), .d(new_n37_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n86_), .O(z08));
  nand4  g54(.a(new_n83_), .b(new_n70_), .c(x07), .d(x06), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n83_), .b(new_n90_), .c(new_n75_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  aoi22  g58(.a(new_n92_), .b(new_n76_), .c(new_n89_), .d(x13), .O(new_n93_));
  aoi22  g59(.a(new_n54_), .b(new_n53_), .c(x13), .d(new_n37_), .O(new_n94_));
  oai21  g60(.a(new_n93_), .b(new_n42_), .c(new_n94_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  inv1   g63(.a(x03), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x02), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(x16), .c(new_n66_), .O(new_n100_));
  inv1   g66(.a(x14), .O(new_n101_));
  nor2   g67(.a(new_n50_), .b(new_n101_), .O(new_n102_));
  aoi21  g68(.a(new_n100_), .b(new_n97_), .c(new_n102_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n48_), .c(new_n53_), .O(z10));
  nor2   g70(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n36_), .c(x03), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n48_), .c(new_n53_), .O(z12));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n53_), .O(z14));
endmodule


