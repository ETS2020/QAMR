// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n98_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n39_), .c(new_n36_), .O(z00));
  oai21  g09(.a(new_n38_), .b(x05), .c(new_n37_), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  nor2   g11(.a(new_n45_), .b(x15), .O(new_n46_));
  oai21  g12(.a(new_n46_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x04), .c(x06), .O(z02));
  nor2   g17(.a(new_n50_), .b(new_n37_), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nor2   g19(.a(new_n53_), .b(new_n38_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(new_n55_));
  nand2  g21(.a(new_n53_), .b(new_n38_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n52_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n55_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n52_), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  nand2  g29(.a(new_n59_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n52_), .O(z05));
  nand2  g33(.a(new_n66_), .b(x10), .O(new_n68_));
  inv1   g34(.a(x10), .O(new_n69_));
  nand4  g35(.a(new_n54_), .b(new_n69_), .c(new_n65_), .d(x08), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n52_), .O(z06));
  nor2   g37(.a(x11), .b(x10), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nand3  g39(.a(new_n65_), .b(x08), .c(x07), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n73_), .c(x04), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  aoi21  g42(.a(new_n70_), .b(x11), .c(new_n50_), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n37_), .c(new_n76_), .O(z07));
  oai21  g44(.a(new_n70_), .b(x11), .c(x12), .O(new_n79_));
  inv1   g45(.a(new_n59_), .O(new_n80_));
  nor2   g46(.a(x12), .b(x09), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n72_), .c(new_n80_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n79_), .c(new_n52_), .O(z08));
  nor2   g49(.a(x13), .b(x12), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n72_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n74_), .c(x04), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x06), .O(new_n87_));
  aoi21  g53(.a(new_n82_), .b(x13), .c(new_n50_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n37_), .c(new_n87_), .O(z09));
  inv1   g55(.a(x00), .O(new_n90_));
  nand2  g56(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  nor2   g57(.a(new_n66_), .b(new_n50_), .O(new_n92_));
  oai21  g58(.a(x03), .b(new_n35_), .c(x16), .O(new_n93_));
  nor2   g59(.a(new_n93_), .b(new_n41_), .O(new_n94_));
  aoi21  g60(.a(new_n92_), .b(new_n91_), .c(new_n94_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n37_), .c(new_n39_), .O(z10));
  nand2  g62(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g63(.a(x16), .b(new_n35_), .c(x03), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n37_), .c(new_n39_), .O(z12));
  nand2  g65(.a(x06), .b(new_n37_), .O(z13));
  and2   g66(.a(x17), .b(x04), .O(z14));
endmodule


