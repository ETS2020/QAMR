// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n107_, new_n109_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand2  g02(.a(x06), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor3   g15(.a(new_n49_), .b(x06), .c(new_n36_), .O(z02));
  inv1   g16(.a(new_n49_), .O(new_n51_));
  oai21  g17(.a(x06), .b(new_n36_), .c(x07), .O(new_n52_));
  inv1   g18(.a(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n52_), .c(new_n51_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n37_), .O(z03));
  aoi22  g23(.a(x08), .b(x04), .c(x07), .d(x06), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n58_), .c(new_n37_), .O(z04));
  inv1   g27(.a(x09), .O(new_n62_));
  inv1   g28(.a(new_n59_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g30(.a(new_n59_), .b(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n64_), .c(new_n51_), .d(x04), .O(z05));
  inv1   g32(.a(x10), .O(new_n67_));
  nand2  g33(.a(x08), .b(x07), .O(new_n68_));
  nor2   g34(.a(new_n68_), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(x06), .O(new_n70_));
  nand2  g36(.a(new_n67_), .b(x04), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n64_), .c(new_n49_), .O(new_n72_));
  aoi22  g38(.a(new_n72_), .b(new_n70_), .c(x06), .d(new_n36_), .O(z06));
  oai21  g39(.a(x11), .b(new_n36_), .c(new_n53_), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  nand4  g41(.a(new_n69_), .b(new_n75_), .c(new_n67_), .d(x06), .O(new_n76_));
  nand3  g42(.a(new_n62_), .b(x08), .c(x07), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(x10), .c(x11), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n76_), .c(new_n51_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n74_), .O(z07));
  nand2  g47(.a(new_n76_), .b(x12), .O(new_n82_));
  nor3   g48(.a(x12), .b(x11), .c(x10), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n69_), .c(x06), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n82_), .c(new_n51_), .d(x04), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n53_), .O(new_n88_));
  nand4  g54(.a(new_n83_), .b(new_n63_), .c(new_n86_), .d(new_n62_), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n75_), .c(new_n67_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n77_), .c(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n89_), .c(new_n51_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x04), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n88_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  oai21  g62(.a(new_n87_), .b(new_n91_), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n47_), .b(x02), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(x16), .c(new_n77_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n97_), .c(new_n36_), .O(new_n100_));
  nand3  g66(.a(new_n49_), .b(x14), .c(x04), .O(new_n101_));
  oai21  g67(.a(new_n100_), .b(new_n53_), .c(new_n101_), .O(z10));
  inv1   g68(.a(x02), .O(new_n103_));
  nand2  g69(.a(new_n37_), .b(new_n103_), .O(z11));
  nand2  g70(.a(x16), .b(new_n103_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n47_), .c(new_n36_), .O(z12));
  nor2   g72(.a(x06), .b(x04), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  aoi21  g75(.a(new_n109_), .b(x04), .c(new_n107_), .O(z14));
endmodule


