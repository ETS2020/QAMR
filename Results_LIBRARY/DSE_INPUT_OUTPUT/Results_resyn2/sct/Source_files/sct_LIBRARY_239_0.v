// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x17), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x04), .O(new_n47_));
  inv1   g13(.a(x17), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor4   g17(.a(new_n51_), .b(new_n48_), .c(x06), .d(new_n47_), .O(z02));
  nand2  g18(.a(new_n49_), .b(x02), .O(new_n53_));
  aoi21  g19(.a(new_n53_), .b(x16), .c(new_n47_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(x06), .O(new_n56_));
  inv1   g22(.a(x07), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n55_), .c(new_n54_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n41_), .O(z03));
  inv1   g26(.a(x08), .O(new_n61_));
  oai21  g27(.a(new_n37_), .b(new_n61_), .c(new_n55_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n62_), .c(new_n54_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  oai21  g31(.a(new_n37_), .b(x09), .c(new_n63_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n54_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nand2  g36(.a(x08), .b(x07), .O(new_n71_));
  nor2   g37(.a(new_n71_), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(x06), .O(new_n73_));
  nand2  g39(.a(new_n68_), .b(x10), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n54_), .O(new_n75_));
  and2   g41(.a(new_n75_), .b(new_n41_), .O(z06));
  nand4  g42(.a(new_n70_), .b(new_n67_), .c(x08), .d(x07), .O(new_n77_));
  xor2a  g43(.a(new_n77_), .b(x11), .O(new_n78_));
  oai21  g44(.a(x03), .b(new_n36_), .c(x16), .O(new_n79_));
  nand2  g45(.a(x11), .b(new_n56_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n41_), .O(new_n82_));
  oai21  g48(.a(new_n78_), .b(new_n56_), .c(new_n82_), .O(z07));
  oai21  g49(.a(new_n77_), .b(x11), .c(x12), .O(new_n84_));
  inv1   g50(.a(new_n68_), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(x10), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n85_), .O(new_n90_));
  oai21  g56(.a(new_n48_), .b(x12), .c(new_n56_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n84_), .d(new_n54_), .O(z08));
  oai21  g58(.a(new_n88_), .b(new_n77_), .c(x13), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n87_), .c(new_n86_), .d(new_n70_), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n85_), .O(new_n97_));
  oai21  g63(.a(new_n48_), .b(x13), .c(new_n56_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n97_), .c(new_n93_), .d(new_n54_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n95_), .b(new_n100_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n85_), .c(new_n79_), .O(new_n102_));
  nand3  g68(.a(new_n51_), .b(new_n41_), .c(x14), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n47_), .O(z10));
  nand2  g70(.a(new_n41_), .b(new_n36_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n36_), .c(x03), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n47_), .c(new_n41_), .O(z12));
  nor2   g73(.a(new_n37_), .b(new_n47_), .O(z13));
  oai21  g74(.a(new_n48_), .b(new_n47_), .c(new_n41_), .O(z14));
endmodule


