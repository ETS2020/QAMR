// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n107_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x06), .b(x05), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x15), .O(new_n43_));
  nand3  g09(.a(x18), .b(new_n43_), .c(new_n42_), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n41_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x02), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(x16), .c(new_n49_), .O(new_n52_));
  nand3  g18(.a(new_n52_), .b(new_n48_), .c(x05), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  oai21  g22(.a(x06), .b(new_n42_), .c(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n52_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  oai22  g26(.a(new_n37_), .b(new_n60_), .c(new_n55_), .d(new_n48_), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(new_n52_), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  nor2   g30(.a(new_n62_), .b(x09), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nand3  g32(.a(x08), .b(x07), .c(x06), .O(new_n67_));
  oai21  g33(.a(new_n37_), .b(x09), .c(new_n67_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n52_), .O(z05));
  oai21  g35(.a(new_n37_), .b(x10), .c(new_n66_), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(new_n65_), .b(new_n71_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n52_), .O(z06));
  inv1   g39(.a(x09), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  inv1   g41(.a(new_n67_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n75_), .c(new_n71_), .d(new_n74_), .O(new_n77_));
  nand2  g43(.a(new_n71_), .b(new_n74_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n67_), .c(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n52_), .O(new_n80_));
  and2   g46(.a(new_n80_), .b(new_n41_), .O(z07));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand2  g48(.a(x08), .b(x07), .O(new_n83_));
  nor2   g49(.a(new_n83_), .b(x09), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x12), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand3  g53(.a(new_n82_), .b(new_n65_), .c(new_n87_), .O(new_n88_));
  oai21  g54(.a(x12), .b(new_n42_), .c(new_n48_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(new_n52_), .O(z08));
  nand2  g56(.a(new_n87_), .b(x06), .O(new_n91_));
  oai22  g57(.a(new_n91_), .b(new_n85_), .c(new_n37_), .d(x13), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand3  g59(.a(new_n82_), .b(new_n93_), .c(new_n87_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n65_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n92_), .c(new_n52_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n94_), .b(new_n98_), .O(new_n99_));
  aoi21  g65(.a(new_n51_), .b(x16), .c(new_n48_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n99_), .c(new_n84_), .O(new_n101_));
  nand4  g67(.a(new_n51_), .b(new_n41_), .c(x16), .d(x14), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n49_), .O(z10));
  nor2   g69(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n36_), .c(x03), .O(new_n105_));
  nor2   g71(.a(new_n37_), .b(new_n49_), .O(z13));
  inv1   g72(.a(z13), .O(new_n107_));
  nor2   g73(.a(new_n107_), .b(new_n105_), .O(z12));
  and2   g74(.a(z13), .b(x17), .O(z14));
endmodule


