// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n94_, new_n97_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x12), .b(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n38_), .O(new_n52_));
  xnor2a g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n48_), .c(new_n52_), .O(z03));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(x08), .O(new_n57_));
  nand2  g23(.a(new_n56_), .b(x08), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n52_), .O(new_n59_));
  nor3   g25(.a(new_n59_), .b(new_n57_), .c(new_n48_), .O(z04));
  inv1   g26(.a(x12), .O(new_n61_));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(x09), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n61_), .c(x06), .O(new_n64_));
  nand3  g30(.a(new_n56_), .b(x12), .c(x08), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n49_), .O(z05));
  nand2  g33(.a(new_n64_), .b(x10), .O(new_n68_));
  nand2  g34(.a(new_n48_), .b(new_n52_), .O(new_n69_));
  inv1   g35(.a(new_n65_), .O(new_n70_));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n69_), .c(new_n68_), .O(z06));
  nor2   g39(.a(new_n62_), .b(x11), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x12), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  inv1   g43(.a(new_n71_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n65_), .c(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n49_), .O(z07));
  oai21  g46(.a(new_n75_), .b(new_n37_), .c(x12), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n69_), .O(z08));
  nand2  g48(.a(new_n52_), .b(x13), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n69_), .O(z09));
  inv1   g50(.a(x03), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x02), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(x16), .c(x14), .O(new_n87_));
  and2   g53(.a(x06), .b(x00), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n63_), .c(new_n47_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x04), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n52_), .O(z10));
  nor2   g58(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g59(.a(x16), .b(new_n36_), .c(x03), .O(new_n94_));
  nor3   g60(.a(new_n94_), .b(new_n38_), .c(new_n44_), .O(z12));
  nor2   g61(.a(new_n38_), .b(new_n44_), .O(z13));
  nand2  g62(.a(x17), .b(x04), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n52_), .O(z14));
endmodule


