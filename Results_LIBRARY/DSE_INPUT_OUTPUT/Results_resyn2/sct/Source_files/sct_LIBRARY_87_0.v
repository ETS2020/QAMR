// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n109_,
    new_n111_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x04), .c(x06), .O(z02));
  oai21  g16(.a(x03), .b(new_n36_), .c(x16), .O(new_n51_));
  inv1   g17(.a(x04), .O(new_n52_));
  inv1   g18(.a(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  oai21  g20(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n51_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n41_), .O(z03));
  nand2  g24(.a(x06), .b(new_n52_), .O(z13));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n56_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n61_), .c(z13), .d(new_n51_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n41_), .O(z04));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x08), .c(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x04), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  aoi21  g34(.a(new_n62_), .b(x09), .c(new_n49_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n52_), .c(new_n68_), .O(z05));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nor2   g37(.a(new_n60_), .b(new_n54_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x04), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  nand2  g42(.a(new_n51_), .b(new_n76_), .O(new_n77_));
  nand4  g43(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n49_), .O(new_n79_));
  inv1   g45(.a(new_n79_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n77_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n75_), .O(z06));
  oai21  g48(.a(new_n73_), .b(x11), .c(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  inv1   g51(.a(new_n62_), .O(new_n86_));
  aoi21  g52(.a(new_n71_), .b(new_n86_), .c(new_n85_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n49_), .c(x04), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n84_), .O(z07));
  nor2   g55(.a(x12), .b(x11), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n72_), .c(new_n71_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x04), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x06), .O(new_n93_));
  nand4  g59(.a(new_n72_), .b(new_n71_), .c(new_n85_), .d(x06), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(x12), .c(new_n49_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n52_), .c(new_n93_), .O(z08));
  oai21  g62(.a(new_n91_), .b(new_n53_), .c(x13), .O(new_n97_));
  inv1   g63(.a(x13), .O(new_n98_));
  nand3  g64(.a(new_n90_), .b(new_n98_), .c(new_n76_), .O(new_n99_));
  or2    g65(.a(new_n99_), .b(new_n78_), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n97_), .c(new_n51_), .d(x04), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n99_), .b(new_n102_), .O(new_n103_));
  inv1   g69(.a(x14), .O(new_n104_));
  nor2   g70(.a(new_n51_), .b(new_n104_), .O(new_n105_));
  aoi21  g71(.a(new_n103_), .b(new_n79_), .c(new_n105_), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n52_), .c(new_n41_), .O(z10));
  nor2   g73(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g74(.a(x16), .b(new_n36_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n52_), .c(new_n41_), .O(z12));
  nor2   g76(.a(x17), .b(new_n52_), .O(new_n111_));
  aoi21  g77(.a(x06), .b(new_n52_), .c(new_n111_), .O(z14));
endmodule


