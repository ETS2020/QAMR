// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x17), .b(x06), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  nor2   g05(.a(x15), .b(x05), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  aoi21  g08(.a(new_n40_), .b(x18), .c(new_n42_), .O(z01));
  oai21  g09(.a(x03), .b(new_n35_), .c(x16), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(x04), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x06), .c(new_n37_), .O(z02));
  inv1   g12(.a(x04), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g17(.a(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand4  g21(.a(new_n55_), .b(new_n54_), .c(new_n51_), .d(new_n37_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  xor2a  g23(.a(new_n55_), .b(x08), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n45_), .c(new_n37_), .O(z04));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(x17), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x17), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x08), .c(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n63_), .c(new_n51_), .O(z05));
  nand2  g35(.a(new_n68_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand3  g37(.a(new_n64_), .b(new_n71_), .c(x06), .O(new_n72_));
  or2    g38(.a(new_n72_), .b(new_n66_), .O(new_n73_));
  nand2  g39(.a(new_n45_), .b(new_n37_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n70_), .O(z06));
  inv1   g41(.a(x11), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n71_), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n66_), .c(new_n64_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  nand2  g45(.a(new_n73_), .b(x11), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n51_), .O(z07));
  nand2  g47(.a(new_n79_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nor2   g49(.a(x11), .b(x10), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n65_), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  aoi22  g52(.a(new_n86_), .b(new_n61_), .c(new_n45_), .d(new_n37_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n82_), .O(z08));
  nor2   g54(.a(x13), .b(x12), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n84_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n66_), .c(new_n64_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x06), .O(new_n92_));
  oai21  g58(.a(new_n85_), .b(new_n62_), .c(x13), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n51_), .O(z09));
  nand2  g60(.a(new_n37_), .b(x04), .O(new_n95_));
  nand2  g61(.a(new_n50_), .b(x14), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n90_), .b(new_n97_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n61_), .c(new_n44_), .d(new_n65_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n96_), .c(new_n95_), .O(z10));
  aoi21  g66(.a(x17), .b(x06), .c(new_n35_), .O(z11));
  nand2  g67(.a(x16), .b(new_n35_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n48_), .c(new_n95_), .O(z12));
  nand2  g69(.a(new_n37_), .b(new_n47_), .O(z13));
  aoi21  g70(.a(new_n52_), .b(new_n47_), .c(new_n64_), .O(z14));
endmodule


