// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x12), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(x12), .c(new_n46_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nor2   g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(new_n37_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor3   g21(.a(new_n55_), .b(new_n52_), .c(new_n48_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  oai21  g23(.a(new_n37_), .b(new_n57_), .c(new_n54_), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n58_), .c(new_n49_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand2  g27(.a(new_n48_), .b(new_n53_), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  aoi21  g29(.a(new_n59_), .b(new_n63_), .c(new_n37_), .O(new_n64_));
  oai21  g30(.a(new_n59_), .b(new_n63_), .c(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n62_), .O(z05));
  inv1   g32(.a(x10), .O(new_n67_));
  nor2   g33(.a(new_n59_), .b(x09), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  oai22  g35(.a(new_n59_), .b(x09), .c(new_n37_), .d(x10), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n49_), .O(z06));
  nand2  g37(.a(x08), .b(x07), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n67_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x11), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n68_), .O(new_n77_));
  inv1   g43(.a(x12), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(x11), .c(new_n46_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n77_), .c(new_n75_), .d(new_n49_), .O(z07));
  nand2  g46(.a(new_n77_), .b(x12), .O(new_n81_));
  nor3   g47(.a(x12), .b(x11), .c(x10), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n68_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n62_), .O(z08));
  nand3  g50(.a(new_n82_), .b(new_n68_), .c(x13), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand2  g53(.a(new_n53_), .b(new_n87_), .O(new_n88_));
  aoi21  g54(.a(new_n82_), .b(new_n73_), .c(new_n88_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n86_), .c(new_n49_), .O(z09));
  nor2   g56(.a(x13), .b(x12), .O(new_n91_));
  aoi22  g57(.a(new_n91_), .b(new_n76_), .c(x06), .d(x00), .O(new_n92_));
  nand2  g58(.a(new_n73_), .b(new_n47_), .O(new_n93_));
  inv1   g59(.a(x03), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x02), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(x16), .c(x14), .O(new_n96_));
  oai21  g62(.a(new_n93_), .b(new_n92_), .c(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x04), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n53_), .O(z10));
  nor2   g65(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n36_), .c(x03), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n43_), .c(new_n53_), .O(z12));
  nor2   g68(.a(new_n37_), .b(new_n43_), .O(z13));
  and2   g69(.a(z13), .b(x17), .O(z14));
endmodule


