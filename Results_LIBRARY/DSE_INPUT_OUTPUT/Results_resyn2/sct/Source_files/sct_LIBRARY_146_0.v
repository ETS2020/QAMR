// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n101_,
    new_n104_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x18), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x18), .c(x06), .O(z02));
  nor2   g14(.a(x07), .b(x06), .O(new_n49_));
  nand2  g15(.a(x07), .b(x06), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n38_), .O(new_n51_));
  nor3   g17(.a(new_n51_), .b(new_n49_), .c(new_n47_), .O(z03));
  xor2a  g18(.a(new_n50_), .b(x08), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n47_), .c(new_n38_), .O(z04));
  inv1   g20(.a(x08), .O(new_n55_));
  oai21  g21(.a(new_n50_), .b(new_n55_), .c(x09), .O(new_n56_));
  inv1   g22(.a(x09), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(x08), .c(x07), .d(x06), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n56_), .c(new_n46_), .d(x04), .O(new_n59_));
  and2   g25(.a(new_n59_), .b(new_n38_), .O(z05));
  inv1   g26(.a(new_n47_), .O(new_n61_));
  inv1   g27(.a(x06), .O(new_n62_));
  inv1   g28(.a(x18), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x10), .c(new_n62_), .O(new_n64_));
  inv1   g30(.a(x10), .O(new_n65_));
  inv1   g31(.a(new_n58_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g33(.a(new_n57_), .b(x08), .c(x07), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x10), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n64_), .d(new_n61_), .O(z06));
  oai22  g36(.a(new_n58_), .b(x10), .c(new_n37_), .d(x11), .O(new_n71_));
  inv1   g37(.a(x11), .O(new_n72_));
  nand3  g38(.a(new_n66_), .b(new_n72_), .c(new_n65_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n61_), .O(z07));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n57_), .c(x08), .d(x07), .O(new_n76_));
  xor2a  g42(.a(new_n76_), .b(x12), .O(new_n77_));
  nand2  g43(.a(x12), .b(new_n62_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n46_), .c(x04), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n38_), .O(new_n80_));
  oai21  g46(.a(new_n77_), .b(new_n62_), .c(new_n80_), .O(z08));
  oai21  g47(.a(new_n76_), .b(x12), .c(x13), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  inv1   g49(.a(x13), .O(new_n84_));
  nand3  g50(.a(new_n75_), .b(new_n84_), .c(new_n83_), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n66_), .O(new_n87_));
  oai21  g53(.a(new_n63_), .b(x13), .c(new_n62_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n87_), .c(new_n82_), .d(new_n61_), .O(z09));
  nor2   g55(.a(new_n37_), .b(x04), .O(new_n90_));
  inv1   g56(.a(x00), .O(new_n91_));
  nand2  g57(.a(new_n85_), .b(new_n91_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n66_), .c(new_n46_), .O(new_n93_));
  inv1   g59(.a(x03), .O(new_n94_));
  inv1   g60(.a(x16), .O(new_n95_));
  aoi21  g61(.a(new_n94_), .b(x02), .c(new_n95_), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(x14), .c(new_n37_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n93_), .c(new_n90_), .O(z10));
  nand2  g64(.a(new_n38_), .b(new_n35_), .O(z11));
  inv1   g65(.a(x04), .O(new_n100_));
  aoi21  g66(.a(x16), .b(new_n35_), .c(x03), .O(new_n101_));
  nor3   g67(.a(new_n101_), .b(new_n37_), .c(new_n100_), .O(z12));
  inv1   g68(.a(new_n90_), .O(z13));
  inv1   g69(.a(x17), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n100_), .c(new_n38_), .O(z14));
endmodule


