// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n110_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x13), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n37_), .c(new_n47_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n38_), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(new_n57_));
  nor2   g23(.a(new_n57_), .b(new_n49_), .O(z03));
  xor2a  g24(.a(new_n56_), .b(x08), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n49_), .c(new_n53_), .O(z04));
  inv1   g26(.a(x08), .O(new_n61_));
  oai22  g27(.a(new_n56_), .b(new_n61_), .c(new_n38_), .d(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(new_n50_), .O(z05));
  inv1   g31(.a(x10), .O(new_n66_));
  nand3  g32(.a(new_n63_), .b(x08), .c(x07), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  nand4  g34(.a(new_n66_), .b(new_n63_), .c(x08), .d(x07), .O(new_n69_));
  oai21  g35(.a(new_n68_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  nor2   g37(.a(new_n66_), .b(x06), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n49_), .c(new_n53_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(z06));
  nand2  g40(.a(x08), .b(x07), .O(new_n75_));
  nor2   g41(.a(new_n75_), .b(x09), .O(new_n76_));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  aoi22  g43(.a(new_n77_), .b(new_n76_), .c(new_n69_), .d(x11), .O(new_n78_));
  nand2  g44(.a(x11), .b(new_n47_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n48_), .c(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n53_), .O(new_n81_));
  oai21  g47(.a(new_n78_), .b(new_n47_), .c(new_n81_), .O(z07));
  nand4  g48(.a(new_n77_), .b(new_n63_), .c(x08), .d(x07), .O(new_n83_));
  nor3   g49(.a(x12), .b(x11), .c(x10), .O(new_n84_));
  aoi22  g50(.a(new_n84_), .b(new_n68_), .c(new_n83_), .d(x12), .O(new_n85_));
  nand2  g51(.a(x12), .b(new_n47_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n48_), .c(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n53_), .O(new_n88_));
  oai21  g54(.a(new_n85_), .b(new_n47_), .c(new_n88_), .O(z08));
  nand2  g55(.a(new_n49_), .b(new_n53_), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n66_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n67_), .c(new_n37_), .O(new_n94_));
  nand3  g60(.a(new_n84_), .b(new_n68_), .c(x13), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n94_), .c(x06), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n90_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  oai21  g64(.a(new_n93_), .b(x13), .c(new_n98_), .O(new_n99_));
  inv1   g65(.a(x03), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x02), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x16), .c(new_n47_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n99_), .c(new_n76_), .O(new_n103_));
  nand4  g69(.a(new_n101_), .b(new_n53_), .c(x16), .d(x14), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n103_), .c(new_n44_), .O(z10));
  nor2   g71(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n36_), .c(x03), .O(new_n107_));
  nor3   g73(.a(new_n107_), .b(new_n38_), .c(new_n44_), .O(z12));
  nand2  g74(.a(new_n53_), .b(new_n44_), .O(z13));
  inv1   g75(.a(x17), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(new_n44_), .c(new_n53_), .O(z14));
endmodule


