// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n103_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x12), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x12), .c(x06), .O(z02));
  xnor2a g14(.a(x07), .b(x06), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(new_n47_), .c(new_n38_), .O(z03));
  inv1   g16(.a(x03), .O(new_n51_));
  inv1   g17(.a(x16), .O(new_n52_));
  aoi21  g18(.a(new_n51_), .b(x02), .c(new_n52_), .O(new_n53_));
  nor2   g19(.a(new_n53_), .b(new_n43_), .O(new_n54_));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  oai21  g22(.a(new_n37_), .b(new_n55_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  oai21  g26(.a(new_n37_), .b(x09), .c(new_n58_), .O(new_n61_));
  nor2   g27(.a(new_n58_), .b(x09), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n54_), .O(z05));
  inv1   g30(.a(x10), .O(new_n65_));
  nand2  g31(.a(new_n62_), .b(new_n65_), .O(new_n66_));
  oai21  g32(.a(new_n58_), .b(x09), .c(x10), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n46_), .c(x04), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n66_), .c(new_n37_), .O(z06));
  inv1   g36(.a(x06), .O(new_n71_));
  nand2  g37(.a(x08), .b(x07), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(x09), .O(new_n73_));
  inv1   g39(.a(x09), .O(new_n74_));
  nand4  g40(.a(new_n65_), .b(new_n74_), .c(x08), .d(x07), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  aoi22  g42(.a(new_n76_), .b(new_n73_), .c(new_n75_), .d(x11), .O(new_n77_));
  nand2  g43(.a(x11), .b(new_n71_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n46_), .c(x04), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n38_), .O(new_n80_));
  oai21  g46(.a(new_n77_), .b(new_n71_), .c(new_n80_), .O(z07));
  nand2  g47(.a(new_n76_), .b(new_n73_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x12), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nand3  g50(.a(new_n76_), .b(new_n73_), .c(new_n84_), .O(new_n85_));
  nor2   g51(.a(new_n71_), .b(new_n43_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n83_), .d(new_n46_), .O(z08));
  nand2  g53(.a(new_n85_), .b(x13), .O(new_n88_));
  inv1   g54(.a(x13), .O(new_n89_));
  nand3  g55(.a(new_n76_), .b(new_n89_), .c(new_n84_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n73_), .O(new_n92_));
  oai21  g58(.a(x13), .b(new_n84_), .c(new_n71_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n92_), .c(new_n88_), .d(new_n54_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n62_), .c(new_n46_), .O(new_n97_));
  nand3  g63(.a(new_n53_), .b(new_n38_), .c(x14), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n97_), .c(new_n43_), .O(z10));
  nand2  g65(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n35_), .c(x03), .O(new_n101_));
  nor2   g67(.a(new_n37_), .b(new_n43_), .O(z13));
  inv1   g68(.a(z13), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n101_), .O(z12));
  and2   g70(.a(z13), .b(x17), .O(z14));
endmodule


