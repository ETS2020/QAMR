// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n103_,
    new_n105_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x07), .O(new_n37_));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g05(.a(x14), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  oai21  g06(.a(new_n36_), .b(new_n35_), .c(new_n40_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  oai21  g08(.a(x15), .b(x05), .c(new_n37_), .O(new_n43_));
  aoi22  g09(.a(new_n43_), .b(x18), .c(x05), .d(x04), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  nor2   g12(.a(new_n39_), .b(new_n46_), .O(z13));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand3  g14(.a(new_n48_), .b(z13), .c(new_n45_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  inv1   g16(.a(new_n39_), .O(new_n51_));
  inv1   g17(.a(x03), .O(new_n52_));
  inv1   g18(.a(x16), .O(new_n53_));
  aoi21  g19(.a(new_n52_), .b(x02), .c(new_n53_), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand2  g21(.a(new_n37_), .b(new_n45_), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(new_n56_), .c(new_n55_), .d(new_n51_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  inv1   g25(.a(new_n55_), .O(new_n60_));
  xor2a  g26(.a(new_n57_), .b(x08), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n60_), .c(new_n51_), .O(z04));
  aoi21  g28(.a(x08), .b(x06), .c(x18), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n37_), .c(x09), .O(new_n64_));
  nand2  g30(.a(new_n60_), .b(new_n51_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n38_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n65_), .c(new_n64_), .O(z05));
  nand3  g36(.a(new_n66_), .b(x08), .c(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n38_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x10), .O(new_n74_));
  inv1   g40(.a(x10), .O(new_n75_));
  nand3  g41(.a(new_n68_), .b(new_n38_), .c(new_n75_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n65_), .O(z06));
  nand2  g43(.a(new_n76_), .b(x11), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n66_), .c(x08), .d(x06), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n38_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x07), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n78_), .c(new_n55_), .O(z07));
  nand2  g49(.a(new_n82_), .b(x12), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  nand4  g51(.a(new_n79_), .b(new_n68_), .c(new_n38_), .d(new_n85_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n65_), .O(z08));
  inv1   g53(.a(x13), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n80_), .c(new_n38_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x07), .O(new_n91_));
  nand2  g57(.a(new_n86_), .b(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n55_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nand3  g60(.a(new_n79_), .b(new_n88_), .c(new_n85_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor2   g62(.a(new_n67_), .b(new_n54_), .O(new_n97_));
  inv1   g63(.a(x14), .O(new_n98_));
  nor2   g64(.a(new_n48_), .b(new_n98_), .O(new_n99_));
  aoi21  g65(.a(new_n97_), .b(new_n96_), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n46_), .c(new_n51_), .O(z10));
  nor2   g67(.a(new_n39_), .b(new_n36_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n36_), .c(x03), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n46_), .c(new_n51_), .O(z12));
  nand2  g70(.a(z13), .b(x17), .O(new_n105_));
  inv1   g71(.a(new_n105_), .O(z14));
endmodule


