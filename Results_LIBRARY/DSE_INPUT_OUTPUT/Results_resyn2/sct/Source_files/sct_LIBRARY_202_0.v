// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_, new_n104_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  aoi21  g03(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x04), .O(new_n40_));
  inv1   g06(.a(x06), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x05), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(x04), .c(x06), .O(z02));
  nor2   g15(.a(new_n48_), .b(new_n40_), .O(new_n50_));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n41_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(new_n37_), .O(new_n56_));
  inv1   g22(.a(x02), .O(new_n57_));
  oai21  g23(.a(x03), .b(new_n57_), .c(x16), .O(new_n58_));
  nand2  g24(.a(x06), .b(new_n40_), .O(z13));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n51_), .b(new_n61_), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n60_), .c(z13), .d(new_n58_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n56_), .O(z04));
  nand2  g30(.a(new_n60_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n50_), .O(z05));
  inv1   g34(.a(new_n60_), .O(new_n69_));
  nor2   g35(.a(x10), .b(x09), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g37(.a(new_n67_), .b(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n50_), .O(z06));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n66_), .c(x08), .d(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  aoi21  g43(.a(new_n71_), .b(x11), .c(new_n48_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n40_), .c(new_n77_), .O(z07));
  oai21  g45(.a(new_n75_), .b(x12), .c(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nor3   g48(.a(x11), .b(x10), .c(x09), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n69_), .c(new_n82_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n48_), .c(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n81_), .O(z08));
  nand3  g52(.a(new_n66_), .b(x08), .c(x07), .O(new_n87_));
  nor2   g53(.a(x13), .b(x12), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n74_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n87_), .c(x04), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  nand4  g57(.a(new_n74_), .b(new_n69_), .c(new_n82_), .d(new_n66_), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(x13), .c(new_n48_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n40_), .c(new_n91_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n89_), .b(new_n95_), .O(new_n96_));
  nor2   g62(.a(new_n67_), .b(new_n48_), .O(new_n97_));
  nor2   g63(.a(new_n58_), .b(new_n36_), .O(new_n98_));
  aoi21  g64(.a(new_n97_), .b(new_n96_), .c(new_n98_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n40_), .c(new_n56_), .O(z10));
  nor2   g66(.a(new_n37_), .b(new_n57_), .O(z11));
  nand2  g67(.a(x16), .b(new_n57_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n46_), .c(new_n40_), .O(z12));
  nor2   g69(.a(x17), .b(new_n40_), .O(new_n104_));
  aoi21  g70(.a(x06), .b(new_n40_), .c(new_n104_), .O(z14));
endmodule


