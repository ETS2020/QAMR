// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n93_, new_n94_, new_n97_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x13), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x15), .c(new_n42_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n44_), .c(new_n37_), .O(z01));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(x06), .c(new_n37_), .O(z02));
  xnor2a g17(.a(x07), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n50_), .c(new_n37_), .O(z03));
  aoi21  g19(.a(x07), .b(x06), .c(x08), .O(new_n54_));
  nand3  g20(.a(x08), .b(x07), .c(x06), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(new_n56_));
  nor3   g22(.a(new_n56_), .b(new_n54_), .c(new_n50_), .O(z04));
  oai21  g23(.a(new_n55_), .b(new_n36_), .c(x09), .O(new_n58_));
  nand2  g24(.a(x08), .b(x07), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(x09), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n36_), .c(x06), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(new_n58_), .c(new_n49_), .d(x04), .O(z05));
  inv1   g28(.a(x10), .O(new_n63_));
  nor3   g29(.a(new_n55_), .b(new_n36_), .c(x09), .O(new_n64_));
  nor2   g30(.a(x10), .b(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x08), .c(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x13), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(x06), .c(new_n50_), .O(new_n68_));
  oai21  g34(.a(new_n64_), .b(new_n63_), .c(new_n68_), .O(z06));
  nand2  g35(.a(new_n67_), .b(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x11), .O(new_n71_));
  nand2  g37(.a(new_n50_), .b(new_n37_), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  nand3  g39(.a(new_n64_), .b(new_n73_), .c(new_n63_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n72_), .c(new_n71_), .O(z07));
  nand4  g41(.a(new_n65_), .b(new_n73_), .c(x08), .d(x07), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x13), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x06), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x12), .O(new_n79_));
  nor3   g45(.a(x12), .b(x11), .c(x10), .O(new_n80_));
  aoi22  g46(.a(new_n80_), .b(new_n64_), .c(new_n50_), .d(new_n37_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n79_), .O(z08));
  nand3  g48(.a(new_n80_), .b(new_n60_), .c(x06), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x13), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n72_), .O(z09));
  nand3  g51(.a(x13), .b(x06), .c(x00), .O(new_n86_));
  inv1   g52(.a(new_n86_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n60_), .c(new_n49_), .O(new_n88_));
  inv1   g54(.a(new_n49_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n37_), .c(x14), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(new_n88_), .c(new_n45_), .O(z10));
  nand2  g57(.a(new_n37_), .b(new_n48_), .O(z11));
  aoi21  g58(.a(x16), .b(new_n48_), .c(x03), .O(new_n93_));
  nand2  g59(.a(new_n37_), .b(x04), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n93_), .O(z12));
  nand2  g61(.a(new_n37_), .b(new_n45_), .O(z13));
  inv1   g62(.a(x17), .O(new_n97_));
  nor2   g63(.a(new_n94_), .b(new_n97_), .O(z14));
endmodule


