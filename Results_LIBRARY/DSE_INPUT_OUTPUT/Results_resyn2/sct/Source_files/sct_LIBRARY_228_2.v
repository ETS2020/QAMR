// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n104_, new_n107_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x15), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x06), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x05), .c(new_n42_), .O(new_n44_));
  aoi22  g10(.a(new_n44_), .b(new_n36_), .c(x05), .d(x04), .O(z01));
  inv1   g11(.a(x02), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n46_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  oai21  g18(.a(new_n36_), .b(new_n42_), .c(new_n52_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(new_n49_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n54_), .b(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n37_), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n48_), .O(z04));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(x09), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n36_), .c(x06), .O(new_n64_));
  nand4  g30(.a(x15), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n49_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nand2  g34(.a(new_n63_), .b(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x15), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  oai21  g37(.a(new_n65_), .b(x09), .c(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n49_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  nand3  g40(.a(new_n63_), .b(new_n74_), .c(new_n68_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x15), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nor2   g43(.a(new_n65_), .b(x09), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n68_), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(x11), .c(new_n48_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n77_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand4  g48(.a(new_n63_), .b(new_n82_), .c(new_n74_), .d(new_n68_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x15), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x06), .O(new_n85_));
  nand3  g51(.a(new_n78_), .b(new_n74_), .c(new_n68_), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(x12), .c(new_n48_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n85_), .O(z08));
  inv1   g54(.a(x13), .O(new_n89_));
  aoi21  g55(.a(new_n83_), .b(x15), .c(new_n42_), .O(new_n90_));
  nand4  g56(.a(new_n89_), .b(new_n82_), .c(new_n74_), .d(new_n68_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  aoi22  g58(.a(new_n92_), .b(new_n78_), .c(new_n48_), .d(new_n37_), .O(new_n93_));
  oai21  g59(.a(new_n90_), .b(new_n89_), .c(new_n93_), .O(z09));
  inv1   g60(.a(x04), .O(new_n95_));
  inv1   g61(.a(new_n47_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n37_), .c(x14), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n91_), .b(new_n98_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n78_), .c(new_n47_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n97_), .c(new_n95_), .O(z10));
  aoi21  g67(.a(new_n36_), .b(x06), .c(new_n46_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n46_), .c(x03), .O(new_n103_));
  nand2  g69(.a(new_n37_), .b(x04), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n103_), .O(z12));
  inv1   g71(.a(new_n104_), .O(z13));
  inv1   g72(.a(x17), .O(new_n107_));
  nor2   g73(.a(new_n104_), .b(new_n107_), .O(z14));
endmodule


