// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n95_,
    new_n96_, new_n98_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  nor2   g03(.a(x06), .b(new_n37_), .O(z02));
  aoi21  g04(.a(x14), .b(new_n36_), .c(z02), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x15), .O(new_n42_));
  nand2  g08(.a(x18), .b(new_n42_), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(z02), .O(new_n44_));
  nand2  g10(.a(x06), .b(x04), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x05), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x07), .O(new_n48_));
  inv1   g14(.a(x02), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n49_), .c(x16), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(x06), .c(new_n37_), .O(z03));
  inv1   g18(.a(x03), .O(new_n53_));
  inv1   g19(.a(x16), .O(new_n54_));
  aoi21  g20(.a(new_n53_), .b(x02), .c(new_n54_), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n45_), .O(new_n56_));
  nand2  g22(.a(x08), .b(x07), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n48_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand2  g27(.a(new_n57_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x07), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(new_n50_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x04), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nor2   g34(.a(new_n57_), .b(x09), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g36(.a(new_n64_), .b(x10), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n56_), .O(z06));
  nand2  g38(.a(new_n70_), .b(x11), .O(new_n73_));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n56_), .O(z07));
  inv1   g42(.a(x12), .O(new_n77_));
  nand3  g43(.a(new_n74_), .b(new_n69_), .c(new_n77_), .O(new_n78_));
  nand2  g44(.a(new_n75_), .b(x12), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n56_), .O(z08));
  nand2  g46(.a(new_n78_), .b(x13), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  inv1   g48(.a(x13), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n77_), .c(new_n82_), .d(new_n68_), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n81_), .c(new_n56_), .O(z09));
  inv1   g53(.a(x00), .O(new_n88_));
  nand2  g54(.a(new_n84_), .b(new_n88_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n69_), .c(new_n50_), .O(new_n90_));
  inv1   g56(.a(x06), .O(new_n91_));
  aoi21  g57(.a(new_n55_), .b(x14), .c(new_n91_), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n90_), .c(new_n37_), .O(z10));
  nor2   g59(.a(z02), .b(new_n49_), .O(z11));
  nand2  g60(.a(x16), .b(new_n49_), .O(new_n95_));
  nor2   g61(.a(new_n91_), .b(x03), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n95_), .c(new_n37_), .O(z12));
  inv1   g63(.a(x17), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(x06), .c(new_n37_), .O(z14));
  buf    g65(.a(x04), .O(z13));
endmodule


