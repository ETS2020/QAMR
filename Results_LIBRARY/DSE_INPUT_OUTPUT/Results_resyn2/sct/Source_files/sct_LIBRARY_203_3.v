// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n109_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x05), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g05(.a(x14), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  oai21  g06(.a(new_n36_), .b(new_n35_), .c(new_n40_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nor2   g10(.a(new_n37_), .b(new_n44_), .O(new_n45_));
  aoi22  g11(.a(new_n45_), .b(new_n38_), .c(new_n43_), .d(x18), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n38_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  oai21  g20(.a(new_n38_), .b(x05), .c(new_n54_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n53_), .c(new_n50_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(new_n39_), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n50_), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n60_), .O(z04));
  oai21  g30(.a(new_n59_), .b(x05), .c(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand2  g32(.a(new_n60_), .b(new_n66_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n50_), .O(z05));
  inv1   g34(.a(x10), .O(new_n69_));
  oai21  g35(.a(x03), .b(new_n36_), .c(x16), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(x04), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n38_), .c(new_n37_), .O(new_n72_));
  oai21  g38(.a(new_n59_), .b(x09), .c(x10), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(x08), .c(x07), .d(x06), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n73_), .c(new_n70_), .d(x04), .O(new_n76_));
  inv1   g42(.a(new_n76_), .O(new_n77_));
  nor2   g43(.a(new_n77_), .b(new_n72_), .O(z06));
  inv1   g44(.a(new_n59_), .O(new_n79_));
  nand3  g45(.a(new_n74_), .b(new_n79_), .c(new_n37_), .O(new_n80_));
  oai21  g46(.a(new_n39_), .b(x11), .c(new_n80_), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  nand3  g48(.a(new_n74_), .b(new_n79_), .c(new_n82_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n50_), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(new_n83_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g53(.a(new_n83_), .b(x12), .O(new_n88_));
  oai21  g54(.a(x12), .b(x06), .c(x05), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n50_), .O(z08));
  oai21  g56(.a(new_n83_), .b(x12), .c(x13), .O(new_n91_));
  nor2   g57(.a(new_n59_), .b(x09), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n85_), .c(new_n82_), .d(new_n69_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  oai21  g62(.a(x13), .b(x06), .c(x05), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n96_), .c(new_n91_), .d(new_n50_), .O(z09));
  nand2  g64(.a(new_n58_), .b(x04), .O(new_n99_));
  nand2  g65(.a(new_n49_), .b(x14), .O(new_n100_));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n94_), .b(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n92_), .c(new_n70_), .d(new_n37_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n100_), .c(new_n99_), .O(z10));
  nand2  g70(.a(new_n58_), .b(new_n36_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n36_), .c(x03), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n99_), .O(z12));
  inv1   g73(.a(new_n99_), .O(z13));
  nand2  g74(.a(x17), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n58_), .O(z14));
endmodule


