// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n106_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(x13), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  inv1   g20(.a(x07), .O(new_n55_));
  inv1   g21(.a(x13), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n36_), .c(new_n55_), .O(new_n57_));
  nand2  g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n57_), .c(new_n52_), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z03));
  inv1   g26(.a(x02), .O(new_n61_));
  oai21  g27(.a(x03), .b(new_n61_), .c(x16), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x04), .O(new_n63_));
  xor2a  g29(.a(new_n58_), .b(x08), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n63_), .c(new_n38_), .O(z04));
  nand2  g31(.a(x08), .b(x07), .O(new_n66_));
  nor2   g32(.a(new_n66_), .b(x09), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n56_), .c(x06), .O(new_n68_));
  nand4  g34(.a(x13), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n52_), .O(z05));
  and2   g37(.a(x08), .b(x07), .O(new_n72_));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x13), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  oai21  g42(.a(new_n69_), .b(x09), .c(x10), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n52_), .O(z06));
  aoi21  g44(.a(new_n73_), .b(new_n72_), .c(new_n56_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n36_), .c(x11), .O(new_n80_));
  inv1   g46(.a(x09), .O(new_n81_));
  inv1   g47(.a(x10), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n69_), .O(new_n85_));
  aoi21  g51(.a(new_n63_), .b(new_n38_), .c(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n80_), .O(z07));
  inv1   g53(.a(x12), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(x08), .c(x07), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n84_), .c(x13), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  oai21  g57(.a(new_n84_), .b(new_n69_), .c(x12), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n52_), .O(z08));
  nand2  g59(.a(new_n63_), .b(new_n38_), .O(new_n94_));
  nand4  g60(.a(new_n73_), .b(new_n72_), .c(new_n88_), .d(new_n83_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n36_), .c(x13), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n94_), .O(z09));
  nor2   g63(.a(new_n56_), .b(new_n36_), .O(new_n98_));
  nand4  g64(.a(new_n67_), .b(new_n98_), .c(new_n62_), .d(x00), .O(new_n99_));
  nand3  g65(.a(new_n51_), .b(new_n38_), .c(x14), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n99_), .c(new_n48_), .O(z10));
  nor2   g67(.a(new_n37_), .b(new_n61_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n61_), .c(x03), .O(new_n103_));
  nor3   g69(.a(new_n103_), .b(new_n37_), .c(new_n48_), .O(z12));
  nor2   g70(.a(new_n37_), .b(new_n48_), .O(z13));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n38_), .O(z14));
endmodule


