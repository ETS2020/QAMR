// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x04), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x04), .O(new_n40_));
  oai21  g06(.a(x06), .b(x05), .c(new_n40_), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g10(.a(x03), .O(new_n45_));
  inv1   g11(.a(x16), .O(new_n46_));
  aoi21  g12(.a(new_n45_), .b(x02), .c(new_n46_), .O(new_n47_));
  nor2   g13(.a(new_n47_), .b(new_n40_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n36_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n51_), .c(new_n48_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  nand3  g21(.a(x08), .b(x07), .c(x06), .O(new_n56_));
  inv1   g22(.a(x08), .O(new_n57_));
  nand2  g23(.a(new_n51_), .b(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(new_n48_), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  oai21  g26(.a(x09), .b(new_n40_), .c(new_n56_), .O(new_n61_));
  inv1   g27(.a(new_n51_), .O(new_n62_));
  nor2   g28(.a(x09), .b(new_n57_), .O(new_n63_));
  aoi21  g29(.a(new_n63_), .b(new_n62_), .c(new_n47_), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(new_n61_), .c(new_n37_), .O(z05));
  inv1   g31(.a(new_n37_), .O(new_n66_));
  inv1   g32(.a(new_n47_), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x08), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x04), .O(new_n71_));
  oai21  g37(.a(new_n69_), .b(new_n51_), .c(new_n71_), .O(new_n72_));
  nand3  g38(.a(new_n63_), .b(new_n62_), .c(new_n70_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n67_), .O(new_n74_));
  and2   g40(.a(new_n74_), .b(new_n66_), .O(z06));
  nand2  g41(.a(new_n73_), .b(x11), .O(new_n76_));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n63_), .c(x07), .d(x06), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n76_), .c(new_n48_), .O(z07));
  oai21  g45(.a(x12), .b(new_n40_), .c(new_n36_), .O(new_n80_));
  nand3  g46(.a(new_n68_), .b(x08), .c(x07), .O(new_n81_));
  inv1   g47(.a(x11), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n70_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n81_), .c(x12), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(new_n56_), .O(new_n86_));
  nand4  g52(.a(new_n77_), .b(new_n86_), .c(new_n85_), .d(new_n68_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n84_), .c(new_n67_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x04), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n80_), .O(z08));
  oai21  g56(.a(x13), .b(new_n40_), .c(new_n36_), .O(new_n91_));
  nand3  g57(.a(new_n85_), .b(new_n82_), .c(new_n70_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n81_), .c(x13), .O(new_n93_));
  nor2   g59(.a(x13), .b(x12), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n77_), .c(new_n63_), .d(new_n62_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n93_), .c(new_n67_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x04), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n91_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n94_), .b(new_n77_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g67(.a(new_n45_), .b(x02), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(x16), .c(new_n81_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n101_), .c(new_n40_), .O(new_n104_));
  nand3  g70(.a(new_n47_), .b(x14), .c(x04), .O(new_n105_));
  oai21  g71(.a(new_n104_), .b(new_n36_), .c(new_n105_), .O(z10));
  nand2  g72(.a(new_n66_), .b(new_n35_), .O(z11));
  nand2  g73(.a(x16), .b(new_n35_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n45_), .c(new_n40_), .O(z12));
  nand2  g75(.a(new_n36_), .b(new_n40_), .O(z13));
  and2   g76(.a(x17), .b(x04), .O(z14));
endmodule


