// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  oai21  g08(.a(new_n37_), .b(x05), .c(new_n36_), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n36_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  aoi21  g18(.a(x06), .b(new_n36_), .c(new_n49_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n38_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n54_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n60_), .c(new_n53_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n38_), .O(z04));
  nand2  g29(.a(x08), .b(x07), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(x09), .c(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  aoi21  g32(.a(new_n61_), .b(x09), .c(new_n49_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n36_), .c(new_n66_), .O(z05));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(x08), .c(x07), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x04), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x06), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  inv1   g39(.a(x02), .O(new_n74_));
  oai21  g40(.a(x03), .b(new_n74_), .c(x16), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  inv1   g42(.a(x09), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(x08), .c(x07), .d(x06), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(new_n49_), .O(new_n79_));
  inv1   g45(.a(new_n79_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n76_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n72_), .O(z06));
  oai21  g48(.a(new_n70_), .b(x11), .c(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  inv1   g51(.a(new_n61_), .O(new_n86_));
  aoi21  g52(.a(new_n69_), .b(new_n86_), .c(new_n85_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n49_), .c(x04), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n84_), .O(z07));
  inv1   g55(.a(x12), .O(new_n90_));
  nand2  g56(.a(new_n85_), .b(new_n73_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n78_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  oai21  g59(.a(new_n91_), .b(new_n78_), .c(x12), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n50_), .O(z08));
  nand2  g61(.a(new_n93_), .b(x13), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand3  g63(.a(new_n92_), .b(new_n97_), .c(new_n90_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n96_), .c(new_n50_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n97_), .b(new_n90_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n91_), .c(new_n100_), .O(new_n102_));
  inv1   g68(.a(x14), .O(new_n103_));
  nor2   g69(.a(new_n75_), .b(new_n103_), .O(new_n104_));
  aoi21  g70(.a(new_n102_), .b(new_n79_), .c(new_n104_), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n36_), .c(new_n38_), .O(z10));
  nand2  g72(.a(new_n38_), .b(new_n74_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n74_), .c(x03), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n36_), .c(new_n38_), .O(z12));
  nand2  g75(.a(x06), .b(new_n36_), .O(z13));
  and2   g76(.a(x17), .b(x04), .O(z14));
endmodule


