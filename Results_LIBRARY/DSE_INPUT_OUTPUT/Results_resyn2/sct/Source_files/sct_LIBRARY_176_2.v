// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n109_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x07), .b(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(new_n38_), .O(new_n42_));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n42_), .O(z02));
  inv1   g16(.a(x04), .O(new_n51_));
  inv1   g17(.a(x03), .O(new_n52_));
  inv1   g18(.a(x16), .O(new_n53_));
  aoi21  g19(.a(new_n52_), .b(x02), .c(new_n53_), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(x07), .c(new_n37_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  xnor2a g23(.a(x08), .b(x06), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n49_), .c(new_n42_), .O(z04));
  nand2  g25(.a(x08), .b(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x08), .c(x06), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n55_), .O(new_n64_));
  and2   g30(.a(new_n64_), .b(new_n42_), .O(z05));
  nand3  g31(.a(x10), .b(new_n62_), .c(x08), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n49_), .c(x07), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  nand4  g34(.a(new_n63_), .b(new_n48_), .c(new_n68_), .d(x04), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  aoi21  g36(.a(new_n67_), .b(x06), .c(new_n70_), .O(z06));
  nand3  g37(.a(new_n68_), .b(new_n62_), .c(x08), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x11), .O(new_n73_));
  inv1   g39(.a(new_n60_), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n62_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nand2  g43(.a(x11), .b(new_n37_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n48_), .c(x04), .O(new_n79_));
  aoi21  g45(.a(new_n77_), .b(x07), .c(new_n79_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n38_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand4  g48(.a(new_n75_), .b(new_n82_), .c(new_n62_), .d(x08), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x07), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x06), .O(new_n85_));
  inv1   g51(.a(x07), .O(new_n86_));
  oai21  g52(.a(new_n76_), .b(new_n86_), .c(x12), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n85_), .c(new_n55_), .O(z08));
  oai21  g54(.a(new_n83_), .b(x13), .c(x07), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x06), .O(new_n90_));
  nand3  g56(.a(new_n75_), .b(new_n82_), .c(new_n62_), .O(new_n91_));
  nand2  g57(.a(new_n74_), .b(x07), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n91_), .c(x13), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n90_), .c(new_n55_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nor2   g61(.a(x13), .b(x12), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n75_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nor2   g64(.a(new_n63_), .b(new_n54_), .O(new_n99_));
  inv1   g65(.a(x14), .O(new_n100_));
  nor2   g66(.a(new_n48_), .b(new_n100_), .O(new_n101_));
  aoi21  g67(.a(new_n99_), .b(new_n98_), .c(new_n101_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n51_), .c(new_n42_), .O(z10));
  nor2   g69(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n36_), .c(x03), .O(new_n105_));
  nand2  g71(.a(new_n42_), .b(x04), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n105_), .O(z12));
  inv1   g73(.a(new_n106_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  nor2   g75(.a(new_n106_), .b(new_n109_), .O(z14));
endmodule


