// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n100_, new_n102_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(x16), .b(x13), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(new_n37_), .O(new_n44_));
  aoi21  g10(.a(new_n42_), .b(x18), .c(new_n44_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  inv1   g14(.a(x06), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n48_), .c(new_n37_), .O(z02));
  inv1   g17(.a(x04), .O(new_n52_));
  inv1   g18(.a(x13), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n46_), .c(x02), .O(new_n54_));
  aoi21  g20(.a(new_n54_), .b(x16), .c(new_n52_), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(x07), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(new_n55_), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z03));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n55_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  xor2a  g31(.a(new_n61_), .b(x09), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n55_), .O(z05));
  oai21  g33(.a(new_n61_), .b(x09), .c(x10), .O(new_n68_));
  nand2  g34(.a(new_n48_), .b(new_n53_), .O(new_n69_));
  inv1   g35(.a(new_n61_), .O(new_n70_));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n69_), .c(new_n68_), .d(x04), .O(new_n73_));
  and2   g39(.a(new_n73_), .b(new_n37_), .O(z06));
  nand2  g40(.a(new_n72_), .b(x11), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  nand3  g42(.a(new_n71_), .b(new_n70_), .c(new_n76_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n69_), .c(x04), .O(new_n78_));
  inv1   g44(.a(new_n78_), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n75_), .c(new_n38_), .O(z07));
  nand2  g46(.a(new_n77_), .b(x12), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nand4  g48(.a(new_n71_), .b(new_n70_), .c(new_n82_), .d(new_n76_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n55_), .O(z08));
  inv1   g50(.a(new_n83_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n48_), .c(new_n53_), .O(new_n86_));
  nand2  g52(.a(new_n37_), .b(new_n52_), .O(z13));
  nand3  g53(.a(new_n83_), .b(new_n47_), .c(x13), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(z13), .c(new_n86_), .O(z09));
  nor2   g55(.a(new_n61_), .b(x09), .O(new_n90_));
  inv1   g56(.a(x00), .O(new_n91_));
  aoi21  g57(.a(new_n54_), .b(x16), .c(new_n91_), .O(new_n92_));
  inv1   g58(.a(x10), .O(new_n93_));
  nand4  g59(.a(new_n53_), .b(new_n82_), .c(new_n76_), .d(new_n93_), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n48_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n92_), .c(new_n90_), .O(new_n96_));
  nand3  g62(.a(new_n48_), .b(x14), .c(new_n53_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n96_), .c(new_n52_), .O(z10));
  nand2  g64(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n36_), .c(x03), .O(new_n100_));
  nor3   g66(.a(new_n100_), .b(new_n38_), .c(new_n52_), .O(z12));
  inv1   g67(.a(x17), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n52_), .c(new_n37_), .O(z14));
endmodule


