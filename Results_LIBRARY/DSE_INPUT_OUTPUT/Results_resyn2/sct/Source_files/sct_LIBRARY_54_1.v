// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n104_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x07), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(new_n38_), .O(new_n42_));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(new_n37_), .c(x06), .O(z02));
  nand2  g16(.a(new_n37_), .b(x06), .O(new_n51_));
  aoi22  g17(.a(new_n51_), .b(new_n42_), .c(new_n49_), .d(new_n37_), .O(z03));
  inv1   g18(.a(new_n49_), .O(new_n53_));
  nand2  g19(.a(x08), .b(x07), .O(new_n54_));
  inv1   g20(.a(x06), .O(new_n55_));
  inv1   g21(.a(x08), .O(new_n56_));
  oai21  g22(.a(new_n37_), .b(new_n55_), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n54_), .c(new_n53_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  xor2a  g25(.a(new_n54_), .b(x09), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(new_n53_), .c(new_n38_), .O(z05));
  inv1   g27(.a(x09), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x08), .c(x07), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x10), .O(new_n64_));
  nor2   g30(.a(x10), .b(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(x08), .c(x07), .O(new_n66_));
  oai21  g32(.a(x10), .b(x07), .c(new_n55_), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n66_), .c(new_n64_), .d(new_n53_), .O(z06));
  inv1   g34(.a(x11), .O(new_n69_));
  aoi21  g35(.a(new_n65_), .b(x08), .c(new_n69_), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(new_n69_), .b(new_n71_), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n63_), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(new_n70_), .c(x06), .O(new_n74_));
  nand2  g40(.a(x11), .b(new_n37_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n48_), .c(x04), .O(new_n76_));
  inv1   g42(.a(new_n76_), .O(new_n77_));
  aoi21  g43(.a(new_n77_), .b(new_n74_), .c(new_n38_), .O(z07));
  inv1   g44(.a(x12), .O(new_n79_));
  nand2  g45(.a(new_n42_), .b(new_n79_), .O(new_n80_));
  nand2  g46(.a(new_n73_), .b(x06), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g48(.a(new_n54_), .b(x09), .O(new_n83_));
  nor2   g49(.a(x11), .b(x10), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n79_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n82_), .c(new_n53_), .O(z08));
  xor2a  g52(.a(new_n85_), .b(x13), .O(new_n87_));
  oai21  g53(.a(x13), .b(x07), .c(new_n55_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n53_), .O(z09));
  inv1   g55(.a(x04), .O(new_n90_));
  inv1   g56(.a(x00), .O(new_n91_));
  nor2   g57(.a(x13), .b(x12), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n84_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n83_), .c(new_n48_), .d(x06), .O(new_n95_));
  inv1   g61(.a(x03), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x02), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n42_), .c(x16), .d(x14), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n95_), .c(new_n90_), .O(z10));
  nand2  g65(.a(new_n42_), .b(new_n36_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n36_), .c(x03), .O(new_n101_));
  nor3   g67(.a(new_n101_), .b(new_n38_), .c(new_n90_), .O(z12));
  nand2  g68(.a(new_n42_), .b(new_n90_), .O(z13));
  inv1   g69(.a(x17), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n90_), .c(new_n42_), .O(z14));
endmodule


