// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n98_, new_n100_, new_n102_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x04), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x04), .O(new_n42_));
  oai21  g08(.a(x06), .b(x05), .c(new_n42_), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n42_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n39_), .O(z02));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n42_), .c(new_n52_), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n58_), .c(new_n47_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n39_), .O(z04));
  oai21  g27(.a(x09), .b(new_n42_), .c(new_n59_), .O(new_n62_));
  nor2   g28(.a(new_n59_), .b(x09), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n48_), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(new_n62_), .c(new_n38_), .O(z05));
  oai21  g31(.a(new_n59_), .b(x09), .c(x10), .O(new_n66_));
  nor2   g32(.a(x10), .b(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n49_), .O(z06));
  nor2   g35(.a(new_n57_), .b(new_n53_), .O(new_n70_));
  nand4  g36(.a(new_n67_), .b(new_n70_), .c(new_n47_), .d(x11), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x04), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  nand4  g39(.a(new_n68_), .b(new_n47_), .c(new_n73_), .d(x04), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n72_), .b(x06), .c(new_n75_), .O(z07));
  oai21  g42(.a(x12), .b(new_n42_), .c(new_n37_), .O(new_n77_));
  nand4  g43(.a(new_n67_), .b(new_n73_), .c(x08), .d(x07), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x12), .O(new_n79_));
  nor2   g45(.a(x12), .b(x11), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n67_), .c(new_n70_), .d(x06), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n79_), .c(new_n47_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n77_), .O(z08));
  inv1   g50(.a(x10), .O(new_n85_));
  inv1   g51(.a(x13), .O(new_n86_));
  nand3  g52(.a(new_n80_), .b(new_n86_), .c(new_n85_), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n63_), .O(new_n89_));
  nand2  g55(.a(new_n81_), .b(x13), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n49_), .O(z09));
  nand2  g57(.a(new_n48_), .b(x14), .O(new_n92_));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n87_), .b(new_n93_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n63_), .c(new_n47_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n92_), .c(new_n42_), .O(z10));
  nor2   g62(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g63(.a(x16), .b(new_n35_), .c(x03), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n42_), .O(z12));
  nor2   g65(.a(x06), .b(x04), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(z13));
  inv1   g67(.a(x17), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(x04), .c(new_n100_), .O(z14));
endmodule


