// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n108_, new_n110_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(x06), .b(x05), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  inv1   g09(.a(x05), .O(new_n44_));
  inv1   g10(.a(x06), .O(new_n45_));
  aoi21  g11(.a(new_n45_), .b(new_n43_), .c(new_n44_), .O(new_n46_));
  aoi21  g12(.a(new_n42_), .b(x18), .c(new_n46_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n45_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  oai21  g21(.a(new_n45_), .b(x05), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  oai21  g24(.a(x03), .b(new_n36_), .c(x16), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x04), .O(new_n60_));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n37_), .O(new_n62_));
  aoi21  g28(.a(x07), .b(x06), .c(x08), .O(new_n63_));
  nor3   g29(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(z04));
  inv1   g30(.a(x09), .O(new_n65_));
  nand2  g31(.a(new_n62_), .b(new_n65_), .O(new_n66_));
  nand4  g32(.a(x08), .b(x07), .c(x06), .d(new_n44_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n51_), .O(z05));
  nor2   g35(.a(new_n61_), .b(x09), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(new_n37_), .b(new_n71_), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand2  g39(.a(x10), .b(new_n65_), .O(new_n74_));
  nor2   g40(.a(new_n74_), .b(new_n67_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n73_), .c(new_n51_), .O(z06));
  nand4  g42(.a(new_n71_), .b(new_n65_), .c(x08), .d(x07), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  aoi22  g44(.a(new_n78_), .b(new_n70_), .c(new_n77_), .d(x11), .O(new_n79_));
  nand2  g45(.a(x11), .b(new_n45_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n59_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n37_), .O(new_n82_));
  oai21  g48(.a(new_n79_), .b(x05), .c(new_n82_), .O(z07));
  inv1   g49(.a(x12), .O(new_n84_));
  nand3  g50(.a(new_n78_), .b(new_n70_), .c(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n78_), .b(new_n70_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x12), .O(new_n87_));
  oai21  g53(.a(x12), .b(x06), .c(x05), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n87_), .c(new_n85_), .d(new_n51_), .O(z08));
  nand2  g55(.a(new_n85_), .b(x13), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand3  g57(.a(new_n78_), .b(new_n91_), .c(new_n84_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n70_), .O(new_n94_));
  oai21  g60(.a(x13), .b(x06), .c(x05), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(new_n90_), .d(new_n51_), .O(z09));
  nor2   g62(.a(new_n38_), .b(new_n43_), .O(z13));
  inv1   g63(.a(z13), .O(new_n98_));
  nand2  g64(.a(new_n50_), .b(x14), .O(new_n99_));
  inv1   g65(.a(new_n67_), .O(new_n100_));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n92_), .b(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n48_), .b(x02), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(x16), .c(x09), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(new_n102_), .c(new_n100_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n99_), .c(new_n98_), .O(z10));
  nor2   g72(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n36_), .c(x03), .O(new_n108_));
  nor2   g74(.a(new_n108_), .b(new_n98_), .O(z12));
  nand2  g75(.a(z13), .b(x17), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z14));
endmodule


