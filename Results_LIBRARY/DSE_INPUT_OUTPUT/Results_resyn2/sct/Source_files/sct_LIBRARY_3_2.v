// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n98_, new_n101_;
  nand2  g00(.a(x18), .b(x01), .O(new_n35_));
  aoi22  g01(.a(new_n35_), .b(x14), .c(x02), .d(x01), .O(z00));
  inv1   g02(.a(x18), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x04), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  aoi22  g07(.a(new_n41_), .b(x18), .c(new_n40_), .d(x05), .O(z01));
  inv1   g08(.a(x06), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  inv1   g10(.a(x03), .O(new_n45_));
  inv1   g11(.a(x16), .O(new_n46_));
  aoi21  g12(.a(new_n45_), .b(x02), .c(new_n46_), .O(new_n47_));
  nor2   g13(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n38_), .O(z02));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n43_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n51_), .c(new_n48_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n38_), .O(z03));
  nor2   g21(.a(new_n47_), .b(new_n39_), .O(new_n56_));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n51_), .b(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  inv1   g27(.a(x02), .O(new_n62_));
  oai21  g28(.a(x03), .b(new_n62_), .c(x16), .O(new_n63_));
  nand2  g29(.a(new_n57_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  inv1   g31(.a(new_n57_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n64_), .c(new_n63_), .d(x04), .O(new_n68_));
  and2   g34(.a(new_n68_), .b(new_n38_), .O(z05));
  inv1   g35(.a(new_n38_), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nor2   g37(.a(new_n57_), .b(x09), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g39(.a(new_n57_), .b(x09), .c(x10), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n63_), .c(x04), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z06));
  nand2  g43(.a(new_n73_), .b(x11), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n72_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n56_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand3  g48(.a(new_n79_), .b(new_n72_), .c(new_n82_), .O(new_n83_));
  nand2  g49(.a(new_n80_), .b(x12), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n56_), .O(z08));
  nand2  g51(.a(new_n83_), .b(x13), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand3  g53(.a(new_n79_), .b(new_n87_), .c(new_n82_), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n72_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n86_), .c(new_n56_), .O(z09));
  nand2  g57(.a(new_n47_), .b(x14), .O(new_n92_));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n72_), .c(new_n63_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n92_), .c(new_n39_), .O(z10));
  nor2   g62(.a(new_n70_), .b(new_n62_), .O(z11));
  aoi21  g63(.a(x16), .b(new_n62_), .c(x03), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n39_), .O(z12));
  nand2  g65(.a(new_n38_), .b(new_n44_), .O(z13));
  inv1   g66(.a(x17), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n44_), .c(new_n38_), .O(z14));
endmodule


