// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n103_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x17), .b(x06), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi22  g08(.a(x17), .b(x06), .c(x05), .d(new_n42_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(new_n46_));
  nor3   g12(.a(new_n46_), .b(x06), .c(new_n42_), .O(z02));
  nand2  g13(.a(new_n45_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  inv1   g15(.a(x06), .O(new_n50_));
  inv1   g16(.a(x07), .O(new_n51_));
  oai21  g17(.a(x17), .b(new_n50_), .c(new_n51_), .O(new_n52_));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n52_), .c(new_n49_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  xor2a  g21(.a(new_n53_), .b(x08), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n48_), .c(new_n37_), .O(z04));
  nand2  g23(.a(x08), .b(x07), .O(new_n58_));
  nor2   g24(.a(new_n58_), .b(x09), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x17), .c(x06), .O(new_n60_));
  inv1   g26(.a(x17), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n60_), .c(new_n49_), .O(z05));
  inv1   g30(.a(x10), .O(new_n65_));
  nand2  g31(.a(new_n59_), .b(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  oai21  g34(.a(new_n62_), .b(x09), .c(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n49_), .O(z06));
  inv1   g36(.a(x11), .O(new_n71_));
  nand3  g37(.a(new_n59_), .b(new_n71_), .c(new_n65_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n61_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  inv1   g40(.a(x09), .O(new_n75_));
  inv1   g41(.a(new_n62_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n65_), .c(new_n75_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x11), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n74_), .c(new_n49_), .O(z07));
  inv1   g45(.a(x12), .O(new_n80_));
  nand4  g46(.a(new_n59_), .b(new_n80_), .c(new_n71_), .d(new_n65_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n61_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x06), .O(new_n83_));
  oai21  g49(.a(new_n77_), .b(x11), .c(x12), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n49_), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  aoi21  g52(.a(new_n81_), .b(new_n61_), .c(new_n50_), .O(new_n87_));
  nor2   g53(.a(new_n62_), .b(x09), .O(new_n88_));
  nand4  g54(.a(new_n86_), .b(new_n80_), .c(new_n71_), .d(new_n65_), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  aoi22  g56(.a(new_n90_), .b(new_n88_), .c(new_n48_), .d(new_n37_), .O(new_n91_));
  oai21  g57(.a(new_n87_), .b(new_n86_), .c(new_n91_), .O(z09));
  nand2  g58(.a(new_n37_), .b(x04), .O(new_n93_));
  nand2  g59(.a(new_n46_), .b(x14), .O(new_n94_));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n89_), .b(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n88_), .c(new_n45_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n94_), .c(new_n93_), .O(z10));
  nand2  g64(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n35_), .c(x03), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n42_), .c(new_n37_), .O(z12));
  inv1   g67(.a(new_n93_), .O(z13));
  nand3  g68(.a(x17), .b(new_n50_), .c(x04), .O(new_n103_));
  inv1   g69(.a(new_n103_), .O(z14));
endmodule


