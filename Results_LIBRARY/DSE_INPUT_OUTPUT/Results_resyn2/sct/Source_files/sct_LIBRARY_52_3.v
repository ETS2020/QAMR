// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x06), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(new_n36_), .b(new_n39_), .O(new_n40_));
  inv1   g06(.a(x02), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x01), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n40_), .c(new_n38_), .O(z00));
  nor2   g09(.a(x15), .b(x05), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  aoi21  g12(.a(new_n44_), .b(x18), .c(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n41_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n38_), .O(z02));
  inv1   g16(.a(new_n49_), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  oai21  g18(.a(x14), .b(new_n35_), .c(new_n52_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  nand2  g23(.a(new_n54_), .b(new_n57_), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n58_), .c(new_n38_), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n49_), .O(z04));
  nor2   g27(.a(new_n57_), .b(new_n52_), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(x14), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  aoi21  g30(.a(new_n48_), .b(x04), .c(new_n37_), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nor2   g34(.a(new_n68_), .b(x14), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n66_), .c(new_n64_), .O(z05));
  inv1   g37(.a(x10), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n67_), .c(x08), .d(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n36_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand2  g41(.a(new_n70_), .b(x10), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n51_), .O(z06));
  nand2  g43(.a(new_n75_), .b(x11), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand3  g45(.a(new_n36_), .b(new_n79_), .c(x06), .O(new_n80_));
  or2    g46(.a(new_n80_), .b(new_n73_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n78_), .c(new_n66_), .O(z07));
  nor2   g48(.a(x11), .b(x10), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n62_), .c(x12), .d(new_n67_), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n36_), .c(new_n35_), .O(new_n85_));
  aoi21  g51(.a(new_n83_), .b(new_n69_), .c(x12), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n85_), .c(new_n66_), .O(z08));
  inv1   g53(.a(x12), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n79_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n73_), .c(new_n36_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x13), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand3  g59(.a(new_n83_), .b(new_n93_), .c(new_n88_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n69_), .c(new_n65_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n92_), .O(z09));
  nor2   g63(.a(new_n37_), .b(x04), .O(new_n98_));
  nand2  g64(.a(new_n48_), .b(new_n35_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x14), .O(new_n100_));
  inv1   g66(.a(new_n68_), .O(new_n101_));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n94_), .b(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n101_), .c(new_n48_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n100_), .c(new_n98_), .O(z10));
  nand2  g71(.a(new_n38_), .b(new_n41_), .O(z11));
  nand2  g72(.a(x16), .b(new_n41_), .O(new_n107_));
  nor2   g73(.a(new_n37_), .b(x03), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n107_), .c(new_n98_), .O(z12));
  inv1   g75(.a(new_n98_), .O(z13));
  nand2  g76(.a(x17), .b(x04), .O(new_n111_));
  nor2   g77(.a(new_n111_), .b(new_n37_), .O(z14));
endmodule


