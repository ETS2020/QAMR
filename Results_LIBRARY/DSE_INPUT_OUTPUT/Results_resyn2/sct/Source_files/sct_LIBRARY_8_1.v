// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x18), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x07), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  oai21  g07(.a(x15), .b(x05), .c(x07), .O(new_n42_));
  aoi22  g08(.a(new_n42_), .b(x18), .c(x05), .d(x04), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand4  g11(.a(new_n45_), .b(new_n39_), .c(new_n44_), .d(x04), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z02));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nor2   g19(.a(new_n53_), .b(new_n44_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(new_n55_));
  nand2  g21(.a(new_n53_), .b(new_n44_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n52_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n39_), .O(z03));
  nand2  g24(.a(new_n39_), .b(x08), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n45_), .c(x04), .O(new_n61_));
  aoi21  g27(.a(new_n59_), .b(new_n55_), .c(new_n61_), .O(z04));
  inv1   g28(.a(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n64_));
  nand2  g30(.a(new_n60_), .b(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n64_), .c(new_n45_), .d(x04), .O(new_n66_));
  and2   g32(.a(new_n66_), .b(new_n39_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nand2  g34(.a(new_n64_), .b(new_n68_), .O(new_n69_));
  inv1   g35(.a(x08), .O(new_n70_));
  nor2   g36(.a(x09), .b(new_n70_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n54_), .c(x10), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n52_), .c(new_n38_), .O(z06));
  oai21  g40(.a(x18), .b(x11), .c(new_n53_), .O(new_n75_));
  inv1   g41(.a(new_n60_), .O(new_n76_));
  nor3   g42(.a(x11), .b(x10), .c(x09), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand4  g44(.a(new_n68_), .b(new_n63_), .c(x08), .d(x06), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x11), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n78_), .c(new_n75_), .d(new_n52_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand3  g48(.a(new_n77_), .b(new_n76_), .c(new_n82_), .O(new_n83_));
  oai21  g49(.a(new_n79_), .b(x11), .c(x12), .O(new_n84_));
  oai21  g50(.a(x18), .b(x12), .c(new_n53_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n52_), .O(z08));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n77_), .b(new_n76_), .c(new_n87_), .d(new_n82_), .O(new_n88_));
  inv1   g54(.a(x11), .O(new_n89_));
  nor2   g55(.a(x10), .b(new_n44_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n71_), .c(new_n82_), .d(new_n89_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x13), .O(new_n92_));
  oai21  g58(.a(x18), .b(x13), .c(new_n53_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n45_), .c(x04), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n92_), .c(new_n88_), .O(z09));
  nor2   g62(.a(new_n38_), .b(x04), .O(new_n97_));
  inv1   g63(.a(new_n64_), .O(new_n98_));
  inv1   g64(.a(x00), .O(new_n99_));
  nand4  g65(.a(new_n87_), .b(new_n82_), .c(new_n89_), .d(new_n68_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n98_), .c(new_n45_), .O(new_n102_));
  aoi21  g68(.a(new_n51_), .b(x14), .c(new_n38_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n97_), .O(z10));
  nor2   g70(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n35_), .c(x03), .O(new_n106_));
  nor3   g72(.a(new_n106_), .b(new_n38_), .c(new_n48_), .O(z12));
  inv1   g73(.a(new_n97_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n48_), .c(new_n39_), .O(z14));
endmodule


