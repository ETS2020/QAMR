// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n91_, new_n92_, new_n95_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x07), .O(new_n36_));
  nor2   g02(.a(x11), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x02), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n49_), .c(x16), .O(new_n50_));
  nand4  g16(.a(new_n50_), .b(new_n38_), .c(new_n48_), .d(x04), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(x11), .b(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand4  g21(.a(new_n55_), .b(new_n54_), .c(new_n50_), .d(x04), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  nand2  g23(.a(new_n50_), .b(x04), .O(new_n58_));
  xor2a  g24(.a(new_n55_), .b(x08), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n58_), .c(new_n38_), .O(z04));
  inv1   g26(.a(new_n58_), .O(new_n61_));
  nand2  g27(.a(x08), .b(x06), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(x09), .c(x11), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x07), .O(new_n64_));
  oai21  g30(.a(new_n62_), .b(new_n53_), .c(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n64_), .c(new_n61_), .O(z05));
  nand2  g32(.a(new_n64_), .b(x10), .O(new_n67_));
  nand2  g33(.a(new_n58_), .b(new_n38_), .O(new_n68_));
  nor2   g34(.a(new_n62_), .b(x09), .O(new_n69_));
  nor2   g35(.a(new_n53_), .b(x10), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n67_), .O(z06));
  inv1   g38(.a(x11), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n36_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n61_), .O(z07));
  nand2  g42(.a(new_n38_), .b(x12), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n68_), .O(z08));
  nand2  g44(.a(new_n38_), .b(x13), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n68_), .O(z09));
  nor2   g46(.a(new_n37_), .b(x04), .O(new_n81_));
  nand2  g47(.a(x08), .b(x07), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(x09), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n50_), .c(x06), .d(x00), .O(new_n84_));
  inv1   g50(.a(x03), .O(new_n85_));
  inv1   g51(.a(x16), .O(new_n86_));
  aoi21  g52(.a(new_n85_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(x14), .c(new_n37_), .O(new_n88_));
  aoi21  g54(.a(new_n88_), .b(new_n84_), .c(new_n81_), .O(z10));
  nor2   g55(.a(new_n37_), .b(new_n49_), .O(z11));
  nand2  g56(.a(x16), .b(new_n49_), .O(new_n91_));
  nor2   g57(.a(new_n37_), .b(x03), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n91_), .c(new_n81_), .O(z12));
  inv1   g59(.a(new_n81_), .O(z13));
  nor2   g60(.a(new_n37_), .b(x17), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n81_), .O(z14));
endmodule


