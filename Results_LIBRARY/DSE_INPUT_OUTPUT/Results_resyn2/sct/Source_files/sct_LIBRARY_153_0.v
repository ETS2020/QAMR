// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x17), .b(x07), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  nor2   g13(.a(new_n37_), .b(new_n47_), .O(z13));
  oai21  g14(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(z13), .c(new_n46_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x03), .O(new_n52_));
  inv1   g18(.a(x16), .O(new_n53_));
  aoi21  g19(.a(new_n52_), .b(x02), .c(new_n53_), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n38_), .O(new_n58_));
  nor3   g24(.a(new_n58_), .b(new_n54_), .c(new_n47_), .O(z03));
  nor2   g25(.a(new_n54_), .b(new_n47_), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  oai21  g27(.a(new_n37_), .b(new_n61_), .c(new_n57_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  nor2   g31(.a(new_n63_), .b(x09), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  oai21  g33(.a(new_n37_), .b(x09), .c(new_n63_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nand2  g36(.a(new_n66_), .b(new_n70_), .O(new_n71_));
  oai21  g37(.a(new_n63_), .b(x09), .c(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n49_), .c(x04), .O(new_n73_));
  inv1   g39(.a(new_n73_), .O(new_n74_));
  aoi21  g40(.a(new_n74_), .b(new_n71_), .c(new_n37_), .O(z06));
  nand2  g41(.a(x08), .b(x06), .O(new_n76_));
  nor2   g42(.a(new_n76_), .b(x09), .O(new_n77_));
  inv1   g43(.a(x09), .O(new_n78_));
  nand4  g44(.a(new_n70_), .b(new_n78_), .c(x08), .d(x06), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  aoi22  g46(.a(new_n80_), .b(new_n77_), .c(new_n79_), .d(x11), .O(new_n81_));
  nand2  g47(.a(x11), .b(new_n55_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n49_), .c(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n38_), .O(new_n84_));
  oai21  g50(.a(new_n81_), .b(new_n55_), .c(new_n84_), .O(z07));
  nand4  g51(.a(new_n80_), .b(new_n78_), .c(x08), .d(x06), .O(new_n86_));
  xor2a  g52(.a(new_n86_), .b(x12), .O(new_n87_));
  nand2  g53(.a(x12), .b(new_n55_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n49_), .c(x04), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n38_), .O(new_n90_));
  oai21  g56(.a(new_n87_), .b(new_n55_), .c(new_n90_), .O(z08));
  oai21  g57(.a(new_n86_), .b(x12), .c(x13), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand3  g60(.a(new_n80_), .b(new_n94_), .c(new_n93_), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n66_), .O(new_n97_));
  inv1   g63(.a(x17), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(x13), .c(new_n55_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n97_), .c(new_n92_), .d(new_n60_), .O(z09));
  nand3  g66(.a(new_n54_), .b(new_n38_), .c(x14), .O(new_n101_));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n95_), .b(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n66_), .c(new_n49_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n101_), .c(new_n47_), .O(z10));
  nor2   g71(.a(new_n37_), .b(new_n35_), .O(z11));
  oai21  g72(.a(new_n53_), .b(x02), .c(new_n52_), .O(new_n107_));
  and2   g73(.a(new_n107_), .b(z13), .O(z12));
  oai21  g74(.a(new_n98_), .b(new_n47_), .c(new_n38_), .O(z14));
endmodule


