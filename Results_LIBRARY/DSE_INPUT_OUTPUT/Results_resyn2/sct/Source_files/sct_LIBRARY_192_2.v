// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n72_, new_n73_, new_n76_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n90_, new_n92_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x10), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  inv1   g18(.a(x10), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n37_), .c(new_n52_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n54_), .c(new_n49_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(new_n55_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(x08), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n55_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n39_), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n48_), .O(z04));
  nand2  g29(.a(new_n48_), .b(new_n39_), .O(new_n64_));
  nor2   g30(.a(new_n59_), .b(x09), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x10), .O(new_n66_));
  nor2   g32(.a(new_n60_), .b(new_n52_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n53_), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n66_), .c(new_n64_), .O(z05));
  oai21  g36(.a(new_n65_), .b(new_n53_), .c(new_n64_), .O(z06));
  inv1   g37(.a(x11), .O(new_n72_));
  inv1   g38(.a(x04), .O(new_n73_));
  nor2   g39(.a(new_n38_), .b(new_n73_), .O(z13));
  nand3  g40(.a(z13), .b(new_n47_), .c(new_n72_), .O(z07));
  inv1   g41(.a(x12), .O(new_n76_));
  nand3  g42(.a(z13), .b(new_n47_), .c(new_n76_), .O(z08));
  nand2  g43(.a(new_n39_), .b(x13), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n64_), .O(z09));
  inv1   g45(.a(x03), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x02), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(x16), .c(x14), .O(new_n82_));
  nand2  g48(.a(x06), .b(x00), .O(new_n83_));
  nor2   g49(.a(new_n83_), .b(x09), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n67_), .c(new_n47_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n39_), .O(z10));
  nor2   g54(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g55(.a(x16), .b(new_n35_), .c(x03), .O(new_n90_));
  nor3   g56(.a(new_n90_), .b(new_n38_), .c(new_n73_), .O(z12));
  nand2  g57(.a(x17), .b(x04), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n39_), .O(z14));
endmodule


