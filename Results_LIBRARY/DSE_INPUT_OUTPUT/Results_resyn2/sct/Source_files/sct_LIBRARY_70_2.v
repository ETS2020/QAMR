// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n94_, new_n97_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  nor2   g03(.a(x06), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor3   g08(.a(new_n38_), .b(new_n42_), .c(x15), .O(new_n43_));
  nand2  g09(.a(x06), .b(x04), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(x05), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x07), .O(new_n48_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand2  g14(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g15(.a(new_n50_), .b(x06), .c(new_n37_), .O(z03));
  nand2  g16(.a(x08), .b(x07), .O(new_n52_));
  inv1   g17(.a(x08), .O(new_n53_));
  nand2  g18(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nand3  g19(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(new_n55_));
  aoi21  g20(.a(new_n55_), .b(x06), .c(new_n37_), .O(z04));
  inv1   g21(.a(new_n49_), .O(new_n57_));
  nor2   g22(.a(new_n57_), .b(new_n44_), .O(new_n58_));
  nand2  g23(.a(new_n52_), .b(x09), .O(new_n59_));
  inv1   g24(.a(x09), .O(new_n60_));
  nand3  g25(.a(new_n60_), .b(x08), .c(x07), .O(new_n61_));
  nand3  g26(.a(new_n61_), .b(new_n59_), .c(new_n58_), .O(z05));
  inv1   g27(.a(x10), .O(new_n63_));
  nor2   g28(.a(new_n52_), .b(x09), .O(new_n64_));
  nand2  g29(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g30(.a(new_n61_), .b(x10), .O(new_n66_));
  nand3  g31(.a(new_n66_), .b(new_n65_), .c(new_n58_), .O(z06));
  nand2  g32(.a(new_n65_), .b(x11), .O(new_n68_));
  nor2   g33(.a(x11), .b(x10), .O(new_n69_));
  nand2  g34(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  nand3  g35(.a(new_n70_), .b(new_n68_), .c(new_n58_), .O(z07));
  inv1   g36(.a(x11), .O(new_n72_));
  nand2  g37(.a(new_n72_), .b(new_n63_), .O(new_n73_));
  oai21  g38(.a(new_n73_), .b(new_n61_), .c(x12), .O(new_n74_));
  nor3   g39(.a(x12), .b(x11), .c(x10), .O(new_n75_));
  nand2  g40(.a(new_n75_), .b(new_n64_), .O(new_n76_));
  nand3  g41(.a(new_n76_), .b(new_n74_), .c(new_n49_), .O(new_n77_));
  nand2  g42(.a(new_n77_), .b(x06), .O(new_n78_));
  nand2  g43(.a(new_n78_), .b(x04), .O(z08));
  inv1   g44(.a(x13), .O(new_n80_));
  aoi21  g45(.a(new_n75_), .b(new_n64_), .c(new_n80_), .O(new_n81_));
  nor2   g46(.a(x13), .b(x12), .O(new_n82_));
  nand2  g47(.a(new_n82_), .b(new_n69_), .O(new_n83_));
  oai21  g48(.a(new_n83_), .b(new_n61_), .c(new_n49_), .O(new_n84_));
  oai21  g49(.a(new_n84_), .b(new_n81_), .c(x06), .O(new_n85_));
  nand2  g50(.a(new_n85_), .b(x04), .O(z09));
  inv1   g51(.a(x00), .O(new_n87_));
  nand2  g52(.a(new_n83_), .b(new_n87_), .O(new_n88_));
  nand3  g53(.a(new_n88_), .b(new_n64_), .c(new_n49_), .O(new_n89_));
  inv1   g54(.a(x06), .O(new_n90_));
  aoi21  g55(.a(new_n57_), .b(x14), .c(new_n90_), .O(new_n91_));
  aoi21  g56(.a(new_n91_), .b(new_n89_), .c(new_n37_), .O(z10));
  nor2   g57(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g58(.a(x16), .b(new_n35_), .c(x03), .O(new_n94_));
  nor2   g59(.a(new_n94_), .b(new_n44_), .O(z12));
  inv1   g60(.a(new_n44_), .O(z13));
  inv1   g61(.a(x17), .O(new_n97_));
  aoi21  g62(.a(new_n97_), .b(x06), .c(new_n37_), .O(z14));
  zero   g63(.O(z02));
endmodule


