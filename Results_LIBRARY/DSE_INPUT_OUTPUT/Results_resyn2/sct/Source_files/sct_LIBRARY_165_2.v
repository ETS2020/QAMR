// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x13), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x09), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n43_), .c(new_n38_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nand2  g16(.a(new_n38_), .b(x04), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(x07), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n55_), .c(new_n52_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n52_), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  nor2   g30(.a(new_n60_), .b(x09), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n60_), .b(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n52_), .O(z05));
  inv1   g34(.a(new_n38_), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  nand2  g36(.a(new_n65_), .b(new_n70_), .O(new_n71_));
  oai21  g37(.a(x03), .b(new_n35_), .c(x16), .O(new_n72_));
  oai21  g38(.a(new_n60_), .b(x09), .c(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(x04), .O(new_n74_));
  inv1   g40(.a(new_n74_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n71_), .c(new_n69_), .O(z06));
  inv1   g42(.a(x09), .O(new_n77_));
  oai21  g43(.a(new_n60_), .b(x10), .c(x11), .O(new_n78_));
  inv1   g44(.a(new_n60_), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n77_), .O(new_n83_));
  nand2  g49(.a(new_n72_), .b(x04), .O(new_n84_));
  and2   g50(.a(x11), .b(x09), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n84_), .c(new_n38_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n83_), .O(z07));
  inv1   g53(.a(new_n84_), .O(new_n88_));
  nand2  g54(.a(new_n81_), .b(x12), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nand4  g56(.a(new_n80_), .b(new_n79_), .c(new_n90_), .d(new_n77_), .O(new_n91_));
  oai21  g57(.a(new_n37_), .b(x12), .c(x09), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n91_), .c(new_n89_), .d(new_n88_), .O(z08));
  nand2  g59(.a(new_n80_), .b(new_n90_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n60_), .c(x13), .O(new_n95_));
  nand3  g61(.a(new_n80_), .b(new_n37_), .c(new_n90_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n79_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n95_), .c(new_n88_), .d(new_n77_), .O(z09));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n96_), .b(new_n100_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n65_), .c(new_n72_), .O(new_n102_));
  nand3  g68(.a(new_n50_), .b(new_n38_), .c(x14), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n44_), .O(z10));
  nor2   g70(.a(new_n69_), .b(new_n35_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n35_), .c(x03), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n51_), .O(z12));
  inv1   g73(.a(new_n51_), .O(z13));
  nand2  g74(.a(x17), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n38_), .O(z14));
endmodule


