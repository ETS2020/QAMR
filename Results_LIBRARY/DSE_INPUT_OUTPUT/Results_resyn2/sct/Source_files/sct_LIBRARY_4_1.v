// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  nand2  g03(.a(x06), .b(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(new_n47_));
  inv1   g13(.a(x06), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(new_n47_), .c(new_n38_), .O(z02));
  inv1   g16(.a(x07), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n49_), .b(new_n51_), .c(new_n52_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n46_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n38_), .O(z03));
  nand3  g21(.a(new_n52_), .b(new_n49_), .c(x08), .O(new_n56_));
  inv1   g22(.a(x08), .O(new_n57_));
  oai21  g23(.a(new_n51_), .b(new_n48_), .c(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(new_n46_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n38_), .O(z04));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  oai21  g27(.a(x09), .b(new_n37_), .c(new_n61_), .O(new_n62_));
  and2   g28(.a(x07), .b(x06), .O(new_n63_));
  nor2   g29(.a(x09), .b(new_n57_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n62_), .c(new_n46_), .O(new_n66_));
  and2   g32(.a(new_n66_), .b(new_n38_), .O(z05));
  oai21  g33(.a(x10), .b(new_n37_), .c(new_n48_), .O(new_n68_));
  inv1   g34(.a(x10), .O(new_n69_));
  nand3  g35(.a(new_n64_), .b(new_n63_), .c(new_n69_), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(x08), .c(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n70_), .c(new_n46_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x04), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n68_), .O(z06));
  oai21  g42(.a(x11), .b(new_n37_), .c(new_n48_), .O(new_n77_));
  oai21  g43(.a(new_n72_), .b(x10), .c(x11), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n64_), .c(new_n63_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n46_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x04), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n77_), .O(z07));
  oai21  g49(.a(x12), .b(new_n37_), .c(new_n48_), .O(new_n84_));
  inv1   g50(.a(x11), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n72_), .c(x12), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  inv1   g54(.a(new_n61_), .O(new_n89_));
  nand4  g55(.a(new_n79_), .b(new_n89_), .c(new_n88_), .d(new_n71_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n87_), .c(new_n46_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x04), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n84_), .O(z08));
  oai21  g59(.a(x13), .b(new_n37_), .c(new_n48_), .O(new_n94_));
  nand3  g60(.a(new_n88_), .b(new_n85_), .c(new_n69_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n72_), .c(x13), .O(new_n96_));
  nor2   g62(.a(x13), .b(x12), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n79_), .c(new_n64_), .d(new_n63_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n96_), .c(new_n46_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x04), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n94_), .O(z09));
  nand2  g67(.a(new_n47_), .b(x14), .O(new_n102_));
  inv1   g68(.a(x00), .O(new_n103_));
  nand2  g69(.a(new_n97_), .b(new_n79_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g71(.a(new_n105_), .b(new_n64_), .c(new_n63_), .d(new_n46_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n102_), .c(new_n37_), .O(z10));
  aoi21  g73(.a(x06), .b(new_n37_), .c(new_n35_), .O(z11));
  aoi21  g74(.a(x16), .b(new_n35_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n37_), .c(new_n38_), .O(z12));
  and2   g76(.a(x17), .b(x04), .O(z14));
  buf    g77(.a(x04), .O(z13));
endmodule


