// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n100_, new_n102_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand2  g02(.a(x06), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  oai21  g07(.a(x06), .b(x05), .c(new_n36_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g11(.a(x02), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n46_), .c(x16), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nor3   g14(.a(new_n48_), .b(x06), .c(new_n36_), .O(z02));
  inv1   g15(.a(x06), .O(new_n50_));
  nand3  g16(.a(x07), .b(new_n50_), .c(x04), .O(new_n51_));
  oai21  g17(.a(x07), .b(new_n50_), .c(new_n51_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n37_), .O(z03));
  nor2   g20(.a(x06), .b(x04), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z13));
  aoi22  g22(.a(z13), .b(x08), .c(x07), .d(x06), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n57_), .c(new_n37_), .O(z04));
  nand2  g26(.a(new_n58_), .b(x09), .O(new_n61_));
  nor2   g27(.a(new_n58_), .b(x09), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  nor2   g29(.a(new_n48_), .b(new_n36_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(z05));
  nand2  g31(.a(new_n63_), .b(x10), .O(new_n66_));
  inv1   g32(.a(new_n58_), .O(new_n67_));
  nor2   g33(.a(x10), .b(x09), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n66_), .c(new_n64_), .O(z06));
  oai21  g36(.a(x11), .b(new_n36_), .c(new_n50_), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  nand3  g39(.a(new_n62_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  nand3  g40(.a(new_n68_), .b(x08), .c(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n47_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x04), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n71_), .O(z07));
  oai21  g45(.a(x12), .b(new_n36_), .c(new_n50_), .O(new_n80_));
  nand4  g46(.a(new_n68_), .b(new_n73_), .c(x08), .d(x07), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand4  g49(.a(new_n68_), .b(new_n67_), .c(new_n83_), .d(new_n73_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n82_), .c(new_n47_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x04), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n80_), .O(z08));
  nand2  g53(.a(new_n84_), .b(x13), .O(new_n88_));
  inv1   g54(.a(x13), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n83_), .c(new_n73_), .d(new_n72_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n62_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n88_), .c(new_n64_), .O(z09));
  nand2  g59(.a(new_n48_), .b(x14), .O(new_n94_));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n62_), .c(new_n47_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n94_), .c(new_n36_), .O(z10));
  aoi21  g64(.a(x06), .b(new_n36_), .c(new_n46_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n46_), .c(x03), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n36_), .c(new_n37_), .O(z12));
  inv1   g67(.a(x17), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(x04), .c(new_n55_), .O(z14));
endmodule


