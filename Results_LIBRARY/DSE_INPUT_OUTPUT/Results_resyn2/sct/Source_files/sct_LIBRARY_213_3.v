// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n95_,
    new_n98_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(x14), .b(x13), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  nor2   g05(.a(x15), .b(x05), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  aoi21  g08(.a(new_n40_), .b(x18), .c(new_n42_), .O(z01));
  oai21  g09(.a(x03), .b(new_n35_), .c(x16), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(x04), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x06), .c(new_n36_), .O(z02));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x07), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand2  g16(.a(new_n48_), .b(new_n47_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n45_), .c(new_n36_), .O(z03));
  inv1   g19(.a(x03), .O(new_n54_));
  inv1   g20(.a(x16), .O(new_n55_));
  aoi21  g21(.a(new_n54_), .b(x02), .c(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n36_), .b(x04), .O(new_n57_));
  nor2   g23(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g24(.a(new_n49_), .b(x08), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n50_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n58_), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  nand2  g29(.a(new_n59_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n58_), .O(z05));
  nand2  g33(.a(new_n66_), .b(x10), .O(new_n68_));
  nor2   g34(.a(new_n66_), .b(x10), .O(new_n69_));
  nor2   g35(.a(new_n69_), .b(new_n45_), .O(new_n70_));
  aoi21  g36(.a(new_n70_), .b(new_n68_), .c(new_n37_), .O(z06));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  inv1   g39(.a(new_n66_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(new_n75_));
  oai21  g41(.a(new_n66_), .b(x10), .c(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n58_), .O(z07));
  nand2  g43(.a(new_n75_), .b(x12), .O(new_n78_));
  nor2   g44(.a(x12), .b(x11), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(new_n69_), .c(new_n45_), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n78_), .c(new_n37_), .O(z08));
  inv1   g47(.a(x13), .O(new_n82_));
  nand3  g48(.a(new_n79_), .b(new_n69_), .c(new_n82_), .O(new_n83_));
  nand2  g49(.a(new_n79_), .b(new_n69_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x13), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n83_), .c(new_n58_), .O(z09));
  inv1   g52(.a(x04), .O(new_n87_));
  oai21  g53(.a(new_n56_), .b(x13), .c(x14), .O(new_n88_));
  inv1   g54(.a(x00), .O(new_n89_));
  nand3  g55(.a(new_n79_), .b(new_n82_), .c(new_n72_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n74_), .c(new_n44_), .O(new_n92_));
  aoi22  g58(.a(new_n92_), .b(new_n88_), .c(new_n36_), .d(new_n87_), .O(z10));
  nor2   g59(.a(new_n37_), .b(new_n35_), .O(z11));
  nand2  g60(.a(x16), .b(new_n35_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n54_), .c(new_n57_), .O(z12));
  nand2  g62(.a(new_n36_), .b(new_n87_), .O(z13));
  nand3  g63(.a(new_n36_), .b(x17), .c(x04), .O(new_n98_));
  inv1   g64(.a(new_n98_), .O(z14));
endmodule


