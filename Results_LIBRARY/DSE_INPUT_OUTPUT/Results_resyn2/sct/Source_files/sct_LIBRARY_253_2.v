// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n96_, new_n97_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x13), .b(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(new_n38_), .O(new_n42_));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n42_), .O(z02));
  xnor2a g16(.a(x07), .b(x06), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n49_), .c(new_n42_), .O(z03));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  xor2a  g19(.a(new_n53_), .b(x08), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n49_), .c(new_n42_), .O(z04));
  inv1   g21(.a(new_n49_), .O(new_n56_));
  inv1   g22(.a(x13), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n57_), .c(x09), .O(new_n59_));
  nand2  g25(.a(x08), .b(x07), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(x09), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n57_), .c(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(z05));
  nand2  g29(.a(new_n62_), .b(x10), .O(new_n64_));
  inv1   g30(.a(x10), .O(new_n65_));
  nand4  g31(.a(new_n61_), .b(x13), .c(new_n65_), .d(x06), .O(new_n66_));
  nand2  g32(.a(new_n49_), .b(new_n42_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(z06));
  inv1   g34(.a(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(x08), .c(x07), .O(new_n70_));
  inv1   g36(.a(x11), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n70_), .c(x13), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(new_n66_), .b(x11), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n56_), .O(z07));
  nand2  g42(.a(new_n74_), .b(x12), .O(new_n77_));
  nor2   g43(.a(new_n72_), .b(new_n70_), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nand3  g45(.a(x13), .b(new_n79_), .c(x06), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  aoi22  g47(.a(new_n81_), .b(new_n78_), .c(new_n49_), .d(new_n42_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n77_), .O(z08));
  nor3   g49(.a(new_n80_), .b(new_n72_), .c(new_n70_), .O(new_n84_));
  nor2   g50(.a(x13), .b(x06), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n84_), .c(new_n56_), .O(z09));
  inv1   g52(.a(x03), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x02), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(x16), .c(x14), .O(new_n89_));
  and2   g55(.a(x06), .b(x00), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n61_), .c(new_n48_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x04), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n42_), .O(z10));
  nor2   g60(.a(new_n38_), .b(new_n36_), .O(z11));
  inv1   g61(.a(x04), .O(new_n96_));
  aoi21  g62(.a(x16), .b(new_n36_), .c(x03), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n96_), .c(new_n42_), .O(z12));
  nor2   g64(.a(new_n38_), .b(new_n96_), .O(z13));
  and2   g65(.a(z13), .b(x17), .O(z14));
endmodule


