// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n94_, new_n97_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x12), .b(x06), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g05(.a(x05), .O(new_n40_));
  inv1   g06(.a(x18), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x15), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  nand2  g09(.a(x05), .b(new_n43_), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n42_), .c(new_n37_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  oai21  g13(.a(new_n47_), .b(x06), .c(new_n37_), .O(z02));
  xnor2a g14(.a(x07), .b(x06), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(new_n47_), .c(new_n37_), .O(z03));
  aoi21  g16(.a(x07), .b(x06), .c(x08), .O(new_n51_));
  nand3  g17(.a(x08), .b(x07), .c(x06), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(new_n53_));
  nor3   g19(.a(new_n53_), .b(new_n51_), .c(new_n47_), .O(z04));
  inv1   g20(.a(new_n47_), .O(new_n55_));
  oai21  g21(.a(new_n52_), .b(x12), .c(x09), .O(new_n56_));
  inv1   g22(.a(x12), .O(new_n57_));
  inv1   g23(.a(x09), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(x08), .c(x07), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n56_), .c(new_n55_), .O(z05));
  nand2  g28(.a(new_n61_), .b(x10), .O(new_n63_));
  nand2  g29(.a(new_n47_), .b(new_n37_), .O(new_n64_));
  nor2   g30(.a(new_n52_), .b(x12), .O(new_n65_));
  nor2   g31(.a(x10), .b(x09), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n64_), .c(new_n63_), .O(z06));
  inv1   g34(.a(x06), .O(new_n69_));
  and2   g35(.a(x08), .b(x07), .O(new_n70_));
  aoi21  g36(.a(new_n66_), .b(new_n70_), .c(x12), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n69_), .c(x11), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  nand3  g39(.a(new_n66_), .b(new_n65_), .c(new_n73_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n72_), .c(new_n64_), .O(z07));
  inv1   g41(.a(new_n52_), .O(new_n76_));
  nand3  g42(.a(new_n66_), .b(new_n76_), .c(new_n73_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n55_), .c(new_n57_), .O(z08));
  nand2  g44(.a(new_n74_), .b(x13), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  inv1   g46(.a(x13), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n73_), .c(new_n80_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n59_), .c(new_n57_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n79_), .c(new_n55_), .O(z09));
  nand2  g51(.a(new_n37_), .b(x04), .O(new_n86_));
  inv1   g52(.a(new_n46_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x14), .O(new_n88_));
  inv1   g54(.a(x00), .O(new_n89_));
  nand2  g55(.a(new_n82_), .b(new_n89_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n65_), .c(new_n46_), .d(new_n58_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n88_), .c(new_n86_), .O(z10));
  nand2  g58(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g59(.a(x16), .b(new_n35_), .c(x03), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n86_), .O(z12));
  inv1   g61(.a(new_n86_), .O(z13));
  nand2  g62(.a(z13), .b(x17), .O(new_n97_));
  inv1   g63(.a(new_n97_), .O(z14));
endmodule


