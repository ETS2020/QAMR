// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  oai21  g10(.a(x03), .b(new_n35_), .c(x16), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(x04), .O(new_n46_));
  aoi21  g12(.a(new_n46_), .b(new_n36_), .c(x06), .O(z02));
  inv1   g13(.a(new_n37_), .O(new_n48_));
  nand2  g14(.a(x07), .b(x06), .O(new_n49_));
  inv1   g15(.a(x06), .O(new_n50_));
  inv1   g16(.a(x07), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n46_), .c(new_n48_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(new_n49_), .b(new_n55_), .O(new_n56_));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n46_), .c(new_n48_), .O(z04));
  oai21  g25(.a(new_n37_), .b(x09), .c(new_n57_), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(new_n60_), .c(new_n45_), .d(x04), .O(z05));
  nand3  g29(.a(new_n61_), .b(x08), .c(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x10), .O(new_n65_));
  inv1   g31(.a(x10), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(new_n61_), .c(x08), .d(x07), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g34(.a(x10), .b(new_n50_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n45_), .c(x04), .O(new_n70_));
  aoi21  g36(.a(new_n68_), .b(x06), .c(new_n70_), .O(new_n71_));
  nor2   g37(.a(new_n71_), .b(new_n37_), .O(z06));
  nand2  g38(.a(new_n67_), .b(x11), .O(new_n73_));
  nor2   g39(.a(new_n55_), .b(new_n51_), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n61_), .O(new_n76_));
  and2   g42(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(x06), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n46_), .c(new_n48_), .O(new_n80_));
  oai21  g46(.a(new_n77_), .b(new_n50_), .c(new_n80_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand2  g48(.a(new_n75_), .b(new_n82_), .O(new_n83_));
  nor2   g49(.a(new_n83_), .b(new_n64_), .O(new_n84_));
  aoi21  g50(.a(new_n76_), .b(x12), .c(new_n84_), .O(new_n85_));
  nor2   g51(.a(new_n82_), .b(x06), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n46_), .c(new_n48_), .O(new_n87_));
  oai21  g53(.a(new_n85_), .b(new_n50_), .c(new_n87_), .O(z08));
  oai21  g54(.a(new_n83_), .b(new_n64_), .c(x13), .O(new_n89_));
  nor2   g55(.a(x13), .b(x12), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n75_), .c(new_n74_), .d(new_n61_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x06), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(x06), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n46_), .c(new_n48_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n93_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n90_), .b(new_n75_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  inv1   g66(.a(x03), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x02), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(x16), .c(new_n62_), .O(new_n103_));
  nor2   g69(.a(new_n45_), .b(new_n36_), .O(new_n104_));
  aoi21  g70(.a(new_n103_), .b(new_n100_), .c(new_n104_), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n42_), .c(new_n48_), .O(z10));
  nor2   g72(.a(new_n37_), .b(new_n35_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n35_), .c(x03), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n42_), .c(new_n48_), .O(z12));
  nor2   g75(.a(new_n37_), .b(new_n42_), .O(z13));
  and2   g76(.a(z13), .b(x17), .O(z14));
endmodule


