// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n108_,
    new_n110_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  inv1   g02(.a(x18), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n36_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n38_), .O(z02));
  inv1   g20(.a(x07), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand2  g23(.a(new_n55_), .b(new_n48_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n57_), .c(new_n52_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n38_), .O(z03));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n57_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n52_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n38_), .O(z04));
  nand2  g31(.a(new_n61_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n52_), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(new_n38_), .O(z05));
  nand2  g36(.a(new_n68_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(new_n68_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n71_), .c(new_n52_), .O(z06));
  nand2  g41(.a(new_n74_), .b(x11), .O(new_n76_));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n76_), .c(new_n52_), .O(z07));
  nor2   g45(.a(new_n37_), .b(x04), .O(new_n80_));
  inv1   g46(.a(new_n80_), .O(new_n81_));
  inv1   g47(.a(new_n51_), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n72_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n68_), .c(x12), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  inv1   g52(.a(new_n61_), .O(new_n87_));
  nand4  g53(.a(new_n77_), .b(new_n87_), .c(new_n86_), .d(new_n67_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n85_), .c(new_n82_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x04), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n81_), .O(z08));
  nand4  g57(.a(new_n86_), .b(new_n83_), .c(new_n72_), .d(new_n67_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n61_), .c(x13), .O(new_n93_));
  nor2   g59(.a(x09), .b(new_n62_), .O(new_n94_));
  nor2   g60(.a(x13), .b(x12), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n77_), .c(new_n94_), .d(new_n56_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n93_), .c(new_n82_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x04), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n81_), .O(z09));
  nand2  g65(.a(new_n51_), .b(x14), .O(new_n100_));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n95_), .b(new_n77_), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n73_), .c(new_n82_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n100_), .c(new_n36_), .O(z10));
  inv1   g71(.a(x02), .O(new_n106_));
  aoi21  g72(.a(new_n37_), .b(new_n36_), .c(new_n106_), .O(z11));
  nand2  g73(.a(x16), .b(new_n106_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n49_), .c(new_n36_), .O(z12));
  nor2   g75(.a(x17), .b(new_n36_), .O(new_n110_));
  nor2   g76(.a(new_n110_), .b(new_n80_), .O(z14));
  buf    g77(.a(x04), .O(z13));
endmodule


