// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x17), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x16), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(new_n38_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n47_), .O(z02));
  inv1   g16(.a(x06), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  nand2  g20(.a(new_n52_), .b(new_n51_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n49_), .c(new_n47_), .O(z03));
  xor2a  g23(.a(new_n54_), .b(x08), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n49_), .c(new_n47_), .O(z04));
  inv1   g25(.a(x03), .O(new_n60_));
  inv1   g26(.a(x16), .O(new_n61_));
  aoi21  g27(.a(new_n60_), .b(x02), .c(new_n61_), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n44_), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n63_), .O(new_n68_));
  and2   g34(.a(new_n68_), .b(new_n47_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nand4  g36(.a(new_n53_), .b(new_n70_), .c(new_n66_), .d(x08), .O(new_n71_));
  aoi21  g37(.a(new_n67_), .b(x10), .c(new_n49_), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(new_n71_), .c(new_n38_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  inv1   g40(.a(new_n67_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n70_), .O(new_n76_));
  aoi21  g42(.a(new_n71_), .b(x11), .c(new_n49_), .O(new_n77_));
  aoi21  g43(.a(new_n77_), .b(new_n76_), .c(new_n38_), .O(z07));
  nand2  g44(.a(new_n76_), .b(x12), .O(new_n79_));
  inv1   g45(.a(x12), .O(new_n80_));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n66_), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(new_n64_), .O(new_n83_));
  nor2   g49(.a(new_n83_), .b(new_n49_), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n79_), .c(new_n38_), .O(z08));
  oai21  g51(.a(new_n82_), .b(new_n64_), .c(x13), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand3  g53(.a(new_n81_), .b(new_n87_), .c(new_n80_), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n75_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n86_), .c(new_n63_), .d(new_n47_), .O(z09));
  inv1   g57(.a(x00), .O(new_n92_));
  nand2  g58(.a(new_n88_), .b(new_n92_), .O(new_n93_));
  nor2   g59(.a(new_n67_), .b(new_n62_), .O(new_n94_));
  inv1   g60(.a(x14), .O(new_n95_));
  nor2   g61(.a(new_n48_), .b(new_n95_), .O(new_n96_));
  aoi21  g62(.a(new_n94_), .b(new_n93_), .c(new_n96_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n44_), .c(new_n47_), .O(z10));
  nand2  g64(.a(new_n47_), .b(new_n36_), .O(z11));
  nand2  g65(.a(new_n37_), .b(x03), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n48_), .c(new_n44_), .O(z12));
  nor2   g67(.a(new_n38_), .b(new_n44_), .O(z13));
  aoi21  g68(.a(x16), .b(new_n44_), .c(new_n37_), .O(z14));
endmodule


