// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n101_,
    new_n104_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  inv1   g03(.a(x13), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  or2    g06(.a(x14), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  nor2   g13(.a(x13), .b(new_n37_), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n37_), .c(new_n48_), .O(z02));
  xnor2a g17(.a(x07), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n50_), .c(new_n40_), .O(z03));
  nand2  g19(.a(x08), .b(x07), .O(new_n54_));
  aoi21  g20(.a(new_n54_), .b(new_n38_), .c(new_n37_), .O(new_n55_));
  aoi21  g21(.a(x07), .b(x06), .c(x08), .O(new_n56_));
  nor3   g22(.a(new_n56_), .b(new_n55_), .c(new_n50_), .O(z04));
  nand2  g23(.a(new_n50_), .b(new_n40_), .O(new_n58_));
  inv1   g24(.a(x09), .O(new_n59_));
  inv1   g25(.a(new_n54_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n48_), .c(new_n59_), .O(new_n61_));
  or2    g27(.a(new_n55_), .b(new_n59_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(new_n58_), .O(z05));
  inv1   g29(.a(new_n50_), .O(new_n64_));
  nor2   g30(.a(x10), .b(x09), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n60_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n38_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand2  g34(.a(new_n61_), .b(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n64_), .O(z06));
  aoi21  g36(.a(new_n65_), .b(new_n60_), .c(x13), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n37_), .c(x11), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n60_), .c(new_n48_), .d(new_n59_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n72_), .c(new_n58_), .O(z07));
  inv1   g41(.a(x10), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  inv1   g43(.a(x12), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n59_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n54_), .c(new_n38_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  nand2  g47(.a(new_n74_), .b(x12), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(new_n64_), .O(z08));
  nand3  g49(.a(x08), .b(x07), .c(x06), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(new_n79_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n50_), .c(new_n38_), .O(new_n86_));
  nand2  g52(.a(x13), .b(new_n37_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n86_), .O(z09));
  inv1   g54(.a(new_n49_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x14), .O(new_n90_));
  inv1   g56(.a(new_n84_), .O(new_n91_));
  inv1   g57(.a(x00), .O(new_n92_));
  nand3  g58(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g60(.a(x13), .b(x09), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(new_n91_), .d(new_n49_), .O(new_n96_));
  nand2  g62(.a(new_n40_), .b(x04), .O(new_n97_));
  aoi21  g63(.a(new_n96_), .b(new_n90_), .c(new_n97_), .O(z10));
  nor2   g64(.a(new_n39_), .b(new_n35_), .O(z11));
  nand2  g65(.a(x16), .b(new_n35_), .O(new_n100_));
  nor2   g66(.a(new_n39_), .b(x03), .O(new_n101_));
  aoi22  g67(.a(new_n101_), .b(new_n100_), .c(new_n40_), .d(new_n45_), .O(z12));
  nand2  g68(.a(new_n40_), .b(new_n45_), .O(z13));
  nand3  g69(.a(new_n40_), .b(x17), .c(x04), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(z14));
endmodule


