// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n98_, new_n101_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x12), .O(new_n37_));
  nor2   g03(.a(x13), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n44_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n39_), .c(new_n47_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n54_), .b(new_n47_), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(new_n56_), .c(new_n51_), .d(new_n39_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  nand2  g25(.a(new_n55_), .b(x08), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n51_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n39_), .O(z04));
  nand2  g30(.a(new_n60_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n51_), .O(new_n68_));
  and2   g34(.a(new_n68_), .b(new_n39_), .O(z05));
  nand2  g35(.a(new_n51_), .b(new_n39_), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  nor2   g37(.a(new_n67_), .b(x10), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nand2  g39(.a(new_n67_), .b(x10), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n71_), .O(z06));
  nand2  g41(.a(new_n73_), .b(x11), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  nand2  g43(.a(new_n72_), .b(new_n77_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n76_), .c(new_n71_), .O(z07));
  nand3  g45(.a(new_n72_), .b(new_n37_), .c(new_n77_), .O(new_n80_));
  nand2  g46(.a(new_n78_), .b(x12), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n71_), .O(z08));
  nand2  g48(.a(new_n78_), .b(x13), .O(new_n83_));
  inv1   g49(.a(x13), .O(new_n84_));
  nand3  g50(.a(new_n72_), .b(new_n84_), .c(new_n77_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n83_), .c(new_n51_), .d(new_n37_), .O(z09));
  nand2  g52(.a(new_n50_), .b(x14), .O(new_n87_));
  oai21  g53(.a(x03), .b(new_n35_), .c(x16), .O(new_n88_));
  nor2   g54(.a(x09), .b(new_n61_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n55_), .c(new_n88_), .d(x00), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n39_), .O(new_n92_));
  nor2   g58(.a(new_n50_), .b(x12), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n72_), .c(new_n84_), .d(new_n77_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n92_), .c(new_n44_), .O(z10));
  nor2   g61(.a(new_n38_), .b(new_n35_), .O(z11));
  nand2  g62(.a(x16), .b(new_n35_), .O(new_n97_));
  nor2   g63(.a(new_n38_), .b(x03), .O(new_n98_));
  aoi22  g64(.a(new_n98_), .b(new_n97_), .c(new_n39_), .d(new_n44_), .O(z12));
  nand2  g65(.a(new_n39_), .b(new_n44_), .O(z13));
  inv1   g66(.a(x17), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n44_), .c(new_n39_), .O(z14));
endmodule


