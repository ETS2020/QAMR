// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n105_;
  inv1   g00(.a(x01), .O(new_n35_));
  nand2  g01(.a(x14), .b(new_n35_), .O(new_n36_));
  nand2  g02(.a(x02), .b(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(x18), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nand2  g07(.a(new_n39_), .b(x04), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  nor2   g09(.a(new_n38_), .b(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  nor2   g11(.a(new_n45_), .b(x15), .O(new_n46_));
  aoi22  g12(.a(new_n46_), .b(new_n44_), .c(new_n43_), .d(x05), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n39_), .O(z02));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(x07), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n48_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n39_), .O(z03));
  nor2   g26(.a(new_n52_), .b(new_n42_), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  inv1   g28(.a(x08), .O(new_n63_));
  nand2  g29(.a(new_n56_), .b(new_n63_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(z04));
  inv1   g32(.a(x02), .O(new_n67_));
  oai21  g33(.a(x03), .b(new_n67_), .c(x16), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  inv1   g35(.a(new_n62_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g37(.a(new_n62_), .b(x09), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n71_), .c(new_n68_), .d(x04), .O(new_n73_));
  and2   g39(.a(new_n73_), .b(new_n39_), .O(z05));
  oai21  g40(.a(new_n62_), .b(x09), .c(x10), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  nand3  g42(.a(new_n70_), .b(new_n76_), .c(new_n69_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n75_), .c(new_n68_), .d(x04), .O(new_n78_));
  and2   g44(.a(new_n78_), .b(new_n39_), .O(z06));
  nand2  g45(.a(new_n77_), .b(x11), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nand4  g47(.a(new_n70_), .b(new_n81_), .c(new_n76_), .d(new_n69_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(new_n61_), .O(z07));
  nand2  g49(.a(new_n82_), .b(x12), .O(new_n84_));
  nor2   g50(.a(x12), .b(x11), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n70_), .c(new_n76_), .d(new_n69_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n61_), .O(z08));
  nand2  g53(.a(new_n86_), .b(x13), .O(new_n88_));
  nor2   g54(.a(new_n62_), .b(x09), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n85_), .b(new_n90_), .c(new_n76_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n88_), .c(new_n61_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n91_), .b(new_n95_), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n89_), .c(new_n52_), .O(new_n97_));
  oai21  g63(.a(new_n52_), .b(x18), .c(new_n38_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x04), .O(new_n99_));
  nor2   g65(.a(new_n99_), .b(new_n97_), .O(z10));
  aoi21  g66(.a(x18), .b(new_n38_), .c(new_n67_), .O(z11));
  aoi21  g67(.a(x16), .b(new_n67_), .c(x03), .O(new_n102_));
  nor2   g68(.a(new_n102_), .b(new_n42_), .O(z12));
  nand2  g69(.a(new_n39_), .b(new_n49_), .O(z13));
  inv1   g70(.a(x17), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n49_), .c(new_n39_), .O(z14));
endmodule


