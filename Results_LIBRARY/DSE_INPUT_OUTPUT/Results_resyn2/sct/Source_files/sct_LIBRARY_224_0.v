// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n97_, new_n99_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(x18), .b(x12), .O(new_n37_));
  aoi21  g03(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x04), .O(new_n40_));
  nor2   g06(.a(new_n37_), .b(new_n40_), .O(z13));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  aoi22  g08(.a(new_n42_), .b(x18), .c(z13), .d(x05), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  inv1   g10(.a(x02), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n45_), .c(x16), .O(new_n46_));
  and2   g12(.a(new_n46_), .b(z13), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(z02));
  inv1   g15(.a(x07), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n44_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand2  g18(.a(new_n50_), .b(new_n44_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n52_), .c(new_n47_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(new_n37_), .O(new_n56_));
  nand2  g22(.a(new_n46_), .b(x04), .O(new_n57_));
  nand2  g23(.a(new_n51_), .b(x08), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n52_), .b(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n57_), .c(new_n56_), .O(z04));
  nand2  g28(.a(new_n58_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n47_), .O(z05));
  nand2  g32(.a(new_n65_), .b(x10), .O(new_n67_));
  nor2   g33(.a(new_n65_), .b(x10), .O(new_n68_));
  nor2   g34(.a(new_n68_), .b(new_n57_), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n67_), .c(new_n37_), .O(z06));
  oai21  g36(.a(new_n65_), .b(x10), .c(x11), .O(new_n71_));
  nor2   g37(.a(x09), .b(new_n59_), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n51_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n71_), .c(new_n47_), .O(z07));
  inv1   g41(.a(x18), .O(new_n76_));
  nor2   g42(.a(new_n76_), .b(x12), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n73_), .c(new_n72_), .d(new_n51_), .O(new_n78_));
  aoi21  g44(.a(new_n74_), .b(x12), .c(new_n57_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n37_), .c(new_n78_), .O(z08));
  inv1   g46(.a(x12), .O(new_n81_));
  oai21  g47(.a(new_n74_), .b(x13), .c(x18), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g49(.a(new_n78_), .b(x13), .c(new_n57_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n83_), .O(z09));
  inv1   g51(.a(x00), .O(new_n86_));
  nor2   g52(.a(x13), .b(x12), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n73_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  inv1   g55(.a(x03), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x02), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(x16), .c(new_n65_), .O(new_n92_));
  nor2   g58(.a(new_n46_), .b(new_n36_), .O(new_n93_));
  aoi21  g59(.a(new_n92_), .b(new_n89_), .c(new_n93_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n40_), .c(new_n56_), .O(z10));
  nand2  g61(.a(new_n56_), .b(new_n45_), .O(z11));
  aoi21  g62(.a(x16), .b(new_n45_), .c(x03), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n40_), .c(new_n56_), .O(z12));
  nand2  g64(.a(x17), .b(x04), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n56_), .O(z14));
endmodule


