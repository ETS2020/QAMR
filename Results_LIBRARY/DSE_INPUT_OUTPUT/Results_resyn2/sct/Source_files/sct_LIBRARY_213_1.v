// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n100_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x13), .O(new_n37_));
  nand2  g03(.a(x14), .b(new_n37_), .O(new_n38_));
  and2   g04(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  oai21  g05(.a(x14), .b(x01), .c(new_n39_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  oai21  g13(.a(new_n47_), .b(x06), .c(new_n38_), .O(z02));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x07), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand2  g18(.a(new_n50_), .b(new_n49_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n47_), .c(new_n38_), .O(z03));
  inv1   g21(.a(x03), .O(new_n56_));
  inv1   g22(.a(x16), .O(new_n57_));
  aoi21  g23(.a(new_n56_), .b(x02), .c(new_n57_), .O(new_n58_));
  nand2  g24(.a(new_n38_), .b(x04), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g26(.a(new_n51_), .b(x08), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n52_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  nand2  g31(.a(new_n61_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n60_), .O(z05));
  inv1   g35(.a(new_n38_), .O(new_n70_));
  nand2  g36(.a(new_n68_), .b(x10), .O(new_n71_));
  nor2   g37(.a(new_n68_), .b(x10), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n47_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n71_), .c(new_n70_), .O(z06));
  inv1   g40(.a(x10), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  inv1   g42(.a(new_n68_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  oai21  g44(.a(new_n68_), .b(x10), .c(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n60_), .O(z07));
  nand2  g46(.a(new_n78_), .b(x12), .O(new_n81_));
  nor2   g47(.a(x12), .b(x11), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n72_), .c(new_n47_), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n81_), .c(new_n70_), .O(z08));
  nand3  g50(.a(new_n82_), .b(new_n72_), .c(new_n37_), .O(new_n85_));
  nand2  g51(.a(new_n82_), .b(new_n72_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x13), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n85_), .c(new_n60_), .O(z09));
  inv1   g54(.a(x04), .O(new_n89_));
  oai21  g55(.a(new_n58_), .b(new_n37_), .c(x14), .O(new_n90_));
  inv1   g56(.a(x00), .O(new_n91_));
  nand3  g57(.a(new_n82_), .b(new_n37_), .c(new_n75_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n77_), .c(new_n46_), .O(new_n94_));
  aoi22  g60(.a(new_n94_), .b(new_n90_), .c(new_n38_), .d(new_n89_), .O(z10));
  nor2   g61(.a(new_n70_), .b(new_n35_), .O(z11));
  aoi21  g62(.a(x16), .b(new_n35_), .c(x03), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(new_n59_), .O(z12));
  nand2  g64(.a(new_n38_), .b(new_n89_), .O(z13));
  inv1   g65(.a(x17), .O(new_n100_));
  nor2   g66(.a(new_n59_), .b(new_n100_), .O(z14));
endmodule


