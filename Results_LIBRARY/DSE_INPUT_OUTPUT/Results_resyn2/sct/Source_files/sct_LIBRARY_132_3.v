// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n111_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(x17), .b(x08), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n37_), .c(new_n47_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xnor2a g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n49_), .c(new_n37_), .O(z03));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(x17), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(x08), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  inv1   g24(.a(new_n55_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(x08), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n50_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nor2   g28(.a(new_n55_), .b(x09), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x17), .c(x08), .O(new_n64_));
  inv1   g30(.a(new_n57_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n64_), .c(new_n50_), .O(z05));
  inv1   g34(.a(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(x07), .c(x06), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(x10), .c(new_n56_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x08), .O(new_n72_));
  oai21  g38(.a(new_n66_), .b(x09), .c(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n50_), .O(z06));
  nand2  g40(.a(new_n72_), .b(x11), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor3   g44(.a(new_n78_), .b(new_n70_), .c(new_n57_), .O(new_n79_));
  aoi21  g45(.a(new_n49_), .b(new_n37_), .c(new_n79_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n75_), .O(z07));
  nand2  g47(.a(new_n49_), .b(new_n37_), .O(new_n82_));
  inv1   g48(.a(x08), .O(new_n83_));
  nor2   g49(.a(x11), .b(x10), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n63_), .c(x17), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n83_), .c(x12), .O(new_n86_));
  nor3   g52(.a(x12), .b(x11), .c(x10), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n63_), .c(new_n65_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n86_), .c(new_n82_), .O(z08));
  aoi21  g55(.a(new_n87_), .b(new_n63_), .c(x17), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n83_), .c(x13), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  inv1   g58(.a(x13), .O(new_n93_));
  nand3  g59(.a(new_n79_), .b(new_n93_), .c(new_n92_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n91_), .c(new_n82_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand3  g62(.a(new_n84_), .b(new_n93_), .c(new_n92_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  inv1   g64(.a(x03), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x02), .O(new_n100_));
  nand4  g66(.a(new_n69_), .b(x08), .c(x07), .d(x06), .O(new_n101_));
  aoi21  g67(.a(new_n100_), .b(x16), .c(new_n101_), .O(new_n102_));
  inv1   g68(.a(x14), .O(new_n103_));
  nor2   g69(.a(new_n48_), .b(new_n103_), .O(new_n104_));
  aoi21  g70(.a(new_n102_), .b(new_n98_), .c(new_n104_), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n44_), .c(new_n37_), .O(z10));
  nor2   g72(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n36_), .c(x03), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n44_), .c(new_n37_), .O(z12));
  nor2   g75(.a(new_n38_), .b(new_n44_), .O(z13));
  nand3  g76(.a(x17), .b(new_n83_), .c(x04), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(z14));
endmodule


