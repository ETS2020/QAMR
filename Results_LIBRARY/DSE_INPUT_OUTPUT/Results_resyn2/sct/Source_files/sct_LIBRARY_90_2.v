// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n92_, new_n95_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nor2   g02(.a(x06), .b(new_n36_), .O(z02));
  inv1   g03(.a(z02), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  inv1   g08(.a(x15), .O(new_n43_));
  nand2  g09(.a(x18), .b(new_n43_), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(z02), .O(new_n45_));
  nand2  g11(.a(x06), .b(x04), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x05), .O(new_n47_));
  oai21  g13(.a(new_n45_), .b(x05), .c(new_n47_), .O(z01));
  inv1   g14(.a(x07), .O(new_n49_));
  inv1   g15(.a(x02), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n50_), .c(x16), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  aoi21  g18(.a(new_n52_), .b(x06), .c(new_n36_), .O(z03));
  nand2  g19(.a(x08), .b(x07), .O(new_n54_));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n49_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  aoi21  g23(.a(new_n57_), .b(x06), .c(new_n36_), .O(z04));
  inv1   g24(.a(x03), .O(new_n59_));
  inv1   g25(.a(x16), .O(new_n60_));
  aoi21  g26(.a(new_n59_), .b(x02), .c(new_n60_), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n46_), .O(new_n62_));
  nand2  g28(.a(new_n54_), .b(x09), .O(new_n63_));
  nor2   g29(.a(new_n54_), .b(x09), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(z05));
  inv1   g32(.a(x10), .O(new_n67_));
  nand2  g33(.a(new_n64_), .b(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n65_), .b(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n62_), .O(z06));
  nand2  g36(.a(new_n68_), .b(x11), .O(new_n71_));
  nor2   g37(.a(x11), .b(x10), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n64_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n62_), .O(z07));
  inv1   g40(.a(x12), .O(new_n75_));
  nand3  g41(.a(new_n72_), .b(new_n64_), .c(new_n75_), .O(new_n76_));
  nand2  g42(.a(new_n73_), .b(x12), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n62_), .O(z08));
  nand2  g44(.a(new_n76_), .b(x13), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  inv1   g46(.a(x13), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n75_), .c(new_n80_), .d(new_n67_), .O(new_n82_));
  inv1   g48(.a(new_n82_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n64_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n79_), .c(new_n62_), .O(z09));
  inv1   g51(.a(x00), .O(new_n86_));
  nand2  g52(.a(new_n82_), .b(new_n86_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n64_), .c(new_n51_), .O(new_n88_));
  nand2  g54(.a(new_n61_), .b(x14), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(new_n88_), .c(new_n46_), .O(z10));
  nand2  g56(.a(new_n38_), .b(new_n50_), .O(z11));
  nand2  g57(.a(x16), .b(new_n50_), .O(new_n92_));
  aoi21  g58(.a(new_n92_), .b(new_n59_), .c(new_n46_), .O(z12));
  inv1   g59(.a(new_n46_), .O(z13));
  nand2  g60(.a(z13), .b(x17), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(z14));
endmodule


