// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  aoi21  g03(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(new_n37_), .O(new_n40_));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(x04), .c(x06), .O(z02));
  aoi21  g15(.a(x06), .b(x04), .c(x07), .O(new_n50_));
  inv1   g16(.a(x02), .O(new_n51_));
  oai21  g17(.a(x03), .b(new_n51_), .c(x16), .O(new_n52_));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n50_), .c(new_n40_), .O(z03));
  inv1   g21(.a(x04), .O(new_n56_));
  nor2   g22(.a(new_n48_), .b(new_n56_), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(x06), .O(new_n63_));
  nor2   g29(.a(x09), .b(new_n59_), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(x07), .c(new_n56_), .O(new_n65_));
  aoi21  g31(.a(new_n58_), .b(x09), .c(new_n48_), .O(new_n66_));
  oai22  g32(.a(new_n66_), .b(new_n56_), .c(new_n65_), .d(new_n63_), .O(z05));
  inv1   g33(.a(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand4  g37(.a(new_n64_), .b(new_n71_), .c(x07), .d(x06), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n57_), .O(z06));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n64_), .c(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  aoi21  g43(.a(new_n72_), .b(x11), .c(new_n48_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n56_), .c(new_n77_), .O(z07));
  oai21  g45(.a(new_n75_), .b(x12), .c(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  inv1   g47(.a(new_n74_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n69_), .c(x12), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n52_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n81_), .O(z08));
  inv1   g52(.a(x12), .O(new_n87_));
  inv1   g53(.a(new_n69_), .O(new_n88_));
  nand3  g54(.a(new_n74_), .b(new_n88_), .c(new_n87_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x13), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand3  g57(.a(new_n74_), .b(new_n91_), .c(new_n87_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n88_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n90_), .c(new_n57_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n92_), .b(new_n96_), .O(new_n97_));
  nor2   g63(.a(new_n69_), .b(new_n48_), .O(new_n98_));
  nor2   g64(.a(new_n52_), .b(new_n36_), .O(new_n99_));
  aoi21  g65(.a(new_n98_), .b(new_n97_), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n56_), .c(new_n40_), .O(z10));
  nor2   g67(.a(new_n37_), .b(new_n51_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n51_), .c(x03), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n56_), .c(new_n40_), .O(z12));
  nand2  g70(.a(x06), .b(new_n56_), .O(z13));
  and2   g71(.a(x17), .b(x04), .O(z14));
endmodule


