// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n104_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x13), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x08), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n38_), .c(new_n48_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z02));
  xor2a  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n53_), .c(new_n38_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x02), .O(new_n59_));
  oai21  g25(.a(x03), .b(new_n59_), .c(x16), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n38_), .c(x04), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  inv1   g28(.a(x08), .O(new_n63_));
  nand2  g29(.a(x07), .b(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nor2   g32(.a(new_n66_), .b(new_n61_), .O(z04));
  inv1   g33(.a(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nand2  g35(.a(new_n62_), .b(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n69_), .c(new_n60_), .d(x04), .O(new_n71_));
  and2   g37(.a(new_n71_), .b(new_n38_), .O(z05));
  inv1   g38(.a(x10), .O(new_n73_));
  nor2   g39(.a(new_n62_), .b(x09), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g41(.a(new_n68_), .b(x07), .c(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x10), .O(new_n77_));
  oai21  g43(.a(x13), .b(x10), .c(new_n63_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n77_), .c(new_n75_), .d(new_n53_), .O(z06));
  oai22  g45(.a(new_n69_), .b(x10), .c(new_n37_), .d(x11), .O(new_n80_));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(new_n53_), .O(z07));
  inv1   g49(.a(x12), .O(new_n84_));
  nand3  g50(.a(new_n81_), .b(new_n74_), .c(new_n84_), .O(new_n85_));
  inv1   g51(.a(new_n81_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n76_), .c(x12), .O(new_n87_));
  oai21  g53(.a(x13), .b(x12), .c(new_n63_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n87_), .c(new_n85_), .d(new_n53_), .O(z08));
  nand2  g55(.a(new_n85_), .b(x13), .O(new_n90_));
  nand4  g56(.a(new_n81_), .b(new_n74_), .c(new_n36_), .d(new_n84_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n90_), .c(new_n53_), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  inv1   g59(.a(x11), .O(new_n94_));
  nand4  g60(.a(new_n36_), .b(new_n84_), .c(new_n94_), .d(new_n73_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n74_), .c(new_n60_), .O(new_n97_));
  nand3  g63(.a(new_n52_), .b(new_n38_), .c(x14), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n97_), .c(new_n49_), .O(z10));
  nor2   g65(.a(new_n37_), .b(new_n59_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n59_), .c(x03), .O(new_n101_));
  nor3   g67(.a(new_n101_), .b(new_n37_), .c(new_n49_), .O(z12));
  nand2  g68(.a(new_n38_), .b(new_n49_), .O(z13));
  inv1   g69(.a(x17), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n49_), .c(new_n38_), .O(z14));
endmodule


