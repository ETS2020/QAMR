// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n107_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  inv1   g03(.a(x15), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g05(.a(x14), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  oai21  g06(.a(new_n36_), .b(new_n35_), .c(new_n40_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  inv1   g09(.a(new_n39_), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  aoi21  g12(.a(new_n43_), .b(x18), .c(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  oai21  g19(.a(x15), .b(new_n37_), .c(new_n53_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n54_), .c(new_n50_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  aoi21  g24(.a(new_n55_), .b(new_n58_), .c(new_n39_), .O(new_n59_));
  oai21  g25(.a(new_n55_), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n49_), .O(z04));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(x09), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x15), .c(x06), .O(new_n64_));
  nand4  g30(.a(new_n38_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n50_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nand2  g34(.a(new_n63_), .b(new_n68_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n38_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  oai21  g37(.a(new_n65_), .b(x09), .c(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n50_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  nand3  g40(.a(new_n63_), .b(new_n74_), .c(new_n68_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n38_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  inv1   g43(.a(x09), .O(new_n78_));
  inv1   g44(.a(new_n65_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n68_), .c(new_n78_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n77_), .c(new_n50_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  nand4  g49(.a(new_n63_), .b(new_n83_), .c(new_n74_), .d(new_n68_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n38_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  oai21  g52(.a(new_n80_), .b(x11), .c(x12), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n50_), .O(z08));
  inv1   g54(.a(x13), .O(new_n89_));
  aoi21  g55(.a(new_n84_), .b(new_n38_), .c(new_n37_), .O(new_n90_));
  nor2   g56(.a(new_n65_), .b(x09), .O(new_n91_));
  nand4  g57(.a(new_n89_), .b(new_n83_), .c(new_n74_), .d(new_n68_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  aoi22  g59(.a(new_n93_), .b(new_n91_), .c(new_n49_), .d(new_n44_), .O(new_n94_));
  oai21  g60(.a(new_n90_), .b(new_n89_), .c(new_n94_), .O(z09));
  nand2  g61(.a(new_n44_), .b(x04), .O(new_n96_));
  inv1   g62(.a(new_n48_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x14), .O(new_n98_));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n92_), .b(new_n99_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n91_), .c(new_n48_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n98_), .c(new_n96_), .O(z10));
  nor2   g68(.a(new_n39_), .b(new_n36_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n36_), .c(x03), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n96_), .O(z12));
  inv1   g71(.a(new_n96_), .O(z13));
  nand2  g72(.a(z13), .b(x17), .O(new_n107_));
  inv1   g73(.a(new_n107_), .O(z14));
endmodule


