// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  aoi22  g03(.a(new_n37_), .b(x06), .c(x14), .d(new_n35_), .O(z00));
  nor2   g04(.a(x15), .b(x05), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x18), .O(new_n40_));
  inv1   g06(.a(x06), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x01), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  inv1   g11(.a(x04), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n41_), .c(new_n35_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  oai21  g18(.a(x03), .b(new_n36_), .c(x16), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x04), .O(new_n54_));
  xnor2a g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n54_), .c(new_n42_), .O(z03));
  aoi22  g22(.a(new_n42_), .b(x08), .c(x07), .d(x06), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(new_n59_));
  nor3   g25(.a(new_n59_), .b(new_n57_), .c(new_n54_), .O(z04));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(new_n50_), .O(new_n65_));
  and2   g31(.a(new_n65_), .b(new_n42_), .O(z05));
  inv1   g32(.a(new_n42_), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  inv1   g34(.a(new_n64_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n63_), .O(new_n70_));
  aoi21  g36(.a(new_n64_), .b(x10), .c(new_n54_), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n70_), .c(new_n67_), .O(z06));
  inv1   g38(.a(x11), .O(new_n73_));
  nand4  g39(.a(new_n59_), .b(new_n73_), .c(new_n68_), .d(new_n63_), .O(new_n74_));
  nand2  g40(.a(new_n68_), .b(new_n63_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n58_), .c(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n50_), .O(new_n77_));
  and2   g43(.a(new_n77_), .b(new_n42_), .O(z07));
  nand2  g44(.a(x08), .b(x07), .O(new_n79_));
  nor2   g45(.a(new_n79_), .b(x09), .O(new_n80_));
  nor3   g46(.a(x12), .b(x11), .c(x10), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(x06), .O(new_n82_));
  nor2   g48(.a(x11), .b(x10), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n69_), .O(new_n84_));
  oai21  g50(.a(new_n67_), .b(x12), .c(new_n84_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n82_), .c(new_n50_), .O(z08));
  inv1   g52(.a(x13), .O(new_n87_));
  aoi21  g53(.a(new_n81_), .b(new_n80_), .c(new_n87_), .O(new_n88_));
  nand3  g54(.a(new_n63_), .b(x08), .c(x07), .O(new_n89_));
  nor2   g55(.a(x13), .b(x12), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n83_), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n88_), .c(x06), .O(new_n93_));
  nor2   g59(.a(new_n87_), .b(x06), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n54_), .c(new_n42_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n93_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n91_), .b(new_n97_), .O(new_n98_));
  nor2   g64(.a(new_n64_), .b(new_n49_), .O(new_n99_));
  inv1   g65(.a(x14), .O(new_n100_));
  nor2   g66(.a(new_n53_), .b(new_n100_), .O(new_n101_));
  aoi21  g67(.a(new_n99_), .b(new_n98_), .c(new_n101_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n46_), .c(new_n42_), .O(z10));
  nor2   g69(.a(new_n67_), .b(new_n36_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n36_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n46_), .c(new_n42_), .O(z12));
  nor2   g72(.a(new_n67_), .b(new_n46_), .O(z13));
  nand2  g73(.a(x17), .b(x04), .O(new_n108_));
  nand2  g74(.a(new_n108_), .b(new_n42_), .O(z14));
endmodule


