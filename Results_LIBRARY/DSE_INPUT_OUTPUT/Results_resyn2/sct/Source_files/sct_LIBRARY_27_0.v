// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n93_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x04), .O(new_n41_));
  inv1   g07(.a(x06), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x05), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x04), .c(x06), .O(z02));
  inv1   g16(.a(new_n37_), .O(new_n51_));
  oai21  g17(.a(x03), .b(new_n36_), .c(x16), .O(new_n52_));
  oai21  g18(.a(new_n42_), .b(x04), .c(new_n52_), .O(new_n53_));
  xnor2a g19(.a(x07), .b(x06), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(z03));
  nand3  g21(.a(x08), .b(x07), .c(x06), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  aoi21  g23(.a(x07), .b(x06), .c(x08), .O(new_n58_));
  or2    g24(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n53_), .c(new_n51_), .O(z04));
  nor2   g26(.a(new_n49_), .b(new_n41_), .O(new_n61_));
  nand2  g27(.a(new_n56_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(z05));
  nor2   g31(.a(x10), .b(x09), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n57_), .O(new_n67_));
  nand2  g33(.a(new_n64_), .b(x10), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(new_n61_), .O(z06));
  inv1   g35(.a(x11), .O(new_n70_));
  nand3  g36(.a(new_n66_), .b(new_n57_), .c(new_n70_), .O(new_n71_));
  nand2  g37(.a(new_n67_), .b(x11), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n61_), .O(z07));
  nand2  g39(.a(new_n71_), .b(x12), .O(new_n74_));
  inv1   g40(.a(x12), .O(new_n75_));
  nand4  g41(.a(new_n66_), .b(new_n57_), .c(new_n75_), .d(new_n70_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n61_), .O(z08));
  nor2   g43(.a(x13), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n75_), .c(new_n70_), .O(new_n79_));
  nand3  g45(.a(new_n63_), .b(x08), .c(x07), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n79_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  aoi21  g48(.a(new_n76_), .b(x13), .c(new_n49_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n41_), .c(new_n82_), .O(z09));
  inv1   g50(.a(x00), .O(new_n85_));
  nand2  g51(.a(new_n79_), .b(new_n85_), .O(new_n86_));
  nor2   g52(.a(new_n64_), .b(new_n49_), .O(new_n87_));
  inv1   g53(.a(x14), .O(new_n88_));
  nor2   g54(.a(new_n52_), .b(new_n88_), .O(new_n89_));
  aoi21  g55(.a(new_n87_), .b(new_n86_), .c(new_n89_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n41_), .c(new_n51_), .O(z10));
  nor2   g57(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g58(.a(x16), .b(new_n36_), .c(x03), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n41_), .c(new_n51_), .O(z12));
  and2   g60(.a(x17), .b(x04), .O(z14));
  buf    g61(.a(x04), .O(z13));
endmodule


