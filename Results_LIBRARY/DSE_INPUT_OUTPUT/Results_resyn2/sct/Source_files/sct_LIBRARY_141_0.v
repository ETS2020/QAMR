// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n105_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x12), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n36_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x12), .c(x06), .O(z02));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x12), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nor2   g19(.a(x07), .b(x06), .O(new_n54_));
  nor3   g20(.a(new_n54_), .b(new_n53_), .c(new_n47_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  aoi21  g22(.a(new_n52_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  aoi21  g23(.a(x07), .b(x06), .c(x08), .O(new_n58_));
  nor3   g24(.a(new_n58_), .b(new_n57_), .c(new_n47_), .O(z04));
  inv1   g25(.a(x09), .O(new_n60_));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand2  g29(.a(new_n61_), .b(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n63_), .c(new_n46_), .d(x04), .O(new_n65_));
  and2   g31(.a(new_n65_), .b(new_n51_), .O(z05));
  inv1   g32(.a(x10), .O(new_n67_));
  nor2   g33(.a(new_n61_), .b(x09), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  oai21  g35(.a(new_n61_), .b(x09), .c(x10), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n46_), .c(x04), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  aoi21  g38(.a(new_n72_), .b(new_n69_), .c(new_n37_), .O(z06));
  nand2  g39(.a(x08), .b(x07), .O(new_n74_));
  nor2   g40(.a(new_n74_), .b(x09), .O(new_n75_));
  nand4  g41(.a(new_n67_), .b(new_n60_), .c(x08), .d(x07), .O(new_n76_));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  aoi22  g43(.a(new_n77_), .b(new_n75_), .c(new_n76_), .d(x11), .O(new_n78_));
  nand2  g44(.a(x11), .b(new_n49_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n46_), .c(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n51_), .O(new_n81_));
  oai21  g47(.a(new_n78_), .b(new_n49_), .c(new_n81_), .O(z07));
  and2   g48(.a(x08), .b(x07), .O(new_n83_));
  nand4  g49(.a(new_n77_), .b(new_n83_), .c(new_n50_), .d(new_n60_), .O(new_n84_));
  nand2  g50(.a(new_n77_), .b(new_n75_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x12), .O(new_n86_));
  nor2   g52(.a(new_n49_), .b(new_n43_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n86_), .c(new_n84_), .d(new_n46_), .O(z08));
  inv1   g54(.a(new_n47_), .O(new_n89_));
  xor2a  g55(.a(new_n84_), .b(x13), .O(new_n90_));
  oai21  g56(.a(x13), .b(new_n50_), .c(new_n49_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nor2   g59(.a(x13), .b(x12), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n77_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n68_), .c(new_n46_), .O(new_n97_));
  inv1   g63(.a(new_n46_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n51_), .c(x14), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n97_), .c(new_n43_), .O(z10));
  nor2   g66(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g67(.a(x16), .b(new_n36_), .c(x03), .O(new_n102_));
  nor3   g68(.a(new_n102_), .b(new_n37_), .c(new_n43_), .O(z12));
  nor2   g69(.a(new_n37_), .b(new_n43_), .O(z13));
  nand2  g70(.a(x17), .b(x04), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n51_), .O(z14));
endmodule


