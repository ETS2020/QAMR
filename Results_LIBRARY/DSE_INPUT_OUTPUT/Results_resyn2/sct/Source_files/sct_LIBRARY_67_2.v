// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n97_, new_n99_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x13), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x06), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x15), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n43_), .c(new_n38_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  oai21  g14(.a(new_n48_), .b(x06), .c(new_n38_), .O(z02));
  xnor2a g15(.a(x07), .b(x06), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n48_), .c(new_n38_), .O(z03));
  aoi21  g17(.a(x07), .b(x06), .c(x08), .O(new_n52_));
  nand3  g18(.a(x08), .b(x07), .c(x06), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  or2    g20(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n48_), .c(new_n38_), .O(z04));
  inv1   g22(.a(x09), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(x08), .c(x07), .d(x06), .O(new_n58_));
  nand2  g24(.a(new_n53_), .b(x09), .O(new_n59_));
  nand4  g25(.a(new_n59_), .b(new_n58_), .c(new_n47_), .d(x04), .O(new_n60_));
  and2   g26(.a(new_n60_), .b(new_n38_), .O(z05));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(x09), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n37_), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x10), .O(new_n65_));
  nand2  g31(.a(new_n48_), .b(new_n38_), .O(new_n66_));
  nor2   g32(.a(x10), .b(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n54_), .c(x13), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(z06));
  inv1   g35(.a(x06), .O(new_n70_));
  and2   g36(.a(x08), .b(x07), .O(new_n71_));
  aoi21  g37(.a(new_n67_), .b(new_n71_), .c(new_n37_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n70_), .c(x11), .O(new_n73_));
  inv1   g39(.a(x11), .O(new_n74_));
  nand4  g40(.a(new_n67_), .b(new_n54_), .c(x13), .d(new_n74_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n66_), .O(z07));
  nand3  g42(.a(new_n57_), .b(x08), .c(x07), .O(new_n77_));
  inv1   g43(.a(x10), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n74_), .c(new_n78_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n77_), .c(x13), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  aoi21  g48(.a(new_n75_), .b(x12), .c(new_n48_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n82_), .O(z08));
  nand2  g50(.a(new_n38_), .b(x04), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(z13));
  nand2  g52(.a(x13), .b(new_n70_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(z13), .c(new_n81_), .d(new_n47_), .O(z09));
  inv1   g54(.a(x03), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x02), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n38_), .c(x16), .d(x14), .O(new_n91_));
  nand3  g57(.a(x13), .b(x06), .c(x00), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n63_), .c(new_n47_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n91_), .c(new_n44_), .O(z10));
  aoi21  g61(.a(new_n37_), .b(x06), .c(new_n35_), .O(z11));
  aoi21  g62(.a(x16), .b(new_n35_), .c(x03), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(new_n85_), .O(z12));
  inv1   g64(.a(x17), .O(new_n99_));
  nor2   g65(.a(new_n85_), .b(new_n99_), .O(z14));
endmodule


