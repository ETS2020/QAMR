// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n93_, new_n95_, new_n97_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x18), .b(x17), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  nand2  g05(.a(new_n37_), .b(x04), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x17), .O(new_n44_));
  aoi22  g10(.a(new_n44_), .b(new_n42_), .c(new_n41_), .d(x05), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n40_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  oai21  g18(.a(x03), .b(new_n35_), .c(x16), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x04), .O(new_n54_));
  xnor2a g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n54_), .c(new_n37_), .O(z03));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  xor2a  g23(.a(new_n57_), .b(x08), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n54_), .c(new_n37_), .O(z04));
  inv1   g25(.a(x08), .O(new_n60_));
  oai21  g26(.a(new_n57_), .b(new_n60_), .c(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(new_n61_), .c(new_n53_), .d(x04), .O(new_n64_));
  and2   g30(.a(new_n64_), .b(new_n37_), .O(z05));
  inv1   g31(.a(new_n37_), .O(new_n66_));
  nand2  g32(.a(new_n63_), .b(x10), .O(new_n67_));
  nor2   g33(.a(new_n63_), .b(x10), .O(new_n68_));
  nor2   g34(.a(new_n68_), .b(new_n54_), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(z06));
  oai21  g36(.a(new_n63_), .b(x10), .c(x11), .O(new_n71_));
  inv1   g37(.a(new_n63_), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n72_), .c(new_n54_), .O(new_n74_));
  aoi21  g40(.a(new_n74_), .b(new_n71_), .c(new_n66_), .O(z07));
  nand2  g41(.a(new_n73_), .b(new_n72_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x12), .O(new_n77_));
  inv1   g43(.a(x12), .O(new_n78_));
  nand3  g44(.a(new_n73_), .b(new_n72_), .c(new_n78_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n50_), .O(z08));
  nand2  g46(.a(new_n79_), .b(x13), .O(new_n81_));
  inv1   g47(.a(x13), .O(new_n82_));
  nand3  g48(.a(new_n73_), .b(new_n82_), .c(new_n78_), .O(new_n83_));
  inv1   g49(.a(new_n83_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n72_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n81_), .c(new_n50_), .O(z09));
  inv1   g52(.a(x00), .O(new_n87_));
  nand2  g53(.a(new_n83_), .b(new_n87_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n72_), .c(new_n53_), .O(new_n89_));
  nand2  g55(.a(new_n49_), .b(x14), .O(new_n90_));
  aoi21  g56(.a(new_n90_), .b(new_n89_), .c(new_n40_), .O(z10));
  nand2  g57(.a(new_n37_), .b(new_n35_), .O(z11));
  nand2  g58(.a(x16), .b(new_n35_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(new_n47_), .c(new_n40_), .O(z12));
  inv1   g60(.a(x04), .O(new_n95_));
  nand2  g61(.a(new_n37_), .b(new_n95_), .O(z13));
  inv1   g62(.a(x17), .O(new_n97_));
  aoi21  g63(.a(new_n43_), .b(new_n95_), .c(new_n97_), .O(z14));
endmodule


