// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x05), .O(new_n37_));
  nor2   g03(.a(x06), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x15), .O(new_n42_));
  nand3  g08(.a(x18), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  inv1   g09(.a(x06), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x04), .c(x05), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x16), .c(new_n48_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n44_), .c(new_n37_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n44_), .O(new_n55_));
  oai21  g21(.a(x06), .b(x05), .c(x07), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n51_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  and2   g24(.a(x07), .b(x06), .O(new_n59_));
  aoi21  g25(.a(new_n39_), .b(x08), .c(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n59_), .b(x08), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n51_), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n60_), .O(z04));
  oai21  g29(.a(new_n38_), .b(x09), .c(new_n61_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n51_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nand4  g34(.a(new_n59_), .b(new_n68_), .c(new_n65_), .d(x08), .O(new_n69_));
  nand2  g35(.a(new_n66_), .b(x10), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n51_), .O(new_n71_));
  and2   g37(.a(new_n71_), .b(new_n39_), .O(z06));
  nand3  g38(.a(new_n65_), .b(x08), .c(x07), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(x10), .c(x11), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  inv1   g41(.a(new_n66_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n68_), .O(new_n77_));
  oai21  g43(.a(x11), .b(x05), .c(new_n44_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n77_), .c(new_n74_), .d(new_n51_), .O(z07));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n65_), .c(x08), .d(x07), .O(new_n81_));
  xor2a  g47(.a(new_n81_), .b(x12), .O(new_n82_));
  oai21  g48(.a(x03), .b(new_n35_), .c(x16), .O(new_n83_));
  nand2  g49(.a(x12), .b(new_n44_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n39_), .O(new_n86_));
  oai21  g52(.a(new_n82_), .b(new_n44_), .c(new_n86_), .O(z08));
  oai21  g53(.a(new_n81_), .b(x12), .c(x13), .O(new_n88_));
  nor2   g54(.a(x13), .b(x12), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n76_), .c(new_n75_), .d(new_n68_), .O(new_n90_));
  oai21  g56(.a(x13), .b(x05), .c(new_n44_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n88_), .d(new_n51_), .O(z09));
  nor2   g58(.a(new_n38_), .b(x04), .O(new_n93_));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n89_), .b(new_n80_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n76_), .c(new_n83_), .O(new_n97_));
  inv1   g63(.a(x16), .O(new_n98_));
  aoi21  g64(.a(new_n49_), .b(x02), .c(new_n98_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(x14), .c(new_n38_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n97_), .c(new_n93_), .O(z10));
  nor2   g67(.a(new_n38_), .b(new_n35_), .O(z11));
  nand2  g68(.a(x16), .b(new_n35_), .O(new_n103_));
  nor2   g69(.a(new_n38_), .b(x03), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n103_), .c(new_n93_), .O(z12));
  inv1   g71(.a(new_n93_), .O(z13));
  inv1   g72(.a(x17), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(new_n48_), .c(new_n39_), .O(z14));
endmodule


