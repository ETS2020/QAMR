// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n100_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x18), .b(x09), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x04), .O(new_n41_));
  nor2   g07(.a(new_n37_), .b(new_n41_), .O(z13));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  aoi22  g09(.a(new_n43_), .b(x18), .c(z13), .d(x05), .O(z01));
  inv1   g10(.a(new_n37_), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n36_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  oai21  g13(.a(new_n47_), .b(x06), .c(new_n45_), .O(z02));
  xnor2a g14(.a(x07), .b(x06), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(new_n47_), .c(new_n45_), .O(z03));
  nand3  g16(.a(x08), .b(x07), .c(x06), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  aoi21  g18(.a(x07), .b(x06), .c(x08), .O(new_n53_));
  or2    g19(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n47_), .c(new_n45_), .O(z04));
  nand2  g21(.a(new_n47_), .b(new_n45_), .O(new_n56_));
  inv1   g22(.a(x18), .O(new_n57_));
  nor2   g23(.a(new_n57_), .b(x09), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n52_), .O(new_n59_));
  nand2  g25(.a(new_n51_), .b(x09), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(z05));
  inv1   g27(.a(x10), .O(new_n62_));
  aoi21  g28(.a(new_n52_), .b(new_n62_), .c(new_n57_), .O(new_n63_));
  aoi21  g29(.a(new_n59_), .b(x10), .c(new_n47_), .O(new_n64_));
  oai21  g30(.a(new_n63_), .b(x09), .c(new_n64_), .O(z06));
  inv1   g31(.a(new_n47_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  inv1   g33(.a(x11), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n62_), .O(new_n69_));
  nor2   g35(.a(new_n69_), .b(new_n51_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n57_), .c(new_n67_), .O(new_n71_));
  nand3  g37(.a(new_n58_), .b(new_n52_), .c(new_n62_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x11), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n66_), .O(z07));
  nand3  g40(.a(new_n52_), .b(new_n68_), .c(new_n62_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(x12), .c(x18), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n67_), .O(new_n77_));
  nand2  g43(.a(new_n70_), .b(new_n58_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x12), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n66_), .O(z08));
  inv1   g46(.a(x12), .O(new_n81_));
  inv1   g47(.a(x13), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n75_), .c(x18), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  nand2  g51(.a(new_n58_), .b(new_n81_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n75_), .c(x13), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n85_), .c(new_n66_), .O(z09));
  inv1   g54(.a(x00), .O(new_n89_));
  oai21  g55(.a(new_n83_), .b(new_n69_), .c(new_n89_), .O(new_n90_));
  inv1   g56(.a(x03), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x02), .O(new_n92_));
  nand4  g58(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n93_));
  aoi21  g59(.a(new_n92_), .b(x16), .c(new_n93_), .O(new_n94_));
  inv1   g60(.a(x14), .O(new_n95_));
  nor2   g61(.a(new_n46_), .b(new_n95_), .O(new_n96_));
  aoi21  g62(.a(new_n94_), .b(new_n90_), .c(new_n96_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n41_), .c(new_n45_), .O(z10));
  nand2  g64(.a(new_n45_), .b(new_n36_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n36_), .c(x03), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n41_), .c(new_n45_), .O(z12));
  and2   g67(.a(z13), .b(x17), .O(z14));
endmodule


