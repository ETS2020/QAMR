// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  inv1   g02(.a(x14), .O(new_n37_));
  nand3  g03(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  nor2   g05(.a(x15), .b(x05), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x18), .O(new_n41_));
  nor2   g07(.a(new_n36_), .b(x01), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n36_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n36_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n49_), .c(new_n43_), .O(z03));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n49_), .c(new_n43_), .O(z04));
  nand2  g28(.a(new_n58_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n63_), .c(new_n50_), .d(new_n43_), .O(z05));
  oai21  g32(.a(x10), .b(x06), .c(new_n35_), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n64_), .c(x08), .d(x07), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  nand2  g37(.a(new_n65_), .b(x10), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n71_), .c(new_n67_), .d(new_n50_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  oai21  g40(.a(new_n69_), .b(new_n36_), .c(new_n74_), .O(new_n75_));
  nand2  g41(.a(new_n70_), .b(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(x01), .O(new_n77_));
  nand2  g43(.a(x11), .b(new_n36_), .O(new_n78_));
  nand2  g44(.a(new_n49_), .b(new_n43_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(z07));
  oai21  g46(.a(new_n69_), .b(x11), .c(x12), .O(new_n81_));
  nor2   g47(.a(x09), .b(new_n59_), .O(new_n82_));
  nor2   g48(.a(x10), .b(new_n54_), .O(new_n83_));
  nor2   g49(.a(x12), .b(x11), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(x06), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n81_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x01), .O(new_n87_));
  nand2  g53(.a(x12), .b(new_n36_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n79_), .O(z08));
  nand3  g55(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(new_n90_));
  nor2   g56(.a(x13), .b(x10), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n84_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n65_), .O(new_n93_));
  aoi21  g59(.a(new_n90_), .b(x13), .c(new_n93_), .O(new_n94_));
  aoi22  g60(.a(new_n49_), .b(new_n43_), .c(x13), .d(new_n36_), .O(new_n95_));
  oai21  g61(.a(new_n94_), .b(new_n35_), .c(new_n95_), .O(z09));
  inv1   g62(.a(x04), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  inv1   g65(.a(x03), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x02), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x16), .c(new_n65_), .O(new_n102_));
  nor2   g68(.a(new_n48_), .b(new_n37_), .O(new_n103_));
  aoi21  g69(.a(new_n102_), .b(new_n99_), .c(new_n103_), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n97_), .c(new_n43_), .O(z10));
  nor2   g71(.a(new_n42_), .b(new_n47_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n47_), .c(x03), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(new_n97_), .c(new_n43_), .O(z12));
  nor2   g74(.a(new_n42_), .b(new_n97_), .O(z13));
  and2   g75(.a(z13), .b(x17), .O(z14));
endmodule


