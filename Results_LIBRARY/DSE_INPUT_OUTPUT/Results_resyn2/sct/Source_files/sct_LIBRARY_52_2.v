// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n112_, new_n115_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(x06), .b(new_n35_), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  nor2   g04(.a(x15), .b(x05), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x18), .O(new_n40_));
  inv1   g06(.a(x06), .O(new_n41_));
  nor2   g07(.a(x14), .b(new_n41_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n40_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  nor2   g12(.a(new_n36_), .b(new_n41_), .O(new_n47_));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n41_), .c(new_n47_), .O(z02));
  inv1   g17(.a(new_n50_), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  oai21  g19(.a(new_n36_), .b(new_n41_), .c(new_n53_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n54_), .c(new_n52_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n55_), .b(new_n58_), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n59_), .c(new_n43_), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n50_), .O(z04));
  nor2   g28(.a(new_n58_), .b(new_n53_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n36_), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  aoi21  g31(.a(new_n49_), .b(x04), .c(new_n42_), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nor2   g35(.a(new_n69_), .b(new_n36_), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n67_), .c(new_n65_), .O(z05));
  nor2   g38(.a(x10), .b(x09), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n63_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x14), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand2  g42(.a(new_n71_), .b(x10), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n52_), .O(z06));
  aoi21  g44(.a(new_n73_), .b(new_n63_), .c(new_n36_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n41_), .c(x11), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nand4  g47(.a(new_n73_), .b(new_n63_), .c(new_n47_), .d(new_n81_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(new_n67_), .O(z07));
  nand4  g49(.a(new_n73_), .b(new_n81_), .c(x08), .d(x07), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x14), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x12), .O(new_n87_));
  nor3   g53(.a(x12), .b(x11), .c(x10), .O(new_n88_));
  aoi21  g54(.a(new_n88_), .b(new_n70_), .c(new_n66_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n87_), .O(z08));
  nor2   g56(.a(x12), .b(x11), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n73_), .c(x08), .d(x07), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x14), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x06), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x13), .O(new_n95_));
  inv1   g61(.a(x10), .O(new_n96_));
  inv1   g62(.a(x13), .O(new_n97_));
  nand3  g63(.a(new_n91_), .b(new_n97_), .c(new_n96_), .O(new_n98_));
  inv1   g64(.a(new_n98_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n70_), .c(new_n66_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n95_), .O(z09));
  nor2   g67(.a(new_n42_), .b(x04), .O(new_n102_));
  inv1   g68(.a(new_n69_), .O(new_n103_));
  oai21  g69(.a(new_n99_), .b(x00), .c(new_n103_), .O(new_n104_));
  inv1   g70(.a(x03), .O(new_n105_));
  inv1   g71(.a(x16), .O(new_n106_));
  aoi21  g72(.a(new_n105_), .b(x02), .c(new_n106_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(x14), .c(new_n42_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n104_), .c(new_n102_), .O(z10));
  nand2  g75(.a(new_n43_), .b(new_n48_), .O(z11));
  nand2  g76(.a(x16), .b(new_n48_), .O(new_n111_));
  nor2   g77(.a(new_n42_), .b(x03), .O(new_n112_));
  aoi21  g78(.a(new_n112_), .b(new_n111_), .c(new_n102_), .O(z12));
  inv1   g79(.a(new_n102_), .O(z13));
  nand2  g80(.a(x17), .b(x04), .O(new_n115_));
  nor2   g81(.a(new_n115_), .b(new_n42_), .O(z14));
endmodule


