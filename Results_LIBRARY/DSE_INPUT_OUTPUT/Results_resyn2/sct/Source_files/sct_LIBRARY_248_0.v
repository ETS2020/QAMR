// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n104_, new_n107_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand3  g02(.a(new_n36_), .b(x06), .c(new_n35_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  inv1   g04(.a(x18), .O(new_n39_));
  nor2   g05(.a(new_n39_), .b(x15), .O(new_n40_));
  inv1   g06(.a(x04), .O(new_n41_));
  nor2   g07(.a(x06), .b(x01), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n41_), .c(new_n42_), .O(new_n43_));
  oai21  g09(.a(new_n40_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x02), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n46_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n45_), .c(x01), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n45_), .O(new_n53_));
  oai21  g19(.a(x06), .b(new_n35_), .c(x07), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(new_n49_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(new_n42_), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  oai21  g25(.a(new_n52_), .b(new_n45_), .c(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n48_), .c(new_n57_), .O(z04));
  oai21  g28(.a(new_n42_), .b(x09), .c(new_n58_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n49_), .O(z05));
  oai21  g32(.a(new_n42_), .b(x10), .c(new_n65_), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  inv1   g34(.a(new_n65_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n67_), .c(new_n49_), .O(z06));
  nand3  g37(.a(new_n64_), .b(x08), .c(x07), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(x10), .c(x11), .O(new_n73_));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n64_), .c(x08), .d(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  nor2   g44(.a(new_n78_), .b(x06), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n48_), .c(new_n57_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n77_), .O(z07));
  xor2a  g47(.a(new_n75_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nor2   g49(.a(new_n83_), .b(x06), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n48_), .c(new_n57_), .O(new_n85_));
  oai21  g51(.a(new_n82_), .b(new_n45_), .c(new_n85_), .O(z08));
  oai21  g52(.a(new_n75_), .b(x12), .c(x13), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand3  g54(.a(new_n74_), .b(new_n88_), .c(new_n83_), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n69_), .O(new_n91_));
  oai21  g57(.a(x13), .b(new_n35_), .c(new_n45_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n91_), .c(new_n87_), .d(new_n49_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  inv1   g61(.a(x03), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x02), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(x16), .c(new_n65_), .O(new_n98_));
  nor2   g64(.a(new_n47_), .b(new_n36_), .O(new_n99_));
  aoi21  g65(.a(new_n98_), .b(new_n95_), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n41_), .c(new_n57_), .O(z10));
  nor2   g67(.a(new_n42_), .b(new_n46_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n46_), .c(x03), .O(new_n103_));
  nand2  g69(.a(new_n57_), .b(x04), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n103_), .O(z12));
  inv1   g71(.a(new_n104_), .O(z13));
  nand2  g72(.a(x17), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n57_), .O(z14));
endmodule


