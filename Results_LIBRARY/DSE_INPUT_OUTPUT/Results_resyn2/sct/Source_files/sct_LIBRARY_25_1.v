// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand2  g02(.a(x06), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  oai21  g07(.a(x06), .b(x05), .c(new_n36_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g11(.a(x02), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n46_), .c(x16), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  inv1   g14(.a(x06), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n48_), .c(new_n37_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n50_), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  nor2   g23(.a(new_n52_), .b(new_n49_), .O(new_n58_));
  nor2   g24(.a(x06), .b(x04), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  oai21  g27(.a(new_n58_), .b(x08), .c(new_n47_), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(new_n37_), .O(z04));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nor2   g33(.a(new_n48_), .b(new_n36_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(new_n65_), .O(z05));
  nand2  g35(.a(new_n67_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  inv1   g37(.a(new_n67_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n70_), .c(new_n68_), .O(z06));
  nand2  g40(.a(new_n73_), .b(x11), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(new_n68_), .O(z07));
  oai21  g44(.a(x12), .b(new_n36_), .c(new_n49_), .O(new_n79_));
  inv1   g45(.a(x12), .O(new_n80_));
  nand2  g46(.a(x08), .b(x07), .O(new_n81_));
  nor2   g47(.a(new_n81_), .b(x09), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(new_n76_), .c(new_n80_), .O(new_n83_));
  nor2   g49(.a(x12), .b(x09), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n76_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n64_), .c(new_n47_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n83_), .c(x04), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n79_), .O(z08));
  oai21  g54(.a(x13), .b(new_n36_), .c(new_n49_), .O(new_n89_));
  nand4  g55(.a(new_n84_), .b(new_n76_), .c(x08), .d(x07), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x13), .O(new_n91_));
  nor2   g57(.a(x09), .b(new_n57_), .O(new_n92_));
  nor2   g58(.a(x13), .b(x12), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n76_), .c(new_n92_), .d(new_n58_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n91_), .c(new_n47_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x04), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n89_), .O(z09));
  nand2  g63(.a(new_n48_), .b(x14), .O(new_n98_));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n93_), .b(new_n76_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n72_), .c(new_n47_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n98_), .c(new_n36_), .O(z10));
  aoi21  g69(.a(x06), .b(new_n36_), .c(new_n46_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n46_), .c(x03), .O(new_n105_));
  nor2   g71(.a(new_n105_), .b(new_n36_), .O(z12));
  inv1   g72(.a(new_n59_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(x04), .c(new_n59_), .O(z14));
endmodule


