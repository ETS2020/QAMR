// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x13), .b(x09), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  inv1   g08(.a(new_n37_), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n37_), .b(x04), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n54_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n51_), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  nor2   g29(.a(new_n59_), .b(x09), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n59_), .b(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n65_), .c(new_n51_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nand2  g34(.a(new_n64_), .b(new_n68_), .O(new_n69_));
  oai21  g35(.a(x03), .b(new_n35_), .c(x16), .O(new_n70_));
  oai21  g36(.a(new_n59_), .b(x09), .c(x10), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(x04), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n69_), .c(new_n43_), .O(z06));
  inv1   g40(.a(x09), .O(new_n75_));
  oai21  g41(.a(new_n59_), .b(x10), .c(x11), .O(new_n76_));
  inv1   g42(.a(new_n59_), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n75_), .O(new_n81_));
  nand2  g47(.a(new_n70_), .b(x04), .O(new_n82_));
  and2   g48(.a(x11), .b(x09), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n82_), .c(new_n37_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n81_), .O(z07));
  inv1   g51(.a(new_n82_), .O(new_n86_));
  nand2  g52(.a(new_n79_), .b(x12), .O(new_n87_));
  inv1   g53(.a(x12), .O(new_n88_));
  nand3  g54(.a(new_n78_), .b(new_n64_), .c(new_n88_), .O(new_n89_));
  oai21  g55(.a(x13), .b(x12), .c(x09), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n89_), .c(new_n87_), .d(new_n86_), .O(z08));
  inv1   g57(.a(x13), .O(new_n92_));
  nand4  g58(.a(new_n78_), .b(new_n64_), .c(new_n92_), .d(new_n88_), .O(new_n93_));
  nand2  g59(.a(new_n89_), .b(x13), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n86_), .O(z09));
  nand2  g61(.a(new_n49_), .b(x14), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand3  g63(.a(new_n78_), .b(new_n92_), .c(new_n88_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n64_), .c(new_n70_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n96_), .c(new_n50_), .O(z10));
  nor2   g67(.a(new_n43_), .b(new_n35_), .O(z11));
  nand2  g68(.a(x16), .b(new_n35_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n47_), .c(new_n50_), .O(z12));
  inv1   g70(.a(new_n50_), .O(z13));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n37_), .O(z14));
endmodule


