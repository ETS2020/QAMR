// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n109_, new_n112_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x06), .b(x05), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x15), .O(new_n42_));
  nand3  g08(.a(x18), .b(new_n42_), .c(x06), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(new_n41_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x04), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n46_), .c(x05), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(new_n37_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(x07), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n46_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n55_), .c(new_n51_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n54_), .O(z03));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n51_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n54_), .O(z04));
  oai21  g30(.a(x03), .b(new_n36_), .c(x16), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand2  g33(.a(new_n60_), .b(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n67_), .c(new_n65_), .d(x04), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(new_n54_), .O(z05));
  inv1   g36(.a(x10), .O(new_n71_));
  inv1   g37(.a(new_n67_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g39(.a(x08), .b(x07), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(x09), .c(x10), .O(new_n75_));
  oai21  g41(.a(x10), .b(new_n41_), .c(new_n46_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n75_), .c(new_n73_), .d(new_n51_), .O(z06));
  nor2   g43(.a(new_n74_), .b(x09), .O(new_n78_));
  nand4  g44(.a(new_n71_), .b(new_n66_), .c(x08), .d(x07), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  aoi22  g46(.a(new_n80_), .b(new_n78_), .c(new_n79_), .d(x11), .O(new_n81_));
  nand2  g47(.a(x11), .b(new_n46_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n65_), .c(x04), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n54_), .O(new_n84_));
  oai21  g50(.a(new_n81_), .b(new_n46_), .c(new_n84_), .O(z07));
  inv1   g51(.a(x12), .O(new_n86_));
  nand4  g52(.a(new_n80_), .b(new_n78_), .c(new_n86_), .d(x06), .O(new_n87_));
  nand2  g53(.a(new_n80_), .b(new_n78_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n46_), .c(x12), .O(new_n89_));
  nand3  g55(.a(new_n65_), .b(new_n54_), .c(x04), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n89_), .c(new_n87_), .O(z08));
  oai21  g58(.a(new_n88_), .b(x12), .c(x13), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand3  g60(.a(new_n80_), .b(new_n94_), .c(new_n86_), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  oai21  g62(.a(x13), .b(new_n41_), .c(new_n46_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n65_), .c(x04), .O(new_n98_));
  aoi21  g64(.a(new_n96_), .b(new_n72_), .c(new_n98_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n93_), .O(z09));
  nor2   g66(.a(new_n37_), .b(x04), .O(new_n101_));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n95_), .b(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n72_), .c(new_n65_), .O(new_n104_));
  aoi21  g70(.a(new_n50_), .b(x14), .c(new_n37_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(new_n101_), .O(z10));
  nor2   g72(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n36_), .c(x03), .O(new_n108_));
  nand2  g74(.a(new_n54_), .b(x04), .O(new_n109_));
  nor2   g75(.a(new_n109_), .b(new_n108_), .O(z12));
  inv1   g76(.a(new_n101_), .O(z13));
  inv1   g77(.a(x17), .O(new_n112_));
  nor2   g78(.a(new_n109_), .b(new_n112_), .O(z14));
endmodule


