// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x07), .O(new_n37_));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  or2    g06(.a(x14), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n36_), .O(z00));
  oai21  g08(.a(x15), .b(x05), .c(new_n37_), .O(new_n43_));
  aoi22  g09(.a(new_n43_), .b(x18), .c(x05), .d(x04), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand4  g12(.a(new_n46_), .b(new_n40_), .c(new_n45_), .d(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(z02));
  nand2  g14(.a(new_n46_), .b(x04), .O(new_n49_));
  xnor2a g15(.a(x07), .b(x06), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n49_), .c(new_n40_), .O(z03));
  nand2  g17(.a(x08), .b(x06), .O(new_n52_));
  aoi21  g18(.a(new_n52_), .b(new_n38_), .c(new_n37_), .O(new_n53_));
  nor2   g19(.a(new_n37_), .b(new_n45_), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(x08), .O(new_n55_));
  nor3   g21(.a(new_n55_), .b(new_n53_), .c(new_n49_), .O(z04));
  inv1   g22(.a(x09), .O(new_n57_));
  or2    g23(.a(new_n53_), .b(new_n57_), .O(new_n58_));
  nand2  g24(.a(new_n49_), .b(new_n40_), .O(new_n59_));
  nand4  g25(.a(new_n57_), .b(x08), .c(x07), .d(x06), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n38_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n59_), .c(new_n58_), .O(z05));
  nand3  g29(.a(new_n57_), .b(x08), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n38_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x10), .O(new_n67_));
  nor2   g33(.a(x18), .b(x10), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(new_n59_), .O(z06));
  inv1   g36(.a(new_n49_), .O(new_n71_));
  nor2   g37(.a(x11), .b(x10), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n57_), .c(x08), .d(x06), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n38_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x07), .O(new_n75_));
  nand2  g41(.a(new_n69_), .b(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n71_), .O(z07));
  oai21  g43(.a(new_n73_), .b(x12), .c(new_n38_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x07), .O(new_n79_));
  inv1   g45(.a(x08), .O(new_n80_));
  nor2   g46(.a(x09), .b(new_n80_), .O(new_n81_));
  nand4  g47(.a(new_n72_), .b(new_n81_), .c(new_n54_), .d(new_n38_), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(x12), .c(new_n49_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n79_), .O(z08));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(x13), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n73_), .c(new_n38_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x07), .O(new_n89_));
  nor3   g55(.a(x12), .b(x11), .c(x10), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n81_), .c(new_n54_), .d(new_n38_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(x13), .c(new_n49_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n89_), .O(z09));
  nor2   g59(.a(new_n39_), .b(x04), .O(new_n94_));
  inv1   g60(.a(x00), .O(new_n95_));
  nand3  g61(.a(new_n72_), .b(new_n86_), .c(new_n85_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n61_), .c(new_n46_), .O(new_n98_));
  inv1   g64(.a(x03), .O(new_n99_));
  inv1   g65(.a(x16), .O(new_n100_));
  aoi21  g66(.a(new_n99_), .b(x02), .c(new_n100_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x14), .c(new_n39_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n98_), .c(new_n94_), .O(z10));
  nor2   g69(.a(new_n39_), .b(new_n35_), .O(z11));
  inv1   g70(.a(x04), .O(new_n105_));
  aoi21  g71(.a(x16), .b(new_n35_), .c(x03), .O(new_n106_));
  nor3   g72(.a(new_n106_), .b(new_n39_), .c(new_n105_), .O(z12));
  inv1   g73(.a(new_n94_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n105_), .c(new_n40_), .O(z14));
endmodule


