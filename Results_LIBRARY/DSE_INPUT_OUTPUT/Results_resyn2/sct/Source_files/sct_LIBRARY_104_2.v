// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n108_,
    new_n111_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x08), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x16), .c(new_n48_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n47_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n37_), .O(z03));
  nand3  g24(.a(new_n54_), .b(new_n51_), .c(x08), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z04));
  oai21  g26(.a(x09), .b(new_n55_), .c(x08), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x06), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(new_n51_), .O(z05));
  oai21  g31(.a(new_n63_), .b(x09), .c(x10), .O(new_n66_));
  nor2   g32(.a(x10), .b(x09), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x07), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x08), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n66_), .c(new_n51_), .O(z06));
  inv1   g37(.a(x11), .O(new_n72_));
  nand3  g38(.a(new_n67_), .b(new_n72_), .c(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x08), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  inv1   g41(.a(new_n63_), .O(new_n76_));
  nand2  g42(.a(new_n67_), .b(new_n76_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x11), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n75_), .c(new_n51_), .O(z07));
  oai21  g45(.a(new_n77_), .b(x11), .c(x12), .O(new_n80_));
  nor2   g46(.a(x12), .b(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n67_), .c(x07), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x08), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n80_), .c(new_n51_), .O(z08));
  inv1   g51(.a(x13), .O(new_n86_));
  nand4  g52(.a(new_n81_), .b(new_n67_), .c(new_n86_), .d(x07), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x08), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x06), .O(new_n89_));
  nand3  g55(.a(new_n81_), .b(new_n67_), .c(new_n76_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x13), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n89_), .c(new_n51_), .O(z09));
  inv1   g58(.a(x02), .O(new_n93_));
  oai21  g59(.a(x03), .b(new_n93_), .c(x16), .O(new_n94_));
  nor2   g60(.a(new_n63_), .b(x09), .O(new_n95_));
  inv1   g61(.a(x00), .O(new_n96_));
  inv1   g62(.a(x10), .O(new_n97_));
  inv1   g63(.a(x12), .O(new_n98_));
  nand4  g64(.a(new_n86_), .b(new_n98_), .c(new_n72_), .d(new_n97_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n96_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n95_), .O(new_n101_));
  inv1   g67(.a(x14), .O(new_n102_));
  aoi21  g68(.a(new_n36_), .b(x06), .c(new_n102_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n94_), .c(x04), .O(new_n104_));
  aoi21  g70(.a(new_n101_), .b(new_n94_), .c(new_n104_), .O(z10));
  nand2  g71(.a(new_n37_), .b(new_n93_), .O(z11));
  nand2  g72(.a(x16), .b(new_n93_), .O(new_n107_));
  aoi21  g73(.a(new_n36_), .b(x06), .c(x03), .O(new_n108_));
  aoi22  g74(.a(new_n108_), .b(new_n107_), .c(new_n37_), .d(new_n48_), .O(z12));
  nand2  g75(.a(new_n37_), .b(new_n48_), .O(z13));
  inv1   g76(.a(x17), .O(new_n111_));
  oai21  g77(.a(new_n111_), .b(new_n48_), .c(new_n37_), .O(z14));
endmodule


