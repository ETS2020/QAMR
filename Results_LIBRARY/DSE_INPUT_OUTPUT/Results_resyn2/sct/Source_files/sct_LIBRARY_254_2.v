// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x15), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x15), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x05), .c(new_n37_), .O(new_n44_));
  aoi22  g10(.a(new_n44_), .b(new_n42_), .c(x05), .d(x04), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n37_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n47_), .c(new_n39_), .O(z03));
  xor2a  g21(.a(new_n51_), .b(x08), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n47_), .c(new_n39_), .O(z04));
  inv1   g23(.a(x09), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(x08), .c(x07), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n42_), .c(x06), .O(new_n61_));
  nand4  g27(.a(x15), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n48_), .O(z05));
  inv1   g30(.a(x10), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n58_), .c(x08), .d(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x15), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  oai21  g34(.a(new_n62_), .b(x09), .c(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n48_), .O(z06));
  inv1   g36(.a(x11), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n59_), .c(x15), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(x15), .b(x06), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n66_), .c(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n48_), .O(z07));
  nand2  g43(.a(new_n74_), .b(x12), .O(new_n78_));
  nor2   g44(.a(new_n72_), .b(new_n59_), .O(new_n79_));
  nor2   g45(.a(new_n75_), .b(x12), .O(new_n80_));
  aoi22  g46(.a(new_n80_), .b(new_n79_), .c(new_n47_), .d(new_n39_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n78_), .O(z08));
  nor2   g48(.a(x13), .b(x12), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n71_), .c(new_n65_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n59_), .c(x15), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand2  g52(.a(new_n80_), .b(new_n79_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x13), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n86_), .c(new_n48_), .O(z09));
  inv1   g55(.a(x04), .O(new_n90_));
  inv1   g56(.a(x00), .O(new_n91_));
  nand2  g57(.a(new_n84_), .b(new_n91_), .O(new_n92_));
  inv1   g58(.a(x03), .O(new_n93_));
  inv1   g59(.a(x16), .O(new_n94_));
  aoi21  g60(.a(new_n93_), .b(x02), .c(new_n94_), .O(new_n95_));
  nor2   g61(.a(new_n75_), .b(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n92_), .c(new_n60_), .O(new_n97_));
  nand3  g63(.a(new_n95_), .b(new_n39_), .c(x14), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n97_), .c(new_n90_), .O(z10));
  nand2  g65(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n35_), .c(x03), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n90_), .c(new_n39_), .O(z12));
  nor2   g68(.a(new_n38_), .b(new_n90_), .O(z13));
  and2   g69(.a(z13), .b(x17), .O(z14));
endmodule


