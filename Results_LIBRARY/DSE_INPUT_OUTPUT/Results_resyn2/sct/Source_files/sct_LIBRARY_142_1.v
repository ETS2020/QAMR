// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x13), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n37_), .c(x06), .O(z02));
  inv1   g15(.a(x06), .O(new_n50_));
  inv1   g16(.a(x07), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n52_), .c(new_n39_), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n48_), .O(z03));
  inv1   g21(.a(new_n53_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(x08), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n48_), .c(new_n39_), .O(z04));
  inv1   g27(.a(new_n48_), .O(new_n62_));
  oai21  g28(.a(new_n38_), .b(x09), .c(new_n57_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  inv1   g30(.a(new_n57_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n63_), .c(new_n62_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nand3  g34(.a(new_n65_), .b(new_n68_), .c(new_n64_), .O(new_n69_));
  nand2  g35(.a(x08), .b(x07), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(x09), .c(x10), .O(new_n71_));
  oai21  g37(.a(x13), .b(x10), .c(new_n50_), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n71_), .c(new_n69_), .d(new_n62_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  nand4  g40(.a(new_n68_), .b(new_n64_), .c(x08), .d(x07), .O(new_n75_));
  xor2a  g41(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand2  g43(.a(x11), .b(new_n50_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n47_), .c(x04), .O(new_n79_));
  inv1   g45(.a(new_n79_), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n77_), .c(new_n38_), .O(z07));
  inv1   g47(.a(new_n75_), .O(new_n82_));
  nor2   g48(.a(x10), .b(x09), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n74_), .c(x08), .d(x07), .O(new_n84_));
  nor2   g50(.a(x12), .b(x11), .O(new_n85_));
  aoi22  g51(.a(new_n85_), .b(new_n82_), .c(new_n84_), .d(x12), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x06), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n48_), .c(new_n39_), .O(new_n89_));
  oai21  g55(.a(new_n86_), .b(new_n50_), .c(new_n89_), .O(z08));
  nand2  g56(.a(new_n85_), .b(new_n82_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x13), .O(new_n92_));
  nor2   g58(.a(new_n70_), .b(x09), .O(new_n93_));
  nand3  g59(.a(new_n85_), .b(new_n37_), .c(new_n68_), .O(new_n94_));
  inv1   g60(.a(new_n94_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n93_), .c(new_n48_), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n92_), .c(z02), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n94_), .b(new_n98_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n93_), .c(new_n47_), .d(x06), .O(new_n100_));
  inv1   g66(.a(new_n47_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n39_), .c(x14), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n100_), .c(new_n44_), .O(z10));
  nor2   g69(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n35_), .c(x03), .O(new_n105_));
  nand2  g71(.a(new_n39_), .b(x04), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n105_), .O(z12));
  nand2  g73(.a(new_n39_), .b(new_n44_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  nor2   g75(.a(new_n106_), .b(new_n109_), .O(z14));
endmodule


