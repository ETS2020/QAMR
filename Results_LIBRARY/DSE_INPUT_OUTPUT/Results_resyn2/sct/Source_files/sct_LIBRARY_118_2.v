// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n93_, new_n96_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x12), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  xnor2a g17(.a(x07), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n48_), .c(new_n39_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n39_), .O(new_n58_));
  nor2   g24(.a(new_n58_), .b(new_n48_), .O(z04));
  inv1   g25(.a(x12), .O(new_n60_));
  nand2  g26(.a(x08), .b(x07), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(x09), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n60_), .c(x06), .O(new_n63_));
  nand2  g29(.a(x12), .b(x06), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n61_), .c(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n49_), .O(z05));
  nand2  g32(.a(new_n63_), .b(x10), .O(new_n67_));
  nand2  g33(.a(new_n48_), .b(new_n39_), .O(new_n68_));
  inv1   g34(.a(x09), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n69_), .c(x08), .d(x07), .O(new_n71_));
  inv1   g37(.a(new_n71_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(x12), .c(x06), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n68_), .c(new_n67_), .O(z06));
  nand2  g40(.a(new_n71_), .b(x12), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x11), .O(new_n77_));
  nor2   g43(.a(x11), .b(new_n37_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n72_), .c(x12), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n68_), .O(z07));
  nor3   g46(.a(new_n71_), .b(x11), .c(new_n37_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n60_), .c(new_n68_), .O(z08));
  nand2  g48(.a(new_n39_), .b(x13), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n68_), .O(z09));
  inv1   g50(.a(x04), .O(new_n85_));
  nand4  g51(.a(new_n62_), .b(new_n47_), .c(x06), .d(x00), .O(new_n86_));
  inv1   g52(.a(x03), .O(new_n87_));
  inv1   g53(.a(x16), .O(new_n88_));
  aoi21  g54(.a(new_n87_), .b(x02), .c(new_n88_), .O(new_n89_));
  aoi21  g55(.a(new_n89_), .b(x14), .c(new_n38_), .O(new_n90_));
  aoi22  g56(.a(new_n90_), .b(new_n86_), .c(new_n39_), .d(new_n85_), .O(z10));
  nand2  g57(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g58(.a(x16), .b(new_n35_), .c(x03), .O(new_n93_));
  nor3   g59(.a(new_n93_), .b(new_n38_), .c(new_n85_), .O(z12));
  nand2  g60(.a(new_n39_), .b(new_n85_), .O(z13));
  inv1   g61(.a(x17), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n85_), .c(new_n39_), .O(z14));
endmodule


