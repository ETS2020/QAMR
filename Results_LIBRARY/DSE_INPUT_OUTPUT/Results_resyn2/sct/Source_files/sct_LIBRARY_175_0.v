// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n109_;
  inv1   g00(.a(x06), .O(new_n35_));
  nor2   g01(.a(new_n35_), .b(x01), .O(new_n36_));
  aoi22  g02(.a(new_n36_), .b(x14), .c(x02), .d(x01), .O(z00));
  nor2   g03(.a(x15), .b(x05), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x18), .O(new_n39_));
  nor2   g05(.a(x06), .b(x01), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(new_n41_));
  nand2  g07(.a(x05), .b(x04), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n41_), .c(new_n39_), .O(new_n43_));
  inv1   g09(.a(new_n43_), .O(z01));
  inv1   g10(.a(x04), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n35_), .c(x01), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x02), .O(new_n52_));
  oai21  g18(.a(x03), .b(new_n52_), .c(x16), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x04), .O(new_n54_));
  xnor2a g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n54_), .c(new_n41_), .O(z03));
  inv1   g22(.a(x08), .O(new_n57_));
  nand2  g23(.a(x07), .b(x06), .O(new_n58_));
  oai21  g24(.a(new_n40_), .b(new_n57_), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n59_), .c(new_n49_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  oai21  g28(.a(new_n58_), .b(new_n57_), .c(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n49_), .O(new_n66_));
  and2   g32(.a(new_n66_), .b(new_n41_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  inv1   g34(.a(new_n65_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n64_), .O(new_n70_));
  aoi21  g36(.a(new_n65_), .b(x10), .c(new_n54_), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n70_), .c(new_n40_), .O(z06));
  inv1   g38(.a(x11), .O(new_n73_));
  inv1   g39(.a(new_n60_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n73_), .c(new_n68_), .d(new_n64_), .O(new_n75_));
  nand2  g41(.a(new_n68_), .b(new_n64_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n60_), .c(x11), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(new_n49_), .O(new_n78_));
  and2   g44(.a(new_n78_), .b(new_n41_), .O(z07));
  nand2  g45(.a(x08), .b(x07), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(x09), .O(new_n81_));
  nor3   g47(.a(x12), .b(x11), .c(x10), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(x06), .O(new_n83_));
  nor2   g49(.a(x11), .b(x10), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n69_), .O(new_n85_));
  oai21  g51(.a(new_n40_), .b(x12), .c(new_n85_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n83_), .c(new_n49_), .O(z08));
  inv1   g53(.a(x13), .O(new_n88_));
  aoi21  g54(.a(new_n82_), .b(new_n81_), .c(new_n88_), .O(new_n89_));
  nand3  g55(.a(new_n64_), .b(x08), .c(x07), .O(new_n90_));
  nor2   g56(.a(x13), .b(x12), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n84_), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n89_), .c(x06), .O(new_n94_));
  nor2   g60(.a(new_n88_), .b(x06), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n54_), .c(new_n41_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n94_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  nor2   g65(.a(new_n65_), .b(new_n48_), .O(new_n100_));
  inv1   g66(.a(x14), .O(new_n101_));
  nor2   g67(.a(new_n53_), .b(new_n101_), .O(new_n102_));
  aoi21  g68(.a(new_n100_), .b(new_n99_), .c(new_n102_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n45_), .c(new_n41_), .O(z10));
  nor2   g70(.a(new_n40_), .b(new_n52_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n52_), .c(x03), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n45_), .c(new_n41_), .O(z12));
  nor2   g73(.a(new_n40_), .b(new_n45_), .O(z13));
  nand2  g74(.a(x17), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n41_), .O(z14));
endmodule


