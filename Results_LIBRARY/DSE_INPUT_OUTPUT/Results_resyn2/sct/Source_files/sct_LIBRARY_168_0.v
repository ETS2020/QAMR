// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n110_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x04), .c(x06), .O(z02));
  nor2   g16(.a(new_n49_), .b(new_n37_), .O(new_n51_));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n38_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n52_), .c(new_n51_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n52_), .b(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n51_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  nor2   g29(.a(new_n62_), .b(x09), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n51_), .O(z05));
  nor2   g32(.a(x10), .b(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(x08), .c(x07), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  oai21  g36(.a(x03), .b(new_n35_), .c(x16), .O(new_n71_));
  nand2  g37(.a(new_n64_), .b(new_n71_), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nand2  g39(.a(new_n71_), .b(new_n73_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n72_), .c(x04), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n70_), .O(z06));
  nand2  g42(.a(x08), .b(x07), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  nand2  g44(.a(new_n67_), .b(new_n78_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n77_), .c(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  inv1   g47(.a(new_n57_), .O(new_n82_));
  aoi21  g48(.a(new_n67_), .b(new_n82_), .c(new_n78_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n49_), .c(x04), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n81_), .O(z07));
  inv1   g51(.a(new_n62_), .O(new_n86_));
  nand3  g52(.a(new_n67_), .b(new_n86_), .c(new_n78_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(x12), .c(new_n49_), .O(new_n88_));
  inv1   g54(.a(x09), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(x08), .c(x07), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n78_), .c(new_n73_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n90_), .c(x04), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x06), .O(new_n94_));
  oai21  g60(.a(new_n88_), .b(new_n37_), .c(new_n94_), .O(z08));
  inv1   g61(.a(x13), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n91_), .c(new_n78_), .d(new_n73_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n90_), .c(x04), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(x06), .O(new_n99_));
  nor2   g65(.a(x12), .b(x11), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n67_), .c(new_n86_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x13), .c(new_n49_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n37_), .c(new_n99_), .O(z09));
  inv1   g69(.a(x00), .O(new_n104_));
  nand2  g70(.a(new_n97_), .b(new_n104_), .O(new_n105_));
  nand3  g71(.a(new_n105_), .b(new_n64_), .c(new_n71_), .O(new_n106_));
  nand2  g72(.a(new_n49_), .b(x14), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n37_), .O(z10));
  nand2  g74(.a(new_n39_), .b(new_n35_), .O(z11));
  nand2  g75(.a(x16), .b(new_n35_), .O(new_n110_));
  aoi21  g76(.a(new_n110_), .b(new_n47_), .c(new_n37_), .O(z12));
  and2   g77(.a(x17), .b(x04), .O(z14));
  buf    g78(.a(x04), .O(z13));
endmodule


