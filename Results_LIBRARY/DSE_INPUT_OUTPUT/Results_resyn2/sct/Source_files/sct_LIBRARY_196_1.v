// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n109_;
  inv1   g00(.a(x06), .O(new_n35_));
  nand2  g01(.a(x14), .b(new_n35_), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  inv1   g03(.a(x14), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x02), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n36_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n40_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(new_n38_), .c(x06), .O(z02));
  xnor2a g16(.a(x07), .b(x06), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n49_), .c(new_n36_), .O(z03));
  aoi21  g18(.a(x07), .b(x06), .c(x08), .O(new_n53_));
  nand3  g19(.a(x08), .b(x07), .c(x06), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n36_), .O(new_n55_));
  nor3   g21(.a(new_n55_), .b(new_n53_), .c(new_n49_), .O(z04));
  inv1   g22(.a(x09), .O(new_n57_));
  inv1   g23(.a(new_n54_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g25(.a(new_n54_), .b(x09), .O(new_n60_));
  nand4  g26(.a(new_n60_), .b(new_n59_), .c(new_n48_), .d(x04), .O(new_n61_));
  and2   g27(.a(new_n61_), .b(new_n36_), .O(z05));
  inv1   g28(.a(new_n49_), .O(new_n63_));
  inv1   g29(.a(x10), .O(new_n64_));
  nor2   g30(.a(new_n54_), .b(x09), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g32(.a(new_n57_), .b(x08), .c(x07), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x10), .O(new_n68_));
  oai21  g34(.a(x14), .b(x10), .c(new_n35_), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n68_), .c(new_n66_), .d(new_n63_), .O(z06));
  nand2  g36(.a(x08), .b(x07), .O(new_n71_));
  nor2   g37(.a(new_n71_), .b(x09), .O(new_n72_));
  nand4  g38(.a(new_n64_), .b(new_n57_), .c(x08), .d(x07), .O(new_n73_));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  aoi22  g40(.a(new_n74_), .b(new_n72_), .c(new_n73_), .d(x11), .O(new_n75_));
  nand2  g41(.a(x11), .b(new_n35_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n48_), .c(x04), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n36_), .O(new_n78_));
  oai21  g44(.a(new_n75_), .b(new_n35_), .c(new_n78_), .O(z07));
  nand4  g45(.a(new_n74_), .b(new_n57_), .c(x08), .d(x07), .O(new_n80_));
  nor3   g46(.a(x12), .b(x11), .c(x10), .O(new_n81_));
  aoi22  g47(.a(new_n81_), .b(new_n72_), .c(new_n80_), .d(x12), .O(new_n82_));
  nand2  g48(.a(x12), .b(new_n35_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n48_), .c(x04), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n36_), .O(new_n85_));
  oai21  g51(.a(new_n82_), .b(new_n35_), .c(new_n85_), .O(z08));
  inv1   g52(.a(x13), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x06), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n49_), .c(new_n36_), .O(new_n89_));
  nand3  g55(.a(new_n81_), .b(new_n72_), .c(x13), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  inv1   g57(.a(x12), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n64_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n67_), .c(new_n87_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n90_), .c(x06), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n89_), .O(z09));
  aoi21  g62(.a(x14), .b(new_n35_), .c(x04), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  oai21  g64(.a(new_n93_), .b(x13), .c(new_n98_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n65_), .c(new_n48_), .O(new_n100_));
  nand2  g66(.a(new_n48_), .b(x06), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x14), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n100_), .c(new_n97_), .O(z10));
  nand2  g69(.a(new_n36_), .b(new_n40_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n40_), .c(x03), .O(new_n105_));
  nand2  g71(.a(new_n36_), .b(x04), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n105_), .O(z12));
  inv1   g73(.a(new_n97_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  nor2   g75(.a(new_n106_), .b(new_n109_), .O(z14));
endmodule


