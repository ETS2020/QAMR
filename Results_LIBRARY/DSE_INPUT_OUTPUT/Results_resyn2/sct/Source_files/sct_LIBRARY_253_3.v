// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n104_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(x13), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(new_n37_), .O(new_n44_));
  aoi21  g10(.a(new_n42_), .b(x18), .c(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n36_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  oai21  g13(.a(new_n47_), .b(x06), .c(new_n37_), .O(z02));
  inv1   g14(.a(x06), .O(new_n49_));
  inv1   g15(.a(x07), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  nand2  g18(.a(new_n50_), .b(new_n49_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n47_), .c(new_n37_), .O(z03));
  xor2a  g21(.a(new_n52_), .b(x08), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n47_), .c(new_n37_), .O(z04));
  inv1   g23(.a(new_n47_), .O(new_n58_));
  inv1   g24(.a(x13), .O(new_n59_));
  nand3  g25(.a(new_n51_), .b(new_n59_), .c(x08), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x08), .c(x07), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n59_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n61_), .c(new_n58_), .O(z05));
  nand2  g32(.a(new_n65_), .b(x10), .O(new_n67_));
  nand2  g33(.a(new_n47_), .b(new_n37_), .O(new_n68_));
  nand2  g34(.a(new_n51_), .b(x08), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n59_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n68_), .c(new_n67_), .O(z06));
  inv1   g39(.a(x10), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(new_n63_), .c(new_n59_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x06), .O(new_n78_));
  inv1   g44(.a(new_n71_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n60_), .c(x11), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n58_), .O(z07));
  nand2  g47(.a(new_n78_), .b(x12), .O(new_n82_));
  nand4  g48(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nand4  g50(.a(new_n59_), .b(new_n84_), .c(new_n75_), .d(new_n74_), .O(new_n85_));
  or2    g51(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n82_), .c(new_n68_), .O(z08));
  nand4  g53(.a(new_n71_), .b(new_n70_), .c(new_n84_), .d(new_n75_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n58_), .c(new_n59_), .O(z09));
  inv1   g55(.a(x04), .O(new_n90_));
  inv1   g56(.a(x00), .O(new_n91_));
  oai21  g57(.a(new_n76_), .b(x12), .c(new_n91_), .O(new_n92_));
  inv1   g58(.a(x03), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x02), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(x16), .c(new_n83_), .O(new_n95_));
  inv1   g61(.a(x14), .O(new_n96_));
  nor2   g62(.a(new_n46_), .b(new_n96_), .O(new_n97_));
  aoi21  g63(.a(new_n95_), .b(new_n92_), .c(new_n97_), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n90_), .c(new_n37_), .O(z10));
  nor2   g65(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n36_), .c(x03), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n90_), .c(new_n37_), .O(z12));
  nor2   g68(.a(new_n38_), .b(new_n90_), .O(z13));
  nand2  g69(.a(z13), .b(x17), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(z14));
endmodule


