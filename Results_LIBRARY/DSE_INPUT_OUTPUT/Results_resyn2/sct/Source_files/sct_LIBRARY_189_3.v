// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  inv1   g03(.a(x13), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  or2    g06(.a(x14), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n38_), .b(x06), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n50_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nand2  g24(.a(x08), .b(x07), .O(new_n59_));
  aoi21  g25(.a(new_n59_), .b(new_n38_), .c(new_n37_), .O(new_n60_));
  aoi21  g26(.a(x07), .b(x06), .c(x08), .O(new_n61_));
  nor3   g27(.a(new_n61_), .b(new_n60_), .c(new_n49_), .O(z04));
  aoi21  g28(.a(new_n48_), .b(x04), .c(new_n39_), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nand3  g30(.a(x08), .b(x07), .c(x06), .O(new_n65_));
  nor3   g31(.a(new_n65_), .b(x13), .c(x09), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  or2    g34(.a(new_n60_), .b(new_n68_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(new_n64_), .O(z05));
  nor2   g36(.a(new_n59_), .b(x09), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(x13), .c(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n68_), .c(x08), .d(x07), .O(new_n75_));
  nor2   g41(.a(new_n75_), .b(new_n54_), .O(new_n76_));
  nor2   g42(.a(new_n76_), .b(new_n63_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n73_), .O(z06));
  nand2  g44(.a(new_n75_), .b(new_n38_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x06), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x11), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n66_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n64_), .O(z07));
  aoi21  g50(.a(new_n82_), .b(new_n71_), .c(x13), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n37_), .c(x12), .O(new_n86_));
  nor2   g52(.a(x12), .b(x11), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n76_), .c(new_n63_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n86_), .O(z08));
  inv1   g55(.a(new_n65_), .O(new_n90_));
  nand4  g56(.a(new_n87_), .b(new_n90_), .c(new_n74_), .d(new_n68_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n50_), .c(new_n38_), .O(z09));
  inv1   g58(.a(x14), .O(new_n93_));
  or2    g59(.a(new_n48_), .b(new_n93_), .O(new_n94_));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n87_), .b(new_n74_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n66_), .c(new_n48_), .O(new_n98_));
  nand2  g64(.a(new_n40_), .b(x04), .O(new_n99_));
  aoi21  g65(.a(new_n98_), .b(new_n94_), .c(new_n99_), .O(z10));
  nand2  g66(.a(new_n40_), .b(new_n35_), .O(z11));
  nand2  g67(.a(x16), .b(new_n35_), .O(new_n102_));
  nor2   g68(.a(new_n39_), .b(x03), .O(new_n103_));
  aoi22  g69(.a(new_n103_), .b(new_n102_), .c(new_n40_), .d(new_n45_), .O(z12));
  nand2  g70(.a(new_n40_), .b(new_n45_), .O(z13));
  inv1   g71(.a(x17), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n45_), .c(new_n40_), .O(z14));
endmodule


