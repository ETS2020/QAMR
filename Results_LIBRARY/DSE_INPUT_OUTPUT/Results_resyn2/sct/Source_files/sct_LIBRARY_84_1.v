// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n91_, new_n94_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(x15), .b(new_n35_), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  nor2   g04(.a(x15), .b(x05), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x18), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand2  g07(.a(x15), .b(new_n36_), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  inv1   g09(.a(new_n43_), .O(z01));
  inv1   g10(.a(x02), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n45_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  oai21  g13(.a(new_n47_), .b(x06), .c(new_n42_), .O(z02));
  xnor2a g14(.a(x07), .b(x06), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(new_n47_), .c(new_n42_), .O(z03));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  xor2a  g17(.a(new_n51_), .b(x08), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n47_), .c(new_n42_), .O(z04));
  inv1   g19(.a(x03), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x02), .O(new_n55_));
  nand2  g21(.a(new_n42_), .b(x04), .O(new_n56_));
  aoi21  g22(.a(new_n55_), .b(x16), .c(new_n56_), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  oai21  g24(.a(new_n51_), .b(new_n58_), .c(x09), .O(new_n59_));
  inv1   g25(.a(x09), .O(new_n60_));
  nand4  g26(.a(new_n60_), .b(x08), .c(x07), .d(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n57_), .O(z05));
  nand2  g28(.a(new_n61_), .b(x10), .O(new_n63_));
  nor2   g29(.a(new_n61_), .b(x10), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n47_), .O(new_n65_));
  aoi22  g31(.a(new_n65_), .b(new_n63_), .c(x15), .d(new_n36_), .O(z06));
  inv1   g32(.a(x11), .O(new_n67_));
  nand2  g33(.a(new_n64_), .b(new_n67_), .O(new_n68_));
  oai21  g34(.a(new_n61_), .b(x10), .c(x11), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n57_), .O(z07));
  inv1   g36(.a(x12), .O(new_n71_));
  nand3  g37(.a(new_n64_), .b(new_n71_), .c(new_n67_), .O(new_n72_));
  nand2  g38(.a(new_n68_), .b(x12), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n57_), .O(z08));
  nand2  g40(.a(new_n72_), .b(x13), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  inv1   g42(.a(x13), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n71_), .c(new_n67_), .d(new_n76_), .O(new_n78_));
  or2    g44(.a(new_n78_), .b(new_n61_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n75_), .c(new_n57_), .O(z09));
  inv1   g46(.a(x04), .O(new_n81_));
  inv1   g47(.a(x00), .O(new_n82_));
  nand2  g48(.a(new_n78_), .b(new_n82_), .O(new_n83_));
  inv1   g49(.a(x16), .O(new_n84_));
  nor2   g50(.a(new_n84_), .b(x14), .O(new_n85_));
  aoi21  g51(.a(new_n85_), .b(new_n55_), .c(new_n61_), .O(new_n86_));
  nor2   g52(.a(new_n46_), .b(new_n36_), .O(new_n87_));
  aoi21  g53(.a(new_n86_), .b(new_n83_), .c(new_n87_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n81_), .c(new_n42_), .O(z10));
  nand2  g55(.a(new_n42_), .b(new_n45_), .O(z11));
  aoi21  g56(.a(x16), .b(new_n45_), .c(x03), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(new_n56_), .O(z12));
  inv1   g58(.a(new_n56_), .O(z13));
  inv1   g59(.a(x17), .O(new_n94_));
  nor2   g60(.a(new_n56_), .b(new_n94_), .O(z14));
endmodule


