// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n111_, new_n114_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  inv1   g02(.a(x10), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  nand2  g06(.a(x14), .b(new_n40_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(z00));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  aoi21  g12(.a(x05), .b(new_n46_), .c(new_n38_), .O(new_n47_));
  oai21  g13(.a(new_n45_), .b(x05), .c(new_n47_), .O(z01));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  inv1   g20(.a(x02), .O(new_n55_));
  oai21  g21(.a(x03), .b(new_n55_), .c(x16), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(x04), .O(new_n57_));
  nand2  g23(.a(x07), .b(x06), .O(new_n58_));
  inv1   g24(.a(x07), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n36_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n57_), .c(new_n39_), .O(z03));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  inv1   g29(.a(x08), .O(new_n64_));
  nand2  g30(.a(new_n58_), .b(new_n64_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n57_), .c(new_n39_), .O(z04));
  oai21  g33(.a(new_n63_), .b(x10), .c(x09), .O(new_n68_));
  nor3   g34(.a(x09), .b(new_n64_), .c(new_n59_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(x10), .c(x06), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n52_), .O(z05));
  inv1   g37(.a(x09), .O(new_n72_));
  inv1   g38(.a(new_n63_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n56_), .c(new_n37_), .d(x04), .O(new_n75_));
  and2   g41(.a(new_n75_), .b(new_n39_), .O(z06));
  inv1   g42(.a(x11), .O(new_n77_));
  nand2  g43(.a(new_n74_), .b(new_n77_), .O(new_n78_));
  nor2   g44(.a(new_n64_), .b(new_n59_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(x11), .c(new_n72_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n37_), .O(new_n81_));
  nand2  g47(.a(x11), .b(new_n36_), .O(new_n82_));
  nand2  g48(.a(new_n57_), .b(new_n39_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(z07));
  nand4  g50(.a(new_n77_), .b(new_n72_), .c(x08), .d(x07), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x12), .O(new_n86_));
  nor2   g52(.a(x12), .b(x11), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n73_), .c(new_n72_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n37_), .O(new_n90_));
  inv1   g56(.a(x12), .O(new_n91_));
  nor2   g57(.a(new_n91_), .b(x06), .O(new_n92_));
  aoi21  g58(.a(new_n57_), .b(new_n39_), .c(new_n92_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n90_), .O(z08));
  inv1   g60(.a(x13), .O(new_n95_));
  nand2  g61(.a(new_n88_), .b(new_n95_), .O(new_n96_));
  nor2   g62(.a(x11), .b(x09), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n79_), .c(x13), .d(new_n91_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n96_), .c(new_n37_), .O(new_n99_));
  nand2  g65(.a(x13), .b(new_n36_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n99_), .c(new_n83_), .O(z09));
  nor2   g67(.a(new_n38_), .b(x04), .O(new_n102_));
  nor2   g68(.a(new_n63_), .b(x09), .O(new_n103_));
  inv1   g69(.a(x00), .O(new_n104_));
  nand2  g70(.a(new_n87_), .b(new_n95_), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g72(.a(new_n106_), .b(new_n103_), .c(new_n56_), .O(new_n107_));
  aoi21  g73(.a(new_n51_), .b(x14), .c(new_n38_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n107_), .c(new_n102_), .O(z10));
  nand2  g75(.a(new_n39_), .b(new_n55_), .O(z11));
  aoi21  g76(.a(x16), .b(new_n55_), .c(x03), .O(new_n111_));
  nor3   g77(.a(new_n111_), .b(new_n38_), .c(new_n46_), .O(z12));
  inv1   g78(.a(new_n102_), .O(z13));
  nand3  g79(.a(new_n39_), .b(x17), .c(x04), .O(new_n114_));
  inv1   g80(.a(new_n114_), .O(z14));
endmodule


