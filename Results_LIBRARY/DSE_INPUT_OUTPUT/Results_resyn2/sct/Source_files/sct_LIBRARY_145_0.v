// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x04), .c(x06), .O(z02));
  inv1   g17(.a(x02), .O(new_n52_));
  oai21  g18(.a(x03), .b(new_n52_), .c(x16), .O(new_n53_));
  oai21  g19(.a(new_n37_), .b(x04), .c(new_n53_), .O(new_n54_));
  xnor2a g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n54_), .c(new_n38_), .O(z03));
  and2   g22(.a(x07), .b(x06), .O(new_n57_));
  xnor2a g23(.a(new_n57_), .b(x08), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n54_), .c(new_n38_), .O(z04));
  inv1   g25(.a(x09), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(x08), .c(x07), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x04), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x06), .O(new_n63_));
  nand2  g29(.a(new_n57_), .b(x08), .O(new_n64_));
  aoi21  g30(.a(new_n64_), .b(x09), .c(new_n50_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(new_n36_), .c(new_n63_), .O(z05));
  nor2   g32(.a(new_n50_), .b(new_n36_), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  nand4  g34(.a(new_n60_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand2  g37(.a(new_n69_), .b(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n67_), .O(z06));
  nand2  g39(.a(new_n71_), .b(x11), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n57_), .c(new_n60_), .d(x08), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n67_), .O(z07));
  inv1   g43(.a(x12), .O(new_n78_));
  nand2  g44(.a(new_n75_), .b(new_n78_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n61_), .c(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  aoi21  g47(.a(new_n76_), .b(x12), .c(new_n50_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n36_), .c(new_n81_), .O(z08));
  inv1   g49(.a(x13), .O(new_n84_));
  nand4  g50(.a(new_n75_), .b(new_n70_), .c(new_n84_), .d(new_n78_), .O(new_n85_));
  oai21  g51(.a(new_n79_), .b(new_n69_), .c(x13), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n67_), .O(z09));
  inv1   g53(.a(x00), .O(new_n88_));
  nand3  g54(.a(new_n75_), .b(new_n84_), .c(new_n78_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g56(.a(new_n69_), .b(new_n50_), .O(new_n91_));
  inv1   g57(.a(x14), .O(new_n92_));
  nor2   g58(.a(new_n53_), .b(new_n92_), .O(new_n93_));
  aoi21  g59(.a(new_n91_), .b(new_n90_), .c(new_n93_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n36_), .c(new_n38_), .O(z10));
  nand2  g61(.a(new_n38_), .b(new_n52_), .O(z11));
  aoi21  g62(.a(x16), .b(new_n52_), .c(x03), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n36_), .c(new_n38_), .O(z12));
  and2   g64(.a(x17), .b(x04), .O(z14));
  buf    g65(.a(x04), .O(z13));
endmodule


