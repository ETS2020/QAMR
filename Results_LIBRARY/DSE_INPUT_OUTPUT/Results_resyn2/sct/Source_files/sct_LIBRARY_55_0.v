// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n109_,
    new_n111_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x09), .b(x08), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  nor2   g14(.a(new_n37_), .b(new_n48_), .O(z13));
  oai21  g15(.a(x03), .b(new_n36_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(z13), .c(new_n47_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(new_n50_), .b(x04), .O(new_n53_));
  xnor2a g19(.a(x07), .b(x06), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n53_), .c(new_n41_), .O(z03));
  inv1   g21(.a(x03), .O(new_n56_));
  inv1   g22(.a(x16), .O(new_n57_));
  aoi21  g23(.a(new_n56_), .b(x02), .c(new_n57_), .O(new_n58_));
  nor2   g24(.a(new_n58_), .b(new_n48_), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  inv1   g26(.a(x09), .O(new_n61_));
  nand2  g27(.a(x07), .b(x06), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  and2   g29(.a(x07), .b(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x08), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n59_), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(z04));
  nor2   g33(.a(new_n62_), .b(x09), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  aoi21  g35(.a(new_n62_), .b(x09), .c(new_n60_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n59_), .O(z05));
  aoi21  g37(.a(new_n64_), .b(x10), .c(new_n60_), .O(new_n72_));
  oai21  g38(.a(new_n68_), .b(x10), .c(new_n72_), .O(new_n73_));
  nand2  g39(.a(new_n53_), .b(new_n41_), .O(new_n74_));
  nand2  g40(.a(x10), .b(x09), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(z06));
  oai21  g42(.a(new_n62_), .b(x10), .c(x11), .O(new_n77_));
  inv1   g43(.a(x10), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(x07), .d(x06), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(x09), .c(new_n77_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x08), .O(new_n82_));
  nand2  g48(.a(x11), .b(x09), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n74_), .O(z07));
  nand2  g50(.a(new_n80_), .b(x12), .O(new_n85_));
  inv1   g51(.a(x12), .O(new_n86_));
  nor2   g52(.a(x11), .b(x10), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n64_), .c(new_n86_), .d(new_n61_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x08), .O(new_n90_));
  nor2   g56(.a(new_n86_), .b(new_n61_), .O(new_n91_));
  aoi21  g57(.a(new_n53_), .b(new_n41_), .c(new_n91_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n90_), .O(z08));
  nand2  g59(.a(new_n88_), .b(x13), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand3  g61(.a(new_n87_), .b(new_n95_), .c(new_n86_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n68_), .O(new_n98_));
  oai21  g64(.a(x13), .b(new_n61_), .c(new_n60_), .O(new_n99_));
  nand4  g65(.a(new_n99_), .b(new_n98_), .c(new_n94_), .d(new_n59_), .O(z09));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n96_), .b(new_n101_), .O(new_n102_));
  nand2  g68(.a(new_n56_), .b(x02), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(x16), .c(new_n60_), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(new_n102_), .c(new_n68_), .O(new_n105_));
  nand3  g71(.a(new_n58_), .b(new_n41_), .c(x14), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n105_), .c(new_n48_), .O(z10));
  nand2  g73(.a(new_n41_), .b(new_n36_), .O(z11));
  aoi21  g74(.a(x16), .b(new_n36_), .c(x03), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n48_), .c(new_n41_), .O(z12));
  nand2  g76(.a(x17), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n41_), .O(z14));
endmodule


