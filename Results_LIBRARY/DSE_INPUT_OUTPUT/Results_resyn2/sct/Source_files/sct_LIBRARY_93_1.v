// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n109_, new_n112_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x16), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n37_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  aoi21  g13(.a(new_n36_), .b(new_n45_), .c(x06), .O(z02));
  inv1   g14(.a(x02), .O(new_n49_));
  oai21  g15(.a(x03), .b(new_n49_), .c(x16), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x06), .c(x07), .O(new_n51_));
  inv1   g17(.a(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n52_), .c(x04), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n51_), .c(new_n38_), .O(z03));
  nand2  g21(.a(x08), .b(x07), .O(new_n56_));
  inv1   g22(.a(x08), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand4  g24(.a(new_n58_), .b(new_n56_), .c(new_n50_), .d(x06), .O(new_n59_));
  nand3  g25(.a(new_n36_), .b(x08), .c(new_n52_), .O(new_n60_));
  aoi21  g26(.a(new_n60_), .b(new_n59_), .c(new_n45_), .O(z04));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  oai21  g28(.a(new_n37_), .b(x09), .c(new_n62_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  inv1   g30(.a(new_n62_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g32(.a(x03), .O(new_n67_));
  aoi21  g33(.a(new_n67_), .b(x02), .c(new_n36_), .O(new_n68_));
  nor2   g34(.a(new_n68_), .b(new_n45_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n66_), .c(new_n63_), .O(z05));
  oai21  g36(.a(new_n56_), .b(x09), .c(x10), .O(new_n71_));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(x08), .c(x07), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n71_), .c(new_n50_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  oai21  g42(.a(new_n76_), .b(x06), .c(x04), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n38_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n75_), .O(z06));
  oai22  g45(.a(new_n73_), .b(new_n52_), .c(new_n37_), .d(x11), .O(new_n80_));
  inv1   g46(.a(x11), .O(new_n81_));
  nand3  g47(.a(new_n72_), .b(new_n65_), .c(new_n81_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(new_n69_), .O(z07));
  nor2   g49(.a(new_n56_), .b(x09), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n81_), .c(new_n76_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x12), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  nand4  g53(.a(new_n72_), .b(new_n65_), .c(new_n87_), .d(new_n81_), .O(new_n88_));
  oai21  g54(.a(x16), .b(x12), .c(new_n52_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(new_n69_), .O(z08));
  nand4  g56(.a(new_n84_), .b(new_n87_), .c(new_n81_), .d(new_n76_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x13), .O(new_n92_));
  nor2   g58(.a(x13), .b(x12), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n72_), .c(new_n65_), .d(new_n81_), .O(new_n94_));
  oai21  g60(.a(x16), .b(x13), .c(new_n52_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n50_), .c(x04), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n94_), .c(new_n92_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  inv1   g65(.a(x13), .O(new_n100_));
  nand4  g66(.a(new_n100_), .b(new_n87_), .c(new_n81_), .d(new_n76_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n84_), .c(new_n50_), .O(new_n103_));
  nand2  g69(.a(new_n68_), .b(x14), .O(new_n104_));
  nand2  g70(.a(x06), .b(x04), .O(new_n105_));
  aoi21  g71(.a(new_n104_), .b(new_n103_), .c(new_n105_), .O(z10));
  nand2  g72(.a(new_n38_), .b(new_n49_), .O(z11));
  nand2  g73(.a(x16), .b(new_n49_), .O(new_n108_));
  nor2   g74(.a(new_n37_), .b(x03), .O(new_n109_));
  aoi22  g75(.a(new_n109_), .b(new_n108_), .c(new_n38_), .d(new_n45_), .O(z12));
  nand2  g76(.a(new_n38_), .b(new_n45_), .O(z13));
  nand2  g77(.a(x17), .b(x04), .O(new_n112_));
  nor2   g78(.a(new_n112_), .b(new_n37_), .O(z14));
endmodule


