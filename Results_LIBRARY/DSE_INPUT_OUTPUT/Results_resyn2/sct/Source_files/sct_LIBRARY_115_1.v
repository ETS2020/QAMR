// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n111_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x13), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n36_), .c(new_n48_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z02));
  inv1   g21(.a(x02), .O(new_n56_));
  oai21  g22(.a(x03), .b(new_n56_), .c(x16), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(x04), .O(new_n58_));
  inv1   g24(.a(x07), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n48_), .O(new_n60_));
  nand2  g26(.a(x07), .b(x06), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n60_), .c(new_n38_), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n58_), .O(z03));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  inv1   g30(.a(x08), .O(new_n65_));
  nand2  g31(.a(new_n61_), .b(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n58_), .c(new_n38_), .O(z04));
  oai21  g34(.a(new_n37_), .b(x09), .c(new_n64_), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(x08), .c(x07), .d(x06), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n69_), .c(new_n53_), .O(z05));
  inv1   g38(.a(x10), .O(new_n73_));
  nand2  g39(.a(new_n36_), .b(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n48_), .O(new_n75_));
  nand3  g41(.a(new_n70_), .b(x08), .c(x07), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x10), .O(new_n77_));
  inv1   g43(.a(new_n71_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n73_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n77_), .c(new_n75_), .d(new_n53_), .O(z06));
  inv1   g46(.a(x11), .O(new_n81_));
  nand4  g47(.a(new_n73_), .b(new_n70_), .c(x08), .d(x07), .O(new_n82_));
  xor2a  g48(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  nor2   g50(.a(new_n81_), .b(x06), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n58_), .c(new_n38_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n84_), .O(z07));
  oai21  g53(.a(new_n82_), .b(x11), .c(x12), .O(new_n88_));
  nor3   g54(.a(x12), .b(x11), .c(x10), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n78_), .O(new_n90_));
  oai21  g56(.a(x13), .b(x12), .c(new_n48_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n88_), .d(new_n53_), .O(z08));
  nand2  g58(.a(new_n58_), .b(new_n38_), .O(new_n93_));
  inv1   g59(.a(x12), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n81_), .c(new_n73_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n76_), .c(new_n36_), .O(new_n96_));
  nor2   g62(.a(new_n65_), .b(new_n59_), .O(new_n97_));
  nand4  g63(.a(new_n89_), .b(new_n97_), .c(x13), .d(new_n70_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n96_), .c(x06), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n93_), .O(z09));
  nand3  g66(.a(new_n52_), .b(new_n38_), .c(x14), .O(new_n101_));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n94_), .b(new_n81_), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n74_), .c(new_n102_), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(new_n78_), .c(new_n57_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n101_), .c(new_n49_), .O(z10));
  nor2   g72(.a(new_n37_), .b(new_n56_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n56_), .c(x03), .O(new_n108_));
  nor3   g74(.a(new_n108_), .b(new_n37_), .c(new_n49_), .O(z12));
  nor2   g75(.a(new_n37_), .b(new_n49_), .O(z13));
  nand2  g76(.a(x17), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n38_), .O(z14));
endmodule


