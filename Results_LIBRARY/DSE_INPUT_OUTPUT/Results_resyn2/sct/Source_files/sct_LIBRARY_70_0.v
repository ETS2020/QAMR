// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n106_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  aoi21  g03(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x04), .O(new_n40_));
  inv1   g06(.a(x06), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x05), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n40_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n41_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n37_), .O(new_n52_));
  nor2   g18(.a(new_n41_), .b(x04), .O(new_n53_));
  nor2   g19(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n41_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand2  g23(.a(new_n55_), .b(new_n41_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n52_), .O(z03));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n57_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n54_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n52_), .O(z04));
  nand2  g31(.a(new_n61_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n49_), .O(z05));
  nand2  g35(.a(new_n68_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nor2   g37(.a(x09), .b(new_n62_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n56_), .c(new_n71_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n70_), .c(new_n49_), .O(z06));
  nand2  g40(.a(new_n73_), .b(x11), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n72_), .c(new_n56_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n75_), .c(new_n49_), .O(z07));
  nor2   g44(.a(x12), .b(new_n55_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n76_), .c(new_n72_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  aoi21  g48(.a(new_n77_), .b(x12), .c(new_n48_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n40_), .c(new_n82_), .O(z08));
  oai21  g50(.a(new_n80_), .b(x13), .c(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  inv1   g52(.a(x02), .O(new_n87_));
  oai21  g53(.a(x03), .b(new_n87_), .c(x16), .O(new_n88_));
  nor2   g54(.a(x12), .b(x09), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n76_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n61_), .c(x13), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x04), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n86_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nor2   g61(.a(x13), .b(x12), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n76_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nor2   g64(.a(new_n68_), .b(new_n48_), .O(new_n99_));
  nor2   g65(.a(new_n88_), .b(new_n36_), .O(new_n100_));
  aoi21  g66(.a(new_n99_), .b(new_n98_), .c(new_n100_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n40_), .c(new_n52_), .O(z10));
  nor2   g68(.a(new_n37_), .b(new_n87_), .O(z11));
  nand2  g69(.a(x16), .b(new_n87_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n46_), .c(new_n40_), .O(z12));
  nor2   g71(.a(x17), .b(new_n40_), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n53_), .O(z14));
  buf    g73(.a(x04), .O(z13));
endmodule


