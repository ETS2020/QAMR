// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n100_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  inv1   g03(.a(x13), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  or2    g06(.a(x14), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n40_), .O(z02));
  inv1   g16(.a(x03), .O(new_n51_));
  inv1   g17(.a(x16), .O(new_n52_));
  aoi21  g18(.a(new_n51_), .b(x02), .c(new_n52_), .O(new_n53_));
  nor2   g19(.a(new_n53_), .b(new_n45_), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(new_n56_), .c(new_n54_), .d(new_n40_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  xor2a  g25(.a(new_n57_), .b(x08), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n49_), .c(new_n40_), .O(z04));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(x13), .c(x09), .O(new_n63_));
  nand2  g29(.a(x08), .b(x07), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(x09), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x13), .c(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n63_), .c(new_n54_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nor2   g34(.a(x13), .b(new_n37_), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(new_n65_), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  nand2  g37(.a(new_n68_), .b(new_n71_), .O(new_n72_));
  oai21  g38(.a(new_n72_), .b(new_n64_), .c(new_n38_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(x06), .c(new_n49_), .O(new_n74_));
  oai21  g40(.a(new_n70_), .b(new_n68_), .c(new_n74_), .O(z06));
  nand2  g41(.a(new_n73_), .b(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x11), .O(new_n77_));
  nand2  g43(.a(new_n49_), .b(new_n40_), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n70_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n77_), .O(z07));
  aoi21  g47(.a(new_n79_), .b(new_n65_), .c(x13), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n37_), .c(x12), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nand3  g50(.a(new_n79_), .b(new_n70_), .c(new_n84_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n83_), .c(new_n78_), .O(z08));
  oai21  g52(.a(new_n49_), .b(x13), .c(new_n40_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(new_n85_), .O(z09));
  nand2  g54(.a(new_n40_), .b(x04), .O(new_n89_));
  nand2  g55(.a(new_n53_), .b(x14), .O(new_n90_));
  inv1   g56(.a(x00), .O(new_n91_));
  nand2  g57(.a(new_n79_), .b(new_n84_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand4  g59(.a(new_n93_), .b(new_n69_), .c(new_n65_), .d(new_n48_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n90_), .c(new_n89_), .O(z10));
  nor2   g61(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g62(.a(x16), .b(new_n35_), .c(x03), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(new_n89_), .O(z12));
  nand2  g64(.a(new_n40_), .b(new_n45_), .O(z13));
  nand3  g65(.a(new_n40_), .b(x17), .c(x04), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(z14));
endmodule


