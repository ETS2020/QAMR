// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n99_, new_n101_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x04), .O(new_n41_));
  inv1   g07(.a(x06), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x05), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x04), .c(x06), .O(z02));
  inv1   g16(.a(new_n37_), .O(new_n51_));
  aoi21  g17(.a(x06), .b(x04), .c(x07), .O(new_n52_));
  oai21  g18(.a(x03), .b(new_n36_), .c(x16), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n52_), .c(new_n51_), .O(z03));
  nor2   g22(.a(new_n49_), .b(new_n41_), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n54_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x07), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x04), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x06), .O(new_n66_));
  aoi21  g32(.a(new_n58_), .b(x09), .c(new_n49_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(new_n41_), .c(new_n66_), .O(z05));
  nand4  g34(.a(new_n63_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  inv1   g37(.a(new_n69_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n70_), .c(new_n57_), .O(z06));
  inv1   g40(.a(x11), .O(new_n75_));
  nand3  g41(.a(new_n72_), .b(new_n75_), .c(new_n71_), .O(new_n76_));
  nand2  g42(.a(new_n73_), .b(x11), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n57_), .O(z07));
  inv1   g44(.a(x12), .O(new_n79_));
  nand4  g45(.a(new_n72_), .b(new_n79_), .c(new_n75_), .d(new_n71_), .O(new_n80_));
  nand2  g46(.a(new_n76_), .b(x12), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n57_), .O(z08));
  nand2  g48(.a(new_n80_), .b(x13), .O(new_n83_));
  inv1   g49(.a(x13), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n79_), .c(new_n75_), .d(new_n71_), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n72_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n83_), .c(new_n57_), .O(z09));
  inv1   g54(.a(x00), .O(new_n89_));
  nand2  g55(.a(new_n85_), .b(new_n89_), .O(new_n90_));
  nor2   g56(.a(new_n69_), .b(new_n49_), .O(new_n91_));
  inv1   g57(.a(x14), .O(new_n92_));
  nor2   g58(.a(new_n53_), .b(new_n92_), .O(new_n93_));
  aoi21  g59(.a(new_n91_), .b(new_n90_), .c(new_n93_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n41_), .c(new_n51_), .O(z10));
  nor2   g61(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g62(.a(x16), .b(new_n36_), .c(x03), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n41_), .c(new_n51_), .O(z12));
  nor2   g64(.a(new_n42_), .b(x04), .O(new_n99_));
  inv1   g65(.a(new_n99_), .O(z13));
  nor2   g66(.a(x17), .b(new_n41_), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n99_), .O(z14));
endmodule


