// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x15), .O(new_n36_));
  inv1   g02(.a(x18), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  inv1   g08(.a(x05), .O(new_n43_));
  nand2  g09(.a(x15), .b(new_n43_), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  nand2  g11(.a(x05), .b(new_n45_), .O(new_n46_));
  nand3  g12(.a(new_n46_), .b(new_n44_), .c(new_n38_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x16), .c(new_n45_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n38_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n38_), .O(z03));
  inv1   g24(.a(x16), .O(new_n59_));
  aoi21  g25(.a(new_n49_), .b(x02), .c(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n38_), .b(x04), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g28(.a(new_n54_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x08), .O(new_n64_));
  inv1   g30(.a(x08), .O(new_n65_));
  nand2  g31(.a(new_n54_), .b(new_n65_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n62_), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(z04));
  nand2  g34(.a(new_n64_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(x08), .c(x07), .d(x06), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n69_), .c(new_n62_), .O(z05));
  nand2  g38(.a(new_n71_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand4  g40(.a(new_n63_), .b(new_n74_), .c(new_n70_), .d(x08), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n51_), .O(new_n76_));
  and2   g42(.a(new_n76_), .b(new_n38_), .O(z06));
  inv1   g43(.a(x11), .O(new_n78_));
  inv1   g44(.a(new_n71_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n74_), .O(new_n80_));
  nand2  g46(.a(new_n75_), .b(x11), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n80_), .c(new_n62_), .O(z07));
  nand2  g48(.a(new_n80_), .b(x12), .O(new_n83_));
  nor2   g49(.a(x12), .b(x11), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n79_), .c(new_n74_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n83_), .c(new_n62_), .O(z08));
  nand2  g52(.a(new_n85_), .b(x13), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand3  g54(.a(new_n84_), .b(new_n88_), .c(new_n74_), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n79_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n87_), .c(new_n62_), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n89_), .b(new_n93_), .O(new_n94_));
  nor2   g60(.a(new_n71_), .b(new_n60_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g62(.a(new_n60_), .b(x14), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n96_), .c(new_n61_), .O(z10));
  inv1   g64(.a(x02), .O(new_n99_));
  nand2  g65(.a(new_n38_), .b(new_n99_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n99_), .c(x03), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n61_), .O(z12));
  nand2  g68(.a(new_n38_), .b(new_n45_), .O(z13));
  inv1   g69(.a(x17), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n45_), .c(new_n38_), .O(z14));
endmodule


