// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n106_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x08), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(x08), .c(x06), .O(z02));
  xnor2a g15(.a(x07), .b(x06), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n48_), .c(new_n41_), .O(z03));
  inv1   g17(.a(x04), .O(new_n52_));
  inv1   g18(.a(x03), .O(new_n53_));
  inv1   g19(.a(x16), .O(new_n54_));
  aoi21  g20(.a(new_n53_), .b(x02), .c(new_n54_), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(x07), .b(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n57_), .c(new_n56_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nand2  g28(.a(new_n57_), .b(x09), .O(new_n63_));
  nor2   g29(.a(new_n57_), .b(x09), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n63_), .c(new_n56_), .d(new_n41_), .O(z05));
  inv1   g32(.a(x06), .O(new_n67_));
  oai21  g33(.a(x10), .b(new_n58_), .c(new_n67_), .O(new_n68_));
  nand2  g34(.a(x08), .b(x07), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(x09), .c(x10), .O(new_n70_));
  inv1   g36(.a(x09), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(new_n59_), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(x08), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n70_), .c(new_n68_), .d(new_n56_), .O(z06));
  nand4  g41(.a(new_n72_), .b(new_n71_), .c(x08), .d(x07), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x11), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n64_), .O(new_n79_));
  oai21  g45(.a(x11), .b(new_n58_), .c(new_n67_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n79_), .c(new_n77_), .d(new_n56_), .O(z07));
  oai21  g47(.a(new_n76_), .b(x11), .c(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand3  g49(.a(new_n78_), .b(new_n64_), .c(new_n83_), .O(new_n84_));
  oai21  g50(.a(x12), .b(new_n58_), .c(new_n67_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n82_), .d(new_n56_), .O(z08));
  inv1   g52(.a(x13), .O(new_n87_));
  nand2  g53(.a(new_n41_), .b(new_n87_), .O(new_n88_));
  nand3  g54(.a(new_n78_), .b(new_n64_), .c(new_n83_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g56(.a(x11), .O(new_n91_));
  nand4  g57(.a(new_n87_), .b(new_n83_), .c(new_n91_), .d(new_n72_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n90_), .c(new_n56_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n92_), .b(new_n96_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n64_), .c(new_n47_), .O(new_n98_));
  nand3  g64(.a(new_n55_), .b(new_n41_), .c(x14), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(new_n52_), .O(z10));
  nand2  g66(.a(new_n41_), .b(new_n36_), .O(z11));
  nand2  g67(.a(x16), .b(new_n36_), .O(new_n102_));
  nor2   g68(.a(new_n37_), .b(x03), .O(new_n103_));
  aoi22  g69(.a(new_n103_), .b(new_n102_), .c(new_n41_), .d(new_n52_), .O(z12));
  nand2  g70(.a(new_n41_), .b(new_n52_), .O(z13));
  inv1   g71(.a(x17), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n52_), .c(new_n41_), .O(z14));
endmodule


