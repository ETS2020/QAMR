// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n110_;
  inv1   g00(.a(x06), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  aoi22  g02(.a(new_n36_), .b(x14), .c(x02), .d(x01), .O(z00));
  inv1   g03(.a(x18), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(x15), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(x14), .b(x06), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n40_), .c(new_n42_), .O(new_n43_));
  oai21  g09(.a(new_n39_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x02), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n45_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n35_), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(z02));
  inv1   g16(.a(x07), .O(new_n51_));
  inv1   g17(.a(x14), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(x06), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n54_), .c(new_n48_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n55_), .b(new_n58_), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n59_), .c(new_n41_), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n47_), .O(z04));
  nor2   g28(.a(new_n58_), .b(new_n51_), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(x14), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(new_n60_), .O(new_n66_));
  nor2   g32(.a(x14), .b(x09), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g34(.a(new_n47_), .b(new_n41_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n65_), .O(z05));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n63_), .c(x14), .O(new_n72_));
  aoi21  g38(.a(new_n68_), .b(x10), .c(new_n47_), .O(new_n73_));
  oai21  g39(.a(new_n72_), .b(new_n35_), .c(new_n73_), .O(z06));
  oai21  g40(.a(new_n72_), .b(new_n35_), .c(x11), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  nand4  g42(.a(new_n71_), .b(new_n76_), .c(x08), .d(x07), .O(new_n77_));
  or2    g43(.a(new_n77_), .b(new_n53_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n75_), .c(new_n69_), .O(z07));
  nand2  g45(.a(new_n77_), .b(new_n52_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x12), .O(new_n82_));
  nor3   g48(.a(new_n60_), .b(x14), .c(x09), .O(new_n83_));
  nor3   g49(.a(x12), .b(x11), .c(x10), .O(new_n84_));
  aoi22  g50(.a(new_n84_), .b(new_n83_), .c(new_n47_), .d(new_n41_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n82_), .O(z08));
  nor2   g52(.a(x12), .b(x11), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n71_), .c(x08), .d(x07), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n52_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x06), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x13), .O(new_n91_));
  inv1   g57(.a(x10), .O(new_n92_));
  inv1   g58(.a(x12), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n93_), .c(new_n76_), .d(new_n92_), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  aoi22  g62(.a(new_n96_), .b(new_n83_), .c(new_n47_), .d(new_n41_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n91_), .O(z09));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n95_), .b(new_n99_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n83_), .c(new_n46_), .O(new_n101_));
  inv1   g67(.a(x03), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x02), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(x16), .c(x14), .d(new_n35_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n101_), .c(new_n40_), .O(z10));
  nor2   g71(.a(new_n42_), .b(new_n45_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n45_), .c(x03), .O(new_n107_));
  nor3   g73(.a(new_n107_), .b(new_n42_), .c(new_n40_), .O(z12));
  nand2  g74(.a(new_n41_), .b(new_n40_), .O(z13));
  nand3  g75(.a(new_n41_), .b(x17), .c(x04), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z14));
endmodule


