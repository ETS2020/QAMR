// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n106_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x13), .O(new_n36_));
  inv1   g02(.a(x16), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x02), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(x16), .c(new_n49_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n38_), .O(z02));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(x07), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n48_), .O(new_n57_));
  nand4  g23(.a(new_n57_), .b(new_n55_), .c(new_n52_), .d(new_n38_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  nand2  g25(.a(new_n52_), .b(new_n38_), .O(new_n60_));
  xor2a  g26(.a(new_n55_), .b(x08), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n60_), .O(z04));
  inv1   g28(.a(new_n60_), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  inv1   g32(.a(new_n64_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n65_), .c(new_n63_), .O(z05));
  oai21  g35(.a(new_n64_), .b(x09), .c(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand3  g37(.a(new_n67_), .b(new_n71_), .c(new_n66_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n52_), .O(new_n73_));
  and2   g39(.a(new_n73_), .b(new_n38_), .O(z06));
  nor2   g40(.a(new_n64_), .b(x09), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g43(.a(new_n71_), .b(new_n66_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n64_), .c(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n52_), .O(new_n80_));
  and2   g46(.a(new_n80_), .b(new_n38_), .O(z07));
  nand2  g47(.a(new_n77_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand3  g49(.a(new_n76_), .b(new_n75_), .c(new_n83_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n82_), .c(new_n63_), .O(z08));
  nand2  g51(.a(new_n84_), .b(x13), .O(new_n86_));
  nand2  g52(.a(new_n38_), .b(new_n49_), .O(z13));
  inv1   g53(.a(new_n51_), .O(new_n88_));
  inv1   g54(.a(x11), .O(new_n89_));
  nand4  g55(.a(new_n36_), .b(new_n83_), .c(new_n89_), .d(new_n71_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n68_), .c(new_n88_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x16), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(z13), .c(new_n86_), .O(z09));
  nand3  g59(.a(x16), .b(new_n50_), .c(x02), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n90_), .O(new_n95_));
  oai21  g61(.a(x16), .b(x13), .c(x00), .O(new_n96_));
  aoi21  g62(.a(new_n51_), .b(x16), .c(new_n96_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n95_), .c(new_n75_), .O(new_n98_));
  nand3  g64(.a(new_n51_), .b(x16), .c(x14), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(new_n49_), .O(z10));
  inv1   g66(.a(x02), .O(new_n101_));
  nand2  g67(.a(new_n38_), .b(new_n101_), .O(z11));
  nand2  g68(.a(new_n51_), .b(x16), .O(new_n103_));
  nand2  g69(.a(x13), .b(x03), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n103_), .c(new_n49_), .O(z12));
  inv1   g71(.a(x17), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n49_), .c(new_n38_), .O(z14));
endmodule


