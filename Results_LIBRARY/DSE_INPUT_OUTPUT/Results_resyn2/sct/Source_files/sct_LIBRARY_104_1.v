// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n102_,
    new_n105_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x08), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n36_), .c(x06), .O(z02));
  xnor2a g17(.a(x07), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n50_), .c(new_n38_), .O(z03));
  inv1   g19(.a(x04), .O(new_n54_));
  inv1   g20(.a(x03), .O(new_n55_));
  inv1   g21(.a(x16), .O(new_n56_));
  aoi21  g22(.a(new_n55_), .b(x02), .c(new_n56_), .O(new_n57_));
  nor2   g23(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  inv1   g24(.a(x07), .O(new_n59_));
  nand2  g25(.a(new_n36_), .b(new_n59_), .O(new_n60_));
  inv1   g26(.a(x06), .O(new_n61_));
  and2   g27(.a(x08), .b(x07), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n60_), .c(new_n58_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  inv1   g31(.a(x09), .O(new_n66_));
  nand2  g32(.a(new_n62_), .b(new_n66_), .O(new_n67_));
  nand2  g33(.a(x08), .b(x07), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n67_), .c(new_n58_), .d(new_n38_), .O(z05));
  nand4  g36(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x10), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  nor2   g39(.a(new_n68_), .b(x09), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n37_), .c(new_n73_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n72_), .c(new_n58_), .O(z06));
  nand2  g42(.a(new_n73_), .b(x06), .O(new_n77_));
  oai22  g43(.a(new_n77_), .b(new_n67_), .c(new_n37_), .d(x11), .O(new_n78_));
  nor2   g44(.a(x11), .b(x10), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n74_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n58_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand4  g48(.a(new_n79_), .b(new_n62_), .c(new_n82_), .d(new_n66_), .O(new_n83_));
  nand2  g49(.a(new_n80_), .b(x12), .O(new_n84_));
  oai21  g50(.a(x12), .b(x08), .c(new_n61_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n58_), .O(z08));
  xor2a  g52(.a(new_n83_), .b(x13), .O(new_n87_));
  oai21  g53(.a(x13), .b(x08), .c(new_n61_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n49_), .c(x04), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n87_), .O(z09));
  inv1   g57(.a(new_n71_), .O(new_n92_));
  inv1   g58(.a(x00), .O(new_n93_));
  nor2   g59(.a(x13), .b(x12), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n79_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n92_), .c(new_n49_), .O(new_n97_));
  nand3  g63(.a(new_n57_), .b(new_n38_), .c(x14), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n97_), .c(new_n54_), .O(z10));
  nand2  g65(.a(new_n38_), .b(new_n48_), .O(z11));
  nand2  g66(.a(x16), .b(new_n48_), .O(new_n101_));
  nor2   g67(.a(new_n37_), .b(x03), .O(new_n102_));
  aoi22  g68(.a(new_n102_), .b(new_n101_), .c(new_n38_), .d(new_n54_), .O(z12));
  nand2  g69(.a(new_n38_), .b(new_n54_), .O(z13));
  inv1   g70(.a(x17), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n54_), .c(new_n38_), .O(z14));
endmodule


