// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n97_, new_n98_, new_n101_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nand2  g02(.a(x13), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  aoi21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(x02), .b(new_n35_), .c(new_n39_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(x05), .b(x04), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n37_), .O(new_n43_));
  aoi21  g09(.a(new_n41_), .b(x18), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x02), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n46_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  xnor2a g17(.a(x07), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n48_), .c(new_n37_), .O(z03));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  xor2a  g20(.a(new_n54_), .b(x08), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n48_), .c(new_n37_), .O(z04));
  nand2  g22(.a(x08), .b(x07), .O(new_n57_));
  nor2   g23(.a(new_n57_), .b(x09), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(x13), .c(x06), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(x13), .c(x09), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n49_), .O(z05));
  inv1   g28(.a(x13), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  inv1   g30(.a(x10), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n64_), .c(x08), .d(x07), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x06), .O(new_n68_));
  nand3  g34(.a(new_n64_), .b(x08), .c(x07), .O(new_n69_));
  nand2  g35(.a(new_n63_), .b(x06), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n69_), .c(x10), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n49_), .O(z06));
  nand2  g38(.a(new_n68_), .b(x11), .O(new_n73_));
  nand2  g39(.a(new_n48_), .b(new_n37_), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n58_), .c(new_n63_), .d(x06), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n73_), .O(z07));
  inv1   g43(.a(x11), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n65_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n69_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(x13), .c(x06), .O(new_n82_));
  nand2  g48(.a(new_n76_), .b(x12), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n49_), .O(z08));
  nand2  g50(.a(new_n81_), .b(x06), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n49_), .c(new_n63_), .O(z09));
  nor2   g52(.a(new_n70_), .b(new_n69_), .O(new_n87_));
  inv1   g53(.a(x00), .O(new_n88_));
  nand2  g54(.a(new_n80_), .b(new_n88_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  inv1   g56(.a(x03), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x02), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(x16), .c(new_n36_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n37_), .c(x04), .O(new_n94_));
  aoi21  g60(.a(new_n90_), .b(new_n47_), .c(new_n94_), .O(z10));
  nor2   g61(.a(new_n38_), .b(new_n46_), .O(z11));
  inv1   g62(.a(x04), .O(new_n97_));
  aoi21  g63(.a(x16), .b(new_n46_), .c(x03), .O(new_n98_));
  nor3   g64(.a(new_n98_), .b(new_n38_), .c(new_n97_), .O(z12));
  nand2  g65(.a(new_n37_), .b(new_n97_), .O(z13));
  inv1   g66(.a(x17), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n97_), .c(new_n37_), .O(z14));
endmodule


