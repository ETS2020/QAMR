// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x04), .O(new_n36_));
  nand2  g02(.a(x06), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x16), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  inv1   g16(.a(x06), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(x04), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n50_), .c(new_n37_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n52_), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n37_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  aoi21  g25(.a(new_n55_), .b(new_n52_), .c(new_n59_), .O(new_n60_));
  nor3   g26(.a(x08), .b(new_n54_), .c(new_n51_), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(new_n60_), .c(new_n49_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n37_), .O(z04));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  nor2   g31(.a(new_n59_), .b(new_n54_), .O(new_n66_));
  nor2   g32(.a(x09), .b(new_n51_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nor2   g34(.a(new_n50_), .b(new_n36_), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n65_), .O(z05));
  inv1   g36(.a(new_n64_), .O(new_n71_));
  nor2   g37(.a(x10), .b(x09), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g39(.a(new_n68_), .b(x10), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n69_), .O(z06));
  oai21  g41(.a(x11), .b(new_n36_), .c(new_n51_), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  nand3  g43(.a(new_n72_), .b(new_n71_), .c(new_n77_), .O(new_n78_));
  nand3  g44(.a(new_n72_), .b(x08), .c(x07), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x11), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n49_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x04), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n76_), .O(z07));
  nand2  g49(.a(new_n78_), .b(x12), .O(new_n84_));
  nor2   g50(.a(x12), .b(x11), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n72_), .c(new_n71_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n69_), .O(z08));
  oai21  g53(.a(x13), .b(new_n36_), .c(new_n51_), .O(new_n88_));
  nand4  g54(.a(new_n85_), .b(new_n72_), .c(x08), .d(x07), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x13), .O(new_n90_));
  nor2   g56(.a(x13), .b(x10), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n85_), .c(new_n67_), .d(new_n66_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n90_), .c(new_n49_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x04), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n88_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n91_), .b(new_n85_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  inv1   g64(.a(x09), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(x08), .c(x07), .O(new_n100_));
  aoi21  g66(.a(new_n48_), .b(x16), .c(new_n100_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n98_), .c(new_n36_), .O(new_n102_));
  nand3  g68(.a(new_n50_), .b(x14), .c(x04), .O(new_n103_));
  oai21  g69(.a(new_n102_), .b(new_n51_), .c(new_n103_), .O(z10));
  inv1   g70(.a(x02), .O(new_n105_));
  nand2  g71(.a(new_n37_), .b(new_n105_), .O(z11));
  nand2  g72(.a(x16), .b(new_n105_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n47_), .c(new_n36_), .O(z12));
  and2   g74(.a(x17), .b(x04), .O(z14));
  buf    g75(.a(x04), .O(z13));
endmodule


