// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n105_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x07), .O(new_n37_));
  inv1   g03(.a(x17), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  or2    g06(.a(x14), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n36_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(new_n40_), .O(new_n45_));
  aoi21  g11(.a(new_n43_), .b(x18), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  nor2   g14(.a(new_n39_), .b(new_n48_), .O(z13));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(z13), .c(new_n47_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z02));
  nor2   g21(.a(new_n52_), .b(new_n48_), .O(new_n56_));
  oai21  g22(.a(x17), .b(new_n37_), .c(new_n47_), .O(new_n57_));
  nand2  g23(.a(x07), .b(x06), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z03));
  nand2  g26(.a(new_n53_), .b(x04), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n58_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(x08), .b(x07), .c(x06), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n63_), .c(new_n40_), .O(new_n65_));
  nor2   g31(.a(new_n65_), .b(new_n61_), .O(z04));
  nand2  g32(.a(x08), .b(x06), .O(new_n67_));
  nor2   g33(.a(new_n67_), .b(x09), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(x17), .c(x07), .O(new_n69_));
  nand4  g35(.a(new_n38_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x09), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n69_), .c(new_n56_), .O(z05));
  nand2  g38(.a(new_n69_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nor2   g40(.a(x17), .b(new_n37_), .O(new_n75_));
  nand3  g41(.a(new_n68_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  nand2  g42(.a(new_n61_), .b(new_n40_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n73_), .O(z06));
  aoi21  g44(.a(new_n68_), .b(new_n74_), .c(x17), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n37_), .c(x11), .O(new_n80_));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n68_), .c(new_n75_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n80_), .c(new_n77_), .O(z07));
  aoi21  g49(.a(new_n81_), .b(new_n68_), .c(x17), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n37_), .c(x12), .O(new_n85_));
  inv1   g51(.a(x09), .O(new_n86_));
  inv1   g52(.a(x12), .O(new_n87_));
  inv1   g53(.a(new_n70_), .O(new_n88_));
  nand4  g54(.a(new_n81_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n85_), .c(new_n77_), .O(z08));
  nand2  g56(.a(new_n89_), .b(x13), .O(new_n91_));
  nor2   g57(.a(x13), .b(x12), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n81_), .c(new_n68_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n38_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x07), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n91_), .c(new_n56_), .O(z09));
  inv1   g62(.a(z13), .O(new_n97_));
  nand2  g63(.a(new_n52_), .b(x14), .O(new_n98_));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n92_), .b(new_n81_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(new_n88_), .c(new_n53_), .d(new_n86_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n98_), .c(new_n97_), .O(z10));
  nor2   g69(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n35_), .c(x03), .O(new_n105_));
  nor2   g71(.a(new_n105_), .b(new_n97_), .O(z12));
  aoi21  g72(.a(new_n37_), .b(new_n48_), .c(new_n38_), .O(z14));
endmodule


