// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n109_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  oai21  g02(.a(x06), .b(new_n35_), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(x02), .b(new_n35_), .c(new_n37_), .O(z00));
  nor2   g04(.a(x15), .b(x05), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(x18), .O(new_n40_));
  inv1   g06(.a(x06), .O(new_n41_));
  nor2   g07(.a(x14), .b(new_n41_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n40_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n41_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  xnor2a g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n49_), .c(new_n43_), .O(z03));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  xor2a  g21(.a(new_n55_), .b(x08), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n49_), .c(new_n43_), .O(z04));
  nand2  g23(.a(new_n49_), .b(new_n43_), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nor3   g25(.a(new_n59_), .b(new_n36_), .c(x09), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(new_n61_));
  nand2  g27(.a(x08), .b(x07), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x14), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x06), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n61_), .c(new_n58_), .O(z05));
  inv1   g32(.a(x09), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n67_), .c(x08), .d(x07), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x14), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x06), .O(new_n71_));
  nand2  g37(.a(new_n61_), .b(x10), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n50_), .O(z06));
  nand2  g39(.a(new_n71_), .b(x11), .O(new_n74_));
  inv1   g40(.a(x11), .O(new_n75_));
  nand3  g41(.a(x14), .b(new_n75_), .c(x06), .O(new_n76_));
  or2    g42(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n74_), .c(new_n58_), .O(z07));
  nor2   g44(.a(new_n62_), .b(x09), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n79_), .c(new_n36_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n41_), .c(x12), .O(new_n82_));
  nor3   g48(.a(x12), .b(x11), .c(x10), .O(new_n83_));
  aoi22  g49(.a(new_n83_), .b(new_n60_), .c(new_n49_), .d(new_n43_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n82_), .O(z08));
  nand2  g51(.a(new_n83_), .b(new_n60_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x13), .O(new_n87_));
  nand3  g53(.a(new_n67_), .b(x08), .c(x07), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n80_), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n88_), .c(x14), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x06), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n87_), .c(new_n50_), .O(z09));
  nor2   g60(.a(new_n42_), .b(x04), .O(new_n95_));
  nor2   g61(.a(new_n59_), .b(x09), .O(new_n96_));
  inv1   g62(.a(new_n91_), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(x00), .c(new_n96_), .O(new_n98_));
  inv1   g64(.a(x03), .O(new_n99_));
  inv1   g65(.a(x16), .O(new_n100_));
  aoi21  g66(.a(new_n99_), .b(x02), .c(new_n100_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(x14), .c(new_n42_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n98_), .c(new_n95_), .O(z10));
  nor2   g69(.a(new_n42_), .b(new_n47_), .O(z11));
  inv1   g70(.a(x04), .O(new_n105_));
  aoi21  g71(.a(x16), .b(new_n47_), .c(x03), .O(new_n106_));
  nor3   g72(.a(new_n106_), .b(new_n42_), .c(new_n105_), .O(z12));
  inv1   g73(.a(new_n95_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n105_), .c(new_n43_), .O(z14));
endmodule


