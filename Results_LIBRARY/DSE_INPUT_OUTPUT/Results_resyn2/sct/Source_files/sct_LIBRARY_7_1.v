// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n105_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x15), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x04), .O(new_n42_));
  nor2   g08(.a(new_n38_), .b(new_n42_), .O(new_n43_));
  nor2   g09(.a(x15), .b(x05), .O(new_n44_));
  aoi22  g10(.a(new_n44_), .b(x18), .c(new_n43_), .d(x05), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n37_), .c(new_n46_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  oai21  g18(.a(x03), .b(new_n35_), .c(x16), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x04), .O(new_n54_));
  xnor2a g20(.a(x07), .b(x06), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n54_), .c(new_n39_), .O(z03));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(x07), .b(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n57_), .c(new_n39_), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n54_), .O(z04));
  oai21  g28(.a(new_n38_), .b(x09), .c(new_n57_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n50_), .O(z05));
  oai21  g32(.a(new_n38_), .b(x10), .c(new_n65_), .O(new_n67_));
  inv1   g33(.a(x10), .O(new_n68_));
  inv1   g34(.a(new_n65_), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n67_), .c(new_n50_), .O(z06));
  nand4  g37(.a(new_n68_), .b(new_n64_), .c(x08), .d(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x11), .O(new_n73_));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  oai21  g41(.a(x15), .b(x11), .c(new_n46_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n75_), .c(new_n73_), .d(new_n50_), .O(z07));
  oai21  g43(.a(new_n72_), .b(x11), .c(x12), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nand3  g45(.a(new_n74_), .b(new_n69_), .c(new_n79_), .O(new_n80_));
  oai21  g46(.a(x15), .b(x12), .c(new_n46_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n80_), .c(new_n78_), .d(new_n50_), .O(z08));
  nand2  g48(.a(x13), .b(new_n46_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n53_), .c(x04), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n39_), .O(new_n85_));
  inv1   g51(.a(x13), .O(new_n86_));
  nand3  g52(.a(new_n64_), .b(x08), .c(x07), .O(new_n87_));
  inv1   g53(.a(x11), .O(new_n88_));
  nand3  g54(.a(new_n79_), .b(new_n88_), .c(new_n68_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n87_), .c(new_n86_), .O(new_n90_));
  nor2   g56(.a(x09), .b(new_n58_), .O(new_n91_));
  nor3   g57(.a(x12), .b(x11), .c(x10), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n91_), .c(x13), .d(x07), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n90_), .c(x06), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n85_), .O(z09));
  nor2   g61(.a(new_n38_), .b(x04), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand4  g63(.a(new_n86_), .b(new_n79_), .c(new_n88_), .d(new_n68_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand3  g65(.a(new_n99_), .b(new_n69_), .c(new_n53_), .O(new_n100_));
  aoi21  g66(.a(new_n49_), .b(x14), .c(new_n38_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n100_), .c(new_n96_), .O(z10));
  nor2   g68(.a(new_n38_), .b(new_n35_), .O(z11));
  nand2  g69(.a(new_n39_), .b(x04), .O(new_n104_));
  aoi21  g70(.a(x16), .b(new_n35_), .c(x03), .O(new_n105_));
  nor2   g71(.a(new_n105_), .b(new_n104_), .O(z12));
  inv1   g72(.a(new_n96_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  nor2   g74(.a(new_n104_), .b(new_n108_), .O(z14));
endmodule


