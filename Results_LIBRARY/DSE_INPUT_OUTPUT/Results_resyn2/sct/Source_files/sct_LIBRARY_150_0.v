// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n104_, new_n107_;
  nor2   g00(.a(x14), .b(x09), .O(new_n35_));
  inv1   g01(.a(new_n35_), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  nand2  g03(.a(x14), .b(new_n37_), .O(new_n38_));
  nand2  g04(.a(x02), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n36_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n36_), .O(z02));
  xnor2a g16(.a(x07), .b(x06), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n49_), .c(new_n36_), .O(z03));
  nand3  g18(.a(x08), .b(x07), .c(x06), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  aoi21  g20(.a(x07), .b(x06), .c(x08), .O(new_n55_));
  or2    g21(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n49_), .c(new_n36_), .O(z04));
  nand2  g23(.a(new_n49_), .b(new_n36_), .O(new_n58_));
  nand2  g24(.a(new_n53_), .b(x09), .O(new_n59_));
  inv1   g25(.a(x14), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(x09), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n59_), .c(new_n58_), .O(z05));
  inv1   g29(.a(x10), .O(new_n64_));
  nor2   g30(.a(new_n53_), .b(x09), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  oai21  g32(.a(new_n53_), .b(x09), .c(x10), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n48_), .c(x04), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n66_), .c(new_n35_), .O(z06));
  inv1   g36(.a(new_n49_), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  nor3   g38(.a(new_n53_), .b(x11), .c(x10), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(new_n60_), .c(new_n72_), .O(new_n74_));
  oai21  g40(.a(new_n62_), .b(x10), .c(x11), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n71_), .O(z07));
  inv1   g42(.a(x12), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n54_), .c(new_n77_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x14), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n72_), .O(new_n81_));
  nand3  g47(.a(new_n78_), .b(new_n61_), .c(new_n54_), .O(new_n82_));
  aoi21  g48(.a(new_n82_), .b(x12), .c(new_n49_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n81_), .O(z08));
  inv1   g50(.a(new_n61_), .O(new_n85_));
  oai21  g51(.a(new_n79_), .b(new_n85_), .c(x13), .O(new_n86_));
  inv1   g52(.a(x11), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n77_), .c(new_n87_), .d(new_n64_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n53_), .c(x14), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n72_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n86_), .c(new_n71_), .O(z09));
  inv1   g58(.a(x04), .O(new_n93_));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n65_), .O(new_n96_));
  inv1   g62(.a(x03), .O(new_n97_));
  inv1   g63(.a(x16), .O(new_n98_));
  aoi21  g64(.a(new_n97_), .b(x02), .c(new_n98_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(x14), .c(new_n35_), .O(new_n100_));
  aoi22  g66(.a(new_n100_), .b(new_n96_), .c(new_n36_), .d(new_n93_), .O(z10));
  nand2  g67(.a(new_n36_), .b(new_n47_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n47_), .c(x03), .O(new_n103_));
  nand2  g69(.a(new_n36_), .b(x04), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n103_), .O(z12));
  nand2  g71(.a(new_n36_), .b(new_n93_), .O(z13));
  inv1   g72(.a(x17), .O(new_n107_));
  nor2   g73(.a(new_n104_), .b(new_n107_), .O(z14));
endmodule


