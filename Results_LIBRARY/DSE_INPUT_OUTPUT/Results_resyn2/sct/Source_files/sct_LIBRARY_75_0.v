// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(z02));
  oai21  g20(.a(x03), .b(new_n36_), .c(x16), .O(new_n55_));
  inv1   g21(.a(x07), .O(new_n56_));
  oai21  g22(.a(new_n47_), .b(new_n48_), .c(new_n56_), .O(new_n57_));
  nor2   g23(.a(new_n56_), .b(new_n47_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n55_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n41_), .O(z03));
  nand2  g27(.a(new_n58_), .b(x08), .O(new_n62_));
  inv1   g28(.a(x08), .O(new_n63_));
  nand2  g29(.a(new_n59_), .b(new_n63_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n62_), .c(new_n52_), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(z04));
  inv1   g32(.a(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(x08), .c(x07), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x04), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x06), .O(new_n70_));
  aoi21  g36(.a(new_n62_), .b(x09), .c(new_n51_), .O(new_n71_));
  oai21  g37(.a(new_n71_), .b(new_n48_), .c(new_n70_), .O(z05));
  nand4  g38(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x10), .O(new_n74_));
  inv1   g40(.a(x10), .O(new_n75_));
  nand4  g41(.a(new_n58_), .b(new_n75_), .c(new_n67_), .d(x08), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n52_), .O(z06));
  inv1   g43(.a(x11), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n68_), .c(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  aoi21  g47(.a(new_n76_), .b(x11), .c(new_n51_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n48_), .c(new_n81_), .O(z07));
  inv1   g49(.a(x12), .O(new_n84_));
  nor2   g50(.a(new_n79_), .b(new_n73_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  oai21  g52(.a(new_n79_), .b(new_n73_), .c(x12), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n52_), .O(z08));
  nand2  g54(.a(new_n86_), .b(x13), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n85_), .b(new_n90_), .c(new_n84_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n89_), .c(new_n52_), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n90_), .b(new_n84_), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n79_), .c(new_n93_), .O(new_n95_));
  nor2   g61(.a(new_n73_), .b(new_n51_), .O(new_n96_));
  inv1   g62(.a(x14), .O(new_n97_));
  nor2   g63(.a(new_n55_), .b(new_n97_), .O(new_n98_));
  aoi21  g64(.a(new_n96_), .b(new_n95_), .c(new_n98_), .O(new_n99_));
  oai21  g65(.a(new_n99_), .b(new_n48_), .c(new_n41_), .O(z10));
  nor2   g66(.a(new_n37_), .b(new_n36_), .O(z11));
  nand2  g67(.a(x16), .b(new_n36_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n49_), .c(new_n48_), .O(z12));
  nand2  g69(.a(x06), .b(new_n48_), .O(z13));
  and2   g70(.a(x17), .b(x04), .O(z14));
endmodule


