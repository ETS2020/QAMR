// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n105_, new_n108_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x06), .b(x05), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x15), .O(new_n42_));
  nand3  g08(.a(x18), .b(new_n42_), .c(new_n41_), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n38_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x16), .c(new_n48_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n47_), .c(x05), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  oai21  g21(.a(x06), .b(new_n41_), .c(x07), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n51_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  oai22  g25(.a(new_n37_), .b(new_n59_), .c(new_n54_), .d(new_n47_), .O(new_n60_));
  and2   g26(.a(x07), .b(x06), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x08), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n51_), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  oai21  g30(.a(new_n37_), .b(x09), .c(new_n62_), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n51_), .O(z05));
  inv1   g34(.a(x10), .O(new_n69_));
  nand4  g35(.a(new_n61_), .b(new_n69_), .c(new_n66_), .d(x08), .O(new_n70_));
  nand2  g36(.a(new_n67_), .b(x10), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n51_), .O(new_n72_));
  and2   g38(.a(new_n72_), .b(new_n38_), .O(z06));
  nand3  g39(.a(new_n66_), .b(x08), .c(x07), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(x10), .c(x11), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  inv1   g42(.a(new_n67_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n69_), .O(new_n78_));
  oai21  g44(.a(x11), .b(new_n41_), .c(new_n47_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(new_n75_), .d(new_n51_), .O(z07));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n66_), .c(x08), .d(x07), .O(new_n82_));
  xor2a  g48(.a(new_n82_), .b(x12), .O(new_n83_));
  oai21  g49(.a(x03), .b(new_n35_), .c(x16), .O(new_n84_));
  nand2  g50(.a(x12), .b(new_n47_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(x04), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n38_), .O(new_n87_));
  oai21  g53(.a(new_n83_), .b(new_n47_), .c(new_n87_), .O(z08));
  oai21  g54(.a(new_n82_), .b(x12), .c(x13), .O(new_n89_));
  nor2   g55(.a(x13), .b(x12), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n77_), .c(new_n76_), .d(new_n69_), .O(new_n91_));
  oai21  g57(.a(x13), .b(new_n41_), .c(new_n47_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n91_), .c(new_n89_), .d(new_n51_), .O(z09));
  nor2   g59(.a(new_n37_), .b(x04), .O(new_n94_));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(new_n81_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n77_), .c(new_n84_), .O(new_n98_));
  inv1   g64(.a(x16), .O(new_n99_));
  aoi21  g65(.a(new_n49_), .b(x02), .c(new_n99_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x14), .c(new_n37_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n98_), .c(new_n94_), .O(z10));
  nor2   g68(.a(new_n37_), .b(new_n35_), .O(z11));
  nand2  g69(.a(x16), .b(new_n35_), .O(new_n104_));
  nor2   g70(.a(new_n37_), .b(x03), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(new_n94_), .O(z12));
  inv1   g72(.a(new_n94_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n48_), .c(new_n38_), .O(z14));
endmodule


