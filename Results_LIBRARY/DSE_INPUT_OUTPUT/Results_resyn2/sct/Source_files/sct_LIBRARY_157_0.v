// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n104_, new_n106_;
  inv1   g00(.a(x01), .O(new_n35_));
  nand2  g01(.a(x04), .b(new_n35_), .O(new_n36_));
  oai22  g02(.a(new_n36_), .b(x14), .c(x02), .d(new_n35_), .O(z00));
  nor2   g03(.a(x15), .b(x05), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(x18), .O(new_n39_));
  inv1   g05(.a(x04), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  nand2  g07(.a(x05), .b(x04), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n41_), .c(new_n39_), .O(new_n43_));
  inv1   g09(.a(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x02), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n46_), .c(x16), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n40_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n41_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n45_), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  nand2  g20(.a(new_n52_), .b(new_n45_), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n54_), .c(new_n49_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n54_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n49_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z04));
  nand2  g28(.a(new_n58_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n49_), .O(z05));
  nor2   g32(.a(x04), .b(new_n35_), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(z13));
  nand2  g34(.a(new_n65_), .b(x10), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  nor2   g36(.a(x09), .b(new_n59_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n53_), .c(new_n70_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n69_), .c(new_n47_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x04), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(z13), .O(z06));
  nand2  g41(.a(new_n72_), .b(x11), .O(new_n76_));
  inv1   g42(.a(new_n65_), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n76_), .c(new_n49_), .O(z07));
  inv1   g46(.a(x11), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n70_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n65_), .c(x12), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  inv1   g50(.a(new_n58_), .O(new_n85_));
  nand4  g51(.a(new_n78_), .b(new_n85_), .c(new_n84_), .d(new_n64_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n83_), .c(new_n47_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x04), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(z13), .O(z08));
  nand4  g55(.a(new_n84_), .b(new_n81_), .c(new_n70_), .d(new_n64_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n58_), .c(x13), .O(new_n91_));
  nor2   g57(.a(x13), .b(x12), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n78_), .c(new_n71_), .d(new_n53_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n91_), .c(new_n47_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x04), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(z13), .O(z09));
  nand2  g62(.a(new_n48_), .b(x14), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n92_), .b(new_n78_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n77_), .c(new_n47_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n97_), .c(new_n40_), .O(z10));
  aoi21  g68(.a(new_n40_), .b(new_n35_), .c(new_n46_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n46_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n40_), .c(new_n41_), .O(z12));
  nor2   g71(.a(x17), .b(new_n40_), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n67_), .O(z14));
endmodule


