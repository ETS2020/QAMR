// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n99_, new_n102_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x17), .b(x16), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(new_n37_), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  oai21  g14(.a(new_n48_), .b(x06), .c(new_n46_), .O(z02));
  inv1   g15(.a(x06), .O(new_n50_));
  inv1   g16(.a(x07), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  nand2  g19(.a(new_n51_), .b(new_n50_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n48_), .c(new_n46_), .O(z03));
  xor2a  g22(.a(new_n53_), .b(x08), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n48_), .c(new_n46_), .O(z04));
  inv1   g24(.a(x03), .O(new_n59_));
  inv1   g25(.a(x16), .O(new_n60_));
  aoi21  g26(.a(new_n59_), .b(x02), .c(new_n60_), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n43_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n62_), .O(new_n67_));
  and2   g33(.a(new_n67_), .b(new_n46_), .O(z05));
  inv1   g34(.a(x10), .O(new_n69_));
  nand4  g35(.a(new_n52_), .b(new_n69_), .c(new_n65_), .d(x08), .O(new_n70_));
  aoi21  g36(.a(new_n66_), .b(x10), .c(new_n48_), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n70_), .c(new_n37_), .O(z06));
  inv1   g38(.a(x11), .O(new_n73_));
  inv1   g39(.a(new_n66_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n69_), .O(new_n75_));
  aoi21  g41(.a(new_n70_), .b(x11), .c(new_n48_), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n75_), .c(new_n37_), .O(z07));
  nand2  g43(.a(new_n75_), .b(x12), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n65_), .O(new_n81_));
  nor2   g47(.a(new_n81_), .b(new_n63_), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(new_n48_), .O(new_n83_));
  aoi21  g49(.a(new_n83_), .b(new_n78_), .c(new_n37_), .O(z08));
  oai21  g50(.a(new_n81_), .b(new_n63_), .c(x13), .O(new_n85_));
  inv1   g51(.a(x13), .O(new_n86_));
  nand3  g52(.a(new_n80_), .b(new_n86_), .c(new_n79_), .O(new_n87_));
  inv1   g53(.a(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n74_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n85_), .c(new_n62_), .d(new_n46_), .O(z09));
  inv1   g56(.a(x00), .O(new_n91_));
  nand2  g57(.a(new_n87_), .b(new_n91_), .O(new_n92_));
  nor2   g58(.a(new_n66_), .b(new_n61_), .O(new_n93_));
  inv1   g59(.a(x14), .O(new_n94_));
  nor2   g60(.a(new_n47_), .b(new_n94_), .O(new_n95_));
  aoi21  g61(.a(new_n93_), .b(new_n92_), .c(new_n95_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n43_), .c(new_n46_), .O(z10));
  nand2  g63(.a(new_n46_), .b(new_n36_), .O(z11));
  nand2  g64(.a(x17), .b(x03), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n47_), .c(new_n43_), .O(z12));
  nor2   g66(.a(new_n37_), .b(new_n43_), .O(z13));
  nand2  g67(.a(x17), .b(x04), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(new_n46_), .O(z14));
endmodule


