// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  inv1   g06(.a(x14), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g08(.a(new_n42_), .b(new_n39_), .c(new_n36_), .O(z00));
  oai21  g09(.a(new_n38_), .b(x05), .c(new_n37_), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  nor2   g11(.a(new_n45_), .b(x15), .O(new_n46_));
  oai21  g12(.a(new_n46_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n38_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  oai21  g19(.a(x03), .b(new_n35_), .c(x16), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  oai21  g21(.a(new_n38_), .b(new_n37_), .c(new_n55_), .O(new_n56_));
  nor2   g22(.a(new_n55_), .b(new_n38_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n39_), .O(z03));
  nand3  g26(.a(x08), .b(x07), .c(x06), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n58_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n51_), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(z04));
  nand2  g31(.a(new_n61_), .b(x09), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n51_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n67_), .c(x08), .d(x07), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x04), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x06), .O(new_n73_));
  nand2  g39(.a(new_n54_), .b(new_n70_), .O(new_n74_));
  nor2   g40(.a(new_n68_), .b(new_n50_), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(x04), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n73_), .O(z06));
  nand4  g44(.a(new_n57_), .b(new_n70_), .c(new_n67_), .d(x08), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(x11), .c(new_n50_), .O(new_n80_));
  oai21  g46(.a(new_n71_), .b(x11), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  oai21  g48(.a(new_n80_), .b(new_n37_), .c(new_n82_), .O(z07));
  oai21  g49(.a(new_n79_), .b(x11), .c(x12), .O(new_n84_));
  inv1   g50(.a(new_n61_), .O(new_n85_));
  nor2   g51(.a(x11), .b(x10), .O(new_n86_));
  nor2   g52(.a(x12), .b(x09), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n84_), .c(new_n51_), .O(z08));
  nand3  g55(.a(new_n67_), .b(x08), .c(x07), .O(new_n90_));
  nor2   g56(.a(x13), .b(x12), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n86_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n90_), .c(x04), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x06), .O(new_n94_));
  aoi21  g60(.a(new_n88_), .b(x13), .c(new_n50_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n37_), .c(new_n94_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nor2   g64(.a(new_n54_), .b(new_n41_), .O(new_n99_));
  aoi21  g65(.a(new_n98_), .b(new_n75_), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n37_), .c(new_n39_), .O(z10));
  nand2  g67(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n35_), .c(x03), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n37_), .c(new_n39_), .O(z12));
  and2   g70(.a(x17), .b(x04), .O(z14));
  buf    g71(.a(x04), .O(z13));
endmodule


