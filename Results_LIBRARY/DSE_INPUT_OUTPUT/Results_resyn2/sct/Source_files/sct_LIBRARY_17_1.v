// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x08), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(new_n37_), .c(x06), .O(z02));
  inv1   g15(.a(new_n48_), .O(new_n50_));
  inv1   g16(.a(x06), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  oai21  g18(.a(x08), .b(new_n52_), .c(new_n51_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xor2a  g22(.a(x08), .b(x07), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n50_), .c(x06), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z04));
  nand2  g25(.a(x08), .b(x07), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(x09), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(new_n62_));
  nand2  g28(.a(new_n60_), .b(x09), .O(new_n63_));
  nand4  g29(.a(new_n63_), .b(new_n62_), .c(new_n50_), .d(new_n39_), .O(z05));
  oai21  g30(.a(new_n62_), .b(new_n51_), .c(x10), .O(new_n65_));
  inv1   g31(.a(x10), .O(new_n66_));
  oai21  g32(.a(new_n61_), .b(new_n38_), .c(new_n66_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(new_n50_), .O(z06));
  nand3  g34(.a(new_n61_), .b(new_n66_), .c(x06), .O(new_n69_));
  oai21  g35(.a(new_n38_), .b(x11), .c(new_n69_), .O(new_n70_));
  nor2   g36(.a(x11), .b(x10), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n61_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n50_), .O(z07));
  xor2a  g39(.a(new_n72_), .b(x12), .O(new_n74_));
  oai21  g40(.a(x12), .b(x08), .c(new_n51_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n50_), .O(z08));
  inv1   g42(.a(x11), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n66_), .O(new_n78_));
  inv1   g44(.a(x09), .O(new_n79_));
  inv1   g45(.a(x12), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(x07), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n78_), .c(x13), .O(new_n82_));
  nor2   g48(.a(x13), .b(x12), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n71_), .c(new_n61_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(x08), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n48_), .c(new_n39_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n86_), .O(z09));
  nor2   g56(.a(new_n38_), .b(x04), .O(new_n91_));
  inv1   g57(.a(x00), .O(new_n92_));
  nand2  g58(.a(new_n83_), .b(new_n71_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n61_), .c(new_n47_), .O(new_n95_));
  inv1   g61(.a(x03), .O(new_n96_));
  inv1   g62(.a(x16), .O(new_n97_));
  aoi21  g63(.a(new_n96_), .b(x02), .c(new_n97_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(x14), .c(new_n38_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n95_), .c(new_n91_), .O(z10));
  nor2   g66(.a(new_n38_), .b(new_n35_), .O(z11));
  nand2  g67(.a(x16), .b(new_n35_), .O(new_n102_));
  nor2   g68(.a(new_n38_), .b(x03), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n91_), .O(z12));
  inv1   g70(.a(new_n91_), .O(z13));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n38_), .O(z14));
endmodule


