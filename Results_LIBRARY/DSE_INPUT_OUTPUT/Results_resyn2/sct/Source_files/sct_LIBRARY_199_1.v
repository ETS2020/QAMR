// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n102_,
    new_n104_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x04), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x04), .O(new_n42_));
  oai21  g08(.a(x06), .b(x05), .c(new_n42_), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  and2   g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n37_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n50_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nand2  g24(.a(new_n53_), .b(x08), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n54_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n50_), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  oai21  g29(.a(x09), .b(new_n42_), .c(new_n59_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nor2   g33(.a(new_n67_), .b(new_n49_), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(new_n64_), .c(new_n38_), .O(z05));
  oai21  g35(.a(x10), .b(new_n42_), .c(new_n37_), .O(new_n70_));
  oai21  g36(.a(x03), .b(new_n35_), .c(x16), .O(new_n71_));
  nand3  g37(.a(new_n65_), .b(x08), .c(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand4  g40(.a(new_n53_), .b(new_n74_), .c(new_n65_), .d(x08), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x04), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n70_), .O(z06));
  oai22  g44(.a(new_n66_), .b(x10), .c(x11), .d(new_n42_), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n67_), .c(new_n49_), .O(new_n81_));
  aoi21  g47(.a(new_n81_), .b(new_n79_), .c(new_n38_), .O(z07));
  inv1   g48(.a(x12), .O(new_n83_));
  nand3  g49(.a(new_n80_), .b(new_n67_), .c(new_n83_), .O(new_n84_));
  nand2  g50(.a(new_n80_), .b(new_n67_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x12), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n84_), .c(new_n50_), .O(z08));
  nand2  g53(.a(new_n84_), .b(x13), .O(new_n88_));
  inv1   g54(.a(x13), .O(new_n89_));
  nand3  g55(.a(new_n80_), .b(new_n89_), .c(new_n83_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n67_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n88_), .c(new_n50_), .O(z09));
  nand2  g59(.a(new_n49_), .b(x14), .O(new_n94_));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n67_), .c(new_n71_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n94_), .c(new_n42_), .O(z10));
  nand2  g64(.a(new_n39_), .b(new_n35_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n35_), .c(x03), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n42_), .c(new_n39_), .O(z12));
  nor2   g67(.a(x06), .b(x04), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(z13));
  inv1   g69(.a(x17), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(x04), .c(new_n102_), .O(z14));
endmodule


