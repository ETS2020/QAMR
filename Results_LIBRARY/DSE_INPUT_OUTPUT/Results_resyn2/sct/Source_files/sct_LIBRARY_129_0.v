// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n111_, new_n112_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(x07), .b(x06), .O(new_n37_));
  aoi21  g03(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(new_n37_), .O(new_n40_));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  inv1   g11(.a(x04), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(x07), .c(x06), .O(z02));
  inv1   g18(.a(x02), .O(new_n53_));
  oai21  g19(.a(x03), .b(new_n53_), .c(x16), .O(new_n54_));
  nor2   g20(.a(new_n37_), .b(new_n46_), .O(z13));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(z13), .c(new_n54_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n56_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n50_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n40_), .O(z04));
  oai21  g29(.a(new_n37_), .b(x09), .c(new_n59_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n50_), .O(z05));
  inv1   g33(.a(x06), .O(new_n68_));
  inv1   g34(.a(x07), .O(new_n69_));
  oai21  g35(.a(x10), .b(new_n69_), .c(new_n68_), .O(new_n70_));
  nor2   g36(.a(x09), .b(new_n60_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  inv1   g40(.a(new_n66_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand4  g42(.a(new_n76_), .b(new_n73_), .c(new_n70_), .d(new_n50_), .O(z06));
  inv1   g43(.a(x11), .O(new_n78_));
  nand3  g44(.a(new_n54_), .b(new_n78_), .c(x04), .O(new_n79_));
  aoi21  g45(.a(new_n79_), .b(x06), .c(x07), .O(new_n80_));
  nand2  g46(.a(new_n76_), .b(x11), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n71_), .c(x06), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n54_), .c(x04), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  aoi21  g51(.a(new_n85_), .b(new_n81_), .c(new_n80_), .O(z07));
  nand3  g52(.a(new_n82_), .b(new_n71_), .c(x07), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x12), .O(new_n88_));
  nor3   g54(.a(x12), .b(x11), .c(x10), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n75_), .O(new_n90_));
  oai21  g56(.a(x12), .b(new_n69_), .c(new_n68_), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n90_), .c(new_n88_), .d(new_n50_), .O(z08));
  inv1   g58(.a(x13), .O(new_n93_));
  nand3  g59(.a(new_n54_), .b(new_n93_), .c(x04), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(x06), .c(x07), .O(new_n95_));
  nand3  g61(.a(new_n89_), .b(new_n71_), .c(x06), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nor2   g63(.a(new_n93_), .b(new_n68_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n89_), .c(new_n71_), .d(x07), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n50_), .c(new_n95_), .O(z09));
  inv1   g67(.a(x00), .O(new_n102_));
  nor2   g68(.a(x13), .b(x12), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n82_), .O(new_n104_));
  nand2  g70(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nor2   g71(.a(new_n66_), .b(new_n49_), .O(new_n106_));
  nor2   g72(.a(new_n54_), .b(new_n36_), .O(new_n107_));
  aoi21  g73(.a(new_n106_), .b(new_n105_), .c(new_n107_), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n46_), .c(new_n40_), .O(z10));
  nand2  g75(.a(new_n40_), .b(new_n53_), .O(z11));
  inv1   g76(.a(z13), .O(new_n111_));
  aoi21  g77(.a(x16), .b(new_n53_), .c(x03), .O(new_n112_));
  nor2   g78(.a(new_n112_), .b(new_n111_), .O(z12));
  and2   g79(.a(z13), .b(x17), .O(z14));
endmodule


