// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n103_,
    new_n105_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x04), .O(new_n41_));
  inv1   g07(.a(x06), .O(new_n42_));
  oai21  g08(.a(new_n42_), .b(x05), .c(new_n41_), .O(new_n43_));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x04), .c(x06), .O(z02));
  nor2   g16(.a(new_n49_), .b(new_n41_), .O(new_n51_));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n42_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n52_), .c(new_n51_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(new_n49_), .O(new_n57_));
  nand2  g23(.a(x06), .b(new_n41_), .O(z13));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n52_), .b(new_n60_), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(new_n59_), .c(z13), .d(new_n57_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n38_), .O(z04));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(x08), .c(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  aoi21  g34(.a(new_n68_), .b(x09), .c(new_n49_), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n41_), .c(new_n67_), .O(z05));
  oai21  g36(.a(new_n68_), .b(x09), .c(x10), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(new_n59_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n64_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n71_), .c(new_n51_), .O(z06));
  inv1   g41(.a(x11), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n65_), .c(x04), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  aoi21  g45(.a(new_n74_), .b(x11), .c(new_n49_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n41_), .c(new_n79_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n76_), .c(new_n72_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n65_), .c(x04), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x06), .O(new_n85_));
  nor2   g51(.a(x11), .b(x10), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n73_), .c(new_n64_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(x12), .c(new_n49_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n41_), .c(new_n85_), .O(z08));
  oai21  g55(.a(new_n87_), .b(x12), .c(x13), .O(new_n90_));
  nor2   g56(.a(new_n68_), .b(x09), .O(new_n91_));
  inv1   g57(.a(x13), .O(new_n92_));
  nand3  g58(.a(new_n86_), .b(new_n92_), .c(new_n82_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n90_), .c(new_n51_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n93_), .b(new_n97_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n91_), .c(new_n57_), .O(new_n99_));
  nand2  g65(.a(new_n49_), .b(x14), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n99_), .c(new_n41_), .O(z10));
  nor2   g67(.a(new_n37_), .b(new_n35_), .O(z11));
  nand2  g68(.a(x16), .b(new_n35_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n47_), .c(new_n41_), .O(z12));
  nor2   g70(.a(x17), .b(new_n41_), .O(new_n105_));
  aoi21  g71(.a(x06), .b(new_n41_), .c(new_n105_), .O(z14));
endmodule


