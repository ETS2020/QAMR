// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n102_,
    new_n105_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  inv1   g02(.a(x12), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  nand2  g06(.a(x14), .b(new_n40_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(z00));
  inv1   g09(.a(x18), .O(new_n44_));
  nor2   g10(.a(new_n44_), .b(x15), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  aoi21  g12(.a(x05), .b(new_n46_), .c(new_n38_), .O(new_n47_));
  oai21  g13(.a(new_n45_), .b(x05), .c(new_n47_), .O(z01));
  nor2   g14(.a(x12), .b(new_n36_), .O(new_n49_));
  inv1   g15(.a(x02), .O(new_n50_));
  oai21  g16(.a(x03), .b(new_n50_), .c(x16), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(x04), .O(new_n52_));
  aoi21  g18(.a(new_n52_), .b(new_n36_), .c(new_n49_), .O(z02));
  inv1   g19(.a(x03), .O(new_n54_));
  inv1   g20(.a(x16), .O(new_n55_));
  aoi21  g21(.a(new_n54_), .b(x02), .c(new_n55_), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(new_n46_), .O(new_n57_));
  inv1   g23(.a(x07), .O(new_n58_));
  oai21  g24(.a(x12), .b(new_n36_), .c(new_n58_), .O(new_n59_));
  nand2  g25(.a(x07), .b(x06), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(z03));
  xor2a  g28(.a(new_n60_), .b(x08), .O(new_n63_));
  oai21  g29(.a(new_n63_), .b(new_n52_), .c(new_n39_), .O(z04));
  nand2  g30(.a(x08), .b(x07), .O(new_n65_));
  nor2   g31(.a(new_n65_), .b(x09), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(x12), .c(x06), .O(new_n67_));
  nand3  g33(.a(x08), .b(x07), .c(x06), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(x12), .c(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(new_n57_), .O(z05));
  nand2  g36(.a(new_n67_), .b(x10), .O(new_n71_));
  nand2  g37(.a(new_n52_), .b(new_n39_), .O(new_n72_));
  inv1   g38(.a(new_n68_), .O(new_n73_));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n37_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n72_), .c(new_n71_), .O(z06));
  and2   g42(.a(x08), .b(x07), .O(new_n77_));
  aoi21  g43(.a(new_n74_), .b(new_n77_), .c(x12), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n36_), .c(x11), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  nand4  g46(.a(new_n74_), .b(new_n73_), .c(new_n37_), .d(new_n80_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n79_), .c(new_n72_), .O(z07));
  nand3  g48(.a(new_n74_), .b(new_n73_), .c(new_n80_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n57_), .c(new_n37_), .O(z08));
  inv1   g50(.a(x13), .O(new_n85_));
  nand2  g51(.a(new_n74_), .b(new_n80_), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n68_), .c(new_n85_), .O(new_n87_));
  nand4  g53(.a(new_n74_), .b(new_n77_), .c(x13), .d(new_n80_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n37_), .O(new_n89_));
  nand2  g55(.a(x13), .b(new_n36_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n89_), .c(new_n72_), .O(z09));
  nor2   g57(.a(new_n38_), .b(x04), .O(new_n92_));
  inv1   g58(.a(x00), .O(new_n93_));
  inv1   g59(.a(x10), .O(new_n94_));
  nand3  g60(.a(new_n85_), .b(new_n80_), .c(new_n94_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand4  g62(.a(new_n96_), .b(new_n66_), .c(new_n51_), .d(x06), .O(new_n97_));
  aoi21  g63(.a(new_n56_), .b(x14), .c(new_n38_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n97_), .c(new_n92_), .O(z10));
  nand2  g65(.a(new_n39_), .b(new_n50_), .O(z11));
  nand2  g66(.a(x16), .b(new_n50_), .O(new_n101_));
  nor2   g67(.a(new_n38_), .b(x03), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n92_), .O(z12));
  inv1   g69(.a(new_n92_), .O(z13));
  inv1   g70(.a(x17), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n46_), .c(new_n39_), .O(z14));
endmodule


