// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n99_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  aoi21  g03(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x02), .b(new_n35_), .c(new_n38_), .O(z00));
  inv1   g05(.a(x04), .O(new_n40_));
  inv1   g06(.a(x06), .O(new_n41_));
  oai21  g07(.a(new_n41_), .b(x05), .c(new_n40_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(x04), .c(x06), .O(z02));
  nor2   g15(.a(new_n48_), .b(new_n40_), .O(new_n50_));
  inv1   g16(.a(x07), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n41_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  nand2  g19(.a(new_n51_), .b(new_n41_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  inv1   g22(.a(new_n37_), .O(new_n57_));
  aoi21  g23(.a(x06), .b(new_n40_), .c(new_n48_), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n58_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n57_), .O(z04));
  nand2  g29(.a(new_n59_), .b(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n50_), .O(z05));
  nand2  g33(.a(new_n66_), .b(x10), .O(new_n68_));
  inv1   g34(.a(x10), .O(new_n69_));
  nand4  g35(.a(new_n52_), .b(new_n69_), .c(new_n65_), .d(x08), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n50_), .O(z06));
  nor2   g37(.a(x11), .b(x10), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  nand3  g39(.a(new_n65_), .b(x08), .c(x07), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n73_), .c(x04), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  aoi21  g42(.a(new_n70_), .b(x11), .c(new_n48_), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(new_n40_), .c(new_n76_), .O(z07));
  oai21  g44(.a(new_n70_), .b(x11), .c(x12), .O(new_n79_));
  inv1   g45(.a(new_n59_), .O(new_n80_));
  nor2   g46(.a(x12), .b(x09), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n72_), .c(new_n80_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n79_), .c(new_n50_), .O(z08));
  nor2   g49(.a(x13), .b(x12), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n72_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n74_), .c(x04), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x06), .O(new_n87_));
  aoi21  g53(.a(new_n82_), .b(x13), .c(new_n48_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n40_), .c(new_n87_), .O(z09));
  inv1   g55(.a(x00), .O(new_n90_));
  nand2  g56(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  nor2   g57(.a(new_n66_), .b(new_n48_), .O(new_n92_));
  inv1   g58(.a(x02), .O(new_n93_));
  oai21  g59(.a(x03), .b(new_n93_), .c(x16), .O(new_n94_));
  nor2   g60(.a(new_n94_), .b(new_n36_), .O(new_n95_));
  aoi21  g61(.a(new_n92_), .b(new_n91_), .c(new_n95_), .O(new_n96_));
  oai21  g62(.a(new_n96_), .b(new_n40_), .c(new_n57_), .O(z10));
  nor2   g63(.a(new_n37_), .b(new_n93_), .O(z11));
  nand2  g64(.a(x16), .b(new_n93_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n46_), .c(new_n40_), .O(z12));
  and2   g66(.a(x17), .b(x04), .O(z14));
  buf    g67(.a(x04), .O(z13));
endmodule


