// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n103_;
  inv1   g00(.a(x14), .O(new_n35_));
  nor2   g01(.a(x18), .b(x14), .O(new_n36_));
  aoi21  g02(.a(x02), .b(x01), .c(new_n36_), .O(new_n37_));
  oai21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x04), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  nor2   g09(.a(x15), .b(x05), .O(new_n44_));
  aoi22  g10(.a(new_n44_), .b(x18), .c(new_n43_), .d(x05), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x04), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n41_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n41_), .O(z03));
  nor2   g24(.a(new_n50_), .b(new_n42_), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n54_), .b(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n59_), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  inv1   g30(.a(x02), .O(new_n65_));
  oai21  g31(.a(x03), .b(new_n65_), .c(x16), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  inv1   g33(.a(new_n60_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g35(.a(new_n60_), .b(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(new_n69_), .c(new_n66_), .d(x04), .O(new_n71_));
  and2   g37(.a(new_n71_), .b(new_n41_), .O(z05));
  oai21  g38(.a(new_n60_), .b(x09), .c(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand3  g40(.a(new_n68_), .b(new_n74_), .c(new_n67_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n73_), .c(new_n66_), .d(x04), .O(new_n76_));
  and2   g42(.a(new_n76_), .b(new_n41_), .O(z06));
  nand2  g43(.a(new_n75_), .b(x11), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand4  g45(.a(new_n68_), .b(new_n79_), .c(new_n74_), .d(new_n67_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n59_), .O(z07));
  nand2  g47(.a(new_n80_), .b(x12), .O(new_n82_));
  nor2   g48(.a(x12), .b(x11), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n68_), .c(new_n74_), .d(new_n67_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n82_), .c(new_n59_), .O(z08));
  nand2  g51(.a(new_n84_), .b(x13), .O(new_n86_));
  nor2   g52(.a(new_n60_), .b(x09), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand3  g54(.a(new_n83_), .b(new_n88_), .c(new_n74_), .O(new_n89_));
  inv1   g55(.a(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n86_), .c(new_n59_), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand2  g59(.a(new_n89_), .b(new_n93_), .O(new_n94_));
  aoi21  g60(.a(new_n94_), .b(new_n87_), .c(new_n50_), .O(new_n95_));
  oai21  g61(.a(new_n50_), .b(new_n40_), .c(new_n35_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x04), .O(new_n97_));
  nor2   g63(.a(new_n97_), .b(new_n95_), .O(z10));
  nor2   g64(.a(new_n36_), .b(new_n65_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n65_), .c(x03), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(new_n42_), .O(z12));
  nand2  g67(.a(new_n41_), .b(new_n47_), .O(z13));
  inv1   g68(.a(x17), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n47_), .c(new_n41_), .O(z14));
endmodule


