// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n94_, new_n95_,
    new_n98_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x18), .b(x16), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  and2   g07(.a(x05), .b(x04), .O(new_n42_));
  aoi22  g08(.a(new_n42_), .b(new_n38_), .c(new_n41_), .d(x18), .O(z01));
  inv1   g09(.a(x06), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(new_n38_), .c(new_n44_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n44_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand4  g20(.a(new_n54_), .b(new_n53_), .c(new_n49_), .d(new_n38_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  oai21  g22(.a(x03), .b(new_n35_), .c(x16), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(x04), .O(new_n58_));
  xor2a  g24(.a(new_n54_), .b(x08), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n58_), .c(new_n38_), .O(z04));
  inv1   g26(.a(x09), .O(new_n61_));
  nand4  g27(.a(new_n61_), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n62_), .c(new_n49_), .d(new_n38_), .O(z05));
  inv1   g31(.a(new_n63_), .O(new_n66_));
  nor2   g32(.a(x10), .b(x09), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g34(.a(new_n62_), .b(x10), .O(new_n69_));
  nand4  g35(.a(new_n69_), .b(new_n68_), .c(new_n49_), .d(new_n38_), .O(z06));
  inv1   g36(.a(x11), .O(new_n71_));
  nand3  g37(.a(new_n67_), .b(new_n66_), .c(new_n71_), .O(new_n72_));
  nand2  g38(.a(new_n68_), .b(x11), .O(new_n73_));
  nand4  g39(.a(new_n73_), .b(new_n72_), .c(new_n49_), .d(new_n38_), .O(z07));
  nand2  g40(.a(new_n72_), .b(x12), .O(new_n75_));
  inv1   g41(.a(x12), .O(new_n76_));
  nand4  g42(.a(new_n67_), .b(new_n66_), .c(new_n76_), .d(new_n71_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n75_), .c(new_n49_), .d(new_n38_), .O(z08));
  nand2  g44(.a(new_n77_), .b(x13), .O(new_n79_));
  inv1   g45(.a(x10), .O(new_n80_));
  inv1   g46(.a(x13), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n76_), .c(new_n71_), .d(new_n80_), .O(new_n82_));
  nor2   g48(.a(new_n82_), .b(new_n62_), .O(new_n83_));
  nor2   g49(.a(new_n83_), .b(new_n58_), .O(new_n84_));
  aoi21  g50(.a(new_n84_), .b(new_n79_), .c(new_n37_), .O(z09));
  nor2   g51(.a(new_n37_), .b(x04), .O(new_n86_));
  inv1   g52(.a(new_n62_), .O(new_n87_));
  inv1   g53(.a(x00), .O(new_n88_));
  nand2  g54(.a(new_n82_), .b(new_n88_), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n87_), .c(new_n57_), .O(new_n90_));
  aoi21  g56(.a(new_n48_), .b(x14), .c(new_n37_), .O(new_n91_));
  aoi21  g57(.a(new_n91_), .b(new_n90_), .c(new_n86_), .O(z10));
  nand2  g58(.a(new_n38_), .b(new_n35_), .O(z11));
  nand2  g59(.a(x16), .b(new_n35_), .O(new_n94_));
  nor2   g60(.a(new_n37_), .b(x03), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n94_), .c(new_n86_), .O(z12));
  inv1   g62(.a(new_n86_), .O(z13));
  inv1   g63(.a(x17), .O(new_n98_));
  oai21  g64(.a(new_n98_), .b(new_n45_), .c(new_n38_), .O(z14));
endmodule


