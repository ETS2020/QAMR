// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x05), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x15), .O(new_n43_));
  nand3  g09(.a(x18), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  oai21  g11(.a(new_n42_), .b(x04), .c(new_n45_), .O(z01));
  nand2  g12(.a(x06), .b(x05), .O(new_n47_));
  inv1   g13(.a(new_n47_), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n36_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n37_), .c(new_n48_), .O(z02));
  inv1   g17(.a(new_n50_), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nor2   g19(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  aoi21  g20(.a(new_n47_), .b(new_n53_), .c(new_n54_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  inv1   g23(.a(new_n38_), .O(new_n58_));
  nand2  g24(.a(new_n54_), .b(x08), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  oai21  g26(.a(new_n53_), .b(new_n37_), .c(new_n60_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n50_), .c(new_n58_), .O(z04));
  oai22  g29(.a(new_n59_), .b(new_n42_), .c(new_n38_), .d(x09), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n52_), .O(z05));
  nand3  g33(.a(new_n65_), .b(x08), .c(x07), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x10), .O(new_n69_));
  inv1   g35(.a(x10), .O(new_n70_));
  nor2   g36(.a(x09), .b(new_n60_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n54_), .c(new_n70_), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x05), .O(new_n74_));
  aoi22  g40(.a(new_n50_), .b(new_n58_), .c(x10), .d(new_n37_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n74_), .O(z06));
  oai22  g42(.a(new_n72_), .b(new_n42_), .c(new_n38_), .d(x11), .O(new_n77_));
  inv1   g43(.a(x11), .O(new_n78_));
  nor2   g44(.a(new_n66_), .b(x10), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n77_), .c(new_n52_), .O(z07));
  nand2  g47(.a(new_n80_), .b(x12), .O(new_n82_));
  nor2   g48(.a(x12), .b(x11), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  oai21  g50(.a(x12), .b(x06), .c(new_n42_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n84_), .c(new_n82_), .d(new_n52_), .O(z08));
  nand2  g52(.a(new_n83_), .b(new_n70_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n68_), .c(x13), .O(new_n88_));
  nor2   g54(.a(x13), .b(x10), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n83_), .c(new_n71_), .d(new_n54_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x05), .O(new_n92_));
  aoi22  g58(.a(new_n50_), .b(new_n58_), .c(x13), .d(new_n37_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n92_), .O(z09));
  inv1   g60(.a(x04), .O(new_n95_));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n89_), .b(new_n83_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  inv1   g64(.a(x03), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x02), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x16), .c(new_n66_), .O(new_n101_));
  inv1   g67(.a(x14), .O(new_n102_));
  nor2   g68(.a(new_n49_), .b(new_n102_), .O(new_n103_));
  aoi21  g69(.a(new_n101_), .b(new_n98_), .c(new_n103_), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n95_), .c(new_n58_), .O(z10));
  nor2   g71(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n36_), .c(x03), .O(new_n107_));
  nand2  g73(.a(new_n58_), .b(x04), .O(new_n108_));
  nor2   g74(.a(new_n108_), .b(new_n107_), .O(z12));
  inv1   g75(.a(new_n108_), .O(z13));
  inv1   g76(.a(x17), .O(new_n111_));
  nor2   g77(.a(new_n108_), .b(new_n111_), .O(z14));
endmodule


