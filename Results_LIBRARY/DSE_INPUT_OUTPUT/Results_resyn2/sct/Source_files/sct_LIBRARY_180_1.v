// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n99_, new_n100_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x15), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x12), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x04), .O(new_n42_));
  nor2   g08(.a(new_n38_), .b(new_n42_), .O(new_n43_));
  nor2   g09(.a(x15), .b(x05), .O(new_n44_));
  aoi22  g10(.a(new_n44_), .b(x18), .c(new_n43_), .d(x05), .O(z01));
  inv1   g11(.a(new_n38_), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  oai21  g14(.a(new_n48_), .b(x06), .c(new_n46_), .O(z02));
  inv1   g15(.a(x06), .O(new_n50_));
  inv1   g16(.a(x07), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  nand2  g19(.a(new_n51_), .b(new_n50_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n48_), .c(new_n46_), .O(z03));
  inv1   g22(.a(new_n48_), .O(new_n57_));
  nand2  g23(.a(new_n52_), .b(x08), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n46_), .O(z04));
  nand2  g28(.a(new_n58_), .b(x09), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n63_), .c(new_n57_), .d(new_n46_), .O(z05));
  nand2  g32(.a(new_n65_), .b(x10), .O(new_n67_));
  nor2   g33(.a(new_n65_), .b(x10), .O(new_n68_));
  nor2   g34(.a(new_n68_), .b(new_n48_), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n67_), .c(new_n38_), .O(z06));
  oai21  g36(.a(new_n65_), .b(x10), .c(x11), .O(new_n71_));
  inv1   g37(.a(x11), .O(new_n72_));
  aoi21  g38(.a(new_n68_), .b(new_n72_), .c(new_n48_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n71_), .c(new_n38_), .O(z07));
  nand2  g40(.a(new_n48_), .b(new_n46_), .O(new_n75_));
  inv1   g41(.a(x10), .O(new_n76_));
  nor2   g42(.a(x09), .b(new_n59_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n52_), .c(new_n72_), .d(new_n76_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x12), .O(new_n79_));
  nor2   g45(.a(x15), .b(x12), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n68_), .c(new_n72_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n79_), .c(new_n75_), .O(z08));
  inv1   g48(.a(x12), .O(new_n83_));
  oai21  g49(.a(new_n78_), .b(x13), .c(new_n37_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  aoi21  g51(.a(new_n81_), .b(x13), .c(new_n48_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n85_), .O(z09));
  inv1   g53(.a(x03), .O(new_n88_));
  inv1   g54(.a(x16), .O(new_n89_));
  aoi21  g55(.a(new_n88_), .b(x02), .c(new_n89_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x14), .O(new_n91_));
  nand4  g57(.a(new_n77_), .b(new_n52_), .c(new_n47_), .d(x00), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n46_), .O(new_n94_));
  nor2   g60(.a(new_n90_), .b(x13), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n80_), .c(new_n68_), .d(new_n72_), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n94_), .c(new_n42_), .O(z10));
  nand2  g63(.a(new_n46_), .b(new_n36_), .O(z11));
  inv1   g64(.a(new_n43_), .O(new_n99_));
  aoi21  g65(.a(x16), .b(new_n36_), .c(x03), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(new_n99_), .O(z12));
  nand2  g67(.a(new_n46_), .b(new_n42_), .O(z13));
  and2   g68(.a(new_n43_), .b(x17), .O(z14));
endmodule


