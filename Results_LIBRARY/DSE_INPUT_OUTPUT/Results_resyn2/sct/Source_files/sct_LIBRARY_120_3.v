// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n108_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  inv1   g02(.a(x09), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  inv1   g05(.a(x01), .O(new_n40_));
  nand2  g06(.a(x14), .b(new_n40_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(z00));
  nor2   g09(.a(x15), .b(x05), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand2  g11(.a(new_n45_), .b(new_n39_), .O(new_n46_));
  aoi21  g12(.a(new_n44_), .b(x18), .c(new_n46_), .O(z01));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n39_), .O(z02));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  inv1   g21(.a(x07), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n36_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n55_), .c(new_n52_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n39_), .O(z03));
  inv1   g25(.a(x02), .O(new_n60_));
  oai21  g26(.a(x03), .b(new_n60_), .c(x16), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  aoi21  g29(.a(x07), .b(x06), .c(x08), .O(new_n64_));
  nor3   g30(.a(new_n64_), .b(new_n63_), .c(new_n38_), .O(z04));
  inv1   g31(.a(new_n63_), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(new_n37_), .c(new_n38_), .O(z05));
  inv1   g33(.a(x10), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(x08), .c(x07), .d(x06), .O(new_n69_));
  nand3  g35(.a(x08), .b(x07), .c(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x10), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n69_), .c(new_n61_), .d(x04), .O(new_n72_));
  and2   g38(.a(new_n72_), .b(new_n39_), .O(z06));
  nor2   g39(.a(x11), .b(x10), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(x08), .c(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n37_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand2  g43(.a(new_n68_), .b(new_n37_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n70_), .c(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n52_), .O(z07));
  oai21  g46(.a(new_n75_), .b(x12), .c(new_n37_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  inv1   g48(.a(new_n62_), .O(new_n83_));
  nand3  g49(.a(new_n74_), .b(new_n83_), .c(new_n37_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x12), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n82_), .c(new_n52_), .O(z08));
  inv1   g52(.a(x12), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n75_), .c(new_n37_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  oai21  g57(.a(new_n84_), .b(x12), .c(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n52_), .O(z09));
  nand2  g59(.a(new_n39_), .b(x04), .O(new_n94_));
  nand2  g60(.a(new_n51_), .b(x14), .O(new_n95_));
  inv1   g61(.a(new_n70_), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand3  g63(.a(new_n74_), .b(new_n88_), .c(new_n87_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g65(.a(new_n49_), .b(x02), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x16), .c(x09), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n99_), .c(new_n96_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n95_), .c(new_n94_), .O(z10));
  nor2   g69(.a(new_n38_), .b(new_n60_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n60_), .c(x03), .O(new_n105_));
  nor2   g71(.a(new_n105_), .b(new_n94_), .O(z12));
  nand2  g72(.a(new_n39_), .b(new_n48_), .O(z13));
  inv1   g73(.a(x17), .O(new_n108_));
  oai21  g74(.a(new_n108_), .b(new_n48_), .c(new_n39_), .O(z14));
endmodule


