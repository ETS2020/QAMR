// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n110_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nand2  g02(.a(x07), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x07), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(new_n53_));
  aoi21  g19(.a(new_n53_), .b(new_n47_), .c(x06), .O(z02));
  nand2  g20(.a(new_n47_), .b(x06), .O(new_n55_));
  aoi22  g21(.a(new_n55_), .b(new_n37_), .c(new_n53_), .d(new_n47_), .O(z03));
  nand2  g22(.a(x08), .b(x07), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n52_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n37_), .O(z04));
  nand2  g27(.a(new_n57_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x07), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n62_), .c(new_n52_), .d(new_n37_), .O(z05));
  inv1   g31(.a(x10), .O(new_n66_));
  nand2  g32(.a(new_n63_), .b(x08), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  nor2   g34(.a(new_n57_), .b(x09), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  oai21  g36(.a(new_n68_), .b(new_n66_), .c(new_n70_), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(x06), .O(new_n72_));
  nand2  g38(.a(new_n53_), .b(new_n37_), .O(new_n73_));
  nand2  g39(.a(x10), .b(new_n47_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(z06));
  inv1   g41(.a(x11), .O(new_n76_));
  nand3  g42(.a(new_n70_), .b(new_n37_), .c(new_n76_), .O(new_n77_));
  nand2  g43(.a(x11), .b(x06), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n70_), .c(new_n77_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n52_), .O(z07));
  nor2   g46(.a(x11), .b(x10), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n67_), .c(x12), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nand3  g50(.a(new_n81_), .b(new_n69_), .c(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x06), .O(new_n87_));
  nand2  g53(.a(x12), .b(new_n47_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n73_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nand2  g56(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  nand4  g57(.a(new_n81_), .b(new_n68_), .c(x13), .d(new_n84_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(x06), .O(new_n93_));
  nand2  g59(.a(x13), .b(new_n47_), .O(new_n94_));
  nand3  g60(.a(new_n94_), .b(new_n93_), .c(new_n73_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand3  g62(.a(new_n81_), .b(new_n90_), .c(new_n84_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor2   g64(.a(new_n64_), .b(new_n51_), .O(new_n99_));
  inv1   g65(.a(x14), .O(new_n100_));
  inv1   g66(.a(x02), .O(new_n101_));
  oai21  g67(.a(x03), .b(new_n101_), .c(x16), .O(new_n102_));
  nor2   g68(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  aoi21  g69(.a(new_n99_), .b(new_n98_), .c(new_n103_), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n48_), .c(new_n37_), .O(z10));
  nand2  g71(.a(new_n37_), .b(new_n101_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n101_), .c(x03), .O(new_n107_));
  oai21  g73(.a(new_n107_), .b(new_n48_), .c(new_n37_), .O(z12));
  aoi21  g74(.a(x07), .b(new_n36_), .c(new_n48_), .O(z13));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nand2  g76(.a(new_n110_), .b(new_n37_), .O(z14));
endmodule


