// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nand2  g02(.a(x15), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x05), .O(new_n42_));
  inv1   g08(.a(x15), .O(new_n43_));
  nand3  g09(.a(x18), .b(new_n43_), .c(new_n42_), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n37_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x04), .O(new_n48_));
  inv1   g14(.a(x03), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x02), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(x16), .c(new_n48_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  aoi21  g18(.a(new_n52_), .b(new_n43_), .c(x06), .O(z02));
  xnor2a g19(.a(x07), .b(x06), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n52_), .c(new_n37_), .O(z03));
  aoi22  g21(.a(new_n37_), .b(x08), .c(x07), .d(x06), .O(new_n56_));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n51_), .O(new_n58_));
  nor2   g24(.a(new_n58_), .b(new_n56_), .O(z04));
  inv1   g25(.a(x09), .O(new_n60_));
  nand4  g26(.a(new_n60_), .b(x08), .c(x07), .d(x06), .O(new_n61_));
  nand2  g27(.a(new_n57_), .b(x09), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(new_n51_), .O(new_n63_));
  and2   g29(.a(new_n63_), .b(new_n37_), .O(z05));
  inv1   g30(.a(x10), .O(new_n65_));
  nand2  g31(.a(x08), .b(x07), .O(new_n66_));
  nor2   g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n65_), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n61_), .b(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n51_), .O(new_n70_));
  and2   g36(.a(new_n70_), .b(new_n37_), .O(z06));
  nand4  g37(.a(new_n65_), .b(new_n60_), .c(x08), .d(x07), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  aoi22  g39(.a(new_n73_), .b(new_n67_), .c(new_n72_), .d(x11), .O(new_n74_));
  inv1   g40(.a(x02), .O(new_n75_));
  oai21  g41(.a(x03), .b(new_n75_), .c(x16), .O(new_n76_));
  nand2  g42(.a(x11), .b(new_n36_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(x04), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n37_), .O(new_n79_));
  oai21  g45(.a(new_n74_), .b(new_n36_), .c(new_n79_), .O(z07));
  nand4  g46(.a(new_n73_), .b(new_n60_), .c(x08), .d(x07), .O(new_n81_));
  xor2a  g47(.a(new_n81_), .b(x12), .O(new_n82_));
  nand2  g48(.a(x12), .b(new_n36_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n76_), .c(x04), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n37_), .O(new_n85_));
  oai21  g51(.a(new_n82_), .b(new_n36_), .c(new_n85_), .O(z08));
  oai21  g52(.a(new_n81_), .b(x12), .c(x13), .O(new_n87_));
  inv1   g53(.a(new_n61_), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n73_), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n88_), .O(new_n93_));
  oai21  g59(.a(x15), .b(x13), .c(new_n36_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n93_), .c(new_n87_), .d(new_n51_), .O(z09));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n88_), .c(new_n76_), .O(new_n98_));
  nand4  g64(.a(new_n50_), .b(new_n37_), .c(x16), .d(x14), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(new_n48_), .O(z10));
  inv1   g66(.a(new_n37_), .O(new_n101_));
  nor2   g67(.a(new_n101_), .b(new_n75_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n75_), .c(x03), .O(new_n103_));
  nor3   g69(.a(new_n103_), .b(new_n101_), .c(new_n48_), .O(z12));
  nor2   g70(.a(new_n101_), .b(new_n48_), .O(z13));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n37_), .O(z14));
endmodule


