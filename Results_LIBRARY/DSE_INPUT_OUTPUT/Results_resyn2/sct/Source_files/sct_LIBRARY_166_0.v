// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n104_, new_n106_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x08), .b(x05), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x15), .O(new_n41_));
  nand3  g07(.a(x18), .b(new_n41_), .c(x08), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nand2  g10(.a(x05), .b(new_n44_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  nor2   g13(.a(new_n37_), .b(new_n44_), .O(z13));
  oai21  g14(.a(x03), .b(new_n36_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(z13), .c(new_n47_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n37_), .O(new_n52_));
  nand2  g18(.a(new_n49_), .b(x04), .O(new_n53_));
  xnor2a g19(.a(x07), .b(x06), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n58_), .c(new_n52_), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(new_n53_), .O(z04));
  inv1   g27(.a(new_n53_), .O(new_n62_));
  oai21  g28(.a(new_n37_), .b(x09), .c(new_n59_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(z05));
  inv1   g32(.a(x10), .O(new_n67_));
  inv1   g33(.a(new_n65_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  oai21  g35(.a(new_n37_), .b(x10), .c(new_n65_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n62_), .O(z06));
  oai21  g37(.a(new_n37_), .b(x11), .c(new_n69_), .O(new_n72_));
  inv1   g38(.a(x11), .O(new_n73_));
  nand3  g39(.a(new_n68_), .b(new_n73_), .c(new_n67_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n72_), .c(new_n62_), .O(z07));
  nor2   g41(.a(new_n57_), .b(x09), .O(new_n76_));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor3   g44(.a(x12), .b(x11), .c(x10), .O(new_n79_));
  aoi22  g45(.a(new_n79_), .b(new_n76_), .c(new_n78_), .d(x12), .O(new_n80_));
  inv1   g46(.a(x12), .O(new_n81_));
  nor2   g47(.a(new_n81_), .b(x08), .O(new_n82_));
  oai21  g48(.a(new_n82_), .b(new_n53_), .c(new_n52_), .O(new_n83_));
  oai21  g49(.a(new_n80_), .b(new_n56_), .c(new_n83_), .O(z08));
  inv1   g50(.a(x13), .O(new_n85_));
  nor2   g51(.a(new_n85_), .b(x08), .O(new_n86_));
  oai21  g52(.a(new_n86_), .b(new_n53_), .c(new_n52_), .O(new_n87_));
  nand2  g53(.a(new_n79_), .b(new_n76_), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nand2  g55(.a(new_n88_), .b(new_n85_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x08), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n89_), .c(new_n87_), .O(z09));
  inv1   g58(.a(x00), .O(new_n93_));
  nand3  g59(.a(new_n77_), .b(new_n85_), .c(new_n81_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  inv1   g61(.a(x03), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x02), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(x16), .c(new_n65_), .O(new_n98_));
  inv1   g64(.a(x14), .O(new_n99_));
  nor2   g65(.a(new_n49_), .b(new_n99_), .O(new_n100_));
  aoi21  g66(.a(new_n98_), .b(new_n95_), .c(new_n100_), .O(new_n101_));
  oai21  g67(.a(new_n101_), .b(new_n44_), .c(new_n52_), .O(z10));
  nor2   g68(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n36_), .c(x03), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n44_), .c(new_n52_), .O(z12));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n52_), .O(z14));
endmodule


