// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n100_, new_n102_,
    new_n104_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x06), .b(x04), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x04), .c(x06), .O(z02));
  inv1   g16(.a(x04), .O(new_n51_));
  nor2   g17(.a(new_n49_), .b(new_n51_), .O(new_n52_));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n53_), .c(new_n52_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n52_), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  inv1   g29(.a(x09), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(x08), .c(x07), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x04), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(x06), .O(new_n67_));
  aoi21  g33(.a(new_n59_), .b(x09), .c(new_n49_), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n51_), .c(new_n67_), .O(z05));
  nand4  g35(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  inv1   g38(.a(new_n70_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n71_), .c(new_n52_), .O(z06));
  inv1   g41(.a(x11), .O(new_n76_));
  nand3  g42(.a(new_n73_), .b(new_n76_), .c(new_n72_), .O(new_n77_));
  nand2  g43(.a(new_n74_), .b(x11), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n52_), .O(z07));
  nand2  g45(.a(new_n77_), .b(x12), .O(new_n80_));
  inv1   g46(.a(new_n59_), .O(new_n81_));
  nor2   g47(.a(x12), .b(x11), .O(new_n82_));
  nor2   g48(.a(x10), .b(x09), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n80_), .c(new_n52_), .O(z08));
  inv1   g51(.a(x12), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n86_), .c(new_n76_), .d(new_n72_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n65_), .c(x04), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x06), .O(new_n90_));
  aoi21  g56(.a(new_n84_), .b(x13), .c(new_n49_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n51_), .c(new_n90_), .O(z09));
  nand2  g58(.a(new_n49_), .b(x14), .O(new_n93_));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n88_), .b(new_n94_), .O(new_n95_));
  nor2   g61(.a(new_n70_), .b(new_n49_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n93_), .c(new_n51_), .O(z10));
  nor2   g64(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g65(.a(x16), .b(new_n36_), .c(x03), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n51_), .c(new_n41_), .O(z12));
  nor2   g67(.a(new_n54_), .b(x04), .O(new_n102_));
  inv1   g68(.a(new_n102_), .O(z13));
  nor2   g69(.a(x17), .b(new_n51_), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n102_), .O(z14));
endmodule


