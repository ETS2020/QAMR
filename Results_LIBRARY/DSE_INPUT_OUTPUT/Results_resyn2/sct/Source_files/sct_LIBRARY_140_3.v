// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n104_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(x12), .b(x10), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(x06), .c(new_n37_), .O(z02));
  xnor2a g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n51_), .c(new_n37_), .O(z03));
  nand2  g20(.a(new_n50_), .b(new_n37_), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(x08), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n56_), .b(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n55_), .O(z04));
  inv1   g28(.a(new_n55_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  inv1   g31(.a(new_n65_), .O(new_n66_));
  aoi21  g32(.a(new_n58_), .b(x09), .c(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n63_), .O(z05));
  inv1   g34(.a(x10), .O(new_n69_));
  nand2  g35(.a(new_n66_), .b(new_n69_), .O(new_n70_));
  nand2  g36(.a(new_n65_), .b(x10), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n63_), .O(z06));
  nand2  g38(.a(new_n70_), .b(x11), .O(new_n73_));
  inv1   g39(.a(x11), .O(new_n74_));
  nor2   g40(.a(x09), .b(new_n59_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(new_n57_), .c(new_n74_), .d(new_n69_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n73_), .c(new_n63_), .O(z07));
  nand3  g43(.a(new_n66_), .b(x12), .c(new_n74_), .O(new_n78_));
  inv1   g44(.a(x12), .O(new_n79_));
  oai21  g45(.a(new_n65_), .b(x11), .c(new_n79_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n78_), .c(new_n69_), .O(new_n81_));
  oai21  g47(.a(new_n50_), .b(new_n38_), .c(new_n81_), .O(z08));
  inv1   g48(.a(x13), .O(new_n83_));
  oai21  g49(.a(x03), .b(new_n36_), .c(x16), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x12), .O(new_n87_));
  nand2  g53(.a(new_n76_), .b(x13), .O(new_n88_));
  nand2  g54(.a(x13), .b(new_n79_), .O(new_n89_));
  nand4  g55(.a(new_n89_), .b(new_n66_), .c(new_n74_), .d(new_n69_), .O(new_n90_));
  nand3  g56(.a(new_n90_), .b(new_n88_), .c(new_n50_), .O(new_n91_));
  and2   g57(.a(new_n91_), .b(new_n87_), .O(z09));
  nand2  g58(.a(new_n49_), .b(x14), .O(new_n93_));
  nand2  g59(.a(new_n84_), .b(x00), .O(new_n94_));
  oai21  g60(.a(new_n94_), .b(new_n65_), .c(new_n93_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n37_), .O(new_n96_));
  inv1   g62(.a(new_n76_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n84_), .c(new_n83_), .d(new_n79_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n96_), .c(new_n44_), .O(z10));
  nor2   g65(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g66(.a(x16), .b(new_n36_), .c(x03), .O(new_n101_));
  nor3   g67(.a(new_n101_), .b(new_n38_), .c(new_n44_), .O(z12));
  nand2  g68(.a(new_n37_), .b(new_n44_), .O(z13));
  inv1   g69(.a(x17), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n44_), .c(new_n37_), .O(z14));
endmodule


