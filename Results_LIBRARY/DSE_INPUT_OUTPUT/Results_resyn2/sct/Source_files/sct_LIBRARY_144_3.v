// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n106_, new_n109_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x00), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(new_n40_));
  or2    g06(.a(x14), .b(x01), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n40_), .c(new_n36_), .O(z00));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g10(.a(x04), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n45_), .c(new_n39_), .O(new_n46_));
  oai21  g12(.a(new_n44_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n38_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  oai21  g21(.a(new_n38_), .b(x00), .c(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n40_), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n54_), .b(new_n61_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n51_), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n60_), .O(z04));
  inv1   g30(.a(x09), .O(new_n65_));
  nand2  g31(.a(new_n60_), .b(new_n65_), .O(new_n66_));
  nand4  g32(.a(x08), .b(x07), .c(x06), .d(new_n37_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n51_), .O(z05));
  nor2   g35(.a(new_n59_), .b(x09), .O(new_n70_));
  nor3   g36(.a(new_n70_), .b(new_n39_), .c(x10), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nor3   g38(.a(new_n67_), .b(new_n72_), .c(x09), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(new_n71_), .c(new_n51_), .O(z06));
  nand4  g40(.a(new_n72_), .b(new_n65_), .c(x08), .d(x07), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x11), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  and2   g43(.a(x08), .b(x07), .O(new_n78_));
  nor2   g44(.a(x10), .b(x09), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(new_n77_), .d(x06), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n76_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n37_), .O(new_n82_));
  oai21  g48(.a(x03), .b(new_n35_), .c(x16), .O(new_n83_));
  nand2  g49(.a(x11), .b(new_n38_), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(new_n82_), .c(new_n39_), .O(z07));
  oai22  g53(.a(new_n80_), .b(x00), .c(new_n39_), .d(x12), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  inv1   g55(.a(new_n75_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n89_), .c(new_n77_), .d(x06), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n88_), .c(new_n51_), .O(z08));
  nor2   g58(.a(x10), .b(x00), .O(new_n93_));
  nor3   g59(.a(x13), .b(x12), .c(x11), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n93_), .c(new_n70_), .d(new_n83_), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(x04), .O(new_n97_));
  nand2  g63(.a(new_n91_), .b(x13), .O(new_n98_));
  oai21  g64(.a(x13), .b(x06), .c(x00), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n51_), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n98_), .c(new_n97_), .O(z09));
  nand3  g68(.a(new_n50_), .b(new_n40_), .c(x14), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n95_), .c(new_n45_), .O(z10));
  nand2  g70(.a(new_n40_), .b(new_n35_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n35_), .c(x03), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n45_), .c(new_n40_), .O(z12));
  nor2   g73(.a(new_n39_), .b(new_n45_), .O(z13));
  nand2  g74(.a(x17), .b(x04), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(new_n40_), .O(z14));
endmodule


