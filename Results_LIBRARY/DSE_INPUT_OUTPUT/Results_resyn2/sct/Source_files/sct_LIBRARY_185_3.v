// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n96_, new_n97_, new_n100_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x13), .b(x06), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  nor2   g05(.a(x15), .b(x05), .O(new_n40_));
  nand2  g06(.a(x05), .b(x04), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  aoi21  g08(.a(new_n40_), .b(x18), .c(new_n42_), .O(z01));
  oai21  g09(.a(x03), .b(new_n35_), .c(x16), .O(new_n44_));
  nand2  g10(.a(new_n44_), .b(x04), .O(new_n45_));
  oai21  g11(.a(new_n45_), .b(x06), .c(new_n37_), .O(z02));
  nand2  g12(.a(x07), .b(x06), .O(new_n47_));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x07), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n45_), .c(new_n37_), .O(z03));
  inv1   g18(.a(x08), .O(new_n53_));
  nand2  g19(.a(new_n47_), .b(new_n53_), .O(new_n54_));
  nand3  g20(.a(x08), .b(x07), .c(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n54_), .c(new_n37_), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(new_n45_), .O(z04));
  inv1   g23(.a(new_n45_), .O(new_n58_));
  oai21  g24(.a(new_n55_), .b(x13), .c(x09), .O(new_n59_));
  nand2  g25(.a(x08), .b(x07), .O(new_n60_));
  nor2   g26(.a(new_n60_), .b(x09), .O(new_n61_));
  oai21  g27(.a(new_n61_), .b(x13), .c(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n59_), .c(new_n58_), .O(z05));
  nor2   g29(.a(x13), .b(new_n48_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(x10), .O(new_n66_));
  inv1   g32(.a(new_n60_), .O(new_n67_));
  nor2   g33(.a(x10), .b(x09), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g35(.a(new_n69_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(x13), .c(x06), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n66_), .c(new_n58_), .O(z06));
  aoi21  g38(.a(new_n68_), .b(new_n67_), .c(x13), .O(new_n73_));
  oai21  g39(.a(new_n73_), .b(new_n48_), .c(x11), .O(new_n74_));
  nand2  g40(.a(new_n45_), .b(new_n37_), .O(new_n75_));
  inv1   g41(.a(new_n65_), .O(new_n76_));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n75_), .c(new_n74_), .O(z07));
  aoi21  g45(.a(new_n77_), .b(new_n61_), .c(x13), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n48_), .c(x12), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nand3  g48(.a(new_n77_), .b(new_n76_), .c(new_n82_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n75_), .O(z08));
  oai21  g50(.a(new_n45_), .b(x13), .c(new_n37_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n83_), .O(z09));
  nand2  g52(.a(new_n37_), .b(x04), .O(new_n87_));
  inv1   g53(.a(new_n44_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x14), .O(new_n89_));
  inv1   g55(.a(x00), .O(new_n90_));
  nand2  g56(.a(new_n77_), .b(new_n82_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n64_), .c(new_n61_), .d(new_n44_), .O(new_n93_));
  aoi21  g59(.a(new_n93_), .b(new_n89_), .c(new_n87_), .O(z10));
  aoi21  g60(.a(x13), .b(x06), .c(new_n35_), .O(z11));
  inv1   g61(.a(x04), .O(new_n96_));
  aoi21  g62(.a(x16), .b(new_n35_), .c(x03), .O(new_n97_));
  oai21  g63(.a(new_n97_), .b(new_n96_), .c(new_n37_), .O(z12));
  inv1   g64(.a(new_n87_), .O(z13));
  nand2  g65(.a(z13), .b(x17), .O(new_n100_));
  inv1   g66(.a(new_n100_), .O(z14));
endmodule


