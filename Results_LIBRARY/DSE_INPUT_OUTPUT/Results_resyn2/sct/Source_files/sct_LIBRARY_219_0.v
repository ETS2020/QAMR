// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n105_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x10), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  oai21  g12(.a(x03), .b(new_n36_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(x10), .c(x06), .O(z02));
  nor2   g15(.a(x07), .b(x06), .O(new_n50_));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n41_), .O(new_n52_));
  nor3   g18(.a(new_n52_), .b(new_n50_), .c(new_n48_), .O(z03));
  xor2a  g19(.a(new_n51_), .b(x08), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n48_), .c(new_n41_), .O(z04));
  nand2  g21(.a(new_n48_), .b(new_n41_), .O(new_n56_));
  inv1   g22(.a(x06), .O(new_n57_));
  nand3  g23(.a(x10), .b(x09), .c(new_n57_), .O(new_n58_));
  inv1   g24(.a(x09), .O(new_n59_));
  aoi21  g25(.a(x08), .b(x07), .c(new_n59_), .O(new_n60_));
  nand2  g26(.a(x08), .b(x07), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(x09), .O(new_n62_));
  oai21  g28(.a(new_n62_), .b(new_n60_), .c(x06), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n58_), .c(new_n56_), .O(z05));
  inv1   g30(.a(x10), .O(new_n65_));
  nand4  g31(.a(new_n59_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g34(.a(new_n66_), .b(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(new_n56_), .O(z06));
  oai21  g36(.a(new_n61_), .b(x09), .c(x11), .O(new_n71_));
  nor2   g37(.a(x11), .b(x10), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n59_), .c(x08), .d(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  nand2  g41(.a(x11), .b(x10), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n56_), .O(z07));
  inv1   g43(.a(x04), .O(new_n78_));
  inv1   g44(.a(x03), .O(new_n79_));
  inv1   g45(.a(x16), .O(new_n80_));
  aoi21  g46(.a(new_n79_), .b(x02), .c(new_n80_), .O(new_n81_));
  nor2   g47(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  oai21  g49(.a(x10), .b(x06), .c(new_n83_), .O(new_n84_));
  aoi21  g50(.a(new_n72_), .b(new_n62_), .c(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n72_), .b(x12), .O(new_n86_));
  nor2   g52(.a(new_n86_), .b(new_n66_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n85_), .c(new_n82_), .O(z08));
  oai21  g54(.a(new_n73_), .b(x12), .c(x13), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand4  g56(.a(new_n72_), .b(new_n62_), .c(new_n90_), .d(new_n83_), .O(new_n91_));
  oai21  g57(.a(x13), .b(new_n65_), .c(new_n57_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n91_), .c(new_n89_), .d(new_n82_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  inv1   g60(.a(x11), .O(new_n95_));
  nand4  g61(.a(new_n90_), .b(new_n83_), .c(new_n95_), .d(new_n65_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n67_), .c(new_n47_), .O(new_n98_));
  nand3  g64(.a(new_n81_), .b(new_n41_), .c(x14), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n98_), .c(new_n78_), .O(z10));
  nor2   g66(.a(new_n37_), .b(new_n36_), .O(z11));
  oai21  g67(.a(new_n80_), .b(x02), .c(new_n79_), .O(new_n102_));
  nor2   g68(.a(new_n37_), .b(new_n78_), .O(z13));
  and2   g69(.a(z13), .b(new_n102_), .O(z12));
  nand2  g70(.a(x17), .b(x04), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n41_), .O(z14));
endmodule


