// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_, new_n103_,
    new_n105_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x18), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x06), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x04), .O(new_n41_));
  aoi21  g07(.a(new_n37_), .b(x06), .c(new_n41_), .O(z13));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  aoi22  g09(.a(new_n43_), .b(x18), .c(z13), .d(x05), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  nand2  g11(.a(x18), .b(x06), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(new_n45_), .c(new_n47_), .O(z02));
  inv1   g16(.a(new_n49_), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n46_), .b(new_n52_), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xor2a  g22(.a(new_n54_), .b(x08), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n49_), .c(new_n38_), .O(z04));
  nand2  g24(.a(new_n49_), .b(new_n38_), .O(new_n59_));
  nand2  g25(.a(x08), .b(x07), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x18), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(x06), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x07), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n46_), .O(new_n65_));
  aoi21  g31(.a(new_n62_), .b(x09), .c(new_n65_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n59_), .O(z05));
  nor2   g33(.a(new_n60_), .b(x09), .O(new_n68_));
  oai21  g34(.a(new_n68_), .b(new_n37_), .c(x06), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(new_n65_), .b(new_n71_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n70_), .c(new_n59_), .O(z06));
  nand4  g39(.a(new_n71_), .b(new_n63_), .c(x08), .d(x07), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x18), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x11), .O(new_n77_));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n65_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n59_), .O(z07));
  aoi21  g46(.a(new_n78_), .b(new_n68_), .c(new_n37_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n45_), .c(x12), .O(new_n82_));
  nor3   g48(.a(x12), .b(x11), .c(x10), .O(new_n83_));
  aoi22  g49(.a(new_n83_), .b(new_n65_), .c(new_n49_), .d(new_n38_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n82_), .O(z08));
  inv1   g51(.a(x12), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand3  g53(.a(new_n78_), .b(new_n87_), .c(new_n86_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n64_), .c(x18), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(x06), .O(new_n90_));
  nand2  g56(.a(new_n83_), .b(new_n65_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n90_), .c(new_n51_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nand2  g60(.a(new_n88_), .b(new_n94_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n65_), .c(new_n48_), .O(new_n96_));
  inv1   g62(.a(x03), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x02), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n38_), .c(x16), .d(x14), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n96_), .c(new_n41_), .O(z10));
  nand2  g66(.a(new_n38_), .b(new_n35_), .O(z11));
  nand2  g67(.a(new_n38_), .b(x04), .O(new_n102_));
  aoi21  g68(.a(x16), .b(new_n35_), .c(x03), .O(new_n103_));
  nor2   g69(.a(new_n103_), .b(new_n102_), .O(z12));
  nand2  g70(.a(x17), .b(x04), .O(new_n105_));
  nand2  g71(.a(new_n105_), .b(new_n38_), .O(z14));
endmodule


