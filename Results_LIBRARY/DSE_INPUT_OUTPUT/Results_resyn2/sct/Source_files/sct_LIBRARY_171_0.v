// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n102_,
    new_n105_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x12), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x02), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x16), .c(new_n43_), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  aoi21  g15(.a(new_n49_), .b(x12), .c(x06), .O(z02));
  inv1   g16(.a(new_n37_), .O(new_n51_));
  inv1   g17(.a(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g20(.a(new_n53_), .b(new_n52_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  nor2   g23(.a(new_n57_), .b(new_n49_), .O(z03));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n56_), .b(new_n59_), .O(new_n60_));
  nand2  g26(.a(new_n55_), .b(x08), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n60_), .c(new_n48_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n51_), .O(z04));
  oai21  g29(.a(new_n37_), .b(x09), .c(new_n61_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n48_), .O(z05));
  nand2  g33(.a(new_n66_), .b(x10), .O(new_n68_));
  inv1   g34(.a(x10), .O(new_n69_));
  nor2   g35(.a(x09), .b(new_n59_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n55_), .c(new_n69_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n48_), .O(new_n72_));
  and2   g38(.a(new_n72_), .b(new_n51_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  inv1   g40(.a(new_n66_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n69_), .O(new_n76_));
  oai21  g42(.a(new_n37_), .b(x11), .c(new_n71_), .O(new_n77_));
  nand3  g43(.a(new_n77_), .b(new_n76_), .c(new_n48_), .O(z07));
  nand2  g44(.a(new_n47_), .b(x16), .O(new_n79_));
  nand3  g45(.a(new_n65_), .b(x08), .c(x07), .O(new_n80_));
  nand2  g46(.a(new_n74_), .b(new_n69_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n80_), .c(x12), .O(new_n82_));
  inv1   g48(.a(new_n80_), .O(new_n83_));
  inv1   g49(.a(x12), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n74_), .c(new_n69_), .O(new_n85_));
  inv1   g51(.a(new_n85_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nor2   g53(.a(new_n52_), .b(new_n43_), .O(new_n88_));
  nand4  g54(.a(new_n88_), .b(new_n87_), .c(new_n82_), .d(new_n79_), .O(z08));
  nor2   g55(.a(new_n37_), .b(x13), .O(new_n90_));
  oai21  g56(.a(new_n85_), .b(new_n80_), .c(new_n90_), .O(new_n91_));
  nand3  g57(.a(new_n86_), .b(new_n75_), .c(x13), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n48_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  oai21  g61(.a(new_n85_), .b(x13), .c(new_n95_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n75_), .c(new_n79_), .O(new_n97_));
  nand4  g63(.a(new_n47_), .b(new_n51_), .c(x16), .d(x14), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n97_), .c(new_n43_), .O(z10));
  nor2   g65(.a(new_n37_), .b(new_n36_), .O(z11));
  nand2  g66(.a(x16), .b(new_n36_), .O(new_n101_));
  nor2   g67(.a(new_n37_), .b(x03), .O(new_n102_));
  aoi22  g68(.a(new_n102_), .b(new_n101_), .c(new_n51_), .d(new_n43_), .O(z12));
  nand2  g69(.a(new_n51_), .b(new_n43_), .O(z13));
  nand2  g70(.a(x17), .b(x04), .O(new_n105_));
  nor2   g71(.a(new_n105_), .b(new_n37_), .O(z14));
endmodule


