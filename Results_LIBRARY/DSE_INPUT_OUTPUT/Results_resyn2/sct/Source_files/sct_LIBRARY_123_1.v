// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x04), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x04), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n35_), .c(x16), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nor3   g15(.a(new_n49_), .b(x06), .c(new_n47_), .O(z02));
  oai21  g16(.a(x06), .b(new_n47_), .c(x07), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n37_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n51_), .c(new_n48_), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n39_), .O(z03));
  and2   g21(.a(x07), .b(x06), .O(new_n56_));
  aoi21  g22(.a(x08), .b(x04), .c(new_n56_), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n48_), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n57_), .c(new_n39_), .O(z04));
  oai21  g26(.a(x09), .b(new_n47_), .c(new_n58_), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n49_), .O(new_n65_));
  aoi21  g31(.a(new_n65_), .b(new_n61_), .c(new_n38_), .O(z05));
  inv1   g32(.a(x10), .O(new_n67_));
  nand2  g33(.a(new_n64_), .b(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n67_), .b(x04), .O(new_n69_));
  aoi21  g35(.a(new_n69_), .b(new_n63_), .c(new_n49_), .O(new_n70_));
  aoi21  g36(.a(new_n70_), .b(new_n68_), .c(new_n38_), .O(z06));
  nand2  g37(.a(new_n68_), .b(x11), .O(new_n72_));
  nor2   g38(.a(x11), .b(x10), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n64_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n72_), .c(new_n48_), .d(x04), .O(z07));
  oai21  g41(.a(x12), .b(new_n47_), .c(new_n37_), .O(new_n76_));
  inv1   g42(.a(x11), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n67_), .O(new_n78_));
  nand3  g44(.a(new_n62_), .b(x08), .c(x07), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n78_), .c(x12), .O(new_n80_));
  inv1   g46(.a(x12), .O(new_n81_));
  inv1   g47(.a(new_n58_), .O(new_n82_));
  nand4  g48(.a(new_n73_), .b(new_n82_), .c(new_n81_), .d(new_n62_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n80_), .c(new_n48_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x04), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n76_), .O(z08));
  oai21  g52(.a(x13), .b(new_n47_), .c(new_n37_), .O(new_n87_));
  nand3  g53(.a(new_n81_), .b(new_n77_), .c(new_n67_), .O(new_n88_));
  oai21  g54(.a(new_n88_), .b(new_n79_), .c(x13), .O(new_n89_));
  inv1   g55(.a(x08), .O(new_n90_));
  nor2   g56(.a(x09), .b(new_n90_), .O(new_n91_));
  nor2   g57(.a(x13), .b(x12), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n73_), .c(new_n91_), .d(new_n56_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n89_), .c(new_n48_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x04), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n87_), .O(z09));
  nand2  g62(.a(new_n49_), .b(x14), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n92_), .b(new_n73_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n64_), .c(new_n48_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n97_), .c(new_n47_), .O(z10));
  nor2   g68(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g69(.a(x16), .b(new_n35_), .c(x03), .O(new_n104_));
  nor2   g70(.a(new_n104_), .b(new_n47_), .O(z12));
  and2   g71(.a(x17), .b(x04), .O(z14));
  buf    g72(.a(x04), .O(z13));
endmodule


