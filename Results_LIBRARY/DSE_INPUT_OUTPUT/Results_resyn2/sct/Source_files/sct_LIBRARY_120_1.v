// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n99_, new_n102_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x06), .O(new_n36_));
  nand2  g02(.a(x09), .b(new_n36_), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x09), .O(new_n47_));
  inv1   g13(.a(x02), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n48_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  aoi21  g16(.a(new_n50_), .b(new_n47_), .c(x06), .O(z02));
  xnor2a g17(.a(x07), .b(x06), .O(new_n52_));
  oai21  g18(.a(new_n52_), .b(new_n50_), .c(new_n37_), .O(z03));
  aoi22  g19(.a(new_n37_), .b(x08), .c(x07), .d(x06), .O(new_n54_));
  nand3  g20(.a(x08), .b(x07), .c(x06), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nor3   g22(.a(new_n56_), .b(new_n54_), .c(new_n50_), .O(z04));
  inv1   g23(.a(new_n50_), .O(new_n58_));
  nand2  g24(.a(new_n50_), .b(new_n47_), .O(new_n59_));
  and2   g25(.a(x08), .b(x07), .O(new_n60_));
  xor2a  g26(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  aoi22  g27(.a(new_n61_), .b(new_n58_), .c(new_n59_), .d(new_n36_), .O(z05));
  inv1   g28(.a(x10), .O(new_n63_));
  nand3  g29(.a(new_n49_), .b(new_n63_), .c(x04), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n47_), .O(new_n65_));
  oai21  g31(.a(new_n55_), .b(x09), .c(x10), .O(new_n66_));
  nand4  g32(.a(new_n63_), .b(new_n47_), .c(x08), .d(x07), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n49_), .c(x04), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  aoi22  g35(.a(new_n69_), .b(new_n66_), .c(new_n65_), .d(new_n36_), .O(z06));
  nor2   g36(.a(x11), .b(x10), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n60_), .c(new_n47_), .d(x06), .O(new_n72_));
  nand2  g38(.a(new_n67_), .b(x11), .O(new_n73_));
  oai21  g39(.a(x11), .b(x09), .c(new_n36_), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n58_), .O(z07));
  nand3  g41(.a(new_n71_), .b(new_n60_), .c(new_n47_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x12), .O(new_n77_));
  inv1   g43(.a(x12), .O(new_n78_));
  nor2   g44(.a(new_n55_), .b(x09), .O(new_n79_));
  nand3  g45(.a(new_n71_), .b(new_n79_), .c(new_n78_), .O(new_n80_));
  oai21  g46(.a(x12), .b(x09), .c(new_n36_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n80_), .c(new_n77_), .d(new_n58_), .O(z08));
  inv1   g48(.a(new_n37_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(x13), .c(new_n80_), .O(new_n84_));
  inv1   g50(.a(x13), .O(new_n85_));
  nand3  g51(.a(new_n71_), .b(new_n85_), .c(new_n78_), .O(new_n86_));
  inv1   g52(.a(new_n86_), .O(new_n87_));
  aoi21  g53(.a(new_n87_), .b(new_n79_), .c(new_n50_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n84_), .O(z09));
  inv1   g55(.a(x04), .O(new_n90_));
  inv1   g56(.a(x00), .O(new_n91_));
  nand2  g57(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n79_), .c(new_n49_), .O(new_n93_));
  inv1   g59(.a(x03), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x02), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n37_), .c(x16), .d(x14), .O(new_n96_));
  aoi21  g62(.a(new_n96_), .b(new_n93_), .c(new_n90_), .O(z10));
  nor2   g63(.a(new_n83_), .b(new_n48_), .O(z11));
  aoi21  g64(.a(x16), .b(new_n48_), .c(x03), .O(new_n99_));
  nor3   g65(.a(new_n99_), .b(new_n83_), .c(new_n90_), .O(z12));
  nand2  g66(.a(new_n37_), .b(new_n90_), .O(z13));
  inv1   g67(.a(x17), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n90_), .c(new_n37_), .O(z14));
endmodule


