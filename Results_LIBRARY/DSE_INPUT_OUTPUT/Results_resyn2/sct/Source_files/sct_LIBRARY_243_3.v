// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n109_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  nand2  g01(.a(x13), .b(x06), .O(new_n36_));
  inv1   g02(.a(x01), .O(new_n37_));
  nand2  g03(.a(x14), .b(new_n37_), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(x05), .b(x04), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(new_n36_), .O(new_n43_));
  aoi21  g09(.a(new_n41_), .b(x18), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  inv1   g11(.a(x04), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(x02), .O(new_n53_));
  oai21  g19(.a(x03), .b(new_n53_), .c(x16), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x04), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(x07), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n45_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n55_), .c(new_n36_), .O(z03));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n62_), .c(new_n36_), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n55_), .O(z04));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand2  g33(.a(new_n63_), .b(x09), .O(new_n68_));
  nand4  g34(.a(new_n68_), .b(new_n67_), .c(new_n54_), .d(x04), .O(new_n69_));
  and2   g35(.a(new_n69_), .b(new_n36_), .O(z05));
  inv1   g36(.a(x13), .O(new_n71_));
  nand3  g37(.a(new_n66_), .b(x08), .c(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x10), .O(new_n75_));
  nand2  g41(.a(new_n55_), .b(new_n36_), .O(new_n76_));
  nor2   g42(.a(x09), .b(new_n61_), .O(new_n77_));
  nor2   g43(.a(x13), .b(new_n45_), .O(new_n78_));
  nor2   g44(.a(x10), .b(new_n57_), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n76_), .c(new_n75_), .O(z06));
  inv1   g47(.a(x10), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(new_n66_), .c(x08), .d(x07), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(x11), .c(new_n71_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x06), .O(new_n85_));
  nand2  g51(.a(new_n80_), .b(x11), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n85_), .c(new_n50_), .O(z07));
  inv1   g53(.a(x11), .O(new_n88_));
  inv1   g54(.a(x12), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n88_), .c(new_n82_), .O(new_n90_));
  nor2   g56(.a(new_n90_), .b(new_n72_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(x13), .c(x06), .O(new_n92_));
  oai21  g58(.a(new_n80_), .b(x11), .c(x12), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n50_), .O(z08));
  oai21  g60(.a(new_n55_), .b(x13), .c(new_n36_), .O(new_n95_));
  nand2  g61(.a(new_n91_), .b(new_n78_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(z09));
  nand2  g63(.a(new_n36_), .b(x04), .O(new_n98_));
  nand2  g64(.a(new_n49_), .b(x14), .O(new_n99_));
  nor2   g65(.a(new_n63_), .b(x09), .O(new_n100_));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n90_), .b(new_n101_), .O(new_n102_));
  nand4  g68(.a(new_n102_), .b(new_n100_), .c(new_n54_), .d(new_n71_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n99_), .c(new_n98_), .O(z10));
  nand2  g70(.a(new_n36_), .b(new_n53_), .O(z11));
  nand2  g71(.a(x16), .b(new_n53_), .O(new_n106_));
  aoi21  g72(.a(new_n106_), .b(new_n47_), .c(new_n98_), .O(z12));
  nand2  g73(.a(new_n36_), .b(new_n46_), .O(z13));
  nand3  g74(.a(new_n36_), .b(x17), .c(x04), .O(new_n109_));
  inv1   g75(.a(new_n109_), .O(z14));
endmodule


