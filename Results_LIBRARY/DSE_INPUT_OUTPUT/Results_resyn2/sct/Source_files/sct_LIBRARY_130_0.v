// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x00), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nor2   g10(.a(x06), .b(x00), .O(new_n45_));
  aoi21  g11(.a(x05), .b(new_n44_), .c(new_n45_), .O(new_n46_));
  oai21  g12(.a(new_n43_), .b(x05), .c(new_n46_), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n44_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n38_), .c(x00), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n38_), .O(new_n55_));
  oai21  g21(.a(x06), .b(new_n37_), .c(x07), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n55_), .c(new_n51_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  inv1   g25(.a(x08), .O(new_n60_));
  oai21  g26(.a(new_n54_), .b(new_n38_), .c(new_n60_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n39_), .O(new_n62_));
  nor3   g28(.a(new_n62_), .b(new_n50_), .c(new_n44_), .O(z04));
  inv1   g29(.a(x09), .O(new_n64_));
  inv1   g30(.a(new_n59_), .O(new_n65_));
  nand2  g31(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  oai21  g32(.a(new_n45_), .b(x09), .c(new_n59_), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n66_), .c(new_n51_), .O(z05));
  oai21  g34(.a(x10), .b(new_n37_), .c(new_n38_), .O(new_n69_));
  nand3  g35(.a(new_n64_), .b(x08), .c(x07), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n64_), .O(new_n72_));
  nor2   g38(.a(new_n72_), .b(new_n59_), .O(new_n73_));
  aoi21  g39(.a(new_n70_), .b(x10), .c(new_n73_), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n69_), .c(new_n51_), .O(z06));
  inv1   g41(.a(x11), .O(new_n76_));
  nand2  g42(.a(new_n73_), .b(new_n76_), .O(new_n77_));
  nand2  g43(.a(x08), .b(x07), .O(new_n78_));
  oai21  g44(.a(new_n72_), .b(new_n78_), .c(x11), .O(new_n79_));
  oai21  g45(.a(x11), .b(new_n37_), .c(new_n38_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n79_), .c(new_n77_), .d(new_n51_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand3  g48(.a(new_n73_), .b(new_n82_), .c(new_n76_), .O(new_n83_));
  nand2  g49(.a(new_n76_), .b(new_n71_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n70_), .c(x12), .O(new_n85_));
  oai21  g51(.a(x12), .b(new_n37_), .c(new_n38_), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n85_), .c(new_n83_), .d(new_n51_), .O(z08));
  inv1   g53(.a(x13), .O(new_n88_));
  nand2  g54(.a(new_n39_), .b(new_n88_), .O(new_n89_));
  nand3  g55(.a(new_n82_), .b(new_n76_), .c(new_n71_), .O(new_n90_));
  oai21  g56(.a(new_n90_), .b(new_n66_), .c(new_n89_), .O(new_n91_));
  nor2   g57(.a(new_n59_), .b(x09), .O(new_n92_));
  nand4  g58(.a(new_n88_), .b(new_n82_), .c(new_n76_), .d(new_n71_), .O(new_n93_));
  inv1   g59(.a(new_n93_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n91_), .c(new_n51_), .O(z09));
  oai21  g62(.a(x03), .b(new_n35_), .c(x16), .O(new_n97_));
  nand2  g63(.a(new_n93_), .b(new_n37_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n92_), .c(new_n97_), .O(new_n99_));
  nand3  g65(.a(new_n50_), .b(new_n39_), .c(x14), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n99_), .c(new_n44_), .O(z10));
  nor2   g67(.a(new_n45_), .b(new_n35_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n35_), .c(x03), .O(new_n103_));
  nor3   g69(.a(new_n103_), .b(new_n45_), .c(new_n44_), .O(z12));
  nand2  g70(.a(new_n39_), .b(new_n44_), .O(z13));
  inv1   g71(.a(x17), .O(new_n106_));
  oai21  g72(.a(new_n106_), .b(new_n44_), .c(new_n39_), .O(z14));
endmodule


