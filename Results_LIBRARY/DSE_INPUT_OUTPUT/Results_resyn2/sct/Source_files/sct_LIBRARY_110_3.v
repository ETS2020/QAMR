// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(x14), .b(x06), .O(new_n36_));
  inv1   g02(.a(new_n36_), .O(new_n37_));
  aoi21  g03(.a(new_n35_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(x14), .b(x01), .c(new_n38_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  aoi21  g08(.a(x05), .b(new_n42_), .c(new_n37_), .O(new_n43_));
  oai21  g09(.a(new_n41_), .b(x05), .c(new_n43_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  nor2   g11(.a(x14), .b(new_n45_), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  aoi21  g17(.a(new_n51_), .b(new_n45_), .c(new_n46_), .O(z02));
  or2    g18(.a(new_n46_), .b(x07), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  xor2a  g22(.a(new_n54_), .b(x08), .O(new_n57_));
  oai21  g23(.a(new_n57_), .b(new_n51_), .c(new_n36_), .O(z04));
  nand3  g24(.a(x08), .b(x07), .c(x06), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(x14), .c(x09), .O(new_n60_));
  inv1   g26(.a(x14), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(x08), .c(x07), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(x06), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n60_), .c(new_n50_), .O(z05));
  nand2  g32(.a(new_n65_), .b(x10), .O(new_n67_));
  nand2  g33(.a(new_n51_), .b(new_n36_), .O(new_n68_));
  nor2   g34(.a(new_n59_), .b(x14), .O(new_n69_));
  nor2   g35(.a(x10), .b(x09), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n67_), .O(z06));
  oai21  g38(.a(new_n63_), .b(x10), .c(new_n61_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x11), .O(new_n75_));
  or2    g41(.a(new_n71_), .b(x11), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n75_), .c(new_n68_), .O(z07));
  nor2   g43(.a(x11), .b(x10), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n62_), .c(x08), .d(x07), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n61_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x06), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand4  g49(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n78_), .c(new_n61_), .d(new_n83_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n82_), .c(new_n68_), .O(z08));
  inv1   g53(.a(x13), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n83_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n79_), .c(new_n61_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  nand2  g57(.a(new_n86_), .b(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n50_), .O(z09));
  inv1   g59(.a(x00), .O(new_n94_));
  nand3  g60(.a(new_n78_), .b(new_n88_), .c(new_n83_), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor2   g62(.a(new_n84_), .b(new_n49_), .O(new_n97_));
  oai21  g63(.a(x03), .b(new_n35_), .c(x16), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n61_), .O(new_n99_));
  aoi21  g65(.a(new_n97_), .b(new_n96_), .c(new_n99_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n42_), .c(new_n36_), .O(z10));
  nor2   g67(.a(new_n37_), .b(new_n35_), .O(z11));
  nand2  g68(.a(x16), .b(new_n35_), .O(new_n103_));
  nand2  g69(.a(new_n36_), .b(x04), .O(new_n104_));
  aoi21  g70(.a(new_n103_), .b(new_n47_), .c(new_n104_), .O(z12));
  inv1   g71(.a(new_n104_), .O(z13));
  nand2  g72(.a(x17), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n36_), .O(z14));
endmodule


