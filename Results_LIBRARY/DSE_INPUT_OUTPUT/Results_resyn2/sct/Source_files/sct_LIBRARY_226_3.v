// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n101_, new_n104_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x12), .b(x06), .O(new_n37_));
  or2    g03(.a(x14), .b(x01), .O(new_n38_));
  nand3  g04(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z00));
  inv1   g05(.a(x18), .O(new_n40_));
  nor2   g06(.a(new_n40_), .b(x15), .O(new_n41_));
  inv1   g07(.a(x04), .O(new_n42_));
  inv1   g08(.a(new_n37_), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  oai21  g10(.a(new_n41_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x02), .O(new_n48_));
  aoi21  g14(.a(new_n48_), .b(x16), .c(new_n42_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n49_), .O(new_n52_));
  xnor2a g18(.a(x07), .b(x06), .O(new_n53_));
  oai21  g19(.a(new_n53_), .b(new_n52_), .c(new_n37_), .O(z03));
  inv1   g20(.a(x08), .O(new_n55_));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n57_), .c(new_n37_), .O(new_n59_));
  nor2   g25(.a(new_n59_), .b(new_n52_), .O(z04));
  nor2   g26(.a(new_n58_), .b(x09), .O(new_n61_));
  inv1   g27(.a(new_n61_), .O(new_n62_));
  nand2  g28(.a(new_n58_), .b(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n62_), .c(new_n49_), .O(new_n64_));
  and2   g30(.a(new_n64_), .b(new_n37_), .O(z05));
  inv1   g31(.a(x10), .O(new_n66_));
  nand2  g32(.a(new_n61_), .b(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n48_), .b(x16), .O(new_n68_));
  oai21  g34(.a(new_n58_), .b(x09), .c(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n68_), .c(x04), .O(new_n70_));
  inv1   g36(.a(new_n70_), .O(new_n71_));
  aoi21  g37(.a(new_n71_), .b(new_n67_), .c(new_n43_), .O(z06));
  inv1   g38(.a(x12), .O(new_n73_));
  inv1   g39(.a(x09), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(x08), .c(x07), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(x10), .c(new_n73_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x11), .O(new_n78_));
  and2   g44(.a(x08), .b(x07), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nor2   g46(.a(x09), .b(new_n46_), .O(new_n81_));
  nand4  g47(.a(new_n81_), .b(new_n80_), .c(new_n79_), .d(new_n73_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n49_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(new_n37_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n78_), .O(z07));
  nand3  g51(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n86_));
  nand3  g52(.a(new_n86_), .b(new_n49_), .c(new_n73_), .O(z08));
  inv1   g53(.a(x13), .O(new_n88_));
  nand2  g54(.a(new_n80_), .b(new_n88_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n75_), .c(new_n73_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  nand2  g57(.a(new_n82_), .b(x13), .O(new_n92_));
  nand3  g58(.a(new_n92_), .b(new_n91_), .c(new_n49_), .O(z09));
  nand2  g59(.a(new_n37_), .b(x04), .O(new_n94_));
  nand3  g60(.a(new_n48_), .b(x16), .c(x14), .O(new_n95_));
  inv1   g61(.a(x00), .O(new_n96_));
  nand2  g62(.a(new_n89_), .b(new_n96_), .O(new_n97_));
  nand4  g63(.a(new_n97_), .b(new_n61_), .c(new_n68_), .d(new_n73_), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(new_n95_), .c(new_n94_), .O(z10));
  nor2   g65(.a(new_n43_), .b(new_n35_), .O(z11));
  nand2  g66(.a(x16), .b(new_n35_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n47_), .c(new_n94_), .O(z12));
  nand2  g68(.a(new_n37_), .b(new_n42_), .O(z13));
  inv1   g69(.a(x17), .O(new_n104_));
  oai21  g70(.a(new_n104_), .b(new_n42_), .c(new_n37_), .O(z14));
endmodule


