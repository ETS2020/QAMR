// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n103_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x13), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x06), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  oai21  g12(.a(x03), .b(new_n35_), .c(x16), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(x13), .b(x06), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n54_), .c(new_n49_), .O(new_n56_));
  inv1   g22(.a(new_n56_), .O(z03));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(new_n55_), .b(new_n59_), .O(new_n60_));
  nand3  g26(.a(new_n60_), .b(new_n58_), .c(new_n38_), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(new_n48_), .O(z04));
  nand2  g28(.a(x08), .b(x07), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(x09), .O(new_n64_));
  oai21  g30(.a(new_n64_), .b(new_n37_), .c(x06), .O(new_n65_));
  inv1   g31(.a(new_n53_), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x08), .c(x07), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(x09), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n65_), .c(new_n49_), .O(z05));
  inv1   g35(.a(x09), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand4  g37(.a(new_n71_), .b(new_n70_), .c(x08), .d(x07), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x13), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x06), .O(new_n74_));
  oai21  g40(.a(new_n67_), .b(x09), .c(x10), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n49_), .O(z06));
  nand2  g42(.a(new_n74_), .b(x11), .O(new_n77_));
  nand2  g43(.a(new_n48_), .b(new_n38_), .O(new_n78_));
  inv1   g44(.a(new_n67_), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n70_), .O(new_n81_));
  nand3  g47(.a(new_n81_), .b(new_n78_), .c(new_n77_), .O(z07));
  aoi21  g48(.a(new_n80_), .b(new_n64_), .c(new_n37_), .O(new_n83_));
  oai21  g49(.a(new_n83_), .b(new_n46_), .c(x12), .O(new_n84_));
  nand3  g50(.a(new_n70_), .b(x08), .c(x07), .O(new_n85_));
  inv1   g51(.a(x11), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n71_), .O(new_n87_));
  nor2   g53(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nor2   g54(.a(new_n53_), .b(x12), .O(new_n89_));
  aoi22  g55(.a(new_n89_), .b(new_n88_), .c(new_n48_), .d(new_n38_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(new_n84_), .O(z08));
  nand2  g57(.a(new_n89_), .b(new_n88_), .O(new_n92_));
  nand2  g58(.a(new_n37_), .b(new_n46_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(new_n49_), .O(z09));
  inv1   g61(.a(x04), .O(new_n96_));
  inv1   g62(.a(x03), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x02), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n38_), .c(x16), .d(x14), .O(new_n99_));
  nand4  g65(.a(new_n64_), .b(new_n66_), .c(new_n47_), .d(x00), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n99_), .c(new_n96_), .O(z10));
  nand2  g67(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g68(.a(x16), .b(new_n35_), .c(x03), .O(new_n103_));
  oai21  g69(.a(new_n103_), .b(new_n96_), .c(new_n38_), .O(z12));
  aoi21  g70(.a(new_n37_), .b(x06), .c(new_n96_), .O(z13));
  nand2  g71(.a(x17), .b(x04), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n38_), .O(z14));
endmodule


