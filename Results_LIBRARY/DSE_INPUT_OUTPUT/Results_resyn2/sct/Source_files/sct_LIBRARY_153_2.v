// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x07), .O(new_n37_));
  nor2   g03(.a(x17), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(new_n38_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  inv1   g13(.a(x04), .O(new_n48_));
  nor2   g14(.a(new_n38_), .b(new_n48_), .O(z13));
  oai21  g15(.a(x03), .b(new_n35_), .c(x16), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(z13), .c(new_n47_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(new_n50_), .b(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  inv1   g20(.a(x17), .O(new_n55_));
  oai21  g21(.a(new_n55_), .b(new_n37_), .c(new_n47_), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n54_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n57_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(new_n39_), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n53_), .O(z04));
  nand2  g30(.a(x08), .b(x06), .O(new_n65_));
  nor2   g31(.a(new_n65_), .b(x09), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n55_), .c(x07), .O(new_n67_));
  nand4  g33(.a(x17), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(new_n54_), .O(z05));
  nand2  g36(.a(new_n67_), .b(x10), .O(new_n71_));
  inv1   g37(.a(x10), .O(new_n72_));
  nor2   g38(.a(new_n55_), .b(new_n37_), .O(new_n73_));
  nand3  g39(.a(new_n66_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  nand2  g40(.a(new_n53_), .b(new_n39_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n74_), .c(new_n71_), .O(z06));
  inv1   g42(.a(x09), .O(new_n77_));
  nand4  g43(.a(new_n72_), .b(new_n77_), .c(x08), .d(x06), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x17), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x07), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x11), .O(new_n81_));
  nor2   g47(.a(x11), .b(x10), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n66_), .c(new_n73_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n81_), .c(new_n75_), .O(z07));
  aoi21  g50(.a(new_n82_), .b(new_n66_), .c(new_n55_), .O(new_n85_));
  oai21  g51(.a(new_n85_), .b(new_n37_), .c(x12), .O(new_n86_));
  nor2   g52(.a(new_n68_), .b(x09), .O(new_n87_));
  nor3   g53(.a(x12), .b(x11), .c(x10), .O(new_n88_));
  aoi22  g54(.a(new_n88_), .b(new_n87_), .c(new_n53_), .d(new_n39_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n86_), .O(z08));
  nand2  g56(.a(new_n88_), .b(new_n87_), .O(new_n91_));
  nand2  g57(.a(new_n91_), .b(x13), .O(new_n92_));
  nor2   g58(.a(x13), .b(x12), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n82_), .c(new_n66_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x17), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(x07), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n92_), .c(new_n54_), .O(z09));
  inv1   g63(.a(x00), .O(new_n98_));
  nand2  g64(.a(new_n93_), .b(new_n82_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n87_), .c(new_n50_), .O(new_n101_));
  inv1   g67(.a(x03), .O(new_n102_));
  nand2  g68(.a(new_n102_), .b(x02), .O(new_n103_));
  nand4  g69(.a(new_n103_), .b(new_n39_), .c(x16), .d(x14), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n101_), .c(new_n48_), .O(z10));
  nor2   g71(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n35_), .c(x03), .O(new_n107_));
  nor3   g73(.a(new_n107_), .b(new_n38_), .c(new_n48_), .O(z12));
  oai21  g74(.a(new_n55_), .b(new_n48_), .c(new_n39_), .O(z14));
endmodule


