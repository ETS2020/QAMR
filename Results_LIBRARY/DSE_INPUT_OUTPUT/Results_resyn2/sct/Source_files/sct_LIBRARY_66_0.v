// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n102_,
    new_n104_, new_n106_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  inv1   g03(.a(x06), .O(new_n38_));
  nand2  g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  or2    g05(.a(x14), .b(x01), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n39_), .c(new_n36_), .O(z00));
  oai21  g07(.a(new_n38_), .b(x05), .c(new_n37_), .O(new_n42_));
  inv1   g08(.a(x18), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g10(.a(new_n44_), .b(x05), .c(new_n42_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  nor2   g14(.a(new_n48_), .b(new_n37_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n38_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  nand2  g17(.a(x07), .b(x06), .O(new_n52_));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n38_), .O(new_n54_));
  nand3  g20(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(z03));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n52_), .b(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n49_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n63_));
  nor2   g29(.a(new_n57_), .b(x09), .O(new_n64_));
  inv1   g30(.a(new_n64_), .O(new_n65_));
  nand3  g31(.a(new_n65_), .b(new_n63_), .c(new_n49_), .O(z05));
  nand2  g32(.a(new_n65_), .b(x10), .O(new_n67_));
  inv1   g33(.a(x09), .O(new_n68_));
  inv1   g34(.a(x10), .O(new_n69_));
  inv1   g35(.a(new_n57_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n67_), .c(new_n49_), .O(z06));
  nand2  g38(.a(x08), .b(x07), .O(new_n73_));
  inv1   g39(.a(x11), .O(new_n74_));
  nand3  g40(.a(new_n74_), .b(new_n69_), .c(new_n68_), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(new_n73_), .c(x04), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  aoi21  g43(.a(new_n71_), .b(x11), .c(new_n48_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n37_), .c(new_n77_), .O(z07));
  inv1   g45(.a(x12), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n74_), .c(new_n69_), .d(new_n68_), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n73_), .c(x04), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(x06), .O(new_n83_));
  inv1   g49(.a(new_n62_), .O(new_n84_));
  nor2   g50(.a(x11), .b(x10), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n68_), .O(new_n86_));
  aoi21  g52(.a(new_n86_), .b(x12), .c(new_n48_), .O(new_n87_));
  oai21  g53(.a(new_n87_), .b(new_n37_), .c(new_n83_), .O(z08));
  oai21  g54(.a(new_n81_), .b(new_n62_), .c(x13), .O(new_n89_));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n85_), .b(new_n90_), .c(new_n80_), .O(new_n91_));
  inv1   g57(.a(new_n91_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n64_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n89_), .c(new_n49_), .O(z09));
  nand2  g60(.a(new_n48_), .b(x14), .O(new_n95_));
  inv1   g61(.a(new_n48_), .O(new_n96_));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n91_), .b(new_n97_), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(new_n64_), .c(new_n96_), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(new_n95_), .c(new_n37_), .O(z10));
  aoi21  g66(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z11));
  nand2  g67(.a(x16), .b(new_n35_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n46_), .c(new_n37_), .O(z12));
  nor2   g69(.a(new_n38_), .b(x04), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(z13));
  nor2   g71(.a(x17), .b(new_n37_), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n104_), .O(z14));
endmodule


