// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n98_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  nand2  g03(.a(x06), .b(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  oai21  g06(.a(x06), .b(x05), .c(new_n37_), .O(new_n41_));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  oai21  g09(.a(new_n43_), .b(x05), .c(new_n41_), .O(z01));
  inv1   g10(.a(x06), .O(new_n45_));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(new_n47_));
  nor2   g13(.a(new_n47_), .b(new_n37_), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(new_n38_), .O(z02));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  inv1   g17(.a(x07), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(new_n45_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n51_), .c(new_n48_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z03));
  inv1   g21(.a(x08), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n37_), .c(new_n51_), .O(new_n57_));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n57_), .c(new_n46_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n38_), .O(z04));
  oai21  g26(.a(new_n51_), .b(new_n56_), .c(x09), .O(new_n61_));
  inv1   g27(.a(new_n51_), .O(new_n62_));
  nor2   g28(.a(x09), .b(new_n56_), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n61_), .c(new_n48_), .O(z05));
  nand2  g31(.a(new_n64_), .b(x10), .O(new_n66_));
  nor2   g32(.a(x10), .b(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(new_n62_), .c(x08), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n48_), .O(z06));
  or2    g35(.a(new_n68_), .b(x11), .O(new_n70_));
  nand2  g36(.a(new_n68_), .b(x11), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n70_), .c(new_n48_), .O(z07));
  oai21  g38(.a(x12), .b(new_n37_), .c(new_n45_), .O(new_n73_));
  inv1   g39(.a(x12), .O(new_n74_));
  nand2  g40(.a(x08), .b(x07), .O(new_n75_));
  nor2   g41(.a(new_n75_), .b(x11), .O(new_n76_));
  aoi21  g42(.a(new_n76_), .b(new_n67_), .c(new_n74_), .O(new_n77_));
  nor2   g43(.a(x12), .b(x11), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  oai21  g45(.a(new_n79_), .b(new_n58_), .c(new_n46_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n77_), .c(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n73_), .O(z08));
  oai21  g48(.a(x13), .b(new_n37_), .c(new_n45_), .O(new_n83_));
  nand4  g49(.a(new_n78_), .b(new_n67_), .c(x08), .d(x07), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(x13), .O(new_n85_));
  nor2   g51(.a(x13), .b(x10), .O(new_n86_));
  nand4  g52(.a(new_n86_), .b(new_n78_), .c(new_n63_), .d(new_n62_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n85_), .c(new_n46_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x04), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n83_), .O(z09));
  nand2  g56(.a(new_n47_), .b(x14), .O(new_n91_));
  inv1   g57(.a(x00), .O(new_n92_));
  nand2  g58(.a(new_n86_), .b(new_n78_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n63_), .c(new_n62_), .d(new_n46_), .O(new_n95_));
  aoi21  g61(.a(new_n95_), .b(new_n91_), .c(new_n37_), .O(z10));
  aoi21  g62(.a(x06), .b(new_n37_), .c(new_n35_), .O(z11));
  aoi21  g63(.a(x16), .b(new_n35_), .c(x03), .O(new_n98_));
  nor2   g64(.a(new_n98_), .b(new_n37_), .O(z12));
  nand2  g65(.a(new_n45_), .b(new_n37_), .O(z13));
  and2   g66(.a(x17), .b(x04), .O(z14));
endmodule


