// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n118_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x08), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(x08), .c(new_n47_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  nand2  g18(.a(x07), .b(x06), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n49_), .c(new_n41_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  nand2  g24(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  inv1   g25(.a(new_n53_), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(x08), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n59_), .c(new_n50_), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(z04));
  inv1   g29(.a(x09), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(x06), .O(new_n65_));
  nor2   g31(.a(new_n53_), .b(x09), .O(new_n66_));
  oai21  g32(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  nor2   g33(.a(x09), .b(new_n58_), .O(new_n68_));
  nor2   g34(.a(new_n58_), .b(new_n54_), .O(new_n69_));
  nor2   g35(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  oai22  g36(.a(new_n70_), .b(new_n49_), .c(new_n68_), .d(x06), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n67_), .O(z05));
  aoi21  g38(.a(x10), .b(x06), .c(new_n58_), .O(new_n73_));
  oai21  g39(.a(new_n66_), .b(x10), .c(new_n73_), .O(new_n74_));
  nand2  g40(.a(new_n68_), .b(x07), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(x10), .c(x06), .O(new_n76_));
  oai21  g42(.a(x10), .b(new_n58_), .c(new_n47_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n49_), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(z06));
  inv1   g45(.a(x10), .O(new_n80_));
  nand3  g46(.a(new_n68_), .b(new_n80_), .c(x07), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x11), .O(new_n82_));
  inv1   g48(.a(x11), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n80_), .c(new_n64_), .d(x06), .O(new_n84_));
  inv1   g50(.a(new_n84_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  oai21  g52(.a(x11), .b(new_n58_), .c(new_n47_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n86_), .c(new_n82_), .d(new_n50_), .O(z07));
  oai21  g54(.a(new_n81_), .b(x11), .c(x12), .O(new_n89_));
  inv1   g55(.a(x12), .O(new_n90_));
  nand3  g56(.a(new_n85_), .b(new_n69_), .c(new_n90_), .O(new_n91_));
  oai21  g57(.a(x12), .b(new_n58_), .c(new_n47_), .O(new_n92_));
  nand4  g58(.a(new_n92_), .b(new_n91_), .c(new_n89_), .d(new_n50_), .O(z08));
  nand2  g59(.a(new_n90_), .b(new_n83_), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nor2   g61(.a(new_n95_), .b(new_n47_), .O(new_n96_));
  oai21  g62(.a(new_n94_), .b(new_n81_), .c(new_n96_), .O(new_n97_));
  inv1   g63(.a(new_n66_), .O(new_n98_));
  nand2  g64(.a(x13), .b(new_n47_), .O(new_n99_));
  nand4  g65(.a(new_n95_), .b(new_n90_), .c(new_n83_), .d(new_n80_), .O(new_n100_));
  oai21  g66(.a(new_n100_), .b(new_n98_), .c(new_n99_), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x08), .O(new_n102_));
  oai21  g68(.a(x13), .b(new_n58_), .c(new_n47_), .O(new_n103_));
  nand2  g69(.a(new_n103_), .b(new_n49_), .O(new_n104_));
  nand3  g70(.a(new_n104_), .b(new_n102_), .c(new_n97_), .O(z09));
  inv1   g71(.a(x04), .O(new_n106_));
  inv1   g72(.a(x00), .O(new_n107_));
  nand2  g73(.a(new_n100_), .b(new_n107_), .O(new_n108_));
  inv1   g74(.a(x03), .O(new_n109_));
  nand2  g75(.a(new_n109_), .b(x02), .O(new_n110_));
  nand4  g76(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n111_));
  aoi21  g77(.a(new_n110_), .b(x16), .c(new_n111_), .O(new_n112_));
  inv1   g78(.a(x14), .O(new_n113_));
  nor2   g79(.a(new_n48_), .b(new_n113_), .O(new_n114_));
  aoi21  g80(.a(new_n112_), .b(new_n108_), .c(new_n114_), .O(new_n115_));
  oai21  g81(.a(new_n115_), .b(new_n106_), .c(new_n41_), .O(z10));
  nand2  g82(.a(new_n41_), .b(new_n36_), .O(z11));
  aoi21  g83(.a(x16), .b(new_n36_), .c(x03), .O(new_n118_));
  oai21  g84(.a(new_n118_), .b(new_n106_), .c(new_n41_), .O(z12));
  nor2   g85(.a(new_n37_), .b(new_n106_), .O(z13));
  and2   g86(.a(z13), .b(x17), .O(z14));
endmodule


