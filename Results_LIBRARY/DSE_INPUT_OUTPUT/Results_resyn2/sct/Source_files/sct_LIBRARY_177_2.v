// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n77_, new_n80_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x09), .O(new_n36_));
  nand2  g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  nand2  g04(.a(x14), .b(new_n38_), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n37_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x02), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n47_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  oai21  g15(.a(new_n49_), .b(x06), .c(new_n37_), .O(z02));
  xnor2a g16(.a(x07), .b(x06), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n49_), .c(new_n37_), .O(z03));
  nand3  g18(.a(new_n48_), .b(new_n37_), .c(x04), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  xor2a  g20(.a(new_n54_), .b(x08), .O(new_n55_));
  nor2   g21(.a(new_n55_), .b(new_n53_), .O(z04));
  inv1   g22(.a(x04), .O(new_n57_));
  inv1   g23(.a(x03), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(x02), .O(new_n59_));
  aoi21  g25(.a(new_n59_), .b(x16), .c(new_n57_), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  oai21  g27(.a(new_n54_), .b(new_n61_), .c(x09), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n37_), .O(z05));
  inv1   g29(.a(x10), .O(new_n64_));
  inv1   g30(.a(x06), .O(new_n65_));
  nor2   g31(.a(x09), .b(new_n65_), .O(new_n66_));
  aoi21  g32(.a(new_n60_), .b(new_n64_), .c(new_n66_), .O(z06));
  inv1   g33(.a(x11), .O(new_n68_));
  nand3  g34(.a(new_n60_), .b(new_n37_), .c(new_n68_), .O(z07));
  inv1   g35(.a(x12), .O(new_n70_));
  aoi21  g36(.a(new_n60_), .b(new_n70_), .c(new_n66_), .O(z08));
  inv1   g37(.a(x13), .O(new_n72_));
  nand3  g38(.a(new_n60_), .b(new_n37_), .c(new_n72_), .O(z09));
  nand2  g39(.a(x14), .b(x04), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n48_), .c(new_n37_), .O(z10));
  nand2  g41(.a(new_n37_), .b(new_n47_), .O(z11));
  aoi21  g42(.a(x16), .b(new_n47_), .c(x03), .O(new_n77_));
  nor3   g43(.a(new_n77_), .b(new_n66_), .c(new_n57_), .O(z12));
  nor2   g44(.a(new_n66_), .b(new_n57_), .O(z13));
  nand2  g45(.a(x17), .b(x04), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(new_n37_), .O(z14));
endmodule


