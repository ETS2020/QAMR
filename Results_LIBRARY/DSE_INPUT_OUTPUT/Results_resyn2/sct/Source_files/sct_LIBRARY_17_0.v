// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n107_, new_n110_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x08), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n35_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x08), .c(x06), .O(z02));
  inv1   g14(.a(x03), .O(new_n49_));
  inv1   g15(.a(x16), .O(new_n50_));
  aoi21  g16(.a(new_n49_), .b(x02), .c(new_n50_), .O(new_n51_));
  nor2   g17(.a(new_n51_), .b(new_n43_), .O(new_n52_));
  inv1   g18(.a(x06), .O(new_n53_));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(new_n55_));
  nand2  g21(.a(x08), .b(x07), .O(new_n56_));
  aoi21  g22(.a(new_n56_), .b(new_n53_), .c(new_n55_), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n52_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  nand2  g25(.a(new_n55_), .b(x08), .O(new_n60_));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n54_), .b(new_n61_), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n60_), .c(new_n52_), .O(new_n63_));
  inv1   g29(.a(new_n63_), .O(z04));
  nand2  g30(.a(new_n60_), .b(x09), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand4  g32(.a(new_n66_), .b(x08), .c(x07), .d(x06), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(new_n65_), .c(new_n52_), .d(new_n38_), .O(z05));
  oai21  g34(.a(x10), .b(new_n61_), .c(new_n53_), .O(new_n69_));
  oai21  g35(.a(new_n56_), .b(x09), .c(x10), .O(new_n70_));
  inv1   g36(.a(x10), .O(new_n71_));
  nand4  g37(.a(new_n55_), .b(new_n71_), .c(new_n66_), .d(x08), .O(new_n72_));
  nand4  g38(.a(new_n72_), .b(new_n70_), .c(new_n69_), .d(new_n52_), .O(z06));
  nand4  g39(.a(new_n71_), .b(new_n66_), .c(x08), .d(x07), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x11), .O(new_n75_));
  inv1   g41(.a(new_n67_), .O(new_n76_));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  oai21  g44(.a(x11), .b(new_n61_), .c(new_n53_), .O(new_n79_));
  nand4  g45(.a(new_n79_), .b(new_n78_), .c(new_n75_), .d(new_n52_), .O(z07));
  oai21  g46(.a(new_n74_), .b(x11), .c(x12), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nand3  g48(.a(new_n77_), .b(new_n76_), .c(new_n82_), .O(new_n83_));
  oai21  g49(.a(x12), .b(new_n61_), .c(new_n53_), .O(new_n84_));
  nand4  g50(.a(new_n84_), .b(new_n83_), .c(new_n81_), .d(new_n52_), .O(z08));
  inv1   g51(.a(x11), .O(new_n86_));
  nand2  g52(.a(new_n82_), .b(new_n86_), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nor2   g54(.a(new_n88_), .b(new_n53_), .O(new_n89_));
  oai21  g55(.a(new_n87_), .b(new_n74_), .c(new_n89_), .O(new_n90_));
  nand3  g56(.a(new_n66_), .b(x07), .c(x06), .O(new_n91_));
  nand2  g57(.a(x13), .b(new_n53_), .O(new_n92_));
  nand4  g58(.a(new_n88_), .b(new_n82_), .c(new_n86_), .d(new_n71_), .O(new_n93_));
  oai21  g59(.a(new_n93_), .b(new_n91_), .c(new_n92_), .O(new_n94_));
  nand2  g60(.a(new_n94_), .b(x08), .O(new_n95_));
  oai21  g61(.a(x13), .b(new_n61_), .c(new_n53_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n47_), .O(new_n97_));
  nand3  g63(.a(new_n97_), .b(new_n95_), .c(new_n90_), .O(z09));
  nor2   g64(.a(new_n37_), .b(x04), .O(new_n99_));
  inv1   g65(.a(x00), .O(new_n100_));
  nand2  g66(.a(new_n93_), .b(new_n100_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n76_), .c(new_n46_), .O(new_n102_));
  aoi21  g68(.a(new_n51_), .b(x14), .c(new_n37_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n99_), .O(z10));
  nor2   g70(.a(new_n37_), .b(new_n35_), .O(z11));
  nand2  g71(.a(x16), .b(new_n35_), .O(new_n106_));
  nor2   g72(.a(new_n37_), .b(x03), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n99_), .O(z12));
  inv1   g74(.a(new_n99_), .O(z13));
  nand2  g75(.a(x17), .b(x04), .O(new_n110_));
  nor2   g76(.a(new_n110_), .b(new_n37_), .O(z14));
endmodule


