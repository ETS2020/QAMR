// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n107_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nor2   g02(.a(x15), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x04), .O(new_n41_));
  nor2   g07(.a(new_n37_), .b(new_n41_), .O(new_n42_));
  inv1   g08(.a(x06), .O(new_n43_));
  nor2   g09(.a(new_n43_), .b(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  nor2   g11(.a(new_n45_), .b(x15), .O(new_n46_));
  aoi22  g12(.a(new_n46_), .b(new_n44_), .c(new_n42_), .d(x05), .O(z01));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n41_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(x15), .c(new_n43_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  oai21  g19(.a(x03), .b(new_n35_), .c(x16), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x04), .O(new_n55_));
  xnor2a g21(.a(x07), .b(x06), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n55_), .c(new_n38_), .O(z03));
  nand3  g23(.a(x08), .b(x07), .c(x06), .O(new_n58_));
  inv1   g24(.a(x08), .O(new_n59_));
  nand2  g25(.a(x07), .b(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g27(.a(new_n61_), .b(new_n58_), .c(new_n38_), .O(new_n62_));
  nor2   g28(.a(new_n62_), .b(new_n55_), .O(z04));
  oai21  g29(.a(new_n37_), .b(x09), .c(new_n58_), .O(new_n64_));
  inv1   g30(.a(x09), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(x08), .c(x07), .d(x06), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(new_n64_), .c(new_n51_), .O(z05));
  oai21  g33(.a(new_n37_), .b(x10), .c(new_n66_), .O(new_n68_));
  inv1   g34(.a(x10), .O(new_n69_));
  inv1   g35(.a(new_n66_), .O(new_n70_));
  nand2  g36(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n68_), .c(new_n51_), .O(z06));
  nand4  g38(.a(new_n69_), .b(new_n65_), .c(x08), .d(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(x11), .O(new_n74_));
  nor2   g40(.a(x11), .b(x10), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(new_n70_), .O(new_n76_));
  inv1   g42(.a(x15), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(x11), .c(new_n43_), .O(new_n78_));
  nand4  g44(.a(new_n78_), .b(new_n76_), .c(new_n74_), .d(new_n51_), .O(z07));
  oai21  g45(.a(new_n73_), .b(x11), .c(x12), .O(new_n80_));
  inv1   g46(.a(x12), .O(new_n81_));
  nand3  g47(.a(new_n75_), .b(new_n70_), .c(new_n81_), .O(new_n82_));
  oai21  g48(.a(new_n77_), .b(x12), .c(new_n43_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n82_), .c(new_n80_), .d(new_n51_), .O(z08));
  nand2  g50(.a(x13), .b(new_n43_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n54_), .c(x04), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n38_), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand3  g54(.a(new_n65_), .b(x08), .c(x07), .O(new_n89_));
  inv1   g55(.a(x11), .O(new_n90_));
  nand3  g56(.a(new_n81_), .b(new_n90_), .c(new_n69_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n89_), .c(new_n88_), .O(new_n92_));
  nor2   g58(.a(x09), .b(new_n59_), .O(new_n93_));
  nor3   g59(.a(x12), .b(x11), .c(x10), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n93_), .c(x13), .d(x07), .O(new_n95_));
  nand3  g61(.a(new_n95_), .b(new_n92_), .c(x06), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n87_), .O(z09));
  nor2   g63(.a(new_n37_), .b(x04), .O(new_n98_));
  inv1   g64(.a(x00), .O(new_n99_));
  nand4  g65(.a(new_n88_), .b(new_n81_), .c(new_n90_), .d(new_n69_), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g67(.a(new_n101_), .b(new_n70_), .c(new_n54_), .O(new_n102_));
  aoi21  g68(.a(new_n50_), .b(x14), .c(new_n37_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n102_), .c(new_n98_), .O(z10));
  nor2   g70(.a(new_n37_), .b(new_n35_), .O(z11));
  inv1   g71(.a(new_n42_), .O(new_n106_));
  aoi21  g72(.a(x16), .b(new_n35_), .c(x03), .O(new_n107_));
  nor2   g73(.a(new_n107_), .b(new_n106_), .O(z12));
  inv1   g74(.a(new_n98_), .O(z13));
  and2   g75(.a(new_n42_), .b(x17), .O(z14));
endmodule


