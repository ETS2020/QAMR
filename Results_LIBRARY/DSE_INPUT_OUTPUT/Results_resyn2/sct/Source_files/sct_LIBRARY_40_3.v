// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n109_,
    new_n111_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nand2  g02(.a(x17), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(new_n37_), .O(new_n44_));
  aoi21  g10(.a(new_n42_), .b(x18), .c(new_n44_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  inv1   g12(.a(x16), .O(new_n47_));
  aoi21  g13(.a(new_n46_), .b(x02), .c(new_n47_), .O(new_n48_));
  inv1   g14(.a(x06), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  oai21  g16(.a(new_n50_), .b(new_n48_), .c(new_n37_), .O(z02));
  oai21  g17(.a(x03), .b(new_n36_), .c(x16), .O(new_n52_));
  nand2  g18(.a(new_n52_), .b(x04), .O(new_n53_));
  inv1   g19(.a(new_n53_), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  oai21  g21(.a(x17), .b(new_n49_), .c(new_n55_), .O(new_n56_));
  nand2  g22(.a(x07), .b(x06), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n54_), .O(new_n58_));
  inv1   g24(.a(new_n58_), .O(z03));
  inv1   g25(.a(x08), .O(new_n60_));
  nand2  g26(.a(new_n57_), .b(new_n60_), .O(new_n61_));
  nand3  g27(.a(x08), .b(x07), .c(x06), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(new_n37_), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n53_), .O(z04));
  nand2  g30(.a(new_n53_), .b(new_n37_), .O(new_n65_));
  nand2  g31(.a(x08), .b(x07), .O(new_n66_));
  inv1   g32(.a(new_n66_), .O(new_n67_));
  oai21  g33(.a(new_n67_), .b(x17), .c(x06), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x09), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nor2   g36(.a(x17), .b(new_n49_), .O(new_n71_));
  nand3  g37(.a(new_n67_), .b(new_n71_), .c(new_n70_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n69_), .c(new_n65_), .O(z05));
  nor2   g39(.a(x10), .b(x09), .O(new_n74_));
  aoi21  g40(.a(new_n74_), .b(new_n67_), .c(x17), .O(new_n75_));
  aoi21  g41(.a(new_n72_), .b(x10), .c(new_n53_), .O(new_n76_));
  oai21  g42(.a(new_n75_), .b(new_n49_), .c(new_n76_), .O(z06));
  oai21  g43(.a(new_n75_), .b(new_n49_), .c(x11), .O(new_n78_));
  nand2  g44(.a(new_n74_), .b(new_n67_), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  nand2  g46(.a(new_n71_), .b(new_n80_), .O(new_n81_));
  or2    g47(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n78_), .c(new_n65_), .O(z07));
  inv1   g49(.a(x17), .O(new_n84_));
  nor2   g50(.a(x12), .b(x11), .O(new_n85_));
  nand4  g51(.a(new_n85_), .b(new_n74_), .c(x08), .d(x07), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand2  g53(.a(new_n87_), .b(x06), .O(new_n88_));
  oai21  g54(.a(new_n81_), .b(new_n79_), .c(x12), .O(new_n89_));
  nand3  g55(.a(new_n89_), .b(new_n88_), .c(new_n54_), .O(z08));
  nand2  g56(.a(new_n88_), .b(x13), .O(new_n91_));
  nand3  g57(.a(new_n84_), .b(new_n70_), .c(x06), .O(new_n92_));
  nor2   g58(.a(new_n92_), .b(new_n66_), .O(new_n93_));
  inv1   g59(.a(x10), .O(new_n94_));
  inv1   g60(.a(x13), .O(new_n95_));
  nand3  g61(.a(new_n85_), .b(new_n95_), .c(new_n94_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  aoi22  g63(.a(new_n97_), .b(new_n93_), .c(new_n53_), .d(new_n37_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n91_), .O(z09));
  nand2  g65(.a(new_n37_), .b(x04), .O(new_n100_));
  nand2  g66(.a(new_n48_), .b(x14), .O(new_n101_));
  inv1   g67(.a(x00), .O(new_n102_));
  nand2  g68(.a(new_n96_), .b(new_n102_), .O(new_n103_));
  nand3  g69(.a(new_n103_), .b(new_n93_), .c(new_n52_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n101_), .c(new_n100_), .O(z10));
  nor2   g71(.a(new_n38_), .b(new_n36_), .O(z11));
  nand2  g72(.a(x16), .b(new_n36_), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n46_), .c(new_n100_), .O(z12));
  inv1   g74(.a(x04), .O(new_n109_));
  nand2  g75(.a(new_n37_), .b(new_n109_), .O(z13));
  nand3  g76(.a(x17), .b(new_n49_), .c(x04), .O(new_n111_));
  inv1   g77(.a(new_n111_), .O(z14));
endmodule


