// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x04), .O(new_n37_));
  nand2  g03(.a(x06), .b(new_n37_), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g06(.a(x15), .b(x05), .O(new_n41_));
  nand2  g07(.a(new_n41_), .b(x18), .O(new_n42_));
  nand2  g08(.a(x05), .b(x04), .O(new_n43_));
  nand3  g09(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  inv1   g10(.a(new_n44_), .O(z01));
  inv1   g11(.a(x03), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x02), .O(new_n47_));
  nand2  g13(.a(new_n47_), .b(x16), .O(new_n48_));
  inv1   g14(.a(new_n48_), .O(new_n49_));
  inv1   g15(.a(x06), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(x04), .O(new_n51_));
  oai21  g17(.a(new_n51_), .b(new_n49_), .c(new_n38_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(x06), .O(new_n54_));
  oai21  g20(.a(new_n51_), .b(new_n53_), .c(new_n54_), .O(new_n55_));
  nand2  g21(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(new_n38_), .O(z03));
  inv1   g23(.a(x08), .O(new_n58_));
  aoi21  g24(.a(new_n54_), .b(new_n51_), .c(new_n58_), .O(new_n59_));
  nor3   g25(.a(x08), .b(new_n53_), .c(new_n50_), .O(new_n60_));
  oai21  g26(.a(new_n60_), .b(new_n59_), .c(new_n48_), .O(new_n61_));
  nand2  g27(.a(new_n61_), .b(new_n38_), .O(z04));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n64_));
  nor2   g30(.a(new_n58_), .b(new_n53_), .O(new_n65_));
  nor2   g31(.a(x09), .b(new_n50_), .O(new_n66_));
  nand2  g32(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nor2   g33(.a(new_n49_), .b(new_n37_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n67_), .c(new_n64_), .O(z05));
  inv1   g35(.a(new_n63_), .O(new_n70_));
  nor2   g36(.a(x10), .b(x09), .O(new_n71_));
  nand2  g37(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g38(.a(new_n67_), .b(x10), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n72_), .c(new_n68_), .O(z06));
  oai21  g40(.a(x11), .b(new_n37_), .c(new_n50_), .O(new_n75_));
  inv1   g41(.a(x11), .O(new_n76_));
  nand3  g42(.a(new_n71_), .b(new_n70_), .c(new_n76_), .O(new_n77_));
  nand3  g43(.a(new_n71_), .b(x08), .c(x07), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x11), .O(new_n79_));
  nand3  g45(.a(new_n79_), .b(new_n77_), .c(new_n48_), .O(new_n80_));
  nand2  g46(.a(new_n80_), .b(x04), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n75_), .O(z07));
  nand2  g48(.a(new_n77_), .b(x12), .O(new_n83_));
  nor2   g49(.a(x12), .b(x11), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n71_), .c(new_n70_), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n83_), .c(new_n68_), .O(z08));
  oai21  g52(.a(x13), .b(new_n37_), .c(new_n50_), .O(new_n87_));
  nand4  g53(.a(new_n84_), .b(new_n71_), .c(x08), .d(x07), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x13), .O(new_n89_));
  nor2   g55(.a(x13), .b(x10), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n84_), .c(new_n66_), .d(new_n65_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n89_), .c(new_n48_), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x04), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n87_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(new_n84_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  inv1   g63(.a(x09), .O(new_n98_));
  nand3  g64(.a(new_n98_), .b(x08), .c(x07), .O(new_n99_));
  aoi21  g65(.a(new_n47_), .b(x16), .c(new_n99_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(new_n97_), .c(new_n37_), .O(new_n101_));
  nand3  g67(.a(new_n49_), .b(x14), .c(x04), .O(new_n102_));
  oai21  g68(.a(new_n101_), .b(new_n50_), .c(new_n102_), .O(z10));
  nand2  g69(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n35_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n37_), .c(new_n38_), .O(z12));
  and2   g72(.a(x17), .b(x04), .O(z14));
  buf    g73(.a(x04), .O(z13));
endmodule


