// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n109_;
  nand2  g00(.a(x02), .b(x01), .O(new_n35_));
  inv1   g01(.a(x10), .O(new_n36_));
  nor2   g02(.a(new_n36_), .b(x06), .O(new_n37_));
  inv1   g03(.a(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  nand2  g05(.a(x14), .b(new_n39_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n38_), .c(new_n35_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  inv1   g14(.a(x04), .O(new_n49_));
  inv1   g15(.a(x03), .O(new_n50_));
  inv1   g16(.a(x16), .O(new_n51_));
  aoi21  g17(.a(new_n50_), .b(x02), .c(new_n51_), .O(new_n52_));
  nor2   g18(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand3  g19(.a(new_n53_), .b(new_n36_), .c(new_n48_), .O(new_n54_));
  inv1   g20(.a(new_n54_), .O(z02));
  nand2  g21(.a(x07), .b(x06), .O(new_n56_));
  inv1   g22(.a(x07), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n48_), .O(new_n58_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(new_n59_));
  nand2  g25(.a(new_n59_), .b(new_n38_), .O(z03));
  inv1   g26(.a(x08), .O(new_n61_));
  nand2  g27(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  nand3  g28(.a(x08), .b(x07), .c(x06), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n62_), .c(new_n53_), .O(new_n64_));
  nand2  g30(.a(new_n64_), .b(new_n38_), .O(z04));
  oai21  g31(.a(new_n37_), .b(x09), .c(new_n63_), .O(new_n66_));
  nor2   g32(.a(new_n63_), .b(x09), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n53_), .O(z05));
  nand2  g35(.a(x08), .b(x07), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(x09), .c(x10), .O(new_n71_));
  inv1   g37(.a(x09), .O(new_n72_));
  nand4  g38(.a(new_n36_), .b(new_n72_), .c(x08), .d(x07), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(x06), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n53_), .c(new_n37_), .O(z06));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n67_), .O(new_n78_));
  nand2  g44(.a(new_n73_), .b(x11), .O(new_n79_));
  oai21  g45(.a(x11), .b(x10), .c(new_n48_), .O(new_n80_));
  nand4  g46(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n53_), .O(z07));
  inv1   g47(.a(x12), .O(new_n82_));
  nand3  g48(.a(new_n77_), .b(new_n67_), .c(new_n82_), .O(new_n83_));
  inv1   g49(.a(new_n70_), .O(new_n84_));
  nand3  g50(.a(new_n77_), .b(new_n84_), .c(new_n72_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x12), .O(new_n86_));
  oai21  g52(.a(x12), .b(x10), .c(new_n48_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n86_), .c(new_n83_), .d(new_n53_), .O(z08));
  oai21  g54(.a(new_n85_), .b(x12), .c(x13), .O(new_n89_));
  inv1   g55(.a(x11), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n82_), .c(new_n90_), .d(new_n36_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n67_), .O(new_n94_));
  oai21  g60(.a(x13), .b(x10), .c(new_n48_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(new_n89_), .d(new_n53_), .O(z09));
  nor2   g62(.a(new_n37_), .b(x04), .O(new_n97_));
  inv1   g63(.a(new_n52_), .O(new_n98_));
  inv1   g64(.a(x00), .O(new_n99_));
  nand2  g65(.a(new_n92_), .b(new_n99_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n67_), .c(new_n98_), .O(new_n101_));
  aoi21  g67(.a(new_n52_), .b(x14), .c(new_n37_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n97_), .O(z10));
  inv1   g69(.a(x02), .O(new_n104_));
  nand2  g70(.a(new_n38_), .b(new_n104_), .O(z11));
  aoi21  g71(.a(x16), .b(new_n104_), .c(x03), .O(new_n106_));
  nor3   g72(.a(new_n106_), .b(new_n37_), .c(new_n49_), .O(z12));
  inv1   g73(.a(new_n97_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  oai21  g75(.a(new_n109_), .b(new_n49_), .c(new_n38_), .O(z14));
endmodule


