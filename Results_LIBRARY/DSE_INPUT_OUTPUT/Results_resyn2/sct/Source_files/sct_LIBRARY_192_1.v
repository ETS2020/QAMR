// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n108_,
    new_n111_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(x10), .b(new_n37_), .O(new_n38_));
  inv1   g04(.a(x01), .O(new_n39_));
  inv1   g05(.a(x14), .O(new_n40_));
  nand2  g06(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand3  g07(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  nand2  g09(.a(new_n43_), .b(x18), .O(new_n44_));
  nand2  g10(.a(x05), .b(x04), .O(new_n45_));
  nand3  g11(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(new_n46_));
  inv1   g12(.a(new_n46_), .O(z01));
  inv1   g13(.a(x10), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n48_), .c(new_n37_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  nand2  g19(.a(x07), .b(x06), .O(new_n54_));
  inv1   g20(.a(x07), .O(new_n55_));
  oai21  g21(.a(x10), .b(new_n55_), .c(new_n37_), .O(new_n56_));
  nand3  g22(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  inv1   g23(.a(new_n57_), .O(z03));
  nand2  g24(.a(new_n38_), .b(x08), .O(new_n59_));
  nand3  g25(.a(x08), .b(x07), .c(x06), .O(new_n60_));
  nand2  g26(.a(new_n60_), .b(new_n51_), .O(new_n61_));
  aoi21  g27(.a(new_n59_), .b(new_n54_), .c(new_n61_), .O(z04));
  nand2  g28(.a(new_n50_), .b(new_n38_), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  inv1   g30(.a(new_n38_), .O(new_n65_));
  aoi21  g31(.a(new_n60_), .b(new_n64_), .c(new_n65_), .O(new_n66_));
  oai21  g32(.a(new_n60_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n63_), .O(z05));
  nand3  g34(.a(new_n64_), .b(x08), .c(x07), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(x10), .O(new_n70_));
  nor2   g36(.a(x10), .b(new_n55_), .O(new_n71_));
  inv1   g37(.a(x08), .O(new_n72_));
  nor2   g38(.a(x09), .b(new_n72_), .O(new_n73_));
  nand2  g39(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  nand2  g41(.a(new_n75_), .b(x06), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n63_), .O(z06));
  oai21  g43(.a(x11), .b(x10), .c(new_n37_), .O(new_n78_));
  nand2  g44(.a(new_n74_), .b(x11), .O(new_n79_));
  nor2   g45(.a(x11), .b(x10), .O(new_n80_));
  nand4  g46(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n81_));
  inv1   g47(.a(new_n81_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n79_), .c(new_n78_), .d(new_n51_), .O(z07));
  nor2   g50(.a(x12), .b(x11), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n73_), .c(new_n71_), .O(new_n86_));
  or2    g52(.a(new_n86_), .b(new_n37_), .O(new_n87_));
  oai21  g53(.a(new_n65_), .b(x12), .c(new_n83_), .O(new_n88_));
  nand3  g54(.a(new_n88_), .b(new_n87_), .c(new_n51_), .O(z08));
  inv1   g55(.a(x13), .O(new_n90_));
  nand3  g56(.a(new_n85_), .b(new_n90_), .c(x06), .O(new_n91_));
  oai22  g57(.a(new_n91_), .b(new_n69_), .c(new_n90_), .d(x06), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n48_), .O(new_n93_));
  aoi21  g59(.a(new_n90_), .b(new_n48_), .c(x06), .O(new_n94_));
  aoi21  g60(.a(new_n86_), .b(x13), .c(new_n50_), .O(new_n95_));
  oai21  g61(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(z09));
  inv1   g62(.a(x04), .O(new_n97_));
  inv1   g63(.a(x00), .O(new_n98_));
  nand3  g64(.a(new_n85_), .b(new_n90_), .c(new_n48_), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  inv1   g66(.a(x03), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x02), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(x16), .c(new_n81_), .O(new_n103_));
  nor2   g69(.a(new_n49_), .b(new_n40_), .O(new_n104_));
  aoi21  g70(.a(new_n103_), .b(new_n100_), .c(new_n104_), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n97_), .c(new_n38_), .O(z10));
  nor2   g72(.a(new_n65_), .b(new_n35_), .O(z11));
  aoi21  g73(.a(x16), .b(new_n35_), .c(x03), .O(new_n108_));
  nor3   g74(.a(new_n108_), .b(new_n65_), .c(new_n97_), .O(z12));
  nor2   g75(.a(new_n65_), .b(new_n97_), .O(z13));
  nand2  g76(.a(x17), .b(x04), .O(new_n111_));
  nand2  g77(.a(new_n111_), .b(new_n38_), .O(z14));
endmodule


