// Benchmark "FAU" written by ABC on Tue Aug 11 19:44:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n108_, new_n110_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  nand2  g02(.a(x18), .b(x06), .O(new_n37_));
  inv1   g03(.a(x01), .O(new_n38_));
  inv1   g04(.a(x14), .O(new_n39_));
  nand2  g05(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g06(.a(new_n40_), .b(new_n37_), .c(new_n36_), .O(z00));
  nand2  g07(.a(new_n37_), .b(x04), .O(new_n42_));
  inv1   g08(.a(new_n42_), .O(z13));
  nor2   g09(.a(x06), .b(x05), .O(new_n44_));
  inv1   g10(.a(x18), .O(new_n45_));
  nor2   g11(.a(new_n45_), .b(x15), .O(new_n46_));
  aoi22  g12(.a(new_n46_), .b(new_n44_), .c(z13), .d(x05), .O(z01));
  inv1   g13(.a(x06), .O(new_n48_));
  oai21  g14(.a(x03), .b(new_n35_), .c(x16), .O(new_n49_));
  nand2  g15(.a(new_n49_), .b(x04), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nor2   g20(.a(x18), .b(new_n48_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nor2   g22(.a(new_n54_), .b(new_n48_), .O(new_n57_));
  aoi21  g23(.a(new_n56_), .b(new_n54_), .c(new_n57_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n51_), .O(new_n59_));
  inv1   g25(.a(new_n59_), .O(z03));
  nor2   g26(.a(new_n57_), .b(x08), .O(new_n61_));
  nand2  g27(.a(new_n57_), .b(x08), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n37_), .O(new_n63_));
  nor3   g29(.a(new_n63_), .b(new_n61_), .c(new_n50_), .O(z04));
  nand2  g30(.a(new_n50_), .b(new_n37_), .O(new_n65_));
  nand2  g31(.a(x08), .b(x07), .O(new_n66_));
  nor2   g32(.a(new_n66_), .b(x09), .O(new_n67_));
  nand2  g33(.a(new_n67_), .b(new_n55_), .O(new_n68_));
  aoi21  g34(.a(x08), .b(x07), .c(x18), .O(new_n69_));
  oai21  g35(.a(new_n69_), .b(new_n48_), .c(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n68_), .c(new_n65_), .O(z05));
  oai21  g37(.a(new_n67_), .b(x18), .c(x06), .O(new_n72_));
  nand2  g38(.a(new_n72_), .b(x10), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  nand3  g40(.a(new_n67_), .b(new_n55_), .c(new_n74_), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n65_), .O(z06));
  nor2   g42(.a(x11), .b(x10), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n67_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(new_n45_), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x06), .O(new_n80_));
  aoi21  g46(.a(new_n75_), .b(x11), .c(new_n50_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(new_n80_), .O(z07));
  oai21  g48(.a(new_n78_), .b(x12), .c(new_n45_), .O(new_n83_));
  nand2  g49(.a(new_n83_), .b(x06), .O(new_n84_));
  oai21  g50(.a(new_n78_), .b(new_n56_), .c(x12), .O(new_n85_));
  nand3  g51(.a(new_n85_), .b(new_n84_), .c(new_n51_), .O(z08));
  inv1   g52(.a(x12), .O(new_n87_));
  inv1   g53(.a(x13), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n78_), .c(new_n45_), .O(new_n90_));
  nand2  g56(.a(new_n90_), .b(x06), .O(new_n91_));
  nand2  g57(.a(new_n55_), .b(new_n87_), .O(new_n92_));
  oai21  g58(.a(new_n92_), .b(new_n78_), .c(x13), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n91_), .c(new_n51_), .O(z09));
  inv1   g60(.a(x04), .O(new_n95_));
  inv1   g61(.a(x00), .O(new_n96_));
  nand3  g62(.a(new_n77_), .b(new_n88_), .c(new_n87_), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  inv1   g64(.a(x03), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(x02), .O(new_n100_));
  inv1   g66(.a(x09), .O(new_n101_));
  nand4  g67(.a(new_n101_), .b(x08), .c(x07), .d(x06), .O(new_n102_));
  aoi21  g68(.a(new_n100_), .b(x16), .c(new_n102_), .O(new_n103_));
  nor2   g69(.a(new_n49_), .b(new_n39_), .O(new_n104_));
  aoi21  g70(.a(new_n103_), .b(new_n98_), .c(new_n104_), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n95_), .c(new_n37_), .O(z10));
  nand2  g72(.a(new_n37_), .b(new_n35_), .O(z11));
  nand2  g73(.a(x16), .b(new_n35_), .O(new_n108_));
  aoi21  g74(.a(new_n108_), .b(new_n99_), .c(new_n42_), .O(z12));
  nand2  g75(.a(z13), .b(x17), .O(new_n110_));
  inv1   g76(.a(new_n110_), .O(z14));
endmodule


