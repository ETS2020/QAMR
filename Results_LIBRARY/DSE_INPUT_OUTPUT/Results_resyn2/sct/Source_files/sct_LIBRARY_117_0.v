// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n106_, new_n107_, new_n110_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x12), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  oai21  g11(.a(x03), .b(new_n36_), .c(x16), .O(new_n46_));
  nand2  g12(.a(new_n46_), .b(x04), .O(new_n47_));
  aoi21  g13(.a(new_n47_), .b(x12), .c(x06), .O(z02));
  nor2   g14(.a(x07), .b(x06), .O(new_n49_));
  inv1   g15(.a(new_n37_), .O(new_n50_));
  nand2  g16(.a(x07), .b(x06), .O(new_n51_));
  nand2  g17(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor3   g18(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z03));
  inv1   g19(.a(x08), .O(new_n54_));
  nand2  g20(.a(new_n51_), .b(new_n54_), .O(new_n55_));
  inv1   g21(.a(new_n51_), .O(new_n56_));
  nand2  g22(.a(new_n56_), .b(x08), .O(new_n57_));
  nand2  g23(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(z04));
  inv1   g25(.a(new_n47_), .O(new_n60_));
  oai21  g26(.a(new_n37_), .b(x09), .c(new_n57_), .O(new_n61_));
  nor2   g27(.a(x09), .b(new_n54_), .O(new_n62_));
  nand2  g28(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(z05));
  inv1   g30(.a(x10), .O(new_n65_));
  inv1   g31(.a(x09), .O(new_n66_));
  nand3  g32(.a(new_n66_), .b(x08), .c(x07), .O(new_n67_));
  xor2a  g33(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x06), .O(new_n69_));
  inv1   g35(.a(x06), .O(new_n70_));
  nand2  g36(.a(x10), .b(new_n70_), .O(new_n71_));
  nand3  g37(.a(new_n71_), .b(new_n46_), .c(x04), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  aoi21  g39(.a(new_n73_), .b(new_n69_), .c(new_n37_), .O(z06));
  oai21  g40(.a(new_n67_), .b(x10), .c(x11), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n62_), .c(x07), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  inv1   g45(.a(x11), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(x06), .O(new_n81_));
  oai21  g47(.a(new_n81_), .b(new_n47_), .c(new_n50_), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n79_), .O(z07));
  nand2  g49(.a(new_n77_), .b(x12), .O(new_n84_));
  inv1   g50(.a(x12), .O(new_n85_));
  inv1   g51(.a(new_n67_), .O(new_n86_));
  nand3  g52(.a(new_n76_), .b(new_n86_), .c(new_n85_), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n84_), .c(new_n60_), .d(x06), .O(z08));
  nand2  g54(.a(new_n80_), .b(new_n65_), .O(new_n89_));
  oai21  g55(.a(new_n89_), .b(new_n67_), .c(x13), .O(new_n90_));
  nor2   g56(.a(x13), .b(x12), .O(new_n91_));
  nand4  g57(.a(new_n91_), .b(new_n76_), .c(new_n62_), .d(x07), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x06), .O(new_n94_));
  aoi22  g60(.a(new_n47_), .b(new_n50_), .c(x13), .d(x12), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n94_), .O(z09));
  nor2   g62(.a(new_n37_), .b(x04), .O(new_n97_));
  nand2  g63(.a(new_n91_), .b(new_n76_), .O(new_n98_));
  nand2  g64(.a(x06), .b(x00), .O(new_n99_));
  nand2  g65(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n86_), .c(new_n46_), .O(new_n101_));
  inv1   g67(.a(new_n46_), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(x14), .c(new_n37_), .O(new_n103_));
  aoi21  g69(.a(new_n103_), .b(new_n101_), .c(new_n97_), .O(z10));
  nand2  g70(.a(new_n50_), .b(new_n36_), .O(z11));
  nand2  g71(.a(x16), .b(new_n36_), .O(new_n106_));
  nor2   g72(.a(new_n37_), .b(x03), .O(new_n107_));
  aoi21  g73(.a(new_n107_), .b(new_n106_), .c(new_n97_), .O(z12));
  inv1   g74(.a(new_n97_), .O(z13));
  inv1   g75(.a(x17), .O(new_n110_));
  oai21  g76(.a(new_n110_), .b(new_n43_), .c(new_n50_), .O(z14));
endmodule


