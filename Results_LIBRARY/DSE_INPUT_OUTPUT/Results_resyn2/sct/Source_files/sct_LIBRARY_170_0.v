// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n104_, new_n105_, new_n108_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x11), .b(x07), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(new_n37_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  oai21  g13(.a(x03), .b(new_n36_), .c(x16), .O(new_n48_));
  nand2  g14(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g15(.a(new_n49_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(new_n41_), .c(new_n47_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(x07), .O(new_n53_));
  nand2  g19(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand2  g20(.a(x07), .b(x06), .O(new_n55_));
  nand3  g21(.a(new_n55_), .b(new_n54_), .c(new_n41_), .O(new_n56_));
  nor2   g22(.a(new_n56_), .b(new_n49_), .O(z03));
  xor2a  g23(.a(new_n55_), .b(x08), .O(new_n58_));
  oai21  g24(.a(new_n58_), .b(new_n49_), .c(new_n41_), .O(z04));
  inv1   g25(.a(x08), .O(new_n60_));
  oai22  g26(.a(new_n55_), .b(new_n60_), .c(new_n37_), .d(x09), .O(new_n61_));
  inv1   g27(.a(x09), .O(new_n62_));
  nand4  g28(.a(new_n62_), .b(x08), .c(x07), .d(x06), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n50_), .O(z05));
  nand2  g30(.a(new_n49_), .b(new_n41_), .O(new_n65_));
  aoi21  g31(.a(x11), .b(x10), .c(x07), .O(new_n66_));
  nor2   g32(.a(x10), .b(x09), .O(new_n67_));
  nand3  g33(.a(new_n67_), .b(x08), .c(x06), .O(new_n68_));
  inv1   g34(.a(new_n68_), .O(new_n69_));
  aoi21  g35(.a(new_n63_), .b(x10), .c(new_n69_), .O(new_n70_));
  oai21  g36(.a(new_n70_), .b(new_n66_), .c(new_n65_), .O(z06));
  nand2  g37(.a(new_n68_), .b(x11), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  inv1   g39(.a(x11), .O(new_n74_));
  nand3  g40(.a(new_n62_), .b(x08), .c(x06), .O(new_n75_));
  inv1   g41(.a(new_n75_), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n74_), .c(new_n73_), .O(new_n77_));
  nand4  g43(.a(new_n77_), .b(new_n72_), .c(new_n50_), .d(x07), .O(z07));
  inv1   g44(.a(x12), .O(new_n79_));
  nand2  g45(.a(new_n74_), .b(new_n73_), .O(new_n80_));
  oai21  g46(.a(new_n80_), .b(new_n75_), .c(new_n79_), .O(new_n81_));
  nand2  g47(.a(new_n69_), .b(x12), .O(new_n82_));
  nand3  g48(.a(new_n82_), .b(new_n81_), .c(x07), .O(new_n83_));
  nand2  g49(.a(x12), .b(x11), .O(new_n84_));
  nand3  g50(.a(new_n84_), .b(new_n83_), .c(new_n65_), .O(z08));
  nand4  g51(.a(new_n67_), .b(new_n79_), .c(x08), .d(x06), .O(new_n86_));
  inv1   g52(.a(x13), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n79_), .c(new_n74_), .d(new_n73_), .O(new_n88_));
  inv1   g54(.a(new_n88_), .O(new_n89_));
  aoi22  g55(.a(new_n89_), .b(new_n76_), .c(new_n86_), .d(x13), .O(new_n90_));
  nor2   g56(.a(new_n87_), .b(new_n74_), .O(new_n91_));
  aoi21  g57(.a(new_n49_), .b(new_n41_), .c(new_n91_), .O(new_n92_));
  oai21  g58(.a(new_n90_), .b(new_n53_), .c(new_n92_), .O(z09));
  nor2   g59(.a(new_n37_), .b(x04), .O(new_n94_));
  nand2  g60(.a(x07), .b(x00), .O(new_n95_));
  nand2  g61(.a(new_n95_), .b(new_n88_), .O(new_n96_));
  nand3  g62(.a(new_n96_), .b(new_n76_), .c(new_n48_), .O(new_n97_));
  inv1   g63(.a(x03), .O(new_n98_));
  inv1   g64(.a(x16), .O(new_n99_));
  aoi21  g65(.a(new_n98_), .b(x02), .c(new_n99_), .O(new_n100_));
  aoi21  g66(.a(new_n100_), .b(x14), .c(new_n37_), .O(new_n101_));
  aoi21  g67(.a(new_n101_), .b(new_n97_), .c(new_n94_), .O(z10));
  nor2   g68(.a(new_n37_), .b(new_n36_), .O(z11));
  nand2  g69(.a(x16), .b(new_n36_), .O(new_n104_));
  nor2   g70(.a(new_n37_), .b(x03), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n104_), .c(new_n94_), .O(z12));
  inv1   g72(.a(new_n94_), .O(z13));
  nor2   g73(.a(new_n37_), .b(x17), .O(new_n108_));
  nor2   g74(.a(new_n108_), .b(new_n94_), .O(z14));
endmodule


