// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n107_, new_n108_,
    new_n111_;
  inv1   g00(.a(x02), .O(new_n35_));
  nand2  g01(.a(new_n35_), .b(x01), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nand2  g03(.a(new_n37_), .b(x05), .O(new_n38_));
  or2    g04(.a(x14), .b(x01), .O(new_n39_));
  nand3  g05(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z00));
  inv1   g06(.a(x05), .O(new_n41_));
  inv1   g07(.a(x15), .O(new_n42_));
  nand3  g08(.a(x18), .b(new_n42_), .c(new_n41_), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n38_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x04), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand3  g17(.a(new_n51_), .b(new_n37_), .c(new_n41_), .O(new_n52_));
  inv1   g18(.a(new_n52_), .O(z02));
  inv1   g19(.a(x07), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n37_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n54_), .b(new_n37_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n51_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n38_), .O(z03));
  aoi21  g25(.a(new_n38_), .b(x08), .c(new_n55_), .O(new_n60_));
  oai21  g26(.a(x03), .b(new_n35_), .c(x16), .O(new_n61_));
  nand2  g27(.a(new_n55_), .b(x08), .O(new_n62_));
  nand3  g28(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  nor2   g29(.a(new_n63_), .b(new_n60_), .O(z04));
  nor2   g30(.a(x06), .b(new_n41_), .O(new_n65_));
  oai21  g31(.a(new_n65_), .b(x09), .c(new_n62_), .O(new_n66_));
  inv1   g32(.a(x09), .O(new_n67_));
  nand4  g33(.a(new_n67_), .b(x08), .c(x07), .d(x06), .O(new_n68_));
  nand3  g34(.a(new_n68_), .b(new_n66_), .c(new_n51_), .O(z05));
  inv1   g35(.a(x10), .O(new_n70_));
  nand2  g36(.a(new_n68_), .b(new_n70_), .O(new_n71_));
  inv1   g37(.a(x08), .O(new_n72_));
  nor2   g38(.a(x09), .b(new_n72_), .O(new_n73_));
  nand3  g39(.a(new_n73_), .b(new_n55_), .c(x10), .O(new_n74_));
  nand2  g40(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  aoi21  g41(.a(new_n75_), .b(new_n51_), .c(new_n65_), .O(z06));
  nand3  g42(.a(new_n67_), .b(x08), .c(x07), .O(new_n77_));
  oai21  g43(.a(new_n77_), .b(x10), .c(x11), .O(new_n78_));
  inv1   g44(.a(x11), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(new_n70_), .O(new_n80_));
  nor2   g46(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nand2  g47(.a(new_n81_), .b(x06), .O(new_n82_));
  oai21  g48(.a(x11), .b(x05), .c(new_n37_), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n82_), .c(new_n78_), .d(new_n51_), .O(z07));
  inv1   g50(.a(x12), .O(new_n85_));
  nand3  g51(.a(new_n81_), .b(new_n85_), .c(x06), .O(new_n86_));
  nor2   g52(.a(x11), .b(x10), .O(new_n87_));
  nand4  g53(.a(new_n87_), .b(new_n67_), .c(x08), .d(x07), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(x12), .O(new_n89_));
  oai21  g55(.a(x12), .b(x05), .c(new_n37_), .O(new_n90_));
  nand4  g56(.a(new_n90_), .b(new_n89_), .c(new_n86_), .d(new_n51_), .O(z08));
  oai21  g57(.a(new_n88_), .b(x12), .c(x13), .O(new_n92_));
  inv1   g58(.a(new_n68_), .O(new_n93_));
  inv1   g59(.a(x13), .O(new_n94_));
  nand4  g60(.a(new_n94_), .b(new_n85_), .c(new_n79_), .d(new_n70_), .O(new_n95_));
  inv1   g61(.a(new_n95_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  oai21  g63(.a(x13), .b(x05), .c(new_n37_), .O(new_n98_));
  nand4  g64(.a(new_n98_), .b(new_n97_), .c(new_n92_), .d(new_n51_), .O(z09));
  nor2   g65(.a(new_n65_), .b(x04), .O(new_n100_));
  inv1   g66(.a(x00), .O(new_n101_));
  nand2  g67(.a(new_n95_), .b(new_n101_), .O(new_n102_));
  nand3  g68(.a(new_n102_), .b(new_n93_), .c(new_n61_), .O(new_n103_));
  aoi21  g69(.a(new_n50_), .b(x14), .c(new_n65_), .O(new_n104_));
  aoi21  g70(.a(new_n104_), .b(new_n103_), .c(new_n100_), .O(z10));
  nand2  g71(.a(new_n38_), .b(new_n35_), .O(z11));
  aoi21  g72(.a(x16), .b(new_n35_), .c(x03), .O(new_n107_));
  nand2  g73(.a(new_n38_), .b(x04), .O(new_n108_));
  nor2   g74(.a(new_n108_), .b(new_n107_), .O(z12));
  inv1   g75(.a(new_n100_), .O(z13));
  inv1   g76(.a(x17), .O(new_n111_));
  nor2   g77(.a(new_n108_), .b(new_n111_), .O(z14));
endmodule


