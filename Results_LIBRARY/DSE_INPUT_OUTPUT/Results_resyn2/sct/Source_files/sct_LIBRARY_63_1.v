// Benchmark "FAU" written by ABC on Tue Aug 11 19:45:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x14), .O(new_n36_));
  inv1   g02(.a(x07), .O(new_n37_));
  nor2   g03(.a(new_n37_), .b(x06), .O(new_n38_));
  aoi21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(x02), .b(new_n35_), .c(new_n39_), .O(z00));
  inv1   g06(.a(new_n38_), .O(new_n41_));
  nor2   g07(.a(x15), .b(x05), .O(new_n42_));
  nand2  g08(.a(new_n42_), .b(x18), .O(new_n43_));
  nand2  g09(.a(x05), .b(x04), .O(new_n44_));
  nand3  g10(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(new_n45_));
  inv1   g11(.a(new_n45_), .O(z01));
  inv1   g12(.a(x04), .O(new_n47_));
  inv1   g13(.a(x03), .O(new_n48_));
  inv1   g14(.a(x16), .O(new_n49_));
  aoi21  g15(.a(new_n48_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g16(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(new_n52_));
  aoi21  g18(.a(new_n52_), .b(new_n37_), .c(x06), .O(z02));
  nand2  g19(.a(new_n37_), .b(x06), .O(new_n54_));
  aoi22  g20(.a(new_n54_), .b(new_n41_), .c(new_n52_), .d(new_n37_), .O(z03));
  nand2  g21(.a(x08), .b(x07), .O(new_n56_));
  inv1   g22(.a(x08), .O(new_n57_));
  nand2  g23(.a(x07), .b(x06), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n56_), .c(new_n51_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand2  g27(.a(new_n56_), .b(x09), .O(new_n62_));
  inv1   g28(.a(x09), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(x08), .c(x07), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(new_n62_), .c(new_n51_), .d(new_n41_), .O(z05));
  inv1   g31(.a(x10), .O(new_n66_));
  nand2  g32(.a(new_n64_), .b(new_n66_), .O(new_n67_));
  nor2   g33(.a(x09), .b(new_n57_), .O(new_n68_));
  nand2  g34(.a(new_n68_), .b(x10), .O(new_n69_));
  nand3  g35(.a(new_n69_), .b(new_n67_), .c(x06), .O(new_n70_));
  nand2  g36(.a(new_n52_), .b(new_n41_), .O(new_n71_));
  nand2  g37(.a(x10), .b(new_n37_), .O(new_n72_));
  nand3  g38(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(z06));
  inv1   g39(.a(x11), .O(new_n74_));
  aoi21  g40(.a(new_n68_), .b(new_n66_), .c(new_n74_), .O(new_n75_));
  nor2   g41(.a(x11), .b(x10), .O(new_n76_));
  nand3  g42(.a(new_n76_), .b(new_n63_), .c(x08), .O(new_n77_));
  nor2   g43(.a(new_n77_), .b(new_n37_), .O(new_n78_));
  oai21  g44(.a(new_n78_), .b(new_n75_), .c(x06), .O(new_n79_));
  nand2  g45(.a(x11), .b(new_n37_), .O(new_n80_));
  nand3  g46(.a(new_n80_), .b(new_n79_), .c(new_n71_), .O(z07));
  nand2  g47(.a(new_n77_), .b(x12), .O(new_n82_));
  inv1   g48(.a(x12), .O(new_n83_));
  nand4  g49(.a(new_n76_), .b(new_n68_), .c(new_n83_), .d(x07), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand2  g52(.a(x12), .b(new_n37_), .O(new_n87_));
  nand3  g53(.a(new_n87_), .b(new_n86_), .c(new_n71_), .O(z08));
  inv1   g54(.a(x13), .O(new_n89_));
  nand2  g55(.a(new_n84_), .b(new_n89_), .O(new_n90_));
  nand4  g56(.a(new_n76_), .b(new_n68_), .c(x13), .d(new_n83_), .O(new_n91_));
  nand3  g57(.a(new_n91_), .b(new_n90_), .c(x06), .O(new_n92_));
  nand2  g58(.a(x13), .b(new_n37_), .O(new_n93_));
  nand3  g59(.a(new_n93_), .b(new_n92_), .c(new_n71_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand3  g61(.a(new_n76_), .b(new_n89_), .c(new_n83_), .O(new_n96_));
  nand2  g62(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nor2   g63(.a(new_n64_), .b(new_n50_), .O(new_n98_));
  inv1   g64(.a(x02), .O(new_n99_));
  oai21  g65(.a(x03), .b(new_n99_), .c(x16), .O(new_n100_));
  nor2   g66(.a(new_n100_), .b(new_n36_), .O(new_n101_));
  aoi21  g67(.a(new_n98_), .b(new_n97_), .c(new_n101_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n47_), .c(new_n41_), .O(z10));
  nand2  g69(.a(new_n41_), .b(new_n99_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n99_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n47_), .c(new_n41_), .O(z12));
  nor2   g72(.a(new_n38_), .b(new_n47_), .O(z13));
  and2   g73(.a(z13), .b(x17), .O(z14));
endmodule


