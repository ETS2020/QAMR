// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x06), .O(new_n37_));
  nor2   g03(.a(x17), .b(new_n37_), .O(new_n38_));
  aoi21  g04(.a(x14), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g05(.a(new_n36_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  inv1   g06(.a(new_n40_), .O(z00));
  inv1   g07(.a(x18), .O(new_n42_));
  nor2   g08(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  aoi21  g10(.a(x05), .b(new_n44_), .c(new_n38_), .O(new_n45_));
  oai21  g11(.a(new_n43_), .b(x05), .c(new_n45_), .O(z01));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  nand2  g16(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n38_), .O(new_n53_));
  inv1   g19(.a(x07), .O(new_n54_));
  nor2   g20(.a(new_n54_), .b(new_n37_), .O(new_n55_));
  inv1   g21(.a(new_n55_), .O(new_n56_));
  nand2  g22(.a(new_n54_), .b(new_n37_), .O(new_n57_));
  nand3  g23(.a(new_n57_), .b(new_n56_), .c(new_n50_), .O(new_n58_));
  nand2  g24(.a(new_n58_), .b(new_n53_), .O(z03));
  inv1   g25(.a(new_n50_), .O(new_n60_));
  nand2  g26(.a(new_n55_), .b(x08), .O(new_n61_));
  inv1   g27(.a(x08), .O(new_n62_));
  nand2  g28(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nand3  g29(.a(new_n63_), .b(new_n61_), .c(new_n53_), .O(new_n64_));
  nor2   g30(.a(new_n64_), .b(new_n60_), .O(z04));
  oai21  g31(.a(x03), .b(new_n36_), .c(x16), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(x04), .c(new_n38_), .O(new_n67_));
  inv1   g33(.a(new_n67_), .O(new_n68_));
  inv1   g34(.a(x17), .O(new_n69_));
  inv1   g35(.a(x09), .O(new_n70_));
  nand4  g36(.a(new_n70_), .b(x08), .c(x07), .d(x06), .O(new_n71_));
  nor2   g37(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  inv1   g38(.a(new_n72_), .O(new_n73_));
  aoi21  g39(.a(x08), .b(x07), .c(new_n69_), .O(new_n74_));
  oai21  g40(.a(new_n74_), .b(new_n37_), .c(x09), .O(new_n75_));
  nand3  g41(.a(new_n75_), .b(new_n73_), .c(new_n68_), .O(z05));
  nand3  g42(.a(new_n70_), .b(x08), .c(x07), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(x17), .O(new_n78_));
  nand2  g44(.a(new_n78_), .b(x06), .O(new_n79_));
  nand2  g45(.a(new_n79_), .b(x10), .O(new_n80_));
  inv1   g46(.a(x10), .O(new_n81_));
  inv1   g47(.a(new_n61_), .O(new_n82_));
  nand4  g48(.a(new_n82_), .b(x17), .c(new_n81_), .d(new_n70_), .O(new_n83_));
  nand3  g49(.a(new_n83_), .b(new_n80_), .c(new_n68_), .O(z06));
  oai21  g50(.a(new_n77_), .b(x10), .c(x17), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(x11), .O(new_n87_));
  nor2   g53(.a(x11), .b(x10), .O(new_n88_));
  aoi21  g54(.a(new_n88_), .b(new_n72_), .c(new_n67_), .O(new_n89_));
  nand2  g55(.a(new_n89_), .b(new_n87_), .O(z07));
  inv1   g56(.a(new_n88_), .O(new_n91_));
  oai21  g57(.a(new_n91_), .b(new_n77_), .c(x17), .O(new_n92_));
  nand2  g58(.a(new_n92_), .b(x06), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(x12), .O(new_n94_));
  inv1   g60(.a(x12), .O(new_n95_));
  nand2  g61(.a(new_n88_), .b(new_n95_), .O(new_n96_));
  inv1   g62(.a(new_n96_), .O(new_n97_));
  aoi21  g63(.a(new_n97_), .b(new_n72_), .c(new_n67_), .O(new_n98_));
  nand2  g64(.a(new_n98_), .b(new_n94_), .O(z08));
  oai21  g65(.a(new_n96_), .b(new_n77_), .c(x17), .O(new_n100_));
  nand2  g66(.a(new_n100_), .b(x06), .O(new_n101_));
  nand2  g67(.a(new_n101_), .b(x13), .O(new_n102_));
  inv1   g68(.a(x13), .O(new_n103_));
  nand3  g69(.a(new_n88_), .b(new_n103_), .c(new_n95_), .O(new_n104_));
  inv1   g70(.a(new_n104_), .O(new_n105_));
  aoi21  g71(.a(new_n105_), .b(new_n72_), .c(new_n67_), .O(new_n106_));
  nand2  g72(.a(new_n106_), .b(new_n102_), .O(z09));
  inv1   g73(.a(x00), .O(new_n108_));
  nand2  g74(.a(new_n104_), .b(new_n108_), .O(new_n109_));
  nor2   g75(.a(new_n71_), .b(new_n49_), .O(new_n110_));
  inv1   g76(.a(x14), .O(new_n111_));
  nor2   g77(.a(new_n66_), .b(new_n111_), .O(new_n112_));
  aoi21  g78(.a(new_n110_), .b(new_n109_), .c(new_n112_), .O(new_n113_));
  oai21  g79(.a(new_n113_), .b(new_n44_), .c(new_n53_), .O(z10));
  nor2   g80(.a(new_n38_), .b(new_n36_), .O(z11));
  aoi21  g81(.a(x16), .b(new_n36_), .c(x03), .O(new_n116_));
  nor3   g82(.a(new_n116_), .b(new_n38_), .c(new_n44_), .O(z12));
  nor2   g83(.a(new_n38_), .b(new_n44_), .O(z13));
  nor2   g84(.a(new_n69_), .b(new_n44_), .O(z14));
endmodule


