// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n105_, new_n107_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  inv1   g02(.a(x05), .O(new_n37_));
  inv1   g03(.a(x08), .O(new_n38_));
  nor2   g04(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g05(.a(x14), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  oai21  g06(.a(new_n36_), .b(new_n35_), .c(new_n40_), .O(new_n41_));
  inv1   g07(.a(new_n41_), .O(z00));
  nor2   g08(.a(x15), .b(x05), .O(new_n43_));
  inv1   g09(.a(x04), .O(new_n44_));
  nor2   g10(.a(new_n37_), .b(new_n44_), .O(new_n45_));
  aoi22  g11(.a(new_n45_), .b(new_n38_), .c(new_n43_), .d(x18), .O(z01));
  inv1   g12(.a(x06), .O(new_n47_));
  nor2   g13(.a(new_n39_), .b(new_n44_), .O(z13));
  oai21  g14(.a(x03), .b(new_n36_), .c(x16), .O(new_n49_));
  nand3  g15(.a(new_n49_), .b(z13), .c(new_n47_), .O(new_n50_));
  inv1   g16(.a(new_n50_), .O(z02));
  inv1   g17(.a(new_n39_), .O(new_n52_));
  nand2  g18(.a(new_n49_), .b(x04), .O(new_n53_));
  xnor2a g19(.a(x07), .b(x06), .O(new_n54_));
  oai21  g20(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z03));
  inv1   g21(.a(new_n53_), .O(new_n56_));
  nand3  g22(.a(x08), .b(x07), .c(x06), .O(new_n57_));
  nand2  g23(.a(x07), .b(x06), .O(new_n58_));
  oai21  g24(.a(new_n38_), .b(x05), .c(new_n58_), .O(new_n59_));
  nand3  g25(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(new_n60_));
  inv1   g26(.a(new_n60_), .O(z04));
  nand2  g27(.a(new_n57_), .b(x09), .O(new_n62_));
  oai21  g28(.a(x09), .b(x08), .c(x05), .O(new_n63_));
  inv1   g29(.a(x09), .O(new_n64_));
  nand4  g30(.a(new_n64_), .b(x08), .c(x07), .d(x06), .O(new_n65_));
  nand4  g31(.a(new_n65_), .b(new_n63_), .c(new_n62_), .d(new_n56_), .O(z05));
  oai22  g32(.a(new_n65_), .b(x05), .c(new_n39_), .d(x10), .O(new_n67_));
  inv1   g33(.a(new_n57_), .O(new_n68_));
  nor2   g34(.a(x10), .b(x09), .O(new_n69_));
  nand2  g35(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n67_), .c(new_n56_), .O(z06));
  inv1   g37(.a(x11), .O(new_n72_));
  nand3  g38(.a(new_n70_), .b(new_n52_), .c(new_n72_), .O(new_n73_));
  inv1   g39(.a(x10), .O(new_n74_));
  inv1   g40(.a(new_n65_), .O(new_n75_));
  nand4  g41(.a(new_n75_), .b(x11), .c(new_n74_), .d(new_n37_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nand2  g43(.a(new_n77_), .b(new_n56_), .O(z07));
  inv1   g44(.a(x12), .O(new_n79_));
  nor2   g45(.a(new_n58_), .b(x11), .O(new_n80_));
  aoi21  g46(.a(new_n80_), .b(new_n69_), .c(new_n79_), .O(new_n81_));
  nor2   g47(.a(x12), .b(x11), .O(new_n82_));
  nand2  g48(.a(new_n82_), .b(new_n69_), .O(new_n83_));
  nor2   g49(.a(new_n83_), .b(new_n57_), .O(new_n84_));
  oai21  g50(.a(new_n84_), .b(new_n81_), .c(new_n37_), .O(new_n85_));
  aoi22  g51(.a(new_n53_), .b(new_n52_), .c(x12), .d(new_n38_), .O(new_n86_));
  nand2  g52(.a(new_n86_), .b(new_n85_), .O(z08));
  nand4  g53(.a(new_n82_), .b(new_n69_), .c(x07), .d(x06), .O(new_n88_));
  inv1   g54(.a(x13), .O(new_n89_));
  nand3  g55(.a(new_n82_), .b(new_n89_), .c(new_n74_), .O(new_n90_));
  inv1   g56(.a(new_n90_), .O(new_n91_));
  aoi22  g57(.a(new_n91_), .b(new_n75_), .c(new_n88_), .d(x13), .O(new_n92_));
  aoi22  g58(.a(new_n53_), .b(new_n52_), .c(x13), .d(new_n38_), .O(new_n93_));
  oai21  g59(.a(new_n92_), .b(x05), .c(new_n93_), .O(z09));
  inv1   g60(.a(x00), .O(new_n95_));
  nand2  g61(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  inv1   g62(.a(x03), .O(new_n97_));
  nand2  g63(.a(new_n97_), .b(x02), .O(new_n98_));
  aoi21  g64(.a(new_n98_), .b(x16), .c(new_n65_), .O(new_n99_));
  inv1   g65(.a(x14), .O(new_n100_));
  nor2   g66(.a(new_n49_), .b(new_n100_), .O(new_n101_));
  aoi21  g67(.a(new_n99_), .b(new_n96_), .c(new_n101_), .O(new_n102_));
  oai21  g68(.a(new_n102_), .b(new_n44_), .c(new_n52_), .O(z10));
  nor2   g69(.a(new_n39_), .b(new_n36_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n36_), .c(x03), .O(new_n105_));
  oai21  g71(.a(new_n105_), .b(new_n44_), .c(new_n52_), .O(z12));
  nand2  g72(.a(x17), .b(x04), .O(new_n107_));
  nand2  g73(.a(new_n107_), .b(new_n52_), .O(z14));
endmodule


