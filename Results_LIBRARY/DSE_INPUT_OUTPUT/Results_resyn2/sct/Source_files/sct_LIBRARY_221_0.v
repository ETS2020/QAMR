// Benchmark "FAU" written by ABC on Tue Aug 11 19:46:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14;
  wire new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n105_, new_n106_, new_n109_;
  inv1   g00(.a(x01), .O(new_n35_));
  inv1   g01(.a(x02), .O(new_n36_));
  nor2   g02(.a(x09), .b(x06), .O(new_n37_));
  aoi21  g03(.a(x14), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  oai21  g04(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  inv1   g05(.a(new_n39_), .O(z00));
  inv1   g06(.a(x18), .O(new_n41_));
  nor2   g07(.a(new_n41_), .b(x15), .O(new_n42_));
  inv1   g08(.a(x04), .O(new_n43_));
  aoi21  g09(.a(x05), .b(new_n43_), .c(new_n37_), .O(new_n44_));
  oai21  g10(.a(new_n42_), .b(x05), .c(new_n44_), .O(z01));
  inv1   g11(.a(x06), .O(new_n46_));
  inv1   g12(.a(x03), .O(new_n47_));
  inv1   g13(.a(x16), .O(new_n48_));
  aoi21  g14(.a(new_n47_), .b(x02), .c(new_n48_), .O(new_n49_));
  nor2   g15(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  nand3  g16(.a(new_n50_), .b(x09), .c(new_n46_), .O(new_n51_));
  inv1   g17(.a(new_n51_), .O(z02));
  inv1   g18(.a(new_n37_), .O(new_n53_));
  oai21  g19(.a(x03), .b(new_n36_), .c(x16), .O(new_n54_));
  nand2  g20(.a(new_n54_), .b(x04), .O(new_n55_));
  xnor2a g21(.a(x07), .b(x06), .O(new_n56_));
  oai21  g22(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(z03));
  nand2  g23(.a(x07), .b(x06), .O(new_n58_));
  xor2a  g24(.a(new_n58_), .b(x08), .O(new_n59_));
  oai21  g25(.a(new_n59_), .b(new_n55_), .c(new_n53_), .O(z04));
  nand2  g26(.a(x08), .b(x07), .O(new_n61_));
  nor2   g27(.a(new_n61_), .b(x09), .O(new_n62_));
  inv1   g28(.a(new_n62_), .O(new_n63_));
  aoi21  g29(.a(new_n61_), .b(x09), .c(new_n46_), .O(new_n64_));
  nand3  g30(.a(new_n64_), .b(new_n63_), .c(new_n50_), .O(z05));
  and2   g31(.a(x08), .b(x07), .O(new_n66_));
  aoi21  g32(.a(new_n66_), .b(x10), .c(new_n46_), .O(new_n67_));
  oai21  g33(.a(new_n62_), .b(x10), .c(new_n67_), .O(new_n68_));
  nand2  g34(.a(new_n55_), .b(new_n53_), .O(new_n69_));
  nand2  g35(.a(x10), .b(x09), .O(new_n70_));
  nand3  g36(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(z06));
  oai21  g37(.a(new_n61_), .b(x10), .c(x11), .O(new_n72_));
  inv1   g38(.a(x10), .O(new_n73_));
  inv1   g39(.a(x11), .O(new_n74_));
  nand4  g40(.a(new_n74_), .b(new_n73_), .c(x08), .d(x07), .O(new_n75_));
  oai21  g41(.a(new_n75_), .b(x09), .c(new_n72_), .O(new_n76_));
  nand2  g42(.a(new_n76_), .b(x06), .O(new_n77_));
  nand2  g43(.a(x11), .b(x09), .O(new_n78_));
  nand3  g44(.a(new_n78_), .b(new_n77_), .c(new_n69_), .O(z07));
  nand2  g45(.a(new_n75_), .b(x12), .O(new_n80_));
  inv1   g46(.a(x09), .O(new_n81_));
  inv1   g47(.a(x12), .O(new_n82_));
  nor2   g48(.a(x11), .b(x10), .O(new_n83_));
  nand4  g49(.a(new_n83_), .b(new_n66_), .c(new_n82_), .d(new_n81_), .O(new_n84_));
  nand2  g50(.a(new_n84_), .b(new_n80_), .O(new_n85_));
  nand2  g51(.a(new_n85_), .b(x06), .O(new_n86_));
  nor2   g52(.a(new_n82_), .b(new_n81_), .O(new_n87_));
  aoi21  g53(.a(new_n55_), .b(new_n53_), .c(new_n87_), .O(new_n88_));
  nand2  g54(.a(new_n88_), .b(new_n86_), .O(z08));
  nand2  g55(.a(new_n84_), .b(x13), .O(new_n90_));
  inv1   g56(.a(x13), .O(new_n91_));
  nand3  g57(.a(new_n83_), .b(new_n91_), .c(new_n82_), .O(new_n92_));
  inv1   g58(.a(new_n92_), .O(new_n93_));
  nand2  g59(.a(new_n93_), .b(new_n62_), .O(new_n94_));
  oai21  g60(.a(x13), .b(new_n81_), .c(new_n46_), .O(new_n95_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(new_n90_), .d(new_n50_), .O(z09));
  inv1   g62(.a(x00), .O(new_n97_));
  nand2  g63(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nand2  g64(.a(new_n47_), .b(x02), .O(new_n99_));
  aoi21  g65(.a(new_n99_), .b(x16), .c(new_n46_), .O(new_n100_));
  nand3  g66(.a(new_n100_), .b(new_n98_), .c(new_n62_), .O(new_n101_));
  nand3  g67(.a(new_n49_), .b(new_n53_), .c(x14), .O(new_n102_));
  aoi21  g68(.a(new_n102_), .b(new_n101_), .c(new_n43_), .O(z10));
  nor2   g69(.a(new_n37_), .b(new_n36_), .O(z11));
  aoi21  g70(.a(x16), .b(new_n36_), .c(x03), .O(new_n105_));
  nand2  g71(.a(new_n53_), .b(x04), .O(new_n106_));
  nor2   g72(.a(new_n106_), .b(new_n105_), .O(z12));
  nand2  g73(.a(new_n53_), .b(new_n43_), .O(z13));
  inv1   g74(.a(x17), .O(new_n109_));
  nor2   g75(.a(new_n106_), .b(new_n109_), .O(z14));
endmodule


