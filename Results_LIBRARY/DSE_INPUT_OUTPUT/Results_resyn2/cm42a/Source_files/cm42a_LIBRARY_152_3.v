// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:42 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n25_;
  nor2   g00(.a(x2), .b(x1), .O(new_n15_));
  nor2   g01(.a(x3), .b(x0), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  inv1   g03(.a(x0), .O(new_n18_));
  inv1   g04(.a(x1), .O(new_n19_));
  oai21  g05(.a(x3), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  xnor2a g06(.a(x2), .b(x1), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n20_), .O(z1));
  inv1   g08(.a(x2), .O(new_n23_));
  nand3  g09(.a(new_n16_), .b(new_n23_), .c(x1), .O(z2));
  inv1   g10(.a(x3), .O(new_n25_));
  nand4  g11(.a(new_n25_), .b(new_n23_), .c(x1), .d(x0), .O(z3));
  oai21  g12(.a(new_n16_), .b(x1), .c(x2), .O(z4));
  nand2  g13(.a(new_n20_), .b(x2), .O(z5));
  nand2  g14(.a(x2), .b(x1), .O(z7));
  nand3  g15(.a(new_n15_), .b(x3), .c(new_n18_), .O(z8));
  nand3  g16(.a(new_n15_), .b(x3), .c(x0), .O(z9));
  one    g17(.O(z6));
endmodule


