// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:12 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n19_, new_n20_, new_n22_, new_n24_, new_n25_,
    new_n28_, new_n29_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x2), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  inv1   g03(.a(x0), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand4  g05(.a(new_n20_), .b(new_n16_), .c(x1), .d(new_n19_), .O(z2));
  nor2   g06(.a(x3), .b(new_n19_), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n15_), .c(new_n16_), .O(z3));
  nand3  g08(.a(new_n20_), .b(x2), .c(new_n19_), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n15_), .O(z4));
  nand3  g11(.a(new_n22_), .b(x2), .c(new_n15_), .O(z5));
  nand2  g12(.a(x2), .b(new_n15_), .O(new_n28_));
  nand2  g13(.a(new_n24_), .b(x1), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n28_), .O(z6));
  nand3  g15(.a(new_n22_), .b(x2), .c(x1), .O(z7));
  one    g16(.O(z1));
  one    g17(.O(z8));
  one    g18(.O(z9));
endmodule


