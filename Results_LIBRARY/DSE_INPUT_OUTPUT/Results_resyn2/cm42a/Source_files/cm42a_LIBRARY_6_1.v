// Benchmark "FAU" written by ABC on Sat Aug  8 19:40:53 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n23_, new_n25_, new_n27_,
    new_n30_;
  nor2   g00(.a(x3), .b(x2), .O(new_n15_));
  nor2   g01(.a(x1), .b(x0), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  oai21  g03(.a(x3), .b(x2), .c(x0), .O(new_n18_));
  xor2a  g04(.a(x1), .b(x0), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n18_), .O(z1));
  nand2  g06(.a(new_n18_), .b(x1), .O(z3));
  inv1   g07(.a(x3), .O(new_n23_));
  nand3  g08(.a(new_n16_), .b(new_n23_), .c(x2), .O(z4));
  inv1   g09(.a(x1), .O(new_n25_));
  nand4  g10(.a(new_n23_), .b(x2), .c(new_n25_), .d(x0), .O(z5));
  inv1   g11(.a(x0), .O(new_n27_));
  nand2  g12(.a(x1), .b(new_n27_), .O(z6));
  nand4  g13(.a(new_n23_), .b(x2), .c(x1), .d(x0), .O(z7));
  oai21  g14(.a(new_n23_), .b(x2), .c(new_n25_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n27_), .O(z8));
  nand2  g16(.a(new_n30_), .b(new_n19_), .O(z9));
  one    g17(.O(z2));
endmodule


