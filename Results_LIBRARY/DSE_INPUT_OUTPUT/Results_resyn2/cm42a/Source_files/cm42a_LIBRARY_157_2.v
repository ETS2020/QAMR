// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:43 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n19_, new_n22_, new_n25_, new_n26_, new_n27_,
    new_n29_, new_n31_;
  inv1   g00(.a(x1), .O(new_n15_));
  nor2   g01(.a(x3), .b(x2), .O(new_n16_));
  oai21  g02(.a(new_n16_), .b(x0), .c(new_n15_), .O(z0));
  inv1   g03(.a(x0), .O(new_n19_));
  nand3  g04(.a(new_n16_), .b(x1), .c(new_n19_), .O(z2));
  nand3  g05(.a(new_n16_), .b(x1), .c(x0), .O(z3));
  inv1   g06(.a(x3), .O(new_n22_));
  nand4  g07(.a(new_n22_), .b(x2), .c(new_n15_), .d(new_n19_), .O(z4));
  nand2  g08(.a(x1), .b(x0), .O(new_n25_));
  nand3  g09(.a(new_n22_), .b(x2), .c(x1), .O(new_n26_));
  nand2  g10(.a(new_n26_), .b(new_n19_), .O(new_n27_));
  nand2  g11(.a(new_n27_), .b(new_n25_), .O(z6));
  inv1   g12(.a(new_n26_), .O(new_n29_));
  nand2  g13(.a(new_n29_), .b(x0), .O(z7));
  inv1   g14(.a(x2), .O(new_n31_));
  nand4  g15(.a(x3), .b(new_n31_), .c(new_n15_), .d(new_n19_), .O(z8));
  one    g16(.O(z1));
  one    g17(.O(z5));
  one    g18(.O(z9));
endmodule


