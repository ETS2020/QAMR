// Benchmark "FAU" written by ABC on Sat Aug  8 19:40:59 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n20_, new_n23_, new_n25_,
    new_n26_;
  inv1   g00(.a(x2), .O(new_n15_));
  nor2   g01(.a(x3), .b(x0), .O(new_n16_));
  oai21  g02(.a(new_n16_), .b(x1), .c(new_n15_), .O(z0));
  inv1   g03(.a(x3), .O(new_n18_));
  inv1   g04(.a(x0), .O(new_n19_));
  nor2   g05(.a(x1), .b(new_n19_), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n18_), .c(new_n15_), .O(z1));
  nand2  g07(.a(new_n15_), .b(x1), .O(z2));
  inv1   g08(.a(x1), .O(new_n23_));
  nand4  g09(.a(new_n18_), .b(x2), .c(new_n23_), .d(new_n19_), .O(z4));
  oai21  g10(.a(x3), .b(new_n15_), .c(new_n23_), .O(new_n25_));
  oai21  g11(.a(x1), .b(new_n19_), .c(x2), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n25_), .O(z5));
  nand3  g13(.a(new_n16_), .b(x2), .c(x1), .O(z6));
  nand4  g14(.a(new_n18_), .b(x2), .c(x1), .d(x0), .O(z7));
  nand4  g15(.a(x3), .b(new_n15_), .c(new_n23_), .d(new_n19_), .O(z8));
  nand3  g16(.a(new_n20_), .b(x3), .c(new_n15_), .O(z9));
  nand2  g17(.a(new_n15_), .b(x1), .O(z3));
endmodule


