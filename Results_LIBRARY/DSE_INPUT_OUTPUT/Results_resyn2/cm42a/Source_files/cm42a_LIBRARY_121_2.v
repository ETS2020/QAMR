// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:31 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n20_, new_n22_, new_n23_, new_n25_,
    new_n29_, new_n30_, new_n32_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  nor2   g02(.a(x2), .b(x1), .O(new_n17_));
  nand3  g03(.a(new_n17_), .b(new_n16_), .c(new_n15_), .O(z0));
  nand3  g04(.a(new_n17_), .b(new_n16_), .c(x0), .O(z1));
  inv1   g05(.a(x2), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(x1), .O(z2));
  oai21  g07(.a(x3), .b(x0), .c(x2), .O(new_n22_));
  xor2a  g08(.a(x2), .b(x1), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n22_), .O(z4));
  oai21  g10(.a(x3), .b(new_n15_), .c(x2), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n23_), .O(z5));
  nand2  g12(.a(new_n22_), .b(x1), .O(z6));
  nand2  g13(.a(new_n25_), .b(x1), .O(z7));
  inv1   g14(.a(x1), .O(new_n29_));
  oai21  g15(.a(new_n16_), .b(x0), .c(new_n29_), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(new_n20_), .O(z8));
  oai21  g17(.a(new_n16_), .b(new_n15_), .c(new_n29_), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(new_n20_), .O(z9));
  nand2  g19(.a(new_n20_), .b(x1), .O(z3));
endmodule


