// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:53 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n29_, new_n32_;
  nor2   g00(.a(x3), .b(x1), .O(new_n15_));
  nor2   g01(.a(x2), .b(x0), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  inv1   g03(.a(x0), .O(new_n18_));
  nor2   g04(.a(x2), .b(new_n18_), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n15_), .O(z1));
  inv1   g06(.a(x3), .O(new_n23_));
  inv1   g07(.a(x1), .O(new_n24_));
  inv1   g08(.a(x2), .O(new_n25_));
  oai21  g09(.a(new_n25_), .b(x0), .c(new_n24_), .O(new_n26_));
  nand2  g10(.a(new_n26_), .b(new_n23_), .O(z4));
  nand3  g11(.a(new_n15_), .b(x2), .c(x0), .O(z5));
  nor2   g12(.a(x3), .b(new_n24_), .O(new_n29_));
  inv1   g13(.a(new_n29_), .O(z6));
  nor2   g14(.a(new_n23_), .b(x1), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(new_n16_), .c(new_n29_), .O(z8));
  aoi21  g16(.a(new_n32_), .b(new_n19_), .c(new_n29_), .O(z9));
  one    g17(.O(z2));
  one    g18(.O(z3));
  one    g19(.O(z7));
endmodule


