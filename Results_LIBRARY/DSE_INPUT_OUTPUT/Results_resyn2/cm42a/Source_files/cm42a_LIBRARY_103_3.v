// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:26 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n26_, new_n27_, new_n28_, new_n31_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  nor2   g02(.a(x2), .b(x0), .O(new_n17_));
  nand3  g03(.a(new_n17_), .b(new_n16_), .c(new_n15_), .O(z0));
  inv1   g04(.a(x0), .O(new_n19_));
  oai21  g05(.a(x1), .b(new_n19_), .c(new_n16_), .O(new_n20_));
  oai21  g06(.a(x3), .b(x2), .c(new_n15_), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n20_), .O(z1));
  nor2   g08(.a(x3), .b(x2), .O(new_n23_));
  nand3  g09(.a(new_n23_), .b(x1), .c(new_n19_), .O(z2));
  nand3  g10(.a(new_n23_), .b(x1), .c(x0), .O(z3));
  inv1   g11(.a(x2), .O(new_n26_));
  oai21  g12(.a(x3), .b(new_n26_), .c(new_n15_), .O(new_n27_));
  oai21  g13(.a(x1), .b(x0), .c(new_n16_), .O(new_n28_));
  nand2  g14(.a(new_n28_), .b(new_n27_), .O(z4));
  nand2  g15(.a(new_n27_), .b(new_n20_), .O(z5));
  nor2   g16(.a(x3), .b(new_n26_), .O(new_n31_));
  nand3  g17(.a(new_n31_), .b(x1), .c(new_n19_), .O(z6));
  nand3  g18(.a(new_n31_), .b(x1), .c(x0), .O(z7));
  oai21  g19(.a(new_n17_), .b(x1), .c(x3), .O(z8));
  nand4  g20(.a(x3), .b(new_n26_), .c(new_n15_), .d(x0), .O(z9));
endmodule


