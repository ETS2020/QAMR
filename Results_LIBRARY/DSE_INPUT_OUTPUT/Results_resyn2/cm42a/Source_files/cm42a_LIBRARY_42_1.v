// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:05 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n20_, new_n21_, new_n22_, new_n25_,
    new_n27_, new_n28_, new_n31_;
  nor2   g00(.a(x1), .b(x0), .O(new_n15_));
  nor2   g01(.a(x3), .b(x2), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  inv1   g03(.a(x1), .O(new_n18_));
  nand3  g04(.a(new_n16_), .b(new_n18_), .c(x0), .O(z1));
  inv1   g05(.a(x2), .O(new_n20_));
  inv1   g06(.a(x3), .O(new_n21_));
  oai21  g07(.a(new_n18_), .b(x0), .c(new_n21_), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n20_), .O(z2));
  nand3  g09(.a(new_n16_), .b(x1), .c(x0), .O(z3));
  xor2a  g10(.a(x3), .b(x2), .O(new_n25_));
  oai21  g11(.a(new_n15_), .b(x3), .c(new_n25_), .O(z4));
  inv1   g12(.a(x0), .O(new_n27_));
  oai21  g13(.a(x1), .b(new_n27_), .c(new_n21_), .O(new_n28_));
  nand2  g14(.a(new_n28_), .b(new_n25_), .O(z5));
  nand2  g15(.a(new_n25_), .b(new_n22_), .O(z6));
  oai21  g16(.a(new_n18_), .b(new_n27_), .c(new_n21_), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n25_), .O(z7));
  nand2  g18(.a(x3), .b(new_n20_), .O(z8));
  nand2  g19(.a(x3), .b(new_n20_), .O(z9));
endmodule


