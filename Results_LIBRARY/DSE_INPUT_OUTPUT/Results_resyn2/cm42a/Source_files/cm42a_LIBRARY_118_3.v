// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:31 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n19_, new_n20_, new_n22_, new_n24_,
    new_n26_, new_n27_, new_n28_, new_n30_, new_n32_;
  inv1   g00(.a(x2), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  nor2   g02(.a(x1), .b(x0), .O(new_n17_));
  nand3  g03(.a(new_n17_), .b(new_n16_), .c(new_n15_), .O(z0));
  inv1   g04(.a(x0), .O(new_n19_));
  nor2   g05(.a(x1), .b(new_n19_), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n16_), .c(new_n15_), .O(z1));
  nor2   g07(.a(x2), .b(x0), .O(new_n22_));
  oai21  g08(.a(new_n22_), .b(x3), .c(x1), .O(z2));
  oai21  g09(.a(x2), .b(new_n19_), .c(new_n16_), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(x1), .O(z3));
  inv1   g11(.a(x1), .O(new_n26_));
  oai21  g12(.a(x3), .b(new_n15_), .c(new_n26_), .O(new_n27_));
  oai21  g13(.a(x1), .b(x0), .c(new_n16_), .O(new_n28_));
  nand2  g14(.a(new_n28_), .b(new_n27_), .O(z4));
  oai21  g15(.a(x1), .b(new_n19_), .c(new_n16_), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(new_n27_), .O(z5));
  oai21  g17(.a(new_n15_), .b(x0), .c(new_n16_), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(x1), .O(z6));
  nand4  g19(.a(new_n16_), .b(x2), .c(x1), .d(x0), .O(z7));
  nand3  g20(.a(new_n17_), .b(x3), .c(new_n15_), .O(z8));
  nand3  g21(.a(new_n20_), .b(x3), .c(new_n15_), .O(z9));
endmodule


