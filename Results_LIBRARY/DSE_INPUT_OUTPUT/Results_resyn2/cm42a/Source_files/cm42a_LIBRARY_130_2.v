// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:34 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n19_, new_n20_, new_n24_, new_n26_,
    new_n28_, new_n31_, new_n32_, new_n34_;
  inv1   g00(.a(x2), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  oai21  g02(.a(x3), .b(x0), .c(new_n16_), .O(new_n17_));
  nand2  g03(.a(new_n17_), .b(new_n15_), .O(z0));
  inv1   g04(.a(x0), .O(new_n19_));
  nor2   g05(.a(x3), .b(new_n19_), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n15_), .c(new_n16_), .O(z1));
  nand2  g07(.a(new_n15_), .b(x1), .O(z3));
  xor2a  g08(.a(x2), .b(x1), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n17_), .O(z4));
  oai21  g10(.a(x3), .b(new_n19_), .c(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n24_), .O(z5));
  oai21  g12(.a(x3), .b(x0), .c(x2), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x1), .O(z6));
  nand3  g14(.a(new_n20_), .b(x2), .c(x1), .O(z7));
  inv1   g15(.a(x3), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x0), .c(new_n16_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n15_), .O(z8));
  oai21  g18(.a(new_n31_), .b(new_n19_), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n15_), .O(z9));
  one    g20(.O(z2));
endmodule


