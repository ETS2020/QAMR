// Benchmark "FAU" written by ABC on Sat Aug  8 19:40:58 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n20_, new_n21_, new_n22_, new_n25_,
    new_n27_, new_n29_;
  inv1   g00(.a(x1), .O(new_n15_));
  oai21  g01(.a(x3), .b(x2), .c(new_n15_), .O(new_n16_));
  xnor2a g02(.a(x1), .b(x0), .O(new_n17_));
  nand2  g03(.a(new_n17_), .b(new_n16_), .O(z0));
  nand2  g04(.a(new_n16_), .b(x0), .O(z1));
  inv1   g05(.a(x0), .O(new_n20_));
  inv1   g06(.a(x2), .O(new_n21_));
  inv1   g07(.a(x3), .O(new_n22_));
  nand4  g08(.a(new_n22_), .b(new_n21_), .c(x1), .d(new_n20_), .O(z2));
  nor2   g09(.a(x1), .b(x0), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(new_n22_), .c(x2), .O(z4));
  oai21  g11(.a(x3), .b(new_n21_), .c(new_n15_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x0), .O(z5));
  oai21  g13(.a(x3), .b(new_n21_), .c(new_n20_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x1), .O(z6));
  nand2  g15(.a(x1), .b(x0), .O(z7));
  nand3  g16(.a(new_n25_), .b(x3), .c(new_n21_), .O(z8));
  nand4  g17(.a(x3), .b(new_n21_), .c(new_n15_), .d(x0), .O(z9));
  one    g18(.O(z3));
endmodule


