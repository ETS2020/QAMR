// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:53 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n17_, new_n18_, new_n20_, new_n22_, new_n24_, new_n25_,
    new_n27_;
  nor2   g00(.a(x1), .b(x0), .O(new_n15_));
  inv1   g01(.a(new_n15_), .O(z0));
  inv1   g02(.a(x1), .O(new_n17_));
  nor2   g03(.a(x3), .b(x2), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(x0), .O(z1));
  inv1   g05(.a(x0), .O(new_n20_));
  nand3  g06(.a(new_n18_), .b(x1), .c(new_n20_), .O(z2));
  and2   g07(.a(x1), .b(x0), .O(new_n22_));
  aoi21  g08(.a(new_n22_), .b(new_n18_), .c(new_n15_), .O(z3));
  inv1   g09(.a(x2), .O(new_n24_));
  oai21  g10(.a(x3), .b(new_n24_), .c(x0), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n17_), .O(z5));
  inv1   g12(.a(x3), .O(new_n27_));
  nand4  g13(.a(new_n27_), .b(x2), .c(x1), .d(new_n20_), .O(z6));
  nand4  g14(.a(new_n27_), .b(x2), .c(x1), .d(x0), .O(z7));
  nand4  g15(.a(x3), .b(new_n24_), .c(new_n17_), .d(x0), .O(z9));
  inv1   g16(.a(new_n15_), .O(z4));
  inv1   g17(.a(new_n15_), .O(z8));
endmodule


