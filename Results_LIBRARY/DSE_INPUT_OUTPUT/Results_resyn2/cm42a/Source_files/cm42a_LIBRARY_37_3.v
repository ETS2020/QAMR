// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:04 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n20_, new_n21_, new_n23_, new_n24_,
    new_n26_, new_n27_, new_n30_;
  and2   g00(.a(x3), .b(x0), .O(new_n15_));
  nor2   g01(.a(x3), .b(x0), .O(new_n16_));
  nor2   g02(.a(x2), .b(x1), .O(new_n17_));
  aoi21  g03(.a(new_n17_), .b(new_n16_), .c(new_n15_), .O(z0));
  oai21  g04(.a(new_n17_), .b(x3), .c(x0), .O(z1));
  inv1   g05(.a(x1), .O(new_n20_));
  nor2   g06(.a(x2), .b(new_n20_), .O(new_n21_));
  aoi21  g07(.a(new_n21_), .b(new_n16_), .c(new_n15_), .O(z2));
  inv1   g08(.a(x0), .O(new_n23_));
  nor2   g09(.a(x3), .b(new_n23_), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n21_), .O(z3));
  inv1   g11(.a(x2), .O(new_n26_));
  nor2   g12(.a(new_n26_), .b(x1), .O(new_n27_));
  aoi21  g13(.a(new_n27_), .b(new_n16_), .c(new_n15_), .O(z4));
  nand2  g14(.a(new_n27_), .b(new_n24_), .O(z5));
  nor2   g15(.a(new_n26_), .b(new_n20_), .O(new_n30_));
  aoi21  g16(.a(new_n30_), .b(new_n16_), .c(new_n15_), .O(z6));
  nand2  g17(.a(new_n30_), .b(new_n24_), .O(z7));
  oai21  g18(.a(new_n17_), .b(x0), .c(x3), .O(z8));
  one    g19(.O(z9));
endmodule


