// Benchmark "FAU" written by ABC on Sat Aug  8 19:42:06 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n20_, new_n24_, new_n26_,
    new_n27_;
  inv1   g00(.a(x2), .O(new_n15_));
  nor2   g01(.a(x1), .b(x0), .O(new_n16_));
  oai21  g02(.a(new_n16_), .b(x3), .c(new_n15_), .O(z0));
  inv1   g03(.a(x0), .O(new_n18_));
  nor2   g04(.a(x1), .b(new_n18_), .O(new_n19_));
  nor2   g05(.a(x3), .b(x2), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n19_), .O(z1));
  nand3  g07(.a(new_n20_), .b(x1), .c(new_n18_), .O(z2));
  nand3  g08(.a(new_n20_), .b(x1), .c(x0), .O(z3));
  nor2   g09(.a(x3), .b(new_n15_), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n16_), .O(z4));
  inv1   g11(.a(x3), .O(new_n26_));
  nor2   g12(.a(new_n26_), .b(x2), .O(new_n27_));
  aoi21  g13(.a(new_n24_), .b(new_n19_), .c(new_n27_), .O(z5));
  nand3  g14(.a(new_n24_), .b(x1), .c(new_n18_), .O(z6));
  nand3  g15(.a(new_n24_), .b(x1), .c(x0), .O(z7));
  one    g16(.O(z8));
  one    g17(.O(z9));
endmodule


