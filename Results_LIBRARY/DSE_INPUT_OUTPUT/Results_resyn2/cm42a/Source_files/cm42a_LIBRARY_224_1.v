// Benchmark "FAU" written by ABC on Sat Aug  8 19:42:06 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n26_, new_n28_, new_n31_, new_n32_, new_n34_, new_n35_;
  inv1   g00(.a(x2), .O(new_n15_));
  nor3   g01(.a(x3), .b(x1), .c(x0), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  inv1   g03(.a(x1), .O(new_n18_));
  inv1   g04(.a(x3), .O(new_n19_));
  nand3  g05(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  inv1   g06(.a(new_n20_), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n15_), .O(z1));
  oai21  g08(.a(new_n18_), .b(x0), .c(new_n19_), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n15_), .O(z2));
  nand4  g10(.a(new_n19_), .b(new_n15_), .c(x1), .d(x0), .O(z3));
  nand2  g11(.a(new_n19_), .b(new_n15_), .O(new_n26_));
  oai21  g12(.a(new_n16_), .b(new_n15_), .c(new_n26_), .O(z4));
  nand2  g13(.a(new_n20_), .b(x2), .O(new_n28_));
  nand2  g14(.a(new_n28_), .b(new_n26_), .O(z5));
  nand2  g15(.a(x3), .b(new_n15_), .O(z9));
  nand2  g16(.a(new_n19_), .b(x2), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(z9), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(new_n23_), .O(z6));
  nand2  g19(.a(x1), .b(x0), .O(new_n34_));
  oai21  g20(.a(new_n34_), .b(x3), .c(x2), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n26_), .O(z7));
  one    g22(.O(z8));
endmodule


