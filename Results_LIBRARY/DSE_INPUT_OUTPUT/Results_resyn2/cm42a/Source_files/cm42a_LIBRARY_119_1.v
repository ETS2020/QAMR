// Benchmark "FAU" written by ABC on Sat Aug  8 19:41:31 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n20_, new_n21_, new_n22_, new_n24_,
    new_n28_, new_n30_, new_n31_, new_n33_, new_n35_;
  nor2   g00(.a(x2), .b(x1), .O(new_n15_));
  nor2   g01(.a(x3), .b(x0), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  inv1   g03(.a(x3), .O(new_n18_));
  nand3  g04(.a(new_n15_), .b(new_n18_), .c(x0), .O(z1));
  inv1   g05(.a(new_n15_), .O(new_n20_));
  oai21  g06(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  nand2  g07(.a(x1), .b(x0), .O(new_n22_));
  nand3  g08(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(z2));
  inv1   g09(.a(x2), .O(new_n24_));
  nand4  g10(.a(new_n18_), .b(new_n24_), .c(x1), .d(x0), .O(z3));
  inv1   g11(.a(x1), .O(new_n28_));
  oai21  g12(.a(new_n16_), .b(new_n28_), .c(x2), .O(z6));
  inv1   g13(.a(x0), .O(new_n30_));
  oai21  g14(.a(x3), .b(new_n30_), .c(x1), .O(new_n31_));
  nand2  g15(.a(new_n31_), .b(x2), .O(z7));
  oai21  g16(.a(new_n18_), .b(x0), .c(new_n24_), .O(new_n33_));
  nand2  g17(.a(new_n33_), .b(new_n28_), .O(z8));
  oai21  g18(.a(new_n18_), .b(new_n30_), .c(new_n24_), .O(new_n35_));
  nand2  g19(.a(new_n35_), .b(new_n28_), .O(z9));
  one    g20(.O(z4));
  one    g21(.O(z5));
endmodule


