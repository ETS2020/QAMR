// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(x1), .O(new_n26_));
  aoi21  g11(.a(x3), .b(new_n16_), .c(x2), .O(new_n27_));
  aoi21  g12(.a(new_n26_), .b(new_n24_), .c(new_n27_), .O(new_n28_));
  nand3  g13(.a(x3), .b(new_n16_), .c(x0), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x6), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n25_), .O(new_n31_));
  oai21  g16(.a(new_n28_), .b(x0), .c(new_n31_), .O(z1));
  inv1   g17(.a(x4), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x2), .c(new_n19_), .O(new_n34_));
  aoi22  g19(.a(new_n34_), .b(x1), .c(new_n26_), .d(new_n23_), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x0), .c(new_n31_), .O(z2));
  nor2   g21(.a(new_n16_), .b(x0), .O(new_n37_));
  oai21  g22(.a(x4), .b(x3), .c(x7), .O(new_n38_));
  nand4  g23(.a(new_n38_), .b(new_n37_), .c(x6), .d(new_n25_), .O(new_n39_));
  nand2  g24(.a(x5), .b(new_n18_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n26_), .c(new_n19_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z3));
  inv1   g27(.a(x6), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  inv1   g29(.a(new_n44_), .O(new_n45_));
  oai21  g30(.a(new_n19_), .b(x1), .c(new_n25_), .O(new_n46_));
  nand2  g31(.a(x3), .b(x2), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n18_), .c(new_n45_), .O(new_n49_));
  xor2a  g34(.a(x3), .b(x2), .O(new_n50_));
  nand2  g35(.a(new_n25_), .b(new_n18_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n49_), .O(z4));
  nand3  g38(.a(x5), .b(x2), .c(new_n18_), .O(new_n54_));
  nand3  g39(.a(new_n50_), .b(new_n44_), .c(x0), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n16_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n49_), .O(z5));
  oai21  g43(.a(x4), .b(x2), .c(new_n19_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n44_), .c(new_n37_), .O(new_n60_));
  aoi21  g45(.a(new_n44_), .b(x0), .c(new_n23_), .O(new_n61_));
  nand2  g46(.a(new_n50_), .b(new_n16_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(z6));
endmodule


