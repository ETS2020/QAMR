// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(x4), .c(x1), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand3  g05(.a(new_n17_), .b(new_n20_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  oai21  g07(.a(x5), .b(x3), .c(x2), .O(new_n23_));
  nand3  g08(.a(x6), .b(new_n17_), .c(new_n16_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n23_), .c(x0), .O(new_n25_));
  nand2  g10(.a(new_n24_), .b(x1), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  inv1   g12(.a(x4), .O(new_n28_));
  nor2   g13(.a(x2), .b(x0), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n28_), .c(x1), .O(new_n30_));
  nand4  g15(.a(x3), .b(new_n16_), .c(new_n20_), .d(x0), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n30_), .c(new_n27_), .O(z1));
  inv1   g17(.a(x0), .O(new_n33_));
  nand2  g18(.a(new_n17_), .b(x2), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(x4), .c(x1), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nor2   g21(.a(x3), .b(x1), .O(new_n37_));
  nand2  g22(.a(x6), .b(new_n16_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n35_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n33_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n31_), .O(z2));
  inv1   g27(.a(x7), .O(new_n43_));
  aoi21  g28(.a(new_n29_), .b(new_n43_), .c(new_n28_), .O(new_n44_));
  nand2  g29(.a(x5), .b(new_n33_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n37_), .c(x2), .O(new_n46_));
  oai21  g31(.a(new_n44_), .b(new_n20_), .c(new_n46_), .O(z3));
  nand2  g32(.a(x3), .b(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n33_), .O(new_n49_));
  xor2a  g34(.a(x3), .b(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x0), .O(new_n51_));
  nor2   g36(.a(x5), .b(x3), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(x2), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n20_), .O(new_n55_));
  nand4  g40(.a(new_n34_), .b(x4), .c(x1), .d(new_n33_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(z4));
  aoi21  g42(.a(new_n34_), .b(new_n48_), .c(new_n33_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n25_), .c(new_n20_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n56_), .O(z5));
  aoi22  g45(.a(new_n50_), .b(x0), .c(new_n52_), .d(x2), .O(new_n61_));
  oai21  g46(.a(new_n18_), .b(new_n28_), .c(x1), .O(new_n62_));
  oai21  g47(.a(new_n61_), .b(x1), .c(new_n62_), .O(z6));
endmodule


