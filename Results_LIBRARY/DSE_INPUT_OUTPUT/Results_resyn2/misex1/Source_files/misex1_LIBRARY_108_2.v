// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_;
  inv1   g00(.a(x3), .O(new_n16_));
  nor2   g01(.a(x3), .b(x1), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  oai21  g03(.a(new_n16_), .b(x0), .c(new_n18_), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  inv1   g06(.a(x2), .O(new_n22_));
  aoi21  g07(.a(new_n21_), .b(new_n20_), .c(new_n22_), .O(new_n23_));
  and2   g08(.a(new_n23_), .b(new_n19_), .O(z0));
  nand3  g09(.a(x3), .b(new_n22_), .c(x1), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(new_n22_), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n16_), .c(x2), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n21_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n25_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n20_), .O(new_n32_));
  nand3  g17(.a(x3), .b(new_n22_), .c(x0), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n21_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n32_), .O(z1));
  nor2   g21(.a(new_n16_), .b(new_n21_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n20_), .O(new_n38_));
  nand2  g23(.a(new_n28_), .b(x2), .O(new_n39_));
  nand2  g24(.a(new_n26_), .b(new_n22_), .O(new_n40_));
  nand2  g25(.a(new_n16_), .b(new_n20_), .O(new_n41_));
  aoi21  g26(.a(new_n40_), .b(new_n39_), .c(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n34_), .c(new_n21_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n38_), .O(z2));
  inv1   g29(.a(x7), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n20_), .O(new_n46_));
  nor2   g31(.a(new_n28_), .b(x0), .O(new_n47_));
  nand2  g32(.a(new_n17_), .b(x2), .O(new_n48_));
  oai22  g33(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n25_), .O(z3));
  nand2  g34(.a(new_n19_), .b(x2), .O(new_n50_));
  oai21  g35(.a(new_n37_), .b(new_n17_), .c(new_n20_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n35_), .O(z4));
  xnor2a g37(.a(x3), .b(x2), .O(new_n53_));
  nor2   g38(.a(new_n53_), .b(new_n20_), .O(new_n54_));
  nand3  g39(.a(x5), .b(x2), .c(new_n20_), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n54_), .c(new_n21_), .O(new_n57_));
  aoi21  g42(.a(x3), .b(x0), .c(new_n21_), .O(new_n58_));
  aoi21  g43(.a(new_n26_), .b(new_n22_), .c(x0), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n53_), .c(new_n58_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n57_), .O(z5));
  inv1   g46(.a(new_n29_), .O(new_n62_));
  oai21  g47(.a(new_n54_), .b(new_n62_), .c(new_n21_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n38_), .O(z6));
endmodule


