// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nand2  g05(.a(x3), .b(x2), .O(new_n21_));
  and2   g06(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g07(.a(x0), .O(new_n23_));
  aoi21  g08(.a(x1), .b(new_n23_), .c(new_n17_), .O(new_n24_));
  aoi21  g09(.a(new_n22_), .b(new_n18_), .c(new_n24_), .O(z0));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  nand2  g11(.a(x2), .b(new_n19_), .O(new_n27_));
  nand3  g12(.a(x3), .b(new_n16_), .c(x1), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(new_n26_), .c(new_n28_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  inv1   g15(.a(new_n20_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n17_), .c(new_n16_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n30_), .O(z1));
  nand2  g18(.a(x3), .b(x1), .O(new_n34_));
  nand3  g19(.a(new_n26_), .b(x2), .c(new_n19_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n23_), .O(new_n37_));
  nand3  g22(.a(new_n31_), .b(x3), .c(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n37_), .O(z2));
  nand2  g24(.a(x3), .b(new_n16_), .O(new_n40_));
  inv1   g25(.a(x7), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(x1), .c(new_n23_), .O(new_n42_));
  inv1   g27(.a(x5), .O(new_n43_));
  nor2   g28(.a(new_n43_), .b(x0), .O(new_n44_));
  nor2   g29(.a(x3), .b(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n19_), .O(new_n46_));
  oai22  g31(.a(new_n46_), .b(new_n44_), .c(new_n42_), .d(new_n40_), .O(z3));
  nand2  g32(.a(x1), .b(new_n23_), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n20_), .c(new_n17_), .O(new_n49_));
  xnor2a g34(.a(new_n49_), .b(new_n27_), .O(z4));
  aoi21  g35(.a(x3), .b(x2), .c(new_n23_), .O(new_n51_));
  nand3  g36(.a(x5), .b(x2), .c(new_n23_), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n19_), .O(new_n54_));
  inv1   g39(.a(new_n45_), .O(new_n55_));
  nand2  g40(.a(new_n16_), .b(new_n19_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n23_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n54_), .c(new_n18_), .O(z5));
  aoi21  g43(.a(new_n21_), .b(x0), .c(new_n26_), .O(new_n59_));
  oai22  g44(.a(new_n59_), .b(x1), .c(new_n45_), .d(new_n24_), .O(z6));
endmodule


