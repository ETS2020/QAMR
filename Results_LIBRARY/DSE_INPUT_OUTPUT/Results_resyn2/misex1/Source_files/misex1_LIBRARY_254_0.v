// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x0), .O(new_n16_));
  nor2   g01(.a(x1), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(x2), .c(new_n18_), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(new_n20_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  oai21  g10(.a(new_n17_), .b(new_n18_), .c(new_n25_), .O(new_n26_));
  nor2   g11(.a(x2), .b(x1), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x0), .O(new_n28_));
  oai21  g13(.a(x5), .b(x3), .c(new_n21_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x2), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n26_), .O(z1));
  nor2   g17(.a(x3), .b(new_n25_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x1), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n29_), .c(new_n16_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n26_), .O(z2));
  inv1   g21(.a(x5), .O(new_n37_));
  nor2   g22(.a(new_n37_), .b(x0), .O(new_n38_));
  nand2  g23(.a(new_n33_), .b(new_n21_), .O(new_n39_));
  nand2  g24(.a(x3), .b(new_n25_), .O(new_n40_));
  inv1   g25(.a(x7), .O(new_n41_));
  nand2  g26(.a(new_n22_), .b(new_n41_), .O(new_n42_));
  oai22  g27(.a(new_n42_), .b(new_n40_), .c(new_n39_), .d(new_n38_), .O(z3));
  xor2a  g28(.a(new_n27_), .b(x0), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n18_), .c(new_n39_), .O(z4));
  xnor2a g30(.a(x3), .b(x2), .O(new_n46_));
  nand3  g31(.a(x5), .b(x2), .c(new_n16_), .O(new_n47_));
  oai21  g32(.a(new_n46_), .b(new_n16_), .c(new_n47_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n21_), .O(new_n49_));
  oai21  g34(.a(new_n28_), .b(new_n18_), .c(new_n19_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n49_), .O(z5));
  nand2  g36(.a(new_n22_), .b(x3), .O(new_n52_));
  nor2   g37(.a(x5), .b(x3), .O(new_n53_));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  aoi22  g39(.a(new_n54_), .b(x0), .c(new_n53_), .d(x2), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(x1), .c(new_n52_), .O(z6));
endmodule


