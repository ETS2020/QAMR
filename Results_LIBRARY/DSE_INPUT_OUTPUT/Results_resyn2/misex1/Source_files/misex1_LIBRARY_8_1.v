// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:33 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  inv1   g05(.a(x7), .O(new_n21_));
  nand4  g06(.a(new_n21_), .b(new_n20_), .c(new_n17_), .d(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  oai21  g09(.a(x7), .b(new_n24_), .c(new_n20_), .O(new_n25_));
  nor2   g10(.a(new_n16_), .b(x1), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n17_), .O(new_n28_));
  aoi21  g13(.a(x7), .b(new_n20_), .c(x2), .O(new_n29_));
  aoi22  g14(.a(new_n29_), .b(new_n28_), .c(new_n26_), .d(new_n25_), .O(new_n30_));
  nor2   g15(.a(new_n20_), .b(x2), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n17_), .c(x0), .O(new_n32_));
  oai21  g17(.a(new_n30_), .b(x0), .c(new_n32_), .O(z1));
  inv1   g18(.a(x0), .O(new_n34_));
  nand3  g19(.a(new_n21_), .b(x4), .c(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n20_), .c(new_n17_), .O(new_n36_));
  nand3  g21(.a(new_n21_), .b(new_n20_), .c(new_n17_), .O(new_n37_));
  nand2  g22(.a(x5), .b(x2), .O(new_n38_));
  oai21  g23(.a(new_n27_), .b(x2), .c(new_n38_), .O(new_n39_));
  nor2   g24(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n36_), .c(new_n34_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n32_), .O(z2));
  nand2  g27(.a(x5), .b(new_n34_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n26_), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n18_), .b(new_n21_), .c(new_n16_), .O(new_n45_));
  oai21  g30(.a(new_n44_), .b(x3), .c(new_n45_), .O(z3));
  nand2  g31(.a(x7), .b(new_n20_), .O(new_n47_));
  nand3  g32(.a(x3), .b(new_n16_), .c(x0), .O(new_n48_));
  nand2  g33(.a(new_n20_), .b(x2), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n17_), .O(new_n51_));
  nand2  g36(.a(new_n31_), .b(new_n17_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n49_), .c(new_n34_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n51_), .c(new_n47_), .O(z4));
  nor2   g39(.a(x5), .b(x0), .O(new_n55_));
  nand3  g40(.a(new_n21_), .b(new_n20_), .c(x2), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n55_), .c(new_n48_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n17_), .O(new_n58_));
  nand2  g43(.a(new_n21_), .b(new_n16_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n20_), .c(x0), .O(new_n60_));
  oai21  g45(.a(new_n28_), .b(x2), .c(new_n60_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n58_), .O(z5));
  nand3  g47(.a(new_n50_), .b(new_n43_), .c(new_n17_), .O(new_n63_));
  oai21  g48(.a(x4), .b(x2), .c(new_n20_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n18_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n63_), .c(new_n47_), .O(z6));
endmodule


