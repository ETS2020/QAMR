// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:38 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand3  g02(.a(new_n17_), .b(x2), .c(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x4), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n16_), .O(new_n20_));
  nor2   g05(.a(new_n17_), .b(x0), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x2), .c(x1), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(z0));
  nor2   g08(.a(x5), .b(x3), .O(new_n24_));
  nand2  g09(.a(x2), .b(new_n16_), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n17_), .c(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n25_), .b(new_n24_), .c(new_n27_), .O(new_n28_));
  nor2   g13(.a(x2), .b(new_n16_), .O(new_n29_));
  aoi21  g14(.a(new_n28_), .b(x4), .c(new_n29_), .O(new_n30_));
  nand2  g15(.a(x4), .b(new_n26_), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand4  g17(.a(new_n32_), .b(x3), .c(new_n16_), .d(x0), .O(new_n33_));
  oai21  g18(.a(new_n30_), .b(x0), .c(new_n33_), .O(z1));
  inv1   g19(.a(x0), .O(new_n35_));
  aoi21  g20(.a(new_n31_), .b(new_n17_), .c(new_n16_), .O(new_n36_));
  inv1   g21(.a(x5), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(x2), .c(new_n16_), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n26_), .O(new_n40_));
  nand2  g25(.a(x4), .b(new_n17_), .O(new_n41_));
  aoi21  g26(.a(new_n40_), .b(new_n38_), .c(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n36_), .c(new_n35_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n33_), .O(z2));
  inv1   g29(.a(new_n41_), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n35_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n45_), .c(x2), .d(new_n16_), .O(new_n47_));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n29_), .c(new_n35_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  nand2  g35(.a(new_n21_), .b(new_n16_), .O(new_n51_));
  oai21  g36(.a(new_n17_), .b(x1), .c(x0), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n51_), .c(new_n26_), .O(new_n53_));
  inv1   g38(.a(x4), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n16_), .O(new_n55_));
  nor2   g40(.a(x3), .b(x1), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n21_), .c(x2), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n55_), .c(new_n53_), .O(z4));
  oai21  g43(.a(x3), .b(x2), .c(x0), .O(new_n59_));
  aoi21  g44(.a(x3), .b(x2), .c(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n46_), .b(new_n26_), .c(x4), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n60_), .c(new_n16_), .O(new_n62_));
  aoi21  g47(.a(new_n17_), .b(x2), .c(x0), .O(new_n63_));
  nor2   g48(.a(new_n39_), .b(x3), .O(new_n64_));
  nand2  g49(.a(new_n32_), .b(new_n16_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n62_), .O(z5));
  nor2   g52(.a(new_n63_), .b(new_n16_), .O(new_n68_));
  nand2  g53(.a(new_n17_), .b(new_n26_), .O(new_n69_));
  nand2  g54(.a(x3), .b(x2), .O(new_n70_));
  oai21  g55(.a(new_n24_), .b(x0), .c(new_n70_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n16_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  aoi21  g58(.a(new_n73_), .b(x4), .c(new_n68_), .O(z6));
endmodule


