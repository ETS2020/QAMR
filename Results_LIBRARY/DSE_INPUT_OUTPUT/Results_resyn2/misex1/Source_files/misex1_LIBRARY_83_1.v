// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x6), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nand3  g06(.a(x3), .b(x1), .c(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n20_), .c(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  aoi21  g09(.a(new_n17_), .b(new_n16_), .c(new_n24_), .O(z0));
  nand2  g10(.a(new_n18_), .b(x0), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x1), .c(x2), .O(new_n28_));
  oai21  g13(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n28_), .c(new_n21_), .O(new_n30_));
  nand3  g15(.a(new_n17_), .b(x3), .c(new_n16_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n26_), .c(new_n30_), .O(z1));
  inv1   g17(.a(x4), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x2), .c(new_n19_), .O(new_n34_));
  nand2  g19(.a(x5), .b(x2), .O(new_n35_));
  nor2   g20(.a(x3), .b(x1), .O(new_n36_));
  aoi22  g21(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(x1), .O(new_n37_));
  nand3  g22(.a(x3), .b(new_n18_), .c(x0), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n17_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n16_), .O(new_n40_));
  oai21  g25(.a(new_n37_), .b(x0), .c(new_n40_), .O(z2));
  nand2  g26(.a(x1), .b(new_n21_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n43_), .c(x6), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n21_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n36_), .c(x2), .O(new_n47_));
  oai21  g32(.a(new_n45_), .b(x2), .c(new_n47_), .O(z3));
  aoi21  g33(.a(new_n19_), .b(x2), .c(x0), .O(new_n49_));
  aoi21  g34(.a(x3), .b(new_n18_), .c(x6), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(x2), .c(new_n49_), .O(new_n51_));
  nand2  g36(.a(new_n19_), .b(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n31_), .O(new_n53_));
  nor2   g38(.a(new_n49_), .b(x1), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n51_), .O(z4));
  aoi21  g41(.a(new_n35_), .b(new_n21_), .c(x1), .O(new_n57_));
  oai21  g42(.a(new_n53_), .b(new_n21_), .c(new_n57_), .O(new_n58_));
  nand2  g43(.a(new_n49_), .b(new_n29_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n58_), .O(z5));
  nand2  g45(.a(x3), .b(new_n16_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n52_), .O(new_n62_));
  oai21  g47(.a(x5), .b(x3), .c(new_n21_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n62_), .c(new_n18_), .O(new_n64_));
  oai21  g49(.a(new_n42_), .b(x4), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n16_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n64_), .c(new_n22_), .O(z6));
endmodule


