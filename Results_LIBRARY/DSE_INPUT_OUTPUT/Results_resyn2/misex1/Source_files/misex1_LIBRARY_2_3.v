// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:31 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(x0), .c(x4), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g09(.a(x0), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n20_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n17_), .c(x2), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  inv1   g13(.a(x4), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(x2), .c(new_n17_), .O(new_n30_));
  nor2   g15(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n27_), .c(new_n25_), .O(new_n32_));
  nand3  g17(.a(x3), .b(new_n16_), .c(x0), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n17_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n32_), .O(z1));
  inv1   g21(.a(x5), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n29_), .c(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n16_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(new_n21_), .O(new_n41_));
  nand2  g26(.a(x4), .b(x2), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(x1), .O(new_n43_));
  aoi21  g28(.a(new_n29_), .b(new_n20_), .c(new_n43_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n41_), .c(new_n25_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n35_), .O(z2));
  nand2  g31(.a(x5), .b(new_n25_), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n22_), .c(x4), .O(new_n48_));
  oai21  g33(.a(x4), .b(x3), .c(x7), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n18_), .c(new_n16_), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(new_n16_), .c(new_n50_), .O(z3));
  aoi21  g36(.a(new_n20_), .b(x2), .c(x0), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(x4), .c(x2), .O(new_n53_));
  aoi21  g38(.a(x3), .b(new_n17_), .c(x0), .O(new_n54_));
  oai21  g39(.a(new_n52_), .b(x4), .c(new_n54_), .O(new_n55_));
  nand2  g40(.a(new_n20_), .b(x2), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n33_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n17_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n55_), .c(new_n53_), .O(z4));
  nand2  g44(.a(x3), .b(new_n16_), .O(new_n60_));
  aoi21  g45(.a(new_n56_), .b(new_n60_), .c(new_n25_), .O(new_n61_));
  nand3  g46(.a(x5), .b(x2), .c(new_n25_), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n61_), .c(new_n17_), .O(new_n64_));
  nand2  g49(.a(new_n26_), .b(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n52_), .b(new_n65_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n64_), .c(new_n53_), .O(z5));
  xor2a  g52(.a(x3), .b(x2), .O(new_n68_));
  aoi22  g53(.a(new_n68_), .b(x0), .c(new_n28_), .d(x2), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n20_), .O(new_n70_));
  aoi22  g55(.a(new_n70_), .b(new_n18_), .c(x4), .d(x2), .O(new_n71_));
  oai21  g56(.a(new_n69_), .b(x1), .c(new_n71_), .O(z6));
endmodule


