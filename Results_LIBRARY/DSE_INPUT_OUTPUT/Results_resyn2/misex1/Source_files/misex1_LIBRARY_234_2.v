// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x0), .c(x2), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand3  g05(.a(new_n17_), .b(x2), .c(new_n20_), .O(new_n21_));
  oai21  g06(.a(new_n21_), .b(new_n16_), .c(new_n19_), .O(z0));
  nand2  g07(.a(x2), .b(new_n20_), .O(new_n23_));
  nor2   g08(.a(x5), .b(x3), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n17_), .c(new_n25_), .O(new_n26_));
  oai21  g11(.a(new_n24_), .b(new_n23_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n16_), .O(new_n28_));
  oai21  g13(.a(new_n17_), .b(new_n16_), .c(new_n20_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n28_), .O(z1));
  nand2  g16(.a(new_n29_), .b(new_n18_), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n25_), .O(new_n34_));
  oai21  g19(.a(new_n23_), .b(x5), .c(new_n34_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n17_), .c(new_n16_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n32_), .O(z2));
  aoi21  g22(.a(x5), .b(new_n16_), .c(new_n21_), .O(z3));
  nor2   g23(.a(new_n17_), .b(new_n25_), .O(new_n39_));
  nor2   g24(.a(x3), .b(x1), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n39_), .c(new_n16_), .O(new_n41_));
  xor2a  g26(.a(x3), .b(x2), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n20_), .c(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n41_), .O(z4));
  xnor2a g29(.a(x3), .b(x2), .O(new_n45_));
  nand3  g30(.a(x5), .b(x2), .c(new_n16_), .O(new_n46_));
  oai21  g31(.a(new_n45_), .b(new_n16_), .c(new_n46_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n20_), .O(new_n48_));
  aoi21  g33(.a(new_n33_), .b(new_n25_), .c(x0), .O(new_n49_));
  aoi22  g34(.a(new_n49_), .b(new_n45_), .c(new_n25_), .d(x1), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(z5));
  aoi22  g36(.a(new_n42_), .b(x0), .c(new_n24_), .d(x2), .O(new_n52_));
  nand4  g37(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n53_));
  oai21  g38(.a(new_n52_), .b(x1), .c(new_n53_), .O(z6));
endmodule


