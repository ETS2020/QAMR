// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x5), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x2), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  aoi21  g04(.a(new_n19_), .b(x0), .c(new_n16_), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x0), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(x3), .c(x2), .O(new_n23_));
  oai21  g08(.a(new_n20_), .b(x1), .c(new_n23_), .O(z0));
  inv1   g09(.a(x0), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n26_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n25_), .c(new_n16_), .O(new_n29_));
  nand3  g14(.a(x3), .b(new_n21_), .c(x0), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n22_), .c(new_n26_), .O(new_n32_));
  oai21  g17(.a(new_n29_), .b(x1), .c(new_n32_), .O(z1));
  aoi21  g18(.a(x4), .b(new_n26_), .c(x3), .O(new_n34_));
  nand4  g19(.a(new_n27_), .b(new_n17_), .c(new_n26_), .d(new_n21_), .O(new_n35_));
  oai21  g20(.a(new_n34_), .b(new_n21_), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n25_), .O(new_n37_));
  nand3  g22(.a(x3), .b(new_n26_), .c(x0), .O(new_n38_));
  inv1   g23(.a(new_n38_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n16_), .c(new_n21_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n37_), .O(z2));
  nor2   g26(.a(new_n16_), .b(x1), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n19_), .c(x0), .O(new_n43_));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n22_), .c(new_n26_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z3));
  xor2a  g31(.a(x3), .b(x2), .O(new_n47_));
  inv1   g32(.a(new_n47_), .O(new_n48_));
  aoi22  g33(.a(new_n48_), .b(x5), .c(new_n18_), .d(x1), .O(new_n49_));
  oai21  g34(.a(new_n39_), .b(new_n19_), .c(new_n42_), .O(new_n50_));
  oai21  g35(.a(new_n49_), .b(x0), .c(new_n50_), .O(z4));
  nand2  g36(.a(new_n28_), .b(new_n25_), .O(new_n52_));
  nand2  g37(.a(new_n47_), .b(x0), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n52_), .c(x5), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n21_), .O(new_n55_));
  nand3  g40(.a(new_n18_), .b(x1), .c(new_n25_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(z5));
  aoi21  g42(.a(new_n47_), .b(x0), .c(new_n16_), .O(new_n58_));
  oai21  g43(.a(x4), .b(x2), .c(new_n17_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n22_), .O(new_n60_));
  oai21  g45(.a(new_n58_), .b(x1), .c(new_n60_), .O(z6));
endmodule


