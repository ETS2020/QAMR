// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  inv1   g04(.a(x7), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(x3), .c(new_n20_), .O(new_n23_));
  oai22  g08(.a(new_n23_), .b(new_n16_), .c(new_n19_), .d(new_n17_), .O(z0));
  inv1   g09(.a(x0), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n18_), .c(new_n21_), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  nand2  g12(.a(x2), .b(new_n16_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  inv1   g15(.a(new_n17_), .O(new_n31_));
  nor2   g16(.a(new_n18_), .b(x2), .O(new_n32_));
  oai21  g17(.a(x2), .b(x0), .c(x7), .O(new_n33_));
  aoi22  g18(.a(new_n33_), .b(x1), .c(new_n32_), .d(new_n31_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n30_), .O(z1));
  nand2  g20(.a(x1), .b(new_n25_), .O(new_n36_));
  oai21  g21(.a(new_n17_), .b(x2), .c(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x3), .O(new_n38_));
  nand3  g23(.a(x4), .b(new_n21_), .c(new_n25_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x7), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x1), .O(new_n41_));
  inv1   g26(.a(x6), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n21_), .O(new_n43_));
  oai21  g28(.a(x5), .b(new_n21_), .c(new_n43_), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n18_), .c(new_n16_), .d(new_n25_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n41_), .c(new_n38_), .O(z2));
  nor2   g31(.a(x4), .b(x2), .O(new_n47_));
  nor2   g32(.a(x3), .b(x0), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n47_), .c(new_n20_), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n25_), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n18_), .c(x2), .d(new_n16_), .O(new_n51_));
  oai21  g36(.a(new_n49_), .b(new_n16_), .c(new_n51_), .O(z3));
  oai21  g37(.a(new_n18_), .b(x2), .c(new_n25_), .O(new_n53_));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(x0), .O(new_n55_));
  nand2  g40(.a(new_n27_), .b(x2), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n16_), .O(new_n58_));
  inv1   g43(.a(new_n36_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n19_), .c(x7), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n58_), .O(z4));
  nand3  g46(.a(x5), .b(x2), .c(new_n25_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n16_), .O(new_n64_));
  inv1   g49(.a(new_n54_), .O(new_n65_));
  aoi21  g50(.a(new_n42_), .b(new_n21_), .c(x0), .O(new_n66_));
  aoi22  g51(.a(new_n66_), .b(new_n65_), .c(new_n33_), .d(x1), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n64_), .O(z5));
  aoi22  g53(.a(new_n54_), .b(x0), .c(new_n27_), .d(x2), .O(new_n69_));
  nand2  g54(.a(new_n59_), .b(x7), .O(new_n70_));
  nor2   g55(.a(new_n47_), .b(x3), .O(new_n71_));
  oai22  g56(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(x1), .O(z6));
endmodule


