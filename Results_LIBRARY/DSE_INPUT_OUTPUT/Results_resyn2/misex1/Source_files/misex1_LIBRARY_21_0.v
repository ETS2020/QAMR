// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:38 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nor2   g06(.a(x1), .b(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(x5), .c(new_n20_), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n19_), .c(new_n16_), .O(z0));
  nor2   g09(.a(new_n16_), .b(x1), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n20_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n17_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n16_), .c(new_n25_), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n20_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n21_), .O(new_n31_));
  nor2   g16(.a(new_n20_), .b(x2), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n22_), .O(new_n33_));
  oai21  g18(.a(new_n31_), .b(new_n28_), .c(new_n33_), .O(z1));
  nand2  g19(.a(x4), .b(x1), .O(new_n35_));
  inv1   g20(.a(x6), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n20_), .c(new_n17_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n35_), .c(x0), .O(new_n38_));
  nand3  g23(.a(x3), .b(new_n17_), .c(x0), .O(new_n39_));
  inv1   g24(.a(new_n39_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n38_), .c(new_n16_), .O(new_n41_));
  and2   g26(.a(new_n30_), .b(new_n19_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n41_), .O(z2));
  nand4  g28(.a(new_n20_), .b(x2), .c(new_n17_), .d(x0), .O(new_n44_));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n16_), .c(x1), .d(new_n21_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x5), .O(new_n48_));
  inv1   g33(.a(x7), .O(new_n49_));
  nand3  g34(.a(new_n32_), .b(new_n18_), .c(new_n49_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(z3));
  nand3  g36(.a(x3), .b(new_n16_), .c(x0), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  nand3  g38(.a(x5), .b(new_n20_), .c(x2), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n53_), .c(new_n17_), .O(new_n56_));
  nand2  g41(.a(new_n32_), .b(new_n17_), .O(new_n57_));
  oai21  g42(.a(new_n29_), .b(x2), .c(new_n20_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(new_n21_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n56_), .O(z4));
  nand3  g45(.a(new_n26_), .b(new_n16_), .c(new_n17_), .O(new_n61_));
  nor2   g46(.a(x3), .b(new_n16_), .O(new_n62_));
  nor2   g47(.a(new_n62_), .b(x0), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  oai21  g49(.a(new_n53_), .b(new_n62_), .c(new_n17_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n64_), .c(new_n30_), .O(z5));
  oai21  g51(.a(new_n55_), .b(new_n32_), .c(new_n22_), .O(new_n67_));
  nand2  g52(.a(x5), .b(new_n16_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(x4), .c(new_n20_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n18_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n67_), .O(z6));
endmodule


