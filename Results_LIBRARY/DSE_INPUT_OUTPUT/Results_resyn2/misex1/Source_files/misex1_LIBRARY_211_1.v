// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x6), .O(new_n16_));
  nand2  g01(.a(x7), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  nor2   g07(.a(x1), .b(new_n22_), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x3), .c(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n17_), .O(z0));
  inv1   g10(.a(new_n17_), .O(new_n26_));
  nor2   g11(.a(new_n18_), .b(x2), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n23_), .c(new_n26_), .O(new_n28_));
  inv1   g13(.a(x2), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n18_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n19_), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n18_), .c(x2), .O(new_n33_));
  aoi21  g18(.a(x2), .b(x1), .c(x0), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n33_), .c(new_n31_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n28_), .O(z1));
  nand2  g21(.a(x4), .b(new_n29_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n18_), .c(new_n19_), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nand2  g24(.a(x6), .b(new_n29_), .O(new_n40_));
  nor2   g25(.a(x3), .b(x1), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n38_), .c(new_n22_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n28_), .O(z2));
  nand2  g30(.a(x5), .b(new_n22_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n17_), .c(x2), .d(new_n19_), .O(new_n47_));
  nor2   g32(.a(x4), .b(x2), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n20_), .c(x6), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n18_), .O(new_n51_));
  nor2   g36(.a(x7), .b(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n20_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n51_), .O(z3));
  aoi21  g39(.a(new_n18_), .b(x2), .c(x0), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  xnor2a g41(.a(x3), .b(x2), .O(new_n57_));
  or2    g42(.a(new_n57_), .b(x1), .O(new_n58_));
  nand4  g43(.a(x3), .b(new_n29_), .c(new_n19_), .d(new_n22_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n17_), .O(new_n60_));
  aoi21  g45(.a(new_n58_), .b(new_n56_), .c(new_n60_), .O(z4));
  nor2   g46(.a(new_n57_), .b(new_n22_), .O(new_n62_));
  nand3  g47(.a(x5), .b(x2), .c(new_n22_), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n62_), .c(new_n19_), .O(new_n65_));
  aoi21  g50(.a(new_n55_), .b(new_n31_), .c(new_n26_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n65_), .O(z5));
  inv1   g52(.a(new_n33_), .O(new_n68_));
  oai21  g53(.a(new_n62_), .b(new_n68_), .c(new_n19_), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n18_), .O(new_n70_));
  aoi21  g55(.a(new_n70_), .b(new_n20_), .c(new_n26_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n69_), .O(z6));
endmodule


