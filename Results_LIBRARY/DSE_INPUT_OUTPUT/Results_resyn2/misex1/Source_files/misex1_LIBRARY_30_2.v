// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x2), .c(new_n16_), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x5), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x0), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n21_), .c(new_n19_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n22_), .c(new_n25_), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  nand2  g12(.a(x2), .b(new_n20_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n16_), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  nor2   g16(.a(x2), .b(x0), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n31_), .c(x1), .O(new_n33_));
  nand2  g18(.a(x3), .b(new_n25_), .O(new_n34_));
  or2    g19(.a(new_n34_), .b(new_n21_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n33_), .c(new_n30_), .O(z1));
  nand2  g21(.a(new_n22_), .b(new_n20_), .O(new_n37_));
  nand2  g22(.a(new_n31_), .b(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n25_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  nand2  g26(.a(x4), .b(new_n25_), .O(new_n42_));
  nand2  g27(.a(x5), .b(x1), .O(new_n43_));
  aoi21  g28(.a(new_n42_), .b(new_n22_), .c(new_n43_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n41_), .c(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n35_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n32_), .c(new_n31_), .O(new_n48_));
  nor2   g33(.a(new_n31_), .b(x0), .O(new_n49_));
  nand3  g34(.a(new_n22_), .b(x2), .c(new_n20_), .O(new_n50_));
  oai22  g35(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n20_), .O(z3));
  nand2  g36(.a(new_n34_), .b(new_n16_), .O(new_n52_));
  xor2a  g37(.a(x3), .b(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(x0), .O(new_n54_));
  nand2  g39(.a(new_n27_), .b(x2), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n52_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n20_), .O(new_n57_));
  nand3  g42(.a(new_n49_), .b(new_n23_), .c(x1), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(z4));
  nand3  g44(.a(x5), .b(x2), .c(new_n16_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n54_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n20_), .O(new_n62_));
  nand4  g47(.a(new_n40_), .b(new_n34_), .c(new_n23_), .d(new_n16_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n62_), .c(new_n33_), .O(z5));
  aoi22  g49(.a(new_n53_), .b(x0), .c(new_n27_), .d(x2), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n22_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n49_), .c(x1), .O(new_n67_));
  oai21  g52(.a(new_n65_), .b(x1), .c(new_n67_), .O(z6));
endmodule


