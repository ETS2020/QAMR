// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x7), .O(new_n17_));
  nand2  g02(.a(x3), .b(x2), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(x0), .c(new_n17_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x1), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(x2), .c(new_n21_), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n16_), .c(new_n20_), .O(z0));
  inv1   g09(.a(x5), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(new_n22_), .c(x2), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x3), .b(new_n27_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand4  g15(.a(new_n30_), .b(new_n28_), .c(new_n26_), .d(new_n21_), .O(new_n31_));
  nand3  g16(.a(new_n17_), .b(new_n27_), .c(x1), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  nand4  g19(.a(x3), .b(new_n27_), .c(new_n21_), .d(x0), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n34_), .O(z1));
  nand2  g21(.a(new_n22_), .b(new_n21_), .O(new_n37_));
  nand2  g22(.a(new_n25_), .b(x2), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n30_), .c(new_n37_), .O(new_n39_));
  nand2  g24(.a(x4), .b(new_n27_), .O(new_n40_));
  nand2  g25(.a(new_n17_), .b(x1), .O(new_n41_));
  aoi21  g26(.a(new_n40_), .b(new_n22_), .c(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n39_), .c(new_n16_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n35_), .O(z2));
  nor2   g29(.a(new_n25_), .b(x0), .O(new_n45_));
  nor2   g30(.a(new_n21_), .b(x0), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n17_), .c(new_n27_), .O(new_n47_));
  oai21  g32(.a(new_n45_), .b(new_n23_), .c(new_n47_), .O(z3));
  nand2  g33(.a(new_n28_), .b(new_n16_), .O(new_n49_));
  xor2a  g34(.a(x3), .b(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x0), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n49_), .c(new_n26_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  nand2  g38(.a(new_n32_), .b(new_n18_), .O(new_n54_));
  nor2   g39(.a(new_n17_), .b(new_n21_), .O(new_n55_));
  aoi21  g40(.a(new_n54_), .b(new_n16_), .c(new_n55_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n53_), .O(z4));
  nand4  g42(.a(new_n30_), .b(new_n28_), .c(new_n26_), .d(new_n16_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n51_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n21_), .O(new_n60_));
  nand2  g45(.a(new_n22_), .b(x2), .O(new_n61_));
  nand3  g46(.a(new_n46_), .b(new_n61_), .c(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z5));
  nand2  g48(.a(new_n51_), .b(new_n26_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n21_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n22_), .O(new_n66_));
  aoi21  g51(.a(new_n66_), .b(new_n46_), .c(new_n55_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z6));
endmodule


