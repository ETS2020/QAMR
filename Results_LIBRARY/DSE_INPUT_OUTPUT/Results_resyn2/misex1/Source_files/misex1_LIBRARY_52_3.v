// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x5), .O(new_n16_));
  inv1   g01(.a(x6), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand2  g05(.a(x1), .b(new_n20_), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  inv1   g07(.a(x1), .O(new_n23_));
  inv1   g08(.a(x3), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  inv1   g10(.a(new_n25_), .O(new_n26_));
  nor2   g11(.a(new_n26_), .b(new_n22_), .O(new_n27_));
  oai21  g12(.a(x3), .b(x0), .c(x2), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n27_), .c(new_n19_), .O(z0));
  inv1   g14(.a(x2), .O(new_n30_));
  nand3  g15(.a(x6), .b(new_n24_), .c(new_n20_), .O(new_n31_));
  nand3  g16(.a(x3), .b(new_n23_), .c(x0), .O(new_n32_));
  nand4  g17(.a(new_n32_), .b(new_n31_), .c(new_n21_), .d(new_n30_), .O(new_n33_));
  nor2   g18(.a(x5), .b(x3), .O(new_n34_));
  nand2  g19(.a(new_n23_), .b(new_n20_), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(new_n34_), .c(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n33_), .c(new_n19_), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(z1));
  nand2  g23(.a(x4), .b(new_n30_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n24_), .c(new_n23_), .O(new_n40_));
  nand2  g25(.a(new_n16_), .b(x2), .O(new_n41_));
  nand2  g26(.a(new_n17_), .b(new_n30_), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n41_), .c(new_n25_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n40_), .c(new_n20_), .O(new_n44_));
  inv1   g29(.a(new_n32_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n30_), .c(new_n18_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n44_), .O(z2));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n22_), .c(new_n30_), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n20_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n26_), .c(x2), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n49_), .c(new_n19_), .O(z3));
  nand2  g37(.a(new_n24_), .b(x2), .O(new_n53_));
  nand2  g38(.a(x3), .b(new_n30_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n20_), .c(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n23_), .O(new_n56_));
  aoi21  g41(.a(new_n24_), .b(x2), .c(x0), .O(new_n57_));
  oai21  g42(.a(new_n54_), .b(x1), .c(new_n57_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n56_), .c(new_n19_), .O(z4));
  xnor2a g44(.a(x3), .b(x2), .O(new_n60_));
  nand3  g45(.a(x5), .b(new_n24_), .c(x2), .O(new_n61_));
  oai21  g46(.a(new_n60_), .b(new_n20_), .c(new_n61_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n23_), .O(new_n63_));
  oai21  g48(.a(new_n17_), .b(x3), .c(new_n30_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(x1), .c(new_n57_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n63_), .c(new_n19_), .O(z5));
  oai22  g51(.a(new_n60_), .b(new_n20_), .c(new_n41_), .d(x3), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n23_), .O(new_n68_));
  oai21  g53(.a(x4), .b(x2), .c(new_n24_), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(new_n22_), .c(new_n18_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n68_), .O(z6));
endmodule


