// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(new_n17_), .c(x5), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nor2   g06(.a(new_n16_), .b(x0), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(x3), .c(x2), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n21_), .O(z0));
  inv1   g09(.a(x5), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n26_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n17_), .c(new_n25_), .O(new_n29_));
  nand3  g14(.a(x3), .b(new_n16_), .c(x0), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n22_), .c(new_n26_), .O(new_n32_));
  oai21  g17(.a(new_n29_), .b(x1), .c(new_n32_), .O(z1));
  nor2   g18(.a(new_n25_), .b(x1), .O(new_n34_));
  inv1   g19(.a(new_n34_), .O(new_n35_));
  inv1   g20(.a(x4), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(x2), .c(new_n18_), .O(new_n37_));
  nor3   g22(.a(x6), .b(x3), .c(x2), .O(new_n38_));
  aoi22  g23(.a(new_n38_), .b(new_n34_), .c(new_n37_), .d(x1), .O(new_n39_));
  nand3  g24(.a(x3), .b(new_n26_), .c(x0), .O(new_n40_));
  oai22  g25(.a(new_n40_), .b(new_n35_), .c(new_n39_), .d(x0), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n22_), .c(new_n26_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n21_), .O(z3));
  nand2  g29(.a(x3), .b(new_n26_), .O(new_n45_));
  aoi22  g30(.a(new_n45_), .b(new_n34_), .c(new_n19_), .d(x1), .O(new_n46_));
  nand2  g31(.a(new_n40_), .b(new_n19_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n34_), .O(new_n48_));
  oai21  g33(.a(new_n46_), .b(x0), .c(new_n48_), .O(z4));
  nand2  g34(.a(new_n28_), .b(new_n17_), .O(new_n50_));
  xor2a  g35(.a(x3), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x0), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n50_), .c(x5), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n16_), .O(new_n54_));
  nand3  g39(.a(new_n19_), .b(x1), .c(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n54_), .O(z5));
  oai21  g41(.a(x4), .b(x2), .c(new_n18_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n22_), .O(new_n58_));
  oai21  g43(.a(new_n52_), .b(new_n35_), .c(new_n58_), .O(z6));
endmodule


