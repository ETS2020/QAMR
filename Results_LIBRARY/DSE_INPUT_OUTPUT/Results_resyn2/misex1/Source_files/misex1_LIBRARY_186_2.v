// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n56_,
    new_n57_;
  inv1   g00(.a(x2), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  oai21  g04(.a(new_n16_), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  and2   g05(.a(new_n20_), .b(new_n17_), .O(z0));
  oai21  g06(.a(x5), .b(x3), .c(x2), .O(new_n22_));
  inv1   g07(.a(x3), .O(new_n23_));
  nand3  g08(.a(x6), .b(new_n23_), .c(new_n16_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nand2  g10(.a(new_n24_), .b(x1), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n25_), .c(new_n18_), .O(new_n27_));
  oai21  g12(.a(x2), .b(x0), .c(x3), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x1), .O(new_n29_));
  nand3  g14(.a(x3), .b(new_n16_), .c(x0), .O(new_n30_));
  or2    g15(.a(new_n30_), .b(x1), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(z1));
  nand2  g17(.a(x5), .b(x2), .O(new_n33_));
  nand2  g18(.a(x6), .b(new_n16_), .O(new_n34_));
  nor2   g19(.a(x3), .b(x0), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n19_), .O(new_n38_));
  nor2   g23(.a(new_n23_), .b(new_n19_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n18_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(z2));
  nor3   g26(.a(x7), .b(x2), .c(x0), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n39_), .O(new_n43_));
  nand2  g28(.a(x5), .b(new_n18_), .O(new_n44_));
  nor2   g29(.a(x3), .b(x1), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n44_), .c(x2), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n43_), .O(z3));
  oai21  g32(.a(new_n45_), .b(new_n39_), .c(new_n18_), .O(new_n48_));
  nand2  g33(.a(x3), .b(x0), .O(new_n49_));
  nand2  g34(.a(new_n23_), .b(x1), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n49_), .c(x2), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n48_), .c(new_n31_), .O(z4));
  nand2  g37(.a(new_n25_), .b(new_n18_), .O(new_n53_));
  nand2  g38(.a(new_n49_), .b(new_n20_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n31_), .c(new_n53_), .O(z5));
  inv1   g40(.a(x5), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n23_), .c(x2), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n54_), .c(new_n31_), .O(z6));
endmodule


