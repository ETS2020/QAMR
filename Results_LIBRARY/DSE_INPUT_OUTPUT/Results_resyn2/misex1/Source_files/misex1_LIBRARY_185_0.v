// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x5), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x3), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  oai21  g06(.a(new_n21_), .b(x1), .c(x2), .O(new_n22_));
  aoi21  g07(.a(new_n20_), .b(new_n17_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand2  g09(.a(x6), .b(new_n21_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n16_), .c(new_n18_), .O(new_n27_));
  inv1   g12(.a(x0), .O(new_n28_));
  nor2   g13(.a(new_n21_), .b(x1), .O(new_n29_));
  aoi21  g14(.a(new_n16_), .b(new_n28_), .c(x2), .O(new_n30_));
  oai21  g15(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  oai21  g16(.a(new_n27_), .b(x0), .c(new_n31_), .O(z1));
  aoi21  g17(.a(x4), .b(new_n24_), .c(x3), .O(new_n33_));
  inv1   g18(.a(x6), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(new_n21_), .c(new_n24_), .d(new_n16_), .O(new_n35_));
  oai21  g20(.a(new_n33_), .b(new_n16_), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n19_), .O(new_n37_));
  nand3  g22(.a(new_n29_), .b(new_n24_), .c(x0), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n37_), .O(z2));
  nand2  g24(.a(new_n21_), .b(x2), .O(new_n40_));
  inv1   g25(.a(x4), .O(new_n41_));
  inv1   g26(.a(x7), .O(new_n42_));
  aoi21  g27(.a(new_n41_), .b(new_n21_), .c(new_n42_), .O(new_n43_));
  nand3  g28(.a(new_n19_), .b(new_n24_), .c(x1), .O(new_n44_));
  oai22  g29(.a(new_n44_), .b(new_n43_), .c(new_n40_), .d(new_n17_), .O(z3));
  xnor2a g30(.a(x3), .b(x2), .O(new_n46_));
  nor2   g31(.a(new_n46_), .b(new_n28_), .O(new_n47_));
  nand3  g32(.a(x5), .b(new_n21_), .c(x2), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n47_), .c(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n29_), .b(new_n24_), .O(new_n51_));
  nand3  g36(.a(new_n40_), .b(new_n51_), .c(new_n19_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z4));
  aoi21  g38(.a(new_n25_), .b(new_n24_), .c(x0), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n47_), .c(new_n16_), .O(new_n55_));
  aoi21  g40(.a(new_n21_), .b(x2), .c(new_n16_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n18_), .c(new_n28_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n55_), .O(z5));
  oai21  g43(.a(x4), .b(x2), .c(new_n21_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(x1), .c(new_n18_), .O(new_n60_));
  oai22  g45(.a(new_n60_), .b(x0), .c(new_n46_), .d(new_n17_), .O(z6));
endmodule


