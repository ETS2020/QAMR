// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand4  g05(.a(x4), .b(new_n20_), .c(new_n17_), .d(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x4), .O(new_n23_));
  nand3  g08(.a(x3), .b(new_n16_), .c(x0), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(new_n23_), .c(new_n17_), .O(new_n26_));
  inv1   g11(.a(x0), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x1), .c(x2), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n20_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n16_), .c(new_n17_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n26_), .O(z1));
  oai21  g18(.a(new_n23_), .b(x2), .c(new_n20_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x1), .O(new_n35_));
  oai21  g20(.a(x5), .b(x1), .c(x2), .O(new_n36_));
  nand2  g21(.a(x6), .b(new_n16_), .O(new_n37_));
  nand4  g22(.a(new_n37_), .b(new_n36_), .c(x4), .d(new_n20_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n35_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n27_), .O(new_n40_));
  nor2   g25(.a(new_n23_), .b(x1), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n25_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n40_), .O(z2));
  nand2  g28(.a(new_n20_), .b(x2), .O(new_n44_));
  aoi21  g29(.a(x5), .b(new_n27_), .c(new_n44_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n23_), .c(new_n17_), .O(new_n46_));
  oai22  g31(.a(x7), .b(new_n17_), .c(x4), .d(x3), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n16_), .c(new_n27_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(z3));
  nand2  g34(.a(x3), .b(new_n16_), .O(new_n50_));
  aoi22  g35(.a(new_n44_), .b(x1), .c(new_n41_), .d(new_n50_), .O(new_n51_));
  nand2  g36(.a(new_n44_), .b(new_n24_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n41_), .O(new_n53_));
  oai21  g38(.a(new_n51_), .b(x0), .c(new_n53_), .O(z4));
  aoi21  g39(.a(new_n44_), .b(new_n50_), .c(new_n27_), .O(new_n55_));
  nand3  g40(.a(x5), .b(x2), .c(new_n27_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(x4), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n55_), .c(new_n17_), .O(new_n58_));
  nand3  g43(.a(new_n44_), .b(new_n31_), .c(new_n27_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n58_), .O(z5));
  oai21  g45(.a(new_n45_), .b(new_n25_), .c(new_n41_), .O(new_n61_));
  oai21  g46(.a(x4), .b(x2), .c(new_n20_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n18_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(z6));
endmodule


