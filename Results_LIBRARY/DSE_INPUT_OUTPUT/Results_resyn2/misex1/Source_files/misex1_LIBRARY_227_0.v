// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x6), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nor2   g02(.a(x3), .b(new_n17_), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x0), .c(new_n16_), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  inv1   g06(.a(x1), .O(new_n22_));
  nor2   g07(.a(new_n22_), .b(x0), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n21_), .O(new_n24_));
  oai21  g09(.a(new_n19_), .b(x1), .c(new_n24_), .O(z0));
  inv1   g10(.a(x5), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n20_), .O(new_n27_));
  nor2   g12(.a(new_n17_), .b(x1), .O(new_n28_));
  aoi21  g13(.a(x3), .b(new_n22_), .c(x2), .O(new_n29_));
  aoi21  g14(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand3  g15(.a(x3), .b(new_n17_), .c(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x6), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n22_), .O(new_n33_));
  oai21  g18(.a(new_n30_), .b(x0), .c(new_n33_), .O(z1));
  inv1   g19(.a(x0), .O(new_n35_));
  nand3  g20(.a(new_n26_), .b(new_n20_), .c(x2), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n22_), .O(new_n37_));
  nand2  g22(.a(x4), .b(new_n17_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n20_), .c(x1), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n33_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n23_), .c(new_n17_), .O(new_n43_));
  nand2  g28(.a(x5), .b(new_n35_), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n18_), .c(x6), .d(new_n22_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z3));
  nand3  g31(.a(x3), .b(new_n22_), .c(new_n35_), .O(new_n47_));
  oai21  g32(.a(new_n20_), .b(x1), .c(x0), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n47_), .c(new_n17_), .O(new_n49_));
  nand2  g34(.a(new_n16_), .b(new_n22_), .O(new_n50_));
  aoi21  g35(.a(new_n20_), .b(x1), .c(new_n17_), .O(new_n51_));
  oai21  g36(.a(new_n20_), .b(new_n35_), .c(new_n51_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n50_), .c(new_n49_), .O(z4));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(x0), .O(new_n55_));
  nand3  g40(.a(x5), .b(x2), .c(new_n35_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(x6), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n22_), .O(new_n58_));
  oai21  g43(.a(new_n29_), .b(new_n21_), .c(new_n35_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n58_), .O(z5));
  nand3  g45(.a(new_n55_), .b(new_n36_), .c(x6), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n22_), .O(new_n62_));
  oai21  g47(.a(x4), .b(x2), .c(new_n20_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n23_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n62_), .O(z6));
endmodule


