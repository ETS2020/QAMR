// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x2), .O(new_n16_));
  nand2  g01(.a(x6), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  oai21  g06(.a(x3), .b(x0), .c(x2), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n21_), .c(new_n17_), .O(z0));
  nor2   g08(.a(x6), .b(x2), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x3), .c(x0), .O(new_n25_));
  inv1   g10(.a(x0), .O(new_n26_));
  oai21  g11(.a(x5), .b(x3), .c(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n16_), .c(new_n25_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n18_), .O(new_n29_));
  nand2  g14(.a(new_n24_), .b(new_n19_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(z1));
  inv1   g16(.a(new_n25_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n18_), .O(new_n33_));
  inv1   g18(.a(x6), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  nor2   g20(.a(x4), .b(x3), .O(new_n36_));
  nand2  g21(.a(x3), .b(x2), .O(new_n37_));
  oai21  g22(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n20_), .c(new_n17_), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  aoi21  g26(.a(new_n38_), .b(x1), .c(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(x0), .c(new_n33_), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n19_), .c(x6), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n26_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n20_), .c(x2), .O(new_n47_));
  oai21  g32(.a(new_n45_), .b(x2), .c(new_n47_), .O(z3));
  inv1   g33(.a(x3), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(x2), .c(new_n32_), .O(new_n50_));
  nand2  g35(.a(new_n49_), .b(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n37_), .O(new_n52_));
  aoi21  g37(.a(new_n16_), .b(x1), .c(new_n52_), .O(new_n53_));
  nand2  g38(.a(new_n17_), .b(new_n26_), .O(new_n54_));
  oai22  g39(.a(new_n54_), .b(new_n53_), .c(new_n50_), .d(x1), .O(z4));
  oai22  g40(.a(new_n52_), .b(new_n26_), .c(new_n46_), .d(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n18_), .O(new_n57_));
  oai21  g42(.a(x2), .b(new_n18_), .c(new_n37_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n26_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n57_), .c(new_n17_), .O(z5));
  nand4  g45(.a(new_n51_), .b(new_n37_), .c(new_n27_), .d(new_n18_), .O(new_n61_));
  oai21  g46(.a(x4), .b(x2), .c(new_n49_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n19_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n61_), .c(new_n17_), .O(z6));
endmodule


