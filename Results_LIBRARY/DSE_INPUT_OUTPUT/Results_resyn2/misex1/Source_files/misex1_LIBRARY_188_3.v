// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(x0), .c(x6), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand3  g07(.a(x3), .b(new_n17_), .c(x0), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n18_), .c(new_n16_), .O(new_n25_));
  inv1   g10(.a(x0), .O(new_n26_));
  inv1   g11(.a(x3), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n27_), .O(new_n28_));
  nor2   g13(.a(x5), .b(x3), .O(new_n29_));
  nand2  g14(.a(x2), .b(new_n17_), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n29_), .c(new_n28_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n26_), .O(new_n32_));
  nand2  g17(.a(x6), .b(x2), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n32_), .c(new_n25_), .O(z1));
  nand2  g19(.a(x3), .b(x1), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n27_), .c(new_n17_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n35_), .c(x6), .O(new_n38_));
  nor2   g23(.a(x4), .b(x3), .O(new_n39_));
  nand2  g24(.a(new_n16_), .b(x1), .O(new_n40_));
  nor2   g25(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n38_), .c(new_n26_), .O(new_n42_));
  nand4  g27(.a(x3), .b(new_n16_), .c(new_n17_), .d(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n42_), .O(z2));
  nand2  g29(.a(x5), .b(new_n26_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n20_), .c(x6), .O(new_n46_));
  inv1   g31(.a(new_n40_), .O(new_n47_));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n47_), .c(new_n26_), .O(new_n49_));
  oai21  g34(.a(new_n46_), .b(new_n16_), .c(new_n49_), .O(z3));
  nand2  g35(.a(x3), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n28_), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  nand2  g38(.a(x3), .b(x0), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n16_), .c(x1), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  oai21  g41(.a(x6), .b(new_n27_), .c(x2), .O(new_n57_));
  nand3  g42(.a(x3), .b(new_n16_), .c(new_n17_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(new_n26_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n56_), .O(z4));
  nand2  g45(.a(new_n54_), .b(new_n16_), .O(new_n61_));
  nand3  g46(.a(x3), .b(x2), .c(x0), .O(new_n62_));
  inv1   g47(.a(x5), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n26_), .O(new_n64_));
  nand4  g49(.a(new_n64_), .b(new_n62_), .c(new_n61_), .d(new_n17_), .O(new_n65_));
  oai21  g50(.a(new_n52_), .b(new_n47_), .c(new_n26_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n65_), .c(new_n33_), .O(z5));
  nand3  g52(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(new_n68_));
  nand2  g53(.a(x4), .b(new_n27_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n57_), .c(new_n18_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n68_), .O(z6));
endmodule


