// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(new_n22_));
  inv1   g07(.a(x4), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(x3), .O(new_n24_));
  or2    g09(.a(new_n24_), .b(new_n22_), .O(z0));
  inv1   g10(.a(x6), .O(new_n26_));
  nor2   g11(.a(x2), .b(x1), .O(new_n27_));
  oai21  g12(.a(new_n26_), .b(x3), .c(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n17_), .O(new_n29_));
  nand2  g14(.a(new_n20_), .b(x2), .O(new_n30_));
  oai22  g15(.a(new_n30_), .b(x5), .c(new_n16_), .d(new_n19_), .O(new_n31_));
  nor2   g16(.a(x1), .b(new_n17_), .O(new_n32_));
  nor2   g17(.a(new_n20_), .b(x2), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n32_), .c(new_n24_), .O(new_n34_));
  oai21  g19(.a(new_n31_), .b(new_n29_), .c(new_n34_), .O(z1));
  nand3  g20(.a(x3), .b(new_n16_), .c(x0), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  nand2  g24(.a(new_n26_), .b(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nor3   g26(.a(x4), .b(x3), .c(x0), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n41_), .c(new_n37_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(x1), .c(new_n18_), .O(z2));
  nand2  g29(.a(x5), .b(new_n17_), .O(new_n45_));
  nor2   g30(.a(new_n16_), .b(x1), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n45_), .c(x4), .O(new_n47_));
  nand2  g32(.a(x7), .b(x3), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n49_));
  oai21  g34(.a(new_n47_), .b(x3), .c(new_n49_), .O(z3));
  nand2  g35(.a(new_n36_), .b(new_n30_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n19_), .O(new_n52_));
  nand2  g37(.a(new_n27_), .b(x3), .O(new_n53_));
  aoi21  g38(.a(new_n20_), .b(x2), .c(x0), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n24_), .c(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n52_), .O(z4));
  nor2   g41(.a(x5), .b(x0), .O(new_n57_));
  nand3  g42(.a(new_n23_), .b(new_n20_), .c(x2), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n57_), .c(new_n36_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n19_), .O(new_n60_));
  oai21  g45(.a(x4), .b(x2), .c(new_n20_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n28_), .c(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z5));
  nor2   g48(.a(new_n38_), .b(x0), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n30_), .c(new_n36_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n19_), .O(new_n66_));
  aoi21  g51(.a(new_n54_), .b(x1), .c(new_n24_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n66_), .O(z6));
endmodule


