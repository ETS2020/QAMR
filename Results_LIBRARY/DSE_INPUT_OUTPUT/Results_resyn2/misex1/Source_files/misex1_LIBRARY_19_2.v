// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:38 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x2), .c(new_n16_), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x4), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x1), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x2), .O(new_n21_));
  oai21  g06(.a(new_n21_), .b(new_n16_), .c(new_n19_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  nand3  g08(.a(x4), .b(new_n23_), .c(x1), .O(new_n24_));
  inv1   g09(.a(x1), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(new_n26_), .c(x2), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n23_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n23_), .O(new_n31_));
  nand4  g16(.a(new_n31_), .b(new_n29_), .c(new_n28_), .d(new_n25_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n24_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  nor2   g19(.a(new_n26_), .b(x2), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n25_), .c(x0), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(z1));
  nand2  g22(.a(new_n27_), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n31_), .O(new_n39_));
  aoi21  g24(.a(new_n26_), .b(x2), .c(new_n25_), .O(new_n40_));
  aoi22  g25(.a(new_n40_), .b(x4), .c(new_n39_), .d(new_n20_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x0), .c(new_n36_), .O(z2));
  nor2   g27(.a(new_n27_), .b(x0), .O(new_n43_));
  inv1   g28(.a(x7), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n16_), .O(new_n45_));
  oai22  g30(.a(new_n45_), .b(new_n24_), .c(new_n43_), .d(new_n21_), .O(z3));
  oai21  g31(.a(x3), .b(new_n23_), .c(new_n16_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(x4), .c(x1), .O(new_n48_));
  nand2  g33(.a(new_n35_), .b(x0), .O(new_n49_));
  nand2  g34(.a(new_n29_), .b(new_n16_), .O(new_n50_));
  aoi21  g35(.a(new_n26_), .b(x2), .c(x1), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  and2   g37(.a(new_n52_), .b(new_n48_), .O(z4));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  aoi21  g39(.a(new_n30_), .b(new_n23_), .c(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n40_), .c(new_n16_), .O(new_n56_));
  nand2  g41(.a(x4), .b(x1), .O(new_n57_));
  nand2  g42(.a(new_n54_), .b(x0), .O(new_n58_));
  nand2  g43(.a(new_n43_), .b(x2), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n58_), .c(new_n25_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n56_), .O(z5));
  nand2  g47(.a(new_n58_), .b(new_n28_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n25_), .O(new_n64_));
  oai21  g49(.a(new_n26_), .b(x0), .c(x4), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(x1), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z6));
endmodule


