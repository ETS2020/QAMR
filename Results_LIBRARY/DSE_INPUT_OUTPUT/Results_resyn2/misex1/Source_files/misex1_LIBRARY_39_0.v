// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  nor2   g07(.a(x4), .b(x3), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x1), .c(x2), .O(new_n27_));
  inv1   g12(.a(x2), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n19_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n28_), .c(new_n18_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n27_), .c(new_n16_), .O(new_n31_));
  nand4  g16(.a(x3), .b(new_n28_), .c(new_n18_), .d(x0), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n31_), .c(new_n24_), .O(z1));
  nand2  g18(.a(x4), .b(new_n28_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n19_), .c(new_n18_), .O(new_n35_));
  inv1   g20(.a(x5), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(x2), .c(new_n18_), .O(new_n37_));
  inv1   g22(.a(x6), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n28_), .O(new_n39_));
  nand2  g24(.a(x4), .b(new_n19_), .O(new_n40_));
  aoi21  g25(.a(new_n39_), .b(new_n37_), .c(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n35_), .c(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n32_), .O(z2));
  inv1   g28(.a(x4), .O(new_n44_));
  nor2   g29(.a(new_n28_), .b(x1), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n16_), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  inv1   g32(.a(x7), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n28_), .c(x1), .d(new_n16_), .O(new_n49_));
  oai21  g34(.a(new_n47_), .b(x3), .c(new_n49_), .O(z3));
  nand2  g35(.a(x3), .b(new_n28_), .O(new_n51_));
  oai22  g36(.a(new_n40_), .b(new_n28_), .c(new_n51_), .d(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n18_), .O(new_n53_));
  aoi21  g38(.a(new_n34_), .b(new_n19_), .c(x0), .O(new_n54_));
  oai21  g39(.a(new_n51_), .b(x1), .c(new_n54_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(z4));
  nand2  g41(.a(new_n19_), .b(x2), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n51_), .c(new_n16_), .O(new_n58_));
  nand3  g43(.a(x5), .b(new_n19_), .c(x2), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n18_), .O(new_n61_));
  aoi21  g46(.a(new_n19_), .b(x2), .c(x0), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n30_), .c(new_n23_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(z5));
  xor2a  g49(.a(x3), .b(x2), .O(new_n65_));
  aoi22  g50(.a(new_n65_), .b(x0), .c(new_n26_), .d(x2), .O(new_n66_));
  and2   g51(.a(new_n24_), .b(new_n17_), .O(new_n67_));
  oai21  g52(.a(new_n66_), .b(x1), .c(new_n67_), .O(z6));
endmodule


