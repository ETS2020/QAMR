// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  oai21  g05(.a(new_n19_), .b(x0), .c(new_n20_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n18_), .c(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nor2   g09(.a(new_n24_), .b(x1), .O(new_n25_));
  oai21  g10(.a(x5), .b(x3), .c(new_n25_), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n17_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n24_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n26_), .c(x0), .O(z1));
  nand2  g15(.a(x3), .b(x1), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nand4  g17(.a(new_n32_), .b(new_n19_), .c(x2), .d(new_n17_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  aoi21  g20(.a(x4), .b(x1), .c(x0), .O(new_n36_));
  oai21  g21(.a(new_n20_), .b(x6), .c(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n24_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n35_), .O(z2));
  nand2  g24(.a(new_n20_), .b(x2), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x0), .O(new_n41_));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nor2   g27(.a(x2), .b(x0), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n42_), .c(x1), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n41_), .c(new_n33_), .O(z3));
  nand2  g30(.a(new_n21_), .b(x2), .O(new_n46_));
  oai21  g31(.a(new_n19_), .b(x1), .c(new_n43_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(z4));
  aoi21  g33(.a(new_n25_), .b(new_n19_), .c(new_n16_), .O(new_n49_));
  oai21  g34(.a(new_n32_), .b(x1), .c(new_n19_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x2), .O(new_n51_));
  aoi21  g36(.a(new_n28_), .b(new_n24_), .c(x0), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(z5));
  nand2  g38(.a(new_n25_), .b(new_n19_), .O(new_n54_));
  nor2   g39(.a(new_n32_), .b(x0), .O(new_n55_));
  inv1   g40(.a(x4), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n24_), .c(x3), .O(new_n57_));
  nand2  g42(.a(x1), .b(new_n16_), .O(new_n58_));
  oai22  g43(.a(new_n58_), .b(new_n57_), .c(new_n55_), .d(new_n54_), .O(z6));
endmodule


