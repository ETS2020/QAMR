// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_;
  aoi21  g00(.a(x2), .b(x0), .c(x3), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand2  g02(.a(x2), .b(new_n17_), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x1), .c(new_n16_), .O(z0));
  inv1   g04(.a(x1), .O(new_n20_));
  inv1   g05(.a(x6), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n20_), .c(x2), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n17_), .O(new_n23_));
  inv1   g08(.a(x3), .O(new_n24_));
  nand3  g09(.a(x5), .b(x2), .c(new_n17_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n20_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n23_), .O(z1));
  inv1   g13(.a(x4), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x2), .c(new_n24_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x1), .O(new_n31_));
  nand2  g16(.a(x5), .b(x2), .O(new_n32_));
  inv1   g17(.a(x2), .O(new_n33_));
  nand2  g18(.a(x6), .b(new_n33_), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(new_n32_), .c(new_n24_), .d(new_n20_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n31_), .c(x0), .O(z2));
  inv1   g21(.a(x5), .O(new_n37_));
  nor2   g22(.a(new_n37_), .b(x0), .O(new_n38_));
  nand3  g23(.a(new_n24_), .b(x2), .c(new_n20_), .O(new_n39_));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n33_), .c(x1), .d(new_n17_), .O(new_n41_));
  oai21  g26(.a(new_n39_), .b(new_n38_), .c(new_n41_), .O(z3));
  nor2   g27(.a(new_n24_), .b(x1), .O(new_n43_));
  oai21  g28(.a(x3), .b(new_n33_), .c(new_n17_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(z4));
  aoi21  g30(.a(new_n37_), .b(new_n17_), .c(new_n33_), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(x3), .c(new_n20_), .O(new_n47_));
  oai21  g32(.a(new_n22_), .b(x3), .c(new_n17_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n47_), .O(z5));
  nand3  g34(.a(new_n29_), .b(new_n33_), .c(x1), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n24_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n17_), .O(new_n52_));
  aoi21  g37(.a(x5), .b(new_n17_), .c(new_n33_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(x3), .c(new_n20_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n52_), .O(z6));
endmodule


