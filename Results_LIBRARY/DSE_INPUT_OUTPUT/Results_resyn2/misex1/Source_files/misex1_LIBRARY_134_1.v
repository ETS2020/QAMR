// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x1), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  nor2   g03(.a(x3), .b(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  oai21  g05(.a(x3), .b(x0), .c(x2), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(new_n20_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  inv1   g08(.a(x6), .O(new_n24_));
  inv1   g09(.a(x1), .O(new_n25_));
  nand2  g10(.a(x3), .b(new_n25_), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n16_), .c(new_n24_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n23_), .O(new_n28_));
  nor2   g13(.a(x5), .b(x3), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x1), .c(x2), .O(new_n30_));
  aoi21  g15(.a(new_n23_), .b(new_n25_), .c(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(z1));
  inv1   g18(.a(x3), .O(new_n34_));
  inv1   g19(.a(x4), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x2), .c(new_n34_), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  aoi22  g22(.a(new_n37_), .b(new_n19_), .c(new_n36_), .d(x1), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x0), .c(new_n28_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nor2   g25(.a(x6), .b(x2), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n18_), .O(new_n42_));
  nand2  g27(.a(x5), .b(new_n16_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n19_), .c(x2), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(z3));
  nor2   g30(.a(new_n34_), .b(new_n23_), .O(new_n46_));
  aoi21  g31(.a(new_n41_), .b(new_n26_), .c(new_n46_), .O(new_n47_));
  nand2  g32(.a(new_n34_), .b(x2), .O(new_n48_));
  nand3  g33(.a(new_n24_), .b(x3), .c(new_n23_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g35(.a(new_n48_), .b(new_n16_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n25_), .O(new_n52_));
  oai21  g37(.a(new_n47_), .b(x0), .c(new_n52_), .O(z4));
  nand2  g38(.a(new_n37_), .b(new_n16_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n25_), .O(new_n55_));
  nand3  g40(.a(new_n54_), .b(new_n50_), .c(new_n25_), .O(new_n56_));
  nor2   g41(.a(x6), .b(new_n25_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n48_), .c(new_n46_), .O(new_n58_));
  aoi22  g43(.a(new_n58_), .b(new_n55_), .c(new_n56_), .d(x0), .O(z5));
  oai21  g44(.a(x5), .b(x3), .c(new_n16_), .O(new_n60_));
  nand2  g45(.a(new_n34_), .b(new_n23_), .O(new_n61_));
  nor2   g46(.a(new_n46_), .b(x1), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nand2  g48(.a(new_n18_), .b(x3), .O(new_n64_));
  oai21  g49(.a(new_n17_), .b(x4), .c(new_n24_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n23_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(z6));
endmodule


