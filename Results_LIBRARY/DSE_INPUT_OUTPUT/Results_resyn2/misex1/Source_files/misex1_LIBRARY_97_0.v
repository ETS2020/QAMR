// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x5), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(x0), .O(new_n21_));
  aoi21  g06(.a(new_n19_), .b(x0), .c(new_n21_), .O(new_n22_));
  nand2  g07(.a(x3), .b(new_n19_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n22_), .c(new_n18_), .O(z0));
  nor2   g10(.a(x2), .b(new_n19_), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n16_), .O(new_n28_));
  aoi21  g13(.a(new_n17_), .b(new_n20_), .c(x1), .O(new_n29_));
  aoi22  g14(.a(new_n29_), .b(new_n28_), .c(new_n26_), .d(x5), .O(new_n30_));
  nand3  g15(.a(x5), .b(x3), .c(new_n16_), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n19_), .c(x0), .O(new_n33_));
  oai21  g18(.a(new_n30_), .b(x0), .c(new_n33_), .O(z1));
  inv1   g19(.a(x4), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x2), .c(new_n20_), .O(new_n36_));
  nor2   g21(.a(x3), .b(x1), .O(new_n37_));
  oai21  g22(.a(x6), .b(x2), .c(x5), .O(new_n38_));
  aoi22  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(x1), .O(new_n39_));
  inv1   g24(.a(x0), .O(new_n40_));
  oai21  g25(.a(new_n23_), .b(new_n40_), .c(x5), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n16_), .O(new_n42_));
  oai21  g27(.a(new_n39_), .b(x0), .c(new_n42_), .O(z2));
  nor2   g28(.a(new_n19_), .b(x0), .O(new_n44_));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n44_), .c(new_n17_), .O(new_n46_));
  oai21  g31(.a(new_n16_), .b(new_n40_), .c(x5), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n37_), .O(new_n48_));
  oai21  g33(.a(new_n46_), .b(x2), .c(new_n48_), .O(z3));
  aoi21  g34(.a(x3), .b(new_n19_), .c(x0), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n41_), .c(new_n16_), .O(new_n51_));
  nand2  g36(.a(x5), .b(new_n16_), .O(new_n52_));
  aoi22  g37(.a(new_n52_), .b(new_n37_), .c(new_n21_), .d(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n51_), .O(z4));
  nand2  g39(.a(new_n20_), .b(x2), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n31_), .c(new_n40_), .O(new_n56_));
  nand3  g41(.a(x5), .b(x2), .c(new_n40_), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n56_), .c(new_n19_), .O(new_n59_));
  nand2  g44(.a(x6), .b(new_n20_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n16_), .c(new_n19_), .O(new_n61_));
  aoi21  g46(.a(new_n20_), .b(x2), .c(x0), .O(new_n62_));
  aoi22  g47(.a(new_n62_), .b(new_n61_), .c(new_n17_), .d(new_n16_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n59_), .O(z5));
  nand3  g49(.a(new_n17_), .b(new_n20_), .c(x2), .O(new_n65_));
  inv1   g50(.a(new_n65_), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n56_), .c(new_n19_), .O(new_n67_));
  oai21  g52(.a(x4), .b(x2), .c(new_n20_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n44_), .c(new_n18_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n67_), .O(z6));
endmodule


