// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n50_, new_n51_, new_n52_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x0), .c(x2), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand3  g05(.a(new_n17_), .b(x2), .c(new_n20_), .O(new_n21_));
  oai21  g06(.a(new_n21_), .b(new_n16_), .c(new_n19_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  nand3  g08(.a(x3), .b(new_n23_), .c(x0), .O(new_n24_));
  nand2  g09(.a(x3), .b(new_n23_), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n23_), .c(x0), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n17_), .c(x2), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n25_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n24_), .c(x1), .O(z1));
  nand2  g16(.a(new_n26_), .b(new_n23_), .O(new_n32_));
  nand3  g17(.a(new_n28_), .b(x2), .c(new_n20_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n17_), .c(new_n16_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n24_), .c(new_n19_), .O(z2));
  nor2   g21(.a(new_n28_), .b(x0), .O(new_n37_));
  oai22  g22(.a(new_n37_), .b(new_n21_), .c(x2), .d(new_n20_), .O(z3));
  oai21  g23(.a(x3), .b(new_n23_), .c(new_n24_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n20_), .O(new_n40_));
  nand2  g25(.a(new_n17_), .b(x1), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n25_), .c(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n40_), .O(z4));
  xnor2a g28(.a(x3), .b(x2), .O(new_n44_));
  nand3  g29(.a(x5), .b(x2), .c(new_n16_), .O(new_n45_));
  oai21  g30(.a(new_n44_), .b(new_n16_), .c(new_n45_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n20_), .O(new_n47_));
  aoi22  g32(.a(new_n44_), .b(new_n27_), .c(new_n23_), .d(x1), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n47_), .O(z5));
  oai21  g34(.a(new_n44_), .b(new_n16_), .c(new_n29_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n20_), .O(new_n51_));
  nand4  g36(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(z6));
endmodule


