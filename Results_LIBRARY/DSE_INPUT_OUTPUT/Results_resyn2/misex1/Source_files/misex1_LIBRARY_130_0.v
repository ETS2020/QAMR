// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x1), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  inv1   g05(.a(x5), .O(new_n21_));
  inv1   g06(.a(x6), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g08(.a(new_n19_), .b(new_n16_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n23_), .c(x2), .O(new_n25_));
  aoi21  g10(.a(new_n20_), .b(new_n17_), .c(new_n25_), .O(z0));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(new_n18_), .b(x0), .O(new_n28_));
  oai21  g13(.a(x6), .b(x5), .c(x3), .O(new_n29_));
  nor2   g14(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nor2   g15(.a(x6), .b(x5), .O(new_n31_));
  nand3  g16(.a(x6), .b(new_n19_), .c(new_n16_), .O(new_n32_));
  oai21  g17(.a(new_n31_), .b(new_n17_), .c(new_n32_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n30_), .c(new_n27_), .O(new_n34_));
  oai21  g19(.a(new_n22_), .b(new_n19_), .c(new_n21_), .O(new_n35_));
  nand4  g20(.a(new_n35_), .b(x2), .c(new_n18_), .d(new_n16_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(z1));
  nand2  g22(.a(x4), .b(new_n27_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n19_), .c(new_n18_), .O(new_n39_));
  nand2  g24(.a(new_n21_), .b(x2), .O(new_n40_));
  nand2  g25(.a(new_n22_), .b(new_n27_), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n40_), .c(new_n20_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n39_), .c(new_n16_), .O(new_n43_));
  nor2   g28(.a(new_n19_), .b(x1), .O(new_n44_));
  nor2   g29(.a(x2), .b(new_n16_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n44_), .c(new_n31_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n43_), .O(z2));
  nor2   g32(.a(x4), .b(x2), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n23_), .c(x1), .d(new_n16_), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n16_), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n23_), .c(x2), .d(new_n18_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n19_), .O(new_n53_));
  inv1   g38(.a(new_n17_), .O(new_n54_));
  nor2   g39(.a(x7), .b(x2), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n23_), .c(new_n54_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n53_), .O(z3));
  nor2   g42(.a(new_n45_), .b(new_n44_), .O(new_n58_));
  oai21  g43(.a(new_n27_), .b(new_n18_), .c(new_n58_), .O(new_n59_));
  nand2  g44(.a(new_n28_), .b(new_n27_), .O(new_n60_));
  aoi21  g45(.a(x2), .b(x0), .c(new_n19_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n60_), .c(new_n31_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n59_), .O(z4));
  nand2  g48(.a(x3), .b(new_n27_), .O(new_n64_));
  nand2  g49(.a(new_n19_), .b(x2), .O(new_n65_));
  aoi21  g50(.a(new_n65_), .b(new_n64_), .c(new_n16_), .O(new_n66_));
  nand3  g51(.a(x5), .b(new_n19_), .c(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n66_), .c(new_n18_), .O(new_n69_));
  oai21  g54(.a(new_n22_), .b(x3), .c(new_n27_), .O(new_n70_));
  aoi21  g55(.a(new_n19_), .b(x2), .c(x0), .O(new_n71_));
  oai21  g56(.a(new_n70_), .b(x1), .c(new_n71_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n69_), .c(new_n23_), .O(z5));
  nand3  g58(.a(new_n21_), .b(new_n19_), .c(x2), .O(new_n74_));
  inv1   g59(.a(new_n74_), .O(new_n75_));
  oai21  g60(.a(new_n75_), .b(new_n66_), .c(new_n18_), .O(new_n76_));
  oai21  g61(.a(x4), .b(x2), .c(new_n19_), .O(new_n77_));
  aoi21  g62(.a(new_n77_), .b(new_n54_), .c(new_n31_), .O(new_n78_));
  nand2  g63(.a(new_n78_), .b(new_n76_), .O(z6));
endmodule


