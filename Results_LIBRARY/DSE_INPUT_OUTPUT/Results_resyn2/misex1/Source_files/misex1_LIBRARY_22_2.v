// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x5), .O(new_n20_));
  nor2   g05(.a(x3), .b(x1), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(x0), .c(new_n20_), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n19_), .c(new_n16_), .O(z0));
  nor2   g08(.a(x2), .b(x0), .O(new_n24_));
  nor2   g09(.a(new_n24_), .b(x1), .O(new_n25_));
  nand2  g10(.a(x3), .b(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x0), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n17_), .O(new_n30_));
  nor2   g15(.a(x5), .b(new_n16_), .O(new_n31_));
  aoi21  g16(.a(new_n30_), .b(new_n24_), .c(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(z1));
  inv1   g18(.a(x0), .O(new_n34_));
  inv1   g19(.a(x3), .O(new_n35_));
  nand3  g20(.a(new_n29_), .b(new_n35_), .c(new_n17_), .O(new_n36_));
  oai21  g21(.a(x4), .b(x3), .c(x1), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n36_), .c(new_n34_), .O(new_n38_));
  oai21  g23(.a(new_n35_), .b(x1), .c(x0), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(new_n16_), .O(new_n40_));
  nand3  g25(.a(new_n18_), .b(x5), .c(x3), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(z2));
  nand2  g27(.a(new_n17_), .b(x0), .O(new_n43_));
  nand3  g28(.a(x5), .b(new_n35_), .c(x2), .O(new_n44_));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n24_), .c(x1), .O(new_n46_));
  oai21  g31(.a(new_n44_), .b(new_n43_), .c(new_n46_), .O(z3));
  nand2  g32(.a(new_n35_), .b(x2), .O(new_n48_));
  oai21  g33(.a(new_n26_), .b(new_n34_), .c(new_n48_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n17_), .O(new_n50_));
  aoi21  g35(.a(x3), .b(new_n17_), .c(x0), .O(new_n51_));
  aoi21  g36(.a(new_n35_), .b(x2), .c(x0), .O(new_n52_));
  oai22  g37(.a(new_n52_), .b(new_n20_), .c(new_n51_), .d(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n50_), .O(z4));
  nand3  g39(.a(new_n44_), .b(new_n26_), .c(x0), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n25_), .O(new_n56_));
  aoi21  g41(.a(new_n52_), .b(new_n30_), .c(new_n31_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(z5));
  nand2  g43(.a(new_n44_), .b(new_n26_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n17_), .c(x0), .O(new_n60_));
  nor2   g45(.a(x4), .b(x2), .O(new_n61_));
  aoi21  g46(.a(new_n20_), .b(x2), .c(new_n35_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n61_), .c(new_n18_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n60_), .O(z6));
endmodule


