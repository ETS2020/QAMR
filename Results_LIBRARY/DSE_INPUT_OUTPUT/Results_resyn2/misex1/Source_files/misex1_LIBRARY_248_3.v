// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x3), .O(new_n16_));
  aoi21  g01(.a(new_n16_), .b(x2), .c(x1), .O(new_n17_));
  aoi21  g02(.a(x3), .b(x2), .c(x0), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(new_n17_), .O(z0));
  inv1   g04(.a(x0), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  nand3  g06(.a(x6), .b(new_n16_), .c(new_n21_), .O(new_n22_));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  inv1   g08(.a(x1), .O(new_n24_));
  nand2  g09(.a(x2), .b(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(new_n23_), .c(new_n22_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n20_), .O(new_n27_));
  nand3  g12(.a(x3), .b(new_n21_), .c(x0), .O(new_n28_));
  oai21  g13(.a(new_n21_), .b(x0), .c(x1), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(z1));
  nand2  g15(.a(x4), .b(new_n21_), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n16_), .c(new_n24_), .O(new_n32_));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x2), .O(new_n34_));
  inv1   g19(.a(x6), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n21_), .O(new_n36_));
  nand2  g21(.a(new_n16_), .b(new_n24_), .O(new_n37_));
  aoi21  g22(.a(new_n36_), .b(new_n34_), .c(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n32_), .c(new_n20_), .O(new_n39_));
  inv1   g24(.a(new_n28_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n24_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n21_), .c(x1), .O(new_n44_));
  or2    g29(.a(new_n37_), .b(new_n34_), .O(new_n45_));
  nor2   g30(.a(x3), .b(new_n21_), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(x1), .c(x0), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(z3));
  nor2   g33(.a(new_n24_), .b(x0), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  oai21  g35(.a(new_n16_), .b(x2), .c(new_n20_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n28_), .c(new_n17_), .O(new_n52_));
  and2   g37(.a(new_n52_), .b(new_n50_), .O(z4));
  xnor2a g38(.a(x3), .b(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(x0), .O(new_n55_));
  nand2  g40(.a(x6), .b(new_n16_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n21_), .c(new_n20_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand3  g43(.a(new_n16_), .b(x2), .c(new_n20_), .O(new_n59_));
  aoi21  g44(.a(x5), .b(new_n24_), .c(new_n59_), .O(new_n60_));
  aoi21  g45(.a(new_n58_), .b(new_n24_), .c(new_n60_), .O(z5));
  oai21  g46(.a(x4), .b(x2), .c(new_n16_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n49_), .O(new_n63_));
  nand2  g48(.a(new_n34_), .b(new_n20_), .O(new_n64_));
  nor2   g49(.a(new_n54_), .b(x1), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n63_), .O(z6));
endmodule


