// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x3), .O(new_n16_));
  nand2  g01(.a(x7), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand2  g03(.a(x1), .b(new_n18_), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  inv1   g06(.a(x1), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x0), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n16_), .c(new_n21_), .O(new_n24_));
  oai21  g09(.a(new_n20_), .b(new_n16_), .c(new_n24_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n17_), .O(z0));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x1), .c(x2), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n16_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n21_), .c(new_n22_), .O(new_n30_));
  nand4  g15(.a(new_n30_), .b(new_n28_), .c(new_n17_), .d(new_n18_), .O(new_n31_));
  nand2  g16(.a(x3), .b(new_n21_), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n23_), .c(new_n31_), .O(z1));
  inv1   g18(.a(x7), .O(new_n34_));
  nand2  g19(.a(x4), .b(new_n21_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(new_n22_), .O(new_n36_));
  inv1   g21(.a(x5), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n21_), .O(new_n40_));
  nand2  g25(.a(new_n16_), .b(new_n22_), .O(new_n41_));
  aoi21  g26(.a(new_n40_), .b(new_n38_), .c(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n36_), .c(new_n18_), .O(new_n43_));
  oai21  g28(.a(new_n23_), .b(x2), .c(new_n19_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(x3), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n43_), .c(new_n17_), .O(z2));
  inv1   g31(.a(new_n41_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n18_), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n47_), .c(new_n21_), .O(new_n49_));
  oai21  g34(.a(new_n20_), .b(x2), .c(new_n34_), .O(new_n50_));
  nor2   g35(.a(new_n50_), .b(new_n49_), .O(z3));
  nand2  g36(.a(new_n34_), .b(new_n21_), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n16_), .c(x0), .O(new_n53_));
  oai21  g38(.a(new_n32_), .b(x1), .c(new_n53_), .O(new_n54_));
  nand3  g39(.a(new_n34_), .b(new_n16_), .c(x2), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n18_), .O(new_n56_));
  nand2  g41(.a(new_n55_), .b(new_n32_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n22_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n54_), .O(z4));
  aoi21  g44(.a(new_n55_), .b(new_n32_), .c(new_n18_), .O(new_n60_));
  nand4  g45(.a(new_n34_), .b(x5), .c(new_n16_), .d(x2), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n22_), .O(new_n63_));
  nand2  g48(.a(new_n53_), .b(new_n30_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n63_), .O(z5));
  nand4  g50(.a(new_n34_), .b(new_n37_), .c(new_n16_), .d(x2), .O(new_n66_));
  inv1   g51(.a(new_n66_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n60_), .c(new_n22_), .O(new_n68_));
  oai21  g53(.a(new_n52_), .b(x4), .c(new_n16_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n20_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n68_), .O(z6));
endmodule


