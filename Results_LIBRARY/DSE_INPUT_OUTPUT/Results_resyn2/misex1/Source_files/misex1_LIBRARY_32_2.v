// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:43 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_;
  inv1   g00(.a(x1), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(new_n17_), .O(new_n23_));
  inv1   g08(.a(x0), .O(new_n24_));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n19_), .c(new_n18_), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n16_), .O(new_n28_));
  oai21  g13(.a(new_n23_), .b(x2), .c(new_n28_), .O(z1));
  inv1   g14(.a(x4), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x2), .c(new_n19_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n17_), .O(new_n32_));
  nand2  g17(.a(x5), .b(x2), .O(new_n33_));
  aoi21  g18(.a(x6), .b(new_n18_), .c(x3), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n33_), .c(x0), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x1), .c(new_n32_), .O(z2));
  nor2   g21(.a(x2), .b(new_n16_), .O(new_n37_));
  oai21  g22(.a(x4), .b(x3), .c(x7), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nor2   g24(.a(x5), .b(x3), .O(new_n40_));
  nor2   g25(.a(new_n18_), .b(x1), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n39_), .c(x0), .O(z3));
  nor2   g28(.a(x3), .b(x2), .O(new_n44_));
  nor2   g29(.a(new_n44_), .b(new_n20_), .O(new_n45_));
  nor2   g30(.a(new_n41_), .b(new_n37_), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n45_), .c(x0), .O(z4));
  inv1   g32(.a(x5), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(x1), .c(new_n19_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(x2), .O(new_n50_));
  aoi21  g35(.a(new_n44_), .b(x6), .c(new_n37_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n50_), .c(x0), .O(z5));
  aoi21  g37(.a(new_n30_), .b(new_n18_), .c(x3), .O(new_n53_));
  aoi21  g38(.a(new_n40_), .b(x2), .c(x0), .O(new_n54_));
  oai22  g39(.a(new_n54_), .b(x1), .c(new_n53_), .d(new_n23_), .O(z6));
endmodule


