// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  nor2   g00(.a(x7), .b(x2), .O(new_n16_));
  inv1   g01(.a(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n18_), .b(x0), .O(new_n20_));
  aoi21  g05(.a(new_n19_), .b(new_n18_), .c(new_n20_), .O(new_n21_));
  oai21  g06(.a(x3), .b(x0), .c(x2), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n21_), .c(new_n17_), .O(z0));
  nor2   g08(.a(x5), .b(x3), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nor2   g11(.a(new_n26_), .b(x1), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x3), .c(new_n18_), .O(new_n29_));
  inv1   g14(.a(x7), .O(new_n30_));
  nor2   g15(.a(new_n30_), .b(x2), .O(new_n31_));
  aoi22  g16(.a(new_n31_), .b(new_n29_), .c(new_n27_), .d(new_n25_), .O(new_n32_));
  nand3  g17(.a(x3), .b(new_n18_), .c(x0), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  oai21  g20(.a(new_n32_), .b(x0), .c(new_n35_), .O(z1));
  inv1   g21(.a(x0), .O(new_n37_));
  nand2  g22(.a(x4), .b(new_n26_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n19_), .c(new_n18_), .O(new_n39_));
  nand2  g24(.a(x5), .b(x2), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n19_), .c(new_n18_), .O(new_n41_));
  aoi21  g26(.a(x6), .b(new_n26_), .c(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(new_n43_));
  oai21  g28(.a(new_n34_), .b(new_n30_), .c(new_n26_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(z2));
  nand2  g30(.a(x5), .b(new_n37_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n27_), .O(new_n47_));
  inv1   g32(.a(x4), .O(new_n48_));
  nand3  g33(.a(new_n31_), .b(new_n20_), .c(new_n48_), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(new_n47_), .c(x3), .O(z3));
  nand2  g35(.a(new_n19_), .b(x2), .O(new_n51_));
  nand4  g36(.a(x7), .b(x3), .c(new_n26_), .d(x0), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n18_), .O(new_n54_));
  aoi21  g39(.a(new_n19_), .b(x2), .c(x0), .O(new_n55_));
  nand3  g40(.a(x3), .b(new_n26_), .c(new_n18_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n17_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n54_), .O(z4));
  nand2  g43(.a(x3), .b(new_n26_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n51_), .c(new_n37_), .O(new_n60_));
  nand3  g45(.a(x5), .b(x2), .c(new_n37_), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n18_), .O(new_n63_));
  nand2  g48(.a(x6), .b(new_n19_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n26_), .c(new_n18_), .O(new_n65_));
  aoi21  g50(.a(new_n65_), .b(new_n55_), .c(new_n16_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n63_), .O(z5));
  xor2a  g52(.a(x3), .b(x2), .O(new_n68_));
  aoi22  g53(.a(new_n68_), .b(x0), .c(new_n24_), .d(x2), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n19_), .O(new_n70_));
  aoi21  g55(.a(new_n70_), .b(new_n20_), .c(new_n16_), .O(new_n71_));
  oai21  g56(.a(new_n69_), .b(x1), .c(new_n71_), .O(z6));
endmodule


