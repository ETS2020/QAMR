// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n53_, new_n54_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x3), .b(x2), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(x1), .c(new_n16_), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(z0));
  inv1   g05(.a(x3), .O(new_n21_));
  inv1   g06(.a(x5), .O(new_n22_));
  inv1   g07(.a(x1), .O(new_n23_));
  nand2  g08(.a(x2), .b(new_n23_), .O(new_n24_));
  aoi21  g09(.a(new_n22_), .b(new_n21_), .c(new_n24_), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n21_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n23_), .c(x2), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(new_n16_), .O(new_n28_));
  inv1   g13(.a(x2), .O(new_n29_));
  nand4  g14(.a(x3), .b(new_n29_), .c(new_n23_), .d(x0), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n28_), .O(z1));
  nand2  g16(.a(new_n29_), .b(new_n16_), .O(new_n32_));
  nor2   g17(.a(x4), .b(x3), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n32_), .c(new_n17_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x1), .O(new_n35_));
  aoi21  g20(.a(x3), .b(new_n23_), .c(x2), .O(new_n36_));
  or2    g21(.a(new_n36_), .b(new_n16_), .O(new_n37_));
  oai22  g22(.a(new_n32_), .b(x6), .c(x5), .d(new_n29_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n21_), .c(new_n23_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(z2));
  nor3   g25(.a(x5), .b(x3), .c(x1), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x0), .c(x2), .O(new_n42_));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n29_), .c(x1), .d(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(z3));
  oai21  g30(.a(new_n36_), .b(new_n18_), .c(new_n16_), .O(new_n46_));
  oai21  g31(.a(new_n21_), .b(x2), .c(x0), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n32_), .c(new_n23_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(z4));
  oai21  g34(.a(new_n24_), .b(new_n22_), .c(new_n17_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n27_), .c(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n30_), .O(z5));
  oai21  g37(.a(x4), .b(x2), .c(new_n21_), .O(new_n53_));
  aoi22  g38(.a(new_n53_), .b(x1), .c(new_n41_), .d(x2), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(x0), .c(new_n30_), .O(z6));
endmodule


