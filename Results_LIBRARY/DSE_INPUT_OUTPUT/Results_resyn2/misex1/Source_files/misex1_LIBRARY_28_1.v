// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nor2   g03(.a(x1), .b(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(x4), .O(new_n21_));
  nand4  g06(.a(new_n21_), .b(x3), .c(x1), .d(new_n18_), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n20_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x1), .O(new_n24_));
  nand2  g09(.a(x6), .b(new_n17_), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n24_), .c(x2), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  nand2  g12(.a(x2), .b(new_n24_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n27_), .c(new_n21_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n26_), .c(new_n18_), .O(new_n30_));
  nor2   g15(.a(new_n17_), .b(x2), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n19_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n30_), .O(z1));
  nand3  g18(.a(x3), .b(new_n16_), .c(x0), .O(new_n34_));
  nand3  g19(.a(new_n21_), .b(new_n17_), .c(new_n18_), .O(new_n35_));
  inv1   g20(.a(x6), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  oai21  g22(.a(new_n36_), .b(x2), .c(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n35_), .c(new_n34_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n24_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n22_), .O(z2));
  nand3  g26(.a(new_n27_), .b(x2), .c(new_n24_), .O(new_n42_));
  nand2  g27(.a(x7), .b(x3), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n16_), .c(x1), .d(new_n18_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n21_), .O(new_n46_));
  nand3  g31(.a(new_n19_), .b(new_n17_), .c(x2), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(z3));
  nand2  g33(.a(x3), .b(new_n16_), .O(new_n49_));
  nand2  g34(.a(new_n17_), .b(x2), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n49_), .c(new_n18_), .O(new_n51_));
  nand3  g36(.a(new_n21_), .b(new_n17_), .c(x2), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n24_), .O(new_n54_));
  nand2  g39(.a(new_n31_), .b(new_n24_), .O(new_n55_));
  nand4  g40(.a(new_n55_), .b(new_n50_), .c(new_n21_), .d(new_n18_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n54_), .O(z4));
  nor2   g42(.a(new_n37_), .b(x0), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n51_), .c(new_n24_), .O(new_n59_));
  oai21  g44(.a(new_n17_), .b(new_n16_), .c(new_n21_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n26_), .c(new_n18_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n59_), .O(z5));
  inv1   g47(.a(x5), .O(new_n63_));
  nand4  g48(.a(new_n63_), .b(new_n21_), .c(new_n17_), .d(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n51_), .c(new_n24_), .O(new_n66_));
  nand4  g51(.a(new_n50_), .b(new_n21_), .c(x1), .d(new_n18_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n66_), .O(z6));
endmodule


