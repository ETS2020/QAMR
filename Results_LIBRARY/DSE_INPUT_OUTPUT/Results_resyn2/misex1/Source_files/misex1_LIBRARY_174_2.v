// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_;
  inv1   g00(.a(x6), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x3), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  nand2  g06(.a(new_n19_), .b(x0), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n18_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n17_), .O(z0));
  inv1   g10(.a(x0), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x6), .b(x1), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x3), .O(new_n29_));
  nand2  g14(.a(new_n16_), .b(new_n19_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n27_), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n18_), .O(new_n33_));
  nand4  g18(.a(new_n33_), .b(new_n17_), .c(x2), .d(new_n19_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n26_), .O(new_n36_));
  nand3  g21(.a(x6), .b(x3), .c(new_n27_), .O(new_n37_));
  or2    g22(.a(new_n37_), .b(new_n22_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(z1));
  aoi21  g24(.a(x4), .b(new_n27_), .c(x3), .O(new_n40_));
  oai21  g25(.a(x6), .b(new_n18_), .c(x1), .O(new_n41_));
  nor2   g26(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g27(.a(x5), .b(x2), .O(new_n43_));
  nand2  g28(.a(x6), .b(new_n27_), .O(new_n44_));
  nor2   g29(.a(x3), .b(x1), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  inv1   g31(.a(new_n46_), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n42_), .c(new_n26_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n38_), .O(z2));
  nand2  g34(.a(x2), .b(new_n19_), .O(new_n50_));
  nor2   g35(.a(new_n32_), .b(x0), .O(new_n51_));
  nand2  g36(.a(x7), .b(x4), .O(new_n52_));
  nand4  g37(.a(new_n52_), .b(new_n27_), .c(x1), .d(new_n26_), .O(new_n53_));
  oai21  g38(.a(new_n51_), .b(new_n50_), .c(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n18_), .O(new_n55_));
  inv1   g40(.a(x7), .O(new_n56_));
  nand4  g41(.a(new_n20_), .b(new_n56_), .c(x6), .d(new_n27_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n55_), .O(z3));
  aoi21  g43(.a(new_n28_), .b(x3), .c(x2), .O(new_n59_));
  nand3  g44(.a(x6), .b(x3), .c(x2), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n26_), .O(new_n62_));
  nand2  g47(.a(new_n18_), .b(x2), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n37_), .O(new_n64_));
  nand2  g49(.a(new_n63_), .b(new_n26_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n64_), .c(new_n19_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n62_), .O(z4));
  aoi21  g52(.a(new_n63_), .b(new_n37_), .c(new_n26_), .O(new_n68_));
  nor2   g53(.a(new_n43_), .b(x3), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n68_), .c(new_n19_), .O(new_n70_));
  nand2  g55(.a(new_n60_), .b(new_n31_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n26_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n70_), .O(z5));
  nand3  g58(.a(new_n32_), .b(new_n18_), .c(x2), .O(new_n74_));
  inv1   g59(.a(new_n74_), .O(new_n75_));
  oai21  g60(.a(new_n75_), .b(new_n68_), .c(new_n19_), .O(new_n76_));
  oai21  g61(.a(x4), .b(x2), .c(new_n18_), .O(new_n77_));
  nand2  g62(.a(new_n77_), .b(new_n20_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(new_n76_), .c(new_n17_), .O(z6));
endmodule


