// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(x4), .b(x0), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n18_), .c(new_n17_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  nand3  g07(.a(x3), .b(x1), .c(new_n22_), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n21_), .c(new_n16_), .O(z0));
  nand4  g09(.a(x4), .b(x3), .c(new_n16_), .d(new_n17_), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n18_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n17_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n16_), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n18_), .O(new_n30_));
  nor2   g15(.a(new_n16_), .b(x1), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n30_), .c(x0), .O(new_n32_));
  aoi22  g17(.a(new_n32_), .b(new_n28_), .c(new_n25_), .d(x0), .O(z1));
  nand2  g18(.a(new_n25_), .b(x4), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x0), .O(new_n35_));
  nand2  g20(.a(x4), .b(new_n16_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n18_), .c(new_n17_), .O(new_n37_));
  nand2  g22(.a(x5), .b(x2), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n18_), .c(new_n17_), .O(new_n39_));
  aoi21  g24(.a(x6), .b(new_n16_), .c(new_n39_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n37_), .c(new_n22_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n35_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n16_), .c(x1), .O(new_n44_));
  nand4  g29(.a(new_n29_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n22_), .O(new_n47_));
  oai21  g32(.a(new_n21_), .b(new_n16_), .c(new_n47_), .O(z3));
  xor2a  g33(.a(x3), .b(x2), .O(new_n49_));
  aoi22  g34(.a(new_n49_), .b(new_n20_), .c(x2), .d(new_n22_), .O(new_n50_));
  nor2   g35(.a(x2), .b(new_n17_), .O(new_n51_));
  xnor2a g36(.a(x3), .b(x2), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n51_), .c(new_n22_), .O(new_n53_));
  oai21  g38(.a(new_n50_), .b(x1), .c(new_n53_), .O(z4));
  nor2   g39(.a(new_n52_), .b(new_n19_), .O(new_n55_));
  nand3  g40(.a(x5), .b(x2), .c(new_n22_), .O(new_n56_));
  inv1   g41(.a(new_n56_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n55_), .c(new_n17_), .O(new_n58_));
  aoi21  g43(.a(new_n26_), .b(new_n17_), .c(x2), .O(new_n59_));
  nor2   g44(.a(new_n18_), .b(new_n16_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n59_), .c(new_n22_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n58_), .O(z5));
  nor2   g47(.a(x4), .b(x2), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(x3), .c(x1), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n45_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n22_), .O(new_n66_));
  nand2  g51(.a(new_n55_), .b(new_n17_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n66_), .O(z6));
endmodule


