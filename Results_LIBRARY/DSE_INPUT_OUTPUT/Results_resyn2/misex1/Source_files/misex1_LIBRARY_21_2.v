// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x5), .b(x3), .O(new_n17_));
  oai21  g02(.a(x1), .b(new_n16_), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(x0), .c(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n18_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  nand3  g07(.a(x5), .b(x2), .c(new_n19_), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  inv1   g09(.a(x3), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n25_), .O(new_n26_));
  oai21  g11(.a(x5), .b(new_n25_), .c(x1), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n26_), .c(x2), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n24_), .c(new_n16_), .O(new_n29_));
  inv1   g14(.a(new_n17_), .O(new_n30_));
  nor2   g15(.a(x2), .b(x1), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x0), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n29_), .O(z1));
  inv1   g20(.a(x5), .O(new_n36_));
  oai21  g21(.a(new_n33_), .b(new_n36_), .c(x3), .O(new_n37_));
  inv1   g22(.a(x2), .O(new_n38_));
  nand2  g23(.a(new_n25_), .b(new_n38_), .O(new_n39_));
  aoi21  g24(.a(new_n36_), .b(x2), .c(x1), .O(new_n40_));
  oai21  g25(.a(new_n39_), .b(x6), .c(new_n40_), .O(new_n41_));
  nand2  g26(.a(x4), .b(new_n38_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n25_), .c(x1), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n41_), .c(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n37_), .O(z2));
  nand2  g30(.a(x2), .b(new_n19_), .O(new_n46_));
  nand2  g31(.a(x7), .b(x4), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n38_), .c(x1), .d(new_n16_), .O(new_n48_));
  nor2   g33(.a(new_n36_), .b(x0), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n46_), .c(new_n48_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n25_), .O(new_n51_));
  nor2   g36(.a(new_n19_), .b(x0), .O(new_n52_));
  nor2   g37(.a(x7), .b(new_n36_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n52_), .c(new_n38_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n51_), .O(z3));
  aoi21  g40(.a(new_n25_), .b(x2), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n31_), .b(new_n36_), .c(x3), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g43(.a(new_n25_), .b(x2), .O(new_n59_));
  nand3  g44(.a(x5), .b(x3), .c(new_n38_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g46(.a(new_n56_), .b(x1), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n58_), .O(z4));
  aoi21  g49(.a(new_n60_), .b(new_n59_), .c(new_n16_), .O(new_n65_));
  nand3  g50(.a(x5), .b(new_n25_), .c(x2), .O(new_n66_));
  inv1   g51(.a(new_n66_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n65_), .c(new_n19_), .O(new_n68_));
  nand2  g53(.a(new_n31_), .b(new_n26_), .O(new_n69_));
  aoi22  g54(.a(new_n69_), .b(new_n56_), .c(new_n36_), .d(x3), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n68_), .O(z5));
  nand3  g56(.a(new_n36_), .b(new_n25_), .c(x2), .O(new_n72_));
  inv1   g57(.a(new_n72_), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n65_), .c(new_n19_), .O(new_n74_));
  oai21  g59(.a(new_n39_), .b(x4), .c(new_n17_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n52_), .O(new_n76_));
  nand2  g61(.a(new_n76_), .b(new_n74_), .O(z6));
endmodule


