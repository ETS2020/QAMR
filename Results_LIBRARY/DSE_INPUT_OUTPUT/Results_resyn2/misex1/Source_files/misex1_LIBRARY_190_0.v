// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  oai21  g02(.a(x1), .b(new_n16_), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(x0), .c(x3), .O(new_n20_));
  nor2   g05(.a(x7), .b(x6), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  nand4  g07(.a(new_n22_), .b(new_n20_), .c(new_n18_), .d(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(z0));
  nor2   g09(.a(x5), .b(x3), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x1), .c(x2), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  nor2   g12(.a(x2), .b(x1), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(x3), .c(new_n28_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n26_), .c(new_n16_), .O(new_n30_));
  inv1   g15(.a(x2), .O(new_n31_));
  nand4  g16(.a(x3), .b(new_n31_), .c(new_n19_), .d(x0), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n30_), .c(new_n22_), .O(z1));
  inv1   g18(.a(new_n32_), .O(new_n34_));
  aoi21  g19(.a(x4), .b(new_n31_), .c(x3), .O(new_n35_));
  inv1   g20(.a(x5), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n17_), .c(x2), .d(new_n19_), .O(new_n37_));
  oai21  g22(.a(new_n35_), .b(new_n19_), .c(new_n37_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n16_), .c(new_n34_), .O(new_n39_));
  nor2   g24(.a(x3), .b(x0), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n28_), .c(x7), .d(new_n27_), .O(new_n41_));
  oai21  g26(.a(new_n39_), .b(new_n21_), .c(new_n41_), .O(z2));
  nor2   g27(.a(new_n19_), .b(x0), .O(new_n43_));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n43_), .c(new_n31_), .O(new_n45_));
  nand2  g30(.a(new_n17_), .b(x2), .O(new_n46_));
  inv1   g31(.a(new_n46_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n16_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n47_), .c(new_n19_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n45_), .c(new_n22_), .O(z3));
  nand2  g35(.a(new_n46_), .b(new_n16_), .O(new_n51_));
  nand2  g36(.a(x3), .b(new_n31_), .O(new_n52_));
  nand2  g37(.a(new_n46_), .b(new_n52_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n51_), .c(new_n22_), .O(new_n55_));
  aoi21  g40(.a(new_n54_), .b(new_n51_), .c(new_n55_), .O(z4));
  nand2  g41(.a(new_n53_), .b(x0), .O(new_n57_));
  nand3  g42(.a(x5), .b(x2), .c(new_n16_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n19_), .O(new_n60_));
  inv1   g45(.a(new_n51_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n29_), .c(new_n21_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z5));
  nand2  g48(.a(new_n47_), .b(new_n36_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n57_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n19_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n17_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n43_), .c(new_n21_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n66_), .O(z6));
endmodule


