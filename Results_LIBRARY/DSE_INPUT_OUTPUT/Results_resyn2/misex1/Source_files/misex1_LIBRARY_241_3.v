// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nor2   g06(.a(x1), .b(new_n21_), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n20_), .c(x5), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand2  g09(.a(x2), .b(x0), .O(new_n25_));
  nor2   g10(.a(x2), .b(x0), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  nand4  g12(.a(new_n27_), .b(new_n25_), .c(x3), .d(new_n17_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n17_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n26_), .O(new_n31_));
  nand2  g16(.a(x5), .b(x2), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n31_), .c(new_n28_), .O(z1));
  nand2  g18(.a(x6), .b(new_n16_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n20_), .c(x1), .O(new_n35_));
  inv1   g20(.a(x4), .O(new_n36_));
  nor2   g21(.a(new_n36_), .b(x2), .O(new_n37_));
  nand2  g22(.a(new_n20_), .b(x1), .O(new_n38_));
  aoi21  g23(.a(x5), .b(x2), .c(x0), .O(new_n39_));
  oai21  g24(.a(new_n38_), .b(new_n37_), .c(new_n39_), .O(new_n40_));
  nand3  g25(.a(new_n22_), .b(x3), .c(new_n16_), .O(new_n41_));
  oai21  g26(.a(new_n40_), .b(new_n35_), .c(new_n41_), .O(z2));
  inv1   g27(.a(x5), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n20_), .c(x2), .O(new_n44_));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n26_), .c(x1), .O(new_n46_));
  oai21  g31(.a(new_n44_), .b(x1), .c(new_n46_), .O(z3));
  nand3  g32(.a(x3), .b(new_n16_), .c(x0), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n17_), .O(new_n50_));
  aoi21  g35(.a(x3), .b(new_n17_), .c(x0), .O(new_n51_));
  aoi21  g36(.a(new_n20_), .b(x2), .c(x0), .O(new_n52_));
  oai22  g37(.a(new_n52_), .b(x5), .c(new_n51_), .d(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n50_), .O(z4));
  oai21  g39(.a(new_n30_), .b(x2), .c(new_n52_), .O(new_n55_));
  nand2  g40(.a(x3), .b(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n20_), .b(x2), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n22_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n55_), .c(new_n32_), .O(z5));
  nand2  g45(.a(new_n48_), .b(new_n57_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n17_), .O(new_n62_));
  oai21  g47(.a(x4), .b(x2), .c(new_n20_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n18_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n62_), .c(new_n32_), .O(z6));
endmodule


