// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x5), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n17_), .c(x3), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n18_), .c(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n20_), .c(new_n16_), .O(z0));
  nand3  g08(.a(new_n16_), .b(new_n18_), .c(x0), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x5), .c(x3), .O(new_n26_));
  nand3  g11(.a(new_n17_), .b(new_n21_), .c(x2), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n21_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n16_), .c(new_n18_), .O(new_n29_));
  aoi21  g14(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n27_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n26_), .O(z1));
  inv1   g17(.a(x0), .O(new_n33_));
  nand2  g18(.a(new_n21_), .b(x2), .O(new_n34_));
  inv1   g19(.a(x4), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n21_), .O(new_n36_));
  nand2  g21(.a(x5), .b(x3), .O(new_n37_));
  nand4  g22(.a(new_n37_), .b(new_n36_), .c(new_n34_), .d(x1), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nor2   g24(.a(x3), .b(x1), .O(new_n40_));
  nand2  g25(.a(x6), .b(new_n16_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n33_), .O(new_n44_));
  nand3  g29(.a(new_n25_), .b(new_n17_), .c(x3), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z2));
  nand2  g31(.a(new_n36_), .b(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n19_), .c(new_n16_), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n33_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n40_), .c(x2), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n48_), .c(new_n37_), .O(z3));
  nand2  g36(.a(x3), .b(new_n16_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n33_), .c(new_n34_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n18_), .O(new_n54_));
  inv1   g39(.a(new_n37_), .O(new_n55_));
  aoi21  g40(.a(new_n21_), .b(x2), .c(x0), .O(new_n56_));
  nand3  g41(.a(x3), .b(new_n16_), .c(new_n18_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n54_), .O(z4));
  inv1   g44(.a(new_n39_), .O(new_n60_));
  aoi21  g45(.a(new_n52_), .b(new_n34_), .c(new_n33_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n60_), .c(new_n18_), .O(new_n62_));
  aoi21  g47(.a(new_n56_), .b(new_n29_), .c(new_n55_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n62_), .O(z5));
  inv1   g49(.a(new_n27_), .O(new_n65_));
  oai21  g50(.a(new_n61_), .b(new_n65_), .c(new_n18_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n21_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n19_), .c(new_n55_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n66_), .O(z6));
endmodule


