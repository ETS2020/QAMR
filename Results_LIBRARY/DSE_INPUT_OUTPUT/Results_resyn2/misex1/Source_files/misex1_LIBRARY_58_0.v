// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x7), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x3), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  oai21  g06(.a(new_n21_), .b(x1), .c(x2), .O(new_n22_));
  aoi21  g07(.a(new_n20_), .b(new_n17_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand4  g09(.a(x3), .b(new_n24_), .c(new_n16_), .d(x0), .O(new_n25_));
  inv1   g10(.a(x0), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n21_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n16_), .c(x2), .O(new_n28_));
  nor2   g13(.a(x5), .b(x3), .O(new_n29_));
  nand2  g14(.a(x2), .b(new_n16_), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n29_), .c(x7), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n25_), .O(z1));
  nand2  g18(.a(x6), .b(new_n24_), .O(new_n34_));
  nor2   g19(.a(x3), .b(x1), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n21_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n37_), .c(x7), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n26_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n25_), .O(z2));
  nor2   g28(.a(x4), .b(x2), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n21_), .c(x1), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(x7), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n26_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n26_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n35_), .c(x2), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  nand2  g35(.a(new_n21_), .b(x2), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n19_), .c(new_n16_), .O(new_n52_));
  nand2  g37(.a(x3), .b(new_n24_), .O(new_n53_));
  aoi21  g38(.a(new_n51_), .b(new_n53_), .c(new_n26_), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n53_), .b(x1), .c(new_n19_), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n55_), .c(new_n52_), .O(z4));
  nand3  g42(.a(x5), .b(x2), .c(new_n26_), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n54_), .c(new_n16_), .O(new_n60_));
  oai21  g45(.a(new_n21_), .b(new_n24_), .c(x7), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n28_), .c(new_n26_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z5));
  nand2  g48(.a(new_n29_), .b(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n54_), .c(new_n16_), .O(new_n66_));
  oai21  g51(.a(new_n44_), .b(x3), .c(x1), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(x7), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n26_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z6));
endmodule


