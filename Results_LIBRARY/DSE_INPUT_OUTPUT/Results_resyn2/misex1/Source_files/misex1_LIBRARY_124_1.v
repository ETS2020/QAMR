// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  inv1   g05(.a(x7), .O(new_n21_));
  nand4  g06(.a(new_n21_), .b(new_n20_), .c(new_n17_), .d(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand3  g08(.a(x3), .b(new_n16_), .c(x0), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x7), .c(new_n17_), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n20_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n17_), .c(new_n16_), .O(new_n29_));
  inv1   g14(.a(x0), .O(new_n30_));
  nand2  g15(.a(x6), .b(new_n20_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n16_), .c(new_n17_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n29_), .c(new_n26_), .O(z1));
  nand2  g19(.a(x4), .b(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n20_), .c(new_n17_), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n20_), .c(new_n17_), .O(new_n38_));
  aoi21  g23(.a(x6), .b(new_n16_), .c(new_n38_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n36_), .c(new_n30_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n26_), .O(z2));
  nand2  g26(.a(x5), .b(new_n30_), .O(new_n42_));
  nand2  g27(.a(new_n20_), .b(x2), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n42_), .c(x7), .O(new_n45_));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n18_), .c(new_n16_), .O(new_n47_));
  oai21  g32(.a(new_n45_), .b(x1), .c(new_n47_), .O(z3));
  nand2  g33(.a(x3), .b(new_n16_), .O(new_n49_));
  nor2   g34(.a(x7), .b(x1), .O(new_n50_));
  aoi22  g35(.a(new_n50_), .b(new_n49_), .c(new_n43_), .d(x1), .O(new_n51_));
  oai21  g36(.a(new_n44_), .b(new_n25_), .c(new_n50_), .O(new_n52_));
  oai21  g37(.a(new_n51_), .b(x0), .c(new_n52_), .O(z4));
  aoi21  g38(.a(new_n43_), .b(new_n49_), .c(new_n30_), .O(new_n54_));
  oai21  g39(.a(new_n42_), .b(new_n16_), .c(new_n21_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n17_), .O(new_n56_));
  nand3  g41(.a(new_n43_), .b(new_n32_), .c(new_n30_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(z5));
  oai21  g43(.a(new_n28_), .b(new_n16_), .c(new_n21_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n54_), .c(new_n17_), .O(new_n60_));
  oai21  g45(.a(x4), .b(x2), .c(new_n20_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n18_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z6));
endmodule


