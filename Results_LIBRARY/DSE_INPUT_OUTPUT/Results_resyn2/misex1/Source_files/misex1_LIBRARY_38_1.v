// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  nand2  g01(.a(x7), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  oai21  g07(.a(x1), .b(new_n22_), .c(new_n18_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n17_), .O(z0));
  nand2  g10(.a(x6), .b(new_n18_), .O(new_n26_));
  inv1   g11(.a(x7), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n16_), .O(new_n28_));
  aoi21  g13(.a(new_n26_), .b(new_n19_), .c(new_n28_), .O(new_n29_));
  inv1   g14(.a(x5), .O(new_n30_));
  nand2  g15(.a(x2), .b(new_n19_), .O(new_n31_));
  aoi21  g16(.a(new_n30_), .b(new_n18_), .c(new_n31_), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n29_), .c(new_n22_), .O(new_n33_));
  nand4  g18(.a(new_n27_), .b(x3), .c(new_n16_), .d(x0), .O(new_n34_));
  inv1   g19(.a(new_n34_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n19_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n33_), .O(z1));
  nand2  g22(.a(new_n30_), .b(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand3  g24(.a(new_n27_), .b(new_n39_), .c(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n18_), .b(new_n19_), .O(new_n41_));
  aoi21  g26(.a(new_n40_), .b(new_n38_), .c(new_n41_), .O(new_n42_));
  aoi21  g27(.a(x4), .b(new_n16_), .c(x3), .O(new_n43_));
  oai21  g28(.a(new_n27_), .b(x2), .c(x1), .O(new_n44_));
  nor2   g29(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n42_), .c(new_n22_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n36_), .O(z2));
  oai21  g32(.a(new_n20_), .b(x7), .c(new_n16_), .O(new_n48_));
  nor2   g33(.a(new_n30_), .b(x0), .O(new_n49_));
  nand3  g34(.a(new_n18_), .b(x2), .c(new_n19_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(z3));
  nand2  g36(.a(new_n18_), .b(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n34_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  xor2a  g39(.a(x3), .b(x2), .O(new_n55_));
  oai21  g40(.a(x2), .b(new_n19_), .c(new_n55_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n17_), .c(new_n22_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n54_), .O(z4));
  nand3  g43(.a(new_n27_), .b(x3), .c(new_n16_), .O(new_n59_));
  aoi21  g44(.a(new_n52_), .b(new_n59_), .c(new_n22_), .O(new_n60_));
  nand3  g45(.a(x5), .b(x2), .c(new_n22_), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n19_), .O(new_n63_));
  nor2   g48(.a(new_n18_), .b(new_n16_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n29_), .c(new_n22_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n63_), .O(z5));
  nand2  g51(.a(new_n38_), .b(new_n22_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n55_), .c(new_n19_), .O(new_n68_));
  oai21  g53(.a(x4), .b(x2), .c(new_n18_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n20_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(new_n68_), .c(new_n17_), .O(z6));
endmodule


