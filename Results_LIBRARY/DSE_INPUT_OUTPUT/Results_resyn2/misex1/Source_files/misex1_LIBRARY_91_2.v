// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:08 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x6), .b(x1), .c(new_n16_), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x3), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n16_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n18_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n19_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x2), .c(new_n23_), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x3), .b(new_n23_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(x6), .c(new_n27_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n16_), .O(new_n31_));
  nand4  g16(.a(x3), .b(new_n27_), .c(new_n23_), .d(x0), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n31_), .O(z1));
  aoi21  g18(.a(x4), .b(new_n27_), .c(x3), .O(new_n34_));
  inv1   g19(.a(x6), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n19_), .c(new_n27_), .O(new_n36_));
  oai21  g21(.a(new_n34_), .b(new_n23_), .c(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n35_), .b(x1), .O(new_n39_));
  nand2  g24(.a(new_n25_), .b(x2), .O(new_n40_));
  nand2  g25(.a(new_n19_), .b(x0), .O(new_n41_));
  aoi21  g26(.a(new_n27_), .b(new_n16_), .c(x1), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n39_), .c(new_n38_), .O(z2));
  nor2   g29(.a(new_n35_), .b(x2), .O(new_n45_));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n45_), .c(x1), .d(new_n16_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n16_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n19_), .c(x2), .d(new_n23_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  xor2a  g35(.a(x3), .b(x2), .O(new_n51_));
  oai21  g36(.a(x5), .b(x3), .c(x0), .O(new_n52_));
  aoi22  g37(.a(new_n52_), .b(x2), .c(new_n51_), .d(x0), .O(new_n53_));
  xnor2a g38(.a(x3), .b(x2), .O(new_n54_));
  oai21  g39(.a(x2), .b(x0), .c(x6), .O(new_n55_));
  aoi22  g40(.a(new_n55_), .b(x1), .c(new_n54_), .d(new_n16_), .O(new_n56_));
  oai21  g41(.a(new_n53_), .b(x1), .c(new_n56_), .O(z4));
  nand2  g42(.a(new_n51_), .b(x0), .O(new_n58_));
  nand3  g43(.a(new_n25_), .b(x2), .c(new_n16_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n23_), .O(new_n61_));
  nand2  g46(.a(new_n19_), .b(x2), .O(new_n62_));
  nand4  g47(.a(new_n62_), .b(new_n28_), .c(x6), .d(new_n16_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(z5));
  nor2   g49(.a(x5), .b(x3), .O(new_n65_));
  aoi22  g50(.a(new_n51_), .b(x0), .c(new_n65_), .d(x2), .O(new_n66_));
  inv1   g51(.a(x4), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n27_), .c(x3), .O(new_n68_));
  oai22  g53(.a(new_n68_), .b(new_n17_), .c(new_n66_), .d(x1), .O(z6));
endmodule


