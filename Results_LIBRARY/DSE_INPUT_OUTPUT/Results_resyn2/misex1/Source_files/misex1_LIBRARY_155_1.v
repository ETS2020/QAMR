// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:38 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n43_;
  inv1   g00(.a(x0), .O(new_n16_));
  nor2   g01(.a(x1), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  nor2   g03(.a(x3), .b(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(new_n20_), .O(z0));
  nand3  g06(.a(x3), .b(new_n18_), .c(x0), .O(new_n22_));
  inv1   g07(.a(x3), .O(new_n23_));
  inv1   g08(.a(x5), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n23_), .c(x2), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(new_n18_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(new_n25_), .c(new_n16_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n22_), .c(x1), .O(z1));
  inv1   g14(.a(x1), .O(new_n30_));
  nand2  g15(.a(new_n25_), .b(new_n30_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n16_), .O(new_n32_));
  oai21  g17(.a(x6), .b(x0), .c(new_n23_), .O(new_n33_));
  oai21  g18(.a(x1), .b(new_n16_), .c(x3), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n33_), .c(new_n18_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n32_), .O(z2));
  nand2  g21(.a(new_n32_), .b(new_n20_), .O(z3));
  nand3  g22(.a(x3), .b(new_n18_), .c(new_n30_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  xor2a  g24(.a(x3), .b(x2), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n17_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z4));
  nand2  g27(.a(new_n40_), .b(x0), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n28_), .c(x1), .O(z5));
  nand2  g29(.a(new_n41_), .b(new_n32_), .O(z6));
endmodule


