// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x4), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x3), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  oai21  g07(.a(x1), .b(new_n22_), .c(new_n18_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n17_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n19_), .c(x0), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n16_), .c(x3), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n18_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n19_), .c(x2), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nand2  g17(.a(x2), .b(new_n19_), .O(new_n33_));
  aoi21  g18(.a(new_n32_), .b(new_n18_), .c(new_n33_), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(new_n31_), .c(new_n22_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n29_), .O(z1));
  nand2  g21(.a(new_n32_), .b(x2), .O(new_n37_));
  inv1   g22(.a(x6), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n26_), .O(new_n39_));
  nand2  g24(.a(new_n18_), .b(new_n19_), .O(new_n40_));
  aoi21  g25(.a(new_n39_), .b(new_n37_), .c(new_n40_), .O(new_n41_));
  nand2  g26(.a(x4), .b(x1), .O(new_n42_));
  aoi21  g27(.a(new_n18_), .b(x2), .c(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n41_), .c(new_n22_), .O(new_n44_));
  nand3  g29(.a(new_n28_), .b(x4), .c(x3), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z2));
  nand2  g31(.a(x7), .b(x4), .O(new_n47_));
  nor2   g32(.a(x2), .b(x0), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n47_), .c(new_n17_), .d(x1), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n22_), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n18_), .c(x2), .d(new_n19_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(z3));
  nor2   g37(.a(new_n48_), .b(x3), .O(new_n53_));
  nand3  g38(.a(x3), .b(new_n26_), .c(new_n19_), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n22_), .c(new_n16_), .O(new_n55_));
  oai21  g40(.a(new_n18_), .b(new_n22_), .c(new_n26_), .O(new_n56_));
  nand2  g41(.a(x3), .b(x2), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n19_), .O(new_n58_));
  oai21  g43(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(z4));
  inv1   g44(.a(new_n57_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n31_), .c(new_n22_), .O(new_n61_));
  nand3  g46(.a(x5), .b(new_n18_), .c(x2), .O(new_n62_));
  aoi21  g47(.a(x4), .b(x3), .c(x2), .O(new_n63_));
  nand2  g48(.a(new_n57_), .b(x0), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n19_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n61_), .c(new_n17_), .O(z5));
  nand3  g52(.a(new_n32_), .b(new_n18_), .c(x2), .O(new_n68_));
  oai21  g53(.a(new_n64_), .b(new_n63_), .c(new_n68_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n19_), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n18_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n20_), .c(new_n17_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n70_), .O(z6));
endmodule


