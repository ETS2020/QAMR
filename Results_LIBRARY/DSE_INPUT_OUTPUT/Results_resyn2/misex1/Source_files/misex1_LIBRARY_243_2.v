// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:21 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nor2   g05(.a(new_n16_), .b(x0), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n17_), .O(z0));
  nor2   g08(.a(new_n18_), .b(x1), .O(new_n24_));
  oai21  g09(.a(x5), .b(x3), .c(new_n24_), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(new_n16_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n18_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n25_), .c(x0), .O(z1));
  inv1   g14(.a(x4), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x2), .c(new_n19_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n21_), .O(new_n32_));
  nand2  g17(.a(x6), .b(new_n18_), .O(new_n33_));
  aoi21  g18(.a(x5), .b(x2), .c(x3), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n33_), .c(x0), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x1), .c(new_n32_), .O(z2));
  inv1   g21(.a(x5), .O(new_n37_));
  nand3  g22(.a(new_n24_), .b(new_n37_), .c(new_n19_), .O(new_n38_));
  oai21  g23(.a(x4), .b(x3), .c(x7), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n18_), .c(x1), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(x0), .O(z3));
  oai21  g26(.a(new_n19_), .b(x1), .c(new_n18_), .O(new_n42_));
  nor2   g27(.a(new_n24_), .b(new_n20_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n42_), .c(x0), .O(z4));
  aoi21  g29(.a(new_n27_), .b(new_n18_), .c(new_n20_), .O(new_n45_));
  nor2   g30(.a(new_n37_), .b(new_n18_), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(x0), .c(new_n16_), .O(new_n47_));
  oai21  g32(.a(new_n45_), .b(x0), .c(new_n47_), .O(z5));
  nor2   g33(.a(x4), .b(x2), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(x3), .c(x1), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n38_), .c(x0), .O(z6));
endmodule


