// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x3), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  inv1   g04(.a(x4), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n21_));
  oai21  g06(.a(x1), .b(x0), .c(x2), .O(new_n22_));
  aoi21  g07(.a(new_n21_), .b(new_n17_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x3), .b(new_n24_), .c(new_n18_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n20_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x0), .O(new_n27_));
  nor2   g12(.a(x1), .b(x0), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n19_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n28_), .c(x2), .O(new_n31_));
  nor2   g16(.a(x4), .b(new_n16_), .O(new_n32_));
  nand3  g17(.a(x6), .b(new_n19_), .c(new_n16_), .O(new_n33_));
  oai21  g18(.a(new_n32_), .b(new_n18_), .c(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n24_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n31_), .c(new_n27_), .O(z1));
  oai21  g21(.a(new_n20_), .b(x2), .c(new_n17_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x1), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nand2  g24(.a(x6), .b(new_n24_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n39_), .c(new_n28_), .d(new_n19_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n38_), .c(new_n27_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n24_), .c(x1), .O(new_n44_));
  nand4  g29(.a(new_n29_), .b(new_n19_), .c(x2), .d(new_n18_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n16_), .O(new_n47_));
  nand3  g32(.a(new_n19_), .b(x2), .c(new_n18_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n20_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(x0), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n47_), .O(z3));
  xor2a  g36(.a(x3), .b(x2), .O(new_n52_));
  aoi22  g37(.a(new_n52_), .b(new_n32_), .c(x2), .d(new_n16_), .O(new_n53_));
  aoi21  g38(.a(new_n19_), .b(x2), .c(x0), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n25_), .O(new_n55_));
  oai21  g40(.a(new_n53_), .b(x1), .c(new_n55_), .O(z4));
  nor2   g41(.a(new_n39_), .b(x0), .O(new_n57_));
  aoi21  g42(.a(new_n52_), .b(new_n32_), .c(new_n57_), .O(new_n58_));
  nand2  g43(.a(x6), .b(new_n19_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n24_), .c(new_n18_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n54_), .O(new_n61_));
  oai21  g46(.a(new_n58_), .b(x1), .c(new_n61_), .O(z5));
  nor2   g47(.a(x4), .b(x2), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(x3), .c(x1), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n45_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n16_), .O(new_n66_));
  nand2  g51(.a(new_n52_), .b(new_n18_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n20_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(x0), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z6));
endmodule


