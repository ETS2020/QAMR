// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(new_n17_), .c(new_n16_), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand2  g05(.a(x2), .b(new_n20_), .O(new_n21_));
  nand2  g06(.a(x3), .b(x1), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(x2), .c(new_n20_), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  aoi21  g09(.a(new_n21_), .b(new_n19_), .c(new_n24_), .O(z0));
  nor2   g10(.a(new_n17_), .b(x1), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n18_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g14(.a(new_n17_), .b(x1), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n29_), .c(x0), .O(z1));
  inv1   g16(.a(x4), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x2), .c(new_n18_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x1), .O(new_n34_));
  nand3  g19(.a(new_n26_), .b(new_n27_), .c(new_n18_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(x0), .O(z2));
  oai21  g21(.a(x4), .b(x3), .c(x7), .O(new_n37_));
  nand4  g22(.a(new_n37_), .b(new_n17_), .c(x1), .d(new_n20_), .O(new_n38_));
  nand2  g23(.a(x5), .b(new_n20_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n26_), .c(new_n18_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(z3));
  oai21  g26(.a(x3), .b(new_n17_), .c(new_n20_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n19_), .O(z4));
  aoi21  g28(.a(new_n26_), .b(new_n18_), .c(new_n20_), .O(new_n44_));
  oai21  g29(.a(x3), .b(new_n17_), .c(x1), .O(new_n45_));
  aoi21  g30(.a(new_n28_), .b(new_n26_), .c(x0), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(z5));
  aoi21  g32(.a(new_n39_), .b(new_n18_), .c(new_n17_), .O(new_n48_));
  oai21  g33(.a(x4), .b(x2), .c(new_n22_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n20_), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(x1), .c(new_n50_), .O(z6));
endmodule


