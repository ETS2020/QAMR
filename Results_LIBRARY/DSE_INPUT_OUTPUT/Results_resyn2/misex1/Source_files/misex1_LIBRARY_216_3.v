// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:10 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x0), .O(new_n19_));
  aoi21  g04(.a(new_n19_), .b(new_n17_), .c(new_n16_), .O(z0));
  inv1   g05(.a(x6), .O(new_n21_));
  oai21  g06(.a(new_n21_), .b(x3), .c(new_n18_), .O(new_n22_));
  nor2   g07(.a(x2), .b(x0), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g09(.a(x0), .O(new_n25_));
  oai21  g10(.a(new_n17_), .b(new_n25_), .c(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n18_), .O(new_n27_));
  nand2  g12(.a(x5), .b(new_n25_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n17_), .O(new_n29_));
  nand2  g14(.a(x3), .b(x2), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n27_), .c(new_n24_), .O(z1));
  oai21  g17(.a(x4), .b(x3), .c(x1), .O(new_n33_));
  nand3  g18(.a(new_n21_), .b(new_n17_), .c(new_n18_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n33_), .c(x2), .O(new_n35_));
  inv1   g20(.a(x5), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n17_), .c(x2), .d(new_n18_), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n35_), .c(new_n25_), .O(new_n39_));
  nand2  g24(.a(x3), .b(new_n16_), .O(new_n40_));
  or2    g25(.a(new_n40_), .b(new_n19_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand2  g28(.a(new_n23_), .b(x1), .O(new_n44_));
  inv1   g29(.a(new_n44_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  nand4  g31(.a(new_n28_), .b(new_n17_), .c(x2), .d(new_n18_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(z3));
  oai21  g33(.a(new_n17_), .b(x1), .c(new_n23_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n30_), .c(new_n27_), .O(z4));
  nand2  g35(.a(new_n17_), .b(x2), .O(new_n51_));
  aoi21  g36(.a(new_n40_), .b(new_n51_), .c(new_n25_), .O(new_n52_));
  nand3  g37(.a(x5), .b(new_n17_), .c(x2), .O(new_n53_));
  inv1   g38(.a(new_n53_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n52_), .c(new_n18_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n24_), .O(z5));
  nand3  g41(.a(new_n36_), .b(new_n17_), .c(x2), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n52_), .b(new_n58_), .c(new_n18_), .O(new_n59_));
  nand2  g44(.a(x4), .b(new_n17_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n45_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n59_), .O(z6));
endmodule


