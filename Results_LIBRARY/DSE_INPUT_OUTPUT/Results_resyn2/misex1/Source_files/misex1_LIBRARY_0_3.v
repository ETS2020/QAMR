// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_;
  nand2  g00(.a(x3), .b(x1), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x2), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g06(.a(x0), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  nand2  g08(.a(x3), .b(new_n23_), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand4  g11(.a(new_n26_), .b(new_n24_), .c(new_n20_), .d(new_n22_), .O(new_n27_));
  inv1   g12(.a(new_n24_), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nor2   g14(.a(new_n29_), .b(x0), .O(new_n30_));
  nor2   g15(.a(x2), .b(x0), .O(new_n31_));
  nor2   g16(.a(new_n31_), .b(x1), .O(new_n32_));
  oai21  g17(.a(new_n30_), .b(new_n28_), .c(new_n32_), .O(new_n33_));
  oai21  g18(.a(new_n31_), .b(x3), .c(x1), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n33_), .c(new_n27_), .O(z1));
  nand2  g20(.a(new_n29_), .b(x2), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n26_), .c(x1), .O(new_n37_));
  nand3  g22(.a(x4), .b(new_n23_), .c(x1), .O(new_n38_));
  inv1   g23(.a(new_n38_), .O(new_n39_));
  nor2   g24(.a(x3), .b(x0), .O(new_n40_));
  oai21  g25(.a(new_n39_), .b(new_n37_), .c(new_n40_), .O(new_n41_));
  nand3  g26(.a(new_n28_), .b(new_n17_), .c(x0), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n41_), .O(z2));
  nand2  g28(.a(x7), .b(x4), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n31_), .c(x1), .O(new_n45_));
  nand2  g30(.a(x2), .b(new_n17_), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(new_n30_), .c(new_n45_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n19_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n16_), .O(z3));
  oai21  g34(.a(x3), .b(x2), .c(new_n46_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n22_), .O(new_n51_));
  oai21  g36(.a(new_n24_), .b(new_n22_), .c(new_n20_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n17_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n51_), .O(z4));
  nand2  g39(.a(x5), .b(new_n17_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n19_), .c(x2), .O(new_n56_));
  nand4  g41(.a(new_n56_), .b(new_n26_), .c(new_n24_), .d(new_n22_), .O(new_n57_));
  oai21  g42(.a(new_n20_), .b(x1), .c(new_n24_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(x0), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n57_), .c(new_n34_), .O(z5));
  nor2   g45(.a(x4), .b(x3), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n31_), .c(x1), .O(new_n62_));
  oai21  g47(.a(new_n53_), .b(new_n30_), .c(new_n62_), .O(z6));
endmodule


