// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x5), .O(new_n23_));
  nand2  g08(.a(x2), .b(new_n16_), .O(new_n24_));
  aoi21  g09(.a(new_n23_), .b(new_n19_), .c(new_n24_), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n19_), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x7), .b(new_n27_), .O(new_n28_));
  aoi21  g13(.a(new_n26_), .b(new_n16_), .c(new_n28_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n25_), .c(new_n18_), .O(new_n30_));
  nand4  g15(.a(x7), .b(x3), .c(new_n27_), .d(x0), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n16_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n30_), .O(z1));
  nand2  g19(.a(new_n23_), .b(x2), .O(new_n35_));
  inv1   g20(.a(x6), .O(new_n36_));
  nand3  g21(.a(x7), .b(new_n36_), .c(new_n27_), .O(new_n37_));
  nand2  g22(.a(new_n19_), .b(new_n16_), .O(new_n38_));
  aoi21  g23(.a(new_n37_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g24(.a(x7), .b(x2), .c(x3), .O(new_n40_));
  nand3  g25(.a(x7), .b(x4), .c(new_n27_), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n40_), .c(new_n16_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n39_), .c(new_n18_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n33_), .O(z2));
  nand2  g29(.a(x5), .b(new_n18_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(x2), .c(new_n16_), .O(new_n46_));
  nor2   g31(.a(new_n16_), .b(x0), .O(new_n47_));
  inv1   g32(.a(x4), .O(new_n48_));
  nand3  g33(.a(x7), .b(new_n48_), .c(new_n27_), .O(new_n49_));
  inv1   g34(.a(new_n49_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n46_), .c(x3), .O(z3));
  nand2  g37(.a(new_n19_), .b(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n31_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n16_), .O(new_n55_));
  inv1   g40(.a(new_n28_), .O(new_n56_));
  nor2   g41(.a(new_n19_), .b(new_n27_), .O(new_n57_));
  nand2  g42(.a(x3), .b(new_n16_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n56_), .c(new_n57_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(x0), .c(new_n55_), .O(z4));
  nand3  g45(.a(x7), .b(x3), .c(new_n27_), .O(new_n61_));
  aoi21  g46(.a(new_n53_), .b(new_n61_), .c(new_n18_), .O(new_n62_));
  nand3  g47(.a(x5), .b(x2), .c(new_n18_), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n62_), .c(new_n16_), .O(new_n65_));
  oai21  g50(.a(new_n57_), .b(new_n29_), .c(new_n18_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n65_), .O(z5));
  nand3  g52(.a(new_n23_), .b(new_n19_), .c(x2), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n62_), .c(new_n16_), .O(new_n70_));
  nand2  g55(.a(new_n49_), .b(new_n40_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n47_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n70_), .O(z6));
endmodule


