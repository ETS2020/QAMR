// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_,
    new_n50_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  oai21  g02(.a(x3), .b(new_n17_), .c(x2), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n16_), .O(new_n19_));
  inv1   g04(.a(x2), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nor2   g07(.a(new_n16_), .b(x0), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n19_), .O(z0));
  nor3   g10(.a(x5), .b(x3), .c(x1), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  xor2a  g12(.a(x2), .b(x1), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n27_), .c(new_n17_), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(z1));
  inv1   g15(.a(x4), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x2), .c(new_n21_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(x1), .c(new_n26_), .O(new_n33_));
  nand2  g18(.a(new_n20_), .b(new_n16_), .O(new_n34_));
  oai21  g19(.a(new_n33_), .b(x0), .c(new_n34_), .O(z2));
  oai21  g20(.a(x4), .b(x3), .c(x7), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n17_), .c(new_n16_), .O(new_n37_));
  nand2  g22(.a(x5), .b(new_n17_), .O(new_n38_));
  nand3  g23(.a(new_n21_), .b(x2), .c(new_n16_), .O(new_n39_));
  inv1   g24(.a(new_n39_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  oai21  g26(.a(new_n37_), .b(x2), .c(new_n41_), .O(z3));
  nor2   g27(.a(x2), .b(new_n16_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n22_), .c(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n39_), .O(z4));
  inv1   g30(.a(new_n18_), .O(new_n46_));
  aoi21  g31(.a(x5), .b(new_n16_), .c(x3), .O(new_n47_));
  aoi22  g32(.a(new_n47_), .b(new_n46_), .c(new_n19_), .d(x0), .O(z5));
  oai21  g33(.a(x4), .b(x2), .c(new_n21_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n23_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n41_), .O(z6));
endmodule


