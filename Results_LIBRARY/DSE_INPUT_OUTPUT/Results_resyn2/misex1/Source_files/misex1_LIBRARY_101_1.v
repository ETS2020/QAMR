// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:13 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x5), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x3), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  nor2   g07(.a(x1), .b(new_n22_), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n21_), .c(new_n16_), .O(new_n24_));
  aoi22  g09(.a(new_n24_), .b(new_n20_), .c(new_n17_), .d(new_n16_), .O(z0));
  nand2  g10(.a(x3), .b(x2), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n17_), .c(x0), .O(new_n27_));
  nand3  g12(.a(x3), .b(new_n16_), .c(x0), .O(new_n28_));
  inv1   g13(.a(new_n28_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n27_), .c(new_n18_), .O(new_n30_));
  inv1   g15(.a(x6), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x3), .c(new_n18_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n22_), .c(x5), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x2), .c(new_n30_), .O(z1));
  nand2  g19(.a(x4), .b(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n21_), .c(new_n18_), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n21_), .c(new_n18_), .O(new_n38_));
  aoi21  g23(.a(x6), .b(new_n16_), .c(new_n38_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n36_), .c(new_n22_), .O(new_n40_));
  nand2  g25(.a(new_n23_), .b(x3), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n17_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n40_), .O(z2));
  nor2   g29(.a(x3), .b(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n16_), .c(x1), .d(new_n22_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n17_), .O(new_n50_));
  nand2  g35(.a(new_n45_), .b(new_n23_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(z3));
  oai21  g37(.a(new_n45_), .b(new_n29_), .c(new_n18_), .O(new_n53_));
  nand2  g38(.a(new_n21_), .b(x2), .O(new_n54_));
  nand3  g39(.a(x3), .b(new_n16_), .c(new_n18_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n22_), .O(new_n56_));
  nand2  g41(.a(x5), .b(new_n16_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n53_), .O(z4));
  nand3  g43(.a(new_n17_), .b(x3), .c(new_n16_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n54_), .c(new_n22_), .O(new_n60_));
  nor2   g45(.a(new_n37_), .b(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n60_), .c(new_n18_), .O(new_n62_));
  nand2  g47(.a(new_n17_), .b(new_n16_), .O(new_n63_));
  aoi21  g48(.a(x6), .b(new_n21_), .c(x1), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n63_), .c(new_n26_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n22_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n62_), .O(z5));
  nand3  g52(.a(new_n17_), .b(new_n21_), .c(x2), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n60_), .c(new_n18_), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n21_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n19_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n70_), .c(new_n57_), .O(z6));
endmodule


