// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(x0), .c(x5), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  nor2   g07(.a(x2), .b(x0), .O(new_n23_));
  inv1   g08(.a(x6), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(x3), .c(new_n17_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  inv1   g11(.a(new_n23_), .O(new_n27_));
  nand2  g12(.a(x2), .b(x0), .O(new_n28_));
  nand4  g13(.a(new_n28_), .b(new_n27_), .c(x3), .d(new_n17_), .O(new_n29_));
  nand2  g14(.a(x5), .b(x2), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n26_), .O(z1));
  inv1   g16(.a(x3), .O(new_n32_));
  inv1   g17(.a(x4), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x2), .c(new_n32_), .O(new_n34_));
  nand2  g19(.a(x6), .b(new_n16_), .O(new_n35_));
  aoi22  g20(.a(new_n35_), .b(new_n20_), .c(new_n34_), .d(x1), .O(new_n36_));
  inv1   g21(.a(x0), .O(new_n37_));
  nor2   g22(.a(x1), .b(new_n37_), .O(new_n38_));
  nor2   g23(.a(new_n32_), .b(x2), .O(new_n39_));
  aoi22  g24(.a(new_n39_), .b(new_n38_), .c(x5), .d(x2), .O(new_n40_));
  oai21  g25(.a(new_n36_), .b(x0), .c(new_n40_), .O(z2));
  inv1   g26(.a(x5), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n32_), .c(x2), .O(new_n43_));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n23_), .c(x1), .O(new_n45_));
  oai21  g30(.a(new_n43_), .b(x1), .c(new_n45_), .O(z3));
  inv1   g31(.a(new_n43_), .O(new_n47_));
  nand3  g32(.a(x3), .b(new_n16_), .c(x0), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n47_), .c(new_n17_), .O(new_n50_));
  aoi21  g35(.a(x3), .b(new_n17_), .c(x2), .O(new_n51_));
  nand2  g36(.a(x3), .b(x2), .O(new_n52_));
  nor2   g37(.a(new_n52_), .b(x5), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n37_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n50_), .O(z4));
  oai21  g40(.a(new_n47_), .b(new_n39_), .c(new_n38_), .O(new_n56_));
  aoi21  g41(.a(new_n25_), .b(new_n16_), .c(new_n53_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(x0), .c(new_n56_), .O(z5));
  nor2   g43(.a(x3), .b(new_n16_), .O(new_n59_));
  oai21  g44(.a(new_n49_), .b(new_n59_), .c(new_n17_), .O(new_n60_));
  oai21  g45(.a(x4), .b(x2), .c(new_n32_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n18_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n60_), .c(new_n30_), .O(z6));
endmodule


