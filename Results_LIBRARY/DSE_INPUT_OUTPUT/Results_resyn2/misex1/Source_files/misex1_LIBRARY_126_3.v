// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x1), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  inv1   g05(.a(x5), .O(new_n21_));
  inv1   g06(.a(x6), .O(new_n22_));
  nor2   g07(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  aoi21  g10(.a(new_n19_), .b(new_n16_), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  aoi21  g12(.a(new_n20_), .b(new_n17_), .c(new_n27_), .O(z0));
  nand3  g13(.a(x6), .b(new_n19_), .c(new_n16_), .O(new_n29_));
  nand3  g14(.a(x3), .b(new_n18_), .c(x0), .O(new_n30_));
  nand4  g15(.a(new_n30_), .b(new_n29_), .c(new_n17_), .d(new_n25_), .O(new_n31_));
  nor2   g16(.a(x5), .b(x3), .O(new_n32_));
  nand2  g17(.a(new_n18_), .b(new_n16_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n32_), .c(x2), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n31_), .c(new_n24_), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(z1));
  nand2  g21(.a(x4), .b(new_n25_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n19_), .c(new_n18_), .O(new_n38_));
  nand2  g23(.a(new_n21_), .b(x2), .O(new_n39_));
  nand2  g24(.a(new_n22_), .b(new_n25_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n39_), .c(new_n20_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n38_), .c(new_n16_), .O(new_n42_));
  inv1   g27(.a(new_n30_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n25_), .c(new_n23_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(z2));
  inv1   g30(.a(new_n17_), .O(new_n46_));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n46_), .c(new_n25_), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n16_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n19_), .c(x2), .d(new_n18_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n48_), .c(new_n24_), .O(z3));
  nand2  g36(.a(new_n19_), .b(x2), .O(new_n52_));
  nand2  g37(.a(x3), .b(new_n25_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n16_), .c(new_n52_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n18_), .O(new_n55_));
  aoi21  g40(.a(new_n19_), .b(x2), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n53_), .b(x1), .c(new_n56_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n55_), .c(new_n24_), .O(z4));
  xnor2a g43(.a(x3), .b(x2), .O(new_n59_));
  nand3  g44(.a(x5), .b(new_n19_), .c(x2), .O(new_n60_));
  oai21  g45(.a(new_n59_), .b(new_n16_), .c(new_n60_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n18_), .O(new_n62_));
  oai21  g47(.a(new_n22_), .b(x3), .c(new_n25_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(x1), .c(new_n56_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n62_), .c(new_n24_), .O(z5));
  oai22  g50(.a(new_n59_), .b(new_n16_), .c(new_n39_), .d(x3), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n18_), .O(new_n67_));
  oai21  g52(.a(x4), .b(x2), .c(new_n19_), .O(new_n68_));
  aoi21  g53(.a(new_n68_), .b(new_n46_), .c(new_n23_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n67_), .O(z6));
endmodule


