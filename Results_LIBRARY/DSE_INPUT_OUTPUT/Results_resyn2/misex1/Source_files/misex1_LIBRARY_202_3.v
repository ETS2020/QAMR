// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  nor2   g03(.a(x3), .b(x1), .O(new_n19_));
  aoi21  g04(.a(new_n19_), .b(x0), .c(x4), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g06(.a(x1), .O(new_n22_));
  nand3  g07(.a(x3), .b(new_n16_), .c(x0), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  aoi22  g09(.a(new_n24_), .b(new_n22_), .c(x4), .d(x2), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(x2), .c(new_n22_), .O(new_n29_));
  nand3  g14(.a(x6), .b(new_n26_), .c(new_n16_), .O(new_n30_));
  oai21  g15(.a(x4), .b(new_n16_), .c(x1), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n17_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n25_), .O(z1));
  inv1   g19(.a(x4), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n26_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(x1), .O(new_n37_));
  nand2  g22(.a(x5), .b(x2), .O(new_n38_));
  nand2  g23(.a(x6), .b(new_n16_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(new_n19_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n37_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n17_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n25_), .O(z2));
  nand2  g28(.a(new_n36_), .b(x7), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n45_));
  nor2   g30(.a(new_n27_), .b(x0), .O(new_n46_));
  nand3  g31(.a(new_n19_), .b(new_n35_), .c(x2), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(z3));
  nor2   g33(.a(x3), .b(new_n16_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n24_), .c(new_n22_), .O(new_n50_));
  aoi21  g35(.a(x3), .b(new_n22_), .c(x0), .O(new_n51_));
  aoi21  g36(.a(new_n26_), .b(x2), .c(x0), .O(new_n52_));
  oai22  g37(.a(new_n52_), .b(x4), .c(new_n51_), .d(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n50_), .O(z4));
  nand2  g39(.a(x3), .b(new_n16_), .O(new_n55_));
  nand3  g40(.a(new_n35_), .b(new_n26_), .c(x2), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n55_), .c(new_n17_), .O(new_n57_));
  nand4  g42(.a(x5), .b(new_n35_), .c(new_n26_), .d(x2), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n57_), .c(new_n22_), .O(new_n60_));
  nand2  g45(.a(new_n16_), .b(x1), .O(new_n61_));
  nand3  g46(.a(new_n35_), .b(x3), .c(x2), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n61_), .c(new_n30_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n17_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n60_), .O(z5));
  oai21  g50(.a(new_n56_), .b(new_n46_), .c(new_n23_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n22_), .O(new_n67_));
  nand2  g52(.a(new_n55_), .b(x4), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n52_), .c(x1), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n67_), .O(z6));
endmodule


