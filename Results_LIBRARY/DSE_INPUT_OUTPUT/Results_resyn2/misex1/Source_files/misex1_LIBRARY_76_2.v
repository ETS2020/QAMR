// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x1), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x0), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x3), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n16_), .c(x0), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  inv1   g06(.a(x5), .O(new_n22_));
  nor2   g07(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  aoi21  g09(.a(new_n20_), .b(new_n18_), .c(new_n24_), .O(z0));
  inv1   g10(.a(x0), .O(new_n26_));
  nand3  g11(.a(x3), .b(new_n21_), .c(x0), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  aoi21  g13(.a(new_n23_), .b(new_n26_), .c(new_n28_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n16_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n21_), .c(new_n26_), .O(new_n32_));
  oai21  g17(.a(new_n29_), .b(x1), .c(new_n32_), .O(z1));
  aoi21  g18(.a(x4), .b(new_n21_), .c(x3), .O(new_n34_));
  nand4  g19(.a(new_n30_), .b(new_n19_), .c(new_n21_), .d(new_n16_), .O(new_n35_));
  oai21  g20(.a(new_n34_), .b(new_n16_), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n26_), .O(new_n37_));
  nor2   g22(.a(x1), .b(new_n26_), .O(new_n38_));
  nor2   g23(.a(new_n19_), .b(x2), .O(new_n39_));
  aoi22  g24(.a(new_n39_), .b(new_n38_), .c(new_n22_), .d(x2), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n37_), .O(z2));
  nand2  g26(.a(new_n20_), .b(x5), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(x2), .O(new_n43_));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n17_), .c(new_n21_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z3));
  nor2   g31(.a(x3), .b(new_n21_), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n28_), .c(new_n16_), .O(new_n48_));
  aoi21  g33(.a(x3), .b(new_n16_), .c(x0), .O(new_n49_));
  aoi21  g34(.a(new_n19_), .b(x2), .c(x0), .O(new_n50_));
  oai22  g35(.a(new_n50_), .b(new_n22_), .c(new_n49_), .d(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n48_), .O(z4));
  nand3  g37(.a(x5), .b(new_n19_), .c(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n27_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n16_), .O(new_n55_));
  aoi22  g40(.a(new_n31_), .b(new_n21_), .c(new_n23_), .d(x3), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(x0), .c(new_n55_), .O(z5));
  nand2  g42(.a(new_n22_), .b(x2), .O(new_n58_));
  oai21  g43(.a(new_n47_), .b(new_n39_), .c(new_n38_), .O(new_n59_));
  oai21  g44(.a(x4), .b(x2), .c(new_n19_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n17_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n59_), .c(new_n58_), .O(z6));
endmodule


