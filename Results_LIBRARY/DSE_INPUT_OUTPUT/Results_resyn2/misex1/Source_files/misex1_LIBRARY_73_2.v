// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand4  g03(.a(x3), .b(x2), .c(x1), .d(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(z0));
  inv1   g05(.a(x2), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x1), .O(new_n22_));
  oai21  g07(.a(x5), .b(x3), .c(new_n22_), .O(new_n23_));
  inv1   g08(.a(x6), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(x3), .c(new_n16_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n21_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n23_), .c(x0), .O(z1));
  inv1   g12(.a(x3), .O(new_n28_));
  inv1   g13(.a(x4), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x2), .c(new_n28_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x1), .O(new_n31_));
  nand2  g16(.a(x5), .b(x2), .O(new_n32_));
  nor2   g17(.a(x3), .b(x1), .O(new_n33_));
  nand2  g18(.a(x6), .b(new_n21_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n31_), .c(x0), .O(z2));
  oai21  g21(.a(x4), .b(x3), .c(x7), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n21_), .c(x1), .O(new_n38_));
  inv1   g23(.a(x5), .O(new_n39_));
  nand4  g24(.a(new_n39_), .b(new_n28_), .c(x2), .d(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n18_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n17_), .O(z3));
  aoi21  g28(.a(x3), .b(x1), .c(new_n22_), .O(new_n44_));
  aoi21  g29(.a(new_n21_), .b(x1), .c(new_n33_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n44_), .c(x0), .O(z4));
  nor2   g31(.a(new_n28_), .b(new_n21_), .O(new_n47_));
  aoi21  g32(.a(new_n25_), .b(new_n21_), .c(new_n47_), .O(new_n48_));
  nand2  g33(.a(new_n32_), .b(new_n18_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n16_), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(x0), .c(new_n50_), .O(z5));
  nor2   g36(.a(x4), .b(x2), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(x3), .c(x1), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n40_), .c(x0), .O(z6));
endmodule


