// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  nand2  g04(.a(x3), .b(new_n19_), .O(new_n20_));
  oai21  g05(.a(x1), .b(x0), .c(x2), .O(new_n21_));
  aoi21  g06(.a(new_n20_), .b(new_n18_), .c(new_n21_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  nand4  g08(.a(x3), .b(new_n23_), .c(new_n16_), .d(x0), .O(new_n24_));
  nor2   g09(.a(x5), .b(x3), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x1), .c(x2), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n17_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(new_n23_), .c(new_n16_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n26_), .c(new_n19_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n24_), .O(z1));
  nand2  g15(.a(x4), .b(new_n23_), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n17_), .c(new_n16_), .O(new_n32_));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x2), .O(new_n34_));
  inv1   g19(.a(x6), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n23_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n34_), .c(new_n18_), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n32_), .c(new_n19_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n24_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n23_), .c(x1), .O(new_n41_));
  oai21  g26(.a(x5), .b(x1), .c(new_n19_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n17_), .c(x2), .O(new_n43_));
  nand2  g28(.a(x1), .b(x0), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z3));
  nand2  g30(.a(x3), .b(new_n23_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n16_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x0), .O(new_n48_));
  nand3  g33(.a(new_n46_), .b(new_n16_), .c(new_n19_), .O(new_n49_));
  oai21  g34(.a(x3), .b(new_n23_), .c(x1), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n43_), .O(z4));
  xnor2a g36(.a(x2), .b(x0), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n36_), .c(new_n17_), .O(new_n53_));
  oai21  g38(.a(new_n33_), .b(x1), .c(new_n17_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(x2), .c(new_n19_), .O(new_n55_));
  nand4  g40(.a(new_n55_), .b(new_n53_), .c(new_n50_), .d(new_n48_), .O(z5));
  nor2   g41(.a(x4), .b(x2), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(x3), .c(x1), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n48_), .c(new_n43_), .O(z6));
endmodule


