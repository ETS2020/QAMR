// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x4), .O(new_n17_));
  nand3  g02(.a(new_n17_), .b(x1), .c(new_n16_), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  oai21  g05(.a(x1), .b(new_n16_), .c(new_n20_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n19_), .c(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  oai21  g08(.a(x5), .b(x3), .c(x2), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n20_), .c(new_n25_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n24_), .c(x0), .O(new_n27_));
  nand2  g12(.a(new_n26_), .b(x1), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  inv1   g14(.a(x1), .O(new_n30_));
  nand2  g15(.a(x3), .b(new_n25_), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n30_), .c(x0), .O(new_n33_));
  nor2   g18(.a(x2), .b(x0), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x4), .c(x1), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n33_), .c(new_n29_), .O(z1));
  oai21  g21(.a(new_n20_), .b(x0), .c(new_n17_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x1), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  aoi21  g24(.a(x6), .b(new_n25_), .c(x0), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n39_), .c(new_n32_), .O(new_n41_));
  oai21  g26(.a(new_n20_), .b(x0), .c(new_n30_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n41_), .c(new_n38_), .O(z2));
  nand2  g28(.a(x7), .b(x3), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n34_), .c(x4), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n16_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n20_), .c(x2), .d(new_n30_), .O(new_n47_));
  oai21  g32(.a(new_n45_), .b(new_n30_), .c(new_n47_), .O(z3));
  nand2  g33(.a(new_n20_), .b(x2), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(new_n31_), .c(new_n16_), .O(new_n50_));
  inv1   g35(.a(x5), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n20_), .c(x2), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n50_), .c(new_n30_), .O(new_n54_));
  nand3  g39(.a(new_n31_), .b(new_n30_), .c(new_n16_), .O(new_n55_));
  aoi21  g40(.a(new_n20_), .b(x2), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(x4), .c(x1), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z4));
  oai21  g43(.a(new_n50_), .b(new_n27_), .c(new_n30_), .O(new_n59_));
  inv1   g44(.a(new_n18_), .O(new_n60_));
  nand2  g45(.a(new_n49_), .b(new_n60_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n59_), .O(z5));
  nand2  g47(.a(new_n61_), .b(new_n54_), .O(z6));
endmodule


