// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(x5), .c(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x5), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x0), .c(x2), .O(new_n24_));
  nand2  g09(.a(new_n20_), .b(new_n16_), .O(new_n25_));
  nor2   g10(.a(x2), .b(x0), .O(new_n26_));
  nor2   g11(.a(new_n26_), .b(x1), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(new_n25_), .c(new_n24_), .O(new_n28_));
  nand2  g13(.a(new_n23_), .b(x3), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n20_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n17_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n26_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n29_), .c(new_n28_), .O(z1));
  nor2   g18(.a(x2), .b(x1), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(x0), .c(new_n23_), .O(new_n35_));
  inv1   g20(.a(x0), .O(new_n36_));
  inv1   g21(.a(x6), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n20_), .c(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n23_), .b(x2), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(new_n17_), .O(new_n40_));
  nand2  g25(.a(x4), .b(new_n16_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n20_), .c(x1), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n40_), .c(new_n36_), .O(new_n43_));
  oai21  g28(.a(new_n35_), .b(new_n20_), .c(new_n43_), .O(z2));
  nand2  g29(.a(x7), .b(x4), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n16_), .c(x1), .d(new_n36_), .O(new_n46_));
  oai21  g31(.a(new_n24_), .b(x1), .c(new_n46_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n20_), .O(new_n48_));
  nor2   g33(.a(x7), .b(new_n23_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n18_), .c(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(z3));
  nand2  g36(.a(new_n34_), .b(x3), .O(new_n52_));
  aoi21  g37(.a(new_n20_), .b(x2), .c(x0), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n52_), .c(new_n29_), .O(new_n54_));
  nand2  g39(.a(new_n20_), .b(x2), .O(new_n55_));
  nand3  g40(.a(x5), .b(x3), .c(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g42(.a(new_n53_), .b(x1), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n54_), .O(z4));
  aoi21  g45(.a(new_n56_), .b(new_n55_), .c(new_n36_), .O(new_n61_));
  nand3  g46(.a(x5), .b(new_n20_), .c(x2), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n61_), .c(new_n17_), .O(new_n64_));
  nand2  g49(.a(new_n34_), .b(new_n30_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n53_), .c(new_n29_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z5));
  inv1   g52(.a(new_n39_), .O(new_n68_));
  oai21  g53(.a(new_n61_), .b(new_n68_), .c(new_n17_), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n20_), .O(new_n70_));
  aoi22  g55(.a(new_n70_), .b(new_n18_), .c(new_n23_), .d(x3), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n69_), .O(z6));
endmodule


