// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  nor2   g04(.a(new_n17_), .b(x0), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(x6), .c(x3), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x0), .O(new_n23_));
  oai21  g08(.a(x5), .b(x3), .c(x2), .O(new_n24_));
  nor2   g09(.a(new_n18_), .b(x1), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n16_), .O(new_n26_));
  oai22  g11(.a(new_n26_), .b(new_n25_), .c(new_n24_), .d(x1), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n23_), .O(new_n28_));
  nand4  g13(.a(x3), .b(new_n16_), .c(new_n17_), .d(x0), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n28_), .O(z1));
  nand2  g15(.a(x4), .b(new_n16_), .O(new_n31_));
  nand2  g16(.a(x6), .b(x1), .O(new_n32_));
  aoi21  g17(.a(new_n31_), .b(new_n18_), .c(new_n32_), .O(new_n33_));
  nand2  g18(.a(x5), .b(x2), .O(new_n34_));
  nor2   g19(.a(x3), .b(x1), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n34_), .c(new_n26_), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n33_), .c(new_n23_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n29_), .O(z2));
  inv1   g24(.a(x6), .O(new_n40_));
  oai21  g25(.a(x4), .b(x3), .c(x7), .O(new_n41_));
  nor2   g26(.a(x2), .b(x0), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  nand2  g28(.a(x5), .b(new_n23_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n35_), .c(x2), .O(new_n45_));
  oai21  g30(.a(new_n43_), .b(new_n17_), .c(new_n45_), .O(z3));
  oai21  g31(.a(new_n18_), .b(x2), .c(new_n23_), .O(new_n47_));
  xor2a  g32(.a(x3), .b(x2), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(x0), .O(new_n49_));
  inv1   g34(.a(x5), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n18_), .c(x2), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n49_), .c(new_n47_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n17_), .O(new_n53_));
  nand2  g38(.a(new_n18_), .b(x2), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n20_), .c(x6), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(z4));
  oai21  g41(.a(new_n40_), .b(new_n18_), .c(x2), .O(new_n57_));
  oai21  g42(.a(new_n40_), .b(x3), .c(new_n17_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(new_n23_), .O(new_n59_));
  nor2   g44(.a(new_n24_), .b(x0), .O(new_n60_));
  nand2  g45(.a(new_n49_), .b(new_n17_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n60_), .c(new_n32_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n59_), .O(z5));
  and2   g48(.a(new_n51_), .b(new_n49_), .O(new_n64_));
  oai21  g49(.a(x4), .b(x2), .c(new_n18_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n20_), .c(x6), .O(new_n66_));
  oai21  g51(.a(new_n64_), .b(x1), .c(new_n66_), .O(z6));
endmodule


