// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x2), .O(new_n18_));
  aoi21  g03(.a(x3), .b(x2), .c(x0), .O(new_n19_));
  aoi21  g04(.a(new_n18_), .b(new_n16_), .c(new_n19_), .O(z0));
  inv1   g05(.a(x0), .O(new_n21_));
  nor2   g06(.a(x5), .b(x3), .O(new_n22_));
  nand2  g07(.a(x2), .b(new_n16_), .O(new_n23_));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x6), .b(new_n17_), .c(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n23_), .b(new_n22_), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n21_), .O(new_n27_));
  nor2   g12(.a(x2), .b(new_n16_), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n24_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n16_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(x0), .c(new_n28_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n27_), .O(z1));
  nand2  g17(.a(new_n30_), .b(x0), .O(new_n33_));
  nor2   g18(.a(x5), .b(new_n24_), .O(new_n34_));
  nor2   g19(.a(x6), .b(x2), .O(new_n35_));
  nor3   g20(.a(x3), .b(x1), .c(x0), .O(new_n36_));
  oai21  g21(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n17_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n37_), .c(new_n33_), .O(z2));
  inv1   g26(.a(x5), .O(new_n42_));
  nor2   g27(.a(new_n42_), .b(x0), .O(new_n43_));
  nand3  g28(.a(new_n17_), .b(x2), .c(new_n16_), .O(new_n44_));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n28_), .c(new_n21_), .O(new_n46_));
  oai21  g31(.a(new_n44_), .b(new_n43_), .c(new_n46_), .O(z3));
  nand2  g32(.a(new_n44_), .b(new_n29_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(x0), .O(new_n49_));
  nand2  g34(.a(x2), .b(new_n21_), .O(new_n50_));
  oai21  g35(.a(x3), .b(x0), .c(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g37(.a(new_n17_), .b(x1), .c(new_n24_), .O(new_n53_));
  oai21  g38(.a(new_n22_), .b(new_n21_), .c(new_n53_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(z4));
  nand2  g40(.a(new_n18_), .b(x1), .O(new_n56_));
  oai21  g41(.a(new_n42_), .b(x1), .c(new_n17_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(x2), .c(new_n21_), .O(new_n58_));
  nand2  g43(.a(x6), .b(new_n21_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n23_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n50_), .c(new_n17_), .O(new_n61_));
  nand4  g46(.a(new_n61_), .b(new_n58_), .c(new_n56_), .d(new_n33_), .O(z5));
  nor2   g47(.a(new_n34_), .b(x0), .O(new_n63_));
  nand2  g48(.a(new_n29_), .b(new_n18_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n16_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n17_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(x1), .c(new_n21_), .O(new_n67_));
  oai21  g52(.a(new_n65_), .b(new_n63_), .c(new_n67_), .O(z6));
endmodule


