// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:37 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x2), .O(new_n16_));
  nand2  g01(.a(x6), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  oai21  g07(.a(x1), .b(new_n22_), .c(new_n18_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n17_), .O(z0));
  inv1   g10(.a(x6), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n16_), .c(x1), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  nand2  g13(.a(x2), .b(new_n19_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n22_), .O(new_n31_));
  nor2   g16(.a(x6), .b(x2), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(x3), .c(x0), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n19_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n31_), .O(z1));
  nand2  g21(.a(new_n26_), .b(new_n16_), .O(new_n37_));
  nor2   g22(.a(x4), .b(x3), .O(new_n38_));
  nand2  g23(.a(x3), .b(x2), .O(new_n39_));
  oai21  g24(.a(new_n38_), .b(new_n37_), .c(new_n39_), .O(new_n40_));
  inv1   g25(.a(x5), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x2), .O(new_n42_));
  nand2  g27(.a(new_n18_), .b(new_n19_), .O(new_n43_));
  aoi21  g28(.a(new_n42_), .b(new_n37_), .c(new_n43_), .O(new_n44_));
  aoi21  g29(.a(new_n40_), .b(x1), .c(new_n44_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(x0), .c(new_n35_), .O(z2));
  nor2   g31(.a(new_n41_), .b(x0), .O(new_n47_));
  nand3  g32(.a(new_n18_), .b(x2), .c(new_n19_), .O(new_n48_));
  inv1   g33(.a(x7), .O(new_n49_));
  nor2   g34(.a(new_n38_), .b(new_n49_), .O(new_n50_));
  inv1   g35(.a(new_n27_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n22_), .O(new_n52_));
  oai22  g37(.a(new_n52_), .b(new_n50_), .c(new_n48_), .d(new_n47_), .O(z3));
  nand2  g38(.a(new_n18_), .b(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n33_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n19_), .O(new_n56_));
  inv1   g41(.a(new_n39_), .O(new_n57_));
  nand2  g42(.a(x3), .b(new_n19_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n32_), .c(new_n57_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(x0), .c(new_n56_), .O(z4));
  nand2  g45(.a(new_n32_), .b(x3), .O(new_n61_));
  aoi21  g46(.a(new_n54_), .b(new_n61_), .c(new_n22_), .O(new_n62_));
  nand3  g47(.a(x5), .b(x2), .c(new_n22_), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n62_), .c(new_n19_), .O(new_n65_));
  oai21  g50(.a(new_n57_), .b(new_n51_), .c(new_n22_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n65_), .O(z5));
  nand2  g52(.a(new_n28_), .b(x2), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n62_), .c(new_n19_), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n18_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n20_), .c(new_n17_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n70_), .O(z6));
endmodule


