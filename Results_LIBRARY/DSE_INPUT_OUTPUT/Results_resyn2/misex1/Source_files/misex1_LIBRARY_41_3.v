// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand3  g02(.a(new_n17_), .b(x2), .c(new_n16_), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  inv1   g05(.a(new_n20_), .O(z0));
  inv1   g06(.a(x2), .O(new_n22_));
  nor2   g07(.a(new_n17_), .b(x1), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n22_), .c(x0), .O(new_n24_));
  inv1   g09(.a(x0), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x1), .c(x2), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n17_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n22_), .c(new_n16_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n25_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n24_), .O(z1));
  nor3   g16(.a(x5), .b(x3), .c(x0), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x1), .c(x2), .O(new_n33_));
  inv1   g18(.a(x4), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n17_), .c(new_n16_), .O(new_n35_));
  nor3   g20(.a(x6), .b(x3), .c(x1), .O(new_n36_));
  nor2   g21(.a(x2), .b(x0), .O(new_n37_));
  oai21  g22(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n33_), .c(new_n24_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n25_), .c(x2), .O(new_n41_));
  inv1   g26(.a(x5), .O(new_n42_));
  nor2   g27(.a(new_n42_), .b(x0), .O(new_n43_));
  oai22  g28(.a(new_n43_), .b(new_n18_), .c(new_n41_), .d(new_n16_), .O(z3));
  inv1   g29(.a(new_n37_), .O(new_n45_));
  aoi21  g30(.a(x3), .b(x0), .c(x2), .O(new_n46_));
  nand3  g31(.a(x3), .b(x2), .c(x0), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n16_), .O(new_n48_));
  oai22  g33(.a(new_n48_), .b(new_n46_), .c(new_n45_), .d(new_n23_), .O(z4));
  nand2  g34(.a(new_n28_), .b(new_n22_), .O(new_n50_));
  nand2  g35(.a(new_n26_), .b(x2), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n25_), .O(new_n52_));
  oai21  g37(.a(x2), .b(new_n25_), .c(x1), .O(new_n53_));
  nand3  g38(.a(x3), .b(new_n22_), .c(new_n16_), .O(new_n54_));
  nand2  g39(.a(new_n17_), .b(x2), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(x0), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n53_), .c(new_n52_), .O(z5));
  aoi21  g43(.a(x4), .b(new_n17_), .c(x0), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(x2), .c(x1), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n57_), .c(new_n51_), .O(z6));
endmodule


