// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  aoi21  g03(.a(x3), .b(new_n17_), .c(new_n18_), .O(new_n19_));
  nor2   g04(.a(x3), .b(new_n18_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n16_), .c(x0), .O(new_n21_));
  oai21  g06(.a(new_n19_), .b(new_n16_), .c(new_n21_), .O(z0));
  inv1   g07(.a(x3), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n17_), .c(new_n16_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n18_), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x1), .c(x2), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x3), .c(new_n18_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n17_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n25_), .O(z1));
  inv1   g16(.a(x5), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x2), .O(new_n33_));
  nand2  g18(.a(new_n28_), .b(new_n18_), .O(new_n34_));
  nand2  g19(.a(new_n23_), .b(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n34_), .b(new_n33_), .c(new_n35_), .O(new_n36_));
  nand3  g21(.a(x3), .b(x2), .c(x1), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n36_), .c(new_n17_), .O(new_n39_));
  nand4  g24(.a(x3), .b(new_n18_), .c(new_n16_), .d(x0), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(z2));
  nand2  g26(.a(new_n18_), .b(x1), .O(new_n42_));
  nor2   g27(.a(new_n32_), .b(x0), .O(new_n43_));
  nand2  g28(.a(new_n20_), .b(new_n16_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z3));
  xor2a  g30(.a(x3), .b(x2), .O(new_n46_));
  nor2   g31(.a(new_n46_), .b(x0), .O(new_n47_));
  inv1   g32(.a(new_n47_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n44_), .c(new_n25_), .O(z4));
  nand2  g34(.a(new_n46_), .b(x0), .O(new_n50_));
  nand3  g35(.a(x5), .b(x2), .c(new_n17_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n16_), .O(new_n53_));
  nand2  g38(.a(new_n47_), .b(new_n34_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n53_), .c(new_n42_), .O(z5));
  nand2  g40(.a(x3), .b(new_n17_), .O(new_n56_));
  nand2  g41(.a(x2), .b(x1), .O(new_n57_));
  aoi22  g42(.a(new_n46_), .b(x0), .c(new_n26_), .d(x2), .O(new_n58_));
  oai22  g43(.a(new_n58_), .b(x1), .c(new_n57_), .d(new_n56_), .O(z6));
endmodule


