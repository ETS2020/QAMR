// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:40 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x1), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x0), .O(new_n17_));
  nand3  g02(.a(new_n17_), .b(x3), .c(x2), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(z0));
  nor2   g04(.a(x2), .b(x1), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(x3), .c(x0), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x1), .c(x2), .O(new_n24_));
  inv1   g09(.a(x3), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n20_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(new_n24_), .c(new_n22_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n21_), .O(z1));
  inv1   g14(.a(x4), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x2), .c(new_n25_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n17_), .O(new_n32_));
  oai21  g17(.a(new_n20_), .b(new_n25_), .c(x0), .O(new_n33_));
  inv1   g18(.a(x2), .O(new_n34_));
  nor2   g19(.a(x5), .b(new_n34_), .O(new_n35_));
  nor2   g20(.a(x6), .b(x2), .O(new_n36_));
  nor2   g21(.a(x3), .b(x1), .O(new_n37_));
  oai21  g22(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n33_), .c(new_n32_), .O(z2));
  oai22  g24(.a(x7), .b(x0), .c(x4), .d(x3), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n34_), .c(x1), .O(new_n41_));
  inv1   g26(.a(x5), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(x2), .c(new_n16_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n22_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n41_), .O(z3));
  oai21  g31(.a(new_n17_), .b(new_n25_), .c(new_n34_), .O(new_n47_));
  nand2  g32(.a(new_n25_), .b(x1), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x2), .c(new_n22_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n47_), .c(new_n33_), .O(z4));
  nand2  g35(.a(x3), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n34_), .b(x1), .O(new_n52_));
  nand3  g37(.a(x5), .b(x2), .c(new_n16_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n22_), .O(new_n55_));
  nand3  g40(.a(x6), .b(new_n25_), .c(new_n34_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n33_), .O(z5));
  oai21  g42(.a(x5), .b(x3), .c(new_n16_), .O(new_n58_));
  nand2  g43(.a(new_n30_), .b(new_n34_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n25_), .c(x1), .O(new_n60_));
  nor2   g45(.a(new_n20_), .b(x0), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n21_), .O(z6));
endmodule


