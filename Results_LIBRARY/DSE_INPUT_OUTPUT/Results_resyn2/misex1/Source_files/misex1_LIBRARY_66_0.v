// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x3), .b(new_n16_), .O(new_n17_));
  oai21  g02(.a(x1), .b(new_n16_), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x7), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(x3), .c(new_n19_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n18_), .c(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x3), .b(new_n24_), .c(x0), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(x7), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n19_), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x1), .c(x2), .O(new_n29_));
  inv1   g14(.a(x3), .O(new_n30_));
  nand2  g15(.a(x6), .b(new_n30_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n24_), .c(new_n19_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n29_), .c(new_n16_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n27_), .O(z1));
  nand2  g19(.a(x4), .b(new_n24_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n30_), .c(new_n19_), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n30_), .c(new_n19_), .O(new_n38_));
  aoi21  g23(.a(x6), .b(new_n24_), .c(new_n38_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n36_), .c(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n27_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nor2   g27(.a(new_n19_), .b(x0), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n42_), .c(new_n24_), .O(new_n44_));
  nand2  g29(.a(new_n30_), .b(x2), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n16_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(x7), .c(new_n19_), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(z3));
  nor2   g33(.a(x1), .b(new_n16_), .O(new_n49_));
  aoi22  g34(.a(new_n21_), .b(new_n16_), .c(new_n49_), .d(x3), .O(new_n50_));
  nand2  g35(.a(new_n30_), .b(new_n19_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n17_), .O(new_n52_));
  nor2   g37(.a(x7), .b(x1), .O(new_n53_));
  aoi21  g38(.a(new_n52_), .b(x2), .c(new_n53_), .O(new_n54_));
  oai21  g39(.a(new_n50_), .b(x2), .c(new_n54_), .O(z4));
  xor2a  g40(.a(x3), .b(x2), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(x0), .O(new_n57_));
  nand3  g42(.a(x5), .b(x2), .c(new_n16_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(x7), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n19_), .O(new_n60_));
  nand3  g45(.a(new_n45_), .b(new_n32_), .c(new_n16_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n60_), .O(z5));
  aoi21  g47(.a(new_n28_), .b(x2), .c(new_n20_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n19_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n30_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n43_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z6));
endmodule


