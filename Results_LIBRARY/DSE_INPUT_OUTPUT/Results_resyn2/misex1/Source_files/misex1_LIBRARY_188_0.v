// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x6), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(x0), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(new_n19_), .O(new_n22_));
  inv1   g07(.a(x0), .O(new_n23_));
  oai21  g08(.a(x1), .b(new_n23_), .c(new_n19_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(x2), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(new_n22_), .c(new_n18_), .O(z0));
  nand3  g11(.a(x3), .b(new_n20_), .c(x0), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x6), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n16_), .O(new_n29_));
  nor2   g14(.a(x5), .b(x3), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x1), .c(x2), .O(new_n31_));
  nand3  g16(.a(x3), .b(new_n16_), .c(new_n20_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n31_), .c(new_n23_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n29_), .O(z1));
  nand4  g19(.a(x6), .b(x3), .c(new_n16_), .d(x0), .O(new_n35_));
  inv1   g20(.a(x5), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n19_), .c(x2), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x0), .c(new_n35_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n20_), .O(new_n39_));
  nor2   g24(.a(x4), .b(x3), .O(new_n40_));
  nand2  g25(.a(x6), .b(new_n16_), .O(new_n41_));
  oai22  g26(.a(new_n41_), .b(new_n40_), .c(new_n19_), .d(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n21_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n39_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n21_), .c(new_n17_), .O(new_n46_));
  nand2  g31(.a(x5), .b(new_n23_), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n19_), .c(x2), .d(new_n20_), .O(new_n48_));
  oai21  g33(.a(new_n46_), .b(x2), .c(new_n48_), .O(z3));
  nand2  g34(.a(new_n19_), .b(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n35_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n20_), .O(new_n52_));
  nand4  g37(.a(new_n50_), .b(new_n32_), .c(new_n18_), .d(new_n23_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(z4));
  nand3  g39(.a(x6), .b(x3), .c(new_n16_), .O(new_n55_));
  aoi21  g40(.a(new_n50_), .b(new_n55_), .c(new_n23_), .O(new_n56_));
  nand3  g41(.a(x5), .b(x2), .c(new_n23_), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n56_), .c(new_n20_), .O(new_n59_));
  nand2  g44(.a(new_n18_), .b(x0), .O(new_n60_));
  nand4  g45(.a(x6), .b(x3), .c(new_n16_), .d(new_n20_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n60_), .c(new_n50_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n59_), .O(z5));
  inv1   g48(.a(new_n37_), .O(new_n64_));
  oai21  g49(.a(new_n56_), .b(new_n64_), .c(new_n20_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n19_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n21_), .c(new_n18_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z6));
endmodule


