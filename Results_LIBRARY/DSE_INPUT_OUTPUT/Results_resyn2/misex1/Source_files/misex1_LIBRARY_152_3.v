// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:37 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x3), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(x5), .b(new_n16_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(x3), .b(new_n18_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  aoi21  g07(.a(new_n20_), .b(new_n17_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand3  g10(.a(x3), .b(new_n25_), .c(new_n18_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x0), .O(new_n28_));
  nand2  g13(.a(new_n17_), .b(new_n24_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(x2), .c(new_n18_), .O(new_n30_));
  inv1   g15(.a(x6), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x3), .c(new_n18_), .O(new_n32_));
  nor2   g17(.a(x2), .b(x0), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n30_), .c(new_n28_), .O(z1));
  inv1   g20(.a(x3), .O(new_n36_));
  nand2  g21(.a(x4), .b(new_n25_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n36_), .c(new_n18_), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n36_), .c(new_n18_), .O(new_n40_));
  aoi21  g25(.a(x6), .b(new_n25_), .c(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n38_), .c(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n28_), .O(z2));
  nor2   g28(.a(x3), .b(x1), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n24_), .c(x2), .O(new_n45_));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n33_), .c(x1), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z3));
  oai21  g33(.a(x5), .b(new_n16_), .c(x3), .O(new_n49_));
  oai21  g34(.a(new_n24_), .b(new_n16_), .c(new_n44_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x2), .O(new_n52_));
  nand2  g37(.a(new_n33_), .b(new_n21_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n52_), .c(new_n28_), .O(z4));
  xor2a  g39(.a(x3), .b(x2), .O(new_n55_));
  nor2   g40(.a(new_n39_), .b(x0), .O(new_n56_));
  aoi21  g41(.a(new_n55_), .b(new_n19_), .c(new_n56_), .O(new_n57_));
  aoi21  g42(.a(new_n36_), .b(x2), .c(x0), .O(new_n58_));
  oai21  g43(.a(new_n32_), .b(x2), .c(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n57_), .b(x1), .c(new_n59_), .O(z5));
  oai21  g45(.a(x4), .b(x2), .c(new_n36_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(x1), .c(new_n16_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n45_), .c(new_n28_), .O(z6));
endmodule


