// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  nor2   g02(.a(x7), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(x1), .c(x2), .O(new_n21_));
  aoi21  g06(.a(new_n19_), .b(new_n17_), .c(new_n21_), .O(z0));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x1), .c(x2), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n20_), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n25_), .c(new_n16_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n24_), .c(x7), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n25_), .O(new_n29_));
  or2    g14(.a(new_n29_), .b(new_n17_), .O(new_n30_));
  oai21  g15(.a(new_n28_), .b(x0), .c(new_n30_), .O(z1));
  inv1   g16(.a(x0), .O(new_n32_));
  inv1   g17(.a(x7), .O(new_n33_));
  inv1   g18(.a(x4), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x2), .c(new_n20_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x1), .O(new_n36_));
  nor2   g21(.a(x5), .b(new_n25_), .O(new_n37_));
  nor2   g22(.a(x6), .b(x2), .O(new_n38_));
  nor2   g23(.a(x3), .b(x1), .O(new_n39_));
  oai21  g24(.a(new_n38_), .b(new_n37_), .c(new_n39_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n36_), .c(new_n33_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n32_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n30_), .O(z2));
  aoi21  g28(.a(new_n25_), .b(x1), .c(x7), .O(new_n44_));
  inv1   g29(.a(x5), .O(new_n45_));
  nor2   g30(.a(new_n45_), .b(x0), .O(new_n46_));
  nand2  g31(.a(new_n39_), .b(x2), .O(new_n47_));
  oai22  g32(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(x0), .O(z3));
  nand2  g33(.a(new_n20_), .b(x2), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(new_n18_), .c(new_n16_), .O(new_n50_));
  aoi21  g35(.a(new_n49_), .b(new_n29_), .c(new_n32_), .O(new_n51_));
  nand3  g36(.a(x3), .b(new_n25_), .c(new_n16_), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n18_), .c(new_n51_), .O(new_n53_));
  nor2   g38(.a(new_n53_), .b(new_n50_), .O(z4));
  nand3  g39(.a(x5), .b(x2), .c(new_n32_), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n51_), .c(new_n16_), .O(new_n57_));
  aoi21  g42(.a(new_n49_), .b(new_n27_), .c(x7), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(x0), .c(new_n57_), .O(z5));
  nand2  g44(.a(new_n23_), .b(x2), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n51_), .c(new_n16_), .O(new_n62_));
  nor2   g47(.a(x4), .b(x2), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(x3), .c(x1), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n33_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n32_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n62_), .O(z6));
endmodule


