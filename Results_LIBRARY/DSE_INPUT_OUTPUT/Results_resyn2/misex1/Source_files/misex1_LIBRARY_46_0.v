// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(x1), .b(new_n17_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(x4), .c(new_n19_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x4), .O(new_n23_));
  oai21  g08(.a(x5), .b(new_n23_), .c(x2), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n19_), .O(new_n25_));
  nand2  g10(.a(x2), .b(x0), .O(new_n26_));
  nor2   g11(.a(x2), .b(x0), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x1), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n26_), .c(new_n25_), .O(new_n29_));
  inv1   g14(.a(x1), .O(new_n30_));
  inv1   g15(.a(x6), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x3), .c(new_n30_), .O(new_n32_));
  nor2   g17(.a(x4), .b(x3), .O(new_n33_));
  aoi21  g18(.a(new_n32_), .b(new_n27_), .c(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n29_), .O(z1));
  nand2  g20(.a(x6), .b(new_n16_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n19_), .c(x1), .O(new_n37_));
  nor2   g22(.a(x5), .b(x1), .O(new_n38_));
  nand2  g23(.a(new_n19_), .b(x2), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n38_), .c(new_n17_), .O(new_n40_));
  nand2  g25(.a(x3), .b(new_n16_), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n20_), .c(new_n33_), .O(new_n43_));
  oai21  g28(.a(new_n40_), .b(new_n37_), .c(new_n43_), .O(z2));
  nand2  g29(.a(x5), .b(new_n17_), .O(new_n45_));
  nor2   g30(.a(new_n16_), .b(x1), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n45_), .c(new_n23_), .O(new_n47_));
  inv1   g32(.a(x7), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n49_));
  oai21  g34(.a(new_n47_), .b(x3), .c(new_n49_), .O(z3));
  nand2  g35(.a(x4), .b(new_n16_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n19_), .c(x0), .O(new_n52_));
  oai21  g37(.a(new_n41_), .b(x1), .c(new_n52_), .O(new_n53_));
  nand3  g38(.a(x4), .b(new_n19_), .c(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n54_), .b(new_n41_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n30_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n53_), .O(z4));
  aoi21  g43(.a(new_n54_), .b(new_n41_), .c(new_n17_), .O(new_n59_));
  nand4  g44(.a(x5), .b(x4), .c(new_n19_), .d(x2), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n30_), .O(new_n62_));
  oai21  g47(.a(new_n32_), .b(x2), .c(new_n52_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n62_), .O(z5));
  inv1   g49(.a(x5), .O(new_n65_));
  nand4  g50(.a(new_n65_), .b(x4), .c(new_n19_), .d(x2), .O(new_n66_));
  inv1   g51(.a(new_n66_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n59_), .c(new_n30_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n18_), .O(z6));
endmodule


