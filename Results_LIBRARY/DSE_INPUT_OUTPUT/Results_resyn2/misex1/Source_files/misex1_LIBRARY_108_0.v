// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand4  g01(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(z0));
  inv1   g03(.a(x2), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x1), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  nand3  g06(.a(x3), .b(x2), .c(new_n21_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n16_), .O(new_n24_));
  nor2   g09(.a(x2), .b(new_n16_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x3), .c(new_n21_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n24_), .O(z1));
  nand2  g12(.a(x1), .b(new_n16_), .O(new_n28_));
  aoi21  g13(.a(x4), .b(new_n19_), .c(x3), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(z2));
  inv1   g15(.a(x3), .O(new_n31_));
  inv1   g16(.a(x4), .O(new_n32_));
  inv1   g17(.a(x7), .O(new_n33_));
  aoi21  g18(.a(new_n32_), .b(new_n31_), .c(new_n33_), .O(new_n34_));
  nor3   g19(.a(new_n34_), .b(new_n20_), .c(x0), .O(z3));
  nand2  g20(.a(x3), .b(x2), .O(new_n36_));
  nand2  g21(.a(new_n20_), .b(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n26_), .O(z4));
  nand2  g24(.a(x2), .b(new_n21_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n20_), .c(new_n36_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n16_), .O(new_n42_));
  oai21  g27(.a(new_n25_), .b(new_n31_), .c(new_n21_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n42_), .O(z5));
  aoi21  g29(.a(new_n32_), .b(new_n19_), .c(x3), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n28_), .c(new_n26_), .O(z6));
endmodule


