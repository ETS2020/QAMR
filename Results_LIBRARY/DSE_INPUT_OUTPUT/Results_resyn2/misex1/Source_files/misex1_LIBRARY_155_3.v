// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n16_), .b(x0), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  nand2  g08(.a(x3), .b(new_n23_), .O(new_n24_));
  inv1   g09(.a(x0), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x1), .c(x2), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n18_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n23_), .c(new_n16_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n25_), .O(new_n30_));
  oai21  g15(.a(new_n24_), .b(new_n19_), .c(new_n30_), .O(z1));
  inv1   g16(.a(x5), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x2), .O(new_n33_));
  oai21  g18(.a(x6), .b(x2), .c(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n18_), .b(new_n25_), .O(new_n35_));
  nor2   g20(.a(new_n35_), .b(x1), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n18_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  nand2  g25(.a(new_n24_), .b(new_n16_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x0), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n40_), .c(new_n37_), .O(z2));
  nand2  g28(.a(new_n18_), .b(x2), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n16_), .O(new_n45_));
  oai21  g30(.a(x5), .b(x1), .c(new_n25_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n23_), .c(x1), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  oai21  g35(.a(new_n46_), .b(new_n44_), .c(new_n45_), .O(new_n51_));
  nand3  g36(.a(new_n24_), .b(new_n16_), .c(new_n25_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n51_), .c(new_n42_), .O(z4));
  xnor2a g38(.a(x3), .b(x2), .O(new_n54_));
  nand3  g39(.a(x5), .b(x2), .c(new_n25_), .O(new_n55_));
  oai21  g40(.a(new_n54_), .b(new_n25_), .c(new_n55_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n16_), .O(new_n57_));
  nand3  g42(.a(new_n44_), .b(new_n29_), .c(new_n25_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(z5));
  nor2   g44(.a(x4), .b(x2), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n35_), .c(x1), .O(new_n61_));
  xor2a  g46(.a(x3), .b(x2), .O(new_n62_));
  aoi22  g47(.a(new_n62_), .b(x0), .c(new_n26_), .d(x2), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(x1), .c(new_n61_), .O(z6));
endmodule


