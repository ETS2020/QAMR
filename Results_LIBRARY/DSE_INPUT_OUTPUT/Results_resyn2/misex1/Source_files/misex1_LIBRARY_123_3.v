// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  nor2   g03(.a(x3), .b(new_n18_), .O(new_n19_));
  aoi21  g04(.a(new_n19_), .b(new_n17_), .c(x7), .O(new_n20_));
  nor2   g05(.a(new_n17_), .b(x0), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x3), .c(x2), .O(new_n22_));
  oai21  g07(.a(new_n20_), .b(new_n16_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x3), .O(new_n24_));
  nand2  g09(.a(x6), .b(new_n24_), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n17_), .c(x2), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  nor3   g12(.a(new_n27_), .b(new_n18_), .c(x1), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n26_), .c(new_n16_), .O(new_n29_));
  nand3  g14(.a(x3), .b(new_n18_), .c(new_n17_), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x7), .c(x0), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(z1));
  nand2  g18(.a(x4), .b(new_n18_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n24_), .c(new_n17_), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand2  g21(.a(x6), .b(new_n18_), .O(new_n37_));
  nor2   g22(.a(x3), .b(x1), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  inv1   g24(.a(new_n39_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n35_), .c(new_n16_), .O(new_n41_));
  nor2   g26(.a(x7), .b(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n31_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n41_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n18_), .c(x1), .O(new_n46_));
  inv1   g31(.a(x5), .O(new_n47_));
  nand3  g32(.a(new_n19_), .b(new_n47_), .c(new_n17_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n16_), .O(new_n50_));
  nand3  g35(.a(new_n42_), .b(new_n19_), .c(new_n17_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(z3));
  nand2  g37(.a(new_n24_), .b(x2), .O(new_n53_));
  nand2  g38(.a(x3), .b(new_n18_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  aoi22  g40(.a(new_n55_), .b(new_n42_), .c(x2), .d(new_n16_), .O(new_n56_));
  nand3  g41(.a(new_n30_), .b(new_n53_), .c(new_n16_), .O(new_n57_));
  oai21  g42(.a(new_n56_), .b(x1), .c(new_n57_), .O(z4));
  oai22  g43(.a(new_n36_), .b(x1), .c(new_n24_), .d(new_n18_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n26_), .c(new_n16_), .O(new_n60_));
  and2   g45(.a(x7), .b(x0), .O(new_n61_));
  nor2   g46(.a(x1), .b(new_n16_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n55_), .c(new_n61_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n60_), .O(z5));
  aoi22  g49(.a(new_n55_), .b(x0), .c(new_n27_), .d(x2), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n24_), .O(new_n66_));
  aoi21  g51(.a(new_n66_), .b(new_n21_), .c(new_n61_), .O(new_n67_));
  oai21  g52(.a(new_n65_), .b(x1), .c(new_n67_), .O(z6));
endmodule


