// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(x4), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x3), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  nor2   g10(.a(x2), .b(x1), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x0), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n23_), .c(x3), .O(new_n29_));
  nor2   g14(.a(x5), .b(x3), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x1), .c(x2), .O(new_n31_));
  inv1   g16(.a(x6), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x3), .c(new_n26_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n31_), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n29_), .O(z1));
  nand2  g20(.a(x4), .b(x1), .O(new_n36_));
  aoi21  g21(.a(new_n19_), .b(x2), .c(new_n36_), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  inv1   g24(.a(x2), .O(new_n40_));
  nand2  g25(.a(new_n32_), .b(new_n40_), .O(new_n41_));
  nand2  g26(.a(new_n19_), .b(new_n18_), .O(new_n42_));
  aoi21  g27(.a(new_n41_), .b(new_n39_), .c(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n37_), .c(new_n16_), .O(new_n44_));
  nand3  g29(.a(new_n28_), .b(x4), .c(x3), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z2));
  nand2  g31(.a(x7), .b(x4), .O(new_n47_));
  nand3  g32(.a(new_n40_), .b(x1), .c(new_n16_), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g35(.a(x5), .b(new_n16_), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(new_n19_), .c(x2), .d(new_n18_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n50_), .c(new_n24_), .O(z3));
  nand2  g38(.a(new_n19_), .b(x2), .O(new_n54_));
  nand4  g39(.a(x4), .b(x3), .c(new_n40_), .d(x0), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n18_), .O(new_n57_));
  oai21  g42(.a(new_n23_), .b(new_n19_), .c(x2), .O(new_n58_));
  nand3  g43(.a(new_n36_), .b(x3), .c(new_n40_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n58_), .c(new_n16_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n57_), .O(z4));
  nand2  g46(.a(x3), .b(new_n40_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n54_), .c(new_n16_), .O(new_n63_));
  nand3  g48(.a(x5), .b(new_n19_), .c(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n18_), .O(new_n66_));
  aoi21  g51(.a(new_n19_), .b(x2), .c(x0), .O(new_n67_));
  aoi22  g52(.a(new_n67_), .b(new_n33_), .c(new_n23_), .d(x3), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n66_), .O(z5));
  xor2a  g54(.a(x3), .b(x2), .O(new_n70_));
  aoi22  g55(.a(new_n70_), .b(x0), .c(new_n30_), .d(x2), .O(new_n71_));
  nand2  g56(.a(new_n48_), .b(new_n19_), .O(new_n72_));
  nand2  g57(.a(new_n17_), .b(x4), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai21  g59(.a(new_n71_), .b(x1), .c(new_n74_), .O(z6));
endmodule


