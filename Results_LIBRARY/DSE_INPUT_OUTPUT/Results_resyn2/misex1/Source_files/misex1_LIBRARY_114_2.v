// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x4), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n17_), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(x0), .c(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g10(.a(x0), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n21_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n17_), .c(x2), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nand3  g14(.a(x4), .b(x2), .c(new_n17_), .O(new_n30_));
  aoi21  g15(.a(new_n29_), .b(new_n21_), .c(new_n30_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  nand3  g17(.a(x3), .b(new_n16_), .c(x0), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n17_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n32_), .O(z1));
  inv1   g21(.a(x6), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n16_), .O(new_n38_));
  nand3  g23(.a(new_n29_), .b(x4), .c(x2), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n38_), .c(new_n22_), .O(new_n40_));
  nand2  g25(.a(x4), .b(x3), .O(new_n41_));
  oai21  g26(.a(x4), .b(x3), .c(x1), .O(new_n42_));
  aoi21  g27(.a(new_n41_), .b(x2), .c(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n40_), .c(new_n26_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n35_), .O(z2));
  oai22  g30(.a(x7), .b(x2), .c(x4), .d(x3), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n18_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n26_), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n23_), .c(new_n20_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n16_), .c(new_n47_), .O(z3));
  nor2   g35(.a(new_n21_), .b(x2), .O(new_n51_));
  nor2   g36(.a(x3), .b(new_n16_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n51_), .c(new_n17_), .O(new_n53_));
  nand3  g38(.a(x4), .b(new_n21_), .c(x2), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n53_), .c(new_n26_), .O(new_n55_));
  nand2  g40(.a(new_n20_), .b(x2), .O(new_n56_));
  oai21  g41(.a(new_n52_), .b(new_n34_), .c(new_n17_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z4));
  nor2   g43(.a(x5), .b(x0), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n54_), .c(new_n33_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n17_), .O(new_n61_));
  nand3  g46(.a(new_n27_), .b(new_n16_), .c(new_n17_), .O(new_n62_));
  aoi21  g47(.a(new_n41_), .b(x2), .c(x0), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n61_), .O(z5));
  inv1   g50(.a(new_n54_), .O(new_n66_));
  aoi21  g51(.a(new_n66_), .b(new_n48_), .c(new_n34_), .O(new_n67_));
  nand2  g52(.a(x4), .b(new_n21_), .O(new_n68_));
  nand3  g53(.a(new_n56_), .b(new_n68_), .c(new_n18_), .O(new_n69_));
  oai21  g54(.a(new_n67_), .b(x1), .c(new_n69_), .O(z6));
endmodule


