// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_,
    new_n42_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n16_), .c(new_n18_), .O(new_n24_));
  nand4  g09(.a(x3), .b(new_n23_), .c(new_n16_), .d(x0), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n24_), .O(z1));
  nand2  g11(.a(x1), .b(new_n18_), .O(new_n27_));
  aoi21  g12(.a(x4), .b(new_n23_), .c(x3), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n27_), .c(new_n25_), .O(z2));
  nor2   g14(.a(new_n16_), .b(x0), .O(new_n30_));
  oai21  g15(.a(x4), .b(x3), .c(x7), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n30_), .c(new_n23_), .O(new_n32_));
  nor2   g17(.a(x1), .b(new_n18_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n19_), .c(x2), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n32_), .O(z3));
  xor2a  g20(.a(x3), .b(x2), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  nand2  g22(.a(new_n19_), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n30_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n37_), .O(z4));
  oai21  g25(.a(x4), .b(x2), .c(new_n19_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n30_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n37_), .O(z6));
  nand2  g28(.a(new_n39_), .b(new_n37_), .O(z5));
endmodule


