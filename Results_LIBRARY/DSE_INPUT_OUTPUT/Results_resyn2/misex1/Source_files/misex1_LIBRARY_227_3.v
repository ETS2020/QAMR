// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x6), .O(new_n17_));
  nand2  g02(.a(x3), .b(x2), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(x0), .c(new_n17_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x1), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(x2), .c(new_n21_), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n16_), .c(new_n20_), .O(z0));
  nand2  g09(.a(x2), .b(new_n21_), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(new_n22_), .b(new_n27_), .O(new_n28_));
  oai22  g13(.a(new_n28_), .b(new_n17_), .c(new_n26_), .d(new_n25_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n16_), .O(new_n30_));
  nand3  g15(.a(new_n27_), .b(new_n21_), .c(x0), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x3), .O(new_n33_));
  oai21  g18(.a(x2), .b(x0), .c(new_n17_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x1), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n33_), .c(new_n30_), .O(z1));
  nor2   g21(.a(new_n21_), .b(x0), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n32_), .c(x3), .O(new_n38_));
  nand3  g23(.a(x4), .b(new_n27_), .c(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n17_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x1), .O(new_n41_));
  nand2  g26(.a(new_n17_), .b(new_n27_), .O(new_n42_));
  oai21  g27(.a(x5), .b(new_n27_), .c(new_n42_), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n22_), .c(new_n21_), .d(new_n16_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n41_), .c(new_n38_), .O(z2));
  inv1   g30(.a(x5), .O(new_n46_));
  nor2   g31(.a(new_n46_), .b(x0), .O(new_n47_));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n37_), .c(new_n17_), .d(new_n27_), .O(new_n49_));
  oai21  g34(.a(new_n47_), .b(new_n23_), .c(new_n49_), .O(z3));
  xor2a  g35(.a(x3), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x0), .O(new_n52_));
  oai21  g37(.a(new_n26_), .b(new_n16_), .c(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n21_), .O(new_n55_));
  inv1   g40(.a(new_n51_), .O(new_n56_));
  aoi22  g41(.a(new_n56_), .b(new_n16_), .c(new_n34_), .d(x1), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n55_), .O(z4));
  nand3  g43(.a(x5), .b(x2), .c(new_n16_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n52_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n21_), .O(new_n61_));
  nand3  g46(.a(new_n56_), .b(new_n42_), .c(new_n16_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n61_), .c(new_n35_), .O(z5));
  nand2  g48(.a(new_n26_), .b(x2), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n21_), .O(new_n66_));
  nor2   g51(.a(x4), .b(x2), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(x3), .c(new_n16_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n17_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(x1), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n66_), .O(z6));
endmodule


