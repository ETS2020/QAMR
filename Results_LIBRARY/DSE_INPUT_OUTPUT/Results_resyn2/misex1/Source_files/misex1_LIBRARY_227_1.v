// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x2), .O(new_n16_));
  nor2   g01(.a(x3), .b(new_n16_), .O(new_n17_));
  aoi21  g02(.a(new_n17_), .b(x0), .c(x6), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(x3), .c(x2), .O(new_n21_));
  oai21  g06(.a(new_n18_), .b(x1), .c(new_n21_), .O(z0));
  inv1   g07(.a(x6), .O(new_n23_));
  nand3  g08(.a(x3), .b(new_n16_), .c(x0), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  inv1   g11(.a(x0), .O(new_n27_));
  nand2  g12(.a(x2), .b(new_n27_), .O(new_n28_));
  nor2   g13(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n25_), .c(new_n19_), .O(new_n30_));
  nand2  g15(.a(new_n20_), .b(new_n16_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(z1));
  inv1   g17(.a(x3), .O(new_n33_));
  inv1   g18(.a(x4), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x2), .c(new_n33_), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nor2   g21(.a(x3), .b(x1), .O(new_n37_));
  aoi22  g22(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(x1), .O(new_n38_));
  nand2  g23(.a(new_n25_), .b(new_n19_), .O(new_n39_));
  oai21  g24(.a(new_n38_), .b(x0), .c(new_n39_), .O(z2));
  inv1   g25(.a(x7), .O(new_n41_));
  aoi21  g26(.a(new_n34_), .b(new_n33_), .c(new_n41_), .O(new_n42_));
  nand2  g27(.a(x5), .b(new_n27_), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n17_), .c(new_n23_), .d(new_n19_), .O(new_n44_));
  oai21  g29(.a(new_n42_), .b(new_n31_), .c(new_n44_), .O(z3));
  nor2   g30(.a(new_n33_), .b(x0), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n19_), .O(new_n47_));
  oai21  g32(.a(new_n33_), .b(x1), .c(x0), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n47_), .c(new_n16_), .O(new_n49_));
  nand2  g34(.a(x6), .b(new_n19_), .O(new_n50_));
  oai21  g35(.a(new_n46_), .b(new_n37_), .c(x2), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(z4));
  xor2a  g37(.a(x3), .b(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(x0), .O(new_n54_));
  nand3  g39(.a(x5), .b(x2), .c(new_n27_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n23_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n19_), .O(new_n57_));
  nand2  g42(.a(new_n16_), .b(new_n19_), .O(new_n58_));
  nor2   g43(.a(new_n17_), .b(x0), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n57_), .O(z5));
  aoi21  g46(.a(new_n26_), .b(x2), .c(x6), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n54_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n19_), .O(new_n64_));
  oai21  g49(.a(x4), .b(x2), .c(new_n33_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n20_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z6));
endmodule


