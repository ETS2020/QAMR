// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand3  g03(.a(x3), .b(x1), .c(new_n18_), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  nor2   g05(.a(x1), .b(new_n18_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n17_), .c(new_n20_), .O(new_n22_));
  inv1   g07(.a(x4), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x3), .O(new_n24_));
  oai21  g09(.a(new_n22_), .b(new_n16_), .c(new_n24_), .O(z0));
  aoi21  g10(.a(new_n21_), .b(new_n16_), .c(new_n23_), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x1), .c(x2), .O(new_n28_));
  nor2   g13(.a(x2), .b(x1), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n17_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n28_), .c(new_n18_), .O(new_n32_));
  oai21  g17(.a(new_n26_), .b(new_n17_), .c(new_n32_), .O(z1));
  nand2  g18(.a(new_n17_), .b(x2), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(x4), .c(x1), .O(new_n35_));
  nand2  g20(.a(x6), .b(new_n16_), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nor2   g22(.a(x3), .b(x1), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n35_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n18_), .O(new_n41_));
  nand3  g26(.a(x4), .b(x3), .c(new_n16_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n21_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n41_), .O(z2));
  nand3  g30(.a(new_n16_), .b(x1), .c(new_n18_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n17_), .O(new_n47_));
  oai21  g32(.a(new_n46_), .b(x7), .c(x4), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g34(.a(x1), .O(new_n50_));
  nand2  g35(.a(x5), .b(new_n18_), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(new_n17_), .c(x2), .d(new_n50_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n49_), .O(z3));
  nand3  g38(.a(x3), .b(new_n16_), .c(x0), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n34_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nand2  g41(.a(new_n29_), .b(x3), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n34_), .c(new_n18_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n56_), .c(new_n24_), .O(z4));
  aoi21  g44(.a(new_n42_), .b(new_n34_), .c(new_n18_), .O(new_n60_));
  nand3  g45(.a(x6), .b(new_n16_), .c(new_n18_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n37_), .c(x3), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n50_), .O(new_n63_));
  inv1   g48(.a(new_n29_), .O(new_n64_));
  nand4  g49(.a(new_n34_), .b(new_n64_), .c(new_n24_), .d(new_n18_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n63_), .O(z5));
  nand2  g51(.a(new_n27_), .b(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n60_), .c(new_n50_), .O(new_n69_));
  oai21  g54(.a(new_n20_), .b(new_n23_), .c(new_n47_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n69_), .O(z6));
endmodule


