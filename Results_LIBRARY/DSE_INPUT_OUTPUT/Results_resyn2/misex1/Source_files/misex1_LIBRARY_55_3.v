// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  nand2  g04(.a(x3), .b(x2), .O(new_n20_));
  aoi22  g05(.a(new_n20_), .b(new_n16_), .c(new_n19_), .d(new_n17_), .O(z0));
  inv1   g06(.a(x2), .O(new_n22_));
  nand3  g07(.a(x6), .b(new_n18_), .c(new_n22_), .O(new_n23_));
  nor2   g08(.a(x5), .b(x3), .O(new_n24_));
  nand2  g09(.a(x2), .b(new_n17_), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(new_n24_), .c(new_n23_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n16_), .O(new_n27_));
  oai21  g12(.a(new_n22_), .b(x0), .c(x1), .O(new_n28_));
  nand3  g13(.a(x3), .b(new_n22_), .c(x0), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(z1));
  nand2  g15(.a(x4), .b(new_n22_), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n18_), .c(new_n17_), .O(new_n32_));
  nor2   g17(.a(x3), .b(x1), .O(new_n33_));
  nand2  g18(.a(x6), .b(new_n22_), .O(new_n34_));
  nand2  g19(.a(x5), .b(x2), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n32_), .c(new_n16_), .O(new_n38_));
  inv1   g23(.a(new_n29_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n17_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nor2   g27(.a(new_n17_), .b(x0), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n42_), .c(new_n22_), .O(new_n44_));
  nand2  g29(.a(x5), .b(new_n16_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n33_), .c(x2), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n44_), .O(z3));
  nand2  g32(.a(new_n19_), .b(new_n16_), .O(new_n48_));
  xor2a  g33(.a(x3), .b(x2), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n17_), .O(new_n50_));
  xor2a  g35(.a(new_n50_), .b(new_n48_), .O(z4));
  nand2  g36(.a(new_n49_), .b(x0), .O(new_n52_));
  oai21  g37(.a(new_n45_), .b(new_n22_), .c(new_n52_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n17_), .O(new_n54_));
  nand2  g39(.a(new_n19_), .b(x1), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n23_), .c(new_n20_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n16_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n54_), .O(z5));
  aoi22  g43(.a(new_n49_), .b(x0), .c(new_n24_), .d(x2), .O(new_n59_));
  oai21  g44(.a(x4), .b(x2), .c(new_n18_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n43_), .O(new_n61_));
  oai21  g46(.a(new_n59_), .b(x1), .c(new_n61_), .O(z6));
endmodule


