// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n44_, new_n45_, new_n47_, new_n48_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x2), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(z0));
  inv1   g05(.a(x2), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x1), .O(new_n22_));
  oai21  g07(.a(x5), .b(x3), .c(new_n22_), .O(new_n23_));
  inv1   g08(.a(x1), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x3), .c(new_n24_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n21_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n23_), .c(x0), .O(z1));
  nand2  g13(.a(x1), .b(new_n16_), .O(new_n29_));
  aoi21  g14(.a(x4), .b(new_n21_), .c(x3), .O(new_n30_));
  nand2  g15(.a(x6), .b(new_n21_), .O(new_n31_));
  aoi21  g16(.a(x5), .b(x2), .c(x3), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n31_), .c(x0), .O(new_n33_));
  oai22  g18(.a(new_n33_), .b(x1), .c(new_n30_), .d(new_n29_), .O(z2));
  nor2   g19(.a(x5), .b(x3), .O(new_n35_));
  nand2  g20(.a(new_n22_), .b(new_n35_), .O(new_n36_));
  oai21  g21(.a(x4), .b(x3), .c(x7), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n21_), .c(x1), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n36_), .c(x0), .O(z3));
  inv1   g24(.a(x3), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(x0), .c(new_n24_), .O(new_n41_));
  nand3  g26(.a(new_n21_), .b(x1), .c(new_n16_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n41_), .c(new_n17_), .O(z4));
  nor2   g28(.a(new_n40_), .b(new_n21_), .O(new_n44_));
  aoi21  g29(.a(new_n22_), .b(x5), .c(new_n44_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n27_), .c(x0), .O(z5));
  nor2   g31(.a(x4), .b(x2), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(x3), .c(x1), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n36_), .c(x0), .O(z6));
endmodule


