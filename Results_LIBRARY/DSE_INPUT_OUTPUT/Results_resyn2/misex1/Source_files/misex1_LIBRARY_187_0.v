// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x6), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  oai21  g07(.a(x1), .b(new_n22_), .c(new_n16_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n18_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x3), .b(new_n26_), .c(new_n19_), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  aoi22  g13(.a(new_n28_), .b(x0), .c(new_n17_), .d(new_n16_), .O(new_n29_));
  nor2   g14(.a(x5), .b(x3), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x1), .c(x2), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n27_), .c(new_n22_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(z1));
  aoi21  g18(.a(x4), .b(new_n26_), .c(x3), .O(new_n34_));
  nand3  g19(.a(new_n16_), .b(x2), .c(new_n19_), .O(new_n35_));
  oai22  g20(.a(new_n35_), .b(x5), .c(new_n34_), .d(new_n19_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n22_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n29_), .O(z2));
  inv1   g23(.a(x5), .O(new_n39_));
  nor2   g24(.a(new_n39_), .b(x0), .O(new_n40_));
  nor2   g25(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  inv1   g26(.a(x4), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  nand3  g28(.a(new_n26_), .b(x1), .c(new_n22_), .O(new_n44_));
  aoi21  g29(.a(new_n43_), .b(x7), .c(new_n44_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n41_), .c(x6), .O(new_n46_));
  inv1   g31(.a(x7), .O(new_n47_));
  nor2   g32(.a(new_n16_), .b(x2), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n20_), .c(new_n47_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n46_), .O(z3));
  nand3  g35(.a(x3), .b(new_n26_), .c(x0), .O(new_n51_));
  oai21  g36(.a(x3), .b(new_n26_), .c(new_n51_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n19_), .O(new_n53_));
  nand2  g38(.a(new_n18_), .b(x0), .O(new_n54_));
  nand3  g39(.a(x6), .b(new_n16_), .c(x2), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n27_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n53_), .O(z4));
  nand3  g42(.a(x6), .b(new_n42_), .c(new_n26_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n16_), .c(new_n19_), .O(new_n59_));
  aoi21  g44(.a(x6), .b(new_n26_), .c(x3), .O(new_n60_));
  nor2   g45(.a(new_n60_), .b(new_n48_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n22_), .O(new_n62_));
  nor2   g47(.a(x5), .b(x0), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n55_), .c(new_n51_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n19_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(z5));
  oai21  g51(.a(new_n55_), .b(new_n40_), .c(new_n51_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n19_), .O(new_n68_));
  nand2  g53(.a(new_n59_), .b(new_n22_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n68_), .O(z6));
endmodule


