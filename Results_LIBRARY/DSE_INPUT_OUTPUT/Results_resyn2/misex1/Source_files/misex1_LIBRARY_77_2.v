// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand4  g02(.a(x4), .b(x3), .c(x1), .d(new_n17_), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(x1), .b(new_n17_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n19_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x2), .c(new_n23_), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n19_), .c(new_n16_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n17_), .O(new_n29_));
  nand3  g14(.a(new_n20_), .b(x3), .c(new_n16_), .O(new_n30_));
  oai21  g15(.a(x2), .b(x0), .c(x4), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n30_), .c(new_n29_), .O(z1));
  nand2  g18(.a(new_n19_), .b(x2), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(x4), .c(x1), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nor2   g21(.a(x3), .b(x1), .O(new_n37_));
  nand2  g22(.a(x6), .b(new_n16_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n35_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n17_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n30_), .O(z2));
  inv1   g27(.a(x4), .O(new_n43_));
  nor3   g28(.a(x7), .b(x2), .c(x0), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n43_), .c(x1), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n17_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n37_), .c(x2), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z3));
  oai21  g33(.a(new_n19_), .b(x2), .c(new_n17_), .O(new_n49_));
  xor2a  g34(.a(x3), .b(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x0), .O(new_n51_));
  nor2   g36(.a(x5), .b(x3), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(x2), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n23_), .O(new_n55_));
  nand4  g40(.a(new_n34_), .b(x4), .c(x1), .d(new_n17_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(z4));
  inv1   g42(.a(new_n27_), .O(new_n58_));
  nand2  g43(.a(x4), .b(x3), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(x2), .c(new_n23_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n17_), .O(new_n61_));
  nand2  g46(.a(x4), .b(x1), .O(new_n62_));
  nand3  g47(.a(new_n25_), .b(x2), .c(new_n17_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n51_), .c(new_n23_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n61_), .O(z5));
  aoi22  g51(.a(new_n50_), .b(x0), .c(new_n52_), .d(x2), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(x1), .c(new_n18_), .O(z6));
endmodule


