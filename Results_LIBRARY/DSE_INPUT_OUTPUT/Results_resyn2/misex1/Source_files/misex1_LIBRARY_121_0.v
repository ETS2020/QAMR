// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:21 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x0), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n16_), .c(x2), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nand3  g06(.a(x3), .b(x2), .c(x1), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x5), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n21_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n20_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x5), .b(x1), .c(new_n21_), .O(new_n27_));
  oai21  g12(.a(new_n18_), .b(new_n16_), .c(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n16_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n26_), .c(x0), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(x5), .c(new_n17_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(z1));
  nand2  g18(.a(x4), .b(new_n26_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n16_), .c(new_n17_), .O(new_n35_));
  inv1   g20(.a(x6), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n16_), .c(new_n26_), .d(new_n17_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x5), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n35_), .c(new_n21_), .O(new_n39_));
  nand3  g24(.a(new_n19_), .b(x3), .c(new_n26_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(z2));
  inv1   g26(.a(x4), .O(new_n42_));
  inv1   g27(.a(x7), .O(new_n43_));
  aoi21  g28(.a(new_n42_), .b(new_n16_), .c(new_n43_), .O(new_n44_));
  inv1   g29(.a(x5), .O(new_n45_));
  nor2   g30(.a(new_n45_), .b(x0), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n26_), .c(x1), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n44_), .c(new_n20_), .O(z3));
  nand2  g33(.a(new_n16_), .b(x2), .O(new_n49_));
  nand2  g34(.a(x3), .b(new_n26_), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n49_), .c(new_n21_), .O(new_n51_));
  nand3  g36(.a(x5), .b(new_n16_), .c(x2), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n17_), .O(new_n54_));
  nand3  g39(.a(x3), .b(new_n26_), .c(new_n17_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n46_), .c(new_n49_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n54_), .O(z4));
  oai21  g42(.a(new_n51_), .b(new_n31_), .c(new_n17_), .O(new_n58_));
  aoi21  g43(.a(new_n16_), .b(x2), .c(new_n17_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n45_), .c(new_n21_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n58_), .O(z5));
  and2   g46(.a(new_n50_), .b(new_n49_), .O(new_n62_));
  aoi21  g47(.a(new_n42_), .b(new_n26_), .c(x3), .O(new_n63_));
  oai22  g48(.a(new_n63_), .b(new_n27_), .c(new_n62_), .d(new_n18_), .O(z6));
endmodule


