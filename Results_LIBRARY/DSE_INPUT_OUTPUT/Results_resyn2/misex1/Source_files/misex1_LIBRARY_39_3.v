// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_;
  nand2  g00(.a(x4), .b(x3), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  oai21  g06(.a(x1), .b(new_n21_), .c(new_n17_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n20_), .c(new_n16_), .O(z0));
  nor2   g09(.a(x2), .b(x1), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(x0), .c(x4), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x1), .c(x2), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n25_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n28_), .c(new_n21_), .O(new_n31_));
  oai21  g16(.a(new_n26_), .b(new_n17_), .c(new_n31_), .O(z1));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x2), .O(new_n34_));
  inv1   g19(.a(x2), .O(new_n35_));
  nand2  g20(.a(new_n29_), .b(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n17_), .b(new_n18_), .O(new_n37_));
  aoi21  g22(.a(new_n36_), .b(new_n34_), .c(new_n37_), .O(new_n38_));
  oai21  g23(.a(x3), .b(new_n35_), .c(new_n16_), .O(new_n39_));
  oai21  g24(.a(x4), .b(x3), .c(x1), .O(new_n40_));
  nor2   g25(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n38_), .c(new_n21_), .O(new_n42_));
  nor2   g27(.a(x4), .b(x2), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(x3), .c(x0), .O(new_n44_));
  inv1   g29(.a(new_n44_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n42_), .O(z2));
  nand2  g32(.a(new_n17_), .b(x2), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n21_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n49_), .c(new_n18_), .O(new_n51_));
  oai21  g36(.a(x4), .b(x3), .c(x7), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n19_), .c(new_n35_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n51_), .c(new_n16_), .O(z3));
  oai21  g39(.a(new_n45_), .b(new_n49_), .c(new_n18_), .O(new_n55_));
  nand2  g40(.a(new_n25_), .b(x3), .O(new_n56_));
  nand4  g41(.a(new_n56_), .b(new_n48_), .c(new_n16_), .d(new_n21_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n55_), .O(z4));
  nand2  g43(.a(x3), .b(new_n35_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n48_), .c(new_n21_), .O(new_n60_));
  nand3  g45(.a(x5), .b(new_n17_), .c(x2), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n18_), .O(new_n63_));
  nand3  g48(.a(new_n48_), .b(new_n30_), .c(new_n21_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n63_), .c(new_n16_), .O(z5));
  nand2  g50(.a(new_n27_), .b(x2), .O(new_n66_));
  inv1   g51(.a(new_n66_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n60_), .c(new_n18_), .O(new_n68_));
  oai22  g53(.a(new_n43_), .b(x3), .c(new_n19_), .d(x4), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n68_), .O(z6));
endmodule


