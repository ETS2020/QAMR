// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  inv1   g06(.a(x5), .O(new_n22_));
  nor2   g07(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g10(.a(x3), .O(new_n26_));
  nand2  g11(.a(x2), .b(new_n17_), .O(new_n27_));
  aoi21  g12(.a(new_n22_), .b(new_n26_), .c(new_n27_), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n26_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n17_), .c(x2), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n28_), .c(new_n21_), .O(new_n31_));
  nand3  g16(.a(x3), .b(new_n16_), .c(new_n17_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n22_), .c(x0), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(z1));
  nand2  g20(.a(x4), .b(new_n16_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n26_), .c(new_n17_), .O(new_n37_));
  nand2  g22(.a(x5), .b(x2), .O(new_n38_));
  nand2  g23(.a(x6), .b(new_n16_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(new_n20_), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n37_), .c(new_n21_), .O(new_n42_));
  nand2  g27(.a(new_n33_), .b(new_n23_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n42_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n16_), .c(x1), .O(new_n46_));
  nand3  g31(.a(new_n22_), .b(new_n26_), .c(x2), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(x1), .c(new_n46_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n21_), .O(new_n49_));
  nand3  g34(.a(new_n23_), .b(new_n20_), .c(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n49_), .O(z3));
  nand2  g36(.a(x3), .b(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n26_), .b(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi22  g39(.a(new_n54_), .b(new_n23_), .c(x2), .d(new_n21_), .O(new_n55_));
  nand3  g40(.a(new_n53_), .b(new_n32_), .c(new_n21_), .O(new_n56_));
  oai21  g41(.a(new_n55_), .b(x1), .c(new_n56_), .O(z4));
  nand2  g42(.a(x5), .b(new_n17_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n26_), .c(new_n16_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n30_), .c(new_n21_), .O(new_n60_));
  nor2   g45(.a(x5), .b(new_n21_), .O(new_n61_));
  nor2   g46(.a(x1), .b(new_n21_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n54_), .c(new_n61_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n60_), .O(z5));
  inv1   g49(.a(new_n47_), .O(new_n65_));
  aoi21  g50(.a(new_n54_), .b(x0), .c(new_n65_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n26_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n18_), .c(new_n61_), .O(new_n68_));
  oai21  g53(.a(new_n66_), .b(x1), .c(new_n68_), .O(z6));
endmodule


