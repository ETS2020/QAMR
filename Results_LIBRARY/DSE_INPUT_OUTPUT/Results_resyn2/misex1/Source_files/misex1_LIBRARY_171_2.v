// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand3  g02(.a(new_n17_), .b(x2), .c(new_n16_), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x4), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nand4  g06(.a(x3), .b(x2), .c(x1), .d(new_n21_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x3), .b(new_n24_), .c(new_n16_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(x4), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x0), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x1), .c(x2), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n17_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n24_), .c(new_n16_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n29_), .c(new_n21_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n27_), .O(z1));
  nand2  g18(.a(x4), .b(new_n24_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n17_), .c(new_n16_), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n17_), .c(new_n16_), .O(new_n37_));
  aoi21  g22(.a(x6), .b(new_n24_), .c(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n35_), .c(new_n21_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n27_), .O(z2));
  oai22  g25(.a(x7), .b(x0), .c(x4), .d(x3), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n24_), .c(x1), .O(new_n42_));
  or2    g27(.a(new_n18_), .b(x5), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n42_), .c(new_n20_), .O(z3));
  aoi21  g29(.a(new_n17_), .b(x2), .c(x0), .O(new_n45_));
  nor2   g30(.a(new_n45_), .b(new_n16_), .O(new_n46_));
  nand2  g31(.a(new_n17_), .b(x2), .O(new_n47_));
  nand2  g32(.a(x3), .b(new_n24_), .O(new_n48_));
  nand2  g33(.a(x4), .b(x0), .O(new_n49_));
  aoi21  g34(.a(new_n48_), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  inv1   g35(.a(new_n50_), .O(new_n51_));
  nand2  g36(.a(new_n25_), .b(new_n21_), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n51_), .c(new_n46_), .O(z4));
  nand3  g38(.a(x5), .b(x2), .c(new_n21_), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n50_), .c(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n45_), .b(new_n31_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(z5));
  nand3  g43(.a(new_n28_), .b(x2), .c(new_n21_), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n50_), .c(new_n16_), .O(new_n61_));
  oai21  g46(.a(x4), .b(x2), .c(new_n17_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(x1), .c(new_n21_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(z6));
endmodule


