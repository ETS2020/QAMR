// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  oai21  g02(.a(x5), .b(new_n17_), .c(new_n16_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nand2  g04(.a(new_n17_), .b(new_n19_), .O(new_n20_));
  oai21  g05(.a(new_n19_), .b(x0), .c(new_n20_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n18_), .c(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nor2   g09(.a(x2), .b(x1), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(x0), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x3), .O(new_n28_));
  nor2   g13(.a(x5), .b(x3), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x1), .c(x2), .O(new_n30_));
  nand2  g15(.a(x6), .b(new_n17_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n25_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n30_), .c(new_n16_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n28_), .O(z1));
  inv1   g19(.a(x2), .O(new_n35_));
  nand2  g20(.a(x4), .b(new_n35_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n17_), .c(new_n19_), .O(new_n37_));
  nand2  g22(.a(new_n24_), .b(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n35_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(new_n20_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n37_), .c(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n28_), .O(z2));
  nand2  g28(.a(x7), .b(x4), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n35_), .c(x1), .d(new_n16_), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n16_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(x2), .c(new_n19_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n17_), .O(new_n49_));
  nor2   g34(.a(new_n19_), .b(x0), .O(new_n50_));
  nor2   g35(.a(x7), .b(x5), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n35_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n49_), .O(z3));
  aoi21  g38(.a(new_n17_), .b(x2), .c(x0), .O(new_n54_));
  oai21  g39(.a(new_n25_), .b(x5), .c(x3), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g41(.a(new_n17_), .b(x2), .O(new_n57_));
  nand3  g42(.a(new_n24_), .b(x3), .c(new_n35_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g44(.a(new_n54_), .b(x1), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n56_), .O(z4));
  aoi21  g47(.a(new_n58_), .b(new_n57_), .c(new_n16_), .O(new_n63_));
  nand3  g48(.a(x5), .b(new_n17_), .c(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n19_), .O(new_n66_));
  aoi21  g51(.a(x5), .b(x3), .c(new_n19_), .O(new_n67_));
  nand2  g52(.a(new_n38_), .b(new_n31_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n67_), .c(new_n54_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z5));
  nand2  g55(.a(new_n29_), .b(x2), .O(new_n71_));
  inv1   g56(.a(new_n71_), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(new_n63_), .c(new_n19_), .O(new_n73_));
  oai21  g58(.a(x4), .b(x2), .c(new_n17_), .O(new_n74_));
  aoi22  g59(.a(new_n74_), .b(new_n50_), .c(x5), .d(x3), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n73_), .O(z6));
endmodule


