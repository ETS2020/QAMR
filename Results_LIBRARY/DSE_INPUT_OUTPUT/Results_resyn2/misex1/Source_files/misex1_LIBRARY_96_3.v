// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand3  g03(.a(x3), .b(x1), .c(new_n18_), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  nor2   g05(.a(x1), .b(new_n18_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n17_), .c(new_n20_), .O(new_n22_));
  nand2  g07(.a(x7), .b(x4), .O(new_n23_));
  oai21  g08(.a(new_n22_), .b(new_n16_), .c(new_n23_), .O(z0));
  inv1   g09(.a(x5), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(new_n17_), .c(x2), .O(new_n26_));
  inv1   g11(.a(x1), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n17_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n16_), .c(new_n27_), .O(new_n29_));
  aoi21  g14(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n26_), .O(new_n31_));
  inv1   g16(.a(new_n23_), .O(new_n32_));
  nor2   g17(.a(new_n17_), .b(x2), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n21_), .c(new_n32_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(z1));
  nand3  g20(.a(x3), .b(new_n16_), .c(x0), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand2  g22(.a(x6), .b(new_n16_), .O(new_n38_));
  nor2   g23(.a(x3), .b(x0), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n27_), .O(new_n42_));
  nand3  g27(.a(new_n16_), .b(x1), .c(new_n18_), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(x4), .c(new_n20_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n42_), .c(new_n32_), .O(z2));
  inv1   g31(.a(x4), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n16_), .c(x1), .d(new_n18_), .O(new_n48_));
  nor2   g33(.a(new_n25_), .b(x0), .O(new_n49_));
  nand3  g34(.a(new_n23_), .b(x2), .c(new_n27_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n17_), .O(new_n52_));
  oai21  g37(.a(new_n43_), .b(x7), .c(new_n52_), .O(z3));
  nor2   g38(.a(x3), .b(new_n16_), .O(new_n54_));
  inv1   g39(.a(new_n36_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n27_), .O(new_n56_));
  nor2   g41(.a(new_n54_), .b(x0), .O(new_n57_));
  nand3  g42(.a(x3), .b(new_n16_), .c(new_n27_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n56_), .c(new_n23_), .O(z4));
  xnor2a g45(.a(x3), .b(x2), .O(new_n61_));
  nand3  g46(.a(x5), .b(new_n17_), .c(x2), .O(new_n62_));
  oai21  g47(.a(new_n61_), .b(new_n18_), .c(new_n62_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n27_), .O(new_n64_));
  nand2  g49(.a(new_n57_), .b(new_n29_), .O(new_n65_));
  aoi21  g50(.a(new_n65_), .b(new_n64_), .c(new_n32_), .O(z5));
  oai21  g51(.a(new_n61_), .b(new_n18_), .c(new_n26_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n27_), .O(new_n68_));
  and2   g53(.a(new_n48_), .b(new_n19_), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(new_n68_), .c(new_n32_), .O(z6));
endmodule


