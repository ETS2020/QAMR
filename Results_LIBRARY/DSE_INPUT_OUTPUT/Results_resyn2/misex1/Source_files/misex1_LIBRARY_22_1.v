// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  nand2  g01(.a(x5), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  oai21  g06(.a(x3), .b(x0), .c(x2), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n21_), .c(new_n17_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(x3), .b(x2), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n24_), .c(x0), .O(new_n26_));
  nand4  g11(.a(new_n24_), .b(x3), .c(new_n16_), .d(x0), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n26_), .c(new_n18_), .O(new_n29_));
  inv1   g14(.a(x3), .O(new_n30_));
  aoi21  g15(.a(x6), .b(new_n30_), .c(x1), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x0), .c(new_n24_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n16_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n29_), .O(z1));
  inv1   g19(.a(x0), .O(new_n35_));
  inv1   g20(.a(x4), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n16_), .c(x1), .O(new_n38_));
  nand2  g23(.a(x6), .b(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n20_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(x5), .O(new_n41_));
  inv1   g26(.a(new_n25_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(x1), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n41_), .c(new_n35_), .O(new_n45_));
  nand2  g30(.a(new_n28_), .b(new_n18_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n45_), .O(z2));
  nand2  g32(.a(new_n37_), .b(x7), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n19_), .c(new_n24_), .d(new_n16_), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n35_), .O(new_n50_));
  nor2   g35(.a(x3), .b(new_n16_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n18_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n49_), .O(z3));
  nand3  g38(.a(x3), .b(new_n16_), .c(x0), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n51_), .b(new_n55_), .c(new_n18_), .O(new_n56_));
  inv1   g41(.a(new_n51_), .O(new_n57_));
  nand3  g42(.a(x3), .b(new_n16_), .c(new_n18_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(new_n35_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n56_), .c(new_n17_), .O(z4));
  oai21  g45(.a(x5), .b(new_n30_), .c(new_n16_), .O(new_n61_));
  aoi21  g46(.a(x3), .b(x2), .c(new_n35_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n50_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n18_), .O(new_n65_));
  nand2  g50(.a(new_n42_), .b(new_n35_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n65_), .c(new_n33_), .O(z5));
  aoi22  g52(.a(new_n62_), .b(new_n61_), .c(new_n51_), .d(new_n24_), .O(new_n68_));
  oai21  g53(.a(x4), .b(x2), .c(new_n30_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n19_), .c(new_n17_), .O(new_n70_));
  oai21  g55(.a(new_n68_), .b(x1), .c(new_n70_), .O(z6));
endmodule


