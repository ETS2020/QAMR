// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nor2   g06(.a(x4), .b(new_n21_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g09(.a(x4), .O(new_n25_));
  nand3  g10(.a(x3), .b(new_n16_), .c(new_n17_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x0), .O(new_n28_));
  inv1   g13(.a(x3), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n29_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n17_), .c(x2), .O(new_n31_));
  nor2   g16(.a(x5), .b(x3), .O(new_n32_));
  nand2  g17(.a(x2), .b(new_n17_), .O(new_n33_));
  nor2   g18(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(new_n31_), .c(new_n21_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n28_), .O(z1));
  nand2  g21(.a(x4), .b(new_n16_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n29_), .c(new_n17_), .O(new_n38_));
  nand2  g23(.a(new_n29_), .b(new_n17_), .O(new_n39_));
  inv1   g24(.a(x5), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x2), .O(new_n41_));
  inv1   g26(.a(x6), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n41_), .c(new_n39_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n38_), .c(new_n21_), .O(new_n45_));
  nor2   g30(.a(x4), .b(x2), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(x3), .c(new_n17_), .d(x0), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z2));
  nand3  g33(.a(new_n20_), .b(new_n40_), .c(x2), .O(new_n49_));
  oai21  g34(.a(x4), .b(x3), .c(x7), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n16_), .c(x1), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  nand3  g38(.a(new_n22_), .b(new_n20_), .c(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(z3));
  xor2a  g40(.a(x3), .b(x2), .O(new_n56_));
  nor2   g41(.a(new_n16_), .b(x0), .O(new_n57_));
  aoi21  g42(.a(new_n56_), .b(new_n22_), .c(new_n57_), .O(new_n58_));
  nand2  g43(.a(new_n29_), .b(x2), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n26_), .c(new_n21_), .O(new_n60_));
  oai21  g45(.a(new_n58_), .b(x1), .c(new_n60_), .O(z4));
  nand2  g46(.a(x5), .b(new_n17_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n29_), .c(new_n16_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n31_), .c(new_n21_), .O(new_n64_));
  nand2  g49(.a(new_n56_), .b(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n25_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(x0), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n64_), .O(z5));
  aoi22  g53(.a(new_n57_), .b(new_n32_), .c(new_n56_), .d(new_n22_), .O(new_n69_));
  oai21  g54(.a(new_n46_), .b(x3), .c(new_n18_), .O(new_n70_));
  oai21  g55(.a(new_n69_), .b(x1), .c(new_n70_), .O(z6));
endmodule


