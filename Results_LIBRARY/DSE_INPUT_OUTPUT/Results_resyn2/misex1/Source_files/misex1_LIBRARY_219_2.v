// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x0), .O(new_n16_));
  nor2   g01(.a(x1), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  nor2   g03(.a(x3), .b(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  nand3  g05(.a(x3), .b(x2), .c(new_n16_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x5), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x1), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n20_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n26_), .c(new_n18_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n25_), .c(x1), .O(new_n28_));
  nand3  g13(.a(x5), .b(new_n18_), .c(x1), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n28_), .c(new_n16_), .O(new_n31_));
  nand3  g16(.a(new_n17_), .b(x3), .c(new_n18_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n31_), .O(z1));
  nand2  g18(.a(x4), .b(x1), .O(new_n34_));
  inv1   g19(.a(x1), .O(new_n35_));
  inv1   g20(.a(x6), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n26_), .c(new_n35_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n34_), .c(x0), .O(new_n38_));
  nand3  g23(.a(x3), .b(new_n35_), .c(x0), .O(new_n39_));
  inv1   g24(.a(new_n39_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n38_), .c(new_n18_), .O(new_n41_));
  inv1   g26(.a(x5), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n26_), .c(x2), .O(new_n43_));
  oai21  g28(.a(new_n26_), .b(new_n35_), .c(new_n43_), .O(new_n44_));
  nor2   g29(.a(x5), .b(new_n35_), .O(new_n45_));
  aoi21  g30(.a(new_n44_), .b(new_n16_), .c(new_n45_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n41_), .O(z2));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n18_), .c(x1), .d(new_n16_), .O(new_n49_));
  oai21  g34(.a(new_n19_), .b(x1), .c(new_n42_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n49_), .c(new_n20_), .O(z3));
  nand2  g36(.a(new_n26_), .b(x2), .O(new_n52_));
  nand2  g37(.a(x3), .b(new_n18_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n52_), .c(new_n16_), .O(new_n54_));
  aoi21  g39(.a(x3), .b(new_n18_), .c(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n35_), .O(new_n56_));
  oai21  g41(.a(new_n19_), .b(x0), .c(x5), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(x1), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n56_), .c(new_n43_), .O(z4));
  inv1   g44(.a(new_n45_), .O(new_n60_));
  nor2   g45(.a(new_n25_), .b(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n54_), .c(new_n35_), .O(new_n62_));
  oai21  g47(.a(new_n19_), .b(new_n35_), .c(new_n27_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n16_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(z5));
  inv1   g50(.a(new_n43_), .O(new_n66_));
  oai21  g51(.a(new_n54_), .b(new_n66_), .c(new_n35_), .O(new_n67_));
  oai21  g52(.a(x4), .b(x2), .c(new_n26_), .O(new_n68_));
  nand4  g53(.a(new_n68_), .b(x5), .c(x1), .d(new_n16_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n67_), .O(z6));
endmodule


