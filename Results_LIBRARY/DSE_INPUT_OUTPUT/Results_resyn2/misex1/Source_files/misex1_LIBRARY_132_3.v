// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:27 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  nand2  g04(.a(x3), .b(x2), .O(new_n20_));
  aoi22  g05(.a(new_n20_), .b(new_n16_), .c(new_n19_), .d(new_n17_), .O(z0));
  nor2   g06(.a(x5), .b(x3), .O(new_n22_));
  nand2  g07(.a(x2), .b(new_n17_), .O(new_n23_));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x6), .b(new_n18_), .c(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n23_), .b(new_n22_), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n16_), .O(new_n27_));
  nand2  g12(.a(x3), .b(new_n24_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n17_), .O(new_n29_));
  nand2  g14(.a(new_n24_), .b(x1), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  aoi21  g16(.a(new_n29_), .b(x0), .c(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n27_), .O(z1));
  nand2  g18(.a(new_n29_), .b(x0), .O(new_n34_));
  aoi21  g19(.a(x6), .b(new_n24_), .c(x1), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n35_), .c(new_n18_), .d(new_n16_), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n18_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n37_), .c(new_n34_), .O(z2));
  nand2  g26(.a(new_n19_), .b(new_n17_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(x0), .O(new_n43_));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n31_), .O(new_n45_));
  nand3  g30(.a(new_n22_), .b(x2), .c(new_n17_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n45_), .c(new_n43_), .O(z3));
  aoi21  g32(.a(new_n19_), .b(new_n16_), .c(new_n17_), .O(new_n48_));
  xor2a  g33(.a(x3), .b(x2), .O(new_n49_));
  nand2  g34(.a(new_n17_), .b(new_n16_), .O(new_n50_));
  oai22  g35(.a(new_n50_), .b(new_n28_), .c(new_n49_), .d(new_n16_), .O(new_n51_));
  nor2   g36(.a(new_n51_), .b(new_n48_), .O(z4));
  nand3  g37(.a(new_n30_), .b(new_n25_), .c(new_n20_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n16_), .O(new_n54_));
  aoi21  g39(.a(new_n36_), .b(new_n16_), .c(x1), .O(new_n55_));
  oai21  g40(.a(new_n49_), .b(new_n16_), .c(new_n55_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n54_), .O(z5));
  nand3  g42(.a(new_n49_), .b(new_n17_), .c(x0), .O(new_n58_));
  nand2  g43(.a(new_n18_), .b(new_n16_), .O(new_n59_));
  nor2   g44(.a(x4), .b(x2), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n59_), .c(x1), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n58_), .c(new_n46_), .O(z6));
endmodule


