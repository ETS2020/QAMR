// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:58 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x5), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  nor2   g04(.a(x1), .b(new_n19_), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(new_n17_), .c(new_n18_), .O(new_n21_));
  nand3  g06(.a(x3), .b(x1), .c(new_n19_), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n21_), .c(new_n16_), .O(z0));
  nor2   g08(.a(x2), .b(x0), .O(new_n24_));
  nor2   g09(.a(new_n24_), .b(x1), .O(new_n25_));
  nor2   g10(.a(new_n17_), .b(x2), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n19_), .c(new_n25_), .O(new_n27_));
  inv1   g12(.a(x1), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n28_), .O(new_n30_));
  nor2   g15(.a(x5), .b(new_n16_), .O(new_n31_));
  aoi21  g16(.a(new_n30_), .b(new_n24_), .c(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n27_), .O(z1));
  oai21  g18(.a(x4), .b(x3), .c(x1), .O(new_n34_));
  nand3  g19(.a(new_n29_), .b(new_n17_), .c(new_n28_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(x0), .O(new_n36_));
  nand3  g21(.a(x3), .b(new_n28_), .c(x0), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n36_), .c(new_n16_), .O(new_n39_));
  inv1   g24(.a(new_n22_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x5), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n24_), .c(x1), .O(new_n44_));
  oai21  g29(.a(new_n21_), .b(new_n16_), .c(new_n44_), .O(z3));
  nand3  g30(.a(x5), .b(new_n17_), .c(x2), .O(new_n46_));
  and2   g31(.a(new_n46_), .b(new_n19_), .O(new_n47_));
  nand2  g32(.a(x3), .b(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n46_), .b(new_n48_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n28_), .O(new_n50_));
  nand2  g35(.a(new_n26_), .b(new_n28_), .O(new_n51_));
  inv1   g36(.a(new_n51_), .O(new_n52_));
  oai21  g37(.a(new_n18_), .b(new_n17_), .c(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  oai22  g39(.a(new_n54_), .b(new_n52_), .c(new_n50_), .d(new_n47_), .O(z4));
  oai21  g40(.a(new_n49_), .b(new_n19_), .c(new_n25_), .O(new_n56_));
  aoi21  g41(.a(new_n17_), .b(x2), .c(x0), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n30_), .c(new_n31_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n56_), .O(z5));
  inv1   g44(.a(x4), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(x3), .c(x1), .O(new_n61_));
  oai22  g46(.a(new_n61_), .b(new_n54_), .c(new_n50_), .d(new_n19_), .O(z6));
endmodule


