// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x1), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x0), .O(new_n17_));
  nand2  g02(.a(x3), .b(x2), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(new_n20_), .O(z0));
  inv1   g06(.a(x0), .O(new_n22_));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  nand2  g08(.a(x2), .b(new_n16_), .O(new_n24_));
  nor2   g09(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n26_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n16_), .c(x2), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n25_), .c(new_n22_), .O(new_n29_));
  inv1   g14(.a(x2), .O(new_n30_));
  nand4  g15(.a(x3), .b(new_n30_), .c(new_n16_), .d(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n29_), .O(z1));
  nand2  g17(.a(new_n30_), .b(new_n22_), .O(new_n33_));
  nor2   g18(.a(x4), .b(x3), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(new_n33_), .c(new_n18_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x1), .O(new_n36_));
  aoi21  g21(.a(x3), .b(new_n16_), .c(x2), .O(new_n37_));
  or2    g22(.a(new_n37_), .b(new_n22_), .O(new_n38_));
  oai22  g23(.a(new_n33_), .b(x6), .c(x5), .d(new_n30_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n26_), .c(new_n16_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n38_), .c(new_n36_), .O(z2));
  aoi21  g26(.a(new_n23_), .b(new_n16_), .c(x0), .O(new_n42_));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n30_), .c(x1), .d(new_n22_), .O(new_n44_));
  oai21  g29(.a(new_n42_), .b(new_n30_), .c(new_n44_), .O(z3));
  oai21  g30(.a(new_n37_), .b(new_n19_), .c(new_n22_), .O(new_n46_));
  oai21  g31(.a(new_n26_), .b(x2), .c(x0), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n33_), .c(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(z4));
  inv1   g34(.a(x5), .O(new_n50_));
  oai21  g35(.a(new_n24_), .b(new_n50_), .c(new_n18_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n28_), .c(new_n22_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n31_), .O(z5));
  oai21  g38(.a(x4), .b(x2), .c(new_n26_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n17_), .O(new_n55_));
  nand3  g40(.a(new_n23_), .b(x2), .c(new_n16_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n38_), .O(z6));
endmodule


