// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:12 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  oai21  g02(.a(x3), .b(new_n17_), .c(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n16_), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand3  g05(.a(x3), .b(x2), .c(new_n20_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n19_), .O(z0));
  inv1   g07(.a(x3), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x1), .c(x0), .O(new_n24_));
  nand2  g09(.a(new_n16_), .b(new_n20_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(new_n24_), .c(new_n17_), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(z1));
  nand2  g12(.a(x1), .b(new_n20_), .O(new_n28_));
  aoi21  g13(.a(x4), .b(new_n17_), .c(x3), .O(new_n29_));
  nand2  g14(.a(x3), .b(new_n17_), .O(new_n30_));
  nand2  g15(.a(new_n16_), .b(x0), .O(new_n31_));
  oai22  g16(.a(new_n31_), .b(new_n30_), .c(new_n29_), .d(new_n28_), .O(z2));
  oai21  g17(.a(x4), .b(x3), .c(x7), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n17_), .c(new_n20_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n19_), .O(z3));
  nand2  g20(.a(new_n24_), .b(new_n17_), .O(new_n36_));
  nand2  g21(.a(new_n18_), .b(x3), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n36_), .c(new_n19_), .O(z4));
  nor2   g23(.a(x3), .b(new_n17_), .O(new_n39_));
  xor2a  g24(.a(x3), .b(x2), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n16_), .c(x0), .O(new_n41_));
  oai21  g26(.a(new_n28_), .b(new_n39_), .c(new_n41_), .O(z5));
  nor2   g27(.a(new_n40_), .b(new_n20_), .O(new_n43_));
  nor2   g28(.a(x4), .b(x2), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(x3), .c(new_n20_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(x1), .c(new_n43_), .O(z6));
endmodule


