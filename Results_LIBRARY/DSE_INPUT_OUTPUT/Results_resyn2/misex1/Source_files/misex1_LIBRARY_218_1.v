// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:10 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nor2   g05(.a(x6), .b(x1), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x0), .O(new_n24_));
  nand2  g09(.a(new_n16_), .b(x1), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  nand2  g11(.a(new_n21_), .b(x2), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n24_), .O(new_n29_));
  nand3  g14(.a(x3), .b(new_n16_), .c(x0), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n21_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(z1));
  inv1   g18(.a(x4), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x2), .c(new_n20_), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nor2   g21(.a(x3), .b(x1), .O(new_n37_));
  aoi22  g22(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(x1), .O(new_n38_));
  oai21  g23(.a(new_n31_), .b(x6), .c(new_n17_), .O(new_n39_));
  oai21  g24(.a(new_n38_), .b(x0), .c(new_n39_), .O(z2));
  oai21  g25(.a(x4), .b(x3), .c(x7), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(new_n16_), .c(x1), .d(new_n24_), .O(new_n42_));
  nand2  g27(.a(x5), .b(new_n24_), .O(new_n43_));
  nor2   g28(.a(x3), .b(new_n16_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n43_), .c(new_n21_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n42_), .O(z3));
  inv1   g31(.a(x6), .O(new_n47_));
  aoi21  g32(.a(new_n20_), .b(x2), .c(new_n17_), .O(new_n48_));
  xor2a  g33(.a(x3), .b(x2), .O(new_n49_));
  inv1   g34(.a(new_n49_), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n47_), .c(new_n48_), .O(new_n51_));
  oai21  g36(.a(new_n44_), .b(new_n31_), .c(new_n21_), .O(new_n52_));
  oai21  g37(.a(new_n51_), .b(x0), .c(new_n52_), .O(z4));
  inv1   g38(.a(x5), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(x1), .c(new_n20_), .O(new_n55_));
  nor2   g40(.a(x6), .b(new_n16_), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n55_), .c(new_n48_), .O(new_n57_));
  nand3  g42(.a(new_n49_), .b(new_n21_), .c(x0), .O(new_n58_));
  oai21  g43(.a(new_n57_), .b(x0), .c(new_n58_), .O(z5));
  oai21  g44(.a(x4), .b(x2), .c(new_n20_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n18_), .O(new_n61_));
  nand2  g46(.a(new_n49_), .b(x0), .O(new_n62_));
  aoi21  g47(.a(new_n26_), .b(x2), .c(x6), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n61_), .O(z6));
endmodule


