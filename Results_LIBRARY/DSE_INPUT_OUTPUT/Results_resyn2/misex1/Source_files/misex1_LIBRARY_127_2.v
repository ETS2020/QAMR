// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x4), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(new_n17_), .c(new_n16_), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(x0), .O(new_n21_));
  nor2   g06(.a(x3), .b(x1), .O(new_n22_));
  or2    g07(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n19_), .c(x2), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(z0));
  oai21  g10(.a(x4), .b(new_n17_), .c(x1), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n17_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n26_), .c(x2), .O(new_n28_));
  nand2  g13(.a(new_n18_), .b(x3), .O(new_n29_));
  inv1   g14(.a(x5), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n17_), .O(new_n31_));
  nand4  g16(.a(new_n31_), .b(new_n29_), .c(x2), .d(new_n20_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n28_), .c(new_n16_), .O(new_n34_));
  inv1   g19(.a(x2), .O(new_n35_));
  nand4  g20(.a(x4), .b(x3), .c(new_n35_), .d(x0), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n20_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n34_), .O(z1));
  nand2  g24(.a(x5), .b(x2), .O(new_n40_));
  nand2  g25(.a(x6), .b(new_n35_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n22_), .O(new_n42_));
  nand2  g27(.a(new_n17_), .b(x2), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(x4), .c(x1), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n16_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n38_), .O(z2));
  aoi21  g32(.a(x5), .b(new_n16_), .c(new_n43_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n20_), .O(new_n49_));
  nand2  g34(.a(x7), .b(x4), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n21_), .c(new_n35_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n49_), .c(new_n29_), .O(z3));
  nand3  g37(.a(x4), .b(x3), .c(x2), .O(new_n53_));
  inv1   g38(.a(new_n53_), .O(new_n54_));
  nor2   g39(.a(x3), .b(x2), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n18_), .b(new_n16_), .c(new_n43_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(x1), .O(new_n58_));
  nor2   g43(.a(new_n18_), .b(new_n17_), .O(new_n59_));
  nand4  g44(.a(new_n59_), .b(new_n35_), .c(new_n20_), .d(new_n16_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(z4));
  oai21  g47(.a(new_n54_), .b(new_n28_), .c(new_n16_), .O(new_n63_));
  nor2   g48(.a(x5), .b(x0), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n43_), .c(new_n36_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n20_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n63_), .O(z5));
  oai21  g52(.a(new_n48_), .b(new_n37_), .c(new_n20_), .O(new_n68_));
  nor3   g53(.a(x4), .b(x3), .c(x2), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n59_), .c(new_n21_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n68_), .O(z6));
endmodule


