// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x1), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x0), .O(new_n17_));
  nor2   g02(.a(x3), .b(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(new_n17_), .O(new_n19_));
  oai21  g04(.a(x3), .b(x0), .c(x2), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n19_), .O(z0));
  inv1   g06(.a(x2), .O(new_n22_));
  inv1   g07(.a(x7), .O(new_n23_));
  nand3  g08(.a(x3), .b(new_n16_), .c(x0), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n22_), .O(new_n26_));
  inv1   g11(.a(x0), .O(new_n27_));
  inv1   g12(.a(x3), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n28_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n16_), .c(x2), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  nand2  g16(.a(x2), .b(new_n16_), .O(new_n32_));
  aoi21  g17(.a(new_n31_), .b(new_n28_), .c(new_n32_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n30_), .c(new_n27_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n26_), .O(z1));
  nand2  g20(.a(x4), .b(new_n22_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n28_), .c(new_n16_), .O(new_n37_));
  nand2  g22(.a(x5), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n18_), .O(new_n39_));
  aoi21  g24(.a(x6), .b(new_n22_), .c(new_n39_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n37_), .c(new_n27_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n26_), .O(z2));
  oai21  g27(.a(new_n17_), .b(x7), .c(new_n22_), .O(new_n43_));
  nor2   g28(.a(new_n31_), .b(x0), .O(new_n44_));
  nand2  g29(.a(new_n18_), .b(x2), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(z3));
  nand2  g31(.a(new_n23_), .b(new_n22_), .O(new_n47_));
  aoi21  g32(.a(x3), .b(new_n16_), .c(new_n47_), .O(new_n48_));
  nor2   g33(.a(new_n28_), .b(new_n22_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n48_), .c(new_n27_), .O(new_n50_));
  nand2  g35(.a(new_n28_), .b(x2), .O(new_n51_));
  nand3  g36(.a(new_n23_), .b(x3), .c(new_n22_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g38(.a(new_n51_), .b(new_n27_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n53_), .c(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n50_), .O(z4));
  aoi21  g41(.a(new_n52_), .b(new_n51_), .c(new_n27_), .O(new_n57_));
  nand3  g42(.a(x5), .b(x2), .c(new_n27_), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n57_), .c(new_n16_), .O(new_n60_));
  inv1   g45(.a(new_n49_), .O(new_n61_));
  inv1   g46(.a(x6), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(x3), .c(new_n16_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n23_), .c(new_n22_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n27_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n60_), .O(z5));
  nand3  g52(.a(new_n31_), .b(new_n28_), .c(x2), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n57_), .c(new_n16_), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n28_), .O(new_n71_));
  nand2  g56(.a(x7), .b(new_n22_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n71_), .c(new_n17_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n70_), .O(z6));
endmodule


