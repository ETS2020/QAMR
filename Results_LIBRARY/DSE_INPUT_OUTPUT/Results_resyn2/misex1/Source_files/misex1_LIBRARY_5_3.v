// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x0), .O(new_n16_));
  nor2   g01(.a(x1), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(x6), .O(new_n21_));
  inv1   g06(.a(x1), .O(new_n22_));
  nand2  g07(.a(x3), .b(new_n22_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n21_), .c(x2), .O(new_n24_));
  nor2   g09(.a(new_n24_), .b(new_n20_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nand2  g11(.a(x1), .b(new_n16_), .O(new_n27_));
  nand2  g12(.a(new_n17_), .b(x3), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nor2   g15(.a(x5), .b(x3), .O(new_n31_));
  nand2  g16(.a(x2), .b(new_n22_), .O(new_n32_));
  oai22  g17(.a(new_n32_), .b(new_n31_), .c(new_n21_), .d(x3), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  nand2  g19(.a(x6), .b(x2), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n34_), .c(new_n30_), .O(z1));
  nand2  g21(.a(x4), .b(new_n26_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n18_), .c(new_n22_), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nand2  g24(.a(x6), .b(new_n26_), .O(new_n40_));
  nor2   g25(.a(x3), .b(x1), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n38_), .c(new_n16_), .O(new_n44_));
  nand2  g29(.a(x3), .b(new_n26_), .O(new_n45_));
  inv1   g30(.a(new_n45_), .O(new_n46_));
  aoi22  g31(.a(new_n46_), .b(new_n17_), .c(x6), .d(x2), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n44_), .O(z2));
  nand2  g33(.a(x5), .b(new_n16_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n21_), .c(new_n18_), .d(x2), .O(new_n50_));
  oai21  g35(.a(x4), .b(x3), .c(x7), .O(new_n51_));
  nor2   g36(.a(x2), .b(x0), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n51_), .c(x1), .O(new_n53_));
  oai21  g38(.a(new_n50_), .b(x1), .c(new_n53_), .O(z3));
  nand2  g39(.a(new_n18_), .b(x2), .O(new_n55_));
  nand3  g40(.a(x3), .b(new_n26_), .c(x0), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n22_), .O(new_n58_));
  nand2  g43(.a(new_n52_), .b(new_n23_), .O(new_n59_));
  oai21  g44(.a(new_n19_), .b(x6), .c(x2), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(z4));
  aoi21  g46(.a(new_n55_), .b(new_n45_), .c(new_n16_), .O(new_n62_));
  nand3  g47(.a(x5), .b(x2), .c(new_n16_), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n62_), .c(new_n22_), .O(new_n65_));
  oai22  g50(.a(new_n21_), .b(x3), .c(x2), .d(new_n22_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n16_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n65_), .c(new_n60_), .O(z5));
  nand2  g53(.a(new_n56_), .b(new_n50_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n22_), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n18_), .O(new_n71_));
  nand4  g56(.a(new_n71_), .b(new_n35_), .c(x1), .d(new_n16_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n70_), .O(z6));
endmodule


