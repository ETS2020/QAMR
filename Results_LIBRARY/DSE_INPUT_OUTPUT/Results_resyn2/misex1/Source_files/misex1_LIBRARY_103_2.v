// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x7), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x3), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  oai21  g07(.a(x1), .b(new_n22_), .c(new_n18_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n17_), .O(z0));
  nand2  g10(.a(x6), .b(new_n18_), .O(new_n26_));
  oai21  g11(.a(x7), .b(new_n18_), .c(x1), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n26_), .c(x2), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n18_), .O(new_n30_));
  nand4  g15(.a(new_n30_), .b(new_n17_), .c(x2), .d(new_n19_), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n28_), .c(new_n22_), .O(new_n33_));
  nor2   g18(.a(new_n16_), .b(new_n18_), .O(new_n34_));
  inv1   g19(.a(x2), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n19_), .c(x0), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n33_), .O(z1));
  oai21  g24(.a(new_n37_), .b(new_n16_), .c(x3), .O(new_n40_));
  nand2  g25(.a(x4), .b(new_n35_), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n18_), .c(new_n19_), .O(new_n42_));
  nand2  g27(.a(new_n29_), .b(x2), .O(new_n43_));
  inv1   g28(.a(x6), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n35_), .O(new_n45_));
  nand2  g30(.a(new_n18_), .b(new_n19_), .O(new_n46_));
  aoi21  g31(.a(new_n45_), .b(new_n43_), .c(new_n46_), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n42_), .c(new_n22_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n40_), .O(z2));
  nand2  g34(.a(x5), .b(new_n22_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(x2), .c(new_n19_), .O(new_n51_));
  nand2  g36(.a(x7), .b(x4), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n20_), .c(new_n35_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n51_), .c(x3), .O(z3));
  nand2  g39(.a(new_n18_), .b(x2), .O(new_n55_));
  nand3  g40(.a(x7), .b(x3), .c(new_n35_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g42(.a(new_n55_), .b(new_n22_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(new_n19_), .O(new_n59_));
  nand3  g44(.a(x3), .b(new_n35_), .c(new_n19_), .O(new_n60_));
  nand4  g45(.a(new_n60_), .b(new_n55_), .c(new_n17_), .d(new_n22_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n59_), .O(z4));
  aoi21  g47(.a(new_n56_), .b(new_n55_), .c(new_n22_), .O(new_n63_));
  nand3  g48(.a(x5), .b(new_n18_), .c(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n19_), .O(new_n66_));
  nand2  g51(.a(new_n34_), .b(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n28_), .c(new_n22_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z5));
  nand3  g55(.a(new_n29_), .b(new_n18_), .c(x2), .O(new_n71_));
  inv1   g56(.a(new_n71_), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(new_n63_), .c(new_n19_), .O(new_n73_));
  nor3   g58(.a(x4), .b(x3), .c(x2), .O(new_n74_));
  oai21  g59(.a(new_n74_), .b(new_n34_), .c(new_n20_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n73_), .O(z6));
endmodule


