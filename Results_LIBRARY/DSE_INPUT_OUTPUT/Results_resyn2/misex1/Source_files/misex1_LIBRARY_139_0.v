// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n19_), .O(new_n25_));
  nor2   g10(.a(new_n23_), .b(x1), .O(new_n26_));
  nand2  g11(.a(x3), .b(new_n16_), .O(new_n27_));
  aoi22  g12(.a(new_n27_), .b(new_n23_), .c(new_n26_), .d(new_n25_), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(new_n18_), .c(x6), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  oai21  g15(.a(new_n28_), .b(x0), .c(new_n30_), .O(z1));
  nand3  g16(.a(new_n24_), .b(new_n19_), .c(x2), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n16_), .O(new_n33_));
  nand2  g18(.a(x4), .b(new_n23_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n19_), .c(x1), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n33_), .c(new_n18_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n30_), .O(z2));
  inv1   g22(.a(x6), .O(new_n38_));
  nor2   g23(.a(new_n16_), .b(x0), .O(new_n39_));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(new_n41_));
  nand2  g26(.a(x5), .b(new_n18_), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n19_), .c(x2), .d(new_n16_), .O(new_n43_));
  oai21  g28(.a(new_n41_), .b(x2), .c(new_n43_), .O(z3));
  nand2  g29(.a(new_n19_), .b(x2), .O(new_n45_));
  nand3  g30(.a(x3), .b(new_n23_), .c(new_n16_), .O(new_n46_));
  nand2  g31(.a(new_n38_), .b(new_n23_), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n18_), .O(new_n48_));
  nand2  g33(.a(new_n45_), .b(new_n18_), .O(new_n49_));
  nand3  g34(.a(x6), .b(x3), .c(new_n23_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n49_), .c(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n48_), .O(z4));
  aoi21  g38(.a(new_n50_), .b(new_n45_), .c(new_n18_), .O(new_n54_));
  nand3  g39(.a(x5), .b(x2), .c(new_n18_), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n54_), .c(new_n16_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n48_), .O(z5));
  inv1   g43(.a(new_n32_), .O(new_n59_));
  oai21  g44(.a(new_n54_), .b(new_n59_), .c(new_n16_), .O(new_n60_));
  oai21  g45(.a(x4), .b(x2), .c(new_n19_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n47_), .c(new_n39_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z6));
endmodule


