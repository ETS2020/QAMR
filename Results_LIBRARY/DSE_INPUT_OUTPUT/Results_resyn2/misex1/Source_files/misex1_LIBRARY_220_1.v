// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x2), .O(new_n16_));
  nand2  g01(.a(x7), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  oai21  g06(.a(x3), .b(x0), .c(x2), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n21_), .c(new_n17_), .O(z0));
  inv1   g08(.a(x6), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(x3), .c(new_n18_), .O(new_n25_));
  nor2   g10(.a(x7), .b(x2), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  nor2   g13(.a(new_n16_), .b(x1), .O(new_n29_));
  aoi22  g14(.a(new_n29_), .b(new_n28_), .c(new_n26_), .d(new_n25_), .O(new_n30_));
  nand3  g15(.a(new_n26_), .b(x3), .c(x0), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n18_), .O(new_n33_));
  oai21  g18(.a(new_n30_), .b(x0), .c(new_n33_), .O(z1));
  inv1   g19(.a(x0), .O(new_n35_));
  inv1   g20(.a(x3), .O(new_n36_));
  nand2  g21(.a(x4), .b(new_n16_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n36_), .c(new_n18_), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n36_), .c(new_n18_), .O(new_n40_));
  aoi21  g25(.a(x6), .b(new_n16_), .c(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n38_), .c(new_n35_), .O(new_n42_));
  nand2  g27(.a(x3), .b(x0), .O(new_n43_));
  nor2   g28(.a(new_n43_), .b(x1), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(x7), .c(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n42_), .O(z2));
  nand2  g31(.a(new_n26_), .b(new_n19_), .O(new_n47_));
  inv1   g32(.a(x5), .O(new_n48_));
  nor2   g33(.a(new_n48_), .b(x0), .O(new_n49_));
  nand2  g34(.a(new_n20_), .b(x2), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n49_), .c(new_n47_), .O(z3));
  nor2   g36(.a(x3), .b(new_n16_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n32_), .c(new_n18_), .O(new_n53_));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  oai21  g39(.a(x2), .b(new_n18_), .c(new_n54_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n17_), .c(new_n35_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n53_), .O(z4));
  xnor2a g42(.a(x3), .b(x2), .O(new_n58_));
  nand3  g43(.a(x5), .b(x2), .c(new_n35_), .O(new_n59_));
  oai21  g44(.a(new_n58_), .b(new_n35_), .c(new_n59_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n18_), .O(new_n61_));
  nand2  g46(.a(x6), .b(new_n36_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n16_), .c(new_n18_), .O(new_n63_));
  nor2   g48(.a(new_n52_), .b(x0), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n61_), .c(new_n17_), .O(z5));
  aoi22  g51(.a(new_n54_), .b(x0), .c(new_n27_), .d(x2), .O(new_n67_));
  oai21  g52(.a(x4), .b(x2), .c(new_n36_), .O(new_n68_));
  aoi22  g53(.a(new_n68_), .b(new_n19_), .c(x7), .d(new_n16_), .O(new_n69_));
  oai21  g54(.a(new_n67_), .b(x1), .c(new_n69_), .O(z6));
endmodule


