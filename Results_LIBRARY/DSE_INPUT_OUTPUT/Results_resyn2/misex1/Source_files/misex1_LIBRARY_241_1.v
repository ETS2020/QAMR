// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x2), .O(new_n16_));
  nand2  g01(.a(x5), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  oai21  g06(.a(x3), .b(x0), .c(x2), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n21_), .c(new_n17_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(x3), .b(x2), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n24_), .c(x0), .O(new_n26_));
  inv1   g11(.a(x3), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x2), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n24_), .c(x0), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n26_), .c(new_n18_), .O(new_n31_));
  aoi21  g16(.a(x6), .b(new_n27_), .c(x1), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x0), .c(new_n24_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(z1));
  inv1   g20(.a(x0), .O(new_n36_));
  inv1   g21(.a(x4), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n27_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n16_), .c(x1), .O(new_n39_));
  nand2  g24(.a(x6), .b(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n20_), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n39_), .c(x5), .O(new_n42_));
  inv1   g27(.a(new_n25_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x1), .O(new_n44_));
  inv1   g29(.a(new_n44_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n42_), .c(new_n36_), .O(new_n46_));
  nand2  g31(.a(new_n30_), .b(new_n18_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(z2));
  nand2  g33(.a(x5), .b(new_n36_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n20_), .c(x2), .O(new_n50_));
  nand2  g35(.a(new_n38_), .b(x7), .O(new_n51_));
  nor2   g36(.a(x5), .b(x0), .O(new_n52_));
  nand4  g37(.a(new_n52_), .b(new_n51_), .c(new_n16_), .d(x1), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n50_), .O(z3));
  nand2  g39(.a(x3), .b(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n27_), .b(x2), .O(new_n56_));
  oai21  g41(.a(new_n55_), .b(new_n36_), .c(new_n56_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n18_), .O(new_n58_));
  nand2  g43(.a(new_n28_), .b(new_n18_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n56_), .c(new_n36_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n58_), .c(new_n17_), .O(z4));
  nand3  g46(.a(new_n56_), .b(new_n55_), .c(x0), .O(new_n62_));
  nor2   g47(.a(new_n52_), .b(x1), .O(new_n63_));
  aoi22  g48(.a(new_n63_), .b(new_n62_), .c(new_n43_), .d(new_n36_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n34_), .O(z5));
  nand3  g50(.a(new_n57_), .b(new_n49_), .c(new_n18_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n27_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n19_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n66_), .c(new_n17_), .O(z6));
endmodule


