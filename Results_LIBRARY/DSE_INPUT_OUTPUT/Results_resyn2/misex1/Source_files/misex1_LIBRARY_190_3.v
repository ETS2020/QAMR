// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  oai21  g02(.a(x1), .b(new_n16_), .c(new_n17_), .O(new_n18_));
  nand2  g03(.a(x1), .b(new_n16_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x3), .O(new_n20_));
  nand2  g05(.a(x7), .b(x6), .O(new_n21_));
  nand4  g06(.a(new_n21_), .b(new_n20_), .c(new_n18_), .d(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n17_), .c(x2), .O(new_n25_));
  inv1   g10(.a(x1), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n17_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  aoi21  g14(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n25_), .O(new_n31_));
  nand4  g16(.a(x3), .b(new_n27_), .c(new_n26_), .d(x0), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n31_), .c(new_n21_), .O(z1));
  aoi21  g18(.a(x4), .b(new_n27_), .c(x3), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(new_n19_), .c(new_n32_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n21_), .O(new_n36_));
  oai21  g21(.a(x7), .b(new_n27_), .c(x6), .O(new_n37_));
  nand2  g22(.a(x5), .b(x2), .O(new_n38_));
  nor3   g23(.a(x3), .b(x1), .c(x0), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n36_), .O(z2));
  nand2  g26(.a(x5), .b(new_n16_), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n17_), .c(x2), .d(new_n26_), .O(new_n43_));
  inv1   g28(.a(new_n19_), .O(new_n44_));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n44_), .c(new_n27_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n43_), .c(new_n21_), .O(z3));
  aoi21  g32(.a(new_n17_), .b(x2), .c(x0), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(new_n49_));
  xnor2a g34(.a(x3), .b(x2), .O(new_n50_));
  or2    g35(.a(new_n50_), .b(x1), .O(new_n51_));
  nand4  g36(.a(x3), .b(new_n27_), .c(new_n26_), .d(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  aoi21  g38(.a(new_n51_), .b(new_n49_), .c(new_n53_), .O(z4));
  nor2   g39(.a(new_n50_), .b(new_n16_), .O(new_n55_));
  nand3  g40(.a(x5), .b(x2), .c(new_n16_), .O(new_n56_));
  inv1   g41(.a(new_n56_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n55_), .c(new_n26_), .O(new_n58_));
  inv1   g43(.a(new_n21_), .O(new_n59_));
  aoi21  g44(.a(new_n48_), .b(new_n29_), .c(new_n59_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n58_), .O(z5));
  inv1   g46(.a(new_n25_), .O(new_n62_));
  oai21  g47(.a(new_n55_), .b(new_n62_), .c(new_n26_), .O(new_n63_));
  oai21  g48(.a(x4), .b(x2), .c(new_n17_), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n44_), .c(new_n59_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n63_), .O(z6));
endmodule


