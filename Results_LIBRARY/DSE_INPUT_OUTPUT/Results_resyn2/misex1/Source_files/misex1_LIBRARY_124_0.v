// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand4  g05(.a(x7), .b(new_n20_), .c(new_n17_), .d(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  nor2   g07(.a(new_n20_), .b(x2), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x0), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(x7), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n17_), .O(new_n26_));
  inv1   g11(.a(x0), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x1), .c(x2), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n20_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n16_), .c(new_n17_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n26_), .O(z1));
  nand2  g18(.a(x4), .b(new_n16_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n20_), .c(new_n17_), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n20_), .c(new_n17_), .O(new_n37_));
  aoi21  g22(.a(x6), .b(new_n16_), .c(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n35_), .c(new_n27_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n26_), .O(z2));
  inv1   g25(.a(x7), .O(new_n41_));
  nand2  g26(.a(x5), .b(new_n27_), .O(new_n42_));
  nand2  g27(.a(new_n20_), .b(x2), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(new_n45_));
  nand2  g30(.a(new_n20_), .b(x1), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(x4), .c(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n16_), .c(new_n27_), .O(new_n48_));
  oai21  g33(.a(new_n45_), .b(x1), .c(new_n48_), .O(z3));
  oai21  g34(.a(new_n23_), .b(new_n41_), .c(new_n17_), .O(new_n50_));
  nor2   g35(.a(new_n44_), .b(x0), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g37(.a(new_n43_), .b(new_n24_), .O(new_n53_));
  nor2   g38(.a(new_n41_), .b(x1), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n52_), .O(z4));
  nand2  g41(.a(x3), .b(new_n16_), .O(new_n57_));
  aoi21  g42(.a(new_n43_), .b(new_n57_), .c(new_n27_), .O(new_n58_));
  oai21  g43(.a(new_n42_), .b(new_n16_), .c(x7), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n58_), .c(new_n17_), .O(new_n60_));
  nand2  g45(.a(new_n51_), .b(new_n31_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n60_), .O(z5));
  nand2  g47(.a(new_n28_), .b(x2), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(x7), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n58_), .c(new_n17_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n20_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n18_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z6));
endmodule


