// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:33 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand4  g01(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(z0));
  inv1   g03(.a(x2), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand2  g05(.a(x3), .b(new_n20_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(new_n22_));
  inv1   g07(.a(x6), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x3), .c(new_n20_), .O(new_n24_));
  nor2   g09(.a(x2), .b(x0), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n24_), .c(new_n22_), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(x2), .c(new_n20_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n26_), .O(z1));
  aoi21  g15(.a(x4), .b(new_n19_), .c(x3), .O(new_n31_));
  inv1   g16(.a(x3), .O(new_n32_));
  nand4  g17(.a(new_n23_), .b(new_n32_), .c(new_n19_), .d(new_n20_), .O(new_n33_));
  oai21  g18(.a(new_n31_), .b(new_n20_), .c(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  nand3  g20(.a(new_n27_), .b(x2), .c(new_n20_), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  nor2   g22(.a(new_n37_), .b(new_n22_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n35_), .O(z2));
  aoi21  g24(.a(new_n27_), .b(new_n20_), .c(x0), .O(new_n40_));
  oai21  g25(.a(x4), .b(x3), .c(x7), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n25_), .c(x1), .O(new_n42_));
  oai21  g27(.a(new_n40_), .b(new_n19_), .c(new_n42_), .O(z3));
  inv1   g28(.a(new_n22_), .O(new_n44_));
  nand3  g29(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(new_n45_));
  oai21  g30(.a(x3), .b(new_n20_), .c(x2), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(z4));
  inv1   g32(.a(x5), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(x1), .c(new_n32_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n26_), .O(z5));
  nor2   g36(.a(x4), .b(x2), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(x3), .c(x1), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n36_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n16_), .O(new_n55_));
  nand4  g40(.a(x3), .b(new_n19_), .c(new_n20_), .d(x0), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(z6));
endmodule


