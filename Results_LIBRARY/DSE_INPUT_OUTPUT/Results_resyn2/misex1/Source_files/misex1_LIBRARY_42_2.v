// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x1), .b(new_n16_), .O(new_n17_));
  nor2   g02(.a(x3), .b(x1), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  oai21  g04(.a(x3), .b(x0), .c(x2), .O(new_n20_));
  aoi21  g05(.a(new_n19_), .b(new_n17_), .c(new_n20_), .O(z0));
  nor2   g06(.a(x1), .b(x0), .O(new_n22_));
  oai21  g07(.a(x5), .b(x3), .c(new_n22_), .O(new_n23_));
  inv1   g08(.a(x3), .O(new_n24_));
  nand2  g09(.a(x6), .b(new_n24_), .O(new_n25_));
  inv1   g10(.a(x1), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n16_), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x2), .O(new_n28_));
  aoi22  g13(.a(new_n28_), .b(new_n25_), .c(new_n23_), .d(x2), .O(z1));
  inv1   g14(.a(x4), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x2), .c(new_n24_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(new_n32_));
  nand2  g17(.a(x5), .b(x2), .O(new_n33_));
  inv1   g18(.a(x2), .O(new_n34_));
  nand2  g19(.a(x6), .b(new_n34_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n33_), .c(new_n18_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n32_), .c(x0), .O(z2));
  oai21  g22(.a(x4), .b(x3), .c(x7), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n34_), .c(x1), .O(new_n39_));
  oai21  g24(.a(new_n18_), .b(new_n34_), .c(x0), .O(new_n40_));
  inv1   g25(.a(x5), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(new_n24_), .c(x2), .d(new_n26_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(z3));
  nand2  g28(.a(new_n27_), .b(new_n34_), .O(new_n44_));
  nand3  g29(.a(x3), .b(x2), .c(new_n16_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n44_), .c(new_n19_), .O(z4));
  nand3  g31(.a(new_n25_), .b(new_n22_), .c(new_n34_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n26_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n24_), .c(x2), .d(new_n16_), .O(new_n49_));
  nand3  g34(.a(new_n24_), .b(x2), .c(new_n26_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x0), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n49_), .c(new_n47_), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(z5));
  aoi21  g38(.a(new_n30_), .b(new_n34_), .c(x3), .O(new_n54_));
  nor2   g39(.a(new_n41_), .b(x0), .O(new_n55_));
  oai22  g40(.a(new_n55_), .b(new_n50_), .c(new_n54_), .d(new_n17_), .O(z6));
endmodule


