// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  inv1   g04(.a(x5), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nand3  g06(.a(x3), .b(x2), .c(new_n21_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x1), .O(new_n24_));
  oai21  g09(.a(new_n19_), .b(new_n17_), .c(new_n24_), .O(z0));
  nand3  g10(.a(new_n20_), .b(new_n18_), .c(x2), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x3), .b(new_n27_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n28_), .c(new_n26_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n16_), .O(new_n32_));
  oai21  g17(.a(x5), .b(x2), .c(x1), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n32_), .c(new_n21_), .O(new_n34_));
  inv1   g19(.a(new_n17_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(x3), .c(new_n27_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(z1));
  nand2  g22(.a(x1), .b(new_n21_), .O(new_n38_));
  oai21  g23(.a(new_n17_), .b(x2), .c(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x3), .O(new_n40_));
  nor2   g25(.a(x2), .b(x0), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x4), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n20_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x1), .O(new_n44_));
  oai21  g29(.a(x5), .b(new_n27_), .c(new_n30_), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n18_), .c(new_n16_), .d(new_n21_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n44_), .c(new_n40_), .O(z2));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n41_), .c(x5), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n21_), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n18_), .c(x2), .d(new_n16_), .O(new_n51_));
  oai21  g36(.a(new_n49_), .b(new_n16_), .c(new_n51_), .O(z3));
  nand2  g37(.a(new_n28_), .b(new_n21_), .O(new_n53_));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(x0), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n53_), .c(new_n26_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n16_), .O(new_n57_));
  inv1   g42(.a(new_n38_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n19_), .c(new_n20_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n57_), .O(z4));
  nand3  g45(.a(new_n32_), .b(new_n26_), .c(new_n21_), .O(new_n61_));
  nor2   g46(.a(new_n20_), .b(new_n16_), .O(new_n62_));
  aoi21  g47(.a(new_n54_), .b(new_n35_), .c(new_n62_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(z5));
  nand2  g49(.a(new_n55_), .b(new_n26_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n16_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n18_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n58_), .c(new_n62_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n66_), .O(z6));
endmodule


