// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand2  g03(.a(x3), .b(new_n18_), .O(new_n19_));
  inv1   g04(.a(x7), .O(new_n20_));
  nand2  g05(.a(x3), .b(new_n16_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(x2), .O(new_n22_));
  aoi21  g07(.a(new_n19_), .b(new_n17_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x3), .O(new_n24_));
  nand2  g09(.a(x6), .b(new_n24_), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n16_), .c(x2), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  nand3  g12(.a(new_n20_), .b(x2), .c(new_n16_), .O(new_n28_));
  nor2   g13(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n26_), .c(new_n18_), .O(new_n30_));
  inv1   g15(.a(x2), .O(new_n31_));
  nand3  g16(.a(x3), .b(new_n31_), .c(x0), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n30_), .O(z1));
  inv1   g20(.a(x6), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n31_), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand3  g23(.a(new_n20_), .b(new_n38_), .c(x2), .O(new_n39_));
  nand2  g24(.a(new_n24_), .b(new_n16_), .O(new_n40_));
  aoi21  g25(.a(new_n39_), .b(new_n37_), .c(new_n40_), .O(new_n41_));
  nand2  g26(.a(new_n20_), .b(x3), .O(new_n42_));
  oai21  g27(.a(x4), .b(x3), .c(x1), .O(new_n43_));
  aoi21  g28(.a(new_n42_), .b(x2), .c(new_n43_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n41_), .c(new_n18_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n34_), .O(z2));
  inv1   g31(.a(new_n40_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n18_), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n47_), .c(x7), .O(new_n49_));
  oai21  g34(.a(x4), .b(x3), .c(x7), .O(new_n50_));
  nor2   g35(.a(x2), .b(x0), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(x1), .O(new_n52_));
  oai21  g37(.a(new_n49_), .b(new_n31_), .c(new_n52_), .O(z3));
  nand2  g38(.a(new_n24_), .b(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n32_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n51_), .b(new_n21_), .O(new_n57_));
  nand2  g42(.a(new_n19_), .b(new_n20_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(x2), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(z4));
  nand2  g45(.a(x3), .b(new_n31_), .O(new_n61_));
  aoi21  g46(.a(new_n54_), .b(new_n61_), .c(new_n18_), .O(new_n62_));
  nand3  g47(.a(x5), .b(x2), .c(new_n18_), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n62_), .c(new_n16_), .O(new_n65_));
  nand2  g50(.a(new_n26_), .b(new_n18_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n65_), .c(new_n59_), .O(z5));
  xor2a  g52(.a(x3), .b(x2), .O(new_n68_));
  aoi22  g53(.a(new_n68_), .b(x0), .c(new_n27_), .d(x2), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n24_), .O(new_n70_));
  nor2   g55(.a(new_n16_), .b(x0), .O(new_n71_));
  aoi22  g56(.a(new_n71_), .b(new_n70_), .c(x7), .d(x2), .O(new_n72_));
  oai21  g57(.a(new_n69_), .b(x1), .c(new_n72_), .O(z6));
endmodule


