// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  inv1   g03(.a(x4), .O(new_n19_));
  nand4  g04(.a(new_n19_), .b(new_n18_), .c(new_n17_), .d(x0), .O(new_n20_));
  nor2   g05(.a(new_n17_), .b(x0), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x3), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n20_), .c(new_n16_), .O(z0));
  nand3  g08(.a(x3), .b(new_n16_), .c(x0), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n19_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n17_), .O(new_n26_));
  inv1   g11(.a(x0), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x1), .c(x2), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n18_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n16_), .c(new_n17_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n26_), .O(z1));
  inv1   g18(.a(x5), .O(new_n34_));
  nor2   g19(.a(x6), .b(x2), .O(new_n35_));
  aoi21  g20(.a(new_n34_), .b(x2), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n18_), .b(new_n17_), .O(new_n37_));
  nor2   g22(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nor2   g23(.a(new_n19_), .b(x2), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n38_), .c(new_n27_), .O(new_n40_));
  nand3  g25(.a(new_n16_), .b(new_n17_), .c(x0), .O(new_n41_));
  oai21  g26(.a(new_n17_), .b(x0), .c(new_n41_), .O(new_n42_));
  aoi22  g27(.a(new_n42_), .b(x3), .c(x4), .d(new_n17_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n40_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nor2   g30(.a(x2), .b(x0), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n45_), .c(x1), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n27_), .O(new_n48_));
  nor2   g33(.a(x4), .b(x1), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n48_), .c(new_n18_), .d(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n47_), .O(z3));
  oai21  g36(.a(new_n18_), .b(x0), .c(new_n37_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(x2), .O(new_n53_));
  nand2  g38(.a(new_n49_), .b(x3), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n53_), .c(new_n26_), .O(z4));
  xor2a  g41(.a(x3), .b(x2), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(x0), .O(new_n58_));
  nand3  g43(.a(x5), .b(x2), .c(new_n27_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(x1), .O(new_n60_));
  nor3   g45(.a(new_n57_), .b(new_n35_), .c(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n60_), .c(new_n19_), .O(new_n62_));
  oai21  g47(.a(x3), .b(new_n16_), .c(new_n21_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n62_), .O(z5));
  oai21  g49(.a(x4), .b(x2), .c(new_n18_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n21_), .O(new_n66_));
  aoi21  g51(.a(new_n28_), .b(x2), .c(x4), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n58_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n17_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z6));
endmodule


