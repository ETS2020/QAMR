// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nor2   g02(.a(new_n16_), .b(x0), .O(new_n18_));
  aoi21  g03(.a(new_n17_), .b(new_n16_), .c(new_n18_), .O(new_n19_));
  oai21  g04(.a(x3), .b(x0), .c(x2), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n19_), .O(z0));
  inv1   g06(.a(x0), .O(new_n22_));
  inv1   g07(.a(x5), .O(new_n23_));
  nand2  g08(.a(x2), .b(new_n16_), .O(new_n24_));
  aoi21  g09(.a(new_n23_), .b(new_n17_), .c(new_n24_), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n17_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n16_), .c(x2), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(new_n22_), .O(new_n28_));
  inv1   g13(.a(x2), .O(new_n29_));
  nand3  g14(.a(x3), .b(new_n16_), .c(x0), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x7), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(z1));
  nand2  g18(.a(x4), .b(new_n29_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n17_), .c(new_n16_), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n17_), .c(new_n16_), .O(new_n37_));
  aoi21  g22(.a(x6), .b(new_n29_), .c(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n35_), .c(new_n22_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n32_), .O(z2));
  inv1   g25(.a(new_n24_), .O(new_n41_));
  nand2  g26(.a(x5), .b(new_n22_), .O(new_n42_));
  nor2   g27(.a(x4), .b(x2), .O(new_n43_));
  aoi22  g28(.a(new_n43_), .b(new_n18_), .c(new_n42_), .d(new_n41_), .O(new_n44_));
  inv1   g29(.a(x7), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n29_), .O(new_n46_));
  oai21  g31(.a(new_n44_), .b(x3), .c(new_n46_), .O(z3));
  nor2   g32(.a(new_n17_), .b(new_n29_), .O(new_n48_));
  nand2  g33(.a(x7), .b(new_n29_), .O(new_n49_));
  aoi21  g34(.a(x3), .b(new_n16_), .c(new_n49_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n48_), .c(new_n22_), .O(new_n51_));
  nand2  g36(.a(new_n17_), .b(x2), .O(new_n52_));
  nand3  g37(.a(x7), .b(x3), .c(new_n29_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g39(.a(new_n52_), .b(new_n22_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n51_), .O(z4));
  aoi21  g42(.a(new_n53_), .b(new_n52_), .c(new_n22_), .O(new_n58_));
  nand3  g43(.a(x5), .b(x2), .c(new_n22_), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n16_), .O(new_n61_));
  inv1   g46(.a(new_n48_), .O(new_n62_));
  inv1   g47(.a(x6), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(x3), .c(new_n16_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(x7), .c(new_n29_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n22_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n61_), .O(z5));
  nand3  g53(.a(new_n23_), .b(new_n17_), .c(x2), .O(new_n69_));
  inv1   g54(.a(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n58_), .c(new_n16_), .O(new_n71_));
  oai21  g56(.a(x4), .b(x2), .c(new_n17_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n46_), .c(new_n18_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n71_), .O(z6));
endmodule


