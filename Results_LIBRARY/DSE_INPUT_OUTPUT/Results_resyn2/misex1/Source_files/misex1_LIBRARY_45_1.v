// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x2), .O(new_n16_));
  nor2   g01(.a(x3), .b(new_n16_), .O(new_n17_));
  aoi21  g02(.a(new_n17_), .b(x0), .c(x4), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  nand2  g04(.a(x1), .b(new_n19_), .O(new_n20_));
  nand2  g05(.a(x3), .b(x2), .O(new_n21_));
  oai22  g06(.a(new_n21_), .b(new_n20_), .c(new_n18_), .d(x1), .O(z0));
  inv1   g07(.a(x4), .O(new_n23_));
  nor2   g08(.a(x5), .b(x3), .O(new_n24_));
  inv1   g09(.a(x1), .O(new_n25_));
  nand2  g10(.a(x2), .b(new_n25_), .O(new_n26_));
  inv1   g11(.a(x3), .O(new_n27_));
  nand3  g12(.a(x6), .b(new_n27_), .c(new_n16_), .O(new_n28_));
  oai21  g13(.a(new_n26_), .b(new_n24_), .c(new_n28_), .O(new_n29_));
  nor2   g14(.a(x2), .b(new_n25_), .O(new_n30_));
  aoi21  g15(.a(new_n29_), .b(new_n23_), .c(new_n30_), .O(new_n31_));
  nand3  g16(.a(x3), .b(new_n16_), .c(x0), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nor2   g18(.a(x4), .b(x1), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g20(.a(new_n31_), .b(x0), .c(new_n35_), .O(z1));
  nor2   g21(.a(new_n23_), .b(x2), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  inv1   g24(.a(x6), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n16_), .O(new_n41_));
  nand2  g26(.a(new_n27_), .b(new_n25_), .O(new_n42_));
  aoi21  g27(.a(new_n41_), .b(new_n39_), .c(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n37_), .c(new_n19_), .O(new_n44_));
  nand2  g29(.a(x4), .b(new_n25_), .O(new_n45_));
  nand3  g30(.a(new_n16_), .b(new_n25_), .c(x0), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n20_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x3), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(z2));
  oai21  g34(.a(x4), .b(x3), .c(x7), .O(new_n50_));
  nand3  g35(.a(new_n16_), .b(x1), .c(new_n19_), .O(new_n51_));
  inv1   g36(.a(new_n51_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g38(.a(x5), .b(new_n19_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n34_), .c(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(z3));
  xnor2a g41(.a(x3), .b(x2), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n45_), .c(new_n30_), .O(new_n58_));
  oai21  g43(.a(new_n33_), .b(new_n17_), .c(new_n34_), .O(new_n59_));
  oai21  g44(.a(new_n58_), .b(x0), .c(new_n59_), .O(z4));
  nor2   g45(.a(new_n57_), .b(new_n19_), .O(new_n61_));
  oai21  g46(.a(new_n54_), .b(new_n16_), .c(new_n23_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n61_), .c(new_n25_), .O(new_n63_));
  inv1   g48(.a(new_n30_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n28_), .c(new_n21_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n19_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n63_), .O(z5));
  nand3  g52(.a(new_n38_), .b(new_n27_), .c(x2), .O(new_n68_));
  oai21  g53(.a(new_n57_), .b(new_n19_), .c(new_n68_), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(new_n25_), .c(new_n52_), .O(new_n70_));
  nand3  g55(.a(x3), .b(x1), .c(new_n19_), .O(new_n71_));
  oai21  g56(.a(new_n70_), .b(x4), .c(new_n71_), .O(z6));
endmodule


