// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  nor2   g03(.a(x3), .b(x1), .O(new_n19_));
  aoi21  g04(.a(new_n19_), .b(x0), .c(x6), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g06(.a(x1), .O(new_n22_));
  inv1   g07(.a(x3), .O(new_n23_));
  nand2  g08(.a(x6), .b(new_n23_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n22_), .c(x2), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(x2), .c(new_n22_), .O(new_n28_));
  nor2   g13(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n25_), .c(new_n17_), .O(new_n30_));
  nand3  g15(.a(x3), .b(new_n16_), .c(new_n22_), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x0), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n30_), .O(z1));
  nand2  g19(.a(x3), .b(x1), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n23_), .c(new_n22_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n35_), .c(x6), .O(new_n38_));
  nor2   g23(.a(x4), .b(x3), .O(new_n39_));
  nand2  g24(.a(new_n16_), .b(x1), .O(new_n40_));
  nor2   g25(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n38_), .c(new_n17_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n33_), .O(z2));
  nand2  g28(.a(x5), .b(new_n17_), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n19_), .c(x6), .O(new_n45_));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n47_));
  oai21  g32(.a(new_n45_), .b(new_n16_), .c(new_n47_), .O(z3));
  nand2  g33(.a(x3), .b(x2), .O(new_n49_));
  oai21  g34(.a(new_n23_), .b(new_n17_), .c(new_n16_), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n49_), .c(new_n24_), .d(new_n22_), .O(new_n51_));
  nor2   g36(.a(new_n27_), .b(new_n16_), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  nand2  g38(.a(new_n23_), .b(x2), .O(new_n54_));
  nand4  g39(.a(new_n54_), .b(new_n53_), .c(new_n31_), .d(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n51_), .O(z4));
  nand2  g41(.a(x3), .b(new_n16_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n54_), .c(new_n17_), .O(new_n58_));
  nand3  g43(.a(x5), .b(x2), .c(new_n17_), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n22_), .O(new_n61_));
  nand3  g46(.a(new_n49_), .b(new_n40_), .c(new_n24_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n17_), .c(new_n52_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(z5));
  xor2a  g49(.a(x3), .b(x2), .O(new_n65_));
  aoi22  g50(.a(new_n65_), .b(x0), .c(new_n26_), .d(x2), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n23_), .O(new_n67_));
  nor2   g52(.a(new_n22_), .b(x0), .O(new_n68_));
  aoi21  g53(.a(new_n68_), .b(new_n67_), .c(new_n52_), .O(new_n69_));
  oai21  g54(.a(new_n66_), .b(x1), .c(new_n69_), .O(z6));
endmodule


