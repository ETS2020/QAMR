// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:17 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x2), .O(new_n16_));
  nor2   g01(.a(x3), .b(new_n16_), .O(new_n17_));
  aoi21  g02(.a(new_n17_), .b(x0), .c(x4), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  nand2  g04(.a(x1), .b(new_n19_), .O(new_n20_));
  inv1   g05(.a(new_n20_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x3), .c(x2), .O(new_n22_));
  oai21  g07(.a(new_n18_), .b(x1), .c(new_n22_), .O(z0));
  inv1   g08(.a(x3), .O(new_n24_));
  nor2   g09(.a(new_n24_), .b(x2), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(x0), .c(x4), .O(new_n26_));
  inv1   g11(.a(x1), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n24_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n27_), .c(new_n16_), .O(new_n30_));
  nor2   g15(.a(x4), .b(x2), .O(new_n31_));
  nand2  g16(.a(x6), .b(new_n24_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n31_), .c(new_n27_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n19_), .O(new_n34_));
  oai22  g19(.a(new_n34_), .b(new_n30_), .c(new_n26_), .d(x1), .O(z1));
  nand2  g20(.a(x4), .b(new_n16_), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nor2   g22(.a(x3), .b(x1), .O(new_n38_));
  nand2  g23(.a(x6), .b(new_n16_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n19_), .O(new_n42_));
  nand3  g27(.a(new_n16_), .b(new_n27_), .c(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n20_), .O(new_n44_));
  aoi22  g29(.a(new_n44_), .b(x3), .c(x4), .d(new_n27_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n42_), .O(z2));
  nand2  g31(.a(x5), .b(new_n19_), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n17_), .c(x4), .O(new_n48_));
  oai21  g33(.a(x4), .b(x3), .c(x7), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n21_), .c(new_n16_), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(x1), .c(new_n50_), .O(z3));
  nand2  g36(.a(x4), .b(new_n27_), .O(new_n52_));
  nor2   g37(.a(new_n24_), .b(x0), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n27_), .O(new_n54_));
  oai21  g39(.a(new_n24_), .b(x1), .c(x0), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n16_), .O(new_n56_));
  oai21  g41(.a(new_n53_), .b(new_n38_), .c(x2), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n52_), .O(z4));
  inv1   g43(.a(x4), .O(new_n59_));
  xor2a  g44(.a(x3), .b(x2), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(x0), .O(new_n61_));
  nand3  g46(.a(x5), .b(x2), .c(new_n19_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n27_), .O(new_n64_));
  inv1   g49(.a(new_n17_), .O(new_n65_));
  nand3  g50(.a(new_n33_), .b(new_n65_), .c(new_n19_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z5));
  oai21  g52(.a(new_n31_), .b(x3), .c(new_n21_), .O(new_n68_));
  nand3  g53(.a(new_n28_), .b(new_n24_), .c(x2), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n61_), .c(new_n59_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n27_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n68_), .O(z6));
endmodule


