// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  oai21  g02(.a(x1), .b(new_n16_), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(x0), .c(x3), .O(new_n20_));
  inv1   g05(.a(x6), .O(new_n21_));
  nand2  g06(.a(x7), .b(new_n21_), .O(new_n22_));
  nand4  g07(.a(new_n22_), .b(new_n20_), .c(new_n18_), .d(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  nand4  g10(.a(x3), .b(new_n25_), .c(new_n19_), .d(x0), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(new_n17_), .c(x2), .O(new_n28_));
  nand2  g13(.a(x2), .b(new_n19_), .O(new_n29_));
  nand2  g14(.a(new_n25_), .b(x1), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n28_), .c(new_n16_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n26_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n22_), .O(new_n34_));
  nor2   g19(.a(x3), .b(x2), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(x6), .c(new_n16_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(z1));
  inv1   g22(.a(new_n22_), .O(new_n38_));
  inv1   g23(.a(new_n26_), .O(new_n39_));
  aoi21  g24(.a(x4), .b(new_n25_), .c(x3), .O(new_n40_));
  nand4  g25(.a(new_n27_), .b(new_n17_), .c(x2), .d(new_n19_), .O(new_n41_));
  oai21  g26(.a(new_n40_), .b(new_n19_), .c(new_n41_), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n16_), .c(new_n39_), .O(new_n43_));
  nor2   g28(.a(x7), .b(x0), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n35_), .c(new_n21_), .d(new_n19_), .O(new_n45_));
  oai21  g30(.a(new_n43_), .b(new_n38_), .c(new_n45_), .O(z2));
  inv1   g31(.a(new_n44_), .O(new_n47_));
  nor2   g32(.a(new_n19_), .b(x0), .O(new_n48_));
  nor2   g33(.a(x4), .b(x2), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n48_), .c(x6), .O(new_n50_));
  nand2  g35(.a(x5), .b(new_n16_), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(new_n22_), .c(x2), .d(new_n19_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n17_), .O(new_n54_));
  oai21  g39(.a(new_n47_), .b(new_n30_), .c(new_n54_), .O(z3));
  nand2  g40(.a(x3), .b(new_n25_), .O(new_n56_));
  nand2  g41(.a(new_n17_), .b(x2), .O(new_n57_));
  oai21  g42(.a(new_n56_), .b(new_n16_), .c(new_n57_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n19_), .O(new_n59_));
  aoi21  g44(.a(new_n17_), .b(x2), .c(x0), .O(new_n60_));
  nand3  g45(.a(x3), .b(new_n25_), .c(new_n19_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n60_), .c(new_n38_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n59_), .O(z4));
  aoi21  g48(.a(new_n57_), .b(new_n56_), .c(new_n16_), .O(new_n64_));
  nand3  g49(.a(x5), .b(x2), .c(new_n16_), .O(new_n65_));
  inv1   g50(.a(new_n65_), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n64_), .c(new_n19_), .O(new_n67_));
  nand2  g52(.a(new_n35_), .b(x6), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n25_), .c(new_n19_), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(new_n60_), .c(new_n38_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n67_), .O(z5));
  inv1   g56(.a(new_n28_), .O(new_n72_));
  oai21  g57(.a(new_n64_), .b(new_n72_), .c(new_n19_), .O(new_n73_));
  oai21  g58(.a(x4), .b(x2), .c(new_n17_), .O(new_n74_));
  aoi21  g59(.a(new_n74_), .b(new_n48_), .c(new_n38_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n73_), .O(z6));
endmodule


