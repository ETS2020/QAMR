// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  inv1   g08(.a(x4), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n22_), .O(z0));
  nand3  g11(.a(x3), .b(new_n18_), .c(x0), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x4), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n23_), .O(new_n29_));
  nor2   g14(.a(x5), .b(x3), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x1), .c(x2), .O(new_n31_));
  nand2  g16(.a(x6), .b(new_n19_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n23_), .c(new_n18_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n31_), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n29_), .O(z1));
  nand2  g20(.a(new_n19_), .b(x2), .O(new_n36_));
  oai21  g21(.a(x5), .b(x1), .c(x2), .O(new_n37_));
  aoi21  g22(.a(x6), .b(new_n23_), .c(x3), .O(new_n38_));
  aoi22  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(x1), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(x0), .c(new_n29_), .O(z2));
  nand2  g25(.a(x1), .b(new_n16_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x7), .c(x4), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n23_), .O(new_n43_));
  nor2   g28(.a(x3), .b(new_n23_), .O(new_n44_));
  nand2  g29(.a(x5), .b(new_n16_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n44_), .c(new_n18_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n43_), .O(z3));
  nand2  g32(.a(new_n36_), .b(new_n16_), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(new_n49_));
  nand3  g34(.a(x3), .b(new_n23_), .c(new_n18_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n49_), .c(new_n25_), .O(new_n51_));
  nand3  g36(.a(x4), .b(x3), .c(new_n23_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n48_), .c(new_n18_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n51_), .O(z4));
  aoi21  g40(.a(new_n52_), .b(new_n36_), .c(new_n16_), .O(new_n56_));
  nand3  g41(.a(x5), .b(x2), .c(new_n16_), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n56_), .c(new_n18_), .O(new_n59_));
  nand3  g44(.a(new_n49_), .b(new_n33_), .c(new_n25_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n59_), .O(z5));
  nand2  g46(.a(new_n30_), .b(x2), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n56_), .c(new_n18_), .O(new_n64_));
  inv1   g49(.a(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n25_), .b(new_n65_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z6));
endmodule


