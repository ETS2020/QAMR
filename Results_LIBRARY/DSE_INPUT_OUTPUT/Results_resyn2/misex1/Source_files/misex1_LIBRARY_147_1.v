// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x5), .O(new_n16_));
  aoi21  g01(.a(new_n16_), .b(x0), .c(x1), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  inv1   g04(.a(x2), .O(new_n20_));
  aoi21  g05(.a(new_n19_), .b(x1), .c(new_n20_), .O(new_n21_));
  oai21  g06(.a(new_n19_), .b(new_n18_), .c(new_n21_), .O(new_n22_));
  nor2   g07(.a(new_n22_), .b(new_n17_), .O(z0));
  inv1   g08(.a(x1), .O(new_n24_));
  nor2   g09(.a(new_n20_), .b(new_n18_), .O(new_n25_));
  oai21  g10(.a(x2), .b(x0), .c(x3), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n25_), .c(new_n16_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n24_), .O(new_n28_));
  nor2   g13(.a(x2), .b(x0), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  nor2   g15(.a(x5), .b(x1), .O(new_n31_));
  oai21  g16(.a(new_n30_), .b(x3), .c(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n28_), .O(z1));
  inv1   g19(.a(x4), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x2), .c(new_n19_), .O(new_n36_));
  nand2  g21(.a(x6), .b(new_n20_), .O(new_n37_));
  nor3   g22(.a(x5), .b(x3), .c(x1), .O(new_n38_));
  aoi22  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(x1), .O(new_n39_));
  nand3  g24(.a(x3), .b(new_n20_), .c(x0), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n31_), .O(new_n42_));
  oai21  g27(.a(new_n39_), .b(x0), .c(new_n42_), .O(z2));
  nor2   g28(.a(x3), .b(new_n20_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(x5), .c(new_n24_), .O(new_n45_));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n29_), .c(x1), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z3));
  oai21  g33(.a(new_n19_), .b(x1), .c(x0), .O(new_n49_));
  nand3  g34(.a(x3), .b(new_n24_), .c(new_n18_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n49_), .c(new_n20_), .O(new_n51_));
  nand2  g36(.a(x5), .b(new_n24_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n51_), .c(new_n22_), .O(z4));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(x0), .c(x5), .O(new_n55_));
  nand2  g40(.a(new_n20_), .b(new_n24_), .O(new_n56_));
  aoi21  g41(.a(x6), .b(new_n19_), .c(new_n56_), .O(new_n57_));
  oai21  g42(.a(x3), .b(new_n20_), .c(new_n18_), .O(new_n58_));
  oai22  g43(.a(new_n58_), .b(new_n57_), .c(new_n55_), .d(x1), .O(z5));
  oai21  g44(.a(new_n44_), .b(new_n41_), .c(new_n31_), .O(new_n60_));
  oai21  g45(.a(x4), .b(x2), .c(new_n19_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(x1), .c(new_n18_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z6));
endmodule


