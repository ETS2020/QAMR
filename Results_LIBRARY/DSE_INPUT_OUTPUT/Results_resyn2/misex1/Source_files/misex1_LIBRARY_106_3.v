// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x6), .O(new_n16_));
  oai21  g01(.a(x3), .b(x1), .c(x0), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n21_));
  nand4  g06(.a(new_n21_), .b(new_n17_), .c(new_n16_), .d(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  nand2  g08(.a(x6), .b(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(x2), .O(new_n25_));
  inv1   g10(.a(x5), .O(new_n26_));
  nand3  g11(.a(new_n16_), .b(x2), .c(new_n19_), .O(new_n27_));
  aoi21  g12(.a(new_n26_), .b(new_n20_), .c(new_n27_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n25_), .c(new_n18_), .O(new_n29_));
  inv1   g14(.a(x2), .O(new_n30_));
  nand3  g15(.a(x3), .b(new_n30_), .c(x0), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n19_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n29_), .O(z1));
  nand3  g19(.a(x5), .b(new_n20_), .c(x2), .O(new_n35_));
  xnor2a g20(.a(x3), .b(x1), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n35_), .c(new_n16_), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n20_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n30_), .c(x1), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n37_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n18_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n33_), .O(z2));
  nor2   g28(.a(x3), .b(x1), .O(new_n44_));
  nand2  g29(.a(x5), .b(new_n18_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n44_), .c(x6), .O(new_n46_));
  nand2  g31(.a(new_n39_), .b(x7), .O(new_n47_));
  nor2   g32(.a(new_n19_), .b(x0), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n47_), .c(new_n30_), .O(new_n49_));
  oai21  g34(.a(new_n46_), .b(new_n30_), .c(new_n49_), .O(z3));
  nand2  g35(.a(new_n20_), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n31_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n19_), .O(new_n53_));
  aoi21  g38(.a(x3), .b(new_n19_), .c(x0), .O(new_n54_));
  aoi21  g39(.a(new_n20_), .b(x2), .c(x0), .O(new_n55_));
  oai22  g40(.a(new_n55_), .b(x6), .c(new_n54_), .d(x2), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n53_), .O(z4));
  nand2  g42(.a(x3), .b(new_n30_), .O(new_n58_));
  nand3  g43(.a(new_n16_), .b(new_n20_), .c(x2), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(new_n18_), .O(new_n60_));
  nand4  g45(.a(new_n16_), .b(x5), .c(x2), .d(new_n18_), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n19_), .O(new_n63_));
  nand3  g48(.a(new_n16_), .b(x3), .c(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n25_), .c(new_n18_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n63_), .O(z5));
  nor2   g52(.a(new_n26_), .b(x0), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n51_), .c(new_n31_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n19_), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n20_), .O(new_n71_));
  aoi22  g56(.a(new_n71_), .b(new_n48_), .c(x6), .d(x2), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n70_), .O(z6));
endmodule


