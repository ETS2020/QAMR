// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:43 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  nor2   g04(.a(x3), .b(x1), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(x0), .c(x5), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x0), .O(new_n23_));
  oai21  g08(.a(x5), .b(new_n16_), .c(new_n23_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x3), .c(new_n17_), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n26_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n17_), .c(x2), .O(new_n28_));
  inv1   g13(.a(new_n28_), .O(new_n29_));
  nand3  g14(.a(x3), .b(new_n16_), .c(new_n17_), .O(new_n30_));
  and2   g15(.a(new_n30_), .b(x0), .O(new_n31_));
  aoi21  g16(.a(new_n29_), .b(new_n25_), .c(new_n31_), .O(z1));
  nand2  g17(.a(x6), .b(new_n16_), .O(new_n33_));
  nand2  g18(.a(x5), .b(x2), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n33_), .c(new_n20_), .O(new_n35_));
  oai21  g20(.a(x5), .b(new_n26_), .c(x2), .O(new_n36_));
  inv1   g21(.a(x4), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n26_), .c(new_n17_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n36_), .c(x0), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n35_), .c(new_n31_), .O(z2));
  inv1   g25(.a(x5), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n26_), .c(x2), .O(new_n42_));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n18_), .c(new_n16_), .O(new_n44_));
  oai21  g29(.a(new_n42_), .b(x1), .c(new_n44_), .O(z3));
  nand3  g30(.a(x3), .b(new_n16_), .c(x0), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n42_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n17_), .O(new_n48_));
  nand3  g33(.a(new_n36_), .b(new_n30_), .c(new_n23_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(z4));
  nand3  g35(.a(new_n41_), .b(x3), .c(x2), .O(new_n51_));
  inv1   g36(.a(new_n51_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n28_), .c(new_n23_), .O(new_n53_));
  oai21  g38(.a(new_n26_), .b(x2), .c(new_n42_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n17_), .c(x0), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(z5));
  oai21  g41(.a(x3), .b(new_n16_), .c(new_n46_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n17_), .O(new_n58_));
  oai21  g43(.a(x4), .b(x2), .c(new_n26_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n18_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n58_), .c(new_n34_), .O(z6));
endmodule


