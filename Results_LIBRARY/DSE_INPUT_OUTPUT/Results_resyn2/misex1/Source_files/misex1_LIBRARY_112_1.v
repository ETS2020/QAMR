// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  nor2   g00(.a(x4), .b(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand2  g02(.a(x1), .b(new_n17_), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x3), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  inv1   g07(.a(x1), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x0), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n22_), .c(new_n21_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n20_), .c(new_n16_), .O(z0));
  inv1   g12(.a(x6), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x3), .c(new_n23_), .O(new_n29_));
  inv1   g14(.a(x5), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n22_), .O(new_n31_));
  nor2   g16(.a(new_n21_), .b(x1), .O(new_n32_));
  aoi22  g17(.a(new_n32_), .b(new_n31_), .c(new_n29_), .d(new_n16_), .O(new_n33_));
  inv1   g18(.a(x4), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(x3), .c(new_n21_), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n25_), .O(new_n37_));
  oai21  g22(.a(new_n33_), .b(x0), .c(new_n37_), .O(z1));
  oai21  g23(.a(new_n24_), .b(x2), .c(new_n18_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x3), .O(new_n40_));
  nand2  g25(.a(new_n30_), .b(x2), .O(new_n41_));
  oai21  g26(.a(x6), .b(x2), .c(new_n41_), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n22_), .c(new_n23_), .d(new_n17_), .O(new_n43_));
  nor2   g28(.a(new_n34_), .b(x2), .O(new_n44_));
  inv1   g29(.a(new_n44_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(z2));
  nand2  g31(.a(x7), .b(x3), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n19_), .c(x4), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n17_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n22_), .c(x2), .d(new_n23_), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(x2), .c(new_n50_), .O(z3));
  nand2  g36(.a(new_n22_), .b(x2), .O(new_n52_));
  nand3  g37(.a(x3), .b(new_n21_), .c(x0), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n23_), .O(new_n55_));
  aoi21  g40(.a(new_n22_), .b(x2), .c(x0), .O(new_n56_));
  nand3  g41(.a(x3), .b(new_n21_), .c(new_n23_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n56_), .c(new_n44_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n55_), .O(z4));
  aoi21  g44(.a(new_n52_), .b(new_n35_), .c(new_n17_), .O(new_n60_));
  nand3  g45(.a(x5), .b(x2), .c(new_n17_), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n23_), .O(new_n63_));
  nand2  g48(.a(x6), .b(new_n22_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n21_), .c(new_n23_), .O(new_n65_));
  aoi21  g50(.a(new_n65_), .b(new_n56_), .c(new_n44_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n63_), .O(z5));
  nand3  g52(.a(new_n30_), .b(new_n22_), .c(x2), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n60_), .c(new_n23_), .O(new_n70_));
  nand3  g55(.a(new_n52_), .b(new_n45_), .c(new_n19_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n70_), .O(z6));
endmodule


