// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:38 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  inv1   g02(.a(x4), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n17_), .c(new_n18_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n16_), .O(new_n22_));
  inv1   g07(.a(x0), .O(new_n23_));
  nand4  g08(.a(x3), .b(x2), .c(x1), .d(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  nand2  g11(.a(x2), .b(new_n16_), .O(new_n27_));
  nand3  g12(.a(x6), .b(new_n19_), .c(new_n17_), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(new_n26_), .c(new_n28_), .O(new_n29_));
  nor2   g14(.a(x2), .b(new_n16_), .O(new_n30_));
  aoi21  g15(.a(new_n29_), .b(new_n18_), .c(new_n30_), .O(new_n31_));
  nand2  g16(.a(x3), .b(x0), .O(new_n32_));
  nor2   g17(.a(x4), .b(x2), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  or2    g19(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  oai21  g20(.a(new_n31_), .b(x0), .c(new_n35_), .O(z1));
  nand2  g21(.a(x4), .b(new_n17_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n19_), .c(new_n16_), .O(new_n38_));
  inv1   g23(.a(x5), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x2), .O(new_n40_));
  inv1   g25(.a(x6), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n17_), .O(new_n42_));
  nor2   g27(.a(x4), .b(x3), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n16_), .O(new_n44_));
  aoi21  g29(.a(new_n42_), .b(new_n40_), .c(new_n44_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n38_), .c(new_n23_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n35_), .O(z2));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n30_), .c(new_n23_), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n23_), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n43_), .c(x2), .d(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(z3));
  aoi21  g37(.a(new_n19_), .b(x2), .c(x0), .O(new_n53_));
  nor2   g38(.a(new_n53_), .b(new_n16_), .O(new_n54_));
  nand3  g39(.a(x3), .b(new_n16_), .c(new_n23_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n20_), .c(new_n17_), .O(new_n56_));
  aoi21  g41(.a(new_n32_), .b(x2), .c(x4), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n56_), .c(new_n54_), .O(z4));
  nor2   g43(.a(new_n41_), .b(x3), .O(new_n59_));
  oai21  g44(.a(new_n34_), .b(new_n59_), .c(new_n53_), .O(new_n60_));
  xor2a  g45(.a(x3), .b(x2), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(x0), .O(new_n62_));
  nand3  g47(.a(x5), .b(x2), .c(new_n23_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n62_), .c(new_n18_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n16_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n60_), .O(z5));
  aoi21  g51(.a(new_n26_), .b(x2), .c(x4), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n62_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n16_), .O(new_n69_));
  nor2   g54(.a(new_n16_), .b(x0), .O(new_n70_));
  oai21  g55(.a(new_n33_), .b(x3), .c(new_n70_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n69_), .O(z6));
endmodule


