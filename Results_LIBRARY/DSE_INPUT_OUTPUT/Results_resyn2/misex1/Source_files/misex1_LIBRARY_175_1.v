// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x4), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x0), .c(x3), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  oai21  g05(.a(x1), .b(new_n19_), .c(new_n20_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n18_), .c(x2), .O(new_n22_));
  oai21  g07(.a(new_n16_), .b(x2), .c(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x3), .b(new_n17_), .c(x0), .O(new_n25_));
  inv1   g10(.a(new_n25_), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x4), .c(new_n24_), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n20_), .c(x2), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  nor2   g15(.a(x2), .b(x1), .O(new_n31_));
  oai21  g16(.a(new_n30_), .b(x3), .c(new_n31_), .O(new_n32_));
  aoi21  g17(.a(x2), .b(x1), .c(x0), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n32_), .c(new_n29_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n27_), .O(z1));
  nand2  g20(.a(new_n28_), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n30_), .b(new_n16_), .c(new_n24_), .O(new_n37_));
  nand2  g22(.a(new_n20_), .b(new_n17_), .O(new_n38_));
  aoi21  g23(.a(new_n37_), .b(new_n36_), .c(new_n38_), .O(new_n39_));
  nand2  g24(.a(x3), .b(x1), .O(new_n40_));
  aoi21  g25(.a(x4), .b(new_n24_), .c(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n39_), .c(new_n19_), .O(new_n42_));
  nor2   g27(.a(x4), .b(x2), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n26_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(z2));
  nand2  g30(.a(x5), .b(new_n19_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n20_), .c(x2), .d(new_n17_), .O(new_n47_));
  nand2  g32(.a(x7), .b(x3), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n43_), .c(x1), .d(new_n19_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  nand2  g35(.a(new_n20_), .b(x2), .O(new_n51_));
  nand2  g36(.a(x3), .b(new_n24_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n19_), .c(new_n51_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n17_), .O(new_n54_));
  nor2   g39(.a(new_n16_), .b(x2), .O(new_n55_));
  aoi21  g40(.a(new_n20_), .b(x2), .c(x0), .O(new_n56_));
  nand2  g41(.a(new_n31_), .b(x3), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n54_), .O(z4));
  aoi21  g44(.a(new_n52_), .b(new_n51_), .c(new_n19_), .O(new_n60_));
  nand3  g45(.a(x5), .b(x2), .c(new_n19_), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n17_), .O(new_n63_));
  aoi21  g48(.a(new_n56_), .b(new_n32_), .c(new_n55_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n63_), .O(z5));
  inv1   g50(.a(new_n29_), .O(new_n66_));
  oai21  g51(.a(new_n60_), .b(new_n66_), .c(new_n17_), .O(new_n67_));
  aoi21  g52(.a(new_n56_), .b(x1), .c(new_n55_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n67_), .O(z6));
endmodule


