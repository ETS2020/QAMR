// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  inv1   g06(.a(x4), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n21_), .c(new_n17_), .O(new_n23_));
  or2    g08(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(z0));
  nor2   g10(.a(new_n21_), .b(x2), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(x0), .c(x4), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x1), .c(x2), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n21_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n16_), .c(new_n17_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n29_), .c(new_n20_), .O(new_n32_));
  oai21  g17(.a(new_n27_), .b(x1), .c(new_n32_), .O(z1));
  nand2  g18(.a(new_n26_), .b(x0), .O(new_n34_));
  nand2  g19(.a(x4), .b(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n21_), .c(new_n17_), .O(new_n36_));
  inv1   g21(.a(x5), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n16_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(new_n23_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n36_), .c(new_n20_), .O(new_n42_));
  nor2   g27(.a(x4), .b(x1), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n34_), .c(new_n42_), .O(z2));
  nand2  g30(.a(new_n21_), .b(x2), .O(new_n46_));
  aoi21  g31(.a(x5), .b(new_n20_), .c(new_n46_), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(x4), .c(new_n17_), .O(new_n48_));
  oai21  g33(.a(x4), .b(x3), .c(x7), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n18_), .c(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(z3));
  nor2   g36(.a(new_n44_), .b(new_n26_), .O(new_n52_));
  aoi21  g37(.a(new_n46_), .b(x1), .c(new_n52_), .O(new_n53_));
  nand2  g38(.a(new_n46_), .b(new_n34_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  oai21  g40(.a(new_n53_), .b(x0), .c(new_n55_), .O(z4));
  xor2a  g41(.a(x3), .b(x2), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(x0), .O(new_n58_));
  nand3  g43(.a(x5), .b(x2), .c(new_n20_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(x1), .O(new_n60_));
  nand2  g45(.a(new_n40_), .b(new_n20_), .O(new_n61_));
  nor2   g46(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n22_), .O(new_n63_));
  nand3  g48(.a(new_n46_), .b(x1), .c(new_n20_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n63_), .O(z5));
  oai21  g50(.a(x4), .b(x2), .c(new_n21_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n18_), .O(new_n67_));
  aoi21  g52(.a(new_n28_), .b(x2), .c(x4), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n17_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n67_), .O(z6));
endmodule


