// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:08 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nor2   g02(.a(new_n16_), .b(x0), .O(new_n18_));
  aoi21  g03(.a(new_n17_), .b(new_n16_), .c(new_n18_), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand2  g05(.a(new_n17_), .b(new_n20_), .O(new_n21_));
  inv1   g06(.a(x6), .O(new_n22_));
  inv1   g07(.a(x7), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n21_), .c(x2), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(new_n19_), .O(z0));
  inv1   g11(.a(x2), .O(new_n27_));
  nand4  g12(.a(x3), .b(new_n27_), .c(new_n16_), .d(x0), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n17_), .c(x2), .O(new_n30_));
  xor2a  g15(.a(x2), .b(x1), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n30_), .c(new_n20_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n24_), .O(new_n34_));
  nor2   g19(.a(new_n22_), .b(x2), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n17_), .c(new_n20_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(z1));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n17_), .O(new_n39_));
  aoi21  g24(.a(x6), .b(new_n27_), .c(x3), .O(new_n40_));
  aoi21  g25(.a(x5), .b(x2), .c(x1), .O(new_n41_));
  aoi22  g26(.a(new_n41_), .b(new_n40_), .c(new_n39_), .d(x1), .O(new_n42_));
  and2   g27(.a(new_n28_), .b(new_n24_), .O(new_n43_));
  oai21  g28(.a(new_n42_), .b(x0), .c(new_n43_), .O(z2));
  nand2  g29(.a(x5), .b(new_n20_), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n24_), .c(x2), .d(new_n16_), .O(new_n46_));
  nor2   g31(.a(x4), .b(x2), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(x7), .c(x1), .d(new_n20_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n17_), .O(new_n50_));
  nand3  g35(.a(new_n35_), .b(new_n18_), .c(new_n23_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(z3));
  nand3  g37(.a(x3), .b(new_n27_), .c(x0), .O(new_n53_));
  oai21  g38(.a(x3), .b(new_n27_), .c(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n27_), .b(x1), .O(new_n56_));
  inv1   g41(.a(new_n56_), .O(new_n57_));
  xnor2a g42(.a(x3), .b(x2), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n57_), .c(new_n20_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n55_), .c(new_n24_), .O(z4));
  nor2   g45(.a(new_n58_), .b(new_n20_), .O(new_n61_));
  nand3  g46(.a(x5), .b(x2), .c(new_n20_), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n61_), .c(new_n16_), .O(new_n64_));
  nor2   g49(.a(new_n17_), .b(x2), .O(new_n65_));
  oai21  g50(.a(new_n40_), .b(new_n65_), .c(new_n56_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n20_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n64_), .c(new_n24_), .O(z5));
  inv1   g53(.a(new_n30_), .O(new_n69_));
  oai21  g54(.a(new_n61_), .b(new_n69_), .c(new_n16_), .O(new_n70_));
  oai21  g55(.a(new_n47_), .b(x3), .c(new_n18_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n70_), .c(new_n24_), .O(z6));
endmodule


