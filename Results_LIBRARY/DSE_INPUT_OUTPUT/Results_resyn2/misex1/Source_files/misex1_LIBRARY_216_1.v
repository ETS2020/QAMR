// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:09 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(new_n16_), .c(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nor2   g05(.a(new_n16_), .b(x1), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(x0), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n19_), .O(z0));
  oai21  g08(.a(x5), .b(x3), .c(new_n21_), .O(new_n24_));
  nor2   g09(.a(x3), .b(x2), .O(new_n25_));
  oai21  g10(.a(x6), .b(x1), .c(new_n25_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n24_), .c(x0), .O(z1));
  nand2  g12(.a(x3), .b(x2), .O(new_n28_));
  nand3  g13(.a(x4), .b(new_n20_), .c(new_n16_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x1), .O(new_n31_));
  nand2  g16(.a(new_n20_), .b(new_n17_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nand2  g18(.a(x6), .b(new_n16_), .O(new_n34_));
  nand2  g19(.a(x5), .b(x2), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n31_), .c(x0), .O(z2));
  inv1   g22(.a(x5), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x0), .c(new_n21_), .O(new_n39_));
  nand2  g24(.a(x7), .b(x4), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n18_), .c(new_n16_), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n39_), .c(x3), .O(z3));
  inv1   g27(.a(x0), .O(new_n43_));
  oai21  g28(.a(x3), .b(new_n43_), .c(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n33_), .b(x2), .O(new_n45_));
  nand2  g30(.a(x3), .b(new_n43_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(z4));
  nand3  g32(.a(new_n38_), .b(new_n20_), .c(new_n43_), .O(new_n48_));
  nand2  g33(.a(new_n46_), .b(new_n32_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n48_), .c(x2), .O(new_n50_));
  oai21  g35(.a(new_n26_), .b(x0), .c(new_n50_), .O(z5));
  nor2   g36(.a(new_n38_), .b(x0), .O(new_n52_));
  inv1   g37(.a(new_n28_), .O(new_n53_));
  nor3   g38(.a(x4), .b(x3), .c(x2), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n53_), .c(new_n18_), .O(new_n55_));
  oai21  g40(.a(new_n45_), .b(new_n52_), .c(new_n55_), .O(z6));
endmodule


