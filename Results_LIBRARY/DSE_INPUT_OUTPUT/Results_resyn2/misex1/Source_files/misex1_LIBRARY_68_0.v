// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  oai21  g08(.a(new_n16_), .b(new_n18_), .c(new_n23_), .O(new_n24_));
  nor2   g09(.a(x5), .b(x3), .O(new_n25_));
  nand2  g10(.a(new_n16_), .b(new_n18_), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(z1));
  inv1   g12(.a(x4), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(x2), .c(new_n19_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x1), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  nor2   g16(.a(x3), .b(new_n23_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n31_), .c(new_n16_), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n30_), .c(x0), .O(z2));
  oai21  g19(.a(x4), .b(x3), .c(x7), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n18_), .c(new_n16_), .O(new_n36_));
  nand2  g21(.a(x5), .b(new_n18_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n32_), .c(new_n16_), .O(new_n38_));
  oai21  g23(.a(new_n36_), .b(x2), .c(new_n38_), .O(z3));
  nand2  g24(.a(new_n19_), .b(x2), .O(new_n40_));
  nand2  g25(.a(new_n23_), .b(new_n16_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n18_), .O(new_n42_));
  oai21  g27(.a(new_n40_), .b(x1), .c(new_n42_), .O(z4));
  oai21  g28(.a(new_n31_), .b(x1), .c(new_n19_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n18_), .O(new_n45_));
  nand3  g30(.a(new_n32_), .b(new_n16_), .c(x0), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n45_), .c(new_n24_), .O(z5));
  oai21  g32(.a(x4), .b(x2), .c(new_n19_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x1), .c(new_n18_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n38_), .O(z6));
endmodule


