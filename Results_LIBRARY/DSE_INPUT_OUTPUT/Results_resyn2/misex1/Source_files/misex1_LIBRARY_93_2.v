// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:09 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_;
  oai21  g00(.a(x3), .b(x1), .c(x2), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand4  g03(.a(x3), .b(x2), .c(x1), .d(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(z0));
  inv1   g05(.a(x2), .O(new_n21_));
  nor2   g06(.a(x1), .b(x0), .O(new_n22_));
  inv1   g07(.a(x3), .O(new_n23_));
  nand2  g08(.a(x6), .b(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n21_), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  nand2  g12(.a(new_n22_), .b(x2), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(z1));
  inv1   g14(.a(x4), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x2), .c(new_n23_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(new_n32_));
  nor2   g17(.a(x3), .b(x1), .O(new_n33_));
  nor2   g18(.a(x5), .b(new_n21_), .O(new_n34_));
  nor2   g19(.a(x6), .b(x2), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n32_), .c(x0), .O(z2));
  oai21  g22(.a(new_n34_), .b(x0), .c(new_n16_), .O(new_n38_));
  oai21  g23(.a(x4), .b(x3), .c(x7), .O(new_n39_));
  nor2   g24(.a(x2), .b(x0), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n39_), .c(x1), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n38_), .O(z3));
  aoi21  g27(.a(x3), .b(new_n18_), .c(new_n33_), .O(new_n43_));
  oai21  g28(.a(new_n23_), .b(x1), .c(new_n40_), .O(new_n44_));
  oai21  g29(.a(new_n43_), .b(new_n21_), .c(new_n44_), .O(z4));
  inv1   g30(.a(x1), .O(new_n46_));
  nand2  g31(.a(x5), .b(new_n46_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n23_), .c(x2), .O(new_n48_));
  nand3  g33(.a(new_n24_), .b(new_n21_), .c(new_n46_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n48_), .c(new_n18_), .O(new_n50_));
  nand3  g35(.a(new_n33_), .b(x2), .c(x0), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n50_), .O(z5));
  oai21  g37(.a(x4), .b(x2), .c(new_n23_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(x1), .c(new_n18_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n38_), .O(z6));
endmodule


