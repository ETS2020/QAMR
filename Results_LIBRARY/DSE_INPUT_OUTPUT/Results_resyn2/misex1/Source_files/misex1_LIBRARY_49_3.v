// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:50 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nor2   g02(.a(x3), .b(new_n17_), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x0), .O(new_n22_));
  aoi21  g07(.a(new_n19_), .b(new_n16_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x0), .O(new_n24_));
  nand3  g09(.a(x6), .b(new_n20_), .c(new_n17_), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  nand2  g11(.a(x2), .b(new_n16_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n24_), .O(new_n29_));
  oai21  g14(.a(new_n20_), .b(x2), .c(new_n16_), .O(new_n30_));
  oai21  g15(.a(x2), .b(new_n16_), .c(new_n24_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(z1));
  nand2  g18(.a(x4), .b(new_n17_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n20_), .c(new_n16_), .O(new_n35_));
  nand2  g20(.a(x6), .b(new_n17_), .O(new_n36_));
  nor2   g21(.a(x3), .b(x1), .O(new_n37_));
  nand2  g22(.a(x5), .b(x2), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  inv1   g24(.a(new_n39_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n35_), .c(new_n24_), .O(new_n41_));
  nor2   g26(.a(new_n20_), .b(x2), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n16_), .c(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n41_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n17_), .c(x1), .d(new_n24_), .O(new_n46_));
  nand2  g31(.a(x5), .b(new_n24_), .O(new_n47_));
  nand2  g32(.a(new_n37_), .b(x2), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n46_), .O(z3));
  nor2   g36(.a(new_n18_), .b(x0), .O(new_n52_));
  oai21  g37(.a(new_n42_), .b(new_n18_), .c(new_n16_), .O(new_n53_));
  xnor2a g38(.a(new_n53_), .b(new_n52_), .O(z4));
  inv1   g39(.a(new_n25_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n21_), .c(new_n24_), .O(new_n56_));
  oai21  g41(.a(x5), .b(x0), .c(new_n49_), .O(new_n57_));
  aoi22  g42(.a(new_n30_), .b(x0), .c(new_n19_), .d(x1), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(z5));
  aoi22  g44(.a(new_n47_), .b(new_n18_), .c(new_n42_), .d(x0), .O(new_n60_));
  oai21  g45(.a(x4), .b(x2), .c(new_n20_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(x1), .c(new_n24_), .O(new_n62_));
  oai21  g47(.a(new_n60_), .b(x1), .c(new_n62_), .O(z6));
endmodule


