// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:17 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x4), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x0), .c(x3), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  oai21  g05(.a(x1), .b(new_n19_), .c(new_n20_), .O(new_n21_));
  nand4  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .d(x2), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x2), .c(new_n17_), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand3  g12(.a(x6), .b(new_n20_), .c(new_n27_), .O(new_n28_));
  oai21  g13(.a(x4), .b(new_n27_), .c(x1), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n19_), .O(new_n31_));
  nor2   g16(.a(x1), .b(new_n19_), .O(new_n32_));
  nand2  g17(.a(x3), .b(new_n27_), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  aoi22  g19(.a(new_n34_), .b(new_n32_), .c(x4), .d(x2), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n31_), .O(z1));
  nand2  g21(.a(new_n16_), .b(new_n20_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x1), .O(new_n38_));
  nor2   g23(.a(x3), .b(x1), .O(new_n39_));
  nand2  g24(.a(x6), .b(new_n27_), .O(new_n40_));
  nand2  g25(.a(x5), .b(x2), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n19_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n35_), .O(z2));
  nand2  g30(.a(x5), .b(new_n19_), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n39_), .c(x4), .O(new_n47_));
  nor2   g32(.a(new_n17_), .b(x0), .O(new_n48_));
  nand2  g33(.a(new_n37_), .b(x7), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n48_), .c(new_n27_), .O(new_n50_));
  oai21  g35(.a(new_n47_), .b(new_n27_), .c(new_n50_), .O(z3));
  aoi21  g36(.a(new_n16_), .b(x3), .c(new_n27_), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  nor2   g38(.a(x2), .b(x1), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(x3), .c(x0), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand3  g41(.a(new_n16_), .b(new_n20_), .c(x2), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n19_), .O(new_n58_));
  nand2  g43(.a(new_n57_), .b(new_n33_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n58_), .c(new_n17_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n56_), .O(z4));
  aoi21  g46(.a(new_n57_), .b(new_n33_), .c(new_n19_), .O(new_n62_));
  nand4  g47(.a(x5), .b(new_n16_), .c(x2), .d(new_n19_), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n62_), .c(new_n17_), .O(new_n65_));
  oai21  g50(.a(new_n54_), .b(new_n52_), .c(new_n28_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n19_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z5));
  nand4  g53(.a(new_n24_), .b(new_n16_), .c(new_n20_), .d(x2), .O(new_n69_));
  inv1   g54(.a(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n62_), .c(new_n17_), .O(new_n71_));
  nand2  g56(.a(x4), .b(new_n20_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n53_), .c(new_n48_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n71_), .O(z6));
endmodule


