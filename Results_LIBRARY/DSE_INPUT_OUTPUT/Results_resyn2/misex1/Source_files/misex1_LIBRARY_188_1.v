// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_;
  nor2   g00(.a(x6), .b(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x2), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  nor2   g07(.a(x1), .b(new_n22_), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n21_), .c(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand3  g10(.a(x3), .b(new_n17_), .c(x0), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x6), .c(new_n20_), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n21_), .c(x2), .O(new_n30_));
  nand2  g15(.a(x2), .b(x1), .O(new_n31_));
  nand2  g16(.a(new_n20_), .b(new_n17_), .O(new_n32_));
  nand4  g17(.a(new_n32_), .b(new_n31_), .c(new_n30_), .d(new_n22_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n28_), .O(z1));
  inv1   g19(.a(x6), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x2), .c(x3), .O(new_n36_));
  nand2  g21(.a(new_n16_), .b(x4), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n36_), .c(new_n17_), .O(new_n38_));
  nand2  g23(.a(x6), .b(new_n20_), .O(new_n39_));
  nand2  g24(.a(x5), .b(x2), .O(new_n40_));
  nor2   g25(.a(x3), .b(x1), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n38_), .c(new_n22_), .O(new_n44_));
  nand2  g29(.a(new_n27_), .b(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n18_), .c(x6), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n22_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n41_), .c(x2), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(x2), .c(new_n50_), .O(z3));
  nand3  g36(.a(x3), .b(new_n20_), .c(new_n17_), .O(new_n52_));
  aoi21  g37(.a(new_n21_), .b(x2), .c(x0), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n52_), .c(new_n39_), .O(new_n54_));
  nand2  g39(.a(new_n21_), .b(x2), .O(new_n55_));
  nand2  g40(.a(new_n16_), .b(x3), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g42(.a(new_n53_), .b(x1), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n54_), .O(z4));
  aoi21  g45(.a(new_n56_), .b(new_n55_), .c(new_n22_), .O(new_n61_));
  nor2   g46(.a(new_n40_), .b(x0), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n61_), .c(new_n17_), .O(new_n63_));
  nand2  g48(.a(new_n53_), .b(new_n32_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n63_), .c(new_n39_), .O(z5));
  inv1   g50(.a(new_n30_), .O(new_n66_));
  oai21  g51(.a(new_n61_), .b(new_n66_), .c(new_n17_), .O(new_n67_));
  inv1   g52(.a(new_n36_), .O(new_n68_));
  nor3   g53(.a(x6), .b(x4), .c(x2), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n68_), .c(new_n18_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n67_), .O(z6));
endmodule


