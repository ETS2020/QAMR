// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n44_, new_n45_, new_n47_, new_n48_;
  inv1   g00(.a(x2), .O(new_n16_));
  nor2   g01(.a(x3), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nor2   g03(.a(x1), .b(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(new_n20_), .O(z0));
  inv1   g06(.a(x3), .O(new_n22_));
  aoi21  g07(.a(x6), .b(new_n22_), .c(x1), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n16_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n18_), .O(new_n25_));
  nand3  g10(.a(new_n19_), .b(x3), .c(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(z1));
  nand2  g12(.a(new_n19_), .b(x3), .O(new_n28_));
  oai21  g13(.a(x4), .b(x3), .c(x1), .O(new_n29_));
  inv1   g14(.a(x1), .O(new_n30_));
  inv1   g15(.a(x6), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n22_), .c(new_n30_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n18_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n28_), .c(x2), .O(z2));
  nand2  g20(.a(new_n17_), .b(new_n30_), .O(new_n36_));
  oai21  g21(.a(x4), .b(x3), .c(x7), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(x1), .c(x2), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x0), .c(new_n36_), .O(z3));
  nor2   g24(.a(new_n22_), .b(x2), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n30_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n18_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n36_), .c(new_n26_), .O(z4));
  oai21  g28(.a(new_n40_), .b(new_n17_), .c(new_n19_), .O(new_n44_));
  nand2  g29(.a(new_n16_), .b(new_n18_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n23_), .c(new_n44_), .O(z5));
  nand2  g31(.a(x4), .b(new_n22_), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n16_), .c(x1), .d(new_n18_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n44_), .O(z6));
endmodule


