// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  aoi21  g03(.a(x3), .b(new_n17_), .c(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n16_), .b(x0), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  oai22  g07(.a(new_n22_), .b(new_n20_), .c(new_n19_), .d(new_n16_), .O(z0));
  nand3  g08(.a(x6), .b(new_n21_), .c(new_n18_), .O(new_n24_));
  nor2   g09(.a(x5), .b(x3), .O(new_n25_));
  nand2  g10(.a(x2), .b(new_n16_), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n17_), .O(new_n28_));
  oai21  g13(.a(new_n21_), .b(new_n17_), .c(new_n16_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n18_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n28_), .O(z1));
  nand2  g16(.a(x5), .b(x2), .O(new_n32_));
  nand2  g17(.a(x6), .b(new_n18_), .O(new_n33_));
  nor2   g18(.a(x3), .b(x1), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  nand3  g20(.a(x3), .b(x2), .c(x1), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n17_), .O(new_n38_));
  nand2  g23(.a(x3), .b(new_n18_), .O(new_n39_));
  or2    g24(.a(new_n39_), .b(new_n20_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(z2));
  nor2   g26(.a(new_n18_), .b(new_n16_), .O(new_n42_));
  nand2  g27(.a(x5), .b(new_n17_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n21_), .c(x2), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n16_), .c(new_n42_), .O(z3));
  nand3  g30(.a(new_n39_), .b(new_n22_), .c(new_n17_), .O(new_n46_));
  nand3  g31(.a(new_n21_), .b(x2), .c(new_n16_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n46_), .c(new_n30_), .O(z4));
  aoi21  g33(.a(x5), .b(new_n16_), .c(x3), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n18_), .c(new_n24_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n17_), .O(new_n51_));
  nand2  g36(.a(x3), .b(new_n17_), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(x2), .c(new_n16_), .O(new_n53_));
  aoi21  g38(.a(new_n47_), .b(new_n39_), .c(new_n17_), .O(new_n54_));
  nor2   g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n51_), .O(z5));
  or2    g41(.a(new_n47_), .b(x5), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n55_), .O(z6));
endmodule


