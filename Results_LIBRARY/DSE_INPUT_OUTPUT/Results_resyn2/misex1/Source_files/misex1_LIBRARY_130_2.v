// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  oai21  g02(.a(x1), .b(new_n16_), .c(new_n17_), .O(new_n18_));
  nand2  g03(.a(x1), .b(new_n16_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x3), .O(new_n20_));
  inv1   g05(.a(x6), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x5), .O(new_n22_));
  nand4  g07(.a(new_n22_), .b(new_n20_), .c(new_n18_), .d(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(z0));
  nand3  g09(.a(x6), .b(new_n17_), .c(new_n16_), .O(new_n25_));
  inv1   g10(.a(new_n25_), .O(new_n26_));
  inv1   g11(.a(x1), .O(new_n27_));
  nand3  g12(.a(x3), .b(new_n27_), .c(x0), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n19_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n22_), .c(new_n26_), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n17_), .O(new_n32_));
  inv1   g17(.a(x2), .O(new_n33_));
  nor2   g18(.a(new_n33_), .b(x1), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(new_n32_), .c(new_n22_), .d(new_n16_), .O(new_n35_));
  oai21  g20(.a(new_n30_), .b(x2), .c(new_n35_), .O(z1));
  inv1   g21(.a(x4), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x2), .c(new_n17_), .O(new_n38_));
  oai21  g23(.a(x5), .b(new_n33_), .c(x6), .O(new_n39_));
  nor2   g24(.a(x3), .b(x1), .O(new_n40_));
  aoi22  g25(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(x1), .O(new_n41_));
  nor2   g26(.a(x1), .b(new_n16_), .O(new_n42_));
  inv1   g27(.a(new_n22_), .O(new_n43_));
  nand2  g28(.a(x3), .b(new_n33_), .O(new_n44_));
  inv1   g29(.a(new_n44_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n42_), .c(new_n43_), .O(new_n46_));
  oai21  g31(.a(new_n41_), .b(x0), .c(new_n46_), .O(z2));
  nor2   g32(.a(x4), .b(x2), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n22_), .c(x1), .d(new_n16_), .O(new_n49_));
  nand2  g34(.a(x6), .b(x0), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x5), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n34_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n17_), .O(new_n54_));
  inv1   g39(.a(new_n19_), .O(new_n55_));
  nor2   g40(.a(x7), .b(x2), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n22_), .c(new_n55_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n54_), .O(z3));
  nand2  g43(.a(new_n17_), .b(x2), .O(new_n59_));
  oai21  g44(.a(new_n44_), .b(new_n16_), .c(new_n59_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n27_), .O(new_n61_));
  aoi21  g46(.a(new_n17_), .b(x2), .c(x0), .O(new_n62_));
  oai21  g47(.a(new_n44_), .b(x1), .c(new_n62_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n61_), .c(new_n22_), .O(z4));
  aoi21  g49(.a(new_n59_), .b(new_n44_), .c(new_n16_), .O(new_n65_));
  nand3  g50(.a(x5), .b(new_n17_), .c(x2), .O(new_n66_));
  inv1   g51(.a(new_n66_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n65_), .c(new_n27_), .O(new_n68_));
  nand2  g53(.a(x6), .b(new_n17_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n33_), .c(new_n27_), .O(new_n70_));
  aoi21  g55(.a(new_n70_), .b(new_n62_), .c(new_n43_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n68_), .O(z5));
  nand3  g57(.a(new_n31_), .b(new_n17_), .c(x2), .O(new_n73_));
  inv1   g58(.a(new_n73_), .O(new_n74_));
  oai21  g59(.a(new_n74_), .b(new_n65_), .c(new_n27_), .O(new_n75_));
  oai21  g60(.a(x4), .b(x2), .c(new_n17_), .O(new_n76_));
  aoi21  g61(.a(new_n76_), .b(new_n55_), .c(new_n43_), .O(new_n77_));
  nand2  g62(.a(new_n77_), .b(new_n75_), .O(z6));
endmodule


