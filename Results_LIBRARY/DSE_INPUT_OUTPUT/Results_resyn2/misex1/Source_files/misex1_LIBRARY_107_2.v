// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x6), .b(x1), .c(new_n16_), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x3), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n16_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n18_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  oai21  g07(.a(x5), .b(x3), .c(x2), .O(new_n23_));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x6), .b(new_n19_), .c(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n23_), .b(x1), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n16_), .O(new_n27_));
  nor2   g12(.a(x1), .b(new_n16_), .O(new_n28_));
  oai21  g13(.a(x2), .b(x0), .c(x6), .O(new_n29_));
  nand2  g14(.a(x3), .b(new_n24_), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  aoi22  g16(.a(new_n31_), .b(new_n28_), .c(new_n29_), .d(x1), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n27_), .O(z1));
  inv1   g18(.a(x1), .O(new_n34_));
  aoi21  g19(.a(x4), .b(new_n24_), .c(x3), .O(new_n35_));
  inv1   g20(.a(x6), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n19_), .c(new_n24_), .O(new_n37_));
  oai21  g22(.a(new_n35_), .b(new_n34_), .c(new_n37_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  nor2   g24(.a(x6), .b(new_n34_), .O(new_n40_));
  oai21  g25(.a(x5), .b(x0), .c(new_n30_), .O(new_n41_));
  nand2  g26(.a(new_n19_), .b(x2), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n16_), .c(x1), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n39_), .O(z2));
  nor2   g30(.a(x2), .b(x0), .O(new_n46_));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n46_), .c(new_n36_), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n16_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n19_), .c(x2), .d(new_n34_), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(new_n34_), .c(new_n50_), .O(z3));
  xor2a  g36(.a(x3), .b(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(x0), .O(new_n53_));
  nand2  g38(.a(new_n30_), .b(new_n16_), .O(new_n54_));
  inv1   g39(.a(x5), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n19_), .c(x2), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n34_), .O(new_n58_));
  aoi21  g43(.a(new_n19_), .b(x2), .c(x0), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n36_), .c(x1), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n58_), .O(z4));
  aoi21  g46(.a(new_n42_), .b(new_n30_), .c(new_n16_), .O(new_n62_));
  nor2   g47(.a(new_n23_), .b(x0), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n62_), .c(new_n34_), .O(new_n64_));
  aoi21  g49(.a(x3), .b(new_n34_), .c(new_n36_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z5));
  nand2  g52(.a(new_n56_), .b(new_n53_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n34_), .O(new_n69_));
  inv1   g54(.a(new_n17_), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n19_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n69_), .O(z6));
endmodule


