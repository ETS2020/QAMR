// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:08 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  nand3  g08(.a(x3), .b(new_n16_), .c(x0), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(x6), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x1), .c(x2), .O(new_n28_));
  nand3  g13(.a(x3), .b(new_n23_), .c(new_n16_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n28_), .c(new_n18_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n26_), .O(z1));
  nand4  g16(.a(x6), .b(x3), .c(new_n23_), .d(x0), .O(new_n32_));
  inv1   g17(.a(x5), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n19_), .c(x2), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x0), .c(new_n32_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n16_), .O(new_n36_));
  nor2   g21(.a(new_n16_), .b(x0), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(x6), .b(x2), .c(x3), .O(new_n39_));
  nand2  g24(.a(x6), .b(new_n23_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n38_), .c(new_n39_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n36_), .O(z2));
  inv1   g28(.a(x6), .O(new_n44_));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n37_), .c(new_n44_), .O(new_n46_));
  nand2  g31(.a(x5), .b(new_n18_), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n19_), .c(x2), .d(new_n16_), .O(new_n48_));
  oai21  g33(.a(new_n46_), .b(x2), .c(new_n48_), .O(z3));
  nand2  g34(.a(new_n19_), .b(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n32_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n44_), .b(new_n23_), .O(new_n53_));
  nand4  g38(.a(new_n53_), .b(new_n50_), .c(new_n29_), .d(new_n18_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n52_), .O(z4));
  nand3  g40(.a(x6), .b(x3), .c(new_n23_), .O(new_n56_));
  aoi21  g41(.a(new_n50_), .b(new_n56_), .c(new_n18_), .O(new_n57_));
  nand3  g42(.a(x5), .b(x2), .c(new_n18_), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n57_), .c(new_n16_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n54_), .O(z5));
  inv1   g46(.a(new_n34_), .O(new_n62_));
  oai21  g47(.a(new_n57_), .b(new_n62_), .c(new_n16_), .O(new_n63_));
  oai21  g48(.a(new_n40_), .b(x4), .c(new_n39_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n37_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n63_), .O(z6));
endmodule


