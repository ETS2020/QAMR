// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:12 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n43_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand3  g02(.a(new_n17_), .b(x2), .c(new_n16_), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  inv1   g05(.a(new_n20_), .O(z0));
  inv1   g06(.a(x2), .O(new_n22_));
  nand3  g07(.a(x3), .b(new_n22_), .c(x0), .O(new_n23_));
  inv1   g08(.a(x0), .O(new_n24_));
  nand2  g09(.a(x3), .b(new_n22_), .O(new_n25_));
  inv1   g10(.a(x5), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n17_), .c(x2), .O(new_n27_));
  nor2   g12(.a(x6), .b(x2), .O(new_n28_));
  inv1   g13(.a(new_n28_), .O(new_n29_));
  nand4  g14(.a(new_n29_), .b(new_n27_), .c(new_n25_), .d(new_n24_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n23_), .c(x1), .O(z1));
  nor2   g16(.a(x5), .b(new_n22_), .O(new_n32_));
  nor2   g17(.a(x3), .b(x0), .O(new_n33_));
  oai21  g18(.a(new_n32_), .b(new_n28_), .c(new_n33_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n23_), .c(x1), .O(z2));
  nor2   g20(.a(new_n26_), .b(x0), .O(new_n36_));
  oai22  g21(.a(new_n36_), .b(new_n18_), .c(new_n16_), .d(x0), .O(z3));
  nand2  g22(.a(new_n25_), .b(new_n24_), .O(new_n38_));
  nor2   g23(.a(x3), .b(new_n22_), .O(new_n39_));
  xor2a  g24(.a(x3), .b(x2), .O(new_n40_));
  aoi22  g25(.a(new_n40_), .b(x0), .c(new_n39_), .d(new_n26_), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n38_), .c(x1), .O(z4));
  nand2  g27(.a(new_n40_), .b(x0), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n30_), .c(x1), .O(z5));
  nor2   g29(.a(new_n41_), .b(x1), .O(z6));
endmodule


