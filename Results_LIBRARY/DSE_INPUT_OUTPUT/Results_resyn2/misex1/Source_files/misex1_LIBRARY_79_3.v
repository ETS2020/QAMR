// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand4  g02(.a(new_n17_), .b(x2), .c(new_n16_), .d(x0), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(z0));
  inv1   g04(.a(x6), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(x3), .c(new_n16_), .O(new_n21_));
  nor2   g06(.a(x2), .b(x0), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  inv1   g08(.a(x0), .O(new_n24_));
  nand2  g09(.a(x5), .b(new_n24_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n17_), .O(new_n26_));
  nand2  g11(.a(x3), .b(x2), .O(new_n27_));
  nor2   g12(.a(new_n22_), .b(x1), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n23_), .O(z1));
  inv1   g15(.a(x5), .O(new_n31_));
  nand4  g16(.a(new_n31_), .b(new_n17_), .c(x2), .d(new_n16_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  oai21  g18(.a(x4), .b(x3), .c(x1), .O(new_n34_));
  nand3  g19(.a(new_n20_), .b(new_n17_), .c(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(x2), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(new_n33_), .c(new_n24_), .O(new_n37_));
  inv1   g22(.a(x2), .O(new_n38_));
  nand4  g23(.a(x3), .b(new_n38_), .c(new_n16_), .d(x0), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n37_), .O(z2));
  inv1   g25(.a(x4), .O(new_n41_));
  inv1   g26(.a(x7), .O(new_n42_));
  aoi21  g27(.a(new_n41_), .b(new_n17_), .c(new_n42_), .O(new_n43_));
  nand2  g28(.a(new_n22_), .b(x1), .O(new_n44_));
  aoi21  g29(.a(new_n25_), .b(new_n16_), .c(x3), .O(new_n45_));
  oai22  g30(.a(new_n45_), .b(new_n38_), .c(new_n44_), .d(new_n43_), .O(z3));
  xnor2a g31(.a(x3), .b(x2), .O(new_n47_));
  nor2   g32(.a(new_n47_), .b(x1), .O(new_n48_));
  xor2a  g33(.a(new_n48_), .b(new_n22_), .O(z4));
  nor2   g34(.a(new_n47_), .b(new_n24_), .O(new_n50_));
  nor2   g35(.a(new_n31_), .b(new_n38_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n50_), .c(new_n16_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n27_), .c(new_n23_), .O(z5));
  nand3  g38(.a(new_n31_), .b(new_n17_), .c(x2), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n50_), .b(new_n55_), .c(new_n16_), .O(new_n56_));
  inv1   g41(.a(new_n44_), .O(new_n57_));
  oai21  g42(.a(new_n41_), .b(x3), .c(new_n57_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n56_), .O(z6));
endmodule


