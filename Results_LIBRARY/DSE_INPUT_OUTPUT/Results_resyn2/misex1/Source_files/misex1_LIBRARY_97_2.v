// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x5), .O(new_n18_));
  inv1   g03(.a(x0), .O(new_n19_));
  nor2   g04(.a(x1), .b(new_n19_), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(new_n17_), .c(new_n18_), .O(new_n21_));
  nand3  g06(.a(x3), .b(x1), .c(new_n19_), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n21_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x1), .O(new_n24_));
  nand3  g09(.a(x3), .b(new_n16_), .c(x0), .O(new_n25_));
  nand3  g10(.a(x5), .b(x2), .c(new_n19_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n24_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n24_), .O(new_n30_));
  nor2   g15(.a(x2), .b(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(z1));
  aoi21  g18(.a(x4), .b(new_n16_), .c(x3), .O(new_n34_));
  nand4  g19(.a(new_n29_), .b(new_n17_), .c(new_n16_), .d(new_n24_), .O(new_n35_));
  oai21  g20(.a(new_n34_), .b(new_n24_), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n19_), .O(new_n37_));
  nand2  g22(.a(x3), .b(new_n16_), .O(new_n38_));
  inv1   g23(.a(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n18_), .b(x2), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  aoi21  g26(.a(new_n39_), .b(new_n20_), .c(new_n41_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n37_), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n31_), .c(x1), .O(new_n45_));
  oai21  g30(.a(new_n21_), .b(new_n16_), .c(new_n45_), .O(z3));
  nand2  g31(.a(new_n17_), .b(x2), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n25_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n24_), .O(new_n49_));
  oai21  g34(.a(new_n17_), .b(x0), .c(x5), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x2), .O(new_n51_));
  oai21  g36(.a(new_n17_), .b(x1), .c(new_n31_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(z4));
  oai21  g38(.a(new_n47_), .b(new_n18_), .c(new_n38_), .O(new_n54_));
  nor2   g39(.a(new_n31_), .b(x1), .O(new_n55_));
  oai21  g40(.a(new_n54_), .b(new_n19_), .c(new_n55_), .O(new_n56_));
  aoi21  g41(.a(new_n17_), .b(x2), .c(x0), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n30_), .c(new_n41_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n56_), .O(z5));
  nand2  g44(.a(x1), .b(new_n19_), .O(new_n60_));
  nand2  g45(.a(new_n54_), .b(new_n20_), .O(new_n61_));
  nor2   g46(.a(x4), .b(x2), .O(new_n62_));
  aoi21  g47(.a(new_n40_), .b(x3), .c(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n60_), .c(new_n61_), .O(z6));
endmodule


