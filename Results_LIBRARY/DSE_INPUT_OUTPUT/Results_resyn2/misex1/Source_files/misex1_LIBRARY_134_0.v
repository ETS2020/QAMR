// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  nor2   g08(.a(x5), .b(x3), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  nor2   g10(.a(new_n23_), .b(x1), .O(new_n26_));
  nand2  g11(.a(x3), .b(new_n16_), .O(new_n27_));
  aoi22  g12(.a(new_n27_), .b(new_n23_), .c(new_n26_), .d(new_n25_), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(new_n18_), .c(x6), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  oai21  g15(.a(new_n28_), .b(x0), .c(new_n30_), .O(z1));
  inv1   g16(.a(x4), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x2), .c(new_n19_), .O(new_n33_));
  aoi22  g18(.a(new_n33_), .b(x1), .c(new_n26_), .d(new_n24_), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x0), .c(new_n30_), .O(z2));
  nand2  g20(.a(new_n19_), .b(x2), .O(new_n36_));
  inv1   g21(.a(x5), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x0), .c(new_n16_), .O(new_n38_));
  nor2   g23(.a(new_n16_), .b(x0), .O(new_n39_));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n39_), .c(x6), .d(new_n23_), .O(new_n41_));
  oai21  g26(.a(new_n38_), .b(new_n36_), .c(new_n41_), .O(z3));
  nand2  g27(.a(x3), .b(x2), .O(new_n43_));
  nand3  g28(.a(new_n27_), .b(x6), .c(new_n23_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nand3  g31(.a(x6), .b(x3), .c(new_n23_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n36_), .O(new_n48_));
  nand2  g33(.a(new_n36_), .b(new_n18_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n48_), .c(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n46_), .O(z4));
  aoi21  g36(.a(new_n47_), .b(new_n36_), .c(new_n18_), .O(new_n52_));
  nand3  g37(.a(x5), .b(x2), .c(new_n18_), .O(new_n53_));
  inv1   g38(.a(new_n53_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n52_), .c(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n46_), .O(z5));
  nand2  g41(.a(new_n25_), .b(new_n18_), .O(new_n57_));
  nand2  g42(.a(new_n19_), .b(new_n23_), .O(new_n58_));
  nand4  g43(.a(new_n58_), .b(new_n57_), .c(new_n43_), .d(new_n16_), .O(new_n59_));
  inv1   g44(.a(x6), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n23_), .O(new_n61_));
  oai21  g46(.a(x4), .b(x2), .c(new_n19_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n39_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n61_), .c(new_n59_), .O(z6));
endmodule


