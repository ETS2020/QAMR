// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:50 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(new_n17_), .c(new_n16_), .O(new_n19_));
  oai21  g04(.a(x3), .b(x1), .c(x0), .O(new_n20_));
  nor2   g05(.a(x6), .b(x4), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  nand4  g07(.a(new_n22_), .b(new_n20_), .c(new_n19_), .d(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(new_n17_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(x2), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n18_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n27_), .c(new_n17_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n26_), .c(new_n16_), .O(new_n30_));
  nand4  g15(.a(x3), .b(new_n27_), .c(new_n17_), .d(x0), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n30_), .c(new_n22_), .O(z1));
  oai21  g17(.a(new_n27_), .b(x1), .c(new_n18_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n25_), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n22_), .O(new_n36_));
  oai21  g21(.a(x6), .b(x3), .c(new_n17_), .O(new_n37_));
  nor2   g22(.a(x2), .b(x0), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n37_), .c(x4), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n36_), .O(z2));
  inv1   g25(.a(x7), .O(new_n41_));
  nand3  g26(.a(new_n38_), .b(new_n41_), .c(x1), .O(new_n42_));
  inv1   g27(.a(x5), .O(new_n43_));
  nor2   g28(.a(new_n43_), .b(x0), .O(new_n44_));
  nand3  g29(.a(new_n18_), .b(x2), .c(new_n17_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n22_), .O(new_n47_));
  nor2   g32(.a(x4), .b(new_n17_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n38_), .c(x6), .d(new_n18_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(z3));
  nand2  g35(.a(x3), .b(new_n27_), .O(new_n51_));
  nand2  g36(.a(new_n18_), .b(x2), .O(new_n52_));
  oai21  g37(.a(new_n51_), .b(new_n16_), .c(new_n52_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n17_), .O(new_n54_));
  aoi21  g39(.a(new_n18_), .b(x2), .c(x0), .O(new_n55_));
  oai21  g40(.a(new_n51_), .b(x1), .c(new_n55_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n54_), .c(new_n22_), .O(z4));
  aoi21  g42(.a(new_n52_), .b(new_n51_), .c(new_n16_), .O(new_n58_));
  nand3  g43(.a(x5), .b(x2), .c(new_n16_), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n17_), .O(new_n61_));
  aoi21  g46(.a(new_n55_), .b(new_n29_), .c(new_n21_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n61_), .O(z5));
  nand3  g48(.a(new_n43_), .b(new_n18_), .c(x2), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n58_), .c(new_n17_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n18_), .O(new_n67_));
  nor2   g52(.a(new_n17_), .b(x0), .O(new_n68_));
  aoi21  g53(.a(new_n68_), .b(new_n67_), .c(new_n21_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z6));
endmodule


