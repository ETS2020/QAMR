// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:38 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x5), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  aoi21  g04(.a(new_n17_), .b(new_n16_), .c(new_n19_), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nor2   g06(.a(x1), .b(new_n21_), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(x3), .c(x2), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(new_n20_), .O(z0));
  nand2  g09(.a(x3), .b(x2), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(x1), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n18_), .O(new_n28_));
  aoi21  g13(.a(x5), .b(new_n16_), .c(x2), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(new_n30_));
  inv1   g15(.a(x2), .O(new_n31_));
  nand2  g16(.a(x3), .b(new_n31_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n22_), .O(new_n34_));
  oai21  g19(.a(new_n30_), .b(x0), .c(new_n34_), .O(z1));
  inv1   g20(.a(x4), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(x2), .c(new_n16_), .O(new_n37_));
  nand2  g22(.a(x6), .b(new_n31_), .O(new_n38_));
  nor2   g23(.a(x3), .b(x1), .O(new_n39_));
  aoi22  g24(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(x1), .O(new_n40_));
  nor2   g25(.a(new_n17_), .b(x3), .O(new_n41_));
  aoi21  g26(.a(new_n33_), .b(new_n22_), .c(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n40_), .b(x0), .c(new_n42_), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n31_), .c(x1), .d(new_n21_), .O(new_n45_));
  nand2  g30(.a(new_n39_), .b(x2), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n17_), .O(new_n48_));
  inv1   g33(.a(x7), .O(new_n49_));
  nand3  g34(.a(new_n33_), .b(new_n19_), .c(new_n49_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(z3));
  nand3  g36(.a(new_n17_), .b(new_n16_), .c(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n32_), .O(new_n53_));
  nand2  g38(.a(new_n52_), .b(new_n21_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n53_), .c(new_n18_), .O(new_n55_));
  nand2  g40(.a(new_n33_), .b(new_n18_), .O(new_n56_));
  oai21  g41(.a(x5), .b(x2), .c(new_n16_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n56_), .c(new_n21_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n55_), .O(z4));
  aoi21  g44(.a(x6), .b(new_n16_), .c(x1), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(x2), .c(new_n25_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n21_), .O(new_n62_));
  aoi21  g47(.a(new_n53_), .b(new_n22_), .c(new_n41_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n62_), .O(z5));
  nor3   g49(.a(x5), .b(x4), .c(x2), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(x3), .c(new_n19_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n55_), .O(z6));
endmodule


