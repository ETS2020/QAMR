// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x4), .O(new_n16_));
  nor2   g01(.a(x3), .b(x1), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x2), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n16_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x0), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(x2), .c(x1), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n20_), .O(z0));
  inv1   g09(.a(x0), .O(new_n25_));
  inv1   g10(.a(x1), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n21_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n26_), .c(x2), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nand2  g14(.a(x2), .b(new_n26_), .O(new_n30_));
  aoi21  g15(.a(new_n29_), .b(new_n21_), .c(new_n30_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n28_), .c(new_n25_), .O(new_n32_));
  inv1   g17(.a(x2), .O(new_n33_));
  nand3  g18(.a(new_n16_), .b(new_n26_), .c(x0), .O(new_n34_));
  inv1   g19(.a(new_n34_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(x3), .c(new_n33_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n32_), .O(z1));
  nand2  g22(.a(x4), .b(new_n33_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n21_), .c(new_n26_), .O(new_n39_));
  nand2  g24(.a(x5), .b(x2), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n21_), .c(new_n26_), .O(new_n41_));
  aoi21  g26(.a(x6), .b(new_n33_), .c(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n39_), .c(new_n25_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n36_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nor2   g30(.a(x2), .b(x0), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n45_), .c(x1), .O(new_n47_));
  nand3  g32(.a(new_n17_), .b(new_n29_), .c(x2), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n47_), .c(new_n20_), .O(z3));
  nand3  g34(.a(x3), .b(new_n33_), .c(new_n26_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x0), .O(new_n52_));
  oai21  g37(.a(new_n21_), .b(x1), .c(new_n46_), .O(new_n53_));
  oai21  g38(.a(new_n22_), .b(new_n17_), .c(x2), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z4));
  oai22  g40(.a(new_n40_), .b(x1), .c(new_n21_), .d(new_n33_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n28_), .c(new_n25_), .O(new_n57_));
  nand2  g42(.a(x4), .b(x0), .O(new_n58_));
  xor2a  g43(.a(x3), .b(x2), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n35_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(z5));
  nor2   g46(.a(x4), .b(x2), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(x3), .c(x1), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n48_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n25_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n60_), .O(z6));
endmodule


