// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:10 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  oai21  g02(.a(x1), .b(new_n16_), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(x0), .c(x3), .O(new_n20_));
  inv1   g05(.a(x6), .O(new_n21_));
  nor2   g06(.a(x7), .b(new_n21_), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nand4  g08(.a(new_n23_), .b(new_n20_), .c(new_n18_), .d(x2), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(z0));
  nor2   g10(.a(x1), .b(new_n16_), .O(new_n26_));
  nor2   g11(.a(new_n17_), .b(x2), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n26_), .c(new_n22_), .O(new_n28_));
  inv1   g13(.a(x2), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n17_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n19_), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n17_), .c(x2), .O(new_n33_));
  aoi21  g18(.a(x2), .b(x1), .c(x0), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n33_), .c(new_n31_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n28_), .O(z1));
  nand2  g21(.a(x4), .b(new_n29_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n17_), .c(new_n19_), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nor2   g24(.a(x3), .b(x1), .O(new_n40_));
  nand2  g25(.a(x6), .b(new_n29_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n38_), .c(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n28_), .O(z2));
  nor2   g30(.a(new_n19_), .b(x0), .O(new_n46_));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n46_), .c(new_n29_), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n16_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n40_), .c(x2), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n48_), .c(new_n23_), .O(z3));
  aoi21  g36(.a(new_n17_), .b(x2), .c(x0), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  xnor2a g38(.a(x3), .b(x2), .O(new_n54_));
  or2    g39(.a(new_n54_), .b(x1), .O(new_n55_));
  nand4  g40(.a(x3), .b(new_n29_), .c(new_n19_), .d(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n23_), .O(new_n57_));
  aoi21  g42(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(z4));
  oai22  g43(.a(new_n54_), .b(new_n16_), .c(new_n49_), .d(new_n29_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n19_), .O(new_n60_));
  aoi21  g45(.a(new_n52_), .b(new_n31_), .c(new_n22_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n60_), .O(z5));
  oai21  g47(.a(x4), .b(x2), .c(new_n17_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n46_), .O(new_n64_));
  oai21  g49(.a(new_n54_), .b(new_n16_), .c(new_n33_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n19_), .O(new_n66_));
  aoi21  g51(.a(new_n66_), .b(new_n64_), .c(new_n22_), .O(z6));
endmodule


