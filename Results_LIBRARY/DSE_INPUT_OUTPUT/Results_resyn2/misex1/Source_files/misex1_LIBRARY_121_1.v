// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:21 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x5), .O(new_n17_));
  nand3  g02(.a(x3), .b(x2), .c(x1), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n17_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n16_), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand4  g07(.a(new_n22_), .b(x2), .c(new_n21_), .d(x0), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n20_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  nor2   g10(.a(x5), .b(x0), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n21_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n26_), .c(new_n25_), .O(new_n29_));
  nor2   g14(.a(new_n26_), .b(new_n25_), .O(new_n30_));
  nor2   g15(.a(x2), .b(x0), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(x3), .c(new_n21_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n30_), .c(new_n29_), .O(z1));
  nand3  g19(.a(x3), .b(new_n25_), .c(new_n21_), .O(new_n35_));
  inv1   g20(.a(x4), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(x2), .c(new_n22_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x1), .O(new_n38_));
  nand2  g23(.a(new_n17_), .b(new_n16_), .O(new_n39_));
  nand2  g24(.a(x6), .b(new_n25_), .O(new_n40_));
  nor2   g25(.a(x3), .b(x1), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  aoi22  g27(.a(new_n42_), .b(new_n38_), .c(new_n35_), .d(x0), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n31_), .c(x1), .O(new_n45_));
  nand2  g30(.a(new_n41_), .b(x2), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n17_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n23_), .O(z3));
  xnor2a g34(.a(x3), .b(x2), .O(new_n50_));
  nand3  g35(.a(new_n17_), .b(new_n22_), .c(x2), .O(new_n51_));
  oai21  g36(.a(new_n50_), .b(new_n16_), .c(new_n51_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  nand2  g38(.a(new_n22_), .b(x2), .O(new_n54_));
  nand3  g39(.a(new_n35_), .b(new_n26_), .c(new_n54_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(z4));
  oai21  g41(.a(new_n22_), .b(new_n25_), .c(new_n17_), .O(new_n57_));
  aoi21  g42(.a(new_n28_), .b(new_n25_), .c(new_n57_), .O(new_n58_));
  nand2  g43(.a(x3), .b(new_n25_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n54_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n21_), .c(x0), .O(new_n61_));
  oai21  g46(.a(new_n58_), .b(x0), .c(new_n61_), .O(z5));
  oai21  g47(.a(x4), .b(x2), .c(new_n22_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n26_), .c(x1), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n53_), .O(z6));
endmodule


