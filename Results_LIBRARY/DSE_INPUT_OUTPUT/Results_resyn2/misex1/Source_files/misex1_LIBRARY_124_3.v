// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x7), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(x1), .c(new_n17_), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  inv1   g06(.a(x1), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x0), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n16_), .c(new_n21_), .O(new_n24_));
  oai21  g09(.a(new_n20_), .b(new_n16_), .c(new_n24_), .O(new_n25_));
  inv1   g10(.a(new_n25_), .O(z0));
  nand3  g11(.a(x6), .b(new_n16_), .c(new_n21_), .O(new_n27_));
  nor2   g12(.a(x5), .b(x3), .O(new_n28_));
  nand2  g13(.a(x2), .b(new_n22_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n17_), .O(new_n31_));
  nand2  g16(.a(x3), .b(new_n21_), .O(new_n32_));
  or2    g17(.a(new_n32_), .b(new_n23_), .O(new_n33_));
  nor2   g18(.a(x2), .b(x0), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x7), .c(x1), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n33_), .c(new_n31_), .O(z1));
  nand2  g21(.a(x1), .b(new_n17_), .O(new_n37_));
  oai21  g22(.a(new_n23_), .b(x2), .c(new_n37_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x3), .O(new_n39_));
  nand2  g24(.a(new_n34_), .b(x4), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n18_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x1), .O(new_n42_));
  inv1   g27(.a(x6), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n21_), .O(new_n44_));
  oai21  g29(.a(x5), .b(new_n21_), .c(new_n44_), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n16_), .c(new_n22_), .d(new_n17_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n42_), .c(new_n39_), .O(z2));
  nand2  g32(.a(x5), .b(new_n17_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n16_), .c(x2), .d(new_n22_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n35_), .O(z3));
  nand2  g35(.a(new_n32_), .b(new_n17_), .O(new_n51_));
  xor2a  g36(.a(x3), .b(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(x0), .O(new_n53_));
  nand2  g38(.a(new_n28_), .b(x2), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n22_), .O(new_n56_));
  oai21  g41(.a(x3), .b(new_n21_), .c(new_n20_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(z4));
  nand3  g43(.a(x5), .b(x2), .c(new_n17_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n53_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n22_), .O(new_n61_));
  nor2   g46(.a(new_n18_), .b(new_n22_), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n43_), .b(x3), .c(new_n21_), .O(new_n64_));
  aoi21  g49(.a(new_n16_), .b(x2), .c(x0), .O(new_n65_));
  oai21  g50(.a(new_n64_), .b(x1), .c(new_n65_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n63_), .c(new_n61_), .O(z5));
  aoi22  g52(.a(new_n52_), .b(x0), .c(new_n28_), .d(x2), .O(new_n68_));
  nor2   g53(.a(new_n22_), .b(x0), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n16_), .O(new_n70_));
  aoi21  g55(.a(new_n70_), .b(new_n69_), .c(new_n62_), .O(new_n71_));
  oai21  g56(.a(new_n68_), .b(x1), .c(new_n71_), .O(z6));
endmodule


