// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_;
  inv1   g00(.a(x1), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x0), .O(new_n17_));
  nand3  g02(.a(new_n17_), .b(x3), .c(x2), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(z0));
  inv1   g04(.a(x0), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  inv1   g06(.a(x6), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(x3), .c(new_n16_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n21_), .c(new_n20_), .O(new_n24_));
  nand2  g09(.a(x3), .b(new_n16_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n21_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x0), .O(new_n27_));
  nor2   g12(.a(new_n21_), .b(x1), .O(new_n28_));
  oai21  g13(.a(x5), .b(x3), .c(new_n28_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n24_), .O(z1));
  inv1   g15(.a(x3), .O(new_n31_));
  nand2  g16(.a(x4), .b(new_n21_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n31_), .c(new_n16_), .O(new_n33_));
  nand2  g18(.a(x5), .b(x2), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n31_), .c(new_n16_), .O(new_n35_));
  aoi21  g20(.a(x6), .b(new_n21_), .c(new_n35_), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(new_n33_), .c(new_n20_), .O(new_n37_));
  nand3  g22(.a(x3), .b(new_n16_), .c(x0), .O(new_n38_));
  inv1   g23(.a(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n21_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n37_), .O(z2));
  nor3   g26(.a(x5), .b(x3), .c(x1), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(x0), .c(x2), .O(new_n43_));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n17_), .c(new_n21_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z3));
  nand3  g31(.a(new_n25_), .b(new_n21_), .c(new_n20_), .O(new_n47_));
  nand2  g32(.a(new_n17_), .b(new_n31_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(x2), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n47_), .c(new_n38_), .O(z4));
  oai22  g35(.a(new_n34_), .b(x1), .c(new_n31_), .d(new_n21_), .O(new_n51_));
  aoi21  g36(.a(new_n23_), .b(new_n21_), .c(new_n51_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(x0), .c(new_n40_), .O(z5));
  oai21  g38(.a(x4), .b(x2), .c(new_n31_), .O(new_n54_));
  aoi22  g39(.a(new_n54_), .b(x1), .c(new_n42_), .d(x2), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(x0), .c(new_n40_), .O(z6));
endmodule


