// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  inv1   g04(.a(x2), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(x3), .c(x7), .O(new_n22_));
  oai22  g07(.a(new_n22_), .b(new_n16_), .c(new_n19_), .d(new_n17_), .O(z0));
  inv1   g08(.a(x0), .O(new_n24_));
  nand3  g09(.a(x6), .b(new_n18_), .c(new_n20_), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  nand2  g11(.a(x2), .b(new_n16_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n24_), .O(new_n29_));
  nand2  g14(.a(x3), .b(new_n20_), .O(new_n30_));
  or2    g15(.a(new_n30_), .b(new_n17_), .O(new_n31_));
  nor2   g16(.a(x2), .b(x0), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x7), .c(x1), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n31_), .c(new_n29_), .O(z1));
  nand2  g19(.a(x1), .b(new_n24_), .O(new_n35_));
  oai21  g20(.a(new_n17_), .b(x2), .c(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(x3), .O(new_n37_));
  inv1   g22(.a(x7), .O(new_n38_));
  nand2  g23(.a(new_n32_), .b(x4), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x1), .O(new_n41_));
  inv1   g26(.a(x6), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n20_), .O(new_n43_));
  oai21  g28(.a(x5), .b(new_n20_), .c(new_n43_), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n18_), .c(new_n16_), .d(new_n24_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n41_), .c(new_n37_), .O(z2));
  nand2  g31(.a(x5), .b(new_n24_), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n18_), .c(x2), .d(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n33_), .O(z3));
  nand2  g34(.a(new_n30_), .b(new_n24_), .O(new_n50_));
  xor2a  g35(.a(x3), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x0), .O(new_n52_));
  nand2  g37(.a(new_n26_), .b(x2), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n52_), .c(new_n50_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n16_), .O(new_n55_));
  inv1   g40(.a(new_n35_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n19_), .c(new_n38_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n55_), .O(z4));
  nand3  g43(.a(x5), .b(x2), .c(new_n24_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n52_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n16_), .O(new_n61_));
  nand4  g46(.a(new_n43_), .b(new_n30_), .c(new_n19_), .d(new_n24_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n61_), .c(new_n33_), .O(z5));
  aoi22  g48(.a(new_n51_), .b(x0), .c(new_n26_), .d(x2), .O(new_n64_));
  oai21  g49(.a(x4), .b(x2), .c(new_n18_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n56_), .c(new_n38_), .O(new_n66_));
  oai21  g51(.a(new_n64_), .b(x1), .c(new_n66_), .O(z6));
endmodule


