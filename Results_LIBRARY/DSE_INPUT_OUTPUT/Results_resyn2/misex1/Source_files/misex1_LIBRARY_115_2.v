// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x7), .b(x1), .c(new_n16_), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x3), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n16_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n18_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  inv1   g08(.a(x1), .O(new_n24_));
  nand2  g09(.a(x2), .b(new_n24_), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n19_), .c(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n25_), .b(new_n23_), .c(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n16_), .O(new_n29_));
  nand3  g14(.a(new_n26_), .b(new_n24_), .c(x0), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x3), .O(new_n32_));
  oai21  g17(.a(x2), .b(x0), .c(x7), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x1), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n32_), .c(new_n29_), .O(z1));
  nor2   g20(.a(new_n24_), .b(x0), .O(new_n36_));
  oai21  g21(.a(new_n31_), .b(new_n36_), .c(x3), .O(new_n37_));
  nand3  g22(.a(x4), .b(new_n26_), .c(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x7), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  inv1   g25(.a(x6), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n26_), .O(new_n42_));
  oai21  g27(.a(x5), .b(new_n26_), .c(new_n42_), .O(new_n43_));
  nor3   g28(.a(x3), .b(x1), .c(x0), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n40_), .c(new_n37_), .O(z2));
  nand2  g31(.a(x5), .b(new_n16_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(x2), .c(new_n24_), .O(new_n48_));
  inv1   g33(.a(new_n17_), .O(new_n49_));
  nor2   g34(.a(x4), .b(x2), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n48_), .c(x3), .O(z3));
  xor2a  g37(.a(x3), .b(x2), .O(new_n53_));
  oai21  g38(.a(x5), .b(x3), .c(x0), .O(new_n54_));
  aoi22  g39(.a(new_n54_), .b(x2), .c(new_n53_), .d(x0), .O(new_n55_));
  xnor2a g40(.a(x3), .b(x2), .O(new_n56_));
  aoi22  g41(.a(new_n56_), .b(new_n16_), .c(new_n33_), .d(x1), .O(new_n57_));
  oai21  g42(.a(new_n55_), .b(x1), .c(new_n57_), .O(z4));
  nand3  g43(.a(x5), .b(x2), .c(new_n16_), .O(new_n59_));
  oai21  g44(.a(new_n56_), .b(new_n16_), .c(new_n59_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n24_), .O(new_n61_));
  nand3  g46(.a(new_n56_), .b(new_n42_), .c(new_n16_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n61_), .c(new_n34_), .O(z5));
  aoi22  g48(.a(new_n53_), .b(x0), .c(new_n23_), .d(x2), .O(new_n64_));
  oai21  g49(.a(x4), .b(x2), .c(new_n19_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n49_), .O(new_n66_));
  oai21  g51(.a(new_n64_), .b(x1), .c(new_n66_), .O(z6));
endmodule


