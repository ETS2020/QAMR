// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  oai21  g02(.a(x1), .b(new_n16_), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(x0), .c(x3), .O(new_n20_));
  inv1   g05(.a(x7), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x5), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nand4  g08(.a(new_n23_), .b(new_n20_), .c(new_n18_), .d(x2), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nand4  g11(.a(x3), .b(new_n26_), .c(new_n19_), .d(x0), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  nor2   g13(.a(new_n28_), .b(new_n22_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n19_), .O(new_n31_));
  nor2   g16(.a(new_n31_), .b(x2), .O(new_n32_));
  nor2   g17(.a(x5), .b(x3), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x1), .c(x2), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(new_n32_), .c(new_n29_), .O(z1));
  nand2  g21(.a(x4), .b(new_n26_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n17_), .c(new_n19_), .O(new_n38_));
  nor2   g23(.a(x3), .b(x1), .O(new_n39_));
  nand2  g24(.a(x6), .b(new_n26_), .O(new_n40_));
  nand2  g25(.a(x5), .b(x2), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n38_), .c(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n29_), .O(z2));
  nand3  g30(.a(new_n17_), .b(x2), .c(new_n19_), .O(new_n46_));
  inv1   g31(.a(new_n46_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n16_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n47_), .c(new_n23_), .O(new_n49_));
  nor2   g34(.a(x4), .b(x3), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(x5), .c(new_n21_), .O(new_n51_));
  nor2   g36(.a(new_n19_), .b(x0), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n26_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(z3));
  nand2  g39(.a(x3), .b(new_n26_), .O(new_n55_));
  aoi21  g40(.a(new_n17_), .b(x2), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n55_), .b(x1), .c(new_n56_), .O(new_n57_));
  nor2   g42(.a(new_n47_), .b(new_n28_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n57_), .c(new_n22_), .O(z4));
  nor2   g44(.a(x1), .b(new_n16_), .O(new_n60_));
  nand2  g45(.a(new_n17_), .b(x2), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n55_), .O(new_n62_));
  aoi22  g47(.a(new_n62_), .b(new_n60_), .c(new_n56_), .d(new_n31_), .O(new_n63_));
  inv1   g48(.a(x5), .O(new_n64_));
  oai22  g49(.a(x7), .b(new_n17_), .c(new_n64_), .d(x1), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(x2), .c(new_n16_), .O(new_n66_));
  oai21  g51(.a(new_n63_), .b(new_n22_), .c(new_n66_), .O(z5));
  oai21  g52(.a(x4), .b(x2), .c(new_n17_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  aoi22  g54(.a(new_n62_), .b(new_n60_), .c(new_n47_), .d(new_n64_), .O(new_n70_));
  aoi21  g55(.a(new_n70_), .b(new_n69_), .c(new_n22_), .O(z6));
endmodule


