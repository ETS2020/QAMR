// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x5), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  inv1   g03(.a(x2), .O(new_n19_));
  aoi21  g04(.a(x3), .b(x0), .c(new_n19_), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nor2   g06(.a(x1), .b(new_n21_), .O(new_n22_));
  inv1   g07(.a(x1), .O(new_n23_));
  nor2   g08(.a(new_n16_), .b(new_n23_), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n22_), .c(new_n20_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n18_), .O(z0));
  nand2  g11(.a(x3), .b(new_n19_), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n22_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n23_), .O(new_n31_));
  nor2   g16(.a(new_n19_), .b(x1), .O(new_n32_));
  aoi21  g17(.a(new_n31_), .b(new_n19_), .c(new_n32_), .O(new_n33_));
  nand2  g18(.a(new_n18_), .b(new_n21_), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(new_n33_), .c(new_n29_), .O(z1));
  nand3  g20(.a(x5), .b(x4), .c(new_n19_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n16_), .c(new_n23_), .O(new_n37_));
  nand2  g22(.a(x5), .b(new_n19_), .O(new_n38_));
  nand3  g23(.a(new_n30_), .b(new_n16_), .c(new_n23_), .O(new_n39_));
  nor2   g24(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n37_), .c(new_n21_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n29_), .O(z2));
  nand3  g27(.a(x2), .b(new_n23_), .c(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x5), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n16_), .O(new_n45_));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n19_), .c(x1), .d(new_n21_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z3));
  nand3  g33(.a(x3), .b(new_n19_), .c(x0), .O(new_n49_));
  inv1   g34(.a(new_n49_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n20_), .c(new_n23_), .O(new_n51_));
  aoi21  g36(.a(new_n16_), .b(x2), .c(x0), .O(new_n52_));
  oai21  g37(.a(new_n27_), .b(x1), .c(new_n52_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n51_), .c(new_n18_), .O(z4));
  oai21  g39(.a(new_n31_), .b(x2), .c(new_n52_), .O(new_n55_));
  nand2  g40(.a(new_n16_), .b(x2), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n23_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n55_), .c(new_n18_), .O(z5));
  oai21  g44(.a(new_n56_), .b(new_n17_), .c(new_n27_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n22_), .O(new_n61_));
  oai21  g46(.a(new_n38_), .b(x4), .c(new_n16_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(x1), .c(new_n21_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(z6));
endmodule


