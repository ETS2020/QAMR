// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x1), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x0), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x3), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nor2   g05(.a(x1), .b(new_n20_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n19_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n18_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  inv1   g09(.a(x4), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(x3), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n24_), .O(z0));
  aoi21  g13(.a(x6), .b(new_n19_), .c(x2), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n16_), .O(new_n30_));
  nor2   g15(.a(x5), .b(x3), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x1), .c(x2), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n30_), .c(new_n20_), .O(new_n33_));
  nor2   g18(.a(new_n19_), .b(x2), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n21_), .c(new_n26_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(z1));
  inv1   g21(.a(x2), .O(new_n37_));
  nand3  g22(.a(x3), .b(new_n37_), .c(x0), .O(new_n38_));
  nand3  g23(.a(new_n25_), .b(new_n19_), .c(new_n20_), .O(new_n39_));
  inv1   g24(.a(x6), .O(new_n40_));
  nand2  g25(.a(x5), .b(x2), .O(new_n41_));
  oai21  g26(.a(new_n40_), .b(x2), .c(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n39_), .c(new_n38_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n18_), .O(z2));
  nand2  g30(.a(x7), .b(x3), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n27_), .c(new_n17_), .d(new_n37_), .O(new_n47_));
  nand3  g32(.a(new_n25_), .b(new_n19_), .c(x2), .O(new_n48_));
  aoi21  g33(.a(x5), .b(new_n20_), .c(new_n48_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n47_), .O(z3));
  nand2  g36(.a(new_n19_), .b(x2), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n38_), .c(x1), .O(new_n53_));
  nor2   g38(.a(new_n53_), .b(new_n26_), .O(new_n54_));
  nand2  g39(.a(new_n34_), .b(new_n16_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n52_), .c(new_n20_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n54_), .O(z4));
  nand2  g42(.a(new_n25_), .b(new_n37_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n19_), .c(new_n16_), .O(new_n59_));
  nor2   g44(.a(x3), .b(new_n37_), .O(new_n60_));
  nor2   g45(.a(new_n60_), .b(new_n29_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n20_), .O(new_n62_));
  inv1   g47(.a(x5), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n20_), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n53_), .c(new_n26_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(z5));
  inv1   g51(.a(new_n38_), .O(new_n67_));
  oai21  g52(.a(new_n49_), .b(new_n67_), .c(new_n16_), .O(new_n68_));
  nand2  g53(.a(new_n59_), .b(new_n20_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n68_), .O(z6));
endmodule


