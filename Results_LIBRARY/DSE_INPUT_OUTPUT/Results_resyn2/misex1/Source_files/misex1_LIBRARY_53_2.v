// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x7), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x6), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand2  g03(.a(x1), .b(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x3), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  oai21  g06(.a(x1), .b(new_n18_), .c(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n20_), .c(x2), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n17_), .O(z0));
  xnor2a g09(.a(x2), .b(x1), .O(new_n25_));
  inv1   g10(.a(x5), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n21_), .c(x2), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n18_), .O(new_n28_));
  inv1   g13(.a(x1), .O(new_n29_));
  inv1   g14(.a(x2), .O(new_n30_));
  nand4  g15(.a(x3), .b(new_n30_), .c(new_n29_), .d(x0), .O(new_n31_));
  oai21  g16(.a(new_n28_), .b(new_n25_), .c(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n17_), .O(new_n33_));
  nor2   g18(.a(x2), .b(x0), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(x7), .c(x6), .d(new_n21_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(z1));
  aoi21  g21(.a(x4), .b(new_n30_), .c(x3), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n19_), .c(new_n31_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n17_), .O(new_n39_));
  oai21  g24(.a(new_n16_), .b(new_n30_), .c(x6), .O(new_n40_));
  aoi21  g25(.a(x5), .b(x2), .c(x0), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(new_n40_), .c(new_n21_), .d(new_n29_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n39_), .O(z2));
  nand2  g28(.a(x5), .b(new_n18_), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n17_), .c(x2), .d(new_n29_), .O(new_n45_));
  nor2   g30(.a(new_n29_), .b(x0), .O(new_n46_));
  nor2   g31(.a(x4), .b(x2), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n46_), .c(x7), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n21_), .O(new_n50_));
  nor3   g35(.a(x7), .b(x6), .c(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z3));
  nand2  g38(.a(new_n21_), .b(x2), .O(new_n54_));
  nand2  g39(.a(x3), .b(new_n30_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n18_), .c(new_n54_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n29_), .O(new_n57_));
  nand2  g42(.a(new_n55_), .b(new_n54_), .O(new_n58_));
  oai21  g43(.a(x2), .b(new_n29_), .c(new_n58_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n18_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n57_), .c(new_n17_), .O(z4));
  inv1   g46(.a(new_n17_), .O(new_n62_));
  nand3  g47(.a(new_n55_), .b(new_n54_), .c(x0), .O(new_n63_));
  nor2   g48(.a(new_n41_), .b(x1), .O(new_n64_));
  nand2  g49(.a(new_n30_), .b(new_n29_), .O(new_n65_));
  aoi21  g50(.a(new_n21_), .b(x2), .c(x0), .O(new_n66_));
  aoi22  g51(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n62_), .c(new_n35_), .O(z5));
  nand3  g53(.a(new_n58_), .b(new_n28_), .c(new_n29_), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n21_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n69_), .c(new_n17_), .O(z6));
endmodule


