// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_;
  inv1   g00(.a(x1), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x0), .O(new_n17_));
  nor2   g02(.a(x3), .b(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(new_n17_), .O(new_n19_));
  oai21  g04(.a(x3), .b(x0), .c(x2), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n19_), .O(z0));
  inv1   g06(.a(x2), .O(new_n22_));
  nor2   g07(.a(new_n22_), .b(x1), .O(new_n23_));
  oai21  g08(.a(x5), .b(x3), .c(new_n23_), .O(new_n24_));
  nor2   g09(.a(x3), .b(x2), .O(new_n25_));
  oai21  g10(.a(x6), .b(x1), .c(new_n25_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n24_), .c(x0), .O(z1));
  nand2  g12(.a(x3), .b(x2), .O(new_n28_));
  inv1   g13(.a(x3), .O(new_n29_));
  nand3  g14(.a(x4), .b(new_n29_), .c(new_n22_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(new_n32_));
  nand2  g17(.a(x5), .b(x2), .O(new_n33_));
  nand2  g18(.a(x6), .b(new_n22_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n33_), .c(new_n18_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n32_), .c(x0), .O(z2));
  nand2  g21(.a(x7), .b(x4), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n17_), .c(x3), .O(new_n38_));
  inv1   g23(.a(x0), .O(new_n39_));
  nand2  g24(.a(x5), .b(new_n39_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n23_), .c(new_n29_), .O(new_n41_));
  oai21  g26(.a(new_n38_), .b(x2), .c(new_n41_), .O(z3));
  inv1   g27(.a(new_n18_), .O(new_n43_));
  inv1   g28(.a(new_n28_), .O(new_n44_));
  nor2   g29(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  nand2  g30(.a(new_n29_), .b(new_n22_), .O(new_n46_));
  aoi21  g31(.a(new_n28_), .b(new_n46_), .c(new_n39_), .O(new_n47_));
  aoi21  g32(.a(new_n45_), .b(new_n43_), .c(new_n47_), .O(z4));
  oai21  g33(.a(x6), .b(x1), .c(new_n22_), .O(new_n49_));
  inv1   g34(.a(x5), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n39_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n23_), .c(x3), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z5));
  oai21  g38(.a(new_n46_), .b(x4), .c(new_n28_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n41_), .O(z6));
endmodule


