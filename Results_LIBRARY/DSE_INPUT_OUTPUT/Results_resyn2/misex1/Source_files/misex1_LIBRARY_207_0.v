// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x1), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x5), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nor2   g06(.a(x1), .b(new_n21_), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(x3), .c(x2), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(new_n20_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  nand2  g11(.a(x1), .b(new_n21_), .O(new_n27_));
  nand2  g12(.a(new_n16_), .b(x0), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n26_), .c(new_n27_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  inv1   g15(.a(x6), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x3), .c(new_n25_), .O(new_n32_));
  nor2   g17(.a(x1), .b(x0), .O(new_n33_));
  nor2   g18(.a(x5), .b(x3), .O(new_n34_));
  aoi21  g19(.a(new_n33_), .b(new_n32_), .c(new_n34_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n30_), .O(z1));
  nor2   g21(.a(x2), .b(x1), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(x3), .c(new_n21_), .O(new_n38_));
  nand3  g23(.a(x5), .b(x4), .c(new_n25_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n26_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x1), .O(new_n41_));
  nor3   g26(.a(x6), .b(x2), .c(x1), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n19_), .c(x0), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n41_), .c(new_n38_), .O(z2));
  aoi21  g29(.a(new_n22_), .b(x2), .c(new_n18_), .O(new_n45_));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n17_), .c(new_n25_), .O(new_n47_));
  oai21  g32(.a(new_n45_), .b(x3), .c(new_n47_), .O(z3));
  nand3  g33(.a(x3), .b(new_n25_), .c(x0), .O(new_n49_));
  inv1   g34(.a(new_n49_), .O(new_n50_));
  aoi21  g35(.a(x3), .b(x0), .c(new_n25_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n50_), .c(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n37_), .b(x3), .O(new_n53_));
  aoi21  g38(.a(new_n26_), .b(x2), .c(x0), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n53_), .c(new_n34_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n52_), .O(z4));
  inv1   g41(.a(new_n34_), .O(new_n57_));
  nor2   g42(.a(x3), .b(new_n25_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n50_), .c(new_n16_), .O(new_n59_));
  oai21  g44(.a(new_n32_), .b(x1), .c(new_n54_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(z5));
  nor2   g46(.a(new_n26_), .b(x2), .O(new_n62_));
  oai21  g47(.a(new_n58_), .b(new_n62_), .c(new_n22_), .O(new_n63_));
  oai21  g48(.a(x4), .b(x2), .c(new_n26_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n17_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n63_), .c(new_n57_), .O(z6));
endmodule


