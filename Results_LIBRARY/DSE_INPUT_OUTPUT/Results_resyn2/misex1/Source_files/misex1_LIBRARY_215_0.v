// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:09 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x6), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nor2   g02(.a(x3), .b(new_n17_), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x0), .c(new_n16_), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(x0), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x3), .c(x2), .O(new_n22_));
  oai21  g07(.a(new_n19_), .b(x1), .c(new_n22_), .O(z0));
  nor2   g08(.a(x2), .b(new_n20_), .O(new_n24_));
  nand2  g09(.a(x2), .b(new_n20_), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n17_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nor2   g13(.a(x5), .b(x3), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(x2), .c(new_n16_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n28_), .c(new_n24_), .O(new_n31_));
  nand3  g16(.a(x3), .b(new_n17_), .c(x0), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nor2   g18(.a(new_n16_), .b(x1), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g20(.a(new_n31_), .b(x0), .c(new_n35_), .O(z1));
  nor2   g21(.a(new_n17_), .b(x1), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n26_), .O(new_n39_));
  aoi22  g24(.a(new_n39_), .b(x1), .c(new_n29_), .d(new_n37_), .O(new_n40_));
  oai21  g25(.a(new_n33_), .b(new_n16_), .c(new_n20_), .O(new_n41_));
  oai21  g26(.a(new_n40_), .b(x0), .c(new_n41_), .O(z2));
  inv1   g27(.a(x0), .O(new_n43_));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n24_), .c(new_n43_), .O(new_n45_));
  nand2  g30(.a(x5), .b(new_n43_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n34_), .c(new_n18_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z3));
  aoi21  g33(.a(new_n26_), .b(x2), .c(new_n20_), .O(new_n49_));
  xor2a  g34(.a(x3), .b(x2), .O(new_n50_));
  inv1   g35(.a(new_n50_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(x6), .c(new_n49_), .O(new_n52_));
  oai21  g37(.a(new_n33_), .b(new_n18_), .c(new_n34_), .O(new_n53_));
  oai21  g38(.a(new_n52_), .b(x0), .c(new_n53_), .O(z4));
  nand3  g39(.a(x5), .b(x2), .c(new_n20_), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n50_), .c(new_n16_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n49_), .c(new_n43_), .O(new_n57_));
  nand3  g42(.a(new_n50_), .b(new_n34_), .c(x0), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n57_), .O(z5));
  nand2  g44(.a(new_n50_), .b(x0), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n30_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n20_), .O(new_n62_));
  oai21  g47(.a(x4), .b(x2), .c(new_n26_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n21_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n62_), .O(z6));
endmodule


