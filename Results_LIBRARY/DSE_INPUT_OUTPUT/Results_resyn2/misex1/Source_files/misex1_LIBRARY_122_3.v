// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand2  g03(.a(x3), .b(new_n18_), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(x1), .c(x2), .O(new_n21_));
  aoi21  g06(.a(new_n19_), .b(new_n17_), .c(new_n21_), .O(z0));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  nand2  g08(.a(x2), .b(new_n16_), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n20_), .c(new_n25_), .O(new_n26_));
  oai21  g11(.a(new_n24_), .b(new_n23_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n18_), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n25_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n16_), .O(new_n30_));
  oai21  g15(.a(x2), .b(new_n16_), .c(new_n18_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(z1));
  nand3  g18(.a(x3), .b(new_n25_), .c(new_n16_), .O(new_n34_));
  and2   g19(.a(new_n34_), .b(x0), .O(new_n35_));
  nand2  g20(.a(x6), .b(new_n25_), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand4  g22(.a(new_n37_), .b(new_n36_), .c(new_n20_), .d(new_n16_), .O(new_n38_));
  inv1   g23(.a(x4), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(x2), .c(new_n20_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(x1), .c(x0), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n38_), .c(new_n35_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n25_), .c(x1), .O(new_n44_));
  inv1   g29(.a(x5), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n20_), .c(x2), .O(new_n46_));
  inv1   g31(.a(new_n46_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n20_), .b(x2), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x0), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n48_), .c(new_n44_), .O(z3));
  xor2a  g37(.a(x3), .b(x2), .O(new_n53_));
  oai21  g38(.a(x5), .b(x3), .c(x0), .O(new_n54_));
  aoi22  g39(.a(new_n54_), .b(x2), .c(new_n53_), .d(x0), .O(new_n55_));
  nand3  g40(.a(new_n49_), .b(new_n34_), .c(new_n18_), .O(new_n56_));
  oai21  g41(.a(new_n55_), .b(x1), .c(new_n56_), .O(z4));
  nand2  g42(.a(new_n49_), .b(x1), .O(new_n58_));
  nand2  g43(.a(new_n30_), .b(x0), .O(new_n59_));
  nand2  g44(.a(new_n36_), .b(new_n20_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n29_), .c(new_n18_), .O(new_n61_));
  nand2  g46(.a(new_n45_), .b(new_n18_), .O(new_n62_));
  nand4  g47(.a(new_n62_), .b(new_n20_), .c(x2), .d(new_n16_), .O(new_n63_));
  nand4  g48(.a(new_n63_), .b(new_n61_), .c(new_n59_), .d(new_n58_), .O(z5));
  xnor2a g49(.a(x3), .b(x2), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n18_), .c(new_n46_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n16_), .O(new_n67_));
  oai21  g52(.a(x4), .b(x2), .c(new_n20_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(x1), .c(new_n18_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n67_), .O(z6));
endmodule


