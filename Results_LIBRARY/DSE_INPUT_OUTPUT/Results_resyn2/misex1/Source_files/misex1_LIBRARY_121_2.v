// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:21 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand3  g02(.a(new_n17_), .b(x2), .c(new_n16_), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x5), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x0), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nand2  g06(.a(x2), .b(new_n21_), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(x3), .c(x1), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n20_), .O(z0));
  nand2  g10(.a(x2), .b(new_n16_), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n17_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n16_), .c(x2), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n28_), .c(new_n21_), .O(new_n31_));
  inv1   g16(.a(x2), .O(new_n32_));
  nand3  g17(.a(x3), .b(new_n32_), .c(new_n16_), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  nand2  g19(.a(x5), .b(x0), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n31_), .O(z1));
  aoi21  g23(.a(x4), .b(new_n32_), .c(x3), .O(new_n39_));
  inv1   g24(.a(x6), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n17_), .c(new_n32_), .d(new_n16_), .O(new_n41_));
  oai21  g26(.a(new_n39_), .b(new_n16_), .c(new_n41_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n21_), .O(new_n43_));
  aoi21  g28(.a(new_n33_), .b(x5), .c(new_n21_), .O(new_n44_));
  nand3  g29(.a(new_n27_), .b(x2), .c(new_n16_), .O(new_n45_));
  inv1   g30(.a(new_n45_), .O(new_n46_));
  nor2   g31(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n43_), .O(z2));
  oai21  g33(.a(x4), .b(x3), .c(x7), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n32_), .c(x1), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n21_), .O(new_n52_));
  inv1   g37(.a(new_n18_), .O(new_n53_));
  nand2  g38(.a(new_n36_), .b(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n52_), .O(z3));
  nand2  g40(.a(x3), .b(new_n32_), .O(new_n56_));
  nand2  g41(.a(new_n17_), .b(x2), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n56_), .c(new_n35_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n23_), .c(new_n16_), .O(new_n59_));
  nand3  g44(.a(new_n57_), .b(new_n33_), .c(new_n21_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n59_), .O(z4));
  nand3  g46(.a(x5), .b(x2), .c(new_n16_), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n30_), .c(new_n21_), .O(new_n64_));
  nand3  g49(.a(new_n32_), .b(new_n16_), .c(x0), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n22_), .O(new_n66_));
  aoi22  g51(.a(new_n66_), .b(x3), .c(new_n19_), .d(x0), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n64_), .O(z5));
  nand3  g53(.a(new_n27_), .b(x2), .c(new_n21_), .O(new_n69_));
  inv1   g54(.a(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n58_), .c(new_n16_), .O(new_n71_));
  oai21  g56(.a(x4), .b(x2), .c(new_n17_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(x1), .c(new_n21_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n71_), .O(z6));
endmodule


