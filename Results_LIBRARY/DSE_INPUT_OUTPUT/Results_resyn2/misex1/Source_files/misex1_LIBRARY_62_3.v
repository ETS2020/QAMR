// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x2), .O(new_n18_));
  inv1   g03(.a(x2), .O(new_n19_));
  nor2   g04(.a(new_n17_), .b(new_n19_), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(x0), .O(new_n21_));
  aoi21  g06(.a(new_n18_), .b(new_n16_), .c(new_n21_), .O(z0));
  inv1   g07(.a(x0), .O(new_n23_));
  nor2   g08(.a(x5), .b(x3), .O(new_n24_));
  nand2  g09(.a(x2), .b(new_n16_), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n17_), .c(new_n19_), .O(new_n26_));
  oai21  g11(.a(new_n25_), .b(new_n24_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n23_), .O(new_n28_));
  oai21  g13(.a(new_n17_), .b(x2), .c(new_n16_), .O(new_n29_));
  oai21  g14(.a(x2), .b(new_n16_), .c(new_n23_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n28_), .O(z1));
  nand3  g17(.a(x3), .b(new_n19_), .c(new_n16_), .O(new_n33_));
  and2   g18(.a(new_n33_), .b(x0), .O(new_n34_));
  nand2  g19(.a(x6), .b(new_n19_), .O(new_n35_));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n35_), .c(new_n17_), .d(new_n16_), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n17_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(x1), .c(x0), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n37_), .c(new_n34_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n19_), .c(x1), .O(new_n43_));
  inv1   g28(.a(x5), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n17_), .c(x2), .O(new_n45_));
  inv1   g30(.a(new_n45_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n16_), .O(new_n47_));
  nand2  g32(.a(new_n18_), .b(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(x0), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n47_), .c(new_n43_), .O(z3));
  xor2a  g35(.a(x3), .b(x2), .O(new_n51_));
  oai21  g36(.a(x5), .b(x3), .c(x0), .O(new_n52_));
  aoi22  g37(.a(new_n52_), .b(x2), .c(new_n51_), .d(x0), .O(new_n53_));
  nand3  g38(.a(new_n33_), .b(new_n18_), .c(new_n23_), .O(new_n54_));
  oai21  g39(.a(new_n53_), .b(x1), .c(new_n54_), .O(z4));
  inv1   g40(.a(new_n26_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n20_), .c(new_n23_), .O(new_n57_));
  nand2  g42(.a(new_n44_), .b(new_n23_), .O(new_n58_));
  nand4  g43(.a(new_n58_), .b(new_n17_), .c(x2), .d(new_n16_), .O(new_n59_));
  aoi22  g44(.a(new_n29_), .b(x0), .c(new_n18_), .d(x1), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(z5));
  xnor2a g46(.a(x3), .b(x2), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n23_), .c(new_n45_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n16_), .O(new_n64_));
  oai21  g49(.a(x4), .b(x2), .c(new_n17_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(x1), .c(new_n23_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z6));
endmodule


