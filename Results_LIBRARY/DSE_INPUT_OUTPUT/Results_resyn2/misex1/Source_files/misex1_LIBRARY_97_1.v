// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x5), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x2), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n19_), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  oai21  g09(.a(x3), .b(x0), .c(x2), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(new_n24_), .c(new_n18_), .O(z0));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n19_), .O(new_n28_));
  nor2   g13(.a(x5), .b(x2), .O(new_n29_));
  nand2  g14(.a(new_n16_), .b(new_n21_), .O(new_n30_));
  inv1   g15(.a(x2), .O(new_n31_));
  nor2   g16(.a(new_n31_), .b(x1), .O(new_n32_));
  aoi22  g17(.a(new_n32_), .b(new_n30_), .c(new_n29_), .d(new_n28_), .O(new_n33_));
  nand3  g18(.a(x3), .b(new_n19_), .c(x0), .O(new_n34_));
  inv1   g19(.a(new_n34_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n29_), .O(new_n36_));
  oai21  g21(.a(new_n33_), .b(x0), .c(new_n36_), .O(z1));
  inv1   g22(.a(x0), .O(new_n38_));
  nand2  g23(.a(x4), .b(new_n31_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n21_), .c(new_n19_), .O(new_n40_));
  nand2  g25(.a(new_n16_), .b(x2), .O(new_n41_));
  nand2  g26(.a(new_n27_), .b(new_n31_), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n41_), .c(new_n22_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n40_), .c(new_n38_), .O(new_n44_));
  oai21  g29(.a(new_n35_), .b(x5), .c(new_n31_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n20_), .c(x5), .O(new_n48_));
  nor2   g33(.a(new_n16_), .b(x0), .O(new_n49_));
  nand2  g34(.a(new_n23_), .b(x2), .O(new_n50_));
  oai22  g35(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x2), .O(z3));
  nand2  g36(.a(new_n21_), .b(x2), .O(new_n52_));
  nand3  g37(.a(x3), .b(new_n31_), .c(x0), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n19_), .O(new_n55_));
  nand3  g40(.a(x3), .b(new_n31_), .c(new_n19_), .O(new_n56_));
  aoi21  g41(.a(new_n21_), .b(x2), .c(x0), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n56_), .c(new_n17_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n55_), .O(z4));
  nand3  g44(.a(new_n16_), .b(x3), .c(new_n31_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n52_), .c(new_n38_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n49_), .c(new_n19_), .O(new_n62_));
  nand2  g47(.a(x6), .b(new_n21_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n31_), .c(new_n19_), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n57_), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(z5));
  nand3  g51(.a(new_n16_), .b(new_n21_), .c(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n61_), .c(new_n19_), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n21_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(new_n20_), .c(new_n18_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n69_), .O(z6));
endmodule


