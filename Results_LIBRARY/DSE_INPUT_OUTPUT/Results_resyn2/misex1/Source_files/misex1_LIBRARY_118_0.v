// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x2), .O(new_n16_));
  nand2  g01(.a(x7), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand3  g03(.a(x3), .b(x1), .c(new_n18_), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(x0), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n19_), .c(x2), .O(new_n23_));
  and2   g08(.a(new_n23_), .b(new_n17_), .O(z0));
  inv1   g09(.a(x5), .O(new_n25_));
  nand2  g10(.a(x2), .b(new_n20_), .O(new_n26_));
  aoi21  g11(.a(new_n25_), .b(new_n21_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n21_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n20_), .c(new_n17_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n27_), .c(new_n18_), .O(new_n30_));
  nand3  g15(.a(x7), .b(x3), .c(new_n16_), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n20_), .c(x0), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n30_), .O(z1));
  nand2  g19(.a(x4), .b(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n21_), .c(new_n20_), .O(new_n36_));
  nand2  g21(.a(new_n25_), .b(x2), .O(new_n37_));
  inv1   g22(.a(x6), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n21_), .b(new_n20_), .O(new_n40_));
  aoi21  g25(.a(new_n39_), .b(new_n37_), .c(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n36_), .c(new_n18_), .O(new_n42_));
  nand2  g27(.a(x3), .b(new_n20_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n18_), .c(x7), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n42_), .O(z2));
  nand2  g31(.a(x5), .b(new_n18_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(x2), .c(new_n20_), .O(new_n48_));
  inv1   g33(.a(x4), .O(new_n49_));
  inv1   g34(.a(new_n17_), .O(new_n50_));
  nor2   g35(.a(new_n20_), .b(x0), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n48_), .c(x3), .O(z3));
  nor2   g38(.a(new_n21_), .b(new_n16_), .O(new_n54_));
  aoi21  g39(.a(new_n43_), .b(new_n50_), .c(new_n54_), .O(new_n55_));
  nand2  g40(.a(new_n21_), .b(x2), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n31_), .O(new_n57_));
  nand2  g42(.a(new_n56_), .b(new_n18_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(new_n20_), .O(new_n59_));
  oai21  g44(.a(new_n55_), .b(x0), .c(new_n59_), .O(z4));
  aoi21  g45(.a(new_n56_), .b(new_n31_), .c(new_n18_), .O(new_n61_));
  nand3  g46(.a(x5), .b(x2), .c(new_n18_), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n61_), .c(new_n20_), .O(new_n64_));
  oai21  g49(.a(new_n54_), .b(new_n29_), .c(new_n18_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n64_), .O(z5));
  nand3  g51(.a(new_n25_), .b(new_n21_), .c(x2), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n61_), .c(new_n20_), .O(new_n69_));
  oai21  g54(.a(x7), .b(x2), .c(x3), .O(new_n70_));
  oai21  g55(.a(new_n17_), .b(x4), .c(new_n70_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n51_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n69_), .O(z6));
endmodule


