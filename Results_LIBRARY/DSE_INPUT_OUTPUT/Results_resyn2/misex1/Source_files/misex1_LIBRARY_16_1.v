// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:37 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n38_, new_n39_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x1), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x2), .O(new_n19_));
  nor2   g04(.a(x3), .b(new_n19_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  oai21  g06(.a(new_n21_), .b(new_n16_), .c(new_n17_), .O(z0));
  inv1   g07(.a(x3), .O(new_n23_));
  inv1   g08(.a(x5), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n23_), .c(x2), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(new_n19_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(new_n25_), .c(new_n16_), .O(new_n28_));
  nor2   g13(.a(new_n23_), .b(x2), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x0), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n28_), .c(x1), .O(z1));
  nand2  g16(.a(x5), .b(x2), .O(new_n32_));
  nand2  g17(.a(x6), .b(new_n19_), .O(new_n33_));
  nand4  g18(.a(new_n33_), .b(new_n32_), .c(new_n23_), .d(new_n16_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n30_), .c(x1), .O(z2));
  nor2   g20(.a(new_n24_), .b(x0), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(new_n21_), .c(new_n17_), .O(z3));
  oai21  g22(.a(new_n23_), .b(x2), .c(new_n16_), .O(new_n38_));
  aoi21  g23(.a(new_n29_), .b(x0), .c(new_n20_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n38_), .c(x1), .O(z4));
  aoi21  g25(.a(new_n27_), .b(new_n25_), .c(x1), .O(new_n41_));
  nor2   g26(.a(x1), .b(new_n16_), .O(new_n42_));
  oai21  g27(.a(new_n29_), .b(new_n20_), .c(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n41_), .b(x0), .c(new_n43_), .O(z5));
  nand2  g29(.a(new_n25_), .b(new_n18_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n16_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n43_), .O(z6));
endmodule


