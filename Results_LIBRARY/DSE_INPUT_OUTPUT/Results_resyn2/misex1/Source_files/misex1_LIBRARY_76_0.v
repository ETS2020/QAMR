// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  nand2  g05(.a(x3), .b(new_n16_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  aoi21  g07(.a(new_n20_), .b(new_n17_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x5), .b(new_n24_), .c(x1), .O(new_n25_));
  inv1   g10(.a(new_n25_), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n18_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n24_), .O(new_n28_));
  nor2   g13(.a(x5), .b(x3), .O(new_n29_));
  nor2   g14(.a(new_n29_), .b(x1), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n28_), .c(new_n26_), .O(new_n31_));
  nand3  g16(.a(x5), .b(x3), .c(new_n24_), .O(new_n32_));
  or2    g17(.a(new_n32_), .b(new_n17_), .O(new_n33_));
  oai21  g18(.a(new_n31_), .b(x0), .c(new_n33_), .O(z1));
  inv1   g19(.a(x4), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x2), .c(new_n18_), .O(new_n36_));
  nor2   g21(.a(x3), .b(x1), .O(new_n37_));
  oai21  g22(.a(x6), .b(x2), .c(x5), .O(new_n38_));
  aoi22  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(x1), .O(new_n39_));
  inv1   g24(.a(x0), .O(new_n40_));
  oai21  g25(.a(new_n21_), .b(new_n40_), .c(x5), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n24_), .O(new_n42_));
  oai21  g27(.a(new_n39_), .b(x0), .c(new_n42_), .O(z2));
  inv1   g28(.a(x5), .O(new_n44_));
  nand2  g29(.a(x1), .b(new_n40_), .O(new_n45_));
  inv1   g30(.a(new_n45_), .O(new_n46_));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n48_));
  oai21  g33(.a(new_n24_), .b(new_n40_), .c(x5), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  oai21  g35(.a(new_n48_), .b(x2), .c(new_n50_), .O(z3));
  aoi21  g36(.a(x3), .b(new_n16_), .c(x0), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n41_), .c(new_n24_), .O(new_n53_));
  nand2  g38(.a(x5), .b(new_n24_), .O(new_n54_));
  aoi22  g39(.a(new_n54_), .b(new_n37_), .c(new_n19_), .d(x2), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(z4));
  nand2  g41(.a(new_n18_), .b(x2), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n32_), .c(new_n40_), .O(new_n58_));
  nand2  g43(.a(x5), .b(new_n40_), .O(new_n59_));
  aoi21  g44(.a(new_n27_), .b(new_n24_), .c(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n16_), .O(new_n61_));
  nor2   g46(.a(new_n18_), .b(new_n24_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n26_), .c(new_n40_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n61_), .O(z5));
  oai21  g49(.a(new_n58_), .b(new_n29_), .c(new_n16_), .O(new_n65_));
  oai21  g50(.a(new_n45_), .b(x4), .c(x5), .O(new_n66_));
  nand3  g51(.a(x3), .b(x1), .c(new_n40_), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  aoi21  g53(.a(new_n66_), .b(new_n24_), .c(new_n68_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n65_), .O(z6));
endmodule


