// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n16_), .O(z0));
  nor2   g06(.a(new_n16_), .b(x0), .O(new_n22_));
  nand3  g07(.a(x3), .b(new_n16_), .c(x0), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n22_), .c(new_n18_), .O(new_n25_));
  inv1   g10(.a(x0), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai22  g12(.a(new_n27_), .b(x3), .c(x2), .d(new_n18_), .O(new_n28_));
  nor2   g13(.a(x3), .b(new_n16_), .O(new_n29_));
  aoi21  g14(.a(new_n28_), .b(new_n26_), .c(new_n29_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n25_), .O(z1));
  nor3   g16(.a(x6), .b(x3), .c(x0), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n24_), .c(new_n18_), .O(new_n33_));
  inv1   g18(.a(x4), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n17_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n19_), .c(new_n29_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n33_), .O(z2));
  inv1   g22(.a(new_n29_), .O(new_n38_));
  oai21  g23(.a(x7), .b(x2), .c(new_n35_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n19_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(z3));
  nand3  g26(.a(x3), .b(new_n16_), .c(new_n18_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x0), .O(new_n44_));
  nand2  g29(.a(new_n42_), .b(new_n26_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(z4));
  nand3  g31(.a(x6), .b(new_n17_), .c(new_n16_), .O(new_n47_));
  inv1   g32(.a(new_n47_), .O(new_n48_));
  nor2   g33(.a(x2), .b(x1), .O(new_n49_));
  aoi21  g34(.a(new_n17_), .b(x2), .c(new_n49_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n48_), .c(new_n26_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n44_), .O(z5));
  oai21  g37(.a(x4), .b(x2), .c(new_n17_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n44_), .c(new_n38_), .O(z6));
endmodule


