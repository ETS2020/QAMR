// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  nor2   g01(.a(x3), .b(new_n16_), .O(new_n17_));
  aoi21  g02(.a(new_n17_), .b(x0), .c(x5), .O(new_n18_));
  nand2  g03(.a(x3), .b(x2), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(x0), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  oai21  g08(.a(new_n18_), .b(x1), .c(new_n23_), .O(z0));
  inv1   g09(.a(x5), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n26_), .c(new_n16_), .O(new_n27_));
  nand3  g12(.a(x3), .b(x2), .c(new_n21_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n27_), .c(x0), .O(new_n29_));
  nand4  g14(.a(x3), .b(new_n16_), .c(new_n21_), .d(x0), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n29_), .c(new_n25_), .O(new_n32_));
  nand2  g17(.a(new_n22_), .b(new_n16_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n32_), .O(z1));
  inv1   g19(.a(x4), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x2), .c(new_n26_), .O(new_n36_));
  nand2  g21(.a(x6), .b(new_n16_), .O(new_n37_));
  nor2   g22(.a(x3), .b(x1), .O(new_n38_));
  aoi22  g23(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(x1), .O(new_n39_));
  nand3  g24(.a(x3), .b(new_n16_), .c(x0), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n25_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n21_), .O(new_n42_));
  oai21  g27(.a(new_n39_), .b(x0), .c(new_n42_), .O(z2));
  inv1   g28(.a(x7), .O(new_n44_));
  aoi21  g29(.a(new_n35_), .b(new_n26_), .c(new_n44_), .O(new_n45_));
  oai21  g30(.a(new_n17_), .b(x5), .c(new_n21_), .O(new_n46_));
  oai21  g31(.a(new_n45_), .b(new_n33_), .c(new_n46_), .O(z3));
  nand3  g32(.a(x3), .b(new_n21_), .c(x0), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(new_n49_));
  aoi21  g34(.a(x3), .b(new_n21_), .c(x0), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n49_), .c(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n26_), .b(new_n21_), .O(new_n52_));
  inv1   g37(.a(x0), .O(new_n53_));
  nand2  g38(.a(x3), .b(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  aoi22  g40(.a(new_n55_), .b(x2), .c(x5), .d(new_n21_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n51_), .O(z4));
  xnor2a g42(.a(x3), .b(x2), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n53_), .c(new_n25_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n21_), .O(new_n60_));
  oai21  g45(.a(x3), .b(new_n16_), .c(x1), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n27_), .c(new_n19_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n53_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n60_), .O(z5));
  oai21  g49(.a(x4), .b(x2), .c(new_n26_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n22_), .O(new_n66_));
  nand3  g51(.a(new_n54_), .b(new_n25_), .c(new_n21_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n58_), .c(new_n66_), .O(z6));
endmodule


