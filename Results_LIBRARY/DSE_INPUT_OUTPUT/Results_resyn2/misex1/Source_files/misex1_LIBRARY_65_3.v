// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nor2   g03(.a(x1), .b(new_n16_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(x7), .b(x4), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  aoi21  g07(.a(new_n20_), .b(new_n17_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n18_), .c(x2), .O(new_n25_));
  inv1   g10(.a(x1), .O(new_n26_));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x6), .b(new_n18_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  aoi21  g14(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n25_), .O(new_n31_));
  inv1   g16(.a(new_n21_), .O(new_n32_));
  nor2   g17(.a(new_n18_), .b(x2), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n19_), .c(new_n32_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(z1));
  nand3  g20(.a(x3), .b(new_n27_), .c(x0), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand2  g22(.a(x6), .b(new_n27_), .O(new_n38_));
  nor2   g23(.a(x3), .b(x0), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n26_), .O(new_n42_));
  inv1   g27(.a(new_n17_), .O(new_n43_));
  nand3  g28(.a(new_n27_), .b(x1), .c(new_n16_), .O(new_n44_));
  inv1   g29(.a(new_n44_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(x4), .c(new_n43_), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n42_), .c(new_n32_), .O(z2));
  inv1   g32(.a(x4), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n27_), .c(x1), .d(new_n16_), .O(new_n49_));
  oai21  g34(.a(new_n24_), .b(x0), .c(new_n26_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n22_), .c(new_n49_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n18_), .O(new_n52_));
  inv1   g37(.a(x7), .O(new_n53_));
  nand2  g38(.a(new_n45_), .b(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n52_), .O(z3));
  nor2   g40(.a(x3), .b(new_n27_), .O(new_n56_));
  inv1   g41(.a(new_n36_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n56_), .c(new_n26_), .O(new_n58_));
  nor2   g43(.a(new_n56_), .b(x0), .O(new_n59_));
  nand3  g44(.a(x3), .b(new_n27_), .c(new_n26_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n58_), .c(new_n21_), .O(z4));
  xnor2a g47(.a(x3), .b(x2), .O(new_n63_));
  nand3  g48(.a(x5), .b(new_n18_), .c(x2), .O(new_n64_));
  oai21  g49(.a(new_n63_), .b(new_n16_), .c(new_n64_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n26_), .O(new_n66_));
  nand2  g51(.a(new_n59_), .b(new_n29_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n66_), .c(new_n32_), .O(z5));
  oai21  g53(.a(new_n63_), .b(new_n16_), .c(new_n25_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n26_), .O(new_n70_));
  and2   g55(.a(new_n49_), .b(new_n17_), .O(new_n71_));
  aoi21  g56(.a(new_n71_), .b(new_n70_), .c(new_n32_), .O(z6));
endmodule


