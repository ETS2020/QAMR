// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x7), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x6), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  nor2   g07(.a(x1), .b(new_n22_), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x3), .c(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n17_), .O(z0));
  inv1   g10(.a(new_n17_), .O(new_n26_));
  nor2   g11(.a(new_n18_), .b(x2), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n23_), .c(new_n26_), .O(new_n28_));
  nor2   g13(.a(x5), .b(x3), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x1), .c(x2), .O(new_n30_));
  inv1   g15(.a(x2), .O(new_n31_));
  nand2  g16(.a(x6), .b(new_n18_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n31_), .c(new_n19_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n30_), .c(new_n22_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n28_), .O(z1));
  nand2  g20(.a(x4), .b(new_n31_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n18_), .c(new_n19_), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  inv1   g24(.a(x6), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n31_), .O(new_n41_));
  nand2  g26(.a(new_n18_), .b(new_n19_), .O(new_n42_));
  aoi21  g27(.a(new_n41_), .b(new_n39_), .c(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n37_), .c(new_n22_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n28_), .O(z2));
  nand2  g30(.a(x5), .b(new_n22_), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(new_n17_), .c(x2), .d(new_n19_), .O(new_n47_));
  nor2   g32(.a(x4), .b(x2), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(x7), .c(x1), .d(new_n22_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n18_), .O(new_n51_));
  nand4  g36(.a(new_n20_), .b(new_n16_), .c(new_n40_), .d(new_n31_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(z3));
  aoi21  g38(.a(new_n18_), .b(x2), .c(x0), .O(new_n54_));
  xor2a  g39(.a(x3), .b(x2), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n19_), .O(new_n56_));
  oai21  g41(.a(x3), .b(new_n31_), .c(new_n22_), .O(new_n57_));
  xnor2a g42(.a(x3), .b(x2), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(x1), .c(new_n57_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n56_), .c(new_n17_), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(z4));
  oai22  g46(.a(new_n58_), .b(new_n22_), .c(new_n46_), .d(new_n31_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n19_), .O(new_n63_));
  aoi21  g48(.a(new_n54_), .b(new_n33_), .c(new_n26_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n63_), .O(z5));
  aoi22  g50(.a(new_n55_), .b(x0), .c(new_n29_), .d(x2), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n18_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n20_), .c(new_n26_), .O(new_n68_));
  oai21  g53(.a(new_n66_), .b(x1), .c(new_n68_), .O(z6));
endmodule


