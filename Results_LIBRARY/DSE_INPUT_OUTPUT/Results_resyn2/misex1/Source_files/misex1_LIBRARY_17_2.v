// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:37 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_;
  inv1   g00(.a(x2), .O(new_n16_));
  nor2   g01(.a(x3), .b(x1), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(new_n16_), .c(x0), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(x3), .c(x2), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n18_), .O(z0));
  nor2   g07(.a(new_n16_), .b(x1), .O(new_n23_));
  oai21  g08(.a(x5), .b(x3), .c(new_n23_), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x3), .c(new_n19_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n16_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n24_), .c(x0), .O(z1));
  inv1   g13(.a(x3), .O(new_n29_));
  inv1   g14(.a(x4), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x2), .c(new_n29_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(new_n32_));
  nand2  g17(.a(x6), .b(new_n16_), .O(new_n33_));
  nand2  g18(.a(x5), .b(x2), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n33_), .c(new_n17_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n32_), .c(x0), .O(z2));
  inv1   g21(.a(x0), .O(new_n37_));
  nand2  g22(.a(x5), .b(new_n37_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n23_), .c(new_n29_), .O(new_n39_));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nor2   g25(.a(x2), .b(x0), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(x1), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n39_), .O(z3));
  aoi21  g28(.a(x3), .b(new_n37_), .c(new_n17_), .O(new_n44_));
  oai21  g29(.a(new_n29_), .b(x1), .c(new_n41_), .O(new_n45_));
  oai21  g30(.a(new_n44_), .b(new_n16_), .c(new_n45_), .O(z4));
  aoi21  g31(.a(new_n23_), .b(new_n29_), .c(new_n37_), .O(new_n47_));
  nand2  g32(.a(x3), .b(x2), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n37_), .O(new_n49_));
  aoi21  g34(.a(new_n23_), .b(x5), .c(new_n49_), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n27_), .c(new_n47_), .O(z5));
  oai21  g36(.a(x4), .b(x2), .c(new_n29_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n20_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n39_), .O(z6));
endmodule


