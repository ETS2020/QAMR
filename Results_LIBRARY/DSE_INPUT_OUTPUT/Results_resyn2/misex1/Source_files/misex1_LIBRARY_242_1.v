// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:21 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n48_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nor2   g02(.a(x1), .b(new_n17_), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n16_), .c(x2), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(z0));
  inv1   g05(.a(x5), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n16_), .c(x2), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  inv1   g08(.a(x6), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(x3), .c(new_n23_), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n22_), .c(x1), .O(new_n26_));
  nor2   g11(.a(new_n16_), .b(x2), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n18_), .O(new_n28_));
  oai21  g13(.a(new_n26_), .b(x0), .c(new_n28_), .O(z1));
  aoi21  g14(.a(x6), .b(new_n23_), .c(x3), .O(new_n30_));
  oai21  g15(.a(new_n21_), .b(new_n23_), .c(new_n30_), .O(new_n31_));
  inv1   g16(.a(x1), .O(new_n32_));
  oai21  g17(.a(new_n27_), .b(new_n17_), .c(new_n32_), .O(new_n33_));
  aoi21  g18(.a(new_n31_), .b(new_n17_), .c(new_n33_), .O(z2));
  nand3  g19(.a(new_n16_), .b(x2), .c(new_n32_), .O(new_n35_));
  nand3  g20(.a(new_n22_), .b(new_n32_), .c(new_n17_), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  aoi21  g22(.a(new_n35_), .b(x0), .c(new_n37_), .O(z3));
  nand2  g23(.a(x3), .b(new_n23_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n17_), .O(new_n40_));
  nand2  g25(.a(new_n22_), .b(new_n17_), .O(new_n41_));
  nand2  g26(.a(new_n16_), .b(x2), .O(new_n42_));
  nand2  g27(.a(new_n39_), .b(new_n42_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n41_), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n40_), .c(x1), .O(z4));
  nand2  g30(.a(new_n43_), .b(new_n18_), .O(new_n46_));
  oai21  g31(.a(new_n26_), .b(x0), .c(new_n46_), .O(z5));
  nand3  g32(.a(new_n43_), .b(new_n41_), .c(new_n32_), .O(new_n48_));
  inv1   g33(.a(new_n48_), .O(z6));
endmodule


