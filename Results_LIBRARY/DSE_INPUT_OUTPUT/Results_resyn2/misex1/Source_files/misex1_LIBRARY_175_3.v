// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(x0), .c(x4), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand3  g09(.a(x3), .b(new_n16_), .c(x0), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  inv1   g11(.a(x0), .O(new_n27_));
  nand2  g12(.a(x2), .b(new_n27_), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n26_), .c(new_n25_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n17_), .O(new_n30_));
  nand2  g15(.a(x4), .b(x2), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x3), .c(new_n17_), .O(new_n34_));
  nor2   g19(.a(x2), .b(x0), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(new_n32_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n30_), .O(z1));
  inv1   g22(.a(x4), .O(new_n38_));
  inv1   g23(.a(x5), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(x2), .O(new_n40_));
  nand2  g25(.a(new_n33_), .b(new_n16_), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n40_), .c(new_n21_), .O(new_n42_));
  nand2  g27(.a(new_n31_), .b(x1), .O(new_n43_));
  aoi21  g28(.a(new_n38_), .b(new_n20_), .c(new_n43_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n42_), .c(new_n27_), .O(new_n45_));
  inv1   g30(.a(new_n25_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n17_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z2));
  oai21  g33(.a(x4), .b(x3), .c(x7), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n35_), .c(x1), .O(new_n50_));
  nand2  g35(.a(x5), .b(new_n27_), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(new_n22_), .c(new_n38_), .d(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z3));
  aoi21  g38(.a(new_n20_), .b(x2), .c(x0), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(x4), .c(x2), .O(new_n55_));
  aoi21  g40(.a(x3), .b(new_n17_), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n54_), .b(x4), .c(new_n56_), .O(new_n57_));
  nand2  g42(.a(new_n20_), .b(x2), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n25_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n17_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n57_), .c(new_n55_), .O(z4));
  nand2  g46(.a(x3), .b(new_n16_), .O(new_n62_));
  aoi21  g47(.a(new_n58_), .b(new_n62_), .c(new_n27_), .O(new_n63_));
  nand3  g48(.a(x5), .b(x2), .c(new_n27_), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n17_), .O(new_n66_));
  nand2  g51(.a(new_n54_), .b(new_n34_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(new_n66_), .c(new_n55_), .O(z5));
  xor2a  g53(.a(x3), .b(x2), .O(new_n69_));
  aoi22  g54(.a(new_n69_), .b(x0), .c(new_n26_), .d(x2), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n20_), .O(new_n71_));
  aoi21  g56(.a(new_n71_), .b(new_n18_), .c(new_n32_), .O(new_n72_));
  oai21  g57(.a(new_n70_), .b(x1), .c(new_n72_), .O(z6));
endmodule


