// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x4), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(new_n16_), .c(new_n17_), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand2  g05(.a(new_n17_), .b(new_n20_), .O(new_n21_));
  oai21  g06(.a(new_n20_), .b(x0), .c(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n19_), .c(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  nand3  g10(.a(x3), .b(new_n25_), .c(new_n20_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x4), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x0), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n17_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n20_), .c(x2), .O(new_n30_));
  nor2   g15(.a(x5), .b(x3), .O(new_n31_));
  nand2  g16(.a(x2), .b(new_n20_), .O(new_n32_));
  nor2   g17(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n30_), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n28_), .O(z1));
  nand2  g20(.a(x4), .b(new_n25_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n17_), .c(new_n20_), .O(new_n37_));
  inv1   g22(.a(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  inv1   g24(.a(x6), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n25_), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n39_), .c(new_n21_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n37_), .c(new_n16_), .O(new_n43_));
  nor2   g28(.a(new_n18_), .b(new_n16_), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(x3), .c(new_n25_), .d(new_n20_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z2));
  nand4  g31(.a(new_n38_), .b(new_n17_), .c(x2), .d(new_n20_), .O(new_n47_));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n25_), .c(x1), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n16_), .O(new_n51_));
  nand4  g36(.a(new_n44_), .b(new_n17_), .c(x2), .d(new_n20_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(z3));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  nor2   g39(.a(new_n25_), .b(x0), .O(new_n55_));
  aoi21  g40(.a(new_n54_), .b(new_n44_), .c(new_n55_), .O(new_n56_));
  nand2  g41(.a(new_n17_), .b(x2), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n26_), .c(new_n16_), .O(new_n58_));
  oai21  g43(.a(new_n56_), .b(x1), .c(new_n58_), .O(z4));
  nand2  g44(.a(x5), .b(new_n20_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n17_), .c(new_n25_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n30_), .c(new_n16_), .O(new_n62_));
  nand2  g47(.a(new_n54_), .b(new_n20_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(x4), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(x0), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(z5));
  aoi22  g51(.a(new_n55_), .b(new_n31_), .c(new_n54_), .d(new_n44_), .O(new_n67_));
  oai21  g52(.a(x4), .b(x2), .c(new_n17_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(x1), .c(new_n16_), .O(new_n69_));
  oai21  g54(.a(new_n67_), .b(x1), .c(new_n69_), .O(z6));
endmodule


