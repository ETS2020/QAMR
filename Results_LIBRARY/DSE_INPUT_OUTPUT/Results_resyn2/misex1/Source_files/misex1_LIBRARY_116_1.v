// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand4  g02(.a(new_n17_), .b(x2), .c(new_n16_), .d(x0), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(z0));
  inv1   g04(.a(x0), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  inv1   g06(.a(x6), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n16_), .O(new_n23_));
  nand4  g08(.a(new_n23_), .b(new_n17_), .c(new_n21_), .d(new_n20_), .O(new_n24_));
  nand2  g09(.a(x3), .b(new_n21_), .O(new_n25_));
  nand2  g10(.a(x5), .b(new_n20_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g12(.a(new_n17_), .b(x2), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n20_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n16_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n24_), .O(z1));
  inv1   g16(.a(x5), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x2), .O(new_n33_));
  nand2  g18(.a(new_n22_), .b(new_n21_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n33_), .c(x1), .O(new_n35_));
  nand3  g20(.a(x4), .b(new_n21_), .c(x1), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  nor2   g22(.a(x3), .b(x0), .O(new_n38_));
  oai21  g23(.a(new_n37_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  nand4  g24(.a(x3), .b(new_n21_), .c(new_n16_), .d(x0), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(z2));
  nand2  g26(.a(x7), .b(x4), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n21_), .c(x1), .d(new_n20_), .O(new_n43_));
  nand3  g28(.a(new_n26_), .b(x2), .c(new_n16_), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n43_), .c(x3), .O(z3));
  and2   g30(.a(new_n25_), .b(new_n28_), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(x1), .c(new_n29_), .O(z4));
  aoi21  g32(.a(new_n25_), .b(new_n28_), .c(new_n20_), .O(new_n48_));
  nand3  g33(.a(x5), .b(new_n17_), .c(x2), .O(new_n49_));
  inv1   g34(.a(new_n49_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n48_), .c(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n24_), .O(z5));
  nand3  g37(.a(new_n32_), .b(x2), .c(new_n20_), .O(new_n53_));
  inv1   g38(.a(new_n53_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n48_), .c(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n21_), .b(x1), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(x4), .c(new_n17_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n20_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n55_), .O(z6));
endmodule


