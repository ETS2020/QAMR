// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:27 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(x4), .O(new_n23_));
  inv1   g08(.a(x6), .O(new_n24_));
  nor2   g09(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  inv1   g10(.a(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n22_), .O(z0));
  nor2   g12(.a(new_n19_), .b(x2), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n18_), .c(x0), .O(new_n29_));
  nor2   g14(.a(x5), .b(x3), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x1), .c(x2), .O(new_n31_));
  nor2   g16(.a(x2), .b(x1), .O(new_n32_));
  oai21  g17(.a(new_n24_), .b(x3), .c(new_n32_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n31_), .c(new_n16_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n29_), .c(new_n25_), .O(z1));
  inv1   g20(.a(x2), .O(new_n36_));
  nand2  g21(.a(x4), .b(new_n36_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n19_), .c(new_n18_), .O(new_n38_));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n19_), .c(new_n18_), .O(new_n40_));
  aoi21  g25(.a(x6), .b(new_n36_), .c(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n38_), .c(new_n16_), .O(new_n42_));
  and2   g27(.a(new_n29_), .b(new_n26_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n42_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n36_), .b(x1), .c(new_n16_), .O(new_n46_));
  inv1   g31(.a(new_n46_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n16_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n19_), .c(x2), .d(new_n18_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n48_), .c(new_n26_), .O(z3));
  nand2  g36(.a(x3), .b(new_n36_), .O(new_n52_));
  nand2  g37(.a(new_n19_), .b(x2), .O(new_n53_));
  oai21  g38(.a(new_n52_), .b(new_n16_), .c(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n18_), .O(new_n55_));
  aoi21  g40(.a(new_n19_), .b(x2), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n52_), .b(x1), .c(new_n56_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n55_), .c(new_n26_), .O(z4));
  xnor2a g43(.a(x3), .b(x2), .O(new_n59_));
  oai22  g44(.a(new_n59_), .b(new_n16_), .c(new_n49_), .d(new_n36_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n18_), .O(new_n61_));
  nor3   g46(.a(x3), .b(x2), .c(x0), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(x4), .c(x6), .O(new_n63_));
  oai21  g48(.a(x2), .b(x1), .c(new_n56_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(z5));
  inv1   g50(.a(x5), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n19_), .c(x2), .O(new_n67_));
  oai21  g52(.a(new_n59_), .b(new_n16_), .c(new_n67_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n18_), .O(new_n69_));
  inv1   g54(.a(new_n17_), .O(new_n70_));
  aoi21  g55(.a(new_n47_), .b(new_n23_), .c(new_n70_), .O(new_n71_));
  aoi21  g56(.a(new_n71_), .b(new_n69_), .c(new_n25_), .O(z6));
endmodule


