// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x3), .O(new_n19_));
  inv1   g04(.a(x4), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n17_), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(x0), .c(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand3  g10(.a(x3), .b(new_n16_), .c(x0), .O(new_n26_));
  nor2   g11(.a(x5), .b(x3), .O(new_n27_));
  inv1   g12(.a(x0), .O(new_n28_));
  nand2  g13(.a(x2), .b(new_n28_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n27_), .c(new_n26_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n17_), .O(new_n31_));
  nor2   g16(.a(x4), .b(new_n16_), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x3), .c(new_n17_), .O(new_n34_));
  nor2   g19(.a(x2), .b(x0), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(new_n32_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n31_), .O(z1));
  inv1   g22(.a(x5), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(x4), .c(x2), .O(new_n39_));
  nand2  g24(.a(new_n33_), .b(new_n16_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n39_), .c(new_n22_), .O(new_n41_));
  aoi21  g26(.a(x3), .b(new_n16_), .c(x4), .O(new_n42_));
  oai21  g27(.a(x3), .b(new_n16_), .c(x1), .O(new_n43_));
  nor2   g28(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n41_), .c(new_n28_), .O(new_n45_));
  inv1   g30(.a(new_n26_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n17_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z2));
  nand2  g33(.a(x5), .b(new_n28_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n23_), .c(x4), .d(x2), .O(new_n50_));
  oai21  g35(.a(x4), .b(x3), .c(x7), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n35_), .c(x1), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z3));
  nand2  g38(.a(new_n21_), .b(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n26_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n17_), .O(new_n56_));
  oai21  g41(.a(new_n21_), .b(x1), .c(new_n35_), .O(new_n57_));
  oai21  g42(.a(new_n21_), .b(x0), .c(x4), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(x2), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(z4));
  nand2  g45(.a(new_n35_), .b(new_n34_), .O(new_n61_));
  nand2  g46(.a(x3), .b(new_n16_), .O(new_n62_));
  aoi21  g47(.a(new_n54_), .b(new_n62_), .c(new_n28_), .O(new_n63_));
  nand3  g48(.a(x5), .b(x2), .c(new_n28_), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n17_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(new_n59_), .c(new_n61_), .O(z5));
  nand2  g52(.a(new_n27_), .b(x2), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n63_), .c(new_n17_), .O(new_n70_));
  nand2  g55(.a(x4), .b(new_n21_), .O(new_n71_));
  aoi21  g56(.a(new_n71_), .b(new_n18_), .c(new_n32_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n70_), .O(z6));
endmodule


