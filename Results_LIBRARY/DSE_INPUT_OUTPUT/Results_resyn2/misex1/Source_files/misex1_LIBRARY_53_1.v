// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x6), .O(new_n16_));
  nand2  g01(.a(x7), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(new_n18_), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  oai21  g07(.a(x1), .b(new_n22_), .c(new_n18_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n21_), .c(new_n17_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nand4  g11(.a(x3), .b(new_n26_), .c(new_n19_), .d(x0), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n18_), .c(x2), .O(new_n29_));
  nand2  g14(.a(x2), .b(x1), .O(new_n30_));
  nand2  g15(.a(new_n26_), .b(new_n19_), .O(new_n31_));
  nand4  g16(.a(new_n31_), .b(new_n30_), .c(new_n29_), .d(new_n22_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n27_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n17_), .O(new_n34_));
  nor2   g19(.a(x2), .b(x0), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(x6), .c(new_n18_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(z1));
  inv1   g22(.a(new_n17_), .O(new_n38_));
  inv1   g23(.a(new_n27_), .O(new_n39_));
  aoi21  g24(.a(x4), .b(new_n26_), .c(x3), .O(new_n40_));
  nand4  g25(.a(new_n28_), .b(new_n18_), .c(x2), .d(new_n19_), .O(new_n41_));
  oai21  g26(.a(new_n40_), .b(new_n19_), .c(new_n41_), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n22_), .c(new_n39_), .O(new_n43_));
  nor2   g28(.a(x7), .b(x6), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n35_), .c(new_n18_), .d(new_n19_), .O(new_n45_));
  oai21  g30(.a(new_n43_), .b(new_n38_), .c(new_n45_), .O(z2));
  nand2  g31(.a(x5), .b(new_n22_), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n17_), .c(x2), .d(new_n19_), .O(new_n48_));
  nor2   g33(.a(x4), .b(x2), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n20_), .c(x6), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n18_), .O(new_n52_));
  nor2   g37(.a(x7), .b(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n20_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n52_), .O(z3));
  nor2   g40(.a(new_n18_), .b(x2), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(x0), .O(new_n57_));
  oai21  g42(.a(x3), .b(new_n26_), .c(new_n57_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n19_), .O(new_n59_));
  aoi21  g44(.a(new_n18_), .b(x2), .c(x0), .O(new_n60_));
  nand2  g45(.a(new_n56_), .b(new_n19_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n60_), .c(new_n38_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n59_), .O(z4));
  xor2a  g48(.a(x3), .b(x2), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(x0), .O(new_n65_));
  nand3  g50(.a(x5), .b(x2), .c(new_n22_), .O(new_n66_));
  aoi21  g51(.a(new_n66_), .b(new_n65_), .c(x1), .O(new_n67_));
  and2   g52(.a(new_n60_), .b(new_n31_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n67_), .c(new_n17_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n36_), .O(z5));
  and2   g55(.a(new_n65_), .b(new_n29_), .O(new_n71_));
  oai21  g56(.a(x4), .b(x2), .c(new_n18_), .O(new_n72_));
  aoi21  g57(.a(new_n72_), .b(new_n20_), .c(new_n38_), .O(new_n73_));
  oai21  g58(.a(new_n71_), .b(x1), .c(new_n73_), .O(z6));
endmodule


