// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x5), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x4), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nor2   g05(.a(new_n20_), .b(x0), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(new_n19_), .O(new_n22_));
  nand2  g07(.a(new_n20_), .b(x0), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(x3), .c(x2), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(new_n22_), .c(new_n18_), .O(z0));
  inv1   g11(.a(x2), .O(new_n27_));
  nand2  g12(.a(x3), .b(new_n27_), .O(new_n28_));
  inv1   g13(.a(new_n28_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n24_), .c(new_n17_), .O(new_n30_));
  inv1   g15(.a(x0), .O(new_n31_));
  nor2   g16(.a(x5), .b(x3), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x1), .c(x2), .O(new_n33_));
  nand2  g18(.a(x6), .b(new_n19_), .O(new_n34_));
  nor2   g19(.a(x2), .b(x1), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n33_), .c(new_n31_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n30_), .O(z1));
  nand2  g23(.a(x4), .b(new_n27_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n19_), .c(new_n20_), .O(new_n40_));
  nand2  g25(.a(x5), .b(x2), .O(new_n41_));
  nor2   g26(.a(x3), .b(x1), .O(new_n42_));
  nand2  g27(.a(x6), .b(new_n27_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(new_n44_));
  inv1   g29(.a(new_n44_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n40_), .c(new_n31_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n30_), .O(z2));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n21_), .c(new_n27_), .O(new_n49_));
  nand2  g34(.a(x5), .b(new_n31_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n42_), .c(x2), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n49_), .c(new_n18_), .O(z3));
  nand2  g37(.a(new_n19_), .b(x2), .O(new_n53_));
  oai21  g38(.a(new_n28_), .b(new_n31_), .c(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n20_), .O(new_n55_));
  aoi21  g40(.a(new_n35_), .b(x3), .c(x0), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n53_), .c(new_n17_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n55_), .O(z4));
  nand2  g43(.a(new_n43_), .b(new_n19_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  aoi21  g45(.a(new_n53_), .b(new_n28_), .c(new_n23_), .O(new_n61_));
  nand3  g46(.a(new_n27_), .b(x1), .c(new_n31_), .O(new_n62_));
  nand3  g47(.a(new_n42_), .b(x5), .c(x2), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nor2   g49(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  aoi21  g50(.a(new_n65_), .b(new_n60_), .c(new_n17_), .O(z5));
  aoi21  g51(.a(x4), .b(x0), .c(new_n16_), .O(new_n67_));
  oai21  g52(.a(x4), .b(x2), .c(new_n19_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n21_), .c(new_n18_), .O(new_n69_));
  oai21  g54(.a(new_n67_), .b(new_n55_), .c(new_n69_), .O(z6));
endmodule


