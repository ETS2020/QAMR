// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x1), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(x1), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n17_), .c(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(z0));
  nor2   g07(.a(x5), .b(x3), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(x1), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n16_), .O(new_n28_));
  inv1   g13(.a(x7), .O(new_n29_));
  nor2   g14(.a(new_n29_), .b(x2), .O(new_n30_));
  aoi22  g15(.a(new_n30_), .b(new_n28_), .c(new_n26_), .d(new_n24_), .O(new_n31_));
  nand2  g16(.a(x3), .b(new_n25_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  nand4  g18(.a(new_n33_), .b(x7), .c(new_n16_), .d(x0), .O(new_n34_));
  oai21  g19(.a(new_n31_), .b(x0), .c(new_n34_), .O(z1));
  inv1   g20(.a(x5), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(x2), .O(new_n37_));
  nand3  g22(.a(x7), .b(new_n27_), .c(new_n25_), .O(new_n38_));
  nand2  g23(.a(new_n19_), .b(new_n16_), .O(new_n39_));
  aoi21  g24(.a(new_n38_), .b(new_n37_), .c(new_n39_), .O(new_n40_));
  nand2  g25(.a(x4), .b(new_n25_), .O(new_n41_));
  oai21  g26(.a(x7), .b(x2), .c(x1), .O(new_n42_));
  aoi21  g27(.a(new_n41_), .b(new_n19_), .c(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n40_), .c(new_n18_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n34_), .O(z2));
  nor2   g30(.a(x7), .b(x2), .O(new_n46_));
  inv1   g31(.a(new_n46_), .O(new_n47_));
  nor2   g32(.a(new_n16_), .b(x0), .O(new_n48_));
  nand2  g33(.a(x5), .b(new_n18_), .O(new_n49_));
  nor2   g34(.a(x4), .b(x2), .O(new_n50_));
  aoi22  g35(.a(new_n50_), .b(new_n48_), .c(new_n49_), .d(new_n26_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(x3), .c(new_n47_), .O(z3));
  nand2  g37(.a(new_n19_), .b(x2), .O(new_n53_));
  oai21  g38(.a(new_n32_), .b(new_n18_), .c(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n16_), .O(new_n55_));
  aoi21  g40(.a(new_n19_), .b(x2), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n32_), .b(x1), .c(new_n56_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(z4));
  aoi21  g43(.a(new_n53_), .b(new_n32_), .c(new_n18_), .O(new_n59_));
  nand3  g44(.a(x5), .b(x2), .c(new_n18_), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n16_), .O(new_n62_));
  nand2  g47(.a(x6), .b(new_n19_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n25_), .c(new_n16_), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n56_), .c(new_n46_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(z5));
  xor2a  g51(.a(x3), .b(x2), .O(new_n67_));
  aoi22  g52(.a(new_n67_), .b(x0), .c(new_n23_), .d(x2), .O(new_n68_));
  oai21  g53(.a(x4), .b(x2), .c(new_n19_), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(new_n48_), .c(new_n46_), .O(new_n70_));
  oai21  g55(.a(new_n68_), .b(x1), .c(new_n70_), .O(z6));
endmodule


