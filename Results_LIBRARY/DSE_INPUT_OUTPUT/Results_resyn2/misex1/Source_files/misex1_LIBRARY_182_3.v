// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n51_, new_n52_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nand2  g02(.a(x3), .b(x1), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(new_n16_), .c(new_n17_), .O(z0));
  inv1   g04(.a(x1), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  inv1   g06(.a(x5), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(x2), .c(new_n20_), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x3), .c(new_n20_), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n17_), .c(new_n16_), .O(new_n27_));
  oai21  g12(.a(new_n21_), .b(x1), .c(new_n17_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x0), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n27_), .c(new_n24_), .O(z1));
  aoi21  g15(.a(x4), .b(new_n17_), .c(x3), .O(new_n31_));
  nand4  g16(.a(new_n25_), .b(new_n21_), .c(new_n17_), .d(new_n20_), .O(new_n32_));
  oai21  g17(.a(new_n31_), .b(new_n20_), .c(new_n32_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(new_n34_));
  nand4  g19(.a(new_n22_), .b(new_n21_), .c(x2), .d(new_n20_), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  aoi21  g21(.a(new_n28_), .b(x0), .c(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n34_), .O(z2));
  oai21  g23(.a(x4), .b(x3), .c(x7), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n17_), .c(x1), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n35_), .c(x0), .O(z3));
  oai21  g26(.a(new_n21_), .b(new_n17_), .c(new_n28_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  oai21  g28(.a(new_n21_), .b(x2), .c(x0), .O(new_n44_));
  aoi21  g29(.a(new_n17_), .b(new_n16_), .c(x1), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n43_), .O(z4));
  oai21  g32(.a(new_n22_), .b(x1), .c(new_n21_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(x2), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n29_), .c(new_n27_), .O(z5));
  oai21  g35(.a(x4), .b(x2), .c(new_n21_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(x1), .c(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n37_), .O(z6));
endmodule


