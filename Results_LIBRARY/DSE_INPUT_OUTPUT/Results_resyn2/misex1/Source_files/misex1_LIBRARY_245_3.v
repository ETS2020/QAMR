// Benchmark "FAU" written by ABC on Tue Aug 11 19:26:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x2), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x1), .O(new_n20_));
  nand2  g05(.a(x3), .b(x2), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n16_), .c(new_n20_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  nand3  g08(.a(x6), .b(new_n17_), .c(new_n23_), .O(new_n24_));
  nor2   g09(.a(x5), .b(x3), .O(new_n25_));
  inv1   g10(.a(x1), .O(new_n26_));
  nand2  g11(.a(x2), .b(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(new_n24_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n16_), .O(new_n29_));
  nand2  g14(.a(x3), .b(new_n23_), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  aoi21  g16(.a(x2), .b(new_n16_), .c(new_n26_), .O(new_n32_));
  aoi21  g17(.a(new_n31_), .b(x0), .c(new_n32_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n29_), .O(z1));
  nand2  g19(.a(x4), .b(new_n23_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n17_), .c(new_n26_), .O(new_n36_));
  nand2  g21(.a(x5), .b(x2), .O(new_n37_));
  nand2  g22(.a(x6), .b(new_n23_), .O(new_n38_));
  nor2   g23(.a(x3), .b(x1), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n36_), .c(new_n16_), .O(new_n42_));
  nand3  g27(.a(new_n31_), .b(new_n26_), .c(x0), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n42_), .O(z2));
  inv1   g29(.a(x5), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n26_), .c(x0), .O(new_n46_));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n23_), .c(x1), .O(new_n48_));
  oai21  g33(.a(new_n46_), .b(new_n20_), .c(new_n48_), .O(z3));
  aoi21  g34(.a(new_n30_), .b(new_n18_), .c(new_n16_), .O(new_n50_));
  inv1   g35(.a(new_n50_), .O(new_n51_));
  nand3  g36(.a(new_n30_), .b(new_n18_), .c(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n19_), .b(new_n26_), .O(new_n53_));
  nand2  g38(.a(new_n18_), .b(x1), .O(new_n54_));
  nand4  g39(.a(new_n54_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(z4));
  nand3  g40(.a(x5), .b(x2), .c(new_n16_), .O(new_n56_));
  inv1   g41(.a(new_n56_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n50_), .c(new_n26_), .O(new_n58_));
  nand3  g43(.a(new_n54_), .b(new_n24_), .c(new_n21_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n16_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n58_), .O(z5));
  nand2  g46(.a(new_n25_), .b(x2), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n50_), .c(new_n26_), .O(new_n64_));
  oai21  g49(.a(x4), .b(x2), .c(new_n17_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(x1), .c(new_n16_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z6));
endmodule


