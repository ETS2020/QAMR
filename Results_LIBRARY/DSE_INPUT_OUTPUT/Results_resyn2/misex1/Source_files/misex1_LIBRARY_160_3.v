// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nor2   g03(.a(x1), .b(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(x4), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x1), .c(new_n18_), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x3), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n20_), .c(new_n16_), .O(z0));
  nor2   g10(.a(x4), .b(x2), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x1), .O(new_n27_));
  inv1   g12(.a(x1), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x3), .c(new_n16_), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n17_), .c(x2), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n27_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n18_), .O(new_n35_));
  nand2  g20(.a(x3), .b(new_n16_), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n19_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n35_), .O(z1));
  nand2  g24(.a(x5), .b(x2), .O(new_n40_));
  aoi21  g25(.a(x6), .b(new_n16_), .c(x3), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n40_), .c(x0), .O(new_n42_));
  oai21  g27(.a(new_n37_), .b(new_n18_), .c(new_n28_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n42_), .c(new_n24_), .O(z2));
  nand2  g29(.a(x7), .b(x3), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n26_), .c(x1), .d(new_n18_), .O(new_n46_));
  nor2   g31(.a(x3), .b(new_n16_), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n18_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n47_), .c(new_n28_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n46_), .O(z3));
  nand2  g35(.a(new_n19_), .b(x3), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n22_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n16_), .O(new_n53_));
  nand4  g38(.a(new_n21_), .b(x3), .c(x2), .d(new_n18_), .O(new_n54_));
  nand2  g39(.a(new_n17_), .b(x2), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(x0), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n36_), .c(new_n28_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n54_), .c(new_n53_), .O(z4));
  aoi21  g43(.a(new_n36_), .b(new_n55_), .c(new_n18_), .O(new_n59_));
  nor2   g44(.a(new_n40_), .b(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n59_), .c(new_n28_), .O(new_n61_));
  nor2   g46(.a(new_n47_), .b(x0), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n30_), .O(new_n63_));
  oai21  g48(.a(new_n62_), .b(x4), .c(x1), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(z5));
  inv1   g50(.a(new_n32_), .O(new_n66_));
  oai21  g51(.a(new_n59_), .b(new_n66_), .c(new_n28_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n64_), .O(z6));
endmodule


