// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nor2   g02(.a(x1), .b(new_n17_), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n16_), .O(z0));
  oai21  g05(.a(new_n18_), .b(x2), .c(x3), .O(new_n21_));
  inv1   g06(.a(x1), .O(new_n22_));
  inv1   g07(.a(x3), .O(new_n23_));
  nand2  g08(.a(x6), .b(new_n23_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n22_), .c(x2), .O(new_n25_));
  nand3  g10(.a(x5), .b(x2), .c(new_n22_), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(new_n17_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n21_), .O(z1));
  inv1   g14(.a(x6), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n23_), .c(new_n16_), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x2), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n31_), .c(new_n22_), .O(new_n34_));
  nand2  g19(.a(x4), .b(new_n16_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n23_), .c(x1), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n34_), .c(new_n17_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n21_), .O(z2));
  oai21  g23(.a(x4), .b(x3), .c(x7), .O(new_n39_));
  nand3  g24(.a(new_n16_), .b(x1), .c(new_n17_), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  aoi21  g27(.a(x5), .b(new_n17_), .c(x1), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(x3), .c(x2), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(z3));
  oai21  g30(.a(new_n23_), .b(new_n17_), .c(new_n16_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n22_), .O(new_n47_));
  nand2  g32(.a(x3), .b(x2), .O(new_n48_));
  nand2  g33(.a(x3), .b(new_n22_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n16_), .c(new_n17_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n48_), .c(new_n47_), .O(z4));
  nand2  g36(.a(new_n23_), .b(new_n16_), .O(new_n52_));
  nand3  g37(.a(new_n48_), .b(new_n52_), .c(x0), .O(new_n53_));
  nand3  g38(.a(x5), .b(new_n23_), .c(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n22_), .O(new_n56_));
  nand2  g41(.a(new_n25_), .b(new_n17_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(z5));
  nand2  g43(.a(new_n53_), .b(new_n33_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n22_), .O(new_n60_));
  nand2  g45(.a(x4), .b(new_n23_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n41_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n60_), .c(new_n48_), .O(z6));
endmodule


