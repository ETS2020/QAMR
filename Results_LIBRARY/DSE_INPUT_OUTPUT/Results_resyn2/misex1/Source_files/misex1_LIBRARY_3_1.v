// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:31 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nor2   g02(.a(x3), .b(new_n17_), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n16_), .c(x0), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(z0));
  inv1   g05(.a(x0), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  inv1   g07(.a(x6), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n16_), .c(x2), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n22_), .c(new_n21_), .O(new_n25_));
  oai21  g10(.a(x3), .b(new_n17_), .c(new_n21_), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  oai22  g12(.a(new_n27_), .b(x0), .c(new_n22_), .d(x2), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n26_), .c(new_n16_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n25_), .O(z1));
  nand3  g15(.a(x4), .b(new_n17_), .c(x1), .O(new_n31_));
  nand2  g16(.a(x5), .b(x2), .O(new_n32_));
  nand2  g17(.a(x6), .b(new_n17_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n32_), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n21_), .O(new_n36_));
  oai21  g21(.a(x2), .b(x1), .c(x0), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x3), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(z2));
  nand2  g24(.a(new_n18_), .b(new_n16_), .O(new_n40_));
  nor2   g25(.a(new_n27_), .b(x0), .O(new_n41_));
  nor2   g26(.a(x2), .b(new_n16_), .O(new_n42_));
  nand2  g27(.a(x7), .b(x4), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n42_), .c(x3), .O(new_n44_));
  oai22  g29(.a(new_n44_), .b(x0), .c(new_n41_), .d(new_n40_), .O(z3));
  xor2a  g30(.a(x3), .b(x2), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n16_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n26_), .O(z4));
  nand2  g33(.a(new_n22_), .b(new_n21_), .O(new_n49_));
  nor2   g34(.a(new_n32_), .b(x0), .O(new_n50_));
  aoi21  g35(.a(new_n46_), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  oai21  g36(.a(new_n24_), .b(x3), .c(new_n21_), .O(new_n52_));
  oai21  g37(.a(new_n51_), .b(x1), .c(new_n52_), .O(z5));
  nor2   g38(.a(new_n22_), .b(x2), .O(new_n54_));
  nand2  g39(.a(x3), .b(x0), .O(new_n55_));
  aoi21  g40(.a(x5), .b(new_n21_), .c(new_n17_), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  inv1   g42(.a(x4), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n17_), .c(x1), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n22_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n21_), .O(new_n61_));
  oai21  g46(.a(new_n57_), .b(x1), .c(new_n61_), .O(z6));
endmodule


