// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x6), .O(new_n16_));
  nor2   g01(.a(new_n16_), .b(x4), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x0), .O(new_n20_));
  nor2   g05(.a(x3), .b(x1), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  oai21  g07(.a(x3), .b(x0), .c(x2), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n22_), .c(new_n18_), .O(z0));
  inv1   g09(.a(x3), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(x2), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n19_), .c(x0), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n25_), .c(x2), .O(new_n29_));
  nand2  g14(.a(x2), .b(x1), .O(new_n30_));
  nor2   g15(.a(x2), .b(x1), .O(new_n31_));
  nor2   g16(.a(new_n31_), .b(x0), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n30_), .c(new_n29_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n27_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n18_), .O(new_n35_));
  inv1   g20(.a(x0), .O(new_n36_));
  inv1   g21(.a(x2), .O(new_n37_));
  nand2  g22(.a(x4), .b(new_n37_), .O(new_n38_));
  inv1   g23(.a(new_n38_), .O(new_n39_));
  nand4  g24(.a(new_n39_), .b(x6), .c(new_n25_), .d(new_n36_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n35_), .O(z1));
  aoi21  g26(.a(new_n38_), .b(new_n25_), .c(new_n19_), .O(new_n42_));
  inv1   g27(.a(new_n21_), .O(new_n43_));
  nand2  g28(.a(new_n28_), .b(x2), .O(new_n44_));
  nand2  g29(.a(new_n16_), .b(new_n37_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(new_n42_), .c(new_n36_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n27_), .c(new_n18_), .O(z2));
  oai21  g33(.a(x4), .b(x3), .c(x7), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n20_), .c(new_n37_), .O(new_n50_));
  nand2  g35(.a(x5), .b(new_n36_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n21_), .c(x2), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n50_), .c(new_n18_), .O(z3));
  nand2  g38(.a(new_n25_), .b(x2), .O(new_n54_));
  nand2  g39(.a(new_n26_), .b(x0), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n19_), .O(new_n57_));
  nand2  g42(.a(new_n26_), .b(new_n19_), .O(new_n58_));
  aoi21  g43(.a(new_n25_), .b(x2), .c(x0), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(new_n17_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n57_), .O(z4));
  xnor2a g46(.a(x3), .b(x2), .O(new_n62_));
  oai22  g47(.a(new_n62_), .b(new_n36_), .c(new_n51_), .d(new_n37_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n19_), .O(new_n64_));
  oai21  g49(.a(new_n16_), .b(x3), .c(new_n31_), .O(new_n65_));
  aoi21  g50(.a(new_n65_), .b(new_n59_), .c(new_n17_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z5));
  nand3  g52(.a(x3), .b(x1), .c(new_n36_), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n62_), .b(new_n36_), .c(new_n29_), .O(new_n70_));
  aoi21  g55(.a(new_n70_), .b(new_n19_), .c(new_n69_), .O(new_n71_));
  inv1   g56(.a(x4), .O(new_n72_));
  nand4  g57(.a(new_n20_), .b(new_n16_), .c(new_n72_), .d(new_n37_), .O(new_n73_));
  oai21  g58(.a(new_n71_), .b(new_n17_), .c(new_n73_), .O(z6));
endmodule


