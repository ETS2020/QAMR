// Benchmark "FAU" written by ABC on Tue Aug 11 19:25:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x6), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(x1), .c(new_n17_), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  inv1   g06(.a(x1), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x0), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n16_), .c(new_n21_), .O(new_n24_));
  oai21  g09(.a(new_n20_), .b(new_n16_), .c(new_n24_), .O(new_n25_));
  inv1   g10(.a(new_n25_), .O(z0));
  nand2  g11(.a(x3), .b(new_n21_), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n16_), .c(x2), .O(new_n29_));
  oai21  g14(.a(new_n18_), .b(x3), .c(new_n21_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n29_), .c(x1), .O(new_n31_));
  nand2  g16(.a(new_n18_), .b(new_n21_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x1), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n17_), .O(new_n34_));
  oai22  g19(.a(new_n34_), .b(new_n31_), .c(new_n27_), .d(new_n23_), .O(z1));
  nand2  g20(.a(x1), .b(new_n17_), .O(new_n36_));
  oai21  g21(.a(new_n23_), .b(x2), .c(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x3), .O(new_n38_));
  nor2   g23(.a(x2), .b(x0), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x4), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n18_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x1), .O(new_n42_));
  oai21  g27(.a(x5), .b(new_n21_), .c(new_n32_), .O(new_n43_));
  nand4  g28(.a(new_n43_), .b(new_n16_), .c(new_n22_), .d(new_n17_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n42_), .c(new_n38_), .O(z2));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n39_), .c(x6), .O(new_n47_));
  nand2  g32(.a(x5), .b(new_n17_), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n16_), .c(x2), .d(new_n22_), .O(new_n49_));
  oai21  g34(.a(new_n47_), .b(new_n22_), .c(new_n49_), .O(z3));
  nand2  g35(.a(new_n27_), .b(new_n17_), .O(new_n51_));
  xor2a  g36(.a(x3), .b(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(x0), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n51_), .c(new_n29_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n22_), .O(new_n55_));
  oai21  g40(.a(x3), .b(new_n21_), .c(new_n20_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(z4));
  nand4  g42(.a(new_n27_), .b(new_n32_), .c(new_n29_), .d(new_n17_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n53_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n22_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n56_), .O(z5));
  nand2  g46(.a(new_n53_), .b(new_n29_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n22_), .O(new_n63_));
  nor2   g48(.a(x4), .b(x2), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(x3), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n18_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(x1), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n63_), .O(z6));
endmodule


