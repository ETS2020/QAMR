// Benchmark "FAU" written by ABC on Tue Aug 11 19:24:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  nand2  g07(.a(x4), .b(new_n19_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(z0));
  nor2   g09(.a(x5), .b(x3), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x1), .c(x2), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  nor2   g12(.a(x2), .b(x1), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(x3), .c(new_n28_), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n26_), .c(new_n16_), .O(new_n30_));
  nor2   g15(.a(x1), .b(new_n16_), .O(new_n31_));
  inv1   g16(.a(x2), .O(new_n32_));
  nand2  g17(.a(x3), .b(new_n32_), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n30_), .c(new_n23_), .O(z1));
  nand3  g21(.a(x3), .b(new_n32_), .c(x0), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  inv1   g23(.a(x5), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x2), .O(new_n40_));
  nand2  g25(.a(new_n27_), .b(new_n32_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nor3   g27(.a(x4), .b(x3), .c(x0), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n42_), .c(new_n38_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(x1), .c(new_n17_), .O(z2));
  nand2  g30(.a(x5), .b(new_n16_), .O(new_n46_));
  nor2   g31(.a(new_n32_), .b(x1), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n46_), .c(x4), .O(new_n48_));
  nor2   g33(.a(new_n18_), .b(x0), .O(new_n49_));
  nand2  g34(.a(x7), .b(x3), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n23_), .c(new_n49_), .d(new_n32_), .O(new_n51_));
  oai21  g36(.a(new_n48_), .b(x3), .c(new_n51_), .O(z3));
  inv1   g37(.a(x4), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n19_), .c(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n37_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n18_), .O(new_n56_));
  aoi21  g41(.a(new_n19_), .b(x2), .c(x0), .O(new_n57_));
  nand2  g42(.a(new_n28_), .b(x3), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(new_n23_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n56_), .O(z4));
  nand2  g45(.a(new_n19_), .b(x2), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n33_), .c(new_n16_), .O(new_n62_));
  nand3  g47(.a(x5), .b(new_n19_), .c(x2), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n62_), .c(new_n18_), .O(new_n65_));
  inv1   g50(.a(new_n23_), .O(new_n66_));
  aoi21  g51(.a(new_n57_), .b(new_n29_), .c(new_n66_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z5));
  xor2a  g53(.a(x3), .b(x2), .O(new_n69_));
  aoi22  g54(.a(new_n69_), .b(x0), .c(new_n25_), .d(x2), .O(new_n70_));
  aoi21  g55(.a(new_n61_), .b(new_n49_), .c(new_n66_), .O(new_n71_));
  oai21  g56(.a(new_n70_), .b(x1), .c(new_n71_), .O(z6));
endmodule


