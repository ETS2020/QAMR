// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n131_,
    new_n132_, new_n133_, new_n136_;
  nand2  g00(.a(x16), .b(x13), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  inv1   g15(.a(new_n44_), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n50_), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(new_n61_), .c(x12), .O(new_n64_));
  nor2   g21(.a(x17), .b(x02), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n51_), .c(new_n47_), .d(x18), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(new_n64_), .c(new_n59_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n50_), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n60_), .c(new_n44_), .O(z04));
  nand2  g28(.a(x10), .b(x02), .O(new_n72_));
  nand3  g29(.a(x09), .b(x01), .c(x00), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n72_), .c(new_n44_), .O(z05));
  oai21  g31(.a(new_n62_), .b(new_n60_), .c(new_n44_), .O(z06));
  nand2  g32(.a(new_n63_), .b(new_n69_), .O(new_n76_));
  nand3  g33(.a(new_n44_), .b(x02), .c(x00), .O(new_n77_));
  aoi21  g34(.a(new_n76_), .b(x01), .c(new_n77_), .O(z07));
  nor2   g35(.a(new_n45_), .b(x08), .O(new_n79_));
  inv1   g36(.a(x05), .O(new_n80_));
  nor2   g37(.a(x07), .b(x06), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(new_n80_), .c(x04), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nand3  g40(.a(new_n51_), .b(new_n83_), .c(x02), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n79_), .c(x19), .d(new_n46_), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(new_n44_), .O(z08));
  inv1   g44(.a(x20), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n47_), .c(x18), .d(x01), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(new_n44_), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  nor2   g49(.a(new_n69_), .b(x11), .O(new_n93_));
  inv1   g50(.a(x01), .O(new_n94_));
  inv1   g51(.a(x13), .O(new_n95_));
  inv1   g52(.a(x14), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(x02), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nor2   g55(.a(new_n88_), .b(x16), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(new_n93_), .d(new_n92_), .O(new_n100_));
  inv1   g57(.a(x00), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  aoi21  g61(.a(new_n100_), .b(new_n91_), .c(new_n104_), .O(z09));
  nand2  g62(.a(x22), .b(x21), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n89_), .O(new_n107_));
  nand4  g64(.a(new_n103_), .b(x20), .c(x16), .d(new_n96_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand2  g66(.a(new_n102_), .b(x15), .O(new_n110_));
  nand4  g67(.a(x12), .b(new_n68_), .c(x02), .d(new_n94_), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n109_), .c(new_n107_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g71(.a(new_n103_), .b(new_n101_), .O(new_n115_));
  nand3  g72(.a(new_n90_), .b(new_n44_), .c(x21), .O(new_n116_));
  inv1   g73(.a(new_n110_), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n99_), .c(new_n98_), .d(new_n93_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(z11));
  nand2  g76(.a(x01), .b(x00), .O(new_n120_));
  aoi21  g77(.a(x10), .b(x02), .c(new_n120_), .O(new_n121_));
  inv1   g78(.a(new_n51_), .O(new_n122_));
  oai21  g79(.a(x19), .b(x02), .c(x23), .O(new_n123_));
  nand3  g80(.a(new_n47_), .b(x17), .c(new_n49_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n123_), .c(new_n122_), .O(new_n125_));
  nor2   g82(.a(x24), .b(new_n50_), .O(new_n126_));
  oai21  g83(.a(new_n125_), .b(new_n121_), .c(new_n126_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n44_), .O(z12));
  oai21  g85(.a(new_n124_), .b(new_n122_), .c(new_n44_), .O(z13));
  nor3   g86(.a(new_n57_), .b(new_n52_), .c(new_n59_), .O(z14));
  oai21  g87(.a(x10), .b(new_n94_), .c(x02), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x00), .O(new_n132_));
  nand3  g89(.a(x19), .b(new_n49_), .c(new_n94_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(new_n132_), .c(new_n59_), .O(z15));
  oai21  g91(.a(new_n94_), .b(x00), .c(new_n44_), .O(z16));
  nand3  g92(.a(new_n51_), .b(new_n44_), .c(x02), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z17));
endmodule


