// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n122_, new_n124_, new_n125_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x09), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(new_n44_), .c(x00), .O(z00));
  nand2  g07(.a(x10), .b(x09), .O(new_n51_));
  nor2   g08(.a(x01), .b(x00), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n46_), .c(new_n45_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n51_), .O(z01));
  nand4  g11(.a(new_n46_), .b(new_n47_), .c(x09), .d(new_n45_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x00), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  nor2   g14(.a(new_n44_), .b(new_n57_), .O(new_n58_));
  inv1   g15(.a(x11), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n45_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n58_), .c(new_n48_), .d(x12), .O(new_n61_));
  inv1   g18(.a(x17), .O(new_n62_));
  nor2   g19(.a(x19), .b(x02), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g21(.a(new_n52_), .b(x18), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n64_), .c(new_n61_), .O(z03));
  nand2  g23(.a(new_n58_), .b(new_n48_), .O(new_n67_));
  inv1   g24(.a(x12), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n59_), .c(x02), .O(new_n69_));
  nor2   g26(.a(new_n69_), .b(new_n67_), .O(z04));
  nor4   g27(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(z05));
  nor3   g28(.a(new_n67_), .b(new_n59_), .c(new_n45_), .O(z06));
  nand3  g29(.a(new_n48_), .b(new_n68_), .c(x11), .O(new_n73_));
  inv1   g30(.a(new_n52_), .O(new_n74_));
  oai21  g31(.a(x02), .b(new_n57_), .c(new_n74_), .O(new_n75_));
  aoi21  g32(.a(new_n73_), .b(new_n58_), .c(new_n75_), .O(z07));
  inv1   g33(.a(x08), .O(new_n77_));
  inv1   g34(.a(x18), .O(new_n78_));
  nand4  g35(.a(x19), .b(new_n78_), .c(x17), .d(new_n77_), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(new_n80_));
  nor2   g37(.a(x03), .b(new_n45_), .O(new_n81_));
  inv1   g38(.a(x05), .O(new_n82_));
  inv1   g39(.a(x06), .O(new_n83_));
  inv1   g40(.a(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(new_n82_), .d(x04), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(new_n81_), .c(new_n80_), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g45(.a(x15), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(x12), .O(new_n91_));
  inv1   g48(.a(x14), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(new_n59_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(x20), .O(new_n98_));
  nand2  g55(.a(new_n52_), .b(x02), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n95_), .c(new_n89_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(z09));
  inv1   g59(.a(new_n98_), .O(new_n103_));
  nand3  g60(.a(x16), .b(x15), .c(new_n92_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n90_), .b(x12), .c(new_n59_), .d(x02), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n44_), .c(x00), .O(z10));
  nand3  g66(.a(new_n100_), .b(new_n95_), .c(x15), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(z11));
  oai21  g68(.a(new_n47_), .b(new_n45_), .c(new_n58_), .O(new_n112_));
  inv1   g69(.a(x23), .O(new_n113_));
  oai21  g70(.a(x19), .b(x02), .c(new_n113_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n64_), .c(new_n52_), .O(new_n115_));
  inv1   g72(.a(x24), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(x09), .O(new_n117_));
  aoi21  g74(.a(new_n115_), .b(new_n112_), .c(new_n117_), .O(z12));
  nand2  g75(.a(new_n63_), .b(x17), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n44_), .c(x00), .O(z13));
  inv1   g77(.a(x09), .O(new_n121_));
  nand2  g78(.a(new_n47_), .b(new_n121_), .O(new_n122_));
  nor2   g79(.a(new_n122_), .b(new_n53_), .O(z14));
  oai21  g80(.a(new_n47_), .b(new_n57_), .c(x01), .O(new_n124_));
  oai21  g81(.a(x19), .b(x00), .c(new_n45_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(z15));
  inv1   g83(.a(new_n99_), .O(z17));
  zero   g84(.O(z16));
endmodule


