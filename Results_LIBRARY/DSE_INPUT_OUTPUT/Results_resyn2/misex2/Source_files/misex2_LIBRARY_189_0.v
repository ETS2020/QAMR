// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n124_, new_n126_, new_n127_, new_n128_;
  nor2   g00(.a(x16), .b(x13), .O(new_n44_));
  inv1   g01(.a(new_n44_), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(new_n55_), .O(new_n57_));
  nor2   g14(.a(new_n44_), .b(x19), .O(new_n58_));
  nor2   g15(.a(x17), .b(x10), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n51_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(z02));
  nand3  g18(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(x11), .b(x10), .c(new_n47_), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n63_), .c(x12), .O(new_n66_));
  nor2   g23(.a(x19), .b(x02), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n48_), .c(x18), .d(new_n50_), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(new_n66_), .c(new_n44_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n47_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n62_), .c(new_n45_), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  inv1   g31(.a(x00), .O(new_n75_));
  inv1   g32(.a(x01), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x09), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(new_n74_), .c(new_n45_), .O(z05));
  nor3   g36(.a(new_n64_), .b(new_n62_), .c(new_n44_), .O(z06));
  oai21  g37(.a(new_n64_), .b(x12), .c(x01), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n45_), .O(z07));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  inv1   g42(.a(x04), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(x03), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n85_), .c(new_n84_), .d(new_n45_), .O(new_n88_));
  nand3  g45(.a(x02), .b(new_n76_), .c(new_n75_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x19), .c(new_n51_), .d(x17), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n88_), .O(z08));
  nand3  g49(.a(x18), .b(x01), .c(new_n75_), .O(new_n93_));
  inv1   g50(.a(x21), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nor2   g52(.a(x20), .b(x19), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  oai21  g54(.a(new_n97_), .b(new_n93_), .c(new_n45_), .O(z09));
  nand3  g55(.a(x20), .b(x16), .c(x15), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  inv1   g57(.a(x13), .O(new_n101_));
  inv1   g58(.a(x14), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n101_), .c(x12), .d(new_n70_), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  nand4  g61(.a(new_n95_), .b(new_n94_), .c(x02), .d(new_n76_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n104_), .c(new_n100_), .O(new_n107_));
  nor2   g64(.a(new_n51_), .b(new_n76_), .O(new_n108_));
  nor2   g65(.a(new_n95_), .b(new_n94_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n108_), .c(new_n96_), .d(new_n45_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n107_), .c(x00), .O(z10));
  nand3  g68(.a(new_n96_), .b(new_n95_), .c(x21), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(new_n93_), .c(new_n45_), .O(z11));
  nand2  g70(.a(new_n74_), .b(new_n77_), .O(new_n114_));
  nand2  g71(.a(new_n67_), .b(new_n50_), .O(new_n115_));
  inv1   g72(.a(x23), .O(new_n116_));
  oai21  g73(.a(x19), .b(x02), .c(new_n116_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n115_), .c(new_n48_), .O(new_n118_));
  nor2   g75(.a(x24), .b(new_n47_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n45_), .O(new_n120_));
  aoi21  g77(.a(new_n118_), .b(new_n114_), .c(new_n120_), .O(z12));
  nand4  g78(.a(x17), .b(new_n46_), .c(new_n76_), .d(new_n75_), .O(new_n122_));
  nor3   g79(.a(new_n122_), .b(new_n44_), .c(x19), .O(z13));
  nand3  g80(.a(new_n59_), .b(new_n52_), .c(new_n51_), .O(new_n124_));
  oai21  g81(.a(new_n124_), .b(new_n49_), .c(new_n45_), .O(z14));
  oai21  g82(.a(x10), .b(new_n76_), .c(x02), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x00), .O(new_n127_));
  nand3  g84(.a(x19), .b(new_n46_), .c(new_n76_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n127_), .c(new_n44_), .O(z15));
  oai21  g86(.a(new_n76_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g87(.a(new_n89_), .b(new_n45_), .O(z17));
endmodule


