// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n130_;
  inv1   g00(.a(x00), .O(new_n44_));
  nor2   g01(.a(x09), .b(x02), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g05(.a(x09), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(x02), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n46_), .c(x10), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g09(.a(x10), .O(new_n53_));
  nand3  g10(.a(new_n50_), .b(new_n46_), .c(new_n53_), .O(new_n54_));
  aoi21  g11(.a(new_n54_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g12(.a(x01), .O(new_n56_));
  nand4  g13(.a(x11), .b(x10), .c(new_n49_), .d(x02), .O(new_n57_));
  inv1   g14(.a(new_n57_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(x12), .c(new_n56_), .O(new_n59_));
  inv1   g16(.a(x18), .O(new_n60_));
  nor2   g17(.a(x19), .b(new_n60_), .O(new_n61_));
  nor3   g18(.a(x17), .b(x02), .c(x01), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  oai21  g20(.a(new_n59_), .b(new_n44_), .c(new_n63_), .O(z03));
  inv1   g21(.a(x11), .O(new_n65_));
  inv1   g22(.a(x12), .O(new_n66_));
  inv1   g23(.a(x02), .O(new_n67_));
  nor2   g24(.a(new_n53_), .b(new_n67_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n66_), .c(new_n65_), .d(new_n49_), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x01), .c(new_n44_), .O(z04));
  nand2  g27(.a(new_n68_), .b(x09), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x01), .c(new_n44_), .O(z05));
  nand2  g29(.a(x01), .b(x00), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n57_), .O(z06));
  nand2  g31(.a(new_n58_), .b(new_n66_), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x01), .c(new_n44_), .O(z07));
  inv1   g33(.a(x08), .O(new_n77_));
  nand4  g34(.a(x19), .b(new_n60_), .c(x17), .d(new_n77_), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(new_n79_));
  nor2   g36(.a(x03), .b(new_n67_), .O(new_n80_));
  inv1   g37(.a(x04), .O(new_n81_));
  nor2   g38(.a(x05), .b(new_n81_), .O(new_n82_));
  nor2   g39(.a(x07), .b(x06), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(new_n80_), .d(new_n79_), .O(new_n84_));
  aoi21  g41(.a(new_n84_), .b(new_n44_), .c(x01), .O(z08));
  nand2  g42(.a(new_n65_), .b(x02), .O(new_n86_));
  inv1   g43(.a(x13), .O(new_n87_));
  inv1   g44(.a(x14), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n87_), .c(x12), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n91_), .c(x20), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  inv1   g51(.a(x16), .O(new_n95_));
  nand2  g52(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  aoi21  g54(.a(new_n97_), .b(new_n90_), .c(x00), .O(new_n98_));
  nand3  g55(.a(x18), .b(x01), .c(new_n44_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  nor2   g57(.a(x20), .b(x19), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(new_n92_), .d(new_n91_), .O(new_n102_));
  oai21  g59(.a(new_n98_), .b(x01), .c(new_n102_), .O(z09));
  nor2   g60(.a(x20), .b(new_n56_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n61_), .c(x22), .d(x21), .O(new_n105_));
  nand3  g62(.a(x16), .b(x15), .c(new_n56_), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n93_), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(new_n90_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n105_), .c(x00), .O(z10));
  nand2  g66(.a(new_n95_), .b(x15), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(new_n93_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n90_), .c(x00), .O(new_n112_));
  nand4  g69(.a(new_n101_), .b(new_n100_), .c(new_n92_), .d(x21), .O(new_n113_));
  oai21  g70(.a(new_n112_), .b(x01), .c(new_n113_), .O(z11));
  inv1   g71(.a(new_n73_), .O(new_n115_));
  oai21  g72(.a(new_n53_), .b(new_n67_), .c(new_n115_), .O(new_n116_));
  inv1   g73(.a(x23), .O(new_n117_));
  nor2   g74(.a(x19), .b(x02), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x17), .O(new_n119_));
  oai21  g76(.a(new_n118_), .b(new_n117_), .c(new_n119_), .O(new_n120_));
  nor2   g77(.a(x01), .b(x00), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n116_), .c(new_n124_), .O(z12));
  inv1   g82(.a(new_n121_), .O(new_n126_));
  nor2   g83(.a(new_n126_), .b(new_n119_), .O(z13));
  nand3  g84(.a(new_n46_), .b(new_n45_), .c(new_n53_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n44_), .c(x01), .O(z14));
  nand3  g86(.a(new_n121_), .b(x19), .c(new_n67_), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n116_), .O(z15));
  nor2   g88(.a(new_n121_), .b(new_n115_), .O(z16));
  aoi21  g89(.a(new_n67_), .b(new_n44_), .c(x01), .O(z17));
endmodule


