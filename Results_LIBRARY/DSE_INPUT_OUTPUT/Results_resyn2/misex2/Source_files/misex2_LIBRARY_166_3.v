// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n133_, new_n134_, new_n136_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x23), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  inv1   g08(.a(x09), .O(new_n52_));
  nor2   g09(.a(x01), .b(x00), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x02), .O(new_n55_));
  nand2  g12(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n51_), .c(new_n47_), .O(z00));
  nand2  g14(.a(new_n55_), .b(x09), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n51_), .c(new_n47_), .O(z01));
  inv1   g16(.a(x10), .O(new_n60_));
  nand4  g17(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n60_), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n58_), .c(new_n47_), .O(z02));
  inv1   g19(.a(new_n55_), .O(new_n63_));
  inv1   g20(.a(x00), .O(new_n64_));
  nor2   g21(.a(new_n44_), .b(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(x12), .c(x10), .O(new_n66_));
  inv1   g23(.a(x02), .O(new_n67_));
  nor2   g24(.a(x09), .b(new_n67_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n45_), .c(x11), .O(new_n69_));
  nand3  g26(.a(new_n50_), .b(x18), .c(new_n48_), .O(new_n70_));
  oai22  g27(.a(new_n70_), .b(new_n63_), .c(new_n69_), .d(new_n66_), .O(z03));
  nor2   g28(.a(x12), .b(x11), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n68_), .c(x10), .d(x00), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(new_n45_), .c(new_n44_), .O(z04));
  nand2  g31(.a(new_n65_), .b(x02), .O(new_n75_));
  nand3  g32(.a(new_n45_), .b(x10), .c(x09), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(new_n75_), .O(z05));
  nand4  g34(.a(new_n45_), .b(x11), .c(x10), .d(new_n52_), .O(new_n78_));
  nor2   g35(.a(new_n78_), .b(new_n75_), .O(z06));
  or2    g36(.a(new_n78_), .b(x12), .O(new_n80_));
  nand2  g37(.a(x02), .b(x00), .O(new_n81_));
  aoi21  g38(.a(new_n80_), .b(x01), .c(new_n81_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nand2  g43(.a(new_n53_), .b(x02), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(z17));
  nand4  g45(.a(z17), .b(x19), .c(new_n49_), .d(x17), .O(new_n89_));
  oai21  g46(.a(new_n89_), .b(new_n86_), .c(new_n47_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n50_), .c(x18), .d(x01), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand2  g50(.a(x02), .b(new_n44_), .O(new_n94_));
  inv1   g51(.a(x11), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n95_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  inv1   g55(.a(x14), .O(new_n99_));
  inv1   g56(.a(x15), .O(new_n100_));
  inv1   g57(.a(x16), .O(new_n101_));
  nand4  g58(.a(x20), .b(new_n101_), .c(new_n100_), .d(new_n99_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(new_n98_), .c(new_n93_), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  inv1   g62(.a(x22), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n64_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n104_), .c(new_n47_), .O(z09));
  nor2   g65(.a(new_n46_), .b(new_n64_), .O(new_n109_));
  nand2  g66(.a(x15), .b(new_n99_), .O(new_n110_));
  nand4  g67(.a(new_n106_), .b(new_n105_), .c(x20), .d(x16), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n98_), .O(new_n113_));
  nor2   g70(.a(new_n92_), .b(new_n105_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(x22), .c(new_n46_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n113_), .c(new_n109_), .O(z10));
  nand2  g73(.a(new_n106_), .b(new_n64_), .O(new_n117_));
  nand3  g74(.a(new_n105_), .b(x20), .c(new_n101_), .O(new_n118_));
  nor2   g75(.a(new_n118_), .b(new_n110_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n98_), .c(new_n114_), .O(new_n120_));
  oai21  g77(.a(new_n120_), .b(new_n117_), .c(new_n47_), .O(z11));
  nand2  g78(.a(x10), .b(x02), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n65_), .c(new_n45_), .O(new_n123_));
  nor2   g80(.a(x19), .b(x02), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x17), .O(new_n125_));
  oai21  g82(.a(new_n124_), .b(new_n45_), .c(new_n125_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n53_), .O(new_n127_));
  inv1   g84(.a(x24), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x09), .O(new_n129_));
  aoi21  g86(.a(new_n127_), .b(new_n123_), .c(new_n129_), .O(z12));
  nor2   g87(.a(new_n125_), .b(new_n54_), .O(z13));
  nor2   g88(.a(new_n61_), .b(new_n56_), .O(z14));
  aoi21  g89(.a(x19), .b(new_n44_), .c(new_n109_), .O(new_n133_));
  nand3  g90(.a(new_n65_), .b(new_n45_), .c(new_n60_), .O(new_n134_));
  oai21  g91(.a(new_n133_), .b(x02), .c(new_n134_), .O(z15));
  nand3  g92(.a(new_n45_), .b(x01), .c(new_n64_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z16));
endmodule


