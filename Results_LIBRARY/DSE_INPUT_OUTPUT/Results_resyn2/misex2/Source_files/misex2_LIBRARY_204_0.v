// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n80_,
    new_n82_, new_n83_, new_n86_, new_n87_;
  nor2   g00(.a(x01), .b(x00), .O(z00));
  inv1   g01(.a(x10), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(x09), .O(new_n47_));
  nand3  g03(.a(x02), .b(x01), .c(x00), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(new_n47_), .c(x12), .d(x11), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(z03));
  inv1   g07(.a(x09), .O(new_n52_));
  nand2  g08(.a(x10), .b(new_n52_), .O(new_n53_));
  inv1   g09(.a(x11), .O(new_n54_));
  inv1   g10(.a(x12), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor3   g12(.a(new_n56_), .b(new_n48_), .c(new_n53_), .O(z04));
  nand2  g13(.a(x10), .b(x02), .O(new_n58_));
  inv1   g14(.a(x00), .O(new_n59_));
  inv1   g15(.a(x01), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(x09), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n58_), .O(z05));
  nor3   g19(.a(new_n48_), .b(new_n53_), .c(new_n54_), .O(z06));
  nor2   g20(.a(x02), .b(new_n59_), .O(new_n65_));
  and2   g21(.a(x11), .b(x00), .O(new_n66_));
  nand4  g22(.a(new_n66_), .b(new_n55_), .c(x10), .d(new_n52_), .O(new_n67_));
  aoi21  g23(.a(new_n67_), .b(x01), .c(new_n65_), .O(z07));
  nand2  g24(.a(x01), .b(new_n59_), .O(new_n69_));
  inv1   g25(.a(x19), .O(new_n70_));
  inv1   g26(.a(x20), .O(new_n71_));
  nand3  g27(.a(new_n71_), .b(new_n70_), .c(x18), .O(new_n72_));
  inv1   g28(.a(x21), .O(new_n73_));
  inv1   g29(.a(x22), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor3   g31(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z09));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand4  g33(.a(new_n77_), .b(new_n71_), .c(new_n70_), .d(x18), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(x01), .c(x00), .O(z10));
  nand2  g35(.a(new_n74_), .b(x21), .O(new_n80_));
  nor3   g36(.a(new_n80_), .b(new_n72_), .c(new_n69_), .O(z11));
  inv1   g37(.a(x24), .O(new_n82_));
  nand2  g38(.a(new_n58_), .b(new_n82_), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n62_), .O(z12));
  nand2  g40(.a(new_n61_), .b(new_n46_), .O(new_n86_));
  nor2   g41(.a(new_n65_), .b(z00), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(z15));
  inv1   g43(.a(new_n69_), .O(z16));
  zero   g44(.O(z01));
  zero   g45(.O(z14));
  zero   g46(.O(z17));
  nor2   g47(.a(x01), .b(x00), .O(z02));
  nor2   g48(.a(x01), .b(x00), .O(z08));
  nor2   g49(.a(x01), .b(x00), .O(z13));
endmodule


