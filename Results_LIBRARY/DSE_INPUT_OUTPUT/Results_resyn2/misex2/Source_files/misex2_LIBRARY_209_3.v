// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_;
  nand2  g00(.a(x23), .b(x12), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand3  g13(.a(new_n46_), .b(new_n45_), .c(new_n56_), .O(new_n57_));
  inv1   g14(.a(new_n54_), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n57_), .c(new_n44_), .O(z02));
  nand3  g17(.a(new_n51_), .b(new_n44_), .c(new_n47_), .O(new_n61_));
  nand3  g18(.a(x18), .b(new_n45_), .c(new_n49_), .O(new_n62_));
  inv1   g19(.a(x00), .O(new_n63_));
  inv1   g20(.a(x01), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n50_), .c(x02), .O(new_n66_));
  inv1   g23(.a(x23), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(x12), .c(x11), .d(x10), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n66_), .c(new_n62_), .d(new_n61_), .O(z03));
  inv1   g26(.a(x12), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(x10), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n66_), .c(x11), .O(z04));
  nand2  g29(.a(new_n65_), .b(new_n44_), .O(new_n73_));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n73_), .c(new_n50_), .O(z05));
  nand2  g32(.a(x11), .b(new_n50_), .O(new_n76_));
  nor3   g33(.a(new_n76_), .b(new_n74_), .c(new_n73_), .O(z06));
  oai21  g34(.a(new_n76_), .b(new_n71_), .c(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n44_), .O(z07));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nand3  g41(.a(x02), .b(new_n64_), .c(new_n63_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x19), .c(new_n46_), .d(x17), .O(new_n87_));
  oai21  g44(.a(new_n87_), .b(new_n84_), .c(new_n44_), .O(z08));
  inv1   g45(.a(x20), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n47_), .c(x18), .d(x01), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand2  g48(.a(x02), .b(new_n64_), .O(new_n92_));
  inv1   g49(.a(x11), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n93_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  inv1   g53(.a(x14), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nand4  g56(.a(x20), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n96_), .c(new_n91_), .O(new_n102_));
  inv1   g59(.a(x21), .O(new_n103_));
  inv1   g60(.a(x22), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n103_), .c(new_n63_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n102_), .c(new_n44_), .O(z09));
  nand2  g63(.a(x22), .b(x21), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n90_), .O(new_n108_));
  nand2  g65(.a(x15), .b(new_n97_), .O(new_n109_));
  nand4  g66(.a(new_n104_), .b(new_n103_), .c(x20), .d(x16), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n96_), .c(new_n108_), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g70(.a(new_n104_), .b(new_n63_), .O(new_n114_));
  nor3   g71(.a(x20), .b(new_n46_), .c(new_n64_), .O(new_n115_));
  nand4  g72(.a(new_n115_), .b(new_n44_), .c(x21), .d(new_n47_), .O(new_n116_));
  nand4  g73(.a(new_n67_), .b(new_n103_), .c(x20), .d(new_n99_), .O(new_n117_));
  nor2   g74(.a(new_n117_), .b(new_n109_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n96_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n116_), .c(new_n114_), .O(z11));
  nand2  g77(.a(new_n74_), .b(new_n65_), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(new_n67_), .O(new_n122_));
  nand3  g79(.a(new_n47_), .b(new_n45_), .c(new_n49_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n122_), .c(new_n51_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nor2   g82(.a(x24), .b(new_n50_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n44_), .O(z12));
  nor3   g85(.a(new_n61_), .b(new_n45_), .c(x02), .O(z13));
  nor4   g86(.a(new_n61_), .b(new_n57_), .c(x09), .d(x02), .O(z14));
  inv1   g87(.a(new_n44_), .O(new_n131_));
  oai21  g88(.a(x10), .b(new_n64_), .c(x02), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(x00), .O(new_n133_));
  nand3  g90(.a(x19), .b(new_n49_), .c(new_n64_), .O(new_n134_));
  aoi21  g91(.a(new_n134_), .b(new_n133_), .c(new_n131_), .O(z15));
  nand3  g92(.a(new_n44_), .b(x01), .c(new_n63_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z16));
  nand2  g94(.a(new_n85_), .b(new_n44_), .O(z17));
endmodule


