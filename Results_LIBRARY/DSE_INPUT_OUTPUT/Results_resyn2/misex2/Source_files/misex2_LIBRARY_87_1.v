// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n136_;
  inv1   g00(.a(x22), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x02), .b(x00), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  inv1   g04(.a(x10), .O(new_n48_));
  nor2   g05(.a(x17), .b(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x01), .O(z00));
  nand2  g08(.a(new_n47_), .b(new_n44_), .O(new_n52_));
  inv1   g09(.a(x01), .O(new_n53_));
  nand3  g10(.a(new_n46_), .b(x09), .c(new_n53_), .O(new_n54_));
  nor4   g11(.a(new_n54_), .b(new_n52_), .c(x17), .d(new_n48_), .O(z01));
  nor4   g12(.a(new_n54_), .b(new_n52_), .c(x17), .d(x10), .O(z02));
  inv1   g13(.a(x19), .O(new_n57_));
  nand2  g14(.a(new_n57_), .b(x18), .O(new_n58_));
  inv1   g15(.a(new_n58_), .O(new_n59_));
  nor3   g16(.a(x17), .b(x02), .c(x00), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(new_n59_), .c(x22), .O(new_n61_));
  nor2   g18(.a(new_n48_), .b(x09), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n62_), .c(x12), .d(x11), .O(new_n65_));
  oai21  g22(.a(new_n61_), .b(x01), .c(new_n65_), .O(z03));
  nand2  g23(.a(new_n64_), .b(new_n62_), .O(new_n67_));
  nor3   g24(.a(new_n67_), .b(x12), .c(x11), .O(z04));
  nand2  g25(.a(x22), .b(new_n53_), .O(new_n69_));
  nand2  g26(.a(x10), .b(x02), .O(new_n70_));
  nand3  g27(.a(x09), .b(x01), .c(x00), .O(new_n71_));
  oai21  g28(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(z05));
  inv1   g29(.a(x11), .O(new_n73_));
  nor2   g30(.a(new_n67_), .b(new_n73_), .O(z06));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  nor2   g32(.a(x12), .b(new_n73_), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(new_n62_), .c(new_n53_), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n75_), .c(new_n69_), .O(z07));
  inv1   g35(.a(x18), .O(new_n79_));
  nand3  g36(.a(x19), .b(new_n79_), .c(x17), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  inv1   g40(.a(x00), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand4  g42(.a(x04), .b(new_n85_), .c(x02), .d(new_n84_), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(new_n88_));
  aoi21  g45(.a(new_n88_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand3  g47(.a(new_n59_), .b(new_n90_), .c(x01), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n53_), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n73_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nor2   g53(.a(x15), .b(x14), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(x20), .d(new_n92_), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  nand3  g56(.a(new_n44_), .b(new_n99_), .c(new_n84_), .O(new_n100_));
  aoi21  g57(.a(new_n98_), .b(new_n91_), .c(new_n100_), .O(z09));
  inv1   g58(.a(x14), .O(new_n102_));
  nand3  g59(.a(new_n99_), .b(x15), .c(new_n102_), .O(new_n103_));
  nand2  g60(.a(x20), .b(x16), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g62(.a(x22), .b(x21), .c(new_n90_), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n58_), .O(new_n107_));
  aoi21  g64(.a(new_n105_), .b(new_n96_), .c(new_n107_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(x00), .c(new_n69_), .O(z10));
  nand2  g66(.a(new_n44_), .b(new_n84_), .O(new_n110_));
  nand4  g67(.a(new_n59_), .b(x21), .c(new_n90_), .d(x01), .O(new_n111_));
  inv1   g68(.a(new_n103_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n96_), .c(x20), .d(new_n92_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(z11));
  nand3  g71(.a(new_n70_), .b(x01), .c(x00), .O(new_n115_));
  inv1   g72(.a(x23), .O(new_n116_));
  nor2   g73(.a(x19), .b(x02), .O(new_n117_));
  nor2   g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g75(.a(new_n117_), .b(x17), .O(new_n119_));
  inv1   g76(.a(new_n119_), .O(new_n120_));
  nor2   g77(.a(new_n110_), .b(x01), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n118_), .c(new_n121_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n115_), .c(new_n124_), .O(z12));
  nand2  g82(.a(new_n120_), .b(new_n84_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n44_), .c(x01), .O(z13));
  nor2   g84(.a(x17), .b(x10), .O(new_n128_));
  nand4  g85(.a(new_n128_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n129_));
  aoi21  g86(.a(new_n129_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g87(.a(x19), .b(new_n53_), .c(x00), .O(new_n131_));
  inv1   g88(.a(x02), .O(new_n132_));
  nand2  g89(.a(new_n44_), .b(new_n132_), .O(new_n133_));
  oai21  g90(.a(new_n133_), .b(new_n131_), .c(new_n115_), .O(z15));
  nor2   g91(.a(new_n53_), .b(x00), .O(z16));
  nand2  g92(.a(new_n121_), .b(x02), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z17));
endmodule


