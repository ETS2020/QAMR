// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x19), .O(new_n44_));
  inv1   g01(.a(x21), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(x16), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n48_), .O(z01));
  nand3  g12(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n56_));
  nor3   g13(.a(new_n54_), .b(new_n56_), .c(x10), .O(z02));
  inv1   g14(.a(x16), .O(new_n58_));
  nor2   g15(.a(x21), .b(new_n58_), .O(new_n59_));
  inv1   g16(.a(x17), .O(new_n60_));
  nor2   g17(.a(x19), .b(x02), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n51_), .c(x18), .d(new_n60_), .O(new_n62_));
  inv1   g19(.a(x10), .O(new_n63_));
  inv1   g20(.a(x11), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor2   g22(.a(x09), .b(new_n49_), .O(new_n66_));
  nand2  g23(.a(x01), .b(x00), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n66_), .c(new_n65_), .d(x12), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(new_n62_), .c(new_n59_), .O(z03));
  nor2   g27(.a(new_n67_), .b(new_n59_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(new_n66_), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(new_n73_), .b(new_n64_), .c(x10), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n72_), .O(z04));
  nand4  g32(.a(new_n71_), .b(x10), .c(x09), .d(x02), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z05));
  nand3  g34(.a(new_n71_), .b(new_n66_), .c(new_n65_), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(z06));
  nand3  g36(.a(new_n65_), .b(new_n73_), .c(new_n50_), .O(new_n80_));
  nand3  g37(.a(new_n46_), .b(x02), .c(x00), .O(new_n81_));
  aoi21  g38(.a(new_n80_), .b(x01), .c(new_n81_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  inv1   g43(.a(x00), .O(new_n87_));
  inv1   g44(.a(x01), .O(new_n88_));
  nand3  g45(.a(x02), .b(new_n88_), .c(new_n87_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nor2   g47(.a(x18), .b(new_n60_), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n90_), .c(x19), .O(new_n92_));
  oai21  g49(.a(new_n92_), .b(new_n86_), .c(new_n46_), .O(z08));
  nor2   g50(.a(x20), .b(x19), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x18), .c(x01), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  nand4  g53(.a(x12), .b(new_n64_), .c(x02), .d(new_n88_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nor2   g55(.a(x14), .b(x13), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(x20), .d(new_n96_), .O(new_n100_));
  nor2   g57(.a(x22), .b(x00), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n45_), .c(new_n58_), .O(new_n102_));
  aoi21  g59(.a(new_n100_), .b(new_n95_), .c(new_n102_), .O(z09));
  nand4  g60(.a(new_n94_), .b(x21), .c(x18), .d(x01), .O(new_n104_));
  nand2  g61(.a(x22), .b(new_n87_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n104_), .c(new_n46_), .O(z10));
  nand4  g63(.a(new_n99_), .b(new_n45_), .c(x20), .d(x15), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n97_), .c(new_n104_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n101_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n46_), .O(z11));
  oai21  g67(.a(new_n63_), .b(new_n49_), .c(new_n68_), .O(new_n111_));
  nand2  g68(.a(new_n61_), .b(new_n60_), .O(new_n112_));
  inv1   g69(.a(x23), .O(new_n113_));
  oai21  g70(.a(x19), .b(x02), .c(new_n113_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n112_), .c(new_n51_), .O(new_n115_));
  inv1   g72(.a(x24), .O(new_n116_));
  nand3  g73(.a(new_n46_), .b(new_n116_), .c(x09), .O(new_n117_));
  aoi21  g74(.a(new_n115_), .b(new_n111_), .c(new_n117_), .O(z12));
  nand3  g75(.a(new_n61_), .b(new_n51_), .c(x17), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n46_), .O(z13));
  nand3  g77(.a(new_n47_), .b(new_n44_), .c(new_n63_), .O(new_n121_));
  oai21  g78(.a(new_n121_), .b(new_n52_), .c(new_n46_), .O(z14));
  oai21  g79(.a(x10), .b(new_n88_), .c(x02), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x00), .O(new_n124_));
  nand3  g81(.a(x19), .b(new_n49_), .c(new_n88_), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(new_n124_), .c(new_n46_), .O(z15));
  nor3   g83(.a(new_n59_), .b(new_n88_), .c(x00), .O(z16));
  nand2  g84(.a(new_n89_), .b(new_n46_), .O(z17));
endmodule


