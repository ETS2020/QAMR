// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n127_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x19), .O(new_n44_));
  nor2   g01(.a(x23), .b(x16), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n48_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand3  g13(.a(new_n47_), .b(new_n44_), .c(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n46_), .O(z02));
  inv1   g15(.a(x12), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nand3  g17(.a(x11), .b(x10), .c(new_n50_), .O(new_n61_));
  or2    g18(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g19(.a(x17), .O(new_n63_));
  nor2   g20(.a(x19), .b(x02), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n51_), .c(x18), .d(new_n63_), .O(new_n65_));
  and2   g22(.a(new_n65_), .b(new_n46_), .O(new_n66_));
  oai21  g23(.a(new_n62_), .b(new_n59_), .c(new_n66_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  nand4  g25(.a(new_n59_), .b(new_n68_), .c(x10), .d(new_n50_), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n60_), .c(new_n46_), .O(z04));
  nand2  g27(.a(x01), .b(x00), .O(new_n71_));
  nand2  g28(.a(x10), .b(x02), .O(new_n72_));
  nor4   g29(.a(new_n72_), .b(new_n71_), .c(new_n45_), .d(new_n50_), .O(z05));
  nand2  g30(.a(new_n62_), .b(new_n46_), .O(z06));
  oai21  g31(.a(new_n61_), .b(x12), .c(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n46_), .O(z07));
  nand3  g34(.a(new_n51_), .b(new_n46_), .c(x02), .O(new_n78_));
  inv1   g35(.a(x18), .O(new_n79_));
  nand3  g36(.a(x19), .b(new_n79_), .c(x17), .O(new_n80_));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n80_), .c(new_n78_), .O(z08));
  inv1   g42(.a(x20), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n44_), .c(x18), .d(x01), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  inv1   g45(.a(x01), .O(new_n89_));
  nand2  g46(.a(x02), .b(new_n89_), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n68_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand4  g53(.a(x20), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  aoi21  g55(.a(new_n98_), .b(new_n93_), .c(new_n88_), .O(new_n99_));
  inv1   g56(.a(x00), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  oai21  g60(.a(new_n103_), .b(new_n99_), .c(new_n46_), .O(z09));
  nand4  g61(.a(new_n88_), .b(new_n46_), .c(x22), .d(x21), .O(new_n105_));
  nand4  g62(.a(new_n102_), .b(new_n101_), .c(x20), .d(x16), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n93_), .c(x15), .d(new_n94_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n105_), .c(x00), .O(z10));
  nand2  g66(.a(new_n102_), .b(new_n100_), .O(new_n110_));
  nand3  g67(.a(new_n88_), .b(new_n46_), .c(x21), .O(new_n111_));
  nand4  g68(.a(x23), .b(new_n101_), .c(x20), .d(new_n96_), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(new_n113_));
  nand4  g70(.a(new_n113_), .b(new_n93_), .c(x15), .d(new_n94_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n111_), .c(new_n110_), .O(z11));
  nand4  g72(.a(new_n72_), .b(new_n46_), .c(x01), .d(x00), .O(new_n116_));
  nand2  g73(.a(new_n64_), .b(new_n63_), .O(new_n117_));
  inv1   g74(.a(x23), .O(new_n118_));
  nand2  g75(.a(new_n64_), .b(x16), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n117_), .c(new_n51_), .O(new_n121_));
  inv1   g78(.a(x24), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x09), .O(new_n123_));
  aoi21  g80(.a(new_n121_), .b(new_n116_), .c(new_n123_), .O(z12));
  nand3  g81(.a(new_n64_), .b(new_n51_), .c(x17), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n46_), .O(z13));
  nand3  g83(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n127_));
  nor3   g84(.a(new_n52_), .b(new_n127_), .c(x10), .O(z14));
  oai21  g85(.a(x10), .b(new_n89_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n49_), .c(new_n89_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n130_), .c(new_n45_), .O(z15));
  nor3   g89(.a(new_n45_), .b(new_n89_), .c(x00), .O(z16));
  inv1   g90(.a(new_n78_), .O(z17));
endmodule


