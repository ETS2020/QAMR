// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n118_, new_n120_, new_n121_;
  inv1   g00(.a(x01), .O(new_n44_));
  nor2   g01(.a(x09), .b(x02), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(new_n44_), .c(x00), .O(z00));
  inv1   g05(.a(x09), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(x02), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n46_), .c(x10), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x00), .O(z01));
  inv1   g09(.a(x10), .O(new_n53_));
  nand3  g10(.a(new_n50_), .b(new_n46_), .c(new_n53_), .O(new_n54_));
  aoi21  g11(.a(new_n54_), .b(new_n44_), .c(x00), .O(z02));
  inv1   g12(.a(x00), .O(new_n56_));
  inv1   g13(.a(x19), .O(new_n57_));
  nand2  g14(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  inv1   g15(.a(x02), .O(new_n59_));
  inv1   g16(.a(x17), .O(new_n60_));
  nand4  g17(.a(x18), .b(new_n60_), .c(new_n59_), .d(new_n44_), .O(new_n61_));
  nand2  g18(.a(x10), .b(x02), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(x11), .c(new_n49_), .O(new_n64_));
  nand3  g21(.a(x12), .b(x01), .c(x00), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n61_), .d(new_n58_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nor2   g24(.a(x12), .b(new_n53_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n67_), .c(new_n49_), .d(x02), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x00), .c(new_n44_), .O(z04));
  nand2  g27(.a(new_n63_), .b(x09), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x00), .c(new_n44_), .O(z05));
  aoi21  g29(.a(new_n64_), .b(x00), .c(new_n44_), .O(z06));
  nand3  g30(.a(new_n68_), .b(x11), .c(new_n49_), .O(new_n74_));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  aoi21  g32(.a(new_n74_), .b(x01), .c(new_n75_), .O(z07));
  inv1   g33(.a(x03), .O(new_n77_));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(new_n78_), .c(x04), .d(new_n77_), .O(new_n80_));
  nor2   g37(.a(x01), .b(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(x02), .O(new_n82_));
  inv1   g39(.a(x18), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n83_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n82_), .c(new_n80_), .O(z08));
  inv1   g42(.a(x12), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(x11), .O(new_n87_));
  nor2   g44(.a(x14), .b(x13), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n81_), .d(x02), .O(new_n89_));
  inv1   g46(.a(x15), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  inv1   g48(.a(x21), .O(new_n92_));
  inv1   g49(.a(x22), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(x20), .d(new_n91_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand2  g52(.a(new_n95_), .b(new_n90_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n89_), .O(z09));
  inv1   g54(.a(new_n82_), .O(new_n98_));
  inv1   g55(.a(x13), .O(new_n99_));
  inv1   g56(.a(x14), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n99_), .c(x12), .d(new_n67_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n93_), .b(new_n92_), .c(x20), .d(x16), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n102_), .c(new_n98_), .d(x15), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(z10));
  nand4  g63(.a(new_n95_), .b(new_n102_), .c(new_n98_), .d(x15), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(z11));
  nand3  g65(.a(new_n62_), .b(x01), .c(x00), .O(new_n109_));
  oai21  g66(.a(x19), .b(x02), .c(x23), .O(new_n110_));
  nand3  g67(.a(new_n57_), .b(x17), .c(new_n59_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n81_), .O(new_n113_));
  inv1   g70(.a(x24), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(x09), .O(new_n115_));
  aoi21  g72(.a(new_n113_), .b(new_n109_), .c(new_n115_), .O(z12));
  aoi21  g73(.a(new_n111_), .b(new_n44_), .c(x00), .O(z13));
  nand3  g74(.a(new_n46_), .b(new_n45_), .c(new_n53_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n44_), .c(x00), .O(z14));
  oai21  g76(.a(new_n53_), .b(new_n56_), .c(x01), .O(new_n120_));
  nand2  g77(.a(new_n58_), .b(new_n59_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(z15));
  nor2   g79(.a(new_n44_), .b(x00), .O(z16));
  aoi21  g80(.a(new_n59_), .b(new_n44_), .c(x00), .O(z17));
endmodule


