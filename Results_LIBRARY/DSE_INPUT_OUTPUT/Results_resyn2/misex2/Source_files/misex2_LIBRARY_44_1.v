// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n117_, new_n119_,
    new_n120_;
  inv1   g00(.a(x02), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  nor2   g04(.a(x17), .b(new_n47_), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(x09), .O(z00));
  inv1   g07(.a(x09), .O(new_n51_));
  nor2   g08(.a(new_n49_), .b(new_n51_), .O(z01));
  nand2  g09(.a(new_n45_), .b(new_n44_), .O(new_n53_));
  nor2   g10(.a(x17), .b(x10), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(new_n46_), .c(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x19), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g16(.a(x01), .O(new_n60_));
  inv1   g17(.a(x17), .O(new_n61_));
  nand4  g18(.a(x18), .b(new_n61_), .c(new_n44_), .d(new_n60_), .O(new_n62_));
  nand4  g19(.a(x11), .b(x10), .c(new_n51_), .d(x02), .O(new_n63_));
  nand2  g20(.a(x01), .b(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(x12), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n63_), .c(new_n62_), .d(new_n59_), .O(z03));
  inv1   g24(.a(x12), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(x10), .c(new_n51_), .O(new_n69_));
  nor4   g26(.a(new_n69_), .b(new_n64_), .c(x11), .d(new_n44_), .O(z04));
  nand2  g27(.a(x10), .b(x02), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n64_), .c(new_n51_), .O(z05));
  aoi21  g29(.a(new_n63_), .b(x00), .c(new_n60_), .O(z06));
  inv1   g30(.a(new_n69_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x11), .O(new_n75_));
  nand2  g32(.a(x02), .b(x00), .O(new_n76_));
  aoi21  g33(.a(new_n75_), .b(x01), .c(new_n76_), .O(z07));
  inv1   g34(.a(x08), .O(new_n78_));
  inv1   g35(.a(x18), .O(new_n79_));
  nand4  g36(.a(x19), .b(new_n79_), .c(x17), .d(new_n78_), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nor2   g38(.a(x03), .b(new_n44_), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  inv1   g40(.a(x06), .O(new_n84_));
  inv1   g41(.a(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(x04), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n82_), .c(new_n81_), .O(new_n88_));
  aoi21  g45(.a(new_n88_), .b(new_n60_), .c(x00), .O(z08));
  inv1   g46(.a(x16), .O(new_n90_));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(x20), .d(new_n90_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  inv1   g51(.a(x11), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(x12), .c(new_n95_), .d(x02), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nor2   g55(.a(x15), .b(x14), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(new_n94_), .O(new_n100_));
  aoi21  g57(.a(new_n100_), .b(new_n60_), .c(x00), .O(z09));
  inv1   g58(.a(x14), .O(new_n102_));
  nand3  g59(.a(new_n45_), .b(x15), .c(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n92_), .b(new_n91_), .c(x20), .d(x16), .O(new_n104_));
  nor3   g61(.a(new_n104_), .b(new_n103_), .c(new_n97_), .O(z10));
  nor3   g62(.a(new_n103_), .b(new_n97_), .c(new_n93_), .O(z11));
  nand2  g63(.a(new_n71_), .b(new_n65_), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g65(.a(new_n58_), .b(x17), .c(new_n44_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n45_), .O(new_n111_));
  inv1   g68(.a(x24), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(x09), .O(new_n113_));
  aoi21  g70(.a(new_n111_), .b(new_n107_), .c(new_n113_), .O(z12));
  aoi21  g71(.a(new_n109_), .b(new_n60_), .c(x00), .O(z13));
  nor2   g72(.a(x19), .b(x02), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(new_n54_), .c(new_n79_), .d(new_n51_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n60_), .c(x00), .O(z14));
  oai21  g75(.a(new_n47_), .b(new_n57_), .c(x01), .O(new_n119_));
  nand2  g76(.a(new_n59_), .b(new_n44_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n119_), .O(z15));
  nor2   g78(.a(new_n60_), .b(x00), .O(z16));
  aoi21  g79(.a(new_n44_), .b(new_n60_), .c(x00), .O(z17));
endmodule


