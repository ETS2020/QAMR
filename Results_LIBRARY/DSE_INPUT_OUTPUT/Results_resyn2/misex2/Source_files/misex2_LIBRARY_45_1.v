// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n122_, new_n124_,
    new_n125_;
  inv1   g00(.a(x02), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  inv1   g04(.a(x16), .O(new_n48_));
  nor3   g05(.a(x19), .b(x18), .c(x17), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n47_), .O(z00));
  nand3  g08(.a(new_n46_), .b(x09), .c(new_n44_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nor2   g11(.a(x02), .b(x00), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n49_), .c(new_n54_), .d(x09), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n48_), .c(x01), .O(z02));
  inv1   g14(.a(x12), .O(new_n58_));
  inv1   g15(.a(x18), .O(new_n59_));
  nor2   g16(.a(x19), .b(new_n59_), .O(new_n60_));
  nor3   g17(.a(x17), .b(x02), .c(x00), .O(new_n61_));
  aoi21  g18(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  and2   g19(.a(x11), .b(x10), .O(new_n63_));
  nand4  g20(.a(new_n45_), .b(x02), .c(x01), .d(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n58_), .c(new_n62_), .d(x01), .O(z03));
  nor4   g24(.a(new_n64_), .b(x12), .c(x11), .d(new_n54_), .O(z04));
  inv1   g25(.a(x01), .O(new_n69_));
  nand2  g26(.a(x16), .b(new_n69_), .O(new_n70_));
  nand2  g27(.a(x01), .b(x00), .O(new_n71_));
  nand3  g28(.a(x10), .b(x09), .c(x02), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(z05));
  nand2  g30(.a(new_n70_), .b(new_n66_), .O(z06));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  nand2  g32(.a(new_n48_), .b(new_n69_), .O(new_n76_));
  nand4  g33(.a(new_n63_), .b(new_n58_), .c(new_n45_), .d(x01), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nand3  g39(.a(new_n46_), .b(new_n48_), .c(x02), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n59_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(z08));
  nor2   g42(.a(new_n44_), .b(x01), .O(new_n86_));
  nor2   g43(.a(new_n58_), .b(x11), .O(new_n87_));
  nor2   g44(.a(x14), .b(x13), .O(new_n88_));
  inv1   g45(.a(x20), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x15), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n91_));
  nand3  g48(.a(new_n60_), .b(new_n89_), .c(x01), .O(new_n92_));
  nand2  g49(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  inv1   g50(.a(x00), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nand2  g52(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(x21), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(new_n93_), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n70_), .O(z09));
  inv1   g56(.a(new_n92_), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(x22), .c(x21), .d(new_n94_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(z10));
  nand4  g59(.a(new_n60_), .b(x21), .c(new_n89_), .d(x01), .O(new_n103_));
  inv1   g60(.a(x21), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(x20), .c(new_n48_), .d(x15), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n103_), .c(new_n96_), .O(z11));
  nand2  g65(.a(x10), .b(x02), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x01), .c(x00), .O(new_n110_));
  inv1   g67(.a(x19), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n44_), .c(x23), .O(new_n112_));
  inv1   g69(.a(x17), .O(new_n113_));
  nand3  g70(.a(new_n111_), .b(new_n113_), .c(new_n44_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n46_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n112_), .c(new_n110_), .O(new_n116_));
  nor2   g73(.a(x24), .b(new_n45_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n70_), .O(z12));
  nand3  g76(.a(new_n55_), .b(new_n111_), .c(x17), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n48_), .c(x01), .O(z13));
  nand3  g78(.a(new_n49_), .b(new_n48_), .c(new_n54_), .O(new_n122_));
  nor2   g79(.a(new_n122_), .b(new_n47_), .O(z14));
  aoi21  g80(.a(x19), .b(new_n69_), .c(x00), .O(new_n124_));
  nand2  g81(.a(new_n48_), .b(new_n44_), .O(new_n125_));
  oai21  g82(.a(new_n125_), .b(new_n124_), .c(new_n110_), .O(z15));
  nor2   g83(.a(new_n69_), .b(x00), .O(z16));
  inv1   g84(.a(new_n83_), .O(z17));
endmodule


