// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n127_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x19), .O(new_n44_));
  nor2   g01(.a(x06), .b(x04), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nor3   g06(.a(x02), .b(x01), .c(x00), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n48_), .O(z00));
  nand2  g09(.a(new_n50_), .b(x09), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n48_), .O(z01));
  nor2   g11(.a(x19), .b(x10), .O(new_n55_));
  nand2  g12(.a(new_n55_), .b(new_n47_), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n53_), .c(new_n46_), .O(z02));
  inv1   g14(.a(x00), .O(new_n58_));
  inv1   g15(.a(x01), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g17(.a(x11), .b(x10), .c(new_n49_), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(new_n60_), .c(x12), .d(x02), .O(new_n63_));
  inv1   g20(.a(x17), .O(new_n64_));
  nand4  g21(.a(new_n50_), .b(new_n44_), .c(x18), .d(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n63_), .c(new_n46_), .O(z03));
  nand2  g23(.a(new_n60_), .b(x02), .O(new_n67_));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n49_), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n67_), .c(new_n46_), .O(z04));
  nand2  g28(.a(x10), .b(x02), .O(new_n72_));
  nand2  g29(.a(new_n60_), .b(x09), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n72_), .c(new_n46_), .O(z05));
  nor3   g31(.a(new_n61_), .b(new_n67_), .c(new_n45_), .O(z06));
  nand2  g32(.a(new_n62_), .b(new_n69_), .O(new_n76_));
  nand3  g33(.a(new_n46_), .b(x02), .c(x00), .O(new_n77_));
  aoi21  g34(.a(new_n76_), .b(x01), .c(new_n77_), .O(z07));
  nor2   g35(.a(x01), .b(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x02), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nor2   g38(.a(x18), .b(new_n64_), .O(new_n82_));
  nor2   g39(.a(x05), .b(x03), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n82_), .c(new_n81_), .d(x19), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(x04), .c(x06), .O(z08));
  inv1   g45(.a(x20), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n44_), .c(x18), .d(x01), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  nand2  g48(.a(x02), .b(new_n59_), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(x12), .c(new_n68_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nor2   g52(.a(x15), .b(x14), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(x20), .d(new_n91_), .O(new_n97_));
  inv1   g54(.a(x21), .O(new_n98_));
  inv1   g55(.a(x22), .O(new_n99_));
  nand4  g56(.a(new_n46_), .b(new_n99_), .c(new_n98_), .d(new_n58_), .O(new_n100_));
  aoi21  g57(.a(new_n97_), .b(new_n90_), .c(new_n100_), .O(z09));
  nand2  g58(.a(new_n46_), .b(new_n58_), .O(new_n102_));
  nor2   g59(.a(new_n90_), .b(new_n98_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(x22), .O(new_n104_));
  inv1   g61(.a(x14), .O(new_n105_));
  nand2  g62(.a(x15), .b(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n99_), .b(new_n98_), .c(x20), .d(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n95_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n104_), .c(new_n102_), .O(z10));
  nand3  g67(.a(new_n98_), .b(x20), .c(new_n91_), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n106_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n95_), .c(new_n103_), .O(new_n113_));
  nand2  g70(.a(new_n99_), .b(new_n58_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n113_), .c(new_n46_), .O(z11));
  nand2  g72(.a(new_n72_), .b(new_n60_), .O(new_n116_));
  inv1   g73(.a(x02), .O(new_n117_));
  nand3  g74(.a(new_n44_), .b(new_n64_), .c(new_n117_), .O(new_n118_));
  inv1   g75(.a(x23), .O(new_n119_));
  oai21  g76(.a(x19), .b(x02), .c(new_n119_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n118_), .c(new_n79_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n49_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n46_), .O(new_n123_));
  aoi21  g80(.a(new_n121_), .b(new_n116_), .c(new_n123_), .O(z12));
  nor3   g81(.a(new_n45_), .b(x19), .c(new_n64_), .O(new_n125_));
  and2   g82(.a(new_n125_), .b(new_n50_), .O(z13));
  nand3  g83(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n127_));
  nor3   g84(.a(new_n51_), .b(new_n127_), .c(x10), .O(z14));
  oai21  g85(.a(x10), .b(new_n59_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n117_), .c(new_n59_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n130_), .c(new_n45_), .O(z15));
  nor2   g89(.a(new_n102_), .b(new_n59_), .O(z16));
  nand2  g90(.a(new_n80_), .b(new_n46_), .O(z17));
endmodule


