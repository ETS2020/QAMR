// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n115_, new_n116_, new_n117_, new_n120_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x00), .O(z00));
  nand4  g06(.a(new_n47_), .b(x10), .c(x09), .d(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x00), .O(z01));
  inv1   g08(.a(new_n47_), .O(new_n52_));
  inv1   g09(.a(x10), .O(new_n53_));
  nor2   g10(.a(x01), .b(x00), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n53_), .c(x09), .d(new_n45_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  nand2  g13(.a(x11), .b(new_n46_), .O(new_n57_));
  nand4  g14(.a(x10), .b(x02), .c(x01), .d(x00), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n57_), .O(z06));
  nand2  g16(.a(z06), .b(x12), .O(new_n60_));
  inv1   g17(.a(x19), .O(new_n61_));
  inv1   g18(.a(x18), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x17), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n54_), .c(new_n61_), .d(new_n45_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n60_), .O(z03));
  inv1   g22(.a(x12), .O(new_n66_));
  nor2   g23(.a(x11), .b(new_n45_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x10), .d(new_n46_), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(x00), .c(new_n44_), .O(z04));
  nor2   g26(.a(new_n58_), .b(new_n46_), .O(z05));
  nand4  g27(.a(new_n66_), .b(x11), .c(x10), .d(new_n46_), .O(new_n71_));
  nand2  g28(.a(x02), .b(x00), .O(new_n72_));
  aoi21  g29(.a(new_n71_), .b(x01), .c(new_n72_), .O(z07));
  inv1   g30(.a(x08), .O(new_n74_));
  nand4  g31(.a(x19), .b(new_n62_), .c(x17), .d(new_n74_), .O(new_n75_));
  inv1   g32(.a(new_n75_), .O(new_n76_));
  nor2   g33(.a(x03), .b(new_n45_), .O(new_n77_));
  inv1   g34(.a(x05), .O(new_n78_));
  inv1   g35(.a(x06), .O(new_n79_));
  inv1   g36(.a(x07), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(x04), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(new_n77_), .c(new_n76_), .O(new_n83_));
  aoi21  g40(.a(new_n83_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g41(.a(x13), .O(new_n85_));
  inv1   g42(.a(x14), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(new_n85_), .c(x12), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  inv1   g45(.a(x21), .O(new_n89_));
  inv1   g46(.a(x22), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nor2   g49(.a(x16), .b(x15), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(new_n88_), .d(new_n67_), .O(new_n94_));
  aoi21  g51(.a(new_n94_), .b(new_n44_), .c(x00), .O(z09));
  and2   g52(.a(x16), .b(x15), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n92_), .c(new_n88_), .d(new_n67_), .O(new_n97_));
  aoi21  g54(.a(new_n97_), .b(new_n44_), .c(x00), .O(z10));
  inv1   g55(.a(x15), .O(new_n99_));
  nor2   g56(.a(x16), .b(new_n99_), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n92_), .c(new_n88_), .d(new_n67_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n44_), .c(x00), .O(z11));
  nand2  g59(.a(x10), .b(x02), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(x01), .c(x00), .O(new_n104_));
  oai21  g61(.a(x19), .b(x02), .c(x23), .O(new_n105_));
  nand3  g62(.a(new_n61_), .b(x17), .c(new_n45_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(new_n54_), .O(new_n108_));
  inv1   g65(.a(x24), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(x09), .O(new_n110_));
  aoi21  g67(.a(new_n108_), .b(new_n104_), .c(new_n110_), .O(z12));
  aoi21  g68(.a(new_n106_), .b(new_n44_), .c(x00), .O(z13));
  nand4  g69(.a(new_n47_), .b(new_n53_), .c(new_n46_), .d(new_n45_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n44_), .c(x00), .O(z14));
  nand2  g71(.a(x10), .b(x00), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x01), .O(new_n116_));
  oai21  g73(.a(x19), .b(x00), .c(new_n45_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(z15));
  nand2  g75(.a(new_n54_), .b(x02), .O(new_n120_));
  inv1   g76(.a(new_n120_), .O(z17));
  zero   g77(.O(z16));
endmodule


