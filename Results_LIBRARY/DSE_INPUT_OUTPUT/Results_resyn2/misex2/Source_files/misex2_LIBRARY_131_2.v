// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n73_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nor2   g06(.a(x09), .b(x00), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x02), .O(z00));
  inv1   g09(.a(x02), .O(new_n53_));
  nor2   g10(.a(x01), .b(x00), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n48_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  nand2  g16(.a(new_n54_), .b(new_n53_), .O(new_n60_));
  nand3  g17(.a(new_n47_), .b(x18), .c(new_n45_), .O(new_n61_));
  nand3  g18(.a(x12), .b(x11), .c(x10), .O(new_n62_));
  inv1   g19(.a(x09), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x02), .c(x01), .d(x00), .O(new_n64_));
  oai22  g21(.a(new_n64_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  nor2   g23(.a(x12), .b(new_n57_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(new_n63_), .d(x00), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(x02), .c(new_n44_), .O(z04));
  inv1   g26(.a(x00), .O(new_n70_));
  nand3  g27(.a(x10), .b(x09), .c(x02), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n44_), .c(new_n70_), .O(z05));
  nand4  g29(.a(x11), .b(x10), .c(new_n63_), .d(x00), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x02), .c(new_n44_), .O(z06));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  nand3  g32(.a(new_n67_), .b(x11), .c(new_n63_), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x01), .c(new_n75_), .O(z07));
  nand2  g34(.a(new_n53_), .b(x01), .O(new_n78_));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nand2  g39(.a(new_n54_), .b(x02), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(z17));
  nand4  g41(.a(z17), .b(x19), .c(new_n46_), .d(x17), .O(new_n85_));
  oai21  g42(.a(new_n85_), .b(new_n82_), .c(new_n78_), .O(z08));
  inv1   g43(.a(x20), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n47_), .c(x18), .d(x01), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  nand2  g46(.a(x02), .b(new_n44_), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n66_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand4  g53(.a(x20), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  aoi21  g55(.a(new_n98_), .b(new_n93_), .c(new_n89_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n70_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n99_), .c(new_n78_), .O(z09));
  nor2   g60(.a(new_n88_), .b(new_n100_), .O(new_n104_));
  nand4  g61(.a(new_n100_), .b(x20), .c(x15), .d(new_n94_), .O(new_n105_));
  nand2  g62(.a(new_n101_), .b(x16), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi22  g64(.a(new_n107_), .b(new_n93_), .c(new_n104_), .d(x22), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(x00), .c(new_n78_), .O(z10));
  nand2  g66(.a(new_n101_), .b(new_n70_), .O(new_n110_));
  nor2   g67(.a(new_n105_), .b(x16), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n93_), .c(new_n104_), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(new_n110_), .c(new_n78_), .O(z11));
  inv1   g70(.a(x23), .O(new_n114_));
  inv1   g71(.a(new_n54_), .O(new_n115_));
  nand2  g72(.a(new_n57_), .b(x01), .O(new_n116_));
  oai22  g73(.a(new_n116_), .b(new_n70_), .c(new_n115_), .d(new_n114_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x02), .O(new_n118_));
  nand3  g75(.a(new_n47_), .b(x17), .c(new_n53_), .O(new_n119_));
  oai21  g76(.a(new_n114_), .b(new_n47_), .c(new_n119_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n54_), .O(new_n121_));
  inv1   g78(.a(x24), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x09), .O(new_n123_));
  aoi21  g80(.a(new_n121_), .b(new_n118_), .c(new_n123_), .O(z12));
  nor2   g81(.a(new_n119_), .b(new_n115_), .O(z13));
  inv1   g82(.a(new_n58_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n50_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(new_n44_), .c(x02), .O(z14));
  inv1   g85(.a(new_n78_), .O(new_n129_));
  nand2  g86(.a(new_n116_), .b(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand2  g88(.a(x19), .b(new_n53_), .O(new_n132_));
  aoi21  g89(.a(new_n132_), .b(new_n131_), .c(new_n129_), .O(z15));
  aoi21  g90(.a(x02), .b(x00), .c(new_n44_), .O(z16));
endmodule


