// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n79_, new_n80_, new_n82_, new_n83_;
  nor2   g00(.a(x01), .b(x00), .O(z02));
  inv1   g01(.a(x12), .O(new_n47_));
  nand3  g02(.a(x02), .b(x01), .c(x00), .O(new_n48_));
  inv1   g03(.a(x09), .O(new_n49_));
  nand3  g04(.a(x11), .b(x10), .c(new_n49_), .O(new_n50_));
  nor3   g05(.a(new_n50_), .b(new_n48_), .c(new_n47_), .O(z03));
  inv1   g06(.a(z02), .O(new_n52_));
  inv1   g07(.a(x11), .O(new_n53_));
  nand4  g08(.a(new_n47_), .b(new_n53_), .c(x10), .d(new_n49_), .O(new_n54_));
  oai21  g09(.a(new_n54_), .b(new_n48_), .c(new_n52_), .O(z04));
  nand2  g10(.a(x10), .b(x02), .O(new_n56_));
  and2   g11(.a(x01), .b(x00), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(x09), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n56_), .O(z05));
  oai21  g14(.a(new_n50_), .b(new_n48_), .c(new_n52_), .O(z06));
  inv1   g15(.a(x01), .O(new_n61_));
  inv1   g16(.a(x02), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(x00), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand3  g19(.a(new_n47_), .b(x02), .c(x00), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(new_n50_), .c(new_n64_), .O(z07));
  inv1   g21(.a(x20), .O(new_n67_));
  nor2   g22(.a(new_n61_), .b(x00), .O(z16));
  inv1   g23(.a(x18), .O(new_n69_));
  nor2   g24(.a(x19), .b(new_n69_), .O(new_n70_));
  nand3  g25(.a(new_n70_), .b(z16), .c(new_n67_), .O(new_n71_));
  inv1   g26(.a(x21), .O(new_n72_));
  inv1   g27(.a(x22), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n71_), .O(z09));
  nand4  g30(.a(new_n70_), .b(z16), .c(x21), .d(new_n67_), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n73_), .O(z10));
  nor2   g32(.a(new_n76_), .b(x22), .O(z11));
  inv1   g33(.a(x24), .O(new_n79_));
  nand2  g34(.a(new_n56_), .b(new_n79_), .O(new_n80_));
  nor2   g35(.a(new_n80_), .b(new_n58_), .O(z12));
  inv1   g36(.a(x10), .O(new_n82_));
  nand2  g37(.a(new_n57_), .b(new_n82_), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n63_), .c(new_n52_), .O(z15));
  zero   g39(.O(z00));
  zero   g40(.O(z01));
  nor2   g41(.a(x01), .b(x00), .O(z08));
  nor2   g42(.a(x01), .b(x00), .O(z13));
  nor2   g43(.a(x01), .b(x00), .O(z14));
  nor2   g44(.a(x01), .b(x00), .O(z17));
endmodule


