// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x19), .O(new_n44_));
  nor2   g01(.a(x21), .b(x14), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n48_), .O(z01));
  nand3  g12(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n56_));
  nor3   g13(.a(new_n54_), .b(new_n56_), .c(x10), .O(z02));
  inv1   g14(.a(x17), .O(new_n58_));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n51_), .c(x18), .d(new_n58_), .O(new_n60_));
  inv1   g17(.a(x10), .O(new_n61_));
  inv1   g18(.a(x11), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nor2   g20(.a(x09), .b(new_n49_), .O(new_n64_));
  and2   g21(.a(x01), .b(x00), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(x12), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(new_n60_), .c(new_n45_), .O(z03));
  nand3  g24(.a(new_n65_), .b(new_n64_), .c(new_n46_), .O(new_n68_));
  nand2  g25(.a(new_n62_), .b(x10), .O(new_n69_));
  nor3   g26(.a(new_n69_), .b(new_n68_), .c(x12), .O(z04));
  nand2  g27(.a(x10), .b(x02), .O(new_n71_));
  nand2  g28(.a(new_n65_), .b(x09), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n71_), .c(new_n46_), .O(z05));
  inv1   g30(.a(new_n68_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n63_), .O(new_n75_));
  inv1   g32(.a(new_n75_), .O(z06));
  inv1   g33(.a(x01), .O(new_n77_));
  nor2   g34(.a(x12), .b(x09), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(new_n63_), .c(new_n77_), .O(new_n79_));
  nand2  g36(.a(x02), .b(x00), .O(new_n80_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n46_), .O(z07));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  inv1   g40(.a(x04), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(x03), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n83_), .c(new_n82_), .d(new_n46_), .O(new_n86_));
  nand2  g43(.a(new_n51_), .b(x02), .O(new_n87_));
  inv1   g44(.a(x18), .O(new_n88_));
  nand3  g45(.a(x19), .b(new_n88_), .c(x17), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n87_), .c(new_n86_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nor2   g48(.a(new_n77_), .b(x00), .O(new_n92_));
  nor2   g49(.a(x19), .b(new_n88_), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(x14), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n94_), .O(z09));
  nand4  g55(.a(new_n93_), .b(new_n92_), .c(x21), .d(new_n91_), .O(new_n99_));
  oai21  g56(.a(new_n99_), .b(new_n96_), .c(new_n46_), .O(z10));
  oai21  g57(.a(new_n99_), .b(x22), .c(new_n46_), .O(z11));
  nand2  g58(.a(new_n71_), .b(new_n65_), .O(new_n102_));
  nand2  g59(.a(new_n59_), .b(new_n58_), .O(new_n103_));
  inv1   g60(.a(x23), .O(new_n104_));
  oai21  g61(.a(x19), .b(x02), .c(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n103_), .c(new_n51_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n102_), .O(new_n107_));
  nor2   g64(.a(x24), .b(new_n50_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n46_), .O(z12));
  nand3  g67(.a(new_n59_), .b(new_n51_), .c(x17), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n46_), .O(z13));
  nand3  g69(.a(new_n47_), .b(new_n44_), .c(new_n61_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n52_), .c(new_n46_), .O(z14));
  oai21  g71(.a(x10), .b(new_n77_), .c(x02), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x00), .O(new_n116_));
  nand3  g73(.a(x19), .b(new_n49_), .c(new_n77_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n116_), .c(new_n45_), .O(z15));
  nor3   g75(.a(new_n45_), .b(new_n77_), .c(x00), .O(z16));
  nand2  g76(.a(new_n87_), .b(new_n46_), .O(z17));
endmodule


