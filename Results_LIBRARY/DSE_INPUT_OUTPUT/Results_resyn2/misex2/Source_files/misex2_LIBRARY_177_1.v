// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n130_;
  inv1   g00(.a(x16), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x10), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z00));
  nor2   g09(.a(x02), .b(x00), .O(new_n53_));
  nor2   g10(.a(x16), .b(x01), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand2  g13(.a(new_n56_), .b(x09), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n49_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  inv1   g18(.a(x09), .O(new_n62_));
  nand3  g19(.a(x11), .b(x10), .c(new_n62_), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  inv1   g21(.a(x00), .O(new_n65_));
  inv1   g22(.a(x01), .O(new_n66_));
  nor2   g23(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n64_), .c(x12), .d(x02), .O(new_n68_));
  nand4  g25(.a(new_n56_), .b(new_n48_), .c(x18), .d(new_n46_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n68_), .O(z03));
  nand2  g27(.a(new_n67_), .b(x02), .O(new_n71_));
  nand2  g28(.a(x16), .b(new_n66_), .O(new_n72_));
  inv1   g29(.a(x11), .O(new_n73_));
  inv1   g30(.a(x12), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(new_n73_), .c(x10), .d(new_n62_), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n71_), .c(new_n72_), .O(z04));
  nand2  g33(.a(x10), .b(x02), .O(new_n77_));
  nand2  g34(.a(new_n67_), .b(x09), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(new_n77_), .c(new_n72_), .O(z05));
  nor2   g36(.a(new_n71_), .b(new_n63_), .O(z06));
  oai21  g37(.a(new_n63_), .b(x12), .c(x01), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n72_), .O(z07));
  nand3  g40(.a(x19), .b(new_n47_), .c(x17), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nor2   g42(.a(x06), .b(x05), .O(new_n86_));
  nor2   g43(.a(x08), .b(x07), .O(new_n87_));
  inv1   g44(.a(x03), .O(new_n88_));
  nand4  g45(.a(x04), .b(new_n88_), .c(x02), .d(new_n65_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n87_), .c(new_n86_), .d(new_n85_), .O(new_n91_));
  aoi21  g48(.a(new_n91_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g49(.a(x21), .O(new_n93_));
  nor2   g50(.a(x20), .b(x19), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x18), .c(x01), .O(new_n95_));
  nand4  g52(.a(x12), .b(new_n73_), .c(x02), .d(new_n66_), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  nor2   g54(.a(x14), .b(x13), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(x20), .c(new_n97_), .O(new_n99_));
  oai21  g56(.a(new_n99_), .b(new_n96_), .c(new_n95_), .O(new_n100_));
  nor2   g57(.a(x22), .b(x00), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n93_), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n72_), .O(z09));
  nand4  g60(.a(new_n94_), .b(x21), .c(x18), .d(x01), .O(new_n104_));
  nand2  g61(.a(x22), .b(new_n65_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n104_), .c(new_n72_), .O(z10));
  nand4  g63(.a(new_n98_), .b(new_n93_), .c(x20), .d(x15), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n96_), .c(new_n104_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n101_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n72_), .O(z11));
  nand3  g67(.a(new_n77_), .b(x01), .c(x00), .O(new_n111_));
  nor3   g68(.a(x19), .b(x17), .c(x02), .O(new_n112_));
  nor2   g69(.a(x19), .b(x02), .O(new_n113_));
  nor2   g70(.a(x01), .b(x00), .O(new_n114_));
  oai21  g71(.a(new_n113_), .b(x23), .c(new_n114_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n112_), .c(new_n111_), .O(new_n116_));
  nor2   g73(.a(x24), .b(new_n62_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n72_), .O(z12));
  nand3  g76(.a(new_n53_), .b(new_n48_), .c(x17), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n44_), .c(x01), .O(z13));
  inv1   g78(.a(new_n60_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n45_), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n44_), .c(x01), .O(z14));
  inv1   g81(.a(x02), .O(new_n125_));
  aoi21  g82(.a(new_n59_), .b(x01), .c(new_n125_), .O(new_n126_));
  aoi21  g83(.a(x19), .b(new_n125_), .c(x16), .O(new_n127_));
  oai22  g84(.a(new_n127_), .b(x01), .c(new_n126_), .d(new_n65_), .O(z15));
  nor2   g85(.a(new_n67_), .b(new_n54_), .O(z16));
  nand2  g86(.a(x02), .b(new_n65_), .O(new_n130_));
  aoi21  g87(.a(new_n130_), .b(new_n44_), .c(x01), .O(z17));
endmodule


