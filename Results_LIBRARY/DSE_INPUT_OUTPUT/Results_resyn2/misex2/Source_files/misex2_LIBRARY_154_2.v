// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n124_,
    new_n126_, new_n127_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x10), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(x09), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nor2   g04(.a(x19), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z00));
  nand2  g09(.a(x10), .b(x09), .O(new_n53_));
  nor2   g10(.a(x01), .b(x00), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z01));
  nand4  g13(.a(new_n48_), .b(new_n47_), .c(new_n45_), .d(x09), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g15(.a(x12), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(new_n46_), .c(x11), .O(new_n62_));
  inv1   g19(.a(x17), .O(new_n63_));
  nand4  g20(.a(new_n54_), .b(new_n48_), .c(x18), .d(new_n63_), .O(new_n64_));
  oai21  g21(.a(new_n62_), .b(new_n59_), .c(new_n64_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  nand4  g23(.a(new_n46_), .b(new_n59_), .c(new_n66_), .d(x02), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(x01), .c(new_n44_), .O(z04));
  nor2   g25(.a(new_n60_), .b(new_n53_), .O(z05));
  inv1   g26(.a(new_n62_), .O(z06));
  nor2   g27(.a(new_n62_), .b(x12), .O(z07));
  nor2   g28(.a(new_n63_), .b(x08), .O(new_n72_));
  inv1   g29(.a(x19), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(x18), .O(new_n74_));
  inv1   g31(.a(x02), .O(new_n75_));
  nor2   g32(.a(x03), .b(new_n75_), .O(new_n76_));
  inv1   g33(.a(x05), .O(new_n77_));
  nor2   g34(.a(x07), .b(x06), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(new_n77_), .c(x04), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n76_), .c(new_n74_), .d(new_n72_), .O(new_n81_));
  aoi21  g38(.a(new_n81_), .b(new_n44_), .c(x01), .O(z08));
  nand2  g39(.a(new_n66_), .b(x02), .O(new_n83_));
  inv1   g40(.a(x13), .O(new_n84_));
  inv1   g41(.a(x14), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(new_n84_), .c(x12), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  inv1   g44(.a(x21), .O(new_n88_));
  inv1   g45(.a(x22), .O(new_n89_));
  nand3  g46(.a(new_n89_), .b(new_n88_), .c(x20), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nand2  g49(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  aoi21  g51(.a(new_n94_), .b(new_n87_), .c(x00), .O(new_n95_));
  nand3  g52(.a(x18), .b(x01), .c(new_n44_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nor2   g54(.a(x20), .b(x19), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n97_), .c(new_n89_), .d(new_n88_), .O(new_n99_));
  oai21  g56(.a(new_n95_), .b(x01), .c(new_n99_), .O(z09));
  nand2  g57(.a(x16), .b(x15), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(new_n90_), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n87_), .c(x00), .O(new_n103_));
  nand2  g60(.a(new_n98_), .b(x21), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n97_), .c(x22), .O(new_n106_));
  oai21  g63(.a(new_n103_), .b(x01), .c(new_n106_), .O(z10));
  nand2  g64(.a(new_n92_), .b(x15), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n90_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n87_), .c(x00), .O(new_n110_));
  nand3  g67(.a(new_n105_), .b(new_n97_), .c(new_n89_), .O(new_n111_));
  oai21  g68(.a(new_n110_), .b(x01), .c(new_n111_), .O(z11));
  aoi21  g69(.a(x10), .b(x02), .c(new_n44_), .O(new_n113_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  nand2  g71(.a(new_n48_), .b(x17), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n114_), .c(x01), .O(new_n116_));
  inv1   g73(.a(x09), .O(new_n117_));
  nor2   g74(.a(x24), .b(new_n117_), .O(new_n118_));
  oai21  g75(.a(new_n116_), .b(new_n113_), .c(new_n118_), .O(new_n119_));
  inv1   g76(.a(x01), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x00), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n119_), .O(z12));
  aoi21  g79(.a(new_n115_), .b(new_n44_), .c(x01), .O(z13));
  nand4  g80(.a(new_n54_), .b(new_n50_), .c(new_n45_), .d(new_n117_), .O(new_n124_));
  inv1   g81(.a(new_n124_), .O(z14));
  aoi21  g82(.a(x19), .b(new_n75_), .c(x00), .O(new_n126_));
  nor2   g83(.a(new_n113_), .b(new_n120_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(new_n126_), .O(z15));
  nor2   g85(.a(new_n120_), .b(x00), .O(z16));
  aoi21  g86(.a(new_n75_), .b(new_n44_), .c(x01), .O(z17));
endmodule


