// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x01), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n49_), .O(z00));
  nand2  g11(.a(new_n48_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z01));
  nand2  g13(.a(x15), .b(x01), .O(new_n57_));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n55_), .c(new_n57_), .O(z02));
  inv1   g17(.a(x15), .O(new_n61_));
  inv1   g18(.a(x02), .O(new_n62_));
  nor2   g19(.a(x09), .b(new_n62_), .O(new_n63_));
  nand4  g20(.a(x11), .b(x10), .c(x01), .d(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n63_), .c(new_n61_), .d(x12), .O(new_n66_));
  nand2  g23(.a(new_n48_), .b(new_n52_), .O(new_n67_));
  nand2  g24(.a(x18), .b(new_n50_), .O(new_n68_));
  oai21  g25(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  nor2   g27(.a(x12), .b(x09), .O(new_n71_));
  nor2   g28(.a(new_n58_), .b(new_n62_), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(x00), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(new_n61_), .c(new_n46_), .O(z04));
  nand4  g31(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(new_n61_), .c(new_n46_), .O(z05));
  nand2  g33(.a(new_n63_), .b(new_n61_), .O(new_n77_));
  nor2   g34(.a(new_n64_), .b(new_n77_), .O(z06));
  nand2  g35(.a(x02), .b(x00), .O(new_n79_));
  nor2   g36(.a(new_n70_), .b(new_n58_), .O(new_n80_));
  aoi21  g37(.a(new_n71_), .b(new_n80_), .c(new_n46_), .O(new_n81_));
  oai21  g38(.a(new_n81_), .b(new_n79_), .c(new_n57_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nand3  g43(.a(x02), .b(new_n46_), .c(new_n45_), .O(new_n87_));
  nand3  g44(.a(x19), .b(new_n51_), .c(x17), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n52_), .c(x18), .d(x01), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n46_), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n70_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  inv1   g53(.a(x14), .O(new_n97_));
  inv1   g54(.a(x16), .O(new_n98_));
  nand4  g55(.a(x20), .b(new_n98_), .c(new_n61_), .d(new_n97_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  aoi21  g57(.a(new_n100_), .b(new_n96_), .c(new_n92_), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n45_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n101_), .c(new_n57_), .O(z09));
  nand4  g62(.a(new_n92_), .b(x22), .c(x21), .d(new_n61_), .O(new_n106_));
  nor2   g63(.a(new_n61_), .b(x14), .O(new_n107_));
  nand4  g64(.a(new_n103_), .b(new_n102_), .c(x20), .d(x16), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n107_), .c(new_n96_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n106_), .c(x00), .O(z10));
  nand2  g68(.a(new_n103_), .b(new_n45_), .O(new_n112_));
  nand3  g69(.a(new_n92_), .b(x21), .c(new_n61_), .O(new_n113_));
  nor2   g70(.a(new_n90_), .b(x16), .O(new_n114_));
  nand4  g71(.a(new_n107_), .b(new_n114_), .c(new_n96_), .d(new_n102_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(z11));
  nand2  g73(.a(x01), .b(x00), .O(new_n117_));
  nor2   g74(.a(new_n72_), .b(new_n117_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n52_), .b(x17), .c(new_n62_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(new_n47_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n44_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n57_), .O(z12));
  oai21  g81(.a(new_n67_), .b(new_n50_), .c(new_n57_), .O(z13));
  nor2   g82(.a(new_n59_), .b(new_n49_), .O(z14));
  oai21  g83(.a(x10), .b(new_n46_), .c(x02), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x00), .O(new_n128_));
  nand3  g85(.a(x19), .b(new_n62_), .c(new_n46_), .O(new_n129_));
  nand3  g86(.a(new_n129_), .b(new_n128_), .c(new_n57_), .O(z15));
  aoi21  g87(.a(new_n61_), .b(x00), .c(new_n46_), .O(z16));
  nand2  g88(.a(new_n87_), .b(new_n57_), .O(z17));
endmodule


