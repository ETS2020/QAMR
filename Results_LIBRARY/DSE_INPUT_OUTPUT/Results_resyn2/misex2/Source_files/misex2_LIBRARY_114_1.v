// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n119_,
    new_n120_, new_n123_;
  inv1   g00(.a(x02), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  nor3   g04(.a(x20), .b(x19), .c(x18), .O(new_n48_));
  inv1   g05(.a(x10), .O(new_n49_));
  nor2   g06(.a(x17), .b(new_n49_), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n47_), .O(z00));
  nand3  g09(.a(new_n46_), .b(x09), .c(new_n44_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n51_), .O(z01));
  nor2   g11(.a(x17), .b(x10), .O(new_n55_));
  nand2  g12(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n53_), .O(z02));
  nor2   g14(.a(new_n49_), .b(x09), .O(new_n58_));
  nand3  g15(.a(x02), .b(x01), .c(x00), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n58_), .c(x12), .d(x11), .O(new_n61_));
  inv1   g18(.a(x20), .O(new_n62_));
  inv1   g19(.a(x18), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(x17), .O(new_n64_));
  nor2   g21(.a(x19), .b(x02), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(new_n46_), .d(new_n62_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n61_), .O(z03));
  nand2  g24(.a(new_n60_), .b(new_n58_), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(x12), .c(x11), .O(z04));
  inv1   g26(.a(x01), .O(new_n70_));
  nand2  g27(.a(x20), .b(new_n70_), .O(new_n71_));
  nand2  g28(.a(x01), .b(x00), .O(new_n72_));
  nand3  g29(.a(x10), .b(x09), .c(x02), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(z05));
  inv1   g31(.a(x11), .O(new_n75_));
  nor2   g32(.a(new_n68_), .b(new_n75_), .O(z06));
  inv1   g33(.a(x12), .O(new_n77_));
  nand4  g34(.a(new_n77_), .b(x11), .c(x10), .d(new_n45_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n71_), .O(z07));
  nand3  g38(.a(x19), .b(new_n63_), .c(x17), .O(new_n82_));
  inv1   g39(.a(new_n82_), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  inv1   g41(.a(x06), .O(new_n85_));
  inv1   g42(.a(x07), .O(new_n86_));
  inv1   g43(.a(x08), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(new_n84_), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  inv1   g46(.a(x00), .O(new_n90_));
  inv1   g47(.a(x03), .O(new_n91_));
  nand4  g48(.a(x04), .b(new_n91_), .c(x02), .d(new_n90_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n89_), .c(new_n83_), .O(new_n94_));
  aoi21  g51(.a(new_n94_), .b(new_n62_), .c(x01), .O(z08));
  nand3  g52(.a(x18), .b(x01), .c(new_n90_), .O(new_n96_));
  inv1   g53(.a(x19), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n62_), .c(new_n97_), .O(new_n99_));
  nor3   g56(.a(new_n99_), .b(new_n96_), .c(x21), .O(z09));
  inv1   g57(.a(new_n96_), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(x21), .O(new_n102_));
  nand3  g59(.a(x22), .b(new_n62_), .c(new_n97_), .O(new_n103_));
  oai21  g60(.a(new_n103_), .b(new_n102_), .c(new_n71_), .O(z10));
  oai21  g61(.a(new_n102_), .b(new_n99_), .c(new_n71_), .O(z11));
  nand2  g62(.a(x10), .b(x02), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(x01), .c(x00), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g65(.a(new_n97_), .b(x17), .c(new_n44_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nor3   g67(.a(x20), .b(x01), .c(x00), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  inv1   g69(.a(x24), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(x09), .O(new_n114_));
  aoi21  g71(.a(new_n112_), .b(new_n107_), .c(new_n114_), .O(z12));
  nand3  g72(.a(new_n65_), .b(x17), .c(new_n90_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n62_), .c(x01), .O(z13));
  nor2   g74(.a(new_n56_), .b(new_n47_), .O(z14));
  aoi21  g75(.a(x19), .b(new_n70_), .c(x00), .O(new_n119_));
  nand2  g76(.a(new_n62_), .b(new_n44_), .O(new_n120_));
  oai21  g77(.a(new_n120_), .b(new_n119_), .c(new_n107_), .O(z15));
  oai21  g78(.a(new_n70_), .b(x00), .c(new_n71_), .O(z16));
  nand2  g79(.a(new_n111_), .b(x02), .O(new_n123_));
  inv1   g80(.a(new_n123_), .O(z17));
endmodule


