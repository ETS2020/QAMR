// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n127_, new_n128_, new_n131_;
  inv1   g00(.a(x16), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(x10), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nor2   g06(.a(x02), .b(x00), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z00));
  nand3  g09(.a(new_n50_), .b(new_n49_), .c(x09), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand3  g12(.a(new_n47_), .b(new_n46_), .c(new_n55_), .O(new_n56_));
  inv1   g13(.a(new_n56_), .O(new_n57_));
  nand3  g14(.a(new_n57_), .b(new_n50_), .c(x09), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g16(.a(x12), .O(new_n60_));
  nand4  g17(.a(new_n45_), .b(x02), .c(x01), .d(x00), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x11), .c(x10), .O(new_n63_));
  inv1   g20(.a(x01), .O(new_n64_));
  nand3  g21(.a(new_n50_), .b(new_n44_), .c(new_n64_), .O(new_n65_));
  inv1   g22(.a(x19), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(x18), .c(new_n46_), .O(new_n67_));
  oai22  g24(.a(new_n67_), .b(new_n65_), .c(new_n63_), .d(new_n60_), .O(z03));
  nand2  g25(.a(x16), .b(new_n64_), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  nor2   g27(.a(x12), .b(new_n55_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n61_), .c(new_n69_), .O(z04));
  nand2  g30(.a(x01), .b(x00), .O(new_n74_));
  nand3  g31(.a(x10), .b(x09), .c(x02), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n69_), .O(z05));
  nand2  g33(.a(new_n69_), .b(new_n63_), .O(z06));
  nand2  g34(.a(x02), .b(x00), .O(new_n78_));
  nand2  g35(.a(new_n44_), .b(new_n64_), .O(new_n79_));
  nand4  g36(.a(new_n71_), .b(x11), .c(new_n45_), .d(x01), .O(new_n80_));
  aoi21  g37(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(new_n46_), .b(x16), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nor2   g42(.a(x01), .b(x00), .O(new_n86_));
  nor2   g43(.a(x08), .b(x07), .O(new_n87_));
  nor2   g44(.a(new_n66_), .b(x18), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x02), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n85_), .O(z08));
  inv1   g47(.a(x02), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(x01), .O(new_n92_));
  nor2   g49(.a(new_n60_), .b(x11), .O(new_n93_));
  nor2   g50(.a(x14), .b(x13), .O(new_n94_));
  inv1   g51(.a(x20), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(x15), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n94_), .c(new_n93_), .d(new_n92_), .O(new_n97_));
  nand4  g54(.a(new_n95_), .b(new_n66_), .c(x18), .d(x01), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  inv1   g56(.a(x00), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(x21), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n99_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n69_), .O(z09));
  inv1   g62(.a(new_n98_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(x21), .O(new_n107_));
  nand2  g64(.a(x22), .b(new_n100_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(new_n107_), .c(new_n69_), .O(z10));
  inv1   g66(.a(x21), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(x20), .c(new_n44_), .d(x15), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n94_), .c(new_n93_), .d(new_n92_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n107_), .c(new_n102_), .O(z11));
  nand2  g71(.a(x10), .b(x02), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(x01), .c(x00), .O(new_n116_));
  aoi21  g73(.a(new_n66_), .b(new_n91_), .c(x23), .O(new_n117_));
  nand3  g74(.a(new_n66_), .b(new_n46_), .c(new_n91_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n86_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n117_), .c(new_n116_), .O(new_n120_));
  nor2   g77(.a(x24), .b(new_n45_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n69_), .O(z12));
  nand2  g80(.a(new_n86_), .b(new_n83_), .O(new_n124_));
  nor3   g81(.a(new_n124_), .b(x19), .c(x02), .O(z13));
  nor3   g82(.a(new_n65_), .b(new_n56_), .c(x09), .O(z14));
  aoi21  g83(.a(x19), .b(new_n64_), .c(x00), .O(new_n127_));
  nand2  g84(.a(new_n44_), .b(new_n91_), .O(new_n128_));
  oai21  g85(.a(new_n128_), .b(new_n127_), .c(new_n116_), .O(z15));
  nor2   g86(.a(new_n64_), .b(x00), .O(z16));
  nand3  g87(.a(new_n86_), .b(new_n44_), .c(x02), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z17));
endmodule


