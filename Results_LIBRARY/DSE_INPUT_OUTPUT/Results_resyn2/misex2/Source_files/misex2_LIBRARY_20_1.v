// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n131_,
    new_n132_, new_n133_;
  inv1   g00(.a(x11), .O(new_n44_));
  nand2  g01(.a(x21), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g11(.a(new_n45_), .O(new_n55_));
  nand3  g12(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n56_));
  nor3   g13(.a(new_n56_), .b(new_n53_), .c(new_n55_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n56_), .c(new_n45_), .O(z02));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x10), .c(new_n47_), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n44_), .O(z06));
  nand2  g21(.a(z06), .b(x12), .O(new_n65_));
  nor2   g22(.a(x17), .b(x02), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n48_), .c(new_n52_), .d(x18), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n65_), .c(new_n45_), .O(z03));
  nor4   g25(.a(new_n63_), .b(x21), .c(x12), .d(x11), .O(z04));
  nand2  g26(.a(x10), .b(x02), .O(new_n70_));
  nand3  g27(.a(x09), .b(x01), .c(x00), .O(new_n71_));
  oai21  g28(.a(new_n71_), .b(new_n70_), .c(new_n45_), .O(z05));
  inv1   g29(.a(x12), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(x11), .c(x10), .d(new_n47_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n45_), .O(z07));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  inv1   g36(.a(x04), .O(new_n80_));
  nor2   g37(.a(new_n80_), .b(x03), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n79_), .c(new_n78_), .d(new_n45_), .O(new_n82_));
  inv1   g39(.a(x00), .O(new_n83_));
  inv1   g40(.a(x01), .O(new_n84_));
  nand3  g41(.a(x02), .b(new_n84_), .c(new_n83_), .O(new_n85_));
  nand3  g42(.a(x19), .b(new_n51_), .c(x17), .O(new_n86_));
  nor3   g43(.a(new_n86_), .b(new_n85_), .c(new_n82_), .O(z08));
  inv1   g44(.a(x22), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(new_n83_), .O(new_n89_));
  nand2  g46(.a(x02), .b(new_n84_), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  inv1   g48(.a(x14), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(x12), .d(new_n44_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand3  g53(.a(x20), .b(new_n96_), .c(new_n95_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  inv1   g55(.a(x20), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n52_), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(x18), .c(x01), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  aoi21  g60(.a(new_n98_), .b(new_n94_), .c(new_n103_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n89_), .c(new_n45_), .O(z09));
  nand4  g62(.a(new_n88_), .b(x20), .c(x16), .d(x15), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand2  g64(.a(new_n52_), .b(x18), .O(new_n108_));
  nand4  g65(.a(x22), .b(x21), .c(new_n99_), .d(x01), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g67(.a(new_n107_), .b(new_n94_), .c(new_n110_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(x00), .c(new_n45_), .O(z10));
  nand3  g69(.a(x20), .b(new_n96_), .c(x15), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(new_n114_));
  nand3  g71(.a(x21), .b(x18), .c(x01), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n100_), .O(new_n116_));
  aoi21  g73(.a(new_n114_), .b(new_n94_), .c(new_n116_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n89_), .c(new_n45_), .O(z11));
  nand3  g75(.a(new_n70_), .b(x01), .c(x00), .O(new_n119_));
  inv1   g76(.a(x23), .O(new_n120_));
  nor2   g77(.a(x19), .b(x02), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x17), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n120_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n48_), .O(new_n124_));
  inv1   g81(.a(x24), .O(new_n125_));
  nand3  g82(.a(new_n45_), .b(new_n125_), .c(x09), .O(new_n126_));
  aoi21  g83(.a(new_n124_), .b(new_n119_), .c(new_n126_), .O(z12));
  inv1   g84(.a(new_n48_), .O(new_n128_));
  oai21  g85(.a(new_n122_), .b(new_n128_), .c(new_n45_), .O(z13));
  oai21  g86(.a(new_n59_), .b(new_n49_), .c(new_n45_), .O(z14));
  oai21  g87(.a(x10), .b(new_n84_), .c(x02), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x00), .O(new_n132_));
  nand3  g89(.a(x19), .b(new_n46_), .c(new_n84_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(new_n132_), .c(new_n55_), .O(z15));
  oai21  g91(.a(new_n84_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g92(.a(new_n85_), .b(new_n45_), .O(z17));
endmodule


