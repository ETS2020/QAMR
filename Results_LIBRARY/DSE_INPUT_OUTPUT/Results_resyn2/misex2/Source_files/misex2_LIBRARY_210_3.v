// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n130_;
  nand2  g00(.a(x19), .b(x01), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  inv1   g08(.a(x18), .O(new_n52_));
  inv1   g09(.a(x19), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n50_), .c(new_n44_), .O(z00));
  nand2  g12(.a(new_n49_), .b(x09), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n54_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n56_), .O(z02));
  nand3  g17(.a(new_n49_), .b(x18), .c(new_n51_), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n45_), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n63_), .c(x12), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(new_n61_), .c(x19), .O(z03));
  nand2  g24(.a(new_n65_), .b(new_n53_), .O(new_n68_));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(x10), .d(new_n45_), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n68_), .O(z04));
  nor3   g29(.a(new_n68_), .b(new_n58_), .c(new_n45_), .O(z05));
  nor2   g30(.a(new_n68_), .b(new_n62_), .O(z06));
  nand3  g31(.a(new_n63_), .b(new_n53_), .c(new_n70_), .O(new_n75_));
  nand2  g32(.a(x02), .b(x00), .O(new_n76_));
  aoi21  g33(.a(new_n75_), .b(x01), .c(new_n76_), .O(z07));
  inv1   g34(.a(x01), .O(new_n78_));
  inv1   g35(.a(x00), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(new_n52_), .c(x17), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x04), .c(new_n83_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n82_), .c(x02), .d(new_n79_), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(new_n78_), .c(new_n53_), .O(z08));
  nor3   g45(.a(x20), .b(new_n52_), .c(new_n78_), .O(new_n89_));
  nand2  g46(.a(x02), .b(new_n78_), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n69_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand4  g53(.a(x20), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  aoi21  g55(.a(new_n98_), .b(new_n93_), .c(new_n89_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n79_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n99_), .c(new_n44_), .O(z09));
  inv1   g60(.a(x20), .O(new_n104_));
  nand4  g61(.a(x21), .b(new_n104_), .c(x18), .d(x01), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(x22), .c(new_n53_), .O(new_n107_));
  nand2  g64(.a(x15), .b(new_n94_), .O(new_n108_));
  nand4  g65(.a(new_n101_), .b(new_n100_), .c(x20), .d(x16), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n93_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n107_), .c(x00), .O(z10));
  nand2  g69(.a(new_n101_), .b(new_n79_), .O(new_n113_));
  nand3  g70(.a(new_n100_), .b(x20), .c(new_n96_), .O(new_n114_));
  nor2   g71(.a(new_n114_), .b(new_n108_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n93_), .c(new_n106_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n113_), .c(new_n44_), .O(z11));
  nand2  g74(.a(x10), .b(x02), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(x01), .c(x00), .O(new_n119_));
  oai21  g76(.a(new_n48_), .b(new_n51_), .c(new_n119_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n53_), .O(new_n121_));
  nand2  g78(.a(new_n53_), .b(new_n46_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n47_), .c(x23), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n121_), .c(new_n125_), .O(z12));
  nand3  g83(.a(new_n49_), .b(new_n53_), .c(x17), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n44_), .O(z13));
  oai21  g85(.a(new_n59_), .b(new_n50_), .c(new_n44_), .O(z14));
  nand2  g86(.a(new_n118_), .b(x00), .O(new_n130_));
  aoi22  g87(.a(new_n130_), .b(new_n53_), .c(x02), .d(new_n78_), .O(z15));
  aoi21  g88(.a(new_n53_), .b(x00), .c(new_n78_), .O(z16));
  oai21  g89(.a(new_n90_), .b(x00), .c(new_n44_), .O(z17));
endmodule


