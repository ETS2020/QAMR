// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n129_, new_n130_, new_n131_,
    new_n133_;
  inv1   g00(.a(x16), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x10), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand2  g13(.a(new_n54_), .b(x09), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n49_), .c(new_n45_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  inv1   g18(.a(x12), .O(new_n62_));
  nand3  g19(.a(x10), .b(x02), .c(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(x11), .c(new_n50_), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n62_), .c(x16), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(x01), .O(new_n67_));
  nand4  g24(.a(new_n54_), .b(new_n48_), .c(x18), .d(new_n46_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n67_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x02), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n51_), .O(new_n72_));
  nor2   g29(.a(x12), .b(new_n59_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n72_), .c(new_n70_), .d(new_n50_), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x16), .c(new_n52_), .O(z04));
  nor4   g32(.a(new_n63_), .b(new_n44_), .c(new_n50_), .d(new_n52_), .O(z05));
  aoi21  g33(.a(new_n65_), .b(x16), .c(new_n52_), .O(z06));
  nand3  g34(.a(new_n73_), .b(x11), .c(new_n50_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n72_), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n45_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nand3  g42(.a(x02), .b(new_n52_), .c(new_n51_), .O(new_n86_));
  nand3  g43(.a(x19), .b(new_n47_), .c(x17), .O(new_n87_));
  nor3   g44(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(z08));
  inv1   g45(.a(x20), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n48_), .c(x18), .d(x01), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n70_), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  nor2   g50(.a(x16), .b(x15), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(x20), .c(new_n93_), .d(x02), .O(new_n95_));
  oai21  g52(.a(new_n95_), .b(new_n92_), .c(new_n90_), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(new_n51_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(x21), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n96_), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n45_), .O(z09));
  nand2  g58(.a(x22), .b(x21), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(new_n90_), .O(new_n103_));
  nand2  g60(.a(x02), .b(new_n52_), .O(new_n104_));
  nor2   g61(.a(new_n92_), .b(new_n104_), .O(new_n105_));
  inv1   g62(.a(x21), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(x20), .c(x15), .d(new_n93_), .O(new_n107_));
  nand2  g64(.a(new_n97_), .b(x16), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n105_), .c(new_n103_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n45_), .O(z10));
  inv1   g68(.a(new_n107_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n105_), .c(new_n44_), .O(new_n113_));
  inv1   g70(.a(new_n90_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(x21), .c(x16), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n113_), .c(new_n98_), .O(z11));
  inv1   g73(.a(x23), .O(new_n117_));
  nor2   g74(.a(x19), .b(x02), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x17), .O(new_n119_));
  oai21  g76(.a(new_n118_), .b(new_n117_), .c(new_n119_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n52_), .c(new_n51_), .O(new_n121_));
  nand2  g78(.a(x10), .b(x02), .O(new_n122_));
  nand4  g79(.a(new_n122_), .b(x16), .c(x01), .d(x00), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n121_), .c(new_n125_), .O(z12));
  nor2   g83(.a(new_n119_), .b(new_n53_), .O(z13));
  nor2   g84(.a(new_n60_), .b(new_n55_), .O(z14));
  oai21  g85(.a(x10), .b(new_n52_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n71_), .c(new_n52_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n130_), .c(new_n45_), .O(z15));
  nand3  g89(.a(x16), .b(x01), .c(new_n51_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z16));
  nand2  g91(.a(new_n86_), .b(new_n45_), .O(z17));
endmodule


