// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  inv1   g02(.a(x16), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(x15), .O(new_n47_));
  nor2   g04(.a(x18), .b(x17), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor3   g07(.a(x02), .b(x01), .c(x00), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor3   g09(.a(new_n52_), .b(new_n49_), .c(new_n44_), .O(z00));
  nand2  g10(.a(new_n51_), .b(x09), .O(new_n54_));
  nand3  g11(.a(new_n48_), .b(new_n45_), .c(x10), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n54_), .c(new_n47_), .O(z01));
  or2    g13(.a(new_n49_), .b(x10), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g15(.a(x17), .O(new_n59_));
  nand4  g16(.a(new_n51_), .b(new_n45_), .c(x18), .d(new_n59_), .O(new_n60_));
  inv1   g17(.a(x11), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n44_), .O(new_n62_));
  inv1   g19(.a(x02), .O(new_n63_));
  nor2   g20(.a(x09), .b(new_n63_), .O(new_n64_));
  inv1   g21(.a(x00), .O(new_n65_));
  inv1   g22(.a(x01), .O(new_n66_));
  nor2   g23(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n64_), .c(new_n62_), .d(x12), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n60_), .c(new_n47_), .O(z03));
  nand3  g26(.a(new_n67_), .b(new_n64_), .c(new_n47_), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(new_n61_), .c(x10), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n70_), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nand2  g31(.a(new_n67_), .b(x09), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n47_), .O(z05));
  inv1   g33(.a(new_n70_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n62_), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(z06));
  nand3  g36(.a(new_n62_), .b(new_n71_), .c(new_n50_), .O(new_n80_));
  nand3  g37(.a(new_n47_), .b(x02), .c(x00), .O(new_n81_));
  aoi21  g38(.a(new_n80_), .b(x01), .c(new_n81_), .O(z07));
  nand2  g39(.a(new_n47_), .b(x02), .O(new_n83_));
  nand2  g40(.a(new_n66_), .b(new_n65_), .O(new_n84_));
  inv1   g41(.a(x18), .O(new_n85_));
  nand3  g42(.a(x19), .b(new_n85_), .c(x17), .O(new_n86_));
  inv1   g43(.a(x03), .O(new_n87_));
  nor2   g44(.a(x06), .b(x05), .O(new_n88_));
  nor2   g45(.a(x08), .b(x07), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n88_), .c(x04), .d(new_n87_), .O(new_n90_));
  nor4   g47(.a(new_n90_), .b(new_n86_), .c(new_n84_), .d(new_n83_), .O(z08));
  nand2  g48(.a(x02), .b(new_n66_), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n93_), .c(x12), .d(new_n61_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  inv1   g53(.a(x20), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(x16), .O(new_n98_));
  nand4  g55(.a(new_n97_), .b(new_n45_), .c(x18), .d(x01), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  aoi21  g57(.a(new_n98_), .b(new_n96_), .c(new_n100_), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n65_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n101_), .c(new_n47_), .O(z09));
  nand2  g62(.a(x22), .b(x21), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n99_), .O(new_n107_));
  nand4  g64(.a(new_n103_), .b(new_n102_), .c(x20), .d(x15), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n96_), .c(new_n107_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n47_), .O(z10));
  nand3  g68(.a(new_n103_), .b(x21), .c(new_n65_), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(new_n99_), .c(new_n47_), .O(z11));
  nand2  g70(.a(new_n74_), .b(new_n67_), .O(new_n114_));
  nand2  g71(.a(new_n45_), .b(new_n63_), .O(new_n115_));
  inv1   g72(.a(x23), .O(new_n116_));
  aoi21  g73(.a(new_n115_), .b(new_n116_), .c(new_n84_), .O(new_n117_));
  oai21  g74(.a(new_n115_), .b(x17), .c(new_n117_), .O(new_n118_));
  inv1   g75(.a(x24), .O(new_n119_));
  nand3  g76(.a(new_n47_), .b(new_n119_), .c(x09), .O(new_n120_));
  aoi21  g77(.a(new_n118_), .b(new_n114_), .c(new_n120_), .O(z12));
  inv1   g78(.a(new_n47_), .O(new_n122_));
  nand2  g79(.a(new_n51_), .b(x17), .O(new_n123_));
  nor3   g80(.a(new_n123_), .b(new_n122_), .c(x19), .O(z13));
  nor2   g81(.a(new_n57_), .b(new_n52_), .O(z14));
  aoi21  g82(.a(new_n44_), .b(x01), .c(new_n63_), .O(new_n126_));
  nor2   g83(.a(x02), .b(x01), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(x19), .c(new_n122_), .O(new_n128_));
  oai21  g85(.a(new_n126_), .b(new_n65_), .c(new_n128_), .O(z15));
  nor3   g86(.a(new_n122_), .b(new_n66_), .c(x00), .O(z16));
  nor2   g87(.a(new_n84_), .b(new_n83_), .O(z17));
endmodule


