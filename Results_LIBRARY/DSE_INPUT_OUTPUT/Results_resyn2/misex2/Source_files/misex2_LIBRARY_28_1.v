// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n98_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n111_,
    new_n113_, new_n114_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x10), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(x09), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nor2   g04(.a(x19), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x00), .O(z00));
  nand3  g09(.a(new_n50_), .b(x10), .c(x09), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x00), .O(z01));
  nand4  g11(.a(new_n48_), .b(new_n47_), .c(new_n45_), .d(x09), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x00), .O(z02));
  nand3  g13(.a(new_n46_), .b(x11), .c(x02), .O(new_n57_));
  nand3  g14(.a(x12), .b(x01), .c(x00), .O(new_n58_));
  nor2   g15(.a(x01), .b(x00), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  inv1   g17(.a(x17), .O(new_n61_));
  nand3  g18(.a(new_n48_), .b(x18), .c(new_n61_), .O(new_n62_));
  oai22  g19(.a(new_n62_), .b(new_n60_), .c(new_n58_), .d(new_n57_), .O(z03));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(x11), .O(new_n65_));
  inv1   g22(.a(x12), .O(new_n66_));
  nand3  g23(.a(new_n46_), .b(new_n66_), .c(new_n65_), .O(new_n67_));
  nor2   g24(.a(new_n67_), .b(new_n64_), .O(z04));
  inv1   g25(.a(x09), .O(new_n69_));
  nor3   g26(.a(new_n64_), .b(new_n45_), .c(new_n69_), .O(z05));
  aoi21  g27(.a(new_n57_), .b(x00), .c(new_n44_), .O(z06));
  inv1   g28(.a(x00), .O(new_n72_));
  nand2  g29(.a(x01), .b(new_n72_), .O(new_n73_));
  nand4  g30(.a(new_n66_), .b(x11), .c(x10), .d(new_n69_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n73_), .O(z07));
  inv1   g34(.a(x03), .O(new_n78_));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n79_), .c(x04), .d(new_n78_), .O(new_n81_));
  nand2  g38(.a(new_n59_), .b(x02), .O(new_n82_));
  inv1   g39(.a(x18), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n83_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(z08));
  nor2   g42(.a(x14), .b(x13), .O(new_n86_));
  nor2   g43(.a(x16), .b(x15), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(x12), .d(new_n65_), .O(new_n88_));
  inv1   g45(.a(x21), .O(new_n89_));
  inv1   g46(.a(x22), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n91_));
  nor3   g48(.a(new_n91_), .b(new_n88_), .c(new_n82_), .O(z09));
  inv1   g49(.a(new_n91_), .O(new_n93_));
  nand4  g50(.a(x15), .b(x12), .c(new_n65_), .d(x02), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n93_), .c(new_n86_), .d(x16), .O(new_n96_));
  aoi21  g53(.a(new_n96_), .b(new_n44_), .c(x00), .O(z10));
  inv1   g54(.a(x16), .O(new_n98_));
  nand4  g55(.a(new_n95_), .b(new_n93_), .c(new_n86_), .d(new_n98_), .O(new_n99_));
  aoi21  g56(.a(new_n99_), .b(new_n44_), .c(x00), .O(z11));
  aoi21  g57(.a(x10), .b(x02), .c(new_n44_), .O(new_n101_));
  oai21  g58(.a(x19), .b(x02), .c(x23), .O(new_n102_));
  inv1   g59(.a(x02), .O(new_n103_));
  inv1   g60(.a(x19), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(x17), .c(new_n103_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n102_), .c(x00), .O(new_n106_));
  nor2   g63(.a(x24), .b(new_n69_), .O(new_n107_));
  oai21  g64(.a(new_n106_), .b(new_n101_), .c(new_n107_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n73_), .O(z12));
  nor2   g66(.a(new_n105_), .b(new_n60_), .O(z13));
  nand4  g67(.a(new_n48_), .b(new_n47_), .c(new_n45_), .d(new_n69_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n44_), .c(x00), .O(z14));
  aoi21  g69(.a(new_n45_), .b(x01), .c(new_n103_), .O(new_n113_));
  nand3  g70(.a(x19), .b(new_n103_), .c(new_n44_), .O(new_n114_));
  oai21  g71(.a(new_n113_), .b(new_n72_), .c(new_n114_), .O(z15));
  inv1   g72(.a(new_n82_), .O(z17));
  zero   g73(.O(z16));
endmodule


