// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n123_, new_n125_,
    new_n126_;
  inv1   g00(.a(x01), .O(new_n44_));
  nor2   g01(.a(x09), .b(x02), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(new_n44_), .c(x00), .O(z00));
  inv1   g05(.a(x09), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(x02), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n46_), .c(x10), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x00), .O(z01));
  inv1   g09(.a(x10), .O(new_n53_));
  nand3  g10(.a(new_n50_), .b(new_n46_), .c(new_n53_), .O(new_n54_));
  aoi21  g11(.a(new_n54_), .b(new_n44_), .c(x00), .O(z02));
  inv1   g12(.a(x00), .O(new_n56_));
  inv1   g13(.a(x17), .O(new_n57_));
  nor2   g14(.a(x19), .b(x02), .O(new_n58_));
  nand3  g15(.a(new_n58_), .b(x18), .c(new_n57_), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n44_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  nand2  g18(.a(x10), .b(x02), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  inv1   g20(.a(x11), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(x09), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n63_), .c(x12), .d(x01), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n61_), .O(z03));
  inv1   g24(.a(x02), .O(new_n68_));
  nor2   g25(.a(x11), .b(new_n68_), .O(new_n69_));
  nor2   g26(.a(x12), .b(new_n53_), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(new_n69_), .c(new_n49_), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x00), .c(new_n44_), .O(z04));
  nand2  g29(.a(new_n63_), .b(x09), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x00), .c(new_n44_), .O(z05));
  nand2  g31(.a(new_n65_), .b(new_n63_), .O(new_n75_));
  nor3   g32(.a(new_n75_), .b(new_n44_), .c(new_n56_), .O(z06));
  nand2  g33(.a(x01), .b(new_n56_), .O(new_n77_));
  aoi21  g34(.a(new_n70_), .b(new_n65_), .c(new_n44_), .O(new_n78_));
  nand2  g35(.a(x02), .b(x00), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(z07));
  inv1   g37(.a(x08), .O(new_n81_));
  inv1   g38(.a(x18), .O(new_n82_));
  nand4  g39(.a(x19), .b(new_n82_), .c(x17), .d(new_n81_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nor2   g41(.a(x03), .b(new_n68_), .O(new_n85_));
  inv1   g42(.a(x05), .O(new_n86_));
  inv1   g43(.a(x06), .O(new_n87_));
  inv1   g44(.a(x07), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x04), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n85_), .c(new_n84_), .O(new_n91_));
  aoi21  g48(.a(new_n91_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g49(.a(x13), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x12), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  nor2   g57(.a(x16), .b(x15), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(new_n96_), .d(new_n69_), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n44_), .c(x00), .O(z09));
  nand2  g60(.a(new_n96_), .b(new_n69_), .O(new_n104_));
  nor2   g61(.a(x01), .b(x00), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n100_), .c(x16), .d(x15), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n104_), .O(z10));
  inv1   g64(.a(x15), .O(new_n108_));
  nor2   g65(.a(x16), .b(new_n108_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n100_), .c(new_n96_), .d(new_n69_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n44_), .c(x00), .O(z11));
  nand3  g68(.a(new_n62_), .b(x01), .c(x00), .O(new_n112_));
  nand2  g69(.a(new_n58_), .b(new_n57_), .O(new_n113_));
  inv1   g70(.a(x23), .O(new_n114_));
  inv1   g71(.a(x19), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n68_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n105_), .c(new_n113_), .O(new_n118_));
  inv1   g75(.a(x24), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(x09), .O(new_n120_));
  aoi21  g77(.a(new_n118_), .b(new_n112_), .c(new_n120_), .O(z12));
  nor4   g78(.a(new_n116_), .b(new_n57_), .c(x01), .d(x00), .O(z13));
  nand3  g79(.a(new_n46_), .b(new_n45_), .c(new_n53_), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n44_), .c(x00), .O(z14));
  aoi21  g81(.a(new_n53_), .b(x01), .c(new_n68_), .O(new_n125_));
  nand3  g82(.a(x19), .b(new_n68_), .c(new_n44_), .O(new_n126_));
  oai21  g83(.a(new_n125_), .b(new_n56_), .c(new_n126_), .O(z15));
  inv1   g84(.a(new_n77_), .O(z16));
  aoi21  g85(.a(new_n68_), .b(new_n44_), .c(x00), .O(z17));
endmodule


