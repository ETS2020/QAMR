// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n124_,
    new_n126_, new_n127_;
  inv1   g00(.a(x09), .O(new_n44_));
  nand2  g01(.a(x10), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n45_), .O(z00));
  nand2  g07(.a(x10), .b(x09), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n49_), .O(z01));
  inv1   g09(.a(x01), .O(new_n53_));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n47_), .b(new_n54_), .c(x09), .d(new_n46_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n53_), .c(x00), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x17), .O(new_n58_));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x18), .c(new_n58_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nor2   g19(.a(x09), .b(new_n46_), .O(new_n63_));
  inv1   g20(.a(x11), .O(new_n64_));
  inv1   g21(.a(x12), .O(new_n65_));
  nor2   g22(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n63_), .c(x10), .d(x01), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n62_), .O(z03));
  nand4  g25(.a(new_n63_), .b(new_n65_), .c(new_n64_), .d(x10), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x00), .c(new_n53_), .O(z04));
  nor2   g27(.a(new_n53_), .b(new_n57_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(x02), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n51_), .O(z05));
  nand3  g30(.a(x11), .b(x10), .c(new_n44_), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n72_), .O(z06));
  inv1   g32(.a(new_n74_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n65_), .O(new_n77_));
  nand2  g34(.a(new_n53_), .b(new_n57_), .O(new_n78_));
  oai21  g35(.a(x02), .b(new_n57_), .c(new_n78_), .O(new_n79_));
  aoi21  g36(.a(new_n77_), .b(new_n71_), .c(new_n79_), .O(z07));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nand3  g41(.a(x02), .b(new_n53_), .c(new_n57_), .O(new_n85_));
  inv1   g42(.a(x18), .O(new_n86_));
  nand3  g43(.a(x19), .b(new_n86_), .c(x17), .O(new_n87_));
  nor3   g44(.a(new_n87_), .b(new_n85_), .c(new_n84_), .O(z08));
  inv1   g45(.a(new_n85_), .O(z17));
  nor2   g46(.a(new_n65_), .b(x11), .O(new_n90_));
  nor2   g47(.a(x14), .b(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n90_), .c(z17), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  inv1   g52(.a(x20), .O(new_n96_));
  nor2   g53(.a(x21), .b(new_n96_), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n95_), .c(new_n94_), .d(new_n93_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n92_), .O(z09));
  inv1   g56(.a(x13), .O(new_n100_));
  inv1   g57(.a(x14), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(x12), .d(new_n64_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  inv1   g60(.a(x21), .O(new_n104_));
  nand4  g61(.a(new_n95_), .b(new_n104_), .c(x20), .d(x16), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n103_), .c(z17), .d(x15), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(z10));
  nand4  g65(.a(new_n95_), .b(new_n104_), .c(x20), .d(new_n94_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n103_), .c(z17), .d(x15), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(z11));
  oai21  g69(.a(new_n54_), .b(new_n46_), .c(new_n71_), .O(new_n113_));
  nand2  g70(.a(new_n59_), .b(new_n58_), .O(new_n114_));
  inv1   g71(.a(x23), .O(new_n115_));
  oai21  g72(.a(x19), .b(x02), .c(new_n115_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n114_), .c(new_n48_), .O(new_n117_));
  inv1   g74(.a(x24), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x09), .O(new_n119_));
  aoi21  g76(.a(new_n117_), .b(new_n113_), .c(new_n119_), .O(z12));
  nand2  g77(.a(new_n59_), .b(x17), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n53_), .c(x00), .O(z13));
  nand2  g79(.a(new_n47_), .b(new_n54_), .O(new_n123_));
  nand3  g80(.a(new_n48_), .b(new_n44_), .c(new_n46_), .O(new_n124_));
  nor2   g81(.a(new_n124_), .b(new_n123_), .O(z14));
  oai21  g82(.a(new_n54_), .b(new_n57_), .c(x01), .O(new_n126_));
  oai21  g83(.a(x19), .b(x00), .c(new_n46_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n126_), .O(z15));
  nor2   g85(.a(new_n53_), .b(x00), .O(z16));
endmodule


