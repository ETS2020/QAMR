// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n129_, new_n130_, new_n131_;
  nand2  g00(.a(x16), .b(x01), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  nor2   g05(.a(x01), .b(x00), .O(new_n49_));
  nor2   g06(.a(x09), .b(x02), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai21  g08(.a(new_n51_), .b(new_n48_), .c(new_n44_), .O(z00));
  inv1   g09(.a(x02), .O(new_n53_));
  nand3  g10(.a(new_n49_), .b(x09), .c(new_n53_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  inv1   g15(.a(x01), .O(new_n59_));
  nand2  g16(.a(x02), .b(x00), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x09), .O(new_n61_));
  nand3  g18(.a(x12), .b(x11), .c(x10), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  aoi21  g20(.a(new_n63_), .b(new_n61_), .c(x16), .O(new_n64_));
  nor2   g21(.a(x19), .b(new_n46_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n49_), .c(new_n45_), .d(new_n53_), .O(new_n66_));
  oai21  g23(.a(new_n64_), .b(new_n59_), .c(new_n66_), .O(z03));
  inv1   g24(.a(x16), .O(new_n68_));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n61_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(new_n68_), .c(new_n59_), .O(z04));
  nand4  g29(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(new_n68_), .c(new_n59_), .O(z05));
  nand2  g31(.a(x11), .b(x10), .O(new_n75_));
  nand2  g32(.a(new_n68_), .b(x01), .O(new_n76_));
  nor4   g33(.a(new_n76_), .b(new_n75_), .c(new_n60_), .d(x09), .O(z06));
  nor2   g34(.a(new_n56_), .b(x09), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(new_n68_), .c(new_n70_), .d(x11), .O(new_n79_));
  aoi21  g36(.a(new_n79_), .b(x01), .c(new_n60_), .O(z07));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nand2  g41(.a(new_n49_), .b(x02), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(z17));
  nand4  g43(.a(z17), .b(x19), .c(new_n46_), .d(x17), .O(new_n87_));
  oai21  g44(.a(new_n87_), .b(new_n84_), .c(new_n44_), .O(z08));
  nor2   g45(.a(x20), .b(new_n59_), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(new_n65_), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  nand4  g48(.a(x12), .b(new_n69_), .c(x02), .d(new_n59_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nor2   g50(.a(x14), .b(x13), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(x20), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n93_), .c(new_n91_), .O(new_n97_));
  inv1   g54(.a(x00), .O(new_n98_));
  nor2   g55(.a(x22), .b(x21), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n68_), .c(new_n98_), .O(new_n100_));
  aoi21  g57(.a(new_n97_), .b(new_n90_), .c(new_n100_), .O(z09));
  nand4  g58(.a(new_n89_), .b(new_n65_), .c(x22), .d(x21), .O(new_n102_));
  nor2   g59(.a(x11), .b(new_n53_), .O(new_n103_));
  nand4  g60(.a(x20), .b(x16), .c(x15), .d(x12), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n99_), .c(new_n94_), .d(new_n103_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n102_), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(new_n98_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n44_), .O(z10));
  nand3  g66(.a(new_n89_), .b(new_n65_), .c(x21), .O(new_n110_));
  nor2   g67(.a(x21), .b(new_n91_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n96_), .c(new_n93_), .O(new_n112_));
  inv1   g69(.a(x22), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n68_), .c(new_n98_), .O(new_n114_));
  aoi21  g71(.a(new_n112_), .b(new_n110_), .c(new_n114_), .O(z11));
  inv1   g72(.a(x23), .O(new_n116_));
  nor2   g73(.a(x19), .b(x02), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x17), .O(new_n118_));
  oai21  g75(.a(new_n117_), .b(new_n116_), .c(new_n118_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n49_), .O(new_n120_));
  nand2  g77(.a(x10), .b(x02), .O(new_n121_));
  nand4  g78(.a(new_n121_), .b(new_n68_), .c(x01), .d(x00), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n120_), .c(new_n124_), .O(z12));
  inv1   g82(.a(new_n49_), .O(new_n126_));
  oai21  g83(.a(new_n118_), .b(new_n126_), .c(new_n44_), .O(z13));
  nor2   g84(.a(new_n57_), .b(new_n51_), .O(z14));
  oai21  g85(.a(x10), .b(new_n59_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n53_), .c(new_n59_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n130_), .c(new_n44_), .O(z15));
  aoi21  g89(.a(new_n68_), .b(x00), .c(new_n59_), .O(z16));
endmodule


