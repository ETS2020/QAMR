// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n125_, new_n126_;
  inv1   g00(.a(x19), .O(new_n44_));
  nor2   g01(.a(x21), .b(x16), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n48_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand3  g13(.a(new_n47_), .b(new_n44_), .c(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n46_), .O(z02));
  nand3  g15(.a(x11), .b(x10), .c(new_n50_), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n60_), .c(x12), .d(x02), .O(new_n64_));
  inv1   g21(.a(x18), .O(new_n65_));
  nor2   g22(.a(x19), .b(new_n65_), .O(new_n66_));
  nor2   g23(.a(x02), .b(x01), .O(new_n67_));
  nor2   g24(.a(x17), .b(x00), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(new_n64_), .c(new_n45_), .O(z03));
  nand2  g27(.a(new_n63_), .b(x02), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n72_), .c(x10), .d(new_n50_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n71_), .c(new_n46_), .O(z04));
  nand2  g32(.a(x10), .b(x02), .O(new_n76_));
  nand2  g33(.a(new_n63_), .b(x09), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n76_), .c(new_n46_), .O(z05));
  nand2  g35(.a(new_n46_), .b(x10), .O(new_n79_));
  nand2  g36(.a(x11), .b(new_n50_), .O(new_n80_));
  nor3   g37(.a(new_n71_), .b(new_n80_), .c(new_n79_), .O(z06));
  oai21  g38(.a(new_n59_), .b(x12), .c(x01), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(new_n46_), .O(z07));
  nand3  g41(.a(new_n51_), .b(new_n46_), .c(x02), .O(new_n85_));
  nand3  g42(.a(x19), .b(new_n65_), .c(x17), .O(new_n86_));
  inv1   g43(.a(x03), .O(new_n87_));
  nor2   g44(.a(x06), .b(x05), .O(new_n88_));
  nor2   g45(.a(x08), .b(x07), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n88_), .c(x04), .d(new_n87_), .O(new_n90_));
  nor3   g47(.a(new_n90_), .b(new_n86_), .c(new_n85_), .O(z08));
  nor2   g48(.a(new_n62_), .b(x00), .O(new_n92_));
  nor2   g49(.a(x22), .b(x20), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(new_n66_), .O(new_n94_));
  aoi21  g51(.a(new_n94_), .b(x16), .c(x21), .O(z09));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(x20), .d(x15), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nand2  g56(.a(x02), .b(new_n62_), .O(new_n100_));
  inv1   g57(.a(x13), .O(new_n101_));
  inv1   g58(.a(x14), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n101_), .c(x12), .d(new_n72_), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  nand2  g61(.a(new_n44_), .b(x18), .O(new_n105_));
  inv1   g62(.a(x20), .O(new_n106_));
  nand4  g63(.a(x22), .b(x21), .c(new_n106_), .d(x01), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  aoi21  g65(.a(new_n104_), .b(new_n99_), .c(new_n108_), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(x00), .c(new_n46_), .O(z10));
  or2    g67(.a(new_n94_), .b(new_n96_), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(z11));
  nand2  g69(.a(new_n76_), .b(new_n63_), .O(new_n113_));
  inv1   g70(.a(x23), .O(new_n114_));
  nor2   g71(.a(x19), .b(x02), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x17), .O(new_n116_));
  oai21  g73(.a(new_n115_), .b(new_n114_), .c(new_n116_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n51_), .O(new_n118_));
  nor2   g75(.a(x24), .b(new_n50_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n46_), .O(new_n120_));
  aoi21  g77(.a(new_n118_), .b(new_n113_), .c(new_n120_), .O(z12));
  nand3  g78(.a(new_n115_), .b(new_n51_), .c(x17), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n46_), .O(z13));
  oai21  g80(.a(new_n57_), .b(new_n52_), .c(new_n46_), .O(z14));
  aoi21  g81(.a(new_n56_), .b(x01), .c(new_n49_), .O(new_n125_));
  aoi21  g82(.a(new_n67_), .b(x19), .c(new_n45_), .O(new_n126_));
  oai21  g83(.a(new_n125_), .b(new_n61_), .c(new_n126_), .O(z15));
  oai21  g84(.a(new_n62_), .b(x00), .c(new_n46_), .O(z16));
  inv1   g85(.a(new_n85_), .O(z17));
endmodule


