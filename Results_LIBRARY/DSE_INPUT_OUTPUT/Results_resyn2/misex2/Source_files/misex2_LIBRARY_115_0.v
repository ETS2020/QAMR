// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n130_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(x15), .c(x10), .O(new_n48_));
  nor2   g05(.a(new_n48_), .b(new_n46_), .O(z00));
  nand2  g06(.a(new_n45_), .b(x09), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n48_), .O(z01));
  inv1   g08(.a(x00), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nor2   g10(.a(x10), .b(new_n44_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n47_), .c(new_n53_), .d(new_n52_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(x15), .c(x01), .O(z02));
  inv1   g13(.a(x01), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n52_), .O(new_n58_));
  nand3  g15(.a(x11), .b(x10), .c(new_n44_), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n58_), .c(x12), .d(x02), .O(new_n61_));
  inv1   g18(.a(x18), .O(new_n62_));
  nor2   g19(.a(x19), .b(new_n62_), .O(new_n63_));
  inv1   g20(.a(x15), .O(new_n64_));
  nor2   g21(.a(x17), .b(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n63_), .c(new_n45_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n61_), .O(z03));
  nand2  g24(.a(new_n58_), .b(x02), .O(new_n68_));
  nand2  g25(.a(new_n64_), .b(new_n57_), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n44_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n68_), .c(new_n69_), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nand2  g31(.a(new_n58_), .b(x09), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n69_), .O(z05));
  nor2   g33(.a(new_n59_), .b(new_n68_), .O(z06));
  oai21  g34(.a(new_n59_), .b(x12), .c(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n69_), .O(z07));
  inv1   g37(.a(x05), .O(new_n81_));
  nor2   g38(.a(x01), .b(x00), .O(new_n82_));
  nor2   g39(.a(x03), .b(new_n53_), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(new_n81_), .d(x04), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n62_), .c(x17), .O(new_n85_));
  inv1   g42(.a(x08), .O(new_n86_));
  nor2   g43(.a(x07), .b(x06), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(x15), .c(new_n86_), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n85_), .c(new_n84_), .O(z08));
  nor2   g46(.a(x20), .b(x19), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(new_n52_), .O(new_n91_));
  inv1   g48(.a(x21), .O(new_n92_));
  inv1   g49(.a(x22), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(x18), .d(x01), .O(new_n94_));
  oai21  g51(.a(new_n94_), .b(new_n91_), .c(new_n69_), .O(z09));
  nor2   g52(.a(x20), .b(new_n57_), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n63_), .c(x22), .d(x21), .O(new_n97_));
  nand2  g54(.a(x02), .b(new_n57_), .O(new_n98_));
  inv1   g55(.a(x13), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(x12), .c(new_n70_), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nor2   g58(.a(new_n64_), .b(x14), .O(new_n102_));
  nand4  g59(.a(new_n93_), .b(new_n92_), .c(x20), .d(x16), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n97_), .c(x00), .O(z10));
  nand4  g63(.a(new_n90_), .b(x21), .c(x18), .d(x01), .O(new_n107_));
  inv1   g64(.a(x20), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(x16), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n102_), .c(new_n101_), .d(new_n92_), .O(new_n110_));
  nand2  g67(.a(new_n93_), .b(new_n52_), .O(new_n111_));
  aoi21  g68(.a(new_n110_), .b(new_n107_), .c(new_n111_), .O(z11));
  nand3  g69(.a(new_n74_), .b(x01), .c(x00), .O(new_n113_));
  nor3   g70(.a(x19), .b(x17), .c(x02), .O(new_n114_));
  nor2   g71(.a(x19), .b(x02), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(x23), .c(new_n82_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(new_n117_));
  nor2   g74(.a(x24), .b(new_n44_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n69_), .O(z12));
  nand3  g77(.a(new_n115_), .b(x17), .c(new_n52_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(x15), .c(x01), .O(z13));
  inv1   g79(.a(x10), .O(new_n123_));
  nand3  g80(.a(new_n47_), .b(x15), .c(new_n123_), .O(new_n124_));
  nor2   g81(.a(new_n124_), .b(new_n46_), .O(z14));
  aoi21  g82(.a(new_n123_), .b(x01), .c(new_n53_), .O(new_n126_));
  aoi21  g83(.a(x19), .b(new_n53_), .c(new_n64_), .O(new_n127_));
  oai22  g84(.a(new_n127_), .b(x01), .c(new_n126_), .d(new_n52_), .O(z15));
  aoi21  g85(.a(x15), .b(new_n57_), .c(new_n58_), .O(z16));
  nand2  g86(.a(x02), .b(new_n52_), .O(new_n130_));
  aoi21  g87(.a(new_n130_), .b(x15), .c(x01), .O(z17));
endmodule


