// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n120_;
  nand2  g00(.a(x01), .b(x00), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor2   g05(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  inv1   g08(.a(x18), .O(new_n52_));
  inv1   g09(.a(x19), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n50_), .c(new_n44_), .O(z00));
  nand2  g12(.a(new_n49_), .b(x09), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n54_), .c(new_n44_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n56_), .O(z02));
  nand3  g17(.a(new_n53_), .b(x18), .c(new_n51_), .O(new_n61_));
  nor3   g18(.a(new_n61_), .b(new_n48_), .c(x02), .O(z03));
  inv1   g19(.a(new_n44_), .O(z04));
  inv1   g20(.a(x02), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n47_), .c(new_n46_), .O(z07));
  inv1   g22(.a(x03), .O(new_n68_));
  nor2   g23(.a(x06), .b(x05), .O(new_n69_));
  nor2   g24(.a(x08), .b(x07), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(x04), .d(new_n68_), .O(new_n71_));
  nor2   g26(.a(x01), .b(x00), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x02), .O(new_n73_));
  nand3  g28(.a(x19), .b(new_n52_), .c(x17), .O(new_n74_));
  nor3   g29(.a(new_n74_), .b(new_n73_), .c(new_n71_), .O(z08));
  inv1   g30(.a(x21), .O(new_n76_));
  inv1   g31(.a(x22), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  inv1   g33(.a(x20), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n53_), .c(x18), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n78_), .c(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(x01), .O(new_n82_));
  inv1   g37(.a(new_n73_), .O(z17));
  inv1   g38(.a(x11), .O(new_n84_));
  inv1   g39(.a(x13), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(x12), .c(new_n84_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nand3  g42(.a(new_n77_), .b(new_n76_), .c(x20), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor3   g44(.a(x16), .b(x15), .c(x14), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n89_), .c(new_n87_), .d(z17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n82_), .O(z09));
  nand2  g47(.a(x22), .b(x21), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n80_), .c(new_n46_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(x01), .O(new_n95_));
  inv1   g50(.a(x14), .O(new_n96_));
  nand3  g51(.a(x16), .b(x15), .c(new_n96_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(new_n88_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n87_), .c(z17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n95_), .O(z10));
  inv1   g55(.a(x15), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(x14), .O(new_n102_));
  inv1   g57(.a(x16), .O(new_n103_));
  nand3  g58(.a(new_n76_), .b(x20), .c(new_n103_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  nor2   g60(.a(new_n66_), .b(x01), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n105_), .c(new_n102_), .d(new_n87_), .O(new_n107_));
  inv1   g62(.a(new_n80_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(x21), .c(x01), .O(new_n109_));
  nand2  g64(.a(new_n77_), .b(new_n46_), .O(new_n110_));
  aoi21  g65(.a(new_n109_), .b(new_n107_), .c(new_n110_), .O(z11));
  oai21  g66(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  nand3  g67(.a(new_n53_), .b(x17), .c(new_n66_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nor2   g69(.a(x24), .b(new_n45_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n72_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n44_), .O(z12));
  nor2   g72(.a(new_n113_), .b(new_n48_), .O(z13));
  oai21  g73(.a(new_n59_), .b(new_n50_), .c(new_n44_), .O(z14));
  aoi21  g74(.a(x19), .b(new_n47_), .c(x00), .O(new_n120_));
  nor2   g75(.a(new_n120_), .b(new_n106_), .O(z15));
  nor2   g76(.a(new_n47_), .b(x00), .O(z16));
  zero   g77(.O(z05));
  zero   g78(.O(z06));
endmodule


