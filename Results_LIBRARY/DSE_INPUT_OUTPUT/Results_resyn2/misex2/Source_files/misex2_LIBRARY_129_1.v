// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n117_;
  nor3   g00(.a(x19), .b(x18), .c(x17), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x10), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x02), .b(x01), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n45_), .O(z00));
  inv1   g07(.a(x01), .O(new_n51_));
  inv1   g08(.a(x02), .O(new_n52_));
  nand4  g09(.a(new_n44_), .b(x10), .c(x09), .d(new_n52_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n51_), .c(x00), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand2  g12(.a(new_n44_), .b(new_n55_), .O(new_n56_));
  nor2   g13(.a(x01), .b(x00), .O(new_n57_));
  nand3  g14(.a(new_n57_), .b(x09), .c(new_n52_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n56_), .O(z02));
  nand2  g16(.a(x11), .b(new_n47_), .O(new_n60_));
  nand4  g17(.a(x10), .b(x02), .c(x01), .d(x00), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n60_), .O(z06));
  nand2  g19(.a(z06), .b(x12), .O(new_n63_));
  inv1   g20(.a(x17), .O(new_n64_));
  nor2   g21(.a(x19), .b(x00), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n48_), .c(x18), .d(new_n64_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n63_), .O(z03));
  inv1   g24(.a(x12), .O(new_n68_));
  nor2   g25(.a(x11), .b(new_n52_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n47_), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x00), .c(new_n51_), .O(z04));
  nor2   g28(.a(new_n61_), .b(new_n47_), .O(z05));
  nand2  g29(.a(x01), .b(new_n46_), .O(new_n73_));
  nand2  g30(.a(new_n68_), .b(x10), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n60_), .c(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n73_), .O(z07));
  inv1   g34(.a(x03), .O(new_n78_));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n79_), .c(x04), .d(new_n78_), .O(new_n81_));
  nor2   g38(.a(x18), .b(new_n64_), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n57_), .c(x19), .d(x02), .O(new_n83_));
  nor2   g40(.a(new_n83_), .b(new_n81_), .O(z08));
  nor2   g41(.a(x13), .b(new_n68_), .O(new_n85_));
  nor2   g42(.a(x14), .b(x11), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(new_n57_), .d(x02), .O(new_n87_));
  inv1   g44(.a(x15), .O(new_n88_));
  inv1   g45(.a(x16), .O(new_n89_));
  inv1   g46(.a(x21), .O(new_n90_));
  inv1   g47(.a(x22), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(x20), .d(new_n89_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n88_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n87_), .O(z09));
  nor2   g52(.a(new_n89_), .b(new_n88_), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n91_), .c(new_n90_), .d(x20), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n87_), .O(z10));
  inv1   g55(.a(x11), .O(new_n99_));
  inv1   g56(.a(x14), .O(new_n100_));
  nand4  g57(.a(x15), .b(new_n100_), .c(new_n99_), .d(x02), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n93_), .c(new_n85_), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(new_n51_), .c(x00), .O(z11));
  nand2  g61(.a(x10), .b(x02), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(x01), .c(x00), .O(new_n106_));
  oai21  g63(.a(x19), .b(x02), .c(x23), .O(new_n107_));
  inv1   g64(.a(x19), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(x17), .c(new_n52_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n57_), .O(new_n111_));
  inv1   g68(.a(x24), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(x09), .O(new_n113_));
  aoi21  g70(.a(new_n111_), .b(new_n106_), .c(new_n113_), .O(z12));
  nor3   g71(.a(new_n109_), .b(x01), .c(x00), .O(z13));
  nor2   g72(.a(new_n56_), .b(new_n49_), .O(z14));
  oai21  g73(.a(new_n55_), .b(new_n46_), .c(x01), .O(new_n117_));
  oai21  g74(.a(new_n65_), .b(x02), .c(new_n117_), .O(z15));
  inv1   g75(.a(new_n73_), .O(z16));
  nor2   g76(.a(new_n48_), .b(x00), .O(z17));
endmodule


