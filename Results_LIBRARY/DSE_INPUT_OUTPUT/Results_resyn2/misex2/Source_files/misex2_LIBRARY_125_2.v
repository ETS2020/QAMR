// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n123_, new_n124_,
    new_n126_, new_n127_;
  inv1   g00(.a(x00), .O(new_n44_));
  nor2   g01(.a(x09), .b(x02), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x10), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g09(.a(x02), .O(new_n53_));
  nor2   g10(.a(x01), .b(x00), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n49_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g16(.a(x12), .O(new_n60_));
  inv1   g17(.a(x09), .O(new_n61_));
  nand4  g18(.a(x11), .b(x10), .c(new_n61_), .d(x02), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n60_), .c(x01), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x00), .O(new_n64_));
  nor2   g21(.a(x02), .b(x01), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n48_), .c(x18), .d(new_n46_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n64_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  nand3  g25(.a(new_n60_), .b(new_n61_), .c(x02), .O(new_n69_));
  inv1   g26(.a(new_n69_), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(new_n68_), .c(x10), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x01), .c(new_n44_), .O(z04));
  nand2  g29(.a(x01), .b(x00), .O(new_n73_));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n73_), .c(new_n61_), .O(z05));
  nor2   g32(.a(new_n73_), .b(new_n62_), .O(z06));
  nand3  g33(.a(new_n70_), .b(x11), .c(x10), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x01), .c(new_n44_), .O(z07));
  inv1   g35(.a(x08), .O(new_n79_));
  nand4  g36(.a(x19), .b(new_n47_), .c(x17), .d(new_n79_), .O(new_n80_));
  inv1   g37(.a(x05), .O(new_n81_));
  nor2   g38(.a(x07), .b(x06), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(new_n81_), .c(x04), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nand3  g41(.a(new_n54_), .b(new_n84_), .c(x02), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n83_), .c(new_n80_), .O(z08));
  inv1   g43(.a(x20), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n48_), .c(x18), .d(x01), .O(new_n88_));
  inv1   g45(.a(x01), .O(new_n89_));
  nand2  g46(.a(x02), .b(new_n89_), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n68_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nor2   g50(.a(x15), .b(x14), .O(new_n94_));
  nor2   g51(.a(new_n87_), .b(x16), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n44_), .O(new_n99_));
  aoi21  g56(.a(new_n96_), .b(new_n88_), .c(new_n99_), .O(z09));
  inv1   g57(.a(new_n88_), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(x22), .c(x21), .O(new_n102_));
  inv1   g59(.a(x14), .O(new_n103_));
  nand4  g60(.a(new_n97_), .b(x20), .c(x15), .d(new_n103_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  inv1   g62(.a(x16), .O(new_n106_));
  nor2   g63(.a(x22), .b(new_n106_), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(new_n105_), .c(new_n93_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n102_), .c(x00), .O(z10));
  nand2  g66(.a(new_n98_), .b(new_n44_), .O(new_n110_));
  nand2  g67(.a(new_n101_), .b(x21), .O(new_n111_));
  nand3  g68(.a(new_n105_), .b(new_n93_), .c(new_n106_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(z11));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  nand3  g71(.a(new_n48_), .b(x17), .c(new_n53_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n54_), .O(new_n117_));
  nand3  g74(.a(new_n74_), .b(x01), .c(x00), .O(new_n118_));
  inv1   g75(.a(x24), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(x09), .O(new_n120_));
  aoi21  g77(.a(new_n118_), .b(new_n117_), .c(new_n120_), .O(z12));
  aoi21  g78(.a(new_n115_), .b(new_n44_), .c(x01), .O(z13));
  inv1   g79(.a(new_n58_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n45_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g82(.a(x19), .b(new_n53_), .c(x00), .O(new_n126_));
  aoi21  g83(.a(new_n74_), .b(x00), .c(new_n89_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(new_n126_), .O(z15));
  nor2   g85(.a(new_n89_), .b(x00), .O(z16));
  aoi21  g86(.a(new_n53_), .b(new_n44_), .c(x01), .O(z17));
endmodule


