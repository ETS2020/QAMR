// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n127_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x23), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x10), .O(new_n46_));
  nor2   g03(.a(new_n46_), .b(x09), .O(new_n47_));
  inv1   g04(.a(x00), .O(new_n48_));
  inv1   g05(.a(x01), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x02), .O(new_n51_));
  nor3   g08(.a(x19), .b(x18), .c(x17), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n51_), .c(new_n47_), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(new_n45_), .O(z00));
  nand2  g11(.a(new_n52_), .b(new_n51_), .O(new_n55_));
  nand2  g12(.a(x10), .b(x09), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g14(.a(new_n46_), .b(x09), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  nand2  g16(.a(x02), .b(x00), .O(new_n60_));
  inv1   g17(.a(x09), .O(new_n61_));
  nand4  g18(.a(x12), .b(x11), .c(x10), .d(new_n61_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n60_), .c(x23), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x01), .O(new_n64_));
  inv1   g21(.a(x17), .O(new_n65_));
  inv1   g22(.a(x18), .O(new_n66_));
  nor2   g23(.a(x19), .b(new_n66_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n51_), .c(new_n65_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n64_), .O(z03));
  nand2  g26(.a(new_n47_), .b(x23), .O(new_n70_));
  nand3  g27(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n71_), .c(new_n70_), .O(z04));
  or2    g32(.a(new_n60_), .b(new_n56_), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x23), .c(new_n49_), .O(z05));
  nor3   g34(.a(new_n71_), .b(new_n70_), .c(new_n72_), .O(z06));
  nand4  g35(.a(new_n47_), .b(x23), .c(new_n73_), .d(x11), .O(new_n79_));
  aoi21  g36(.a(new_n79_), .b(x01), .c(new_n60_), .O(z07));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nand3  g41(.a(x02), .b(new_n49_), .c(new_n48_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x19), .c(new_n66_), .d(x17), .O(new_n87_));
  oai21  g44(.a(new_n87_), .b(new_n84_), .c(new_n45_), .O(z08));
  inv1   g45(.a(x20), .O(new_n89_));
  nand3  g46(.a(x23), .b(new_n89_), .c(x01), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand2  g48(.a(new_n91_), .b(new_n67_), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n49_), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n72_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nor2   g53(.a(x15), .b(x14), .O(new_n97_));
  nor2   g54(.a(new_n89_), .b(x16), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n48_), .O(new_n102_));
  aoi21  g59(.a(new_n99_), .b(new_n92_), .c(new_n102_), .O(z09));
  nand4  g60(.a(new_n91_), .b(new_n67_), .c(x22), .d(x21), .O(new_n104_));
  inv1   g61(.a(x14), .O(new_n105_));
  nand4  g62(.a(new_n100_), .b(x20), .c(x15), .d(new_n105_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  inv1   g64(.a(x16), .O(new_n108_));
  nor2   g65(.a(x22), .b(new_n108_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n107_), .c(new_n96_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n104_), .c(x00), .O(z10));
  nand2  g68(.a(new_n101_), .b(new_n48_), .O(new_n112_));
  nand3  g69(.a(new_n91_), .b(new_n67_), .c(x21), .O(new_n113_));
  nand3  g70(.a(new_n107_), .b(new_n96_), .c(new_n108_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(z11));
  oai21  g72(.a(x19), .b(x02), .c(x23), .O(new_n116_));
  inv1   g73(.a(x02), .O(new_n117_));
  inv1   g74(.a(x19), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(x17), .c(new_n117_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n116_), .c(new_n50_), .O(new_n120_));
  nand2  g77(.a(x01), .b(x00), .O(new_n121_));
  aoi21  g78(.a(x10), .b(x02), .c(new_n121_), .O(new_n122_));
  nor2   g79(.a(x24), .b(new_n61_), .O(new_n123_));
  oai21  g80(.a(new_n122_), .b(new_n120_), .c(new_n123_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n45_), .O(z12));
  nor2   g82(.a(new_n119_), .b(new_n50_), .O(z13));
  nand2  g83(.a(new_n46_), .b(new_n61_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(new_n55_), .O(z14));
  oai21  g85(.a(x10), .b(new_n49_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n117_), .c(new_n49_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n130_), .c(new_n45_), .O(z15));
  aoi21  g89(.a(x23), .b(x00), .c(new_n49_), .O(z16));
  nand2  g90(.a(new_n85_), .b(new_n45_), .O(z17));
endmodule


