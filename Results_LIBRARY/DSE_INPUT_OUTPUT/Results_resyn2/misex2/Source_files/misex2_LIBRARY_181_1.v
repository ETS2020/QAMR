// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n125_,
    new_n126_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x09), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(new_n44_), .c(x00), .O(z00));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(x09), .c(new_n45_), .O(new_n52_));
  nand2  g09(.a(new_n46_), .b(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n52_), .O(z01));
  nand2  g11(.a(new_n46_), .b(new_n47_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x17), .O(new_n58_));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x18), .c(new_n58_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n44_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  inv1   g19(.a(x11), .O(new_n63_));
  inv1   g20(.a(x12), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n48_), .c(x02), .d(x01), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n62_), .O(z03));
  nand2  g24(.a(x01), .b(x00), .O(new_n68_));
  nand2  g25(.a(x10), .b(x02), .O(new_n69_));
  nor3   g26(.a(new_n69_), .b(new_n68_), .c(x09), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(new_n64_), .c(new_n63_), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(z04));
  nand3  g29(.a(x10), .b(x09), .c(x02), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x00), .c(new_n44_), .O(z05));
  nand2  g31(.a(new_n70_), .b(x11), .O(new_n75_));
  inv1   g32(.a(new_n75_), .O(z06));
  nand2  g33(.a(x01), .b(new_n57_), .O(new_n77_));
  inv1   g34(.a(x09), .O(new_n78_));
  nand4  g35(.a(new_n64_), .b(x11), .c(x10), .d(new_n78_), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n77_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nand2  g43(.a(new_n51_), .b(x02), .O(new_n87_));
  inv1   g44(.a(x18), .O(new_n88_));
  nand3  g45(.a(x19), .b(new_n88_), .c(x17), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n87_), .c(new_n86_), .O(z08));
  inv1   g47(.a(x15), .O(new_n91_));
  inv1   g48(.a(new_n87_), .O(z17));
  inv1   g49(.a(x13), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(x12), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n63_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(x20), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n98_), .c(z17), .d(new_n91_), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(z09));
  nand3  g61(.a(x16), .b(x15), .c(new_n95_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n93_), .b(x12), .c(new_n63_), .d(x02), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n106_), .c(new_n102_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n44_), .c(x00), .O(z10));
  nand4  g67(.a(new_n102_), .b(new_n98_), .c(z17), .d(x15), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(z11));
  nand3  g69(.a(new_n69_), .b(x01), .c(x00), .O(new_n113_));
  nand2  g70(.a(new_n59_), .b(new_n58_), .O(new_n114_));
  inv1   g71(.a(x23), .O(new_n115_));
  oai21  g72(.a(x19), .b(x02), .c(new_n115_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n114_), .c(new_n51_), .O(new_n117_));
  inv1   g74(.a(x24), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x09), .O(new_n119_));
  aoi21  g76(.a(new_n117_), .b(new_n113_), .c(new_n119_), .O(z12));
  nand2  g77(.a(new_n59_), .b(x17), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n44_), .c(x00), .O(z13));
  nand3  g79(.a(new_n51_), .b(new_n78_), .c(new_n45_), .O(new_n123_));
  nor2   g80(.a(new_n123_), .b(new_n55_), .O(z14));
  oai21  g81(.a(new_n47_), .b(new_n57_), .c(x01), .O(new_n125_));
  oai21  g82(.a(x19), .b(x00), .c(new_n45_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n125_), .O(z15));
  inv1   g84(.a(new_n77_), .O(z16));
endmodule


