// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n132_, new_n134_, new_n135_, new_n136_, new_n138_;
  inv1   g00(.a(x04), .O(new_n44_));
  inv1   g01(.a(x06), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nor2   g03(.a(new_n46_), .b(x19), .O(new_n47_));
  nor2   g04(.a(x18), .b(x17), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(x10), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor3   g07(.a(x02), .b(x01), .c(x00), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n49_), .O(z00));
  nand2  g10(.a(new_n51_), .b(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n49_), .O(z01));
  inv1   g12(.a(new_n46_), .O(new_n56_));
  inv1   g13(.a(x10), .O(new_n57_));
  inv1   g14(.a(x19), .O(new_n58_));
  nand3  g15(.a(new_n48_), .b(new_n58_), .c(new_n57_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n54_), .c(new_n56_), .O(z02));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g20(.a(x11), .b(x10), .c(new_n50_), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n63_), .c(x12), .d(x02), .O(new_n66_));
  inv1   g23(.a(x17), .O(new_n67_));
  nand4  g24(.a(new_n51_), .b(new_n58_), .c(x18), .d(new_n67_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n66_), .c(new_n56_), .O(z03));
  nand2  g26(.a(new_n63_), .b(x02), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(x10), .d(new_n50_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n70_), .c(new_n56_), .O(z04));
  nand2  g31(.a(x10), .b(x02), .O(new_n75_));
  nand2  g32(.a(new_n63_), .b(x09), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n56_), .O(z05));
  nor3   g34(.a(new_n64_), .b(new_n70_), .c(new_n46_), .O(z06));
  aoi21  g35(.a(new_n65_), .b(new_n72_), .c(new_n62_), .O(new_n79_));
  nand3  g36(.a(new_n56_), .b(x02), .c(x00), .O(new_n80_));
  nor2   g37(.a(new_n80_), .b(new_n79_), .O(z07));
  nor2   g38(.a(x01), .b(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x02), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nor2   g41(.a(x18), .b(new_n67_), .O(new_n85_));
  nor2   g42(.a(x05), .b(x03), .O(new_n86_));
  nor2   g43(.a(x08), .b(x07), .O(new_n87_));
  nand2  g44(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n85_), .c(new_n84_), .d(x19), .O(new_n90_));
  aoi21  g47(.a(new_n90_), .b(new_n45_), .c(new_n44_), .O(z08));
  inv1   g48(.a(x20), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n58_), .c(x18), .d(x01), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  nand2  g51(.a(x02), .b(new_n62_), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n71_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nor2   g55(.a(x15), .b(x14), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(x20), .d(new_n94_), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nor2   g59(.a(new_n46_), .b(x00), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  aoi21  g61(.a(new_n100_), .b(new_n93_), .c(new_n104_), .O(z09));
  inv1   g62(.a(new_n103_), .O(new_n106_));
  nor2   g63(.a(new_n93_), .b(new_n101_), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(x22), .O(new_n108_));
  inv1   g65(.a(x14), .O(new_n109_));
  nand2  g66(.a(x15), .b(new_n109_), .O(new_n110_));
  nand4  g67(.a(new_n102_), .b(new_n101_), .c(x20), .d(x16), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n98_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n108_), .c(new_n106_), .O(z10));
  nand3  g71(.a(new_n101_), .b(x20), .c(new_n94_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n110_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n98_), .c(new_n107_), .O(new_n117_));
  nand2  g74(.a(new_n102_), .b(new_n61_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n117_), .c(new_n56_), .O(z11));
  nand2  g76(.a(new_n75_), .b(new_n63_), .O(new_n120_));
  inv1   g77(.a(x02), .O(new_n121_));
  nand3  g78(.a(new_n58_), .b(new_n67_), .c(new_n121_), .O(new_n122_));
  inv1   g79(.a(x23), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(new_n123_), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(new_n122_), .c(new_n82_), .O(new_n125_));
  nor2   g82(.a(x24), .b(new_n50_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n56_), .O(new_n127_));
  aoi21  g84(.a(new_n125_), .b(new_n120_), .c(new_n127_), .O(z12));
  inv1   g85(.a(new_n51_), .O(new_n129_));
  nand2  g86(.a(new_n47_), .b(x17), .O(new_n130_));
  nor2   g87(.a(new_n130_), .b(new_n129_), .O(z13));
  nand3  g88(.a(new_n48_), .b(new_n47_), .c(new_n57_), .O(new_n132_));
  nor2   g89(.a(new_n132_), .b(new_n52_), .O(z14));
  oai21  g90(.a(x10), .b(new_n62_), .c(x02), .O(new_n134_));
  nand2  g91(.a(new_n134_), .b(x00), .O(new_n135_));
  nand3  g92(.a(x19), .b(new_n121_), .c(new_n62_), .O(new_n136_));
  aoi21  g93(.a(new_n136_), .b(new_n135_), .c(new_n46_), .O(z15));
  nand2  g94(.a(new_n103_), .b(x01), .O(new_n138_));
  inv1   g95(.a(new_n138_), .O(z16));
  nand2  g96(.a(new_n83_), .b(new_n56_), .O(z17));
endmodule


