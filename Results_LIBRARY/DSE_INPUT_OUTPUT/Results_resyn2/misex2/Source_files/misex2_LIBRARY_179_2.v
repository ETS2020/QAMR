// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x01), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n49_), .O(z00));
  nand2  g11(.a(new_n48_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nor3   g17(.a(new_n60_), .b(new_n57_), .c(x09), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x21), .c(x12), .d(x11), .O(new_n62_));
  nand2  g19(.a(new_n48_), .b(new_n52_), .O(new_n63_));
  nand2  g20(.a(x18), .b(new_n50_), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(z03));
  inv1   g22(.a(x21), .O(new_n66_));
  inv1   g23(.a(new_n60_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(x10), .c(new_n44_), .O(new_n68_));
  nor4   g25(.a(new_n68_), .b(new_n66_), .c(x12), .d(x11), .O(z04));
  nor4   g26(.a(new_n60_), .b(new_n66_), .c(new_n57_), .d(new_n44_), .O(z05));
  inv1   g27(.a(x11), .O(new_n71_));
  nor3   g28(.a(new_n68_), .b(new_n66_), .c(new_n71_), .O(z06));
  nand2  g29(.a(new_n66_), .b(x01), .O(new_n73_));
  inv1   g30(.a(x12), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(x11), .c(x10), .d(new_n44_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(x01), .O(new_n76_));
  nand3  g33(.a(new_n76_), .b(x02), .c(x00), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n73_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nand3  g39(.a(x02), .b(new_n46_), .c(new_n45_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(x19), .c(new_n51_), .d(x17), .O(new_n85_));
  oai21  g42(.a(new_n85_), .b(new_n82_), .c(new_n73_), .O(z08));
  inv1   g43(.a(x13), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(x12), .c(new_n71_), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  inv1   g46(.a(x14), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n90_), .c(x02), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nor2   g50(.a(x22), .b(x00), .O(new_n94_));
  inv1   g51(.a(x20), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(x16), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n94_), .c(new_n93_), .d(new_n89_), .O(new_n97_));
  aoi21  g54(.a(new_n97_), .b(new_n46_), .c(x21), .O(z09));
  inv1   g55(.a(x22), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n66_), .c(x20), .O(new_n100_));
  nand4  g57(.a(x16), .b(x15), .c(new_n90_), .d(x02), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand3  g59(.a(x22), .b(x18), .c(x01), .O(new_n103_));
  nor3   g60(.a(new_n103_), .b(x20), .c(x19), .O(new_n104_));
  aoi21  g61(.a(new_n102_), .b(new_n89_), .c(new_n104_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(x00), .c(new_n73_), .O(z10));
  inv1   g63(.a(new_n94_), .O(new_n107_));
  nor2   g64(.a(x20), .b(x19), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(x21), .c(x18), .d(x01), .O(new_n109_));
  inv1   g66(.a(x02), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(x01), .O(new_n111_));
  inv1   g68(.a(x16), .O(new_n112_));
  nand4  g69(.a(x20), .b(new_n112_), .c(x15), .d(new_n90_), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n89_), .c(new_n111_), .d(new_n66_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n109_), .c(new_n107_), .O(z11));
  nand2  g73(.a(x01), .b(x00), .O(new_n117_));
  aoi21  g74(.a(x10), .b(x02), .c(new_n117_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n52_), .b(x17), .c(new_n110_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(new_n47_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n44_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n73_), .O(z12));
  nor2   g81(.a(new_n63_), .b(new_n50_), .O(z13));
  nor2   g82(.a(new_n58_), .b(new_n49_), .O(z14));
  oai21  g83(.a(x10), .b(new_n46_), .c(x02), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x00), .O(new_n128_));
  nand3  g85(.a(x19), .b(new_n110_), .c(new_n46_), .O(new_n129_));
  nand3  g86(.a(new_n129_), .b(new_n128_), .c(new_n73_), .O(z15));
  aoi21  g87(.a(x21), .b(x00), .c(new_n46_), .O(z16));
  nand2  g88(.a(new_n83_), .b(new_n73_), .O(z17));
endmodule


