// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n71_, new_n73_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n129_, new_n130_, new_n132_;
  inv1   g00(.a(x01), .O(new_n44_));
  nor2   g01(.a(x05), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  nand2  g09(.a(new_n44_), .b(new_n52_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n50_), .c(new_n46_), .O(z00));
  nand2  g13(.a(new_n54_), .b(x09), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n50_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  inv1   g18(.a(x05), .O(new_n62_));
  nor2   g19(.a(x09), .b(new_n62_), .O(new_n63_));
  nand4  g20(.a(x10), .b(x02), .c(x01), .d(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n63_), .c(x12), .d(x11), .O(new_n66_));
  nand4  g23(.a(new_n54_), .b(new_n49_), .c(x18), .d(new_n47_), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n66_), .O(z03));
  nand2  g25(.a(new_n65_), .b(new_n63_), .O(new_n69_));
  nor3   g26(.a(new_n69_), .b(x12), .c(x11), .O(z04));
  nand4  g27(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x05), .c(new_n44_), .O(z05));
  inv1   g29(.a(x11), .O(new_n73_));
  nor2   g30(.a(new_n69_), .b(new_n73_), .O(z06));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  inv1   g32(.a(x12), .O(new_n76_));
  nand4  g33(.a(new_n63_), .b(new_n76_), .c(x11), .d(x10), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x01), .c(new_n75_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nand3  g39(.a(x02), .b(new_n44_), .c(new_n52_), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n48_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(z08));
  inv1   g42(.a(x20), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(new_n49_), .c(x18), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(x05), .c(x01), .O(new_n89_));
  inv1   g46(.a(x15), .O(new_n90_));
  nand2  g47(.a(x02), .b(new_n44_), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n73_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand3  g53(.a(x20), .b(new_n96_), .c(new_n95_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n94_), .c(new_n90_), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n52_), .O(new_n101_));
  or2    g58(.a(new_n101_), .b(x21), .O(new_n102_));
  aoi21  g59(.a(new_n99_), .b(new_n89_), .c(new_n102_), .O(z09));
  aoi21  g60(.a(new_n62_), .b(x01), .c(new_n52_), .O(new_n104_));
  nor2   g61(.a(x21), .b(new_n90_), .O(new_n105_));
  nand4  g62(.a(new_n100_), .b(x20), .c(x16), .d(new_n95_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(new_n105_), .c(new_n94_), .O(new_n108_));
  nand3  g65(.a(x22), .b(x21), .c(x01), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n88_), .c(new_n45_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n108_), .c(new_n104_), .O(z10));
  nand4  g69(.a(new_n88_), .b(x21), .c(x05), .d(x01), .O(new_n113_));
  nand3  g70(.a(new_n105_), .b(new_n98_), .c(new_n94_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n113_), .c(new_n101_), .O(z11));
  inv1   g72(.a(x02), .O(new_n116_));
  nand3  g73(.a(x05), .b(x01), .c(x00), .O(new_n117_));
  inv1   g74(.a(new_n117_), .O(new_n118_));
  oai21  g75(.a(new_n59_), .b(new_n116_), .c(new_n118_), .O(new_n119_));
  oai21  g76(.a(x19), .b(x02), .c(x23), .O(new_n120_));
  nand3  g77(.a(new_n49_), .b(x17), .c(new_n116_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n44_), .c(new_n52_), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n119_), .c(new_n125_), .O(z12));
  oai21  g83(.a(new_n121_), .b(new_n53_), .c(new_n46_), .O(z13));
  nor2   g84(.a(new_n60_), .b(new_n55_), .O(z14));
  nor2   g85(.a(new_n49_), .b(x01), .O(new_n129_));
  oai21  g86(.a(new_n129_), .b(new_n104_), .c(new_n116_), .O(new_n130_));
  oai21  g87(.a(new_n117_), .b(x10), .c(new_n130_), .O(z15));
  nand3  g88(.a(x05), .b(x01), .c(new_n52_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z16));
  nand2  g90(.a(new_n83_), .b(new_n46_), .O(z17));
endmodule


