// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor2   g02(.a(x16), .b(x08), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(new_n47_));
  nor2   g04(.a(x19), .b(x18), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  inv1   g06(.a(x00), .O(new_n50_));
  inv1   g07(.a(x01), .O(new_n51_));
  inv1   g08(.a(x02), .O(new_n52_));
  inv1   g09(.a(x09), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n54_));
  nor3   g11(.a(new_n54_), .b(new_n49_), .c(new_n44_), .O(z00));
  nor2   g12(.a(x01), .b(x00), .O(new_n56_));
  nand3  g13(.a(new_n56_), .b(x09), .c(new_n52_), .O(new_n57_));
  nand3  g14(.a(new_n48_), .b(new_n45_), .c(x10), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n57_), .c(new_n47_), .O(z01));
  nand3  g16(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n57_), .c(new_n47_), .O(z02));
  nand3  g18(.a(x11), .b(x10), .c(new_n53_), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nor2   g20(.a(new_n51_), .b(new_n50_), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n63_), .c(x12), .d(x02), .O(new_n65_));
  nor2   g22(.a(x19), .b(x02), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n56_), .c(x18), .d(new_n45_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n65_), .c(new_n47_), .O(z03));
  nand2  g25(.a(new_n64_), .b(x02), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n53_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n69_), .c(new_n47_), .O(z04));
  nand2  g30(.a(new_n64_), .b(new_n47_), .O(new_n74_));
  nand3  g31(.a(x10), .b(x09), .c(x02), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n74_), .O(z05));
  nor3   g33(.a(new_n74_), .b(new_n62_), .c(new_n52_), .O(z06));
  oai21  g34(.a(new_n62_), .b(x12), .c(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n47_), .O(z07));
  nand3  g37(.a(x02), .b(new_n51_), .c(new_n50_), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nor2   g39(.a(new_n45_), .b(x07), .O(new_n83_));
  inv1   g40(.a(x19), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(x18), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nor2   g43(.a(x06), .b(x05), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(x04), .c(new_n86_), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n85_), .c(new_n83_), .d(new_n82_), .O(new_n90_));
  aoi21  g47(.a(new_n90_), .b(x16), .c(x08), .O(z08));
  nand2  g48(.a(x02), .b(new_n51_), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n93_), .c(x12), .d(new_n70_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  inv1   g54(.a(x16), .O(new_n98_));
  nand3  g55(.a(x20), .b(new_n98_), .c(new_n97_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n84_), .c(x18), .d(x01), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  aoi21  g60(.a(new_n100_), .b(new_n96_), .c(new_n103_), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  inv1   g62(.a(x22), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n50_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n104_), .c(new_n47_), .O(z09));
  nand4  g65(.a(new_n103_), .b(new_n47_), .c(x22), .d(x21), .O(new_n109_));
  nand3  g66(.a(new_n106_), .b(new_n105_), .c(x20), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n96_), .c(x16), .d(x15), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n109_), .c(x00), .O(z10));
  nand2  g70(.a(new_n106_), .b(new_n50_), .O(new_n114_));
  nand3  g71(.a(new_n103_), .b(new_n47_), .c(x21), .O(new_n115_));
  nand2  g72(.a(x15), .b(x08), .O(new_n116_));
  nor2   g73(.a(new_n116_), .b(x21), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n96_), .c(x20), .d(new_n98_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n115_), .c(new_n114_), .O(z11));
  oai21  g76(.a(new_n44_), .b(new_n52_), .c(new_n64_), .O(new_n120_));
  inv1   g77(.a(x23), .O(new_n121_));
  nor2   g78(.a(new_n66_), .b(new_n121_), .O(new_n122_));
  nand2  g79(.a(new_n66_), .b(x17), .O(new_n123_));
  inv1   g80(.a(new_n123_), .O(new_n124_));
  oai21  g81(.a(new_n124_), .b(new_n122_), .c(new_n56_), .O(new_n125_));
  nor2   g82(.a(x24), .b(new_n53_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n47_), .O(new_n127_));
  aoi21  g84(.a(new_n125_), .b(new_n120_), .c(new_n127_), .O(z12));
  nand3  g85(.a(new_n124_), .b(new_n56_), .c(new_n47_), .O(new_n129_));
  inv1   g86(.a(new_n129_), .O(z13));
  nor3   g87(.a(new_n54_), .b(new_n49_), .c(x10), .O(z14));
  oai21  g88(.a(x10), .b(new_n51_), .c(x02), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(x00), .O(new_n133_));
  nand3  g90(.a(x19), .b(new_n52_), .c(new_n51_), .O(new_n134_));
  aoi21  g91(.a(new_n134_), .b(new_n133_), .c(new_n46_), .O(z15));
  nor3   g92(.a(new_n46_), .b(new_n51_), .c(x00), .O(z16));
  nand2  g93(.a(new_n81_), .b(new_n47_), .O(z17));
endmodule


