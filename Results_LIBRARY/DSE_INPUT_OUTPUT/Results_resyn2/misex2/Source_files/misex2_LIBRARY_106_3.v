// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n135_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x23), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n44_), .d(x10), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nor3   g08(.a(x02), .b(x01), .c(x00), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n50_), .c(new_n47_), .O(z00));
  nand2  g11(.a(new_n52_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n50_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n49_), .b(new_n48_), .c(new_n44_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n51_), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(new_n61_), .c(x12), .O(new_n64_));
  nor2   g21(.a(x19), .b(new_n48_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n52_), .c(new_n44_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n64_), .c(new_n47_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n51_), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n60_), .c(new_n47_), .O(z04));
  nand2  g28(.a(x10), .b(x02), .O(new_n72_));
  nand3  g29(.a(x09), .b(x01), .c(x00), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n72_), .c(new_n47_), .O(z05));
  oai21  g31(.a(new_n62_), .b(new_n60_), .c(new_n47_), .O(z06));
  inv1   g32(.a(x01), .O(new_n76_));
  aoi21  g33(.a(new_n63_), .b(new_n69_), .c(new_n76_), .O(new_n77_));
  nand3  g34(.a(new_n47_), .b(x02), .c(x00), .O(new_n78_));
  nor2   g35(.a(new_n78_), .b(new_n77_), .O(z07));
  nor2   g36(.a(x18), .b(x08), .O(new_n80_));
  inv1   g37(.a(x05), .O(new_n81_));
  nor2   g38(.a(x07), .b(x06), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(new_n81_), .c(x04), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nor2   g41(.a(x01), .b(x00), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(new_n84_), .c(x02), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n80_), .c(x19), .O(new_n88_));
  aoi21  g45(.a(new_n88_), .b(new_n45_), .c(new_n44_), .O(z08));
  nor2   g46(.a(x20), .b(new_n76_), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(new_n65_), .O(new_n91_));
  nand2  g48(.a(x02), .b(new_n76_), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(x12), .c(new_n68_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  inv1   g52(.a(x20), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(x16), .O(new_n97_));
  nor2   g54(.a(x15), .b(x14), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nor2   g58(.a(new_n46_), .b(x00), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  aoi21  g60(.a(new_n99_), .b(new_n91_), .c(new_n103_), .O(z09));
  inv1   g61(.a(new_n102_), .O(new_n105_));
  nand4  g62(.a(new_n90_), .b(new_n65_), .c(x22), .d(x21), .O(new_n106_));
  inv1   g63(.a(x15), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(x14), .O(new_n108_));
  nand4  g65(.a(new_n101_), .b(new_n100_), .c(x20), .d(x16), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n108_), .c(new_n95_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n106_), .c(new_n105_), .O(z10));
  nand3  g69(.a(new_n90_), .b(new_n65_), .c(x21), .O(new_n113_));
  nand4  g70(.a(new_n108_), .b(new_n97_), .c(new_n95_), .d(new_n100_), .O(new_n114_));
  nand2  g71(.a(new_n102_), .b(new_n101_), .O(new_n115_));
  aoi21  g72(.a(new_n114_), .b(new_n113_), .c(new_n115_), .O(z11));
  nand3  g73(.a(new_n72_), .b(x01), .c(x00), .O(new_n117_));
  nor3   g74(.a(x19), .b(x17), .c(x02), .O(new_n118_));
  nor2   g75(.a(x19), .b(x02), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(x23), .c(new_n85_), .O(new_n120_));
  oai21  g77(.a(new_n120_), .b(new_n118_), .c(new_n117_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n51_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n47_), .O(z12));
  nand2  g81(.a(new_n119_), .b(new_n85_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n45_), .c(new_n44_), .O(z13));
  nor2   g83(.a(new_n58_), .b(new_n53_), .O(z14));
  oai21  g84(.a(x10), .b(new_n76_), .c(x02), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x00), .O(new_n129_));
  inv1   g86(.a(x02), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n130_), .c(new_n76_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n129_), .c(new_n46_), .O(z15));
  nand2  g89(.a(new_n102_), .b(x01), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z16));
  nand3  g91(.a(new_n85_), .b(new_n47_), .c(x02), .O(new_n135_));
  inv1   g92(.a(new_n135_), .O(z17));
endmodule


