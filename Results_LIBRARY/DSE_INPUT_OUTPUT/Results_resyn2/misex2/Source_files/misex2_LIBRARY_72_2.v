// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n127_, new_n129_, new_n130_, new_n133_;
  nor3   g00(.a(x19), .b(x18), .c(x17), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x10), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n45_), .O(z00));
  nor2   g07(.a(new_n47_), .b(x02), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n45_), .O(z01));
  inv1   g10(.a(x00), .O(new_n54_));
  inv1   g11(.a(x10), .O(new_n55_));
  nand3  g12(.a(new_n51_), .b(new_n44_), .c(new_n55_), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n54_), .c(x01), .O(z02));
  inv1   g14(.a(x01), .O(new_n58_));
  nand4  g15(.a(x11), .b(x10), .c(new_n47_), .d(x02), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(x12), .c(new_n58_), .O(new_n61_));
  nor2   g18(.a(x19), .b(x02), .O(new_n62_));
  nor2   g19(.a(x17), .b(x01), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(new_n62_), .c(x18), .O(new_n64_));
  oai21  g21(.a(new_n61_), .b(new_n54_), .c(new_n64_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nor2   g24(.a(new_n55_), .b(new_n46_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n47_), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x01), .c(new_n54_), .O(z04));
  nand2  g27(.a(new_n68_), .b(x09), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x01), .c(new_n54_), .O(z05));
  nor3   g29(.a(new_n59_), .b(new_n58_), .c(new_n54_), .O(z06));
  nand2  g30(.a(new_n60_), .b(new_n67_), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x01), .c(new_n54_), .O(z07));
  inv1   g32(.a(x08), .O(new_n76_));
  inv1   g33(.a(x18), .O(new_n77_));
  nand4  g34(.a(x19), .b(new_n77_), .c(x17), .d(new_n76_), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(new_n79_));
  nor2   g36(.a(x03), .b(new_n46_), .O(new_n80_));
  inv1   g37(.a(x04), .O(new_n81_));
  nor2   g38(.a(x05), .b(new_n81_), .O(new_n82_));
  nor2   g39(.a(x07), .b(x06), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(new_n80_), .d(new_n79_), .O(new_n84_));
  aoi21  g41(.a(new_n84_), .b(new_n54_), .c(x01), .O(z08));
  nand2  g42(.a(new_n66_), .b(x02), .O(new_n86_));
  inv1   g43(.a(x13), .O(new_n87_));
  inv1   g44(.a(x14), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n87_), .c(x12), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  nand2  g48(.a(x20), .b(new_n91_), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  inv1   g50(.a(x21), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n92_), .O(new_n97_));
  aoi21  g54(.a(new_n97_), .b(new_n90_), .c(x00), .O(new_n98_));
  nand2  g55(.a(new_n95_), .b(new_n94_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(x00), .O(new_n100_));
  nor2   g57(.a(x20), .b(x19), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(x18), .d(x01), .O(new_n102_));
  oai21  g59(.a(new_n98_), .b(x01), .c(new_n102_), .O(z09));
  nand3  g60(.a(x20), .b(x16), .c(x15), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n99_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n90_), .c(x00), .O(new_n106_));
  nand3  g63(.a(x21), .b(x18), .c(x01), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n101_), .c(x22), .d(new_n54_), .O(new_n109_));
  oai21  g66(.a(new_n106_), .b(x01), .c(new_n109_), .O(z10));
  nand2  g67(.a(new_n108_), .b(new_n101_), .O(new_n111_));
  nand3  g68(.a(new_n94_), .b(x15), .c(new_n58_), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n92_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n90_), .O(new_n114_));
  nand2  g71(.a(new_n95_), .b(new_n54_), .O(new_n115_));
  aoi21  g72(.a(new_n114_), .b(new_n111_), .c(new_n115_), .O(z11));
  inv1   g73(.a(x23), .O(new_n117_));
  nand2  g74(.a(new_n62_), .b(x17), .O(new_n118_));
  oai21  g75(.a(new_n62_), .b(new_n117_), .c(new_n118_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n48_), .O(new_n120_));
  inv1   g77(.a(new_n68_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(x01), .c(x00), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n120_), .c(new_n124_), .O(z12));
  nor3   g82(.a(new_n118_), .b(x01), .c(x00), .O(z13));
  nand2  g83(.a(new_n44_), .b(new_n55_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(new_n49_), .O(z14));
  aoi21  g85(.a(x19), .b(new_n46_), .c(x00), .O(new_n129_));
  nand2  g86(.a(new_n121_), .b(x00), .O(new_n130_));
  aoi21  g87(.a(new_n130_), .b(x01), .c(new_n129_), .O(z15));
  nor2   g88(.a(new_n58_), .b(x00), .O(z16));
  nand2  g89(.a(new_n48_), .b(x02), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z17));
endmodule


