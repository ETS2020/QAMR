// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n73_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n141_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor2   g01(.a(x19), .b(x18), .O(new_n45_));
  inv1   g02(.a(x21), .O(new_n46_));
  nand2  g03(.a(x23), .b(new_n46_), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nor2   g08(.a(x01), .b(x00), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n48_), .O(z00));
  nand2  g11(.a(new_n52_), .b(new_n45_), .O(new_n55_));
  nor2   g12(.a(x17), .b(x02), .O(new_n56_));
  nand3  g13(.a(new_n56_), .b(x10), .c(x09), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(z01));
  nand4  g15(.a(new_n52_), .b(new_n50_), .c(x09), .d(new_n49_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n48_), .O(z02));
  inv1   g17(.a(new_n47_), .O(new_n61_));
  inv1   g18(.a(x12), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(new_n51_), .c(x11), .O(new_n65_));
  or2    g22(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  inv1   g23(.a(x19), .O(new_n67_));
  nand4  g24(.a(new_n56_), .b(new_n52_), .c(new_n67_), .d(x18), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(new_n66_), .c(new_n61_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  nand3  g27(.a(new_n51_), .b(new_n62_), .c(new_n70_), .O(new_n71_));
  oai21  g28(.a(new_n71_), .b(new_n63_), .c(new_n47_), .O(z04));
  nand4  g29(.a(new_n64_), .b(new_n47_), .c(x10), .d(x09), .O(new_n73_));
  inv1   g30(.a(new_n73_), .O(z05));
  nand2  g31(.a(new_n65_), .b(new_n47_), .O(z06));
  nand3  g32(.a(new_n51_), .b(new_n62_), .c(x11), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n47_), .O(z07));
  nor2   g36(.a(new_n44_), .b(x08), .O(new_n80_));
  nor2   g37(.a(new_n67_), .b(x18), .O(new_n81_));
  inv1   g38(.a(x05), .O(new_n82_));
  nor2   g39(.a(x07), .b(x06), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand3  g42(.a(new_n52_), .b(new_n85_), .c(x02), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n81_), .c(new_n80_), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(new_n47_), .O(z08));
  inv1   g46(.a(x23), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nor2   g48(.a(new_n49_), .b(x01), .O(new_n92_));
  nor2   g49(.a(new_n62_), .b(x11), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nand4  g54(.a(x20), .b(new_n97_), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  inv1   g55(.a(x20), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n67_), .c(x18), .d(x01), .O(new_n100_));
  oai21  g57(.a(new_n98_), .b(new_n94_), .c(new_n100_), .O(new_n101_));
  nor2   g58(.a(x22), .b(x00), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(new_n90_), .c(x21), .O(z09));
  inv1   g61(.a(x01), .O(new_n105_));
  nand2  g62(.a(x02), .b(new_n105_), .O(new_n106_));
  nand3  g63(.a(new_n91_), .b(x12), .c(new_n70_), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nor2   g65(.a(new_n100_), .b(new_n46_), .O(new_n109_));
  nand4  g66(.a(new_n46_), .b(x20), .c(x15), .d(new_n95_), .O(new_n110_));
  inv1   g67(.a(x22), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(x16), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  aoi22  g70(.a(new_n113_), .b(new_n108_), .c(new_n109_), .d(x22), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(x00), .c(new_n47_), .O(z10));
  inv1   g72(.a(new_n102_), .O(new_n116_));
  inv1   g73(.a(new_n109_), .O(new_n117_));
  inv1   g74(.a(new_n110_), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n108_), .c(new_n90_), .d(new_n97_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n117_), .c(new_n116_), .O(z11));
  nand2  g77(.a(x01), .b(x00), .O(new_n121_));
  aoi21  g78(.a(x10), .b(x02), .c(new_n121_), .O(new_n122_));
  oai21  g79(.a(x19), .b(x02), .c(x23), .O(new_n123_));
  nand3  g80(.a(new_n67_), .b(x17), .c(new_n49_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  and2   g82(.a(new_n125_), .b(new_n52_), .O(new_n126_));
  inv1   g83(.a(x09), .O(new_n127_));
  nor2   g84(.a(x24), .b(new_n127_), .O(new_n128_));
  oai21  g85(.a(new_n126_), .b(new_n122_), .c(new_n128_), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(new_n47_), .O(z12));
  inv1   g87(.a(new_n124_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n52_), .c(new_n47_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z13));
  nand3  g90(.a(new_n56_), .b(new_n50_), .c(new_n127_), .O(new_n134_));
  oai21  g91(.a(new_n134_), .b(new_n55_), .c(new_n47_), .O(z14));
  oai21  g92(.a(x10), .b(new_n105_), .c(x02), .O(new_n136_));
  nand2  g93(.a(new_n136_), .b(x00), .O(new_n137_));
  nand3  g94(.a(x19), .b(new_n49_), .c(new_n105_), .O(new_n138_));
  aoi21  g95(.a(new_n138_), .b(new_n137_), .c(new_n61_), .O(z15));
  oai21  g96(.a(new_n105_), .b(x00), .c(new_n47_), .O(z16));
  nand3  g97(.a(new_n52_), .b(new_n47_), .c(x02), .O(new_n141_));
  inv1   g98(.a(new_n141_), .O(z17));
endmodule


