// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n71_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n126_, new_n127_, new_n128_, new_n130_;
  nand2  g00(.a(x24), .b(x22), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nor2   g06(.a(x01), .b(x00), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  oai21  g08(.a(new_n51_), .b(new_n47_), .c(new_n44_), .O(z00));
  nand3  g09(.a(new_n50_), .b(x09), .c(new_n48_), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n47_), .c(new_n44_), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand4  g12(.a(new_n46_), .b(new_n44_), .c(new_n45_), .d(new_n55_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n53_), .O(z02));
  inv1   g14(.a(new_n44_), .O(new_n58_));
  inv1   g15(.a(x19), .O(new_n59_));
  nor2   g16(.a(x17), .b(x02), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n50_), .c(new_n59_), .d(x18), .O(new_n61_));
  inv1   g18(.a(x11), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  nor2   g20(.a(x09), .b(new_n48_), .O(new_n64_));
  nand2  g21(.a(x01), .b(x00), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n64_), .c(new_n63_), .d(x12), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(new_n61_), .c(new_n58_), .O(z03));
  nand3  g25(.a(new_n66_), .b(new_n64_), .c(new_n44_), .O(new_n69_));
  nor4   g26(.a(new_n69_), .b(x12), .c(x11), .d(new_n55_), .O(z04));
  nand3  g27(.a(x10), .b(x09), .c(x02), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n65_), .c(new_n58_), .O(z05));
  nor3   g29(.a(new_n69_), .b(new_n62_), .c(new_n55_), .O(z06));
  nor2   g30(.a(x12), .b(x09), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n63_), .O(new_n75_));
  nand3  g32(.a(new_n44_), .b(x02), .c(x00), .O(new_n76_));
  aoi21  g33(.a(new_n75_), .b(x01), .c(new_n76_), .O(z07));
  nand3  g34(.a(new_n50_), .b(new_n44_), .c(x02), .O(new_n78_));
  inv1   g35(.a(x18), .O(new_n79_));
  nand3  g36(.a(x19), .b(new_n79_), .c(x17), .O(new_n80_));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n80_), .c(new_n78_), .O(z08));
  inv1   g42(.a(x20), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n59_), .c(x18), .d(x01), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  inv1   g45(.a(x01), .O(new_n89_));
  nand2  g46(.a(x02), .b(new_n89_), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n62_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand4  g53(.a(x20), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  aoi21  g55(.a(new_n98_), .b(new_n93_), .c(new_n88_), .O(new_n99_));
  inv1   g56(.a(x00), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  oai21  g60(.a(new_n103_), .b(new_n99_), .c(new_n44_), .O(z09));
  nor2   g61(.a(new_n87_), .b(new_n101_), .O(new_n105_));
  nand4  g62(.a(new_n101_), .b(x20), .c(x15), .d(new_n94_), .O(new_n106_));
  nand2  g63(.a(new_n102_), .b(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi22  g65(.a(new_n108_), .b(new_n93_), .c(new_n105_), .d(x22), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g67(.a(new_n102_), .b(new_n100_), .O(new_n111_));
  inv1   g68(.a(new_n105_), .O(new_n112_));
  inv1   g69(.a(new_n106_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n93_), .c(new_n96_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(z11));
  aoi21  g72(.a(x10), .b(x02), .c(new_n65_), .O(new_n116_));
  inv1   g73(.a(new_n50_), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n59_), .b(x17), .c(new_n48_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  nor2   g77(.a(x24), .b(new_n49_), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n116_), .c(new_n121_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n44_), .O(z12));
  oai21  g80(.a(new_n119_), .b(new_n117_), .c(new_n44_), .O(z13));
  nor2   g81(.a(new_n56_), .b(new_n51_), .O(z14));
  oai21  g82(.a(x10), .b(new_n89_), .c(x02), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x00), .O(new_n127_));
  nand3  g84(.a(x19), .b(new_n48_), .c(new_n89_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n127_), .c(new_n44_), .O(z15));
  nand3  g86(.a(new_n44_), .b(x01), .c(new_n100_), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z16));
  inv1   g88(.a(new_n78_), .O(z17));
endmodule


