// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n116_, new_n117_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  nor3   g05(.a(new_n48_), .b(new_n44_), .c(x09), .O(z00));
  nand2  g06(.a(x10), .b(x09), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n48_), .O(z01));
  inv1   g08(.a(x01), .O(new_n52_));
  nand4  g09(.a(new_n46_), .b(new_n44_), .c(x09), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n52_), .c(x00), .O(z02));
  inv1   g11(.a(x00), .O(new_n55_));
  inv1   g12(.a(x17), .O(new_n56_));
  nor2   g13(.a(x19), .b(x02), .O(new_n57_));
  nand3  g14(.a(new_n57_), .b(x18), .c(new_n56_), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n52_), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  nor2   g17(.a(new_n44_), .b(x09), .O(new_n61_));
  nor2   g18(.a(new_n45_), .b(new_n52_), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(new_n61_), .c(x12), .d(x11), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n60_), .O(z03));
  nand2  g21(.a(new_n62_), .b(new_n61_), .O(new_n65_));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n66_), .c(x00), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n65_), .O(z04));
  nor4   g26(.a(new_n50_), .b(new_n45_), .c(new_n52_), .d(new_n55_), .O(z05));
  nand4  g27(.a(new_n62_), .b(new_n61_), .c(x11), .d(x00), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(z06));
  nand2  g29(.a(x01), .b(new_n55_), .O(new_n73_));
  inv1   g30(.a(x09), .O(new_n74_));
  nand4  g31(.a(new_n67_), .b(x11), .c(x10), .d(new_n74_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(x01), .O(new_n76_));
  nand3  g33(.a(new_n76_), .b(x02), .c(x00), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n73_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nor2   g39(.a(x18), .b(new_n56_), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n47_), .c(x19), .d(x02), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(new_n82_), .O(z08));
  inv1   g42(.a(x13), .O(new_n86_));
  inv1   g43(.a(x14), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(x12), .d(new_n66_), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  inv1   g46(.a(x16), .O(new_n90_));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(x20), .d(new_n90_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  nor2   g51(.a(x15), .b(new_n45_), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(new_n89_), .O(new_n96_));
  aoi21  g53(.a(new_n96_), .b(new_n52_), .c(x00), .O(z09));
  nand3  g54(.a(new_n47_), .b(x15), .c(x02), .O(new_n98_));
  nand4  g55(.a(new_n92_), .b(new_n91_), .c(x20), .d(x16), .O(new_n99_));
  nor3   g56(.a(new_n99_), .b(new_n98_), .c(new_n88_), .O(z10));
  nor3   g57(.a(new_n98_), .b(new_n93_), .c(new_n88_), .O(z11));
  aoi21  g58(.a(x10), .b(x02), .c(new_n52_), .O(new_n102_));
  inv1   g59(.a(x23), .O(new_n103_));
  inv1   g60(.a(x19), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n45_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  aoi21  g63(.a(new_n57_), .b(new_n56_), .c(x00), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n106_), .c(new_n102_), .O(new_n108_));
  inv1   g65(.a(x24), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(x09), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(new_n108_), .c(new_n73_), .O(z12));
  nand2  g68(.a(new_n47_), .b(x17), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n105_), .O(z13));
  nand4  g70(.a(new_n46_), .b(new_n44_), .c(new_n74_), .d(new_n45_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n52_), .c(x00), .O(z14));
  oai21  g72(.a(new_n44_), .b(new_n55_), .c(x01), .O(new_n116_));
  oai21  g73(.a(x19), .b(x00), .c(new_n45_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(z15));
  aoi21  g75(.a(new_n45_), .b(new_n52_), .c(x00), .O(z17));
  zero   g76(.O(z16));
endmodule


