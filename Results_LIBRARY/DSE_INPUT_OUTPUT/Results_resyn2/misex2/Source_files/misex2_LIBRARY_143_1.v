// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n135_;
  inv1   g00(.a(x23), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor3   g02(.a(x09), .b(x02), .c(x00), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  nand2  g06(.a(new_n45_), .b(x10), .O(new_n50_));
  nor3   g07(.a(x02), .b(x01), .c(x00), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(x09), .O(new_n52_));
  nand2  g09(.a(new_n47_), .b(new_n44_), .O(new_n53_));
  nor3   g10(.a(new_n53_), .b(new_n52_), .c(new_n50_), .O(z01));
  nor2   g11(.a(x17), .b(x10), .O(new_n55_));
  nand3  g12(.a(new_n55_), .b(new_n47_), .c(new_n44_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n52_), .O(z02));
  inv1   g14(.a(x09), .O(new_n58_));
  nand2  g15(.a(x10), .b(new_n58_), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(x12), .c(x11), .O(new_n62_));
  nor2   g19(.a(x23), .b(x19), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n51_), .c(x18), .d(new_n45_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n62_), .O(z03));
  inv1   g22(.a(new_n61_), .O(new_n66_));
  nor3   g23(.a(new_n66_), .b(x12), .c(x11), .O(z04));
  inv1   g24(.a(x01), .O(new_n68_));
  nand2  g25(.a(x23), .b(new_n68_), .O(new_n69_));
  inv1   g26(.a(x00), .O(new_n70_));
  nor2   g27(.a(new_n68_), .b(new_n70_), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(new_n72_));
  nand3  g29(.a(x10), .b(x09), .c(x02), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n72_), .c(new_n69_), .O(z05));
  inv1   g31(.a(x11), .O(new_n75_));
  nor2   g32(.a(new_n66_), .b(new_n75_), .O(z06));
  inv1   g33(.a(x12), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x11), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(new_n59_), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n69_), .O(z07));
  nor2   g38(.a(x18), .b(new_n45_), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(x04), .b(new_n86_), .c(x02), .d(new_n70_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n82_), .c(x19), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g47(.a(x19), .O(new_n91_));
  inv1   g48(.a(x20), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(x18), .d(x01), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  nand2  g51(.a(x02), .b(new_n68_), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n75_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nor3   g55(.a(x23), .b(x15), .c(x14), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(x20), .d(new_n94_), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n70_), .O(new_n103_));
  aoi21  g60(.a(new_n100_), .b(new_n93_), .c(new_n103_), .O(z09));
  nor2   g61(.a(new_n93_), .b(new_n101_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(x22), .O(new_n106_));
  nor3   g63(.a(x23), .b(x22), .c(x21), .O(new_n107_));
  inv1   g64(.a(x14), .O(new_n108_));
  nand4  g65(.a(x20), .b(x16), .c(x15), .d(new_n108_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n107_), .c(new_n98_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n106_), .c(x00), .O(z10));
  nand2  g69(.a(new_n102_), .b(new_n70_), .O(new_n113_));
  nand2  g70(.a(x15), .b(new_n108_), .O(new_n114_));
  nand3  g71(.a(new_n101_), .b(x20), .c(new_n94_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n98_), .c(new_n105_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n113_), .c(new_n69_), .O(z11));
  nand2  g75(.a(x10), .b(x02), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n71_), .O(new_n120_));
  nand2  g77(.a(new_n63_), .b(x17), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n51_), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n120_), .c(new_n125_), .O(z12));
  inv1   g83(.a(new_n123_), .O(z13));
  nand3  g84(.a(new_n55_), .b(new_n47_), .c(new_n46_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g86(.a(x19), .b(new_n68_), .c(x00), .O(new_n130_));
  inv1   g87(.a(x02), .O(new_n131_));
  nand2  g88(.a(new_n44_), .b(new_n131_), .O(new_n132_));
  oai21  g89(.a(new_n132_), .b(new_n130_), .c(new_n120_), .O(z15));
  aoi21  g90(.a(new_n44_), .b(new_n68_), .c(new_n71_), .O(z16));
  nand4  g91(.a(new_n44_), .b(x02), .c(new_n68_), .d(new_n70_), .O(new_n135_));
  inv1   g92(.a(new_n135_), .O(z17));
endmodule


