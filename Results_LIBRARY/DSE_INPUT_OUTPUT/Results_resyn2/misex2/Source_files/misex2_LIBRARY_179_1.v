// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n130_, new_n131_,
    new_n133_, new_n135_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  nor2   g06(.a(x19), .b(x18), .O(new_n50_));
  nor2   g07(.a(x21), .b(x17), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n49_), .O(z00));
  nand2  g10(.a(new_n48_), .b(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n52_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand3  g13(.a(new_n51_), .b(new_n50_), .c(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g15(.a(x11), .O(new_n59_));
  nand3  g16(.a(x10), .b(new_n44_), .c(x02), .O(new_n60_));
  nand2  g17(.a(x01), .b(x00), .O(new_n61_));
  nor3   g18(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(z06));
  nand2  g19(.a(z06), .b(x12), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nand4  g21(.a(new_n51_), .b(new_n48_), .c(new_n64_), .d(x18), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n63_), .O(z03));
  nor4   g23(.a(new_n61_), .b(new_n60_), .c(x12), .d(x11), .O(z04));
  nand2  g24(.a(x10), .b(x02), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(new_n61_), .c(new_n44_), .O(z05));
  inv1   g26(.a(x01), .O(new_n70_));
  nand2  g27(.a(x21), .b(new_n70_), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(x11), .c(x10), .d(new_n44_), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(x01), .O(new_n74_));
  nand3  g31(.a(new_n74_), .b(x02), .c(x00), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(new_n71_), .O(z07));
  inv1   g33(.a(x21), .O(new_n77_));
  inv1   g34(.a(x17), .O(new_n78_));
  nor2   g35(.a(x18), .b(new_n78_), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  inv1   g39(.a(x00), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nand4  g41(.a(x04), .b(new_n84_), .c(x02), .d(new_n83_), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(new_n79_), .c(x19), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(new_n77_), .c(x01), .O(z08));
  inv1   g45(.a(x22), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(new_n83_), .O(new_n90_));
  nand2  g47(.a(x02), .b(new_n70_), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n59_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nand4  g54(.a(x20), .b(new_n97_), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  inv1   g56(.a(x20), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n64_), .O(new_n101_));
  nand3  g58(.a(new_n77_), .b(x18), .c(x01), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  aoi21  g60(.a(new_n99_), .b(new_n94_), .c(new_n103_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n90_), .c(new_n71_), .O(z09));
  nand2  g62(.a(x15), .b(new_n95_), .O(new_n106_));
  nand3  g63(.a(new_n89_), .b(x20), .c(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g65(.a(new_n64_), .b(x18), .O(new_n109_));
  nand3  g66(.a(x22), .b(x21), .c(new_n100_), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  aoi21  g68(.a(new_n108_), .b(new_n94_), .c(new_n111_), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(x00), .c(new_n71_), .O(z10));
  inv1   g70(.a(new_n101_), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(x21), .c(x18), .d(x01), .O(new_n115_));
  nor2   g72(.a(new_n100_), .b(x16), .O(new_n116_));
  inv1   g73(.a(new_n106_), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(new_n116_), .c(new_n94_), .d(new_n77_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n115_), .c(new_n90_), .O(z11));
  nand3  g76(.a(new_n68_), .b(x01), .c(x00), .O(new_n120_));
  nor3   g77(.a(x19), .b(x17), .c(x02), .O(new_n121_));
  nor2   g78(.a(x19), .b(x02), .O(new_n122_));
  oai21  g79(.a(new_n122_), .b(x23), .c(new_n46_), .O(new_n123_));
  oai21  g80(.a(new_n123_), .b(new_n121_), .c(new_n120_), .O(new_n124_));
  nor2   g81(.a(x24), .b(new_n44_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n71_), .O(z12));
  nor4   g84(.a(new_n47_), .b(x21), .c(x19), .d(new_n78_), .O(z13));
  nor2   g85(.a(new_n57_), .b(new_n49_), .O(z14));
  aoi21  g86(.a(new_n56_), .b(x01), .c(new_n45_), .O(new_n130_));
  aoi21  g87(.a(x19), .b(new_n45_), .c(x21), .O(new_n131_));
  oai22  g88(.a(new_n131_), .b(x01), .c(new_n130_), .d(new_n83_), .O(z15));
  nor2   g89(.a(x21), .b(x01), .O(new_n133_));
  aoi21  g90(.a(x01), .b(x00), .c(new_n133_), .O(z16));
  nand2  g91(.a(x02), .b(new_n83_), .O(new_n135_));
  aoi21  g92(.a(new_n135_), .b(new_n77_), .c(x01), .O(z17));
endmodule


