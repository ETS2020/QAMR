// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n132_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x01), .O(new_n45_));
  nand3  g02(.a(x06), .b(new_n45_), .c(new_n44_), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x09), .O(new_n48_));
  nor3   g05(.a(x19), .b(x18), .c(x17), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nor3   g07(.a(new_n50_), .b(new_n46_), .c(x02), .O(z00));
  inv1   g08(.a(x02), .O(new_n52_));
  inv1   g09(.a(new_n46_), .O(new_n53_));
  nand3  g10(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  nand2  g11(.a(new_n49_), .b(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n54_), .O(z01));
  nand2  g13(.a(new_n49_), .b(new_n47_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g15(.a(x12), .O(new_n59_));
  nor2   g16(.a(new_n45_), .b(new_n44_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(x02), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand2  g19(.a(new_n48_), .b(x11), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  inv1   g22(.a(x17), .O(new_n66_));
  inv1   g23(.a(x18), .O(new_n67_));
  nor2   g24(.a(x19), .b(new_n67_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n53_), .c(new_n66_), .d(new_n52_), .O(new_n69_));
  oai21  g26(.a(new_n65_), .b(new_n59_), .c(new_n69_), .O(z03));
  inv1   g27(.a(x11), .O(new_n71_));
  nand3  g28(.a(new_n48_), .b(new_n59_), .c(new_n71_), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n61_), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nand2  g31(.a(new_n60_), .b(x09), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n74_), .O(z05));
  inv1   g33(.a(x06), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n45_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n65_), .O(z06));
  nand2  g36(.a(new_n64_), .b(new_n59_), .O(new_n80_));
  nand3  g37(.a(new_n78_), .b(x02), .c(x00), .O(new_n81_));
  aoi21  g38(.a(new_n80_), .b(x01), .c(new_n81_), .O(z07));
  inv1   g39(.a(new_n78_), .O(z08));
  inv1   g40(.a(x19), .O(new_n84_));
  inv1   g41(.a(x20), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x18), .d(x01), .O(new_n86_));
  inv1   g43(.a(x13), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(x12), .c(new_n71_), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  inv1   g46(.a(x14), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n90_), .c(x06), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nor2   g50(.a(new_n85_), .b(x16), .O(new_n94_));
  nor2   g51(.a(new_n52_), .b(x01), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(new_n89_), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n44_), .O(new_n99_));
  aoi21  g56(.a(new_n96_), .b(new_n86_), .c(new_n99_), .O(z09));
  nor2   g57(.a(new_n86_), .b(new_n97_), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(x22), .O(new_n102_));
  nand4  g59(.a(x15), .b(new_n90_), .c(x02), .d(new_n45_), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n88_), .O(new_n104_));
  nand3  g61(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n104_), .c(x16), .d(x06), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n102_), .c(x00), .O(z10));
  nand2  g65(.a(new_n98_), .b(new_n44_), .O(new_n109_));
  inv1   g66(.a(x16), .O(new_n110_));
  nand3  g67(.a(new_n97_), .b(x20), .c(new_n110_), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n104_), .c(new_n101_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n109_), .c(new_n78_), .O(z11));
  nand2  g71(.a(new_n74_), .b(new_n60_), .O(new_n115_));
  inv1   g72(.a(x23), .O(new_n116_));
  nor2   g73(.a(x19), .b(x02), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x17), .O(new_n118_));
  oai21  g75(.a(new_n117_), .b(new_n116_), .c(new_n118_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n53_), .O(new_n120_));
  inv1   g77(.a(x24), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x09), .O(new_n122_));
  aoi21  g79(.a(new_n120_), .b(new_n115_), .c(new_n122_), .O(z12));
  nor2   g80(.a(new_n118_), .b(new_n46_), .O(z13));
  nor3   g81(.a(x09), .b(x02), .c(x00), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(new_n49_), .c(new_n47_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(x06), .c(x01), .O(z14));
  aoi21  g84(.a(new_n47_), .b(x01), .c(new_n52_), .O(new_n128_));
  aoi21  g85(.a(x19), .b(new_n52_), .c(new_n77_), .O(new_n129_));
  oai22  g86(.a(new_n129_), .b(x01), .c(new_n128_), .d(new_n44_), .O(z15));
  nor2   g87(.a(new_n45_), .b(x00), .O(z16));
  nand2  g88(.a(new_n53_), .b(x02), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z17));
endmodule


