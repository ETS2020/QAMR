// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n128_, new_n129_, new_n130_, new_n132_;
  inv1   g00(.a(x01), .O(new_n44_));
  nor2   g01(.a(x16), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nor2   g08(.a(x01), .b(x00), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n50_), .c(new_n46_), .O(z00));
  nand2  g13(.a(new_n54_), .b(x09), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n50_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  nand3  g18(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  nor3   g19(.a(new_n62_), .b(new_n59_), .c(x09), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x16), .c(x12), .d(x11), .O(new_n64_));
  nand4  g21(.a(new_n54_), .b(new_n49_), .c(x18), .d(new_n47_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n64_), .O(z03));
  inv1   g23(.a(new_n62_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(x10), .c(new_n51_), .O(new_n68_));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand3  g27(.a(x16), .b(new_n70_), .c(new_n69_), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n68_), .O(z04));
  inv1   g29(.a(x16), .O(new_n73_));
  nor4   g30(.a(new_n62_), .b(new_n73_), .c(new_n59_), .d(new_n51_), .O(z05));
  nor3   g31(.a(new_n68_), .b(new_n73_), .c(new_n69_), .O(z06));
  nand4  g32(.a(new_n70_), .b(x11), .c(x10), .d(new_n51_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n46_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand2  g40(.a(new_n52_), .b(x02), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(z17));
  nand4  g42(.a(z17), .b(x19), .c(new_n48_), .d(x17), .O(new_n86_));
  oai21  g43(.a(new_n86_), .b(new_n83_), .c(new_n46_), .O(z08));
  inv1   g44(.a(x20), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n49_), .c(x18), .d(x01), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x12), .c(new_n69_), .d(x02), .O(new_n91_));
  inv1   g48(.a(x14), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  nand4  g50(.a(x20), .b(new_n73_), .c(new_n93_), .d(new_n92_), .O(new_n94_));
  oai21  g51(.a(new_n94_), .b(new_n91_), .c(new_n89_), .O(new_n95_));
  inv1   g52(.a(x00), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(x21), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n95_), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n46_), .O(z09));
  nor2   g58(.a(new_n45_), .b(new_n96_), .O(new_n102_));
  nor2   g59(.a(new_n91_), .b(x01), .O(new_n103_));
  inv1   g60(.a(x21), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(x20), .c(x15), .d(new_n92_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nor2   g63(.a(x22), .b(new_n73_), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(new_n106_), .c(new_n103_), .O(new_n108_));
  inv1   g65(.a(new_n89_), .O(new_n109_));
  nor2   g66(.a(new_n97_), .b(new_n104_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n109_), .c(new_n45_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n108_), .c(new_n102_), .O(z10));
  nand3  g69(.a(new_n106_), .b(new_n103_), .c(new_n73_), .O(new_n113_));
  nand3  g70(.a(new_n109_), .b(x21), .c(x16), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n113_), .c(new_n98_), .O(z11));
  nand2  g72(.a(x10), .b(x02), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(x16), .c(x01), .d(x00), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  inv1   g75(.a(x02), .O(new_n119_));
  nand3  g76(.a(new_n49_), .b(x17), .c(new_n119_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n52_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n117_), .c(new_n124_), .O(z12));
  nor2   g82(.a(new_n120_), .b(new_n53_), .O(z13));
  nor2   g83(.a(new_n60_), .b(new_n55_), .O(z14));
  nor2   g84(.a(new_n49_), .b(x01), .O(new_n128_));
  oai21  g85(.a(new_n128_), .b(new_n102_), .c(new_n119_), .O(new_n129_));
  nand4  g86(.a(x16), .b(new_n59_), .c(x01), .d(x00), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n129_), .O(z15));
  nand3  g88(.a(x16), .b(x01), .c(new_n96_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z16));
endmodule


