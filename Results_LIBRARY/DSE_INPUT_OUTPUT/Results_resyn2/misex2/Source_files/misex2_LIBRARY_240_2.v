// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n124_, new_n125_,
    new_n126_;
  inv1   g00(.a(x15), .O(new_n44_));
  nor2   g01(.a(x20), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(x02), .O(new_n51_));
  inv1   g08(.a(x09), .O(new_n52_));
  nor2   g09(.a(x01), .b(x00), .O(new_n53_));
  nand3  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n50_), .c(new_n46_), .O(z00));
  nand3  g12(.a(new_n53_), .b(x09), .c(new_n51_), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n50_), .c(new_n46_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n56_), .c(new_n46_), .O(z02));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x02), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nor2   g22(.a(new_n58_), .b(x09), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(x11), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n65_), .c(x12), .O(new_n69_));
  nor2   g26(.a(x17), .b(x02), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n53_), .c(new_n49_), .d(x18), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(new_n69_), .c(new_n45_), .O(z03));
  nor2   g29(.a(x12), .b(x11), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n66_), .c(new_n65_), .d(new_n46_), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(z04));
  nor4   g32(.a(new_n64_), .b(new_n45_), .c(new_n58_), .d(new_n52_), .O(z05));
  oai21  g33(.a(new_n67_), .b(new_n64_), .c(new_n46_), .O(z06));
  oai21  g34(.a(new_n67_), .b(x12), .c(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n46_), .O(z07));
  nand3  g37(.a(new_n53_), .b(new_n46_), .c(x02), .O(new_n81_));
  nand3  g38(.a(x19), .b(new_n48_), .c(x17), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nor3   g43(.a(new_n86_), .b(new_n82_), .c(new_n81_), .O(z08));
  inv1   g44(.a(x20), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n49_), .c(x18), .d(x01), .O(new_n89_));
  inv1   g46(.a(x16), .O(new_n90_));
  nand2  g47(.a(x02), .b(new_n62_), .O(new_n91_));
  inv1   g48(.a(x11), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n93_), .c(x12), .d(new_n92_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x20), .c(new_n90_), .O(new_n97_));
  inv1   g54(.a(x21), .O(new_n98_));
  inv1   g55(.a(x22), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(new_n44_), .d(new_n61_), .O(new_n100_));
  aoi21  g57(.a(new_n97_), .b(new_n89_), .c(new_n100_), .O(z09));
  nor2   g58(.a(new_n89_), .b(new_n98_), .O(new_n102_));
  nand4  g59(.a(new_n99_), .b(new_n98_), .c(x16), .d(x15), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  aoi22  g61(.a(new_n104_), .b(new_n96_), .c(new_n102_), .d(x22), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(x00), .c(new_n46_), .O(z10));
  nand3  g63(.a(new_n98_), .b(new_n90_), .c(x15), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n96_), .c(new_n102_), .O(new_n109_));
  nand2  g66(.a(new_n99_), .b(new_n61_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(new_n109_), .c(new_n46_), .O(z11));
  oai21  g68(.a(new_n58_), .b(new_n51_), .c(new_n63_), .O(new_n112_));
  inv1   g69(.a(x23), .O(new_n113_));
  nor2   g70(.a(x19), .b(x02), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(x17), .O(new_n115_));
  oai21  g72(.a(new_n114_), .b(new_n113_), .c(new_n115_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n53_), .O(new_n117_));
  inv1   g74(.a(x24), .O(new_n118_));
  nand3  g75(.a(new_n46_), .b(new_n118_), .c(x09), .O(new_n119_));
  aoi21  g76(.a(new_n117_), .b(new_n112_), .c(new_n119_), .O(z12));
  nand3  g77(.a(new_n114_), .b(new_n53_), .c(x17), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n46_), .O(z13));
  nor3   g79(.a(new_n59_), .b(new_n54_), .c(new_n45_), .O(z14));
  oai21  g80(.a(x10), .b(new_n62_), .c(x02), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x00), .O(new_n125_));
  nand3  g82(.a(x19), .b(new_n51_), .c(new_n62_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n125_), .c(new_n45_), .O(z15));
  nor3   g84(.a(new_n45_), .b(new_n62_), .c(x00), .O(z16));
  inv1   g85(.a(new_n81_), .O(z17));
endmodule


