// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n135_;
  inv1   g00(.a(x19), .O(new_n44_));
  nand2  g01(.a(x21), .b(x16), .O(new_n45_));
  nor2   g02(.a(x18), .b(x17), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(x10), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nor2   g08(.a(x01), .b(x00), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n49_), .O(z00));
  nand3  g11(.a(new_n52_), .b(x09), .c(new_n50_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n49_), .O(z01));
  nor3   g13(.a(new_n55_), .b(new_n47_), .c(x10), .O(z02));
  nand3  g14(.a(x02), .b(x01), .c(x00), .O(new_n58_));
  nand3  g15(.a(x11), .b(x10), .c(new_n51_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(x12), .O(new_n61_));
  inv1   g18(.a(x17), .O(new_n62_));
  nor2   g19(.a(x19), .b(x02), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n52_), .c(x18), .d(new_n62_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(new_n61_), .c(new_n45_), .O(z03));
  nor2   g22(.a(x12), .b(x11), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n45_), .c(x10), .d(new_n51_), .O(new_n67_));
  nor2   g24(.a(new_n67_), .b(new_n58_), .O(z04));
  nand2  g25(.a(x10), .b(x02), .O(new_n69_));
  inv1   g26(.a(x00), .O(new_n70_));
  inv1   g27(.a(x01), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(x09), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n69_), .c(new_n45_), .O(z05));
  oai21  g31(.a(new_n59_), .b(new_n58_), .c(new_n45_), .O(z06));
  oai21  g32(.a(new_n59_), .b(x12), .c(x01), .O(new_n76_));
  nand3  g33(.a(new_n76_), .b(x02), .c(x00), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n45_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nor2   g39(.a(x18), .b(new_n62_), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n52_), .c(x19), .d(x02), .O(new_n84_));
  oai21  g41(.a(new_n84_), .b(new_n82_), .c(new_n45_), .O(z08));
  inv1   g42(.a(x20), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n44_), .c(x18), .d(x01), .O(new_n87_));
  inv1   g44(.a(x11), .O(new_n88_));
  nand4  g45(.a(x12), .b(new_n88_), .c(x02), .d(new_n71_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nor2   g47(.a(x14), .b(x13), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nand3  g50(.a(x20), .b(new_n93_), .c(new_n92_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n91_), .c(new_n90_), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n70_), .O(new_n99_));
  aoi21  g56(.a(new_n96_), .b(new_n87_), .c(new_n99_), .O(z09));
  inv1   g57(.a(new_n87_), .O(new_n101_));
  nor2   g58(.a(new_n98_), .b(x16), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(x21), .O(new_n103_));
  inv1   g60(.a(x13), .O(new_n104_));
  inv1   g61(.a(x14), .O(new_n105_));
  nand4  g62(.a(new_n97_), .b(x20), .c(new_n105_), .d(new_n104_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand2  g64(.a(x16), .b(x15), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(x22), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n107_), .c(new_n90_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n103_), .c(x00), .O(z10));
  nand2  g68(.a(new_n101_), .b(x21), .O(new_n112_));
  nand3  g69(.a(new_n107_), .b(new_n90_), .c(x15), .O(new_n113_));
  nand3  g70(.a(new_n98_), .b(new_n93_), .c(new_n70_), .O(new_n114_));
  aoi21  g71(.a(new_n113_), .b(new_n112_), .c(new_n114_), .O(z11));
  nand2  g72(.a(new_n69_), .b(new_n72_), .O(new_n116_));
  nand2  g73(.a(new_n63_), .b(new_n62_), .O(new_n117_));
  inv1   g74(.a(x23), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(new_n118_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n117_), .c(new_n52_), .O(new_n120_));
  inv1   g77(.a(x24), .O(new_n121_));
  nand3  g78(.a(new_n45_), .b(new_n121_), .c(x09), .O(new_n122_));
  aoi21  g79(.a(new_n120_), .b(new_n116_), .c(new_n122_), .O(z12));
  inv1   g80(.a(new_n45_), .O(new_n124_));
  nand4  g81(.a(x17), .b(new_n50_), .c(new_n71_), .d(new_n70_), .O(new_n125_));
  nor3   g82(.a(new_n125_), .b(new_n124_), .c(x19), .O(z13));
  nor2   g83(.a(x19), .b(x10), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n46_), .O(new_n128_));
  oai21  g85(.a(new_n128_), .b(new_n53_), .c(new_n45_), .O(z14));
  oai21  g86(.a(x10), .b(new_n71_), .c(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand3  g88(.a(x19), .b(new_n50_), .c(new_n71_), .O(new_n132_));
  aoi21  g89(.a(new_n132_), .b(new_n131_), .c(new_n124_), .O(z15));
  oai21  g90(.a(new_n71_), .b(x00), .c(new_n45_), .O(z16));
  nand3  g91(.a(new_n52_), .b(new_n45_), .c(x02), .O(new_n135_));
  inv1   g92(.a(new_n135_), .O(z17));
endmodule


