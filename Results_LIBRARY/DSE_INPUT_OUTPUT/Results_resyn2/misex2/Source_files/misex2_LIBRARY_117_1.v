// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n121_, new_n122_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n46_), .c(new_n45_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x00), .O(z00));
  nor2   g10(.a(x01), .b(x00), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(x09), .c(new_n45_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n50_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g16(.a(x00), .O(new_n60_));
  nand2  g17(.a(new_n49_), .b(new_n60_), .O(new_n61_));
  nand4  g18(.a(x18), .b(new_n47_), .c(new_n45_), .d(new_n44_), .O(new_n62_));
  nand3  g19(.a(x11), .b(x10), .c(new_n46_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(x12), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n63_), .c(new_n62_), .d(new_n61_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n46_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n64_), .O(z04));
  nand3  g28(.a(x10), .b(x09), .c(x02), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x00), .c(new_n44_), .O(z05));
  inv1   g30(.a(new_n63_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x02), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x00), .c(new_n44_), .O(z06));
  nand2  g33(.a(x02), .b(x00), .O(new_n77_));
  nand2  g34(.a(new_n74_), .b(new_n69_), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(x01), .c(new_n77_), .O(z07));
  inv1   g36(.a(x08), .O(new_n80_));
  nand4  g37(.a(x19), .b(new_n48_), .c(x17), .d(new_n80_), .O(new_n81_));
  inv1   g38(.a(x05), .O(new_n82_));
  nor2   g39(.a(x07), .b(x06), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand3  g42(.a(new_n54_), .b(new_n85_), .c(x02), .O(new_n86_));
  nor3   g43(.a(new_n86_), .b(new_n84_), .c(new_n81_), .O(z08));
  inv1   g44(.a(x15), .O(new_n88_));
  inv1   g45(.a(x14), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(x02), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  nand2  g48(.a(new_n54_), .b(new_n91_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  inv1   g50(.a(x21), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(x20), .O(new_n96_));
  inv1   g53(.a(x13), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(x12), .c(new_n68_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n93_), .c(new_n88_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(z09));
  inv1   g58(.a(new_n96_), .O(new_n102_));
  inv1   g59(.a(new_n98_), .O(new_n103_));
  nand4  g60(.a(x16), .b(x15), .c(new_n89_), .d(x02), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n44_), .c(x00), .O(z10));
  nand3  g64(.a(new_n99_), .b(new_n93_), .c(x15), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(z11));
  aoi21  g66(.a(x10), .b(x02), .c(new_n44_), .O(new_n110_));
  oai21  g67(.a(x19), .b(x02), .c(x23), .O(new_n111_));
  nand3  g68(.a(new_n49_), .b(x17), .c(new_n45_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n111_), .c(x00), .O(new_n113_));
  nor2   g70(.a(x24), .b(new_n46_), .O(new_n114_));
  oai21  g71(.a(new_n113_), .b(new_n110_), .c(new_n114_), .O(new_n115_));
  nand2  g72(.a(x01), .b(new_n60_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n115_), .O(z12));
  inv1   g74(.a(new_n54_), .O(new_n118_));
  nor2   g75(.a(new_n112_), .b(new_n118_), .O(z13));
  nor4   g76(.a(new_n58_), .b(new_n118_), .c(x09), .d(x02), .O(z14));
  oai21  g77(.a(new_n57_), .b(new_n60_), .c(x01), .O(new_n121_));
  nand2  g78(.a(new_n61_), .b(new_n45_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n121_), .O(z15));
  inv1   g80(.a(new_n116_), .O(z16));
  aoi21  g81(.a(new_n45_), .b(new_n44_), .c(x00), .O(z17));
endmodule


