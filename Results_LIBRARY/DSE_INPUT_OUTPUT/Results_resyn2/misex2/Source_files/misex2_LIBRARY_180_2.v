// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n126_, new_n127_, new_n128_, new_n130_;
  inv1   g00(.a(x02), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n47_), .O(z00));
  nand3  g09(.a(new_n46_), .b(x09), .c(new_n44_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n51_), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand4  g12(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n55_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n53_), .O(z02));
  inv1   g14(.a(x01), .O(new_n58_));
  nand2  g15(.a(x02), .b(x00), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x09), .O(new_n60_));
  nand3  g17(.a(x12), .b(x11), .c(x10), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(new_n60_), .c(new_n50_), .O(new_n63_));
  nor2   g20(.a(x02), .b(x00), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n50_), .c(x18), .d(new_n48_), .O(new_n65_));
  oai21  g22(.a(new_n63_), .b(new_n58_), .c(new_n65_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  inv1   g24(.a(x12), .O(new_n68_));
  nand4  g25(.a(new_n60_), .b(new_n68_), .c(new_n67_), .d(x10), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x19), .c(new_n58_), .O(z04));
  nand4  g27(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x19), .c(new_n58_), .O(z05));
  nand4  g29(.a(x19), .b(x11), .c(x10), .d(x01), .O(new_n73_));
  nor3   g30(.a(new_n73_), .b(new_n59_), .c(x09), .O(z06));
  nor2   g31(.a(new_n55_), .b(x09), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(x19), .c(new_n68_), .d(x11), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x01), .c(new_n59_), .O(z07));
  nand2  g34(.a(new_n50_), .b(x01), .O(new_n78_));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  inv1   g39(.a(x00), .O(new_n83_));
  nand3  g40(.a(x02), .b(new_n58_), .c(new_n83_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(z17));
  nor2   g42(.a(x18), .b(new_n48_), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(z17), .c(x19), .O(new_n87_));
  oai21  g44(.a(new_n87_), .b(new_n82_), .c(new_n78_), .O(z08));
  inv1   g45(.a(x16), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(x12), .c(new_n67_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n84_), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n95_));
  inv1   g52(.a(x14), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n95_), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n92_), .c(new_n89_), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n78_), .O(z09));
  inv1   g58(.a(new_n95_), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(x15), .c(new_n96_), .O(new_n103_));
  inv1   g60(.a(new_n91_), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(z17), .c(x16), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(new_n103_), .O(z10));
  nand2  g63(.a(x15), .b(new_n96_), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n95_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n92_), .c(new_n89_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n78_), .O(z11));
  nand2  g67(.a(x10), .b(x02), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(x01), .c(x00), .O(new_n112_));
  nand3  g69(.a(x23), .b(new_n58_), .c(new_n83_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(x19), .O(new_n115_));
  nand2  g72(.a(x23), .b(x02), .O(new_n116_));
  nand3  g73(.a(new_n50_), .b(x17), .c(new_n44_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n46_), .O(new_n119_));
  inv1   g76(.a(x24), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x09), .O(new_n121_));
  aoi21  g78(.a(new_n119_), .b(new_n115_), .c(new_n121_), .O(z12));
  nand2  g79(.a(new_n64_), .b(x17), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n58_), .c(x19), .O(z13));
  nor2   g81(.a(new_n56_), .b(new_n47_), .O(z14));
  oai21  g82(.a(x10), .b(new_n58_), .c(x02), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x00), .O(new_n127_));
  nand3  g84(.a(x19), .b(new_n44_), .c(new_n58_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n127_), .c(new_n78_), .O(z15));
  nand3  g86(.a(x19), .b(x01), .c(new_n83_), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z16));
endmodule


