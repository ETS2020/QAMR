// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n138_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(new_n46_), .b(x02), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand2  g10(.a(new_n47_), .b(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n52_), .O(z01));
  nand2  g12(.a(x17), .b(x11), .O(new_n56_));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n54_), .c(new_n56_), .O(z02));
  nand3  g16(.a(new_n47_), .b(new_n51_), .c(x18), .O(new_n60_));
  nand3  g17(.a(x11), .b(x10), .c(new_n44_), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand2  g19(.a(x01), .b(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n62_), .c(x12), .d(x02), .O(new_n65_));
  aoi21  g22(.a(new_n65_), .b(new_n60_), .c(x17), .O(z03));
  inv1   g23(.a(x02), .O(new_n67_));
  nor2   g24(.a(new_n57_), .b(new_n67_), .O(new_n68_));
  nor2   g25(.a(x12), .b(x11), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(new_n64_), .d(new_n44_), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(z04));
  nand2  g28(.a(new_n68_), .b(new_n64_), .O(new_n72_));
  nand2  g29(.a(new_n56_), .b(x09), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n72_), .O(z05));
  inv1   g31(.a(x11), .O(new_n75_));
  nand3  g32(.a(new_n68_), .b(new_n64_), .c(new_n44_), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(new_n49_), .c(new_n75_), .O(z06));
  oai21  g34(.a(new_n61_), .b(x12), .c(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n56_), .O(z07));
  nand2  g37(.a(new_n45_), .b(x02), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nor2   g40(.a(new_n51_), .b(x18), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nor2   g42(.a(x06), .b(x05), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(x04), .c(new_n85_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(new_n75_), .c(new_n49_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n51_), .c(x18), .d(x01), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n56_), .O(new_n94_));
  inv1   g51(.a(x01), .O(new_n95_));
  nand2  g52(.a(x02), .b(new_n95_), .O(new_n96_));
  inv1   g53(.a(x13), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(x12), .c(new_n75_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nor2   g56(.a(x15), .b(x14), .O(new_n100_));
  nor2   g57(.a(new_n91_), .b(x16), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  inv1   g59(.a(x00), .O(new_n103_));
  inv1   g60(.a(x21), .O(new_n104_));
  inv1   g61(.a(x22), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  aoi21  g63(.a(new_n102_), .b(new_n94_), .c(new_n106_), .O(z09));
  nor2   g64(.a(new_n92_), .b(new_n104_), .O(new_n108_));
  inv1   g65(.a(x14), .O(new_n109_));
  nand4  g66(.a(new_n104_), .b(x20), .c(x15), .d(new_n109_), .O(new_n110_));
  nand2  g67(.a(new_n105_), .b(x16), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  aoi22  g69(.a(new_n112_), .b(new_n99_), .c(new_n108_), .d(x22), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(x00), .c(new_n56_), .O(z10));
  nand2  g71(.a(new_n105_), .b(new_n103_), .O(new_n115_));
  nor2   g72(.a(new_n110_), .b(x16), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n99_), .c(new_n108_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n115_), .c(new_n56_), .O(z11));
  inv1   g75(.a(x23), .O(new_n119_));
  oai22  g76(.a(new_n81_), .b(new_n119_), .c(new_n68_), .d(new_n63_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n56_), .O(new_n121_));
  nand4  g78(.a(new_n51_), .b(x17), .c(new_n75_), .d(new_n67_), .O(new_n122_));
  nor2   g79(.a(x19), .b(x17), .O(new_n123_));
  nand2  g80(.a(new_n56_), .b(x23), .O(new_n124_));
  oai21  g81(.a(new_n124_), .b(new_n123_), .c(new_n122_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n45_), .O(new_n126_));
  inv1   g83(.a(x24), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x09), .O(new_n128_));
  aoi21  g85(.a(new_n126_), .b(new_n121_), .c(new_n128_), .O(z12));
  nor2   g86(.a(new_n122_), .b(new_n46_), .O(z13));
  nor2   g87(.a(new_n58_), .b(new_n48_), .O(z14));
  oai21  g88(.a(x10), .b(new_n95_), .c(x02), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(x00), .O(new_n133_));
  nand3  g90(.a(x19), .b(new_n67_), .c(new_n95_), .O(new_n134_));
  nand3  g91(.a(new_n134_), .b(new_n133_), .c(new_n56_), .O(z15));
  nand3  g92(.a(new_n56_), .b(x01), .c(new_n103_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z16));
  nand2  g94(.a(new_n82_), .b(new_n56_), .O(new_n138_));
  inv1   g95(.a(new_n138_), .O(z17));
endmodule


