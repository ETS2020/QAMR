// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n126_, new_n128_, new_n129_;
  inv1   g00(.a(x02), .O(new_n44_));
  nor3   g01(.a(x19), .b(x18), .c(x17), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  inv1   g04(.a(x09), .O(new_n48_));
  nand2  g05(.a(x10), .b(new_n48_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n47_), .O(z00));
  nand2  g07(.a(x10), .b(x09), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n47_), .O(z01));
  inv1   g09(.a(x01), .O(new_n53_));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n45_), .b(new_n54_), .c(x09), .d(new_n44_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n53_), .c(x00), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x17), .O(new_n58_));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x18), .c(new_n58_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nand4  g19(.a(x11), .b(x10), .c(new_n48_), .d(x02), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(x12), .c(x01), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n62_), .O(z03));
  nor2   g23(.a(new_n53_), .b(new_n57_), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(x02), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(x10), .c(new_n48_), .O(new_n70_));
  nor3   g27(.a(new_n70_), .b(new_n68_), .c(x11), .O(z04));
  nor2   g28(.a(new_n68_), .b(new_n51_), .O(z05));
  aoi21  g29(.a(new_n63_), .b(x00), .c(new_n53_), .O(z06));
  inv1   g30(.a(new_n70_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x11), .O(new_n75_));
  inv1   g32(.a(new_n46_), .O(new_n76_));
  oai21  g33(.a(x02), .b(new_n57_), .c(new_n76_), .O(new_n77_));
  aoi21  g34(.a(new_n75_), .b(new_n67_), .c(new_n77_), .O(z07));
  inv1   g35(.a(x08), .O(new_n79_));
  inv1   g36(.a(x18), .O(new_n80_));
  nand4  g37(.a(x19), .b(new_n80_), .c(x17), .d(new_n79_), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nor2   g39(.a(x03), .b(new_n44_), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  inv1   g41(.a(x06), .O(new_n85_));
  inv1   g42(.a(x07), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(x04), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n83_), .c(new_n82_), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(new_n53_), .c(x00), .O(z08));
  inv1   g47(.a(x15), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand2  g49(.a(new_n92_), .b(x12), .O(new_n93_));
  inv1   g50(.a(x11), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n93_), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(x20), .O(new_n101_));
  nand2  g58(.a(new_n46_), .b(x02), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n98_), .c(new_n91_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(z09));
  inv1   g62(.a(new_n101_), .O(new_n106_));
  nand3  g63(.a(x16), .b(x15), .c(new_n95_), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand4  g65(.a(new_n92_), .b(x12), .c(new_n94_), .d(x02), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n53_), .c(x00), .O(z10));
  nand3  g69(.a(new_n103_), .b(new_n98_), .c(x15), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(z11));
  oai21  g71(.a(new_n54_), .b(new_n44_), .c(new_n67_), .O(new_n115_));
  nand2  g72(.a(new_n59_), .b(new_n58_), .O(new_n116_));
  inv1   g73(.a(x23), .O(new_n117_));
  inv1   g74(.a(x19), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n44_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n116_), .c(new_n46_), .O(new_n121_));
  inv1   g78(.a(x24), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x09), .O(new_n123_));
  aoi21  g80(.a(new_n121_), .b(new_n115_), .c(new_n123_), .O(z12));
  nor3   g81(.a(new_n119_), .b(new_n76_), .c(new_n58_), .O(z13));
  nand2  g82(.a(new_n54_), .b(new_n48_), .O(new_n126_));
  nor2   g83(.a(new_n126_), .b(new_n47_), .O(z14));
  aoi21  g84(.a(new_n54_), .b(x01), .c(new_n44_), .O(new_n128_));
  nand3  g85(.a(x19), .b(new_n44_), .c(new_n53_), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(new_n57_), .c(new_n129_), .O(z15));
  inv1   g87(.a(new_n102_), .O(z17));
  zero   g88(.O(z16));
endmodule


