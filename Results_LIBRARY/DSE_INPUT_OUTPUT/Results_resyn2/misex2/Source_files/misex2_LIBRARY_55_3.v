// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n73_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n128_;
  nand2  g00(.a(x15), .b(x01), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nor2   g06(.a(x01), .b(x00), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x02), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand2  g11(.a(new_n52_), .b(x09), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  nand2  g17(.a(x02), .b(x00), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n49_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g20(.a(new_n63_), .b(x12), .c(x15), .O(new_n64_));
  nand4  g21(.a(new_n52_), .b(new_n47_), .c(x18), .d(new_n45_), .O(new_n65_));
  oai21  g22(.a(new_n64_), .b(new_n60_), .c(new_n65_), .O(z03));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g24(.a(x15), .O(new_n68_));
  nor2   g25(.a(x12), .b(x11), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n49_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n67_), .O(z04));
  nor4   g28(.a(new_n67_), .b(x15), .c(new_n57_), .d(new_n49_), .O(z05));
  nor2   g29(.a(new_n63_), .b(x15), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n60_), .O(z06));
  inv1   g31(.a(new_n61_), .O(new_n75_));
  oai21  g32(.a(new_n62_), .b(x12), .c(x01), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n44_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nand2  g39(.a(new_n50_), .b(x02), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n46_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(z08));
  inv1   g42(.a(x20), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n47_), .c(x18), .d(x01), .O(new_n87_));
  inv1   g44(.a(x11), .O(new_n88_));
  inv1   g45(.a(x13), .O(new_n89_));
  inv1   g46(.a(x14), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(x02), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nor2   g49(.a(new_n86_), .b(x16), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(x12), .d(new_n60_), .O(new_n94_));
  inv1   g51(.a(x00), .O(new_n95_));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(new_n68_), .d(new_n95_), .O(new_n98_));
  aoi21  g55(.a(new_n94_), .b(new_n87_), .c(new_n98_), .O(z09));
  nor2   g56(.a(new_n87_), .b(new_n96_), .O(new_n100_));
  nand2  g57(.a(x15), .b(x12), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(new_n91_), .O(new_n102_));
  nand4  g59(.a(new_n97_), .b(new_n96_), .c(x20), .d(x16), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  aoi22  g61(.a(new_n104_), .b(new_n102_), .c(new_n100_), .d(x22), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(x00), .c(new_n44_), .O(z10));
  inv1   g63(.a(x16), .O(new_n107_));
  nand3  g64(.a(new_n96_), .b(x20), .c(new_n107_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n102_), .c(new_n100_), .O(new_n110_));
  nand2  g67(.a(new_n97_), .b(new_n95_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(new_n110_), .c(new_n44_), .O(z11));
  inv1   g69(.a(x02), .O(new_n113_));
  nor2   g70(.a(new_n57_), .b(new_n113_), .O(new_n114_));
  nand3  g71(.a(new_n68_), .b(x01), .c(x00), .O(new_n115_));
  or2    g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  inv1   g73(.a(x23), .O(new_n117_));
  nor2   g74(.a(x19), .b(x02), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x17), .O(new_n119_));
  oai21  g76(.a(new_n118_), .b(new_n117_), .c(new_n119_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n50_), .O(new_n121_));
  inv1   g78(.a(x24), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x09), .O(new_n123_));
  aoi21  g80(.a(new_n121_), .b(new_n116_), .c(new_n123_), .O(z12));
  oai21  g81(.a(new_n119_), .b(new_n51_), .c(new_n44_), .O(z13));
  nor2   g82(.a(new_n58_), .b(new_n53_), .O(z14));
  aoi21  g83(.a(new_n68_), .b(x00), .c(new_n60_), .O(z16));
  oai21  g84(.a(x19), .b(x00), .c(new_n113_), .O(new_n128_));
  oai22  g85(.a(new_n128_), .b(z16), .c(new_n115_), .d(x10), .O(z15));
  inv1   g86(.a(new_n83_), .O(z17));
endmodule


