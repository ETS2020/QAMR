// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n121_, new_n123_, new_n124_, new_n127_;
  inv1   g00(.a(x15), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x02), .O(new_n47_));
  nor2   g04(.a(x19), .b(x18), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  inv1   g07(.a(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x09), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n50_), .c(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x01), .O(z00));
  nand2  g11(.a(x10), .b(x09), .O(new_n55_));
  nor2   g12(.a(x01), .b(x00), .O(new_n56_));
  nand3  g13(.a(new_n56_), .b(new_n44_), .c(new_n47_), .O(new_n57_));
  inv1   g14(.a(new_n57_), .O(new_n58_));
  nand3  g15(.a(new_n58_), .b(new_n48_), .c(new_n45_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n55_), .O(z01));
  nand4  g17(.a(new_n50_), .b(new_n45_), .c(new_n51_), .d(x09), .O(new_n61_));
  aoi21  g18(.a(new_n61_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g19(.a(x12), .O(new_n63_));
  inv1   g20(.a(x18), .O(new_n64_));
  nor2   g21(.a(x19), .b(new_n64_), .O(new_n65_));
  nor3   g22(.a(x17), .b(x02), .c(x00), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(new_n65_), .c(x15), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(new_n52_), .c(x11), .O(new_n70_));
  oai22  g27(.a(new_n70_), .b(new_n63_), .c(new_n67_), .d(x01), .O(z03));
  nand2  g28(.a(new_n69_), .b(new_n52_), .O(new_n72_));
  nor3   g29(.a(new_n72_), .b(x12), .c(x11), .O(z04));
  inv1   g30(.a(x01), .O(new_n74_));
  nand2  g31(.a(x15), .b(new_n74_), .O(new_n75_));
  oai21  g32(.a(new_n68_), .b(new_n55_), .c(new_n75_), .O(z05));
  nand2  g33(.a(new_n75_), .b(new_n70_), .O(z06));
  nand3  g34(.a(new_n52_), .b(new_n63_), .c(x11), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n75_), .O(z07));
  nand3  g38(.a(x19), .b(new_n64_), .c(x17), .O(new_n82_));
  inv1   g39(.a(new_n82_), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(x04), .b(new_n86_), .c(x02), .d(new_n46_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(new_n44_), .c(x01), .O(z08));
  nor2   g47(.a(x16), .b(x14), .O(new_n91_));
  nor2   g48(.a(x15), .b(new_n47_), .O(new_n92_));
  inv1   g49(.a(x11), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(x12), .c(new_n93_), .d(new_n74_), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n92_), .c(new_n91_), .d(x20), .O(new_n97_));
  nor2   g54(.a(x20), .b(new_n74_), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n65_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n46_), .O(new_n102_));
  aoi21  g59(.a(new_n99_), .b(new_n97_), .c(new_n102_), .O(z09));
  nor2   g60(.a(new_n74_), .b(x00), .O(new_n104_));
  nor2   g61(.a(new_n100_), .b(x20), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n104_), .c(new_n65_), .d(x22), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(z10));
  nand4  g64(.a(new_n105_), .b(new_n104_), .c(new_n65_), .d(new_n101_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(z11));
  nand2  g66(.a(x10), .b(x02), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(x01), .c(x00), .O(new_n111_));
  nor3   g68(.a(x19), .b(x17), .c(x02), .O(new_n112_));
  nor2   g69(.a(x19), .b(x02), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(x23), .c(new_n56_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  inv1   g72(.a(x09), .O(new_n116_));
  nor2   g73(.a(x24), .b(new_n116_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n75_), .O(z12));
  nor3   g76(.a(new_n57_), .b(x19), .c(new_n45_), .O(z13));
  nand2  g77(.a(new_n51_), .b(new_n116_), .O(new_n121_));
  nor2   g78(.a(new_n121_), .b(new_n59_), .O(z14));
  aoi21  g79(.a(new_n51_), .b(x01), .c(new_n47_), .O(new_n123_));
  aoi21  g80(.a(x19), .b(new_n47_), .c(x15), .O(new_n124_));
  oai22  g81(.a(new_n124_), .b(x01), .c(new_n123_), .d(new_n46_), .O(z15));
  oai21  g82(.a(new_n74_), .b(x00), .c(new_n75_), .O(z16));
  nand2  g83(.a(new_n92_), .b(new_n56_), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(z17));
endmodule


