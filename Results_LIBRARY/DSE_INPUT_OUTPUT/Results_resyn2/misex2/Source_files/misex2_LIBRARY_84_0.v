// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n129_, new_n130_,
    new_n131_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor2   g02(.a(x15), .b(x13), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(new_n47_));
  nor2   g04(.a(x19), .b(x18), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor3   g07(.a(x02), .b(x01), .c(x00), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor3   g09(.a(new_n52_), .b(new_n49_), .c(new_n44_), .O(z00));
  nand2  g10(.a(new_n51_), .b(x09), .O(new_n54_));
  nand3  g11(.a(new_n48_), .b(new_n45_), .c(x10), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n54_), .c(new_n47_), .O(z01));
  nand3  g13(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n47_), .O(z02));
  inv1   g15(.a(x19), .O(new_n59_));
  nand4  g16(.a(new_n51_), .b(new_n59_), .c(x18), .d(new_n45_), .O(new_n60_));
  inv1   g17(.a(x11), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n44_), .O(new_n62_));
  inv1   g19(.a(x02), .O(new_n63_));
  nor2   g20(.a(x09), .b(new_n63_), .O(new_n64_));
  nand2  g21(.a(x01), .b(x00), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n64_), .c(new_n62_), .d(x12), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n60_), .c(new_n47_), .O(z03));
  nor2   g25(.a(new_n65_), .b(new_n46_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(new_n61_), .c(x10), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n70_), .O(z04));
  nand3  g30(.a(x10), .b(x09), .c(x02), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n65_), .c(new_n46_), .O(z05));
  nand3  g32(.a(new_n69_), .b(new_n64_), .c(new_n62_), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z06));
  nand3  g34(.a(new_n62_), .b(new_n71_), .c(new_n50_), .O(new_n78_));
  nand3  g35(.a(new_n47_), .b(x02), .c(x00), .O(new_n79_));
  aoi21  g36(.a(new_n78_), .b(x01), .c(new_n79_), .O(z07));
  inv1   g37(.a(x00), .O(new_n81_));
  inv1   g38(.a(x01), .O(new_n82_));
  nand4  g39(.a(new_n47_), .b(x02), .c(new_n82_), .d(new_n81_), .O(new_n83_));
  inv1   g40(.a(x18), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n84_), .c(x17), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nor2   g43(.a(x06), .b(x05), .O(new_n87_));
  nor2   g44(.a(x08), .b(x07), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(x04), .d(new_n86_), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n85_), .c(new_n83_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n59_), .c(x18), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n93_), .c(x01), .d(new_n81_), .O(new_n95_));
  oai21  g52(.a(new_n95_), .b(new_n92_), .c(new_n47_), .O(z09));
  nand3  g53(.a(x21), .b(new_n91_), .c(new_n59_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nor2   g55(.a(new_n84_), .b(new_n82_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(new_n47_), .d(x22), .O(new_n100_));
  nand2  g57(.a(x02), .b(new_n82_), .O(new_n101_));
  inv1   g58(.a(x13), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(x12), .c(new_n61_), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  inv1   g61(.a(x15), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(x14), .O(new_n106_));
  nand4  g63(.a(new_n94_), .b(new_n93_), .c(x20), .d(x16), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n100_), .c(x00), .O(z10));
  nand3  g67(.a(new_n99_), .b(new_n98_), .c(new_n47_), .O(new_n111_));
  inv1   g68(.a(x16), .O(new_n112_));
  nand3  g69(.a(new_n93_), .b(x20), .c(new_n112_), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n106_), .c(new_n104_), .O(new_n115_));
  nand2  g72(.a(new_n94_), .b(new_n81_), .O(new_n116_));
  aoi21  g73(.a(new_n115_), .b(new_n111_), .c(new_n116_), .O(z11));
  aoi21  g74(.a(x10), .b(x02), .c(new_n65_), .O(new_n118_));
  nand2  g75(.a(new_n82_), .b(new_n81_), .O(new_n119_));
  oai21  g76(.a(x19), .b(x02), .c(x23), .O(new_n120_));
  nand3  g77(.a(new_n59_), .b(x17), .c(new_n63_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  nor2   g79(.a(x24), .b(new_n50_), .O(new_n123_));
  oai21  g80(.a(new_n122_), .b(new_n118_), .c(new_n123_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n47_), .O(z12));
  nor3   g82(.a(new_n46_), .b(x19), .c(new_n45_), .O(new_n126_));
  and2   g83(.a(new_n126_), .b(new_n51_), .O(z13));
  nor3   g84(.a(new_n52_), .b(new_n49_), .c(x10), .O(z14));
  oai21  g85(.a(x10), .b(new_n82_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n63_), .c(new_n82_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n130_), .c(new_n46_), .O(z15));
  oai21  g89(.a(new_n82_), .b(x00), .c(new_n47_), .O(z16));
  inv1   g90(.a(new_n83_), .O(z17));
endmodule


