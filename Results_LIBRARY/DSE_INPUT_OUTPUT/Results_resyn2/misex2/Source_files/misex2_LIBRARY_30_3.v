// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n127_, new_n129_, new_n130_, new_n131_;
  nand2  g00(.a(x16), .b(x14), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  nor2   g05(.a(x17), .b(x02), .O(new_n49_));
  inv1   g06(.a(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  inv1   g10(.a(x17), .O(new_n54_));
  nand4  g11(.a(new_n44_), .b(new_n46_), .c(new_n45_), .d(new_n54_), .O(new_n55_));
  inv1   g12(.a(x02), .O(new_n56_));
  nand4  g13(.a(new_n47_), .b(x10), .c(x09), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n55_), .O(z01));
  nand3  g15(.a(new_n49_), .b(new_n50_), .c(x09), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n48_), .c(new_n44_), .O(z02));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  nor3   g18(.a(new_n61_), .b(new_n50_), .c(x09), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x12), .c(x11), .O(new_n63_));
  nand4  g20(.a(new_n49_), .b(new_n47_), .c(new_n46_), .d(x18), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(new_n63_), .c(new_n44_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nand3  g24(.a(new_n62_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n44_), .O(z04));
  nand3  g26(.a(new_n44_), .b(x10), .c(x09), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n61_), .O(z05));
  nand2  g28(.a(new_n62_), .b(x11), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n44_), .O(z06));
  nand3  g30(.a(new_n51_), .b(new_n67_), .c(x11), .O(new_n74_));
  nand3  g31(.a(new_n44_), .b(x02), .c(x00), .O(new_n75_));
  aoi21  g32(.a(new_n74_), .b(x01), .c(new_n75_), .O(z07));
  inv1   g33(.a(x03), .O(new_n77_));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(new_n78_), .c(x04), .d(new_n77_), .O(new_n80_));
  inv1   g37(.a(x00), .O(new_n81_));
  inv1   g38(.a(x01), .O(new_n82_));
  nand3  g39(.a(x02), .b(new_n82_), .c(new_n81_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(x19), .c(new_n45_), .d(x17), .O(new_n85_));
  oai21  g42(.a(new_n85_), .b(new_n80_), .c(new_n44_), .O(z08));
  nand2  g43(.a(x02), .b(new_n82_), .O(new_n87_));
  inv1   g44(.a(x13), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(x12), .c(new_n66_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  nor2   g48(.a(x16), .b(x14), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x20), .c(new_n91_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  inv1   g51(.a(x20), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n46_), .c(x18), .d(x01), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  aoi21  g54(.a(new_n94_), .b(new_n90_), .c(new_n97_), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(new_n81_), .O(new_n101_));
  oai21  g58(.a(new_n101_), .b(new_n98_), .c(new_n44_), .O(z09));
  nand2  g59(.a(x22), .b(x21), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n96_), .O(new_n104_));
  nand2  g61(.a(new_n99_), .b(x20), .O(new_n105_));
  nand3  g62(.a(new_n100_), .b(x16), .c(x15), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n90_), .c(new_n104_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g66(.a(new_n100_), .b(new_n81_), .O(new_n110_));
  nand3  g67(.a(new_n97_), .b(new_n44_), .c(x21), .O(new_n111_));
  nand2  g68(.a(new_n92_), .b(x15), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n105_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n90_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n111_), .c(new_n110_), .O(z11));
  nand2  g72(.a(x01), .b(x00), .O(new_n116_));
  aoi21  g73(.a(x10), .b(x02), .c(new_n116_), .O(new_n117_));
  inv1   g74(.a(new_n47_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n46_), .b(x17), .c(new_n56_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  inv1   g78(.a(x09), .O(new_n122_));
  nor2   g79(.a(x24), .b(new_n122_), .O(new_n123_));
  oai21  g80(.a(new_n121_), .b(new_n117_), .c(new_n123_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n44_), .O(z12));
  oai21  g82(.a(new_n120_), .b(new_n118_), .c(new_n44_), .O(z13));
  nand4  g83(.a(new_n47_), .b(new_n50_), .c(new_n122_), .d(new_n56_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(new_n55_), .O(z14));
  oai21  g85(.a(x10), .b(new_n82_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n56_), .c(new_n82_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n130_), .c(new_n44_), .O(z15));
  oai21  g89(.a(new_n82_), .b(x00), .c(new_n44_), .O(z16));
  nand2  g90(.a(new_n83_), .b(new_n44_), .O(z17));
endmodule


