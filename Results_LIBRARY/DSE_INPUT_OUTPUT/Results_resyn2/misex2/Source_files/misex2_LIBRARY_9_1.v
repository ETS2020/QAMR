// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n130_, new_n131_,
    new_n134_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  nor2   g02(.a(x08), .b(x02), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n49_), .O(z00));
  nand2  g11(.a(new_n48_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  nor2   g16(.a(x19), .b(new_n51_), .O(new_n60_));
  nor3   g17(.a(x17), .b(x02), .c(x00), .O(new_n61_));
  aoi21  g18(.a(new_n61_), .b(new_n60_), .c(x08), .O(new_n62_));
  inv1   g19(.a(x00), .O(new_n63_));
  inv1   g20(.a(x01), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g22(.a(x11), .b(x10), .c(new_n44_), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n65_), .c(x12), .d(x02), .O(new_n68_));
  oai21  g25(.a(new_n62_), .b(x01), .c(new_n68_), .O(z03));
  nand2  g26(.a(new_n65_), .b(x02), .O(new_n70_));
  nand2  g27(.a(x08), .b(new_n64_), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n72_), .c(x10), .d(new_n44_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n70_), .c(new_n71_), .O(z04));
  nand2  g32(.a(x10), .b(x02), .O(new_n76_));
  nand2  g33(.a(new_n65_), .b(x09), .O(new_n77_));
  nor2   g34(.a(new_n77_), .b(new_n76_), .O(z05));
  nor2   g35(.a(new_n66_), .b(new_n70_), .O(z06));
  oai21  g36(.a(new_n66_), .b(x12), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n71_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nor2   g43(.a(x18), .b(new_n50_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n45_), .c(x19), .d(x02), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(new_n86_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n52_), .c(x18), .d(x01), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n72_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  nor3   g51(.a(x15), .b(x14), .c(x08), .O(new_n95_));
  nor2   g52(.a(new_n90_), .b(x16), .O(new_n96_));
  inv1   g53(.a(x02), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(x01), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n63_), .O(new_n102_));
  aoi21  g59(.a(new_n99_), .b(new_n91_), .c(new_n102_), .O(z09));
  nor2   g60(.a(new_n91_), .b(new_n100_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(x22), .O(new_n105_));
  inv1   g62(.a(x08), .O(new_n106_));
  inv1   g63(.a(x14), .O(new_n107_));
  nand4  g64(.a(x15), .b(new_n107_), .c(x02), .d(new_n64_), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n93_), .O(new_n109_));
  nand3  g66(.a(new_n101_), .b(new_n100_), .c(x20), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n109_), .c(x16), .d(new_n106_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n105_), .c(x00), .O(z10));
  nand2  g70(.a(new_n101_), .b(new_n63_), .O(new_n114_));
  inv1   g71(.a(x16), .O(new_n115_));
  nand3  g72(.a(new_n100_), .b(x20), .c(new_n115_), .O(new_n116_));
  inv1   g73(.a(new_n116_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n109_), .c(new_n104_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n114_), .c(new_n71_), .O(z11));
  nand2  g76(.a(new_n76_), .b(new_n65_), .O(new_n120_));
  nor3   g77(.a(x19), .b(x17), .c(x02), .O(new_n121_));
  nor2   g78(.a(x19), .b(x02), .O(new_n122_));
  oai21  g79(.a(new_n122_), .b(x23), .c(new_n45_), .O(new_n123_));
  oai21  g80(.a(new_n123_), .b(new_n121_), .c(new_n120_), .O(new_n124_));
  nor2   g81(.a(x24), .b(new_n44_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n71_), .O(z12));
  nor3   g84(.a(new_n47_), .b(x19), .c(new_n50_), .O(z13));
  nor2   g85(.a(new_n58_), .b(new_n49_), .O(z14));
  inv1   g86(.a(new_n46_), .O(new_n130_));
  aoi21  g87(.a(x19), .b(new_n64_), .c(x00), .O(new_n131_));
  oai21  g88(.a(new_n131_), .b(new_n130_), .c(new_n120_), .O(z15));
  nor2   g89(.a(new_n64_), .b(x00), .O(z16));
  nand3  g90(.a(new_n45_), .b(new_n106_), .c(x02), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(z17));
endmodule


