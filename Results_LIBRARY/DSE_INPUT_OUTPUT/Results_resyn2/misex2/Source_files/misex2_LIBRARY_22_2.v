// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n130_,
    new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor2   g01(.a(x19), .b(x18), .O(new_n45_));
  inv1   g02(.a(x14), .O(new_n46_));
  nor2   g03(.a(x16), .b(new_n46_), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  inv1   g07(.a(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x09), .O(new_n52_));
  nor2   g09(.a(x01), .b(x00), .O(new_n53_));
  nand3  g10(.a(new_n53_), .b(new_n52_), .c(new_n50_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n49_), .O(z00));
  nand2  g12(.a(new_n53_), .b(new_n45_), .O(new_n56_));
  nand4  g13(.a(new_n44_), .b(x10), .c(x09), .d(new_n50_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n56_), .c(new_n48_), .O(z01));
  nand4  g15(.a(new_n53_), .b(new_n51_), .c(x09), .d(new_n50_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n49_), .O(z02));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x02), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand2  g22(.a(new_n52_), .b(x11), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n65_), .c(x12), .O(new_n68_));
  inv1   g25(.a(x19), .O(new_n69_));
  nor2   g26(.a(x17), .b(x02), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n53_), .c(new_n69_), .d(x18), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(new_n68_), .c(new_n47_), .O(z03));
  nor2   g29(.a(x12), .b(x11), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(new_n52_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n64_), .c(new_n48_), .O(z04));
  nor2   g32(.a(new_n51_), .b(new_n50_), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(new_n63_), .c(new_n48_), .d(x09), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(z05));
  nand2  g35(.a(new_n63_), .b(new_n48_), .O(new_n79_));
  nor3   g36(.a(new_n79_), .b(new_n66_), .c(new_n50_), .O(z06));
  oai21  g37(.a(new_n66_), .b(x12), .c(x01), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n48_), .O(z07));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  inv1   g42(.a(x04), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(x03), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n85_), .c(new_n84_), .d(new_n48_), .O(new_n88_));
  nand3  g45(.a(x02), .b(new_n62_), .c(new_n61_), .O(new_n89_));
  inv1   g46(.a(x18), .O(new_n90_));
  nand3  g47(.a(x19), .b(new_n90_), .c(x17), .O(new_n91_));
  nor3   g48(.a(new_n91_), .b(new_n89_), .c(new_n88_), .O(z08));
  inv1   g49(.a(x20), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n69_), .c(x18), .d(x01), .O(new_n94_));
  inv1   g51(.a(x11), .O(new_n95_));
  nand4  g52(.a(x12), .b(new_n95_), .c(x02), .d(new_n62_), .O(new_n96_));
  inv1   g53(.a(x13), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nand4  g56(.a(x20), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  oai21  g57(.a(new_n100_), .b(new_n96_), .c(new_n94_), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n61_), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(x21), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n101_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n48_), .O(z09));
  inv1   g63(.a(x21), .O(new_n107_));
  nor2   g64(.a(new_n94_), .b(new_n107_), .O(new_n108_));
  nand4  g65(.a(new_n107_), .b(x20), .c(x15), .d(new_n97_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n102_), .b(x16), .c(new_n46_), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n96_), .O(new_n112_));
  aoi22  g69(.a(new_n112_), .b(new_n110_), .c(new_n108_), .d(x22), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(x00), .c(new_n48_), .O(z10));
  nor2   g71(.a(new_n96_), .b(x16), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n110_), .c(new_n108_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n103_), .c(new_n48_), .O(z11));
  oai21  g74(.a(new_n51_), .b(new_n50_), .c(new_n63_), .O(new_n118_));
  inv1   g75(.a(x23), .O(new_n119_));
  nor2   g76(.a(x19), .b(x02), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x17), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n119_), .c(new_n121_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n53_), .O(new_n123_));
  inv1   g80(.a(x09), .O(new_n124_));
  nor2   g81(.a(x24), .b(new_n124_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n48_), .O(new_n126_));
  aoi21  g83(.a(new_n123_), .b(new_n118_), .c(new_n126_), .O(z12));
  nand3  g84(.a(new_n120_), .b(new_n53_), .c(x17), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n48_), .O(z13));
  nand3  g86(.a(new_n70_), .b(new_n51_), .c(new_n124_), .O(new_n130_));
  oai21  g87(.a(new_n130_), .b(new_n56_), .c(new_n48_), .O(z14));
  oai21  g88(.a(x10), .b(new_n62_), .c(x02), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(x00), .O(new_n133_));
  nand3  g90(.a(x19), .b(new_n50_), .c(new_n62_), .O(new_n134_));
  aoi21  g91(.a(new_n134_), .b(new_n133_), .c(new_n47_), .O(z15));
  oai21  g92(.a(new_n62_), .b(x00), .c(new_n48_), .O(z16));
  nand2  g93(.a(new_n89_), .b(new_n48_), .O(z17));
endmodule


