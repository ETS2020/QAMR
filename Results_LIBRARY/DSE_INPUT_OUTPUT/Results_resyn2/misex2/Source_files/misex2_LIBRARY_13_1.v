// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n122_, new_n124_,
    new_n125_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x18), .b(x17), .O(new_n46_));
  nor2   g03(.a(x19), .b(x02), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(x10), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x00), .O(z00));
  nand2  g06(.a(new_n47_), .b(new_n46_), .O(new_n50_));
  nand2  g07(.a(x10), .b(x09), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  nand2  g09(.a(new_n44_), .b(new_n52_), .O(new_n53_));
  nor3   g10(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand4  g12(.a(new_n47_), .b(new_n46_), .c(new_n55_), .d(x09), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n44_), .c(x00), .O(z02));
  inv1   g14(.a(x17), .O(new_n58_));
  nand3  g15(.a(new_n47_), .b(x18), .c(new_n58_), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n44_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  inv1   g18(.a(x02), .O(new_n62_));
  nor2   g19(.a(x09), .b(new_n62_), .O(new_n63_));
  inv1   g20(.a(x11), .O(new_n64_));
  inv1   g21(.a(x12), .O(new_n65_));
  nor2   g22(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n63_), .c(x10), .d(x01), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n61_), .O(z03));
  nand4  g25(.a(new_n63_), .b(new_n65_), .c(new_n64_), .d(x10), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x00), .c(new_n44_), .O(z04));
  nor2   g27(.a(new_n44_), .b(new_n52_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(x02), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n51_), .O(z05));
  nand3  g30(.a(x11), .b(x10), .c(new_n45_), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n72_), .O(z06));
  inv1   g32(.a(new_n74_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n65_), .O(new_n77_));
  nand2  g34(.a(x02), .b(x00), .O(new_n78_));
  aoi21  g35(.a(new_n77_), .b(x01), .c(new_n78_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand3  g40(.a(x02), .b(new_n44_), .c(new_n52_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nor2   g42(.a(x18), .b(new_n58_), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(new_n85_), .c(x19), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n83_), .O(z08));
  nor2   g45(.a(new_n65_), .b(x11), .O(new_n89_));
  nor2   g46(.a(x14), .b(x13), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n89_), .c(new_n85_), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  inv1   g51(.a(x20), .O(new_n95_));
  nor2   g52(.a(x21), .b(new_n95_), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n94_), .c(new_n93_), .d(new_n92_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n91_), .O(z09));
  inv1   g55(.a(x13), .O(new_n99_));
  inv1   g56(.a(x14), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n99_), .c(x12), .d(new_n64_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  inv1   g59(.a(x21), .O(new_n103_));
  nand4  g60(.a(new_n94_), .b(new_n103_), .c(x20), .d(x16), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n102_), .c(new_n85_), .d(x15), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(z10));
  nand4  g64(.a(new_n94_), .b(new_n103_), .c(x20), .d(new_n93_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n102_), .c(new_n85_), .d(x15), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(z11));
  oai21  g68(.a(new_n55_), .b(new_n62_), .c(new_n71_), .O(new_n112_));
  nand2  g69(.a(new_n47_), .b(new_n58_), .O(new_n113_));
  inv1   g70(.a(x23), .O(new_n114_));
  oai21  g71(.a(x19), .b(x02), .c(new_n114_), .O(new_n115_));
  nand4  g72(.a(new_n115_), .b(new_n113_), .c(new_n44_), .d(new_n52_), .O(new_n116_));
  inv1   g73(.a(x24), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x09), .O(new_n118_));
  aoi21  g75(.a(new_n116_), .b(new_n112_), .c(new_n118_), .O(z12));
  nand2  g76(.a(new_n47_), .b(x17), .O(new_n120_));
  nor2   g77(.a(new_n120_), .b(new_n53_), .O(z13));
  nand4  g78(.a(new_n47_), .b(new_n46_), .c(new_n55_), .d(new_n45_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n44_), .c(x00), .O(z14));
  oai21  g80(.a(new_n55_), .b(new_n52_), .c(x01), .O(new_n124_));
  oai21  g81(.a(x19), .b(x00), .c(new_n62_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(z15));
  nor2   g83(.a(new_n44_), .b(x00), .O(z16));
  aoi21  g84(.a(new_n62_), .b(new_n44_), .c(x00), .O(z17));
endmodule


