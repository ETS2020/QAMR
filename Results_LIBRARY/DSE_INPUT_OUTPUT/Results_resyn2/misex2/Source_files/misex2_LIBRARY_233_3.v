// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n115_;
  nand2  g00(.a(x01), .b(x00), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  inv1   g06(.a(x00), .O(new_n50_));
  inv1   g07(.a(x01), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x02), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand2  g12(.a(new_n53_), .b(x09), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n56_), .c(new_n44_), .O(z02));
  inv1   g17(.a(new_n53_), .O(new_n61_));
  nand3  g18(.a(new_n47_), .b(x18), .c(new_n45_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(new_n44_), .O(z03));
  inv1   g20(.a(new_n44_), .O(z04));
  nand3  g21(.a(x02), .b(new_n51_), .c(x00), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z07));
  inv1   g23(.a(x03), .O(new_n69_));
  nor2   g24(.a(x06), .b(x05), .O(new_n70_));
  nor2   g25(.a(x08), .b(x07), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(x04), .d(new_n69_), .O(new_n72_));
  nor2   g27(.a(x01), .b(x00), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x02), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z17));
  nor2   g30(.a(x18), .b(new_n45_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(z17), .c(x19), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(new_n72_), .c(new_n44_), .O(z08));
  inv1   g33(.a(x20), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n47_), .c(x18), .d(x01), .O(new_n80_));
  nand2  g35(.a(x02), .b(new_n51_), .O(new_n81_));
  inv1   g36(.a(x11), .O(new_n82_));
  inv1   g37(.a(x13), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(x12), .c(new_n82_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  nor2   g40(.a(x15), .b(x14), .O(new_n86_));
  nor2   g41(.a(new_n79_), .b(x16), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  inv1   g43(.a(x21), .O(new_n89_));
  inv1   g44(.a(x22), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n89_), .c(new_n50_), .O(new_n91_));
  aoi21  g46(.a(new_n88_), .b(new_n80_), .c(new_n91_), .O(z09));
  inv1   g47(.a(new_n80_), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(x22), .c(x21), .O(new_n94_));
  inv1   g49(.a(x14), .O(new_n95_));
  nand4  g50(.a(new_n89_), .b(x20), .c(x15), .d(new_n95_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  inv1   g52(.a(x16), .O(new_n98_));
  nor2   g53(.a(x22), .b(new_n98_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n97_), .c(new_n85_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n94_), .c(x00), .O(z10));
  nand2  g56(.a(new_n90_), .b(new_n50_), .O(new_n102_));
  nand2  g57(.a(new_n93_), .b(x21), .O(new_n103_));
  nand3  g58(.a(new_n97_), .b(new_n85_), .c(new_n98_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(z11));
  oai21  g60(.a(x19), .b(x02), .c(x23), .O(new_n106_));
  inv1   g61(.a(x02), .O(new_n107_));
  nand3  g62(.a(new_n47_), .b(x17), .c(new_n107_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nor2   g64(.a(x24), .b(new_n49_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n73_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n44_), .O(z12));
  nor2   g67(.a(new_n108_), .b(new_n52_), .O(z13));
  nor2   g68(.a(new_n59_), .b(new_n54_), .O(z14));
  nand2  g69(.a(new_n107_), .b(new_n51_), .O(new_n115_));
  aoi21  g70(.a(new_n47_), .b(new_n50_), .c(new_n115_), .O(z15));
  nor2   g71(.a(new_n51_), .b(x00), .O(z16));
  zero   g72(.O(z05));
  zero   g73(.O(z06));
endmodule


