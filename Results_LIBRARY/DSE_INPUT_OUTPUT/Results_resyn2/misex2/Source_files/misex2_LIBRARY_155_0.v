// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n133_, new_n134_, new_n135_;
  nor2   g00(.a(x24), .b(x21), .O(new_n44_));
  inv1   g01(.a(new_n44_), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nor2   g03(.a(x09), .b(x02), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n45_), .O(z00));
  inv1   g10(.a(x02), .O(new_n54_));
  nand3  g11(.a(new_n46_), .b(x09), .c(new_n54_), .O(new_n55_));
  nor3   g12(.a(new_n55_), .b(new_n52_), .c(new_n44_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  nor2   g16(.a(x17), .b(x02), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n46_), .c(new_n51_), .d(x18), .O(new_n61_));
  inv1   g18(.a(x11), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  nor2   g20(.a(x09), .b(new_n54_), .O(new_n64_));
  inv1   g21(.a(x00), .O(new_n65_));
  inv1   g22(.a(x01), .O(new_n66_));
  nor2   g23(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n64_), .c(new_n63_), .d(x12), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(new_n61_), .c(new_n44_), .O(z03));
  nand3  g26(.a(new_n67_), .b(new_n64_), .c(new_n45_), .O(new_n70_));
  nor4   g27(.a(new_n70_), .b(x12), .c(x11), .d(new_n57_), .O(z04));
  nand2  g28(.a(x10), .b(x02), .O(new_n72_));
  nand2  g29(.a(new_n67_), .b(x09), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n72_), .c(new_n45_), .O(z05));
  inv1   g31(.a(new_n70_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(new_n63_), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z06));
  nor2   g34(.a(x12), .b(x09), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(new_n63_), .c(new_n66_), .O(new_n79_));
  nand2  g36(.a(x02), .b(x00), .O(new_n80_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n45_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nor2   g42(.a(new_n54_), .b(x01), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(new_n65_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x19), .c(new_n50_), .d(x17), .O(new_n89_));
  oai21  g46(.a(new_n89_), .b(new_n85_), .c(new_n45_), .O(z08));
  inv1   g47(.a(x13), .O(new_n91_));
  inv1   g48(.a(x12), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(x11), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n86_), .c(new_n91_), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nand4  g54(.a(x20), .b(new_n97_), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  inv1   g55(.a(x20), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n51_), .c(x18), .d(x01), .O(new_n100_));
  oai21  g57(.a(new_n98_), .b(new_n94_), .c(new_n100_), .O(new_n101_));
  nor2   g58(.a(x22), .b(x00), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(x24), .c(x21), .O(z09));
  nand2  g61(.a(x02), .b(new_n66_), .O(new_n105_));
  nand3  g62(.a(new_n91_), .b(x12), .c(new_n62_), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g64(.a(x21), .O(new_n108_));
  nor2   g65(.a(new_n100_), .b(new_n108_), .O(new_n109_));
  nand4  g66(.a(new_n108_), .b(x20), .c(x15), .d(new_n95_), .O(new_n110_));
  inv1   g67(.a(x22), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(x16), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  aoi22  g70(.a(new_n113_), .b(new_n107_), .c(new_n109_), .d(x22), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(x00), .c(new_n45_), .O(z10));
  inv1   g72(.a(new_n102_), .O(new_n116_));
  inv1   g73(.a(new_n109_), .O(new_n117_));
  inv1   g74(.a(new_n110_), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n107_), .c(x24), .d(new_n97_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n117_), .c(new_n116_), .O(z11));
  nand2  g77(.a(new_n72_), .b(new_n67_), .O(new_n121_));
  inv1   g78(.a(x23), .O(new_n122_));
  nor2   g79(.a(x19), .b(x02), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x17), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n122_), .c(new_n124_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n46_), .O(new_n126_));
  inv1   g83(.a(x24), .O(new_n127_));
  nand3  g84(.a(new_n127_), .b(x21), .c(x09), .O(new_n128_));
  aoi21  g85(.a(new_n126_), .b(new_n121_), .c(new_n128_), .O(z12));
  inv1   g86(.a(new_n46_), .O(new_n130_));
  oai21  g87(.a(new_n124_), .b(new_n130_), .c(new_n45_), .O(z13));
  oai21  g88(.a(new_n58_), .b(new_n48_), .c(new_n45_), .O(z14));
  aoi21  g89(.a(new_n57_), .b(x01), .c(new_n54_), .O(new_n133_));
  nor2   g90(.a(x02), .b(x01), .O(new_n134_));
  aoi21  g91(.a(new_n134_), .b(x19), .c(new_n44_), .O(new_n135_));
  oai21  g92(.a(new_n133_), .b(new_n65_), .c(new_n135_), .O(z15));
  oai21  g93(.a(new_n66_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g94(.a(new_n87_), .b(new_n45_), .O(z17));
endmodule


