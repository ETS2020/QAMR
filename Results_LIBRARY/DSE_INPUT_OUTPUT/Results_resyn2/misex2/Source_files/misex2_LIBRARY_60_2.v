// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n129_, new_n130_, new_n131_,
    new_n133_;
  inv1   g00(.a(x01), .O(new_n44_));
  nor2   g01(.a(x07), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  nand2  g09(.a(new_n44_), .b(new_n52_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n50_), .c(new_n46_), .O(z00));
  nand2  g13(.a(new_n54_), .b(x09), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n50_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  and2   g18(.a(x02), .b(x00), .O(new_n62_));
  and2   g19(.a(x11), .b(x10), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n62_), .c(x12), .d(new_n51_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x07), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(x01), .O(new_n66_));
  nand4  g23(.a(new_n54_), .b(new_n49_), .c(x18), .d(new_n47_), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n66_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand2  g27(.a(x02), .b(x00), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(x09), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x07), .c(new_n44_), .O(z04));
  nand3  g31(.a(new_n62_), .b(x10), .c(x09), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x07), .c(new_n44_), .O(z05));
  nand2  g33(.a(new_n63_), .b(new_n72_), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x07), .c(new_n44_), .O(z06));
  nand4  g35(.a(new_n63_), .b(new_n70_), .c(new_n51_), .d(x07), .O(new_n79_));
  aoi21  g36(.a(new_n79_), .b(x01), .c(new_n71_), .O(z07));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nand3  g41(.a(x02), .b(new_n44_), .c(new_n52_), .O(new_n85_));
  nand3  g42(.a(x19), .b(new_n48_), .c(x17), .O(new_n86_));
  nor3   g43(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(z08));
  inv1   g44(.a(x20), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n49_), .c(x18), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(x07), .c(x01), .O(new_n91_));
  nand2  g48(.a(x02), .b(new_n44_), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(x12), .c(new_n69_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nor2   g52(.a(x15), .b(x14), .O(new_n96_));
  nor2   g53(.a(new_n88_), .b(x16), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(new_n52_), .O(new_n101_));
  aoi21  g58(.a(new_n98_), .b(new_n91_), .c(new_n101_), .O(z09));
  nor2   g59(.a(new_n45_), .b(new_n52_), .O(new_n103_));
  inv1   g60(.a(x14), .O(new_n104_));
  nand4  g61(.a(new_n99_), .b(x20), .c(x15), .d(new_n104_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  inv1   g63(.a(x16), .O(new_n107_));
  nor2   g64(.a(x22), .b(new_n107_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n106_), .c(new_n95_), .O(new_n109_));
  nand3  g66(.a(x22), .b(x21), .c(x01), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n90_), .c(new_n45_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n109_), .c(new_n103_), .O(z10));
  nand2  g70(.a(new_n100_), .b(new_n52_), .O(new_n114_));
  nand4  g71(.a(new_n90_), .b(x21), .c(x07), .d(x01), .O(new_n115_));
  nand3  g72(.a(new_n106_), .b(new_n95_), .c(new_n107_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(z11));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  inv1   g75(.a(x02), .O(new_n119_));
  nand3  g76(.a(new_n49_), .b(x17), .c(new_n119_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n118_), .c(new_n53_), .O(new_n121_));
  nand2  g78(.a(x01), .b(x00), .O(new_n122_));
  aoi21  g79(.a(x10), .b(x02), .c(new_n122_), .O(new_n123_));
  nor2   g80(.a(x24), .b(new_n51_), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n121_), .c(new_n124_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n46_), .O(z12));
  oai21  g83(.a(new_n120_), .b(new_n53_), .c(new_n46_), .O(z13));
  nor2   g84(.a(new_n60_), .b(new_n55_), .O(z14));
  nor2   g85(.a(new_n49_), .b(x01), .O(new_n129_));
  oai21  g86(.a(new_n129_), .b(new_n103_), .c(new_n119_), .O(new_n130_));
  nand4  g87(.a(new_n59_), .b(x07), .c(x01), .d(x00), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(new_n130_), .O(z15));
  nand3  g89(.a(x07), .b(x01), .c(new_n52_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z16));
  nand2  g91(.a(new_n85_), .b(new_n46_), .O(z17));
endmodule


