// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n124_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor2   g01(.a(x19), .b(x18), .O(new_n45_));
  nor2   g02(.a(x16), .b(x14), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nor2   g08(.a(x01), .b(x00), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n48_), .O(z00));
  nand2  g11(.a(new_n52_), .b(new_n45_), .O(new_n55_));
  nand4  g12(.a(new_n44_), .b(x10), .c(x09), .d(new_n49_), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n55_), .c(new_n47_), .O(z01));
  nand4  g14(.a(new_n52_), .b(new_n50_), .c(x09), .d(new_n49_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n48_), .O(z02));
  inv1   g16(.a(x00), .O(new_n60_));
  inv1   g17(.a(x01), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x02), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand2  g21(.a(new_n51_), .b(x11), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n64_), .c(x12), .O(new_n67_));
  inv1   g24(.a(x19), .O(new_n68_));
  nor2   g25(.a(x17), .b(x02), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n52_), .c(new_n68_), .d(x18), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(new_n67_), .c(new_n46_), .O(z03));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(new_n51_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n63_), .c(new_n47_), .O(z04));
  nand2  g32(.a(new_n62_), .b(new_n47_), .O(new_n76_));
  nand3  g33(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  nor2   g34(.a(new_n77_), .b(new_n76_), .O(z05));
  nor3   g35(.a(new_n76_), .b(new_n65_), .c(new_n49_), .O(z06));
  nand3  g36(.a(new_n51_), .b(new_n73_), .c(x11), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(x01), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n47_), .O(z07));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  inv1   g42(.a(x04), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(x03), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n85_), .c(new_n84_), .d(new_n47_), .O(new_n88_));
  nand3  g45(.a(x02), .b(new_n61_), .c(new_n60_), .O(new_n89_));
  inv1   g46(.a(x18), .O(new_n90_));
  nand3  g47(.a(x19), .b(new_n90_), .c(x17), .O(new_n91_));
  nor3   g48(.a(new_n91_), .b(new_n89_), .c(new_n88_), .O(z08));
  nand3  g49(.a(x18), .b(x01), .c(new_n60_), .O(new_n93_));
  inv1   g50(.a(x20), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(new_n68_), .O(new_n97_));
  oai21  g54(.a(new_n97_), .b(new_n93_), .c(new_n47_), .O(z09));
  nor2   g55(.a(x14), .b(x13), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(x20), .c(x15), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  nand2  g58(.a(new_n96_), .b(new_n95_), .O(new_n102_));
  nand4  g59(.a(x12), .b(new_n72_), .c(x02), .d(new_n61_), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g61(.a(new_n68_), .b(x18), .O(new_n105_));
  nand4  g62(.a(x22), .b(x21), .c(new_n94_), .d(x01), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g64(.a(new_n104_), .b(new_n101_), .c(new_n107_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(x00), .c(new_n47_), .O(z10));
  nand4  g66(.a(new_n96_), .b(x21), .c(new_n94_), .d(new_n68_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(new_n93_), .c(new_n47_), .O(z11));
  oai21  g68(.a(new_n50_), .b(new_n49_), .c(new_n62_), .O(new_n112_));
  inv1   g69(.a(x23), .O(new_n113_));
  nor2   g70(.a(x19), .b(x02), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(x17), .O(new_n115_));
  oai21  g72(.a(new_n114_), .b(new_n113_), .c(new_n115_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n52_), .O(new_n117_));
  inv1   g74(.a(x09), .O(new_n118_));
  nor2   g75(.a(x24), .b(new_n118_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n47_), .O(new_n120_));
  aoi21  g77(.a(new_n117_), .b(new_n112_), .c(new_n120_), .O(z12));
  nand3  g78(.a(new_n114_), .b(new_n52_), .c(x17), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n47_), .O(z13));
  nand3  g80(.a(new_n69_), .b(new_n50_), .c(new_n118_), .O(new_n124_));
  oai21  g81(.a(new_n124_), .b(new_n55_), .c(new_n47_), .O(z14));
  oai21  g82(.a(x10), .b(new_n61_), .c(x02), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x00), .O(new_n127_));
  nand3  g84(.a(x19), .b(new_n49_), .c(new_n61_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n127_), .c(new_n46_), .O(z15));
  oai21  g86(.a(new_n61_), .b(x00), .c(new_n47_), .O(z16));
  nand2  g87(.a(new_n89_), .b(new_n47_), .O(z17));
endmodule


