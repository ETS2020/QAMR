// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n119_, new_n120_;
  inv1   g00(.a(x02), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n47_), .O(z00));
  nand3  g09(.a(new_n46_), .b(x09), .c(new_n44_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n51_), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand4  g12(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n55_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n53_), .O(z02));
  inv1   g14(.a(x00), .O(new_n58_));
  inv1   g15(.a(x01), .O(new_n59_));
  nor2   g16(.a(x19), .b(x02), .O(new_n60_));
  nand3  g17(.a(new_n60_), .b(x18), .c(new_n48_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nor2   g20(.a(new_n55_), .b(new_n44_), .O(new_n64_));
  inv1   g21(.a(x11), .O(new_n65_));
  nor2   g22(.a(new_n65_), .b(x09), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand2  g24(.a(x12), .b(x01), .O(new_n68_));
  oai21  g25(.a(new_n68_), .b(new_n67_), .c(new_n63_), .O(z03));
  inv1   g26(.a(x12), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(new_n65_), .c(x10), .O(new_n71_));
  nand2  g28(.a(x02), .b(x00), .O(new_n72_));
  nor4   g29(.a(new_n72_), .b(new_n71_), .c(x09), .d(new_n59_), .O(z04));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x00), .c(new_n59_), .O(z05));
  aoi21  g32(.a(new_n67_), .b(x00), .c(new_n59_), .O(z06));
  nand3  g33(.a(new_n66_), .b(new_n70_), .c(x10), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x01), .c(new_n72_), .O(z07));
  inv1   g35(.a(x08), .O(new_n79_));
  nand4  g36(.a(x19), .b(new_n49_), .c(x17), .d(new_n79_), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nor2   g38(.a(x03), .b(new_n44_), .O(new_n82_));
  inv1   g39(.a(x04), .O(new_n83_));
  nor2   g40(.a(x05), .b(new_n83_), .O(new_n84_));
  nor2   g41(.a(x07), .b(x06), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(new_n82_), .d(new_n81_), .O(new_n86_));
  aoi21  g43(.a(new_n86_), .b(new_n59_), .c(x00), .O(z08));
  inv1   g44(.a(x16), .O(new_n88_));
  inv1   g45(.a(x21), .O(new_n89_));
  inv1   g46(.a(x22), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n89_), .c(x20), .d(new_n88_), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  nand2  g49(.a(new_n65_), .b(x02), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(x12), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n46_), .c(new_n92_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n91_), .O(z09));
  nand4  g56(.a(new_n90_), .b(new_n89_), .c(x20), .d(x16), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n97_), .c(x15), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n59_), .c(x00), .O(z10));
  inv1   g60(.a(new_n91_), .O(new_n104_));
  nand3  g61(.a(new_n97_), .b(new_n104_), .c(x15), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n59_), .c(x00), .O(z11));
  aoi21  g63(.a(x10), .b(x02), .c(new_n59_), .O(new_n107_));
  inv1   g64(.a(x23), .O(new_n108_));
  oai21  g65(.a(x19), .b(x02), .c(new_n108_), .O(new_n109_));
  aoi21  g66(.a(new_n60_), .b(new_n48_), .c(x00), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n109_), .c(new_n107_), .O(new_n111_));
  inv1   g68(.a(x24), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(x09), .O(new_n113_));
  nand2  g70(.a(x01), .b(new_n58_), .O(new_n114_));
  oai21  g71(.a(new_n113_), .b(new_n111_), .c(new_n114_), .O(z12));
  nand2  g72(.a(new_n60_), .b(x17), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n59_), .c(x00), .O(z13));
  nor2   g74(.a(new_n56_), .b(new_n47_), .O(z14));
  aoi21  g75(.a(new_n55_), .b(x01), .c(new_n44_), .O(new_n119_));
  nand3  g76(.a(x19), .b(new_n44_), .c(new_n59_), .O(new_n120_));
  oai21  g77(.a(new_n119_), .b(new_n58_), .c(new_n120_), .O(z15));
  inv1   g78(.a(new_n114_), .O(z16));
  aoi21  g79(.a(new_n44_), .b(new_n59_), .c(x00), .O(z17));
endmodule


