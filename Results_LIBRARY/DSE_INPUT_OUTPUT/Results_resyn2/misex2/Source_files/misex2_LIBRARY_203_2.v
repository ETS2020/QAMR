// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n131_,
    new_n132_, new_n133_;
  inv1   g00(.a(x23), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x22), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g11(.a(new_n45_), .O(new_n55_));
  nand3  g12(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n56_));
  nor3   g13(.a(new_n56_), .b(new_n53_), .c(new_n55_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n56_), .c(new_n45_), .O(z02));
  nand3  g17(.a(x11), .b(x10), .c(new_n47_), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  inv1   g19(.a(x00), .O(new_n63_));
  nor2   g20(.a(new_n46_), .b(new_n63_), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n62_), .c(x12), .d(x01), .O(new_n65_));
  nor2   g22(.a(x19), .b(new_n51_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n48_), .c(new_n50_), .d(new_n46_), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(new_n65_), .c(new_n55_), .O(z03));
  nand2  g25(.a(new_n45_), .b(x02), .O(new_n69_));
  inv1   g26(.a(x01), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n63_), .O(new_n71_));
  nor2   g28(.a(x12), .b(x11), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(x10), .d(new_n47_), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n69_), .O(z04));
  nand2  g31(.a(x10), .b(x02), .O(new_n75_));
  nand2  g32(.a(new_n71_), .b(x09), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n45_), .O(z05));
  nand2  g34(.a(new_n64_), .b(x01), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(new_n61_), .c(new_n45_), .O(z06));
  oai21  g36(.a(new_n61_), .b(x12), .c(x01), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n64_), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n45_), .O(z07));
  nand3  g39(.a(new_n48_), .b(new_n45_), .c(x02), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n51_), .c(x17), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nor2   g42(.a(x06), .b(x05), .O(new_n86_));
  nor2   g43(.a(x08), .b(x07), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(x04), .d(new_n85_), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n84_), .c(new_n83_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nor3   g47(.a(x19), .b(new_n51_), .c(new_n70_), .O(new_n91_));
  nand2  g48(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n70_), .O(new_n93_));
  inv1   g50(.a(x11), .O(new_n94_));
  inv1   g51(.a(x13), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x12), .c(new_n94_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nor2   g54(.a(x15), .b(x14), .O(new_n98_));
  nor2   g55(.a(new_n90_), .b(x16), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n63_), .O(new_n103_));
  aoi21  g60(.a(new_n100_), .b(new_n92_), .c(new_n103_), .O(z09));
  inv1   g61(.a(x14), .O(new_n105_));
  nand4  g62(.a(new_n101_), .b(x20), .c(x15), .d(new_n105_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  inv1   g64(.a(x16), .O(new_n108_));
  nor2   g65(.a(x22), .b(new_n108_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n107_), .c(new_n97_), .O(new_n110_));
  nand3  g67(.a(x23), .b(x22), .c(x01), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n66_), .c(x21), .d(new_n90_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n110_), .c(x00), .O(z10));
  nand2  g71(.a(new_n102_), .b(new_n63_), .O(new_n115_));
  nand3  g72(.a(new_n107_), .b(new_n97_), .c(new_n108_), .O(new_n116_));
  nand3  g73(.a(new_n91_), .b(x21), .c(new_n90_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(z11));
  nor2   g75(.a(x19), .b(x02), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n102_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n44_), .O(new_n121_));
  nand2  g78(.a(new_n119_), .b(new_n50_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n121_), .c(new_n48_), .O(new_n123_));
  nand3  g80(.a(new_n75_), .b(new_n71_), .c(new_n45_), .O(new_n124_));
  inv1   g81(.a(x24), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x09), .O(new_n126_));
  aoi21  g83(.a(new_n124_), .b(new_n123_), .c(new_n126_), .O(z12));
  nand3  g84(.a(new_n119_), .b(new_n48_), .c(x17), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n45_), .O(z13));
  oai21  g86(.a(new_n59_), .b(new_n49_), .c(new_n45_), .O(z14));
  oai21  g87(.a(x10), .b(new_n70_), .c(x02), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x00), .O(new_n132_));
  nand3  g89(.a(x19), .b(new_n46_), .c(new_n70_), .O(new_n133_));
  nand3  g90(.a(new_n133_), .b(new_n132_), .c(new_n45_), .O(z15));
  nor3   g91(.a(new_n55_), .b(new_n70_), .c(x00), .O(z16));
  inv1   g92(.a(new_n83_), .O(z17));
endmodule


