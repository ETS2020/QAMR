// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n135_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x23), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n44_), .d(x10), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nor3   g08(.a(x02), .b(x01), .c(x00), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n50_), .c(new_n47_), .O(z00));
  nand2  g11(.a(new_n52_), .b(x09), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n50_), .c(new_n47_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n49_), .b(new_n48_), .c(new_n44_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  nand4  g16(.a(x10), .b(new_n51_), .c(x01), .d(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  inv1   g18(.a(x02), .O(new_n62_));
  nor2   g19(.a(new_n46_), .b(new_n62_), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n61_), .c(x12), .d(x11), .O(new_n64_));
  nand4  g21(.a(new_n52_), .b(new_n49_), .c(x18), .d(new_n44_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n64_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  inv1   g24(.a(x12), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n67_), .c(x02), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n60_), .c(new_n47_), .O(z04));
  nand2  g27(.a(x01), .b(x00), .O(new_n71_));
  nand2  g28(.a(x10), .b(x02), .O(new_n72_));
  nor4   g29(.a(new_n72_), .b(new_n71_), .c(new_n46_), .d(new_n51_), .O(z05));
  nand2  g30(.a(new_n63_), .b(x11), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n60_), .O(z06));
  nand4  g32(.a(new_n68_), .b(x11), .c(x10), .d(new_n51_), .O(new_n76_));
  nand2  g33(.a(new_n63_), .b(x00), .O(new_n77_));
  aoi21  g34(.a(new_n76_), .b(x01), .c(new_n77_), .O(z07));
  nor2   g35(.a(x18), .b(x08), .O(new_n79_));
  inv1   g36(.a(x05), .O(new_n80_));
  nor2   g37(.a(x07), .b(x06), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(new_n80_), .c(x04), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x01), .b(x00), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(new_n83_), .c(x02), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(new_n79_), .c(x19), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(new_n45_), .c(new_n44_), .O(z08));
  inv1   g45(.a(x20), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n49_), .c(x18), .d(x01), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand2  g48(.a(new_n67_), .b(x02), .O(new_n92_));
  inv1   g49(.a(x01), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n93_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  inv1   g53(.a(x14), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nand4  g56(.a(x20), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n96_), .c(new_n91_), .O(new_n102_));
  inv1   g59(.a(x00), .O(new_n103_));
  inv1   g60(.a(x21), .O(new_n104_));
  inv1   g61(.a(x22), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(new_n102_), .c(new_n47_), .O(z09));
  nor2   g64(.a(new_n90_), .b(new_n104_), .O(new_n108_));
  nand4  g65(.a(new_n104_), .b(x20), .c(x15), .d(new_n97_), .O(new_n109_));
  nand2  g66(.a(new_n105_), .b(x16), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  aoi22  g68(.a(new_n111_), .b(new_n96_), .c(new_n108_), .d(x22), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(x00), .c(new_n47_), .O(z10));
  nand2  g70(.a(new_n105_), .b(new_n103_), .O(new_n114_));
  nor2   g71(.a(new_n109_), .b(x16), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n96_), .c(new_n108_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n114_), .c(new_n47_), .O(z11));
  nand4  g74(.a(new_n72_), .b(new_n47_), .c(x01), .d(x00), .O(new_n118_));
  nor2   g75(.a(x19), .b(x02), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n44_), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(new_n45_), .O(new_n121_));
  nand4  g78(.a(new_n121_), .b(new_n120_), .c(new_n84_), .d(new_n47_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n118_), .c(new_n124_), .O(z12));
  nand2  g82(.a(new_n119_), .b(new_n84_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n45_), .c(new_n44_), .O(z13));
  nor2   g84(.a(new_n58_), .b(new_n53_), .O(z14));
  oai21  g85(.a(x10), .b(new_n93_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n62_), .c(new_n93_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n130_), .c(new_n46_), .O(z15));
  nand3  g89(.a(new_n47_), .b(x01), .c(new_n103_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z16));
  nand3  g91(.a(new_n84_), .b(new_n47_), .c(x02), .O(new_n135_));
  inv1   g92(.a(new_n135_), .O(z17));
endmodule


