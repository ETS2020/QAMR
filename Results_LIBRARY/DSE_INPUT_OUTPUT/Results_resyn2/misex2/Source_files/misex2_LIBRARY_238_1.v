// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n133_, new_n134_, new_n137_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  inv1   g05(.a(x16), .O(new_n49_));
  nor3   g06(.a(x19), .b(x18), .c(x17), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n48_), .O(z00));
  nand2  g09(.a(new_n46_), .b(new_n44_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n50_), .c(x10), .d(x09), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n49_), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand3  g14(.a(new_n50_), .b(new_n49_), .c(new_n57_), .O(new_n58_));
  nand4  g15(.a(x09), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n58_), .O(z02));
  inv1   g17(.a(x17), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x18), .c(new_n61_), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n53_), .c(new_n49_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n45_), .O(new_n65_));
  nor2   g22(.a(new_n57_), .b(x09), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n66_), .c(x12), .d(x11), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n65_), .O(z03));
  nand2  g27(.a(new_n68_), .b(new_n66_), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(x12), .c(x11), .O(z04));
  nor3   g29(.a(new_n67_), .b(new_n57_), .c(new_n47_), .O(z05));
  inv1   g30(.a(x11), .O(new_n74_));
  nor2   g31(.a(new_n71_), .b(new_n74_), .O(z06));
  nand2  g32(.a(x16), .b(new_n45_), .O(new_n76_));
  inv1   g33(.a(x12), .O(new_n77_));
  nand4  g34(.a(new_n77_), .b(x11), .c(x10), .d(new_n47_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n76_), .O(z07));
  inv1   g38(.a(x18), .O(new_n82_));
  nand3  g39(.a(x19), .b(new_n82_), .c(x17), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  inv1   g41(.a(x05), .O(new_n85_));
  inv1   g42(.a(x06), .O(new_n86_));
  inv1   g43(.a(x07), .O(new_n87_));
  inv1   g44(.a(x08), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n85_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  inv1   g47(.a(x03), .O(new_n91_));
  nand4  g48(.a(x04), .b(new_n91_), .c(x02), .d(new_n44_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n90_), .c(new_n84_), .O(new_n94_));
  aoi21  g51(.a(new_n94_), .b(new_n49_), .c(x01), .O(z08));
  nor2   g52(.a(new_n46_), .b(x01), .O(new_n96_));
  inv1   g53(.a(x13), .O(new_n97_));
  inv1   g54(.a(x14), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n97_), .c(x12), .d(new_n74_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  inv1   g57(.a(x15), .O(new_n101_));
  nand3  g58(.a(x20), .b(new_n49_), .c(new_n101_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n100_), .c(new_n96_), .O(new_n104_));
  inv1   g61(.a(x20), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n62_), .c(x18), .d(x01), .O(new_n106_));
  inv1   g63(.a(x21), .O(new_n107_));
  inv1   g64(.a(x22), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n107_), .c(new_n44_), .O(new_n109_));
  aoi21  g66(.a(new_n106_), .b(new_n104_), .c(new_n109_), .O(z09));
  inv1   g67(.a(new_n106_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(x21), .O(new_n112_));
  nand2  g69(.a(x22), .b(new_n44_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n112_), .c(new_n76_), .O(z10));
  nand2  g71(.a(new_n108_), .b(new_n44_), .O(new_n115_));
  nand4  g72(.a(new_n107_), .b(x20), .c(new_n49_), .d(x15), .O(new_n116_));
  inv1   g73(.a(new_n116_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n100_), .c(new_n96_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n112_), .c(new_n115_), .O(z11));
  nand2  g76(.a(x10), .b(x02), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(x01), .c(x00), .O(new_n121_));
  nor3   g78(.a(x16), .b(x01), .c(x00), .O(new_n122_));
  inv1   g79(.a(x23), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(new_n123_), .O(new_n124_));
  nand3  g81(.a(new_n62_), .b(new_n61_), .c(new_n46_), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(new_n124_), .c(new_n122_), .O(new_n126_));
  inv1   g83(.a(x24), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x09), .O(new_n128_));
  aoi21  g85(.a(new_n126_), .b(new_n121_), .c(new_n128_), .O(z12));
  nand3  g86(.a(new_n54_), .b(new_n62_), .c(x17), .O(new_n130_));
  aoi21  g87(.a(new_n130_), .b(new_n49_), .c(x01), .O(z13));
  nor2   g88(.a(new_n58_), .b(new_n48_), .O(z14));
  aoi21  g89(.a(new_n57_), .b(x01), .c(new_n46_), .O(new_n133_));
  aoi21  g90(.a(x19), .b(new_n46_), .c(x16), .O(new_n134_));
  oai22  g91(.a(new_n134_), .b(x01), .c(new_n133_), .d(new_n44_), .O(z15));
  nor2   g92(.a(new_n45_), .b(x00), .O(z16));
  nand2  g93(.a(new_n122_), .b(x02), .O(new_n137_));
  inv1   g94(.a(new_n137_), .O(z17));
endmodule


