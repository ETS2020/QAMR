// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n127_, new_n128_, new_n130_;
  nand2  g00(.a(x19), .b(x13), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor2   g05(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  inv1   g08(.a(x18), .O(new_n52_));
  inv1   g09(.a(x19), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n50_), .c(new_n44_), .O(z00));
  nand2  g12(.a(new_n49_), .b(x09), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n54_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n56_), .O(z02));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand3  g19(.a(x11), .b(x10), .c(new_n45_), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(new_n62_), .c(x12), .O(new_n65_));
  nand4  g22(.a(new_n49_), .b(new_n53_), .c(x18), .d(new_n51_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n65_), .c(new_n44_), .O(z03));
  nand2  g24(.a(x10), .b(new_n45_), .O(new_n68_));
  nand2  g25(.a(new_n62_), .b(new_n44_), .O(new_n69_));
  nor4   g26(.a(new_n69_), .b(new_n68_), .c(x12), .d(x11), .O(z04));
  nor3   g27(.a(new_n69_), .b(new_n58_), .c(new_n45_), .O(z05));
  oai21  g28(.a(new_n63_), .b(new_n61_), .c(new_n44_), .O(z06));
  oai21  g29(.a(new_n63_), .b(x12), .c(x01), .O(new_n73_));
  nand3  g30(.a(new_n73_), .b(x02), .c(x00), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n44_), .O(z07));
  inv1   g32(.a(x13), .O(new_n76_));
  nand3  g33(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  nor2   g36(.a(x18), .b(new_n51_), .O(new_n80_));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x04), .c(new_n81_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n80_), .c(new_n79_), .d(new_n78_), .O(new_n85_));
  aoi21  g42(.a(new_n85_), .b(new_n76_), .c(new_n53_), .O(z08));
  nand2  g43(.a(x02), .b(new_n47_), .O(new_n87_));
  inv1   g44(.a(x11), .O(new_n88_));
  nand3  g45(.a(new_n76_), .b(x12), .c(new_n88_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  inv1   g47(.a(x14), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nand4  g50(.a(x20), .b(new_n93_), .c(new_n92_), .d(new_n91_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  inv1   g52(.a(x20), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n53_), .c(x18), .d(x01), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  aoi21  g55(.a(new_n95_), .b(new_n90_), .c(new_n98_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n46_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n99_), .c(new_n44_), .O(z09));
  nor2   g60(.a(new_n97_), .b(new_n100_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(x22), .O(new_n105_));
  nand2  g62(.a(x15), .b(new_n91_), .O(new_n106_));
  nand4  g63(.a(new_n101_), .b(new_n100_), .c(x20), .d(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n90_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n105_), .c(x00), .O(z10));
  nand2  g67(.a(new_n101_), .b(new_n46_), .O(new_n111_));
  nand3  g68(.a(new_n100_), .b(x20), .c(new_n93_), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n106_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n90_), .c(new_n104_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n111_), .c(new_n44_), .O(z11));
  nand2  g72(.a(x01), .b(x00), .O(new_n116_));
  aoi21  g73(.a(x10), .b(x02), .c(new_n116_), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  inv1   g75(.a(x02), .O(new_n119_));
  nand3  g76(.a(new_n53_), .b(x17), .c(new_n119_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n118_), .c(new_n48_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n45_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n117_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n44_), .O(z12));
  nor2   g81(.a(new_n120_), .b(new_n48_), .O(z13));
  oai21  g82(.a(new_n59_), .b(new_n50_), .c(new_n44_), .O(z14));
  aoi21  g83(.a(new_n58_), .b(x01), .c(new_n119_), .O(new_n127_));
  aoi21  g84(.a(new_n119_), .b(new_n47_), .c(x13), .O(new_n128_));
  oai22  g85(.a(new_n128_), .b(new_n53_), .c(new_n127_), .d(new_n46_), .O(z15));
  nand3  g86(.a(new_n44_), .b(x01), .c(new_n46_), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z16));
  nand2  g88(.a(new_n77_), .b(new_n44_), .O(z17));
endmodule


