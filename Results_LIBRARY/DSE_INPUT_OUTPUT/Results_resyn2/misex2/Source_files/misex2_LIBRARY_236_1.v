// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n119_,
    new_n120_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(x10), .O(new_n49_));
  nor4   g06(.a(new_n49_), .b(new_n47_), .c(x09), .d(x02), .O(z00));
  inv1   g07(.a(x09), .O(new_n51_));
  nor4   g08(.a(new_n49_), .b(new_n47_), .c(new_n51_), .d(x02), .O(z01));
  inv1   g09(.a(x01), .O(new_n53_));
  inv1   g10(.a(x02), .O(new_n54_));
  inv1   g11(.a(x10), .O(new_n55_));
  inv1   g12(.a(new_n47_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n55_), .c(x09), .d(new_n54_), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(new_n53_), .c(x00), .O(z02));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n44_), .O(new_n60_));
  nand2  g17(.a(new_n48_), .b(x18), .O(new_n61_));
  nor2   g18(.a(new_n55_), .b(new_n54_), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x11), .c(new_n51_), .O(new_n63_));
  nand3  g20(.a(x12), .b(x01), .c(x00), .O(new_n64_));
  oai22  g21(.a(new_n64_), .b(new_n63_), .c(new_n61_), .d(new_n60_), .O(z03));
  inv1   g22(.a(x12), .O(new_n66_));
  inv1   g23(.a(x11), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(x02), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n66_), .c(x10), .d(new_n51_), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x00), .c(new_n53_), .O(z04));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x00), .c(new_n53_), .O(z05));
  aoi21  g30(.a(new_n63_), .b(x00), .c(new_n53_), .O(z06));
  inv1   g31(.a(x00), .O(new_n75_));
  nand2  g32(.a(x01), .b(new_n75_), .O(new_n76_));
  nand2  g33(.a(x11), .b(new_n51_), .O(new_n77_));
  nand2  g34(.a(new_n66_), .b(x10), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(new_n77_), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n76_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nand2  g42(.a(new_n48_), .b(x02), .O(new_n86_));
  nand3  g43(.a(x19), .b(new_n45_), .c(x17), .O(new_n87_));
  nor3   g44(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(z08));
  inv1   g45(.a(x15), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  inv1   g47(.a(x14), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n90_), .c(x12), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n68_), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(x20), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n93_), .c(new_n89_), .O(new_n99_));
  aoi21  g56(.a(new_n99_), .b(new_n53_), .c(x00), .O(z09));
  nand4  g57(.a(new_n96_), .b(new_n95_), .c(x20), .d(x16), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n93_), .c(x15), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(new_n53_), .c(x00), .O(z10));
  nand3  g61(.a(new_n98_), .b(new_n93_), .c(x15), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n53_), .c(x00), .O(z11));
  aoi21  g63(.a(x10), .b(x02), .c(new_n53_), .O(new_n107_));
  inv1   g64(.a(x23), .O(new_n108_));
  oai21  g65(.a(x19), .b(x02), .c(new_n108_), .O(new_n109_));
  aoi21  g66(.a(new_n59_), .b(new_n44_), .c(x00), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n109_), .c(new_n107_), .O(new_n111_));
  inv1   g68(.a(x24), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(x09), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n111_), .c(new_n76_), .O(z12));
  nand2  g71(.a(new_n59_), .b(x17), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n53_), .c(x00), .O(z13));
  nand4  g73(.a(new_n56_), .b(new_n55_), .c(new_n51_), .d(new_n54_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n53_), .c(x00), .O(z14));
  oai21  g75(.a(new_n55_), .b(new_n75_), .c(x01), .O(new_n119_));
  oai21  g76(.a(x19), .b(x00), .c(new_n54_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n119_), .O(z15));
  inv1   g78(.a(new_n86_), .O(z17));
  zero   g79(.O(z16));
endmodule


