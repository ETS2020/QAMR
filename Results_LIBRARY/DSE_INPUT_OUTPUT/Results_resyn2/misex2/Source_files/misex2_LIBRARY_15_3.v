// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n130_, new_n131_,
    new_n132_, new_n134_;
  inv1   g00(.a(x18), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  nand3  g02(.a(new_n45_), .b(new_n44_), .c(x10), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  nor2   g06(.a(x01), .b(x00), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  inv1   g08(.a(new_n51_), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n46_), .O(z00));
  nand2  g11(.a(new_n52_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n46_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand3  g14(.a(new_n45_), .b(new_n44_), .c(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  nand2  g16(.a(new_n45_), .b(x18), .O(new_n60_));
  inv1   g17(.a(x00), .O(new_n61_));
  nor2   g18(.a(new_n48_), .b(new_n61_), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(x11), .c(x10), .d(new_n47_), .O(new_n63_));
  inv1   g20(.a(x01), .O(new_n64_));
  nor2   g21(.a(x06), .b(new_n64_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(x12), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n63_), .c(new_n60_), .d(new_n51_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand3  g26(.a(new_n65_), .b(new_n69_), .c(new_n68_), .O(new_n70_));
  nand3  g27(.a(new_n62_), .b(x10), .c(new_n47_), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n70_), .O(z04));
  nand2  g29(.a(new_n65_), .b(x00), .O(new_n73_));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n73_), .c(new_n47_), .O(z05));
  inv1   g32(.a(x06), .O(new_n76_));
  aoi21  g33(.a(new_n63_), .b(new_n76_), .c(new_n64_), .O(z06));
  inv1   g34(.a(new_n62_), .O(new_n78_));
  nor2   g35(.a(x09), .b(x06), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(new_n69_), .c(x11), .d(x10), .O(new_n80_));
  aoi21  g37(.a(new_n80_), .b(x01), .c(new_n78_), .O(z07));
  nand2  g38(.a(x06), .b(x01), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nand2  g43(.a(new_n50_), .b(x02), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(z17));
  nand4  g45(.a(z17), .b(x19), .c(new_n44_), .d(x17), .O(new_n89_));
  oai21  g46(.a(new_n89_), .b(new_n86_), .c(new_n82_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n45_), .c(x18), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n65_), .O(new_n94_));
  nand2  g51(.a(x02), .b(new_n64_), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n68_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nor2   g55(.a(x15), .b(x14), .O(new_n99_));
  nor2   g56(.a(new_n91_), .b(x16), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n61_), .O(new_n104_));
  aoi21  g61(.a(new_n101_), .b(new_n94_), .c(new_n104_), .O(z09));
  nand4  g62(.a(new_n93_), .b(new_n65_), .c(x22), .d(x21), .O(new_n106_));
  inv1   g63(.a(x14), .O(new_n107_));
  nand4  g64(.a(new_n102_), .b(x20), .c(x15), .d(new_n107_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n98_), .c(new_n103_), .d(x16), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n106_), .c(x00), .O(z10));
  nand2  g68(.a(new_n103_), .b(new_n61_), .O(new_n112_));
  nor2   g69(.a(new_n108_), .b(x16), .O(new_n113_));
  nand2  g70(.a(x21), .b(x01), .O(new_n114_));
  nor2   g71(.a(new_n114_), .b(new_n92_), .O(new_n115_));
  aoi21  g72(.a(new_n113_), .b(new_n98_), .c(new_n115_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n112_), .c(new_n82_), .O(z11));
  inv1   g74(.a(x23), .O(new_n118_));
  nor2   g75(.a(x19), .b(x02), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(x17), .O(new_n120_));
  oai21  g77(.a(new_n119_), .b(new_n118_), .c(new_n120_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n50_), .O(new_n122_));
  nand3  g79(.a(new_n74_), .b(new_n65_), .c(x00), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n122_), .c(new_n125_), .O(z12));
  inv1   g83(.a(new_n50_), .O(new_n127_));
  nor2   g84(.a(new_n120_), .b(new_n127_), .O(z13));
  oai21  g85(.a(new_n58_), .b(new_n53_), .c(new_n82_), .O(z14));
  oai21  g86(.a(x10), .b(new_n64_), .c(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand3  g88(.a(x19), .b(new_n48_), .c(new_n64_), .O(new_n132_));
  nand3  g89(.a(new_n132_), .b(new_n131_), .c(new_n82_), .O(z15));
  nand3  g90(.a(new_n76_), .b(x01), .c(new_n61_), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(z16));
endmodule


