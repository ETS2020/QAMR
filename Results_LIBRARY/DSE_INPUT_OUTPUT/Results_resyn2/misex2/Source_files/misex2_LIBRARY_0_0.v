// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n123_, new_n124_, new_n126_;
  inv1   g00(.a(x10), .O(new_n44_));
  nor2   g01(.a(new_n44_), .b(x09), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nor2   g05(.a(x19), .b(x02), .O(new_n49_));
  inv1   g06(.a(x16), .O(new_n50_));
  nor2   g07(.a(x17), .b(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n46_), .O(z00));
  inv1   g10(.a(x17), .O(new_n54_));
  nand2  g11(.a(new_n49_), .b(new_n54_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nor2   g13(.a(x18), .b(x00), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n56_), .c(x10), .d(x09), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(x16), .c(x01), .O(z01));
  inv1   g16(.a(x09), .O(new_n60_));
  nor3   g17(.a(new_n52_), .b(x10), .c(new_n60_), .O(z02));
  inv1   g18(.a(x11), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n45_), .O(new_n65_));
  nor2   g22(.a(new_n65_), .b(new_n62_), .O(z06));
  nand2  g23(.a(z06), .b(x12), .O(new_n67_));
  inv1   g24(.a(x02), .O(new_n68_));
  nor2   g25(.a(x19), .b(new_n47_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n51_), .c(new_n48_), .d(new_n68_), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n67_), .O(z03));
  nor3   g28(.a(new_n65_), .b(x12), .c(x11), .O(z04));
  nand2  g29(.a(x10), .b(x09), .O(new_n73_));
  inv1   g30(.a(x01), .O(new_n74_));
  nand2  g31(.a(new_n50_), .b(new_n74_), .O(new_n75_));
  oai21  g32(.a(new_n63_), .b(new_n73_), .c(new_n75_), .O(z05));
  nand2  g33(.a(x02), .b(x00), .O(new_n77_));
  nor2   g34(.a(x12), .b(new_n62_), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(new_n45_), .c(new_n74_), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(z07));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g40(.a(x00), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand4  g42(.a(x04), .b(new_n85_), .c(x02), .d(new_n84_), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(x19), .c(new_n47_), .d(x17), .O(new_n88_));
  aoi21  g45(.a(new_n88_), .b(x16), .c(x01), .O(z08));
  nor2   g46(.a(new_n74_), .b(x00), .O(z16));
  nand2  g47(.a(z16), .b(new_n69_), .O(new_n91_));
  inv1   g48(.a(x21), .O(new_n92_));
  inv1   g49(.a(x22), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor3   g51(.a(new_n94_), .b(new_n91_), .c(x20), .O(z09));
  nor2   g52(.a(x14), .b(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x20), .c(x15), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand4  g55(.a(x12), .b(new_n62_), .c(x02), .d(new_n74_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(new_n94_), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  nand2  g58(.a(x21), .b(new_n101_), .O(new_n102_));
  inv1   g59(.a(x19), .O(new_n103_));
  nand4  g60(.a(x22), .b(new_n103_), .c(x18), .d(x01), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  aoi21  g62(.a(new_n100_), .b(new_n98_), .c(new_n105_), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(x00), .c(new_n75_), .O(z10));
  nand3  g64(.a(new_n93_), .b(x21), .c(new_n101_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(new_n91_), .c(new_n75_), .O(z11));
  nand2  g66(.a(x10), .b(x02), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(x01), .c(x00), .O(new_n111_));
  inv1   g68(.a(x23), .O(new_n112_));
  oai21  g69(.a(x19), .b(x02), .c(new_n112_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n55_), .c(new_n48_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n111_), .O(new_n115_));
  nor2   g72(.a(x24), .b(new_n60_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n75_), .O(z12));
  nand2  g75(.a(new_n49_), .b(new_n48_), .O(new_n119_));
  nor3   g76(.a(new_n119_), .b(new_n54_), .c(new_n50_), .O(z13));
  nand4  g77(.a(new_n57_), .b(new_n56_), .c(new_n44_), .d(new_n60_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(x16), .c(x01), .O(z14));
  aoi21  g79(.a(x19), .b(new_n74_), .c(x00), .O(new_n123_));
  nand2  g80(.a(x16), .b(new_n68_), .O(new_n124_));
  oai21  g81(.a(new_n124_), .b(new_n123_), .c(new_n111_), .O(z15));
  nand2  g82(.a(x02), .b(new_n84_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(x16), .c(x01), .O(z17));
endmodule


