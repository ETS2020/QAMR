// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n117_,
    new_n118_, new_n120_, new_n121_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor2   g05(.a(new_n48_), .b(x17), .O(new_n49_));
  inv1   g06(.a(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n49_), .c(new_n45_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x00), .O(z00));
  inv1   g10(.a(x09), .O(new_n54_));
  nor2   g11(.a(new_n50_), .b(new_n54_), .O(new_n55_));
  nand3  g12(.a(new_n55_), .b(new_n49_), .c(new_n45_), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n44_), .c(x00), .O(z01));
  nand2  g14(.a(new_n49_), .b(new_n45_), .O(new_n58_));
  nor2   g15(.a(x01), .b(x00), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(new_n50_), .c(x09), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n58_), .O(z02));
  inv1   g18(.a(x11), .O(new_n62_));
  nand4  g19(.a(x10), .b(x02), .c(x01), .d(x00), .O(new_n63_));
  nor3   g20(.a(new_n63_), .b(new_n62_), .c(x09), .O(z06));
  nand2  g21(.a(z06), .b(x12), .O(new_n65_));
  nand3  g22(.a(new_n59_), .b(new_n49_), .c(x18), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n65_), .O(z03));
  nor2   g24(.a(x12), .b(new_n50_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n62_), .c(new_n54_), .d(x02), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x00), .c(new_n44_), .O(z04));
  nand2  g27(.a(x02), .b(x00), .O(new_n71_));
  nand2  g28(.a(new_n55_), .b(x01), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n71_), .O(z05));
  inv1   g30(.a(x00), .O(new_n74_));
  nor2   g31(.a(new_n44_), .b(new_n74_), .O(new_n75_));
  nand2  g32(.a(x01), .b(new_n74_), .O(new_n76_));
  nand3  g33(.a(new_n68_), .b(x11), .c(new_n54_), .O(new_n77_));
  aoi22  g34(.a(new_n77_), .b(new_n75_), .c(new_n76_), .d(new_n71_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nand2  g39(.a(new_n59_), .b(x02), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n45_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(z08));
  inv1   g42(.a(x15), .O(new_n86_));
  inv1   g43(.a(x16), .O(new_n87_));
  inv1   g44(.a(x21), .O(new_n88_));
  inv1   g45(.a(x22), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n88_), .c(x20), .d(new_n87_), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand2  g48(.a(x12), .b(new_n62_), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x02), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n91_), .c(new_n86_), .O(new_n97_));
  aoi21  g54(.a(new_n97_), .b(new_n44_), .c(x00), .O(z09));
  nand3  g55(.a(new_n89_), .b(new_n88_), .c(x20), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  nor2   g57(.a(new_n87_), .b(new_n86_), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n96_), .c(new_n100_), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n44_), .c(x00), .O(z10));
  nand3  g60(.a(x15), .b(new_n94_), .c(new_n93_), .O(new_n104_));
  nor4   g61(.a(new_n104_), .b(new_n92_), .c(new_n90_), .d(new_n83_), .O(z11));
  aoi21  g62(.a(x10), .b(x02), .c(new_n44_), .O(new_n106_));
  inv1   g63(.a(x23), .O(new_n107_));
  nand2  g64(.a(new_n48_), .b(new_n107_), .O(new_n108_));
  inv1   g65(.a(x17), .O(new_n109_));
  nor2   g66(.a(x19), .b(x02), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n109_), .c(x00), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n108_), .c(new_n106_), .O(new_n112_));
  inv1   g69(.a(x24), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(x09), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n112_), .c(new_n76_), .O(z12));
  nor4   g72(.a(new_n48_), .b(new_n109_), .c(x01), .d(x00), .O(z13));
  nor2   g73(.a(x10), .b(x09), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n49_), .c(new_n45_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n44_), .c(x00), .O(z14));
  oai21  g76(.a(new_n50_), .b(new_n74_), .c(x01), .O(new_n120_));
  oai21  g77(.a(x19), .b(x00), .c(new_n46_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(z15));
  inv1   g79(.a(new_n83_), .O(z17));
  zero   g80(.O(z16));
endmodule


