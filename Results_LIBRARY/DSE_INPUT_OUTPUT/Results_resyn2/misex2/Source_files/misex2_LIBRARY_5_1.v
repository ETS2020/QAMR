// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  inv1   g04(.a(x00), .O(new_n48_));
  inv1   g05(.a(x01), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nor3   g08(.a(new_n51_), .b(new_n47_), .c(x09), .O(z00));
  inv1   g09(.a(x09), .O(new_n53_));
  or2    g10(.a(new_n51_), .b(new_n53_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n47_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x18), .c(new_n44_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n49_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n48_), .O(new_n62_));
  nand4  g19(.a(x10), .b(new_n53_), .c(x02), .d(x01), .O(new_n63_));
  nand2  g20(.a(x12), .b(x11), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nor2   g24(.a(new_n63_), .b(new_n48_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  inv1   g26(.a(new_n69_), .O(z04));
  nand3  g27(.a(x09), .b(x01), .c(x00), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n56_), .c(new_n50_), .O(z05));
  nand2  g29(.a(new_n68_), .b(x11), .O(new_n73_));
  inv1   g30(.a(new_n73_), .O(z06));
  nand2  g31(.a(x01), .b(new_n48_), .O(new_n75_));
  nand4  g32(.a(new_n67_), .b(x11), .c(x10), .d(new_n53_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n75_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand3  g40(.a(x02), .b(new_n49_), .c(new_n48_), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n45_), .c(x17), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(z08));
  inv1   g43(.a(x16), .O(new_n87_));
  inv1   g44(.a(new_n84_), .O(new_n88_));
  inv1   g45(.a(x21), .O(new_n89_));
  inv1   g46(.a(x22), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nor2   g49(.a(x14), .b(x13), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(x12), .c(new_n66_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n92_), .c(new_n88_), .d(new_n87_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(x15), .O(z09));
  inv1   g54(.a(x15), .O(new_n98_));
  nor2   g55(.a(new_n87_), .b(new_n98_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n95_), .c(new_n92_), .d(new_n88_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(z10));
  nor2   g58(.a(new_n96_), .b(new_n98_), .O(z11));
  aoi21  g59(.a(x10), .b(x02), .c(new_n49_), .O(new_n103_));
  inv1   g60(.a(x23), .O(new_n104_));
  oai21  g61(.a(x19), .b(x02), .c(new_n104_), .O(new_n105_));
  aoi21  g62(.a(new_n59_), .b(new_n44_), .c(x00), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(new_n107_));
  inv1   g64(.a(x24), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(x09), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(new_n107_), .c(new_n75_), .O(z12));
  nand2  g67(.a(new_n59_), .b(x17), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n49_), .c(x00), .O(z13));
  inv1   g69(.a(new_n57_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n53_), .c(new_n50_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n49_), .c(x00), .O(z14));
  oai21  g72(.a(new_n56_), .b(new_n48_), .c(x01), .O(new_n116_));
  oai21  g73(.a(x19), .b(x00), .c(new_n50_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(z15));
  aoi21  g75(.a(new_n50_), .b(new_n49_), .c(x00), .O(z17));
  zero   g76(.O(z16));
endmodule


