// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n130_, new_n131_,
    new_n133_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x10), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(x09), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nor2   g05(.a(x17), .b(x00), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x02), .O(z00));
  nand3  g10(.a(new_n51_), .b(x10), .c(x09), .O(new_n54_));
  aoi21  g11(.a(new_n54_), .b(new_n44_), .c(x02), .O(z01));
  inv1   g12(.a(x09), .O(new_n56_));
  nor2   g13(.a(x02), .b(x01), .O(new_n57_));
  inv1   g14(.a(new_n57_), .O(new_n58_));
  nor4   g15(.a(new_n58_), .b(new_n50_), .c(x10), .d(new_n56_), .O(z02));
  nand2  g16(.a(new_n48_), .b(x18), .O(new_n60_));
  nand2  g17(.a(new_n57_), .b(new_n49_), .O(new_n61_));
  nand3  g18(.a(x12), .b(x11), .c(x02), .O(new_n62_));
  nand3  g19(.a(new_n46_), .b(x01), .c(x00), .O(new_n63_));
  oai22  g20(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(z03));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g22(.a(x12), .O(new_n66_));
  nand2  g23(.a(new_n46_), .b(new_n66_), .O(new_n67_));
  nor3   g24(.a(new_n67_), .b(new_n65_), .c(x11), .O(z04));
  nor3   g25(.a(new_n65_), .b(new_n45_), .c(new_n56_), .O(z05));
  nand3  g26(.a(new_n46_), .b(x11), .c(x00), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x02), .c(new_n44_), .O(z06));
  inv1   g28(.a(x02), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(x01), .O(new_n73_));
  inv1   g30(.a(x11), .O(new_n74_));
  oai21  g31(.a(new_n67_), .b(new_n74_), .c(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n73_), .O(z07));
  nor2   g34(.a(x08), .b(x07), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x19), .c(new_n47_), .O(new_n79_));
  inv1   g36(.a(x04), .O(new_n80_));
  nor2   g37(.a(new_n80_), .b(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nand2  g39(.a(x02), .b(new_n44_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  inv1   g41(.a(x17), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(x00), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n84_), .c(new_n82_), .d(new_n81_), .O(new_n87_));
  oai21  g44(.a(new_n87_), .b(new_n79_), .c(new_n73_), .O(z08));
  inv1   g45(.a(x20), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n48_), .c(x18), .d(x01), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n74_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n83_), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nand4  g54(.a(x20), .b(new_n97_), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  aoi21  g56(.a(new_n99_), .b(new_n94_), .c(new_n91_), .O(new_n100_));
  inv1   g57(.a(x00), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n100_), .c(new_n73_), .O(z09));
  nor2   g62(.a(new_n90_), .b(new_n102_), .O(new_n106_));
  nand4  g63(.a(new_n102_), .b(x20), .c(x15), .d(new_n95_), .O(new_n107_));
  nand2  g64(.a(new_n103_), .b(x16), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  aoi22  g66(.a(new_n109_), .b(new_n94_), .c(new_n106_), .d(x22), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n73_), .O(z10));
  nand2  g68(.a(new_n103_), .b(new_n101_), .O(new_n112_));
  nor2   g69(.a(new_n107_), .b(x16), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n94_), .c(new_n106_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n112_), .c(new_n73_), .O(z11));
  inv1   g72(.a(x24), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(new_n45_), .c(x09), .d(x00), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x02), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x01), .O(new_n119_));
  nor2   g76(.a(x19), .b(x02), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x17), .O(new_n121_));
  nand2  g78(.a(x23), .b(new_n44_), .O(new_n122_));
  oai21  g79(.a(new_n122_), .b(new_n120_), .c(new_n121_), .O(new_n123_));
  nand4  g80(.a(new_n123_), .b(new_n116_), .c(x09), .d(new_n101_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n119_), .O(z12));
  nand2  g82(.a(new_n86_), .b(new_n48_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n44_), .c(x02), .O(z13));
  nand4  g84(.a(new_n57_), .b(new_n51_), .c(new_n45_), .d(new_n56_), .O(new_n128_));
  inv1   g85(.a(new_n128_), .O(z14));
  aoi21  g86(.a(new_n45_), .b(x01), .c(new_n72_), .O(new_n130_));
  nand2  g87(.a(new_n73_), .b(x00), .O(new_n131_));
  oai22  g88(.a(new_n131_), .b(new_n130_), .c(new_n58_), .d(new_n48_), .O(z15));
  nand3  g89(.a(x02), .b(x01), .c(new_n101_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z16));
  oai21  g91(.a(new_n83_), .b(x00), .c(new_n73_), .O(z17));
endmodule


