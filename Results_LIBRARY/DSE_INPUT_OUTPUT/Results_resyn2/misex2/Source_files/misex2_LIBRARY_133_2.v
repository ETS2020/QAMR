// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n129_, new_n131_,
    new_n132_, new_n133_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  inv1   g02(.a(x16), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(x14), .O(new_n47_));
  nor2   g04(.a(x18), .b(x17), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  or2    g06(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nor2   g08(.a(x09), .b(x02), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n50_), .O(z00));
  inv1   g11(.a(x02), .O(new_n55_));
  nand3  g12(.a(new_n51_), .b(x09), .c(new_n55_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n50_), .O(z01));
  nor3   g14(.a(new_n56_), .b(new_n49_), .c(x10), .O(z02));
  nor2   g15(.a(x17), .b(x02), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n51_), .c(new_n45_), .d(x18), .O(new_n60_));
  inv1   g17(.a(x11), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n44_), .O(new_n62_));
  nor2   g19(.a(x09), .b(new_n55_), .O(new_n63_));
  inv1   g20(.a(x00), .O(new_n64_));
  inv1   g21(.a(x01), .O(new_n65_));
  nor2   g22(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n63_), .c(new_n62_), .d(x12), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n60_), .c(new_n47_), .O(z03));
  and2   g25(.a(new_n66_), .b(new_n47_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(new_n61_), .c(x10), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n70_), .O(z04));
  nand4  g30(.a(new_n69_), .b(x10), .c(x09), .d(x02), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(z05));
  nand3  g32(.a(new_n69_), .b(new_n63_), .c(new_n62_), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z06));
  nor2   g34(.a(x12), .b(x09), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(new_n62_), .c(new_n65_), .O(new_n79_));
  nand2  g36(.a(x02), .b(x00), .O(new_n80_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n47_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  inv1   g42(.a(x18), .O(new_n86_));
  nand2  g43(.a(new_n51_), .b(x02), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x19), .c(new_n86_), .d(x17), .O(new_n89_));
  oai21  g46(.a(new_n89_), .b(new_n85_), .c(new_n47_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n45_), .c(x18), .d(x01), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n47_), .O(new_n94_));
  nand2  g51(.a(x02), .b(new_n65_), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n61_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nor2   g55(.a(x15), .b(x14), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(x20), .d(new_n46_), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n64_), .O(new_n103_));
  aoi21  g60(.a(new_n100_), .b(new_n94_), .c(new_n103_), .O(z09));
  nand2  g61(.a(x22), .b(x21), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(new_n92_), .O(new_n106_));
  inv1   g63(.a(x14), .O(new_n107_));
  nand4  g64(.a(new_n101_), .b(x20), .c(x15), .d(new_n107_), .O(new_n108_));
  nand2  g65(.a(new_n102_), .b(x16), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n98_), .c(new_n106_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(x00), .c(new_n47_), .O(z10));
  nand2  g69(.a(new_n102_), .b(new_n64_), .O(new_n113_));
  nand3  g70(.a(new_n93_), .b(new_n47_), .c(x21), .O(new_n114_));
  inv1   g71(.a(new_n108_), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n98_), .c(new_n46_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(z11));
  oai21  g74(.a(new_n44_), .b(new_n55_), .c(new_n66_), .O(new_n118_));
  inv1   g75(.a(x23), .O(new_n119_));
  nor2   g76(.a(x19), .b(x02), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x17), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n119_), .c(new_n121_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n51_), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand3  g81(.a(new_n47_), .b(new_n124_), .c(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n118_), .c(new_n125_), .O(z12));
  inv1   g83(.a(new_n51_), .O(new_n127_));
  oai21  g84(.a(new_n121_), .b(new_n127_), .c(new_n47_), .O(z13));
  nand3  g85(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(new_n129_));
  oai21  g86(.a(new_n129_), .b(new_n53_), .c(new_n47_), .O(z14));
  oai21  g87(.a(x10), .b(new_n65_), .c(x02), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x00), .O(new_n132_));
  nand3  g89(.a(x19), .b(new_n55_), .c(new_n65_), .O(new_n133_));
  nand3  g90(.a(new_n133_), .b(new_n132_), .c(new_n47_), .O(z15));
  oai21  g91(.a(new_n65_), .b(x00), .c(new_n47_), .O(z16));
  aoi21  g92(.a(new_n46_), .b(x14), .c(new_n87_), .O(z17));
endmodule


