// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n87_, new_n88_;
  nor2   g00(.a(x01), .b(x00), .O(z00));
  inv1   g01(.a(z00), .O(new_n46_));
  nand3  g02(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x09), .O(new_n49_));
  nand3  g05(.a(x11), .b(x10), .c(new_n49_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(new_n48_), .c(x12), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n46_), .O(z03));
  inv1   g09(.a(x11), .O(new_n54_));
  inv1   g10(.a(x12), .O(new_n55_));
  nand4  g11(.a(new_n55_), .b(new_n54_), .c(x10), .d(new_n49_), .O(new_n56_));
  oai21  g12(.a(new_n56_), .b(new_n47_), .c(new_n46_), .O(z04));
  nand2  g13(.a(x10), .b(x02), .O(new_n58_));
  nand3  g14(.a(x09), .b(x01), .c(x00), .O(new_n59_));
  oai21  g15(.a(new_n59_), .b(new_n58_), .c(new_n46_), .O(z05));
  oai21  g16(.a(new_n50_), .b(new_n47_), .c(new_n46_), .O(z06));
  inv1   g17(.a(x00), .O(new_n62_));
  inv1   g18(.a(x01), .O(new_n63_));
  oai21  g19(.a(x02), .b(new_n62_), .c(new_n63_), .O(new_n64_));
  nand3  g20(.a(new_n55_), .b(x02), .c(x00), .O(new_n65_));
  oai21  g21(.a(new_n65_), .b(new_n50_), .c(new_n64_), .O(z07));
  nand2  g22(.a(x01), .b(new_n62_), .O(new_n67_));
  inv1   g23(.a(x19), .O(new_n68_));
  inv1   g24(.a(x20), .O(new_n69_));
  nand3  g25(.a(new_n69_), .b(new_n68_), .c(x18), .O(new_n70_));
  inv1   g26(.a(x21), .O(new_n71_));
  inv1   g27(.a(x22), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor3   g29(.a(new_n73_), .b(new_n70_), .c(new_n67_), .O(z09));
  nand2  g30(.a(x22), .b(x21), .O(new_n75_));
  nor3   g31(.a(new_n75_), .b(new_n70_), .c(new_n67_), .O(z10));
  inv1   g32(.a(x18), .O(new_n77_));
  nor2   g33(.a(x19), .b(new_n77_), .O(new_n78_));
  nor2   g34(.a(x22), .b(new_n71_), .O(new_n79_));
  nand3  g35(.a(new_n79_), .b(new_n78_), .c(new_n69_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(x01), .c(x00), .O(z11));
  inv1   g37(.a(x24), .O(new_n82_));
  nand2  g38(.a(new_n58_), .b(new_n82_), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n59_), .c(new_n46_), .O(z12));
  inv1   g40(.a(x10), .O(new_n87_));
  nand2  g41(.a(new_n87_), .b(x01), .O(new_n88_));
  aoi21  g42(.a(new_n88_), .b(x02), .c(new_n62_), .O(z15));
  inv1   g43(.a(new_n67_), .O(z16));
  zero   g44(.O(z01));
  zero   g45(.O(z13));
  zero   g46(.O(z14));
  nor2   g47(.a(x01), .b(x00), .O(z02));
  nor2   g48(.a(x01), .b(x00), .O(z08));
  nor2   g49(.a(x01), .b(x00), .O(z17));
endmodule


