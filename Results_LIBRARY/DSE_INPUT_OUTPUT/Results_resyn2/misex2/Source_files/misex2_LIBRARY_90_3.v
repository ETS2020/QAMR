// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n128_, new_n129_, new_n131_, new_n133_;
  nand2  g00(.a(x21), .b(x19), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  inv1   g15(.a(x12), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nand3  g17(.a(x11), .b(x10), .c(new_n50_), .O(new_n61_));
  or2    g18(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nor2   g19(.a(x02), .b(x01), .O(new_n63_));
  nor2   g20(.a(x17), .b(x00), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n63_), .c(new_n47_), .d(x18), .O(new_n65_));
  and2   g22(.a(new_n65_), .b(new_n44_), .O(new_n66_));
  oai21  g23(.a(new_n62_), .b(new_n59_), .c(new_n66_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  nand4  g25(.a(new_n59_), .b(new_n68_), .c(x10), .d(new_n50_), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n60_), .c(new_n44_), .O(z04));
  nand3  g27(.a(new_n44_), .b(x10), .c(x09), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n60_), .O(z05));
  nand2  g29(.a(new_n62_), .b(new_n44_), .O(z06));
  oai21  g30(.a(new_n61_), .b(x12), .c(x01), .O(new_n74_));
  nand3  g31(.a(new_n74_), .b(x02), .c(x00), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(new_n44_), .O(z07));
  inv1   g33(.a(x03), .O(new_n77_));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(new_n78_), .c(x04), .d(new_n77_), .O(new_n80_));
  nor2   g37(.a(x18), .b(new_n45_), .O(new_n81_));
  nor2   g38(.a(x21), .b(new_n47_), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(new_n51_), .d(x02), .O(new_n83_));
  nor2   g40(.a(new_n83_), .b(new_n80_), .O(z08));
  inv1   g41(.a(x01), .O(new_n85_));
  nand2  g42(.a(x02), .b(new_n85_), .O(new_n86_));
  inv1   g43(.a(x13), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(x12), .c(new_n68_), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  inv1   g46(.a(x14), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nand4  g49(.a(x20), .b(new_n92_), .c(new_n91_), .d(new_n90_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  inv1   g51(.a(x20), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n47_), .c(x18), .d(x01), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  aoi21  g54(.a(new_n94_), .b(new_n89_), .c(new_n97_), .O(new_n98_));
  inv1   g55(.a(x00), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n98_), .c(new_n44_), .O(z09));
  nand3  g60(.a(x21), .b(x18), .c(x01), .O(new_n104_));
  nor3   g61(.a(new_n104_), .b(new_n101_), .c(x20), .O(new_n105_));
  nand4  g62(.a(new_n100_), .b(x20), .c(x15), .d(new_n90_), .O(new_n106_));
  nand2  g63(.a(new_n101_), .b(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n89_), .c(new_n105_), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g67(.a(new_n101_), .b(new_n99_), .O(new_n111_));
  inv1   g68(.a(new_n104_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n95_), .c(new_n47_), .O(new_n113_));
  inv1   g70(.a(new_n106_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n89_), .c(new_n92_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n113_), .c(new_n111_), .O(z11));
  nand2  g73(.a(x01), .b(x00), .O(new_n117_));
  aoi21  g74(.a(x10), .b(x02), .c(new_n117_), .O(new_n118_));
  inv1   g75(.a(new_n51_), .O(new_n119_));
  oai21  g76(.a(x19), .b(x02), .c(x23), .O(new_n120_));
  nand3  g77(.a(new_n47_), .b(x17), .c(new_n49_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  nor2   g79(.a(x24), .b(new_n50_), .O(new_n123_));
  oai21  g80(.a(new_n122_), .b(new_n118_), .c(new_n123_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n44_), .O(z12));
  nor2   g82(.a(new_n121_), .b(new_n119_), .O(z13));
  nor2   g83(.a(new_n57_), .b(new_n52_), .O(z14));
  aoi21  g84(.a(new_n56_), .b(x01), .c(new_n49_), .O(new_n128_));
  oai21  g85(.a(new_n63_), .b(x21), .c(x19), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(new_n99_), .c(new_n129_), .O(z15));
  nand3  g87(.a(new_n44_), .b(x01), .c(new_n99_), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z16));
  nand3  g89(.a(new_n51_), .b(new_n44_), .c(x02), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z17));
endmodule


