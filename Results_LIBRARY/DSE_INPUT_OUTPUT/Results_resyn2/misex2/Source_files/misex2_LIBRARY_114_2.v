// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n124_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor3   g01(.a(x02), .b(x01), .c(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n46_), .O(z00));
  nand2  g08(.a(new_n45_), .b(x09), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  nand2  g13(.a(x10), .b(x02), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x09), .O(new_n58_));
  nand3  g15(.a(x20), .b(x01), .c(x00), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n58_), .c(x12), .d(x11), .O(new_n61_));
  nand4  g18(.a(new_n45_), .b(new_n49_), .c(x18), .d(new_n47_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n61_), .O(z03));
  inv1   g20(.a(new_n58_), .O(new_n64_));
  inv1   g21(.a(x11), .O(new_n65_));
  inv1   g22(.a(x12), .O(new_n66_));
  nand3  g23(.a(new_n60_), .b(new_n66_), .c(new_n65_), .O(new_n67_));
  nor2   g24(.a(new_n67_), .b(new_n64_), .O(z04));
  inv1   g25(.a(x01), .O(new_n69_));
  nand4  g26(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x20), .c(new_n69_), .O(z05));
  nand2  g28(.a(new_n60_), .b(x11), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n64_), .O(z06));
  inv1   g30(.a(x20), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x01), .O(new_n75_));
  nand4  g32(.a(new_n66_), .b(x11), .c(x10), .d(new_n44_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n75_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  inv1   g40(.a(x00), .O(new_n84_));
  nand3  g41(.a(x02), .b(new_n69_), .c(new_n84_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(z17));
  nor2   g43(.a(x18), .b(new_n47_), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(z17), .c(x19), .O(new_n88_));
  oai21  g45(.a(new_n88_), .b(new_n83_), .c(new_n75_), .O(z08));
  inv1   g46(.a(x16), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n65_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(z17), .c(new_n90_), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nor2   g55(.a(x15), .b(x14), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(new_n94_), .O(z09));
  nor2   g58(.a(new_n92_), .b(new_n85_), .O(new_n102_));
  inv1   g59(.a(x14), .O(new_n103_));
  nand2  g60(.a(x15), .b(new_n103_), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n97_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n102_), .c(x16), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n75_), .O(z10));
  nand3  g64(.a(new_n105_), .b(new_n102_), .c(new_n90_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n75_), .O(z11));
  nand2  g66(.a(new_n60_), .b(new_n57_), .O(new_n110_));
  nor2   g67(.a(x01), .b(x00), .O(new_n111_));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  inv1   g69(.a(x02), .O(new_n113_));
  nand3  g70(.a(new_n49_), .b(x17), .c(new_n113_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n111_), .O(new_n116_));
  inv1   g73(.a(x24), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x09), .O(new_n118_));
  aoi21  g75(.a(new_n116_), .b(new_n110_), .c(new_n118_), .O(z12));
  nand3  g76(.a(new_n45_), .b(new_n49_), .c(x17), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n75_), .O(z13));
  nor2   g78(.a(new_n55_), .b(new_n46_), .O(z14));
  aoi21  g79(.a(x20), .b(x00), .c(new_n69_), .O(z16));
  oai21  g80(.a(x19), .b(x00), .c(new_n113_), .O(new_n124_));
  oai22  g81(.a(new_n124_), .b(z16), .c(new_n59_), .d(x10), .O(z15));
endmodule


