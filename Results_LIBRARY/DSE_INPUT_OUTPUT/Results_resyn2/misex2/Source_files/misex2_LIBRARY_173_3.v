// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n118_,
    new_n119_, new_n120_, new_n122_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x01), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n49_), .O(z00));
  nand2  g11(.a(new_n48_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z01));
  nand2  g13(.a(x16), .b(x01), .O(new_n57_));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n55_), .c(new_n57_), .O(z02));
  inv1   g17(.a(x11), .O(new_n61_));
  nor2   g18(.a(x16), .b(new_n61_), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nor3   g20(.a(new_n63_), .b(new_n58_), .c(x09), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(new_n62_), .c(x12), .O(new_n65_));
  nand2  g22(.a(new_n48_), .b(new_n52_), .O(new_n66_));
  nand2  g23(.a(x18), .b(new_n50_), .O(new_n67_));
  oai21  g24(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(z03));
  nor3   g25(.a(x16), .b(x12), .c(x11), .O(new_n69_));
  and2   g26(.a(new_n69_), .b(new_n64_), .O(z04));
  nor4   g27(.a(new_n63_), .b(x16), .c(new_n58_), .d(new_n44_), .O(z05));
  and2   g28(.a(new_n64_), .b(new_n62_), .O(z06));
  inv1   g29(.a(x12), .O(new_n73_));
  nand4  g30(.a(new_n62_), .b(new_n73_), .c(x10), .d(new_n44_), .O(new_n74_));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  aoi21  g32(.a(new_n74_), .b(x01), .c(new_n75_), .O(z07));
  inv1   g33(.a(x03), .O(new_n77_));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(new_n78_), .c(x04), .d(new_n77_), .O(new_n80_));
  nand3  g37(.a(x02), .b(new_n46_), .c(new_n45_), .O(new_n81_));
  nand3  g38(.a(x19), .b(new_n51_), .c(x17), .O(new_n82_));
  nor3   g39(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(z08));
  inv1   g40(.a(x20), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n52_), .c(x18), .d(x01), .O(new_n85_));
  inv1   g42(.a(x15), .O(new_n86_));
  nand4  g43(.a(x12), .b(new_n61_), .c(x02), .d(new_n46_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nor2   g45(.a(x14), .b(x13), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n88_), .c(x20), .d(new_n86_), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n91_), .c(new_n45_), .O(new_n93_));
  or2    g50(.a(new_n93_), .b(x21), .O(new_n94_));
  aoi21  g51(.a(new_n90_), .b(new_n85_), .c(new_n94_), .O(z09));
  inv1   g52(.a(x21), .O(new_n96_));
  nand3  g53(.a(new_n89_), .b(new_n96_), .c(x20), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nor3   g55(.a(x22), .b(new_n91_), .c(new_n86_), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(new_n88_), .O(new_n100_));
  inv1   g57(.a(new_n85_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(x22), .c(x21), .d(new_n91_), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n100_), .c(x00), .O(z10));
  nand2  g60(.a(new_n101_), .b(x21), .O(new_n104_));
  nand3  g61(.a(new_n98_), .b(new_n88_), .c(x15), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n104_), .c(new_n93_), .O(z11));
  nand2  g63(.a(x01), .b(x00), .O(new_n107_));
  aoi21  g64(.a(x10), .b(x02), .c(new_n107_), .O(new_n108_));
  oai21  g65(.a(x19), .b(x02), .c(x23), .O(new_n109_));
  inv1   g66(.a(x02), .O(new_n110_));
  nand3  g67(.a(new_n52_), .b(x17), .c(new_n110_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n109_), .c(new_n47_), .O(new_n112_));
  nor2   g69(.a(x24), .b(new_n44_), .O(new_n113_));
  oai21  g70(.a(new_n112_), .b(new_n108_), .c(new_n113_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n57_), .O(z12));
  oai21  g72(.a(new_n66_), .b(new_n50_), .c(new_n57_), .O(z13));
  nor2   g73(.a(new_n59_), .b(new_n49_), .O(z14));
  oai21  g74(.a(x10), .b(new_n46_), .c(x02), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x00), .O(new_n119_));
  nand3  g76(.a(x19), .b(new_n110_), .c(new_n46_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n119_), .c(new_n57_), .O(z15));
  nand3  g78(.a(new_n91_), .b(x01), .c(new_n45_), .O(new_n122_));
  inv1   g79(.a(new_n122_), .O(z16));
  inv1   g80(.a(new_n81_), .O(z17));
endmodule


