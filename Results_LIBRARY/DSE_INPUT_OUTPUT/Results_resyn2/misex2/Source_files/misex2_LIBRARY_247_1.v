// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n129_, new_n131_,
    new_n132_, new_n133_;
  inv1   g00(.a(x19), .O(new_n44_));
  inv1   g01(.a(x21), .O(new_n45_));
  nand2  g02(.a(x22), .b(new_n45_), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nor3   g06(.a(x02), .b(x01), .c(x00), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n48_), .O(z00));
  nand2  g09(.a(new_n50_), .b(x09), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n48_), .O(z01));
  nand3  g11(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n55_));
  nor3   g12(.a(new_n53_), .b(new_n55_), .c(x10), .O(z02));
  inv1   g13(.a(x17), .O(new_n57_));
  inv1   g14(.a(x18), .O(new_n58_));
  nor2   g15(.a(x19), .b(new_n58_), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(new_n50_), .c(new_n57_), .O(new_n60_));
  inv1   g17(.a(x10), .O(new_n61_));
  inv1   g18(.a(x11), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g20(.a(x02), .O(new_n64_));
  nor2   g21(.a(x09), .b(new_n64_), .O(new_n65_));
  nand2  g22(.a(x01), .b(x00), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n65_), .c(new_n63_), .d(x12), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n60_), .c(new_n46_), .O(z03));
  inv1   g26(.a(x22), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x21), .O(new_n71_));
  nor2   g28(.a(new_n66_), .b(new_n71_), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n65_), .O(new_n73_));
  inv1   g30(.a(x12), .O(new_n74_));
  nand3  g31(.a(new_n74_), .b(new_n62_), .c(x10), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n73_), .O(z04));
  nand4  g33(.a(new_n72_), .b(x10), .c(x09), .d(x02), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(z05));
  nand3  g35(.a(new_n72_), .b(new_n65_), .c(new_n63_), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z06));
  inv1   g37(.a(x01), .O(new_n81_));
  nor2   g38(.a(x12), .b(x09), .O(new_n82_));
  aoi21  g39(.a(new_n82_), .b(new_n63_), .c(new_n81_), .O(new_n83_));
  nand2  g40(.a(x02), .b(x00), .O(new_n84_));
  oai21  g41(.a(new_n84_), .b(new_n83_), .c(new_n46_), .O(z07));
  nor2   g42(.a(x06), .b(x05), .O(new_n86_));
  nor2   g43(.a(x08), .b(x07), .O(new_n87_));
  inv1   g44(.a(x04), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(x03), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n87_), .c(new_n86_), .d(new_n46_), .O(new_n90_));
  inv1   g47(.a(x00), .O(new_n91_));
  nand3  g48(.a(x02), .b(new_n81_), .c(new_n91_), .O(new_n92_));
  nand3  g49(.a(x19), .b(new_n58_), .c(x17), .O(new_n93_));
  nor3   g50(.a(new_n93_), .b(new_n92_), .c(new_n90_), .O(z08));
  nor2   g51(.a(x20), .b(new_n81_), .O(new_n95_));
  nand2  g52(.a(new_n95_), .b(new_n59_), .O(new_n96_));
  nand2  g53(.a(x02), .b(new_n81_), .O(new_n97_));
  inv1   g54(.a(x13), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(x12), .c(new_n62_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nor2   g57(.a(x15), .b(x14), .O(new_n101_));
  inv1   g58(.a(x20), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(x16), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  nand3  g61(.a(new_n70_), .b(new_n45_), .c(new_n91_), .O(new_n105_));
  aoi21  g62(.a(new_n104_), .b(new_n96_), .c(new_n105_), .O(z09));
  nand4  g63(.a(new_n95_), .b(new_n59_), .c(x22), .d(x21), .O(new_n107_));
  inv1   g64(.a(x14), .O(new_n108_));
  nand4  g65(.a(new_n45_), .b(x20), .c(x15), .d(new_n108_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  inv1   g67(.a(x16), .O(new_n111_));
  nor2   g68(.a(x22), .b(new_n111_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n110_), .c(new_n100_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n107_), .c(x00), .O(z10));
  nand2  g71(.a(new_n70_), .b(new_n91_), .O(new_n115_));
  nand3  g72(.a(new_n95_), .b(new_n59_), .c(x21), .O(new_n116_));
  nand3  g73(.a(new_n110_), .b(new_n100_), .c(new_n111_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(z11));
  oai21  g75(.a(new_n61_), .b(new_n64_), .c(new_n67_), .O(new_n119_));
  nand3  g76(.a(new_n44_), .b(new_n57_), .c(new_n64_), .O(new_n120_));
  inv1   g77(.a(x23), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(new_n121_), .O(new_n122_));
  nand4  g79(.a(new_n122_), .b(new_n120_), .c(new_n81_), .d(new_n91_), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand3  g81(.a(new_n46_), .b(new_n124_), .c(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n119_), .c(new_n125_), .O(z12));
  nand3  g83(.a(new_n64_), .b(new_n81_), .c(new_n91_), .O(new_n127_));
  nor4   g84(.a(new_n127_), .b(new_n71_), .c(x19), .d(new_n57_), .O(z13));
  nand3  g85(.a(new_n47_), .b(new_n44_), .c(new_n61_), .O(new_n129_));
  oai21  g86(.a(new_n129_), .b(new_n51_), .c(new_n46_), .O(z14));
  oai21  g87(.a(x10), .b(new_n81_), .c(x02), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x00), .O(new_n132_));
  nand3  g89(.a(x19), .b(new_n64_), .c(new_n81_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(new_n132_), .c(new_n71_), .O(z15));
  nor3   g91(.a(new_n71_), .b(new_n81_), .c(x00), .O(z16));
  nand2  g92(.a(new_n92_), .b(new_n46_), .O(z17));
endmodule


