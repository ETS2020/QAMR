// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x24), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x10), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  inv1   g08(.a(new_n51_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x02), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand2  g12(.a(new_n53_), .b(x09), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n49_), .c(new_n45_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n56_), .c(new_n45_), .O(z02));
  inv1   g17(.a(x01), .O(new_n61_));
  nand2  g18(.a(x02), .b(x00), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x09), .O(new_n63_));
  nand3  g20(.a(x12), .b(x11), .c(x10), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  aoi21  g22(.a(new_n65_), .b(new_n63_), .c(new_n44_), .O(new_n66_));
  nor2   g23(.a(x19), .b(new_n47_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n53_), .c(new_n46_), .O(new_n68_));
  oai21  g25(.a(new_n66_), .b(new_n61_), .c(new_n68_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n63_), .b(new_n71_), .c(new_n70_), .d(x10), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x24), .c(new_n61_), .O(z04));
  nand4  g30(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x24), .c(new_n61_), .O(z05));
  nand2  g32(.a(x11), .b(x10), .O(new_n76_));
  nand2  g33(.a(x24), .b(x01), .O(new_n77_));
  nor4   g34(.a(new_n77_), .b(new_n76_), .c(new_n62_), .d(x09), .O(z06));
  nor2   g35(.a(x12), .b(x09), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x24), .c(x11), .d(x10), .O(new_n80_));
  aoi21  g37(.a(new_n80_), .b(x01), .c(new_n62_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nand2  g42(.a(new_n51_), .b(x02), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(z17));
  nand4  g44(.a(z17), .b(x19), .c(new_n47_), .d(x17), .O(new_n88_));
  oai21  g45(.a(new_n88_), .b(new_n85_), .c(new_n45_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n67_), .b(x24), .c(new_n90_), .d(x01), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n61_), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n70_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nor2   g53(.a(x15), .b(x14), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(x20), .d(new_n92_), .O(new_n98_));
  inv1   g55(.a(x00), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  aoi21  g59(.a(new_n98_), .b(new_n91_), .c(new_n102_), .O(z09));
  inv1   g60(.a(x14), .O(new_n104_));
  nand4  g61(.a(new_n100_), .b(x20), .c(x15), .d(new_n104_), .O(new_n105_));
  nand2  g62(.a(new_n101_), .b(x16), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g64(.a(new_n48_), .b(x18), .c(x01), .O(new_n108_));
  nand3  g65(.a(x22), .b(x21), .c(new_n90_), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g67(.a(new_n107_), .b(new_n96_), .c(new_n110_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(x00), .c(new_n45_), .O(z10));
  nand2  g69(.a(new_n101_), .b(new_n99_), .O(new_n113_));
  inv1   g70(.a(new_n105_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n96_), .c(new_n92_), .O(new_n115_));
  inv1   g72(.a(new_n108_), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(x24), .c(x21), .d(new_n90_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(z11));
  inv1   g75(.a(x23), .O(new_n119_));
  nor2   g76(.a(x19), .b(x02), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x17), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n119_), .c(new_n121_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(x09), .c(new_n99_), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n61_), .c(x24), .O(z12));
  nor2   g81(.a(new_n121_), .b(new_n52_), .O(z13));
  nor2   g82(.a(new_n59_), .b(new_n54_), .O(z14));
  oai21  g83(.a(x10), .b(new_n61_), .c(x02), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x00), .O(new_n128_));
  inv1   g85(.a(x02), .O(new_n129_));
  nand3  g86(.a(x19), .b(new_n129_), .c(new_n61_), .O(new_n130_));
  nand3  g87(.a(new_n130_), .b(new_n128_), .c(new_n45_), .O(z15));
  aoi21  g88(.a(x24), .b(x00), .c(new_n61_), .O(z16));
endmodule


