// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n127_, new_n128_, new_n129_, new_n130_;
  nand2  g00(.a(x15), .b(x01), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nand2  g02(.a(x10), .b(new_n45_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  nor2   g05(.a(x19), .b(x02), .O(new_n49_));
  nor2   g06(.a(x01), .b(x00), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  oai21  g08(.a(new_n51_), .b(new_n46_), .c(new_n44_), .O(z00));
  inv1   g09(.a(x10), .O(new_n53_));
  nor3   g10(.a(new_n51_), .b(new_n53_), .c(new_n45_), .O(z01));
  nand2  g11(.a(new_n53_), .b(x09), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n51_), .c(new_n44_), .O(z02));
  inv1   g13(.a(x01), .O(new_n57_));
  nand2  g14(.a(x02), .b(x00), .O(new_n58_));
  nand3  g15(.a(x11), .b(x10), .c(new_n45_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(x12), .c(x15), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nor2   g19(.a(x17), .b(x02), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n50_), .c(new_n62_), .d(x18), .O(new_n64_));
  oai21  g21(.a(new_n61_), .b(new_n57_), .c(new_n64_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  inv1   g24(.a(x15), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(x10), .O(new_n69_));
  nor4   g26(.a(new_n69_), .b(new_n58_), .c(x09), .d(new_n57_), .O(z04));
  inv1   g27(.a(new_n58_), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(x10), .c(x09), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(new_n68_), .c(new_n57_), .O(z05));
  nor2   g30(.a(new_n60_), .b(x15), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n57_), .O(z06));
  oai21  g32(.a(new_n59_), .b(x12), .c(x01), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n71_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n44_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nand2  g39(.a(new_n50_), .b(x02), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(z17));
  nand4  g41(.a(z17), .b(x19), .c(new_n48_), .d(x17), .O(new_n85_));
  oai21  g42(.a(new_n85_), .b(new_n82_), .c(new_n44_), .O(z08));
  inv1   g43(.a(x20), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n62_), .c(x18), .d(x01), .O(new_n88_));
  nand2  g45(.a(x02), .b(new_n57_), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(x12), .c(new_n66_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  nand3  g51(.a(x20), .b(new_n94_), .c(new_n93_), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n92_), .O(new_n97_));
  inv1   g54(.a(x00), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n99_), .c(new_n68_), .d(new_n98_), .O(new_n101_));
  aoi21  g58(.a(new_n97_), .b(new_n88_), .c(new_n101_), .O(z09));
  inv1   g59(.a(new_n88_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(x22), .c(x21), .d(new_n68_), .O(new_n104_));
  nor2   g61(.a(x21), .b(new_n68_), .O(new_n105_));
  nand4  g62(.a(new_n100_), .b(x20), .c(x16), .d(new_n93_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(new_n105_), .c(new_n92_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n104_), .c(x00), .O(z10));
  nand2  g66(.a(new_n100_), .b(new_n98_), .O(new_n110_));
  nand3  g67(.a(new_n103_), .b(x21), .c(new_n68_), .O(new_n111_));
  nand3  g68(.a(new_n105_), .b(new_n96_), .c(new_n92_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(z11));
  nand2  g70(.a(x10), .b(x02), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(x01), .c(x00), .O(new_n115_));
  nand2  g72(.a(new_n49_), .b(new_n47_), .O(new_n116_));
  inv1   g73(.a(x23), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(new_n117_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n50_), .c(new_n116_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  nor2   g77(.a(x24), .b(new_n45_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n44_), .O(z12));
  nand2  g80(.a(new_n50_), .b(x17), .O(new_n124_));
  nor3   g81(.a(new_n124_), .b(x19), .c(x02), .O(z13));
  nor3   g82(.a(new_n51_), .b(x10), .c(x09), .O(z14));
  oai21  g83(.a(x10), .b(new_n57_), .c(x02), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x00), .O(new_n128_));
  inv1   g85(.a(x02), .O(new_n129_));
  nand3  g86(.a(x19), .b(new_n129_), .c(new_n57_), .O(new_n130_));
  nand3  g87(.a(new_n130_), .b(new_n128_), .c(new_n44_), .O(z15));
  aoi21  g88(.a(new_n68_), .b(x00), .c(new_n57_), .O(z16));
endmodule


