// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n119_, new_n121_, new_n122_, new_n123_;
  nor2   g00(.a(x16), .b(x14), .O(new_n44_));
  inv1   g01(.a(new_n44_), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor2   g05(.a(x17), .b(x02), .O(new_n49_));
  inv1   g06(.a(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n45_), .O(z00));
  inv1   g10(.a(x17), .O(new_n54_));
  nand3  g11(.a(new_n47_), .b(new_n45_), .c(new_n54_), .O(new_n55_));
  inv1   g12(.a(x02), .O(new_n56_));
  nand4  g13(.a(new_n46_), .b(x10), .c(x09), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n55_), .O(z01));
  nand3  g15(.a(new_n49_), .b(new_n50_), .c(x09), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n48_), .c(new_n45_), .O(z02));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  nor3   g18(.a(new_n61_), .b(new_n50_), .c(x09), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x12), .c(x11), .O(new_n63_));
  inv1   g20(.a(x18), .O(new_n64_));
  nor2   g21(.a(x19), .b(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n49_), .c(new_n46_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n63_), .c(new_n45_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand3  g26(.a(new_n62_), .b(new_n69_), .c(new_n68_), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n45_), .O(z04));
  inv1   g28(.a(x09), .O(new_n72_));
  nor4   g29(.a(new_n61_), .b(new_n44_), .c(new_n50_), .d(new_n72_), .O(z05));
  nand2  g30(.a(new_n62_), .b(x11), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n45_), .O(z06));
  nand3  g32(.a(new_n51_), .b(new_n69_), .c(x11), .O(new_n76_));
  nand3  g33(.a(new_n45_), .b(x02), .c(x00), .O(new_n77_));
  aoi21  g34(.a(new_n76_), .b(x01), .c(new_n77_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  inv1   g39(.a(x00), .O(new_n83_));
  inv1   g40(.a(x01), .O(new_n84_));
  nand3  g41(.a(x02), .b(new_n84_), .c(new_n83_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x19), .c(new_n64_), .d(x17), .O(new_n87_));
  oai21  g44(.a(new_n87_), .b(new_n82_), .c(new_n45_), .O(z08));
  nand3  g45(.a(new_n65_), .b(x01), .c(new_n83_), .O(new_n89_));
  inv1   g46(.a(x20), .O(new_n90_));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  oai21  g50(.a(new_n93_), .b(new_n89_), .c(new_n45_), .O(z09));
  nor2   g51(.a(x14), .b(x13), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x20), .c(x15), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nand2  g54(.a(new_n92_), .b(new_n91_), .O(new_n98_));
  nand4  g55(.a(x12), .b(new_n68_), .c(x02), .d(new_n84_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g57(.a(x21), .b(new_n90_), .O(new_n101_));
  inv1   g58(.a(x19), .O(new_n102_));
  nand4  g59(.a(x22), .b(new_n102_), .c(x18), .d(x01), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g61(.a(new_n100_), .b(new_n97_), .c(new_n104_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(x00), .c(new_n45_), .O(z10));
  nand2  g63(.a(new_n45_), .b(new_n92_), .O(new_n107_));
  nor3   g64(.a(new_n107_), .b(new_n101_), .c(new_n89_), .O(z11));
  nand2  g65(.a(x01), .b(x00), .O(new_n109_));
  aoi21  g66(.a(x10), .b(x02), .c(new_n109_), .O(new_n110_));
  inv1   g67(.a(new_n46_), .O(new_n111_));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  nand3  g69(.a(new_n102_), .b(x17), .c(new_n56_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  nor2   g71(.a(x24), .b(new_n72_), .O(new_n115_));
  oai21  g72(.a(new_n114_), .b(new_n110_), .c(new_n115_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n45_), .O(z12));
  oai21  g74(.a(new_n113_), .b(new_n111_), .c(new_n45_), .O(z13));
  nand4  g75(.a(new_n46_), .b(new_n50_), .c(new_n72_), .d(new_n56_), .O(new_n119_));
  nor2   g76(.a(new_n119_), .b(new_n55_), .O(z14));
  aoi21  g77(.a(new_n50_), .b(x01), .c(new_n56_), .O(new_n121_));
  nor2   g78(.a(x02), .b(x01), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(x19), .c(new_n44_), .O(new_n123_));
  oai21  g80(.a(new_n121_), .b(new_n83_), .c(new_n123_), .O(z15));
  oai21  g81(.a(new_n84_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g82(.a(new_n85_), .b(new_n45_), .O(z17));
endmodule


