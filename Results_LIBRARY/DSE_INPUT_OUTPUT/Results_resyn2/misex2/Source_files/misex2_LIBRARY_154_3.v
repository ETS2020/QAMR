// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n116_;
  nand2  g00(.a(x01), .b(x00), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(x10), .c(new_n50_), .d(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n46_), .b(new_n45_), .c(new_n51_), .O(new_n54_));
  nand2  g11(.a(new_n47_), .b(new_n49_), .O(new_n55_));
  nand2  g12(.a(x10), .b(x09), .O(new_n56_));
  nor3   g13(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n51_), .b(new_n58_), .c(x09), .d(new_n49_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n48_), .c(new_n44_), .O(z02));
  nand3  g17(.a(new_n46_), .b(x18), .c(new_n51_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n55_), .O(z03));
  inv1   g19(.a(new_n44_), .O(z04));
  inv1   g20(.a(x01), .O(new_n66_));
  nand3  g21(.a(x02), .b(new_n66_), .c(x00), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z07));
  inv1   g23(.a(x03), .O(new_n69_));
  nor2   g24(.a(x06), .b(x05), .O(new_n70_));
  nor2   g25(.a(x08), .b(x07), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(x04), .d(new_n69_), .O(new_n72_));
  inv1   g27(.a(x00), .O(new_n73_));
  nand3  g28(.a(x02), .b(new_n66_), .c(new_n73_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(x19), .c(new_n45_), .d(x17), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(new_n72_), .c(new_n44_), .O(z08));
  inv1   g32(.a(x21), .O(new_n78_));
  inv1   g33(.a(x22), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  inv1   g35(.a(x20), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n46_), .c(x18), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n80_), .c(new_n73_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x01), .O(new_n84_));
  inv1   g39(.a(x16), .O(new_n85_));
  inv1   g40(.a(x11), .O(new_n86_));
  inv1   g41(.a(x13), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(x12), .c(new_n86_), .O(new_n88_));
  nor2   g43(.a(new_n88_), .b(new_n74_), .O(new_n89_));
  nand3  g44(.a(new_n79_), .b(new_n78_), .c(x20), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nor2   g46(.a(x15), .b(x14), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n91_), .c(new_n89_), .d(new_n85_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n84_), .O(z09));
  nand4  g49(.a(x21), .b(new_n81_), .c(new_n46_), .d(x18), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n79_), .c(new_n73_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(x01), .O(new_n97_));
  inv1   g52(.a(x15), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x14), .O(new_n99_));
  nand4  g54(.a(new_n99_), .b(new_n91_), .c(new_n89_), .d(x16), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n97_), .O(z10));
  oai21  g56(.a(new_n95_), .b(x22), .c(new_n73_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x01), .O(new_n103_));
  nand4  g58(.a(new_n99_), .b(new_n91_), .c(new_n89_), .d(new_n85_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n103_), .O(z11));
  oai21  g60(.a(x19), .b(x02), .c(x23), .O(new_n106_));
  nand3  g61(.a(new_n46_), .b(x17), .c(new_n49_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nor2   g63(.a(x24), .b(new_n50_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n44_), .O(z12));
  inv1   g66(.a(new_n47_), .O(new_n112_));
  oai21  g67(.a(new_n107_), .b(new_n112_), .c(new_n44_), .O(z13));
  nand2  g68(.a(new_n58_), .b(new_n50_), .O(new_n114_));
  nor3   g69(.a(new_n114_), .b(new_n55_), .c(new_n54_), .O(z14));
  nand2  g70(.a(x19), .b(new_n66_), .O(new_n116_));
  aoi22  g71(.a(new_n116_), .b(new_n73_), .c(x02), .d(new_n66_), .O(z15));
  nor2   g72(.a(new_n66_), .b(x00), .O(z16));
  nand2  g73(.a(new_n74_), .b(new_n44_), .O(z17));
  zero   g74(.O(z05));
  zero   g75(.O(z06));
endmodule


