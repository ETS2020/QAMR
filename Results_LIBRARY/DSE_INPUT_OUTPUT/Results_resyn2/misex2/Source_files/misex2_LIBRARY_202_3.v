// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n129_, new_n130_, new_n131_,
    new_n133_;
  nand2  g00(.a(x24), .b(x15), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  inv1   g15(.a(new_n44_), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n50_), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(new_n61_), .c(x12), .O(new_n64_));
  nor2   g21(.a(x17), .b(x02), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n51_), .c(new_n47_), .d(x18), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(new_n64_), .c(new_n59_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n50_), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n60_), .c(new_n44_), .O(z04));
  nand2  g28(.a(x01), .b(x00), .O(new_n72_));
  nand2  g29(.a(x10), .b(x09), .O(new_n73_));
  nor4   g30(.a(new_n73_), .b(new_n72_), .c(new_n59_), .d(new_n49_), .O(z05));
  oai21  g31(.a(new_n62_), .b(new_n60_), .c(new_n44_), .O(z06));
  nand2  g32(.a(new_n63_), .b(new_n69_), .O(new_n76_));
  nand3  g33(.a(new_n44_), .b(x02), .c(x00), .O(new_n77_));
  aoi21  g34(.a(new_n76_), .b(x01), .c(new_n77_), .O(z07));
  nand3  g35(.a(new_n51_), .b(new_n44_), .c(x02), .O(new_n79_));
  nand3  g36(.a(x19), .b(new_n46_), .c(x17), .O(new_n80_));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(z08));
  inv1   g42(.a(x01), .O(new_n86_));
  nand2  g43(.a(x02), .b(new_n86_), .O(new_n87_));
  inv1   g44(.a(x13), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(x12), .c(new_n68_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  inv1   g47(.a(x14), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nand4  g50(.a(x20), .b(new_n93_), .c(new_n92_), .d(new_n91_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  inv1   g52(.a(x20), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n47_), .c(x18), .d(x01), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  aoi21  g55(.a(new_n95_), .b(new_n90_), .c(new_n98_), .O(new_n99_));
  inv1   g56(.a(x00), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  oai21  g60(.a(new_n103_), .b(new_n99_), .c(new_n44_), .O(z09));
  nand2  g61(.a(x22), .b(x21), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(new_n97_), .O(new_n106_));
  nand2  g63(.a(x15), .b(new_n91_), .O(new_n107_));
  nand4  g64(.a(new_n102_), .b(new_n101_), .c(x20), .d(x16), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n90_), .c(new_n106_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g68(.a(new_n102_), .b(new_n100_), .O(new_n112_));
  nand3  g69(.a(new_n98_), .b(new_n44_), .c(x21), .O(new_n113_));
  inv1   g70(.a(x24), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n101_), .c(x20), .d(new_n93_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n107_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n90_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n113_), .c(new_n112_), .O(z11));
  aoi21  g75(.a(x10), .b(x02), .c(new_n72_), .O(new_n119_));
  inv1   g76(.a(new_n51_), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(x23), .O(new_n121_));
  nand3  g78(.a(new_n47_), .b(x17), .c(new_n49_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n121_), .c(new_n120_), .O(new_n123_));
  nor2   g80(.a(x24), .b(new_n50_), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n119_), .c(new_n124_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n44_), .O(z12));
  oai21  g83(.a(new_n122_), .b(new_n120_), .c(new_n44_), .O(z13));
  nor3   g84(.a(new_n57_), .b(new_n52_), .c(new_n59_), .O(z14));
  oai21  g85(.a(x10), .b(new_n86_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n49_), .c(new_n86_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n130_), .c(new_n59_), .O(z15));
  nand3  g89(.a(new_n44_), .b(x01), .c(new_n100_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z16));
  inv1   g91(.a(new_n79_), .O(z17));
endmodule


