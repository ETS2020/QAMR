// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n138_, new_n139_, new_n142_;
  inv1   g00(.a(x23), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(x10), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nor2   g06(.a(x02), .b(x00), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z00));
  nand3  g09(.a(new_n50_), .b(new_n49_), .c(x09), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x01), .O(z01));
  nor2   g11(.a(x17), .b(x10), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n50_), .c(new_n47_), .d(x09), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g14(.a(x00), .O(new_n58_));
  inv1   g15(.a(x01), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(x02), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n45_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(z06));
  nand2  g20(.a(z06), .b(x12), .O(new_n64_));
  nand2  g21(.a(new_n50_), .b(new_n59_), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nor2   g23(.a(x23), .b(x19), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x18), .d(new_n46_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n64_), .O(z03));
  nand2  g26(.a(x23), .b(new_n59_), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(x10), .d(new_n45_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n61_), .c(new_n70_), .O(z04));
  nand2  g31(.a(x10), .b(x02), .O(new_n75_));
  nand2  g32(.a(new_n60_), .b(x09), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(new_n75_), .O(z05));
  inv1   g34(.a(new_n62_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n72_), .O(new_n79_));
  nand3  g36(.a(new_n70_), .b(x02), .c(x00), .O(new_n80_));
  aoi21  g37(.a(new_n79_), .b(x01), .c(new_n80_), .O(z07));
  inv1   g38(.a(x18), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(x04), .b(new_n86_), .c(x02), .d(new_n58_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x19), .c(new_n82_), .d(x17), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g47(.a(x19), .O(new_n91_));
  inv1   g48(.a(x20), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(x18), .d(x01), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  nand2  g51(.a(x02), .b(new_n59_), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n71_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  inv1   g55(.a(x14), .O(new_n99_));
  inv1   g56(.a(x15), .O(new_n100_));
  inv1   g57(.a(x16), .O(new_n101_));
  nand4  g58(.a(x20), .b(new_n101_), .c(new_n100_), .d(new_n99_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(new_n98_), .c(new_n94_), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  inv1   g62(.a(x22), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n58_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n104_), .c(new_n70_), .O(z09));
  inv1   g65(.a(x02), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(x01), .O(new_n110_));
  nor2   g67(.a(new_n72_), .b(x11), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n110_), .c(new_n96_), .O(new_n112_));
  and2   g69(.a(x18), .b(x01), .O(new_n113_));
  nor2   g70(.a(x20), .b(x19), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n113_), .c(x22), .d(x21), .O(new_n115_));
  nor2   g72(.a(new_n100_), .b(x14), .O(new_n116_));
  nor2   g73(.a(x21), .b(new_n92_), .O(new_n117_));
  nor2   g74(.a(x22), .b(new_n101_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n112_), .c(new_n115_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n58_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n70_), .O(z10));
  nand2  g79(.a(new_n106_), .b(new_n58_), .O(new_n123_));
  nor2   g80(.a(new_n93_), .b(new_n105_), .O(new_n124_));
  nand4  g81(.a(new_n105_), .b(x20), .c(x15), .d(new_n99_), .O(new_n125_));
  nor2   g82(.a(new_n125_), .b(x16), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n98_), .c(new_n124_), .O(new_n127_));
  oai21  g84(.a(new_n127_), .b(new_n123_), .c(new_n70_), .O(z11));
  nand2  g85(.a(new_n75_), .b(new_n60_), .O(new_n129_));
  nand2  g86(.a(new_n67_), .b(x17), .O(new_n130_));
  nor2   g87(.a(new_n130_), .b(new_n65_), .O(z13));
  inv1   g88(.a(z13), .O(new_n132_));
  inv1   g89(.a(x24), .O(new_n133_));
  nand2  g90(.a(new_n133_), .b(x09), .O(new_n134_));
  aoi21  g91(.a(new_n132_), .b(new_n129_), .c(new_n134_), .O(z12));
  nand4  g92(.a(new_n67_), .b(new_n55_), .c(new_n82_), .d(new_n45_), .O(new_n136_));
  nor2   g93(.a(new_n136_), .b(new_n65_), .O(z14));
  aoi21  g94(.a(x19), .b(new_n59_), .c(x00), .O(new_n138_));
  nand2  g95(.a(new_n44_), .b(new_n109_), .O(new_n139_));
  oai21  g96(.a(new_n139_), .b(new_n138_), .c(new_n129_), .O(z15));
  nor2   g97(.a(new_n59_), .b(x00), .O(z16));
  nand3  g98(.a(new_n110_), .b(new_n44_), .c(new_n58_), .O(new_n142_));
  inv1   g99(.a(new_n142_), .O(z17));
endmodule


