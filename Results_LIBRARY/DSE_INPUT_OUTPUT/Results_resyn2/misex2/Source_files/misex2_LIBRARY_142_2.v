// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nor4   g08(.a(new_n51_), .b(new_n47_), .c(new_n44_), .d(x09), .O(z00));
  inv1   g09(.a(x00), .O(new_n53_));
  inv1   g10(.a(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x02), .O(new_n55_));
  nor2   g12(.a(x19), .b(x18), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n55_), .c(new_n48_), .d(x10), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(new_n53_), .c(x01), .O(z01));
  nand4  g15(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n44_), .O(new_n59_));
  nor3   g16(.a(new_n59_), .b(new_n47_), .c(new_n54_), .O(z02));
  inv1   g17(.a(x12), .O(new_n61_));
  nand2  g18(.a(x01), .b(x00), .O(new_n62_));
  nand2  g19(.a(x10), .b(x02), .O(new_n63_));
  nor3   g20(.a(new_n63_), .b(new_n62_), .c(x09), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x11), .O(new_n65_));
  nand3  g22(.a(new_n50_), .b(x18), .c(new_n48_), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n47_), .c(new_n65_), .d(new_n61_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  nand3  g25(.a(new_n64_), .b(new_n61_), .c(new_n68_), .O(new_n69_));
  inv1   g26(.a(new_n69_), .O(z04));
  inv1   g27(.a(new_n63_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(x09), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n53_), .O(z05));
  inv1   g30(.a(new_n65_), .O(z06));
  nor2   g31(.a(new_n65_), .b(x12), .O(z07));
  inv1   g32(.a(x08), .O(new_n76_));
  nand4  g33(.a(x19), .b(new_n49_), .c(x17), .d(new_n76_), .O(new_n77_));
  inv1   g34(.a(x05), .O(new_n78_));
  nor2   g35(.a(x07), .b(x06), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(new_n78_), .c(x04), .O(new_n80_));
  inv1   g37(.a(x03), .O(new_n81_));
  nand3  g38(.a(new_n46_), .b(new_n81_), .c(x02), .O(new_n82_));
  nor3   g39(.a(new_n82_), .b(new_n80_), .c(new_n77_), .O(z08));
  nor2   g40(.a(x20), .b(x19), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x18), .c(x01), .O(new_n85_));
  inv1   g42(.a(x16), .O(new_n86_));
  nand2  g43(.a(new_n68_), .b(x02), .O(new_n87_));
  inv1   g44(.a(x13), .O(new_n88_));
  inv1   g45(.a(x14), .O(new_n89_));
  nand3  g46(.a(new_n89_), .b(new_n88_), .c(x12), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nor2   g48(.a(x15), .b(x01), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(x20), .d(new_n86_), .O(new_n93_));
  inv1   g50(.a(x21), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(new_n53_), .O(new_n96_));
  aoi21  g53(.a(new_n93_), .b(new_n85_), .c(new_n96_), .O(z09));
  nand3  g54(.a(new_n95_), .b(new_n94_), .c(x20), .O(new_n98_));
  nand2  g55(.a(x16), .b(x15), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  aoi21  g57(.a(new_n100_), .b(new_n91_), .c(x00), .O(new_n101_));
  nand4  g58(.a(x21), .b(x18), .c(x01), .d(new_n53_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n84_), .c(x22), .O(new_n104_));
  oai21  g61(.a(new_n101_), .b(x01), .c(new_n104_), .O(z10));
  nand2  g62(.a(new_n86_), .b(x15), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n98_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n91_), .c(x00), .O(new_n108_));
  nand3  g65(.a(new_n103_), .b(new_n84_), .c(new_n95_), .O(new_n109_));
  oai21  g66(.a(new_n108_), .b(x01), .c(new_n109_), .O(z11));
  nor2   g67(.a(new_n71_), .b(new_n53_), .O(new_n111_));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  nand3  g69(.a(new_n50_), .b(x17), .c(new_n45_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n112_), .c(x01), .O(new_n114_));
  nor2   g71(.a(x24), .b(new_n54_), .O(new_n115_));
  oai21  g72(.a(new_n114_), .b(new_n111_), .c(new_n115_), .O(new_n116_));
  inv1   g73(.a(x01), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x00), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n116_), .O(z12));
  nor3   g76(.a(new_n113_), .b(x01), .c(x00), .O(z13));
  nor3   g77(.a(new_n59_), .b(new_n47_), .c(x09), .O(z14));
  oai22  g78(.a(new_n71_), .b(new_n62_), .c(new_n47_), .d(new_n50_), .O(z15));
  nor2   g79(.a(new_n117_), .b(x00), .O(z16));
  aoi21  g80(.a(new_n45_), .b(new_n53_), .c(x01), .O(z17));
endmodule


