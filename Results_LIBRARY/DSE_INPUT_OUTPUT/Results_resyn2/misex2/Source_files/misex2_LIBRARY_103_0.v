// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x18), .O(new_n44_));
  inv1   g01(.a(x10), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nor2   g04(.a(x19), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n46_), .c(new_n44_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(x11), .c(x17), .O(z00));
  nand4  g09(.a(new_n50_), .b(new_n44_), .c(x10), .d(x09), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(x11), .c(x17), .O(z01));
  nand4  g11(.a(new_n50_), .b(new_n44_), .c(new_n45_), .d(x09), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(x11), .c(x17), .O(z02));
  inv1   g13(.a(x17), .O(new_n57_));
  oai21  g14(.a(new_n49_), .b(new_n44_), .c(x11), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g16(.a(x11), .O(new_n60_));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n46_), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n60_), .O(z06));
  nand2  g21(.a(z06), .b(x12), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n59_), .O(z03));
  inv1   g23(.a(x12), .O(new_n67_));
  nand3  g24(.a(x17), .b(new_n67_), .c(new_n60_), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n63_), .O(z04));
  nand2  g26(.a(x10), .b(x09), .O(new_n70_));
  nor2   g27(.a(x17), .b(x11), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(new_n72_));
  oai21  g29(.a(new_n61_), .b(new_n70_), .c(new_n72_), .O(z05));
  nand3  g30(.a(new_n46_), .b(new_n67_), .c(x11), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n72_), .O(z07));
  inv1   g34(.a(x03), .O(new_n78_));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n79_), .c(x04), .d(new_n78_), .O(new_n81_));
  nand2  g38(.a(new_n47_), .b(x02), .O(new_n82_));
  nand3  g39(.a(x19), .b(new_n44_), .c(x17), .O(new_n83_));
  nor3   g40(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(z08));
  inv1   g41(.a(x19), .O(new_n85_));
  inv1   g42(.a(x20), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(x18), .d(x01), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(new_n72_), .O(new_n89_));
  inv1   g46(.a(x01), .O(new_n90_));
  nand2  g47(.a(x02), .b(new_n90_), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n60_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  inv1   g51(.a(x16), .O(new_n95_));
  nand3  g52(.a(x20), .b(x17), .c(new_n95_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nor2   g54(.a(x15), .b(x14), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n94_), .O(new_n99_));
  inv1   g56(.a(x00), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  aoi21  g60(.a(new_n99_), .b(new_n89_), .c(new_n103_), .O(z09));
  inv1   g61(.a(x14), .O(new_n105_));
  nand3  g62(.a(new_n101_), .b(x15), .c(new_n105_), .O(new_n106_));
  nand3  g63(.a(new_n102_), .b(x20), .c(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g65(.a(x22), .b(x21), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n87_), .O(new_n110_));
  aoi21  g67(.a(new_n108_), .b(new_n94_), .c(new_n110_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(x00), .c(new_n72_), .O(z10));
  nand2  g69(.a(new_n102_), .b(new_n100_), .O(new_n113_));
  nand3  g70(.a(new_n88_), .b(new_n72_), .c(x21), .O(new_n114_));
  nor2   g71(.a(new_n106_), .b(new_n96_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n94_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(z11));
  oai21  g74(.a(new_n48_), .b(new_n60_), .c(new_n57_), .O(new_n118_));
  or2    g75(.a(new_n48_), .b(x23), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n118_), .c(new_n47_), .O(new_n120_));
  nand2  g77(.a(x10), .b(x02), .O(new_n121_));
  nand4  g78(.a(new_n121_), .b(new_n72_), .c(x01), .d(x00), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n120_), .c(new_n124_), .O(z12));
  nor2   g82(.a(x17), .b(new_n60_), .O(new_n126_));
  aoi21  g83(.a(new_n49_), .b(x17), .c(new_n126_), .O(z13));
  nor2   g84(.a(x10), .b(x09), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nor3   g86(.a(new_n129_), .b(new_n49_), .c(x18), .O(z14));
  oai21  g87(.a(x10), .b(new_n90_), .c(x02), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x00), .O(new_n132_));
  nor2   g89(.a(x02), .b(x01), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(x19), .c(new_n71_), .O(new_n134_));
  nand2  g91(.a(new_n134_), .b(new_n132_), .O(z15));
  nor3   g92(.a(new_n71_), .b(new_n90_), .c(x00), .O(z16));
  nor2   g93(.a(new_n82_), .b(new_n71_), .O(z17));
endmodule


