// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n128_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x16), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x15), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n45_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n45_), .O(z01));
  nor2   g12(.a(x17), .b(x10), .O(new_n56_));
  nand3  g13(.a(new_n56_), .b(new_n47_), .c(new_n45_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  nand3  g15(.a(x02), .b(x01), .c(x00), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand3  g17(.a(x11), .b(x10), .c(new_n50_), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(new_n60_), .c(x12), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nor2   g21(.a(x02), .b(x01), .O(new_n65_));
  nor2   g22(.a(x17), .b(x00), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(x18), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n63_), .c(new_n45_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(x10), .d(new_n50_), .O(new_n71_));
  oai21  g28(.a(new_n71_), .b(new_n59_), .c(new_n45_), .O(z04));
  nand2  g29(.a(x10), .b(x02), .O(new_n73_));
  inv1   g30(.a(x00), .O(new_n74_));
  inv1   g31(.a(x01), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x09), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n73_), .c(new_n45_), .O(z05));
  oai21  g35(.a(new_n61_), .b(new_n59_), .c(new_n45_), .O(z06));
  nand2  g36(.a(new_n62_), .b(new_n70_), .O(new_n80_));
  nand3  g37(.a(new_n45_), .b(x02), .c(x00), .O(new_n81_));
  aoi21  g38(.a(new_n80_), .b(x01), .c(new_n81_), .O(z07));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  inv1   g41(.a(x04), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n84_), .c(new_n83_), .d(new_n45_), .O(new_n87_));
  nor2   g44(.a(new_n49_), .b(x01), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(new_n74_), .O(new_n89_));
  inv1   g46(.a(x18), .O(new_n90_));
  nand3  g47(.a(x19), .b(new_n90_), .c(x17), .O(new_n91_));
  nor3   g48(.a(new_n91_), .b(new_n89_), .c(new_n87_), .O(z08));
  aoi21  g49(.a(new_n44_), .b(x15), .c(new_n75_), .O(new_n93_));
  inv1   g50(.a(x20), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n64_), .c(x18), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nor2   g54(.a(x14), .b(x13), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(x12), .c(new_n69_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  inv1   g57(.a(x15), .O(new_n101_));
  nand3  g58(.a(x20), .b(new_n44_), .c(new_n101_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n100_), .c(new_n88_), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  inv1   g62(.a(x22), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n74_), .O(new_n107_));
  aoi21  g64(.a(new_n104_), .b(new_n97_), .c(new_n107_), .O(z09));
  nand4  g65(.a(new_n96_), .b(new_n93_), .c(x22), .d(x21), .O(new_n109_));
  nand2  g66(.a(x16), .b(x15), .O(new_n110_));
  nand3  g67(.a(new_n106_), .b(new_n105_), .c(x20), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n100_), .c(new_n88_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n109_), .c(x00), .O(z10));
  nand4  g71(.a(new_n106_), .b(x21), .c(x01), .d(new_n74_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n95_), .c(new_n45_), .O(z11));
  nand2  g73(.a(new_n73_), .b(new_n76_), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n64_), .b(x17), .c(new_n49_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n51_), .O(new_n121_));
  inv1   g78(.a(x24), .O(new_n122_));
  nand3  g79(.a(new_n45_), .b(new_n122_), .c(x09), .O(new_n123_));
  aoi21  g80(.a(new_n121_), .b(new_n117_), .c(new_n123_), .O(z12));
  inv1   g81(.a(new_n51_), .O(new_n125_));
  oai21  g82(.a(new_n119_), .b(new_n125_), .c(new_n45_), .O(z13));
  nor2   g83(.a(new_n57_), .b(new_n52_), .O(z14));
  inv1   g84(.a(new_n45_), .O(new_n128_));
  oai21  g85(.a(x10), .b(new_n75_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand2  g87(.a(new_n65_), .b(x19), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n130_), .c(new_n128_), .O(z15));
  and2   g89(.a(new_n93_), .b(new_n74_), .O(z16));
  nand2  g90(.a(new_n89_), .b(new_n45_), .O(z17));
endmodule


