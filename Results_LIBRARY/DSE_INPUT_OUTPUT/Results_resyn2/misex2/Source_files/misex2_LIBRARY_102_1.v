// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n120_, new_n122_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x00), .O(z00));
  nor2   g06(.a(new_n46_), .b(x02), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n47_), .c(x10), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x00), .O(z01));
  inv1   g09(.a(x10), .O(new_n53_));
  nand3  g10(.a(new_n50_), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  aoi21  g11(.a(new_n54_), .b(new_n44_), .c(x00), .O(z02));
  nor2   g12(.a(new_n53_), .b(new_n45_), .O(new_n56_));
  inv1   g13(.a(x11), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x09), .O(new_n58_));
  inv1   g15(.a(x00), .O(new_n59_));
  nor2   g16(.a(new_n44_), .b(new_n59_), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n58_), .c(new_n56_), .d(x12), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nor2   g19(.a(x01), .b(x00), .O(new_n63_));
  inv1   g20(.a(x18), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(x17), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n63_), .c(new_n62_), .d(new_n45_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n61_), .O(z03));
  nor2   g24(.a(x12), .b(new_n53_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n57_), .c(new_n46_), .d(x02), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x00), .c(new_n44_), .O(z04));
  nand2  g27(.a(new_n56_), .b(x09), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x00), .c(new_n44_), .O(z05));
  inv1   g29(.a(new_n56_), .O(new_n73_));
  nand2  g30(.a(new_n60_), .b(new_n58_), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n73_), .O(z06));
  nand2  g32(.a(new_n68_), .b(new_n58_), .O(new_n76_));
  inv1   g33(.a(new_n63_), .O(new_n77_));
  oai21  g34(.a(x02), .b(new_n59_), .c(new_n77_), .O(new_n78_));
  aoi21  g35(.a(new_n76_), .b(new_n60_), .c(new_n78_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand2  g40(.a(new_n63_), .b(x02), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n64_), .c(x17), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(z08));
  inv1   g43(.a(x12), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(x11), .O(new_n88_));
  inv1   g45(.a(x13), .O(new_n89_));
  inv1   g46(.a(x14), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n89_), .c(x02), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nor2   g53(.a(x16), .b(x15), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(new_n92_), .d(new_n88_), .O(new_n98_));
  aoi21  g55(.a(new_n98_), .b(new_n44_), .c(x00), .O(z09));
  and2   g56(.a(x16), .b(x15), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n96_), .c(new_n92_), .d(new_n88_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n44_), .c(x00), .O(z10));
  inv1   g59(.a(x16), .O(new_n103_));
  nor2   g60(.a(x14), .b(x13), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n88_), .c(new_n103_), .d(x15), .O(new_n105_));
  inv1   g62(.a(new_n84_), .O(z17));
  nand2  g63(.a(new_n96_), .b(z17), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n105_), .O(z11));
  nand2  g65(.a(new_n60_), .b(new_n73_), .O(new_n109_));
  oai21  g66(.a(x19), .b(x02), .c(x23), .O(new_n110_));
  nand3  g67(.a(new_n62_), .b(x17), .c(new_n45_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n63_), .O(new_n113_));
  inv1   g70(.a(x24), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(x09), .O(new_n115_));
  aoi21  g72(.a(new_n113_), .b(new_n109_), .c(new_n115_), .O(z12));
  nor2   g73(.a(new_n111_), .b(new_n77_), .O(z13));
  nand2  g74(.a(new_n47_), .b(new_n53_), .O(new_n118_));
  nand2  g75(.a(new_n45_), .b(new_n44_), .O(new_n119_));
  nand2  g76(.a(new_n46_), .b(new_n59_), .O(new_n120_));
  nor3   g77(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(z14));
  aoi21  g78(.a(new_n53_), .b(x01), .c(new_n45_), .O(new_n122_));
  oai22  g79(.a(new_n122_), .b(new_n59_), .c(new_n119_), .d(new_n62_), .O(z15));
  zero   g80(.O(z16));
endmodule


