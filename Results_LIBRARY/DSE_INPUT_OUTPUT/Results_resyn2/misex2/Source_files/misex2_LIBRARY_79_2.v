// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n127_, new_n128_, new_n130_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x01), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n49_), .O(z00));
  inv1   g11(.a(x15), .O(new_n55_));
  nand2  g12(.a(new_n55_), .b(x01), .O(new_n56_));
  nand2  g13(.a(new_n48_), .b(x09), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n53_), .c(new_n56_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  nand2  g18(.a(x01), .b(x00), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(new_n44_), .c(x02), .O(new_n64_));
  nand4  g21(.a(x15), .b(x12), .c(x11), .d(x10), .O(new_n65_));
  nor2   g22(.a(x19), .b(new_n51_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n48_), .c(new_n50_), .O(new_n67_));
  oai21  g24(.a(new_n65_), .b(new_n64_), .c(new_n67_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(x15), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n64_), .O(z04));
  inv1   g29(.a(x02), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n45_), .O(new_n74_));
  nand3  g31(.a(new_n74_), .b(x10), .c(x09), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x15), .c(new_n46_), .O(z05));
  nand4  g33(.a(new_n74_), .b(x11), .c(x10), .d(new_n44_), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x15), .c(new_n46_), .O(z06));
  nand3  g35(.a(x11), .b(x10), .c(new_n44_), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(x12), .c(x01), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n56_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nand3  g43(.a(x02), .b(new_n46_), .c(new_n45_), .O(new_n87_));
  nand3  g44(.a(x19), .b(new_n51_), .c(x17), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n52_), .c(x18), .d(x01), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nand4  g49(.a(x20), .b(new_n92_), .c(new_n55_), .d(x02), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nor2   g51(.a(x14), .b(x11), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(x12), .O(new_n96_));
  oai21  g53(.a(new_n96_), .b(new_n93_), .c(new_n91_), .O(new_n97_));
  nor3   g54(.a(x22), .b(x21), .c(x00), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n56_), .O(z09));
  nor2   g57(.a(new_n55_), .b(x00), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  inv1   g59(.a(new_n91_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(x22), .c(x21), .O(new_n104_));
  nand4  g61(.a(new_n94_), .b(x12), .c(x02), .d(new_n46_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  inv1   g63(.a(x21), .O(new_n107_));
  inv1   g64(.a(x22), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n107_), .c(x20), .d(x16), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n106_), .c(new_n95_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n104_), .c(new_n102_), .O(z10));
  nand2  g69(.a(new_n103_), .b(x21), .O(new_n113_));
  nor2   g70(.a(new_n90_), .b(x16), .O(new_n114_));
  nand4  g71(.a(new_n106_), .b(new_n95_), .c(new_n114_), .d(new_n107_), .O(new_n115_));
  nand2  g72(.a(new_n101_), .b(new_n108_), .O(new_n116_));
  aoi21  g73(.a(new_n115_), .b(new_n113_), .c(new_n116_), .O(z11));
  aoi21  g74(.a(x10), .b(x02), .c(new_n62_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n52_), .b(x17), .c(new_n73_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(new_n47_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n44_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n56_), .O(z12));
  oai21  g81(.a(new_n120_), .b(new_n47_), .c(new_n56_), .O(z13));
  nor2   g82(.a(new_n60_), .b(new_n49_), .O(z14));
  aoi22  g83(.a(new_n56_), .b(x00), .c(x19), .d(new_n46_), .O(new_n127_));
  nand3  g84(.a(new_n63_), .b(x15), .c(new_n59_), .O(new_n128_));
  oai21  g85(.a(new_n127_), .b(x02), .c(new_n128_), .O(z15));
  nand2  g86(.a(new_n101_), .b(x01), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z16));
  nand2  g88(.a(new_n87_), .b(new_n56_), .O(z17));
endmodule


