// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n124_, new_n125_, new_n127_, new_n128_;
  inv1   g00(.a(x00), .O(new_n44_));
  nor2   g01(.a(x09), .b(x02), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x10), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g09(.a(x02), .O(new_n53_));
  nor2   g10(.a(x01), .b(x00), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n49_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  inv1   g17(.a(x09), .O(new_n61_));
  nand4  g18(.a(x11), .b(x10), .c(new_n61_), .d(x02), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  aoi21  g20(.a(new_n63_), .b(x12), .c(new_n60_), .O(new_n64_));
  nor2   g21(.a(x02), .b(x01), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n48_), .c(x18), .d(new_n46_), .O(new_n66_));
  oai21  g23(.a(new_n64_), .b(new_n44_), .c(new_n66_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nor2   g26(.a(new_n57_), .b(new_n53_), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n61_), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x01), .c(new_n44_), .O(z04));
  nand2  g29(.a(new_n70_), .b(x09), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x01), .c(new_n44_), .O(z05));
  nor3   g31(.a(new_n62_), .b(new_n60_), .c(new_n44_), .O(z06));
  nand2  g32(.a(z06), .b(new_n69_), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z07));
  inv1   g34(.a(x08), .O(new_n78_));
  nand4  g35(.a(x19), .b(new_n47_), .c(x17), .d(new_n78_), .O(new_n79_));
  inv1   g36(.a(x05), .O(new_n80_));
  nor2   g37(.a(x07), .b(x06), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(new_n80_), .c(x04), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nand3  g40(.a(new_n54_), .b(new_n83_), .c(x02), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n82_), .c(new_n79_), .O(z08));
  inv1   g42(.a(x13), .O(new_n86_));
  inv1   g43(.a(x14), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(new_n68_), .d(x02), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(new_n89_));
  nor2   g46(.a(new_n69_), .b(x01), .O(new_n90_));
  nor2   g47(.a(x16), .b(x15), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(x20), .O(new_n92_));
  inv1   g49(.a(x20), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n48_), .c(x18), .d(x01), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n44_), .O(new_n97_));
  aoi21  g54(.a(new_n94_), .b(new_n92_), .c(new_n97_), .O(z09));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n99_));
  nand2  g56(.a(x16), .b(x15), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n90_), .c(new_n89_), .O(new_n102_));
  nor2   g59(.a(new_n94_), .b(new_n95_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(x22), .O(new_n104_));
  aoi21  g61(.a(new_n104_), .b(new_n102_), .c(x00), .O(z10));
  inv1   g62(.a(new_n99_), .O(new_n106_));
  inv1   g63(.a(x16), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(x15), .c(x12), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n88_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n106_), .c(x00), .O(new_n110_));
  nand3  g67(.a(new_n103_), .b(new_n96_), .c(new_n44_), .O(new_n111_));
  oai21  g68(.a(new_n110_), .b(x01), .c(new_n111_), .O(z11));
  inv1   g69(.a(x23), .O(new_n113_));
  nor2   g70(.a(x19), .b(x02), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(x17), .O(new_n115_));
  oai21  g72(.a(new_n114_), .b(new_n113_), .c(new_n115_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n54_), .O(new_n117_));
  inv1   g74(.a(new_n70_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(x01), .c(x00), .O(new_n119_));
  inv1   g76(.a(x24), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x09), .O(new_n121_));
  aoi21  g78(.a(new_n119_), .b(new_n117_), .c(new_n121_), .O(z12));
  aoi21  g79(.a(new_n115_), .b(new_n44_), .c(x01), .O(z13));
  inv1   g80(.a(new_n58_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n45_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g83(.a(x19), .b(new_n53_), .c(x00), .O(new_n127_));
  nand2  g84(.a(new_n118_), .b(x00), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(x01), .c(new_n127_), .O(z15));
  nor2   g86(.a(new_n60_), .b(x00), .O(z16));
  aoi21  g87(.a(new_n53_), .b(new_n44_), .c(x01), .O(z17));
endmodule


