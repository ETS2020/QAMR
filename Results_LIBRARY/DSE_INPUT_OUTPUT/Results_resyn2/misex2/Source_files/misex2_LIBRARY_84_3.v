// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nand2  g02(.a(x15), .b(x13), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x00), .O(new_n49_));
  inv1   g06(.a(x01), .O(new_n50_));
  inv1   g07(.a(x02), .O(new_n51_));
  inv1   g08(.a(x09), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  nor3   g10(.a(new_n53_), .b(new_n48_), .c(new_n44_), .O(z00));
  nor2   g11(.a(x01), .b(x00), .O(new_n55_));
  nand3  g12(.a(new_n55_), .b(x09), .c(new_n51_), .O(new_n56_));
  nand3  g13(.a(new_n47_), .b(new_n45_), .c(x10), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n56_), .c(new_n46_), .O(z01));
  nand3  g15(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n56_), .c(new_n46_), .O(z02));
  nor2   g17(.a(x19), .b(x02), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n55_), .c(x18), .d(new_n45_), .O(new_n62_));
  nor2   g19(.a(x09), .b(new_n51_), .O(new_n63_));
  inv1   g20(.a(x11), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n44_), .O(new_n65_));
  nand2  g22(.a(x01), .b(x00), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n65_), .c(new_n63_), .d(x12), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n62_), .c(new_n46_), .O(z03));
  nand3  g26(.a(new_n67_), .b(new_n63_), .c(new_n46_), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(new_n64_), .c(x10), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n70_), .O(z04));
  inv1   g30(.a(new_n46_), .O(new_n74_));
  nand3  g31(.a(x10), .b(x09), .c(x02), .O(new_n75_));
  nor3   g32(.a(new_n75_), .b(new_n66_), .c(new_n74_), .O(z05));
  nor3   g33(.a(new_n70_), .b(new_n64_), .c(new_n44_), .O(z06));
  nand3  g34(.a(new_n65_), .b(new_n71_), .c(new_n52_), .O(new_n78_));
  nand3  g35(.a(new_n46_), .b(x02), .c(x00), .O(new_n79_));
  aoi21  g36(.a(new_n78_), .b(x01), .c(new_n79_), .O(z07));
  nand3  g37(.a(new_n55_), .b(new_n46_), .c(x02), .O(new_n81_));
  inv1   g38(.a(x18), .O(new_n82_));
  nand3  g39(.a(x19), .b(new_n82_), .c(x17), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nor2   g41(.a(x06), .b(x05), .O(new_n85_));
  nor2   g42(.a(x08), .b(x07), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(x04), .d(new_n84_), .O(new_n87_));
  nor3   g44(.a(new_n87_), .b(new_n83_), .c(new_n81_), .O(z08));
  inv1   g45(.a(x19), .O(new_n89_));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n89_), .c(x18), .d(x01), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n50_), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n64_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  inv1   g53(.a(x14), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nand4  g56(.a(x20), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n96_), .c(new_n92_), .O(new_n102_));
  inv1   g59(.a(x21), .O(new_n103_));
  inv1   g60(.a(x22), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n103_), .c(new_n49_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n102_), .c(new_n46_), .O(z09));
  nand4  g63(.a(new_n92_), .b(new_n46_), .c(x22), .d(x21), .O(new_n107_));
  nor2   g64(.a(new_n98_), .b(x14), .O(new_n108_));
  nand4  g65(.a(new_n104_), .b(new_n103_), .c(x20), .d(x16), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n108_), .c(new_n96_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n107_), .c(x00), .O(z10));
  nand2  g69(.a(new_n104_), .b(new_n49_), .O(new_n113_));
  nand3  g70(.a(new_n92_), .b(new_n46_), .c(x21), .O(new_n114_));
  nor2   g71(.a(new_n90_), .b(x16), .O(new_n115_));
  nand4  g72(.a(new_n108_), .b(new_n115_), .c(new_n96_), .d(new_n103_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(z11));
  aoi21  g74(.a(x10), .b(x02), .c(new_n66_), .O(new_n118_));
  inv1   g75(.a(new_n55_), .O(new_n119_));
  oai21  g76(.a(x19), .b(x02), .c(x23), .O(new_n120_));
  nand2  g77(.a(new_n61_), .b(x17), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  nor2   g79(.a(x24), .b(new_n52_), .O(new_n123_));
  oai21  g80(.a(new_n122_), .b(new_n118_), .c(new_n123_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n46_), .O(z12));
  nor3   g82(.a(new_n121_), .b(new_n119_), .c(new_n74_), .O(z13));
  nor3   g83(.a(new_n53_), .b(new_n48_), .c(x10), .O(z14));
  oai21  g84(.a(x10), .b(new_n50_), .c(x02), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x00), .O(new_n129_));
  nand3  g86(.a(x19), .b(new_n51_), .c(new_n50_), .O(new_n130_));
  aoi21  g87(.a(new_n130_), .b(new_n129_), .c(new_n74_), .O(z15));
  oai21  g88(.a(new_n50_), .b(x00), .c(new_n46_), .O(z16));
  inv1   g89(.a(new_n81_), .O(z17));
endmodule


