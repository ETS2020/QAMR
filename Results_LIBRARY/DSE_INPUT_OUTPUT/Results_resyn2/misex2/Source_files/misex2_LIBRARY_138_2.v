// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n128_, new_n129_, new_n130_, new_n132_;
  inv1   g00(.a(x18), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  nand3  g02(.a(new_n45_), .b(new_n44_), .c(x10), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  nor2   g06(.a(x01), .b(x00), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  inv1   g08(.a(new_n51_), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n46_), .O(z00));
  nand2  g11(.a(new_n52_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n46_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand3  g14(.a(new_n45_), .b(new_n44_), .c(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  inv1   g17(.a(x16), .O(new_n61_));
  nand2  g18(.a(x02), .b(x00), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x09), .O(new_n63_));
  nand3  g20(.a(x12), .b(x11), .c(x10), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  aoi21  g22(.a(new_n65_), .b(new_n63_), .c(new_n61_), .O(new_n66_));
  nand2  g23(.a(new_n45_), .b(x18), .O(new_n67_));
  oai22  g24(.a(new_n67_), .b(new_n51_), .c(new_n66_), .d(new_n60_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n63_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x16), .c(new_n60_), .O(z04));
  nand3  g29(.a(x02), .b(x01), .c(x00), .O(new_n73_));
  nor4   g30(.a(new_n73_), .b(new_n61_), .c(new_n57_), .d(new_n47_), .O(z05));
  nand4  g31(.a(x16), .b(x11), .c(x10), .d(new_n47_), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n73_), .O(z06));
  inv1   g33(.a(new_n75_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(x01), .c(new_n62_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand2  g40(.a(new_n50_), .b(x02), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n44_), .c(x17), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(z08));
  nor2   g43(.a(x16), .b(new_n60_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  inv1   g45(.a(x20), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n45_), .c(x18), .d(x01), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(x12), .c(new_n69_), .d(x02), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  nand4  g51(.a(x20), .b(new_n61_), .c(new_n94_), .d(new_n93_), .O(new_n95_));
  oai21  g52(.a(new_n95_), .b(new_n92_), .c(new_n90_), .O(new_n96_));
  inv1   g53(.a(x00), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(x21), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n96_), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n88_), .O(z09));
  nor2   g59(.a(new_n87_), .b(new_n97_), .O(new_n103_));
  nor2   g60(.a(new_n92_), .b(x01), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(x20), .c(x15), .d(new_n93_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nor2   g64(.a(x22), .b(new_n61_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n107_), .c(new_n104_), .O(new_n109_));
  inv1   g66(.a(new_n90_), .O(new_n110_));
  nor2   g67(.a(new_n98_), .b(new_n105_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n110_), .c(new_n87_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n109_), .c(new_n103_), .O(z10));
  nand3  g70(.a(new_n107_), .b(new_n104_), .c(new_n61_), .O(new_n114_));
  nand3  g71(.a(new_n110_), .b(x21), .c(x16), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n114_), .c(new_n99_), .O(z11));
  nand2  g73(.a(x10), .b(x02), .O(new_n117_));
  nand4  g74(.a(new_n117_), .b(x16), .c(x01), .d(x00), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n45_), .b(x17), .c(new_n48_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n50_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n118_), .c(new_n124_), .O(z12));
  nor3   g82(.a(new_n120_), .b(x01), .c(x00), .O(z13));
  oai21  g83(.a(new_n58_), .b(new_n53_), .c(new_n88_), .O(z14));
  nor2   g84(.a(new_n45_), .b(x01), .O(new_n128_));
  oai21  g85(.a(new_n128_), .b(new_n103_), .c(new_n48_), .O(new_n129_));
  nand4  g86(.a(x16), .b(new_n57_), .c(x01), .d(x00), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n129_), .O(z15));
  nand3  g88(.a(x16), .b(x01), .c(new_n97_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z16));
  inv1   g90(.a(new_n84_), .O(z17));
endmodule


