// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n133_, new_n134_, new_n135_, new_n138_;
  nand2  g00(.a(x21), .b(x19), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n52_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n47_), .c(x18), .d(new_n49_), .O(new_n60_));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x02), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nor2   g22(.a(new_n56_), .b(x09), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n65_), .c(x11), .O(new_n67_));
  nand2  g24(.a(new_n44_), .b(x12), .O(new_n68_));
  oai21  g25(.a(new_n68_), .b(new_n67_), .c(new_n60_), .O(z03));
  inv1   g26(.a(x12), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(x10), .c(new_n46_), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  nand2  g29(.a(new_n65_), .b(new_n72_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n71_), .c(new_n44_), .O(z04));
  nand2  g31(.a(x10), .b(x02), .O(new_n75_));
  nand2  g32(.a(new_n63_), .b(x09), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n44_), .O(z05));
  nand2  g34(.a(new_n67_), .b(new_n44_), .O(z06));
  oai21  g35(.a(new_n71_), .b(new_n72_), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n44_), .O(z07));
  inv1   g38(.a(x21), .O(new_n82_));
  nand2  g39(.a(new_n47_), .b(x02), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nor2   g42(.a(x18), .b(new_n49_), .O(new_n86_));
  inv1   g43(.a(x03), .O(new_n87_));
  nor2   g44(.a(x06), .b(x05), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(x04), .c(new_n87_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n86_), .c(new_n85_), .d(new_n84_), .O(new_n91_));
  aoi21  g48(.a(new_n91_), .b(new_n82_), .c(new_n51_), .O(z08));
  nand2  g49(.a(x02), .b(new_n62_), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n72_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  inv1   g53(.a(x14), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nand4  g56(.a(x20), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  inv1   g58(.a(x20), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n51_), .c(x18), .d(x01), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  aoi21  g61(.a(new_n101_), .b(new_n96_), .c(new_n104_), .O(new_n105_));
  inv1   g62(.a(x22), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n82_), .c(new_n61_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n105_), .c(new_n44_), .O(z09));
  nand4  g65(.a(x21), .b(new_n102_), .c(x18), .d(x01), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand4  g67(.a(new_n82_), .b(x20), .c(x15), .d(new_n97_), .O(new_n111_));
  nand2  g68(.a(new_n106_), .b(x16), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n96_), .c(new_n110_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g72(.a(new_n106_), .b(new_n61_), .O(new_n116_));
  inv1   g73(.a(new_n109_), .O(new_n117_));
  nor2   g74(.a(new_n111_), .b(x16), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n96_), .c(new_n117_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n116_), .c(new_n44_), .O(z11));
  nand2  g77(.a(new_n75_), .b(new_n63_), .O(new_n121_));
  nand2  g78(.a(new_n59_), .b(new_n49_), .O(new_n122_));
  inv1   g79(.a(x23), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(new_n123_), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(new_n122_), .c(new_n47_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nor2   g83(.a(x24), .b(new_n46_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n44_), .O(z12));
  nand2  g86(.a(new_n47_), .b(x17), .O(new_n130_));
  nor3   g87(.a(new_n130_), .b(x19), .c(x02), .O(z13));
  oai21  g88(.a(new_n57_), .b(new_n48_), .c(new_n44_), .O(z14));
  oai21  g89(.a(x10), .b(new_n62_), .c(x02), .O(new_n133_));
  oai21  g90(.a(new_n51_), .b(x01), .c(new_n61_), .O(new_n134_));
  nand3  g91(.a(new_n134_), .b(new_n133_), .c(new_n44_), .O(new_n135_));
  inv1   g92(.a(new_n135_), .O(z15));
  oai21  g93(.a(new_n62_), .b(x00), .c(new_n44_), .O(z16));
  nand2  g94(.a(new_n84_), .b(new_n44_), .O(new_n138_));
  inv1   g95(.a(new_n138_), .O(z17));
endmodule


