// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  inv1   g02(.a(x04), .O(new_n46_));
  nand2  g03(.a(x06), .b(new_n46_), .O(new_n47_));
  nor2   g04(.a(x18), .b(x17), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  or2    g06(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nor3   g08(.a(x02), .b(x01), .c(x00), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n50_), .O(z00));
  nand2  g11(.a(new_n52_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n50_), .O(z01));
  nand3  g13(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(z02));
  inv1   g15(.a(x00), .O(new_n59_));
  inv1   g16(.a(x01), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(x02), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(x11), .b(x10), .c(new_n51_), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n63_), .c(x12), .O(new_n66_));
  inv1   g23(.a(x17), .O(new_n67_));
  nand4  g24(.a(new_n52_), .b(new_n45_), .c(x18), .d(new_n67_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n66_), .c(new_n47_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n51_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n62_), .c(new_n47_), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nand2  g31(.a(new_n61_), .b(x09), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n47_), .O(z05));
  nand3  g33(.a(new_n65_), .b(new_n63_), .c(new_n47_), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(z06));
  nand2  g35(.a(new_n65_), .b(new_n71_), .O(new_n79_));
  nand3  g36(.a(new_n47_), .b(x02), .c(x00), .O(new_n80_));
  aoi21  g37(.a(new_n79_), .b(x01), .c(new_n80_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nand3  g42(.a(x02), .b(new_n60_), .c(new_n59_), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nor2   g44(.a(x18), .b(new_n67_), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n87_), .c(x19), .O(new_n89_));
  oai21  g46(.a(new_n89_), .b(new_n85_), .c(new_n47_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n45_), .c(x18), .d(x01), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nand2  g50(.a(x02), .b(new_n60_), .O(new_n94_));
  inv1   g51(.a(x13), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x12), .c(new_n70_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nor2   g54(.a(x15), .b(x14), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n97_), .c(x20), .d(new_n93_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand4  g58(.a(new_n47_), .b(new_n101_), .c(new_n100_), .d(new_n59_), .O(new_n102_));
  aoi21  g59(.a(new_n99_), .b(new_n92_), .c(new_n102_), .O(z09));
  nand2  g60(.a(new_n47_), .b(new_n59_), .O(new_n104_));
  nor2   g61(.a(new_n92_), .b(new_n100_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(x22), .O(new_n106_));
  inv1   g63(.a(x14), .O(new_n107_));
  nand2  g64(.a(x15), .b(new_n107_), .O(new_n108_));
  nand4  g65(.a(new_n101_), .b(new_n100_), .c(x20), .d(x16), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n97_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n106_), .c(new_n104_), .O(z10));
  nand3  g69(.a(new_n100_), .b(x20), .c(new_n93_), .O(new_n113_));
  nor2   g70(.a(new_n113_), .b(new_n108_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n97_), .c(new_n105_), .O(new_n115_));
  nand2  g72(.a(new_n101_), .b(new_n59_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n115_), .c(new_n47_), .O(z11));
  nand2  g74(.a(new_n74_), .b(new_n61_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  inv1   g76(.a(x02), .O(new_n120_));
  nand3  g77(.a(new_n45_), .b(x17), .c(new_n120_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n60_), .c(new_n59_), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand3  g81(.a(new_n47_), .b(new_n124_), .c(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n118_), .c(new_n125_), .O(z12));
  inv1   g83(.a(x06), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(x04), .O(new_n128_));
  nor3   g85(.a(new_n128_), .b(x19), .c(new_n67_), .O(new_n129_));
  and2   g86(.a(new_n129_), .b(new_n52_), .O(z13));
  nor3   g87(.a(new_n53_), .b(new_n49_), .c(x10), .O(z14));
  oai21  g88(.a(x10), .b(new_n60_), .c(x02), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(x00), .O(new_n133_));
  nand3  g90(.a(x19), .b(new_n120_), .c(new_n60_), .O(new_n134_));
  aoi21  g91(.a(new_n134_), .b(new_n133_), .c(new_n128_), .O(z15));
  nor2   g92(.a(new_n104_), .b(new_n60_), .O(z16));
  nand2  g93(.a(new_n86_), .b(new_n47_), .O(z17));
endmodule


