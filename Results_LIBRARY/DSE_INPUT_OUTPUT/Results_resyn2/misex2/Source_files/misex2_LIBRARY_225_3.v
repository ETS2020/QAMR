// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n128_, new_n129_, new_n131_;
  nand2  g00(.a(x24), .b(x21), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  nor2   g03(.a(x02), .b(x01), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  inv1   g08(.a(x18), .O(new_n52_));
  inv1   g09(.a(x19), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n50_), .c(new_n44_), .O(z00));
  inv1   g12(.a(new_n44_), .O(new_n56_));
  nand2  g13(.a(new_n49_), .b(x09), .O(new_n57_));
  nor3   g14(.a(new_n57_), .b(new_n54_), .c(new_n56_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n59_), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n57_), .c(new_n44_), .O(z02));
  nand4  g18(.a(new_n49_), .b(new_n53_), .c(x18), .d(new_n51_), .O(new_n62_));
  inv1   g19(.a(x11), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n59_), .O(new_n64_));
  inv1   g21(.a(x02), .O(new_n65_));
  nor2   g22(.a(x09), .b(new_n65_), .O(new_n66_));
  inv1   g23(.a(x01), .O(new_n67_));
  nor2   g24(.a(new_n67_), .b(new_n46_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n66_), .c(new_n64_), .d(x12), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(new_n62_), .c(new_n44_), .O(z03));
  nand3  g27(.a(new_n68_), .b(new_n66_), .c(new_n44_), .O(new_n71_));
  nand2  g28(.a(new_n63_), .b(x10), .O(new_n72_));
  nor3   g29(.a(new_n72_), .b(new_n71_), .c(x12), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nand2  g31(.a(new_n68_), .b(x09), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n44_), .O(z05));
  nor3   g33(.a(new_n71_), .b(new_n63_), .c(new_n59_), .O(z06));
  nor2   g34(.a(x12), .b(x09), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(new_n64_), .c(new_n67_), .O(new_n79_));
  nand2  g36(.a(x02), .b(x00), .O(new_n80_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n44_), .O(z07));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  inv1   g40(.a(x04), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(x03), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n83_), .c(new_n82_), .d(new_n44_), .O(new_n86_));
  nand3  g43(.a(x02), .b(new_n67_), .c(new_n46_), .O(new_n87_));
  nand3  g44(.a(x19), .b(new_n52_), .c(x17), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n53_), .c(x18), .d(x01), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n67_), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n63_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nor2   g53(.a(x15), .b(x14), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(x20), .d(new_n92_), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(new_n46_), .O(new_n101_));
  aoi21  g58(.a(new_n98_), .b(new_n91_), .c(new_n101_), .O(z09));
  nand2  g59(.a(x22), .b(x21), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n91_), .O(new_n104_));
  inv1   g61(.a(x14), .O(new_n105_));
  nand4  g62(.a(new_n99_), .b(x20), .c(x15), .d(new_n105_), .O(new_n106_));
  nand2  g63(.a(new_n100_), .b(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n96_), .c(new_n104_), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g67(.a(new_n100_), .b(new_n46_), .O(new_n111_));
  inv1   g68(.a(new_n106_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n96_), .c(new_n92_), .O(new_n113_));
  inv1   g70(.a(new_n91_), .O(new_n114_));
  nor2   g71(.a(x24), .b(new_n99_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n113_), .c(new_n111_), .O(z11));
  and2   g74(.a(new_n74_), .b(new_n68_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n53_), .b(x17), .c(new_n65_), .O(new_n120_));
  nand2  g77(.a(new_n67_), .b(new_n46_), .O(new_n121_));
  aoi21  g78(.a(new_n120_), .b(new_n119_), .c(new_n121_), .O(new_n122_));
  nor2   g79(.a(x24), .b(new_n45_), .O(new_n123_));
  oai21  g80(.a(new_n122_), .b(new_n118_), .c(new_n123_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n44_), .O(z12));
  nor4   g82(.a(new_n48_), .b(new_n56_), .c(x19), .d(new_n51_), .O(z13));
  oai21  g83(.a(new_n60_), .b(new_n50_), .c(new_n44_), .O(z14));
  aoi21  g84(.a(new_n59_), .b(x01), .c(new_n65_), .O(new_n128_));
  aoi21  g85(.a(new_n47_), .b(x19), .c(new_n56_), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(new_n46_), .c(new_n129_), .O(z15));
  nand3  g87(.a(new_n44_), .b(x01), .c(new_n46_), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z16));
  nand2  g89(.a(new_n87_), .b(new_n44_), .O(z17));
endmodule


