// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n129_,
    new_n131_, new_n132_, new_n135_;
  nor3   g00(.a(x09), .b(x02), .c(x00), .O(new_n44_));
  nor3   g01(.a(x19), .b(x18), .c(x17), .O(new_n45_));
  nand3  g02(.a(new_n45_), .b(new_n44_), .c(x10), .O(new_n46_));
  aoi21  g03(.a(new_n46_), .b(x16), .c(x01), .O(z00));
  nor2   g04(.a(x02), .b(x00), .O(new_n48_));
  nand4  g05(.a(new_n45_), .b(new_n48_), .c(x10), .d(x09), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(x16), .c(x01), .O(z01));
  nand2  g07(.a(new_n48_), .b(x09), .O(new_n51_));
  inv1   g08(.a(x10), .O(new_n52_));
  nand2  g09(.a(new_n45_), .b(new_n52_), .O(new_n53_));
  inv1   g10(.a(x01), .O(new_n54_));
  nand2  g11(.a(x16), .b(new_n54_), .O(new_n55_));
  nor3   g12(.a(new_n55_), .b(new_n53_), .c(new_n51_), .O(z02));
  inv1   g13(.a(x16), .O(new_n57_));
  inv1   g14(.a(x18), .O(new_n58_));
  nor2   g15(.a(x19), .b(new_n58_), .O(new_n59_));
  nor3   g16(.a(x17), .b(x02), .c(x00), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(new_n61_));
  nand3  g18(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  inv1   g20(.a(x09), .O(new_n64_));
  nand3  g21(.a(x11), .b(x10), .c(new_n64_), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n63_), .c(x12), .O(new_n67_));
  oai21  g24(.a(new_n61_), .b(x01), .c(new_n67_), .O(z03));
  nand2  g25(.a(new_n57_), .b(new_n54_), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n64_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n62_), .c(new_n69_), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  inv1   g31(.a(x00), .O(new_n75_));
  nor2   g32(.a(new_n54_), .b(new_n75_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x09), .O(new_n77_));
  nor2   g34(.a(new_n77_), .b(new_n74_), .O(z05));
  nor2   g35(.a(new_n65_), .b(new_n62_), .O(z06));
  oai21  g36(.a(new_n65_), .b(x12), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n69_), .O(z07));
  nand3  g39(.a(x19), .b(new_n58_), .c(x17), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  inv1   g41(.a(x05), .O(new_n85_));
  inv1   g42(.a(x06), .O(new_n86_));
  inv1   g43(.a(x07), .O(new_n87_));
  inv1   g44(.a(x08), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n85_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  inv1   g47(.a(x03), .O(new_n91_));
  nand4  g48(.a(x04), .b(new_n91_), .c(x02), .d(new_n75_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n90_), .c(new_n84_), .O(new_n94_));
  aoi21  g51(.a(new_n94_), .b(x16), .c(x01), .O(z08));
  inv1   g52(.a(x22), .O(new_n96_));
  nor2   g53(.a(new_n54_), .b(x00), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n59_), .c(new_n96_), .O(new_n98_));
  nor3   g55(.a(new_n98_), .b(x21), .c(x20), .O(z09));
  inv1   g56(.a(x21), .O(new_n100_));
  nand3  g57(.a(new_n96_), .b(new_n100_), .c(x20), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  inv1   g59(.a(x13), .O(new_n103_));
  inv1   g60(.a(x14), .O(new_n104_));
  nand4  g61(.a(x15), .b(new_n104_), .c(new_n103_), .d(x02), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand4  g63(.a(x16), .b(x12), .c(new_n70_), .d(new_n54_), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n106_), .c(new_n102_), .O(new_n109_));
  nor2   g66(.a(new_n100_), .b(x20), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n59_), .c(x22), .d(x01), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n109_), .c(x00), .O(z10));
  inv1   g69(.a(new_n110_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n98_), .c(new_n69_), .O(z11));
  nand2  g71(.a(new_n74_), .b(new_n76_), .O(new_n115_));
  inv1   g72(.a(x02), .O(new_n116_));
  inv1   g73(.a(x17), .O(new_n117_));
  inv1   g74(.a(x19), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(new_n119_));
  inv1   g76(.a(x23), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(new_n120_), .O(new_n121_));
  nor2   g78(.a(x01), .b(x00), .O(new_n122_));
  nand4  g79(.a(new_n122_), .b(new_n121_), .c(new_n119_), .d(x16), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n115_), .c(new_n125_), .O(z12));
  nand3  g83(.a(new_n48_), .b(new_n118_), .c(x17), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(x16), .c(x01), .O(z13));
  nand3  g85(.a(new_n45_), .b(new_n44_), .c(new_n52_), .O(new_n129_));
  aoi21  g86(.a(new_n129_), .b(x16), .c(x01), .O(z14));
  aoi21  g87(.a(new_n52_), .b(x01), .c(new_n116_), .O(new_n131_));
  aoi21  g88(.a(x19), .b(new_n116_), .c(new_n57_), .O(new_n132_));
  oai22  g89(.a(new_n132_), .b(x01), .c(new_n131_), .d(new_n75_), .O(z15));
  oai21  g90(.a(new_n54_), .b(x00), .c(new_n69_), .O(z16));
  nand2  g91(.a(x02), .b(new_n75_), .O(new_n135_));
  aoi21  g92(.a(new_n135_), .b(x16), .c(x01), .O(z17));
endmodule


