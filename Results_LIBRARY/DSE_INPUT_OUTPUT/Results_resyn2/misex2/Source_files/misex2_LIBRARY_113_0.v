// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n79_, new_n80_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n127_, new_n129_, new_n130_,
    new_n133_;
  nor3   g00(.a(x09), .b(x02), .c(x00), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(x06), .c(x01), .O(z00));
  nor2   g07(.a(x02), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n48_), .b(new_n51_), .c(x09), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(x06), .c(x01), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand3  g11(.a(new_n46_), .b(new_n45_), .c(new_n54_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand3  g13(.a(new_n56_), .b(new_n51_), .c(x09), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(x06), .c(x01), .O(z02));
  inv1   g15(.a(x06), .O(new_n59_));
  inv1   g16(.a(x18), .O(new_n60_));
  nor2   g17(.a(x19), .b(new_n60_), .O(new_n61_));
  nor3   g18(.a(x17), .b(x02), .c(x00), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(new_n63_));
  inv1   g20(.a(x09), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(x02), .c(x01), .d(x00), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(x12), .c(x11), .d(x10), .O(new_n67_));
  oai21  g24(.a(new_n63_), .b(x01), .c(new_n67_), .O(z03));
  inv1   g25(.a(x01), .O(new_n69_));
  nand2  g26(.a(new_n59_), .b(new_n69_), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  nor2   g28(.a(x12), .b(new_n54_), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n65_), .c(new_n70_), .O(z04));
  nand2  g31(.a(x01), .b(x00), .O(new_n75_));
  nand3  g32(.a(x10), .b(x09), .c(x02), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n70_), .O(z05));
  nor3   g34(.a(new_n65_), .b(new_n71_), .c(new_n54_), .O(z06));
  nand2  g35(.a(x02), .b(x00), .O(new_n79_));
  nand2  g36(.a(x06), .b(new_n69_), .O(new_n80_));
  nand4  g37(.a(new_n72_), .b(x11), .c(new_n64_), .d(x01), .O(new_n81_));
  aoi21  g38(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(z07));
  inv1   g39(.a(new_n70_), .O(z08));
  nor2   g40(.a(x20), .b(new_n69_), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n61_), .O(new_n85_));
  nand4  g42(.a(new_n71_), .b(x06), .c(x02), .d(new_n69_), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nor2   g44(.a(x14), .b(x13), .O(new_n88_));
  inv1   g45(.a(x15), .O(new_n89_));
  inv1   g46(.a(x16), .O(new_n90_));
  nand3  g47(.a(x20), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n88_), .c(new_n87_), .d(x12), .O(new_n93_));
  inv1   g50(.a(x00), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  aoi21  g54(.a(new_n93_), .b(new_n85_), .c(new_n97_), .O(z09));
  nand4  g55(.a(new_n84_), .b(new_n61_), .c(x22), .d(x21), .O(new_n99_));
  inv1   g56(.a(x13), .O(new_n100_));
  inv1   g57(.a(x14), .O(new_n101_));
  nand4  g58(.a(new_n95_), .b(x20), .c(new_n101_), .d(new_n100_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n96_), .b(x16), .c(x15), .d(x12), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n103_), .c(new_n87_), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n99_), .c(x00), .O(z10));
  nand2  g64(.a(new_n96_), .b(new_n94_), .O(new_n108_));
  nand3  g65(.a(new_n84_), .b(new_n61_), .c(x21), .O(new_n109_));
  nor2   g66(.a(x16), .b(new_n89_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n103_), .c(new_n87_), .d(x12), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(z11));
  nand2  g69(.a(x10), .b(x02), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(x01), .c(x00), .O(new_n114_));
  oai21  g71(.a(x19), .b(x02), .c(x23), .O(new_n115_));
  inv1   g72(.a(x02), .O(new_n116_));
  inv1   g73(.a(x19), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(x17), .c(new_n116_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n115_), .O(new_n119_));
  nand3  g76(.a(x06), .b(new_n69_), .c(new_n94_), .O(new_n120_));
  inv1   g77(.a(new_n120_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n114_), .c(new_n124_), .O(z12));
  nor2   g82(.a(new_n120_), .b(new_n118_), .O(z13));
  nand2  g83(.a(new_n56_), .b(new_n44_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(x06), .c(x01), .O(z14));
  aoi21  g85(.a(new_n54_), .b(x01), .c(new_n116_), .O(new_n129_));
  aoi21  g86(.a(x19), .b(new_n116_), .c(new_n59_), .O(new_n130_));
  oai22  g87(.a(new_n130_), .b(x01), .c(new_n129_), .d(new_n94_), .O(z15));
  nor2   g88(.a(new_n69_), .b(x00), .O(z16));
  nand2  g89(.a(x02), .b(new_n94_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(x06), .c(x01), .O(z17));
endmodule


