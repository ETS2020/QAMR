// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n127_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor2   g01(.a(x02), .b(x00), .O(new_n45_));
  nor2   g02(.a(x06), .b(x01), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  nor3   g06(.a(x19), .b(x18), .c(x17), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n49_), .O(z00));
  nand3  g09(.a(new_n46_), .b(new_n45_), .c(x09), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n51_), .O(z01));
  inv1   g11(.a(x06), .O(new_n55_));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n50_), .b(new_n45_), .c(new_n56_), .d(x09), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(new_n55_), .c(x01), .O(z02));
  nand2  g15(.a(x02), .b(x00), .O(new_n59_));
  nand2  g16(.a(x11), .b(x10), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nor2   g19(.a(x09), .b(new_n62_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n59_), .O(z06));
  nand2  g22(.a(z06), .b(x12), .O(new_n66_));
  inv1   g23(.a(x17), .O(new_n67_));
  inv1   g24(.a(x19), .O(new_n68_));
  nand4  g25(.a(new_n48_), .b(new_n68_), .c(x18), .d(new_n67_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n66_), .O(z03));
  inv1   g27(.a(x12), .O(new_n71_));
  nand2  g28(.a(new_n63_), .b(new_n71_), .O(new_n72_));
  nor4   g29(.a(new_n72_), .b(new_n59_), .c(x11), .d(new_n56_), .O(z04));
  nand2  g30(.a(x06), .b(new_n62_), .O(new_n74_));
  nand2  g31(.a(x01), .b(x00), .O(new_n75_));
  nand3  g32(.a(x10), .b(x09), .c(x02), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(z05));
  inv1   g34(.a(new_n46_), .O(new_n78_));
  nand3  g35(.a(new_n63_), .b(new_n61_), .c(new_n71_), .O(new_n79_));
  aoi21  g36(.a(new_n79_), .b(new_n78_), .c(new_n59_), .O(z07));
  nor2   g37(.a(new_n67_), .b(x08), .O(new_n81_));
  nor2   g38(.a(new_n68_), .b(x18), .O(new_n82_));
  inv1   g39(.a(x00), .O(new_n83_));
  nand2  g40(.a(x02), .b(new_n83_), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nor2   g42(.a(x07), .b(x05), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(x04), .c(new_n85_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n82_), .c(new_n81_), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(new_n55_), .c(x01), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n68_), .c(x18), .d(x01), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n62_), .O(new_n93_));
  inv1   g50(.a(x11), .O(new_n94_));
  inv1   g51(.a(x13), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x12), .c(new_n94_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nor2   g54(.a(x14), .b(x06), .O(new_n98_));
  nor2   g55(.a(x16), .b(x15), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(x20), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n83_), .O(new_n103_));
  aoi21  g60(.a(new_n100_), .b(new_n92_), .c(new_n103_), .O(z09));
  nor2   g61(.a(new_n92_), .b(new_n101_), .O(new_n105_));
  inv1   g62(.a(x14), .O(new_n106_));
  nand4  g63(.a(new_n101_), .b(x20), .c(x15), .d(new_n106_), .O(new_n107_));
  nand2  g64(.a(new_n102_), .b(x16), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  aoi22  g66(.a(new_n109_), .b(new_n97_), .c(new_n105_), .d(x22), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n74_), .O(z10));
  nand2  g68(.a(new_n102_), .b(new_n83_), .O(new_n112_));
  nor2   g69(.a(new_n107_), .b(x16), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n97_), .c(new_n105_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n112_), .c(new_n74_), .O(z11));
  nand2  g72(.a(x10), .b(x02), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(x01), .c(x00), .O(new_n117_));
  inv1   g74(.a(x23), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(new_n118_), .O(new_n119_));
  inv1   g76(.a(x02), .O(new_n120_));
  nand3  g77(.a(new_n68_), .b(new_n67_), .c(new_n120_), .O(new_n121_));
  nand4  g78(.a(new_n121_), .b(new_n119_), .c(new_n46_), .d(new_n83_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n117_), .c(new_n124_), .O(z12));
  nor3   g82(.a(new_n47_), .b(x19), .c(new_n67_), .O(z13));
  nand2  g83(.a(new_n50_), .b(new_n56_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(new_n49_), .O(z14));
  nand3  g85(.a(new_n46_), .b(x19), .c(new_n120_), .O(new_n129_));
  aoi21  g86(.a(new_n56_), .b(x01), .c(new_n120_), .O(new_n130_));
  nand2  g87(.a(new_n74_), .b(x00), .O(new_n131_));
  oai21  g88(.a(new_n131_), .b(new_n130_), .c(new_n129_), .O(z15));
  nor2   g89(.a(new_n62_), .b(x00), .O(z16));
  aoi21  g90(.a(new_n84_), .b(new_n55_), .c(x01), .O(z17));
endmodule


