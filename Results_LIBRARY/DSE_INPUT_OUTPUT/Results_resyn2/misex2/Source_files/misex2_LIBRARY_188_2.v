// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n130_, new_n132_,
    new_n134_;
  inv1   g00(.a(x16), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x10), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand2  g13(.a(new_n54_), .b(x09), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n49_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  nand4  g18(.a(new_n50_), .b(x02), .c(x01), .d(x00), .O(new_n62_));
  nand4  g19(.a(x16), .b(x12), .c(x11), .d(x10), .O(new_n63_));
  or2    g20(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nor2   g21(.a(x19), .b(new_n47_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n54_), .c(new_n46_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n64_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand4  g26(.a(x16), .b(new_n69_), .c(new_n68_), .d(x10), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n62_), .O(z04));
  inv1   g28(.a(x02), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n51_), .O(new_n73_));
  nand3  g30(.a(new_n73_), .b(x10), .c(x09), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x16), .c(new_n52_), .O(z05));
  nand4  g32(.a(new_n73_), .b(x11), .c(x10), .d(new_n50_), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x16), .c(new_n52_), .O(z06));
  nand3  g34(.a(x11), .b(x10), .c(new_n50_), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(x12), .c(x01), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n73_), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n45_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nor2   g42(.a(new_n72_), .b(x01), .O(new_n86_));
  nor2   g43(.a(x18), .b(new_n46_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(x19), .d(new_n51_), .O(new_n88_));
  oai21  g45(.a(new_n88_), .b(new_n85_), .c(new_n45_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nor2   g47(.a(new_n44_), .b(new_n52_), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n65_), .c(new_n90_), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n68_), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  inv1   g53(.a(x14), .O(new_n97_));
  nand3  g54(.a(x20), .b(new_n44_), .c(new_n97_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n96_), .c(new_n86_), .d(new_n93_), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n51_), .O(new_n102_));
  or2    g59(.a(new_n102_), .b(x21), .O(new_n103_));
  aoi21  g60(.a(new_n100_), .b(new_n92_), .c(new_n103_), .O(z09));
  nand4  g61(.a(x21), .b(new_n90_), .c(new_n48_), .d(x18), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(x22), .c(x01), .O(new_n107_));
  nor2   g64(.a(x21), .b(new_n93_), .O(new_n108_));
  nand3  g65(.a(new_n101_), .b(x20), .c(new_n97_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n108_), .c(new_n96_), .d(new_n86_), .O(new_n111_));
  nand2  g68(.a(x16), .b(new_n51_), .O(new_n112_));
  aoi21  g69(.a(new_n111_), .b(new_n107_), .c(new_n112_), .O(z10));
  nand4  g70(.a(new_n108_), .b(new_n99_), .c(new_n96_), .d(new_n86_), .O(new_n114_));
  nand2  g71(.a(new_n106_), .b(new_n91_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n114_), .c(new_n102_), .O(z11));
  nand3  g73(.a(x16), .b(x01), .c(x00), .O(new_n117_));
  inv1   g74(.a(new_n117_), .O(new_n118_));
  oai21  g75(.a(new_n59_), .b(new_n72_), .c(new_n118_), .O(new_n119_));
  inv1   g76(.a(new_n53_), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(x23), .O(new_n121_));
  nand3  g78(.a(new_n48_), .b(x17), .c(new_n72_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  inv1   g81(.a(x24), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x09), .O(new_n126_));
  aoi21  g83(.a(new_n124_), .b(new_n119_), .c(new_n126_), .O(z12));
  oai21  g84(.a(new_n122_), .b(new_n53_), .c(new_n45_), .O(z13));
  nor2   g85(.a(new_n60_), .b(new_n55_), .O(z14));
  aoi22  g86(.a(new_n45_), .b(x00), .c(x19), .d(new_n52_), .O(new_n130_));
  oai22  g87(.a(new_n130_), .b(x02), .c(new_n117_), .d(x10), .O(z15));
  nand3  g88(.a(x16), .b(x01), .c(new_n51_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z16));
  nand2  g90(.a(new_n86_), .b(new_n51_), .O(new_n134_));
  nand2  g91(.a(new_n134_), .b(new_n45_), .O(z17));
endmodule


