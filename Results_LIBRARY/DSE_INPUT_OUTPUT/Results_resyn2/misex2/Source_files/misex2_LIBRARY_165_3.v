// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n128_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  nand2  g03(.a(x23), .b(x08), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(x10), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  inv1   g09(.a(x01), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x02), .O(new_n55_));
  nand2  g12(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n50_), .O(z00));
  nand2  g14(.a(new_n55_), .b(x09), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n50_), .O(z01));
  nor3   g16(.a(new_n58_), .b(new_n48_), .c(x10), .O(z02));
  inv1   g17(.a(new_n47_), .O(new_n61_));
  nand2  g18(.a(x01), .b(x00), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(x11), .b(x10), .c(new_n51_), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n63_), .c(x12), .d(x02), .O(new_n66_));
  nand4  g23(.a(new_n55_), .b(new_n46_), .c(x18), .d(new_n44_), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(z03));
  nand2  g25(.a(new_n63_), .b(x02), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n51_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n69_), .c(new_n47_), .O(z04));
  inv1   g30(.a(x02), .O(new_n74_));
  inv1   g31(.a(x10), .O(new_n75_));
  nand2  g32(.a(new_n63_), .b(new_n47_), .O(new_n76_));
  nor4   g33(.a(new_n76_), .b(new_n75_), .c(new_n51_), .d(new_n74_), .O(z05));
  nor3   g34(.a(new_n76_), .b(new_n64_), .c(new_n74_), .O(z06));
  nand2  g35(.a(new_n65_), .b(new_n71_), .O(new_n79_));
  nand3  g36(.a(new_n47_), .b(x02), .c(x00), .O(new_n80_));
  aoi21  g37(.a(new_n79_), .b(x01), .c(new_n80_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nand3  g42(.a(x02), .b(new_n53_), .c(new_n52_), .O(new_n86_));
  nand3  g43(.a(x19), .b(new_n45_), .c(x17), .O(new_n87_));
  nor3   g44(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(z08));
  inv1   g45(.a(x20), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n46_), .c(x18), .d(x01), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  nand2  g48(.a(x02), .b(new_n53_), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(x12), .c(new_n70_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nor2   g52(.a(x15), .b(x14), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(x20), .d(new_n91_), .O(new_n97_));
  inv1   g54(.a(x21), .O(new_n98_));
  inv1   g55(.a(x22), .O(new_n99_));
  nand4  g56(.a(new_n47_), .b(new_n99_), .c(new_n98_), .d(new_n52_), .O(new_n100_));
  aoi21  g57(.a(new_n97_), .b(new_n90_), .c(new_n100_), .O(z09));
  inv1   g58(.a(x14), .O(new_n102_));
  nand2  g59(.a(x15), .b(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n99_), .b(new_n98_), .c(x20), .d(x16), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nor2   g62(.a(new_n90_), .b(new_n98_), .O(new_n106_));
  aoi22  g63(.a(new_n106_), .b(x22), .c(new_n105_), .d(new_n95_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(x00), .c(new_n47_), .O(z10));
  nand3  g65(.a(new_n98_), .b(x20), .c(new_n91_), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n103_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n95_), .c(new_n106_), .O(new_n111_));
  nand2  g68(.a(new_n99_), .b(new_n52_), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(new_n111_), .c(new_n47_), .O(z11));
  aoi21  g70(.a(x10), .b(x02), .c(new_n62_), .O(new_n114_));
  oai21  g71(.a(x19), .b(x02), .c(x23), .O(new_n115_));
  nand3  g72(.a(new_n46_), .b(x17), .c(new_n74_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n115_), .c(new_n54_), .O(new_n117_));
  nor2   g74(.a(x24), .b(new_n51_), .O(new_n118_));
  oai21  g75(.a(new_n117_), .b(new_n114_), .c(new_n118_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n47_), .O(z12));
  oai21  g77(.a(new_n116_), .b(new_n54_), .c(new_n47_), .O(z13));
  nand4  g78(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n75_), .O(new_n122_));
  oai21  g79(.a(new_n122_), .b(new_n56_), .c(new_n47_), .O(z14));
  oai21  g80(.a(x10), .b(new_n53_), .c(x02), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x00), .O(new_n125_));
  nand3  g82(.a(x19), .b(new_n74_), .c(new_n53_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n125_), .c(new_n47_), .O(z15));
  nand3  g84(.a(new_n47_), .b(x01), .c(new_n52_), .O(new_n128_));
  inv1   g85(.a(new_n128_), .O(z16));
  nand2  g86(.a(new_n86_), .b(new_n47_), .O(z17));
endmodule


