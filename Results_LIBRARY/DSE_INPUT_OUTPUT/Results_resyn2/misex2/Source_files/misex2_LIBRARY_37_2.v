// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n128_, new_n130_, new_n131_,
    new_n132_;
  inv1   g00(.a(x04), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x01), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x19), .b(x18), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g09(.a(x17), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(x10), .O(new_n54_));
  nor3   g11(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(z00));
  nand4  g12(.a(new_n51_), .b(new_n48_), .c(x10), .d(x09), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n44_), .c(x17), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n51_), .b(new_n48_), .c(new_n58_), .d(x09), .O(new_n59_));
  aoi21  g16(.a(new_n59_), .b(new_n44_), .c(x17), .O(z02));
  inv1   g17(.a(x12), .O(new_n61_));
  inv1   g18(.a(x11), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  inv1   g20(.a(x02), .O(new_n64_));
  nor2   g21(.a(x17), .b(new_n44_), .O(new_n65_));
  nor2   g22(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g23(.a(x01), .b(x00), .O(new_n67_));
  nor2   g24(.a(new_n67_), .b(x09), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n66_), .c(new_n63_), .O(new_n69_));
  inv1   g26(.a(x19), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(x18), .c(new_n53_), .O(new_n71_));
  oai22  g28(.a(new_n71_), .b(new_n49_), .c(new_n69_), .d(new_n61_), .O(z03));
  nand2  g29(.a(new_n68_), .b(new_n66_), .O(new_n73_));
  nand3  g30(.a(new_n61_), .b(new_n62_), .c(x10), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n73_), .O(z04));
  inv1   g32(.a(new_n65_), .O(new_n76_));
  nand3  g33(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n67_), .c(new_n76_), .O(z05));
  inv1   g35(.a(new_n69_), .O(z06));
  nor2   g36(.a(x12), .b(x09), .O(new_n80_));
  aoi21  g37(.a(new_n80_), .b(new_n63_), .c(new_n46_), .O(new_n81_));
  nand2  g38(.a(x02), .b(x00), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(new_n81_), .c(new_n76_), .O(z07));
  inv1   g40(.a(x08), .O(new_n84_));
  inv1   g41(.a(x18), .O(new_n85_));
  nand4  g42(.a(x19), .b(new_n85_), .c(x17), .d(new_n84_), .O(new_n86_));
  inv1   g43(.a(x05), .O(new_n87_));
  nor2   g44(.a(x07), .b(x06), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  inv1   g46(.a(x03), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(x02), .O(new_n91_));
  nor4   g48(.a(new_n91_), .b(new_n89_), .c(new_n86_), .d(new_n47_), .O(z08));
  inv1   g49(.a(x20), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n70_), .c(x18), .d(x01), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand2  g52(.a(x02), .b(new_n46_), .O(new_n96_));
  inv1   g53(.a(x13), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(x12), .c(new_n62_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  inv1   g56(.a(x14), .O(new_n100_));
  inv1   g57(.a(x15), .O(new_n101_));
  inv1   g58(.a(x16), .O(new_n102_));
  nand4  g59(.a(x20), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  aoi21  g61(.a(new_n104_), .b(new_n99_), .c(new_n95_), .O(new_n105_));
  inv1   g62(.a(x21), .O(new_n106_));
  inv1   g63(.a(x22), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(new_n106_), .c(new_n45_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(new_n105_), .c(new_n76_), .O(z09));
  nor2   g66(.a(new_n94_), .b(new_n106_), .O(new_n110_));
  nand4  g67(.a(new_n106_), .b(x20), .c(x15), .d(new_n100_), .O(new_n111_));
  nand2  g68(.a(new_n107_), .b(x16), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  aoi22  g70(.a(new_n113_), .b(new_n99_), .c(new_n110_), .d(x22), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(x00), .c(new_n76_), .O(z10));
  nand2  g72(.a(new_n107_), .b(new_n45_), .O(new_n116_));
  nor2   g73(.a(new_n111_), .b(x16), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n99_), .c(new_n110_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n116_), .c(new_n76_), .O(z11));
  aoi21  g76(.a(x10), .b(x02), .c(new_n67_), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(x23), .O(new_n121_));
  nand3  g78(.a(new_n70_), .b(x17), .c(new_n64_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n121_), .c(new_n47_), .O(new_n123_));
  nor2   g80(.a(x24), .b(new_n50_), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n120_), .c(new_n124_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n76_), .O(z12));
  nor2   g83(.a(new_n122_), .b(new_n47_), .O(z13));
  nand2  g84(.a(new_n53_), .b(new_n58_), .O(new_n128_));
  nor3   g85(.a(new_n128_), .b(new_n52_), .c(new_n49_), .O(z14));
  oai21  g86(.a(x10), .b(new_n46_), .c(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand3  g88(.a(x19), .b(new_n64_), .c(new_n46_), .O(new_n132_));
  aoi21  g89(.a(new_n132_), .b(new_n131_), .c(new_n65_), .O(z15));
  oai21  g90(.a(new_n46_), .b(x00), .c(new_n76_), .O(z16));
  nor3   g91(.a(new_n65_), .b(new_n47_), .c(new_n64_), .O(z17));
endmodule


