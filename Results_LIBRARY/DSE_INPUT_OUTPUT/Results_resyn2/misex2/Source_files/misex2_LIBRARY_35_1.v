// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n136_;
  inv1   g00(.a(x24), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(x10), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nor2   g06(.a(x02), .b(x00), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z00));
  nand3  g09(.a(new_n50_), .b(new_n49_), .c(x09), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x01), .O(z01));
  nor2   g11(.a(x17), .b(x10), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n50_), .c(new_n47_), .d(x09), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g14(.a(x00), .O(new_n58_));
  inv1   g15(.a(x01), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(x02), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n45_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(z06));
  nand2  g20(.a(z06), .b(x12), .O(new_n64_));
  inv1   g21(.a(x02), .O(new_n65_));
  nor2   g22(.a(x01), .b(x00), .O(new_n66_));
  nor2   g23(.a(x24), .b(x19), .O(new_n67_));
  inv1   g24(.a(x18), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(x17), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n64_), .O(z03));
  nand2  g28(.a(x24), .b(new_n59_), .O(new_n72_));
  inv1   g29(.a(x11), .O(new_n73_));
  inv1   g30(.a(x12), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(new_n73_), .c(x10), .d(new_n45_), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n61_), .c(new_n72_), .O(z04));
  nand2  g33(.a(x10), .b(x02), .O(new_n77_));
  nand2  g34(.a(new_n60_), .b(x09), .O(new_n78_));
  nor2   g35(.a(new_n78_), .b(new_n77_), .O(z05));
  oai21  g36(.a(new_n62_), .b(x12), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n72_), .O(z07));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(x04), .b(new_n86_), .c(x02), .d(new_n58_), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x19), .c(new_n68_), .d(x17), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(new_n44_), .c(x01), .O(z08));
  nand2  g47(.a(x02), .b(new_n59_), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(x12), .d(new_n73_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nand3  g54(.a(x20), .b(new_n97_), .c(new_n96_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  inv1   g56(.a(x19), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(x18), .d(x01), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  aoi21  g60(.a(new_n99_), .b(new_n95_), .c(new_n103_), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  inv1   g62(.a(x22), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n58_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n104_), .c(new_n72_), .O(z09));
  nand3  g65(.a(new_n103_), .b(x22), .c(x21), .O(new_n109_));
  nand3  g66(.a(new_n44_), .b(new_n105_), .c(x20), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  nor3   g68(.a(x22), .b(new_n97_), .c(new_n96_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n111_), .c(new_n95_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n109_), .c(x00), .O(z10));
  nand2  g71(.a(new_n106_), .b(new_n58_), .O(new_n115_));
  nand2  g72(.a(new_n103_), .b(x21), .O(new_n116_));
  nand4  g73(.a(new_n111_), .b(new_n95_), .c(new_n97_), .d(x15), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(z11));
  nand2  g75(.a(new_n77_), .b(new_n60_), .O(new_n119_));
  nand3  g76(.a(new_n100_), .b(new_n46_), .c(new_n65_), .O(new_n120_));
  inv1   g77(.a(x23), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(new_n121_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n120_), .c(new_n66_), .O(new_n123_));
  nand2  g80(.a(new_n44_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n123_), .b(new_n119_), .c(new_n124_), .O(z12));
  nand2  g82(.a(new_n66_), .b(new_n65_), .O(new_n126_));
  nand2  g83(.a(new_n67_), .b(x17), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(new_n126_), .O(z13));
  nand4  g85(.a(new_n67_), .b(new_n55_), .c(new_n68_), .d(new_n45_), .O(new_n129_));
  nor2   g86(.a(new_n129_), .b(new_n126_), .O(z14));
  inv1   g87(.a(x10), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(x01), .c(new_n65_), .O(new_n132_));
  aoi21  g89(.a(x19), .b(new_n65_), .c(x24), .O(new_n133_));
  oai22  g90(.a(new_n133_), .b(x01), .c(new_n132_), .d(new_n58_), .O(z15));
  aoi21  g91(.a(new_n44_), .b(new_n59_), .c(new_n60_), .O(z16));
  nand4  g92(.a(new_n44_), .b(x02), .c(new_n59_), .d(new_n58_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z17));
endmodule


