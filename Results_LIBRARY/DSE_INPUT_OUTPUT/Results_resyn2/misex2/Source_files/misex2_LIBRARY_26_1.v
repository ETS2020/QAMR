// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n125_, new_n126_, new_n128_, new_n129_;
  inv1   g00(.a(x02), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  nor3   g07(.a(new_n50_), .b(new_n46_), .c(x09), .O(z00));
  nand3  g08(.a(new_n45_), .b(x09), .c(new_n44_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n50_), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n54_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n52_), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x01), .O(new_n58_));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x18), .c(new_n47_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nand2  g19(.a(x10), .b(x02), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  inv1   g21(.a(x11), .O(new_n65_));
  nor2   g22(.a(new_n65_), .b(x09), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n64_), .c(x12), .d(x01), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n62_), .O(z03));
  inv1   g25(.a(x09), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(x10), .c(new_n69_), .O(new_n71_));
  nand2  g28(.a(x01), .b(x00), .O(new_n72_));
  nor4   g29(.a(new_n72_), .b(new_n71_), .c(x11), .d(new_n44_), .O(z04));
  nor3   g30(.a(new_n72_), .b(new_n63_), .c(new_n69_), .O(z05));
  nand2  g31(.a(new_n66_), .b(new_n64_), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x00), .c(new_n58_), .O(z06));
  nand2  g33(.a(x01), .b(new_n57_), .O(new_n77_));
  nand4  g34(.a(new_n70_), .b(x11), .c(x10), .d(new_n69_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n77_), .O(z07));
  inv1   g38(.a(x08), .O(new_n82_));
  nand4  g39(.a(x19), .b(new_n48_), .c(x17), .d(new_n82_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nor2   g41(.a(x03), .b(new_n44_), .O(new_n85_));
  inv1   g42(.a(x05), .O(new_n86_));
  inv1   g43(.a(x06), .O(new_n87_));
  inv1   g44(.a(x07), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x04), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n85_), .c(new_n84_), .O(new_n91_));
  aoi21  g48(.a(new_n91_), .b(new_n58_), .c(x00), .O(z08));
  nand2  g49(.a(new_n45_), .b(x02), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(z17));
  nor2   g51(.a(x13), .b(new_n70_), .O(new_n95_));
  nor2   g52(.a(x14), .b(x11), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(z17), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(x20), .d(new_n99_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n97_), .O(z09));
  nor2   g62(.a(new_n99_), .b(new_n98_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n101_), .c(new_n100_), .d(x20), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n97_), .O(z10));
  inv1   g65(.a(x14), .O(new_n109_));
  nand4  g66(.a(x15), .b(new_n109_), .c(new_n65_), .d(x02), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n103_), .c(new_n95_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n58_), .c(x00), .O(z11));
  nand3  g70(.a(new_n63_), .b(x01), .c(x00), .O(new_n114_));
  nand2  g71(.a(new_n59_), .b(new_n47_), .O(new_n115_));
  inv1   g72(.a(x23), .O(new_n116_));
  nand2  g73(.a(new_n49_), .b(new_n44_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n115_), .c(new_n45_), .O(new_n119_));
  inv1   g76(.a(x24), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x09), .O(new_n121_));
  aoi21  g78(.a(new_n119_), .b(new_n114_), .c(new_n121_), .O(z12));
  nand2  g79(.a(new_n45_), .b(x17), .O(new_n123_));
  nor2   g80(.a(new_n123_), .b(new_n117_), .O(z13));
  inv1   g81(.a(new_n55_), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(new_n69_), .c(new_n44_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n58_), .c(x00), .O(z14));
  oai21  g84(.a(new_n54_), .b(new_n57_), .c(x01), .O(new_n128_));
  oai21  g85(.a(x19), .b(x00), .c(new_n44_), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(new_n128_), .O(z15));
  inv1   g87(.a(new_n77_), .O(z16));
endmodule


