// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n132_, new_n133_, new_n134_, new_n136_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(new_n46_), .b(x02), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand2  g10(.a(new_n47_), .b(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n52_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g15(.a(x12), .O(new_n59_));
  nand3  g16(.a(x10), .b(x02), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(x11), .c(new_n44_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n59_), .c(x04), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x01), .O(new_n64_));
  nand4  g21(.a(new_n47_), .b(new_n51_), .c(x18), .d(new_n49_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n64_), .O(z03));
  inv1   g23(.a(x01), .O(new_n67_));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x00), .O(new_n69_));
  inv1   g26(.a(x02), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g28(.a(x12), .b(new_n56_), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(new_n68_), .d(new_n44_), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x04), .c(new_n67_), .O(z04));
  nand2  g31(.a(new_n61_), .b(x09), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x04), .c(new_n67_), .O(z05));
  aoi21  g33(.a(new_n62_), .b(x04), .c(new_n67_), .O(z06));
  nand3  g34(.a(new_n72_), .b(x11), .c(new_n44_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n71_), .O(new_n80_));
  nor2   g37(.a(x04), .b(new_n67_), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n80_), .O(z07));
  inv1   g40(.a(x03), .O(new_n84_));
  nor2   g41(.a(x06), .b(x05), .O(new_n85_));
  nor2   g42(.a(x08), .b(x07), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(x04), .d(new_n84_), .O(new_n87_));
  nand2  g44(.a(new_n45_), .b(x02), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(z17));
  nand4  g46(.a(z17), .b(x19), .c(new_n50_), .d(x17), .O(new_n90_));
  oai21  g47(.a(new_n90_), .b(new_n87_), .c(new_n82_), .O(z08));
  inv1   g48(.a(x20), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n51_), .c(x18), .d(x01), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  nand2  g51(.a(x02), .b(new_n67_), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n68_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  inv1   g55(.a(x14), .O(new_n99_));
  inv1   g56(.a(x15), .O(new_n100_));
  inv1   g57(.a(x16), .O(new_n101_));
  nand4  g58(.a(x20), .b(new_n101_), .c(new_n100_), .d(new_n99_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(new_n98_), .c(new_n94_), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  inv1   g62(.a(x22), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n69_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n104_), .c(new_n82_), .O(z09));
  nor2   g65(.a(new_n81_), .b(new_n69_), .O(new_n109_));
  nand2  g66(.a(x15), .b(new_n99_), .O(new_n110_));
  nand4  g67(.a(new_n106_), .b(new_n105_), .c(x20), .d(x16), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n98_), .O(new_n113_));
  nor2   g70(.a(new_n93_), .b(new_n105_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(x22), .c(new_n81_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n113_), .c(new_n109_), .O(z10));
  nand2  g73(.a(new_n106_), .b(new_n69_), .O(new_n117_));
  nand3  g74(.a(new_n105_), .b(x20), .c(new_n101_), .O(new_n118_));
  nor2   g75(.a(new_n118_), .b(new_n110_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n98_), .c(new_n114_), .O(new_n120_));
  oai21  g77(.a(new_n120_), .b(new_n117_), .c(new_n82_), .O(z11));
  nand2  g78(.a(x01), .b(x00), .O(new_n122_));
  aoi21  g79(.a(x10), .b(x02), .c(new_n122_), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  nand3  g81(.a(new_n51_), .b(x17), .c(new_n70_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n124_), .c(new_n46_), .O(new_n126_));
  nor2   g83(.a(x24), .b(new_n44_), .O(new_n127_));
  oai21  g84(.a(new_n126_), .b(new_n123_), .c(new_n127_), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n82_), .O(z12));
  nor2   g86(.a(new_n125_), .b(new_n46_), .O(z13));
  nor2   g87(.a(new_n57_), .b(new_n48_), .O(z14));
  nor2   g88(.a(new_n51_), .b(x01), .O(new_n132_));
  oai21  g89(.a(new_n132_), .b(new_n109_), .c(new_n70_), .O(new_n133_));
  nand2  g90(.a(new_n56_), .b(x04), .O(new_n134_));
  oai21  g91(.a(new_n134_), .b(new_n122_), .c(new_n133_), .O(z15));
  nand3  g92(.a(x04), .b(x01), .c(new_n69_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z16));
endmodule


