// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n126_, new_n128_, new_n129_;
  inv1   g00(.a(x19), .O(new_n44_));
  inv1   g01(.a(x21), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(x16), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nor3   g06(.a(x02), .b(x01), .c(x00), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n48_), .O(z00));
  nand2  g09(.a(new_n50_), .b(x09), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n48_), .O(z01));
  nand3  g11(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n55_));
  nor3   g12(.a(new_n53_), .b(new_n55_), .c(x10), .O(z02));
  inv1   g13(.a(x16), .O(new_n57_));
  nor2   g14(.a(x21), .b(new_n57_), .O(new_n58_));
  inv1   g15(.a(x17), .O(new_n59_));
  nand4  g16(.a(new_n50_), .b(new_n44_), .c(x18), .d(new_n59_), .O(new_n60_));
  inv1   g17(.a(x10), .O(new_n61_));
  inv1   g18(.a(x11), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g20(.a(x02), .O(new_n64_));
  nor2   g21(.a(x09), .b(new_n64_), .O(new_n65_));
  inv1   g22(.a(x00), .O(new_n66_));
  inv1   g23(.a(x01), .O(new_n67_));
  nor2   g24(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n65_), .c(new_n63_), .d(x12), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(new_n60_), .c(new_n58_), .O(z03));
  and2   g27(.a(new_n68_), .b(new_n46_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(new_n73_), .b(new_n62_), .c(x10), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n72_), .O(z04));
  nand4  g32(.a(new_n71_), .b(x10), .c(x09), .d(x02), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z05));
  nand3  g34(.a(new_n71_), .b(new_n65_), .c(new_n63_), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(z06));
  nor2   g36(.a(x12), .b(x09), .O(new_n80_));
  aoi21  g37(.a(new_n80_), .b(new_n63_), .c(new_n67_), .O(new_n81_));
  nand2  g38(.a(x02), .b(x00), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(new_n81_), .c(new_n46_), .O(z07));
  nor2   g40(.a(x01), .b(x00), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(new_n46_), .c(x02), .O(new_n85_));
  inv1   g42(.a(x18), .O(new_n86_));
  nand3  g43(.a(x19), .b(new_n86_), .c(x17), .O(new_n87_));
  inv1   g44(.a(x03), .O(new_n88_));
  nor2   g45(.a(x06), .b(x05), .O(new_n89_));
  nor2   g46(.a(x08), .b(x07), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n89_), .c(x04), .d(new_n88_), .O(new_n91_));
  nor3   g48(.a(new_n91_), .b(new_n87_), .c(new_n85_), .O(z08));
  inv1   g49(.a(x22), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n94_), .c(x12), .d(new_n62_), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  nand4  g54(.a(x20), .b(new_n97_), .c(x02), .d(new_n67_), .O(new_n98_));
  inv1   g55(.a(x20), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n44_), .c(x18), .d(x01), .O(new_n100_));
  oai21  g57(.a(new_n98_), .b(new_n96_), .c(new_n100_), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n93_), .c(new_n66_), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n57_), .c(x21), .O(z09));
  inv1   g60(.a(new_n100_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(x22), .c(x21), .d(new_n66_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(z10));
  nand2  g63(.a(new_n93_), .b(new_n66_), .O(new_n107_));
  nand2  g64(.a(new_n104_), .b(x21), .O(new_n108_));
  inv1   g65(.a(new_n96_), .O(new_n109_));
  nand4  g66(.a(new_n45_), .b(x20), .c(new_n57_), .d(x15), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n109_), .c(x02), .d(new_n67_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n108_), .c(new_n107_), .O(z11));
  nand2  g70(.a(x10), .b(x02), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(x01), .c(x00), .O(new_n115_));
  nor3   g72(.a(x19), .b(x17), .c(x02), .O(new_n116_));
  nor2   g73(.a(x19), .b(x02), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(x23), .c(new_n84_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(new_n119_));
  nor2   g76(.a(x24), .b(new_n49_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n46_), .O(z12));
  nor2   g79(.a(x02), .b(x01), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n66_), .O(new_n124_));
  nor4   g81(.a(new_n124_), .b(new_n58_), .c(x19), .d(new_n59_), .O(z13));
  nand3  g82(.a(new_n47_), .b(new_n44_), .c(new_n61_), .O(new_n126_));
  oai21  g83(.a(new_n126_), .b(new_n51_), .c(new_n46_), .O(z14));
  aoi21  g84(.a(new_n61_), .b(x01), .c(new_n64_), .O(new_n128_));
  aoi21  g85(.a(new_n123_), .b(x19), .c(new_n58_), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(new_n66_), .c(new_n129_), .O(z15));
  oai21  g87(.a(new_n67_), .b(x00), .c(new_n46_), .O(z16));
  inv1   g88(.a(new_n85_), .O(z17));
endmodule


