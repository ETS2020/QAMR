// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n122_, new_n124_, new_n125_;
  nor3   g00(.a(x19), .b(x18), .c(x17), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x10), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor3   g05(.a(new_n48_), .b(new_n45_), .c(x09), .O(z00));
  inv1   g06(.a(x01), .O(new_n50_));
  nand4  g07(.a(new_n44_), .b(x10), .c(x09), .d(new_n46_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n50_), .c(x00), .O(z01));
  inv1   g09(.a(x09), .O(new_n53_));
  inv1   g10(.a(x10), .O(new_n54_));
  nand2  g11(.a(new_n44_), .b(new_n54_), .O(new_n55_));
  nor3   g12(.a(new_n55_), .b(new_n48_), .c(new_n53_), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x17), .O(new_n58_));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x18), .c(new_n58_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n50_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nand2  g19(.a(x10), .b(x02), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  inv1   g21(.a(x11), .O(new_n65_));
  nor2   g22(.a(new_n65_), .b(x09), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n64_), .c(x12), .d(x01), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n62_), .O(z03));
  nand3  g25(.a(new_n65_), .b(new_n53_), .c(x02), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(x10), .c(x01), .d(x00), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n69_), .O(z04));
  nand2  g29(.a(new_n64_), .b(x09), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x00), .c(new_n50_), .O(z05));
  nand2  g31(.a(new_n66_), .b(new_n64_), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x00), .c(new_n50_), .O(z06));
  nand3  g33(.a(new_n66_), .b(new_n70_), .c(x10), .O(new_n77_));
  nand2  g34(.a(x02), .b(x00), .O(new_n78_));
  aoi21  g35(.a(new_n77_), .b(x01), .c(new_n78_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand2  g40(.a(new_n47_), .b(x02), .O(new_n84_));
  inv1   g41(.a(x18), .O(new_n85_));
  nand3  g42(.a(x19), .b(new_n85_), .c(x17), .O(new_n86_));
  nor3   g43(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(z08));
  inv1   g44(.a(x15), .O(new_n88_));
  inv1   g45(.a(new_n84_), .O(z17));
  inv1   g46(.a(x13), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(x12), .O(new_n91_));
  inv1   g48(.a(x14), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(new_n65_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(x20), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n95_), .c(z17), .d(new_n88_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(z09));
  nand3  g58(.a(x16), .b(x15), .c(new_n92_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n90_), .b(x12), .c(new_n65_), .d(x02), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n103_), .c(new_n99_), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n50_), .c(x00), .O(z10));
  nand4  g64(.a(new_n99_), .b(new_n95_), .c(z17), .d(x15), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(z11));
  nand3  g66(.a(new_n63_), .b(x01), .c(x00), .O(new_n110_));
  nand2  g67(.a(new_n59_), .b(new_n58_), .O(new_n111_));
  inv1   g68(.a(x23), .O(new_n112_));
  inv1   g69(.a(x19), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n46_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n111_), .c(new_n47_), .O(new_n116_));
  inv1   g73(.a(x24), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x09), .O(new_n118_));
  aoi21  g75(.a(new_n116_), .b(new_n110_), .c(new_n118_), .O(z12));
  nand2  g76(.a(new_n47_), .b(x17), .O(new_n120_));
  nor2   g77(.a(new_n120_), .b(new_n114_), .O(z13));
  nand4  g78(.a(new_n44_), .b(new_n54_), .c(new_n53_), .d(new_n46_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n50_), .c(x00), .O(z14));
  aoi21  g80(.a(new_n54_), .b(x01), .c(new_n46_), .O(new_n124_));
  nand3  g81(.a(x19), .b(new_n46_), .c(new_n50_), .O(new_n125_));
  oai21  g82(.a(new_n124_), .b(new_n57_), .c(new_n125_), .O(z15));
  nor2   g83(.a(new_n50_), .b(x00), .O(z16));
endmodule


