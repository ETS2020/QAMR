// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n128_, new_n130_, new_n131_,
    new_n134_;
  inv1   g00(.a(x23), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(x10), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nor3   g05(.a(x17), .b(x02), .c(x00), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g08(.a(x17), .O(new_n52_));
  inv1   g09(.a(x00), .O(new_n53_));
  inv1   g10(.a(x02), .O(new_n54_));
  nand3  g11(.a(x09), .b(new_n54_), .c(new_n53_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand3  g13(.a(new_n56_), .b(new_n48_), .c(new_n52_), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand3  g16(.a(new_n46_), .b(new_n52_), .c(new_n59_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n56_), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g20(.a(x18), .O(new_n64_));
  nor2   g21(.a(x19), .b(new_n64_), .O(new_n65_));
  aoi21  g22(.a(new_n65_), .b(new_n49_), .c(x23), .O(new_n66_));
  inv1   g23(.a(x11), .O(new_n67_));
  nand4  g24(.a(new_n45_), .b(x02), .c(x01), .d(x00), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(new_n67_), .c(new_n59_), .O(z06));
  nand2  g26(.a(z06), .b(x12), .O(new_n70_));
  oai21  g27(.a(new_n66_), .b(x01), .c(new_n70_), .O(z03));
  nor4   g28(.a(new_n68_), .b(x12), .c(x11), .d(new_n59_), .O(z04));
  inv1   g29(.a(x01), .O(new_n73_));
  nand2  g30(.a(x23), .b(new_n73_), .O(new_n74_));
  nand2  g31(.a(x01), .b(x00), .O(new_n75_));
  nor2   g32(.a(new_n59_), .b(new_n54_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x09), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(z05));
  nand2  g35(.a(x02), .b(x00), .O(new_n79_));
  nand2  g36(.a(new_n44_), .b(new_n73_), .O(new_n80_));
  nor3   g37(.a(x12), .b(new_n67_), .c(new_n59_), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(new_n45_), .c(x01), .O(new_n82_));
  aoi21  g39(.a(new_n82_), .b(new_n80_), .c(new_n79_), .O(z07));
  inv1   g40(.a(x05), .O(new_n84_));
  nor2   g41(.a(x01), .b(x00), .O(new_n85_));
  nor2   g42(.a(x03), .b(new_n54_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(x04), .O(new_n87_));
  inv1   g44(.a(x19), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(x18), .O(new_n89_));
  nor2   g46(.a(x07), .b(x06), .O(new_n90_));
  nor2   g47(.a(new_n52_), .b(x08), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(new_n44_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n87_), .O(z08));
  inv1   g50(.a(x20), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n88_), .c(x18), .d(x01), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand2  g53(.a(x02), .b(new_n73_), .O(new_n97_));
  inv1   g54(.a(x13), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(x12), .c(new_n67_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  inv1   g57(.a(x14), .O(new_n101_));
  inv1   g58(.a(x15), .O(new_n102_));
  inv1   g59(.a(x16), .O(new_n103_));
  nand4  g60(.a(x20), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n100_), .c(new_n96_), .O(new_n106_));
  inv1   g63(.a(x21), .O(new_n107_));
  inv1   g64(.a(x22), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n107_), .c(new_n53_), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(new_n106_), .c(new_n74_), .O(z09));
  nor2   g67(.a(new_n95_), .b(new_n107_), .O(new_n111_));
  nand4  g68(.a(new_n107_), .b(x20), .c(x15), .d(new_n101_), .O(new_n112_));
  nand2  g69(.a(new_n108_), .b(x16), .O(new_n113_));
  nor2   g70(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  aoi22  g71(.a(new_n114_), .b(new_n100_), .c(new_n111_), .d(x22), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(x00), .c(new_n74_), .O(z10));
  nand2  g73(.a(new_n108_), .b(new_n53_), .O(new_n117_));
  inv1   g74(.a(new_n111_), .O(new_n118_));
  inv1   g75(.a(new_n112_), .O(new_n119_));
  nand4  g76(.a(new_n119_), .b(new_n100_), .c(new_n44_), .d(new_n103_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n118_), .c(new_n117_), .O(z11));
  nand4  g78(.a(new_n85_), .b(new_n88_), .c(x17), .d(new_n54_), .O(new_n122_));
  oai21  g79(.a(new_n76_), .b(new_n75_), .c(new_n122_), .O(new_n123_));
  nor2   g80(.a(x24), .b(new_n45_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n74_), .O(z12));
  nor2   g83(.a(new_n122_), .b(x23), .O(z13));
  nand2  g84(.a(new_n85_), .b(new_n54_), .O(new_n128_));
  nor4   g85(.a(new_n128_), .b(new_n60_), .c(x23), .d(x09), .O(z14));
  aoi21  g86(.a(new_n59_), .b(x01), .c(new_n54_), .O(new_n130_));
  aoi21  g87(.a(x19), .b(new_n54_), .c(x23), .O(new_n131_));
  oai22  g88(.a(new_n131_), .b(x01), .c(new_n130_), .d(new_n53_), .O(z15));
  nor2   g89(.a(new_n73_), .b(x00), .O(z16));
  nand2  g90(.a(x02), .b(new_n53_), .O(new_n134_));
  aoi21  g91(.a(new_n134_), .b(new_n44_), .c(x01), .O(z17));
endmodule


