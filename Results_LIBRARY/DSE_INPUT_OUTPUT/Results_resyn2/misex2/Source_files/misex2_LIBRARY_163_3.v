// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_;
  nand2  g000(.a(x23), .b(x18), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  inv1   g002(.a(x18), .O(new_n46_));
  inv1   g003(.a(x19), .O(new_n47_));
  nand4  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g005(.a(x02), .O(new_n49_));
  inv1   g006(.a(x09), .O(new_n50_));
  nor2   g007(.a(x01), .b(x00), .O(new_n51_));
  nand3  g008(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g009(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g010(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  oai21  g011(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g012(.a(x10), .O(new_n56_));
  nand4  g013(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n56_), .O(new_n57_));
  nor2   g014(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g015(.a(x23), .O(new_n59_));
  nor2   g016(.a(x19), .b(x02), .O(new_n60_));
  nand2  g017(.a(new_n60_), .b(new_n51_), .O(new_n61_));
  inv1   g018(.a(new_n61_), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n59_), .c(x18), .d(new_n45_), .O(new_n63_));
  inv1   g020(.a(x00), .O(new_n64_));
  inv1   g021(.a(x01), .O(new_n65_));
  nor2   g022(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(x02), .O(new_n67_));
  inv1   g024(.a(new_n67_), .O(new_n68_));
  nor2   g025(.a(new_n56_), .b(x09), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(new_n68_), .c(x11), .O(new_n70_));
  nand2  g027(.a(new_n44_), .b(x12), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n70_), .c(new_n63_), .O(z03));
  inv1   g029(.a(x12), .O(new_n73_));
  nand2  g030(.a(new_n69_), .b(new_n73_), .O(new_n74_));
  inv1   g031(.a(x11), .O(new_n75_));
  nand2  g032(.a(new_n68_), .b(new_n75_), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n74_), .c(new_n44_), .O(z04));
  nand2  g034(.a(x10), .b(x02), .O(new_n78_));
  nand2  g035(.a(new_n66_), .b(x09), .O(new_n79_));
  oai21  g036(.a(new_n79_), .b(new_n78_), .c(new_n44_), .O(z05));
  nand2  g037(.a(new_n70_), .b(new_n44_), .O(z06));
  oai21  g038(.a(new_n74_), .b(new_n75_), .c(x01), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n44_), .O(z07));
  inv1   g041(.a(x03), .O(new_n85_));
  nor2   g042(.a(x06), .b(x05), .O(new_n86_));
  nor2   g043(.a(x08), .b(x07), .O(new_n87_));
  nand4  g044(.a(new_n87_), .b(new_n86_), .c(x04), .d(new_n85_), .O(new_n88_));
  nor2   g045(.a(new_n49_), .b(x01), .O(new_n89_));
  nand2  g046(.a(new_n89_), .b(new_n64_), .O(new_n90_));
  nand3  g047(.a(x19), .b(new_n46_), .c(x17), .O(new_n91_));
  nor3   g048(.a(new_n91_), .b(new_n90_), .c(new_n88_), .O(z08));
  inv1   g049(.a(x20), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n47_), .c(x18), .d(x01), .O(new_n94_));
  inv1   g051(.a(new_n94_), .O(new_n95_));
  nand2  g052(.a(x02), .b(new_n65_), .O(new_n96_));
  inv1   g053(.a(x13), .O(new_n97_));
  nand3  g054(.a(new_n97_), .b(x12), .c(new_n75_), .O(new_n98_));
  nor2   g055(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  inv1   g056(.a(x14), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nand4  g059(.a(x20), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g060(.a(new_n103_), .O(new_n104_));
  aoi21  g061(.a(new_n104_), .b(new_n99_), .c(new_n95_), .O(new_n105_));
  inv1   g062(.a(x21), .O(new_n106_));
  inv1   g063(.a(x22), .O(new_n107_));
  nand3  g064(.a(new_n107_), .b(new_n106_), .c(new_n64_), .O(new_n108_));
  oai21  g065(.a(new_n108_), .b(new_n105_), .c(new_n44_), .O(z09));
  nor2   g066(.a(new_n73_), .b(x11), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(new_n89_), .c(new_n97_), .O(new_n111_));
  and2   g068(.a(x18), .b(x01), .O(new_n112_));
  nor2   g069(.a(x20), .b(x19), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(new_n112_), .c(x22), .d(x21), .O(new_n114_));
  nor2   g071(.a(new_n101_), .b(x14), .O(new_n115_));
  nor2   g072(.a(x21), .b(new_n93_), .O(new_n116_));
  nor2   g073(.a(x22), .b(new_n102_), .O(new_n117_));
  nand3  g074(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(new_n118_));
  oai21  g075(.a(new_n118_), .b(new_n111_), .c(new_n114_), .O(new_n119_));
  nand2  g076(.a(new_n119_), .b(new_n64_), .O(new_n120_));
  nand2  g077(.a(new_n120_), .b(new_n44_), .O(z10));
  nand2  g078(.a(new_n107_), .b(new_n64_), .O(new_n122_));
  nor2   g079(.a(new_n94_), .b(new_n106_), .O(new_n123_));
  nand4  g080(.a(new_n106_), .b(x20), .c(x15), .d(new_n100_), .O(new_n124_));
  nor2   g081(.a(new_n124_), .b(x16), .O(new_n125_));
  aoi21  g082(.a(new_n125_), .b(new_n99_), .c(new_n123_), .O(new_n126_));
  oai21  g083(.a(new_n126_), .b(new_n122_), .c(new_n44_), .O(z11));
  nand2  g084(.a(new_n60_), .b(new_n45_), .O(new_n128_));
  oai21  g085(.a(x19), .b(x02), .c(new_n59_), .O(new_n129_));
  nand4  g086(.a(new_n129_), .b(new_n128_), .c(new_n51_), .d(new_n44_), .O(new_n130_));
  nand3  g087(.a(new_n78_), .b(new_n66_), .c(new_n44_), .O(new_n131_));
  inv1   g088(.a(x24), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(x09), .O(new_n133_));
  aoi21  g090(.a(new_n131_), .b(new_n130_), .c(new_n133_), .O(z12));
  oai21  g091(.a(new_n61_), .b(new_n45_), .c(new_n44_), .O(z13));
  nor2   g092(.a(new_n57_), .b(new_n52_), .O(z14));
  inv1   g093(.a(new_n44_), .O(new_n137_));
  oai21  g094(.a(x10), .b(new_n65_), .c(x02), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(x00), .O(new_n139_));
  nand3  g096(.a(x19), .b(new_n49_), .c(new_n65_), .O(new_n140_));
  aoi21  g097(.a(new_n140_), .b(new_n139_), .c(new_n137_), .O(z15));
  nand3  g098(.a(new_n44_), .b(x01), .c(new_n64_), .O(new_n142_));
  inv1   g099(.a(new_n142_), .O(z16));
  nand2  g100(.a(new_n90_), .b(new_n44_), .O(z17));
endmodule


