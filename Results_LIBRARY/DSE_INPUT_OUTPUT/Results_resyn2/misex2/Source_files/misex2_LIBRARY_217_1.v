// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n123_, new_n124_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  nor3   g05(.a(new_n48_), .b(new_n44_), .c(x09), .O(z00));
  nand2  g06(.a(x10), .b(x09), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(new_n48_), .O(z01));
  inv1   g08(.a(x01), .O(new_n52_));
  nand4  g09(.a(new_n46_), .b(new_n44_), .c(x09), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n52_), .c(x00), .O(z02));
  inv1   g11(.a(x00), .O(new_n55_));
  inv1   g12(.a(x17), .O(new_n56_));
  nor2   g13(.a(x19), .b(x02), .O(new_n57_));
  nand3  g14(.a(new_n57_), .b(x18), .c(new_n56_), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n52_), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  nor2   g17(.a(x09), .b(new_n45_), .O(new_n61_));
  inv1   g18(.a(x11), .O(new_n62_));
  inv1   g19(.a(x12), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n61_), .c(x10), .d(x01), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n60_), .O(z03));
  nand4  g23(.a(new_n61_), .b(new_n63_), .c(new_n62_), .d(x10), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(x00), .c(new_n52_), .O(z04));
  nand3  g25(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nor2   g26(.a(new_n69_), .b(new_n50_), .O(z05));
  inv1   g27(.a(x09), .O(new_n71_));
  nand3  g28(.a(x11), .b(x10), .c(new_n71_), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n69_), .O(z06));
  nand2  g30(.a(x01), .b(new_n55_), .O(new_n74_));
  oai21  g31(.a(new_n72_), .b(x12), .c(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n74_), .O(z07));
  inv1   g34(.a(x08), .O(new_n78_));
  inv1   g35(.a(x18), .O(new_n79_));
  nand4  g36(.a(x19), .b(new_n79_), .c(x17), .d(new_n78_), .O(new_n80_));
  inv1   g37(.a(x05), .O(new_n81_));
  nor2   g38(.a(x07), .b(x06), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(new_n81_), .c(x04), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nand3  g41(.a(new_n47_), .b(new_n84_), .c(x02), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n83_), .c(new_n80_), .O(z08));
  inv1   g43(.a(x15), .O(new_n87_));
  inv1   g44(.a(x14), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(x02), .O(new_n89_));
  inv1   g46(.a(x16), .O(new_n90_));
  nand2  g47(.a(new_n47_), .b(new_n90_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n62_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n92_), .c(new_n87_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(z09));
  inv1   g57(.a(new_n95_), .O(new_n101_));
  inv1   g58(.a(new_n97_), .O(new_n102_));
  nand4  g59(.a(x16), .b(x15), .c(new_n88_), .d(x02), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n52_), .c(x00), .O(z10));
  nand3  g63(.a(new_n98_), .b(new_n92_), .c(x15), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(z11));
  aoi21  g65(.a(x10), .b(x02), .c(new_n52_), .O(new_n109_));
  inv1   g66(.a(x23), .O(new_n110_));
  inv1   g67(.a(x19), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n45_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  aoi21  g70(.a(new_n57_), .b(new_n56_), .c(x00), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n113_), .c(new_n109_), .O(new_n115_));
  inv1   g72(.a(x24), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(x09), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n115_), .c(new_n74_), .O(z12));
  nand2  g75(.a(new_n47_), .b(x17), .O(new_n119_));
  nor2   g76(.a(new_n119_), .b(new_n112_), .O(z13));
  nand4  g77(.a(new_n46_), .b(new_n44_), .c(new_n71_), .d(new_n45_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n52_), .c(x00), .O(z14));
  aoi21  g79(.a(new_n44_), .b(x01), .c(new_n45_), .O(new_n123_));
  nand3  g80(.a(x19), .b(new_n45_), .c(new_n52_), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n55_), .c(new_n124_), .O(z15));
  inv1   g82(.a(new_n74_), .O(z16));
  aoi21  g83(.a(new_n45_), .b(new_n52_), .c(x00), .O(z17));
endmodule


