// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n127_, new_n129_, new_n130_, new_n131_;
  nor2   g00(.a(x16), .b(x08), .O(new_n44_));
  inv1   g01(.a(new_n44_), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor2   g05(.a(x17), .b(x02), .O(new_n49_));
  inv1   g06(.a(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n45_), .O(z00));
  inv1   g10(.a(new_n46_), .O(new_n54_));
  inv1   g11(.a(x17), .O(new_n55_));
  nand3  g12(.a(new_n47_), .b(new_n45_), .c(new_n55_), .O(new_n56_));
  nand2  g13(.a(x10), .b(x09), .O(new_n57_));
  nor4   g14(.a(new_n57_), .b(new_n56_), .c(new_n54_), .d(x02), .O(z01));
  nand3  g15(.a(new_n49_), .b(new_n50_), .c(x09), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n48_), .c(new_n45_), .O(z02));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(new_n51_), .c(x12), .d(x11), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nand4  g21(.a(new_n49_), .b(new_n46_), .c(new_n64_), .d(x18), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n63_), .c(new_n45_), .O(z03));
  nor2   g23(.a(x12), .b(x11), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n62_), .c(new_n51_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n45_), .O(z04));
  oai21  g26(.a(new_n61_), .b(new_n57_), .c(new_n45_), .O(z05));
  nand2  g27(.a(new_n51_), .b(x11), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n61_), .c(new_n44_), .O(z06));
  oai21  g29(.a(new_n71_), .b(x12), .c(x01), .O(new_n73_));
  nand3  g30(.a(new_n73_), .b(x02), .c(x00), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n45_), .O(z07));
  inv1   g32(.a(x00), .O(new_n76_));
  inv1   g33(.a(x01), .O(new_n77_));
  nand3  g34(.a(x02), .b(new_n77_), .c(new_n76_), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(new_n79_));
  nor2   g36(.a(new_n55_), .b(x07), .O(new_n80_));
  nor2   g37(.a(new_n64_), .b(x18), .O(new_n81_));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x04), .c(new_n82_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n81_), .c(new_n80_), .d(new_n79_), .O(new_n86_));
  aoi21  g43(.a(new_n86_), .b(x16), .c(x08), .O(z08));
  inv1   g44(.a(x20), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n64_), .c(x18), .d(x01), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand2  g47(.a(x02), .b(new_n77_), .O(new_n91_));
  inv1   g48(.a(x11), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(x12), .c(new_n92_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  inv1   g52(.a(x14), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  inv1   g54(.a(x16), .O(new_n98_));
  nand4  g55(.a(x20), .b(new_n98_), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  aoi21  g57(.a(new_n100_), .b(new_n95_), .c(new_n90_), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n76_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n101_), .c(new_n45_), .O(z09));
  nor2   g62(.a(new_n89_), .b(new_n102_), .O(new_n106_));
  nand4  g63(.a(new_n102_), .b(x20), .c(x15), .d(new_n96_), .O(new_n107_));
  nand2  g64(.a(new_n103_), .b(x16), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  aoi22  g66(.a(new_n109_), .b(new_n95_), .c(new_n106_), .d(x22), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n45_), .O(z10));
  nand2  g68(.a(new_n103_), .b(new_n76_), .O(new_n112_));
  nor2   g69(.a(new_n107_), .b(x16), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n95_), .c(new_n106_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n112_), .c(new_n45_), .O(z11));
  nand2  g72(.a(x01), .b(x00), .O(new_n116_));
  aoi21  g73(.a(x10), .b(x02), .c(new_n116_), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  inv1   g75(.a(x02), .O(new_n119_));
  nand3  g76(.a(new_n64_), .b(x17), .c(new_n119_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n118_), .c(new_n54_), .O(new_n121_));
  inv1   g78(.a(x09), .O(new_n122_));
  nor2   g79(.a(x24), .b(new_n122_), .O(new_n123_));
  oai21  g80(.a(new_n121_), .b(new_n117_), .c(new_n123_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n45_), .O(z12));
  oai21  g82(.a(new_n120_), .b(new_n54_), .c(new_n45_), .O(z13));
  nand4  g83(.a(new_n46_), .b(new_n50_), .c(new_n122_), .d(new_n119_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(new_n56_), .O(z14));
  aoi21  g85(.a(new_n50_), .b(x01), .c(new_n119_), .O(new_n129_));
  nor2   g86(.a(x02), .b(x01), .O(new_n130_));
  aoi21  g87(.a(new_n130_), .b(x19), .c(new_n44_), .O(new_n131_));
  oai21  g88(.a(new_n129_), .b(new_n76_), .c(new_n131_), .O(z15));
  nor3   g89(.a(new_n44_), .b(new_n77_), .c(x00), .O(z16));
  nand2  g90(.a(new_n78_), .b(new_n45_), .O(z17));
endmodule


