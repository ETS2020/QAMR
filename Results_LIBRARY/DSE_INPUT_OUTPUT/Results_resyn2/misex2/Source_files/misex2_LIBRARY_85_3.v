// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n127_, new_n128_, new_n129_;
  nand2  g00(.a(x09), .b(x01), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x01), .O(new_n46_));
  inv1   g03(.a(x02), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  or2    g05(.a(new_n48_), .b(x17), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x19), .b(x18), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(x10), .c(new_n50_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n49_), .c(new_n44_), .O(z00));
  inv1   g10(.a(x17), .O(new_n54_));
  inv1   g11(.a(x18), .O(new_n55_));
  inv1   g12(.a(x19), .O(new_n56_));
  nand3  g13(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g14(.a(x10), .b(x09), .O(new_n58_));
  nor3   g15(.a(new_n58_), .b(new_n57_), .c(new_n48_), .O(z01));
  nor2   g16(.a(x17), .b(x10), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n51_), .c(new_n47_), .d(new_n45_), .O(new_n61_));
  aoi21  g18(.a(new_n61_), .b(new_n46_), .c(new_n50_), .O(z02));
  nand3  g19(.a(x10), .b(x02), .c(x00), .O(new_n63_));
  nand2  g20(.a(x12), .b(x11), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n63_), .c(new_n50_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(x01), .O(new_n66_));
  nand2  g23(.a(new_n56_), .b(x18), .O(new_n67_));
  oai21  g24(.a(new_n67_), .b(new_n49_), .c(new_n66_), .O(z03));
  inv1   g25(.a(new_n63_), .O(new_n69_));
  nor2   g26(.a(x09), .b(new_n46_), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(x12), .c(x11), .O(z04));
  inv1   g29(.a(new_n44_), .O(z05));
  inv1   g30(.a(x11), .O(new_n74_));
  nor2   g31(.a(new_n71_), .b(new_n74_), .O(z06));
  nand2  g32(.a(x02), .b(x00), .O(new_n76_));
  inv1   g33(.a(x12), .O(new_n77_));
  nand4  g34(.a(new_n77_), .b(x11), .c(x10), .d(new_n50_), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(x01), .c(new_n76_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand3  g40(.a(x02), .b(new_n46_), .c(new_n45_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(x19), .c(new_n55_), .d(x17), .O(new_n86_));
  oai21  g43(.a(new_n86_), .b(new_n83_), .c(new_n44_), .O(z08));
  inv1   g44(.a(x20), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n56_), .c(x18), .d(x01), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand2  g47(.a(x02), .b(new_n46_), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n74_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nand4  g54(.a(x20), .b(new_n97_), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  aoi21  g56(.a(new_n99_), .b(new_n94_), .c(new_n90_), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n45_), .O(new_n103_));
  oai21  g60(.a(new_n103_), .b(new_n100_), .c(new_n44_), .O(z09));
  nand3  g61(.a(new_n88_), .b(new_n56_), .c(x18), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n70_), .c(x22), .d(x21), .O(new_n107_));
  nor2   g64(.a(new_n96_), .b(x14), .O(new_n108_));
  nand4  g65(.a(new_n102_), .b(new_n101_), .c(x20), .d(x16), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n108_), .c(new_n94_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n107_), .c(x00), .O(z10));
  nand2  g69(.a(new_n102_), .b(new_n45_), .O(new_n113_));
  nand3  g70(.a(new_n106_), .b(new_n70_), .c(x21), .O(new_n114_));
  nor2   g71(.a(new_n88_), .b(x16), .O(new_n115_));
  nand4  g72(.a(new_n108_), .b(new_n115_), .c(new_n94_), .d(new_n101_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(z11));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n56_), .b(x17), .c(new_n47_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nor2   g77(.a(x24), .b(x00), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n46_), .c(new_n50_), .O(z12));
  nand2  g80(.a(new_n46_), .b(new_n45_), .O(new_n124_));
  oai21  g81(.a(new_n119_), .b(new_n124_), .c(new_n44_), .O(z13));
  nor4   g82(.a(new_n57_), .b(new_n48_), .c(x10), .d(x09), .O(z14));
  oai21  g83(.a(x10), .b(new_n46_), .c(x02), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x00), .O(new_n128_));
  nand3  g85(.a(x19), .b(new_n47_), .c(new_n46_), .O(new_n129_));
  nand3  g86(.a(new_n129_), .b(new_n128_), .c(new_n44_), .O(z15));
  aoi21  g87(.a(new_n50_), .b(x00), .c(new_n46_), .O(z16));
  nand2  g88(.a(new_n84_), .b(new_n44_), .O(z17));
endmodule


