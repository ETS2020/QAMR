// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n126_, new_n127_, new_n128_, new_n130_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x01), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n49_), .O(z00));
  nand2  g11(.a(new_n48_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z01));
  inv1   g13(.a(x16), .O(new_n57_));
  nand2  g14(.a(new_n57_), .b(x01), .O(new_n58_));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n59_), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n55_), .c(new_n58_), .O(z02));
  inv1   g18(.a(x11), .O(new_n62_));
  nor2   g19(.a(new_n57_), .b(new_n62_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nor3   g21(.a(new_n64_), .b(new_n59_), .c(x09), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n63_), .c(x12), .O(new_n66_));
  nor2   g23(.a(x19), .b(new_n51_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n48_), .c(new_n50_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n66_), .O(z03));
  inv1   g26(.a(x12), .O(new_n70_));
  nand3  g27(.a(x16), .b(new_n70_), .c(new_n62_), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(new_n72_));
  and2   g29(.a(new_n72_), .b(new_n65_), .O(z04));
  nor4   g30(.a(new_n64_), .b(new_n57_), .c(new_n59_), .d(new_n44_), .O(z05));
  and2   g31(.a(new_n65_), .b(new_n63_), .O(z06));
  nand4  g32(.a(new_n63_), .b(new_n70_), .c(x10), .d(new_n44_), .O(new_n76_));
  nand2  g33(.a(x02), .b(x00), .O(new_n77_));
  aoi21  g34(.a(new_n76_), .b(x01), .c(new_n77_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  nand3  g39(.a(x02), .b(new_n46_), .c(new_n45_), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n51_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(z08));
  inv1   g42(.a(x20), .O(new_n86_));
  nor2   g43(.a(new_n57_), .b(new_n46_), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n67_), .c(new_n86_), .O(new_n88_));
  inv1   g45(.a(x02), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x01), .O(new_n90_));
  inv1   g47(.a(x13), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n62_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nor2   g50(.a(new_n86_), .b(x16), .O(new_n94_));
  nor2   g51(.a(x15), .b(x14), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(new_n90_), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n45_), .O(new_n99_));
  aoi21  g56(.a(new_n96_), .b(new_n88_), .c(new_n99_), .O(z09));
  nand2  g57(.a(x16), .b(new_n45_), .O(new_n101_));
  inv1   g58(.a(x15), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(x14), .O(new_n103_));
  nand3  g60(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n103_), .c(new_n93_), .d(new_n90_), .O(new_n106_));
  nor2   g63(.a(new_n98_), .b(new_n46_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n67_), .c(x21), .d(new_n86_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n106_), .c(new_n101_), .O(z10));
  nand2  g66(.a(new_n98_), .b(new_n45_), .O(new_n110_));
  nand3  g67(.a(new_n97_), .b(x20), .c(new_n57_), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n103_), .c(new_n93_), .d(new_n90_), .O(new_n113_));
  nand4  g70(.a(new_n87_), .b(new_n67_), .c(x21), .d(new_n86_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n113_), .c(new_n110_), .O(z11));
  nand2  g72(.a(x01), .b(x00), .O(new_n116_));
  aoi21  g73(.a(x10), .b(x02), .c(new_n116_), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n52_), .b(x17), .c(new_n89_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n118_), .c(new_n47_), .O(new_n120_));
  nor2   g77(.a(x24), .b(new_n44_), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n117_), .c(new_n121_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n58_), .O(z12));
  oai21  g80(.a(new_n119_), .b(new_n47_), .c(new_n58_), .O(z13));
  nor2   g81(.a(new_n60_), .b(new_n49_), .O(z14));
  oai21  g82(.a(x10), .b(new_n46_), .c(x02), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x00), .O(new_n127_));
  nand3  g84(.a(x19), .b(new_n89_), .c(new_n46_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n127_), .c(new_n58_), .O(z15));
  nand3  g86(.a(x16), .b(x01), .c(new_n45_), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z16));
  inv1   g88(.a(new_n83_), .O(z17));
endmodule


