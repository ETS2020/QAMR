// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n127_, new_n128_;
  nand2  g00(.a(x16), .b(x01), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  inv1   g06(.a(x00), .O(new_n50_));
  inv1   g07(.a(x01), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x02), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand2  g12(.a(new_n53_), .b(x09), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n56_), .O(z02));
  inv1   g17(.a(x16), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n49_), .O(new_n62_));
  nand3  g19(.a(x12), .b(x02), .c(x00), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x01), .O(new_n65_));
  nand4  g22(.a(new_n53_), .b(new_n47_), .c(x18), .d(new_n45_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n65_), .O(z03));
  nand2  g24(.a(x01), .b(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(x02), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n61_), .c(x10), .d(new_n49_), .O(new_n72_));
  inv1   g29(.a(x11), .O(new_n73_));
  inv1   g30(.a(x12), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n72_), .O(z04));
  nor4   g33(.a(new_n70_), .b(x16), .c(new_n58_), .d(new_n49_), .O(z05));
  nor2   g34(.a(new_n72_), .b(new_n73_), .O(z06));
  nand2  g35(.a(x02), .b(x00), .O(new_n79_));
  inv1   g36(.a(new_n62_), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(new_n61_), .c(new_n74_), .O(new_n81_));
  aoi21  g38(.a(new_n81_), .b(x01), .c(new_n79_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nand3  g43(.a(x02), .b(new_n51_), .c(new_n50_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x19), .c(new_n46_), .d(x17), .O(new_n89_));
  oai21  g46(.a(new_n89_), .b(new_n86_), .c(new_n44_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n47_), .c(x18), .d(x01), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n51_), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n94_), .c(x12), .d(new_n73_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nor2   g54(.a(new_n91_), .b(x15), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(new_n61_), .d(new_n50_), .O(new_n102_));
  aoi21  g59(.a(new_n99_), .b(new_n92_), .c(new_n102_), .O(z09));
  inv1   g60(.a(new_n96_), .O(new_n104_));
  nor2   g61(.a(new_n92_), .b(new_n100_), .O(new_n105_));
  nand2  g62(.a(new_n100_), .b(x20), .O(new_n106_));
  nand4  g63(.a(new_n101_), .b(x16), .c(x15), .d(x02), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi22  g65(.a(new_n108_), .b(new_n104_), .c(new_n105_), .d(x22), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g67(.a(new_n101_), .b(new_n50_), .O(new_n111_));
  nand4  g68(.a(new_n100_), .b(x20), .c(new_n61_), .d(x15), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n97_), .c(new_n105_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n111_), .c(new_n44_), .O(z11));
  aoi21  g72(.a(x10), .b(x02), .c(new_n68_), .O(new_n116_));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n117_));
  inv1   g74(.a(x02), .O(new_n118_));
  nand3  g75(.a(new_n47_), .b(x17), .c(new_n118_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n117_), .c(new_n52_), .O(new_n120_));
  nor2   g77(.a(x24), .b(new_n49_), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n116_), .c(new_n121_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n44_), .O(z12));
  nor2   g80(.a(new_n119_), .b(new_n52_), .O(z13));
  nor2   g81(.a(new_n59_), .b(new_n54_), .O(z14));
  aoi21  g82(.a(new_n61_), .b(x00), .c(new_n51_), .O(z16));
  oai21  g83(.a(x19), .b(x00), .c(new_n118_), .O(new_n127_));
  nand2  g84(.a(new_n61_), .b(new_n58_), .O(new_n128_));
  oai22  g85(.a(new_n128_), .b(new_n68_), .c(new_n127_), .d(z16), .O(z15));
  nand2  g86(.a(new_n87_), .b(new_n44_), .O(z17));
endmodule


