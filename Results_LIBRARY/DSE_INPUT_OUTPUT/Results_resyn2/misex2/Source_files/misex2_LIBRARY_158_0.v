// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n120_, new_n122_, new_n123_, new_n125_;
  inv1   g00(.a(x09), .O(new_n44_));
  nand2  g01(.a(x10), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nor2   g04(.a(x19), .b(x18), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x16), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n45_), .O(z00));
  inv1   g07(.a(new_n48_), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nand3  g10(.a(new_n46_), .b(new_n53_), .c(new_n52_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand3  g12(.a(new_n55_), .b(x10), .c(x09), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(x16), .c(x01), .O(z01));
  nor3   g14(.a(new_n49_), .b(x10), .c(new_n44_), .O(z02));
  inv1   g15(.a(x12), .O(new_n59_));
  inv1   g16(.a(x16), .O(new_n60_));
  inv1   g17(.a(new_n54_), .O(new_n61_));
  inv1   g18(.a(x18), .O(new_n62_));
  nor2   g19(.a(x19), .b(new_n62_), .O(new_n63_));
  aoi21  g20(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(new_n64_));
  inv1   g21(.a(new_n45_), .O(new_n65_));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n65_), .c(x11), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n59_), .c(new_n64_), .d(x01), .O(z03));
  nor4   g26(.a(new_n66_), .b(new_n45_), .c(x12), .d(x11), .O(z04));
  inv1   g27(.a(x10), .O(new_n71_));
  nor3   g28(.a(new_n66_), .b(new_n71_), .c(new_n44_), .O(z05));
  inv1   g29(.a(x01), .O(new_n73_));
  nand2  g30(.a(new_n60_), .b(new_n73_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n68_), .O(z06));
  inv1   g32(.a(x11), .O(new_n76_));
  nor2   g33(.a(x12), .b(new_n76_), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(new_n65_), .c(new_n73_), .O(new_n78_));
  nand2  g35(.a(x02), .b(x00), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(new_n78_), .c(new_n74_), .O(z07));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nand4  g41(.a(x04), .b(new_n84_), .c(x02), .d(new_n52_), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x19), .c(new_n62_), .d(x17), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(x16), .c(x01), .O(z08));
  nor2   g45(.a(new_n73_), .b(x00), .O(z16));
  nand2  g46(.a(z16), .b(new_n63_), .O(new_n90_));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nand2  g49(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nor3   g50(.a(new_n93_), .b(new_n90_), .c(x20), .O(z09));
  nor2   g51(.a(x14), .b(x13), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x20), .c(x15), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nand4  g54(.a(x12), .b(new_n76_), .c(x02), .d(new_n73_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n93_), .O(new_n99_));
  inv1   g56(.a(x20), .O(new_n100_));
  nand2  g57(.a(x21), .b(new_n100_), .O(new_n101_));
  inv1   g58(.a(x19), .O(new_n102_));
  nand4  g59(.a(x22), .b(new_n102_), .c(x18), .d(x01), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g61(.a(new_n99_), .b(new_n97_), .c(new_n104_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(x00), .c(new_n74_), .O(z10));
  nand3  g63(.a(new_n92_), .b(x21), .c(new_n100_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n90_), .c(new_n74_), .O(z11));
  nand2  g65(.a(x10), .b(x02), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x01), .c(x00), .O(new_n110_));
  oai21  g67(.a(x19), .b(x02), .c(x23), .O(new_n111_));
  nand3  g68(.a(new_n102_), .b(x17), .c(new_n53_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand4  g70(.a(new_n113_), .b(x16), .c(new_n73_), .d(new_n52_), .O(new_n114_));
  inv1   g71(.a(x24), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x09), .O(new_n116_));
  aoi21  g73(.a(new_n114_), .b(new_n110_), .c(new_n116_), .O(z12));
  nor3   g74(.a(x19), .b(new_n46_), .c(new_n60_), .O(new_n118_));
  and2   g75(.a(new_n118_), .b(new_n47_), .O(z13));
  nand3  g76(.a(new_n55_), .b(new_n71_), .c(new_n44_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(x16), .c(x01), .O(z14));
  aoi21  g78(.a(x19), .b(new_n73_), .c(x00), .O(new_n122_));
  nand2  g79(.a(x16), .b(new_n53_), .O(new_n123_));
  oai21  g80(.a(new_n123_), .b(new_n122_), .c(new_n110_), .O(z15));
  nand2  g81(.a(x02), .b(new_n52_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(x16), .c(x01), .O(z17));
endmodule


