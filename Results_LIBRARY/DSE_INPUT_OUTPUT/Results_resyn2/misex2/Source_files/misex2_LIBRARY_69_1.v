// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n126_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x02), .b(x01), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(x10), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x00), .O(z00));
  nand4  g06(.a(new_n47_), .b(new_n46_), .c(x10), .d(x09), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x00), .O(z01));
  inv1   g08(.a(x10), .O(new_n52_));
  nand2  g09(.a(new_n47_), .b(new_n52_), .O(new_n53_));
  nor2   g10(.a(x17), .b(x00), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(new_n46_), .c(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x19), .O(new_n58_));
  nand3  g15(.a(new_n46_), .b(new_n58_), .c(x18), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n44_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand3  g18(.a(x12), .b(x11), .c(x02), .O(new_n62_));
  nand4  g19(.a(x10), .b(new_n45_), .c(x01), .d(x00), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(z03));
  inv1   g21(.a(x11), .O(new_n65_));
  inv1   g22(.a(x12), .O(new_n66_));
  nand2  g23(.a(x01), .b(x00), .O(new_n67_));
  nand2  g24(.a(x10), .b(x02), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(new_n67_), .c(x09), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(new_n66_), .c(new_n65_), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(z04));
  nor3   g28(.a(new_n68_), .b(new_n67_), .c(new_n45_), .O(z05));
  nand2  g29(.a(new_n69_), .b(x11), .O(new_n73_));
  inv1   g30(.a(new_n73_), .O(z06));
  nand2  g31(.a(x17), .b(new_n57_), .O(new_n75_));
  nand4  g32(.a(new_n66_), .b(x11), .c(x10), .d(new_n45_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n75_), .O(z07));
  inv1   g36(.a(new_n75_), .O(z08));
  inv1   g37(.a(x20), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n58_), .c(x18), .d(x01), .O(new_n82_));
  inv1   g39(.a(x01), .O(new_n83_));
  nand2  g40(.a(x02), .b(new_n83_), .O(new_n84_));
  inv1   g41(.a(x13), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(x12), .c(new_n65_), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nor2   g44(.a(new_n81_), .b(x16), .O(new_n88_));
  nor2   g45(.a(x15), .b(x14), .O(new_n89_));
  nand3  g46(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nand3  g49(.a(new_n54_), .b(new_n92_), .c(new_n91_), .O(new_n93_));
  aoi21  g50(.a(new_n90_), .b(new_n82_), .c(new_n93_), .O(z09));
  inv1   g51(.a(x15), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(x14), .O(new_n96_));
  nand4  g53(.a(new_n92_), .b(new_n91_), .c(x20), .d(x16), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n96_), .c(new_n87_), .O(new_n99_));
  inv1   g56(.a(new_n82_), .O(new_n100_));
  nor2   g57(.a(new_n92_), .b(new_n91_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n100_), .c(x17), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n99_), .c(x00), .O(z10));
  nand2  g60(.a(new_n100_), .b(x21), .O(new_n104_));
  inv1   g61(.a(x16), .O(new_n105_));
  nand3  g62(.a(new_n91_), .b(x20), .c(new_n105_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(new_n96_), .c(new_n87_), .O(new_n108_));
  nand2  g65(.a(new_n54_), .b(new_n92_), .O(new_n109_));
  aoi21  g66(.a(new_n108_), .b(new_n104_), .c(new_n109_), .O(z11));
  inv1   g67(.a(new_n68_), .O(new_n111_));
  nor2   g68(.a(x19), .b(x02), .O(new_n112_));
  nand3  g69(.a(x23), .b(new_n83_), .c(new_n57_), .O(new_n113_));
  oai22  g70(.a(new_n113_), .b(new_n112_), .c(new_n111_), .d(new_n67_), .O(new_n114_));
  nor2   g71(.a(x24), .b(new_n45_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n75_), .O(z12));
  nand4  g74(.a(new_n47_), .b(new_n46_), .c(new_n52_), .d(new_n45_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n44_), .c(x00), .O(z14));
  inv1   g76(.a(x02), .O(new_n120_));
  aoi21  g77(.a(new_n52_), .b(x01), .c(new_n120_), .O(new_n121_));
  nand3  g78(.a(new_n46_), .b(x19), .c(new_n44_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n57_), .c(new_n122_), .O(z15));
  nand2  g80(.a(new_n54_), .b(x01), .O(new_n124_));
  inv1   g81(.a(new_n124_), .O(z16));
  nand3  g82(.a(new_n54_), .b(x02), .c(new_n83_), .O(new_n126_));
  inv1   g83(.a(new_n126_), .O(z17));
  inv1   g84(.a(new_n75_), .O(z13));
endmodule


