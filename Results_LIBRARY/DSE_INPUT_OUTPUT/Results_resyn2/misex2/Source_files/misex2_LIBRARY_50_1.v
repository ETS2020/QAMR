// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n131_,
    new_n132_, new_n133_;
  inv1   g00(.a(x12), .O(new_n44_));
  nand2  g01(.a(x21), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(x10), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nor3   g06(.a(x02), .b(x01), .c(x00), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai21  g08(.a(new_n51_), .b(new_n48_), .c(new_n45_), .O(z00));
  nand2  g09(.a(new_n50_), .b(x09), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n48_), .c(new_n45_), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand4  g12(.a(new_n47_), .b(new_n45_), .c(new_n46_), .d(new_n55_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n53_), .O(z02));
  inv1   g14(.a(x17), .O(new_n58_));
  nand4  g15(.a(new_n50_), .b(new_n46_), .c(x18), .d(new_n58_), .O(new_n59_));
  inv1   g16(.a(x00), .O(new_n60_));
  inv1   g17(.a(x01), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x10), .c(new_n49_), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(x12), .c(x11), .d(x02), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n59_), .c(new_n45_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nor2   g24(.a(x21), .b(x12), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n67_), .c(x02), .O(new_n69_));
  nor2   g26(.a(new_n69_), .b(new_n63_), .O(z04));
  nand2  g27(.a(x10), .b(x02), .O(new_n71_));
  nand2  g28(.a(new_n62_), .b(x09), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n71_), .c(new_n45_), .O(z05));
  nand3  g30(.a(new_n45_), .b(x11), .c(x02), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n63_), .O(z06));
  nand2  g32(.a(new_n45_), .b(new_n61_), .O(new_n76_));
  nand4  g33(.a(new_n68_), .b(x11), .c(x10), .d(new_n49_), .O(new_n77_));
  nand2  g34(.a(x02), .b(x00), .O(new_n78_));
  aoi21  g35(.a(new_n77_), .b(new_n76_), .c(new_n78_), .O(z07));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  inv1   g38(.a(x04), .O(new_n82_));
  nor2   g39(.a(new_n82_), .b(x03), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n81_), .c(new_n80_), .d(new_n45_), .O(new_n84_));
  inv1   g41(.a(x02), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(x01), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(new_n60_), .O(new_n87_));
  inv1   g44(.a(x18), .O(new_n88_));
  nand3  g45(.a(x19), .b(new_n88_), .c(x17), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n87_), .c(new_n84_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n46_), .c(x18), .d(x01), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  inv1   g51(.a(x13), .O(new_n95_));
  inv1   g52(.a(x14), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g54(.a(x12), .b(new_n67_), .c(x02), .d(new_n61_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(x20), .c(new_n94_), .d(new_n93_), .O(new_n100_));
  nor2   g57(.a(x22), .b(x21), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n60_), .O(new_n102_));
  aoi21  g59(.a(new_n100_), .b(new_n92_), .c(new_n102_), .O(z09));
  inv1   g60(.a(x21), .O(new_n104_));
  nor2   g61(.a(new_n92_), .b(new_n104_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(x22), .O(new_n106_));
  inv1   g63(.a(new_n97_), .O(new_n107_));
  nand4  g64(.a(x20), .b(x16), .c(x15), .d(new_n67_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n101_), .c(new_n107_), .d(new_n86_), .O(new_n110_));
  nand2  g67(.a(x12), .b(new_n60_), .O(new_n111_));
  aoi21  g68(.a(new_n110_), .b(new_n106_), .c(new_n111_), .O(z10));
  nand4  g69(.a(new_n104_), .b(x20), .c(new_n94_), .d(x15), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n99_), .c(new_n105_), .O(new_n115_));
  inv1   g72(.a(x22), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n60_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n115_), .c(new_n45_), .O(z11));
  nand2  g75(.a(new_n71_), .b(new_n62_), .O(new_n119_));
  oai21  g76(.a(x19), .b(x02), .c(x23), .O(new_n120_));
  nand3  g77(.a(new_n46_), .b(x17), .c(new_n85_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n61_), .c(new_n60_), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand3  g81(.a(new_n45_), .b(new_n124_), .c(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n119_), .c(new_n125_), .O(z12));
  inv1   g83(.a(new_n45_), .O(new_n127_));
  nand2  g84(.a(new_n50_), .b(x17), .O(new_n128_));
  nor3   g85(.a(new_n128_), .b(new_n127_), .c(x19), .O(z13));
  nor2   g86(.a(new_n56_), .b(new_n51_), .O(z14));
  oai21  g87(.a(x10), .b(new_n61_), .c(x02), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x00), .O(new_n132_));
  nand3  g89(.a(x19), .b(new_n85_), .c(new_n61_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(new_n132_), .c(new_n127_), .O(z15));
  oai21  g91(.a(new_n61_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g92(.a(new_n87_), .b(new_n45_), .O(z17));
endmodule


