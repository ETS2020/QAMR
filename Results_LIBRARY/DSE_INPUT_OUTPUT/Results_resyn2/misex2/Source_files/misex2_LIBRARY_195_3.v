// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n126_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n134_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  nand2  g03(.a(x24), .b(x16), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(x10), .c(new_n50_), .d(new_n49_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand4  g10(.a(new_n51_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n54_));
  nand3  g11(.a(x10), .b(x09), .c(new_n49_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n54_), .c(new_n47_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n51_), .b(new_n57_), .c(x09), .d(new_n49_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n48_), .O(z02));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nor2   g18(.a(new_n57_), .b(x09), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x11), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(new_n61_), .c(x12), .O(new_n65_));
  nor2   g22(.a(x19), .b(x02), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n51_), .c(x18), .d(new_n44_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n65_), .c(new_n47_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n62_), .b(new_n47_), .c(new_n70_), .d(new_n69_), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n60_), .O(z04));
  nand2  g29(.a(x10), .b(x09), .O(new_n73_));
  oai21  g30(.a(new_n60_), .b(new_n73_), .c(new_n47_), .O(z05));
  oai21  g31(.a(new_n63_), .b(new_n60_), .c(new_n47_), .O(z06));
  nand2  g32(.a(new_n64_), .b(new_n70_), .O(new_n76_));
  nand3  g33(.a(new_n47_), .b(x02), .c(x00), .O(new_n77_));
  aoi21  g34(.a(new_n76_), .b(x01), .c(new_n77_), .O(z07));
  nor2   g35(.a(new_n44_), .b(x08), .O(new_n79_));
  inv1   g36(.a(x05), .O(new_n80_));
  nor2   g37(.a(x07), .b(x06), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(new_n80_), .c(x04), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nand3  g40(.a(new_n51_), .b(new_n83_), .c(x02), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n79_), .c(x19), .d(new_n45_), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(new_n47_), .O(z08));
  inv1   g44(.a(x01), .O(new_n88_));
  nand2  g45(.a(x02), .b(new_n88_), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  inv1   g47(.a(x14), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(x12), .d(new_n69_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  inv1   g51(.a(x16), .O(new_n95_));
  nand3  g52(.a(x20), .b(new_n95_), .c(new_n94_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  inv1   g54(.a(x20), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n46_), .c(x18), .d(x01), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  aoi21  g57(.a(new_n97_), .b(new_n93_), .c(new_n100_), .O(new_n101_));
  inv1   g58(.a(x00), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  or2    g61(.a(new_n104_), .b(x21), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n101_), .c(new_n47_), .O(z09));
  nand4  g63(.a(new_n100_), .b(new_n47_), .c(x22), .d(x21), .O(new_n107_));
  nor2   g64(.a(x21), .b(new_n98_), .O(new_n108_));
  nor2   g65(.a(new_n95_), .b(new_n94_), .O(new_n109_));
  nor2   g66(.a(x24), .b(x22), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n109_), .c(new_n108_), .d(new_n93_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n107_), .c(x00), .O(z10));
  nand3  g69(.a(new_n100_), .b(new_n47_), .c(x21), .O(new_n113_));
  nand4  g70(.a(new_n108_), .b(new_n93_), .c(new_n95_), .d(x15), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n113_), .c(new_n104_), .O(z11));
  nand2  g72(.a(x01), .b(x00), .O(new_n116_));
  aoi21  g73(.a(x10), .b(x02), .c(new_n116_), .O(new_n117_));
  inv1   g74(.a(new_n51_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand2  g76(.a(new_n66_), .b(x17), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n50_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n117_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n47_), .O(z12));
  oai21  g81(.a(new_n120_), .b(new_n118_), .c(new_n47_), .O(z13));
  nand3  g82(.a(new_n57_), .b(new_n50_), .c(new_n49_), .O(new_n126_));
  oai21  g83(.a(new_n126_), .b(new_n54_), .c(new_n47_), .O(z14));
  oai21  g84(.a(x10), .b(new_n88_), .c(x02), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x00), .O(new_n129_));
  nand3  g86(.a(x19), .b(new_n49_), .c(new_n88_), .O(new_n130_));
  nand3  g87(.a(new_n130_), .b(new_n129_), .c(new_n47_), .O(z15));
  nand3  g88(.a(new_n47_), .b(x01), .c(new_n102_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z16));
  nand3  g90(.a(new_n51_), .b(new_n47_), .c(x02), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(z17));
endmodule


