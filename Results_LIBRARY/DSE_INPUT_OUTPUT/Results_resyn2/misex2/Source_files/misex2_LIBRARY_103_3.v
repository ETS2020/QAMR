// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n138_;
  nand2  g00(.a(x17), .b(x11), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  inv1   g15(.a(x11), .O(new_n59_));
  nand2  g16(.a(x01), .b(x00), .O(new_n60_));
  nand2  g17(.a(x10), .b(x02), .O(new_n61_));
  nor3   g18(.a(new_n61_), .b(new_n60_), .c(x09), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(x12), .c(x17), .O(new_n63_));
  nor2   g20(.a(x02), .b(x01), .O(new_n64_));
  nor2   g21(.a(x17), .b(x00), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(new_n47_), .d(x18), .O(new_n66_));
  oai21  g23(.a(new_n63_), .b(new_n59_), .c(new_n66_), .O(z03));
  inv1   g24(.a(x12), .O(new_n68_));
  nand3  g25(.a(new_n62_), .b(new_n68_), .c(new_n59_), .O(new_n69_));
  inv1   g26(.a(new_n69_), .O(z04));
  inv1   g27(.a(new_n61_), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(x09), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n60_), .c(new_n44_), .O(z05));
  nor2   g30(.a(x17), .b(new_n59_), .O(new_n74_));
  and2   g31(.a(new_n74_), .b(new_n62_), .O(z06));
  nand4  g32(.a(new_n68_), .b(x11), .c(x10), .d(new_n50_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n44_), .O(z07));
  nand2  g36(.a(x17), .b(new_n59_), .O(new_n80_));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x04), .c(new_n81_), .O(new_n83_));
  nand2  g40(.a(new_n51_), .b(x02), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(x19), .c(new_n46_), .O(new_n86_));
  nor4   g43(.a(new_n86_), .b(new_n84_), .c(new_n83_), .d(new_n80_), .O(z08));
  inv1   g44(.a(x20), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n47_), .c(x18), .d(x01), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(new_n44_), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  inv1   g49(.a(x01), .O(new_n93_));
  nand2  g50(.a(x02), .b(new_n93_), .O(new_n94_));
  inv1   g51(.a(x13), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x12), .c(new_n59_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nor2   g54(.a(x15), .b(x14), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n97_), .c(x20), .d(new_n92_), .O(new_n99_));
  inv1   g56(.a(x00), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  aoi21  g60(.a(new_n99_), .b(new_n91_), .c(new_n103_), .O(z09));
  nand2  g61(.a(x22), .b(x21), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(new_n89_), .O(new_n106_));
  inv1   g63(.a(x14), .O(new_n107_));
  nand4  g64(.a(new_n101_), .b(x20), .c(x15), .d(new_n107_), .O(new_n108_));
  nand2  g65(.a(new_n102_), .b(x16), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n97_), .c(new_n106_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g69(.a(new_n102_), .b(new_n100_), .O(new_n113_));
  nand3  g70(.a(new_n90_), .b(new_n44_), .c(x21), .O(new_n114_));
  inv1   g71(.a(new_n108_), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n97_), .c(new_n92_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(z11));
  nand3  g74(.a(new_n51_), .b(x23), .c(x02), .O(new_n118_));
  oai21  g75(.a(new_n71_), .b(new_n60_), .c(new_n118_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n44_), .O(new_n120_));
  nand2  g77(.a(new_n47_), .b(new_n49_), .O(new_n121_));
  nor2   g78(.a(x19), .b(x17), .O(new_n122_));
  nand2  g79(.a(new_n44_), .b(x23), .O(new_n123_));
  oai22  g80(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n80_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n51_), .O(new_n125_));
  inv1   g82(.a(x24), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x09), .O(new_n127_));
  aoi21  g84(.a(new_n125_), .b(new_n120_), .c(new_n127_), .O(z12));
  nand3  g85(.a(new_n51_), .b(new_n47_), .c(new_n49_), .O(new_n129_));
  aoi21  g86(.a(new_n129_), .b(new_n59_), .c(new_n45_), .O(z13));
  nor2   g87(.a(new_n57_), .b(new_n52_), .O(z14));
  oai21  g88(.a(x10), .b(new_n93_), .c(x02), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(x00), .O(new_n133_));
  nand2  g90(.a(new_n64_), .b(x19), .O(new_n134_));
  nand3  g91(.a(new_n134_), .b(new_n133_), .c(new_n44_), .O(z15));
  nand3  g92(.a(new_n44_), .b(x01), .c(new_n100_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z16));
  nand3  g94(.a(new_n51_), .b(new_n44_), .c(x02), .O(new_n138_));
  inv1   g95(.a(new_n138_), .O(z17));
endmodule


