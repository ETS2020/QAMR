// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_;
  nor2   g00(.a(x22), .b(x21), .O(z09));
  inv1   g01(.a(z09), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nor2   g06(.a(x01), .b(x00), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n47_), .O(z00));
  nand3  g09(.a(new_n50_), .b(x09), .c(new_n48_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n47_), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand2  g12(.a(new_n46_), .b(new_n55_), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n53_), .c(new_n45_), .O(z02));
  nand3  g14(.a(x02), .b(x01), .c(x00), .O(new_n58_));
  inv1   g15(.a(new_n58_), .O(new_n59_));
  nand3  g16(.a(x11), .b(x10), .c(new_n49_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(new_n59_), .c(x12), .O(new_n62_));
  inv1   g19(.a(x18), .O(new_n63_));
  nor2   g20(.a(x19), .b(new_n63_), .O(new_n64_));
  nor2   g21(.a(x17), .b(x02), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n64_), .c(new_n50_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n62_), .c(new_n45_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n49_), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n58_), .c(new_n45_), .O(z04));
  nand2  g28(.a(x10), .b(x02), .O(new_n72_));
  inv1   g29(.a(x00), .O(new_n73_));
  inv1   g30(.a(x01), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(x09), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n72_), .c(new_n45_), .O(z05));
  oai21  g34(.a(new_n60_), .b(new_n58_), .c(new_n45_), .O(z06));
  nand2  g35(.a(new_n61_), .b(new_n69_), .O(new_n79_));
  nand3  g36(.a(new_n45_), .b(x02), .c(x00), .O(new_n80_));
  aoi21  g37(.a(new_n79_), .b(x01), .c(new_n80_), .O(z07));
  inv1   g38(.a(x08), .O(new_n82_));
  nand4  g39(.a(x19), .b(new_n63_), .c(x17), .d(new_n82_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  inv1   g41(.a(x05), .O(new_n85_));
  nor2   g42(.a(x07), .b(x06), .O(new_n86_));
  nand3  g43(.a(new_n86_), .b(new_n85_), .c(x04), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  inv1   g45(.a(x03), .O(new_n89_));
  nand3  g46(.a(new_n50_), .b(new_n89_), .c(x02), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n88_), .c(new_n84_), .O(new_n92_));
  nand2  g49(.a(new_n92_), .b(new_n45_), .O(z08));
  nor2   g50(.a(new_n74_), .b(x00), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(x20), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n94_), .c(new_n64_), .d(x22), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(z10));
  inv1   g55(.a(x22), .O(new_n99_));
  nand4  g56(.a(new_n96_), .b(new_n94_), .c(new_n64_), .d(new_n99_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(z11));
  nand2  g58(.a(new_n72_), .b(new_n75_), .O(new_n102_));
  oai21  g59(.a(x19), .b(x02), .c(x23), .O(new_n103_));
  inv1   g60(.a(x19), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(x17), .c(new_n48_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n50_), .O(new_n107_));
  nor2   g64(.a(x24), .b(new_n49_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n45_), .O(new_n109_));
  aoi21  g66(.a(new_n107_), .b(new_n102_), .c(new_n109_), .O(z12));
  inv1   g67(.a(new_n50_), .O(new_n111_));
  oai21  g68(.a(new_n105_), .b(new_n111_), .c(new_n45_), .O(z13));
  nand2  g69(.a(new_n46_), .b(new_n45_), .O(new_n113_));
  nand4  g70(.a(new_n50_), .b(new_n55_), .c(new_n49_), .d(new_n48_), .O(new_n114_));
  nor2   g71(.a(new_n114_), .b(new_n113_), .O(z14));
  aoi21  g72(.a(new_n55_), .b(x01), .c(new_n48_), .O(new_n116_));
  nor2   g73(.a(x02), .b(x01), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(x19), .c(z09), .O(new_n118_));
  oai21  g75(.a(new_n116_), .b(new_n73_), .c(new_n118_), .O(z15));
  nor3   g76(.a(z09), .b(new_n74_), .c(x00), .O(z16));
  nor3   g77(.a(new_n111_), .b(z09), .c(new_n48_), .O(z17));
endmodule


