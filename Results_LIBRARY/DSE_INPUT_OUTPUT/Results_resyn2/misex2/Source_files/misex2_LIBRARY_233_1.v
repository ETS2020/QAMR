// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n120_, new_n121_, new_n123_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x18), .b(x17), .O(new_n46_));
  nor2   g03(.a(x19), .b(x02), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(x10), .c(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x00), .O(z00));
  nand3  g08(.a(new_n49_), .b(x10), .c(x09), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x00), .O(z01));
  inv1   g10(.a(x02), .O(new_n54_));
  inv1   g11(.a(x10), .O(new_n55_));
  inv1   g12(.a(x19), .O(new_n56_));
  nand3  g13(.a(new_n46_), .b(new_n56_), .c(new_n55_), .O(new_n57_));
  inv1   g14(.a(new_n57_), .O(new_n58_));
  nand3  g15(.a(new_n58_), .b(x09), .c(new_n54_), .O(new_n59_));
  aoi21  g16(.a(new_n59_), .b(new_n44_), .c(x00), .O(z02));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x17), .O(new_n62_));
  nand3  g19(.a(new_n47_), .b(x18), .c(new_n62_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n44_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  nor2   g22(.a(x09), .b(new_n54_), .O(new_n66_));
  inv1   g23(.a(x11), .O(new_n67_));
  inv1   g24(.a(x12), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n66_), .c(x10), .d(x01), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n65_), .O(z03));
  nand4  g28(.a(new_n66_), .b(new_n68_), .c(new_n67_), .d(x10), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x00), .c(new_n44_), .O(z04));
  nand3  g30(.a(x02), .b(x01), .c(x00), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n55_), .c(new_n45_), .O(z05));
  nand3  g32(.a(x11), .b(x10), .c(new_n45_), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(new_n74_), .O(z06));
  inv1   g34(.a(new_n76_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n68_), .O(new_n79_));
  nand2  g36(.a(x02), .b(x00), .O(new_n80_));
  aoi21  g37(.a(new_n79_), .b(x01), .c(new_n80_), .O(z07));
  inv1   g38(.a(x08), .O(new_n82_));
  inv1   g39(.a(x18), .O(new_n83_));
  nand4  g40(.a(x19), .b(new_n83_), .c(x17), .d(new_n82_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nor2   g42(.a(x03), .b(new_n54_), .O(new_n86_));
  inv1   g43(.a(x04), .O(new_n87_));
  nor2   g44(.a(x05), .b(new_n87_), .O(new_n88_));
  nor2   g45(.a(x07), .b(x06), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n88_), .c(new_n86_), .d(new_n85_), .O(new_n90_));
  aoi21  g47(.a(new_n90_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g48(.a(x16), .O(new_n92_));
  nand3  g49(.a(x02), .b(new_n44_), .c(new_n61_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(z17));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nor2   g55(.a(x14), .b(x13), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(x12), .c(new_n67_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n98_), .c(z17), .d(new_n92_), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(x15), .O(z09));
  inv1   g60(.a(x15), .O(new_n104_));
  nor2   g61(.a(new_n92_), .b(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n101_), .c(new_n98_), .d(z17), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(z10));
  nor2   g64(.a(new_n102_), .b(new_n104_), .O(z11));
  aoi21  g65(.a(x10), .b(x02), .c(new_n44_), .O(new_n109_));
  inv1   g66(.a(x23), .O(new_n110_));
  nand2  g67(.a(new_n56_), .b(new_n54_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  aoi21  g69(.a(new_n47_), .b(new_n62_), .c(x00), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n112_), .c(new_n109_), .O(new_n114_));
  inv1   g71(.a(x24), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x09), .O(new_n116_));
  nand2  g73(.a(x01), .b(new_n61_), .O(new_n117_));
  oai21  g74(.a(new_n116_), .b(new_n114_), .c(new_n117_), .O(z12));
  nor4   g75(.a(new_n111_), .b(new_n62_), .c(x01), .d(x00), .O(z13));
  nand2  g76(.a(new_n54_), .b(new_n44_), .O(new_n120_));
  nand2  g77(.a(new_n45_), .b(new_n61_), .O(new_n121_));
  nor3   g78(.a(new_n121_), .b(new_n120_), .c(new_n57_), .O(z14));
  aoi21  g79(.a(new_n55_), .b(x01), .c(new_n54_), .O(new_n123_));
  oai22  g80(.a(new_n123_), .b(new_n61_), .c(new_n120_), .d(new_n56_), .O(z15));
  zero   g81(.O(z16));
endmodule


