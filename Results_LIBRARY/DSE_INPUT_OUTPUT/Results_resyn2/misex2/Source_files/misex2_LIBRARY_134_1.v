// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n72_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n119_, new_n120_;
  inv1   g00(.a(x02), .O(new_n44_));
  inv1   g01(.a(x09), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  nor3   g04(.a(x19), .b(x18), .c(x17), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(x10), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n47_), .O(z00));
  inv1   g07(.a(x01), .O(new_n51_));
  nand4  g08(.a(new_n48_), .b(x10), .c(x09), .d(new_n44_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n51_), .c(x00), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand2  g11(.a(new_n48_), .b(new_n54_), .O(new_n55_));
  nand3  g12(.a(new_n46_), .b(x09), .c(new_n44_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n55_), .O(z02));
  inv1   g14(.a(x00), .O(new_n58_));
  inv1   g15(.a(x17), .O(new_n59_));
  nor2   g16(.a(x19), .b(x02), .O(new_n60_));
  nand3  g17(.a(new_n60_), .b(x18), .c(new_n59_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n51_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nor2   g20(.a(x09), .b(new_n44_), .O(new_n64_));
  inv1   g21(.a(x11), .O(new_n65_));
  inv1   g22(.a(x12), .O(new_n66_));
  nor2   g23(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n64_), .c(x10), .d(x01), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n63_), .O(z03));
  nand4  g26(.a(new_n64_), .b(new_n66_), .c(new_n65_), .d(x10), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x00), .c(new_n51_), .O(z04));
  nor2   g28(.a(new_n51_), .b(new_n58_), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(x02), .O(new_n73_));
  nor3   g30(.a(new_n73_), .b(new_n54_), .c(new_n45_), .O(z05));
  nand3  g31(.a(x11), .b(x10), .c(new_n45_), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n73_), .O(z06));
  inv1   g33(.a(new_n75_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n66_), .O(new_n78_));
  inv1   g35(.a(new_n46_), .O(new_n79_));
  oai21  g36(.a(x02), .b(new_n58_), .c(new_n79_), .O(new_n80_));
  aoi21  g37(.a(new_n78_), .b(new_n72_), .c(new_n80_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nand2  g42(.a(new_n46_), .b(x02), .O(new_n86_));
  inv1   g43(.a(x18), .O(new_n87_));
  nand3  g44(.a(x19), .b(new_n87_), .c(x17), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n86_), .c(new_n85_), .O(z08));
  nor2   g46(.a(x14), .b(x13), .O(new_n90_));
  nor2   g47(.a(x16), .b(x15), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(x12), .d(new_n65_), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n95_));
  nor3   g52(.a(new_n95_), .b(new_n92_), .c(new_n86_), .O(z09));
  inv1   g53(.a(new_n95_), .O(new_n97_));
  nand3  g54(.a(x12), .b(new_n65_), .c(x02), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  and2   g56(.a(x16), .b(x15), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n99_), .c(new_n97_), .d(new_n90_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n51_), .c(x00), .O(z10));
  inv1   g59(.a(x15), .O(new_n103_));
  nor2   g60(.a(x16), .b(new_n103_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n99_), .c(new_n97_), .d(new_n90_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n51_), .c(x00), .O(z11));
  oai21  g63(.a(new_n54_), .b(new_n44_), .c(new_n72_), .O(new_n107_));
  nand2  g64(.a(new_n60_), .b(new_n59_), .O(new_n108_));
  inv1   g65(.a(x23), .O(new_n109_));
  inv1   g66(.a(x19), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n44_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n108_), .c(new_n46_), .O(new_n113_));
  inv1   g70(.a(x24), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(x09), .O(new_n115_));
  aoi21  g72(.a(new_n113_), .b(new_n107_), .c(new_n115_), .O(z12));
  nor3   g73(.a(new_n111_), .b(new_n79_), .c(new_n59_), .O(z13));
  nor2   g74(.a(new_n55_), .b(new_n47_), .O(z14));
  aoi21  g75(.a(new_n54_), .b(x01), .c(new_n44_), .O(new_n119_));
  nand3  g76(.a(x19), .b(new_n44_), .c(new_n51_), .O(new_n120_));
  oai21  g77(.a(new_n119_), .b(new_n58_), .c(new_n120_), .O(z15));
  nor2   g78(.a(new_n51_), .b(x00), .O(z16));
  inv1   g79(.a(new_n86_), .O(z17));
endmodule


