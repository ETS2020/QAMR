// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x11), .O(new_n44_));
  nor2   g01(.a(x21), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  inv1   g03(.a(x02), .O(new_n47_));
  inv1   g04(.a(x09), .O(new_n48_));
  nor2   g05(.a(x01), .b(x00), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  inv1   g08(.a(x18), .O(new_n52_));
  inv1   g09(.a(x19), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n50_), .c(new_n46_), .O(z00));
  nand3  g12(.a(new_n49_), .b(x09), .c(new_n47_), .O(new_n56_));
  nor3   g13(.a(new_n56_), .b(new_n54_), .c(new_n45_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n56_), .c(new_n46_), .O(z02));
  inv1   g17(.a(x21), .O(new_n61_));
  nand2  g18(.a(x01), .b(x00), .O(new_n62_));
  nand2  g19(.a(x10), .b(x02), .O(new_n63_));
  nor3   g20(.a(new_n63_), .b(new_n62_), .c(x09), .O(new_n64_));
  aoi21  g21(.a(new_n64_), .b(x12), .c(new_n61_), .O(new_n65_));
  nor2   g22(.a(x17), .b(x02), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n49_), .c(new_n53_), .d(x18), .O(new_n67_));
  oai21  g24(.a(new_n65_), .b(new_n44_), .c(new_n67_), .O(z03));
  inv1   g25(.a(x12), .O(new_n69_));
  nand3  g26(.a(new_n64_), .b(new_n69_), .c(new_n44_), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(z04));
  nand3  g28(.a(x10), .b(x09), .c(x02), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n62_), .c(new_n46_), .O(z05));
  nand3  g30(.a(new_n64_), .b(x21), .c(x11), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(z06));
  nand4  g32(.a(new_n69_), .b(x11), .c(x10), .d(new_n48_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n46_), .O(z07));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  inv1   g38(.a(x04), .O(new_n82_));
  nor2   g39(.a(new_n82_), .b(x03), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n81_), .c(new_n80_), .d(new_n46_), .O(new_n84_));
  inv1   g41(.a(x00), .O(new_n85_));
  inv1   g42(.a(x01), .O(new_n86_));
  nand3  g43(.a(x02), .b(new_n86_), .c(new_n85_), .O(new_n87_));
  nand3  g44(.a(x19), .b(new_n52_), .c(x17), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n87_), .c(new_n84_), .O(z08));
  inv1   g46(.a(x13), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x12), .c(x02), .d(new_n86_), .O(new_n91_));
  inv1   g48(.a(x14), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  nand4  g51(.a(x20), .b(new_n94_), .c(new_n93_), .d(new_n92_), .O(new_n95_));
  inv1   g52(.a(x20), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n53_), .c(x18), .d(x01), .O(new_n97_));
  oai21  g54(.a(new_n95_), .b(new_n91_), .c(new_n97_), .O(new_n98_));
  nor2   g55(.a(x22), .b(x00), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  aoi21  g57(.a(new_n100_), .b(new_n44_), .c(x21), .O(z09));
  nor2   g58(.a(new_n97_), .b(new_n61_), .O(new_n102_));
  nand2  g59(.a(x15), .b(new_n92_), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n91_), .O(new_n104_));
  inv1   g61(.a(x22), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n61_), .c(x20), .d(x16), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  aoi22  g64(.a(new_n107_), .b(new_n104_), .c(new_n102_), .d(x22), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(x00), .c(new_n46_), .O(z10));
  inv1   g66(.a(new_n99_), .O(new_n110_));
  nand3  g67(.a(new_n61_), .b(x20), .c(new_n94_), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n104_), .c(new_n102_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n110_), .c(new_n46_), .O(z11));
  nand3  g71(.a(new_n63_), .b(x01), .c(x00), .O(new_n115_));
  inv1   g72(.a(x23), .O(new_n116_));
  nor2   g73(.a(x19), .b(x02), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x17), .O(new_n118_));
  oai21  g75(.a(new_n117_), .b(new_n116_), .c(new_n118_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n49_), .O(new_n120_));
  nor2   g77(.a(x24), .b(new_n48_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n46_), .O(new_n122_));
  aoi21  g79(.a(new_n120_), .b(new_n115_), .c(new_n122_), .O(z12));
  inv1   g80(.a(new_n49_), .O(new_n124_));
  oai21  g81(.a(new_n118_), .b(new_n124_), .c(new_n46_), .O(z13));
  oai21  g82(.a(new_n59_), .b(new_n50_), .c(new_n46_), .O(z14));
  oai21  g83(.a(x10), .b(new_n86_), .c(x02), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x00), .O(new_n128_));
  nand3  g85(.a(x19), .b(new_n47_), .c(new_n86_), .O(new_n129_));
  aoi21  g86(.a(new_n129_), .b(new_n128_), .c(new_n45_), .O(z15));
  oai21  g87(.a(new_n86_), .b(x00), .c(new_n46_), .O(z16));
  nand2  g88(.a(new_n87_), .b(new_n46_), .O(z17));
endmodule


