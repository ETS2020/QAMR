// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n132_,
    new_n133_;
  inv1   g00(.a(x18), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  nand3  g02(.a(new_n45_), .b(new_n44_), .c(x10), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nor2   g05(.a(x17), .b(x02), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n46_), .O(z00));
  nand2  g10(.a(new_n51_), .b(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n46_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand3  g13(.a(new_n45_), .b(new_n44_), .c(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g15(.a(x01), .O(new_n59_));
  nand2  g16(.a(x02), .b(x00), .O(new_n60_));
  nand3  g17(.a(x11), .b(x10), .c(new_n47_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(x12), .c(x05), .O(new_n63_));
  nand3  g20(.a(new_n51_), .b(new_n45_), .c(x18), .O(new_n64_));
  oai21  g21(.a(new_n63_), .b(new_n59_), .c(new_n64_), .O(z03));
  inv1   g22(.a(x05), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(x01), .c(x00), .O(new_n67_));
  nand2  g24(.a(x10), .b(x02), .O(new_n68_));
  inv1   g25(.a(x11), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n47_), .O(new_n70_));
  nor4   g27(.a(new_n70_), .b(new_n68_), .c(new_n67_), .d(x12), .O(z04));
  inv1   g28(.a(new_n60_), .O(new_n72_));
  nand3  g29(.a(new_n72_), .b(x10), .c(x09), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(new_n66_), .c(new_n59_), .O(z05));
  nor2   g31(.a(new_n62_), .b(x05), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n59_), .O(z06));
  oai21  g33(.a(new_n61_), .b(x12), .c(x01), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  nand2  g35(.a(x05), .b(x01), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n78_), .O(z07));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nand2  g41(.a(new_n48_), .b(x02), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(z17));
  nand4  g43(.a(z17), .b(x19), .c(new_n44_), .d(x17), .O(new_n87_));
  oai21  g44(.a(new_n87_), .b(new_n84_), .c(new_n79_), .O(z08));
  nand2  g45(.a(x02), .b(new_n59_), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(x12), .c(new_n69_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  inv1   g51(.a(x16), .O(new_n95_));
  nand4  g52(.a(x20), .b(new_n95_), .c(new_n94_), .d(new_n93_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  inv1   g54(.a(x20), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n45_), .c(x18), .d(x01), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  aoi21  g57(.a(new_n97_), .b(new_n92_), .c(new_n100_), .O(new_n101_));
  inv1   g58(.a(x00), .O(new_n102_));
  inv1   g59(.a(x21), .O(new_n103_));
  inv1   g60(.a(x22), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n101_), .c(new_n79_), .O(z09));
  nand2  g63(.a(x22), .b(x21), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n99_), .O(new_n108_));
  nand4  g65(.a(new_n103_), .b(x20), .c(x15), .d(new_n93_), .O(new_n109_));
  nand2  g66(.a(new_n104_), .b(x16), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n92_), .c(new_n108_), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(x00), .c(new_n79_), .O(z10));
  nand2  g70(.a(new_n104_), .b(new_n102_), .O(new_n114_));
  inv1   g71(.a(new_n109_), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n92_), .c(new_n95_), .O(new_n116_));
  nand3  g73(.a(new_n100_), .b(x21), .c(new_n66_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n116_), .c(new_n114_), .O(z11));
  nand4  g75(.a(new_n68_), .b(new_n66_), .c(x01), .d(x00), .O(new_n119_));
  inv1   g76(.a(x23), .O(new_n120_));
  nor2   g77(.a(x19), .b(x02), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x17), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n120_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n48_), .O(new_n124_));
  inv1   g81(.a(x24), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x09), .O(new_n126_));
  aoi21  g83(.a(new_n124_), .b(new_n119_), .c(new_n126_), .O(z12));
  inv1   g84(.a(new_n48_), .O(new_n128_));
  nor2   g85(.a(new_n122_), .b(new_n128_), .O(z13));
  oai21  g86(.a(new_n57_), .b(new_n52_), .c(new_n79_), .O(z14));
  aoi21  g87(.a(new_n66_), .b(x00), .c(new_n59_), .O(z16));
  inv1   g88(.a(x02), .O(new_n132_));
  oai21  g89(.a(x19), .b(x00), .c(new_n132_), .O(new_n133_));
  oai22  g90(.a(new_n133_), .b(z16), .c(new_n67_), .d(x10), .O(z15));
endmodule


