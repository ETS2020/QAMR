// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  nor2   g02(.a(x01), .b(x00), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n49_), .O(z00));
  nand2  g11(.a(new_n48_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g16(.a(x12), .O(new_n60_));
  nor2   g17(.a(new_n51_), .b(x02), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  inv1   g19(.a(x00), .O(new_n63_));
  inv1   g20(.a(x01), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g22(.a(x11), .b(x10), .c(new_n44_), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n65_), .c(x02), .O(new_n68_));
  oai21  g25(.a(new_n68_), .b(new_n60_), .c(new_n62_), .O(z03));
  nand2  g26(.a(new_n65_), .b(x02), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  nand4  g28(.a(new_n60_), .b(new_n71_), .c(x10), .d(new_n44_), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n70_), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nand2  g31(.a(new_n65_), .b(x09), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n62_), .O(z05));
  nand2  g33(.a(new_n68_), .b(new_n62_), .O(z06));
  oai21  g34(.a(new_n66_), .b(x12), .c(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n62_), .O(z07));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x18), .b(new_n50_), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nand2  g41(.a(new_n46_), .b(x02), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(z17));
  nor2   g43(.a(x08), .b(x07), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(z17), .c(x19), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(new_n84_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n52_), .c(x18), .d(x01), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(x12), .c(new_n71_), .d(new_n64_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  nor2   g51(.a(x15), .b(x14), .O(new_n95_));
  nor2   g52(.a(new_n90_), .b(x16), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  inv1   g54(.a(x21), .O(new_n98_));
  inv1   g55(.a(x22), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(x02), .d(new_n63_), .O(new_n100_));
  aoi21  g57(.a(new_n97_), .b(new_n91_), .c(new_n100_), .O(z09));
  nor2   g58(.a(new_n61_), .b(new_n63_), .O(new_n102_));
  nor2   g59(.a(new_n93_), .b(new_n45_), .O(new_n103_));
  inv1   g60(.a(x14), .O(new_n104_));
  nand4  g61(.a(new_n98_), .b(x20), .c(x15), .d(new_n104_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n103_), .c(new_n99_), .d(x16), .O(new_n107_));
  nor2   g64(.a(new_n91_), .b(new_n98_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(x22), .c(new_n61_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n107_), .c(new_n102_), .O(z10));
  nand2  g67(.a(new_n99_), .b(new_n63_), .O(new_n111_));
  nor2   g68(.a(new_n105_), .b(x16), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n103_), .c(new_n108_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n111_), .c(new_n62_), .O(z11));
  nand2  g71(.a(new_n74_), .b(new_n65_), .O(new_n115_));
  nor3   g72(.a(x19), .b(x17), .c(x02), .O(new_n116_));
  nor2   g73(.a(x19), .b(x02), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(x23), .c(new_n46_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(new_n119_));
  nor2   g76(.a(x24), .b(new_n44_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n62_), .O(z12));
  nand2  g79(.a(new_n82_), .b(new_n52_), .O(new_n123_));
  nor2   g80(.a(new_n123_), .b(new_n47_), .O(z13));
  nor2   g81(.a(new_n58_), .b(new_n49_), .O(z14));
  inv1   g82(.a(new_n102_), .O(new_n126_));
  aoi21  g83(.a(new_n57_), .b(x01), .c(new_n45_), .O(new_n127_));
  nand4  g84(.a(x19), .b(new_n51_), .c(new_n45_), .d(new_n64_), .O(new_n128_));
  oai21  g85(.a(new_n127_), .b(new_n126_), .c(new_n128_), .O(z15));
  oai21  g86(.a(new_n64_), .b(x00), .c(new_n62_), .O(z16));
endmodule


