// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n135_, new_n136_, new_n137_,
    new_n140_;
  inv1   g00(.a(x23), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x17), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x10), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand2  g13(.a(new_n54_), .b(x09), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n49_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  nand3  g18(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(x11), .b(x10), .c(new_n50_), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n63_), .c(x12), .O(new_n66_));
  nor2   g23(.a(x19), .b(new_n47_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n54_), .c(new_n46_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n66_), .c(new_n45_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n50_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n62_), .c(new_n45_), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nand3  g31(.a(x09), .b(x01), .c(x00), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n45_), .O(z05));
  oai21  g33(.a(new_n64_), .b(new_n62_), .c(new_n45_), .O(z06));
  nand2  g34(.a(new_n65_), .b(new_n71_), .O(new_n78_));
  nand3  g35(.a(new_n45_), .b(x02), .c(x00), .O(new_n79_));
  aoi21  g36(.a(new_n78_), .b(x01), .c(new_n79_), .O(z07));
  inv1   g37(.a(x08), .O(new_n81_));
  nand3  g38(.a(x19), .b(new_n47_), .c(new_n81_), .O(new_n82_));
  inv1   g39(.a(new_n82_), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  nor2   g41(.a(x07), .b(x06), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(new_n84_), .c(x04), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  inv1   g44(.a(x03), .O(new_n88_));
  nor2   g45(.a(x01), .b(x00), .O(new_n89_));
  nand3  g46(.a(new_n89_), .b(new_n88_), .c(x02), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n87_), .c(new_n83_), .O(new_n92_));
  aoi21  g49(.a(new_n92_), .b(x23), .c(new_n46_), .O(z08));
  nor2   g50(.a(x20), .b(new_n52_), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n67_), .O(new_n95_));
  nand2  g52(.a(x02), .b(new_n52_), .O(new_n96_));
  inv1   g53(.a(x13), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(x12), .c(new_n70_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  inv1   g56(.a(x20), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(x16), .O(new_n101_));
  nor2   g58(.a(x15), .b(x14), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n99_), .O(new_n103_));
  inv1   g60(.a(x21), .O(new_n104_));
  inv1   g61(.a(x22), .O(new_n105_));
  nand4  g62(.a(new_n45_), .b(new_n105_), .c(new_n104_), .d(new_n51_), .O(new_n106_));
  aoi21  g63(.a(new_n103_), .b(new_n95_), .c(new_n106_), .O(z09));
  nand2  g64(.a(new_n45_), .b(new_n51_), .O(new_n108_));
  nand4  g65(.a(new_n94_), .b(new_n67_), .c(x22), .d(x21), .O(new_n109_));
  inv1   g66(.a(x15), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(x14), .O(new_n111_));
  nand4  g68(.a(new_n105_), .b(new_n104_), .c(x20), .d(x16), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n111_), .c(new_n99_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n109_), .c(new_n108_), .O(z10));
  nand3  g72(.a(new_n94_), .b(new_n67_), .c(x21), .O(new_n116_));
  inv1   g73(.a(x16), .O(new_n117_));
  nand3  g74(.a(new_n104_), .b(x20), .c(new_n117_), .O(new_n118_));
  inv1   g75(.a(new_n118_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n111_), .c(new_n99_), .O(new_n120_));
  nand3  g77(.a(new_n45_), .b(new_n105_), .c(new_n51_), .O(new_n121_));
  aoi21  g78(.a(new_n120_), .b(new_n116_), .c(new_n121_), .O(z11));
  nand2  g79(.a(x01), .b(x00), .O(new_n123_));
  aoi21  g80(.a(x10), .b(x02), .c(new_n123_), .O(new_n124_));
  oai21  g81(.a(x19), .b(x02), .c(x23), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n46_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n89_), .c(new_n124_), .O(new_n127_));
  inv1   g84(.a(x24), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x09), .O(new_n129_));
  oai21  g86(.a(new_n129_), .b(new_n127_), .c(new_n45_), .O(z12));
  inv1   g87(.a(x02), .O(new_n131_));
  nand3  g88(.a(new_n89_), .b(new_n48_), .c(new_n131_), .O(new_n132_));
  aoi21  g89(.a(new_n132_), .b(x23), .c(new_n46_), .O(z13));
  nor2   g90(.a(new_n60_), .b(new_n55_), .O(z14));
  oai21  g91(.a(x10), .b(new_n52_), .c(x02), .O(new_n135_));
  nand2  g92(.a(new_n135_), .b(x00), .O(new_n136_));
  nand3  g93(.a(x19), .b(new_n131_), .c(new_n52_), .O(new_n137_));
  aoi22  g94(.a(new_n137_), .b(new_n136_), .c(new_n44_), .d(x17), .O(z15));
  nor2   g95(.a(new_n108_), .b(new_n52_), .O(z16));
  nand2  g96(.a(new_n45_), .b(x02), .O(new_n140_));
  nor2   g97(.a(new_n140_), .b(new_n53_), .O(z17));
endmodule


