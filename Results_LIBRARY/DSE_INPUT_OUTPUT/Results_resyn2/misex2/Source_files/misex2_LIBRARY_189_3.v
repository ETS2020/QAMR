// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n130_, new_n131_,
    new_n132_;
  nand2  g00(.a(x16), .b(x13), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n52_), .c(new_n44_), .O(z01));
  nand2  g12(.a(new_n44_), .b(new_n51_), .O(new_n56_));
  nor2   g13(.a(x17), .b(x10), .O(new_n57_));
  nand2  g14(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  nor3   g15(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(z02));
  inv1   g16(.a(new_n44_), .O(new_n60_));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g20(.a(x11), .b(x10), .c(new_n46_), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n63_), .c(x12), .d(x02), .O(new_n66_));
  nor2   g23(.a(x19), .b(x02), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n47_), .c(x18), .d(new_n49_), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(new_n66_), .c(new_n60_), .O(z03));
  nand2  g26(.a(new_n63_), .b(x02), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(x10), .d(new_n46_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n70_), .c(new_n44_), .O(z04));
  nand2  g31(.a(x10), .b(x02), .O(new_n75_));
  nand2  g32(.a(new_n63_), .b(x09), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n44_), .O(z05));
  nor3   g34(.a(new_n64_), .b(new_n70_), .c(new_n60_), .O(z06));
  oai21  g35(.a(new_n64_), .b(x12), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n44_), .O(z07));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  inv1   g40(.a(x04), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(x03), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n83_), .c(new_n82_), .d(new_n44_), .O(new_n86_));
  nand3  g43(.a(x02), .b(new_n62_), .c(new_n61_), .O(new_n87_));
  nand3  g44(.a(x19), .b(new_n50_), .c(x17), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n51_), .c(x18), .d(x01), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand2  g49(.a(x02), .b(new_n62_), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n71_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  inv1   g53(.a(x14), .O(new_n97_));
  inv1   g54(.a(x15), .O(new_n98_));
  inv1   g55(.a(x16), .O(new_n99_));
  nand4  g56(.a(x20), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n96_), .c(new_n92_), .O(new_n102_));
  inv1   g59(.a(x21), .O(new_n103_));
  inv1   g60(.a(x22), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n103_), .c(new_n61_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n102_), .c(new_n44_), .O(z09));
  nor2   g63(.a(new_n91_), .b(new_n103_), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(new_n44_), .c(x22), .O(new_n108_));
  nand2  g65(.a(x15), .b(new_n97_), .O(new_n109_));
  nand4  g66(.a(new_n104_), .b(new_n103_), .c(x20), .d(x16), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n96_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n108_), .c(x00), .O(z10));
  nand2  g70(.a(new_n104_), .b(new_n61_), .O(new_n114_));
  nand3  g71(.a(new_n103_), .b(x20), .c(new_n99_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n109_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n96_), .c(new_n107_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n114_), .c(new_n44_), .O(z11));
  nand2  g75(.a(new_n75_), .b(new_n63_), .O(new_n119_));
  nand2  g76(.a(new_n67_), .b(new_n49_), .O(new_n120_));
  or2    g77(.a(new_n67_), .b(x23), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n120_), .c(new_n47_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand3  g80(.a(new_n44_), .b(new_n123_), .c(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n119_), .c(new_n124_), .O(z12));
  nand4  g82(.a(x17), .b(new_n45_), .c(new_n62_), .d(new_n61_), .O(new_n126_));
  nor2   g83(.a(new_n126_), .b(new_n56_), .O(z13));
  nand3  g84(.a(new_n57_), .b(new_n51_), .c(new_n50_), .O(new_n128_));
  oai21  g85(.a(new_n128_), .b(new_n48_), .c(new_n44_), .O(z14));
  oai21  g86(.a(x10), .b(new_n62_), .c(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand3  g88(.a(x19), .b(new_n45_), .c(new_n62_), .O(new_n132_));
  aoi21  g89(.a(new_n132_), .b(new_n131_), .c(new_n60_), .O(z15));
  oai21  g90(.a(new_n62_), .b(x00), .c(new_n44_), .O(z16));
  nand2  g91(.a(new_n87_), .b(new_n44_), .O(z17));
endmodule


