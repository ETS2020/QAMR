// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n134_, new_n135_, new_n136_, new_n139_;
  inv1   g00(.a(x16), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  inv1   g17(.a(x00), .O(new_n61_));
  nor3   g18(.a(x09), .b(new_n46_), .c(new_n61_), .O(new_n62_));
  nand3  g19(.a(x12), .b(x11), .c(x10), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  aoi21  g21(.a(new_n64_), .b(new_n62_), .c(new_n44_), .O(new_n65_));
  nor2   g22(.a(x19), .b(new_n51_), .O(new_n66_));
  nor2   g23(.a(x02), .b(x01), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(new_n50_), .d(new_n61_), .O(new_n68_));
  oai21  g25(.a(new_n65_), .b(new_n60_), .c(new_n68_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n62_), .b(new_n71_), .c(new_n70_), .d(x10), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x16), .c(new_n60_), .O(z04));
  nor2   g30(.a(new_n46_), .b(new_n61_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x01), .O(new_n75_));
  nand2  g32(.a(x16), .b(x10), .O(new_n76_));
  nor3   g33(.a(new_n76_), .b(new_n75_), .c(new_n47_), .O(z05));
  nand2  g34(.a(x11), .b(new_n47_), .O(new_n78_));
  nor3   g35(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(z06));
  nand2  g36(.a(new_n71_), .b(x10), .O(new_n80_));
  oai21  g37(.a(new_n78_), .b(new_n80_), .c(x01), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n45_), .O(z07));
  inv1   g40(.a(x03), .O(new_n84_));
  nor2   g41(.a(x06), .b(x05), .O(new_n85_));
  nor2   g42(.a(x08), .b(x07), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(x04), .d(new_n84_), .O(new_n87_));
  nor2   g44(.a(new_n46_), .b(x01), .O(new_n88_));
  nor2   g45(.a(x18), .b(new_n50_), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n88_), .c(x19), .d(new_n61_), .O(new_n90_));
  oai21  g47(.a(new_n90_), .b(new_n87_), .c(new_n45_), .O(z08));
  inv1   g48(.a(x20), .O(new_n92_));
  nand4  g49(.a(new_n66_), .b(new_n92_), .c(x16), .d(x01), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand4  g51(.a(x20), .b(new_n44_), .c(new_n94_), .d(x12), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nor3   g53(.a(x15), .b(x14), .c(x11), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(new_n88_), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(new_n61_), .O(new_n101_));
  aoi21  g58(.a(new_n98_), .b(new_n93_), .c(new_n101_), .O(z09));
  nand2  g59(.a(new_n92_), .b(x01), .O(new_n103_));
  nand3  g60(.a(x21), .b(new_n52_), .c(x18), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(x22), .O(new_n106_));
  nor2   g63(.a(x13), .b(new_n71_), .O(new_n107_));
  nand4  g64(.a(new_n100_), .b(new_n99_), .c(x20), .d(new_n70_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  inv1   g66(.a(x14), .O(new_n110_));
  nand2  g67(.a(x15), .b(new_n110_), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n109_), .c(new_n107_), .d(new_n88_), .O(new_n113_));
  nand2  g70(.a(x16), .b(new_n61_), .O(new_n114_));
  aoi21  g71(.a(new_n113_), .b(new_n106_), .c(new_n114_), .O(z10));
  nand2  g72(.a(new_n100_), .b(new_n61_), .O(new_n116_));
  nand3  g73(.a(new_n99_), .b(new_n70_), .c(x02), .O(new_n117_));
  nor2   g74(.a(new_n117_), .b(new_n111_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n96_), .c(new_n105_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n116_), .c(new_n45_), .O(z11));
  inv1   g77(.a(x23), .O(new_n121_));
  nor2   g78(.a(x19), .b(x02), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x17), .O(new_n123_));
  oai21  g80(.a(new_n122_), .b(new_n121_), .c(new_n123_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n48_), .O(new_n125_));
  nand2  g82(.a(x10), .b(x02), .O(new_n126_));
  nand4  g83(.a(new_n126_), .b(x16), .c(x01), .d(x00), .O(new_n127_));
  inv1   g84(.a(x24), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x09), .O(new_n129_));
  aoi21  g86(.a(new_n127_), .b(new_n125_), .c(new_n129_), .O(z12));
  nand3  g87(.a(new_n122_), .b(new_n48_), .c(x17), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(new_n45_), .O(z13));
  oai21  g89(.a(new_n58_), .b(new_n49_), .c(new_n45_), .O(z14));
  oai21  g90(.a(x10), .b(new_n60_), .c(x02), .O(new_n134_));
  nand2  g91(.a(new_n134_), .b(x00), .O(new_n135_));
  nand2  g92(.a(new_n67_), .b(x19), .O(new_n136_));
  nand3  g93(.a(new_n136_), .b(new_n135_), .c(new_n45_), .O(z15));
  aoi21  g94(.a(x16), .b(x00), .c(new_n60_), .O(z16));
  nand2  g95(.a(new_n88_), .b(new_n61_), .O(new_n139_));
  nand2  g96(.a(new_n139_), .b(new_n45_), .O(z17));
endmodule


