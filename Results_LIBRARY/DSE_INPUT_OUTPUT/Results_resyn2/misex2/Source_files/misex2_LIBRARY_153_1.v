// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n119_,
    new_n120_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x10), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(x09), .O(new_n46_));
  inv1   g03(.a(x02), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g08(.a(new_n51_), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x00), .O(z00));
  nand3  g11(.a(new_n52_), .b(x10), .c(x09), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x00), .O(z01));
  nor2   g13(.a(x01), .b(x00), .O(new_n57_));
  nand2  g14(.a(new_n57_), .b(new_n52_), .O(new_n58_));
  nand2  g15(.a(new_n45_), .b(x09), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n58_), .O(z02));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(new_n46_), .c(x12), .d(x11), .O(new_n63_));
  nor2   g20(.a(x02), .b(x01), .O(new_n64_));
  nor2   g21(.a(x17), .b(x00), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(new_n50_), .d(x18), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n63_), .O(z03));
  inv1   g24(.a(x12), .O(new_n68_));
  nand2  g25(.a(new_n46_), .b(new_n68_), .O(new_n69_));
  nor2   g26(.a(x11), .b(new_n47_), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(x01), .c(x00), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n69_), .O(z04));
  inv1   g29(.a(x09), .O(new_n73_));
  nor3   g30(.a(new_n61_), .b(new_n45_), .c(new_n73_), .O(z05));
  nand2  g31(.a(new_n46_), .b(x11), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n61_), .O(z06));
  nand3  g33(.a(new_n46_), .b(new_n68_), .c(x11), .O(new_n77_));
  nand2  g34(.a(x02), .b(x00), .O(new_n78_));
  aoi21  g35(.a(new_n77_), .b(x01), .c(new_n78_), .O(z07));
  inv1   g36(.a(x08), .O(new_n80_));
  nand4  g37(.a(x19), .b(new_n49_), .c(x17), .d(new_n80_), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nor2   g39(.a(x03), .b(new_n47_), .O(new_n83_));
  inv1   g40(.a(x04), .O(new_n84_));
  nor2   g41(.a(x05), .b(new_n84_), .O(new_n85_));
  nor2   g42(.a(x07), .b(x06), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(new_n83_), .d(new_n82_), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g45(.a(x13), .O(new_n89_));
  inv1   g46(.a(x14), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n89_), .c(x12), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n70_), .c(new_n57_), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  inv1   g51(.a(x16), .O(new_n95_));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(x20), .d(new_n95_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n94_), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(new_n93_), .O(z09));
  nor2   g58(.a(new_n95_), .b(new_n94_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n97_), .c(new_n96_), .d(x20), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n93_), .O(z10));
  nand4  g61(.a(new_n99_), .b(new_n92_), .c(new_n70_), .d(x15), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n44_), .c(x00), .O(z11));
  aoi21  g63(.a(x10), .b(x02), .c(new_n44_), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g65(.a(new_n50_), .b(x17), .c(new_n47_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n108_), .c(x00), .O(new_n110_));
  nor2   g67(.a(x24), .b(new_n73_), .O(new_n111_));
  oai21  g68(.a(new_n110_), .b(new_n107_), .c(new_n111_), .O(new_n112_));
  inv1   g69(.a(x00), .O(new_n113_));
  nand2  g70(.a(x01), .b(new_n113_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n112_), .O(z12));
  aoi21  g72(.a(new_n109_), .b(new_n44_), .c(x00), .O(z13));
  nand4  g73(.a(new_n57_), .b(new_n52_), .c(new_n45_), .d(new_n73_), .O(new_n117_));
  inv1   g74(.a(new_n117_), .O(z14));
  aoi21  g75(.a(new_n45_), .b(x01), .c(new_n47_), .O(new_n119_));
  nand2  g76(.a(new_n64_), .b(x19), .O(new_n120_));
  oai21  g77(.a(new_n119_), .b(new_n113_), .c(new_n120_), .O(z15));
  inv1   g78(.a(new_n114_), .O(z16));
  nor2   g79(.a(new_n64_), .b(x00), .O(z17));
endmodule


