// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n126_, new_n127_, new_n129_;
  inv1   g00(.a(x22), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n45_), .O(z00));
  nand2  g10(.a(new_n47_), .b(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n52_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  nand3  g15(.a(x11), .b(x10), .c(new_n46_), .O(new_n59_));
  nand3  g16(.a(x12), .b(x02), .c(x00), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n59_), .c(x22), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(x01), .O(new_n62_));
  nand4  g19(.a(new_n47_), .b(new_n51_), .c(x18), .d(new_n49_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n62_), .O(z03));
  nand3  g21(.a(x22), .b(x01), .c(x00), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(x10), .c(new_n46_), .d(x02), .O(new_n67_));
  nor3   g24(.a(new_n67_), .b(x12), .c(x11), .O(z04));
  inv1   g25(.a(x01), .O(new_n69_));
  inv1   g26(.a(x00), .O(new_n70_));
  inv1   g27(.a(x02), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g29(.a(new_n72_), .b(x10), .c(x09), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x22), .c(new_n69_), .O(z05));
  inv1   g31(.a(x11), .O(new_n75_));
  nor2   g32(.a(new_n67_), .b(new_n75_), .O(z06));
  oai21  g33(.a(new_n59_), .b(x12), .c(x01), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n45_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nor2   g40(.a(x01), .b(x00), .O(new_n84_));
  nor2   g41(.a(x18), .b(new_n49_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x19), .d(x02), .O(new_n86_));
  oai21  g43(.a(new_n86_), .b(new_n83_), .c(new_n45_), .O(z08));
  inv1   g44(.a(x15), .O(new_n88_));
  nand2  g45(.a(new_n84_), .b(x02), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(z17));
  inv1   g47(.a(x13), .O(new_n91_));
  nand2  g48(.a(new_n91_), .b(x12), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(new_n75_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  nand3  g54(.a(new_n44_), .b(new_n97_), .c(x20), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n96_), .c(z17), .d(new_n88_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(z09));
  aoi21  g58(.a(new_n44_), .b(x01), .c(new_n70_), .O(new_n102_));
  inv1   g59(.a(x20), .O(new_n103_));
  nand4  g60(.a(x21), .b(new_n103_), .c(new_n51_), .d(x18), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(x22), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(x01), .O(new_n106_));
  nand3  g63(.a(x16), .b(x15), .c(new_n93_), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand4  g65(.a(new_n91_), .b(x12), .c(new_n75_), .d(x02), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n108_), .c(new_n99_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n106_), .c(new_n102_), .O(z10));
  nand4  g69(.a(new_n99_), .b(new_n96_), .c(z17), .d(x15), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(z11));
  oai21  g71(.a(new_n56_), .b(new_n71_), .c(new_n66_), .O(new_n115_));
  oai21  g72(.a(x19), .b(x02), .c(x23), .O(new_n116_));
  nand3  g73(.a(new_n51_), .b(x17), .c(new_n71_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n84_), .O(new_n119_));
  inv1   g76(.a(x24), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x09), .O(new_n121_));
  aoi21  g78(.a(new_n119_), .b(new_n115_), .c(new_n121_), .O(z12));
  nand3  g79(.a(new_n47_), .b(new_n51_), .c(x17), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n45_), .O(z13));
  oai21  g81(.a(new_n57_), .b(new_n48_), .c(new_n45_), .O(z14));
  nor2   g82(.a(new_n51_), .b(x01), .O(new_n126_));
  oai21  g83(.a(new_n126_), .b(new_n102_), .c(new_n71_), .O(new_n127_));
  oai21  g84(.a(new_n65_), .b(x10), .c(new_n127_), .O(z15));
  nand3  g85(.a(x22), .b(x01), .c(new_n70_), .O(new_n129_));
  inv1   g86(.a(new_n129_), .O(z16));
endmodule


