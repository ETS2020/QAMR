// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n117_, new_n118_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n46_), .c(new_n45_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x00), .O(z00));
  inv1   g10(.a(x00), .O(new_n54_));
  nor2   g11(.a(x02), .b(x01), .O(new_n55_));
  nand3  g12(.a(new_n55_), .b(x09), .c(new_n54_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n50_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n56_), .O(z02));
  nand2  g17(.a(new_n55_), .b(new_n54_), .O(new_n61_));
  nand3  g18(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  nand3  g19(.a(x11), .b(x10), .c(new_n46_), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x12), .O(new_n65_));
  nand3  g22(.a(new_n49_), .b(x18), .c(new_n47_), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n61_), .c(new_n65_), .d(new_n62_), .O(z03));
  nand2  g24(.a(x10), .b(new_n46_), .O(new_n68_));
  nor4   g25(.a(new_n68_), .b(new_n62_), .c(x12), .d(x11), .O(z04));
  nand3  g26(.a(x10), .b(x09), .c(x02), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x00), .c(new_n44_), .O(z05));
  nand2  g28(.a(new_n64_), .b(x02), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x00), .c(new_n44_), .O(z06));
  oai21  g30(.a(new_n63_), .b(x12), .c(x01), .O(new_n74_));
  nand3  g31(.a(new_n74_), .b(x02), .c(x00), .O(new_n75_));
  nand2  g32(.a(x01), .b(new_n54_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n75_), .O(z07));
  inv1   g34(.a(x08), .O(new_n78_));
  nand4  g35(.a(x19), .b(new_n48_), .c(x17), .d(new_n78_), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(new_n80_));
  nor2   g37(.a(x03), .b(new_n45_), .O(new_n81_));
  inv1   g38(.a(x04), .O(new_n82_));
  nor2   g39(.a(x05), .b(new_n82_), .O(new_n83_));
  nor2   g40(.a(x07), .b(x06), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(new_n81_), .d(new_n80_), .O(new_n85_));
  aoi21  g42(.a(new_n85_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g43(.a(x16), .O(new_n87_));
  inv1   g44(.a(x11), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(x02), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  inv1   g47(.a(x14), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n90_), .c(x12), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(x20), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n93_), .c(new_n87_), .O(new_n99_));
  aoi21  g56(.a(new_n99_), .b(new_n44_), .c(x00), .O(z09));
  nand4  g57(.a(new_n96_), .b(new_n95_), .c(x20), .d(x15), .O(new_n101_));
  nor2   g58(.a(x01), .b(x00), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n93_), .c(x16), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n101_), .O(z10));
  inv1   g61(.a(new_n101_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n93_), .c(new_n87_), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n44_), .c(x00), .O(z11));
  aoi21  g64(.a(x10), .b(x02), .c(new_n44_), .O(new_n108_));
  oai21  g65(.a(x19), .b(x02), .c(x23), .O(new_n109_));
  nand3  g66(.a(new_n49_), .b(x17), .c(new_n45_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n109_), .c(x00), .O(new_n111_));
  nor2   g68(.a(x24), .b(new_n46_), .O(new_n112_));
  oai21  g69(.a(new_n111_), .b(new_n108_), .c(new_n112_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n76_), .O(z12));
  aoi21  g71(.a(new_n110_), .b(new_n44_), .c(x00), .O(z13));
  nor3   g72(.a(new_n59_), .b(new_n61_), .c(x09), .O(z14));
  aoi21  g73(.a(new_n58_), .b(x01), .c(new_n45_), .O(new_n117_));
  nand2  g74(.a(new_n55_), .b(x19), .O(new_n118_));
  oai21  g75(.a(new_n117_), .b(new_n54_), .c(new_n118_), .O(z15));
  inv1   g76(.a(new_n76_), .O(z16));
  nor2   g77(.a(new_n55_), .b(x00), .O(z17));
endmodule


