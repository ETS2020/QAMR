// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n116_, new_n118_, new_n119_,
    new_n122_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  nor2   g02(.a(x18), .b(x17), .O(new_n46_));
  nor2   g03(.a(x09), .b(x02), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x00), .O(z00));
  nor2   g06(.a(x19), .b(x02), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n46_), .c(x09), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  nand3  g09(.a(x10), .b(new_n44_), .c(new_n52_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n51_), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand4  g12(.a(new_n50_), .b(new_n46_), .c(new_n55_), .d(x09), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n44_), .c(x00), .O(z02));
  inv1   g14(.a(x17), .O(new_n58_));
  nand3  g15(.a(new_n50_), .b(x18), .c(new_n58_), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n44_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  inv1   g18(.a(x09), .O(new_n62_));
  inv1   g19(.a(x02), .O(new_n63_));
  nor2   g20(.a(new_n55_), .b(new_n63_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(x11), .c(new_n62_), .O(new_n65_));
  nand2  g22(.a(x12), .b(x01), .O(new_n66_));
  oai21  g23(.a(new_n66_), .b(new_n65_), .c(new_n61_), .O(z03));
  inv1   g24(.a(x12), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(x10), .c(x02), .d(x00), .O(new_n69_));
  nor4   g26(.a(new_n69_), .b(x11), .c(x09), .d(new_n44_), .O(z04));
  nand2  g27(.a(new_n64_), .b(x09), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x00), .c(new_n44_), .O(z05));
  aoi21  g29(.a(new_n65_), .b(x00), .c(new_n44_), .O(z06));
  nand4  g30(.a(new_n68_), .b(x11), .c(x10), .d(new_n62_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  nand2  g33(.a(x01), .b(new_n52_), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n76_), .O(z07));
  inv1   g35(.a(x08), .O(new_n79_));
  inv1   g36(.a(x18), .O(new_n80_));
  nand4  g37(.a(x19), .b(new_n80_), .c(x17), .d(new_n79_), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nor2   g39(.a(x03), .b(new_n63_), .O(new_n83_));
  inv1   g40(.a(x04), .O(new_n84_));
  nor2   g41(.a(x05), .b(new_n84_), .O(new_n85_));
  nor2   g42(.a(x07), .b(x06), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(new_n83_), .d(new_n82_), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g45(.a(x11), .O(new_n89_));
  inv1   g46(.a(x13), .O(new_n90_));
  inv1   g47(.a(x14), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(x12), .d(new_n89_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(x20), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nor2   g55(.a(x15), .b(new_n63_), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(new_n93_), .O(new_n100_));
  aoi21  g57(.a(new_n100_), .b(new_n44_), .c(x00), .O(z09));
  nand3  g58(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n102_));
  nand4  g59(.a(x15), .b(x02), .c(new_n44_), .d(new_n52_), .O(new_n103_));
  nor4   g60(.a(new_n103_), .b(new_n102_), .c(new_n92_), .d(new_n94_), .O(z10));
  nor3   g61(.a(new_n103_), .b(new_n97_), .c(new_n92_), .O(z11));
  aoi21  g62(.a(x10), .b(x02), .c(new_n44_), .O(new_n106_));
  inv1   g63(.a(x23), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(new_n107_), .O(new_n108_));
  aoi21  g65(.a(new_n50_), .b(new_n58_), .c(x00), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n108_), .c(new_n106_), .O(new_n110_));
  inv1   g67(.a(x24), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(x09), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(new_n110_), .c(new_n77_), .O(z12));
  nand2  g70(.a(new_n50_), .b(x17), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n44_), .c(x00), .O(z13));
  nand4  g72(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n55_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n44_), .c(x00), .O(z14));
  aoi21  g74(.a(new_n55_), .b(x01), .c(new_n63_), .O(new_n118_));
  nand3  g75(.a(x19), .b(new_n63_), .c(new_n44_), .O(new_n119_));
  oai21  g76(.a(new_n118_), .b(new_n52_), .c(new_n119_), .O(z15));
  nand3  g77(.a(x02), .b(new_n44_), .c(new_n52_), .O(new_n122_));
  inv1   g78(.a(new_n122_), .O(z17));
  zero   g79(.O(z16));
endmodule


