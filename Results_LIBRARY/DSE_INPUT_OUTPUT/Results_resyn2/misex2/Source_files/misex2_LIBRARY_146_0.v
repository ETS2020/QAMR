// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n131_,
    new_n132_, new_n133_;
  inv1   g00(.a(x19), .O(new_n44_));
  nor2   g01(.a(x16), .b(x12), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n48_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g15(.a(x17), .O(new_n59_));
  nor2   g16(.a(x19), .b(x02), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n51_), .c(x18), .d(new_n59_), .O(new_n61_));
  inv1   g18(.a(x00), .O(new_n62_));
  inv1   g19(.a(x01), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(new_n50_), .c(x02), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(x12), .c(x11), .d(x10), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n61_), .c(new_n46_), .O(z03));
  inv1   g25(.a(x12), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(x10), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  nand2  g28(.a(x16), .b(new_n71_), .O(new_n72_));
  nor3   g29(.a(new_n72_), .b(new_n70_), .c(new_n65_), .O(z04));
  nand2  g30(.a(new_n64_), .b(new_n46_), .O(new_n74_));
  nand2  g31(.a(x10), .b(x02), .O(new_n75_));
  nor3   g32(.a(new_n75_), .b(new_n74_), .c(new_n50_), .O(z05));
  nand2  g33(.a(x11), .b(new_n50_), .O(new_n77_));
  nor3   g34(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(z06));
  oai21  g35(.a(new_n77_), .b(new_n70_), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n46_), .O(z07));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  inv1   g40(.a(x04), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(x03), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n83_), .c(new_n82_), .d(new_n46_), .O(new_n86_));
  nand3  g43(.a(x02), .b(new_n63_), .c(new_n62_), .O(new_n87_));
  inv1   g44(.a(x18), .O(new_n88_));
  nand3  g45(.a(x19), .b(new_n88_), .c(x17), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n87_), .c(new_n86_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n44_), .c(x18), .d(x01), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n71_), .c(x02), .d(new_n63_), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nand4  g54(.a(x20), .b(new_n97_), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  oai21  g55(.a(new_n98_), .b(new_n94_), .c(new_n92_), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n62_), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(x21), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n99_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n46_), .O(z09));
  inv1   g61(.a(new_n94_), .O(new_n105_));
  nor2   g62(.a(x21), .b(new_n91_), .O(new_n106_));
  nand4  g63(.a(x16), .b(x15), .c(new_n95_), .d(x12), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n106_), .c(new_n105_), .d(new_n100_), .O(new_n109_));
  inv1   g66(.a(x21), .O(new_n110_));
  nor2   g67(.a(new_n92_), .b(new_n110_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n46_), .c(x22), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n109_), .c(x00), .O(z10));
  nand2  g70(.a(x15), .b(new_n95_), .O(new_n114_));
  nand3  g71(.a(new_n110_), .b(x20), .c(new_n97_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n105_), .c(new_n111_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(new_n101_), .c(new_n46_), .O(z11));
  nand2  g75(.a(new_n75_), .b(new_n64_), .O(new_n119_));
  nand2  g76(.a(new_n60_), .b(new_n59_), .O(new_n120_));
  inv1   g77(.a(x23), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(new_n121_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n120_), .c(new_n51_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nor2   g81(.a(x24), .b(new_n50_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n46_), .O(z12));
  nand3  g84(.a(new_n60_), .b(new_n51_), .c(x17), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n46_), .O(z13));
  nor2   g86(.a(new_n57_), .b(new_n52_), .O(z14));
  oai21  g87(.a(x10), .b(new_n63_), .c(x02), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x00), .O(new_n132_));
  nand3  g89(.a(x19), .b(new_n49_), .c(new_n63_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(new_n132_), .c(new_n45_), .O(z15));
  nor3   g91(.a(new_n45_), .b(new_n63_), .c(x00), .O(z16));
  nand2  g92(.a(new_n87_), .b(new_n46_), .O(z17));
endmodule


