// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n127_, new_n128_, new_n129_, new_n131_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(new_n46_), .b(x02), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand2  g10(.a(new_n47_), .b(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n52_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g15(.a(x01), .O(new_n59_));
  inv1   g16(.a(x00), .O(new_n60_));
  inv1   g17(.a(x02), .O(new_n61_));
  nor3   g18(.a(x09), .b(new_n61_), .c(new_n60_), .O(new_n62_));
  nand3  g19(.a(x12), .b(x11), .c(x10), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  aoi21  g21(.a(new_n64_), .b(new_n62_), .c(x08), .O(new_n65_));
  nand4  g22(.a(new_n47_), .b(new_n51_), .c(x18), .d(new_n49_), .O(new_n66_));
  oai21  g23(.a(new_n65_), .b(new_n59_), .c(new_n66_), .O(z03));
  inv1   g24(.a(x08), .O(new_n68_));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n62_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(new_n68_), .c(new_n59_), .O(z04));
  nor2   g29(.a(new_n61_), .b(new_n60_), .O(new_n73_));
  nor2   g30(.a(x08), .b(new_n59_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor3   g32(.a(new_n75_), .b(new_n56_), .c(new_n44_), .O(z05));
  nand3  g33(.a(x11), .b(x10), .c(new_n44_), .O(new_n77_));
  nor2   g34(.a(new_n77_), .b(new_n75_), .O(z06));
  oai21  g35(.a(new_n77_), .b(x12), .c(x01), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n73_), .O(new_n80_));
  nand2  g37(.a(x08), .b(x01), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n80_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nand2  g43(.a(new_n45_), .b(x02), .O(new_n87_));
  nand3  g44(.a(x19), .b(new_n50_), .c(x17), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n51_), .c(x18), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand2  g49(.a(new_n92_), .b(new_n74_), .O(new_n93_));
  nand2  g50(.a(x02), .b(new_n59_), .O(new_n94_));
  inv1   g51(.a(x13), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x12), .c(new_n69_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nor2   g54(.a(x15), .b(x14), .O(new_n98_));
  nor2   g55(.a(new_n90_), .b(x16), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  inv1   g57(.a(x21), .O(new_n101_));
  inv1   g58(.a(x22), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n101_), .c(new_n60_), .O(new_n103_));
  aoi21  g60(.a(new_n100_), .b(new_n93_), .c(new_n103_), .O(z09));
  nand4  g61(.a(new_n92_), .b(new_n74_), .c(x22), .d(x21), .O(new_n105_));
  inv1   g62(.a(x14), .O(new_n106_));
  nand4  g63(.a(new_n101_), .b(x20), .c(x15), .d(new_n106_), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n97_), .c(new_n102_), .d(x16), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n105_), .c(x00), .O(z10));
  nand2  g67(.a(new_n102_), .b(new_n60_), .O(new_n111_));
  nor2   g68(.a(new_n107_), .b(x16), .O(new_n112_));
  nand2  g69(.a(x21), .b(x01), .O(new_n113_));
  nor2   g70(.a(new_n113_), .b(new_n91_), .O(new_n114_));
  aoi21  g71(.a(new_n112_), .b(new_n97_), .c(new_n114_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n111_), .c(new_n81_), .O(z11));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n117_));
  nand3  g74(.a(new_n51_), .b(x17), .c(new_n61_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n117_), .c(new_n46_), .O(new_n119_));
  nand2  g76(.a(x01), .b(x00), .O(new_n120_));
  aoi21  g77(.a(x10), .b(x02), .c(new_n120_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n44_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n119_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n81_), .O(z12));
  nor2   g81(.a(new_n118_), .b(new_n46_), .O(z13));
  nor2   g82(.a(new_n57_), .b(new_n48_), .O(z14));
  nand3  g83(.a(new_n74_), .b(new_n56_), .c(x00), .O(new_n127_));
  aoi21  g84(.a(x19), .b(new_n59_), .c(x00), .O(new_n128_));
  nand2  g85(.a(new_n81_), .b(new_n61_), .O(new_n129_));
  oai21  g86(.a(new_n129_), .b(new_n128_), .c(new_n127_), .O(z15));
  nand3  g87(.a(new_n68_), .b(x01), .c(new_n60_), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z16));
  inv1   g89(.a(new_n87_), .O(z17));
endmodule


