// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n123_, new_n124_, new_n127_;
  inv1   g00(.a(x21), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x16), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x19), .O(new_n50_));
  nor2   g07(.a(x18), .b(x17), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g10(.a(x10), .O(new_n54_));
  nand3  g11(.a(new_n51_), .b(new_n45_), .c(new_n50_), .O(new_n55_));
  nand3  g12(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n56_));
  nor3   g13(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(z01));
  nand4  g14(.a(new_n51_), .b(new_n45_), .c(new_n50_), .d(new_n54_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n56_), .O(z02));
  inv1   g16(.a(new_n45_), .O(new_n60_));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n47_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x12), .O(new_n64_));
  inv1   g21(.a(x17), .O(new_n65_));
  nor2   g22(.a(x19), .b(x02), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n48_), .c(x18), .d(new_n65_), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(new_n64_), .c(new_n60_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(x10), .d(new_n47_), .O(new_n71_));
  oai21  g28(.a(new_n71_), .b(new_n61_), .c(new_n45_), .O(z04));
  nand2  g29(.a(x10), .b(x02), .O(new_n73_));
  inv1   g30(.a(x00), .O(new_n74_));
  inv1   g31(.a(x01), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x09), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n73_), .c(new_n45_), .O(z05));
  oai21  g35(.a(new_n62_), .b(new_n61_), .c(new_n45_), .O(z06));
  oai21  g36(.a(new_n62_), .b(x12), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n45_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nor2   g43(.a(x18), .b(new_n65_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n48_), .c(x19), .d(x02), .O(new_n88_));
  oai21  g45(.a(new_n88_), .b(new_n86_), .c(new_n45_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n50_), .c(x18), .d(x01), .O(new_n91_));
  nor2   g48(.a(x14), .b(x13), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n69_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  nor2   g51(.a(new_n90_), .b(x15), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n94_), .c(x02), .d(new_n75_), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n44_), .c(new_n97_), .d(new_n74_), .O(new_n99_));
  aoi21  g56(.a(new_n96_), .b(new_n91_), .c(new_n99_), .O(z09));
  inv1   g57(.a(new_n91_), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(x21), .O(new_n102_));
  nand2  g59(.a(x22), .b(new_n74_), .O(new_n103_));
  oai21  g60(.a(new_n103_), .b(new_n102_), .c(new_n45_), .O(z10));
  nand2  g61(.a(new_n98_), .b(new_n74_), .O(new_n105_));
  nand4  g62(.a(new_n44_), .b(x20), .c(new_n97_), .d(x15), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n94_), .c(x02), .d(new_n75_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n102_), .c(new_n105_), .O(z11));
  nand2  g66(.a(new_n73_), .b(new_n76_), .O(new_n110_));
  nand2  g67(.a(new_n66_), .b(new_n65_), .O(new_n111_));
  inv1   g68(.a(x23), .O(new_n112_));
  oai21  g69(.a(x19), .b(x02), .c(new_n112_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n111_), .c(new_n48_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n110_), .O(new_n115_));
  nor2   g72(.a(x24), .b(new_n47_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n45_), .O(z12));
  nor2   g75(.a(x02), .b(x01), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(x17), .c(new_n74_), .O(new_n120_));
  nor3   g77(.a(new_n120_), .b(new_n60_), .c(x19), .O(z13));
  nor2   g78(.a(new_n58_), .b(new_n49_), .O(z14));
  aoi21  g79(.a(new_n54_), .b(x01), .c(new_n46_), .O(new_n123_));
  aoi21  g80(.a(new_n119_), .b(x19), .c(new_n60_), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n74_), .c(new_n124_), .O(z15));
  nor3   g82(.a(new_n60_), .b(new_n75_), .c(x00), .O(z16));
  nand2  g83(.a(new_n48_), .b(x02), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(new_n60_), .O(z17));
endmodule


