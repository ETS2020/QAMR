// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n126_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x16), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x13), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  inv1   g03(.a(x02), .O(new_n47_));
  inv1   g04(.a(x09), .O(new_n48_));
  nor2   g05(.a(x01), .b(x00), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n46_), .c(x10), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g10(.a(x10), .O(new_n54_));
  nand3  g11(.a(new_n49_), .b(new_n46_), .c(new_n45_), .O(new_n55_));
  nand2  g12(.a(x09), .b(new_n47_), .O(new_n56_));
  nor3   g13(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(z01));
  nor3   g14(.a(new_n56_), .b(new_n55_), .c(x10), .O(z02));
  inv1   g15(.a(new_n45_), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand3  g18(.a(x11), .b(x10), .c(new_n48_), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(new_n61_), .c(x12), .O(new_n64_));
  inv1   g21(.a(x19), .O(new_n65_));
  nor2   g22(.a(x17), .b(x02), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n49_), .c(new_n65_), .d(x18), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(new_n64_), .c(new_n59_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(x10), .d(new_n48_), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n60_), .c(new_n59_), .O(z04));
  nand4  g29(.a(new_n61_), .b(new_n45_), .c(x10), .d(x09), .O(new_n73_));
  inv1   g30(.a(new_n73_), .O(z05));
  oai21  g31(.a(new_n62_), .b(new_n60_), .c(new_n45_), .O(z06));
  oai21  g32(.a(new_n62_), .b(x12), .c(x01), .O(new_n76_));
  nand3  g33(.a(new_n76_), .b(x02), .c(x00), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n45_), .O(z07));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  inv1   g37(.a(x04), .O(new_n81_));
  nor2   g38(.a(new_n81_), .b(x03), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n80_), .c(new_n79_), .d(new_n45_), .O(new_n83_));
  inv1   g40(.a(x00), .O(new_n84_));
  inv1   g41(.a(x01), .O(new_n85_));
  nand3  g42(.a(x02), .b(new_n85_), .c(new_n84_), .O(new_n86_));
  inv1   g43(.a(x18), .O(new_n87_));
  nand3  g44(.a(x19), .b(new_n87_), .c(x17), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n86_), .c(new_n83_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n65_), .c(x18), .d(x01), .O(new_n91_));
  nand4  g48(.a(x12), .b(new_n69_), .c(x02), .d(new_n85_), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  nand4  g51(.a(x20), .b(new_n44_), .c(new_n94_), .d(new_n93_), .O(new_n95_));
  oai21  g52(.a(new_n95_), .b(new_n92_), .c(new_n91_), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(new_n84_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(x21), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n96_), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n45_), .O(z09));
  nand2  g58(.a(x22), .b(x21), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(new_n91_), .O(new_n103_));
  nor2   g60(.a(new_n92_), .b(x13), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(x20), .c(x15), .d(new_n93_), .O(new_n106_));
  nand2  g63(.a(new_n97_), .b(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n104_), .c(new_n103_), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(x00), .c(new_n45_), .O(z10));
  nor2   g67(.a(new_n106_), .b(x16), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n104_), .O(new_n112_));
  inv1   g69(.a(new_n91_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n45_), .c(x21), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n112_), .c(new_n98_), .O(z11));
  nand2  g72(.a(x01), .b(x00), .O(new_n116_));
  aoi21  g73(.a(x10), .b(x02), .c(new_n116_), .O(new_n117_));
  inv1   g74(.a(new_n49_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n65_), .b(x17), .c(new_n47_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n48_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n117_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n45_), .O(z12));
  oai21  g81(.a(new_n120_), .b(new_n118_), .c(new_n45_), .O(z13));
  nand3  g82(.a(new_n51_), .b(new_n46_), .c(new_n54_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n45_), .O(z14));
  oai21  g84(.a(x10), .b(new_n85_), .c(x02), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(x00), .O(new_n129_));
  nand3  g86(.a(x19), .b(new_n47_), .c(new_n85_), .O(new_n130_));
  nand3  g87(.a(new_n130_), .b(new_n129_), .c(new_n45_), .O(z15));
  nor3   g88(.a(new_n59_), .b(new_n85_), .c(x00), .O(z16));
  nand2  g89(.a(new_n86_), .b(new_n45_), .O(z17));
endmodule


