// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n116_, new_n117_,
    new_n119_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  nor2   g02(.a(x18), .b(x17), .O(new_n46_));
  nor2   g03(.a(x09), .b(x02), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x00), .O(z00));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x02), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x00), .O(z01));
  nor2   g10(.a(x19), .b(x02), .O(new_n54_));
  nor2   g11(.a(x01), .b(x00), .O(new_n55_));
  nand3  g12(.a(new_n55_), .b(new_n54_), .c(new_n46_), .O(new_n56_));
  nor3   g13(.a(new_n56_), .b(x10), .c(new_n50_), .O(z02));
  inv1   g14(.a(x02), .O(new_n58_));
  inv1   g15(.a(x10), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g17(.a(new_n60_), .b(x11), .c(new_n50_), .O(new_n61_));
  inv1   g18(.a(x00), .O(new_n62_));
  nor2   g19(.a(new_n44_), .b(new_n62_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x12), .O(new_n64_));
  nand2  g21(.a(new_n58_), .b(new_n44_), .O(new_n65_));
  inv1   g22(.a(x17), .O(new_n66_));
  nand4  g23(.a(new_n45_), .b(x18), .c(new_n66_), .d(new_n62_), .O(new_n67_));
  oai22  g24(.a(new_n67_), .b(new_n65_), .c(new_n64_), .d(new_n61_), .O(z03));
  inv1   g25(.a(x12), .O(new_n69_));
  nor2   g26(.a(x11), .b(new_n58_), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(x10), .d(new_n50_), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x00), .c(new_n44_), .O(z04));
  nand2  g29(.a(new_n60_), .b(x09), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x00), .c(new_n44_), .O(z05));
  aoi21  g31(.a(new_n61_), .b(x00), .c(new_n44_), .O(z06));
  nand4  g32(.a(new_n69_), .b(x11), .c(x10), .d(new_n50_), .O(new_n76_));
  inv1   g33(.a(new_n55_), .O(new_n77_));
  oai21  g34(.a(x02), .b(new_n62_), .c(new_n77_), .O(new_n78_));
  aoi21  g35(.a(new_n76_), .b(new_n63_), .c(new_n78_), .O(z07));
  inv1   g36(.a(x08), .O(new_n80_));
  inv1   g37(.a(x18), .O(new_n81_));
  nand4  g38(.a(x19), .b(new_n81_), .c(x17), .d(new_n80_), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  nor2   g40(.a(x07), .b(x06), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand3  g43(.a(new_n55_), .b(new_n86_), .c(x02), .O(new_n87_));
  nor3   g44(.a(new_n87_), .b(new_n85_), .c(new_n82_), .O(z08));
  inv1   g45(.a(x13), .O(new_n89_));
  inv1   g46(.a(x14), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n89_), .c(x12), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nor2   g53(.a(x16), .b(x15), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(new_n92_), .d(new_n70_), .O(new_n98_));
  aoi21  g55(.a(new_n98_), .b(new_n44_), .c(x00), .O(z09));
  and2   g56(.a(x16), .b(x15), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n96_), .c(new_n92_), .d(new_n70_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n44_), .c(x00), .O(z10));
  inv1   g59(.a(x15), .O(new_n103_));
  nor2   g60(.a(x16), .b(new_n103_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n96_), .c(new_n92_), .d(new_n70_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n44_), .c(x00), .O(z11));
  oai21  g63(.a(new_n59_), .b(new_n58_), .c(new_n63_), .O(new_n107_));
  oai21  g64(.a(x19), .b(x02), .c(x23), .O(new_n108_));
  nand3  g65(.a(new_n45_), .b(x17), .c(new_n58_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n55_), .O(new_n111_));
  inv1   g68(.a(x24), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(x09), .O(new_n113_));
  aoi21  g70(.a(new_n111_), .b(new_n107_), .c(new_n113_), .O(z12));
  nor2   g71(.a(new_n109_), .b(new_n77_), .O(z13));
  inv1   g72(.a(new_n56_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n59_), .c(new_n50_), .O(new_n117_));
  inv1   g74(.a(new_n117_), .O(z14));
  aoi21  g75(.a(new_n59_), .b(x01), .c(new_n58_), .O(new_n119_));
  oai22  g76(.a(new_n119_), .b(new_n62_), .c(new_n65_), .d(new_n45_), .O(z15));
  aoi21  g77(.a(new_n58_), .b(new_n44_), .c(x00), .O(z17));
  zero   g78(.O(z16));
endmodule


