// Benchmark "FAU" written by ABC on Tue Aug 11 19:57:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n125_, new_n128_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n46_), .c(new_n45_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x01), .O(z00));
  nor2   g10(.a(x01), .b(x00), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(x09), .c(new_n45_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n50_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g16(.a(x12), .O(new_n60_));
  nand4  g17(.a(x11), .b(x10), .c(new_n46_), .d(x02), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x00), .O(new_n63_));
  inv1   g20(.a(x01), .O(new_n64_));
  nor2   g21(.a(x19), .b(x02), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(x18), .c(new_n47_), .d(new_n64_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n63_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  nand3  g25(.a(new_n60_), .b(new_n46_), .c(x02), .O(new_n69_));
  inv1   g26(.a(new_n69_), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(new_n68_), .c(x10), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x01), .c(new_n44_), .O(z04));
  nand2  g29(.a(x01), .b(x00), .O(new_n73_));
  nor2   g30(.a(new_n57_), .b(new_n45_), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x09), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n73_), .O(z05));
  nor2   g33(.a(new_n73_), .b(new_n61_), .O(z06));
  nand3  g34(.a(new_n70_), .b(x11), .c(x10), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(x01), .c(new_n44_), .O(z07));
  inv1   g36(.a(x08), .O(new_n80_));
  nand4  g37(.a(x19), .b(new_n48_), .c(x17), .d(new_n80_), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nor2   g39(.a(x03), .b(new_n45_), .O(new_n83_));
  inv1   g40(.a(x04), .O(new_n84_));
  nor2   g41(.a(x05), .b(new_n84_), .O(new_n85_));
  nor2   g42(.a(x07), .b(x06), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(new_n83_), .d(new_n82_), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(new_n44_), .c(x01), .O(z08));
  nor2   g45(.a(x20), .b(x19), .O(new_n89_));
  nand3  g46(.a(new_n89_), .b(x18), .c(x01), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  nand2  g48(.a(new_n68_), .b(x02), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x12), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  nor2   g53(.a(x15), .b(x01), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(x20), .d(new_n91_), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(new_n44_), .O(new_n101_));
  aoi21  g58(.a(new_n98_), .b(new_n90_), .c(new_n101_), .O(z09));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(x20), .O(new_n103_));
  nand2  g60(.a(x16), .b(x15), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n96_), .c(x00), .O(new_n106_));
  nand4  g63(.a(x21), .b(x18), .c(x01), .d(new_n44_), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(new_n89_), .c(x22), .O(new_n109_));
  oai21  g66(.a(new_n106_), .b(x01), .c(new_n109_), .O(z10));
  nand2  g67(.a(new_n91_), .b(x15), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n103_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n96_), .c(x00), .O(new_n113_));
  nand3  g70(.a(new_n108_), .b(new_n89_), .c(new_n100_), .O(new_n114_));
  oai21  g71(.a(new_n113_), .b(x01), .c(new_n114_), .O(z11));
  nor2   g72(.a(new_n74_), .b(new_n44_), .O(new_n116_));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n117_));
  nand2  g74(.a(new_n65_), .b(x17), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n117_), .c(x01), .O(new_n119_));
  nor2   g76(.a(x24), .b(new_n46_), .O(new_n120_));
  oai21  g77(.a(new_n119_), .b(new_n116_), .c(new_n120_), .O(new_n121_));
  nand2  g78(.a(new_n64_), .b(x00), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n121_), .O(z12));
  nor3   g80(.a(new_n118_), .b(x01), .c(x00), .O(z13));
  nand2  g81(.a(new_n54_), .b(new_n45_), .O(new_n125_));
  nor3   g82(.a(new_n58_), .b(new_n125_), .c(x09), .O(z14));
  oai22  g83(.a(new_n74_), .b(new_n73_), .c(new_n125_), .d(new_n49_), .O(z15));
  inv1   g84(.a(new_n73_), .O(new_n128_));
  nor2   g85(.a(new_n128_), .b(new_n54_), .O(z16));
  aoi21  g86(.a(new_n45_), .b(new_n44_), .c(x01), .O(z17));
endmodule


