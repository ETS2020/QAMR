// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n97_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n116_;
  nor2   g00(.a(x21), .b(x11), .O(new_n44_));
  inv1   g01(.a(new_n44_), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  nor3   g12(.a(new_n55_), .b(new_n53_), .c(new_n44_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  inv1   g18(.a(x11), .O(new_n62_));
  inv1   g19(.a(x12), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n61_), .c(x10), .d(new_n47_), .O(new_n65_));
  nor2   g22(.a(x19), .b(x02), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n48_), .c(x18), .d(new_n50_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n65_), .c(new_n45_), .O(z03));
  nand3  g25(.a(new_n61_), .b(x10), .c(new_n47_), .O(new_n69_));
  nand3  g26(.a(x21), .b(new_n63_), .c(new_n62_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n69_), .O(z04));
  nand2  g28(.a(x10), .b(x02), .O(new_n72_));
  nand3  g29(.a(x09), .b(x01), .c(x00), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n72_), .c(new_n45_), .O(z05));
  nor2   g31(.a(new_n69_), .b(new_n62_), .O(z06));
  nand4  g32(.a(new_n63_), .b(x11), .c(x10), .d(new_n47_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n45_), .O(z07));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  inv1   g38(.a(x04), .O(new_n82_));
  nor2   g39(.a(new_n82_), .b(x03), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n81_), .c(new_n80_), .d(new_n45_), .O(new_n84_));
  nor2   g41(.a(x18), .b(new_n50_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n48_), .c(x19), .d(x02), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(new_n84_), .O(z08));
  inv1   g44(.a(x00), .O(new_n88_));
  nand3  g45(.a(x18), .b(x01), .c(new_n88_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nor3   g47(.a(x22), .b(x20), .c(x19), .O(new_n91_));
  nand2  g48(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  aoi21  g49(.a(new_n92_), .b(x11), .c(x21), .O(z09));
  inv1   g50(.a(x20), .O(new_n94_));
  nand4  g51(.a(x22), .b(x21), .c(new_n94_), .d(new_n52_), .O(new_n95_));
  oai21  g52(.a(new_n95_), .b(new_n89_), .c(new_n45_), .O(z10));
  inv1   g53(.a(x21), .O(new_n97_));
  oai21  g54(.a(new_n92_), .b(new_n97_), .c(new_n45_), .O(z11));
  nand3  g55(.a(new_n72_), .b(x01), .c(x00), .O(new_n99_));
  oai21  g56(.a(x19), .b(x02), .c(x23), .O(new_n100_));
  nand3  g57(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  nor2   g60(.a(x24), .b(new_n47_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n45_), .O(new_n105_));
  aoi21  g62(.a(new_n103_), .b(new_n99_), .c(new_n105_), .O(z12));
  nand3  g63(.a(new_n66_), .b(new_n48_), .c(x17), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(new_n45_), .O(z13));
  oai21  g65(.a(new_n58_), .b(new_n49_), .c(new_n45_), .O(z14));
  inv1   g66(.a(x01), .O(new_n110_));
  oai21  g67(.a(x10), .b(new_n110_), .c(x02), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(x00), .O(new_n112_));
  nand3  g69(.a(x19), .b(new_n46_), .c(new_n110_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n112_), .c(new_n44_), .O(z15));
  oai21  g71(.a(new_n110_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g72(.a(new_n48_), .b(x02), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n45_), .O(z17));
endmodule


