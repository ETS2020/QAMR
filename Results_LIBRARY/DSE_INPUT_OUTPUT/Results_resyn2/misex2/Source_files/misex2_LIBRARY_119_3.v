// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n115_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x01), .O(new_n46_));
  nand2  g03(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n49_), .O(z00));
  nand2  g11(.a(new_n48_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z01));
  nand2  g13(.a(x01), .b(x00), .O(new_n57_));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n55_), .c(new_n57_), .O(z02));
  inv1   g17(.a(new_n48_), .O(new_n61_));
  nand3  g18(.a(new_n52_), .b(x18), .c(new_n50_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(new_n57_), .O(z03));
  inv1   g20(.a(x02), .O(new_n67_));
  aoi21  g21(.a(new_n67_), .b(new_n46_), .c(new_n45_), .O(z07));
  inv1   g22(.a(x03), .O(new_n69_));
  nor2   g23(.a(x06), .b(x05), .O(new_n70_));
  nor2   g24(.a(x08), .b(x07), .O(new_n71_));
  nand4  g25(.a(new_n71_), .b(new_n70_), .c(x04), .d(new_n69_), .O(new_n72_));
  nand3  g26(.a(x02), .b(new_n46_), .c(new_n45_), .O(new_n73_));
  nand3  g27(.a(x19), .b(new_n51_), .c(x17), .O(new_n74_));
  nor3   g28(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(z08));
  inv1   g29(.a(x21), .O(new_n76_));
  inv1   g30(.a(x22), .O(new_n77_));
  nand2  g31(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  inv1   g32(.a(x20), .O(new_n79_));
  nand3  g33(.a(new_n79_), .b(new_n52_), .c(x18), .O(new_n80_));
  oai21  g34(.a(new_n80_), .b(new_n78_), .c(new_n45_), .O(new_n81_));
  nand2  g35(.a(new_n81_), .b(x01), .O(new_n82_));
  inv1   g36(.a(new_n73_), .O(new_n83_));
  inv1   g37(.a(x11), .O(new_n84_));
  inv1   g38(.a(x13), .O(new_n85_));
  nand3  g39(.a(new_n85_), .b(x12), .c(new_n84_), .O(new_n86_));
  nand3  g40(.a(new_n77_), .b(new_n76_), .c(x20), .O(new_n87_));
  nor2   g41(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor3   g42(.a(x16), .b(x15), .c(x14), .O(new_n89_));
  nand3  g43(.a(new_n89_), .b(new_n88_), .c(new_n83_), .O(new_n90_));
  nand2  g44(.a(new_n90_), .b(new_n82_), .O(z09));
  inv1   g45(.a(new_n80_), .O(new_n92_));
  nand4  g46(.a(new_n92_), .b(x22), .c(x21), .d(x01), .O(new_n93_));
  inv1   g47(.a(x14), .O(new_n94_));
  nand4  g48(.a(x15), .b(new_n94_), .c(x02), .d(new_n46_), .O(new_n95_));
  inv1   g49(.a(new_n95_), .O(new_n96_));
  nand3  g50(.a(new_n96_), .b(new_n88_), .c(x16), .O(new_n97_));
  aoi21  g51(.a(new_n97_), .b(new_n93_), .c(x00), .O(z10));
  nand3  g52(.a(new_n92_), .b(x21), .c(x01), .O(new_n99_));
  inv1   g53(.a(x16), .O(new_n100_));
  nand3  g54(.a(new_n76_), .b(x20), .c(new_n100_), .O(new_n101_));
  nor2   g55(.a(new_n101_), .b(new_n86_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(new_n96_), .O(new_n103_));
  nand2  g57(.a(new_n77_), .b(new_n45_), .O(new_n104_));
  aoi21  g58(.a(new_n103_), .b(new_n99_), .c(new_n104_), .O(z11));
  inv1   g59(.a(new_n47_), .O(new_n106_));
  oai21  g60(.a(x19), .b(x02), .c(x23), .O(new_n107_));
  nand3  g61(.a(new_n52_), .b(x17), .c(new_n67_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nor2   g63(.a(x24), .b(new_n44_), .O(new_n110_));
  nand3  g64(.a(new_n110_), .b(new_n109_), .c(new_n106_), .O(new_n111_));
  nand2  g65(.a(new_n111_), .b(new_n57_), .O(z12));
  nor2   g66(.a(new_n108_), .b(new_n47_), .O(z13));
  oai21  g67(.a(new_n59_), .b(new_n49_), .c(new_n57_), .O(z14));
  nand2  g68(.a(x19), .b(new_n46_), .O(new_n115_));
  aoi22  g69(.a(new_n115_), .b(new_n45_), .c(x02), .d(new_n46_), .O(z15));
  nor2   g70(.a(new_n46_), .b(x00), .O(z16));
  nand2  g71(.a(new_n73_), .b(new_n57_), .O(z17));
  zero   g72(.O(z04));
  zero   g73(.O(z05));
  zero   g74(.O(z06));
endmodule


