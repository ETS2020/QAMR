// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n129_, new_n130_, new_n131_,
    new_n133_;
  nand2  g00(.a(x21), .b(x16), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n44_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n55_));
  nand3  g12(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n56_));
  nor3   g13(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(z01));
  inv1   g14(.a(new_n55_), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n56_), .O(z02));
  inv1   g17(.a(x12), .O(new_n61_));
  inv1   g18(.a(x00), .O(new_n62_));
  inv1   g19(.a(x01), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x02), .O(new_n65_));
  nand3  g22(.a(x11), .b(x10), .c(new_n46_), .O(new_n66_));
  or2    g23(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nor2   g24(.a(x17), .b(x02), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n47_), .c(new_n51_), .d(x18), .O(new_n69_));
  and2   g26(.a(new_n69_), .b(new_n44_), .O(new_n70_));
  oai21  g27(.a(new_n67_), .b(new_n61_), .c(new_n70_), .O(z03));
  inv1   g28(.a(x11), .O(new_n72_));
  nand4  g29(.a(new_n61_), .b(new_n72_), .c(x10), .d(new_n46_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n65_), .c(new_n44_), .O(z04));
  nor2   g31(.a(new_n54_), .b(new_n45_), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(new_n64_), .c(x09), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n44_), .O(z05));
  nand2  g34(.a(new_n67_), .b(new_n44_), .O(z06));
  oai21  g35(.a(new_n66_), .b(x12), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n44_), .O(z07));
  nand3  g38(.a(new_n47_), .b(new_n44_), .c(x02), .O(new_n82_));
  nand3  g39(.a(x19), .b(new_n50_), .c(x17), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nor2   g41(.a(x06), .b(x05), .O(new_n85_));
  nor2   g42(.a(x08), .b(x07), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(x04), .d(new_n84_), .O(new_n87_));
  nor3   g44(.a(new_n87_), .b(new_n83_), .c(new_n82_), .O(z08));
  inv1   g45(.a(x20), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n51_), .c(x18), .d(x01), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand2  g48(.a(x02), .b(new_n63_), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(x12), .c(new_n72_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  inv1   g52(.a(x14), .O(new_n96_));
  inv1   g53(.a(x15), .O(new_n97_));
  inv1   g54(.a(x16), .O(new_n98_));
  nand4  g55(.a(x20), .b(new_n98_), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  aoi21  g57(.a(new_n100_), .b(new_n95_), .c(new_n91_), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(new_n62_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n101_), .c(new_n44_), .O(z09));
  nor2   g62(.a(new_n90_), .b(new_n102_), .O(new_n106_));
  nand2  g63(.a(x15), .b(new_n96_), .O(new_n107_));
  nand3  g64(.a(new_n103_), .b(x20), .c(x16), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  aoi22  g66(.a(new_n109_), .b(new_n95_), .c(new_n106_), .d(x22), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g68(.a(new_n103_), .b(new_n62_), .O(new_n112_));
  nand3  g69(.a(new_n102_), .b(x20), .c(new_n98_), .O(new_n113_));
  nor2   g70(.a(new_n113_), .b(new_n107_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n95_), .c(new_n106_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n112_), .c(new_n44_), .O(z11));
  inv1   g73(.a(new_n64_), .O(new_n117_));
  nor3   g74(.a(x19), .b(x17), .c(x02), .O(new_n118_));
  nor2   g75(.a(x19), .b(x02), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(x23), .c(new_n47_), .O(new_n120_));
  oai22  g77(.a(new_n120_), .b(new_n118_), .c(new_n75_), .d(new_n117_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n46_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n44_), .O(z12));
  inv1   g81(.a(new_n44_), .O(new_n125_));
  nand4  g82(.a(x17), .b(new_n45_), .c(new_n63_), .d(new_n62_), .O(new_n126_));
  nor3   g83(.a(new_n126_), .b(new_n125_), .c(x19), .O(z13));
  nor2   g84(.a(new_n59_), .b(new_n48_), .O(z14));
  oai21  g85(.a(x10), .b(new_n63_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n45_), .c(new_n63_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n130_), .c(new_n125_), .O(z15));
  nand3  g89(.a(new_n44_), .b(x01), .c(new_n62_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z16));
  inv1   g91(.a(new_n82_), .O(z17));
endmodule


