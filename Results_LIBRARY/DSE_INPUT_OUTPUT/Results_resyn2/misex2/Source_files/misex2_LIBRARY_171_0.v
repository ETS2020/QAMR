// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n123_, new_n124_;
  inv1   g00(.a(x18), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  nor2   g02(.a(x21), .b(x16), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(new_n47_));
  inv1   g04(.a(x10), .O(new_n48_));
  nor2   g05(.a(x17), .b(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n47_), .c(new_n45_), .d(new_n44_), .O(new_n50_));
  inv1   g07(.a(x02), .O(new_n51_));
  inv1   g08(.a(x09), .O(new_n52_));
  nor2   g09(.a(x01), .b(x00), .O(new_n53_));
  nand3  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n50_), .O(z00));
  nand3  g12(.a(new_n53_), .b(x09), .c(new_n51_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n50_), .O(z01));
  inv1   g14(.a(x17), .O(new_n58_));
  nand4  g15(.a(new_n45_), .b(new_n44_), .c(new_n58_), .d(new_n48_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n56_), .c(new_n47_), .O(z02));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g20(.a(x11), .b(x10), .c(new_n52_), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n63_), .c(x12), .d(x02), .O(new_n66_));
  nor2   g23(.a(x19), .b(new_n44_), .O(new_n67_));
  nor2   g24(.a(x02), .b(x01), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n67_), .c(new_n58_), .d(new_n61_), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(new_n66_), .c(new_n46_), .O(z03));
  nand2  g27(.a(new_n63_), .b(x02), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n72_), .c(x10), .d(new_n52_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n71_), .c(new_n47_), .O(z04));
  nand2  g32(.a(new_n63_), .b(new_n47_), .O(new_n76_));
  nand2  g33(.a(x10), .b(x02), .O(new_n77_));
  nor3   g34(.a(new_n77_), .b(new_n76_), .c(new_n52_), .O(z05));
  nor3   g35(.a(new_n76_), .b(new_n64_), .c(new_n51_), .O(z06));
  oai21  g36(.a(new_n64_), .b(x12), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n47_), .O(z07));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  inv1   g41(.a(x04), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n84_), .c(new_n83_), .d(new_n47_), .O(new_n87_));
  nand3  g44(.a(x02), .b(new_n62_), .c(new_n61_), .O(new_n88_));
  nand3  g45(.a(x19), .b(new_n44_), .c(x17), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(z08));
  nor2   g47(.a(new_n62_), .b(x00), .O(new_n91_));
  nor2   g48(.a(x22), .b(x20), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n91_), .c(new_n67_), .O(new_n93_));
  aoi21  g50(.a(new_n93_), .b(x16), .c(x21), .O(z09));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n95_), .c(x20), .d(x15), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand2  g55(.a(x02), .b(new_n62_), .O(new_n99_));
  inv1   g56(.a(x13), .O(new_n100_));
  inv1   g57(.a(x14), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(x12), .d(new_n72_), .O(new_n102_));
  nor2   g59(.a(new_n102_), .b(new_n99_), .O(new_n103_));
  nand2  g60(.a(new_n45_), .b(x18), .O(new_n104_));
  inv1   g61(.a(x20), .O(new_n105_));
  nand4  g62(.a(x22), .b(x21), .c(new_n105_), .d(x01), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g64(.a(new_n103_), .b(new_n98_), .c(new_n107_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(x00), .c(new_n47_), .O(z10));
  or2    g66(.a(new_n93_), .b(new_n95_), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(z11));
  nand2  g68(.a(new_n77_), .b(new_n63_), .O(new_n112_));
  nor3   g69(.a(x19), .b(x17), .c(x02), .O(new_n113_));
  nor2   g70(.a(x19), .b(x02), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(x23), .c(new_n53_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  nor2   g73(.a(x24), .b(new_n52_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n47_), .O(z12));
  nand3  g76(.a(new_n68_), .b(x17), .c(new_n61_), .O(new_n120_));
  nor3   g77(.a(new_n120_), .b(new_n46_), .c(x19), .O(z13));
  oai21  g78(.a(new_n59_), .b(new_n54_), .c(new_n47_), .O(z14));
  aoi21  g79(.a(new_n48_), .b(x01), .c(new_n51_), .O(new_n123_));
  aoi21  g80(.a(new_n68_), .b(x19), .c(new_n46_), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n61_), .c(new_n124_), .O(z15));
  nor3   g82(.a(new_n46_), .b(new_n62_), .c(x00), .O(z16));
  nand2  g83(.a(new_n88_), .b(new_n47_), .O(z17));
endmodule


