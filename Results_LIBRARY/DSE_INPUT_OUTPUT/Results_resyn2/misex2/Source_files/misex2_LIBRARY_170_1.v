// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n135_;
  inv1   g00(.a(x09), .O(new_n44_));
  inv1   g01(.a(x19), .O(new_n45_));
  inv1   g02(.a(x22), .O(new_n46_));
  inv1   g03(.a(x02), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n51_));
  nor2   g08(.a(x18), .b(x17), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n51_), .O(z00));
  inv1   g11(.a(new_n52_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n49_), .O(new_n56_));
  nand3  g13(.a(new_n56_), .b(x10), .c(x09), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(new_n46_), .c(x19), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand3  g16(.a(new_n56_), .b(new_n59_), .c(x09), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(new_n46_), .c(x19), .O(z02));
  nand3  g18(.a(new_n50_), .b(new_n46_), .c(new_n45_), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(x11), .c(x10), .d(new_n44_), .O(new_n65_));
  nand2  g22(.a(x22), .b(new_n45_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(x12), .O(new_n67_));
  inv1   g24(.a(x17), .O(new_n68_));
  nand2  g25(.a(x18), .b(new_n68_), .O(new_n69_));
  oai22  g26(.a(new_n69_), .b(new_n62_), .c(new_n67_), .d(new_n65_), .O(z03));
  inv1   g27(.a(x12), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(x10), .c(new_n44_), .O(new_n72_));
  inv1   g29(.a(x11), .O(new_n73_));
  nand2  g30(.a(new_n64_), .b(new_n73_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n72_), .c(new_n66_), .O(z04));
  nand2  g32(.a(x10), .b(x09), .O(new_n76_));
  oai21  g33(.a(new_n63_), .b(new_n76_), .c(new_n66_), .O(z05));
  nand2  g34(.a(new_n66_), .b(new_n65_), .O(z06));
  inv1   g35(.a(new_n72_), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(x11), .O(new_n80_));
  nand3  g37(.a(new_n66_), .b(x02), .c(x00), .O(new_n81_));
  aoi21  g38(.a(new_n80_), .b(x01), .c(new_n81_), .O(z07));
  inv1   g39(.a(x08), .O(new_n83_));
  inv1   g40(.a(x18), .O(new_n84_));
  nand4  g41(.a(x19), .b(new_n84_), .c(x17), .d(new_n83_), .O(new_n85_));
  inv1   g42(.a(x05), .O(new_n86_));
  nor2   g43(.a(x07), .b(x06), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n86_), .c(x04), .O(new_n88_));
  inv1   g45(.a(x03), .O(new_n89_));
  nand3  g46(.a(new_n48_), .b(new_n89_), .c(x02), .O(new_n90_));
  nor3   g47(.a(new_n90_), .b(new_n88_), .c(new_n85_), .O(z08));
  inv1   g48(.a(x01), .O(new_n92_));
  nand2  g49(.a(new_n73_), .b(x02), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(x12), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nor2   g54(.a(x16), .b(x15), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n97_), .c(x20), .d(new_n92_), .O(new_n99_));
  inv1   g56(.a(x20), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n45_), .c(x18), .d(x01), .O(new_n101_));
  inv1   g58(.a(x00), .O(new_n102_));
  inv1   g59(.a(x21), .O(new_n103_));
  nand3  g60(.a(new_n46_), .b(new_n103_), .c(new_n102_), .O(new_n104_));
  aoi21  g61(.a(new_n101_), .b(new_n99_), .c(new_n104_), .O(z09));
  nand3  g62(.a(new_n46_), .b(x16), .c(x15), .O(new_n106_));
  nand3  g63(.a(new_n48_), .b(new_n103_), .c(x20), .O(new_n107_));
  nor4   g64(.a(new_n107_), .b(new_n106_), .c(new_n96_), .d(new_n93_), .O(z10));
  nand2  g65(.a(new_n46_), .b(new_n102_), .O(new_n109_));
  inv1   g66(.a(new_n101_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(x21), .O(new_n111_));
  inv1   g68(.a(x16), .O(new_n112_));
  nand4  g69(.a(new_n103_), .b(x20), .c(new_n112_), .d(x15), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n97_), .c(new_n92_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n111_), .c(new_n109_), .O(z11));
  nand2  g73(.a(x10), .b(x02), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(x01), .c(x00), .O(new_n118_));
  nor3   g75(.a(x19), .b(x17), .c(x02), .O(new_n119_));
  nor2   g76(.a(x19), .b(x02), .O(new_n120_));
  oai21  g77(.a(new_n120_), .b(x23), .c(new_n48_), .O(new_n121_));
  oai21  g78(.a(new_n121_), .b(new_n119_), .c(new_n118_), .O(new_n122_));
  nor2   g79(.a(x24), .b(new_n44_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n66_), .O(z12));
  nand4  g82(.a(new_n50_), .b(new_n46_), .c(new_n45_), .d(x17), .O(new_n126_));
  inv1   g83(.a(new_n126_), .O(z13));
  nand2  g84(.a(new_n52_), .b(new_n59_), .O(new_n128_));
  nor2   g85(.a(new_n128_), .b(new_n51_), .O(z14));
  oai21  g86(.a(x10), .b(new_n92_), .c(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand3  g88(.a(x19), .b(new_n47_), .c(new_n92_), .O(new_n132_));
  nand3  g89(.a(new_n132_), .b(new_n131_), .c(new_n66_), .O(z15));
  oai21  g90(.a(new_n92_), .b(x00), .c(new_n66_), .O(z16));
  nand2  g91(.a(new_n66_), .b(x02), .O(new_n135_));
  nor3   g92(.a(new_n135_), .b(x01), .c(x00), .O(z17));
endmodule


