// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n126_, new_n127_, new_n128_;
  nand2  g00(.a(x16), .b(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n52_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g15(.a(x16), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(new_n46_), .c(x02), .O(new_n60_));
  inv1   g17(.a(x11), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n56_), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x01), .c(x00), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n60_), .O(z06));
  nand2  g21(.a(z06), .b(x12), .O(new_n65_));
  nor2   g22(.a(x17), .b(x02), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n47_), .c(new_n51_), .d(x18), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n65_), .O(z03));
  inv1   g25(.a(x01), .O(new_n69_));
  nor2   g26(.a(x12), .b(x09), .O(new_n70_));
  nor2   g27(.a(new_n56_), .b(new_n45_), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(new_n61_), .d(x00), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(new_n59_), .c(new_n69_), .O(z04));
  nand4  g30(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(new_n59_), .c(new_n69_), .O(z05));
  nand2  g32(.a(x02), .b(x00), .O(new_n76_));
  aoi21  g33(.a(new_n70_), .b(new_n62_), .c(new_n69_), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n76_), .c(new_n44_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  inv1   g39(.a(x00), .O(new_n83_));
  nand3  g40(.a(x02), .b(new_n69_), .c(new_n83_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(x19), .c(new_n50_), .d(x17), .O(new_n86_));
  oai21  g43(.a(new_n86_), .b(new_n82_), .c(new_n44_), .O(z08));
  inv1   g44(.a(x20), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n51_), .c(x18), .d(x01), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand2  g47(.a(x02), .b(new_n69_), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(x12), .d(new_n61_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  nand3  g53(.a(x20), .b(new_n59_), .c(new_n96_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  aoi21  g55(.a(new_n98_), .b(new_n95_), .c(new_n90_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n83_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n99_), .c(new_n44_), .O(z09));
  inv1   g60(.a(new_n94_), .O(new_n104_));
  nor2   g61(.a(new_n89_), .b(new_n100_), .O(new_n105_));
  nand2  g62(.a(new_n100_), .b(x20), .O(new_n106_));
  nand4  g63(.a(new_n101_), .b(x16), .c(x15), .d(x02), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi22  g65(.a(new_n108_), .b(new_n104_), .c(new_n105_), .d(x22), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g67(.a(new_n101_), .b(new_n83_), .O(new_n111_));
  nand4  g68(.a(new_n100_), .b(x20), .c(new_n59_), .d(x15), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n95_), .c(new_n105_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n111_), .c(new_n44_), .O(z11));
  nor3   g72(.a(new_n71_), .b(new_n69_), .c(new_n83_), .O(new_n116_));
  inv1   g73(.a(new_n47_), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  nor2   g77(.a(x24), .b(new_n46_), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n116_), .c(new_n121_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n44_), .O(z12));
  oai21  g80(.a(new_n119_), .b(new_n117_), .c(new_n44_), .O(z13));
  oai21  g81(.a(new_n57_), .b(new_n48_), .c(new_n44_), .O(z14));
  oai21  g82(.a(x10), .b(new_n69_), .c(x02), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x00), .O(new_n127_));
  nand3  g84(.a(x19), .b(new_n45_), .c(new_n69_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n127_), .c(new_n44_), .O(z15));
  aoi21  g86(.a(new_n59_), .b(x00), .c(new_n69_), .O(z16));
  nand2  g87(.a(new_n84_), .b(new_n44_), .O(z17));
endmodule


