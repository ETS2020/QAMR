// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x20), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x10), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand2  g13(.a(new_n54_), .b(x09), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n49_), .c(new_n45_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n59_), .O(new_n60_));
  oai21  g17(.a(new_n60_), .b(new_n57_), .c(new_n45_), .O(z02));
  inv1   g18(.a(x11), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x09), .O(new_n63_));
  nand3  g20(.a(x10), .b(x02), .c(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n63_), .c(x12), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(x20), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(x01), .O(new_n68_));
  nand4  g25(.a(new_n54_), .b(new_n48_), .c(x18), .d(new_n46_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n68_), .O(z03));
  inv1   g27(.a(x02), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n51_), .O(new_n72_));
  nor2   g29(.a(x12), .b(new_n59_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n72_), .c(new_n62_), .d(new_n50_), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x20), .c(new_n52_), .O(z04));
  nand2  g32(.a(new_n65_), .b(x09), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x20), .c(new_n52_), .O(z05));
  nand2  g34(.a(new_n65_), .b(new_n63_), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(x20), .c(new_n52_), .O(z06));
  nand2  g36(.a(x02), .b(x00), .O(new_n80_));
  aoi21  g37(.a(new_n73_), .b(new_n63_), .c(new_n52_), .O(new_n81_));
  oai21  g38(.a(new_n81_), .b(new_n80_), .c(new_n45_), .O(z07));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(x04), .d(new_n83_), .O(new_n86_));
  nor2   g43(.a(x01), .b(x00), .O(new_n87_));
  nand2  g44(.a(new_n87_), .b(x02), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(z17));
  nand4  g46(.a(z17), .b(x19), .c(new_n47_), .d(x17), .O(new_n90_));
  oai21  g47(.a(new_n90_), .b(new_n86_), .c(new_n45_), .O(z08));
  inv1   g48(.a(x16), .O(new_n92_));
  inv1   g49(.a(x13), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(x12), .c(new_n62_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n88_), .O(new_n95_));
  nand2  g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  nor2   g57(.a(x15), .b(x14), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n96_), .c(new_n45_), .O(z09));
  inv1   g60(.a(x14), .O(new_n104_));
  nand3  g61(.a(new_n100_), .b(x15), .c(new_n104_), .O(new_n105_));
  nand2  g62(.a(new_n95_), .b(x16), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(new_n105_), .c(new_n45_), .O(z10));
  nor2   g64(.a(new_n105_), .b(new_n96_), .O(z11));
  oai21  g65(.a(x19), .b(x02), .c(x23), .O(new_n109_));
  nand3  g66(.a(new_n48_), .b(x17), .c(new_n71_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n109_), .c(new_n53_), .O(new_n111_));
  nand2  g68(.a(x01), .b(x00), .O(new_n112_));
  aoi21  g69(.a(x10), .b(x02), .c(new_n112_), .O(new_n113_));
  nor2   g70(.a(x24), .b(new_n50_), .O(new_n114_));
  oai21  g71(.a(new_n113_), .b(new_n111_), .c(new_n114_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n45_), .O(z12));
  nand2  g73(.a(new_n54_), .b(new_n48_), .O(new_n117_));
  nor2   g74(.a(new_n117_), .b(new_n46_), .O(z13));
  nor2   g75(.a(new_n60_), .b(new_n55_), .O(z14));
  oai21  g76(.a(x10), .b(new_n52_), .c(x02), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x00), .O(new_n121_));
  nand3  g78(.a(x19), .b(new_n71_), .c(new_n52_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n121_), .c(new_n45_), .O(z15));
  aoi21  g80(.a(x20), .b(x00), .c(new_n52_), .O(z16));
endmodule


