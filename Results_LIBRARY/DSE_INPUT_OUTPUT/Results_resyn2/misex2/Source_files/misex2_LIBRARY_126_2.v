// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n131_,
    new_n132_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nor2   g06(.a(x01), .b(x00), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  inv1   g08(.a(new_n51_), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(x23), .c(new_n44_), .O(z00));
  nand3  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n55_));
  nand2  g12(.a(new_n50_), .b(new_n45_), .O(new_n56_));
  nand3  g13(.a(x23), .b(x10), .c(x09), .O(new_n57_));
  nor3   g14(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z01));
  inv1   g15(.a(x23), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(x10), .O(new_n60_));
  nand4  g17(.a(new_n47_), .b(new_n44_), .c(x09), .d(new_n45_), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n51_), .c(new_n60_), .O(z02));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nand3  g20(.a(x23), .b(x11), .c(x10), .O(new_n64_));
  nor3   g21(.a(new_n64_), .b(new_n63_), .c(x09), .O(z06));
  nand2  g22(.a(z06), .b(x12), .O(new_n66_));
  nor2   g23(.a(x19), .b(x02), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n47_), .O(new_n68_));
  nand3  g25(.a(new_n60_), .b(new_n50_), .c(x18), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n68_), .c(new_n66_), .O(z03));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  inv1   g29(.a(new_n63_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n46_), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x23), .c(new_n44_), .O(z04));
  nand2  g32(.a(new_n73_), .b(x09), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x23), .c(new_n44_), .O(z05));
  nand2  g34(.a(x02), .b(x00), .O(new_n78_));
  inv1   g35(.a(x01), .O(new_n79_));
  nand2  g36(.a(new_n60_), .b(new_n79_), .O(new_n80_));
  nor2   g37(.a(new_n44_), .b(x09), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(x23), .c(new_n72_), .d(x11), .O(new_n82_));
  aoi21  g39(.a(new_n82_), .b(new_n80_), .c(new_n78_), .O(z07));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nor2   g41(.a(x08), .b(x07), .O(new_n85_));
  inv1   g42(.a(x04), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(x03), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n85_), .c(new_n84_), .d(new_n60_), .O(new_n88_));
  inv1   g45(.a(x00), .O(new_n89_));
  nand3  g46(.a(x02), .b(new_n79_), .c(new_n89_), .O(new_n90_));
  nand3  g47(.a(x19), .b(new_n48_), .c(x17), .O(new_n91_));
  nor3   g48(.a(new_n91_), .b(new_n90_), .c(new_n88_), .O(z08));
  inv1   g49(.a(x20), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n49_), .c(x18), .d(x01), .O(new_n94_));
  inv1   g51(.a(x16), .O(new_n95_));
  nand2  g52(.a(x02), .b(new_n79_), .O(new_n96_));
  inv1   g53(.a(x13), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(x12), .c(new_n71_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nor2   g56(.a(x15), .b(x14), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n99_), .c(x20), .d(new_n95_), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand4  g60(.a(new_n60_), .b(new_n103_), .c(new_n102_), .d(new_n89_), .O(new_n104_));
  aoi21  g61(.a(new_n101_), .b(new_n94_), .c(new_n104_), .O(z09));
  nand2  g62(.a(new_n60_), .b(new_n89_), .O(new_n106_));
  nor2   g63(.a(new_n94_), .b(new_n102_), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(x22), .O(new_n108_));
  inv1   g65(.a(x14), .O(new_n109_));
  nand2  g66(.a(x15), .b(new_n109_), .O(new_n110_));
  nand4  g67(.a(new_n103_), .b(new_n102_), .c(x20), .d(x16), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n99_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n108_), .c(new_n106_), .O(z10));
  nand3  g71(.a(new_n102_), .b(x20), .c(new_n95_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n110_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n99_), .c(new_n107_), .O(new_n117_));
  nand2  g74(.a(new_n103_), .b(new_n89_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n117_), .c(new_n60_), .O(z11));
  nand2  g76(.a(new_n67_), .b(new_n44_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n59_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n68_), .c(new_n50_), .O(new_n122_));
  oai21  g79(.a(new_n59_), .b(x02), .c(x10), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(x01), .c(x00), .O(new_n124_));
  inv1   g81(.a(x24), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x09), .O(new_n126_));
  aoi21  g83(.a(new_n124_), .b(new_n122_), .c(new_n126_), .O(z12));
  nand3  g84(.a(new_n67_), .b(new_n50_), .c(x17), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n60_), .O(z13));
  nor4   g86(.a(new_n56_), .b(new_n55_), .c(x10), .d(x09), .O(z14));
  oai21  g87(.a(new_n49_), .b(x01), .c(new_n89_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n123_), .c(new_n96_), .O(new_n132_));
  inv1   g89(.a(new_n132_), .O(z15));
  oai21  g90(.a(new_n79_), .b(x00), .c(new_n60_), .O(z16));
  nand2  g91(.a(new_n90_), .b(new_n60_), .O(z17));
endmodule


