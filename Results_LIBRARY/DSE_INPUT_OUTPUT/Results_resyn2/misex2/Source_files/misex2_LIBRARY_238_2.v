// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n132_, new_n133_, new_n134_, new_n136_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(new_n46_), .b(x02), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  inv1   g10(.a(x16), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(x01), .O(new_n55_));
  nand2  g12(.a(new_n47_), .b(x09), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n52_), .c(new_n55_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n56_), .O(z02));
  inv1   g17(.a(x01), .O(new_n61_));
  inv1   g18(.a(x00), .O(new_n62_));
  inv1   g19(.a(x02), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n44_), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(new_n64_), .c(new_n54_), .O(new_n67_));
  nand4  g24(.a(new_n47_), .b(new_n51_), .c(x18), .d(new_n49_), .O(new_n68_));
  oai21  g25(.a(new_n67_), .b(new_n61_), .c(new_n68_), .O(z03));
  nor2   g26(.a(new_n58_), .b(x09), .O(new_n70_));
  nand3  g27(.a(new_n70_), .b(new_n64_), .c(x01), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(x16), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n71_), .O(z04));
  nand2  g32(.a(new_n64_), .b(x01), .O(new_n76_));
  nand3  g33(.a(x16), .b(x10), .c(x09), .O(new_n77_));
  nor2   g34(.a(new_n77_), .b(new_n76_), .O(z05));
  nor3   g35(.a(new_n71_), .b(new_n54_), .c(new_n72_), .O(z06));
  nand3  g36(.a(new_n70_), .b(new_n73_), .c(x11), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(x01), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n64_), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n55_), .O(z07));
  inv1   g40(.a(x03), .O(new_n84_));
  nor2   g41(.a(x06), .b(x05), .O(new_n85_));
  nor2   g42(.a(x08), .b(x07), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(x04), .d(new_n84_), .O(new_n87_));
  nand2  g44(.a(new_n45_), .b(x02), .O(new_n88_));
  inv1   g45(.a(new_n88_), .O(z17));
  nand4  g46(.a(z17), .b(x19), .c(new_n50_), .d(x17), .O(new_n90_));
  oai21  g47(.a(new_n90_), .b(new_n87_), .c(new_n55_), .O(z08));
  nand2  g48(.a(x16), .b(x01), .O(new_n92_));
  inv1   g49(.a(x20), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n51_), .c(x18), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand2  g53(.a(x02), .b(new_n61_), .O(new_n97_));
  inv1   g54(.a(x13), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(x12), .c(new_n72_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nor2   g57(.a(x15), .b(x14), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(x20), .d(new_n54_), .O(new_n102_));
  inv1   g59(.a(x21), .O(new_n103_));
  inv1   g60(.a(x22), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n103_), .c(new_n62_), .O(new_n105_));
  aoi21  g62(.a(new_n102_), .b(new_n96_), .c(new_n105_), .O(z09));
  inv1   g63(.a(x14), .O(new_n107_));
  nand4  g64(.a(new_n103_), .b(x20), .c(x15), .d(new_n107_), .O(new_n108_));
  nand2  g65(.a(new_n104_), .b(x16), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g67(.a(x22), .b(x21), .c(x01), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n94_), .O(new_n112_));
  aoi21  g69(.a(new_n110_), .b(new_n100_), .c(new_n112_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(x00), .c(new_n55_), .O(z10));
  nand2  g71(.a(new_n104_), .b(new_n62_), .O(new_n115_));
  nand2  g72(.a(new_n95_), .b(x21), .O(new_n116_));
  inv1   g73(.a(new_n108_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n100_), .c(new_n54_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(z11));
  inv1   g76(.a(x23), .O(new_n120_));
  nor2   g77(.a(x19), .b(x02), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x17), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n120_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n45_), .O(new_n124_));
  nand2  g81(.a(x10), .b(x02), .O(new_n125_));
  nand4  g82(.a(new_n125_), .b(x16), .c(x01), .d(x00), .O(new_n126_));
  inv1   g83(.a(x24), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(x09), .O(new_n128_));
  aoi21  g85(.a(new_n126_), .b(new_n124_), .c(new_n128_), .O(z12));
  oai21  g86(.a(new_n122_), .b(new_n46_), .c(new_n55_), .O(z13));
  nor2   g87(.a(new_n59_), .b(new_n48_), .O(z14));
  oai21  g88(.a(x10), .b(new_n61_), .c(x02), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(x00), .O(new_n133_));
  nand3  g90(.a(x19), .b(new_n63_), .c(new_n61_), .O(new_n134_));
  nand3  g91(.a(new_n134_), .b(new_n133_), .c(new_n55_), .O(z15));
  nand3  g92(.a(x16), .b(x01), .c(new_n62_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z16));
endmodule


