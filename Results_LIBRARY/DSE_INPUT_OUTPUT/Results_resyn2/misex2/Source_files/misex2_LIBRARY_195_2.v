// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n74_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n128_, new_n130_, new_n131_,
    new_n132_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  inv1   g03(.a(x16), .O(new_n47_));
  nor2   g04(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n50_));
  inv1   g07(.a(x02), .O(new_n51_));
  inv1   g08(.a(x09), .O(new_n52_));
  nor2   g09(.a(x01), .b(x00), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(x10), .c(new_n52_), .d(new_n51_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n50_), .O(z00));
  nand4  g12(.a(new_n53_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n56_));
  nand3  g13(.a(x10), .b(x09), .c(new_n51_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n56_), .c(new_n49_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n53_), .b(new_n59_), .c(x09), .d(new_n51_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n50_), .O(z02));
  nand3  g18(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nor2   g20(.a(new_n59_), .b(x09), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x11), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n63_), .c(x12), .O(new_n67_));
  nor2   g24(.a(x19), .b(x02), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n53_), .c(x18), .d(new_n44_), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(new_n67_), .c(new_n49_), .O(z03));
  nor2   g27(.a(x12), .b(x11), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n64_), .c(new_n63_), .d(new_n49_), .O(new_n72_));
  inv1   g29(.a(new_n72_), .O(z04));
  nand3  g30(.a(new_n63_), .b(x10), .c(x09), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n49_), .O(z05));
  oai21  g32(.a(new_n65_), .b(new_n62_), .c(new_n49_), .O(z06));
  inv1   g33(.a(x12), .O(new_n77_));
  nand2  g34(.a(new_n66_), .b(new_n77_), .O(new_n78_));
  nand3  g35(.a(new_n49_), .b(x02), .c(x00), .O(new_n79_));
  aoi21  g36(.a(new_n78_), .b(x01), .c(new_n79_), .O(z07));
  nor2   g37(.a(new_n44_), .b(x08), .O(new_n81_));
  inv1   g38(.a(x05), .O(new_n82_));
  nor2   g39(.a(x07), .b(x06), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand3  g42(.a(new_n53_), .b(new_n85_), .c(x02), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n81_), .c(x19), .d(new_n45_), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(new_n49_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n46_), .c(x18), .d(x01), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  inv1   g49(.a(x01), .O(new_n93_));
  nand2  g50(.a(x02), .b(new_n93_), .O(new_n94_));
  inv1   g51(.a(x11), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  inv1   g53(.a(x14), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(x12), .d(new_n95_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n94_), .O(new_n99_));
  inv1   g56(.a(x15), .O(new_n100_));
  nand3  g57(.a(x20), .b(new_n47_), .c(new_n100_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n99_), .c(new_n92_), .O(new_n103_));
  inv1   g60(.a(x00), .O(new_n104_));
  inv1   g61(.a(x21), .O(new_n105_));
  inv1   g62(.a(x22), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n103_), .c(new_n49_), .O(z09));
  nand4  g65(.a(new_n92_), .b(new_n49_), .c(x22), .d(x21), .O(new_n109_));
  nand4  g66(.a(x24), .b(new_n106_), .c(x20), .d(x16), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n99_), .c(new_n105_), .d(x15), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n109_), .c(x00), .O(z10));
  nand2  g70(.a(new_n106_), .b(new_n104_), .O(new_n114_));
  nand3  g71(.a(new_n92_), .b(new_n49_), .c(x21), .O(new_n115_));
  nor2   g72(.a(new_n90_), .b(x16), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(new_n99_), .c(new_n105_), .d(x15), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(z11));
  nand2  g75(.a(x01), .b(x00), .O(new_n119_));
  aoi21  g76(.a(x10), .b(x02), .c(new_n119_), .O(new_n120_));
  nand2  g77(.a(new_n93_), .b(new_n104_), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(x23), .O(new_n122_));
  nand3  g79(.a(new_n46_), .b(x17), .c(new_n51_), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n122_), .c(new_n121_), .O(new_n124_));
  oai21  g81(.a(new_n124_), .b(new_n120_), .c(x09), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n47_), .c(x24), .O(z12));
  oai21  g83(.a(new_n123_), .b(new_n121_), .c(new_n49_), .O(z13));
  nand3  g84(.a(new_n59_), .b(new_n52_), .c(new_n51_), .O(new_n128_));
  oai21  g85(.a(new_n128_), .b(new_n56_), .c(new_n49_), .O(z14));
  oai21  g86(.a(x10), .b(new_n93_), .c(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand3  g88(.a(x19), .b(new_n51_), .c(new_n93_), .O(new_n132_));
  nand3  g89(.a(new_n132_), .b(new_n131_), .c(new_n49_), .O(z15));
  nor3   g90(.a(new_n48_), .b(new_n93_), .c(x00), .O(z16));
  nor3   g91(.a(new_n121_), .b(new_n48_), .c(new_n51_), .O(z17));
endmodule


