// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n125_, new_n127_, new_n128_;
  inv1   g00(.a(x01), .O(new_n44_));
  nor2   g01(.a(x09), .b(x02), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(new_n44_), .c(x00), .O(z00));
  inv1   g05(.a(x02), .O(new_n49_));
  nand4  g06(.a(new_n46_), .b(x10), .c(x09), .d(new_n49_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x00), .O(z01));
  inv1   g08(.a(x10), .O(new_n52_));
  nand2  g09(.a(new_n46_), .b(new_n52_), .O(new_n53_));
  nor2   g10(.a(x01), .b(x00), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(x09), .c(new_n49_), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n53_), .O(z02));
  inv1   g13(.a(x00), .O(new_n57_));
  inv1   g14(.a(x17), .O(new_n58_));
  nor2   g15(.a(x19), .b(x02), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x18), .c(new_n58_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n44_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nand2  g19(.a(x10), .b(x02), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  inv1   g21(.a(x11), .O(new_n65_));
  nor2   g22(.a(new_n65_), .b(x09), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n64_), .c(x12), .d(x01), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n62_), .O(z03));
  inv1   g25(.a(x09), .O(new_n69_));
  nand3  g26(.a(new_n65_), .b(new_n69_), .c(x02), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(x10), .c(x01), .d(x00), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n70_), .O(z04));
  nand2  g30(.a(new_n64_), .b(x09), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x00), .c(new_n44_), .O(z05));
  nand2  g32(.a(new_n66_), .b(new_n64_), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x00), .c(new_n44_), .O(z06));
  nand3  g34(.a(new_n66_), .b(new_n71_), .c(x10), .O(new_n78_));
  nand2  g35(.a(x02), .b(x00), .O(new_n79_));
  aoi21  g36(.a(new_n78_), .b(x01), .c(new_n79_), .O(z07));
  inv1   g37(.a(x08), .O(new_n81_));
  inv1   g38(.a(x18), .O(new_n82_));
  nand4  g39(.a(x19), .b(new_n82_), .c(x17), .d(new_n81_), .O(new_n83_));
  inv1   g40(.a(new_n83_), .O(new_n84_));
  nor2   g41(.a(x03), .b(new_n49_), .O(new_n85_));
  inv1   g42(.a(x05), .O(new_n86_));
  inv1   g43(.a(x06), .O(new_n87_));
  inv1   g44(.a(x07), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x04), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n85_), .c(new_n84_), .O(new_n91_));
  aoi21  g48(.a(new_n91_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g49(.a(x15), .O(new_n93_));
  nand2  g50(.a(new_n65_), .b(x02), .O(new_n94_));
  inv1   g51(.a(x13), .O(new_n95_));
  inv1   g52(.a(x14), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(x12), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n99_), .c(x20), .O(new_n101_));
  inv1   g58(.a(x16), .O(new_n102_));
  nand2  g59(.a(new_n54_), .b(new_n102_), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(new_n98_), .c(new_n93_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(z09));
  inv1   g63(.a(new_n94_), .O(new_n107_));
  inv1   g64(.a(new_n97_), .O(new_n108_));
  inv1   g65(.a(new_n101_), .O(new_n109_));
  nor2   g66(.a(new_n102_), .b(new_n93_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n109_), .c(new_n108_), .d(new_n107_), .O(new_n111_));
  aoi21  g68(.a(new_n111_), .b(new_n44_), .c(x00), .O(z10));
  nand3  g69(.a(new_n104_), .b(new_n98_), .c(x15), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(z11));
  nand3  g71(.a(new_n63_), .b(x01), .c(x00), .O(new_n115_));
  nand2  g72(.a(new_n59_), .b(new_n58_), .O(new_n116_));
  inv1   g73(.a(x23), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(new_n117_), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(new_n116_), .c(new_n54_), .O(new_n119_));
  inv1   g76(.a(x24), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x09), .O(new_n121_));
  aoi21  g78(.a(new_n119_), .b(new_n115_), .c(new_n121_), .O(z12));
  nand2  g79(.a(new_n59_), .b(x17), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n44_), .c(x00), .O(z13));
  nand3  g81(.a(new_n46_), .b(new_n45_), .c(new_n52_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n44_), .c(x00), .O(z14));
  oai21  g83(.a(new_n52_), .b(new_n57_), .c(x01), .O(new_n127_));
  oai21  g84(.a(x19), .b(x00), .c(new_n49_), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n127_), .O(z15));
  nor2   g86(.a(new_n44_), .b(x00), .O(z16));
  aoi21  g87(.a(new_n49_), .b(new_n44_), .c(x00), .O(z17));
endmodule


