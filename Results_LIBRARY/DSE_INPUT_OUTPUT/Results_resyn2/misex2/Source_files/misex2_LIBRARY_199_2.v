// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n130_,
    new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x21), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x18), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  inv1   g03(.a(x18), .O(new_n47_));
  inv1   g04(.a(x19), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x10), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nor2   g08(.a(x01), .b(x00), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n52_), .b(x09), .c(new_n50_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n49_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  nor2   g16(.a(x19), .b(x02), .O(new_n60_));
  nand3  g17(.a(new_n60_), .b(new_n52_), .c(new_n46_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(x21), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x18), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand3  g22(.a(x11), .b(x10), .c(new_n51_), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n65_), .c(x12), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n63_), .O(z03));
  nor2   g26(.a(new_n57_), .b(x09), .O(new_n70_));
  nor2   g27(.a(x12), .b(x11), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(new_n65_), .d(new_n45_), .O(new_n72_));
  inv1   g29(.a(new_n72_), .O(z04));
  nand2  g30(.a(new_n45_), .b(x09), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n64_), .c(new_n57_), .O(z05));
  oai21  g32(.a(new_n66_), .b(new_n64_), .c(new_n45_), .O(z06));
  inv1   g33(.a(x12), .O(new_n77_));
  nand2  g34(.a(new_n67_), .b(new_n77_), .O(new_n78_));
  nand3  g35(.a(new_n45_), .b(x02), .c(x00), .O(new_n79_));
  aoi21  g36(.a(new_n78_), .b(x01), .c(new_n79_), .O(z07));
  inv1   g37(.a(x03), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(x04), .d(new_n81_), .O(new_n84_));
  nand2  g41(.a(new_n52_), .b(x02), .O(new_n85_));
  nand3  g42(.a(x19), .b(new_n47_), .c(x17), .O(new_n86_));
  nor3   g43(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(z08));
  inv1   g44(.a(x22), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n44_), .c(x20), .d(new_n47_), .O(new_n89_));
  inv1   g46(.a(x11), .O(new_n90_));
  nor2   g47(.a(x14), .b(x13), .O(new_n91_));
  nor2   g48(.a(x16), .b(x15), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(x12), .d(new_n90_), .O(new_n93_));
  nor3   g50(.a(new_n93_), .b(new_n89_), .c(new_n85_), .O(z09));
  inv1   g51(.a(new_n89_), .O(new_n95_));
  inv1   g52(.a(x01), .O(new_n96_));
  nand4  g53(.a(x12), .b(new_n90_), .c(x02), .d(new_n96_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  inv1   g55(.a(x13), .O(new_n99_));
  inv1   g56(.a(x14), .O(new_n100_));
  nand4  g57(.a(x16), .b(x15), .c(new_n100_), .d(new_n99_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(new_n98_), .c(new_n95_), .O(new_n103_));
  nor2   g60(.a(new_n47_), .b(new_n96_), .O(new_n104_));
  nor2   g61(.a(x20), .b(x19), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n104_), .c(x22), .d(x21), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n103_), .c(x00), .O(z10));
  nand3  g64(.a(new_n105_), .b(new_n104_), .c(x21), .O(new_n108_));
  nand3  g65(.a(new_n44_), .b(x20), .c(new_n47_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  inv1   g67(.a(x16), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(x15), .c(new_n100_), .d(new_n99_), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n98_), .c(new_n110_), .O(new_n114_));
  inv1   g71(.a(x00), .O(new_n115_));
  nand2  g72(.a(new_n88_), .b(new_n115_), .O(new_n116_));
  aoi21  g73(.a(new_n114_), .b(new_n108_), .c(new_n116_), .O(z11));
  nand2  g74(.a(x10), .b(x02), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(x01), .c(x00), .O(new_n119_));
  nand2  g76(.a(new_n60_), .b(new_n46_), .O(new_n120_));
  inv1   g77(.a(x23), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(new_n121_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n120_), .c(new_n52_), .O(new_n123_));
  inv1   g80(.a(x24), .O(new_n124_));
  nand3  g81(.a(new_n45_), .b(new_n124_), .c(x09), .O(new_n125_));
  aoi21  g82(.a(new_n123_), .b(new_n119_), .c(new_n125_), .O(z12));
  nand3  g83(.a(new_n60_), .b(new_n52_), .c(x17), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n45_), .O(z13));
  nor2   g85(.a(new_n58_), .b(new_n53_), .O(z14));
  inv1   g86(.a(new_n45_), .O(new_n130_));
  oai21  g87(.a(x10), .b(new_n96_), .c(x02), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(x00), .O(new_n132_));
  nand3  g89(.a(x19), .b(new_n50_), .c(new_n96_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(new_n132_), .c(new_n130_), .O(z15));
  nor3   g91(.a(new_n130_), .b(new_n96_), .c(x00), .O(z16));
  nor2   g92(.a(new_n85_), .b(new_n130_), .O(z17));
endmodule


