// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n127_;
  nor3   g00(.a(x19), .b(x18), .c(x17), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x10), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor3   g05(.a(new_n48_), .b(new_n45_), .c(x09), .O(z00));
  inv1   g06(.a(x00), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x02), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n44_), .c(x10), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n50_), .c(x01), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand3  g12(.a(new_n52_), .b(new_n44_), .c(new_n55_), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n50_), .c(x01), .O(z02));
  inv1   g14(.a(x17), .O(new_n58_));
  inv1   g15(.a(x19), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(new_n58_), .c(new_n46_), .O(new_n60_));
  nand2  g17(.a(new_n47_), .b(x18), .O(new_n61_));
  nand2  g18(.a(x10), .b(x02), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(x11), .c(new_n51_), .O(new_n64_));
  nand3  g21(.a(x12), .b(x01), .c(x00), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n61_), .d(new_n60_), .O(z03));
  inv1   g23(.a(x01), .O(new_n67_));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand3  g26(.a(new_n69_), .b(new_n68_), .c(new_n51_), .O(new_n70_));
  nor4   g27(.a(new_n70_), .b(new_n62_), .c(new_n67_), .d(new_n50_), .O(z04));
  nand2  g28(.a(new_n63_), .b(x09), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n50_), .O(z05));
  aoi21  g30(.a(new_n64_), .b(x01), .c(new_n50_), .O(z06));
  inv1   g31(.a(new_n64_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(new_n69_), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x01), .c(new_n50_), .O(z07));
  inv1   g34(.a(x03), .O(new_n78_));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n79_), .c(x04), .d(new_n78_), .O(new_n81_));
  nand2  g38(.a(new_n47_), .b(x02), .O(new_n82_));
  inv1   g39(.a(x18), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n83_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(z08));
  nand2  g42(.a(new_n68_), .b(x02), .O(new_n86_));
  inv1   g43(.a(x13), .O(new_n87_));
  inv1   g44(.a(x14), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n87_), .c(x12), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  nand2  g48(.a(x20), .b(new_n91_), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  inv1   g50(.a(x21), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n92_), .O(new_n97_));
  aoi21  g54(.a(new_n97_), .b(new_n90_), .c(x00), .O(new_n98_));
  nand2  g55(.a(new_n95_), .b(new_n94_), .O(new_n99_));
  nor2   g56(.a(new_n99_), .b(x00), .O(new_n100_));
  nor2   g57(.a(x20), .b(x19), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(x18), .d(x01), .O(new_n102_));
  oai21  g59(.a(new_n98_), .b(x01), .c(new_n102_), .O(z09));
  nand3  g60(.a(x20), .b(x16), .c(x15), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n99_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n90_), .c(x00), .O(new_n106_));
  nand3  g63(.a(x21), .b(x18), .c(x01), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n101_), .c(x22), .d(new_n50_), .O(new_n109_));
  oai21  g66(.a(new_n106_), .b(x01), .c(new_n109_), .O(z10));
  nand2  g67(.a(new_n108_), .b(new_n101_), .O(new_n111_));
  nand3  g68(.a(new_n94_), .b(x15), .c(new_n67_), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n92_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n90_), .O(new_n114_));
  nand2  g71(.a(new_n95_), .b(new_n50_), .O(new_n115_));
  aoi21  g72(.a(new_n114_), .b(new_n111_), .c(new_n115_), .O(z11));
  nand3  g73(.a(new_n62_), .b(x01), .c(x00), .O(new_n117_));
  inv1   g74(.a(x23), .O(new_n118_));
  nand2  g75(.a(new_n59_), .b(new_n46_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n60_), .c(new_n47_), .O(new_n121_));
  inv1   g78(.a(x24), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x09), .O(new_n123_));
  aoi21  g80(.a(new_n121_), .b(new_n117_), .c(new_n123_), .O(z12));
  nand2  g81(.a(new_n47_), .b(x17), .O(new_n125_));
  nor2   g82(.a(new_n125_), .b(new_n119_), .O(z13));
  nand4  g83(.a(new_n44_), .b(new_n55_), .c(new_n51_), .d(new_n46_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(new_n50_), .c(x01), .O(z14));
  oai21  g85(.a(new_n48_), .b(new_n59_), .c(new_n117_), .O(z15));
  nor2   g86(.a(new_n67_), .b(x00), .O(z16));
  inv1   g87(.a(new_n82_), .O(z17));
endmodule


