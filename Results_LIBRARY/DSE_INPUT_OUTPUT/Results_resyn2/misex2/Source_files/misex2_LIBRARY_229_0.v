// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n119_, new_n120_, new_n121_;
  nor2   g00(.a(x16), .b(x14), .O(new_n44_));
  inv1   g01(.a(new_n44_), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  nor2   g03(.a(x18), .b(x17), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n45_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n45_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n47_), .b(new_n45_), .c(new_n46_), .d(new_n56_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  nand3  g15(.a(x02), .b(x01), .c(x00), .O(new_n59_));
  inv1   g16(.a(new_n59_), .O(new_n60_));
  nand3  g17(.a(x11), .b(x10), .c(new_n50_), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(new_n60_), .c(x12), .O(new_n63_));
  inv1   g20(.a(x18), .O(new_n64_));
  nor2   g21(.a(x19), .b(new_n64_), .O(new_n65_));
  nor2   g22(.a(x02), .b(x01), .O(new_n66_));
  nor2   g23(.a(x17), .b(x00), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(new_n63_), .c(new_n44_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n50_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n59_), .c(new_n45_), .O(z04));
  nand2  g30(.a(x10), .b(x02), .O(new_n74_));
  nand3  g31(.a(x09), .b(x01), .c(x00), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n45_), .O(z05));
  nor3   g33(.a(new_n61_), .b(new_n59_), .c(new_n44_), .O(z06));
  nand2  g34(.a(new_n62_), .b(new_n71_), .O(new_n78_));
  nand3  g35(.a(new_n45_), .b(x02), .c(x00), .O(new_n79_));
  aoi21  g36(.a(new_n78_), .b(x01), .c(new_n79_), .O(z07));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  inv1   g39(.a(x04), .O(new_n83_));
  nor2   g40(.a(new_n83_), .b(x03), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n82_), .c(new_n81_), .d(new_n45_), .O(new_n85_));
  inv1   g42(.a(x00), .O(new_n86_));
  inv1   g43(.a(x01), .O(new_n87_));
  nand3  g44(.a(x02), .b(new_n87_), .c(new_n86_), .O(new_n88_));
  nand3  g45(.a(x19), .b(new_n64_), .c(x17), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n88_), .c(new_n85_), .O(z08));
  nor2   g47(.a(x22), .b(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n65_), .c(x01), .d(new_n86_), .O(new_n92_));
  oai21  g49(.a(new_n92_), .b(x21), .c(new_n45_), .O(z09));
  inv1   g50(.a(x21), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(x20), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nor2   g54(.a(x14), .b(x13), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(x16), .c(x15), .O(new_n99_));
  nand4  g56(.a(x12), .b(new_n70_), .c(x02), .d(new_n87_), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n97_), .O(new_n102_));
  nor2   g59(.a(new_n44_), .b(new_n94_), .O(new_n103_));
  nor2   g60(.a(x20), .b(new_n87_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n103_), .c(new_n65_), .d(x22), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n102_), .c(x00), .O(z10));
  nor3   g63(.a(new_n92_), .b(new_n44_), .c(new_n94_), .O(z11));
  nand3  g64(.a(new_n74_), .b(x01), .c(x00), .O(new_n108_));
  nor3   g65(.a(x19), .b(x17), .c(x02), .O(new_n109_));
  nor2   g66(.a(x19), .b(x02), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x23), .c(new_n51_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  nor2   g69(.a(x24), .b(new_n50_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n45_), .O(z12));
  nand3  g72(.a(new_n66_), .b(x17), .c(new_n86_), .O(new_n116_));
  nor3   g73(.a(new_n116_), .b(new_n44_), .c(x19), .O(z13));
  nor2   g74(.a(new_n57_), .b(new_n52_), .O(z14));
  oai21  g75(.a(x10), .b(new_n87_), .c(x02), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(x00), .O(new_n120_));
  nand2  g77(.a(new_n66_), .b(x19), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n120_), .c(new_n44_), .O(z15));
  nor3   g79(.a(new_n44_), .b(new_n87_), .c(x00), .O(z16));
  nand2  g80(.a(new_n88_), .b(new_n45_), .O(z17));
endmodule


