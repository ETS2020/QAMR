// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n130_, new_n131_,
    new_n132_, new_n133_;
  inv1   g00(.a(x01), .O(new_n44_));
  nor2   g01(.a(x15), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nor2   g08(.a(x01), .b(x00), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n50_), .c(new_n46_), .O(z00));
  nand2  g13(.a(new_n54_), .b(x09), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n50_), .c(new_n46_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  inv1   g18(.a(x15), .O(new_n62_));
  nand2  g19(.a(x02), .b(x00), .O(new_n63_));
  nand3  g20(.a(x11), .b(x10), .c(new_n51_), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g22(.a(new_n65_), .b(x12), .c(new_n62_), .O(new_n66_));
  nor2   g23(.a(x19), .b(new_n48_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n54_), .c(new_n47_), .O(new_n68_));
  oai21  g25(.a(new_n66_), .b(new_n44_), .c(new_n68_), .O(z03));
  nand3  g26(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  nor2   g27(.a(x12), .b(x11), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(x15), .c(x10), .d(new_n51_), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n70_), .O(z04));
  nor4   g30(.a(new_n70_), .b(new_n62_), .c(new_n59_), .d(new_n51_), .O(z05));
  nor2   g31(.a(new_n65_), .b(new_n62_), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n44_), .O(z06));
  oai21  g33(.a(new_n64_), .b(x12), .c(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n46_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand2  g40(.a(new_n52_), .b(x02), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n48_), .c(x17), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(z08));
  inv1   g43(.a(x20), .O(new_n87_));
  nand4  g44(.a(new_n67_), .b(new_n87_), .c(x15), .d(x01), .O(new_n88_));
  inv1   g45(.a(x14), .O(new_n89_));
  nand2  g46(.a(x02), .b(new_n44_), .O(new_n90_));
  inv1   g47(.a(x11), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n91_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  nor2   g51(.a(new_n87_), .b(x16), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n94_), .c(new_n62_), .d(new_n89_), .O(new_n96_));
  inv1   g53(.a(x00), .O(new_n97_));
  inv1   g54(.a(x21), .O(new_n98_));
  inv1   g55(.a(x22), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  aoi21  g57(.a(new_n96_), .b(new_n88_), .c(new_n100_), .O(z09));
  nor2   g58(.a(new_n45_), .b(new_n97_), .O(new_n102_));
  nand2  g59(.a(x15), .b(new_n89_), .O(new_n103_));
  nand4  g60(.a(new_n99_), .b(new_n98_), .c(x20), .d(x16), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n94_), .O(new_n106_));
  nand2  g63(.a(new_n87_), .b(x01), .O(new_n107_));
  nand3  g64(.a(x21), .b(new_n49_), .c(x18), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(x22), .c(new_n45_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n106_), .c(new_n102_), .O(z10));
  inv1   g68(.a(x16), .O(new_n112_));
  nand3  g69(.a(new_n98_), .b(x20), .c(new_n112_), .O(new_n113_));
  nor2   g70(.a(new_n113_), .b(new_n103_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n94_), .c(new_n109_), .O(new_n115_));
  nand2  g72(.a(new_n99_), .b(new_n97_), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n115_), .c(new_n46_), .O(z11));
  nand2  g74(.a(x10), .b(x02), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(x15), .c(x01), .d(x00), .O(new_n119_));
  inv1   g76(.a(x23), .O(new_n120_));
  nor2   g77(.a(x19), .b(x02), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x17), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n120_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n52_), .O(new_n124_));
  inv1   g81(.a(x24), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x09), .O(new_n126_));
  aoi21  g83(.a(new_n124_), .b(new_n119_), .c(new_n126_), .O(z12));
  oai21  g84(.a(new_n122_), .b(new_n53_), .c(new_n46_), .O(z13));
  nor2   g85(.a(new_n60_), .b(new_n55_), .O(z14));
  inv1   g86(.a(x02), .O(new_n130_));
  nor2   g87(.a(new_n49_), .b(x01), .O(new_n131_));
  oai21  g88(.a(new_n131_), .b(new_n102_), .c(new_n130_), .O(new_n132_));
  nand4  g89(.a(x15), .b(new_n59_), .c(x01), .d(x00), .O(new_n133_));
  nand2  g90(.a(new_n133_), .b(new_n132_), .O(z15));
  aoi21  g91(.a(x15), .b(x00), .c(new_n44_), .O(z16));
  inv1   g92(.a(new_n84_), .O(z17));
endmodule


