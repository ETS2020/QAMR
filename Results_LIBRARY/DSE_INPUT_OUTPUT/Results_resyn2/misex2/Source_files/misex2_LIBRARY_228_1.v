// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n120_;
  nor3   g00(.a(x19), .b(x18), .c(x17), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x10), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x02), .b(x01), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(new_n45_), .O(z00));
  inv1   g07(.a(x01), .O(new_n51_));
  inv1   g08(.a(x02), .O(new_n52_));
  nand4  g09(.a(new_n44_), .b(x10), .c(x09), .d(new_n52_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n51_), .c(x00), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand2  g12(.a(new_n44_), .b(new_n55_), .O(new_n56_));
  nor2   g13(.a(x01), .b(x00), .O(new_n57_));
  nand3  g14(.a(new_n57_), .b(x09), .c(new_n52_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n56_), .O(z02));
  nand2  g16(.a(x11), .b(new_n47_), .O(new_n60_));
  nand4  g17(.a(x10), .b(x02), .c(x01), .d(x00), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n60_), .O(z06));
  nand2  g19(.a(z06), .b(x12), .O(new_n63_));
  inv1   g20(.a(x17), .O(new_n64_));
  nor2   g21(.a(x19), .b(x00), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n48_), .c(x18), .d(new_n64_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n63_), .O(z03));
  inv1   g24(.a(x12), .O(new_n68_));
  nor2   g25(.a(x11), .b(new_n52_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n47_), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(x00), .c(new_n51_), .O(z04));
  nor2   g28(.a(new_n61_), .b(new_n47_), .O(z05));
  nor2   g29(.a(new_n51_), .b(new_n46_), .O(new_n73_));
  nand4  g30(.a(new_n68_), .b(x11), .c(x10), .d(new_n47_), .O(new_n74_));
  inv1   g31(.a(new_n57_), .O(new_n75_));
  oai21  g32(.a(x02), .b(new_n46_), .c(new_n75_), .O(new_n76_));
  aoi21  g33(.a(new_n74_), .b(new_n73_), .c(new_n76_), .O(z07));
  inv1   g34(.a(x08), .O(new_n78_));
  inv1   g35(.a(x18), .O(new_n79_));
  nand4  g36(.a(x19), .b(new_n79_), .c(x17), .d(new_n78_), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nor2   g38(.a(x03), .b(new_n52_), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  inv1   g40(.a(x06), .O(new_n84_));
  inv1   g41(.a(x07), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(x04), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n82_), .c(new_n81_), .O(new_n88_));
  aoi21  g45(.a(new_n88_), .b(new_n51_), .c(x00), .O(z08));
  inv1   g46(.a(x13), .O(new_n90_));
  inv1   g47(.a(x14), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n90_), .c(x12), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n69_), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n57_), .c(new_n96_), .d(new_n95_), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(new_n94_), .O(z09));
  nor2   g59(.a(new_n96_), .b(new_n95_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n100_), .c(new_n93_), .d(new_n69_), .O(new_n104_));
  aoi21  g61(.a(new_n104_), .b(new_n51_), .c(x00), .O(z10));
  nor2   g62(.a(x16), .b(new_n95_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n100_), .c(new_n93_), .d(new_n69_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n51_), .c(x00), .O(z11));
  oai21  g65(.a(new_n55_), .b(new_n52_), .c(new_n73_), .O(new_n109_));
  oai21  g66(.a(x19), .b(x02), .c(x23), .O(new_n110_));
  inv1   g67(.a(x19), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(x17), .c(new_n52_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n57_), .O(new_n114_));
  inv1   g71(.a(x24), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x09), .O(new_n116_));
  aoi21  g73(.a(new_n114_), .b(new_n109_), .c(new_n116_), .O(z12));
  aoi21  g74(.a(new_n112_), .b(new_n51_), .c(x00), .O(z13));
  nor2   g75(.a(new_n56_), .b(new_n49_), .O(z14));
  oai21  g76(.a(new_n55_), .b(new_n46_), .c(x01), .O(new_n120_));
  oai21  g77(.a(new_n65_), .b(x02), .c(new_n120_), .O(z15));
  nor2   g78(.a(new_n48_), .b(x00), .O(z17));
  zero   g79(.O(z16));
endmodule


