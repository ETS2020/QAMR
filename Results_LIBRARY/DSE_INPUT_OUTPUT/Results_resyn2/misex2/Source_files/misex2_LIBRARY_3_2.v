// Benchmark "FAU" written by ABC on Tue Aug 11 19:54:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n73_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n125_, new_n127_, new_n128_;
  nor3   g00(.a(x19), .b(x18), .c(x17), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x10), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nor2   g05(.a(new_n48_), .b(new_n45_), .O(z00));
  inv1   g06(.a(x00), .O(new_n50_));
  nor2   g07(.a(new_n46_), .b(x02), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n44_), .c(x10), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n50_), .c(x01), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand3  g11(.a(new_n51_), .b(new_n44_), .c(new_n54_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n50_), .c(x01), .O(z02));
  inv1   g13(.a(x12), .O(new_n57_));
  inv1   g14(.a(x02), .O(new_n58_));
  nor2   g15(.a(x09), .b(new_n58_), .O(new_n59_));
  inv1   g16(.a(x01), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n50_), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(x11), .c(x10), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n59_), .O(new_n64_));
  inv1   g21(.a(x17), .O(new_n65_));
  inv1   g22(.a(x18), .O(new_n66_));
  nor2   g23(.a(x19), .b(new_n66_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n47_), .c(new_n65_), .O(new_n68_));
  oai21  g25(.a(new_n64_), .b(new_n57_), .c(new_n68_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  nand4  g27(.a(new_n59_), .b(new_n57_), .c(new_n70_), .d(x10), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x01), .c(new_n50_), .O(z04));
  nand3  g29(.a(x10), .b(x09), .c(x02), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x01), .c(new_n50_), .O(z05));
  inv1   g31(.a(new_n64_), .O(z06));
  nor2   g32(.a(new_n64_), .b(x12), .O(z07));
  inv1   g33(.a(x03), .O(new_n77_));
  nor2   g34(.a(x06), .b(x05), .O(new_n78_));
  nor2   g35(.a(x08), .b(x07), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(new_n78_), .c(x04), .d(new_n77_), .O(new_n80_));
  nor2   g37(.a(x01), .b(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(x02), .O(new_n82_));
  nand3  g39(.a(x19), .b(new_n66_), .c(x17), .O(new_n83_));
  nor3   g40(.a(new_n83_), .b(new_n82_), .c(new_n80_), .O(z08));
  nor2   g41(.a(x22), .b(x21), .O(new_n85_));
  nand2  g42(.a(new_n85_), .b(x20), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  inv1   g44(.a(x13), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x12), .c(new_n70_), .d(x02), .O(new_n89_));
  inv1   g46(.a(x14), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n89_), .O(new_n94_));
  aoi21  g51(.a(new_n94_), .b(new_n87_), .c(x00), .O(new_n95_));
  nor2   g52(.a(x20), .b(new_n60_), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n67_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n85_), .c(new_n50_), .O(new_n99_));
  oai21  g56(.a(new_n95_), .b(x01), .c(new_n99_), .O(z09));
  nand3  g57(.a(new_n98_), .b(x22), .c(x21), .O(new_n101_));
  nor2   g58(.a(new_n89_), .b(x01), .O(new_n102_));
  nor2   g59(.a(new_n91_), .b(x14), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n102_), .c(new_n87_), .d(x16), .O(new_n104_));
  aoi21  g61(.a(new_n104_), .b(new_n101_), .c(x00), .O(z10));
  nand2  g62(.a(new_n98_), .b(x21), .O(new_n106_));
  inv1   g63(.a(x21), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(x20), .c(new_n92_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n103_), .c(new_n102_), .O(new_n110_));
  inv1   g67(.a(x22), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n50_), .O(new_n112_));
  aoi21  g69(.a(new_n110_), .b(new_n106_), .c(new_n112_), .O(z11));
  inv1   g70(.a(x23), .O(new_n114_));
  nor2   g71(.a(x19), .b(x02), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x17), .O(new_n116_));
  oai21  g73(.a(new_n115_), .b(new_n114_), .c(new_n116_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n81_), .O(new_n118_));
  nand2  g75(.a(x10), .b(x02), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n61_), .O(new_n120_));
  inv1   g77(.a(x24), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x09), .O(new_n122_));
  aoi21  g79(.a(new_n120_), .b(new_n118_), .c(new_n122_), .O(z12));
  aoi21  g80(.a(new_n116_), .b(new_n50_), .c(x01), .O(z13));
  nand2  g81(.a(new_n44_), .b(new_n54_), .O(new_n125_));
  nor2   g82(.a(new_n125_), .b(new_n48_), .O(z14));
  aoi21  g83(.a(x19), .b(new_n58_), .c(x00), .O(new_n127_));
  aoi21  g84(.a(new_n119_), .b(x00), .c(new_n60_), .O(new_n128_));
  nor2   g85(.a(new_n128_), .b(new_n127_), .O(z15));
  nor2   g86(.a(new_n61_), .b(new_n81_), .O(z16));
  inv1   g87(.a(new_n82_), .O(z17));
endmodule


