// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n120_, new_n121_, new_n124_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  inv1   g05(.a(x16), .O(new_n49_));
  nor3   g06(.a(x19), .b(x18), .c(x17), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n48_), .O(z00));
  nand4  g09(.a(x09), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(new_n51_), .O(z01));
  nor2   g11(.a(x16), .b(x10), .O(new_n55_));
  nand2  g12(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n53_), .O(z02));
  nand2  g14(.a(new_n46_), .b(new_n44_), .O(new_n58_));
  inv1   g15(.a(x17), .O(new_n59_));
  inv1   g16(.a(x19), .O(new_n60_));
  nand3  g17(.a(new_n60_), .b(x18), .c(new_n59_), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n58_), .c(new_n49_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n45_), .O(new_n63_));
  nand3  g20(.a(x11), .b(x10), .c(new_n47_), .O(new_n64_));
  nor2   g21(.a(new_n45_), .b(new_n44_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(x02), .O(new_n66_));
  nor2   g23(.a(new_n66_), .b(new_n64_), .O(z06));
  nand2  g24(.a(z06), .b(x12), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n63_), .O(z03));
  nand2  g26(.a(x16), .b(new_n45_), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(x10), .d(new_n47_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n66_), .c(new_n70_), .O(z04));
  nand2  g31(.a(x10), .b(x02), .O(new_n75_));
  nand2  g32(.a(new_n65_), .b(x09), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n70_), .O(z05));
  oai21  g34(.a(new_n64_), .b(x12), .c(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n70_), .O(z07));
  nor2   g37(.a(x18), .b(new_n59_), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand4  g42(.a(x04), .b(new_n85_), .c(x02), .d(new_n44_), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n81_), .c(x19), .O(new_n88_));
  aoi21  g45(.a(new_n88_), .b(new_n49_), .c(x01), .O(z08));
  inv1   g46(.a(x21), .O(new_n90_));
  inv1   g47(.a(x20), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n60_), .c(x18), .d(x01), .O(new_n92_));
  nand4  g49(.a(x12), .b(new_n71_), .c(x02), .d(new_n45_), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  nor2   g51(.a(x14), .b(x13), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x20), .c(new_n94_), .O(new_n96_));
  oai21  g53(.a(new_n96_), .b(new_n93_), .c(new_n92_), .O(new_n97_));
  nor2   g54(.a(x22), .b(x00), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n90_), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n70_), .O(z09));
  nor2   g57(.a(new_n92_), .b(new_n90_), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(x22), .c(new_n44_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(z10));
  nand4  g60(.a(new_n95_), .b(new_n90_), .c(x20), .d(x15), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n93_), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n101_), .c(new_n98_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n70_), .O(z11));
  nand2  g64(.a(new_n75_), .b(new_n65_), .O(new_n108_));
  inv1   g65(.a(x23), .O(new_n109_));
  oai21  g66(.a(x19), .b(x02), .c(new_n109_), .O(new_n110_));
  nor3   g67(.a(x16), .b(x01), .c(x00), .O(new_n111_));
  nand3  g68(.a(new_n60_), .b(new_n59_), .c(new_n46_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(new_n113_));
  inv1   g70(.a(x24), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(x09), .O(new_n115_));
  aoi21  g72(.a(new_n113_), .b(new_n108_), .c(new_n115_), .O(z12));
  nand4  g73(.a(new_n60_), .b(x17), .c(new_n46_), .d(new_n44_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n49_), .c(x01), .O(z13));
  nor2   g75(.a(new_n56_), .b(new_n48_), .O(z14));
  aoi21  g76(.a(x19), .b(new_n45_), .c(x00), .O(new_n120_));
  nand2  g77(.a(new_n49_), .b(new_n46_), .O(new_n121_));
  oai21  g78(.a(new_n121_), .b(new_n120_), .c(new_n108_), .O(z15));
  aoi21  g79(.a(new_n49_), .b(new_n45_), .c(new_n65_), .O(z16));
  nand2  g80(.a(new_n111_), .b(x02), .O(new_n124_));
  inv1   g81(.a(new_n124_), .O(z17));
endmodule


