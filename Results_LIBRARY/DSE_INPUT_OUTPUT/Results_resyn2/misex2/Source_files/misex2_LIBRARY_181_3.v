// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n114_;
  nand2  g00(.a(x01), .b(x00), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  inv1   g06(.a(x00), .O(new_n50_));
  inv1   g07(.a(x01), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x02), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand2  g12(.a(new_n53_), .b(x09), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n48_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n56_), .O(z02));
  nor2   g17(.a(x19), .b(new_n46_), .O(new_n61_));
  nand3  g18(.a(new_n61_), .b(new_n53_), .c(new_n45_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n44_), .O(z03));
  inv1   g20(.a(new_n44_), .O(z05));
  inv1   g21(.a(x02), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(new_n51_), .c(new_n50_), .O(z07));
  inv1   g23(.a(x03), .O(new_n69_));
  nor2   g24(.a(x06), .b(x05), .O(new_n70_));
  nor2   g25(.a(x08), .b(x07), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(x04), .d(new_n69_), .O(new_n72_));
  nor2   g27(.a(x01), .b(x00), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x02), .O(new_n74_));
  nand3  g29(.a(x19), .b(new_n46_), .c(x17), .O(new_n75_));
  nor3   g30(.a(new_n75_), .b(new_n74_), .c(new_n72_), .O(z08));
  inv1   g31(.a(x21), .O(new_n77_));
  inv1   g32(.a(x22), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n77_), .c(new_n50_), .O(new_n79_));
  inv1   g34(.a(x13), .O(new_n80_));
  inv1   g35(.a(x14), .O(new_n81_));
  nand3  g36(.a(x20), .b(new_n81_), .c(new_n80_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(new_n83_));
  inv1   g38(.a(x11), .O(new_n84_));
  nand4  g39(.a(x12), .b(new_n84_), .c(x02), .d(new_n51_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  nor2   g41(.a(x16), .b(x15), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n86_), .c(new_n83_), .O(new_n88_));
  inv1   g43(.a(x20), .O(new_n89_));
  nand3  g44(.a(new_n61_), .b(new_n89_), .c(x01), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n88_), .c(new_n79_), .O(z09));
  nand4  g46(.a(x21), .b(new_n89_), .c(new_n47_), .d(x18), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n78_), .c(new_n50_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(x01), .O(new_n94_));
  nand2  g49(.a(x16), .b(x15), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n79_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n86_), .c(new_n83_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n94_), .O(z10));
  inv1   g53(.a(x16), .O(new_n99_));
  nand3  g54(.a(new_n77_), .b(new_n99_), .c(x15), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n86_), .c(new_n83_), .O(new_n102_));
  inv1   g57(.a(new_n92_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x01), .O(new_n104_));
  nand2  g59(.a(new_n78_), .b(new_n50_), .O(new_n105_));
  aoi21  g60(.a(new_n104_), .b(new_n102_), .c(new_n105_), .O(z11));
  oai21  g61(.a(x19), .b(x02), .c(x23), .O(new_n107_));
  nand3  g62(.a(new_n47_), .b(x17), .c(new_n67_), .O(new_n108_));
  inv1   g63(.a(x24), .O(new_n109_));
  nand3  g64(.a(new_n73_), .b(new_n109_), .c(x09), .O(new_n110_));
  aoi21  g65(.a(new_n108_), .b(new_n107_), .c(new_n110_), .O(z12));
  oai21  g66(.a(new_n108_), .b(new_n52_), .c(new_n44_), .O(z13));
  nor2   g67(.a(new_n59_), .b(new_n54_), .O(z14));
  nand2  g68(.a(x19), .b(new_n51_), .O(new_n114_));
  aoi22  g69(.a(new_n114_), .b(new_n50_), .c(x02), .d(new_n51_), .O(z15));
  inv1   g70(.a(new_n74_), .O(z17));
  zero   g71(.O(z04));
  zero   g72(.O(z06));
  buf    g73(.a(x01), .O(z16));
endmodule


