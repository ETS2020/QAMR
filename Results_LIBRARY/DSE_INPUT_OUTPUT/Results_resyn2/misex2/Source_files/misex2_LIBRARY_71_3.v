// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n128_, new_n129_, new_n130_;
  nand2  g00(.a(x19), .b(x01), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nor2   g06(.a(x01), .b(x00), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x02), .O(new_n52_));
  nand2  g09(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand2  g11(.a(new_n52_), .b(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(new_n48_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n55_), .O(z02));
  nand3  g16(.a(new_n52_), .b(x18), .c(new_n45_), .O(new_n60_));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  inv1   g19(.a(x11), .O(new_n63_));
  inv1   g20(.a(x12), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n62_), .c(x10), .d(new_n49_), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(new_n60_), .c(x19), .O(z03));
  inv1   g24(.a(x01), .O(new_n68_));
  nand3  g25(.a(new_n64_), .b(x10), .c(new_n49_), .O(new_n69_));
  inv1   g26(.a(new_n69_), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n63_), .c(x02), .d(x00), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(new_n47_), .c(new_n68_), .O(z04));
  nand3  g29(.a(new_n62_), .b(new_n47_), .c(x10), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n49_), .O(z05));
  nor3   g31(.a(new_n73_), .b(new_n63_), .c(x09), .O(z06));
  oai21  g32(.a(new_n69_), .b(new_n63_), .c(x01), .O(new_n76_));
  nand3  g33(.a(new_n76_), .b(x02), .c(x00), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n44_), .O(z07));
  inv1   g35(.a(x00), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(new_n46_), .c(x17), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x04), .c(new_n83_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n82_), .c(x02), .d(new_n79_), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(new_n68_), .c(new_n47_), .O(z08));
  inv1   g45(.a(x20), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n47_), .c(x18), .d(x01), .O(new_n90_));
  nand2  g47(.a(x02), .b(new_n68_), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x12), .c(new_n63_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nor2   g51(.a(x15), .b(x14), .O(new_n95_));
  nor2   g52(.a(new_n89_), .b(x16), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  inv1   g54(.a(x21), .O(new_n98_));
  inv1   g55(.a(x22), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(new_n79_), .O(new_n100_));
  aoi21  g57(.a(new_n97_), .b(new_n90_), .c(new_n100_), .O(z09));
  inv1   g58(.a(new_n90_), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(x22), .c(x21), .O(new_n103_));
  inv1   g60(.a(x14), .O(new_n104_));
  nand4  g61(.a(new_n98_), .b(x20), .c(x15), .d(new_n104_), .O(new_n105_));
  inv1   g62(.a(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n94_), .c(new_n99_), .d(x16), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n103_), .c(x00), .O(z10));
  nand2  g65(.a(new_n99_), .b(new_n79_), .O(new_n109_));
  nor2   g66(.a(new_n105_), .b(x16), .O(new_n110_));
  nand4  g67(.a(x21), .b(new_n89_), .c(x18), .d(x01), .O(new_n111_));
  inv1   g68(.a(new_n111_), .O(new_n112_));
  aoi21  g69(.a(new_n110_), .b(new_n94_), .c(new_n112_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n109_), .c(new_n44_), .O(z11));
  nand2  g71(.a(x01), .b(x00), .O(new_n115_));
  aoi21  g72(.a(x10), .b(x02), .c(new_n115_), .O(new_n116_));
  inv1   g73(.a(x02), .O(new_n117_));
  nand3  g74(.a(new_n50_), .b(x17), .c(new_n117_), .O(new_n118_));
  inv1   g75(.a(new_n118_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n116_), .c(new_n47_), .O(new_n120_));
  nand2  g77(.a(new_n47_), .b(new_n117_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n50_), .c(x23), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n120_), .c(new_n124_), .O(z12));
  nor2   g82(.a(new_n118_), .b(x19), .O(z13));
  nor2   g83(.a(new_n58_), .b(new_n53_), .O(z14));
  nand2  g84(.a(new_n47_), .b(new_n57_), .O(new_n128_));
  nor2   g85(.a(x19), .b(x00), .O(new_n129_));
  nand2  g86(.a(new_n44_), .b(new_n117_), .O(new_n130_));
  oai22  g87(.a(new_n130_), .b(new_n129_), .c(new_n115_), .d(new_n128_), .O(z15));
  aoi21  g88(.a(new_n47_), .b(x00), .c(new_n68_), .O(z16));
  oai21  g89(.a(new_n51_), .b(new_n117_), .c(new_n44_), .O(z17));
endmodule


