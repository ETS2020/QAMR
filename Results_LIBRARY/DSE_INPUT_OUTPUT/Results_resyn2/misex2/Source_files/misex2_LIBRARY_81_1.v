// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n115_;
  nor3   g00(.a(x19), .b(x18), .c(x17), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x10), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(x00), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n45_), .O(z00));
  nand4  g09(.a(new_n44_), .b(x10), .c(x09), .d(new_n48_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n47_), .c(x00), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  nand2  g12(.a(new_n44_), .b(new_n55_), .O(new_n56_));
  nand2  g13(.a(new_n50_), .b(x09), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n56_), .O(z02));
  inv1   g15(.a(x11), .O(new_n59_));
  nand4  g16(.a(x10), .b(x02), .c(x01), .d(x00), .O(new_n60_));
  nor3   g17(.a(new_n60_), .b(new_n59_), .c(x09), .O(z06));
  nand2  g18(.a(z06), .b(x12), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  inv1   g20(.a(x18), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(x17), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n50_), .c(new_n63_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n62_), .O(z03));
  inv1   g24(.a(x12), .O(new_n68_));
  nand2  g25(.a(new_n59_), .b(x02), .O(new_n69_));
  inv1   g26(.a(new_n69_), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n68_), .c(x10), .d(new_n46_), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x00), .c(new_n47_), .O(z04));
  nor2   g29(.a(new_n60_), .b(new_n46_), .O(z05));
  nand4  g30(.a(new_n68_), .b(x11), .c(x10), .d(new_n46_), .O(new_n74_));
  nand2  g31(.a(x02), .b(x00), .O(new_n75_));
  aoi21  g32(.a(new_n74_), .b(x01), .c(new_n75_), .O(z07));
  inv1   g33(.a(x08), .O(new_n77_));
  nand4  g34(.a(x19), .b(new_n64_), .c(x17), .d(new_n77_), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(new_n79_));
  nor2   g36(.a(x03), .b(new_n48_), .O(new_n80_));
  inv1   g37(.a(x04), .O(new_n81_));
  nor2   g38(.a(x05), .b(new_n81_), .O(new_n82_));
  nor2   g39(.a(x07), .b(x06), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n82_), .c(new_n80_), .d(new_n79_), .O(new_n84_));
  aoi21  g41(.a(new_n84_), .b(new_n47_), .c(x00), .O(z08));
  inv1   g42(.a(x15), .O(new_n86_));
  inv1   g43(.a(x13), .O(new_n87_));
  inv1   g44(.a(x14), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n87_), .c(x12), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n69_), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  inv1   g48(.a(x21), .O(new_n92_));
  inv1   g49(.a(x22), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(x20), .d(new_n91_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n90_), .c(new_n86_), .O(new_n96_));
  aoi21  g53(.a(new_n96_), .b(new_n47_), .c(x00), .O(z09));
  nand4  g54(.a(new_n93_), .b(new_n92_), .c(x20), .d(x16), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n90_), .c(x15), .O(new_n100_));
  aoi21  g57(.a(new_n100_), .b(new_n47_), .c(x00), .O(z10));
  nand3  g58(.a(new_n95_), .b(new_n90_), .c(x15), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n47_), .c(x00), .O(z11));
  aoi21  g60(.a(x10), .b(x02), .c(new_n47_), .O(new_n104_));
  oai21  g61(.a(x19), .b(x02), .c(x23), .O(new_n105_));
  nand3  g62(.a(new_n63_), .b(x17), .c(new_n48_), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n105_), .c(x00), .O(new_n107_));
  nor2   g64(.a(x24), .b(new_n46_), .O(new_n108_));
  oai21  g65(.a(new_n107_), .b(new_n104_), .c(new_n108_), .O(new_n109_));
  inv1   g66(.a(x00), .O(new_n110_));
  nand2  g67(.a(x01), .b(new_n110_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n109_), .O(z12));
  nor3   g69(.a(new_n106_), .b(x01), .c(x00), .O(z13));
  nor2   g70(.a(new_n56_), .b(new_n51_), .O(z14));
  aoi21  g71(.a(new_n55_), .b(x01), .c(new_n48_), .O(new_n115_));
  oai22  g72(.a(new_n115_), .b(new_n110_), .c(new_n49_), .d(new_n63_), .O(z15));
  aoi21  g73(.a(new_n48_), .b(new_n47_), .c(x00), .O(z17));
  zero   g74(.O(z16));
endmodule


