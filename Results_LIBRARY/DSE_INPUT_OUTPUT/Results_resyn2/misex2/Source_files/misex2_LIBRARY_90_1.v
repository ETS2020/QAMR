// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x21), .O(new_n44_));
  inv1   g01(.a(x10), .O(new_n45_));
  nor2   g02(.a(new_n45_), .b(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nor2   g04(.a(x18), .b(x17), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(new_n44_), .c(x19), .O(z00));
  inv1   g07(.a(x09), .O(new_n51_));
  nor2   g08(.a(new_n45_), .b(new_n51_), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n48_), .c(new_n47_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x19), .O(z01));
  nand4  g11(.a(new_n48_), .b(new_n47_), .c(new_n45_), .d(x09), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x19), .O(z02));
  inv1   g13(.a(x12), .O(new_n57_));
  inv1   g14(.a(x18), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(x17), .O(new_n59_));
  aoi21  g16(.a(new_n59_), .b(new_n47_), .c(x21), .O(new_n60_));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  nor3   g18(.a(new_n61_), .b(new_n45_), .c(x09), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x11), .O(new_n63_));
  oai22  g20(.a(new_n63_), .b(new_n57_), .c(new_n60_), .d(x19), .O(z03));
  nor2   g21(.a(new_n44_), .b(x19), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  inv1   g23(.a(x11), .O(new_n67_));
  nand3  g24(.a(new_n62_), .b(new_n57_), .c(new_n67_), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n66_), .O(z04));
  inv1   g26(.a(new_n61_), .O(new_n70_));
  nand3  g27(.a(new_n66_), .b(new_n70_), .c(new_n52_), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(z05));
  nand2  g29(.a(new_n66_), .b(new_n63_), .O(z06));
  nand3  g30(.a(new_n46_), .b(new_n57_), .c(x11), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n66_), .O(z07));
  inv1   g34(.a(x03), .O(new_n78_));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  nor2   g36(.a(x08), .b(x07), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n79_), .c(x04), .d(new_n78_), .O(new_n81_));
  nor2   g38(.a(x01), .b(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x02), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n58_), .c(x17), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n83_), .c(new_n81_), .O(z08));
  nor2   g42(.a(x22), .b(x00), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  inv1   g44(.a(x13), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(x12), .c(new_n67_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  inv1   g47(.a(x14), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  inv1   g51(.a(x01), .O(new_n95_));
  nand4  g52(.a(new_n44_), .b(x20), .c(x02), .d(new_n95_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  inv1   g54(.a(x19), .O(new_n98_));
  inv1   g55(.a(x20), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(x18), .d(x01), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  aoi21  g58(.a(new_n97_), .b(new_n90_), .c(new_n101_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n87_), .c(new_n66_), .O(z09));
  nand4  g60(.a(new_n90_), .b(new_n44_), .c(x20), .d(new_n91_), .O(new_n104_));
  inv1   g61(.a(x02), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(x01), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n86_), .c(x16), .d(x15), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(new_n104_), .c(new_n66_), .O(z10));
  nand4  g65(.a(new_n106_), .b(new_n86_), .c(new_n93_), .d(x15), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n104_), .O(z11));
  nand2  g67(.a(x10), .b(x02), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(x01), .c(x00), .O(new_n112_));
  nor3   g69(.a(x19), .b(x17), .c(x02), .O(new_n113_));
  nor2   g70(.a(x19), .b(x02), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(x23), .c(new_n82_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  nor2   g73(.a(x24), .b(new_n51_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n66_), .O(z12));
  inv1   g76(.a(new_n47_), .O(new_n120_));
  nand3  g77(.a(new_n44_), .b(new_n98_), .c(x17), .O(new_n121_));
  nor2   g78(.a(new_n121_), .b(new_n120_), .O(z13));
  nand3  g79(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n123_));
  nand3  g80(.a(new_n44_), .b(new_n98_), .c(new_n51_), .O(new_n124_));
  nor2   g81(.a(new_n124_), .b(new_n123_), .O(z14));
  oai21  g82(.a(x10), .b(new_n95_), .c(x02), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(x00), .O(new_n127_));
  nand3  g84(.a(x19), .b(new_n105_), .c(new_n95_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n127_), .c(new_n66_), .O(z15));
  nor3   g86(.a(new_n65_), .b(new_n95_), .c(x00), .O(z16));
  nor2   g87(.a(new_n83_), .b(new_n65_), .O(z17));
endmodule


