// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n127_, new_n128_;
  inv1   g00(.a(x19), .O(new_n44_));
  nor2   g01(.a(x21), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  nand4  g05(.a(new_n44_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  inv1   g08(.a(new_n51_), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x02), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n49_), .c(new_n46_), .O(z00));
  nand2  g12(.a(new_n53_), .b(x09), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n49_), .c(new_n46_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n44_), .b(new_n48_), .c(new_n47_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n56_), .O(z02));
  nand4  g17(.a(new_n53_), .b(new_n44_), .c(x18), .d(new_n47_), .O(new_n61_));
  inv1   g18(.a(x02), .O(new_n62_));
  nor2   g19(.a(x09), .b(new_n62_), .O(new_n63_));
  inv1   g20(.a(x11), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(new_n58_), .O(new_n65_));
  nand2  g22(.a(x01), .b(x00), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n65_), .c(new_n63_), .d(x12), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n61_), .c(new_n46_), .O(z03));
  nor2   g26(.a(new_n66_), .b(new_n45_), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n63_), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  nand3  g29(.a(new_n72_), .b(new_n64_), .c(x10), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n71_), .O(z04));
  nand4  g31(.a(new_n70_), .b(x10), .c(x09), .d(x02), .O(new_n75_));
  inv1   g32(.a(new_n75_), .O(z05));
  nand3  g33(.a(new_n70_), .b(new_n65_), .c(new_n63_), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(z06));
  inv1   g35(.a(x01), .O(new_n79_));
  nor2   g36(.a(x12), .b(x09), .O(new_n80_));
  aoi21  g37(.a(new_n80_), .b(new_n65_), .c(new_n79_), .O(new_n81_));
  nand2  g38(.a(x02), .b(x00), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(new_n81_), .c(new_n46_), .O(z07));
  inv1   g40(.a(x03), .O(new_n84_));
  nor2   g41(.a(x06), .b(x05), .O(new_n85_));
  nor2   g42(.a(x08), .b(x07), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(x04), .d(new_n84_), .O(new_n87_));
  nand2  g44(.a(new_n51_), .b(x02), .O(new_n88_));
  nand4  g45(.a(x21), .b(x19), .c(new_n48_), .d(x17), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(z08));
  inv1   g47(.a(x20), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x18), .c(x01), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  nand2  g51(.a(x02), .b(new_n79_), .O(new_n95_));
  inv1   g52(.a(x13), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(x12), .c(new_n64_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nor2   g55(.a(new_n91_), .b(x16), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n98_), .c(new_n94_), .d(new_n93_), .O(new_n100_));
  inv1   g57(.a(x00), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n102_), .c(new_n44_), .d(new_n101_), .O(new_n104_));
  aoi21  g61(.a(new_n100_), .b(new_n92_), .c(new_n104_), .O(z09));
  nand2  g62(.a(x15), .b(new_n93_), .O(new_n106_));
  nand4  g63(.a(new_n103_), .b(new_n102_), .c(x20), .d(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand4  g65(.a(x21), .b(new_n91_), .c(x18), .d(x01), .O(new_n109_));
  nand2  g66(.a(x22), .b(new_n44_), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  aoi21  g68(.a(new_n108_), .b(new_n98_), .c(new_n111_), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(x00), .c(new_n46_), .O(z10));
  inv1   g70(.a(new_n106_), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n99_), .c(new_n98_), .d(new_n102_), .O(new_n115_));
  nand3  g72(.a(new_n103_), .b(new_n44_), .c(new_n101_), .O(new_n116_));
  aoi21  g73(.a(new_n115_), .b(new_n109_), .c(new_n116_), .O(z11));
  aoi21  g74(.a(x10), .b(x02), .c(new_n66_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n44_), .b(x17), .c(new_n62_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(new_n52_), .O(new_n121_));
  nor2   g78(.a(x24), .b(new_n50_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n46_), .O(z12));
  oai21  g81(.a(new_n120_), .b(new_n52_), .c(new_n46_), .O(z13));
  nor2   g82(.a(new_n59_), .b(new_n54_), .O(z14));
  aoi21  g83(.a(new_n58_), .b(x01), .c(new_n62_), .O(new_n127_));
  aoi21  g84(.a(new_n62_), .b(new_n79_), .c(new_n102_), .O(new_n128_));
  oai22  g85(.a(new_n128_), .b(new_n44_), .c(new_n127_), .d(new_n101_), .O(z15));
  nor3   g86(.a(new_n45_), .b(new_n79_), .c(x00), .O(z16));
  nand2  g87(.a(new_n88_), .b(new_n46_), .O(z17));
endmodule


