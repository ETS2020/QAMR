// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n134_, new_n135_;
  inv1   g00(.a(x01), .O(new_n44_));
  nor2   g01(.a(x04), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  inv1   g04(.a(x18), .O(new_n48_));
  inv1   g05(.a(x19), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x10), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  inv1   g08(.a(x00), .O(new_n52_));
  nand2  g09(.a(new_n44_), .b(new_n52_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n50_), .c(new_n46_), .O(z00));
  nand2  g13(.a(new_n54_), .b(x09), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n50_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(new_n57_), .O(z02));
  nand3  g18(.a(x11), .b(x10), .c(new_n51_), .O(new_n62_));
  nand3  g19(.a(x12), .b(x02), .c(x00), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n62_), .c(x04), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x01), .O(new_n65_));
  nand4  g22(.a(new_n54_), .b(new_n49_), .c(x18), .d(new_n47_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n65_), .O(z03));
  nand3  g24(.a(x04), .b(x01), .c(x00), .O(new_n68_));
  nand2  g25(.a(x10), .b(x02), .O(new_n69_));
  nor3   g26(.a(new_n69_), .b(new_n68_), .c(x09), .O(new_n70_));
  nor2   g27(.a(x12), .b(x11), .O(new_n71_));
  nand2  g28(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g29(.a(new_n72_), .O(z04));
  nor3   g30(.a(new_n69_), .b(new_n68_), .c(new_n51_), .O(z05));
  nand2  g31(.a(new_n70_), .b(x11), .O(new_n75_));
  inv1   g32(.a(new_n75_), .O(z06));
  oai21  g33(.a(new_n62_), .b(x12), .c(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n46_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nor2   g40(.a(x01), .b(x00), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(x02), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(z17));
  nor2   g43(.a(x18), .b(new_n47_), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(z17), .c(x19), .O(new_n88_));
  oai21  g45(.a(new_n88_), .b(new_n83_), .c(new_n46_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n49_), .c(x18), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(x04), .c(x01), .O(new_n93_));
  inv1   g50(.a(x15), .O(new_n94_));
  nand2  g51(.a(x02), .b(new_n44_), .O(new_n95_));
  inv1   g52(.a(x11), .O(new_n96_));
  inv1   g53(.a(x13), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(x12), .c(new_n96_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n95_), .O(new_n99_));
  inv1   g56(.a(x14), .O(new_n100_));
  inv1   g57(.a(x16), .O(new_n101_));
  nand3  g58(.a(x20), .b(new_n101_), .c(new_n100_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n99_), .c(new_n94_), .O(new_n104_));
  inv1   g61(.a(x22), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n52_), .O(new_n106_));
  or2    g63(.a(new_n106_), .b(x21), .O(new_n107_));
  aoi21  g64(.a(new_n104_), .b(new_n93_), .c(new_n107_), .O(z09));
  inv1   g65(.a(x04), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(x01), .c(new_n52_), .O(new_n110_));
  nor2   g67(.a(x21), .b(new_n94_), .O(new_n111_));
  nand4  g68(.a(new_n105_), .b(x20), .c(x16), .d(new_n100_), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n111_), .c(new_n99_), .O(new_n114_));
  nand3  g71(.a(x22), .b(x21), .c(x01), .O(new_n115_));
  inv1   g72(.a(new_n115_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n92_), .c(new_n45_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n114_), .c(new_n110_), .O(z10));
  nand4  g75(.a(new_n92_), .b(x21), .c(x04), .d(x01), .O(new_n119_));
  nand3  g76(.a(new_n111_), .b(new_n103_), .c(new_n99_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(new_n106_), .O(z11));
  inv1   g78(.a(new_n68_), .O(new_n122_));
  nand2  g79(.a(new_n69_), .b(new_n122_), .O(new_n123_));
  oai21  g80(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  inv1   g81(.a(x02), .O(new_n125_));
  nand3  g82(.a(new_n49_), .b(x17), .c(new_n125_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n84_), .O(new_n128_));
  inv1   g85(.a(x24), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x09), .O(new_n130_));
  aoi21  g87(.a(new_n128_), .b(new_n123_), .c(new_n130_), .O(z12));
  oai21  g88(.a(new_n126_), .b(new_n53_), .c(new_n46_), .O(z13));
  nor2   g89(.a(new_n60_), .b(new_n55_), .O(z14));
  nor2   g90(.a(new_n49_), .b(x01), .O(new_n134_));
  oai21  g91(.a(new_n134_), .b(new_n110_), .c(new_n125_), .O(new_n135_));
  oai21  g92(.a(new_n68_), .b(x10), .c(new_n135_), .O(z15));
  aoi21  g93(.a(x04), .b(x00), .c(new_n44_), .O(z16));
endmodule


