// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x10), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand2  g04(.a(x21), .b(x03), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  inv1   g07(.a(x09), .O(new_n51_));
  nor2   g08(.a(x01), .b(x00), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nor3   g10(.a(new_n53_), .b(new_n49_), .c(new_n44_), .O(z00));
  nand3  g11(.a(new_n52_), .b(x09), .c(new_n50_), .O(new_n55_));
  nand4  g12(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n55_), .c(new_n48_), .O(z01));
  inv1   g14(.a(new_n49_), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n44_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(new_n55_), .O(z02));
  inv1   g17(.a(new_n48_), .O(new_n61_));
  nand3  g18(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  nand3  g19(.a(x11), .b(x10), .c(new_n51_), .O(new_n63_));
  nor2   g20(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x12), .O(new_n65_));
  nor2   g22(.a(x19), .b(x02), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n52_), .c(x18), .d(new_n45_), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(new_n65_), .c(new_n61_), .O(z03));
  nor2   g25(.a(x12), .b(x11), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n48_), .c(x10), .d(new_n51_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n62_), .O(z04));
  nand2  g28(.a(x10), .b(x02), .O(new_n72_));
  inv1   g29(.a(x00), .O(new_n73_));
  inv1   g30(.a(x01), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(x09), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n72_), .c(new_n48_), .O(z05));
  oai21  g34(.a(new_n63_), .b(new_n62_), .c(new_n48_), .O(z06));
  oai21  g35(.a(new_n63_), .b(x12), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n48_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nand3  g42(.a(x02), .b(new_n74_), .c(new_n73_), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(x19), .c(new_n46_), .d(x17), .O(new_n88_));
  oai21  g45(.a(new_n88_), .b(new_n85_), .c(new_n48_), .O(z08));
  inv1   g46(.a(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n47_), .c(x18), .d(x01), .O(new_n91_));
  nand2  g48(.a(x02), .b(new_n74_), .O(new_n92_));
  inv1   g49(.a(x11), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(x12), .c(new_n93_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  nor2   g53(.a(x15), .b(x14), .O(new_n97_));
  nor2   g54(.a(new_n90_), .b(x16), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n73_), .O(new_n102_));
  aoi21  g59(.a(new_n99_), .b(new_n91_), .c(new_n102_), .O(z09));
  inv1   g60(.a(new_n91_), .O(new_n104_));
  nor2   g61(.a(new_n100_), .b(x03), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n104_), .c(x22), .O(new_n106_));
  inv1   g63(.a(x14), .O(new_n107_));
  nand4  g64(.a(new_n100_), .b(x20), .c(x15), .d(new_n107_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  inv1   g66(.a(x16), .O(new_n110_));
  nor2   g67(.a(x22), .b(new_n110_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n109_), .c(new_n96_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n106_), .c(x00), .O(z10));
  nand2  g70(.a(new_n101_), .b(new_n73_), .O(new_n114_));
  nand2  g71(.a(new_n105_), .b(new_n104_), .O(new_n115_));
  nand3  g72(.a(new_n109_), .b(new_n96_), .c(new_n110_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(z11));
  nand2  g74(.a(new_n72_), .b(new_n75_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand2  g76(.a(new_n66_), .b(x17), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n52_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand3  g80(.a(new_n48_), .b(new_n123_), .c(x09), .O(new_n124_));
  aoi21  g81(.a(new_n122_), .b(new_n118_), .c(new_n124_), .O(z12));
  nand3  g82(.a(new_n66_), .b(new_n52_), .c(x17), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n48_), .O(z13));
  nor2   g84(.a(new_n59_), .b(new_n53_), .O(z14));
  oai21  g85(.a(x10), .b(new_n74_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n50_), .c(new_n74_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n130_), .c(new_n48_), .O(z15));
  oai21  g89(.a(new_n74_), .b(x00), .c(new_n48_), .O(z16));
  nand2  g90(.a(new_n86_), .b(new_n48_), .O(z17));
endmodule


