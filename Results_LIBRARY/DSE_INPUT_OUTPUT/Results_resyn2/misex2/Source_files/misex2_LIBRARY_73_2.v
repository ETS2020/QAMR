// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n123_, new_n124_,
    new_n125_, new_n126_;
  inv1   g00(.a(x09), .O(new_n44_));
  nor2   g01(.a(x01), .b(x00), .O(new_n45_));
  inv1   g02(.a(new_n45_), .O(new_n46_));
  nor2   g03(.a(new_n46_), .b(x02), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(new_n48_), .O(z00));
  nand2  g10(.a(new_n47_), .b(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n52_), .O(z01));
  inv1   g12(.a(x21), .O(new_n56_));
  nand2  g13(.a(new_n56_), .b(x18), .O(new_n57_));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n54_), .c(new_n57_), .O(z02));
  nor2   g17(.a(new_n56_), .b(x17), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n47_), .c(new_n51_), .d(x18), .O(new_n62_));
  nand4  g19(.a(new_n57_), .b(x02), .c(x01), .d(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand3  g21(.a(x11), .b(x10), .c(new_n44_), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n64_), .c(x12), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n62_), .O(z03));
  nand3  g25(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n44_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n69_), .c(new_n57_), .O(z04));
  nand3  g30(.a(new_n64_), .b(x10), .c(x09), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(z05));
  oai21  g32(.a(new_n69_), .b(new_n65_), .c(new_n57_), .O(z06));
  nand3  g33(.a(new_n57_), .b(x02), .c(x00), .O(new_n77_));
  nand4  g34(.a(new_n71_), .b(x11), .c(x10), .d(new_n44_), .O(new_n78_));
  aoi21  g35(.a(new_n78_), .b(x01), .c(new_n77_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand2  g40(.a(new_n45_), .b(x02), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n50_), .c(x17), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(z08));
  inv1   g43(.a(x15), .O(new_n87_));
  inv1   g44(.a(x16), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g46(.a(x22), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n56_), .c(x20), .d(new_n50_), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(x12), .d(new_n70_), .O(new_n94_));
  nor4   g51(.a(new_n94_), .b(new_n91_), .c(new_n89_), .d(new_n84_), .O(z09));
  inv1   g52(.a(new_n91_), .O(new_n96_));
  inv1   g53(.a(x01), .O(new_n97_));
  nand2  g54(.a(x02), .b(new_n97_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(new_n94_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n96_), .c(x16), .d(x15), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n51_), .c(x18), .d(x01), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(x22), .c(x21), .O(new_n104_));
  aoi21  g61(.a(new_n104_), .b(new_n100_), .c(x00), .O(z10));
  nand4  g62(.a(new_n56_), .b(x20), .c(new_n88_), .d(x15), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n99_), .c(new_n103_), .O(new_n108_));
  inv1   g65(.a(x00), .O(new_n109_));
  nand2  g66(.a(new_n90_), .b(new_n109_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(new_n108_), .c(new_n57_), .O(z11));
  nand2  g68(.a(x01), .b(x00), .O(new_n112_));
  aoi21  g69(.a(x10), .b(x02), .c(new_n112_), .O(new_n113_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  inv1   g71(.a(x02), .O(new_n115_));
  nand3  g72(.a(new_n51_), .b(x17), .c(new_n115_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n114_), .c(new_n46_), .O(new_n117_));
  nor2   g74(.a(x24), .b(new_n44_), .O(new_n118_));
  oai21  g75(.a(new_n117_), .b(new_n113_), .c(new_n118_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n57_), .O(z12));
  oai21  g77(.a(new_n116_), .b(new_n46_), .c(new_n57_), .O(z13));
  nor2   g78(.a(new_n59_), .b(new_n48_), .O(z14));
  inv1   g79(.a(new_n57_), .O(new_n123_));
  oai21  g80(.a(x10), .b(new_n97_), .c(x02), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x00), .O(new_n125_));
  nand3  g82(.a(x19), .b(new_n115_), .c(new_n97_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n125_), .c(new_n123_), .O(z15));
  nor3   g84(.a(new_n123_), .b(new_n97_), .c(x00), .O(z16));
  nand2  g85(.a(new_n84_), .b(new_n57_), .O(z17));
endmodule


