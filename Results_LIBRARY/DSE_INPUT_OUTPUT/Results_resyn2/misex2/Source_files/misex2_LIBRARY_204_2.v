// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n131_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  nor2   g04(.a(new_n47_), .b(x09), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(new_n44_), .c(x01), .O(z00));
  nand2  g07(.a(x10), .b(x09), .O(new_n51_));
  nor2   g08(.a(x18), .b(x17), .O(new_n52_));
  nor2   g09(.a(x01), .b(x00), .O(new_n53_));
  nor2   g10(.a(x19), .b(x02), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand2  g13(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n51_), .O(z01));
  nand4  g15(.a(new_n46_), .b(new_n47_), .c(x09), .d(new_n45_), .O(new_n59_));
  aoi21  g16(.a(new_n59_), .b(new_n44_), .c(x01), .O(z02));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g18(.a(new_n61_), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(new_n48_), .c(x12), .d(x11), .O(new_n63_));
  inv1   g20(.a(x18), .O(new_n64_));
  nor2   g21(.a(new_n64_), .b(x17), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n56_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n63_), .O(z03));
  nand2  g24(.a(new_n62_), .b(new_n48_), .O(new_n68_));
  nor3   g25(.a(new_n68_), .b(x12), .c(x11), .O(z04));
  nor2   g26(.a(new_n61_), .b(new_n51_), .O(z05));
  inv1   g27(.a(x11), .O(new_n71_));
  nor2   g28(.a(new_n68_), .b(new_n71_), .O(z06));
  inv1   g29(.a(x12), .O(new_n73_));
  nand4  g30(.a(new_n48_), .b(new_n73_), .c(x11), .d(x02), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x01), .c(new_n44_), .O(z07));
  inv1   g32(.a(x08), .O(new_n76_));
  nand4  g33(.a(x19), .b(new_n64_), .c(x17), .d(new_n76_), .O(new_n77_));
  inv1   g34(.a(new_n77_), .O(new_n78_));
  nor2   g35(.a(x03), .b(new_n45_), .O(new_n79_));
  inv1   g36(.a(x04), .O(new_n80_));
  nor2   g37(.a(x05), .b(new_n80_), .O(new_n81_));
  nor2   g38(.a(x07), .b(x06), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(new_n79_), .d(new_n78_), .O(new_n83_));
  aoi21  g40(.a(new_n83_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g41(.a(x01), .O(new_n85_));
  nand2  g42(.a(new_n71_), .b(x02), .O(new_n86_));
  inv1   g43(.a(x13), .O(new_n87_));
  inv1   g44(.a(x14), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n87_), .c(x12), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nor2   g47(.a(x16), .b(x15), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n90_), .c(x20), .d(new_n85_), .O(new_n92_));
  inv1   g49(.a(x19), .O(new_n93_));
  inv1   g50(.a(x20), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n93_), .c(x18), .d(x01), .O(new_n95_));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(new_n44_), .O(new_n98_));
  aoi21  g55(.a(new_n95_), .b(new_n92_), .c(new_n98_), .O(z09));
  nand2  g56(.a(new_n96_), .b(x20), .O(new_n100_));
  nand3  g57(.a(new_n97_), .b(x16), .c(x15), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n90_), .c(x00), .O(new_n103_));
  inv1   g60(.a(new_n95_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(x22), .c(x21), .d(new_n44_), .O(new_n105_));
  oai21  g62(.a(new_n103_), .b(x01), .c(new_n105_), .O(z10));
  nand2  g63(.a(new_n104_), .b(x21), .O(new_n107_));
  inv1   g64(.a(x16), .O(new_n108_));
  nand4  g65(.a(new_n96_), .b(x20), .c(new_n108_), .d(x15), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n90_), .c(new_n85_), .O(new_n111_));
  nand2  g68(.a(new_n97_), .b(new_n44_), .O(new_n112_));
  aoi21  g69(.a(new_n111_), .b(new_n107_), .c(new_n112_), .O(z11));
  inv1   g70(.a(x23), .O(new_n114_));
  nand2  g71(.a(new_n54_), .b(x17), .O(new_n115_));
  oai21  g72(.a(new_n54_), .b(new_n114_), .c(new_n115_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n53_), .O(new_n117_));
  nand2  g74(.a(x10), .b(x02), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(x01), .c(x00), .O(new_n119_));
  inv1   g76(.a(x24), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x09), .O(new_n121_));
  aoi21  g78(.a(new_n119_), .b(new_n117_), .c(new_n121_), .O(z12));
  aoi21  g79(.a(new_n115_), .b(new_n44_), .c(x01), .O(z13));
  nor2   g80(.a(x10), .b(x09), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(new_n56_), .c(new_n52_), .O(new_n125_));
  inv1   g82(.a(new_n125_), .O(z14));
  aoi21  g83(.a(x19), .b(new_n45_), .c(x00), .O(new_n127_));
  aoi21  g84(.a(new_n118_), .b(x00), .c(new_n85_), .O(new_n128_));
  nor2   g85(.a(new_n128_), .b(new_n127_), .O(z15));
  nor2   g86(.a(new_n85_), .b(x00), .O(z16));
  nand2  g87(.a(new_n53_), .b(x02), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z17));
endmodule


