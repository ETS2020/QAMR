// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n129_, new_n131_, new_n132_;
  nand2  g00(.a(x17), .b(x01), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  nor2   g04(.a(x17), .b(new_n47_), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor3   g07(.a(x02), .b(x01), .c(x00), .O(new_n51_));
  nand2  g08(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n49_), .c(new_n44_), .O(z00));
  nand2  g10(.a(new_n51_), .b(x09), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n49_), .O(z01));
  inv1   g12(.a(x17), .O(new_n56_));
  nand4  g13(.a(new_n46_), .b(new_n45_), .c(new_n56_), .d(new_n47_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n54_), .O(z02));
  inv1   g15(.a(x01), .O(new_n59_));
  nand2  g16(.a(x02), .b(x00), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x09), .O(new_n61_));
  nand3  g18(.a(x12), .b(x11), .c(x10), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  aoi21  g20(.a(new_n63_), .b(new_n61_), .c(x17), .O(new_n64_));
  nand4  g21(.a(new_n51_), .b(new_n46_), .c(x18), .d(new_n56_), .O(new_n65_));
  oai21  g22(.a(new_n64_), .b(new_n59_), .c(new_n65_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  inv1   g24(.a(x12), .O(new_n68_));
  nand4  g25(.a(new_n61_), .b(new_n68_), .c(new_n67_), .d(x10), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(new_n56_), .c(new_n59_), .O(z04));
  nand3  g27(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  nand2  g28(.a(new_n48_), .b(x09), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n71_), .O(z05));
  nand3  g30(.a(new_n48_), .b(x11), .c(new_n50_), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n71_), .O(z06));
  inv1   g32(.a(new_n74_), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n68_), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x01), .c(new_n60_), .O(z07));
  inv1   g35(.a(x03), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nor2   g37(.a(x08), .b(x07), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n80_), .c(x04), .d(new_n79_), .O(new_n82_));
  inv1   g39(.a(x00), .O(new_n83_));
  nand3  g40(.a(x02), .b(new_n59_), .c(new_n83_), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n45_), .c(x17), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n84_), .c(new_n82_), .O(z08));
  nand2  g43(.a(x02), .b(new_n59_), .O(new_n87_));
  inv1   g44(.a(x13), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(x12), .c(new_n67_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  inv1   g47(.a(x14), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nand4  g50(.a(x20), .b(new_n93_), .c(new_n92_), .d(new_n91_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  inv1   g52(.a(x20), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n46_), .c(x18), .d(x01), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  aoi21  g55(.a(new_n95_), .b(new_n90_), .c(new_n98_), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n83_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n99_), .c(new_n44_), .O(z09));
  nand2  g60(.a(x22), .b(x21), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n97_), .O(new_n105_));
  nand4  g62(.a(new_n100_), .b(x20), .c(x15), .d(new_n91_), .O(new_n106_));
  nand2  g63(.a(new_n101_), .b(x16), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n90_), .c(new_n105_), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(x00), .c(new_n44_), .O(z10));
  nand2  g67(.a(new_n101_), .b(new_n83_), .O(new_n111_));
  inv1   g68(.a(new_n106_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n90_), .c(new_n93_), .O(new_n113_));
  nand3  g70(.a(new_n98_), .b(x21), .c(new_n56_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n113_), .c(new_n111_), .O(z11));
  nand2  g72(.a(x01), .b(x00), .O(new_n116_));
  aoi21  g73(.a(x10), .b(x02), .c(new_n116_), .O(new_n117_));
  inv1   g74(.a(x23), .O(new_n118_));
  oai22  g75(.a(new_n118_), .b(x01), .c(x19), .d(x02), .O(new_n119_));
  nor2   g76(.a(x19), .b(x02), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n56_), .c(x00), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n119_), .c(new_n117_), .O(new_n122_));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  oai21  g81(.a(new_n124_), .b(new_n122_), .c(new_n44_), .O(z12));
  nand2  g82(.a(new_n120_), .b(new_n83_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n59_), .c(new_n56_), .O(z13));
  oai21  g84(.a(new_n57_), .b(new_n52_), .c(new_n44_), .O(z14));
  nand2  g85(.a(new_n56_), .b(new_n47_), .O(new_n129_));
  aoi21  g86(.a(new_n56_), .b(x00), .c(new_n59_), .O(z16));
  inv1   g87(.a(x02), .O(new_n131_));
  oai21  g88(.a(x19), .b(x00), .c(new_n131_), .O(new_n132_));
  oai22  g89(.a(new_n132_), .b(z16), .c(new_n116_), .d(new_n129_), .O(z15));
  inv1   g90(.a(new_n84_), .O(z17));
endmodule


