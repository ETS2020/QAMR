// Benchmark "FAU" written by ABC on Tue Aug 11 19:56:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n128_;
  nand2  g00(.a(x21), .b(x18), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x19), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(x01), .b(x00), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n51_), .b(x09), .c(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  inv1   g15(.a(x21), .O(new_n59_));
  nor2   g16(.a(x19), .b(x02), .O(new_n60_));
  nand3  g17(.a(new_n60_), .b(new_n51_), .c(new_n45_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x18), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand3  g22(.a(x11), .b(x10), .c(new_n50_), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(new_n65_), .c(x12), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n63_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand3  g28(.a(new_n44_), .b(new_n71_), .c(new_n70_), .O(new_n72_));
  nor4   g29(.a(new_n72_), .b(new_n64_), .c(new_n56_), .d(x09), .O(z04));
  nand2  g30(.a(new_n44_), .b(x09), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n64_), .c(new_n56_), .O(z05));
  oai21  g32(.a(new_n66_), .b(new_n64_), .c(new_n44_), .O(z06));
  nand2  g33(.a(new_n67_), .b(new_n71_), .O(new_n77_));
  nand3  g34(.a(new_n44_), .b(x02), .c(x00), .O(new_n78_));
  aoi21  g35(.a(new_n77_), .b(x01), .c(new_n78_), .O(z07));
  inv1   g36(.a(x03), .O(new_n80_));
  nor2   g37(.a(x06), .b(x05), .O(new_n81_));
  nor2   g38(.a(x08), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n81_), .c(x04), .d(new_n80_), .O(new_n83_));
  nand2  g40(.a(new_n51_), .b(x02), .O(new_n84_));
  nand3  g41(.a(x19), .b(new_n46_), .c(x17), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(z08));
  inv1   g43(.a(x13), .O(new_n87_));
  inv1   g44(.a(x14), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(x12), .d(new_n70_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nor2   g47(.a(new_n49_), .b(x01), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  nand3  g50(.a(x20), .b(new_n93_), .c(new_n92_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n91_), .c(new_n90_), .O(new_n96_));
  inv1   g53(.a(x20), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n47_), .c(x18), .d(x01), .O(new_n98_));
  inv1   g55(.a(x00), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n59_), .c(new_n99_), .O(new_n101_));
  aoi21  g58(.a(new_n98_), .b(new_n96_), .c(new_n101_), .O(z09));
  inv1   g59(.a(new_n84_), .O(new_n103_));
  nand3  g60(.a(new_n90_), .b(new_n103_), .c(x15), .O(new_n104_));
  nand4  g61(.a(new_n100_), .b(new_n59_), .c(x20), .d(x16), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(new_n104_), .O(z10));
  nand4  g63(.a(new_n100_), .b(new_n59_), .c(x20), .d(new_n93_), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n104_), .O(z11));
  nand2  g65(.a(x10), .b(x02), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x01), .c(x00), .O(new_n110_));
  nand2  g67(.a(new_n60_), .b(new_n45_), .O(new_n111_));
  inv1   g68(.a(x23), .O(new_n112_));
  oai21  g69(.a(x19), .b(x02), .c(new_n112_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n111_), .c(new_n51_), .O(new_n114_));
  inv1   g71(.a(x24), .O(new_n115_));
  nand3  g72(.a(new_n44_), .b(new_n115_), .c(x09), .O(new_n116_));
  aoi21  g73(.a(new_n114_), .b(new_n110_), .c(new_n116_), .O(z12));
  nand3  g74(.a(new_n60_), .b(new_n51_), .c(x17), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n44_), .O(z13));
  nor2   g76(.a(new_n57_), .b(new_n52_), .O(z14));
  inv1   g77(.a(x01), .O(new_n121_));
  oai21  g78(.a(x10), .b(new_n121_), .c(x02), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(x00), .O(new_n123_));
  nand3  g80(.a(x19), .b(new_n49_), .c(new_n121_), .O(new_n124_));
  aoi22  g81(.a(new_n124_), .b(new_n123_), .c(x21), .d(x18), .O(z15));
  nand3  g82(.a(new_n44_), .b(x01), .c(new_n99_), .O(new_n126_));
  inv1   g83(.a(new_n126_), .O(z16));
  nand2  g84(.a(new_n103_), .b(new_n44_), .O(new_n128_));
  inv1   g85(.a(new_n128_), .O(z17));
endmodule


