// Benchmark "FAU" written by ABC on Tue Aug 11 19:55:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n129_, new_n130_, new_n131_,
    new_n133_;
  nand2  g00(.a(x16), .b(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n52_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  inv1   g15(.a(x16), .O(new_n59_));
  inv1   g16(.a(x11), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x09), .O(new_n61_));
  nor2   g18(.a(new_n56_), .b(new_n45_), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(new_n61_), .c(x12), .d(x00), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n59_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(x01), .O(new_n65_));
  nor2   g22(.a(x19), .b(new_n50_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n47_), .c(new_n49_), .d(new_n45_), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n65_), .O(z03));
  inv1   g25(.a(x01), .O(new_n69_));
  nand2  g26(.a(x02), .b(x00), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(new_n71_));
  nor2   g28(.a(x12), .b(x11), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(x10), .d(new_n46_), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(new_n59_), .c(new_n69_), .O(z04));
  nand2  g31(.a(x09), .b(x01), .O(new_n75_));
  nor4   g32(.a(new_n75_), .b(new_n70_), .c(x16), .d(new_n56_), .O(z05));
  nand3  g33(.a(new_n62_), .b(new_n61_), .c(x00), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(new_n59_), .c(new_n69_), .O(z06));
  inv1   g35(.a(x12), .O(new_n79_));
  nand4  g36(.a(new_n61_), .b(new_n59_), .c(new_n79_), .d(x10), .O(new_n80_));
  aoi21  g37(.a(new_n80_), .b(x01), .c(new_n70_), .O(z07));
  inv1   g38(.a(x03), .O(new_n82_));
  nor2   g39(.a(x06), .b(x05), .O(new_n83_));
  nor2   g40(.a(x08), .b(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n83_), .c(x04), .d(new_n82_), .O(new_n85_));
  nand2  g42(.a(new_n47_), .b(x02), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(z17));
  nand4  g44(.a(z17), .b(x19), .c(new_n50_), .d(x17), .O(new_n88_));
  oai21  g45(.a(new_n88_), .b(new_n85_), .c(new_n44_), .O(z08));
  nor2   g46(.a(x20), .b(new_n69_), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(new_n66_), .O(new_n91_));
  inv1   g48(.a(x15), .O(new_n92_));
  nand4  g49(.a(x12), .b(new_n60_), .c(x02), .d(new_n69_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  nor2   g51(.a(x14), .b(x13), .O(new_n95_));
  nand2  g52(.a(new_n95_), .b(x20), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n94_), .c(new_n92_), .O(new_n98_));
  nor2   g55(.a(x22), .b(x21), .O(new_n99_));
  nor2   g56(.a(x16), .b(x00), .O(new_n100_));
  nand2  g57(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  aoi21  g58(.a(new_n98_), .b(new_n91_), .c(new_n101_), .O(z09));
  inv1   g59(.a(x00), .O(new_n103_));
  nand4  g60(.a(new_n90_), .b(new_n66_), .c(x22), .d(x21), .O(new_n104_));
  nor2   g61(.a(x11), .b(new_n45_), .O(new_n105_));
  nand4  g62(.a(x20), .b(x16), .c(x15), .d(x12), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n99_), .c(new_n95_), .d(new_n105_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n104_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n103_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n44_), .O(z10));
  nand3  g68(.a(new_n90_), .b(new_n66_), .c(x21), .O(new_n112_));
  nor2   g69(.a(x21), .b(new_n92_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n97_), .c(new_n94_), .O(new_n114_));
  inv1   g71(.a(x22), .O(new_n115_));
  nand2  g72(.a(new_n100_), .b(new_n115_), .O(new_n116_));
  aoi21  g73(.a(new_n114_), .b(new_n112_), .c(new_n116_), .O(z11));
  inv1   g74(.a(new_n47_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nand2  g78(.a(x01), .b(x00), .O(new_n122_));
  nor2   g79(.a(new_n122_), .b(new_n62_), .O(new_n123_));
  nor2   g80(.a(x24), .b(new_n46_), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n121_), .c(new_n124_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n44_), .O(z12));
  oai21  g83(.a(new_n120_), .b(new_n118_), .c(new_n44_), .O(z13));
  oai21  g84(.a(new_n57_), .b(new_n48_), .c(new_n44_), .O(z14));
  oai21  g85(.a(x10), .b(new_n69_), .c(x02), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x00), .O(new_n130_));
  nand3  g87(.a(x19), .b(new_n45_), .c(new_n69_), .O(new_n131_));
  nand3  g88(.a(new_n131_), .b(new_n130_), .c(new_n44_), .O(z15));
  nand2  g89(.a(new_n100_), .b(x01), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z16));
endmodule


