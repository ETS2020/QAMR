// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  inv1   g006(.a(x28), .O(new_n58_));
  aoi21  g007(.a(new_n57_), .b(x15), .c(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n56_), .b(new_n53_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n52_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  xor2a  g011(.a(new_n53_), .b(new_n62_), .O(new_n63_));
  nor2   g012(.a(x16), .b(x14), .O(new_n64_));
  nand2  g013(.a(x28), .b(new_n52_), .O(new_n65_));
  nor2   g014(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  oai21  g015(.a(new_n63_), .b(new_n57_), .c(new_n66_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  aoi21  g017(.a(new_n53_), .b(new_n62_), .c(new_n68_), .O(new_n69_));
  nand4  g018(.a(new_n68_), .b(new_n62_), .c(new_n55_), .d(new_n54_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x16), .O(new_n71_));
  aoi21  g020(.a(new_n57_), .b(x13), .c(new_n58_), .O(new_n72_));
  oai21  g021(.a(new_n71_), .b(new_n69_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n52_), .O(z02));
  inv1   g023(.a(x22), .O(new_n75_));
  inv1   g024(.a(new_n70_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g026(.a(new_n70_), .b(x22), .c(new_n57_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g028(.a(new_n57_), .b(x12), .c(new_n58_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n52_), .O(z03));
  nor2   g031(.a(x21), .b(x20), .O(new_n83_));
  nor2   g032(.a(x23), .b(x22), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n83_), .c(new_n53_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(x16), .O(new_n86_));
  aoi21  g035(.a(new_n77_), .b(x23), .c(new_n86_), .O(new_n87_));
  inv1   g036(.a(x11), .O(new_n88_));
  oai21  g037(.a(x16), .b(new_n88_), .c(x28), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n87_), .c(new_n52_), .O(z04));
  nand2  g039(.a(new_n85_), .b(x24), .O(new_n91_));
  inv1   g040(.a(x24), .O(new_n92_));
  nand4  g041(.a(new_n84_), .b(new_n83_), .c(new_n53_), .d(new_n92_), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n91_), .c(x16), .O(new_n94_));
  aoi21  g043(.a(new_n57_), .b(x10), .c(new_n58_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n52_), .O(z05));
  nand2  g046(.a(new_n93_), .b(x25), .O(new_n98_));
  nor2   g047(.a(x25), .b(x24), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n84_), .c(new_n83_), .d(new_n53_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x16), .O(new_n102_));
  nor2   g051(.a(x16), .b(x09), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n65_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n102_), .O(z06));
  and2   g054(.a(new_n100_), .b(x26), .O(new_n106_));
  oai21  g055(.a(new_n100_), .b(x26), .c(x16), .O(new_n107_));
  aoi21  g056(.a(new_n57_), .b(x08), .c(new_n58_), .O(new_n108_));
  oai21  g057(.a(new_n107_), .b(new_n106_), .c(new_n108_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n52_), .O(z07));
  inv1   g059(.a(x26), .O(new_n111_));
  inv1   g060(.a(x23), .O(new_n112_));
  inv1   g061(.a(x25), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n92_), .c(new_n112_), .d(new_n75_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  nand3  g064(.a(new_n115_), .b(new_n76_), .c(new_n111_), .O(new_n116_));
  inv1   g065(.a(x27), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n111_), .c(new_n113_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n93_), .O(new_n119_));
  aoi21  g068(.a(new_n116_), .b(x27), .c(new_n119_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x07), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n65_), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n57_), .c(new_n122_), .O(z08));
  nor3   g072(.a(new_n118_), .b(new_n93_), .c(new_n57_), .O(new_n124_));
  inv1   g073(.a(x06), .O(new_n125_));
  oai21  g074(.a(x16), .b(new_n125_), .c(x28), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n124_), .c(new_n52_), .O(z09));
  nand2  g076(.a(new_n57_), .b(x05), .O(new_n128_));
  inv1   g077(.a(x29), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n128_), .c(x28), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n52_), .O(z10));
  nand2  g081(.a(new_n57_), .b(x04), .O(new_n133_));
  inv1   g082(.a(x30), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n133_), .c(x28), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n52_), .O(z11));
  nand2  g086(.a(new_n57_), .b(x03), .O(new_n138_));
  inv1   g087(.a(x31), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(x16), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n138_), .c(x28), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n52_), .O(z12));
  nand2  g091(.a(new_n57_), .b(x02), .O(new_n143_));
  inv1   g092(.a(x32), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(x16), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n143_), .c(x28), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n52_), .O(z13));
  nand2  g096(.a(x33), .b(x16), .O(new_n148_));
  inv1   g097(.a(x01), .O(new_n149_));
  aoi21  g098(.a(new_n57_), .b(new_n149_), .c(new_n65_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n148_), .O(z14));
  nand2  g100(.a(new_n57_), .b(x00), .O(new_n152_));
  inv1   g101(.a(x34), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(x28), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n52_), .O(z15));
endmodule


