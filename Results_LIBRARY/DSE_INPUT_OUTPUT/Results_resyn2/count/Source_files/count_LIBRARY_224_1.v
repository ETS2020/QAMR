// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  nor2   g009(.a(x18), .b(x16), .O(new_n61_));
  inv1   g010(.a(new_n61_), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n55_), .b(new_n63_), .O(new_n64_));
  inv1   g013(.a(x18), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n64_), .c(x16), .O(new_n69_));
  oai21  g018(.a(new_n62_), .b(x14), .c(new_n69_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nand4  g020(.a(new_n71_), .b(new_n63_), .c(new_n53_), .d(new_n52_), .O(new_n72_));
  inv1   g021(.a(new_n72_), .O(new_n73_));
  aoi21  g022(.a(new_n67_), .b(x21), .c(new_n73_), .O(new_n74_));
  inv1   g023(.a(x13), .O(new_n75_));
  aoi21  g024(.a(new_n58_), .b(new_n75_), .c(x18), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n58_), .c(new_n76_), .O(z02));
  xor2a  g026(.a(new_n72_), .b(x22), .O(new_n78_));
  inv1   g027(.a(x12), .O(new_n79_));
  aoi21  g028(.a(new_n58_), .b(new_n79_), .c(x18), .O(new_n80_));
  oai21  g029(.a(new_n78_), .b(new_n58_), .c(new_n80_), .O(z03));
  inv1   g030(.a(x22), .O(new_n82_));
  nor2   g031(.a(x21), .b(x20), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n55_), .c(new_n82_), .O(new_n84_));
  nor2   g033(.a(x23), .b(x22), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n83_), .c(new_n55_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n65_), .O(new_n87_));
  aoi21  g036(.a(new_n84_), .b(x23), .c(new_n87_), .O(new_n88_));
  oai22  g037(.a(new_n88_), .b(new_n58_), .c(new_n62_), .d(x11), .O(z04));
  nor2   g038(.a(x24), .b(x23), .O(new_n90_));
  nor2   g039(.a(x22), .b(x21), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n90_), .c(new_n55_), .d(new_n63_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n65_), .O(new_n93_));
  aoi21  g042(.a(new_n86_), .b(x24), .c(new_n93_), .O(new_n94_));
  oai22  g043(.a(new_n94_), .b(new_n58_), .c(new_n62_), .d(x10), .O(z05));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n85_), .c(new_n83_), .d(new_n55_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n65_), .O(new_n98_));
  aoi21  g047(.a(new_n92_), .b(x25), .c(new_n98_), .O(new_n99_));
  oai22  g048(.a(new_n99_), .b(new_n58_), .c(new_n62_), .d(x09), .O(z06));
  inv1   g049(.a(x08), .O(new_n101_));
  nand2  g050(.a(new_n61_), .b(new_n101_), .O(new_n102_));
  nand2  g051(.a(new_n97_), .b(x26), .O(new_n103_));
  inv1   g052(.a(new_n86_), .O(new_n104_));
  inv1   g053(.a(x24), .O(new_n105_));
  nor2   g054(.a(x26), .b(x25), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g056(.a(new_n107_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n104_), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n103_), .c(new_n65_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x16), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n102_), .O(z07));
  oai21  g061(.a(new_n107_), .b(new_n86_), .c(x27), .O(new_n113_));
  nand2  g062(.a(new_n91_), .b(new_n90_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  nor3   g064(.a(x27), .b(x26), .c(x25), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n115_), .c(new_n66_), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n113_), .c(new_n65_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(x16), .O(new_n119_));
  inv1   g068(.a(x07), .O(new_n120_));
  nand2  g069(.a(new_n61_), .b(new_n120_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n119_), .O(z08));
  nor2   g071(.a(x28), .b(x27), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n106_), .c(new_n90_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n84_), .c(new_n65_), .O(new_n125_));
  aoi21  g074(.a(new_n117_), .b(x28), .c(new_n125_), .O(new_n126_));
  inv1   g075(.a(x06), .O(new_n127_));
  nand2  g076(.a(new_n61_), .b(new_n127_), .O(new_n128_));
  oai21  g077(.a(new_n126_), .b(new_n58_), .c(new_n128_), .O(z09));
  oai21  g078(.a(new_n124_), .b(new_n84_), .c(x29), .O(new_n130_));
  nor3   g079(.a(x29), .b(x28), .c(x27), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n108_), .c(new_n104_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n130_), .c(new_n65_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  inv1   g083(.a(x05), .O(new_n135_));
  nand2  g084(.a(new_n61_), .b(new_n135_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n134_), .O(z10));
  inv1   g086(.a(x29), .O(new_n138_));
  nand4  g087(.a(new_n123_), .b(new_n106_), .c(new_n138_), .d(new_n105_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n86_), .c(x30), .O(new_n140_));
  nor2   g089(.a(x30), .b(x29), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n123_), .O(new_n142_));
  inv1   g091(.a(new_n142_), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n108_), .c(new_n104_), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n140_), .c(new_n65_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(x16), .O(new_n146_));
  inv1   g095(.a(x04), .O(new_n147_));
  nand2  g096(.a(new_n61_), .b(new_n147_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n146_), .O(z11));
  nand4  g098(.a(new_n141_), .b(new_n123_), .c(new_n106_), .d(new_n105_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n86_), .c(x31), .O(new_n151_));
  inv1   g100(.a(x28), .O(new_n152_));
  inv1   g101(.a(x30), .O(new_n153_));
  inv1   g102(.a(x31), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n153_), .c(new_n138_), .d(new_n152_), .O(new_n155_));
  inv1   g104(.a(new_n155_), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n116_), .c(new_n115_), .d(new_n66_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n151_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x16), .O(new_n159_));
  inv1   g108(.a(x03), .O(new_n160_));
  aoi21  g109(.a(new_n58_), .b(new_n160_), .c(x18), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n159_), .O(z12));
  inv1   g111(.a(x25), .O(new_n163_));
  nor2   g112(.a(x27), .b(x26), .O(new_n164_));
  nor2   g113(.a(x31), .b(x28), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n141_), .c(new_n164_), .d(new_n163_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n92_), .c(x32), .O(new_n167_));
  nor2   g116(.a(x32), .b(x31), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n106_), .O(new_n169_));
  inv1   g118(.a(new_n169_), .O(new_n170_));
  nand4  g119(.a(new_n170_), .b(new_n143_), .c(new_n115_), .d(new_n66_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n167_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x16), .O(new_n173_));
  inv1   g122(.a(x02), .O(new_n174_));
  aoi21  g123(.a(new_n58_), .b(new_n174_), .c(x18), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n173_), .O(z13));
  nand4  g125(.a(new_n168_), .b(new_n141_), .c(new_n123_), .d(new_n106_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(new_n92_), .c(x33), .O(new_n178_));
  inv1   g127(.a(x23), .O(new_n179_));
  nand4  g128(.a(new_n163_), .b(new_n105_), .c(new_n179_), .d(new_n82_), .O(new_n180_));
  inv1   g129(.a(new_n180_), .O(new_n181_));
  inv1   g130(.a(x26), .O(new_n182_));
  inv1   g131(.a(x27), .O(new_n183_));
  inv1   g132(.a(x32), .O(new_n184_));
  inv1   g133(.a(x33), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n184_), .c(new_n183_), .d(new_n182_), .O(new_n186_));
  inv1   g135(.a(new_n186_), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n156_), .c(new_n181_), .d(new_n73_), .O(new_n188_));
  nand3  g137(.a(new_n188_), .b(new_n178_), .c(new_n65_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(x16), .O(new_n190_));
  inv1   g139(.a(x01), .O(new_n191_));
  nand2  g140(.a(new_n61_), .b(new_n191_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n190_), .O(z14));
  inv1   g142(.a(new_n97_), .O(new_n194_));
  nand4  g143(.a(new_n187_), .b(new_n156_), .c(new_n194_), .d(x34), .O(new_n195_));
  inv1   g144(.a(x34), .O(new_n196_));
  nand2  g145(.a(new_n188_), .b(new_n196_), .O(new_n197_));
  nand3  g146(.a(new_n197_), .b(new_n195_), .c(x16), .O(new_n198_));
  inv1   g147(.a(x00), .O(new_n199_));
  aoi21  g148(.a(new_n58_), .b(new_n199_), .c(x18), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n198_), .O(z15));
endmodule


