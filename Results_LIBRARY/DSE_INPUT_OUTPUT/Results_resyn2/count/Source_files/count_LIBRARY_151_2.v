// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x30), .O(new_n58_));
  nor2   g007(.a(x33), .b(new_n58_), .O(new_n59_));
  inv1   g008(.a(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  aoi21  g010(.a(new_n56_), .b(new_n55_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n54_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n53_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n53_), .O(new_n73_));
  oai21  g022(.a(new_n66_), .b(new_n71_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n56_), .b(new_n76_), .c(x18), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n75_), .c(new_n59_), .O(z02));
  xor2a  g027(.a(new_n73_), .b(x22), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n61_), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n56_), .c(new_n81_), .O(z03));
  inv1   g031(.a(x22), .O(new_n83_));
  nand3  g032(.a(new_n72_), .b(new_n53_), .c(new_n83_), .O(new_n84_));
  nor2   g033(.a(x23), .b(x22), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n72_), .c(new_n53_), .O(new_n86_));
  inv1   g035(.a(new_n86_), .O(new_n87_));
  aoi21  g036(.a(new_n84_), .b(x23), .c(new_n87_), .O(new_n88_));
  nor2   g037(.a(x16), .b(x11), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n61_), .O(new_n90_));
  oai21  g039(.a(new_n88_), .b(new_n56_), .c(new_n90_), .O(z04));
  nor2   g040(.a(x24), .b(x21), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  aoi22  g043(.a(new_n94_), .b(new_n66_), .c(new_n86_), .d(x24), .O(new_n95_));
  nor2   g044(.a(x16), .b(x10), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n61_), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n56_), .c(new_n97_), .O(z05));
  nand4  g047(.a(new_n92_), .b(new_n85_), .c(new_n53_), .d(new_n64_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(x25), .O(new_n100_));
  nor2   g049(.a(x25), .b(x24), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n85_), .c(new_n72_), .d(new_n53_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g053(.a(x09), .O(new_n105_));
  aoi21  g054(.a(new_n56_), .b(new_n105_), .c(x18), .O(new_n106_));
  aoi21  g055(.a(new_n106_), .b(new_n104_), .c(new_n59_), .O(z06));
  inv1   g056(.a(x24), .O(new_n108_));
  nor2   g057(.a(x26), .b(x25), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n86_), .O(new_n111_));
  aoi21  g060(.a(new_n102_), .b(x26), .c(new_n111_), .O(new_n112_));
  nor2   g061(.a(x16), .b(x08), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n61_), .O(new_n114_));
  oai21  g063(.a(new_n112_), .b(new_n56_), .c(new_n114_), .O(z07));
  inv1   g064(.a(x27), .O(new_n116_));
  nor3   g065(.a(x27), .b(x26), .c(x25), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n94_), .c(new_n66_), .O(new_n118_));
  oai21  g067(.a(new_n111_), .b(new_n116_), .c(new_n118_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x16), .O(new_n120_));
  inv1   g069(.a(x07), .O(new_n121_));
  aoi21  g070(.a(new_n56_), .b(new_n121_), .c(x18), .O(new_n122_));
  aoi21  g071(.a(new_n122_), .b(new_n120_), .c(new_n59_), .O(z08));
  nor2   g072(.a(x28), .b(x27), .O(new_n124_));
  aoi22  g073(.a(new_n124_), .b(new_n111_), .c(new_n118_), .d(x28), .O(new_n125_));
  nor2   g074(.a(x16), .b(x06), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n61_), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n56_), .c(new_n127_), .O(z09));
  inv1   g077(.a(x29), .O(new_n129_));
  aoi21  g078(.a(new_n124_), .b(new_n111_), .c(new_n129_), .O(new_n130_));
  nand4  g079(.a(new_n109_), .b(new_n85_), .c(new_n72_), .d(new_n108_), .O(new_n131_));
  nand3  g080(.a(new_n124_), .b(new_n53_), .c(new_n129_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n130_), .c(x16), .O(new_n134_));
  inv1   g083(.a(x05), .O(new_n135_));
  aoi21  g084(.a(new_n56_), .b(new_n135_), .c(x18), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n134_), .c(new_n59_), .O(z10));
  xor2a  g086(.a(new_n133_), .b(x30), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(x16), .O(new_n139_));
  inv1   g088(.a(x04), .O(new_n140_));
  aoi21  g089(.a(new_n56_), .b(new_n140_), .c(x18), .O(new_n141_));
  aoi21  g090(.a(new_n141_), .b(new_n139_), .c(new_n59_), .O(z11));
  oai21  g091(.a(new_n132_), .b(new_n131_), .c(x31), .O(new_n143_));
  inv1   g092(.a(new_n110_), .O(new_n144_));
  nand2  g093(.a(new_n85_), .b(new_n72_), .O(new_n145_));
  inv1   g094(.a(new_n145_), .O(new_n146_));
  nor2   g095(.a(x31), .b(x29), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n124_), .c(new_n53_), .O(new_n148_));
  inv1   g097(.a(new_n148_), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n146_), .c(new_n144_), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n143_), .c(x30), .O(new_n151_));
  inv1   g100(.a(x31), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n58_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x33), .O(new_n154_));
  inv1   g103(.a(new_n154_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n151_), .c(x16), .O(new_n156_));
  oai21  g105(.a(new_n153_), .b(new_n57_), .c(x16), .O(new_n157_));
  aoi21  g106(.a(new_n57_), .b(x03), .c(new_n59_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n156_), .O(z12));
  oai21  g109(.a(new_n148_), .b(new_n131_), .c(x32), .O(new_n161_));
  inv1   g110(.a(x28), .O(new_n162_));
  inv1   g111(.a(x32), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n152_), .c(new_n129_), .d(new_n162_), .O(new_n164_));
  inv1   g113(.a(new_n164_), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n117_), .c(new_n94_), .d(new_n66_), .O(new_n166_));
  aoi21  g115(.a(new_n166_), .b(new_n161_), .c(x30), .O(new_n167_));
  nor2   g116(.a(new_n163_), .b(new_n58_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(x33), .O(new_n169_));
  inv1   g118(.a(new_n169_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n167_), .c(x16), .O(new_n171_));
  oai21  g120(.a(new_n168_), .b(new_n57_), .c(x16), .O(new_n172_));
  aoi21  g121(.a(new_n57_), .b(x02), .c(new_n59_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n172_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n171_), .O(z13));
  inv1   g124(.a(x33), .O(new_n176_));
  inv1   g125(.a(x25), .O(new_n177_));
  nor2   g126(.a(x27), .b(x26), .O(new_n178_));
  nor2   g127(.a(x32), .b(x28), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n147_), .c(new_n178_), .d(new_n177_), .O(new_n180_));
  nor2   g129(.a(new_n180_), .b(new_n99_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n176_), .O(new_n182_));
  nand2  g131(.a(new_n147_), .b(new_n124_), .O(new_n183_));
  nand3  g132(.a(new_n109_), .b(new_n176_), .c(new_n163_), .O(new_n184_));
  nor3   g133(.a(new_n184_), .b(new_n183_), .c(new_n99_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n182_), .c(x16), .O(new_n186_));
  aoi21  g135(.a(x33), .b(new_n56_), .c(new_n58_), .O(new_n187_));
  oai21  g136(.a(x16), .b(x01), .c(new_n57_), .O(new_n188_));
  nor2   g137(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n186_), .O(z14));
  oai21  g139(.a(x16), .b(x00), .c(new_n57_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n60_), .O(new_n192_));
  oai21  g141(.a(new_n180_), .b(new_n99_), .c(x34), .O(new_n193_));
  inv1   g142(.a(new_n102_), .O(new_n194_));
  inv1   g143(.a(x26), .O(new_n195_));
  inv1   g144(.a(x34), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n176_), .c(new_n116_), .d(new_n195_), .O(new_n197_));
  nor2   g146(.a(new_n197_), .b(new_n164_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n194_), .O(new_n199_));
  aoi21  g148(.a(new_n199_), .b(new_n193_), .c(x30), .O(new_n200_));
  nor2   g149(.a(new_n196_), .b(new_n176_), .O(new_n201_));
  oai21  g150(.a(new_n201_), .b(new_n200_), .c(x16), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n192_), .O(z15));
endmodule


