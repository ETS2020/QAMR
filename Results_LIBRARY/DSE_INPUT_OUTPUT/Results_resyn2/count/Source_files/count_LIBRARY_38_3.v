// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_;
  inv1   g000(.a(x13), .O(new_n52_));
  nand2  g001(.a(x19), .b(x17), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  nand3  g004(.a(new_n55_), .b(new_n53_), .c(new_n52_), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n58_), .c(x18), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(z00));
  inv1   g010(.a(x14), .O(new_n62_));
  oai21  g011(.a(x18), .b(new_n62_), .c(new_n59_), .O(new_n63_));
  nand2  g012(.a(new_n55_), .b(x20), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand2  g014(.a(new_n54_), .b(new_n65_), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n64_), .c(new_n59_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(x18), .c(new_n52_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n63_), .O(z01));
  aoi21  g018(.a(x18), .b(new_n59_), .c(new_n52_), .O(new_n70_));
  nand2  g019(.a(new_n66_), .b(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n54_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  nor3   g023(.a(new_n74_), .b(x18), .c(new_n59_), .O(new_n75_));
  aoi21  g024(.a(new_n75_), .b(new_n71_), .c(new_n70_), .O(z02));
  inv1   g025(.a(x12), .O(new_n77_));
  oai21  g026(.a(x18), .b(new_n77_), .c(new_n59_), .O(new_n78_));
  nand2  g027(.a(new_n73_), .b(x22), .O(new_n79_));
  inv1   g028(.a(x22), .O(new_n80_));
  nand3  g029(.a(new_n72_), .b(new_n54_), .c(new_n80_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(x16), .c(x18), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(x13), .c(new_n78_), .O(z03));
  nor2   g033(.a(new_n59_), .b(new_n52_), .O(new_n85_));
  nand2  g034(.a(new_n81_), .b(x23), .O(new_n86_));
  nor2   g035(.a(x23), .b(x22), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n72_), .c(new_n54_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(x16), .O(new_n90_));
  inv1   g039(.a(x11), .O(new_n91_));
  aoi21  g040(.a(new_n59_), .b(new_n91_), .c(x18), .O(new_n92_));
  aoi21  g041(.a(new_n92_), .b(new_n90_), .c(new_n85_), .O(z04));
  inv1   g042(.a(x18), .O(new_n94_));
  nand2  g043(.a(new_n59_), .b(x10), .O(new_n95_));
  nor2   g044(.a(x24), .b(x23), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n72_), .c(new_n54_), .d(new_n80_), .O(new_n97_));
  nand2  g046(.a(new_n88_), .b(x24), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n97_), .c(x16), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n95_), .O(new_n100_));
  aoi21  g049(.a(new_n100_), .b(new_n94_), .c(new_n85_), .O(z05));
  nor2   g050(.a(x25), .b(x24), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n87_), .c(new_n72_), .d(new_n54_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n52_), .O(new_n104_));
  aoi21  g053(.a(new_n97_), .b(x25), .c(new_n104_), .O(new_n105_));
  inv1   g054(.a(x09), .O(new_n106_));
  aoi21  g055(.a(new_n59_), .b(new_n106_), .c(x18), .O(new_n107_));
  oai21  g056(.a(new_n105_), .b(new_n59_), .c(new_n107_), .O(z06));
  inv1   g057(.a(new_n88_), .O(new_n109_));
  nor3   g058(.a(x26), .b(x25), .c(x24), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n52_), .O(new_n112_));
  aoi21  g061(.a(new_n103_), .b(x26), .c(new_n112_), .O(new_n113_));
  inv1   g062(.a(x08), .O(new_n114_));
  aoi21  g063(.a(new_n59_), .b(new_n114_), .c(x18), .O(new_n115_));
  oai21  g064(.a(new_n113_), .b(new_n59_), .c(new_n115_), .O(z07));
  inv1   g065(.a(x07), .O(new_n117_));
  nor2   g066(.a(x16), .b(new_n117_), .O(new_n118_));
  inv1   g067(.a(x27), .O(new_n119_));
  nor2   g068(.a(x26), .b(x25), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nor2   g070(.a(new_n59_), .b(x13), .O(new_n122_));
  oai21  g071(.a(new_n121_), .b(new_n97_), .c(new_n122_), .O(new_n123_));
  aoi21  g072(.a(new_n111_), .b(x27), .c(new_n123_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n118_), .c(new_n94_), .O(z08));
  inv1   g074(.a(x28), .O(new_n126_));
  nor2   g075(.a(new_n121_), .b(new_n97_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nor3   g077(.a(x27), .b(x26), .c(x25), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n96_), .c(new_n126_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n81_), .c(new_n52_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n128_), .c(x16), .O(new_n132_));
  inv1   g081(.a(x06), .O(new_n133_));
  aoi21  g082(.a(new_n59_), .b(new_n133_), .c(x18), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(z09));
  oai21  g084(.a(new_n130_), .b(new_n81_), .c(x29), .O(new_n136_));
  nor3   g085(.a(x29), .b(x28), .c(x27), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n110_), .c(new_n109_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(x16), .O(new_n140_));
  inv1   g089(.a(x05), .O(new_n141_));
  aoi21  g090(.a(new_n59_), .b(new_n141_), .c(x18), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n140_), .c(new_n85_), .O(z10));
  inv1   g092(.a(x30), .O(new_n144_));
  nand4  g093(.a(new_n137_), .b(new_n110_), .c(new_n109_), .d(new_n144_), .O(new_n145_));
  nand2  g094(.a(new_n138_), .b(x30), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n145_), .c(new_n52_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(x16), .O(new_n148_));
  inv1   g097(.a(x04), .O(new_n149_));
  aoi21  g098(.a(new_n59_), .b(new_n149_), .c(x18), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n148_), .O(z11));
  nand2  g100(.a(new_n145_), .b(x31), .O(new_n152_));
  nor2   g101(.a(x29), .b(x28), .O(new_n153_));
  nor2   g102(.a(x31), .b(x30), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  inv1   g104(.a(new_n155_), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n127_), .c(x13), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n152_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x16), .O(new_n159_));
  inv1   g108(.a(x03), .O(new_n160_));
  aoi21  g109(.a(new_n59_), .b(new_n160_), .c(x18), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n159_), .O(z12));
  nand2  g111(.a(new_n156_), .b(new_n127_), .O(new_n163_));
  nor3   g112(.a(x32), .b(x31), .c(x30), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n137_), .c(new_n120_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n97_), .c(new_n52_), .O(new_n166_));
  aoi21  g115(.a(new_n163_), .b(x32), .c(new_n166_), .O(new_n167_));
  inv1   g116(.a(x02), .O(new_n168_));
  aoi21  g117(.a(new_n59_), .b(new_n168_), .c(x18), .O(new_n169_));
  oai21  g118(.a(new_n167_), .b(new_n59_), .c(new_n169_), .O(z13));
  inv1   g119(.a(new_n97_), .O(new_n171_));
  nand4  g120(.a(new_n164_), .b(new_n137_), .c(new_n120_), .d(new_n171_), .O(new_n172_));
  nor2   g121(.a(x27), .b(x26), .O(new_n173_));
  nor2   g122(.a(x33), .b(x32), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n173_), .c(new_n154_), .d(new_n153_), .O(new_n175_));
  oai21  g124(.a(new_n175_), .b(new_n103_), .c(new_n52_), .O(new_n176_));
  aoi21  g125(.a(new_n172_), .b(x33), .c(new_n176_), .O(new_n177_));
  inv1   g126(.a(x01), .O(new_n178_));
  aoi21  g127(.a(new_n59_), .b(new_n178_), .c(x18), .O(new_n179_));
  oai21  g128(.a(new_n177_), .b(new_n59_), .c(new_n179_), .O(z14));
  nor2   g129(.a(new_n175_), .b(new_n103_), .O(new_n181_));
  xor2a  g130(.a(new_n181_), .b(x34), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(x16), .O(new_n183_));
  inv1   g132(.a(x00), .O(new_n184_));
  aoi21  g133(.a(new_n59_), .b(new_n184_), .c(x18), .O(new_n185_));
  aoi21  g134(.a(new_n185_), .b(new_n183_), .c(new_n85_), .O(z15));
endmodule


