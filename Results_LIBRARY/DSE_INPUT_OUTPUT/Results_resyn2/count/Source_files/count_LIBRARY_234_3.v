// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(x17), .b(new_n58_), .O(new_n62_));
  aoi21  g011(.a(new_n62_), .b(new_n61_), .c(x18), .O(new_n63_));
  inv1   g012(.a(x18), .O(new_n64_));
  nor2   g013(.a(x16), .b(x14), .O(new_n65_));
  nor3   g014(.a(new_n55_), .b(new_n61_), .c(new_n58_), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  oai21  g016(.a(new_n63_), .b(x19), .c(new_n67_), .O(z01));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n62_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n53_), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  inv1   g022(.a(x21), .O(new_n74_));
  nor3   g023(.a(x20), .b(x19), .c(x17), .O(new_n75_));
  nor3   g024(.a(new_n75_), .b(new_n74_), .c(new_n58_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(new_n73_), .c(new_n64_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n72_), .O(z02));
  nand4  g027(.a(new_n74_), .b(new_n61_), .c(new_n53_), .d(new_n52_), .O(new_n79_));
  xor2a  g028(.a(new_n79_), .b(x22), .O(new_n80_));
  inv1   g029(.a(x12), .O(new_n81_));
  aoi21  g030(.a(new_n58_), .b(new_n81_), .c(x18), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n58_), .c(new_n82_), .O(z03));
  nor2   g032(.a(x23), .b(x22), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n69_), .c(new_n62_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n64_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n53_), .O(new_n87_));
  inv1   g036(.a(x22), .O(new_n88_));
  inv1   g037(.a(new_n79_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  inv1   g040(.a(x23), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n58_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n90_), .c(new_n91_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(x18), .c(new_n87_), .O(z04));
  nand3  g044(.a(new_n84_), .b(new_n69_), .c(new_n55_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x24), .O(new_n97_));
  nor2   g046(.a(x22), .b(x21), .O(new_n98_));
  nor2   g047(.a(x24), .b(x23), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n98_), .c(new_n55_), .d(new_n61_), .O(new_n100_));
  aoi21  g049(.a(new_n100_), .b(new_n97_), .c(new_n58_), .O(new_n101_));
  oai21  g050(.a(x16), .b(x10), .c(new_n64_), .O(new_n102_));
  or2    g051(.a(new_n102_), .b(new_n101_), .O(z05));
  nand2  g052(.a(new_n100_), .b(x25), .O(new_n104_));
  nor2   g053(.a(x25), .b(x24), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n84_), .c(new_n69_), .d(new_n55_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(x16), .O(new_n108_));
  inv1   g057(.a(x09), .O(new_n109_));
  aoi21  g058(.a(new_n58_), .b(new_n109_), .c(x18), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n108_), .O(z06));
  nor3   g060(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n84_), .c(new_n69_), .d(new_n62_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n64_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n53_), .O(new_n115_));
  nor2   g064(.a(x16), .b(x08), .O(new_n116_));
  inv1   g065(.a(x26), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n58_), .O(new_n118_));
  aoi21  g067(.a(new_n118_), .b(new_n106_), .c(new_n116_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(x18), .c(new_n115_), .O(z07));
  nor3   g069(.a(x27), .b(x26), .c(x25), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n99_), .c(new_n98_), .d(new_n75_), .O(new_n122_));
  inv1   g071(.a(x24), .O(new_n123_));
  nor2   g072(.a(x26), .b(x25), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n84_), .c(new_n123_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n79_), .c(x27), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n122_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(x16), .O(new_n128_));
  inv1   g077(.a(x07), .O(new_n129_));
  aoi21  g078(.a(new_n58_), .b(new_n129_), .c(x18), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n128_), .O(z08));
  nor2   g080(.a(x28), .b(x27), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n124_), .c(new_n123_), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n96_), .O(new_n134_));
  aoi21  g083(.a(new_n122_), .b(x28), .c(new_n134_), .O(new_n135_));
  inv1   g084(.a(x06), .O(new_n136_));
  aoi21  g085(.a(new_n58_), .b(new_n136_), .c(x18), .O(new_n137_));
  oai21  g086(.a(new_n135_), .b(new_n58_), .c(new_n137_), .O(z09));
  inv1   g087(.a(x29), .O(new_n139_));
  nand2  g088(.a(new_n132_), .b(new_n139_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n113_), .c(new_n64_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n53_), .O(new_n142_));
  nand2  g091(.a(x29), .b(x16), .O(new_n143_));
  inv1   g092(.a(x05), .O(new_n144_));
  nand2  g093(.a(new_n58_), .b(new_n144_), .O(new_n145_));
  oai21  g094(.a(new_n143_), .b(new_n134_), .c(new_n145_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n64_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n142_), .O(z10));
  nor2   g097(.a(x30), .b(x29), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n132_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n113_), .c(new_n64_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n53_), .O(new_n152_));
  nand4  g101(.a(new_n132_), .b(new_n69_), .c(new_n55_), .d(new_n139_), .O(new_n153_));
  inv1   g102(.a(x30), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n58_), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n125_), .c(new_n155_), .O(new_n156_));
  inv1   g105(.a(x04), .O(new_n157_));
  nand2  g106(.a(new_n58_), .b(new_n157_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n156_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n64_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n152_), .O(z11));
  nand4  g110(.a(new_n99_), .b(new_n98_), .c(new_n62_), .d(new_n61_), .O(new_n162_));
  inv1   g111(.a(x25), .O(new_n163_));
  nor2   g112(.a(x27), .b(x26), .O(new_n164_));
  nor2   g113(.a(x31), .b(x28), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n149_), .c(new_n164_), .d(new_n163_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n162_), .c(new_n64_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n53_), .O(new_n168_));
  nand4  g117(.a(new_n149_), .b(new_n132_), .c(new_n69_), .d(new_n55_), .O(new_n169_));
  inv1   g118(.a(x31), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n58_), .O(new_n171_));
  oai21  g120(.a(new_n169_), .b(new_n125_), .c(new_n171_), .O(new_n172_));
  inv1   g121(.a(x03), .O(new_n173_));
  nand2  g122(.a(new_n58_), .b(new_n173_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n172_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n64_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n168_), .O(z12));
  nor2   g126(.a(x32), .b(x31), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n149_), .c(new_n132_), .d(new_n124_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n162_), .c(new_n64_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n53_), .O(new_n181_));
  inv1   g130(.a(x32), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n58_), .O(new_n183_));
  oai21  g132(.a(new_n166_), .b(new_n100_), .c(new_n183_), .O(new_n184_));
  inv1   g133(.a(x02), .O(new_n185_));
  nand2  g134(.a(new_n58_), .b(new_n185_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n184_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n64_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n181_), .O(z13));
  oai21  g138(.a(new_n179_), .b(new_n100_), .c(x33), .O(new_n190_));
  nand4  g139(.a(new_n163_), .b(new_n123_), .c(new_n92_), .d(new_n88_), .O(new_n191_));
  inv1   g140(.a(new_n191_), .O(new_n192_));
  inv1   g141(.a(x28), .O(new_n193_));
  nand4  g142(.a(new_n170_), .b(new_n154_), .c(new_n139_), .d(new_n193_), .O(new_n194_));
  inv1   g143(.a(new_n194_), .O(new_n195_));
  inv1   g144(.a(x27), .O(new_n196_));
  inv1   g145(.a(x33), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n182_), .c(new_n196_), .d(new_n117_), .O(new_n198_));
  inv1   g147(.a(new_n198_), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n195_), .c(new_n192_), .d(new_n89_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n190_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(x16), .O(new_n202_));
  inv1   g151(.a(x01), .O(new_n203_));
  aoi21  g152(.a(new_n58_), .b(new_n203_), .c(x18), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n202_), .O(z14));
  nor2   g154(.a(x33), .b(x32), .O(new_n206_));
  nand4  g155(.a(new_n206_), .b(new_n165_), .c(new_n149_), .d(new_n164_), .O(new_n207_));
  nor3   g156(.a(x34), .b(x25), .c(x24), .O(new_n208_));
  nand4  g157(.a(new_n208_), .b(new_n84_), .c(new_n69_), .d(new_n62_), .O(new_n209_));
  oai21  g158(.a(new_n209_), .b(new_n207_), .c(new_n64_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(new_n53_), .O(new_n211_));
  and2   g160(.a(x34), .b(x16), .O(new_n212_));
  oai21  g161(.a(new_n207_), .b(new_n106_), .c(new_n212_), .O(new_n213_));
  inv1   g162(.a(x00), .O(new_n214_));
  nand2  g163(.a(new_n58_), .b(new_n214_), .O(new_n215_));
  nand2  g164(.a(new_n215_), .b(new_n213_), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(new_n64_), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n211_), .O(z15));
endmodule


