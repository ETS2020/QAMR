// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x24), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(x18), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  oai21  g013(.a(new_n55_), .b(new_n64_), .c(x16), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(x14), .c(x18), .O(new_n66_));
  oai21  g015(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n62_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(new_n63_), .b(new_n69_), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n55_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  aoi21  g022(.a(new_n58_), .b(x13), .c(x18), .O(new_n74_));
  oai21  g023(.a(new_n73_), .b(new_n70_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n62_), .O(z02));
  inv1   g025(.a(x18), .O(new_n77_));
  inv1   g026(.a(x22), .O(new_n78_));
  nand4  g027(.a(new_n63_), .b(new_n78_), .c(new_n69_), .d(x16), .O(new_n79_));
  inv1   g028(.a(x12), .O(new_n80_));
  nand2  g029(.a(new_n58_), .b(new_n80_), .O(new_n81_));
  nand3  g030(.a(new_n72_), .b(x22), .c(x16), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n81_), .c(new_n79_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n77_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n62_), .O(z03));
  nor2   g034(.a(x20), .b(x19), .O(new_n86_));
  nand4  g035(.a(new_n86_), .b(new_n78_), .c(new_n69_), .d(new_n52_), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n71_), .c(new_n55_), .O(new_n89_));
  inv1   g038(.a(new_n89_), .O(new_n90_));
  aoi21  g039(.a(new_n87_), .b(x23), .c(new_n90_), .O(new_n91_));
  inv1   g040(.a(x11), .O(new_n92_));
  aoi21  g041(.a(new_n58_), .b(new_n92_), .c(x18), .O(new_n93_));
  oai21  g042(.a(new_n91_), .b(new_n58_), .c(new_n93_), .O(z04));
  nor2   g043(.a(x24), .b(x21), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n88_), .O(new_n96_));
  inv1   g045(.a(new_n96_), .O(new_n97_));
  aoi22  g046(.a(new_n97_), .b(new_n63_), .c(new_n89_), .d(x24), .O(new_n98_));
  inv1   g047(.a(x10), .O(new_n99_));
  aoi21  g048(.a(new_n58_), .b(new_n99_), .c(x18), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n58_), .c(new_n100_), .O(z05));
  nand4  g050(.a(new_n95_), .b(new_n88_), .c(new_n86_), .d(new_n52_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(x25), .O(new_n103_));
  nor2   g052(.a(x25), .b(x24), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n88_), .c(new_n71_), .d(new_n55_), .O(new_n105_));
  aoi21  g054(.a(new_n105_), .b(new_n103_), .c(new_n58_), .O(new_n106_));
  oai21  g055(.a(x16), .b(x09), .c(new_n77_), .O(new_n107_));
  or2    g056(.a(new_n107_), .b(new_n106_), .O(z06));
  inv1   g057(.a(x26), .O(new_n109_));
  inv1   g058(.a(x25), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n69_), .c(new_n52_), .d(x16), .O(new_n111_));
  inv1   g060(.a(x23), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n78_), .c(new_n64_), .d(new_n53_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  aoi21  g063(.a(new_n114_), .b(new_n109_), .c(x18), .O(new_n115_));
  nor2   g064(.a(x16), .b(x08), .O(new_n116_));
  nor2   g065(.a(new_n109_), .b(new_n58_), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n105_), .c(new_n116_), .O(new_n118_));
  oai22  g067(.a(new_n118_), .b(x18), .c(new_n115_), .d(x24), .O(z07));
  nor2   g068(.a(x27), .b(x26), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n114_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n77_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n61_), .O(new_n123_));
  nor2   g072(.a(x26), .b(x25), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n61_), .O(new_n125_));
  inv1   g074(.a(x27), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n58_), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n89_), .c(new_n127_), .O(new_n128_));
  oai21  g077(.a(x16), .b(x07), .c(new_n128_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n77_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n123_), .O(z08));
  inv1   g080(.a(x28), .O(new_n132_));
  nand3  g081(.a(new_n120_), .b(new_n114_), .c(new_n132_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n77_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n61_), .O(new_n135_));
  nand3  g084(.a(new_n120_), .b(new_n104_), .c(new_n112_), .O(new_n136_));
  nor2   g085(.a(new_n132_), .b(new_n58_), .O(new_n137_));
  oai21  g086(.a(new_n136_), .b(new_n87_), .c(new_n137_), .O(new_n138_));
  inv1   g087(.a(x06), .O(new_n139_));
  nand2  g088(.a(new_n58_), .b(new_n139_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n77_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n135_), .O(z09));
  nand4  g092(.a(new_n120_), .b(new_n104_), .c(new_n132_), .d(new_n112_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n87_), .c(x29), .O(new_n145_));
  nor2   g094(.a(x29), .b(x28), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n124_), .c(new_n126_), .d(new_n61_), .O(new_n147_));
  inv1   g096(.a(new_n147_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n90_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n145_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  inv1   g100(.a(x05), .O(new_n152_));
  aoi21  g101(.a(new_n58_), .b(new_n152_), .c(x18), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n151_), .O(z10));
  nor2   g103(.a(x30), .b(x27), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n146_), .c(new_n124_), .d(new_n112_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n79_), .c(new_n77_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n61_), .O(new_n158_));
  inv1   g107(.a(x30), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n58_), .O(new_n160_));
  oai21  g109(.a(new_n147_), .b(new_n89_), .c(new_n160_), .O(new_n161_));
  inv1   g110(.a(x04), .O(new_n162_));
  nand2  g111(.a(new_n58_), .b(new_n162_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n161_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n77_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n158_), .O(z11));
  nor2   g115(.a(x31), .b(x30), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n146_), .c(new_n120_), .d(new_n110_), .O(new_n168_));
  nor3   g117(.a(new_n168_), .b(new_n89_), .c(new_n58_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(x18), .c(new_n61_), .O(new_n170_));
  nand4  g119(.a(new_n155_), .b(new_n146_), .c(new_n124_), .d(new_n61_), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(new_n89_), .c(x31), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x16), .O(new_n173_));
  aoi21  g122(.a(new_n58_), .b(x03), .c(x18), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n173_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n170_), .O(z12));
  oai21  g125(.a(new_n168_), .b(new_n102_), .c(x32), .O(new_n177_));
  nand2  g126(.a(new_n155_), .b(new_n146_), .O(new_n178_));
  inv1   g127(.a(new_n178_), .O(new_n179_));
  nor2   g128(.a(x32), .b(x31), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n124_), .O(new_n181_));
  inv1   g130(.a(new_n181_), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n179_), .c(new_n97_), .d(new_n63_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n177_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(x16), .O(new_n185_));
  inv1   g134(.a(x02), .O(new_n186_));
  aoi21  g135(.a(new_n58_), .b(new_n186_), .c(x18), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n185_), .O(z13));
  inv1   g137(.a(x29), .O(new_n189_));
  inv1   g138(.a(x31), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n159_), .c(new_n189_), .d(new_n132_), .O(new_n191_));
  inv1   g140(.a(x32), .O(new_n192_));
  inv1   g141(.a(x33), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n192_), .c(new_n126_), .d(new_n109_), .O(new_n194_));
  nor2   g143(.a(new_n194_), .b(new_n191_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n114_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n77_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n61_), .O(new_n198_));
  nand4  g147(.a(new_n180_), .b(new_n155_), .c(new_n146_), .d(new_n124_), .O(new_n199_));
  nor2   g148(.a(new_n193_), .b(new_n58_), .O(new_n200_));
  oai21  g149(.a(new_n199_), .b(new_n102_), .c(new_n200_), .O(new_n201_));
  inv1   g150(.a(x01), .O(new_n202_));
  nand2  g151(.a(new_n58_), .b(new_n202_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n201_), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n77_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(new_n198_), .O(z14));
  inv1   g155(.a(x34), .O(new_n207_));
  nand3  g156(.a(new_n195_), .b(new_n114_), .c(new_n207_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n77_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n61_), .O(new_n210_));
  nor2   g159(.a(x33), .b(x32), .O(new_n211_));
  nand4  g160(.a(new_n211_), .b(new_n167_), .c(new_n146_), .d(new_n120_), .O(new_n212_));
  nor2   g161(.a(new_n207_), .b(new_n58_), .O(new_n213_));
  oai21  g162(.a(new_n212_), .b(new_n105_), .c(new_n213_), .O(new_n214_));
  inv1   g163(.a(x00), .O(new_n215_));
  nand2  g164(.a(new_n58_), .b(new_n215_), .O(new_n216_));
  nand2  g165(.a(new_n216_), .b(new_n214_), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n77_), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n210_), .O(z15));
endmodule


