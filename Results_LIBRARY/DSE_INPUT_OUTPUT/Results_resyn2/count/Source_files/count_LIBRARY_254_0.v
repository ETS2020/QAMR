// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  nand2  g006(.a(x30), .b(new_n57_), .O(new_n58_));
  aoi21  g007(.a(new_n56_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n54_), .O(z00));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  inv1   g010(.a(x20), .O(new_n62_));
  oai21  g011(.a(new_n53_), .b(new_n62_), .c(x16), .O(new_n63_));
  inv1   g012(.a(x30), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n63_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n57_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  xor2a  g017(.a(new_n61_), .b(new_n68_), .O(new_n69_));
  nor2   g018(.a(x16), .b(x13), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n56_), .c(new_n71_), .O(z02));
  nand2  g021(.a(new_n61_), .b(new_n68_), .O(new_n73_));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  aoi22  g023(.a(new_n74_), .b(new_n61_), .c(new_n73_), .d(x22), .O(new_n75_));
  nor2   g024(.a(x16), .b(x12), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n58_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n56_), .c(new_n77_), .O(z03));
  inv1   g027(.a(x23), .O(new_n79_));
  aoi21  g028(.a(new_n74_), .b(new_n61_), .c(new_n79_), .O(new_n80_));
  nand4  g029(.a(new_n74_), .b(new_n53_), .c(new_n79_), .d(new_n62_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x16), .O(new_n82_));
  aoi21  g031(.a(new_n56_), .b(x11), .c(new_n64_), .O(new_n83_));
  oai21  g032(.a(new_n82_), .b(new_n80_), .c(new_n83_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n57_), .O(z04));
  inv1   g034(.a(x24), .O(new_n86_));
  inv1   g035(.a(new_n81_), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g037(.a(new_n81_), .b(x24), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n88_), .c(x16), .O(new_n90_));
  nor2   g039(.a(x16), .b(x10), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n58_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n90_), .O(z05));
  nand4  g042(.a(new_n74_), .b(new_n61_), .c(new_n86_), .d(new_n79_), .O(new_n94_));
  inv1   g043(.a(x25), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n86_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n81_), .O(new_n97_));
  aoi21  g046(.a(new_n94_), .b(x25), .c(new_n97_), .O(new_n98_));
  nor2   g047(.a(x16), .b(x09), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n58_), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n56_), .c(new_n100_), .O(z06));
  nor2   g050(.a(x26), .b(x25), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n89_), .O(new_n103_));
  oai21  g052(.a(new_n96_), .b(new_n81_), .c(x26), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n103_), .c(x16), .O(new_n105_));
  aoi21  g054(.a(new_n56_), .b(x08), .c(new_n64_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n57_), .O(z07));
  inv1   g057(.a(x27), .O(new_n109_));
  aoi21  g058(.a(new_n102_), .b(new_n89_), .c(new_n109_), .O(new_n110_));
  nand2  g059(.a(new_n102_), .b(new_n109_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n94_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n110_), .c(x16), .O(new_n113_));
  nor2   g062(.a(x16), .b(x07), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n58_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n113_), .O(z08));
  nand4  g065(.a(new_n102_), .b(new_n87_), .c(new_n109_), .d(new_n86_), .O(new_n117_));
  inv1   g066(.a(x28), .O(new_n118_));
  nor2   g067(.a(x27), .b(x26), .O(new_n119_));
  nand4  g068(.a(new_n119_), .b(new_n118_), .c(new_n95_), .d(new_n86_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n81_), .O(new_n121_));
  aoi21  g070(.a(new_n117_), .b(x28), .c(new_n121_), .O(new_n122_));
  nor2   g071(.a(x16), .b(x06), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n58_), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n56_), .c(new_n124_), .O(z09));
  oai21  g074(.a(new_n120_), .b(new_n81_), .c(x29), .O(new_n126_));
  inv1   g075(.a(x29), .O(new_n127_));
  nand2  g076(.a(new_n121_), .b(new_n127_), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n126_), .c(x16), .O(new_n129_));
  aoi21  g078(.a(new_n56_), .b(x05), .c(new_n64_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n57_), .O(z10));
  nor2   g081(.a(x16), .b(x04), .O(new_n133_));
  aoi21  g082(.a(new_n128_), .b(x16), .c(new_n133_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n64_), .c(new_n57_), .O(z11));
  nand2  g084(.a(new_n56_), .b(x03), .O(new_n136_));
  inv1   g085(.a(x31), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n136_), .c(x30), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n57_), .O(z12));
  inv1   g089(.a(new_n58_), .O(new_n141_));
  nand2  g090(.a(x32), .b(x16), .O(new_n142_));
  inv1   g091(.a(x02), .O(new_n143_));
  nand2  g092(.a(new_n56_), .b(new_n143_), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n142_), .c(new_n141_), .O(z13));
  nand2  g094(.a(new_n56_), .b(x01), .O(new_n146_));
  inv1   g095(.a(x33), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(x16), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n146_), .c(x30), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n57_), .O(z14));
  nand2  g099(.a(x34), .b(x16), .O(new_n151_));
  inv1   g100(.a(x00), .O(new_n152_));
  nand2  g101(.a(new_n56_), .b(new_n152_), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n151_), .c(new_n141_), .O(z15));
endmodule


