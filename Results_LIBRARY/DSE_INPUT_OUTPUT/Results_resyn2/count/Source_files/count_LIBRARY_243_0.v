// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_;
  inv1   g00(.a(x18), .O(new_n52_));
  inv1   g01(.a(x17), .O(new_n53_));
  inv1   g02(.a(x19), .O(new_n54_));
  nand2  g03(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g04(.a(x19), .b(x17), .O(new_n56_));
  nand3  g05(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g06(.a(x16), .O(new_n58_));
  inv1   g07(.a(x26), .O(new_n59_));
  aoi21  g08(.a(new_n58_), .b(x15), .c(new_n59_), .O(new_n60_));
  nand2  g09(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand2  g10(.a(new_n61_), .b(new_n52_), .O(z00));
  inv1   g11(.a(x20), .O(new_n63_));
  nand3  g12(.a(new_n63_), .b(new_n54_), .c(new_n53_), .O(new_n64_));
  nand2  g13(.a(new_n55_), .b(x20), .O(new_n65_));
  nand3  g14(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  aoi21  g15(.a(new_n58_), .b(x14), .c(new_n59_), .O(new_n67_));
  nand2  g16(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g17(.a(new_n68_), .b(new_n52_), .O(z01));
  inv1   g18(.a(x21), .O(new_n70_));
  nor3   g19(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  nand2  g20(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g21(.a(new_n64_), .b(x21), .O(new_n73_));
  nand3  g22(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  aoi21  g23(.a(new_n58_), .b(x13), .c(new_n59_), .O(new_n75_));
  nand2  g24(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g25(.a(new_n76_), .b(new_n52_), .O(z02));
  oai21  g26(.a(new_n64_), .b(x21), .c(x22), .O(new_n78_));
  nor2   g27(.a(x22), .b(x21), .O(new_n79_));
  nand2  g28(.a(new_n79_), .b(new_n71_), .O(new_n80_));
  nand3  g29(.a(new_n80_), .b(new_n78_), .c(x16), .O(new_n81_));
  aoi21  g30(.a(new_n58_), .b(x12), .c(new_n59_), .O(new_n82_));
  nand2  g31(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g32(.a(new_n83_), .b(new_n52_), .O(z03));
  nor3   g33(.a(x23), .b(x22), .c(x21), .O(new_n85_));
  aoi22  g34(.a(new_n85_), .b(new_n71_), .c(new_n80_), .d(x23), .O(new_n86_));
  nor2   g35(.a(x16), .b(x11), .O(new_n87_));
  nand2  g36(.a(x26), .b(new_n52_), .O(new_n88_));
  nor2   g37(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g38(.a(new_n86_), .b(new_n58_), .c(new_n89_), .O(z04));
  inv1   g39(.a(x22), .O(new_n91_));
  inv1   g40(.a(x23), .O(new_n92_));
  nand3  g41(.a(new_n92_), .b(new_n91_), .c(new_n70_), .O(new_n93_));
  oai21  g42(.a(new_n93_), .b(new_n64_), .c(x24), .O(new_n94_));
  inv1   g43(.a(x24), .O(new_n95_));
  nand3  g44(.a(new_n85_), .b(new_n71_), .c(new_n95_), .O(new_n96_));
  nand3  g45(.a(new_n96_), .b(new_n94_), .c(x16), .O(new_n97_));
  aoi21  g46(.a(new_n58_), .b(x10), .c(new_n59_), .O(new_n98_));
  nand2  g47(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g48(.a(new_n99_), .b(new_n52_), .O(z05));
  xor2a  g49(.a(new_n96_), .b(x25), .O(new_n101_));
  nor2   g50(.a(x16), .b(x09), .O(new_n102_));
  nor2   g51(.a(new_n102_), .b(new_n88_), .O(new_n103_));
  oai21  g52(.a(new_n101_), .b(new_n58_), .c(new_n103_), .O(z06));
  oai21  g53(.a(new_n96_), .b(x25), .c(x16), .O(new_n105_));
  nor2   g54(.a(x16), .b(x08), .O(new_n106_));
  nor2   g55(.a(new_n106_), .b(new_n88_), .O(new_n107_));
  nand2  g56(.a(new_n107_), .b(new_n105_), .O(z07));
  inv1   g57(.a(new_n88_), .O(new_n109_));
  nand2  g58(.a(x27), .b(x16), .O(new_n110_));
  inv1   g59(.a(x07), .O(new_n111_));
  nand2  g60(.a(new_n58_), .b(new_n111_), .O(new_n112_));
  nand3  g61(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(z08));
  nand2  g62(.a(new_n58_), .b(x06), .O(new_n114_));
  inv1   g63(.a(x28), .O(new_n115_));
  nand2  g64(.a(new_n115_), .b(x16), .O(new_n116_));
  nand3  g65(.a(new_n116_), .b(new_n114_), .c(x26), .O(new_n117_));
  nand2  g66(.a(new_n117_), .b(new_n52_), .O(z09));
  nand2  g67(.a(x29), .b(x16), .O(new_n119_));
  inv1   g68(.a(x05), .O(new_n120_));
  nand2  g69(.a(new_n58_), .b(new_n120_), .O(new_n121_));
  nand3  g70(.a(new_n121_), .b(new_n119_), .c(new_n109_), .O(z10));
  nand2  g71(.a(x30), .b(x16), .O(new_n123_));
  inv1   g72(.a(x04), .O(new_n124_));
  nand2  g73(.a(new_n58_), .b(new_n124_), .O(new_n125_));
  nand3  g74(.a(new_n125_), .b(new_n123_), .c(new_n109_), .O(z11));
  nand2  g75(.a(new_n58_), .b(x03), .O(new_n127_));
  inv1   g76(.a(x31), .O(new_n128_));
  nand2  g77(.a(new_n128_), .b(x16), .O(new_n129_));
  nand3  g78(.a(new_n129_), .b(new_n127_), .c(x26), .O(new_n130_));
  nand2  g79(.a(new_n130_), .b(new_n52_), .O(z12));
  nand2  g80(.a(new_n58_), .b(x02), .O(new_n132_));
  inv1   g81(.a(x32), .O(new_n133_));
  nand2  g82(.a(new_n133_), .b(x16), .O(new_n134_));
  nand3  g83(.a(new_n134_), .b(new_n132_), .c(x26), .O(new_n135_));
  nand2  g84(.a(new_n135_), .b(new_n52_), .O(z13));
  nand2  g85(.a(new_n58_), .b(x01), .O(new_n137_));
  inv1   g86(.a(x33), .O(new_n138_));
  nand2  g87(.a(new_n138_), .b(x16), .O(new_n139_));
  nand3  g88(.a(new_n139_), .b(new_n137_), .c(x26), .O(new_n140_));
  nand2  g89(.a(new_n140_), .b(new_n52_), .O(z14));
  nand2  g90(.a(new_n58_), .b(x00), .O(new_n142_));
  inv1   g91(.a(x34), .O(new_n143_));
  nand2  g92(.a(new_n143_), .b(x16), .O(new_n144_));
  nand3  g93(.a(new_n144_), .b(new_n142_), .c(x26), .O(new_n145_));
  nand2  g94(.a(new_n145_), .b(new_n52_), .O(z15));
endmodule


