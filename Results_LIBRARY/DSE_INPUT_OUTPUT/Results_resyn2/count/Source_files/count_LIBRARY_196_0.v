// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x30), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x15), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n52_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  nor2   g013(.a(x16), .b(x14), .O(new_n65_));
  nand2  g014(.a(x30), .b(new_n52_), .O(new_n66_));
  nor2   g015(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n64_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(new_n63_), .b(new_n69_), .O(new_n70_));
  nand3  g019(.a(new_n53_), .b(new_n69_), .c(new_n61_), .O(new_n71_));
  inv1   g020(.a(new_n71_), .O(new_n72_));
  oai21  g021(.a(new_n72_), .b(new_n70_), .c(x16), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  nor2   g023(.a(new_n74_), .b(new_n66_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n73_), .O(z02));
  nor2   g025(.a(x22), .b(x21), .O(new_n77_));
  aoi22  g026(.a(new_n77_), .b(new_n63_), .c(new_n71_), .d(x22), .O(new_n78_));
  nor2   g027(.a(x16), .b(x12), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  oai21  g029(.a(new_n78_), .b(new_n56_), .c(new_n80_), .O(z03));
  inv1   g030(.a(x23), .O(new_n82_));
  aoi21  g031(.a(new_n77_), .b(new_n63_), .c(new_n82_), .O(new_n83_));
  nand4  g032(.a(new_n77_), .b(new_n53_), .c(new_n82_), .d(new_n61_), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  oai21  g034(.a(new_n85_), .b(new_n83_), .c(x16), .O(new_n86_));
  nor2   g035(.a(x16), .b(x11), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n66_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n86_), .O(z04));
  nand2  g038(.a(new_n84_), .b(x24), .O(new_n90_));
  nor2   g039(.a(x24), .b(x23), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n77_), .c(new_n53_), .d(new_n61_), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n90_), .c(x16), .O(new_n93_));
  aoi21  g042(.a(new_n56_), .b(x10), .c(new_n57_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n52_), .O(z05));
  inv1   g045(.a(x22), .O(new_n97_));
  nor2   g046(.a(x25), .b(x24), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n82_), .c(new_n97_), .O(new_n99_));
  inv1   g048(.a(new_n99_), .O(new_n100_));
  aoi22  g049(.a(new_n100_), .b(new_n72_), .c(new_n92_), .d(x25), .O(new_n101_));
  nor2   g050(.a(x16), .b(x09), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n66_), .O(new_n103_));
  oai21  g052(.a(new_n101_), .b(new_n56_), .c(new_n103_), .O(z06));
  oai21  g053(.a(new_n99_), .b(new_n71_), .c(x26), .O(new_n105_));
  nor3   g054(.a(x26), .b(x25), .c(x24), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n85_), .O(new_n107_));
  nand3  g056(.a(new_n107_), .b(new_n105_), .c(x16), .O(new_n108_));
  aoi21  g057(.a(new_n56_), .b(x08), .c(new_n57_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n52_), .O(z07));
  inv1   g060(.a(x27), .O(new_n112_));
  nor2   g061(.a(x26), .b(x25), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n92_), .O(new_n115_));
  aoi21  g064(.a(new_n107_), .b(x27), .c(new_n115_), .O(new_n116_));
  nor2   g065(.a(x16), .b(x07), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n66_), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n56_), .c(new_n118_), .O(z08));
  oai21  g068(.a(new_n114_), .b(new_n92_), .c(x28), .O(new_n120_));
  nor3   g069(.a(x28), .b(x27), .c(x23), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n106_), .c(new_n77_), .d(new_n63_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n120_), .c(x16), .O(new_n123_));
  aoi21  g072(.a(new_n56_), .b(x06), .c(new_n57_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n52_), .O(z09));
  nor3   g075(.a(x29), .b(x28), .c(x27), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n106_), .c(new_n85_), .O(new_n128_));
  nand2  g077(.a(new_n122_), .b(x29), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n128_), .c(x16), .O(new_n130_));
  aoi21  g079(.a(new_n56_), .b(x05), .c(new_n57_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n52_), .O(z10));
  nand2  g082(.a(new_n128_), .b(x16), .O(new_n134_));
  nor2   g083(.a(x16), .b(x04), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n66_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n134_), .O(z11));
  inv1   g086(.a(new_n66_), .O(new_n138_));
  nand2  g087(.a(x31), .b(x16), .O(new_n139_));
  inv1   g088(.a(x03), .O(new_n140_));
  nand2  g089(.a(new_n56_), .b(new_n140_), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n139_), .c(new_n138_), .O(z12));
  nand2  g091(.a(new_n56_), .b(x02), .O(new_n143_));
  inv1   g092(.a(x32), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(x16), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n143_), .c(x30), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n52_), .O(z13));
  nand2  g096(.a(x33), .b(x16), .O(new_n148_));
  inv1   g097(.a(x01), .O(new_n149_));
  nand2  g098(.a(new_n56_), .b(new_n149_), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n148_), .c(new_n138_), .O(z14));
  nand2  g100(.a(new_n56_), .b(x00), .O(new_n152_));
  inv1   g101(.a(x34), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(x30), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n52_), .O(z15));
endmodule


