// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x25), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g008(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nand2  g011(.a(new_n53_), .b(new_n62_), .O(new_n63_));
  oai21  g012(.a(x19), .b(x17), .c(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  aoi21  g014(.a(new_n56_), .b(x14), .c(x25), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n57_), .O(z01));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  aoi22  g018(.a(new_n69_), .b(new_n53_), .c(new_n63_), .d(x21), .O(new_n70_));
  nor2   g019(.a(x16), .b(x13), .O(new_n71_));
  nor2   g020(.a(new_n71_), .b(new_n59_), .O(new_n72_));
  oai21  g021(.a(new_n70_), .b(new_n56_), .c(new_n72_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  aoi21  g023(.a(new_n69_), .b(new_n53_), .c(new_n74_), .O(new_n75_));
  nand3  g024(.a(new_n69_), .b(new_n53_), .c(new_n74_), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(new_n75_), .c(x16), .O(new_n78_));
  nor2   g027(.a(x16), .b(x12), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n59_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n78_), .O(z03));
  nand2  g030(.a(new_n76_), .b(x23), .O(new_n82_));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n69_), .c(new_n53_), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  aoi21  g034(.a(new_n56_), .b(x11), .c(x25), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n57_), .O(z04));
  nand2  g037(.a(new_n84_), .b(x24), .O(new_n89_));
  nor2   g038(.a(x24), .b(x21), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n83_), .c(new_n53_), .d(new_n62_), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n89_), .c(x16), .O(new_n92_));
  aoi21  g041(.a(new_n56_), .b(x10), .c(x25), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n57_), .O(z05));
  nor2   g044(.a(x16), .b(x09), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n59_), .O(new_n97_));
  oai21  g046(.a(new_n91_), .b(new_n56_), .c(new_n97_), .O(z06));
  inv1   g047(.a(x26), .O(new_n99_));
  inv1   g048(.a(new_n91_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  inv1   g050(.a(x24), .O(new_n102_));
  nand4  g051(.a(new_n83_), .b(new_n69_), .c(new_n53_), .d(new_n102_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(x26), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  nor2   g054(.a(x16), .b(x08), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n59_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n105_), .O(z07));
  oai21  g057(.a(new_n103_), .b(x26), .c(x27), .O(new_n109_));
  inv1   g058(.a(new_n84_), .O(new_n110_));
  nor2   g059(.a(x27), .b(x26), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n110_), .c(new_n102_), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n109_), .c(x16), .O(new_n113_));
  aoi21  g062(.a(new_n56_), .b(x07), .c(x25), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n57_), .O(z08));
  inv1   g065(.a(new_n111_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n103_), .c(x28), .O(new_n118_));
  nor2   g067(.a(x28), .b(x27), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n100_), .c(new_n99_), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n118_), .c(x16), .O(new_n121_));
  aoi21  g070(.a(new_n56_), .b(x06), .c(x25), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n57_), .O(z09));
  nand2  g073(.a(new_n119_), .b(new_n99_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n91_), .c(x29), .O(new_n126_));
  nor3   g075(.a(x29), .b(x28), .c(x23), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n111_), .c(new_n77_), .d(new_n102_), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n126_), .c(x16), .O(new_n129_));
  aoi21  g078(.a(new_n56_), .b(x05), .c(x25), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n57_), .O(z10));
  inv1   g081(.a(x28), .O(new_n133_));
  nor2   g082(.a(x30), .b(x29), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n111_), .c(new_n133_), .d(new_n102_), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n84_), .O(new_n136_));
  aoi21  g085(.a(new_n128_), .b(x30), .c(new_n136_), .O(new_n137_));
  nor2   g086(.a(x16), .b(x04), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n59_), .O(new_n139_));
  oai21  g088(.a(new_n137_), .b(new_n56_), .c(new_n139_), .O(z11));
  oai21  g089(.a(new_n135_), .b(new_n84_), .c(x31), .O(new_n141_));
  inv1   g090(.a(x31), .O(new_n142_));
  inv1   g091(.a(new_n135_), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n110_), .c(new_n142_), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n141_), .c(x16), .O(new_n145_));
  aoi21  g094(.a(new_n56_), .b(x03), .c(x25), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n57_), .O(z12));
  nor2   g097(.a(x32), .b(x31), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n134_), .c(new_n119_), .d(new_n99_), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n91_), .O(new_n151_));
  aoi21  g100(.a(new_n144_), .b(x32), .c(new_n151_), .O(new_n152_));
  nor2   g101(.a(x16), .b(x02), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n59_), .O(new_n154_));
  oai21  g103(.a(new_n152_), .b(new_n56_), .c(new_n154_), .O(z13));
  nand2  g104(.a(new_n134_), .b(new_n133_), .O(new_n156_));
  inv1   g105(.a(x33), .O(new_n157_));
  nand3  g106(.a(new_n149_), .b(new_n111_), .c(new_n157_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n156_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n100_), .O(new_n160_));
  oai21  g109(.a(new_n150_), .b(new_n91_), .c(x33), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n160_), .c(x16), .O(new_n162_));
  aoi21  g111(.a(new_n56_), .b(x01), .c(x25), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n57_), .O(z14));
  inv1   g114(.a(x34), .O(new_n166_));
  aoi21  g115(.a(new_n159_), .b(new_n100_), .c(new_n166_), .O(new_n167_));
  nor2   g116(.a(x34), .b(x33), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n149_), .c(new_n134_), .d(new_n119_), .O(new_n169_));
  nor3   g118(.a(new_n169_), .b(new_n103_), .c(x26), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n167_), .c(x16), .O(new_n171_));
  nor2   g120(.a(x16), .b(x00), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n59_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z15));
endmodule


