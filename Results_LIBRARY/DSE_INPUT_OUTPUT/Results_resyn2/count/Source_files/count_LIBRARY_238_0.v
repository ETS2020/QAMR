// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  inv1   g006(.a(x18), .O(new_n58_));
  inv1   g007(.a(x28), .O(new_n59_));
  inv1   g008(.a(x31), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  inv1   g011(.a(new_n62_), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(new_n53_), .b(new_n65_), .O(new_n66_));
  nor3   g015(.a(x20), .b(x19), .c(x17), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n66_), .c(x16), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n62_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z01));
  inv1   g020(.a(new_n61_), .O(new_n72_));
  inv1   g021(.a(x21), .O(new_n73_));
  nor2   g022(.a(new_n67_), .b(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n67_), .b(new_n73_), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(new_n74_), .c(x16), .O(new_n77_));
  inv1   g026(.a(x13), .O(new_n78_));
  aoi21  g027(.a(new_n56_), .b(new_n78_), .c(x18), .O(new_n79_));
  aoi21  g028(.a(new_n79_), .b(new_n77_), .c(new_n72_), .O(z02));
  nand2  g029(.a(new_n75_), .b(x22), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n53_), .c(new_n65_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(x16), .O(new_n85_));
  inv1   g034(.a(x12), .O(new_n86_));
  aoi21  g035(.a(new_n56_), .b(new_n86_), .c(x18), .O(new_n87_));
  aoi21  g036(.a(new_n87_), .b(new_n85_), .c(new_n72_), .O(z03));
  nand2  g037(.a(new_n83_), .b(x23), .O(new_n89_));
  inv1   g038(.a(x23), .O(new_n90_));
  nand4  g039(.a(new_n82_), .b(new_n53_), .c(new_n90_), .d(new_n65_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  inv1   g042(.a(x11), .O(new_n94_));
  aoi21  g043(.a(new_n56_), .b(new_n94_), .c(x18), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n93_), .c(new_n72_), .O(z04));
  nand2  g045(.a(new_n91_), .b(x24), .O(new_n97_));
  inv1   g046(.a(x24), .O(new_n98_));
  inv1   g047(.a(new_n91_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n97_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x16), .O(new_n102_));
  inv1   g051(.a(x10), .O(new_n103_));
  aoi21  g052(.a(new_n56_), .b(new_n103_), .c(x18), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n102_), .c(new_n72_), .O(z05));
  nor2   g054(.a(x25), .b(x24), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n91_), .O(new_n108_));
  aoi21  g057(.a(new_n100_), .b(x25), .c(new_n108_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x09), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n62_), .O(new_n111_));
  oai21  g060(.a(new_n109_), .b(new_n56_), .c(new_n111_), .O(z06));
  inv1   g061(.a(x26), .O(new_n113_));
  nor3   g062(.a(x26), .b(x25), .c(x24), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n82_), .c(new_n67_), .d(new_n90_), .O(new_n115_));
  oai21  g064(.a(new_n108_), .b(new_n113_), .c(new_n115_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(x16), .O(new_n117_));
  inv1   g066(.a(x08), .O(new_n118_));
  aoi21  g067(.a(new_n56_), .b(new_n118_), .c(x18), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n117_), .c(new_n72_), .O(z07));
  nand2  g069(.a(new_n115_), .b(x27), .O(new_n121_));
  nor2   g070(.a(x27), .b(x23), .O(new_n122_));
  nand4  g071(.a(new_n122_), .b(new_n114_), .c(new_n82_), .d(new_n67_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(x16), .O(new_n125_));
  inv1   g074(.a(x07), .O(new_n126_));
  aoi21  g075(.a(new_n56_), .b(new_n126_), .c(x18), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n125_), .c(new_n72_), .O(z08));
  nor2   g077(.a(new_n123_), .b(new_n56_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n60_), .c(new_n59_), .O(new_n130_));
  nor2   g079(.a(new_n59_), .b(new_n56_), .O(new_n131_));
  oai21  g080(.a(x16), .b(x06), .c(new_n58_), .O(new_n132_));
  aoi21  g081(.a(new_n131_), .b(new_n123_), .c(new_n132_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n130_), .O(z09));
  oai21  g083(.a(x16), .b(x05), .c(new_n58_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n61_), .O(new_n136_));
  inv1   g085(.a(x29), .O(new_n137_));
  nand3  g086(.a(new_n122_), .b(new_n106_), .c(new_n113_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n83_), .c(x31), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n59_), .c(new_n137_), .O(new_n140_));
  inv1   g089(.a(x27), .O(new_n141_));
  nand4  g090(.a(x31), .b(new_n137_), .c(new_n59_), .d(new_n141_), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n115_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n140_), .c(x16), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n136_), .O(z10));
  oai21  g094(.a(x16), .b(x04), .c(new_n58_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n61_), .O(new_n147_));
  inv1   g096(.a(x30), .O(new_n148_));
  nand4  g097(.a(new_n122_), .b(new_n106_), .c(new_n137_), .d(new_n113_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n83_), .c(x31), .O(new_n150_));
  aoi21  g099(.a(new_n150_), .b(new_n59_), .c(new_n148_), .O(new_n151_));
  nand4  g100(.a(x31), .b(new_n148_), .c(new_n137_), .d(new_n59_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n123_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n151_), .c(x16), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n147_), .O(z11));
  inv1   g104(.a(x03), .O(new_n156_));
  aoi21  g105(.a(new_n56_), .b(new_n156_), .c(x18), .O(new_n157_));
  nand4  g106(.a(new_n148_), .b(new_n137_), .c(new_n59_), .d(new_n141_), .O(new_n158_));
  nor2   g107(.a(new_n60_), .b(new_n56_), .O(new_n159_));
  oai21  g108(.a(new_n158_), .b(new_n115_), .c(new_n159_), .O(new_n160_));
  oai21  g109(.a(new_n157_), .b(new_n72_), .c(new_n160_), .O(z12));
  inv1   g110(.a(x02), .O(new_n162_));
  nand2  g111(.a(new_n56_), .b(new_n162_), .O(new_n163_));
  nand2  g112(.a(x32), .b(x16), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n163_), .c(new_n63_), .O(z13));
  nand2  g114(.a(x33), .b(x16), .O(new_n166_));
  inv1   g115(.a(x01), .O(new_n167_));
  aoi21  g116(.a(new_n56_), .b(new_n167_), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n166_), .c(new_n72_), .O(z14));
  inv1   g118(.a(x00), .O(new_n170_));
  nand2  g119(.a(new_n56_), .b(new_n170_), .O(new_n171_));
  nand2  g120(.a(x34), .b(x16), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n171_), .c(new_n63_), .O(z15));
endmodule


