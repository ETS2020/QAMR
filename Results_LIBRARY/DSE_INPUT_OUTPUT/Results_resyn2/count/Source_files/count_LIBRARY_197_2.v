// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x13), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nor2   g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  oai21  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  nor2   g012(.a(x16), .b(x14), .O(new_n64_));
  inv1   g013(.a(x13), .O(new_n65_));
  nand2  g014(.a(new_n52_), .b(new_n65_), .O(new_n66_));
  nor2   g015(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n63_), .O(z01));
  inv1   g017(.a(new_n66_), .O(new_n69_));
  nand2  g018(.a(new_n53_), .b(new_n60_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x21), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n53_), .O(new_n73_));
  nand4  g022(.a(new_n73_), .b(new_n71_), .c(new_n69_), .d(x16), .O(z02));
  inv1   g023(.a(x22), .O(new_n75_));
  nand3  g024(.a(new_n72_), .b(new_n53_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n73_), .b(x22), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n76_), .c(x16), .O(new_n78_));
  aoi21  g027(.a(new_n56_), .b(x12), .c(x13), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n52_), .O(z03));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n72_), .c(new_n53_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi21  g033(.a(new_n76_), .b(x23), .c(new_n84_), .O(new_n85_));
  nor2   g034(.a(x16), .b(x11), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n66_), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n56_), .c(new_n87_), .O(z04));
  nor2   g037(.a(x24), .b(x21), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n82_), .c(new_n53_), .d(new_n60_), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  aoi21  g040(.a(new_n83_), .b(x24), .c(new_n91_), .O(new_n92_));
  nor2   g041(.a(x16), .b(x10), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n66_), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n56_), .c(new_n94_), .O(z05));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n82_), .c(new_n72_), .d(new_n53_), .O(new_n97_));
  inv1   g046(.a(new_n97_), .O(new_n98_));
  aoi21  g047(.a(new_n90_), .b(x25), .c(new_n98_), .O(new_n99_));
  nor2   g048(.a(x16), .b(x09), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n66_), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n56_), .c(new_n101_), .O(z06));
  inv1   g051(.a(x24), .O(new_n103_));
  inv1   g052(.a(x25), .O(new_n104_));
  inv1   g053(.a(x26), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n83_), .O(new_n107_));
  oai21  g056(.a(new_n98_), .b(new_n105_), .c(x16), .O(new_n108_));
  aoi21  g057(.a(new_n56_), .b(x08), .c(x13), .O(new_n109_));
  oai21  g058(.a(new_n108_), .b(new_n107_), .c(new_n109_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n52_), .O(z07));
  inv1   g060(.a(x27), .O(new_n112_));
  nor3   g061(.a(x27), .b(x26), .c(x25), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n89_), .c(new_n82_), .d(new_n62_), .O(new_n114_));
  oai21  g063(.a(new_n107_), .b(new_n112_), .c(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  nor2   g065(.a(x16), .b(x07), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n66_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n116_), .O(z08));
  nor2   g068(.a(x28), .b(x27), .O(new_n120_));
  aoi22  g069(.a(new_n120_), .b(new_n107_), .c(new_n114_), .d(x28), .O(new_n121_));
  nor2   g070(.a(x16), .b(x06), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n66_), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n56_), .c(new_n123_), .O(z09));
  nand2  g073(.a(new_n120_), .b(new_n107_), .O(new_n125_));
  nor2   g074(.a(x26), .b(x25), .O(new_n126_));
  nor2   g075(.a(x29), .b(x28), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n126_), .c(new_n112_), .d(new_n103_), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n83_), .O(new_n129_));
  aoi21  g078(.a(new_n125_), .b(x29), .c(new_n129_), .O(new_n130_));
  nor2   g079(.a(x16), .b(x05), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n66_), .O(new_n132_));
  oai21  g081(.a(new_n130_), .b(new_n56_), .c(new_n132_), .O(z10));
  inv1   g082(.a(x30), .O(new_n134_));
  inv1   g083(.a(x28), .O(new_n135_));
  inv1   g084(.a(x29), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n135_), .c(new_n112_), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n106_), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n84_), .c(new_n134_), .O(new_n139_));
  oai21  g088(.a(new_n128_), .b(new_n83_), .c(x30), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n139_), .c(x16), .O(new_n141_));
  aoi21  g090(.a(new_n56_), .b(x04), .c(x13), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n52_), .O(z11));
  nor2   g093(.a(x27), .b(x26), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n104_), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n90_), .O(new_n147_));
  nor2   g096(.a(x31), .b(x30), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n127_), .O(new_n149_));
  inv1   g098(.a(new_n149_), .O(new_n150_));
  aoi22  g099(.a(new_n150_), .b(new_n147_), .c(new_n139_), .d(x31), .O(new_n151_));
  nor2   g100(.a(x16), .b(x03), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n66_), .O(new_n153_));
  oai21  g102(.a(new_n151_), .b(new_n56_), .c(new_n153_), .O(z12));
  nand3  g103(.a(new_n150_), .b(new_n113_), .c(new_n91_), .O(new_n155_));
  nor3   g104(.a(x29), .b(x28), .c(x27), .O(new_n156_));
  nor3   g105(.a(x32), .b(x31), .c(x30), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n156_), .c(new_n126_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n90_), .O(new_n159_));
  aoi21  g108(.a(new_n155_), .b(x32), .c(new_n159_), .O(new_n160_));
  nor2   g109(.a(x16), .b(x02), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n66_), .O(new_n162_));
  oai21  g111(.a(new_n160_), .b(new_n56_), .c(new_n162_), .O(z13));
  inv1   g112(.a(new_n126_), .O(new_n164_));
  inv1   g113(.a(x32), .O(new_n165_));
  nand2  g114(.a(new_n148_), .b(new_n165_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n164_), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n156_), .c(new_n91_), .O(new_n168_));
  nor2   g117(.a(x33), .b(x32), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n148_), .c(new_n127_), .d(new_n145_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n97_), .c(x16), .O(new_n171_));
  aoi21  g120(.a(new_n168_), .b(x33), .c(new_n171_), .O(new_n172_));
  inv1   g121(.a(x01), .O(new_n173_));
  oai21  g122(.a(x16), .b(new_n173_), .c(new_n65_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n172_), .c(new_n52_), .O(z14));
  nor3   g124(.a(new_n170_), .b(new_n97_), .c(x34), .O(new_n176_));
  oai21  g125(.a(new_n170_), .b(new_n97_), .c(x34), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(x16), .O(new_n178_));
  aoi21  g127(.a(new_n56_), .b(x00), .c(x13), .O(new_n179_));
  oai21  g128(.a(new_n178_), .b(new_n176_), .c(new_n179_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n52_), .O(z15));
endmodule


