// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x25), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g009(.a(x19), .b(x17), .c(x20), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(x16), .O(new_n62_));
  aoi21  g011(.a(new_n56_), .b(x14), .c(x25), .O(new_n63_));
  oai21  g012(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n52_), .O(z01));
  inv1   g014(.a(x17), .O(new_n66_));
  inv1   g015(.a(x19), .O(new_n67_));
  inv1   g016(.a(x20), .O(new_n68_));
  inv1   g017(.a(x21), .O(new_n69_));
  nand4  g018(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  inv1   g019(.a(new_n60_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x21), .O(new_n72_));
  nand3  g021(.a(new_n72_), .b(new_n70_), .c(x16), .O(new_n73_));
  aoi21  g022(.a(new_n56_), .b(x13), .c(x25), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n52_), .O(z02));
  inv1   g025(.a(x22), .O(new_n77_));
  nand3  g026(.a(new_n60_), .b(new_n77_), .c(new_n69_), .O(new_n78_));
  nand2  g027(.a(new_n70_), .b(x22), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n78_), .c(x16), .O(new_n80_));
  aoi21  g029(.a(new_n56_), .b(x12), .c(x25), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n52_), .O(z03));
  nor3   g032(.a(x23), .b(x22), .c(x21), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n60_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(x16), .O(new_n86_));
  aoi21  g035(.a(new_n78_), .b(x23), .c(new_n86_), .O(new_n87_));
  inv1   g036(.a(x11), .O(new_n88_));
  inv1   g037(.a(x25), .O(new_n89_));
  oai21  g038(.a(x16), .b(new_n88_), .c(new_n89_), .O(new_n90_));
  oai21  g039(.a(new_n90_), .b(new_n87_), .c(new_n52_), .O(z04));
  inv1   g040(.a(x24), .O(new_n92_));
  aoi21  g041(.a(new_n84_), .b(new_n60_), .c(new_n92_), .O(new_n93_));
  inv1   g042(.a(x23), .O(new_n94_));
  nand3  g043(.a(new_n92_), .b(new_n94_), .c(new_n77_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n70_), .O(new_n96_));
  oai21  g045(.a(new_n96_), .b(new_n93_), .c(x16), .O(new_n97_));
  nand2  g046(.a(new_n89_), .b(new_n52_), .O(new_n98_));
  nor2   g047(.a(x16), .b(x10), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n97_), .O(z05));
  aoi21  g050(.a(new_n56_), .b(x09), .c(x25), .O(new_n102_));
  oai21  g051(.a(new_n96_), .b(new_n56_), .c(new_n102_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n52_), .O(z06));
  inv1   g053(.a(x26), .O(new_n105_));
  nand4  g054(.a(new_n84_), .b(new_n60_), .c(new_n105_), .d(new_n92_), .O(new_n106_));
  oai21  g055(.a(new_n96_), .b(new_n105_), .c(new_n106_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(x16), .O(new_n108_));
  nor2   g057(.a(x16), .b(x08), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n98_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n108_), .O(z07));
  nor3   g060(.a(x27), .b(x26), .c(x24), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n84_), .c(new_n60_), .O(new_n113_));
  inv1   g062(.a(new_n113_), .O(new_n114_));
  aoi21  g063(.a(new_n106_), .b(x27), .c(new_n114_), .O(new_n115_));
  nor2   g064(.a(x16), .b(x07), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n98_), .O(new_n117_));
  oai21  g066(.a(new_n115_), .b(new_n56_), .c(new_n117_), .O(z08));
  nor3   g067(.a(x28), .b(x27), .c(x26), .O(new_n119_));
  aoi22  g068(.a(new_n119_), .b(new_n96_), .c(new_n113_), .d(x28), .O(new_n120_));
  nor2   g069(.a(x16), .b(x06), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n98_), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n56_), .c(new_n122_), .O(z09));
  nand2  g072(.a(new_n119_), .b(new_n96_), .O(new_n124_));
  nor3   g073(.a(x29), .b(x28), .c(x23), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n112_), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n78_), .O(new_n127_));
  aoi21  g076(.a(new_n124_), .b(x29), .c(new_n127_), .O(new_n128_));
  nor2   g077(.a(x16), .b(x05), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n98_), .O(new_n130_));
  oai21  g079(.a(new_n128_), .b(new_n56_), .c(new_n130_), .O(z10));
  oai21  g080(.a(new_n126_), .b(new_n78_), .c(x30), .O(new_n132_));
  nor3   g081(.a(x30), .b(x29), .c(x28), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n112_), .c(new_n84_), .d(new_n60_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(x16), .O(new_n136_));
  nor2   g085(.a(x16), .b(x04), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n98_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n136_), .O(z11));
  inv1   g088(.a(x28), .O(new_n140_));
  inv1   g089(.a(x31), .O(new_n141_));
  nor2   g090(.a(x30), .b(x29), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n141_), .c(new_n140_), .O(new_n143_));
  inv1   g092(.a(new_n143_), .O(new_n144_));
  aoi22  g093(.a(new_n144_), .b(new_n114_), .c(new_n134_), .d(x31), .O(new_n145_));
  nor2   g094(.a(x16), .b(x03), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n98_), .O(new_n147_));
  oai21  g096(.a(new_n145_), .b(new_n56_), .c(new_n147_), .O(z12));
  oai21  g097(.a(new_n143_), .b(new_n113_), .c(x32), .O(new_n149_));
  inv1   g098(.a(new_n70_), .O(new_n150_));
  inv1   g099(.a(new_n95_), .O(new_n151_));
  inv1   g100(.a(x29), .O(new_n152_));
  inv1   g101(.a(x30), .O(new_n153_));
  inv1   g102(.a(x32), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n141_), .c(new_n153_), .d(new_n152_), .O(new_n155_));
  inv1   g104(.a(new_n155_), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n119_), .c(new_n151_), .d(new_n150_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n149_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x16), .O(new_n159_));
  nor2   g108(.a(x16), .b(x02), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n98_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n159_), .O(z13));
  inv1   g111(.a(x27), .O(new_n163_));
  inv1   g112(.a(x33), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n154_), .c(new_n163_), .d(new_n105_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n143_), .O(new_n166_));
  aoi22  g115(.a(new_n166_), .b(new_n96_), .c(new_n157_), .d(x33), .O(new_n167_));
  nor2   g116(.a(x16), .b(x01), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n98_), .O(new_n169_));
  oai21  g118(.a(new_n167_), .b(new_n56_), .c(new_n169_), .O(z14));
  inv1   g119(.a(x34), .O(new_n171_));
  aoi21  g120(.a(new_n166_), .b(new_n96_), .c(new_n171_), .O(new_n172_));
  nor2   g121(.a(x28), .b(x27), .O(new_n173_));
  nor2   g122(.a(x32), .b(x31), .O(new_n174_));
  nor2   g123(.a(x34), .b(x33), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n174_), .c(new_n142_), .d(new_n173_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n106_), .c(x16), .O(new_n177_));
  aoi21  g126(.a(new_n56_), .b(x00), .c(x25), .O(new_n178_));
  oai21  g127(.a(new_n177_), .b(new_n172_), .c(new_n178_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n52_), .O(z15));
endmodule


