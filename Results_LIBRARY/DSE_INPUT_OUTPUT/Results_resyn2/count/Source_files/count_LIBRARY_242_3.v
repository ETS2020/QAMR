// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  xor2a  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g008(.a(x14), .O(new_n60_));
  aoi21  g009(.a(new_n52_), .b(new_n60_), .c(x18), .O(new_n61_));
  oai21  g010(.a(new_n59_), .b(new_n52_), .c(new_n61_), .O(z01));
  inv1   g011(.a(x11), .O(new_n63_));
  nand2  g012(.a(x18), .b(new_n63_), .O(new_n64_));
  inv1   g013(.a(x21), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n57_), .c(new_n65_), .O(new_n66_));
  nor2   g015(.a(x21), .b(x20), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n58_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(x16), .O(new_n69_));
  aoi21  g018(.a(new_n52_), .b(x13), .c(x18), .O(new_n70_));
  oai21  g019(.a(new_n69_), .b(new_n66_), .c(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n64_), .O(z02));
  inv1   g021(.a(x22), .O(new_n73_));
  nand3  g022(.a(new_n67_), .b(new_n58_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n68_), .b(x22), .O(new_n75_));
  nand3  g024(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  aoi21  g025(.a(new_n52_), .b(x12), .c(x18), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n64_), .O(z03));
  inv1   g028(.a(x18), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(x16), .O(new_n81_));
  nand2  g030(.a(new_n74_), .b(x23), .O(new_n82_));
  inv1   g031(.a(new_n68_), .O(new_n83_));
  nor2   g032(.a(x23), .b(x22), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n83_), .c(new_n81_), .O(new_n85_));
  aoi22  g034(.a(new_n85_), .b(new_n82_), .c(new_n81_), .d(x11), .O(z04));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n67_), .c(new_n58_), .d(new_n73_), .O(new_n88_));
  nand2  g037(.a(new_n84_), .b(new_n83_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(x24), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n88_), .c(x16), .O(new_n91_));
  aoi21  g040(.a(new_n52_), .b(x10), .c(x18), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n64_), .O(z05));
  nand2  g043(.a(new_n88_), .b(x25), .O(new_n95_));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n84_), .c(new_n67_), .d(new_n58_), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n95_), .c(x16), .O(new_n98_));
  aoi21  g047(.a(new_n52_), .b(x09), .c(x18), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n64_), .O(z06));
  inv1   g050(.a(x25), .O(new_n102_));
  inv1   g051(.a(x26), .O(new_n103_));
  nand3  g052(.a(new_n87_), .b(new_n103_), .c(new_n102_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n74_), .O(new_n105_));
  aoi21  g054(.a(new_n97_), .b(x26), .c(new_n105_), .O(new_n106_));
  inv1   g055(.a(x08), .O(new_n107_));
  aoi21  g056(.a(new_n52_), .b(new_n107_), .c(x18), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n52_), .c(new_n108_), .O(z07));
  inv1   g058(.a(x27), .O(new_n110_));
  xor2a  g059(.a(new_n105_), .b(new_n110_), .O(new_n111_));
  inv1   g060(.a(x07), .O(new_n112_));
  aoi21  g061(.a(new_n52_), .b(new_n112_), .c(x18), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n52_), .c(new_n113_), .O(z08));
  inv1   g063(.a(x28), .O(new_n115_));
  aoi21  g064(.a(new_n105_), .b(new_n110_), .c(new_n115_), .O(new_n116_));
  inv1   g065(.a(new_n74_), .O(new_n117_));
  inv1   g066(.a(new_n104_), .O(new_n118_));
  nor2   g067(.a(x28), .b(x27), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  inv1   g069(.a(new_n120_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n116_), .c(x16), .O(new_n122_));
  inv1   g071(.a(x06), .O(new_n123_));
  aoi21  g072(.a(new_n52_), .b(new_n123_), .c(x18), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n122_), .O(z09));
  inv1   g074(.a(x29), .O(new_n126_));
  nand3  g075(.a(new_n119_), .b(new_n105_), .c(new_n126_), .O(new_n127_));
  nand2  g076(.a(new_n120_), .b(x29), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(x16), .O(new_n129_));
  aoi21  g078(.a(new_n52_), .b(x05), .c(x18), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n64_), .O(z10));
  nor2   g081(.a(x30), .b(x29), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n119_), .O(new_n134_));
  inv1   g083(.a(new_n134_), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n118_), .c(new_n117_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(x16), .O(new_n137_));
  aoi21  g086(.a(new_n127_), .b(x30), .c(new_n137_), .O(new_n138_));
  inv1   g087(.a(x04), .O(new_n139_));
  oai21  g088(.a(x16), .b(new_n139_), .c(new_n80_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n138_), .c(new_n64_), .O(z11));
  nor2   g090(.a(x31), .b(x28), .O(new_n142_));
  nor2   g091(.a(x27), .b(x26), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n142_), .c(new_n133_), .d(new_n102_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n88_), .c(x16), .O(new_n145_));
  aoi21  g094(.a(new_n136_), .b(x31), .c(new_n145_), .O(new_n146_));
  inv1   g095(.a(x03), .O(new_n147_));
  oai21  g096(.a(x16), .b(new_n147_), .c(new_n80_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n146_), .c(new_n64_), .O(z12));
  oai21  g098(.a(new_n144_), .b(new_n88_), .c(x32), .O(new_n150_));
  nor2   g099(.a(x32), .b(x31), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n135_), .c(new_n105_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  inv1   g103(.a(x02), .O(new_n155_));
  aoi21  g104(.a(new_n52_), .b(new_n155_), .c(x18), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n154_), .O(z13));
  nor2   g106(.a(x33), .b(x32), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n143_), .c(new_n142_), .d(new_n133_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n97_), .O(new_n160_));
  aoi21  g109(.a(new_n152_), .b(x33), .c(new_n160_), .O(new_n161_));
  inv1   g110(.a(x01), .O(new_n162_));
  aoi21  g111(.a(new_n52_), .b(new_n162_), .c(x18), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n52_), .c(new_n163_), .O(z14));
  oai21  g113(.a(new_n159_), .b(new_n97_), .c(x34), .O(new_n165_));
  inv1   g114(.a(x34), .O(new_n166_));
  nand2  g115(.a(new_n160_), .b(new_n166_), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n165_), .c(x16), .O(new_n168_));
  aoi21  g117(.a(new_n52_), .b(x00), .c(x18), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n64_), .O(z15));
endmodule


