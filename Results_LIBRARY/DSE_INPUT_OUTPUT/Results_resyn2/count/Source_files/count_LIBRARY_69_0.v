// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x12), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(x15), .c(new_n57_), .O(new_n59_));
  oai21  g008(.a(new_n56_), .b(new_n53_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n52_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n55_), .c(new_n54_), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  oai21  g013(.a(new_n53_), .b(new_n62_), .c(x16), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(x14), .c(new_n57_), .O(new_n66_));
  oai21  g015(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n52_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n53_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  aoi21  g022(.a(new_n58_), .b(x13), .c(new_n57_), .O(new_n74_));
  oai21  g023(.a(new_n73_), .b(new_n70_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n52_), .O(z02));
  xor2a  g025(.a(new_n72_), .b(x22), .O(new_n77_));
  nand2  g026(.a(x16), .b(x12), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n77_), .c(new_n52_), .O(z03));
  inv1   g028(.a(x22), .O(new_n80_));
  nand3  g029(.a(new_n71_), .b(new_n53_), .c(new_n80_), .O(new_n81_));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n71_), .c(new_n53_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi21  g033(.a(new_n81_), .b(x23), .c(new_n84_), .O(new_n85_));
  nand2  g034(.a(new_n52_), .b(x12), .O(new_n86_));
  nor2   g035(.a(x16), .b(x11), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g037(.a(new_n85_), .b(new_n58_), .c(new_n88_), .O(z04));
  inv1   g038(.a(x23), .O(new_n90_));
  inv1   g039(.a(x24), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n90_), .c(new_n80_), .d(new_n69_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n63_), .O(new_n93_));
  aoi21  g042(.a(new_n83_), .b(x24), .c(new_n93_), .O(new_n94_));
  nor2   g043(.a(x16), .b(x10), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n86_), .O(new_n96_));
  oai21  g045(.a(new_n94_), .b(new_n58_), .c(new_n96_), .O(z05));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nor2   g047(.a(x22), .b(x21), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n98_), .c(new_n53_), .d(new_n62_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(x25), .O(new_n101_));
  nor2   g050(.a(x25), .b(x24), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n82_), .c(new_n71_), .d(new_n53_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n101_), .c(x16), .O(new_n104_));
  aoi21  g053(.a(new_n58_), .b(x09), .c(new_n57_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n52_), .O(z06));
  inv1   g056(.a(x25), .O(new_n108_));
  inv1   g057(.a(x26), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n108_), .c(new_n91_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n83_), .O(new_n111_));
  aoi21  g060(.a(new_n103_), .b(x26), .c(new_n111_), .O(new_n112_));
  nor2   g061(.a(x16), .b(x08), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n86_), .O(new_n114_));
  oai21  g063(.a(new_n112_), .b(new_n58_), .c(new_n114_), .O(z07));
  inv1   g064(.a(x27), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n109_), .c(new_n108_), .O(new_n117_));
  oai22  g066(.a(new_n117_), .b(new_n100_), .c(new_n111_), .d(new_n116_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(x16), .O(new_n119_));
  nor2   g068(.a(x16), .b(x07), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n86_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n119_), .O(z08));
  nor2   g071(.a(x27), .b(x26), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n93_), .c(new_n108_), .O(new_n124_));
  inv1   g073(.a(x28), .O(new_n125_));
  nor2   g074(.a(x26), .b(x25), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n98_), .c(new_n125_), .d(new_n116_), .O(new_n127_));
  oai21  g076(.a(new_n127_), .b(new_n81_), .c(x16), .O(new_n128_));
  aoi21  g077(.a(new_n124_), .b(x28), .c(new_n128_), .O(new_n129_));
  inv1   g078(.a(x06), .O(new_n130_));
  oai21  g079(.a(x16), .b(new_n130_), .c(x12), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n129_), .c(new_n52_), .O(z09));
  oai21  g081(.a(new_n127_), .b(new_n81_), .c(x29), .O(new_n133_));
  nor2   g082(.a(x29), .b(x28), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n126_), .c(new_n116_), .d(new_n91_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n83_), .c(new_n133_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(x16), .O(new_n137_));
  nor2   g086(.a(x16), .b(x05), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n86_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n137_), .O(z10));
  oai21  g089(.a(new_n135_), .b(new_n83_), .c(x30), .O(new_n141_));
  inv1   g090(.a(x30), .O(new_n142_));
  inv1   g091(.a(x29), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n125_), .c(new_n116_), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n110_), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n84_), .c(new_n142_), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n141_), .c(x16), .O(new_n147_));
  aoi21  g096(.a(new_n58_), .b(x04), .c(new_n57_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n52_), .O(z11));
  nor2   g099(.a(x31), .b(x30), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n134_), .c(new_n123_), .d(new_n108_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n100_), .O(new_n153_));
  aoi21  g102(.a(new_n146_), .b(x31), .c(new_n153_), .O(new_n154_));
  nor2   g103(.a(x16), .b(x03), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n86_), .O(new_n156_));
  oai21  g105(.a(new_n154_), .b(new_n58_), .c(new_n156_), .O(z12));
  inv1   g106(.a(x32), .O(new_n158_));
  inv1   g107(.a(x31), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n142_), .c(new_n143_), .d(new_n125_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n117_), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n93_), .c(new_n158_), .O(new_n162_));
  oai21  g111(.a(new_n152_), .b(new_n100_), .c(x32), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n162_), .c(x16), .O(new_n164_));
  aoi21  g113(.a(new_n58_), .b(x02), .c(new_n57_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n52_), .O(z13));
  nor2   g116(.a(x33), .b(x32), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n151_), .c(new_n134_), .d(new_n123_), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n103_), .O(new_n170_));
  aoi21  g119(.a(new_n162_), .b(x33), .c(new_n170_), .O(new_n171_));
  nor2   g120(.a(x16), .b(x01), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n86_), .O(new_n173_));
  oai21  g122(.a(new_n171_), .b(new_n58_), .c(new_n173_), .O(z14));
  nor3   g123(.a(new_n169_), .b(new_n103_), .c(x34), .O(new_n175_));
  oai21  g124(.a(new_n169_), .b(new_n103_), .c(x34), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  aoi21  g126(.a(new_n58_), .b(x00), .c(new_n57_), .O(new_n178_));
  oai21  g127(.a(new_n177_), .b(new_n175_), .c(new_n178_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n52_), .O(z15));
endmodule


