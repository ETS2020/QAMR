// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  oai21  g012(.a(new_n55_), .b(new_n63_), .c(x16), .O(new_n64_));
  nand2  g013(.a(new_n58_), .b(x14), .O(new_n65_));
  oai21  g014(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n61_), .O(new_n68_));
  aoi21  g017(.a(new_n66_), .b(new_n61_), .c(new_n68_), .O(z01));
  nor2   g018(.a(new_n62_), .b(new_n67_), .O(new_n70_));
  nand4  g019(.a(new_n67_), .b(new_n63_), .c(new_n53_), .d(new_n52_), .O(new_n71_));
  inv1   g020(.a(new_n71_), .O(new_n72_));
  oai21  g021(.a(new_n72_), .b(new_n70_), .c(x16), .O(new_n73_));
  inv1   g022(.a(x13), .O(new_n74_));
  aoi21  g023(.a(new_n58_), .b(new_n74_), .c(x18), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n73_), .O(z02));
  inv1   g025(.a(x22), .O(new_n77_));
  nor2   g026(.a(new_n72_), .b(new_n77_), .O(new_n78_));
  nor2   g027(.a(x21), .b(x20), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n55_), .c(new_n77_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  oai21  g030(.a(new_n81_), .b(new_n78_), .c(x16), .O(new_n82_));
  inv1   g031(.a(x12), .O(new_n83_));
  aoi21  g032(.a(new_n58_), .b(new_n83_), .c(x18), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n82_), .O(z03));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n79_), .c(new_n55_), .O(new_n87_));
  inv1   g036(.a(new_n87_), .O(new_n88_));
  aoi21  g037(.a(new_n80_), .b(x23), .c(new_n88_), .O(new_n89_));
  inv1   g038(.a(x11), .O(new_n90_));
  aoi21  g039(.a(new_n58_), .b(new_n90_), .c(x18), .O(new_n91_));
  oai21  g040(.a(new_n89_), .b(new_n58_), .c(new_n91_), .O(z04));
  inv1   g041(.a(x24), .O(new_n93_));
  nand4  g042(.a(new_n86_), .b(new_n62_), .c(new_n93_), .d(x16), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n61_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n67_), .O(new_n96_));
  nand3  g045(.a(new_n87_), .b(x24), .c(x16), .O(new_n97_));
  oai21  g046(.a(x16), .b(x10), .c(new_n97_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n61_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n96_), .O(z05));
  nor2   g049(.a(x24), .b(x23), .O(new_n101_));
  nor2   g050(.a(x22), .b(x21), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n101_), .c(new_n55_), .d(new_n63_), .O(new_n103_));
  inv1   g052(.a(x23), .O(new_n104_));
  inv1   g053(.a(x25), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n93_), .c(new_n104_), .d(new_n77_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  aoi22  g056(.a(new_n107_), .b(new_n72_), .c(new_n103_), .d(x25), .O(new_n108_));
  inv1   g057(.a(x09), .O(new_n109_));
  aoi21  g058(.a(new_n58_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n58_), .c(new_n110_), .O(z06));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n86_), .c(new_n79_), .d(new_n55_), .O(new_n113_));
  inv1   g062(.a(x26), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n105_), .c(new_n93_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n87_), .O(new_n116_));
  aoi21  g065(.a(new_n113_), .b(x26), .c(new_n116_), .O(new_n117_));
  inv1   g066(.a(x08), .O(new_n118_));
  aoi21  g067(.a(new_n58_), .b(new_n118_), .c(x18), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n58_), .c(new_n119_), .O(z07));
  nor3   g069(.a(x27), .b(x26), .c(x25), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n94_), .c(new_n61_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n67_), .O(new_n124_));
  nand2  g073(.a(x27), .b(x16), .O(new_n125_));
  inv1   g074(.a(x07), .O(new_n126_));
  nand2  g075(.a(new_n58_), .b(new_n126_), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n116_), .c(new_n127_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n61_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n124_), .O(z08));
  nand4  g079(.a(new_n93_), .b(new_n104_), .c(new_n77_), .d(new_n67_), .O(new_n131_));
  inv1   g080(.a(new_n131_), .O(new_n132_));
  nand3  g081(.a(new_n121_), .b(new_n132_), .c(new_n62_), .O(new_n133_));
  nor2   g082(.a(x26), .b(x25), .O(new_n134_));
  nor2   g083(.a(x28), .b(x27), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n134_), .c(new_n101_), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n80_), .O(new_n137_));
  aoi21  g086(.a(new_n133_), .b(x28), .c(new_n137_), .O(new_n138_));
  inv1   g087(.a(x06), .O(new_n139_));
  aoi21  g088(.a(new_n58_), .b(new_n139_), .c(x18), .O(new_n140_));
  oai21  g089(.a(new_n138_), .b(new_n58_), .c(new_n140_), .O(z09));
  oai21  g090(.a(new_n136_), .b(new_n80_), .c(x29), .O(new_n142_));
  inv1   g091(.a(x27), .O(new_n143_));
  inv1   g092(.a(x28), .O(new_n144_));
  inv1   g093(.a(x29), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n144_), .c(new_n143_), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n115_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n88_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n142_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  inv1   g099(.a(x05), .O(new_n151_));
  aoi21  g100(.a(new_n58_), .b(new_n151_), .c(x18), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(z10));
  nand3  g102(.a(new_n86_), .b(new_n62_), .c(x16), .O(new_n154_));
  nor2   g103(.a(x30), .b(x29), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n135_), .c(new_n134_), .d(new_n93_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n154_), .c(new_n61_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n67_), .O(new_n158_));
  nand4  g107(.a(new_n135_), .b(new_n134_), .c(new_n145_), .d(new_n93_), .O(new_n159_));
  inv1   g108(.a(x30), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n58_), .O(new_n161_));
  oai21  g110(.a(new_n159_), .b(new_n87_), .c(new_n161_), .O(new_n162_));
  inv1   g111(.a(x04), .O(new_n163_));
  nand2  g112(.a(new_n58_), .b(new_n163_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n61_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n158_), .O(z11));
  oai21  g116(.a(new_n156_), .b(new_n87_), .c(x31), .O(new_n168_));
  inv1   g117(.a(x31), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n160_), .c(new_n145_), .d(new_n144_), .O(new_n170_));
  inv1   g119(.a(new_n170_), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n121_), .c(new_n132_), .d(new_n62_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n168_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(x16), .O(new_n174_));
  inv1   g123(.a(x03), .O(new_n175_));
  aoi21  g124(.a(new_n58_), .b(new_n175_), .c(x18), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n174_), .O(z12));
  nor2   g126(.a(x32), .b(x31), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n155_), .c(new_n135_), .d(new_n134_), .O(new_n179_));
  oai21  g128(.a(new_n179_), .b(new_n94_), .c(new_n61_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n67_), .O(new_n181_));
  nor2   g130(.a(x27), .b(x26), .O(new_n182_));
  nor2   g131(.a(x31), .b(x28), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n155_), .c(new_n182_), .d(new_n105_), .O(new_n184_));
  inv1   g133(.a(x32), .O(new_n185_));
  nor2   g134(.a(new_n185_), .b(new_n58_), .O(new_n186_));
  oai21  g135(.a(new_n184_), .b(new_n103_), .c(new_n186_), .O(new_n187_));
  inv1   g136(.a(x02), .O(new_n188_));
  nand2  g137(.a(new_n58_), .b(new_n188_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n187_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n61_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n181_), .O(z13));
  oai21  g141(.a(new_n179_), .b(new_n103_), .c(x33), .O(new_n193_));
  inv1   g142(.a(x33), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n185_), .c(new_n143_), .d(new_n114_), .O(new_n195_));
  inv1   g144(.a(new_n195_), .O(new_n196_));
  nand4  g145(.a(new_n196_), .b(new_n171_), .c(new_n107_), .d(new_n72_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n193_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(x16), .O(new_n199_));
  inv1   g148(.a(x01), .O(new_n200_));
  aoi21  g149(.a(new_n58_), .b(new_n200_), .c(x18), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n199_), .O(z14));
  inv1   g151(.a(x34), .O(new_n203_));
  nor2   g152(.a(new_n197_), .b(new_n203_), .O(new_n204_));
  nor2   g153(.a(x33), .b(x32), .O(new_n205_));
  nand4  g154(.a(new_n205_), .b(new_n183_), .c(new_n155_), .d(new_n182_), .O(new_n206_));
  oai21  g155(.a(new_n206_), .b(new_n113_), .c(new_n203_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(x16), .O(new_n208_));
  inv1   g157(.a(x00), .O(new_n209_));
  aoi21  g158(.a(new_n58_), .b(new_n209_), .c(x18), .O(new_n210_));
  oai21  g159(.a(new_n208_), .b(new_n204_), .c(new_n210_), .O(z15));
endmodule


