// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x30), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g008(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nand2  g011(.a(new_n53_), .b(new_n62_), .O(new_n63_));
  oai21  g012(.a(x19), .b(x17), .c(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  aoi21  g014(.a(new_n56_), .b(x14), .c(x30), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n57_), .O(z01));
  and2   g017(.a(new_n63_), .b(x21), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x16), .O(new_n72_));
  aoi21  g021(.a(new_n56_), .b(x13), .c(x30), .O(new_n73_));
  oai21  g022(.a(new_n72_), .b(new_n69_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n57_), .O(z02));
  xor2a  g024(.a(new_n71_), .b(x22), .O(new_n76_));
  nor2   g025(.a(x16), .b(x12), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n59_), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n56_), .c(new_n78_), .O(z03));
  inv1   g028(.a(x22), .O(new_n80_));
  nand3  g029(.a(new_n70_), .b(new_n53_), .c(new_n80_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x23), .O(new_n82_));
  inv1   g031(.a(x23), .O(new_n83_));
  nand4  g032(.a(new_n70_), .b(new_n53_), .c(new_n83_), .d(new_n80_), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  aoi21  g034(.a(new_n56_), .b(x11), .c(x30), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n57_), .O(z04));
  nor2   g037(.a(x24), .b(x23), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n70_), .c(new_n53_), .d(new_n80_), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  aoi21  g040(.a(new_n84_), .b(x24), .c(new_n91_), .O(new_n92_));
  nor2   g041(.a(x16), .b(x10), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n59_), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n56_), .c(new_n94_), .O(z05));
  inv1   g044(.a(x25), .O(new_n96_));
  nor2   g045(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  oai21  g046(.a(new_n90_), .b(x25), .c(x16), .O(new_n98_));
  aoi21  g047(.a(new_n56_), .b(x09), .c(x30), .O(new_n99_));
  oai21  g048(.a(new_n98_), .b(new_n97_), .c(new_n99_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n57_), .O(z06));
  nand2  g050(.a(new_n91_), .b(new_n96_), .O(new_n102_));
  inv1   g051(.a(x26), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n96_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n90_), .O(new_n105_));
  aoi21  g054(.a(new_n102_), .b(x26), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x08), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n59_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n56_), .c(new_n108_), .O(z07));
  oai21  g058(.a(new_n104_), .b(new_n90_), .c(x27), .O(new_n110_));
  inv1   g059(.a(x27), .O(new_n111_));
  nor2   g060(.a(x26), .b(x25), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n89_), .c(new_n111_), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n81_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  nand3  g064(.a(new_n115_), .b(new_n110_), .c(x16), .O(new_n116_));
  aoi21  g065(.a(new_n56_), .b(x07), .c(x30), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n57_), .O(z08));
  inv1   g068(.a(x28), .O(new_n120_));
  nor2   g069(.a(new_n114_), .b(new_n120_), .O(new_n121_));
  inv1   g070(.a(new_n81_), .O(new_n122_));
  inv1   g071(.a(x24), .O(new_n123_));
  nand3  g072(.a(new_n111_), .b(new_n123_), .c(new_n83_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n104_), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n122_), .c(new_n120_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(x16), .O(new_n127_));
  aoi21  g076(.a(new_n56_), .b(x06), .c(x30), .O(new_n128_));
  oai21  g077(.a(new_n127_), .b(new_n121_), .c(new_n128_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n57_), .O(z09));
  nor2   g079(.a(x29), .b(x28), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n112_), .c(new_n111_), .d(new_n123_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n84_), .O(new_n133_));
  aoi22  g082(.a(new_n133_), .b(new_n58_), .c(new_n126_), .d(x29), .O(new_n134_));
  nor2   g083(.a(x16), .b(x05), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n59_), .O(new_n136_));
  oai21  g085(.a(new_n134_), .b(new_n56_), .c(new_n136_), .O(z10));
  nor2   g086(.a(x16), .b(x04), .O(new_n138_));
  nand3  g087(.a(new_n131_), .b(new_n111_), .c(new_n103_), .O(new_n139_));
  nand3  g088(.a(new_n89_), .b(new_n96_), .c(x16), .O(new_n140_));
  nor3   g089(.a(new_n140_), .b(new_n139_), .c(new_n81_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n138_), .c(new_n58_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n57_), .O(z11));
  oai21  g092(.a(new_n132_), .b(new_n84_), .c(x31), .O(new_n144_));
  inv1   g093(.a(x31), .O(new_n145_));
  inv1   g094(.a(new_n84_), .O(new_n146_));
  inv1   g095(.a(new_n132_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n146_), .c(new_n145_), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n144_), .c(x16), .O(new_n149_));
  aoi21  g098(.a(new_n56_), .b(x03), .c(x30), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n57_), .O(z12));
  inv1   g101(.a(x32), .O(new_n153_));
  nand3  g102(.a(new_n131_), .b(new_n153_), .c(new_n145_), .O(new_n154_));
  nor3   g103(.a(new_n154_), .b(new_n113_), .c(new_n81_), .O(new_n155_));
  aoi21  g104(.a(new_n148_), .b(x32), .c(new_n155_), .O(new_n156_));
  nor2   g105(.a(x16), .b(x02), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n59_), .O(new_n158_));
  oai21  g107(.a(new_n156_), .b(new_n56_), .c(new_n158_), .O(z13));
  inv1   g108(.a(new_n154_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n114_), .O(new_n161_));
  nor3   g110(.a(x29), .b(x28), .c(x27), .O(new_n162_));
  nor3   g111(.a(x33), .b(x32), .c(x31), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n162_), .c(new_n112_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n90_), .O(new_n165_));
  aoi21  g114(.a(new_n161_), .b(x33), .c(new_n165_), .O(new_n166_));
  nor2   g115(.a(x16), .b(x01), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n59_), .O(new_n168_));
  oai21  g117(.a(new_n166_), .b(new_n56_), .c(new_n168_), .O(z14));
  oai21  g118(.a(new_n164_), .b(new_n90_), .c(x34), .O(new_n170_));
  inv1   g119(.a(x33), .O(new_n171_));
  inv1   g120(.a(x34), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n171_), .c(new_n153_), .d(new_n145_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n139_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n91_), .c(new_n96_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n170_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  nor2   g126(.a(x16), .b(x00), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n59_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z15));
endmodule


