// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x23), .O(new_n52_));
  inv1   g001(.a(x29), .O(new_n53_));
  nand2  g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  and2   g004(.a(x19), .b(x17), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  xor2a  g012(.a(new_n57_), .b(new_n63_), .O(new_n64_));
  inv1   g013(.a(x18), .O(new_n65_));
  nand2  g014(.a(new_n54_), .b(new_n65_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  oai21  g017(.a(new_n64_), .b(new_n60_), .c(new_n68_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  aoi21  g019(.a(new_n57_), .b(new_n63_), .c(new_n70_), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n57_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(new_n71_), .c(x16), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n60_), .b(new_n76_), .c(x18), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n75_), .c(new_n55_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  nor2   g028(.a(new_n74_), .b(new_n79_), .O(new_n80_));
  nand3  g029(.a(new_n72_), .b(new_n57_), .c(new_n79_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n60_), .b(new_n84_), .c(x18), .O(new_n85_));
  aoi21  g034(.a(new_n85_), .b(new_n83_), .c(new_n55_), .O(z03));
  aoi21  g035(.a(new_n82_), .b(x16), .c(new_n53_), .O(new_n87_));
  nor2   g036(.a(new_n52_), .b(new_n60_), .O(new_n88_));
  oai21  g037(.a(x16), .b(x11), .c(new_n65_), .O(new_n89_));
  aoi21  g038(.a(new_n88_), .b(new_n81_), .c(new_n89_), .O(new_n90_));
  oai21  g039(.a(new_n87_), .b(x23), .c(new_n90_), .O(z04));
  nand2  g040(.a(new_n82_), .b(x16), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(x24), .c(x29), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n52_), .O(new_n94_));
  nand4  g043(.a(new_n72_), .b(new_n57_), .c(new_n52_), .d(new_n79_), .O(new_n95_));
  and2   g044(.a(x24), .b(x16), .O(new_n96_));
  oai21  g045(.a(x16), .b(x10), .c(new_n65_), .O(new_n97_));
  aoi21  g046(.a(new_n96_), .b(new_n95_), .c(new_n97_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n94_), .O(z05));
  nor2   g048(.a(x25), .b(x24), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n82_), .c(x16), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x29), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n52_), .O(new_n103_));
  inv1   g052(.a(new_n95_), .O(new_n104_));
  nor2   g053(.a(new_n53_), .b(x24), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(x25), .c(x16), .O(new_n107_));
  inv1   g056(.a(x09), .O(new_n108_));
  aoi21  g057(.a(new_n60_), .b(new_n108_), .c(x18), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n107_), .c(new_n103_), .O(z06));
  inv1   g059(.a(x26), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n60_), .O(new_n112_));
  oai21  g061(.a(new_n106_), .b(x25), .c(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n100_), .b(new_n111_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n92_), .c(x29), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n52_), .O(new_n116_));
  inv1   g065(.a(x08), .O(new_n117_));
  aoi21  g066(.a(new_n60_), .b(new_n117_), .c(x18), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n116_), .c(new_n113_), .O(z07));
  inv1   g068(.a(x25), .O(new_n120_));
  nand4  g069(.a(new_n105_), .b(new_n104_), .c(new_n111_), .d(new_n120_), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(x27), .c(x16), .O(new_n122_));
  nor2   g071(.a(x24), .b(x22), .O(new_n123_));
  nor3   g072(.a(x27), .b(x26), .c(x25), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n123_), .c(new_n74_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n60_), .c(x29), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n52_), .O(new_n127_));
  inv1   g076(.a(x07), .O(new_n128_));
  aoi21  g077(.a(new_n60_), .b(new_n128_), .c(x18), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n127_), .c(new_n122_), .O(z08));
  oai21  g079(.a(x16), .b(x06), .c(new_n65_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n54_), .O(new_n132_));
  inv1   g081(.a(x28), .O(new_n133_));
  inv1   g082(.a(x27), .O(new_n134_));
  nor2   g083(.a(x26), .b(x25), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n123_), .c(new_n134_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n73_), .c(x29), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n52_), .c(new_n133_), .O(new_n138_));
  nand4  g087(.a(new_n100_), .b(new_n133_), .c(new_n134_), .d(new_n111_), .O(new_n139_));
  nor3   g088(.a(new_n139_), .b(new_n95_), .c(new_n53_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n138_), .c(x16), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n132_), .O(z09));
  nor2   g091(.a(new_n139_), .b(new_n95_), .O(new_n143_));
  oai21  g092(.a(x29), .b(new_n52_), .c(x16), .O(new_n144_));
  nor2   g093(.a(x16), .b(x05), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n66_), .O(new_n146_));
  oai21  g095(.a(new_n144_), .b(new_n143_), .c(new_n146_), .O(z10));
  inv1   g096(.a(new_n66_), .O(new_n148_));
  inv1   g097(.a(x04), .O(new_n149_));
  nand2  g098(.a(new_n60_), .b(new_n149_), .O(new_n150_));
  nand2  g099(.a(x30), .b(x16), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n150_), .c(new_n148_), .O(z11));
  nand2  g101(.a(x31), .b(x16), .O(new_n153_));
  inv1   g102(.a(x03), .O(new_n154_));
  aoi21  g103(.a(new_n60_), .b(new_n154_), .c(x18), .O(new_n155_));
  aoi21  g104(.a(new_n155_), .b(new_n153_), .c(new_n55_), .O(z12));
  nand2  g105(.a(x32), .b(x16), .O(new_n157_));
  inv1   g106(.a(x02), .O(new_n158_));
  aoi21  g107(.a(new_n60_), .b(new_n158_), .c(x18), .O(new_n159_));
  aoi21  g108(.a(new_n159_), .b(new_n157_), .c(new_n55_), .O(z13));
  inv1   g109(.a(x01), .O(new_n161_));
  nand2  g110(.a(new_n60_), .b(new_n161_), .O(new_n162_));
  nand2  g111(.a(x33), .b(x16), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n162_), .c(new_n148_), .O(z14));
  nand2  g113(.a(x34), .b(x16), .O(new_n165_));
  inv1   g114(.a(x00), .O(new_n166_));
  aoi21  g115(.a(new_n60_), .b(new_n166_), .c(x18), .O(new_n167_));
  aoi21  g116(.a(new_n167_), .b(new_n165_), .c(new_n55_), .O(z15));
endmodule


