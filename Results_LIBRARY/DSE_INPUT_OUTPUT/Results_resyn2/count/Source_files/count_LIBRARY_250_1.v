// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  xor2a  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g008(.a(x14), .O(new_n60_));
  aoi21  g009(.a(new_n52_), .b(new_n60_), .c(x18), .O(new_n61_));
  oai21  g010(.a(new_n59_), .b(new_n52_), .c(new_n61_), .O(z01));
  nand2  g011(.a(new_n58_), .b(new_n57_), .O(new_n63_));
  nor2   g012(.a(x21), .b(x20), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n58_), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  aoi21  g015(.a(new_n63_), .b(x21), .c(new_n66_), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n52_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n52_), .c(new_n69_), .O(z02));
  xor2a  g019(.a(new_n65_), .b(x22), .O(new_n71_));
  inv1   g020(.a(x12), .O(new_n72_));
  aoi21  g021(.a(new_n52_), .b(new_n72_), .c(x18), .O(new_n73_));
  oai21  g022(.a(new_n71_), .b(new_n52_), .c(new_n73_), .O(z03));
  nor2   g023(.a(x18), .b(x16), .O(new_n75_));
  inv1   g024(.a(new_n75_), .O(new_n76_));
  inv1   g025(.a(x18), .O(new_n77_));
  oai21  g026(.a(new_n65_), .b(x22), .c(x23), .O(new_n78_));
  nor2   g027(.a(x23), .b(x22), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n78_), .c(new_n77_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x16), .O(new_n82_));
  oai21  g031(.a(new_n76_), .b(x11), .c(new_n82_), .O(z04));
  xor2a  g032(.a(new_n80_), .b(x24), .O(new_n84_));
  inv1   g033(.a(x10), .O(new_n85_));
  aoi21  g034(.a(new_n52_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n52_), .c(new_n86_), .O(z05));
  inv1   g036(.a(x24), .O(new_n88_));
  nand4  g037(.a(new_n79_), .b(new_n64_), .c(new_n58_), .d(new_n88_), .O(new_n89_));
  nor2   g038(.a(x25), .b(x24), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n79_), .c(new_n64_), .d(new_n58_), .O(new_n91_));
  inv1   g040(.a(new_n91_), .O(new_n92_));
  aoi21  g041(.a(new_n89_), .b(x25), .c(new_n92_), .O(new_n93_));
  inv1   g042(.a(x09), .O(new_n94_));
  aoi21  g043(.a(new_n52_), .b(new_n94_), .c(x18), .O(new_n95_));
  oai21  g044(.a(new_n93_), .b(new_n52_), .c(new_n95_), .O(z06));
  nand2  g045(.a(new_n52_), .b(x08), .O(new_n97_));
  nor2   g046(.a(new_n91_), .b(x26), .O(new_n98_));
  inv1   g047(.a(x26), .O(new_n99_));
  oai21  g048(.a(new_n92_), .b(new_n99_), .c(x16), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n77_), .O(z07));
  inv1   g051(.a(new_n89_), .O(new_n103_));
  nor3   g052(.a(x27), .b(x26), .c(x25), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  oai21  g054(.a(new_n91_), .b(x26), .c(x27), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(new_n105_), .c(new_n77_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(x16), .O(new_n108_));
  oai21  g057(.a(new_n76_), .b(x07), .c(new_n108_), .O(z08));
  inv1   g058(.a(x28), .O(new_n110_));
  aoi21  g059(.a(new_n104_), .b(new_n103_), .c(new_n110_), .O(new_n111_));
  nand3  g060(.a(new_n104_), .b(new_n103_), .c(new_n110_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n77_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(new_n111_), .c(x16), .O(new_n114_));
  inv1   g063(.a(x06), .O(new_n115_));
  nand2  g064(.a(new_n75_), .b(new_n115_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n114_), .O(z09));
  nand2  g066(.a(new_n104_), .b(new_n110_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n89_), .c(x29), .O(new_n119_));
  nor3   g068(.a(x29), .b(x28), .c(x27), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n98_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(x16), .O(new_n123_));
  inv1   g072(.a(x05), .O(new_n124_));
  aoi21  g073(.a(new_n52_), .b(new_n124_), .c(x18), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n123_), .O(z10));
  inv1   g075(.a(x30), .O(new_n127_));
  aoi21  g076(.a(new_n120_), .b(new_n98_), .c(new_n127_), .O(new_n128_));
  inv1   g077(.a(x27), .O(new_n129_));
  nor2   g078(.a(x29), .b(x28), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n127_), .c(new_n129_), .O(new_n131_));
  inv1   g080(.a(new_n131_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n92_), .c(new_n99_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n77_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n128_), .c(x16), .O(new_n135_));
  inv1   g084(.a(x04), .O(new_n136_));
  nand2  g085(.a(new_n75_), .b(new_n136_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n135_), .O(z11));
  nor2   g087(.a(x31), .b(x30), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n130_), .c(new_n104_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n89_), .O(new_n141_));
  aoi21  g090(.a(new_n133_), .b(x31), .c(new_n141_), .O(new_n142_));
  inv1   g091(.a(x03), .O(new_n143_));
  aoi21  g092(.a(new_n52_), .b(new_n143_), .c(x18), .O(new_n144_));
  oai21  g093(.a(new_n142_), .b(new_n52_), .c(new_n144_), .O(z12));
  oai21  g094(.a(new_n140_), .b(new_n89_), .c(x32), .O(new_n146_));
  nor2   g095(.a(x26), .b(x25), .O(new_n147_));
  nor2   g096(.a(x32), .b(x31), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  inv1   g098(.a(new_n149_), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n132_), .c(new_n103_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n146_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  inv1   g102(.a(x02), .O(new_n154_));
  aoi21  g103(.a(new_n52_), .b(new_n154_), .c(x18), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n153_), .O(z13));
  nor2   g105(.a(x27), .b(x26), .O(new_n157_));
  nor2   g106(.a(x33), .b(x32), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n139_), .c(new_n130_), .d(new_n157_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n91_), .c(new_n77_), .O(new_n160_));
  aoi21  g109(.a(new_n151_), .b(x33), .c(new_n160_), .O(new_n161_));
  oai22  g110(.a(new_n161_), .b(new_n52_), .c(new_n76_), .d(x01), .O(z14));
  inv1   g111(.a(x00), .O(new_n163_));
  nand2  g112(.a(new_n75_), .b(new_n163_), .O(new_n164_));
  nor3   g113(.a(new_n159_), .b(new_n91_), .c(x34), .O(new_n165_));
  oai21  g114(.a(new_n159_), .b(new_n91_), .c(x34), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n77_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n165_), .c(x16), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n164_), .O(z15));
endmodule


