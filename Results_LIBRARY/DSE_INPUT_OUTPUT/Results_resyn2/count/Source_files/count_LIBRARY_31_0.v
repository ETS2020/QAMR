// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_;
  nor2   g00(.a(x19), .b(x17), .O(new_n52_));
  inv1   g01(.a(new_n52_), .O(new_n53_));
  inv1   g02(.a(x16), .O(new_n54_));
  nor2   g03(.a(x18), .b(new_n54_), .O(new_n55_));
  nand2  g04(.a(x19), .b(x17), .O(new_n56_));
  nand3  g05(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(z00));
  nand2  g06(.a(new_n53_), .b(x20), .O(new_n58_));
  inv1   g07(.a(x20), .O(new_n59_));
  nand2  g08(.a(new_n52_), .b(new_n59_), .O(new_n60_));
  nand3  g09(.a(new_n60_), .b(new_n58_), .c(new_n55_), .O(z01));
  inv1   g10(.a(x18), .O(new_n62_));
  nor2   g11(.a(x21), .b(x20), .O(new_n63_));
  aoi22  g12(.a(new_n63_), .b(new_n52_), .c(new_n60_), .d(x21), .O(new_n64_));
  oai21  g13(.a(new_n64_), .b(new_n54_), .c(new_n62_), .O(z02));
  nand2  g14(.a(new_n63_), .b(new_n52_), .O(new_n66_));
  xor2a  g15(.a(new_n66_), .b(x22), .O(new_n67_));
  oai21  g16(.a(new_n67_), .b(new_n54_), .c(new_n62_), .O(z03));
  inv1   g17(.a(x22), .O(new_n69_));
  nand3  g18(.a(new_n63_), .b(new_n52_), .c(new_n69_), .O(new_n70_));
  nor2   g19(.a(x23), .b(x22), .O(new_n71_));
  nand3  g20(.a(new_n71_), .b(new_n63_), .c(new_n52_), .O(new_n72_));
  inv1   g21(.a(new_n72_), .O(new_n73_));
  aoi21  g22(.a(new_n70_), .b(x23), .c(new_n73_), .O(new_n74_));
  oai21  g23(.a(new_n74_), .b(new_n54_), .c(new_n62_), .O(z04));
  xor2a  g24(.a(new_n72_), .b(x24), .O(new_n76_));
  oai21  g25(.a(new_n76_), .b(new_n54_), .c(new_n62_), .O(z05));
  inv1   g26(.a(x24), .O(new_n78_));
  nand4  g27(.a(new_n71_), .b(new_n63_), .c(new_n52_), .d(new_n78_), .O(new_n79_));
  nand2  g28(.a(new_n79_), .b(x25), .O(new_n80_));
  nor2   g29(.a(x25), .b(x24), .O(new_n81_));
  nand4  g30(.a(new_n81_), .b(new_n71_), .c(new_n63_), .d(new_n52_), .O(new_n82_));
  nand3  g31(.a(new_n82_), .b(new_n80_), .c(new_n55_), .O(z06));
  nor2   g32(.a(x26), .b(x25), .O(new_n84_));
  nand2  g33(.a(new_n84_), .b(new_n78_), .O(new_n85_));
  nor2   g34(.a(new_n85_), .b(new_n72_), .O(new_n86_));
  aoi21  g35(.a(new_n82_), .b(x26), .c(new_n86_), .O(new_n87_));
  oai21  g36(.a(new_n87_), .b(new_n54_), .c(new_n62_), .O(z07));
  inv1   g37(.a(new_n85_), .O(new_n89_));
  nand2  g38(.a(new_n89_), .b(new_n73_), .O(new_n90_));
  inv1   g39(.a(x25), .O(new_n91_));
  nor2   g40(.a(x27), .b(x26), .O(new_n92_));
  nand2  g41(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nor2   g42(.a(new_n93_), .b(new_n79_), .O(new_n94_));
  aoi21  g43(.a(new_n90_), .b(x27), .c(new_n94_), .O(new_n95_));
  oai21  g44(.a(new_n95_), .b(new_n54_), .c(new_n62_), .O(z08));
  oai21  g45(.a(new_n93_), .b(new_n79_), .c(x28), .O(new_n97_));
  nor2   g46(.a(x28), .b(x27), .O(new_n98_));
  nand2  g47(.a(new_n98_), .b(new_n86_), .O(new_n99_));
  nand3  g48(.a(new_n99_), .b(new_n97_), .c(new_n55_), .O(z09));
  nand2  g49(.a(new_n99_), .b(x29), .O(new_n101_));
  inv1   g50(.a(x29), .O(new_n102_));
  nand4  g51(.a(new_n98_), .b(new_n84_), .c(new_n102_), .d(new_n78_), .O(new_n103_));
  or2    g52(.a(new_n103_), .b(new_n72_), .O(new_n104_));
  nand3  g53(.a(new_n104_), .b(new_n101_), .c(new_n55_), .O(z10));
  oai21  g54(.a(new_n103_), .b(new_n72_), .c(x30), .O(new_n106_));
  nor2   g55(.a(x30), .b(x29), .O(new_n107_));
  nand2  g56(.a(new_n107_), .b(new_n98_), .O(new_n108_));
  inv1   g57(.a(new_n108_), .O(new_n109_));
  nand3  g58(.a(new_n109_), .b(new_n89_), .c(new_n73_), .O(new_n110_));
  nand2  g59(.a(new_n110_), .b(new_n106_), .O(new_n111_));
  nand2  g60(.a(new_n111_), .b(x16), .O(new_n112_));
  nand2  g61(.a(new_n112_), .b(new_n62_), .O(z11));
  nor2   g62(.a(x31), .b(x28), .O(new_n114_));
  nand4  g63(.a(new_n114_), .b(new_n107_), .c(new_n92_), .d(new_n91_), .O(new_n115_));
  nor2   g64(.a(new_n115_), .b(new_n79_), .O(new_n116_));
  aoi21  g65(.a(new_n110_), .b(x31), .c(new_n116_), .O(new_n117_));
  oai21  g66(.a(new_n117_), .b(new_n54_), .c(new_n62_), .O(z12));
  oai21  g67(.a(new_n115_), .b(new_n79_), .c(x32), .O(new_n119_));
  inv1   g68(.a(new_n79_), .O(new_n120_));
  nor2   g69(.a(x32), .b(x31), .O(new_n121_));
  nand4  g70(.a(new_n121_), .b(new_n109_), .c(new_n84_), .d(new_n120_), .O(new_n122_));
  nand3  g71(.a(new_n122_), .b(new_n119_), .c(new_n55_), .O(z13));
  nand2  g72(.a(new_n122_), .b(x33), .O(new_n124_));
  nor2   g73(.a(x33), .b(x32), .O(new_n125_));
  nand4  g74(.a(new_n125_), .b(new_n114_), .c(new_n107_), .d(new_n92_), .O(new_n126_));
  nor2   g75(.a(new_n126_), .b(new_n82_), .O(new_n127_));
  inv1   g76(.a(new_n127_), .O(new_n128_));
  nand3  g77(.a(new_n128_), .b(new_n124_), .c(new_n55_), .O(z14));
  inv1   g78(.a(x34), .O(new_n130_));
  oai21  g79(.a(new_n126_), .b(new_n82_), .c(new_n130_), .O(new_n131_));
  nand2  g80(.a(new_n127_), .b(x34), .O(new_n132_));
  nand3  g81(.a(new_n132_), .b(new_n131_), .c(x16), .O(new_n133_));
  nand2  g82(.a(new_n133_), .b(new_n62_), .O(z15));
endmodule


