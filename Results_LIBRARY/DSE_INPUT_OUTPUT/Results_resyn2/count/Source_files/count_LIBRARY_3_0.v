// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x18), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x15), .O(new_n55_));
  inv1   g004(.a(new_n55_), .O(new_n56_));
  oai21  g005(.a(new_n53_), .b(new_n52_), .c(new_n56_), .O(z00));
  inv1   g006(.a(x20), .O(new_n58_));
  nor2   g007(.a(x19), .b(x17), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  oai21  g009(.a(x19), .b(x17), .c(x20), .O(new_n61_));
  nand3  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g011(.a(x15), .O(new_n63_));
  aoi21  g012(.a(new_n52_), .b(x14), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n54_), .O(z01));
  nor2   g015(.a(x21), .b(x20), .O(new_n67_));
  aoi22  g016(.a(new_n67_), .b(new_n59_), .c(new_n60_), .d(x21), .O(new_n68_));
  nor2   g017(.a(x16), .b(x13), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n55_), .O(new_n70_));
  oai21  g019(.a(new_n68_), .b(new_n52_), .c(new_n70_), .O(z02));
  inv1   g020(.a(x22), .O(new_n72_));
  aoi21  g021(.a(new_n67_), .b(new_n59_), .c(new_n72_), .O(new_n73_));
  nand3  g022(.a(new_n67_), .b(new_n59_), .c(new_n72_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(new_n73_), .c(x16), .O(new_n76_));
  nor2   g025(.a(x16), .b(x12), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n55_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n76_), .O(z03));
  nand2  g028(.a(new_n74_), .b(x23), .O(new_n80_));
  inv1   g029(.a(x23), .O(new_n81_));
  nand4  g030(.a(new_n67_), .b(new_n59_), .c(new_n81_), .d(new_n72_), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  aoi21  g032(.a(new_n52_), .b(x11), .c(new_n63_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n54_), .O(z04));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n67_), .c(new_n59_), .d(new_n72_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  aoi21  g038(.a(new_n82_), .b(x24), .c(new_n89_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x10), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n55_), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n52_), .c(new_n92_), .O(z05));
  nor2   g042(.a(new_n88_), .b(x25), .O(new_n94_));
  inv1   g043(.a(x25), .O(new_n95_));
  oai21  g044(.a(new_n89_), .b(new_n95_), .c(x16), .O(new_n96_));
  aoi21  g045(.a(new_n52_), .b(x09), .c(new_n63_), .O(new_n97_));
  oai21  g046(.a(new_n96_), .b(new_n94_), .c(new_n97_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n54_), .O(z06));
  inv1   g048(.a(new_n82_), .O(new_n100_));
  nand2  g049(.a(new_n89_), .b(new_n95_), .O(new_n101_));
  nor3   g050(.a(x26), .b(x25), .c(x24), .O(new_n102_));
  aoi22  g051(.a(new_n102_), .b(new_n100_), .c(new_n101_), .d(x26), .O(new_n103_));
  nor2   g052(.a(x16), .b(x08), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n55_), .O(new_n105_));
  oai21  g054(.a(new_n103_), .b(new_n52_), .c(new_n105_), .O(z07));
  nor3   g055(.a(x27), .b(x26), .c(x25), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n89_), .O(new_n108_));
  inv1   g057(.a(new_n102_), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n82_), .c(x27), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n108_), .c(x16), .O(new_n111_));
  aoi21  g060(.a(new_n52_), .b(x07), .c(new_n63_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n54_), .O(z08));
  inv1   g063(.a(x28), .O(new_n115_));
  aoi21  g064(.a(new_n107_), .b(new_n89_), .c(new_n115_), .O(new_n116_));
  nand3  g065(.a(new_n107_), .b(new_n89_), .c(new_n115_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(x16), .O(new_n118_));
  aoi21  g067(.a(new_n52_), .b(x06), .c(new_n63_), .O(new_n119_));
  oai21  g068(.a(new_n118_), .b(new_n116_), .c(new_n119_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n54_), .O(z09));
  nand2  g070(.a(new_n107_), .b(new_n115_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n88_), .c(x29), .O(new_n123_));
  nor3   g072(.a(x29), .b(x28), .c(x27), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n102_), .c(new_n100_), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n123_), .c(x16), .O(new_n126_));
  aoi21  g075(.a(new_n52_), .b(x05), .c(new_n63_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n54_), .O(z10));
  xor2a  g078(.a(new_n125_), .b(x30), .O(new_n130_));
  nor2   g079(.a(x16), .b(x04), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n55_), .O(new_n132_));
  oai21  g081(.a(new_n130_), .b(new_n52_), .c(new_n132_), .O(z11));
  inv1   g082(.a(x30), .O(new_n134_));
  nand4  g083(.a(new_n124_), .b(new_n102_), .c(new_n100_), .d(new_n134_), .O(new_n135_));
  nor2   g084(.a(x29), .b(x28), .O(new_n136_));
  nor2   g085(.a(x31), .b(x30), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n136_), .c(new_n107_), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n88_), .O(new_n139_));
  aoi21  g088(.a(new_n135_), .b(x31), .c(new_n139_), .O(new_n140_));
  nor2   g089(.a(x16), .b(x03), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n55_), .O(new_n142_));
  oai21  g091(.a(new_n140_), .b(new_n52_), .c(new_n142_), .O(z12));
  oai21  g092(.a(new_n138_), .b(new_n88_), .c(x32), .O(new_n144_));
  inv1   g093(.a(x32), .O(new_n145_));
  nand2  g094(.a(new_n137_), .b(new_n136_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n107_), .c(new_n89_), .d(new_n145_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  nor2   g099(.a(x16), .b(x02), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n55_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(z13));
  nor2   g102(.a(x27), .b(x26), .O(new_n154_));
  nor2   g103(.a(x33), .b(x32), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n154_), .c(new_n137_), .d(new_n136_), .O(new_n156_));
  nor3   g105(.a(new_n156_), .b(new_n88_), .c(x25), .O(new_n157_));
  aoi21  g106(.a(new_n148_), .b(x33), .c(new_n157_), .O(new_n158_));
  nor2   g107(.a(x16), .b(x01), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n55_), .O(new_n160_));
  oai21  g109(.a(new_n158_), .b(new_n52_), .c(new_n160_), .O(z14));
  nand2  g110(.a(new_n157_), .b(x34), .O(new_n162_));
  inv1   g111(.a(x34), .O(new_n163_));
  oai21  g112(.a(new_n156_), .b(new_n101_), .c(new_n163_), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n162_), .c(x16), .O(new_n165_));
  nor2   g114(.a(x16), .b(x00), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n55_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n165_), .O(z15));
endmodule


