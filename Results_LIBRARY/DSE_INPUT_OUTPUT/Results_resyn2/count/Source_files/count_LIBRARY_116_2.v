// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_;
  nand2  g000(.a(x25), .b(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x18), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  oai21  g010(.a(new_n53_), .b(new_n61_), .c(x16), .O(new_n62_));
  aoi21  g011(.a(new_n56_), .b(x14), .c(x18), .O(new_n63_));
  oai21  g012(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n52_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  nand2  g015(.a(new_n60_), .b(new_n66_), .O(new_n67_));
  nand2  g016(.a(new_n53_), .b(new_n61_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(x21), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(x16), .O(new_n70_));
  aoi21  g019(.a(new_n56_), .b(x13), .c(x18), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n52_), .O(z02));
  inv1   g022(.a(x18), .O(new_n74_));
  nand2  g023(.a(new_n56_), .b(x12), .O(new_n75_));
  nand2  g024(.a(new_n67_), .b(x22), .O(new_n76_));
  nor2   g025(.a(x22), .b(x21), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n60_), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n76_), .c(x16), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n74_), .O(z03));
  nand2  g030(.a(new_n78_), .b(x23), .O(new_n82_));
  nor3   g031(.a(x23), .b(x22), .c(x21), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n60_), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  aoi21  g034(.a(new_n56_), .b(x11), .c(x18), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n52_), .O(z04));
  inv1   g037(.a(x24), .O(new_n89_));
  inv1   g038(.a(x23), .O(new_n90_));
  nand2  g039(.a(new_n77_), .b(new_n90_), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n68_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n77_), .c(new_n53_), .d(new_n61_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x16), .O(new_n96_));
  nand2  g045(.a(new_n56_), .b(x10), .O(new_n97_));
  oai21  g046(.a(new_n96_), .b(new_n93_), .c(new_n97_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n74_), .O(z05));
  inv1   g048(.a(x25), .O(new_n100_));
  nor2   g049(.a(x25), .b(x24), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n90_), .c(x16), .O(new_n102_));
  oai22  g051(.a(new_n102_), .b(new_n78_), .c(x16), .d(x09), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n74_), .O(new_n104_));
  aoi21  g053(.a(new_n95_), .b(x16), .c(x18), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(new_n100_), .c(new_n104_), .O(z06));
  aoi21  g055(.a(x26), .b(x16), .c(x18), .O(new_n107_));
  nand2  g056(.a(new_n95_), .b(x26), .O(new_n108_));
  nor2   g057(.a(x26), .b(x24), .O(new_n109_));
  nand4  g058(.a(new_n109_), .b(new_n83_), .c(new_n60_), .d(new_n100_), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n108_), .c(x16), .O(new_n111_));
  aoi21  g060(.a(new_n56_), .b(x08), .c(x18), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  oai21  g062(.a(new_n107_), .b(new_n100_), .c(new_n113_), .O(z07));
  inv1   g063(.a(x27), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n56_), .c(new_n74_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(x25), .O(new_n117_));
  inv1   g066(.a(new_n95_), .O(new_n118_));
  nor3   g067(.a(x27), .b(x26), .c(x25), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  inv1   g069(.a(new_n109_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n84_), .c(x27), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n120_), .c(x16), .O(new_n123_));
  aoi21  g072(.a(new_n56_), .b(x07), .c(x18), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n117_), .O(z08));
  inv1   g075(.a(new_n77_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n68_), .O(new_n128_));
  nand2  g077(.a(new_n101_), .b(new_n90_), .O(new_n129_));
  inv1   g078(.a(x26), .O(new_n130_));
  nor2   g079(.a(x28), .b(x27), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  aoi22  g082(.a(new_n133_), .b(new_n128_), .c(new_n120_), .d(x28), .O(new_n134_));
  inv1   g083(.a(x06), .O(new_n135_));
  aoi21  g084(.a(new_n56_), .b(new_n135_), .c(x18), .O(new_n136_));
  oai21  g085(.a(new_n134_), .b(new_n56_), .c(new_n136_), .O(z09));
  inv1   g086(.a(x29), .O(new_n138_));
  aoi21  g087(.a(new_n133_), .b(new_n128_), .c(new_n138_), .O(new_n139_));
  nand2  g088(.a(new_n131_), .b(new_n138_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n110_), .O(new_n141_));
  oai21  g090(.a(new_n141_), .b(new_n139_), .c(x16), .O(new_n142_));
  inv1   g091(.a(x05), .O(new_n143_));
  aoi21  g092(.a(new_n56_), .b(new_n143_), .c(x18), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n142_), .O(z10));
  nand2  g094(.a(new_n141_), .b(x30), .O(new_n146_));
  inv1   g095(.a(x30), .O(new_n147_));
  oai21  g096(.a(new_n140_), .b(new_n110_), .c(new_n147_), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n146_), .c(x16), .O(new_n149_));
  inv1   g098(.a(x04), .O(new_n150_));
  aoi21  g099(.a(new_n56_), .b(new_n150_), .c(x18), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n149_), .O(z11));
  nand2  g101(.a(x31), .b(x16), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n74_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(x25), .O(new_n155_));
  nor2   g104(.a(x30), .b(x29), .O(new_n156_));
  nor2   g105(.a(x31), .b(x28), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  inv1   g107(.a(new_n158_), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n119_), .c(new_n118_), .O(new_n160_));
  nand3  g109(.a(new_n156_), .b(new_n131_), .c(new_n109_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n84_), .c(x31), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n160_), .c(x16), .O(new_n163_));
  aoi21  g112(.a(new_n56_), .b(x03), .c(x18), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n155_), .O(z12));
  nor2   g115(.a(x32), .b(x31), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n156_), .c(new_n131_), .d(new_n130_), .O(new_n168_));
  nor3   g117(.a(new_n168_), .b(new_n95_), .c(x25), .O(new_n169_));
  aoi21  g118(.a(new_n160_), .b(x32), .c(new_n169_), .O(new_n170_));
  inv1   g119(.a(x02), .O(new_n171_));
  aoi21  g120(.a(new_n56_), .b(new_n171_), .c(x18), .O(new_n172_));
  oai21  g121(.a(new_n170_), .b(new_n56_), .c(new_n172_), .O(z13));
  nand2  g122(.a(x33), .b(x16), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n74_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(x25), .O(new_n176_));
  oai21  g125(.a(new_n168_), .b(new_n95_), .c(x33), .O(new_n177_));
  nor2   g126(.a(x33), .b(x32), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n101_), .c(new_n115_), .d(new_n130_), .O(new_n179_));
  inv1   g128(.a(new_n179_), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n159_), .c(new_n92_), .O(new_n181_));
  nand3  g130(.a(new_n181_), .b(new_n177_), .c(x16), .O(new_n182_));
  aoi21  g131(.a(new_n56_), .b(x01), .c(x18), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n182_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n176_), .O(z14));
  xor2a  g134(.a(new_n181_), .b(x34), .O(new_n186_));
  inv1   g135(.a(x00), .O(new_n187_));
  aoi21  g136(.a(new_n56_), .b(new_n187_), .c(x18), .O(new_n188_));
  oai21  g137(.a(new_n186_), .b(new_n56_), .c(new_n188_), .O(z15));
endmodule


