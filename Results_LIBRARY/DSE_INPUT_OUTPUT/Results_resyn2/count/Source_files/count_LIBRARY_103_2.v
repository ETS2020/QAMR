// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  nand2  g000(.a(x34), .b(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x18), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nor2   g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  inv1   g010(.a(x18), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  inv1   g013(.a(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n61_), .c(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n56_), .b(new_n67_), .c(x18), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n66_), .O(z01));
  nand2  g018(.a(new_n53_), .b(new_n60_), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  aoi22  g020(.a(new_n71_), .b(new_n53_), .c(new_n70_), .d(x21), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n56_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n56_), .c(new_n74_), .O(z02));
  nand2  g024(.a(new_n71_), .b(new_n53_), .O(new_n76_));
  xor2a  g025(.a(new_n76_), .b(x22), .O(new_n77_));
  inv1   g026(.a(x12), .O(new_n78_));
  aoi21  g027(.a(new_n56_), .b(new_n78_), .c(x18), .O(new_n79_));
  oai21  g028(.a(new_n77_), .b(new_n56_), .c(new_n79_), .O(z03));
  oai21  g029(.a(new_n76_), .b(x22), .c(x23), .O(new_n81_));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n71_), .c(new_n53_), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n81_), .c(x16), .O(new_n84_));
  aoi21  g033(.a(new_n56_), .b(x11), .c(x18), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n52_), .O(z04));
  nor2   g036(.a(x24), .b(x21), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n82_), .O(new_n89_));
  inv1   g038(.a(new_n89_), .O(new_n90_));
  aoi22  g039(.a(new_n90_), .b(new_n65_), .c(new_n83_), .d(x24), .O(new_n91_));
  inv1   g040(.a(x10), .O(new_n92_));
  aoi21  g041(.a(new_n56_), .b(new_n92_), .c(x18), .O(new_n93_));
  oai21  g042(.a(new_n91_), .b(new_n56_), .c(new_n93_), .O(z05));
  nor2   g043(.a(x25), .b(x24), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n82_), .c(new_n71_), .d(new_n53_), .O(new_n96_));
  oai21  g045(.a(new_n89_), .b(new_n70_), .c(x25), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n96_), .c(x16), .O(new_n98_));
  aoi21  g047(.a(new_n56_), .b(x09), .c(x18), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n52_), .O(z06));
  nand2  g050(.a(new_n96_), .b(x26), .O(new_n102_));
  inv1   g051(.a(new_n83_), .O(new_n103_));
  nor3   g052(.a(x26), .b(x25), .c(x24), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n102_), .c(x16), .O(new_n106_));
  aoi21  g055(.a(new_n56_), .b(x08), .c(x18), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n52_), .O(z07));
  nor3   g058(.a(x27), .b(x26), .c(x25), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n88_), .c(new_n82_), .d(new_n63_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(x16), .O(new_n112_));
  aoi21  g061(.a(new_n105_), .b(x27), .c(new_n112_), .O(new_n113_));
  inv1   g062(.a(x07), .O(new_n114_));
  oai21  g063(.a(x16), .b(new_n114_), .c(new_n62_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n113_), .c(new_n52_), .O(z08));
  inv1   g065(.a(x28), .O(new_n117_));
  nor2   g066(.a(x27), .b(x26), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n96_), .O(new_n120_));
  aoi21  g069(.a(new_n111_), .b(x28), .c(new_n120_), .O(new_n121_));
  inv1   g070(.a(x06), .O(new_n122_));
  aoi21  g071(.a(new_n56_), .b(new_n122_), .c(x18), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n56_), .c(new_n123_), .O(z09));
  inv1   g073(.a(x29), .O(new_n125_));
  nor2   g074(.a(x29), .b(x28), .O(new_n126_));
  inv1   g075(.a(new_n126_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(x27), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n104_), .c(new_n103_), .O(new_n129_));
  oai21  g078(.a(new_n120_), .b(new_n125_), .c(new_n129_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  inv1   g080(.a(x05), .O(new_n132_));
  aoi21  g081(.a(new_n56_), .b(new_n132_), .c(x18), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n131_), .O(z10));
  xor2a  g083(.a(new_n129_), .b(x30), .O(new_n135_));
  inv1   g084(.a(x04), .O(new_n136_));
  aoi21  g085(.a(new_n56_), .b(new_n136_), .c(x18), .O(new_n137_));
  oai21  g086(.a(new_n135_), .b(new_n56_), .c(new_n137_), .O(z11));
  inv1   g087(.a(x30), .O(new_n139_));
  nand4  g088(.a(new_n128_), .b(new_n104_), .c(new_n103_), .d(new_n139_), .O(new_n140_));
  nor2   g089(.a(x31), .b(x30), .O(new_n141_));
  nor2   g090(.a(new_n127_), .b(new_n111_), .O(new_n142_));
  aoi22  g091(.a(new_n142_), .b(new_n141_), .c(new_n140_), .d(x31), .O(new_n143_));
  inv1   g092(.a(x03), .O(new_n144_));
  aoi21  g093(.a(new_n56_), .b(new_n144_), .c(x18), .O(new_n145_));
  oai21  g094(.a(new_n143_), .b(new_n56_), .c(new_n145_), .O(z12));
  nor2   g095(.a(new_n89_), .b(new_n70_), .O(new_n147_));
  nand4  g096(.a(new_n141_), .b(new_n126_), .c(new_n110_), .d(new_n147_), .O(new_n148_));
  nor3   g097(.a(x32), .b(x31), .c(x30), .O(new_n149_));
  aoi22  g098(.a(new_n149_), .b(new_n142_), .c(new_n148_), .d(x32), .O(new_n150_));
  inv1   g099(.a(x02), .O(new_n151_));
  aoi21  g100(.a(new_n56_), .b(new_n151_), .c(x18), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n56_), .c(new_n152_), .O(z13));
  nand4  g102(.a(new_n149_), .b(new_n126_), .c(new_n110_), .d(new_n147_), .O(new_n154_));
  inv1   g103(.a(x33), .O(new_n155_));
  nand4  g104(.a(new_n149_), .b(new_n126_), .c(new_n118_), .d(new_n155_), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n96_), .O(new_n157_));
  aoi21  g106(.a(new_n154_), .b(x33), .c(new_n157_), .O(new_n158_));
  inv1   g107(.a(x01), .O(new_n159_));
  aoi21  g108(.a(new_n56_), .b(new_n159_), .c(x18), .O(new_n160_));
  oai21  g109(.a(new_n158_), .b(new_n56_), .c(new_n160_), .O(z14));
  inv1   g110(.a(new_n96_), .O(new_n162_));
  nand2  g111(.a(new_n118_), .b(new_n155_), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n127_), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n149_), .c(new_n162_), .O(new_n165_));
  inv1   g114(.a(x00), .O(new_n166_));
  nand2  g115(.a(new_n56_), .b(new_n166_), .O(new_n167_));
  inv1   g116(.a(x34), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(x16), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n165_), .c(new_n167_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n62_), .O(new_n171_));
  oai21  g120(.a(new_n157_), .b(new_n56_), .c(new_n62_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x34), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z15));
endmodule


