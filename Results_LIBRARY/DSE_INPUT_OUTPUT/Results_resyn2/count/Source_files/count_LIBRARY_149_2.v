// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_;
  nand2  g000(.a(x29), .b(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x18), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nand2  g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  oai21  g010(.a(x19), .b(x17), .c(x20), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  aoi21  g012(.a(new_n56_), .b(x14), .c(x18), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n52_), .O(z01));
  and2   g015(.a(new_n61_), .b(x21), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x16), .O(new_n70_));
  aoi21  g019(.a(new_n56_), .b(x13), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n70_), .b(new_n67_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n52_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  nand3  g023(.a(new_n68_), .b(new_n53_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n69_), .b(x22), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n75_), .c(x16), .O(new_n77_));
  aoi21  g026(.a(new_n56_), .b(x12), .c(x18), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n52_), .O(z03));
  nor2   g029(.a(x23), .b(x22), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n68_), .c(new_n53_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  aoi21  g032(.a(new_n75_), .b(x23), .c(new_n83_), .O(new_n84_));
  inv1   g033(.a(x11), .O(new_n85_));
  aoi21  g034(.a(new_n56_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n56_), .c(new_n86_), .O(z04));
  nor2   g036(.a(x24), .b(x23), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n68_), .c(new_n53_), .d(new_n74_), .O(new_n89_));
  inv1   g038(.a(new_n89_), .O(new_n90_));
  aoi21  g039(.a(new_n82_), .b(x24), .c(new_n90_), .O(new_n91_));
  inv1   g040(.a(x10), .O(new_n92_));
  aoi21  g041(.a(new_n56_), .b(new_n92_), .c(x18), .O(new_n93_));
  oai21  g042(.a(new_n91_), .b(new_n56_), .c(new_n93_), .O(z05));
  inv1   g043(.a(x25), .O(new_n95_));
  nand2  g044(.a(new_n88_), .b(new_n95_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n75_), .O(new_n97_));
  aoi21  g046(.a(new_n89_), .b(x25), .c(new_n97_), .O(new_n98_));
  inv1   g047(.a(x09), .O(new_n99_));
  aoi21  g048(.a(new_n56_), .b(new_n99_), .c(x18), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n56_), .c(new_n100_), .O(z06));
  nor3   g050(.a(x25), .b(x24), .c(x23), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n68_), .c(new_n53_), .d(new_n74_), .O(new_n103_));
  xor2a  g052(.a(new_n103_), .b(x26), .O(new_n104_));
  inv1   g053(.a(x08), .O(new_n105_));
  aoi21  g054(.a(new_n56_), .b(new_n105_), .c(x18), .O(new_n106_));
  oai21  g055(.a(new_n104_), .b(new_n56_), .c(new_n106_), .O(z07));
  inv1   g056(.a(x26), .O(new_n108_));
  inv1   g057(.a(x27), .O(new_n109_));
  aoi21  g058(.a(new_n97_), .b(new_n108_), .c(new_n109_), .O(new_n110_));
  nand2  g059(.a(new_n109_), .b(new_n108_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n103_), .c(x16), .O(new_n112_));
  aoi21  g061(.a(new_n56_), .b(x07), .c(x18), .O(new_n113_));
  oai21  g062(.a(new_n112_), .b(new_n110_), .c(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n52_), .O(z08));
  inv1   g064(.a(x28), .O(new_n116_));
  inv1   g065(.a(new_n111_), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n97_), .c(new_n116_), .O(new_n118_));
  nand2  g067(.a(new_n117_), .b(new_n116_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n103_), .c(x16), .O(new_n120_));
  aoi21  g069(.a(new_n56_), .b(x06), .c(x18), .O(new_n121_));
  oai21  g070(.a(new_n120_), .b(new_n118_), .c(new_n121_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n52_), .O(z09));
  inv1   g072(.a(x29), .O(new_n124_));
  nand2  g073(.a(new_n56_), .b(x05), .O(new_n125_));
  inv1   g074(.a(new_n75_), .O(new_n126_));
  nor2   g075(.a(new_n111_), .b(x28), .O(new_n127_));
  nor2   g076(.a(new_n124_), .b(new_n56_), .O(new_n128_));
  nand4  g077(.a(new_n128_), .b(new_n127_), .c(new_n102_), .d(new_n126_), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n125_), .c(x18), .O(new_n130_));
  inv1   g079(.a(x18), .O(new_n131_));
  nand2  g080(.a(new_n120_), .b(new_n131_), .O(new_n132_));
  aoi21  g081(.a(new_n132_), .b(new_n124_), .c(new_n130_), .O(z10));
  inv1   g082(.a(x30), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n56_), .c(new_n131_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(x29), .O(new_n136_));
  aoi21  g085(.a(new_n127_), .b(new_n97_), .c(new_n134_), .O(new_n137_));
  nand3  g086(.a(new_n134_), .b(new_n116_), .c(new_n109_), .O(new_n138_));
  inv1   g087(.a(x24), .O(new_n139_));
  nand3  g088(.a(new_n108_), .b(new_n95_), .c(new_n139_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n83_), .c(new_n124_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(x16), .O(new_n143_));
  aoi21  g092(.a(new_n56_), .b(x04), .c(x18), .O(new_n144_));
  oai21  g093(.a(new_n143_), .b(new_n137_), .c(new_n144_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n136_), .O(z11));
  inv1   g095(.a(x31), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n56_), .c(new_n131_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x29), .O(new_n149_));
  nor2   g098(.a(x31), .b(x29), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n134_), .c(new_n116_), .O(new_n151_));
  inv1   g100(.a(new_n151_), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n117_), .c(new_n102_), .d(new_n126_), .O(new_n153_));
  nor3   g102(.a(x30), .b(x28), .c(x27), .O(new_n154_));
  nor3   g103(.a(x26), .b(x25), .c(x24), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n82_), .c(x31), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n153_), .c(x16), .O(new_n158_));
  aoi21  g107(.a(new_n56_), .b(x03), .c(x18), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n149_), .O(z12));
  nor3   g110(.a(x32), .b(x26), .c(x25), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n150_), .c(new_n154_), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n89_), .O(new_n164_));
  aoi21  g113(.a(new_n153_), .b(x32), .c(new_n164_), .O(new_n165_));
  inv1   g114(.a(x02), .O(new_n166_));
  aoi21  g115(.a(new_n56_), .b(new_n166_), .c(x18), .O(new_n167_));
  oai21  g116(.a(new_n165_), .b(new_n56_), .c(new_n167_), .O(z13));
  oai21  g117(.a(new_n163_), .b(new_n89_), .c(x33), .O(new_n169_));
  inv1   g118(.a(new_n150_), .O(new_n170_));
  inv1   g119(.a(x32), .O(new_n171_));
  inv1   g120(.a(x33), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n171_), .c(new_n134_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n170_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n127_), .c(new_n97_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n169_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  inv1   g126(.a(x01), .O(new_n178_));
  aoi21  g127(.a(new_n56_), .b(new_n178_), .c(x18), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z14));
  nand4  g129(.a(new_n174_), .b(new_n127_), .c(new_n97_), .d(x34), .O(new_n181_));
  inv1   g130(.a(x34), .O(new_n182_));
  nand2  g131(.a(new_n175_), .b(new_n182_), .O(new_n183_));
  nand3  g132(.a(new_n183_), .b(new_n181_), .c(x16), .O(new_n184_));
  inv1   g133(.a(x00), .O(new_n185_));
  aoi21  g134(.a(new_n56_), .b(new_n185_), .c(x18), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n184_), .O(z15));
endmodule


