// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_;
  inv1   g000(.a(x06), .O(new_n52_));
  nand2  g001(.a(x18), .b(new_n52_), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(x19), .b(x17), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x15), .c(x18), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n54_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n53_), .O(z00));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  inv1   g010(.a(x20), .O(new_n62_));
  oai21  g011(.a(new_n54_), .b(new_n62_), .c(x16), .O(new_n63_));
  aoi21  g012(.a(new_n57_), .b(x14), .c(x18), .O(new_n64_));
  oai21  g013(.a(new_n63_), .b(new_n61_), .c(new_n64_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n53_), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  nor2   g016(.a(new_n61_), .b(new_n67_), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n54_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x16), .O(new_n71_));
  aoi21  g020(.a(new_n57_), .b(x13), .c(x18), .O(new_n72_));
  oai21  g021(.a(new_n71_), .b(new_n68_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n53_), .O(z02));
  xor2a  g023(.a(new_n70_), .b(x22), .O(new_n75_));
  inv1   g024(.a(x12), .O(new_n76_));
  aoi21  g025(.a(new_n57_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n57_), .c(new_n77_), .O(z03));
  oai21  g027(.a(new_n70_), .b(x22), .c(x23), .O(new_n79_));
  nor2   g028(.a(x23), .b(x22), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n69_), .c(new_n54_), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n79_), .c(x16), .O(new_n82_));
  aoi21  g031(.a(new_n57_), .b(x11), .c(x18), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n53_), .O(z04));
  nand2  g034(.a(new_n81_), .b(x24), .O(new_n86_));
  nor2   g035(.a(x24), .b(x21), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n80_), .c(new_n54_), .d(new_n62_), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n86_), .c(x16), .O(new_n89_));
  aoi21  g038(.a(new_n57_), .b(x10), .c(x18), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n53_), .O(z05));
  nor2   g041(.a(x25), .b(x24), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n80_), .c(new_n69_), .d(new_n54_), .O(new_n94_));
  nand2  g043(.a(new_n88_), .b(x25), .O(new_n95_));
  nand3  g044(.a(new_n95_), .b(new_n94_), .c(x16), .O(new_n96_));
  aoi21  g045(.a(new_n57_), .b(x09), .c(x18), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n53_), .O(z06));
  nor2   g048(.a(new_n94_), .b(x26), .O(new_n100_));
  nand2  g049(.a(new_n94_), .b(x26), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x16), .O(new_n102_));
  aoi21  g051(.a(new_n57_), .b(x08), .c(x18), .O(new_n103_));
  oai21  g052(.a(new_n102_), .b(new_n100_), .c(new_n103_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n53_), .O(z07));
  oai21  g054(.a(new_n94_), .b(x26), .c(x27), .O(new_n106_));
  nor3   g055(.a(x27), .b(x26), .c(x25), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n87_), .c(new_n80_), .d(new_n61_), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n106_), .c(x16), .O(new_n109_));
  aoi21  g058(.a(new_n57_), .b(x07), .c(x18), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n53_), .O(z08));
  inv1   g061(.a(x18), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  inv1   g063(.a(x28), .O(new_n115_));
  inv1   g064(.a(new_n81_), .O(new_n116_));
  inv1   g065(.a(x24), .O(new_n117_));
  inv1   g066(.a(x27), .O(new_n118_));
  nor2   g067(.a(x26), .b(x25), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  inv1   g069(.a(new_n120_), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n116_), .c(new_n115_), .O(new_n122_));
  aoi21  g071(.a(new_n108_), .b(x28), .c(new_n114_), .O(new_n123_));
  aoi22  g072(.a(new_n123_), .b(new_n122_), .c(new_n114_), .d(x06), .O(z09));
  nand4  g073(.a(new_n80_), .b(new_n69_), .c(new_n54_), .d(new_n115_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n120_), .c(x29), .O(new_n126_));
  nor2   g075(.a(x29), .b(x28), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n121_), .c(new_n116_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  inv1   g079(.a(x05), .O(new_n131_));
  aoi21  g080(.a(new_n57_), .b(new_n131_), .c(x18), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n130_), .O(z10));
  nand4  g082(.a(new_n127_), .b(new_n80_), .c(new_n69_), .d(new_n54_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n120_), .c(x30), .O(new_n135_));
  nor3   g084(.a(x26), .b(x25), .c(x24), .O(new_n136_));
  nor2   g085(.a(x30), .b(x27), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n127_), .O(new_n138_));
  inv1   g087(.a(new_n138_), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n136_), .c(new_n116_), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n135_), .c(x16), .O(new_n141_));
  aoi21  g090(.a(new_n57_), .b(x04), .c(x18), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n53_), .O(z11));
  inv1   g093(.a(x22), .O(new_n145_));
  inv1   g094(.a(x23), .O(new_n146_));
  nand4  g095(.a(new_n117_), .b(new_n146_), .c(new_n145_), .d(new_n67_), .O(new_n147_));
  inv1   g096(.a(new_n147_), .O(new_n148_));
  inv1   g097(.a(x29), .O(new_n149_));
  inv1   g098(.a(x30), .O(new_n150_));
  inv1   g099(.a(x31), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n150_), .c(new_n149_), .d(new_n115_), .O(new_n152_));
  inv1   g101(.a(new_n152_), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n107_), .c(new_n148_), .d(new_n61_), .O(new_n154_));
  nand4  g103(.a(new_n137_), .b(new_n127_), .c(new_n119_), .d(new_n117_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n81_), .c(x31), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n154_), .c(x16), .O(new_n157_));
  aoi21  g106(.a(new_n57_), .b(x03), .c(x18), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n53_), .O(z12));
  nor2   g109(.a(x32), .b(x31), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n137_), .c(new_n127_), .d(new_n119_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n88_), .c(x16), .O(new_n163_));
  aoi21  g112(.a(new_n154_), .b(x32), .c(new_n163_), .O(new_n164_));
  inv1   g113(.a(x02), .O(new_n165_));
  oai21  g114(.a(x16), .b(new_n165_), .c(new_n113_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n164_), .c(new_n53_), .O(z13));
  oai21  g116(.a(new_n162_), .b(new_n88_), .c(x33), .O(new_n168_));
  inv1   g117(.a(new_n94_), .O(new_n169_));
  inv1   g118(.a(x26), .O(new_n170_));
  inv1   g119(.a(x32), .O(new_n171_));
  inv1   g120(.a(x33), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n171_), .c(new_n118_), .d(new_n170_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n152_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n169_), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n168_), .c(x16), .O(new_n176_));
  aoi21  g125(.a(new_n57_), .b(x01), .c(x18), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n53_), .O(z14));
  nand3  g128(.a(new_n174_), .b(new_n169_), .c(x34), .O(new_n180_));
  inv1   g129(.a(x34), .O(new_n181_));
  nand2  g130(.a(new_n175_), .b(new_n181_), .O(new_n182_));
  nand3  g131(.a(new_n182_), .b(new_n180_), .c(x16), .O(new_n183_));
  inv1   g132(.a(x00), .O(new_n184_));
  aoi21  g133(.a(new_n57_), .b(new_n184_), .c(x18), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n183_), .O(z15));
endmodule


