// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  nand2  g006(.a(x33), .b(x30), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g008(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  xnor2a g010(.a(new_n53_), .b(x20), .O(new_n62_));
  nor2   g011(.a(x16), .b(x14), .O(new_n63_));
  nor2   g012(.a(new_n63_), .b(new_n59_), .O(new_n64_));
  oai21  g013(.a(new_n62_), .b(new_n56_), .c(new_n64_), .O(z01));
  inv1   g014(.a(new_n58_), .O(new_n66_));
  inv1   g015(.a(x21), .O(new_n67_));
  nor3   g016(.a(x20), .b(x19), .c(x17), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g018(.a(new_n68_), .b(new_n67_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  oai21  g020(.a(new_n71_), .b(new_n69_), .c(x16), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n56_), .b(new_n73_), .c(x18), .O(new_n74_));
  aoi21  g023(.a(new_n74_), .b(new_n72_), .c(new_n66_), .O(z02));
  xor2a  g024(.a(new_n70_), .b(x22), .O(new_n76_));
  nor2   g025(.a(x16), .b(x12), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n59_), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n56_), .c(new_n78_), .O(z03));
  inv1   g028(.a(x22), .O(new_n80_));
  nand2  g029(.a(new_n71_), .b(new_n80_), .O(new_n81_));
  nor2   g030(.a(x21), .b(x20), .O(new_n82_));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n82_), .c(new_n53_), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  aoi21  g034(.a(new_n81_), .b(x23), .c(new_n85_), .O(new_n86_));
  nor2   g035(.a(x16), .b(x11), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n59_), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n56_), .c(new_n88_), .O(z04));
  inv1   g038(.a(x24), .O(new_n90_));
  nor2   g039(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  nand4  g040(.a(new_n83_), .b(new_n82_), .c(new_n53_), .d(new_n90_), .O(new_n92_));
  inv1   g041(.a(new_n92_), .O(new_n93_));
  oai21  g042(.a(new_n93_), .b(new_n91_), .c(x16), .O(new_n94_));
  nor2   g043(.a(x16), .b(x10), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n59_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n94_), .O(z05));
  inv1   g046(.a(x25), .O(new_n98_));
  nor2   g047(.a(x25), .b(x24), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n83_), .c(new_n82_), .d(new_n53_), .O(new_n100_));
  oai21  g049(.a(new_n93_), .b(new_n98_), .c(new_n100_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x16), .O(new_n102_));
  inv1   g051(.a(x09), .O(new_n103_));
  aoi21  g052(.a(new_n56_), .b(new_n103_), .c(x18), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n102_), .c(new_n66_), .O(z06));
  inv1   g054(.a(x26), .O(new_n106_));
  nand2  g055(.a(new_n99_), .b(new_n106_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n84_), .O(new_n108_));
  aoi21  g057(.a(new_n100_), .b(x26), .c(new_n108_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x08), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n59_), .O(new_n111_));
  oai21  g060(.a(new_n109_), .b(new_n56_), .c(new_n111_), .O(z07));
  xor2a  g061(.a(new_n108_), .b(x27), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  inv1   g063(.a(x07), .O(new_n115_));
  aoi21  g064(.a(new_n56_), .b(new_n115_), .c(x18), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n114_), .c(new_n66_), .O(z08));
  inv1   g066(.a(x27), .O(new_n118_));
  nand2  g067(.a(new_n108_), .b(new_n118_), .O(new_n119_));
  nor2   g068(.a(x28), .b(x27), .O(new_n120_));
  aoi22  g069(.a(new_n120_), .b(new_n108_), .c(new_n119_), .d(x28), .O(new_n121_));
  nor2   g070(.a(x16), .b(x06), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n59_), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n56_), .c(new_n123_), .O(z09));
  inv1   g073(.a(x29), .O(new_n125_));
  aoi21  g074(.a(new_n120_), .b(new_n108_), .c(new_n125_), .O(new_n126_));
  nand4  g075(.a(new_n120_), .b(new_n99_), .c(new_n125_), .d(new_n106_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n84_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n126_), .c(x16), .O(new_n129_));
  inv1   g078(.a(x05), .O(new_n130_));
  aoi21  g079(.a(new_n56_), .b(new_n130_), .c(x18), .O(new_n131_));
  aoi21  g080(.a(new_n131_), .b(new_n129_), .c(new_n66_), .O(z10));
  xor2a  g081(.a(new_n128_), .b(x30), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  inv1   g083(.a(x04), .O(new_n135_));
  aoi21  g084(.a(new_n56_), .b(new_n135_), .c(x18), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n134_), .c(new_n66_), .O(z11));
  oai21  g086(.a(new_n127_), .b(new_n84_), .c(x31), .O(new_n138_));
  inv1   g087(.a(new_n107_), .O(new_n139_));
  nor2   g088(.a(x31), .b(x29), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n120_), .O(new_n141_));
  inv1   g090(.a(new_n141_), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n139_), .c(new_n85_), .O(new_n143_));
  aoi21  g092(.a(new_n143_), .b(new_n138_), .c(x30), .O(new_n144_));
  nand2  g093(.a(x31), .b(x30), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(x33), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  nand2  g096(.a(new_n145_), .b(x18), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x16), .O(new_n149_));
  nand2  g098(.a(new_n57_), .b(x03), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n149_), .c(new_n58_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n147_), .O(z12));
  nand4  g101(.a(new_n140_), .b(new_n120_), .c(new_n99_), .d(new_n106_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n84_), .c(x32), .O(new_n154_));
  inv1   g103(.a(x28), .O(new_n155_));
  inv1   g104(.a(x32), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n155_), .c(new_n118_), .O(new_n157_));
  inv1   g106(.a(x31), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n125_), .c(new_n106_), .d(new_n98_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n93_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n154_), .c(x30), .O(new_n162_));
  nand2  g111(.a(x32), .b(x30), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(x33), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n162_), .c(x16), .O(new_n165_));
  nand2  g114(.a(new_n163_), .b(x18), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(x16), .O(new_n167_));
  nand2  g116(.a(new_n57_), .b(x02), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n167_), .c(new_n58_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n165_), .O(z13));
  nand2  g119(.a(new_n161_), .b(x33), .O(new_n171_));
  inv1   g120(.a(new_n100_), .O(new_n172_));
  nand4  g121(.a(new_n125_), .b(new_n155_), .c(new_n118_), .d(new_n106_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  nor2   g123(.a(x31), .b(x30), .O(new_n175_));
  nor2   g124(.a(x33), .b(x32), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n175_), .c(new_n174_), .d(new_n172_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n171_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x16), .O(new_n179_));
  nor2   g128(.a(x16), .b(x01), .O(new_n180_));
  nor2   g129(.a(new_n180_), .b(new_n59_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n179_), .O(z14));
  nand2  g131(.a(x34), .b(x30), .O(new_n183_));
  inv1   g132(.a(x30), .O(new_n184_));
  inv1   g133(.a(x34), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n156_), .c(new_n158_), .d(new_n184_), .O(new_n186_));
  nor2   g135(.a(new_n186_), .b(new_n173_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n172_), .O(new_n188_));
  aoi21  g137(.a(new_n188_), .b(new_n183_), .c(x33), .O(new_n189_));
  nor2   g138(.a(new_n153_), .b(new_n84_), .O(new_n190_));
  nand2  g139(.a(x34), .b(new_n184_), .O(new_n191_));
  aoi21  g140(.a(new_n176_), .b(new_n190_), .c(new_n191_), .O(new_n192_));
  oai21  g141(.a(new_n192_), .b(new_n189_), .c(x16), .O(new_n193_));
  nand2  g142(.a(new_n183_), .b(x18), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(x16), .O(new_n195_));
  nand2  g144(.a(new_n57_), .b(x00), .O(new_n196_));
  nand3  g145(.a(new_n196_), .b(new_n195_), .c(new_n58_), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n193_), .O(z15));
endmodule


