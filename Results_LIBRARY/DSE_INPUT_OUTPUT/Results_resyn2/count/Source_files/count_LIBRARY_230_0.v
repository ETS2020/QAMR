// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x12), .O(new_n58_));
  aoi21  g007(.a(new_n56_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n54_), .O(z00));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  inv1   g010(.a(x20), .O(new_n62_));
  oai21  g011(.a(new_n53_), .b(new_n62_), .c(x16), .O(new_n63_));
  inv1   g012(.a(x12), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n63_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n57_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  xor2a  g017(.a(new_n61_), .b(new_n68_), .O(new_n69_));
  nor2   g018(.a(x16), .b(x13), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n56_), .c(new_n71_), .O(z02));
  nand2  g021(.a(new_n61_), .b(new_n68_), .O(new_n73_));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  aoi22  g023(.a(new_n74_), .b(new_n61_), .c(new_n73_), .d(x22), .O(new_n75_));
  nand2  g024(.a(x16), .b(x12), .O(new_n76_));
  oai21  g025(.a(new_n76_), .b(new_n75_), .c(new_n57_), .O(z03));
  inv1   g026(.a(x23), .O(new_n78_));
  aoi21  g027(.a(new_n74_), .b(new_n61_), .c(new_n78_), .O(new_n79_));
  nand2  g028(.a(new_n53_), .b(new_n62_), .O(new_n80_));
  nand2  g029(.a(new_n74_), .b(new_n78_), .O(new_n81_));
  nor2   g030(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n79_), .c(x16), .O(new_n83_));
  nor2   g032(.a(x16), .b(x11), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n58_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n83_), .O(z04));
  inv1   g035(.a(x24), .O(new_n87_));
  nand2  g036(.a(new_n82_), .b(new_n87_), .O(new_n88_));
  nor3   g037(.a(x23), .b(x22), .c(x21), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n61_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x24), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n88_), .c(x16), .O(new_n92_));
  aoi21  g041(.a(new_n56_), .b(x10), .c(new_n64_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n57_), .O(z05));
  inv1   g044(.a(x25), .O(new_n96_));
  nand4  g045(.a(new_n89_), .b(new_n61_), .c(new_n96_), .d(new_n87_), .O(new_n97_));
  oai21  g046(.a(new_n90_), .b(x24), .c(x25), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n97_), .c(x16), .O(new_n99_));
  aoi21  g048(.a(new_n56_), .b(x09), .c(new_n64_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n57_), .O(z06));
  nor3   g051(.a(x26), .b(x25), .c(x24), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n89_), .c(new_n61_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  aoi21  g054(.a(new_n97_), .b(x26), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x08), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n58_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n56_), .c(new_n108_), .O(z07));
  xor2a  g058(.a(new_n104_), .b(x27), .O(new_n110_));
  nor2   g059(.a(x16), .b(x07), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n58_), .O(new_n112_));
  oai21  g061(.a(new_n110_), .b(new_n56_), .c(new_n112_), .O(z08));
  inv1   g062(.a(x27), .O(new_n114_));
  nand4  g063(.a(new_n103_), .b(new_n89_), .c(new_n61_), .d(new_n114_), .O(new_n115_));
  nor2   g064(.a(x28), .b(x27), .O(new_n116_));
  aoi22  g065(.a(new_n116_), .b(new_n105_), .c(new_n115_), .d(x28), .O(new_n117_));
  nor2   g066(.a(x16), .b(x06), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n58_), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n56_), .c(new_n119_), .O(z09));
  nand3  g069(.a(new_n116_), .b(new_n103_), .c(new_n82_), .O(new_n121_));
  xor2a  g070(.a(new_n121_), .b(x29), .O(new_n122_));
  nor2   g071(.a(x16), .b(x05), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n58_), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n56_), .c(new_n124_), .O(z10));
  inv1   g074(.a(x29), .O(new_n126_));
  nand4  g075(.a(new_n116_), .b(new_n103_), .c(new_n82_), .d(new_n126_), .O(new_n127_));
  nor2   g076(.a(x30), .b(x29), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n116_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n104_), .c(x16), .O(new_n130_));
  aoi21  g079(.a(new_n127_), .b(x30), .c(new_n130_), .O(new_n131_));
  inv1   g080(.a(x04), .O(new_n132_));
  oai21  g081(.a(x16), .b(new_n132_), .c(x12), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n131_), .c(new_n57_), .O(z11));
  nor2   g083(.a(x31), .b(x28), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n128_), .O(new_n136_));
  inv1   g085(.a(new_n136_), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n103_), .c(new_n82_), .d(new_n114_), .O(new_n138_));
  oai21  g087(.a(new_n129_), .b(new_n104_), .c(x31), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n138_), .c(x16), .O(new_n140_));
  aoi21  g089(.a(new_n56_), .b(x03), .c(new_n64_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n57_), .O(z12));
  oai21  g092(.a(new_n136_), .b(new_n115_), .c(x32), .O(new_n144_));
  inv1   g093(.a(new_n129_), .O(new_n145_));
  nor2   g094(.a(x32), .b(x31), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n145_), .c(new_n103_), .d(new_n82_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x16), .O(new_n149_));
  nor2   g098(.a(x16), .b(x02), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n58_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n149_), .O(z13));
  nor2   g101(.a(x27), .b(x26), .O(new_n153_));
  nor2   g102(.a(x33), .b(x32), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n153_), .c(new_n135_), .d(new_n128_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n97_), .O(new_n156_));
  aoi21  g105(.a(new_n147_), .b(x33), .c(new_n156_), .O(new_n157_));
  nor2   g106(.a(x16), .b(x01), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n58_), .O(new_n159_));
  oai21  g108(.a(new_n157_), .b(new_n56_), .c(new_n159_), .O(z14));
  nand2  g109(.a(new_n156_), .b(x34), .O(new_n161_));
  inv1   g110(.a(x34), .O(new_n162_));
  oai21  g111(.a(new_n155_), .b(new_n97_), .c(new_n162_), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n161_), .c(x16), .O(new_n164_));
  nor2   g113(.a(x16), .b(x00), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n58_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n164_), .O(z15));
endmodule


