// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g009(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g010(.a(x14), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n61_), .O(z01));
  nand2  g013(.a(new_n58_), .b(new_n57_), .O(new_n65_));
  nor2   g014(.a(x21), .b(x20), .O(new_n66_));
  aoi22  g015(.a(new_n66_), .b(new_n58_), .c(new_n65_), .d(x21), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n52_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n52_), .c(new_n69_), .O(z02));
  inv1   g019(.a(x32), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x18), .O(new_n72_));
  inv1   g021(.a(x22), .O(new_n73_));
  nand3  g022(.a(new_n66_), .b(new_n58_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n66_), .b(new_n58_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x22), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n74_), .c(x16), .O(new_n77_));
  aoi21  g026(.a(new_n52_), .b(x12), .c(x18), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n72_), .O(z03));
  nor2   g029(.a(x23), .b(x22), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n66_), .c(new_n58_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  aoi21  g032(.a(new_n74_), .b(x23), .c(new_n83_), .O(new_n84_));
  inv1   g033(.a(x11), .O(new_n85_));
  aoi21  g034(.a(new_n52_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n52_), .c(new_n86_), .O(z04));
  nand2  g036(.a(new_n82_), .b(x24), .O(new_n88_));
  nor2   g037(.a(x24), .b(x21), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n81_), .c(new_n58_), .d(new_n57_), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n88_), .c(x16), .O(new_n91_));
  aoi21  g040(.a(new_n52_), .b(x10), .c(x18), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n72_), .O(z05));
  nor2   g043(.a(x25), .b(x24), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n81_), .c(new_n66_), .d(new_n58_), .O(new_n96_));
  nand2  g045(.a(new_n90_), .b(x25), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n96_), .c(x16), .O(new_n98_));
  aoi21  g047(.a(new_n52_), .b(x09), .c(x18), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n72_), .O(z06));
  inv1   g050(.a(x26), .O(new_n102_));
  inv1   g051(.a(new_n96_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nor3   g053(.a(new_n96_), .b(x26), .c(x18), .O(new_n105_));
  oai21  g054(.a(new_n105_), .b(new_n104_), .c(x16), .O(new_n106_));
  inv1   g055(.a(x08), .O(new_n107_));
  aoi21  g056(.a(new_n52_), .b(new_n107_), .c(x18), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(z07));
  oai21  g058(.a(new_n96_), .b(x26), .c(x27), .O(new_n110_));
  inv1   g059(.a(new_n90_), .O(new_n111_));
  inv1   g060(.a(x25), .O(new_n112_));
  nor2   g061(.a(x27), .b(x26), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  inv1   g063(.a(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n111_), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n110_), .c(x16), .O(new_n117_));
  aoi21  g066(.a(new_n52_), .b(x07), .c(x18), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n72_), .O(z08));
  oai21  g069(.a(new_n114_), .b(new_n90_), .c(x28), .O(new_n121_));
  inv1   g070(.a(x27), .O(new_n122_));
  inv1   g071(.a(x28), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n122_), .c(new_n102_), .d(new_n112_), .O(new_n124_));
  inv1   g073(.a(new_n124_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n111_), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n121_), .c(x16), .O(new_n127_));
  aoi21  g076(.a(new_n52_), .b(x06), .c(x18), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n72_), .O(z09));
  inv1   g079(.a(x29), .O(new_n131_));
  nor2   g080(.a(new_n124_), .b(new_n90_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nor2   g082(.a(x29), .b(x28), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n113_), .c(new_n95_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n82_), .c(x16), .O(new_n136_));
  aoi21  g085(.a(new_n52_), .b(x05), .c(x18), .O(new_n137_));
  oai21  g086(.a(new_n136_), .b(new_n133_), .c(new_n137_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n72_), .O(z10));
  oai21  g088(.a(new_n135_), .b(new_n82_), .c(x30), .O(new_n140_));
  inv1   g089(.a(x30), .O(new_n141_));
  inv1   g090(.a(new_n135_), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n83_), .c(new_n141_), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n140_), .c(x16), .O(new_n144_));
  aoi21  g093(.a(new_n52_), .b(x04), .c(x18), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n72_), .O(z11));
  nor3   g096(.a(x31), .b(x30), .c(x29), .O(new_n148_));
  aoi22  g097(.a(new_n148_), .b(new_n132_), .c(new_n143_), .d(x31), .O(new_n149_));
  inv1   g098(.a(x03), .O(new_n150_));
  aoi21  g099(.a(new_n52_), .b(new_n150_), .c(x18), .O(new_n151_));
  oai21  g100(.a(new_n149_), .b(new_n52_), .c(new_n151_), .O(z12));
  inv1   g101(.a(x21), .O(new_n153_));
  inv1   g102(.a(x23), .O(new_n154_));
  inv1   g103(.a(x24), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n154_), .c(new_n73_), .d(new_n153_), .O(new_n156_));
  inv1   g105(.a(new_n156_), .O(new_n157_));
  nand4  g106(.a(new_n148_), .b(new_n125_), .c(new_n157_), .d(new_n60_), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(x32), .c(new_n52_), .O(new_n159_));
  inv1   g108(.a(x02), .O(new_n160_));
  inv1   g109(.a(x18), .O(new_n161_));
  oai21  g110(.a(x16), .b(new_n160_), .c(new_n161_), .O(new_n162_));
  inv1   g111(.a(x31), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n141_), .c(new_n131_), .d(new_n123_), .O(new_n164_));
  inv1   g113(.a(new_n164_), .O(new_n165_));
  nand2  g114(.a(new_n58_), .b(x16), .O(new_n166_));
  nand4  g115(.a(new_n113_), .b(new_n95_), .c(new_n81_), .d(new_n66_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n165_), .c(x18), .O(new_n169_));
  oai22  g118(.a(new_n169_), .b(x32), .c(new_n162_), .d(new_n159_), .O(z13));
  nand4  g119(.a(new_n71_), .b(new_n163_), .c(new_n141_), .d(new_n131_), .O(new_n171_));
  inv1   g120(.a(new_n171_), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n125_), .c(new_n157_), .d(new_n60_), .O(new_n173_));
  inv1   g122(.a(x33), .O(new_n174_));
  nand3  g123(.a(new_n113_), .b(new_n174_), .c(new_n71_), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n164_), .O(new_n176_));
  aoi22  g125(.a(new_n176_), .b(new_n103_), .c(new_n173_), .d(x33), .O(new_n177_));
  inv1   g126(.a(x01), .O(new_n178_));
  aoi21  g127(.a(new_n52_), .b(new_n178_), .c(x18), .O(new_n179_));
  oai21  g128(.a(new_n177_), .b(new_n52_), .c(new_n179_), .O(z14));
  nand2  g129(.a(x34), .b(x16), .O(new_n181_));
  aoi21  g130(.a(new_n176_), .b(new_n103_), .c(new_n181_), .O(new_n182_));
  nor2   g131(.a(x16), .b(x00), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n182_), .c(new_n161_), .O(new_n184_));
  inv1   g133(.a(x34), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n174_), .c(new_n57_), .O(new_n186_));
  nor2   g135(.a(new_n186_), .b(new_n156_), .O(new_n187_));
  nor2   g136(.a(new_n166_), .b(new_n114_), .O(new_n188_));
  nand3  g137(.a(new_n188_), .b(new_n187_), .c(new_n165_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n161_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n71_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n184_), .O(z15));
endmodule


