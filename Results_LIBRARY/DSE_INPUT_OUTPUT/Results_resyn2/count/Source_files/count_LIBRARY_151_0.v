// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x30), .O(new_n58_));
  inv1   g007(.a(x33), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  aoi21  g010(.a(new_n56_), .b(new_n55_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n54_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n53_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(z01));
  inv1   g019(.a(new_n60_), .O(new_n71_));
  inv1   g020(.a(x21), .O(new_n72_));
  nor2   g021(.a(new_n66_), .b(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n66_), .b(new_n72_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(new_n73_), .c(x16), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n56_), .b(new_n77_), .c(x18), .O(new_n78_));
  aoi21  g027(.a(new_n78_), .b(new_n76_), .c(new_n71_), .O(z02));
  xor2a  g028(.a(new_n74_), .b(x22), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  nor2   g030(.a(new_n81_), .b(new_n61_), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n56_), .c(new_n82_), .O(z03));
  inv1   g032(.a(x22), .O(new_n84_));
  nand2  g033(.a(new_n75_), .b(new_n84_), .O(new_n85_));
  nor2   g034(.a(x21), .b(x20), .O(new_n86_));
  nor2   g035(.a(x23), .b(x22), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n86_), .c(new_n53_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  aoi21  g038(.a(new_n85_), .b(x23), .c(new_n89_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n61_), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n56_), .c(new_n92_), .O(z04));
  xor2a  g042(.a(new_n88_), .b(x24), .O(new_n94_));
  nor2   g043(.a(x16), .b(x10), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n61_), .O(new_n96_));
  oai21  g045(.a(new_n94_), .b(new_n56_), .c(new_n96_), .O(z05));
  inv1   g046(.a(x24), .O(new_n98_));
  nand4  g047(.a(new_n87_), .b(new_n86_), .c(new_n53_), .d(new_n98_), .O(new_n99_));
  and2   g048(.a(new_n99_), .b(x25), .O(new_n100_));
  nor2   g049(.a(new_n99_), .b(x25), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n100_), .c(x16), .O(new_n102_));
  inv1   g051(.a(x09), .O(new_n103_));
  aoi21  g052(.a(new_n56_), .b(new_n103_), .c(x18), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(new_n102_), .c(new_n71_), .O(z06));
  inv1   g054(.a(x26), .O(new_n106_));
  nor2   g055(.a(new_n101_), .b(new_n106_), .O(new_n107_));
  nor2   g056(.a(x25), .b(x24), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n88_), .O(new_n110_));
  oai21  g059(.a(new_n110_), .b(new_n107_), .c(x16), .O(new_n111_));
  nor2   g060(.a(x16), .b(x08), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n61_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n111_), .O(z07));
  xor2a  g063(.a(new_n110_), .b(x27), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  inv1   g065(.a(x07), .O(new_n117_));
  aoi21  g066(.a(new_n56_), .b(new_n117_), .c(x18), .O(new_n118_));
  aoi21  g067(.a(new_n118_), .b(new_n116_), .c(new_n71_), .O(z08));
  inv1   g068(.a(x27), .O(new_n120_));
  nand2  g069(.a(new_n110_), .b(new_n120_), .O(new_n121_));
  nand4  g070(.a(new_n108_), .b(new_n87_), .c(new_n86_), .d(new_n106_), .O(new_n122_));
  nor2   g071(.a(x28), .b(x27), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n53_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  aoi21  g074(.a(new_n121_), .b(x28), .c(new_n125_), .O(new_n126_));
  nor2   g075(.a(x16), .b(x06), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n61_), .O(new_n128_));
  oai21  g077(.a(new_n126_), .b(new_n56_), .c(new_n128_), .O(z09));
  inv1   g078(.a(x29), .O(new_n130_));
  nor2   g079(.a(new_n125_), .b(new_n130_), .O(new_n131_));
  nand3  g080(.a(new_n123_), .b(new_n53_), .c(new_n130_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n122_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n131_), .c(x16), .O(new_n134_));
  inv1   g083(.a(x05), .O(new_n135_));
  aoi21  g084(.a(new_n56_), .b(new_n135_), .c(x18), .O(new_n136_));
  aoi21  g085(.a(new_n136_), .b(new_n134_), .c(new_n71_), .O(z10));
  oai21  g086(.a(x16), .b(x04), .c(new_n57_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n60_), .O(new_n139_));
  inv1   g088(.a(new_n109_), .O(new_n140_));
  inv1   g089(.a(x28), .O(new_n141_));
  nand4  g090(.a(x33), .b(new_n58_), .c(new_n130_), .d(new_n141_), .O(new_n142_));
  inv1   g091(.a(new_n142_), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n140_), .c(new_n89_), .d(new_n120_), .O(new_n144_));
  oai21  g093(.a(new_n133_), .b(new_n58_), .c(new_n144_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(x16), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n139_), .O(z11));
  oai21  g096(.a(x16), .b(x03), .c(new_n57_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n60_), .O(new_n149_));
  inv1   g098(.a(x31), .O(new_n150_));
  oai21  g099(.a(new_n132_), .b(new_n122_), .c(x33), .O(new_n151_));
  aoi21  g100(.a(new_n151_), .b(new_n58_), .c(new_n150_), .O(new_n152_));
  nand4  g101(.a(x33), .b(new_n150_), .c(new_n58_), .d(new_n130_), .O(new_n153_));
  nor3   g102(.a(new_n153_), .b(new_n124_), .c(new_n122_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n152_), .c(x16), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n149_), .O(z12));
  oai21  g105(.a(x16), .b(x02), .c(new_n57_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n60_), .O(new_n158_));
  inv1   g107(.a(x32), .O(new_n159_));
  nand4  g108(.a(new_n123_), .b(new_n53_), .c(new_n150_), .d(new_n130_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n122_), .c(x33), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n58_), .c(new_n159_), .O(new_n162_));
  nand2  g111(.a(new_n159_), .b(new_n150_), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n144_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n162_), .c(x16), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n158_), .O(z13));
  nor4   g115(.a(new_n163_), .b(new_n132_), .c(new_n122_), .d(x30), .O(new_n167_));
  nand2  g116(.a(x33), .b(x16), .O(new_n168_));
  nor2   g117(.a(x16), .b(x01), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n61_), .O(new_n170_));
  oai21  g119(.a(new_n168_), .b(new_n167_), .c(new_n170_), .O(z14));
  nand2  g120(.a(x34), .b(x16), .O(new_n172_));
  inv1   g121(.a(x00), .O(new_n173_));
  aoi21  g122(.a(new_n56_), .b(new_n173_), .c(x18), .O(new_n174_));
  aoi21  g123(.a(new_n174_), .b(new_n172_), .c(new_n71_), .O(z15));
endmodule


