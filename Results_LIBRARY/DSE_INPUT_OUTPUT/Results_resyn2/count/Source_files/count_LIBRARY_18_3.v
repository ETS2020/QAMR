// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x33), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x18), .O(new_n58_));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(x19), .b(x17), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  inv1   g010(.a(new_n61_), .O(new_n62_));
  oai21  g011(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n63_));
  aoi21  g012(.a(new_n52_), .b(x14), .c(x18), .O(new_n64_));
  oai21  g013(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n58_), .O(z01));
  xor2a  g015(.a(new_n61_), .b(x21), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n52_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n52_), .c(new_n69_), .O(z02));
  inv1   g019(.a(x21), .O(new_n71_));
  nand2  g020(.a(new_n62_), .b(new_n71_), .O(new_n72_));
  nor2   g021(.a(x22), .b(x21), .O(new_n73_));
  aoi22  g022(.a(new_n73_), .b(new_n62_), .c(new_n72_), .d(x22), .O(new_n74_));
  inv1   g023(.a(x12), .O(new_n75_));
  aoi21  g024(.a(new_n52_), .b(new_n75_), .c(x18), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n52_), .c(new_n76_), .O(z03));
  inv1   g026(.a(x18), .O(new_n78_));
  nand2  g027(.a(new_n52_), .b(x11), .O(new_n79_));
  inv1   g028(.a(x23), .O(new_n80_));
  nand4  g029(.a(new_n73_), .b(new_n60_), .c(new_n80_), .d(new_n59_), .O(new_n81_));
  nand3  g030(.a(new_n73_), .b(new_n60_), .c(new_n59_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(x23), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n81_), .c(x16), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n79_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n78_), .O(z04));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n73_), .c(new_n60_), .d(new_n59_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  aoi21  g038(.a(new_n81_), .b(x24), .c(new_n89_), .O(new_n90_));
  inv1   g039(.a(x10), .O(new_n91_));
  aoi21  g040(.a(new_n52_), .b(new_n91_), .c(x18), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n52_), .c(new_n92_), .O(z05));
  inv1   g042(.a(x25), .O(new_n94_));
  nand2  g043(.a(new_n87_), .b(new_n94_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n82_), .O(new_n96_));
  aoi21  g045(.a(new_n88_), .b(x25), .c(new_n96_), .O(new_n97_));
  inv1   g046(.a(x09), .O(new_n98_));
  aoi21  g047(.a(new_n52_), .b(new_n98_), .c(x18), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n52_), .c(new_n99_), .O(z06));
  inv1   g049(.a(x24), .O(new_n101_));
  inv1   g050(.a(x26), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n94_), .c(new_n101_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n81_), .O(new_n104_));
  oai21  g053(.a(new_n96_), .b(new_n102_), .c(x16), .O(new_n105_));
  aoi21  g054(.a(new_n52_), .b(x08), .c(x18), .O(new_n106_));
  oai21  g055(.a(new_n105_), .b(new_n104_), .c(new_n106_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n58_), .O(z07));
  inv1   g057(.a(new_n81_), .O(new_n109_));
  nor3   g058(.a(x26), .b(x25), .c(x24), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nor3   g060(.a(x27), .b(x26), .c(x25), .O(new_n112_));
  aoi22  g061(.a(new_n112_), .b(new_n89_), .c(new_n111_), .d(x27), .O(new_n113_));
  inv1   g062(.a(x07), .O(new_n114_));
  aoi21  g063(.a(new_n52_), .b(new_n114_), .c(x18), .O(new_n115_));
  oai21  g064(.a(new_n113_), .b(new_n52_), .c(new_n115_), .O(z08));
  inv1   g065(.a(x28), .O(new_n117_));
  aoi21  g066(.a(new_n112_), .b(new_n89_), .c(new_n117_), .O(new_n118_));
  inv1   g067(.a(x27), .O(new_n119_));
  nand4  g068(.a(new_n117_), .b(new_n119_), .c(new_n102_), .d(new_n94_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n88_), .c(x16), .O(new_n121_));
  aoi21  g070(.a(new_n52_), .b(x06), .c(x18), .O(new_n122_));
  oai21  g071(.a(new_n121_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n58_), .O(z09));
  nor3   g073(.a(x29), .b(x28), .c(x27), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n104_), .O(new_n126_));
  oai21  g075(.a(new_n120_), .b(new_n88_), .c(x29), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n126_), .c(x16), .O(new_n128_));
  aoi21  g077(.a(new_n52_), .b(x05), .c(x18), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n58_), .O(z10));
  inv1   g080(.a(x30), .O(new_n132_));
  nand3  g081(.a(new_n125_), .b(new_n110_), .c(new_n132_), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n81_), .O(new_n134_));
  aoi21  g083(.a(new_n126_), .b(x30), .c(new_n134_), .O(new_n135_));
  inv1   g084(.a(x04), .O(new_n136_));
  aoi21  g085(.a(new_n52_), .b(new_n136_), .c(x18), .O(new_n137_));
  oai21  g086(.a(new_n135_), .b(new_n52_), .c(new_n137_), .O(z11));
  inv1   g087(.a(x31), .O(new_n139_));
  inv1   g088(.a(x29), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n117_), .c(new_n119_), .O(new_n141_));
  nor3   g090(.a(new_n141_), .b(new_n103_), .c(x30), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n109_), .c(new_n139_), .O(new_n143_));
  nand4  g092(.a(new_n139_), .b(new_n132_), .c(new_n140_), .d(new_n117_), .O(new_n144_));
  inv1   g093(.a(new_n144_), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n112_), .c(new_n89_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(x16), .O(new_n147_));
  aoi21  g096(.a(new_n52_), .b(x03), .c(x18), .O(new_n148_));
  oai21  g097(.a(new_n147_), .b(new_n143_), .c(new_n148_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n58_), .O(z12));
  nor2   g099(.a(new_n120_), .b(new_n88_), .O(new_n151_));
  inv1   g100(.a(x32), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n139_), .c(new_n132_), .d(new_n140_), .O(new_n153_));
  inv1   g102(.a(new_n153_), .O(new_n154_));
  aoi22  g103(.a(new_n154_), .b(new_n151_), .c(new_n146_), .d(x32), .O(new_n155_));
  inv1   g104(.a(x02), .O(new_n156_));
  aoi21  g105(.a(new_n52_), .b(new_n156_), .c(x18), .O(new_n157_));
  oai21  g106(.a(new_n155_), .b(new_n52_), .c(new_n157_), .O(z13));
  nand3  g107(.a(new_n154_), .b(new_n151_), .c(x16), .O(new_n159_));
  aoi21  g108(.a(new_n159_), .b(new_n78_), .c(new_n57_), .O(new_n160_));
  nor3   g109(.a(new_n153_), .b(new_n120_), .c(new_n88_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(x33), .c(x16), .O(new_n162_));
  inv1   g111(.a(x01), .O(new_n163_));
  aoi21  g112(.a(new_n52_), .b(new_n163_), .c(x18), .O(new_n164_));
  aoi21  g113(.a(new_n164_), .b(new_n162_), .c(new_n160_), .O(z14));
  inv1   g114(.a(x34), .O(new_n166_));
  nand4  g115(.a(new_n154_), .b(new_n151_), .c(new_n166_), .d(x16), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n78_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n57_), .O(new_n169_));
  nand4  g118(.a(new_n57_), .b(new_n152_), .c(new_n119_), .d(new_n102_), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n144_), .O(new_n171_));
  nand2  g120(.a(x34), .b(x16), .O(new_n172_));
  aoi21  g121(.a(new_n171_), .b(new_n96_), .c(new_n172_), .O(new_n173_));
  nor2   g122(.a(x16), .b(x00), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n173_), .c(new_n78_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n169_), .O(z15));
endmodule


