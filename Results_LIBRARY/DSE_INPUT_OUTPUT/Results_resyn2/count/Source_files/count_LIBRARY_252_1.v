// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x29), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g008(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n53_), .b(new_n62_), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  nor2   g015(.a(new_n66_), .b(new_n59_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n65_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n53_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  aoi21  g022(.a(new_n56_), .b(x13), .c(x29), .O(new_n74_));
  oai21  g023(.a(new_n73_), .b(new_n70_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n57_), .O(z02));
  inv1   g025(.a(x22), .O(new_n77_));
  nand3  g026(.a(new_n71_), .b(new_n53_), .c(new_n77_), .O(new_n78_));
  nand2  g027(.a(new_n72_), .b(x22), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n78_), .c(x16), .O(new_n80_));
  aoi21  g029(.a(new_n56_), .b(x12), .c(x29), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n57_), .O(z03));
  nor2   g032(.a(x23), .b(x22), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n71_), .c(new_n53_), .O(new_n85_));
  inv1   g034(.a(new_n85_), .O(new_n86_));
  aoi21  g035(.a(new_n78_), .b(x23), .c(new_n86_), .O(new_n87_));
  nor2   g036(.a(x16), .b(x11), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n59_), .O(new_n89_));
  oai21  g038(.a(new_n87_), .b(new_n56_), .c(new_n89_), .O(z04));
  inv1   g039(.a(x23), .O(new_n91_));
  inv1   g040(.a(x24), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n91_), .c(new_n77_), .d(new_n69_), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  aoi22  g043(.a(new_n94_), .b(new_n64_), .c(new_n85_), .d(x24), .O(new_n95_));
  nor2   g044(.a(x16), .b(x10), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n59_), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n56_), .c(new_n97_), .O(z05));
  nor2   g047(.a(x24), .b(x21), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n84_), .c(new_n53_), .d(new_n62_), .O(new_n100_));
  nor2   g049(.a(x25), .b(x24), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n84_), .c(new_n71_), .d(new_n53_), .O(new_n102_));
  inv1   g051(.a(new_n102_), .O(new_n103_));
  aoi21  g052(.a(new_n100_), .b(x25), .c(new_n103_), .O(new_n104_));
  nor2   g053(.a(x16), .b(x09), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n59_), .O(new_n106_));
  oai21  g055(.a(new_n104_), .b(new_n56_), .c(new_n106_), .O(z06));
  inv1   g056(.a(x26), .O(new_n108_));
  nand2  g057(.a(new_n101_), .b(new_n108_), .O(new_n109_));
  inv1   g058(.a(new_n109_), .O(new_n110_));
  aoi22  g059(.a(new_n110_), .b(new_n86_), .c(new_n102_), .d(x26), .O(new_n111_));
  nor2   g060(.a(x16), .b(x08), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n59_), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n56_), .c(new_n113_), .O(z07));
  oai21  g063(.a(new_n109_), .b(new_n85_), .c(x27), .O(new_n115_));
  inv1   g064(.a(x25), .O(new_n116_));
  inv1   g065(.a(x27), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n108_), .c(new_n116_), .O(new_n118_));
  inv1   g067(.a(new_n118_), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n94_), .c(new_n64_), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n115_), .c(x16), .O(new_n121_));
  aoi21  g070(.a(new_n56_), .b(x07), .c(x29), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n57_), .O(z08));
  inv1   g073(.a(new_n59_), .O(new_n125_));
  nand2  g074(.a(new_n56_), .b(x06), .O(new_n126_));
  oai21  g075(.a(new_n118_), .b(new_n100_), .c(x28), .O(new_n127_));
  inv1   g076(.a(new_n78_), .O(new_n128_));
  inv1   g077(.a(x28), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n117_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(x23), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n110_), .c(new_n128_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n127_), .c(x16), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n126_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n125_), .O(z09));
  nand2  g084(.a(new_n132_), .b(x16), .O(new_n136_));
  nand2  g085(.a(new_n56_), .b(x05), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n125_), .O(z10));
  nor2   g088(.a(new_n130_), .b(x30), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n110_), .c(new_n86_), .O(new_n141_));
  nor2   g090(.a(x28), .b(x27), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n101_), .c(new_n108_), .d(new_n91_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n78_), .c(x30), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n141_), .c(x16), .O(new_n145_));
  aoi21  g094(.a(new_n56_), .b(x04), .c(x29), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n57_), .O(z11));
  nor2   g097(.a(x31), .b(x30), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n142_), .c(new_n101_), .d(new_n108_), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n85_), .O(new_n151_));
  aoi21  g100(.a(new_n141_), .b(x31), .c(new_n151_), .O(new_n152_));
  nor2   g101(.a(x16), .b(x03), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n59_), .O(new_n154_));
  oai21  g103(.a(new_n152_), .b(new_n56_), .c(new_n154_), .O(z12));
  oai21  g104(.a(new_n150_), .b(new_n85_), .c(x32), .O(new_n156_));
  inv1   g105(.a(x30), .O(new_n157_));
  inv1   g106(.a(x31), .O(new_n158_));
  inv1   g107(.a(x32), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n158_), .c(new_n157_), .d(new_n129_), .O(new_n160_));
  inv1   g109(.a(new_n160_), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n119_), .c(new_n94_), .d(new_n64_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n156_), .c(x16), .O(new_n163_));
  aoi21  g112(.a(new_n56_), .b(x02), .c(x29), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n57_), .O(z13));
  nor2   g115(.a(x26), .b(x25), .O(new_n167_));
  nor2   g116(.a(x33), .b(x32), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n167_), .c(new_n149_), .d(new_n142_), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n100_), .O(new_n170_));
  aoi21  g119(.a(new_n162_), .b(x33), .c(new_n170_), .O(new_n171_));
  nor2   g120(.a(x16), .b(x01), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n59_), .O(new_n173_));
  oai21  g122(.a(new_n171_), .b(new_n56_), .c(new_n173_), .O(z14));
  inv1   g123(.a(x33), .O(new_n175_));
  inv1   g124(.a(x34), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n175_), .c(new_n117_), .d(new_n108_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n160_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n103_), .O(new_n179_));
  oai21  g128(.a(new_n169_), .b(new_n100_), .c(x34), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n179_), .c(x16), .O(new_n181_));
  aoi21  g130(.a(new_n56_), .b(x00), .c(x29), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n57_), .O(z15));
endmodule


