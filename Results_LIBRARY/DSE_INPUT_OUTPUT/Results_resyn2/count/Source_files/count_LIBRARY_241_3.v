// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  inv1   g000(.a(x10), .O(new_n52_));
  nand2  g001(.a(x18), .b(new_n52_), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(x19), .b(x17), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x15), .c(x18), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n54_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n53_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n54_), .b(new_n61_), .O(new_n62_));
  inv1   g011(.a(x18), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n62_), .c(x16), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  aoi21  g017(.a(new_n57_), .b(new_n68_), .c(x18), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(new_n64_), .b(new_n71_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n54_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(new_n72_), .c(x16), .O(new_n76_));
  inv1   g025(.a(x13), .O(new_n77_));
  aoi21  g026(.a(new_n57_), .b(new_n77_), .c(x18), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n76_), .O(z02));
  xor2a  g028(.a(new_n74_), .b(x22), .O(new_n80_));
  inv1   g029(.a(x12), .O(new_n81_));
  aoi21  g030(.a(new_n57_), .b(new_n81_), .c(x18), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n57_), .c(new_n82_), .O(z03));
  oai21  g032(.a(new_n74_), .b(x22), .c(x23), .O(new_n84_));
  nor2   g033(.a(x23), .b(x22), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n73_), .c(new_n54_), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n84_), .c(x16), .O(new_n87_));
  aoi21  g036(.a(new_n57_), .b(x11), .c(x18), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n53_), .O(z04));
  nor2   g039(.a(x24), .b(x21), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n85_), .O(new_n92_));
  inv1   g041(.a(new_n92_), .O(new_n93_));
  aoi22  g042(.a(new_n93_), .b(new_n66_), .c(new_n86_), .d(x24), .O(new_n94_));
  aoi21  g043(.a(new_n57_), .b(new_n52_), .c(x18), .O(new_n95_));
  oai21  g044(.a(new_n94_), .b(new_n57_), .c(new_n95_), .O(z05));
  nor2   g045(.a(x25), .b(x24), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n85_), .c(new_n73_), .d(new_n54_), .O(new_n98_));
  nand4  g047(.a(new_n91_), .b(new_n85_), .c(new_n54_), .d(new_n61_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(x25), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n98_), .c(x16), .O(new_n101_));
  aoi21  g050(.a(new_n57_), .b(x09), .c(x18), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n53_), .O(z06));
  nand2  g053(.a(new_n98_), .b(x26), .O(new_n105_));
  inv1   g054(.a(x24), .O(new_n106_));
  nor2   g055(.a(x26), .b(x25), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n86_), .O(new_n109_));
  inv1   g058(.a(new_n109_), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n105_), .c(x16), .O(new_n111_));
  aoi21  g060(.a(new_n57_), .b(x08), .c(x18), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n53_), .O(z07));
  inv1   g063(.a(x27), .O(new_n115_));
  nor2   g064(.a(new_n109_), .b(new_n115_), .O(new_n116_));
  nor3   g065(.a(x27), .b(x26), .c(x25), .O(new_n117_));
  nand4  g066(.a(new_n117_), .b(new_n91_), .c(new_n85_), .d(new_n64_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(x16), .O(new_n119_));
  aoi21  g068(.a(new_n57_), .b(x07), .c(x18), .O(new_n120_));
  oai21  g069(.a(new_n119_), .b(new_n116_), .c(new_n120_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n53_), .O(z08));
  nor2   g071(.a(x28), .b(x27), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n109_), .O(new_n124_));
  nand2  g073(.a(new_n118_), .b(x28), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n124_), .c(x16), .O(new_n126_));
  aoi21  g075(.a(new_n57_), .b(x06), .c(x18), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n53_), .O(z09));
  inv1   g078(.a(x29), .O(new_n130_));
  aoi21  g079(.a(new_n123_), .b(new_n109_), .c(new_n130_), .O(new_n131_));
  inv1   g080(.a(new_n86_), .O(new_n132_));
  inv1   g081(.a(new_n108_), .O(new_n133_));
  nand2  g082(.a(new_n123_), .b(new_n130_), .O(new_n134_));
  inv1   g083(.a(new_n134_), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n133_), .c(new_n132_), .O(new_n136_));
  inv1   g085(.a(new_n136_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n131_), .c(x16), .O(new_n138_));
  inv1   g087(.a(x05), .O(new_n139_));
  aoi21  g088(.a(new_n57_), .b(new_n139_), .c(x18), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n138_), .O(z10));
  nor2   g090(.a(x30), .b(x29), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n123_), .O(new_n143_));
  inv1   g092(.a(new_n143_), .O(new_n144_));
  aoi22  g093(.a(new_n144_), .b(new_n109_), .c(new_n136_), .d(x30), .O(new_n145_));
  inv1   g094(.a(x04), .O(new_n146_));
  aoi21  g095(.a(new_n57_), .b(new_n146_), .c(x18), .O(new_n147_));
  oai21  g096(.a(new_n145_), .b(new_n57_), .c(new_n147_), .O(z11));
  inv1   g097(.a(x31), .O(new_n149_));
  aoi21  g098(.a(new_n144_), .b(new_n109_), .c(new_n149_), .O(new_n150_));
  inv1   g099(.a(x28), .O(new_n151_));
  nand3  g100(.a(new_n142_), .b(new_n149_), .c(new_n151_), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n118_), .c(x16), .O(new_n153_));
  aoi21  g102(.a(new_n57_), .b(x03), .c(x18), .O(new_n154_));
  oai21  g103(.a(new_n153_), .b(new_n150_), .c(new_n154_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n53_), .O(z12));
  inv1   g105(.a(new_n152_), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n117_), .c(new_n93_), .d(new_n64_), .O(new_n158_));
  nor2   g107(.a(x32), .b(x31), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n142_), .c(new_n123_), .d(new_n107_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n99_), .c(x16), .O(new_n161_));
  aoi21  g110(.a(new_n158_), .b(x32), .c(new_n161_), .O(new_n162_));
  inv1   g111(.a(x02), .O(new_n163_));
  oai21  g112(.a(x16), .b(new_n163_), .c(new_n63_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n162_), .c(new_n53_), .O(z13));
  inv1   g114(.a(new_n98_), .O(new_n166_));
  nor2   g115(.a(x27), .b(x26), .O(new_n167_));
  nor2   g116(.a(x33), .b(x32), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n152_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n166_), .O(new_n171_));
  oai21  g120(.a(new_n160_), .b(new_n99_), .c(x33), .O(new_n172_));
  nand3  g121(.a(new_n172_), .b(new_n171_), .c(x16), .O(new_n173_));
  aoi21  g122(.a(new_n57_), .b(x01), .c(x18), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n173_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n53_), .O(z14));
  nand3  g125(.a(new_n170_), .b(new_n166_), .c(x34), .O(new_n177_));
  inv1   g126(.a(x34), .O(new_n178_));
  nand2  g127(.a(new_n171_), .b(new_n178_), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n177_), .c(x16), .O(new_n180_));
  inv1   g129(.a(x00), .O(new_n181_));
  aoi21  g130(.a(new_n57_), .b(new_n181_), .c(x18), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n180_), .O(z15));
endmodule


