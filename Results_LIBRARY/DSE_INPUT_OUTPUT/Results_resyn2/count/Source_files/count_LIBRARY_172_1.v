// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  xor2a  g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  inv1   g011(.a(x14), .O(new_n63_));
  aoi21  g012(.a(new_n58_), .b(new_n63_), .c(x18), .O(new_n64_));
  oai21  g013(.a(new_n62_), .b(new_n58_), .c(new_n64_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  aoi21  g015(.a(new_n55_), .b(new_n61_), .c(new_n66_), .O(new_n67_));
  nand4  g016(.a(new_n66_), .b(new_n61_), .c(new_n53_), .d(new_n52_), .O(new_n68_));
  inv1   g017(.a(new_n68_), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n67_), .c(x16), .O(new_n70_));
  inv1   g019(.a(x13), .O(new_n71_));
  aoi21  g020(.a(new_n58_), .b(new_n71_), .c(x18), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n70_), .O(z02));
  xor2a  g022(.a(new_n68_), .b(x22), .O(new_n74_));
  inv1   g023(.a(x12), .O(new_n75_));
  aoi21  g024(.a(new_n58_), .b(new_n75_), .c(x18), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n58_), .c(new_n76_), .O(z03));
  oai21  g026(.a(new_n68_), .b(x22), .c(x23), .O(new_n78_));
  nor2   g027(.a(x23), .b(x22), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n69_), .O(new_n80_));
  aoi21  g029(.a(new_n80_), .b(new_n78_), .c(new_n58_), .O(new_n81_));
  inv1   g030(.a(x18), .O(new_n82_));
  oai21  g031(.a(x16), .b(x11), .c(new_n82_), .O(new_n83_));
  or2    g032(.a(new_n83_), .b(new_n81_), .O(z04));
  xor2a  g033(.a(new_n80_), .b(x24), .O(new_n85_));
  inv1   g034(.a(x10), .O(new_n86_));
  aoi21  g035(.a(new_n58_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n58_), .c(new_n87_), .O(z05));
  nor2   g037(.a(x18), .b(x16), .O(new_n89_));
  inv1   g038(.a(new_n89_), .O(new_n90_));
  inv1   g039(.a(x24), .O(new_n91_));
  nor2   g040(.a(x21), .b(x20), .O(new_n92_));
  nand4  g041(.a(new_n79_), .b(new_n92_), .c(new_n55_), .d(new_n91_), .O(new_n93_));
  nor2   g042(.a(x25), .b(x24), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n79_), .c(new_n92_), .d(new_n55_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n82_), .O(new_n96_));
  aoi21  g045(.a(new_n93_), .b(x25), .c(new_n96_), .O(new_n97_));
  oai22  g046(.a(new_n97_), .b(new_n58_), .c(new_n90_), .d(x09), .O(z06));
  inv1   g047(.a(x08), .O(new_n99_));
  nand2  g048(.a(new_n89_), .b(new_n99_), .O(new_n100_));
  nor2   g049(.a(new_n95_), .b(x26), .O(new_n101_));
  inv1   g050(.a(x26), .O(new_n102_));
  inv1   g051(.a(new_n95_), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n102_), .c(new_n82_), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n100_), .O(z07));
  inv1   g055(.a(x07), .O(new_n107_));
  nand2  g056(.a(new_n89_), .b(new_n107_), .O(new_n108_));
  inv1   g057(.a(new_n93_), .O(new_n109_));
  nor3   g058(.a(x27), .b(x26), .c(x25), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  oai21  g060(.a(new_n95_), .b(x26), .c(x27), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n111_), .c(new_n82_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n108_), .O(z08));
  inv1   g064(.a(new_n110_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n93_), .c(x28), .O(new_n117_));
  inv1   g066(.a(x28), .O(new_n118_));
  nand3  g067(.a(new_n110_), .b(new_n109_), .c(new_n118_), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n117_), .c(new_n82_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(x16), .O(new_n121_));
  inv1   g070(.a(x06), .O(new_n122_));
  nand2  g071(.a(new_n89_), .b(new_n122_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n121_), .O(z09));
  nand2  g073(.a(new_n110_), .b(new_n118_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n93_), .c(x29), .O(new_n126_));
  inv1   g075(.a(x22), .O(new_n127_));
  inv1   g076(.a(x23), .O(new_n128_));
  inv1   g077(.a(x25), .O(new_n129_));
  nand4  g078(.a(new_n129_), .b(new_n91_), .c(new_n128_), .d(new_n127_), .O(new_n130_));
  inv1   g079(.a(new_n130_), .O(new_n131_));
  nor3   g080(.a(x29), .b(x28), .c(x27), .O(new_n132_));
  nand4  g081(.a(new_n132_), .b(new_n131_), .c(new_n69_), .d(new_n102_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n126_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  inv1   g084(.a(x05), .O(new_n136_));
  aoi21  g085(.a(new_n58_), .b(new_n136_), .c(x18), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n135_), .O(z10));
  inv1   g087(.a(x27), .O(new_n139_));
  inv1   g088(.a(x30), .O(new_n140_));
  nor2   g089(.a(x29), .b(x28), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n140_), .c(new_n139_), .O(new_n142_));
  inv1   g091(.a(new_n142_), .O(new_n143_));
  aoi22  g092(.a(new_n143_), .b(new_n101_), .c(new_n133_), .d(x30), .O(new_n144_));
  inv1   g093(.a(x04), .O(new_n145_));
  aoi21  g094(.a(new_n58_), .b(new_n145_), .c(x18), .O(new_n146_));
  oai21  g095(.a(new_n144_), .b(new_n58_), .c(new_n146_), .O(z11));
  nand3  g096(.a(new_n143_), .b(new_n103_), .c(new_n102_), .O(new_n148_));
  nor2   g097(.a(x27), .b(x26), .O(new_n149_));
  nor2   g098(.a(x31), .b(x30), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n141_), .c(new_n149_), .d(new_n129_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n93_), .c(new_n82_), .O(new_n152_));
  aoi21  g101(.a(new_n148_), .b(x31), .c(new_n152_), .O(new_n153_));
  inv1   g102(.a(x03), .O(new_n154_));
  nand2  g103(.a(new_n89_), .b(new_n154_), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n58_), .c(new_n155_), .O(z12));
  oai21  g105(.a(new_n151_), .b(new_n93_), .c(x32), .O(new_n157_));
  nor2   g106(.a(x32), .b(x31), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n102_), .c(new_n129_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n143_), .c(new_n109_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(x16), .O(new_n163_));
  inv1   g112(.a(x02), .O(new_n164_));
  aoi21  g113(.a(new_n58_), .b(new_n164_), .c(x18), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n163_), .O(z13));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n150_), .c(new_n141_), .d(new_n149_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n95_), .O(new_n169_));
  aoi21  g118(.a(new_n161_), .b(x33), .c(new_n169_), .O(new_n170_));
  inv1   g119(.a(x01), .O(new_n171_));
  aoi21  g120(.a(new_n58_), .b(new_n171_), .c(x18), .O(new_n172_));
  oai21  g121(.a(new_n170_), .b(new_n58_), .c(new_n172_), .O(z14));
  inv1   g122(.a(x00), .O(new_n174_));
  nand2  g123(.a(new_n89_), .b(new_n174_), .O(new_n175_));
  nor3   g124(.a(new_n168_), .b(new_n95_), .c(x34), .O(new_n176_));
  oai21  g125(.a(new_n168_), .b(new_n95_), .c(x34), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n82_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n176_), .c(x16), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n175_), .O(z15));
endmodule


