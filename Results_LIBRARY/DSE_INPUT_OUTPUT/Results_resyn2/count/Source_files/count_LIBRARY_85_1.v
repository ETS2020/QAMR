// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  nor2   g005(.a(x18), .b(x16), .O(new_n57_));
  inv1   g006(.a(new_n57_), .O(new_n58_));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(x19), .b(x17), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  oai21  g010(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(x16), .O(new_n63_));
  oai21  g012(.a(new_n58_), .b(x14), .c(new_n63_), .O(z01));
  inv1   g013(.a(x21), .O(new_n65_));
  aoi21  g014(.a(new_n60_), .b(new_n59_), .c(new_n65_), .O(new_n66_));
  inv1   g015(.a(x18), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n60_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  oai21  g019(.a(new_n70_), .b(new_n66_), .c(x16), .O(new_n71_));
  oai21  g020(.a(new_n58_), .b(x13), .c(new_n71_), .O(z02));
  inv1   g021(.a(x12), .O(new_n73_));
  nand2  g022(.a(new_n57_), .b(new_n73_), .O(new_n74_));
  inv1   g023(.a(x22), .O(new_n75_));
  inv1   g024(.a(new_n69_), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g026(.a(new_n76_), .b(new_n75_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  oai21  g028(.a(new_n79_), .b(new_n77_), .c(x16), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n74_), .O(z03));
  inv1   g030(.a(x11), .O(new_n82_));
  nand2  g031(.a(new_n57_), .b(new_n82_), .O(new_n83_));
  nand2  g032(.a(new_n78_), .b(x23), .O(new_n84_));
  nor2   g033(.a(x23), .b(x22), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n76_), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n84_), .c(new_n67_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(x16), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n83_), .O(z04));
  xor2a  g038(.a(new_n86_), .b(x24), .O(new_n90_));
  inv1   g039(.a(x10), .O(new_n91_));
  aoi21  g040(.a(new_n52_), .b(new_n91_), .c(x18), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n52_), .c(new_n92_), .O(z05));
  inv1   g042(.a(x24), .O(new_n94_));
  nand4  g043(.a(new_n85_), .b(new_n68_), .c(new_n60_), .d(new_n94_), .O(new_n95_));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n85_), .c(new_n68_), .d(new_n60_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n67_), .O(new_n98_));
  aoi21  g047(.a(new_n95_), .b(x25), .c(new_n98_), .O(new_n99_));
  oai22  g048(.a(new_n99_), .b(new_n52_), .c(new_n58_), .d(x09), .O(z06));
  nand2  g049(.a(new_n52_), .b(x08), .O(new_n101_));
  nor2   g050(.a(new_n97_), .b(x26), .O(new_n102_));
  nand2  g051(.a(new_n97_), .b(x26), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(x16), .O(new_n104_));
  oai21  g053(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n67_), .O(z07));
  inv1   g055(.a(new_n95_), .O(new_n107_));
  nor3   g056(.a(x27), .b(x26), .c(x25), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  oai21  g058(.a(new_n97_), .b(x26), .c(x27), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n109_), .c(new_n67_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(x16), .O(new_n112_));
  oai21  g061(.a(new_n58_), .b(x07), .c(new_n112_), .O(z08));
  inv1   g062(.a(x28), .O(new_n114_));
  aoi21  g063(.a(new_n108_), .b(new_n107_), .c(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n108_), .b(new_n114_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n95_), .c(new_n67_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n115_), .c(x16), .O(new_n118_));
  inv1   g067(.a(x06), .O(new_n119_));
  nand2  g068(.a(new_n57_), .b(new_n119_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(z09));
  oai21  g070(.a(new_n116_), .b(new_n95_), .c(x29), .O(new_n122_));
  nor3   g071(.a(x29), .b(x28), .c(x27), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n102_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n122_), .c(new_n67_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(x16), .O(new_n126_));
  inv1   g075(.a(x05), .O(new_n127_));
  nand2  g076(.a(new_n57_), .b(new_n127_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(z10));
  inv1   g078(.a(x30), .O(new_n130_));
  aoi21  g079(.a(new_n123_), .b(new_n102_), .c(new_n130_), .O(new_n131_));
  inv1   g080(.a(x26), .O(new_n132_));
  inv1   g081(.a(new_n97_), .O(new_n133_));
  inv1   g082(.a(x27), .O(new_n134_));
  nor2   g083(.a(x29), .b(x28), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n130_), .c(new_n134_), .O(new_n136_));
  inv1   g085(.a(new_n136_), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n133_), .c(new_n132_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n67_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(new_n131_), .c(x16), .O(new_n140_));
  inv1   g089(.a(x04), .O(new_n141_));
  nand2  g090(.a(new_n57_), .b(new_n141_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n140_), .O(z11));
  nor2   g092(.a(x31), .b(x30), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n135_), .c(new_n108_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n95_), .c(new_n67_), .O(new_n146_));
  aoi21  g095(.a(new_n138_), .b(x31), .c(new_n146_), .O(new_n147_));
  inv1   g096(.a(x03), .O(new_n148_));
  nand2  g097(.a(new_n57_), .b(new_n148_), .O(new_n149_));
  oai21  g098(.a(new_n147_), .b(new_n52_), .c(new_n149_), .O(z12));
  oai21  g099(.a(new_n145_), .b(new_n95_), .c(x32), .O(new_n151_));
  nor2   g100(.a(x26), .b(x25), .O(new_n152_));
  nor2   g101(.a(x32), .b(x31), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n152_), .c(new_n137_), .d(new_n107_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n151_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(x16), .O(new_n156_));
  inv1   g105(.a(x02), .O(new_n157_));
  aoi21  g106(.a(new_n52_), .b(new_n157_), .c(x18), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n156_), .O(z13));
  nor2   g108(.a(x27), .b(x26), .O(new_n160_));
  nor2   g109(.a(x33), .b(x32), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n144_), .c(new_n135_), .d(new_n160_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n97_), .O(new_n163_));
  aoi21  g112(.a(new_n154_), .b(x33), .c(new_n163_), .O(new_n164_));
  inv1   g113(.a(x01), .O(new_n165_));
  aoi21  g114(.a(new_n52_), .b(new_n165_), .c(x18), .O(new_n166_));
  oai21  g115(.a(new_n164_), .b(new_n52_), .c(new_n166_), .O(z14));
  inv1   g116(.a(x00), .O(new_n168_));
  nand2  g117(.a(new_n57_), .b(new_n168_), .O(new_n169_));
  nor3   g118(.a(new_n162_), .b(new_n97_), .c(x34), .O(new_n170_));
  inv1   g119(.a(x34), .O(new_n171_));
  oai21  g120(.a(new_n163_), .b(new_n171_), .c(new_n67_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n170_), .c(x16), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n169_), .O(z15));
endmodule


