// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x04), .O(new_n61_));
  nand2  g010(.a(x18), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  oai21  g013(.a(new_n55_), .b(new_n64_), .c(x16), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(x14), .c(x18), .O(new_n66_));
  oai21  g015(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n62_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nand2  g018(.a(new_n63_), .b(new_n69_), .O(new_n70_));
  nand3  g019(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x21), .O(new_n72_));
  nand3  g021(.a(new_n72_), .b(new_n70_), .c(x16), .O(new_n73_));
  aoi21  g022(.a(new_n58_), .b(x13), .c(x18), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n62_), .O(z02));
  nor2   g025(.a(x22), .b(x21), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n55_), .c(new_n64_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(x16), .O(new_n79_));
  aoi21  g028(.a(new_n70_), .b(x22), .c(new_n79_), .O(new_n80_));
  inv1   g029(.a(x12), .O(new_n81_));
  inv1   g030(.a(x18), .O(new_n82_));
  oai21  g031(.a(x16), .b(new_n81_), .c(new_n82_), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(new_n80_), .c(new_n62_), .O(z03));
  inv1   g033(.a(x22), .O(new_n85_));
  inv1   g034(.a(x23), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n85_), .c(new_n69_), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n71_), .O(new_n88_));
  aoi21  g037(.a(new_n78_), .b(x23), .c(new_n88_), .O(new_n89_));
  inv1   g038(.a(x11), .O(new_n90_));
  aoi21  g039(.a(new_n58_), .b(new_n90_), .c(x18), .O(new_n91_));
  oai21  g040(.a(new_n89_), .b(new_n58_), .c(new_n91_), .O(z04));
  nand4  g041(.a(new_n77_), .b(new_n55_), .c(new_n86_), .d(new_n64_), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n77_), .c(new_n55_), .d(new_n64_), .O(new_n95_));
  inv1   g044(.a(new_n95_), .O(new_n96_));
  aoi21  g045(.a(new_n93_), .b(x24), .c(new_n96_), .O(new_n97_));
  inv1   g046(.a(x10), .O(new_n98_));
  aoi21  g047(.a(new_n58_), .b(new_n98_), .c(x18), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n58_), .c(new_n99_), .O(z05));
  nor2   g049(.a(x25), .b(x24), .O(new_n101_));
  aoi22  g050(.a(new_n101_), .b(new_n88_), .c(new_n95_), .d(x25), .O(new_n102_));
  inv1   g051(.a(x09), .O(new_n103_));
  aoi21  g052(.a(new_n58_), .b(new_n103_), .c(x18), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n58_), .c(new_n104_), .O(z06));
  inv1   g054(.a(x26), .O(new_n106_));
  aoi21  g055(.a(new_n101_), .b(new_n88_), .c(new_n106_), .O(new_n107_));
  nor3   g056(.a(x26), .b(x25), .c(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n77_), .c(new_n63_), .d(new_n86_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(x16), .O(new_n110_));
  aoi21  g059(.a(new_n58_), .b(x08), .c(x18), .O(new_n111_));
  oai21  g060(.a(new_n110_), .b(new_n107_), .c(new_n111_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n62_), .O(z07));
  nor3   g062(.a(x27), .b(x26), .c(x25), .O(new_n114_));
  aoi22  g063(.a(new_n114_), .b(new_n96_), .c(new_n109_), .d(x27), .O(new_n115_));
  inv1   g064(.a(x07), .O(new_n116_));
  aoi21  g065(.a(new_n58_), .b(new_n116_), .c(x18), .O(new_n117_));
  oai21  g066(.a(new_n115_), .b(new_n58_), .c(new_n117_), .O(z08));
  nand2  g067(.a(new_n114_), .b(new_n96_), .O(new_n119_));
  nor2   g068(.a(x26), .b(x25), .O(new_n120_));
  nor2   g069(.a(x28), .b(x27), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n120_), .c(new_n94_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n78_), .c(x16), .O(new_n123_));
  aoi21  g072(.a(new_n119_), .b(x28), .c(new_n123_), .O(new_n124_));
  inv1   g073(.a(x06), .O(new_n125_));
  oai21  g074(.a(x16), .b(new_n125_), .c(new_n82_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n124_), .c(new_n62_), .O(z09));
  nor3   g076(.a(x29), .b(x28), .c(x27), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n108_), .c(new_n88_), .O(new_n129_));
  oai21  g078(.a(new_n122_), .b(new_n78_), .c(x29), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n129_), .c(x16), .O(new_n131_));
  aoi21  g080(.a(new_n58_), .b(x05), .c(x18), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n62_), .O(z10));
  inv1   g083(.a(x24), .O(new_n135_));
  nor2   g084(.a(x30), .b(x29), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n121_), .c(new_n120_), .d(new_n135_), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n93_), .O(new_n138_));
  aoi21  g087(.a(new_n129_), .b(x30), .c(new_n138_), .O(new_n139_));
  aoi21  g088(.a(new_n58_), .b(new_n61_), .c(x18), .O(new_n140_));
  oai21  g089(.a(new_n139_), .b(new_n58_), .c(new_n140_), .O(z11));
  oai21  g090(.a(new_n137_), .b(new_n93_), .c(x31), .O(new_n142_));
  nand2  g091(.a(new_n94_), .b(new_n77_), .O(new_n143_));
  inv1   g092(.a(new_n143_), .O(new_n144_));
  inv1   g093(.a(x28), .O(new_n145_));
  inv1   g094(.a(x29), .O(new_n146_));
  inv1   g095(.a(x30), .O(new_n147_));
  inv1   g096(.a(x31), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n147_), .c(new_n146_), .d(new_n145_), .O(new_n149_));
  inv1   g098(.a(new_n149_), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n114_), .c(new_n144_), .d(new_n63_), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n142_), .c(x16), .O(new_n152_));
  aoi21  g101(.a(new_n58_), .b(x03), .c(x18), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n62_), .O(z12));
  nor2   g104(.a(x32), .b(x31), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n136_), .c(new_n121_), .d(new_n120_), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n95_), .O(new_n158_));
  aoi21  g107(.a(new_n151_), .b(x32), .c(new_n158_), .O(new_n159_));
  inv1   g108(.a(x02), .O(new_n160_));
  aoi21  g109(.a(new_n58_), .b(new_n160_), .c(x18), .O(new_n161_));
  oai21  g110(.a(new_n159_), .b(new_n58_), .c(new_n161_), .O(z13));
  inv1   g111(.a(x27), .O(new_n163_));
  inv1   g112(.a(x32), .O(new_n164_));
  inv1   g113(.a(x33), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n164_), .c(new_n163_), .d(new_n106_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n149_), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n101_), .c(new_n88_), .O(new_n168_));
  oai21  g117(.a(new_n157_), .b(new_n95_), .c(x33), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n168_), .c(x16), .O(new_n170_));
  aoi21  g119(.a(new_n58_), .b(x01), .c(x18), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n62_), .O(z14));
  nand4  g122(.a(new_n167_), .b(new_n101_), .c(new_n88_), .d(x34), .O(new_n174_));
  inv1   g123(.a(x34), .O(new_n175_));
  nand2  g124(.a(new_n168_), .b(new_n175_), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n174_), .c(x16), .O(new_n177_));
  inv1   g126(.a(x00), .O(new_n178_));
  aoi21  g127(.a(new_n58_), .b(new_n178_), .c(x18), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z15));
endmodule


