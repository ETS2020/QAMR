// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n54_), .O(z00));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g011(.a(x14), .O(new_n63_));
  aoi21  g012(.a(new_n56_), .b(new_n63_), .c(x18), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n62_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  inv1   g015(.a(x18), .O(new_n67_));
  nor2   g016(.a(x20), .b(new_n56_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n66_), .O(new_n71_));
  nor2   g020(.a(x16), .b(x13), .O(new_n72_));
  nor3   g021(.a(new_n61_), .b(new_n66_), .c(new_n56_), .O(new_n73_));
  oai21  g022(.a(new_n73_), .b(new_n72_), .c(new_n67_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n71_), .O(z02));
  inv1   g024(.a(x22), .O(new_n76_));
  nand4  g025(.a(new_n53_), .b(new_n76_), .c(new_n59_), .d(x16), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n67_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n66_), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  nor2   g029(.a(x21), .b(x20), .O(new_n81_));
  nand2  g030(.a(x22), .b(x16), .O(new_n82_));
  aoi21  g031(.a(new_n81_), .b(new_n53_), .c(new_n82_), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(new_n80_), .c(new_n67_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n79_), .O(z03));
  oai21  g034(.a(new_n77_), .b(x23), .c(new_n67_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n66_), .O(new_n87_));
  nand3  g036(.a(new_n81_), .b(new_n53_), .c(new_n76_), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(x23), .c(x16), .O(new_n89_));
  oai21  g038(.a(x16), .b(x11), .c(new_n89_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n67_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n87_), .O(z04));
  nor2   g041(.a(x24), .b(x23), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n68_), .c(new_n53_), .d(new_n76_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n67_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n66_), .O(new_n96_));
  nor2   g045(.a(x23), .b(x22), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n81_), .c(new_n53_), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(x24), .c(x16), .O(new_n99_));
  oai21  g048(.a(x16), .b(x10), .c(new_n99_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n67_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n96_), .O(z05));
  nor2   g051(.a(x22), .b(x21), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n93_), .c(new_n53_), .d(new_n59_), .O(new_n104_));
  nor2   g053(.a(x25), .b(x24), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n97_), .c(new_n81_), .d(new_n53_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  aoi21  g056(.a(new_n104_), .b(x25), .c(new_n107_), .O(new_n108_));
  inv1   g057(.a(x09), .O(new_n109_));
  aoi21  g058(.a(new_n56_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n56_), .c(new_n110_), .O(z06));
  inv1   g060(.a(x25), .O(new_n112_));
  inv1   g061(.a(x26), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n94_), .c(new_n67_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n66_), .O(new_n116_));
  nand3  g065(.a(new_n106_), .b(x26), .c(x16), .O(new_n117_));
  oai21  g066(.a(x16), .b(x08), .c(new_n117_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n67_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n116_), .O(z07));
  inv1   g069(.a(x27), .O(new_n121_));
  nor2   g070(.a(x26), .b(x25), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n93_), .c(new_n121_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n77_), .c(new_n67_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n66_), .O(new_n125_));
  inv1   g074(.a(new_n88_), .O(new_n126_));
  nand2  g075(.a(new_n122_), .b(new_n93_), .O(new_n127_));
  inv1   g076(.a(new_n127_), .O(new_n128_));
  nand2  g077(.a(x27), .b(x16), .O(new_n129_));
  aoi21  g078(.a(new_n128_), .b(new_n126_), .c(new_n129_), .O(new_n130_));
  nor2   g079(.a(x16), .b(x07), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n130_), .c(new_n67_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n125_), .O(z08));
  oai21  g082(.a(new_n123_), .b(new_n88_), .c(x28), .O(new_n134_));
  nor2   g083(.a(x28), .b(x27), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n122_), .c(new_n93_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n88_), .c(new_n134_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  inv1   g087(.a(x06), .O(new_n139_));
  aoi21  g088(.a(new_n56_), .b(new_n139_), .c(x18), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n138_), .O(z09));
  oai21  g090(.a(new_n136_), .b(new_n88_), .c(x29), .O(new_n142_));
  inv1   g091(.a(x24), .O(new_n143_));
  inv1   g092(.a(x29), .O(new_n144_));
  nand4  g093(.a(new_n135_), .b(new_n122_), .c(new_n144_), .d(new_n143_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n98_), .c(new_n142_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(x16), .O(new_n147_));
  inv1   g096(.a(x05), .O(new_n148_));
  aoi21  g097(.a(new_n56_), .b(new_n148_), .c(x18), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n147_), .O(z10));
  oai21  g099(.a(new_n145_), .b(new_n98_), .c(x30), .O(new_n151_));
  inv1   g100(.a(new_n98_), .O(new_n152_));
  nor2   g101(.a(new_n114_), .b(x24), .O(new_n153_));
  nor2   g102(.a(x30), .b(x29), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n153_), .c(new_n135_), .d(new_n152_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(x16), .O(new_n157_));
  inv1   g106(.a(x04), .O(new_n158_));
  aoi21  g107(.a(new_n56_), .b(new_n158_), .c(x18), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n157_), .O(z11));
  nor2   g109(.a(x31), .b(x28), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n154_), .O(new_n162_));
  nor3   g111(.a(new_n162_), .b(new_n123_), .c(new_n77_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(x18), .c(new_n66_), .O(new_n164_));
  nand4  g113(.a(new_n154_), .b(new_n135_), .c(new_n122_), .d(new_n143_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n98_), .O(new_n166_));
  nand2  g115(.a(x31), .b(x16), .O(new_n167_));
  inv1   g116(.a(x03), .O(new_n168_));
  nand2  g117(.a(new_n56_), .b(new_n168_), .O(new_n169_));
  oai21  g118(.a(new_n167_), .b(new_n166_), .c(new_n169_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n67_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n164_), .O(z12));
  nor2   g121(.a(x32), .b(x31), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n154_), .c(new_n135_), .d(new_n122_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n94_), .c(new_n67_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n66_), .O(new_n176_));
  nand4  g125(.a(new_n161_), .b(new_n154_), .c(new_n122_), .d(new_n121_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n104_), .O(new_n178_));
  nand2  g127(.a(x32), .b(x16), .O(new_n179_));
  inv1   g128(.a(x02), .O(new_n180_));
  nand2  g129(.a(new_n56_), .b(new_n180_), .O(new_n181_));
  oai21  g130(.a(new_n179_), .b(new_n178_), .c(new_n181_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n67_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n176_), .O(z13));
  oai21  g133(.a(new_n174_), .b(new_n104_), .c(x33), .O(new_n185_));
  nor2   g134(.a(x27), .b(x26), .O(new_n186_));
  nor2   g135(.a(x33), .b(x32), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n186_), .c(new_n161_), .d(new_n154_), .O(new_n188_));
  inv1   g137(.a(new_n188_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n107_), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n185_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(x16), .O(new_n192_));
  inv1   g141(.a(x01), .O(new_n193_));
  aoi21  g142(.a(new_n56_), .b(new_n193_), .c(x18), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n192_), .O(z14));
  inv1   g144(.a(x34), .O(new_n196_));
  nor2   g145(.a(new_n190_), .b(new_n196_), .O(new_n197_));
  oai21  g146(.a(new_n188_), .b(new_n106_), .c(new_n196_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(x16), .O(new_n199_));
  inv1   g148(.a(x00), .O(new_n200_));
  aoi21  g149(.a(new_n56_), .b(new_n200_), .c(x18), .O(new_n201_));
  oai21  g150(.a(new_n199_), .b(new_n197_), .c(new_n201_), .O(z15));
endmodule


