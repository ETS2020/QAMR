// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_;
  inv1   g000(.a(x32), .O(new_n52_));
  nand2  g001(.a(new_n52_), .b(x18), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(x19), .b(x17), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x15), .c(x18), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n54_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n53_), .O(z00));
  oai21  g009(.a(x19), .b(x17), .c(x20), .O(new_n61_));
  inv1   g010(.a(x17), .O(new_n62_));
  nor2   g011(.a(x20), .b(x19), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n61_), .c(x16), .O(new_n65_));
  aoi21  g014(.a(new_n57_), .b(x14), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n53_), .O(z01));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n54_), .O(new_n70_));
  inv1   g019(.a(new_n70_), .O(new_n71_));
  aoi21  g020(.a(new_n64_), .b(x21), .c(new_n71_), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n57_), .b(new_n73_), .c(x18), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n57_), .c(new_n74_), .O(z02));
  inv1   g024(.a(x22), .O(new_n76_));
  nand2  g025(.a(new_n71_), .b(new_n76_), .O(new_n77_));
  aoi21  g026(.a(new_n70_), .b(x22), .c(new_n57_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g028(.a(new_n57_), .b(x12), .c(x18), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n53_), .O(z03));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n69_), .c(new_n54_), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  aoi21  g034(.a(new_n77_), .b(x23), .c(new_n85_), .O(new_n86_));
  inv1   g035(.a(x11), .O(new_n87_));
  aoi21  g036(.a(new_n57_), .b(new_n87_), .c(x18), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n57_), .c(new_n88_), .O(z04));
  inv1   g038(.a(x21), .O(new_n90_));
  inv1   g039(.a(x23), .O(new_n91_));
  inv1   g040(.a(x24), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n91_), .c(new_n76_), .d(new_n90_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  aoi21  g043(.a(new_n84_), .b(x24), .c(new_n94_), .O(new_n95_));
  inv1   g044(.a(x10), .O(new_n96_));
  aoi21  g045(.a(new_n57_), .b(new_n96_), .c(x18), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n57_), .c(new_n97_), .O(z05));
  nand4  g047(.a(new_n83_), .b(new_n69_), .c(new_n54_), .d(new_n92_), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(x25), .O(new_n100_));
  inv1   g049(.a(x25), .O(new_n101_));
  oai21  g050(.a(new_n94_), .b(new_n101_), .c(x16), .O(new_n102_));
  aoi21  g051(.a(new_n57_), .b(x09), .c(x18), .O(new_n103_));
  oai21  g052(.a(new_n102_), .b(new_n100_), .c(new_n103_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n53_), .O(z06));
  inv1   g054(.a(x26), .O(new_n106_));
  inv1   g055(.a(x18), .O(new_n107_));
  inv1   g056(.a(new_n99_), .O(new_n108_));
  nor2   g057(.a(x26), .b(x25), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  oai21  g059(.a(new_n100_), .b(new_n106_), .c(new_n110_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(x16), .O(new_n112_));
  inv1   g061(.a(x08), .O(new_n113_));
  aoi21  g062(.a(new_n57_), .b(new_n113_), .c(x18), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n112_), .O(z07));
  inv1   g064(.a(x27), .O(new_n116_));
  aoi21  g065(.a(new_n109_), .b(new_n108_), .c(new_n116_), .O(new_n117_));
  nor3   g066(.a(x20), .b(x19), .c(x17), .O(new_n118_));
  nor2   g067(.a(x24), .b(x21), .O(new_n119_));
  nor3   g068(.a(x27), .b(x26), .c(x25), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n119_), .c(new_n83_), .d(new_n118_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x16), .O(new_n122_));
  aoi21  g071(.a(new_n57_), .b(x07), .c(x18), .O(new_n123_));
  oai21  g072(.a(new_n122_), .b(new_n117_), .c(new_n123_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n53_), .O(z08));
  inv1   g074(.a(x28), .O(new_n126_));
  nand3  g075(.a(new_n120_), .b(new_n94_), .c(new_n126_), .O(new_n127_));
  nand2  g076(.a(new_n121_), .b(x28), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(x16), .O(new_n129_));
  aoi21  g078(.a(new_n57_), .b(x06), .c(x18), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n53_), .O(z09));
  nor2   g081(.a(x29), .b(x28), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n109_), .c(new_n116_), .d(new_n92_), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n84_), .O(new_n135_));
  aoi21  g084(.a(new_n127_), .b(x29), .c(new_n135_), .O(new_n136_));
  inv1   g085(.a(x05), .O(new_n137_));
  aoi21  g086(.a(new_n57_), .b(new_n137_), .c(x18), .O(new_n138_));
  oai21  g087(.a(new_n136_), .b(new_n57_), .c(new_n138_), .O(z10));
  inv1   g088(.a(x30), .O(new_n140_));
  nand2  g089(.a(new_n135_), .b(new_n140_), .O(new_n141_));
  oai21  g090(.a(new_n134_), .b(new_n84_), .c(x30), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n141_), .c(x16), .O(new_n143_));
  aoi21  g092(.a(new_n57_), .b(x04), .c(x18), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n53_), .O(z11));
  inv1   g095(.a(x31), .O(new_n147_));
  aoi21  g096(.a(new_n135_), .b(new_n140_), .c(new_n147_), .O(new_n148_));
  nor2   g097(.a(x30), .b(x29), .O(new_n149_));
  nor2   g098(.a(x31), .b(x28), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n121_), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n148_), .c(x16), .O(new_n153_));
  inv1   g102(.a(x03), .O(new_n154_));
  aoi21  g103(.a(new_n57_), .b(new_n154_), .c(x18), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n153_), .O(z12));
  oai21  g105(.a(new_n151_), .b(new_n121_), .c(x32), .O(new_n157_));
  nand3  g106(.a(new_n149_), .b(new_n52_), .c(new_n147_), .O(new_n158_));
  inv1   g107(.a(new_n158_), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n120_), .c(new_n94_), .d(new_n126_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n157_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(x16), .O(new_n162_));
  inv1   g111(.a(x02), .O(new_n163_));
  aoi21  g112(.a(new_n57_), .b(new_n163_), .c(x18), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n162_), .O(z13));
  inv1   g114(.a(x33), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n57_), .O(new_n167_));
  nor2   g116(.a(x16), .b(x01), .O(new_n168_));
  aoi21  g117(.a(new_n167_), .b(new_n160_), .c(new_n168_), .O(new_n169_));
  nand3  g118(.a(new_n150_), .b(new_n149_), .c(new_n63_), .O(new_n170_));
  inv1   g119(.a(new_n170_), .O(new_n171_));
  nand3  g120(.a(new_n166_), .b(new_n62_), .c(x16), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n93_), .O(new_n173_));
  nand3  g122(.a(new_n173_), .b(new_n171_), .c(new_n120_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n107_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n52_), .O(new_n176_));
  oai21  g125(.a(new_n169_), .b(x18), .c(new_n176_), .O(z14));
  nand4  g126(.a(new_n166_), .b(new_n52_), .c(new_n116_), .d(new_n106_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n151_), .O(new_n179_));
  aoi21  g128(.a(new_n179_), .b(new_n100_), .c(x34), .O(new_n180_));
  nand3  g129(.a(new_n179_), .b(new_n100_), .c(x34), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(x16), .O(new_n182_));
  inv1   g131(.a(x00), .O(new_n183_));
  aoi21  g132(.a(new_n57_), .b(new_n183_), .c(x18), .O(new_n184_));
  oai21  g133(.a(new_n182_), .b(new_n180_), .c(new_n184_), .O(z15));
endmodule


