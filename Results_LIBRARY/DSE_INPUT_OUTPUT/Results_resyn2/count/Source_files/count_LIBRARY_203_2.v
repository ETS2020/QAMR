// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_;
  nand2  g000(.a(x24), .b(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x18), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nand2  g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  oai21  g010(.a(x19), .b(x17), .c(x20), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  aoi21  g012(.a(new_n56_), .b(x14), .c(x18), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n52_), .O(z01));
  nor2   g015(.a(x21), .b(x20), .O(new_n67_));
  aoi22  g016(.a(new_n67_), .b(new_n53_), .c(new_n61_), .d(x21), .O(new_n68_));
  inv1   g017(.a(x13), .O(new_n69_));
  aoi21  g018(.a(new_n56_), .b(new_n69_), .c(x18), .O(new_n70_));
  oai21  g019(.a(new_n68_), .b(new_n56_), .c(new_n70_), .O(z02));
  nand2  g020(.a(new_n67_), .b(new_n53_), .O(new_n72_));
  xor2a  g021(.a(new_n72_), .b(x22), .O(new_n73_));
  inv1   g022(.a(x12), .O(new_n74_));
  aoi21  g023(.a(new_n56_), .b(new_n74_), .c(x18), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(new_n56_), .c(new_n75_), .O(z03));
  inv1   g025(.a(x18), .O(new_n77_));
  nand2  g026(.a(new_n56_), .b(x11), .O(new_n78_));
  oai21  g027(.a(new_n72_), .b(x22), .c(x23), .O(new_n79_));
  nor2   g028(.a(x23), .b(x22), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n67_), .c(new_n53_), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n79_), .c(x16), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n77_), .O(z04));
  inv1   g033(.a(x24), .O(new_n85_));
  aoi21  g034(.a(new_n81_), .b(x16), .c(x18), .O(new_n86_));
  nand4  g035(.a(new_n80_), .b(new_n67_), .c(new_n53_), .d(new_n85_), .O(new_n87_));
  inv1   g036(.a(new_n87_), .O(new_n88_));
  aoi21  g037(.a(new_n56_), .b(x10), .c(x18), .O(new_n89_));
  oai21  g038(.a(new_n88_), .b(new_n56_), .c(new_n89_), .O(new_n90_));
  oai21  g039(.a(new_n86_), .b(new_n85_), .c(new_n90_), .O(z05));
  inv1   g040(.a(x25), .O(new_n92_));
  nor2   g041(.a(new_n88_), .b(new_n92_), .O(new_n93_));
  nor3   g042(.a(new_n81_), .b(x25), .c(x24), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n93_), .c(x16), .O(new_n95_));
  inv1   g044(.a(x09), .O(new_n96_));
  aoi21  g045(.a(new_n56_), .b(new_n96_), .c(x18), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n95_), .O(z06));
  aoi21  g047(.a(x26), .b(x16), .c(x18), .O(new_n99_));
  oai21  g048(.a(new_n81_), .b(x25), .c(x26), .O(new_n100_));
  inv1   g049(.a(new_n81_), .O(new_n101_));
  nor2   g050(.a(x26), .b(x25), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n101_), .c(new_n85_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n100_), .c(x16), .O(new_n104_));
  aoi21  g053(.a(new_n56_), .b(x08), .c(x18), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  oai21  g055(.a(new_n99_), .b(new_n85_), .c(new_n106_), .O(z07));
  aoi21  g056(.a(x27), .b(x16), .c(x18), .O(new_n108_));
  nor2   g057(.a(x27), .b(x26), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n92_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n87_), .O(new_n111_));
  inv1   g060(.a(new_n102_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(new_n81_), .c(x27), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  aoi21  g063(.a(new_n56_), .b(x07), .c(x18), .O(new_n115_));
  oai21  g064(.a(new_n114_), .b(new_n111_), .c(new_n115_), .O(new_n116_));
  oai21  g065(.a(new_n108_), .b(new_n85_), .c(new_n116_), .O(z08));
  inv1   g066(.a(x28), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n56_), .c(new_n77_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x24), .O(new_n120_));
  nor3   g069(.a(new_n110_), .b(new_n87_), .c(x28), .O(new_n121_));
  nor2   g070(.a(new_n110_), .b(new_n81_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n118_), .c(x16), .O(new_n123_));
  aoi21  g072(.a(new_n56_), .b(x06), .c(x18), .O(new_n124_));
  oai21  g073(.a(new_n123_), .b(new_n121_), .c(new_n124_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n120_), .O(z09));
  inv1   g075(.a(x29), .O(new_n127_));
  oai21  g076(.a(new_n127_), .b(new_n56_), .c(new_n77_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(x24), .O(new_n129_));
  aoi21  g078(.a(new_n122_), .b(new_n118_), .c(new_n127_), .O(new_n130_));
  inv1   g079(.a(x27), .O(new_n131_));
  nand3  g080(.a(new_n127_), .b(new_n118_), .c(new_n131_), .O(new_n132_));
  inv1   g081(.a(new_n132_), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n102_), .c(new_n101_), .d(new_n85_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(x16), .O(new_n135_));
  aoi21  g084(.a(new_n56_), .b(x05), .c(x18), .O(new_n136_));
  oai21  g085(.a(new_n135_), .b(new_n130_), .c(new_n136_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n129_), .O(z10));
  xor2a  g087(.a(new_n134_), .b(x30), .O(new_n139_));
  inv1   g088(.a(x04), .O(new_n140_));
  aoi21  g089(.a(new_n56_), .b(new_n140_), .c(x18), .O(new_n141_));
  oai21  g090(.a(new_n139_), .b(new_n56_), .c(new_n141_), .O(z11));
  nand2  g091(.a(x31), .b(x16), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n77_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(x24), .O(new_n145_));
  nor2   g094(.a(x29), .b(x28), .O(new_n146_));
  nor2   g095(.a(x31), .b(x30), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  inv1   g097(.a(new_n148_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n111_), .O(new_n150_));
  nor3   g099(.a(x27), .b(x26), .c(x25), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n80_), .c(new_n67_), .d(new_n53_), .O(new_n152_));
  inv1   g101(.a(x30), .O(new_n153_));
  nand2  g102(.a(new_n146_), .b(new_n153_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n152_), .c(x31), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n150_), .c(x16), .O(new_n156_));
  aoi21  g105(.a(new_n56_), .b(x03), .c(x18), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n145_), .O(z12));
  inv1   g108(.a(x32), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n56_), .c(new_n77_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(x24), .O(new_n162_));
  aoi21  g111(.a(new_n149_), .b(new_n122_), .c(new_n160_), .O(new_n163_));
  nand3  g112(.a(new_n147_), .b(new_n102_), .c(new_n160_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n132_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n88_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(x16), .O(new_n167_));
  aoi21  g116(.a(new_n56_), .b(x02), .c(x18), .O(new_n168_));
  oai21  g117(.a(new_n167_), .b(new_n163_), .c(new_n168_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n162_), .O(z13));
  inv1   g119(.a(x33), .O(new_n171_));
  aoi21  g120(.a(new_n165_), .b(new_n88_), .c(new_n171_), .O(new_n172_));
  nand3  g121(.a(new_n101_), .b(new_n92_), .c(new_n85_), .O(new_n173_));
  nor2   g122(.a(x33), .b(x32), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n109_), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(new_n149_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n173_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n172_), .c(x16), .O(new_n179_));
  inv1   g128(.a(x01), .O(new_n180_));
  aoi21  g129(.a(new_n56_), .b(new_n180_), .c(x18), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n179_), .O(z14));
  inv1   g131(.a(new_n177_), .O(new_n183_));
  nand3  g132(.a(new_n183_), .b(new_n94_), .c(x34), .O(new_n184_));
  inv1   g133(.a(x34), .O(new_n185_));
  oai21  g134(.a(new_n177_), .b(new_n173_), .c(new_n185_), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n184_), .c(x16), .O(new_n187_));
  inv1   g136(.a(x00), .O(new_n188_));
  aoi21  g137(.a(new_n56_), .b(new_n188_), .c(x18), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n187_), .O(z15));
endmodule


