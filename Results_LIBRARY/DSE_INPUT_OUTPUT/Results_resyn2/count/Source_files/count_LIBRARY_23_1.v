// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x21), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g008(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(x16), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  nand2  g013(.a(x20), .b(x16), .O(new_n65_));
  oai22  g014(.a(new_n65_), .b(new_n53_), .c(x16), .d(x14), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n64_), .c(new_n58_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n57_), .O(z01));
  nor2   g017(.a(x16), .b(x13), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n64_), .c(new_n58_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n57_), .O(z02));
  inv1   g020(.a(x22), .O(new_n72_));
  nor2   g021(.a(new_n62_), .b(new_n72_), .O(new_n73_));
  nor2   g022(.a(x22), .b(x20), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n53_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  aoi21  g025(.a(new_n56_), .b(x12), .c(x21), .O(new_n77_));
  oai21  g026(.a(new_n76_), .b(new_n73_), .c(new_n77_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n57_), .O(z03));
  inv1   g028(.a(x23), .O(new_n80_));
  nand3  g029(.a(new_n74_), .b(new_n53_), .c(new_n80_), .O(new_n81_));
  nand2  g030(.a(new_n75_), .b(x23), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n81_), .c(x16), .O(new_n83_));
  aoi21  g032(.a(new_n56_), .b(x11), .c(x21), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n57_), .O(z04));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n74_), .c(new_n53_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  aoi21  g038(.a(new_n81_), .b(x24), .c(new_n89_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x10), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n59_), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n56_), .c(new_n92_), .O(z05));
  nand2  g042(.a(new_n88_), .b(x25), .O(new_n94_));
  inv1   g043(.a(x20), .O(new_n95_));
  nor2   g044(.a(x25), .b(x22), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n87_), .c(new_n53_), .d(new_n95_), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n94_), .c(x16), .O(new_n98_));
  aoi21  g047(.a(new_n56_), .b(x09), .c(x21), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n57_), .O(z06));
  nor2   g050(.a(x26), .b(x25), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n87_), .c(new_n74_), .d(new_n53_), .O(new_n103_));
  nand2  g052(.a(new_n97_), .b(x26), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n103_), .c(x16), .O(new_n105_));
  aoi21  g054(.a(new_n56_), .b(x08), .c(x21), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n57_), .O(z07));
  inv1   g057(.a(x27), .O(new_n109_));
  nand2  g058(.a(new_n102_), .b(new_n109_), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n88_), .O(new_n111_));
  aoi21  g060(.a(new_n103_), .b(x27), .c(new_n111_), .O(new_n112_));
  nor2   g061(.a(x16), .b(x07), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n59_), .O(new_n114_));
  oai21  g063(.a(new_n112_), .b(new_n56_), .c(new_n114_), .O(z08));
  inv1   g064(.a(x28), .O(new_n116_));
  nor2   g065(.a(new_n111_), .b(new_n116_), .O(new_n117_));
  nor3   g066(.a(x28), .b(x27), .c(x26), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n96_), .c(new_n87_), .d(new_n62_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(x16), .O(new_n120_));
  aoi21  g069(.a(new_n56_), .b(x06), .c(x21), .O(new_n121_));
  oai21  g070(.a(new_n120_), .b(new_n117_), .c(new_n121_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n57_), .O(z09));
  inv1   g072(.a(x24), .O(new_n124_));
  nor2   g073(.a(x29), .b(x28), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n102_), .c(new_n109_), .d(new_n124_), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n81_), .O(new_n127_));
  aoi21  g076(.a(new_n119_), .b(x29), .c(new_n127_), .O(new_n128_));
  nor2   g077(.a(x16), .b(x05), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n59_), .O(new_n130_));
  oai21  g079(.a(new_n128_), .b(new_n56_), .c(new_n130_), .O(z10));
  nor3   g080(.a(x30), .b(x29), .c(x28), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n111_), .O(new_n133_));
  oai21  g082(.a(new_n126_), .b(new_n81_), .c(x30), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n133_), .c(x16), .O(new_n135_));
  aoi21  g084(.a(new_n56_), .b(x04), .c(x21), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n57_), .O(z11));
  nor2   g087(.a(x31), .b(x30), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n125_), .O(new_n140_));
  nor3   g089(.a(new_n140_), .b(new_n110_), .c(new_n88_), .O(new_n141_));
  aoi21  g090(.a(new_n133_), .b(x31), .c(new_n141_), .O(new_n142_));
  nor2   g091(.a(x16), .b(x03), .O(new_n143_));
  nor2   g092(.a(new_n143_), .b(new_n59_), .O(new_n144_));
  oai21  g093(.a(new_n142_), .b(new_n56_), .c(new_n144_), .O(z12));
  inv1   g094(.a(x32), .O(new_n146_));
  inv1   g095(.a(new_n140_), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n111_), .c(new_n146_), .O(new_n148_));
  inv1   g097(.a(x29), .O(new_n149_));
  nand3  g098(.a(new_n139_), .b(new_n146_), .c(new_n149_), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n119_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n148_), .c(x16), .O(new_n152_));
  nor2   g101(.a(x16), .b(x02), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n59_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n152_), .O(z13));
  nand2  g104(.a(new_n96_), .b(new_n87_), .O(new_n156_));
  inv1   g105(.a(new_n156_), .O(new_n157_));
  inv1   g106(.a(new_n150_), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n118_), .c(new_n157_), .d(new_n62_), .O(new_n159_));
  nor2   g108(.a(x27), .b(x26), .O(new_n160_));
  nor2   g109(.a(x33), .b(x32), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n160_), .c(new_n139_), .d(new_n125_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n97_), .O(new_n163_));
  aoi21  g112(.a(new_n159_), .b(x33), .c(new_n163_), .O(new_n164_));
  nor2   g113(.a(x16), .b(x01), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n59_), .O(new_n166_));
  oai21  g115(.a(new_n164_), .b(new_n56_), .c(new_n166_), .O(z14));
  nor2   g116(.a(x34), .b(x33), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n116_), .c(new_n109_), .O(new_n169_));
  nor3   g118(.a(new_n169_), .b(new_n150_), .c(new_n103_), .O(new_n170_));
  oai21  g119(.a(new_n162_), .b(new_n97_), .c(x34), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x16), .O(new_n172_));
  aoi21  g121(.a(new_n56_), .b(x00), .c(x21), .O(new_n173_));
  oai21  g122(.a(new_n172_), .b(new_n170_), .c(new_n173_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n57_), .O(z15));
endmodule


