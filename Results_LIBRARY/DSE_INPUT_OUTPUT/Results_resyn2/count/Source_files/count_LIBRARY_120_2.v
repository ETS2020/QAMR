// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_;
  nand2  g000(.a(x32), .b(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x15), .c(x18), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n52_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nand3  g010(.a(new_n61_), .b(new_n55_), .c(new_n54_), .O(new_n62_));
  oai22  g011(.a(new_n62_), .b(x18), .c(new_n53_), .d(new_n61_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n57_), .b(new_n65_), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nor3   g017(.a(x20), .b(x19), .c(x17), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g019(.a(new_n62_), .b(x21), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  aoi21  g021(.a(new_n57_), .b(x13), .c(x18), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n52_), .O(z02));
  xor2a  g024(.a(new_n70_), .b(x22), .O(new_n76_));
  inv1   g025(.a(x12), .O(new_n77_));
  aoi21  g026(.a(new_n57_), .b(new_n77_), .c(x18), .O(new_n78_));
  oai21  g027(.a(new_n76_), .b(new_n57_), .c(new_n78_), .O(z03));
  inv1   g028(.a(x22), .O(new_n80_));
  nand3  g029(.a(new_n69_), .b(new_n80_), .c(new_n68_), .O(new_n81_));
  inv1   g030(.a(x23), .O(new_n82_));
  nor2   g031(.a(x22), .b(x21), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n62_), .O(new_n85_));
  aoi21  g034(.a(new_n81_), .b(x23), .c(new_n85_), .O(new_n86_));
  inv1   g035(.a(x11), .O(new_n87_));
  aoi21  g036(.a(new_n57_), .b(new_n87_), .c(x18), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n57_), .c(new_n88_), .O(z04));
  inv1   g038(.a(x24), .O(new_n90_));
  nor2   g039(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  nor2   g040(.a(x20), .b(x19), .O(new_n92_));
  nor2   g041(.a(x24), .b(x23), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n83_), .c(new_n92_), .d(new_n54_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(x16), .O(new_n95_));
  aoi21  g044(.a(new_n57_), .b(x10), .c(x18), .O(new_n96_));
  oai21  g045(.a(new_n95_), .b(new_n91_), .c(new_n96_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n52_), .O(z05));
  nor3   g047(.a(x23), .b(x22), .c(x21), .O(new_n99_));
  nor2   g048(.a(x25), .b(x24), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n99_), .c(new_n69_), .O(new_n101_));
  nand2  g050(.a(new_n94_), .b(x25), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n101_), .c(x16), .O(new_n103_));
  aoi21  g052(.a(new_n57_), .b(x09), .c(x18), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n52_), .O(z06));
  nor3   g055(.a(x26), .b(x25), .c(x24), .O(new_n107_));
  aoi22  g056(.a(new_n107_), .b(new_n85_), .c(new_n101_), .d(x26), .O(new_n108_));
  inv1   g057(.a(x08), .O(new_n109_));
  aoi21  g058(.a(new_n57_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n57_), .c(new_n110_), .O(z07));
  nand2  g060(.a(new_n107_), .b(new_n85_), .O(new_n112_));
  inv1   g061(.a(x25), .O(new_n113_));
  nor2   g062(.a(x27), .b(x26), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n94_), .O(new_n116_));
  aoi21  g065(.a(new_n112_), .b(x27), .c(new_n116_), .O(new_n117_));
  inv1   g066(.a(x07), .O(new_n118_));
  aoi21  g067(.a(new_n57_), .b(new_n118_), .c(x18), .O(new_n119_));
  oai21  g068(.a(new_n117_), .b(new_n57_), .c(new_n119_), .O(z08));
  inv1   g069(.a(x28), .O(new_n121_));
  xor2a  g070(.a(new_n116_), .b(new_n121_), .O(new_n122_));
  inv1   g071(.a(x06), .O(new_n123_));
  aoi21  g072(.a(new_n57_), .b(new_n123_), .c(x18), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n57_), .c(new_n124_), .O(z09));
  nand4  g074(.a(new_n90_), .b(new_n82_), .c(new_n80_), .d(new_n68_), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n62_), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(new_n121_), .O(new_n129_));
  nor3   g078(.a(x29), .b(x28), .c(x27), .O(new_n130_));
  nand4  g079(.a(new_n130_), .b(new_n107_), .c(new_n99_), .d(new_n69_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(x16), .O(new_n132_));
  aoi21  g081(.a(new_n129_), .b(x29), .c(new_n132_), .O(new_n133_));
  inv1   g082(.a(x05), .O(new_n134_));
  inv1   g083(.a(x18), .O(new_n135_));
  oai21  g084(.a(x16), .b(new_n134_), .c(new_n135_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n133_), .c(new_n52_), .O(z10));
  nand2  g086(.a(new_n131_), .b(x30), .O(new_n138_));
  inv1   g087(.a(x27), .O(new_n139_));
  inv1   g088(.a(x29), .O(new_n140_));
  inv1   g089(.a(x30), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n140_), .c(new_n121_), .d(new_n139_), .O(new_n142_));
  inv1   g091(.a(new_n142_), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n107_), .c(new_n85_), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n138_), .c(x16), .O(new_n145_));
  aoi21  g094(.a(new_n57_), .b(x04), .c(x18), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n52_), .O(z11));
  nor2   g097(.a(x30), .b(x29), .O(new_n149_));
  nor2   g098(.a(x31), .b(x28), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n149_), .c(new_n114_), .d(new_n113_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n94_), .c(x16), .O(new_n152_));
  aoi21  g101(.a(new_n144_), .b(x31), .c(new_n152_), .O(new_n153_));
  inv1   g102(.a(x03), .O(new_n154_));
  oai21  g103(.a(x16), .b(new_n154_), .c(new_n135_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n153_), .c(new_n52_), .O(z12));
  inv1   g105(.a(x31), .O(new_n157_));
  nand4  g106(.a(new_n149_), .b(new_n92_), .c(new_n157_), .d(x16), .O(new_n158_));
  nor3   g107(.a(x32), .b(x28), .c(x17), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n128_), .c(new_n93_), .d(new_n83_), .O(new_n160_));
  oai22  g109(.a(new_n160_), .b(new_n158_), .c(x16), .d(x02), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n135_), .O(new_n162_));
  nand2  g111(.a(new_n152_), .b(new_n135_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(x32), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n162_), .O(z13));
  inv1   g114(.a(x33), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n57_), .c(new_n135_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(x32), .O(new_n168_));
  nor2   g117(.a(x33), .b(x32), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n150_), .c(new_n149_), .d(new_n114_), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n101_), .O(new_n171_));
  oai21  g120(.a(new_n151_), .b(new_n94_), .c(x33), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x16), .O(new_n173_));
  aoi21  g122(.a(new_n57_), .b(x01), .c(x18), .O(new_n174_));
  oai21  g123(.a(new_n173_), .b(new_n171_), .c(new_n174_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n168_), .O(z14));
  nand2  g125(.a(new_n157_), .b(x16), .O(new_n177_));
  nand4  g126(.a(x34), .b(new_n166_), .c(new_n141_), .d(new_n140_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n177_), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n128_), .c(new_n127_), .d(new_n121_), .O(new_n180_));
  aoi21  g129(.a(new_n180_), .b(new_n135_), .c(x32), .O(new_n181_));
  inv1   g130(.a(x34), .O(new_n182_));
  oai21  g131(.a(new_n170_), .b(new_n101_), .c(new_n182_), .O(new_n183_));
  oai21  g132(.a(x16), .b(x00), .c(new_n135_), .O(new_n184_));
  aoi21  g133(.a(new_n183_), .b(x16), .c(new_n184_), .O(new_n185_));
  nor2   g134(.a(new_n185_), .b(new_n181_), .O(z15));
endmodule


