// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  xor2a  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g008(.a(x14), .O(new_n60_));
  aoi21  g009(.a(new_n52_), .b(new_n60_), .c(x18), .O(new_n61_));
  oai21  g010(.a(new_n59_), .b(new_n52_), .c(new_n61_), .O(z01));
  inv1   g011(.a(x29), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(x18), .O(new_n64_));
  inv1   g013(.a(x21), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n57_), .c(new_n65_), .O(new_n66_));
  nor2   g015(.a(x21), .b(x20), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n58_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(x16), .O(new_n69_));
  aoi21  g018(.a(new_n52_), .b(x13), .c(x18), .O(new_n70_));
  oai21  g019(.a(new_n69_), .b(new_n66_), .c(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n64_), .O(z02));
  inv1   g021(.a(x22), .O(new_n73_));
  nand3  g022(.a(new_n67_), .b(new_n58_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n68_), .b(x22), .O(new_n75_));
  nand3  g024(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  aoi21  g025(.a(new_n52_), .b(x12), .c(x18), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n64_), .O(z03));
  nor2   g028(.a(x23), .b(x22), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n67_), .c(new_n58_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  aoi21  g031(.a(new_n74_), .b(x23), .c(new_n82_), .O(new_n83_));
  inv1   g032(.a(x11), .O(new_n84_));
  aoi21  g033(.a(new_n52_), .b(new_n84_), .c(x18), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n52_), .c(new_n85_), .O(z04));
  inv1   g035(.a(x18), .O(new_n87_));
  nand2  g036(.a(new_n52_), .b(x10), .O(new_n88_));
  inv1   g037(.a(x24), .O(new_n89_));
  nand4  g038(.a(new_n80_), .b(new_n67_), .c(new_n58_), .d(new_n89_), .O(new_n90_));
  nand2  g039(.a(new_n81_), .b(x24), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n88_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n87_), .O(z05));
  nor2   g043(.a(x25), .b(x24), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n80_), .c(new_n67_), .d(new_n58_), .O(new_n96_));
  inv1   g045(.a(new_n96_), .O(new_n97_));
  aoi21  g046(.a(new_n90_), .b(x25), .c(new_n97_), .O(new_n98_));
  inv1   g047(.a(x09), .O(new_n99_));
  aoi21  g048(.a(new_n52_), .b(new_n99_), .c(x18), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n52_), .c(new_n100_), .O(z06));
  nor3   g050(.a(x26), .b(x25), .c(x24), .O(new_n102_));
  aoi22  g051(.a(new_n102_), .b(new_n82_), .c(new_n96_), .d(x26), .O(new_n103_));
  inv1   g052(.a(x08), .O(new_n104_));
  aoi21  g053(.a(new_n52_), .b(new_n104_), .c(x18), .O(new_n105_));
  oai21  g054(.a(new_n103_), .b(new_n52_), .c(new_n105_), .O(z07));
  inv1   g055(.a(x27), .O(new_n107_));
  aoi21  g056(.a(new_n102_), .b(new_n82_), .c(new_n107_), .O(new_n108_));
  nand3  g057(.a(new_n102_), .b(new_n82_), .c(new_n107_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(x16), .O(new_n110_));
  aoi21  g059(.a(new_n52_), .b(x07), .c(x18), .O(new_n111_));
  oai21  g060(.a(new_n110_), .b(new_n108_), .c(new_n111_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n64_), .O(z08));
  inv1   g062(.a(x25), .O(new_n114_));
  inv1   g063(.a(x26), .O(new_n115_));
  nor2   g064(.a(x28), .b(x27), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(new_n117_));
  nor2   g066(.a(x24), .b(x23), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n67_), .c(new_n58_), .d(new_n73_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  aoi21  g069(.a(new_n109_), .b(x28), .c(new_n120_), .O(new_n121_));
  inv1   g070(.a(x06), .O(new_n122_));
  aoi21  g071(.a(new_n52_), .b(new_n122_), .c(x18), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n52_), .c(new_n123_), .O(z09));
  nor3   g073(.a(x29), .b(x28), .c(x27), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n102_), .c(new_n82_), .O(new_n126_));
  oai21  g075(.a(new_n120_), .b(new_n63_), .c(new_n126_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(x16), .O(new_n128_));
  inv1   g077(.a(x05), .O(new_n129_));
  aoi21  g078(.a(new_n52_), .b(new_n129_), .c(x18), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n128_), .O(z10));
  inv1   g080(.a(x30), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n52_), .O(new_n133_));
  nor2   g082(.a(x16), .b(x04), .O(new_n134_));
  aoi21  g083(.a(new_n133_), .b(new_n126_), .c(new_n134_), .O(new_n135_));
  nand4  g084(.a(new_n116_), .b(new_n95_), .c(new_n132_), .d(new_n115_), .O(new_n136_));
  nand4  g085(.a(new_n80_), .b(new_n67_), .c(new_n58_), .d(x16), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n136_), .c(new_n87_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n63_), .O(new_n139_));
  oai21  g088(.a(new_n135_), .b(x18), .c(new_n139_), .O(z11));
  nor2   g089(.a(x30), .b(x29), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n116_), .c(new_n102_), .d(new_n82_), .O(new_n142_));
  nor3   g091(.a(x31), .b(x30), .c(x26), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n125_), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n96_), .O(new_n145_));
  aoi21  g094(.a(new_n142_), .b(x31), .c(new_n145_), .O(new_n146_));
  inv1   g095(.a(x03), .O(new_n147_));
  aoi21  g096(.a(new_n52_), .b(new_n147_), .c(x18), .O(new_n148_));
  oai21  g097(.a(new_n146_), .b(new_n52_), .c(new_n148_), .O(z12));
  inv1   g098(.a(x31), .O(new_n150_));
  inv1   g099(.a(x32), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n150_), .c(x16), .O(new_n152_));
  nor3   g101(.a(new_n152_), .b(new_n136_), .c(new_n81_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(x18), .c(new_n63_), .O(new_n154_));
  nor2   g103(.a(new_n151_), .b(new_n52_), .O(new_n155_));
  oai21  g104(.a(new_n144_), .b(new_n96_), .c(new_n155_), .O(new_n156_));
  inv1   g105(.a(x02), .O(new_n157_));
  nand2  g106(.a(new_n52_), .b(new_n157_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n156_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n87_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n154_), .O(z13));
  inv1   g110(.a(x33), .O(new_n162_));
  inv1   g111(.a(new_n90_), .O(new_n163_));
  nand3  g112(.a(new_n141_), .b(new_n151_), .c(new_n150_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n117_), .O(new_n165_));
  aoi21  g114(.a(new_n165_), .b(new_n163_), .c(new_n162_), .O(new_n166_));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n143_), .c(new_n125_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n96_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n166_), .c(x16), .O(new_n170_));
  inv1   g119(.a(x01), .O(new_n171_));
  aoi21  g120(.a(new_n52_), .b(new_n171_), .c(x18), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n170_), .O(z14));
  and2   g122(.a(x34), .b(x16), .O(new_n174_));
  oai21  g123(.a(new_n168_), .b(new_n96_), .c(new_n174_), .O(new_n175_));
  inv1   g124(.a(x00), .O(new_n176_));
  nand2  g125(.a(new_n52_), .b(new_n176_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n175_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n87_), .O(new_n179_));
  nand4  g128(.a(new_n167_), .b(new_n118_), .c(new_n150_), .d(new_n132_), .O(new_n180_));
  nor2   g129(.a(x34), .b(x22), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n67_), .c(new_n58_), .d(x16), .O(new_n182_));
  nor3   g131(.a(new_n182_), .b(new_n180_), .c(new_n117_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(x18), .c(new_n63_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n179_), .O(z15));
endmodule


