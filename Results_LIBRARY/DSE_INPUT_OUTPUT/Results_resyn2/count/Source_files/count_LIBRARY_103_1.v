// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n151_, new_n152_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x34), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  xor2a  g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  nor2   g010(.a(x16), .b(x14), .O(new_n62_));
  inv1   g011(.a(x34), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n52_), .O(new_n64_));
  nor2   g013(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  oai21  g014(.a(new_n61_), .b(new_n56_), .c(new_n65_), .O(z01));
  nand2  g015(.a(new_n53_), .b(new_n60_), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  aoi22  g017(.a(new_n68_), .b(new_n53_), .c(new_n67_), .d(x21), .O(new_n69_));
  nor2   g018(.a(x16), .b(x13), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n56_), .c(new_n71_), .O(z02));
  nand2  g021(.a(new_n68_), .b(new_n53_), .O(new_n73_));
  xor2a  g022(.a(new_n73_), .b(x22), .O(new_n74_));
  nor2   g023(.a(x16), .b(x12), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(new_n64_), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n56_), .c(new_n76_), .O(z03));
  oai21  g026(.a(new_n73_), .b(x22), .c(x23), .O(new_n78_));
  nor2   g027(.a(x23), .b(x22), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n68_), .c(new_n53_), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n78_), .c(x16), .O(new_n81_));
  aoi21  g030(.a(new_n56_), .b(x11), .c(x34), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n52_), .O(z04));
  xor2a  g033(.a(new_n80_), .b(x24), .O(new_n85_));
  nor2   g034(.a(x16), .b(x10), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n64_), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n56_), .c(new_n87_), .O(z05));
  oai21  g037(.a(new_n80_), .b(x24), .c(x25), .O(new_n89_));
  inv1   g038(.a(new_n80_), .O(new_n90_));
  nor2   g039(.a(x25), .b(x24), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n89_), .c(x16), .O(new_n93_));
  aoi21  g042(.a(new_n56_), .b(x09), .c(x34), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n52_), .O(z06));
  nand2  g045(.a(new_n92_), .b(x26), .O(new_n97_));
  inv1   g046(.a(x26), .O(new_n98_));
  nand2  g047(.a(new_n91_), .b(new_n98_), .O(new_n99_));
  inv1   g048(.a(new_n99_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n90_), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n97_), .c(x16), .O(new_n102_));
  aoi21  g051(.a(new_n56_), .b(x08), .c(x34), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n52_), .O(z07));
  inv1   g054(.a(x27), .O(new_n106_));
  nor2   g055(.a(new_n99_), .b(new_n80_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g057(.a(new_n101_), .b(x27), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n108_), .c(x16), .O(new_n110_));
  aoi21  g059(.a(new_n56_), .b(x07), .c(x34), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n52_), .O(z08));
  inv1   g062(.a(x28), .O(new_n114_));
  aoi21  g063(.a(new_n107_), .b(new_n106_), .c(new_n114_), .O(new_n115_));
  nor2   g064(.a(x28), .b(x27), .O(new_n116_));
  inv1   g065(.a(new_n116_), .O(new_n117_));
  nor3   g066(.a(new_n117_), .b(new_n99_), .c(new_n80_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n115_), .c(x16), .O(new_n119_));
  nor2   g068(.a(x16), .b(x06), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n64_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n119_), .O(z09));
  inv1   g071(.a(x29), .O(new_n123_));
  xor2a  g072(.a(new_n118_), .b(new_n123_), .O(new_n124_));
  nor2   g073(.a(x16), .b(x05), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n64_), .O(new_n126_));
  oai21  g075(.a(new_n124_), .b(new_n56_), .c(new_n126_), .O(z10));
  nand4  g076(.a(new_n116_), .b(new_n100_), .c(new_n90_), .d(new_n123_), .O(new_n128_));
  nor2   g077(.a(x30), .b(x29), .O(new_n129_));
  aoi22  g078(.a(new_n129_), .b(new_n118_), .c(new_n128_), .d(x30), .O(new_n130_));
  nor2   g079(.a(x16), .b(x04), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n64_), .O(new_n132_));
  oai21  g081(.a(new_n130_), .b(new_n56_), .c(new_n132_), .O(z11));
  nand3  g082(.a(new_n129_), .b(new_n116_), .c(new_n107_), .O(new_n134_));
  inv1   g083(.a(x31), .O(new_n135_));
  nand3  g084(.a(new_n129_), .b(new_n116_), .c(new_n135_), .O(new_n136_));
  nor3   g085(.a(new_n136_), .b(new_n99_), .c(new_n80_), .O(new_n137_));
  aoi21  g086(.a(new_n134_), .b(x31), .c(new_n137_), .O(new_n138_));
  nor2   g087(.a(x16), .b(x03), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n64_), .O(new_n140_));
  oai21  g089(.a(new_n138_), .b(new_n56_), .c(new_n140_), .O(z12));
  inv1   g090(.a(x32), .O(new_n142_));
  nand3  g091(.a(new_n129_), .b(new_n142_), .c(new_n135_), .O(new_n143_));
  inv1   g092(.a(new_n143_), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n116_), .c(new_n100_), .d(new_n90_), .O(new_n145_));
  oai21  g094(.a(new_n137_), .b(new_n142_), .c(new_n145_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(x16), .O(new_n147_));
  nor2   g096(.a(x16), .b(x02), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n64_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n147_), .O(z13));
  nor3   g099(.a(x34), .b(x33), .c(x32), .O(new_n151_));
  aoi22  g100(.a(new_n151_), .b(new_n137_), .c(new_n145_), .d(x33), .O(new_n152_));
  nor2   g101(.a(x16), .b(x01), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n64_), .O(new_n154_));
  oai21  g103(.a(new_n152_), .b(new_n56_), .c(new_n154_), .O(z14));
  inv1   g104(.a(x00), .O(new_n156_));
  nand2  g105(.a(new_n56_), .b(new_n156_), .O(new_n157_));
  inv1   g106(.a(x33), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x16), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n145_), .c(new_n157_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n63_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n52_), .O(z15));
endmodule


