// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  inv1   g000(.a(x13), .O(new_n52_));
  nand2  g001(.a(x18), .b(new_n52_), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(x19), .b(x17), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x15), .c(x18), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n54_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n53_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  nand2  g010(.a(new_n57_), .b(x14), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  oai21  g013(.a(new_n54_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n61_), .O(z01));
  nand2  g016(.a(new_n54_), .b(new_n64_), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  aoi22  g018(.a(new_n69_), .b(new_n54_), .c(new_n68_), .d(x21), .O(new_n70_));
  aoi21  g019(.a(new_n57_), .b(new_n52_), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n70_), .b(new_n57_), .c(new_n71_), .O(z02));
  inv1   g021(.a(x22), .O(new_n73_));
  nand3  g022(.a(new_n69_), .b(new_n54_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n69_), .b(new_n54_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x22), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n74_), .c(x16), .O(new_n77_));
  aoi21  g026(.a(new_n57_), .b(x12), .c(x18), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n53_), .O(z03));
  nor2   g029(.a(x23), .b(x22), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n69_), .c(new_n54_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  aoi21  g032(.a(new_n74_), .b(x23), .c(new_n83_), .O(new_n84_));
  inv1   g033(.a(x11), .O(new_n85_));
  aoi21  g034(.a(new_n57_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n57_), .c(new_n86_), .O(z04));
  nor2   g036(.a(x24), .b(x21), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n81_), .c(new_n54_), .d(new_n64_), .O(new_n89_));
  inv1   g038(.a(new_n89_), .O(new_n90_));
  aoi21  g039(.a(new_n82_), .b(x24), .c(new_n90_), .O(new_n91_));
  inv1   g040(.a(x10), .O(new_n92_));
  aoi21  g041(.a(new_n57_), .b(new_n92_), .c(x18), .O(new_n93_));
  oai21  g042(.a(new_n91_), .b(new_n57_), .c(new_n93_), .O(z05));
  nor2   g043(.a(x25), .b(x24), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n81_), .c(new_n69_), .d(new_n54_), .O(new_n96_));
  inv1   g045(.a(new_n96_), .O(new_n97_));
  aoi21  g046(.a(new_n89_), .b(x25), .c(new_n97_), .O(new_n98_));
  inv1   g047(.a(x09), .O(new_n99_));
  aoi21  g048(.a(new_n57_), .b(new_n99_), .c(x18), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n57_), .c(new_n100_), .O(z06));
  inv1   g050(.a(x24), .O(new_n102_));
  inv1   g051(.a(x25), .O(new_n103_));
  inv1   g052(.a(x26), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n82_), .O(new_n106_));
  oai21  g055(.a(new_n97_), .b(new_n104_), .c(x16), .O(new_n107_));
  aoi21  g056(.a(new_n57_), .b(x08), .c(x18), .O(new_n108_));
  oai21  g057(.a(new_n107_), .b(new_n106_), .c(new_n108_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n53_), .O(z07));
  inv1   g059(.a(x27), .O(new_n111_));
  nor3   g060(.a(x27), .b(x26), .c(x25), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n88_), .c(new_n81_), .d(new_n63_), .O(new_n113_));
  oai21  g062(.a(new_n106_), .b(new_n111_), .c(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g064(.a(x07), .O(new_n116_));
  aoi21  g065(.a(new_n57_), .b(new_n116_), .c(x18), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n115_), .O(z08));
  nor2   g067(.a(x28), .b(x27), .O(new_n119_));
  aoi22  g068(.a(new_n119_), .b(new_n106_), .c(new_n113_), .d(x28), .O(new_n120_));
  inv1   g069(.a(x06), .O(new_n121_));
  aoi21  g070(.a(new_n57_), .b(new_n121_), .c(x18), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n57_), .c(new_n122_), .O(z09));
  nand2  g072(.a(new_n119_), .b(new_n106_), .O(new_n124_));
  nor2   g073(.a(x26), .b(x25), .O(new_n125_));
  nor2   g074(.a(x29), .b(x28), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n125_), .c(new_n111_), .d(new_n102_), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n82_), .O(new_n128_));
  aoi21  g077(.a(new_n124_), .b(x29), .c(new_n128_), .O(new_n129_));
  inv1   g078(.a(x05), .O(new_n130_));
  aoi21  g079(.a(new_n57_), .b(new_n130_), .c(x18), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n57_), .c(new_n131_), .O(z10));
  inv1   g081(.a(x30), .O(new_n133_));
  inv1   g082(.a(x28), .O(new_n134_));
  inv1   g083(.a(x29), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n134_), .c(new_n111_), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n105_), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n83_), .c(new_n133_), .O(new_n138_));
  oai21  g087(.a(new_n127_), .b(new_n82_), .c(x30), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n138_), .c(x16), .O(new_n140_));
  aoi21  g089(.a(new_n57_), .b(x04), .c(x18), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n53_), .O(z11));
  nor2   g092(.a(x27), .b(x26), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n103_), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n89_), .O(new_n146_));
  nor2   g095(.a(x31), .b(x30), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n126_), .O(new_n148_));
  inv1   g097(.a(new_n148_), .O(new_n149_));
  aoi22  g098(.a(new_n149_), .b(new_n146_), .c(new_n138_), .d(x31), .O(new_n150_));
  inv1   g099(.a(x03), .O(new_n151_));
  aoi21  g100(.a(new_n57_), .b(new_n151_), .c(x18), .O(new_n152_));
  oai21  g101(.a(new_n150_), .b(new_n57_), .c(new_n152_), .O(z12));
  nand3  g102(.a(new_n149_), .b(new_n112_), .c(new_n90_), .O(new_n154_));
  nor3   g103(.a(x29), .b(x28), .c(x27), .O(new_n155_));
  nor3   g104(.a(x32), .b(x31), .c(x30), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n155_), .c(new_n125_), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n89_), .O(new_n158_));
  aoi21  g107(.a(new_n154_), .b(x32), .c(new_n158_), .O(new_n159_));
  inv1   g108(.a(x02), .O(new_n160_));
  aoi21  g109(.a(new_n57_), .b(new_n160_), .c(x18), .O(new_n161_));
  oai21  g110(.a(new_n159_), .b(new_n57_), .c(new_n161_), .O(z13));
  inv1   g111(.a(new_n125_), .O(new_n163_));
  inv1   g112(.a(x32), .O(new_n164_));
  nand2  g113(.a(new_n147_), .b(new_n164_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n163_), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n155_), .c(new_n90_), .O(new_n167_));
  nor2   g116(.a(x33), .b(x32), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n147_), .c(new_n126_), .d(new_n144_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n96_), .c(x16), .O(new_n170_));
  aoi21  g119(.a(new_n167_), .b(x33), .c(new_n170_), .O(new_n171_));
  inv1   g120(.a(x01), .O(new_n172_));
  oai21  g121(.a(x16), .b(new_n172_), .c(new_n61_), .O(new_n173_));
  oai21  g122(.a(new_n173_), .b(new_n171_), .c(new_n53_), .O(z14));
  nor3   g123(.a(new_n169_), .b(new_n96_), .c(x34), .O(new_n175_));
  oai21  g124(.a(new_n169_), .b(new_n96_), .c(x34), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  aoi21  g126(.a(new_n57_), .b(x00), .c(x18), .O(new_n178_));
  oai21  g127(.a(new_n177_), .b(new_n175_), .c(new_n178_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n53_), .O(z15));
endmodule


