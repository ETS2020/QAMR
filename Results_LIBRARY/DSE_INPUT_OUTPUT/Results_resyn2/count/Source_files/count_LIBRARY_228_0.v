// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  nand2  g008(.a(x29), .b(new_n59_), .O(new_n60_));
  aoi21  g009(.a(new_n58_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n56_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n55_), .b(new_n63_), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n60_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n66_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nand4  g019(.a(new_n70_), .b(new_n63_), .c(new_n53_), .d(new_n52_), .O(new_n71_));
  nand3  g020(.a(new_n63_), .b(new_n53_), .c(new_n52_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x21), .O(new_n73_));
  nand3  g022(.a(new_n73_), .b(new_n71_), .c(x16), .O(new_n74_));
  inv1   g023(.a(x29), .O(new_n75_));
  aoi21  g024(.a(new_n58_), .b(x13), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n59_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n70_), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n72_), .O(new_n81_));
  aoi21  g030(.a(new_n71_), .b(x22), .c(new_n81_), .O(new_n82_));
  nor2   g031(.a(x16), .b(x12), .O(new_n83_));
  nor2   g032(.a(new_n83_), .b(new_n60_), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n58_), .c(new_n84_), .O(z03));
  inv1   g034(.a(x23), .O(new_n86_));
  xor2a  g035(.a(new_n81_), .b(new_n86_), .O(new_n87_));
  nor2   g036(.a(x16), .b(x11), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n60_), .O(new_n89_));
  oai21  g038(.a(new_n87_), .b(new_n58_), .c(new_n89_), .O(z04));
  inv1   g039(.a(x24), .O(new_n91_));
  aoi21  g040(.a(new_n81_), .b(new_n86_), .c(new_n91_), .O(new_n92_));
  nor2   g041(.a(x22), .b(x21), .O(new_n93_));
  nor2   g042(.a(x24), .b(x23), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n93_), .c(new_n65_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x16), .O(new_n96_));
  aoi21  g045(.a(new_n58_), .b(x10), .c(new_n75_), .O(new_n97_));
  oai21  g046(.a(new_n96_), .b(new_n92_), .c(new_n97_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n59_), .O(z05));
  nand2  g048(.a(new_n95_), .b(x25), .O(new_n100_));
  inv1   g049(.a(new_n71_), .O(new_n101_));
  inv1   g050(.a(x25), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n91_), .c(new_n86_), .d(new_n79_), .O(new_n103_));
  inv1   g052(.a(new_n103_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n101_), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n100_), .c(x16), .O(new_n106_));
  aoi21  g055(.a(new_n58_), .b(x09), .c(new_n75_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n59_), .O(z06));
  oai21  g058(.a(new_n103_), .b(new_n71_), .c(x26), .O(new_n110_));
  inv1   g059(.a(x26), .O(new_n111_));
  nand3  g060(.a(new_n104_), .b(new_n101_), .c(new_n111_), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n110_), .c(x16), .O(new_n113_));
  aoi21  g062(.a(new_n58_), .b(x08), .c(new_n75_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n59_), .O(z07));
  inv1   g065(.a(x27), .O(new_n117_));
  nand4  g066(.a(new_n93_), .b(new_n55_), .c(new_n117_), .d(new_n63_), .O(new_n118_));
  nor2   g067(.a(x26), .b(x25), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n94_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  aoi21  g070(.a(new_n112_), .b(x27), .c(new_n121_), .O(new_n122_));
  nor2   g071(.a(x16), .b(x07), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n60_), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n58_), .c(new_n124_), .O(z08));
  oai21  g074(.a(new_n120_), .b(new_n118_), .c(x28), .O(new_n126_));
  inv1   g075(.a(x28), .O(new_n127_));
  nand3  g076(.a(new_n119_), .b(new_n94_), .c(new_n127_), .O(new_n128_));
  inv1   g077(.a(new_n128_), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n81_), .c(new_n117_), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n126_), .c(x16), .O(new_n131_));
  aoi21  g080(.a(new_n58_), .b(x06), .c(new_n75_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n59_), .O(z09));
  nand2  g083(.a(new_n130_), .b(x16), .O(new_n135_));
  nor2   g084(.a(x16), .b(x05), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n60_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n135_), .O(z10));
  nand2  g087(.a(x30), .b(x16), .O(new_n139_));
  inv1   g088(.a(x04), .O(new_n140_));
  aoi21  g089(.a(new_n58_), .b(new_n140_), .c(new_n60_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n139_), .O(z11));
  nand2  g091(.a(new_n58_), .b(x03), .O(new_n143_));
  inv1   g092(.a(x31), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(x16), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n143_), .c(x29), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n59_), .O(z12));
  nand2  g096(.a(new_n58_), .b(x02), .O(new_n148_));
  inv1   g097(.a(x32), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n148_), .c(x29), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n59_), .O(z13));
  nand2  g101(.a(new_n58_), .b(x01), .O(new_n153_));
  inv1   g102(.a(x33), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(x16), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n153_), .c(x29), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n59_), .O(z14));
  nand2  g106(.a(new_n58_), .b(x00), .O(new_n158_));
  inv1   g107(.a(x34), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(x16), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n158_), .c(x29), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n59_), .O(z15));
endmodule


