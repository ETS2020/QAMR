// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x25), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x18), .O(new_n58_));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(x19), .b(x17), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  oai21  g010(.a(x19), .b(x17), .c(x20), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  aoi21  g012(.a(new_n52_), .b(x14), .c(x18), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n58_), .O(z01));
  nor2   g015(.a(x21), .b(x20), .O(new_n67_));
  aoi22  g016(.a(new_n67_), .b(new_n60_), .c(new_n61_), .d(x21), .O(new_n68_));
  inv1   g017(.a(x13), .O(new_n69_));
  aoi21  g018(.a(new_n52_), .b(new_n69_), .c(x18), .O(new_n70_));
  oai21  g019(.a(new_n68_), .b(new_n52_), .c(new_n70_), .O(z02));
  inv1   g020(.a(x18), .O(new_n72_));
  nand2  g021(.a(new_n52_), .b(x12), .O(new_n73_));
  inv1   g022(.a(x22), .O(new_n74_));
  nand3  g023(.a(new_n67_), .b(new_n60_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n67_), .b(new_n60_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x22), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(x16), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n73_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n72_), .O(z03));
  nand2  g029(.a(new_n75_), .b(x23), .O(new_n81_));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n67_), .c(new_n60_), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n81_), .c(x16), .O(new_n84_));
  aoi21  g033(.a(new_n52_), .b(x11), .c(x18), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n58_), .O(z04));
  inv1   g036(.a(x24), .O(new_n88_));
  nand4  g037(.a(new_n82_), .b(new_n67_), .c(new_n60_), .d(new_n88_), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n52_), .O(new_n90_));
  nand3  g039(.a(new_n83_), .b(x24), .c(x16), .O(new_n91_));
  oai21  g040(.a(x16), .b(x10), .c(new_n91_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n90_), .c(new_n72_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n58_), .O(z05));
  inv1   g043(.a(x23), .O(new_n95_));
  nand2  g044(.a(new_n88_), .b(new_n95_), .O(new_n96_));
  nor3   g045(.a(new_n96_), .b(new_n75_), .c(x25), .O(new_n97_));
  aoi21  g046(.a(new_n89_), .b(x25), .c(new_n97_), .O(new_n98_));
  inv1   g047(.a(x09), .O(new_n99_));
  aoi21  g048(.a(new_n52_), .b(new_n99_), .c(x18), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n52_), .c(new_n100_), .O(z06));
  inv1   g050(.a(new_n75_), .O(new_n102_));
  nor2   g051(.a(new_n96_), .b(x25), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  xor2a  g053(.a(new_n104_), .b(x26), .O(new_n105_));
  inv1   g054(.a(x08), .O(new_n106_));
  aoi21  g055(.a(new_n52_), .b(new_n106_), .c(x18), .O(new_n107_));
  oai21  g056(.a(new_n105_), .b(new_n52_), .c(new_n107_), .O(z07));
  nor2   g057(.a(x27), .b(x26), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(x16), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n89_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(x18), .c(new_n57_), .O(new_n112_));
  inv1   g061(.a(x26), .O(new_n113_));
  nand3  g062(.a(new_n103_), .b(new_n102_), .c(new_n113_), .O(new_n114_));
  inv1   g063(.a(x27), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n52_), .O(new_n116_));
  nor2   g065(.a(x16), .b(x07), .O(new_n117_));
  aoi21  g066(.a(new_n116_), .b(new_n114_), .c(new_n117_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(x18), .c(new_n112_), .O(z08));
  inv1   g068(.a(x28), .O(new_n120_));
  aoi21  g069(.a(new_n111_), .b(new_n120_), .c(x18), .O(new_n121_));
  nor2   g070(.a(x24), .b(x23), .O(new_n122_));
  nand3  g071(.a(new_n109_), .b(new_n122_), .c(new_n57_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n75_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n120_), .c(x16), .O(new_n125_));
  aoi21  g074(.a(new_n52_), .b(x06), .c(x18), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  oai21  g076(.a(new_n121_), .b(x25), .c(new_n127_), .O(z09));
  nand2  g077(.a(new_n124_), .b(new_n120_), .O(new_n129_));
  inv1   g078(.a(x29), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n52_), .O(new_n131_));
  nor2   g080(.a(x16), .b(x05), .O(new_n132_));
  aoi21  g081(.a(new_n131_), .b(new_n129_), .c(new_n132_), .O(new_n133_));
  inv1   g082(.a(x21), .O(new_n134_));
  nand4  g083(.a(new_n74_), .b(new_n134_), .c(new_n59_), .d(x16), .O(new_n135_));
  nor2   g084(.a(x29), .b(x28), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n109_), .c(new_n122_), .d(new_n60_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n135_), .c(new_n72_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n57_), .O(new_n139_));
  oai21  g088(.a(new_n133_), .b(x18), .c(new_n139_), .O(z10));
  nor2   g089(.a(x26), .b(x25), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n136_), .c(new_n115_), .d(new_n88_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n83_), .c(x30), .O(new_n143_));
  inv1   g092(.a(x30), .O(new_n144_));
  inv1   g093(.a(new_n83_), .O(new_n145_));
  nand3  g094(.a(new_n130_), .b(new_n120_), .c(new_n115_), .O(new_n146_));
  nand3  g095(.a(new_n113_), .b(new_n57_), .c(new_n88_), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n145_), .c(new_n144_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n143_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  inv1   g100(.a(x04), .O(new_n152_));
  aoi21  g101(.a(new_n52_), .b(new_n152_), .c(x18), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n151_), .O(z11));
  nor2   g103(.a(x31), .b(x30), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n136_), .c(new_n109_), .O(new_n156_));
  inv1   g105(.a(new_n156_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n90_), .c(x18), .O(new_n158_));
  and2   g107(.a(x31), .b(x16), .O(new_n159_));
  nor2   g108(.a(x16), .b(x03), .O(new_n160_));
  aoi21  g109(.a(new_n159_), .b(new_n149_), .c(new_n160_), .O(new_n161_));
  oai22  g110(.a(new_n161_), .b(x18), .c(new_n158_), .d(x25), .O(z12));
  nand4  g111(.a(new_n155_), .b(new_n136_), .c(new_n109_), .d(new_n57_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n89_), .c(x32), .O(new_n164_));
  inv1   g113(.a(new_n89_), .O(new_n165_));
  inv1   g114(.a(x32), .O(new_n166_));
  nand3  g115(.a(new_n155_), .b(new_n141_), .c(new_n166_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n146_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n164_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x16), .O(new_n171_));
  inv1   g120(.a(x02), .O(new_n172_));
  aoi21  g121(.a(new_n52_), .b(new_n172_), .c(x18), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z13));
  nor2   g123(.a(x33), .b(x32), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n155_), .c(new_n136_), .d(new_n109_), .O(new_n176_));
  nor3   g125(.a(new_n176_), .b(new_n89_), .c(new_n52_), .O(new_n177_));
  oai21  g126(.a(new_n177_), .b(x18), .c(new_n57_), .O(new_n178_));
  nand2  g127(.a(x33), .b(x16), .O(new_n179_));
  aoi21  g128(.a(new_n168_), .b(new_n165_), .c(new_n179_), .O(new_n180_));
  nor2   g129(.a(x16), .b(x01), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n180_), .c(new_n72_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n178_), .O(z14));
  inv1   g132(.a(x34), .O(new_n184_));
  oai21  g133(.a(new_n176_), .b(new_n104_), .c(new_n184_), .O(new_n185_));
  inv1   g134(.a(new_n176_), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n97_), .c(x34), .O(new_n187_));
  nand3  g136(.a(new_n187_), .b(new_n185_), .c(x16), .O(new_n188_));
  inv1   g137(.a(x00), .O(new_n189_));
  aoi21  g138(.a(new_n52_), .b(new_n189_), .c(x18), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n188_), .O(z15));
endmodule


