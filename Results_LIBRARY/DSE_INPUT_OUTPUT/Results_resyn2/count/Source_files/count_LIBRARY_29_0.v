// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  nand2  g006(.a(x31), .b(new_n57_), .O(new_n58_));
  aoi21  g007(.a(new_n56_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n54_), .O(z00));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  inv1   g010(.a(x20), .O(new_n62_));
  oai21  g011(.a(new_n53_), .b(new_n62_), .c(x16), .O(new_n63_));
  inv1   g012(.a(x31), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n63_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n57_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  xor2a  g017(.a(new_n61_), .b(new_n68_), .O(new_n69_));
  nor2   g018(.a(x16), .b(x13), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n56_), .c(new_n71_), .O(z02));
  nand2  g021(.a(new_n61_), .b(new_n68_), .O(new_n73_));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  aoi22  g023(.a(new_n74_), .b(new_n61_), .c(new_n73_), .d(x22), .O(new_n75_));
  nor2   g024(.a(x16), .b(x12), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n58_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n56_), .c(new_n77_), .O(z03));
  inv1   g027(.a(x23), .O(new_n79_));
  nand4  g028(.a(new_n74_), .b(new_n53_), .c(new_n79_), .d(new_n62_), .O(new_n80_));
  nand3  g029(.a(new_n74_), .b(new_n53_), .c(new_n62_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x23), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  aoi21  g032(.a(new_n56_), .b(x11), .c(new_n64_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n57_), .O(z04));
  inv1   g035(.a(x24), .O(new_n87_));
  inv1   g036(.a(new_n80_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  aoi21  g038(.a(new_n80_), .b(x24), .c(new_n56_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  aoi21  g040(.a(new_n56_), .b(x10), .c(new_n64_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n57_), .O(z05));
  nor2   g043(.a(x25), .b(x24), .O(new_n95_));
  aoi22  g044(.a(new_n95_), .b(new_n88_), .c(new_n89_), .d(x25), .O(new_n96_));
  nor2   g045(.a(x16), .b(x09), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n58_), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n56_), .c(new_n98_), .O(z06));
  nand2  g048(.a(new_n95_), .b(new_n88_), .O(new_n100_));
  nor3   g049(.a(x26), .b(x25), .c(x24), .O(new_n101_));
  aoi22  g050(.a(new_n101_), .b(new_n88_), .c(new_n100_), .d(x26), .O(new_n102_));
  nor2   g051(.a(x16), .b(x08), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n58_), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n56_), .c(new_n104_), .O(z07));
  nand4  g054(.a(new_n101_), .b(new_n74_), .c(new_n61_), .d(new_n79_), .O(new_n106_));
  inv1   g055(.a(x26), .O(new_n107_));
  nor2   g056(.a(x27), .b(x23), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n95_), .c(new_n107_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n81_), .O(new_n110_));
  aoi21  g059(.a(new_n106_), .b(x27), .c(new_n110_), .O(new_n111_));
  nor2   g060(.a(x16), .b(x07), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n58_), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n56_), .c(new_n113_), .O(z08));
  inv1   g063(.a(x28), .O(new_n115_));
  nand2  g064(.a(new_n110_), .b(new_n115_), .O(new_n116_));
  oai21  g065(.a(new_n109_), .b(new_n81_), .c(x28), .O(new_n117_));
  nand3  g066(.a(new_n117_), .b(new_n116_), .c(x16), .O(new_n118_));
  aoi21  g067(.a(new_n56_), .b(x06), .c(new_n64_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n57_), .O(z09));
  inv1   g070(.a(x29), .O(new_n122_));
  aoi21  g071(.a(new_n110_), .b(new_n115_), .c(new_n122_), .O(new_n123_));
  nor3   g072(.a(x29), .b(x28), .c(x27), .O(new_n124_));
  inv1   g073(.a(new_n124_), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n106_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n123_), .c(x16), .O(new_n127_));
  nor2   g076(.a(x16), .b(x05), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n58_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n127_), .O(z10));
  nand3  g079(.a(new_n124_), .b(new_n101_), .c(new_n88_), .O(new_n131_));
  xor2a  g080(.a(new_n131_), .b(x30), .O(new_n132_));
  nor2   g081(.a(x16), .b(x04), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n58_), .O(new_n134_));
  oai21  g083(.a(new_n132_), .b(new_n56_), .c(new_n134_), .O(z11));
  inv1   g084(.a(x30), .O(new_n136_));
  nand3  g085(.a(new_n126_), .b(new_n136_), .c(x16), .O(new_n137_));
  aoi21  g086(.a(new_n56_), .b(x03), .c(new_n64_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n57_), .O(z12));
  nand2  g089(.a(new_n56_), .b(x02), .O(new_n141_));
  inv1   g090(.a(x32), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(x16), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n141_), .c(x31), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n57_), .O(z13));
  nand2  g094(.a(x33), .b(x16), .O(new_n146_));
  inv1   g095(.a(x01), .O(new_n147_));
  aoi21  g096(.a(new_n56_), .b(new_n147_), .c(new_n58_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n146_), .O(z14));
  nand2  g098(.a(new_n56_), .b(x00), .O(new_n150_));
  inv1   g099(.a(x34), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(x16), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n150_), .c(x31), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n57_), .O(z15));
endmodule


