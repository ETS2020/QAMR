// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  xor2a  g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  inv1   g011(.a(x14), .O(new_n63_));
  aoi21  g012(.a(new_n58_), .b(new_n63_), .c(x18), .O(new_n64_));
  oai21  g013(.a(new_n62_), .b(new_n58_), .c(new_n64_), .O(z01));
  nand3  g014(.a(new_n61_), .b(new_n53_), .c(new_n52_), .O(new_n66_));
  nor2   g015(.a(x21), .b(x20), .O(new_n67_));
  aoi22  g016(.a(new_n67_), .b(new_n55_), .c(new_n66_), .d(x21), .O(new_n68_));
  inv1   g017(.a(x13), .O(new_n69_));
  aoi21  g018(.a(new_n58_), .b(new_n69_), .c(x18), .O(new_n70_));
  oai21  g019(.a(new_n68_), .b(new_n58_), .c(new_n70_), .O(z02));
  inv1   g020(.a(x22), .O(new_n72_));
  aoi21  g021(.a(new_n67_), .b(new_n55_), .c(new_n72_), .O(new_n73_));
  nand3  g022(.a(new_n67_), .b(new_n55_), .c(new_n72_), .O(new_n74_));
  inv1   g023(.a(new_n74_), .O(new_n75_));
  oai21  g024(.a(new_n75_), .b(new_n73_), .c(x16), .O(new_n76_));
  inv1   g025(.a(x12), .O(new_n77_));
  aoi21  g026(.a(new_n58_), .b(new_n77_), .c(x18), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n76_), .O(z03));
  nor2   g028(.a(x23), .b(x22), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n67_), .c(new_n55_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  aoi21  g031(.a(new_n74_), .b(x23), .c(new_n82_), .O(new_n83_));
  inv1   g032(.a(x11), .O(new_n84_));
  aoi21  g033(.a(new_n58_), .b(new_n84_), .c(x18), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n58_), .c(new_n85_), .O(z04));
  nand2  g035(.a(x18), .b(x10), .O(new_n87_));
  nand2  g036(.a(new_n81_), .b(x24), .O(new_n88_));
  nor2   g037(.a(x24), .b(x23), .O(new_n89_));
  nor2   g038(.a(x22), .b(x21), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n89_), .c(new_n55_), .d(new_n61_), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n88_), .c(x16), .O(new_n92_));
  aoi21  g041(.a(new_n58_), .b(x10), .c(x18), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n87_), .O(z05));
  nand2  g044(.a(new_n91_), .b(x25), .O(new_n96_));
  nor2   g045(.a(x25), .b(x24), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n80_), .c(new_n67_), .d(new_n55_), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n96_), .c(x16), .O(new_n99_));
  aoi21  g048(.a(new_n58_), .b(x09), .c(x18), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n87_), .O(z06));
  nand2  g051(.a(new_n98_), .b(x26), .O(new_n103_));
  inv1   g052(.a(x24), .O(new_n104_));
  nor2   g053(.a(x26), .b(x25), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n82_), .c(new_n104_), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(new_n103_), .c(x16), .O(new_n107_));
  aoi21  g056(.a(new_n58_), .b(x08), .c(x18), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n87_), .O(z07));
  nand2  g059(.a(new_n105_), .b(new_n104_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n81_), .c(x27), .O(new_n112_));
  inv1   g061(.a(x21), .O(new_n113_));
  inv1   g062(.a(x23), .O(new_n114_));
  nand4  g063(.a(new_n104_), .b(new_n114_), .c(new_n72_), .d(new_n113_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n66_), .O(new_n116_));
  inv1   g065(.a(x25), .O(new_n117_));
  inv1   g066(.a(x26), .O(new_n118_));
  inv1   g067(.a(x27), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  inv1   g069(.a(new_n120_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n116_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n112_), .c(x16), .O(new_n123_));
  aoi21  g072(.a(new_n58_), .b(x07), .c(x18), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n87_), .O(z08));
  inv1   g075(.a(x28), .O(new_n127_));
  nand4  g076(.a(new_n105_), .b(new_n89_), .c(new_n127_), .d(new_n119_), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n74_), .O(new_n129_));
  aoi21  g078(.a(new_n122_), .b(x28), .c(new_n129_), .O(new_n130_));
  inv1   g079(.a(x06), .O(new_n131_));
  aoi21  g080(.a(new_n58_), .b(new_n131_), .c(x18), .O(new_n132_));
  oai21  g081(.a(new_n130_), .b(new_n58_), .c(new_n132_), .O(z09));
  oai21  g082(.a(new_n128_), .b(new_n74_), .c(x29), .O(new_n134_));
  inv1   g083(.a(x29), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n127_), .c(new_n104_), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n120_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n82_), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n134_), .c(x16), .O(new_n139_));
  aoi21  g088(.a(new_n58_), .b(x05), .c(x18), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n87_), .O(z10));
  nor2   g091(.a(x29), .b(x28), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n105_), .c(new_n119_), .d(new_n104_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n81_), .c(x30), .O(new_n145_));
  inv1   g094(.a(x30), .O(new_n146_));
  nand3  g095(.a(new_n137_), .b(new_n82_), .c(new_n146_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n145_), .c(x16), .O(new_n148_));
  aoi21  g097(.a(new_n58_), .b(x04), .c(x18), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n87_), .O(z11));
  nor2   g100(.a(x31), .b(x30), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n143_), .c(new_n105_), .d(new_n119_), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n91_), .O(new_n154_));
  aoi21  g103(.a(new_n147_), .b(x31), .c(new_n154_), .O(new_n155_));
  inv1   g104(.a(x03), .O(new_n156_));
  aoi21  g105(.a(new_n58_), .b(new_n156_), .c(x18), .O(new_n157_));
  oai21  g106(.a(new_n155_), .b(new_n58_), .c(new_n157_), .O(z12));
  inv1   g107(.a(x32), .O(new_n159_));
  inv1   g108(.a(x31), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n146_), .c(new_n135_), .d(new_n127_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n120_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n116_), .c(new_n159_), .O(new_n163_));
  oai21  g112(.a(new_n153_), .b(new_n91_), .c(x32), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n163_), .c(x16), .O(new_n165_));
  aoi21  g114(.a(new_n58_), .b(x02), .c(x18), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n87_), .O(z13));
  nor2   g117(.a(x27), .b(x26), .O(new_n169_));
  nor2   g118(.a(x33), .b(x32), .O(new_n170_));
  nand4  g119(.a(new_n170_), .b(new_n169_), .c(new_n152_), .d(new_n143_), .O(new_n171_));
  nor2   g120(.a(new_n171_), .b(new_n98_), .O(new_n172_));
  aoi21  g121(.a(new_n163_), .b(x33), .c(new_n172_), .O(new_n173_));
  inv1   g122(.a(x01), .O(new_n174_));
  aoi21  g123(.a(new_n58_), .b(new_n174_), .c(x18), .O(new_n175_));
  oai21  g124(.a(new_n173_), .b(new_n58_), .c(new_n175_), .O(z14));
  nor3   g125(.a(new_n171_), .b(new_n98_), .c(x34), .O(new_n177_));
  oai21  g126(.a(new_n171_), .b(new_n98_), .c(x34), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x16), .O(new_n179_));
  aoi21  g128(.a(new_n58_), .b(x00), .c(x18), .O(new_n180_));
  oai21  g129(.a(new_n179_), .b(new_n177_), .c(new_n180_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n87_), .O(z15));
endmodule


