// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_;
  inv1   g00(.a(x18), .O(new_n52_));
  nor2   g01(.a(x19), .b(x17), .O(new_n53_));
  nand2  g02(.a(x19), .b(x17), .O(new_n54_));
  nand2  g03(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g04(.a(x16), .O(new_n56_));
  inv1   g05(.a(x25), .O(new_n57_));
  aoi21  g06(.a(new_n56_), .b(x15), .c(new_n57_), .O(new_n58_));
  oai21  g07(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  nand2  g08(.a(new_n59_), .b(new_n52_), .O(z00));
  nor3   g09(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  inv1   g10(.a(x20), .O(new_n62_));
  oai21  g11(.a(new_n53_), .b(new_n62_), .c(x16), .O(new_n63_));
  aoi21  g12(.a(new_n56_), .b(x14), .c(new_n57_), .O(new_n64_));
  oai21  g13(.a(new_n63_), .b(new_n61_), .c(new_n64_), .O(new_n65_));
  nand2  g14(.a(new_n65_), .b(new_n52_), .O(z01));
  inv1   g15(.a(x21), .O(new_n67_));
  nand3  g16(.a(new_n53_), .b(new_n67_), .c(new_n62_), .O(new_n68_));
  nand2  g17(.a(new_n53_), .b(new_n62_), .O(new_n69_));
  nand2  g18(.a(new_n69_), .b(x21), .O(new_n70_));
  nand3  g19(.a(new_n70_), .b(new_n68_), .c(x16), .O(new_n71_));
  aoi21  g20(.a(new_n56_), .b(x13), .c(new_n57_), .O(new_n72_));
  nand2  g21(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g22(.a(new_n73_), .b(new_n52_), .O(z02));
  nor2   g23(.a(x22), .b(x21), .O(new_n75_));
  aoi22  g24(.a(new_n75_), .b(new_n61_), .c(new_n68_), .d(x22), .O(new_n76_));
  nor2   g25(.a(x16), .b(x12), .O(new_n77_));
  nor2   g26(.a(new_n57_), .b(x18), .O(new_n78_));
  inv1   g27(.a(new_n78_), .O(new_n79_));
  nor2   g28(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  oai21  g29(.a(new_n76_), .b(new_n56_), .c(new_n80_), .O(z03));
  inv1   g30(.a(x23), .O(new_n82_));
  aoi21  g31(.a(new_n75_), .b(new_n61_), .c(new_n82_), .O(new_n83_));
  nand4  g32(.a(new_n75_), .b(new_n53_), .c(new_n82_), .d(new_n62_), .O(new_n84_));
  nand2  g33(.a(new_n84_), .b(x16), .O(new_n85_));
  aoi21  g34(.a(new_n56_), .b(x11), .c(new_n57_), .O(new_n86_));
  oai21  g35(.a(new_n85_), .b(new_n83_), .c(new_n86_), .O(new_n87_));
  nand2  g36(.a(new_n87_), .b(new_n52_), .O(z04));
  xor2a  g37(.a(new_n84_), .b(x24), .O(new_n89_));
  nor2   g38(.a(x16), .b(x10), .O(new_n90_));
  nor2   g39(.a(new_n90_), .b(new_n79_), .O(new_n91_));
  oai21  g40(.a(new_n89_), .b(new_n56_), .c(new_n91_), .O(z05));
  inv1   g41(.a(x24), .O(new_n93_));
  nand4  g42(.a(new_n75_), .b(new_n61_), .c(new_n93_), .d(new_n82_), .O(new_n94_));
  aoi21  g43(.a(new_n56_), .b(x09), .c(new_n57_), .O(new_n95_));
  oai21  g44(.a(new_n94_), .b(new_n56_), .c(new_n95_), .O(new_n96_));
  nand2  g45(.a(new_n96_), .b(new_n52_), .O(z06));
  nand2  g46(.a(new_n56_), .b(x08), .O(new_n98_));
  inv1   g47(.a(x26), .O(new_n99_));
  nand2  g48(.a(new_n99_), .b(x16), .O(new_n100_));
  nand3  g49(.a(new_n100_), .b(new_n98_), .c(x25), .O(new_n101_));
  nand2  g50(.a(new_n101_), .b(new_n52_), .O(z07));
  nand2  g51(.a(new_n56_), .b(x07), .O(new_n103_));
  inv1   g52(.a(x27), .O(new_n104_));
  nand2  g53(.a(new_n104_), .b(x16), .O(new_n105_));
  nand3  g54(.a(new_n105_), .b(new_n103_), .c(x25), .O(new_n106_));
  nand2  g55(.a(new_n106_), .b(new_n52_), .O(z08));
  nand2  g56(.a(x28), .b(x16), .O(new_n108_));
  inv1   g57(.a(x06), .O(new_n109_));
  nand2  g58(.a(new_n56_), .b(new_n109_), .O(new_n110_));
  nand3  g59(.a(new_n110_), .b(new_n108_), .c(new_n78_), .O(z09));
  nand2  g60(.a(x29), .b(x16), .O(new_n112_));
  inv1   g61(.a(x05), .O(new_n113_));
  nand2  g62(.a(new_n56_), .b(new_n113_), .O(new_n114_));
  nand3  g63(.a(new_n114_), .b(new_n112_), .c(new_n78_), .O(z10));
  nand2  g64(.a(x30), .b(x16), .O(new_n116_));
  inv1   g65(.a(x04), .O(new_n117_));
  nand2  g66(.a(new_n56_), .b(new_n117_), .O(new_n118_));
  nand3  g67(.a(new_n118_), .b(new_n116_), .c(new_n78_), .O(z11));
  nand2  g68(.a(new_n56_), .b(x03), .O(new_n120_));
  inv1   g69(.a(x31), .O(new_n121_));
  nand2  g70(.a(new_n121_), .b(x16), .O(new_n122_));
  nand3  g71(.a(new_n122_), .b(new_n120_), .c(x25), .O(new_n123_));
  nand2  g72(.a(new_n123_), .b(new_n52_), .O(z12));
  nand2  g73(.a(x32), .b(x16), .O(new_n125_));
  inv1   g74(.a(x02), .O(new_n126_));
  nand2  g75(.a(new_n56_), .b(new_n126_), .O(new_n127_));
  nand3  g76(.a(new_n127_), .b(new_n125_), .c(new_n78_), .O(z13));
  nand2  g77(.a(new_n56_), .b(x01), .O(new_n129_));
  inv1   g78(.a(x33), .O(new_n130_));
  nand2  g79(.a(new_n130_), .b(x16), .O(new_n131_));
  nand3  g80(.a(new_n131_), .b(new_n129_), .c(x25), .O(new_n132_));
  nand2  g81(.a(new_n132_), .b(new_n52_), .O(z14));
  nand2  g82(.a(x34), .b(x16), .O(new_n134_));
  inv1   g83(.a(x00), .O(new_n135_));
  nand2  g84(.a(new_n56_), .b(new_n135_), .O(new_n136_));
  nand3  g85(.a(new_n136_), .b(new_n134_), .c(new_n78_), .O(z15));
endmodule


