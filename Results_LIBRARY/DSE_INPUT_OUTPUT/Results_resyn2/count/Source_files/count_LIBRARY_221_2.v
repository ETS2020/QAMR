// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_;
  nand2  g000(.a(x22), .b(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x18), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  oai21  g010(.a(new_n53_), .b(new_n61_), .c(x16), .O(new_n62_));
  aoi21  g011(.a(new_n56_), .b(x14), .c(x18), .O(new_n63_));
  oai21  g012(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n52_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  nand3  g015(.a(new_n53_), .b(new_n66_), .c(new_n61_), .O(new_n67_));
  nand2  g016(.a(new_n53_), .b(new_n61_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(x21), .O(new_n69_));
  nand3  g018(.a(new_n69_), .b(new_n67_), .c(x16), .O(new_n70_));
  aoi21  g019(.a(new_n56_), .b(x13), .c(x18), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n52_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  aoi21  g023(.a(new_n67_), .b(x16), .c(x18), .O(new_n75_));
  nor2   g024(.a(x22), .b(x21), .O(new_n76_));
  aoi21  g025(.a(new_n76_), .b(new_n60_), .c(new_n56_), .O(new_n77_));
  inv1   g026(.a(x12), .O(new_n78_));
  inv1   g027(.a(x18), .O(new_n79_));
  oai21  g028(.a(x16), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  oai22  g029(.a(new_n80_), .b(new_n77_), .c(new_n75_), .d(new_n74_), .O(z03));
  nand2  g030(.a(new_n76_), .b(new_n60_), .O(new_n82_));
  xor2a  g031(.a(new_n82_), .b(x23), .O(new_n83_));
  inv1   g032(.a(x11), .O(new_n84_));
  aoi21  g033(.a(new_n56_), .b(new_n84_), .c(x18), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n56_), .c(new_n85_), .O(z04));
  inv1   g035(.a(x23), .O(new_n87_));
  nand4  g036(.a(new_n76_), .b(new_n53_), .c(new_n87_), .d(new_n61_), .O(new_n88_));
  nor2   g037(.a(x24), .b(x23), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n76_), .c(new_n53_), .d(new_n61_), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  aoi21  g040(.a(new_n88_), .b(x24), .c(new_n91_), .O(new_n92_));
  inv1   g041(.a(x10), .O(new_n93_));
  aoi21  g042(.a(new_n56_), .b(new_n93_), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n56_), .c(new_n94_), .O(z05));
  nand2  g044(.a(new_n56_), .b(x09), .O(new_n96_));
  inv1   g045(.a(x25), .O(new_n97_));
  nand4  g046(.a(new_n89_), .b(new_n76_), .c(new_n60_), .d(new_n97_), .O(new_n98_));
  nand2  g047(.a(new_n90_), .b(x25), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n98_), .c(x16), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n96_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n79_), .O(z06));
  inv1   g051(.a(x26), .O(new_n103_));
  oai21  g052(.a(new_n103_), .b(new_n56_), .c(new_n79_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(x22), .O(new_n105_));
  nand2  g054(.a(new_n89_), .b(new_n97_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n67_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n103_), .O(new_n108_));
  inv1   g057(.a(x24), .O(new_n109_));
  inv1   g058(.a(new_n88_), .O(new_n110_));
  nor2   g059(.a(x26), .b(x25), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(x16), .O(new_n113_));
  aoi21  g062(.a(new_n56_), .b(x08), .c(x18), .O(new_n114_));
  oai21  g063(.a(new_n113_), .b(new_n108_), .c(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n105_), .O(z07));
  inv1   g065(.a(x27), .O(new_n117_));
  nand2  g066(.a(new_n111_), .b(new_n117_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n90_), .O(new_n119_));
  aoi21  g068(.a(new_n112_), .b(x27), .c(new_n119_), .O(new_n120_));
  inv1   g069(.a(x07), .O(new_n121_));
  aoi21  g070(.a(new_n56_), .b(new_n121_), .c(x18), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n56_), .c(new_n122_), .O(z08));
  nand2  g072(.a(x28), .b(x16), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n79_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(x22), .O(new_n126_));
  nor3   g075(.a(x28), .b(x27), .c(x26), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n91_), .c(new_n97_), .O(new_n128_));
  nand3  g077(.a(new_n89_), .b(new_n60_), .c(new_n66_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(new_n118_), .c(x28), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n128_), .c(x16), .O(new_n131_));
  aoi21  g080(.a(new_n56_), .b(x06), .c(x18), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n126_), .O(z09));
  inv1   g083(.a(x05), .O(new_n135_));
  oai21  g084(.a(x16), .b(new_n135_), .c(new_n79_), .O(new_n136_));
  nand3  g085(.a(new_n127_), .b(new_n89_), .c(new_n97_), .O(new_n137_));
  oai22  g086(.a(new_n137_), .b(new_n82_), .c(x29), .d(x18), .O(new_n138_));
  nor2   g087(.a(x29), .b(x28), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n111_), .c(new_n117_), .d(new_n109_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n88_), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n56_), .O(new_n142_));
  aoi22  g091(.a(new_n142_), .b(new_n138_), .c(new_n136_), .d(new_n52_), .O(z10));
  inv1   g092(.a(x30), .O(new_n144_));
  xor2a  g093(.a(new_n141_), .b(new_n144_), .O(new_n145_));
  inv1   g094(.a(x04), .O(new_n146_));
  aoi21  g095(.a(new_n56_), .b(new_n146_), .c(x18), .O(new_n147_));
  oai21  g096(.a(new_n145_), .b(new_n56_), .c(new_n147_), .O(z11));
  nand2  g097(.a(new_n141_), .b(new_n144_), .O(new_n149_));
  nor2   g098(.a(x31), .b(x30), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n139_), .c(new_n111_), .d(new_n117_), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n90_), .O(new_n152_));
  aoi21  g101(.a(new_n149_), .b(x31), .c(new_n152_), .O(new_n153_));
  inv1   g102(.a(x03), .O(new_n154_));
  aoi21  g103(.a(new_n56_), .b(new_n154_), .c(x18), .O(new_n155_));
  oai21  g104(.a(new_n153_), .b(new_n56_), .c(new_n155_), .O(z12));
  inv1   g105(.a(x32), .O(new_n157_));
  oai21  g106(.a(new_n157_), .b(new_n56_), .c(new_n79_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(x22), .O(new_n159_));
  nand2  g108(.a(new_n152_), .b(new_n157_), .O(new_n160_));
  oai21  g109(.a(new_n151_), .b(new_n129_), .c(x32), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n160_), .c(x16), .O(new_n162_));
  aoi21  g111(.a(new_n56_), .b(x02), .c(x18), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n159_), .O(z13));
  inv1   g114(.a(x33), .O(new_n166_));
  aoi21  g115(.a(new_n152_), .b(new_n157_), .c(new_n166_), .O(new_n167_));
  nor2   g116(.a(x27), .b(x26), .O(new_n168_));
  nor2   g117(.a(x33), .b(x32), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n150_), .c(new_n139_), .d(new_n168_), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n98_), .O(new_n171_));
  oai21  g120(.a(new_n171_), .b(new_n167_), .c(x16), .O(new_n172_));
  inv1   g121(.a(x01), .O(new_n173_));
  aoi21  g122(.a(new_n56_), .b(new_n173_), .c(x18), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n172_), .O(z14));
  inv1   g124(.a(new_n170_), .O(new_n176_));
  inv1   g125(.a(x34), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n56_), .O(new_n178_));
  nand3  g127(.a(new_n178_), .b(new_n176_), .c(new_n107_), .O(new_n179_));
  aoi21  g128(.a(new_n179_), .b(new_n79_), .c(x22), .O(new_n180_));
  oai21  g129(.a(new_n170_), .b(new_n98_), .c(new_n177_), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(x16), .O(new_n182_));
  inv1   g131(.a(x00), .O(new_n183_));
  aoi21  g132(.a(new_n56_), .b(new_n183_), .c(x18), .O(new_n184_));
  aoi21  g133(.a(new_n184_), .b(new_n182_), .c(new_n180_), .O(z15));
endmodule


