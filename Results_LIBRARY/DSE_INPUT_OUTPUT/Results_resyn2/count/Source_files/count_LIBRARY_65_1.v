// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x32), .b(x18), .O(new_n58_));
  nand3  g007(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  nand2  g010(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  oai21  g011(.a(x19), .b(x17), .c(x20), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(x32), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n60_), .O(z01));
  and2   g016(.a(new_n62_), .b(x21), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x16), .O(new_n71_));
  aoi21  g020(.a(new_n56_), .b(x13), .c(x32), .O(new_n72_));
  oai21  g021(.a(new_n71_), .b(new_n68_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n60_), .O(z02));
  inv1   g023(.a(x22), .O(new_n75_));
  nand3  g024(.a(new_n69_), .b(new_n53_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n70_), .b(x22), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n76_), .c(x16), .O(new_n78_));
  aoi21  g027(.a(new_n56_), .b(x12), .c(x32), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n60_), .O(z03));
  nand2  g030(.a(new_n76_), .b(x23), .O(new_n82_));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n69_), .c(new_n53_), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  aoi21  g034(.a(new_n56_), .b(x11), .c(x32), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n60_), .O(z04));
  inv1   g037(.a(x24), .O(new_n89_));
  nand4  g038(.a(new_n83_), .b(new_n69_), .c(new_n53_), .d(new_n89_), .O(new_n90_));
  nand2  g039(.a(new_n84_), .b(x24), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  aoi21  g041(.a(new_n56_), .b(x10), .c(x32), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n60_), .O(z05));
  nor2   g044(.a(new_n90_), .b(x25), .O(new_n96_));
  inv1   g045(.a(x25), .O(new_n97_));
  inv1   g046(.a(new_n90_), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(new_n97_), .c(x16), .O(new_n99_));
  aoi21  g048(.a(new_n56_), .b(x09), .c(x32), .O(new_n100_));
  oai21  g049(.a(new_n99_), .b(new_n96_), .c(new_n100_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n60_), .O(z06));
  inv1   g051(.a(x26), .O(new_n103_));
  nor2   g052(.a(new_n96_), .b(new_n103_), .O(new_n104_));
  nor2   g053(.a(x26), .b(x25), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n98_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(x16), .O(new_n107_));
  aoi21  g056(.a(new_n56_), .b(x08), .c(x32), .O(new_n108_));
  oai21  g057(.a(new_n107_), .b(new_n104_), .c(new_n108_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n60_), .O(z07));
  inv1   g059(.a(x27), .O(new_n111_));
  aoi21  g060(.a(new_n105_), .b(new_n98_), .c(new_n111_), .O(new_n112_));
  nand3  g061(.a(new_n111_), .b(new_n103_), .c(new_n97_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(new_n90_), .c(x16), .O(new_n114_));
  aoi21  g063(.a(new_n56_), .b(x07), .c(x32), .O(new_n115_));
  oai21  g064(.a(new_n114_), .b(new_n112_), .c(new_n115_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n60_), .O(z08));
  inv1   g066(.a(x28), .O(new_n118_));
  nor2   g067(.a(new_n113_), .b(new_n90_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nor2   g069(.a(x24), .b(x23), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n105_), .c(new_n118_), .d(new_n111_), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n76_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n120_), .c(x16), .O(new_n124_));
  inv1   g073(.a(new_n58_), .O(new_n125_));
  nor2   g074(.a(x16), .b(x06), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n124_), .O(z09));
  inv1   g077(.a(x29), .O(new_n129_));
  nor2   g078(.a(new_n123_), .b(new_n129_), .O(new_n130_));
  nor2   g079(.a(x29), .b(x28), .O(new_n131_));
  nor2   g080(.a(x27), .b(x24), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n131_), .c(new_n105_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n84_), .c(x16), .O(new_n134_));
  aoi21  g083(.a(new_n56_), .b(x05), .c(x32), .O(new_n135_));
  oai21  g084(.a(new_n134_), .b(new_n130_), .c(new_n135_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n60_), .O(z10));
  inv1   g086(.a(x30), .O(new_n138_));
  inv1   g087(.a(new_n84_), .O(new_n139_));
  inv1   g088(.a(new_n133_), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n139_), .c(new_n138_), .O(new_n141_));
  oai21  g090(.a(new_n133_), .b(new_n84_), .c(x30), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n141_), .c(x16), .O(new_n143_));
  aoi21  g092(.a(new_n56_), .b(x04), .c(x32), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n60_), .O(z11));
  nor2   g095(.a(x31), .b(x30), .O(new_n147_));
  nor2   g096(.a(x32), .b(x29), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  inv1   g098(.a(new_n149_), .O(new_n150_));
  aoi22  g099(.a(new_n150_), .b(new_n123_), .c(new_n141_), .d(x31), .O(new_n151_));
  nor2   g100(.a(x16), .b(x03), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n125_), .O(new_n153_));
  oai21  g102(.a(new_n151_), .b(new_n56_), .c(new_n153_), .O(z12));
  nor2   g103(.a(x16), .b(x02), .O(new_n155_));
  nand3  g104(.a(new_n121_), .b(new_n75_), .c(x16), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n70_), .O(new_n157_));
  nand2  g106(.a(new_n147_), .b(new_n131_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n113_), .O(new_n159_));
  aoi21  g108(.a(new_n159_), .b(new_n157_), .c(new_n155_), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(x32), .c(new_n60_), .O(z13));
  inv1   g110(.a(x33), .O(new_n162_));
  inv1   g111(.a(new_n158_), .O(new_n163_));
  aoi21  g112(.a(new_n163_), .b(new_n119_), .c(new_n162_), .O(new_n164_));
  inv1   g113(.a(new_n76_), .O(new_n165_));
  inv1   g114(.a(x23), .O(new_n166_));
  nand3  g115(.a(new_n118_), .b(new_n89_), .c(new_n166_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n113_), .O(new_n168_));
  nand3  g117(.a(new_n147_), .b(new_n162_), .c(new_n129_), .O(new_n169_));
  inv1   g118(.a(new_n169_), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n168_), .c(new_n165_), .O(new_n171_));
  inv1   g120(.a(new_n171_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n164_), .c(x16), .O(new_n173_));
  nor2   g122(.a(x16), .b(x01), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n125_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n173_), .O(z14));
  inv1   g125(.a(x34), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n162_), .c(new_n111_), .d(new_n103_), .O(new_n178_));
  nor2   g127(.a(new_n178_), .b(new_n158_), .O(new_n179_));
  aoi22  g128(.a(new_n179_), .b(new_n96_), .c(new_n171_), .d(x34), .O(new_n180_));
  nor2   g129(.a(x16), .b(x00), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n125_), .O(new_n182_));
  oai21  g131(.a(new_n180_), .b(new_n56_), .c(new_n182_), .O(z15));
endmodule


