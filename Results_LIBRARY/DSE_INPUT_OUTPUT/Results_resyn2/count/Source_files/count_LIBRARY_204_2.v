// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_;
  nand2  g000(.a(x26), .b(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x18), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  oai21  g010(.a(new_n53_), .b(new_n61_), .c(x16), .O(new_n62_));
  aoi21  g011(.a(new_n56_), .b(x14), .c(x18), .O(new_n63_));
  oai21  g012(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n52_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  nor2   g015(.a(new_n60_), .b(new_n66_), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x16), .O(new_n70_));
  aoi21  g019(.a(new_n56_), .b(x13), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n70_), .b(new_n67_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n52_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  nand3  g023(.a(new_n68_), .b(new_n53_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n69_), .b(x22), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n75_), .c(x16), .O(new_n77_));
  aoi21  g026(.a(new_n56_), .b(x12), .c(x18), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n52_), .O(z03));
  nor2   g029(.a(x23), .b(x22), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n68_), .c(new_n53_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  aoi21  g032(.a(new_n75_), .b(x23), .c(new_n83_), .O(new_n84_));
  inv1   g033(.a(x11), .O(new_n85_));
  aoi21  g034(.a(new_n56_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n56_), .c(new_n86_), .O(z04));
  nand2  g036(.a(new_n53_), .b(new_n61_), .O(new_n88_));
  nor2   g037(.a(x24), .b(x21), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n81_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  aoi21  g040(.a(new_n82_), .b(x24), .c(new_n91_), .O(new_n92_));
  inv1   g041(.a(x10), .O(new_n93_));
  aoi21  g042(.a(new_n56_), .b(new_n93_), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n56_), .c(new_n94_), .O(z05));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n81_), .c(new_n68_), .d(new_n53_), .O(new_n97_));
  nand3  g046(.a(new_n89_), .b(new_n81_), .c(new_n60_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(x25), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n97_), .c(x16), .O(new_n100_));
  aoi21  g049(.a(new_n56_), .b(x09), .c(x18), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n52_), .O(z06));
  inv1   g052(.a(x18), .O(new_n104_));
  nor3   g053(.a(x26), .b(x25), .c(x24), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  aoi22  g056(.a(new_n107_), .b(new_n83_), .c(new_n97_), .d(x26), .O(new_n108_));
  inv1   g057(.a(x08), .O(new_n109_));
  aoi21  g058(.a(new_n56_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n56_), .c(new_n110_), .O(z07));
  inv1   g060(.a(x07), .O(new_n112_));
  oai21  g061(.a(x16), .b(new_n112_), .c(new_n104_), .O(new_n113_));
  nand2  g062(.a(new_n105_), .b(new_n83_), .O(new_n114_));
  oai21  g063(.a(x27), .b(x18), .c(new_n114_), .O(new_n115_));
  nor3   g064(.a(x27), .b(x26), .c(x25), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n91_), .c(new_n56_), .O(new_n117_));
  aoi22  g066(.a(new_n117_), .b(new_n115_), .c(new_n113_), .d(new_n52_), .O(z08));
  nand4  g067(.a(new_n116_), .b(new_n89_), .c(new_n81_), .d(new_n60_), .O(new_n119_));
  inv1   g068(.a(x25), .O(new_n120_));
  inv1   g069(.a(x26), .O(new_n121_));
  inv1   g070(.a(x27), .O(new_n122_));
  inv1   g071(.a(x28), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n120_), .O(new_n124_));
  inv1   g073(.a(new_n124_), .O(new_n125_));
  aoi22  g074(.a(new_n125_), .b(new_n91_), .c(new_n119_), .d(x28), .O(new_n126_));
  inv1   g075(.a(x06), .O(new_n127_));
  aoi21  g076(.a(new_n56_), .b(new_n127_), .c(x18), .O(new_n128_));
  oai21  g077(.a(new_n126_), .b(new_n56_), .c(new_n128_), .O(z09));
  oai21  g078(.a(new_n124_), .b(new_n98_), .c(x29), .O(new_n130_));
  nor2   g079(.a(x29), .b(x28), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n105_), .c(new_n83_), .d(new_n122_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  inv1   g083(.a(x05), .O(new_n135_));
  aoi21  g084(.a(new_n56_), .b(new_n135_), .c(x18), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n134_), .O(z10));
  xor2a  g086(.a(new_n132_), .b(x30), .O(new_n138_));
  inv1   g087(.a(x04), .O(new_n139_));
  aoi21  g088(.a(new_n56_), .b(new_n139_), .c(x18), .O(new_n140_));
  oai21  g089(.a(new_n138_), .b(new_n56_), .c(new_n140_), .O(z11));
  inv1   g090(.a(x31), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n56_), .c(new_n104_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x26), .O(new_n144_));
  nand2  g093(.a(new_n96_), .b(new_n122_), .O(new_n145_));
  nor2   g094(.a(new_n145_), .b(new_n82_), .O(new_n146_));
  nor3   g095(.a(x30), .b(x29), .c(x28), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n146_), .c(new_n142_), .O(new_n148_));
  nor2   g097(.a(x31), .b(x30), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n131_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n119_), .c(x16), .O(new_n151_));
  aoi21  g100(.a(new_n56_), .b(x03), .c(x18), .O(new_n152_));
  oai21  g101(.a(new_n151_), .b(new_n148_), .c(new_n152_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n144_), .O(z12));
  inv1   g103(.a(x32), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n56_), .c(new_n104_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(x26), .O(new_n157_));
  inv1   g106(.a(new_n150_), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(new_n146_), .c(new_n155_), .O(new_n159_));
  inv1   g108(.a(new_n90_), .O(new_n160_));
  inv1   g109(.a(x29), .O(new_n161_));
  inv1   g110(.a(x30), .O(new_n162_));
  nand4  g111(.a(new_n155_), .b(new_n142_), .c(new_n162_), .d(new_n161_), .O(new_n163_));
  inv1   g112(.a(new_n163_), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n125_), .c(new_n160_), .d(new_n60_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(x16), .O(new_n166_));
  aoi21  g115(.a(new_n56_), .b(x02), .c(x18), .O(new_n167_));
  oai21  g116(.a(new_n166_), .b(new_n159_), .c(new_n167_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n157_), .O(z13));
  nand2  g118(.a(x33), .b(x16), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n104_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(x26), .O(new_n172_));
  nor3   g121(.a(x28), .b(x27), .c(x25), .O(new_n173_));
  nand3  g122(.a(new_n164_), .b(new_n173_), .c(new_n91_), .O(new_n174_));
  nor3   g123(.a(x31), .b(x30), .c(x29), .O(new_n175_));
  nor2   g124(.a(x33), .b(x32), .O(new_n176_));
  nor3   g125(.a(x28), .b(x27), .c(x26), .O(new_n177_));
  nand3  g126(.a(new_n177_), .b(new_n176_), .c(new_n175_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n97_), .c(x16), .O(new_n179_));
  aoi21  g128(.a(new_n174_), .b(x33), .c(new_n179_), .O(new_n180_));
  inv1   g129(.a(x01), .O(new_n181_));
  oai21  g130(.a(x16), .b(new_n181_), .c(new_n104_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n180_), .c(new_n172_), .O(z14));
  inv1   g132(.a(x34), .O(new_n184_));
  oai21  g133(.a(new_n184_), .b(new_n56_), .c(new_n104_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(x26), .O(new_n186_));
  inv1   g135(.a(new_n97_), .O(new_n187_));
  nand2  g136(.a(new_n149_), .b(new_n161_), .O(new_n188_));
  nor2   g137(.a(x27), .b(x26), .O(new_n189_));
  nand3  g138(.a(new_n189_), .b(new_n176_), .c(new_n123_), .O(new_n190_));
  nor2   g139(.a(new_n190_), .b(new_n188_), .O(new_n191_));
  nand3  g140(.a(new_n191_), .b(new_n187_), .c(new_n184_), .O(new_n192_));
  nor2   g141(.a(x27), .b(x25), .O(new_n193_));
  nand4  g142(.a(new_n176_), .b(new_n149_), .c(new_n131_), .d(new_n193_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n98_), .c(x34), .O(new_n195_));
  nand3  g144(.a(new_n195_), .b(new_n192_), .c(x16), .O(new_n196_));
  aoi21  g145(.a(new_n56_), .b(x00), .c(x18), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n196_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n186_), .O(z15));
endmodule


