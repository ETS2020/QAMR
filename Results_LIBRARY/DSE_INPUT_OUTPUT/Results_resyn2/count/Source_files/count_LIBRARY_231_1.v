// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_;
  inv1   g000(.a(x16), .O(new_n52_));
  nor2   g001(.a(x18), .b(x16), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  inv1   g003(.a(x17), .O(new_n55_));
  inv1   g004(.a(x19), .O(new_n56_));
  nor2   g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor3   g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  oai22  g008(.a(new_n59_), .b(new_n52_), .c(new_n54_), .d(x15), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n58_), .b(new_n61_), .O(new_n62_));
  inv1   g011(.a(x18), .O(new_n63_));
  nand2  g012(.a(new_n58_), .b(new_n61_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n62_), .c(x16), .O(new_n66_));
  oai21  g015(.a(new_n54_), .b(x14), .c(new_n66_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nand4  g017(.a(new_n68_), .b(new_n61_), .c(new_n56_), .d(new_n55_), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  aoi21  g019(.a(new_n64_), .b(x21), .c(new_n70_), .O(new_n71_));
  inv1   g020(.a(x13), .O(new_n72_));
  aoi21  g021(.a(new_n52_), .b(new_n72_), .c(x18), .O(new_n73_));
  oai21  g022(.a(new_n71_), .b(new_n52_), .c(new_n73_), .O(z02));
  inv1   g023(.a(x22), .O(new_n75_));
  nor2   g024(.a(new_n70_), .b(new_n75_), .O(new_n76_));
  oai21  g025(.a(new_n69_), .b(x22), .c(new_n63_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(new_n76_), .c(x16), .O(new_n78_));
  oai21  g027(.a(new_n54_), .b(x12), .c(new_n78_), .O(z03));
  oai21  g028(.a(new_n69_), .b(x22), .c(x23), .O(new_n80_));
  nor2   g029(.a(x23), .b(x22), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n70_), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n80_), .c(new_n63_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(x16), .O(new_n84_));
  oai21  g033(.a(new_n54_), .b(x11), .c(new_n84_), .O(z04));
  inv1   g034(.a(x24), .O(new_n86_));
  nor2   g035(.a(x21), .b(x20), .O(new_n87_));
  nand4  g036(.a(new_n81_), .b(new_n87_), .c(new_n58_), .d(new_n86_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n63_), .O(new_n89_));
  aoi21  g038(.a(new_n82_), .b(x24), .c(new_n89_), .O(new_n90_));
  oai22  g039(.a(new_n90_), .b(new_n52_), .c(new_n54_), .d(x10), .O(z05));
  nor2   g040(.a(x25), .b(x24), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n81_), .c(new_n87_), .d(new_n58_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n63_), .O(new_n94_));
  aoi21  g043(.a(new_n88_), .b(x25), .c(new_n94_), .O(new_n95_));
  oai22  g044(.a(new_n95_), .b(new_n52_), .c(new_n54_), .d(x09), .O(z06));
  inv1   g045(.a(x08), .O(new_n97_));
  nand2  g046(.a(new_n53_), .b(new_n97_), .O(new_n98_));
  nor2   g047(.a(new_n93_), .b(x26), .O(new_n99_));
  inv1   g048(.a(x26), .O(new_n100_));
  inv1   g049(.a(new_n93_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n100_), .c(new_n63_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n99_), .c(x16), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n98_), .O(z07));
  nand2  g053(.a(new_n101_), .b(new_n100_), .O(new_n105_));
  nor3   g054(.a(x27), .b(x26), .c(x25), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n88_), .O(new_n108_));
  aoi21  g057(.a(new_n105_), .b(x27), .c(new_n108_), .O(new_n109_));
  inv1   g058(.a(x07), .O(new_n110_));
  aoi21  g059(.a(new_n52_), .b(new_n110_), .c(x18), .O(new_n111_));
  oai21  g060(.a(new_n109_), .b(new_n52_), .c(new_n111_), .O(z08));
  oai21  g061(.a(new_n107_), .b(new_n88_), .c(x28), .O(new_n113_));
  inv1   g062(.a(x28), .O(new_n114_));
  inv1   g063(.a(new_n88_), .O(new_n115_));
  nand3  g064(.a(new_n106_), .b(new_n115_), .c(new_n114_), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n113_), .c(new_n63_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(x16), .O(new_n118_));
  inv1   g067(.a(x06), .O(new_n119_));
  nand2  g068(.a(new_n53_), .b(new_n119_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(z09));
  nand2  g070(.a(new_n106_), .b(new_n114_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n88_), .c(x29), .O(new_n123_));
  inv1   g072(.a(x23), .O(new_n124_));
  inv1   g073(.a(x25), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n86_), .c(new_n124_), .d(new_n75_), .O(new_n126_));
  inv1   g075(.a(new_n126_), .O(new_n127_));
  nor3   g076(.a(x29), .b(x28), .c(x27), .O(new_n128_));
  nand4  g077(.a(new_n128_), .b(new_n127_), .c(new_n70_), .d(new_n100_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n123_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  inv1   g080(.a(x05), .O(new_n132_));
  aoi21  g081(.a(new_n52_), .b(new_n132_), .c(x18), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n131_), .O(z10));
  inv1   g083(.a(x27), .O(new_n135_));
  inv1   g084(.a(x30), .O(new_n136_));
  nor2   g085(.a(x29), .b(x28), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n136_), .c(new_n135_), .O(new_n138_));
  inv1   g087(.a(new_n138_), .O(new_n139_));
  aoi22  g088(.a(new_n139_), .b(new_n99_), .c(new_n129_), .d(x30), .O(new_n140_));
  inv1   g089(.a(x04), .O(new_n141_));
  aoi21  g090(.a(new_n52_), .b(new_n141_), .c(x18), .O(new_n142_));
  oai21  g091(.a(new_n140_), .b(new_n52_), .c(new_n142_), .O(z11));
  nand3  g092(.a(new_n139_), .b(new_n101_), .c(new_n100_), .O(new_n144_));
  nor2   g093(.a(x31), .b(x30), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n137_), .c(new_n106_), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n88_), .O(new_n147_));
  aoi21  g096(.a(new_n144_), .b(x31), .c(new_n147_), .O(new_n148_));
  inv1   g097(.a(x03), .O(new_n149_));
  aoi21  g098(.a(new_n52_), .b(new_n149_), .c(x18), .O(new_n150_));
  oai21  g099(.a(new_n148_), .b(new_n52_), .c(new_n150_), .O(z12));
  oai21  g100(.a(new_n146_), .b(new_n88_), .c(x32), .O(new_n152_));
  nor2   g101(.a(x32), .b(x31), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n100_), .c(new_n125_), .O(new_n154_));
  inv1   g103(.a(new_n154_), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n139_), .c(new_n115_), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n152_), .c(new_n63_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(x16), .O(new_n158_));
  inv1   g107(.a(x02), .O(new_n159_));
  nand2  g108(.a(new_n53_), .b(new_n159_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n158_), .O(z13));
  nor2   g110(.a(x27), .b(x26), .O(new_n162_));
  nor2   g111(.a(x33), .b(x32), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n145_), .c(new_n137_), .d(new_n162_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n93_), .c(new_n63_), .O(new_n165_));
  aoi21  g114(.a(new_n156_), .b(x33), .c(new_n165_), .O(new_n166_));
  inv1   g115(.a(x01), .O(new_n167_));
  nand2  g116(.a(new_n53_), .b(new_n167_), .O(new_n168_));
  oai21  g117(.a(new_n166_), .b(new_n52_), .c(new_n168_), .O(z14));
  inv1   g118(.a(x00), .O(new_n170_));
  nand2  g119(.a(new_n53_), .b(new_n170_), .O(new_n171_));
  nor3   g120(.a(new_n164_), .b(new_n93_), .c(x34), .O(new_n172_));
  oai21  g121(.a(new_n164_), .b(new_n93_), .c(x34), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n63_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n172_), .c(x16), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n171_), .O(z15));
endmodule


