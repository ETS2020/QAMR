// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x23), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  xor2a  g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  nor2   g010(.a(x16), .b(x14), .O(new_n62_));
  inv1   g011(.a(x23), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n52_), .O(new_n64_));
  nor2   g013(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  oai21  g014(.a(new_n61_), .b(new_n56_), .c(new_n65_), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  aoi21  g016(.a(new_n53_), .b(new_n60_), .c(new_n67_), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x16), .O(new_n71_));
  aoi21  g020(.a(new_n56_), .b(x13), .c(x23), .O(new_n72_));
  oai21  g021(.a(new_n71_), .b(new_n68_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n52_), .O(z02));
  xor2a  g023(.a(new_n70_), .b(x22), .O(new_n75_));
  nor2   g024(.a(x16), .b(x12), .O(new_n76_));
  nor2   g025(.a(new_n76_), .b(new_n64_), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n56_), .c(new_n77_), .O(z03));
  inv1   g027(.a(x22), .O(new_n79_));
  nand3  g028(.a(new_n69_), .b(new_n53_), .c(new_n79_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(x16), .O(new_n81_));
  aoi21  g030(.a(new_n56_), .b(x11), .c(x23), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n52_), .O(z04));
  nand2  g033(.a(new_n80_), .b(x24), .O(new_n85_));
  nor2   g034(.a(x24), .b(x22), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n69_), .c(new_n53_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(x16), .O(new_n88_));
  aoi21  g037(.a(new_n56_), .b(x10), .c(x23), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n52_), .O(z05));
  inv1   g040(.a(x25), .O(new_n92_));
  nand4  g041(.a(new_n86_), .b(new_n69_), .c(new_n53_), .d(new_n92_), .O(new_n93_));
  nand2  g042(.a(new_n87_), .b(x25), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n93_), .c(x16), .O(new_n95_));
  aoi21  g044(.a(new_n56_), .b(x09), .c(x23), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n52_), .O(z06));
  inv1   g047(.a(x26), .O(new_n99_));
  inv1   g048(.a(new_n93_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nor3   g050(.a(new_n93_), .b(x26), .c(x23), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n101_), .c(x16), .O(new_n103_));
  nor2   g052(.a(x16), .b(x08), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n64_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n103_), .O(z07));
  oai21  g055(.a(new_n93_), .b(x26), .c(x27), .O(new_n107_));
  inv1   g056(.a(new_n87_), .O(new_n108_));
  nor2   g057(.a(x27), .b(x26), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n108_), .c(new_n92_), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n107_), .c(x16), .O(new_n111_));
  aoi21  g060(.a(new_n56_), .b(x07), .c(x23), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n52_), .O(z08));
  nor2   g063(.a(x28), .b(x25), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n109_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n87_), .c(x16), .O(new_n117_));
  aoi21  g066(.a(new_n110_), .b(x28), .c(new_n117_), .O(new_n118_));
  inv1   g067(.a(x06), .O(new_n119_));
  oai21  g068(.a(x16), .b(new_n119_), .c(new_n63_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n118_), .c(new_n52_), .O(z09));
  inv1   g070(.a(x29), .O(new_n122_));
  nor2   g071(.a(new_n116_), .b(new_n87_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  oai21  g073(.a(new_n116_), .b(new_n87_), .c(x29), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n124_), .c(x16), .O(new_n126_));
  aoi21  g075(.a(new_n56_), .b(x05), .c(x23), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n52_), .O(z10));
  nor2   g078(.a(x30), .b(x29), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n115_), .c(new_n109_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n87_), .c(x16), .O(new_n132_));
  aoi21  g081(.a(new_n124_), .b(x30), .c(new_n132_), .O(new_n133_));
  inv1   g082(.a(x04), .O(new_n134_));
  oai21  g083(.a(x16), .b(new_n134_), .c(new_n63_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(new_n133_), .c(new_n52_), .O(z11));
  oai21  g085(.a(new_n131_), .b(new_n87_), .c(x31), .O(new_n137_));
  inv1   g086(.a(x31), .O(new_n138_));
  nor2   g087(.a(new_n131_), .b(new_n87_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n137_), .c(x16), .O(new_n141_));
  aoi21  g090(.a(new_n56_), .b(x03), .c(x23), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n52_), .O(z12));
  inv1   g093(.a(x32), .O(new_n145_));
  aoi21  g094(.a(new_n139_), .b(new_n138_), .c(new_n145_), .O(new_n146_));
  nor2   g095(.a(x32), .b(x31), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n130_), .O(new_n148_));
  nor3   g097(.a(new_n148_), .b(new_n116_), .c(new_n87_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n146_), .c(x16), .O(new_n150_));
  nor2   g099(.a(x16), .b(x02), .O(new_n151_));
  nor2   g100(.a(new_n151_), .b(new_n64_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(z13));
  inv1   g102(.a(new_n148_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n123_), .O(new_n155_));
  nor2   g104(.a(x28), .b(x27), .O(new_n156_));
  nor2   g105(.a(x33), .b(x26), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n156_), .c(new_n147_), .d(new_n130_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n93_), .O(new_n159_));
  aoi21  g108(.a(new_n155_), .b(x33), .c(new_n159_), .O(new_n160_));
  nor2   g109(.a(x16), .b(x01), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n64_), .O(new_n162_));
  oai21  g111(.a(new_n160_), .b(new_n56_), .c(new_n162_), .O(z14));
  oai21  g112(.a(new_n158_), .b(new_n93_), .c(x34), .O(new_n164_));
  nor2   g113(.a(x34), .b(x33), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n156_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n148_), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n100_), .c(new_n99_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n164_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(x16), .O(new_n170_));
  nor2   g119(.a(x16), .b(x00), .O(new_n171_));
  nor2   g120(.a(new_n171_), .b(new_n64_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n170_), .O(z15));
endmodule


