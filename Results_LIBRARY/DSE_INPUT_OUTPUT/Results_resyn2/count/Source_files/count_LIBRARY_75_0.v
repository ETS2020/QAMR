// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x06), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(x15), .c(new_n57_), .O(new_n59_));
  oai21  g008(.a(new_n56_), .b(new_n53_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n52_), .O(z00));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  oai21  g012(.a(new_n53_), .b(new_n63_), .c(x16), .O(new_n64_));
  aoi21  g013(.a(new_n58_), .b(x14), .c(new_n57_), .O(new_n65_));
  oai21  g014(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n52_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(new_n62_), .b(new_n68_), .O(new_n69_));
  nand4  g018(.a(new_n68_), .b(new_n63_), .c(new_n55_), .d(new_n54_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x16), .O(new_n71_));
  aoi21  g020(.a(new_n58_), .b(x13), .c(new_n57_), .O(new_n72_));
  oai21  g021(.a(new_n71_), .b(new_n69_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n52_), .O(z02));
  aoi21  g023(.a(new_n70_), .b(x22), .c(new_n58_), .O(new_n75_));
  oai21  g024(.a(new_n70_), .b(x22), .c(new_n75_), .O(new_n76_));
  aoi21  g025(.a(new_n58_), .b(x12), .c(new_n57_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n52_), .O(z03));
  oai21  g028(.a(new_n70_), .b(x22), .c(x23), .O(new_n80_));
  nor2   g029(.a(x21), .b(x20), .O(new_n81_));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n81_), .c(new_n53_), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  aoi21  g033(.a(new_n58_), .b(x11), .c(new_n57_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n52_), .O(z04));
  nor2   g036(.a(x24), .b(x21), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n82_), .O(new_n89_));
  inv1   g038(.a(new_n89_), .O(new_n90_));
  aoi22  g039(.a(new_n90_), .b(new_n62_), .c(new_n83_), .d(x24), .O(new_n91_));
  nand2  g040(.a(new_n52_), .b(x06), .O(new_n92_));
  nor2   g041(.a(x16), .b(x10), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g043(.a(new_n91_), .b(new_n58_), .c(new_n94_), .O(z05));
  nand4  g044(.a(new_n88_), .b(new_n82_), .c(new_n53_), .d(new_n63_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(x25), .O(new_n97_));
  nor2   g046(.a(x25), .b(x24), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n82_), .c(new_n81_), .d(new_n53_), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n97_), .c(x16), .O(new_n100_));
  aoi21  g049(.a(new_n58_), .b(x09), .c(new_n57_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n52_), .O(z06));
  inv1   g052(.a(new_n83_), .O(new_n104_));
  inv1   g053(.a(x24), .O(new_n105_));
  nor2   g054(.a(x26), .b(x25), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g056(.a(new_n107_), .O(new_n108_));
  aoi22  g057(.a(new_n108_), .b(new_n104_), .c(new_n99_), .d(x26), .O(new_n109_));
  nor2   g058(.a(x16), .b(x08), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n92_), .O(new_n111_));
  oai21  g060(.a(new_n109_), .b(new_n58_), .c(new_n111_), .O(z07));
  oai21  g061(.a(new_n107_), .b(new_n83_), .c(x27), .O(new_n113_));
  nor3   g062(.a(x27), .b(x26), .c(x25), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n90_), .c(new_n62_), .O(new_n115_));
  nand3  g064(.a(new_n115_), .b(new_n113_), .c(x16), .O(new_n116_));
  aoi21  g065(.a(new_n58_), .b(x07), .c(new_n57_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n52_), .O(z08));
  inv1   g068(.a(new_n92_), .O(new_n120_));
  inv1   g069(.a(x27), .O(new_n121_));
  nand3  g070(.a(new_n106_), .b(new_n121_), .c(new_n105_), .O(new_n122_));
  inv1   g071(.a(x28), .O(new_n123_));
  nand4  g072(.a(new_n82_), .b(new_n81_), .c(new_n53_), .d(new_n123_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  aoi21  g074(.a(new_n115_), .b(x28), .c(new_n125_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n58_), .c(new_n120_), .O(z09));
  oai21  g076(.a(new_n124_), .b(new_n122_), .c(x29), .O(new_n128_));
  inv1   g077(.a(new_n122_), .O(new_n129_));
  nor2   g078(.a(x29), .b(x28), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n129_), .c(new_n104_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(x16), .O(new_n133_));
  nor2   g082(.a(x16), .b(x05), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n92_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n133_), .O(z10));
  nand4  g085(.a(new_n130_), .b(new_n82_), .c(new_n81_), .d(new_n53_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n122_), .c(x30), .O(new_n138_));
  nor2   g087(.a(x30), .b(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n130_), .c(new_n108_), .d(new_n104_), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n138_), .c(x16), .O(new_n141_));
  aoi21  g090(.a(new_n58_), .b(x04), .c(new_n57_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n52_), .O(z11));
  nand4  g093(.a(new_n139_), .b(new_n130_), .c(new_n106_), .d(new_n105_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n83_), .c(x31), .O(new_n146_));
  inv1   g095(.a(x29), .O(new_n147_));
  inv1   g096(.a(x30), .O(new_n148_));
  inv1   g097(.a(x31), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n148_), .c(new_n147_), .d(new_n123_), .O(new_n150_));
  inv1   g099(.a(new_n150_), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n114_), .c(new_n90_), .d(new_n62_), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n146_), .c(x16), .O(new_n153_));
  aoi21  g102(.a(new_n58_), .b(x03), .c(new_n57_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n52_), .O(z12));
  nor2   g105(.a(x32), .b(x31), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n139_), .c(new_n130_), .d(new_n106_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n96_), .O(new_n159_));
  aoi21  g108(.a(new_n152_), .b(x32), .c(new_n159_), .O(new_n160_));
  nor2   g109(.a(x16), .b(x02), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n92_), .O(new_n162_));
  oai21  g111(.a(new_n160_), .b(new_n58_), .c(new_n162_), .O(z13));
  oai21  g112(.a(new_n158_), .b(new_n96_), .c(x33), .O(new_n164_));
  inv1   g113(.a(x22), .O(new_n165_));
  inv1   g114(.a(x23), .O(new_n166_));
  inv1   g115(.a(x25), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n105_), .c(new_n166_), .d(new_n165_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n70_), .O(new_n169_));
  inv1   g118(.a(x26), .O(new_n170_));
  inv1   g119(.a(x32), .O(new_n171_));
  inv1   g120(.a(x33), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n171_), .c(new_n121_), .d(new_n170_), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n150_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n169_), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n164_), .c(x16), .O(new_n176_));
  aoi21  g125(.a(new_n58_), .b(x01), .c(new_n57_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n52_), .O(z14));
  inv1   g128(.a(x34), .O(new_n180_));
  nand3  g129(.a(new_n174_), .b(new_n169_), .c(new_n180_), .O(new_n181_));
  nor2   g130(.a(x27), .b(x26), .O(new_n182_));
  nor2   g131(.a(x31), .b(x30), .O(new_n183_));
  nor2   g132(.a(x33), .b(x32), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n183_), .c(new_n130_), .d(new_n182_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n99_), .c(x34), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n181_), .c(x16), .O(new_n187_));
  aoi21  g136(.a(new_n58_), .b(x00), .c(new_n57_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n52_), .O(z15));
endmodule


