// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_;
  inv1   g000(.a(x17), .O(new_n52_));
  nand2  g001(.a(new_n52_), .b(x16), .O(new_n53_));
  nor2   g002(.a(x19), .b(x18), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand3  g006(.a(new_n57_), .b(new_n54_), .c(new_n53_), .O(z00));
  inv1   g007(.a(x18), .O(new_n59_));
  nor2   g008(.a(x20), .b(x17), .O(new_n60_));
  nand2  g009(.a(x20), .b(x17), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(x16), .O(new_n62_));
  aoi21  g011(.a(new_n56_), .b(x14), .c(x19), .O(new_n63_));
  oai21  g012(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n59_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  nor2   g015(.a(new_n60_), .b(new_n66_), .O(new_n67_));
  nor3   g016(.a(x21), .b(x20), .c(x17), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n67_), .c(x16), .O(new_n69_));
  inv1   g018(.a(new_n54_), .O(new_n70_));
  nor2   g019(.a(x16), .b(x13), .O(new_n71_));
  nor2   g020(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n69_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  nor2   g023(.a(new_n68_), .b(new_n74_), .O(new_n75_));
  nor2   g024(.a(x22), .b(x21), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n60_), .O(new_n77_));
  inv1   g026(.a(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n75_), .c(x16), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n70_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n79_), .O(z03));
  xor2a  g031(.a(new_n77_), .b(x23), .O(new_n83_));
  nor2   g032(.a(x16), .b(x11), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n70_), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n56_), .c(new_n85_), .O(z04));
  inv1   g035(.a(x23), .O(new_n87_));
  nand3  g036(.a(new_n76_), .b(new_n60_), .c(new_n87_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(x24), .O(new_n89_));
  nor2   g038(.a(x24), .b(x23), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n76_), .c(new_n60_), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n89_), .c(x16), .O(new_n92_));
  aoi21  g041(.a(new_n56_), .b(x10), .c(x19), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n59_), .O(z05));
  nand2  g044(.a(new_n91_), .b(x25), .O(new_n96_));
  nor2   g045(.a(x25), .b(x22), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n90_), .c(new_n68_), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n96_), .c(new_n56_), .O(new_n99_));
  oai21  g048(.a(x16), .b(x09), .c(new_n54_), .O(new_n100_));
  or2    g049(.a(new_n100_), .b(new_n99_), .O(z06));
  nor2   g050(.a(x26), .b(x25), .O(new_n102_));
  nand4  g051(.a(new_n102_), .b(new_n90_), .c(new_n76_), .d(new_n60_), .O(new_n103_));
  inv1   g052(.a(new_n103_), .O(new_n104_));
  aoi21  g053(.a(new_n98_), .b(x26), .c(new_n104_), .O(new_n105_));
  nor2   g054(.a(x16), .b(x08), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n70_), .O(new_n107_));
  oai21  g056(.a(new_n105_), .b(new_n56_), .c(new_n107_), .O(z07));
  nand2  g057(.a(new_n103_), .b(x27), .O(new_n109_));
  inv1   g058(.a(x27), .O(new_n110_));
  nand2  g059(.a(new_n102_), .b(new_n110_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n91_), .O(new_n112_));
  inv1   g061(.a(new_n112_), .O(new_n113_));
  nand3  g062(.a(new_n113_), .b(new_n109_), .c(x16), .O(new_n114_));
  aoi21  g063(.a(new_n56_), .b(x07), .c(x19), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n59_), .O(z08));
  inv1   g066(.a(x28), .O(new_n118_));
  nor2   g067(.a(new_n112_), .b(new_n118_), .O(new_n119_));
  nor3   g068(.a(x28), .b(x27), .c(x26), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n97_), .c(new_n90_), .d(new_n68_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x16), .O(new_n122_));
  aoi21  g071(.a(new_n56_), .b(x06), .c(x19), .O(new_n123_));
  oai21  g072(.a(new_n122_), .b(new_n119_), .c(new_n123_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n59_), .O(z09));
  nor3   g074(.a(x27), .b(x26), .c(x25), .O(new_n126_));
  nor3   g075(.a(x29), .b(x28), .c(x24), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n88_), .O(new_n129_));
  aoi21  g078(.a(new_n121_), .b(x29), .c(new_n129_), .O(new_n130_));
  nor2   g079(.a(x16), .b(x05), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n70_), .O(new_n132_));
  oai21  g081(.a(new_n130_), .b(new_n56_), .c(new_n132_), .O(z10));
  inv1   g082(.a(x30), .O(new_n134_));
  inv1   g083(.a(new_n91_), .O(new_n135_));
  nor2   g084(.a(x30), .b(x29), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n126_), .c(new_n135_), .d(new_n118_), .O(new_n137_));
  oai21  g086(.a(new_n129_), .b(new_n134_), .c(new_n137_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(x16), .O(new_n139_));
  nor2   g088(.a(x16), .b(x04), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n70_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n139_), .O(z11));
  inv1   g091(.a(new_n136_), .O(new_n143_));
  nor3   g092(.a(new_n143_), .b(new_n111_), .c(new_n91_), .O(new_n144_));
  nor2   g093(.a(x31), .b(x28), .O(new_n145_));
  aoi22  g094(.a(new_n145_), .b(new_n144_), .c(new_n137_), .d(x31), .O(new_n146_));
  nor2   g095(.a(x16), .b(x03), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n70_), .O(new_n148_));
  oai21  g097(.a(new_n146_), .b(new_n56_), .c(new_n148_), .O(z12));
  nand3  g098(.a(new_n145_), .b(new_n136_), .c(new_n112_), .O(new_n150_));
  inv1   g099(.a(x31), .O(new_n151_));
  inv1   g100(.a(x32), .O(new_n152_));
  nand3  g101(.a(new_n136_), .b(new_n152_), .c(new_n151_), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n121_), .O(new_n154_));
  aoi21  g103(.a(new_n150_), .b(x32), .c(new_n154_), .O(new_n155_));
  nor2   g104(.a(x16), .b(x02), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n70_), .O(new_n157_));
  oai21  g106(.a(new_n155_), .b(new_n56_), .c(new_n157_), .O(z13));
  nand2  g107(.a(new_n97_), .b(new_n90_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  inv1   g109(.a(new_n153_), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n120_), .c(new_n160_), .d(new_n68_), .O(new_n162_));
  nor2   g111(.a(x27), .b(x26), .O(new_n163_));
  nor2   g112(.a(x33), .b(x32), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n163_), .c(new_n145_), .d(new_n136_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n98_), .c(x16), .O(new_n166_));
  aoi21  g115(.a(new_n162_), .b(x33), .c(new_n166_), .O(new_n167_));
  inv1   g116(.a(x01), .O(new_n168_));
  inv1   g117(.a(x19), .O(new_n169_));
  oai21  g118(.a(x16), .b(new_n168_), .c(new_n169_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n167_), .c(new_n59_), .O(z14));
  nor2   g120(.a(x28), .b(x27), .O(new_n172_));
  nor2   g121(.a(x34), .b(x33), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n161_), .c(new_n172_), .d(new_n104_), .O(new_n174_));
  oai21  g123(.a(new_n165_), .b(new_n98_), .c(x34), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n174_), .c(x16), .O(new_n176_));
  aoi21  g125(.a(new_n56_), .b(x00), .c(x19), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n59_), .O(z15));
endmodule


