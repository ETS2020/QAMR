// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g009(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g010(.a(x14), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n61_), .O(z01));
  nand2  g013(.a(new_n58_), .b(new_n57_), .O(new_n65_));
  nor2   g014(.a(x21), .b(x20), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n58_), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  aoi21  g017(.a(new_n65_), .b(x21), .c(new_n68_), .O(new_n69_));
  inv1   g018(.a(x13), .O(new_n70_));
  aoi21  g019(.a(new_n52_), .b(new_n70_), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n52_), .c(new_n71_), .O(z02));
  nand2  g021(.a(x32), .b(x18), .O(new_n73_));
  inv1   g022(.a(x22), .O(new_n74_));
  nand2  g023(.a(new_n68_), .b(new_n74_), .O(new_n75_));
  aoi21  g024(.a(new_n67_), .b(x22), .c(new_n52_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g026(.a(new_n52_), .b(x12), .c(x18), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n73_), .O(z03));
  nor2   g029(.a(x23), .b(x22), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n66_), .c(new_n58_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  aoi21  g032(.a(new_n75_), .b(x23), .c(new_n83_), .O(new_n84_));
  inv1   g033(.a(x11), .O(new_n85_));
  aoi21  g034(.a(new_n52_), .b(new_n85_), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n52_), .c(new_n86_), .O(z04));
  nand2  g036(.a(new_n82_), .b(x24), .O(new_n88_));
  nor2   g037(.a(x24), .b(x21), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n81_), .c(new_n58_), .d(new_n57_), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n88_), .c(x16), .O(new_n91_));
  aoi21  g040(.a(new_n52_), .b(x10), .c(x18), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n73_), .O(z05));
  nor2   g043(.a(x25), .b(x24), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n81_), .c(new_n66_), .d(new_n58_), .O(new_n96_));
  nand2  g045(.a(new_n90_), .b(x25), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n96_), .c(x16), .O(new_n98_));
  aoi21  g047(.a(new_n52_), .b(x09), .c(x18), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n73_), .O(z06));
  nand2  g050(.a(new_n96_), .b(x26), .O(new_n102_));
  inv1   g051(.a(x18), .O(new_n103_));
  inv1   g052(.a(x26), .O(new_n104_));
  nand2  g053(.a(new_n95_), .b(new_n104_), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n82_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n103_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n102_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(x16), .O(new_n109_));
  inv1   g058(.a(x08), .O(new_n110_));
  aoi21  g059(.a(new_n52_), .b(new_n110_), .c(x18), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n109_), .O(z07));
  inv1   g061(.a(x27), .O(new_n113_));
  nor2   g062(.a(new_n106_), .b(new_n113_), .O(new_n114_));
  nor3   g063(.a(x27), .b(x26), .c(x25), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n89_), .c(new_n81_), .d(new_n60_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(x16), .O(new_n117_));
  aoi21  g066(.a(new_n52_), .b(x07), .c(x18), .O(new_n118_));
  oai21  g067(.a(new_n117_), .b(new_n114_), .c(new_n118_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n73_), .O(z08));
  nand2  g069(.a(new_n116_), .b(x28), .O(new_n121_));
  inv1   g070(.a(new_n90_), .O(new_n122_));
  inv1   g071(.a(x25), .O(new_n123_));
  inv1   g072(.a(x28), .O(new_n124_));
  nor2   g073(.a(x27), .b(x26), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n124_), .c(new_n123_), .O(new_n126_));
  inv1   g075(.a(new_n126_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n122_), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n121_), .c(x16), .O(new_n129_));
  aoi21  g078(.a(new_n52_), .b(x06), .c(x18), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n73_), .O(z09));
  nor3   g081(.a(x29), .b(x28), .c(x27), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n106_), .O(new_n134_));
  oai21  g083(.a(new_n126_), .b(new_n90_), .c(x29), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n134_), .c(x16), .O(new_n136_));
  aoi21  g085(.a(new_n52_), .b(x05), .c(x18), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n73_), .O(z10));
  inv1   g088(.a(x30), .O(new_n140_));
  aoi21  g089(.a(new_n133_), .b(new_n106_), .c(new_n140_), .O(new_n141_));
  inv1   g090(.a(new_n105_), .O(new_n142_));
  nand4  g091(.a(new_n133_), .b(new_n142_), .c(new_n83_), .d(new_n140_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  aoi21  g093(.a(new_n52_), .b(x04), .c(x18), .O(new_n145_));
  oai21  g094(.a(new_n144_), .b(new_n141_), .c(new_n145_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n73_), .O(z11));
  nand2  g096(.a(new_n125_), .b(new_n123_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n90_), .O(new_n149_));
  nor2   g098(.a(x30), .b(x29), .O(new_n150_));
  nor2   g099(.a(x31), .b(x28), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  inv1   g101(.a(new_n152_), .O(new_n153_));
  aoi22  g102(.a(new_n153_), .b(new_n149_), .c(new_n143_), .d(x31), .O(new_n154_));
  inv1   g103(.a(x03), .O(new_n155_));
  aoi21  g104(.a(new_n52_), .b(new_n155_), .c(x18), .O(new_n156_));
  oai21  g105(.a(new_n154_), .b(new_n52_), .c(new_n156_), .O(z12));
  aoi21  g106(.a(new_n153_), .b(new_n149_), .c(new_n52_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(x18), .c(x32), .O(new_n159_));
  inv1   g108(.a(x31), .O(new_n160_));
  nor3   g109(.a(x32), .b(x30), .c(x29), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n127_), .c(new_n122_), .d(new_n160_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(x16), .O(new_n163_));
  aoi21  g112(.a(new_n52_), .b(x02), .c(x18), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n159_), .O(z13));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n151_), .c(new_n150_), .d(new_n125_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n96_), .O(new_n169_));
  aoi21  g118(.a(new_n162_), .b(x33), .c(new_n169_), .O(new_n170_));
  inv1   g119(.a(x01), .O(new_n171_));
  aoi21  g120(.a(new_n52_), .b(new_n171_), .c(x18), .O(new_n172_));
  oai21  g121(.a(new_n170_), .b(new_n52_), .c(new_n172_), .O(z14));
  inv1   g122(.a(x32), .O(new_n174_));
  nor2   g123(.a(x34), .b(new_n52_), .O(new_n175_));
  oai21  g124(.a(new_n168_), .b(new_n96_), .c(new_n175_), .O(new_n176_));
  nand2  g125(.a(new_n52_), .b(x00), .O(new_n177_));
  aoi21  g126(.a(new_n177_), .b(new_n176_), .c(x18), .O(new_n178_));
  nand3  g127(.a(new_n127_), .b(new_n122_), .c(new_n160_), .O(new_n179_));
  inv1   g128(.a(x33), .O(new_n180_));
  nand4  g129(.a(new_n150_), .b(x34), .c(new_n180_), .d(x16), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n179_), .c(new_n103_), .O(new_n182_));
  aoi21  g131(.a(new_n182_), .b(new_n174_), .c(new_n178_), .O(z15));
endmodule


