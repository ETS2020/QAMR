// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_;
  nand2  g000(.a(x18), .b(x01), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x15), .c(x18), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n52_), .O(z00));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  inv1   g010(.a(x20), .O(new_n62_));
  oai21  g011(.a(new_n53_), .b(new_n62_), .c(x16), .O(new_n63_));
  aoi21  g012(.a(new_n57_), .b(x14), .c(x18), .O(new_n64_));
  oai21  g013(.a(new_n63_), .b(new_n61_), .c(new_n64_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n52_), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  nor2   g016(.a(new_n61_), .b(new_n67_), .O(new_n68_));
  nand4  g017(.a(new_n67_), .b(new_n62_), .c(new_n55_), .d(new_n54_), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  oai21  g019(.a(new_n70_), .b(new_n68_), .c(x16), .O(new_n71_));
  inv1   g020(.a(x13), .O(new_n72_));
  aoi21  g021(.a(new_n57_), .b(new_n72_), .c(x18), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n71_), .O(z02));
  inv1   g023(.a(x22), .O(new_n75_));
  nor2   g024(.a(x21), .b(x20), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n53_), .c(new_n75_), .O(new_n77_));
  nand2  g026(.a(new_n69_), .b(x22), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n77_), .c(x16), .O(new_n79_));
  aoi21  g028(.a(new_n57_), .b(x12), .c(x18), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n52_), .O(z03));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n76_), .c(new_n53_), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  aoi21  g034(.a(new_n77_), .b(x23), .c(new_n85_), .O(new_n86_));
  inv1   g035(.a(x11), .O(new_n87_));
  aoi21  g036(.a(new_n57_), .b(new_n87_), .c(x18), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n57_), .c(new_n88_), .O(z04));
  nand2  g038(.a(new_n84_), .b(x24), .O(new_n90_));
  nor2   g039(.a(x24), .b(x23), .O(new_n91_));
  nor2   g040(.a(x22), .b(x21), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n91_), .c(new_n53_), .d(new_n62_), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n90_), .c(x16), .O(new_n94_));
  aoi21  g043(.a(new_n57_), .b(x10), .c(x18), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n52_), .O(z05));
  inv1   g046(.a(x23), .O(new_n98_));
  inv1   g047(.a(x24), .O(new_n99_));
  inv1   g048(.a(x25), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(new_n75_), .O(new_n101_));
  nor2   g050(.a(new_n101_), .b(new_n69_), .O(new_n102_));
  aoi21  g051(.a(new_n93_), .b(x25), .c(new_n102_), .O(new_n103_));
  inv1   g052(.a(x09), .O(new_n104_));
  aoi21  g053(.a(new_n57_), .b(new_n104_), .c(x18), .O(new_n105_));
  oai21  g054(.a(new_n103_), .b(new_n57_), .c(new_n105_), .O(z06));
  nor2   g055(.a(x26), .b(x25), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n99_), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n84_), .O(new_n109_));
  inv1   g058(.a(x26), .O(new_n110_));
  oai21  g059(.a(new_n102_), .b(new_n110_), .c(x16), .O(new_n111_));
  aoi21  g060(.a(new_n57_), .b(x08), .c(x18), .O(new_n112_));
  oai21  g061(.a(new_n111_), .b(new_n109_), .c(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n52_), .O(z07));
  oai21  g063(.a(new_n108_), .b(new_n84_), .c(x27), .O(new_n115_));
  nand2  g064(.a(new_n92_), .b(new_n91_), .O(new_n116_));
  inv1   g065(.a(new_n116_), .O(new_n117_));
  nor3   g066(.a(x27), .b(x26), .c(x25), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n117_), .c(new_n61_), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n115_), .c(x16), .O(new_n120_));
  aoi21  g069(.a(new_n57_), .b(x07), .c(x18), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n52_), .O(z08));
  inv1   g072(.a(x27), .O(new_n124_));
  nand2  g073(.a(new_n107_), .b(new_n124_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n93_), .c(x28), .O(new_n126_));
  inv1   g075(.a(x28), .O(new_n127_));
  inv1   g076(.a(new_n77_), .O(new_n128_));
  nand4  g077(.a(new_n118_), .b(new_n91_), .c(new_n128_), .d(new_n127_), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n126_), .c(x16), .O(new_n130_));
  aoi21  g079(.a(new_n57_), .b(x06), .c(x18), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n52_), .O(z09));
  nand4  g082(.a(new_n107_), .b(new_n91_), .c(new_n127_), .d(new_n124_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n77_), .c(x29), .O(new_n135_));
  inv1   g084(.a(x29), .O(new_n136_));
  nor2   g085(.a(x28), .b(x24), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n118_), .c(new_n85_), .d(new_n136_), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n135_), .c(x16), .O(new_n139_));
  aoi21  g088(.a(new_n57_), .b(x05), .c(x18), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n52_), .O(z10));
  nand4  g091(.a(new_n137_), .b(new_n107_), .c(new_n136_), .d(new_n124_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n84_), .c(x30), .O(new_n144_));
  inv1   g093(.a(new_n108_), .O(new_n145_));
  nor2   g094(.a(x29), .b(x28), .O(new_n146_));
  nor2   g095(.a(x30), .b(x27), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n146_), .c(new_n145_), .d(new_n85_), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n144_), .c(x16), .O(new_n149_));
  aoi21  g098(.a(new_n57_), .b(x04), .c(x18), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n52_), .O(z11));
  nand4  g101(.a(new_n147_), .b(new_n146_), .c(new_n107_), .d(new_n99_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n84_), .c(x31), .O(new_n154_));
  inv1   g103(.a(x30), .O(new_n155_));
  inv1   g104(.a(x31), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n155_), .c(new_n136_), .d(new_n127_), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n118_), .c(new_n117_), .d(new_n61_), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n154_), .c(x16), .O(new_n160_));
  aoi21  g109(.a(new_n57_), .b(x03), .c(x18), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n52_), .O(z12));
  nor2   g112(.a(x32), .b(x31), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n147_), .c(new_n146_), .d(new_n107_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n93_), .O(new_n166_));
  aoi21  g115(.a(new_n159_), .b(x32), .c(new_n166_), .O(new_n167_));
  inv1   g116(.a(x02), .O(new_n168_));
  aoi21  g117(.a(new_n57_), .b(new_n168_), .c(x18), .O(new_n169_));
  oai21  g118(.a(new_n167_), .b(new_n57_), .c(new_n169_), .O(z13));
  oai21  g119(.a(new_n165_), .b(new_n93_), .c(x33), .O(new_n171_));
  inv1   g120(.a(x32), .O(new_n172_));
  inv1   g121(.a(x33), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n172_), .c(new_n124_), .d(new_n110_), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n157_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n102_), .O(new_n176_));
  nand3  g125(.a(new_n176_), .b(new_n171_), .c(x16), .O(new_n177_));
  aoi21  g126(.a(new_n57_), .b(x01), .c(x18), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n177_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n52_), .O(z14));
  inv1   g129(.a(x34), .O(new_n181_));
  nand3  g130(.a(new_n175_), .b(new_n102_), .c(new_n181_), .O(new_n182_));
  nor2   g131(.a(x25), .b(x24), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n83_), .c(new_n76_), .d(new_n53_), .O(new_n184_));
  nor2   g133(.a(x31), .b(x30), .O(new_n185_));
  nor2   g134(.a(x27), .b(x26), .O(new_n186_));
  nor2   g135(.a(x33), .b(x32), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n186_), .c(new_n185_), .d(new_n146_), .O(new_n188_));
  oai21  g137(.a(new_n188_), .b(new_n184_), .c(x34), .O(new_n189_));
  nand3  g138(.a(new_n189_), .b(new_n182_), .c(x16), .O(new_n190_));
  aoi21  g139(.a(new_n57_), .b(x00), .c(x18), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n190_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n52_), .O(z15));
endmodule


