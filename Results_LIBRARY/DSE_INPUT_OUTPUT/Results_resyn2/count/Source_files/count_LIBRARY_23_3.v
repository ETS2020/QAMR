// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n205_, new_n206_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  inv1   g010(.a(x20), .O(new_n62_));
  nand4  g011(.a(new_n62_), .b(new_n53_), .c(new_n52_), .d(x16), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  nand2  g013(.a(x20), .b(x16), .O(new_n65_));
  oai22  g014(.a(new_n65_), .b(new_n55_), .c(x16), .d(x14), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n64_), .c(new_n61_), .O(new_n67_));
  oai21  g016(.a(x21), .b(new_n61_), .c(new_n67_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  aoi21  g018(.a(new_n63_), .b(new_n61_), .c(new_n69_), .O(new_n70_));
  nor3   g019(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  oai21  g020(.a(new_n71_), .b(x21), .c(x16), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n58_), .b(new_n73_), .c(x18), .O(new_n74_));
  aoi21  g023(.a(new_n74_), .b(new_n72_), .c(new_n70_), .O(z02));
  oai21  g024(.a(new_n63_), .b(x22), .c(new_n61_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  nor2   g026(.a(x16), .b(x12), .O(new_n78_));
  nor2   g027(.a(x21), .b(x20), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n55_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  nand2  g030(.a(x22), .b(x16), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(new_n78_), .c(new_n61_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n77_), .O(z03));
  inv1   g034(.a(x22), .O(new_n86_));
  inv1   g035(.a(x23), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(new_n63_), .c(new_n61_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n69_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x11), .O(new_n91_));
  nand2  g040(.a(new_n81_), .b(new_n86_), .O(new_n92_));
  nor2   g041(.a(new_n87_), .b(new_n58_), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(x18), .c(new_n90_), .O(z04));
  nor2   g044(.a(x23), .b(x22), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n79_), .c(new_n55_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(x24), .O(new_n98_));
  nor2   g047(.a(x22), .b(x21), .O(new_n99_));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n99_), .c(new_n55_), .d(new_n62_), .O(new_n101_));
  aoi21  g050(.a(new_n101_), .b(new_n98_), .c(new_n58_), .O(new_n102_));
  oai21  g051(.a(x16), .b(x10), .c(new_n61_), .O(new_n103_));
  or2    g052(.a(new_n103_), .b(new_n102_), .O(z05));
  inv1   g053(.a(x24), .O(new_n105_));
  inv1   g054(.a(x25), .O(new_n106_));
  nand4  g055(.a(new_n106_), .b(new_n105_), .c(new_n87_), .d(new_n86_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n63_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(x18), .c(new_n69_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x09), .O(new_n110_));
  nor2   g059(.a(new_n106_), .b(new_n58_), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(new_n101_), .c(new_n110_), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(x18), .c(new_n109_), .O(z06));
  nor2   g062(.a(x26), .b(x25), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n96_), .c(new_n105_), .O(new_n115_));
  inv1   g064(.a(new_n115_), .O(new_n116_));
  aoi21  g065(.a(new_n116_), .b(new_n64_), .c(x18), .O(new_n117_));
  nor2   g066(.a(x25), .b(x24), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n96_), .c(new_n79_), .d(new_n55_), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(x26), .c(new_n58_), .O(new_n120_));
  inv1   g069(.a(x08), .O(new_n121_));
  oai21  g070(.a(x16), .b(new_n121_), .c(new_n61_), .O(new_n122_));
  oai22  g071(.a(new_n122_), .b(new_n120_), .c(new_n117_), .d(x21), .O(z07));
  nor2   g072(.a(x27), .b(x26), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n106_), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n101_), .O(new_n126_));
  inv1   g075(.a(x27), .O(new_n127_));
  aoi21  g076(.a(new_n116_), .b(new_n81_), .c(new_n127_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n126_), .c(x16), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  aoi21  g079(.a(new_n58_), .b(new_n130_), .c(x18), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n129_), .O(z08));
  nor3   g081(.a(x26), .b(x25), .c(x24), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n96_), .c(new_n71_), .d(x16), .O(new_n134_));
  nor2   g083(.a(x28), .b(x27), .O(new_n135_));
  inv1   g084(.a(new_n135_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n134_), .c(new_n61_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n69_), .O(new_n138_));
  nand2  g087(.a(x28), .b(x16), .O(new_n139_));
  inv1   g088(.a(x06), .O(new_n140_));
  nand2  g089(.a(new_n58_), .b(new_n140_), .O(new_n141_));
  oai21  g090(.a(new_n139_), .b(new_n126_), .c(new_n141_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n61_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n138_), .O(z09));
  nand3  g093(.a(new_n135_), .b(new_n114_), .c(new_n105_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n97_), .c(x29), .O(new_n146_));
  inv1   g095(.a(x29), .O(new_n147_));
  nand4  g096(.a(new_n135_), .b(new_n79_), .c(new_n55_), .d(new_n147_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n115_), .c(new_n146_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  inv1   g099(.a(x05), .O(new_n151_));
  aoi21  g100(.a(new_n58_), .b(new_n151_), .c(x18), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(z10));
  nor2   g102(.a(x30), .b(x29), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n135_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n134_), .c(new_n61_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n69_), .O(new_n157_));
  inv1   g106(.a(x30), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n58_), .O(new_n159_));
  oai21  g108(.a(new_n148_), .b(new_n115_), .c(new_n159_), .O(new_n160_));
  inv1   g109(.a(x04), .O(new_n161_));
  nand2  g110(.a(new_n58_), .b(new_n161_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n61_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n157_), .O(z11));
  nand4  g114(.a(new_n154_), .b(new_n135_), .c(new_n79_), .d(new_n55_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n115_), .c(x31), .O(new_n167_));
  nand2  g116(.a(new_n100_), .b(new_n99_), .O(new_n168_));
  inv1   g117(.a(new_n168_), .O(new_n169_));
  inv1   g118(.a(new_n125_), .O(new_n170_));
  inv1   g119(.a(x28), .O(new_n171_));
  inv1   g120(.a(x31), .O(new_n172_));
  nand4  g121(.a(new_n172_), .b(new_n158_), .c(new_n147_), .d(new_n171_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n170_), .c(new_n169_), .d(new_n71_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n167_), .O(new_n176_));
  nand2  g125(.a(new_n176_), .b(x16), .O(new_n177_));
  inv1   g126(.a(x03), .O(new_n178_));
  aoi21  g127(.a(new_n58_), .b(new_n178_), .c(x18), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n177_), .O(z12));
  nor2   g129(.a(x31), .b(x28), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n154_), .c(new_n124_), .d(new_n106_), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n101_), .c(x32), .O(new_n183_));
  inv1   g132(.a(new_n155_), .O(new_n184_));
  nor2   g133(.a(x32), .b(x31), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n114_), .O(new_n186_));
  inv1   g135(.a(new_n186_), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n184_), .c(new_n169_), .d(new_n71_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n183_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(x16), .O(new_n190_));
  inv1   g139(.a(x02), .O(new_n191_));
  aoi21  g140(.a(new_n58_), .b(new_n191_), .c(x18), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n190_), .O(z13));
  nand4  g142(.a(new_n185_), .b(new_n154_), .c(new_n135_), .d(new_n114_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n101_), .c(x33), .O(new_n195_));
  inv1   g144(.a(new_n119_), .O(new_n196_));
  inv1   g145(.a(x26), .O(new_n197_));
  inv1   g146(.a(x32), .O(new_n198_));
  inv1   g147(.a(x33), .O(new_n199_));
  nand4  g148(.a(new_n199_), .b(new_n198_), .c(new_n127_), .d(new_n197_), .O(new_n200_));
  nor2   g149(.a(new_n200_), .b(new_n173_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n196_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n195_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(x16), .O(new_n204_));
  inv1   g153(.a(x01), .O(new_n205_));
  aoi21  g154(.a(new_n58_), .b(new_n205_), .c(x18), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n204_), .O(z14));
  inv1   g156(.a(x34), .O(new_n208_));
  nand3  g157(.a(new_n201_), .b(new_n108_), .c(new_n208_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(new_n61_), .O(new_n210_));
  nand2  g159(.a(new_n210_), .b(new_n69_), .O(new_n211_));
  nor2   g160(.a(x33), .b(x32), .O(new_n212_));
  nand4  g161(.a(new_n212_), .b(new_n181_), .c(new_n154_), .d(new_n124_), .O(new_n213_));
  nor2   g162(.a(new_n208_), .b(new_n58_), .O(new_n214_));
  oai21  g163(.a(new_n213_), .b(new_n119_), .c(new_n214_), .O(new_n215_));
  inv1   g164(.a(x00), .O(new_n216_));
  nand2  g165(.a(new_n58_), .b(new_n216_), .O(new_n217_));
  nand2  g166(.a(new_n217_), .b(new_n215_), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(new_n61_), .O(new_n219_));
  nand2  g168(.a(new_n219_), .b(new_n211_), .O(z15));
endmodule


