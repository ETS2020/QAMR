// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_;
  inv1   g000(.a(x29), .O(new_n52_));
  inv1   g001(.a(x31), .O(new_n53_));
  nand2  g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  and2   g004(.a(x19), .b(x17), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n57_), .b(new_n63_), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n60_), .b(new_n67_), .c(x18), .O(new_n68_));
  aoi21  g017(.a(new_n68_), .b(new_n66_), .c(new_n55_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nor2   g019(.a(new_n65_), .b(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n65_), .b(new_n70_), .O(new_n72_));
  inv1   g021(.a(new_n72_), .O(new_n73_));
  oai21  g022(.a(new_n73_), .b(new_n71_), .c(x16), .O(new_n74_));
  inv1   g023(.a(x13), .O(new_n75_));
  aoi21  g024(.a(new_n60_), .b(new_n75_), .c(x18), .O(new_n76_));
  aoi21  g025(.a(new_n76_), .b(new_n74_), .c(new_n55_), .O(z02));
  nand2  g026(.a(new_n72_), .b(x22), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n57_), .c(new_n63_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x16), .O(new_n82_));
  inv1   g031(.a(x12), .O(new_n83_));
  aoi21  g032(.a(new_n60_), .b(new_n83_), .c(x18), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(new_n55_), .O(z03));
  nand2  g034(.a(new_n80_), .b(x23), .O(new_n86_));
  inv1   g035(.a(x23), .O(new_n87_));
  nand3  g036(.a(new_n79_), .b(new_n65_), .c(new_n87_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(x16), .O(new_n90_));
  inv1   g039(.a(x11), .O(new_n91_));
  aoi21  g040(.a(new_n60_), .b(new_n91_), .c(x18), .O(new_n92_));
  aoi21  g041(.a(new_n92_), .b(new_n90_), .c(new_n55_), .O(z04));
  nand2  g042(.a(new_n88_), .b(x24), .O(new_n94_));
  nor2   g043(.a(x24), .b(x23), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n79_), .c(new_n57_), .d(new_n63_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(x16), .O(new_n98_));
  inv1   g047(.a(x10), .O(new_n99_));
  aoi21  g048(.a(new_n60_), .b(new_n99_), .c(x18), .O(new_n100_));
  aoi21  g049(.a(new_n100_), .b(new_n98_), .c(new_n55_), .O(z05));
  nand2  g050(.a(new_n96_), .b(x25), .O(new_n102_));
  inv1   g051(.a(x25), .O(new_n103_));
  nand4  g052(.a(new_n95_), .b(new_n79_), .c(new_n65_), .d(new_n103_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  inv1   g055(.a(x09), .O(new_n107_));
  aoi21  g056(.a(new_n60_), .b(new_n107_), .c(x18), .O(new_n108_));
  aoi21  g057(.a(new_n108_), .b(new_n106_), .c(new_n55_), .O(z06));
  nand2  g058(.a(new_n104_), .b(x26), .O(new_n110_));
  inv1   g059(.a(x26), .O(new_n111_));
  inv1   g060(.a(new_n96_), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n111_), .c(new_n103_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g064(.a(x08), .O(new_n116_));
  aoi21  g065(.a(new_n60_), .b(new_n116_), .c(x18), .O(new_n117_));
  aoi21  g066(.a(new_n117_), .b(new_n115_), .c(new_n55_), .O(z07));
  inv1   g067(.a(x27), .O(new_n119_));
  nor2   g068(.a(x26), .b(x25), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n96_), .O(new_n122_));
  aoi21  g071(.a(new_n113_), .b(x27), .c(new_n122_), .O(new_n123_));
  nor2   g072(.a(x16), .b(x07), .O(new_n124_));
  nor3   g073(.a(new_n124_), .b(new_n55_), .c(x18), .O(new_n125_));
  oai21  g074(.a(new_n123_), .b(new_n60_), .c(new_n125_), .O(z08));
  inv1   g075(.a(x28), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  nor3   g077(.a(x28), .b(x24), .c(x23), .O(new_n129_));
  nand4  g078(.a(new_n129_), .b(new_n128_), .c(new_n79_), .d(new_n65_), .O(new_n130_));
  oai21  g079(.a(new_n122_), .b(new_n127_), .c(new_n130_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(x16), .O(new_n132_));
  inv1   g081(.a(x06), .O(new_n133_));
  aoi21  g082(.a(new_n60_), .b(new_n133_), .c(x18), .O(new_n134_));
  aoi21  g083(.a(new_n134_), .b(new_n132_), .c(new_n55_), .O(z09));
  nor2   g084(.a(new_n130_), .b(new_n60_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n53_), .c(new_n52_), .O(new_n137_));
  nor2   g086(.a(new_n52_), .b(new_n60_), .O(new_n138_));
  inv1   g087(.a(x18), .O(new_n139_));
  oai21  g088(.a(x16), .b(x05), .c(new_n139_), .O(new_n140_));
  aoi21  g089(.a(new_n138_), .b(new_n130_), .c(new_n140_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n137_), .O(z10));
  oai21  g091(.a(x16), .b(x04), .c(new_n139_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n54_), .O(new_n144_));
  inv1   g093(.a(x30), .O(new_n145_));
  nand4  g094(.a(new_n120_), .b(new_n95_), .c(new_n127_), .d(new_n119_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n80_), .c(x31), .O(new_n147_));
  aoi21  g096(.a(new_n147_), .b(new_n52_), .c(new_n145_), .O(new_n148_));
  nand4  g097(.a(x31), .b(new_n145_), .c(new_n52_), .d(new_n127_), .O(new_n149_));
  nor3   g098(.a(new_n149_), .b(new_n121_), .c(new_n96_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n148_), .c(x16), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n144_), .O(z11));
  inv1   g101(.a(x03), .O(new_n153_));
  aoi21  g102(.a(new_n60_), .b(new_n153_), .c(x18), .O(new_n154_));
  nand4  g103(.a(new_n145_), .b(new_n52_), .c(new_n127_), .d(new_n119_), .O(new_n155_));
  nor2   g104(.a(new_n53_), .b(new_n60_), .O(new_n156_));
  oai21  g105(.a(new_n155_), .b(new_n113_), .c(new_n156_), .O(new_n157_));
  oai21  g106(.a(new_n154_), .b(new_n55_), .c(new_n157_), .O(z12));
  nand2  g107(.a(x32), .b(x16), .O(new_n159_));
  inv1   g108(.a(x02), .O(new_n160_));
  aoi21  g109(.a(new_n60_), .b(new_n160_), .c(x18), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(new_n159_), .c(new_n55_), .O(z13));
  inv1   g111(.a(x01), .O(new_n163_));
  nand2  g112(.a(new_n60_), .b(new_n163_), .O(new_n164_));
  nand2  g113(.a(x33), .b(x16), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n164_), .c(new_n54_), .d(new_n139_), .O(z14));
  nand2  g115(.a(x34), .b(x16), .O(new_n167_));
  inv1   g116(.a(x00), .O(new_n168_));
  aoi21  g117(.a(new_n60_), .b(new_n168_), .c(x18), .O(new_n169_));
  aoi21  g118(.a(new_n169_), .b(new_n167_), .c(new_n55_), .O(z15));
endmodule


