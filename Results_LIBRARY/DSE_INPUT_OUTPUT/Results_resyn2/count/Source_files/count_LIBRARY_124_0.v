// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x18), .O(new_n52_));
  nor2   g01(.a(x19), .b(x17), .O(new_n53_));
  inv1   g02(.a(x17), .O(new_n54_));
  inv1   g03(.a(x19), .O(new_n55_));
  oai21  g04(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g05(.a(x16), .O(new_n57_));
  inv1   g06(.a(x23), .O(new_n58_));
  aoi21  g07(.a(new_n57_), .b(x15), .c(new_n58_), .O(new_n59_));
  oai21  g08(.a(new_n56_), .b(new_n53_), .c(new_n59_), .O(new_n60_));
  nand2  g09(.a(new_n60_), .b(new_n52_), .O(z00));
  inv1   g10(.a(x20), .O(new_n62_));
  nor2   g11(.a(new_n53_), .b(new_n62_), .O(new_n63_));
  nand3  g12(.a(new_n62_), .b(new_n55_), .c(new_n54_), .O(new_n64_));
  nand2  g13(.a(new_n64_), .b(x16), .O(new_n65_));
  aoi21  g14(.a(new_n57_), .b(x14), .c(new_n58_), .O(new_n66_));
  oai21  g15(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand2  g16(.a(new_n67_), .b(new_n52_), .O(z01));
  nor2   g17(.a(x21), .b(x20), .O(new_n69_));
  aoi22  g18(.a(new_n69_), .b(new_n53_), .c(new_n64_), .d(x21), .O(new_n70_));
  nand2  g19(.a(x23), .b(new_n52_), .O(new_n71_));
  nor2   g20(.a(x16), .b(x13), .O(new_n72_));
  nor2   g21(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g22(.a(new_n70_), .b(new_n57_), .c(new_n73_), .O(z02));
  inv1   g23(.a(x21), .O(new_n75_));
  nand4  g24(.a(new_n75_), .b(new_n62_), .c(new_n55_), .d(new_n54_), .O(new_n76_));
  xor2a  g25(.a(new_n76_), .b(x22), .O(new_n77_));
  nor2   g26(.a(x16), .b(x12), .O(new_n78_));
  nor2   g27(.a(new_n78_), .b(new_n71_), .O(new_n79_));
  oai21  g28(.a(new_n77_), .b(new_n57_), .c(new_n79_), .O(z03));
  inv1   g29(.a(x22), .O(new_n81_));
  nand4  g30(.a(new_n69_), .b(new_n53_), .c(new_n81_), .d(x16), .O(new_n82_));
  aoi21  g31(.a(new_n57_), .b(x11), .c(new_n58_), .O(new_n83_));
  nand2  g32(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g33(.a(new_n84_), .b(new_n52_), .O(z04));
  inv1   g34(.a(new_n71_), .O(new_n86_));
  nand2  g35(.a(x24), .b(x16), .O(new_n87_));
  inv1   g36(.a(x10), .O(new_n88_));
  nand2  g37(.a(new_n57_), .b(new_n88_), .O(new_n89_));
  nand3  g38(.a(new_n89_), .b(new_n87_), .c(new_n86_), .O(z05));
  nand2  g39(.a(x25), .b(x16), .O(new_n91_));
  inv1   g40(.a(x09), .O(new_n92_));
  nand2  g41(.a(new_n57_), .b(new_n92_), .O(new_n93_));
  nand3  g42(.a(new_n93_), .b(new_n91_), .c(new_n86_), .O(z06));
  nand2  g43(.a(new_n57_), .b(x08), .O(new_n95_));
  inv1   g44(.a(x26), .O(new_n96_));
  nand2  g45(.a(new_n96_), .b(x16), .O(new_n97_));
  nand3  g46(.a(new_n97_), .b(new_n95_), .c(x23), .O(new_n98_));
  nand2  g47(.a(new_n98_), .b(new_n52_), .O(z07));
  nand2  g48(.a(new_n57_), .b(x07), .O(new_n100_));
  inv1   g49(.a(x27), .O(new_n101_));
  nand2  g50(.a(new_n101_), .b(x16), .O(new_n102_));
  nand3  g51(.a(new_n102_), .b(new_n100_), .c(x23), .O(new_n103_));
  nand2  g52(.a(new_n103_), .b(new_n52_), .O(z08));
  nand2  g53(.a(x28), .b(x16), .O(new_n105_));
  inv1   g54(.a(x06), .O(new_n106_));
  nand2  g55(.a(new_n57_), .b(new_n106_), .O(new_n107_));
  nand3  g56(.a(new_n107_), .b(new_n105_), .c(new_n86_), .O(z09));
  nand2  g57(.a(x29), .b(x16), .O(new_n109_));
  inv1   g58(.a(x05), .O(new_n110_));
  nand2  g59(.a(new_n57_), .b(new_n110_), .O(new_n111_));
  nand3  g60(.a(new_n111_), .b(new_n109_), .c(new_n86_), .O(z10));
  nand2  g61(.a(x30), .b(x16), .O(new_n113_));
  inv1   g62(.a(x04), .O(new_n114_));
  nand2  g63(.a(new_n57_), .b(new_n114_), .O(new_n115_));
  nand3  g64(.a(new_n115_), .b(new_n113_), .c(new_n86_), .O(z11));
  nand2  g65(.a(x31), .b(x16), .O(new_n117_));
  inv1   g66(.a(x03), .O(new_n118_));
  nand2  g67(.a(new_n57_), .b(new_n118_), .O(new_n119_));
  nand3  g68(.a(new_n119_), .b(new_n117_), .c(new_n86_), .O(z12));
  nand2  g69(.a(x32), .b(x16), .O(new_n121_));
  inv1   g70(.a(x02), .O(new_n122_));
  nand2  g71(.a(new_n57_), .b(new_n122_), .O(new_n123_));
  nand3  g72(.a(new_n123_), .b(new_n121_), .c(new_n86_), .O(z13));
  nand2  g73(.a(x33), .b(x16), .O(new_n125_));
  inv1   g74(.a(x01), .O(new_n126_));
  nand2  g75(.a(new_n57_), .b(new_n126_), .O(new_n127_));
  nand3  g76(.a(new_n127_), .b(new_n125_), .c(new_n86_), .O(z14));
  nand2  g77(.a(new_n57_), .b(x00), .O(new_n129_));
  inv1   g78(.a(x34), .O(new_n130_));
  nand2  g79(.a(new_n130_), .b(x16), .O(new_n131_));
  nand3  g80(.a(new_n131_), .b(new_n129_), .c(x23), .O(new_n132_));
  nand2  g81(.a(new_n132_), .b(new_n52_), .O(z15));
endmodule


