// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x02), .O(new_n57_));
  inv1   g006(.a(x18), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g008(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  oai21  g012(.a(new_n53_), .b(new_n63_), .c(x16), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(x02), .O(new_n65_));
  oai21  g014(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n58_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(new_n62_), .b(new_n68_), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  inv1   g020(.a(new_n71_), .O(new_n72_));
  oai21  g021(.a(new_n72_), .b(new_n69_), .c(x16), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  nor2   g023(.a(new_n74_), .b(new_n59_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n73_), .O(z02));
  inv1   g025(.a(x22), .O(new_n77_));
  nor2   g026(.a(new_n72_), .b(new_n77_), .O(new_n78_));
  nand3  g027(.a(new_n70_), .b(new_n53_), .c(new_n77_), .O(new_n79_));
  inv1   g028(.a(new_n79_), .O(new_n80_));
  oai21  g029(.a(new_n80_), .b(new_n78_), .c(x16), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n59_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n81_), .O(z03));
  nand2  g033(.a(new_n79_), .b(x23), .O(new_n85_));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n70_), .c(new_n53_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(x16), .O(new_n88_));
  aoi21  g037(.a(new_n56_), .b(x11), .c(x02), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n58_), .O(z04));
  nand2  g040(.a(new_n87_), .b(x24), .O(new_n92_));
  nor2   g041(.a(x24), .b(x21), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n86_), .c(new_n53_), .d(new_n63_), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n92_), .c(x16), .O(new_n95_));
  aoi21  g044(.a(new_n56_), .b(x10), .c(x02), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n58_), .O(z05));
  nor2   g047(.a(x25), .b(x24), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n86_), .c(new_n70_), .d(new_n53_), .O(new_n100_));
  inv1   g049(.a(new_n100_), .O(new_n101_));
  aoi21  g050(.a(new_n94_), .b(x25), .c(new_n101_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x09), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n59_), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n56_), .c(new_n104_), .O(z06));
  inv1   g054(.a(new_n87_), .O(new_n106_));
  inv1   g055(.a(x26), .O(new_n107_));
  nand2  g056(.a(new_n99_), .b(new_n107_), .O(new_n108_));
  inv1   g057(.a(new_n108_), .O(new_n109_));
  aoi22  g058(.a(new_n109_), .b(new_n106_), .c(new_n100_), .d(x26), .O(new_n110_));
  nor2   g059(.a(x16), .b(x08), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n59_), .O(new_n112_));
  oai21  g061(.a(new_n110_), .b(new_n56_), .c(new_n112_), .O(z07));
  oai21  g062(.a(new_n108_), .b(new_n87_), .c(x27), .O(new_n114_));
  inv1   g063(.a(x23), .O(new_n115_));
  inv1   g064(.a(x24), .O(new_n116_));
  nand4  g065(.a(new_n116_), .b(new_n115_), .c(new_n77_), .d(new_n68_), .O(new_n117_));
  inv1   g066(.a(new_n117_), .O(new_n118_));
  inv1   g067(.a(x25), .O(new_n119_));
  inv1   g068(.a(x27), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n107_), .c(new_n119_), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n118_), .c(new_n62_), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n114_), .c(x16), .O(new_n124_));
  aoi21  g073(.a(new_n56_), .b(x07), .c(x02), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n58_), .O(z08));
  oai21  g076(.a(new_n121_), .b(new_n94_), .c(x28), .O(new_n128_));
  inv1   g077(.a(x28), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n120_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(x23), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n109_), .c(new_n80_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n128_), .c(x16), .O(new_n133_));
  aoi21  g082(.a(new_n56_), .b(x06), .c(x02), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n58_), .O(z09));
  nor2   g085(.a(new_n130_), .b(x29), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n109_), .c(new_n106_), .O(new_n138_));
  nor2   g087(.a(x28), .b(x27), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n99_), .c(new_n107_), .d(new_n115_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n79_), .c(x29), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n138_), .c(x16), .O(new_n142_));
  aoi21  g091(.a(new_n56_), .b(x05), .c(x02), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n58_), .O(z10));
  nor2   g094(.a(x30), .b(x29), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n139_), .c(new_n99_), .d(new_n107_), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n87_), .O(new_n148_));
  aoi21  g097(.a(new_n138_), .b(x30), .c(new_n148_), .O(new_n149_));
  nor2   g098(.a(x16), .b(x04), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n59_), .O(new_n151_));
  oai21  g100(.a(new_n149_), .b(new_n56_), .c(new_n151_), .O(z11));
  oai21  g101(.a(new_n147_), .b(new_n87_), .c(x31), .O(new_n153_));
  inv1   g102(.a(x29), .O(new_n154_));
  inv1   g103(.a(x30), .O(new_n155_));
  inv1   g104(.a(x31), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n155_), .c(new_n154_), .d(new_n129_), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n122_), .c(new_n118_), .d(new_n62_), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n153_), .c(x16), .O(new_n160_));
  aoi21  g109(.a(new_n56_), .b(x03), .c(x02), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n58_), .O(z12));
  nor2   g112(.a(x26), .b(x25), .O(new_n164_));
  nor2   g113(.a(x32), .b(x31), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n164_), .c(new_n146_), .d(new_n139_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n94_), .c(x16), .O(new_n167_));
  aoi21  g116(.a(new_n159_), .b(x32), .c(new_n167_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(x02), .c(new_n58_), .O(z13));
  inv1   g118(.a(x32), .O(new_n170_));
  inv1   g119(.a(x33), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n170_), .c(new_n120_), .d(new_n107_), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n157_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n101_), .O(new_n174_));
  oai21  g123(.a(new_n166_), .b(new_n94_), .c(x33), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n174_), .c(x16), .O(new_n176_));
  aoi21  g125(.a(new_n56_), .b(x01), .c(x02), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n58_), .O(z14));
  nand3  g128(.a(new_n173_), .b(new_n101_), .c(x34), .O(new_n180_));
  inv1   g129(.a(x34), .O(new_n181_));
  nand2  g130(.a(new_n174_), .b(new_n181_), .O(new_n182_));
  nand3  g131(.a(new_n182_), .b(new_n180_), .c(x16), .O(new_n183_));
  nor2   g132(.a(x16), .b(x00), .O(new_n184_));
  nor2   g133(.a(new_n184_), .b(new_n59_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n183_), .O(z15));
endmodule


