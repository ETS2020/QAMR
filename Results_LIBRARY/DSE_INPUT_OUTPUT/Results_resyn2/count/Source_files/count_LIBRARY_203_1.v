// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x24), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nand2  g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  oai21  g010(.a(x19), .b(x17), .c(x20), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  aoi21  g012(.a(new_n56_), .b(x14), .c(x24), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n52_), .O(z01));
  nor2   g015(.a(x21), .b(x20), .O(new_n67_));
  aoi22  g016(.a(new_n67_), .b(new_n53_), .c(new_n61_), .d(x21), .O(new_n68_));
  nor2   g017(.a(x16), .b(x13), .O(new_n69_));
  inv1   g018(.a(x24), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n52_), .O(new_n71_));
  nor2   g020(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  oai21  g021(.a(new_n68_), .b(new_n56_), .c(new_n72_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  aoi21  g023(.a(new_n67_), .b(new_n53_), .c(new_n74_), .O(new_n75_));
  nand3  g024(.a(new_n67_), .b(new_n53_), .c(new_n74_), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(new_n75_), .c(x16), .O(new_n78_));
  nor2   g027(.a(x16), .b(x12), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n71_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n78_), .O(z03));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n67_), .c(new_n53_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi21  g033(.a(new_n76_), .b(x23), .c(new_n84_), .O(new_n85_));
  nor2   g034(.a(x16), .b(x11), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n71_), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n56_), .c(new_n87_), .O(z04));
  aoi21  g037(.a(new_n56_), .b(x10), .c(x24), .O(new_n89_));
  oai21  g038(.a(new_n84_), .b(new_n56_), .c(new_n89_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n52_), .O(z05));
  xor2a  g040(.a(new_n83_), .b(x25), .O(new_n92_));
  nor2   g041(.a(x16), .b(x09), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n71_), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n56_), .c(new_n94_), .O(z06));
  inv1   g044(.a(x25), .O(new_n96_));
  nand4  g045(.a(new_n82_), .b(new_n67_), .c(new_n53_), .d(new_n96_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(x26), .O(new_n98_));
  inv1   g047(.a(x23), .O(new_n99_));
  inv1   g048(.a(x26), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n96_), .c(new_n99_), .O(new_n101_));
  inv1   g050(.a(new_n101_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n77_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n98_), .c(x16), .O(new_n104_));
  aoi21  g053(.a(new_n56_), .b(x08), .c(x24), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n52_), .O(z07));
  oai21  g056(.a(new_n101_), .b(new_n76_), .c(x27), .O(new_n108_));
  nor3   g057(.a(x27), .b(x26), .c(x25), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n84_), .O(new_n110_));
  nand3  g059(.a(new_n110_), .b(new_n108_), .c(x16), .O(new_n111_));
  aoi21  g060(.a(new_n56_), .b(x07), .c(x24), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n52_), .O(z08));
  nor2   g063(.a(x28), .b(x27), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n100_), .c(new_n96_), .d(new_n99_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n76_), .c(x16), .O(new_n117_));
  aoi21  g066(.a(new_n110_), .b(x28), .c(new_n117_), .O(new_n118_));
  inv1   g067(.a(x06), .O(new_n119_));
  oai21  g068(.a(x16), .b(new_n119_), .c(new_n70_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n118_), .c(new_n52_), .O(z09));
  inv1   g070(.a(x29), .O(new_n122_));
  inv1   g071(.a(x28), .O(new_n123_));
  nor2   g072(.a(x27), .b(x26), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n123_), .c(new_n96_), .O(new_n125_));
  inv1   g074(.a(new_n125_), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n84_), .c(new_n122_), .O(new_n127_));
  oai21  g076(.a(new_n116_), .b(new_n76_), .c(x29), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(x16), .O(new_n129_));
  aoi21  g078(.a(new_n56_), .b(x05), .c(x24), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n52_), .O(z10));
  nand4  g081(.a(new_n82_), .b(new_n67_), .c(new_n53_), .d(new_n122_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n125_), .c(x30), .O(new_n134_));
  nor2   g083(.a(x30), .b(x29), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n126_), .c(new_n84_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  nor2   g087(.a(x16), .b(x04), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n71_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n138_), .O(z11));
  nand4  g090(.a(new_n135_), .b(new_n82_), .c(new_n67_), .d(new_n53_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n125_), .c(x31), .O(new_n143_));
  nor2   g092(.a(x31), .b(x28), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n135_), .O(new_n145_));
  inv1   g094(.a(new_n145_), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n109_), .c(new_n84_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n143_), .c(x16), .O(new_n148_));
  aoi21  g097(.a(new_n56_), .b(x03), .c(x24), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n52_), .O(z12));
  nand4  g100(.a(new_n144_), .b(new_n135_), .c(new_n124_), .d(new_n96_), .O(new_n152_));
  oai21  g101(.a(new_n152_), .b(new_n83_), .c(x32), .O(new_n153_));
  inv1   g102(.a(x27), .O(new_n154_));
  nand2  g103(.a(new_n123_), .b(new_n154_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n101_), .O(new_n156_));
  inv1   g105(.a(x30), .O(new_n157_));
  inv1   g106(.a(x31), .O(new_n158_));
  inv1   g107(.a(x32), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n158_), .c(new_n157_), .d(new_n122_), .O(new_n160_));
  inv1   g109(.a(new_n160_), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n156_), .c(new_n77_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n153_), .c(x16), .O(new_n163_));
  aoi21  g112(.a(new_n56_), .b(x02), .c(x24), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n52_), .O(z13));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n144_), .c(new_n135_), .d(new_n124_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n97_), .O(new_n169_));
  aoi21  g118(.a(new_n162_), .b(x33), .c(new_n169_), .O(new_n170_));
  nor2   g119(.a(x16), .b(x01), .O(new_n171_));
  nor2   g120(.a(new_n171_), .b(new_n71_), .O(new_n172_));
  oai21  g121(.a(new_n170_), .b(new_n56_), .c(new_n172_), .O(z14));
  oai21  g122(.a(new_n168_), .b(new_n97_), .c(x34), .O(new_n174_));
  inv1   g123(.a(x33), .O(new_n175_));
  inv1   g124(.a(x34), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n175_), .c(new_n123_), .d(new_n154_), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n160_), .O(new_n178_));
  nand3  g127(.a(new_n178_), .b(new_n102_), .c(new_n77_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n174_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(x16), .O(new_n181_));
  nor2   g130(.a(x16), .b(x00), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n71_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n181_), .O(z15));
endmodule


