// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_;
  inv1   g000(.a(x01), .O(new_n52_));
  nand2  g001(.a(x18), .b(new_n52_), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  inv1   g003(.a(x17), .O(new_n55_));
  inv1   g004(.a(x19), .O(new_n56_));
  oai21  g005(.a(new_n56_), .b(new_n55_), .c(x16), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(x15), .c(x18), .O(new_n59_));
  oai21  g008(.a(new_n57_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n53_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nand2  g011(.a(new_n54_), .b(new_n62_), .O(new_n63_));
  oai21  g012(.a(x19), .b(x17), .c(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(x14), .c(x18), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n53_), .O(z01));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  aoi22  g018(.a(new_n69_), .b(new_n54_), .c(new_n63_), .d(x21), .O(new_n70_));
  inv1   g019(.a(x13), .O(new_n71_));
  aoi21  g020(.a(new_n58_), .b(new_n71_), .c(x18), .O(new_n72_));
  oai21  g021(.a(new_n70_), .b(new_n58_), .c(new_n72_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  nand3  g023(.a(new_n69_), .b(new_n54_), .c(new_n74_), .O(new_n75_));
  inv1   g024(.a(x21), .O(new_n76_));
  nand4  g025(.a(new_n76_), .b(new_n62_), .c(new_n56_), .d(new_n55_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(x22), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n75_), .c(x16), .O(new_n79_));
  aoi21  g028(.a(new_n58_), .b(x12), .c(x18), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n53_), .O(z03));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n69_), .c(new_n54_), .O(new_n84_));
  inv1   g033(.a(new_n84_), .O(new_n85_));
  aoi21  g034(.a(new_n75_), .b(x23), .c(new_n85_), .O(new_n86_));
  inv1   g035(.a(x11), .O(new_n87_));
  aoi21  g036(.a(new_n58_), .b(new_n87_), .c(x18), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n58_), .c(new_n88_), .O(z04));
  nor2   g038(.a(x24), .b(x23), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n69_), .c(new_n54_), .d(new_n74_), .O(new_n91_));
  nand2  g040(.a(new_n84_), .b(x24), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n91_), .c(x16), .O(new_n93_));
  aoi21  g042(.a(new_n58_), .b(x10), .c(x18), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n53_), .O(z05));
  inv1   g045(.a(x23), .O(new_n97_));
  inv1   g046(.a(x24), .O(new_n98_));
  inv1   g047(.a(x25), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(new_n74_), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n77_), .O(new_n101_));
  aoi21  g050(.a(new_n91_), .b(x25), .c(new_n101_), .O(new_n102_));
  inv1   g051(.a(x09), .O(new_n103_));
  aoi21  g052(.a(new_n58_), .b(new_n103_), .c(x18), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n58_), .c(new_n104_), .O(z06));
  inv1   g054(.a(x26), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n99_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(x24), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n85_), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n83_), .c(new_n69_), .d(new_n54_), .O(new_n111_));
  aoi21  g060(.a(new_n111_), .b(x26), .c(new_n58_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n109_), .O(new_n113_));
  aoi21  g062(.a(new_n58_), .b(x08), .c(x18), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n53_), .O(z07));
  inv1   g065(.a(x27), .O(new_n117_));
  nor2   g066(.a(x26), .b(x25), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n90_), .c(new_n117_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n75_), .c(x16), .O(new_n120_));
  aoi21  g069(.a(new_n109_), .b(x27), .c(new_n120_), .O(new_n121_));
  inv1   g070(.a(x07), .O(new_n122_));
  inv1   g071(.a(x18), .O(new_n123_));
  oai21  g072(.a(x16), .b(new_n122_), .c(new_n123_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n121_), .c(new_n53_), .O(z08));
  inv1   g074(.a(x28), .O(new_n126_));
  inv1   g075(.a(new_n75_), .O(new_n127_));
  inv1   g076(.a(new_n119_), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(new_n129_));
  oai21  g078(.a(new_n119_), .b(new_n75_), .c(x28), .O(new_n130_));
  nand3  g079(.a(new_n130_), .b(new_n129_), .c(x16), .O(new_n131_));
  aoi21  g080(.a(new_n58_), .b(x06), .c(x18), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n53_), .O(z09));
  nor2   g083(.a(x29), .b(x28), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n118_), .c(new_n117_), .d(new_n98_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n84_), .c(x16), .O(new_n137_));
  aoi21  g086(.a(new_n129_), .b(x29), .c(new_n137_), .O(new_n138_));
  inv1   g087(.a(x05), .O(new_n139_));
  oai21  g088(.a(x16), .b(new_n139_), .c(new_n123_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n138_), .c(new_n53_), .O(z10));
  oai21  g090(.a(new_n136_), .b(new_n84_), .c(x30), .O(new_n142_));
  nor2   g091(.a(x30), .b(x27), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n135_), .O(new_n144_));
  inv1   g093(.a(new_n144_), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n108_), .c(new_n85_), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n142_), .c(x16), .O(new_n147_));
  aoi21  g096(.a(new_n58_), .b(x04), .c(x18), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n53_), .O(z11));
  nand4  g099(.a(new_n143_), .b(new_n135_), .c(new_n118_), .d(new_n98_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n84_), .c(x31), .O(new_n152_));
  inv1   g101(.a(x29), .O(new_n153_));
  inv1   g102(.a(x30), .O(new_n154_));
  inv1   g103(.a(x31), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n154_), .c(new_n153_), .d(new_n126_), .O(new_n156_));
  inv1   g105(.a(new_n156_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n128_), .c(new_n127_), .O(new_n158_));
  nand3  g107(.a(new_n158_), .b(new_n152_), .c(x16), .O(new_n159_));
  aoi21  g108(.a(new_n58_), .b(x03), .c(x18), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n53_), .O(z12));
  nor2   g111(.a(x32), .b(x31), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n143_), .c(new_n135_), .d(new_n118_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n91_), .O(new_n165_));
  aoi21  g114(.a(new_n158_), .b(x32), .c(new_n165_), .O(new_n166_));
  inv1   g115(.a(x02), .O(new_n167_));
  aoi21  g116(.a(new_n58_), .b(new_n167_), .c(x18), .O(new_n168_));
  oai21  g117(.a(new_n166_), .b(new_n58_), .c(new_n168_), .O(z13));
  nand2  g118(.a(new_n123_), .b(x16), .O(new_n170_));
  oai21  g119(.a(new_n164_), .b(new_n91_), .c(x33), .O(new_n171_));
  inv1   g120(.a(x32), .O(new_n172_));
  inv1   g121(.a(x33), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n172_), .c(new_n117_), .d(new_n106_), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n156_), .O(new_n175_));
  aoi21  g124(.a(new_n175_), .b(new_n101_), .c(new_n170_), .O(new_n176_));
  aoi22  g125(.a(new_n176_), .b(new_n171_), .c(new_n170_), .d(x01), .O(z14));
  inv1   g126(.a(x34), .O(new_n178_));
  nand3  g127(.a(new_n175_), .b(new_n101_), .c(new_n178_), .O(new_n179_));
  nor2   g128(.a(x31), .b(x30), .O(new_n180_));
  nor2   g129(.a(x27), .b(x26), .O(new_n181_));
  nor2   g130(.a(x33), .b(x32), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n181_), .c(new_n180_), .d(new_n135_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n111_), .c(x34), .O(new_n184_));
  nand3  g133(.a(new_n184_), .b(new_n179_), .c(x16), .O(new_n185_));
  aoi21  g134(.a(new_n58_), .b(x00), .c(x18), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n185_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n53_), .O(z15));
endmodule


