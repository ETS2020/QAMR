// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_;
  inv1   g000(.a(x33), .O(new_n52_));
  nor2   g001(.a(new_n52_), .b(x32), .O(new_n53_));
  and2   g002(.a(x19), .b(x17), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  aoi21  g008(.a(new_n59_), .b(new_n56_), .c(new_n53_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g013(.a(x14), .O(new_n65_));
  aoi21  g014(.a(new_n58_), .b(new_n65_), .c(x18), .O(new_n66_));
  aoi21  g015(.a(new_n66_), .b(new_n64_), .c(new_n53_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n55_), .O(new_n70_));
  oai21  g019(.a(new_n63_), .b(new_n68_), .c(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x16), .O(new_n72_));
  inv1   g021(.a(x13), .O(new_n73_));
  aoi21  g022(.a(new_n58_), .b(new_n73_), .c(x18), .O(new_n74_));
  aoi21  g023(.a(new_n74_), .b(new_n72_), .c(new_n53_), .O(z02));
  xor2a  g024(.a(new_n70_), .b(x22), .O(new_n76_));
  inv1   g025(.a(x18), .O(new_n77_));
  inv1   g026(.a(new_n53_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  oai21  g030(.a(new_n76_), .b(new_n58_), .c(new_n81_), .O(z03));
  inv1   g031(.a(x22), .O(new_n83_));
  nand3  g032(.a(new_n69_), .b(new_n55_), .c(new_n83_), .O(new_n84_));
  nor3   g033(.a(x23), .b(x22), .c(x21), .O(new_n85_));
  aoi22  g034(.a(new_n85_), .b(new_n63_), .c(new_n84_), .d(x23), .O(new_n86_));
  nor2   g035(.a(x16), .b(x11), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n79_), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n58_), .c(new_n88_), .O(z04));
  inv1   g038(.a(x24), .O(new_n90_));
  aoi21  g039(.a(new_n85_), .b(new_n63_), .c(new_n90_), .O(new_n91_));
  nor2   g040(.a(x24), .b(x23), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n69_), .c(new_n55_), .d(new_n83_), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n91_), .c(x16), .O(new_n95_));
  inv1   g044(.a(x10), .O(new_n96_));
  aoi21  g045(.a(new_n58_), .b(new_n96_), .c(x18), .O(new_n97_));
  aoi21  g046(.a(new_n97_), .b(new_n95_), .c(new_n53_), .O(z05));
  nor2   g047(.a(x25), .b(x24), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n85_), .c(new_n63_), .O(new_n100_));
  inv1   g049(.a(new_n100_), .O(new_n101_));
  aoi21  g050(.a(new_n93_), .b(x25), .c(new_n101_), .O(new_n102_));
  nor2   g051(.a(x16), .b(x09), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n79_), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n58_), .c(new_n104_), .O(z06));
  nand2  g054(.a(new_n100_), .b(x26), .O(new_n106_));
  inv1   g055(.a(x26), .O(new_n107_));
  nand4  g056(.a(new_n99_), .b(new_n85_), .c(new_n63_), .d(new_n107_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(x16), .O(new_n110_));
  inv1   g059(.a(x08), .O(new_n111_));
  aoi21  g060(.a(new_n58_), .b(new_n111_), .c(x18), .O(new_n112_));
  aoi21  g061(.a(new_n112_), .b(new_n110_), .c(new_n53_), .O(z07));
  inv1   g062(.a(x23), .O(new_n114_));
  nor2   g063(.a(x22), .b(x21), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n55_), .c(new_n114_), .d(new_n61_), .O(new_n116_));
  nand2  g065(.a(new_n99_), .b(new_n107_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n116_), .c(x27), .O(new_n118_));
  inv1   g067(.a(new_n84_), .O(new_n119_));
  inv1   g068(.a(x27), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n107_), .O(new_n121_));
  inv1   g070(.a(x25), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n90_), .c(new_n114_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n119_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n118_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(x16), .O(new_n127_));
  inv1   g076(.a(x07), .O(new_n128_));
  aoi21  g077(.a(new_n58_), .b(new_n128_), .c(x18), .O(new_n129_));
  aoi21  g078(.a(new_n129_), .b(new_n127_), .c(new_n53_), .O(z08));
  inv1   g079(.a(x28), .O(new_n131_));
  nor2   g080(.a(x27), .b(x26), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n92_), .c(new_n122_), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n84_), .O(new_n134_));
  xor2a  g083(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nor2   g084(.a(x16), .b(x06), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n79_), .O(new_n137_));
  oai21  g086(.a(new_n135_), .b(new_n58_), .c(new_n137_), .O(z09));
  nand2  g087(.a(new_n134_), .b(new_n131_), .O(new_n139_));
  inv1   g088(.a(x29), .O(new_n140_));
  nand4  g089(.a(new_n132_), .b(new_n99_), .c(new_n140_), .d(new_n131_), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n116_), .O(new_n142_));
  aoi21  g091(.a(new_n139_), .b(x29), .c(new_n142_), .O(new_n143_));
  nor2   g092(.a(x16), .b(x05), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n79_), .O(new_n145_));
  oai21  g094(.a(new_n143_), .b(new_n58_), .c(new_n145_), .O(z10));
  oai21  g095(.a(new_n141_), .b(new_n116_), .c(x30), .O(new_n147_));
  nor3   g096(.a(x30), .b(x29), .c(x28), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n124_), .c(new_n119_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  inv1   g100(.a(x04), .O(new_n152_));
  aoi21  g101(.a(new_n58_), .b(new_n152_), .c(x18), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n151_), .c(new_n53_), .O(z11));
  inv1   g103(.a(x30), .O(new_n155_));
  inv1   g104(.a(x31), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n155_), .c(new_n140_), .d(new_n131_), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  aoi22  g107(.a(new_n158_), .b(new_n134_), .c(new_n149_), .d(x31), .O(new_n159_));
  nor2   g108(.a(x16), .b(x03), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n79_), .O(new_n161_));
  oai21  g110(.a(new_n159_), .b(new_n58_), .c(new_n161_), .O(z12));
  inv1   g111(.a(x32), .O(new_n163_));
  aoi21  g112(.a(new_n158_), .b(new_n134_), .c(new_n163_), .O(new_n164_));
  nor2   g113(.a(x33), .b(x32), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n148_), .c(new_n132_), .d(new_n156_), .O(new_n166_));
  nor2   g115(.a(new_n166_), .b(new_n100_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n164_), .c(x16), .O(new_n168_));
  oai21  g117(.a(x16), .b(x02), .c(new_n77_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n78_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n168_), .O(z13));
  nand2  g120(.a(new_n165_), .b(new_n132_), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n157_), .O(new_n173_));
  aoi22  g122(.a(new_n173_), .b(new_n101_), .c(x33), .d(x32), .O(new_n174_));
  oai21  g123(.a(x16), .b(x01), .c(new_n77_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n78_), .O(new_n176_));
  oai21  g125(.a(new_n174_), .b(new_n58_), .c(new_n176_), .O(z14));
  nand3  g126(.a(new_n158_), .b(new_n134_), .c(new_n163_), .O(new_n178_));
  nand3  g127(.a(new_n178_), .b(x34), .c(x16), .O(new_n179_));
  nand2  g128(.a(new_n148_), .b(new_n124_), .O(new_n180_));
  inv1   g129(.a(x34), .O(new_n181_));
  nor2   g130(.a(x31), .b(new_n58_), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n115_), .c(new_n63_), .d(new_n181_), .O(new_n183_));
  oai21  g132(.a(new_n183_), .b(new_n180_), .c(new_n52_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n163_), .O(new_n185_));
  nand2  g134(.a(new_n77_), .b(x00), .O(new_n186_));
  oai21  g135(.a(new_n53_), .b(new_n77_), .c(x16), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n186_), .O(new_n188_));
  nand3  g137(.a(new_n188_), .b(new_n185_), .c(new_n179_), .O(z15));
endmodule


