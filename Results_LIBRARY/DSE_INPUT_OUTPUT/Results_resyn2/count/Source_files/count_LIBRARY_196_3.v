// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  inv1   g000(.a(x30), .O(new_n52_));
  nand2  g001(.a(new_n52_), .b(x18), .O(new_n53_));
  nor2   g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(x19), .b(x17), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x15), .c(x18), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n54_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n53_), .O(z00));
  inv1   g009(.a(x18), .O(new_n61_));
  nand2  g010(.a(new_n57_), .b(x14), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nand2  g012(.a(new_n54_), .b(new_n63_), .O(new_n64_));
  oai21  g013(.a(x19), .b(x17), .c(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n62_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n61_), .O(z01));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  aoi22  g018(.a(new_n69_), .b(new_n54_), .c(new_n64_), .d(x21), .O(new_n70_));
  inv1   g019(.a(x13), .O(new_n71_));
  aoi21  g020(.a(new_n57_), .b(new_n71_), .c(x18), .O(new_n72_));
  oai21  g021(.a(new_n70_), .b(new_n57_), .c(new_n72_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  aoi21  g023(.a(new_n69_), .b(new_n54_), .c(new_n74_), .O(new_n75_));
  nand3  g024(.a(new_n69_), .b(new_n54_), .c(new_n74_), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  oai21  g026(.a(new_n77_), .b(new_n75_), .c(x16), .O(new_n78_));
  inv1   g027(.a(x12), .O(new_n79_));
  aoi21  g028(.a(new_n57_), .b(new_n79_), .c(x18), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n78_), .O(z03));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n69_), .c(new_n54_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi21  g033(.a(new_n76_), .b(x23), .c(new_n84_), .O(new_n85_));
  inv1   g034(.a(x11), .O(new_n86_));
  aoi21  g035(.a(new_n57_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n57_), .c(new_n87_), .O(z04));
  nor2   g037(.a(x24), .b(x23), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n77_), .O(new_n90_));
  aoi21  g039(.a(new_n83_), .b(x24), .c(new_n57_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  aoi21  g041(.a(new_n57_), .b(x10), .c(x18), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n53_), .O(z05));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n82_), .c(new_n69_), .d(new_n54_), .O(new_n97_));
  inv1   g046(.a(new_n97_), .O(new_n98_));
  aoi21  g047(.a(new_n90_), .b(x25), .c(new_n98_), .O(new_n99_));
  inv1   g048(.a(x09), .O(new_n100_));
  aoi21  g049(.a(new_n57_), .b(new_n100_), .c(x18), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n57_), .c(new_n101_), .O(z06));
  nand2  g051(.a(new_n97_), .b(x26), .O(new_n103_));
  nor3   g052(.a(x26), .b(x25), .c(x24), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n84_), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n103_), .c(x16), .O(new_n106_));
  aoi21  g055(.a(new_n57_), .b(x08), .c(x18), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n53_), .O(z07));
  inv1   g058(.a(x27), .O(new_n110_));
  nor2   g059(.a(x26), .b(x25), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n89_), .c(new_n110_), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n76_), .O(new_n113_));
  aoi21  g062(.a(new_n105_), .b(x27), .c(new_n113_), .O(new_n114_));
  inv1   g063(.a(x07), .O(new_n115_));
  aoi21  g064(.a(new_n57_), .b(new_n115_), .c(x18), .O(new_n116_));
  oai21  g065(.a(new_n114_), .b(new_n57_), .c(new_n116_), .O(z08));
  inv1   g066(.a(x28), .O(new_n118_));
  nand2  g067(.a(new_n113_), .b(new_n118_), .O(new_n119_));
  oai21  g068(.a(new_n112_), .b(new_n76_), .c(x28), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n119_), .c(x16), .O(new_n121_));
  aoi21  g070(.a(new_n57_), .b(x06), .c(x18), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n53_), .O(z09));
  inv1   g073(.a(x29), .O(new_n125_));
  aoi21  g074(.a(new_n113_), .b(new_n118_), .c(new_n125_), .O(new_n126_));
  nor3   g075(.a(x29), .b(x28), .c(x27), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n104_), .c(new_n84_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(x16), .O(new_n129_));
  aoi21  g078(.a(new_n57_), .b(x05), .c(x18), .O(new_n130_));
  oai21  g079(.a(new_n129_), .b(new_n126_), .c(new_n130_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n53_), .O(z10));
  xor2a  g081(.a(new_n128_), .b(x30), .O(new_n133_));
  inv1   g082(.a(x04), .O(new_n134_));
  aoi21  g083(.a(new_n57_), .b(new_n134_), .c(x18), .O(new_n135_));
  oai21  g084(.a(new_n133_), .b(new_n57_), .c(new_n135_), .O(z11));
  nand4  g085(.a(new_n127_), .b(new_n104_), .c(new_n84_), .d(new_n52_), .O(new_n137_));
  inv1   g086(.a(x31), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n52_), .c(new_n125_), .d(new_n118_), .O(new_n139_));
  nor3   g088(.a(new_n139_), .b(new_n112_), .c(new_n76_), .O(new_n140_));
  aoi21  g089(.a(new_n137_), .b(x31), .c(new_n140_), .O(new_n141_));
  inv1   g090(.a(x03), .O(new_n142_));
  aoi21  g091(.a(new_n57_), .b(new_n142_), .c(x18), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n57_), .c(new_n143_), .O(z12));
  inv1   g093(.a(x26), .O(new_n145_));
  nor2   g094(.a(x32), .b(x31), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n127_), .c(new_n145_), .d(x16), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n97_), .c(new_n61_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n52_), .O(new_n149_));
  inv1   g098(.a(new_n139_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n113_), .O(new_n151_));
  nor2   g100(.a(x16), .b(x02), .O(new_n152_));
  inv1   g101(.a(x32), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n57_), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n151_), .c(new_n152_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(x18), .c(new_n149_), .O(z13));
  inv1   g105(.a(x33), .O(new_n157_));
  nand3  g106(.a(new_n52_), .b(new_n125_), .c(new_n118_), .O(new_n158_));
  inv1   g107(.a(new_n146_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  aoi21  g109(.a(new_n160_), .b(new_n113_), .c(new_n157_), .O(new_n161_));
  nand4  g110(.a(new_n157_), .b(new_n153_), .c(new_n110_), .d(new_n145_), .O(new_n162_));
  nor3   g111(.a(new_n162_), .b(new_n139_), .c(new_n97_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n161_), .c(x16), .O(new_n164_));
  inv1   g113(.a(x01), .O(new_n165_));
  aoi21  g114(.a(new_n57_), .b(new_n165_), .c(x18), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n164_), .O(z14));
  nand2  g116(.a(new_n157_), .b(new_n153_), .O(new_n168_));
  inv1   g117(.a(x34), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n138_), .c(new_n125_), .d(x16), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n168_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n113_), .c(new_n118_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n61_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n52_), .O(new_n174_));
  nor2   g123(.a(new_n162_), .b(new_n139_), .O(new_n175_));
  nand2  g124(.a(x34), .b(x16), .O(new_n176_));
  aoi21  g125(.a(new_n175_), .b(new_n98_), .c(new_n176_), .O(new_n177_));
  nor2   g126(.a(x16), .b(x00), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n177_), .c(new_n61_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n174_), .O(z15));
endmodule


