// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_;
  inv1   g00(.a(x18), .O(new_n52_));
  nor2   g01(.a(x19), .b(x17), .O(new_n53_));
  nand2  g02(.a(x19), .b(x17), .O(new_n54_));
  nand2  g03(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g04(.a(x16), .O(new_n56_));
  inv1   g05(.a(x28), .O(new_n57_));
  aoi21  g06(.a(new_n56_), .b(x15), .c(new_n57_), .O(new_n58_));
  oai21  g07(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  nand2  g08(.a(new_n59_), .b(new_n52_), .O(z00));
  inv1   g09(.a(x20), .O(new_n61_));
  nor2   g10(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  nor3   g11(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g12(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  nor2   g13(.a(x16), .b(x14), .O(new_n65_));
  nand2  g14(.a(x28), .b(new_n52_), .O(new_n66_));
  nor2   g15(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g16(.a(new_n67_), .b(new_n64_), .O(z01));
  inv1   g17(.a(new_n63_), .O(new_n69_));
  nor2   g18(.a(x21), .b(x20), .O(new_n70_));
  nand2  g19(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  inv1   g20(.a(new_n71_), .O(new_n72_));
  aoi21  g21(.a(new_n69_), .b(x21), .c(new_n72_), .O(new_n73_));
  nor2   g22(.a(x16), .b(x13), .O(new_n74_));
  nor2   g23(.a(new_n74_), .b(new_n66_), .O(new_n75_));
  oai21  g24(.a(new_n73_), .b(new_n56_), .c(new_n75_), .O(z02));
  nor2   g25(.a(x22), .b(x21), .O(new_n77_));
  aoi22  g26(.a(new_n77_), .b(new_n63_), .c(new_n71_), .d(x22), .O(new_n78_));
  nor2   g27(.a(x16), .b(x12), .O(new_n79_));
  nor2   g28(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  oai21  g29(.a(new_n78_), .b(new_n56_), .c(new_n80_), .O(z03));
  inv1   g30(.a(x23), .O(new_n82_));
  aoi21  g31(.a(new_n77_), .b(new_n63_), .c(new_n82_), .O(new_n83_));
  nand4  g32(.a(new_n77_), .b(new_n53_), .c(new_n82_), .d(new_n61_), .O(new_n84_));
  nand2  g33(.a(new_n84_), .b(x16), .O(new_n85_));
  aoi21  g34(.a(new_n56_), .b(x11), .c(new_n57_), .O(new_n86_));
  oai21  g35(.a(new_n85_), .b(new_n83_), .c(new_n86_), .O(new_n87_));
  nand2  g36(.a(new_n87_), .b(new_n52_), .O(z04));
  nand2  g37(.a(new_n84_), .b(x24), .O(new_n89_));
  nor2   g38(.a(x24), .b(x23), .O(new_n90_));
  nand4  g39(.a(new_n90_), .b(new_n77_), .c(new_n53_), .d(new_n61_), .O(new_n91_));
  nand2  g40(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g41(.a(new_n92_), .b(x16), .O(new_n93_));
  nor2   g42(.a(x16), .b(x10), .O(new_n94_));
  nor2   g43(.a(new_n94_), .b(new_n66_), .O(new_n95_));
  nand2  g44(.a(new_n95_), .b(new_n93_), .O(z05));
  nand2  g45(.a(new_n91_), .b(x25), .O(new_n97_));
  nor2   g46(.a(x25), .b(x22), .O(new_n98_));
  nand4  g47(.a(new_n98_), .b(new_n90_), .c(new_n70_), .d(new_n53_), .O(new_n99_));
  nand3  g48(.a(new_n99_), .b(new_n97_), .c(x16), .O(new_n100_));
  aoi21  g49(.a(new_n56_), .b(x09), .c(new_n57_), .O(new_n101_));
  nand2  g50(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g51(.a(new_n102_), .b(new_n52_), .O(z06));
  and2   g52(.a(new_n99_), .b(x26), .O(new_n104_));
  oai21  g53(.a(new_n99_), .b(x26), .c(x16), .O(new_n105_));
  aoi21  g54(.a(new_n56_), .b(x08), .c(new_n57_), .O(new_n106_));
  oai21  g55(.a(new_n105_), .b(new_n104_), .c(new_n106_), .O(new_n107_));
  nand2  g56(.a(new_n107_), .b(new_n52_), .O(z07));
  oai21  g57(.a(new_n99_), .b(x26), .c(x27), .O(new_n109_));
  nor3   g58(.a(x27), .b(x26), .c(x25), .O(new_n110_));
  nand4  g59(.a(new_n110_), .b(new_n90_), .c(new_n77_), .d(new_n63_), .O(new_n111_));
  nand2  g60(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g61(.a(new_n112_), .b(x16), .O(new_n113_));
  nor2   g62(.a(x16), .b(x07), .O(new_n114_));
  nor2   g63(.a(new_n114_), .b(new_n66_), .O(new_n115_));
  nand2  g64(.a(new_n115_), .b(new_n113_), .O(z08));
  nand2  g65(.a(new_n111_), .b(x16), .O(new_n117_));
  nor2   g66(.a(x16), .b(x06), .O(new_n118_));
  nor2   g67(.a(new_n118_), .b(new_n66_), .O(new_n119_));
  nand2  g68(.a(new_n119_), .b(new_n117_), .O(z09));
  inv1   g69(.a(new_n66_), .O(new_n121_));
  nand2  g70(.a(x29), .b(x16), .O(new_n122_));
  inv1   g71(.a(x05), .O(new_n123_));
  nand2  g72(.a(new_n56_), .b(new_n123_), .O(new_n124_));
  nand3  g73(.a(new_n124_), .b(new_n122_), .c(new_n121_), .O(z10));
  nand2  g74(.a(x30), .b(x16), .O(new_n126_));
  inv1   g75(.a(x04), .O(new_n127_));
  nand2  g76(.a(new_n56_), .b(new_n127_), .O(new_n128_));
  nand3  g77(.a(new_n128_), .b(new_n126_), .c(new_n121_), .O(z11));
  nand2  g78(.a(x31), .b(x16), .O(new_n130_));
  inv1   g79(.a(x03), .O(new_n131_));
  nand2  g80(.a(new_n56_), .b(new_n131_), .O(new_n132_));
  nand3  g81(.a(new_n132_), .b(new_n130_), .c(new_n121_), .O(z12));
  nand2  g82(.a(new_n56_), .b(x02), .O(new_n134_));
  inv1   g83(.a(x32), .O(new_n135_));
  nand2  g84(.a(new_n135_), .b(x16), .O(new_n136_));
  nand3  g85(.a(new_n136_), .b(new_n134_), .c(x28), .O(new_n137_));
  nand2  g86(.a(new_n137_), .b(new_n52_), .O(z13));
  nand2  g87(.a(new_n56_), .b(x01), .O(new_n139_));
  inv1   g88(.a(x33), .O(new_n140_));
  nand2  g89(.a(new_n140_), .b(x16), .O(new_n141_));
  nand3  g90(.a(new_n141_), .b(new_n139_), .c(x28), .O(new_n142_));
  nand2  g91(.a(new_n142_), .b(new_n52_), .O(z14));
  nand2  g92(.a(x34), .b(x16), .O(new_n144_));
  inv1   g93(.a(x00), .O(new_n145_));
  nand2  g94(.a(new_n56_), .b(new_n145_), .O(new_n146_));
  nand3  g95(.a(new_n146_), .b(new_n144_), .c(new_n121_), .O(z15));
endmodule


