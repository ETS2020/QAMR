// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_, new_n162_;
  nor2   g000(.a(x19), .b(x17), .O(new_n52_));
  inv1   g001(.a(new_n52_), .O(new_n53_));
  inv1   g002(.a(x16), .O(new_n54_));
  nor2   g003(.a(x18), .b(new_n54_), .O(new_n55_));
  nand2  g004(.a(x19), .b(x17), .O(new_n56_));
  nand3  g005(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(z00));
  inv1   g006(.a(x18), .O(new_n58_));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(new_n52_), .b(new_n59_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n58_), .O(z01));
  inv1   g012(.a(x21), .O(new_n64_));
  nor2   g013(.a(new_n61_), .b(new_n64_), .O(new_n65_));
  nor2   g014(.a(x21), .b(x20), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n52_), .O(new_n67_));
  inv1   g016(.a(new_n67_), .O(new_n68_));
  oai21  g017(.a(new_n68_), .b(new_n65_), .c(x16), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n58_), .O(z02));
  xor2a  g019(.a(new_n67_), .b(x22), .O(new_n71_));
  oai21  g020(.a(new_n71_), .b(new_n54_), .c(new_n58_), .O(z03));
  inv1   g021(.a(x22), .O(new_n73_));
  nand3  g022(.a(new_n66_), .b(new_n52_), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x23), .b(x22), .O(new_n75_));
  nand3  g024(.a(new_n75_), .b(new_n66_), .c(new_n52_), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  aoi21  g026(.a(new_n74_), .b(x23), .c(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n54_), .c(new_n58_), .O(z04));
  nand2  g028(.a(new_n76_), .b(x24), .O(new_n80_));
  nor2   g029(.a(x24), .b(x21), .O(new_n81_));
  nand4  g030(.a(new_n81_), .b(new_n75_), .c(new_n52_), .d(new_n59_), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n80_), .c(new_n55_), .O(z05));
  nand2  g032(.a(new_n82_), .b(x25), .O(new_n84_));
  nor2   g033(.a(x25), .b(x24), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n77_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(x16), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n58_), .O(z06));
  nand2  g038(.a(new_n86_), .b(x26), .O(new_n90_));
  inv1   g039(.a(x26), .O(new_n91_));
  nand3  g040(.a(new_n85_), .b(new_n77_), .c(new_n91_), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n90_), .c(new_n55_), .O(z07));
  nand2  g042(.a(new_n85_), .b(new_n91_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n76_), .c(x27), .O(new_n95_));
  inv1   g044(.a(x23), .O(new_n96_));
  inv1   g045(.a(x24), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n96_), .c(new_n73_), .d(new_n64_), .O(new_n98_));
  inv1   g047(.a(new_n98_), .O(new_n99_));
  nor3   g048(.a(x27), .b(x26), .c(x25), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n99_), .c(new_n61_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n95_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(x16), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n58_), .O(z08));
  nor2   g053(.a(x26), .b(x25), .O(new_n105_));
  nor2   g054(.a(x28), .b(x27), .O(new_n106_));
  nand4  g055(.a(new_n106_), .b(new_n105_), .c(new_n97_), .d(new_n96_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n74_), .O(new_n108_));
  aoi21  g057(.a(new_n101_), .b(x28), .c(new_n108_), .O(new_n109_));
  oai21  g058(.a(new_n109_), .b(new_n54_), .c(new_n58_), .O(z09));
  oai21  g059(.a(new_n107_), .b(new_n74_), .c(x29), .O(new_n111_));
  nor3   g060(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  nor3   g061(.a(x29), .b(x28), .c(x27), .O(new_n113_));
  nand3  g062(.a(new_n113_), .b(new_n112_), .c(new_n77_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n111_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n58_), .O(z10));
  inv1   g066(.a(x29), .O(new_n118_));
  nand4  g067(.a(new_n106_), .b(new_n105_), .c(new_n118_), .d(new_n97_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n76_), .c(x30), .O(new_n120_));
  inv1   g069(.a(x27), .O(new_n121_));
  inv1   g070(.a(x28), .O(new_n122_));
  inv1   g071(.a(x30), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n118_), .c(new_n122_), .d(new_n121_), .O(new_n124_));
  inv1   g073(.a(new_n124_), .O(new_n125_));
  nand3  g074(.a(new_n125_), .b(new_n112_), .c(new_n77_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n120_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(x16), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n58_), .O(z11));
  nor2   g078(.a(x30), .b(x29), .O(new_n130_));
  nand4  g079(.a(new_n130_), .b(new_n106_), .c(new_n105_), .d(new_n97_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n76_), .c(x31), .O(new_n132_));
  inv1   g081(.a(x31), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n123_), .c(new_n118_), .d(new_n122_), .O(new_n134_));
  inv1   g083(.a(new_n134_), .O(new_n135_));
  nand4  g084(.a(new_n135_), .b(new_n100_), .c(new_n99_), .d(new_n61_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n58_), .O(z12));
  inv1   g088(.a(x25), .O(new_n140_));
  nor2   g089(.a(x27), .b(x26), .O(new_n141_));
  nor2   g090(.a(x31), .b(x28), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n130_), .c(new_n141_), .d(new_n140_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n82_), .c(x32), .O(new_n144_));
  inv1   g093(.a(x32), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n133_), .c(new_n91_), .d(new_n140_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n125_), .c(new_n99_), .d(new_n61_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x16), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n58_), .O(z13));
  nand2  g100(.a(new_n148_), .b(x33), .O(new_n152_));
  inv1   g101(.a(new_n86_), .O(new_n153_));
  inv1   g102(.a(x33), .O(new_n154_));
  nand3  g103(.a(new_n141_), .b(new_n154_), .c(new_n145_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n134_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n152_), .c(new_n55_), .O(z14));
  inv1   g107(.a(x34), .O(new_n159_));
  nand3  g108(.a(new_n156_), .b(new_n153_), .c(new_n159_), .O(new_n160_));
  inv1   g109(.a(new_n156_), .O(new_n161_));
  oai21  g110(.a(new_n161_), .b(new_n86_), .c(x34), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n160_), .c(new_n55_), .O(z15));
endmodule


