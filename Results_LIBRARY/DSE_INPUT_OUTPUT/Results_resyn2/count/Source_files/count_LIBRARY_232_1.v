// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x24), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g008(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  oai21  g012(.a(new_n53_), .b(new_n63_), .c(x16), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(x24), .O(new_n65_));
  oai21  g014(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n57_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(new_n62_), .b(new_n68_), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x16), .O(new_n72_));
  aoi21  g021(.a(new_n56_), .b(x13), .c(x24), .O(new_n73_));
  oai21  g022(.a(new_n72_), .b(new_n69_), .c(new_n73_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n57_), .O(z02));
  inv1   g024(.a(x22), .O(new_n76_));
  nand3  g025(.a(new_n70_), .b(new_n53_), .c(new_n76_), .O(new_n77_));
  nand2  g026(.a(new_n71_), .b(x22), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n77_), .c(x16), .O(new_n79_));
  aoi21  g028(.a(new_n56_), .b(x12), .c(x24), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n57_), .O(z03));
  inv1   g031(.a(new_n59_), .O(new_n83_));
  nand2  g032(.a(new_n56_), .b(x11), .O(new_n84_));
  and2   g033(.a(new_n77_), .b(x23), .O(new_n85_));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n70_), .c(new_n53_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(x16), .O(new_n88_));
  oai21  g037(.a(new_n88_), .b(new_n85_), .c(new_n84_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n83_), .O(z04));
  nand2  g039(.a(new_n56_), .b(x10), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n83_), .O(z05));
  nor2   g042(.a(x25), .b(x21), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n86_), .O(new_n95_));
  inv1   g044(.a(new_n95_), .O(new_n96_));
  aoi22  g045(.a(new_n96_), .b(new_n62_), .c(new_n87_), .d(x25), .O(new_n97_));
  nor2   g046(.a(x16), .b(x09), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n59_), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n56_), .c(new_n99_), .O(z06));
  nor2   g049(.a(x26), .b(x25), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n86_), .c(new_n70_), .d(new_n53_), .O(new_n102_));
  nand4  g051(.a(new_n94_), .b(new_n86_), .c(new_n53_), .d(new_n63_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(x26), .O(new_n104_));
  nand3  g053(.a(new_n104_), .b(new_n102_), .c(x16), .O(new_n105_));
  aoi21  g054(.a(new_n56_), .b(x08), .c(x24), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n57_), .O(z07));
  nand2  g057(.a(new_n102_), .b(x27), .O(new_n109_));
  inv1   g058(.a(new_n87_), .O(new_n110_));
  nor3   g059(.a(x27), .b(x26), .c(x25), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n109_), .c(x16), .O(new_n113_));
  aoi21  g062(.a(new_n56_), .b(x07), .c(x24), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n57_), .O(z08));
  inv1   g065(.a(x28), .O(new_n117_));
  aoi21  g066(.a(new_n111_), .b(new_n110_), .c(new_n117_), .O(new_n118_));
  inv1   g067(.a(x26), .O(new_n119_));
  inv1   g068(.a(x27), .O(new_n120_));
  nand3  g069(.a(new_n117_), .b(new_n120_), .c(new_n119_), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  nand3  g071(.a(new_n122_), .b(new_n96_), .c(new_n62_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(x16), .O(new_n124_));
  aoi21  g073(.a(new_n56_), .b(x06), .c(x24), .O(new_n125_));
  oai21  g074(.a(new_n124_), .b(new_n118_), .c(new_n125_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n57_), .O(z09));
  inv1   g076(.a(x23), .O(new_n128_));
  inv1   g077(.a(x25), .O(new_n129_));
  nor2   g078(.a(x27), .b(x26), .O(new_n130_));
  nor2   g079(.a(x29), .b(x28), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n130_), .c(new_n129_), .d(new_n128_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n77_), .O(new_n133_));
  aoi21  g082(.a(new_n123_), .b(x29), .c(new_n133_), .O(new_n134_));
  nor2   g083(.a(x16), .b(x05), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n59_), .O(new_n136_));
  oai21  g085(.a(new_n134_), .b(new_n56_), .c(new_n136_), .O(z10));
  oai21  g086(.a(new_n132_), .b(new_n77_), .c(x30), .O(new_n138_));
  inv1   g087(.a(x30), .O(new_n139_));
  nand4  g088(.a(new_n131_), .b(new_n111_), .c(new_n110_), .d(new_n139_), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n138_), .c(x16), .O(new_n141_));
  aoi21  g090(.a(new_n56_), .b(x04), .c(x24), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n57_), .O(z11));
  nand4  g093(.a(new_n131_), .b(new_n130_), .c(new_n139_), .d(new_n129_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n87_), .c(x31), .O(new_n146_));
  nor2   g095(.a(x31), .b(x30), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n131_), .O(new_n148_));
  inv1   g097(.a(new_n148_), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n111_), .c(new_n110_), .O(new_n150_));
  nand3  g099(.a(new_n150_), .b(new_n146_), .c(x16), .O(new_n151_));
  aoi21  g100(.a(new_n56_), .b(x03), .c(x24), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n57_), .O(z12));
  nor2   g103(.a(new_n121_), .b(new_n103_), .O(new_n155_));
  nor2   g104(.a(x32), .b(x29), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n147_), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  aoi22  g107(.a(new_n158_), .b(new_n155_), .c(new_n150_), .d(x32), .O(new_n159_));
  nor2   g108(.a(x16), .b(x02), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n59_), .O(new_n161_));
  oai21  g110(.a(new_n159_), .b(new_n56_), .c(new_n161_), .O(z13));
  nand4  g111(.a(new_n158_), .b(new_n122_), .c(new_n96_), .d(new_n62_), .O(new_n163_));
  nor2   g112(.a(x33), .b(x32), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n147_), .c(new_n131_), .d(new_n130_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n103_), .c(x16), .O(new_n166_));
  aoi21  g115(.a(new_n163_), .b(x33), .c(new_n166_), .O(new_n167_));
  inv1   g116(.a(x01), .O(new_n168_));
  oai21  g117(.a(x16), .b(new_n168_), .c(new_n58_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n167_), .c(new_n57_), .O(z14));
  nor2   g119(.a(x34), .b(x33), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n117_), .c(new_n120_), .O(new_n172_));
  nor3   g121(.a(new_n172_), .b(new_n157_), .c(new_n102_), .O(new_n173_));
  oai21  g122(.a(new_n165_), .b(new_n103_), .c(x34), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x16), .O(new_n175_));
  aoi21  g124(.a(new_n56_), .b(x00), .c(x24), .O(new_n176_));
  oai21  g125(.a(new_n175_), .b(new_n173_), .c(new_n176_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n57_), .O(z15));
endmodule


