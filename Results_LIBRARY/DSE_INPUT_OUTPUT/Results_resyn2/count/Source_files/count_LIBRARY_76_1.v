// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  inv1   g008(.a(x21), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g010(.a(new_n58_), .b(new_n57_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(x16), .O(new_n67_));
  aoi21  g016(.a(new_n58_), .b(x14), .c(x21), .O(new_n68_));
  oai21  g017(.a(new_n67_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n59_), .O(z01));
  inv1   g019(.a(x13), .O(new_n71_));
  aoi21  g020(.a(new_n58_), .b(new_n71_), .c(new_n61_), .O(new_n72_));
  oai21  g021(.a(new_n66_), .b(new_n58_), .c(new_n72_), .O(z02));
  xor2a  g022(.a(new_n66_), .b(x22), .O(new_n74_));
  nor2   g023(.a(x16), .b(x12), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(new_n61_), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n58_), .c(new_n76_), .O(z03));
  oai21  g026(.a(new_n66_), .b(x22), .c(x23), .O(new_n78_));
  inv1   g027(.a(x22), .O(new_n79_));
  inv1   g028(.a(x23), .O(new_n80_));
  nand4  g029(.a(new_n55_), .b(new_n80_), .c(new_n79_), .d(new_n64_), .O(new_n81_));
  and2   g030(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nor2   g031(.a(x16), .b(x11), .O(new_n83_));
  nor2   g032(.a(new_n83_), .b(new_n61_), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n58_), .c(new_n84_), .O(z04));
  nand2  g034(.a(new_n81_), .b(x24), .O(new_n86_));
  nor3   g035(.a(x20), .b(x19), .c(x17), .O(new_n87_));
  nor3   g036(.a(x24), .b(x23), .c(x22), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n86_), .c(x16), .O(new_n90_));
  aoi21  g039(.a(new_n58_), .b(x10), .c(x21), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n59_), .O(z05));
  xor2a  g042(.a(new_n89_), .b(x25), .O(new_n94_));
  nor2   g043(.a(x16), .b(x09), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n61_), .O(new_n96_));
  oai21  g045(.a(new_n94_), .b(new_n58_), .c(new_n96_), .O(z06));
  inv1   g046(.a(x24), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n80_), .c(new_n79_), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n66_), .O(new_n100_));
  inv1   g049(.a(x25), .O(new_n101_));
  nand3  g050(.a(new_n88_), .b(new_n87_), .c(new_n101_), .O(new_n102_));
  nor2   g051(.a(x26), .b(x25), .O(new_n103_));
  aoi22  g052(.a(new_n103_), .b(new_n100_), .c(new_n102_), .d(x26), .O(new_n104_));
  nor2   g053(.a(x16), .b(x08), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n61_), .O(new_n106_));
  oai21  g055(.a(new_n104_), .b(new_n58_), .c(new_n106_), .O(z07));
  inv1   g056(.a(x27), .O(new_n108_));
  aoi21  g057(.a(new_n103_), .b(new_n100_), .c(new_n108_), .O(new_n109_));
  nand2  g058(.a(new_n103_), .b(new_n108_), .O(new_n110_));
  oai21  g059(.a(new_n110_), .b(new_n89_), .c(x16), .O(new_n111_));
  aoi21  g060(.a(new_n58_), .b(x07), .c(x21), .O(new_n112_));
  oai21  g061(.a(new_n111_), .b(new_n109_), .c(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n59_), .O(z08));
  inv1   g063(.a(new_n61_), .O(new_n115_));
  nand2  g064(.a(new_n58_), .b(x06), .O(new_n116_));
  inv1   g065(.a(x28), .O(new_n117_));
  inv1   g066(.a(new_n110_), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n100_), .c(new_n117_), .O(new_n119_));
  oai21  g068(.a(new_n110_), .b(new_n89_), .c(x28), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n119_), .c(x16), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n116_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n115_), .O(z09));
  nor2   g072(.a(x29), .b(x28), .O(new_n124_));
  nand4  g073(.a(new_n124_), .b(new_n103_), .c(new_n108_), .d(new_n98_), .O(new_n125_));
  nor2   g074(.a(new_n125_), .b(new_n81_), .O(new_n126_));
  aoi21  g075(.a(new_n119_), .b(x29), .c(new_n126_), .O(new_n127_));
  nor2   g076(.a(x16), .b(x05), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n61_), .O(new_n129_));
  oai21  g078(.a(new_n127_), .b(new_n58_), .c(new_n129_), .O(z10));
  inv1   g079(.a(x30), .O(new_n131_));
  nand4  g080(.a(new_n124_), .b(new_n118_), .c(new_n100_), .d(new_n131_), .O(new_n132_));
  oai21  g081(.a(new_n125_), .b(new_n81_), .c(x30), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n132_), .c(x16), .O(new_n134_));
  aoi21  g083(.a(new_n58_), .b(x04), .c(x21), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n59_), .O(z11));
  nor2   g086(.a(x31), .b(x30), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n124_), .c(new_n103_), .d(new_n108_), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n89_), .O(new_n140_));
  aoi21  g089(.a(new_n132_), .b(x31), .c(new_n140_), .O(new_n141_));
  nor2   g090(.a(x16), .b(x03), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n61_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n58_), .c(new_n143_), .O(z12));
  nor2   g093(.a(x32), .b(x29), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n138_), .O(new_n146_));
  nor2   g095(.a(x27), .b(x26), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n117_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand3  g098(.a(new_n149_), .b(new_n100_), .c(new_n101_), .O(new_n150_));
  oai21  g099(.a(new_n139_), .b(new_n89_), .c(x32), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n150_), .c(x16), .O(new_n152_));
  aoi21  g101(.a(new_n58_), .b(x02), .c(x21), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n59_), .O(z13));
  nor2   g104(.a(x33), .b(x32), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n147_), .c(new_n138_), .d(new_n124_), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n102_), .O(new_n158_));
  aoi21  g107(.a(new_n150_), .b(x33), .c(new_n158_), .O(new_n159_));
  nor2   g108(.a(x16), .b(x01), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n61_), .O(new_n161_));
  oai21  g110(.a(new_n159_), .b(new_n58_), .c(new_n161_), .O(z14));
  oai21  g111(.a(new_n157_), .b(new_n102_), .c(x34), .O(new_n163_));
  nor2   g112(.a(x34), .b(x33), .O(new_n164_));
  nand3  g113(.a(new_n164_), .b(new_n145_), .c(new_n138_), .O(new_n165_));
  inv1   g114(.a(new_n165_), .O(new_n166_));
  nand4  g115(.a(new_n166_), .b(new_n118_), .c(new_n100_), .d(new_n117_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n163_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(x16), .O(new_n169_));
  nor2   g118(.a(x16), .b(x00), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n61_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n169_), .O(z15));
endmodule


