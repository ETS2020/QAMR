// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_;
  nand2  g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(new_n53_), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  aoi21  g004(.a(new_n55_), .b(x16), .c(x18), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  oai21  g007(.a(x18), .b(new_n57_), .c(new_n58_), .O(new_n59_));
  oai21  g008(.a(new_n56_), .b(x27), .c(new_n59_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  inv1   g011(.a(x27), .O(new_n63_));
  inv1   g012(.a(x17), .O(new_n64_));
  inv1   g013(.a(x19), .O(new_n65_));
  nand3  g014(.a(new_n61_), .b(new_n65_), .c(new_n64_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n62_), .c(x16), .O(new_n68_));
  inv1   g017(.a(x14), .O(new_n69_));
  aoi21  g018(.a(new_n58_), .b(new_n69_), .c(x18), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z01));
  aoi21  g020(.a(new_n66_), .b(x21), .c(x27), .O(new_n72_));
  oai21  g021(.a(new_n66_), .b(x21), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(x16), .O(new_n74_));
  inv1   g023(.a(x13), .O(new_n75_));
  aoi21  g024(.a(new_n58_), .b(new_n75_), .c(x18), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n74_), .O(z02));
  inv1   g026(.a(x12), .O(new_n78_));
  oai21  g027(.a(x18), .b(new_n78_), .c(new_n58_), .O(new_n79_));
  oai21  g028(.a(new_n66_), .b(x21), .c(x22), .O(new_n80_));
  nor3   g029(.a(x20), .b(x19), .c(x17), .O(new_n81_));
  nor2   g030(.a(x22), .b(x21), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g032(.a(new_n83_), .b(new_n80_), .c(new_n58_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(x18), .c(new_n63_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n79_), .O(z03));
  nand2  g035(.a(x27), .b(x16), .O(new_n87_));
  inv1   g036(.a(new_n87_), .O(new_n88_));
  inv1   g037(.a(x23), .O(new_n89_));
  aoi21  g038(.a(new_n82_), .b(new_n81_), .c(new_n89_), .O(new_n90_));
  inv1   g039(.a(x21), .O(new_n91_));
  inv1   g040(.a(x22), .O(new_n92_));
  nand3  g041(.a(new_n89_), .b(new_n92_), .c(new_n91_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n66_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n90_), .c(x16), .O(new_n95_));
  inv1   g044(.a(x11), .O(new_n96_));
  aoi21  g045(.a(new_n58_), .b(new_n96_), .c(x18), .O(new_n97_));
  aoi21  g046(.a(new_n97_), .b(new_n95_), .c(new_n88_), .O(z04));
  nand4  g047(.a(new_n82_), .b(new_n53_), .c(new_n89_), .d(new_n61_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(x24), .O(new_n100_));
  inv1   g049(.a(x24), .O(new_n101_));
  nor3   g050(.a(x23), .b(x22), .c(x21), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n81_), .c(new_n101_), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n100_), .c(new_n63_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(x16), .O(new_n105_));
  inv1   g054(.a(x10), .O(new_n106_));
  aoi21  g055(.a(new_n58_), .b(new_n106_), .c(x18), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n105_), .O(z05));
  nand2  g057(.a(new_n103_), .b(x25), .O(new_n109_));
  nor2   g058(.a(x25), .b(x24), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n94_), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n109_), .c(new_n63_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(x16), .O(new_n113_));
  inv1   g062(.a(x09), .O(new_n114_));
  aoi21  g063(.a(new_n58_), .b(new_n114_), .c(x18), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n113_), .O(z06));
  inv1   g065(.a(x26), .O(new_n117_));
  aoi21  g066(.a(new_n110_), .b(new_n94_), .c(new_n117_), .O(new_n118_));
  nor3   g067(.a(x26), .b(x25), .c(x24), .O(new_n119_));
  inv1   g068(.a(new_n119_), .O(new_n120_));
  nor2   g069(.a(new_n120_), .b(new_n99_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n118_), .c(x16), .O(new_n122_));
  inv1   g071(.a(x08), .O(new_n123_));
  aoi21  g072(.a(new_n58_), .b(new_n123_), .c(x18), .O(new_n124_));
  aoi21  g073(.a(new_n124_), .b(new_n122_), .c(new_n88_), .O(z07));
  oai21  g074(.a(new_n121_), .b(x27), .c(x16), .O(new_n126_));
  inv1   g075(.a(x07), .O(new_n127_));
  aoi21  g076(.a(new_n58_), .b(new_n127_), .c(x18), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(z08));
  oai21  g078(.a(new_n120_), .b(new_n99_), .c(x28), .O(new_n130_));
  nor2   g079(.a(x28), .b(x26), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n110_), .c(new_n94_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n130_), .c(new_n63_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  inv1   g083(.a(x06), .O(new_n135_));
  aoi21  g084(.a(new_n58_), .b(new_n135_), .c(x18), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n134_), .O(z09));
  nor2   g086(.a(x29), .b(x28), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n119_), .c(new_n102_), .d(new_n81_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n63_), .O(new_n140_));
  aoi21  g089(.a(new_n132_), .b(x29), .c(new_n140_), .O(new_n141_));
  inv1   g090(.a(x05), .O(new_n142_));
  aoi21  g091(.a(new_n58_), .b(new_n142_), .c(x18), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n58_), .c(new_n143_), .O(z10));
  nand2  g093(.a(new_n139_), .b(x30), .O(new_n145_));
  inv1   g094(.a(x30), .O(new_n146_));
  nand4  g095(.a(new_n138_), .b(new_n119_), .c(new_n94_), .d(new_n146_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n145_), .c(new_n63_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(x16), .O(new_n149_));
  inv1   g098(.a(x04), .O(new_n150_));
  aoi21  g099(.a(new_n58_), .b(new_n150_), .c(x18), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n149_), .O(z11));
  nor2   g101(.a(x26), .b(x25), .O(new_n153_));
  nor2   g102(.a(x31), .b(x30), .O(new_n154_));
  nand4  g103(.a(new_n154_), .b(new_n138_), .c(new_n153_), .d(new_n101_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n99_), .c(new_n63_), .O(new_n156_));
  aoi21  g105(.a(new_n147_), .b(x31), .c(new_n156_), .O(new_n157_));
  inv1   g106(.a(x03), .O(new_n158_));
  aoi21  g107(.a(new_n58_), .b(new_n158_), .c(x18), .O(new_n159_));
  oai21  g108(.a(new_n157_), .b(new_n58_), .c(new_n159_), .O(z12));
  oai21  g109(.a(new_n155_), .b(new_n99_), .c(x32), .O(new_n161_));
  inv1   g110(.a(x25), .O(new_n162_));
  nor2   g111(.a(x32), .b(x29), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n154_), .c(new_n131_), .d(new_n162_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n103_), .c(new_n161_), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n63_), .c(x16), .O(new_n166_));
  nor2   g115(.a(x16), .b(x02), .O(new_n167_));
  aoi21  g116(.a(new_n87_), .b(x18), .c(new_n167_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(z13));
  oai21  g118(.a(new_n164_), .b(new_n103_), .c(x33), .O(new_n170_));
  nor2   g119(.a(x33), .b(x32), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n154_), .c(new_n138_), .d(new_n153_), .O(new_n172_));
  inv1   g121(.a(new_n172_), .O(new_n173_));
  nand3  g122(.a(new_n173_), .b(new_n94_), .c(new_n101_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n170_), .c(new_n63_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(x16), .O(new_n176_));
  inv1   g125(.a(x01), .O(new_n177_));
  aoi21  g126(.a(new_n58_), .b(new_n177_), .c(x18), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n176_), .O(z14));
  nor2   g128(.a(x34), .b(x33), .O(new_n180_));
  nand3  g129(.a(new_n180_), .b(new_n163_), .c(new_n154_), .O(new_n181_));
  inv1   g130(.a(new_n181_), .O(new_n182_));
  nand4  g131(.a(new_n182_), .b(new_n131_), .c(new_n110_), .d(new_n94_), .O(new_n183_));
  oai21  g132(.a(new_n172_), .b(new_n103_), .c(x34), .O(new_n184_));
  nand3  g133(.a(new_n184_), .b(new_n183_), .c(new_n63_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(x16), .O(new_n186_));
  inv1   g135(.a(x00), .O(new_n187_));
  aoi21  g136(.a(new_n58_), .b(new_n187_), .c(x18), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n186_), .O(z15));
endmodule


