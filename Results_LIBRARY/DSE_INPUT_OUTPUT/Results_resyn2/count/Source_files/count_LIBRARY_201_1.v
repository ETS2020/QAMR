// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  inv1   g008(.a(x32), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g010(.a(new_n58_), .b(new_n57_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n67_), .b(new_n65_), .c(x16), .O(new_n68_));
  nor2   g017(.a(x16), .b(x14), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n61_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n68_), .O(z01));
  inv1   g020(.a(new_n61_), .O(new_n72_));
  nand2  g021(.a(new_n58_), .b(x13), .O(new_n73_));
  inv1   g022(.a(x21), .O(new_n74_));
  nand3  g023(.a(new_n55_), .b(new_n74_), .c(new_n64_), .O(new_n75_));
  nand2  g024(.a(new_n66_), .b(x21), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n75_), .c(x16), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n72_), .O(z02));
  inv1   g028(.a(x22), .O(new_n80_));
  inv1   g029(.a(new_n75_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g031(.a(new_n75_), .b(x22), .c(new_n58_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g033(.a(new_n58_), .b(x12), .c(x32), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n59_), .O(z03));
  inv1   g036(.a(x23), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n80_), .c(new_n74_), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n66_), .O(new_n90_));
  aoi21  g039(.a(new_n82_), .b(x23), .c(new_n90_), .O(new_n91_));
  nor2   g040(.a(x16), .b(x11), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n61_), .O(new_n93_));
  oai21  g042(.a(new_n91_), .b(new_n58_), .c(new_n93_), .O(z04));
  inv1   g043(.a(x24), .O(new_n95_));
  nor2   g044(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nor2   g045(.a(x23), .b(x22), .O(new_n97_));
  nor2   g046(.a(x24), .b(x21), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n97_), .c(new_n55_), .d(new_n64_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(x16), .O(new_n100_));
  aoi21  g049(.a(new_n58_), .b(x10), .c(x32), .O(new_n101_));
  oai21  g050(.a(new_n100_), .b(new_n96_), .c(new_n101_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n59_), .O(z05));
  nor2   g052(.a(x25), .b(x24), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n90_), .O(new_n105_));
  aoi21  g054(.a(new_n99_), .b(x25), .c(new_n58_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g056(.a(new_n58_), .b(x09), .c(x32), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n59_), .O(z06));
  nand4  g059(.a(new_n97_), .b(new_n55_), .c(new_n74_), .d(new_n64_), .O(new_n111_));
  inv1   g060(.a(x25), .O(new_n112_));
  inv1   g061(.a(x26), .O(new_n113_));
  nand3  g062(.a(new_n113_), .b(new_n112_), .c(new_n95_), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n111_), .O(new_n115_));
  aoi21  g064(.a(new_n105_), .b(x26), .c(new_n115_), .O(new_n116_));
  nor2   g065(.a(x16), .b(x08), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n61_), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n58_), .c(new_n118_), .O(z07));
  oai21  g068(.a(new_n114_), .b(new_n111_), .c(x27), .O(new_n120_));
  nand2  g069(.a(new_n98_), .b(new_n97_), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  inv1   g071(.a(x27), .O(new_n123_));
  nor2   g072(.a(x26), .b(x25), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  inv1   g074(.a(new_n125_), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n122_), .c(new_n67_), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n120_), .c(x16), .O(new_n128_));
  aoi21  g077(.a(new_n58_), .b(x07), .c(x32), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n59_), .O(z08));
  oai21  g080(.a(new_n125_), .b(new_n99_), .c(x28), .O(new_n132_));
  nor2   g081(.a(x28), .b(x27), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n124_), .O(new_n134_));
  inv1   g083(.a(new_n134_), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n122_), .c(new_n67_), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n132_), .c(x16), .O(new_n137_));
  aoi21  g086(.a(new_n58_), .b(x06), .c(x32), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n59_), .O(z09));
  oai21  g089(.a(new_n134_), .b(new_n99_), .c(x29), .O(new_n141_));
  inv1   g090(.a(x28), .O(new_n142_));
  inv1   g091(.a(x29), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n142_), .c(new_n123_), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n114_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n90_), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n141_), .c(x16), .O(new_n147_));
  aoi21  g096(.a(new_n58_), .b(x05), .c(x32), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n59_), .O(z10));
  inv1   g099(.a(x30), .O(new_n151_));
  aoi21  g100(.a(new_n145_), .b(new_n90_), .c(new_n151_), .O(new_n152_));
  nand3  g101(.a(new_n145_), .b(new_n90_), .c(new_n151_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  aoi21  g103(.a(new_n58_), .b(x04), .c(x32), .O(new_n155_));
  oai21  g104(.a(new_n154_), .b(new_n152_), .c(new_n155_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n59_), .O(z11));
  nor2   g106(.a(new_n134_), .b(new_n99_), .O(new_n158_));
  nor3   g107(.a(x31), .b(x30), .c(x29), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n60_), .O(new_n160_));
  inv1   g109(.a(new_n160_), .O(new_n161_));
  aoi22  g110(.a(new_n161_), .b(new_n158_), .c(new_n153_), .d(x31), .O(new_n162_));
  nor2   g111(.a(x16), .b(x03), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n61_), .O(new_n164_));
  oai21  g113(.a(new_n162_), .b(new_n58_), .c(new_n164_), .O(z12));
  inv1   g114(.a(x02), .O(new_n166_));
  nand2  g115(.a(new_n58_), .b(new_n166_), .O(new_n167_));
  nand3  g116(.a(new_n97_), .b(new_n95_), .c(x16), .O(new_n168_));
  inv1   g117(.a(new_n168_), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n159_), .c(new_n135_), .d(new_n81_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n167_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n60_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n59_), .O(z13));
  nor2   g122(.a(x31), .b(x30), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n143_), .c(new_n142_), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n126_), .c(new_n122_), .d(new_n67_), .O(new_n177_));
  nor2   g126(.a(x33), .b(x29), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n174_), .c(new_n133_), .d(new_n124_), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n99_), .O(new_n180_));
  aoi21  g129(.a(new_n177_), .b(x33), .c(new_n180_), .O(new_n181_));
  nor2   g130(.a(x16), .b(x01), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n61_), .O(new_n183_));
  oai21  g132(.a(new_n181_), .b(new_n58_), .c(new_n183_), .O(z14));
  nor2   g133(.a(x34), .b(x33), .O(new_n185_));
  nand3  g134(.a(new_n185_), .b(new_n123_), .c(new_n113_), .O(new_n186_));
  inv1   g135(.a(new_n186_), .O(new_n187_));
  nand4  g136(.a(new_n187_), .b(new_n176_), .c(new_n104_), .d(new_n90_), .O(new_n188_));
  oai21  g137(.a(new_n179_), .b(new_n99_), .c(x34), .O(new_n189_));
  nand3  g138(.a(new_n189_), .b(new_n188_), .c(x16), .O(new_n190_));
  aoi21  g139(.a(new_n58_), .b(x00), .c(x32), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n190_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n59_), .O(z15));
endmodule


