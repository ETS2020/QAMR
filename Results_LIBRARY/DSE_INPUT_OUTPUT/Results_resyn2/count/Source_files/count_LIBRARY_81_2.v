// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x18), .b(x07), .O(new_n58_));
  nand3  g007(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nor2   g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  oai21  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g012(.a(new_n58_), .O(new_n64_));
  nor2   g013(.a(x16), .b(x14), .O(new_n65_));
  nor2   g014(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n63_), .O(z01));
  inv1   g016(.a(x18), .O(new_n68_));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(new_n62_), .b(new_n69_), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n53_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x16), .O(new_n73_));
  aoi21  g022(.a(new_n56_), .b(x13), .c(x07), .O(new_n74_));
  oai21  g023(.a(new_n73_), .b(new_n70_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n68_), .O(z02));
  nor2   g025(.a(x22), .b(x21), .O(new_n77_));
  aoi22  g026(.a(new_n77_), .b(new_n62_), .c(new_n72_), .d(x22), .O(new_n78_));
  nor2   g027(.a(x16), .b(x12), .O(new_n79_));
  nor2   g028(.a(new_n79_), .b(new_n64_), .O(new_n80_));
  oai21  g029(.a(new_n78_), .b(new_n56_), .c(new_n80_), .O(z03));
  nand2  g030(.a(new_n77_), .b(new_n62_), .O(new_n82_));
  xor2a  g031(.a(new_n82_), .b(x23), .O(new_n83_));
  nor2   g032(.a(x16), .b(x11), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n64_), .O(new_n85_));
  oai21  g034(.a(new_n83_), .b(new_n56_), .c(new_n85_), .O(z04));
  inv1   g035(.a(x23), .O(new_n87_));
  nand3  g036(.a(new_n77_), .b(new_n62_), .c(new_n87_), .O(new_n88_));
  nor2   g037(.a(x24), .b(x23), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n77_), .c(new_n53_), .d(new_n60_), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  aoi21  g040(.a(new_n88_), .b(x24), .c(new_n91_), .O(new_n92_));
  nor2   g041(.a(x16), .b(x10), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n56_), .c(new_n94_), .O(z05));
  nand2  g044(.a(new_n90_), .b(x25), .O(new_n96_));
  nor2   g045(.a(x25), .b(x22), .O(new_n97_));
  nand4  g046(.a(new_n97_), .b(new_n89_), .c(new_n71_), .d(new_n53_), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n96_), .c(x16), .O(new_n99_));
  aoi21  g048(.a(new_n56_), .b(x09), .c(x07), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n68_), .O(z06));
  nor2   g051(.a(x26), .b(x25), .O(new_n103_));
  inv1   g052(.a(new_n103_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n90_), .O(new_n105_));
  aoi21  g054(.a(new_n98_), .b(x26), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x08), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n64_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n56_), .c(new_n108_), .O(z07));
  inv1   g058(.a(x07), .O(new_n110_));
  nand4  g059(.a(new_n103_), .b(new_n89_), .c(new_n77_), .d(new_n62_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(x27), .O(new_n112_));
  nor3   g061(.a(x27), .b(x26), .c(x25), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n91_), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n110_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n68_), .O(z08));
  nor2   g066(.a(x28), .b(x27), .O(new_n118_));
  nand3  g067(.a(new_n118_), .b(new_n103_), .c(new_n89_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n82_), .c(x16), .O(new_n120_));
  aoi21  g069(.a(new_n114_), .b(x28), .c(new_n120_), .O(new_n121_));
  inv1   g070(.a(x06), .O(new_n122_));
  oai21  g071(.a(x16), .b(new_n122_), .c(new_n110_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n121_), .c(new_n68_), .O(z09));
  oai21  g073(.a(new_n119_), .b(new_n82_), .c(x29), .O(new_n125_));
  nor3   g074(.a(x29), .b(x28), .c(x27), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n105_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(x16), .O(new_n129_));
  nor2   g078(.a(x16), .b(x05), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n64_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n129_), .O(z10));
  inv1   g081(.a(x30), .O(new_n133_));
  aoi21  g082(.a(new_n126_), .b(new_n105_), .c(new_n133_), .O(new_n134_));
  inv1   g083(.a(x27), .O(new_n135_));
  nor2   g084(.a(x29), .b(x28), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n133_), .c(new_n135_), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n111_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n134_), .c(x16), .O(new_n139_));
  nor2   g088(.a(x16), .b(x04), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n64_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n139_), .O(z11));
  inv1   g091(.a(new_n137_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n105_), .O(new_n144_));
  nor2   g093(.a(x31), .b(x30), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n136_), .c(new_n113_), .O(new_n146_));
  nor2   g095(.a(new_n146_), .b(new_n90_), .O(new_n147_));
  aoi21  g096(.a(new_n144_), .b(x31), .c(new_n147_), .O(new_n148_));
  nor2   g097(.a(x16), .b(x03), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n64_), .O(new_n150_));
  oai21  g099(.a(new_n148_), .b(new_n56_), .c(new_n150_), .O(z12));
  oai21  g100(.a(new_n146_), .b(new_n90_), .c(x32), .O(new_n152_));
  nor2   g101(.a(x32), .b(x31), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n143_), .c(new_n103_), .d(new_n91_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(x16), .O(new_n155_));
  aoi21  g104(.a(new_n56_), .b(x02), .c(x07), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n68_), .O(z13));
  nor2   g107(.a(x27), .b(x26), .O(new_n159_));
  nor2   g108(.a(x33), .b(x32), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n145_), .c(new_n136_), .d(new_n159_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n98_), .O(new_n162_));
  aoi21  g111(.a(new_n154_), .b(x33), .c(new_n162_), .O(new_n163_));
  nor2   g112(.a(x16), .b(x01), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n64_), .O(new_n165_));
  oai21  g114(.a(new_n163_), .b(new_n56_), .c(new_n165_), .O(z14));
  nor3   g115(.a(new_n161_), .b(new_n98_), .c(x34), .O(new_n167_));
  inv1   g116(.a(x34), .O(new_n168_));
  oai21  g117(.a(new_n162_), .b(new_n168_), .c(x16), .O(new_n169_));
  aoi21  g118(.a(new_n56_), .b(x00), .c(x07), .O(new_n170_));
  oai21  g119(.a(new_n169_), .b(new_n167_), .c(new_n170_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n68_), .O(z15));
endmodule


