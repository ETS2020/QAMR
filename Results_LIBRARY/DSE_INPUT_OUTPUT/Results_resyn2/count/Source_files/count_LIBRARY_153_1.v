// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x21), .b(x18), .O(new_n58_));
  nand3  g007(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nor2   g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  oai21  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g012(.a(new_n58_), .O(new_n64_));
  nor2   g013(.a(x16), .b(x14), .O(new_n65_));
  nor2   g014(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n63_), .O(z01));
  inv1   g016(.a(x18), .O(new_n68_));
  nor2   g017(.a(new_n62_), .b(new_n56_), .O(new_n69_));
  inv1   g018(.a(x13), .O(new_n70_));
  inv1   g019(.a(x21), .O(new_n71_));
  oai21  g020(.a(x16), .b(new_n70_), .c(new_n71_), .O(new_n72_));
  oai21  g021(.a(new_n72_), .b(new_n69_), .c(new_n68_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  nand2  g023(.a(new_n62_), .b(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n53_), .b(new_n60_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x22), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(x16), .O(new_n78_));
  aoi21  g027(.a(new_n56_), .b(x12), .c(x21), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n68_), .O(z03));
  inv1   g030(.a(x23), .O(new_n82_));
  nand3  g031(.a(new_n62_), .b(new_n82_), .c(new_n74_), .O(new_n83_));
  nand2  g032(.a(new_n75_), .b(x23), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  aoi21  g034(.a(new_n56_), .b(x11), .c(x21), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n68_), .O(z04));
  inv1   g037(.a(x24), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n53_), .c(new_n89_), .d(new_n60_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(x16), .O(new_n92_));
  aoi21  g041(.a(new_n83_), .b(x24), .c(new_n92_), .O(new_n93_));
  inv1   g042(.a(x10), .O(new_n94_));
  oai21  g043(.a(x16), .b(new_n94_), .c(new_n71_), .O(new_n95_));
  oai21  g044(.a(new_n95_), .b(new_n93_), .c(new_n68_), .O(z05));
  inv1   g045(.a(x25), .O(new_n97_));
  inv1   g046(.a(new_n91_), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g048(.a(new_n91_), .b(x25), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n99_), .c(x16), .O(new_n101_));
  nor2   g050(.a(x16), .b(x09), .O(new_n102_));
  nor2   g051(.a(new_n102_), .b(new_n64_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n101_), .O(z06));
  inv1   g053(.a(x26), .O(new_n105_));
  nor2   g054(.a(new_n100_), .b(new_n105_), .O(new_n106_));
  nor2   g055(.a(x26), .b(x25), .O(new_n107_));
  inv1   g056(.a(new_n107_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n91_), .c(x16), .O(new_n109_));
  aoi21  g058(.a(new_n56_), .b(x08), .c(x21), .O(new_n110_));
  oai21  g059(.a(new_n109_), .b(new_n106_), .c(new_n110_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n68_), .O(z07));
  inv1   g061(.a(x27), .O(new_n113_));
  nor3   g062(.a(x24), .b(x23), .c(x22), .O(new_n114_));
  nand4  g063(.a(new_n107_), .b(new_n114_), .c(new_n62_), .d(new_n113_), .O(new_n115_));
  oai21  g064(.a(new_n108_), .b(new_n91_), .c(x27), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n115_), .c(x16), .O(new_n117_));
  aoi21  g066(.a(new_n56_), .b(x07), .c(x21), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n68_), .O(z08));
  nand2  g069(.a(new_n115_), .b(x28), .O(new_n121_));
  nor3   g070(.a(x28), .b(x27), .c(x26), .O(new_n122_));
  nand4  g071(.a(new_n122_), .b(new_n114_), .c(new_n62_), .d(new_n97_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(x16), .O(new_n125_));
  nor2   g074(.a(x16), .b(x06), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n64_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n125_), .O(z09));
  xor2a  g077(.a(new_n123_), .b(x29), .O(new_n129_));
  nor2   g078(.a(x16), .b(x05), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n64_), .O(new_n131_));
  oai21  g080(.a(new_n129_), .b(new_n56_), .c(new_n131_), .O(z10));
  inv1   g081(.a(x29), .O(new_n133_));
  nand3  g082(.a(new_n122_), .b(new_n100_), .c(new_n133_), .O(new_n134_));
  inv1   g083(.a(x28), .O(new_n135_));
  nor3   g084(.a(x27), .b(x26), .c(x25), .O(new_n136_));
  nor2   g085(.a(x30), .b(x29), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n136_), .c(new_n135_), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n91_), .O(new_n139_));
  aoi21  g088(.a(new_n134_), .b(x30), .c(new_n139_), .O(new_n140_));
  nor2   g089(.a(x16), .b(x04), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n64_), .O(new_n142_));
  oai21  g091(.a(new_n140_), .b(new_n56_), .c(new_n142_), .O(z11));
  oai21  g092(.a(new_n138_), .b(new_n91_), .c(x31), .O(new_n144_));
  nor2   g093(.a(x31), .b(x28), .O(new_n145_));
  nand4  g094(.a(new_n145_), .b(new_n137_), .c(new_n136_), .d(new_n98_), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  aoi21  g096(.a(new_n56_), .b(x03), .c(x21), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n68_), .O(z12));
  inv1   g099(.a(x31), .O(new_n151_));
  inv1   g100(.a(x32), .O(new_n152_));
  nand3  g101(.a(new_n137_), .b(new_n152_), .c(new_n151_), .O(new_n153_));
  inv1   g102(.a(new_n153_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n122_), .c(new_n100_), .O(new_n155_));
  nand3  g104(.a(new_n145_), .b(new_n137_), .c(new_n136_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n91_), .c(x32), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n155_), .c(x16), .O(new_n158_));
  aoi21  g107(.a(new_n56_), .b(x02), .c(x21), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n68_), .O(z13));
  oai21  g110(.a(new_n153_), .b(new_n123_), .c(x33), .O(new_n162_));
  inv1   g111(.a(x33), .O(new_n163_));
  nand4  g112(.a(new_n145_), .b(new_n137_), .c(new_n163_), .d(new_n152_), .O(new_n164_));
  inv1   g113(.a(new_n164_), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n107_), .c(new_n98_), .d(new_n113_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n162_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(x16), .O(new_n168_));
  nor2   g117(.a(x16), .b(x01), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n64_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n168_), .O(z14));
  oai21  g120(.a(new_n164_), .b(new_n115_), .c(x34), .O(new_n172_));
  nor2   g121(.a(x34), .b(x33), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n154_), .c(new_n122_), .d(new_n100_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n172_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(x16), .O(new_n176_));
  nor2   g125(.a(x16), .b(x00), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n64_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n176_), .O(z15));
endmodule


