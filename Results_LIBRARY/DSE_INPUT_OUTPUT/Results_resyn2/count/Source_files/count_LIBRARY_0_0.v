// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n181_;
  inv1   g000(.a(x32), .O(new_n52_));
  inv1   g001(.a(x33), .O(new_n53_));
  nand2  g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  and2   g004(.a(x19), .b(x17), .O(new_n56_));
  nor2   g005(.a(x19), .b(x17), .O(new_n57_));
  oai21  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  aoi21  g009(.a(new_n60_), .b(new_n59_), .c(x18), .O(new_n61_));
  aoi21  g010(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  xor2a  g012(.a(new_n57_), .b(new_n63_), .O(new_n64_));
  inv1   g013(.a(x18), .O(new_n65_));
  nand2  g014(.a(new_n54_), .b(new_n65_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  oai21  g017(.a(new_n64_), .b(new_n60_), .c(new_n68_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  aoi21  g019(.a(new_n57_), .b(new_n63_), .c(new_n70_), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n57_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(new_n71_), .c(x16), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n60_), .b(new_n76_), .c(x18), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n75_), .c(new_n55_), .O(z02));
  inv1   g027(.a(x22), .O(new_n79_));
  nor2   g028(.a(new_n74_), .b(new_n79_), .O(new_n80_));
  nand3  g029(.a(new_n72_), .b(new_n57_), .c(new_n79_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n60_), .b(new_n84_), .c(x18), .O(new_n85_));
  aoi21  g034(.a(new_n85_), .b(new_n83_), .c(new_n55_), .O(z03));
  inv1   g035(.a(x23), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n72_), .c(new_n57_), .O(new_n89_));
  oai21  g038(.a(new_n82_), .b(new_n87_), .c(new_n89_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x16), .O(new_n91_));
  inv1   g040(.a(x11), .O(new_n92_));
  aoi21  g041(.a(new_n60_), .b(new_n92_), .c(x18), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n91_), .c(new_n55_), .O(z04));
  inv1   g043(.a(x24), .O(new_n95_));
  inv1   g044(.a(new_n89_), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g046(.a(new_n88_), .b(new_n72_), .c(new_n57_), .d(new_n95_), .O(new_n98_));
  inv1   g047(.a(new_n98_), .O(new_n99_));
  oai21  g048(.a(new_n99_), .b(new_n97_), .c(x16), .O(new_n100_));
  inv1   g049(.a(x10), .O(new_n101_));
  aoi21  g050(.a(new_n60_), .b(new_n101_), .c(x18), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(new_n100_), .c(new_n55_), .O(z05));
  xor2a  g052(.a(new_n98_), .b(x25), .O(new_n104_));
  nor2   g053(.a(x16), .b(x09), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n66_), .O(new_n106_));
  oai21  g055(.a(new_n104_), .b(new_n60_), .c(new_n106_), .O(z06));
  inv1   g056(.a(x25), .O(new_n108_));
  nand2  g057(.a(new_n99_), .b(new_n108_), .O(new_n109_));
  nor2   g058(.a(x26), .b(x25), .O(new_n110_));
  inv1   g059(.a(new_n110_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n98_), .O(new_n112_));
  aoi21  g061(.a(new_n109_), .b(x26), .c(new_n112_), .O(new_n113_));
  nor2   g062(.a(x16), .b(x08), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n66_), .O(new_n115_));
  oai21  g064(.a(new_n113_), .b(new_n60_), .c(new_n115_), .O(z07));
  nand2  g065(.a(new_n110_), .b(new_n99_), .O(new_n117_));
  inv1   g066(.a(x27), .O(new_n118_));
  nand3  g067(.a(new_n110_), .b(new_n118_), .c(new_n95_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n89_), .O(new_n120_));
  aoi21  g069(.a(new_n117_), .b(x27), .c(new_n120_), .O(new_n121_));
  nor2   g070(.a(x16), .b(x07), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n66_), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n60_), .c(new_n123_), .O(z08));
  oai21  g073(.a(new_n119_), .b(new_n89_), .c(x28), .O(new_n125_));
  inv1   g074(.a(x26), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n108_), .c(new_n95_), .O(new_n127_));
  inv1   g076(.a(x28), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n118_), .c(new_n87_), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n82_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n125_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(x16), .O(new_n133_));
  inv1   g082(.a(x06), .O(new_n134_));
  aoi21  g083(.a(new_n60_), .b(new_n134_), .c(x18), .O(new_n135_));
  aoi21  g084(.a(new_n135_), .b(new_n133_), .c(new_n55_), .O(z09));
  nor2   g085(.a(x27), .b(x23), .O(new_n137_));
  nand4  g086(.a(new_n137_), .b(new_n110_), .c(new_n128_), .d(new_n95_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n81_), .c(x29), .O(new_n139_));
  inv1   g088(.a(new_n119_), .O(new_n140_));
  nor2   g089(.a(x29), .b(x28), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n140_), .c(new_n96_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  inv1   g093(.a(x05), .O(new_n145_));
  aoi21  g094(.a(new_n60_), .b(new_n145_), .c(x18), .O(new_n146_));
  aoi21  g095(.a(new_n146_), .b(new_n144_), .c(new_n55_), .O(z10));
  nor2   g096(.a(x30), .b(x27), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n141_), .c(new_n110_), .d(new_n95_), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n89_), .O(new_n150_));
  aoi21  g099(.a(new_n142_), .b(x30), .c(new_n150_), .O(new_n151_));
  nor2   g100(.a(x16), .b(x04), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n66_), .O(new_n153_));
  oai21  g102(.a(new_n151_), .b(new_n60_), .c(new_n153_), .O(z11));
  oai21  g103(.a(new_n149_), .b(new_n89_), .c(x31), .O(new_n155_));
  inv1   g104(.a(x30), .O(new_n156_));
  inv1   g105(.a(x31), .O(new_n157_));
  nand3  g106(.a(new_n141_), .b(new_n157_), .c(new_n156_), .O(new_n158_));
  inv1   g107(.a(new_n158_), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n140_), .c(new_n96_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n155_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(x16), .O(new_n162_));
  inv1   g111(.a(x03), .O(new_n163_));
  aoi21  g112(.a(new_n60_), .b(new_n163_), .c(x18), .O(new_n164_));
  aoi21  g113(.a(new_n164_), .b(new_n162_), .c(new_n55_), .O(z12));
  nand2  g114(.a(new_n141_), .b(new_n156_), .O(new_n166_));
  nand2  g115(.a(new_n52_), .b(new_n157_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  aoi22  g117(.a(new_n168_), .b(new_n120_), .c(new_n160_), .d(x32), .O(new_n169_));
  nor2   g118(.a(x16), .b(x02), .O(new_n170_));
  nor2   g119(.a(new_n170_), .b(new_n66_), .O(new_n171_));
  oai21  g120(.a(new_n169_), .b(new_n60_), .c(new_n171_), .O(z13));
  inv1   g121(.a(x01), .O(new_n173_));
  aoi21  g122(.a(new_n60_), .b(new_n173_), .c(x18), .O(new_n174_));
  nand4  g123(.a(new_n148_), .b(new_n141_), .c(new_n52_), .d(new_n157_), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n117_), .O(new_n176_));
  nand2  g125(.a(x33), .b(x16), .O(new_n177_));
  oai22  g126(.a(new_n177_), .b(new_n176_), .c(new_n174_), .d(new_n55_), .O(z14));
  inv1   g127(.a(x00), .O(new_n179_));
  nand2  g128(.a(new_n60_), .b(new_n179_), .O(new_n180_));
  nand2  g129(.a(x34), .b(x16), .O(new_n181_));
  nand4  g130(.a(new_n181_), .b(new_n180_), .c(new_n54_), .d(new_n65_), .O(z15));
endmodule


