// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g009(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g010(.a(x14), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n61_), .O(z01));
  nand2  g013(.a(x18), .b(x07), .O(new_n65_));
  inv1   g014(.a(x21), .O(new_n66_));
  nor2   g015(.a(new_n60_), .b(new_n66_), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x16), .O(new_n70_));
  aoi21  g019(.a(new_n52_), .b(x13), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n70_), .b(new_n67_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n65_), .O(z02));
  inv1   g022(.a(x18), .O(new_n74_));
  nand2  g023(.a(new_n52_), .b(x12), .O(new_n75_));
  inv1   g024(.a(x22), .O(new_n76_));
  nand3  g025(.a(new_n68_), .b(new_n58_), .c(new_n76_), .O(new_n77_));
  nand2  g026(.a(new_n69_), .b(x22), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n77_), .c(x16), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n74_), .O(z03));
  nand2  g030(.a(new_n77_), .b(x23), .O(new_n82_));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n68_), .c(new_n58_), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  aoi21  g034(.a(new_n52_), .b(x11), .c(x18), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n65_), .O(z04));
  nand2  g037(.a(new_n84_), .b(x24), .O(new_n89_));
  nor2   g038(.a(x24), .b(x23), .O(new_n90_));
  nor2   g039(.a(x22), .b(x21), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n90_), .c(new_n58_), .d(new_n57_), .O(new_n92_));
  nand3  g041(.a(new_n92_), .b(new_n89_), .c(x16), .O(new_n93_));
  aoi21  g042(.a(new_n52_), .b(x10), .c(x18), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n65_), .O(z05));
  nand2  g045(.a(new_n92_), .b(x25), .O(new_n97_));
  nor2   g046(.a(x25), .b(x24), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n83_), .c(new_n68_), .d(new_n58_), .O(new_n99_));
  nand3  g048(.a(new_n99_), .b(new_n97_), .c(x16), .O(new_n100_));
  aoi21  g049(.a(new_n52_), .b(x09), .c(x18), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n65_), .O(z06));
  inv1   g052(.a(new_n84_), .O(new_n104_));
  nor3   g053(.a(x26), .b(x25), .c(x24), .O(new_n105_));
  aoi22  g054(.a(new_n105_), .b(new_n104_), .c(new_n99_), .d(x26), .O(new_n106_));
  inv1   g055(.a(x08), .O(new_n107_));
  aoi21  g056(.a(new_n52_), .b(new_n107_), .c(x18), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n52_), .c(new_n108_), .O(z07));
  inv1   g058(.a(x27), .O(new_n110_));
  aoi21  g059(.a(new_n105_), .b(new_n104_), .c(new_n110_), .O(new_n111_));
  nor3   g060(.a(x27), .b(x26), .c(x25), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n91_), .c(new_n90_), .d(new_n60_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  aoi21  g063(.a(new_n52_), .b(x07), .c(x18), .O(new_n115_));
  oai21  g064(.a(new_n114_), .b(new_n111_), .c(new_n115_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n65_), .O(z08));
  nor2   g066(.a(x26), .b(x25), .O(new_n118_));
  nor2   g067(.a(x28), .b(x27), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n118_), .c(new_n90_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n77_), .c(x16), .O(new_n121_));
  aoi21  g070(.a(new_n113_), .b(x28), .c(new_n121_), .O(new_n122_));
  inv1   g071(.a(x06), .O(new_n123_));
  oai21  g072(.a(x16), .b(new_n123_), .c(new_n74_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n122_), .c(new_n65_), .O(z09));
  oai21  g074(.a(new_n120_), .b(new_n77_), .c(x29), .O(new_n126_));
  nor3   g075(.a(x29), .b(x28), .c(x27), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n105_), .c(new_n104_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  inv1   g079(.a(x05), .O(new_n131_));
  aoi21  g080(.a(new_n52_), .b(new_n131_), .c(x18), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n130_), .O(z10));
  xor2a  g082(.a(new_n128_), .b(x30), .O(new_n134_));
  inv1   g083(.a(x04), .O(new_n135_));
  aoi21  g084(.a(new_n52_), .b(new_n135_), .c(x18), .O(new_n136_));
  oai21  g085(.a(new_n134_), .b(new_n52_), .c(new_n136_), .O(z11));
  inv1   g086(.a(x25), .O(new_n138_));
  nor2   g087(.a(x27), .b(x26), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nor2   g089(.a(new_n140_), .b(new_n92_), .O(new_n141_));
  inv1   g090(.a(x30), .O(new_n142_));
  nand4  g091(.a(new_n127_), .b(new_n105_), .c(new_n104_), .d(new_n142_), .O(new_n143_));
  nor2   g092(.a(x29), .b(x28), .O(new_n144_));
  nor2   g093(.a(x31), .b(x30), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  aoi22  g096(.a(new_n147_), .b(new_n141_), .c(new_n143_), .d(x31), .O(new_n148_));
  inv1   g097(.a(x03), .O(new_n149_));
  aoi21  g098(.a(new_n52_), .b(new_n149_), .c(x18), .O(new_n150_));
  oai21  g099(.a(new_n148_), .b(new_n52_), .c(new_n150_), .O(z12));
  inv1   g100(.a(x32), .O(new_n152_));
  aoi21  g101(.a(new_n147_), .b(new_n141_), .c(new_n152_), .O(new_n153_));
  nand3  g102(.a(new_n145_), .b(new_n144_), .c(new_n152_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n113_), .c(x16), .O(new_n155_));
  aoi21  g104(.a(new_n52_), .b(x02), .c(x18), .O(new_n156_));
  oai21  g105(.a(new_n155_), .b(new_n153_), .c(new_n156_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n65_), .O(z13));
  inv1   g107(.a(new_n154_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n141_), .O(new_n160_));
  nor2   g109(.a(x33), .b(x32), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n145_), .c(new_n144_), .d(new_n139_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n99_), .O(new_n163_));
  aoi21  g112(.a(new_n160_), .b(x33), .c(new_n163_), .O(new_n164_));
  inv1   g113(.a(x01), .O(new_n165_));
  aoi21  g114(.a(new_n52_), .b(new_n165_), .c(x18), .O(new_n166_));
  oai21  g115(.a(new_n164_), .b(new_n52_), .c(new_n166_), .O(z14));
  oai21  g116(.a(new_n162_), .b(new_n99_), .c(x34), .O(new_n168_));
  inv1   g117(.a(x34), .O(new_n169_));
  nand2  g118(.a(new_n163_), .b(new_n169_), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n168_), .c(x16), .O(new_n171_));
  aoi21  g120(.a(new_n52_), .b(x00), .c(x18), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n171_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n65_), .O(z15));
endmodule


