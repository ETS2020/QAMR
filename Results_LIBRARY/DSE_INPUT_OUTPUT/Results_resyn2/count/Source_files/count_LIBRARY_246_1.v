// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  nand2  g005(.a(x18), .b(x09), .O(new_n57_));
  inv1   g006(.a(x20), .O(new_n58_));
  nor2   g007(.a(x19), .b(x17), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  oai21  g009(.a(x19), .b(x17), .c(x20), .O(new_n61_));
  nand3  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(x14), .c(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n57_), .O(z01));
  nor2   g014(.a(x21), .b(x20), .O(new_n66_));
  aoi22  g015(.a(new_n66_), .b(new_n59_), .c(new_n60_), .d(x21), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n52_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n52_), .c(new_n69_), .O(z02));
  inv1   g019(.a(x22), .O(new_n71_));
  aoi21  g020(.a(new_n66_), .b(new_n59_), .c(new_n71_), .O(new_n72_));
  nand3  g021(.a(new_n66_), .b(new_n59_), .c(new_n71_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(new_n72_), .c(x16), .O(new_n75_));
  inv1   g024(.a(x12), .O(new_n76_));
  aoi21  g025(.a(new_n52_), .b(new_n76_), .c(x18), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n75_), .O(z03));
  nor2   g027(.a(x23), .b(x22), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n66_), .c(new_n59_), .O(new_n80_));
  inv1   g029(.a(new_n80_), .O(new_n81_));
  aoi21  g030(.a(new_n73_), .b(x23), .c(new_n81_), .O(new_n82_));
  inv1   g031(.a(x11), .O(new_n83_));
  aoi21  g032(.a(new_n52_), .b(new_n83_), .c(x18), .O(new_n84_));
  oai21  g033(.a(new_n82_), .b(new_n52_), .c(new_n84_), .O(z04));
  inv1   g034(.a(x24), .O(new_n86_));
  nor2   g035(.a(new_n81_), .b(new_n86_), .O(new_n87_));
  nand4  g036(.a(new_n79_), .b(new_n66_), .c(new_n59_), .d(new_n86_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  oai21  g038(.a(new_n89_), .b(new_n87_), .c(x16), .O(new_n90_));
  inv1   g039(.a(x10), .O(new_n91_));
  aoi21  g040(.a(new_n52_), .b(new_n91_), .c(x18), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n90_), .O(z05));
  nor2   g042(.a(x25), .b(x24), .O(new_n94_));
  aoi22  g043(.a(new_n94_), .b(new_n81_), .c(new_n88_), .d(x25), .O(new_n95_));
  inv1   g044(.a(x09), .O(new_n96_));
  aoi21  g045(.a(new_n52_), .b(new_n96_), .c(x18), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n52_), .c(new_n97_), .O(z06));
  inv1   g047(.a(x25), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n86_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n80_), .c(x26), .O(new_n101_));
  inv1   g050(.a(x26), .O(new_n102_));
  nand2  g051(.a(new_n94_), .b(new_n102_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n80_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n101_), .c(x16), .O(new_n106_));
  aoi21  g055(.a(new_n52_), .b(x08), .c(x18), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n57_), .O(z07));
  inv1   g058(.a(x27), .O(new_n110_));
  nand2  g059(.a(new_n104_), .b(new_n110_), .O(new_n111_));
  oai21  g060(.a(new_n103_), .b(new_n80_), .c(x27), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n111_), .c(x16), .O(new_n113_));
  aoi21  g062(.a(new_n52_), .b(x07), .c(x18), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n57_), .O(z08));
  inv1   g065(.a(x23), .O(new_n117_));
  nor2   g066(.a(x28), .b(x27), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n94_), .c(new_n102_), .d(new_n117_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n73_), .O(new_n120_));
  aoi21  g069(.a(new_n111_), .b(x28), .c(new_n120_), .O(new_n121_));
  inv1   g070(.a(x06), .O(new_n122_));
  aoi21  g071(.a(new_n52_), .b(new_n122_), .c(x18), .O(new_n123_));
  oai21  g072(.a(new_n121_), .b(new_n52_), .c(new_n123_), .O(z09));
  inv1   g073(.a(new_n103_), .O(new_n125_));
  nor2   g074(.a(x29), .b(x28), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n125_), .c(new_n81_), .d(new_n110_), .O(new_n127_));
  oai21  g076(.a(new_n119_), .b(new_n73_), .c(x29), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(x16), .O(new_n129_));
  aoi21  g078(.a(new_n52_), .b(x05), .c(x18), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n57_), .O(z10));
  nor2   g081(.a(x30), .b(x29), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n118_), .O(new_n134_));
  nor3   g083(.a(new_n134_), .b(new_n103_), .c(new_n80_), .O(new_n135_));
  aoi21  g084(.a(new_n127_), .b(x30), .c(new_n135_), .O(new_n136_));
  inv1   g085(.a(x04), .O(new_n137_));
  aoi21  g086(.a(new_n52_), .b(new_n137_), .c(x18), .O(new_n138_));
  oai21  g087(.a(new_n136_), .b(new_n52_), .c(new_n138_), .O(z11));
  inv1   g088(.a(new_n134_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n104_), .O(new_n141_));
  nor2   g090(.a(x31), .b(x28), .O(new_n142_));
  nor2   g091(.a(x26), .b(x25), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n142_), .c(new_n133_), .d(new_n110_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n88_), .c(x16), .O(new_n145_));
  aoi21  g094(.a(new_n141_), .b(x31), .c(new_n145_), .O(new_n146_));
  inv1   g095(.a(x03), .O(new_n147_));
  inv1   g096(.a(x18), .O(new_n148_));
  oai21  g097(.a(x16), .b(new_n147_), .c(new_n148_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n146_), .c(new_n57_), .O(z12));
  oai21  g099(.a(new_n144_), .b(new_n88_), .c(x32), .O(new_n151_));
  nor2   g100(.a(x32), .b(x31), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n143_), .c(new_n133_), .d(new_n118_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n88_), .c(new_n151_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(x16), .O(new_n155_));
  inv1   g104(.a(x02), .O(new_n156_));
  aoi21  g105(.a(new_n52_), .b(new_n156_), .c(x18), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n155_), .O(z13));
  oai21  g107(.a(new_n153_), .b(new_n88_), .c(x33), .O(new_n159_));
  nor2   g108(.a(x33), .b(x32), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n142_), .c(new_n133_), .O(new_n161_));
  inv1   g110(.a(new_n161_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n104_), .c(new_n110_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n159_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(x16), .O(new_n165_));
  inv1   g114(.a(x01), .O(new_n166_));
  aoi21  g115(.a(new_n52_), .b(new_n166_), .c(x18), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n165_), .O(z14));
  nand4  g117(.a(new_n162_), .b(new_n104_), .c(x34), .d(new_n110_), .O(new_n169_));
  inv1   g118(.a(x34), .O(new_n170_));
  nand2  g119(.a(new_n163_), .b(new_n170_), .O(new_n171_));
  nand3  g120(.a(new_n171_), .b(new_n169_), .c(x16), .O(new_n172_));
  inv1   g121(.a(x00), .O(new_n173_));
  aoi21  g122(.a(new_n52_), .b(new_n173_), .c(x18), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n172_), .O(z15));
endmodule


