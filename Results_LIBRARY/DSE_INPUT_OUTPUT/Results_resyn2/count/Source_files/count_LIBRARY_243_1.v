// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x26), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  oai21  g010(.a(new_n53_), .b(new_n61_), .c(x16), .O(new_n62_));
  aoi21  g011(.a(new_n56_), .b(x14), .c(x26), .O(new_n63_));
  oai21  g012(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n52_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  nor2   g015(.a(new_n60_), .b(new_n66_), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x16), .O(new_n70_));
  aoi21  g019(.a(new_n56_), .b(x13), .c(x26), .O(new_n71_));
  oai21  g020(.a(new_n70_), .b(new_n67_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n52_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  nand3  g023(.a(new_n68_), .b(new_n53_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n69_), .b(x22), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n75_), .c(x16), .O(new_n77_));
  aoi21  g026(.a(new_n56_), .b(x12), .c(x26), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n52_), .O(z03));
  nor2   g029(.a(x23), .b(x22), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n68_), .c(new_n53_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  aoi21  g032(.a(new_n75_), .b(x23), .c(new_n83_), .O(new_n84_));
  inv1   g033(.a(x26), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n52_), .O(new_n86_));
  nor2   g035(.a(x16), .b(x11), .O(new_n87_));
  nor2   g036(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g037(.a(new_n84_), .b(new_n56_), .c(new_n88_), .O(z04));
  nand2  g038(.a(new_n82_), .b(x24), .O(new_n90_));
  nor2   g039(.a(x24), .b(x23), .O(new_n91_));
  nor2   g040(.a(x22), .b(x21), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n91_), .c(new_n53_), .d(new_n61_), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n90_), .c(x16), .O(new_n94_));
  aoi21  g043(.a(new_n56_), .b(x10), .c(x26), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n52_), .O(z05));
  inv1   g046(.a(new_n86_), .O(new_n98_));
  nand2  g047(.a(new_n56_), .b(x09), .O(new_n99_));
  and2   g048(.a(new_n93_), .b(x25), .O(new_n100_));
  inv1   g049(.a(new_n75_), .O(new_n101_));
  inv1   g050(.a(x25), .O(new_n102_));
  nand2  g051(.a(new_n91_), .b(new_n102_), .O(new_n103_));
  inv1   g052(.a(new_n103_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n101_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n100_), .c(new_n99_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n98_), .O(z06));
  nand2  g057(.a(new_n56_), .b(x08), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n98_), .O(z07));
  inv1   g060(.a(x24), .O(new_n112_));
  nor2   g061(.a(x27), .b(x25), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n82_), .O(new_n115_));
  aoi21  g064(.a(new_n105_), .b(x27), .c(new_n115_), .O(new_n116_));
  nor2   g065(.a(x16), .b(x07), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n86_), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n56_), .c(new_n118_), .O(z08));
  nor3   g068(.a(x27), .b(x25), .c(x24), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n81_), .c(new_n68_), .d(new_n53_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x28), .O(new_n122_));
  nor3   g071(.a(x28), .b(x27), .c(x25), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n92_), .c(new_n91_), .d(new_n60_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n122_), .c(x16), .O(new_n125_));
  aoi21  g074(.a(new_n56_), .b(x06), .c(x26), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n52_), .O(z09));
  nor2   g077(.a(new_n103_), .b(new_n75_), .O(new_n129_));
  nor2   g078(.a(x29), .b(x28), .O(new_n130_));
  inv1   g079(.a(new_n130_), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(x27), .O(new_n132_));
  aoi22  g081(.a(new_n132_), .b(new_n129_), .c(new_n124_), .d(x29), .O(new_n133_));
  nor2   g082(.a(x16), .b(x05), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n86_), .O(new_n135_));
  oai21  g084(.a(new_n133_), .b(new_n56_), .c(new_n135_), .O(z10));
  nand2  g085(.a(new_n132_), .b(new_n129_), .O(new_n137_));
  inv1   g086(.a(x30), .O(new_n138_));
  nand2  g087(.a(new_n130_), .b(new_n138_), .O(new_n139_));
  nor3   g088(.a(new_n139_), .b(new_n114_), .c(new_n82_), .O(new_n140_));
  aoi21  g089(.a(new_n137_), .b(x30), .c(new_n140_), .O(new_n141_));
  nor2   g090(.a(x16), .b(x04), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n86_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n56_), .c(new_n143_), .O(z11));
  inv1   g093(.a(x31), .O(new_n145_));
  inv1   g094(.a(new_n139_), .O(new_n146_));
  aoi21  g095(.a(new_n146_), .b(new_n115_), .c(new_n145_), .O(new_n147_));
  nor2   g096(.a(x31), .b(x30), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n130_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n121_), .c(x16), .O(new_n150_));
  aoi21  g099(.a(new_n56_), .b(x03), .c(x26), .O(new_n151_));
  oai21  g100(.a(new_n150_), .b(new_n147_), .c(new_n151_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n52_), .O(z12));
  inv1   g102(.a(x32), .O(new_n154_));
  inv1   g103(.a(new_n149_), .O(new_n155_));
  aoi21  g104(.a(new_n155_), .b(new_n115_), .c(new_n154_), .O(new_n156_));
  inv1   g105(.a(x29), .O(new_n157_));
  nand3  g106(.a(new_n148_), .b(new_n154_), .c(new_n157_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n124_), .c(x16), .O(new_n159_));
  aoi21  g108(.a(new_n56_), .b(x02), .c(x26), .O(new_n160_));
  oai21  g109(.a(new_n159_), .b(new_n156_), .c(new_n160_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n52_), .O(z13));
  nand2  g111(.a(new_n92_), .b(new_n91_), .O(new_n163_));
  inv1   g112(.a(new_n163_), .O(new_n164_));
  inv1   g113(.a(new_n158_), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n123_), .c(new_n164_), .d(new_n60_), .O(new_n166_));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n148_), .c(new_n130_), .d(new_n113_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n93_), .c(x16), .O(new_n169_));
  aoi21  g118(.a(new_n166_), .b(x33), .c(new_n169_), .O(new_n170_));
  inv1   g119(.a(x01), .O(new_n171_));
  oai21  g120(.a(x16), .b(new_n171_), .c(new_n85_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n170_), .c(new_n52_), .O(z14));
  nor2   g122(.a(x28), .b(x27), .O(new_n174_));
  nor2   g123(.a(x34), .b(x33), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  nor2   g125(.a(new_n176_), .b(new_n158_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n129_), .O(new_n178_));
  oai21  g127(.a(new_n168_), .b(new_n93_), .c(x34), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n178_), .c(x16), .O(new_n180_));
  aoi21  g129(.a(new_n56_), .b(x00), .c(x26), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n180_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n52_), .O(z15));
endmodule


