// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x22), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  nor2   g009(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  oai21  g011(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  nor2   g012(.a(x16), .b(x14), .O(new_n64_));
  inv1   g013(.a(x22), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n52_), .O(new_n66_));
  nor2   g015(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n63_), .O(z01));
  nand2  g017(.a(x21), .b(x16), .O(new_n69_));
  nor2   g018(.a(new_n69_), .b(new_n62_), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n53_), .c(x16), .O(new_n72_));
  oai21  g021(.a(x16), .b(x13), .c(new_n72_), .O(new_n73_));
  oai21  g022(.a(new_n73_), .b(new_n70_), .c(new_n65_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n52_), .O(z02));
  oai21  g024(.a(x16), .b(x12), .c(new_n72_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n65_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n52_), .O(z03));
  inv1   g027(.a(x23), .O(new_n79_));
  aoi21  g028(.a(new_n71_), .b(new_n53_), .c(new_n79_), .O(new_n80_));
  nand3  g029(.a(new_n71_), .b(new_n53_), .c(new_n79_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  nor2   g032(.a(x16), .b(x11), .O(new_n84_));
  nor2   g033(.a(new_n84_), .b(new_n66_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n83_), .O(z04));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n71_), .c(new_n53_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  aoi21  g038(.a(new_n81_), .b(x24), .c(new_n89_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x10), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n66_), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n56_), .c(new_n92_), .O(z05));
  nand2  g042(.a(new_n88_), .b(x25), .O(new_n94_));
  nor2   g043(.a(x25), .b(x21), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n87_), .c(new_n53_), .d(new_n60_), .O(new_n96_));
  nand3  g045(.a(new_n96_), .b(new_n94_), .c(x16), .O(new_n97_));
  aoi21  g046(.a(new_n56_), .b(x09), .c(x22), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n52_), .O(z06));
  nor2   g049(.a(x26), .b(x25), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n87_), .c(new_n71_), .d(new_n53_), .O(new_n102_));
  nand2  g051(.a(new_n96_), .b(x26), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n102_), .c(x16), .O(new_n104_));
  aoi21  g053(.a(new_n56_), .b(x08), .c(x22), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n52_), .O(z07));
  nor3   g056(.a(x27), .b(x26), .c(x25), .O(new_n108_));
  aoi22  g057(.a(new_n108_), .b(new_n89_), .c(new_n102_), .d(x27), .O(new_n109_));
  nor2   g058(.a(x16), .b(x07), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n66_), .O(new_n111_));
  oai21  g060(.a(new_n109_), .b(new_n56_), .c(new_n111_), .O(z08));
  inv1   g061(.a(x28), .O(new_n113_));
  aoi21  g062(.a(new_n108_), .b(new_n89_), .c(new_n113_), .O(new_n114_));
  nand2  g063(.a(new_n95_), .b(new_n87_), .O(new_n115_));
  inv1   g064(.a(new_n115_), .O(new_n116_));
  inv1   g065(.a(x26), .O(new_n117_));
  inv1   g066(.a(x27), .O(new_n118_));
  nand3  g067(.a(new_n113_), .b(new_n118_), .c(new_n117_), .O(new_n119_));
  inv1   g068(.a(new_n119_), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n116_), .c(new_n62_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x16), .O(new_n122_));
  aoi21  g071(.a(new_n56_), .b(x06), .c(x22), .O(new_n123_));
  oai21  g072(.a(new_n122_), .b(new_n114_), .c(new_n123_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n52_), .O(z09));
  inv1   g074(.a(x24), .O(new_n126_));
  inv1   g075(.a(x25), .O(new_n127_));
  nor2   g076(.a(x27), .b(x26), .O(new_n128_));
  nor2   g077(.a(x29), .b(x28), .O(new_n129_));
  nand4  g078(.a(new_n129_), .b(new_n128_), .c(new_n127_), .d(new_n126_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n81_), .O(new_n131_));
  aoi21  g080(.a(new_n121_), .b(x29), .c(new_n131_), .O(new_n132_));
  nor2   g081(.a(x16), .b(x05), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n66_), .O(new_n134_));
  oai21  g083(.a(new_n132_), .b(new_n56_), .c(new_n134_), .O(z10));
  oai21  g084(.a(new_n130_), .b(new_n81_), .c(x30), .O(new_n136_));
  inv1   g085(.a(x30), .O(new_n137_));
  nand4  g086(.a(new_n129_), .b(new_n108_), .c(new_n89_), .d(new_n137_), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  aoi21  g088(.a(new_n56_), .b(x04), .c(x22), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n52_), .O(z11));
  nand4  g091(.a(new_n129_), .b(new_n128_), .c(new_n137_), .d(new_n127_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n88_), .c(x31), .O(new_n144_));
  nor2   g093(.a(x31), .b(x30), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n129_), .O(new_n146_));
  inv1   g095(.a(new_n146_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n108_), .c(new_n89_), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n144_), .c(x16), .O(new_n149_));
  aoi21  g098(.a(new_n56_), .b(x03), .c(x22), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n52_), .O(z12));
  nor2   g101(.a(new_n119_), .b(new_n96_), .O(new_n153_));
  nor2   g102(.a(x32), .b(x29), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n145_), .O(new_n155_));
  inv1   g104(.a(new_n155_), .O(new_n156_));
  aoi22  g105(.a(new_n156_), .b(new_n153_), .c(new_n148_), .d(x32), .O(new_n157_));
  nor2   g106(.a(x16), .b(x02), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n66_), .O(new_n159_));
  oai21  g108(.a(new_n157_), .b(new_n56_), .c(new_n159_), .O(z13));
  nand4  g109(.a(new_n156_), .b(new_n120_), .c(new_n116_), .d(new_n62_), .O(new_n161_));
  nor2   g110(.a(x33), .b(x32), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n145_), .c(new_n129_), .d(new_n128_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n96_), .c(x16), .O(new_n164_));
  aoi21  g113(.a(new_n161_), .b(x33), .c(new_n164_), .O(new_n165_));
  inv1   g114(.a(x01), .O(new_n166_));
  oai21  g115(.a(x16), .b(new_n166_), .c(new_n65_), .O(new_n167_));
  oai21  g116(.a(new_n167_), .b(new_n165_), .c(new_n52_), .O(z14));
  nor2   g117(.a(x34), .b(x33), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n113_), .c(new_n118_), .O(new_n170_));
  nor3   g119(.a(new_n170_), .b(new_n155_), .c(new_n102_), .O(new_n171_));
  oai21  g120(.a(new_n163_), .b(new_n96_), .c(x34), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x16), .O(new_n173_));
  aoi21  g122(.a(new_n56_), .b(x00), .c(x22), .O(new_n174_));
  oai21  g123(.a(new_n173_), .b(new_n171_), .c(new_n174_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n52_), .O(z15));
endmodule


