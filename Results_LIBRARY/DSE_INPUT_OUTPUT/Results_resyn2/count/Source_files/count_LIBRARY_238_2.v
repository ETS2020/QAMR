// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x28), .O(new_n58_));
  nor2   g007(.a(x31), .b(new_n58_), .O(new_n59_));
  inv1   g008(.a(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  aoi21  g010(.a(new_n56_), .b(new_n55_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n54_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(new_n53_), .b(new_n64_), .O(new_n65_));
  nor3   g014(.a(x20), .b(x19), .c(x17), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g016(.a(x16), .b(x14), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(new_n66_), .b(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n66_), .b(new_n71_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(new_n72_), .c(x16), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n56_), .b(new_n76_), .c(x18), .O(new_n77_));
  aoi21  g026(.a(new_n77_), .b(new_n75_), .c(new_n59_), .O(z02));
  nand2  g027(.a(new_n73_), .b(x22), .O(new_n79_));
  nor2   g028(.a(x22), .b(x21), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n53_), .c(new_n64_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(x16), .O(new_n83_));
  inv1   g032(.a(x12), .O(new_n84_));
  aoi21  g033(.a(new_n56_), .b(new_n84_), .c(x18), .O(new_n85_));
  aoi21  g034(.a(new_n85_), .b(new_n83_), .c(new_n59_), .O(z03));
  nand2  g035(.a(new_n81_), .b(x23), .O(new_n87_));
  inv1   g036(.a(x23), .O(new_n88_));
  nand4  g037(.a(new_n80_), .b(new_n53_), .c(new_n88_), .d(new_n64_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x16), .O(new_n91_));
  inv1   g040(.a(x11), .O(new_n92_));
  aoi21  g041(.a(new_n56_), .b(new_n92_), .c(x18), .O(new_n93_));
  aoi21  g042(.a(new_n93_), .b(new_n91_), .c(new_n59_), .O(z04));
  inv1   g043(.a(x24), .O(new_n95_));
  nor3   g044(.a(x23), .b(x22), .c(x21), .O(new_n96_));
  aoi21  g045(.a(new_n96_), .b(new_n66_), .c(new_n95_), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n80_), .c(new_n53_), .d(new_n64_), .O(new_n99_));
  inv1   g048(.a(new_n99_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n97_), .c(x16), .O(new_n101_));
  inv1   g050(.a(x10), .O(new_n102_));
  aoi21  g051(.a(new_n56_), .b(new_n102_), .c(x18), .O(new_n103_));
  aoi21  g052(.a(new_n103_), .b(new_n101_), .c(new_n59_), .O(z05));
  inv1   g053(.a(x25), .O(new_n105_));
  nand2  g054(.a(new_n98_), .b(new_n105_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n81_), .O(new_n107_));
  aoi21  g056(.a(new_n99_), .b(x25), .c(new_n107_), .O(new_n108_));
  nor2   g057(.a(x16), .b(x09), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n61_), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n56_), .c(new_n110_), .O(z06));
  inv1   g060(.a(x26), .O(new_n112_));
  nor2   g061(.a(new_n107_), .b(new_n112_), .O(new_n113_));
  nor2   g062(.a(x26), .b(x25), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n95_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n89_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n113_), .c(x16), .O(new_n117_));
  inv1   g066(.a(x08), .O(new_n118_));
  aoi21  g067(.a(new_n56_), .b(new_n118_), .c(x18), .O(new_n119_));
  aoi21  g068(.a(new_n119_), .b(new_n117_), .c(new_n59_), .O(z07));
  inv1   g069(.a(x27), .O(new_n121_));
  nor2   g070(.a(new_n116_), .b(new_n121_), .O(new_n122_));
  nor2   g071(.a(x27), .b(x26), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n98_), .c(new_n105_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n81_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n122_), .c(x16), .O(new_n126_));
  inv1   g075(.a(x07), .O(new_n127_));
  aoi21  g076(.a(new_n56_), .b(new_n127_), .c(x18), .O(new_n128_));
  aoi21  g077(.a(new_n128_), .b(new_n126_), .c(new_n59_), .O(z08));
  xor2a  g078(.a(new_n125_), .b(new_n58_), .O(new_n130_));
  nor2   g079(.a(x16), .b(x06), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n61_), .O(new_n132_));
  oai21  g081(.a(new_n130_), .b(new_n56_), .c(new_n132_), .O(z09));
  oai21  g082(.a(new_n124_), .b(new_n81_), .c(x29), .O(new_n134_));
  nor3   g083(.a(x25), .b(x24), .c(x23), .O(new_n135_));
  nor3   g084(.a(x29), .b(x27), .c(x26), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n135_), .c(new_n80_), .d(new_n66_), .O(new_n137_));
  aoi21  g086(.a(new_n137_), .b(new_n134_), .c(x28), .O(new_n138_));
  inv1   g087(.a(x29), .O(new_n139_));
  nor2   g088(.a(new_n139_), .b(new_n58_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(x31), .O(new_n141_));
  inv1   g090(.a(new_n141_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n138_), .c(x16), .O(new_n143_));
  oai21  g092(.a(new_n140_), .b(new_n57_), .c(x16), .O(new_n144_));
  aoi21  g093(.a(new_n57_), .b(x05), .c(new_n59_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n143_), .O(z10));
  nor2   g096(.a(x29), .b(x27), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n98_), .c(new_n112_), .d(new_n105_), .O(new_n149_));
  oai21  g098(.a(new_n149_), .b(new_n81_), .c(x30), .O(new_n150_));
  nor3   g099(.a(x26), .b(x25), .c(x24), .O(new_n151_));
  nor3   g100(.a(x30), .b(x29), .c(x27), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n151_), .c(new_n96_), .d(new_n66_), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n150_), .c(x28), .O(new_n154_));
  inv1   g103(.a(x30), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n58_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(x31), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n154_), .c(x16), .O(new_n159_));
  oai21  g108(.a(new_n156_), .b(new_n57_), .c(x16), .O(new_n160_));
  aoi21  g109(.a(new_n57_), .b(x04), .c(new_n59_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n159_), .O(z11));
  oai21  g112(.a(x16), .b(x03), .c(new_n57_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n60_), .O(new_n165_));
  oai21  g114(.a(new_n153_), .b(x28), .c(x31), .O(new_n166_));
  nor3   g115(.a(x31), .b(x30), .c(x29), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n125_), .c(new_n58_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(x16), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n165_), .O(z12));
  inv1   g120(.a(x32), .O(new_n172_));
  nand2  g121(.a(new_n167_), .b(new_n172_), .O(new_n173_));
  inv1   g122(.a(new_n173_), .O(new_n174_));
  inv1   g123(.a(x31), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n155_), .c(new_n139_), .d(new_n121_), .O(new_n176_));
  inv1   g125(.a(new_n176_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n116_), .O(new_n178_));
  aoi22  g127(.a(new_n178_), .b(x32), .c(new_n174_), .d(new_n125_), .O(new_n179_));
  nand2  g128(.a(x32), .b(x16), .O(new_n180_));
  aoi21  g129(.a(new_n180_), .b(x31), .c(new_n58_), .O(new_n181_));
  oai21  g130(.a(x16), .b(x02), .c(new_n57_), .O(new_n182_));
  nor2   g131(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  oai21  g132(.a(new_n179_), .b(new_n56_), .c(new_n183_), .O(z13));
  oai21  g133(.a(x16), .b(x01), .c(new_n57_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n60_), .O(new_n186_));
  nor2   g135(.a(x30), .b(x29), .O(new_n187_));
  nor2   g136(.a(x32), .b(x27), .O(new_n188_));
  nand4  g137(.a(new_n188_), .b(new_n187_), .c(new_n114_), .d(new_n95_), .O(new_n189_));
  oai21  g138(.a(new_n189_), .b(new_n89_), .c(x33), .O(new_n190_));
  inv1   g139(.a(x33), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n172_), .c(new_n112_), .d(new_n105_), .O(new_n192_));
  nor2   g141(.a(new_n192_), .b(new_n176_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n100_), .O(new_n194_));
  aoi21  g143(.a(new_n194_), .b(new_n190_), .c(x28), .O(new_n195_));
  nor2   g144(.a(new_n191_), .b(new_n175_), .O(new_n196_));
  oai21  g145(.a(new_n196_), .b(new_n195_), .c(x16), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n186_), .O(z14));
  inv1   g147(.a(x34), .O(new_n199_));
  aoi21  g148(.a(new_n193_), .b(new_n100_), .c(new_n199_), .O(new_n200_));
  nor2   g149(.a(x34), .b(x33), .O(new_n201_));
  nand4  g150(.a(new_n201_), .b(new_n167_), .c(new_n123_), .d(new_n172_), .O(new_n202_));
  nor3   g151(.a(new_n202_), .b(new_n106_), .c(new_n81_), .O(new_n203_));
  oai21  g152(.a(new_n203_), .b(new_n200_), .c(x16), .O(new_n204_));
  nand2  g153(.a(x34), .b(x16), .O(new_n205_));
  aoi21  g154(.a(new_n205_), .b(x31), .c(new_n58_), .O(new_n206_));
  oai21  g155(.a(x16), .b(x00), .c(new_n57_), .O(new_n207_));
  nor2   g156(.a(new_n207_), .b(new_n206_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n204_), .O(z15));
endmodule


