// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  xor2a  g010(.a(new_n55_), .b(new_n61_), .O(new_n62_));
  inv1   g011(.a(x14), .O(new_n63_));
  aoi21  g012(.a(new_n58_), .b(new_n63_), .c(x18), .O(new_n64_));
  oai21  g013(.a(new_n62_), .b(new_n58_), .c(new_n64_), .O(z01));
  nor2   g014(.a(x18), .b(x16), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  inv1   g016(.a(x21), .O(new_n68_));
  aoi21  g017(.a(new_n55_), .b(new_n61_), .c(new_n68_), .O(new_n69_));
  inv1   g018(.a(x18), .O(new_n70_));
  nand4  g019(.a(new_n68_), .b(new_n61_), .c(new_n53_), .d(new_n52_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g021(.a(new_n72_), .b(new_n69_), .c(x16), .O(new_n73_));
  oai21  g022(.a(new_n67_), .b(x13), .c(new_n73_), .O(z02));
  xor2a  g023(.a(new_n71_), .b(x22), .O(new_n75_));
  inv1   g024(.a(x12), .O(new_n76_));
  aoi21  g025(.a(new_n58_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n58_), .c(new_n77_), .O(z03));
  oai21  g027(.a(new_n71_), .b(x22), .c(x23), .O(new_n79_));
  inv1   g028(.a(new_n71_), .O(new_n80_));
  nor2   g029(.a(x23), .b(x22), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n79_), .c(new_n70_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(x16), .O(new_n84_));
  oai21  g033(.a(new_n67_), .b(x11), .c(new_n84_), .O(z04));
  inv1   g034(.a(x24), .O(new_n86_));
  nor2   g035(.a(x21), .b(x20), .O(new_n87_));
  nand4  g036(.a(new_n81_), .b(new_n87_), .c(new_n55_), .d(new_n86_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n70_), .O(new_n89_));
  aoi21  g038(.a(new_n82_), .b(x24), .c(new_n89_), .O(new_n90_));
  oai22  g039(.a(new_n90_), .b(new_n58_), .c(new_n67_), .d(x10), .O(z05));
  nor2   g040(.a(x25), .b(x24), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n81_), .c(new_n87_), .d(new_n55_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n70_), .O(new_n94_));
  aoi21  g043(.a(new_n88_), .b(x25), .c(new_n94_), .O(new_n95_));
  oai22  g044(.a(new_n95_), .b(new_n58_), .c(new_n67_), .d(x09), .O(z06));
  nand2  g045(.a(new_n58_), .b(x08), .O(new_n97_));
  nor2   g046(.a(new_n93_), .b(x26), .O(new_n98_));
  inv1   g047(.a(x26), .O(new_n99_));
  inv1   g048(.a(new_n93_), .O(new_n100_));
  oai21  g049(.a(new_n100_), .b(new_n99_), .c(x16), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n98_), .c(new_n97_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n70_), .O(z07));
  inv1   g052(.a(x07), .O(new_n104_));
  nand2  g053(.a(new_n66_), .b(new_n104_), .O(new_n105_));
  inv1   g054(.a(new_n88_), .O(new_n106_));
  nor3   g055(.a(x27), .b(x26), .c(x25), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  oai21  g057(.a(new_n93_), .b(x26), .c(x27), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n108_), .c(new_n70_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x16), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n105_), .O(z08));
  inv1   g061(.a(x28), .O(new_n113_));
  nand2  g062(.a(new_n107_), .b(new_n113_), .O(new_n114_));
  nor2   g063(.a(new_n114_), .b(new_n88_), .O(new_n115_));
  aoi21  g064(.a(new_n108_), .b(x28), .c(new_n115_), .O(new_n116_));
  inv1   g065(.a(x06), .O(new_n117_));
  aoi21  g066(.a(new_n58_), .b(new_n117_), .c(x18), .O(new_n118_));
  oai21  g067(.a(new_n116_), .b(new_n58_), .c(new_n118_), .O(z09));
  oai21  g068(.a(new_n114_), .b(new_n88_), .c(x29), .O(new_n120_));
  inv1   g069(.a(x22), .O(new_n121_));
  inv1   g070(.a(x23), .O(new_n122_));
  inv1   g071(.a(x25), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n86_), .c(new_n122_), .d(new_n121_), .O(new_n124_));
  inv1   g073(.a(new_n124_), .O(new_n125_));
  nor3   g074(.a(x29), .b(x28), .c(x27), .O(new_n126_));
  nand4  g075(.a(new_n126_), .b(new_n125_), .c(new_n80_), .d(new_n99_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n120_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(x16), .O(new_n129_));
  inv1   g078(.a(x05), .O(new_n130_));
  aoi21  g079(.a(new_n58_), .b(new_n130_), .c(x18), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n129_), .O(z10));
  inv1   g081(.a(x27), .O(new_n133_));
  inv1   g082(.a(x30), .O(new_n134_));
  nor2   g083(.a(x29), .b(x28), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n134_), .c(new_n133_), .O(new_n136_));
  inv1   g085(.a(new_n136_), .O(new_n137_));
  aoi22  g086(.a(new_n137_), .b(new_n98_), .c(new_n127_), .d(x30), .O(new_n138_));
  inv1   g087(.a(x04), .O(new_n139_));
  aoi21  g088(.a(new_n58_), .b(new_n139_), .c(x18), .O(new_n140_));
  oai21  g089(.a(new_n138_), .b(new_n58_), .c(new_n140_), .O(z11));
  nand3  g090(.a(new_n137_), .b(new_n100_), .c(new_n99_), .O(new_n142_));
  nor2   g091(.a(x31), .b(x30), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n135_), .c(new_n107_), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n88_), .O(new_n145_));
  aoi21  g094(.a(new_n142_), .b(x31), .c(new_n145_), .O(new_n146_));
  inv1   g095(.a(x03), .O(new_n147_));
  aoi21  g096(.a(new_n58_), .b(new_n147_), .c(x18), .O(new_n148_));
  oai21  g097(.a(new_n146_), .b(new_n58_), .c(new_n148_), .O(z12));
  oai21  g098(.a(new_n144_), .b(new_n88_), .c(x32), .O(new_n150_));
  nor2   g099(.a(x32), .b(x31), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n99_), .c(new_n123_), .O(new_n152_));
  inv1   g101(.a(new_n152_), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n137_), .c(new_n106_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n150_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(x16), .O(new_n156_));
  inv1   g105(.a(x02), .O(new_n157_));
  aoi21  g106(.a(new_n58_), .b(new_n157_), .c(x18), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n156_), .O(z13));
  nor2   g108(.a(x27), .b(x26), .O(new_n160_));
  nor2   g109(.a(x33), .b(x32), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n143_), .c(new_n135_), .d(new_n160_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n93_), .O(new_n163_));
  aoi21  g112(.a(new_n154_), .b(x33), .c(new_n163_), .O(new_n164_));
  inv1   g113(.a(x01), .O(new_n165_));
  aoi21  g114(.a(new_n58_), .b(new_n165_), .c(x18), .O(new_n166_));
  oai21  g115(.a(new_n164_), .b(new_n58_), .c(new_n166_), .O(z14));
  inv1   g116(.a(x00), .O(new_n168_));
  nand2  g117(.a(new_n66_), .b(new_n168_), .O(new_n169_));
  nor3   g118(.a(new_n162_), .b(new_n93_), .c(x34), .O(new_n170_));
  oai21  g119(.a(new_n162_), .b(new_n93_), .c(x34), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n70_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n170_), .c(x16), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n169_), .O(z15));
endmodule


