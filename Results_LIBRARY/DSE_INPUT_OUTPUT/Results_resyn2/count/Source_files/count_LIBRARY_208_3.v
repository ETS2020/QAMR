// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x04), .O(new_n57_));
  nand2  g006(.a(x18), .b(new_n57_), .O(new_n58_));
  nor3   g007(.a(x20), .b(x19), .c(x17), .O(new_n59_));
  inv1   g008(.a(x20), .O(new_n60_));
  nor2   g009(.a(x19), .b(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(x14), .c(x18), .O(new_n63_));
  oai21  g012(.a(new_n62_), .b(new_n59_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n58_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  xor2a  g015(.a(new_n59_), .b(new_n66_), .O(new_n67_));
  inv1   g016(.a(x13), .O(new_n68_));
  aoi21  g017(.a(new_n52_), .b(new_n68_), .c(x18), .O(new_n69_));
  oai21  g018(.a(new_n67_), .b(new_n52_), .c(new_n69_), .O(z02));
  nand2  g019(.a(new_n59_), .b(new_n66_), .O(new_n71_));
  nor2   g020(.a(x22), .b(x21), .O(new_n72_));
  nand3  g021(.a(new_n72_), .b(new_n61_), .c(new_n60_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(x16), .O(new_n74_));
  aoi21  g023(.a(new_n71_), .b(x22), .c(new_n74_), .O(new_n75_));
  inv1   g024(.a(x12), .O(new_n76_));
  inv1   g025(.a(x18), .O(new_n77_));
  oai21  g026(.a(x16), .b(new_n76_), .c(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n75_), .c(new_n58_), .O(z03));
  inv1   g028(.a(x23), .O(new_n80_));
  nand4  g029(.a(new_n72_), .b(new_n61_), .c(new_n80_), .d(new_n60_), .O(new_n81_));
  nand2  g030(.a(new_n73_), .b(x23), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n81_), .c(x16), .O(new_n83_));
  aoi21  g032(.a(new_n52_), .b(x11), .c(x18), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n58_), .O(z04));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n72_), .c(new_n61_), .d(new_n60_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  aoi21  g038(.a(new_n81_), .b(x24), .c(new_n89_), .O(new_n90_));
  inv1   g039(.a(x10), .O(new_n91_));
  aoi21  g040(.a(new_n52_), .b(new_n91_), .c(x18), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n52_), .c(new_n92_), .O(z05));
  inv1   g042(.a(x25), .O(new_n94_));
  nand2  g043(.a(new_n87_), .b(new_n94_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n73_), .O(new_n96_));
  aoi21  g045(.a(new_n88_), .b(x25), .c(new_n96_), .O(new_n97_));
  inv1   g046(.a(x09), .O(new_n98_));
  aoi21  g047(.a(new_n52_), .b(new_n98_), .c(x18), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n52_), .c(new_n99_), .O(z06));
  nor3   g049(.a(x25), .b(x24), .c(x23), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n72_), .c(new_n59_), .O(new_n102_));
  nor2   g051(.a(x26), .b(x25), .O(new_n103_));
  aoi22  g052(.a(new_n103_), .b(new_n89_), .c(new_n102_), .d(x26), .O(new_n104_));
  inv1   g053(.a(x08), .O(new_n105_));
  aoi21  g054(.a(new_n52_), .b(new_n105_), .c(x18), .O(new_n106_));
  oai21  g055(.a(new_n104_), .b(new_n52_), .c(new_n106_), .O(z07));
  inv1   g056(.a(new_n103_), .O(new_n108_));
  oai21  g057(.a(new_n108_), .b(new_n88_), .c(x27), .O(new_n109_));
  nor2   g058(.a(x27), .b(x26), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n96_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(x16), .O(new_n113_));
  inv1   g062(.a(x07), .O(new_n114_));
  aoi21  g063(.a(new_n52_), .b(new_n114_), .c(x18), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n113_), .O(z08));
  inv1   g065(.a(x28), .O(new_n117_));
  nand3  g066(.a(new_n110_), .b(new_n117_), .c(new_n94_), .O(new_n118_));
  oai21  g067(.a(new_n118_), .b(new_n88_), .c(x16), .O(new_n119_));
  aoi21  g068(.a(new_n111_), .b(x28), .c(new_n119_), .O(new_n120_));
  inv1   g069(.a(x06), .O(new_n121_));
  oai21  g070(.a(x16), .b(new_n121_), .c(new_n77_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n120_), .c(new_n58_), .O(z09));
  inv1   g072(.a(x29), .O(new_n124_));
  inv1   g073(.a(new_n118_), .O(new_n125_));
  aoi21  g074(.a(new_n125_), .b(new_n89_), .c(new_n124_), .O(new_n126_));
  inv1   g075(.a(x24), .O(new_n127_));
  inv1   g076(.a(x27), .O(new_n128_));
  nor2   g077(.a(x29), .b(x28), .O(new_n129_));
  nand4  g078(.a(new_n129_), .b(new_n103_), .c(new_n128_), .d(new_n127_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n81_), .O(new_n131_));
  oai21  g080(.a(new_n131_), .b(new_n126_), .c(x16), .O(new_n132_));
  inv1   g081(.a(x05), .O(new_n133_));
  aoi21  g082(.a(new_n52_), .b(new_n133_), .c(x18), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(z10));
  inv1   g084(.a(x30), .O(new_n136_));
  xor2a  g085(.a(new_n131_), .b(new_n136_), .O(new_n137_));
  aoi21  g086(.a(new_n52_), .b(new_n57_), .c(x18), .O(new_n138_));
  oai21  g087(.a(new_n137_), .b(new_n52_), .c(new_n138_), .O(z11));
  inv1   g088(.a(new_n81_), .O(new_n140_));
  inv1   g089(.a(new_n130_), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n140_), .c(new_n136_), .O(new_n142_));
  nor2   g091(.a(x31), .b(x30), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n129_), .c(new_n110_), .d(new_n94_), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n88_), .O(new_n145_));
  aoi21  g094(.a(new_n142_), .b(x31), .c(new_n145_), .O(new_n146_));
  inv1   g095(.a(x03), .O(new_n147_));
  aoi21  g096(.a(new_n52_), .b(new_n147_), .c(x18), .O(new_n148_));
  oai21  g097(.a(new_n146_), .b(new_n52_), .c(new_n148_), .O(z12));
  inv1   g098(.a(x32), .O(new_n150_));
  inv1   g099(.a(new_n144_), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n89_), .c(new_n150_), .O(new_n152_));
  oai21  g101(.a(new_n144_), .b(new_n88_), .c(x32), .O(new_n153_));
  nand3  g102(.a(new_n153_), .b(new_n152_), .c(x16), .O(new_n154_));
  aoi21  g103(.a(new_n52_), .b(x02), .c(x18), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n58_), .O(z13));
  nor2   g106(.a(x33), .b(x32), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n143_), .c(new_n129_), .d(new_n110_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n102_), .O(new_n160_));
  aoi21  g109(.a(new_n152_), .b(x33), .c(new_n160_), .O(new_n161_));
  inv1   g110(.a(x01), .O(new_n162_));
  aoi21  g111(.a(new_n52_), .b(new_n162_), .c(x18), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n52_), .c(new_n163_), .O(z14));
  oai21  g113(.a(new_n159_), .b(new_n102_), .c(x34), .O(new_n165_));
  inv1   g114(.a(x34), .O(new_n166_));
  inv1   g115(.a(new_n159_), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n96_), .c(new_n166_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(x16), .O(new_n170_));
  inv1   g119(.a(x00), .O(new_n171_));
  aoi21  g120(.a(new_n52_), .b(new_n171_), .c(x18), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n170_), .O(z15));
endmodule


