// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x12), .O(new_n58_));
  aoi21  g007(.a(new_n56_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n54_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  nor2   g013(.a(x16), .b(x14), .O(new_n65_));
  nor2   g014(.a(new_n65_), .b(new_n58_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n64_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(new_n63_), .b(new_n68_), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(x16), .O(new_n72_));
  inv1   g021(.a(x12), .O(new_n73_));
  aoi21  g022(.a(new_n56_), .b(x13), .c(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n69_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n57_), .O(z02));
  inv1   g025(.a(new_n58_), .O(new_n77_));
  xor2a  g026(.a(new_n71_), .b(x22), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n56_), .c(new_n77_), .O(z03));
  inv1   g028(.a(x22), .O(new_n80_));
  nand3  g029(.a(new_n70_), .b(new_n53_), .c(new_n80_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x23), .O(new_n82_));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n70_), .c(new_n53_), .O(new_n84_));
  aoi21  g033(.a(new_n84_), .b(new_n82_), .c(new_n56_), .O(new_n85_));
  oai21  g034(.a(x16), .b(x11), .c(new_n77_), .O(new_n86_));
  or2    g035(.a(new_n86_), .b(new_n85_), .O(z04));
  nand2  g036(.a(new_n84_), .b(x24), .O(new_n88_));
  nor2   g037(.a(x24), .b(x21), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n83_), .c(new_n53_), .d(new_n61_), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n88_), .c(x16), .O(new_n91_));
  aoi21  g040(.a(new_n56_), .b(x10), .c(new_n73_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n57_), .O(z05));
  nor2   g043(.a(x25), .b(x24), .O(new_n95_));
  nand4  g044(.a(new_n95_), .b(new_n83_), .c(new_n70_), .d(new_n53_), .O(new_n96_));
  inv1   g045(.a(new_n96_), .O(new_n97_));
  aoi21  g046(.a(new_n90_), .b(x25), .c(new_n97_), .O(new_n98_));
  nor2   g047(.a(x16), .b(x09), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n58_), .O(new_n100_));
  oai21  g049(.a(new_n98_), .b(new_n56_), .c(new_n100_), .O(z06));
  xor2a  g050(.a(new_n96_), .b(x26), .O(new_n102_));
  nor2   g051(.a(x16), .b(x08), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n58_), .O(new_n104_));
  oai21  g053(.a(new_n102_), .b(new_n56_), .c(new_n104_), .O(z07));
  oai21  g054(.a(new_n96_), .b(x26), .c(x27), .O(new_n106_));
  nor3   g055(.a(x27), .b(x26), .c(x25), .O(new_n107_));
  nand4  g056(.a(new_n107_), .b(new_n89_), .c(new_n83_), .d(new_n63_), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n106_), .c(x16), .O(new_n109_));
  aoi21  g058(.a(new_n56_), .b(x07), .c(new_n73_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n57_), .O(z08));
  inv1   g061(.a(x23), .O(new_n113_));
  inv1   g062(.a(x26), .O(new_n114_));
  nor2   g063(.a(x28), .b(x27), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n95_), .c(new_n114_), .d(new_n113_), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n81_), .O(new_n117_));
  aoi21  g066(.a(new_n108_), .b(x28), .c(new_n117_), .O(new_n118_));
  nor2   g067(.a(x16), .b(x06), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n58_), .O(new_n120_));
  oai21  g069(.a(new_n118_), .b(new_n56_), .c(new_n120_), .O(z09));
  nand2  g070(.a(new_n95_), .b(new_n114_), .O(new_n122_));
  nor2   g071(.a(new_n122_), .b(new_n84_), .O(new_n123_));
  nor3   g072(.a(x29), .b(x28), .c(x27), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  oai21  g074(.a(new_n116_), .b(new_n81_), .c(x29), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n125_), .c(x16), .O(new_n127_));
  aoi21  g076(.a(new_n56_), .b(x05), .c(new_n73_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n57_), .O(z10));
  inv1   g079(.a(x30), .O(new_n131_));
  aoi21  g080(.a(new_n124_), .b(new_n123_), .c(new_n131_), .O(new_n132_));
  nor3   g081(.a(x26), .b(x25), .c(x24), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n83_), .c(new_n70_), .d(new_n53_), .O(new_n134_));
  nor2   g083(.a(x30), .b(x29), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n115_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n134_), .c(x16), .O(new_n137_));
  aoi21  g086(.a(new_n56_), .b(x04), .c(new_n73_), .O(new_n138_));
  oai21  g087(.a(new_n137_), .b(new_n132_), .c(new_n138_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n57_), .O(z11));
  inv1   g089(.a(x31), .O(new_n141_));
  inv1   g090(.a(new_n136_), .O(new_n142_));
  aoi21  g091(.a(new_n142_), .b(new_n123_), .c(new_n141_), .O(new_n143_));
  inv1   g092(.a(x28), .O(new_n144_));
  nand3  g093(.a(new_n135_), .b(new_n141_), .c(new_n144_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n108_), .c(x16), .O(new_n146_));
  aoi21  g095(.a(new_n56_), .b(x03), .c(new_n73_), .O(new_n147_));
  oai21  g096(.a(new_n146_), .b(new_n143_), .c(new_n147_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n57_), .O(z12));
  nand2  g098(.a(new_n89_), .b(new_n83_), .O(new_n150_));
  inv1   g099(.a(new_n150_), .O(new_n151_));
  inv1   g100(.a(new_n145_), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n107_), .c(new_n151_), .d(new_n63_), .O(new_n153_));
  nor2   g102(.a(x26), .b(x25), .O(new_n154_));
  nor2   g103(.a(x32), .b(x31), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n154_), .c(new_n135_), .d(new_n115_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n90_), .c(x16), .O(new_n157_));
  aoi21  g106(.a(new_n153_), .b(x32), .c(new_n157_), .O(new_n158_));
  inv1   g107(.a(x02), .O(new_n159_));
  oai21  g108(.a(x16), .b(new_n159_), .c(x12), .O(new_n160_));
  oai21  g109(.a(new_n160_), .b(new_n158_), .c(new_n57_), .O(z13));
  nor2   g110(.a(x27), .b(x26), .O(new_n162_));
  nor2   g111(.a(x33), .b(x32), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n145_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n97_), .O(new_n166_));
  oai21  g115(.a(new_n156_), .b(new_n90_), .c(x33), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n166_), .c(x16), .O(new_n168_));
  aoi21  g117(.a(new_n56_), .b(x01), .c(new_n73_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n57_), .O(z14));
  nand3  g120(.a(new_n165_), .b(new_n97_), .c(x34), .O(new_n172_));
  inv1   g121(.a(x34), .O(new_n173_));
  nand2  g122(.a(new_n166_), .b(new_n173_), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n172_), .c(x16), .O(new_n175_));
  nor2   g124(.a(x16), .b(x00), .O(new_n176_));
  nor2   g125(.a(new_n176_), .b(new_n58_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n175_), .O(z15));
endmodule


