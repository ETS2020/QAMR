// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x04), .O(new_n58_));
  aoi21  g007(.a(new_n56_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n54_), .O(z00));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  inv1   g010(.a(x20), .O(new_n62_));
  oai21  g011(.a(new_n53_), .b(new_n62_), .c(x16), .O(new_n63_));
  inv1   g012(.a(x04), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(new_n64_), .O(new_n65_));
  oai21  g014(.a(new_n63_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n57_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  xor2a  g017(.a(new_n61_), .b(new_n68_), .O(new_n69_));
  nor2   g018(.a(x16), .b(x13), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n56_), .c(new_n71_), .O(z02));
  nand2  g021(.a(new_n61_), .b(new_n68_), .O(new_n73_));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  nand3  g023(.a(new_n74_), .b(new_n53_), .c(new_n62_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(x16), .O(new_n76_));
  aoi21  g025(.a(new_n73_), .b(x22), .c(new_n76_), .O(new_n77_));
  inv1   g026(.a(x12), .O(new_n78_));
  oai21  g027(.a(x16), .b(new_n78_), .c(x04), .O(new_n79_));
  oai21  g028(.a(new_n79_), .b(new_n77_), .c(new_n57_), .O(z03));
  or2    g029(.a(new_n75_), .b(x23), .O(new_n81_));
  aoi21  g030(.a(new_n75_), .b(x23), .c(new_n56_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g032(.a(new_n56_), .b(x11), .c(new_n64_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n57_), .O(z04));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n74_), .c(new_n53_), .d(new_n62_), .O(new_n88_));
  inv1   g037(.a(new_n88_), .O(new_n89_));
  aoi21  g038(.a(new_n81_), .b(x24), .c(new_n89_), .O(new_n90_));
  nor2   g039(.a(x16), .b(x10), .O(new_n91_));
  nor2   g040(.a(new_n91_), .b(new_n58_), .O(new_n92_));
  oai21  g041(.a(new_n90_), .b(new_n56_), .c(new_n92_), .O(z05));
  inv1   g042(.a(x25), .O(new_n94_));
  nand2  g043(.a(new_n87_), .b(new_n94_), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n75_), .O(new_n96_));
  aoi21  g045(.a(new_n88_), .b(x25), .c(new_n96_), .O(new_n97_));
  nor2   g046(.a(x16), .b(x09), .O(new_n98_));
  nor2   g047(.a(new_n98_), .b(new_n58_), .O(new_n99_));
  oai21  g048(.a(new_n97_), .b(new_n56_), .c(new_n99_), .O(z06));
  nor3   g049(.a(x25), .b(x24), .c(x23), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n74_), .c(new_n61_), .O(new_n102_));
  nor2   g051(.a(x26), .b(x25), .O(new_n103_));
  inv1   g052(.a(new_n103_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n88_), .O(new_n105_));
  aoi21  g054(.a(new_n102_), .b(x26), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x08), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n58_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n56_), .c(new_n108_), .O(z07));
  nand4  g058(.a(new_n103_), .b(new_n87_), .c(new_n74_), .d(new_n61_), .O(new_n110_));
  nor2   g059(.a(x27), .b(x26), .O(new_n111_));
  aoi22  g060(.a(new_n111_), .b(new_n96_), .c(new_n110_), .d(x27), .O(new_n112_));
  nor2   g061(.a(x16), .b(x07), .O(new_n113_));
  nor2   g062(.a(new_n113_), .b(new_n58_), .O(new_n114_));
  oai21  g063(.a(new_n112_), .b(new_n56_), .c(new_n114_), .O(z08));
  inv1   g064(.a(x28), .O(new_n116_));
  nand3  g065(.a(new_n111_), .b(new_n96_), .c(new_n116_), .O(new_n117_));
  nand4  g066(.a(new_n111_), .b(new_n101_), .c(new_n74_), .d(new_n61_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(x28), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n117_), .c(x16), .O(new_n120_));
  aoi21  g069(.a(new_n56_), .b(x06), .c(new_n64_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n57_), .O(z09));
  oai21  g072(.a(new_n118_), .b(x28), .c(x29), .O(new_n124_));
  nor3   g073(.a(x29), .b(x28), .c(x27), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n105_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(x16), .O(new_n128_));
  nor2   g077(.a(x16), .b(x05), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n58_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n128_), .O(z10));
  inv1   g080(.a(new_n58_), .O(new_n132_));
  inv1   g081(.a(x30), .O(new_n133_));
  aoi21  g082(.a(new_n125_), .b(new_n105_), .c(new_n133_), .O(new_n134_));
  inv1   g083(.a(x27), .O(new_n135_));
  nor2   g084(.a(x29), .b(x28), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n133_), .c(new_n135_), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n110_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n134_), .c(x16), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n132_), .O(z11));
  inv1   g089(.a(new_n137_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n105_), .O(new_n142_));
  nor2   g091(.a(x31), .b(x30), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n136_), .c(new_n103_), .d(new_n135_), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n88_), .O(new_n145_));
  aoi21  g094(.a(new_n142_), .b(x31), .c(new_n145_), .O(new_n146_));
  nor2   g095(.a(x16), .b(x03), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n58_), .O(new_n148_));
  oai21  g097(.a(new_n146_), .b(new_n56_), .c(new_n148_), .O(z12));
  oai21  g098(.a(new_n144_), .b(new_n88_), .c(x32), .O(new_n150_));
  nor2   g099(.a(x32), .b(x31), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n103_), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n137_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n89_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n150_), .c(x16), .O(new_n155_));
  aoi21  g104(.a(new_n56_), .b(x02), .c(new_n64_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n57_), .O(z13));
  inv1   g107(.a(x33), .O(new_n159_));
  aoi21  g108(.a(new_n153_), .b(new_n89_), .c(new_n159_), .O(new_n160_));
  nor2   g109(.a(x33), .b(x32), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n143_), .c(new_n136_), .d(new_n111_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n102_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n160_), .c(x16), .O(new_n164_));
  nor2   g113(.a(x16), .b(x01), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n58_), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n164_), .O(z14));
  oai21  g116(.a(new_n162_), .b(new_n102_), .c(x34), .O(new_n168_));
  inv1   g117(.a(x34), .O(new_n169_));
  inv1   g118(.a(new_n162_), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n96_), .c(new_n169_), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n168_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x16), .O(new_n173_));
  nor2   g122(.a(x16), .b(x00), .O(new_n174_));
  nor2   g123(.a(new_n174_), .b(new_n58_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n173_), .O(z15));
endmodule


