// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  inv1   g005(.a(x20), .O(new_n57_));
  nor2   g006(.a(x19), .b(x17), .O(new_n58_));
  nor2   g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  oai21  g009(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  inv1   g010(.a(x14), .O(new_n62_));
  aoi21  g011(.a(new_n52_), .b(new_n62_), .c(x18), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n61_), .O(z01));
  nand2  g013(.a(x25), .b(x18), .O(new_n65_));
  inv1   g014(.a(x21), .O(new_n66_));
  nor2   g015(.a(new_n60_), .b(new_n66_), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x16), .O(new_n70_));
  aoi21  g019(.a(new_n52_), .b(x13), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n70_), .b(new_n67_), .c(new_n71_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n65_), .O(z02));
  inv1   g022(.a(x18), .O(new_n74_));
  nand2  g023(.a(new_n52_), .b(x12), .O(new_n75_));
  inv1   g024(.a(x22), .O(new_n76_));
  nand3  g025(.a(new_n68_), .b(new_n58_), .c(new_n76_), .O(new_n77_));
  nand2  g026(.a(new_n69_), .b(x22), .O(new_n78_));
  nand3  g027(.a(new_n78_), .b(new_n77_), .c(x16), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n74_), .O(z03));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n68_), .c(new_n58_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi21  g033(.a(new_n77_), .b(x23), .c(new_n84_), .O(new_n85_));
  inv1   g034(.a(x11), .O(new_n86_));
  aoi21  g035(.a(new_n52_), .b(new_n86_), .c(x18), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n52_), .c(new_n87_), .O(z04));
  inv1   g037(.a(x24), .O(new_n89_));
  nor2   g038(.a(new_n84_), .b(new_n89_), .O(new_n90_));
  nor2   g039(.a(x24), .b(x21), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n82_), .c(new_n60_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(x16), .O(new_n93_));
  aoi21  g042(.a(new_n52_), .b(x10), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n93_), .b(new_n90_), .c(new_n94_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n65_), .O(z05));
  inv1   g045(.a(x25), .O(new_n97_));
  nor3   g046(.a(x25), .b(x24), .c(x23), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(x16), .O(new_n99_));
  oai22  g048(.a(new_n99_), .b(new_n77_), .c(x16), .d(x09), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n74_), .O(new_n101_));
  aoi21  g050(.a(new_n92_), .b(x16), .c(x18), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n97_), .c(new_n101_), .O(z06));
  inv1   g052(.a(x26), .O(new_n104_));
  nor2   g053(.a(x25), .b(x24), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n83_), .O(new_n107_));
  nand2  g056(.a(new_n58_), .b(new_n57_), .O(new_n108_));
  nand2  g057(.a(new_n91_), .b(new_n82_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(x16), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n107_), .c(new_n74_), .O(new_n112_));
  inv1   g061(.a(x23), .O(new_n113_));
  nand2  g062(.a(new_n105_), .b(new_n113_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(new_n77_), .c(new_n104_), .O(new_n115_));
  oai21  g064(.a(x16), .b(x08), .c(new_n74_), .O(new_n116_));
  aoi21  g065(.a(new_n115_), .b(x16), .c(new_n116_), .O(new_n117_));
  aoi21  g066(.a(new_n112_), .b(new_n97_), .c(new_n117_), .O(z07));
  inv1   g067(.a(x27), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n52_), .c(new_n74_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(x25), .O(new_n121_));
  inv1   g070(.a(new_n77_), .O(new_n122_));
  nor3   g071(.a(x26), .b(x24), .c(x23), .O(new_n123_));
  aoi21  g072(.a(new_n123_), .b(new_n122_), .c(new_n119_), .O(new_n124_));
  nor2   g073(.a(x27), .b(x26), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n98_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n77_), .c(x16), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  inv1   g077(.a(x07), .O(new_n129_));
  oai21  g078(.a(x16), .b(new_n129_), .c(new_n74_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n128_), .c(new_n121_), .O(z08));
  inv1   g080(.a(x28), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n52_), .c(new_n74_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x25), .O(new_n134_));
  nor2   g083(.a(x16), .b(x06), .O(new_n135_));
  oai21  g084(.a(new_n126_), .b(new_n77_), .c(new_n132_), .O(new_n136_));
  nand3  g085(.a(new_n125_), .b(x28), .c(new_n89_), .O(new_n137_));
  inv1   g086(.a(new_n137_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n84_), .c(new_n52_), .O(new_n139_));
  aoi21  g088(.a(new_n139_), .b(new_n136_), .c(new_n135_), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(x18), .c(new_n134_), .O(z09));
  inv1   g090(.a(x29), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n52_), .c(new_n74_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x25), .O(new_n144_));
  nor2   g093(.a(x28), .b(x27), .O(new_n145_));
  nand3  g094(.a(new_n145_), .b(new_n107_), .c(new_n142_), .O(new_n146_));
  nand2  g095(.a(new_n145_), .b(new_n123_), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n77_), .c(x29), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n146_), .c(x16), .O(new_n149_));
  aoi21  g098(.a(new_n52_), .b(x05), .c(x18), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n144_), .O(z10));
  nand2  g101(.a(x30), .b(x16), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n74_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(x25), .O(new_n155_));
  nand3  g104(.a(new_n145_), .b(new_n123_), .c(new_n142_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n77_), .c(x30), .O(new_n157_));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n145_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n107_), .O(new_n161_));
  nand3  g110(.a(new_n161_), .b(new_n157_), .c(x16), .O(new_n162_));
  aoi21  g111(.a(new_n52_), .b(x04), .c(x18), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n155_), .O(z11));
  nor2   g114(.a(x31), .b(x28), .O(new_n166_));
  nand4  g115(.a(new_n166_), .b(new_n158_), .c(new_n125_), .d(new_n97_), .O(new_n167_));
  nor2   g116(.a(new_n167_), .b(new_n92_), .O(new_n168_));
  aoi21  g117(.a(new_n161_), .b(x31), .c(new_n168_), .O(new_n169_));
  inv1   g118(.a(x03), .O(new_n170_));
  aoi21  g119(.a(new_n52_), .b(new_n170_), .c(x18), .O(new_n171_));
  oai21  g120(.a(new_n169_), .b(new_n52_), .c(new_n171_), .O(z12));
  nand2  g121(.a(x32), .b(x16), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n74_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(x25), .O(new_n175_));
  inv1   g124(.a(x31), .O(new_n176_));
  nor2   g125(.a(x32), .b(x26), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n158_), .c(new_n145_), .d(new_n176_), .O(new_n178_));
  inv1   g127(.a(new_n178_), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n110_), .c(new_n97_), .O(new_n180_));
  nand4  g129(.a(new_n166_), .b(new_n158_), .c(new_n125_), .d(new_n89_), .O(new_n181_));
  oai21  g130(.a(new_n181_), .b(new_n83_), .c(x32), .O(new_n182_));
  nand3  g131(.a(new_n182_), .b(new_n180_), .c(x16), .O(new_n183_));
  aoi21  g132(.a(new_n52_), .b(x02), .c(x18), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n183_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n175_), .O(z13));
  nand2  g135(.a(x33), .b(x16), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n74_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(x25), .O(new_n189_));
  nor2   g138(.a(new_n114_), .b(new_n77_), .O(new_n190_));
  nor2   g139(.a(x33), .b(x27), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n177_), .c(new_n166_), .d(new_n158_), .O(new_n192_));
  inv1   g141(.a(new_n192_), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(new_n190_), .O(new_n194_));
  oai21  g143(.a(new_n178_), .b(new_n92_), .c(x33), .O(new_n195_));
  nand3  g144(.a(new_n195_), .b(new_n194_), .c(x16), .O(new_n196_));
  aoi21  g145(.a(new_n52_), .b(x01), .c(x18), .O(new_n197_));
  nand2  g146(.a(new_n197_), .b(new_n196_), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n189_), .O(z14));
  inv1   g148(.a(x34), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(new_n52_), .c(new_n74_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(x25), .O(new_n202_));
  oai21  g151(.a(new_n192_), .b(new_n92_), .c(x34), .O(new_n203_));
  nand3  g152(.a(new_n193_), .b(new_n190_), .c(new_n200_), .O(new_n204_));
  nand3  g153(.a(new_n204_), .b(new_n203_), .c(x16), .O(new_n205_));
  aoi21  g154(.a(new_n52_), .b(x00), .c(x18), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n205_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(new_n202_), .O(z15));
endmodule


