// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x11), .O(new_n58_));
  aoi21  g007(.a(new_n56_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n54_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  xor2a  g010(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  nor2   g011(.a(x16), .b(x14), .O(new_n63_));
  nor2   g012(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  oai21  g013(.a(new_n62_), .b(new_n56_), .c(new_n64_), .O(z01));
  inv1   g014(.a(x21), .O(new_n66_));
  aoi21  g015(.a(new_n53_), .b(new_n61_), .c(new_n66_), .O(new_n67_));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(x16), .O(new_n70_));
  inv1   g019(.a(x11), .O(new_n71_));
  aoi21  g020(.a(new_n56_), .b(x13), .c(new_n71_), .O(new_n72_));
  oai21  g021(.a(new_n70_), .b(new_n67_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n57_), .O(z02));
  inv1   g023(.a(x22), .O(new_n75_));
  nand3  g024(.a(new_n68_), .b(new_n53_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n69_), .b(x22), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n76_), .c(x16), .O(new_n78_));
  aoi21  g027(.a(new_n56_), .b(x12), .c(new_n71_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n57_), .O(z03));
  nand2  g030(.a(new_n76_), .b(x23), .O(new_n82_));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n68_), .c(new_n53_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(x16), .c(x11), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n57_), .O(z04));
  nor2   g036(.a(x24), .b(x23), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n68_), .c(new_n53_), .d(new_n75_), .O(new_n89_));
  nand2  g038(.a(new_n84_), .b(x24), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  aoi21  g040(.a(new_n56_), .b(x10), .c(new_n71_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n57_), .O(z05));
  nand2  g043(.a(new_n89_), .b(x25), .O(new_n95_));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n83_), .c(new_n68_), .d(new_n53_), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n95_), .c(x16), .O(new_n98_));
  aoi21  g047(.a(new_n56_), .b(x09), .c(new_n71_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n57_), .O(z06));
  inv1   g050(.a(x25), .O(new_n102_));
  inv1   g051(.a(x26), .O(new_n103_));
  nand3  g052(.a(new_n88_), .b(new_n103_), .c(new_n102_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n76_), .O(new_n105_));
  aoi21  g054(.a(new_n97_), .b(x26), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x08), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n58_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n56_), .c(new_n108_), .O(z07));
  inv1   g058(.a(x27), .O(new_n110_));
  xor2a  g059(.a(new_n105_), .b(new_n110_), .O(new_n111_));
  nor2   g060(.a(x16), .b(x07), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n58_), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n56_), .c(new_n113_), .O(z08));
  inv1   g063(.a(x28), .O(new_n115_));
  aoi21  g064(.a(new_n105_), .b(new_n110_), .c(new_n115_), .O(new_n116_));
  inv1   g065(.a(new_n76_), .O(new_n117_));
  inv1   g066(.a(new_n104_), .O(new_n118_));
  nor2   g067(.a(x28), .b(x27), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  inv1   g069(.a(new_n120_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n116_), .c(x16), .O(new_n122_));
  nor2   g071(.a(x16), .b(x06), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n58_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n122_), .O(z09));
  inv1   g074(.a(x29), .O(new_n126_));
  nand3  g075(.a(new_n119_), .b(new_n105_), .c(new_n126_), .O(new_n127_));
  nand2  g076(.a(new_n120_), .b(x29), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n127_), .c(x16), .O(new_n129_));
  aoi21  g078(.a(new_n56_), .b(x05), .c(new_n71_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n57_), .O(z10));
  nor2   g081(.a(x30), .b(x29), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n119_), .O(new_n134_));
  inv1   g083(.a(new_n134_), .O(new_n135_));
  nand3  g084(.a(new_n135_), .b(new_n118_), .c(new_n117_), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(x16), .O(new_n137_));
  aoi21  g086(.a(new_n127_), .b(x30), .c(new_n137_), .O(new_n138_));
  inv1   g087(.a(x04), .O(new_n139_));
  oai21  g088(.a(x16), .b(new_n139_), .c(x11), .O(new_n140_));
  oai21  g089(.a(new_n140_), .b(new_n138_), .c(new_n57_), .O(z11));
  nor2   g090(.a(x31), .b(x28), .O(new_n142_));
  nor2   g091(.a(x27), .b(x26), .O(new_n143_));
  nand4  g092(.a(new_n143_), .b(new_n142_), .c(new_n133_), .d(new_n102_), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n89_), .c(x16), .O(new_n145_));
  aoi21  g094(.a(new_n136_), .b(x31), .c(new_n145_), .O(new_n146_));
  inv1   g095(.a(x03), .O(new_n147_));
  oai21  g096(.a(x16), .b(new_n147_), .c(x11), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n146_), .c(new_n57_), .O(z12));
  oai21  g098(.a(new_n144_), .b(new_n89_), .c(x32), .O(new_n150_));
  nor2   g099(.a(x32), .b(x31), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n135_), .c(new_n105_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(x16), .O(new_n154_));
  nor2   g103(.a(x16), .b(x02), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n58_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n154_), .O(z13));
  nor2   g106(.a(x33), .b(x32), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n143_), .c(new_n142_), .d(new_n133_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n97_), .O(new_n160_));
  aoi21  g109(.a(new_n152_), .b(x33), .c(new_n160_), .O(new_n161_));
  nor2   g110(.a(x16), .b(x01), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n58_), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n56_), .c(new_n163_), .O(z14));
  oai21  g113(.a(new_n159_), .b(new_n97_), .c(x34), .O(new_n165_));
  inv1   g114(.a(x34), .O(new_n166_));
  nand2  g115(.a(new_n160_), .b(new_n166_), .O(new_n167_));
  nand3  g116(.a(new_n167_), .b(new_n165_), .c(x16), .O(new_n168_));
  aoi21  g117(.a(new_n56_), .b(x00), .c(new_n71_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(new_n57_), .O(z15));
endmodule


