// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n188_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_, new_n221_;
  inv1   g000(.a(x24), .O(new_n52_));
  nand2  g001(.a(new_n52_), .b(x18), .O(new_n53_));
  inv1   g002(.a(x18), .O(new_n54_));
  nand3  g003(.a(x19), .b(x17), .c(x16), .O(new_n55_));
  inv1   g004(.a(x17), .O(new_n56_));
  inv1   g005(.a(x19), .O(new_n57_));
  nand3  g006(.a(new_n57_), .b(new_n56_), .c(x16), .O(new_n58_));
  inv1   g007(.a(x15), .O(new_n59_));
  inv1   g008(.a(x16), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g010(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n54_), .O(new_n63_));
  nand2  g012(.a(new_n63_), .b(new_n53_), .O(z00));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(x19), .b(x17), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  oai21  g016(.a(x19), .b(x17), .c(x20), .O(new_n68_));
  nand3  g017(.a(new_n68_), .b(new_n67_), .c(x16), .O(new_n69_));
  aoi21  g018(.a(new_n60_), .b(x14), .c(x18), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n53_), .O(z01));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  aoi22  g022(.a(new_n73_), .b(new_n66_), .c(new_n67_), .d(x21), .O(new_n74_));
  inv1   g023(.a(x13), .O(new_n75_));
  aoi21  g024(.a(new_n60_), .b(new_n75_), .c(x18), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n60_), .c(new_n76_), .O(z02));
  nand2  g026(.a(new_n60_), .b(x12), .O(new_n78_));
  inv1   g027(.a(x22), .O(new_n79_));
  nand3  g028(.a(new_n73_), .b(new_n66_), .c(new_n79_), .O(new_n80_));
  nand2  g029(.a(new_n73_), .b(new_n66_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n78_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n54_), .O(z03));
  inv1   g034(.a(x11), .O(new_n86_));
  nor2   g035(.a(x16), .b(new_n86_), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n73_), .c(new_n66_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(x16), .O(new_n90_));
  aoi21  g039(.a(new_n80_), .b(x23), .c(new_n90_), .O(new_n91_));
  oai21  g040(.a(new_n91_), .b(new_n87_), .c(new_n54_), .O(z04));
  nand4  g041(.a(new_n88_), .b(new_n73_), .c(new_n66_), .d(x16), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n54_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n52_), .O(new_n95_));
  oai22  g044(.a(new_n90_), .b(new_n52_), .c(x16), .d(x10), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n54_), .O(new_n97_));
  nand2  g046(.a(new_n97_), .b(new_n95_), .O(z05));
  inv1   g047(.a(x21), .O(new_n99_));
  inv1   g048(.a(x23), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n79_), .c(new_n99_), .d(new_n65_), .O(new_n101_));
  inv1   g050(.a(new_n101_), .O(new_n102_));
  nor2   g051(.a(x22), .b(x21), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n103_), .c(new_n66_), .d(new_n65_), .O(new_n105_));
  inv1   g054(.a(x25), .O(new_n106_));
  nand4  g055(.a(new_n106_), .b(new_n52_), .c(new_n57_), .d(new_n56_), .O(new_n107_));
  inv1   g056(.a(new_n107_), .O(new_n108_));
  aoi22  g057(.a(new_n108_), .b(new_n102_), .c(new_n105_), .d(x25), .O(new_n109_));
  inv1   g058(.a(x09), .O(new_n110_));
  aoi21  g059(.a(new_n60_), .b(new_n110_), .c(x18), .O(new_n111_));
  oai21  g060(.a(new_n109_), .b(new_n60_), .c(new_n111_), .O(z06));
  nor2   g061(.a(x26), .b(x25), .O(new_n113_));
  nor2   g062(.a(x23), .b(x20), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(new_n113_), .c(new_n103_), .O(new_n115_));
  oai21  g064(.a(new_n115_), .b(new_n58_), .c(new_n54_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n52_), .O(new_n117_));
  nor2   g066(.a(x25), .b(x24), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n88_), .c(new_n73_), .d(new_n66_), .O(new_n119_));
  nor2   g068(.a(x16), .b(x08), .O(new_n120_));
  inv1   g069(.a(x26), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n60_), .O(new_n122_));
  aoi21  g071(.a(new_n122_), .b(new_n119_), .c(new_n120_), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(x18), .c(new_n117_), .O(z07));
  nor3   g073(.a(x27), .b(x26), .c(x25), .O(new_n125_));
  inv1   g074(.a(new_n125_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n93_), .c(new_n54_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n52_), .O(new_n128_));
  inv1   g077(.a(new_n89_), .O(new_n129_));
  nor3   g078(.a(x26), .b(x25), .c(x24), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g080(.a(x27), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n60_), .O(new_n133_));
  nor2   g082(.a(x16), .b(x07), .O(new_n134_));
  aoi21  g083(.a(new_n133_), .b(new_n131_), .c(new_n134_), .O(new_n135_));
  oai21  g084(.a(new_n135_), .b(x18), .c(new_n128_), .O(z08));
  inv1   g085(.a(x28), .O(new_n137_));
  inv1   g086(.a(new_n58_), .O(new_n138_));
  nand4  g087(.a(new_n125_), .b(new_n102_), .c(new_n138_), .d(new_n137_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n54_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n52_), .O(new_n141_));
  nor2   g090(.a(x27), .b(x26), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n104_), .c(new_n106_), .O(new_n143_));
  nor2   g092(.a(new_n137_), .b(new_n60_), .O(new_n144_));
  oai21  g093(.a(new_n143_), .b(new_n80_), .c(new_n144_), .O(new_n145_));
  inv1   g094(.a(x06), .O(new_n146_));
  nand2  g095(.a(new_n60_), .b(new_n146_), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n54_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n141_), .O(z09));
  nor2   g099(.a(x29), .b(x28), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n132_), .O(new_n152_));
  nor3   g101(.a(new_n152_), .b(new_n115_), .c(new_n58_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(x18), .c(new_n52_), .O(new_n154_));
  nand4  g103(.a(new_n142_), .b(new_n104_), .c(new_n137_), .d(new_n106_), .O(new_n155_));
  inv1   g104(.a(x29), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n60_), .O(new_n157_));
  oai21  g106(.a(new_n155_), .b(new_n80_), .c(new_n157_), .O(new_n158_));
  inv1   g107(.a(x05), .O(new_n159_));
  nand2  g108(.a(new_n60_), .b(new_n159_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n158_), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n54_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n154_), .O(z10));
  nand4  g112(.a(new_n151_), .b(new_n113_), .c(new_n132_), .d(new_n52_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n89_), .c(x30), .O(new_n165_));
  nor2   g114(.a(x30), .b(x27), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n151_), .O(new_n167_));
  inv1   g116(.a(new_n167_), .O(new_n168_));
  nand3  g117(.a(new_n168_), .b(new_n130_), .c(new_n129_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n165_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x16), .O(new_n171_));
  inv1   g120(.a(x04), .O(new_n172_));
  aoi21  g121(.a(new_n60_), .b(new_n172_), .c(x18), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z11));
  inv1   g123(.a(x30), .O(new_n175_));
  inv1   g124(.a(x31), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n175_), .c(new_n156_), .d(new_n137_), .O(new_n177_));
  inv1   g126(.a(new_n177_), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n125_), .c(new_n102_), .d(new_n138_), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(new_n54_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n52_), .O(new_n181_));
  nand4  g130(.a(new_n166_), .b(new_n151_), .c(new_n113_), .d(new_n52_), .O(new_n182_));
  nor2   g131(.a(new_n176_), .b(new_n60_), .O(new_n183_));
  oai21  g132(.a(new_n182_), .b(new_n89_), .c(new_n183_), .O(new_n184_));
  inv1   g133(.a(x03), .O(new_n185_));
  nand2  g134(.a(new_n60_), .b(new_n185_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n184_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n54_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n181_), .O(z12));
  nor2   g138(.a(x32), .b(x31), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n166_), .c(new_n151_), .d(new_n113_), .O(new_n191_));
  oai21  g140(.a(new_n191_), .b(new_n93_), .c(new_n54_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(new_n52_), .O(new_n193_));
  nor2   g142(.a(x31), .b(x30), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n151_), .c(new_n142_), .d(new_n106_), .O(new_n195_));
  inv1   g144(.a(x32), .O(new_n196_));
  nor2   g145(.a(new_n196_), .b(new_n60_), .O(new_n197_));
  oai21  g146(.a(new_n195_), .b(new_n105_), .c(new_n197_), .O(new_n198_));
  inv1   g147(.a(x02), .O(new_n199_));
  nand2  g148(.a(new_n60_), .b(new_n199_), .O(new_n200_));
  nand2  g149(.a(new_n200_), .b(new_n198_), .O(new_n201_));
  nand2  g150(.a(new_n201_), .b(new_n54_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n193_), .O(z13));
  oai21  g152(.a(new_n191_), .b(new_n105_), .c(x33), .O(new_n204_));
  inv1   g153(.a(x33), .O(new_n205_));
  nand4  g154(.a(new_n205_), .b(new_n196_), .c(new_n132_), .d(new_n121_), .O(new_n206_));
  inv1   g155(.a(new_n206_), .O(new_n207_));
  nand4  g156(.a(new_n207_), .b(new_n178_), .c(new_n108_), .d(new_n102_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(new_n204_), .O(new_n209_));
  nand2  g158(.a(new_n209_), .b(x16), .O(new_n210_));
  inv1   g159(.a(x01), .O(new_n211_));
  aoi21  g160(.a(new_n60_), .b(new_n211_), .c(x18), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n210_), .O(z14));
  inv1   g162(.a(x34), .O(new_n214_));
  nor2   g163(.a(new_n208_), .b(new_n214_), .O(new_n215_));
  nor2   g164(.a(x33), .b(x32), .O(new_n216_));
  nand4  g165(.a(new_n216_), .b(new_n194_), .c(new_n151_), .d(new_n142_), .O(new_n217_));
  oai21  g166(.a(new_n217_), .b(new_n119_), .c(new_n214_), .O(new_n218_));
  nand2  g167(.a(new_n218_), .b(x16), .O(new_n219_));
  inv1   g168(.a(x00), .O(new_n220_));
  aoi21  g169(.a(new_n60_), .b(new_n220_), .c(x18), .O(new_n221_));
  oai21  g170(.a(new_n219_), .b(new_n215_), .c(new_n221_), .O(z15));
endmodule


