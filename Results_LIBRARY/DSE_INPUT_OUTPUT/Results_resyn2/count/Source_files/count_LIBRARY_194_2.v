// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x06), .O(new_n59_));
  inv1   g008(.a(x18), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g010(.a(new_n58_), .b(new_n57_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  xor2a  g013(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  nor2   g015(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  oai21  g016(.a(new_n65_), .b(new_n58_), .c(new_n67_), .O(z01));
  nand3  g017(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n69_));
  nor2   g018(.a(x21), .b(x20), .O(new_n70_));
  aoi22  g019(.a(new_n70_), .b(new_n55_), .c(new_n69_), .d(x21), .O(new_n71_));
  nor2   g020(.a(x16), .b(x13), .O(new_n72_));
  nor2   g021(.a(new_n72_), .b(new_n61_), .O(new_n73_));
  oai21  g022(.a(new_n71_), .b(new_n58_), .c(new_n73_), .O(z02));
  inv1   g023(.a(x22), .O(new_n75_));
  aoi21  g024(.a(new_n70_), .b(new_n55_), .c(new_n75_), .O(new_n76_));
  nand3  g025(.a(new_n70_), .b(new_n55_), .c(new_n75_), .O(new_n77_));
  inv1   g026(.a(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n76_), .c(x16), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n61_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n79_), .O(z03));
  nand2  g031(.a(new_n77_), .b(x23), .O(new_n83_));
  nor2   g032(.a(x23), .b(x22), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n70_), .c(new_n55_), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n83_), .c(x16), .O(new_n86_));
  aoi21  g035(.a(new_n58_), .b(x11), .c(x06), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n60_), .O(z04));
  nand2  g038(.a(new_n85_), .b(x24), .O(new_n90_));
  nor2   g039(.a(x24), .b(x23), .O(new_n91_));
  nor2   g040(.a(x22), .b(x21), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n91_), .c(new_n55_), .d(new_n64_), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n90_), .c(x16), .O(new_n94_));
  aoi21  g043(.a(new_n58_), .b(x10), .c(x06), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n60_), .O(z05));
  nand2  g046(.a(new_n93_), .b(x25), .O(new_n98_));
  nor2   g047(.a(x25), .b(x24), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n84_), .c(new_n70_), .d(new_n55_), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n98_), .c(x16), .O(new_n101_));
  aoi21  g050(.a(new_n58_), .b(x09), .c(x06), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n60_), .O(z06));
  inv1   g053(.a(x24), .O(new_n105_));
  nor2   g054(.a(x26), .b(x25), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n85_), .O(new_n108_));
  aoi21  g057(.a(new_n100_), .b(x26), .c(new_n108_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x08), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n61_), .O(new_n111_));
  oai21  g060(.a(new_n109_), .b(new_n58_), .c(new_n111_), .O(z07));
  oai21  g061(.a(new_n107_), .b(new_n85_), .c(x27), .O(new_n113_));
  inv1   g062(.a(x21), .O(new_n114_));
  inv1   g063(.a(x23), .O(new_n115_));
  nand4  g064(.a(new_n105_), .b(new_n115_), .c(new_n75_), .d(new_n114_), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n69_), .O(new_n117_));
  inv1   g066(.a(x25), .O(new_n118_));
  inv1   g067(.a(x26), .O(new_n119_));
  inv1   g068(.a(x27), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n117_), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n113_), .c(x16), .O(new_n124_));
  aoi21  g073(.a(new_n58_), .b(x07), .c(x06), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g075(.a(new_n126_), .b(new_n60_), .O(z08));
  nand2  g076(.a(new_n123_), .b(x28), .O(new_n128_));
  inv1   g077(.a(x28), .O(new_n129_));
  nand4  g078(.a(new_n106_), .b(new_n91_), .c(new_n129_), .d(new_n120_), .O(new_n130_));
  nor2   g079(.a(new_n130_), .b(new_n77_), .O(new_n131_));
  nor3   g080(.a(new_n131_), .b(new_n61_), .c(new_n58_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n128_), .O(z09));
  oai21  g082(.a(new_n130_), .b(new_n77_), .c(x29), .O(new_n134_));
  inv1   g083(.a(new_n85_), .O(new_n135_));
  inv1   g084(.a(x29), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n129_), .c(new_n105_), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n121_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  nand3  g088(.a(new_n139_), .b(new_n134_), .c(x16), .O(new_n140_));
  aoi21  g089(.a(new_n58_), .b(x05), .c(x06), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n60_), .O(z10));
  nor2   g092(.a(x29), .b(x28), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n106_), .c(new_n120_), .d(new_n105_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n85_), .c(x30), .O(new_n146_));
  inv1   g095(.a(x30), .O(new_n147_));
  nand3  g096(.a(new_n138_), .b(new_n135_), .c(new_n147_), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n146_), .c(x16), .O(new_n149_));
  aoi21  g098(.a(new_n58_), .b(x04), .c(x06), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n60_), .O(z11));
  nor2   g101(.a(x31), .b(x30), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n144_), .c(new_n106_), .d(new_n120_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n93_), .O(new_n155_));
  aoi21  g104(.a(new_n148_), .b(x31), .c(new_n155_), .O(new_n156_));
  nor2   g105(.a(x16), .b(x03), .O(new_n157_));
  nor2   g106(.a(new_n157_), .b(new_n61_), .O(new_n158_));
  oai21  g107(.a(new_n156_), .b(new_n58_), .c(new_n158_), .O(z12));
  inv1   g108(.a(x32), .O(new_n160_));
  inv1   g109(.a(x31), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n147_), .c(new_n136_), .d(new_n129_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n121_), .O(new_n163_));
  nand3  g112(.a(new_n163_), .b(new_n117_), .c(new_n160_), .O(new_n164_));
  oai21  g113(.a(new_n154_), .b(new_n93_), .c(x32), .O(new_n165_));
  nand3  g114(.a(new_n165_), .b(new_n164_), .c(x16), .O(new_n166_));
  aoi21  g115(.a(new_n58_), .b(x02), .c(x06), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n60_), .O(z13));
  nor2   g118(.a(x27), .b(x26), .O(new_n170_));
  nor2   g119(.a(x33), .b(x32), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n170_), .c(new_n153_), .d(new_n144_), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n100_), .O(new_n173_));
  aoi21  g122(.a(new_n164_), .b(x33), .c(new_n173_), .O(new_n174_));
  nor2   g123(.a(x16), .b(x01), .O(new_n175_));
  nor2   g124(.a(new_n175_), .b(new_n61_), .O(new_n176_));
  oai21  g125(.a(new_n174_), .b(new_n58_), .c(new_n176_), .O(z14));
  nor3   g126(.a(new_n172_), .b(new_n100_), .c(x34), .O(new_n178_));
  oai21  g127(.a(new_n172_), .b(new_n100_), .c(x34), .O(new_n179_));
  nand2  g128(.a(new_n179_), .b(x16), .O(new_n180_));
  aoi21  g129(.a(new_n58_), .b(x00), .c(x06), .O(new_n181_));
  oai21  g130(.a(new_n180_), .b(new_n178_), .c(new_n181_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n60_), .O(z15));
endmodule


