// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x33), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g008(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  oai21  g012(.a(new_n53_), .b(new_n63_), .c(x16), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(x33), .O(new_n65_));
  oai21  g014(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n57_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  nand3  g017(.a(new_n53_), .b(new_n68_), .c(new_n63_), .O(new_n69_));
  nand2  g018(.a(new_n53_), .b(new_n63_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x21), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n69_), .c(x16), .O(new_n72_));
  aoi21  g021(.a(new_n56_), .b(x13), .c(x33), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n57_), .O(z02));
  nor2   g024(.a(x22), .b(x21), .O(new_n76_));
  inv1   g025(.a(new_n76_), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  aoi21  g027(.a(new_n69_), .b(x22), .c(new_n78_), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n59_), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n56_), .c(new_n81_), .O(z03));
  nand2  g031(.a(new_n76_), .b(new_n62_), .O(new_n83_));
  xor2a  g032(.a(new_n83_), .b(x23), .O(new_n84_));
  nor2   g033(.a(x16), .b(x11), .O(new_n85_));
  nor2   g034(.a(new_n85_), .b(new_n59_), .O(new_n86_));
  oai21  g035(.a(new_n84_), .b(new_n56_), .c(new_n86_), .O(z04));
  inv1   g036(.a(x23), .O(new_n88_));
  nand3  g037(.a(new_n76_), .b(new_n62_), .c(new_n88_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(x24), .O(new_n90_));
  nor2   g039(.a(x24), .b(x23), .O(new_n91_));
  nand4  g040(.a(new_n91_), .b(new_n76_), .c(new_n53_), .d(new_n63_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(x16), .O(new_n94_));
  nor2   g043(.a(x16), .b(x10), .O(new_n95_));
  nor2   g044(.a(new_n95_), .b(new_n59_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n94_), .O(z05));
  inv1   g046(.a(new_n69_), .O(new_n98_));
  inv1   g047(.a(x22), .O(new_n99_));
  inv1   g048(.a(x25), .O(new_n100_));
  nand3  g049(.a(new_n91_), .b(new_n100_), .c(new_n99_), .O(new_n101_));
  inv1   g050(.a(new_n101_), .O(new_n102_));
  aoi22  g051(.a(new_n102_), .b(new_n98_), .c(new_n92_), .d(x25), .O(new_n103_));
  nor2   g052(.a(x16), .b(x09), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n59_), .O(new_n105_));
  oai21  g054(.a(new_n103_), .b(new_n56_), .c(new_n105_), .O(z06));
  oai21  g055(.a(new_n101_), .b(new_n69_), .c(x26), .O(new_n107_));
  inv1   g056(.a(x24), .O(new_n108_));
  nand4  g057(.a(new_n108_), .b(new_n88_), .c(new_n99_), .d(new_n68_), .O(new_n109_));
  inv1   g058(.a(new_n109_), .O(new_n110_));
  nor2   g059(.a(x26), .b(x25), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n110_), .c(new_n62_), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n107_), .c(x16), .O(new_n113_));
  aoi21  g062(.a(new_n56_), .b(x08), .c(x33), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n57_), .O(z07));
  inv1   g065(.a(x27), .O(new_n117_));
  nand2  g066(.a(new_n111_), .b(new_n117_), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n92_), .O(new_n119_));
  aoi21  g068(.a(new_n112_), .b(x27), .c(new_n119_), .O(new_n120_));
  nor2   g069(.a(x16), .b(x07), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n59_), .O(new_n122_));
  oai21  g071(.a(new_n120_), .b(new_n56_), .c(new_n122_), .O(z08));
  oai21  g072(.a(new_n118_), .b(new_n92_), .c(x28), .O(new_n124_));
  nor3   g073(.a(x27), .b(x26), .c(x25), .O(new_n125_));
  nor3   g074(.a(x28), .b(x24), .c(x23), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n125_), .c(new_n78_), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n124_), .c(x16), .O(new_n128_));
  aoi21  g077(.a(new_n56_), .b(x06), .c(x33), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n57_), .O(z09));
  nor2   g080(.a(x29), .b(x28), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n108_), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n118_), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n78_), .c(new_n88_), .O(new_n135_));
  nand2  g084(.a(new_n126_), .b(new_n125_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n83_), .c(x29), .O(new_n137_));
  nand3  g086(.a(new_n137_), .b(new_n135_), .c(x16), .O(new_n138_));
  aoi21  g087(.a(new_n56_), .b(x05), .c(x33), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n57_), .O(z10));
  nand3  g090(.a(new_n132_), .b(new_n125_), .c(new_n108_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n89_), .c(x30), .O(new_n143_));
  inv1   g092(.a(x28), .O(new_n144_));
  inv1   g093(.a(x29), .O(new_n145_));
  inv1   g094(.a(x30), .O(new_n146_));
  nand4  g095(.a(new_n146_), .b(new_n145_), .c(new_n144_), .d(new_n117_), .O(new_n147_));
  inv1   g096(.a(new_n147_), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n111_), .c(new_n110_), .d(new_n62_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n143_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  nor2   g100(.a(x16), .b(x04), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n59_), .O(new_n153_));
  nand2  g102(.a(new_n153_), .b(new_n151_), .O(z11));
  nor2   g103(.a(x31), .b(x30), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n132_), .O(new_n156_));
  inv1   g105(.a(new_n156_), .O(new_n157_));
  aoi22  g106(.a(new_n157_), .b(new_n119_), .c(new_n149_), .d(x31), .O(new_n158_));
  nor2   g107(.a(x16), .b(x03), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n59_), .O(new_n160_));
  oai21  g109(.a(new_n158_), .b(new_n56_), .c(new_n160_), .O(z12));
  nand4  g110(.a(new_n157_), .b(new_n125_), .c(new_n110_), .d(new_n62_), .O(new_n162_));
  nor2   g111(.a(x30), .b(x27), .O(new_n163_));
  nor2   g112(.a(x32), .b(x31), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n163_), .c(new_n132_), .d(new_n111_), .O(new_n165_));
  nor2   g114(.a(new_n165_), .b(new_n92_), .O(new_n166_));
  aoi21  g115(.a(new_n162_), .b(x32), .c(new_n166_), .O(new_n167_));
  nor2   g116(.a(x16), .b(x02), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n59_), .O(new_n169_));
  oai21  g118(.a(new_n167_), .b(new_n56_), .c(new_n169_), .O(z13));
  aoi21  g119(.a(new_n56_), .b(x01), .c(x33), .O(new_n171_));
  oai21  g120(.a(new_n166_), .b(new_n56_), .c(new_n171_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n57_), .O(z14));
  nor2   g122(.a(x27), .b(x26), .O(new_n174_));
  nor2   g123(.a(x34), .b(x32), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  inv1   g125(.a(new_n176_), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n157_), .c(new_n102_), .d(new_n98_), .O(new_n178_));
  oai21  g127(.a(new_n165_), .b(new_n92_), .c(x34), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n178_), .c(x16), .O(new_n180_));
  aoi21  g129(.a(new_n56_), .b(x00), .c(x33), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n180_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n57_), .O(z15));
endmodule


