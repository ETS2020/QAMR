// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x32), .O(new_n53_));
  nand3  g002(.a(x19), .b(x17), .c(x16), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  nand2  g004(.a(new_n55_), .b(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g008(.a(new_n59_), .b(new_n56_), .c(new_n54_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n52_), .O(z00));
  inv1   g011(.a(x20), .O(new_n63_));
  nor2   g012(.a(new_n55_), .b(new_n63_), .O(new_n64_));
  nor3   g013(.a(x20), .b(x19), .c(x17), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  nand2  g016(.a(new_n53_), .b(new_n52_), .O(new_n68_));
  nor2   g017(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n66_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(new_n65_), .b(new_n71_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n55_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  aoi21  g024(.a(new_n58_), .b(x13), .c(x32), .O(new_n76_));
  oai21  g025(.a(new_n75_), .b(new_n72_), .c(new_n76_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n52_), .O(z02));
  xor2a  g027(.a(new_n74_), .b(x22), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n68_), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n58_), .c(new_n81_), .O(z03));
  or2    g031(.a(new_n74_), .b(x22), .O(new_n83_));
  nor2   g032(.a(x23), .b(x22), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n73_), .c(new_n55_), .O(new_n85_));
  inv1   g034(.a(new_n85_), .O(new_n86_));
  aoi21  g035(.a(new_n83_), .b(x23), .c(new_n86_), .O(new_n87_));
  nor2   g036(.a(x16), .b(x11), .O(new_n88_));
  nor2   g037(.a(new_n88_), .b(new_n68_), .O(new_n89_));
  oai21  g038(.a(new_n87_), .b(new_n58_), .c(new_n89_), .O(z04));
  nand2  g039(.a(new_n85_), .b(x24), .O(new_n91_));
  nor2   g040(.a(x24), .b(x21), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n84_), .c(new_n55_), .d(new_n63_), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n91_), .c(x16), .O(new_n94_));
  aoi21  g043(.a(new_n58_), .b(x10), .c(x32), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n52_), .O(z05));
  nor2   g046(.a(x25), .b(x24), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n84_), .c(new_n73_), .d(new_n55_), .O(new_n99_));
  nand2  g048(.a(new_n93_), .b(x25), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n99_), .c(x16), .O(new_n101_));
  aoi21  g050(.a(new_n58_), .b(x09), .c(x32), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n52_), .O(z06));
  inv1   g053(.a(x26), .O(new_n105_));
  nand2  g054(.a(new_n98_), .b(new_n105_), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n85_), .O(new_n107_));
  aoi21  g056(.a(new_n99_), .b(x26), .c(new_n107_), .O(new_n108_));
  nor2   g057(.a(x16), .b(x08), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n68_), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n58_), .c(new_n110_), .O(z07));
  inv1   g060(.a(x27), .O(new_n112_));
  nor3   g061(.a(x27), .b(x26), .c(x25), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n92_), .c(new_n84_), .d(new_n65_), .O(new_n114_));
  oai21  g063(.a(new_n107_), .b(new_n112_), .c(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  nor2   g065(.a(x16), .b(x07), .O(new_n117_));
  nor2   g066(.a(new_n117_), .b(new_n68_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n116_), .O(z08));
  inv1   g068(.a(x28), .O(new_n120_));
  nand4  g069(.a(new_n84_), .b(new_n73_), .c(new_n55_), .d(new_n120_), .O(new_n121_));
  nor2   g070(.a(x27), .b(x26), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n98_), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  aoi21  g073(.a(new_n114_), .b(x28), .c(new_n124_), .O(new_n125_));
  nor2   g074(.a(x16), .b(x06), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n68_), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n58_), .c(new_n127_), .O(z09));
  oai21  g077(.a(new_n123_), .b(new_n121_), .c(x29), .O(new_n129_));
  inv1   g078(.a(new_n121_), .O(new_n130_));
  nor2   g079(.a(x29), .b(x24), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n130_), .c(new_n113_), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n129_), .c(x16), .O(new_n133_));
  aoi21  g082(.a(new_n58_), .b(x05), .c(x32), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n52_), .O(z10));
  nand2  g085(.a(new_n131_), .b(new_n113_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n121_), .c(x30), .O(new_n138_));
  nor2   g087(.a(x29), .b(x28), .O(new_n139_));
  nor2   g088(.a(x30), .b(x27), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  inv1   g090(.a(new_n141_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n107_), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n138_), .c(x16), .O(new_n144_));
  aoi21  g093(.a(new_n58_), .b(x04), .c(x32), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n52_), .O(z11));
  inv1   g096(.a(x31), .O(new_n148_));
  aoi21  g097(.a(new_n142_), .b(new_n107_), .c(new_n148_), .O(new_n149_));
  inv1   g098(.a(x30), .O(new_n150_));
  nand3  g099(.a(new_n139_), .b(new_n148_), .c(new_n150_), .O(new_n151_));
  oai21  g100(.a(new_n151_), .b(new_n114_), .c(x16), .O(new_n152_));
  aoi21  g101(.a(new_n58_), .b(x03), .c(x32), .O(new_n153_));
  oai21  g102(.a(new_n152_), .b(new_n149_), .c(new_n153_), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n52_), .O(z12));
  nor2   g104(.a(x16), .b(x02), .O(new_n156_));
  nor3   g105(.a(new_n151_), .b(new_n123_), .c(new_n58_), .O(new_n157_));
  aoi21  g106(.a(new_n157_), .b(new_n86_), .c(new_n156_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(x32), .c(new_n52_), .O(z13));
  nand2  g108(.a(new_n92_), .b(new_n84_), .O(new_n160_));
  inv1   g109(.a(new_n160_), .O(new_n161_));
  inv1   g110(.a(new_n151_), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n113_), .c(new_n161_), .d(new_n65_), .O(new_n163_));
  nor2   g112(.a(x26), .b(x25), .O(new_n164_));
  nor2   g113(.a(x33), .b(x31), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n164_), .c(new_n140_), .d(new_n139_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n93_), .c(x16), .O(new_n167_));
  aoi21  g116(.a(new_n163_), .b(x33), .c(new_n167_), .O(new_n168_));
  inv1   g117(.a(x01), .O(new_n169_));
  oai21  g118(.a(x16), .b(new_n169_), .c(new_n53_), .O(new_n170_));
  oai21  g119(.a(new_n170_), .b(new_n168_), .c(new_n52_), .O(z14));
  nor2   g120(.a(x34), .b(x33), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n122_), .O(new_n173_));
  nor3   g122(.a(new_n173_), .b(new_n151_), .c(new_n99_), .O(new_n174_));
  oai21  g123(.a(new_n166_), .b(new_n93_), .c(x34), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(x16), .O(new_n176_));
  aoi21  g125(.a(new_n58_), .b(x00), .c(x32), .O(new_n177_));
  oai21  g126(.a(new_n176_), .b(new_n174_), .c(new_n177_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n52_), .O(z15));
endmodule


