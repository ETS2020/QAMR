// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_;
  nor2   g00(.a(x19), .b(x17), .O(new_n52_));
  inv1   g01(.a(new_n52_), .O(new_n53_));
  inv1   g02(.a(x16), .O(new_n54_));
  nor2   g03(.a(x18), .b(new_n54_), .O(new_n55_));
  nand2  g04(.a(x19), .b(x17), .O(new_n56_));
  nand3  g05(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(z00));
  inv1   g06(.a(x18), .O(new_n58_));
  inv1   g07(.a(x20), .O(new_n59_));
  xor2a  g08(.a(new_n52_), .b(new_n59_), .O(new_n60_));
  oai21  g09(.a(new_n60_), .b(new_n54_), .c(new_n58_), .O(z01));
  oai21  g10(.a(new_n53_), .b(x20), .c(x21), .O(new_n62_));
  nor2   g11(.a(x21), .b(x20), .O(new_n63_));
  nand2  g12(.a(new_n63_), .b(new_n52_), .O(new_n64_));
  nand3  g13(.a(new_n64_), .b(new_n62_), .c(new_n55_), .O(z02));
  nand2  g14(.a(new_n64_), .b(x22), .O(new_n66_));
  inv1   g15(.a(x22), .O(new_n67_));
  nand3  g16(.a(new_n63_), .b(new_n52_), .c(new_n67_), .O(new_n68_));
  nand3  g17(.a(new_n68_), .b(new_n66_), .c(new_n55_), .O(z03));
  nand2  g18(.a(new_n68_), .b(x23), .O(new_n70_));
  nor2   g19(.a(x23), .b(x22), .O(new_n71_));
  nand3  g20(.a(new_n71_), .b(new_n63_), .c(new_n52_), .O(new_n72_));
  nand3  g21(.a(new_n72_), .b(new_n70_), .c(new_n55_), .O(z04));
  nand2  g22(.a(new_n72_), .b(x24), .O(new_n74_));
  nor2   g23(.a(x24), .b(x23), .O(new_n75_));
  nor2   g24(.a(x22), .b(x21), .O(new_n76_));
  nand4  g25(.a(new_n76_), .b(new_n75_), .c(new_n52_), .d(new_n59_), .O(new_n77_));
  nand2  g26(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g27(.a(new_n78_), .b(x16), .O(new_n79_));
  nand2  g28(.a(new_n79_), .b(new_n58_), .O(z05));
  nor2   g29(.a(x25), .b(x24), .O(new_n81_));
  nand4  g30(.a(new_n81_), .b(new_n71_), .c(new_n63_), .d(new_n52_), .O(new_n82_));
  nand2  g31(.a(new_n77_), .b(x25), .O(new_n83_));
  nand3  g32(.a(new_n83_), .b(new_n82_), .c(new_n55_), .O(z06));
  nand2  g33(.a(new_n82_), .b(x26), .O(new_n85_));
  inv1   g34(.a(new_n72_), .O(new_n86_));
  nor3   g35(.a(x26), .b(x25), .c(x24), .O(new_n87_));
  nand2  g36(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g37(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nand2  g38(.a(new_n89_), .b(x16), .O(new_n90_));
  nand2  g39(.a(new_n90_), .b(new_n58_), .O(z07));
  inv1   g40(.a(x27), .O(new_n92_));
  nor2   g41(.a(x26), .b(x25), .O(new_n93_));
  nand2  g42(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g43(.a(new_n94_), .b(new_n77_), .O(new_n95_));
  aoi21  g44(.a(new_n88_), .b(x27), .c(new_n95_), .O(new_n96_));
  oai21  g45(.a(new_n96_), .b(new_n54_), .c(new_n58_), .O(z08));
  oai21  g46(.a(new_n94_), .b(new_n77_), .c(x28), .O(new_n98_));
  inv1   g47(.a(x28), .O(new_n99_));
  nand4  g48(.a(new_n93_), .b(new_n75_), .c(new_n99_), .d(new_n92_), .O(new_n100_));
  or2    g49(.a(new_n100_), .b(new_n68_), .O(new_n101_));
  nand3  g50(.a(new_n101_), .b(new_n98_), .c(new_n55_), .O(z09));
  oai21  g51(.a(new_n100_), .b(new_n68_), .c(x29), .O(new_n103_));
  nor3   g52(.a(x29), .b(x28), .c(x27), .O(new_n104_));
  nand3  g53(.a(new_n104_), .b(new_n87_), .c(new_n86_), .O(new_n105_));
  nand2  g54(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand2  g55(.a(new_n106_), .b(x16), .O(new_n107_));
  nand2  g56(.a(new_n107_), .b(new_n58_), .O(z10));
  nand2  g57(.a(new_n105_), .b(x30), .O(new_n109_));
  inv1   g58(.a(x30), .O(new_n110_));
  nand4  g59(.a(new_n104_), .b(new_n87_), .c(new_n86_), .d(new_n110_), .O(new_n111_));
  nand3  g60(.a(new_n111_), .b(new_n109_), .c(new_n55_), .O(z11));
  nand2  g61(.a(new_n111_), .b(x31), .O(new_n113_));
  nor2   g62(.a(x29), .b(x28), .O(new_n114_));
  nor2   g63(.a(x31), .b(x30), .O(new_n115_));
  nand4  g64(.a(new_n115_), .b(new_n114_), .c(new_n93_), .d(new_n92_), .O(new_n116_));
  nor2   g65(.a(new_n116_), .b(new_n77_), .O(new_n117_));
  nor3   g66(.a(new_n117_), .b(x18), .c(new_n54_), .O(new_n118_));
  nand2  g67(.a(new_n118_), .b(new_n113_), .O(z12));
  inv1   g68(.a(x32), .O(new_n120_));
  xor2a  g69(.a(new_n117_), .b(new_n120_), .O(new_n121_));
  oai21  g70(.a(new_n121_), .b(new_n54_), .c(new_n58_), .O(z13));
  nand2  g71(.a(new_n117_), .b(new_n120_), .O(new_n123_));
  nand2  g72(.a(new_n123_), .b(x33), .O(new_n124_));
  inv1   g73(.a(new_n82_), .O(new_n125_));
  nand2  g74(.a(new_n115_), .b(new_n114_), .O(new_n126_));
  inv1   g75(.a(x26), .O(new_n127_));
  inv1   g76(.a(x33), .O(new_n128_));
  nand4  g77(.a(new_n128_), .b(new_n120_), .c(new_n92_), .d(new_n127_), .O(new_n129_));
  nor2   g78(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand2  g79(.a(new_n130_), .b(new_n125_), .O(new_n131_));
  nand3  g80(.a(new_n131_), .b(new_n124_), .c(new_n55_), .O(z14));
  inv1   g81(.a(x34), .O(new_n133_));
  nand3  g82(.a(new_n130_), .b(new_n125_), .c(new_n133_), .O(new_n134_));
  nand2  g83(.a(new_n131_), .b(x34), .O(new_n135_));
  nand3  g84(.a(new_n135_), .b(new_n134_), .c(new_n55_), .O(z15));
endmodule


