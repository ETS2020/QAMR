// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x33), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(x32), .O(new_n54_));
  inv1   g003(.a(new_n54_), .O(new_n55_));
  inv1   g004(.a(x17), .O(new_n56_));
  inv1   g005(.a(x19), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g007(.a(x19), .b(x17), .O(new_n59_));
  nand3  g008(.a(new_n59_), .b(new_n58_), .c(x16), .O(new_n60_));
  inv1   g009(.a(x16), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(x15), .O(new_n62_));
  nand3  g011(.a(new_n62_), .b(new_n60_), .c(new_n55_), .O(new_n63_));
  oai21  g012(.a(new_n54_), .b(new_n52_), .c(new_n63_), .O(z00));
  inv1   g013(.a(x20), .O(new_n65_));
  nor2   g014(.a(x19), .b(x17), .O(new_n66_));
  nor2   g015(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g016(.a(new_n66_), .b(new_n65_), .O(new_n68_));
  inv1   g017(.a(new_n68_), .O(new_n69_));
  oai21  g018(.a(new_n69_), .b(new_n67_), .c(x16), .O(new_n70_));
  nor2   g019(.a(x16), .b(x14), .O(new_n71_));
  nand2  g020(.a(new_n55_), .b(new_n52_), .O(new_n72_));
  nor2   g021(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n70_), .O(z01));
  inv1   g023(.a(x21), .O(new_n75_));
  nor2   g024(.a(new_n69_), .b(new_n75_), .O(new_n76_));
  nand3  g025(.a(new_n66_), .b(new_n75_), .c(new_n65_), .O(new_n77_));
  inv1   g026(.a(new_n77_), .O(new_n78_));
  oai21  g027(.a(new_n78_), .b(new_n76_), .c(x16), .O(new_n79_));
  inv1   g028(.a(x13), .O(new_n80_));
  aoi21  g029(.a(new_n61_), .b(new_n80_), .c(x18), .O(new_n81_));
  aoi21  g030(.a(new_n81_), .b(new_n79_), .c(new_n54_), .O(z02));
  inv1   g031(.a(x22), .O(new_n83_));
  nor2   g032(.a(new_n78_), .b(new_n83_), .O(new_n84_));
  nor2   g033(.a(x22), .b(x21), .O(new_n85_));
  inv1   g034(.a(new_n85_), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n68_), .O(new_n87_));
  oai21  g036(.a(new_n87_), .b(new_n84_), .c(x16), .O(new_n88_));
  inv1   g037(.a(x12), .O(new_n89_));
  aoi21  g038(.a(new_n61_), .b(new_n89_), .c(x18), .O(new_n90_));
  aoi21  g039(.a(new_n90_), .b(new_n88_), .c(new_n54_), .O(z03));
  inv1   g040(.a(x23), .O(new_n92_));
  nor2   g041(.a(x20), .b(x19), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n85_), .c(new_n92_), .d(new_n56_), .O(new_n94_));
  oai21  g043(.a(new_n87_), .b(new_n92_), .c(new_n94_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x16), .O(new_n96_));
  inv1   g045(.a(x11), .O(new_n97_));
  aoi21  g046(.a(new_n61_), .b(new_n97_), .c(x18), .O(new_n98_));
  aoi21  g047(.a(new_n98_), .b(new_n96_), .c(new_n54_), .O(z04));
  inv1   g048(.a(x24), .O(new_n100_));
  nor3   g049(.a(x23), .b(x22), .c(x21), .O(new_n101_));
  nor3   g050(.a(x20), .b(x19), .c(x17), .O(new_n102_));
  aoi21  g051(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  nor2   g052(.a(x24), .b(x23), .O(new_n104_));
  nand4  g053(.a(new_n104_), .b(new_n93_), .c(new_n85_), .d(new_n56_), .O(new_n105_));
  inv1   g054(.a(new_n105_), .O(new_n106_));
  oai21  g055(.a(new_n106_), .b(new_n103_), .c(x16), .O(new_n107_));
  inv1   g056(.a(x10), .O(new_n108_));
  aoi21  g057(.a(new_n61_), .b(new_n108_), .c(x18), .O(new_n109_));
  aoi21  g058(.a(new_n109_), .b(new_n107_), .c(new_n54_), .O(z05));
  nand2  g059(.a(new_n105_), .b(x25), .O(new_n111_));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n101_), .c(new_n69_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  nor2   g064(.a(x16), .b(x09), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n72_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n115_), .O(z06));
  nand2  g067(.a(new_n85_), .b(new_n92_), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n68_), .O(new_n120_));
  nor3   g069(.a(x26), .b(x25), .c(x24), .O(new_n121_));
  aoi22  g070(.a(new_n121_), .b(new_n120_), .c(new_n113_), .d(x26), .O(new_n122_));
  nor2   g071(.a(x16), .b(x08), .O(new_n123_));
  nor2   g072(.a(new_n123_), .b(new_n72_), .O(new_n124_));
  oai21  g073(.a(new_n122_), .b(new_n61_), .c(new_n124_), .O(z07));
  nand3  g074(.a(new_n121_), .b(new_n101_), .c(new_n69_), .O(new_n126_));
  nor3   g075(.a(x27), .b(x26), .c(x25), .O(new_n127_));
  aoi22  g076(.a(new_n127_), .b(new_n106_), .c(new_n126_), .d(x27), .O(new_n128_));
  nor2   g077(.a(x16), .b(x07), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n72_), .O(new_n130_));
  oai21  g079(.a(new_n128_), .b(new_n61_), .c(new_n130_), .O(z08));
  inv1   g080(.a(x27), .O(new_n132_));
  nor2   g081(.a(x26), .b(x25), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n105_), .c(x28), .O(new_n135_));
  nand4  g084(.a(new_n100_), .b(new_n92_), .c(new_n83_), .d(new_n75_), .O(new_n136_));
  inv1   g085(.a(new_n136_), .O(new_n137_));
  nor2   g086(.a(x28), .b(x27), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n133_), .c(new_n137_), .d(new_n102_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(x16), .O(new_n141_));
  inv1   g090(.a(x06), .O(new_n142_));
  aoi21  g091(.a(new_n61_), .b(new_n142_), .c(x18), .O(new_n143_));
  aoi21  g092(.a(new_n143_), .b(new_n141_), .c(new_n54_), .O(z09));
  nand2  g093(.a(new_n93_), .b(new_n56_), .O(new_n145_));
  nand4  g094(.a(new_n138_), .b(new_n133_), .c(new_n104_), .d(new_n85_), .O(new_n146_));
  oai21  g095(.a(new_n146_), .b(new_n145_), .c(x29), .O(new_n147_));
  nor3   g096(.a(x29), .b(x28), .c(x27), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n121_), .c(new_n102_), .d(new_n101_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(x16), .O(new_n151_));
  inv1   g100(.a(x05), .O(new_n152_));
  aoi21  g101(.a(new_n61_), .b(new_n152_), .c(x18), .O(new_n153_));
  aoi21  g102(.a(new_n153_), .b(new_n151_), .c(new_n54_), .O(z10));
  nor2   g103(.a(x30), .b(x29), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n138_), .c(new_n133_), .d(new_n100_), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n94_), .O(new_n157_));
  aoi21  g106(.a(new_n149_), .b(x30), .c(new_n157_), .O(new_n158_));
  nor2   g107(.a(x16), .b(x04), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n72_), .O(new_n160_));
  oai21  g109(.a(new_n158_), .b(new_n61_), .c(new_n160_), .O(z11));
  oai21  g110(.a(new_n156_), .b(new_n94_), .c(x31), .O(new_n162_));
  inv1   g111(.a(x28), .O(new_n163_));
  inv1   g112(.a(x29), .O(new_n164_));
  inv1   g113(.a(x30), .O(new_n165_));
  inv1   g114(.a(x31), .O(new_n166_));
  nand4  g115(.a(new_n166_), .b(new_n165_), .c(new_n164_), .d(new_n163_), .O(new_n167_));
  inv1   g116(.a(new_n167_), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n127_), .c(new_n137_), .d(new_n102_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(new_n162_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x16), .O(new_n171_));
  inv1   g120(.a(x03), .O(new_n172_));
  aoi21  g121(.a(new_n61_), .b(new_n172_), .c(x18), .O(new_n173_));
  aoi21  g122(.a(new_n173_), .b(new_n171_), .c(new_n54_), .O(z12));
  nand4  g123(.a(new_n168_), .b(new_n127_), .c(new_n106_), .d(x32), .O(new_n175_));
  inv1   g124(.a(x32), .O(new_n176_));
  nand2  g125(.a(new_n101_), .b(new_n69_), .O(new_n177_));
  nand3  g126(.a(new_n168_), .b(new_n127_), .c(new_n100_), .O(new_n178_));
  oai21  g127(.a(new_n178_), .b(new_n177_), .c(new_n176_), .O(new_n179_));
  nand3  g128(.a(new_n179_), .b(new_n175_), .c(x16), .O(new_n180_));
  nor2   g129(.a(x16), .b(x02), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n72_), .O(new_n182_));
  nand2  g131(.a(new_n182_), .b(new_n180_), .O(z13));
  nand2  g132(.a(x33), .b(x32), .O(new_n184_));
  nand4  g133(.a(new_n168_), .b(new_n53_), .c(new_n176_), .d(new_n132_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n126_), .c(new_n184_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(x16), .O(new_n187_));
  oai21  g136(.a(x16), .b(x01), .c(new_n52_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n55_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n187_), .O(z14));
  and2   g139(.a(x34), .b(x16), .O(new_n191_));
  oai21  g140(.a(new_n169_), .b(x32), .c(new_n191_), .O(new_n192_));
  nor2   g141(.a(x17), .b(new_n61_), .O(new_n193_));
  nor2   g142(.a(x34), .b(x31), .O(new_n194_));
  nand4  g143(.a(new_n194_), .b(new_n193_), .c(new_n155_), .d(new_n93_), .O(new_n195_));
  oai21  g144(.a(new_n195_), .b(new_n146_), .c(new_n53_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(new_n176_), .O(new_n197_));
  nand2  g146(.a(new_n52_), .b(x00), .O(new_n198_));
  oai21  g147(.a(new_n54_), .b(new_n52_), .c(x16), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n198_), .O(new_n200_));
  nand3  g149(.a(new_n200_), .b(new_n197_), .c(new_n192_), .O(z15));
endmodule


