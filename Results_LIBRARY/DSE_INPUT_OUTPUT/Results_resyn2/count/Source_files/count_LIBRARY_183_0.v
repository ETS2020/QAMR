// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_;
  inv1   g00(.a(x17), .O(new_n52_));
  inv1   g01(.a(x19), .O(new_n53_));
  nor2   g02(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g03(.a(x19), .b(x17), .O(new_n55_));
  oai21  g04(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g05(.a(x15), .O(new_n57_));
  inv1   g06(.a(x16), .O(new_n58_));
  nand2  g07(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g08(.a(x22), .b(x19), .O(new_n60_));
  nor2   g09(.a(new_n60_), .b(x18), .O(new_n61_));
  nand3  g10(.a(new_n61_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g11(.a(x20), .O(new_n63_));
  nand3  g12(.a(new_n63_), .b(new_n52_), .c(x16), .O(new_n64_));
  nand2  g13(.a(new_n64_), .b(x22), .O(new_n65_));
  nand2  g14(.a(new_n65_), .b(new_n53_), .O(new_n66_));
  inv1   g15(.a(new_n55_), .O(new_n67_));
  nor2   g16(.a(new_n63_), .b(new_n58_), .O(new_n68_));
  inv1   g17(.a(x18), .O(new_n69_));
  oai21  g18(.a(x16), .b(x14), .c(new_n69_), .O(new_n70_));
  aoi21  g19(.a(new_n68_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  nand2  g20(.a(new_n71_), .b(new_n66_), .O(z01));
  inv1   g21(.a(x21), .O(new_n73_));
  nand4  g22(.a(new_n73_), .b(new_n63_), .c(new_n52_), .d(x16), .O(new_n74_));
  nand2  g23(.a(new_n74_), .b(x22), .O(new_n75_));
  nand2  g24(.a(new_n75_), .b(new_n53_), .O(new_n76_));
  nand3  g25(.a(new_n55_), .b(x22), .c(new_n63_), .O(new_n77_));
  nor2   g26(.a(new_n73_), .b(new_n58_), .O(new_n78_));
  oai21  g27(.a(x16), .b(x13), .c(new_n69_), .O(new_n79_));
  aoi21  g28(.a(new_n78_), .b(new_n77_), .c(new_n79_), .O(new_n80_));
  nand2  g29(.a(new_n80_), .b(new_n76_), .O(z02));
  inv1   g30(.a(x12), .O(new_n82_));
  aoi21  g31(.a(new_n58_), .b(new_n82_), .c(x18), .O(new_n83_));
  nand3  g32(.a(new_n55_), .b(new_n73_), .c(new_n63_), .O(new_n84_));
  nand3  g33(.a(new_n84_), .b(x22), .c(x16), .O(new_n85_));
  oai21  g34(.a(new_n83_), .b(new_n60_), .c(new_n85_), .O(z03));
  nand2  g35(.a(x23), .b(x16), .O(new_n87_));
  inv1   g36(.a(x11), .O(new_n88_));
  aoi21  g37(.a(new_n58_), .b(new_n88_), .c(x18), .O(new_n89_));
  aoi21  g38(.a(new_n89_), .b(new_n87_), .c(new_n60_), .O(z04));
  nand2  g39(.a(x24), .b(x16), .O(new_n91_));
  inv1   g40(.a(x10), .O(new_n92_));
  aoi21  g41(.a(new_n58_), .b(new_n92_), .c(x18), .O(new_n93_));
  aoi21  g42(.a(new_n93_), .b(new_n91_), .c(new_n60_), .O(z05));
  nand2  g43(.a(x25), .b(x16), .O(new_n95_));
  inv1   g44(.a(x09), .O(new_n96_));
  aoi21  g45(.a(new_n58_), .b(new_n96_), .c(x18), .O(new_n97_));
  aoi21  g46(.a(new_n97_), .b(new_n95_), .c(new_n60_), .O(z06));
  inv1   g47(.a(x08), .O(new_n99_));
  nand2  g48(.a(new_n58_), .b(new_n99_), .O(new_n100_));
  nand2  g49(.a(x26), .b(x16), .O(new_n101_));
  nand3  g50(.a(new_n101_), .b(new_n100_), .c(new_n61_), .O(z07));
  nand2  g51(.a(x27), .b(x16), .O(new_n103_));
  inv1   g52(.a(x07), .O(new_n104_));
  aoi21  g53(.a(new_n58_), .b(new_n104_), .c(x18), .O(new_n105_));
  aoi21  g54(.a(new_n105_), .b(new_n103_), .c(new_n60_), .O(z08));
  nand2  g55(.a(x28), .b(x16), .O(new_n107_));
  inv1   g56(.a(x06), .O(new_n108_));
  aoi21  g57(.a(new_n58_), .b(new_n108_), .c(x18), .O(new_n109_));
  aoi21  g58(.a(new_n109_), .b(new_n107_), .c(new_n60_), .O(z09));
  nand2  g59(.a(x29), .b(x16), .O(new_n111_));
  inv1   g60(.a(x05), .O(new_n112_));
  aoi21  g61(.a(new_n58_), .b(new_n112_), .c(x18), .O(new_n113_));
  aoi21  g62(.a(new_n113_), .b(new_n111_), .c(new_n60_), .O(z10));
  inv1   g63(.a(x04), .O(new_n115_));
  nand2  g64(.a(new_n58_), .b(new_n115_), .O(new_n116_));
  nand2  g65(.a(x30), .b(x16), .O(new_n117_));
  nand3  g66(.a(new_n117_), .b(new_n116_), .c(new_n61_), .O(z11));
  inv1   g67(.a(x03), .O(new_n119_));
  nand2  g68(.a(new_n58_), .b(new_n119_), .O(new_n120_));
  nand2  g69(.a(x31), .b(x16), .O(new_n121_));
  nand3  g70(.a(new_n121_), .b(new_n120_), .c(new_n61_), .O(z12));
  inv1   g71(.a(x02), .O(new_n123_));
  nand2  g72(.a(new_n58_), .b(new_n123_), .O(new_n124_));
  nand2  g73(.a(x32), .b(x16), .O(new_n125_));
  nand3  g74(.a(new_n125_), .b(new_n124_), .c(new_n61_), .O(z13));
  inv1   g75(.a(x01), .O(new_n127_));
  nand2  g76(.a(new_n58_), .b(new_n127_), .O(new_n128_));
  nand2  g77(.a(x33), .b(x16), .O(new_n129_));
  nand3  g78(.a(new_n129_), .b(new_n128_), .c(new_n61_), .O(z14));
  nand2  g79(.a(x34), .b(x16), .O(new_n131_));
  inv1   g80(.a(x00), .O(new_n132_));
  aoi21  g81(.a(new_n58_), .b(new_n132_), .c(x18), .O(new_n133_));
  aoi21  g82(.a(new_n133_), .b(new_n131_), .c(new_n60_), .O(z15));
endmodule


