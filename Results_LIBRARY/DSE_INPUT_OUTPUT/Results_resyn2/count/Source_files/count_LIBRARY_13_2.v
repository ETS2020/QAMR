// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x17), .O(new_n54_));
  inv1   g003(.a(x19), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  aoi21  g006(.a(new_n57_), .b(x15), .c(x09), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n52_), .O(z00));
  inv1   g009(.a(x20), .O(new_n61_));
  nor2   g010(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  oai21  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  nor2   g013(.a(x16), .b(x14), .O(new_n65_));
  nor2   g014(.a(x18), .b(x09), .O(new_n66_));
  inv1   g015(.a(new_n66_), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n64_), .O(z01));
  nand3  g018(.a(new_n61_), .b(new_n55_), .c(new_n54_), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  aoi22  g020(.a(new_n71_), .b(new_n53_), .c(new_n70_), .d(x21), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  nor2   g022(.a(new_n73_), .b(new_n67_), .O(new_n74_));
  oai21  g023(.a(new_n72_), .b(new_n57_), .c(new_n74_), .O(z02));
  nor2   g024(.a(x22), .b(x21), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n53_), .c(new_n61_), .O(new_n77_));
  nand2  g026(.a(new_n71_), .b(new_n53_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(x22), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n77_), .c(x16), .O(new_n80_));
  aoi21  g029(.a(new_n57_), .b(x12), .c(x09), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n52_), .O(z03));
  inv1   g032(.a(x23), .O(new_n84_));
  inv1   g033(.a(new_n77_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g035(.a(new_n77_), .b(x23), .c(new_n57_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g037(.a(new_n57_), .b(x11), .c(x09), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n52_), .O(z04));
  nor2   g040(.a(x24), .b(x23), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n76_), .c(new_n53_), .d(new_n61_), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  aoi21  g043(.a(new_n86_), .b(x24), .c(new_n94_), .O(new_n95_));
  nor2   g044(.a(x16), .b(x10), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n67_), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n57_), .c(new_n97_), .O(z05));
  nand2  g047(.a(new_n93_), .b(x25), .O(new_n99_));
  nor2   g048(.a(x25), .b(x22), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n92_), .c(new_n71_), .d(new_n53_), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n99_), .c(new_n66_), .d(x16), .O(z06));
  nor2   g051(.a(x26), .b(x25), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n92_), .O(new_n104_));
  nor2   g053(.a(new_n104_), .b(new_n77_), .O(new_n105_));
  aoi21  g054(.a(new_n101_), .b(x26), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x08), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n67_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n57_), .c(new_n108_), .O(z07));
  inv1   g058(.a(x27), .O(new_n110_));
  nor3   g059(.a(x27), .b(x26), .c(x25), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n92_), .c(new_n76_), .d(new_n63_), .O(new_n112_));
  oai21  g061(.a(new_n105_), .b(new_n110_), .c(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x16), .O(new_n114_));
  nor2   g063(.a(x16), .b(x07), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n67_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n114_), .O(z08));
  nand2  g066(.a(new_n112_), .b(x28), .O(new_n118_));
  nor2   g067(.a(x28), .b(x27), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n105_), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n118_), .c(x16), .O(new_n121_));
  aoi21  g070(.a(new_n57_), .b(x06), .c(x09), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n52_), .O(z09));
  xor2a  g073(.a(new_n120_), .b(x29), .O(new_n125_));
  nor2   g074(.a(x16), .b(x05), .O(new_n126_));
  nor2   g075(.a(new_n126_), .b(new_n67_), .O(new_n127_));
  oai21  g076(.a(new_n125_), .b(new_n57_), .c(new_n127_), .O(z10));
  inv1   g077(.a(new_n119_), .O(new_n129_));
  nor3   g078(.a(new_n129_), .b(new_n104_), .c(new_n77_), .O(new_n130_));
  inv1   g079(.a(x29), .O(new_n131_));
  inv1   g080(.a(new_n104_), .O(new_n132_));
  nand4  g081(.a(new_n119_), .b(new_n132_), .c(new_n85_), .d(new_n131_), .O(new_n133_));
  nor2   g082(.a(x30), .b(x29), .O(new_n134_));
  aoi22  g083(.a(new_n134_), .b(new_n130_), .c(new_n133_), .d(x30), .O(new_n135_));
  nor2   g084(.a(x16), .b(x04), .O(new_n136_));
  nor2   g085(.a(new_n136_), .b(new_n67_), .O(new_n137_));
  oai21  g086(.a(new_n135_), .b(new_n57_), .c(new_n137_), .O(z11));
  nand3  g087(.a(new_n134_), .b(new_n119_), .c(new_n105_), .O(new_n139_));
  nor2   g088(.a(x31), .b(x28), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(new_n134_), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n112_), .O(new_n142_));
  aoi21  g091(.a(new_n139_), .b(x31), .c(new_n142_), .O(new_n143_));
  nor2   g092(.a(x16), .b(x03), .O(new_n144_));
  nor2   g093(.a(new_n144_), .b(new_n67_), .O(new_n145_));
  oai21  g094(.a(new_n143_), .b(new_n57_), .c(new_n145_), .O(z12));
  inv1   g095(.a(new_n141_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n111_), .c(new_n94_), .O(new_n148_));
  nor2   g097(.a(x32), .b(x31), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n134_), .c(new_n119_), .d(new_n103_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n93_), .c(x16), .O(new_n151_));
  aoi21  g100(.a(new_n148_), .b(x32), .c(new_n151_), .O(new_n152_));
  inv1   g101(.a(x02), .O(new_n153_));
  inv1   g102(.a(x09), .O(new_n154_));
  oai21  g103(.a(x16), .b(new_n153_), .c(new_n154_), .O(new_n155_));
  oai21  g104(.a(new_n155_), .b(new_n152_), .c(new_n52_), .O(z13));
  oai21  g105(.a(new_n150_), .b(new_n93_), .c(x33), .O(new_n157_));
  nor2   g106(.a(x27), .b(x26), .O(new_n158_));
  nor2   g107(.a(x33), .b(x32), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n140_), .c(new_n134_), .d(new_n158_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n101_), .O(new_n161_));
  inv1   g110(.a(new_n161_), .O(new_n162_));
  nand3  g111(.a(new_n162_), .b(new_n157_), .c(x16), .O(new_n163_));
  aoi21  g112(.a(new_n57_), .b(x01), .c(x09), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n52_), .O(z14));
  inv1   g115(.a(x34), .O(new_n167_));
  nand2  g116(.a(new_n161_), .b(new_n167_), .O(new_n168_));
  oai21  g117(.a(new_n160_), .b(new_n101_), .c(x34), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n168_), .c(x16), .O(new_n170_));
  aoi21  g119(.a(new_n57_), .b(x00), .c(x09), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n52_), .O(z15));
endmodule


