// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_;
  inv1   g000(.a(x16), .O(new_n52_));
  xor2a  g001(.a(x19), .b(x17), .O(new_n53_));
  inv1   g002(.a(x15), .O(new_n54_));
  aoi21  g003(.a(new_n52_), .b(new_n54_), .c(x18), .O(new_n55_));
  oai21  g004(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z00));
  nor2   g005(.a(x18), .b(x16), .O(new_n57_));
  inv1   g006(.a(new_n57_), .O(new_n58_));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(x19), .b(x17), .O(new_n60_));
  nor2   g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  inv1   g010(.a(x18), .O(new_n62_));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  oai21  g014(.a(new_n65_), .b(new_n61_), .c(x16), .O(new_n66_));
  oai21  g015(.a(new_n58_), .b(x14), .c(new_n66_), .O(z01));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n60_), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  aoi21  g019(.a(new_n64_), .b(x21), .c(new_n70_), .O(new_n71_));
  inv1   g020(.a(x13), .O(new_n72_));
  aoi21  g021(.a(new_n52_), .b(new_n72_), .c(x18), .O(new_n73_));
  oai21  g022(.a(new_n71_), .b(new_n52_), .c(new_n73_), .O(z02));
  inv1   g023(.a(x12), .O(new_n75_));
  nand2  g024(.a(new_n57_), .b(new_n75_), .O(new_n76_));
  inv1   g025(.a(x22), .O(new_n77_));
  nor2   g026(.a(new_n70_), .b(new_n77_), .O(new_n78_));
  nand3  g027(.a(new_n68_), .b(new_n60_), .c(new_n77_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n62_), .O(new_n80_));
  oai21  g029(.a(new_n80_), .b(new_n78_), .c(x16), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n76_), .O(z03));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n68_), .c(new_n60_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n62_), .O(new_n85_));
  aoi21  g034(.a(new_n79_), .b(x23), .c(new_n85_), .O(new_n86_));
  oai22  g035(.a(new_n86_), .b(new_n52_), .c(new_n58_), .d(x11), .O(z04));
  nor2   g036(.a(x24), .b(x23), .O(new_n88_));
  nor2   g037(.a(x22), .b(x21), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n88_), .c(new_n60_), .d(new_n59_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n62_), .O(new_n91_));
  aoi21  g040(.a(new_n84_), .b(x24), .c(new_n91_), .O(new_n92_));
  oai22  g041(.a(new_n92_), .b(new_n52_), .c(new_n58_), .d(x10), .O(z05));
  nor2   g042(.a(x25), .b(x24), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n83_), .c(new_n68_), .d(new_n60_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n62_), .O(new_n96_));
  aoi21  g045(.a(new_n90_), .b(x25), .c(new_n96_), .O(new_n97_));
  oai22  g046(.a(new_n97_), .b(new_n52_), .c(new_n58_), .d(x09), .O(z06));
  inv1   g047(.a(x08), .O(new_n99_));
  nand2  g048(.a(new_n57_), .b(new_n99_), .O(new_n100_));
  nand2  g049(.a(new_n95_), .b(x26), .O(new_n101_));
  inv1   g050(.a(new_n84_), .O(new_n102_));
  inv1   g051(.a(x24), .O(new_n103_));
  nor2   g052(.a(x26), .b(x25), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  inv1   g054(.a(new_n105_), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n102_), .O(new_n107_));
  nand3  g056(.a(new_n107_), .b(new_n101_), .c(new_n62_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(x16), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n100_), .O(z07));
  inv1   g059(.a(x27), .O(new_n111_));
  nor2   g060(.a(new_n105_), .b(new_n84_), .O(new_n112_));
  nor3   g061(.a(x27), .b(x26), .c(x25), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n89_), .c(new_n88_), .d(new_n63_), .O(new_n114_));
  oai21  g063(.a(new_n112_), .b(new_n111_), .c(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  inv1   g065(.a(x07), .O(new_n117_));
  aoi21  g066(.a(new_n52_), .b(new_n117_), .c(x18), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n116_), .O(z08));
  nor2   g068(.a(x28), .b(x27), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n104_), .c(new_n88_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n79_), .O(new_n122_));
  aoi21  g071(.a(new_n114_), .b(x28), .c(new_n122_), .O(new_n123_));
  inv1   g072(.a(x06), .O(new_n124_));
  aoi21  g073(.a(new_n52_), .b(new_n124_), .c(x18), .O(new_n125_));
  oai21  g074(.a(new_n123_), .b(new_n52_), .c(new_n125_), .O(z09));
  oai21  g075(.a(new_n121_), .b(new_n79_), .c(x29), .O(new_n127_));
  nor3   g076(.a(x29), .b(x28), .c(x27), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n106_), .c(new_n102_), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n127_), .c(new_n62_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(x16), .O(new_n131_));
  inv1   g080(.a(x05), .O(new_n132_));
  nand2  g081(.a(new_n57_), .b(new_n132_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n131_), .O(z10));
  inv1   g083(.a(x29), .O(new_n135_));
  nand4  g084(.a(new_n120_), .b(new_n104_), .c(new_n135_), .d(new_n103_), .O(new_n136_));
  oai21  g085(.a(new_n136_), .b(new_n84_), .c(x30), .O(new_n137_));
  nor2   g086(.a(x30), .b(x29), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n120_), .O(new_n139_));
  inv1   g088(.a(new_n139_), .O(new_n140_));
  nand3  g089(.a(new_n140_), .b(new_n106_), .c(new_n102_), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n137_), .c(new_n62_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(x16), .O(new_n143_));
  inv1   g092(.a(x04), .O(new_n144_));
  nand2  g093(.a(new_n57_), .b(new_n144_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n143_), .O(z11));
  inv1   g095(.a(x25), .O(new_n147_));
  nor2   g096(.a(x27), .b(x26), .O(new_n148_));
  nor2   g097(.a(x31), .b(x28), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n138_), .c(new_n148_), .d(new_n147_), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n90_), .c(new_n62_), .O(new_n151_));
  aoi21  g100(.a(new_n141_), .b(x31), .c(new_n151_), .O(new_n152_));
  inv1   g101(.a(x03), .O(new_n153_));
  nand2  g102(.a(new_n57_), .b(new_n153_), .O(new_n154_));
  oai21  g103(.a(new_n152_), .b(new_n52_), .c(new_n154_), .O(z12));
  oai21  g104(.a(new_n150_), .b(new_n90_), .c(x32), .O(new_n156_));
  nand2  g105(.a(new_n89_), .b(new_n88_), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  nor2   g107(.a(x32), .b(x31), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n104_), .O(new_n160_));
  inv1   g109(.a(new_n160_), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n140_), .c(new_n158_), .d(new_n63_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n156_), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(x16), .O(new_n164_));
  inv1   g113(.a(x02), .O(new_n165_));
  aoi21  g114(.a(new_n52_), .b(new_n165_), .c(x18), .O(new_n166_));
  nand2  g115(.a(new_n166_), .b(new_n164_), .O(z13));
  nand4  g116(.a(new_n159_), .b(new_n138_), .c(new_n120_), .d(new_n104_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n90_), .c(x33), .O(new_n169_));
  nand2  g118(.a(new_n94_), .b(new_n83_), .O(new_n170_));
  inv1   g119(.a(new_n170_), .O(new_n171_));
  nand2  g120(.a(new_n149_), .b(new_n138_), .O(new_n172_));
  inv1   g121(.a(new_n172_), .O(new_n173_));
  nor2   g122(.a(x33), .b(x32), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n148_), .O(new_n175_));
  inv1   g124(.a(new_n175_), .O(new_n176_));
  nand4  g125(.a(new_n176_), .b(new_n173_), .c(new_n171_), .d(new_n70_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n169_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(x16), .O(new_n179_));
  inv1   g128(.a(x01), .O(new_n180_));
  aoi21  g129(.a(new_n52_), .b(new_n180_), .c(x18), .O(new_n181_));
  nand2  g130(.a(new_n181_), .b(new_n179_), .O(z14));
  inv1   g131(.a(x34), .O(new_n183_));
  nor2   g132(.a(new_n177_), .b(new_n183_), .O(new_n184_));
  nand4  g133(.a(new_n174_), .b(new_n149_), .c(new_n138_), .d(new_n148_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n95_), .c(new_n183_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(x16), .O(new_n187_));
  inv1   g136(.a(x00), .O(new_n188_));
  aoi21  g137(.a(new_n52_), .b(new_n188_), .c(x18), .O(new_n189_));
  oai21  g138(.a(new_n187_), .b(new_n184_), .c(new_n189_), .O(z15));
endmodule


