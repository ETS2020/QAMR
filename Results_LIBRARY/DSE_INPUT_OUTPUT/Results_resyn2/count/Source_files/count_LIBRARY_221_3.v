// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_;
  inv1   g000(.a(x22), .O(new_n52_));
  nand2  g001(.a(new_n52_), .b(x18), .O(new_n53_));
  inv1   g002(.a(x18), .O(new_n54_));
  nand3  g003(.a(x19), .b(x17), .c(x16), .O(new_n55_));
  nor2   g004(.a(x19), .b(x17), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(x16), .O(new_n57_));
  inv1   g006(.a(x15), .O(new_n58_));
  inv1   g007(.a(x16), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n57_), .c(new_n55_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n53_), .O(z00));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  oai21  g014(.a(new_n56_), .b(new_n65_), .c(x16), .O(new_n66_));
  aoi21  g015(.a(new_n59_), .b(x14), .c(x18), .O(new_n67_));
  oai21  g016(.a(new_n66_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n53_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nor2   g019(.a(new_n64_), .b(new_n70_), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n56_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(x16), .O(new_n74_));
  aoi21  g023(.a(new_n59_), .b(x13), .c(x18), .O(new_n75_));
  oai21  g024(.a(new_n74_), .b(new_n71_), .c(new_n75_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n53_), .O(z02));
  nand3  g026(.a(new_n72_), .b(new_n56_), .c(x16), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n54_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n52_), .O(new_n80_));
  nor2   g029(.a(x16), .b(x12), .O(new_n81_));
  inv1   g030(.a(new_n73_), .O(new_n82_));
  nand2  g031(.a(x22), .b(x16), .O(new_n83_));
  nor2   g032(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n81_), .c(new_n54_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n80_), .O(z03));
  nand3  g035(.a(new_n72_), .b(new_n56_), .c(new_n52_), .O(new_n87_));
  nor3   g036(.a(x23), .b(x22), .c(x21), .O(new_n88_));
  aoi22  g037(.a(new_n88_), .b(new_n64_), .c(new_n87_), .d(x23), .O(new_n89_));
  inv1   g038(.a(x11), .O(new_n90_));
  aoi21  g039(.a(new_n59_), .b(new_n90_), .c(x18), .O(new_n91_));
  oai21  g040(.a(new_n89_), .b(new_n59_), .c(new_n91_), .O(z04));
  inv1   g041(.a(x24), .O(new_n93_));
  aoi21  g042(.a(new_n88_), .b(new_n64_), .c(new_n93_), .O(new_n94_));
  nor2   g043(.a(x23), .b(x22), .O(new_n95_));
  nor2   g044(.a(x24), .b(x21), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n95_), .c(new_n56_), .d(new_n65_), .O(new_n97_));
  inv1   g046(.a(new_n97_), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(new_n94_), .c(x16), .O(new_n99_));
  inv1   g048(.a(x10), .O(new_n100_));
  aoi21  g049(.a(new_n59_), .b(new_n100_), .c(x18), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n99_), .O(z05));
  nor2   g051(.a(x25), .b(x24), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n95_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  aoi22  g054(.a(new_n105_), .b(new_n82_), .c(new_n97_), .d(x25), .O(new_n106_));
  inv1   g055(.a(x09), .O(new_n107_));
  aoi21  g056(.a(new_n59_), .b(new_n107_), .c(x18), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n59_), .c(new_n108_), .O(z06));
  nor2   g058(.a(x24), .b(x23), .O(new_n110_));
  nand4  g059(.a(new_n110_), .b(new_n72_), .c(new_n56_), .d(x16), .O(new_n111_));
  nor3   g060(.a(new_n111_), .b(x26), .c(x25), .O(new_n112_));
  oai21  g061(.a(new_n112_), .b(x18), .c(new_n52_), .O(new_n113_));
  nand4  g062(.a(new_n103_), .b(new_n95_), .c(new_n72_), .d(new_n56_), .O(new_n114_));
  nand3  g063(.a(new_n114_), .b(x26), .c(x16), .O(new_n115_));
  oai21  g064(.a(x16), .b(x08), .c(new_n115_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n54_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n113_), .O(z07));
  inv1   g067(.a(x27), .O(new_n119_));
  inv1   g068(.a(x23), .O(new_n120_));
  nand4  g069(.a(new_n93_), .b(new_n120_), .c(new_n52_), .d(new_n70_), .O(new_n121_));
  inv1   g070(.a(new_n121_), .O(new_n122_));
  nor2   g071(.a(x26), .b(x25), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n122_), .c(new_n64_), .d(new_n119_), .O(new_n124_));
  nand2  g073(.a(new_n123_), .b(new_n93_), .O(new_n125_));
  nand3  g074(.a(new_n95_), .b(new_n72_), .c(new_n56_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n125_), .c(x27), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(x16), .O(new_n129_));
  inv1   g078(.a(x07), .O(new_n130_));
  aoi21  g079(.a(new_n59_), .b(new_n130_), .c(x18), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n129_), .O(z08));
  nor2   g081(.a(x28), .b(x27), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n110_), .c(new_n123_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(new_n78_), .c(new_n54_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n52_), .O(new_n136_));
  and2   g085(.a(x28), .b(x16), .O(new_n137_));
  nor2   g086(.a(x16), .b(x06), .O(new_n138_));
  aoi21  g087(.a(new_n137_), .b(new_n124_), .c(new_n138_), .O(new_n139_));
  oai21  g088(.a(new_n139_), .b(x18), .c(new_n136_), .O(z09));
  nor3   g089(.a(x26), .b(x25), .c(x24), .O(new_n141_));
  nor3   g090(.a(x29), .b(x28), .c(x27), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand4  g092(.a(new_n72_), .b(new_n56_), .c(new_n120_), .d(x16), .O(new_n144_));
  oai21  g093(.a(new_n144_), .b(new_n143_), .c(new_n54_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n52_), .O(new_n146_));
  and2   g095(.a(x29), .b(x16), .O(new_n147_));
  oai21  g096(.a(new_n134_), .b(new_n87_), .c(new_n147_), .O(new_n148_));
  oai21  g097(.a(x16), .b(x05), .c(new_n148_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n54_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n146_), .O(z10));
  nand4  g100(.a(new_n142_), .b(new_n141_), .c(new_n88_), .d(new_n64_), .O(new_n152_));
  nor2   g101(.a(x22), .b(x21), .O(new_n153_));
  nand4  g102(.a(new_n153_), .b(new_n56_), .c(new_n120_), .d(new_n65_), .O(new_n154_));
  nor2   g103(.a(x30), .b(x29), .O(new_n155_));
  nand4  g104(.a(new_n155_), .b(new_n133_), .c(new_n123_), .d(new_n93_), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n154_), .O(new_n157_));
  aoi21  g106(.a(new_n152_), .b(x30), .c(new_n157_), .O(new_n158_));
  inv1   g107(.a(x04), .O(new_n159_));
  aoi21  g108(.a(new_n59_), .b(new_n159_), .c(x18), .O(new_n160_));
  oai21  g109(.a(new_n158_), .b(new_n59_), .c(new_n160_), .O(z11));
  oai21  g110(.a(new_n156_), .b(new_n154_), .c(x31), .O(new_n162_));
  nor2   g111(.a(x29), .b(x26), .O(new_n163_));
  nand2  g112(.a(new_n163_), .b(new_n133_), .O(new_n164_));
  inv1   g113(.a(new_n164_), .O(new_n165_));
  nor3   g114(.a(x31), .b(x30), .c(x25), .O(new_n166_));
  nand3  g115(.a(new_n166_), .b(new_n165_), .c(new_n98_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n162_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(x16), .O(new_n169_));
  inv1   g118(.a(x03), .O(new_n170_));
  aoi21  g119(.a(new_n59_), .b(new_n170_), .c(x18), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n169_), .O(z12));
  nor2   g121(.a(x32), .b(x31), .O(new_n173_));
  nand4  g122(.a(new_n173_), .b(new_n155_), .c(new_n133_), .d(new_n123_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n111_), .c(new_n54_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n52_), .O(new_n176_));
  inv1   g125(.a(x31), .O(new_n177_));
  nor2   g126(.a(x30), .b(x25), .O(new_n178_));
  nand4  g127(.a(new_n178_), .b(new_n163_), .c(new_n133_), .d(new_n177_), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n97_), .O(new_n180_));
  nand2  g129(.a(x32), .b(x16), .O(new_n181_));
  inv1   g130(.a(x02), .O(new_n182_));
  nand2  g131(.a(new_n59_), .b(new_n182_), .O(new_n183_));
  oai21  g132(.a(new_n181_), .b(new_n180_), .c(new_n183_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n54_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n176_), .O(z13));
  oai21  g135(.a(new_n174_), .b(new_n97_), .c(x33), .O(new_n187_));
  nor2   g136(.a(x33), .b(x30), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n173_), .O(new_n189_));
  inv1   g138(.a(new_n189_), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n165_), .c(new_n105_), .d(new_n82_), .O(new_n191_));
  nand2  g140(.a(new_n191_), .b(new_n187_), .O(new_n192_));
  nand2  g141(.a(new_n192_), .b(x16), .O(new_n193_));
  inv1   g142(.a(x01), .O(new_n194_));
  aoi21  g143(.a(new_n59_), .b(new_n194_), .c(x18), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n193_), .O(z14));
  nand4  g145(.a(new_n188_), .b(new_n173_), .c(new_n163_), .d(new_n133_), .O(new_n197_));
  inv1   g146(.a(x34), .O(new_n198_));
  nand4  g147(.a(new_n103_), .b(new_n198_), .c(new_n120_), .d(x16), .O(new_n199_));
  nor3   g148(.a(new_n199_), .b(new_n197_), .c(new_n73_), .O(new_n200_));
  oai21  g149(.a(new_n200_), .b(x18), .c(new_n52_), .O(new_n201_));
  nor2   g150(.a(new_n197_), .b(new_n114_), .O(new_n202_));
  nand2  g151(.a(x34), .b(x16), .O(new_n203_));
  inv1   g152(.a(x00), .O(new_n204_));
  nand2  g153(.a(new_n59_), .b(new_n204_), .O(new_n205_));
  oai21  g154(.a(new_n203_), .b(new_n202_), .c(new_n205_), .O(new_n206_));
  nand2  g155(.a(new_n206_), .b(new_n54_), .O(new_n207_));
  nand2  g156(.a(new_n207_), .b(new_n201_), .O(z15));
endmodule


