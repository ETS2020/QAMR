// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x28), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g008(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n54_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n53_), .b(new_n62_), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  nor2   g014(.a(x16), .b(x14), .O(new_n66_));
  nor2   g015(.a(new_n66_), .b(new_n59_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n65_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nor2   g018(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  nor2   g019(.a(x21), .b(x20), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n53_), .O(new_n72_));
  inv1   g021(.a(new_n72_), .O(new_n73_));
  oai21  g022(.a(new_n73_), .b(new_n70_), .c(x16), .O(new_n74_));
  nor2   g023(.a(x16), .b(x13), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(new_n59_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n74_), .O(z02));
  inv1   g026(.a(x22), .O(new_n78_));
  nand3  g027(.a(new_n71_), .b(new_n53_), .c(new_n78_), .O(new_n79_));
  nand2  g028(.a(new_n72_), .b(x22), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n79_), .c(x16), .O(new_n81_));
  aoi21  g030(.a(new_n56_), .b(x12), .c(x28), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n57_), .O(z03));
  nand2  g033(.a(new_n79_), .b(x23), .O(new_n85_));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n71_), .c(new_n53_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(x16), .O(new_n88_));
  aoi21  g037(.a(new_n56_), .b(x11), .c(x28), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n57_), .O(z04));
  nor2   g040(.a(x24), .b(x21), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n86_), .O(new_n93_));
  inv1   g042(.a(new_n93_), .O(new_n94_));
  aoi22  g043(.a(new_n94_), .b(new_n64_), .c(new_n87_), .d(x24), .O(new_n95_));
  nor2   g044(.a(x16), .b(x10), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n59_), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n56_), .c(new_n97_), .O(z05));
  nor2   g047(.a(x25), .b(x24), .O(new_n99_));
  nand4  g048(.a(new_n99_), .b(new_n86_), .c(new_n71_), .d(new_n53_), .O(new_n100_));
  nand4  g049(.a(new_n92_), .b(new_n86_), .c(new_n53_), .d(new_n62_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(x25), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n100_), .c(x16), .O(new_n103_));
  aoi21  g052(.a(new_n56_), .b(x09), .c(x28), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n57_), .O(z06));
  nand2  g055(.a(new_n100_), .b(x26), .O(new_n107_));
  inv1   g056(.a(x26), .O(new_n108_));
  nand2  g057(.a(new_n99_), .b(new_n108_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n87_), .O(new_n110_));
  inv1   g059(.a(new_n110_), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n107_), .c(x16), .O(new_n112_));
  aoi21  g061(.a(new_n56_), .b(x08), .c(x28), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n57_), .O(z07));
  inv1   g064(.a(x27), .O(new_n116_));
  nor2   g065(.a(new_n110_), .b(new_n116_), .O(new_n117_));
  inv1   g066(.a(x25), .O(new_n118_));
  nand3  g067(.a(new_n116_), .b(new_n108_), .c(new_n118_), .O(new_n119_));
  inv1   g068(.a(new_n119_), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n94_), .c(new_n64_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(x16), .O(new_n122_));
  aoi21  g071(.a(new_n56_), .b(x07), .c(x28), .O(new_n123_));
  oai21  g072(.a(new_n122_), .b(new_n117_), .c(new_n123_), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n57_), .O(z08));
  inv1   g074(.a(new_n87_), .O(new_n126_));
  nor2   g075(.a(x24), .b(new_n56_), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n120_), .c(new_n126_), .O(new_n128_));
  nor2   g077(.a(x16), .b(x06), .O(new_n129_));
  nor2   g078(.a(new_n129_), .b(new_n59_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n128_), .O(z09));
  inv1   g080(.a(x23), .O(new_n132_));
  nor2   g081(.a(x29), .b(x27), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n99_), .c(new_n108_), .d(new_n132_), .O(new_n134_));
  nor2   g083(.a(new_n134_), .b(new_n79_), .O(new_n135_));
  aoi21  g084(.a(new_n121_), .b(x29), .c(new_n135_), .O(new_n136_));
  nor2   g085(.a(x16), .b(x05), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n59_), .O(new_n138_));
  oai21  g087(.a(new_n136_), .b(new_n56_), .c(new_n138_), .O(z10));
  inv1   g088(.a(x30), .O(new_n140_));
  inv1   g089(.a(new_n109_), .O(new_n141_));
  nand4  g090(.a(new_n133_), .b(new_n141_), .c(new_n126_), .d(new_n140_), .O(new_n142_));
  oai21  g091(.a(new_n134_), .b(new_n79_), .c(x30), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n142_), .c(x16), .O(new_n144_));
  aoi21  g093(.a(new_n56_), .b(x04), .c(x28), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n57_), .O(z11));
  nor2   g096(.a(x31), .b(x30), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n133_), .O(new_n149_));
  nor3   g098(.a(new_n149_), .b(new_n109_), .c(new_n87_), .O(new_n150_));
  aoi21  g099(.a(new_n142_), .b(x31), .c(new_n150_), .O(new_n151_));
  nor2   g100(.a(x16), .b(x03), .O(new_n152_));
  nor2   g101(.a(new_n152_), .b(new_n59_), .O(new_n153_));
  oai21  g102(.a(new_n151_), .b(new_n56_), .c(new_n153_), .O(z12));
  nor2   g103(.a(new_n119_), .b(new_n101_), .O(new_n155_));
  inv1   g104(.a(new_n149_), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n141_), .c(new_n126_), .O(new_n157_));
  nor2   g106(.a(x32), .b(x29), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n148_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  aoi22  g109(.a(new_n160_), .b(new_n155_), .c(new_n157_), .d(x32), .O(new_n161_));
  nor2   g110(.a(x16), .b(x02), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n59_), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n56_), .c(new_n163_), .O(z13));
  nand4  g113(.a(new_n160_), .b(new_n120_), .c(new_n94_), .d(new_n64_), .O(new_n165_));
  nor2   g114(.a(x26), .b(x25), .O(new_n166_));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n166_), .c(new_n148_), .d(new_n133_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n101_), .c(x16), .O(new_n169_));
  aoi21  g118(.a(new_n165_), .b(x33), .c(new_n169_), .O(new_n170_));
  inv1   g119(.a(x01), .O(new_n171_));
  oai21  g120(.a(x16), .b(new_n171_), .c(new_n58_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n170_), .c(new_n57_), .O(z14));
  nor2   g122(.a(x34), .b(x33), .O(new_n174_));
  nand3  g123(.a(new_n174_), .b(new_n116_), .c(new_n108_), .O(new_n175_));
  nor3   g124(.a(new_n175_), .b(new_n159_), .c(new_n100_), .O(new_n176_));
  oai21  g125(.a(new_n168_), .b(new_n101_), .c(x34), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(x16), .O(new_n178_));
  aoi21  g127(.a(new_n56_), .b(x00), .c(x28), .O(new_n179_));
  oai21  g128(.a(new_n178_), .b(new_n176_), .c(new_n179_), .O(new_n180_));
  nand2  g129(.a(new_n180_), .b(new_n57_), .O(z15));
endmodule


