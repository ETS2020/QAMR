// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n156_;
  nor2   g000(.a(x19), .b(x17), .O(new_n52_));
  inv1   g001(.a(new_n52_), .O(new_n53_));
  inv1   g002(.a(x16), .O(new_n54_));
  nor2   g003(.a(x18), .b(new_n54_), .O(new_n55_));
  nand2  g004(.a(x19), .b(x17), .O(new_n56_));
  nand3  g005(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(z00));
  inv1   g006(.a(x18), .O(new_n58_));
  inv1   g007(.a(x20), .O(new_n59_));
  nor2   g008(.a(new_n52_), .b(new_n59_), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n58_), .O(z01));
  oai21  g012(.a(new_n53_), .b(x20), .c(x21), .O(new_n64_));
  nor2   g013(.a(x21), .b(x20), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n52_), .O(new_n66_));
  nand3  g015(.a(new_n66_), .b(new_n64_), .c(new_n55_), .O(z02));
  nand2  g016(.a(new_n66_), .b(x22), .O(new_n68_));
  inv1   g017(.a(x22), .O(new_n69_));
  nand3  g018(.a(new_n65_), .b(new_n52_), .c(new_n69_), .O(new_n70_));
  nand3  g019(.a(new_n70_), .b(new_n68_), .c(new_n55_), .O(z03));
  nand2  g020(.a(new_n70_), .b(x23), .O(new_n72_));
  nor2   g021(.a(x23), .b(x22), .O(new_n73_));
  nand3  g022(.a(new_n73_), .b(new_n65_), .c(new_n52_), .O(new_n74_));
  nand3  g023(.a(new_n74_), .b(new_n72_), .c(new_n55_), .O(z04));
  nand2  g024(.a(new_n74_), .b(x24), .O(new_n76_));
  nor2   g025(.a(x24), .b(x23), .O(new_n77_));
  nor2   g026(.a(x22), .b(x21), .O(new_n78_));
  nand4  g027(.a(new_n78_), .b(new_n77_), .c(new_n52_), .d(new_n59_), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n76_), .c(new_n55_), .O(z05));
  nand2  g029(.a(new_n79_), .b(x25), .O(new_n81_));
  nor2   g030(.a(x25), .b(x24), .O(new_n82_));
  nand4  g031(.a(new_n82_), .b(new_n73_), .c(new_n65_), .d(new_n52_), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n81_), .c(new_n55_), .O(z06));
  inv1   g033(.a(new_n74_), .O(new_n85_));
  nor3   g034(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g036(.a(new_n83_), .b(x26), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n87_), .c(new_n55_), .O(z07));
  inv1   g038(.a(x25), .O(new_n90_));
  nor2   g039(.a(x27), .b(x26), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g041(.a(new_n92_), .b(new_n79_), .O(new_n93_));
  aoi21  g042(.a(new_n87_), .b(x27), .c(new_n93_), .O(new_n94_));
  oai21  g043(.a(new_n94_), .b(new_n54_), .c(new_n58_), .O(z08));
  oai21  g044(.a(new_n92_), .b(new_n79_), .c(x28), .O(new_n96_));
  nor2   g045(.a(x26), .b(x25), .O(new_n97_));
  nor2   g046(.a(x28), .b(x27), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n97_), .c(new_n77_), .O(new_n99_));
  or2    g048(.a(new_n99_), .b(new_n70_), .O(new_n100_));
  nand3  g049(.a(new_n100_), .b(new_n96_), .c(new_n55_), .O(z09));
  oai21  g050(.a(new_n99_), .b(new_n70_), .c(x29), .O(new_n102_));
  nor3   g051(.a(x29), .b(x28), .c(x27), .O(new_n103_));
  nand3  g052(.a(new_n103_), .b(new_n86_), .c(new_n85_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(x16), .O(new_n106_));
  nand2  g055(.a(new_n106_), .b(new_n58_), .O(z10));
  inv1   g056(.a(x24), .O(new_n108_));
  inv1   g057(.a(x29), .O(new_n109_));
  nand4  g058(.a(new_n98_), .b(new_n97_), .c(new_n109_), .d(new_n108_), .O(new_n110_));
  oai21  g059(.a(new_n110_), .b(new_n74_), .c(x30), .O(new_n111_));
  inv1   g060(.a(x27), .O(new_n112_));
  inv1   g061(.a(x28), .O(new_n113_));
  inv1   g062(.a(x30), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n109_), .c(new_n113_), .d(new_n112_), .O(new_n115_));
  inv1   g064(.a(new_n115_), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n86_), .c(new_n85_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n111_), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(x16), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n58_), .O(z11));
  nor2   g069(.a(x30), .b(x29), .O(new_n121_));
  nand4  g070(.a(new_n121_), .b(new_n98_), .c(new_n97_), .d(new_n108_), .O(new_n122_));
  oai21  g071(.a(new_n122_), .b(new_n74_), .c(x31), .O(new_n123_));
  inv1   g072(.a(x21), .O(new_n124_));
  inv1   g073(.a(x23), .O(new_n125_));
  nand4  g074(.a(new_n108_), .b(new_n125_), .c(new_n69_), .d(new_n124_), .O(new_n126_));
  inv1   g075(.a(new_n126_), .O(new_n127_));
  nor3   g076(.a(x27), .b(x26), .c(x25), .O(new_n128_));
  inv1   g077(.a(x31), .O(new_n129_));
  nand4  g078(.a(new_n129_), .b(new_n114_), .c(new_n109_), .d(new_n113_), .O(new_n130_));
  inv1   g079(.a(new_n130_), .O(new_n131_));
  nand4  g080(.a(new_n131_), .b(new_n128_), .c(new_n127_), .d(new_n61_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n123_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(x16), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n58_), .O(z12));
  nor2   g084(.a(x31), .b(x28), .O(new_n136_));
  nand4  g085(.a(new_n136_), .b(new_n121_), .c(new_n91_), .d(new_n90_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n79_), .c(x32), .O(new_n138_));
  inv1   g087(.a(x26), .O(new_n139_));
  inv1   g088(.a(x32), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n129_), .c(new_n139_), .d(new_n90_), .O(new_n141_));
  inv1   g090(.a(new_n141_), .O(new_n142_));
  nand4  g091(.a(new_n142_), .b(new_n116_), .c(new_n127_), .d(new_n61_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n138_), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(x16), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n58_), .O(z13));
  nand2  g095(.a(new_n143_), .b(x33), .O(new_n147_));
  nor2   g096(.a(x33), .b(x32), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n136_), .c(new_n121_), .d(new_n91_), .O(new_n149_));
  nor2   g098(.a(new_n149_), .b(new_n83_), .O(new_n150_));
  inv1   g099(.a(new_n150_), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n147_), .c(new_n55_), .O(z14));
  inv1   g101(.a(x34), .O(new_n153_));
  oai21  g102(.a(new_n149_), .b(new_n83_), .c(new_n153_), .O(new_n154_));
  nand2  g103(.a(new_n150_), .b(x34), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n154_), .c(x16), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(new_n58_), .O(z15));
endmodule


