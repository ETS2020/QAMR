// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n204_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n54_), .O(z00));
  nor3   g007(.a(x20), .b(x19), .c(x17), .O(new_n59_));
  inv1   g008(.a(x20), .O(new_n60_));
  nor3   g009(.a(new_n53_), .b(new_n60_), .c(x18), .O(new_n61_));
  oai21  g010(.a(new_n61_), .b(new_n59_), .c(x16), .O(new_n62_));
  nor3   g011(.a(x18), .b(x16), .c(x14), .O(new_n63_));
  aoi21  g012(.a(new_n60_), .b(x18), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n62_), .O(z01));
  inv1   g014(.a(x18), .O(new_n66_));
  nand2  g015(.a(new_n56_), .b(x13), .O(new_n67_));
  inv1   g016(.a(x21), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  aoi21  g018(.a(new_n69_), .b(new_n53_), .c(new_n56_), .O(new_n70_));
  oai21  g019(.a(new_n59_), .b(new_n68_), .c(new_n70_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(new_n66_), .O(z02));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  nand3  g023(.a(new_n74_), .b(new_n53_), .c(x16), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n66_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n60_), .O(new_n77_));
  nor2   g026(.a(x16), .b(x12), .O(new_n78_));
  aoi21  g027(.a(new_n70_), .b(x22), .c(new_n78_), .O(new_n79_));
  oai21  g028(.a(new_n79_), .b(x18), .c(new_n77_), .O(z03));
  nor3   g029(.a(x23), .b(x22), .c(x21), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n53_), .c(x16), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n66_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n60_), .O(new_n84_));
  nor2   g033(.a(x16), .b(x11), .O(new_n85_));
  inv1   g034(.a(x22), .O(new_n86_));
  nand2  g035(.a(new_n69_), .b(new_n53_), .O(new_n87_));
  inv1   g036(.a(new_n87_), .O(new_n88_));
  nand2  g037(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  inv1   g038(.a(x23), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n56_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n89_), .c(new_n85_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(x18), .c(new_n84_), .O(z04));
  inv1   g042(.a(x24), .O(new_n94_));
  aoi21  g043(.a(new_n81_), .b(new_n59_), .c(new_n94_), .O(new_n95_));
  nor2   g044(.a(x24), .b(x23), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n74_), .c(new_n53_), .d(new_n60_), .O(new_n97_));
  inv1   g046(.a(new_n97_), .O(new_n98_));
  oai21  g047(.a(new_n98_), .b(new_n95_), .c(x16), .O(new_n99_));
  inv1   g048(.a(x10), .O(new_n100_));
  aoi21  g049(.a(new_n56_), .b(new_n100_), .c(x18), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n99_), .O(z05));
  nor2   g051(.a(x25), .b(x24), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n81_), .c(new_n53_), .d(x16), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n66_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n60_), .O(new_n106_));
  nand3  g055(.a(new_n97_), .b(x25), .c(x16), .O(new_n107_));
  oai21  g056(.a(x16), .b(x09), .c(new_n107_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n66_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n106_), .O(z06));
  inv1   g059(.a(x25), .O(new_n111_));
  inv1   g060(.a(x26), .O(new_n112_));
  nand3  g061(.a(new_n112_), .b(new_n111_), .c(new_n94_), .O(new_n113_));
  oai21  g062(.a(new_n113_), .b(new_n82_), .c(new_n66_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n60_), .O(new_n115_));
  nor2   g064(.a(x23), .b(x22), .O(new_n116_));
  nand4  g065(.a(new_n103_), .b(new_n116_), .c(new_n69_), .d(new_n53_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(x26), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(x16), .O(new_n119_));
  aoi21  g068(.a(new_n56_), .b(x08), .c(x18), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g070(.a(new_n121_), .b(new_n115_), .O(z07));
  inv1   g071(.a(x27), .O(new_n123_));
  nor2   g072(.a(x26), .b(x25), .O(new_n124_));
  nand2  g073(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand4  g074(.a(new_n81_), .b(new_n53_), .c(new_n94_), .d(x16), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n125_), .c(new_n66_), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n60_), .O(new_n128_));
  nor2   g077(.a(x16), .b(x07), .O(new_n129_));
  nand2  g078(.a(new_n90_), .b(new_n86_), .O(new_n130_));
  nor2   g079(.a(new_n113_), .b(new_n130_), .O(new_n131_));
  nand2  g080(.a(x27), .b(x16), .O(new_n132_));
  aoi21  g081(.a(new_n131_), .b(new_n88_), .c(new_n132_), .O(new_n133_));
  oai21  g082(.a(new_n133_), .b(new_n129_), .c(new_n66_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n128_), .O(z08));
  inv1   g084(.a(x28), .O(new_n136_));
  nand3  g085(.a(new_n69_), .b(new_n53_), .c(new_n123_), .O(new_n137_));
  inv1   g086(.a(new_n137_), .O(new_n138_));
  aoi21  g087(.a(new_n138_), .b(new_n131_), .c(new_n136_), .O(new_n139_));
  nand3  g088(.a(new_n124_), .b(new_n116_), .c(new_n94_), .O(new_n140_));
  nor2   g089(.a(x28), .b(x27), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n69_), .c(new_n53_), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  oai21  g092(.a(new_n143_), .b(new_n139_), .c(x16), .O(new_n144_));
  inv1   g093(.a(x06), .O(new_n145_));
  aoi21  g094(.a(new_n56_), .b(new_n145_), .c(x18), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n144_), .O(z09));
  inv1   g096(.a(x29), .O(new_n148_));
  nor3   g097(.a(x26), .b(x25), .c(x24), .O(new_n149_));
  nor3   g098(.a(x29), .b(x28), .c(x27), .O(new_n150_));
  nand4  g099(.a(new_n150_), .b(new_n149_), .c(new_n81_), .d(new_n59_), .O(new_n151_));
  oai21  g100(.a(new_n143_), .b(new_n148_), .c(new_n151_), .O(new_n152_));
  nand2  g101(.a(new_n152_), .b(x16), .O(new_n153_));
  inv1   g102(.a(x05), .O(new_n154_));
  aoi21  g103(.a(new_n56_), .b(new_n154_), .c(x18), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n153_), .O(z10));
  nand4  g105(.a(new_n116_), .b(new_n53_), .c(new_n68_), .d(new_n60_), .O(new_n157_));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n141_), .c(new_n124_), .d(new_n94_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  aoi21  g109(.a(new_n151_), .b(x30), .c(new_n160_), .O(new_n161_));
  inv1   g110(.a(x04), .O(new_n162_));
  aoi21  g111(.a(new_n56_), .b(new_n162_), .c(x18), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n56_), .c(new_n163_), .O(z11));
  nor2   g113(.a(x31), .b(x28), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n158_), .c(new_n124_), .d(new_n123_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n126_), .c(new_n66_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n60_), .O(new_n168_));
  inv1   g117(.a(x31), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n56_), .O(new_n170_));
  oai21  g119(.a(new_n159_), .b(new_n157_), .c(new_n170_), .O(new_n171_));
  inv1   g120(.a(x03), .O(new_n172_));
  nand2  g121(.a(new_n56_), .b(new_n172_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n66_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n168_), .O(z12));
  oai21  g125(.a(new_n166_), .b(new_n97_), .c(x32), .O(new_n177_));
  nand4  g126(.a(new_n94_), .b(new_n90_), .c(new_n86_), .d(new_n68_), .O(new_n178_));
  inv1   g127(.a(new_n178_), .O(new_n179_));
  inv1   g128(.a(x30), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n148_), .c(new_n136_), .d(new_n123_), .O(new_n181_));
  inv1   g130(.a(new_n181_), .O(new_n182_));
  inv1   g131(.a(x32), .O(new_n183_));
  nand4  g132(.a(new_n183_), .b(new_n169_), .c(new_n112_), .d(new_n111_), .O(new_n184_));
  inv1   g133(.a(new_n184_), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n182_), .c(new_n179_), .d(new_n59_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n177_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(x16), .O(new_n188_));
  inv1   g137(.a(x02), .O(new_n189_));
  aoi21  g138(.a(new_n56_), .b(new_n189_), .c(x18), .O(new_n190_));
  nand2  g139(.a(new_n190_), .b(new_n188_), .O(z13));
  nor2   g140(.a(x27), .b(x26), .O(new_n192_));
  nor2   g141(.a(x33), .b(x32), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n192_), .c(new_n165_), .d(new_n158_), .O(new_n194_));
  oai21  g143(.a(new_n194_), .b(new_n104_), .c(new_n66_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n60_), .O(new_n196_));
  nor2   g145(.a(x32), .b(x31), .O(new_n197_));
  nand4  g146(.a(new_n197_), .b(new_n158_), .c(new_n141_), .d(new_n124_), .O(new_n198_));
  and2   g147(.a(x33), .b(x16), .O(new_n199_));
  oai21  g148(.a(new_n198_), .b(new_n97_), .c(new_n199_), .O(new_n200_));
  inv1   g149(.a(x01), .O(new_n201_));
  nand2  g150(.a(new_n56_), .b(new_n201_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n200_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n66_), .O(new_n204_));
  nand2  g153(.a(new_n204_), .b(new_n196_), .O(z14));
  inv1   g154(.a(x34), .O(new_n206_));
  oai21  g155(.a(new_n194_), .b(new_n117_), .c(new_n206_), .O(new_n207_));
  nor2   g156(.a(new_n194_), .b(new_n117_), .O(new_n208_));
  nand2  g157(.a(new_n208_), .b(x34), .O(new_n209_));
  nand3  g158(.a(new_n209_), .b(new_n207_), .c(x16), .O(new_n210_));
  inv1   g159(.a(x00), .O(new_n211_));
  aoi21  g160(.a(new_n56_), .b(new_n211_), .c(x18), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n210_), .O(z15));
endmodule


