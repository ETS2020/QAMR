// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n56_), .O(z00));
  nor2   g009(.a(x18), .b(x16), .O(new_n61_));
  inv1   g010(.a(new_n61_), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  aoi21  g012(.a(new_n55_), .b(new_n63_), .c(x18), .O(new_n64_));
  oai21  g013(.a(new_n55_), .b(new_n63_), .c(new_n64_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(x16), .O(new_n66_));
  oai21  g015(.a(new_n62_), .b(x14), .c(new_n66_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  aoi21  g017(.a(new_n55_), .b(new_n63_), .c(new_n68_), .O(new_n69_));
  inv1   g018(.a(x18), .O(new_n70_));
  nand4  g019(.a(new_n68_), .b(new_n63_), .c(new_n53_), .d(new_n52_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g021(.a(new_n72_), .b(new_n69_), .c(x16), .O(new_n73_));
  oai21  g022(.a(new_n62_), .b(x13), .c(new_n73_), .O(z02));
  xor2a  g023(.a(new_n71_), .b(x22), .O(new_n75_));
  inv1   g024(.a(x12), .O(new_n76_));
  aoi21  g025(.a(new_n58_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n58_), .c(new_n77_), .O(z03));
  oai21  g027(.a(new_n71_), .b(x22), .c(x23), .O(new_n79_));
  inv1   g028(.a(new_n71_), .O(new_n80_));
  nor2   g029(.a(x23), .b(x22), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g031(.a(new_n82_), .b(new_n79_), .c(new_n58_), .O(new_n83_));
  oai21  g032(.a(x16), .b(x11), .c(new_n70_), .O(new_n84_));
  or2    g033(.a(new_n84_), .b(new_n83_), .O(z04));
  xor2a  g034(.a(new_n82_), .b(x24), .O(new_n86_));
  inv1   g035(.a(x10), .O(new_n87_));
  aoi21  g036(.a(new_n58_), .b(new_n87_), .c(x18), .O(new_n88_));
  oai21  g037(.a(new_n86_), .b(new_n58_), .c(new_n88_), .O(z05));
  inv1   g038(.a(x24), .O(new_n90_));
  nor2   g039(.a(x21), .b(x20), .O(new_n91_));
  nand4  g040(.a(new_n81_), .b(new_n91_), .c(new_n55_), .d(new_n90_), .O(new_n92_));
  nor2   g041(.a(x25), .b(x24), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n81_), .c(new_n91_), .d(new_n55_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n70_), .O(new_n95_));
  aoi21  g044(.a(new_n92_), .b(x25), .c(new_n95_), .O(new_n96_));
  oai22  g045(.a(new_n96_), .b(new_n58_), .c(new_n62_), .d(x09), .O(z06));
  nand2  g046(.a(new_n58_), .b(x08), .O(new_n98_));
  nor2   g047(.a(new_n94_), .b(x26), .O(new_n99_));
  inv1   g048(.a(x26), .O(new_n100_));
  inv1   g049(.a(new_n94_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n100_), .c(x16), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n99_), .c(new_n98_), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n70_), .O(z07));
  inv1   g053(.a(x07), .O(new_n105_));
  nand2  g054(.a(new_n61_), .b(new_n105_), .O(new_n106_));
  inv1   g055(.a(x25), .O(new_n107_));
  inv1   g056(.a(new_n92_), .O(new_n108_));
  nor2   g057(.a(x27), .b(x26), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  oai21  g059(.a(new_n94_), .b(x26), .c(x27), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n110_), .c(new_n70_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(x16), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n106_), .O(z08));
  inv1   g063(.a(x28), .O(new_n115_));
  nand3  g064(.a(new_n109_), .b(new_n115_), .c(new_n107_), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n92_), .O(new_n117_));
  aoi21  g066(.a(new_n110_), .b(x28), .c(new_n117_), .O(new_n118_));
  inv1   g067(.a(x06), .O(new_n119_));
  aoi21  g068(.a(new_n58_), .b(new_n119_), .c(x18), .O(new_n120_));
  oai21  g069(.a(new_n118_), .b(new_n58_), .c(new_n120_), .O(z09));
  oai21  g070(.a(new_n116_), .b(new_n92_), .c(x29), .O(new_n122_));
  inv1   g071(.a(x22), .O(new_n123_));
  inv1   g072(.a(x23), .O(new_n124_));
  nand4  g073(.a(new_n107_), .b(new_n90_), .c(new_n124_), .d(new_n123_), .O(new_n125_));
  inv1   g074(.a(new_n125_), .O(new_n126_));
  nor3   g075(.a(x29), .b(x28), .c(x27), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n126_), .c(new_n80_), .d(new_n100_), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n122_), .c(new_n70_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(x16), .O(new_n130_));
  inv1   g079(.a(x05), .O(new_n131_));
  nand2  g080(.a(new_n61_), .b(new_n131_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n130_), .O(z10));
  inv1   g082(.a(x27), .O(new_n134_));
  inv1   g083(.a(x30), .O(new_n135_));
  nor2   g084(.a(x29), .b(x28), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n135_), .c(new_n134_), .O(new_n137_));
  inv1   g086(.a(new_n137_), .O(new_n138_));
  aoi22  g087(.a(new_n138_), .b(new_n99_), .c(new_n128_), .d(x30), .O(new_n139_));
  inv1   g088(.a(x04), .O(new_n140_));
  aoi21  g089(.a(new_n58_), .b(new_n140_), .c(x18), .O(new_n141_));
  oai21  g090(.a(new_n139_), .b(new_n58_), .c(new_n141_), .O(z11));
  nand3  g091(.a(new_n138_), .b(new_n101_), .c(new_n100_), .O(new_n143_));
  nor2   g092(.a(x31), .b(x30), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n136_), .c(new_n109_), .d(new_n107_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n92_), .c(new_n70_), .O(new_n146_));
  aoi21  g095(.a(new_n143_), .b(x31), .c(new_n146_), .O(new_n147_));
  inv1   g096(.a(x03), .O(new_n148_));
  nand2  g097(.a(new_n61_), .b(new_n148_), .O(new_n149_));
  oai21  g098(.a(new_n147_), .b(new_n58_), .c(new_n149_), .O(z12));
  oai21  g099(.a(new_n145_), .b(new_n92_), .c(x32), .O(new_n151_));
  nor2   g100(.a(x32), .b(x31), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n100_), .c(new_n107_), .O(new_n153_));
  inv1   g102(.a(new_n153_), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n138_), .c(new_n108_), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n151_), .c(new_n70_), .O(new_n156_));
  nand2  g105(.a(new_n156_), .b(x16), .O(new_n157_));
  inv1   g106(.a(x02), .O(new_n158_));
  nand2  g107(.a(new_n61_), .b(new_n158_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n157_), .O(z13));
  nor2   g109(.a(x33), .b(x32), .O(new_n161_));
  nand4  g110(.a(new_n161_), .b(new_n144_), .c(new_n136_), .d(new_n109_), .O(new_n162_));
  nor2   g111(.a(new_n162_), .b(new_n94_), .O(new_n163_));
  aoi21  g112(.a(new_n155_), .b(x33), .c(new_n163_), .O(new_n164_));
  inv1   g113(.a(x01), .O(new_n165_));
  aoi21  g114(.a(new_n58_), .b(new_n165_), .c(x18), .O(new_n166_));
  oai21  g115(.a(new_n164_), .b(new_n58_), .c(new_n166_), .O(z14));
  inv1   g116(.a(x00), .O(new_n168_));
  nand2  g117(.a(new_n61_), .b(new_n168_), .O(new_n169_));
  nor3   g118(.a(new_n162_), .b(new_n94_), .c(x34), .O(new_n170_));
  oai21  g119(.a(new_n162_), .b(new_n94_), .c(x34), .O(new_n171_));
  nand2  g120(.a(new_n171_), .b(new_n70_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n170_), .c(x16), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n169_), .O(z15));
endmodule


