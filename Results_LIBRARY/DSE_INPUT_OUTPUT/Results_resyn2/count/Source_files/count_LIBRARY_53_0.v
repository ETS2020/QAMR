// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x18), .O(new_n52_));
  nor2   g01(.a(x19), .b(x17), .O(new_n53_));
  inv1   g02(.a(x17), .O(new_n54_));
  inv1   g03(.a(x19), .O(new_n55_));
  oai21  g04(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g05(.a(x16), .O(new_n57_));
  inv1   g06(.a(x22), .O(new_n58_));
  aoi21  g07(.a(new_n57_), .b(x15), .c(new_n58_), .O(new_n59_));
  oai21  g08(.a(new_n56_), .b(new_n53_), .c(new_n59_), .O(new_n60_));
  nand2  g09(.a(new_n60_), .b(new_n52_), .O(z00));
  inv1   g10(.a(x20), .O(new_n62_));
  xor2a  g11(.a(new_n53_), .b(new_n62_), .O(new_n63_));
  nor2   g12(.a(x16), .b(x14), .O(new_n64_));
  nand2  g13(.a(x22), .b(new_n52_), .O(new_n65_));
  nor2   g14(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  oai21  g15(.a(new_n63_), .b(new_n57_), .c(new_n66_), .O(z01));
  nand3  g16(.a(new_n62_), .b(new_n55_), .c(new_n54_), .O(new_n68_));
  xor2a  g17(.a(new_n68_), .b(x21), .O(new_n69_));
  nor2   g18(.a(x16), .b(x13), .O(new_n70_));
  nor2   g19(.a(new_n70_), .b(new_n65_), .O(new_n71_));
  oai21  g20(.a(new_n69_), .b(new_n57_), .c(new_n71_), .O(z02));
  oai21  g21(.a(new_n68_), .b(x21), .c(x16), .O(new_n73_));
  nor2   g22(.a(x16), .b(x12), .O(new_n74_));
  nor2   g23(.a(new_n74_), .b(new_n65_), .O(new_n75_));
  nand2  g24(.a(new_n75_), .b(new_n73_), .O(z03));
  nand2  g25(.a(new_n57_), .b(x11), .O(new_n77_));
  inv1   g26(.a(x23), .O(new_n78_));
  nand2  g27(.a(new_n78_), .b(x16), .O(new_n79_));
  nand3  g28(.a(new_n79_), .b(new_n77_), .c(x22), .O(new_n80_));
  nand2  g29(.a(new_n80_), .b(new_n52_), .O(z04));
  nand2  g30(.a(new_n57_), .b(x10), .O(new_n82_));
  inv1   g31(.a(x24), .O(new_n83_));
  nand2  g32(.a(new_n83_), .b(x16), .O(new_n84_));
  nand3  g33(.a(new_n84_), .b(new_n82_), .c(x22), .O(new_n85_));
  nand2  g34(.a(new_n85_), .b(new_n52_), .O(z05));
  inv1   g35(.a(new_n65_), .O(new_n87_));
  nand2  g36(.a(x25), .b(x16), .O(new_n88_));
  inv1   g37(.a(x09), .O(new_n89_));
  nand2  g38(.a(new_n57_), .b(new_n89_), .O(new_n90_));
  nand3  g39(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(z06));
  nand2  g40(.a(x26), .b(x16), .O(new_n92_));
  inv1   g41(.a(x08), .O(new_n93_));
  nand2  g42(.a(new_n57_), .b(new_n93_), .O(new_n94_));
  nand3  g43(.a(new_n94_), .b(new_n92_), .c(new_n87_), .O(z07));
  nand2  g44(.a(new_n57_), .b(x07), .O(new_n96_));
  inv1   g45(.a(x27), .O(new_n97_));
  nand2  g46(.a(new_n97_), .b(x16), .O(new_n98_));
  nand3  g47(.a(new_n98_), .b(new_n96_), .c(x22), .O(new_n99_));
  nand2  g48(.a(new_n99_), .b(new_n52_), .O(z08));
  nand2  g49(.a(new_n57_), .b(x06), .O(new_n101_));
  inv1   g50(.a(x28), .O(new_n102_));
  nand2  g51(.a(new_n102_), .b(x16), .O(new_n103_));
  nand3  g52(.a(new_n103_), .b(new_n101_), .c(x22), .O(new_n104_));
  nand2  g53(.a(new_n104_), .b(new_n52_), .O(z09));
  nand2  g54(.a(new_n57_), .b(x05), .O(new_n106_));
  inv1   g55(.a(x29), .O(new_n107_));
  nand2  g56(.a(new_n107_), .b(x16), .O(new_n108_));
  nand3  g57(.a(new_n108_), .b(new_n106_), .c(x22), .O(new_n109_));
  nand2  g58(.a(new_n109_), .b(new_n52_), .O(z10));
  nand2  g59(.a(x30), .b(x16), .O(new_n111_));
  inv1   g60(.a(x04), .O(new_n112_));
  nand2  g61(.a(new_n57_), .b(new_n112_), .O(new_n113_));
  nand3  g62(.a(new_n113_), .b(new_n111_), .c(new_n87_), .O(z11));
  nand2  g63(.a(new_n57_), .b(x03), .O(new_n115_));
  inv1   g64(.a(x31), .O(new_n116_));
  nand2  g65(.a(new_n116_), .b(x16), .O(new_n117_));
  nand3  g66(.a(new_n117_), .b(new_n115_), .c(x22), .O(new_n118_));
  nand2  g67(.a(new_n118_), .b(new_n52_), .O(z12));
  nand2  g68(.a(new_n57_), .b(x02), .O(new_n120_));
  inv1   g69(.a(x32), .O(new_n121_));
  nand2  g70(.a(new_n121_), .b(x16), .O(new_n122_));
  nand3  g71(.a(new_n122_), .b(new_n120_), .c(x22), .O(new_n123_));
  nand2  g72(.a(new_n123_), .b(new_n52_), .O(z13));
  nand2  g73(.a(x33), .b(x16), .O(new_n125_));
  inv1   g74(.a(x01), .O(new_n126_));
  nand2  g75(.a(new_n57_), .b(new_n126_), .O(new_n127_));
  nand3  g76(.a(new_n127_), .b(new_n125_), .c(new_n87_), .O(z14));
  nand2  g77(.a(new_n57_), .b(x00), .O(new_n129_));
  inv1   g78(.a(x34), .O(new_n130_));
  nand2  g79(.a(new_n130_), .b(x16), .O(new_n131_));
  nand3  g80(.a(new_n131_), .b(new_n129_), .c(x22), .O(new_n132_));
  nand2  g81(.a(new_n132_), .b(new_n52_), .O(z15));
endmodule


