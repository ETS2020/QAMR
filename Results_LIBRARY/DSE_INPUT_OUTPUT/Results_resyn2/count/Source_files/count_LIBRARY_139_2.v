// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x00), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  nor3   g008(.a(x20), .b(x19), .c(x17), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  oai21  g010(.a(new_n53_), .b(new_n61_), .c(x16), .O(new_n62_));
  aoi21  g011(.a(new_n56_), .b(x14), .c(x00), .O(new_n63_));
  oai21  g012(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(new_n64_));
  nand2  g013(.a(new_n64_), .b(new_n52_), .O(z01));
  nand2  g014(.a(new_n53_), .b(new_n61_), .O(new_n66_));
  xor2a  g015(.a(new_n66_), .b(x21), .O(new_n67_));
  nor2   g016(.a(x16), .b(x13), .O(new_n68_));
  nor2   g017(.a(x18), .b(x00), .O(new_n69_));
  inv1   g018(.a(new_n69_), .O(new_n70_));
  nor2   g019(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  oai21  g020(.a(new_n67_), .b(new_n56_), .c(new_n71_), .O(z02));
  oai21  g021(.a(new_n66_), .b(x21), .c(x22), .O(new_n73_));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(new_n60_), .O(new_n75_));
  nand3  g024(.a(new_n75_), .b(new_n73_), .c(x16), .O(new_n76_));
  aoi21  g025(.a(new_n56_), .b(x12), .c(x00), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n52_), .O(z03));
  nand2  g028(.a(new_n75_), .b(x23), .O(new_n80_));
  nor3   g029(.a(x23), .b(x22), .c(x21), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n60_), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  aoi21  g032(.a(new_n56_), .b(x11), .c(x00), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n52_), .O(z04));
  inv1   g035(.a(x24), .O(new_n87_));
  inv1   g036(.a(x23), .O(new_n88_));
  nand2  g037(.a(new_n74_), .b(new_n88_), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n66_), .O(new_n90_));
  nor2   g039(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nor2   g040(.a(x24), .b(x23), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n74_), .c(new_n53_), .d(new_n61_), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(x16), .O(new_n94_));
  aoi21  g043(.a(new_n56_), .b(x10), .c(x00), .O(new_n95_));
  oai21  g044(.a(new_n94_), .b(new_n91_), .c(new_n95_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n52_), .O(z05));
  nor2   g046(.a(x25), .b(x24), .O(new_n98_));
  inv1   g047(.a(new_n98_), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n82_), .O(new_n100_));
  inv1   g049(.a(x25), .O(new_n101_));
  inv1   g050(.a(new_n93_), .O(new_n102_));
  oai21  g051(.a(new_n102_), .b(new_n101_), .c(x16), .O(new_n103_));
  aoi21  g052(.a(new_n56_), .b(x09), .c(x00), .O(new_n104_));
  oai21  g053(.a(new_n103_), .b(new_n100_), .c(new_n104_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n52_), .O(z06));
  oai21  g055(.a(new_n99_), .b(new_n82_), .c(x26), .O(new_n107_));
  nor3   g056(.a(x26), .b(x25), .c(x24), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n81_), .c(new_n60_), .O(new_n109_));
  nand3  g058(.a(new_n109_), .b(new_n107_), .c(x16), .O(new_n110_));
  aoi21  g059(.a(new_n56_), .b(x08), .c(x00), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g061(.a(new_n112_), .b(new_n52_), .O(z07));
  nor3   g062(.a(x27), .b(x26), .c(x25), .O(new_n114_));
  aoi22  g063(.a(new_n114_), .b(new_n102_), .c(new_n109_), .d(x27), .O(new_n115_));
  nor2   g064(.a(x16), .b(x07), .O(new_n116_));
  nor2   g065(.a(new_n116_), .b(new_n70_), .O(new_n117_));
  oai21  g066(.a(new_n115_), .b(new_n56_), .c(new_n117_), .O(z08));
  inv1   g067(.a(x28), .O(new_n119_));
  inv1   g068(.a(x27), .O(new_n120_));
  nor2   g069(.a(x26), .b(x25), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(new_n120_), .c(new_n87_), .O(new_n122_));
  inv1   g071(.a(new_n122_), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n90_), .c(new_n119_), .O(new_n124_));
  inv1   g073(.a(new_n114_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n93_), .c(x28), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n124_), .c(x16), .O(new_n127_));
  aoi21  g076(.a(new_n56_), .b(x06), .c(x00), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n52_), .O(z09));
  nor2   g079(.a(x29), .b(x28), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n81_), .c(new_n60_), .O(new_n132_));
  nor2   g081(.a(new_n132_), .b(new_n122_), .O(new_n133_));
  aoi21  g082(.a(new_n124_), .b(x29), .c(new_n133_), .O(new_n134_));
  nor2   g083(.a(x16), .b(x05), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n70_), .O(new_n136_));
  oai21  g085(.a(new_n134_), .b(new_n56_), .c(new_n136_), .O(z10));
  oai21  g086(.a(new_n132_), .b(new_n122_), .c(x30), .O(new_n138_));
  nor2   g087(.a(x30), .b(x27), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n131_), .O(new_n140_));
  inv1   g089(.a(new_n140_), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n108_), .c(new_n90_), .O(new_n142_));
  nand3  g091(.a(new_n142_), .b(new_n138_), .c(x16), .O(new_n143_));
  aoi21  g092(.a(new_n56_), .b(x04), .c(x00), .O(new_n144_));
  nand2  g093(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n52_), .O(z11));
  oai21  g095(.a(new_n140_), .b(new_n109_), .c(x31), .O(new_n147_));
  inv1   g096(.a(x30), .O(new_n148_));
  inv1   g097(.a(x31), .O(new_n149_));
  nand3  g098(.a(new_n131_), .b(new_n149_), .c(new_n148_), .O(new_n150_));
  inv1   g099(.a(new_n150_), .O(new_n151_));
  nand3  g100(.a(new_n151_), .b(new_n114_), .c(new_n102_), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n147_), .c(x16), .O(new_n153_));
  aoi21  g102(.a(new_n56_), .b(x03), .c(x00), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n52_), .O(z12));
  nor2   g105(.a(x32), .b(x31), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n139_), .c(new_n131_), .d(new_n121_), .O(new_n158_));
  oai21  g107(.a(new_n158_), .b(new_n93_), .c(x16), .O(new_n159_));
  aoi21  g108(.a(new_n152_), .b(x32), .c(new_n159_), .O(new_n160_));
  inv1   g109(.a(x00), .O(new_n161_));
  inv1   g110(.a(x02), .O(new_n162_));
  oai21  g111(.a(x16), .b(new_n162_), .c(new_n161_), .O(new_n163_));
  oai21  g112(.a(new_n163_), .b(new_n160_), .c(new_n52_), .O(z13));
  oai21  g113(.a(new_n158_), .b(new_n93_), .c(x33), .O(new_n165_));
  nor2   g114(.a(x27), .b(x26), .O(new_n166_));
  nor2   g115(.a(x33), .b(x32), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nor2   g117(.a(new_n168_), .b(new_n150_), .O(new_n169_));
  nand3  g118(.a(new_n169_), .b(new_n98_), .c(new_n90_), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n165_), .c(x16), .O(new_n171_));
  aoi21  g120(.a(new_n56_), .b(x01), .c(x00), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n171_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n52_), .O(z14));
  nand2  g123(.a(new_n170_), .b(x34), .O(new_n175_));
  inv1   g124(.a(x34), .O(new_n176_));
  nand3  g125(.a(new_n169_), .b(new_n100_), .c(new_n176_), .O(new_n177_));
  nand4  g126(.a(new_n177_), .b(new_n175_), .c(new_n69_), .d(x16), .O(z15));
endmodule


