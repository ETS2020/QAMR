// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x29), .O(new_n57_));
  aoi21  g006(.a(new_n56_), .b(x15), .c(new_n57_), .O(new_n58_));
  oai21  g007(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n52_), .O(z00));
  inv1   g009(.a(x17), .O(new_n61_));
  inv1   g010(.a(x19), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  oai21  g013(.a(x19), .b(x17), .c(x20), .O(new_n65_));
  nand3  g014(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  aoi21  g015(.a(new_n56_), .b(x14), .c(new_n57_), .O(new_n67_));
  nand2  g016(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n52_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  inv1   g019(.a(new_n64_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g021(.a(new_n64_), .b(x21), .c(new_n56_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g023(.a(new_n56_), .b(x13), .c(new_n57_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n52_), .O(z02));
  inv1   g026(.a(x22), .O(new_n78_));
  nand4  g027(.a(new_n53_), .b(new_n78_), .c(new_n70_), .d(new_n63_), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(x16), .O(new_n80_));
  aoi21  g029(.a(new_n72_), .b(x22), .c(new_n80_), .O(new_n81_));
  inv1   g030(.a(x12), .O(new_n82_));
  oai21  g031(.a(x16), .b(new_n82_), .c(x29), .O(new_n83_));
  oai21  g032(.a(new_n83_), .b(new_n81_), .c(new_n52_), .O(z03));
  inv1   g033(.a(x23), .O(new_n85_));
  nand2  g034(.a(new_n78_), .b(new_n70_), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n64_), .O(new_n87_));
  nor2   g036(.a(x23), .b(x22), .O(new_n88_));
  nand4  g037(.a(new_n88_), .b(new_n53_), .c(new_n70_), .d(new_n63_), .O(new_n89_));
  oai21  g038(.a(new_n87_), .b(new_n85_), .c(new_n89_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(x16), .O(new_n91_));
  nor2   g040(.a(x16), .b(x11), .O(new_n92_));
  nand2  g041(.a(x29), .b(new_n52_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n91_), .O(z04));
  xor2a  g044(.a(new_n89_), .b(x24), .O(new_n96_));
  nor2   g045(.a(x16), .b(x10), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n93_), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n56_), .c(new_n98_), .O(z05));
  inv1   g048(.a(x24), .O(new_n100_));
  nand4  g049(.a(new_n88_), .b(new_n71_), .c(new_n100_), .d(new_n70_), .O(new_n101_));
  nor2   g050(.a(x25), .b(x24), .O(new_n102_));
  inv1   g051(.a(new_n102_), .O(new_n103_));
  nor2   g052(.a(new_n103_), .b(new_n89_), .O(new_n104_));
  aoi21  g053(.a(new_n101_), .b(x25), .c(new_n104_), .O(new_n105_));
  nor2   g054(.a(x16), .b(x09), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n93_), .O(new_n107_));
  oai21  g056(.a(new_n105_), .b(new_n56_), .c(new_n107_), .O(z06));
  nand4  g057(.a(new_n102_), .b(new_n88_), .c(new_n71_), .d(new_n70_), .O(new_n109_));
  inv1   g058(.a(x25), .O(new_n110_));
  inv1   g059(.a(x26), .O(new_n111_));
  nand3  g060(.a(new_n111_), .b(new_n110_), .c(new_n100_), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n89_), .O(new_n113_));
  aoi21  g062(.a(new_n109_), .b(x26), .c(new_n113_), .O(new_n114_));
  nor2   g063(.a(x16), .b(x08), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n93_), .O(new_n116_));
  oai21  g065(.a(new_n114_), .b(new_n56_), .c(new_n116_), .O(z07));
  inv1   g066(.a(new_n112_), .O(new_n118_));
  nand4  g067(.a(new_n118_), .b(new_n88_), .c(new_n71_), .d(new_n70_), .O(new_n119_));
  inv1   g068(.a(x27), .O(new_n120_));
  nand4  g069(.a(new_n102_), .b(new_n120_), .c(new_n111_), .d(new_n85_), .O(new_n121_));
  oai21  g070(.a(new_n121_), .b(new_n79_), .c(x16), .O(new_n122_));
  aoi21  g071(.a(new_n119_), .b(x27), .c(new_n122_), .O(new_n123_));
  inv1   g072(.a(x07), .O(new_n124_));
  oai21  g073(.a(x16), .b(new_n124_), .c(x29), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(new_n123_), .c(new_n52_), .O(z08));
  inv1   g075(.a(x28), .O(new_n127_));
  nand2  g076(.a(new_n120_), .b(new_n85_), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n112_), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n87_), .c(new_n127_), .O(new_n130_));
  oai21  g079(.a(new_n121_), .b(new_n79_), .c(x28), .O(new_n131_));
  nand3  g080(.a(new_n131_), .b(new_n130_), .c(x16), .O(new_n132_));
  aoi21  g081(.a(new_n56_), .b(x06), .c(new_n57_), .O(new_n133_));
  nand2  g082(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n52_), .O(z09));
  aoi21  g084(.a(new_n56_), .b(x05), .c(new_n57_), .O(new_n136_));
  oai21  g085(.a(new_n130_), .b(new_n56_), .c(new_n136_), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n52_), .O(z10));
  nand2  g087(.a(new_n56_), .b(x04), .O(new_n139_));
  inv1   g088(.a(x30), .O(new_n140_));
  nand2  g089(.a(new_n140_), .b(x16), .O(new_n141_));
  nand3  g090(.a(new_n141_), .b(new_n139_), .c(x29), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n52_), .O(z11));
  nand2  g092(.a(new_n56_), .b(x03), .O(new_n144_));
  inv1   g093(.a(x31), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(x16), .O(new_n146_));
  nand3  g095(.a(new_n146_), .b(new_n144_), .c(x29), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n52_), .O(z12));
  inv1   g097(.a(new_n93_), .O(new_n149_));
  nand2  g098(.a(x32), .b(x16), .O(new_n150_));
  inv1   g099(.a(x02), .O(new_n151_));
  nand2  g100(.a(new_n56_), .b(new_n151_), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n150_), .c(new_n149_), .O(z13));
  nand2  g102(.a(x33), .b(x16), .O(new_n154_));
  inv1   g103(.a(x01), .O(new_n155_));
  nand2  g104(.a(new_n56_), .b(new_n155_), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n154_), .c(new_n149_), .O(z14));
  nand2  g106(.a(x34), .b(x16), .O(new_n158_));
  inv1   g107(.a(x00), .O(new_n159_));
  nand2  g108(.a(new_n56_), .b(new_n159_), .O(new_n160_));
  nand3  g109(.a(new_n160_), .b(new_n158_), .c(new_n149_), .O(z15));
endmodule


