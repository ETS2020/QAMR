// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g008(.a(x33), .b(x18), .O(new_n60_));
  nand3  g009(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(z00));
  inv1   g010(.a(x20), .O(new_n62_));
  nor2   g011(.a(new_n55_), .b(new_n62_), .O(new_n63_));
  nor3   g012(.a(x20), .b(x19), .c(x17), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  inv1   g014(.a(new_n60_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x14), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n65_), .O(z01));
  nand3  g018(.a(new_n62_), .b(new_n53_), .c(new_n52_), .O(new_n70_));
  xor2a  g019(.a(new_n70_), .b(x21), .O(new_n71_));
  nor2   g020(.a(x16), .b(x13), .O(new_n72_));
  nor2   g021(.a(new_n72_), .b(new_n66_), .O(new_n73_));
  oai21  g022(.a(new_n71_), .b(new_n58_), .c(new_n73_), .O(z02));
  inv1   g023(.a(x18), .O(new_n75_));
  inv1   g024(.a(x21), .O(new_n76_));
  inv1   g025(.a(x22), .O(new_n77_));
  aoi21  g026(.a(new_n64_), .b(new_n76_), .c(new_n77_), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n55_), .c(new_n62_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(x16), .O(new_n81_));
  aoi21  g030(.a(new_n58_), .b(x12), .c(x33), .O(new_n82_));
  oai21  g031(.a(new_n81_), .b(new_n78_), .c(new_n82_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n75_), .O(z03));
  nand2  g033(.a(new_n80_), .b(x23), .O(new_n85_));
  inv1   g034(.a(x23), .O(new_n86_));
  nand3  g035(.a(new_n79_), .b(new_n64_), .c(new_n86_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(x16), .O(new_n88_));
  aoi21  g037(.a(new_n58_), .b(x11), .c(x33), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n75_), .O(z04));
  inv1   g040(.a(x24), .O(new_n92_));
  nand4  g041(.a(new_n92_), .b(new_n86_), .c(new_n77_), .d(new_n76_), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n70_), .O(new_n94_));
  aoi21  g043(.a(new_n87_), .b(x24), .c(new_n94_), .O(new_n95_));
  nor2   g044(.a(x16), .b(x10), .O(new_n96_));
  nor2   g045(.a(new_n96_), .b(new_n66_), .O(new_n97_));
  oai21  g046(.a(new_n95_), .b(new_n58_), .c(new_n97_), .O(z05));
  nand2  g047(.a(new_n79_), .b(new_n86_), .O(new_n99_));
  nor2   g048(.a(new_n99_), .b(new_n70_), .O(new_n100_));
  nor2   g049(.a(x25), .b(x24), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nor2   g051(.a(x24), .b(x23), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n79_), .c(new_n55_), .d(new_n62_), .O(new_n104_));
  aoi21  g053(.a(new_n104_), .b(x25), .c(new_n58_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n102_), .O(new_n106_));
  aoi21  g055(.a(new_n58_), .b(x09), .c(x33), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n75_), .O(z06));
  nor2   g058(.a(x26), .b(x25), .O(new_n110_));
  nand2  g059(.a(new_n110_), .b(new_n103_), .O(new_n111_));
  oai21  g060(.a(new_n111_), .b(new_n80_), .c(x16), .O(new_n112_));
  aoi21  g061(.a(new_n102_), .b(x26), .c(new_n112_), .O(new_n113_));
  inv1   g062(.a(x08), .O(new_n114_));
  inv1   g063(.a(x33), .O(new_n115_));
  oai21  g064(.a(x16), .b(new_n114_), .c(new_n115_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n113_), .c(new_n75_), .O(z07));
  inv1   g066(.a(x27), .O(new_n118_));
  nor2   g067(.a(new_n111_), .b(new_n80_), .O(new_n119_));
  nor3   g068(.a(x27), .b(x26), .c(x25), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n103_), .c(new_n79_), .d(new_n64_), .O(new_n121_));
  oai21  g070(.a(new_n119_), .b(new_n118_), .c(new_n121_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(x16), .O(new_n123_));
  nor2   g072(.a(x16), .b(x07), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n66_), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n123_), .O(z08));
  nor2   g075(.a(x28), .b(x27), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n119_), .O(new_n128_));
  nand2  g077(.a(new_n121_), .b(x28), .O(new_n129_));
  nand3  g078(.a(new_n129_), .b(new_n128_), .c(x16), .O(new_n130_));
  aoi21  g079(.a(new_n58_), .b(x06), .c(x33), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g081(.a(new_n132_), .b(new_n75_), .O(z09));
  nor2   g082(.a(x29), .b(x28), .O(new_n134_));
  nand4  g083(.a(new_n134_), .b(new_n110_), .c(new_n118_), .d(new_n92_), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n87_), .O(new_n136_));
  aoi21  g085(.a(new_n128_), .b(x29), .c(new_n136_), .O(new_n137_));
  nor2   g086(.a(x16), .b(x05), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n66_), .O(new_n139_));
  oai21  g088(.a(new_n137_), .b(new_n58_), .c(new_n139_), .O(z10));
  oai21  g089(.a(new_n135_), .b(new_n87_), .c(x30), .O(new_n141_));
  inv1   g090(.a(x28), .O(new_n142_));
  inv1   g091(.a(x29), .O(new_n143_));
  inv1   g092(.a(x30), .O(new_n144_));
  nand4  g093(.a(new_n144_), .b(new_n143_), .c(new_n142_), .d(new_n118_), .O(new_n145_));
  inv1   g094(.a(new_n145_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n119_), .O(new_n147_));
  nand3  g096(.a(new_n147_), .b(new_n141_), .c(x16), .O(new_n148_));
  aoi21  g097(.a(new_n58_), .b(x04), .c(x33), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n75_), .O(z11));
  inv1   g100(.a(x31), .O(new_n152_));
  aoi21  g101(.a(new_n146_), .b(new_n119_), .c(new_n152_), .O(new_n153_));
  nor2   g102(.a(x30), .b(x29), .O(new_n154_));
  nand3  g103(.a(new_n154_), .b(new_n152_), .c(new_n142_), .O(new_n155_));
  nor2   g104(.a(new_n155_), .b(new_n121_), .O(new_n156_));
  oai21  g105(.a(new_n156_), .b(new_n153_), .c(x16), .O(new_n157_));
  nor2   g106(.a(x16), .b(x03), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n66_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n157_), .O(z12));
  inv1   g109(.a(x25), .O(new_n161_));
  inv1   g110(.a(x26), .O(new_n162_));
  nand3  g111(.a(new_n118_), .b(new_n162_), .c(new_n161_), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n104_), .O(new_n164_));
  inv1   g113(.a(new_n155_), .O(new_n165_));
  nand2  g114(.a(x32), .b(x16), .O(new_n166_));
  aoi21  g115(.a(new_n165_), .b(new_n164_), .c(new_n166_), .O(new_n167_));
  inv1   g116(.a(x32), .O(new_n168_));
  nand4  g117(.a(new_n168_), .b(new_n152_), .c(new_n162_), .d(new_n161_), .O(new_n169_));
  nor2   g118(.a(new_n169_), .b(new_n145_), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n94_), .c(x16), .O(new_n171_));
  inv1   g120(.a(x02), .O(new_n172_));
  nand2  g121(.a(new_n58_), .b(new_n172_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(new_n174_));
  oai21  g123(.a(new_n174_), .b(new_n167_), .c(new_n115_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n75_), .O(z13));
  nor2   g125(.a(x16), .b(x01), .O(new_n177_));
  nor2   g126(.a(new_n177_), .b(new_n66_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n171_), .O(z14));
  inv1   g128(.a(x34), .O(new_n180_));
  nand4  g129(.a(new_n180_), .b(new_n168_), .c(new_n118_), .d(new_n162_), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n155_), .O(new_n182_));
  nand3  g131(.a(new_n182_), .b(new_n101_), .c(new_n100_), .O(new_n183_));
  nor2   g132(.a(x32), .b(x31), .O(new_n184_));
  nand4  g133(.a(new_n184_), .b(new_n154_), .c(new_n127_), .d(new_n110_), .O(new_n185_));
  oai21  g134(.a(new_n185_), .b(new_n104_), .c(x34), .O(new_n186_));
  nand3  g135(.a(new_n186_), .b(new_n183_), .c(x16), .O(new_n187_));
  aoi21  g136(.a(new_n58_), .b(x00), .c(x33), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  nand2  g138(.a(new_n189_), .b(new_n75_), .O(z15));
endmodule


