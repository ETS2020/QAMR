// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x03), .O(new_n59_));
  inv1   g008(.a(x18), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g010(.a(new_n58_), .b(new_n57_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n56_), .O(z00));
  inv1   g012(.a(x20), .O(new_n64_));
  nand3  g013(.a(new_n64_), .b(new_n53_), .c(new_n52_), .O(new_n65_));
  inv1   g014(.a(new_n65_), .O(new_n66_));
  oai21  g015(.a(new_n55_), .b(new_n64_), .c(x16), .O(new_n67_));
  aoi21  g016(.a(new_n58_), .b(x14), .c(x03), .O(new_n68_));
  oai21  g017(.a(new_n67_), .b(new_n66_), .c(new_n68_), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n60_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(new_n66_), .b(new_n71_), .O(new_n72_));
  nor2   g021(.a(x21), .b(x20), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n55_), .O(new_n74_));
  nand2  g023(.a(new_n74_), .b(x16), .O(new_n75_));
  aoi21  g024(.a(new_n58_), .b(x13), .c(x03), .O(new_n76_));
  oai21  g025(.a(new_n75_), .b(new_n72_), .c(new_n76_), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n60_), .O(z02));
  xor2a  g027(.a(new_n74_), .b(x22), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  nor2   g029(.a(new_n80_), .b(new_n61_), .O(new_n81_));
  oai21  g030(.a(new_n79_), .b(new_n58_), .c(new_n81_), .O(z03));
  inv1   g031(.a(x22), .O(new_n83_));
  nand3  g032(.a(new_n73_), .b(new_n55_), .c(new_n83_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(x23), .O(new_n85_));
  nor2   g034(.a(x23), .b(x22), .O(new_n86_));
  nand3  g035(.a(new_n86_), .b(new_n73_), .c(new_n55_), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n85_), .c(x16), .O(new_n88_));
  aoi21  g037(.a(new_n58_), .b(x11), .c(x03), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n60_), .O(z04));
  inv1   g040(.a(x23), .O(new_n92_));
  inv1   g041(.a(x24), .O(new_n93_));
  nand4  g042(.a(new_n93_), .b(new_n92_), .c(new_n83_), .d(new_n71_), .O(new_n94_));
  nor2   g043(.a(new_n94_), .b(new_n65_), .O(new_n95_));
  aoi21  g044(.a(new_n87_), .b(x24), .c(new_n95_), .O(new_n96_));
  nor2   g045(.a(x16), .b(x10), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n61_), .O(new_n98_));
  oai21  g047(.a(new_n96_), .b(new_n58_), .c(new_n98_), .O(z05));
  nor2   g048(.a(x24), .b(x23), .O(new_n100_));
  nor2   g049(.a(x22), .b(x21), .O(new_n101_));
  nand4  g050(.a(new_n101_), .b(new_n100_), .c(new_n55_), .d(new_n64_), .O(new_n102_));
  nor2   g051(.a(x25), .b(x24), .O(new_n103_));
  nand4  g052(.a(new_n103_), .b(new_n86_), .c(new_n73_), .d(new_n55_), .O(new_n104_));
  inv1   g053(.a(new_n104_), .O(new_n105_));
  aoi21  g054(.a(new_n102_), .b(x25), .c(new_n105_), .O(new_n106_));
  nor2   g055(.a(x16), .b(x09), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n61_), .O(new_n108_));
  oai21  g057(.a(new_n106_), .b(new_n58_), .c(new_n108_), .O(z06));
  inv1   g058(.a(x26), .O(new_n110_));
  nand2  g059(.a(new_n103_), .b(new_n110_), .O(new_n111_));
  nor2   g060(.a(new_n111_), .b(new_n87_), .O(new_n112_));
  oai21  g061(.a(new_n105_), .b(new_n110_), .c(x16), .O(new_n113_));
  aoi21  g062(.a(new_n58_), .b(x08), .c(x03), .O(new_n114_));
  oai21  g063(.a(new_n113_), .b(new_n112_), .c(new_n114_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n60_), .O(z07));
  oai21  g065(.a(new_n111_), .b(new_n87_), .c(x27), .O(new_n117_));
  inv1   g066(.a(x25), .O(new_n118_));
  nor2   g067(.a(x27), .b(x26), .O(new_n119_));
  nand3  g068(.a(new_n119_), .b(new_n95_), .c(new_n118_), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  aoi21  g070(.a(new_n58_), .b(x07), .c(x03), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g072(.a(new_n123_), .b(new_n60_), .O(z08));
  nor2   g073(.a(x28), .b(x27), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n100_), .c(new_n110_), .d(new_n118_), .O(new_n126_));
  oai21  g075(.a(new_n126_), .b(new_n84_), .c(x16), .O(new_n127_));
  aoi21  g076(.a(new_n120_), .b(x28), .c(new_n127_), .O(new_n128_));
  inv1   g077(.a(x06), .O(new_n129_));
  oai21  g078(.a(x16), .b(new_n129_), .c(new_n59_), .O(new_n130_));
  oai21  g079(.a(new_n130_), .b(new_n128_), .c(new_n60_), .O(z09));
  inv1   g080(.a(new_n87_), .O(new_n132_));
  inv1   g081(.a(new_n111_), .O(new_n133_));
  nor3   g082(.a(x29), .b(x28), .c(x27), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n133_), .c(new_n132_), .O(new_n135_));
  oai21  g084(.a(new_n126_), .b(new_n84_), .c(x29), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n135_), .c(x16), .O(new_n137_));
  aoi21  g086(.a(new_n58_), .b(x05), .c(x03), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n60_), .O(z10));
  xor2a  g089(.a(new_n135_), .b(x30), .O(new_n141_));
  nor2   g090(.a(x16), .b(x04), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n61_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n58_), .c(new_n143_), .O(z11));
  oai21  g093(.a(new_n135_), .b(x30), .c(x31), .O(new_n145_));
  nor2   g094(.a(x29), .b(x28), .O(new_n146_));
  nor2   g095(.a(x31), .b(x30), .O(new_n147_));
  nand4  g096(.a(new_n147_), .b(new_n146_), .c(new_n119_), .d(new_n118_), .O(new_n148_));
  nor2   g097(.a(new_n148_), .b(new_n102_), .O(new_n149_));
  nor3   g098(.a(new_n149_), .b(new_n61_), .c(new_n58_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n145_), .O(z12));
  nand2  g100(.a(new_n110_), .b(new_n118_), .O(new_n152_));
  inv1   g101(.a(x30), .O(new_n153_));
  inv1   g102(.a(x31), .O(new_n154_));
  inv1   g103(.a(x32), .O(new_n155_));
  nand3  g104(.a(new_n155_), .b(new_n154_), .c(new_n153_), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n152_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n134_), .c(new_n95_), .O(new_n158_));
  oai21  g107(.a(new_n148_), .b(new_n102_), .c(x32), .O(new_n159_));
  nand3  g108(.a(new_n159_), .b(new_n158_), .c(x16), .O(new_n160_));
  aoi21  g109(.a(new_n58_), .b(x02), .c(x03), .O(new_n161_));
  nand2  g110(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  nand2  g111(.a(new_n162_), .b(new_n60_), .O(z13));
  nor2   g112(.a(x33), .b(x32), .O(new_n164_));
  nand4  g113(.a(new_n164_), .b(new_n147_), .c(new_n146_), .d(new_n119_), .O(new_n165_));
  oai21  g114(.a(new_n165_), .b(new_n104_), .c(x16), .O(new_n166_));
  aoi21  g115(.a(new_n158_), .b(x33), .c(new_n166_), .O(new_n167_));
  inv1   g116(.a(x01), .O(new_n168_));
  oai21  g117(.a(x16), .b(new_n168_), .c(new_n59_), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n167_), .c(new_n60_), .O(z14));
  nor3   g119(.a(new_n165_), .b(new_n104_), .c(x34), .O(new_n171_));
  oai21  g120(.a(new_n165_), .b(new_n104_), .c(x34), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(x16), .O(new_n173_));
  aoi21  g122(.a(new_n58_), .b(x00), .c(x03), .O(new_n174_));
  oai21  g123(.a(new_n173_), .b(new_n171_), .c(new_n174_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(new_n60_), .O(z15));
endmodule


