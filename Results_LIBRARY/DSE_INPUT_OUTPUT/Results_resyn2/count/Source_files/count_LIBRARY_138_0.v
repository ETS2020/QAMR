// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n146_, new_n147_, new_n148_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x17), .O(new_n52_));
  inv1   g001(.a(x19), .O(new_n53_));
  nor2   g002(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g003(.a(x19), .b(x17), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n54_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  inv1   g007(.a(x18), .O(new_n59_));
  nand2  g008(.a(x29), .b(new_n59_), .O(new_n60_));
  aoi21  g009(.a(new_n58_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  nand2  g010(.a(new_n61_), .b(new_n56_), .O(z00));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  oai21  g013(.a(new_n55_), .b(new_n64_), .c(x16), .O(new_n65_));
  inv1   g014(.a(x29), .O(new_n66_));
  aoi21  g015(.a(new_n58_), .b(x14), .c(new_n66_), .O(new_n67_));
  oai21  g016(.a(new_n65_), .b(new_n63_), .c(new_n67_), .O(new_n68_));
  nand2  g017(.a(new_n68_), .b(new_n59_), .O(z01));
  inv1   g018(.a(x21), .O(new_n70_));
  nand4  g019(.a(new_n70_), .b(new_n64_), .c(new_n53_), .d(new_n52_), .O(new_n71_));
  inv1   g020(.a(new_n63_), .O(new_n72_));
  nand2  g021(.a(new_n72_), .b(x21), .O(new_n73_));
  nand3  g022(.a(new_n73_), .b(new_n71_), .c(x16), .O(new_n74_));
  aoi21  g023(.a(new_n58_), .b(x13), .c(new_n66_), .O(new_n75_));
  nand2  g024(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(new_n59_), .O(z02));
  nand2  g026(.a(new_n71_), .b(x22), .O(new_n78_));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  nand3  g028(.a(new_n79_), .b(new_n55_), .c(new_n64_), .O(new_n80_));
  nand3  g029(.a(new_n80_), .b(new_n78_), .c(x16), .O(new_n81_));
  aoi21  g030(.a(new_n58_), .b(x12), .c(new_n66_), .O(new_n82_));
  nand2  g031(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n59_), .O(z03));
  nor2   g033(.a(x23), .b(x22), .O(new_n85_));
  nand4  g034(.a(new_n85_), .b(new_n55_), .c(new_n70_), .d(new_n64_), .O(new_n86_));
  nand2  g035(.a(new_n80_), .b(x23), .O(new_n87_));
  nand3  g036(.a(new_n87_), .b(new_n86_), .c(x16), .O(new_n88_));
  aoi21  g037(.a(new_n58_), .b(x11), .c(new_n66_), .O(new_n89_));
  nand2  g038(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n59_), .O(z04));
  nand2  g040(.a(new_n86_), .b(x24), .O(new_n92_));
  nor2   g041(.a(x24), .b(x23), .O(new_n93_));
  nand3  g042(.a(new_n93_), .b(new_n79_), .c(new_n63_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(x16), .O(new_n96_));
  nor2   g045(.a(x16), .b(x10), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n60_), .O(new_n98_));
  nand2  g047(.a(new_n98_), .b(new_n96_), .O(z05));
  nand2  g048(.a(new_n94_), .b(x25), .O(new_n100_));
  inv1   g049(.a(x25), .O(new_n101_));
  nand4  g050(.a(new_n93_), .b(new_n79_), .c(new_n63_), .d(new_n101_), .O(new_n102_));
  nand3  g051(.a(new_n102_), .b(new_n100_), .c(x16), .O(new_n103_));
  aoi21  g052(.a(new_n58_), .b(x09), .c(new_n66_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n59_), .O(z06));
  inv1   g055(.a(x24), .O(new_n107_));
  inv1   g056(.a(x26), .O(new_n108_));
  nand3  g057(.a(new_n108_), .b(new_n101_), .c(new_n107_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(new_n86_), .O(new_n110_));
  aoi21  g059(.a(new_n102_), .b(x26), .c(new_n110_), .O(new_n111_));
  nor2   g060(.a(x16), .b(x08), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n60_), .O(new_n113_));
  oai21  g062(.a(new_n111_), .b(new_n58_), .c(new_n113_), .O(z07));
  inv1   g063(.a(new_n71_), .O(new_n115_));
  inv1   g064(.a(new_n109_), .O(new_n116_));
  nand3  g065(.a(new_n116_), .b(new_n85_), .c(new_n115_), .O(new_n117_));
  inv1   g066(.a(x27), .O(new_n118_));
  nand4  g067(.a(new_n93_), .b(new_n118_), .c(new_n108_), .d(new_n101_), .O(new_n119_));
  oai21  g068(.a(new_n119_), .b(new_n80_), .c(x16), .O(new_n120_));
  aoi21  g069(.a(new_n117_), .b(x27), .c(new_n120_), .O(new_n121_));
  inv1   g070(.a(x07), .O(new_n122_));
  oai21  g071(.a(x16), .b(new_n122_), .c(x29), .O(new_n123_));
  oai21  g072(.a(new_n123_), .b(new_n121_), .c(new_n59_), .O(z08));
  nor2   g073(.a(x28), .b(x27), .O(new_n125_));
  nand4  g074(.a(new_n125_), .b(new_n116_), .c(new_n85_), .d(new_n115_), .O(new_n126_));
  oai21  g075(.a(new_n119_), .b(new_n80_), .c(x28), .O(new_n127_));
  nand3  g076(.a(new_n127_), .b(new_n126_), .c(x16), .O(new_n128_));
  aoi21  g077(.a(new_n58_), .b(x06), .c(new_n66_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n59_), .O(z09));
  nand2  g080(.a(new_n126_), .b(x16), .O(new_n132_));
  nor2   g081(.a(x16), .b(x05), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n60_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n132_), .O(z10));
  nand2  g084(.a(new_n58_), .b(x04), .O(new_n136_));
  inv1   g085(.a(x30), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(x16), .O(new_n138_));
  nand3  g087(.a(new_n138_), .b(new_n136_), .c(x29), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n59_), .O(z11));
  inv1   g089(.a(new_n60_), .O(new_n141_));
  nand2  g090(.a(x31), .b(x16), .O(new_n142_));
  inv1   g091(.a(x03), .O(new_n143_));
  nand2  g092(.a(new_n58_), .b(new_n143_), .O(new_n144_));
  nand3  g093(.a(new_n144_), .b(new_n142_), .c(new_n141_), .O(z12));
  nand2  g094(.a(x32), .b(x16), .O(new_n146_));
  inv1   g095(.a(x02), .O(new_n147_));
  nand2  g096(.a(new_n58_), .b(new_n147_), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n146_), .c(new_n141_), .O(z13));
  nand2  g098(.a(x33), .b(x16), .O(new_n150_));
  inv1   g099(.a(x01), .O(new_n151_));
  nand2  g100(.a(new_n58_), .b(new_n151_), .O(new_n152_));
  nand3  g101(.a(new_n152_), .b(new_n150_), .c(new_n141_), .O(z14));
  nand2  g102(.a(new_n58_), .b(x00), .O(new_n154_));
  inv1   g103(.a(x34), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(x16), .O(new_n156_));
  nand3  g105(.a(new_n156_), .b(new_n154_), .c(x29), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n59_), .O(z15));
endmodule


