// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_;
  nand2  g000(.a(x30), .b(x18), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  nand2  g002(.a(x19), .b(x17), .O(new_n54_));
  nand2  g003(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(x15), .c(x18), .O(new_n57_));
  oai21  g006(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g007(.a(new_n58_), .b(new_n52_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  nand2  g009(.a(new_n56_), .b(x14), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  oai21  g012(.a(new_n53_), .b(new_n63_), .c(x16), .O(new_n64_));
  oai21  g013(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n60_), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  xor2a  g016(.a(new_n62_), .b(new_n67_), .O(new_n68_));
  inv1   g017(.a(x13), .O(new_n69_));
  aoi21  g018(.a(new_n56_), .b(new_n69_), .c(x18), .O(new_n70_));
  oai21  g019(.a(new_n68_), .b(new_n56_), .c(new_n70_), .O(z02));
  nand2  g020(.a(new_n62_), .b(new_n67_), .O(new_n72_));
  nor2   g021(.a(x22), .b(x21), .O(new_n73_));
  aoi22  g022(.a(new_n73_), .b(new_n62_), .c(new_n72_), .d(x22), .O(new_n74_));
  inv1   g023(.a(x12), .O(new_n75_));
  aoi21  g024(.a(new_n56_), .b(new_n75_), .c(x18), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n56_), .c(new_n76_), .O(z03));
  inv1   g026(.a(x23), .O(new_n78_));
  aoi21  g027(.a(new_n73_), .b(new_n62_), .c(new_n78_), .O(new_n79_));
  nand2  g028(.a(new_n53_), .b(new_n63_), .O(new_n80_));
  nand2  g029(.a(new_n73_), .b(new_n78_), .O(new_n81_));
  nor2   g030(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai21  g031(.a(new_n82_), .b(new_n79_), .c(x16), .O(new_n83_));
  inv1   g032(.a(x11), .O(new_n84_));
  aoi21  g033(.a(new_n56_), .b(new_n84_), .c(x18), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n83_), .O(z04));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n73_), .c(new_n53_), .d(new_n63_), .O(new_n88_));
  oai21  g037(.a(new_n81_), .b(new_n80_), .c(x24), .O(new_n89_));
  nand3  g038(.a(new_n89_), .b(new_n88_), .c(x16), .O(new_n90_));
  aoi21  g039(.a(new_n56_), .b(x10), .c(x18), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g041(.a(new_n92_), .b(new_n52_), .O(z05));
  inv1   g042(.a(x25), .O(new_n94_));
  inv1   g043(.a(new_n88_), .O(new_n95_));
  nor3   g044(.a(x23), .b(x22), .c(x21), .O(new_n96_));
  nor2   g045(.a(x25), .b(x24), .O(new_n97_));
  nand3  g046(.a(new_n97_), .b(new_n96_), .c(new_n62_), .O(new_n98_));
  oai21  g047(.a(new_n95_), .b(new_n94_), .c(new_n98_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(x16), .O(new_n100_));
  inv1   g049(.a(x09), .O(new_n101_));
  aoi21  g050(.a(new_n56_), .b(new_n101_), .c(x18), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n100_), .O(z06));
  nand2  g052(.a(new_n98_), .b(x26), .O(new_n104_));
  nor3   g053(.a(x26), .b(x25), .c(x24), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n82_), .O(new_n106_));
  nand3  g055(.a(new_n106_), .b(new_n104_), .c(x16), .O(new_n107_));
  aoi21  g056(.a(new_n56_), .b(x08), .c(x18), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g058(.a(new_n109_), .b(new_n52_), .O(z07));
  nor2   g059(.a(x27), .b(x26), .O(new_n111_));
  nand2  g060(.a(new_n111_), .b(new_n94_), .O(new_n112_));
  nor2   g061(.a(new_n112_), .b(new_n88_), .O(new_n113_));
  aoi21  g062(.a(new_n106_), .b(x27), .c(new_n113_), .O(new_n114_));
  inv1   g063(.a(x07), .O(new_n115_));
  aoi21  g064(.a(new_n56_), .b(new_n115_), .c(x18), .O(new_n116_));
  oai21  g065(.a(new_n114_), .b(new_n56_), .c(new_n116_), .O(z08));
  inv1   g066(.a(new_n113_), .O(new_n118_));
  inv1   g067(.a(x28), .O(new_n119_));
  nand3  g068(.a(new_n111_), .b(new_n119_), .c(new_n94_), .O(new_n120_));
  oai21  g069(.a(new_n120_), .b(new_n88_), .c(x16), .O(new_n121_));
  aoi21  g070(.a(new_n118_), .b(x28), .c(new_n121_), .O(new_n122_));
  inv1   g071(.a(x06), .O(new_n123_));
  oai21  g072(.a(x16), .b(new_n123_), .c(new_n60_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n122_), .c(new_n52_), .O(z09));
  oai21  g074(.a(new_n120_), .b(new_n88_), .c(x29), .O(new_n126_));
  nor3   g075(.a(x29), .b(x28), .c(x27), .O(new_n127_));
  nand4  g076(.a(new_n127_), .b(new_n105_), .c(new_n96_), .d(new_n62_), .O(new_n128_));
  nand3  g077(.a(new_n128_), .b(new_n126_), .c(x16), .O(new_n129_));
  aoi21  g078(.a(new_n56_), .b(x05), .c(x18), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n52_), .O(z10));
  xor2a  g081(.a(new_n128_), .b(x30), .O(new_n133_));
  inv1   g082(.a(x04), .O(new_n134_));
  aoi21  g083(.a(new_n56_), .b(new_n134_), .c(x18), .O(new_n135_));
  oai21  g084(.a(new_n133_), .b(new_n56_), .c(new_n135_), .O(z11));
  oai21  g085(.a(new_n128_), .b(x30), .c(x31), .O(new_n137_));
  nor2   g086(.a(x31), .b(x29), .O(new_n138_));
  nor2   g087(.a(x30), .b(x28), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  inv1   g089(.a(new_n140_), .O(new_n141_));
  nand2  g090(.a(new_n141_), .b(new_n113_), .O(new_n142_));
  nand2  g091(.a(new_n142_), .b(new_n137_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(x16), .O(new_n144_));
  inv1   g093(.a(x03), .O(new_n145_));
  aoi21  g094(.a(new_n56_), .b(new_n145_), .c(x18), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n144_), .O(z12));
  nand2  g096(.a(x32), .b(x16), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n60_), .O(new_n149_));
  nand2  g098(.a(new_n149_), .b(x30), .O(new_n150_));
  nand4  g099(.a(new_n138_), .b(new_n111_), .c(new_n119_), .d(new_n94_), .O(new_n151_));
  inv1   g100(.a(x24), .O(new_n152_));
  nand3  g101(.a(new_n96_), .b(new_n62_), .c(new_n152_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n151_), .c(x32), .O(new_n154_));
  inv1   g103(.a(new_n120_), .O(new_n155_));
  nor2   g104(.a(x32), .b(x30), .O(new_n156_));
  nand4  g105(.a(new_n156_), .b(new_n138_), .c(new_n155_), .d(new_n95_), .O(new_n157_));
  nand3  g106(.a(new_n157_), .b(new_n154_), .c(x16), .O(new_n158_));
  aoi21  g107(.a(new_n56_), .b(x02), .c(x18), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n150_), .O(z13));
  nor2   g110(.a(x33), .b(x32), .O(new_n162_));
  nand4  g111(.a(new_n162_), .b(new_n139_), .c(new_n138_), .d(new_n111_), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n98_), .O(new_n164_));
  aoi21  g113(.a(new_n157_), .b(x33), .c(new_n164_), .O(new_n165_));
  inv1   g114(.a(x01), .O(new_n166_));
  aoi21  g115(.a(new_n56_), .b(new_n166_), .c(x18), .O(new_n167_));
  oai21  g116(.a(new_n165_), .b(new_n56_), .c(new_n167_), .O(z14));
  inv1   g117(.a(x34), .O(new_n169_));
  oai21  g118(.a(new_n169_), .b(new_n56_), .c(new_n60_), .O(new_n170_));
  nand2  g119(.a(new_n170_), .b(x30), .O(new_n171_));
  nand4  g120(.a(new_n162_), .b(new_n87_), .c(new_n73_), .d(new_n62_), .O(new_n172_));
  oai21  g121(.a(new_n172_), .b(new_n151_), .c(x34), .O(new_n173_));
  inv1   g122(.a(new_n163_), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n97_), .c(new_n82_), .d(new_n169_), .O(new_n175_));
  nand3  g124(.a(new_n175_), .b(new_n173_), .c(x16), .O(new_n176_));
  aoi21  g125(.a(new_n56_), .b(x00), .c(x18), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n171_), .O(z15));
endmodule


