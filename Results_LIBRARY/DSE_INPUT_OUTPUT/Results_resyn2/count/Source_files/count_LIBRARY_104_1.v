// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x28), .b(x18), .O(new_n58_));
  nand3  g007(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  inv1   g009(.a(x20), .O(new_n61_));
  nand2  g010(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  oai21  g011(.a(x19), .b(x17), .c(x20), .O(new_n63_));
  nand3  g012(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(x28), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n60_), .O(z01));
  nor2   g016(.a(x21), .b(x20), .O(new_n68_));
  aoi22  g017(.a(new_n68_), .b(new_n53_), .c(new_n62_), .d(x21), .O(new_n69_));
  inv1   g018(.a(new_n58_), .O(new_n70_));
  nor2   g019(.a(x16), .b(x13), .O(new_n71_));
  nor2   g020(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g021(.a(new_n69_), .b(new_n56_), .c(new_n72_), .O(z02));
  inv1   g022(.a(x22), .O(new_n74_));
  nand3  g023(.a(new_n68_), .b(new_n53_), .c(new_n74_), .O(new_n75_));
  nand2  g024(.a(new_n68_), .b(new_n53_), .O(new_n76_));
  nand2  g025(.a(new_n76_), .b(x22), .O(new_n77_));
  nand3  g026(.a(new_n77_), .b(new_n75_), .c(x16), .O(new_n78_));
  aoi21  g027(.a(new_n56_), .b(x12), .c(x28), .O(new_n79_));
  nand2  g028(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g029(.a(new_n80_), .b(new_n60_), .O(z03));
  nor2   g030(.a(x23), .b(x22), .O(new_n82_));
  nand3  g031(.a(new_n82_), .b(new_n68_), .c(new_n53_), .O(new_n83_));
  inv1   g032(.a(new_n83_), .O(new_n84_));
  aoi21  g033(.a(new_n75_), .b(x23), .c(new_n84_), .O(new_n85_));
  nor2   g034(.a(x16), .b(x11), .O(new_n86_));
  nor2   g035(.a(new_n86_), .b(new_n70_), .O(new_n87_));
  oai21  g036(.a(new_n85_), .b(new_n56_), .c(new_n87_), .O(z04));
  nor2   g037(.a(x24), .b(x23), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n68_), .c(new_n53_), .d(new_n74_), .O(new_n90_));
  nand2  g039(.a(new_n83_), .b(x24), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  aoi21  g041(.a(new_n56_), .b(x10), .c(x28), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n60_), .O(z05));
  inv1   g044(.a(x25), .O(new_n96_));
  nand2  g045(.a(new_n89_), .b(new_n96_), .O(new_n97_));
  nor2   g046(.a(new_n97_), .b(new_n75_), .O(new_n98_));
  aoi21  g047(.a(new_n90_), .b(x25), .c(new_n98_), .O(new_n99_));
  nor2   g048(.a(x16), .b(x09), .O(new_n100_));
  nor2   g049(.a(new_n100_), .b(new_n70_), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n56_), .c(new_n101_), .O(z06));
  inv1   g051(.a(x26), .O(new_n103_));
  nand2  g052(.a(new_n98_), .b(new_n103_), .O(new_n104_));
  oai21  g053(.a(new_n97_), .b(new_n75_), .c(x26), .O(new_n105_));
  nand3  g054(.a(new_n105_), .b(new_n104_), .c(x16), .O(new_n106_));
  aoi21  g055(.a(new_n56_), .b(x08), .c(x28), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g057(.a(new_n108_), .b(new_n60_), .O(z07));
  inv1   g058(.a(x27), .O(new_n110_));
  aoi21  g059(.a(new_n98_), .b(new_n103_), .c(new_n110_), .O(new_n111_));
  inv1   g060(.a(x28), .O(new_n112_));
  inv1   g061(.a(new_n75_), .O(new_n113_));
  inv1   g062(.a(new_n97_), .O(new_n114_));
  nor2   g063(.a(x27), .b(x26), .O(new_n115_));
  nand4  g064(.a(new_n115_), .b(new_n114_), .c(new_n113_), .d(new_n112_), .O(new_n116_));
  inv1   g065(.a(new_n116_), .O(new_n117_));
  oai21  g066(.a(new_n117_), .b(new_n111_), .c(x16), .O(new_n118_));
  nor2   g067(.a(x16), .b(x07), .O(new_n119_));
  nor2   g068(.a(new_n119_), .b(new_n70_), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(z08));
  aoi21  g070(.a(new_n115_), .b(new_n98_), .c(new_n56_), .O(new_n122_));
  inv1   g071(.a(x06), .O(new_n123_));
  oai21  g072(.a(x16), .b(new_n123_), .c(new_n112_), .O(new_n124_));
  oai21  g073(.a(new_n124_), .b(new_n122_), .c(new_n60_), .O(z09));
  nand3  g074(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n126_));
  xor2a  g075(.a(new_n126_), .b(x29), .O(new_n127_));
  nor2   g076(.a(x16), .b(x05), .O(new_n128_));
  nor2   g077(.a(new_n128_), .b(new_n70_), .O(new_n129_));
  oai21  g078(.a(new_n127_), .b(new_n56_), .c(new_n129_), .O(z10));
  inv1   g079(.a(x29), .O(new_n131_));
  nand3  g080(.a(new_n115_), .b(new_n98_), .c(new_n131_), .O(new_n132_));
  nor2   g081(.a(x30), .b(x29), .O(new_n133_));
  nor2   g082(.a(x25), .b(x24), .O(new_n134_));
  nand3  g083(.a(new_n134_), .b(new_n133_), .c(new_n115_), .O(new_n135_));
  nor2   g084(.a(new_n135_), .b(new_n83_), .O(new_n136_));
  aoi21  g085(.a(new_n132_), .b(x30), .c(new_n136_), .O(new_n137_));
  nor2   g086(.a(x16), .b(x04), .O(new_n138_));
  nor2   g087(.a(new_n138_), .b(new_n70_), .O(new_n139_));
  oai21  g088(.a(new_n137_), .b(new_n56_), .c(new_n139_), .O(z11));
  oai21  g089(.a(new_n135_), .b(new_n83_), .c(x31), .O(new_n141_));
  inv1   g090(.a(x31), .O(new_n142_));
  nand2  g091(.a(new_n136_), .b(new_n142_), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n141_), .c(x16), .O(new_n144_));
  aoi21  g093(.a(new_n56_), .b(x03), .c(x28), .O(new_n145_));
  nand2  g094(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g095(.a(new_n146_), .b(new_n60_), .O(z12));
  inv1   g096(.a(x32), .O(new_n148_));
  aoi21  g097(.a(new_n136_), .b(new_n142_), .c(new_n148_), .O(new_n149_));
  nor2   g098(.a(x32), .b(x31), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n133_), .O(new_n151_));
  inv1   g100(.a(new_n151_), .O(new_n152_));
  nand4  g101(.a(new_n152_), .b(new_n115_), .c(new_n114_), .d(new_n113_), .O(new_n153_));
  inv1   g102(.a(new_n153_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n149_), .c(x16), .O(new_n155_));
  nor2   g104(.a(x16), .b(x02), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n70_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n155_), .O(z13));
  nor2   g107(.a(x33), .b(x25), .O(new_n159_));
  nand4  g108(.a(new_n159_), .b(new_n150_), .c(new_n133_), .d(new_n115_), .O(new_n160_));
  nor2   g109(.a(new_n160_), .b(new_n90_), .O(new_n161_));
  aoi21  g110(.a(new_n153_), .b(x33), .c(new_n161_), .O(new_n162_));
  nor2   g111(.a(x16), .b(x01), .O(new_n163_));
  nor2   g112(.a(new_n163_), .b(new_n70_), .O(new_n164_));
  oai21  g113(.a(new_n162_), .b(new_n56_), .c(new_n164_), .O(z14));
  oai21  g114(.a(new_n160_), .b(new_n90_), .c(x34), .O(new_n166_));
  nor2   g115(.a(x34), .b(x33), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n152_), .c(new_n115_), .d(new_n98_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n166_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(x16), .O(new_n170_));
  nor2   g119(.a(x16), .b(x00), .O(new_n171_));
  nor2   g120(.a(new_n171_), .b(new_n70_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n170_), .O(z15));
endmodule


