// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x18), .b(x14), .O(new_n58_));
  nand3  g007(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g008(.a(x20), .O(new_n60_));
  inv1   g009(.a(new_n58_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  nor3   g011(.a(new_n62_), .b(new_n61_), .c(new_n56_), .O(new_n63_));
  oai21  g012(.a(new_n53_), .b(new_n60_), .c(new_n63_), .O(z01));
  inv1   g013(.a(x21), .O(new_n65_));
  xor2a  g014(.a(new_n62_), .b(new_n65_), .O(new_n66_));
  nor2   g015(.a(x16), .b(x13), .O(new_n67_));
  nor2   g016(.a(new_n67_), .b(new_n61_), .O(new_n68_));
  oai21  g017(.a(new_n66_), .b(new_n56_), .c(new_n68_), .O(z02));
  nand2  g018(.a(new_n62_), .b(new_n65_), .O(new_n70_));
  nor2   g019(.a(x22), .b(x21), .O(new_n71_));
  nand3  g020(.a(new_n71_), .b(new_n53_), .c(new_n60_), .O(new_n72_));
  inv1   g021(.a(new_n72_), .O(new_n73_));
  aoi21  g022(.a(new_n70_), .b(x22), .c(new_n73_), .O(new_n74_));
  nor2   g023(.a(x16), .b(x12), .O(new_n75_));
  nor2   g024(.a(new_n75_), .b(new_n61_), .O(new_n76_));
  oai21  g025(.a(new_n74_), .b(new_n56_), .c(new_n76_), .O(z03));
  inv1   g026(.a(x18), .O(new_n78_));
  inv1   g027(.a(x23), .O(new_n79_));
  nand4  g028(.a(new_n71_), .b(new_n53_), .c(new_n79_), .d(new_n60_), .O(new_n80_));
  nand2  g029(.a(new_n72_), .b(x23), .O(new_n81_));
  nand3  g030(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  aoi21  g031(.a(new_n56_), .b(x11), .c(x14), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(new_n78_), .O(z04));
  nand2  g034(.a(new_n80_), .b(x24), .O(new_n86_));
  nor2   g035(.a(x24), .b(x23), .O(new_n87_));
  nand4  g036(.a(new_n87_), .b(new_n71_), .c(new_n53_), .d(new_n60_), .O(new_n88_));
  nand3  g037(.a(new_n88_), .b(new_n86_), .c(x16), .O(new_n89_));
  aoi21  g038(.a(new_n56_), .b(x10), .c(x14), .O(new_n90_));
  nand2  g039(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g040(.a(new_n91_), .b(new_n78_), .O(z05));
  nand2  g041(.a(new_n88_), .b(x25), .O(new_n93_));
  nor2   g042(.a(x25), .b(x22), .O(new_n94_));
  nand4  g043(.a(new_n94_), .b(new_n87_), .c(new_n62_), .d(new_n65_), .O(new_n95_));
  aoi21  g044(.a(new_n95_), .b(new_n93_), .c(new_n56_), .O(new_n96_));
  oai21  g045(.a(x16), .b(x09), .c(new_n58_), .O(new_n97_));
  or2    g046(.a(new_n97_), .b(new_n96_), .O(z06));
  nand2  g047(.a(new_n95_), .b(x26), .O(new_n99_));
  nor2   g048(.a(x26), .b(x25), .O(new_n100_));
  nand4  g049(.a(new_n100_), .b(new_n87_), .c(new_n71_), .d(new_n62_), .O(new_n101_));
  nand3  g050(.a(new_n101_), .b(new_n99_), .c(x16), .O(new_n102_));
  aoi21  g051(.a(new_n56_), .b(x08), .c(x14), .O(new_n103_));
  nand2  g052(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n78_), .O(z07));
  inv1   g054(.a(x27), .O(new_n106_));
  nand3  g055(.a(new_n100_), .b(new_n87_), .c(new_n106_), .O(new_n107_));
  nor2   g056(.a(new_n107_), .b(new_n72_), .O(new_n108_));
  aoi21  g057(.a(new_n101_), .b(x27), .c(new_n108_), .O(new_n109_));
  nor2   g058(.a(x16), .b(x07), .O(new_n110_));
  nor2   g059(.a(new_n110_), .b(new_n61_), .O(new_n111_));
  oai21  g060(.a(new_n109_), .b(new_n56_), .c(new_n111_), .O(z08));
  inv1   g061(.a(x28), .O(new_n113_));
  nor2   g062(.a(new_n108_), .b(new_n113_), .O(new_n114_));
  nand3  g063(.a(new_n100_), .b(new_n113_), .c(new_n106_), .O(new_n115_));
  nor2   g064(.a(new_n115_), .b(new_n88_), .O(new_n116_));
  oai21  g065(.a(new_n116_), .b(new_n114_), .c(x16), .O(new_n117_));
  nor2   g066(.a(x16), .b(x06), .O(new_n118_));
  nor2   g067(.a(new_n118_), .b(new_n61_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n117_), .O(z09));
  oai21  g069(.a(new_n115_), .b(new_n88_), .c(x29), .O(new_n121_));
  inv1   g070(.a(x24), .O(new_n122_));
  nor2   g071(.a(x29), .b(x28), .O(new_n123_));
  nand4  g072(.a(new_n123_), .b(new_n100_), .c(new_n106_), .d(new_n122_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n80_), .O(new_n125_));
  inv1   g074(.a(new_n125_), .O(new_n126_));
  nand3  g075(.a(new_n126_), .b(new_n121_), .c(x16), .O(new_n127_));
  aoi21  g076(.a(new_n56_), .b(x05), .c(x14), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n78_), .O(z10));
  inv1   g079(.a(x30), .O(new_n131_));
  xor2a  g080(.a(new_n125_), .b(new_n131_), .O(new_n132_));
  nor2   g081(.a(x16), .b(x04), .O(new_n133_));
  nor2   g082(.a(new_n133_), .b(new_n61_), .O(new_n134_));
  oai21  g083(.a(new_n132_), .b(new_n56_), .c(new_n134_), .O(z11));
  nand2  g084(.a(new_n125_), .b(new_n131_), .O(new_n136_));
  nor2   g085(.a(x31), .b(x30), .O(new_n137_));
  nand2  g086(.a(new_n137_), .b(new_n123_), .O(new_n138_));
  nor3   g087(.a(new_n138_), .b(new_n107_), .c(new_n72_), .O(new_n139_));
  aoi21  g088(.a(new_n136_), .b(x31), .c(new_n139_), .O(new_n140_));
  nor2   g089(.a(x16), .b(x03), .O(new_n141_));
  nor2   g090(.a(new_n141_), .b(new_n61_), .O(new_n142_));
  oai21  g091(.a(new_n140_), .b(new_n56_), .c(new_n142_), .O(z12));
  inv1   g092(.a(x32), .O(new_n144_));
  inv1   g093(.a(new_n138_), .O(new_n145_));
  aoi21  g094(.a(new_n145_), .b(new_n108_), .c(new_n144_), .O(new_n146_));
  nand4  g095(.a(new_n137_), .b(new_n123_), .c(new_n144_), .d(new_n106_), .O(new_n147_));
  nor2   g096(.a(new_n147_), .b(new_n101_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n146_), .c(x16), .O(new_n149_));
  nor2   g098(.a(x16), .b(x02), .O(new_n150_));
  nor2   g099(.a(new_n150_), .b(new_n61_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n149_), .O(z13));
  inv1   g101(.a(x33), .O(new_n153_));
  inv1   g102(.a(new_n100_), .O(new_n154_));
  nor2   g103(.a(new_n154_), .b(new_n88_), .O(new_n155_));
  inv1   g104(.a(new_n147_), .O(new_n156_));
  aoi21  g105(.a(new_n156_), .b(new_n155_), .c(new_n153_), .O(new_n157_));
  inv1   g106(.a(x26), .O(new_n158_));
  nand4  g107(.a(new_n153_), .b(new_n144_), .c(new_n106_), .d(new_n158_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n145_), .O(new_n161_));
  nor2   g110(.a(new_n161_), .b(new_n95_), .O(new_n162_));
  oai21  g111(.a(new_n162_), .b(new_n157_), .c(x16), .O(new_n163_));
  nor2   g112(.a(x16), .b(x01), .O(new_n164_));
  nor2   g113(.a(new_n164_), .b(new_n61_), .O(new_n165_));
  nand2  g114(.a(new_n165_), .b(new_n163_), .O(z14));
  inv1   g115(.a(new_n95_), .O(new_n167_));
  nand4  g116(.a(new_n160_), .b(new_n145_), .c(new_n167_), .d(x34), .O(new_n168_));
  inv1   g117(.a(x34), .O(new_n169_));
  oai21  g118(.a(new_n161_), .b(new_n95_), .c(new_n169_), .O(new_n170_));
  nand3  g119(.a(new_n170_), .b(new_n168_), .c(x16), .O(new_n171_));
  nor2   g120(.a(x16), .b(x00), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n61_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n171_), .O(z15));
endmodule


