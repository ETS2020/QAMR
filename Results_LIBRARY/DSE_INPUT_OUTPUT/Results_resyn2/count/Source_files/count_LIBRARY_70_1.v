// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  nand2  g005(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g006(.a(x29), .b(x18), .O(new_n58_));
  nand3  g007(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(z00));
  inv1   g008(.a(x18), .O(new_n60_));
  nor3   g009(.a(x20), .b(x19), .c(x17), .O(new_n61_));
  inv1   g010(.a(x20), .O(new_n62_));
  oai21  g011(.a(new_n53_), .b(new_n62_), .c(x16), .O(new_n63_));
  aoi21  g012(.a(new_n56_), .b(x14), .c(x29), .O(new_n64_));
  oai21  g013(.a(new_n63_), .b(new_n61_), .c(new_n64_), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(new_n60_), .O(z01));
  inv1   g015(.a(x21), .O(new_n67_));
  nor2   g016(.a(new_n61_), .b(new_n67_), .O(new_n68_));
  nor2   g017(.a(x21), .b(x20), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(x16), .O(new_n71_));
  aoi21  g020(.a(new_n56_), .b(x13), .c(x29), .O(new_n72_));
  oai21  g021(.a(new_n71_), .b(new_n68_), .c(new_n72_), .O(new_n73_));
  nand2  g022(.a(new_n73_), .b(new_n60_), .O(z02));
  xor2a  g023(.a(new_n70_), .b(x22), .O(new_n75_));
  inv1   g024(.a(new_n58_), .O(new_n76_));
  nor2   g025(.a(x16), .b(x12), .O(new_n77_));
  nor2   g026(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  oai21  g027(.a(new_n75_), .b(new_n56_), .c(new_n78_), .O(z03));
  inv1   g028(.a(x22), .O(new_n80_));
  nand3  g029(.a(new_n69_), .b(new_n53_), .c(new_n80_), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(x23), .O(new_n82_));
  nor2   g031(.a(x23), .b(x22), .O(new_n83_));
  nand3  g032(.a(new_n83_), .b(new_n69_), .c(new_n53_), .O(new_n84_));
  nand3  g033(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  aoi21  g034(.a(new_n56_), .b(x11), .c(x29), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n60_), .O(z04));
  nand2  g037(.a(new_n84_), .b(x24), .O(new_n89_));
  nor2   g038(.a(x24), .b(x21), .O(new_n90_));
  nand4  g039(.a(new_n90_), .b(new_n83_), .c(new_n53_), .d(new_n62_), .O(new_n91_));
  nand3  g040(.a(new_n91_), .b(new_n89_), .c(x16), .O(new_n92_));
  aoi21  g041(.a(new_n56_), .b(x10), .c(x29), .O(new_n93_));
  nand2  g042(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g043(.a(new_n94_), .b(new_n60_), .O(z05));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n83_), .c(new_n69_), .d(new_n53_), .O(new_n97_));
  nand2  g046(.a(new_n91_), .b(x25), .O(new_n98_));
  nand3  g047(.a(new_n98_), .b(new_n97_), .c(x16), .O(new_n99_));
  aoi21  g048(.a(new_n56_), .b(x09), .c(x29), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g050(.a(new_n101_), .b(new_n60_), .O(z06));
  inv1   g051(.a(x24), .O(new_n103_));
  nor2   g052(.a(x26), .b(x25), .O(new_n104_));
  nand2  g053(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nor2   g054(.a(new_n105_), .b(new_n84_), .O(new_n106_));
  aoi21  g055(.a(new_n97_), .b(x26), .c(new_n106_), .O(new_n107_));
  nor2   g056(.a(x16), .b(x08), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n76_), .O(new_n109_));
  oai21  g058(.a(new_n107_), .b(new_n56_), .c(new_n109_), .O(z07));
  inv1   g059(.a(x27), .O(new_n111_));
  nor2   g060(.a(new_n106_), .b(new_n111_), .O(new_n112_));
  nor3   g061(.a(x27), .b(x26), .c(x25), .O(new_n113_));
  nand4  g062(.a(new_n113_), .b(new_n90_), .c(new_n83_), .d(new_n61_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(x16), .O(new_n115_));
  aoi21  g064(.a(new_n56_), .b(x07), .c(x29), .O(new_n116_));
  oai21  g065(.a(new_n115_), .b(new_n112_), .c(new_n116_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(new_n60_), .O(z08));
  inv1   g067(.a(x28), .O(new_n119_));
  nor2   g068(.a(x27), .b(x23), .O(new_n120_));
  nand4  g069(.a(new_n120_), .b(new_n104_), .c(new_n119_), .d(new_n103_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n81_), .O(new_n122_));
  aoi21  g071(.a(new_n114_), .b(x28), .c(new_n122_), .O(new_n123_));
  nor2   g072(.a(x16), .b(x06), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n76_), .O(new_n125_));
  oai21  g074(.a(new_n123_), .b(new_n56_), .c(new_n125_), .O(z09));
  aoi21  g075(.a(new_n56_), .b(x05), .c(x29), .O(new_n127_));
  oai21  g076(.a(new_n122_), .b(new_n56_), .c(new_n127_), .O(new_n128_));
  nand2  g077(.a(new_n128_), .b(new_n60_), .O(z10));
  nor3   g078(.a(x30), .b(x28), .c(x27), .O(new_n130_));
  nand2  g079(.a(new_n130_), .b(new_n106_), .O(new_n131_));
  oai21  g080(.a(new_n121_), .b(new_n81_), .c(x30), .O(new_n132_));
  nand3  g081(.a(new_n132_), .b(new_n131_), .c(x16), .O(new_n133_));
  aoi21  g082(.a(new_n56_), .b(x04), .c(x29), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(new_n60_), .O(z11));
  inv1   g085(.a(x30), .O(new_n137_));
  inv1   g086(.a(x31), .O(new_n138_));
  nand4  g087(.a(new_n138_), .b(new_n137_), .c(new_n119_), .d(new_n111_), .O(new_n139_));
  nor3   g088(.a(new_n139_), .b(new_n105_), .c(new_n84_), .O(new_n140_));
  aoi21  g089(.a(new_n131_), .b(x31), .c(new_n140_), .O(new_n141_));
  nor2   g090(.a(x16), .b(x03), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n76_), .O(new_n143_));
  oai21  g092(.a(new_n141_), .b(new_n56_), .c(new_n143_), .O(z12));
  inv1   g093(.a(x32), .O(new_n145_));
  inv1   g094(.a(new_n139_), .O(new_n146_));
  aoi21  g095(.a(new_n146_), .b(new_n106_), .c(new_n145_), .O(new_n147_));
  nand4  g096(.a(new_n145_), .b(new_n138_), .c(new_n137_), .d(new_n119_), .O(new_n148_));
  oai21  g097(.a(new_n148_), .b(new_n114_), .c(x16), .O(new_n149_));
  aoi21  g098(.a(new_n56_), .b(x02), .c(x29), .O(new_n150_));
  oai21  g099(.a(new_n149_), .b(new_n147_), .c(new_n150_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n60_), .O(z13));
  inv1   g101(.a(x33), .O(new_n153_));
  inv1   g102(.a(x25), .O(new_n154_));
  inv1   g103(.a(x26), .O(new_n155_));
  nand3  g104(.a(new_n111_), .b(new_n155_), .c(new_n154_), .O(new_n156_));
  nor2   g105(.a(new_n156_), .b(new_n91_), .O(new_n157_));
  inv1   g106(.a(new_n148_), .O(new_n158_));
  aoi21  g107(.a(new_n158_), .b(new_n157_), .c(new_n153_), .O(new_n159_));
  nand2  g108(.a(new_n90_), .b(new_n83_), .O(new_n160_));
  inv1   g109(.a(new_n160_), .O(new_n161_));
  nand4  g110(.a(new_n153_), .b(new_n145_), .c(new_n155_), .d(new_n154_), .O(new_n162_));
  inv1   g111(.a(new_n162_), .O(new_n163_));
  nand4  g112(.a(new_n163_), .b(new_n146_), .c(new_n161_), .d(new_n61_), .O(new_n164_));
  nand2  g113(.a(new_n164_), .b(x16), .O(new_n165_));
  aoi21  g114(.a(new_n56_), .b(x01), .c(x29), .O(new_n166_));
  oai21  g115(.a(new_n165_), .b(new_n159_), .c(new_n166_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n60_), .O(z14));
  inv1   g117(.a(x34), .O(new_n169_));
  nand4  g118(.a(new_n169_), .b(new_n153_), .c(new_n111_), .d(new_n155_), .O(new_n170_));
  nor3   g119(.a(new_n170_), .b(new_n148_), .c(new_n97_), .O(new_n171_));
  aoi21  g120(.a(new_n164_), .b(x34), .c(new_n171_), .O(new_n172_));
  nor2   g121(.a(x16), .b(x00), .O(new_n173_));
  nor2   g122(.a(new_n173_), .b(new_n76_), .O(new_n174_));
  oai21  g123(.a(new_n172_), .b(new_n56_), .c(new_n174_), .O(z15));
endmodule


