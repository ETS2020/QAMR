// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  inv1   g005(.a(x18), .O(new_n57_));
  inv1   g006(.a(x27), .O(new_n58_));
  inv1   g007(.a(x32), .O(new_n59_));
  nand2  g008(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g009(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  aoi21  g010(.a(new_n56_), .b(new_n55_), .c(new_n61_), .O(new_n62_));
  nand2  g011(.a(new_n62_), .b(new_n54_), .O(z00));
  inv1   g012(.a(new_n60_), .O(new_n64_));
  xor2a  g013(.a(new_n53_), .b(x20), .O(new_n65_));
  nand2  g014(.a(new_n65_), .b(x16), .O(new_n66_));
  inv1   g015(.a(x14), .O(new_n67_));
  aoi21  g016(.a(new_n56_), .b(new_n67_), .c(x18), .O(new_n68_));
  aoi21  g017(.a(new_n68_), .b(new_n66_), .c(new_n64_), .O(z01));
  inv1   g018(.a(x20), .O(new_n70_));
  nand2  g019(.a(new_n53_), .b(new_n70_), .O(new_n71_));
  nor2   g020(.a(x21), .b(x20), .O(new_n72_));
  aoi22  g021(.a(new_n72_), .b(new_n53_), .c(new_n71_), .d(x21), .O(new_n73_));
  nor2   g022(.a(x16), .b(x13), .O(new_n74_));
  nor2   g023(.a(new_n74_), .b(new_n61_), .O(new_n75_));
  oai21  g024(.a(new_n73_), .b(new_n56_), .c(new_n75_), .O(z02));
  inv1   g025(.a(x22), .O(new_n77_));
  aoi21  g026(.a(new_n72_), .b(new_n53_), .c(new_n77_), .O(new_n78_));
  nand3  g027(.a(new_n72_), .b(new_n53_), .c(new_n77_), .O(new_n79_));
  inv1   g028(.a(new_n79_), .O(new_n80_));
  oai21  g029(.a(new_n80_), .b(new_n78_), .c(x16), .O(new_n81_));
  nor2   g030(.a(x16), .b(x12), .O(new_n82_));
  nor2   g031(.a(new_n82_), .b(new_n61_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n81_), .O(z03));
  nor2   g033(.a(x23), .b(x22), .O(new_n85_));
  nand3  g034(.a(new_n85_), .b(new_n72_), .c(new_n53_), .O(new_n86_));
  inv1   g035(.a(new_n86_), .O(new_n87_));
  aoi21  g036(.a(new_n79_), .b(x23), .c(new_n87_), .O(new_n88_));
  nor2   g037(.a(x16), .b(x11), .O(new_n89_));
  nor2   g038(.a(new_n89_), .b(new_n61_), .O(new_n90_));
  oai21  g039(.a(new_n88_), .b(new_n56_), .c(new_n90_), .O(z04));
  xor2a  g040(.a(new_n86_), .b(x24), .O(new_n92_));
  nor2   g041(.a(x16), .b(x10), .O(new_n93_));
  nor2   g042(.a(new_n93_), .b(new_n61_), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n56_), .c(new_n94_), .O(z05));
  oai21  g044(.a(new_n86_), .b(x24), .c(x25), .O(new_n96_));
  inv1   g045(.a(x24), .O(new_n97_));
  inv1   g046(.a(x25), .O(new_n98_));
  nand3  g047(.a(new_n87_), .b(new_n98_), .c(new_n97_), .O(new_n99_));
  nand2  g048(.a(new_n99_), .b(new_n96_), .O(new_n100_));
  nand2  g049(.a(new_n100_), .b(x16), .O(new_n101_));
  inv1   g050(.a(x09), .O(new_n102_));
  aoi21  g051(.a(new_n56_), .b(new_n102_), .c(x18), .O(new_n103_));
  aoi21  g052(.a(new_n103_), .b(new_n101_), .c(new_n64_), .O(z06));
  nor2   g053(.a(x26), .b(x25), .O(new_n105_));
  nand2  g054(.a(new_n105_), .b(new_n97_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n87_), .O(new_n108_));
  nor2   g057(.a(new_n108_), .b(new_n56_), .O(new_n109_));
  nor2   g058(.a(new_n109_), .b(x18), .O(new_n110_));
  nand3  g059(.a(new_n99_), .b(x26), .c(x16), .O(new_n111_));
  oai21  g060(.a(x16), .b(x08), .c(new_n57_), .O(new_n112_));
  nand2  g061(.a(new_n105_), .b(x16), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g063(.a(new_n114_), .b(new_n111_), .c(new_n110_), .d(new_n60_), .O(z07));
  oai21  g064(.a(new_n109_), .b(new_n59_), .c(new_n58_), .O(new_n116_));
  nor2   g065(.a(new_n58_), .b(new_n56_), .O(new_n117_));
  oai21  g066(.a(x16), .b(x07), .c(new_n57_), .O(new_n118_));
  aoi21  g067(.a(new_n117_), .b(new_n108_), .c(new_n118_), .O(new_n119_));
  nand2  g068(.a(new_n119_), .b(new_n116_), .O(z08));
  nor2   g069(.a(new_n106_), .b(new_n86_), .O(new_n121_));
  nand3  g070(.a(new_n121_), .b(x28), .c(new_n58_), .O(new_n122_));
  inv1   g071(.a(x28), .O(new_n123_));
  oai21  g072(.a(new_n108_), .b(x27), .c(new_n123_), .O(new_n124_));
  nand3  g073(.a(new_n124_), .b(new_n122_), .c(x16), .O(new_n125_));
  inv1   g074(.a(x06), .O(new_n126_));
  aoi21  g075(.a(new_n56_), .b(new_n126_), .c(x18), .O(new_n127_));
  aoi21  g076(.a(new_n127_), .b(new_n125_), .c(new_n64_), .O(z09));
  oai21  g077(.a(x16), .b(x05), .c(new_n57_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n60_), .O(new_n130_));
  inv1   g079(.a(x29), .O(new_n131_));
  nand3  g080(.a(new_n105_), .b(new_n123_), .c(new_n97_), .O(new_n132_));
  oai21  g081(.a(new_n132_), .b(new_n86_), .c(x32), .O(new_n133_));
  aoi21  g082(.a(new_n133_), .b(new_n58_), .c(new_n131_), .O(new_n134_));
  nand3  g083(.a(x32), .b(new_n131_), .c(new_n123_), .O(new_n135_));
  inv1   g084(.a(new_n135_), .O(new_n136_));
  nand3  g085(.a(new_n136_), .b(new_n121_), .c(new_n58_), .O(new_n137_));
  inv1   g086(.a(new_n137_), .O(new_n138_));
  oai21  g087(.a(new_n138_), .b(new_n134_), .c(x16), .O(new_n139_));
  nand2  g088(.a(new_n139_), .b(new_n130_), .O(z10));
  nand3  g089(.a(new_n137_), .b(x30), .c(x16), .O(new_n141_));
  inv1   g090(.a(x23), .O(new_n142_));
  nor2   g091(.a(x30), .b(x29), .O(new_n143_));
  nand3  g092(.a(new_n143_), .b(new_n142_), .c(x16), .O(new_n144_));
  nor3   g093(.a(new_n144_), .b(new_n132_), .c(new_n79_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n59_), .c(new_n58_), .O(new_n146_));
  inv1   g095(.a(x04), .O(new_n147_));
  aoi21  g096(.a(new_n56_), .b(new_n147_), .c(x18), .O(new_n148_));
  nand3  g097(.a(new_n148_), .b(new_n146_), .c(new_n141_), .O(z11));
  oai21  g098(.a(x16), .b(x03), .c(new_n57_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n60_), .O(new_n151_));
  inv1   g100(.a(x31), .O(new_n152_));
  nand4  g101(.a(new_n143_), .b(new_n105_), .c(new_n123_), .d(new_n97_), .O(new_n153_));
  oai21  g102(.a(new_n153_), .b(new_n86_), .c(x32), .O(new_n154_));
  aoi21  g103(.a(new_n154_), .b(new_n58_), .c(new_n152_), .O(new_n155_));
  nand3  g104(.a(new_n143_), .b(new_n152_), .c(new_n123_), .O(new_n156_));
  inv1   g105(.a(new_n156_), .O(new_n157_));
  nand4  g106(.a(new_n157_), .b(new_n107_), .c(new_n87_), .d(new_n58_), .O(new_n158_));
  nor2   g107(.a(new_n158_), .b(new_n59_), .O(new_n159_));
  oai21  g108(.a(new_n159_), .b(new_n155_), .c(x16), .O(new_n160_));
  nand2  g109(.a(new_n160_), .b(new_n151_), .O(z12));
  inv1   g110(.a(x02), .O(new_n162_));
  aoi21  g111(.a(new_n56_), .b(new_n162_), .c(x18), .O(new_n163_));
  nand3  g112(.a(new_n158_), .b(x32), .c(x16), .O(new_n164_));
  oai21  g113(.a(new_n163_), .b(new_n64_), .c(new_n164_), .O(z13));
  nand2  g114(.a(x33), .b(x16), .O(new_n166_));
  inv1   g115(.a(x01), .O(new_n167_));
  aoi21  g116(.a(new_n56_), .b(new_n167_), .c(x18), .O(new_n168_));
  aoi21  g117(.a(new_n168_), .b(new_n166_), .c(new_n64_), .O(z14));
  nand2  g118(.a(x34), .b(x16), .O(new_n170_));
  inv1   g119(.a(x00), .O(new_n171_));
  aoi21  g120(.a(new_n56_), .b(new_n171_), .c(x18), .O(new_n172_));
  aoi21  g121(.a(new_n172_), .b(new_n170_), .c(new_n64_), .O(z15));
endmodule


