// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_;
  and2   g000(.a(x19), .b(x17), .O(new_n52_));
  nor2   g001(.a(x19), .b(x17), .O(new_n53_));
  oai21  g002(.a(new_n53_), .b(new_n52_), .c(x16), .O(new_n54_));
  inv1   g003(.a(x15), .O(new_n55_));
  inv1   g004(.a(x16), .O(new_n56_));
  aoi21  g005(.a(new_n56_), .b(new_n55_), .c(x18), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(new_n54_), .O(z00));
  inv1   g007(.a(x18), .O(new_n59_));
  nor2   g008(.a(x30), .b(new_n59_), .O(new_n60_));
  inv1   g009(.a(new_n60_), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  inv1   g011(.a(x20), .O(new_n63_));
  oai21  g012(.a(new_n53_), .b(new_n63_), .c(x16), .O(new_n64_));
  aoi21  g013(.a(new_n56_), .b(x14), .c(x18), .O(new_n65_));
  oai21  g014(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  nand2  g015(.a(new_n66_), .b(new_n61_), .O(z01));
  inv1   g016(.a(x21), .O(new_n68_));
  xor2a  g017(.a(new_n62_), .b(new_n68_), .O(new_n69_));
  inv1   g018(.a(x13), .O(new_n70_));
  aoi21  g019(.a(new_n56_), .b(new_n70_), .c(x18), .O(new_n71_));
  oai21  g020(.a(new_n69_), .b(new_n56_), .c(new_n71_), .O(z02));
  nand2  g021(.a(new_n62_), .b(new_n68_), .O(new_n73_));
  nor2   g022(.a(x22), .b(x21), .O(new_n74_));
  aoi22  g023(.a(new_n74_), .b(new_n62_), .c(new_n73_), .d(x22), .O(new_n75_));
  inv1   g024(.a(x12), .O(new_n76_));
  aoi21  g025(.a(new_n56_), .b(new_n76_), .c(x18), .O(new_n77_));
  oai21  g026(.a(new_n75_), .b(new_n56_), .c(new_n77_), .O(z03));
  inv1   g027(.a(x23), .O(new_n79_));
  aoi21  g028(.a(new_n74_), .b(new_n62_), .c(new_n79_), .O(new_n80_));
  nor2   g029(.a(x23), .b(x20), .O(new_n81_));
  nand2  g030(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  inv1   g031(.a(new_n82_), .O(new_n83_));
  nand2  g032(.a(new_n83_), .b(new_n53_), .O(new_n84_));
  nand2  g033(.a(new_n84_), .b(x16), .O(new_n85_));
  aoi21  g034(.a(new_n56_), .b(x11), .c(x18), .O(new_n86_));
  oai21  g035(.a(new_n85_), .b(new_n80_), .c(new_n86_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n61_), .O(z04));
  nor2   g037(.a(x24), .b(x23), .O(new_n89_));
  nand4  g038(.a(new_n89_), .b(new_n74_), .c(new_n53_), .d(new_n63_), .O(new_n90_));
  inv1   g039(.a(new_n90_), .O(new_n91_));
  aoi21  g040(.a(new_n84_), .b(x24), .c(new_n91_), .O(new_n92_));
  inv1   g041(.a(x10), .O(new_n93_));
  aoi21  g042(.a(new_n56_), .b(new_n93_), .c(x18), .O(new_n94_));
  oai21  g043(.a(new_n92_), .b(new_n56_), .c(new_n94_), .O(z05));
  nor2   g044(.a(x25), .b(x24), .O(new_n96_));
  nand4  g045(.a(new_n96_), .b(new_n81_), .c(new_n74_), .d(new_n53_), .O(new_n97_));
  inv1   g046(.a(new_n97_), .O(new_n98_));
  aoi21  g047(.a(new_n90_), .b(x25), .c(new_n98_), .O(new_n99_));
  inv1   g048(.a(x09), .O(new_n100_));
  aoi21  g049(.a(new_n56_), .b(new_n100_), .c(x18), .O(new_n101_));
  oai21  g050(.a(new_n99_), .b(new_n56_), .c(new_n101_), .O(z06));
  nor2   g051(.a(new_n97_), .b(x26), .O(new_n103_));
  inv1   g052(.a(x26), .O(new_n104_));
  oai21  g053(.a(new_n98_), .b(new_n104_), .c(x16), .O(new_n105_));
  aoi21  g054(.a(new_n56_), .b(x08), .c(x18), .O(new_n106_));
  oai21  g055(.a(new_n105_), .b(new_n103_), .c(new_n106_), .O(new_n107_));
  nand2  g056(.a(new_n107_), .b(new_n61_), .O(z07));
  oai21  g057(.a(new_n97_), .b(x26), .c(x27), .O(new_n109_));
  nor3   g058(.a(x25), .b(x24), .c(x23), .O(new_n110_));
  nor2   g059(.a(x27), .b(x26), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n74_), .c(new_n53_), .d(new_n63_), .O(new_n112_));
  inv1   g061(.a(new_n112_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  nand2  g063(.a(new_n114_), .b(new_n109_), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(x16), .O(new_n116_));
  inv1   g065(.a(x07), .O(new_n117_));
  aoi21  g066(.a(new_n56_), .b(new_n117_), .c(x18), .O(new_n118_));
  nand2  g067(.a(new_n118_), .b(new_n116_), .O(z08));
  inv1   g068(.a(x28), .O(new_n120_));
  nand3  g069(.a(new_n96_), .b(new_n120_), .c(new_n79_), .O(new_n121_));
  nor2   g070(.a(new_n121_), .b(new_n112_), .O(new_n122_));
  aoi21  g071(.a(new_n114_), .b(x28), .c(new_n122_), .O(new_n123_));
  inv1   g072(.a(x06), .O(new_n124_));
  aoi21  g073(.a(new_n56_), .b(new_n124_), .c(x18), .O(new_n125_));
  oai21  g074(.a(new_n123_), .b(new_n56_), .c(new_n125_), .O(z09));
  inv1   g075(.a(x29), .O(new_n127_));
  nor2   g076(.a(new_n127_), .b(x18), .O(new_n128_));
  oai21  g077(.a(new_n121_), .b(new_n112_), .c(new_n128_), .O(new_n129_));
  nand2  g078(.a(x30), .b(x18), .O(new_n130_));
  nor2   g079(.a(x29), .b(x28), .O(new_n131_));
  nand2  g080(.a(new_n131_), .b(new_n111_), .O(new_n132_));
  inv1   g081(.a(new_n132_), .O(new_n133_));
  nand3  g082(.a(new_n133_), .b(new_n130_), .c(new_n98_), .O(new_n134_));
  nand2  g083(.a(new_n134_), .b(new_n129_), .O(new_n135_));
  nand2  g084(.a(new_n135_), .b(x16), .O(new_n136_));
  nor3   g085(.a(x18), .b(x16), .c(x05), .O(new_n137_));
  nor2   g086(.a(new_n137_), .b(new_n60_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n136_), .O(z10));
  inv1   g088(.a(x30), .O(new_n140_));
  nand4  g089(.a(new_n74_), .b(new_n53_), .c(new_n63_), .d(x16), .O(new_n141_));
  nand2  g090(.a(new_n133_), .b(new_n110_), .O(new_n142_));
  oai21  g091(.a(new_n142_), .b(new_n141_), .c(new_n59_), .O(new_n143_));
  nand2  g092(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nor2   g093(.a(new_n132_), .b(new_n97_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(new_n140_), .c(x16), .O(new_n146_));
  aoi21  g095(.a(new_n56_), .b(x04), .c(x18), .O(new_n147_));
  nand2  g096(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand2  g097(.a(new_n148_), .b(new_n144_), .O(z11));
  inv1   g098(.a(x31), .O(new_n150_));
  nor2   g099(.a(x26), .b(new_n56_), .O(new_n151_));
  nand4  g100(.a(new_n151_), .b(new_n81_), .c(new_n74_), .d(new_n150_), .O(new_n152_));
  inv1   g101(.a(x27), .O(new_n153_));
  nand4  g102(.a(new_n131_), .b(new_n96_), .c(new_n53_), .d(new_n153_), .O(new_n154_));
  oai21  g103(.a(new_n154_), .b(new_n152_), .c(new_n59_), .O(new_n155_));
  nand2  g104(.a(new_n155_), .b(new_n140_), .O(new_n156_));
  nand2  g105(.a(new_n96_), .b(new_n53_), .O(new_n157_));
  inv1   g106(.a(new_n157_), .O(new_n158_));
  nand4  g107(.a(new_n140_), .b(new_n127_), .c(new_n120_), .d(new_n153_), .O(new_n159_));
  inv1   g108(.a(new_n159_), .O(new_n160_));
  nand4  g109(.a(new_n160_), .b(new_n158_), .c(new_n83_), .d(new_n104_), .O(new_n161_));
  aoi21  g110(.a(new_n161_), .b(x31), .c(new_n56_), .O(new_n162_));
  inv1   g111(.a(x03), .O(new_n163_));
  oai21  g112(.a(x16), .b(new_n163_), .c(new_n59_), .O(new_n164_));
  oai21  g113(.a(new_n164_), .b(new_n162_), .c(new_n156_), .O(z12));
  inv1   g114(.a(x25), .O(new_n166_));
  nor2   g115(.a(x31), .b(x30), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n131_), .c(new_n111_), .d(new_n166_), .O(new_n168_));
  oai21  g117(.a(new_n168_), .b(new_n90_), .c(x32), .O(new_n169_));
  nand4  g118(.a(new_n120_), .b(new_n153_), .c(new_n104_), .d(new_n166_), .O(new_n170_));
  inv1   g119(.a(x32), .O(new_n171_));
  nand4  g120(.a(new_n171_), .b(new_n150_), .c(new_n140_), .d(new_n127_), .O(new_n172_));
  nor2   g121(.a(new_n172_), .b(new_n170_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n91_), .O(new_n174_));
  nand2  g123(.a(new_n174_), .b(new_n169_), .O(new_n175_));
  nand2  g124(.a(new_n175_), .b(x16), .O(new_n176_));
  inv1   g125(.a(x02), .O(new_n177_));
  aoi21  g126(.a(new_n56_), .b(new_n177_), .c(x18), .O(new_n178_));
  nand2  g127(.a(new_n178_), .b(new_n176_), .O(z13));
  nand2  g128(.a(x33), .b(x16), .O(new_n180_));
  aoi21  g129(.a(new_n173_), .b(new_n91_), .c(new_n180_), .O(new_n181_));
  nor2   g130(.a(x16), .b(x01), .O(new_n182_));
  oai21  g131(.a(new_n182_), .b(new_n181_), .c(new_n59_), .O(new_n183_));
  inv1   g132(.a(x33), .O(new_n184_));
  nor2   g133(.a(x32), .b(x29), .O(new_n185_));
  nand4  g134(.a(new_n185_), .b(new_n89_), .c(new_n184_), .d(new_n150_), .O(new_n186_));
  nor3   g135(.a(new_n186_), .b(new_n170_), .c(new_n141_), .O(new_n187_));
  oai21  g136(.a(new_n187_), .b(x18), .c(new_n140_), .O(new_n188_));
  nand2  g137(.a(new_n188_), .b(new_n183_), .O(z14));
  nand3  g138(.a(new_n167_), .b(new_n184_), .c(new_n171_), .O(new_n190_));
  inv1   g139(.a(new_n190_), .O(new_n191_));
  aoi21  g140(.a(new_n191_), .b(new_n145_), .c(x34), .O(new_n192_));
  nand3  g141(.a(new_n191_), .b(new_n145_), .c(x34), .O(new_n193_));
  nand2  g142(.a(new_n193_), .b(x16), .O(new_n194_));
  inv1   g143(.a(x00), .O(new_n195_));
  aoi21  g144(.a(new_n56_), .b(new_n195_), .c(x18), .O(new_n196_));
  oai21  g145(.a(new_n194_), .b(new_n192_), .c(new_n196_), .O(z15));
endmodule


