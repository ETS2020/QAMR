// Benchmark "FAU" written by ABC on Sat Aug  8 21:27:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_;
  inv1   g000(.a(x18), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g004(.a(x19), .b(x17), .c(x16), .O(new_n56_));
  inv1   g005(.a(x16), .O(new_n57_));
  nand2  g006(.a(new_n57_), .b(x15), .O(new_n58_));
  oai21  g007(.a(new_n56_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  inv1   g008(.a(x23), .O(new_n60_));
  nor2   g009(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  aoi21  g010(.a(new_n59_), .b(new_n52_), .c(new_n61_), .O(z00));
  nor3   g011(.a(x20), .b(x19), .c(x17), .O(new_n63_));
  inv1   g012(.a(x20), .O(new_n64_));
  nor2   g013(.a(x19), .b(x17), .O(new_n65_));
  nor2   g014(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  oai21  g015(.a(new_n66_), .b(new_n63_), .c(x16), .O(new_n67_));
  inv1   g016(.a(x14), .O(new_n68_));
  aoi21  g017(.a(new_n57_), .b(new_n68_), .c(x18), .O(new_n69_));
  nand2  g018(.a(new_n69_), .b(new_n67_), .O(z01));
  inv1   g019(.a(x21), .O(new_n71_));
  nor2   g020(.a(new_n63_), .b(new_n71_), .O(new_n72_));
  nand4  g021(.a(new_n71_), .b(new_n64_), .c(new_n54_), .d(new_n53_), .O(new_n73_));
  inv1   g022(.a(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(new_n72_), .c(x16), .O(new_n75_));
  inv1   g024(.a(x13), .O(new_n76_));
  aoi21  g025(.a(new_n57_), .b(new_n76_), .c(x18), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n75_), .O(z02));
  nor2   g027(.a(x22), .b(x21), .O(new_n79_));
  aoi22  g028(.a(new_n79_), .b(new_n63_), .c(new_n73_), .d(x22), .O(new_n80_));
  inv1   g029(.a(x12), .O(new_n81_));
  aoi21  g030(.a(new_n57_), .b(new_n81_), .c(x18), .O(new_n82_));
  oai21  g031(.a(new_n80_), .b(new_n57_), .c(new_n82_), .O(z03));
  inv1   g032(.a(x22), .O(new_n84_));
  nor2   g033(.a(x21), .b(x20), .O(new_n85_));
  nand4  g034(.a(new_n85_), .b(new_n65_), .c(new_n84_), .d(x16), .O(new_n86_));
  nand2  g035(.a(new_n86_), .b(new_n52_), .O(new_n87_));
  nand2  g036(.a(new_n87_), .b(new_n60_), .O(new_n88_));
  nand3  g037(.a(new_n79_), .b(new_n65_), .c(new_n64_), .O(new_n89_));
  nor2   g038(.a(x16), .b(x11), .O(new_n90_));
  nor2   g039(.a(new_n60_), .b(new_n57_), .O(new_n91_));
  aoi21  g040(.a(new_n91_), .b(new_n89_), .c(new_n90_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(x18), .c(new_n88_), .O(z04));
  nor2   g042(.a(x24), .b(new_n57_), .O(new_n94_));
  nand3  g043(.a(new_n94_), .b(new_n79_), .c(new_n63_), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n52_), .O(new_n96_));
  nand2  g045(.a(new_n96_), .b(new_n60_), .O(new_n97_));
  nor2   g046(.a(x16), .b(x10), .O(new_n98_));
  nor3   g047(.a(x23), .b(x22), .c(x21), .O(new_n99_));
  nand2  g048(.a(x24), .b(x16), .O(new_n100_));
  aoi21  g049(.a(new_n99_), .b(new_n63_), .c(new_n100_), .O(new_n101_));
  oai21  g050(.a(new_n101_), .b(new_n98_), .c(new_n52_), .O(new_n102_));
  nand2  g051(.a(new_n102_), .b(new_n97_), .O(z05));
  inv1   g052(.a(x24), .O(new_n104_));
  inv1   g053(.a(x25), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n104_), .c(new_n84_), .d(x16), .O(new_n106_));
  nor2   g055(.a(new_n106_), .b(new_n73_), .O(new_n107_));
  oai21  g056(.a(new_n107_), .b(x18), .c(new_n60_), .O(new_n108_));
  nor2   g057(.a(x16), .b(x09), .O(new_n109_));
  nor2   g058(.a(x23), .b(x22), .O(new_n110_));
  nor2   g059(.a(x24), .b(x21), .O(new_n111_));
  nand4  g060(.a(new_n111_), .b(new_n110_), .c(new_n65_), .d(new_n64_), .O(new_n112_));
  nor2   g061(.a(new_n105_), .b(new_n57_), .O(new_n113_));
  aoi21  g062(.a(new_n113_), .b(new_n112_), .c(new_n109_), .O(new_n114_));
  oai21  g063(.a(new_n114_), .b(x18), .c(new_n108_), .O(z06));
  inv1   g064(.a(x26), .O(new_n116_));
  nor2   g065(.a(x25), .b(x24), .O(new_n117_));
  nand4  g066(.a(new_n117_), .b(new_n110_), .c(new_n85_), .d(new_n65_), .O(new_n118_));
  inv1   g067(.a(new_n118_), .O(new_n119_));
  nor3   g068(.a(x26), .b(x25), .c(x24), .O(new_n120_));
  nand3  g069(.a(new_n120_), .b(new_n99_), .c(new_n63_), .O(new_n121_));
  oai21  g070(.a(new_n119_), .b(new_n116_), .c(new_n121_), .O(new_n122_));
  nand2  g071(.a(new_n122_), .b(x16), .O(new_n123_));
  inv1   g072(.a(x08), .O(new_n124_));
  aoi21  g073(.a(new_n57_), .b(new_n124_), .c(x18), .O(new_n125_));
  nand2  g074(.a(new_n125_), .b(new_n123_), .O(z07));
  nor2   g075(.a(x27), .b(x26), .O(new_n127_));
  nand2  g076(.a(new_n127_), .b(new_n105_), .O(new_n128_));
  oai21  g077(.a(new_n128_), .b(new_n95_), .c(new_n52_), .O(new_n129_));
  nand2  g078(.a(new_n129_), .b(new_n60_), .O(new_n130_));
  inv1   g079(.a(x27), .O(new_n131_));
  nor2   g080(.a(new_n131_), .b(new_n57_), .O(new_n132_));
  nor2   g081(.a(x16), .b(x07), .O(new_n133_));
  aoi21  g082(.a(new_n132_), .b(new_n121_), .c(new_n133_), .O(new_n134_));
  oai21  g083(.a(new_n134_), .b(x18), .c(new_n130_), .O(z08));
  nor2   g084(.a(x28), .b(x27), .O(new_n136_));
  nand2  g085(.a(new_n136_), .b(new_n120_), .O(new_n137_));
  oai21  g086(.a(new_n137_), .b(new_n86_), .c(new_n52_), .O(new_n138_));
  nand2  g087(.a(new_n138_), .b(new_n60_), .O(new_n139_));
  nor3   g088(.a(x25), .b(x24), .c(x23), .O(new_n140_));
  nand4  g089(.a(new_n140_), .b(new_n127_), .c(new_n79_), .d(new_n63_), .O(new_n141_));
  inv1   g090(.a(x28), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n57_), .O(new_n143_));
  nor2   g092(.a(x16), .b(x06), .O(new_n144_));
  aoi21  g093(.a(new_n143_), .b(new_n141_), .c(new_n144_), .O(new_n145_));
  oai21  g094(.a(new_n145_), .b(x18), .c(new_n139_), .O(z09));
  nand4  g095(.a(new_n127_), .b(new_n117_), .c(new_n142_), .d(new_n60_), .O(new_n147_));
  oai21  g096(.a(new_n147_), .b(new_n89_), .c(x29), .O(new_n148_));
  nor3   g097(.a(x29), .b(x28), .c(x27), .O(new_n149_));
  nand4  g098(.a(new_n149_), .b(new_n120_), .c(new_n99_), .d(new_n63_), .O(new_n150_));
  nand2  g099(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(x16), .O(new_n152_));
  inv1   g101(.a(x05), .O(new_n153_));
  aoi21  g102(.a(new_n57_), .b(new_n153_), .c(x18), .O(new_n154_));
  nand2  g103(.a(new_n154_), .b(new_n152_), .O(z10));
  nand4  g104(.a(new_n110_), .b(new_n65_), .c(new_n71_), .d(new_n64_), .O(new_n156_));
  nor2   g105(.a(x26), .b(x25), .O(new_n157_));
  nor2   g106(.a(x30), .b(x29), .O(new_n158_));
  nand4  g107(.a(new_n158_), .b(new_n136_), .c(new_n157_), .d(new_n104_), .O(new_n159_));
  nor2   g108(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  aoi21  g109(.a(new_n150_), .b(x30), .c(new_n160_), .O(new_n161_));
  inv1   g110(.a(x04), .O(new_n162_));
  aoi21  g111(.a(new_n57_), .b(new_n162_), .c(x18), .O(new_n163_));
  oai21  g112(.a(new_n161_), .b(new_n57_), .c(new_n163_), .O(z11));
  nor2   g113(.a(x31), .b(x28), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n158_), .c(new_n127_), .d(new_n105_), .O(new_n166_));
  oai21  g115(.a(new_n166_), .b(new_n95_), .c(new_n52_), .O(new_n167_));
  nand2  g116(.a(new_n167_), .b(new_n60_), .O(new_n168_));
  nand2  g117(.a(x31), .b(x16), .O(new_n169_));
  inv1   g118(.a(x03), .O(new_n170_));
  nand2  g119(.a(new_n57_), .b(new_n170_), .O(new_n171_));
  oai21  g120(.a(new_n169_), .b(new_n160_), .c(new_n171_), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n52_), .O(new_n173_));
  nand2  g122(.a(new_n173_), .b(new_n168_), .O(z12));
  nor2   g123(.a(x32), .b(x31), .O(new_n175_));
  nand4  g124(.a(new_n175_), .b(new_n158_), .c(new_n136_), .d(new_n157_), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n95_), .c(new_n52_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n60_), .O(new_n178_));
  inv1   g127(.a(x32), .O(new_n179_));
  nor2   g128(.a(new_n179_), .b(new_n57_), .O(new_n180_));
  oai21  g129(.a(new_n166_), .b(new_n112_), .c(new_n180_), .O(new_n181_));
  inv1   g130(.a(x02), .O(new_n182_));
  nand2  g131(.a(new_n57_), .b(new_n182_), .O(new_n183_));
  nand2  g132(.a(new_n183_), .b(new_n181_), .O(new_n184_));
  nand2  g133(.a(new_n184_), .b(new_n52_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n178_), .O(z13));
  oai21  g135(.a(new_n176_), .b(new_n112_), .c(x33), .O(new_n187_));
  inv1   g136(.a(x29), .O(new_n188_));
  inv1   g137(.a(x30), .O(new_n189_));
  inv1   g138(.a(x31), .O(new_n190_));
  nand4  g139(.a(new_n190_), .b(new_n189_), .c(new_n188_), .d(new_n142_), .O(new_n191_));
  inv1   g140(.a(x33), .O(new_n192_));
  nand4  g141(.a(new_n192_), .b(new_n179_), .c(new_n131_), .d(new_n116_), .O(new_n193_));
  nor2   g142(.a(new_n193_), .b(new_n191_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n119_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(new_n187_), .O(new_n196_));
  nand2  g145(.a(new_n196_), .b(x16), .O(new_n197_));
  inv1   g146(.a(x01), .O(new_n198_));
  aoi21  g147(.a(new_n57_), .b(new_n198_), .c(x18), .O(new_n199_));
  nand2  g148(.a(new_n199_), .b(new_n197_), .O(z14));
  inv1   g149(.a(x34), .O(new_n201_));
  nand3  g150(.a(new_n194_), .b(new_n107_), .c(new_n201_), .O(new_n202_));
  nand2  g151(.a(new_n202_), .b(new_n52_), .O(new_n203_));
  nand2  g152(.a(new_n203_), .b(new_n60_), .O(new_n204_));
  nor2   g153(.a(x33), .b(x32), .O(new_n205_));
  nand4  g154(.a(new_n205_), .b(new_n165_), .c(new_n158_), .d(new_n127_), .O(new_n206_));
  nor2   g155(.a(new_n206_), .b(new_n118_), .O(new_n207_));
  nand2  g156(.a(x34), .b(x16), .O(new_n208_));
  inv1   g157(.a(x00), .O(new_n209_));
  nand2  g158(.a(new_n57_), .b(new_n209_), .O(new_n210_));
  oai21  g159(.a(new_n208_), .b(new_n207_), .c(new_n210_), .O(new_n211_));
  nand2  g160(.a(new_n211_), .b(new_n52_), .O(new_n212_));
  nand2  g161(.a(new_n212_), .b(new_n204_), .O(z15));
endmodule


