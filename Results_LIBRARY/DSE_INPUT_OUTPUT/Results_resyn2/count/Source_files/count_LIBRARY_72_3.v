// Benchmark "FAU" written by ABC on Sat Aug  8 21:26:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  nor2   g000(.a(x19), .b(x17), .O(new_n52_));
  inv1   g001(.a(x17), .O(new_n53_));
  inv1   g002(.a(x19), .O(new_n54_));
  nor2   g003(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g004(.a(new_n55_), .b(new_n52_), .c(x16), .O(new_n56_));
  inv1   g005(.a(x15), .O(new_n57_));
  inv1   g006(.a(x16), .O(new_n58_));
  aoi21  g007(.a(new_n58_), .b(new_n57_), .c(x18), .O(new_n59_));
  aoi22  g008(.a(new_n59_), .b(new_n56_), .c(x20), .d(x18), .O(z00));
  oai22  g009(.a(x20), .b(x18), .c(x19), .d(x17), .O(new_n61_));
  nor3   g010(.a(x20), .b(x19), .c(x17), .O(new_n62_));
  nor2   g011(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  inv1   g012(.a(x18), .O(new_n64_));
  inv1   g013(.a(x20), .O(new_n65_));
  nand3  g014(.a(new_n64_), .b(new_n58_), .c(x14), .O(new_n66_));
  oai21  g015(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  aoi21  g016(.a(new_n63_), .b(new_n61_), .c(new_n67_), .O(z01));
  inv1   g017(.a(x21), .O(new_n69_));
  nand3  g018(.a(new_n52_), .b(new_n69_), .c(x16), .O(new_n70_));
  nand2  g019(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  nand2  g020(.a(new_n71_), .b(new_n65_), .O(new_n72_));
  nor2   g021(.a(x16), .b(x13), .O(new_n73_));
  aoi21  g022(.a(new_n63_), .b(x21), .c(new_n73_), .O(new_n74_));
  oai21  g023(.a(new_n74_), .b(x18), .c(new_n72_), .O(z02));
  nor2   g024(.a(x22), .b(x21), .O(new_n76_));
  nand3  g025(.a(new_n76_), .b(new_n52_), .c(x16), .O(new_n77_));
  nand2  g026(.a(new_n77_), .b(new_n64_), .O(new_n78_));
  nand2  g027(.a(new_n78_), .b(new_n65_), .O(new_n79_));
  nor2   g028(.a(x16), .b(x12), .O(new_n80_));
  nand4  g029(.a(new_n69_), .b(new_n65_), .c(new_n54_), .d(new_n53_), .O(new_n81_));
  inv1   g030(.a(new_n81_), .O(new_n82_));
  nand2  g031(.a(x22), .b(x16), .O(new_n83_));
  nor2   g032(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  oai21  g033(.a(new_n84_), .b(new_n80_), .c(new_n64_), .O(new_n85_));
  nand2  g034(.a(new_n85_), .b(new_n79_), .O(z03));
  inv1   g035(.a(x23), .O(new_n87_));
  aoi21  g036(.a(new_n76_), .b(new_n62_), .c(new_n87_), .O(new_n88_));
  nor2   g037(.a(x21), .b(x20), .O(new_n89_));
  nor2   g038(.a(x23), .b(x22), .O(new_n90_));
  nand3  g039(.a(new_n90_), .b(new_n89_), .c(new_n52_), .O(new_n91_));
  inv1   g040(.a(new_n91_), .O(new_n92_));
  oai21  g041(.a(new_n92_), .b(new_n88_), .c(x16), .O(new_n93_));
  inv1   g042(.a(x11), .O(new_n94_));
  aoi21  g043(.a(new_n58_), .b(new_n94_), .c(x18), .O(new_n95_));
  nand2  g044(.a(new_n95_), .b(new_n93_), .O(z04));
  nand2  g045(.a(new_n91_), .b(x24), .O(new_n97_));
  nor2   g046(.a(x24), .b(x23), .O(new_n98_));
  nand4  g047(.a(new_n98_), .b(new_n76_), .c(new_n52_), .d(new_n65_), .O(new_n99_));
  aoi21  g048(.a(new_n99_), .b(new_n97_), .c(new_n58_), .O(new_n100_));
  oai21  g049(.a(x16), .b(x10), .c(new_n64_), .O(new_n101_));
  or2    g050(.a(new_n101_), .b(new_n100_), .O(z05));
  inv1   g051(.a(x22), .O(new_n103_));
  inv1   g052(.a(x24), .O(new_n104_));
  inv1   g053(.a(x25), .O(new_n105_));
  nand4  g054(.a(new_n105_), .b(new_n104_), .c(new_n87_), .d(new_n103_), .O(new_n106_));
  inv1   g055(.a(new_n106_), .O(new_n107_));
  aoi22  g056(.a(new_n107_), .b(new_n82_), .c(new_n99_), .d(x25), .O(new_n108_));
  inv1   g057(.a(x09), .O(new_n109_));
  aoi21  g058(.a(new_n58_), .b(new_n109_), .c(x18), .O(new_n110_));
  oai21  g059(.a(new_n108_), .b(new_n58_), .c(new_n110_), .O(z06));
  nor2   g060(.a(x25), .b(x24), .O(new_n112_));
  nand4  g061(.a(new_n112_), .b(new_n90_), .c(new_n89_), .d(new_n52_), .O(new_n113_));
  nand2  g062(.a(new_n113_), .b(x26), .O(new_n114_));
  nor3   g063(.a(x26), .b(x25), .c(x24), .O(new_n115_));
  nand2  g064(.a(new_n115_), .b(new_n92_), .O(new_n116_));
  nand2  g065(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand2  g066(.a(new_n117_), .b(x16), .O(new_n118_));
  inv1   g067(.a(x08), .O(new_n119_));
  aoi21  g068(.a(new_n58_), .b(new_n119_), .c(x18), .O(new_n120_));
  nand2  g069(.a(new_n120_), .b(new_n118_), .O(z07));
  inv1   g070(.a(x27), .O(new_n122_));
  nor2   g071(.a(x26), .b(x25), .O(new_n123_));
  nand3  g072(.a(new_n123_), .b(new_n98_), .c(new_n122_), .O(new_n124_));
  nor2   g073(.a(new_n124_), .b(new_n77_), .O(new_n125_));
  oai21  g074(.a(new_n125_), .b(x18), .c(new_n65_), .O(new_n126_));
  nor2   g075(.a(new_n122_), .b(new_n58_), .O(new_n127_));
  nor2   g076(.a(x16), .b(x07), .O(new_n128_));
  aoi21  g077(.a(new_n127_), .b(new_n116_), .c(new_n128_), .O(new_n129_));
  oai21  g078(.a(new_n129_), .b(x18), .c(new_n126_), .O(z08));
  inv1   g079(.a(x28), .O(new_n131_));
  aoi21  g080(.a(new_n125_), .b(new_n131_), .c(x18), .O(new_n132_));
  nor3   g081(.a(x27), .b(x26), .c(x25), .O(new_n133_));
  nand4  g082(.a(new_n133_), .b(new_n98_), .c(new_n76_), .d(new_n62_), .O(new_n134_));
  nor2   g083(.a(new_n131_), .b(new_n58_), .O(new_n135_));
  nor2   g084(.a(x16), .b(x06), .O(new_n136_));
  aoi21  g085(.a(new_n135_), .b(new_n134_), .c(new_n136_), .O(new_n137_));
  oai22  g086(.a(new_n137_), .b(x18), .c(new_n132_), .d(x20), .O(z09));
  nor3   g087(.a(x28), .b(x24), .c(x23), .O(new_n139_));
  nand4  g088(.a(new_n139_), .b(new_n133_), .c(new_n76_), .d(new_n62_), .O(new_n140_));
  nor2   g089(.a(x29), .b(x28), .O(new_n141_));
  nand4  g090(.a(new_n141_), .b(new_n123_), .c(new_n122_), .d(new_n104_), .O(new_n142_));
  nor2   g091(.a(new_n142_), .b(new_n91_), .O(new_n143_));
  aoi21  g092(.a(new_n140_), .b(x29), .c(new_n143_), .O(new_n144_));
  inv1   g093(.a(x05), .O(new_n145_));
  aoi21  g094(.a(new_n58_), .b(new_n145_), .c(x18), .O(new_n146_));
  oai21  g095(.a(new_n144_), .b(new_n58_), .c(new_n146_), .O(z10));
  nor2   g096(.a(x30), .b(x27), .O(new_n148_));
  nand4  g097(.a(new_n148_), .b(new_n141_), .c(new_n123_), .d(new_n104_), .O(new_n149_));
  nand4  g098(.a(new_n76_), .b(new_n52_), .c(new_n87_), .d(x16), .O(new_n150_));
  oai21  g099(.a(new_n150_), .b(new_n149_), .c(new_n64_), .O(new_n151_));
  nand2  g100(.a(new_n151_), .b(new_n65_), .O(new_n152_));
  inv1   g101(.a(x30), .O(new_n153_));
  nor2   g102(.a(new_n153_), .b(new_n58_), .O(new_n154_));
  oai21  g103(.a(new_n142_), .b(new_n91_), .c(new_n154_), .O(new_n155_));
  inv1   g104(.a(x04), .O(new_n156_));
  nand2  g105(.a(new_n58_), .b(new_n156_), .O(new_n157_));
  nand2  g106(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  nand2  g107(.a(new_n158_), .b(new_n64_), .O(new_n159_));
  nand2  g108(.a(new_n159_), .b(new_n152_), .O(z11));
  oai21  g109(.a(new_n149_), .b(new_n91_), .c(x31), .O(new_n161_));
  nand4  g110(.a(new_n104_), .b(new_n87_), .c(new_n103_), .d(new_n69_), .O(new_n162_));
  inv1   g111(.a(new_n162_), .O(new_n163_));
  inv1   g112(.a(x29), .O(new_n164_));
  inv1   g113(.a(x31), .O(new_n165_));
  nand4  g114(.a(new_n165_), .b(new_n153_), .c(new_n164_), .d(new_n131_), .O(new_n166_));
  inv1   g115(.a(new_n166_), .O(new_n167_));
  nand4  g116(.a(new_n167_), .b(new_n133_), .c(new_n163_), .d(new_n62_), .O(new_n168_));
  nand2  g117(.a(new_n168_), .b(new_n161_), .O(new_n169_));
  nand2  g118(.a(new_n169_), .b(x16), .O(new_n170_));
  inv1   g119(.a(x03), .O(new_n171_));
  aoi21  g120(.a(new_n58_), .b(new_n171_), .c(x18), .O(new_n172_));
  nand2  g121(.a(new_n172_), .b(new_n170_), .O(z12));
  nor2   g122(.a(x32), .b(x31), .O(new_n174_));
  nand4  g123(.a(new_n174_), .b(new_n148_), .c(new_n141_), .d(new_n123_), .O(new_n175_));
  nand4  g124(.a(new_n98_), .b(new_n76_), .c(new_n52_), .d(x16), .O(new_n176_));
  oai21  g125(.a(new_n176_), .b(new_n175_), .c(new_n64_), .O(new_n177_));
  nand2  g126(.a(new_n177_), .b(new_n65_), .O(new_n178_));
  nor2   g127(.a(x31), .b(x30), .O(new_n179_));
  nand4  g128(.a(new_n179_), .b(new_n141_), .c(new_n123_), .d(new_n122_), .O(new_n180_));
  inv1   g129(.a(x32), .O(new_n181_));
  nor2   g130(.a(new_n181_), .b(new_n58_), .O(new_n182_));
  oai21  g131(.a(new_n180_), .b(new_n99_), .c(new_n182_), .O(new_n183_));
  inv1   g132(.a(x02), .O(new_n184_));
  nand2  g133(.a(new_n58_), .b(new_n184_), .O(new_n185_));
  nand2  g134(.a(new_n185_), .b(new_n183_), .O(new_n186_));
  nand2  g135(.a(new_n186_), .b(new_n64_), .O(new_n187_));
  nand2  g136(.a(new_n187_), .b(new_n178_), .O(z13));
  oai21  g137(.a(new_n175_), .b(new_n99_), .c(x33), .O(new_n189_));
  inv1   g138(.a(x26), .O(new_n190_));
  inv1   g139(.a(x33), .O(new_n191_));
  nand4  g140(.a(new_n191_), .b(new_n181_), .c(new_n122_), .d(new_n190_), .O(new_n192_));
  inv1   g141(.a(new_n192_), .O(new_n193_));
  nand4  g142(.a(new_n193_), .b(new_n167_), .c(new_n107_), .d(new_n82_), .O(new_n194_));
  nand2  g143(.a(new_n194_), .b(new_n189_), .O(new_n195_));
  nand2  g144(.a(new_n195_), .b(x16), .O(new_n196_));
  inv1   g145(.a(x01), .O(new_n197_));
  aoi21  g146(.a(new_n58_), .b(new_n197_), .c(x18), .O(new_n198_));
  nand2  g147(.a(new_n198_), .b(new_n196_), .O(z14));
  inv1   g148(.a(x34), .O(new_n200_));
  nor2   g149(.a(new_n194_), .b(new_n200_), .O(new_n201_));
  nor2   g150(.a(x27), .b(x26), .O(new_n202_));
  nor2   g151(.a(x33), .b(x32), .O(new_n203_));
  nand4  g152(.a(new_n203_), .b(new_n202_), .c(new_n179_), .d(new_n141_), .O(new_n204_));
  oai21  g153(.a(new_n204_), .b(new_n113_), .c(new_n200_), .O(new_n205_));
  nand2  g154(.a(new_n205_), .b(x16), .O(new_n206_));
  inv1   g155(.a(x00), .O(new_n207_));
  aoi21  g156(.a(new_n58_), .b(new_n207_), .c(x18), .O(new_n208_));
  oai21  g157(.a(new_n206_), .b(new_n201_), .c(new_n208_), .O(z15));
endmodule


