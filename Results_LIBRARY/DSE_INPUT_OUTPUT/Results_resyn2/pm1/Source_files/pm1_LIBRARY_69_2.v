// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand3  g04(.a(x14), .b(x12), .c(new_n31_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  nand2  g06(.a(x11), .b(x09), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x14), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x12), .O(z02));
  nand3  g09(.a(x14), .b(x12), .c(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n35_), .c(x11), .O(z03));
  inv1   g12(.a(x14), .O(z04));
  aoi21  g13(.a(z04), .b(x12), .c(x13), .O(z05));
  nand2  g14(.a(x12), .b(new_n31_), .O(new_n44_));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  nand2  g17(.a(z04), .b(x12), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x09), .c(x01), .O(new_n48_));
  aoi21  g19(.a(new_n46_), .b(new_n44_), .c(new_n48_), .O(z06));
  nand2  g20(.a(new_n47_), .b(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  oai21  g24(.a(new_n45_), .b(new_n39_), .c(new_n53_), .O(z08));
  inv1   g25(.a(x00), .O(new_n55_));
  nor2   g26(.a(x10), .b(new_n55_), .O(new_n56_));
  nand2  g27(.a(new_n32_), .b(x11), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n56_), .c(new_n44_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n47_), .O(z09));
  nand4  g30(.a(new_n56_), .b(new_n45_), .c(x11), .d(x09), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(x14), .c(new_n32_), .O(z10));
  nand4  g32(.a(new_n45_), .b(x12), .c(x09), .d(x01), .O(new_n62_));
  nor2   g33(.a(x12), .b(x01), .O(new_n63_));
  aoi21  g34(.a(z04), .b(x12), .c(new_n63_), .O(new_n64_));
  aoi22  g35(.a(new_n64_), .b(new_n62_), .c(new_n52_), .d(new_n47_), .O(z11));
  inv1   g36(.a(x09), .O(new_n66_));
  nand3  g37(.a(new_n56_), .b(x11), .c(new_n66_), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(x14), .c(new_n32_), .O(z12));
endmodule


