// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x04), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  nand2  g05(.a(new_n30_), .b(x04), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand3  g08(.a(x12), .b(x11), .c(x09), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n36_), .O(z02));
  inv1   g11(.a(new_n38_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n37_), .O(z03));
  nor2   g13(.a(new_n36_), .b(x14), .O(z04));
  nor2   g14(.a(new_n36_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  and2   g16(.a(x09), .b(x01), .O(new_n46_));
  oai21  g17(.a(new_n45_), .b(new_n33_), .c(new_n46_), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n35_), .O(z06));
  nor2   g19(.a(new_n36_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n35_), .O(new_n52_));
  nand2  g23(.a(x12), .b(x09), .O(new_n53_));
  oai21  g24(.a(new_n53_), .b(new_n45_), .c(new_n52_), .O(z08));
  nand2  g25(.a(x12), .b(x11), .O(new_n55_));
  nand2  g26(.a(new_n50_), .b(x00), .O(new_n56_));
  inv1   g27(.a(x04), .O(new_n57_));
  nand3  g28(.a(new_n30_), .b(new_n33_), .c(new_n57_), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(new_n55_), .c(new_n56_), .O(z09));
  nand4  g30(.a(new_n45_), .b(x11), .c(new_n50_), .d(x00), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n53_), .O(z10));
  inv1   g32(.a(x01), .O(new_n62_));
  aoi21  g33(.a(x12), .b(x09), .c(new_n62_), .O(new_n63_));
  aoi21  g34(.a(new_n30_), .b(new_n57_), .c(x01), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n63_), .c(new_n60_), .O(z11));
  nor3   g36(.a(new_n56_), .b(new_n55_), .c(x09), .O(z12));
endmodule


