// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n37_, new_n38_, new_n40_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x06), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n31_), .c(new_n34_), .O(z01));
  nand2  g06(.a(x12), .b(x06), .O(z02));
  nand2  g07(.a(x11), .b(x09), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z03));
  inv1   g10(.a(z02), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(x14), .O(z04));
  nor2   g12(.a(new_n40_), .b(x13), .O(z05));
  nand2  g13(.a(x12), .b(new_n31_), .O(new_n43_));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  and2   g16(.a(x09), .b(x01), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(z02), .O(new_n47_));
  aoi21  g18(.a(new_n45_), .b(new_n43_), .c(new_n47_), .O(z06));
  nor2   g19(.a(new_n40_), .b(x15), .O(z07));
  inv1   g20(.a(x09), .O(new_n50_));
  oai21  g21(.a(new_n44_), .b(new_n50_), .c(new_n34_), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(x12), .O(new_n52_));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n52_), .O(z08));
  nand2  g27(.a(new_n32_), .b(x11), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(new_n43_), .c(new_n53_), .d(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(z02), .O(z09));
  nand2  g30(.a(new_n44_), .b(x12), .O(new_n60_));
  nand3  g31(.a(new_n53_), .b(new_n34_), .c(x00), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n60_), .c(new_n37_), .O(z10));
  nand2  g33(.a(new_n32_), .b(new_n30_), .O(new_n63_));
  nand4  g34(.a(new_n46_), .b(new_n44_), .c(x12), .d(new_n34_), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(new_n63_), .c(new_n54_), .O(z11));
  nand3  g36(.a(x12), .b(x11), .c(new_n50_), .O(new_n66_));
  nor2   g37(.a(new_n66_), .b(new_n61_), .O(z12));
endmodule


