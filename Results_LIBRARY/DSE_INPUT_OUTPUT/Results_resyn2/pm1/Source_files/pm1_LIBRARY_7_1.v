// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_;
  inv1   g00(.a(x04), .O(new_n30_));
  nand2  g01(.a(x12), .b(new_n30_), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  nor3   g03(.a(x12), .b(x11), .c(x01), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(new_n32_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n30_), .c(x12), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand2  g08(.a(x11), .b(x09), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x04), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(z02));
  inv1   g11(.a(new_n38_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x04), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z03));
  nand2  g15(.a(new_n31_), .b(x14), .O(z04));
  nand2  g16(.a(new_n31_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  aoi21  g18(.a(x12), .b(new_n35_), .c(new_n47_), .O(new_n48_));
  and2   g19(.a(x09), .b(x01), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n31_), .O(new_n50_));
  nor2   g21(.a(new_n50_), .b(new_n48_), .O(z06));
  nor2   g22(.a(new_n32_), .b(x15), .O(z07));
  inv1   g23(.a(x09), .O(new_n53_));
  nand2  g24(.a(x03), .b(x02), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n53_), .c(x04), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x12), .O(new_n56_));
  inv1   g27(.a(x00), .O(new_n57_));
  nor2   g28(.a(x10), .b(new_n57_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n56_), .c(x11), .O(z08));
  inv1   g30(.a(x12), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x11), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n58_), .c(z01), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(z09));
  nand3  g34(.a(new_n58_), .b(new_n54_), .c(new_n41_), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(x04), .c(new_n60_), .O(z10));
  aoi21  g36(.a(new_n49_), .b(new_n54_), .c(new_n60_), .O(new_n66_));
  nand2  g37(.a(new_n60_), .b(x01), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n58_), .c(x11), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n66_), .c(new_n31_), .O(z11));
  nand3  g40(.a(new_n58_), .b(x11), .c(new_n53_), .O(new_n70_));
  aoi21  g41(.a(new_n70_), .b(x04), .c(new_n60_), .O(z12));
endmodule


