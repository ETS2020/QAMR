// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n48_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x10), .O(new_n31_));
  inv1   g02(.a(x01), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n30_), .O(z00));
  nand3  g07(.a(x12), .b(new_n30_), .c(new_n31_), .O(z01));
  nand2  g08(.a(x12), .b(x09), .O(new_n38_));
  nand3  g09(.a(x11), .b(x08), .c(x07), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x06), .c(x05), .O(new_n41_));
  or2    g12(.a(new_n41_), .b(new_n38_), .O(z02));
  nor2   g13(.a(x11), .b(x10), .O(new_n43_));
  aoi21  g14(.a(new_n38_), .b(x11), .c(new_n43_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n41_), .O(z03));
  nand2  g16(.a(new_n30_), .b(x10), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x14), .O(z04));
  inv1   g18(.a(new_n46_), .O(new_n48_));
  nor2   g19(.a(new_n48_), .b(x13), .O(z05));
  nand2  g20(.a(x12), .b(new_n30_), .O(new_n50_));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand3  g23(.a(new_n46_), .b(x09), .c(x01), .O(new_n53_));
  aoi21  g24(.a(new_n52_), .b(new_n50_), .c(new_n53_), .O(z06));
  nand2  g25(.a(new_n46_), .b(x15), .O(z07));
  nand3  g26(.a(new_n52_), .b(x12), .c(x09), .O(new_n56_));
  nand2  g27(.a(x11), .b(x00), .O(new_n57_));
  nor2   g28(.a(new_n57_), .b(x10), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(new_n56_), .c(new_n48_), .O(z08));
  aoi21  g30(.a(x12), .b(new_n31_), .c(new_n30_), .O(new_n60_));
  nand2  g31(.a(new_n50_), .b(x00), .O(new_n61_));
  oai21  g32(.a(new_n61_), .b(new_n60_), .c(new_n46_), .O(z09));
  nand2  g33(.a(new_n51_), .b(x12), .O(new_n63_));
  nand2  g34(.a(new_n58_), .b(x09), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n63_), .O(z10));
  nand4  g36(.a(new_n51_), .b(x12), .c(x09), .d(x01), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n34_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n58_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n46_), .O(z11));
  nor4   g40(.a(new_n57_), .b(new_n33_), .c(x10), .d(x09), .O(z12));
endmodule


