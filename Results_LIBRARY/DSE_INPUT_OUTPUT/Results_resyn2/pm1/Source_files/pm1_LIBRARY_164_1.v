// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n41_, new_n43_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  nand2  g02(.a(x12), .b(new_n31_), .O(z01));
  nand2  g03(.a(z01), .b(new_n30_), .O(z00));
  nand4  g04(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  and2   g06(.a(x12), .b(x09), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x11), .c(x01), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n35_), .O(z02));
  nand2  g10(.a(new_n38_), .b(new_n34_), .O(z03));
  nor2   g11(.a(new_n31_), .b(x01), .O(new_n41_));
  nor2   g12(.a(new_n41_), .b(x14), .O(z04));
  inv1   g13(.a(new_n41_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  aoi21  g16(.a(x12), .b(new_n31_), .c(new_n45_), .O(new_n46_));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n46_), .c(new_n43_), .O(z06));
  nor2   g19(.a(new_n41_), .b(x15), .O(z07));
  nor2   g20(.a(new_n31_), .b(new_n30_), .O(new_n50_));
  inv1   g21(.a(new_n45_), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n36_), .O(new_n52_));
  inv1   g23(.a(x00), .O(new_n53_));
  nor2   g24(.a(x10), .b(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n52_), .c(new_n50_), .O(z08));
  nor2   g26(.a(x12), .b(new_n31_), .O(new_n56_));
  nand2  g27(.a(new_n54_), .b(z01), .O(new_n57_));
  oai21  g28(.a(new_n57_), .b(new_n56_), .c(new_n43_), .O(z09));
  inv1   g29(.a(x10), .O(new_n59_));
  nand4  g30(.a(new_n45_), .b(new_n36_), .c(new_n59_), .d(x00), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(x01), .c(new_n31_), .O(z10));
  nand4  g32(.a(new_n54_), .b(new_n45_), .c(new_n50_), .d(new_n36_), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(z11));
  inv1   g34(.a(x09), .O(new_n64_));
  nand4  g35(.a(x12), .b(new_n59_), .c(new_n64_), .d(x00), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(x01), .c(new_n31_), .O(z12));
endmodule


