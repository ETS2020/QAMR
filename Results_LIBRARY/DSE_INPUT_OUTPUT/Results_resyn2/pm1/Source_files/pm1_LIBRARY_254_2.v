// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n38_, new_n40_,
    new_n41_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x09), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand4  g07(.a(new_n36_), .b(x12), .c(x11), .d(x09), .O(z02));
  oai21  g08(.a(new_n36_), .b(new_n33_), .c(x12), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x09), .O(z03));
  inv1   g10(.a(x09), .O(new_n40_));
  nor2   g11(.a(x12), .b(new_n40_), .O(new_n41_));
  nor2   g12(.a(new_n41_), .b(x14), .O(z04));
  oai21  g13(.a(x12), .b(new_n40_), .c(x13), .O(z05));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n33_), .c(x01), .O(new_n45_));
  aoi21  g16(.a(new_n45_), .b(x12), .c(new_n40_), .O(z06));
  nor2   g17(.a(new_n41_), .b(x15), .O(z07));
  nand2  g18(.a(new_n44_), .b(x12), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x09), .O(new_n49_));
  inv1   g20(.a(x00), .O(new_n50_));
  nor2   g21(.a(x10), .b(new_n50_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(new_n49_), .c(x11), .O(z08));
  aoi21  g23(.a(new_n33_), .b(new_n40_), .c(x12), .O(new_n53_));
  nand2  g24(.a(new_n51_), .b(z01), .O(new_n54_));
  nor2   g25(.a(new_n54_), .b(new_n53_), .O(z09));
  nand3  g26(.a(new_n51_), .b(new_n44_), .c(x11), .O(new_n56_));
  aoi21  g27(.a(new_n56_), .b(x12), .c(new_n40_), .O(z10));
  inv1   g28(.a(x01), .O(new_n58_));
  inv1   g29(.a(x10), .O(new_n59_));
  nand4  g30(.a(x11), .b(new_n59_), .c(new_n58_), .d(x00), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n40_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n30_), .O(new_n62_));
  nor2   g33(.a(new_n40_), .b(new_n58_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n51_), .c(new_n44_), .d(x11), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n62_), .O(z11));
  nand3  g36(.a(x12), .b(x11), .c(new_n40_), .O(new_n66_));
  nor3   g37(.a(new_n66_), .b(x10), .c(new_n50_), .O(z12));
endmodule


