// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand3  g04(.a(x12), .b(new_n31_), .c(x08), .O(z01));
  nand3  g05(.a(x07), .b(x06), .c(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand3  g07(.a(x12), .b(x09), .c(x08), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n36_), .c(x11), .O(z02));
  nand3  g10(.a(new_n35_), .b(x11), .c(x09), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x08), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z03));
  inv1   g13(.a(x08), .O(new_n43_));
  nand2  g14(.a(x12), .b(new_n43_), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nor2   g16(.a(new_n45_), .b(x14), .O(z04));
  nor2   g17(.a(new_n45_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  aoi21  g21(.a(new_n49_), .b(z01), .c(new_n50_), .O(z06));
  nand2  g22(.a(new_n44_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n44_), .O(new_n55_));
  oai21  g26(.a(new_n48_), .b(new_n37_), .c(new_n55_), .O(z08));
  inv1   g27(.a(x00), .O(new_n57_));
  nor2   g28(.a(x10), .b(new_n57_), .O(new_n58_));
  oai21  g29(.a(new_n31_), .b(new_n43_), .c(x12), .O(new_n59_));
  nand2  g30(.a(new_n32_), .b(x11), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z09));
  nand2  g33(.a(x09), .b(x08), .O(new_n63_));
  nand2  g34(.a(new_n48_), .b(x12), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n54_), .c(new_n63_), .O(z10));
  aoi21  g36(.a(new_n48_), .b(x12), .c(new_n30_), .O(new_n66_));
  nand2  g37(.a(new_n50_), .b(x12), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n58_), .c(x11), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n66_), .c(new_n44_), .O(z11));
  inv1   g40(.a(x09), .O(new_n70_));
  nand3  g41(.a(new_n58_), .b(x11), .c(new_n70_), .O(new_n71_));
  aoi21  g42(.a(new_n71_), .b(x08), .c(new_n32_), .O(z12));
endmodule


