// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x08), .O(new_n30_));
  nand2  g01(.a(x12), .b(new_n30_), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  nor3   g03(.a(x12), .b(x11), .c(x01), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(new_n32_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  nand3  g06(.a(x12), .b(new_n35_), .c(x08), .O(z01));
  nand3  g07(.a(x07), .b(x06), .c(x05), .O(new_n37_));
  nand2  g08(.a(x11), .b(x09), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x08), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(z02));
  inv1   g11(.a(x12), .O(new_n41_));
  nor2   g12(.a(new_n41_), .b(new_n30_), .O(new_n42_));
  nand4  g13(.a(new_n37_), .b(new_n42_), .c(x11), .d(x09), .O(z03));
  nor2   g14(.a(new_n32_), .b(x14), .O(z04));
  nand2  g15(.a(new_n31_), .b(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n31_), .O(new_n47_));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  aoi21  g19(.a(new_n47_), .b(z01), .c(new_n48_), .O(z06));
  nor2   g20(.a(new_n32_), .b(x15), .O(z07));
  inv1   g21(.a(x09), .O(new_n51_));
  oai21  g22(.a(new_n46_), .b(new_n51_), .c(x08), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x12), .O(new_n53_));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x11), .b(new_n54_), .c(x00), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n53_), .O(z08));
  nand3  g28(.a(new_n35_), .b(new_n54_), .c(x00), .O(new_n58_));
  aoi22  g29(.a(new_n58_), .b(new_n41_), .c(new_n55_), .d(new_n42_), .O(z09));
  nand2  g30(.a(new_n46_), .b(x12), .O(new_n60_));
  nand2  g31(.a(x09), .b(x08), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n60_), .c(new_n55_), .O(z10));
  inv1   g33(.a(x01), .O(new_n63_));
  aoi21  g34(.a(new_n46_), .b(x12), .c(new_n63_), .O(new_n64_));
  inv1   g35(.a(x00), .O(new_n65_));
  nor2   g36(.a(x10), .b(new_n65_), .O(new_n66_));
  nand3  g37(.a(x09), .b(x08), .c(x01), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x12), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n66_), .c(x11), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n64_), .O(z11));
  nand2  g41(.a(new_n42_), .b(new_n51_), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n55_), .O(z12));
endmodule


