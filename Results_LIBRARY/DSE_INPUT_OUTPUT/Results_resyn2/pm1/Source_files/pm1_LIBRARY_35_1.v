// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n44_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_;
  inv1   g00(.a(x12), .O(new_n30_));
  nand2  g01(.a(new_n30_), .b(x01), .O(new_n31_));
  nand2  g02(.a(x12), .b(x05), .O(new_n32_));
  nand2  g03(.a(new_n30_), .b(x11), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(z00));
  nand2  g05(.a(x11), .b(x05), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x12), .O(z01));
  nand3  g07(.a(x08), .b(x07), .c(x06), .O(new_n37_));
  nand2  g08(.a(x11), .b(x09), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x05), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(z02));
  inv1   g11(.a(new_n37_), .O(new_n41_));
  oai21  g12(.a(new_n38_), .b(new_n41_), .c(x05), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  nor2   g14(.a(new_n30_), .b(x05), .O(new_n44_));
  nor2   g15(.a(new_n44_), .b(x14), .O(z04));
  inv1   g16(.a(new_n44_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x13), .O(z05));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(x11), .c(x05), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x12), .O(new_n50_));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  oai21  g23(.a(new_n52_), .b(new_n48_), .c(new_n50_), .O(z06));
  nor2   g24(.a(new_n44_), .b(x15), .O(z07));
  inv1   g25(.a(x00), .O(new_n55_));
  nor2   g26(.a(x10), .b(new_n55_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x11), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n46_), .O(new_n58_));
  nand4  g29(.a(new_n52_), .b(x12), .c(x09), .d(x05), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n58_), .O(z08));
  nand3  g31(.a(new_n56_), .b(z01), .c(new_n33_), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z09));
  nand4  g33(.a(new_n56_), .b(new_n51_), .c(x11), .d(x09), .O(new_n63_));
  aoi21  g34(.a(new_n63_), .b(x05), .c(new_n30_), .O(z10));
  nand2  g35(.a(new_n32_), .b(new_n31_), .O(new_n65_));
  nand4  g36(.a(new_n51_), .b(x12), .c(x09), .d(x01), .O(new_n66_));
  aoi22  g37(.a(new_n66_), .b(new_n65_), .c(new_n57_), .d(new_n46_), .O(z11));
  inv1   g38(.a(x09), .O(new_n68_));
  nand3  g39(.a(new_n56_), .b(x11), .c(new_n68_), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(x05), .c(new_n30_), .O(z12));
endmodule


