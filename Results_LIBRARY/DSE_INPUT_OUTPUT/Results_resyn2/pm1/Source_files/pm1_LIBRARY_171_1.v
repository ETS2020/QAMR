// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  inv1   g03(.a(x10), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(x02), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand4  g06(.a(new_n35_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand3  g07(.a(new_n35_), .b(x12), .c(new_n31_), .O(z01));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand3  g10(.a(x12), .b(x11), .c(x09), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  aoi21  g12(.a(new_n41_), .b(new_n39_), .c(new_n34_), .O(z02));
  nand3  g13(.a(new_n41_), .b(new_n38_), .c(new_n35_), .O(z03));
  nand2  g14(.a(new_n35_), .b(x14), .O(z04));
  nor2   g15(.a(new_n34_), .b(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  aoi21  g17(.a(x12), .b(new_n31_), .c(new_n46_), .O(new_n47_));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  nor3   g19(.a(new_n48_), .b(new_n47_), .c(new_n34_), .O(z06));
  nor2   g20(.a(new_n34_), .b(x15), .O(z07));
  nand3  g21(.a(x11), .b(new_n33_), .c(x00), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand2  g23(.a(x03), .b(x02), .O(new_n53_));
  nand3  g24(.a(x12), .b(x09), .c(x04), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z08));
  nand2  g26(.a(new_n32_), .b(new_n31_), .O(new_n56_));
  nand2  g27(.a(x12), .b(x11), .O(new_n57_));
  nand2  g28(.a(new_n33_), .b(x00), .O(new_n58_));
  aoi21  g29(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(z09));
  nand4  g30(.a(new_n46_), .b(new_n41_), .c(new_n33_), .d(x00), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(z10));
  inv1   g32(.a(x02), .O(new_n62_));
  nand3  g33(.a(x12), .b(x11), .c(x00), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n48_), .c(new_n33_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g36(.a(x04), .b(x03), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(x09), .c(x01), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x12), .O(new_n68_));
  nand2  g39(.a(new_n32_), .b(x01), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n68_), .c(new_n52_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n65_), .O(z11));
  nor3   g42(.a(new_n58_), .b(new_n57_), .c(x09), .O(z12));
endmodule


