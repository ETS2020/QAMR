// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n39_, new_n40_, new_n41_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x11), .O(new_n30_));
  nor2   g01(.a(x12), .b(x01), .O(new_n31_));
  aoi22  g02(.a(new_n31_), .b(new_n30_), .c(x12), .d(x09), .O(z00));
  inv1   g03(.a(x09), .O(new_n33_));
  nand3  g04(.a(x12), .b(new_n30_), .c(new_n33_), .O(z01));
  nand2  g05(.a(x12), .b(x09), .O(z02));
  nand2  g06(.a(z02), .b(x14), .O(z04));
  nand2  g07(.a(z02), .b(x13), .O(z05));
  inv1   g08(.a(x12), .O(new_n39_));
  nand3  g09(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  nand2  g10(.a(new_n40_), .b(x01), .O(new_n41_));
  aoi21  g11(.a(new_n41_), .b(new_n39_), .c(new_n33_), .O(z06));
  nand2  g12(.a(z02), .b(x15), .O(z07));
  inv1   g13(.a(x10), .O(new_n44_));
  nand3  g14(.a(x11), .b(new_n44_), .c(x00), .O(new_n45_));
  inv1   g15(.a(new_n45_), .O(new_n46_));
  nand2  g16(.a(new_n46_), .b(z02), .O(z08));
  aoi21  g17(.a(x11), .b(new_n33_), .c(new_n39_), .O(new_n48_));
  nand2  g18(.a(new_n39_), .b(x11), .O(new_n49_));
  nand3  g19(.a(new_n49_), .b(new_n44_), .c(x00), .O(new_n50_));
  nor2   g20(.a(new_n50_), .b(new_n48_), .O(z09));
  nor3   g21(.a(new_n45_), .b(x12), .c(x01), .O(z11));
  aoi21  g22(.a(new_n45_), .b(new_n33_), .c(new_n39_), .O(z12));
  one    g23(.O(z03));
  zero   g24(.O(z10));
endmodule


