// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n44_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n66_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  inv1   g03(.a(x14), .O(z04));
  nand4  g04(.a(z04), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g05(.a(x12), .b(new_n31_), .O(z01));
  nor2   g06(.a(z04), .b(x12), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(x12), .b(x11), .c(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  aoi21  g11(.a(new_n40_), .b(new_n38_), .c(new_n36_), .O(z02));
  nand2  g12(.a(new_n40_), .b(new_n37_), .O(z03));
  nor2   g13(.a(new_n36_), .b(x13), .O(z05));
  nand2  g14(.a(x14), .b(new_n32_), .O(new_n44_));
  nand2  g15(.a(x09), .b(x01), .O(new_n45_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  aoi21  g17(.a(x12), .b(new_n31_), .c(new_n46_), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(z06));
  nand2  g19(.a(new_n44_), .b(x15), .O(z07));
  inv1   g20(.a(new_n46_), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x12), .c(x09), .O(new_n51_));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n51_), .c(new_n44_), .O(z08));
  nand2  g26(.a(new_n32_), .b(x11), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(z01), .c(new_n52_), .d(x00), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n44_), .O(z09));
  nand2  g29(.a(new_n46_), .b(x12), .O(new_n59_));
  nand4  g30(.a(x11), .b(new_n52_), .c(x09), .d(x00), .O(new_n60_));
  oai21  g31(.a(new_n60_), .b(new_n59_), .c(new_n44_), .O(z10));
  nand3  g32(.a(z04), .b(new_n32_), .c(new_n30_), .O(new_n62_));
  nand4  g33(.a(new_n46_), .b(x12), .c(x09), .d(x01), .O(new_n63_));
  aoi21  g34(.a(new_n63_), .b(new_n62_), .c(new_n53_), .O(z11));
  nand2  g35(.a(x12), .b(x11), .O(new_n65_));
  nand2  g36(.a(new_n52_), .b(x00), .O(new_n66_));
  nor3   g37(.a(new_n66_), .b(new_n65_), .c(x09), .O(z12));
endmodule


