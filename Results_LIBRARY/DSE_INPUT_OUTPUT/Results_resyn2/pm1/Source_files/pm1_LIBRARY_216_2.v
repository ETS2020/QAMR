// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n42_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x03), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  or2    g04(.a(new_n33_), .b(x01), .O(z00));
  oai21  g05(.a(x12), .b(x03), .c(new_n31_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand3  g08(.a(x12), .b(x11), .c(x09), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n37_), .O(z02));
  nand2  g11(.a(new_n39_), .b(new_n36_), .O(z03));
  nand2  g12(.a(new_n31_), .b(x03), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x14), .O(z04));
  nand2  g14(.a(new_n42_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(x09), .c(x01), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n42_), .O(z06));
  nand2  g18(.a(new_n42_), .b(x15), .O(z07));
  inv1   g19(.a(new_n45_), .O(new_n49_));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  and2   g22(.a(x12), .b(x09), .O(new_n52_));
  aoi21  g23(.a(new_n52_), .b(new_n49_), .c(new_n51_), .O(new_n53_));
  xor2a  g24(.a(new_n53_), .b(new_n42_), .O(z08));
  nand2  g25(.a(x12), .b(x11), .O(new_n55_));
  nand2  g26(.a(new_n50_), .b(x00), .O(new_n56_));
  aoi21  g27(.a(new_n55_), .b(new_n33_), .c(new_n56_), .O(z09));
  nand2  g28(.a(new_n45_), .b(x12), .O(new_n58_));
  inv1   g29(.a(new_n51_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(x09), .O(new_n60_));
  oai21  g31(.a(new_n60_), .b(new_n58_), .c(new_n42_), .O(z10));
  inv1   g32(.a(x01), .O(new_n62_));
  nand2  g33(.a(new_n32_), .b(new_n62_), .O(new_n63_));
  nand4  g34(.a(new_n45_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n42_), .O(z11));
  nor3   g38(.a(new_n56_), .b(new_n55_), .c(x09), .O(z12));
endmodule


