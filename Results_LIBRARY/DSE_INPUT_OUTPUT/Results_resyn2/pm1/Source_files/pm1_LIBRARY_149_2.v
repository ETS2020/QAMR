// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x03), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g05(.a(x12), .b(new_n32_), .O(z01));
  nand2  g06(.a(new_n33_), .b(x03), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  nand2  g09(.a(x12), .b(x09), .O(new_n39_));
  nor3   g10(.a(new_n39_), .b(new_n38_), .c(new_n32_), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(new_n37_), .O(z02));
  nand3  g12(.a(new_n38_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(new_n43_));
  oai21  g14(.a(x12), .b(x03), .c(new_n43_), .O(z03));
  nor2   g15(.a(new_n37_), .b(x14), .O(z04));
  nand2  g16(.a(new_n36_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  inv1   g18(.a(x09), .O(new_n48_));
  nor2   g19(.a(new_n48_), .b(new_n30_), .O(new_n49_));
  oai21  g20(.a(new_n47_), .b(new_n32_), .c(new_n49_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n36_), .O(z06));
  nand2  g22(.a(new_n36_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n36_), .O(new_n55_));
  oai21  g26(.a(new_n47_), .b(new_n39_), .c(new_n55_), .O(z08));
  aoi21  g27(.a(new_n32_), .b(new_n31_), .c(x12), .O(new_n57_));
  inv1   g28(.a(x00), .O(new_n58_));
  nor2   g29(.a(x10), .b(new_n58_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(z01), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n57_), .O(z09));
  inv1   g32(.a(new_n47_), .O(new_n62_));
  nor3   g33(.a(new_n54_), .b(new_n62_), .c(new_n39_), .O(z10));
  nand3  g34(.a(x12), .b(x09), .c(x01), .O(new_n64_));
  inv1   g35(.a(new_n64_), .O(new_n65_));
  nand2  g36(.a(x04), .b(x02), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n65_), .c(new_n31_), .O(new_n67_));
  oai21  g38(.a(x12), .b(x01), .c(new_n64_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n59_), .c(x11), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n67_), .O(z11));
  nand2  g41(.a(x12), .b(new_n48_), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n54_), .c(new_n36_), .O(z12));
endmodule


