// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n40_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_;
  inv1   g00(.a(x07), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nor2   g02(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g03(.a(x01), .O(new_n33_));
  nand2  g04(.a(new_n31_), .b(new_n33_), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n32_), .O(z00));
  inv1   g07(.a(x11), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(x07), .c(x12), .O(z01));
  inv1   g09(.a(new_n32_), .O(z02));
  nor2   g10(.a(new_n31_), .b(x07), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x11), .c(x09), .O(z03));
  nor2   g12(.a(new_n32_), .b(x14), .O(z04));
  nand2  g13(.a(z02), .b(x13), .O(z05));
  nand2  g14(.a(x09), .b(x01), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(x11), .c(new_n30_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x12), .O(new_n46_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  inv1   g18(.a(new_n47_), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n44_), .c(new_n46_), .O(z06));
  nor2   g20(.a(new_n32_), .b(x15), .O(z07));
  aoi21  g21(.a(new_n48_), .b(x09), .c(x07), .O(new_n51_));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  oai21  g25(.a(new_n51_), .b(new_n31_), .c(new_n54_), .O(z08));
  nand3  g26(.a(new_n37_), .b(new_n52_), .c(x00), .O(new_n56_));
  aoi22  g27(.a(new_n56_), .b(new_n31_), .c(new_n53_), .d(new_n40_), .O(z09));
  nand3  g28(.a(new_n47_), .b(new_n40_), .c(x09), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n53_), .O(z10));
  nand4  g30(.a(new_n47_), .b(new_n40_), .c(x09), .d(x01), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(new_n34_), .c(new_n53_), .O(z11));
  nand2  g32(.a(x12), .b(new_n30_), .O(new_n62_));
  nor3   g33(.a(new_n53_), .b(new_n62_), .c(x09), .O(z12));
endmodule


