// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_;
  inv1   g00(.a(x12), .O(new_n30_));
  inv1   g01(.a(x13), .O(z05));
  nor2   g02(.a(z05), .b(new_n30_), .O(new_n32_));
  nor3   g03(.a(x12), .b(x11), .c(x01), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(new_n32_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  nand3  g06(.a(z05), .b(x12), .c(new_n35_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  nor3   g10(.a(x13), .b(new_n30_), .c(new_n39_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n38_), .c(x11), .O(z02));
  nand3  g12(.a(new_n40_), .b(new_n37_), .c(x11), .O(z03));
  inv1   g13(.a(new_n32_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nand2  g15(.a(x09), .b(x01), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(x11), .c(z05), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n45_), .c(new_n47_), .O(z06));
  nor2   g21(.a(new_n32_), .b(x15), .O(z07));
  nand2  g22(.a(new_n49_), .b(new_n40_), .O(new_n52_));
  inv1   g23(.a(x00), .O(new_n53_));
  nor2   g24(.a(x10), .b(new_n53_), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x11), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n43_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n52_), .O(z08));
  nand2  g28(.a(x12), .b(new_n35_), .O(new_n58_));
  nand2  g29(.a(new_n30_), .b(x11), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n54_), .c(new_n58_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n43_), .O(z09));
  nand4  g32(.a(new_n54_), .b(new_n48_), .c(x11), .d(x09), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(z05), .c(new_n30_), .O(z10));
  nand4  g34(.a(new_n48_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  nor2   g35(.a(x12), .b(x01), .O(new_n65_));
  aoi21  g36(.a(x13), .b(x12), .c(new_n65_), .O(new_n66_));
  aoi22  g37(.a(new_n66_), .b(new_n64_), .c(new_n55_), .d(new_n43_), .O(z11));
  nand3  g38(.a(new_n54_), .b(x11), .c(new_n39_), .O(new_n68_));
  aoi21  g39(.a(new_n68_), .b(z05), .c(new_n30_), .O(z12));
endmodule


