// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_;
  inv1   g00(.a(x15), .O(z07));
  nand2  g01(.a(z07), .b(x09), .O(new_n31_));
  nor2   g02(.a(x12), .b(x01), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  or2    g04(.a(new_n33_), .b(x11), .O(z00));
  inv1   g05(.a(new_n31_), .O(new_n35_));
  inv1   g06(.a(x12), .O(new_n36_));
  nor2   g07(.a(new_n36_), .b(x11), .O(new_n37_));
  nor2   g08(.a(new_n37_), .b(new_n35_), .O(z01));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  nand2  g10(.a(x12), .b(x11), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n39_), .c(x15), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x09), .O(z02));
  inv1   g13(.a(new_n40_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n39_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x15), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x09), .O(z03));
  nand2  g17(.a(new_n31_), .b(x14), .O(z04));
  nor2   g18(.a(new_n35_), .b(x13), .O(z05));
  inv1   g19(.a(x09), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  oai21  g21(.a(new_n50_), .b(new_n37_), .c(x01), .O(new_n51_));
  aoi21  g22(.a(new_n51_), .b(x15), .c(new_n49_), .O(z06));
  inv1   g23(.a(x00), .O(new_n53_));
  nor2   g24(.a(x10), .b(new_n53_), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x11), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n31_), .O(new_n56_));
  inv1   g27(.a(new_n50_), .O(new_n57_));
  nand3  g28(.a(x15), .b(x12), .c(x09), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n56_), .O(z08));
  xnor2a g32(.a(x12), .b(x11), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(new_n54_), .c(new_n31_), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(z09));
  nand3  g35(.a(new_n54_), .b(new_n50_), .c(x11), .O(new_n65_));
  nor2   g36(.a(new_n65_), .b(new_n58_), .O(z10));
  nand3  g37(.a(new_n59_), .b(new_n50_), .c(x01), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n33_), .c(new_n55_), .O(z11));
  nand3  g39(.a(new_n54_), .b(new_n43_), .c(new_n49_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n31_), .O(z12));
endmodule


