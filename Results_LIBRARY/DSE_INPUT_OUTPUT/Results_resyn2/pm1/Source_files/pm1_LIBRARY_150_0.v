// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n41_, new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x04), .O(new_n32_));
  nor2   g03(.a(x12), .b(new_n32_), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g05(.a(x12), .b(x11), .O(new_n35_));
  oai21  g06(.a(x12), .b(new_n32_), .c(new_n35_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g10(.a(new_n37_), .b(x12), .c(x11), .d(x09), .O(z03));
  inv1   g11(.a(x12), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n32_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x14), .O(z04));
  aoi21  g14(.a(new_n41_), .b(new_n32_), .c(x13), .O(z05));
  nand2  g15(.a(x03), .b(x02), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x04), .O(new_n46_));
  oai21  g17(.a(new_n31_), .b(new_n32_), .c(x12), .O(new_n47_));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  aoi21  g19(.a(new_n47_), .b(new_n46_), .c(new_n48_), .O(z06));
  nand2  g20(.a(new_n42_), .b(x15), .O(z07));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x12), .c(x09), .O(new_n53_));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x11), .b(new_n54_), .c(x00), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n53_), .c(new_n42_), .O(z08));
  nand3  g28(.a(new_n31_), .b(new_n54_), .c(x00), .O(new_n58_));
  aoi22  g29(.a(new_n58_), .b(new_n33_), .c(new_n55_), .d(x12), .O(z09));
  nand2  g30(.a(new_n51_), .b(x12), .O(new_n60_));
  nand4  g31(.a(x11), .b(new_n54_), .c(x09), .d(x00), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(new_n60_), .O(z10));
  nand3  g33(.a(new_n41_), .b(x04), .c(new_n30_), .O(new_n63_));
  nand4  g34(.a(new_n51_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(new_n63_), .c(new_n55_), .O(z11));
  nand2  g36(.a(new_n54_), .b(x00), .O(new_n66_));
  nor3   g37(.a(new_n66_), .b(new_n35_), .c(x09), .O(z12));
endmodule


