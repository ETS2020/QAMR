// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x12), .O(new_n30_));
  nand2  g01(.a(x12), .b(x04), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  nor2   g03(.a(x11), .b(x01), .O(new_n33_));
  aoi21  g04(.a(new_n33_), .b(new_n30_), .c(new_n32_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(x04), .c(x12), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nor2   g09(.a(new_n30_), .b(x04), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(new_n38_), .c(x11), .d(x09), .O(z02));
  inv1   g11(.a(x04), .O(new_n41_));
  nand3  g12(.a(new_n37_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z03));
  nor2   g15(.a(new_n32_), .b(x14), .O(z04));
  nand2  g16(.a(new_n31_), .b(x13), .O(z05));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n47_), .c(new_n31_), .O(z06));
  nor2   g21(.a(new_n32_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  and2   g24(.a(new_n53_), .b(new_n31_), .O(z08));
  nand3  g25(.a(new_n35_), .b(new_n52_), .c(x00), .O(new_n55_));
  aoi22  g26(.a(new_n55_), .b(new_n30_), .c(new_n53_), .d(new_n39_), .O(z09));
  nand2  g27(.a(x12), .b(new_n41_), .O(new_n57_));
  nand2  g28(.a(x11), .b(x09), .O(new_n58_));
  nand2  g29(.a(new_n52_), .b(x00), .O(new_n59_));
  nor3   g30(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(z10));
  inv1   g31(.a(x01), .O(new_n61_));
  aoi21  g32(.a(x12), .b(new_n41_), .c(new_n61_), .O(new_n62_));
  nand2  g33(.a(new_n47_), .b(x12), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x11), .c(new_n52_), .d(x00), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n62_), .O(z11));
  nor3   g36(.a(new_n53_), .b(new_n57_), .c(x09), .O(z12));
endmodule


