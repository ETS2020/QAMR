// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nor2   g03(.a(new_n32_), .b(x11), .O(new_n33_));
  aoi21  g04(.a(x13), .b(x12), .c(new_n33_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  inv1   g06(.a(x13), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x12), .c(new_n35_), .O(z01));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nor2   g10(.a(x13), .b(new_n31_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n39_), .c(x11), .d(x09), .O(z02));
  nand4  g12(.a(new_n40_), .b(new_n38_), .c(x11), .d(x09), .O(z03));
  nand2  g13(.a(x13), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nand2  g15(.a(x13), .b(new_n31_), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  and2   g18(.a(x09), .b(x01), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  aoi21  g20(.a(new_n47_), .b(z01), .c(new_n49_), .O(z06));
  nand2  g21(.a(new_n43_), .b(x15), .O(z07));
  nand3  g22(.a(new_n47_), .b(new_n40_), .c(x09), .O(new_n52_));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n52_), .O(z08));
  nand2  g27(.a(x12), .b(new_n35_), .O(new_n57_));
  nand2  g28(.a(new_n31_), .b(x11), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(new_n57_), .c(new_n53_), .d(x00), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n43_), .O(z09));
  and2   g31(.a(x11), .b(x09), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n46_), .c(new_n53_), .d(x00), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(new_n36_), .c(new_n31_), .O(z10));
  nand4  g34(.a(new_n48_), .b(new_n46_), .c(new_n36_), .d(x12), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(new_n32_), .c(new_n54_), .O(z11));
  inv1   g36(.a(x09), .O(new_n66_));
  nand4  g37(.a(x11), .b(new_n53_), .c(new_n66_), .d(x00), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n36_), .c(new_n31_), .O(z12));
endmodule


