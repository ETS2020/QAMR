// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nor2   g04(.a(x14), .b(new_n31_), .O(new_n34_));
  nor2   g05(.a(new_n32_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n34_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(x12), .b(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n38_), .c(x14), .d(x11), .O(z02));
  oai21  g12(.a(new_n39_), .b(new_n38_), .c(x14), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x11), .O(z03));
  inv1   g14(.a(x14), .O(z04));
  nor2   g15(.a(new_n34_), .b(x13), .O(z05));
  inv1   g16(.a(new_n34_), .O(new_n46_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(new_n35_), .O(new_n48_));
  nand2  g19(.a(x09), .b(x01), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(z06));
  nand2  g21(.a(new_n46_), .b(x15), .O(z07));
  inv1   g22(.a(x00), .O(new_n52_));
  nor2   g23(.a(x10), .b(new_n52_), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(x14), .c(x11), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  oai21  g26(.a(new_n47_), .b(new_n39_), .c(new_n55_), .O(z08));
  inv1   g27(.a(new_n35_), .O(new_n57_));
  nand2  g28(.a(new_n32_), .b(x11), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n53_), .c(new_n57_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n46_), .O(z09));
  nand2  g31(.a(new_n47_), .b(new_n40_), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(new_n54_), .O(z10));
  nand2  g33(.a(new_n32_), .b(new_n30_), .O(new_n63_));
  nand4  g34(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n53_), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(x14), .c(new_n31_), .O(z11));
  nor3   g38(.a(new_n54_), .b(new_n32_), .c(x09), .O(z12));
endmodule


