// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  nand2  g03(.a(x12), .b(new_n30_), .O(z01));
  nand4  g04(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand4  g06(.a(new_n35_), .b(x12), .c(x11), .d(x09), .O(z02));
  and2   g07(.a(x12), .b(x09), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x11), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(z01), .O(z03));
  nand2  g11(.a(z00), .b(x14), .O(z04));
  nand2  g12(.a(z00), .b(x13), .O(z05));
  nand2  g13(.a(x09), .b(x01), .O(new_n43_));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  aoi22  g16(.a(new_n45_), .b(x11), .c(new_n43_), .d(z00), .O(z06));
  aoi21  g17(.a(new_n31_), .b(new_n30_), .c(x15), .O(z07));
  nand2  g18(.a(new_n45_), .b(new_n37_), .O(new_n48_));
  nor2   g19(.a(new_n30_), .b(x10), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(new_n48_), .c(x00), .O(z08));
  inv1   g21(.a(x10), .O(new_n51_));
  nand4  g22(.a(x12), .b(x11), .c(new_n51_), .d(x00), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(z00), .O(z09));
  inv1   g24(.a(x00), .O(new_n54_));
  nand3  g25(.a(new_n49_), .b(new_n44_), .c(new_n37_), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n54_), .c(z00), .O(z10));
  inv1   g27(.a(x01), .O(new_n57_));
  nand3  g28(.a(new_n51_), .b(new_n57_), .c(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(x11), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n31_), .O(new_n60_));
  nor2   g31(.a(new_n57_), .b(new_n54_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n49_), .c(new_n44_), .d(new_n37_), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n60_), .O(z11));
  nor2   g34(.a(new_n52_), .b(x09), .O(z12));
endmodule


