// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(new_n30_), .b(x07), .O(new_n31_));
  inv1   g02(.a(x01), .O(new_n32_));
  nand2  g03(.a(new_n30_), .b(new_n32_), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(x11), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(new_n31_), .O(z00));
  nand2  g06(.a(x11), .b(x07), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x12), .O(z01));
  nand3  g08(.a(x12), .b(x11), .c(x07), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  and2   g10(.a(x06), .b(x05), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n39_), .c(x09), .d(x08), .O(z02));
  nand2  g12(.a(new_n40_), .b(x08), .O(new_n42_));
  nand3  g13(.a(new_n42_), .b(new_n39_), .c(x09), .O(z03));
  inv1   g14(.a(new_n31_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  nand2  g16(.a(new_n44_), .b(x13), .O(z05));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(x11), .c(x07), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  oai21  g22(.a(new_n51_), .b(new_n47_), .c(new_n49_), .O(z06));
  nor2   g23(.a(new_n31_), .b(x15), .O(z07));
  inv1   g24(.a(x07), .O(new_n54_));
  aoi21  g25(.a(new_n51_), .b(x09), .c(new_n54_), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n56_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  oai21  g29(.a(new_n55_), .b(new_n30_), .c(new_n58_), .O(z08));
  nand2  g30(.a(new_n30_), .b(x11), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(z01), .c(new_n56_), .d(x00), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z09));
  nand2  g33(.a(new_n58_), .b(x07), .O(new_n63_));
  nand3  g34(.a(new_n50_), .b(x12), .c(x09), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n63_), .O(z10));
  nand4  g36(.a(new_n50_), .b(x12), .c(x09), .d(x01), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n33_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n58_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n44_), .O(z11));
  inv1   g40(.a(x09), .O(new_n70_));
  nand2  g41(.a(x12), .b(new_n70_), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n63_), .O(z12));
endmodule


