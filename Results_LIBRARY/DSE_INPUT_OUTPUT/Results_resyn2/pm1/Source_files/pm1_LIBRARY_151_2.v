// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x06), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  or2    g04(.a(new_n33_), .b(x11), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  nand2  g06(.a(x12), .b(new_n35_), .O(z01));
  nand2  g07(.a(x11), .b(x09), .O(new_n37_));
  nand3  g08(.a(x08), .b(x07), .c(x05), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x12), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x06), .O(z02));
  nand2  g11(.a(new_n32_), .b(new_n31_), .O(new_n41_));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  nand3  g13(.a(new_n42_), .b(x11), .c(x09), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n41_), .O(z03));
  nand2  g16(.a(new_n32_), .b(x06), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(x14), .O(z04));
  nor2   g19(.a(new_n47_), .b(x13), .O(z05));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  aoi21  g21(.a(x12), .b(new_n35_), .c(new_n50_), .O(new_n51_));
  nand3  g22(.a(new_n46_), .b(x09), .c(x01), .O(new_n52_));
  nor2   g23(.a(new_n52_), .b(new_n51_), .O(z06));
  nor2   g24(.a(new_n47_), .b(x15), .O(z07));
  inv1   g25(.a(new_n50_), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x12), .c(x09), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x11), .b(new_n57_), .c(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n56_), .c(new_n46_), .O(z08));
  nand2  g31(.a(new_n32_), .b(x11), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(z01), .c(new_n57_), .d(x00), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n46_), .O(z09));
  nand2  g34(.a(new_n57_), .b(x00), .O(new_n64_));
  nand2  g35(.a(new_n50_), .b(x12), .O(new_n65_));
  nor3   g36(.a(new_n65_), .b(new_n64_), .c(new_n37_), .O(z10));
  nand4  g37(.a(new_n50_), .b(x12), .c(x09), .d(x01), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n33_), .c(new_n58_), .O(z11));
  nor3   g39(.a(new_n58_), .b(new_n32_), .c(x09), .O(z12));
endmodule


