// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  oai21  g02(.a(x11), .b(new_n30_), .c(new_n31_), .O(z00));
  nand2  g03(.a(new_n31_), .b(x11), .O(new_n33_));
  inv1   g04(.a(x11), .O(new_n34_));
  nand2  g05(.a(x12), .b(new_n34_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(z01));
  inv1   g08(.a(x09), .O(new_n38_));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(x12), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x11), .O(z02));
  nand2  g12(.a(new_n39_), .b(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x11), .O(z03));
  nand2  g15(.a(new_n33_), .b(x14), .O(z04));
  inv1   g16(.a(new_n33_), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  aoi21  g19(.a(x12), .b(new_n34_), .c(new_n48_), .O(new_n49_));
  nand3  g20(.a(new_n33_), .b(x09), .c(x01), .O(new_n50_));
  nor2   g21(.a(new_n50_), .b(new_n49_), .O(z06));
  nor2   g22(.a(new_n46_), .b(x15), .O(z07));
  inv1   g23(.a(new_n48_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x09), .O(new_n54_));
  inv1   g25(.a(x10), .O(new_n55_));
  nand4  g26(.a(x12), .b(x11), .c(new_n55_), .d(x00), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n54_), .O(z08));
  nand2  g29(.a(new_n55_), .b(x00), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(new_n36_), .O(z09));
  nand4  g31(.a(new_n48_), .b(new_n55_), .c(x09), .d(x00), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(x12), .c(new_n34_), .O(z10));
  nand3  g33(.a(new_n48_), .b(x09), .c(x01), .O(new_n63_));
  nor2   g34(.a(new_n63_), .b(new_n56_), .O(z11));
  nor2   g35(.a(new_n56_), .b(x09), .O(z12));
endmodule


