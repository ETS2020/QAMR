// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n65_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x01), .O(new_n31_));
  nand2  g02(.a(x12), .b(new_n31_), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n30_), .O(z00));
  inv1   g04(.a(x12), .O(new_n34_));
  aoi21  g05(.a(new_n34_), .b(new_n31_), .c(x11), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(z01));
  nor2   g07(.a(x11), .b(new_n31_), .O(new_n37_));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand3  g10(.a(x12), .b(x11), .c(x09), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  aoi21  g12(.a(new_n41_), .b(new_n39_), .c(new_n37_), .O(z02));
  nand2  g13(.a(x12), .b(x09), .O(new_n43_));
  oai21  g14(.a(new_n43_), .b(new_n39_), .c(x11), .O(new_n44_));
  nand2  g15(.a(new_n30_), .b(new_n31_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(new_n44_), .O(z03));
  nor2   g17(.a(new_n37_), .b(x14), .O(z04));
  nor2   g18(.a(new_n37_), .b(x13), .O(z05));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x09), .O(new_n50_));
  aoi21  g21(.a(new_n50_), .b(x11), .c(new_n31_), .O(z06));
  nor2   g22(.a(new_n37_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  oai21  g26(.a(new_n49_), .b(new_n43_), .c(new_n55_), .O(z08));
  nand2  g27(.a(new_n53_), .b(x00), .O(new_n57_));
  nor2   g28(.a(x12), .b(new_n30_), .O(new_n58_));
  nor3   g29(.a(new_n58_), .b(new_n57_), .c(new_n35_), .O(z09));
  nand3  g30(.a(new_n49_), .b(x12), .c(x09), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n54_), .O(z10));
  nand4  g32(.a(new_n32_), .b(x11), .c(new_n53_), .d(x00), .O(new_n62_));
  aoi21  g33(.a(new_n60_), .b(x01), .c(new_n62_), .O(z11));
  inv1   g34(.a(x09), .O(new_n64_));
  nand3  g35(.a(x12), .b(x11), .c(new_n64_), .O(new_n65_));
  oai22  g36(.a(new_n65_), .b(new_n57_), .c(x11), .d(new_n31_), .O(z12));
endmodule


