// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(new_n30_), .b(x03), .O(new_n31_));
  nor3   g02(.a(x12), .b(x11), .c(x01), .O(new_n32_));
  nor2   g03(.a(new_n32_), .b(new_n31_), .O(z00));
  inv1   g04(.a(x03), .O(new_n34_));
  inv1   g05(.a(x11), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n34_), .c(x12), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(x09), .b(x03), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n38_), .c(x12), .d(x11), .O(z02));
  nand3  g12(.a(new_n37_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x03), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z03));
  nor2   g15(.a(new_n31_), .b(x14), .O(z04));
  inv1   g16(.a(new_n31_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x13), .O(z05));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  aoi21  g20(.a(x12), .b(new_n35_), .c(new_n49_), .O(new_n50_));
  aoi21  g21(.a(new_n48_), .b(new_n46_), .c(new_n50_), .O(z06));
  nor2   g22(.a(new_n31_), .b(x15), .O(z07));
  inv1   g23(.a(x09), .O(new_n53_));
  nand2  g24(.a(x04), .b(x02), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n53_), .c(x03), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x12), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x11), .b(new_n57_), .c(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n56_), .O(z08));
  nand2  g31(.a(new_n57_), .b(x00), .O(new_n61_));
  xor2a  g32(.a(x12), .b(x11), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n61_), .c(new_n46_), .O(z09));
  nand2  g34(.a(new_n54_), .b(x12), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n58_), .c(new_n39_), .O(z10));
  inv1   g36(.a(x01), .O(new_n66_));
  aoi21  g37(.a(new_n54_), .b(x12), .c(new_n66_), .O(new_n67_));
  nand2  g38(.a(new_n48_), .b(x12), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x11), .c(new_n57_), .d(x00), .O(new_n69_));
  oai21  g40(.a(new_n69_), .b(new_n67_), .c(new_n46_), .O(z11));
  nand4  g41(.a(x11), .b(new_n57_), .c(new_n53_), .d(x00), .O(new_n71_));
  aoi21  g42(.a(new_n71_), .b(x03), .c(new_n30_), .O(z12));
endmodule


