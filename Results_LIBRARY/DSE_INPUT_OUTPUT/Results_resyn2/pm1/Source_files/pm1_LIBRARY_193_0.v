// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n43_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_;
  inv1   g00(.a(x01), .O(new_n30_));
  nor2   g01(.a(x09), .b(x04), .O(new_n31_));
  nor2   g02(.a(x12), .b(x11), .O(new_n32_));
  aoi21  g03(.a(new_n32_), .b(new_n30_), .c(new_n31_), .O(z00));
  inv1   g04(.a(x12), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n31_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(x12), .b(x11), .c(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  aoi21  g11(.a(new_n40_), .b(new_n38_), .c(new_n31_), .O(z02));
  nand2  g12(.a(new_n40_), .b(new_n37_), .O(z03));
  inv1   g13(.a(new_n31_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nand2  g15(.a(new_n43_), .b(x13), .O(z05));
  nand2  g16(.a(x03), .b(x02), .O(new_n46_));
  oai21  g17(.a(new_n46_), .b(new_n35_), .c(x09), .O(new_n47_));
  aoi22  g18(.a(new_n47_), .b(x04), .c(x09), .d(new_n30_), .O(z06));
  nor2   g19(.a(new_n31_), .b(x15), .O(z07));
  inv1   g20(.a(new_n46_), .O(new_n50_));
  nand4  g21(.a(new_n50_), .b(x12), .c(x09), .d(x04), .O(new_n51_));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n51_), .c(new_n43_), .O(z08));
  nand2  g26(.a(new_n52_), .b(x00), .O(new_n56_));
  xor2a  g27(.a(x12), .b(x11), .O(new_n57_));
  nor3   g28(.a(new_n57_), .b(new_n56_), .c(new_n31_), .O(z09));
  inv1   g29(.a(x04), .O(new_n59_));
  nand4  g30(.a(x12), .b(x11), .c(new_n52_), .d(x00), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x09), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand3  g33(.a(new_n46_), .b(x12), .c(x09), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n53_), .c(new_n62_), .O(z10));
  oai21  g35(.a(new_n60_), .b(new_n30_), .c(x09), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  nand2  g37(.a(new_n63_), .b(x01), .O(new_n67_));
  nand2  g38(.a(x12), .b(new_n30_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n67_), .c(new_n54_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n66_), .O(z11));
  nor3   g41(.a(new_n60_), .b(x09), .c(new_n59_), .O(z12));
endmodule


