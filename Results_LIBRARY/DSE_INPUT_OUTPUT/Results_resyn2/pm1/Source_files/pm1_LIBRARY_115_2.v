// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nor2   g02(.a(x13), .b(new_n31_), .O(new_n32_));
  nor2   g03(.a(x12), .b(x01), .O(new_n33_));
  aoi21  g04(.a(new_n33_), .b(new_n30_), .c(new_n32_), .O(z00));
  nand2  g05(.a(x13), .b(x11), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x12), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand2  g08(.a(x11), .b(x09), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x13), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(z02));
  and2   g11(.a(x13), .b(x12), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(new_n37_), .c(x11), .d(x09), .O(z03));
  oai21  g13(.a(x13), .b(new_n31_), .c(x14), .O(z04));
  nor2   g14(.a(x13), .b(x12), .O(z05));
  nand2  g15(.a(x09), .b(x01), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(x11), .c(x13), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(new_n47_));
  and2   g18(.a(x09), .b(x01), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n47_), .O(z06));
  nor2   g22(.a(new_n32_), .b(x15), .O(z07));
  nand4  g23(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x13), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x12), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n56_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n55_), .O(z08));
  nand2  g30(.a(new_n31_), .b(x11), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(z01), .c(new_n56_), .d(x00), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z09));
  nand3  g33(.a(new_n49_), .b(new_n41_), .c(x09), .O(new_n63_));
  nor2   g34(.a(new_n63_), .b(new_n57_), .O(z10));
  inv1   g35(.a(new_n33_), .O(new_n65_));
  nand3  g36(.a(new_n49_), .b(new_n48_), .c(new_n41_), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n65_), .c(new_n57_), .O(z11));
  inv1   g38(.a(x09), .O(new_n68_));
  nand4  g39(.a(x11), .b(new_n56_), .c(new_n68_), .d(x00), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(x13), .c(new_n31_), .O(z12));
endmodule


