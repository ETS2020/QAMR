// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(new_n30_), .b(x08), .O(new_n31_));
  inv1   g02(.a(x01), .O(new_n32_));
  nand2  g03(.a(new_n30_), .b(new_n32_), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(x11), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(new_n31_), .O(z00));
  inv1   g06(.a(x11), .O(new_n36_));
  nand3  g07(.a(x12), .b(new_n36_), .c(x08), .O(z01));
  nand3  g08(.a(x07), .b(x06), .c(x05), .O(new_n38_));
  nand2  g09(.a(x11), .b(x09), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(x08), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(z02));
  inv1   g12(.a(new_n38_), .O(new_n42_));
  oai21  g13(.a(new_n39_), .b(new_n42_), .c(x08), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z03));
  nor2   g15(.a(new_n31_), .b(x14), .O(z04));
  nor2   g16(.a(new_n31_), .b(x13), .O(z05));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(x11), .c(x08), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  oai21  g22(.a(new_n51_), .b(new_n47_), .c(new_n49_), .O(z06));
  inv1   g23(.a(new_n31_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x15), .O(z07));
  inv1   g25(.a(x08), .O(new_n55_));
  aoi21  g26(.a(new_n51_), .b(x09), .c(new_n55_), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x11), .b(new_n57_), .c(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  oai21  g30(.a(new_n56_), .b(new_n30_), .c(new_n59_), .O(z08));
  nand2  g31(.a(new_n57_), .b(x00), .O(new_n61_));
  nand3  g32(.a(x12), .b(x11), .c(x08), .O(new_n62_));
  nand2  g33(.a(new_n30_), .b(new_n36_), .O(new_n63_));
  aoi21  g34(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(z09));
  nand2  g35(.a(new_n50_), .b(x12), .O(new_n65_));
  nand3  g36(.a(x11), .b(x09), .c(x08), .O(new_n66_));
  nor3   g37(.a(new_n66_), .b(new_n65_), .c(new_n61_), .O(z10));
  nand4  g38(.a(new_n50_), .b(x12), .c(x09), .d(x01), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n33_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n59_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n53_), .O(z11));
  nor3   g42(.a(new_n62_), .b(new_n61_), .c(x09), .O(z12));
endmodule


