// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n40_,
    new_n43_, new_n44_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n64_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x04), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g05(.a(x12), .b(new_n32_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g09(.a(new_n36_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g10(.a(new_n33_), .b(x04), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x14), .O(z04));
  nand2  g12(.a(new_n40_), .b(x13), .O(z05));
  nand4  g13(.a(x11), .b(x04), .c(x03), .d(x02), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(x09), .c(x01), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n40_), .O(z06));
  nand2  g16(.a(new_n40_), .b(x15), .O(z07));
  inv1   g17(.a(x10), .O(new_n47_));
  nand3  g18(.a(x11), .b(new_n47_), .c(x00), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n40_), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x12), .c(x09), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n49_), .O(z08));
  nand2  g24(.a(new_n47_), .b(x00), .O(new_n54_));
  xor2a  g25(.a(x12), .b(x11), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n54_), .c(new_n40_), .O(z09));
  nand2  g27(.a(x12), .b(x09), .O(new_n57_));
  nand4  g28(.a(new_n50_), .b(x11), .c(new_n47_), .d(x00), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n57_), .O(z10));
  nand3  g30(.a(new_n33_), .b(new_n31_), .c(new_n30_), .O(new_n60_));
  nand3  g31(.a(x12), .b(x09), .c(x01), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(z11));
  inv1   g33(.a(x09), .O(new_n63_));
  nand3  g34(.a(x12), .b(x11), .c(new_n63_), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n54_), .O(z12));
endmodule


