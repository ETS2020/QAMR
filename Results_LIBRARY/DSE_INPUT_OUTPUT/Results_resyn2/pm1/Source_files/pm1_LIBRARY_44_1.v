// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x08), .O(new_n31_));
  nand2  g02(.a(x12), .b(new_n31_), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  nor2   g04(.a(x12), .b(x01), .O(new_n34_));
  aoi21  g05(.a(new_n34_), .b(new_n30_), .c(new_n33_), .O(z00));
  nand2  g06(.a(x12), .b(x08), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n30_), .O(z01));
  nand3  g09(.a(x07), .b(x06), .c(x05), .O(new_n39_));
  nand2  g10(.a(x11), .b(x09), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n39_), .c(x08), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z02));
  inv1   g13(.a(new_n39_), .O(new_n43_));
  oai21  g14(.a(new_n40_), .b(new_n43_), .c(x08), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x12), .O(z03));
  nand2  g16(.a(new_n32_), .b(x14), .O(z04));
  nor2   g17(.a(new_n33_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n32_), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  aoi21  g21(.a(new_n49_), .b(z01), .c(new_n50_), .O(z06));
  nor2   g22(.a(new_n33_), .b(x15), .O(z07));
  inv1   g23(.a(x12), .O(new_n53_));
  inv1   g24(.a(new_n48_), .O(new_n54_));
  aoi21  g25(.a(new_n54_), .b(x09), .c(new_n31_), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n56_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  oai21  g29(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(z08));
  inv1   g30(.a(x00), .O(new_n60_));
  nor2   g31(.a(x10), .b(new_n60_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n30_), .O(new_n62_));
  aoi22  g33(.a(new_n62_), .b(new_n53_), .c(new_n57_), .d(new_n37_), .O(z09));
  nand4  g34(.a(new_n61_), .b(new_n48_), .c(x11), .d(x09), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(x08), .c(new_n53_), .O(z10));
  nand3  g36(.a(x12), .b(x09), .c(x01), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n48_), .c(new_n34_), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n57_), .c(new_n32_), .O(z11));
  nor3   g40(.a(new_n57_), .b(new_n36_), .c(x09), .O(z12));
endmodule


