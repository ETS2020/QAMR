// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x07), .O(new_n31_));
  nand2  g02(.a(x15), .b(new_n31_), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  nor2   g04(.a(x12), .b(x11), .O(new_n34_));
  aoi21  g05(.a(new_n34_), .b(new_n30_), .c(new_n33_), .O(z00));
  inv1   g06(.a(x11), .O(new_n36_));
  nand3  g07(.a(new_n32_), .b(x12), .c(new_n36_), .O(z01));
  nand3  g08(.a(x11), .b(x08), .c(x07), .O(new_n38_));
  nand4  g09(.a(x12), .b(x09), .c(x06), .d(x05), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(new_n33_), .O(z02));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  and2   g13(.a(x12), .b(x11), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n42_), .c(new_n32_), .d(x09), .O(z03));
  nor2   g15(.a(new_n33_), .b(x14), .O(z04));
  nor2   g16(.a(new_n33_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  aoi21  g18(.a(x12), .b(new_n36_), .c(new_n47_), .O(new_n48_));
  nand3  g19(.a(new_n32_), .b(x09), .c(x01), .O(new_n49_));
  nor2   g20(.a(new_n49_), .b(new_n48_), .O(z06));
  nand2  g21(.a(x15), .b(x07), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  inv1   g25(.a(new_n47_), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x12), .c(x09), .O(new_n56_));
  aoi21  g27(.a(new_n56_), .b(new_n54_), .c(new_n33_), .O(z08));
  inv1   g28(.a(x00), .O(new_n58_));
  nor2   g29(.a(x10), .b(new_n58_), .O(new_n59_));
  oai21  g30(.a(new_n43_), .b(new_n34_), .c(new_n59_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n32_), .O(z09));
  nand2  g32(.a(new_n47_), .b(x12), .O(new_n62_));
  nand3  g33(.a(new_n59_), .b(x11), .c(x09), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n62_), .c(new_n32_), .O(z10));
  aoi21  g35(.a(new_n47_), .b(x12), .c(new_n30_), .O(new_n65_));
  nand2  g36(.a(x09), .b(x01), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x12), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n59_), .c(x11), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n65_), .c(new_n32_), .O(z11));
  inv1   g40(.a(x09), .O(new_n70_));
  nand3  g41(.a(new_n59_), .b(new_n43_), .c(new_n70_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n32_), .O(z12));
endmodule


