// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_;
  inv1   g00(.a(x04), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  or2    g04(.a(new_n33_), .b(x01), .O(z00));
  nand2  g05(.a(new_n32_), .b(new_n30_), .O(new_n35_));
  nor2   g06(.a(new_n32_), .b(new_n31_), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n35_), .O(z01));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n36_), .c(x09), .O(z02));
  nand3  g12(.a(new_n39_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n35_), .O(z03));
  aoi21  g15(.a(new_n32_), .b(x04), .c(x14), .O(z04));
  nand2  g16(.a(new_n32_), .b(x04), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x13), .O(z05));
  inv1   g18(.a(x01), .O(new_n48_));
  inv1   g19(.a(x09), .O(new_n49_));
  nand3  g20(.a(x11), .b(x03), .c(x02), .O(new_n50_));
  aoi21  g21(.a(new_n50_), .b(x12), .c(new_n30_), .O(new_n51_));
  nor3   g22(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(z06));
  nand2  g23(.a(new_n46_), .b(x15), .O(z07));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  nor2   g26(.a(new_n32_), .b(new_n49_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g28(.a(new_n31_), .b(x10), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(x00), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n46_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n57_), .O(z08));
  inv1   g32(.a(x10), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(x00), .O(new_n63_));
  aoi21  g34(.a(new_n37_), .b(new_n33_), .c(new_n63_), .O(z09));
  inv1   g35(.a(x00), .O(new_n65_));
  nand3  g36(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n65_), .c(new_n46_), .O(z10));
  nand4  g38(.a(x11), .b(new_n62_), .c(new_n48_), .d(x00), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n30_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n32_), .O(new_n70_));
  nor2   g41(.a(new_n48_), .b(new_n65_), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(new_n58_), .c(new_n56_), .d(new_n54_), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n70_), .O(z11));
  nand2  g44(.a(new_n36_), .b(new_n49_), .O(new_n74_));
  oai21  g45(.a(new_n74_), .b(new_n63_), .c(new_n46_), .O(z12));
endmodule


