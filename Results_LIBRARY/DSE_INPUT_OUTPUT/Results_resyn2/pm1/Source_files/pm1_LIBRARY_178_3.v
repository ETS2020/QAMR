// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x11), .O(new_n30_));
  nand2  g01(.a(x12), .b(x10), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  nor2   g03(.a(x12), .b(x01), .O(new_n33_));
  aoi21  g04(.a(new_n33_), .b(new_n30_), .c(new_n32_), .O(z00));
  inv1   g05(.a(x10), .O(new_n35_));
  nand3  g06(.a(x12), .b(new_n30_), .c(new_n35_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  nor2   g10(.a(new_n30_), .b(new_n39_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n38_), .c(x12), .d(new_n35_), .O(z02));
  nand3  g12(.a(new_n37_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z03));
  nand2  g15(.a(new_n31_), .b(x14), .O(z04));
  nor2   g16(.a(new_n32_), .b(x13), .O(z05));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(x11), .c(new_n35_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n49_), .O(z06));
  nand2  g23(.a(new_n31_), .b(x15), .O(z07));
  inv1   g24(.a(new_n50_), .O(new_n54_));
  and2   g25(.a(x12), .b(x09), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n35_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(new_n56_), .c(new_n32_), .O(z08));
  aoi21  g30(.a(new_n30_), .b(new_n35_), .c(x12), .O(new_n60_));
  inv1   g31(.a(x12), .O(new_n61_));
  oai21  g32(.a(new_n61_), .b(x11), .c(x00), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n60_), .c(new_n31_), .O(z09));
  nand2  g34(.a(new_n50_), .b(x12), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n57_), .c(new_n39_), .O(z10));
  inv1   g36(.a(new_n33_), .O(new_n66_));
  nand4  g37(.a(new_n50_), .b(x12), .c(x09), .d(x01), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n66_), .c(new_n57_), .O(z11));
  nand3  g39(.a(x11), .b(new_n39_), .c(x00), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(new_n35_), .c(new_n61_), .O(z12));
endmodule


