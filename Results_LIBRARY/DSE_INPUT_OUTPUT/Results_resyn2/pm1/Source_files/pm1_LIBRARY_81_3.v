// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n66_, new_n68_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x13), .O(z05));
  aoi21  g05(.a(z05), .b(x11), .c(new_n32_), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nor2   g09(.a(x13), .b(new_n32_), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(new_n38_), .c(x11), .d(x09), .O(z02));
  and2   g11(.a(x11), .b(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(z05), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z03));
  aoi21  g15(.a(x13), .b(x12), .c(x14), .O(z04));
  nand2  g16(.a(x09), .b(x01), .O(new_n46_));
  oai21  g17(.a(new_n46_), .b(x11), .c(z05), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x12), .O(new_n48_));
  and2   g19(.a(x09), .b(x01), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n48_), .O(z06));
  nand2  g23(.a(x13), .b(x12), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x15), .O(z07));
  nand2  g25(.a(new_n39_), .b(x09), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n56_), .c(x00), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  oai21  g29(.a(new_n50_), .b(new_n55_), .c(new_n58_), .O(z08));
  nand2  g30(.a(new_n56_), .b(x00), .O(new_n60_));
  nor2   g31(.a(x12), .b(new_n31_), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n60_), .c(new_n35_), .O(z09));
  nand4  g33(.a(new_n50_), .b(new_n41_), .c(new_n56_), .d(x00), .O(new_n63_));
  aoi21  g34(.a(new_n63_), .b(z05), .c(new_n32_), .O(z10));
  nand4  g35(.a(new_n50_), .b(new_n49_), .c(z05), .d(x12), .O(new_n65_));
  nand2  g36(.a(new_n32_), .b(new_n30_), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n65_), .c(new_n57_), .O(z11));
  nand2  g38(.a(z05), .b(x12), .O(new_n68_));
  nor3   g39(.a(new_n57_), .b(new_n68_), .c(x09), .O(z12));
endmodule


