// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n35_, new_n36_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_;
  inv1   g00(.a(x12), .O(new_n30_));
  nand2  g01(.a(x12), .b(x08), .O(z02));
  inv1   g02(.a(z02), .O(new_n32_));
  nor2   g03(.a(x11), .b(x01), .O(new_n33_));
  aoi21  g04(.a(new_n33_), .b(new_n30_), .c(new_n32_), .O(z00));
  inv1   g05(.a(x08), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand3  g07(.a(x12), .b(new_n36_), .c(new_n35_), .O(z01));
  nand4  g08(.a(x12), .b(x11), .c(x09), .d(new_n35_), .O(z03));
  nor2   g09(.a(new_n32_), .b(x14), .O(z04));
  nand2  g10(.a(z02), .b(x13), .O(z05));
  nand2  g11(.a(x12), .b(new_n36_), .O(new_n41_));
  nand3  g12(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  nand3  g14(.a(z02), .b(x09), .c(x01), .O(new_n44_));
  aoi21  g15(.a(new_n43_), .b(new_n41_), .c(new_n44_), .O(z06));
  nor2   g16(.a(new_n32_), .b(x15), .O(z07));
  inv1   g17(.a(x09), .O(new_n47_));
  oai21  g18(.a(new_n42_), .b(new_n47_), .c(new_n35_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(new_n49_));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n49_), .O(z08));
  nand2  g24(.a(new_n30_), .b(x11), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(new_n41_), .c(new_n50_), .d(x00), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(z02), .O(z09));
  nand2  g27(.a(new_n42_), .b(x12), .O(new_n57_));
  nand2  g28(.a(x09), .b(new_n35_), .O(new_n58_));
  nor3   g29(.a(new_n58_), .b(new_n57_), .c(new_n51_), .O(z10));
  nand2  g30(.a(new_n57_), .b(x01), .O(new_n60_));
  nand3  g31(.a(x09), .b(new_n35_), .c(x01), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x12), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(new_n60_), .c(new_n52_), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(z11));
  nand3  g35(.a(x12), .b(new_n47_), .c(new_n35_), .O(new_n65_));
  nor2   g36(.a(new_n65_), .b(new_n51_), .O(z12));
endmodule


