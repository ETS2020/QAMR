// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n43_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n60_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  oai21  g02(.a(x11), .b(new_n30_), .c(new_n31_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nor2   g04(.a(x12), .b(new_n33_), .O(new_n34_));
  nor2   g05(.a(new_n31_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n34_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g10(.a(x09), .O(new_n40_));
  oai21  g11(.a(new_n38_), .b(new_n40_), .c(x12), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x11), .O(z03));
  inv1   g13(.a(new_n34_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nor2   g15(.a(new_n34_), .b(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  aoi21  g17(.a(x12), .b(new_n33_), .c(new_n46_), .O(new_n47_));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n47_), .c(new_n43_), .O(z06));
  nor2   g20(.a(new_n34_), .b(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand4  g22(.a(x12), .b(x11), .c(new_n51_), .d(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  oai21  g24(.a(new_n46_), .b(new_n40_), .c(new_n53_), .O(z08));
  nand2  g25(.a(new_n51_), .b(x00), .O(new_n55_));
  aoi21  g26(.a(new_n55_), .b(new_n43_), .c(new_n35_), .O(z09));
  nand4  g27(.a(new_n46_), .b(new_n51_), .c(x09), .d(x00), .O(new_n57_));
  aoi21  g28(.a(new_n57_), .b(x12), .c(new_n33_), .O(z10));
  and2   g29(.a(x01), .b(x00), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n46_), .c(new_n51_), .d(x09), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(x12), .c(new_n33_), .O(z11));
  nor2   g32(.a(new_n52_), .b(x09), .O(z12));
endmodule


