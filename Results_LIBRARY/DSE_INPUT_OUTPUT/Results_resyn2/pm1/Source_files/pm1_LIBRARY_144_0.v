// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n31_, new_n32_, new_n35_, new_n36_, new_n39_, new_n40_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_;
  nand4  g00(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n31_));
  inv1   g01(.a(new_n31_), .O(new_n32_));
  nand4  g02(.a(new_n32_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g03(.a(new_n31_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g04(.a(x12), .b(x11), .O(new_n35_));
  inv1   g05(.a(new_n35_), .O(new_n36_));
  nand2  g06(.a(new_n36_), .b(x14), .O(z04));
  nand2  g07(.a(new_n36_), .b(x13), .O(z05));
  nand4  g08(.a(x11), .b(x04), .c(x03), .d(x02), .O(new_n39_));
  nand4  g09(.a(new_n39_), .b(new_n36_), .c(x09), .d(x01), .O(new_n40_));
  inv1   g10(.a(new_n40_), .O(z06));
  nor2   g11(.a(new_n35_), .b(x15), .O(z07));
  inv1   g12(.a(x10), .O(new_n43_));
  nand2  g13(.a(new_n43_), .b(x00), .O(new_n44_));
  nand2  g14(.a(new_n44_), .b(x11), .O(new_n45_));
  inv1   g15(.a(x09), .O(new_n46_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  oai21  g17(.a(new_n47_), .b(new_n46_), .c(x11), .O(new_n48_));
  nand2  g18(.a(new_n48_), .b(x12), .O(new_n49_));
  nand2  g19(.a(new_n49_), .b(new_n45_), .O(z08));
  inv1   g20(.a(x00), .O(new_n51_));
  nor2   g21(.a(x10), .b(new_n51_), .O(new_n52_));
  nand3  g22(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  inv1   g23(.a(new_n53_), .O(z09));
  nand2  g24(.a(x11), .b(x09), .O(new_n55_));
  nand2  g25(.a(new_n47_), .b(x12), .O(new_n56_));
  nor3   g26(.a(new_n56_), .b(new_n44_), .c(new_n55_), .O(z10));
  inv1   g27(.a(x01), .O(new_n58_));
  aoi21  g28(.a(new_n47_), .b(x12), .c(new_n58_), .O(new_n59_));
  nand2  g29(.a(x09), .b(x01), .O(new_n60_));
  nand2  g30(.a(new_n60_), .b(x12), .O(new_n61_));
  nand3  g31(.a(new_n61_), .b(new_n52_), .c(x11), .O(new_n62_));
  nor2   g32(.a(new_n62_), .b(new_n59_), .O(z11));
  nor2   g33(.a(new_n53_), .b(x09), .O(z12));
  one    g34(.O(z00));
  buf    g35(.a(x11), .O(z01));
endmodule


