// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n37_, new_n38_, new_n40_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n66_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x06), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n31_), .c(new_n34_), .O(z01));
  inv1   g06(.a(x09), .O(new_n37_));
  oai21  g07(.a(new_n31_), .b(new_n37_), .c(new_n34_), .O(new_n38_));
  nand2  g08(.a(new_n38_), .b(x12), .O(z03));
  nand2  g09(.a(x12), .b(x06), .O(new_n40_));
  nand2  g10(.a(new_n40_), .b(x14), .O(z04));
  inv1   g11(.a(new_n40_), .O(new_n42_));
  nor2   g12(.a(new_n42_), .b(x13), .O(z05));
  nand3  g13(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  nand2  g14(.a(new_n44_), .b(new_n40_), .O(new_n45_));
  nand2  g15(.a(x09), .b(x01), .O(new_n46_));
  aoi21  g16(.a(new_n45_), .b(z01), .c(new_n46_), .O(z06));
  nor2   g17(.a(new_n42_), .b(x15), .O(z07));
  oai21  g18(.a(new_n44_), .b(new_n37_), .c(new_n34_), .O(new_n49_));
  nand2  g19(.a(new_n49_), .b(x12), .O(new_n50_));
  inv1   g20(.a(x00), .O(new_n51_));
  nor2   g21(.a(x10), .b(new_n51_), .O(new_n52_));
  nand3  g22(.a(new_n52_), .b(new_n50_), .c(x11), .O(z08));
  inv1   g23(.a(x10), .O(new_n54_));
  nand2  g24(.a(new_n54_), .b(x00), .O(new_n55_));
  aoi21  g25(.a(x11), .b(new_n34_), .c(new_n32_), .O(new_n56_));
  nor2   g26(.a(x12), .b(new_n31_), .O(new_n57_));
  nor3   g27(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z09));
  nand2  g28(.a(new_n44_), .b(x12), .O(new_n59_));
  nand3  g29(.a(x11), .b(x09), .c(new_n34_), .O(new_n60_));
  nor3   g30(.a(new_n60_), .b(new_n59_), .c(new_n55_), .O(z10));
  aoi21  g31(.a(new_n44_), .b(x12), .c(new_n30_), .O(new_n62_));
  nand2  g32(.a(new_n46_), .b(x12), .O(new_n63_));
  nand3  g33(.a(new_n63_), .b(new_n52_), .c(x11), .O(new_n64_));
  oai21  g34(.a(new_n64_), .b(new_n62_), .c(new_n40_), .O(z11));
  nand3  g35(.a(new_n52_), .b(x11), .c(new_n37_), .O(new_n66_));
  aoi21  g36(.a(new_n66_), .b(new_n34_), .c(new_n32_), .O(z12));
  one    g37(.O(z02));
endmodule


