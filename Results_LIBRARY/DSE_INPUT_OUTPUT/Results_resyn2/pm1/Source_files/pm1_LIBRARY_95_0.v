// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n63_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand4  g03(.a(new_n32_), .b(new_n31_), .c(x09), .d(new_n30_), .O(z00));
  inv1   g04(.a(x09), .O(new_n34_));
  nand2  g05(.a(new_n32_), .b(new_n34_), .O(new_n35_));
  nand2  g06(.a(x12), .b(new_n31_), .O(new_n36_));
  and2   g07(.a(new_n36_), .b(new_n35_), .O(z01));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g11(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g12(.a(new_n35_), .b(x14), .O(z04));
  nand2  g13(.a(new_n35_), .b(x13), .O(z05));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand2  g16(.a(x09), .b(x01), .O(new_n46_));
  aoi21  g17(.a(new_n45_), .b(new_n36_), .c(new_n46_), .O(z06));
  nand2  g18(.a(new_n35_), .b(x15), .O(z07));
  nand3  g19(.a(new_n45_), .b(x12), .c(x09), .O(new_n49_));
  inv1   g20(.a(x00), .O(new_n50_));
  nor2   g21(.a(x10), .b(new_n50_), .O(new_n51_));
  nand4  g22(.a(new_n51_), .b(new_n49_), .c(new_n35_), .d(x11), .O(z08));
  inv1   g23(.a(new_n51_), .O(new_n53_));
  xor2a  g24(.a(x12), .b(x11), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n53_), .c(new_n35_), .O(z09));
  nand2  g26(.a(new_n44_), .b(x12), .O(new_n56_));
  nand3  g27(.a(new_n51_), .b(x11), .c(x09), .O(new_n57_));
  nor2   g28(.a(new_n57_), .b(new_n56_), .O(z10));
  aoi21  g29(.a(new_n44_), .b(x12), .c(new_n30_), .O(new_n59_));
  nand2  g30(.a(x12), .b(new_n30_), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n51_), .c(x11), .d(x09), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(new_n59_), .O(z11));
  nand3  g33(.a(x12), .b(x11), .c(new_n34_), .O(new_n63_));
  nor2   g34(.a(new_n63_), .b(new_n53_), .O(z12));
endmodule


