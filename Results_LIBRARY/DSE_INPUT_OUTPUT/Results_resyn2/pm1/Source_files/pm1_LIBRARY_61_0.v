// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x03), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nor2   g04(.a(x12), .b(x03), .O(new_n34_));
  aoi21  g05(.a(x12), .b(new_n33_), .c(new_n34_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g09(.a(x09), .O(new_n39_));
  nor2   g10(.a(new_n30_), .b(new_n39_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n36_), .c(x11), .O(new_n41_));
  xor2a  g12(.a(new_n41_), .b(new_n34_), .O(z03));
  nor2   g13(.a(new_n34_), .b(x14), .O(z04));
  inv1   g14(.a(new_n34_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x13), .O(z05));
  nand2  g16(.a(x12), .b(new_n33_), .O(new_n46_));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  aoi22  g20(.a(new_n49_), .b(new_n46_), .c(new_n47_), .d(new_n44_), .O(z06));
  nor2   g21(.a(new_n34_), .b(x15), .O(z07));
  nand2  g22(.a(new_n49_), .b(new_n40_), .O(new_n52_));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(new_n52_), .c(new_n44_), .O(z08));
  nand2  g27(.a(new_n30_), .b(x11), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(new_n46_), .c(new_n53_), .d(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n44_), .O(z09));
  nand2  g30(.a(new_n48_), .b(new_n40_), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n54_), .O(z10));
  inv1   g32(.a(x03), .O(new_n62_));
  oai21  g33(.a(new_n54_), .b(new_n47_), .c(x12), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g35(.a(x04), .b(x02), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(x12), .c(x09), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x01), .O(new_n67_));
  inv1   g38(.a(x01), .O(new_n68_));
  nand2  g39(.a(x12), .b(new_n68_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n67_), .c(new_n55_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n64_), .O(z11));
  nand2  g42(.a(new_n53_), .b(x00), .O(new_n72_));
  nand3  g43(.a(x12), .b(x11), .c(new_n39_), .O(new_n73_));
  oai21  g44(.a(new_n73_), .b(new_n72_), .c(new_n44_), .O(z12));
endmodule


