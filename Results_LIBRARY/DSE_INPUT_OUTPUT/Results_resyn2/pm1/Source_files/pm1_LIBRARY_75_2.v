// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n44_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x13), .O(z05));
  oai21  g05(.a(z05), .b(new_n31_), .c(x12), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  nand2  g07(.a(x11), .b(x09), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(x13), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z02));
  inv1   g10(.a(new_n36_), .O(new_n40_));
  oai21  g11(.a(new_n37_), .b(new_n40_), .c(x13), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z03));
  aoi21  g13(.a(z05), .b(x12), .c(x14), .O(z04));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  aoi21  g15(.a(x12), .b(new_n31_), .c(new_n44_), .O(new_n45_));
  nand2  g16(.a(z05), .b(x12), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(new_n45_), .O(z06));
  nand2  g19(.a(new_n46_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand3  g23(.a(x13), .b(x12), .c(x09), .O(new_n53_));
  oai21  g24(.a(new_n53_), .b(new_n44_), .c(new_n52_), .O(z08));
  inv1   g25(.a(x00), .O(new_n55_));
  nor2   g26(.a(x10), .b(new_n55_), .O(new_n56_));
  nand2  g27(.a(new_n32_), .b(x11), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n56_), .c(z01), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(z09));
  nand4  g30(.a(new_n56_), .b(new_n44_), .c(x11), .d(x09), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(x13), .c(new_n32_), .O(z10));
  nand2  g32(.a(x13), .b(x12), .O(new_n62_));
  oai21  g33(.a(x12), .b(new_n30_), .c(new_n62_), .O(new_n63_));
  nand4  g34(.a(new_n44_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  aoi22  g35(.a(new_n64_), .b(new_n63_), .c(new_n51_), .d(new_n46_), .O(z11));
  nor3   g36(.a(new_n62_), .b(new_n51_), .c(x09), .O(z12));
endmodule


