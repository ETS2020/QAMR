// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n31_, new_n33_, new_n34_, new_n37_, new_n40_, new_n41_, new_n42_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x11), .O(new_n31_));
  nand2  g01(.a(x12), .b(new_n31_), .O(z01));
  nand4  g02(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n33_));
  inv1   g03(.a(new_n33_), .O(new_n34_));
  nand4  g04(.a(new_n34_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g05(.a(new_n33_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g06(.a(x12), .b(x11), .O(new_n37_));
  nor2   g07(.a(new_n37_), .b(x14), .O(z04));
  nor2   g08(.a(new_n37_), .b(x13), .O(z05));
  inv1   g09(.a(new_n37_), .O(new_n40_));
  nand4  g10(.a(x11), .b(x04), .c(x03), .d(x02), .O(new_n41_));
  nand4  g11(.a(new_n41_), .b(new_n40_), .c(x09), .d(x01), .O(new_n42_));
  inv1   g12(.a(new_n42_), .O(z06));
  nor2   g13(.a(new_n37_), .b(x15), .O(z07));
  inv1   g14(.a(x10), .O(new_n45_));
  nand2  g15(.a(new_n45_), .b(x00), .O(new_n46_));
  nand2  g16(.a(new_n46_), .b(x11), .O(new_n47_));
  inv1   g17(.a(x09), .O(new_n48_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  oai21  g19(.a(new_n49_), .b(new_n48_), .c(x11), .O(new_n50_));
  nand2  g20(.a(new_n50_), .b(x12), .O(new_n51_));
  nand2  g21(.a(new_n51_), .b(new_n47_), .O(z08));
  inv1   g22(.a(x00), .O(new_n53_));
  nor2   g23(.a(x10), .b(new_n53_), .O(new_n54_));
  nand3  g24(.a(new_n54_), .b(x12), .c(x11), .O(new_n55_));
  nand2  g25(.a(new_n55_), .b(new_n40_), .O(z09));
  nand2  g26(.a(x11), .b(x09), .O(new_n57_));
  nand2  g27(.a(new_n49_), .b(x12), .O(new_n58_));
  nor3   g28(.a(new_n58_), .b(new_n46_), .c(new_n57_), .O(z10));
  inv1   g29(.a(x01), .O(new_n60_));
  aoi21  g30(.a(new_n49_), .b(x12), .c(new_n60_), .O(new_n61_));
  nand2  g31(.a(x09), .b(x01), .O(new_n62_));
  nand2  g32(.a(new_n62_), .b(x12), .O(new_n63_));
  nand3  g33(.a(new_n63_), .b(new_n54_), .c(x11), .O(new_n64_));
  nor2   g34(.a(new_n64_), .b(new_n61_), .O(z11));
  nor2   g35(.a(new_n55_), .b(x09), .O(z12));
  one    g36(.O(z00));
endmodule


