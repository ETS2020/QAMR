// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n44_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  nor2   g02(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nor3   g03(.a(x12), .b(x11), .c(x01), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(new_n32_), .O(z00));
  nand2  g05(.a(x12), .b(new_n31_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  and2   g08(.a(x12), .b(x09), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(x11), .c(new_n30_), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n37_), .O(z02));
  nand2  g12(.a(new_n40_), .b(new_n36_), .O(z03));
  nor2   g13(.a(new_n32_), .b(x14), .O(z04));
  inv1   g14(.a(new_n32_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x13), .O(z05));
  inv1   g16(.a(x12), .O(new_n46_));
  nand4  g17(.a(new_n46_), .b(x04), .c(x03), .d(x02), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x09), .O(new_n48_));
  aoi21  g19(.a(new_n48_), .b(new_n31_), .c(new_n30_), .O(z06));
  nor2   g20(.a(new_n32_), .b(x15), .O(z07));
  nor2   g21(.a(new_n31_), .b(x01), .O(new_n51_));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n38_), .O(new_n54_));
  inv1   g25(.a(x00), .O(new_n55_));
  nor2   g26(.a(x10), .b(new_n55_), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(z08));
  nand2  g28(.a(new_n46_), .b(x11), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n56_), .c(z01), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n44_), .O(z09));
  inv1   g31(.a(x10), .O(new_n61_));
  nand4  g32(.a(new_n52_), .b(new_n38_), .c(new_n61_), .d(x00), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(new_n30_), .c(new_n31_), .O(z10));
  nand2  g34(.a(new_n56_), .b(new_n51_), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(x12), .O(z11));
  nor2   g36(.a(new_n46_), .b(x09), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n56_), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n30_), .c(new_n31_), .O(z12));
endmodule


