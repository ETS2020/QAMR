// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x08), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  or2    g04(.a(new_n33_), .b(x01), .O(z00));
  nand2  g05(.a(x12), .b(new_n31_), .O(z01));
  nand3  g06(.a(x07), .b(x06), .c(x05), .O(new_n36_));
  nand2  g07(.a(x11), .b(x09), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(x12), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x08), .O(z02));
  nand2  g10(.a(new_n32_), .b(x08), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  inv1   g12(.a(new_n36_), .O(new_n42_));
  nand3  g13(.a(x12), .b(x11), .c(x09), .O(new_n43_));
  aoi21  g14(.a(new_n42_), .b(x08), .c(new_n43_), .O(new_n44_));
  nor2   g15(.a(new_n44_), .b(new_n41_), .O(z03));
  nor2   g16(.a(new_n41_), .b(x14), .O(z04));
  nand2  g17(.a(new_n40_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  nand3  g20(.a(new_n40_), .b(x09), .c(x01), .O(new_n50_));
  aoi21  g21(.a(new_n49_), .b(z01), .c(new_n50_), .O(z06));
  nor2   g22(.a(new_n41_), .b(x15), .O(z07));
  nand2  g23(.a(x12), .b(x09), .O(new_n53_));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x11), .b(new_n54_), .c(x00), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(new_n56_));
  oai22  g27(.a(new_n56_), .b(new_n41_), .c(new_n48_), .d(new_n53_), .O(z08));
  nand2  g28(.a(new_n54_), .b(x00), .O(new_n58_));
  nand2  g29(.a(x12), .b(x11), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(new_n33_), .c(new_n58_), .O(z09));
  nor3   g31(.a(new_n55_), .b(new_n49_), .c(new_n53_), .O(z10));
  inv1   g32(.a(x01), .O(new_n62_));
  nand4  g33(.a(x11), .b(new_n54_), .c(new_n62_), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n30_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n32_), .O(new_n65_));
  inv1   g36(.a(new_n53_), .O(new_n66_));
  nand4  g37(.a(new_n56_), .b(new_n48_), .c(new_n66_), .d(x01), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n65_), .O(z11));
  nor3   g39(.a(new_n59_), .b(new_n58_), .c(x09), .O(z12));
endmodule


