// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  inv1   g03(.a(x08), .O(new_n33_));
  nand2  g04(.a(x13), .b(new_n33_), .O(new_n34_));
  nand4  g05(.a(new_n34_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand3  g06(.a(new_n34_), .b(x12), .c(new_n31_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(x12), .b(x11), .c(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n38_), .O(z02));
  inv1   g12(.a(new_n34_), .O(new_n42_));
  oai21  g13(.a(new_n40_), .b(new_n42_), .c(new_n37_), .O(z03));
  nor2   g14(.a(new_n42_), .b(x14), .O(z04));
  inv1   g15(.a(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  aoi21  g17(.a(x12), .b(new_n31_), .c(new_n46_), .O(new_n47_));
  nand3  g18(.a(new_n34_), .b(x09), .c(x01), .O(new_n48_));
  nor2   g19(.a(new_n48_), .b(new_n47_), .O(z06));
  nand2  g20(.a(new_n34_), .b(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  inv1   g24(.a(new_n46_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x12), .c(x09), .O(new_n55_));
  aoi21  g26(.a(new_n55_), .b(new_n53_), .c(new_n42_), .O(z08));
  nand2  g27(.a(new_n51_), .b(x00), .O(new_n57_));
  xor2a  g28(.a(x12), .b(x11), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(new_n57_), .c(new_n34_), .O(z09));
  nand3  g30(.a(new_n34_), .b(new_n51_), .c(x00), .O(new_n60_));
  nand2  g31(.a(new_n46_), .b(new_n40_), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(new_n60_), .O(z10));
  nand2  g33(.a(new_n32_), .b(new_n30_), .O(new_n63_));
  nand4  g34(.a(new_n46_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  nand4  g35(.a(new_n34_), .b(x11), .c(new_n51_), .d(x00), .O(new_n65_));
  aoi21  g36(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(z11));
  inv1   g37(.a(x09), .O(new_n67_));
  nand3  g38(.a(x12), .b(x11), .c(new_n67_), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n60_), .O(z12));
endmodule


