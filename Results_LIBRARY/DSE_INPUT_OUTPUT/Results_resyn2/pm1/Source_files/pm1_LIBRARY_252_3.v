// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n41_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x07), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nor2   g02(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g03(.a(x01), .O(new_n33_));
  nand2  g04(.a(new_n31_), .b(new_n33_), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n32_), .O(z00));
  nand2  g07(.a(x11), .b(new_n30_), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x12), .O(z01));
  nand4  g09(.a(x12), .b(x11), .c(x09), .d(new_n30_), .O(z03));
  inv1   g10(.a(new_n32_), .O(new_n41_));
  nand2  g11(.a(new_n41_), .b(x14), .O(z04));
  nand2  g12(.a(new_n41_), .b(x13), .O(z05));
  nand2  g13(.a(x09), .b(x01), .O(new_n44_));
  oai21  g14(.a(new_n44_), .b(x11), .c(new_n30_), .O(new_n45_));
  nand2  g15(.a(new_n45_), .b(x12), .O(new_n46_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  inv1   g17(.a(new_n47_), .O(new_n48_));
  oai21  g18(.a(new_n48_), .b(new_n44_), .c(new_n46_), .O(z06));
  nor2   g19(.a(new_n32_), .b(x15), .O(z07));
  aoi21  g20(.a(new_n48_), .b(x09), .c(x07), .O(new_n51_));
  inv1   g21(.a(x10), .O(new_n52_));
  nand3  g22(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  inv1   g23(.a(new_n53_), .O(new_n54_));
  oai21  g24(.a(new_n51_), .b(new_n31_), .c(new_n54_), .O(z08));
  nand2  g25(.a(new_n31_), .b(x11), .O(new_n56_));
  nand3  g26(.a(new_n56_), .b(new_n52_), .c(x00), .O(new_n57_));
  aoi21  g27(.a(new_n37_), .b(x12), .c(new_n57_), .O(z09));
  inv1   g28(.a(x09), .O(new_n59_));
  nand2  g29(.a(new_n47_), .b(x12), .O(new_n60_));
  nand4  g30(.a(x11), .b(new_n52_), .c(new_n30_), .d(x00), .O(new_n61_));
  nor3   g31(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(z10));
  nand4  g32(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n63_));
  nand2  g33(.a(new_n63_), .b(new_n34_), .O(new_n64_));
  nand2  g34(.a(new_n64_), .b(new_n54_), .O(new_n65_));
  nand2  g35(.a(new_n65_), .b(new_n41_), .O(z11));
  nor3   g36(.a(new_n61_), .b(new_n31_), .c(x09), .O(z12));
  one    g37(.O(z02));
endmodule


