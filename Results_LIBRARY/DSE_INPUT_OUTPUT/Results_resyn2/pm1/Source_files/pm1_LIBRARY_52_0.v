// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x13), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nor2   g04(.a(x13), .b(x12), .O(new_n34_));
  aoi21  g05(.a(x12), .b(new_n33_), .c(new_n34_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  nand3  g07(.a(x12), .b(x11), .c(x09), .O(new_n37_));
  nor2   g08(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(new_n34_), .O(z02));
  nand2  g10(.a(x13), .b(new_n30_), .O(new_n40_));
  nand3  g11(.a(new_n36_), .b(x11), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n40_), .O(z03));
  inv1   g14(.a(new_n34_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  nor2   g16(.a(x13), .b(new_n30_), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  aoi21  g18(.a(x12), .b(new_n33_), .c(new_n47_), .O(new_n48_));
  nand2  g19(.a(x09), .b(x01), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n48_), .c(new_n44_), .O(z06));
  nor2   g21(.a(new_n34_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n44_), .O(new_n54_));
  inv1   g25(.a(new_n47_), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x12), .c(x09), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n54_), .O(z08));
  nand2  g28(.a(x12), .b(new_n33_), .O(new_n58_));
  nand2  g29(.a(new_n30_), .b(x11), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n58_), .c(new_n52_), .d(x00), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n44_), .O(z09));
  inv1   g32(.a(x09), .O(new_n62_));
  nand2  g33(.a(new_n47_), .b(x12), .O(new_n63_));
  nor3   g34(.a(new_n63_), .b(new_n53_), .c(new_n62_), .O(z10));
  nand4  g35(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n65_));
  inv1   g36(.a(x01), .O(new_n66_));
  nand3  g37(.a(x13), .b(new_n30_), .c(new_n66_), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n65_), .c(new_n53_), .O(z11));
  nand2  g39(.a(new_n52_), .b(x00), .O(new_n69_));
  nand3  g40(.a(x12), .b(x11), .c(new_n62_), .O(new_n70_));
  oai21  g41(.a(new_n70_), .b(new_n69_), .c(new_n44_), .O(z12));
endmodule


