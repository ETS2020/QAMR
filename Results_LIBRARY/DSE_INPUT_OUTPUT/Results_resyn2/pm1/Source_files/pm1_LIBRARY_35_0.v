// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n45_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x05), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  nand2  g03(.a(new_n30_), .b(x05), .O(new_n33_));
  nand2  g04(.a(x12), .b(x11), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n33_), .O(z01));
  inv1   g06(.a(x05), .O(new_n36_));
  nand2  g07(.a(x11), .b(x09), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(x12), .O(new_n38_));
  nand3  g09(.a(x08), .b(x07), .c(x06), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n38_), .c(new_n33_), .O(z02));
  nand2  g12(.a(new_n40_), .b(x05), .O(new_n42_));
  nand2  g13(.a(new_n37_), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n42_), .O(z03));
  nor2   g15(.a(x12), .b(x05), .O(new_n45_));
  nor2   g16(.a(new_n45_), .b(x14), .O(z04));
  inv1   g17(.a(new_n45_), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x13), .O(z05));
  inv1   g19(.a(x11), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  aoi21  g21(.a(x12), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  nand2  g22(.a(x09), .b(x01), .O(new_n52_));
  oai21  g23(.a(new_n52_), .b(new_n51_), .c(new_n47_), .O(z06));
  nor2   g24(.a(new_n45_), .b(x15), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x11), .b(new_n55_), .c(x00), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(new_n57_));
  inv1   g28(.a(new_n50_), .O(new_n58_));
  inv1   g29(.a(x09), .O(new_n59_));
  nor2   g30(.a(new_n30_), .b(new_n59_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  oai21  g32(.a(new_n57_), .b(new_n45_), .c(new_n61_), .O(z08));
  nand2  g33(.a(new_n55_), .b(x00), .O(new_n63_));
  nand3  g34(.a(new_n30_), .b(new_n49_), .c(x05), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(new_n34_), .c(new_n63_), .O(z09));
  nand2  g36(.a(new_n60_), .b(new_n50_), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n56_), .c(new_n47_), .O(z10));
  inv1   g38(.a(x01), .O(new_n68_));
  nand4  g39(.a(x11), .b(new_n55_), .c(new_n68_), .d(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(x05), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n30_), .O(new_n71_));
  nand4  g42(.a(new_n60_), .b(new_n57_), .c(new_n50_), .d(x01), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n71_), .O(z11));
  nand3  g44(.a(x12), .b(x11), .c(new_n59_), .O(new_n74_));
  oai21  g45(.a(new_n74_), .b(new_n63_), .c(new_n47_), .O(z12));
endmodule


