// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n37_, new_n38_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x05), .O(new_n33_));
  nand2  g04(.a(x11), .b(new_n33_), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x12), .O(z01));
  nand2  g06(.a(x12), .b(x05), .O(z02));
  nand3  g07(.a(x12), .b(x11), .c(new_n33_), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x09), .O(z03));
  nand2  g10(.a(z02), .b(x14), .O(z04));
  nand2  g11(.a(z02), .b(x13), .O(z05));
  nand2  g12(.a(x09), .b(x01), .O(new_n42_));
  oai21  g13(.a(new_n42_), .b(x11), .c(new_n33_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(new_n44_));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  oai21  g17(.a(new_n46_), .b(new_n42_), .c(new_n44_), .O(z06));
  nand2  g18(.a(z02), .b(x15), .O(z07));
  inv1   g19(.a(x09), .O(new_n49_));
  oai21  g20(.a(new_n45_), .b(new_n49_), .c(new_n33_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x12), .O(new_n51_));
  inv1   g22(.a(x00), .O(new_n52_));
  nor2   g23(.a(x10), .b(new_n52_), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(new_n51_), .c(x11), .O(z08));
  nand2  g25(.a(new_n30_), .b(x11), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  aoi21  g27(.a(new_n34_), .b(x12), .c(new_n56_), .O(z09));
  nand4  g28(.a(new_n53_), .b(new_n45_), .c(x11), .d(x09), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(new_n33_), .c(new_n30_), .O(z10));
  nand2  g30(.a(new_n53_), .b(x11), .O(new_n60_));
  nand3  g31(.a(x12), .b(x09), .c(x01), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(new_n62_));
  nor2   g33(.a(x12), .b(x01), .O(new_n63_));
  aoi21  g34(.a(new_n62_), .b(new_n45_), .c(new_n63_), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n60_), .c(z02), .O(z11));
  nand2  g36(.a(new_n53_), .b(new_n49_), .O(new_n66_));
  nor2   g37(.a(new_n66_), .b(new_n37_), .O(z12));
endmodule


