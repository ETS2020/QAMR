// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n44_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x04), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  inv1   g05(.a(x04), .O(new_n35_));
  nor2   g06(.a(x12), .b(new_n35_), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(x12), .b(x11), .c(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  aoi21  g11(.a(new_n40_), .b(new_n38_), .c(new_n36_), .O(z02));
  nand2  g12(.a(new_n40_), .b(new_n37_), .O(z03));
  nor2   g13(.a(new_n36_), .b(x14), .O(z04));
  nand2  g14(.a(new_n30_), .b(x04), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x13), .O(z05));
  nand4  g16(.a(x11), .b(x04), .c(x03), .d(x02), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n44_), .O(z06));
  nand2  g19(.a(new_n44_), .b(x15), .O(z07));
  nand3  g20(.a(x09), .b(x03), .c(x02), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x12), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(x04), .O(new_n52_));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n52_), .O(z08));
  nand2  g27(.a(new_n30_), .b(x11), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(z01), .c(new_n53_), .d(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n44_), .O(z09));
  nand3  g30(.a(x04), .b(x03), .c(x02), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n53_), .c(x00), .O(new_n61_));
  oai21  g32(.a(new_n61_), .b(new_n39_), .c(new_n44_), .O(z10));
  nand3  g33(.a(x12), .b(x09), .c(x01), .O(new_n63_));
  inv1   g34(.a(x01), .O(new_n64_));
  nand3  g35(.a(new_n30_), .b(new_n35_), .c(new_n64_), .O(new_n65_));
  nand4  g36(.a(new_n60_), .b(x11), .c(new_n53_), .d(x00), .O(new_n66_));
  aoi21  g37(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(z11));
  inv1   g38(.a(x09), .O(new_n68_));
  nand2  g39(.a(x12), .b(new_n68_), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n54_), .O(z12));
endmodule


