// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x05), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nor2   g04(.a(x12), .b(x05), .O(new_n34_));
  aoi21  g05(.a(x12), .b(new_n33_), .c(new_n34_), .O(z01));
  inv1   g06(.a(x05), .O(new_n36_));
  nand2  g07(.a(x11), .b(x09), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(x12), .O(new_n38_));
  nand3  g09(.a(x08), .b(x07), .c(x06), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(new_n40_));
  nand2  g11(.a(new_n30_), .b(x05), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(z02));
  nand2  g13(.a(new_n40_), .b(x05), .O(new_n43_));
  nand2  g14(.a(new_n37_), .b(x12), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n43_), .O(z03));
  nor2   g16(.a(new_n34_), .b(x14), .O(z04));
  nor2   g17(.a(new_n34_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  aoi21  g19(.a(x12), .b(new_n33_), .c(new_n48_), .O(new_n49_));
  nand2  g20(.a(new_n30_), .b(new_n36_), .O(new_n50_));
  and2   g21(.a(x09), .b(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor2   g23(.a(new_n52_), .b(new_n49_), .O(z06));
  nor2   g24(.a(new_n34_), .b(x15), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x11), .b(new_n55_), .c(x00), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  inv1   g28(.a(new_n48_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(x12), .c(x09), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n57_), .O(z08));
  nand2  g31(.a(x12), .b(new_n33_), .O(new_n61_));
  nand2  g32(.a(new_n30_), .b(x11), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n61_), .c(new_n55_), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n50_), .O(z09));
  nand2  g35(.a(new_n48_), .b(x12), .O(new_n65_));
  nand4  g36(.a(x11), .b(new_n55_), .c(x09), .d(x00), .O(new_n66_));
  nor2   g37(.a(new_n66_), .b(new_n65_), .O(z10));
  nand4  g38(.a(new_n48_), .b(x12), .c(x09), .d(x01), .O(new_n68_));
  inv1   g39(.a(x01), .O(new_n69_));
  nand3  g40(.a(new_n30_), .b(x05), .c(new_n69_), .O(new_n70_));
  aoi21  g41(.a(new_n70_), .b(new_n68_), .c(new_n56_), .O(z11));
  inv1   g42(.a(x09), .O(new_n72_));
  nand2  g43(.a(x12), .b(new_n72_), .O(new_n73_));
  oai21  g44(.a(new_n73_), .b(new_n56_), .c(new_n50_), .O(z12));
endmodule


