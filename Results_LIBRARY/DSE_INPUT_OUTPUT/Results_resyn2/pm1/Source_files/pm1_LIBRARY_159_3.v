// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n61_,
    new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand2  g07(.a(x12), .b(x09), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nor2   g09(.a(x15), .b(new_n31_), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z02));
  inv1   g11(.a(x15), .O(new_n41_));
  oai21  g12(.a(new_n37_), .b(new_n36_), .c(new_n41_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x11), .O(z03));
  nand2  g14(.a(x15), .b(x11), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  nand2  g16(.a(new_n44_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  inv1   g18(.a(new_n47_), .O(new_n48_));
  nand3  g19(.a(new_n44_), .b(x09), .c(x01), .O(new_n49_));
  aoi21  g20(.a(new_n48_), .b(z01), .c(new_n49_), .O(z06));
  nand2  g21(.a(x15), .b(new_n31_), .O(z07));
  inv1   g22(.a(x00), .O(new_n52_));
  nor2   g23(.a(x10), .b(new_n52_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n39_), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  oai21  g26(.a(new_n47_), .b(new_n37_), .c(new_n55_), .O(z08));
  nand2  g27(.a(new_n32_), .b(x11), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n53_), .c(z01), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n44_), .O(z09));
  nor3   g30(.a(new_n54_), .b(new_n48_), .c(new_n37_), .O(z10));
  nand2  g31(.a(new_n32_), .b(new_n30_), .O(new_n61_));
  nand4  g32(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n53_), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(new_n41_), .c(new_n31_), .O(z11));
  nor3   g36(.a(new_n54_), .b(new_n32_), .c(x09), .O(z12));
endmodule


