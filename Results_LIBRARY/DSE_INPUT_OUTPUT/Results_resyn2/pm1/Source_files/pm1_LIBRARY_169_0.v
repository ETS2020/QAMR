// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand4  g03(.a(new_n32_), .b(new_n31_), .c(x09), .d(new_n30_), .O(z00));
  nor2   g04(.a(x12), .b(x09), .O(new_n34_));
  aoi21  g05(.a(x12), .b(new_n31_), .c(new_n34_), .O(z01));
  nor2   g06(.a(new_n32_), .b(new_n31_), .O(new_n36_));
  nand3  g07(.a(x09), .b(x08), .c(x07), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n36_), .c(x06), .d(x05), .O(z02));
  nand3  g10(.a(new_n38_), .b(x06), .c(x05), .O(new_n40_));
  inv1   g11(.a(x09), .O(new_n41_));
  nand2  g12(.a(x12), .b(new_n41_), .O(new_n42_));
  oai21  g13(.a(new_n32_), .b(new_n31_), .c(x09), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(z03));
  inv1   g15(.a(new_n34_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x14), .O(z04));
  nand2  g17(.a(new_n45_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  aoi21  g19(.a(x12), .b(new_n31_), .c(new_n48_), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  oai21  g21(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(z06));
  nor2   g22(.a(new_n34_), .b(x15), .O(z07));
  inv1   g23(.a(new_n48_), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(x12), .c(x09), .O(new_n54_));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x11), .b(new_n55_), .c(x00), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n54_), .c(new_n45_), .O(z08));
  aoi21  g29(.a(new_n31_), .b(x09), .c(x12), .O(new_n59_));
  nand2  g30(.a(x12), .b(new_n31_), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n55_), .c(x00), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(new_n59_), .O(z09));
  nand2  g33(.a(new_n48_), .b(x12), .O(new_n63_));
  nand4  g34(.a(x11), .b(new_n55_), .c(x09), .d(x00), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n63_), .c(new_n45_), .O(z10));
  nand2  g36(.a(new_n32_), .b(new_n30_), .O(new_n66_));
  nand3  g37(.a(new_n48_), .b(x12), .c(x01), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(z11));
  nor2   g39(.a(new_n56_), .b(new_n42_), .O(z12));
endmodule


