// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x05), .c(new_n30_), .O(z00));
  nor2   g03(.a(new_n30_), .b(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(x11), .O(new_n34_));
  oai21  g05(.a(new_n33_), .b(x05), .c(new_n34_), .O(z01));
  nand3  g06(.a(x08), .b(x07), .c(x06), .O(new_n36_));
  nand2  g07(.a(x11), .b(x09), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(x12), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x05), .O(z02));
  inv1   g10(.a(x05), .O(new_n40_));
  nand3  g11(.a(x12), .b(x11), .c(x09), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  oai21  g13(.a(new_n36_), .b(new_n40_), .c(new_n42_), .O(z03));
  nor2   g14(.a(x12), .b(new_n40_), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x14), .O(z04));
  nor2   g17(.a(new_n44_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  nor2   g19(.a(new_n48_), .b(new_n33_), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  oai21  g21(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(z06));
  nand2  g22(.a(new_n45_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  inv1   g26(.a(new_n48_), .O(new_n56_));
  and2   g27(.a(x12), .b(x09), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g29(.a(new_n55_), .b(new_n44_), .c(new_n58_), .O(z08));
  inv1   g30(.a(x11), .O(new_n60_));
  nand2  g31(.a(x12), .b(new_n60_), .O(new_n61_));
  nand2  g32(.a(new_n30_), .b(x11), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n61_), .c(new_n53_), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n45_), .O(z09));
  nand2  g35(.a(new_n57_), .b(new_n48_), .O(new_n65_));
  nor2   g36(.a(new_n65_), .b(new_n54_), .O(z10));
  inv1   g37(.a(x01), .O(new_n67_));
  nand4  g38(.a(x11), .b(new_n53_), .c(new_n67_), .d(x00), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n40_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n30_), .O(new_n70_));
  nand4  g41(.a(new_n57_), .b(new_n55_), .c(new_n48_), .d(x01), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n70_), .O(z11));
  nand2  g43(.a(new_n53_), .b(x00), .O(new_n73_));
  nor3   g44(.a(new_n73_), .b(new_n34_), .c(x09), .O(z12));
endmodule


