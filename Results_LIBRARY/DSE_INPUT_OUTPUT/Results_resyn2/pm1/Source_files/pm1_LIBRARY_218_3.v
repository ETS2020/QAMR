// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n36_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n45_, new_n46_, new_n48_, new_n49_, new_n52_, new_n53_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  nand2  g05(.a(x12), .b(x11), .O(z02));
  inv1   g06(.a(z02), .O(new_n36_));
  nor2   g07(.a(new_n36_), .b(x14), .O(z04));
  nor2   g08(.a(new_n36_), .b(x13), .O(z05));
  and2   g09(.a(x03), .b(x02), .O(new_n39_));
  inv1   g10(.a(x04), .O(new_n40_));
  nor2   g11(.a(x12), .b(new_n40_), .O(new_n41_));
  nand3  g12(.a(z02), .b(x09), .c(x01), .O(new_n42_));
  aoi21  g13(.a(new_n41_), .b(new_n39_), .c(new_n42_), .O(z06));
  nand2  g14(.a(z02), .b(x15), .O(z07));
  inv1   g15(.a(x00), .O(new_n45_));
  oai21  g16(.a(x10), .b(new_n45_), .c(new_n32_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x11), .O(z08));
  inv1   g18(.a(x10), .O(new_n48_));
  nand4  g19(.a(new_n32_), .b(new_n31_), .c(new_n48_), .d(x00), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(z09));
  nand3  g21(.a(new_n32_), .b(new_n48_), .c(x00), .O(new_n52_));
  nand2  g22(.a(x11), .b(new_n30_), .O(new_n53_));
  nor2   g23(.a(new_n53_), .b(new_n52_), .O(z11));
  zero   g24(.O(z10));
  zero   g25(.O(z12));
  nand2  g26(.a(x12), .b(x11), .O(z03));
endmodule


