// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n44_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_,
    new_n70_;
  nand2  g00(.a(x15), .b(x12), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(new_n31_));
  inv1   g02(.a(x01), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n31_), .O(z00));
  inv1   g07(.a(x11), .O(new_n37_));
  inv1   g08(.a(x15), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(x12), .c(new_n37_), .O(z01));
  nand2  g10(.a(x11), .b(x09), .O(new_n40_));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  oai21  g12(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z02));
  and2   g14(.a(x11), .b(x09), .O(new_n44_));
  nand4  g15(.a(new_n41_), .b(new_n44_), .c(new_n38_), .d(x12), .O(z03));
  nor2   g16(.a(new_n31_), .b(x14), .O(z04));
  nor2   g17(.a(new_n31_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  and2   g20(.a(x09), .b(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n30_), .O(new_n51_));
  aoi21  g22(.a(new_n49_), .b(z01), .c(new_n51_), .O(z06));
  nand2  g23(.a(x15), .b(new_n33_), .O(z07));
  nand4  g24(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n38_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x12), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x11), .b(new_n57_), .c(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n56_), .O(z08));
  nand2  g31(.a(x12), .b(new_n37_), .O(new_n61_));
  nand2  g32(.a(new_n33_), .b(x11), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n61_), .c(new_n57_), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n30_), .O(z09));
  nand4  g35(.a(new_n48_), .b(new_n44_), .c(new_n57_), .d(x00), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n38_), .c(new_n33_), .O(z10));
  nand4  g37(.a(new_n50_), .b(new_n48_), .c(new_n38_), .d(x12), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n34_), .c(new_n58_), .O(z11));
  inv1   g39(.a(x09), .O(new_n69_));
  nand4  g40(.a(x11), .b(new_n57_), .c(new_n69_), .d(x00), .O(new_n70_));
  aoi21  g41(.a(new_n70_), .b(new_n38_), .c(new_n33_), .O(z12));
endmodule


