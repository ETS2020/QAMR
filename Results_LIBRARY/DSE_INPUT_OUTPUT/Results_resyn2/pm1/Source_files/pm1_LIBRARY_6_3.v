// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_;
  inv1   g00(.a(x11), .O(new_n30_));
  nand2  g01(.a(x15), .b(x12), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  nor2   g03(.a(x12), .b(x01), .O(new_n33_));
  aoi21  g04(.a(new_n33_), .b(new_n30_), .c(new_n32_), .O(z00));
  inv1   g05(.a(x12), .O(new_n35_));
  inv1   g06(.a(x15), .O(z07));
  aoi21  g07(.a(z07), .b(x11), .c(new_n35_), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(z01));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  nand2  g10(.a(x11), .b(x09), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n39_), .c(z07), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z02));
  inv1   g13(.a(new_n39_), .O(new_n43_));
  oai21  g14(.a(new_n40_), .b(new_n43_), .c(z07), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x12), .O(z03));
  nand2  g16(.a(new_n31_), .b(x14), .O(z04));
  nor2   g17(.a(new_n32_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  aoi21  g19(.a(x12), .b(new_n30_), .c(new_n48_), .O(new_n49_));
  nand3  g20(.a(new_n31_), .b(x09), .c(x01), .O(new_n50_));
  nor2   g21(.a(new_n50_), .b(new_n49_), .O(z06));
  inv1   g22(.a(x09), .O(new_n52_));
  oai21  g23(.a(new_n48_), .b(new_n52_), .c(z07), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x12), .O(new_n54_));
  inv1   g25(.a(x00), .O(new_n55_));
  nor2   g26(.a(x10), .b(new_n55_), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n54_), .c(x11), .O(z08));
  nand2  g28(.a(new_n35_), .b(x11), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(new_n37_), .O(z09));
  nand4  g31(.a(new_n56_), .b(new_n48_), .c(x11), .d(x09), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(z07), .c(new_n35_), .O(z10));
  nand2  g33(.a(new_n56_), .b(x11), .O(new_n63_));
  nand3  g34(.a(x12), .b(x09), .c(x01), .O(new_n64_));
  inv1   g35(.a(new_n64_), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n48_), .c(new_n33_), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n63_), .c(new_n31_), .O(z11));
  nand3  g38(.a(z07), .b(x12), .c(new_n52_), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n63_), .O(z12));
endmodule


