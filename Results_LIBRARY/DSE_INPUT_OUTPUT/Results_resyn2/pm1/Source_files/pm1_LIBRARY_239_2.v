// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n70_, new_n71_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand3  g04(.a(x14), .b(x12), .c(new_n31_), .O(z01));
  nand2  g05(.a(x11), .b(x09), .O(new_n35_));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x14), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x12), .O(z02));
  and2   g09(.a(x11), .b(x09), .O(new_n39_));
  and2   g10(.a(x14), .b(x12), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n36_), .c(new_n39_), .O(z03));
  nor2   g12(.a(x14), .b(x12), .O(z04));
  inv1   g13(.a(x14), .O(new_n43_));
  aoi21  g14(.a(new_n43_), .b(x12), .c(x13), .O(z05));
  nand2  g15(.a(x09), .b(x01), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(x11), .c(x14), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(new_n47_));
  inv1   g18(.a(new_n45_), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n47_), .O(z06));
  nand2  g22(.a(new_n43_), .b(x12), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x15), .O(z07));
  nand4  g24(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x14), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x12), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x11), .b(new_n57_), .c(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n56_), .O(z08));
  nand2  g31(.a(x12), .b(new_n31_), .O(new_n61_));
  nand2  g32(.a(new_n32_), .b(x11), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n61_), .c(new_n57_), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n52_), .O(z09));
  nand4  g35(.a(new_n49_), .b(new_n39_), .c(new_n57_), .d(x00), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(x14), .c(new_n32_), .O(z10));
  nand3  g37(.a(new_n49_), .b(new_n48_), .c(new_n40_), .O(new_n67_));
  nand2  g38(.a(new_n32_), .b(new_n30_), .O(new_n68_));
  aoi21  g39(.a(new_n68_), .b(new_n67_), .c(new_n58_), .O(z11));
  inv1   g40(.a(x09), .O(new_n70_));
  nand2  g41(.a(new_n40_), .b(new_n70_), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n58_), .O(z12));
endmodule


