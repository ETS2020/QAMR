// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n42_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x00), .O(new_n30_));
  nor3   g01(.a(x12), .b(x11), .c(x01), .O(new_n31_));
  aoi21  g02(.a(x12), .b(new_n30_), .c(new_n31_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand3  g04(.a(x12), .b(new_n33_), .c(x00), .O(z01));
  nand2  g05(.a(x11), .b(x09), .O(new_n35_));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x00), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x12), .O(z02));
  inv1   g09(.a(new_n36_), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n35_), .c(x00), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(z03));
  nand2  g12(.a(x12), .b(new_n30_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x14), .O(z04));
  nand2  g14(.a(new_n42_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  aoi21  g16(.a(x12), .b(new_n33_), .c(new_n45_), .O(new_n46_));
  nand3  g17(.a(new_n42_), .b(x09), .c(x01), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(new_n46_), .O(z06));
  nand2  g19(.a(new_n42_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n50_));
  nand2  g21(.a(x11), .b(new_n50_), .O(new_n51_));
  nand2  g22(.a(x12), .b(x09), .O(new_n52_));
  nor2   g23(.a(new_n52_), .b(new_n45_), .O(new_n53_));
  oai21  g24(.a(new_n53_), .b(new_n51_), .c(x00), .O(new_n54_));
  inv1   g25(.a(x12), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n30_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n54_), .O(z08));
  aoi21  g28(.a(new_n33_), .b(x00), .c(x12), .O(new_n58_));
  oai21  g29(.a(new_n55_), .b(x11), .c(new_n50_), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n58_), .c(new_n42_), .O(z09));
  inv1   g31(.a(x09), .O(new_n61_));
  nand2  g32(.a(new_n45_), .b(x12), .O(new_n62_));
  nand3  g33(.a(x11), .b(new_n50_), .c(x00), .O(new_n63_));
  nor3   g34(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(z10));
  nand2  g35(.a(x09), .b(x01), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(x12), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x11), .c(new_n50_), .d(x00), .O(new_n67_));
  aoi21  g38(.a(new_n62_), .b(x01), .c(new_n67_), .O(z11));
  nand3  g39(.a(x11), .b(new_n50_), .c(new_n61_), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(x00), .c(new_n55_), .O(z12));
endmodule


