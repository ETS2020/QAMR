// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n34_, new_n35_, new_n38_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_;
  inv1   g00(.a(x11), .O(new_n30_));
  nor2   g01(.a(x12), .b(x01), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(new_n30_), .c(x03), .O(z00));
  nand3  g03(.a(x12), .b(new_n30_), .c(x03), .O(z01));
  nand4  g04(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand4  g06(.a(new_n35_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g07(.a(new_n34_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g08(.a(x11), .b(x03), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(x14), .O(z04));
  nor2   g10(.a(new_n38_), .b(x13), .O(z05));
  inv1   g11(.a(new_n38_), .O(new_n41_));
  nand2  g12(.a(x12), .b(new_n30_), .O(new_n42_));
  nand2  g13(.a(x09), .b(x01), .O(new_n43_));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  aoi22  g16(.a(new_n45_), .b(new_n42_), .c(new_n43_), .d(new_n41_), .O(z06));
  nand2  g17(.a(new_n41_), .b(x15), .O(z07));
  inv1   g18(.a(x10), .O(new_n48_));
  nand3  g19(.a(x11), .b(new_n48_), .c(x00), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  nand2  g21(.a(x12), .b(x09), .O(new_n51_));
  oai21  g22(.a(new_n51_), .b(new_n44_), .c(new_n50_), .O(z08));
  nor2   g23(.a(x12), .b(new_n30_), .O(new_n53_));
  nand3  g24(.a(new_n42_), .b(new_n48_), .c(x00), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n53_), .c(new_n41_), .O(z09));
  nor3   g26(.a(new_n51_), .b(new_n49_), .c(new_n45_), .O(z10));
  inv1   g27(.a(x03), .O(new_n57_));
  nand2  g28(.a(new_n48_), .b(x00), .O(new_n58_));
  nand3  g29(.a(x12), .b(x09), .c(x01), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n58_), .c(x11), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  aoi21  g32(.a(x04), .b(x02), .c(new_n59_), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n31_), .c(new_n50_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n61_), .O(z11));
  inv1   g35(.a(x09), .O(new_n65_));
  nand2  g36(.a(x12), .b(new_n65_), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n49_), .c(new_n41_), .O(z12));
endmodule


