// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n70_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(new_n30_), .b(x07), .O(new_n31_));
  inv1   g02(.a(x01), .O(new_n32_));
  nand2  g03(.a(new_n30_), .b(new_n32_), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(x11), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(new_n31_), .O(z00));
  nand2  g06(.a(x11), .b(x07), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x12), .O(z01));
  nand3  g08(.a(x08), .b(x06), .c(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  and2   g10(.a(x11), .b(x09), .O(new_n40_));
  and2   g11(.a(x12), .b(x07), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(z02));
  nand2  g13(.a(new_n40_), .b(new_n38_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x07), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x12), .O(z03));
  oai21  g16(.a(new_n30_), .b(x07), .c(x14), .O(z04));
  nor2   g17(.a(new_n31_), .b(x13), .O(z05));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(x11), .c(x07), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x12), .O(new_n50_));
  inv1   g21(.a(new_n48_), .O(new_n51_));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n50_), .O(z06));
  nor2   g25(.a(new_n31_), .b(x15), .O(z07));
  nand4  g26(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x07), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x12), .O(new_n58_));
  inv1   g29(.a(x10), .O(new_n59_));
  nand3  g30(.a(x11), .b(new_n59_), .c(x00), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n58_), .O(z08));
  inv1   g33(.a(x11), .O(new_n63_));
  nand3  g34(.a(new_n63_), .b(new_n59_), .c(x00), .O(new_n64_));
  aoi22  g35(.a(new_n64_), .b(new_n30_), .c(new_n60_), .d(new_n41_), .O(z09));
  nand4  g36(.a(new_n52_), .b(new_n40_), .c(new_n59_), .d(x00), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(x07), .c(new_n30_), .O(z10));
  nand3  g38(.a(new_n52_), .b(new_n51_), .c(new_n41_), .O(new_n68_));
  aoi21  g39(.a(new_n68_), .b(new_n33_), .c(new_n60_), .O(z11));
  inv1   g40(.a(x09), .O(new_n70_));
  nand4  g41(.a(x11), .b(new_n59_), .c(new_n70_), .d(x00), .O(new_n71_));
  aoi21  g42(.a(new_n71_), .b(x07), .c(new_n30_), .O(z12));
endmodule


