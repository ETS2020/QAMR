// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x14), .O(z04));
  nand3  g05(.a(z04), .b(x12), .c(new_n31_), .O(z01));
  nand2  g06(.a(x11), .b(x09), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(z04), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z02));
  inv1   g10(.a(new_n37_), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n36_), .c(z04), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z03));
  nand2  g13(.a(x14), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x13), .O(z05));
  nand2  g15(.a(x09), .b(x01), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(x11), .c(z04), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n45_), .c(new_n47_), .O(z06));
  nand2  g21(.a(new_n43_), .b(x15), .O(z07));
  inv1   g22(.a(x00), .O(new_n52_));
  nor2   g23(.a(x10), .b(new_n52_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x11), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  nand4  g26(.a(new_n49_), .b(z04), .c(x12), .d(x09), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n55_), .O(z08));
  aoi21  g28(.a(z04), .b(x12), .c(new_n31_), .O(new_n58_));
  oai21  g29(.a(new_n32_), .b(x11), .c(new_n53_), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(new_n58_), .O(z09));
  nand4  g31(.a(new_n53_), .b(new_n48_), .c(x11), .d(x09), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(z04), .c(new_n32_), .O(z10));
  nand4  g33(.a(new_n48_), .b(x12), .c(x09), .d(x01), .O(new_n63_));
  nor2   g34(.a(x12), .b(x01), .O(new_n64_));
  aoi21  g35(.a(x14), .b(x12), .c(new_n64_), .O(new_n65_));
  aoi22  g36(.a(new_n65_), .b(new_n63_), .c(new_n54_), .d(new_n43_), .O(z11));
  inv1   g37(.a(x09), .O(new_n67_));
  nand3  g38(.a(new_n53_), .b(x11), .c(new_n67_), .O(new_n68_));
  aoi21  g39(.a(new_n68_), .b(z04), .c(new_n32_), .O(z12));
endmodule


