// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n58_;
  inv1   g00(.a(x00), .O(new_n30_));
  inv1   g01(.a(x01), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g05(.a(x12), .b(new_n32_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  and2   g08(.a(x11), .b(x09), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n37_), .c(x12), .O(z02));
  nand3  g10(.a(new_n38_), .b(new_n36_), .c(x12), .O(z03));
  aoi21  g11(.a(new_n33_), .b(x00), .c(x14), .O(z04));
  nand2  g12(.a(new_n33_), .b(x00), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x13), .O(z05));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand3  g16(.a(new_n42_), .b(x09), .c(x01), .O(new_n46_));
  aoi21  g17(.a(new_n45_), .b(z01), .c(new_n46_), .O(z06));
  nand2  g18(.a(new_n42_), .b(x15), .O(z07));
  nand2  g19(.a(new_n45_), .b(x09), .O(new_n49_));
  inv1   g20(.a(x10), .O(new_n50_));
  nand4  g21(.a(x12), .b(x11), .c(new_n50_), .d(x00), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n49_), .O(z08));
  nand2  g24(.a(x11), .b(new_n50_), .O(new_n54_));
  aoi21  g25(.a(new_n54_), .b(x12), .c(new_n30_), .O(z09));
  nand3  g26(.a(new_n44_), .b(new_n38_), .c(new_n50_), .O(new_n56_));
  aoi21  g27(.a(new_n56_), .b(x12), .c(new_n30_), .O(z10));
  nand4  g28(.a(new_n44_), .b(new_n38_), .c(new_n50_), .d(x01), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(x12), .c(new_n30_), .O(z11));
  nor2   g30(.a(new_n51_), .b(x09), .O(z12));
endmodule


