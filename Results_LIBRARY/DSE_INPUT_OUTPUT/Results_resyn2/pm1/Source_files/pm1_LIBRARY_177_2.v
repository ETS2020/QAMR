// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_,
    new_n70_;
  inv1   g00(.a(x14), .O(new_n30_));
  nand2  g01(.a(new_n30_), .b(x12), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  inv1   g03(.a(x01), .O(new_n33_));
  inv1   g04(.a(x12), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(x11), .O(new_n36_));
  nor2   g07(.a(new_n36_), .b(new_n32_), .O(z00));
  inv1   g08(.a(x11), .O(new_n38_));
  nand3  g09(.a(x14), .b(x12), .c(new_n38_), .O(z01));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand3  g12(.a(x14), .b(x12), .c(x09), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n41_), .c(x11), .O(z02));
  and2   g15(.a(x11), .b(x09), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(new_n40_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x14), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x12), .O(z03));
  nor2   g19(.a(x14), .b(x12), .O(z04));
  nor2   g20(.a(new_n32_), .b(x13), .O(z05));
  nand2  g21(.a(x12), .b(new_n38_), .O(new_n51_));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  and2   g24(.a(x09), .b(x01), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n31_), .O(new_n55_));
  aoi21  g26(.a(new_n53_), .b(new_n51_), .c(new_n55_), .O(z06));
  nor2   g27(.a(new_n32_), .b(x15), .O(z07));
  inv1   g28(.a(x10), .O(new_n58_));
  nand3  g29(.a(x11), .b(new_n58_), .c(x00), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n31_), .O(new_n60_));
  oai21  g31(.a(new_n52_), .b(new_n42_), .c(new_n60_), .O(z08));
  nand2  g32(.a(new_n34_), .b(x11), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n51_), .c(new_n58_), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n31_), .O(z09));
  nand4  g35(.a(new_n52_), .b(new_n45_), .c(new_n58_), .d(x00), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(x14), .c(new_n34_), .O(z10));
  nand4  g37(.a(new_n54_), .b(new_n52_), .c(x14), .d(x12), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n35_), .c(new_n59_), .O(z11));
  inv1   g39(.a(x09), .O(new_n69_));
  nand4  g40(.a(x11), .b(new_n58_), .c(new_n69_), .d(x00), .O(new_n70_));
  aoi21  g41(.a(new_n70_), .b(x14), .c(new_n34_), .O(z12));
endmodule


