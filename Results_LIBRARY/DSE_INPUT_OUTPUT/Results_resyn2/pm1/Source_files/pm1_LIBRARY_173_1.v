// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x14), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand4  g07(.a(new_n36_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g08(.a(x14), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n30_), .O(new_n39_));
  nand3  g10(.a(new_n35_), .b(x11), .c(x09), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n39_), .O(z03));
  nand2  g13(.a(x14), .b(x12), .O(z04));
  aoi21  g14(.a(x14), .b(new_n30_), .c(x13), .O(z05));
  nand2  g15(.a(x14), .b(new_n30_), .O(new_n45_));
  nand2  g16(.a(x09), .b(x01), .O(new_n46_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  aoi21  g18(.a(x12), .b(new_n33_), .c(new_n47_), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(z06));
  nand2  g20(.a(new_n45_), .b(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n45_), .O(new_n53_));
  inv1   g24(.a(new_n47_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x12), .c(x09), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n53_), .O(z08));
  nand2  g27(.a(new_n30_), .b(x11), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(z01), .c(new_n51_), .d(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n45_), .O(z09));
  inv1   g30(.a(x09), .O(new_n60_));
  nand2  g31(.a(new_n47_), .b(x12), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n52_), .c(new_n60_), .O(z10));
  nand4  g33(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n63_));
  inv1   g34(.a(x01), .O(new_n64_));
  nand3  g35(.a(new_n38_), .b(new_n30_), .c(new_n64_), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n63_), .c(new_n52_), .O(z11));
  nand2  g37(.a(new_n51_), .b(x00), .O(new_n67_));
  nand3  g38(.a(x12), .b(x11), .c(new_n60_), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n67_), .c(new_n45_), .O(z12));
endmodule


