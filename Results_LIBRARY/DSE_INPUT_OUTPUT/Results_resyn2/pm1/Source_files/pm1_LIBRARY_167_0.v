// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand4  g03(.a(new_n32_), .b(new_n31_), .c(x09), .d(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  nor2   g05(.a(x12), .b(x09), .O(new_n35_));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  nand3  g07(.a(x12), .b(x11), .c(x09), .O(new_n37_));
  nor2   g08(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(new_n35_), .O(z02));
  nand2  g10(.a(new_n32_), .b(x09), .O(new_n40_));
  nand3  g11(.a(new_n36_), .b(x11), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n40_), .O(z03));
  inv1   g14(.a(new_n35_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  nor2   g16(.a(new_n35_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  aoi21  g18(.a(x12), .b(new_n31_), .c(new_n47_), .O(new_n48_));
  nand2  g19(.a(x09), .b(x01), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n48_), .c(new_n44_), .O(z06));
  nor2   g21(.a(new_n35_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand2  g25(.a(new_n47_), .b(x12), .O(new_n55_));
  inv1   g26(.a(x09), .O(new_n56_));
  nand2  g27(.a(x12), .b(new_n56_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n55_), .c(new_n40_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n54_), .O(z08));
  nand2  g30(.a(new_n32_), .b(x11), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(z01), .c(new_n52_), .d(x00), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n44_), .O(z09));
  and2   g33(.a(x11), .b(x09), .O(new_n63_));
  nand3  g34(.a(new_n63_), .b(new_n52_), .c(x00), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n55_), .c(new_n44_), .O(z10));
  aoi21  g36(.a(new_n47_), .b(x12), .c(new_n30_), .O(new_n66_));
  nand2  g37(.a(x12), .b(new_n30_), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(new_n63_), .c(new_n52_), .d(x00), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n66_), .O(z11));
  nor2   g40(.a(new_n57_), .b(new_n53_), .O(z12));
endmodule


