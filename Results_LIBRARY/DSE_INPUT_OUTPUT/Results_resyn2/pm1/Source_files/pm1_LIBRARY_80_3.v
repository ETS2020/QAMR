// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n38_, new_n39_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x06), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n31_), .c(new_n34_), .O(z01));
  nor2   g06(.a(new_n32_), .b(new_n34_), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(z02));
  inv1   g08(.a(x09), .O(new_n38_));
  oai21  g09(.a(new_n31_), .b(new_n38_), .c(new_n34_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(z03));
  nor2   g11(.a(new_n36_), .b(x14), .O(z04));
  nor2   g12(.a(new_n36_), .b(x13), .O(z05));
  nand2  g13(.a(x09), .b(x01), .O(new_n43_));
  oai21  g14(.a(new_n43_), .b(x11), .c(new_n34_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x12), .O(new_n45_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n43_), .c(new_n45_), .O(z06));
  nor2   g19(.a(new_n36_), .b(x15), .O(z07));
  aoi21  g20(.a(new_n47_), .b(x09), .c(x06), .O(new_n50_));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  oai21  g24(.a(new_n50_), .b(new_n32_), .c(new_n53_), .O(z08));
  nand2  g25(.a(new_n51_), .b(x00), .O(new_n55_));
  nand3  g26(.a(x12), .b(x11), .c(new_n34_), .O(new_n56_));
  nand2  g27(.a(new_n32_), .b(new_n31_), .O(new_n57_));
  aoi21  g28(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z09));
  nand2  g29(.a(new_n46_), .b(x12), .O(new_n59_));
  nand3  g30(.a(x11), .b(x09), .c(new_n34_), .O(new_n60_));
  nor3   g31(.a(new_n60_), .b(new_n59_), .c(new_n55_), .O(z10));
  nand2  g32(.a(new_n32_), .b(new_n30_), .O(new_n62_));
  nand4  g33(.a(new_n46_), .b(x12), .c(x09), .d(x01), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n53_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(z02), .O(z11));
  nor3   g37(.a(new_n56_), .b(new_n55_), .c(x09), .O(z12));
endmodule


