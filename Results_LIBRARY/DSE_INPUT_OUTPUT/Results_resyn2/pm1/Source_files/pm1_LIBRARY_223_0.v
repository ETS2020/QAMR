// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n42_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x10), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  nor2   g05(.a(x12), .b(x10), .O(new_n35_));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  nand3  g07(.a(x12), .b(x11), .c(x09), .O(new_n37_));
  nor2   g08(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(new_n35_), .O(z02));
  nand4  g10(.a(new_n36_), .b(x12), .c(x11), .d(x09), .O(new_n40_));
  xor2a  g11(.a(new_n40_), .b(new_n35_), .O(z03));
  inv1   g12(.a(new_n35_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x14), .O(z04));
  nand2  g14(.a(new_n42_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  aoi21  g16(.a(x12), .b(new_n33_), .c(new_n45_), .O(new_n46_));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n46_), .c(new_n42_), .O(z06));
  nor2   g19(.a(new_n35_), .b(x15), .O(z07));
  inv1   g20(.a(new_n45_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x09), .O(new_n51_));
  inv1   g22(.a(x10), .O(new_n52_));
  nand4  g23(.a(x12), .b(x11), .c(new_n52_), .d(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(z09));
  nand2  g25(.a(z09), .b(new_n51_), .O(z08));
  inv1   g26(.a(x09), .O(new_n56_));
  nor3   g27(.a(new_n53_), .b(new_n50_), .c(new_n56_), .O(z10));
  inv1   g28(.a(new_n47_), .O(new_n58_));
  and2   g29(.a(x11), .b(x00), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n58_), .c(new_n45_), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(x12), .c(x10), .O(z11));
  nand2  g32(.a(new_n59_), .b(new_n56_), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(x12), .c(x10), .O(z12));
endmodule


