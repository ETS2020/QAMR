// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n49_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(x08), .O(new_n33_));
  inv1   g04(.a(new_n33_), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n30_), .O(z00));
  nand2  g06(.a(x12), .b(new_n31_), .O(z01));
  nor2   g07(.a(x12), .b(x08), .O(new_n37_));
  nand3  g08(.a(x12), .b(x08), .c(x07), .O(new_n38_));
  nand4  g09(.a(x11), .b(x09), .c(x06), .d(x05), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(new_n37_), .O(z02));
  nand3  g12(.a(x07), .b(x06), .c(x05), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x08), .O(new_n44_));
  inv1   g15(.a(x09), .O(new_n45_));
  oai21  g16(.a(new_n31_), .b(new_n45_), .c(x12), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n44_), .O(z03));
  nor2   g18(.a(new_n37_), .b(x14), .O(z04));
  inv1   g19(.a(new_n37_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x13), .O(z05));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand3  g23(.a(new_n49_), .b(x09), .c(x01), .O(new_n53_));
  aoi21  g24(.a(new_n52_), .b(z01), .c(new_n53_), .O(z06));
  nor2   g25(.a(new_n37_), .b(x15), .O(z07));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n56_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  nor2   g29(.a(new_n32_), .b(new_n45_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n52_), .O(new_n60_));
  oai21  g31(.a(new_n58_), .b(new_n37_), .c(new_n60_), .O(z08));
  nand2  g32(.a(new_n56_), .b(x00), .O(new_n62_));
  nand2  g33(.a(x12), .b(x11), .O(new_n63_));
  aoi21  g34(.a(new_n63_), .b(new_n33_), .c(new_n62_), .O(z09));
  nand2  g35(.a(new_n59_), .b(new_n51_), .O(new_n65_));
  nor2   g36(.a(new_n65_), .b(new_n57_), .O(z10));
  nand4  g37(.a(x11), .b(new_n56_), .c(new_n30_), .d(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x08), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n32_), .O(new_n69_));
  nand4  g40(.a(new_n59_), .b(new_n58_), .c(new_n51_), .d(x01), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n69_), .O(z11));
  nor3   g42(.a(new_n63_), .b(new_n62_), .c(x09), .O(z12));
endmodule


