// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n61_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand4  g04(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g05(.a(new_n35_), .O(new_n36_));
  and2   g06(.a(x12), .b(x09), .O(new_n37_));
  nand3  g07(.a(new_n37_), .b(new_n36_), .c(x11), .O(z02));
  inv1   g08(.a(x09), .O(new_n39_));
  oai21  g09(.a(new_n36_), .b(new_n39_), .c(x11), .O(new_n40_));
  nand2  g10(.a(new_n40_), .b(x12), .O(z03));
  nor2   g11(.a(new_n32_), .b(x11), .O(new_n42_));
  inv1   g12(.a(new_n42_), .O(new_n43_));
  nand2  g13(.a(new_n43_), .b(x14), .O(z04));
  nor2   g14(.a(new_n42_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  nand3  g16(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  nand2  g17(.a(new_n47_), .b(new_n43_), .O(z06));
  nand2  g18(.a(new_n43_), .b(x15), .O(z07));
  inv1   g19(.a(new_n46_), .O(new_n50_));
  inv1   g20(.a(x10), .O(new_n51_));
  nand3  g21(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  aoi21  g22(.a(new_n50_), .b(new_n37_), .c(new_n52_), .O(new_n53_));
  xor2a  g23(.a(new_n53_), .b(new_n43_), .O(z08));
  nor2   g24(.a(x12), .b(new_n31_), .O(new_n55_));
  aoi21  g25(.a(new_n51_), .b(x00), .c(new_n42_), .O(new_n56_));
  nor2   g26(.a(new_n56_), .b(new_n55_), .O(z09));
  nand2  g27(.a(new_n46_), .b(x12), .O(new_n58_));
  nor3   g28(.a(new_n58_), .b(new_n52_), .c(new_n39_), .O(z10));
  nand4  g29(.a(new_n46_), .b(x12), .c(x09), .d(x01), .O(new_n60_));
  aoi21  g30(.a(new_n55_), .b(new_n30_), .c(new_n42_), .O(new_n61_));
  aoi21  g31(.a(new_n61_), .b(new_n60_), .c(new_n56_), .O(z11));
  nor3   g32(.a(new_n52_), .b(new_n32_), .c(x09), .O(z12));
  one    g33(.O(z01));
endmodule


