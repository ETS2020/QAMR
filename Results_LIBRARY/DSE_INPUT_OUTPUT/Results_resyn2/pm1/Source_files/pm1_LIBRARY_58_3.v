// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n39_,
    new_n40_, new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_;
  inv1   g00(.a(x12), .O(new_n30_));
  nand2  g01(.a(new_n30_), .b(x01), .O(new_n31_));
  inv1   g02(.a(x06), .O(new_n32_));
  nand2  g03(.a(x12), .b(new_n32_), .O(new_n33_));
  nand2  g04(.a(new_n30_), .b(x11), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(new_n33_), .c(new_n31_), .O(z00));
  inv1   g06(.a(x11), .O(new_n36_));
  nand3  g07(.a(x12), .b(new_n36_), .c(new_n32_), .O(z01));
  nand2  g08(.a(x12), .b(x06), .O(z02));
  nand2  g09(.a(x11), .b(x09), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n32_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(z03));
  inv1   g12(.a(z02), .O(new_n42_));
  nor2   g13(.a(new_n42_), .b(x14), .O(z04));
  nand2  g14(.a(z02), .b(x13), .O(z05));
  nand2  g15(.a(x12), .b(new_n36_), .O(new_n45_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  nand3  g18(.a(z02), .b(x09), .c(x01), .O(new_n48_));
  aoi21  g19(.a(new_n47_), .b(new_n45_), .c(new_n48_), .O(z06));
  nor2   g20(.a(new_n42_), .b(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(z02), .O(new_n53_));
  nand4  g24(.a(new_n47_), .b(x12), .c(x09), .d(new_n32_), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n53_), .O(z08));
  inv1   g26(.a(x00), .O(new_n56_));
  nor2   g27(.a(x10), .b(new_n56_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n45_), .c(new_n34_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(z02), .O(z09));
  nand4  g30(.a(new_n57_), .b(new_n46_), .c(x11), .d(x09), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(new_n32_), .c(new_n30_), .O(z10));
  nand2  g32(.a(new_n33_), .b(new_n31_), .O(new_n62_));
  nand4  g33(.a(new_n46_), .b(x12), .c(x09), .d(x01), .O(new_n63_));
  aoi22  g34(.a(new_n63_), .b(new_n62_), .c(new_n52_), .d(z02), .O(z11));
  nor3   g35(.a(new_n52_), .b(new_n33_), .c(x09), .O(z12));
endmodule


