// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x14), .O(z04));
  nand2  g05(.a(z04), .b(x11), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x12), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(z04), .b(x12), .c(x11), .d(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n38_), .O(z02));
  nand2  g12(.a(new_n40_), .b(new_n37_), .O(z03));
  oai21  g13(.a(z04), .b(new_n32_), .c(x13), .O(z05));
  nand2  g14(.a(x09), .b(x01), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(x11), .c(z04), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x12), .O(new_n46_));
  and2   g17(.a(x09), .b(x01), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n46_), .O(z06));
  aoi21  g21(.a(x14), .b(x12), .c(x15), .O(z07));
  inv1   g22(.a(x09), .O(new_n52_));
  oai21  g23(.a(new_n48_), .b(new_n52_), .c(z04), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x12), .O(new_n54_));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x11), .b(new_n55_), .c(x00), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n54_), .O(z08));
  nand2  g29(.a(new_n32_), .b(x11), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n55_), .c(x00), .O(new_n60_));
  aoi21  g31(.a(new_n35_), .b(x12), .c(new_n60_), .O(z09));
  nand3  g32(.a(new_n48_), .b(z04), .c(x12), .O(new_n62_));
  nor3   g33(.a(new_n62_), .b(new_n56_), .c(new_n52_), .O(z10));
  nand2  g34(.a(new_n32_), .b(new_n30_), .O(new_n64_));
  nand4  g35(.a(new_n48_), .b(new_n47_), .c(z04), .d(x12), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n64_), .c(new_n56_), .O(z11));
  nand3  g37(.a(z04), .b(x12), .c(new_n52_), .O(new_n67_));
  nor2   g38(.a(new_n67_), .b(new_n56_), .O(z12));
endmodule


