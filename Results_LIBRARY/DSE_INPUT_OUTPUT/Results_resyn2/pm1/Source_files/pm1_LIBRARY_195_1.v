// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n67_, new_n68_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  nor2   g02(.a(x15), .b(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand4  g07(.a(new_n36_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g08(.a(new_n32_), .O(new_n38_));
  nand3  g09(.a(new_n35_), .b(x11), .c(x09), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n38_), .O(z03));
  inv1   g12(.a(x12), .O(new_n42_));
  nand2  g13(.a(x15), .b(new_n42_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  aoi21  g15(.a(x15), .b(new_n42_), .c(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  aoi21  g17(.a(x12), .b(new_n31_), .c(new_n46_), .O(new_n47_));
  nand3  g18(.a(new_n43_), .b(x09), .c(x01), .O(new_n48_));
  nor2   g19(.a(new_n48_), .b(new_n47_), .O(z06));
  nand2  g20(.a(x15), .b(x12), .O(z07));
  inv1   g21(.a(new_n46_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x12), .c(x09), .O(new_n52_));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(new_n52_), .c(new_n43_), .O(z08));
  nand2  g27(.a(x12), .b(x11), .O(new_n57_));
  nand2  g28(.a(new_n53_), .b(x00), .O(new_n58_));
  nand2  g29(.a(new_n32_), .b(new_n31_), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(new_n57_), .c(new_n58_), .O(z09));
  nand2  g31(.a(new_n46_), .b(x12), .O(new_n61_));
  nand4  g32(.a(x11), .b(new_n53_), .c(x09), .d(x00), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n61_), .c(new_n43_), .O(z10));
  nand2  g34(.a(new_n32_), .b(new_n30_), .O(new_n64_));
  nand4  g35(.a(new_n46_), .b(x12), .c(x09), .d(x01), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n64_), .c(new_n54_), .O(z11));
  inv1   g37(.a(x09), .O(new_n67_));
  nand3  g38(.a(x12), .b(x11), .c(new_n67_), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n58_), .c(new_n43_), .O(z12));
endmodule


