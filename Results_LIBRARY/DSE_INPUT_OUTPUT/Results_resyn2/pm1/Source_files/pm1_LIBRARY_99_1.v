// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x11), .O(new_n30_));
  nor2   g01(.a(new_n30_), .b(x04), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(x01), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(new_n31_), .O(z00));
  nand2  g06(.a(x11), .b(x04), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n33_), .O(z01));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  nand2  g09(.a(x12), .b(x09), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(x04), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x11), .O(z02));
  nand3  g12(.a(x12), .b(x11), .c(x04), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n38_), .c(x09), .O(z03));
  nor2   g15(.a(new_n31_), .b(x14), .O(z04));
  inv1   g16(.a(new_n31_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x13), .O(z05));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  aoi21  g20(.a(x12), .b(new_n30_), .c(new_n49_), .O(new_n50_));
  aoi21  g21(.a(new_n48_), .b(new_n46_), .c(new_n50_), .O(z06));
  nand2  g22(.a(new_n46_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  and2   g24(.a(x11), .b(x04), .O(new_n54_));
  and2   g25(.a(x12), .b(x09), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x03), .c(x02), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(new_n54_), .c(new_n53_), .d(x00), .O(z08));
  nand2  g28(.a(new_n53_), .b(x00), .O(new_n58_));
  aoi21  g29(.a(new_n42_), .b(new_n33_), .c(new_n58_), .O(z09));
  nand2  g30(.a(x03), .b(x02), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n55_), .c(new_n53_), .d(x00), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(x04), .c(new_n30_), .O(z10));
  inv1   g33(.a(x01), .O(new_n63_));
  aoi21  g34(.a(new_n60_), .b(new_n55_), .c(new_n63_), .O(new_n64_));
  nand2  g35(.a(x12), .b(new_n63_), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(new_n54_), .c(new_n53_), .d(x00), .O(new_n66_));
  nor2   g37(.a(new_n66_), .b(new_n64_), .O(z11));
  nor3   g38(.a(new_n58_), .b(new_n42_), .c(x09), .O(z12));
endmodule


