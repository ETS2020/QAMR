// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x03), .O(new_n34_));
  oai21  g05(.a(new_n31_), .b(new_n34_), .c(x12), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand3  g08(.a(x12), .b(x09), .c(x03), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x11), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n37_), .O(z02));
  nand2  g13(.a(new_n41_), .b(new_n36_), .O(z03));
  aoi21  g14(.a(x12), .b(new_n34_), .c(x14), .O(z04));
  nand2  g15(.a(x12), .b(new_n34_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x13), .O(z05));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  aoi21  g19(.a(x12), .b(new_n31_), .c(new_n48_), .O(new_n49_));
  aoi21  g20(.a(new_n47_), .b(new_n45_), .c(new_n49_), .O(z06));
  nand2  g21(.a(new_n45_), .b(x15), .O(z07));
  nand2  g22(.a(x04), .b(x02), .O(new_n52_));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n45_), .O(new_n55_));
  oai21  g26(.a(new_n52_), .b(new_n38_), .c(new_n55_), .O(z08));
  inv1   g27(.a(x00), .O(new_n57_));
  nor2   g28(.a(x10), .b(new_n57_), .O(new_n58_));
  nand2  g29(.a(new_n32_), .b(x11), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n58_), .c(z01), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(z09));
  nand2  g32(.a(x09), .b(x03), .O(new_n62_));
  nand2  g33(.a(new_n52_), .b(x12), .O(new_n63_));
  nor3   g34(.a(new_n63_), .b(new_n54_), .c(new_n62_), .O(z10));
  aoi21  g35(.a(new_n52_), .b(x12), .c(new_n30_), .O(new_n65_));
  nand2  g36(.a(new_n47_), .b(x12), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(new_n58_), .c(x11), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n65_), .c(new_n45_), .O(z11));
  inv1   g39(.a(x09), .O(new_n69_));
  nand3  g40(.a(new_n58_), .b(x11), .c(new_n69_), .O(new_n70_));
  aoi21  g41(.a(new_n70_), .b(x03), .c(new_n32_), .O(z12));
endmodule


