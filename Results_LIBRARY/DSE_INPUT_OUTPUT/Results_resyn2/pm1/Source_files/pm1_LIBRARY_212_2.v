// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x05), .O(new_n31_));
  nor2   g02(.a(x12), .b(x11), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x11), .O(new_n34_));
  nor2   g05(.a(x12), .b(new_n31_), .O(new_n35_));
  aoi21  g06(.a(x12), .b(new_n34_), .c(new_n35_), .O(z01));
  nand2  g07(.a(x11), .b(x09), .O(new_n37_));
  nand3  g08(.a(x08), .b(x07), .c(x06), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x12), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x05), .O(z02));
  nand3  g11(.a(x12), .b(x11), .c(x09), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  nand2  g13(.a(x12), .b(x05), .O(new_n43_));
  oai22  g14(.a(new_n43_), .b(new_n38_), .c(new_n42_), .d(new_n35_), .O(z03));
  inv1   g15(.a(new_n35_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x14), .O(z04));
  nor2   g17(.a(new_n35_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  aoi21  g19(.a(x12), .b(new_n34_), .c(new_n48_), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  oai21  g21(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(z06));
  nor2   g22(.a(new_n35_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  inv1   g26(.a(new_n48_), .O(new_n56_));
  inv1   g27(.a(x09), .O(new_n57_));
  inv1   g28(.a(x12), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  oai21  g31(.a(new_n55_), .b(new_n35_), .c(new_n60_), .O(z08));
  inv1   g32(.a(x00), .O(new_n62_));
  nor2   g33(.a(x10), .b(new_n62_), .O(new_n63_));
  nor2   g34(.a(new_n58_), .b(new_n34_), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n32_), .c(new_n63_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n45_), .O(z09));
  nand2  g37(.a(new_n59_), .b(new_n48_), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n54_), .c(new_n45_), .O(z10));
  nand4  g39(.a(x11), .b(new_n53_), .c(new_n30_), .d(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n31_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  nand4  g42(.a(new_n59_), .b(new_n55_), .c(new_n48_), .d(x01), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n71_), .O(z11));
  nand3  g44(.a(new_n64_), .b(new_n63_), .c(new_n57_), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n45_), .O(z12));
endmodule


