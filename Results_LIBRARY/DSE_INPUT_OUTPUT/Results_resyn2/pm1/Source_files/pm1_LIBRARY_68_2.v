// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x02), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  inv1   g05(.a(x02), .O(new_n35_));
  nor2   g06(.a(x12), .b(new_n35_), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand3  g08(.a(x12), .b(x11), .c(x09), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n36_), .O(z02));
  nor2   g11(.a(x12), .b(x02), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  nand3  g13(.a(new_n37_), .b(x11), .c(x09), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n42_), .O(z03));
  inv1   g16(.a(new_n36_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x14), .O(z04));
  nand2  g18(.a(new_n46_), .b(x13), .O(z05));
  nand2  g19(.a(x04), .b(x03), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n33_), .c(x12), .O(new_n50_));
  and2   g21(.a(x09), .b(x01), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  aoi21  g23(.a(new_n50_), .b(x02), .c(new_n52_), .O(z06));
  nor2   g24(.a(new_n36_), .b(x15), .O(z07));
  inv1   g25(.a(x09), .O(new_n55_));
  oai21  g26(.a(new_n49_), .b(new_n55_), .c(x12), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x02), .O(new_n57_));
  inv1   g28(.a(x10), .O(new_n58_));
  nand3  g29(.a(x11), .b(new_n58_), .c(x00), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n57_), .O(z08));
  nand2  g32(.a(new_n30_), .b(x11), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(z01), .c(new_n58_), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n46_), .O(z09));
  nand4  g35(.a(x12), .b(x11), .c(new_n58_), .d(x00), .O(new_n65_));
  nand3  g36(.a(x04), .b(x03), .c(x02), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x09), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n65_), .c(new_n46_), .O(z10));
  nand2  g39(.a(new_n30_), .b(x01), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x11), .c(new_n58_), .d(x00), .O(new_n70_));
  oai21  g41(.a(new_n51_), .b(new_n41_), .c(new_n66_), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n70_), .O(z11));
  oai21  g43(.a(new_n65_), .b(x09), .c(new_n46_), .O(z12));
endmodule


