// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_;
  nand2  g00(.a(x11), .b(x10), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x01), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n31_), .O(z00));
  oai21  g07(.a(new_n32_), .b(x10), .c(new_n34_), .O(z01));
  inv1   g08(.a(x10), .O(new_n38_));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  inv1   g11(.a(x09), .O(new_n41_));
  nor2   g12(.a(new_n33_), .b(new_n41_), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(new_n40_), .c(x11), .d(new_n38_), .O(z02));
  nand3  g14(.a(new_n39_), .b(x12), .c(x09), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x11), .O(z03));
  nor2   g17(.a(new_n31_), .b(x14), .O(z04));
  nor2   g18(.a(new_n31_), .b(x13), .O(z05));
  nand2  g19(.a(x12), .b(new_n32_), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  nand3  g22(.a(new_n30_), .b(x09), .c(x01), .O(new_n52_));
  aoi21  g23(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(z06));
  nor2   g24(.a(new_n31_), .b(x15), .O(z07));
  nand2  g25(.a(new_n51_), .b(new_n42_), .O(new_n55_));
  nand3  g26(.a(x11), .b(new_n38_), .c(x00), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n55_), .O(z08));
  nand2  g29(.a(new_n38_), .b(x00), .O(new_n59_));
  nand2  g30(.a(x12), .b(x11), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(new_n34_), .c(new_n59_), .O(z09));
  nand2  g32(.a(new_n50_), .b(x12), .O(new_n62_));
  nor3   g33(.a(new_n62_), .b(new_n56_), .c(new_n41_), .O(z10));
  inv1   g34(.a(x01), .O(new_n64_));
  aoi21  g35(.a(new_n50_), .b(x12), .c(new_n64_), .O(new_n65_));
  nand2  g36(.a(x09), .b(x01), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x12), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(x11), .c(new_n38_), .d(x00), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n65_), .O(z11));
  nor3   g40(.a(new_n60_), .b(new_n59_), .c(x09), .O(z12));
endmodule


