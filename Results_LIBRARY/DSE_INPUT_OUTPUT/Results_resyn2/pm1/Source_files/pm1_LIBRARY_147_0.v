// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  nand4  g03(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n33_));
  inv1   g04(.a(new_n33_), .O(new_n34_));
  nand2  g05(.a(x12), .b(x11), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(new_n34_), .c(x09), .O(z02));
  nand2  g08(.a(new_n35_), .b(z00), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  oai21  g10(.a(new_n34_), .b(new_n39_), .c(x12), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n38_), .O(z03));
  nand2  g12(.a(z00), .b(x14), .O(z04));
  nand2  g13(.a(z00), .b(x13), .O(z05));
  nand2  g14(.a(x09), .b(x01), .O(new_n44_));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  aoi22  g17(.a(new_n46_), .b(x11), .c(new_n44_), .d(z00), .O(z06));
  aoi21  g18(.a(new_n31_), .b(new_n30_), .c(x15), .O(z07));
  inv1   g19(.a(x10), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x00), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x11), .O(new_n51_));
  nand4  g22(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x11), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x12), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n51_), .O(z08));
  and2   g26(.a(new_n51_), .b(new_n38_), .O(z09));
  nor2   g27(.a(x10), .b(new_n39_), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(new_n45_), .c(new_n36_), .d(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(z00), .O(z10));
  inv1   g30(.a(x01), .O(new_n60_));
  nand3  g31(.a(new_n49_), .b(new_n60_), .c(x00), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x11), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n31_), .O(new_n63_));
  and2   g34(.a(x01), .b(x00), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n57_), .c(new_n45_), .d(new_n36_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n63_), .O(z11));
  nand2  g37(.a(new_n36_), .b(new_n39_), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n50_), .c(z00), .O(z12));
  buf    g39(.a(x11), .O(z01));
endmodule


