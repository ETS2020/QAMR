// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x10), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g05(.a(new_n33_), .b(x10), .O(new_n35_));
  nand2  g06(.a(x12), .b(new_n32_), .O(new_n36_));
  and2   g07(.a(new_n36_), .b(new_n35_), .O(z01));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand2  g11(.a(new_n33_), .b(new_n31_), .O(new_n41_));
  nand3  g12(.a(new_n38_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n41_), .O(z03));
  inv1   g15(.a(new_n35_), .O(new_n45_));
  nor2   g16(.a(new_n45_), .b(x14), .O(z04));
  nand2  g17(.a(new_n35_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  nand3  g20(.a(new_n35_), .b(x09), .c(x01), .O(new_n50_));
  aoi21  g21(.a(new_n49_), .b(new_n36_), .c(new_n50_), .O(z06));
  nor2   g22(.a(new_n45_), .b(x15), .O(z07));
  nand3  g23(.a(x11), .b(new_n31_), .c(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n49_), .b(x12), .c(x09), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n54_), .O(z08));
  aoi21  g27(.a(x12), .b(new_n31_), .c(new_n32_), .O(new_n57_));
  nand2  g28(.a(new_n36_), .b(x00), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(new_n57_), .c(new_n35_), .O(z09));
  inv1   g30(.a(x09), .O(new_n60_));
  nand2  g31(.a(new_n48_), .b(x12), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n53_), .c(new_n60_), .O(z10));
  aoi21  g33(.a(new_n48_), .b(x12), .c(new_n30_), .O(new_n63_));
  nand2  g34(.a(x09), .b(x01), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(x12), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x11), .c(new_n31_), .d(x00), .O(new_n66_));
  nor2   g37(.a(new_n66_), .b(new_n63_), .O(z11));
  nand2  g38(.a(new_n31_), .b(x00), .O(new_n68_));
  nand3  g39(.a(x12), .b(x11), .c(new_n60_), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n68_), .O(z12));
endmodule


