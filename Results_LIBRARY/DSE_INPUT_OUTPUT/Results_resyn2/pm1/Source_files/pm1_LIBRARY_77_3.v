// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  and2   g07(.a(x12), .b(x09), .O(new_n37_));
  nor2   g08(.a(x15), .b(new_n31_), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(z02));
  nand3  g10(.a(new_n38_), .b(new_n37_), .c(new_n35_), .O(z03));
  aoi21  g11(.a(x15), .b(x11), .c(x14), .O(z04));
  nand2  g12(.a(x15), .b(x11), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x13), .O(z05));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand3  g16(.a(new_n42_), .b(x09), .c(x01), .O(new_n46_));
  aoi21  g17(.a(new_n45_), .b(z01), .c(new_n46_), .O(z06));
  nand2  g18(.a(x15), .b(new_n31_), .O(z07));
  nand2  g19(.a(new_n45_), .b(new_n37_), .O(new_n49_));
  inv1   g20(.a(x00), .O(new_n50_));
  nor2   g21(.a(x10), .b(new_n50_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(new_n49_), .c(new_n38_), .O(z08));
  inv1   g23(.a(x15), .O(new_n53_));
  aoi21  g24(.a(new_n53_), .b(x12), .c(new_n31_), .O(new_n54_));
  nand2  g25(.a(new_n51_), .b(z01), .O(new_n55_));
  nor2   g26(.a(new_n55_), .b(new_n54_), .O(z09));
  inv1   g27(.a(x10), .O(new_n57_));
  nand4  g28(.a(new_n44_), .b(new_n37_), .c(new_n57_), .d(x00), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(new_n53_), .c(new_n31_), .O(z10));
  aoi21  g30(.a(new_n44_), .b(new_n37_), .c(new_n30_), .O(new_n60_));
  nand2  g31(.a(x12), .b(new_n30_), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n51_), .c(new_n38_), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n60_), .O(z11));
  nor2   g34(.a(new_n32_), .b(x09), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n51_), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n53_), .c(new_n31_), .O(z12));
endmodule


