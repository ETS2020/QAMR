// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(x15), .b(x13), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand3  g05(.a(new_n33_), .b(x12), .c(new_n31_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand3  g08(.a(x12), .b(x11), .c(x09), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n37_), .c(new_n33_), .O(z02));
  inv1   g11(.a(new_n33_), .O(new_n41_));
  aoi21  g12(.a(new_n39_), .b(new_n36_), .c(new_n41_), .O(z03));
  nand2  g13(.a(new_n33_), .b(x14), .O(z04));
  inv1   g14(.a(x15), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x13), .O(z05));
  nand2  g16(.a(x12), .b(new_n31_), .O(new_n46_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  inv1   g18(.a(new_n47_), .O(new_n48_));
  nand3  g19(.a(new_n33_), .b(x09), .c(x01), .O(new_n49_));
  aoi21  g20(.a(new_n48_), .b(new_n46_), .c(new_n49_), .O(z06));
  inv1   g21(.a(x13), .O(new_n51_));
  nand2  g22(.a(x15), .b(new_n51_), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  nand3  g26(.a(new_n48_), .b(x12), .c(x09), .O(new_n56_));
  aoi21  g27(.a(new_n56_), .b(new_n55_), .c(new_n41_), .O(z08));
  nand2  g28(.a(new_n32_), .b(new_n31_), .O(new_n58_));
  nand2  g29(.a(x12), .b(x11), .O(new_n59_));
  inv1   g30(.a(x00), .O(new_n60_));
  nor2   g31(.a(x10), .b(new_n60_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n33_), .O(new_n62_));
  aoi21  g33(.a(new_n59_), .b(new_n58_), .c(new_n62_), .O(z09));
  nand3  g34(.a(new_n47_), .b(x12), .c(x09), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n54_), .c(new_n33_), .O(z10));
  aoi21  g36(.a(new_n47_), .b(x12), .c(new_n30_), .O(new_n66_));
  nand2  g37(.a(x09), .b(x01), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x12), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(new_n61_), .c(new_n33_), .d(x11), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n66_), .O(z11));
  inv1   g41(.a(new_n61_), .O(new_n71_));
  or2    g42(.a(new_n59_), .b(x09), .O(new_n72_));
  oai21  g43(.a(new_n72_), .b(new_n71_), .c(new_n33_), .O(z12));
endmodule


