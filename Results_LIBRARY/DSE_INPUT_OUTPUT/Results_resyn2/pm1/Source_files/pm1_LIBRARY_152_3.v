// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n38_, new_n39_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_, new_n62_,
    new_n63_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x07), .O(new_n34_));
  aoi21  g05(.a(x11), .b(new_n34_), .c(new_n32_), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(z01));
  nand2  g07(.a(x12), .b(x07), .O(z02));
  nand3  g08(.a(x12), .b(x09), .c(new_n34_), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x11), .O(z03));
  nand2  g11(.a(z02), .b(x14), .O(z04));
  nand2  g12(.a(z02), .b(x13), .O(z05));
  nand3  g13(.a(x04), .b(x03), .c(x02), .O(new_n43_));
  aoi21  g14(.a(x12), .b(new_n31_), .c(new_n43_), .O(new_n44_));
  and2   g15(.a(x09), .b(x01), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(z02), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(new_n44_), .O(z06));
  nand2  g18(.a(z02), .b(x15), .O(z07));
  inv1   g19(.a(x10), .O(new_n49_));
  nand3  g20(.a(x11), .b(new_n49_), .c(x00), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(z02), .O(new_n51_));
  oai21  g22(.a(new_n43_), .b(new_n38_), .c(new_n51_), .O(z08));
  nand2  g23(.a(new_n49_), .b(x00), .O(new_n53_));
  nor2   g24(.a(x12), .b(new_n31_), .O(new_n54_));
  nor3   g25(.a(new_n54_), .b(new_n53_), .c(new_n35_), .O(z09));
  and2   g26(.a(x11), .b(x09), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(new_n43_), .c(new_n49_), .d(x00), .O(new_n57_));
  aoi21  g28(.a(new_n57_), .b(new_n34_), .c(new_n32_), .O(z10));
  nand4  g29(.a(new_n45_), .b(new_n43_), .c(x12), .d(new_n34_), .O(new_n59_));
  nand2  g30(.a(new_n32_), .b(new_n30_), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(new_n59_), .c(new_n50_), .O(z11));
  inv1   g32(.a(x09), .O(new_n62_));
  nand4  g33(.a(x11), .b(new_n49_), .c(new_n62_), .d(x00), .O(new_n63_));
  aoi21  g34(.a(new_n63_), .b(new_n34_), .c(new_n32_), .O(z12));
endmodule


