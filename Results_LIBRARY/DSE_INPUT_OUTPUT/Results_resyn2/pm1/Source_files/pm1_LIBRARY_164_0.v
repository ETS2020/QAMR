// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n34_, new_n35_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_;
  nor2   g00(.a(x11), .b(x01), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(z00));
  inv1   g02(.a(x11), .O(new_n32_));
  nand3  g03(.a(x12), .b(new_n32_), .c(x01), .O(z01));
  nand4  g04(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand4  g06(.a(new_n35_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g07(.a(new_n34_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g08(.a(new_n30_), .b(x14), .O(z04));
  nand2  g09(.a(z00), .b(x13), .O(z05));
  inv1   g10(.a(x01), .O(new_n40_));
  aoi21  g11(.a(x12), .b(x09), .c(new_n40_), .O(new_n41_));
  nand3  g12(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  nand3  g13(.a(new_n42_), .b(x09), .c(x01), .O(new_n43_));
  oai21  g14(.a(new_n41_), .b(x11), .c(new_n43_), .O(z06));
  nor2   g15(.a(new_n30_), .b(x15), .O(z07));
  inv1   g16(.a(new_n42_), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x12), .c(x09), .O(new_n47_));
  inv1   g18(.a(x10), .O(new_n48_));
  nand3  g19(.a(x11), .b(new_n48_), .c(x00), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n47_), .O(z08));
  nand2  g22(.a(x12), .b(new_n32_), .O(new_n52_));
  inv1   g23(.a(x12), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x11), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(new_n52_), .c(new_n48_), .d(x00), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(z00), .O(z09));
  nand2  g27(.a(new_n42_), .b(x12), .O(new_n57_));
  nand4  g28(.a(x11), .b(new_n48_), .c(x09), .d(x00), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(new_n57_), .c(z00), .O(z10));
  nand2  g30(.a(new_n53_), .b(new_n40_), .O(new_n60_));
  nand4  g31(.a(new_n42_), .b(x12), .c(x09), .d(x01), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n60_), .c(new_n49_), .O(z11));
  nand2  g33(.a(new_n48_), .b(x00), .O(new_n63_));
  inv1   g34(.a(x09), .O(new_n64_));
  nand3  g35(.a(x12), .b(x11), .c(new_n64_), .O(new_n65_));
  oai21  g36(.a(new_n65_), .b(new_n63_), .c(z00), .O(z12));
endmodule


