// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_;
  inv1   g00(.a(x01), .O(new_n30_));
  nor2   g01(.a(x15), .b(x07), .O(new_n31_));
  nor2   g02(.a(x12), .b(x11), .O(new_n32_));
  aoi21  g03(.a(new_n32_), .b(new_n30_), .c(new_n31_), .O(z00));
  inv1   g04(.a(x11), .O(new_n34_));
  inv1   g05(.a(new_n31_), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(x12), .c(new_n34_), .O(z01));
  nand3  g07(.a(x11), .b(x08), .c(x07), .O(new_n37_));
  nand4  g08(.a(x12), .b(x09), .c(x06), .d(x05), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n31_), .O(z02));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  and2   g12(.a(x12), .b(x11), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(new_n41_), .c(new_n35_), .d(x09), .O(z03));
  nor2   g14(.a(new_n31_), .b(x14), .O(z04));
  nor2   g15(.a(new_n31_), .b(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  aoi21  g17(.a(x12), .b(new_n34_), .c(new_n46_), .O(new_n47_));
  nand3  g18(.a(new_n35_), .b(x09), .c(x01), .O(new_n48_));
  nor2   g19(.a(new_n48_), .b(new_n47_), .O(z06));
  inv1   g20(.a(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  inv1   g24(.a(new_n46_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x12), .c(x09), .O(new_n55_));
  aoi21  g26(.a(new_n55_), .b(new_n53_), .c(new_n31_), .O(z08));
  inv1   g27(.a(x00), .O(new_n57_));
  nor2   g28(.a(x10), .b(new_n57_), .O(new_n58_));
  oai21  g29(.a(new_n42_), .b(new_n32_), .c(new_n58_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n35_), .O(z09));
  nand2  g31(.a(new_n46_), .b(x12), .O(new_n61_));
  nand3  g32(.a(new_n58_), .b(x11), .c(x09), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n61_), .c(new_n35_), .O(z10));
  aoi21  g34(.a(new_n46_), .b(x12), .c(new_n30_), .O(new_n64_));
  nand2  g35(.a(x09), .b(x01), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(x12), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(new_n58_), .c(x11), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n64_), .c(new_n35_), .O(z11));
  inv1   g39(.a(x09), .O(new_n69_));
  nand3  g40(.a(new_n58_), .b(new_n42_), .c(new_n69_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n35_), .O(z12));
endmodule


