// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_;
  inv1   g00(.a(x06), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  nor2   g02(.a(x12), .b(x01), .O(new_n32_));
  aoi22  g03(.a(new_n32_), .b(new_n31_), .c(x09), .d(new_n30_), .O(z00));
  nand2  g04(.a(x09), .b(new_n30_), .O(new_n34_));
  nand2  g05(.a(x12), .b(new_n31_), .O(new_n35_));
  and2   g06(.a(new_n35_), .b(new_n34_), .O(z01));
  nand3  g07(.a(x08), .b(x07), .c(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(x12), .b(x09), .c(x06), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x11), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n38_), .O(z02));
  nand2  g14(.a(new_n42_), .b(new_n37_), .O(z03));
  nand2  g15(.a(new_n34_), .b(x14), .O(z04));
  nand2  g16(.a(new_n34_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  inv1   g18(.a(new_n47_), .O(new_n48_));
  nand3  g19(.a(x09), .b(x06), .c(x01), .O(new_n49_));
  aoi21  g20(.a(new_n48_), .b(new_n35_), .c(new_n49_), .O(z06));
  nand2  g21(.a(new_n34_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n34_), .O(new_n54_));
  oai21  g25(.a(new_n47_), .b(new_n39_), .c(new_n54_), .O(z08));
  inv1   g26(.a(x12), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x11), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(new_n35_), .c(new_n52_), .d(x00), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n34_), .O(z09));
  nand2  g30(.a(x09), .b(x06), .O(new_n60_));
  nand2  g31(.a(new_n47_), .b(x12), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n53_), .c(new_n60_), .O(z10));
  inv1   g33(.a(x01), .O(new_n63_));
  aoi22  g34(.a(new_n56_), .b(new_n63_), .c(x09), .d(new_n30_), .O(new_n64_));
  nand4  g35(.a(new_n47_), .b(x12), .c(x09), .d(x01), .O(new_n65_));
  aoi22  g36(.a(new_n65_), .b(new_n64_), .c(new_n53_), .d(new_n34_), .O(z11));
  inv1   g37(.a(x09), .O(new_n67_));
  nand2  g38(.a(x12), .b(new_n67_), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n53_), .c(new_n34_), .O(z12));
endmodule


