// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(x01), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n30_), .O(z00));
  nor2   g04(.a(new_n31_), .b(x11), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  inv1   g08(.a(x09), .O(new_n38_));
  nor2   g09(.a(new_n31_), .b(new_n38_), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n37_), .c(x11), .O(z02));
  oai21  g11(.a(new_n37_), .b(new_n38_), .c(x11), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z03));
  nand2  g13(.a(z01), .b(x14), .O(z04));
  nand2  g14(.a(z01), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(x09), .c(x01), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(new_n34_), .O(z06));
  nor2   g18(.a(new_n34_), .b(x15), .O(z07));
  inv1   g19(.a(new_n45_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n39_), .O(new_n50_));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  aoi21  g24(.a(new_n53_), .b(new_n50_), .c(new_n34_), .O(z08));
  nand2  g25(.a(new_n51_), .b(x00), .O(new_n55_));
  nor2   g26(.a(x12), .b(new_n30_), .O(new_n56_));
  aoi21  g27(.a(new_n55_), .b(z01), .c(new_n56_), .O(z09));
  nand2  g28(.a(new_n45_), .b(new_n39_), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n52_), .O(z10));
  nand4  g30(.a(new_n32_), .b(x11), .c(new_n51_), .d(x00), .O(new_n60_));
  aoi21  g31(.a(new_n46_), .b(x12), .c(new_n60_), .O(z11));
  nand3  g32(.a(new_n51_), .b(new_n38_), .c(x00), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(x11), .c(new_n31_), .O(z12));
endmodule


