// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_;
  inv1   g00(.a(x03), .O(new_n30_));
  nand2  g01(.a(x12), .b(new_n30_), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  nor3   g03(.a(x12), .b(x11), .c(x01), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(new_n32_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n30_), .c(x12), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand2  g08(.a(x11), .b(x09), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x03), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(z02));
  inv1   g11(.a(new_n37_), .O(new_n41_));
  oai21  g12(.a(new_n38_), .b(new_n41_), .c(x03), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  nor2   g14(.a(new_n32_), .b(x14), .O(z04));
  nand2  g15(.a(new_n31_), .b(x13), .O(z05));
  nand2  g16(.a(x09), .b(x01), .O(new_n46_));
  nand2  g17(.a(x12), .b(new_n35_), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  aoi22  g20(.a(new_n49_), .b(new_n47_), .c(new_n46_), .d(new_n31_), .O(z06));
  nand2  g21(.a(new_n31_), .b(x15), .O(z07));
  inv1   g22(.a(x09), .O(new_n52_));
  nand2  g23(.a(x04), .b(x02), .O(new_n53_));
  oai21  g24(.a(new_n53_), .b(new_n52_), .c(x03), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x12), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n56_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n55_), .O(z08));
  nor2   g30(.a(x12), .b(new_n35_), .O(new_n60_));
  inv1   g31(.a(x00), .O(new_n61_));
  nor2   g32(.a(x10), .b(new_n61_), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n47_), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n60_), .c(new_n31_), .O(z09));
  nand2  g35(.a(new_n56_), .b(x00), .O(new_n65_));
  nand2  g36(.a(new_n53_), .b(x12), .O(new_n66_));
  nand3  g37(.a(x11), .b(x09), .c(x03), .O(new_n67_));
  nor3   g38(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(z10));
  inv1   g39(.a(x01), .O(new_n69_));
  aoi21  g40(.a(new_n53_), .b(x12), .c(new_n69_), .O(new_n70_));
  nand2  g41(.a(new_n46_), .b(x12), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(new_n62_), .c(x11), .O(new_n72_));
  oai21  g43(.a(new_n72_), .b(new_n70_), .c(new_n31_), .O(z11));
  nand3  g44(.a(x12), .b(new_n52_), .c(x03), .O(new_n74_));
  nor2   g45(.a(new_n74_), .b(new_n57_), .O(z12));
endmodule


