// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x03), .O(new_n34_));
  oai21  g05(.a(new_n31_), .b(new_n34_), .c(x12), .O(z01));
  nand3  g06(.a(x12), .b(x11), .c(x03), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n37_), .c(x09), .O(z02));
  nand3  g11(.a(new_n38_), .b(x11), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x03), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  nand2  g14(.a(x12), .b(new_n34_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  aoi21  g16(.a(x12), .b(new_n34_), .c(x13), .O(z05));
  nand2  g17(.a(x12), .b(new_n31_), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  and2   g20(.a(x09), .b(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n44_), .O(new_n51_));
  aoi21  g22(.a(new_n49_), .b(new_n47_), .c(new_n51_), .O(z06));
  nand2  g23(.a(new_n44_), .b(x15), .O(z07));
  inv1   g24(.a(x09), .O(new_n54_));
  nand2  g25(.a(x04), .b(x02), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n54_), .c(x03), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x12), .O(new_n57_));
  inv1   g28(.a(x00), .O(new_n58_));
  nor2   g29(.a(x10), .b(new_n58_), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n57_), .c(x11), .O(z08));
  nand2  g31(.a(new_n32_), .b(x11), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n59_), .c(new_n47_), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n44_), .O(z09));
  nor2   g34(.a(new_n31_), .b(new_n54_), .O(new_n64_));
  nand3  g35(.a(new_n59_), .b(new_n55_), .c(new_n64_), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(x03), .c(new_n32_), .O(z10));
  aoi21  g37(.a(new_n50_), .b(new_n55_), .c(new_n32_), .O(new_n67_));
  nand2  g38(.a(new_n32_), .b(x01), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n59_), .c(x11), .O(new_n69_));
  oai21  g40(.a(new_n69_), .b(new_n67_), .c(new_n44_), .O(z11));
  nand2  g41(.a(new_n59_), .b(new_n54_), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n36_), .O(z12));
endmodule


