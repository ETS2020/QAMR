// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x02), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nor2   g02(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nor3   g03(.a(x12), .b(x11), .c(x01), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(new_n32_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  nor2   g06(.a(new_n31_), .b(x02), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n35_), .O(z01));
  nand2  g08(.a(x11), .b(x09), .O(new_n38_));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(new_n30_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(z02));
  nand3  g12(.a(new_n39_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n30_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z03));
  inv1   g15(.a(new_n32_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x14), .O(z04));
  nand2  g17(.a(new_n45_), .b(x13), .O(z05));
  nand2  g18(.a(x04), .b(x03), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n31_), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  aoi21  g21(.a(new_n49_), .b(x02), .c(new_n50_), .O(z06));
  nor2   g22(.a(new_n32_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n45_), .O(z08));
  nand3  g27(.a(new_n35_), .b(new_n53_), .c(x00), .O(new_n57_));
  aoi22  g28(.a(new_n57_), .b(new_n31_), .c(new_n54_), .d(new_n36_), .O(z09));
  nand4  g29(.a(x11), .b(new_n53_), .c(x09), .d(x00), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(new_n30_), .c(new_n31_), .O(z10));
  inv1   g31(.a(x01), .O(new_n61_));
  aoi21  g32(.a(x12), .b(new_n30_), .c(new_n61_), .O(new_n62_));
  nand2  g33(.a(new_n50_), .b(x12), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x11), .c(new_n53_), .d(x00), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n62_), .O(z11));
  inv1   g36(.a(x09), .O(new_n66_));
  nand4  g37(.a(x11), .b(new_n53_), .c(new_n66_), .d(x00), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n30_), .c(new_n31_), .O(z12));
endmodule


