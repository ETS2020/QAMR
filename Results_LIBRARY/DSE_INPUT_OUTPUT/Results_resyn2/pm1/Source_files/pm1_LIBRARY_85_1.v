// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n63_, new_n64_;
  nand2  g00(.a(x12), .b(x09), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(x11), .O(new_n32_));
  nand2  g03(.a(new_n31_), .b(x01), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n32_), .c(new_n30_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  nand3  g06(.a(x12), .b(new_n35_), .c(x09), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n35_), .c(x09), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z02));
  nand2  g10(.a(new_n37_), .b(x11), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z03));
  inv1   g13(.a(x09), .O(new_n43_));
  nand2  g14(.a(x12), .b(new_n43_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  inv1   g16(.a(new_n44_), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(x13), .O(z05));
  nand2  g18(.a(x12), .b(new_n35_), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  nand2  g21(.a(x09), .b(x01), .O(new_n51_));
  aoi21  g22(.a(new_n50_), .b(new_n48_), .c(new_n51_), .O(z06));
  nor2   g23(.a(new_n46_), .b(x15), .O(z07));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x11), .b(new_n54_), .c(x00), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(new_n56_));
  nand4  g27(.a(x12), .b(x04), .c(x03), .d(x02), .O(new_n57_));
  aoi21  g28(.a(new_n57_), .b(new_n56_), .c(new_n46_), .O(z08));
  nand2  g29(.a(new_n54_), .b(x00), .O(new_n59_));
  nand2  g30(.a(new_n48_), .b(new_n32_), .O(new_n60_));
  oai21  g31(.a(new_n60_), .b(new_n59_), .c(new_n44_), .O(z09));
  nor3   g32(.a(new_n55_), .b(new_n50_), .c(new_n30_), .O(z10));
  nand4  g33(.a(new_n57_), .b(x11), .c(new_n54_), .d(x00), .O(new_n63_));
  xor2a  g34(.a(x12), .b(x01), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n63_), .c(new_n44_), .O(z11));
  zero   g36(.O(z12));
endmodule


