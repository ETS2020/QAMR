// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n45_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x04), .O(new_n34_));
  aoi21  g05(.a(x11), .b(new_n34_), .c(new_n32_), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  nor2   g10(.a(new_n31_), .b(new_n39_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n38_), .c(x12), .d(new_n34_), .O(z02));
  nand3  g12(.a(new_n37_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z03));
  nor2   g15(.a(new_n32_), .b(new_n34_), .O(new_n45_));
  nor2   g16(.a(new_n45_), .b(x14), .O(z04));
  inv1   g17(.a(new_n45_), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x13), .O(z05));
  nand2  g19(.a(x03), .b(x02), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n32_), .O(new_n50_));
  nand2  g21(.a(x09), .b(x01), .O(new_n51_));
  aoi21  g22(.a(new_n50_), .b(x04), .c(new_n51_), .O(z06));
  nand2  g23(.a(new_n47_), .b(x15), .O(z07));
  inv1   g24(.a(x00), .O(new_n54_));
  nor2   g25(.a(x10), .b(new_n54_), .O(new_n55_));
  aoi21  g26(.a(new_n55_), .b(x11), .c(new_n45_), .O(z08));
  nand2  g27(.a(new_n32_), .b(x11), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n35_), .O(z09));
  inv1   g30(.a(x10), .O(new_n60_));
  nand4  g31(.a(x11), .b(new_n60_), .c(x09), .d(x00), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n34_), .c(new_n32_), .O(z10));
  nand2  g33(.a(new_n51_), .b(x12), .O(new_n63_));
  nand2  g34(.a(new_n32_), .b(x01), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n63_), .c(new_n55_), .d(x11), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n47_), .O(z11));
  nand4  g37(.a(x11), .b(new_n60_), .c(new_n39_), .d(x00), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n34_), .c(new_n32_), .O(z12));
endmodule


