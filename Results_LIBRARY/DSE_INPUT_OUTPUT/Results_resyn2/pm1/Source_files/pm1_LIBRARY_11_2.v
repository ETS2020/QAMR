// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n41_, new_n43_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  nor2   g02(.a(x12), .b(x02), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(x11), .O(new_n34_));
  oai21  g05(.a(x12), .b(x02), .c(new_n34_), .O(z01));
  inv1   g06(.a(new_n34_), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n36_), .c(x09), .O(z02));
  nand3  g10(.a(new_n37_), .b(new_n36_), .c(x09), .O(z03));
  inv1   g11(.a(x12), .O(new_n41_));
  aoi21  g12(.a(new_n41_), .b(x02), .c(x14), .O(z04));
  nand2  g13(.a(new_n41_), .b(x02), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x13), .O(z05));
  nand3  g15(.a(x11), .b(x04), .c(x03), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x12), .O(new_n46_));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  aoi21  g18(.a(new_n46_), .b(x02), .c(new_n47_), .O(z06));
  nand2  g19(.a(new_n43_), .b(x15), .O(z07));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n43_), .O(new_n52_));
  nand3  g23(.a(x04), .b(x03), .c(x02), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x12), .c(x09), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n52_), .O(z08));
  nand2  g27(.a(new_n32_), .b(new_n31_), .O(new_n57_));
  nand2  g28(.a(new_n50_), .b(x00), .O(new_n58_));
  aoi21  g29(.a(new_n34_), .b(new_n57_), .c(new_n58_), .O(z09));
  nand2  g30(.a(x12), .b(x09), .O(new_n60_));
  nand4  g31(.a(new_n53_), .b(x11), .c(new_n50_), .d(x00), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(new_n60_), .O(z10));
  nand2  g33(.a(new_n32_), .b(new_n30_), .O(new_n63_));
  nand3  g34(.a(x12), .b(x09), .c(x01), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(z11));
  inv1   g36(.a(x09), .O(new_n66_));
  nand3  g37(.a(x12), .b(x11), .c(new_n66_), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n58_), .c(new_n43_), .O(z12));
endmodule


