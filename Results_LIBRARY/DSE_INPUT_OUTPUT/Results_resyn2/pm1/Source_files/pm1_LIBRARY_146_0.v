// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(x07), .O(new_n33_));
  inv1   g04(.a(new_n33_), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n30_), .O(z00));
  nor2   g06(.a(x12), .b(x07), .O(new_n36_));
  aoi21  g07(.a(x12), .b(new_n31_), .c(new_n36_), .O(z01));
  nand3  g08(.a(x09), .b(x08), .c(x07), .O(new_n38_));
  nand4  g09(.a(x12), .b(x11), .c(x06), .d(x05), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(new_n36_), .O(z02));
  nor2   g12(.a(new_n32_), .b(new_n31_), .O(new_n42_));
  nand4  g13(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n42_), .c(x09), .O(z03));
  nor2   g15(.a(new_n36_), .b(x14), .O(z04));
  inv1   g16(.a(new_n36_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  aoi21  g19(.a(x12), .b(new_n31_), .c(new_n48_), .O(new_n49_));
  nand3  g20(.a(new_n46_), .b(x09), .c(x01), .O(new_n50_));
  nor2   g21(.a(new_n50_), .b(new_n49_), .O(z06));
  nand2  g22(.a(new_n46_), .b(x15), .O(z07));
  inv1   g23(.a(new_n48_), .O(new_n53_));
  inv1   g24(.a(x09), .O(new_n54_));
  nor2   g25(.a(new_n32_), .b(new_n54_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x11), .b(new_n57_), .c(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n56_), .c(new_n46_), .O(z08));
  inv1   g31(.a(new_n42_), .O(new_n61_));
  nand2  g32(.a(new_n57_), .b(x00), .O(new_n62_));
  aoi21  g33(.a(new_n61_), .b(new_n33_), .c(new_n62_), .O(z09));
  nand2  g34(.a(new_n55_), .b(new_n48_), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n58_), .c(new_n46_), .O(z10));
  nand4  g36(.a(x11), .b(new_n57_), .c(new_n30_), .d(x00), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x07), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n32_), .O(new_n68_));
  nand4  g39(.a(new_n59_), .b(new_n55_), .c(new_n48_), .d(x01), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n68_), .O(z11));
  nand2  g41(.a(new_n42_), .b(new_n54_), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n62_), .c(new_n46_), .O(z12));
endmodule


