// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x01), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x14), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n30_), .O(z00));
  nand3  g06(.a(x14), .b(x12), .c(new_n30_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand2  g10(.a(x12), .b(x09), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n38_), .c(x11), .O(new_n41_));
  nand2  g12(.a(x14), .b(new_n30_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n41_), .O(z03));
  inv1   g14(.a(x14), .O(z04));
  nor2   g15(.a(x14), .b(x11), .O(new_n45_));
  nor2   g16(.a(new_n45_), .b(x13), .O(z05));
  oai21  g17(.a(new_n40_), .b(new_n31_), .c(x14), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n30_), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n48_), .O(z06));
  nor2   g22(.a(new_n45_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  oai21  g26(.a(new_n49_), .b(new_n40_), .c(new_n55_), .O(z08));
  nand2  g27(.a(x12), .b(new_n30_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n53_), .c(x00), .O(new_n58_));
  aoi21  g29(.a(new_n42_), .b(new_n32_), .c(new_n58_), .O(z09));
  inv1   g30(.a(x09), .O(new_n60_));
  nand2  g31(.a(new_n49_), .b(x12), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n54_), .c(new_n60_), .O(z10));
  inv1   g33(.a(new_n45_), .O(new_n63_));
  nand4  g34(.a(new_n49_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n33_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n55_), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n63_), .O(z11));
  nand2  g38(.a(new_n53_), .b(x00), .O(new_n68_));
  nand3  g39(.a(x12), .b(x11), .c(new_n60_), .O(new_n69_));
  oai21  g40(.a(new_n69_), .b(new_n68_), .c(new_n63_), .O(z12));
endmodule


