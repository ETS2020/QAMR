// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x07), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x07), .O(new_n33_));
  nand2  g04(.a(new_n30_), .b(new_n33_), .O(new_n34_));
  inv1   g05(.a(x11), .O(new_n35_));
  nand2  g06(.a(x12), .b(new_n35_), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(z01));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g12(.a(new_n39_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g13(.a(new_n34_), .b(x14), .O(z04));
  nand2  g14(.a(new_n34_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  nand3  g17(.a(new_n34_), .b(x09), .c(x01), .O(new_n47_));
  aoi21  g18(.a(new_n46_), .b(new_n36_), .c(new_n47_), .O(z06));
  aoi21  g19(.a(new_n30_), .b(new_n33_), .c(x15), .O(z07));
  inv1   g20(.a(x09), .O(new_n50_));
  nor2   g21(.a(new_n30_), .b(new_n50_), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  inv1   g23(.a(x00), .O(new_n53_));
  inv1   g24(.a(x10), .O(new_n54_));
  nand2  g25(.a(x11), .b(new_n54_), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n53_), .c(new_n34_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n52_), .O(z08));
  nand2  g28(.a(new_n54_), .b(x00), .O(new_n58_));
  nor2   g29(.a(x12), .b(new_n35_), .O(new_n59_));
  nor3   g30(.a(new_n59_), .b(new_n58_), .c(new_n37_), .O(z09));
  inv1   g31(.a(new_n55_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n51_), .c(new_n45_), .d(x00), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(z10));
  inv1   g34(.a(x01), .O(new_n64_));
  nand4  g35(.a(x11), .b(new_n54_), .c(new_n64_), .d(x00), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(x07), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n30_), .O(new_n67_));
  nor2   g38(.a(new_n64_), .b(new_n53_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(new_n61_), .c(new_n51_), .d(new_n45_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n67_), .O(z11));
  nand3  g41(.a(x12), .b(x11), .c(new_n50_), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n58_), .c(new_n34_), .O(z12));
endmodule


