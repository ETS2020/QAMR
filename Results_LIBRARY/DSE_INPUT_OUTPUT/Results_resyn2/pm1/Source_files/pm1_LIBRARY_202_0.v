// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n41_,
    new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_;
  nor2   g00(.a(x12), .b(x11), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(z00));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(x11), .b(x08), .c(x07), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(x12), .c(x09), .O(z02));
  nand3  g06(.a(x12), .b(x11), .c(x09), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  oai22  g08(.a(new_n37_), .b(new_n30_), .c(new_n33_), .d(new_n32_), .O(z03));
  nand2  g09(.a(z00), .b(x14), .O(z04));
  nand2  g10(.a(z00), .b(x13), .O(z05));
  nand4  g11(.a(x11), .b(x04), .c(x03), .d(x02), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(z00), .c(x09), .d(x01), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(z06));
  nor2   g14(.a(new_n30_), .b(x15), .O(z07));
  inv1   g15(.a(x00), .O(new_n45_));
  oai21  g16(.a(x10), .b(new_n45_), .c(x11), .O(new_n46_));
  inv1   g17(.a(x09), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n47_), .c(x11), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x12), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n46_), .O(z08));
  inv1   g22(.a(x10), .O(new_n52_));
  nand4  g23(.a(x12), .b(x11), .c(new_n52_), .d(x00), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(z00), .O(z09));
  nand2  g25(.a(new_n48_), .b(x12), .O(new_n55_));
  nor2   g26(.a(x10), .b(new_n45_), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(x11), .c(x09), .O(new_n57_));
  nor2   g28(.a(new_n57_), .b(new_n55_), .O(z10));
  inv1   g29(.a(x01), .O(new_n59_));
  aoi21  g30(.a(new_n48_), .b(x12), .c(new_n59_), .O(new_n60_));
  nand2  g31(.a(x09), .b(x01), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x12), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(new_n56_), .c(x11), .O(new_n63_));
  nor2   g34(.a(new_n63_), .b(new_n60_), .O(z11));
  oai21  g35(.a(new_n53_), .b(x09), .c(z00), .O(z12));
  buf    g36(.a(x11), .O(z01));
endmodule


