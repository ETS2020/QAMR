// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x04), .O(new_n31_));
  nand2  g02(.a(x12), .b(new_n31_), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  nor2   g04(.a(x12), .b(x01), .O(new_n34_));
  aoi21  g05(.a(new_n34_), .b(new_n30_), .c(new_n33_), .O(z00));
  and2   g06(.a(x12), .b(x04), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n30_), .O(z01));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  nand2  g09(.a(x11), .b(x09), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(x04), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(z02));
  nand3  g12(.a(x12), .b(x09), .c(x04), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n38_), .c(x11), .O(z03));
  nor2   g15(.a(new_n33_), .b(x14), .O(z04));
  nor2   g16(.a(new_n33_), .b(x13), .O(z05));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  nand2  g18(.a(x12), .b(new_n30_), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  aoi22  g21(.a(new_n50_), .b(new_n48_), .c(new_n47_), .d(new_n32_), .O(z06));
  nor2   g22(.a(new_n33_), .b(x15), .O(z07));
  nand2  g23(.a(x03), .b(x02), .O(new_n53_));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x11), .b(new_n54_), .c(x00), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n32_), .O(new_n56_));
  oai21  g27(.a(new_n53_), .b(new_n42_), .c(new_n56_), .O(z08));
  nand2  g28(.a(x12), .b(x04), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(x11), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n48_), .c(new_n54_), .d(x00), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(z09));
  nand2  g32(.a(new_n53_), .b(new_n43_), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n55_), .O(z10));
  inv1   g34(.a(new_n34_), .O(new_n64_));
  and2   g35(.a(x09), .b(x01), .O(new_n65_));
  nand3  g36(.a(new_n53_), .b(new_n65_), .c(new_n36_), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n64_), .c(new_n55_), .O(z11));
  nor3   g38(.a(new_n55_), .b(new_n58_), .c(x09), .O(z12));
endmodule


