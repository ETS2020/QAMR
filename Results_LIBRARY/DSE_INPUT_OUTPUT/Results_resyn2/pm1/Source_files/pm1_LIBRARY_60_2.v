// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n60_,
    new_n62_, new_n64_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  oai21  g02(.a(x11), .b(new_n30_), .c(new_n31_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nor2   g04(.a(x12), .b(new_n33_), .O(new_n34_));
  nor2   g05(.a(new_n31_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n34_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand2  g10(.a(new_n37_), .b(x09), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x11), .O(z03));
  nand2  g13(.a(new_n31_), .b(x11), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nor2   g15(.a(new_n34_), .b(x13), .O(z05));
  inv1   g16(.a(new_n35_), .O(new_n46_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  inv1   g18(.a(new_n47_), .O(new_n48_));
  and2   g19(.a(x09), .b(x01), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  aoi21  g21(.a(new_n48_), .b(new_n46_), .c(new_n50_), .O(z06));
  nor2   g22(.a(new_n34_), .b(x15), .O(z07));
  nand4  g23(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  inv1   g25(.a(x10), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x00), .O(new_n56_));
  oai21  g27(.a(new_n56_), .b(new_n54_), .c(x12), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x11), .O(z08));
  nor3   g29(.a(new_n56_), .b(new_n35_), .c(new_n34_), .O(z09));
  nand4  g30(.a(new_n47_), .b(new_n55_), .c(x09), .d(x00), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(x12), .c(new_n33_), .O(z10));
  nand4  g32(.a(new_n49_), .b(new_n47_), .c(new_n55_), .d(x00), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(x12), .c(new_n33_), .O(z11));
  nand2  g34(.a(x12), .b(x11), .O(new_n64_));
  nor3   g35(.a(new_n56_), .b(new_n64_), .c(x09), .O(z12));
endmodule


