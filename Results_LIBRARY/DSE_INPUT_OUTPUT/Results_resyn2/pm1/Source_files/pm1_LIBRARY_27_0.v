// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n48_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(x04), .O(new_n33_));
  inv1   g04(.a(new_n33_), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n30_), .O(z00));
  nand2  g06(.a(new_n32_), .b(x04), .O(new_n36_));
  nor2   g07(.a(new_n32_), .b(new_n31_), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n36_), .O(z01));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n37_), .c(x09), .O(z02));
  nand3  g13(.a(new_n40_), .b(x11), .c(x09), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n36_), .O(z03));
  nor2   g16(.a(x12), .b(x04), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(x14), .O(z04));
  inv1   g18(.a(new_n46_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x13), .O(z05));
  nand2  g20(.a(x03), .b(x02), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x04), .O(new_n51_));
  inv1   g22(.a(x04), .O(new_n52_));
  oai21  g23(.a(new_n31_), .b(new_n52_), .c(x12), .O(new_n53_));
  nand2  g24(.a(x09), .b(x01), .O(new_n54_));
  aoi21  g25(.a(new_n53_), .b(new_n51_), .c(new_n54_), .O(z06));
  nand2  g26(.a(new_n48_), .b(x15), .O(z07));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x11), .b(new_n57_), .c(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nand3  g30(.a(x12), .b(x09), .c(x04), .O(new_n60_));
  oai22  g31(.a(new_n60_), .b(new_n50_), .c(new_n59_), .d(new_n46_), .O(z08));
  nand2  g32(.a(new_n57_), .b(x00), .O(new_n62_));
  aoi21  g33(.a(new_n38_), .b(new_n33_), .c(new_n62_), .O(z09));
  nand2  g34(.a(new_n50_), .b(x12), .O(new_n64_));
  nand4  g35(.a(x11), .b(new_n57_), .c(x09), .d(x00), .O(new_n65_));
  aoi22  g36(.a(new_n65_), .b(x12), .c(new_n64_), .d(x04), .O(z10));
  oai21  g37(.a(new_n65_), .b(new_n30_), .c(x12), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  nand2  g39(.a(new_n64_), .b(x01), .O(new_n69_));
  nand2  g40(.a(new_n54_), .b(x12), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n69_), .c(new_n59_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n68_), .O(z11));
  inv1   g43(.a(x09), .O(new_n73_));
  nand2  g44(.a(new_n37_), .b(new_n73_), .O(new_n74_));
  oai21  g45(.a(new_n74_), .b(new_n62_), .c(new_n48_), .O(z12));
endmodule


