// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n45_, new_n46_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  nor2   g02(.a(x12), .b(x02), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(new_n32_), .O(new_n34_));
  nand2  g05(.a(x12), .b(x11), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n34_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(x12), .b(x11), .c(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n38_), .O(z02));
  nand3  g12(.a(new_n37_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n34_), .O(z03));
  inv1   g15(.a(x02), .O(new_n45_));
  nor2   g16(.a(x12), .b(new_n45_), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(x14), .O(z04));
  nor2   g18(.a(new_n46_), .b(x13), .O(z05));
  inv1   g19(.a(new_n46_), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  oai21  g23(.a(new_n52_), .b(new_n31_), .c(new_n51_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n49_), .O(z06));
  nand2  g25(.a(new_n49_), .b(x15), .O(z07));
  nand3  g26(.a(x09), .b(x04), .c(x03), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x12), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x02), .O(new_n58_));
  inv1   g29(.a(x00), .O(new_n59_));
  nor2   g30(.a(x10), .b(new_n59_), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n58_), .c(x11), .O(z08));
  nand2  g32(.a(new_n32_), .b(new_n31_), .O(new_n62_));
  inv1   g33(.a(new_n60_), .O(new_n63_));
  aoi21  g34(.a(new_n35_), .b(new_n62_), .c(new_n63_), .O(z09));
  nand3  g35(.a(new_n60_), .b(new_n52_), .c(new_n40_), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(z10));
  oai21  g37(.a(x12), .b(new_n30_), .c(new_n52_), .O(new_n67_));
  oai21  g38(.a(x12), .b(x02), .c(new_n50_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n60_), .c(x11), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n67_), .O(z11));
  or2    g41(.a(new_n35_), .b(x09), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n63_), .c(new_n49_), .O(z12));
endmodule


