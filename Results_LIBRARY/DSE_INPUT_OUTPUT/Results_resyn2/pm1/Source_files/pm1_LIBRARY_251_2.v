// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n58_, new_n60_, new_n61_;
  inv1   g00(.a(x00), .O(new_n30_));
  inv1   g01(.a(x01), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nor2   g05(.a(x12), .b(new_n30_), .O(new_n35_));
  aoi21  g06(.a(x12), .b(new_n32_), .c(new_n35_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(x12), .b(x11), .c(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n38_), .O(z02));
  nand2  g12(.a(new_n40_), .b(new_n37_), .O(z03));
  nand2  g13(.a(new_n33_), .b(x00), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nor2   g15(.a(new_n35_), .b(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  aoi21  g17(.a(x12), .b(new_n32_), .c(new_n46_), .O(new_n47_));
  and2   g18(.a(x09), .b(x01), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nor2   g20(.a(new_n49_), .b(new_n47_), .O(z06));
  nand2  g21(.a(new_n43_), .b(x15), .O(z07));
  inv1   g22(.a(new_n46_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x09), .O(new_n53_));
  inv1   g24(.a(x10), .O(new_n54_));
  nand4  g25(.a(x12), .b(x11), .c(new_n54_), .d(x00), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(z09));
  nand2  g27(.a(z09), .b(new_n53_), .O(z08));
  nand4  g28(.a(new_n46_), .b(new_n40_), .c(new_n54_), .d(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(z10));
  and2   g30(.a(x01), .b(x00), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n46_), .c(new_n40_), .d(new_n54_), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z11));
  nor2   g33(.a(new_n55_), .b(x09), .O(z12));
endmodule


