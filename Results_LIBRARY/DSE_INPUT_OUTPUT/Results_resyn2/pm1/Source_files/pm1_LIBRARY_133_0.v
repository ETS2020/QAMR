// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x00), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  inv1   g05(.a(x00), .O(new_n35_));
  nand2  g06(.a(new_n30_), .b(new_n35_), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  nand3  g09(.a(x12), .b(x11), .c(x09), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(new_n37_), .O(z02));
  nand4  g12(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(new_n42_));
  xor2a  g13(.a(new_n42_), .b(new_n37_), .O(z03));
  nand2  g14(.a(new_n36_), .b(x14), .O(z04));
  nand2  g15(.a(new_n36_), .b(x13), .O(z05));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  nand3  g18(.a(new_n36_), .b(x09), .c(x01), .O(new_n48_));
  aoi21  g19(.a(new_n47_), .b(z01), .c(new_n48_), .O(z06));
  nand2  g20(.a(new_n36_), .b(x15), .O(z07));
  oai21  g21(.a(new_n33_), .b(x10), .c(x00), .O(new_n51_));
  inv1   g22(.a(x09), .O(new_n52_));
  oai21  g23(.a(new_n46_), .b(new_n52_), .c(x00), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x12), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n51_), .O(z08));
  aoi21  g26(.a(x12), .b(x00), .c(new_n33_), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand2  g28(.a(z01), .b(new_n57_), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(new_n56_), .c(new_n36_), .O(z09));
  nand2  g30(.a(new_n46_), .b(x12), .O(new_n60_));
  nor2   g31(.a(x10), .b(new_n35_), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(x11), .c(x09), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n60_), .O(z10));
  inv1   g34(.a(x01), .O(new_n64_));
  aoi21  g35(.a(new_n46_), .b(x12), .c(new_n64_), .O(new_n65_));
  nand2  g36(.a(x09), .b(x01), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x12), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n61_), .c(x11), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n65_), .O(z11));
  nand3  g40(.a(x11), .b(new_n57_), .c(x00), .O(new_n70_));
  nand2  g41(.a(x12), .b(new_n52_), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n70_), .c(new_n36_), .O(z12));
endmodule


