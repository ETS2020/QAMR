// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x09), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n33_), .c(x12), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x09), .O(z02));
  nand3  g08(.a(x12), .b(x11), .c(x09), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n35_), .O(z03));
  inv1   g11(.a(x09), .O(new_n41_));
  nor2   g12(.a(x12), .b(new_n41_), .O(new_n42_));
  nor2   g13(.a(new_n42_), .b(x14), .O(z04));
  nor2   g14(.a(new_n42_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(new_n33_), .c(x01), .O(new_n46_));
  aoi21  g17(.a(new_n46_), .b(x12), .c(new_n41_), .O(z06));
  oai21  g18(.a(x12), .b(new_n41_), .c(x15), .O(z07));
  inv1   g19(.a(x10), .O(new_n49_));
  nand3  g20(.a(x11), .b(new_n49_), .c(x00), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  inv1   g22(.a(new_n45_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x09), .O(new_n53_));
  aoi21  g24(.a(new_n53_), .b(new_n51_), .c(new_n42_), .O(z08));
  aoi21  g25(.a(new_n33_), .b(new_n41_), .c(x12), .O(new_n55_));
  inv1   g26(.a(x00), .O(new_n56_));
  nor2   g27(.a(x10), .b(new_n56_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(z01), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n55_), .O(z09));
  nand3  g30(.a(new_n57_), .b(new_n45_), .c(new_n39_), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(z10));
  inv1   g32(.a(x01), .O(new_n62_));
  nand4  g33(.a(x11), .b(new_n49_), .c(new_n62_), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n41_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n30_), .O(new_n65_));
  nand4  g36(.a(new_n51_), .b(new_n45_), .c(x09), .d(x01), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n65_), .O(z11));
  nor3   g38(.a(new_n50_), .b(new_n30_), .c(x09), .O(z12));
endmodule


