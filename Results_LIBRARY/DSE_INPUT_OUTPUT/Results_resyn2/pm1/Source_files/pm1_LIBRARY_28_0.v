// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x03), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  nor2   g04(.a(x12), .b(x03), .O(new_n34_));
  aoi21  g05(.a(x12), .b(new_n33_), .c(new_n34_), .O(z01));
  inv1   g06(.a(x09), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand2  g08(.a(x12), .b(x11), .O(new_n38_));
  nor3   g09(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n34_), .O(z02));
  nor2   g11(.a(new_n30_), .b(new_n36_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n37_), .c(x11), .O(new_n42_));
  xor2a  g13(.a(new_n42_), .b(new_n34_), .O(z03));
  nor2   g14(.a(new_n34_), .b(x14), .O(z04));
  inv1   g15(.a(new_n34_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x13), .O(z05));
  nand2  g17(.a(x12), .b(new_n33_), .O(new_n47_));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  aoi22  g21(.a(new_n50_), .b(new_n47_), .c(new_n48_), .d(new_n45_), .O(z06));
  nand2  g22(.a(new_n45_), .b(x15), .O(z07));
  nand2  g23(.a(new_n50_), .b(new_n41_), .O(new_n53_));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x11), .b(new_n54_), .c(x00), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n53_), .c(new_n45_), .O(z08));
  nand2  g28(.a(new_n30_), .b(x11), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(new_n47_), .c(new_n54_), .d(x00), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n45_), .O(z09));
  nand2  g31(.a(new_n49_), .b(new_n41_), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(new_n55_), .O(z10));
  inv1   g33(.a(x03), .O(new_n63_));
  oai21  g34(.a(new_n55_), .b(new_n48_), .c(x12), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g36(.a(x04), .b(x02), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(x12), .c(x09), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x01), .O(new_n68_));
  inv1   g39(.a(x01), .O(new_n69_));
  nand2  g40(.a(x12), .b(new_n69_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n68_), .c(new_n56_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n65_), .O(z11));
  nand2  g43(.a(new_n54_), .b(x00), .O(new_n73_));
  nor3   g44(.a(new_n73_), .b(new_n38_), .c(x09), .O(z12));
endmodule


