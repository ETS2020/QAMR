// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n34_, new_n35_, new_n36_, new_n39_, new_n40_,
    new_n43_, new_n44_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n57_, new_n58_, new_n60_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  oai21  g02(.a(x12), .b(new_n30_), .c(new_n31_), .O(z00));
  nand2  g03(.a(x12), .b(new_n31_), .O(z01));
  inv1   g04(.a(x09), .O(new_n34_));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n34_), .c(x11), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x12), .O(z02));
  nand4  g08(.a(new_n35_), .b(x12), .c(x11), .d(x09), .O(z03));
  inv1   g09(.a(x12), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(x11), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(x14), .O(z04));
  nand2  g12(.a(z01), .b(x13), .O(z05));
  nand3  g13(.a(x04), .b(x03), .c(x02), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(z01), .c(x09), .d(x01), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(z06));
  nor2   g16(.a(new_n40_), .b(x15), .O(z07));
  inv1   g17(.a(new_n43_), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x12), .c(x09), .O(new_n48_));
  inv1   g19(.a(x00), .O(new_n49_));
  nor2   g20(.a(x10), .b(new_n49_), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(new_n48_), .c(x11), .O(z08));
  nor2   g22(.a(x12), .b(new_n31_), .O(new_n52_));
  nor2   g23(.a(new_n50_), .b(new_n40_), .O(new_n53_));
  nor2   g24(.a(new_n53_), .b(new_n52_), .O(z09));
  nand3  g25(.a(new_n50_), .b(new_n43_), .c(x09), .O(new_n55_));
  aoi21  g26(.a(new_n55_), .b(x11), .c(new_n39_), .O(z10));
  nand4  g27(.a(new_n43_), .b(x12), .c(x09), .d(x01), .O(new_n57_));
  aoi21  g28(.a(new_n52_), .b(new_n30_), .c(new_n40_), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(new_n57_), .c(new_n53_), .O(z11));
  nand2  g30(.a(new_n50_), .b(new_n34_), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(x11), .c(new_n39_), .O(z12));
endmodule


