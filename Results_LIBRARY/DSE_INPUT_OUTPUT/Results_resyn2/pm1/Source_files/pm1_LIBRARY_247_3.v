// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n45_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  oai21  g04(.a(new_n31_), .b(x01), .c(x12), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  nand2  g06(.a(x11), .b(x09), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(new_n30_), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x12), .O(z02));
  and2   g09(.a(x12), .b(x11), .O(new_n39_));
  inv1   g10(.a(x09), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(x01), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(z03));
  nand2  g13(.a(x12), .b(x01), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  inv1   g15(.a(new_n43_), .O(new_n45_));
  nor2   g16(.a(new_n45_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x09), .O(new_n48_));
  aoi21  g19(.a(new_n48_), .b(new_n32_), .c(new_n30_), .O(z06));
  nor2   g20(.a(new_n45_), .b(x15), .O(z07));
  nand4  g21(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n30_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x12), .O(new_n53_));
  inv1   g24(.a(x00), .O(new_n54_));
  nor2   g25(.a(x10), .b(new_n54_), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(new_n53_), .c(x11), .O(z08));
  inv1   g27(.a(x10), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x00), .O(new_n58_));
  xor2a  g29(.a(x12), .b(x11), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n58_), .c(new_n43_), .O(z09));
  nand4  g31(.a(new_n55_), .b(new_n47_), .c(new_n41_), .d(new_n39_), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z10));
  nand3  g33(.a(new_n32_), .b(x11), .c(new_n30_), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n58_), .c(new_n43_), .O(z11));
  nand4  g35(.a(x11), .b(new_n57_), .c(new_n40_), .d(x00), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n30_), .c(new_n32_), .O(z12));
endmodule


