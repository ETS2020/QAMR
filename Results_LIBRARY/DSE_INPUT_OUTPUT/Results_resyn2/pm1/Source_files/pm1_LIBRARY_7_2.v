// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x04), .c(new_n30_), .O(z00));
  inv1   g03(.a(x04), .O(new_n33_));
  nand2  g04(.a(new_n30_), .b(new_n33_), .O(new_n34_));
  nand2  g05(.a(x12), .b(x11), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n34_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand2  g08(.a(x11), .b(x09), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x12), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n34_), .O(z02));
  inv1   g11(.a(new_n37_), .O(new_n41_));
  oai21  g12(.a(new_n38_), .b(new_n41_), .c(x12), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n34_), .O(z03));
  nand2  g14(.a(new_n30_), .b(x04), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  nand2  g16(.a(new_n44_), .b(x13), .O(z05));
  inv1   g17(.a(x11), .O(new_n47_));
  nand2  g18(.a(x03), .b(x02), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n47_), .c(x12), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  aoi21  g21(.a(new_n49_), .b(x04), .c(new_n50_), .O(z06));
  aoi21  g22(.a(new_n30_), .b(x04), .c(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  inv1   g24(.a(x09), .O(new_n54_));
  oai21  g25(.a(new_n48_), .b(new_n54_), .c(x12), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x04), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(x11), .c(new_n53_), .d(x00), .O(z08));
  nand2  g28(.a(new_n53_), .b(x00), .O(new_n58_));
  nand3  g29(.a(new_n30_), .b(new_n47_), .c(new_n33_), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(new_n35_), .c(new_n58_), .O(z09));
  nand4  g31(.a(x12), .b(x11), .c(new_n53_), .d(x00), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(new_n62_));
  nand3  g33(.a(x04), .b(x03), .c(x02), .O(new_n63_));
  and2   g34(.a(new_n63_), .b(x09), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n44_), .O(z10));
  inv1   g37(.a(x01), .O(new_n67_));
  nand4  g38(.a(x11), .b(new_n53_), .c(new_n67_), .d(x00), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n33_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n30_), .O(new_n70_));
  nand3  g41(.a(new_n64_), .b(new_n62_), .c(x01), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n70_), .O(z11));
  oai21  g43(.a(new_n61_), .b(x09), .c(new_n44_), .O(z12));
endmodule


