// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n44_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x14), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(x12), .c(new_n31_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n34_), .b(x12), .c(x11), .d(x09), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n37_), .O(z02));
  nand2  g11(.a(new_n39_), .b(new_n36_), .O(z03));
  nand2  g12(.a(x14), .b(new_n32_), .O(z04));
  aoi21  g13(.a(x14), .b(x12), .c(x13), .O(z05));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand2  g16(.a(x14), .b(x12), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x09), .c(x01), .O(new_n47_));
  aoi21  g18(.a(new_n45_), .b(z01), .c(new_n47_), .O(z06));
  nand2  g19(.a(new_n46_), .b(x15), .O(z07));
  aoi21  g20(.a(new_n45_), .b(x09), .c(x14), .O(new_n50_));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  oai21  g24(.a(new_n50_), .b(new_n32_), .c(new_n53_), .O(z08));
  nand2  g25(.a(x12), .b(new_n31_), .O(new_n55_));
  inv1   g26(.a(x00), .O(new_n56_));
  nor2   g27(.a(x10), .b(new_n56_), .O(new_n57_));
  nand2  g28(.a(new_n32_), .b(x11), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n57_), .c(new_n55_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n46_), .O(z09));
  nand4  g31(.a(new_n57_), .b(new_n44_), .c(x11), .d(x09), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n34_), .c(new_n32_), .O(z10));
  nand3  g33(.a(x12), .b(x09), .c(x01), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(new_n64_));
  nor2   g35(.a(x12), .b(x01), .O(new_n65_));
  aoi21  g36(.a(new_n64_), .b(new_n44_), .c(new_n65_), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n52_), .c(new_n46_), .O(z11));
  nand2  g38(.a(new_n34_), .b(x12), .O(new_n68_));
  nor3   g39(.a(new_n52_), .b(new_n68_), .c(x09), .O(z12));
endmodule


