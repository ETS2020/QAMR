// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n43_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_;
  inv1   g00(.a(x02), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nor2   g02(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nor3   g03(.a(x12), .b(x11), .c(x01), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(new_n32_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(x02), .c(x12), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand2  g08(.a(x11), .b(x09), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(new_n30_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(z02));
  nor2   g11(.a(new_n31_), .b(x02), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(new_n37_), .c(x11), .d(x09), .O(z03));
  inv1   g13(.a(new_n32_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nor2   g15(.a(new_n32_), .b(x13), .O(z05));
  nand2  g16(.a(x04), .b(x03), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n31_), .O(new_n47_));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  aoi21  g19(.a(new_n47_), .b(x02), .c(new_n48_), .O(z06));
  nand2  g20(.a(new_n43_), .b(x15), .O(z07));
  inv1   g21(.a(x10), .O(new_n51_));
  nand3  g22(.a(x11), .b(new_n51_), .c(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n43_), .O(z08));
  nand3  g25(.a(new_n35_), .b(new_n51_), .c(x00), .O(new_n55_));
  aoi22  g26(.a(new_n55_), .b(new_n31_), .c(new_n52_), .d(new_n41_), .O(z09));
  nand4  g27(.a(x11), .b(new_n51_), .c(x09), .d(x00), .O(new_n57_));
  aoi21  g28(.a(new_n57_), .b(new_n30_), .c(new_n31_), .O(z10));
  inv1   g29(.a(x01), .O(new_n59_));
  aoi21  g30(.a(x12), .b(new_n30_), .c(new_n59_), .O(new_n60_));
  nand2  g31(.a(new_n48_), .b(x12), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(x11), .c(new_n51_), .d(x00), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n60_), .O(z11));
  nand2  g34(.a(x12), .b(new_n30_), .O(new_n64_));
  nor3   g35(.a(new_n52_), .b(new_n64_), .c(x09), .O(z12));
endmodule


