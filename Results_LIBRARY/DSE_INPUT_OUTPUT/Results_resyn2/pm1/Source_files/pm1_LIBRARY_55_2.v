// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_;
  inv1   g00(.a(x04), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  or2    g04(.a(new_n33_), .b(x01), .O(z00));
  nand2  g05(.a(x12), .b(new_n31_), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand3  g09(.a(new_n36_), .b(x11), .c(x09), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(new_n40_));
  oai21  g11(.a(x12), .b(x04), .c(new_n40_), .O(z03));
  nand2  g12(.a(new_n32_), .b(x04), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x14), .O(z04));
  nand2  g14(.a(new_n42_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  inv1   g16(.a(x01), .O(new_n46_));
  inv1   g17(.a(x09), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g19(.a(new_n45_), .b(new_n31_), .c(new_n48_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n42_), .O(z06));
  aoi21  g21(.a(new_n32_), .b(x04), .c(x15), .O(z07));
  inv1   g22(.a(new_n45_), .O(new_n52_));
  nor2   g23(.a(new_n32_), .b(new_n47_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nor2   g25(.a(new_n31_), .b(x10), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x00), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n42_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n54_), .O(z08));
  nand2  g29(.a(x12), .b(x11), .O(new_n59_));
  inv1   g30(.a(x10), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x00), .O(new_n61_));
  aoi21  g32(.a(new_n59_), .b(new_n33_), .c(new_n61_), .O(z09));
  inv1   g33(.a(x00), .O(new_n63_));
  nand3  g34(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n63_), .c(new_n42_), .O(z10));
  nand4  g36(.a(x11), .b(new_n60_), .c(new_n46_), .d(x00), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n30_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n32_), .O(new_n68_));
  nor2   g39(.a(new_n46_), .b(new_n63_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(new_n55_), .c(new_n53_), .d(new_n45_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n68_), .O(z11));
  nor3   g42(.a(new_n61_), .b(new_n59_), .c(x09), .O(z12));
endmodule


