// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n45_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x13), .c(new_n30_), .O(z00));
  nand2  g03(.a(x12), .b(x11), .O(new_n33_));
  oai21  g04(.a(x13), .b(x12), .c(new_n33_), .O(z01));
  nand2  g05(.a(x13), .b(new_n30_), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand3  g08(.a(x12), .b(x11), .c(x09), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n36_), .O(z02));
  inv1   g11(.a(new_n38_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n37_), .O(z03));
  nand2  g13(.a(new_n35_), .b(x14), .O(z04));
  inv1   g14(.a(x13), .O(z05));
  inv1   g15(.a(x11), .O(new_n45_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n46_));
  aoi21  g17(.a(x12), .b(new_n45_), .c(new_n46_), .O(new_n47_));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n47_), .c(new_n35_), .O(z06));
  nor2   g20(.a(new_n36_), .b(x15), .O(z07));
  inv1   g21(.a(new_n46_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x12), .c(x09), .O(new_n52_));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(new_n52_), .c(new_n35_), .O(z08));
  nand2  g27(.a(new_n53_), .b(x00), .O(new_n57_));
  nor2   g28(.a(x13), .b(x12), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n45_), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(new_n33_), .c(new_n57_), .O(z09));
  nand2  g31(.a(new_n46_), .b(x12), .O(new_n61_));
  nand4  g32(.a(x11), .b(new_n53_), .c(x09), .d(x00), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n61_), .O(z10));
  nand4  g34(.a(new_n46_), .b(x12), .c(x09), .d(x01), .O(new_n64_));
  inv1   g35(.a(x01), .O(new_n65_));
  nand2  g36(.a(new_n58_), .b(new_n65_), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n64_), .c(new_n54_), .O(z11));
  inv1   g38(.a(x09), .O(new_n68_));
  nand3  g39(.a(x12), .b(x11), .c(new_n68_), .O(new_n69_));
  oai21  g40(.a(new_n69_), .b(new_n57_), .c(new_n35_), .O(z12));
endmodule


