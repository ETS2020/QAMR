// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x14), .O(new_n31_));
  inv1   g02(.a(x01), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n30_), .O(z00));
  nand3  g07(.a(new_n31_), .b(x12), .c(new_n30_), .O(z01));
  nand4  g08(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand2  g11(.a(x12), .b(x09), .O(new_n41_));
  oai21  g12(.a(new_n41_), .b(new_n39_), .c(x11), .O(new_n42_));
  nand2  g13(.a(new_n31_), .b(new_n30_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n42_), .O(z03));
  nand2  g15(.a(x14), .b(x11), .O(z04));
  nor2   g16(.a(new_n31_), .b(x11), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(x13), .O(z05));
  oai21  g18(.a(new_n41_), .b(new_n32_), .c(new_n31_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n30_), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n49_), .O(z06));
  nor2   g23(.a(new_n46_), .b(x15), .O(z07));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x11), .b(new_n54_), .c(x00), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(new_n56_));
  oai21  g27(.a(new_n50_), .b(new_n41_), .c(new_n56_), .O(z08));
  nand2  g28(.a(x12), .b(new_n30_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n54_), .c(x00), .O(new_n59_));
  aoi21  g30(.a(new_n43_), .b(new_n33_), .c(new_n59_), .O(z09));
  inv1   g31(.a(x09), .O(new_n61_));
  nand2  g32(.a(new_n50_), .b(x12), .O(new_n62_));
  nor3   g33(.a(new_n62_), .b(new_n55_), .c(new_n61_), .O(z10));
  inv1   g34(.a(new_n46_), .O(new_n64_));
  nand4  g35(.a(new_n50_), .b(x12), .c(x09), .d(x01), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n34_), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n56_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n64_), .O(z11));
  nand2  g39(.a(new_n54_), .b(x00), .O(new_n69_));
  nand3  g40(.a(x12), .b(x11), .c(new_n61_), .O(new_n70_));
  oai21  g41(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(z12));
endmodule


