// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n68_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x01), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x02), .c(new_n31_), .O(new_n33_));
  inv1   g04(.a(new_n33_), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n30_), .O(z00));
  nand2  g06(.a(new_n32_), .b(x02), .O(new_n36_));
  nand2  g07(.a(x12), .b(x11), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n36_), .O(z01));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand3  g12(.a(new_n39_), .b(x11), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n36_), .O(z03));
  nor2   g15(.a(x12), .b(x02), .O(new_n45_));
  nor2   g16(.a(new_n45_), .b(x14), .O(z04));
  nor2   g17(.a(new_n45_), .b(x13), .O(z05));
  inv1   g18(.a(new_n45_), .O(new_n48_));
  nand2  g19(.a(x09), .b(x01), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  aoi21  g21(.a(x12), .b(new_n30_), .c(new_n50_), .O(new_n51_));
  aoi21  g22(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(z06));
  nand2  g23(.a(new_n48_), .b(x15), .O(z07));
  inv1   g24(.a(new_n50_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x12), .c(x09), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n56_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n55_), .c(new_n48_), .O(z08));
  nand2  g30(.a(new_n56_), .b(x00), .O(new_n60_));
  nand3  g31(.a(new_n32_), .b(new_n30_), .c(x02), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n37_), .c(new_n60_), .O(z09));
  inv1   g33(.a(x09), .O(new_n63_));
  nand2  g34(.a(new_n50_), .b(x12), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n57_), .c(new_n63_), .O(z10));
  nand4  g36(.a(new_n50_), .b(x12), .c(x09), .d(x01), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n33_), .c(new_n57_), .O(z11));
  nand3  g38(.a(x12), .b(x11), .c(new_n63_), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n60_), .c(new_n48_), .O(z12));
endmodule


