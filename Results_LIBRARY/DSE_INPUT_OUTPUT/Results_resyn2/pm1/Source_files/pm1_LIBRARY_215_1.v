// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_;
  inv1   g00(.a(x01), .O(new_n30_));
  nor2   g01(.a(x12), .b(x11), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x09), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  inv1   g04(.a(x09), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x01), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x12), .c(new_n33_), .O(z01));
  nand2  g08(.a(x12), .b(x11), .O(new_n38_));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(x01), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x09), .O(z02));
  inv1   g12(.a(new_n38_), .O(new_n42_));
  nand2  g13(.a(new_n39_), .b(new_n42_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x01), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x09), .O(z03));
  nor2   g16(.a(new_n35_), .b(x14), .O(z04));
  nor2   g17(.a(new_n35_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  aoi21  g19(.a(x12), .b(new_n33_), .c(new_n48_), .O(new_n49_));
  nor3   g20(.a(new_n49_), .b(new_n34_), .c(new_n30_), .O(z06));
  nand2  g21(.a(new_n36_), .b(x15), .O(z07));
  inv1   g22(.a(x00), .O(new_n52_));
  nor2   g23(.a(x10), .b(new_n52_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x11), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n36_), .O(new_n55_));
  nor2   g26(.a(new_n34_), .b(new_n30_), .O(new_n56_));
  inv1   g27(.a(new_n48_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n56_), .c(x12), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n55_), .O(z08));
  oai21  g30(.a(new_n42_), .b(new_n31_), .c(new_n53_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n36_), .O(z09));
  nand4  g32(.a(new_n53_), .b(new_n48_), .c(x12), .d(x11), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(x01), .c(new_n34_), .O(z10));
  inv1   g34(.a(x12), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(new_n34_), .c(new_n30_), .O(new_n65_));
  nand3  g36(.a(new_n48_), .b(new_n56_), .c(x12), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n65_), .c(new_n54_), .O(z11));
  nand3  g38(.a(new_n53_), .b(new_n42_), .c(new_n34_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n36_), .O(z12));
endmodule


