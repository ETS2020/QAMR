// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x02), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g03(.a(x02), .O(new_n33_));
  nand2  g04(.a(x12), .b(x11), .O(new_n34_));
  oai21  g05(.a(x12), .b(new_n33_), .c(new_n34_), .O(z01));
  nor2   g06(.a(x12), .b(x02), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(x12), .b(x11), .c(x09), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  aoi21  g11(.a(new_n40_), .b(new_n38_), .c(new_n36_), .O(z02));
  nand2  g12(.a(new_n40_), .b(new_n37_), .O(z03));
  inv1   g13(.a(new_n36_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x14), .O(z04));
  nor2   g15(.a(new_n36_), .b(x13), .O(z05));
  nand2  g16(.a(x04), .b(x03), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x02), .O(new_n47_));
  inv1   g18(.a(x11), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n33_), .c(x12), .O(new_n49_));
  nand2  g20(.a(x09), .b(x01), .O(new_n50_));
  aoi21  g21(.a(new_n49_), .b(new_n47_), .c(new_n50_), .O(z06));
  nand2  g22(.a(new_n43_), .b(x15), .O(z07));
  nand3  g23(.a(x04), .b(x03), .c(x02), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x12), .c(x09), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n56_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n55_), .c(new_n43_), .O(z08));
  nor2   g30(.a(x12), .b(new_n33_), .O(new_n60_));
  nand3  g31(.a(new_n48_), .b(new_n56_), .c(x00), .O(new_n61_));
  aoi22  g32(.a(new_n61_), .b(new_n60_), .c(new_n57_), .d(x12), .O(z09));
  nand4  g33(.a(x11), .b(new_n56_), .c(x09), .d(x00), .O(new_n63_));
  aoi21  g34(.a(new_n46_), .b(x12), .c(new_n33_), .O(new_n64_));
  aoi21  g35(.a(new_n63_), .b(x12), .c(new_n64_), .O(z10));
  inv1   g36(.a(x01), .O(new_n66_));
  nand3  g37(.a(new_n30_), .b(x02), .c(new_n66_), .O(new_n67_));
  nand4  g38(.a(new_n53_), .b(x12), .c(x09), .d(x01), .O(new_n68_));
  aoi21  g39(.a(new_n68_), .b(new_n67_), .c(new_n57_), .O(z11));
  nand2  g40(.a(new_n56_), .b(x00), .O(new_n70_));
  nor3   g41(.a(new_n70_), .b(new_n34_), .c(x09), .O(z12));
endmodule


