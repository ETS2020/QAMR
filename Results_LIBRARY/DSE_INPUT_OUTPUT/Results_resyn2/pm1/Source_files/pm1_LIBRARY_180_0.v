// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n34_, new_n35_, new_n40_, new_n41_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(new_n30_), .O(z00));
  nand2  g03(.a(x12), .b(new_n30_), .O(z01));
  nand4  g04(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand4  g06(.a(new_n35_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g07(.a(new_n34_), .b(x12), .c(x11), .d(x09), .O(z03));
  aoi21  g08(.a(new_n31_), .b(new_n30_), .c(x14), .O(z04));
  nand2  g09(.a(z00), .b(x13), .O(z05));
  nand4  g10(.a(x11), .b(x04), .c(x03), .d(x02), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(z00), .c(x09), .d(x01), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(z06));
  nand2  g13(.a(z00), .b(x15), .O(z07));
  inv1   g14(.a(x00), .O(new_n44_));
  oai21  g15(.a(x10), .b(new_n44_), .c(x11), .O(new_n45_));
  inv1   g16(.a(x09), .O(new_n46_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n46_), .c(x11), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n45_), .O(z08));
  nor2   g21(.a(x10), .b(new_n44_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x12), .c(x11), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(z09));
  nand2  g24(.a(new_n47_), .b(x12), .O(new_n54_));
  inv1   g25(.a(x10), .O(new_n55_));
  nand4  g26(.a(x11), .b(new_n55_), .c(x09), .d(x00), .O(new_n56_));
  oai21  g27(.a(new_n56_), .b(new_n54_), .c(z00), .O(z10));
  inv1   g28(.a(x01), .O(new_n58_));
  aoi21  g29(.a(new_n47_), .b(x12), .c(new_n58_), .O(new_n59_));
  nand2  g30(.a(x09), .b(x01), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x12), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n51_), .c(x11), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n59_), .O(z11));
  nor2   g34(.a(new_n52_), .b(x09), .O(z12));
endmodule


