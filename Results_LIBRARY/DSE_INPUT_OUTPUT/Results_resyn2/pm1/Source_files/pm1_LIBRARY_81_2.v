// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g04(.a(x13), .b(x11), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x12), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  and2   g08(.a(x13), .b(x12), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n37_), .c(x11), .d(x09), .O(z02));
  and2   g10(.a(x11), .b(x09), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x13), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  nor2   g14(.a(x13), .b(new_n32_), .O(new_n44_));
  nor2   g15(.a(new_n44_), .b(x14), .O(z04));
  nor2   g16(.a(x13), .b(x12), .O(z05));
  nand2  g17(.a(x09), .b(x01), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(x11), .c(x13), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(new_n49_));
  inv1   g20(.a(new_n47_), .O(new_n50_));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n49_), .O(z06));
  oai21  g24(.a(x13), .b(new_n32_), .c(x15), .O(z07));
  nand2  g25(.a(new_n38_), .b(x09), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x11), .b(new_n56_), .c(x00), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  oai22  g29(.a(new_n58_), .b(new_n44_), .c(new_n51_), .d(new_n55_), .O(z08));
  nand2  g30(.a(new_n32_), .b(x11), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(z01), .c(new_n56_), .d(x00), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z09));
  nand4  g33(.a(new_n51_), .b(new_n40_), .c(new_n56_), .d(x00), .O(new_n63_));
  aoi21  g34(.a(new_n63_), .b(x13), .c(new_n32_), .O(z10));
  nand3  g35(.a(new_n51_), .b(new_n50_), .c(new_n38_), .O(new_n65_));
  nand2  g36(.a(new_n32_), .b(new_n30_), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n65_), .c(new_n57_), .O(z11));
  inv1   g38(.a(x09), .O(new_n68_));
  nand2  g39(.a(new_n38_), .b(new_n68_), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n57_), .O(z12));
endmodule


