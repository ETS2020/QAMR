// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_;
  nor2   g00(.a(x12), .b(x11), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(z00));
  inv1   g02(.a(x11), .O(new_n32_));
  nand2  g03(.a(x12), .b(new_n32_), .O(z01));
  nand4  g04(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n34_));
  nand3  g05(.a(x12), .b(x11), .c(x09), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nor2   g07(.a(new_n36_), .b(new_n30_), .O(z02));
  nand3  g08(.a(new_n34_), .b(x12), .c(x09), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x11), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(z01), .O(z03));
  nand2  g11(.a(z00), .b(x14), .O(z04));
  nand2  g12(.a(z00), .b(x13), .O(z05));
  nand2  g13(.a(x09), .b(x01), .O(new_n43_));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(x09), .c(x01), .O(new_n45_));
  aoi22  g16(.a(new_n45_), .b(x11), .c(new_n43_), .d(x12), .O(z06));
  nor2   g17(.a(new_n30_), .b(x15), .O(z07));
  nand2  g18(.a(x12), .b(x09), .O(new_n48_));
  inv1   g19(.a(x10), .O(new_n49_));
  nand3  g20(.a(x11), .b(new_n49_), .c(x00), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(new_n51_));
  oai21  g22(.a(new_n44_), .b(new_n48_), .c(new_n51_), .O(z08));
  nand4  g23(.a(x12), .b(x11), .c(new_n49_), .d(x00), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(z00), .O(z09));
  nand2  g25(.a(new_n44_), .b(x09), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n53_), .c(z00), .O(z10));
  inv1   g27(.a(x01), .O(new_n57_));
  aoi21  g28(.a(new_n44_), .b(x12), .c(new_n57_), .O(new_n58_));
  nand2  g29(.a(new_n43_), .b(x12), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(x11), .c(new_n49_), .d(x00), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n58_), .O(z11));
  oai21  g32(.a(new_n53_), .b(x09), .c(z00), .O(z12));
endmodule


