// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x13), .O(z05));
  nor2   g03(.a(z05), .b(x12), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g05(.a(new_n33_), .O(new_n35_));
  nand2  g06(.a(x12), .b(x11), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n35_), .O(z01));
  nor2   g08(.a(x13), .b(x12), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nor3   g11(.a(new_n40_), .b(new_n36_), .c(new_n39_), .O(new_n41_));
  nor2   g12(.a(new_n41_), .b(new_n38_), .O(z02));
  nand3  g13(.a(new_n40_), .b(x11), .c(x09), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n35_), .O(z03));
  nor2   g16(.a(new_n38_), .b(x14), .O(z04));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  aoi21  g18(.a(x12), .b(new_n31_), .c(new_n47_), .O(new_n48_));
  inv1   g19(.a(new_n38_), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  nor2   g21(.a(new_n50_), .b(new_n48_), .O(z06));
  nand2  g22(.a(new_n49_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x11), .b(new_n53_), .c(x00), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  inv1   g26(.a(new_n47_), .O(new_n56_));
  inv1   g27(.a(x12), .O(new_n57_));
  nor2   g28(.a(new_n57_), .b(new_n39_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  oai21  g30(.a(new_n55_), .b(new_n38_), .c(new_n59_), .O(z08));
  nand2  g31(.a(new_n33_), .b(new_n31_), .O(new_n61_));
  nand2  g32(.a(new_n53_), .b(x00), .O(new_n62_));
  aoi21  g33(.a(new_n36_), .b(new_n61_), .c(new_n62_), .O(z09));
  nand2  g34(.a(new_n58_), .b(new_n47_), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n54_), .c(new_n49_), .O(z10));
  nand4  g36(.a(x11), .b(new_n53_), .c(new_n30_), .d(x00), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x13), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n57_), .O(new_n68_));
  nand4  g39(.a(new_n58_), .b(new_n55_), .c(new_n47_), .d(x01), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n68_), .O(z11));
  nor3   g41(.a(new_n62_), .b(new_n36_), .c(x09), .O(z12));
endmodule


