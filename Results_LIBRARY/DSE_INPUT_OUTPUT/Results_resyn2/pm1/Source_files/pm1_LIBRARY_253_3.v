// Benchmark "FAU" written by ABC on Tue Aug 11 19:16:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n70_, new_n71_;
  inv1   g00(.a(x12), .O(new_n30_));
  inv1   g01(.a(x14), .O(new_n31_));
  nor2   g02(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g03(.a(x01), .O(new_n33_));
  nand2  g04(.a(new_n30_), .b(new_n33_), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n32_), .O(z00));
  aoi21  g07(.a(new_n31_), .b(x11), .c(new_n30_), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(z01));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  nand2  g10(.a(x11), .b(x09), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n39_), .c(new_n31_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z02));
  inv1   g13(.a(new_n39_), .O(new_n43_));
  oai21  g14(.a(new_n40_), .b(new_n43_), .c(new_n31_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x12), .O(z03));
  nand2  g16(.a(x14), .b(new_n30_), .O(z04));
  inv1   g17(.a(new_n32_), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x13), .O(z05));
  nand2  g19(.a(x09), .b(x01), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(x11), .c(new_n31_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x12), .O(new_n51_));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  oai21  g24(.a(new_n53_), .b(new_n49_), .c(new_n51_), .O(z06));
  nand2  g25(.a(new_n47_), .b(x15), .O(z07));
  inv1   g26(.a(x00), .O(new_n56_));
  nor2   g27(.a(x10), .b(new_n56_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x11), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  nor2   g30(.a(x14), .b(new_n30_), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n53_), .c(x09), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n59_), .O(z08));
  nand2  g33(.a(new_n30_), .b(x11), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n37_), .O(z09));
  nand4  g36(.a(new_n57_), .b(new_n52_), .c(x11), .d(x09), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n31_), .c(new_n30_), .O(z10));
  nand4  g38(.a(new_n60_), .b(new_n52_), .c(x09), .d(x01), .O(new_n68_));
  aoi21  g39(.a(new_n68_), .b(new_n34_), .c(new_n58_), .O(z11));
  inv1   g40(.a(x09), .O(new_n70_));
  nand3  g41(.a(new_n57_), .b(x11), .c(new_n70_), .O(new_n71_));
  aoi21  g42(.a(new_n71_), .b(new_n31_), .c(new_n30_), .O(z12));
endmodule


