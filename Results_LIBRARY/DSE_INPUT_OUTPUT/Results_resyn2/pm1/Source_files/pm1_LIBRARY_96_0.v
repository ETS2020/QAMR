// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n39_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n63_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand4  g03(.a(new_n32_), .b(new_n31_), .c(x08), .d(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand4  g07(.a(new_n36_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g08(.a(new_n35_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g09(.a(x12), .b(x08), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(x14), .O(z04));
  nor2   g11(.a(new_n39_), .b(x13), .O(z05));
  inv1   g12(.a(new_n39_), .O(new_n42_));
  nand2  g13(.a(x09), .b(x01), .O(new_n43_));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  aoi21  g15(.a(x12), .b(new_n31_), .c(new_n44_), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z06));
  nor2   g17(.a(new_n39_), .b(x15), .O(z07));
  inv1   g18(.a(new_n44_), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x12), .c(x09), .O(new_n49_));
  inv1   g20(.a(x10), .O(new_n50_));
  nand3  g21(.a(x11), .b(new_n50_), .c(x00), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(new_n49_), .c(new_n42_), .O(z08));
  nand2  g24(.a(new_n50_), .b(x00), .O(new_n54_));
  xor2a  g25(.a(x12), .b(x11), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n54_), .c(new_n42_), .O(z09));
  nand2  g27(.a(new_n44_), .b(x12), .O(new_n57_));
  nand4  g28(.a(x11), .b(new_n50_), .c(x09), .d(x00), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(new_n57_), .c(new_n42_), .O(z10));
  nand3  g30(.a(new_n32_), .b(x08), .c(new_n30_), .O(new_n60_));
  nand4  g31(.a(new_n44_), .b(x12), .c(x09), .d(x01), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n60_), .c(new_n51_), .O(z11));
  nand2  g33(.a(x12), .b(x11), .O(new_n63_));
  nor3   g34(.a(new_n54_), .b(new_n63_), .c(x09), .O(z12));
endmodule


