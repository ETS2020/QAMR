// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n40_,
    new_n42_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_, new_n62_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x01), .O(new_n31_));
  nand2  g02(.a(x12), .b(new_n31_), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n30_), .O(z00));
  nand3  g04(.a(x12), .b(new_n30_), .c(new_n31_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  and2   g07(.a(x12), .b(x11), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(new_n36_), .c(x09), .O(z02));
  nand3  g09(.a(new_n37_), .b(new_n35_), .c(x09), .O(z03));
  nor2   g10(.a(x11), .b(new_n31_), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(x14), .O(z04));
  inv1   g12(.a(new_n40_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x13), .O(z05));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x09), .O(new_n45_));
  aoi21  g16(.a(new_n45_), .b(x11), .c(new_n31_), .O(z06));
  nor2   g17(.a(new_n40_), .b(x15), .O(z07));
  inv1   g18(.a(x10), .O(new_n48_));
  nand3  g19(.a(x11), .b(new_n48_), .c(x00), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  nand2  g21(.a(x12), .b(x09), .O(new_n51_));
  oai21  g22(.a(new_n51_), .b(new_n44_), .c(new_n50_), .O(z08));
  nand2  g23(.a(new_n48_), .b(x00), .O(new_n53_));
  nor2   g24(.a(x12), .b(x11), .O(new_n54_));
  nor2   g25(.a(new_n54_), .b(new_n37_), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n53_), .c(new_n42_), .O(z09));
  nand3  g27(.a(new_n44_), .b(x12), .c(x09), .O(new_n57_));
  oai21  g28(.a(new_n57_), .b(new_n49_), .c(new_n42_), .O(z10));
  nand4  g29(.a(new_n32_), .b(x11), .c(new_n48_), .d(x00), .O(new_n59_));
  aoi21  g30(.a(new_n57_), .b(x01), .c(new_n59_), .O(z11));
  inv1   g31(.a(x09), .O(new_n61_));
  nand2  g32(.a(new_n37_), .b(new_n61_), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n53_), .c(new_n42_), .O(z12));
endmodule


