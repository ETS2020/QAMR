// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n40_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n56_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x09), .O(new_n34_));
  oai21  g05(.a(new_n31_), .b(new_n34_), .c(x12), .O(z01));
  nand4  g06(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g09(.a(new_n36_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g10(.a(x12), .b(new_n34_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x14), .O(z04));
  nand2  g12(.a(new_n40_), .b(x13), .O(z05));
  aoi21  g13(.a(new_n31_), .b(x01), .c(new_n34_), .O(new_n43_));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x09), .O(new_n45_));
  oai22  g16(.a(new_n45_), .b(new_n30_), .c(new_n43_), .d(new_n32_), .O(z06));
  aoi21  g17(.a(x12), .b(new_n34_), .c(x15), .O(z07));
  aoi21  g18(.a(new_n44_), .b(x09), .c(new_n32_), .O(new_n48_));
  inv1   g19(.a(x10), .O(new_n49_));
  nand3  g20(.a(x11), .b(new_n49_), .c(x00), .O(new_n50_));
  or2    g21(.a(new_n50_), .b(new_n48_), .O(z08));
  nand2  g22(.a(new_n32_), .b(x11), .O(new_n52_));
  nand4  g23(.a(new_n52_), .b(z01), .c(new_n49_), .d(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(z09));
  nor3   g25(.a(new_n50_), .b(new_n45_), .c(new_n32_), .O(z10));
  xor2a  g26(.a(x12), .b(x01), .O(new_n56_));
  nor3   g27(.a(new_n56_), .b(new_n50_), .c(new_n48_), .O(z11));
  zero   g28(.O(z12));
endmodule


