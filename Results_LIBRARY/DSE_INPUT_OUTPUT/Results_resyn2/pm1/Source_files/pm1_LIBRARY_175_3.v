// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n65_;
  nand2  g00(.a(x12), .b(x04), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(new_n31_));
  inv1   g02(.a(x01), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n31_), .O(z00));
  inv1   g07(.a(x11), .O(new_n37_));
  nor2   g08(.a(new_n33_), .b(x04), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n37_), .O(z01));
  nand2  g10(.a(x11), .b(x09), .O(new_n40_));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nor2   g12(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  oai21  g13(.a(new_n42_), .b(x04), .c(x12), .O(z02));
  nand4  g14(.a(new_n41_), .b(new_n38_), .c(x11), .d(x09), .O(z03));
  nand2  g15(.a(new_n30_), .b(x14), .O(z04));
  nor2   g16(.a(new_n31_), .b(x13), .O(z05));
  nand2  g17(.a(x03), .b(x02), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n33_), .O(new_n48_));
  nand2  g19(.a(x09), .b(x01), .O(new_n49_));
  aoi21  g20(.a(new_n48_), .b(x04), .c(new_n49_), .O(z06));
  nor2   g21(.a(new_n31_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x11), .b(new_n52_), .c(x00), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n30_), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(z08));
  nand3  g26(.a(new_n37_), .b(new_n52_), .c(x00), .O(new_n56_));
  aoi22  g27(.a(new_n56_), .b(new_n33_), .c(new_n53_), .d(new_n38_), .O(z09));
  nand3  g28(.a(new_n38_), .b(x11), .c(x00), .O(new_n58_));
  nand2  g29(.a(new_n52_), .b(x09), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(new_n58_), .O(z10));
  nand3  g31(.a(x12), .b(x09), .c(x01), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n34_), .c(new_n30_), .O(new_n62_));
  and2   g33(.a(new_n62_), .b(new_n54_), .O(z11));
  inv1   g34(.a(x09), .O(new_n64_));
  nand2  g35(.a(new_n52_), .b(new_n64_), .O(new_n65_));
  nor2   g36(.a(new_n65_), .b(new_n58_), .O(z12));
endmodule


