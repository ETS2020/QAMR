// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n74_;
  inv1   g00(.a(x12), .O(new_n30_));
  inv1   g01(.a(x14), .O(z04));
  nor2   g02(.a(z04), .b(new_n30_), .O(new_n32_));
  inv1   g03(.a(x01), .O(new_n33_));
  nand2  g04(.a(new_n30_), .b(new_n33_), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(new_n32_), .O(z00));
  inv1   g07(.a(x11), .O(new_n37_));
  nand3  g08(.a(z04), .b(x12), .c(new_n37_), .O(z01));
  nand2  g09(.a(x11), .b(x09), .O(new_n39_));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n39_), .c(z04), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(z02));
  inv1   g13(.a(new_n39_), .O(new_n43_));
  nor2   g14(.a(x14), .b(new_n30_), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(new_n40_), .c(new_n43_), .O(z03));
  inv1   g16(.a(new_n32_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x13), .O(z05));
  nand2  g18(.a(x09), .b(x01), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(x11), .c(z04), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x12), .O(new_n50_));
  and2   g21(.a(x09), .b(x01), .O(new_n51_));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n50_), .O(z06));
  nor2   g25(.a(new_n32_), .b(x15), .O(z07));
  inv1   g26(.a(x09), .O(new_n56_));
  oai21  g27(.a(new_n52_), .b(new_n56_), .c(z04), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x12), .O(new_n58_));
  inv1   g29(.a(x10), .O(new_n59_));
  nand3  g30(.a(x11), .b(new_n59_), .c(x00), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n58_), .O(z08));
  nand2  g33(.a(x12), .b(new_n37_), .O(new_n63_));
  inv1   g34(.a(x00), .O(new_n64_));
  nor2   g35(.a(x10), .b(new_n64_), .O(new_n65_));
  nand2  g36(.a(new_n30_), .b(x11), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(new_n65_), .c(new_n63_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n46_), .O(z09));
  nand2  g39(.a(new_n52_), .b(new_n44_), .O(new_n69_));
  nand2  g40(.a(new_n65_), .b(new_n43_), .O(new_n70_));
  nor2   g41(.a(new_n70_), .b(new_n69_), .O(z10));
  nand4  g42(.a(new_n52_), .b(new_n51_), .c(z04), .d(x12), .O(new_n72_));
  aoi21  g43(.a(new_n72_), .b(new_n34_), .c(new_n60_), .O(z11));
  nand2  g44(.a(new_n44_), .b(new_n56_), .O(new_n74_));
  nor2   g45(.a(new_n74_), .b(new_n60_), .O(z12));
endmodule


