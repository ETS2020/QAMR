// Benchmark "FAU" written by ABC on Tue Aug 11 19:15:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x10), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n31_), .c(new_n34_), .O(z01));
  nand2  g06(.a(x11), .b(x09), .O(new_n36_));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(new_n34_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z02));
  inv1   g10(.a(new_n36_), .O(new_n40_));
  nand4  g11(.a(new_n37_), .b(new_n40_), .c(x12), .d(new_n34_), .O(z03));
  nand2  g12(.a(x12), .b(x10), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x14), .O(z04));
  nand2  g14(.a(new_n42_), .b(x13), .O(z05));
  nand2  g15(.a(x09), .b(x01), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(x11), .c(new_n34_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n45_), .c(new_n47_), .O(z06));
  aoi21  g21(.a(x12), .b(x10), .c(x15), .O(z07));
  nand3  g22(.a(x11), .b(new_n34_), .c(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  nand3  g24(.a(new_n49_), .b(x12), .c(x09), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n53_), .O(z08));
  aoi21  g26(.a(new_n31_), .b(new_n34_), .c(x12), .O(new_n56_));
  oai21  g27(.a(x10), .b(x00), .c(z01), .O(new_n57_));
  nor2   g28(.a(new_n57_), .b(new_n56_), .O(z09));
  nand3  g29(.a(new_n48_), .b(new_n40_), .c(x00), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(new_n34_), .c(new_n32_), .O(z10));
  nand4  g31(.a(new_n48_), .b(x12), .c(x09), .d(x01), .O(new_n61_));
  nand3  g32(.a(new_n32_), .b(new_n34_), .c(new_n30_), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  and2   g34(.a(x11), .b(x00), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n42_), .O(z11));
  nor3   g37(.a(new_n52_), .b(new_n32_), .c(x09), .O(z12));
endmodule


