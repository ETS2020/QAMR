// Benchmark "FAU" written by ABC on Tue Aug 11 19:14:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_;
  inv1   g00(.a(x12), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x03), .c(new_n30_), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  inv1   g04(.a(x03), .O(new_n34_));
  nand2  g05(.a(new_n30_), .b(new_n34_), .O(new_n35_));
  oai21  g06(.a(new_n30_), .b(new_n33_), .c(new_n35_), .O(z01));
  nand4  g07(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n37_));
  nand2  g08(.a(x11), .b(x09), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x12), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n35_), .O(z02));
  inv1   g11(.a(new_n37_), .O(new_n41_));
  oai21  g12(.a(new_n38_), .b(new_n41_), .c(x12), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n35_), .O(z03));
  aoi21  g14(.a(new_n30_), .b(x03), .c(x14), .O(z04));
  nand2  g15(.a(new_n30_), .b(x03), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x13), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  and2   g18(.a(x09), .b(x01), .O(new_n48_));
  oai21  g19(.a(new_n47_), .b(new_n33_), .c(new_n48_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n45_), .O(z06));
  nand2  g21(.a(new_n45_), .b(x15), .O(z07));
  nand3  g22(.a(x09), .b(x04), .c(x02), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x12), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x03), .O(new_n54_));
  inv1   g25(.a(x00), .O(new_n55_));
  nor2   g26(.a(x10), .b(new_n55_), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n54_), .c(x11), .O(z08));
  aoi21  g28(.a(new_n56_), .b(x11), .c(new_n30_), .O(new_n58_));
  aoi21  g29(.a(new_n56_), .b(new_n33_), .c(new_n35_), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(new_n58_), .O(z09));
  inv1   g31(.a(x10), .O(new_n61_));
  nand4  g32(.a(x12), .b(x11), .c(new_n61_), .d(x00), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(new_n63_));
  and2   g34(.a(new_n47_), .b(x09), .O(new_n64_));
  and2   g35(.a(new_n64_), .b(new_n63_), .O(z10));
  inv1   g36(.a(x01), .O(new_n66_));
  nand4  g37(.a(x11), .b(new_n61_), .c(new_n66_), .d(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n34_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n30_), .O(new_n69_));
  nand3  g40(.a(new_n64_), .b(new_n63_), .c(x01), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n69_), .O(z11));
  nor2   g42(.a(new_n62_), .b(x09), .O(z12));
endmodule


