// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_;
  inv1   g00(.a(x00), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  and2   g03(.a(x13), .b(x09), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  nand2  g06(.a(new_n22_), .b(new_n25_), .O(new_n26_));
  nand2  g07(.a(new_n21_), .b(x08), .O(new_n27_));
  nand4  g08(.a(new_n27_), .b(new_n26_), .c(new_n24_), .d(x03), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(new_n20_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x05), .O(z0));
  oai21  g11(.a(new_n21_), .b(x08), .c(x10), .O(new_n31_));
  nor2   g12(.a(x10), .b(x08), .O(new_n32_));
  nand3  g13(.a(new_n32_), .b(x04), .c(x02), .O(new_n33_));
  nand4  g14(.a(new_n33_), .b(new_n31_), .c(new_n24_), .d(x03), .O(new_n34_));
  inv1   g15(.a(x03), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(x01), .c(x00), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x05), .O(z1));
  inv1   g19(.a(x11), .O(new_n39_));
  nand3  g20(.a(new_n32_), .b(new_n22_), .c(new_n39_), .O(new_n40_));
  nand2  g21(.a(new_n33_), .b(x11), .O(new_n41_));
  nand3  g22(.a(new_n41_), .b(new_n40_), .c(new_n24_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(x03), .O(new_n43_));
  nor2   g24(.a(x06), .b(x03), .O(new_n44_));
  nand2  g25(.a(x05), .b(new_n20_), .O(new_n45_));
  nor2   g26(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n43_), .O(z2));
  inv1   g28(.a(x12), .O(new_n48_));
  nand4  g29(.a(new_n32_), .b(new_n22_), .c(new_n48_), .d(new_n39_), .O(new_n49_));
  inv1   g30(.a(x10), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n25_), .O(new_n51_));
  nand3  g32(.a(new_n39_), .b(x04), .c(x02), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n51_), .c(x12), .O(new_n53_));
  nand3  g34(.a(new_n23_), .b(new_n22_), .c(new_n20_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n53_), .c(new_n49_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x03), .O(new_n56_));
  nor2   g37(.a(x07), .b(x03), .O(new_n57_));
  nor2   g38(.a(new_n57_), .b(new_n45_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n56_), .O(z3));
  nand2  g40(.a(new_n23_), .b(x04), .O(new_n60_));
  aoi21  g41(.a(x05), .b(x00), .c(new_n60_), .O(z4));
endmodule


