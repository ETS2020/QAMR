// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  xor2a  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  nand3  g03(.a(x09), .b(x08), .c(x03), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x12), .O(new_n24_));
  oai21  g05(.a(x03), .b(x00), .c(x05), .O(new_n25_));
  aoi21  g06(.a(new_n24_), .b(x13), .c(new_n25_), .O(new_n26_));
  oai21  g07(.a(new_n22_), .b(new_n20_), .c(new_n26_), .O(z0));
  oai21  g08(.a(new_n21_), .b(x08), .c(x10), .O(new_n28_));
  inv1   g09(.a(new_n21_), .O(new_n29_));
  nand2  g10(.a(x13), .b(x09), .O(new_n30_));
  inv1   g11(.a(new_n30_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  inv1   g13(.a(x08), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand4  g15(.a(new_n34_), .b(new_n33_), .c(x04), .d(x02), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(new_n32_), .c(new_n28_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x03), .O(new_n37_));
  nor2   g18(.a(x03), .b(x01), .O(new_n38_));
  inv1   g19(.a(x13), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(x12), .c(x05), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n37_), .O(z1));
  nor2   g23(.a(new_n39_), .b(x12), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  xor2a  g25(.a(new_n35_), .b(new_n44_), .O(new_n45_));
  oai21  g26(.a(x06), .b(x03), .c(x05), .O(new_n46_));
  aoi21  g27(.a(new_n45_), .b(x03), .c(new_n46_), .O(new_n47_));
  nand2  g28(.a(new_n21_), .b(new_n44_), .O(new_n48_));
  nand4  g29(.a(new_n48_), .b(new_n31_), .c(x12), .d(x03), .O(new_n49_));
  oai21  g30(.a(new_n47_), .b(new_n43_), .c(new_n49_), .O(z2));
  nand4  g31(.a(new_n29_), .b(new_n44_), .c(new_n34_), .d(new_n33_), .O(new_n51_));
  nand2  g32(.a(new_n33_), .b(x02), .O(new_n52_));
  inv1   g33(.a(x12), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n44_), .c(new_n34_), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n52_), .c(new_n30_), .O(new_n55_));
  aoi22  g36(.a(new_n55_), .b(x04), .c(new_n51_), .d(x12), .O(new_n56_));
  nor2   g37(.a(x07), .b(x03), .O(new_n57_));
  nor2   g38(.a(new_n57_), .b(new_n40_), .O(new_n58_));
  oai21  g39(.a(new_n56_), .b(new_n20_), .c(new_n58_), .O(z3));
  nand2  g40(.a(x09), .b(x04), .O(new_n60_));
  aoi21  g41(.a(new_n60_), .b(x12), .c(new_n39_), .O(z4));
endmodule


