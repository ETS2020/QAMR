// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_;
  nand3  g00(.a(x09), .b(x08), .c(x03), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x05), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x13), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x04), .c(x02), .O(new_n24_));
  nand2  g05(.a(x04), .b(x02), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x08), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(new_n24_), .c(x03), .O(new_n27_));
  inv1   g08(.a(x03), .O(new_n28_));
  inv1   g09(.a(x05), .O(new_n29_));
  aoi21  g10(.a(new_n28_), .b(x00), .c(new_n29_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n27_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n22_), .O(z0));
  inv1   g13(.a(x10), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n23_), .O(new_n34_));
  nand2  g15(.a(x13), .b(x09), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n34_), .c(new_n25_), .O(new_n36_));
  inv1   g17(.a(new_n25_), .O(new_n37_));
  nand3  g18(.a(new_n35_), .b(new_n37_), .c(new_n23_), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(x10), .c(new_n36_), .O(new_n39_));
  inv1   g20(.a(x01), .O(new_n40_));
  aoi21  g21(.a(new_n28_), .b(new_n40_), .c(new_n29_), .O(new_n41_));
  oai21  g22(.a(new_n39_), .b(new_n28_), .c(new_n41_), .O(z1));
  nor2   g23(.a(new_n36_), .b(x11), .O(new_n43_));
  nand2  g24(.a(new_n37_), .b(x11), .O(new_n44_));
  nand3  g25(.a(new_n35_), .b(new_n33_), .c(new_n23_), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n44_), .c(x03), .O(new_n46_));
  inv1   g27(.a(x06), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n28_), .c(new_n29_), .O(new_n48_));
  oai21  g29(.a(new_n46_), .b(new_n43_), .c(new_n48_), .O(z2));
  inv1   g30(.a(x11), .O(new_n50_));
  nand3  g31(.a(new_n35_), .b(new_n50_), .c(new_n33_), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n24_), .c(x12), .O(new_n52_));
  oai21  g33(.a(x10), .b(x08), .c(new_n35_), .O(new_n53_));
  oai21  g34(.a(x12), .b(x11), .c(new_n35_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n53_), .c(new_n37_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x03), .O(new_n57_));
  inv1   g38(.a(x07), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(new_n28_), .c(new_n29_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n57_), .O(z3));
  inv1   g41(.a(x04), .O(new_n61_));
  oai22  g42(.a(new_n35_), .b(new_n61_), .c(x13), .d(x05), .O(z4));
endmodule


