// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n62_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  aoi21  g05(.a(new_n23_), .b(new_n24_), .c(new_n21_), .O(new_n25_));
  aoi21  g06(.a(new_n23_), .b(new_n21_), .c(new_n25_), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x05), .O(new_n28_));
  aoi21  g09(.a(new_n20_), .b(new_n27_), .c(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n26_), .b(new_n20_), .c(new_n29_), .O(z0));
  nand2  g11(.a(x05), .b(x03), .O(new_n31_));
  oai21  g12(.a(new_n22_), .b(x08), .c(x10), .O(new_n32_));
  inv1   g13(.a(new_n24_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(x04), .c(x02), .O(new_n34_));
  inv1   g15(.a(x10), .O(new_n35_));
  nand3  g16(.a(new_n23_), .b(new_n35_), .c(new_n21_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n34_), .c(new_n32_), .O(new_n37_));
  inv1   g18(.a(new_n37_), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  nand2  g20(.a(new_n31_), .b(new_n39_), .O(new_n40_));
  oai21  g21(.a(new_n38_), .b(new_n31_), .c(new_n40_), .O(z1));
  inv1   g22(.a(x11), .O(new_n42_));
  nand3  g23(.a(new_n42_), .b(new_n35_), .c(new_n21_), .O(new_n43_));
  aoi21  g24(.a(new_n43_), .b(new_n24_), .c(new_n22_), .O(new_n44_));
  nand4  g25(.a(new_n23_), .b(new_n24_), .c(new_n35_), .d(new_n21_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(x11), .c(new_n44_), .O(new_n46_));
  inv1   g27(.a(x06), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n20_), .c(new_n28_), .O(new_n48_));
  oai21  g29(.a(new_n46_), .b(new_n20_), .c(new_n48_), .O(z2));
  nor2   g30(.a(x05), .b(new_n39_), .O(new_n50_));
  inv1   g31(.a(x12), .O(new_n51_));
  nor3   g32(.a(x11), .b(x10), .c(x08), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n33_), .c(new_n23_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand4  g35(.a(new_n24_), .b(x12), .c(x04), .d(x02), .O(new_n55_));
  inv1   g36(.a(new_n55_), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n52_), .c(new_n20_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  inv1   g39(.a(x07), .O(new_n59_));
  aoi21  g40(.a(new_n59_), .b(new_n20_), .c(new_n28_), .O(new_n60_));
  aoi21  g41(.a(new_n60_), .b(new_n58_), .c(new_n50_), .O(z3));
  inv1   g42(.a(x04), .O(new_n62_));
  oai22  g43(.a(new_n24_), .b(new_n62_), .c(x05), .d(new_n39_), .O(z4));
endmodule


