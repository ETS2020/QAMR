// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_;
  inv1   g00(.a(x08), .O(new_n20_));
  inv1   g01(.a(x04), .O(new_n21_));
  aoi21  g02(.a(x13), .b(x09), .c(new_n21_), .O(new_n22_));
  nor2   g03(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nand2  g04(.a(new_n20_), .b(x04), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x03), .O(new_n25_));
  inv1   g06(.a(x02), .O(new_n26_));
  inv1   g07(.a(x03), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(x00), .c(new_n26_), .O(new_n28_));
  oai21  g09(.a(new_n25_), .b(new_n23_), .c(new_n28_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x05), .O(z0));
  and2   g11(.a(x13), .b(x09), .O(new_n31_));
  oai21  g12(.a(new_n24_), .b(new_n31_), .c(x10), .O(new_n32_));
  nor2   g13(.a(x10), .b(x08), .O(new_n33_));
  oai21  g14(.a(new_n33_), .b(new_n31_), .c(x04), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(new_n32_), .c(x03), .O(new_n35_));
  aoi21  g16(.a(new_n27_), .b(x01), .c(new_n26_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x05), .O(z1));
  nor2   g19(.a(x06), .b(x03), .O(new_n39_));
  inv1   g20(.a(x11), .O(new_n40_));
  nand2  g21(.a(new_n34_), .b(new_n40_), .O(new_n41_));
  inv1   g22(.a(x10), .O(new_n42_));
  nand3  g23(.a(x11), .b(new_n42_), .c(new_n20_), .O(new_n43_));
  inv1   g24(.a(new_n43_), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n22_), .c(new_n27_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n41_), .c(new_n39_), .O(new_n46_));
  oai21  g27(.a(new_n46_), .b(new_n26_), .c(x05), .O(z2));
  inv1   g28(.a(new_n31_), .O(new_n48_));
  inv1   g29(.a(x12), .O(new_n49_));
  nand3  g30(.a(new_n33_), .b(new_n49_), .c(new_n40_), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n48_), .c(new_n21_), .O(new_n51_));
  nand3  g32(.a(new_n33_), .b(new_n40_), .c(x04), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(x12), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x03), .O(new_n54_));
  aoi21  g35(.a(x07), .b(new_n27_), .c(new_n26_), .O(new_n55_));
  oai21  g36(.a(new_n54_), .b(new_n51_), .c(new_n55_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x05), .O(z3));
  nand2  g38(.a(new_n31_), .b(x04), .O(new_n58_));
  aoi21  g39(.a(x05), .b(new_n26_), .c(new_n58_), .O(z4));
endmodule


