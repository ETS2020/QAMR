// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_;
  nand4  g00(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand3  g02(.a(new_n21_), .b(x04), .c(x02), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  nand4  g05(.a(new_n24_), .b(new_n22_), .c(new_n20_), .d(x10), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x03), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x03), .O(new_n28_));
  inv1   g09(.a(x05), .O(new_n29_));
  aoi21  g10(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n26_), .O(z0));
  nor2   g12(.a(x10), .b(new_n28_), .O(new_n32_));
  nand2  g13(.a(x13), .b(x09), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(x03), .O(new_n34_));
  nand2  g15(.a(new_n28_), .b(x01), .O(new_n35_));
  oai21  g16(.a(new_n34_), .b(new_n22_), .c(new_n35_), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(x05), .c(new_n32_), .O(z1));
  inv1   g18(.a(x11), .O(new_n38_));
  nand3  g19(.a(new_n20_), .b(new_n38_), .c(x03), .O(new_n39_));
  nand2  g20(.a(x06), .b(new_n28_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(x05), .c(new_n32_), .O(z2));
  nand2  g23(.a(new_n20_), .b(x10), .O(new_n43_));
  oai21  g24(.a(new_n43_), .b(x12), .c(x03), .O(new_n44_));
  inv1   g25(.a(x07), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n28_), .c(new_n29_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n44_), .O(z3));
  inv1   g28(.a(x04), .O(new_n48_));
  nor3   g29(.a(new_n32_), .b(new_n33_), .c(new_n48_), .O(z4));
endmodule


