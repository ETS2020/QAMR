// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand3  g01(.a(x13), .b(x09), .c(x03), .O(new_n21_));
  nor2   g02(.a(x09), .b(x07), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  nand3  g05(.a(new_n20_), .b(x04), .c(x02), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  oai21  g07(.a(x03), .b(x00), .c(x05), .O(new_n27_));
  aoi21  g08(.a(new_n26_), .b(x03), .c(new_n27_), .O(new_n28_));
  oai22  g09(.a(new_n28_), .b(new_n22_), .c(new_n21_), .d(new_n20_), .O(z0));
  inv1   g10(.a(new_n21_), .O(new_n30_));
  inv1   g11(.a(new_n23_), .O(new_n31_));
  oai21  g12(.a(new_n31_), .b(x10), .c(new_n30_), .O(new_n32_));
  inv1   g13(.a(new_n22_), .O(new_n33_));
  inv1   g14(.a(x03), .O(new_n34_));
  nand2  g15(.a(new_n25_), .b(x10), .O(new_n35_));
  inv1   g16(.a(x10), .O(new_n36_));
  nand4  g17(.a(new_n36_), .b(new_n20_), .c(x04), .d(x02), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  oai21  g19(.a(x03), .b(x01), .c(x05), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n38_), .c(new_n33_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n32_), .O(z1));
  nand2  g22(.a(new_n37_), .b(x11), .O(new_n42_));
  nor2   g23(.a(x11), .b(x10), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(new_n31_), .c(new_n20_), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n42_), .c(new_n34_), .O(new_n45_));
  oai21  g26(.a(x06), .b(x03), .c(x05), .O(new_n46_));
  oai21  g27(.a(new_n46_), .b(new_n45_), .c(new_n33_), .O(new_n47_));
  oai21  g28(.a(new_n31_), .b(x11), .c(new_n30_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n47_), .O(z2));
  inv1   g30(.a(x12), .O(new_n50_));
  nand2  g31(.a(new_n44_), .b(new_n50_), .O(new_n51_));
  nand4  g32(.a(new_n43_), .b(new_n31_), .c(x12), .d(new_n20_), .O(new_n52_));
  nand3  g33(.a(new_n52_), .b(new_n51_), .c(x03), .O(new_n53_));
  inv1   g34(.a(x05), .O(new_n54_));
  nor2   g35(.a(x12), .b(x02), .O(new_n55_));
  nand3  g36(.a(x13), .b(x04), .c(x03), .O(new_n56_));
  oai22  g37(.a(new_n56_), .b(new_n55_), .c(x07), .d(x03), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(x09), .c(new_n54_), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(new_n53_), .c(new_n22_), .O(z3));
  nand3  g40(.a(x13), .b(x09), .c(x04), .O(new_n60_));
  inv1   g41(.a(new_n60_), .O(z4));
endmodule


