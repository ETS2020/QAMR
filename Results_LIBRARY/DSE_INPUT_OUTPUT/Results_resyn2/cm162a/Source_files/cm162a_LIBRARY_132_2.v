// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n62_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  aoi21  g02(.a(x13), .b(x09), .c(new_n21_), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nor2   g04(.a(new_n21_), .b(x08), .O(new_n24_));
  aoi21  g05(.a(new_n23_), .b(x08), .c(new_n24_), .O(new_n25_));
  inv1   g06(.a(x00), .O(new_n26_));
  nand2  g07(.a(x07), .b(x05), .O(new_n27_));
  aoi21  g08(.a(new_n20_), .b(new_n26_), .c(new_n27_), .O(new_n28_));
  oai21  g09(.a(new_n25_), .b(new_n20_), .c(new_n28_), .O(z0));
  inv1   g10(.a(x10), .O(new_n30_));
  nor2   g11(.a(new_n24_), .b(new_n30_), .O(new_n31_));
  nand2  g12(.a(x13), .b(x09), .O(new_n32_));
  inv1   g13(.a(new_n32_), .O(new_n33_));
  inv1   g14(.a(new_n21_), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  inv1   g16(.a(x08), .O(new_n36_));
  nand4  g17(.a(new_n30_), .b(new_n36_), .c(x04), .d(x02), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n31_), .c(x03), .O(new_n39_));
  inv1   g20(.a(x01), .O(new_n40_));
  aoi21  g21(.a(new_n20_), .b(new_n40_), .c(new_n27_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n39_), .O(z1));
  inv1   g23(.a(x11), .O(new_n43_));
  nand4  g24(.a(new_n34_), .b(new_n43_), .c(new_n30_), .d(new_n36_), .O(new_n44_));
  nand2  g25(.a(new_n37_), .b(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n44_), .c(new_n35_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(x03), .O(new_n47_));
  inv1   g28(.a(x06), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n20_), .c(new_n27_), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n47_), .O(z2));
  inv1   g31(.a(new_n27_), .O(new_n51_));
  nand3  g32(.a(x13), .b(x09), .c(x07), .O(new_n52_));
  nand3  g33(.a(new_n43_), .b(new_n30_), .c(new_n36_), .O(new_n53_));
  inv1   g34(.a(x12), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(x04), .c(x02), .O(new_n55_));
  aoi21  g36(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(new_n56_));
  nor3   g37(.a(x11), .b(x10), .c(x08), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n22_), .c(new_n54_), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n56_), .c(x03), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n51_), .O(z3));
  inv1   g41(.a(x07), .O(new_n61_));
  nand2  g42(.a(new_n33_), .b(x04), .O(new_n62_));
  aoi21  g43(.a(new_n61_), .b(x05), .c(new_n62_), .O(z4));
endmodule


