// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_;
  and2   g00(.a(x13), .b(x09), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x04), .O(new_n21_));
  inv1   g02(.a(x04), .O(new_n22_));
  nand2  g03(.a(x08), .b(new_n22_), .O(new_n23_));
  inv1   g04(.a(x08), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x04), .O(new_n25_));
  nand4  g06(.a(new_n25_), .b(new_n23_), .c(new_n21_), .d(x03), .O(new_n26_));
  inv1   g07(.a(x02), .O(new_n27_));
  inv1   g08(.a(x03), .O(new_n28_));
  aoi21  g09(.a(new_n28_), .b(x00), .c(new_n27_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x05), .O(z0));
  nand2  g12(.a(new_n25_), .b(x10), .O(new_n32_));
  inv1   g13(.a(x10), .O(new_n33_));
  inv1   g14(.a(new_n25_), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(new_n32_), .c(new_n21_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x03), .O(new_n37_));
  inv1   g18(.a(x01), .O(new_n38_));
  nand2  g19(.a(x05), .b(x02), .O(new_n39_));
  aoi21  g20(.a(new_n28_), .b(new_n38_), .c(new_n39_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n37_), .O(z1));
  nand2  g22(.a(x13), .b(x09), .O(new_n42_));
  nand4  g23(.a(new_n42_), .b(new_n33_), .c(new_n24_), .d(x04), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x11), .O(new_n44_));
  nor3   g25(.a(x11), .b(x10), .c(x08), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n20_), .c(x04), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n44_), .c(x03), .O(new_n47_));
  aoi21  g28(.a(x06), .b(new_n28_), .c(new_n27_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(x05), .O(z2));
  nand2  g31(.a(new_n20_), .b(x02), .O(new_n51_));
  inv1   g32(.a(x12), .O(new_n52_));
  nor2   g33(.a(x11), .b(x10), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n52_), .c(new_n24_), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n51_), .c(new_n22_), .O(new_n55_));
  aoi21  g36(.a(new_n53_), .b(new_n34_), .c(new_n52_), .O(new_n56_));
  oai21  g37(.a(new_n56_), .b(new_n55_), .c(x03), .O(new_n57_));
  inv1   g38(.a(x07), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(new_n28_), .c(new_n39_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n57_), .O(z3));
  aoi21  g41(.a(x05), .b(new_n27_), .c(new_n21_), .O(z4));
endmodule


