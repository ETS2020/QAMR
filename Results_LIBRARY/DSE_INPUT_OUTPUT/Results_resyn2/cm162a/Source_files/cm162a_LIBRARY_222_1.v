// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_;
  inv1   g00(.a(x00), .O(new_n20_));
  nor2   g01(.a(x03), .b(new_n20_), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand3  g03(.a(new_n22_), .b(x04), .c(x02), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(x08), .c(x03), .O(new_n25_));
  aoi21  g06(.a(new_n23_), .b(x08), .c(new_n25_), .O(new_n26_));
  oai21  g07(.a(new_n26_), .b(new_n21_), .c(x05), .O(z0));
  inv1   g08(.a(x05), .O(new_n28_));
  inv1   g09(.a(x12), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  oai21  g11(.a(new_n23_), .b(x08), .c(x10), .O(new_n31_));
  inv1   g12(.a(new_n24_), .O(new_n32_));
  inv1   g13(.a(x08), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n22_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n32_), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(new_n31_), .c(x03), .O(new_n38_));
  inv1   g19(.a(x03), .O(new_n39_));
  aoi21  g20(.a(new_n39_), .b(x01), .c(new_n28_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n30_), .O(z1));
  oai21  g23(.a(new_n35_), .b(new_n23_), .c(x11), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n34_), .c(new_n33_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n22_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n32_), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n43_), .c(x03), .O(new_n48_));
  aoi21  g29(.a(x06), .b(new_n39_), .c(new_n28_), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n30_), .O(z2));
  aoi21  g32(.a(new_n45_), .b(new_n22_), .c(new_n24_), .O(new_n52_));
  aoi21  g33(.a(new_n45_), .b(new_n28_), .c(x12), .O(new_n53_));
  nor2   g34(.a(x10), .b(x08), .O(new_n54_));
  nand4  g35(.a(new_n54_), .b(new_n22_), .c(new_n32_), .d(new_n44_), .O(new_n55_));
  nor2   g36(.a(new_n29_), .b(new_n28_), .O(new_n56_));
  aoi22  g37(.a(new_n56_), .b(new_n55_), .c(new_n53_), .d(new_n52_), .O(new_n57_));
  oai21  g38(.a(x07), .b(x03), .c(x05), .O(new_n58_));
  oai21  g39(.a(new_n29_), .b(x05), .c(new_n58_), .O(new_n59_));
  oai21  g40(.a(new_n57_), .b(new_n39_), .c(new_n59_), .O(z3));
  inv1   g41(.a(x04), .O(new_n61_));
  oai22  g42(.a(new_n22_), .b(new_n61_), .c(new_n29_), .d(x05), .O(z4));
endmodule


