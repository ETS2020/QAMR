// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_;
  inv1   g00(.a(x03), .O(new_n20_));
  and2   g01(.a(x04), .b(x02), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  aoi21  g04(.a(new_n23_), .b(new_n21_), .c(new_n20_), .O(new_n24_));
  nand2  g05(.a(x04), .b(x02), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x08), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  aoi21  g08(.a(new_n20_), .b(x00), .c(x10), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x05), .O(z0));
  inv1   g11(.a(x01), .O(new_n31_));
  nor2   g12(.a(x03), .b(new_n31_), .O(new_n32_));
  inv1   g13(.a(x05), .O(new_n33_));
  nor2   g14(.a(x10), .b(new_n33_), .O(new_n34_));
  oai21  g15(.a(new_n32_), .b(new_n24_), .c(new_n34_), .O(z1));
  and2   g16(.a(x13), .b(x09), .O(new_n36_));
  nor2   g17(.a(x11), .b(x08), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n36_), .c(new_n21_), .O(new_n38_));
  inv1   g19(.a(new_n38_), .O(new_n39_));
  inv1   g20(.a(x11), .O(new_n40_));
  nor2   g21(.a(new_n25_), .b(x08), .O(new_n41_));
  oai21  g22(.a(new_n41_), .b(new_n40_), .c(x03), .O(new_n42_));
  aoi21  g23(.a(x06), .b(new_n20_), .c(x10), .O(new_n43_));
  oai21  g24(.a(new_n42_), .b(new_n39_), .c(new_n43_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(x05), .O(z2));
  inv1   g26(.a(x12), .O(new_n46_));
  nand2  g27(.a(new_n38_), .b(new_n46_), .O(new_n47_));
  nand4  g28(.a(new_n41_), .b(new_n22_), .c(x12), .d(new_n40_), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n47_), .c(new_n20_), .O(new_n49_));
  inv1   g30(.a(x07), .O(new_n50_));
  inv1   g31(.a(x10), .O(new_n51_));
  oai21  g32(.a(new_n50_), .b(x03), .c(new_n51_), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n49_), .c(x05), .O(z3));
  nand2  g34(.a(new_n36_), .b(x04), .O(new_n54_));
  aoi21  g35(.a(x10), .b(x05), .c(new_n54_), .O(z4));
endmodule


