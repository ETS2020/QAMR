// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g05(.a(x03), .O(new_n25_));
  aoi21  g06(.a(new_n21_), .b(new_n20_), .c(new_n25_), .O(new_n26_));
  oai21  g07(.a(new_n24_), .b(new_n20_), .c(new_n26_), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x05), .O(new_n29_));
  aoi21  g10(.a(new_n25_), .b(new_n28_), .c(new_n29_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n27_), .O(z0));
  oai21  g12(.a(x10), .b(x08), .c(new_n23_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n22_), .O(new_n33_));
  nand3  g14(.a(new_n23_), .b(new_n20_), .c(x02), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(x10), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n33_), .c(new_n29_), .O(new_n36_));
  inv1   g17(.a(x10), .O(new_n37_));
  nor2   g18(.a(new_n37_), .b(x04), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n36_), .c(x03), .O(new_n39_));
  inv1   g20(.a(x01), .O(new_n40_));
  nor2   g21(.a(x05), .b(x04), .O(new_n41_));
  nor2   g22(.a(new_n29_), .b(x03), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n40_), .c(new_n41_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n39_), .O(z1));
  aoi21  g25(.a(new_n32_), .b(new_n22_), .c(x11), .O(new_n45_));
  nor2   g26(.a(x10), .b(x08), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(x11), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n24_), .c(x03), .O(new_n48_));
  inv1   g29(.a(x06), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n25_), .c(new_n29_), .O(new_n50_));
  oai21  g31(.a(new_n48_), .b(new_n45_), .c(new_n50_), .O(z2));
  nor2   g32(.a(x12), .b(x11), .O(new_n52_));
  aoi22  g33(.a(new_n52_), .b(new_n46_), .c(x13), .d(x09), .O(new_n53_));
  inv1   g34(.a(x11), .O(new_n54_));
  nand4  g35(.a(new_n54_), .b(new_n37_), .c(new_n20_), .d(x02), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x12), .O(new_n56_));
  oai21  g37(.a(new_n53_), .b(new_n21_), .c(new_n56_), .O(new_n57_));
  inv1   g38(.a(x12), .O(new_n58_));
  nor2   g39(.a(new_n58_), .b(x04), .O(new_n59_));
  aoi21  g40(.a(new_n57_), .b(x05), .c(new_n59_), .O(new_n60_));
  inv1   g41(.a(x07), .O(new_n61_));
  aoi21  g42(.a(new_n42_), .b(new_n61_), .c(new_n41_), .O(new_n62_));
  oai21  g43(.a(new_n60_), .b(new_n25_), .c(new_n62_), .O(z3));
  inv1   g44(.a(x04), .O(new_n64_));
  aoi21  g45(.a(new_n23_), .b(x05), .c(new_n64_), .O(z4));
endmodule


