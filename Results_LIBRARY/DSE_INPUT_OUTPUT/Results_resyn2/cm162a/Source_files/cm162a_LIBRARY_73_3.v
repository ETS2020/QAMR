// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_;
  nand2  g00(.a(x09), .b(x08), .O(new_n20_));
  oai21  g01(.a(x03), .b(x00), .c(x05), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x04), .c(x02), .O(new_n24_));
  inv1   g05(.a(x09), .O(new_n25_));
  nand2  g06(.a(x04), .b(x02), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(new_n25_), .c(x08), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(new_n24_), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x03), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n22_), .O(z0));
  nand2  g11(.a(new_n20_), .b(x05), .O(new_n31_));
  inv1   g12(.a(new_n31_), .O(new_n32_));
  inv1   g13(.a(x03), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(x01), .O(new_n34_));
  nand2  g15(.a(new_n24_), .b(x10), .O(new_n35_));
  inv1   g16(.a(x04), .O(new_n36_));
  nor2   g17(.a(x08), .b(new_n36_), .O(new_n37_));
  nand2  g18(.a(x13), .b(x09), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x10), .O(new_n39_));
  nand3  g20(.a(new_n39_), .b(new_n37_), .c(x02), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(new_n35_), .c(x03), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n34_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n32_), .O(z1));
  nor2   g24(.a(x11), .b(x10), .O(new_n44_));
  nand4  g25(.a(new_n44_), .b(new_n23_), .c(x04), .d(x02), .O(new_n45_));
  nand2  g26(.a(new_n23_), .b(x04), .O(new_n46_));
  inv1   g27(.a(x10), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(x02), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n46_), .c(x11), .O(new_n49_));
  inv1   g30(.a(new_n38_), .O(new_n50_));
  nand3  g31(.a(new_n50_), .b(x04), .c(x02), .O(new_n51_));
  nand3  g32(.a(new_n51_), .b(new_n49_), .c(new_n45_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(x03), .O(new_n53_));
  nor2   g34(.a(x06), .b(x03), .O(new_n54_));
  nor2   g35(.a(new_n54_), .b(new_n31_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n53_), .O(z2));
  inv1   g37(.a(x11), .O(new_n57_));
  inv1   g38(.a(x12), .O(new_n58_));
  nand3  g39(.a(new_n58_), .b(new_n57_), .c(new_n47_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n38_), .O(new_n60_));
  nor2   g41(.a(x12), .b(x02), .O(new_n61_));
  nor2   g42(.a(new_n61_), .b(new_n46_), .O(new_n62_));
  aoi22  g43(.a(new_n62_), .b(new_n60_), .c(new_n45_), .d(x12), .O(new_n63_));
  nor2   g44(.a(x07), .b(x03), .O(new_n64_));
  nor2   g45(.a(new_n64_), .b(new_n31_), .O(new_n65_));
  oai21  g46(.a(new_n63_), .b(new_n33_), .c(new_n65_), .O(z3));
  nand2  g47(.a(new_n50_), .b(new_n37_), .O(new_n67_));
  inv1   g48(.a(new_n67_), .O(z4));
endmodule


