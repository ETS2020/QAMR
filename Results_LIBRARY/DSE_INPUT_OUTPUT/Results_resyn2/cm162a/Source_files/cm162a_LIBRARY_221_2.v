// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_, new_n60_;
  nand2  g00(.a(x04), .b(x02), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x08), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  inv1   g03(.a(new_n20_), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand3  g05(.a(x09), .b(x04), .c(x02), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(new_n24_), .c(new_n21_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  nand2  g10(.a(x13), .b(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n27_), .O(z0));
  oai21  g13(.a(new_n20_), .b(x08), .c(x10), .O(new_n33_));
  and2   g14(.a(new_n25_), .b(x03), .O(new_n34_));
  inv1   g15(.a(x10), .O(new_n35_));
  nand4  g16(.a(new_n35_), .b(new_n22_), .c(x04), .d(x02), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n34_), .c(new_n33_), .O(new_n37_));
  inv1   g18(.a(x13), .O(new_n38_));
  aoi21  g19(.a(new_n29_), .b(x01), .c(new_n38_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x05), .O(z1));
  inv1   g22(.a(x11), .O(new_n42_));
  nor2   g23(.a(x10), .b(x08), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(new_n23_), .c(new_n42_), .O(new_n44_));
  nand2  g25(.a(new_n36_), .b(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n44_), .c(new_n34_), .O(new_n46_));
  aoi21  g27(.a(x06), .b(new_n29_), .c(new_n38_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x05), .O(z2));
  nand3  g30(.a(new_n42_), .b(new_n35_), .c(new_n22_), .O(new_n50_));
  nand2  g31(.a(x13), .b(x09), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n23_), .c(x12), .O(new_n53_));
  nand2  g34(.a(new_n51_), .b(x12), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n44_), .c(x03), .O(new_n55_));
  inv1   g36(.a(x07), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n29_), .c(new_n30_), .O(new_n57_));
  oai21  g38(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(z3));
  inv1   g39(.a(x04), .O(new_n59_));
  inv1   g40(.a(x05), .O(new_n60_));
  oai22  g41(.a(new_n51_), .b(new_n59_), .c(x13), .d(new_n60_), .O(z4));
endmodule


