// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_;
  inv1   g00(.a(x02), .O(new_n20_));
  nor2   g01(.a(x08), .b(new_n20_), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  and2   g03(.a(x13), .b(x09), .O(new_n23_));
  inv1   g04(.a(new_n23_), .O(new_n24_));
  aoi21  g05(.a(new_n24_), .b(x02), .c(new_n22_), .O(new_n25_));
  oai21  g06(.a(new_n25_), .b(new_n21_), .c(x03), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x03), .O(new_n28_));
  nand2  g09(.a(x05), .b(x04), .O(new_n29_));
  aoi21  g10(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n26_), .O(z0));
  nor2   g12(.a(x10), .b(x08), .O(new_n32_));
  oai21  g13(.a(new_n32_), .b(new_n23_), .c(x02), .O(new_n33_));
  inv1   g14(.a(new_n33_), .O(new_n34_));
  nand2  g15(.a(new_n24_), .b(new_n21_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(x10), .c(new_n34_), .O(new_n36_));
  inv1   g17(.a(x01), .O(new_n37_));
  aoi21  g18(.a(new_n28_), .b(new_n37_), .c(new_n29_), .O(new_n38_));
  oai21  g19(.a(new_n36_), .b(new_n28_), .c(new_n38_), .O(z1));
  inv1   g20(.a(x11), .O(new_n40_));
  nand2  g21(.a(new_n33_), .b(new_n40_), .O(new_n41_));
  nand4  g22(.a(new_n32_), .b(new_n24_), .c(x11), .d(x02), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n41_), .c(new_n28_), .O(new_n43_));
  inv1   g24(.a(x06), .O(new_n44_));
  oai21  g25(.a(new_n44_), .b(x03), .c(x04), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n43_), .c(x05), .O(z2));
  nand3  g27(.a(new_n23_), .b(x04), .c(x02), .O(new_n47_));
  inv1   g28(.a(new_n47_), .O(new_n48_));
  nand3  g29(.a(new_n32_), .b(new_n40_), .c(x02), .O(new_n49_));
  xnor2a g30(.a(new_n49_), .b(x12), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n48_), .c(x03), .O(new_n51_));
  inv1   g32(.a(x07), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n28_), .c(new_n29_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(z3));
  nand2  g35(.a(new_n23_), .b(x04), .O(new_n55_));
  inv1   g36(.a(new_n55_), .O(z4));
endmodule


