// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n62_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  xor2a  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  nand3  g03(.a(x09), .b(x08), .c(x03), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x07), .O(new_n24_));
  oai21  g05(.a(x03), .b(x00), .c(x05), .O(new_n25_));
  aoi21  g06(.a(new_n24_), .b(x13), .c(new_n25_), .O(new_n26_));
  oai21  g07(.a(new_n22_), .b(new_n20_), .c(new_n26_), .O(z0));
  inv1   g08(.a(x07), .O(new_n28_));
  nand2  g09(.a(x13), .b(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n21_), .b(x08), .c(x10), .O(new_n30_));
  inv1   g11(.a(x08), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand4  g13(.a(new_n32_), .b(new_n31_), .c(x04), .d(x02), .O(new_n33_));
  aoi21  g14(.a(new_n33_), .b(new_n30_), .c(new_n20_), .O(new_n34_));
  oai21  g15(.a(x03), .b(x01), .c(x05), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n34_), .c(new_n29_), .O(new_n36_));
  inv1   g17(.a(new_n21_), .O(new_n37_));
  nand3  g18(.a(x13), .b(x09), .c(x07), .O(new_n38_));
  inv1   g19(.a(new_n38_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x03), .O(new_n40_));
  inv1   g21(.a(new_n40_), .O(new_n41_));
  oai21  g22(.a(new_n37_), .b(x10), .c(new_n41_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n36_), .O(z1));
  inv1   g24(.a(new_n29_), .O(new_n44_));
  inv1   g25(.a(x11), .O(new_n45_));
  xor2a  g26(.a(new_n33_), .b(new_n45_), .O(new_n46_));
  oai21  g27(.a(x06), .b(x03), .c(x05), .O(new_n47_));
  aoi21  g28(.a(new_n46_), .b(x03), .c(new_n47_), .O(new_n48_));
  oai21  g29(.a(new_n37_), .b(x11), .c(new_n41_), .O(new_n49_));
  oai21  g30(.a(new_n48_), .b(new_n44_), .c(new_n49_), .O(z2));
  nand4  g31(.a(new_n37_), .b(new_n45_), .c(new_n32_), .d(new_n31_), .O(new_n51_));
  nand2  g32(.a(x13), .b(x09), .O(new_n52_));
  inv1   g33(.a(x12), .O(new_n53_));
  nand4  g34(.a(new_n53_), .b(new_n45_), .c(new_n32_), .d(new_n31_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  aoi22  g36(.a(new_n55_), .b(new_n37_), .c(new_n51_), .d(x12), .O(new_n56_));
  inv1   g37(.a(x05), .O(new_n57_));
  inv1   g38(.a(x13), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(x03), .O(new_n59_));
  aoi21  g40(.a(new_n59_), .b(new_n28_), .c(new_n57_), .O(new_n60_));
  oai21  g41(.a(new_n56_), .b(new_n20_), .c(new_n60_), .O(z3));
  nand2  g42(.a(new_n39_), .b(x04), .O(new_n62_));
  inv1   g43(.a(new_n62_), .O(z4));
endmodule


