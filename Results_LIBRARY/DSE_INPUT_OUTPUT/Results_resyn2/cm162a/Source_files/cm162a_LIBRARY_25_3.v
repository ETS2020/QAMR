// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n62_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  aoi21  g02(.a(x13), .b(x09), .c(new_n21_), .O(new_n22_));
  nor2   g03(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nor2   g04(.a(new_n21_), .b(x08), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n23_), .c(x03), .O(new_n25_));
  inv1   g06(.a(x07), .O(new_n26_));
  inv1   g07(.a(x12), .O(new_n27_));
  nor2   g08(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g09(.a(x03), .b(x00), .c(x05), .O(new_n29_));
  nor2   g10(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n25_), .O(z0));
  inv1   g12(.a(x10), .O(new_n32_));
  nor2   g13(.a(new_n24_), .b(new_n32_), .O(new_n33_));
  nand2  g14(.a(x13), .b(x09), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  inv1   g16(.a(new_n35_), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n34_), .c(new_n21_), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n33_), .c(x03), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  inv1   g20(.a(x03), .O(new_n40_));
  inv1   g21(.a(x05), .O(new_n41_));
  aoi21  g22(.a(new_n40_), .b(new_n39_), .c(new_n41_), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n38_), .c(new_n28_), .O(z1));
  inv1   g24(.a(x11), .O(new_n44_));
  nand2  g25(.a(new_n35_), .b(new_n44_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n34_), .c(new_n21_), .O(new_n46_));
  aoi21  g27(.a(new_n35_), .b(new_n22_), .c(new_n44_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n46_), .c(x03), .O(new_n48_));
  inv1   g29(.a(x06), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n40_), .c(new_n41_), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n48_), .c(new_n28_), .O(z2));
  nand3  g32(.a(new_n27_), .b(x03), .c(x02), .O(new_n52_));
  aoi21  g33(.a(new_n45_), .b(new_n34_), .c(new_n52_), .O(new_n53_));
  nand4  g34(.a(x13), .b(x12), .c(x09), .d(new_n26_), .O(new_n54_));
  inv1   g35(.a(new_n54_), .O(new_n55_));
  oai21  g36(.a(new_n55_), .b(new_n53_), .c(x04), .O(new_n56_));
  nor2   g37(.a(new_n21_), .b(x07), .O(new_n57_));
  nand3  g38(.a(new_n57_), .b(new_n35_), .c(new_n44_), .O(new_n58_));
  oai21  g39(.a(x07), .b(x03), .c(x05), .O(new_n59_));
  aoi21  g40(.a(new_n58_), .b(x12), .c(new_n59_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n56_), .O(z3));
  inv1   g42(.a(x04), .O(new_n62_));
  nor3   g43(.a(new_n28_), .b(new_n34_), .c(new_n62_), .O(z4));
endmodule


