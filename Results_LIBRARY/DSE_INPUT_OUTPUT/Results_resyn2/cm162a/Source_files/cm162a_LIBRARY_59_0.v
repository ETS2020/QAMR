// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  inv1   g00(.a(x00), .O(new_n20_));
  nor2   g01(.a(x03), .b(new_n20_), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  inv1   g05(.a(new_n24_), .O(new_n25_));
  nor2   g06(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nor2   g07(.a(new_n26_), .b(new_n22_), .O(new_n27_));
  oai21  g08(.a(new_n23_), .b(x08), .c(x03), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n21_), .c(x05), .O(z0));
  inv1   g11(.a(x05), .O(new_n31_));
  and2   g12(.a(x04), .b(x02), .O(new_n32_));
  nor2   g13(.a(x10), .b(x08), .O(new_n33_));
  oai21  g14(.a(new_n33_), .b(new_n25_), .c(new_n32_), .O(new_n34_));
  nand2  g15(.a(new_n23_), .b(x10), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n34_), .c(new_n31_), .O(new_n36_));
  inv1   g17(.a(x10), .O(new_n37_));
  nor2   g18(.a(new_n37_), .b(new_n22_), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n36_), .c(x03), .O(new_n39_));
  inv1   g20(.a(x01), .O(new_n40_));
  nor2   g21(.a(new_n22_), .b(x05), .O(new_n41_));
  nor2   g22(.a(new_n31_), .b(x03), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n40_), .c(new_n41_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n39_), .O(z1));
  nand4  g25(.a(new_n26_), .b(x11), .c(new_n37_), .d(x03), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(x05), .O(new_n46_));
  inv1   g27(.a(x11), .O(new_n47_));
  nand2  g28(.a(new_n34_), .b(new_n47_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x03), .O(new_n49_));
  inv1   g30(.a(x03), .O(new_n50_));
  inv1   g31(.a(x06), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n50_), .c(new_n31_), .O(new_n52_));
  aoi22  g33(.a(new_n52_), .b(new_n49_), .c(new_n46_), .d(new_n22_), .O(z2));
  nor2   g34(.a(x12), .b(x11), .O(new_n54_));
  aoi22  g35(.a(new_n54_), .b(new_n33_), .c(x13), .d(x09), .O(new_n55_));
  nand4  g36(.a(new_n47_), .b(new_n37_), .c(x04), .d(x02), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x12), .O(new_n57_));
  oai21  g38(.a(new_n55_), .b(new_n23_), .c(new_n57_), .O(new_n58_));
  and2   g39(.a(x12), .b(x08), .O(new_n59_));
  aoi21  g40(.a(new_n58_), .b(x05), .c(new_n59_), .O(new_n60_));
  inv1   g41(.a(x07), .O(new_n61_));
  aoi21  g42(.a(new_n42_), .b(new_n61_), .c(new_n41_), .O(new_n62_));
  oai21  g43(.a(new_n60_), .b(new_n50_), .c(new_n62_), .O(z3));
  nand2  g44(.a(new_n25_), .b(x04), .O(new_n64_));
  aoi21  g45(.a(new_n22_), .b(new_n31_), .c(new_n64_), .O(z4));
endmodule


