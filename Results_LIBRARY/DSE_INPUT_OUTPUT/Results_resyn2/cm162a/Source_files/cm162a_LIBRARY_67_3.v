// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  xor2a  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  nor2   g03(.a(x03), .b(x00), .O(new_n23_));
  inv1   g04(.a(x09), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x05), .O(new_n25_));
  nor2   g06(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  oai21  g07(.a(new_n22_), .b(new_n20_), .c(new_n26_), .O(z0));
  inv1   g08(.a(x08), .O(new_n28_));
  nand3  g09(.a(new_n28_), .b(x04), .c(x02), .O(new_n29_));
  inv1   g10(.a(x10), .O(new_n30_));
  nand4  g11(.a(new_n30_), .b(new_n28_), .c(x04), .d(x02), .O(new_n31_));
  inv1   g12(.a(new_n31_), .O(new_n32_));
  aoi21  g13(.a(new_n29_), .b(x10), .c(new_n32_), .O(new_n33_));
  nor2   g14(.a(x03), .b(x01), .O(new_n34_));
  nor2   g15(.a(new_n34_), .b(new_n25_), .O(new_n35_));
  oai21  g16(.a(new_n33_), .b(new_n20_), .c(new_n35_), .O(z1));
  nand2  g17(.a(new_n31_), .b(x11), .O(new_n37_));
  inv1   g18(.a(x11), .O(new_n38_));
  nor2   g19(.a(x10), .b(x08), .O(new_n39_));
  nand4  g20(.a(new_n39_), .b(new_n38_), .c(x04), .d(x02), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(new_n37_), .c(x03), .O(new_n41_));
  aoi21  g22(.a(x06), .b(new_n20_), .c(x09), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x05), .O(z2));
  nand2  g25(.a(x13), .b(x09), .O(new_n45_));
  nor2   g26(.a(x12), .b(x11), .O(new_n46_));
  nand4  g27(.a(new_n46_), .b(new_n39_), .c(x03), .d(x02), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x04), .O(new_n49_));
  nand3  g30(.a(new_n40_), .b(x12), .c(x03), .O(new_n50_));
  nor2   g31(.a(x07), .b(x03), .O(new_n51_));
  nor2   g32(.a(new_n51_), .b(new_n25_), .O(new_n52_));
  nand3  g33(.a(new_n52_), .b(new_n50_), .c(new_n49_), .O(z3));
  aoi21  g34(.a(x13), .b(x04), .c(x05), .O(new_n54_));
  nor2   g35(.a(new_n54_), .b(new_n24_), .O(z4));
endmodule


