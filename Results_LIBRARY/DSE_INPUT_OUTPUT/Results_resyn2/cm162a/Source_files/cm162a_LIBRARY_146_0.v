// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_;
  nor2   g00(.a(x11), .b(x10), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  nor2   g02(.a(new_n21_), .b(x08), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  and2   g04(.a(x04), .b(x02), .O(new_n24_));
  nand2  g05(.a(x13), .b(x09), .O(new_n25_));
  aoi21  g06(.a(new_n25_), .b(new_n24_), .c(new_n23_), .O(new_n26_));
  oai21  g07(.a(new_n26_), .b(new_n22_), .c(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  aoi21  g12(.a(new_n31_), .b(new_n27_), .c(new_n20_), .O(z0));
  inv1   g13(.a(new_n20_), .O(new_n33_));
  oai21  g14(.a(x03), .b(x01), .c(x05), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand3  g16(.a(new_n25_), .b(new_n24_), .c(new_n23_), .O(new_n36_));
  oai21  g17(.a(x10), .b(x08), .c(new_n25_), .O(new_n37_));
  nand3  g18(.a(x11), .b(x04), .c(x02), .O(new_n38_));
  inv1   g19(.a(new_n38_), .O(new_n39_));
  aoi22  g20(.a(new_n39_), .b(new_n37_), .c(new_n36_), .d(x10), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n29_), .c(new_n35_), .O(z1));
  oai21  g22(.a(x06), .b(x03), .c(x05), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n33_), .O(new_n43_));
  inv1   g24(.a(x10), .O(new_n44_));
  nand4  g25(.a(new_n25_), .b(new_n24_), .c(new_n44_), .d(new_n23_), .O(new_n45_));
  nand4  g26(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n46_));
  nor2   g27(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  aoi21  g28(.a(new_n45_), .b(x11), .c(new_n47_), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n29_), .c(new_n43_), .O(z2));
  inv1   g30(.a(new_n46_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(x12), .c(x03), .O(new_n51_));
  nor2   g32(.a(x07), .b(x03), .O(new_n52_));
  nor3   g33(.a(new_n52_), .b(new_n20_), .c(new_n30_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(z3));
  inv1   g35(.a(x04), .O(new_n55_));
  oai21  g36(.a(new_n25_), .b(new_n55_), .c(new_n33_), .O(z4));
endmodule


