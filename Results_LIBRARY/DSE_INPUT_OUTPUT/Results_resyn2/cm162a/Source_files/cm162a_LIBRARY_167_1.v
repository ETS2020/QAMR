// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_;
  inv1   g00(.a(x05), .O(new_n20_));
  inv1   g01(.a(x11), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  inv1   g04(.a(new_n23_), .O(new_n24_));
  nand3  g05(.a(new_n24_), .b(x13), .c(x09), .O(new_n25_));
  nand2  g06(.a(new_n23_), .b(x08), .O(new_n26_));
  inv1   g07(.a(x08), .O(new_n27_));
  nand2  g08(.a(new_n24_), .b(new_n27_), .O(new_n28_));
  nand4  g09(.a(new_n28_), .b(new_n26_), .c(new_n25_), .d(x03), .O(new_n29_));
  inv1   g10(.a(x03), .O(new_n30_));
  aoi21  g11(.a(new_n30_), .b(x00), .c(new_n20_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n22_), .O(z0));
  oai21  g14(.a(new_n23_), .b(x08), .c(x10), .O(new_n34_));
  inv1   g15(.a(x10), .O(new_n35_));
  nand4  g16(.a(new_n35_), .b(new_n27_), .c(x04), .d(x02), .O(new_n36_));
  nand4  g17(.a(new_n36_), .b(new_n34_), .c(new_n25_), .d(x03), .O(new_n37_));
  aoi21  g18(.a(new_n30_), .b(x01), .c(new_n20_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n22_), .O(z1));
  nor3   g21(.a(x11), .b(x10), .c(x08), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n24_), .O(new_n42_));
  nand2  g23(.a(new_n36_), .b(x11), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(new_n42_), .c(new_n25_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g26(.a(x06), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n30_), .c(new_n20_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n45_), .O(z2));
  nand3  g29(.a(x13), .b(x09), .c(x05), .O(new_n49_));
  inv1   g30(.a(x12), .O(new_n50_));
  nand4  g31(.a(new_n50_), .b(new_n21_), .c(new_n35_), .d(new_n27_), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n49_), .c(new_n23_), .O(new_n52_));
  nand2  g33(.a(x12), .b(x05), .O(new_n53_));
  aoi21  g34(.a(new_n41_), .b(new_n24_), .c(new_n53_), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n52_), .c(x03), .O(new_n55_));
  oai21  g36(.a(x07), .b(x03), .c(x05), .O(new_n56_));
  oai21  g37(.a(new_n21_), .b(x05), .c(new_n56_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n55_), .O(z3));
  nand3  g39(.a(x13), .b(x09), .c(x04), .O(new_n59_));
  aoi21  g40(.a(x11), .b(new_n20_), .c(new_n59_), .O(z4));
endmodule


