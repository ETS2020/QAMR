// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_;
  nand2  g00(.a(x13), .b(x09), .O(new_n20_));
  nand3  g01(.a(new_n20_), .b(x04), .c(x02), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(x08), .c(x03), .O(new_n23_));
  aoi21  g04(.a(new_n21_), .b(x08), .c(new_n23_), .O(new_n24_));
  inv1   g05(.a(x00), .O(new_n25_));
  inv1   g06(.a(x11), .O(new_n26_));
  oai21  g07(.a(x03), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n24_), .c(x05), .O(z0));
  inv1   g09(.a(x08), .O(new_n29_));
  nand4  g10(.a(new_n20_), .b(new_n29_), .c(x04), .d(x02), .O(new_n30_));
  nand3  g11(.a(new_n30_), .b(x10), .c(x03), .O(new_n31_));
  or2    g12(.a(x03), .b(x01), .O(new_n32_));
  oai21  g13(.a(x10), .b(x08), .c(new_n20_), .O(new_n33_));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n34_));
  inv1   g15(.a(new_n34_), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n32_), .c(new_n31_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n26_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x05), .O(z1));
  nor2   g20(.a(x06), .b(x03), .O(new_n40_));
  nand2  g21(.a(new_n26_), .b(x05), .O(new_n41_));
  nor2   g22(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n36_), .O(z2));
  nand3  g24(.a(x13), .b(new_n26_), .c(x09), .O(new_n44_));
  inv1   g25(.a(x10), .O(new_n45_));
  inv1   g26(.a(x12), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n45_), .c(new_n29_), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n44_), .c(new_n22_), .O(new_n48_));
  nor2   g29(.a(new_n22_), .b(x08), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n45_), .c(new_n46_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n48_), .c(x03), .O(new_n51_));
  nor2   g32(.a(x07), .b(x03), .O(new_n52_));
  nor2   g33(.a(new_n52_), .b(new_n41_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(z3));
  nand3  g35(.a(x13), .b(x09), .c(x04), .O(new_n55_));
  aoi21  g36(.a(x11), .b(x05), .c(new_n55_), .O(z4));
endmodule


