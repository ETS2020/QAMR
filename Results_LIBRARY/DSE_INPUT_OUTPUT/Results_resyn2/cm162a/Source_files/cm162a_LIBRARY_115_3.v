// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n59_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  aoi21  g02(.a(x13), .b(x09), .c(new_n21_), .O(new_n22_));
  and2   g03(.a(x04), .b(x02), .O(new_n23_));
  inv1   g04(.a(x00), .O(new_n24_));
  nand3  g05(.a(x05), .b(x03), .c(new_n24_), .O(new_n25_));
  aoi21  g06(.a(new_n23_), .b(new_n20_), .c(new_n25_), .O(new_n26_));
  oai21  g07(.a(new_n22_), .b(new_n20_), .c(new_n26_), .O(z0));
  nand2  g08(.a(x13), .b(x09), .O(new_n28_));
  nand3  g09(.a(new_n28_), .b(new_n23_), .c(new_n20_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x10), .O(new_n30_));
  and2   g11(.a(x13), .b(x09), .O(new_n31_));
  nor2   g12(.a(x10), .b(x08), .O(new_n32_));
  oai21  g13(.a(new_n32_), .b(new_n31_), .c(new_n23_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(new_n30_), .c(x03), .O(new_n34_));
  inv1   g15(.a(x03), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(x01), .c(x00), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x05), .O(z1));
  nor2   g19(.a(x06), .b(x03), .O(new_n39_));
  inv1   g20(.a(x11), .O(new_n40_));
  nand2  g21(.a(new_n33_), .b(new_n40_), .O(new_n41_));
  inv1   g22(.a(x10), .O(new_n42_));
  nand3  g23(.a(x11), .b(new_n42_), .c(new_n20_), .O(new_n43_));
  inv1   g24(.a(new_n43_), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n22_), .c(new_n35_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n41_), .c(new_n39_), .O(new_n46_));
  oai21  g27(.a(new_n46_), .b(x00), .c(x05), .O(z2));
  nand2  g28(.a(new_n40_), .b(new_n42_), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n29_), .c(x12), .O(new_n49_));
  nor2   g30(.a(x12), .b(x11), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n32_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n28_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n23_), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n49_), .c(x03), .O(new_n54_));
  aoi21  g35(.a(x07), .b(new_n35_), .c(x00), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x05), .O(z3));
  inv1   g38(.a(x04), .O(new_n58_));
  nand2  g39(.a(x05), .b(x00), .O(new_n59_));
  oai21  g40(.a(new_n28_), .b(new_n58_), .c(new_n59_), .O(z4));
endmodule


