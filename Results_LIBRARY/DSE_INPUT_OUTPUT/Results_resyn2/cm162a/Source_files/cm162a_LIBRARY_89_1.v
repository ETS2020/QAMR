// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n62_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand3  g01(.a(x13), .b(x12), .c(x09), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x03), .O(new_n23_));
  inv1   g04(.a(x12), .O(new_n24_));
  nand2  g05(.a(x13), .b(new_n24_), .O(new_n25_));
  inv1   g06(.a(new_n25_), .O(new_n26_));
  nand2  g07(.a(x04), .b(x02), .O(new_n27_));
  xor2a  g08(.a(new_n27_), .b(new_n20_), .O(new_n28_));
  oai21  g09(.a(x03), .b(x00), .c(x05), .O(new_n29_));
  aoi21  g10(.a(new_n28_), .b(x03), .c(new_n29_), .O(new_n30_));
  oai22  g11(.a(new_n30_), .b(new_n26_), .c(new_n23_), .d(new_n20_), .O(z0));
  inv1   g12(.a(x03), .O(new_n32_));
  oai21  g13(.a(new_n27_), .b(x08), .c(x10), .O(new_n33_));
  nor2   g14(.a(x10), .b(x08), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n33_), .c(new_n32_), .O(new_n36_));
  oai21  g17(.a(x03), .b(x01), .c(x05), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n36_), .c(new_n25_), .O(new_n38_));
  inv1   g19(.a(new_n23_), .O(new_n39_));
  inv1   g20(.a(new_n27_), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(x10), .c(new_n39_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n38_), .O(z1));
  nand2  g23(.a(x13), .b(x09), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  nand2  g25(.a(new_n34_), .b(new_n44_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n43_), .c(new_n27_), .O(new_n46_));
  aoi21  g27(.a(new_n34_), .b(new_n40_), .c(new_n44_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n46_), .c(x03), .O(new_n48_));
  nand2  g29(.a(new_n25_), .b(x05), .O(new_n49_));
  nor2   g30(.a(x06), .b(x03), .O(new_n50_));
  nor2   g31(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n48_), .O(z2));
  inv1   g33(.a(x04), .O(new_n53_));
  nand4  g34(.a(new_n34_), .b(new_n24_), .c(new_n44_), .d(x02), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n21_), .c(new_n53_), .O(new_n55_));
  nor3   g36(.a(x11), .b(x10), .c(x08), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n40_), .c(new_n24_), .O(new_n57_));
  oai21  g38(.a(new_n57_), .b(new_n55_), .c(x03), .O(new_n58_));
  nor2   g39(.a(x07), .b(x03), .O(new_n59_));
  nor2   g40(.a(new_n59_), .b(new_n49_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n58_), .O(z3));
  nand2  g42(.a(new_n22_), .b(x04), .O(new_n62_));
  inv1   g43(.a(new_n62_), .O(z4));
endmodule


