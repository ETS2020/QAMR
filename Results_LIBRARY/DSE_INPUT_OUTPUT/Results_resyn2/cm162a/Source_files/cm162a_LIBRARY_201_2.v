// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_, new_n61_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  and2   g02(.a(x04), .b(x02), .O(new_n22_));
  inv1   g03(.a(x09), .O(new_n23_));
  aoi21  g04(.a(new_n22_), .b(new_n23_), .c(new_n21_), .O(new_n24_));
  aoi21  g05(.a(new_n22_), .b(new_n21_), .c(new_n24_), .O(new_n25_));
  inv1   g06(.a(x00), .O(new_n26_));
  nand2  g07(.a(x13), .b(x05), .O(new_n27_));
  aoi21  g08(.a(new_n20_), .b(new_n26_), .c(new_n27_), .O(new_n28_));
  oai21  g09(.a(new_n25_), .b(new_n20_), .c(new_n28_), .O(z0));
  inv1   g10(.a(x10), .O(new_n30_));
  aoi21  g11(.a(new_n22_), .b(new_n21_), .c(new_n30_), .O(new_n31_));
  nor2   g12(.a(x10), .b(x08), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n22_), .O(new_n33_));
  nand3  g14(.a(x09), .b(x04), .c(x02), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n31_), .c(x03), .O(new_n36_));
  inv1   g17(.a(x01), .O(new_n37_));
  aoi21  g18(.a(new_n20_), .b(new_n37_), .c(new_n27_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n36_), .O(z1));
  inv1   g20(.a(x11), .O(new_n40_));
  nand3  g21(.a(new_n32_), .b(new_n22_), .c(new_n40_), .O(new_n41_));
  nand2  g22(.a(new_n33_), .b(x11), .O(new_n42_));
  nand2  g23(.a(new_n22_), .b(x09), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g26(.a(x06), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n20_), .c(new_n27_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n45_), .O(z2));
  nand4  g29(.a(new_n32_), .b(new_n22_), .c(new_n40_), .d(new_n23_), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(x12), .O(new_n50_));
  inv1   g31(.a(x12), .O(new_n51_));
  nand4  g32(.a(new_n51_), .b(new_n40_), .c(new_n30_), .d(new_n21_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n23_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n22_), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n50_), .c(x03), .O(new_n55_));
  inv1   g36(.a(x13), .O(new_n56_));
  aoi21  g37(.a(x07), .b(new_n20_), .c(new_n56_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(x05), .O(z3));
  nand2  g40(.a(x09), .b(x04), .O(new_n60_));
  nor2   g41(.a(x13), .b(x05), .O(new_n61_));
  aoi21  g42(.a(new_n60_), .b(x13), .c(new_n61_), .O(z4));
endmodule


