// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_;
  and2   g00(.a(x13), .b(x09), .O(new_n20_));
  nand3  g01(.a(new_n20_), .b(x04), .c(x02), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  or2    g03(.a(new_n22_), .b(x08), .O(new_n23_));
  nand2  g04(.a(new_n22_), .b(x08), .O(new_n24_));
  nand3  g05(.a(new_n24_), .b(new_n23_), .c(new_n21_), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x03), .O(new_n26_));
  nor2   g07(.a(x03), .b(x00), .O(new_n27_));
  inv1   g08(.a(x13), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x12), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x05), .O(new_n30_));
  nor2   g11(.a(new_n30_), .b(new_n27_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n26_), .O(z0));
  nand2  g13(.a(new_n23_), .b(x10), .O(new_n33_));
  nor2   g14(.a(x10), .b(x08), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(x04), .c(x02), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(new_n33_), .c(new_n21_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x03), .O(new_n37_));
  nor2   g18(.a(x03), .b(x01), .O(new_n38_));
  nor2   g19(.a(new_n38_), .b(new_n30_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n37_), .O(z1));
  inv1   g21(.a(x03), .O(new_n41_));
  inv1   g22(.a(x11), .O(new_n42_));
  nand2  g23(.a(new_n22_), .b(new_n42_), .O(new_n43_));
  xor2a  g24(.a(new_n35_), .b(new_n42_), .O(new_n44_));
  aoi22  g25(.a(new_n44_), .b(new_n29_), .c(new_n43_), .d(new_n20_), .O(new_n45_));
  oai21  g26(.a(x06), .b(x03), .c(x05), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n29_), .O(new_n47_));
  oai21  g28(.a(new_n45_), .b(new_n41_), .c(new_n47_), .O(z2));
  inv1   g29(.a(x02), .O(new_n49_));
  nor2   g30(.a(x12), .b(x11), .O(new_n50_));
  aoi22  g31(.a(new_n50_), .b(new_n34_), .c(x13), .d(x09), .O(new_n51_));
  nand2  g32(.a(x12), .b(x09), .O(new_n52_));
  oai21  g33(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  inv1   g34(.a(x12), .O(new_n54_));
  inv1   g35(.a(new_n35_), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n42_), .c(new_n54_), .O(new_n56_));
  aoi21  g37(.a(new_n53_), .b(x04), .c(new_n56_), .O(new_n57_));
  nor2   g38(.a(x07), .b(x03), .O(new_n58_));
  nor2   g39(.a(new_n58_), .b(new_n30_), .O(new_n59_));
  oai21  g40(.a(new_n57_), .b(new_n41_), .c(new_n59_), .O(z3));
  nand2  g41(.a(new_n20_), .b(x04), .O(new_n61_));
  nand2  g42(.a(new_n29_), .b(new_n61_), .O(z4));
endmodule


