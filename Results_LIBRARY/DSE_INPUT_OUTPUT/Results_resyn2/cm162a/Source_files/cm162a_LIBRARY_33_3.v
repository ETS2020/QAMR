// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  nand2  g00(.a(x07), .b(x06), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  and2   g04(.a(x04), .b(x02), .O(new_n24_));
  aoi21  g05(.a(new_n24_), .b(new_n23_), .c(new_n22_), .O(new_n25_));
  nand2  g06(.a(x04), .b(x02), .O(new_n26_));
  nor2   g07(.a(new_n26_), .b(x08), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n25_), .c(x03), .O(new_n28_));
  inv1   g09(.a(x00), .O(new_n29_));
  inv1   g10(.a(x03), .O(new_n30_));
  inv1   g11(.a(x05), .O(new_n31_));
  aoi21  g12(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  aoi21  g13(.a(new_n32_), .b(new_n28_), .c(new_n21_), .O(z0));
  inv1   g14(.a(x10), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n22_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n23_), .c(new_n26_), .O(new_n36_));
  nand2  g17(.a(new_n27_), .b(new_n23_), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(x10), .c(new_n36_), .O(new_n38_));
  nand2  g19(.a(new_n20_), .b(x05), .O(new_n39_));
  nor2   g20(.a(x03), .b(x01), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  oai21  g22(.a(new_n38_), .b(new_n30_), .c(new_n41_), .O(z1));
  inv1   g23(.a(x11), .O(new_n43_));
  and2   g24(.a(x13), .b(x09), .O(new_n44_));
  nor2   g25(.a(x10), .b(x08), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n44_), .c(new_n24_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n43_), .O(new_n47_));
  aoi21  g28(.a(x13), .b(x09), .c(new_n26_), .O(new_n48_));
  nand3  g29(.a(x11), .b(new_n34_), .c(new_n22_), .O(new_n49_));
  inv1   g30(.a(new_n49_), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n48_), .c(new_n30_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  inv1   g33(.a(x06), .O(new_n53_));
  aoi21  g34(.a(new_n53_), .b(new_n30_), .c(new_n31_), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n52_), .c(new_n21_), .O(z2));
  nand2  g36(.a(new_n45_), .b(new_n43_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n23_), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n24_), .c(x12), .O(new_n58_));
  nand2  g39(.a(new_n48_), .b(x12), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n56_), .c(x03), .O(new_n60_));
  nor2   g41(.a(x07), .b(x03), .O(new_n61_));
  nor2   g42(.a(new_n61_), .b(new_n39_), .O(new_n62_));
  oai21  g43(.a(new_n60_), .b(new_n58_), .c(new_n62_), .O(z3));
  inv1   g44(.a(x04), .O(new_n64_));
  oai21  g45(.a(new_n23_), .b(new_n64_), .c(new_n20_), .O(z4));
endmodule


