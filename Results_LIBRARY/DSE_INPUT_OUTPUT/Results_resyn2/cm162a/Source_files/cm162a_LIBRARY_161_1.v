// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  aoi21  g02(.a(x13), .b(x09), .c(new_n21_), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nor2   g04(.a(new_n21_), .b(x08), .O(new_n24_));
  aoi21  g05(.a(new_n23_), .b(x08), .c(new_n24_), .O(new_n25_));
  nor2   g06(.a(x03), .b(x00), .O(new_n26_));
  inv1   g07(.a(x12), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(x10), .c(x05), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  oai21  g10(.a(new_n25_), .b(new_n20_), .c(new_n29_), .O(z0));
  nand2  g11(.a(x13), .b(x09), .O(new_n31_));
  inv1   g12(.a(x08), .O(new_n32_));
  inv1   g13(.a(x10), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(new_n31_), .c(new_n21_), .O(new_n35_));
  nand2  g16(.a(new_n24_), .b(new_n31_), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(x10), .c(new_n35_), .O(new_n37_));
  nor2   g18(.a(x03), .b(x01), .O(new_n38_));
  nor2   g19(.a(new_n38_), .b(new_n28_), .O(new_n39_));
  oai21  g20(.a(new_n37_), .b(new_n20_), .c(new_n39_), .O(z1));
  and2   g21(.a(x05), .b(x03), .O(new_n41_));
  inv1   g22(.a(x11), .O(new_n42_));
  nor2   g23(.a(new_n42_), .b(x08), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(new_n41_), .c(new_n22_), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n27_), .c(x10), .O(new_n45_));
  nand2  g26(.a(new_n42_), .b(x03), .O(new_n46_));
  nand2  g27(.a(x06), .b(new_n20_), .O(new_n47_));
  oai21  g28(.a(new_n46_), .b(new_n35_), .c(new_n47_), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(x05), .c(new_n45_), .O(z2));
  nand2  g30(.a(new_n31_), .b(x11), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n35_), .c(x12), .O(new_n51_));
  nor2   g32(.a(x07), .b(x03), .O(new_n52_));
  nor2   g33(.a(new_n52_), .b(new_n28_), .O(new_n53_));
  oai21  g34(.a(new_n51_), .b(new_n20_), .c(new_n53_), .O(z3));
  nand3  g35(.a(x13), .b(x09), .c(x04), .O(new_n55_));
  aoi21  g36(.a(x12), .b(new_n33_), .c(new_n55_), .O(z4));
endmodule


