// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand2  g02(.a(x09), .b(new_n21_), .O(new_n22_));
  oai21  g03(.a(x03), .b(x00), .c(x05), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  and2   g05(.a(x04), .b(x02), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  nand2  g07(.a(new_n25_), .b(new_n26_), .O(new_n27_));
  nor2   g08(.a(x09), .b(x08), .O(new_n28_));
  aoi22  g09(.a(new_n28_), .b(new_n25_), .c(new_n27_), .d(x08), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n20_), .c(new_n24_), .O(z0));
  nand2  g11(.a(x04), .b(x02), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n21_), .O(new_n33_));
  aoi21  g14(.a(new_n33_), .b(new_n26_), .c(new_n31_), .O(new_n34_));
  aoi21  g15(.a(new_n28_), .b(new_n25_), .c(new_n32_), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n34_), .c(x03), .O(new_n36_));
  inv1   g17(.a(x01), .O(new_n37_));
  nand2  g18(.a(new_n20_), .b(new_n37_), .O(new_n38_));
  nand4  g19(.a(new_n38_), .b(new_n36_), .c(new_n22_), .d(x05), .O(z1));
  inv1   g20(.a(x11), .O(new_n40_));
  inv1   g21(.a(x09), .O(new_n41_));
  oai21  g22(.a(new_n31_), .b(x10), .c(new_n41_), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n21_), .c(new_n40_), .O(new_n43_));
  nor2   g24(.a(x11), .b(x10), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n21_), .O(new_n45_));
  nand2  g26(.a(new_n25_), .b(new_n22_), .O(new_n46_));
  aoi21  g27(.a(new_n45_), .b(new_n26_), .c(new_n46_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n43_), .c(x03), .O(new_n48_));
  oai21  g29(.a(x06), .b(x03), .c(x05), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n22_), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n48_), .O(z2));
  oai21  g32(.a(x07), .b(x03), .c(x05), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n22_), .O(new_n53_));
  nand2  g34(.a(new_n45_), .b(new_n26_), .O(new_n54_));
  inv1   g35(.a(new_n46_), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n54_), .c(x12), .O(new_n56_));
  aoi21  g37(.a(new_n44_), .b(new_n25_), .c(x09), .O(new_n57_));
  nand2  g38(.a(x12), .b(new_n21_), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n57_), .c(x03), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n56_), .c(new_n53_), .O(z3));
  nand2  g41(.a(x13), .b(x04), .O(new_n61_));
  aoi21  g42(.a(new_n61_), .b(x08), .c(new_n41_), .O(z4));
endmodule


