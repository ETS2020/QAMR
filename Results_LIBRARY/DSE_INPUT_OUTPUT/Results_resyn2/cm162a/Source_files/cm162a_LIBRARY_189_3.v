// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  aoi21  g05(.a(new_n23_), .b(new_n24_), .c(new_n21_), .O(new_n25_));
  aoi21  g06(.a(new_n23_), .b(new_n21_), .c(new_n25_), .O(new_n26_));
  nor2   g07(.a(x03), .b(x00), .O(new_n27_));
  inv1   g08(.a(x11), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x05), .O(new_n29_));
  nor2   g10(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  oai21  g11(.a(new_n26_), .b(new_n20_), .c(new_n30_), .O(z0));
  oai21  g12(.a(x10), .b(x08), .c(new_n24_), .O(new_n32_));
  nand3  g13(.a(new_n23_), .b(new_n24_), .c(new_n21_), .O(new_n33_));
  aoi22  g14(.a(new_n33_), .b(x10), .c(new_n32_), .d(new_n23_), .O(new_n34_));
  nor2   g15(.a(x03), .b(x01), .O(new_n35_));
  nor2   g16(.a(new_n35_), .b(new_n29_), .O(new_n36_));
  oai21  g17(.a(new_n34_), .b(new_n20_), .c(new_n36_), .O(z1));
  aoi21  g18(.a(new_n32_), .b(new_n23_), .c(new_n20_), .O(new_n38_));
  inv1   g19(.a(x06), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(x03), .c(new_n28_), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n38_), .c(x05), .O(z2));
  nand2  g22(.a(new_n24_), .b(x12), .O(new_n42_));
  inv1   g23(.a(x02), .O(new_n43_));
  inv1   g24(.a(x04), .O(new_n44_));
  inv1   g25(.a(x12), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n43_), .c(new_n44_), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n42_), .c(new_n32_), .O(new_n47_));
  inv1   g28(.a(x10), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n21_), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n22_), .c(x12), .O(new_n50_));
  nand3  g31(.a(new_n50_), .b(new_n47_), .c(x03), .O(new_n51_));
  aoi21  g32(.a(x07), .b(new_n20_), .c(x11), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x05), .O(z3));
  nand3  g35(.a(x13), .b(x09), .c(x04), .O(new_n55_));
  aoi21  g36(.a(x11), .b(x05), .c(new_n55_), .O(z4));
endmodule


