// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  aoi21  g02(.a(new_n21_), .b(x02), .c(new_n20_), .O(new_n22_));
  inv1   g03(.a(x02), .O(new_n23_));
  oai21  g04(.a(x08), .b(new_n23_), .c(x03), .O(new_n24_));
  inv1   g05(.a(x03), .O(new_n25_));
  inv1   g06(.a(x04), .O(new_n26_));
  aoi21  g07(.a(new_n25_), .b(x00), .c(new_n26_), .O(new_n27_));
  oai21  g08(.a(new_n24_), .b(new_n22_), .c(new_n27_), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x05), .O(z0));
  oai21  g10(.a(x10), .b(x08), .c(new_n21_), .O(new_n30_));
  nand3  g11(.a(new_n21_), .b(new_n20_), .c(x02), .O(new_n31_));
  aoi22  g12(.a(new_n31_), .b(x10), .c(new_n30_), .d(x02), .O(new_n32_));
  inv1   g13(.a(x01), .O(new_n33_));
  nand2  g14(.a(x05), .b(x04), .O(new_n34_));
  aoi21  g15(.a(new_n25_), .b(new_n33_), .c(new_n34_), .O(new_n35_));
  oai21  g16(.a(new_n32_), .b(new_n25_), .c(new_n35_), .O(z1));
  aoi21  g17(.a(new_n30_), .b(x02), .c(x11), .O(new_n37_));
  nand2  g18(.a(new_n21_), .b(x02), .O(new_n38_));
  nor2   g19(.a(x10), .b(x08), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x11), .O(new_n40_));
  oai21  g21(.a(new_n40_), .b(new_n38_), .c(x03), .O(new_n41_));
  inv1   g22(.a(x06), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n25_), .c(new_n34_), .O(new_n43_));
  oai21  g24(.a(new_n41_), .b(new_n37_), .c(new_n43_), .O(z2));
  nand3  g25(.a(x13), .b(x09), .c(x04), .O(new_n45_));
  inv1   g26(.a(new_n45_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(x02), .O(new_n47_));
  inv1   g28(.a(x10), .O(new_n48_));
  inv1   g29(.a(x11), .O(new_n49_));
  nand4  g30(.a(new_n49_), .b(new_n48_), .c(new_n20_), .d(x02), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(x12), .O(new_n51_));
  inv1   g32(.a(x12), .O(new_n52_));
  nand4  g33(.a(new_n39_), .b(new_n52_), .c(new_n49_), .d(x02), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n51_), .c(new_n47_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(x03), .O(new_n55_));
  inv1   g36(.a(x07), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n25_), .c(new_n34_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n55_), .O(z3));
  inv1   g39(.a(x05), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(x04), .c(new_n45_), .O(z4));
endmodule


