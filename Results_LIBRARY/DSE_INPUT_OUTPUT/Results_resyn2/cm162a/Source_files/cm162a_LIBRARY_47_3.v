// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_;
  nand2  g00(.a(x13), .b(x09), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  oai21  g04(.a(new_n21_), .b(x08), .c(x03), .O(new_n24_));
  aoi21  g05(.a(new_n23_), .b(x08), .c(new_n24_), .O(new_n25_));
  oai21  g06(.a(new_n25_), .b(x00), .c(x05), .O(z0));
  oai21  g07(.a(new_n21_), .b(x08), .c(x10), .O(new_n27_));
  inv1   g08(.a(new_n20_), .O(new_n28_));
  nand2  g09(.a(new_n22_), .b(new_n28_), .O(new_n29_));
  inv1   g10(.a(x08), .O(new_n30_));
  inv1   g11(.a(x10), .O(new_n31_));
  nand3  g12(.a(new_n22_), .b(new_n31_), .c(new_n30_), .O(new_n32_));
  nand4  g13(.a(new_n32_), .b(new_n29_), .c(new_n27_), .d(x03), .O(new_n33_));
  inv1   g14(.a(x03), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(x01), .c(x00), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x05), .O(z1));
  inv1   g18(.a(x11), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n31_), .c(new_n30_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n20_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n22_), .O(new_n41_));
  inv1   g22(.a(new_n41_), .O(new_n42_));
  inv1   g23(.a(new_n32_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n20_), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(x11), .c(new_n42_), .O(new_n45_));
  nor2   g26(.a(x06), .b(x03), .O(new_n46_));
  inv1   g27(.a(x00), .O(new_n47_));
  nand2  g28(.a(x05), .b(new_n47_), .O(new_n48_));
  nor2   g29(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  oai21  g30(.a(new_n45_), .b(new_n34_), .c(new_n49_), .O(z2));
  inv1   g31(.a(x12), .O(new_n51_));
  nand4  g32(.a(new_n20_), .b(x12), .c(x04), .d(x02), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n39_), .c(x03), .O(new_n53_));
  aoi21  g34(.a(new_n41_), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  nor2   g35(.a(x07), .b(x03), .O(new_n55_));
  oai21  g36(.a(new_n55_), .b(new_n54_), .c(new_n47_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x05), .O(z3));
  nand2  g38(.a(new_n28_), .b(x04), .O(new_n58_));
  aoi21  g39(.a(x05), .b(x00), .c(new_n58_), .O(z4));
endmodule


