// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  inv1   g00(.a(x03), .O(new_n20_));
  oai21  g01(.a(x03), .b(x00), .c(x05), .O(new_n21_));
  oai21  g02(.a(x09), .b(x08), .c(new_n21_), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  inv1   g05(.a(new_n24_), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand3  g07(.a(x09), .b(x04), .c(x02), .O(new_n27_));
  nor2   g08(.a(new_n27_), .b(x08), .O(new_n28_));
  aoi21  g09(.a(new_n26_), .b(x08), .c(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n20_), .c(new_n22_), .O(z0));
  inv1   g11(.a(x08), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  aoi21  g14(.a(new_n33_), .b(new_n23_), .c(new_n24_), .O(new_n34_));
  inv1   g15(.a(new_n27_), .O(new_n35_));
  nor2   g16(.a(x13), .b(x08), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n35_), .c(new_n32_), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n34_), .c(x03), .O(new_n38_));
  nor2   g19(.a(x03), .b(x01), .O(new_n39_));
  oai21  g20(.a(x09), .b(x08), .c(x05), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n38_), .O(z1));
  nor2   g23(.a(x11), .b(x10), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n31_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n23_), .O(new_n45_));
  nand3  g26(.a(new_n36_), .b(new_n35_), .c(new_n32_), .O(new_n46_));
  aoi22  g27(.a(new_n46_), .b(x11), .c(new_n45_), .d(new_n25_), .O(new_n47_));
  nor2   g28(.a(x06), .b(x03), .O(new_n48_));
  nor2   g29(.a(new_n48_), .b(new_n40_), .O(new_n49_));
  oai21  g30(.a(new_n47_), .b(new_n20_), .c(new_n49_), .O(z2));
  inv1   g31(.a(x04), .O(new_n51_));
  and2   g32(.a(x13), .b(x09), .O(new_n52_));
  inv1   g33(.a(x02), .O(new_n53_));
  inv1   g34(.a(x12), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand4  g37(.a(new_n43_), .b(new_n54_), .c(new_n31_), .d(x02), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n56_), .c(new_n51_), .O(new_n58_));
  aoi21  g39(.a(new_n43_), .b(new_n28_), .c(new_n54_), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n58_), .c(x03), .O(new_n60_));
  nor2   g41(.a(x07), .b(x03), .O(new_n61_));
  nor2   g42(.a(new_n61_), .b(new_n40_), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(new_n60_), .O(z3));
  nand2  g44(.a(new_n52_), .b(x04), .O(new_n64_));
  inv1   g45(.a(new_n64_), .O(z4));
endmodule


