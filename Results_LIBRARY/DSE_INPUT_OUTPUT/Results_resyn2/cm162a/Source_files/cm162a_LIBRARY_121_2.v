// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  nand3  g00(.a(x13), .b(x09), .c(x04), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(z4));
  nand2  g02(.a(z4), .b(x02), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  inv1   g05(.a(new_n24_), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand2  g07(.a(new_n24_), .b(x08), .O(new_n27_));
  nand3  g08(.a(new_n27_), .b(new_n26_), .c(new_n22_), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x03), .O(new_n29_));
  nor2   g10(.a(x03), .b(x00), .O(new_n30_));
  inv1   g11(.a(x13), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(x07), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(x05), .O(new_n33_));
  nor2   g14(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n29_), .O(z0));
  inv1   g16(.a(x03), .O(new_n36_));
  oai21  g17(.a(new_n24_), .b(x08), .c(x10), .O(new_n37_));
  inv1   g18(.a(x10), .O(new_n38_));
  nand4  g19(.a(new_n38_), .b(new_n23_), .c(x04), .d(x02), .O(new_n39_));
  aoi21  g20(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(new_n40_));
  oai21  g21(.a(x03), .b(x01), .c(x05), .O(new_n41_));
  oai21  g22(.a(new_n41_), .b(new_n40_), .c(new_n32_), .O(new_n42_));
  nand3  g23(.a(x13), .b(x09), .c(x03), .O(new_n43_));
  inv1   g24(.a(new_n43_), .O(new_n44_));
  oai21  g25(.a(new_n25_), .b(x10), .c(new_n44_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n42_), .O(z1));
  inv1   g27(.a(new_n32_), .O(new_n47_));
  inv1   g28(.a(x11), .O(new_n48_));
  xor2a  g29(.a(new_n39_), .b(new_n48_), .O(new_n49_));
  oai21  g30(.a(x06), .b(x03), .c(x05), .O(new_n50_));
  aoi21  g31(.a(new_n49_), .b(x03), .c(new_n50_), .O(new_n51_));
  oai21  g32(.a(new_n25_), .b(x11), .c(new_n44_), .O(new_n52_));
  oai21  g33(.a(new_n51_), .b(new_n47_), .c(new_n52_), .O(z2));
  inv1   g34(.a(new_n39_), .O(new_n54_));
  inv1   g35(.a(x12), .O(new_n55_));
  nand2  g36(.a(new_n22_), .b(new_n55_), .O(new_n56_));
  aoi21  g37(.a(new_n54_), .b(new_n48_), .c(new_n56_), .O(new_n57_));
  nand2  g38(.a(new_n54_), .b(new_n48_), .O(new_n58_));
  nand2  g39(.a(new_n20_), .b(x12), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n58_), .c(x03), .O(new_n60_));
  nor2   g41(.a(x07), .b(x03), .O(new_n61_));
  nor2   g42(.a(new_n61_), .b(new_n33_), .O(new_n62_));
  oai21  g43(.a(new_n60_), .b(new_n57_), .c(new_n62_), .O(z3));
endmodule


