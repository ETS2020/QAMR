// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  xor2a  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  inv1   g03(.a(x05), .O(new_n23_));
  nor2   g04(.a(x03), .b(x00), .O(new_n24_));
  nor3   g05(.a(new_n24_), .b(x13), .c(new_n23_), .O(new_n25_));
  oai21  g06(.a(new_n22_), .b(new_n20_), .c(new_n25_), .O(z0));
  oai21  g07(.a(new_n21_), .b(x08), .c(x10), .O(new_n27_));
  inv1   g08(.a(x08), .O(new_n28_));
  inv1   g09(.a(x10), .O(new_n29_));
  nand4  g10(.a(new_n29_), .b(new_n28_), .c(x04), .d(x02), .O(new_n30_));
  nand3  g11(.a(new_n30_), .b(new_n27_), .c(x03), .O(new_n31_));
  aoi21  g12(.a(new_n20_), .b(x01), .c(x13), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(x05), .O(z1));
  nand2  g15(.a(new_n30_), .b(x11), .O(new_n35_));
  inv1   g16(.a(x11), .O(new_n36_));
  nor2   g17(.a(x10), .b(x08), .O(new_n37_));
  nand4  g18(.a(new_n37_), .b(new_n36_), .c(x04), .d(x02), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n35_), .c(x03), .O(new_n39_));
  aoi21  g20(.a(x06), .b(new_n20_), .c(x13), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x05), .O(z2));
  nand2  g23(.a(x13), .b(x09), .O(new_n43_));
  nor2   g24(.a(x12), .b(x11), .O(new_n44_));
  nand4  g25(.a(new_n44_), .b(new_n37_), .c(x03), .d(x02), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(x04), .O(new_n47_));
  and2   g28(.a(x12), .b(x03), .O(new_n48_));
  inv1   g29(.a(x13), .O(new_n49_));
  inv1   g30(.a(x07), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n20_), .O(new_n51_));
  nand3  g32(.a(new_n51_), .b(new_n49_), .c(x05), .O(new_n52_));
  aoi21  g33(.a(new_n48_), .b(new_n38_), .c(new_n52_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n47_), .O(z3));
  nand2  g35(.a(x09), .b(x04), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n23_), .c(new_n49_), .O(z4));
endmodule


