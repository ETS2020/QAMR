// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_;
  inv1   g00(.a(x00), .O(new_n20_));
  nor2   g01(.a(x03), .b(new_n20_), .O(new_n21_));
  and2   g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand2  g05(.a(x04), .b(x02), .O(new_n25_));
  oai21  g06(.a(new_n25_), .b(x08), .c(x03), .O(new_n26_));
  aoi21  g07(.a(new_n24_), .b(x08), .c(new_n26_), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n21_), .c(x05), .O(z0));
  nor2   g09(.a(x12), .b(x05), .O(new_n29_));
  inv1   g10(.a(x03), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x01), .O(new_n31_));
  inv1   g12(.a(x08), .O(new_n32_));
  nand4  g13(.a(new_n23_), .b(new_n32_), .c(x04), .d(x02), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(x10), .O(new_n34_));
  and2   g15(.a(x13), .b(x09), .O(new_n35_));
  nor2   g16(.a(x10), .b(x08), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n35_), .c(new_n22_), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(new_n34_), .c(x03), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n31_), .O(new_n39_));
  aoi21  g20(.a(new_n39_), .b(x05), .c(new_n29_), .O(z1));
  inv1   g21(.a(x11), .O(new_n41_));
  nand2  g22(.a(new_n36_), .b(new_n41_), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n23_), .c(new_n25_), .O(new_n43_));
  inv1   g24(.a(x10), .O(new_n44_));
  nand4  g25(.a(new_n23_), .b(new_n22_), .c(new_n44_), .d(new_n32_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(x11), .c(new_n43_), .O(new_n46_));
  inv1   g27(.a(x05), .O(new_n47_));
  inv1   g28(.a(x06), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n30_), .c(new_n47_), .O(new_n49_));
  oai21  g30(.a(new_n46_), .b(new_n30_), .c(new_n49_), .O(z2));
  nor2   g31(.a(new_n43_), .b(x12), .O(new_n51_));
  nand2  g32(.a(x12), .b(new_n41_), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n45_), .c(x03), .O(new_n53_));
  inv1   g34(.a(x07), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n30_), .c(new_n47_), .O(new_n55_));
  oai21  g36(.a(new_n53_), .b(new_n51_), .c(new_n55_), .O(z3));
  inv1   g37(.a(x04), .O(new_n57_));
  oai22  g38(.a(new_n23_), .b(new_n57_), .c(x12), .d(x05), .O(z4));
endmodule


