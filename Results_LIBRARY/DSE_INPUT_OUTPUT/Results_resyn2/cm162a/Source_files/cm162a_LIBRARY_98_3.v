// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  nand3  g02(.a(new_n21_), .b(x04), .c(x02), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nor2   g04(.a(new_n23_), .b(x08), .O(new_n24_));
  aoi21  g05(.a(new_n22_), .b(x08), .c(new_n24_), .O(new_n25_));
  nor2   g06(.a(x03), .b(x00), .O(new_n26_));
  inv1   g07(.a(x11), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x05), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  oai21  g10(.a(new_n25_), .b(new_n20_), .c(new_n29_), .O(z0));
  inv1   g11(.a(x08), .O(new_n31_));
  nand4  g12(.a(new_n21_), .b(new_n31_), .c(x04), .d(x02), .O(new_n32_));
  nand3  g13(.a(new_n32_), .b(x10), .c(x03), .O(new_n33_));
  inv1   g14(.a(x01), .O(new_n34_));
  nand2  g15(.a(new_n20_), .b(new_n34_), .O(new_n35_));
  oai21  g16(.a(x10), .b(x08), .c(new_n21_), .O(new_n36_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n37_));
  inv1   g18(.a(new_n37_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand3  g20(.a(new_n39_), .b(new_n35_), .c(new_n33_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n27_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x05), .O(z1));
  nor2   g23(.a(x06), .b(x03), .O(new_n43_));
  aoi21  g24(.a(new_n38_), .b(new_n36_), .c(new_n43_), .O(new_n44_));
  oai21  g25(.a(new_n44_), .b(x11), .c(x05), .O(z2));
  nand2  g26(.a(new_n21_), .b(x12), .O(new_n46_));
  inv1   g27(.a(x02), .O(new_n47_));
  inv1   g28(.a(x04), .O(new_n48_));
  inv1   g29(.a(x12), .O(new_n49_));
  aoi21  g30(.a(new_n49_), .b(new_n47_), .c(new_n48_), .O(new_n50_));
  nand3  g31(.a(new_n50_), .b(new_n46_), .c(new_n36_), .O(new_n51_));
  inv1   g32(.a(x10), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n31_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n23_), .c(x12), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n51_), .c(x03), .O(new_n55_));
  aoi21  g36(.a(x07), .b(new_n20_), .c(x11), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(x05), .O(z3));
  nand3  g39(.a(x13), .b(x09), .c(x04), .O(new_n59_));
  aoi21  g40(.a(x11), .b(x05), .c(new_n59_), .O(z4));
endmodule


