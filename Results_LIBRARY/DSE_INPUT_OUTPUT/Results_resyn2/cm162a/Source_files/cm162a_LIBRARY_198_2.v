// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  nand3  g02(.a(new_n21_), .b(x04), .c(x02), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nor2   g04(.a(new_n23_), .b(x08), .O(new_n24_));
  aoi21  g05(.a(new_n22_), .b(x08), .c(new_n24_), .O(new_n25_));
  inv1   g06(.a(x00), .O(new_n26_));
  inv1   g07(.a(x05), .O(new_n27_));
  aoi21  g08(.a(new_n20_), .b(new_n26_), .c(new_n27_), .O(new_n28_));
  oai21  g09(.a(new_n25_), .b(new_n20_), .c(new_n28_), .O(z0));
  inv1   g10(.a(x08), .O(new_n30_));
  inv1   g11(.a(x10), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  aoi21  g13(.a(new_n32_), .b(new_n21_), .c(new_n23_), .O(new_n33_));
  nand2  g14(.a(new_n24_), .b(new_n21_), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(x10), .c(new_n33_), .O(new_n35_));
  inv1   g16(.a(x01), .O(new_n36_));
  aoi21  g17(.a(new_n20_), .b(new_n36_), .c(new_n27_), .O(new_n37_));
  oai21  g18(.a(new_n35_), .b(new_n20_), .c(new_n37_), .O(z1));
  inv1   g19(.a(x04), .O(new_n39_));
  nor2   g20(.a(x10), .b(x08), .O(new_n40_));
  nand4  g21(.a(new_n40_), .b(new_n21_), .c(x11), .d(x02), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(x05), .c(new_n39_), .O(new_n42_));
  oai21  g23(.a(new_n33_), .b(x11), .c(x03), .O(new_n43_));
  inv1   g24(.a(x06), .O(new_n44_));
  nor2   g25(.a(x05), .b(x04), .O(new_n45_));
  nor2   g26(.a(new_n27_), .b(x03), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n44_), .c(new_n45_), .O(new_n47_));
  oai21  g28(.a(new_n43_), .b(new_n42_), .c(new_n47_), .O(z2));
  inv1   g29(.a(x12), .O(new_n49_));
  inv1   g30(.a(x11), .O(new_n50_));
  aoi22  g31(.a(new_n40_), .b(new_n50_), .c(x13), .d(x09), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n23_), .c(new_n49_), .O(new_n52_));
  nand3  g33(.a(x12), .b(new_n50_), .c(new_n31_), .O(new_n53_));
  nand3  g34(.a(new_n21_), .b(new_n30_), .c(x02), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n53_), .c(x05), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x04), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n52_), .c(x03), .O(new_n57_));
  inv1   g38(.a(x07), .O(new_n58_));
  aoi21  g39(.a(new_n46_), .b(new_n58_), .c(new_n45_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n57_), .O(z3));
  aoi21  g41(.a(new_n21_), .b(x05), .c(new_n39_), .O(z4));
endmodule


