// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x05), .b(x00), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(x05), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  aoi21  g09(.a(new_n28_), .b(x03), .c(new_n23_), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(x09), .c(new_n22_), .O(z0));
  nand2  g11(.a(x05), .b(x01), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n20_), .O(new_n32_));
  inv1   g13(.a(x09), .O(new_n33_));
  nand2  g14(.a(new_n27_), .b(x10), .O(new_n34_));
  inv1   g15(.a(x10), .O(new_n35_));
  nand4  g16(.a(new_n35_), .b(new_n26_), .c(x04), .d(x02), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n34_), .c(new_n20_), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n23_), .c(new_n33_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n32_), .O(z1));
  nand2  g20(.a(x06), .b(x05), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n20_), .O(new_n41_));
  nand2  g22(.a(new_n36_), .b(x11), .O(new_n42_));
  nor2   g23(.a(x11), .b(x10), .O(new_n43_));
  nand4  g24(.a(new_n43_), .b(new_n26_), .c(x04), .d(x02), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n42_), .c(new_n20_), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n23_), .c(new_n33_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n41_), .O(z2));
  nor2   g28(.a(new_n33_), .b(new_n20_), .O(new_n48_));
  xnor2a g29(.a(new_n44_), .b(x12), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(x03), .O(new_n50_));
  inv1   g31(.a(x07), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n20_), .c(new_n23_), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n50_), .c(new_n48_), .O(z3));
  nand4  g34(.a(x13), .b(x09), .c(x04), .d(new_n20_), .O(new_n54_));
  inv1   g35(.a(new_n54_), .O(z4));
endmodule


