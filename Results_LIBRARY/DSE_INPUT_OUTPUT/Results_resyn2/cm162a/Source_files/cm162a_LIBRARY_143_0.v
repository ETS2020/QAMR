// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_;
  nand4  g00(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  and2   g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(new_n23_), .c(new_n20_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  nor2   g08(.a(x03), .b(x00), .O(new_n28_));
  oai21  g09(.a(x12), .b(x11), .c(x05), .O(new_n29_));
  nor2   g10(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n27_), .O(z0));
  nor2   g12(.a(x12), .b(x11), .O(new_n32_));
  oai21  g13(.a(new_n24_), .b(x08), .c(x10), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand4  g15(.a(new_n34_), .b(new_n21_), .c(x04), .d(x02), .O(new_n35_));
  nand3  g16(.a(new_n35_), .b(new_n33_), .c(new_n20_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(x03), .O(new_n37_));
  inv1   g18(.a(x05), .O(new_n38_));
  nor2   g19(.a(x03), .b(x01), .O(new_n39_));
  nor2   g20(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g21(.a(new_n40_), .b(new_n37_), .c(new_n32_), .O(z1));
  inv1   g22(.a(x11), .O(new_n42_));
  nand4  g23(.a(new_n22_), .b(new_n42_), .c(new_n34_), .d(new_n21_), .O(new_n43_));
  nand2  g24(.a(new_n35_), .b(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n43_), .c(new_n20_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(x03), .O(new_n46_));
  nor2   g27(.a(x06), .b(x03), .O(new_n47_));
  nor2   g28(.a(new_n47_), .b(new_n29_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n46_), .O(z2));
  nand2  g30(.a(x13), .b(x09), .O(new_n50_));
  inv1   g31(.a(new_n35_), .O(new_n51_));
  nand3  g32(.a(new_n51_), .b(new_n50_), .c(new_n42_), .O(new_n52_));
  inv1   g33(.a(x12), .O(new_n53_));
  nand3  g34(.a(new_n20_), .b(new_n53_), .c(x11), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n52_), .c(x03), .O(new_n55_));
  nor2   g36(.a(x07), .b(x03), .O(new_n56_));
  nor2   g37(.a(new_n56_), .b(new_n29_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n55_), .O(z3));
  inv1   g39(.a(x04), .O(new_n59_));
  nor3   g40(.a(new_n32_), .b(new_n50_), .c(new_n59_), .O(z4));
endmodule


