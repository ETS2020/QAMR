// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  and2   g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  aoi21  g04(.a(new_n22_), .b(new_n23_), .c(new_n21_), .O(new_n24_));
  aoi21  g05(.a(new_n22_), .b(new_n21_), .c(new_n24_), .O(new_n25_));
  nor2   g06(.a(x03), .b(x00), .O(new_n26_));
  nor2   g07(.a(x13), .b(x07), .O(new_n27_));
  inv1   g08(.a(new_n27_), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x05), .O(new_n29_));
  nor2   g10(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  oai21  g11(.a(new_n25_), .b(new_n20_), .c(new_n30_), .O(z0));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(x04), .b(x02), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g15(.a(new_n34_), .b(x13), .c(x09), .O(new_n35_));
  nand4  g16(.a(x10), .b(new_n21_), .c(x04), .d(x02), .O(new_n36_));
  oai21  g17(.a(new_n33_), .b(x08), .c(new_n32_), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(new_n36_), .c(new_n28_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n35_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x03), .O(new_n40_));
  oai21  g21(.a(x03), .b(x01), .c(x05), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n28_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n40_), .O(z1));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n32_), .c(new_n21_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n23_), .O(new_n46_));
  nor2   g27(.a(x10), .b(x08), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n22_), .c(new_n23_), .O(new_n48_));
  aoi22  g29(.a(new_n48_), .b(x11), .c(new_n46_), .d(new_n22_), .O(new_n49_));
  nor2   g30(.a(x06), .b(x03), .O(new_n50_));
  nor2   g31(.a(new_n50_), .b(new_n29_), .O(new_n51_));
  oai21  g32(.a(new_n49_), .b(new_n20_), .c(new_n51_), .O(z2));
  nand3  g33(.a(x12), .b(x04), .c(x02), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n47_), .c(new_n44_), .O(new_n54_));
  nand2  g35(.a(new_n45_), .b(x12), .O(new_n55_));
  oai21  g36(.a(x09), .b(new_n20_), .c(x13), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g38(.a(x04), .b(x02), .c(x12), .O(new_n58_));
  inv1   g39(.a(x07), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n20_), .O(new_n60_));
  oai21  g41(.a(new_n58_), .b(new_n20_), .c(new_n60_), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  aoi21  g43(.a(new_n62_), .b(x05), .c(new_n27_), .O(z3));
  inv1   g44(.a(x04), .O(new_n64_));
  oai21  g45(.a(new_n23_), .b(new_n64_), .c(new_n28_), .O(z4));
endmodule


