// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x09), .O(new_n21_));
  oai21  g02(.a(x03), .b(x00), .c(x05), .O(new_n22_));
  oai21  g03(.a(new_n21_), .b(x08), .c(new_n22_), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  inv1   g05(.a(new_n24_), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  nand2  g07(.a(new_n25_), .b(new_n26_), .O(new_n27_));
  nor2   g08(.a(x09), .b(x08), .O(new_n28_));
  aoi22  g09(.a(new_n28_), .b(new_n25_), .c(new_n27_), .d(x08), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n20_), .c(new_n23_), .O(z0));
  inv1   g11(.a(x08), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  aoi21  g14(.a(new_n33_), .b(new_n26_), .c(new_n24_), .O(new_n34_));
  aoi21  g15(.a(new_n28_), .b(new_n25_), .c(new_n32_), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n34_), .c(x03), .O(new_n36_));
  nor2   g17(.a(x03), .b(x01), .O(new_n37_));
  oai21  g18(.a(new_n21_), .b(x08), .c(x05), .O(new_n38_));
  nor2   g19(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n36_), .O(z1));
  inv1   g21(.a(x11), .O(new_n41_));
  nand3  g22(.a(new_n41_), .b(new_n32_), .c(new_n31_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n26_), .O(new_n43_));
  nand3  g24(.a(new_n28_), .b(new_n25_), .c(new_n32_), .O(new_n44_));
  aoi22  g25(.a(new_n44_), .b(x11), .c(new_n43_), .d(new_n25_), .O(new_n45_));
  nor2   g26(.a(x06), .b(x03), .O(new_n46_));
  nor2   g27(.a(new_n46_), .b(new_n38_), .O(new_n47_));
  oai21  g28(.a(new_n45_), .b(new_n20_), .c(new_n47_), .O(z2));
  nor2   g29(.a(x11), .b(x10), .O(new_n49_));
  nand4  g30(.a(new_n49_), .b(new_n28_), .c(x04), .d(x02), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(x12), .O(new_n51_));
  oai21  g32(.a(x10), .b(x08), .c(new_n26_), .O(new_n52_));
  oai21  g33(.a(x12), .b(x11), .c(new_n26_), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n52_), .c(new_n25_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x03), .O(new_n56_));
  nor2   g37(.a(x07), .b(x03), .O(new_n57_));
  nor2   g38(.a(new_n57_), .b(new_n38_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n56_), .O(z3));
  nand4  g40(.a(x13), .b(x09), .c(x08), .d(x04), .O(new_n60_));
  inv1   g41(.a(new_n60_), .O(z4));
endmodule


