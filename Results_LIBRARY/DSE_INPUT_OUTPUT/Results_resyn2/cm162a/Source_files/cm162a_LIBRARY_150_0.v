// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_;
  inv1   g00(.a(x09), .O(new_n20_));
  inv1   g01(.a(x13), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  nor2   g05(.a(new_n24_), .b(x08), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  inv1   g07(.a(new_n24_), .O(new_n27_));
  nand2  g08(.a(x13), .b(x09), .O(new_n28_));
  aoi21  g09(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n25_), .c(x03), .O(new_n30_));
  inv1   g11(.a(x00), .O(new_n31_));
  inv1   g12(.a(x03), .O(new_n32_));
  inv1   g13(.a(x05), .O(new_n33_));
  aoi21  g14(.a(new_n32_), .b(new_n31_), .c(new_n33_), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(new_n30_), .c(new_n23_), .O(z0));
  inv1   g16(.a(x10), .O(new_n36_));
  nand4  g17(.a(new_n36_), .b(new_n26_), .c(x04), .d(x02), .O(new_n37_));
  inv1   g18(.a(new_n37_), .O(new_n38_));
  oai22  g19(.a(new_n28_), .b(new_n24_), .c(new_n25_), .d(new_n36_), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n38_), .c(x03), .O(new_n40_));
  nor2   g21(.a(x03), .b(x01), .O(new_n41_));
  nand2  g22(.a(new_n22_), .b(x05), .O(new_n42_));
  nor2   g23(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n40_), .O(z1));
  nand2  g25(.a(new_n37_), .b(x11), .O(new_n45_));
  nor2   g26(.a(x11), .b(x10), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n27_), .c(new_n26_), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n45_), .c(new_n32_), .O(new_n48_));
  oai21  g29(.a(x06), .b(x03), .c(x05), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n48_), .c(new_n22_), .O(new_n50_));
  inv1   g31(.a(new_n28_), .O(new_n51_));
  inv1   g32(.a(x11), .O(new_n52_));
  nand2  g33(.a(new_n24_), .b(new_n52_), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n51_), .c(x03), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n50_), .O(z2));
  inv1   g36(.a(x12), .O(new_n56_));
  aoi21  g37(.a(new_n46_), .b(new_n25_), .c(new_n56_), .O(new_n57_));
  nand3  g38(.a(new_n46_), .b(new_n56_), .c(new_n26_), .O(new_n58_));
  oai21  g39(.a(x12), .b(x02), .c(x04), .O(new_n59_));
  aoi21  g40(.a(new_n58_), .b(new_n28_), .c(new_n59_), .O(new_n60_));
  oai21  g41(.a(new_n60_), .b(new_n57_), .c(x03), .O(new_n61_));
  nor2   g42(.a(x07), .b(x03), .O(new_n62_));
  nor2   g43(.a(new_n62_), .b(new_n42_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n61_), .O(z3));
  inv1   g45(.a(x04), .O(new_n65_));
  oai21  g46(.a(new_n28_), .b(new_n65_), .c(new_n22_), .O(z4));
endmodule


