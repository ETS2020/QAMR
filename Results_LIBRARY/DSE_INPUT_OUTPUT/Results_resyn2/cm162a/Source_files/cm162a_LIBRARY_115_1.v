// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  and2   g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  aoi21  g04(.a(x13), .b(x09), .c(new_n23_), .O(new_n24_));
  nor2   g05(.a(new_n24_), .b(new_n21_), .O(new_n25_));
  aoi21  g06(.a(new_n22_), .b(new_n21_), .c(new_n25_), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x05), .O(new_n28_));
  nor2   g09(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  oai21  g10(.a(new_n26_), .b(new_n20_), .c(new_n29_), .O(z0));
  nand2  g11(.a(x13), .b(x09), .O(new_n31_));
  nand3  g12(.a(new_n22_), .b(new_n31_), .c(new_n21_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(x10), .O(new_n33_));
  and2   g14(.a(x13), .b(x09), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n34_), .c(new_n22_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n33_), .c(x03), .O(new_n37_));
  aoi21  g18(.a(new_n20_), .b(x01), .c(new_n27_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x05), .O(z1));
  nor2   g21(.a(x06), .b(x03), .O(new_n41_));
  inv1   g22(.a(x11), .O(new_n42_));
  nand2  g23(.a(new_n36_), .b(new_n42_), .O(new_n43_));
  inv1   g24(.a(x10), .O(new_n44_));
  nand3  g25(.a(x11), .b(new_n44_), .c(new_n21_), .O(new_n45_));
  inv1   g26(.a(new_n45_), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n24_), .c(new_n20_), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n43_), .c(new_n41_), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n27_), .c(x05), .O(z2));
  nand2  g30(.a(new_n42_), .b(new_n44_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n32_), .c(x12), .O(new_n51_));
  nor2   g32(.a(x12), .b(x11), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n35_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n31_), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n22_), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n51_), .c(x03), .O(new_n56_));
  aoi21  g37(.a(x07), .b(new_n20_), .c(new_n27_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(x05), .O(z3));
  inv1   g40(.a(x04), .O(new_n60_));
  oai22  g41(.a(new_n31_), .b(new_n60_), .c(new_n28_), .d(x00), .O(z4));
endmodule


