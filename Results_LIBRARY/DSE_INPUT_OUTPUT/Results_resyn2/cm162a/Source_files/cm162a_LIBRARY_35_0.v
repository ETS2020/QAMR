// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_;
  nand3  g00(.a(x13), .b(x08), .c(x03), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x05), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x09), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nor2   g04(.a(new_n23_), .b(x08), .O(new_n24_));
  nand2  g05(.a(new_n23_), .b(x08), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x03), .O(new_n26_));
  inv1   g07(.a(x03), .O(new_n27_));
  inv1   g08(.a(x05), .O(new_n28_));
  aoi21  g09(.a(new_n27_), .b(x00), .c(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n26_), .b(new_n24_), .c(new_n29_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n22_), .O(z0));
  inv1   g12(.a(x08), .O(new_n32_));
  inv1   g13(.a(x10), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand2  g15(.a(x13), .b(x09), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n34_), .c(new_n23_), .O(new_n36_));
  nand2  g17(.a(new_n35_), .b(new_n24_), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(x10), .c(new_n36_), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  aoi21  g20(.a(new_n27_), .b(new_n39_), .c(new_n28_), .O(new_n40_));
  oai21  g21(.a(new_n38_), .b(new_n27_), .c(new_n40_), .O(z1));
  nor2   g22(.a(new_n36_), .b(x11), .O(new_n42_));
  nand3  g23(.a(x11), .b(x04), .c(x02), .O(new_n43_));
  nand3  g24(.a(new_n35_), .b(new_n33_), .c(new_n32_), .O(new_n44_));
  oai21  g25(.a(new_n44_), .b(new_n43_), .c(x03), .O(new_n45_));
  inv1   g26(.a(x06), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n27_), .c(new_n28_), .O(new_n47_));
  oai21  g28(.a(new_n45_), .b(new_n42_), .c(new_n47_), .O(z2));
  inv1   g29(.a(x04), .O(new_n49_));
  and2   g30(.a(x13), .b(x09), .O(new_n50_));
  inv1   g31(.a(x02), .O(new_n51_));
  inv1   g32(.a(x12), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nor2   g35(.a(x11), .b(x10), .O(new_n55_));
  nor2   g36(.a(x12), .b(x08), .O(new_n56_));
  nand4  g37(.a(new_n56_), .b(new_n55_), .c(x05), .d(x02), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n54_), .c(new_n49_), .O(new_n58_));
  nand2  g39(.a(x12), .b(x05), .O(new_n59_));
  aoi21  g40(.a(new_n55_), .b(new_n24_), .c(new_n59_), .O(new_n60_));
  oai21  g41(.a(new_n60_), .b(new_n58_), .c(x03), .O(new_n61_));
  oai21  g42(.a(x07), .b(x03), .c(x05), .O(new_n62_));
  oai21  g43(.a(x09), .b(x05), .c(new_n62_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n61_), .O(z3));
  nand2  g45(.a(new_n50_), .b(x04), .O(new_n65_));
  inv1   g46(.a(new_n65_), .O(z4));
endmodule


