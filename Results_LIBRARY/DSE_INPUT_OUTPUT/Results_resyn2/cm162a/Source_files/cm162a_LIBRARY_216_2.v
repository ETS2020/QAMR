// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_;
  inv1   g00(.a(x08), .O(new_n20_));
  and2   g01(.a(x04), .b(x02), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  aoi21  g03(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(new_n23_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(x03), .O(new_n25_));
  nor2   g06(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  oai21  g08(.a(x03), .b(new_n27_), .c(x12), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n26_), .c(x05), .O(z0));
  nand4  g10(.a(new_n22_), .b(new_n20_), .c(x04), .d(x02), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x10), .O(new_n31_));
  oai21  g12(.a(x10), .b(x08), .c(new_n22_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n21_), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(new_n31_), .c(x03), .O(new_n34_));
  inv1   g15(.a(x03), .O(new_n35_));
  inv1   g16(.a(x12), .O(new_n36_));
  aoi21  g17(.a(new_n35_), .b(x01), .c(new_n36_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x05), .O(z1));
  inv1   g20(.a(x06), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  aoi21  g22(.a(new_n32_), .b(new_n21_), .c(x11), .O(new_n42_));
  inv1   g23(.a(x10), .O(new_n43_));
  nand2  g24(.a(x11), .b(new_n43_), .O(new_n44_));
  oai21  g25(.a(new_n44_), .b(new_n30_), .c(x03), .O(new_n45_));
  oai21  g26(.a(new_n45_), .b(new_n42_), .c(new_n41_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(x12), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(x05), .O(z2));
  nand2  g29(.a(x07), .b(new_n35_), .O(new_n49_));
  inv1   g30(.a(x11), .O(new_n50_));
  nand3  g31(.a(new_n50_), .b(new_n43_), .c(x03), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n30_), .c(new_n49_), .O(new_n52_));
  nand3  g33(.a(new_n52_), .b(x12), .c(x05), .O(z3));
  nand3  g34(.a(x13), .b(x09), .c(x04), .O(new_n54_));
  aoi21  g35(.a(new_n36_), .b(x05), .c(new_n54_), .O(z4));
endmodule


