// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_;
  nand2  g00(.a(x13), .b(x09), .O(new_n20_));
  nand3  g01(.a(new_n20_), .b(x04), .c(x02), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(x08), .c(x03), .O(new_n23_));
  aoi21  g04(.a(new_n21_), .b(x08), .c(new_n23_), .O(new_n24_));
  inv1   g05(.a(x00), .O(new_n25_));
  inv1   g06(.a(x11), .O(new_n26_));
  oai21  g07(.a(x03), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n24_), .c(x05), .O(z0));
  inv1   g09(.a(x08), .O(new_n29_));
  nand4  g10(.a(new_n20_), .b(new_n29_), .c(x04), .d(x02), .O(new_n30_));
  nand3  g11(.a(new_n30_), .b(x10), .c(x03), .O(new_n31_));
  inv1   g12(.a(x01), .O(new_n32_));
  inv1   g13(.a(x03), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  oai21  g15(.a(x10), .b(x08), .c(new_n20_), .O(new_n35_));
  nand3  g16(.a(x04), .b(x03), .c(x02), .O(new_n36_));
  inv1   g17(.a(new_n36_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n34_), .c(new_n31_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n26_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x05), .O(z1));
  nor2   g22(.a(x06), .b(x03), .O(new_n42_));
  aoi21  g23(.a(new_n37_), .b(new_n35_), .c(new_n42_), .O(new_n43_));
  oai21  g24(.a(new_n43_), .b(x11), .c(x05), .O(z2));
  nand2  g25(.a(new_n20_), .b(x12), .O(new_n45_));
  inv1   g26(.a(x02), .O(new_n46_));
  inv1   g27(.a(x04), .O(new_n47_));
  inv1   g28(.a(x12), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n46_), .c(new_n47_), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n45_), .c(new_n35_), .O(new_n50_));
  inv1   g31(.a(x10), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n29_), .O(new_n52_));
  oai21  g33(.a(new_n52_), .b(new_n22_), .c(x12), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n50_), .c(x03), .O(new_n54_));
  aoi21  g35(.a(x07), .b(new_n33_), .c(x11), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(x05), .O(z3));
  nand3  g38(.a(x13), .b(x09), .c(x04), .O(new_n58_));
  aoi21  g39(.a(x11), .b(x05), .c(new_n58_), .O(z4));
endmodule


