// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_;
  inv1   g00(.a(x05), .O(new_n20_));
  nor2   g01(.a(x03), .b(x00), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  aoi21  g03(.a(x13), .b(x09), .c(new_n22_), .O(new_n23_));
  inv1   g04(.a(new_n23_), .O(new_n24_));
  inv1   g05(.a(x03), .O(new_n25_));
  inv1   g06(.a(x08), .O(new_n26_));
  nor2   g07(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  aoi21  g08(.a(new_n27_), .b(new_n24_), .c(new_n21_), .O(new_n28_));
  nand2  g09(.a(x03), .b(x02), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(x08), .c(x05), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x04), .O(new_n31_));
  oai21  g12(.a(new_n28_), .b(new_n20_), .c(new_n31_), .O(z0));
  nand2  g13(.a(x13), .b(x09), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n26_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n33_), .c(new_n22_), .O(new_n36_));
  nand2  g17(.a(new_n23_), .b(new_n26_), .O(new_n37_));
  aoi21  g18(.a(new_n37_), .b(x10), .c(new_n36_), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  aoi21  g20(.a(new_n25_), .b(new_n39_), .c(new_n20_), .O(new_n40_));
  oai21  g21(.a(new_n38_), .b(new_n25_), .c(new_n40_), .O(z1));
  nor2   g22(.a(new_n36_), .b(x11), .O(new_n42_));
  nor2   g23(.a(x10), .b(x08), .O(new_n43_));
  nand3  g24(.a(new_n43_), .b(new_n23_), .c(x11), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g26(.a(x06), .O(new_n46_));
  aoi21  g27(.a(new_n46_), .b(new_n25_), .c(new_n20_), .O(new_n47_));
  oai21  g28(.a(new_n45_), .b(new_n42_), .c(new_n47_), .O(z2));
  nor2   g29(.a(x12), .b(x11), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n33_), .c(new_n22_), .O(new_n51_));
  nor3   g32(.a(x11), .b(x10), .c(x08), .O(new_n52_));
  nand2  g33(.a(x12), .b(x05), .O(new_n53_));
  aoi21  g34(.a(new_n52_), .b(new_n23_), .c(new_n53_), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n51_), .c(x03), .O(new_n55_));
  inv1   g36(.a(x04), .O(new_n56_));
  nand2  g37(.a(new_n20_), .b(new_n56_), .O(new_n57_));
  oai21  g38(.a(x07), .b(x03), .c(x05), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n55_), .O(z3));
  oai21  g41(.a(new_n33_), .b(new_n56_), .c(new_n57_), .O(z4));
endmodule


