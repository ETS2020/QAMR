// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_;
  nand2  g00(.a(x13), .b(x09), .O(new_n20_));
  nand3  g01(.a(new_n20_), .b(x04), .c(x02), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(x08), .c(x03), .O(new_n23_));
  aoi21  g04(.a(new_n21_), .b(x08), .c(new_n23_), .O(new_n24_));
  inv1   g05(.a(x00), .O(new_n25_));
  inv1   g06(.a(x12), .O(new_n26_));
  oai21  g07(.a(x03), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n24_), .c(x05), .O(z0));
  oai21  g09(.a(new_n22_), .b(x08), .c(x10), .O(new_n29_));
  inv1   g10(.a(new_n20_), .O(new_n30_));
  nand3  g11(.a(new_n30_), .b(x04), .c(x02), .O(new_n31_));
  nor2   g12(.a(x10), .b(x08), .O(new_n32_));
  nand3  g13(.a(new_n32_), .b(x04), .c(x02), .O(new_n33_));
  nand3  g14(.a(new_n33_), .b(new_n31_), .c(new_n29_), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(x03), .O(new_n35_));
  inv1   g16(.a(x05), .O(new_n36_));
  nor2   g17(.a(x03), .b(x01), .O(new_n37_));
  nor3   g18(.a(new_n37_), .b(x12), .c(new_n36_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(new_n35_), .O(z1));
  nor3   g20(.a(x11), .b(x10), .c(x08), .O(new_n40_));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n41_));
  inv1   g22(.a(new_n41_), .O(new_n42_));
  oai21  g23(.a(new_n40_), .b(new_n30_), .c(new_n42_), .O(new_n43_));
  inv1   g24(.a(new_n32_), .O(new_n44_));
  inv1   g25(.a(x03), .O(new_n45_));
  inv1   g26(.a(x11), .O(new_n46_));
  nor2   g27(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g28(.a(new_n44_), .b(new_n21_), .c(new_n47_), .O(new_n48_));
  inv1   g29(.a(x06), .O(new_n49_));
  nand2  g30(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand3  g31(.a(new_n50_), .b(new_n48_), .c(new_n43_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n26_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(x05), .O(z2));
  nand2  g34(.a(new_n32_), .b(new_n46_), .O(new_n54_));
  aoi21  g35(.a(new_n54_), .b(new_n20_), .c(new_n41_), .O(new_n55_));
  nor2   g36(.a(x07), .b(x03), .O(new_n56_));
  oai21  g37(.a(new_n56_), .b(new_n55_), .c(new_n26_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(x05), .O(z3));
  inv1   g39(.a(x04), .O(new_n59_));
  oai22  g40(.a(new_n20_), .b(new_n59_), .c(new_n26_), .d(new_n36_), .O(z4));
endmodule


