// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_;
  inv1   g00(.a(x08), .O(new_n20_));
  and2   g01(.a(x04), .b(x02), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(x08), .O(new_n23_));
  aoi21  g04(.a(new_n23_), .b(new_n21_), .c(x10), .O(new_n24_));
  oai21  g05(.a(new_n21_), .b(new_n20_), .c(new_n24_), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(x03), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x03), .O(new_n28_));
  inv1   g09(.a(x05), .O(new_n29_));
  aoi21  g10(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n26_), .O(z0));
  inv1   g12(.a(x01), .O(new_n32_));
  aoi21  g13(.a(new_n28_), .b(new_n32_), .c(new_n29_), .O(new_n33_));
  oai21  g14(.a(new_n24_), .b(new_n28_), .c(new_n33_), .O(z1));
  and2   g15(.a(x10), .b(x03), .O(new_n35_));
  nand2  g16(.a(x06), .b(new_n28_), .O(new_n36_));
  inv1   g17(.a(x11), .O(new_n37_));
  nand3  g18(.a(new_n23_), .b(new_n21_), .c(new_n37_), .O(new_n38_));
  nand4  g19(.a(new_n22_), .b(new_n20_), .c(x04), .d(x02), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x11), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(new_n38_), .c(x03), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(x05), .c(new_n35_), .O(z2));
  inv1   g24(.a(x12), .O(new_n44_));
  and2   g25(.a(x13), .b(x09), .O(new_n45_));
  nor2   g26(.a(x11), .b(x08), .O(new_n46_));
  oai21  g27(.a(new_n46_), .b(new_n45_), .c(new_n21_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand4  g29(.a(new_n46_), .b(new_n22_), .c(new_n21_), .d(x12), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n48_), .c(x03), .O(new_n50_));
  inv1   g31(.a(x07), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n28_), .c(new_n29_), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n50_), .c(new_n35_), .O(z3));
  nand2  g34(.a(new_n45_), .b(x04), .O(new_n54_));
  nor2   g35(.a(new_n54_), .b(new_n35_), .O(z4));
endmodule


