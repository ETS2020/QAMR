// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_;
  inv1   g00(.a(x08), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  inv1   g04(.a(new_n23_), .O(new_n24_));
  oai21  g05(.a(new_n24_), .b(new_n20_), .c(new_n22_), .O(new_n25_));
  oai21  g06(.a(new_n22_), .b(new_n20_), .c(new_n25_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g08(.a(x05), .O(new_n28_));
  nor2   g09(.a(x03), .b(x00), .O(new_n29_));
  nor2   g10(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(new_n27_), .O(z0));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(new_n25_), .b(new_n32_), .O(new_n33_));
  nand2  g14(.a(x10), .b(new_n20_), .O(new_n34_));
  oai21  g15(.a(new_n34_), .b(new_n21_), .c(x05), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n23_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n33_), .c(x03), .O(new_n37_));
  inv1   g18(.a(x01), .O(new_n38_));
  nor2   g19(.a(new_n28_), .b(x03), .O(new_n39_));
  inv1   g20(.a(x09), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(x05), .O(new_n41_));
  aoi21  g22(.a(new_n39_), .b(new_n38_), .c(new_n41_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n37_), .O(z1));
  inv1   g24(.a(x11), .O(new_n44_));
  nand4  g25(.a(new_n44_), .b(new_n32_), .c(new_n20_), .d(x05), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n23_), .c(new_n21_), .O(new_n46_));
  nand4  g27(.a(new_n32_), .b(new_n20_), .c(x04), .d(x02), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(x05), .O(new_n48_));
  aoi21  g29(.a(new_n48_), .b(new_n23_), .c(new_n44_), .O(new_n49_));
  oai21  g30(.a(new_n49_), .b(new_n46_), .c(x03), .O(new_n50_));
  inv1   g31(.a(x06), .O(new_n51_));
  aoi21  g32(.a(new_n39_), .b(new_n51_), .c(new_n41_), .O(new_n52_));
  nand2  g33(.a(new_n52_), .b(new_n50_), .O(z2));
  and2   g34(.a(x12), .b(x05), .O(new_n54_));
  oai21  g35(.a(new_n47_), .b(x11), .c(new_n54_), .O(new_n55_));
  nand2  g36(.a(new_n45_), .b(new_n23_), .O(new_n56_));
  inv1   g37(.a(x02), .O(new_n57_));
  nand3  g38(.a(x13), .b(x12), .c(x09), .O(new_n58_));
  oai21  g39(.a(x12), .b(new_n57_), .c(new_n58_), .O(new_n59_));
  nand3  g40(.a(new_n59_), .b(new_n56_), .c(x04), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n55_), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(x03), .O(new_n62_));
  inv1   g43(.a(x07), .O(new_n63_));
  aoi21  g44(.a(new_n39_), .b(new_n63_), .c(new_n41_), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(new_n62_), .O(z3));
  nand2  g46(.a(new_n24_), .b(x04), .O(new_n66_));
  inv1   g47(.a(new_n66_), .O(z4));
endmodule


