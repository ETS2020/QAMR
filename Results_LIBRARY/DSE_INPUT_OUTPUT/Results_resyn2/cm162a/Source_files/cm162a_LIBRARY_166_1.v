// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_;
  inv1   g00(.a(x07), .O(new_n20_));
  nand2  g01(.a(x11), .b(new_n20_), .O(new_n21_));
  inv1   g02(.a(new_n21_), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  nand2  g05(.a(x04), .b(x02), .O(new_n25_));
  inv1   g06(.a(new_n25_), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n24_), .c(new_n23_), .O(new_n27_));
  nor2   g08(.a(new_n25_), .b(x08), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(new_n27_), .c(x03), .O(new_n29_));
  inv1   g10(.a(x00), .O(new_n30_));
  inv1   g11(.a(x03), .O(new_n31_));
  inv1   g12(.a(x05), .O(new_n32_));
  aoi21  g13(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  aoi21  g14(.a(new_n33_), .b(new_n29_), .c(new_n22_), .O(z0));
  inv1   g15(.a(x10), .O(new_n35_));
  nand2  g16(.a(new_n28_), .b(new_n35_), .O(new_n36_));
  nand4  g17(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n37_));
  oai21  g18(.a(new_n25_), .b(x08), .c(x10), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(x03), .O(new_n40_));
  nor2   g21(.a(x03), .b(x01), .O(new_n41_));
  nand2  g22(.a(new_n21_), .b(x05), .O(new_n42_));
  nor2   g23(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n40_), .O(z1));
  inv1   g25(.a(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n35_), .c(new_n23_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n24_), .O(new_n47_));
  nand4  g28(.a(new_n26_), .b(new_n24_), .c(new_n35_), .d(new_n23_), .O(new_n48_));
  aoi22  g29(.a(new_n48_), .b(x11), .c(new_n47_), .d(new_n26_), .O(new_n49_));
  nor2   g30(.a(x06), .b(x03), .O(new_n50_));
  nor2   g31(.a(new_n50_), .b(new_n42_), .O(new_n51_));
  oai21  g32(.a(new_n49_), .b(new_n31_), .c(new_n51_), .O(z2));
  inv1   g33(.a(x12), .O(new_n53_));
  nand3  g34(.a(new_n47_), .b(new_n26_), .c(new_n53_), .O(new_n54_));
  nand3  g35(.a(new_n24_), .b(x04), .c(x02), .O(new_n55_));
  nand2  g36(.a(new_n35_), .b(new_n23_), .O(new_n56_));
  oai21  g37(.a(new_n56_), .b(new_n55_), .c(x12), .O(new_n57_));
  nand2  g38(.a(new_n37_), .b(new_n53_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(x11), .O(new_n59_));
  nand3  g40(.a(new_n59_), .b(new_n57_), .c(new_n54_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(x03), .O(new_n61_));
  nand2  g42(.a(new_n45_), .b(x03), .O(new_n62_));
  aoi21  g43(.a(new_n62_), .b(new_n20_), .c(new_n32_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n61_), .O(z3));
  inv1   g45(.a(x04), .O(new_n65_));
  oai21  g46(.a(new_n24_), .b(new_n65_), .c(new_n21_), .O(z4));
endmodule


