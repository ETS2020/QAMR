// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  nor2   g00(.a(x11), .b(x07), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  inv1   g04(.a(new_n23_), .O(new_n24_));
  aoi21  g05(.a(new_n24_), .b(new_n22_), .c(new_n21_), .O(new_n25_));
  nor2   g06(.a(new_n23_), .b(x08), .O(new_n26_));
  oai21  g07(.a(new_n26_), .b(new_n25_), .c(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  aoi21  g12(.a(new_n31_), .b(new_n27_), .c(new_n20_), .O(z0));
  inv1   g13(.a(x10), .O(new_n33_));
  nor2   g14(.a(new_n26_), .b(new_n33_), .O(new_n34_));
  nand4  g15(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n35_));
  nor2   g16(.a(x10), .b(x08), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n24_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n34_), .c(x03), .O(new_n39_));
  nor2   g20(.a(x03), .b(x01), .O(new_n40_));
  inv1   g21(.a(new_n20_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x05), .O(new_n42_));
  nor2   g23(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n39_), .O(z1));
  inv1   g25(.a(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n33_), .c(new_n21_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n22_), .O(new_n47_));
  nand3  g28(.a(new_n36_), .b(new_n24_), .c(new_n22_), .O(new_n48_));
  aoi22  g29(.a(new_n48_), .b(x11), .c(new_n47_), .d(new_n24_), .O(new_n49_));
  nor2   g30(.a(x06), .b(x03), .O(new_n50_));
  nor2   g31(.a(new_n50_), .b(new_n42_), .O(new_n51_));
  oai21  g32(.a(new_n49_), .b(new_n29_), .c(new_n51_), .O(z2));
  inv1   g33(.a(x12), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(x04), .c(x02), .O(new_n54_));
  aoi21  g35(.a(new_n46_), .b(new_n22_), .c(new_n54_), .O(new_n55_));
  aoi21  g36(.a(x13), .b(x09), .c(new_n23_), .O(new_n56_));
  nor3   g37(.a(x11), .b(x10), .c(x08), .O(new_n57_));
  aoi21  g38(.a(new_n57_), .b(new_n56_), .c(new_n53_), .O(new_n58_));
  oai21  g39(.a(new_n58_), .b(new_n55_), .c(x03), .O(new_n59_));
  inv1   g40(.a(x07), .O(new_n60_));
  nand3  g41(.a(new_n53_), .b(x11), .c(x03), .O(new_n61_));
  aoi21  g42(.a(new_n61_), .b(new_n60_), .c(new_n30_), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(new_n59_), .O(z3));
  inv1   g44(.a(x04), .O(new_n64_));
  oai21  g45(.a(new_n22_), .b(new_n64_), .c(new_n41_), .O(z4));
endmodule


