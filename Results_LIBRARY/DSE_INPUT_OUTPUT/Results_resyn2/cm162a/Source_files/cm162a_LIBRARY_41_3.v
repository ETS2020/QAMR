// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n63_;
  nand2  g00(.a(x12), .b(x11), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  inv1   g05(.a(new_n24_), .O(new_n25_));
  aoi21  g06(.a(new_n25_), .b(new_n23_), .c(new_n22_), .O(new_n26_));
  nor2   g07(.a(new_n24_), .b(x08), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n26_), .c(x03), .O(new_n28_));
  inv1   g09(.a(x05), .O(new_n29_));
  nor2   g10(.a(x03), .b(x00), .O(new_n30_));
  nor2   g11(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  aoi21  g12(.a(new_n31_), .b(new_n28_), .c(new_n21_), .O(z0));
  inv1   g13(.a(x10), .O(new_n33_));
  nor2   g14(.a(new_n27_), .b(new_n33_), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  inv1   g16(.a(new_n35_), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n23_), .c(new_n24_), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n34_), .c(x03), .O(new_n38_));
  nand2  g19(.a(new_n20_), .b(x05), .O(new_n39_));
  nor2   g20(.a(x03), .b(x01), .O(new_n40_));
  nor2   g21(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n38_), .O(z1));
  nand4  g23(.a(new_n35_), .b(new_n23_), .c(x04), .d(x02), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(x11), .O(new_n44_));
  oai21  g25(.a(x10), .b(x08), .c(new_n23_), .O(new_n45_));
  nand2  g26(.a(new_n23_), .b(x11), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n45_), .c(new_n25_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x03), .O(new_n49_));
  nor2   g30(.a(x06), .b(x03), .O(new_n50_));
  nor2   g31(.a(new_n50_), .b(new_n39_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n49_), .O(z2));
  inv1   g33(.a(x12), .O(new_n53_));
  nor2   g34(.a(new_n53_), .b(x11), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  aoi21  g36(.a(new_n23_), .b(x11), .c(x12), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n45_), .c(new_n25_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(x03), .O(new_n59_));
  oai21  g40(.a(x07), .b(x03), .c(x05), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n20_), .O(new_n61_));
  nand2  g42(.a(new_n61_), .b(new_n59_), .O(z3));
  nand4  g43(.a(new_n20_), .b(x13), .c(x09), .d(x04), .O(new_n63_));
  inv1   g44(.a(new_n63_), .O(z4));
endmodule


