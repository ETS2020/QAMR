// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_;
  inv1   g00(.a(x03), .O(new_n20_));
  inv1   g01(.a(x06), .O(new_n21_));
  nor2   g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  inv1   g05(.a(new_n24_), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand2  g07(.a(x13), .b(x09), .O(new_n27_));
  nand3  g08(.a(new_n27_), .b(x04), .c(x02), .O(new_n28_));
  inv1   g09(.a(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n23_), .c(new_n26_), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g12(.a(x00), .O(new_n32_));
  inv1   g13(.a(x05), .O(new_n33_));
  aoi21  g14(.a(new_n20_), .b(new_n32_), .c(new_n33_), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(new_n31_), .c(new_n22_), .O(z0));
  inv1   g16(.a(new_n27_), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n26_), .c(x10), .O(new_n37_));
  nor2   g18(.a(x10), .b(x08), .O(new_n38_));
  oai21  g19(.a(new_n38_), .b(new_n36_), .c(new_n25_), .O(new_n39_));
  nand3  g20(.a(new_n39_), .b(new_n37_), .c(new_n21_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x03), .O(new_n41_));
  inv1   g22(.a(x01), .O(new_n42_));
  aoi21  g23(.a(new_n20_), .b(new_n42_), .c(new_n33_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n41_), .O(z1));
  inv1   g25(.a(x11), .O(new_n45_));
  nand2  g26(.a(new_n39_), .b(new_n45_), .O(new_n46_));
  nand3  g27(.a(new_n38_), .b(new_n29_), .c(x11), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n46_), .c(new_n20_), .O(new_n48_));
  oai22  g29(.a(new_n48_), .b(x06), .c(new_n22_), .d(x05), .O(z2));
  nor2   g30(.a(x12), .b(x11), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n38_), .c(new_n36_), .O(new_n51_));
  inv1   g32(.a(x10), .O(new_n52_));
  nand3  g33(.a(new_n45_), .b(new_n52_), .c(new_n23_), .O(new_n53_));
  oai21  g34(.a(new_n53_), .b(new_n28_), .c(x12), .O(new_n54_));
  oai21  g35(.a(new_n51_), .b(new_n24_), .c(new_n54_), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(x03), .c(new_n33_), .O(new_n56_));
  nand2  g37(.a(x07), .b(x05), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n20_), .O(new_n58_));
  oai21  g39(.a(new_n56_), .b(x06), .c(new_n58_), .O(z3));
  inv1   g40(.a(x04), .O(new_n60_));
  oai22  g41(.a(new_n27_), .b(new_n60_), .c(new_n21_), .d(new_n20_), .O(z4));
endmodule


