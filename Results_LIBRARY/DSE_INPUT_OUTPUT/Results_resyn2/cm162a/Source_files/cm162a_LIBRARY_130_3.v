// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x08), .O(new_n20_));
  aoi21  g01(.a(x04), .b(x02), .c(new_n20_), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  nor2   g03(.a(new_n22_), .b(x08), .O(new_n23_));
  oai21  g04(.a(new_n23_), .b(new_n21_), .c(x03), .O(new_n24_));
  inv1   g05(.a(x06), .O(new_n25_));
  nand3  g06(.a(x13), .b(x08), .c(x03), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  oai21  g08(.a(x03), .b(x00), .c(x05), .O(new_n28_));
  aoi21  g09(.a(new_n27_), .b(x09), .c(new_n28_), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(new_n24_), .O(z0));
  nand2  g11(.a(x09), .b(x06), .O(new_n31_));
  inv1   g12(.a(x03), .O(new_n32_));
  oai21  g13(.a(new_n22_), .b(x08), .c(x10), .O(new_n33_));
  inv1   g14(.a(x10), .O(new_n34_));
  nand4  g15(.a(new_n34_), .b(new_n20_), .c(x04), .d(x02), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n33_), .c(new_n32_), .O(new_n36_));
  oai21  g17(.a(x03), .b(x01), .c(x05), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n36_), .c(new_n31_), .O(new_n38_));
  nand2  g19(.a(new_n22_), .b(new_n34_), .O(new_n39_));
  nor2   g20(.a(x06), .b(new_n32_), .O(new_n40_));
  nand4  g21(.a(new_n40_), .b(new_n39_), .c(x13), .d(x09), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n38_), .O(z1));
  inv1   g23(.a(new_n31_), .O(new_n43_));
  nand2  g24(.a(new_n35_), .b(x11), .O(new_n44_));
  nor2   g25(.a(x11), .b(x10), .O(new_n45_));
  nand4  g26(.a(new_n45_), .b(new_n20_), .c(x04), .d(x02), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  oai21  g28(.a(new_n25_), .b(x03), .c(new_n47_), .O(new_n48_));
  inv1   g29(.a(x05), .O(new_n49_));
  nand2  g30(.a(x13), .b(x09), .O(new_n50_));
  aoi21  g31(.a(x04), .b(x02), .c(x11), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n50_), .c(x03), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n25_), .c(new_n49_), .O(new_n53_));
  aoi21  g34(.a(new_n53_), .b(new_n48_), .c(new_n43_), .O(z2));
  nand3  g35(.a(new_n45_), .b(new_n23_), .c(x12), .O(new_n55_));
  inv1   g36(.a(x12), .O(new_n56_));
  aoi21  g37(.a(new_n46_), .b(new_n56_), .c(new_n32_), .O(new_n57_));
  oai21  g38(.a(x07), .b(x03), .c(x05), .O(new_n58_));
  aoi21  g39(.a(new_n57_), .b(new_n55_), .c(new_n58_), .O(new_n59_));
  nand3  g40(.a(x13), .b(x09), .c(x04), .O(new_n60_));
  inv1   g41(.a(new_n60_), .O(new_n61_));
  inv1   g42(.a(x02), .O(new_n62_));
  nand2  g43(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nand3  g44(.a(new_n63_), .b(new_n61_), .c(new_n40_), .O(new_n64_));
  oai21  g45(.a(new_n59_), .b(new_n43_), .c(new_n64_), .O(z3));
  nor2   g46(.a(new_n60_), .b(x06), .O(z4));
endmodule


