// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_;
  inv1   g00(.a(x05), .O(new_n20_));
  nor2   g01(.a(x03), .b(x00), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  inv1   g04(.a(new_n23_), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  and2   g06(.a(x08), .b(x03), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n25_), .c(new_n21_), .O(new_n27_));
  nand2  g08(.a(x04), .b(x03), .O(new_n28_));
  oai21  g09(.a(new_n28_), .b(x08), .c(x05), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x02), .O(new_n30_));
  oai21  g11(.a(new_n27_), .b(new_n20_), .c(new_n30_), .O(z0));
  nor2   g12(.a(x10), .b(x08), .O(new_n32_));
  inv1   g13(.a(new_n32_), .O(new_n33_));
  aoi21  g14(.a(new_n33_), .b(new_n22_), .c(new_n23_), .O(new_n34_));
  inv1   g15(.a(x08), .O(new_n35_));
  nand2  g16(.a(x10), .b(x05), .O(new_n36_));
  aoi21  g17(.a(new_n24_), .b(new_n35_), .c(new_n36_), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n34_), .c(x03), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  inv1   g20(.a(x02), .O(new_n40_));
  nor2   g21(.a(x05), .b(new_n40_), .O(new_n41_));
  nor2   g22(.a(new_n20_), .b(x03), .O(new_n42_));
  aoi21  g23(.a(new_n42_), .b(new_n39_), .c(new_n41_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n38_), .O(z1));
  inv1   g25(.a(x10), .O(new_n45_));
  inv1   g26(.a(x11), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n45_), .c(new_n35_), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n22_), .c(new_n23_), .O(new_n48_));
  nand2  g29(.a(x11), .b(x05), .O(new_n49_));
  aoi21  g30(.a(new_n32_), .b(new_n24_), .c(new_n49_), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n48_), .c(x03), .O(new_n51_));
  inv1   g32(.a(x06), .O(new_n52_));
  aoi21  g33(.a(new_n42_), .b(new_n52_), .c(new_n41_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(z2));
  nand2  g35(.a(new_n47_), .b(new_n22_), .O(new_n55_));
  nand2  g36(.a(new_n22_), .b(x12), .O(new_n56_));
  nand3  g37(.a(new_n56_), .b(new_n55_), .c(new_n24_), .O(new_n57_));
  and2   g38(.a(x12), .b(x05), .O(new_n58_));
  oai21  g39(.a(new_n47_), .b(new_n23_), .c(new_n58_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(x03), .O(new_n61_));
  inv1   g42(.a(x07), .O(new_n62_));
  aoi21  g43(.a(new_n42_), .b(new_n62_), .c(new_n41_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n61_), .O(z3));
  nand3  g45(.a(x13), .b(x09), .c(x04), .O(new_n65_));
  aoi21  g46(.a(new_n20_), .b(new_n40_), .c(new_n65_), .O(z4));
endmodule


