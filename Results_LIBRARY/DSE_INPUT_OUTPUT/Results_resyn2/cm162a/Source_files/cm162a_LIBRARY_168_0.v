// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_;
  nor2   g00(.a(x06), .b(x03), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  and2   g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nor2   g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nor2   g05(.a(new_n24_), .b(new_n21_), .O(new_n25_));
  nor2   g06(.a(new_n23_), .b(x08), .O(new_n26_));
  oai21  g07(.a(new_n26_), .b(new_n25_), .c(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  aoi21  g12(.a(new_n31_), .b(new_n27_), .c(new_n20_), .O(z0));
  nand3  g13(.a(new_n22_), .b(x04), .c(x02), .O(new_n33_));
  inv1   g14(.a(new_n23_), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  oai21  g17(.a(new_n23_), .b(x08), .c(x10), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(new_n36_), .c(new_n33_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x03), .O(new_n39_));
  nand2  g20(.a(x06), .b(x01), .O(new_n40_));
  aoi21  g21(.a(new_n40_), .b(new_n29_), .c(new_n30_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n39_), .O(z1));
  nor3   g23(.a(x11), .b(x10), .c(x08), .O(new_n43_));
  oai21  g24(.a(new_n43_), .b(new_n22_), .c(new_n34_), .O(new_n44_));
  oai21  g25(.a(new_n36_), .b(new_n22_), .c(x11), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(x03), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(x05), .c(new_n20_), .O(z2));
  inv1   g29(.a(x12), .O(new_n49_));
  nand2  g30(.a(new_n44_), .b(new_n49_), .O(new_n50_));
  nand3  g31(.a(new_n43_), .b(new_n24_), .c(x12), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n50_), .c(new_n30_), .O(new_n52_));
  oai21  g33(.a(x07), .b(x03), .c(x05), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x06), .O(new_n54_));
  oai21  g35(.a(new_n52_), .b(new_n29_), .c(new_n54_), .O(z3));
  inv1   g36(.a(new_n20_), .O(new_n56_));
  nand2  g37(.a(new_n22_), .b(x04), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(new_n56_), .O(z4));
endmodule


