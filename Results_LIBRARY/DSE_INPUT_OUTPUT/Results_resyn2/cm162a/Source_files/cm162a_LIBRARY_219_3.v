// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_;
  nand2  g00(.a(x04), .b(x02), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x08), .O(new_n24_));
  inv1   g05(.a(x08), .O(new_n25_));
  nand2  g06(.a(new_n21_), .b(new_n25_), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  nand2  g08(.a(x05), .b(new_n27_), .O(new_n28_));
  inv1   g09(.a(new_n28_), .O(new_n29_));
  nand4  g10(.a(new_n29_), .b(new_n26_), .c(new_n24_), .d(x03), .O(z0));
  inv1   g11(.a(x03), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n25_), .O(new_n33_));
  aoi21  g14(.a(new_n33_), .b(new_n22_), .c(new_n20_), .O(new_n34_));
  nand4  g15(.a(new_n22_), .b(new_n25_), .c(x04), .d(x02), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(x10), .c(new_n34_), .O(new_n36_));
  nor2   g17(.a(x03), .b(x01), .O(new_n37_));
  nor2   g18(.a(new_n37_), .b(new_n28_), .O(new_n38_));
  oai21  g19(.a(new_n36_), .b(new_n31_), .c(new_n38_), .O(z1));
  nor2   g20(.a(new_n34_), .b(x11), .O(new_n40_));
  nand3  g21(.a(x11), .b(new_n32_), .c(new_n25_), .O(new_n41_));
  oai21  g22(.a(new_n41_), .b(new_n23_), .c(x03), .O(new_n42_));
  nor2   g23(.a(x06), .b(x03), .O(new_n43_));
  nor2   g24(.a(new_n43_), .b(new_n28_), .O(new_n44_));
  oai21  g25(.a(new_n42_), .b(new_n40_), .c(new_n44_), .O(z2));
  inv1   g26(.a(x11), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n32_), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n35_), .c(x12), .O(new_n48_));
  inv1   g29(.a(x12), .O(new_n49_));
  nand4  g30(.a(new_n49_), .b(new_n46_), .c(new_n32_), .d(new_n25_), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n22_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n21_), .O(new_n52_));
  nand3  g33(.a(new_n52_), .b(new_n48_), .c(x03), .O(new_n53_));
  aoi21  g34(.a(x07), .b(new_n31_), .c(x00), .O(new_n54_));
  nand2  g35(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(x05), .O(z3));
  inv1   g37(.a(x04), .O(new_n57_));
  nand2  g38(.a(x05), .b(x00), .O(new_n58_));
  oai21  g39(.a(new_n22_), .b(new_n57_), .c(new_n58_), .O(z4));
endmodule


