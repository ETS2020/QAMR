// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_;
  inv1   g00(.a(x00), .O(new_n20_));
  nor2   g01(.a(x03), .b(new_n20_), .O(new_n21_));
  and2   g02(.a(x04), .b(x02), .O(new_n22_));
  nand2  g03(.a(x13), .b(x09), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand2  g05(.a(x04), .b(x02), .O(new_n25_));
  oai21  g06(.a(new_n25_), .b(x08), .c(x03), .O(new_n26_));
  aoi21  g07(.a(new_n24_), .b(x08), .c(new_n26_), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n21_), .c(x05), .O(z0));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x08), .O(new_n30_));
  inv1   g11(.a(x10), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n23_), .O(new_n33_));
  nand3  g14(.a(new_n23_), .b(new_n22_), .c(new_n30_), .O(new_n34_));
  aoi22  g15(.a(new_n34_), .b(x10), .c(new_n33_), .d(new_n22_), .O(new_n35_));
  inv1   g16(.a(x01), .O(new_n36_));
  inv1   g17(.a(x05), .O(new_n37_));
  aoi21  g18(.a(new_n29_), .b(new_n36_), .c(new_n37_), .O(new_n38_));
  oai21  g19(.a(new_n35_), .b(new_n29_), .c(new_n38_), .O(z1));
  oai21  g20(.a(x06), .b(x03), .c(x05), .O(new_n40_));
  oai21  g21(.a(x09), .b(x05), .c(new_n40_), .O(new_n41_));
  nand3  g22(.a(x11), .b(x04), .c(x02), .O(new_n42_));
  oai21  g23(.a(new_n42_), .b(new_n32_), .c(x05), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n23_), .O(new_n44_));
  inv1   g25(.a(x11), .O(new_n45_));
  inv1   g26(.a(new_n23_), .O(new_n46_));
  nor2   g27(.a(x10), .b(x08), .O(new_n47_));
  oai21  g28(.a(new_n47_), .b(new_n46_), .c(new_n22_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand3  g30(.a(new_n49_), .b(new_n44_), .c(x03), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n41_), .O(z2));
  inv1   g32(.a(x12), .O(new_n52_));
  nand4  g33(.a(new_n52_), .b(new_n45_), .c(new_n31_), .d(new_n30_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n23_), .O(new_n54_));
  nor2   g35(.a(x11), .b(x10), .O(new_n55_));
  nand4  g36(.a(new_n55_), .b(new_n23_), .c(new_n22_), .d(new_n30_), .O(new_n56_));
  aoi22  g37(.a(new_n56_), .b(x12), .c(new_n54_), .d(new_n22_), .O(new_n57_));
  inv1   g38(.a(x07), .O(new_n58_));
  aoi21  g39(.a(new_n58_), .b(new_n29_), .c(new_n37_), .O(new_n59_));
  oai21  g40(.a(new_n57_), .b(new_n29_), .c(new_n59_), .O(z3));
  inv1   g41(.a(x04), .O(new_n61_));
  oai22  g42(.a(new_n23_), .b(new_n61_), .c(x09), .d(x05), .O(z4));
endmodule


