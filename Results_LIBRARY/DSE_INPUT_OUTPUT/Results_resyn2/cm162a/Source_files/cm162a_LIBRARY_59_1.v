// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n62_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n21_), .O(new_n24_));
  nor2   g05(.a(new_n22_), .b(x08), .O(new_n25_));
  aoi21  g06(.a(new_n24_), .b(x08), .c(new_n25_), .O(new_n26_));
  inv1   g07(.a(x00), .O(new_n27_));
  inv1   g08(.a(x05), .O(new_n28_));
  aoi21  g09(.a(new_n20_), .b(new_n27_), .c(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n26_), .b(new_n20_), .c(new_n29_), .O(z0));
  inv1   g11(.a(x08), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand3  g14(.a(x13), .b(x09), .c(x05), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(new_n33_), .c(new_n22_), .O(new_n35_));
  nand2  g16(.a(x10), .b(x05), .O(new_n36_));
  nor2   g17(.a(new_n36_), .b(new_n25_), .O(new_n37_));
  oai21  g18(.a(new_n37_), .b(new_n35_), .c(x03), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  nor2   g20(.a(x08), .b(x05), .O(new_n40_));
  nor2   g21(.a(new_n28_), .b(x03), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(new_n39_), .c(new_n40_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n38_), .O(z1));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n32_), .c(new_n31_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  nand4  g27(.a(new_n23_), .b(new_n21_), .c(new_n32_), .d(new_n31_), .O(new_n47_));
  nor2   g28(.a(new_n44_), .b(new_n28_), .O(new_n48_));
  aoi22  g29(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n23_), .O(new_n49_));
  inv1   g30(.a(x06), .O(new_n50_));
  aoi21  g31(.a(new_n41_), .b(new_n50_), .c(new_n40_), .O(new_n51_));
  oai21  g32(.a(new_n49_), .b(new_n20_), .c(new_n51_), .O(z2));
  nand2  g33(.a(new_n34_), .b(x12), .O(new_n53_));
  nand3  g34(.a(new_n53_), .b(new_n46_), .c(new_n23_), .O(new_n54_));
  and2   g35(.a(x12), .b(x05), .O(new_n55_));
  oai21  g36(.a(new_n45_), .b(new_n22_), .c(new_n55_), .O(new_n56_));
  nand2  g37(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand2  g38(.a(new_n57_), .b(x03), .O(new_n58_));
  inv1   g39(.a(x07), .O(new_n59_));
  aoi21  g40(.a(new_n41_), .b(new_n59_), .c(new_n40_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(new_n58_), .O(z3));
  nand3  g42(.a(x13), .b(x09), .c(x04), .O(new_n62_));
  aoi21  g43(.a(x08), .b(new_n28_), .c(new_n62_), .O(z4));
endmodule


