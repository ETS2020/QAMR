// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_;
  nand4  g00(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n21_), .O(new_n24_));
  nand2  g05(.a(new_n22_), .b(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(new_n24_), .c(new_n20_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  inv1   g09(.a(x03), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(new_n27_), .O(z0));
  nand2  g13(.a(x13), .b(x09), .O(new_n33_));
  nor2   g14(.a(x10), .b(x08), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(x05), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n33_), .c(new_n22_), .O(new_n36_));
  inv1   g17(.a(x10), .O(new_n37_));
  oai21  g18(.a(new_n22_), .b(x08), .c(x05), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(new_n33_), .c(new_n37_), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n36_), .c(x03), .O(new_n40_));
  inv1   g21(.a(x01), .O(new_n41_));
  inv1   g22(.a(x13), .O(new_n42_));
  nor2   g23(.a(new_n42_), .b(x05), .O(new_n43_));
  nor2   g24(.a(new_n30_), .b(x03), .O(new_n44_));
  aoi21  g25(.a(new_n44_), .b(new_n41_), .c(new_n43_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n40_), .O(z1));
  nand2  g27(.a(new_n35_), .b(new_n33_), .O(new_n47_));
  aoi21  g28(.a(new_n47_), .b(new_n23_), .c(x11), .O(new_n48_));
  aoi21  g29(.a(new_n34_), .b(new_n23_), .c(new_n30_), .O(new_n49_));
  nand2  g30(.a(new_n33_), .b(x11), .O(new_n50_));
  oai21  g31(.a(new_n50_), .b(new_n49_), .c(x03), .O(new_n51_));
  inv1   g32(.a(x06), .O(new_n52_));
  aoi21  g33(.a(new_n44_), .b(new_n52_), .c(new_n43_), .O(new_n53_));
  oai21  g34(.a(new_n51_), .b(new_n48_), .c(new_n53_), .O(z2));
  inv1   g35(.a(x11), .O(new_n55_));
  inv1   g36(.a(x12), .O(new_n56_));
  nand4  g37(.a(new_n34_), .b(new_n23_), .c(new_n56_), .d(new_n55_), .O(new_n57_));
  nand4  g38(.a(new_n34_), .b(new_n55_), .c(x04), .d(x02), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(x12), .O(new_n59_));
  nand3  g40(.a(new_n59_), .b(new_n57_), .c(new_n20_), .O(new_n60_));
  nand2  g41(.a(new_n60_), .b(x03), .O(new_n61_));
  inv1   g42(.a(x07), .O(new_n62_));
  aoi21  g43(.a(new_n62_), .b(new_n29_), .c(new_n30_), .O(new_n63_));
  nand2  g44(.a(new_n63_), .b(new_n61_), .O(z3));
  inv1   g45(.a(x04), .O(new_n65_));
  oai22  g46(.a(new_n33_), .b(new_n65_), .c(x13), .d(x05), .O(z4));
endmodule


