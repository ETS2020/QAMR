// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_;
  inv1   g00(.a(x10), .O(new_n20_));
  nor2   g01(.a(new_n20_), .b(x05), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  nor2   g03(.a(new_n22_), .b(x08), .O(new_n23_));
  inv1   g04(.a(x08), .O(new_n24_));
  and2   g05(.a(x04), .b(x02), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n23_), .c(x03), .O(new_n28_));
  inv1   g09(.a(x00), .O(new_n29_));
  inv1   g10(.a(x03), .O(new_n30_));
  inv1   g11(.a(x05), .O(new_n31_));
  aoi21  g12(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  aoi21  g13(.a(new_n32_), .b(new_n28_), .c(new_n21_), .O(z0));
  nor2   g14(.a(x10), .b(x08), .O(new_n34_));
  inv1   g15(.a(new_n34_), .O(new_n35_));
  nand3  g16(.a(x13), .b(x09), .c(x05), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(new_n35_), .c(new_n22_), .O(new_n37_));
  nand2  g18(.a(x10), .b(x05), .O(new_n38_));
  nor2   g19(.a(new_n38_), .b(new_n23_), .O(new_n39_));
  oai21  g20(.a(new_n39_), .b(new_n37_), .c(x03), .O(new_n40_));
  inv1   g21(.a(x01), .O(new_n41_));
  nor2   g22(.a(new_n31_), .b(x03), .O(new_n42_));
  nor2   g23(.a(x10), .b(x05), .O(new_n43_));
  aoi21  g24(.a(new_n42_), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  nand2  g25(.a(new_n44_), .b(new_n40_), .O(z1));
  inv1   g26(.a(x11), .O(new_n46_));
  nand3  g27(.a(new_n46_), .b(new_n20_), .c(new_n24_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n26_), .O(new_n48_));
  nand3  g29(.a(new_n34_), .b(new_n26_), .c(new_n25_), .O(new_n49_));
  aoi22  g30(.a(new_n49_), .b(x11), .c(new_n48_), .d(new_n25_), .O(new_n50_));
  inv1   g31(.a(x06), .O(new_n51_));
  aoi21  g32(.a(new_n51_), .b(new_n30_), .c(new_n31_), .O(new_n52_));
  oai21  g33(.a(new_n50_), .b(new_n30_), .c(new_n52_), .O(z2));
  inv1   g34(.a(x12), .O(new_n54_));
  nand3  g35(.a(new_n34_), .b(new_n54_), .c(new_n46_), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n36_), .c(new_n22_), .O(new_n56_));
  nor3   g37(.a(x11), .b(x10), .c(x08), .O(new_n57_));
  nand2  g38(.a(x12), .b(x05), .O(new_n58_));
  aoi21  g39(.a(new_n57_), .b(new_n25_), .c(new_n58_), .O(new_n59_));
  oai21  g40(.a(new_n59_), .b(new_n56_), .c(x03), .O(new_n60_));
  inv1   g41(.a(x07), .O(new_n61_));
  aoi21  g42(.a(new_n42_), .b(new_n61_), .c(new_n43_), .O(new_n62_));
  nand2  g43(.a(new_n62_), .b(new_n60_), .O(z3));
  inv1   g44(.a(x04), .O(new_n64_));
  nor3   g45(.a(new_n26_), .b(new_n21_), .c(new_n64_), .O(z4));
endmodule


