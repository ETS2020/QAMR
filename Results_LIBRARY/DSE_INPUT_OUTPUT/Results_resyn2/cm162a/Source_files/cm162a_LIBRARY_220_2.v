// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_;
  nand4  g00(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  inv1   g03(.a(new_n22_), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n21_), .O(new_n24_));
  nand2  g05(.a(new_n22_), .b(x08), .O(new_n25_));
  nand3  g06(.a(new_n25_), .b(new_n24_), .c(new_n20_), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(x03), .O(new_n27_));
  nor2   g08(.a(x03), .b(x00), .O(new_n28_));
  inv1   g09(.a(x13), .O(new_n29_));
  nand2  g10(.a(new_n29_), .b(x11), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x05), .O(new_n31_));
  nor2   g12(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n27_), .O(z0));
  nand2  g14(.a(new_n24_), .b(x10), .O(new_n34_));
  nor2   g15(.a(x10), .b(x08), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n23_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n34_), .c(new_n20_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x03), .O(new_n38_));
  nor2   g19(.a(x03), .b(x01), .O(new_n39_));
  nor2   g20(.a(new_n39_), .b(new_n31_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(new_n38_), .O(z1));
  nand2  g22(.a(x13), .b(x09), .O(new_n42_));
  inv1   g23(.a(x10), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n43_), .c(new_n21_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n42_), .c(new_n22_), .O(new_n46_));
  nand2  g27(.a(x13), .b(x11), .O(new_n47_));
  aoi21  g28(.a(new_n35_), .b(new_n23_), .c(new_n47_), .O(new_n48_));
  oai21  g29(.a(new_n48_), .b(new_n46_), .c(x03), .O(new_n49_));
  oai21  g30(.a(x06), .b(x03), .c(x05), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n30_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n49_), .O(z2));
  nand2  g33(.a(new_n45_), .b(new_n42_), .O(new_n53_));
  nand2  g34(.a(new_n42_), .b(x12), .O(new_n54_));
  nand3  g35(.a(new_n54_), .b(new_n53_), .c(new_n23_), .O(new_n55_));
  inv1   g36(.a(x12), .O(new_n56_));
  aoi21  g37(.a(new_n29_), .b(x11), .c(new_n56_), .O(new_n57_));
  oai21  g38(.a(new_n45_), .b(new_n22_), .c(new_n57_), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  nand2  g40(.a(new_n59_), .b(x03), .O(new_n60_));
  nor2   g41(.a(new_n56_), .b(new_n44_), .O(new_n61_));
  oai21  g42(.a(new_n61_), .b(x05), .c(x03), .O(new_n62_));
  nand2  g43(.a(x07), .b(x05), .O(new_n63_));
  nand3  g44(.a(new_n63_), .b(new_n62_), .c(new_n30_), .O(new_n64_));
  nand2  g45(.a(new_n64_), .b(new_n60_), .O(z3));
  inv1   g46(.a(x04), .O(new_n66_));
  oai21  g47(.a(new_n42_), .b(new_n66_), .c(new_n30_), .O(z4));
endmodule


