// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_;
  inv1   g00(.a(x05), .O(new_n20_));
  nand2  g01(.a(x07), .b(new_n20_), .O(new_n21_));
  nand2  g02(.a(x13), .b(x09), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  inv1   g04(.a(new_n23_), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  oai21  g06(.a(new_n23_), .b(x08), .c(x03), .O(new_n26_));
  aoi21  g07(.a(new_n25_), .b(x08), .c(new_n26_), .O(new_n27_));
  inv1   g08(.a(x00), .O(new_n28_));
  oai21  g09(.a(x03), .b(new_n28_), .c(x05), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n27_), .c(new_n21_), .O(z0));
  inv1   g11(.a(x08), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  aoi21  g13(.a(new_n24_), .b(new_n31_), .c(new_n32_), .O(new_n33_));
  nand2  g14(.a(new_n32_), .b(new_n31_), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(new_n22_), .c(new_n23_), .O(new_n35_));
  oai21  g16(.a(new_n35_), .b(new_n33_), .c(x03), .O(new_n36_));
  inv1   g17(.a(x01), .O(new_n37_));
  inv1   g18(.a(x03), .O(new_n38_));
  aoi21  g19(.a(new_n38_), .b(new_n37_), .c(new_n20_), .O(new_n39_));
  nand2  g20(.a(new_n39_), .b(new_n36_), .O(z1));
  inv1   g21(.a(x11), .O(new_n41_));
  nand3  g22(.a(new_n41_), .b(new_n32_), .c(new_n31_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(new_n22_), .O(new_n43_));
  nand2  g24(.a(new_n43_), .b(new_n24_), .O(new_n44_));
  oai21  g25(.a(new_n34_), .b(new_n25_), .c(x11), .O(new_n45_));
  nand3  g26(.a(new_n45_), .b(new_n44_), .c(x03), .O(new_n46_));
  aoi21  g27(.a(x06), .b(new_n38_), .c(new_n20_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n21_), .O(z2));
  inv1   g30(.a(x12), .O(new_n50_));
  nand4  g31(.a(new_n22_), .b(x12), .c(x04), .d(x02), .O(new_n51_));
  oai21  g32(.a(new_n51_), .b(new_n42_), .c(x03), .O(new_n52_));
  aoi21  g33(.a(new_n44_), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  nor2   g34(.a(x07), .b(x03), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n53_), .c(x05), .O(new_n55_));
  nand2  g36(.a(new_n55_), .b(new_n21_), .O(z3));
  inv1   g37(.a(x04), .O(new_n57_));
  nor2   g38(.a(x07), .b(x05), .O(new_n58_));
  nor3   g39(.a(new_n58_), .b(new_n22_), .c(new_n57_), .O(z4));
endmodule


