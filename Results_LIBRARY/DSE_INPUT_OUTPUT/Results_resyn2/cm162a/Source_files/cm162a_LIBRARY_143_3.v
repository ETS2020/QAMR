// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_;
  nand2  g00(.a(x04), .b(x02), .O(new_n20_));
  nand2  g01(.a(new_n20_), .b(x08), .O(new_n21_));
  inv1   g02(.a(x08), .O(new_n22_));
  and2   g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  and2   g05(.a(x13), .b(x09), .O(new_n25_));
  nand2  g06(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand3  g07(.a(new_n26_), .b(new_n24_), .c(new_n21_), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x03), .O(new_n28_));
  nor2   g09(.a(x03), .b(x00), .O(new_n29_));
  nand2  g10(.a(x12), .b(x11), .O(new_n30_));
  nand2  g11(.a(new_n30_), .b(x05), .O(new_n31_));
  nor2   g12(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  nand2  g13(.a(new_n32_), .b(new_n28_), .O(z0));
  inv1   g14(.a(new_n30_), .O(new_n34_));
  oai21  g15(.a(new_n20_), .b(x08), .c(x10), .O(new_n35_));
  nor2   g16(.a(x10), .b(x08), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n23_), .O(new_n37_));
  nand3  g18(.a(new_n37_), .b(new_n35_), .c(new_n26_), .O(new_n38_));
  nand2  g19(.a(new_n38_), .b(x03), .O(new_n39_));
  inv1   g20(.a(x01), .O(new_n40_));
  inv1   g21(.a(x03), .O(new_n41_));
  inv1   g22(.a(x05), .O(new_n42_));
  aoi21  g23(.a(new_n41_), .b(new_n40_), .c(new_n42_), .O(new_n43_));
  aoi21  g24(.a(new_n43_), .b(new_n39_), .c(new_n34_), .O(z1));
  nand2  g25(.a(x13), .b(x09), .O(new_n45_));
  inv1   g26(.a(x10), .O(new_n46_));
  inv1   g27(.a(x11), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n46_), .c(new_n22_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand3  g30(.a(new_n36_), .b(new_n45_), .c(new_n23_), .O(new_n50_));
  aoi22  g31(.a(new_n50_), .b(x11), .c(new_n49_), .d(new_n23_), .O(new_n51_));
  nor2   g32(.a(x06), .b(x03), .O(new_n52_));
  nor2   g33(.a(new_n52_), .b(new_n31_), .O(new_n53_));
  oai21  g34(.a(new_n51_), .b(new_n41_), .c(new_n53_), .O(z2));
  aoi21  g35(.a(new_n49_), .b(new_n23_), .c(x12), .O(new_n55_));
  inv1   g36(.a(x12), .O(new_n56_));
  oai21  g37(.a(new_n50_), .b(new_n56_), .c(x03), .O(new_n57_));
  nor2   g38(.a(x07), .b(x03), .O(new_n58_));
  nor2   g39(.a(new_n58_), .b(new_n31_), .O(new_n59_));
  oai21  g40(.a(new_n57_), .b(new_n55_), .c(new_n59_), .O(z3));
  nand3  g41(.a(new_n30_), .b(new_n25_), .c(x04), .O(new_n61_));
  inv1   g42(.a(new_n61_), .O(z4));
endmodule


