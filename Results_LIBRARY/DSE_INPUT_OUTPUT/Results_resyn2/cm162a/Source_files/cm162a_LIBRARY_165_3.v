// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_;
  nand4  g00(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  nand2  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  inv1   g03(.a(x08), .O(new_n23_));
  nand3  g04(.a(new_n23_), .b(x04), .c(x02), .O(new_n24_));
  nand4  g05(.a(new_n24_), .b(new_n22_), .c(new_n20_), .d(x03), .O(new_n25_));
  inv1   g06(.a(x03), .O(new_n26_));
  aoi21  g07(.a(new_n26_), .b(x00), .c(x07), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand2  g09(.a(new_n28_), .b(x05), .O(z0));
  nand2  g10(.a(new_n24_), .b(x10), .O(new_n30_));
  inv1   g11(.a(x10), .O(new_n31_));
  nand4  g12(.a(new_n31_), .b(new_n23_), .c(x04), .d(x02), .O(new_n32_));
  nand4  g13(.a(new_n32_), .b(new_n30_), .c(new_n20_), .d(x03), .O(new_n33_));
  aoi21  g14(.a(new_n26_), .b(x01), .c(x07), .O(new_n34_));
  nand2  g15(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(x05), .O(z1));
  inv1   g17(.a(x11), .O(new_n37_));
  nor2   g18(.a(x10), .b(x08), .O(new_n38_));
  nand4  g19(.a(new_n38_), .b(new_n37_), .c(x04), .d(x02), .O(new_n39_));
  nand2  g20(.a(new_n32_), .b(x11), .O(new_n40_));
  nand3  g21(.a(new_n40_), .b(new_n39_), .c(new_n20_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(x03), .O(new_n42_));
  inv1   g23(.a(x05), .O(new_n43_));
  nor2   g24(.a(x06), .b(x03), .O(new_n44_));
  nor3   g25(.a(new_n44_), .b(x07), .c(new_n43_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n42_), .O(z2));
  nor2   g27(.a(x07), .b(new_n43_), .O(new_n47_));
  nand2  g28(.a(new_n39_), .b(x12), .O(new_n48_));
  nand2  g29(.a(x13), .b(x09), .O(new_n49_));
  nand2  g30(.a(new_n38_), .b(new_n37_), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g32(.a(new_n49_), .b(x12), .O(new_n52_));
  inv1   g33(.a(x04), .O(new_n53_));
  nor2   g34(.a(x12), .b(x02), .O(new_n54_));
  nor2   g35(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n52_), .c(new_n51_), .O(new_n56_));
  nand4  g37(.a(new_n56_), .b(new_n48_), .c(new_n47_), .d(x03), .O(z3));
  nand2  g38(.a(x07), .b(x05), .O(new_n58_));
  oai21  g39(.a(new_n49_), .b(new_n53_), .c(new_n58_), .O(z4));
endmodule


