// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_;
  nand2  g00(.a(x04), .b(x02), .O(new_n20_));
  nand2  g01(.a(x13), .b(x09), .O(new_n21_));
  aoi21  g02(.a(new_n21_), .b(x08), .c(new_n20_), .O(new_n22_));
  nand2  g03(.a(new_n20_), .b(x08), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(x03), .O(new_n24_));
  inv1   g05(.a(x03), .O(new_n25_));
  aoi21  g06(.a(new_n25_), .b(x00), .c(x10), .O(new_n26_));
  oai21  g07(.a(new_n24_), .b(new_n22_), .c(new_n26_), .O(new_n27_));
  nand2  g08(.a(new_n27_), .b(x05), .O(z0));
  nand2  g09(.a(new_n22_), .b(x03), .O(new_n29_));
  nor2   g10(.a(x03), .b(x01), .O(new_n30_));
  inv1   g11(.a(x10), .O(new_n31_));
  nand2  g12(.a(new_n31_), .b(x05), .O(new_n32_));
  nor2   g13(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n29_), .O(z1));
  oai21  g15(.a(new_n20_), .b(x08), .c(x11), .O(new_n35_));
  inv1   g16(.a(x08), .O(new_n36_));
  inv1   g17(.a(x11), .O(new_n37_));
  nand4  g18(.a(new_n37_), .b(new_n36_), .c(x04), .d(x02), .O(new_n38_));
  nand4  g19(.a(x13), .b(x09), .c(x04), .d(x02), .O(new_n39_));
  nand4  g20(.a(new_n39_), .b(new_n38_), .c(new_n35_), .d(x03), .O(new_n40_));
  aoi21  g21(.a(x06), .b(new_n25_), .c(x10), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g23(.a(new_n42_), .b(x05), .O(z2));
  inv1   g24(.a(x12), .O(new_n44_));
  nor2   g25(.a(x11), .b(x08), .O(new_n45_));
  nand4  g26(.a(new_n45_), .b(new_n44_), .c(x04), .d(x02), .O(new_n46_));
  nand2  g27(.a(new_n38_), .b(x12), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n46_), .c(new_n39_), .O(new_n48_));
  nand2  g29(.a(new_n48_), .b(x03), .O(new_n49_));
  nor2   g30(.a(x07), .b(x03), .O(new_n50_));
  nor2   g31(.a(new_n50_), .b(new_n32_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(new_n49_), .O(z3));
  inv1   g33(.a(x04), .O(new_n53_));
  nand2  g34(.a(x10), .b(x05), .O(new_n54_));
  oai21  g35(.a(new_n21_), .b(new_n53_), .c(new_n54_), .O(z4));
endmodule


