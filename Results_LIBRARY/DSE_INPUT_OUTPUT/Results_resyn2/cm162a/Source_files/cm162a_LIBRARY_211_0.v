// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_;
  inv1   g00(.a(x08), .O(new_n20_));
  inv1   g01(.a(x03), .O(new_n21_));
  nand2  g02(.a(x04), .b(x02), .O(new_n22_));
  oai21  g03(.a(new_n22_), .b(new_n21_), .c(x11), .O(new_n23_));
  nand2  g04(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  and2   g05(.a(x04), .b(x02), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand3  g08(.a(new_n27_), .b(x08), .c(x03), .O(new_n28_));
  inv1   g09(.a(x00), .O(new_n29_));
  inv1   g10(.a(x05), .O(new_n30_));
  aoi21  g11(.a(new_n21_), .b(new_n29_), .c(new_n30_), .O(new_n31_));
  nand3  g12(.a(new_n31_), .b(new_n28_), .c(new_n24_), .O(z0));
  inv1   g13(.a(x10), .O(new_n33_));
  nand2  g14(.a(new_n33_), .b(new_n20_), .O(new_n34_));
  aoi21  g15(.a(new_n34_), .b(new_n26_), .c(new_n22_), .O(new_n35_));
  nand4  g16(.a(new_n26_), .b(new_n25_), .c(x11), .d(new_n20_), .O(new_n36_));
  aoi21  g17(.a(new_n36_), .b(x10), .c(new_n35_), .O(new_n37_));
  nor2   g18(.a(x03), .b(x01), .O(new_n38_));
  nor2   g19(.a(x11), .b(x08), .O(new_n39_));
  inv1   g20(.a(new_n39_), .O(new_n40_));
  nand2  g21(.a(new_n40_), .b(x05), .O(new_n41_));
  nor2   g22(.a(new_n41_), .b(new_n38_), .O(new_n42_));
  oai21  g23(.a(new_n37_), .b(new_n21_), .c(new_n42_), .O(z1));
  nor2   g24(.a(new_n34_), .b(new_n27_), .O(new_n44_));
  nand3  g25(.a(new_n25_), .b(x13), .c(x09), .O(new_n45_));
  nor2   g26(.a(x11), .b(new_n20_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g28(.a(new_n47_), .b(x03), .O(new_n48_));
  nor2   g29(.a(x06), .b(x03), .O(new_n49_));
  nor2   g30(.a(new_n49_), .b(new_n41_), .O(new_n50_));
  oai21  g31(.a(new_n48_), .b(new_n44_), .c(new_n50_), .O(z2));
  inv1   g32(.a(x12), .O(new_n52_));
  nand2  g33(.a(new_n45_), .b(new_n52_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(x03), .O(new_n54_));
  inv1   g35(.a(x07), .O(new_n55_));
  aoi21  g36(.a(new_n55_), .b(new_n21_), .c(new_n30_), .O(new_n56_));
  aoi21  g37(.a(new_n56_), .b(new_n54_), .c(new_n39_), .O(z3));
  inv1   g38(.a(x04), .O(new_n58_));
  oai21  g39(.a(new_n26_), .b(new_n58_), .c(new_n40_), .O(z4));
endmodule


