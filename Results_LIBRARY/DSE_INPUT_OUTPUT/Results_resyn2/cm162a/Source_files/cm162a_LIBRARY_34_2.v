// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n57_;
  nand2  g00(.a(x07), .b(x05), .O(new_n20_));
  inv1   g01(.a(new_n20_), .O(new_n21_));
  inv1   g02(.a(x00), .O(new_n22_));
  nor2   g03(.a(x03), .b(new_n22_), .O(new_n23_));
  nand2  g04(.a(x04), .b(x02), .O(new_n24_));
  inv1   g05(.a(new_n24_), .O(new_n25_));
  nand2  g06(.a(x13), .b(x09), .O(new_n26_));
  nand2  g07(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  oai21  g08(.a(new_n24_), .b(x08), .c(x03), .O(new_n28_));
  aoi21  g09(.a(new_n27_), .b(x08), .c(new_n28_), .O(new_n29_));
  oai21  g10(.a(new_n29_), .b(new_n23_), .c(new_n21_), .O(z0));
  inv1   g11(.a(x08), .O(new_n31_));
  inv1   g12(.a(x10), .O(new_n32_));
  nand4  g13(.a(new_n32_), .b(new_n31_), .c(x04), .d(x02), .O(new_n33_));
  oai21  g14(.a(new_n24_), .b(x08), .c(x10), .O(new_n34_));
  inv1   g15(.a(new_n26_), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n25_), .O(new_n36_));
  nand3  g17(.a(new_n36_), .b(new_n34_), .c(new_n33_), .O(new_n37_));
  nand2  g18(.a(new_n37_), .b(x03), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  inv1   g20(.a(x03), .O(new_n40_));
  aoi21  g21(.a(new_n40_), .b(new_n39_), .c(new_n20_), .O(new_n41_));
  nand2  g22(.a(new_n41_), .b(new_n38_), .O(z1));
  oai21  g23(.a(new_n33_), .b(new_n35_), .c(x11), .O(new_n43_));
  inv1   g24(.a(x11), .O(new_n44_));
  nand3  g25(.a(new_n44_), .b(new_n32_), .c(new_n31_), .O(new_n45_));
  nand2  g26(.a(new_n45_), .b(new_n26_), .O(new_n46_));
  nand2  g27(.a(new_n46_), .b(new_n25_), .O(new_n47_));
  nand3  g28(.a(new_n47_), .b(new_n43_), .c(x03), .O(new_n48_));
  inv1   g29(.a(x07), .O(new_n49_));
  aoi21  g30(.a(x06), .b(new_n40_), .c(new_n49_), .O(new_n50_));
  nand2  g31(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g32(.a(new_n51_), .b(x05), .O(z2));
  aoi21  g33(.a(new_n46_), .b(new_n25_), .c(x12), .O(new_n53_));
  nand3  g34(.a(new_n26_), .b(new_n25_), .c(x12), .O(new_n54_));
  oai21  g35(.a(new_n54_), .b(new_n45_), .c(x03), .O(new_n55_));
  oai21  g36(.a(new_n55_), .b(new_n53_), .c(new_n21_), .O(z3));
  nand2  g37(.a(new_n35_), .b(x04), .O(new_n57_));
  aoi21  g38(.a(new_n49_), .b(x05), .c(new_n57_), .O(z4));
endmodule


