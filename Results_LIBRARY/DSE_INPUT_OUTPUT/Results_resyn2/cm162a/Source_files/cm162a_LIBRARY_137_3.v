// Benchmark "FAU" written by ABC on Sat Aug  8 23:02:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n63_;
  inv1   g00(.a(x06), .O(new_n20_));
  inv1   g01(.a(x08), .O(new_n21_));
  nor2   g02(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g03(.a(x04), .b(x02), .O(new_n23_));
  nand2  g04(.a(x13), .b(x09), .O(new_n24_));
  aoi21  g05(.a(new_n24_), .b(x08), .c(new_n23_), .O(new_n25_));
  inv1   g06(.a(new_n23_), .O(new_n26_));
  nor2   g07(.a(new_n26_), .b(new_n21_), .O(new_n27_));
  oai21  g08(.a(new_n27_), .b(new_n25_), .c(x03), .O(new_n28_));
  inv1   g09(.a(x00), .O(new_n29_));
  inv1   g10(.a(x03), .O(new_n30_));
  inv1   g11(.a(x05), .O(new_n31_));
  aoi21  g12(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  aoi21  g13(.a(new_n32_), .b(new_n28_), .c(new_n22_), .O(z0));
  inv1   g14(.a(x10), .O(new_n34_));
  nand2  g15(.a(new_n24_), .b(x08), .O(new_n35_));
  nand2  g16(.a(new_n35_), .b(new_n26_), .O(new_n36_));
  nand2  g17(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand4  g18(.a(new_n24_), .b(new_n26_), .c(x10), .d(new_n21_), .O(new_n38_));
  nand3  g19(.a(new_n38_), .b(new_n37_), .c(x03), .O(new_n39_));
  inv1   g20(.a(x01), .O(new_n40_));
  aoi21  g21(.a(new_n30_), .b(new_n40_), .c(new_n31_), .O(new_n41_));
  aoi21  g22(.a(new_n41_), .b(new_n39_), .c(new_n22_), .O(z1));
  nand2  g23(.a(x05), .b(new_n30_), .O(new_n43_));
  aoi21  g24(.a(new_n43_), .b(new_n21_), .c(new_n20_), .O(new_n44_));
  nand2  g25(.a(new_n34_), .b(new_n21_), .O(new_n45_));
  aoi21  g26(.a(new_n45_), .b(new_n24_), .c(new_n23_), .O(new_n46_));
  nor2   g27(.a(x10), .b(x08), .O(new_n47_));
  nand4  g28(.a(new_n47_), .b(new_n24_), .c(new_n26_), .d(x11), .O(new_n48_));
  oai21  g29(.a(new_n46_), .b(x11), .c(new_n48_), .O(new_n49_));
  nor2   g30(.a(new_n31_), .b(new_n30_), .O(new_n50_));
  aoi21  g31(.a(new_n50_), .b(new_n49_), .c(new_n44_), .O(z2));
  oai21  g32(.a(new_n45_), .b(x11), .c(new_n24_), .O(new_n52_));
  aoi21  g33(.a(new_n52_), .b(new_n26_), .c(x12), .O(new_n53_));
  aoi21  g34(.a(x08), .b(x06), .c(new_n24_), .O(new_n54_));
  nor2   g35(.a(x11), .b(x10), .O(new_n55_));
  nand3  g36(.a(new_n55_), .b(new_n26_), .c(x12), .O(new_n56_));
  oai21  g37(.a(new_n56_), .b(new_n54_), .c(x03), .O(new_n57_));
  nand2  g38(.a(x12), .b(x03), .O(new_n58_));
  nand2  g39(.a(new_n58_), .b(new_n20_), .O(new_n59_));
  oai21  g40(.a(x07), .b(x03), .c(x05), .O(new_n60_));
  aoi21  g41(.a(new_n59_), .b(x08), .c(new_n60_), .O(new_n61_));
  oai21  g42(.a(new_n57_), .b(new_n53_), .c(new_n61_), .O(z3));
  nand2  g43(.a(new_n54_), .b(x04), .O(new_n63_));
  inv1   g44(.a(new_n63_), .O(z4));
endmodule


