// Benchmark "FAU" written by ABC on Sat Aug  8 23:01:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z0, z1, z2, z3, z4;
  wire new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_, new_n61_;
  inv1   g00(.a(x03), .O(new_n20_));
  nand2  g01(.a(x04), .b(x02), .O(new_n21_));
  xor2a  g02(.a(new_n21_), .b(x08), .O(new_n22_));
  inv1   g03(.a(x12), .O(new_n23_));
  nand3  g04(.a(x09), .b(x08), .c(x03), .O(new_n24_));
  nand2  g05(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  oai21  g06(.a(x03), .b(x00), .c(x05), .O(new_n26_));
  aoi21  g07(.a(new_n25_), .b(x13), .c(new_n26_), .O(new_n27_));
  oai21  g08(.a(new_n22_), .b(new_n20_), .c(new_n27_), .O(z0));
  inv1   g09(.a(x08), .O(new_n29_));
  inv1   g10(.a(x10), .O(new_n30_));
  inv1   g11(.a(new_n21_), .O(new_n31_));
  aoi21  g12(.a(new_n31_), .b(new_n29_), .c(new_n30_), .O(new_n32_));
  nand2  g13(.a(x13), .b(x09), .O(new_n33_));
  nor2   g14(.a(x10), .b(x08), .O(new_n34_));
  inv1   g15(.a(new_n34_), .O(new_n35_));
  aoi21  g16(.a(new_n35_), .b(new_n33_), .c(new_n21_), .O(new_n36_));
  oai21  g17(.a(new_n36_), .b(new_n32_), .c(x03), .O(new_n37_));
  nand2  g18(.a(x13), .b(x12), .O(new_n38_));
  inv1   g19(.a(x01), .O(new_n39_));
  nand2  g20(.a(new_n20_), .b(new_n39_), .O(new_n40_));
  nand4  g21(.a(new_n40_), .b(new_n38_), .c(new_n37_), .d(x05), .O(z1));
  inv1   g22(.a(new_n38_), .O(new_n42_));
  inv1   g23(.a(x11), .O(new_n43_));
  nand4  g24(.a(new_n30_), .b(new_n29_), .c(x04), .d(x02), .O(new_n44_));
  xor2a  g25(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  oai21  g26(.a(x06), .b(x03), .c(x05), .O(new_n46_));
  aoi21  g27(.a(new_n45_), .b(x03), .c(new_n46_), .O(new_n47_));
  nand2  g28(.a(new_n21_), .b(new_n43_), .O(new_n48_));
  nor2   g29(.a(x12), .b(new_n20_), .O(new_n49_));
  nand4  g30(.a(new_n49_), .b(new_n48_), .c(x13), .d(x09), .O(new_n50_));
  oai21  g31(.a(new_n47_), .b(new_n42_), .c(new_n50_), .O(z2));
  nand3  g32(.a(new_n34_), .b(new_n31_), .c(new_n43_), .O(new_n52_));
  nand3  g33(.a(new_n34_), .b(new_n23_), .c(new_n43_), .O(new_n53_));
  nand2  g34(.a(new_n53_), .b(new_n33_), .O(new_n54_));
  aoi22  g35(.a(new_n54_), .b(new_n31_), .c(new_n52_), .d(x12), .O(new_n55_));
  nor2   g36(.a(x07), .b(x03), .O(new_n56_));
  nand2  g37(.a(new_n38_), .b(x05), .O(new_n57_));
  nor2   g38(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g39(.a(new_n55_), .b(new_n20_), .c(new_n58_), .O(z3));
  inv1   g40(.a(x13), .O(new_n60_));
  nand2  g41(.a(x09), .b(x04), .O(new_n61_));
  aoi21  g42(.a(new_n61_), .b(new_n23_), .c(new_n60_), .O(z4));
endmodule


