// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x28), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  nand2  g004(.a(x33), .b(x03), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(x32), .b(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n49_), .c(x02), .O(new_n52_));
  inv1   g008(.a(x32), .O(new_n53_));
  aoi21  g009(.a(x33), .b(new_n50_), .c(x02), .O(new_n54_));
  oai21  g010(.a(new_n53_), .b(new_n50_), .c(new_n54_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n47_), .O(z1));
  inv1   g012(.a(x08), .O(new_n57_));
  xor2a  g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  inv1   g015(.a(x02), .O(new_n60_));
  nand2  g016(.a(new_n50_), .b(new_n60_), .O(new_n61_));
  nand2  g017(.a(x03), .b(x02), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n50_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n60_), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n59_), .c(new_n63_), .O(new_n67_));
  xor2a  g023(.a(new_n67_), .b(new_n58_), .O(new_n68_));
  nor4   g024(.a(new_n68_), .b(new_n47_), .c(x09), .d(new_n57_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  inv1   g026(.a(new_n47_), .O(new_n71_));
  nor2   g027(.a(x21), .b(x16), .O(new_n72_));
  nor2   g028(.a(x24), .b(x19), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(x22), .b(x17), .O(new_n75_));
  nor2   g031(.a(x20), .b(x15), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor3   g033(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  inv1   g034(.a(x26), .O(new_n79_));
  nand4  g035(.a(new_n62_), .b(new_n79_), .c(x25), .d(x01), .O(new_n80_));
  aoi21  g036(.a(new_n78_), .b(new_n74_), .c(new_n80_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n71_), .c(new_n70_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(z3));
  nor2   g039(.a(new_n45_), .b(new_n70_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n45_), .b(new_n70_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n85_), .c(new_n81_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n71_), .O(z4));
  inv1   g044(.a(x29), .O(new_n89_));
  nand2  g045(.a(new_n85_), .b(new_n89_), .O(new_n90_));
  nand3  g046(.a(new_n84_), .b(new_n46_), .c(x29), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n90_), .c(new_n81_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n71_), .O(z5));
  inv1   g049(.a(new_n91_), .O(new_n94_));
  nor2   g050(.a(new_n46_), .b(x28), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n94_), .c(new_n81_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(z6));
  inv1   g053(.a(x31), .O(new_n98_));
  aoi21  g054(.a(new_n81_), .b(new_n98_), .c(new_n47_), .O(z7));
  nand4  g055(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n100_));
  nand3  g056(.a(x18), .b(x17), .c(x11), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(x16), .c(x19), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  nand2  g060(.a(x18), .b(x12), .O(new_n105_));
  oai21  g061(.a(x18), .b(x13), .c(x17), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g063(.a(x15), .O(new_n108_));
  inv1   g064(.a(x19), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(x14), .c(new_n100_), .O(new_n110_));
  nand4  g066(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n108_), .c(new_n110_), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(new_n107_), .c(new_n104_), .O(new_n113_));
  nand2  g069(.a(new_n86_), .b(x29), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n46_), .O(new_n115_));
  nand3  g071(.a(x30), .b(x29), .c(x27), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n115_), .c(x31), .O(new_n117_));
  oai21  g073(.a(new_n117_), .b(new_n113_), .c(x32), .O(new_n118_));
  inv1   g074(.a(x17), .O(new_n119_));
  nand3  g075(.a(new_n109_), .b(x18), .c(x14), .O(new_n120_));
  inv1   g076(.a(x18), .O(new_n121_));
  nand3  g077(.a(x19), .b(new_n121_), .c(x13), .O(new_n122_));
  aoi21  g078(.a(new_n122_), .b(new_n120_), .c(new_n119_), .O(new_n123_));
  nand4  g079(.a(x19), .b(x18), .c(new_n119_), .d(x12), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n123_), .c(x16), .O(new_n126_));
  inv1   g082(.a(x16), .O(new_n127_));
  nand3  g083(.a(new_n102_), .b(x19), .c(new_n127_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nor3   g085(.a(new_n111_), .b(new_n109_), .c(x15), .O(new_n130_));
  aoi21  g086(.a(new_n129_), .b(x15), .c(new_n130_), .O(new_n131_));
  nor2   g087(.a(new_n89_), .b(new_n70_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n95_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n115_), .c(new_n53_), .d(x31), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n131_), .c(new_n118_), .O(new_n135_));
  nand2  g091(.a(new_n79_), .b(x00), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n71_), .O(z8));
  nand2  g095(.a(new_n113_), .b(new_n71_), .O(new_n140_));
  nand2  g096(.a(new_n132_), .b(x31), .O(new_n141_));
  nand2  g097(.a(new_n114_), .b(new_n98_), .O(new_n142_));
  aoi22  g098(.a(new_n142_), .b(new_n46_), .c(new_n141_), .d(new_n95_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x33), .O(new_n145_));
  aoi21  g101(.a(new_n128_), .b(new_n126_), .c(new_n108_), .O(new_n146_));
  nor3   g102(.a(x31), .b(x30), .c(x27), .O(new_n147_));
  inv1   g103(.a(new_n116_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(x31), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n147_), .c(new_n45_), .O(new_n151_));
  nand3  g107(.a(new_n98_), .b(new_n46_), .c(new_n89_), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n151_), .c(x33), .O(new_n153_));
  oai21  g109(.a(new_n130_), .b(new_n146_), .c(new_n153_), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n145_), .c(new_n136_), .O(z9));
endmodule


