// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x16), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  xor2a  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  aoi21  g006(.a(new_n49_), .b(new_n50_), .c(new_n46_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(x32), .c(new_n51_), .O(z1));
  inv1   g008(.a(new_n46_), .O(new_n53_));
  xor2a  g009(.a(x07), .b(x04), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nand2  g011(.a(x05), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand4  g015(.a(x06), .b(x05), .c(new_n55_), .d(new_n57_), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(x01), .O(new_n61_));
  and2   g017(.a(new_n48_), .b(x01), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n54_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n63_), .b(new_n54_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n64_), .c(new_n53_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  nor2   g025(.a(x22), .b(x17), .O(new_n70_));
  oai22  g026(.a(x24), .b(x19), .c(x20), .d(x15), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x23), .O(new_n73_));
  nor2   g029(.a(x21), .b(x16), .O(new_n74_));
  aoi21  g030(.a(new_n73_), .b(new_n45_), .c(new_n74_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  inv1   g032(.a(x26), .O(new_n77_));
  nand2  g033(.a(x03), .b(x02), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n77_), .c(x25), .d(x01), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n76_), .c(new_n69_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n53_), .O(z3));
  oai21  g038(.a(new_n74_), .b(new_n73_), .c(new_n45_), .O(new_n83_));
  nand2  g039(.a(new_n80_), .b(new_n53_), .O(new_n84_));
  aoi21  g040(.a(new_n83_), .b(new_n72_), .c(new_n84_), .O(new_n85_));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n69_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n69_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n88_), .c(new_n85_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z4));
  nand2  g047(.a(new_n87_), .b(x29), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  nand2  g049(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n92_), .c(new_n80_), .d(new_n76_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n53_), .O(z5));
  nand3  g052(.a(new_n87_), .b(x30), .c(x29), .O(new_n97_));
  inv1   g053(.a(x30), .O(new_n98_));
  nand2  g054(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n97_), .c(new_n85_), .O(new_n100_));
  inv1   g056(.a(new_n100_), .O(z6));
  xor2a  g057(.a(new_n97_), .b(x31), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n85_), .O(z7));
  inv1   g059(.a(x16), .O(new_n104_));
  inv1   g060(.a(x32), .O(new_n105_));
  nand2  g061(.a(new_n77_), .b(x00), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n105_), .c(new_n45_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  nand3  g064(.a(new_n89_), .b(x30), .c(x29), .O(new_n109_));
  inv1   g065(.a(x31), .O(new_n110_));
  oai21  g066(.a(x28), .b(x27), .c(x29), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n98_), .c(new_n110_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n109_), .O(new_n113_));
  inv1   g069(.a(x17), .O(new_n114_));
  inv1   g070(.a(x19), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x18), .c(x14), .O(new_n116_));
  nand4  g072(.a(x19), .b(new_n45_), .c(x16), .d(x13), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n116_), .c(new_n114_), .O(new_n118_));
  nand4  g074(.a(x19), .b(x18), .c(new_n114_), .d(x12), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n118_), .c(x15), .O(new_n121_));
  inv1   g077(.a(x15), .O(new_n122_));
  nand3  g078(.a(x18), .b(x17), .c(x10), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x19), .c(new_n122_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n121_), .c(new_n113_), .O(new_n126_));
  nand3  g082(.a(x18), .b(x17), .c(x15), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n115_), .c(x14), .O(new_n129_));
  nand2  g085(.a(new_n123_), .b(new_n122_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n127_), .c(x19), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand2  g088(.a(x18), .b(x12), .O(new_n133_));
  oai21  g089(.a(x18), .b(x13), .c(x17), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n133_), .c(new_n105_), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n112_), .c(new_n109_), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n132_), .c(new_n106_), .O(new_n138_));
  oai21  g094(.a(new_n126_), .b(x32), .c(new_n138_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n108_), .O(z8));
  oai21  g096(.a(new_n106_), .b(new_n50_), .c(new_n45_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n104_), .O(new_n142_));
  inv1   g098(.a(new_n106_), .O(new_n143_));
  nand3  g099(.a(new_n111_), .b(new_n110_), .c(new_n98_), .O(new_n144_));
  nand4  g100(.a(new_n89_), .b(x31), .c(x30), .d(x29), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n50_), .O(new_n147_));
  aoi21  g103(.a(new_n125_), .b(new_n121_), .c(new_n147_), .O(new_n148_));
  aoi22  g104(.a(new_n134_), .b(new_n133_), .c(new_n131_), .d(new_n129_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n146_), .c(new_n50_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n148_), .c(new_n143_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n142_), .O(z9));
endmodule


