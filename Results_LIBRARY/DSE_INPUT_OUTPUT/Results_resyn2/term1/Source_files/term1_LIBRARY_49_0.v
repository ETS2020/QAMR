// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_;
  nor2   g000(.a(x27), .b(x24), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xor2a  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  aoi21  g005(.a(new_n48_), .b(new_n49_), .c(new_n45_), .O(new_n50_));
  oai21  g006(.a(new_n48_), .b(x33), .c(new_n50_), .O(z1));
  inv1   g007(.a(x08), .O(new_n52_));
  xor2a  g008(.a(x07), .b(x04), .O(new_n53_));
  nand2  g009(.a(new_n48_), .b(x01), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nand2  g011(.a(x05), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  xor2a  g014(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(x01), .c(new_n54_), .O(new_n60_));
  xor2a  g016(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nor4   g017(.a(new_n61_), .b(new_n45_), .c(x09), .d(new_n52_), .O(z2));
  nor2   g018(.a(new_n55_), .b(new_n57_), .O(new_n63_));
  nor2   g019(.a(x20), .b(x15), .O(new_n64_));
  nor2   g020(.a(x21), .b(x16), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nor2   g022(.a(x22), .b(x17), .O(new_n67_));
  nor2   g023(.a(x23), .b(x18), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n63_), .O(new_n72_));
  inv1   g028(.a(x26), .O(new_n73_));
  nand3  g029(.a(new_n73_), .b(x25), .c(x01), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(x24), .c(x27), .O(z3));
  inv1   g033(.a(new_n63_), .O(new_n78_));
  nand2  g034(.a(new_n75_), .b(new_n78_), .O(new_n79_));
  inv1   g035(.a(x28), .O(new_n80_));
  nor2   g036(.a(x24), .b(x19), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  nor2   g038(.a(new_n80_), .b(new_n82_), .O(new_n83_));
  nor2   g039(.a(x28), .b(x27), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  aoi22  g041(.a(new_n85_), .b(new_n70_), .c(new_n81_), .d(new_n80_), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n79_), .c(new_n46_), .O(z4));
  nor2   g043(.a(new_n81_), .b(new_n70_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  inv1   g045(.a(x29), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n80_), .O(new_n91_));
  nor2   g047(.a(x29), .b(x28), .O(new_n92_));
  nor4   g048(.a(new_n92_), .b(new_n91_), .c(new_n63_), .d(new_n82_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n89_), .O(new_n94_));
  inv1   g050(.a(x24), .O(new_n95_));
  nor3   g051(.a(new_n83_), .b(new_n90_), .c(new_n95_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n72_), .O(new_n97_));
  aoi21  g053(.a(new_n97_), .b(new_n94_), .c(new_n74_), .O(z5));
  inv1   g054(.a(new_n91_), .O(new_n99_));
  nor3   g055(.a(new_n99_), .b(new_n88_), .c(x30), .O(new_n100_));
  nand3  g056(.a(new_n99_), .b(x30), .c(new_n95_), .O(new_n101_));
  aoi21  g057(.a(new_n71_), .b(x19), .c(new_n101_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n100_), .c(x27), .O(new_n103_));
  nand2  g059(.a(new_n83_), .b(x29), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n70_), .c(x30), .d(x24), .O(new_n105_));
  aoi21  g061(.a(new_n105_), .b(new_n103_), .c(new_n79_), .O(z6));
  nor2   g062(.a(new_n79_), .b(new_n45_), .O(new_n107_));
  nand2  g063(.a(new_n104_), .b(x31), .O(new_n108_));
  inv1   g064(.a(x30), .O(new_n109_));
  inv1   g065(.a(x31), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nor2   g067(.a(new_n110_), .b(new_n109_), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n111_), .c(new_n83_), .d(x29), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n108_), .c(new_n107_), .d(new_n89_), .O(z7));
  inv1   g071(.a(x15), .O(new_n116_));
  inv1   g072(.a(x17), .O(new_n117_));
  inv1   g073(.a(x19), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x18), .c(x14), .O(new_n119_));
  inv1   g075(.a(x18), .O(new_n120_));
  nand3  g076(.a(x19), .b(new_n120_), .c(x13), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n119_), .c(new_n117_), .O(new_n122_));
  nand4  g078(.a(x19), .b(x18), .c(new_n117_), .d(x12), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n122_), .c(x16), .O(new_n125_));
  inv1   g081(.a(x16), .O(new_n126_));
  nand3  g082(.a(x18), .b(x17), .c(x11), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x19), .c(new_n126_), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(new_n125_), .c(new_n116_), .O(new_n130_));
  nor2   g086(.a(new_n120_), .b(new_n117_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x16), .c(x10), .O(new_n132_));
  nor3   g088(.a(new_n132_), .b(new_n118_), .c(x15), .O(new_n133_));
  nor2   g089(.a(x32), .b(new_n110_), .O(new_n134_));
  oai21  g090(.a(new_n133_), .b(new_n130_), .c(new_n134_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n109_), .O(new_n136_));
  oai21  g092(.a(x28), .b(x27), .c(x29), .O(new_n137_));
  aoi21  g093(.a(new_n49_), .b(x30), .c(new_n137_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand2  g095(.a(new_n135_), .b(x30), .O(new_n140_));
  inv1   g096(.a(new_n137_), .O(new_n141_));
  aoi21  g097(.a(new_n49_), .b(new_n109_), .c(new_n141_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  oai21  g099(.a(new_n128_), .b(x16), .c(x15), .O(new_n144_));
  inv1   g100(.a(x14), .O(new_n145_));
  nor2   g101(.a(x19), .b(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n131_), .b(x16), .c(x15), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n118_), .O(new_n148_));
  oai21  g104(.a(new_n147_), .b(new_n146_), .c(new_n148_), .O(new_n149_));
  aoi21  g105(.a(new_n144_), .b(new_n132_), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(x18), .b(x12), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n117_), .O(new_n152_));
  inv1   g108(.a(x13), .O(new_n153_));
  nand2  g109(.a(new_n120_), .b(new_n153_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n152_), .c(new_n150_), .d(x31), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x32), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n143_), .c(new_n139_), .O(new_n157_));
  nand2  g113(.a(new_n73_), .b(x00), .O(new_n158_));
  inv1   g114(.a(new_n158_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n46_), .O(z8));
  inv1   g117(.a(x33), .O(new_n162_));
  nor2   g118(.a(new_n137_), .b(new_n112_), .O(new_n163_));
  aoi21  g119(.a(new_n137_), .b(new_n111_), .c(new_n163_), .O(new_n164_));
  oai21  g120(.a(new_n133_), .b(new_n130_), .c(new_n164_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand4  g122(.a(new_n164_), .b(new_n154_), .c(new_n152_), .d(x33), .O(new_n167_));
  inv1   g123(.a(new_n167_), .O(new_n168_));
  aoi21  g124(.a(new_n168_), .b(new_n150_), .c(new_n158_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n166_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n46_), .O(z9));
endmodule


