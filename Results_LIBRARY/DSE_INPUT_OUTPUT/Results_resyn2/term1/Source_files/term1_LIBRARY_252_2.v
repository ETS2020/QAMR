// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_;
  inv1   g000(.a(x06), .O(new_n45_));
  nor2   g001(.a(x18), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  aoi21  g007(.a(new_n50_), .b(new_n51_), .c(new_n46_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(x33), .c(new_n52_), .O(z1));
  inv1   g009(.a(x01), .O(new_n54_));
  aoi21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  oai21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  aoi21  g015(.a(x05), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g016(.a(new_n59_), .b(x05), .c(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  oai21  g018(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  xnor2a g019(.a(x07), .b(x04), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(x08), .O(new_n66_));
  aoi21  g022(.a(new_n64_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  oai21  g023(.a(new_n64_), .b(new_n63_), .c(new_n67_), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n47_), .O(z2));
  nor2   g025(.a(x24), .b(x19), .O(new_n70_));
  nor2   g026(.a(x22), .b(x17), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x16), .O(new_n73_));
  inv1   g029(.a(x21), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x20), .b(x15), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(new_n79_));
  inv1   g035(.a(x26), .O(new_n80_));
  nand3  g036(.a(new_n55_), .b(new_n80_), .c(x25), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(x27), .c(new_n47_), .O(z3));
  inv1   g040(.a(x27), .O(new_n85_));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n85_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g046(.a(new_n90_), .b(new_n83_), .c(new_n47_), .O(z4));
  nand2  g047(.a(new_n87_), .b(x29), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  nand2  g049(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n83_), .c(new_n47_), .O(z5));
  xor2a  g052(.a(new_n92_), .b(x30), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n83_), .c(new_n47_), .O(z6));
  inv1   g054(.a(new_n83_), .O(new_n99_));
  inv1   g055(.a(new_n92_), .O(new_n100_));
  inv1   g056(.a(x31), .O(new_n101_));
  nor2   g057(.a(new_n100_), .b(new_n101_), .O(new_n102_));
  inv1   g058(.a(x30), .O(new_n103_));
  nor2   g059(.a(new_n101_), .b(new_n103_), .O(new_n104_));
  nor2   g060(.a(x31), .b(x30), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  aoi21  g062(.a(new_n106_), .b(new_n100_), .c(new_n102_), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n99_), .c(new_n46_), .O(z7));
  inv1   g064(.a(x15), .O(new_n109_));
  inv1   g065(.a(x17), .O(new_n110_));
  inv1   g066(.a(x18), .O(new_n111_));
  nand3  g067(.a(x19), .b(new_n111_), .c(x13), .O(new_n112_));
  inv1   g068(.a(x19), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x18), .c(x14), .O(new_n114_));
  aoi21  g070(.a(new_n114_), .b(new_n112_), .c(new_n110_), .O(new_n115_));
  nand4  g071(.a(x19), .b(x18), .c(new_n110_), .d(x12), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  oai21  g073(.a(new_n117_), .b(new_n115_), .c(x16), .O(new_n118_));
  nand3  g074(.a(x18), .b(x17), .c(x11), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x19), .c(new_n73_), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n118_), .c(new_n109_), .O(new_n122_));
  nor2   g078(.a(new_n110_), .b(new_n73_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x10), .O(new_n124_));
  nand3  g080(.a(x19), .b(x18), .c(new_n109_), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nor2   g082(.a(x32), .b(new_n101_), .O(new_n127_));
  oai21  g083(.a(new_n126_), .b(new_n122_), .c(new_n127_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n103_), .O(new_n129_));
  nand2  g085(.a(new_n89_), .b(x29), .O(new_n130_));
  aoi21  g086(.a(new_n51_), .b(x30), .c(new_n130_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nor2   g088(.a(x17), .b(x12), .O(new_n133_));
  nand2  g089(.a(x17), .b(x13), .O(new_n134_));
  oai21  g090(.a(new_n133_), .b(new_n111_), .c(new_n134_), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(x16), .c(new_n120_), .O(new_n136_));
  oai21  g092(.a(new_n124_), .b(new_n111_), .c(new_n109_), .O(new_n137_));
  nand2  g093(.a(new_n123_), .b(x15), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n111_), .c(new_n113_), .O(new_n139_));
  nand2  g095(.a(new_n113_), .b(x14), .O(new_n140_));
  nand4  g096(.a(new_n123_), .b(new_n140_), .c(x18), .d(x15), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n139_), .c(new_n137_), .d(x31), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n136_), .c(x32), .O(new_n143_));
  nand2  g099(.a(new_n128_), .b(x30), .O(new_n144_));
  inv1   g100(.a(new_n130_), .O(new_n145_));
  aoi21  g101(.a(new_n51_), .b(new_n103_), .c(new_n145_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n143_), .c(new_n132_), .O(new_n148_));
  nand2  g104(.a(new_n80_), .b(x00), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n47_), .O(z8));
  nand2  g108(.a(x33), .b(x30), .O(new_n153_));
  inv1   g109(.a(x33), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(x19), .c(new_n111_), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n123_), .b(x15), .c(x13), .O(new_n157_));
  inv1   g113(.a(new_n157_), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n156_), .c(new_n105_), .O(new_n159_));
  aoi21  g115(.a(new_n159_), .b(new_n153_), .c(x06), .O(new_n160_));
  nand4  g116(.a(x19), .b(x17), .c(new_n73_), .d(x11), .O(new_n161_));
  inv1   g117(.a(new_n161_), .O(new_n162_));
  nand2  g118(.a(new_n110_), .b(x12), .O(new_n163_));
  oai21  g119(.a(x19), .b(x17), .c(x16), .O(new_n164_));
  aoi21  g120(.a(new_n163_), .b(new_n140_), .c(new_n164_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n162_), .c(x15), .O(new_n166_));
  nand4  g122(.a(new_n123_), .b(x19), .c(new_n109_), .d(x10), .O(new_n167_));
  aoi21  g123(.a(new_n167_), .b(new_n166_), .c(x33), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n105_), .O(new_n169_));
  aoi21  g125(.a(new_n169_), .b(new_n153_), .c(new_n111_), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n160_), .c(new_n130_), .O(new_n171_));
  nor2   g127(.a(new_n154_), .b(x30), .O(new_n172_));
  nand2  g128(.a(new_n158_), .b(new_n104_), .O(new_n173_));
  nor2   g129(.a(new_n173_), .b(new_n155_), .O(new_n174_));
  oai21  g130(.a(new_n174_), .b(new_n172_), .c(new_n45_), .O(new_n175_));
  aoi21  g131(.a(new_n168_), .b(new_n104_), .c(new_n172_), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n111_), .c(new_n175_), .O(new_n177_));
  nor2   g133(.a(new_n138_), .b(new_n140_), .O(new_n178_));
  nand2  g134(.a(new_n138_), .b(x19), .O(new_n179_));
  aoi21  g135(.a(new_n124_), .b(new_n109_), .c(new_n179_), .O(new_n180_));
  aoi21  g136(.a(x17), .b(x11), .c(x16), .O(new_n181_));
  nor2   g137(.a(new_n181_), .b(new_n133_), .O(new_n182_));
  oai21  g138(.a(new_n180_), .b(new_n178_), .c(new_n182_), .O(new_n183_));
  nand2  g139(.a(new_n183_), .b(x18), .O(new_n184_));
  nand2  g140(.a(new_n158_), .b(x19), .O(new_n185_));
  aoi21  g141(.a(new_n185_), .b(new_n111_), .c(new_n106_), .O(new_n186_));
  nand2  g142(.a(new_n47_), .b(x33), .O(new_n187_));
  aoi21  g143(.a(new_n186_), .b(new_n184_), .c(new_n187_), .O(new_n188_));
  aoi21  g144(.a(new_n177_), .b(new_n145_), .c(new_n188_), .O(new_n189_));
  aoi21  g145(.a(new_n189_), .b(new_n171_), .c(new_n149_), .O(z9));
endmodule


