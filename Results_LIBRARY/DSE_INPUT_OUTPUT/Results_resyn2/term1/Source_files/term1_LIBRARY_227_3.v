// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_;
  nand2  g000(.a(x28), .b(x26), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(x33), .O(new_n47_));
  xnor2a g003(.a(x03), .b(x02), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  inv1   g006(.a(new_n48_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g008(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(z1));
  xor2a  g009(.a(x07), .b(x04), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nand2  g011(.a(x05), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  aoi21  g014(.a(new_n58_), .b(new_n56_), .c(x01), .O(new_n59_));
  oai21  g015(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n48_), .b(x01), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x08), .O(new_n64_));
  aoi21  g020(.a(new_n62_), .b(new_n54_), .c(new_n64_), .O(new_n65_));
  oai21  g021(.a(new_n62_), .b(new_n54_), .c(new_n65_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n45_), .O(z2));
  nand2  g023(.a(x03), .b(x02), .O(new_n68_));
  nor2   g024(.a(x20), .b(x15), .O(new_n69_));
  nor2   g025(.a(x24), .b(x19), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g027(.a(x18), .O(new_n72_));
  inv1   g028(.a(x23), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(x22), .b(x17), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n68_), .O(new_n79_));
  inv1   g035(.a(x27), .O(new_n80_));
  nand2  g036(.a(x25), .b(x01), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(x26), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n79_), .c(new_n45_), .O(z3));
  inv1   g040(.a(new_n79_), .O(new_n85_));
  inv1   g041(.a(x28), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n80_), .O(new_n87_));
  nor2   g043(.a(new_n86_), .b(new_n80_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n87_), .c(new_n82_), .d(new_n85_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z4));
  inv1   g047(.a(new_n81_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  nand2  g049(.a(new_n88_), .b(x29), .O(new_n94_));
  inv1   g050(.a(x29), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(x26), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n88_), .c(new_n94_), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n93_), .c(new_n45_), .O(z5));
  inv1   g054(.a(x30), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(x26), .c(new_n94_), .O(new_n100_));
  nand3  g056(.a(x30), .b(x29), .c(x27), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(x28), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n93_), .c(new_n45_), .O(z6));
  oai21  g061(.a(x31), .b(x26), .c(new_n86_), .O(new_n106_));
  inv1   g062(.a(x26), .O(new_n107_));
  nand2  g063(.a(new_n101_), .b(x31), .O(new_n108_));
  oai21  g064(.a(new_n103_), .b(x31), .c(new_n108_), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n93_), .c(new_n107_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n106_), .O(z7));
  inv1   g067(.a(x17), .O(new_n112_));
  inv1   g068(.a(x19), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x18), .c(x14), .O(new_n114_));
  nand3  g070(.a(x19), .b(new_n72_), .c(x13), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n114_), .c(new_n112_), .O(new_n116_));
  nand4  g072(.a(x19), .b(x18), .c(new_n112_), .d(x12), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x16), .O(new_n119_));
  inv1   g075(.a(x16), .O(new_n120_));
  nand3  g076(.a(x18), .b(x17), .c(x11), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x19), .c(new_n120_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nor2   g080(.a(new_n72_), .b(new_n112_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x16), .c(x10), .O(new_n126_));
  nor3   g082(.a(new_n126_), .b(new_n113_), .c(x15), .O(new_n127_));
  aoi21  g083(.a(new_n124_), .b(x15), .c(new_n127_), .O(new_n128_));
  nand2  g084(.a(new_n50_), .b(x31), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n128_), .c(x30), .O(new_n130_));
  aoi22  g086(.a(new_n87_), .b(x29), .c(new_n50_), .d(new_n99_), .O(new_n131_));
  oai21  g087(.a(new_n122_), .b(x16), .c(x15), .O(new_n132_));
  inv1   g088(.a(x14), .O(new_n133_));
  nor2   g089(.a(x19), .b(new_n133_), .O(new_n134_));
  nand3  g090(.a(new_n125_), .b(x16), .c(x15), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n113_), .O(new_n136_));
  oai21  g092(.a(new_n135_), .b(new_n134_), .c(new_n136_), .O(new_n137_));
  aoi21  g093(.a(new_n132_), .b(new_n126_), .c(new_n137_), .O(new_n138_));
  inv1   g094(.a(x31), .O(new_n139_));
  inv1   g095(.a(x13), .O(new_n140_));
  nand2  g096(.a(new_n72_), .b(new_n140_), .O(new_n141_));
  aoi22  g097(.a(new_n141_), .b(x17), .c(x18), .d(x12), .O(new_n142_));
  nor2   g098(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n138_), .c(new_n50_), .O(new_n144_));
  aoi21  g100(.a(new_n131_), .b(new_n130_), .c(new_n144_), .O(new_n145_));
  oai21  g101(.a(new_n129_), .b(new_n128_), .c(new_n99_), .O(new_n146_));
  nand2  g102(.a(new_n50_), .b(x30), .O(new_n147_));
  nand2  g103(.a(x27), .b(new_n107_), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n86_), .c(new_n95_), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n147_), .c(new_n146_), .O(new_n150_));
  oai21  g106(.a(new_n145_), .b(x26), .c(new_n150_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x00), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n45_), .O(z8));
  nand2  g109(.a(new_n87_), .b(x29), .O(new_n154_));
  nand2  g110(.a(x33), .b(x30), .O(new_n155_));
  nand3  g111(.a(new_n47_), .b(new_n139_), .c(new_n99_), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n128_), .c(new_n155_), .O(new_n157_));
  nand2  g113(.a(new_n139_), .b(new_n99_), .O(new_n158_));
  nand2  g114(.a(x31), .b(x30), .O(new_n159_));
  aoi21  g115(.a(new_n159_), .b(new_n158_), .c(new_n142_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n138_), .c(new_n47_), .O(new_n161_));
  aoi21  g117(.a(new_n157_), .b(new_n154_), .c(new_n161_), .O(new_n162_));
  oai21  g118(.a(new_n159_), .b(new_n128_), .c(new_n47_), .O(new_n163_));
  nand3  g119(.a(new_n163_), .b(new_n155_), .c(new_n149_), .O(new_n164_));
  oai21  g120(.a(new_n162_), .b(x26), .c(new_n164_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(x00), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n45_), .O(z9));
endmodule


