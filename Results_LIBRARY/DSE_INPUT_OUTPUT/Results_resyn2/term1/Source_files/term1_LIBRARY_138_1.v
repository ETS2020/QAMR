// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_;
  inv1   g000(.a(x19), .O(new_n45_));
  nand2  g001(.a(x30), .b(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xor2a  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  inv1   g005(.a(new_n46_), .O(new_n50_));
  aoi21  g006(.a(new_n48_), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n48_), .b(x33), .c(new_n51_), .O(z1));
  inv1   g008(.a(x08), .O(new_n53_));
  xor2a  g009(.a(x07), .b(x04), .O(new_n54_));
  nand2  g010(.a(new_n48_), .b(x01), .O(new_n55_));
  inv1   g011(.a(x03), .O(new_n56_));
  nand2  g012(.a(x05), .b(new_n56_), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  xor2a  g015(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  oai21  g016(.a(new_n60_), .b(x01), .c(new_n55_), .O(new_n61_));
  xor2a  g017(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  nor4   g018(.a(new_n62_), .b(new_n50_), .c(x09), .d(new_n53_), .O(z2));
  inv1   g019(.a(x27), .O(new_n64_));
  nor2   g020(.a(x20), .b(x15), .O(new_n65_));
  nor2   g021(.a(x21), .b(x16), .O(new_n66_));
  nor2   g022(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nor2   g023(.a(x22), .b(x17), .O(new_n68_));
  nor2   g024(.a(x23), .b(x18), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n67_), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(new_n72_));
  inv1   g028(.a(x24), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n45_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n72_), .c(new_n50_), .O(new_n75_));
  inv1   g031(.a(x26), .O(new_n76_));
  nand2  g032(.a(x03), .b(x02), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n76_), .c(x25), .d(x01), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n75_), .c(new_n64_), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z3));
  inv1   g037(.a(x28), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n64_), .O(new_n83_));
  nor2   g039(.a(new_n82_), .b(new_n64_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n83_), .c(new_n79_), .d(new_n75_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(z4));
  nand2  g043(.a(new_n74_), .b(new_n72_), .O(new_n88_));
  nand2  g044(.a(new_n84_), .b(x29), .O(new_n89_));
  inv1   g045(.a(x29), .O(new_n90_));
  nand2  g046(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n89_), .c(new_n79_), .d(new_n88_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n46_), .O(z5));
  inv1   g049(.a(new_n89_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(x30), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(new_n96_));
  oai21  g052(.a(new_n94_), .b(x30), .c(new_n71_), .O(new_n97_));
  oai22  g053(.a(new_n97_), .b(new_n96_), .c(new_n89_), .d(new_n74_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n79_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n46_), .O(z6));
  xor2a  g056(.a(new_n95_), .b(x31), .O(new_n101_));
  oai21  g057(.a(new_n74_), .b(x31), .c(new_n72_), .O(new_n102_));
  nor2   g058(.a(x31), .b(x30), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  aoi21  g060(.a(new_n104_), .b(new_n45_), .c(new_n78_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n102_), .c(new_n101_), .O(z7));
  inv1   g062(.a(x17), .O(new_n107_));
  inv1   g063(.a(x18), .O(new_n108_));
  nand3  g064(.a(x19), .b(new_n108_), .c(x13), .O(new_n109_));
  nand3  g065(.a(new_n45_), .b(x18), .c(x14), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n109_), .c(new_n107_), .O(new_n111_));
  nand4  g067(.a(x19), .b(x18), .c(new_n107_), .d(x12), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(x16), .O(new_n114_));
  inv1   g070(.a(x16), .O(new_n115_));
  nand4  g071(.a(x18), .b(x17), .c(new_n115_), .d(x11), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x19), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n114_), .O(new_n119_));
  inv1   g075(.a(x15), .O(new_n120_));
  nor2   g076(.a(new_n108_), .b(new_n107_), .O(new_n121_));
  nand2  g077(.a(x16), .b(x10), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n121_), .c(x19), .d(new_n120_), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  aoi21  g081(.a(new_n119_), .b(x15), .c(new_n125_), .O(new_n126_));
  inv1   g082(.a(x30), .O(new_n127_));
  nand2  g083(.a(new_n83_), .b(x29), .O(new_n128_));
  inv1   g084(.a(new_n128_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand3  g086(.a(new_n123_), .b(new_n121_), .c(new_n120_), .O(new_n131_));
  nand2  g087(.a(new_n108_), .b(x13), .O(new_n132_));
  nand2  g088(.a(new_n107_), .b(x12), .O(new_n133_));
  oai21  g089(.a(x18), .b(x17), .c(x16), .O(new_n134_));
  aoi21  g090(.a(new_n133_), .b(new_n132_), .c(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n117_), .c(x15), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n131_), .O(new_n137_));
  nand2  g093(.a(x30), .b(x19), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n137_), .c(new_n128_), .O(new_n140_));
  oai21  g096(.a(new_n130_), .b(new_n126_), .c(new_n140_), .O(new_n141_));
  inv1   g097(.a(x31), .O(new_n142_));
  nor2   g098(.a(x32), .b(new_n142_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  oai21  g100(.a(new_n122_), .b(new_n107_), .c(new_n120_), .O(new_n145_));
  nand3  g101(.a(x17), .b(x16), .c(x15), .O(new_n146_));
  oai21  g102(.a(x17), .b(x12), .c(x16), .O(new_n147_));
  nand2  g103(.a(x17), .b(x11), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(x18), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n146_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n145_), .c(new_n50_), .O(new_n152_));
  inv1   g108(.a(x13), .O(new_n153_));
  oai22  g109(.a(new_n146_), .b(new_n138_), .c(x30), .d(x18), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nand3  g111(.a(new_n121_), .b(x16), .c(x15), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n45_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n129_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n127_), .O(new_n159_));
  nand2  g115(.a(new_n139_), .b(new_n129_), .O(new_n160_));
  inv1   g116(.a(x14), .O(new_n161_));
  aoi21  g117(.a(new_n127_), .b(new_n161_), .c(x19), .O(new_n162_));
  nor2   g118(.a(new_n162_), .b(new_n156_), .O(new_n163_));
  aoi21  g119(.a(new_n46_), .b(new_n142_), .c(new_n163_), .O(new_n164_));
  nand4  g120(.a(new_n164_), .b(new_n160_), .c(new_n159_), .d(new_n155_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n152_), .c(x32), .O(new_n166_));
  nand2  g122(.a(new_n76_), .b(x00), .O(new_n167_));
  aoi21  g123(.a(new_n166_), .b(new_n144_), .c(new_n167_), .O(z8));
  nand2  g124(.a(new_n128_), .b(new_n103_), .O(new_n169_));
  or2    g125(.a(new_n169_), .b(new_n126_), .O(new_n170_));
  nor2   g126(.a(new_n160_), .b(new_n142_), .O(new_n171_));
  aoi21  g127(.a(new_n171_), .b(new_n137_), .c(x33), .O(new_n172_));
  inv1   g128(.a(new_n167_), .O(new_n173_));
  nand3  g129(.a(new_n157_), .b(new_n128_), .c(new_n142_), .O(new_n174_));
  nand2  g130(.a(new_n174_), .b(new_n127_), .O(new_n175_));
  oai21  g131(.a(new_n128_), .b(new_n142_), .c(new_n139_), .O(new_n176_));
  inv1   g132(.a(x33), .O(new_n177_));
  nor2   g133(.a(new_n163_), .b(new_n177_), .O(new_n178_));
  nand4  g134(.a(new_n178_), .b(new_n176_), .c(new_n175_), .d(new_n155_), .O(new_n179_));
  oai21  g135(.a(new_n179_), .b(new_n152_), .c(new_n173_), .O(new_n180_));
  aoi21  g136(.a(new_n172_), .b(new_n170_), .c(new_n180_), .O(z9));
endmodule


