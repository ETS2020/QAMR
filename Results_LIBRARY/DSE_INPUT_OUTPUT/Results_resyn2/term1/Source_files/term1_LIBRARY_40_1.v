// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x05), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  nand2  g008(.a(x32), .b(x03), .O(new_n53_));
  nand2  g009(.a(x33), .b(new_n48_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n46_), .O(z1));
  inv1   g012(.a(new_n46_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(x01), .O(new_n58_));
  nand2  g014(.a(new_n48_), .b(new_n50_), .O(new_n59_));
  nand2  g015(.a(x03), .b(x02), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n50_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n48_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n57_), .c(new_n63_), .O(new_n65_));
  inv1   g021(.a(x01), .O(new_n66_));
  inv1   g022(.a(new_n64_), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n66_), .O(new_n68_));
  oai22  g024(.a(new_n68_), .b(new_n65_), .c(new_n61_), .d(new_n58_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g027(.a(new_n70_), .O(new_n72_));
  inv1   g028(.a(new_n61_), .O(new_n73_));
  aoi21  g029(.a(new_n64_), .b(new_n57_), .c(new_n62_), .O(new_n74_));
  oai21  g030(.a(new_n67_), .b(new_n63_), .c(new_n66_), .O(new_n75_));
  oai22  g031(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n58_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  inv1   g033(.a(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  aoi21  g035(.a(new_n77_), .b(new_n71_), .c(new_n79_), .O(z2));
  nor2   g036(.a(x21), .b(x16), .O(new_n81_));
  nor2   g037(.a(x24), .b(x19), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g039(.a(x17), .O(new_n84_));
  inv1   g040(.a(x22), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nor2   g042(.a(x20), .b(x15), .O(new_n87_));
  nor2   g043(.a(x23), .b(x18), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n86_), .c(new_n83_), .O(new_n90_));
  inv1   g046(.a(x26), .O(new_n91_));
  nand4  g047(.a(new_n60_), .b(new_n91_), .c(x25), .d(x01), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(x27), .c(new_n57_), .O(z3));
  inv1   g051(.a(x27), .O(new_n96_));
  inv1   g052(.a(x28), .O(new_n97_));
  nor2   g053(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n97_), .b(new_n96_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n94_), .c(new_n57_), .O(z4));
  nand2  g058(.a(new_n98_), .b(x29), .O(new_n103_));
  inv1   g059(.a(x29), .O(new_n104_));
  nand2  g060(.a(new_n99_), .b(new_n104_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n94_), .c(new_n57_), .O(z5));
  inv1   g063(.a(new_n94_), .O(new_n108_));
  nand2  g064(.a(x30), .b(x05), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n103_), .O(new_n110_));
  nand3  g066(.a(new_n98_), .b(x30), .c(x29), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(new_n110_), .c(new_n108_), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(z6));
  xor2a  g069(.a(x31), .b(x30), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nor2   g071(.a(new_n115_), .b(new_n103_), .O(new_n116_));
  aoi21  g072(.a(new_n103_), .b(x31), .c(new_n116_), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n108_), .c(new_n46_), .O(z7));
  nand2  g074(.a(new_n91_), .b(x00), .O(new_n119_));
  inv1   g075(.a(x15), .O(new_n120_));
  inv1   g076(.a(x16), .O(new_n121_));
  nand4  g077(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n122_));
  nand3  g078(.a(x18), .b(x17), .c(x11), .O(new_n123_));
  aoi22  g079(.a(new_n123_), .b(new_n121_), .c(new_n122_), .d(new_n120_), .O(new_n124_));
  inv1   g080(.a(x19), .O(new_n125_));
  inv1   g081(.a(x18), .O(new_n126_));
  nor2   g082(.a(new_n126_), .b(new_n84_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(x16), .c(x15), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  inv1   g085(.a(new_n128_), .O(new_n130_));
  nand2  g086(.a(new_n125_), .b(x14), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  inv1   g088(.a(x31), .O(new_n133_));
  aoi21  g089(.a(x18), .b(x12), .c(x17), .O(new_n134_));
  nor2   g090(.a(x18), .b(x13), .O(new_n135_));
  nor3   g091(.a(new_n135_), .b(new_n134_), .c(new_n133_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n132_), .c(new_n129_), .d(new_n124_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n57_), .O(new_n138_));
  nand2  g094(.a(new_n100_), .b(x29), .O(new_n139_));
  nor2   g095(.a(new_n139_), .b(new_n109_), .O(new_n140_));
  aoi21  g096(.a(new_n139_), .b(new_n45_), .c(new_n140_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n138_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x32), .O(new_n143_));
  nand3  g099(.a(new_n125_), .b(x18), .c(x14), .O(new_n144_));
  nand3  g100(.a(x19), .b(new_n126_), .c(x13), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n144_), .c(new_n84_), .O(new_n146_));
  nand4  g102(.a(x19), .b(x18), .c(new_n84_), .d(x12), .O(new_n147_));
  inv1   g103(.a(new_n147_), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n146_), .c(x16), .O(new_n149_));
  inv1   g105(.a(new_n123_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(x19), .c(new_n121_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n149_), .c(new_n120_), .O(new_n152_));
  nor3   g108(.a(new_n122_), .b(new_n125_), .c(x15), .O(new_n153_));
  xor2a  g109(.a(new_n139_), .b(x30), .O(new_n154_));
  nand2  g110(.a(z0), .b(x31), .O(new_n155_));
  nor2   g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  oai21  g112(.a(new_n153_), .b(new_n152_), .c(new_n156_), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(new_n143_), .c(new_n119_), .O(z8));
  and2   g114(.a(new_n154_), .b(new_n115_), .O(new_n159_));
  oai21  g115(.a(new_n153_), .b(new_n152_), .c(new_n159_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n49_), .O(new_n161_));
  nor3   g117(.a(new_n135_), .b(new_n134_), .c(new_n49_), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(new_n132_), .c(new_n129_), .d(new_n124_), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(new_n164_));
  aoi21  g120(.a(new_n164_), .b(new_n159_), .c(new_n119_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n161_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n57_), .O(z9));
endmodule


