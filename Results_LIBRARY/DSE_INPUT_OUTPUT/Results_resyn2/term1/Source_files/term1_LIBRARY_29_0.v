// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  nor2   g000(.a(x31), .b(x09), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xnor2a g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  aoi21  g006(.a(new_n49_), .b(new_n50_), .c(new_n45_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(x33), .c(new_n51_), .O(z1));
  inv1   g008(.a(x01), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nand2  g010(.a(x05), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(x02), .O(new_n56_));
  nand2  g012(.a(x06), .b(new_n56_), .O(new_n57_));
  or2    g013(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g014(.a(new_n57_), .b(new_n55_), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(new_n58_), .c(new_n53_), .O(new_n60_));
  nor2   g016(.a(new_n49_), .b(new_n53_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  xnor2a g019(.a(x07), .b(x04), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand3  g022(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(new_n66_), .c(x08), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(x31), .c(x09), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  nor2   g026(.a(x22), .b(x17), .O(new_n71_));
  nor2   g027(.a(x20), .b(x15), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g029(.a(x19), .O(new_n74_));
  inv1   g030(.a(x24), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(x21), .b(x16), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(new_n80_));
  inv1   g036(.a(x26), .O(new_n81_));
  nand2  g037(.a(x03), .b(x02), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n81_), .c(x25), .d(x01), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  and2   g040(.a(new_n84_), .b(new_n80_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n46_), .c(new_n70_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(z3));
  inv1   g043(.a(x28), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n70_), .O(new_n89_));
  nor2   g045(.a(new_n88_), .b(new_n70_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n89_), .c(new_n85_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n46_), .O(z4));
  nor2   g049(.a(new_n83_), .b(new_n45_), .O(new_n94_));
  inv1   g050(.a(x29), .O(new_n95_));
  nand2  g051(.a(new_n91_), .b(new_n95_), .O(new_n96_));
  nand2  g052(.a(new_n90_), .b(x29), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n96_), .c(new_n94_), .d(new_n80_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z5));
  inv1   g055(.a(x30), .O(new_n100_));
  xor2a  g056(.a(new_n97_), .b(new_n100_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n85_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n46_), .O(z6));
  xor2a  g059(.a(x31), .b(x30), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n90_), .c(x29), .O(new_n105_));
  nand2  g061(.a(new_n97_), .b(x31), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n105_), .c(new_n94_), .d(new_n80_), .O(z7));
  oai21  g063(.a(x28), .b(x27), .c(x29), .O(new_n108_));
  xor2a  g064(.a(new_n108_), .b(new_n100_), .O(new_n109_));
  inv1   g065(.a(x15), .O(new_n110_));
  inv1   g066(.a(x17), .O(new_n111_));
  nand3  g067(.a(new_n74_), .b(x18), .c(x14), .O(new_n112_));
  inv1   g068(.a(x18), .O(new_n113_));
  nand3  g069(.a(x19), .b(new_n113_), .c(x13), .O(new_n114_));
  aoi21  g070(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  nand4  g071(.a(x19), .b(x18), .c(new_n111_), .d(x12), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  oai21  g073(.a(new_n117_), .b(new_n115_), .c(x16), .O(new_n118_));
  inv1   g074(.a(x16), .O(new_n119_));
  nand3  g075(.a(x18), .b(x17), .c(x11), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x19), .c(new_n119_), .O(new_n122_));
  aoi21  g078(.a(new_n122_), .b(new_n118_), .c(new_n110_), .O(new_n123_));
  nor2   g079(.a(new_n113_), .b(new_n111_), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x16), .c(x10), .O(new_n125_));
  nor3   g081(.a(new_n125_), .b(new_n74_), .c(x15), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(new_n123_), .c(new_n109_), .O(new_n127_));
  nand2  g083(.a(x18), .b(x12), .O(new_n128_));
  oai21  g084(.a(x18), .b(x13), .c(x17), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(x16), .c(new_n121_), .O(new_n131_));
  nand3  g087(.a(new_n124_), .b(x16), .c(x15), .O(new_n132_));
  aoi21  g088(.a(new_n74_), .b(x14), .c(new_n132_), .O(new_n133_));
  nand2  g089(.a(new_n125_), .b(new_n110_), .O(new_n134_));
  nand2  g090(.a(new_n132_), .b(new_n74_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nor3   g092(.a(new_n136_), .b(new_n133_), .c(new_n131_), .O(new_n137_));
  inv1   g093(.a(new_n109_), .O(new_n138_));
  nand2  g094(.a(x32), .b(x31), .O(new_n139_));
  nor2   g095(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g097(.a(new_n81_), .b(x00), .O(new_n142_));
  aoi21  g098(.a(x32), .b(x09), .c(x31), .O(new_n143_));
  nor2   g099(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  aoi21  g101(.a(new_n127_), .b(new_n50_), .c(new_n145_), .O(z8));
  inv1   g102(.a(x33), .O(new_n147_));
  nor2   g103(.a(new_n109_), .b(new_n104_), .O(new_n148_));
  oai21  g104(.a(new_n126_), .b(new_n123_), .c(new_n148_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g106(.a(new_n148_), .b(x33), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n137_), .c(new_n142_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n46_), .O(z9));
endmodule


