// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  nor2   g000(.a(x30), .b(x09), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  nand2  g008(.a(x32), .b(x03), .O(new_n53_));
  nand2  g009(.a(x33), .b(new_n48_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n45_), .O(z1));
  inv1   g012(.a(x01), .O(new_n57_));
  aoi21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  oai21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n50_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n48_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n48_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n59_), .O(new_n64_));
  xnor2a g020(.a(x07), .b(x04), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand3  g022(.a(x30), .b(new_n66_), .c(x08), .O(new_n67_));
  aoi21  g023(.a(new_n65_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  oai21  g024(.a(new_n65_), .b(new_n64_), .c(new_n68_), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z2));
  inv1   g026(.a(x27), .O(new_n71_));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  nor2   g028(.a(x24), .b(x19), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(x21), .b(x16), .O(new_n75_));
  nor2   g031(.a(x20), .b(x15), .O(new_n76_));
  nor2   g032(.a(x22), .b(x17), .O(new_n77_));
  nor3   g033(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  inv1   g034(.a(x26), .O(new_n79_));
  nand3  g035(.a(new_n58_), .b(new_n79_), .c(x25), .O(new_n80_));
  aoi21  g036(.a(new_n78_), .b(new_n74_), .c(new_n80_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n46_), .c(new_n71_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(z3));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n71_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n71_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n86_), .c(new_n81_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n46_), .O(z4));
  nand2  g045(.a(new_n78_), .b(new_n74_), .O(new_n90_));
  nand2  g046(.a(new_n85_), .b(x29), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  inv1   g048(.a(new_n80_), .O(new_n93_));
  oai21  g049(.a(new_n85_), .b(x29), .c(new_n93_), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n92_), .c(new_n46_), .O(z5));
  inv1   g051(.a(x15), .O(new_n96_));
  inv1   g052(.a(x16), .O(new_n97_));
  inv1   g053(.a(x17), .O(new_n98_));
  oai21  g054(.a(new_n97_), .b(new_n96_), .c(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n72_), .O(new_n100_));
  nor2   g056(.a(new_n77_), .b(new_n75_), .O(new_n101_));
  aoi21  g057(.a(x20), .b(x16), .c(x15), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n74_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n101_), .c(new_n100_), .d(new_n86_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n92_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x30), .O(new_n106_));
  inv1   g062(.a(x30), .O(new_n107_));
  inv1   g063(.a(new_n91_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n90_), .c(new_n107_), .d(x09), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(new_n106_), .c(new_n80_), .O(z6));
  xnor2a g066(.a(x31), .b(x30), .O(new_n111_));
  nor2   g067(.a(new_n111_), .b(new_n91_), .O(new_n112_));
  aoi21  g068(.a(new_n91_), .b(x31), .c(new_n112_), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n81_), .c(new_n45_), .O(z7));
  nand2  g070(.a(new_n79_), .b(x00), .O(new_n115_));
  inv1   g071(.a(z0), .O(new_n116_));
  inv1   g072(.a(x18), .O(new_n117_));
  nor2   g073(.a(new_n117_), .b(new_n98_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x16), .c(x10), .O(new_n119_));
  nand3  g075(.a(x18), .b(x17), .c(x11), .O(new_n120_));
  aoi22  g076(.a(new_n120_), .b(new_n97_), .c(new_n119_), .d(new_n96_), .O(new_n121_));
  inv1   g077(.a(x19), .O(new_n122_));
  nand3  g078(.a(new_n118_), .b(x16), .c(x15), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  inv1   g080(.a(new_n123_), .O(new_n125_));
  nand2  g081(.a(new_n122_), .b(x14), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n124_), .c(new_n121_), .O(new_n128_));
  oai21  g084(.a(x28), .b(x27), .c(x29), .O(new_n129_));
  xor2a  g085(.a(new_n129_), .b(x30), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  inv1   g087(.a(x31), .O(new_n132_));
  aoi21  g088(.a(x18), .b(x12), .c(x17), .O(new_n133_));
  nor2   g089(.a(x18), .b(x13), .O(new_n134_));
  nor3   g090(.a(new_n134_), .b(new_n133_), .c(new_n132_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n128_), .c(new_n116_), .O(new_n137_));
  nand3  g093(.a(new_n122_), .b(x18), .c(x14), .O(new_n138_));
  nand3  g094(.a(x19), .b(new_n117_), .c(x13), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n138_), .c(new_n98_), .O(new_n140_));
  nand4  g096(.a(x19), .b(x18), .c(new_n98_), .d(x12), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n140_), .c(x16), .O(new_n143_));
  inv1   g099(.a(new_n120_), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(x19), .c(new_n97_), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n143_), .c(new_n96_), .O(new_n146_));
  nor3   g102(.a(new_n119_), .b(new_n122_), .c(x15), .O(new_n147_));
  nor4   g103(.a(new_n130_), .b(new_n45_), .c(x32), .d(new_n132_), .O(new_n148_));
  oai21  g104(.a(new_n147_), .b(new_n146_), .c(new_n148_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n137_), .c(new_n115_), .O(z8));
  and2   g106(.a(new_n130_), .b(new_n111_), .O(new_n151_));
  oai21  g107(.a(new_n147_), .b(new_n146_), .c(new_n151_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n49_), .O(new_n153_));
  nor3   g109(.a(new_n134_), .b(new_n133_), .c(new_n49_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n127_), .c(new_n124_), .d(new_n121_), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n151_), .c(new_n115_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n46_), .O(z9));
endmodule


