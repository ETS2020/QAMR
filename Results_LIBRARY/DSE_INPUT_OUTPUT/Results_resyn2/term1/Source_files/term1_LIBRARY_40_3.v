// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_;
  inv1   g000(.a(x05), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  nand2  g009(.a(x32), .b(x03), .O(new_n54_));
  nand2  g010(.a(x33), .b(new_n49_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n54_), .c(new_n51_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n47_), .O(z1));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nand4  g016(.a(x06), .b(new_n49_), .c(new_n51_), .d(new_n60_), .O(new_n61_));
  oai21  g017(.a(new_n61_), .b(new_n59_), .c(new_n46_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x08), .O(new_n64_));
  aoi21  g020(.a(new_n62_), .b(x05), .c(new_n64_), .O(new_n65_));
  nand2  g021(.a(new_n49_), .b(new_n51_), .O(new_n66_));
  aoi21  g022(.a(x03), .b(x02), .c(new_n60_), .O(new_n67_));
  and2   g023(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g024(.a(x06), .b(new_n51_), .O(new_n69_));
  nand2  g025(.a(x05), .b(new_n49_), .O(new_n70_));
  nand3  g026(.a(new_n70_), .b(new_n69_), .c(new_n60_), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(new_n72_));
  oai21  g028(.a(new_n72_), .b(new_n68_), .c(new_n58_), .O(new_n73_));
  xor2a  g029(.a(new_n70_), .b(new_n69_), .O(new_n74_));
  nor2   g030(.a(new_n68_), .b(new_n58_), .O(new_n75_));
  oai21  g031(.a(new_n74_), .b(x01), .c(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n73_), .c(new_n65_), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(z2));
  inv1   g034(.a(new_n47_), .O(new_n79_));
  nor2   g035(.a(x21), .b(x16), .O(new_n80_));
  nor2   g036(.a(x24), .b(x19), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  inv1   g038(.a(x17), .O(new_n83_));
  inv1   g039(.a(x22), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nor2   g041(.a(x20), .b(x15), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n85_), .c(new_n82_), .O(new_n89_));
  inv1   g045(.a(x26), .O(new_n90_));
  nand3  g046(.a(new_n67_), .b(new_n90_), .c(x25), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(x27), .c(new_n79_), .O(z3));
  inv1   g050(.a(x27), .O(new_n95_));
  inv1   g051(.a(x28), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(new_n98_));
  nand2  g054(.a(new_n96_), .b(new_n95_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n93_), .c(new_n79_), .O(z4));
  nand2  g057(.a(new_n97_), .b(x29), .O(new_n102_));
  inv1   g058(.a(x29), .O(new_n103_));
  nand2  g059(.a(new_n98_), .b(new_n103_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n93_), .c(new_n79_), .O(z5));
  inv1   g062(.a(new_n93_), .O(new_n107_));
  inv1   g063(.a(new_n102_), .O(new_n108_));
  aoi21  g064(.a(x30), .b(new_n45_), .c(new_n108_), .O(new_n109_));
  aoi21  g065(.a(new_n108_), .b(x30), .c(new_n109_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n107_), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(z6));
  xor2a  g068(.a(x31), .b(x30), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  nor2   g070(.a(new_n114_), .b(new_n102_), .O(new_n115_));
  aoi21  g071(.a(new_n102_), .b(x31), .c(new_n115_), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n107_), .c(new_n47_), .O(z7));
  nand2  g073(.a(new_n90_), .b(x00), .O(new_n118_));
  inv1   g074(.a(x15), .O(new_n119_));
  inv1   g075(.a(x16), .O(new_n120_));
  nand4  g076(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n121_));
  nand3  g077(.a(x18), .b(x17), .c(x11), .O(new_n122_));
  aoi22  g078(.a(new_n122_), .b(new_n120_), .c(new_n121_), .d(new_n119_), .O(new_n123_));
  inv1   g079(.a(x19), .O(new_n124_));
  inv1   g080(.a(x18), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n83_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x16), .c(x15), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  inv1   g084(.a(new_n127_), .O(new_n129_));
  nand2  g085(.a(new_n124_), .b(x14), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g087(.a(x31), .O(new_n132_));
  aoi21  g088(.a(x18), .b(x12), .c(x17), .O(new_n133_));
  nor2   g089(.a(x18), .b(x13), .O(new_n134_));
  nor3   g090(.a(new_n134_), .b(new_n133_), .c(new_n132_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n131_), .c(new_n128_), .d(new_n123_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n79_), .O(new_n137_));
  nand2  g093(.a(new_n99_), .b(x29), .O(new_n138_));
  nor3   g094(.a(new_n138_), .b(new_n46_), .c(x05), .O(new_n139_));
  aoi21  g095(.a(new_n138_), .b(new_n46_), .c(new_n139_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x32), .O(new_n142_));
  nand3  g098(.a(new_n124_), .b(x18), .c(x14), .O(new_n143_));
  nand3  g099(.a(x19), .b(new_n125_), .c(x13), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n143_), .c(new_n83_), .O(new_n145_));
  nand4  g101(.a(x19), .b(x18), .c(new_n83_), .d(x12), .O(new_n146_));
  inv1   g102(.a(new_n146_), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n145_), .c(x16), .O(new_n148_));
  inv1   g104(.a(new_n122_), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(x19), .c(new_n120_), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n148_), .c(new_n119_), .O(new_n151_));
  nor3   g107(.a(new_n121_), .b(new_n124_), .c(x15), .O(new_n152_));
  xor2a  g108(.a(new_n138_), .b(x30), .O(new_n153_));
  nand2  g109(.a(z0), .b(x31), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  oai21  g111(.a(new_n152_), .b(new_n151_), .c(new_n155_), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n142_), .c(new_n118_), .O(z8));
  and2   g113(.a(new_n153_), .b(new_n114_), .O(new_n158_));
  oai21  g114(.a(new_n152_), .b(new_n151_), .c(new_n158_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n50_), .O(new_n160_));
  nor3   g116(.a(new_n134_), .b(new_n133_), .c(new_n50_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(new_n131_), .c(new_n128_), .d(new_n123_), .O(new_n162_));
  inv1   g118(.a(new_n162_), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(new_n158_), .c(new_n118_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n160_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n79_), .O(z9));
endmodule


