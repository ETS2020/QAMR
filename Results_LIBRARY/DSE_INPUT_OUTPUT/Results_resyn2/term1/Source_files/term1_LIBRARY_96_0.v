// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_;
  nor2   g000(.a(x31), .b(x29), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xor2a  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  aoi21  g005(.a(new_n48_), .b(new_n49_), .c(new_n45_), .O(new_n50_));
  oai21  g006(.a(new_n48_), .b(x33), .c(new_n50_), .O(z1));
  inv1   g007(.a(x08), .O(new_n52_));
  xor2a  g008(.a(x07), .b(x04), .O(new_n53_));
  nand2  g009(.a(new_n48_), .b(x01), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nand2  g011(.a(x05), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  xor2a  g014(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(x01), .c(new_n54_), .O(new_n60_));
  xor2a  g016(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nor4   g017(.a(new_n61_), .b(new_n45_), .c(x09), .d(new_n52_), .O(z2));
  inv1   g018(.a(x27), .O(new_n63_));
  nor2   g019(.a(x20), .b(x15), .O(new_n64_));
  nor2   g020(.a(x24), .b(x19), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nor2   g022(.a(x23), .b(x18), .O(new_n67_));
  nor2   g023(.a(x22), .b(x17), .O(new_n68_));
  nor2   g024(.a(x21), .b(x16), .O(new_n69_));
  nor3   g025(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi22  g026(.a(new_n70_), .b(new_n66_), .c(x03), .d(x02), .O(new_n71_));
  nand2  g027(.a(x25), .b(x01), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(x26), .O(new_n73_));
  nand3  g029(.a(new_n73_), .b(new_n71_), .c(new_n46_), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n63_), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(z3));
  inv1   g033(.a(x28), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n63_), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  nor2   g036(.a(x28), .b(x27), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n80_), .c(new_n75_), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(z4));
  nand2  g040(.a(new_n73_), .b(new_n71_), .O(new_n85_));
  nand2  g041(.a(new_n79_), .b(x29), .O(new_n86_));
  inv1   g042(.a(x29), .O(new_n87_));
  nand2  g043(.a(new_n80_), .b(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  oai21  g045(.a(new_n89_), .b(new_n85_), .c(new_n46_), .O(z5));
  inv1   g046(.a(new_n85_), .O(new_n91_));
  inv1   g047(.a(x30), .O(new_n92_));
  oai21  g048(.a(new_n45_), .b(new_n92_), .c(new_n86_), .O(new_n93_));
  nand3  g049(.a(new_n79_), .b(x30), .c(x29), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n93_), .c(new_n91_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z6));
  inv1   g052(.a(x31), .O(new_n97_));
  nand3  g053(.a(new_n79_), .b(new_n97_), .c(x30), .O(new_n98_));
  oai21  g054(.a(new_n80_), .b(new_n92_), .c(x31), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n98_), .c(new_n91_), .d(x29), .O(z7));
  inv1   g056(.a(x26), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g058(.a(x15), .O(new_n103_));
  inv1   g059(.a(x17), .O(new_n104_));
  inv1   g060(.a(x19), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x18), .c(x14), .O(new_n106_));
  inv1   g062(.a(x18), .O(new_n107_));
  nand3  g063(.a(x19), .b(new_n107_), .c(x13), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  nand4  g065(.a(x19), .b(x18), .c(new_n104_), .d(x12), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n109_), .c(x16), .O(new_n112_));
  inv1   g068(.a(x16), .O(new_n113_));
  nand3  g069(.a(x18), .b(x17), .c(x11), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x19), .c(new_n113_), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n112_), .c(new_n103_), .O(new_n117_));
  nor2   g073(.a(new_n107_), .b(new_n104_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x16), .c(x10), .O(new_n119_));
  nor3   g075(.a(new_n119_), .b(new_n105_), .c(x15), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n117_), .c(x30), .O(new_n121_));
  oai22  g077(.a(new_n81_), .b(new_n87_), .c(new_n49_), .d(new_n92_), .O(new_n122_));
  aoi21  g078(.a(new_n121_), .b(new_n49_), .c(new_n122_), .O(new_n123_));
  nand2  g079(.a(new_n116_), .b(new_n112_), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(x15), .c(new_n120_), .O(new_n125_));
  oai21  g081(.a(new_n115_), .b(x16), .c(x19), .O(new_n126_));
  nand3  g082(.a(new_n118_), .b(x16), .c(x15), .O(new_n127_));
  nand2  g083(.a(x18), .b(x12), .O(new_n128_));
  oai21  g084(.a(x18), .b(x13), .c(x17), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g086(.a(new_n119_), .b(new_n103_), .O(new_n131_));
  nand2  g087(.a(new_n105_), .b(x14), .O(new_n132_));
  inv1   g088(.a(new_n127_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n131_), .c(new_n130_), .O(new_n135_));
  aoi21  g091(.a(new_n127_), .b(new_n126_), .c(new_n135_), .O(new_n136_));
  nand4  g092(.a(new_n82_), .b(new_n49_), .c(new_n92_), .d(x29), .O(new_n137_));
  oai22  g093(.a(new_n137_), .b(new_n125_), .c(new_n136_), .d(new_n49_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n123_), .c(x31), .O(new_n139_));
  oai21  g095(.a(new_n81_), .b(new_n92_), .c(x31), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(x32), .c(x29), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(new_n139_), .c(new_n102_), .O(z8));
  oai21  g098(.a(new_n97_), .b(new_n92_), .c(new_n82_), .O(new_n143_));
  nor2   g099(.a(x31), .b(x30), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  oai21  g101(.a(new_n81_), .b(new_n87_), .c(new_n145_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n143_), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n125_), .O(new_n148_));
  nand3  g104(.a(new_n143_), .b(x33), .c(x29), .O(new_n149_));
  aoi21  g105(.a(new_n145_), .b(new_n81_), .c(new_n149_), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n136_), .c(new_n102_), .O(new_n151_));
  oai21  g107(.a(new_n148_), .b(x33), .c(new_n151_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n46_), .O(z9));
endmodule


