// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_;
  inv1   g000(.a(x15), .O(new_n45_));
  nor2   g001(.a(x30), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  inv1   g009(.a(x32), .O(new_n54_));
  aoi21  g010(.a(x33), .b(new_n49_), .c(x02), .O(new_n55_));
  oai21  g011(.a(new_n54_), .b(new_n49_), .c(new_n55_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n46_), .O(z1));
  inv1   g013(.a(x08), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  aoi21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  oai21  g016(.a(x03), .b(x02), .c(new_n60_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n51_), .O(new_n62_));
  aoi21  g018(.a(x05), .b(new_n49_), .c(new_n62_), .O(new_n63_));
  nand3  g019(.a(new_n62_), .b(x05), .c(new_n49_), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n63_), .c(new_n61_), .O(new_n66_));
  xor2a  g022(.a(x07), .b(x04), .O(new_n67_));
  xor2a  g023(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nor4   g024(.a(new_n68_), .b(new_n46_), .c(x09), .d(new_n58_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  nor2   g026(.a(x21), .b(x16), .O(new_n71_));
  nor2   g027(.a(x22), .b(x17), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g029(.a(x23), .b(x18), .O(new_n74_));
  nor2   g030(.a(x24), .b(x19), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nor2   g033(.a(x20), .b(x15), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  inv1   g036(.a(x26), .O(new_n81_));
  nand3  g037(.a(new_n60_), .b(new_n81_), .c(x25), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n80_), .c(new_n70_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(z3));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n70_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n70_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n88_), .c(new_n83_), .O(new_n90_));
  oai21  g046(.a(new_n90_), .b(new_n79_), .c(new_n47_), .O(z4));
  nand2  g047(.a(new_n87_), .b(x29), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  nand2  g049(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n92_), .c(new_n83_), .d(new_n80_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z5));
  nand3  g052(.a(new_n76_), .b(new_n73_), .c(x20), .O(new_n97_));
  nor2   g053(.a(x30), .b(x15), .O(new_n98_));
  aoi21  g054(.a(new_n98_), .b(new_n97_), .c(new_n92_), .O(new_n99_));
  inv1   g055(.a(new_n92_), .O(new_n100_));
  inv1   g056(.a(x30), .O(new_n101_));
  nor2   g057(.a(new_n79_), .b(new_n101_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n100_), .c(new_n83_), .O(new_n103_));
  nor2   g059(.a(new_n103_), .b(new_n99_), .O(z6));
  inv1   g060(.a(x31), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(x30), .O(new_n106_));
  nor3   g062(.a(new_n106_), .b(new_n82_), .c(new_n79_), .O(new_n107_));
  aoi21  g063(.a(new_n92_), .b(new_n105_), .c(new_n101_), .O(new_n108_));
  oai21  g064(.a(new_n92_), .b(new_n105_), .c(new_n108_), .O(new_n109_));
  oai21  g065(.a(new_n107_), .b(new_n46_), .c(new_n109_), .O(z7));
  nand2  g066(.a(new_n89_), .b(x29), .O(new_n111_));
  nand2  g067(.a(new_n54_), .b(x31), .O(new_n112_));
  inv1   g068(.a(x17), .O(new_n113_));
  inv1   g069(.a(x19), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x18), .c(x14), .O(new_n115_));
  inv1   g071(.a(x18), .O(new_n116_));
  nand3  g072(.a(x19), .b(new_n116_), .c(x13), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  nand4  g074(.a(x19), .b(x18), .c(new_n113_), .d(x12), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n118_), .c(x16), .O(new_n121_));
  inv1   g077(.a(x16), .O(new_n122_));
  nand3  g078(.a(x18), .b(x17), .c(x11), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x19), .c(new_n122_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand3  g082(.a(x18), .b(x17), .c(x16), .O(new_n127_));
  nand4  g083(.a(x30), .b(x19), .c(new_n45_), .d(x10), .O(new_n128_));
  nor2   g084(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  aoi21  g085(.a(new_n126_), .b(x15), .c(new_n129_), .O(new_n130_));
  nand2  g086(.a(x32), .b(new_n101_), .O(new_n131_));
  oai21  g087(.a(new_n130_), .b(new_n112_), .c(new_n131_), .O(new_n132_));
  inv1   g088(.a(new_n111_), .O(new_n133_));
  inv1   g089(.a(new_n127_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x10), .O(new_n135_));
  nand4  g091(.a(new_n54_), .b(x31), .c(new_n101_), .d(x19), .O(new_n136_));
  oai22  g092(.a(new_n136_), .b(new_n135_), .c(new_n98_), .d(new_n54_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  nand2  g094(.a(x18), .b(x12), .O(new_n139_));
  oai21  g095(.a(x18), .b(x13), .c(x17), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n139_), .c(new_n122_), .O(new_n141_));
  nand2  g097(.a(new_n134_), .b(x15), .O(new_n142_));
  aoi22  g098(.a(new_n142_), .b(new_n114_), .c(new_n135_), .d(new_n45_), .O(new_n143_));
  oai21  g099(.a(new_n141_), .b(new_n124_), .c(new_n143_), .O(new_n144_));
  nand2  g100(.a(new_n114_), .b(x14), .O(new_n145_));
  nand3  g101(.a(new_n134_), .b(new_n145_), .c(x15), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x31), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n144_), .c(x32), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n138_), .O(new_n149_));
  aoi21  g105(.a(new_n132_), .b(new_n111_), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n81_), .b(x00), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n150_), .c(new_n47_), .O(z8));
  nand2  g108(.a(new_n50_), .b(x31), .O(new_n153_));
  nand2  g109(.a(x33), .b(new_n101_), .O(new_n154_));
  oai21  g110(.a(new_n153_), .b(new_n130_), .c(new_n154_), .O(new_n155_));
  nand4  g111(.a(new_n50_), .b(new_n105_), .c(new_n101_), .d(x19), .O(new_n156_));
  oai22  g112(.a(new_n156_), .b(new_n135_), .c(new_n98_), .d(new_n50_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n111_), .O(new_n158_));
  aoi21  g114(.a(new_n134_), .b(new_n145_), .c(new_n105_), .O(new_n159_));
  nor2   g115(.a(x31), .b(new_n101_), .O(new_n160_));
  nor2   g116(.a(new_n160_), .b(new_n106_), .O(new_n161_));
  oai21  g117(.a(new_n159_), .b(new_n45_), .c(new_n161_), .O(new_n162_));
  oai21  g118(.a(new_n162_), .b(new_n144_), .c(x33), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n158_), .O(new_n164_));
  aoi21  g120(.a(new_n155_), .b(new_n133_), .c(new_n164_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n151_), .c(new_n47_), .O(z9));
endmodule


