// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_;
  inv1   g000(.a(x02), .O(new_n45_));
  nor2   g001(.a(x30), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  nor2   g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g007(.a(x33), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(x03), .c(new_n45_), .O(new_n53_));
  nor2   g009(.a(new_n50_), .b(x03), .O(new_n54_));
  nand2  g010(.a(x33), .b(x03), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(x30), .c(x02), .O(new_n56_));
  oai22  g012(.a(new_n56_), .b(new_n54_), .c(new_n53_), .d(new_n51_), .O(z1));
  nor2   g013(.a(new_n49_), .b(new_n45_), .O(new_n58_));
  nor2   g014(.a(x03), .b(x02), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  nand2  g016(.a(new_n59_), .b(x05), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(x01), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n49_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n45_), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  xor2a  g022(.a(x07), .b(x04), .O(new_n67_));
  aoi21  g023(.a(new_n66_), .b(new_n60_), .c(new_n67_), .O(new_n68_));
  inv1   g024(.a(x01), .O(new_n69_));
  nand2  g025(.a(new_n67_), .b(new_n60_), .O(new_n70_));
  aoi21  g026(.a(new_n65_), .b(new_n69_), .c(new_n70_), .O(new_n71_));
  inv1   g027(.a(x08), .O(new_n72_));
  nor2   g028(.a(x09), .b(new_n72_), .O(new_n73_));
  oai21  g029(.a(new_n71_), .b(new_n68_), .c(new_n73_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n47_), .O(z2));
  inv1   g031(.a(x27), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(x22), .b(x17), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  inv1   g035(.a(x19), .O(new_n80_));
  inv1   g036(.a(x24), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g038(.a(x21), .b(x16), .O(new_n83_));
  nor2   g039(.a(x20), .b(x15), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n82_), .c(new_n79_), .O(new_n86_));
  nand2  g042(.a(x30), .b(new_n49_), .O(new_n87_));
  inv1   g043(.a(x26), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(x25), .c(x01), .O(new_n89_));
  aoi21  g045(.a(new_n87_), .b(x02), .c(new_n89_), .O(new_n90_));
  and2   g046(.a(new_n90_), .b(new_n86_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n76_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z3));
  inv1   g049(.a(new_n89_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n86_), .O(new_n95_));
  inv1   g051(.a(new_n58_), .O(new_n96_));
  inv1   g052(.a(x28), .O(new_n97_));
  nor2   g053(.a(new_n97_), .b(new_n76_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n97_), .b(new_n76_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n99_), .c(new_n96_), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n95_), .c(new_n47_), .O(z4));
  inv1   g058(.a(x29), .O(new_n103_));
  nand2  g059(.a(new_n99_), .b(new_n103_), .O(new_n104_));
  nand2  g060(.a(new_n98_), .b(x29), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n104_), .c(new_n91_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(z5));
  aoi21  g063(.a(new_n94_), .b(new_n86_), .c(new_n46_), .O(new_n108_));
  inv1   g064(.a(x30), .O(new_n109_));
  aoi21  g065(.a(new_n105_), .b(new_n96_), .c(new_n109_), .O(new_n110_));
  nand3  g066(.a(new_n105_), .b(new_n109_), .c(new_n45_), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nor3   g068(.a(new_n112_), .b(new_n110_), .c(new_n108_), .O(z6));
  inv1   g069(.a(new_n108_), .O(new_n114_));
  nand3  g070(.a(new_n98_), .b(x30), .c(x29), .O(new_n115_));
  inv1   g071(.a(x31), .O(new_n116_));
  nor2   g072(.a(new_n46_), .b(new_n116_), .O(new_n117_));
  nand2  g073(.a(new_n96_), .b(x31), .O(new_n118_));
  aoi22  g074(.a(new_n118_), .b(new_n110_), .c(new_n117_), .d(new_n115_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n114_), .O(z7));
  inv1   g076(.a(x17), .O(new_n121_));
  nand3  g077(.a(new_n80_), .b(x18), .c(x14), .O(new_n122_));
  inv1   g078(.a(x18), .O(new_n123_));
  nand3  g079(.a(x19), .b(new_n123_), .c(x13), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n122_), .c(new_n121_), .O(new_n125_));
  nand4  g081(.a(x19), .b(x18), .c(new_n121_), .d(x12), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n125_), .c(x16), .O(new_n128_));
  inv1   g084(.a(x16), .O(new_n129_));
  nand3  g085(.a(x18), .b(x17), .c(x11), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x19), .c(new_n129_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nor2   g089(.a(new_n123_), .b(new_n121_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x16), .c(x10), .O(new_n135_));
  nor3   g091(.a(new_n135_), .b(new_n80_), .c(x15), .O(new_n136_));
  aoi21  g092(.a(new_n133_), .b(x15), .c(new_n136_), .O(new_n137_));
  nand2  g093(.a(new_n100_), .b(x29), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n109_), .O(new_n139_));
  nand3  g095(.a(new_n100_), .b(x30), .c(x29), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n139_), .c(x31), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n137_), .c(new_n50_), .O(new_n142_));
  nand2  g098(.a(new_n88_), .b(x00), .O(new_n143_));
  nand3  g099(.a(new_n134_), .b(x16), .c(x15), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  aoi21  g101(.a(new_n130_), .b(new_n129_), .c(new_n80_), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  nand2  g103(.a(x18), .b(x12), .O(new_n148_));
  oai21  g104(.a(x18), .b(x13), .c(x17), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand2  g106(.a(new_n80_), .b(x14), .O(new_n151_));
  nand2  g107(.a(new_n145_), .b(new_n151_), .O(new_n152_));
  inv1   g108(.a(x15), .O(new_n153_));
  nand2  g109(.a(new_n135_), .b(new_n153_), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(new_n152_), .c(new_n150_), .O(new_n155_));
  nor2   g111(.a(new_n155_), .b(new_n147_), .O(new_n156_));
  nand4  g112(.a(new_n140_), .b(new_n139_), .c(x32), .d(x31), .O(new_n157_));
  inv1   g113(.a(new_n157_), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n156_), .c(new_n143_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n142_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n47_), .O(z8));
  or2    g117(.a(new_n140_), .b(new_n116_), .O(new_n162_));
  nand3  g118(.a(new_n138_), .b(new_n116_), .c(new_n109_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n156_), .O(new_n165_));
  nor2   g121(.a(new_n46_), .b(new_n52_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  aoi21  g123(.a(new_n132_), .b(new_n128_), .c(new_n153_), .O(new_n168_));
  inv1   g124(.a(new_n163_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n45_), .O(new_n170_));
  aoi21  g126(.a(new_n170_), .b(new_n162_), .c(x33), .O(new_n171_));
  oai21  g127(.a(new_n136_), .b(new_n168_), .c(new_n171_), .O(new_n172_));
  aoi21  g128(.a(new_n172_), .b(new_n167_), .c(new_n143_), .O(z9));
endmodule


