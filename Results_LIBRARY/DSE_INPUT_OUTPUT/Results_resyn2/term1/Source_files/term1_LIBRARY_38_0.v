// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_;
  nor2   g000(.a(x26), .b(x24), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(x03), .O(new_n47_));
  inv1   g003(.a(x33), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  aoi21  g005(.a(x32), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n48_), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(x33), .b(new_n47_), .c(x02), .O(new_n53_));
  oai21  g009(.a(new_n52_), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g011(.a(new_n45_), .O(new_n56_));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n47_), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(x06), .c(new_n49_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n49_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(x05), .c(new_n47_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nor2   g020(.a(new_n47_), .b(new_n49_), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(new_n66_));
  aoi21  g022(.a(new_n47_), .b(new_n49_), .c(new_n58_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n57_), .O(new_n69_));
  inv1   g025(.a(x09), .O(new_n70_));
  nand2  g026(.a(new_n68_), .b(new_n57_), .O(new_n71_));
  nand3  g027(.a(new_n71_), .b(new_n70_), .c(x08), .O(new_n72_));
  oai21  g028(.a(new_n72_), .b(new_n69_), .c(new_n56_), .O(z2));
  inv1   g029(.a(x27), .O(new_n74_));
  inv1   g030(.a(x24), .O(new_n75_));
  nor2   g031(.a(x26), .b(new_n75_), .O(new_n76_));
  inv1   g032(.a(x16), .O(new_n77_));
  inv1   g033(.a(x21), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  inv1   g035(.a(x17), .O(new_n80_));
  inv1   g036(.a(x22), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  inv1   g038(.a(x15), .O(new_n83_));
  inv1   g039(.a(x20), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(x18), .O(new_n86_));
  inv1   g042(.a(x23), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n85_), .c(new_n82_), .d(new_n79_), .O(new_n89_));
  nand2  g045(.a(x25), .b(x01), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n65_), .O(new_n91_));
  and2   g047(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n76_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n74_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z3));
  inv1   g052(.a(x28), .O(new_n97_));
  nor2   g053(.a(new_n97_), .b(new_n74_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n97_), .b(new_n74_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n99_), .c(new_n92_), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(x24), .c(x26), .O(z4));
  nand2  g058(.a(new_n98_), .b(x29), .O(new_n103_));
  inv1   g059(.a(x29), .O(new_n104_));
  nand2  g060(.a(new_n99_), .b(new_n104_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n103_), .c(new_n94_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(z5));
  nand3  g063(.a(new_n98_), .b(x30), .c(x29), .O(new_n108_));
  inv1   g064(.a(x30), .O(new_n109_));
  nand2  g065(.a(new_n103_), .b(new_n109_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(new_n108_), .c(new_n94_), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(z6));
  xor2a  g068(.a(new_n108_), .b(x31), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n94_), .O(z7));
  inv1   g070(.a(x19), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x18), .c(x14), .O(new_n116_));
  nand3  g072(.a(x19), .b(new_n86_), .c(x13), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n116_), .c(new_n80_), .O(new_n118_));
  nand4  g074(.a(x19), .b(x18), .c(new_n80_), .d(x12), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n118_), .c(x16), .O(new_n121_));
  nand3  g077(.a(x18), .b(x17), .c(x11), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x19), .c(new_n77_), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n121_), .c(new_n83_), .O(new_n125_));
  nor2   g081(.a(new_n86_), .b(new_n80_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x16), .c(x10), .O(new_n127_));
  nor3   g083(.a(new_n127_), .b(new_n115_), .c(x15), .O(new_n128_));
  inv1   g084(.a(x31), .O(new_n129_));
  nor2   g085(.a(x32), .b(new_n129_), .O(new_n130_));
  oai21  g086(.a(new_n128_), .b(new_n125_), .c(new_n130_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n109_), .O(new_n132_));
  nand2  g088(.a(new_n100_), .b(x29), .O(new_n133_));
  aoi21  g089(.a(new_n52_), .b(x30), .c(new_n133_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g091(.a(new_n131_), .b(x30), .O(new_n136_));
  aoi22  g092(.a(new_n100_), .b(x29), .c(new_n52_), .d(new_n109_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  oai21  g094(.a(new_n123_), .b(x16), .c(x15), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n127_), .O(new_n140_));
  nand3  g096(.a(new_n126_), .b(x16), .c(x15), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n115_), .O(new_n142_));
  nand2  g098(.a(new_n115_), .b(x14), .O(new_n143_));
  inv1   g099(.a(new_n141_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n142_), .c(new_n140_), .O(new_n146_));
  nand2  g102(.a(x18), .b(x12), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n80_), .O(new_n148_));
  inv1   g104(.a(x13), .O(new_n149_));
  nand2  g105(.a(new_n86_), .b(new_n149_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n148_), .c(x31), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n146_), .c(x32), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n138_), .c(new_n135_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(x00), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(x24), .c(x26), .O(z8));
  aoi21  g111(.a(x31), .b(x30), .c(new_n133_), .O(new_n156_));
  nand2  g112(.a(new_n129_), .b(new_n109_), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(new_n133_), .c(new_n156_), .O(new_n158_));
  oai21  g114(.a(new_n128_), .b(new_n125_), .c(new_n158_), .O(new_n159_));
  nand4  g115(.a(new_n158_), .b(new_n150_), .c(new_n148_), .d(x33), .O(new_n160_));
  nand2  g116(.a(new_n76_), .b(x00), .O(new_n161_));
  inv1   g117(.a(new_n161_), .O(new_n162_));
  oai21  g118(.a(new_n160_), .b(new_n146_), .c(new_n162_), .O(new_n163_));
  aoi21  g119(.a(new_n159_), .b(new_n48_), .c(new_n163_), .O(z9));
endmodule


