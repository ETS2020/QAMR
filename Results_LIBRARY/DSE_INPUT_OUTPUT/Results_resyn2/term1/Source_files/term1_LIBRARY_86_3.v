// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  inv1   g000(.a(x15), .O(new_n45_));
  inv1   g001(.a(x26), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(new_n51_), .b(new_n52_), .c(new_n47_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(x33), .c(new_n53_), .O(z1));
  inv1   g010(.a(x01), .O(new_n55_));
  aoi21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  oai21  g012(.a(x03), .b(x02), .c(new_n56_), .O(new_n57_));
  inv1   g013(.a(x03), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(new_n64_));
  xnor2a g020(.a(x07), .b(x04), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(x08), .O(new_n67_));
  aoi21  g023(.a(new_n65_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  oai21  g024(.a(new_n65_), .b(new_n64_), .c(new_n68_), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n48_), .O(z2));
  nor2   g026(.a(x20), .b(x15), .O(new_n71_));
  nor2   g027(.a(x22), .b(x17), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g029(.a(x19), .O(new_n74_));
  inv1   g030(.a(x24), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(x21), .b(x16), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(new_n80_));
  nand3  g036(.a(new_n56_), .b(new_n46_), .c(x25), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(x27), .c(new_n48_), .O(z3));
  inv1   g040(.a(new_n83_), .O(new_n85_));
  inv1   g041(.a(x27), .O(new_n86_));
  inv1   g042(.a(x28), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n87_), .b(new_n86_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n89_), .c(new_n85_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z4));
  nand2  g048(.a(new_n88_), .b(x29), .O(new_n93_));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n93_), .c(new_n85_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(z5));
  nand3  g053(.a(new_n88_), .b(x30), .c(x29), .O(new_n98_));
  inv1   g054(.a(x30), .O(new_n99_));
  nand2  g055(.a(new_n93_), .b(new_n99_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n83_), .c(new_n48_), .O(z6));
  xor2a  g058(.a(new_n98_), .b(x31), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n85_), .c(new_n47_), .O(z7));
  inv1   g060(.a(x17), .O(new_n105_));
  nand3  g061(.a(new_n74_), .b(x18), .c(x14), .O(new_n106_));
  inv1   g062(.a(x18), .O(new_n107_));
  nand3  g063(.a(x19), .b(new_n107_), .c(x13), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  nand4  g065(.a(x19), .b(x18), .c(new_n105_), .d(x12), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n109_), .c(x16), .O(new_n112_));
  inv1   g068(.a(x16), .O(new_n113_));
  nand3  g069(.a(x18), .b(x17), .c(x11), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x19), .c(new_n113_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n112_), .c(x15), .O(new_n117_));
  nand3  g073(.a(x18), .b(x17), .c(x16), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x10), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n74_), .c(new_n45_), .O(new_n121_));
  nand2  g077(.a(new_n90_), .b(x29), .O(new_n122_));
  xor2a  g078(.a(new_n122_), .b(new_n99_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n121_), .c(new_n117_), .d(x31), .O(new_n124_));
  nand2  g080(.a(new_n114_), .b(new_n113_), .O(new_n125_));
  oai21  g081(.a(x17), .b(x12), .c(x18), .O(new_n126_));
  nand2  g082(.a(x17), .b(x13), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n125_), .c(x32), .d(x31), .O(new_n129_));
  nand2  g085(.a(new_n74_), .b(x14), .O(new_n130_));
  nand3  g086(.a(new_n119_), .b(new_n130_), .c(x15), .O(new_n131_));
  nand2  g087(.a(new_n120_), .b(new_n45_), .O(new_n132_));
  oai21  g088(.a(new_n118_), .b(new_n45_), .c(new_n74_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n123_), .O(new_n134_));
  inv1   g090(.a(x00), .O(new_n135_));
  nor2   g091(.a(x26), .b(new_n135_), .O(new_n136_));
  oai21  g092(.a(new_n134_), .b(new_n129_), .c(new_n136_), .O(new_n137_));
  aoi21  g093(.a(new_n124_), .b(new_n52_), .c(new_n137_), .O(z8));
  inv1   g094(.a(new_n122_), .O(new_n139_));
  inv1   g095(.a(x33), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(x30), .O(new_n141_));
  nand3  g097(.a(new_n140_), .b(x17), .c(x16), .O(new_n142_));
  nand4  g098(.a(x19), .b(x18), .c(new_n45_), .d(x10), .O(new_n143_));
  nand2  g099(.a(x31), .b(x30), .O(new_n144_));
  nor3   g100(.a(new_n144_), .b(new_n143_), .c(new_n142_), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n141_), .c(new_n46_), .O(new_n146_));
  nand2  g102(.a(new_n116_), .b(new_n112_), .O(new_n147_));
  nand3  g103(.a(x31), .b(x30), .c(x15), .O(new_n148_));
  inv1   g104(.a(new_n148_), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n147_), .c(new_n140_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n146_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n139_), .O(new_n152_));
  nor2   g108(.a(new_n140_), .b(new_n99_), .O(new_n153_));
  inv1   g109(.a(x31), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n99_), .O(new_n155_));
  nor3   g111(.a(new_n155_), .b(new_n143_), .c(new_n142_), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n153_), .c(new_n46_), .O(new_n157_));
  nand3  g113(.a(new_n154_), .b(new_n99_), .c(x15), .O(new_n158_));
  inv1   g114(.a(new_n158_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n147_), .c(new_n140_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n157_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n122_), .O(new_n162_));
  nand2  g118(.a(new_n155_), .b(new_n144_), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(new_n121_), .c(x26), .O(new_n164_));
  nand2  g120(.a(new_n163_), .b(new_n128_), .O(new_n165_));
  nand2  g121(.a(new_n119_), .b(new_n130_), .O(new_n166_));
  nand2  g122(.a(new_n118_), .b(new_n74_), .O(new_n167_));
  nand3  g123(.a(new_n167_), .b(new_n166_), .c(new_n125_), .O(new_n168_));
  nor2   g124(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  nor2   g125(.a(new_n169_), .b(new_n45_), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n164_), .c(x33), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n162_), .c(new_n152_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(x00), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n48_), .O(z9));
endmodule


