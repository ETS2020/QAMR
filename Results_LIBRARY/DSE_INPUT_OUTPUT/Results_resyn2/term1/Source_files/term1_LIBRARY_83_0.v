// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  nor2   g000(.a(x18), .b(x16), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(new_n45_), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  xor2a  g004(.a(x03), .b(x02), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  or2    g006(.a(new_n49_), .b(x33), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(z1));
  xor2a  g008(.a(x07), .b(x04), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nand2  g010(.a(x05), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(x02), .O(new_n56_));
  nand2  g012(.a(x06), .b(new_n56_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  or2    g014(.a(new_n57_), .b(new_n55_), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  aoi21  g016(.a(new_n49_), .b(x01), .c(new_n60_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  nand2  g019(.a(new_n61_), .b(new_n53_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n63_), .c(x08), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n62_), .c(new_n47_), .O(z2));
  inv1   g022(.a(x27), .O(new_n67_));
  nor2   g023(.a(x21), .b(x16), .O(new_n68_));
  nor2   g024(.a(x22), .b(x17), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g026(.a(x19), .O(new_n71_));
  inv1   g027(.a(x24), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  nor2   g030(.a(x23), .b(x18), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(new_n77_));
  inv1   g033(.a(x26), .O(new_n78_));
  nand2  g034(.a(x03), .b(x02), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n78_), .c(x25), .d(x01), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n77_), .c(new_n67_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n47_), .O(z3));
  nor2   g039(.a(new_n80_), .b(new_n45_), .O(new_n84_));
  and2   g040(.a(new_n84_), .b(new_n77_), .O(new_n85_));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n67_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n67_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n88_), .c(new_n85_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z4));
  nand2  g047(.a(new_n87_), .b(x29), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  nand2  g049(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n92_), .c(new_n81_), .d(new_n77_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n47_), .O(z5));
  nand3  g052(.a(new_n87_), .b(x30), .c(x29), .O(new_n97_));
  inv1   g053(.a(x30), .O(new_n98_));
  nand2  g054(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n97_), .c(new_n85_), .O(new_n100_));
  inv1   g056(.a(new_n100_), .O(z6));
  xor2a  g057(.a(new_n97_), .b(x31), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n85_), .O(z7));
  nand2  g059(.a(x18), .b(x16), .O(new_n104_));
  nand3  g060(.a(new_n71_), .b(x17), .c(x14), .O(new_n105_));
  inv1   g061(.a(x17), .O(new_n106_));
  nand3  g062(.a(x19), .b(new_n106_), .c(x12), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  inv1   g064(.a(x16), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x11), .O(new_n110_));
  inv1   g066(.a(x18), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x13), .O(new_n112_));
  nand2  g068(.a(x19), .b(x17), .O(new_n113_));
  aoi21  g069(.a(new_n112_), .b(new_n110_), .c(new_n113_), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n108_), .c(x15), .O(new_n115_));
  inv1   g071(.a(x15), .O(new_n116_));
  inv1   g072(.a(new_n104_), .O(new_n117_));
  inv1   g073(.a(new_n113_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n117_), .c(new_n116_), .d(x10), .O(new_n119_));
  nand2  g075(.a(new_n48_), .b(x31), .O(new_n120_));
  aoi21  g076(.a(new_n119_), .b(new_n115_), .c(new_n120_), .O(new_n121_));
  nand2  g077(.a(new_n89_), .b(x29), .O(new_n122_));
  aoi21  g078(.a(new_n48_), .b(x30), .c(new_n122_), .O(new_n123_));
  oai21  g079(.a(new_n121_), .b(x30), .c(new_n123_), .O(new_n124_));
  aoi22  g080(.a(new_n89_), .b(x29), .c(new_n48_), .d(new_n98_), .O(new_n125_));
  oai21  g081(.a(new_n121_), .b(new_n98_), .c(new_n125_), .O(new_n126_));
  nand3  g082(.a(x18), .b(x17), .c(x15), .O(new_n127_));
  nand3  g083(.a(x18), .b(x17), .c(x10), .O(new_n128_));
  aoi22  g084(.a(new_n128_), .b(new_n116_), .c(new_n127_), .d(new_n71_), .O(new_n129_));
  nand2  g085(.a(x15), .b(x11), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n113_), .c(new_n109_), .O(new_n131_));
  nand2  g087(.a(new_n71_), .b(x14), .O(new_n132_));
  inv1   g088(.a(new_n127_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n132_), .c(x16), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n131_), .c(new_n129_), .O(new_n135_));
  aoi21  g091(.a(x18), .b(x12), .c(x17), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  nor2   g093(.a(x18), .b(x13), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n137_), .c(x31), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n135_), .c(x32), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n126_), .c(new_n124_), .O(new_n142_));
  inv1   g098(.a(x00), .O(new_n143_));
  nor2   g099(.a(x26), .b(new_n143_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n47_), .O(z8));
  nand2  g102(.a(new_n119_), .b(new_n115_), .O(new_n147_));
  aoi21  g103(.a(x31), .b(x30), .c(new_n122_), .O(new_n148_));
  inv1   g104(.a(x31), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n98_), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n122_), .c(new_n148_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n147_), .c(x33), .O(new_n152_));
  nand4  g108(.a(new_n151_), .b(new_n139_), .c(new_n137_), .d(x33), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n135_), .c(new_n144_), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n152_), .c(new_n47_), .O(z9));
endmodule


