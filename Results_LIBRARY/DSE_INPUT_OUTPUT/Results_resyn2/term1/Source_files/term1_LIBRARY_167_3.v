// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  nand2  g000(.a(x30), .b(x25), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(x33), .O(new_n47_));
  xnor2a g003(.a(x03), .b(x02), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  inv1   g006(.a(new_n48_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g008(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(z1));
  xor2a  g009(.a(x07), .b(x04), .O(new_n54_));
  inv1   g010(.a(x01), .O(new_n55_));
  inv1   g011(.a(x02), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  nand4  g013(.a(x06), .b(x05), .c(new_n57_), .d(new_n56_), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n57_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n56_), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(new_n62_));
  oai21  g018(.a(new_n51_), .b(new_n55_), .c(new_n62_), .O(new_n63_));
  inv1   g019(.a(x09), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(x08), .O(new_n65_));
  aoi21  g021(.a(new_n63_), .b(new_n54_), .c(new_n65_), .O(new_n66_));
  oai21  g022(.a(new_n63_), .b(new_n54_), .c(new_n66_), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n45_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  nand2  g025(.a(x03), .b(x02), .O(new_n70_));
  nor2   g026(.a(x20), .b(x15), .O(new_n71_));
  nor2   g027(.a(x24), .b(x19), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g029(.a(x18), .O(new_n74_));
  inv1   g030(.a(x23), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x22), .b(x17), .O(new_n77_));
  nor2   g033(.a(x21), .b(x16), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(new_n80_));
  and2   g036(.a(new_n80_), .b(new_n70_), .O(new_n81_));
  nor2   g037(.a(x26), .b(new_n55_), .O(new_n82_));
  inv1   g038(.a(x25), .O(new_n83_));
  nor2   g039(.a(x30), .b(new_n83_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n69_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(z3));
  nand2  g044(.a(x28), .b(x27), .O(new_n89_));
  inv1   g045(.a(x28), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n69_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n89_), .c(new_n86_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z4));
  inv1   g049(.a(x30), .O(new_n94_));
  nand3  g050(.a(x29), .b(x28), .c(x27), .O(new_n95_));
  inv1   g051(.a(x29), .O(new_n96_));
  nand2  g052(.a(new_n89_), .b(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n95_), .c(new_n82_), .d(new_n81_), .O(new_n98_));
  aoi21  g054(.a(new_n98_), .b(new_n94_), .c(new_n83_), .O(z5));
  inv1   g055(.a(new_n95_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n82_), .c(new_n81_), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(new_n94_), .c(new_n83_), .O(z6));
  inv1   g058(.a(x31), .O(new_n103_));
  nand2  g059(.a(new_n86_), .b(new_n103_), .O(z7));
  inv1   g060(.a(z0), .O(new_n105_));
  nand4  g061(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  inv1   g063(.a(x16), .O(new_n108_));
  nand3  g064(.a(x18), .b(x17), .c(x11), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(x19), .c(new_n107_), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand2  g068(.a(x17), .b(x13), .O(new_n113_));
  oai21  g069(.a(x17), .b(x12), .c(x18), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  inv1   g071(.a(x19), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x14), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n107_), .O(new_n118_));
  inv1   g074(.a(x15), .O(new_n119_));
  nand4  g075(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n118_), .c(new_n115_), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n112_), .O(new_n124_));
  oai21  g080(.a(x28), .b(x27), .c(x29), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nor2   g082(.a(new_n126_), .b(x30), .O(new_n127_));
  oai21  g083(.a(new_n125_), .b(new_n94_), .c(x31), .O(new_n128_));
  or2    g084(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n124_), .c(new_n105_), .O(new_n130_));
  inv1   g086(.a(x17), .O(new_n131_));
  nand3  g087(.a(new_n116_), .b(x18), .c(x14), .O(new_n132_));
  nand3  g088(.a(x19), .b(new_n74_), .c(x13), .O(new_n133_));
  aoi21  g089(.a(new_n133_), .b(new_n132_), .c(new_n131_), .O(new_n134_));
  nand4  g090(.a(x19), .b(x18), .c(new_n131_), .d(x12), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n134_), .c(x16), .O(new_n137_));
  nand3  g093(.a(x19), .b(x18), .c(x17), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n108_), .c(x11), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n137_), .c(new_n119_), .O(new_n141_));
  nand3  g097(.a(x16), .b(new_n119_), .c(x10), .O(new_n142_));
  nor2   g098(.a(new_n142_), .b(new_n138_), .O(new_n143_));
  nor2   g099(.a(new_n94_), .b(x25), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n126_), .c(new_n50_), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(new_n128_), .O(new_n146_));
  oai21  g102(.a(new_n143_), .b(new_n141_), .c(new_n146_), .O(new_n147_));
  inv1   g103(.a(x26), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(x00), .O(new_n149_));
  aoi21  g105(.a(new_n147_), .b(new_n130_), .c(new_n149_), .O(z8));
  oai21  g106(.a(new_n122_), .b(new_n111_), .c(new_n45_), .O(new_n151_));
  nand2  g107(.a(new_n127_), .b(new_n103_), .O(new_n152_));
  oai21  g108(.a(new_n125_), .b(new_n103_), .c(new_n83_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(x30), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(x33), .O(new_n157_));
  nand3  g113(.a(new_n144_), .b(new_n126_), .c(x31), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n152_), .c(x33), .O(new_n159_));
  oai21  g115(.a(new_n143_), .b(new_n141_), .c(new_n159_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n157_), .c(new_n149_), .O(z9));
endmodule


