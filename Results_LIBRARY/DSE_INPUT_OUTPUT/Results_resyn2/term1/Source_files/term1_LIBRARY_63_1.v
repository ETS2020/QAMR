// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x31), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x26), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  nand2  g004(.a(x33), .b(x03), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(x32), .b(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n49_), .c(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  nand2  g009(.a(x32), .b(x03), .O(new_n54_));
  nand2  g010(.a(x33), .b(new_n50_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n52_), .c(new_n46_), .O(z1));
  xor2a  g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n50_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x06), .c(new_n53_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n53_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x05), .c(new_n50_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(new_n61_), .c(new_n59_), .O(new_n64_));
  nand2  g020(.a(x03), .b(x02), .O(new_n65_));
  nand2  g021(.a(new_n50_), .b(new_n53_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(new_n65_), .c(x01), .O(new_n67_));
  and2   g023(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  inv1   g025(.a(x09), .O(new_n70_));
  nand2  g026(.a(new_n68_), .b(new_n58_), .O(new_n71_));
  nand3  g027(.a(new_n71_), .b(new_n70_), .c(x08), .O(new_n72_));
  oai21  g028(.a(new_n72_), .b(new_n69_), .c(new_n47_), .O(z2));
  inv1   g029(.a(x27), .O(new_n74_));
  nor2   g030(.a(x23), .b(x18), .O(new_n75_));
  nor2   g031(.a(x22), .b(x17), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g033(.a(x24), .b(x19), .O(new_n78_));
  nor2   g034(.a(x21), .b(x16), .O(new_n79_));
  nor2   g035(.a(x20), .b(x15), .O(new_n80_));
  nor3   g036(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  nand2  g037(.a(x25), .b(x01), .O(new_n82_));
  aoi21  g038(.a(new_n81_), .b(new_n77_), .c(new_n82_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n65_), .c(new_n74_), .O(new_n84_));
  aoi21  g040(.a(new_n84_), .b(new_n45_), .c(x26), .O(z3));
  inv1   g041(.a(x26), .O(new_n86_));
  nand3  g042(.a(new_n65_), .b(new_n45_), .c(new_n86_), .O(new_n87_));
  nor2   g043(.a(x28), .b(x27), .O(new_n88_));
  and2   g044(.a(x28), .b(x27), .O(new_n89_));
  nor3   g045(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n83_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z4));
  nand2  g048(.a(new_n89_), .b(x29), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nor2   g050(.a(new_n89_), .b(x29), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n83_), .c(new_n65_), .O(new_n97_));
  aoi21  g053(.a(new_n97_), .b(new_n45_), .c(x26), .O(z5));
  inv1   g054(.a(x30), .O(new_n99_));
  nand2  g055(.a(new_n93_), .b(new_n99_), .O(new_n100_));
  nand2  g056(.a(new_n94_), .b(x30), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n83_), .c(new_n65_), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  aoi21  g060(.a(new_n104_), .b(new_n45_), .c(x26), .O(z6));
  oai21  g061(.a(new_n103_), .b(x31), .c(new_n86_), .O(z7));
  nand2  g062(.a(x32), .b(x00), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n45_), .c(x26), .O(z8));
  nand2  g064(.a(x16), .b(x10), .O(new_n109_));
  nand2  g065(.a(x15), .b(x11), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  inv1   g067(.a(x15), .O(new_n112_));
  inv1   g068(.a(x16), .O(new_n113_));
  nor2   g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand2  g071(.a(x18), .b(x17), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n115_), .c(new_n111_), .d(x19), .O(new_n118_));
  inv1   g074(.a(x17), .O(new_n119_));
  inv1   g075(.a(x19), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x18), .c(x14), .O(new_n121_));
  inv1   g077(.a(x18), .O(new_n122_));
  nand3  g078(.a(x19), .b(new_n122_), .c(x13), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(new_n121_), .c(new_n119_), .O(new_n124_));
  nand4  g080(.a(x19), .b(x18), .c(new_n119_), .d(x12), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(new_n124_), .c(new_n114_), .O(new_n127_));
  inv1   g083(.a(x29), .O(new_n128_));
  oai21  g084(.a(new_n88_), .b(new_n128_), .c(new_n99_), .O(new_n129_));
  aoi21  g085(.a(new_n127_), .b(new_n118_), .c(new_n129_), .O(new_n130_));
  inv1   g086(.a(x00), .O(new_n131_));
  nand2  g087(.a(x18), .b(x12), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n119_), .O(new_n133_));
  inv1   g089(.a(x13), .O(new_n134_));
  nand2  g090(.a(new_n122_), .b(new_n134_), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n133_), .c(x33), .O(new_n136_));
  nor2   g092(.a(new_n136_), .b(new_n129_), .O(new_n137_));
  nand4  g093(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n120_), .O(new_n139_));
  oai21  g095(.a(new_n116_), .b(new_n109_), .c(new_n112_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  inv1   g097(.a(x14), .O(new_n142_));
  nor2   g098(.a(x19), .b(new_n142_), .O(new_n143_));
  nand3  g099(.a(x18), .b(x17), .c(x11), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n113_), .O(new_n145_));
  oai21  g101(.a(new_n138_), .b(new_n143_), .c(new_n145_), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n141_), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n137_), .c(new_n131_), .O(new_n148_));
  oai21  g104(.a(new_n130_), .b(x33), .c(new_n148_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n45_), .c(x26), .O(z9));
endmodule


