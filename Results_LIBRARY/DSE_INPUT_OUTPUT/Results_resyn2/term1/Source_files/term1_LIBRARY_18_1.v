// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x14), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  aoi21  g007(.a(new_n50_), .b(new_n51_), .c(new_n46_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(x33), .c(new_n52_), .O(z1));
  inv1   g009(.a(x01), .O(new_n54_));
  aoi21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  oai21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  aoi21  g015(.a(x05), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g016(.a(new_n59_), .b(x05), .c(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  oai21  g018(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  xor2a  g019(.a(x07), .b(x04), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand3  g022(.a(new_n47_), .b(new_n66_), .c(x08), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n65_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  nor2   g025(.a(x24), .b(x19), .O(new_n70_));
  nor2   g026(.a(x22), .b(x17), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x16), .O(new_n73_));
  inv1   g029(.a(x21), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x20), .b(x15), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(new_n79_));
  inv1   g035(.a(x26), .O(new_n80_));
  nand3  g036(.a(new_n55_), .b(new_n80_), .c(x25), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n79_), .c(new_n69_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n47_), .O(z3));
  nor2   g040(.a(new_n81_), .b(new_n46_), .O(new_n85_));
  and2   g041(.a(new_n85_), .b(new_n79_), .O(new_n86_));
  inv1   g042(.a(x28), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n69_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n87_), .b(new_n69_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n89_), .c(new_n86_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z4));
  nand2  g048(.a(new_n88_), .b(x29), .O(new_n93_));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n93_), .c(new_n86_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(z5));
  inv1   g053(.a(x30), .O(new_n98_));
  xor2a  g054(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n86_), .O(new_n100_));
  inv1   g056(.a(new_n100_), .O(z6));
  xor2a  g057(.a(x31), .b(x30), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n88_), .c(x29), .O(new_n103_));
  nand2  g059(.a(new_n93_), .b(x31), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n103_), .c(new_n86_), .O(z7));
  nand3  g061(.a(x17), .b(x16), .c(x10), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  nand2  g063(.a(x17), .b(x11), .O(new_n108_));
  nor2   g064(.a(x17), .b(x12), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n73_), .c(new_n108_), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(x15), .c(new_n107_), .O(new_n111_));
  nand3  g067(.a(x17), .b(x16), .c(x15), .O(new_n112_));
  nand2  g068(.a(x19), .b(x18), .O(new_n113_));
  nor2   g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n111_), .c(x14), .O(new_n115_));
  oai21  g071(.a(x28), .b(x27), .c(x29), .O(new_n116_));
  xor2a  g072(.a(new_n116_), .b(new_n98_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x31), .O(new_n118_));
  inv1   g074(.a(x19), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(x14), .c(new_n45_), .O(new_n120_));
  aoi21  g076(.a(x19), .b(x13), .c(x18), .O(new_n121_));
  nor2   g077(.a(new_n121_), .b(new_n112_), .O(new_n122_));
  nor2   g078(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  aoi21  g079(.a(new_n118_), .b(new_n47_), .c(new_n123_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n115_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x32), .O(new_n126_));
  nand4  g082(.a(x19), .b(x17), .c(new_n73_), .d(x11), .O(new_n127_));
  inv1   g083(.a(x17), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(x12), .c(new_n119_), .O(new_n129_));
  oai21  g085(.a(x19), .b(x17), .c(x16), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n129_), .c(new_n127_), .O(new_n131_));
  nor3   g087(.a(new_n106_), .b(new_n119_), .c(x15), .O(new_n132_));
  aoi21  g088(.a(new_n131_), .b(x15), .c(new_n132_), .O(new_n133_));
  nand2  g089(.a(x18), .b(x14), .O(new_n134_));
  inv1   g090(.a(new_n112_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(x19), .c(new_n45_), .d(x13), .O(new_n136_));
  oai21  g092(.a(new_n134_), .b(new_n133_), .c(new_n136_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n117_), .c(new_n51_), .d(x31), .O(new_n138_));
  nand2  g094(.a(new_n80_), .b(x00), .O(new_n139_));
  aoi21  g095(.a(new_n138_), .b(new_n126_), .c(new_n139_), .O(z8));
  nor2   g096(.a(new_n117_), .b(new_n102_), .O(new_n141_));
  inv1   g097(.a(x15), .O(new_n142_));
  nand4  g098(.a(x19), .b(x18), .c(new_n128_), .d(x12), .O(new_n143_));
  nand2  g099(.a(new_n113_), .b(x17), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n121_), .c(new_n143_), .O(new_n145_));
  nor2   g101(.a(new_n127_), .b(new_n45_), .O(new_n146_));
  aoi21  g102(.a(new_n145_), .b(x16), .c(new_n146_), .O(new_n147_));
  nand2  g103(.a(new_n132_), .b(x18), .O(new_n148_));
  oai21  g104(.a(new_n147_), .b(new_n142_), .c(new_n148_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n141_), .c(x33), .O(new_n150_));
  oai21  g106(.a(new_n112_), .b(new_n45_), .c(new_n119_), .O(new_n151_));
  inv1   g107(.a(x33), .O(new_n152_));
  aoi21  g108(.a(new_n107_), .b(x18), .c(x15), .O(new_n153_));
  nor3   g109(.a(new_n153_), .b(new_n114_), .c(new_n152_), .O(new_n154_));
  nand2  g110(.a(x17), .b(x13), .O(new_n155_));
  oai21  g111(.a(new_n109_), .b(new_n45_), .c(new_n155_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(x16), .O(new_n157_));
  oai21  g113(.a(new_n108_), .b(new_n45_), .c(new_n157_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n154_), .c(new_n151_), .d(new_n141_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n80_), .c(x00), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n150_), .c(new_n47_), .O(z9));
endmodule


