// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_;
  inv1   g000(.a(x06), .O(new_n45_));
  inv1   g001(.a(x15), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  inv1   g005(.a(x03), .O(new_n50_));
  inv1   g006(.a(x33), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  aoi21  g008(.a(x32), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(new_n50_), .c(new_n53_), .O(new_n54_));
  inv1   g010(.a(x32), .O(new_n55_));
  aoi21  g011(.a(x33), .b(new_n50_), .c(x02), .O(new_n56_));
  oai21  g012(.a(new_n55_), .b(new_n50_), .c(new_n56_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n54_), .c(new_n47_), .O(z1));
  inv1   g014(.a(x01), .O(new_n59_));
  aoi21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  oai21  g016(.a(x03), .b(x02), .c(new_n60_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n52_), .O(new_n62_));
  aoi21  g018(.a(x05), .b(new_n50_), .c(new_n62_), .O(new_n63_));
  nand3  g019(.a(new_n62_), .b(x05), .c(new_n50_), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n63_), .c(new_n61_), .O(new_n66_));
  xnor2a g022(.a(x07), .b(x04), .O(new_n67_));
  inv1   g023(.a(x09), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(x08), .O(new_n69_));
  aoi21  g025(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  oai21  g026(.a(new_n67_), .b(new_n66_), .c(new_n70_), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n48_), .O(z2));
  inv1   g028(.a(x27), .O(new_n73_));
  nor2   g029(.a(x22), .b(x17), .O(new_n74_));
  nor2   g030(.a(x20), .b(x15), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(x24), .b(x19), .O(new_n78_));
  nor2   g034(.a(x21), .b(x16), .O(new_n79_));
  nor3   g035(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  inv1   g036(.a(x26), .O(new_n81_));
  nand3  g037(.a(new_n60_), .b(new_n81_), .c(x25), .O(new_n82_));
  aoi21  g038(.a(new_n80_), .b(new_n76_), .c(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n48_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n73_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(z3));
  inv1   g043(.a(x28), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n73_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n88_), .b(new_n73_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n90_), .c(new_n83_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n48_), .O(z4));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n90_), .b(new_n94_), .O(new_n95_));
  nand2  g051(.a(new_n89_), .b(x29), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n95_), .c(new_n85_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(z5));
  inv1   g054(.a(x30), .O(new_n99_));
  xor2a  g055(.a(new_n96_), .b(new_n99_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n83_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n48_), .O(z6));
  nand2  g058(.a(x31), .b(x30), .O(new_n103_));
  inv1   g059(.a(x31), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n99_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n96_), .O(new_n107_));
  aoi21  g063(.a(new_n96_), .b(x31), .c(new_n107_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n85_), .O(z7));
  nand3  g065(.a(x18), .b(x17), .c(x16), .O(new_n110_));
  nand2  g066(.a(x19), .b(x10), .O(new_n111_));
  nor2   g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  nand2  g069(.a(new_n91_), .b(x29), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x30), .O(new_n116_));
  aoi21  g072(.a(new_n114_), .b(new_n99_), .c(new_n104_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n113_), .c(new_n46_), .O(new_n119_));
  inv1   g075(.a(new_n110_), .O(new_n120_));
  inv1   g076(.a(x19), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x14), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n120_), .c(x15), .O(new_n123_));
  inv1   g079(.a(x16), .O(new_n124_));
  nand3  g080(.a(x18), .b(x17), .c(x11), .O(new_n125_));
  aoi22  g081(.a(new_n125_), .b(new_n124_), .c(new_n110_), .d(new_n121_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  nand2  g083(.a(x17), .b(x13), .O(new_n128_));
  oai21  g084(.a(x17), .b(x12), .c(x18), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n117_), .c(new_n116_), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n127_), .c(new_n45_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n119_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x32), .O(new_n134_));
  inv1   g090(.a(x17), .O(new_n135_));
  nand3  g091(.a(new_n121_), .b(x18), .c(x14), .O(new_n136_));
  inv1   g092(.a(x18), .O(new_n137_));
  nand3  g093(.a(x19), .b(new_n137_), .c(x13), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n136_), .c(new_n135_), .O(new_n139_));
  nand4  g095(.a(x19), .b(x18), .c(new_n135_), .d(x12), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n139_), .c(x16), .O(new_n142_));
  inv1   g098(.a(new_n125_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(x19), .c(new_n124_), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n142_), .c(x06), .O(new_n145_));
  oai21  g101(.a(new_n112_), .b(x15), .c(new_n55_), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n118_), .O(new_n147_));
  oai21  g103(.a(new_n145_), .b(new_n46_), .c(new_n147_), .O(new_n148_));
  nand2  g104(.a(new_n81_), .b(x00), .O(new_n149_));
  aoi21  g105(.a(new_n148_), .b(new_n134_), .c(new_n149_), .O(z8));
  nand2  g106(.a(new_n112_), .b(new_n106_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n46_), .O(new_n152_));
  nand2  g108(.a(new_n130_), .b(new_n106_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n127_), .c(new_n45_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x33), .O(new_n156_));
  nand2  g112(.a(x33), .b(new_n99_), .O(new_n157_));
  nand2  g113(.a(new_n144_), .b(new_n142_), .O(new_n158_));
  nand3  g114(.a(x31), .b(x30), .c(x15), .O(new_n159_));
  inv1   g115(.a(new_n159_), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n158_), .c(new_n51_), .O(new_n161_));
  aoi21  g117(.a(new_n161_), .b(new_n157_), .c(x06), .O(new_n162_));
  nand2  g118(.a(new_n112_), .b(new_n51_), .O(new_n163_));
  oai21  g119(.a(new_n163_), .b(new_n103_), .c(new_n157_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n46_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n115_), .O(new_n166_));
  nand2  g122(.a(x33), .b(x30), .O(new_n167_));
  nand3  g123(.a(new_n104_), .b(new_n99_), .c(x15), .O(new_n168_));
  inv1   g124(.a(new_n168_), .O(new_n169_));
  nand3  g125(.a(new_n169_), .b(new_n158_), .c(new_n51_), .O(new_n170_));
  aoi21  g126(.a(new_n170_), .b(new_n167_), .c(x06), .O(new_n171_));
  oai21  g127(.a(new_n163_), .b(new_n105_), .c(new_n167_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n46_), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n114_), .O(new_n174_));
  oai22  g130(.a(new_n174_), .b(new_n171_), .c(new_n166_), .d(new_n162_), .O(new_n175_));
  aoi21  g131(.a(new_n175_), .b(new_n156_), .c(new_n149_), .O(z9));
endmodule


