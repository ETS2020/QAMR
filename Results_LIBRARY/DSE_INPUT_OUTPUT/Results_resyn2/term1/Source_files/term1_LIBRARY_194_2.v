// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x29), .O(new_n45_));
  nor2   g001(.a(x30), .b(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(x33), .O(new_n53_));
  aoi21  g009(.a(new_n52_), .b(new_n53_), .c(new_n46_), .O(new_n54_));
  oai21  g010(.a(new_n52_), .b(x32), .c(new_n54_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  nand2  g013(.a(new_n52_), .b(x01), .O(new_n58_));
  nand4  g014(.a(x06), .b(x05), .c(new_n50_), .d(new_n49_), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n50_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n49_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n57_), .O(new_n65_));
  nor4   g021(.a(new_n65_), .b(new_n46_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g022(.a(new_n46_), .O(new_n67_));
  nor2   g023(.a(x23), .b(x18), .O(new_n68_));
  nor2   g024(.a(x22), .b(x17), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g026(.a(x19), .O(new_n71_));
  inv1   g027(.a(x24), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g029(.a(x21), .b(x16), .O(new_n74_));
  nor2   g030(.a(x20), .b(x15), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n48_), .O(new_n78_));
  inv1   g034(.a(x27), .O(new_n79_));
  nand2  g035(.a(x25), .b(x01), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(x26), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n78_), .c(new_n67_), .O(z3));
  nor2   g039(.a(x28), .b(x27), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n79_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n85_), .c(new_n81_), .d(new_n67_), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(new_n78_), .O(z4));
  inv1   g046(.a(x30), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n45_), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(new_n87_), .O(new_n93_));
  nand2  g049(.a(new_n81_), .b(new_n48_), .O(new_n94_));
  aoi21  g050(.a(new_n87_), .b(x29), .c(new_n94_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n77_), .O(new_n96_));
  or2    g052(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(z5));
  oai21  g054(.a(new_n96_), .b(new_n91_), .c(new_n67_), .O(z6));
  oai21  g055(.a(new_n88_), .b(x31), .c(x30), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(x29), .O(new_n101_));
  nand2  g057(.a(new_n92_), .b(new_n87_), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(x31), .c(new_n94_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n101_), .c(new_n77_), .O(z7));
  inv1   g060(.a(x32), .O(new_n105_));
  oai21  g061(.a(new_n84_), .b(new_n45_), .c(x31), .O(new_n106_));
  or2    g062(.a(new_n106_), .b(new_n91_), .O(new_n107_));
  inv1   g063(.a(x17), .O(new_n108_));
  nand3  g064(.a(new_n71_), .b(x18), .c(x14), .O(new_n109_));
  inv1   g065(.a(x18), .O(new_n110_));
  nand3  g066(.a(x19), .b(new_n110_), .c(x13), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  nand4  g068(.a(x19), .b(x18), .c(new_n108_), .d(x12), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  inv1   g071(.a(x16), .O(new_n116_));
  nand3  g072(.a(x18), .b(x17), .c(x11), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x19), .c(new_n116_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  nor2   g076(.a(new_n110_), .b(new_n108_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x16), .c(x10), .O(new_n122_));
  nor3   g078(.a(new_n122_), .b(new_n71_), .c(x15), .O(new_n123_));
  aoi21  g079(.a(new_n120_), .b(x15), .c(new_n123_), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n107_), .c(new_n105_), .O(new_n125_));
  inv1   g081(.a(x26), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(x00), .O(new_n127_));
  oai21  g083(.a(new_n118_), .b(x16), .c(x15), .O(new_n128_));
  inv1   g084(.a(x14), .O(new_n129_));
  nor2   g085(.a(x19), .b(new_n129_), .O(new_n130_));
  nand3  g086(.a(new_n121_), .b(x16), .c(x15), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n71_), .O(new_n132_));
  oai21  g088(.a(new_n131_), .b(new_n130_), .c(new_n132_), .O(new_n133_));
  aoi21  g089(.a(new_n128_), .b(new_n122_), .c(new_n133_), .O(new_n134_));
  nand2  g090(.a(x18), .b(x12), .O(new_n135_));
  oai21  g091(.a(x18), .b(x13), .c(x17), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(x32), .O(new_n138_));
  nor2   g094(.a(new_n138_), .b(new_n107_), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n134_), .c(new_n127_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n125_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n67_), .O(z8));
  oai21  g098(.a(x31), .b(new_n91_), .c(new_n106_), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n124_), .c(new_n53_), .O(new_n144_));
  aoi21  g100(.a(new_n85_), .b(x31), .c(new_n45_), .O(new_n145_));
  nand2  g101(.a(new_n137_), .b(x33), .O(new_n146_));
  nor3   g102(.a(new_n146_), .b(new_n145_), .c(new_n143_), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n134_), .c(new_n127_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n67_), .O(z9));
endmodule


