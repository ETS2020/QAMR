// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  nand2  g000(.a(x33), .b(x30), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  and2   g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(new_n52_), .O(new_n53_));
  inv1   g009(.a(x33), .O(new_n54_));
  aoi21  g010(.a(new_n53_), .b(new_n54_), .c(new_n46_), .O(new_n55_));
  oai21  g011(.a(new_n53_), .b(x32), .c(new_n55_), .O(z1));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n50_), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n49_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand4  g016(.a(x06), .b(x05), .c(new_n50_), .d(new_n49_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  aoi21  g018(.a(new_n52_), .b(x01), .c(new_n62_), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n63_), .b(new_n57_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n64_), .c(new_n45_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  nor2   g025(.a(x21), .b(x16), .O(new_n70_));
  nor2   g026(.a(x24), .b(x19), .O(new_n71_));
  inv1   g027(.a(x17), .O(new_n72_));
  inv1   g028(.a(x22), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(x20), .b(x15), .O(new_n75_));
  nor2   g031(.a(x23), .b(x18), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nor3   g034(.a(new_n78_), .b(new_n71_), .c(new_n70_), .O(new_n79_));
  inv1   g035(.a(x26), .O(new_n80_));
  nand4  g036(.a(new_n48_), .b(new_n80_), .c(x25), .d(x01), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n45_), .c(new_n69_), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(z3));
  inv1   g040(.a(x28), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n85_), .b(new_n69_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n87_), .c(new_n82_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n45_), .O(z4));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n87_), .b(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n86_), .b(x29), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n92_), .c(new_n82_), .d(new_n45_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z5));
  inv1   g051(.a(new_n93_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(x30), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(new_n98_));
  inv1   g054(.a(x30), .O(new_n99_));
  nor2   g055(.a(x33), .b(new_n99_), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n96_), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n82_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(z6));
  nand2  g060(.a(new_n97_), .b(x31), .O(new_n105_));
  inv1   g061(.a(x31), .O(new_n106_));
  aoi21  g062(.a(new_n98_), .b(new_n106_), .c(new_n46_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n105_), .c(new_n82_), .O(z7));
  nand4  g064(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  inv1   g066(.a(x15), .O(new_n111_));
  nand4  g067(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  oai21  g069(.a(new_n110_), .b(x19), .c(new_n113_), .O(new_n114_));
  inv1   g070(.a(x16), .O(new_n115_));
  nand3  g071(.a(x18), .b(x17), .c(x11), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  inv1   g073(.a(x19), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x14), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n110_), .O(new_n120_));
  nand2  g076(.a(x18), .b(x12), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n72_), .O(new_n122_));
  nor2   g078(.a(x18), .b(x13), .O(new_n123_));
  nor2   g079(.a(new_n123_), .b(new_n106_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n122_), .c(new_n120_), .d(new_n117_), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n114_), .c(new_n45_), .O(new_n126_));
  aoi21  g082(.a(new_n85_), .b(new_n69_), .c(new_n91_), .O(new_n127_));
  nor2   g083(.a(new_n127_), .b(x30), .O(new_n128_));
  aoi21  g084(.a(new_n127_), .b(new_n100_), .c(new_n128_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x32), .O(new_n131_));
  nand3  g087(.a(new_n118_), .b(x18), .c(x14), .O(new_n132_));
  inv1   g088(.a(x18), .O(new_n133_));
  nand3  g089(.a(x19), .b(new_n133_), .c(x13), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n132_), .c(new_n72_), .O(new_n135_));
  nand4  g091(.a(x19), .b(x18), .c(new_n72_), .d(x12), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n135_), .c(x16), .O(new_n138_));
  inv1   g094(.a(new_n116_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(x19), .c(new_n115_), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n138_), .c(new_n111_), .O(new_n141_));
  nor3   g097(.a(new_n112_), .b(new_n118_), .c(x15), .O(new_n142_));
  inv1   g098(.a(new_n127_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n100_), .O(new_n144_));
  nand2  g100(.a(new_n127_), .b(new_n99_), .O(new_n145_));
  inv1   g101(.a(x32), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x31), .O(new_n147_));
  aoi21  g103(.a(new_n145_), .b(new_n144_), .c(new_n147_), .O(new_n148_));
  oai21  g104(.a(new_n142_), .b(new_n141_), .c(new_n148_), .O(new_n149_));
  nand2  g105(.a(new_n80_), .b(x00), .O(new_n150_));
  aoi21  g106(.a(new_n149_), .b(new_n131_), .c(new_n150_), .O(z8));
  oai21  g107(.a(new_n142_), .b(new_n141_), .c(new_n145_), .O(new_n152_));
  nand2  g108(.a(new_n54_), .b(x31), .O(new_n153_));
  nand2  g109(.a(x33), .b(new_n106_), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n123_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n122_), .c(new_n120_), .d(new_n117_), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n114_), .c(new_n153_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n143_), .O(new_n158_));
  aoi21  g114(.a(new_n153_), .b(x30), .c(new_n150_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  aoi21  g116(.a(new_n152_), .b(new_n54_), .c(new_n160_), .O(z9));
endmodule


