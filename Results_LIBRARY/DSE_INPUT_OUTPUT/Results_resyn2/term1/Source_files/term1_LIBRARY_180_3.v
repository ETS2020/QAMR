// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_;
  inv1   g000(.a(x11), .O(new_n45_));
  inv1   g001(.a(x18), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  nand2  g009(.a(x32), .b(x03), .O(new_n54_));
  nand2  g010(.a(x33), .b(new_n49_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n54_), .c(new_n51_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n47_), .O(z1));
  inv1   g013(.a(x08), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  aoi21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  oai21  g016(.a(x03), .b(x02), .c(new_n60_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n51_), .O(new_n62_));
  aoi21  g018(.a(x05), .b(new_n49_), .c(new_n62_), .O(new_n63_));
  nand3  g019(.a(new_n62_), .b(x05), .c(new_n49_), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n63_), .c(new_n61_), .O(new_n66_));
  xor2a  g022(.a(x07), .b(x04), .O(new_n67_));
  xor2a  g023(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nor4   g024(.a(new_n68_), .b(new_n47_), .c(x09), .d(new_n58_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  inv1   g026(.a(new_n47_), .O(new_n71_));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  nor2   g028(.a(x24), .b(x19), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(x22), .b(x17), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(x20), .b(x15), .O(new_n77_));
  nor3   g033(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  inv1   g034(.a(x26), .O(new_n79_));
  nand3  g035(.a(new_n60_), .b(new_n79_), .c(x25), .O(new_n80_));
  aoi21  g036(.a(new_n78_), .b(new_n74_), .c(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n70_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(z3));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n70_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n70_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n88_), .c(new_n81_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n71_), .O(z4));
  nand2  g047(.a(new_n87_), .b(x29), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  nand2  g049(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n92_), .c(new_n83_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z5));
  inv1   g052(.a(x30), .O(new_n97_));
  nand2  g053(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n87_), .b(x30), .c(x29), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n98_), .c(new_n81_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n71_), .O(z6));
  xor2a  g057(.a(new_n99_), .b(x31), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n83_), .O(z7));
  aoi21  g059(.a(new_n86_), .b(new_n70_), .c(new_n93_), .O(new_n104_));
  nand2  g060(.a(x32), .b(x30), .O(new_n105_));
  inv1   g061(.a(x17), .O(new_n106_));
  nand3  g062(.a(x19), .b(new_n46_), .c(x13), .O(new_n107_));
  inv1   g063(.a(x19), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x18), .c(x14), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  nand4  g066(.a(x19), .b(x18), .c(new_n106_), .d(x12), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n110_), .c(x15), .O(new_n113_));
  inv1   g069(.a(x15), .O(new_n114_));
  nand4  g070(.a(x19), .b(x17), .c(new_n114_), .d(x10), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x18), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  inv1   g074(.a(x31), .O(new_n119_));
  nor2   g075(.a(x32), .b(new_n119_), .O(new_n120_));
  inv1   g076(.a(x16), .O(new_n121_));
  nor2   g077(.a(x30), .b(new_n121_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n120_), .c(new_n118_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n105_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n104_), .O(new_n125_));
  nand2  g081(.a(new_n89_), .b(x29), .O(new_n126_));
  nand2  g082(.a(x32), .b(new_n97_), .O(new_n127_));
  nor2   g083(.a(new_n97_), .b(new_n121_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n120_), .c(new_n118_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  nand3  g087(.a(x18), .b(x17), .c(x15), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  nand2  g089(.a(x17), .b(x10), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n46_), .c(new_n114_), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(x19), .c(new_n133_), .O(new_n136_));
  inv1   g092(.a(x13), .O(new_n137_));
  nor2   g093(.a(x17), .b(x12), .O(new_n138_));
  oai22  g094(.a(new_n138_), .b(new_n46_), .c(new_n106_), .d(new_n137_), .O(new_n139_));
  nand2  g095(.a(new_n108_), .b(x14), .O(new_n140_));
  nand2  g096(.a(new_n133_), .b(new_n140_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n139_), .c(x31), .d(x16), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n136_), .c(x32), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n131_), .c(new_n125_), .O(new_n144_));
  nand2  g100(.a(new_n79_), .b(x00), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n71_), .O(z8));
  oai21  g104(.a(new_n119_), .b(new_n97_), .c(new_n104_), .O(new_n149_));
  oai21  g105(.a(x31), .b(x30), .c(new_n126_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  nand2  g108(.a(x17), .b(x15), .O(new_n153_));
  oai21  g109(.a(new_n138_), .b(new_n108_), .c(new_n153_), .O(new_n154_));
  aoi21  g110(.a(new_n134_), .b(new_n114_), .c(new_n121_), .O(new_n155_));
  nand3  g111(.a(new_n155_), .b(new_n154_), .c(new_n141_), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(new_n152_), .c(x11), .O(new_n158_));
  nor4   g114(.a(new_n153_), .b(new_n108_), .c(new_n121_), .d(new_n137_), .O(new_n159_));
  aoi21  g115(.a(new_n159_), .b(new_n152_), .c(x18), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n158_), .c(x33), .O(new_n161_));
  nor2   g117(.a(new_n153_), .b(new_n107_), .O(new_n162_));
  nand3  g118(.a(x19), .b(new_n106_), .c(x12), .O(new_n163_));
  oai21  g119(.a(new_n140_), .b(new_n106_), .c(new_n163_), .O(new_n164_));
  aoi21  g120(.a(new_n164_), .b(x15), .c(new_n116_), .O(new_n165_));
  nor3   g121(.a(new_n165_), .b(new_n46_), .c(x11), .O(new_n166_));
  nand3  g122(.a(new_n152_), .b(new_n50_), .c(x16), .O(new_n167_));
  inv1   g123(.a(new_n167_), .O(new_n168_));
  oai21  g124(.a(new_n166_), .b(new_n162_), .c(new_n168_), .O(new_n169_));
  aoi21  g125(.a(new_n169_), .b(new_n161_), .c(new_n145_), .O(z9));
endmodule


