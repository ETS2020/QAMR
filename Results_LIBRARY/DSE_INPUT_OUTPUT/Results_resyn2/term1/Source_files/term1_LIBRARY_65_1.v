// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_;
  inv1   g000(.a(x26), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x15), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  inv1   g008(.a(x32), .O(new_n53_));
  aoi21  g009(.a(x33), .b(new_n48_), .c(x02), .O(new_n54_));
  oai21  g010(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n46_), .O(z1));
  inv1   g012(.a(new_n46_), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  aoi21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  oai21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n50_), .O(new_n61_));
  aoi21  g017(.a(x05), .b(new_n48_), .c(new_n61_), .O(new_n62_));
  nand3  g018(.a(new_n61_), .b(x05), .c(new_n48_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  xnor2a g021(.a(x07), .b(x04), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(x08), .O(new_n68_));
  aoi21  g024(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  oai21  g025(.a(new_n66_), .b(new_n65_), .c(new_n69_), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n57_), .O(z2));
  inv1   g027(.a(x27), .O(new_n72_));
  nor2   g028(.a(x23), .b(x18), .O(new_n73_));
  nor2   g029(.a(x22), .b(x17), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x24), .b(x19), .O(new_n76_));
  nor2   g032(.a(x21), .b(x16), .O(new_n77_));
  nor2   g033(.a(x20), .b(x15), .O(new_n78_));
  nor3   g034(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(new_n79_));
  nand3  g035(.a(new_n59_), .b(new_n45_), .c(x25), .O(new_n80_));
  aoi21  g036(.a(new_n79_), .b(new_n75_), .c(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n72_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(z3));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n72_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n72_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n86_), .c(new_n81_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z4));
  nand2  g045(.a(new_n85_), .b(x29), .O(new_n90_));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n90_), .c(new_n81_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(z5));
  inv1   g050(.a(x30), .O(new_n95_));
  xor2a  g051(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n81_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(z6));
  nand2  g054(.a(new_n90_), .b(x31), .O(new_n99_));
  nor2   g055(.a(x31), .b(x30), .O(new_n100_));
  and2   g056(.a(x31), .b(x30), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n85_), .c(x29), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n99_), .c(new_n81_), .O(z7));
  inv1   g060(.a(x15), .O(new_n105_));
  inv1   g061(.a(x17), .O(new_n106_));
  inv1   g062(.a(x19), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x18), .c(x14), .O(new_n108_));
  inv1   g064(.a(x18), .O(new_n109_));
  nand3  g065(.a(x19), .b(new_n109_), .c(x13), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(new_n111_));
  nand4  g067(.a(x19), .b(x18), .c(new_n106_), .d(x12), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(x16), .O(new_n114_));
  inv1   g070(.a(x16), .O(new_n115_));
  nand3  g071(.a(x18), .b(x17), .c(x11), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x19), .c(new_n115_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n114_), .c(new_n53_), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n45_), .c(new_n105_), .O(new_n120_));
  inv1   g076(.a(x00), .O(new_n121_));
  nand3  g077(.a(x18), .b(x17), .c(x16), .O(new_n122_));
  nand2  g078(.a(x19), .b(x10), .O(new_n123_));
  nor2   g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  aoi21  g080(.a(new_n84_), .b(new_n72_), .c(new_n91_), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(x30), .c(x31), .O(new_n126_));
  aoi21  g082(.a(new_n125_), .b(x30), .c(new_n126_), .O(new_n127_));
  oai21  g083(.a(new_n124_), .b(x15), .c(new_n127_), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(new_n53_), .c(new_n121_), .O(new_n129_));
  nand2  g085(.a(x17), .b(x13), .O(new_n130_));
  oai21  g086(.a(x17), .b(x12), .c(x18), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n130_), .c(new_n53_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n127_), .O(new_n133_));
  inv1   g089(.a(new_n122_), .O(new_n134_));
  aoi21  g090(.a(new_n107_), .b(x14), .c(new_n105_), .O(new_n135_));
  oai22  g091(.a(new_n134_), .b(x19), .c(new_n117_), .d(x16), .O(new_n136_));
  aoi21  g092(.a(new_n135_), .b(new_n134_), .c(new_n136_), .O(new_n137_));
  inv1   g093(.a(new_n124_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(x26), .c(new_n105_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n137_), .O(new_n140_));
  oai22  g096(.a(new_n140_), .b(new_n133_), .c(new_n129_), .d(x26), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n120_), .O(z8));
  nand2  g098(.a(new_n118_), .b(new_n114_), .O(new_n143_));
  nand2  g099(.a(new_n101_), .b(x15), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n143_), .c(x33), .O(new_n146_));
  nand2  g102(.a(x33), .b(x30), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n45_), .O(new_n148_));
  nand4  g104(.a(new_n124_), .b(new_n101_), .c(new_n49_), .d(new_n105_), .O(new_n149_));
  oai21  g105(.a(new_n148_), .b(new_n146_), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n125_), .O(new_n151_));
  inv1   g107(.a(new_n125_), .O(new_n152_));
  nand3  g108(.a(new_n124_), .b(new_n100_), .c(new_n49_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n147_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n105_), .O(new_n155_));
  inv1   g111(.a(new_n147_), .O(new_n156_));
  nand3  g112(.a(new_n100_), .b(new_n49_), .c(x15), .O(new_n157_));
  inv1   g113(.a(new_n157_), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n143_), .c(new_n156_), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(x26), .c(new_n155_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n152_), .O(new_n161_));
  oai21  g117(.a(new_n138_), .b(new_n102_), .c(new_n105_), .O(new_n162_));
  aoi21  g118(.a(new_n131_), .b(new_n130_), .c(new_n102_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n137_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n45_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n162_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(x33), .O(new_n167_));
  nand3  g123(.a(new_n167_), .b(new_n161_), .c(new_n151_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(x00), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n57_), .O(z9));
endmodule


