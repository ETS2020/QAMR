// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_;
  nand2  g000(.a(x30), .b(x19), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(new_n45_), .O(new_n47_));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  nand2  g008(.a(x32), .b(x03), .O(new_n53_));
  nand2  g009(.a(x33), .b(new_n48_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n47_), .O(z1));
  inv1   g012(.a(x01), .O(new_n57_));
  aoi21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  oai21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n48_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x06), .c(new_n50_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n50_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x05), .c(new_n48_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  xnor2a g021(.a(x07), .b(x04), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(x08), .O(new_n68_));
  aoi21  g024(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  oai21  g025(.a(new_n66_), .b(new_n65_), .c(new_n69_), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n45_), .O(z2));
  inv1   g027(.a(x27), .O(new_n72_));
  nor2   g028(.a(x20), .b(x15), .O(new_n73_));
  nor2   g029(.a(x21), .b(x16), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x22), .b(x17), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  oai21  g036(.a(x24), .b(x19), .c(new_n80_), .O(new_n81_));
  inv1   g037(.a(x26), .O(new_n82_));
  nand3  g038(.a(new_n58_), .b(new_n82_), .c(x25), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n81_), .c(new_n45_), .d(new_n72_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(z3));
  nand2  g042(.a(new_n84_), .b(new_n81_), .O(new_n87_));
  inv1   g043(.a(x28), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n72_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n88_), .b(new_n72_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n87_), .c(new_n45_), .O(z4));
  nand2  g049(.a(new_n89_), .b(x29), .O(new_n94_));
  inv1   g050(.a(x29), .O(new_n95_));
  nand2  g051(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n87_), .c(new_n45_), .O(z5));
  inv1   g054(.a(x24), .O(new_n99_));
  nor2   g055(.a(new_n79_), .b(new_n99_), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n83_), .O(new_n101_));
  inv1   g057(.a(x30), .O(new_n102_));
  xor2a  g058(.a(new_n94_), .b(new_n102_), .O(new_n103_));
  oai21  g059(.a(new_n80_), .b(x30), .c(x19), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(z6));
  inv1   g062(.a(x19), .O(new_n107_));
  inv1   g063(.a(x31), .O(new_n108_));
  oai21  g064(.a(new_n79_), .b(new_n107_), .c(new_n108_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n102_), .O(new_n110_));
  oai21  g066(.a(new_n100_), .b(new_n83_), .c(new_n45_), .O(new_n111_));
  nand2  g067(.a(new_n108_), .b(x30), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  aoi21  g069(.a(new_n94_), .b(new_n108_), .c(x19), .O(new_n114_));
  oai21  g070(.a(new_n113_), .b(new_n94_), .c(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n111_), .c(new_n110_), .O(z7));
  aoi21  g072(.a(new_n88_), .b(new_n72_), .c(new_n95_), .O(new_n117_));
  nand2  g073(.a(x32), .b(x30), .O(new_n118_));
  nand2  g074(.a(new_n102_), .b(x18), .O(new_n119_));
  nor2   g075(.a(x32), .b(new_n108_), .O(new_n120_));
  nand4  g076(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n119_), .c(new_n118_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n107_), .O(new_n125_));
  nand2  g081(.a(x17), .b(x13), .O(new_n126_));
  oai21  g082(.a(x17), .b(x12), .c(x18), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g084(.a(x18), .b(x17), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n128_), .c(x16), .O(new_n130_));
  inv1   g086(.a(x15), .O(new_n131_));
  inv1   g087(.a(x16), .O(new_n132_));
  nand3  g088(.a(x18), .b(x17), .c(x11), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n132_), .c(new_n131_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n130_), .O(new_n136_));
  nand2  g092(.a(x16), .b(x10), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n129_), .c(new_n131_), .O(new_n138_));
  nand3  g094(.a(new_n120_), .b(new_n102_), .c(x19), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n138_), .c(new_n136_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n125_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n117_), .O(new_n143_));
  nand2  g099(.a(x32), .b(new_n102_), .O(new_n144_));
  inv1   g100(.a(x18), .O(new_n145_));
  nor2   g101(.a(x19), .b(new_n145_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n122_), .c(new_n120_), .d(x30), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n144_), .c(new_n117_), .O(new_n148_));
  and2   g104(.a(x16), .b(x15), .O(new_n149_));
  and2   g105(.a(x18), .b(x17), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n149_), .c(x19), .O(new_n151_));
  nand2  g107(.a(new_n133_), .b(new_n132_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n151_), .c(new_n138_), .d(new_n128_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n102_), .O(new_n154_));
  nand4  g110(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n102_), .c(x14), .O(new_n156_));
  nand2  g112(.a(new_n155_), .b(new_n107_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x31), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n156_), .c(new_n45_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n154_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(x32), .c(new_n148_), .O(new_n161_));
  nand2  g117(.a(new_n82_), .b(x00), .O(new_n162_));
  aoi21  g118(.a(new_n161_), .b(new_n143_), .c(new_n162_), .O(z8));
  oai21  g119(.a(new_n153_), .b(x31), .c(new_n102_), .O(new_n164_));
  inv1   g120(.a(new_n155_), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(new_n112_), .c(x19), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n156_), .c(new_n45_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n164_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(x33), .O(new_n169_));
  nand4  g125(.a(new_n49_), .b(new_n108_), .c(new_n102_), .d(x19), .O(new_n170_));
  inv1   g126(.a(new_n170_), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n138_), .c(new_n136_), .O(new_n172_));
  nand2  g128(.a(x33), .b(x30), .O(new_n173_));
  nand4  g129(.a(new_n49_), .b(new_n108_), .c(new_n102_), .d(x18), .O(new_n174_));
  oai21  g130(.a(new_n174_), .b(new_n121_), .c(new_n173_), .O(new_n175_));
  aoi21  g131(.a(new_n175_), .b(new_n107_), .c(new_n117_), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n172_), .O(new_n177_));
  nand3  g133(.a(new_n146_), .b(x31), .c(x30), .O(new_n178_));
  oai21  g134(.a(new_n178_), .b(new_n121_), .c(new_n49_), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n173_), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n117_), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(new_n177_), .O(new_n182_));
  aoi21  g138(.a(new_n182_), .b(new_n169_), .c(new_n162_), .O(z9));
endmodule


