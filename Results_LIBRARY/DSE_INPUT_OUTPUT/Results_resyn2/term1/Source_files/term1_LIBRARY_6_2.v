// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_;
  inv1   g00(.a(x18), .O(new_n45_));
  nor2   g01(.a(x26), .b(new_n45_), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(x32), .O(z0));
  inv1   g03(.a(x03), .O(new_n48_));
  inv1   g04(.a(x33), .O(new_n49_));
  inv1   g05(.a(x02), .O(new_n50_));
  aoi21  g06(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g07(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  inv1   g08(.a(x32), .O(new_n53_));
  aoi21  g09(.a(x33), .b(new_n48_), .c(x02), .O(new_n54_));
  oai21  g10(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  aoi21  g11(.a(new_n55_), .b(new_n52_), .c(new_n46_), .O(z1));
  inv1   g12(.a(new_n46_), .O(new_n57_));
  xor2a  g13(.a(x07), .b(x04), .O(new_n58_));
  inv1   g14(.a(x01), .O(new_n59_));
  nand2  g15(.a(x05), .b(new_n48_), .O(new_n60_));
  nand3  g16(.a(new_n60_), .b(x06), .c(new_n50_), .O(new_n61_));
  nand2  g17(.a(x06), .b(new_n50_), .O(new_n62_));
  nand3  g18(.a(new_n62_), .b(x05), .c(new_n48_), .O(new_n63_));
  nand3  g19(.a(new_n63_), .b(new_n61_), .c(new_n59_), .O(new_n64_));
  aoi21  g20(.a(new_n48_), .b(new_n50_), .c(new_n59_), .O(new_n65_));
  oai21  g21(.a(new_n48_), .b(new_n50_), .c(new_n65_), .O(new_n66_));
  aoi21  g22(.a(new_n66_), .b(new_n64_), .c(new_n58_), .O(new_n67_));
  inv1   g23(.a(x09), .O(new_n68_));
  nand3  g24(.a(new_n66_), .b(new_n64_), .c(new_n58_), .O(new_n69_));
  nand3  g25(.a(new_n69_), .b(new_n68_), .c(x08), .O(new_n70_));
  oai21  g26(.a(new_n70_), .b(new_n67_), .c(new_n57_), .O(z2));
  inv1   g27(.a(x27), .O(new_n72_));
  inv1   g28(.a(x23), .O(new_n73_));
  nor2   g29(.a(x21), .b(x16), .O(new_n74_));
  nor2   g30(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g31(.a(x22), .b(x17), .O(new_n76_));
  inv1   g32(.a(x19), .O(new_n77_));
  inv1   g33(.a(x24), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  or2    g35(.a(x20), .b(x15), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n76_), .O(new_n82_));
  nand2  g38(.a(x03), .b(x02), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(x25), .c(x01), .O(new_n84_));
  aoi21  g40(.a(new_n82_), .b(new_n75_), .c(new_n84_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n72_), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n45_), .c(x26), .O(z3));
  inv1   g43(.a(x28), .O(new_n88_));
  nor2   g44(.a(new_n88_), .b(new_n72_), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  nand2  g46(.a(new_n88_), .b(new_n72_), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(new_n90_), .c(new_n85_), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n45_), .c(x26), .O(z4));
  nor2   g49(.a(x26), .b(x18), .O(new_n94_));
  nand2  g50(.a(new_n89_), .b(x29), .O(new_n95_));
  inv1   g51(.a(new_n95_), .O(new_n96_));
  nor2   g52(.a(new_n89_), .b(x29), .O(new_n97_));
  nor2   g53(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g54(.a(new_n98_), .b(new_n94_), .c(new_n85_), .O(new_n99_));
  inv1   g55(.a(new_n99_), .O(z5));
  nand2  g56(.a(new_n96_), .b(x30), .O(new_n101_));
  inv1   g57(.a(x30), .O(new_n102_));
  nand2  g58(.a(new_n95_), .b(new_n102_), .O(new_n103_));
  nand4  g59(.a(new_n103_), .b(new_n101_), .c(new_n94_), .d(new_n85_), .O(new_n104_));
  inv1   g60(.a(new_n104_), .O(z6));
  xor2a  g61(.a(new_n101_), .b(x31), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n94_), .c(new_n85_), .O(z7));
  oai21  g63(.a(x28), .b(x27), .c(x29), .O(new_n108_));
  nand2  g64(.a(x32), .b(x30), .O(new_n109_));
  nand4  g65(.a(x17), .b(x16), .c(x15), .d(x13), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  nor2   g67(.a(x30), .b(new_n77_), .O(new_n112_));
  nand4  g68(.a(new_n112_), .b(new_n111_), .c(new_n53_), .d(x31), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n109_), .c(new_n108_), .O(new_n114_));
  nand2  g70(.a(x32), .b(new_n102_), .O(new_n115_));
  nand4  g71(.a(new_n53_), .b(x31), .c(x30), .d(x19), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n110_), .c(new_n115_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n108_), .O(new_n118_));
  nand3  g74(.a(x16), .b(x15), .c(x13), .O(new_n119_));
  nand3  g75(.a(x31), .b(x19), .c(x17), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n119_), .c(x32), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(new_n114_), .c(x00), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n45_), .c(x26), .O(z8));
  inv1   g80(.a(x31), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n102_), .O(new_n126_));
  oai21  g82(.a(new_n108_), .b(new_n125_), .c(new_n126_), .O(new_n127_));
  nand2  g83(.a(x19), .b(x17), .O(new_n128_));
  nor2   g84(.a(new_n128_), .b(new_n119_), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(new_n127_), .c(new_n49_), .O(new_n130_));
  inv1   g86(.a(new_n108_), .O(new_n131_));
  nand2  g87(.a(x33), .b(new_n102_), .O(new_n132_));
  nand4  g88(.a(new_n49_), .b(x31), .c(x30), .d(x19), .O(new_n133_));
  oai21  g89(.a(new_n133_), .b(new_n110_), .c(new_n132_), .O(new_n134_));
  nand2  g90(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nor2   g91(.a(x33), .b(x31), .O(new_n136_));
  nand4  g92(.a(new_n136_), .b(new_n112_), .c(new_n111_), .d(new_n108_), .O(new_n137_));
  nand2  g93(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  oai21  g94(.a(new_n138_), .b(new_n130_), .c(x00), .O(new_n139_));
  aoi21  g95(.a(new_n139_), .b(new_n45_), .c(x26), .O(z9));
endmodule


