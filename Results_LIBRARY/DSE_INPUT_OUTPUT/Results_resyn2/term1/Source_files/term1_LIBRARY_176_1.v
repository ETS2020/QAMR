// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x12), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x33), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(x32), .b(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(x33), .b(new_n49_), .c(x02), .O(new_n53_));
  oai21  g009(.a(new_n52_), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g011(.a(new_n46_), .O(new_n56_));
  inv1   g012(.a(x01), .O(new_n57_));
  aoi21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  oai21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  inv1   g015(.a(x02), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n60_), .O(new_n61_));
  aoi21  g017(.a(x05), .b(new_n49_), .c(new_n61_), .O(new_n62_));
  nand3  g018(.a(new_n61_), .b(x05), .c(new_n49_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n62_), .c(new_n59_), .O(new_n65_));
  xnor2a g021(.a(x07), .b(x04), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(x08), .O(new_n68_));
  aoi21  g024(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  oai21  g025(.a(new_n66_), .b(new_n65_), .c(new_n69_), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n56_), .O(z2));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  nor2   g028(.a(x22), .b(x17), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g030(.a(x19), .O(new_n75_));
  inv1   g031(.a(x24), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g033(.a(x21), .b(x16), .O(new_n78_));
  nor2   g034(.a(x20), .b(x15), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(new_n81_));
  inv1   g037(.a(x26), .O(new_n82_));
  nand3  g038(.a(new_n58_), .b(new_n82_), .c(x25), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(x27), .c(new_n56_), .O(z3));
  inv1   g042(.a(x27), .O(new_n87_));
  inv1   g043(.a(x28), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n88_), .b(new_n87_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n85_), .c(new_n56_), .O(z4));
  nand2  g049(.a(new_n89_), .b(x29), .O(new_n94_));
  inv1   g050(.a(x29), .O(new_n95_));
  nand2  g051(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n85_), .c(new_n56_), .O(z5));
  nand2  g054(.a(new_n94_), .b(new_n45_), .O(new_n99_));
  inv1   g055(.a(new_n94_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(x30), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n85_), .c(new_n56_), .O(z6));
  inv1   g059(.a(new_n85_), .O(new_n104_));
  nand2  g060(.a(new_n101_), .b(x31), .O(new_n105_));
  inv1   g061(.a(x31), .O(new_n106_));
  nand3  g062(.a(new_n100_), .b(new_n106_), .c(x30), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n105_), .c(new_n104_), .d(new_n56_), .O(z7));
  nand2  g064(.a(new_n82_), .b(x00), .O(new_n109_));
  inv1   g065(.a(x12), .O(new_n110_));
  nand2  g066(.a(new_n91_), .b(x29), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nor2   g068(.a(new_n112_), .b(new_n45_), .O(new_n113_));
  inv1   g069(.a(x15), .O(new_n114_));
  nand3  g070(.a(new_n75_), .b(x18), .c(x14), .O(new_n115_));
  inv1   g071(.a(x18), .O(new_n116_));
  nand3  g072(.a(x19), .b(new_n116_), .c(x13), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n115_), .c(x17), .O(new_n118_));
  inv1   g074(.a(x16), .O(new_n119_));
  inv1   g075(.a(x17), .O(new_n120_));
  nand2  g076(.a(x19), .b(x18), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n118_), .O(new_n123_));
  nand3  g079(.a(x19), .b(x18), .c(x17), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n119_), .c(x11), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n123_), .c(new_n114_), .O(new_n127_));
  nand3  g083(.a(x16), .b(new_n114_), .c(x10), .O(new_n128_));
  nor2   g084(.a(new_n128_), .b(new_n124_), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n127_), .c(new_n113_), .O(new_n130_));
  nand3  g086(.a(x18), .b(x16), .c(x15), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n45_), .c(x19), .d(new_n120_), .O(new_n133_));
  or2    g089(.a(new_n133_), .b(new_n111_), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n130_), .c(new_n110_), .O(new_n135_));
  aoi21  g091(.a(new_n117_), .b(new_n115_), .c(new_n119_), .O(new_n136_));
  inv1   g092(.a(new_n121_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n119_), .c(x11), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n136_), .c(x15), .O(new_n140_));
  inv1   g096(.a(new_n128_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n137_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nor2   g099(.a(x30), .b(new_n120_), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n143_), .c(new_n112_), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  nor2   g102(.a(x32), .b(new_n106_), .O(new_n147_));
  oai21  g103(.a(new_n146_), .b(new_n135_), .c(new_n147_), .O(new_n148_));
  xor2a  g104(.a(new_n131_), .b(new_n120_), .O(new_n149_));
  nand3  g105(.a(new_n75_), .b(x17), .c(x14), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  or2    g107(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g108(.a(new_n149_), .b(new_n75_), .O(new_n153_));
  nand3  g109(.a(x18), .b(x16), .c(x10), .O(new_n154_));
  oai21  g110(.a(x18), .b(x13), .c(x16), .O(new_n155_));
  nand2  g111(.a(x18), .b(x11), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x15), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n154_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n153_), .c(new_n152_), .O(new_n160_));
  aoi21  g116(.a(new_n120_), .b(new_n110_), .c(x30), .O(new_n161_));
  inv1   g117(.a(new_n161_), .O(new_n162_));
  nor2   g118(.a(new_n162_), .b(new_n111_), .O(new_n163_));
  oai21  g119(.a(new_n163_), .b(new_n113_), .c(x31), .O(new_n164_));
  nor2   g120(.a(new_n46_), .b(new_n52_), .O(new_n165_));
  oai21  g121(.a(new_n164_), .b(new_n160_), .c(new_n165_), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(new_n148_), .c(new_n109_), .O(z8));
  nor3   g123(.a(new_n111_), .b(new_n106_), .c(new_n45_), .O(new_n168_));
  oai21  g124(.a(new_n129_), .b(new_n127_), .c(new_n168_), .O(new_n169_));
  nor2   g125(.a(new_n112_), .b(x31), .O(new_n170_));
  inv1   g126(.a(new_n170_), .O(new_n171_));
  oai21  g127(.a(new_n171_), .b(new_n133_), .c(new_n169_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(x12), .O(new_n173_));
  and2   g129(.a(new_n144_), .b(new_n143_), .O(new_n174_));
  aoi21  g130(.a(new_n170_), .b(new_n174_), .c(x33), .O(new_n175_));
  inv1   g131(.a(new_n109_), .O(new_n176_));
  aoi21  g132(.a(new_n170_), .b(new_n161_), .c(new_n168_), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n160_), .c(new_n56_), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(x33), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n176_), .O(new_n180_));
  aoi21  g136(.a(new_n175_), .b(new_n173_), .c(new_n180_), .O(z9));
endmodule


