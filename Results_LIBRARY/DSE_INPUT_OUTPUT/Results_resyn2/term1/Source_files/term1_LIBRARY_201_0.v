// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x29), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(new_n47_), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  inv1   g006(.a(x33), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  aoi21  g008(.a(x32), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(new_n50_), .c(new_n53_), .O(new_n54_));
  inv1   g010(.a(x32), .O(new_n55_));
  aoi21  g011(.a(x33), .b(new_n50_), .c(x02), .O(new_n56_));
  oai21  g012(.a(new_n55_), .b(new_n50_), .c(new_n56_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n54_), .c(new_n49_), .O(z1));
  inv1   g014(.a(x01), .O(new_n59_));
  aoi21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  oai21  g016(.a(x03), .b(x02), .c(new_n60_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n52_), .O(new_n62_));
  aoi21  g018(.a(x05), .b(new_n50_), .c(new_n62_), .O(new_n63_));
  nand3  g019(.a(new_n62_), .b(x05), .c(new_n50_), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n63_), .c(new_n61_), .O(new_n66_));
  xor2a  g022(.a(x07), .b(x04), .O(new_n67_));
  xor2a  g023(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  inv1   g024(.a(x09), .O(new_n69_));
  nand3  g025(.a(new_n47_), .b(new_n69_), .c(x08), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n68_), .O(z2));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  nor2   g028(.a(x22), .b(x17), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g030(.a(x19), .O(new_n75_));
  inv1   g031(.a(x24), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g033(.a(x21), .b(x16), .O(new_n78_));
  nor2   g034(.a(x20), .b(x15), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  and2   g036(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  inv1   g037(.a(x26), .O(new_n82_));
  nand3  g038(.a(new_n60_), .b(new_n82_), .c(x25), .O(new_n83_));
  aoi21  g039(.a(new_n81_), .b(new_n74_), .c(new_n83_), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n49_), .O(new_n85_));
  aoi21  g041(.a(new_n47_), .b(x27), .c(new_n85_), .O(z3));
  inv1   g042(.a(x27), .O(new_n87_));
  inv1   g043(.a(x28), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nor2   g046(.a(x28), .b(x27), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n90_), .c(new_n84_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n47_), .O(z4));
  nand2  g050(.a(new_n89_), .b(x29), .O(new_n95_));
  nand2  g051(.a(new_n89_), .b(x30), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n45_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n95_), .c(new_n84_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z5));
  nand2  g055(.a(new_n90_), .b(new_n46_), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(new_n96_), .c(new_n45_), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n85_), .O(z6));
  inv1   g058(.a(x31), .O(new_n103_));
  nand4  g059(.a(new_n89_), .b(new_n103_), .c(x30), .d(x29), .O(new_n104_));
  oai22  g060(.a(new_n96_), .b(new_n45_), .c(new_n49_), .d(x31), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n104_), .c(new_n84_), .O(z7));
  nand4  g062(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  inv1   g064(.a(x11), .O(new_n109_));
  inv1   g065(.a(x16), .O(new_n110_));
  nand2  g066(.a(x18), .b(x17), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n109_), .c(new_n110_), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(x19), .c(new_n108_), .O(new_n113_));
  nand2  g069(.a(x17), .b(x13), .O(new_n114_));
  oai21  g070(.a(x17), .b(x12), .c(x18), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g072(.a(new_n75_), .b(x14), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n108_), .O(new_n118_));
  inv1   g074(.a(x15), .O(new_n119_));
  nand2  g075(.a(x16), .b(x10), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n111_), .c(new_n119_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n118_), .c(new_n116_), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n113_), .c(new_n47_), .O(new_n123_));
  nand2  g079(.a(new_n92_), .b(x30), .O(new_n124_));
  nand2  g080(.a(new_n91_), .b(new_n46_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  aoi22  g082(.a(new_n126_), .b(x29), .c(new_n47_), .d(new_n103_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x32), .O(new_n129_));
  inv1   g085(.a(x17), .O(new_n130_));
  nand3  g086(.a(new_n75_), .b(x18), .c(x14), .O(new_n131_));
  inv1   g087(.a(x18), .O(new_n132_));
  nand3  g088(.a(x19), .b(new_n132_), .c(x13), .O(new_n133_));
  aoi21  g089(.a(new_n133_), .b(new_n131_), .c(new_n130_), .O(new_n134_));
  nand4  g090(.a(x19), .b(x18), .c(new_n130_), .d(x12), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n134_), .c(x16), .O(new_n137_));
  nand3  g093(.a(x19), .b(x18), .c(x17), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n110_), .c(x11), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n137_), .c(new_n119_), .O(new_n141_));
  nor3   g097(.a(new_n138_), .b(new_n120_), .c(x15), .O(new_n142_));
  oai21  g098(.a(new_n91_), .b(new_n45_), .c(x30), .O(new_n143_));
  nand3  g099(.a(new_n92_), .b(new_n46_), .c(x29), .O(new_n144_));
  nand2  g100(.a(new_n55_), .b(x31), .O(new_n145_));
  aoi21  g101(.a(new_n144_), .b(new_n143_), .c(new_n145_), .O(new_n146_));
  oai21  g102(.a(new_n142_), .b(new_n141_), .c(new_n146_), .O(new_n147_));
  nand2  g103(.a(new_n82_), .b(x00), .O(new_n148_));
  aoi21  g104(.a(new_n147_), .b(new_n129_), .c(new_n148_), .O(z8));
  nand2  g105(.a(new_n91_), .b(new_n103_), .O(new_n150_));
  nor2   g106(.a(x30), .b(new_n45_), .O(new_n151_));
  nand2  g107(.a(new_n103_), .b(x30), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n143_), .O(new_n153_));
  aoi21  g109(.a(new_n151_), .b(new_n150_), .c(new_n153_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n123_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x33), .O(new_n156_));
  nand3  g112(.a(new_n152_), .b(new_n51_), .c(x29), .O(new_n157_));
  aoi21  g113(.a(new_n150_), .b(new_n124_), .c(new_n157_), .O(new_n158_));
  oai21  g114(.a(new_n142_), .b(new_n141_), .c(new_n158_), .O(new_n159_));
  aoi21  g115(.a(new_n159_), .b(new_n156_), .c(new_n148_), .O(z9));
endmodule


