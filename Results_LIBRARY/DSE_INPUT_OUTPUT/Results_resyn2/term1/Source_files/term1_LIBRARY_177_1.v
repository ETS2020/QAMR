// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x15), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  aoi21  g007(.a(new_n50_), .b(new_n51_), .c(new_n46_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(x33), .c(new_n52_), .O(z1));
  inv1   g009(.a(x01), .O(new_n54_));
  aoi21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  oai21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  aoi21  g015(.a(x05), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g016(.a(new_n59_), .b(x05), .c(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  oai21  g018(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  xor2a  g019(.a(x07), .b(x04), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand3  g022(.a(new_n47_), .b(new_n66_), .c(x08), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n65_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  inv1   g025(.a(x23), .O(new_n70_));
  nor2   g026(.a(x20), .b(x15), .O(new_n71_));
  oai21  g027(.a(new_n71_), .b(new_n70_), .c(new_n45_), .O(new_n72_));
  nor2   g028(.a(x24), .b(x19), .O(new_n73_));
  oai22  g029(.a(x22), .b(x17), .c(x21), .d(x16), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g031(.a(x26), .O(new_n76_));
  nand3  g032(.a(new_n55_), .b(new_n76_), .c(x25), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n47_), .O(new_n79_));
  aoi21  g035(.a(new_n75_), .b(new_n72_), .c(new_n79_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n69_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(z3));
  aoi21  g038(.a(new_n70_), .b(new_n45_), .c(new_n71_), .O(new_n83_));
  aoi21  g039(.a(new_n83_), .b(new_n75_), .c(new_n77_), .O(new_n84_));
  inv1   g040(.a(x28), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n85_), .b(new_n69_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n87_), .c(new_n84_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n47_), .O(z4));
  nand2  g046(.a(new_n86_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n87_), .b(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n91_), .c(new_n84_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n47_), .O(z5));
  nand3  g051(.a(new_n86_), .b(x30), .c(x29), .O(new_n96_));
  inv1   g052(.a(x30), .O(new_n97_));
  nand2  g053(.a(new_n91_), .b(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n96_), .c(new_n80_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z6));
  xor2a  g056(.a(new_n96_), .b(x31), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n80_), .O(z7));
  inv1   g058(.a(x15), .O(new_n103_));
  nand2  g059(.a(new_n76_), .b(x00), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n51_), .c(new_n45_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand3  g062(.a(new_n88_), .b(x30), .c(x29), .O(new_n107_));
  inv1   g063(.a(x31), .O(new_n108_));
  oai21  g064(.a(x28), .b(x27), .c(x29), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(new_n97_), .c(new_n108_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n107_), .O(new_n111_));
  inv1   g067(.a(x17), .O(new_n112_));
  inv1   g068(.a(x19), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x18), .c(x14), .O(new_n114_));
  nand4  g070(.a(x19), .b(new_n45_), .c(x15), .d(x13), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n114_), .c(new_n112_), .O(new_n116_));
  nand4  g072(.a(x19), .b(x18), .c(new_n112_), .d(x12), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x16), .O(new_n119_));
  inv1   g075(.a(x16), .O(new_n120_));
  nand3  g076(.a(x18), .b(x17), .c(x11), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x19), .c(new_n120_), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(new_n119_), .c(new_n111_), .O(new_n124_));
  nand3  g080(.a(x18), .b(x17), .c(x16), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n113_), .c(x14), .O(new_n127_));
  nand2  g083(.a(new_n121_), .b(new_n120_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n125_), .c(x19), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g086(.a(x18), .b(x12), .O(new_n131_));
  oai21  g087(.a(x18), .b(x13), .c(x17), .O(new_n132_));
  aoi21  g088(.a(new_n132_), .b(new_n131_), .c(new_n51_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n110_), .c(new_n107_), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n130_), .c(new_n104_), .O(new_n136_));
  oai21  g092(.a(new_n124_), .b(x32), .c(new_n136_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n106_), .O(z8));
  inv1   g094(.a(x33), .O(new_n139_));
  oai21  g095(.a(new_n104_), .b(new_n139_), .c(new_n45_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n103_), .O(new_n141_));
  inv1   g097(.a(new_n104_), .O(new_n142_));
  nand3  g098(.a(new_n109_), .b(new_n108_), .c(new_n97_), .O(new_n143_));
  nand4  g099(.a(new_n88_), .b(x31), .c(x30), .d(x29), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n139_), .O(new_n146_));
  aoi21  g102(.a(new_n123_), .b(new_n119_), .c(new_n146_), .O(new_n147_));
  aoi22  g103(.a(new_n132_), .b(new_n131_), .c(new_n129_), .d(new_n127_), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n145_), .c(new_n139_), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n147_), .c(new_n142_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n141_), .O(z9));
endmodule


