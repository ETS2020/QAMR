// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x06), .O(new_n45_));
  nor2   g001(.a(x15), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  inv1   g009(.a(x32), .O(new_n54_));
  aoi21  g010(.a(x33), .b(new_n49_), .c(x02), .O(new_n55_));
  oai21  g011(.a(new_n54_), .b(new_n49_), .c(new_n55_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n46_), .O(z1));
  inv1   g013(.a(x01), .O(new_n58_));
  aoi21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  oai21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(x05), .b(new_n49_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(x06), .c(new_n51_), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n51_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x05), .c(new_n49_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n62_), .c(new_n58_), .O(new_n65_));
  and2   g021(.a(new_n65_), .b(new_n60_), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(new_n67_));
  xnor2a g023(.a(x07), .b(x04), .O(new_n68_));
  inv1   g024(.a(x09), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(x08), .O(new_n70_));
  aoi21  g026(.a(new_n68_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  oai21  g027(.a(new_n68_), .b(new_n67_), .c(new_n71_), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n47_), .O(z2));
  inv1   g029(.a(x27), .O(new_n74_));
  nor2   g030(.a(x22), .b(x17), .O(new_n75_));
  oai22  g031(.a(x24), .b(x19), .c(x23), .d(x18), .O(new_n76_));
  or2    g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g033(.a(x20), .b(x15), .O(new_n78_));
  nor2   g034(.a(x21), .b(x16), .O(new_n79_));
  nor3   g035(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n46_), .O(new_n81_));
  inv1   g037(.a(x26), .O(new_n82_));
  nand3  g038(.a(new_n59_), .b(new_n82_), .c(x25), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n81_), .c(new_n74_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(z3));
  inv1   g042(.a(x28), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n74_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n87_), .b(new_n74_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n89_), .c(new_n84_), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n80_), .c(new_n47_), .O(z4));
  nand2  g048(.a(new_n88_), .b(x29), .O(new_n93_));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n93_), .c(new_n84_), .O(new_n96_));
  nor3   g052(.a(new_n96_), .b(new_n80_), .c(new_n46_), .O(z5));
  inv1   g053(.a(new_n80_), .O(new_n98_));
  inv1   g054(.a(x30), .O(new_n99_));
  nor2   g055(.a(new_n99_), .b(new_n94_), .O(new_n100_));
  aoi21  g056(.a(new_n93_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  inv1   g057(.a(x18), .O(new_n102_));
  inv1   g058(.a(x23), .O(new_n103_));
  nand3  g059(.a(new_n94_), .b(new_n103_), .c(new_n102_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n76_), .c(x17), .O(new_n105_));
  nand2  g061(.a(new_n75_), .b(x29), .O(new_n106_));
  inv1   g062(.a(x17), .O(new_n107_));
  nand3  g063(.a(new_n76_), .b(x22), .c(new_n107_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x16), .c(x15), .O(new_n110_));
  inv1   g066(.a(x21), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x15), .O(new_n112_));
  aoi21  g068(.a(x16), .b(x15), .c(new_n78_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n112_), .c(new_n77_), .O(new_n114_));
  nor2   g070(.a(new_n94_), .b(x06), .O(new_n115_));
  nand2  g071(.a(new_n94_), .b(x15), .O(new_n116_));
  inv1   g072(.a(new_n79_), .O(new_n117_));
  nor2   g073(.a(new_n117_), .b(new_n78_), .O(new_n118_));
  aoi22  g074(.a(new_n118_), .b(new_n116_), .c(new_n115_), .d(new_n78_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n114_), .c(new_n110_), .O(new_n120_));
  nor2   g076(.a(new_n88_), .b(new_n99_), .O(new_n121_));
  aoi22  g077(.a(new_n121_), .b(new_n120_), .c(new_n101_), .d(new_n98_), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n83_), .c(new_n47_), .O(z6));
  nor2   g079(.a(x31), .b(x30), .O(new_n124_));
  nand2  g080(.a(x31), .b(x30), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nor2   g082(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n88_), .c(x29), .O(new_n128_));
  aoi21  g084(.a(new_n93_), .b(x31), .c(new_n83_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n128_), .c(new_n81_), .O(z7));
  inv1   g086(.a(x15), .O(new_n131_));
  inv1   g087(.a(x19), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x18), .c(x14), .O(new_n133_));
  nand3  g089(.a(x19), .b(new_n102_), .c(x13), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n133_), .c(new_n107_), .O(new_n135_));
  nand4  g091(.a(x19), .b(x18), .c(new_n107_), .d(x12), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n135_), .c(x16), .O(new_n138_));
  inv1   g094(.a(x16), .O(new_n139_));
  nor2   g095(.a(new_n132_), .b(new_n102_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(x17), .c(new_n139_), .d(x11), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(new_n138_), .c(new_n131_), .O(new_n142_));
  nand4  g098(.a(x17), .b(x16), .c(new_n131_), .d(x10), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n140_), .c(new_n45_), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  aoi21  g102(.a(new_n87_), .b(new_n74_), .c(new_n94_), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(x30), .c(x31), .O(new_n148_));
  aoi21  g104(.a(new_n147_), .b(x30), .c(new_n148_), .O(new_n149_));
  oai21  g105(.a(new_n146_), .b(new_n142_), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n144_), .b(new_n140_), .O(new_n151_));
  nor2   g107(.a(new_n102_), .b(new_n107_), .O(new_n152_));
  nand2  g108(.a(x18), .b(x12), .O(new_n153_));
  oai21  g109(.a(x18), .b(x13), .c(x17), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  aoi22  g111(.a(new_n155_), .b(x16), .c(new_n152_), .d(x11), .O(new_n156_));
  inv1   g112(.a(x14), .O(new_n157_));
  nor2   g113(.a(x19), .b(new_n157_), .O(new_n158_));
  nand2  g114(.a(new_n152_), .b(x16), .O(new_n159_));
  aoi21  g115(.a(new_n159_), .b(new_n132_), .c(new_n131_), .O(new_n160_));
  oai21  g116(.a(new_n159_), .b(new_n158_), .c(new_n160_), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n156_), .c(new_n151_), .O(new_n162_));
  aoi21  g118(.a(new_n162_), .b(new_n149_), .c(new_n46_), .O(new_n163_));
  nand2  g119(.a(new_n82_), .b(x00), .O(new_n164_));
  inv1   g120(.a(new_n164_), .O(new_n165_));
  oai21  g121(.a(new_n163_), .b(new_n54_), .c(new_n165_), .O(new_n166_));
  aoi21  g122(.a(new_n150_), .b(new_n54_), .c(new_n166_), .O(z8));
  inv1   g123(.a(new_n147_), .O(new_n168_));
  nand2  g124(.a(x33), .b(x30), .O(new_n169_));
  nor3   g125(.a(x33), .b(new_n132_), .c(new_n102_), .O(new_n170_));
  nand3  g126(.a(new_n170_), .b(new_n144_), .c(new_n124_), .O(new_n171_));
  aoi21  g127(.a(new_n171_), .b(new_n169_), .c(x06), .O(new_n172_));
  aoi21  g128(.a(new_n141_), .b(new_n138_), .c(x33), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n124_), .O(new_n174_));
  aoi21  g130(.a(new_n174_), .b(new_n169_), .c(new_n131_), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n172_), .c(new_n168_), .O(new_n176_));
  nand3  g132(.a(new_n170_), .b(new_n144_), .c(new_n126_), .O(new_n177_));
  oai21  g133(.a(new_n50_), .b(x30), .c(new_n177_), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n45_), .O(new_n179_));
  nor2   g135(.a(new_n50_), .b(x30), .O(new_n180_));
  aoi21  g136(.a(new_n173_), .b(new_n126_), .c(new_n180_), .O(new_n181_));
  oai21  g137(.a(new_n181_), .b(new_n131_), .c(new_n179_), .O(new_n182_));
  inv1   g138(.a(new_n127_), .O(new_n183_));
  nand2  g139(.a(new_n47_), .b(x33), .O(new_n184_));
  aoi21  g140(.a(new_n162_), .b(new_n183_), .c(new_n184_), .O(new_n185_));
  aoi21  g141(.a(new_n182_), .b(new_n147_), .c(new_n185_), .O(new_n186_));
  aoi21  g142(.a(new_n186_), .b(new_n176_), .c(new_n164_), .O(z9));
endmodule


