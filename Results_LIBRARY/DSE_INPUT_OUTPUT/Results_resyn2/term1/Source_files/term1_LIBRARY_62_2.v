// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(x23), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x33), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  or2    g007(.a(new_n50_), .b(x32), .O(new_n52_));
  nand3  g008(.a(new_n52_), .b(new_n51_), .c(new_n47_), .O(z1));
  inv1   g009(.a(x01), .O(new_n54_));
  aoi21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  oai21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  aoi21  g015(.a(x05), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g016(.a(new_n59_), .b(x05), .c(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  oai21  g018(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  xnor2a g019(.a(x07), .b(x04), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(x08), .O(new_n66_));
  aoi21  g022(.a(new_n64_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  oai21  g023(.a(new_n64_), .b(new_n63_), .c(new_n67_), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n47_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  oai21  g026(.a(x24), .b(x19), .c(x23), .O(new_n71_));
  nor2   g027(.a(x20), .b(x15), .O(new_n72_));
  oai22  g028(.a(x22), .b(x17), .c(x21), .d(x16), .O(new_n73_));
  nor3   g029(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  inv1   g031(.a(x26), .O(new_n76_));
  nand3  g032(.a(new_n55_), .b(new_n76_), .c(x25), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n70_), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(z3));
  inv1   g036(.a(x28), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n70_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n81_), .b(new_n70_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n83_), .c(new_n78_), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n74_), .c(new_n47_), .O(z4));
  nand2  g042(.a(new_n82_), .b(x29), .O(new_n87_));
  inv1   g043(.a(x29), .O(new_n88_));
  nand2  g044(.a(new_n83_), .b(new_n88_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n87_), .c(new_n78_), .O(new_n90_));
  nor3   g046(.a(new_n90_), .b(new_n74_), .c(new_n46_), .O(z5));
  inv1   g047(.a(x30), .O(new_n92_));
  aoi21  g048(.a(new_n87_), .b(new_n92_), .c(new_n77_), .O(new_n93_));
  oai21  g049(.a(new_n87_), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  aoi21  g050(.a(new_n94_), .b(new_n47_), .c(new_n74_), .O(z6));
  xnor2a g051(.a(x31), .b(x30), .O(new_n96_));
  or2    g052(.a(new_n96_), .b(new_n87_), .O(new_n97_));
  aoi21  g053(.a(new_n87_), .b(x31), .c(new_n77_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n97_), .c(new_n75_), .O(z7));
  inv1   g055(.a(x19), .O(new_n100_));
  nand3  g056(.a(x17), .b(x16), .c(x15), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  inv1   g058(.a(x16), .O(new_n103_));
  nand2  g059(.a(x17), .b(x11), .O(new_n104_));
  nor2   g060(.a(x17), .b(x12), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n103_), .c(new_n104_), .O(new_n106_));
  nand2  g062(.a(new_n84_), .b(x29), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n92_), .O(new_n108_));
  inv1   g064(.a(x15), .O(new_n109_));
  nand3  g065(.a(x17), .b(x16), .c(x10), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n108_), .c(new_n106_), .d(new_n102_), .O(new_n112_));
  nand2  g068(.a(new_n100_), .b(x14), .O(new_n113_));
  inv1   g069(.a(new_n101_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x18), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  nand3  g073(.a(new_n84_), .b(x30), .c(x29), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n117_), .c(x31), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n112_), .c(x23), .O(new_n120_));
  nand2  g076(.a(new_n118_), .b(new_n108_), .O(new_n121_));
  nand4  g077(.a(new_n114_), .b(x31), .c(x19), .d(x13), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n121_), .c(new_n45_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x32), .O(new_n125_));
  inv1   g081(.a(x17), .O(new_n126_));
  nand3  g082(.a(x19), .b(new_n45_), .c(x13), .O(new_n127_));
  nand3  g083(.a(new_n100_), .b(x18), .c(x14), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(new_n129_));
  inv1   g085(.a(x23), .O(new_n130_));
  nand2  g086(.a(new_n127_), .b(new_n130_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand4  g088(.a(x19), .b(x18), .c(new_n126_), .d(x12), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x23), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n132_), .c(new_n103_), .O(new_n136_));
  nand2  g092(.a(x19), .b(x18), .O(new_n137_));
  nor3   g093(.a(new_n137_), .b(new_n104_), .c(x16), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(x23), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n136_), .c(x15), .O(new_n141_));
  nor3   g097(.a(new_n137_), .b(new_n110_), .c(x15), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x23), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  inv1   g100(.a(new_n108_), .O(new_n145_));
  nand2  g101(.a(new_n118_), .b(x31), .O(new_n146_));
  nor3   g102(.a(new_n146_), .b(new_n145_), .c(x32), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand2  g104(.a(new_n76_), .b(x00), .O(new_n149_));
  aoi21  g105(.a(new_n148_), .b(new_n125_), .c(new_n149_), .O(z8));
  inv1   g106(.a(new_n138_), .O(new_n151_));
  oai21  g107(.a(new_n134_), .b(new_n129_), .c(x16), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  aoi21  g109(.a(new_n153_), .b(x15), .c(new_n142_), .O(new_n154_));
  nand2  g110(.a(new_n121_), .b(new_n96_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n154_), .c(new_n49_), .O(new_n156_));
  nand2  g112(.a(new_n117_), .b(x33), .O(new_n157_));
  oai21  g113(.a(new_n110_), .b(new_n45_), .c(new_n109_), .O(new_n158_));
  oai21  g114(.a(new_n116_), .b(x19), .c(new_n158_), .O(new_n159_));
  nor2   g115(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  inv1   g116(.a(new_n104_), .O(new_n161_));
  nand2  g117(.a(x17), .b(x13), .O(new_n162_));
  oai21  g118(.a(new_n105_), .b(new_n45_), .c(new_n162_), .O(new_n163_));
  aoi22  g119(.a(new_n163_), .b(x16), .c(new_n161_), .d(x18), .O(new_n164_));
  nor2   g120(.a(new_n164_), .b(new_n155_), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(new_n160_), .c(new_n149_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n156_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n47_), .O(z9));
endmodule


