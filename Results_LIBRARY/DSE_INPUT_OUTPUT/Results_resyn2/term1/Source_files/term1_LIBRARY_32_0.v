// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_;
  nor2   g000(.a(x18), .b(x00), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nand2  g002(.a(x03), .b(x02), .O(new_n47_));
  inv1   g003(.a(x02), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g007(.a(x33), .O(new_n52_));
  aoi21  g008(.a(new_n51_), .b(new_n52_), .c(new_n45_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(x32), .c(new_n53_), .O(z1));
  inv1   g010(.a(x08), .O(new_n55_));
  xnor2a g011(.a(x07), .b(x04), .O(new_n56_));
  nand2  g012(.a(new_n51_), .b(x01), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n49_), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n48_), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  oai21  g016(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  xor2a  g018(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  nor4   g019(.a(new_n63_), .b(new_n45_), .c(x09), .d(new_n55_), .O(z2));
  inv1   g020(.a(new_n45_), .O(new_n65_));
  nor2   g021(.a(x24), .b(x19), .O(new_n66_));
  nor2   g022(.a(x22), .b(x17), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  inv1   g024(.a(x16), .O(new_n69_));
  inv1   g025(.a(x21), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g027(.a(x20), .b(x15), .O(new_n72_));
  nor2   g028(.a(x23), .b(x18), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(new_n75_));
  inv1   g031(.a(x26), .O(new_n76_));
  nand4  g032(.a(new_n47_), .b(new_n76_), .c(x25), .d(x01), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(x27), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n65_), .O(z3));
  nor2   g036(.a(new_n77_), .b(new_n45_), .O(new_n81_));
  and2   g037(.a(new_n81_), .b(new_n75_), .O(new_n82_));
  inv1   g038(.a(x27), .O(new_n83_));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n83_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n86_), .c(new_n82_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z4));
  nand2  g045(.a(new_n85_), .b(x29), .O(new_n90_));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n90_), .c(new_n82_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(z5));
  inv1   g050(.a(x30), .O(new_n95_));
  xor2a  g051(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n82_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(z6));
  nand2  g054(.a(new_n90_), .b(x31), .O(new_n99_));
  inv1   g055(.a(x31), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n95_), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  nand2  g058(.a(new_n100_), .b(new_n95_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  or2    g060(.a(new_n104_), .b(new_n90_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n99_), .c(new_n82_), .O(z7));
  nand2  g062(.a(new_n87_), .b(x29), .O(new_n107_));
  inv1   g063(.a(x32), .O(new_n108_));
  inv1   g064(.a(x17), .O(new_n109_));
  inv1   g065(.a(x19), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x18), .c(x14), .O(new_n111_));
  inv1   g067(.a(x18), .O(new_n112_));
  nand3  g068(.a(x19), .b(new_n112_), .c(x13), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n111_), .c(new_n109_), .O(new_n114_));
  nand4  g070(.a(x19), .b(x18), .c(new_n109_), .d(x12), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n114_), .c(x16), .O(new_n117_));
  nand2  g073(.a(x19), .b(new_n69_), .O(new_n118_));
  nand3  g074(.a(x18), .b(x17), .c(x11), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  inv1   g076(.a(x15), .O(new_n121_));
  nand3  g077(.a(x17), .b(x16), .c(x10), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x19), .c(new_n121_), .O(new_n124_));
  nor2   g080(.a(new_n124_), .b(new_n112_), .O(new_n125_));
  aoi21  g081(.a(new_n120_), .b(x15), .c(new_n125_), .O(new_n126_));
  nand2  g082(.a(new_n101_), .b(new_n108_), .O(new_n127_));
  oai22  g083(.a(new_n127_), .b(new_n126_), .c(new_n108_), .d(x30), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n107_), .O(new_n129_));
  nand2  g085(.a(x31), .b(new_n95_), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n126_), .c(new_n108_), .O(new_n131_));
  aoi21  g087(.a(x32), .b(new_n95_), .c(new_n107_), .O(new_n132_));
  inv1   g088(.a(x13), .O(new_n133_));
  nor2   g089(.a(x17), .b(x12), .O(new_n134_));
  oai22  g090(.a(new_n134_), .b(new_n112_), .c(new_n109_), .d(new_n133_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x16), .O(new_n136_));
  nand3  g092(.a(x17), .b(x16), .c(x15), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n112_), .c(new_n110_), .O(new_n138_));
  oai21  g094(.a(new_n122_), .b(new_n112_), .c(new_n121_), .O(new_n139_));
  nand2  g095(.a(new_n110_), .b(x14), .O(new_n140_));
  inv1   g096(.a(new_n137_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n140_), .c(x18), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n139_), .c(new_n138_), .d(x31), .O(new_n143_));
  aoi21  g099(.a(new_n136_), .b(new_n119_), .c(new_n143_), .O(new_n144_));
  nor2   g100(.a(new_n144_), .b(new_n108_), .O(new_n145_));
  aoi21  g101(.a(new_n132_), .b(new_n131_), .c(new_n145_), .O(new_n146_));
  nand2  g102(.a(new_n76_), .b(x00), .O(new_n147_));
  aoi21  g103(.a(new_n146_), .b(new_n129_), .c(new_n147_), .O(z8));
  inv1   g104(.a(x00), .O(new_n149_));
  nor2   g105(.a(new_n52_), .b(new_n95_), .O(new_n150_));
  nand4  g106(.a(x19), .b(x17), .c(new_n69_), .d(x11), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  nand2  g108(.a(new_n109_), .b(x12), .O(new_n153_));
  oai21  g109(.a(x19), .b(x17), .c(x16), .O(new_n154_));
  aoi21  g110(.a(new_n153_), .b(new_n140_), .c(new_n154_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n152_), .c(x15), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n124_), .O(new_n157_));
  nor2   g113(.a(new_n103_), .b(x33), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(x18), .O(new_n159_));
  inv1   g115(.a(new_n159_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n157_), .c(new_n150_), .O(new_n161_));
  nor3   g117(.a(new_n137_), .b(new_n110_), .c(new_n133_), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(new_n158_), .c(new_n112_), .O(new_n163_));
  oai21  g119(.a(new_n161_), .b(new_n149_), .c(new_n163_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n107_), .O(new_n165_));
  inv1   g121(.a(new_n107_), .O(new_n166_));
  nand2  g122(.a(x33), .b(new_n95_), .O(new_n167_));
  nand3  g123(.a(new_n162_), .b(new_n101_), .c(new_n52_), .O(new_n168_));
  aoi21  g124(.a(new_n168_), .b(new_n167_), .c(x18), .O(new_n169_));
  nand3  g125(.a(new_n101_), .b(new_n52_), .c(x18), .O(new_n170_));
  inv1   g126(.a(new_n170_), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n157_), .O(new_n172_));
  aoi21  g128(.a(new_n172_), .b(new_n167_), .c(new_n149_), .O(new_n173_));
  oai21  g129(.a(new_n173_), .b(new_n169_), .c(new_n166_), .O(new_n174_));
  aoi21  g130(.a(new_n162_), .b(new_n104_), .c(x18), .O(new_n175_));
  nand2  g131(.a(new_n122_), .b(new_n121_), .O(new_n176_));
  aoi21  g132(.a(new_n176_), .b(x19), .c(new_n141_), .O(new_n177_));
  nand2  g133(.a(x17), .b(x11), .O(new_n178_));
  oai21  g134(.a(new_n134_), .b(new_n69_), .c(new_n178_), .O(new_n179_));
  nand3  g135(.a(new_n179_), .b(new_n142_), .c(new_n104_), .O(new_n180_));
  nor2   g136(.a(new_n180_), .b(new_n177_), .O(new_n181_));
  nor2   g137(.a(new_n181_), .b(new_n149_), .O(new_n182_));
  oai21  g138(.a(new_n182_), .b(new_n175_), .c(x33), .O(new_n183_));
  nand3  g139(.a(new_n183_), .b(new_n174_), .c(new_n165_), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n76_), .O(new_n185_));
  nand2  g141(.a(new_n185_), .b(new_n65_), .O(z9));
endmodule


