// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x00), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(x33), .O(new_n53_));
  aoi21  g009(.a(new_n52_), .b(new_n53_), .c(new_n46_), .O(new_n54_));
  oai21  g010(.a(new_n52_), .b(x32), .c(new_n54_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  nand2  g013(.a(new_n52_), .b(x01), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n50_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n49_), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(x01), .O(new_n61_));
  oai21  g017(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  xor2a  g019(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  nor4   g020(.a(new_n64_), .b(new_n46_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g021(.a(new_n46_), .O(new_n66_));
  nor2   g022(.a(x24), .b(x19), .O(new_n67_));
  nor2   g023(.a(x22), .b(x17), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g025(.a(x16), .O(new_n70_));
  inv1   g026(.a(x21), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor2   g028(.a(x20), .b(x15), .O(new_n73_));
  nor2   g029(.a(x23), .b(x18), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(new_n76_));
  inv1   g032(.a(x26), .O(new_n77_));
  nand4  g033(.a(new_n48_), .b(new_n77_), .c(x25), .d(x01), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(x27), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n66_), .O(z3));
  nor2   g037(.a(new_n78_), .b(new_n46_), .O(new_n82_));
  and2   g038(.a(new_n82_), .b(new_n76_), .O(new_n83_));
  inv1   g039(.a(x27), .O(new_n84_));
  inv1   g040(.a(x28), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n85_), .b(new_n84_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n87_), .c(new_n83_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(z4));
  nand2  g046(.a(new_n86_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n87_), .b(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n91_), .c(new_n83_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z5));
  inv1   g051(.a(x30), .O(new_n96_));
  xor2a  g052(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n83_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z6));
  nand2  g055(.a(x31), .b(x30), .O(new_n100_));
  nor2   g056(.a(x31), .b(x30), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  or2    g059(.a(new_n103_), .b(new_n91_), .O(new_n104_));
  nand2  g060(.a(new_n91_), .b(x31), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n104_), .c(new_n83_), .O(z7));
  inv1   g062(.a(x15), .O(new_n107_));
  inv1   g063(.a(x17), .O(new_n108_));
  nand3  g064(.a(x19), .b(new_n45_), .c(x13), .O(new_n109_));
  inv1   g065(.a(x19), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x18), .c(x14), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  nand4  g068(.a(x19), .b(x18), .c(new_n108_), .d(x12), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  nand2  g071(.a(x17), .b(x11), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(x19), .c(x18), .d(new_n70_), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n115_), .c(new_n107_), .O(new_n119_));
  nand3  g075(.a(x17), .b(x16), .c(x10), .O(new_n120_));
  nor4   g076(.a(new_n120_), .b(new_n110_), .c(new_n45_), .d(x15), .O(new_n121_));
  inv1   g077(.a(x31), .O(new_n122_));
  nor2   g078(.a(x32), .b(new_n122_), .O(new_n123_));
  oai21  g079(.a(new_n121_), .b(new_n119_), .c(new_n123_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n96_), .O(new_n125_));
  inv1   g081(.a(x32), .O(new_n126_));
  nand2  g082(.a(new_n88_), .b(x29), .O(new_n127_));
  aoi21  g083(.a(new_n126_), .b(x30), .c(new_n127_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  inv1   g085(.a(x12), .O(new_n130_));
  nand2  g086(.a(new_n108_), .b(new_n130_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(x18), .O(new_n132_));
  nand2  g088(.a(x17), .b(x13), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x16), .O(new_n135_));
  oai21  g091(.a(new_n116_), .b(new_n45_), .c(new_n135_), .O(new_n136_));
  nand2  g092(.a(new_n110_), .b(x14), .O(new_n137_));
  nand3  g093(.a(x17), .b(x16), .c(x15), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n137_), .c(x18), .O(new_n140_));
  oai21  g096(.a(new_n138_), .b(new_n45_), .c(new_n110_), .O(new_n141_));
  oai21  g097(.a(new_n120_), .b(new_n45_), .c(new_n107_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n141_), .c(new_n140_), .d(x31), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n136_), .c(new_n126_), .O(new_n145_));
  nand2  g101(.a(new_n124_), .b(x30), .O(new_n146_));
  inv1   g102(.a(new_n127_), .O(new_n147_));
  aoi21  g103(.a(new_n126_), .b(new_n96_), .c(new_n147_), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n146_), .c(new_n145_), .O(new_n149_));
  nand2  g105(.a(new_n77_), .b(x00), .O(new_n150_));
  aoi21  g106(.a(new_n149_), .b(new_n129_), .c(new_n150_), .O(z8));
  nor2   g107(.a(new_n53_), .b(x30), .O(new_n152_));
  nor4   g108(.a(new_n138_), .b(new_n109_), .c(new_n100_), .d(x33), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n152_), .c(x00), .O(new_n154_));
  inv1   g110(.a(new_n100_), .O(new_n155_));
  nand4  g111(.a(x19), .b(x17), .c(new_n70_), .d(x11), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  nand2  g113(.a(new_n108_), .b(x12), .O(new_n158_));
  oai21  g114(.a(x19), .b(x17), .c(x16), .O(new_n159_));
  aoi21  g115(.a(new_n158_), .b(new_n137_), .c(new_n159_), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n157_), .c(x15), .O(new_n161_));
  inv1   g117(.a(new_n120_), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(x19), .c(new_n107_), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(new_n161_), .c(x33), .O(new_n164_));
  aoi21  g120(.a(new_n164_), .b(new_n155_), .c(new_n152_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n45_), .c(new_n154_), .O(new_n166_));
  nand2  g122(.a(x19), .b(x13), .O(new_n167_));
  oai21  g123(.a(new_n167_), .b(new_n138_), .c(new_n45_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n103_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(x00), .O(new_n170_));
  nand2  g126(.a(new_n116_), .b(new_n70_), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n131_), .c(new_n103_), .O(new_n172_));
  nand2  g128(.a(new_n139_), .b(new_n137_), .O(new_n173_));
  nand2  g129(.a(new_n138_), .b(new_n110_), .O(new_n174_));
  nand2  g130(.a(new_n120_), .b(new_n107_), .O(new_n175_));
  nand3  g131(.a(new_n175_), .b(new_n174_), .c(new_n173_), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n172_), .c(x18), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n170_), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(x33), .O(new_n179_));
  inv1   g135(.a(x00), .O(new_n180_));
  nand2  g136(.a(x33), .b(x30), .O(new_n181_));
  inv1   g137(.a(new_n109_), .O(new_n182_));
  nand4  g138(.a(new_n139_), .b(new_n182_), .c(new_n101_), .d(new_n53_), .O(new_n183_));
  aoi21  g139(.a(new_n183_), .b(new_n181_), .c(new_n180_), .O(new_n184_));
  nand2  g140(.a(new_n101_), .b(x18), .O(new_n185_));
  inv1   g141(.a(new_n185_), .O(new_n186_));
  aoi21  g142(.a(new_n186_), .b(new_n164_), .c(new_n184_), .O(new_n187_));
  oai21  g143(.a(new_n187_), .b(new_n147_), .c(new_n179_), .O(new_n188_));
  aoi21  g144(.a(new_n166_), .b(new_n147_), .c(new_n188_), .O(new_n189_));
  oai21  g145(.a(new_n189_), .b(x26), .c(new_n66_), .O(z9));
endmodule


