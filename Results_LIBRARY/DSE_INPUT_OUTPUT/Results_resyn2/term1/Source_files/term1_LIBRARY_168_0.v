// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_;
  nor2   g000(.a(x18), .b(x15), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x33), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(x32), .b(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(x33), .b(new_n49_), .c(x02), .O(new_n53_));
  oai21  g009(.a(new_n52_), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g011(.a(x01), .O(new_n56_));
  aoi21  g012(.a(x03), .b(x02), .c(new_n56_), .O(new_n57_));
  oai21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n49_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n49_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n58_), .O(new_n64_));
  xor2a  g020(.a(x07), .b(x04), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand3  g023(.a(new_n46_), .b(new_n67_), .c(x08), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n66_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  oai22  g026(.a(x22), .b(x17), .c(x21), .d(x16), .O(new_n71_));
  oai22  g027(.a(x24), .b(x19), .c(x23), .d(x18), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g029(.a(x20), .b(x15), .c(new_n73_), .O(new_n74_));
  inv1   g030(.a(x26), .O(new_n75_));
  nand3  g031(.a(new_n57_), .b(new_n75_), .c(x25), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(new_n77_));
  and2   g033(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n70_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n46_), .O(z3));
  and2   g036(.a(new_n74_), .b(new_n46_), .O(new_n81_));
  inv1   g037(.a(x28), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n70_), .O(new_n83_));
  nor2   g039(.a(new_n82_), .b(new_n70_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n83_), .c(new_n81_), .d(new_n77_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(z4));
  inv1   g043(.a(x29), .O(new_n88_));
  nand2  g044(.a(new_n85_), .b(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n84_), .b(x29), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n89_), .c(new_n81_), .d(new_n77_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z5));
  inv1   g048(.a(x30), .O(new_n93_));
  nand2  g049(.a(new_n90_), .b(new_n93_), .O(new_n94_));
  nand3  g050(.a(new_n84_), .b(x30), .c(x29), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n94_), .c(new_n78_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n46_), .O(z6));
  inv1   g053(.a(x15), .O(new_n98_));
  nand2  g054(.a(x20), .b(x18), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n73_), .O(new_n101_));
  nor2   g057(.a(new_n95_), .b(x31), .O(new_n102_));
  nand2  g058(.a(new_n95_), .b(x31), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n77_), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n102_), .c(new_n46_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n101_), .O(z7));
  inv1   g062(.a(x17), .O(new_n107_));
  inv1   g063(.a(x19), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x18), .c(x14), .O(new_n109_));
  inv1   g065(.a(x18), .O(new_n110_));
  nand3  g066(.a(x19), .b(new_n110_), .c(x13), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(new_n112_));
  nand4  g068(.a(x19), .b(x18), .c(new_n107_), .d(x12), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  inv1   g071(.a(x16), .O(new_n116_));
  nor2   g072(.a(new_n110_), .b(new_n107_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(x19), .c(new_n116_), .d(x11), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n115_), .c(new_n98_), .O(new_n119_));
  nor2   g075(.a(new_n107_), .b(new_n116_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(x19), .c(new_n98_), .d(x10), .O(new_n121_));
  nor2   g077(.a(new_n121_), .b(new_n110_), .O(new_n122_));
  aoi21  g078(.a(new_n82_), .b(new_n70_), .c(new_n88_), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(x30), .c(x31), .O(new_n124_));
  aoi21  g080(.a(new_n123_), .b(x30), .c(new_n124_), .O(new_n125_));
  oai21  g081(.a(new_n122_), .b(new_n119_), .c(new_n125_), .O(new_n126_));
  nand3  g082(.a(x18), .b(x17), .c(x11), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand2  g084(.a(x18), .b(x12), .O(new_n129_));
  oai21  g085(.a(x18), .b(x13), .c(x17), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(x16), .c(new_n128_), .O(new_n132_));
  nand2  g088(.a(new_n108_), .b(x14), .O(new_n133_));
  nand3  g089(.a(x18), .b(x17), .c(x16), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  aoi21  g092(.a(new_n134_), .b(new_n108_), .c(new_n98_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n132_), .c(new_n121_), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n125_), .c(new_n45_), .O(new_n140_));
  nand2  g096(.a(new_n75_), .b(x00), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n140_), .b(new_n52_), .c(new_n142_), .O(new_n143_));
  aoi21  g099(.a(new_n126_), .b(new_n52_), .c(new_n143_), .O(z8));
  nand2  g100(.a(new_n83_), .b(x29), .O(new_n145_));
  aoi21  g101(.a(x31), .b(x30), .c(new_n145_), .O(new_n146_));
  nor2   g102(.a(x31), .b(x30), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n123_), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n139_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n46_), .c(x33), .O(new_n151_));
  nor3   g107(.a(new_n148_), .b(new_n146_), .c(x33), .O(new_n152_));
  oai21  g108(.a(new_n122_), .b(new_n119_), .c(new_n152_), .O(new_n153_));
  aoi21  g109(.a(new_n153_), .b(new_n151_), .c(new_n141_), .O(z9));
endmodule


