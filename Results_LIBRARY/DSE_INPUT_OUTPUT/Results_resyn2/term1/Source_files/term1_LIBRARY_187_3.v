// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_;
  inv1   g000(.a(x12), .O(new_n45_));
  inv1   g001(.a(x31), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(new_n51_), .b(new_n52_), .c(new_n47_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(x33), .c(new_n53_), .O(z1));
  inv1   g010(.a(x01), .O(new_n55_));
  aoi21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  oai21  g012(.a(x03), .b(x02), .c(new_n56_), .O(new_n57_));
  inv1   g013(.a(x03), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(new_n64_));
  xor2a  g020(.a(x07), .b(x04), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand3  g023(.a(new_n48_), .b(new_n67_), .c(x08), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n66_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  nor2   g026(.a(x23), .b(x18), .O(new_n71_));
  nor2   g027(.a(x22), .b(x17), .O(new_n72_));
  inv1   g028(.a(x19), .O(new_n73_));
  inv1   g029(.a(x24), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(x20), .b(x15), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nor3   g035(.a(new_n79_), .b(new_n72_), .c(new_n71_), .O(new_n80_));
  inv1   g036(.a(x26), .O(new_n81_));
  nand3  g037(.a(new_n56_), .b(new_n81_), .c(x25), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n70_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n48_), .O(z3));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n70_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n70_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n88_), .c(new_n83_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n48_), .O(z4));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n88_), .b(new_n92_), .O(new_n93_));
  nand2  g049(.a(new_n87_), .b(x29), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n93_), .c(new_n83_), .d(new_n48_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z5));
  inv1   g052(.a(x30), .O(new_n97_));
  nand2  g053(.a(new_n94_), .b(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n87_), .b(x30), .c(x29), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n98_), .c(new_n83_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n48_), .O(z6));
  xor2a  g057(.a(new_n99_), .b(x31), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(new_n83_), .c(new_n47_), .O(z7));
  nand2  g059(.a(new_n81_), .b(x00), .O(new_n104_));
  nand2  g060(.a(x32), .b(new_n46_), .O(new_n105_));
  nand3  g061(.a(x18), .b(x16), .c(x15), .O(new_n106_));
  nand3  g062(.a(x18), .b(x16), .c(x10), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  inv1   g064(.a(x15), .O(new_n109_));
  nand2  g065(.a(x16), .b(x13), .O(new_n110_));
  nand2  g066(.a(x18), .b(x11), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n108_), .c(x19), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n106_), .O(new_n114_));
  inv1   g070(.a(new_n106_), .O(new_n115_));
  nand2  g071(.a(new_n73_), .b(x14), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g073(.a(new_n89_), .b(x30), .c(x29), .O(new_n118_));
  inv1   g074(.a(x17), .O(new_n119_));
  oai21  g075(.a(x28), .b(x27), .c(x29), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n97_), .c(new_n119_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n118_), .c(new_n117_), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(new_n114_), .c(new_n52_), .O(new_n124_));
  inv1   g080(.a(x16), .O(new_n125_));
  nand3  g081(.a(new_n73_), .b(x18), .c(x14), .O(new_n126_));
  inv1   g082(.a(x18), .O(new_n127_));
  nand3  g083(.a(x19), .b(new_n127_), .c(x13), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(new_n126_), .c(new_n125_), .O(new_n129_));
  nand4  g085(.a(x19), .b(x18), .c(new_n125_), .d(x11), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n129_), .c(x15), .O(new_n132_));
  nand3  g088(.a(x16), .b(new_n109_), .c(x10), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x19), .c(x18), .O(new_n135_));
  nand4  g091(.a(new_n121_), .b(new_n118_), .c(new_n52_), .d(x31), .O(new_n136_));
  aoi21  g092(.a(new_n135_), .b(new_n132_), .c(new_n136_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n124_), .c(new_n45_), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n105_), .c(new_n104_), .O(z8));
  nand3  g095(.a(new_n120_), .b(new_n46_), .c(new_n97_), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  aoi21  g097(.a(new_n128_), .b(new_n126_), .c(new_n119_), .O(new_n142_));
  nand4  g098(.a(x19), .b(x18), .c(new_n119_), .d(x12), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n142_), .c(x16), .O(new_n145_));
  nand3  g101(.a(x19), .b(x18), .c(x17), .O(new_n146_));
  inv1   g102(.a(new_n146_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n125_), .c(x11), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n145_), .c(new_n109_), .O(new_n149_));
  nor2   g105(.a(new_n146_), .b(new_n133_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n149_), .c(new_n141_), .O(new_n151_));
  nand2  g107(.a(new_n135_), .b(new_n132_), .O(new_n152_));
  nand4  g108(.a(x31), .b(x30), .c(x17), .d(new_n45_), .O(new_n153_));
  nor2   g109(.a(new_n153_), .b(new_n120_), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n152_), .c(x33), .O(new_n155_));
  inv1   g111(.a(new_n104_), .O(new_n156_));
  nand2  g112(.a(new_n120_), .b(new_n97_), .O(new_n157_));
  nand3  g113(.a(new_n115_), .b(x19), .c(new_n119_), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n157_), .c(new_n46_), .O(new_n159_));
  oai21  g115(.a(new_n118_), .b(new_n46_), .c(new_n140_), .O(new_n160_));
  aoi21  g116(.a(new_n116_), .b(new_n115_), .c(new_n119_), .O(new_n161_));
  aoi22  g117(.a(new_n161_), .b(new_n160_), .c(new_n159_), .d(x12), .O(new_n162_));
  oai21  g118(.a(new_n114_), .b(new_n47_), .c(x33), .O(new_n163_));
  oai21  g119(.a(new_n163_), .b(new_n162_), .c(new_n156_), .O(new_n164_));
  aoi21  g120(.a(new_n155_), .b(new_n151_), .c(new_n164_), .O(z9));
endmodule


