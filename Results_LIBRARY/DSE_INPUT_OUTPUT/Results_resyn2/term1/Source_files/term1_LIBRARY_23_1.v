// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_;
  inv1   g000(.a(x31), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x18), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  aoi21  g007(.a(new_n50_), .b(new_n51_), .c(new_n46_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(x33), .c(new_n52_), .O(z1));
  inv1   g009(.a(x01), .O(new_n54_));
  aoi21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  oai21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  aoi21  g015(.a(x05), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g016(.a(new_n59_), .b(x05), .c(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  oai21  g018(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  xor2a  g019(.a(x07), .b(x04), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand3  g022(.a(new_n47_), .b(new_n66_), .c(x08), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n65_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  oai22  g025(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n70_));
  oai22  g026(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g028(.a(x23), .b(x18), .c(new_n72_), .O(new_n73_));
  inv1   g029(.a(x26), .O(new_n74_));
  nand3  g030(.a(new_n55_), .b(new_n74_), .c(x25), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  and2   g032(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n69_), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z3));
  inv1   g035(.a(new_n75_), .O(new_n80_));
  inv1   g036(.a(x28), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n69_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n81_), .b(new_n69_), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n83_), .c(new_n80_), .d(new_n73_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n47_), .O(z4));
  nand2  g042(.a(new_n82_), .b(x29), .O(new_n87_));
  inv1   g043(.a(x29), .O(new_n88_));
  nand2  g044(.a(new_n83_), .b(new_n88_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n87_), .c(new_n77_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z5));
  inv1   g047(.a(x30), .O(new_n92_));
  nand2  g048(.a(new_n87_), .b(new_n92_), .O(new_n93_));
  inv1   g049(.a(new_n87_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(x30), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n93_), .c(new_n77_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(z6));
  or2    g053(.a(new_n75_), .b(new_n72_), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n95_), .c(x18), .O(new_n99_));
  oai21  g055(.a(x23), .b(x18), .c(x31), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n95_), .c(new_n80_), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  aoi22  g058(.a(new_n102_), .b(new_n73_), .c(new_n99_), .d(x31), .O(z7));
  inv1   g059(.a(x16), .O(new_n104_));
  inv1   g060(.a(x17), .O(new_n105_));
  nand3  g061(.a(x19), .b(new_n105_), .c(x12), .O(new_n106_));
  inv1   g062(.a(x19), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x17), .c(x14), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  nand4  g065(.a(x19), .b(x17), .c(new_n104_), .d(x11), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n109_), .c(x15), .O(new_n112_));
  inv1   g068(.a(x15), .O(new_n113_));
  nor2   g069(.a(new_n107_), .b(new_n105_), .O(new_n114_));
  and2   g070(.a(x16), .b(x10), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  nand3  g072(.a(new_n84_), .b(x30), .c(x29), .O(new_n117_));
  oai21  g073(.a(x28), .b(x27), .c(x29), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n92_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n117_), .c(x31), .O(new_n120_));
  aoi21  g076(.a(new_n116_), .b(new_n112_), .c(new_n120_), .O(new_n121_));
  nor2   g077(.a(new_n121_), .b(x32), .O(new_n122_));
  nand2  g078(.a(new_n74_), .b(x00), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  nand2  g080(.a(x17), .b(x11), .O(new_n125_));
  oai21  g081(.a(x17), .b(x12), .c(x16), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n125_), .c(new_n51_), .O(new_n127_));
  inv1   g083(.a(x14), .O(new_n128_));
  nor2   g084(.a(x19), .b(new_n128_), .O(new_n129_));
  nand3  g085(.a(x17), .b(x16), .c(x15), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nand2  g088(.a(new_n115_), .b(x17), .O(new_n133_));
  aoi22  g089(.a(new_n133_), .b(new_n113_), .c(new_n130_), .d(new_n107_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n132_), .c(new_n127_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n120_), .c(new_n124_), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n122_), .c(new_n47_), .O(z8));
  inv1   g093(.a(x33), .O(new_n138_));
  inv1   g094(.a(new_n119_), .O(new_n139_));
  nor2   g095(.a(new_n104_), .b(new_n113_), .O(new_n140_));
  nand4  g096(.a(new_n45_), .b(new_n107_), .c(x18), .d(x14), .O(new_n141_));
  inv1   g097(.a(x18), .O(new_n142_));
  nand3  g098(.a(x19), .b(new_n142_), .c(x13), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n141_), .c(new_n105_), .O(new_n144_));
  nand2  g100(.a(new_n45_), .b(x18), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(new_n106_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n144_), .c(new_n140_), .O(new_n147_));
  nand2  g103(.a(new_n115_), .b(new_n113_), .O(new_n148_));
  nand3  g104(.a(new_n104_), .b(x15), .c(x11), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n114_), .c(new_n45_), .d(x18), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n139_), .O(new_n153_));
  nand2  g109(.a(new_n116_), .b(new_n112_), .O(new_n154_));
  nand2  g110(.a(x31), .b(x30), .O(new_n155_));
  nor2   g111(.a(new_n155_), .b(new_n118_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n154_), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(new_n153_), .c(new_n138_), .O(new_n158_));
  nand2  g114(.a(x17), .b(x13), .O(new_n159_));
  oai21  g115(.a(x17), .b(x12), .c(x18), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n159_), .c(new_n104_), .O(new_n161_));
  nor2   g117(.a(new_n142_), .b(new_n105_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(x11), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(new_n164_));
  oai21  g120(.a(new_n164_), .b(new_n161_), .c(x15), .O(new_n165_));
  nand2  g121(.a(new_n162_), .b(new_n115_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  nand2  g123(.a(new_n118_), .b(x31), .O(new_n168_));
  nand4  g124(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n169_));
  aoi21  g125(.a(new_n169_), .b(new_n107_), .c(new_n138_), .O(new_n170_));
  nand2  g126(.a(new_n155_), .b(new_n119_), .O(new_n171_));
  nand2  g127(.a(new_n45_), .b(new_n142_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n131_), .O(new_n173_));
  nand4  g129(.a(new_n173_), .b(new_n171_), .c(new_n170_), .d(new_n168_), .O(new_n174_));
  inv1   g130(.a(new_n174_), .O(new_n175_));
  aoi21  g131(.a(new_n175_), .b(new_n167_), .c(new_n123_), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n158_), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n47_), .O(z9));
endmodule


