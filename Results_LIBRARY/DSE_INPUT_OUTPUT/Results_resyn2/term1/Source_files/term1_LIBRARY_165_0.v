// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_;
  nor2   g000(.a(x18), .b(x15), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nand2  g002(.a(x03), .b(x02), .O(new_n47_));
  inv1   g003(.a(x02), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g007(.a(x33), .O(new_n52_));
  aoi21  g008(.a(new_n51_), .b(new_n52_), .c(new_n45_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(x32), .c(new_n53_), .O(z1));
  xnor2a g010(.a(x07), .b(x04), .O(new_n55_));
  nand2  g011(.a(new_n51_), .b(x01), .O(new_n56_));
  nand2  g012(.a(x05), .b(new_n49_), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n48_), .O(new_n58_));
  aoi21  g014(.a(new_n58_), .b(new_n57_), .c(x01), .O(new_n59_));
  oai21  g015(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  xor2a  g017(.a(new_n61_), .b(new_n55_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  inv1   g019(.a(new_n45_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n63_), .c(x08), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n62_), .O(z2));
  inv1   g022(.a(x27), .O(new_n67_));
  inv1   g023(.a(new_n47_), .O(new_n68_));
  inv1   g024(.a(x17), .O(new_n69_));
  inv1   g025(.a(x22), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g027(.a(x24), .b(x19), .O(new_n72_));
  nor2   g028(.a(x21), .b(x16), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  oai22  g031(.a(x23), .b(x18), .c(x20), .d(x15), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n68_), .O(new_n78_));
  nand2  g034(.a(x25), .b(x01), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(x26), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n78_), .c(new_n67_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n64_), .O(z3));
  inv1   g038(.a(x28), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n67_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n83_), .b(new_n67_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n85_), .c(new_n80_), .d(new_n78_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n64_), .O(z4));
  nand2  g044(.a(new_n80_), .b(new_n47_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n84_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n85_), .b(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n91_), .c(new_n90_), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n77_), .c(new_n64_), .O(z5));
  nand2  g051(.a(x23), .b(x15), .O(new_n96_));
  oai21  g052(.a(x20), .b(x15), .c(x18), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n74_), .c(new_n71_), .O(new_n99_));
  nor2   g055(.a(new_n89_), .b(new_n45_), .O(new_n100_));
  inv1   g056(.a(x30), .O(new_n101_));
  nand2  g057(.a(new_n91_), .b(new_n101_), .O(new_n102_));
  nand3  g058(.a(new_n84_), .b(x30), .c(x29), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n102_), .c(new_n100_), .d(new_n99_), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(z6));
  nor2   g061(.a(new_n103_), .b(x31), .O(new_n106_));
  nand2  g062(.a(new_n103_), .b(x31), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n90_), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n106_), .c(new_n64_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n99_), .O(z7));
  inv1   g066(.a(x00), .O(new_n111_));
  nor2   g067(.a(x26), .b(new_n111_), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  inv1   g069(.a(x15), .O(new_n114_));
  inv1   g070(.a(x18), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x13), .O(new_n116_));
  inv1   g072(.a(x19), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x14), .O(new_n118_));
  oai21  g074(.a(x19), .b(x18), .c(x17), .O(new_n119_));
  aoi21  g075(.a(new_n118_), .b(new_n116_), .c(new_n119_), .O(new_n120_));
  nand4  g076(.a(x19), .b(x18), .c(new_n69_), .d(x12), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n120_), .c(x16), .O(new_n123_));
  inv1   g079(.a(x16), .O(new_n124_));
  nand3  g080(.a(x19), .b(x18), .c(x17), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n124_), .c(x11), .O(new_n127_));
  aoi21  g083(.a(new_n127_), .b(new_n123_), .c(new_n114_), .O(new_n128_));
  nand2  g084(.a(new_n114_), .b(x10), .O(new_n129_));
  nor3   g085(.a(new_n129_), .b(new_n125_), .c(new_n124_), .O(new_n130_));
  inv1   g086(.a(x32), .O(new_n131_));
  oai21  g087(.a(x28), .b(x27), .c(x29), .O(new_n132_));
  xor2a  g088(.a(new_n132_), .b(new_n101_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n131_), .c(x31), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n130_), .b(new_n128_), .c(new_n135_), .O(new_n136_));
  nand3  g092(.a(x18), .b(x17), .c(x11), .O(new_n137_));
  nand3  g093(.a(x18), .b(x17), .c(x16), .O(new_n138_));
  aoi22  g094(.a(new_n138_), .b(new_n117_), .c(new_n137_), .d(new_n124_), .O(new_n139_));
  aoi21  g095(.a(new_n117_), .b(x14), .c(new_n138_), .O(new_n140_));
  nand2  g096(.a(x17), .b(x13), .O(new_n141_));
  oai21  g097(.a(x17), .b(x12), .c(x18), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n141_), .c(new_n140_), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n139_), .c(new_n114_), .O(new_n144_));
  nor2   g100(.a(new_n117_), .b(new_n69_), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(x16), .c(x10), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n114_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n133_), .c(x31), .O(new_n148_));
  nor2   g104(.a(new_n45_), .b(new_n131_), .O(new_n149_));
  oai21  g105(.a(new_n148_), .b(new_n144_), .c(new_n149_), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n136_), .c(new_n113_), .O(z8));
  nand2  g107(.a(new_n129_), .b(new_n116_), .O(new_n152_));
  nand2  g108(.a(new_n69_), .b(x12), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n118_), .O(new_n154_));
  nand2  g110(.a(x18), .b(x15), .O(new_n155_));
  aoi21  g111(.a(new_n117_), .b(new_n69_), .c(new_n155_), .O(new_n156_));
  aoi22  g112(.a(new_n156_), .b(new_n154_), .c(new_n152_), .d(new_n145_), .O(new_n157_));
  nand4  g113(.a(new_n126_), .b(new_n124_), .c(x15), .d(x11), .O(new_n158_));
  oai21  g114(.a(new_n157_), .b(new_n124_), .c(new_n158_), .O(new_n159_));
  inv1   g115(.a(new_n133_), .O(new_n160_));
  xnor2a g116(.a(x31), .b(x30), .O(new_n161_));
  and2   g117(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  aoi21  g118(.a(new_n162_), .b(new_n159_), .c(x33), .O(new_n163_));
  nand2  g119(.a(new_n142_), .b(new_n141_), .O(new_n164_));
  nand4  g120(.a(new_n161_), .b(new_n147_), .c(new_n164_), .d(x33), .O(new_n165_));
  nand2  g121(.a(new_n140_), .b(x15), .O(new_n166_));
  nand3  g122(.a(new_n166_), .b(new_n139_), .c(new_n160_), .O(new_n167_));
  oai21  g123(.a(new_n167_), .b(new_n165_), .c(new_n112_), .O(new_n168_));
  oai21  g124(.a(new_n168_), .b(new_n163_), .c(new_n64_), .O(z9));
endmodule


