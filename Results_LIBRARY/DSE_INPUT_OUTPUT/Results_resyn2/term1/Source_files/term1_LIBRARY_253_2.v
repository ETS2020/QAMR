// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x29), .O(new_n45_));
  nor2   g001(.a(x30), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xor2a  g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  aoi21  g006(.a(new_n49_), .b(new_n50_), .c(new_n46_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(x33), .c(new_n51_), .O(z1));
  inv1   g008(.a(x09), .O(new_n53_));
  xor2a  g009(.a(x07), .b(x04), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  inv1   g011(.a(x01), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  inv1   g013(.a(x03), .O(new_n58_));
  nand4  g014(.a(x06), .b(x05), .c(new_n58_), .d(new_n57_), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n58_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nor2   g020(.a(new_n49_), .b(new_n56_), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n55_), .O(new_n67_));
  oai21  g023(.a(new_n65_), .b(new_n64_), .c(new_n54_), .O(new_n68_));
  nand4  g024(.a(new_n68_), .b(new_n67_), .c(new_n53_), .d(x08), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n47_), .O(z2));
  nand2  g026(.a(x03), .b(x02), .O(new_n71_));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  nor2   g028(.a(x22), .b(x17), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g030(.a(x19), .O(new_n75_));
  inv1   g031(.a(x24), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g033(.a(x21), .b(x16), .O(new_n78_));
  nor2   g034(.a(x20), .b(x15), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  inv1   g038(.a(x27), .O(new_n83_));
  nand2  g039(.a(x25), .b(x01), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(x26), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n82_), .c(new_n47_), .O(z3));
  inv1   g043(.a(x28), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n83_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n88_), .b(new_n83_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n90_), .c(new_n85_), .d(new_n47_), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(new_n82_), .O(z4));
  inv1   g049(.a(x30), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n45_), .c(new_n90_), .O(new_n95_));
  nand2  g051(.a(new_n85_), .b(new_n71_), .O(new_n96_));
  aoi21  g052(.a(new_n89_), .b(x29), .c(new_n96_), .O(new_n97_));
  and2   g053(.a(new_n97_), .b(new_n81_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n95_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z5));
  nand2  g056(.a(new_n98_), .b(x30), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(z6));
  oai21  g058(.a(new_n90_), .b(x31), .c(x30), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(x29), .O(new_n104_));
  nand3  g060(.a(new_n89_), .b(x30), .c(x29), .O(new_n105_));
  aoi21  g061(.a(new_n105_), .b(x31), .c(new_n96_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n104_), .c(new_n81_), .O(z7));
  inv1   g063(.a(x15), .O(new_n108_));
  inv1   g064(.a(x17), .O(new_n109_));
  nand3  g065(.a(new_n75_), .b(x18), .c(x14), .O(new_n110_));
  inv1   g066(.a(x18), .O(new_n111_));
  nand3  g067(.a(x19), .b(new_n111_), .c(x13), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  nand4  g069(.a(x19), .b(x18), .c(new_n109_), .d(x12), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n113_), .c(x16), .O(new_n116_));
  inv1   g072(.a(x16), .O(new_n117_));
  nand3  g073(.a(x18), .b(x17), .c(x11), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x19), .c(new_n117_), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n116_), .c(new_n108_), .O(new_n121_));
  nand4  g077(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n122_));
  nor3   g078(.a(new_n122_), .b(new_n75_), .c(x15), .O(new_n123_));
  inv1   g079(.a(x31), .O(new_n124_));
  aoi21  g080(.a(new_n91_), .b(x29), .c(new_n124_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n50_), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  oai21  g083(.a(new_n123_), .b(new_n121_), .c(new_n127_), .O(new_n128_));
  nand2  g084(.a(new_n118_), .b(new_n117_), .O(new_n129_));
  nand4  g085(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  aoi21  g087(.a(new_n129_), .b(x19), .c(new_n131_), .O(new_n132_));
  nand2  g088(.a(x18), .b(x12), .O(new_n133_));
  oai21  g089(.a(x18), .b(x13), .c(x17), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g091(.a(new_n122_), .b(new_n108_), .O(new_n136_));
  nand2  g092(.a(new_n75_), .b(x14), .O(new_n137_));
  nand2  g093(.a(new_n131_), .b(new_n137_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n136_), .c(new_n135_), .O(new_n139_));
  nor2   g095(.a(new_n139_), .b(new_n132_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n125_), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(x32), .c(new_n94_), .O(new_n142_));
  inv1   g098(.a(x00), .O(new_n143_));
  nor2   g099(.a(x26), .b(new_n143_), .O(new_n144_));
  oai21  g100(.a(new_n50_), .b(x29), .c(new_n94_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  aoi21  g102(.a(new_n142_), .b(new_n128_), .c(new_n146_), .O(z8));
  inv1   g103(.a(new_n144_), .O(new_n148_));
  nand2  g104(.a(new_n91_), .b(x31), .O(new_n149_));
  aoi22  g105(.a(new_n149_), .b(x30), .c(x31), .d(new_n45_), .O(new_n150_));
  oai21  g106(.a(new_n140_), .b(new_n46_), .c(new_n150_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x33), .O(new_n152_));
  nor2   g108(.a(new_n94_), .b(x29), .O(new_n153_));
  nor3   g109(.a(new_n153_), .b(new_n46_), .c(x33), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n150_), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  oai21  g112(.a(new_n123_), .b(new_n121_), .c(new_n156_), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(new_n152_), .c(new_n148_), .O(z9));
endmodule


