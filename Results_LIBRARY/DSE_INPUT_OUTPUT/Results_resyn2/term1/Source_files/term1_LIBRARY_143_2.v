// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x25), .O(new_n45_));
  nor2   g001(.a(x31), .b(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(x33), .O(new_n53_));
  aoi21  g009(.a(new_n52_), .b(new_n53_), .c(new_n46_), .O(new_n54_));
  oai21  g010(.a(new_n52_), .b(x32), .c(new_n54_), .O(z1));
  xnor2a g011(.a(x07), .b(x04), .O(new_n56_));
  nand2  g012(.a(new_n52_), .b(x01), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n50_), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n49_), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  oai21  g016(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  xor2a  g018(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  inv1   g019(.a(x09), .O(new_n64_));
  inv1   g020(.a(new_n46_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(new_n64_), .c(x08), .O(new_n66_));
  nor2   g022(.a(new_n66_), .b(new_n63_), .O(z2));
  inv1   g023(.a(x27), .O(new_n68_));
  nor2   g024(.a(x20), .b(x15), .O(new_n69_));
  nor2   g025(.a(x24), .b(x19), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g027(.a(x18), .O(new_n72_));
  inv1   g028(.a(x23), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(x22), .b(x17), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(new_n78_));
  and2   g034(.a(new_n78_), .b(new_n48_), .O(new_n79_));
  inv1   g035(.a(x01), .O(new_n80_));
  nor2   g036(.a(x26), .b(new_n80_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(x31), .c(x25), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n79_), .c(new_n68_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(z3));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n68_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n68_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n88_), .c(new_n83_), .d(new_n79_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z4));
  nand2  g047(.a(new_n87_), .b(x29), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nor2   g049(.a(new_n87_), .b(x29), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n81_), .c(new_n79_), .O(new_n96_));
  aoi21  g052(.a(new_n96_), .b(x31), .c(new_n45_), .O(z5));
  inv1   g053(.a(x30), .O(new_n98_));
  nand2  g054(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n93_), .b(x30), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n99_), .c(new_n81_), .d(new_n79_), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(x31), .c(new_n45_), .O(z6));
  inv1   g058(.a(new_n100_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n83_), .c(new_n79_), .O(z7));
  inv1   g060(.a(x32), .O(new_n105_));
  oai21  g061(.a(x28), .b(x27), .c(x29), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n98_), .O(new_n107_));
  nand2  g063(.a(new_n106_), .b(new_n98_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(x31), .O(new_n109_));
  nor2   g065(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  inv1   g066(.a(x15), .O(new_n111_));
  inv1   g067(.a(x17), .O(new_n112_));
  inv1   g068(.a(x19), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x18), .c(x14), .O(new_n114_));
  nand3  g070(.a(x19), .b(new_n72_), .c(x13), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n114_), .c(new_n112_), .O(new_n116_));
  nand4  g072(.a(x19), .b(x18), .c(new_n112_), .d(x12), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x16), .O(new_n119_));
  inv1   g075(.a(x16), .O(new_n120_));
  nand3  g076(.a(x19), .b(x18), .c(x17), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n120_), .c(x11), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(new_n119_), .c(new_n111_), .O(new_n124_));
  nand3  g080(.a(x16), .b(new_n111_), .c(x10), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(new_n124_), .c(new_n110_), .O(new_n127_));
  nand4  g083(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n128_));
  nor2   g084(.a(new_n72_), .b(new_n112_), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(x11), .c(x16), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n113_), .c(new_n128_), .O(new_n131_));
  nand2  g087(.a(x17), .b(x13), .O(new_n132_));
  oai21  g088(.a(x17), .b(x12), .c(x18), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  aoi21  g090(.a(new_n113_), .b(x14), .c(new_n128_), .O(new_n135_));
  nand3  g091(.a(new_n129_), .b(x16), .c(x10), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n111_), .c(new_n135_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n134_), .c(new_n131_), .O(new_n138_));
  nand2  g094(.a(new_n110_), .b(x32), .O(new_n139_));
  inv1   g095(.a(x26), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x00), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n46_), .O(new_n142_));
  oai21  g098(.a(new_n139_), .b(new_n138_), .c(new_n142_), .O(new_n143_));
  aoi21  g099(.a(new_n127_), .b(new_n105_), .c(new_n143_), .O(z8));
  inv1   g100(.a(x31), .O(new_n145_));
  nand3  g101(.a(new_n106_), .b(new_n145_), .c(new_n98_), .O(new_n146_));
  inv1   g102(.a(new_n146_), .O(new_n147_));
  aoi21  g103(.a(new_n107_), .b(x31), .c(new_n147_), .O(new_n148_));
  nor2   g104(.a(new_n46_), .b(new_n53_), .O(new_n149_));
  oai21  g105(.a(new_n148_), .b(new_n138_), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n107_), .b(x31), .O(new_n151_));
  nand2  g107(.a(new_n147_), .b(new_n45_), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n151_), .c(x33), .O(new_n153_));
  oai21  g109(.a(new_n126_), .b(new_n124_), .c(new_n153_), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n150_), .c(new_n141_), .O(z9));
endmodule


