// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_;
  inv1   g000(.a(x27), .O(new_n45_));
  inv1   g001(.a(x29), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  inv1   g009(.a(x32), .O(new_n54_));
  aoi21  g010(.a(x33), .b(new_n49_), .c(x02), .O(new_n55_));
  oai21  g011(.a(new_n54_), .b(new_n49_), .c(new_n55_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n47_), .O(z1));
  inv1   g013(.a(new_n47_), .O(new_n58_));
  xor2a  g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nand2  g016(.a(x05), .b(new_n49_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(x06), .c(new_n51_), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n51_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x05), .c(new_n49_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(new_n66_));
  nand2  g022(.a(x03), .b(x02), .O(new_n67_));
  aoi21  g023(.a(new_n49_), .b(new_n51_), .c(new_n60_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n59_), .O(new_n70_));
  inv1   g026(.a(x09), .O(new_n71_));
  nand2  g027(.a(new_n69_), .b(new_n59_), .O(new_n72_));
  nand3  g028(.a(new_n72_), .b(new_n71_), .c(x08), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n58_), .O(z2));
  nor2   g030(.a(x20), .b(x15), .O(new_n75_));
  nor2   g031(.a(x24), .b(x19), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  inv1   g033(.a(x18), .O(new_n78_));
  inv1   g034(.a(x23), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nor2   g036(.a(x22), .b(x17), .O(new_n81_));
  nor2   g037(.a(x21), .b(x16), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n80_), .c(new_n77_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  nand2  g041(.a(x25), .b(x01), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(x26), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n45_), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n85_), .c(new_n58_), .O(z3));
  inv1   g045(.a(x28), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n45_), .O(new_n91_));
  nand2  g047(.a(x28), .b(x27), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n91_), .c(new_n87_), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n85_), .c(new_n58_), .O(z4));
  nand2  g050(.a(new_n92_), .b(new_n46_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n87_), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n85_), .c(new_n58_), .O(z5));
  nand2  g053(.a(new_n87_), .b(x30), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n85_), .c(new_n58_), .O(z6));
  inv1   g055(.a(new_n85_), .O(new_n100_));
  inv1   g056(.a(x31), .O(new_n101_));
  nand2  g057(.a(new_n87_), .b(new_n101_), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n100_), .c(new_n47_), .O(z7));
  inv1   g060(.a(x16), .O(new_n105_));
  inv1   g061(.a(x19), .O(new_n106_));
  nand2  g062(.a(x32), .b(x18), .O(new_n107_));
  inv1   g063(.a(x13), .O(new_n108_));
  nor2   g064(.a(x18), .b(new_n108_), .O(new_n109_));
  nor2   g065(.a(new_n46_), .b(new_n90_), .O(new_n110_));
  nor2   g066(.a(new_n101_), .b(x30), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n110_), .c(new_n109_), .d(new_n54_), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n107_), .c(new_n106_), .O(new_n113_));
  inv1   g069(.a(x14), .O(new_n114_));
  nand2  g070(.a(x32), .b(new_n114_), .O(new_n115_));
  nor2   g071(.a(x19), .b(new_n114_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n111_), .c(new_n110_), .d(new_n54_), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n115_), .c(new_n78_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n113_), .c(x17), .O(new_n119_));
  nand2  g075(.a(new_n111_), .b(new_n54_), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(new_n121_));
  inv1   g077(.a(x17), .O(new_n122_));
  nand4  g078(.a(x19), .b(x18), .c(new_n122_), .d(x12), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(new_n121_), .c(new_n110_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n119_), .c(new_n105_), .O(new_n126_));
  inv1   g082(.a(new_n110_), .O(new_n127_));
  nand3  g083(.a(x18), .b(x17), .c(x11), .O(new_n128_));
  inv1   g084(.a(new_n128_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x19), .c(new_n105_), .O(new_n130_));
  nor3   g086(.a(new_n130_), .b(new_n120_), .c(new_n127_), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n126_), .c(x15), .O(new_n132_));
  nand2  g088(.a(new_n78_), .b(x13), .O(new_n133_));
  nand2  g089(.a(new_n106_), .b(x14), .O(new_n134_));
  oai21  g090(.a(x19), .b(x18), .c(x17), .O(new_n135_));
  aoi21  g091(.a(new_n134_), .b(new_n133_), .c(new_n135_), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n124_), .c(x16), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n130_), .O(new_n138_));
  nor2   g094(.a(new_n78_), .b(new_n122_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(x16), .c(x10), .O(new_n140_));
  inv1   g096(.a(x15), .O(new_n141_));
  nand2  g097(.a(x19), .b(new_n141_), .O(new_n142_));
  nor2   g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  aoi21  g099(.a(new_n138_), .b(x15), .c(new_n143_), .O(new_n144_));
  nand2  g100(.a(x31), .b(x30), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n144_), .c(new_n54_), .O(new_n146_));
  aoi21  g102(.a(x32), .b(x30), .c(new_n110_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  inv1   g104(.a(x30), .O(new_n149_));
  nand2  g105(.a(new_n143_), .b(new_n121_), .O(new_n150_));
  oai21  g106(.a(new_n54_), .b(new_n149_), .c(new_n150_), .O(new_n151_));
  oai21  g107(.a(new_n129_), .b(x16), .c(x15), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n140_), .O(new_n153_));
  nand3  g109(.a(new_n139_), .b(x16), .c(x15), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n106_), .O(new_n155_));
  aoi21  g111(.a(x18), .b(x12), .c(x17), .O(new_n156_));
  nor2   g112(.a(x18), .b(x13), .O(new_n157_));
  nor3   g113(.a(new_n157_), .b(new_n156_), .c(new_n101_), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n155_), .c(new_n153_), .O(new_n159_));
  aoi22  g115(.a(new_n159_), .b(x32), .c(new_n151_), .d(new_n110_), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n148_), .c(new_n132_), .O(new_n161_));
  inv1   g117(.a(x00), .O(new_n162_));
  nor2   g118(.a(x26), .b(new_n162_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n161_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n58_), .O(z8));
  inv1   g121(.a(new_n144_), .O(new_n166_));
  nand2  g122(.a(new_n127_), .b(new_n101_), .O(new_n167_));
  aoi22  g123(.a(new_n167_), .b(new_n145_), .c(new_n127_), .d(x30), .O(new_n168_));
  aoi21  g124(.a(new_n168_), .b(new_n166_), .c(x33), .O(new_n169_));
  nor3   g125(.a(new_n157_), .b(new_n156_), .c(new_n50_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n155_), .O(new_n171_));
  inv1   g127(.a(new_n171_), .O(new_n172_));
  oai21  g128(.a(new_n127_), .b(new_n101_), .c(x30), .O(new_n173_));
  inv1   g129(.a(new_n154_), .O(new_n174_));
  aoi22  g130(.a(new_n167_), .b(new_n149_), .c(new_n174_), .d(new_n134_), .O(new_n175_));
  nand4  g131(.a(new_n175_), .b(new_n173_), .c(new_n172_), .d(new_n153_), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n163_), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n169_), .c(new_n58_), .O(z9));
endmodule


