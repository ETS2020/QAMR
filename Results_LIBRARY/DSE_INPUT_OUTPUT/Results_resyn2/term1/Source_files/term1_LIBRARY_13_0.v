// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  nor2   g000(.a(x31), .b(x11), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xor2a  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  aoi21  g005(.a(new_n48_), .b(new_n49_), .c(new_n45_), .O(new_n50_));
  oai21  g006(.a(new_n48_), .b(x33), .c(new_n50_), .O(z1));
  inv1   g007(.a(x08), .O(new_n52_));
  xor2a  g008(.a(x07), .b(x04), .O(new_n53_));
  nand2  g009(.a(new_n48_), .b(x01), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nand2  g011(.a(x05), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  xor2a  g014(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(x01), .c(new_n54_), .O(new_n60_));
  xor2a  g016(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nor4   g017(.a(new_n61_), .b(new_n45_), .c(x09), .d(new_n52_), .O(z2));
  nand2  g018(.a(x03), .b(x02), .O(new_n63_));
  nor2   g019(.a(x20), .b(x15), .O(new_n64_));
  nor2   g020(.a(x24), .b(x19), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g022(.a(x18), .O(new_n67_));
  inv1   g023(.a(x23), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nor2   g025(.a(x22), .b(x17), .O(new_n70_));
  nor2   g026(.a(x21), .b(x16), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g028(.a(new_n72_), .b(new_n69_), .c(new_n66_), .O(new_n73_));
  and2   g029(.a(new_n73_), .b(new_n63_), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  inv1   g031(.a(x27), .O(new_n76_));
  inv1   g032(.a(x26), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x25), .c(x01), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n75_), .c(new_n46_), .O(z3));
  nand3  g037(.a(new_n79_), .b(new_n74_), .c(new_n46_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n76_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n76_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n86_), .c(new_n83_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z4));
  nand2  g045(.a(new_n85_), .b(x29), .O(new_n90_));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n90_), .c(new_n83_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(z5));
  inv1   g050(.a(x30), .O(new_n95_));
  nand2  g051(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  inv1   g052(.a(new_n90_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(x30), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n96_), .c(new_n83_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z6));
  inv1   g056(.a(x31), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(x11), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n98_), .O(new_n103_));
  nand3  g059(.a(new_n97_), .b(new_n101_), .c(x30), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n103_), .c(new_n79_), .d(new_n74_), .O(z7));
  inv1   g061(.a(x15), .O(new_n106_));
  inv1   g062(.a(x17), .O(new_n107_));
  inv1   g063(.a(x19), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x18), .c(x14), .O(new_n109_));
  nand3  g065(.a(x19), .b(new_n67_), .c(x13), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n109_), .c(new_n107_), .O(new_n111_));
  nand4  g067(.a(x19), .b(x18), .c(new_n107_), .d(x12), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(x16), .O(new_n114_));
  nand3  g070(.a(x19), .b(x18), .c(x17), .O(new_n115_));
  or2    g071(.a(new_n115_), .b(x16), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n114_), .c(new_n106_), .O(new_n117_));
  nand2  g073(.a(x16), .b(x10), .O(new_n118_));
  nor3   g074(.a(new_n118_), .b(new_n115_), .c(x15), .O(new_n119_));
  nor2   g075(.a(new_n119_), .b(x11), .O(new_n120_));
  aoi21  g076(.a(new_n84_), .b(new_n76_), .c(new_n91_), .O(new_n121_));
  xor2a  g077(.a(new_n121_), .b(new_n95_), .O(new_n122_));
  aoi21  g078(.a(new_n120_), .b(new_n114_), .c(new_n122_), .O(new_n123_));
  oai21  g079(.a(new_n119_), .b(new_n117_), .c(new_n123_), .O(new_n124_));
  nand2  g080(.a(x18), .b(x17), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand2  g082(.a(x18), .b(x12), .O(new_n127_));
  oai21  g083(.a(x18), .b(x13), .c(x17), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  aoi22  g085(.a(new_n129_), .b(x16), .c(new_n126_), .d(x11), .O(new_n130_));
  nand3  g086(.a(new_n126_), .b(x16), .c(x15), .O(new_n131_));
  aoi21  g087(.a(new_n108_), .b(x14), .c(new_n131_), .O(new_n132_));
  oai21  g088(.a(new_n118_), .b(new_n125_), .c(new_n106_), .O(new_n133_));
  nand2  g089(.a(new_n131_), .b(new_n108_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nor3   g091(.a(new_n135_), .b(new_n132_), .c(new_n130_), .O(new_n136_));
  nor3   g092(.a(new_n122_), .b(new_n49_), .c(new_n101_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g094(.a(new_n77_), .b(x00), .O(new_n139_));
  aoi21  g095(.a(x32), .b(x11), .c(x31), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n138_), .O(new_n142_));
  aoi21  g098(.a(new_n124_), .b(new_n49_), .c(new_n142_), .O(z8));
  inv1   g099(.a(x33), .O(new_n144_));
  nand2  g100(.a(new_n116_), .b(new_n114_), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(x15), .c(new_n119_), .O(new_n146_));
  inv1   g102(.a(new_n121_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x30), .O(new_n148_));
  nand3  g104(.a(new_n121_), .b(new_n120_), .c(new_n114_), .O(new_n149_));
  nand2  g105(.a(new_n147_), .b(x31), .O(new_n150_));
  oai21  g106(.a(new_n101_), .b(new_n95_), .c(new_n121_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n150_), .c(new_n149_), .d(new_n148_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n146_), .c(new_n144_), .O(new_n153_));
  nand4  g109(.a(new_n151_), .b(new_n150_), .c(new_n148_), .d(x33), .O(new_n154_));
  inv1   g110(.a(new_n154_), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n136_), .c(new_n139_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n46_), .O(z9));
endmodule


