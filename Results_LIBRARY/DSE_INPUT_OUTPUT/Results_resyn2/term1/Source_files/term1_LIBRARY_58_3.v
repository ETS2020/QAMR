// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  nand2  g000(.a(x30), .b(x21), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x32), .O(new_n48_));
  xor2a  g004(.a(x03), .b(x02), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  or2    g006(.a(new_n49_), .b(x33), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n50_), .c(new_n45_), .O(z1));
  xor2a  g008(.a(x07), .b(x04), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nand2  g010(.a(x05), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(x02), .O(new_n56_));
  nand2  g012(.a(x06), .b(new_n56_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand4  g014(.a(x06), .b(x05), .c(new_n54_), .d(new_n56_), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  aoi21  g016(.a(new_n49_), .b(x01), .c(new_n60_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  nand2  g019(.a(new_n61_), .b(new_n53_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n63_), .c(x08), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n62_), .c(new_n45_), .O(z2));
  inv1   g022(.a(x27), .O(new_n67_));
  oai22  g023(.a(x24), .b(x19), .c(x20), .d(x15), .O(new_n68_));
  oai22  g024(.a(x23), .b(x18), .c(x22), .d(x17), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  oai21  g026(.a(x21), .b(x16), .c(new_n70_), .O(new_n71_));
  inv1   g027(.a(x26), .O(new_n72_));
  nand2  g028(.a(x03), .b(x02), .O(new_n73_));
  nand4  g029(.a(new_n73_), .b(new_n72_), .c(x25), .d(x01), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  and2   g031(.a(new_n75_), .b(new_n71_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n45_), .c(new_n67_), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(z3));
  inv1   g034(.a(x28), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n67_), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n79_), .b(new_n67_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n81_), .c(new_n76_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n45_), .O(z4));
  nand2  g040(.a(new_n80_), .b(x29), .O(new_n85_));
  inv1   g041(.a(x29), .O(new_n86_));
  nand2  g042(.a(new_n81_), .b(new_n86_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n85_), .c(new_n76_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n45_), .O(z5));
  nand2  g045(.a(new_n70_), .b(x16), .O(new_n90_));
  inv1   g046(.a(x30), .O(new_n91_));
  inv1   g047(.a(new_n85_), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g049(.a(new_n80_), .b(new_n91_), .c(x29), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(new_n95_));
  aoi22  g051(.a(new_n95_), .b(new_n71_), .c(new_n93_), .d(new_n90_), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n74_), .c(new_n45_), .O(z6));
  nand2  g053(.a(new_n92_), .b(x30), .O(new_n98_));
  xor2a  g054(.a(new_n98_), .b(x31), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n76_), .c(new_n45_), .O(z7));
  nand2  g056(.a(new_n72_), .b(x00), .O(new_n101_));
  inv1   g057(.a(x15), .O(new_n102_));
  inv1   g058(.a(x17), .O(new_n103_));
  inv1   g059(.a(x19), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(x18), .c(x14), .O(new_n105_));
  inv1   g061(.a(x18), .O(new_n106_));
  nand3  g062(.a(x19), .b(new_n106_), .c(x13), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  nand4  g064(.a(x19), .b(x18), .c(new_n103_), .d(x12), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n108_), .c(x16), .O(new_n111_));
  inv1   g067(.a(x16), .O(new_n112_));
  nand3  g068(.a(x19), .b(x18), .c(x17), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n112_), .c(x11), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n111_), .c(new_n102_), .O(new_n116_));
  nand3  g072(.a(x16), .b(new_n102_), .c(x10), .O(new_n117_));
  nor2   g073(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  oai21  g074(.a(x28), .b(x27), .c(x29), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n91_), .c(x31), .O(new_n120_));
  inv1   g076(.a(new_n119_), .O(new_n121_));
  nor2   g077(.a(new_n91_), .b(x21), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n121_), .c(new_n48_), .O(new_n123_));
  nor2   g079(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  oai21  g080(.a(new_n118_), .b(new_n116_), .c(new_n124_), .O(new_n125_));
  nand4  g081(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  nand3  g083(.a(x18), .b(x17), .c(x11), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n112_), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(x19), .c(new_n127_), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  nand2  g087(.a(x17), .b(x13), .O(new_n132_));
  oai21  g088(.a(x17), .b(x12), .c(x18), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g090(.a(new_n104_), .b(x14), .O(new_n135_));
  nand2  g091(.a(new_n127_), .b(new_n135_), .O(new_n136_));
  nand4  g092(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n102_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n136_), .c(new_n134_), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n131_), .O(new_n141_));
  nor2   g097(.a(new_n121_), .b(x30), .O(new_n142_));
  or2    g098(.a(new_n142_), .b(new_n120_), .O(new_n143_));
  nor2   g099(.a(new_n46_), .b(new_n48_), .O(new_n144_));
  oai21  g100(.a(new_n143_), .b(new_n141_), .c(new_n144_), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n125_), .c(new_n101_), .O(z8));
  oai21  g102(.a(new_n139_), .b(new_n130_), .c(new_n45_), .O(new_n147_));
  inv1   g103(.a(x31), .O(new_n148_));
  nand2  g104(.a(new_n142_), .b(new_n148_), .O(new_n149_));
  nor2   g105(.a(new_n119_), .b(new_n148_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(x21), .c(x30), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n147_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(x33), .O(new_n154_));
  nand2  g110(.a(new_n150_), .b(new_n122_), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n149_), .c(x33), .O(new_n156_));
  oai21  g112(.a(new_n118_), .b(new_n116_), .c(new_n156_), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(new_n154_), .c(new_n101_), .O(z9));
endmodule


