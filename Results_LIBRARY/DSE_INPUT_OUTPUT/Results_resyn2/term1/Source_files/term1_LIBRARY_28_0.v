// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  oai21  g000(.a(x26), .b(x13), .c(x32), .O(z0));
  xnor2a g001(.a(x03), .b(x02), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  nor2   g004(.a(x26), .b(x13), .O(new_n49_));
  aoi21  g005(.a(new_n47_), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n47_), .b(x33), .c(new_n50_), .O(z1));
  inv1   g007(.a(x08), .O(new_n52_));
  inv1   g008(.a(x01), .O(new_n53_));
  aoi21  g009(.a(x03), .b(x02), .c(new_n53_), .O(new_n54_));
  oai21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  inv1   g011(.a(x02), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n57_), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(x06), .c(new_n56_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n56_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x05), .c(new_n57_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(new_n59_), .c(new_n53_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  xor2a  g019(.a(x07), .b(x04), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor4   g021(.a(new_n65_), .b(new_n49_), .c(x09), .d(new_n52_), .O(z2));
  inv1   g022(.a(x27), .O(new_n67_));
  nand2  g023(.a(new_n54_), .b(x25), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(new_n69_));
  nor2   g025(.a(x23), .b(x18), .O(new_n70_));
  nor2   g026(.a(x22), .b(x17), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x19), .O(new_n73_));
  inv1   g029(.a(x24), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(x20), .b(x15), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n69_), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n67_), .O(new_n82_));
  aoi21  g038(.a(new_n82_), .b(x13), .c(x26), .O(z3));
  inv1   g039(.a(x26), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x13), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n68_), .O(new_n86_));
  inv1   g042(.a(x28), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n67_), .O(new_n88_));
  nor2   g044(.a(new_n87_), .b(new_n67_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n88_), .c(new_n86_), .d(new_n79_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z4));
  nand2  g048(.a(new_n89_), .b(x29), .O(new_n93_));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n90_), .b(new_n94_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n93_), .c(new_n86_), .d(new_n79_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(z5));
  inv1   g053(.a(x30), .O(new_n98_));
  xor2a  g054(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n81_), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(x13), .c(x26), .O(z6));
  xor2a  g057(.a(x31), .b(x30), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  nand2  g059(.a(new_n93_), .b(x31), .O(new_n104_));
  oai21  g060(.a(new_n103_), .b(new_n93_), .c(new_n104_), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n80_), .c(x13), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n84_), .O(z7));
  nand2  g063(.a(x18), .b(x14), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n73_), .O(new_n109_));
  nand3  g065(.a(x19), .b(x18), .c(x12), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  aoi21  g067(.a(new_n109_), .b(x17), .c(new_n111_), .O(new_n112_));
  nand3  g068(.a(x19), .b(x18), .c(x17), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x16), .O(new_n114_));
  inv1   g070(.a(x16), .O(new_n115_));
  and2   g071(.a(x17), .b(x11), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(x19), .c(x18), .d(new_n115_), .O(new_n117_));
  oai21  g073(.a(new_n114_), .b(new_n112_), .c(new_n117_), .O(new_n118_));
  inv1   g074(.a(x15), .O(new_n119_));
  nand3  g075(.a(x16), .b(new_n119_), .c(x10), .O(new_n120_));
  nor2   g076(.a(new_n120_), .b(new_n113_), .O(new_n121_));
  aoi21  g077(.a(new_n118_), .b(x15), .c(new_n121_), .O(new_n122_));
  nand2  g078(.a(new_n88_), .b(x29), .O(new_n123_));
  xor2a  g079(.a(new_n123_), .b(new_n98_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x31), .O(new_n125_));
  or2    g081(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand2  g082(.a(x19), .b(x18), .O(new_n127_));
  nor2   g083(.a(x19), .b(x18), .O(new_n128_));
  nand3  g084(.a(x17), .b(x16), .c(x15), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(new_n127_), .c(new_n128_), .O(new_n130_));
  nand3  g086(.a(x17), .b(x16), .c(x10), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n119_), .O(new_n132_));
  inv1   g088(.a(new_n129_), .O(new_n133_));
  nand2  g089(.a(new_n73_), .b(x14), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n133_), .c(x18), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n132_), .c(new_n130_), .O(new_n136_));
  aoi21  g092(.a(x17), .b(x11), .c(x16), .O(new_n137_));
  nor2   g093(.a(x17), .b(x12), .O(new_n138_));
  nand2  g094(.a(x32), .b(x31), .O(new_n139_));
  nor3   g095(.a(new_n139_), .b(new_n138_), .c(new_n137_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n124_), .O(new_n141_));
  nand3  g097(.a(new_n84_), .b(x13), .c(x00), .O(new_n142_));
  inv1   g098(.a(new_n142_), .O(new_n143_));
  oai21  g099(.a(new_n141_), .b(new_n136_), .c(new_n143_), .O(new_n144_));
  aoi21  g100(.a(new_n126_), .b(new_n48_), .c(new_n144_), .O(z8));
  inv1   g101(.a(x33), .O(new_n146_));
  inv1   g102(.a(new_n124_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n103_), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n122_), .c(new_n146_), .O(new_n149_));
  inv1   g105(.a(x00), .O(new_n150_));
  nor2   g106(.a(new_n124_), .b(new_n102_), .O(new_n151_));
  nor3   g107(.a(new_n138_), .b(new_n137_), .c(new_n146_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n135_), .c(new_n132_), .d(new_n130_), .O(new_n153_));
  inv1   g109(.a(new_n153_), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n151_), .c(new_n150_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n149_), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(x13), .c(x26), .O(z9));
endmodule


