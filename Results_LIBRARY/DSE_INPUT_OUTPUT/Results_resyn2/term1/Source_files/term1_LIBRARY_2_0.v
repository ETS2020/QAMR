// Benchmark "FAU" written by ABC on Tue Aug 11 21:12:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x25), .O(new_n45_));
  inv1   g001(.a(x26), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x33), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  inv1   g008(.a(new_n50_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n51_), .c(new_n47_), .O(z1));
  xor2a  g011(.a(x07), .b(x04), .O(new_n56_));
  inv1   g012(.a(x01), .O(new_n57_));
  inv1   g013(.a(x03), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n58_), .O(new_n59_));
  inv1   g015(.a(x02), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n60_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n59_), .c(x01), .O(new_n62_));
  oai21  g018(.a(new_n61_), .b(new_n59_), .c(new_n62_), .O(new_n63_));
  oai21  g019(.a(new_n53_), .b(new_n57_), .c(new_n63_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(x08), .O(new_n66_));
  aoi21  g022(.a(new_n64_), .b(new_n56_), .c(new_n66_), .O(new_n67_));
  oai21  g023(.a(new_n64_), .b(new_n56_), .c(new_n67_), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n47_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  nor3   g026(.a(x26), .b(new_n45_), .c(new_n57_), .O(new_n71_));
  nand2  g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(x23), .b(x18), .O(new_n73_));
  nor2   g029(.a(x22), .b(x17), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g031(.a(x19), .O(new_n76_));
  inv1   g032(.a(x24), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor2   g034(.a(x21), .b(x16), .O(new_n79_));
  nor2   g035(.a(x20), .b(x15), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n78_), .c(new_n75_), .O(new_n82_));
  and2   g038(.a(new_n82_), .b(new_n72_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n71_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n70_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(z3));
  nand2  g043(.a(new_n83_), .b(x01), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  inv1   g045(.a(x28), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n70_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n90_), .b(new_n70_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n92_), .c(new_n89_), .O(new_n94_));
  aoi21  g050(.a(new_n94_), .b(x25), .c(x26), .O(z4));
  inv1   g051(.a(new_n83_), .O(new_n96_));
  nand2  g052(.a(new_n91_), .b(x29), .O(new_n97_));
  inv1   g053(.a(x29), .O(new_n98_));
  nand2  g054(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n97_), .c(new_n71_), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n96_), .O(z5));
  nand3  g057(.a(new_n91_), .b(x30), .c(x29), .O(new_n102_));
  inv1   g058(.a(x30), .O(new_n103_));
  nand2  g059(.a(new_n97_), .b(new_n103_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n102_), .c(new_n89_), .O(new_n105_));
  aoi21  g061(.a(new_n105_), .b(x25), .c(x26), .O(z6));
  xor2a  g062(.a(new_n102_), .b(x31), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n85_), .O(z7));
  inv1   g064(.a(x15), .O(new_n109_));
  inv1   g065(.a(x17), .O(new_n110_));
  nand3  g066(.a(new_n76_), .b(x18), .c(x14), .O(new_n111_));
  inv1   g067(.a(x18), .O(new_n112_));
  nand3  g068(.a(x19), .b(new_n112_), .c(x13), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  nand4  g070(.a(x19), .b(x18), .c(new_n110_), .d(x12), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n114_), .c(x16), .O(new_n117_));
  inv1   g073(.a(x16), .O(new_n118_));
  nand3  g074(.a(x18), .b(x17), .c(x11), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x19), .c(new_n118_), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n117_), .c(new_n109_), .O(new_n122_));
  nor2   g078(.a(new_n112_), .b(new_n110_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x16), .c(x10), .O(new_n124_));
  nor3   g080(.a(new_n124_), .b(new_n76_), .c(x15), .O(new_n125_));
  inv1   g081(.a(x31), .O(new_n126_));
  nor2   g082(.a(x32), .b(new_n126_), .O(new_n127_));
  oai21  g083(.a(new_n125_), .b(new_n122_), .c(new_n127_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n103_), .O(new_n129_));
  aoi21  g085(.a(new_n90_), .b(new_n70_), .c(new_n98_), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  aoi21  g087(.a(new_n52_), .b(x30), .c(new_n131_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  nand2  g089(.a(new_n128_), .b(x30), .O(new_n134_));
  aoi21  g090(.a(new_n52_), .b(new_n103_), .c(new_n130_), .O(new_n135_));
  oai21  g091(.a(new_n120_), .b(x16), .c(x15), .O(new_n136_));
  inv1   g092(.a(x14), .O(new_n137_));
  nor2   g093(.a(x19), .b(new_n137_), .O(new_n138_));
  nand3  g094(.a(new_n123_), .b(x16), .c(x15), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n76_), .O(new_n140_));
  oai21  g096(.a(new_n139_), .b(new_n138_), .c(new_n140_), .O(new_n141_));
  aoi21  g097(.a(new_n136_), .b(new_n124_), .c(new_n141_), .O(new_n142_));
  aoi21  g098(.a(x18), .b(x12), .c(x17), .O(new_n143_));
  nor2   g099(.a(x18), .b(x13), .O(new_n144_));
  nor3   g100(.a(new_n144_), .b(new_n143_), .c(new_n126_), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n142_), .c(new_n52_), .O(new_n146_));
  aoi21  g102(.a(new_n135_), .b(new_n134_), .c(new_n146_), .O(new_n147_));
  nand3  g103(.a(new_n46_), .b(x25), .c(x00), .O(new_n148_));
  aoi21  g104(.a(new_n147_), .b(new_n133_), .c(new_n148_), .O(z8));
  nand2  g105(.a(x31), .b(x30), .O(new_n150_));
  aoi21  g106(.a(new_n126_), .b(new_n103_), .c(new_n130_), .O(new_n151_));
  aoi21  g107(.a(new_n150_), .b(new_n130_), .c(new_n151_), .O(new_n152_));
  oai21  g108(.a(new_n125_), .b(new_n122_), .c(new_n152_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n49_), .O(new_n154_));
  nor3   g110(.a(new_n144_), .b(new_n143_), .c(new_n49_), .O(new_n155_));
  nand3  g111(.a(new_n155_), .b(new_n152_), .c(new_n142_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n154_), .c(x00), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(x25), .c(x26), .O(z9));
endmodule


