// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(x21), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  aoi21  g007(.a(new_n50_), .b(new_n51_), .c(new_n46_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(x33), .c(new_n52_), .O(z1));
  xor2a  g009(.a(x07), .b(x04), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nand2  g011(.a(x05), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  aoi21  g014(.a(new_n58_), .b(new_n56_), .c(x01), .O(new_n59_));
  oai21  g015(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n49_), .b(x01), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x08), .O(new_n64_));
  aoi21  g020(.a(new_n62_), .b(new_n54_), .c(new_n64_), .O(new_n65_));
  oai21  g021(.a(new_n62_), .b(new_n54_), .c(new_n65_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n47_), .O(z2));
  nor2   g023(.a(new_n55_), .b(new_n57_), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(new_n69_));
  nor2   g025(.a(x22), .b(x17), .O(new_n70_));
  oai22  g026(.a(x24), .b(x19), .c(x20), .d(x15), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x23), .O(new_n73_));
  nor2   g029(.a(x21), .b(x16), .O(new_n74_));
  aoi21  g030(.a(new_n73_), .b(new_n45_), .c(new_n74_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  inv1   g033(.a(x27), .O(new_n78_));
  inv1   g034(.a(x26), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x25), .c(x01), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n77_), .c(new_n47_), .O(z3));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n78_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n78_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n86_), .c(new_n81_), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n77_), .c(new_n47_), .O(z4));
  oai21  g045(.a(new_n74_), .b(new_n73_), .c(new_n45_), .O(new_n90_));
  nand3  g046(.a(new_n81_), .b(new_n69_), .c(new_n47_), .O(new_n91_));
  aoi21  g047(.a(new_n90_), .b(new_n72_), .c(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n85_), .b(x29), .O(new_n93_));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n86_), .b(new_n94_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n93_), .c(new_n92_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(z5));
  inv1   g053(.a(x30), .O(new_n98_));
  xor2a  g054(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n92_), .O(new_n100_));
  inv1   g056(.a(new_n100_), .O(z6));
  nand2  g057(.a(new_n93_), .b(x31), .O(new_n102_));
  inv1   g058(.a(x31), .O(new_n103_));
  nor2   g059(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  nor2   g060(.a(x31), .b(x30), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n85_), .c(x29), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n102_), .c(new_n92_), .O(z7));
  nand2  g064(.a(new_n79_), .b(x00), .O(new_n109_));
  inv1   g065(.a(x21), .O(new_n110_));
  inv1   g066(.a(x15), .O(new_n111_));
  inv1   g067(.a(x16), .O(new_n112_));
  inv1   g068(.a(x17), .O(new_n113_));
  nor2   g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n111_), .c(x10), .O(new_n115_));
  inv1   g071(.a(x19), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x14), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand3  g074(.a(x18), .b(x17), .c(x16), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n118_), .c(x15), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n115_), .O(new_n121_));
  nand2  g077(.a(new_n114_), .b(x15), .O(new_n122_));
  nand2  g078(.a(x17), .b(x11), .O(new_n123_));
  oai21  g079(.a(x17), .b(x12), .c(x16), .O(new_n124_));
  aoi22  g080(.a(new_n124_), .b(new_n123_), .c(new_n122_), .d(new_n116_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n121_), .c(new_n110_), .O(new_n126_));
  inv1   g082(.a(new_n122_), .O(new_n127_));
  and2   g083(.a(x19), .b(x13), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n45_), .O(new_n130_));
  nand2  g086(.a(new_n87_), .b(x29), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(x30), .c(x31), .O(new_n133_));
  aoi21  g089(.a(new_n132_), .b(x30), .c(new_n133_), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n46_), .c(new_n130_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n126_), .c(x32), .O(new_n136_));
  nand3  g092(.a(new_n118_), .b(x21), .c(x18), .O(new_n137_));
  nand2  g093(.a(new_n128_), .b(new_n45_), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n137_), .c(new_n113_), .O(new_n139_));
  nand3  g095(.a(x21), .b(x19), .c(x18), .O(new_n140_));
  nand2  g096(.a(new_n113_), .b(x12), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n139_), .c(x16), .O(new_n143_));
  inv1   g099(.a(new_n140_), .O(new_n144_));
  nor2   g100(.a(new_n123_), .b(x16), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(new_n143_), .c(new_n111_), .O(new_n147_));
  nor2   g103(.a(new_n140_), .b(new_n115_), .O(new_n148_));
  nand2  g104(.a(new_n134_), .b(new_n51_), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  oai21  g106(.a(new_n148_), .b(new_n147_), .c(new_n150_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n136_), .c(new_n109_), .O(z8));
  nand2  g108(.a(x33), .b(x30), .O(new_n153_));
  inv1   g109(.a(x33), .O(new_n154_));
  nand4  g110(.a(new_n128_), .b(new_n127_), .c(new_n105_), .d(new_n154_), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n153_), .c(x18), .O(new_n156_));
  nand4  g112(.a(x19), .b(x17), .c(new_n112_), .d(x11), .O(new_n157_));
  inv1   g113(.a(new_n157_), .O(new_n158_));
  oai21  g114(.a(x19), .b(x17), .c(x16), .O(new_n159_));
  aoi21  g115(.a(new_n141_), .b(new_n117_), .c(new_n159_), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n158_), .c(x15), .O(new_n161_));
  nand4  g117(.a(new_n114_), .b(x19), .c(new_n111_), .d(x10), .O(new_n162_));
  aoi21  g118(.a(new_n162_), .b(new_n161_), .c(x33), .O(new_n163_));
  nand3  g119(.a(new_n163_), .b(new_n105_), .c(x18), .O(new_n164_));
  aoi21  g120(.a(new_n164_), .b(new_n153_), .c(new_n110_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n156_), .c(new_n131_), .O(new_n166_));
  nor2   g122(.a(new_n154_), .b(x30), .O(new_n167_));
  nand4  g123(.a(new_n128_), .b(new_n127_), .c(new_n104_), .d(new_n154_), .O(new_n168_));
  inv1   g124(.a(new_n168_), .O(new_n169_));
  oai21  g125(.a(new_n169_), .b(new_n167_), .c(new_n45_), .O(new_n170_));
  nand2  g126(.a(new_n104_), .b(x18), .O(new_n171_));
  inv1   g127(.a(new_n171_), .O(new_n172_));
  aoi21  g128(.a(new_n172_), .b(new_n163_), .c(new_n167_), .O(new_n173_));
  oai21  g129(.a(new_n173_), .b(new_n110_), .c(new_n170_), .O(new_n174_));
  oai21  g130(.a(new_n129_), .b(new_n106_), .c(new_n45_), .O(new_n175_));
  nand2  g131(.a(new_n125_), .b(new_n121_), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n106_), .c(x21), .O(new_n177_));
  aoi21  g133(.a(new_n177_), .b(new_n175_), .c(new_n154_), .O(new_n178_));
  aoi21  g134(.a(new_n174_), .b(new_n132_), .c(new_n178_), .O(new_n179_));
  aoi21  g135(.a(new_n179_), .b(new_n166_), .c(new_n109_), .O(z9));
endmodule


