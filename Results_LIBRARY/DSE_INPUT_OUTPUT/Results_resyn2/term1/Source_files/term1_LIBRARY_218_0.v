// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  nor2   g000(.a(x18), .b(x17), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  inv1   g008(.a(x32), .O(new_n53_));
  aoi21  g009(.a(x33), .b(new_n48_), .c(x02), .O(new_n54_));
  oai21  g010(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n45_), .O(z1));
  inv1   g012(.a(x01), .O(new_n57_));
  aoi21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  oai21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n48_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x06), .c(new_n50_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n50_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x05), .c(new_n48_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  xnor2a g021(.a(x07), .b(x04), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(x08), .O(new_n68_));
  aoi21  g024(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  oai21  g025(.a(new_n66_), .b(new_n65_), .c(new_n69_), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n46_), .O(z2));
  inv1   g027(.a(x15), .O(new_n72_));
  inv1   g028(.a(x20), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(x24), .b(x19), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  oai22  g034(.a(x23), .b(x18), .c(x22), .d(x17), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  inv1   g036(.a(x27), .O(new_n81_));
  inv1   g037(.a(x26), .O(new_n82_));
  nand3  g038(.a(new_n58_), .b(new_n82_), .c(x25), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n80_), .c(new_n46_), .O(z3));
  nor2   g042(.a(new_n80_), .b(new_n45_), .O(new_n87_));
  inv1   g043(.a(x28), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n81_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n88_), .b(new_n81_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n90_), .c(new_n87_), .d(new_n84_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z4));
  nand2  g049(.a(new_n89_), .b(x29), .O(new_n94_));
  inv1   g050(.a(x29), .O(new_n95_));
  nand2  g051(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n94_), .c(new_n84_), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n80_), .c(new_n46_), .O(z5));
  inv1   g054(.a(x30), .O(new_n99_));
  nand2  g055(.a(new_n94_), .b(new_n99_), .O(new_n100_));
  nand3  g056(.a(new_n89_), .b(x30), .c(x29), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n100_), .c(new_n84_), .O(new_n102_));
  nor3   g058(.a(new_n102_), .b(new_n80_), .c(new_n45_), .O(z6));
  nand2  g059(.a(x23), .b(x17), .O(new_n104_));
  oai21  g060(.a(x22), .b(x17), .c(x18), .O(new_n105_));
  and2   g061(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nor2   g062(.a(new_n101_), .b(x31), .O(new_n107_));
  nand2  g063(.a(new_n101_), .b(x31), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n84_), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n107_), .c(new_n46_), .O(new_n110_));
  oai21  g066(.a(new_n106_), .b(new_n78_), .c(new_n110_), .O(z7));
  inv1   g067(.a(x17), .O(new_n112_));
  nand4  g068(.a(x19), .b(x18), .c(new_n112_), .d(x12), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  inv1   g070(.a(x18), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x13), .O(new_n116_));
  inv1   g072(.a(x19), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x14), .O(new_n118_));
  oai21  g074(.a(x19), .b(x18), .c(x17), .O(new_n119_));
  aoi21  g075(.a(new_n118_), .b(new_n116_), .c(new_n119_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n114_), .c(x16), .O(new_n121_));
  inv1   g077(.a(x16), .O(new_n122_));
  nand3  g078(.a(x19), .b(new_n122_), .c(x11), .O(new_n123_));
  nand2  g079(.a(x18), .b(x17), .O(new_n124_));
  or2    g080(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n121_), .c(new_n72_), .O(new_n126_));
  nand2  g082(.a(x16), .b(x10), .O(new_n127_));
  nor2   g083(.a(new_n117_), .b(new_n115_), .O(new_n128_));
  nor2   g084(.a(new_n112_), .b(x15), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  oai21  g087(.a(x28), .b(x27), .c(x29), .O(new_n132_));
  xor2a  g088(.a(new_n132_), .b(new_n99_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n53_), .c(x31), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n131_), .b(new_n126_), .c(new_n135_), .O(new_n136_));
  nand4  g092(.a(x19), .b(x16), .c(x15), .d(x12), .O(new_n137_));
  nand3  g093(.a(x18), .b(x16), .c(x15), .O(new_n138_));
  nand3  g094(.a(x18), .b(x16), .c(x10), .O(new_n139_));
  aoi22  g095(.a(new_n139_), .b(new_n72_), .c(new_n138_), .d(new_n117_), .O(new_n140_));
  inv1   g096(.a(new_n138_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n118_), .O(new_n142_));
  aoi21  g098(.a(x18), .b(x11), .c(x16), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  inv1   g100(.a(x13), .O(new_n145_));
  aoi21  g101(.a(new_n115_), .b(new_n145_), .c(new_n112_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n144_), .c(new_n142_), .d(new_n140_), .O(new_n147_));
  oai21  g103(.a(new_n137_), .b(x17), .c(new_n147_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n133_), .c(x31), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n46_), .c(x32), .O(new_n150_));
  nand2  g106(.a(new_n82_), .b(x00), .O(new_n151_));
  aoi21  g107(.a(new_n150_), .b(new_n136_), .c(new_n151_), .O(z8));
  inv1   g108(.a(new_n131_), .O(new_n153_));
  nand3  g109(.a(new_n117_), .b(x16), .c(x14), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n123_), .c(new_n124_), .O(new_n155_));
  nand2  g111(.a(new_n112_), .b(x12), .O(new_n156_));
  nand2  g112(.a(x19), .b(x16), .O(new_n157_));
  aoi21  g113(.a(new_n116_), .b(new_n156_), .c(new_n157_), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n155_), .c(x15), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n153_), .O(new_n160_));
  inv1   g116(.a(new_n133_), .O(new_n161_));
  xnor2a g117(.a(x31), .b(x30), .O(new_n162_));
  and2   g118(.a(new_n162_), .b(new_n161_), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(new_n160_), .c(x33), .O(new_n164_));
  inv1   g120(.a(new_n151_), .O(new_n165_));
  nand2  g121(.a(new_n137_), .b(new_n112_), .O(new_n166_));
  aoi21  g122(.a(new_n115_), .b(new_n145_), .c(new_n49_), .O(new_n167_));
  nand4  g123(.a(new_n167_), .b(new_n166_), .c(new_n162_), .d(new_n144_), .O(new_n168_));
  nand3  g124(.a(new_n141_), .b(new_n118_), .c(x17), .O(new_n169_));
  nand3  g125(.a(new_n169_), .b(new_n140_), .c(new_n161_), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n168_), .c(new_n165_), .O(new_n171_));
  oai21  g127(.a(new_n171_), .b(new_n164_), .c(new_n46_), .O(z9));
endmodule


