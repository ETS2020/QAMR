// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_;
  inv1   g000(.a(x31), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x12), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  aoi21  g007(.a(new_n50_), .b(new_n51_), .c(new_n46_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(x33), .c(new_n52_), .O(z1));
  inv1   g009(.a(x08), .O(new_n54_));
  inv1   g010(.a(x01), .O(new_n55_));
  aoi21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  oai21  g012(.a(x03), .b(x02), .c(new_n56_), .O(new_n57_));
  inv1   g013(.a(x03), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(new_n64_));
  xor2a  g020(.a(x07), .b(x04), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nor4   g022(.a(new_n66_), .b(new_n46_), .c(x09), .d(new_n54_), .O(z2));
  nor2   g023(.a(x23), .b(x18), .O(new_n68_));
  nor2   g024(.a(x22), .b(x17), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g026(.a(x19), .O(new_n71_));
  inv1   g027(.a(x24), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g029(.a(x21), .b(x16), .O(new_n74_));
  nor2   g030(.a(x20), .b(x15), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(new_n77_));
  inv1   g033(.a(x26), .O(new_n78_));
  nand3  g034(.a(new_n56_), .b(new_n78_), .c(x25), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(x27), .c(new_n47_), .O(z3));
  inv1   g038(.a(x27), .O(new_n83_));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n83_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n81_), .c(new_n47_), .O(z4));
  inv1   g045(.a(new_n81_), .O(new_n90_));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n85_), .b(x29), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n92_), .c(new_n90_), .d(new_n47_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z5));
  inv1   g051(.a(x30), .O(new_n96_));
  nand2  g052(.a(new_n93_), .b(new_n96_), .O(new_n97_));
  nand3  g053(.a(new_n85_), .b(x30), .c(x29), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n81_), .c(new_n47_), .O(z6));
  xor2a  g056(.a(new_n98_), .b(x31), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(new_n90_), .c(new_n46_), .O(z7));
  inv1   g058(.a(x16), .O(new_n103_));
  nand3  g059(.a(new_n71_), .b(x18), .c(x14), .O(new_n104_));
  inv1   g060(.a(x18), .O(new_n105_));
  nand3  g061(.a(x19), .b(new_n105_), .c(x13), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  inv1   g063(.a(x17), .O(new_n108_));
  nand3  g064(.a(x19), .b(x18), .c(new_n108_), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  aoi21  g066(.a(new_n107_), .b(x17), .c(new_n110_), .O(new_n111_));
  nand3  g067(.a(x19), .b(x18), .c(x17), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n103_), .c(x11), .O(new_n114_));
  oai21  g070(.a(new_n111_), .b(new_n103_), .c(new_n114_), .O(new_n115_));
  inv1   g071(.a(x15), .O(new_n116_));
  nand3  g072(.a(x16), .b(new_n116_), .c(x10), .O(new_n117_));
  nor2   g073(.a(new_n117_), .b(new_n112_), .O(new_n118_));
  aoi21  g074(.a(new_n115_), .b(x15), .c(new_n118_), .O(new_n119_));
  nand2  g075(.a(new_n87_), .b(x29), .O(new_n120_));
  xor2a  g076(.a(new_n120_), .b(new_n96_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x31), .O(new_n122_));
  or2    g078(.a(new_n122_), .b(new_n119_), .O(new_n123_));
  inv1   g079(.a(x00), .O(new_n124_));
  nor2   g080(.a(x26), .b(new_n124_), .O(new_n125_));
  nand3  g081(.a(x18), .b(x16), .c(x15), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x19), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n108_), .O(new_n129_));
  nand3  g085(.a(x18), .b(x16), .c(x10), .O(new_n130_));
  aoi22  g086(.a(new_n130_), .b(new_n116_), .c(new_n126_), .d(new_n71_), .O(new_n131_));
  nand2  g087(.a(x16), .b(x13), .O(new_n132_));
  oai21  g088(.a(x16), .b(x11), .c(x18), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g090(.a(new_n71_), .b(x14), .O(new_n135_));
  nand3  g091(.a(new_n127_), .b(new_n135_), .c(x17), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n134_), .c(new_n131_), .d(new_n129_), .O(new_n137_));
  nand2  g093(.a(new_n121_), .b(x32), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n137_), .c(x12), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x31), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n125_), .O(new_n141_));
  aoi21  g097(.a(new_n123_), .b(new_n51_), .c(new_n141_), .O(z8));
  nand4  g098(.a(new_n87_), .b(x31), .c(x30), .d(x29), .O(new_n143_));
  nand3  g099(.a(new_n120_), .b(new_n45_), .c(new_n96_), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  nor2   g101(.a(new_n128_), .b(x17), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  oai21  g103(.a(new_n143_), .b(new_n119_), .c(new_n147_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(x12), .O(new_n149_));
  aoi21  g105(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n150_));
  nor2   g106(.a(new_n71_), .b(new_n105_), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(new_n103_), .c(x11), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n150_), .c(x15), .O(new_n154_));
  inv1   g110(.a(new_n117_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand2  g112(.a(new_n145_), .b(x17), .O(new_n157_));
  aoi21  g113(.a(new_n156_), .b(new_n154_), .c(new_n157_), .O(new_n158_));
  nor2   g114(.a(new_n158_), .b(x33), .O(new_n159_));
  inv1   g115(.a(x33), .O(new_n160_));
  inv1   g116(.a(new_n137_), .O(new_n161_));
  nor2   g117(.a(x17), .b(x12), .O(new_n162_));
  oai21  g118(.a(new_n162_), .b(new_n144_), .c(new_n143_), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(new_n161_), .c(new_n46_), .O(new_n164_));
  oai21  g120(.a(new_n164_), .b(new_n160_), .c(new_n125_), .O(new_n165_));
  aoi21  g121(.a(new_n159_), .b(new_n149_), .c(new_n165_), .O(z9));
endmodule


