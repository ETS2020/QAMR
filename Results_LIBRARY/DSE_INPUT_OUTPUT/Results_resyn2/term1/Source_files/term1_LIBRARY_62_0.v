// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_;
  inv1   g000(.a(x18), .O(new_n45_));
  inv1   g001(.a(x23), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x33), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  inv1   g008(.a(new_n50_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n51_), .c(new_n47_), .O(z1));
  inv1   g011(.a(x01), .O(new_n56_));
  aoi21  g012(.a(x03), .b(x02), .c(new_n56_), .O(new_n57_));
  oai21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  inv1   g014(.a(x03), .O(new_n59_));
  inv1   g015(.a(x02), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n60_), .O(new_n61_));
  aoi21  g017(.a(x05), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand3  g018(.a(new_n61_), .b(x05), .c(new_n59_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n56_), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n62_), .c(new_n58_), .O(new_n65_));
  xnor2a g021(.a(x07), .b(x04), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(x08), .O(new_n68_));
  aoi21  g024(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  oai21  g025(.a(new_n66_), .b(new_n65_), .c(new_n69_), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n47_), .O(z2));
  inv1   g027(.a(x27), .O(new_n72_));
  oai22  g028(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n73_));
  oai22  g029(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g031(.a(x26), .O(new_n76_));
  nand3  g032(.a(new_n57_), .b(new_n76_), .c(x25), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n47_), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n72_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(z3));
  inv1   g038(.a(x28), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n72_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n83_), .b(new_n72_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n85_), .c(new_n78_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n47_), .O(z4));
  nand2  g044(.a(new_n84_), .b(x29), .O(new_n89_));
  inv1   g045(.a(x29), .O(new_n90_));
  nand2  g046(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n89_), .c(new_n80_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z5));
  xnor2a g049(.a(new_n89_), .b(x30), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n78_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n47_), .O(z6));
  xor2a  g052(.a(x31), .b(x30), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(new_n89_), .O(new_n99_));
  aoi21  g055(.a(new_n89_), .b(x31), .c(new_n99_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n80_), .O(z7));
  nand3  g057(.a(x17), .b(x16), .c(x15), .O(new_n102_));
  nand2  g058(.a(x19), .b(x13), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n102_), .c(new_n45_), .O(new_n104_));
  inv1   g060(.a(x31), .O(new_n105_));
  oai21  g061(.a(x28), .b(x27), .c(x29), .O(new_n106_));
  xor2a  g062(.a(new_n106_), .b(x30), .O(new_n107_));
  nor2   g063(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n104_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x23), .O(new_n110_));
  inv1   g066(.a(new_n102_), .O(new_n111_));
  inv1   g067(.a(x15), .O(new_n112_));
  inv1   g068(.a(x17), .O(new_n113_));
  nand2  g069(.a(x16), .b(x10), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(x19), .c(new_n111_), .O(new_n116_));
  inv1   g072(.a(x19), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(x14), .c(new_n102_), .O(new_n118_));
  nand2  g074(.a(x16), .b(x12), .O(new_n119_));
  oai21  g075(.a(x16), .b(x11), .c(x17), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n108_), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n116_), .c(x18), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n110_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x32), .O(new_n125_));
  nand2  g081(.a(x18), .b(x17), .O(new_n126_));
  nor4   g082(.a(new_n126_), .b(new_n114_), .c(new_n117_), .d(x15), .O(new_n127_));
  nand3  g083(.a(x18), .b(x17), .c(x11), .O(new_n128_));
  nor3   g084(.a(new_n128_), .b(new_n117_), .c(x16), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  nand4  g086(.a(x19), .b(x18), .c(new_n113_), .d(x12), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nand3  g088(.a(new_n117_), .b(x18), .c(x14), .O(new_n133_));
  nand3  g089(.a(x19), .b(new_n45_), .c(x13), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n133_), .c(new_n113_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n132_), .c(x16), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n130_), .c(new_n112_), .O(new_n137_));
  nand3  g093(.a(new_n133_), .b(new_n131_), .c(new_n46_), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nor2   g095(.a(new_n129_), .b(new_n127_), .O(new_n140_));
  nand2  g096(.a(new_n108_), .b(new_n52_), .O(new_n141_));
  aoi21  g097(.a(new_n140_), .b(new_n139_), .c(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n137_), .b(new_n127_), .c(new_n142_), .O(new_n143_));
  nand2  g099(.a(new_n76_), .b(x00), .O(new_n144_));
  aoi21  g100(.a(new_n143_), .b(new_n125_), .c(new_n144_), .O(z8));
  nand2  g101(.a(new_n136_), .b(new_n130_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(x15), .c(new_n127_), .O(new_n147_));
  nand2  g103(.a(new_n107_), .b(new_n98_), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n147_), .c(new_n49_), .O(new_n149_));
  nand2  g105(.a(x16), .b(x15), .O(new_n150_));
  oai21  g106(.a(new_n126_), .b(new_n150_), .c(new_n117_), .O(new_n151_));
  inv1   g107(.a(x16), .O(new_n152_));
  nand2  g108(.a(new_n128_), .b(new_n152_), .O(new_n153_));
  oai21  g109(.a(new_n126_), .b(new_n114_), .c(new_n112_), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(new_n153_), .c(new_n151_), .O(new_n155_));
  nand2  g111(.a(new_n118_), .b(x18), .O(new_n156_));
  nand2  g112(.a(x18), .b(x12), .O(new_n157_));
  oai21  g113(.a(x18), .b(x13), .c(x17), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nor2   g115(.a(new_n97_), .b(new_n49_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n159_), .c(new_n156_), .d(new_n107_), .O(new_n161_));
  nor2   g117(.a(new_n161_), .b(new_n155_), .O(new_n162_));
  nor2   g118(.a(new_n162_), .b(new_n144_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n149_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n47_), .O(z9));
endmodule


