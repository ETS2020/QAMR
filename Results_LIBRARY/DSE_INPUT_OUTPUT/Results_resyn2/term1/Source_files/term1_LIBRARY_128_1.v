// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_;
  inv1   g000(.a(x26), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x02), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x33), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x03), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g009(.a(new_n48_), .b(new_n51_), .O(new_n54_));
  nand2  g010(.a(new_n45_), .b(new_n50_), .O(new_n55_));
  aoi21  g011(.a(new_n52_), .b(x03), .c(new_n55_), .O(new_n56_));
  aoi22  g012(.a(new_n56_), .b(new_n54_), .c(new_n53_), .d(new_n49_), .O(z1));
  nor2   g013(.a(new_n51_), .b(new_n50_), .O(new_n58_));
  nor3   g014(.a(x26), .b(x03), .c(x02), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  nand2  g016(.a(x05), .b(new_n51_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n50_), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(x01), .O(new_n63_));
  oai21  g019(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n46_), .c(new_n60_), .O(new_n65_));
  xnor2a g021(.a(x07), .b(x04), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nor2   g023(.a(x03), .b(x02), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n58_), .c(x01), .O(new_n69_));
  nor2   g025(.a(new_n66_), .b(new_n46_), .O(new_n70_));
  nand3  g026(.a(new_n70_), .b(new_n69_), .c(new_n64_), .O(new_n71_));
  inv1   g027(.a(x09), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(x08), .O(new_n73_));
  aoi21  g029(.a(new_n71_), .b(new_n67_), .c(new_n73_), .O(z2));
  inv1   g030(.a(new_n46_), .O(new_n75_));
  inv1   g031(.a(x27), .O(new_n76_));
  aoi21  g032(.a(new_n45_), .b(new_n51_), .c(new_n50_), .O(new_n77_));
  nor2   g033(.a(x23), .b(x18), .O(new_n78_));
  nor2   g034(.a(x22), .b(x17), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nor2   g036(.a(x24), .b(x19), .O(new_n81_));
  oai22  g037(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n77_), .O(new_n86_));
  and2   g042(.a(x25), .b(x01), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n86_), .c(new_n76_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n75_), .O(z3));
  inv1   g045(.a(x28), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n76_), .O(new_n91_));
  nor2   g047(.a(new_n90_), .b(new_n76_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n91_), .c(new_n87_), .d(new_n86_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n75_), .O(z4));
  inv1   g051(.a(x29), .O(new_n96_));
  nand2  g052(.a(new_n93_), .b(new_n96_), .O(new_n97_));
  nand2  g053(.a(new_n92_), .b(x29), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n97_), .c(new_n87_), .d(new_n86_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n75_), .O(z5));
  inv1   g056(.a(x30), .O(new_n101_));
  nand2  g057(.a(new_n98_), .b(new_n101_), .O(new_n102_));
  inv1   g058(.a(new_n98_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(x30), .O(new_n104_));
  nor2   g060(.a(new_n58_), .b(x26), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n104_), .c(new_n102_), .d(new_n87_), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n85_), .O(z6));
  inv1   g063(.a(x31), .O(new_n108_));
  inv1   g064(.a(new_n104_), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n108_), .c(new_n87_), .O(new_n110_));
  aoi21  g066(.a(new_n109_), .b(new_n108_), .c(new_n110_), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n86_), .c(new_n46_), .O(z7));
  inv1   g068(.a(x15), .O(new_n113_));
  inv1   g069(.a(x17), .O(new_n114_));
  inv1   g070(.a(x19), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x18), .c(x14), .O(new_n116_));
  inv1   g072(.a(x18), .O(new_n117_));
  nand3  g073(.a(x19), .b(new_n117_), .c(x13), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n116_), .c(new_n114_), .O(new_n119_));
  nand4  g075(.a(x19), .b(x18), .c(new_n114_), .d(x12), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n119_), .c(x16), .O(new_n122_));
  inv1   g078(.a(x16), .O(new_n123_));
  nand3  g079(.a(x18), .b(x17), .c(x11), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x19), .c(new_n123_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n122_), .c(new_n113_), .O(new_n127_));
  nor2   g083(.a(new_n117_), .b(new_n114_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x16), .c(x10), .O(new_n129_));
  nor3   g085(.a(new_n129_), .b(new_n115_), .c(x15), .O(new_n130_));
  nor2   g086(.a(x32), .b(new_n108_), .O(new_n131_));
  oai21  g087(.a(new_n130_), .b(new_n127_), .c(new_n131_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n101_), .O(new_n133_));
  nand2  g089(.a(new_n91_), .b(x29), .O(new_n134_));
  aoi21  g090(.a(new_n52_), .b(x30), .c(new_n134_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g092(.a(new_n132_), .b(x30), .O(new_n137_));
  inv1   g093(.a(new_n134_), .O(new_n138_));
  aoi21  g094(.a(new_n52_), .b(new_n101_), .c(new_n138_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n137_), .O(new_n140_));
  oai21  g096(.a(new_n125_), .b(x16), .c(x15), .O(new_n141_));
  inv1   g097(.a(x14), .O(new_n142_));
  nor2   g098(.a(x19), .b(new_n142_), .O(new_n143_));
  nand3  g099(.a(new_n128_), .b(x16), .c(x15), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n115_), .O(new_n145_));
  oai21  g101(.a(new_n144_), .b(new_n143_), .c(new_n145_), .O(new_n146_));
  aoi21  g102(.a(new_n141_), .b(new_n129_), .c(new_n146_), .O(new_n147_));
  nand2  g103(.a(x18), .b(x12), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n114_), .O(new_n149_));
  inv1   g105(.a(x13), .O(new_n150_));
  nand2  g106(.a(new_n117_), .b(new_n150_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n149_), .c(new_n147_), .d(x31), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(x32), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n140_), .c(new_n136_), .O(new_n154_));
  nand2  g110(.a(new_n45_), .b(x00), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n154_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n75_), .O(z8));
  aoi21  g114(.a(x31), .b(x30), .c(new_n134_), .O(new_n159_));
  nand2  g115(.a(new_n108_), .b(new_n101_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n134_), .c(new_n159_), .O(new_n161_));
  oai21  g117(.a(new_n130_), .b(new_n127_), .c(new_n161_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n48_), .O(new_n163_));
  aoi21  g119(.a(new_n117_), .b(new_n150_), .c(new_n48_), .O(new_n164_));
  nand4  g120(.a(new_n164_), .b(new_n161_), .c(new_n149_), .d(new_n147_), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(new_n163_), .c(new_n156_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n75_), .O(z9));
endmodule


