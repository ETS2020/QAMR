// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  nor2   g000(.a(x18), .b(x17), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  inv1   g008(.a(x32), .O(new_n53_));
  aoi21  g009(.a(x33), .b(new_n48_), .c(x02), .O(new_n54_));
  oai21  g010(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n45_), .O(z1));
  inv1   g012(.a(x01), .O(new_n57_));
  aoi21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  oai21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n50_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n48_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n48_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n59_), .O(new_n64_));
  xor2a  g020(.a(x07), .b(x04), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand3  g023(.a(new_n46_), .b(new_n67_), .c(x08), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n66_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  inv1   g026(.a(x26), .O(new_n71_));
  nand3  g027(.a(new_n58_), .b(new_n71_), .c(x25), .O(new_n72_));
  nor2   g028(.a(x20), .b(x15), .O(new_n73_));
  oai22  g029(.a(x24), .b(x19), .c(x21), .d(x16), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g031(.a(x18), .O(new_n76_));
  inv1   g032(.a(x23), .O(new_n77_));
  nor2   g033(.a(x22), .b(x17), .O(new_n78_));
  aoi21  g034(.a(new_n77_), .b(new_n76_), .c(new_n78_), .O(new_n79_));
  aoi21  g035(.a(new_n79_), .b(new_n75_), .c(new_n72_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n70_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n46_), .O(z3));
  inv1   g038(.a(x28), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n70_), .O(new_n84_));
  nor2   g040(.a(new_n83_), .b(new_n70_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n84_), .c(new_n80_), .d(new_n46_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(z4));
  nand2  g044(.a(new_n85_), .b(x29), .O(new_n89_));
  inv1   g045(.a(x29), .O(new_n90_));
  nand2  g046(.a(new_n86_), .b(new_n90_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n89_), .c(new_n80_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n46_), .O(z5));
  inv1   g049(.a(x30), .O(new_n94_));
  nand2  g050(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  nand3  g051(.a(new_n85_), .b(x30), .c(x29), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n95_), .c(new_n80_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n46_), .O(z6));
  inv1   g054(.a(x17), .O(new_n99_));
  oai22  g055(.a(new_n78_), .b(new_n76_), .c(new_n77_), .d(new_n99_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n75_), .O(new_n101_));
  nor2   g057(.a(new_n96_), .b(x31), .O(new_n102_));
  inv1   g058(.a(new_n72_), .O(new_n103_));
  nand2  g059(.a(new_n96_), .b(x31), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n102_), .c(new_n46_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n101_), .O(z7));
  oai21  g063(.a(x28), .b(x27), .c(x29), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(x30), .c(x31), .O(new_n110_));
  aoi21  g066(.a(new_n109_), .b(x30), .c(new_n110_), .O(new_n111_));
  inv1   g067(.a(x15), .O(new_n112_));
  inv1   g068(.a(x19), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x18), .c(x14), .O(new_n114_));
  nand3  g070(.a(x19), .b(new_n76_), .c(x13), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n114_), .c(new_n99_), .O(new_n116_));
  and2   g072(.a(x19), .b(x12), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x18), .c(new_n99_), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n116_), .c(x16), .O(new_n120_));
  inv1   g076(.a(x16), .O(new_n121_));
  nand3  g077(.a(x19), .b(x18), .c(x17), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n121_), .c(x11), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n120_), .c(new_n112_), .O(new_n125_));
  nand3  g081(.a(x16), .b(new_n112_), .c(x10), .O(new_n126_));
  nor2   g082(.a(new_n126_), .b(new_n122_), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n125_), .c(new_n111_), .O(new_n128_));
  nand2  g084(.a(new_n71_), .b(x00), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  nand3  g086(.a(x18), .b(x16), .c(x10), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  oai21  g088(.a(x18), .b(x13), .c(x16), .O(new_n133_));
  nand2  g089(.a(x18), .b(x11), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(x15), .c(new_n132_), .O(new_n136_));
  nand3  g092(.a(x18), .b(x16), .c(x15), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n113_), .O(new_n138_));
  nand2  g094(.a(new_n113_), .b(x14), .O(new_n139_));
  inv1   g095(.a(new_n137_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n138_), .c(x17), .O(new_n142_));
  nand4  g098(.a(new_n117_), .b(new_n99_), .c(x16), .d(x15), .O(new_n143_));
  oai21  g099(.a(new_n142_), .b(new_n136_), .c(new_n143_), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n111_), .c(new_n45_), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n53_), .c(new_n130_), .O(new_n146_));
  aoi21  g102(.a(new_n128_), .b(new_n53_), .c(new_n146_), .O(z8));
  aoi21  g103(.a(x31), .b(x30), .c(new_n108_), .O(new_n148_));
  inv1   g104(.a(x31), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n94_), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n108_), .c(new_n148_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n144_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n46_), .c(x33), .O(new_n153_));
  nand2  g109(.a(new_n151_), .b(new_n49_), .O(new_n154_));
  inv1   g110(.a(new_n154_), .O(new_n155_));
  oai21  g111(.a(new_n127_), .b(new_n125_), .c(new_n155_), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n153_), .c(new_n129_), .O(z9));
endmodule


