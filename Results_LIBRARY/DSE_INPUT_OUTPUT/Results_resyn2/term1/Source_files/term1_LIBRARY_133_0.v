// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_;
  nor2   g000(.a(x18), .b(x16), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nand2  g002(.a(x03), .b(x02), .O(new_n47_));
  inv1   g003(.a(x02), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  and2   g006(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  inv1   g008(.a(x33), .O(new_n53_));
  aoi21  g009(.a(new_n52_), .b(new_n53_), .c(new_n45_), .O(new_n54_));
  oai21  g010(.a(new_n52_), .b(x32), .c(new_n54_), .O(z1));
  inv1   g011(.a(new_n45_), .O(new_n56_));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n49_), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n48_), .O(new_n59_));
  xor2a  g015(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(x01), .O(new_n61_));
  aoi21  g017(.a(new_n51_), .b(x01), .c(new_n61_), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  inv1   g019(.a(x09), .O(new_n64_));
  nand2  g020(.a(new_n62_), .b(new_n57_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(new_n64_), .c(x08), .O(new_n66_));
  oai21  g022(.a(new_n66_), .b(new_n63_), .c(new_n56_), .O(z2));
  inv1   g023(.a(x27), .O(new_n68_));
  inv1   g024(.a(x16), .O(new_n69_));
  inv1   g025(.a(x18), .O(new_n70_));
  inv1   g026(.a(x23), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai22  g028(.a(x24), .b(x19), .c(x21), .d(x16), .O(new_n73_));
  oai22  g029(.a(x22), .b(x17), .c(x20), .d(x15), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  oai21  g031(.a(new_n72_), .b(new_n69_), .c(new_n75_), .O(new_n76_));
  inv1   g032(.a(x26), .O(new_n77_));
  nand4  g033(.a(new_n47_), .b(new_n77_), .c(x25), .d(x01), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n45_), .O(new_n79_));
  and2   g035(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n68_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(z3));
  inv1   g038(.a(x28), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n68_), .O(new_n84_));
  nor2   g040(.a(new_n83_), .b(new_n68_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n84_), .c(new_n80_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(z4));
  aoi21  g044(.a(new_n75_), .b(new_n72_), .c(new_n78_), .O(new_n89_));
  nand2  g045(.a(new_n85_), .b(x29), .O(new_n90_));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n56_), .O(z5));
  inv1   g050(.a(x30), .O(new_n95_));
  xor2a  g051(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n89_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n56_), .O(z6));
  nand2  g054(.a(new_n90_), .b(x31), .O(new_n99_));
  nor2   g055(.a(x31), .b(x30), .O(new_n100_));
  nand2  g056(.a(x31), .b(x30), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n85_), .c(x29), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n99_), .c(new_n80_), .O(z7));
  nand2  g061(.a(new_n77_), .b(x00), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  inv1   g063(.a(x32), .O(new_n108_));
  inv1   g064(.a(x15), .O(new_n109_));
  nand2  g065(.a(new_n69_), .b(x11), .O(new_n110_));
  nand2  g066(.a(new_n70_), .b(x13), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  inv1   g068(.a(x17), .O(new_n113_));
  inv1   g069(.a(x19), .O(new_n114_));
  nor2   g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g071(.a(new_n113_), .b(x12), .O(new_n116_));
  nand2  g072(.a(new_n114_), .b(x14), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g074(.a(x18), .b(x16), .O(new_n119_));
  aoi21  g075(.a(new_n114_), .b(new_n113_), .c(new_n119_), .O(new_n120_));
  aoi22  g076(.a(new_n120_), .b(new_n118_), .c(new_n115_), .d(new_n112_), .O(new_n121_));
  nand3  g077(.a(x19), .b(x18), .c(x17), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(x16), .c(new_n109_), .d(x10), .O(new_n124_));
  oai21  g080(.a(new_n121_), .b(new_n109_), .c(new_n124_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n108_), .c(x31), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n95_), .O(new_n127_));
  nand2  g083(.a(new_n84_), .b(x29), .O(new_n128_));
  aoi21  g084(.a(new_n108_), .b(x30), .c(new_n128_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand4  g086(.a(new_n125_), .b(new_n108_), .c(x31), .d(x30), .O(new_n131_));
  nand2  g087(.a(x32), .b(new_n95_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n128_), .O(new_n134_));
  nand2  g090(.a(x17), .b(x13), .O(new_n135_));
  oai21  g091(.a(x17), .b(x12), .c(x18), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  inv1   g093(.a(x31), .O(new_n138_));
  and2   g094(.a(x15), .b(x11), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n115_), .c(x16), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand3  g097(.a(x18), .b(x17), .c(x10), .O(new_n142_));
  nand3  g098(.a(x18), .b(x17), .c(x15), .O(new_n143_));
  aoi22  g099(.a(new_n143_), .b(new_n114_), .c(new_n142_), .d(new_n109_), .O(new_n144_));
  inv1   g100(.a(new_n143_), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n117_), .c(x16), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n144_), .c(new_n141_), .d(new_n137_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x32), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n134_), .c(new_n130_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n107_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n56_), .O(z8));
  inv1   g107(.a(new_n128_), .O(new_n152_));
  nand3  g108(.a(new_n56_), .b(x33), .c(new_n95_), .O(new_n153_));
  nand4  g109(.a(x19), .b(x18), .c(new_n113_), .d(x12), .O(new_n154_));
  aoi22  g110(.a(new_n114_), .b(x14), .c(new_n70_), .d(x13), .O(new_n155_));
  oai21  g111(.a(x19), .b(x18), .c(x17), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n155_), .c(new_n154_), .O(new_n157_));
  nor2   g113(.a(new_n122_), .b(new_n110_), .O(new_n158_));
  aoi21  g114(.a(new_n157_), .b(x16), .c(new_n158_), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n109_), .c(new_n124_), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n102_), .c(new_n53_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n153_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n152_), .O(new_n163_));
  nand3  g119(.a(new_n56_), .b(x33), .c(x30), .O(new_n164_));
  nand3  g120(.a(new_n160_), .b(new_n100_), .c(new_n53_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  oai21  g122(.a(new_n140_), .b(new_n103_), .c(x18), .O(new_n167_));
  aoi21  g123(.a(new_n145_), .b(new_n117_), .c(new_n103_), .O(new_n168_));
  nand3  g124(.a(new_n168_), .b(new_n144_), .c(new_n137_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(x16), .O(new_n170_));
  aoi21  g126(.a(new_n170_), .b(new_n167_), .c(new_n53_), .O(new_n171_));
  aoi21  g127(.a(new_n166_), .b(new_n128_), .c(new_n171_), .O(new_n172_));
  aoi21  g128(.a(new_n172_), .b(new_n163_), .c(new_n106_), .O(z9));
endmodule


