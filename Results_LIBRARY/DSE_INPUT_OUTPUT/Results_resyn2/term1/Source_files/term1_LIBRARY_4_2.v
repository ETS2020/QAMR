// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_;
  inv1   g00(.a(x15), .O(new_n45_));
  nor2   g01(.a(x26), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand2  g03(.a(new_n47_), .b(x32), .O(z0));
  inv1   g04(.a(x03), .O(new_n49_));
  inv1   g05(.a(x33), .O(new_n50_));
  inv1   g06(.a(x02), .O(new_n51_));
  aoi21  g07(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g08(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  inv1   g09(.a(x32), .O(new_n54_));
  aoi21  g10(.a(x33), .b(new_n49_), .c(x02), .O(new_n55_));
  oai21  g11(.a(new_n54_), .b(new_n49_), .c(new_n55_), .O(new_n56_));
  aoi21  g12(.a(new_n56_), .b(new_n53_), .c(new_n46_), .O(z1));
  inv1   g13(.a(x01), .O(new_n58_));
  aoi21  g14(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  oai21  g15(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  nand2  g16(.a(x05), .b(new_n49_), .O(new_n61_));
  nand3  g17(.a(new_n61_), .b(x06), .c(new_n51_), .O(new_n62_));
  nand2  g18(.a(x06), .b(new_n51_), .O(new_n63_));
  nand3  g19(.a(new_n63_), .b(x05), .c(new_n49_), .O(new_n64_));
  nand3  g20(.a(new_n64_), .b(new_n62_), .c(new_n58_), .O(new_n65_));
  xor2a  g21(.a(x07), .b(x04), .O(new_n66_));
  aoi21  g22(.a(new_n65_), .b(new_n60_), .c(new_n66_), .O(new_n67_));
  inv1   g23(.a(x09), .O(new_n68_));
  nand3  g24(.a(new_n66_), .b(new_n65_), .c(new_n60_), .O(new_n69_));
  nand3  g25(.a(new_n69_), .b(new_n68_), .c(x08), .O(new_n70_));
  oai21  g26(.a(new_n70_), .b(new_n67_), .c(new_n47_), .O(z2));
  inv1   g27(.a(x27), .O(new_n72_));
  inv1   g28(.a(x20), .O(new_n73_));
  nor2   g29(.a(x23), .b(x18), .O(new_n74_));
  nor2   g30(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g31(.a(x19), .O(new_n76_));
  inv1   g32(.a(x24), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  or2    g34(.a(x22), .b(x17), .O(new_n79_));
  or2    g35(.a(x21), .b(x16), .O(new_n80_));
  nand4  g36(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n75_), .O(new_n81_));
  nand2  g37(.a(new_n59_), .b(x25), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  nor2   g41(.a(x26), .b(x15), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n85_), .c(new_n72_), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(z3));
  inv1   g44(.a(x28), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n72_), .O(new_n90_));
  nor2   g46(.a(new_n89_), .b(new_n72_), .O(new_n91_));
  inv1   g47(.a(new_n91_), .O(new_n92_));
  nand3  g48(.a(new_n92_), .b(new_n90_), .c(new_n85_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n45_), .c(x26), .O(z4));
  nand2  g50(.a(new_n91_), .b(x29), .O(new_n95_));
  inv1   g51(.a(x29), .O(new_n96_));
  nand2  g52(.a(new_n92_), .b(new_n96_), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(new_n95_), .c(new_n86_), .d(new_n85_), .O(new_n98_));
  inv1   g54(.a(new_n98_), .O(z5));
  nand3  g55(.a(new_n91_), .b(x30), .c(x29), .O(new_n100_));
  inv1   g56(.a(x30), .O(new_n101_));
  nand2  g57(.a(new_n95_), .b(new_n101_), .O(new_n102_));
  nand4  g58(.a(new_n102_), .b(new_n100_), .c(new_n86_), .d(new_n85_), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(z6));
  inv1   g60(.a(x26), .O(new_n105_));
  inv1   g61(.a(x31), .O(new_n106_));
  xor2a  g62(.a(new_n100_), .b(new_n106_), .O(new_n107_));
  oai21  g63(.a(new_n107_), .b(new_n84_), .c(new_n45_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n105_), .O(z7));
  oai21  g65(.a(x28), .b(x27), .c(x29), .O(new_n110_));
  nand2  g66(.a(x32), .b(x30), .O(new_n111_));
  nand4  g67(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  nor2   g69(.a(x30), .b(new_n76_), .O(new_n114_));
  nand4  g70(.a(new_n114_), .b(new_n113_), .c(new_n54_), .d(x31), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n111_), .c(new_n110_), .O(new_n116_));
  nand2  g72(.a(x32), .b(new_n101_), .O(new_n117_));
  nand4  g73(.a(new_n54_), .b(x31), .c(x30), .d(x19), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n112_), .c(new_n117_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n110_), .O(new_n120_));
  nand3  g76(.a(x17), .b(x16), .c(x10), .O(new_n121_));
  nand3  g77(.a(x31), .b(x19), .c(x18), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(new_n121_), .c(x32), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n116_), .c(x00), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n45_), .c(x26), .O(z8));
  nand2  g82(.a(new_n106_), .b(new_n101_), .O(new_n127_));
  oai21  g83(.a(new_n110_), .b(new_n106_), .c(new_n127_), .O(new_n128_));
  nand2  g84(.a(x19), .b(x18), .O(new_n129_));
  nor2   g85(.a(new_n129_), .b(new_n121_), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n128_), .c(new_n50_), .O(new_n131_));
  inv1   g87(.a(new_n110_), .O(new_n132_));
  nand2  g88(.a(x33), .b(new_n101_), .O(new_n133_));
  nand4  g89(.a(new_n50_), .b(x31), .c(x30), .d(x19), .O(new_n134_));
  oai21  g90(.a(new_n134_), .b(new_n112_), .c(new_n133_), .O(new_n135_));
  nand2  g91(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  nor2   g92(.a(x33), .b(x31), .O(new_n137_));
  nand4  g93(.a(new_n137_), .b(new_n114_), .c(new_n113_), .d(new_n110_), .O(new_n138_));
  nand2  g94(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  oai21  g95(.a(new_n139_), .b(new_n131_), .c(x00), .O(new_n140_));
  aoi21  g96(.a(new_n140_), .b(new_n45_), .c(x26), .O(z9));
endmodule


