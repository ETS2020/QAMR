// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  nand2  g000(.a(x30), .b(x29), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(x33), .O(new_n53_));
  aoi21  g009(.a(new_n52_), .b(new_n53_), .c(new_n46_), .O(new_n54_));
  oai21  g010(.a(new_n52_), .b(x32), .c(new_n54_), .O(z1));
  xnor2a g011(.a(x07), .b(x04), .O(new_n56_));
  nand2  g012(.a(new_n52_), .b(x01), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n50_), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n49_), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  oai21  g016(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  xor2a  g018(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  inv1   g019(.a(x09), .O(new_n64_));
  nand3  g020(.a(new_n45_), .b(new_n64_), .c(x08), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n63_), .O(z2));
  nor2   g022(.a(x20), .b(x15), .O(new_n67_));
  nor2   g023(.a(x24), .b(x19), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g025(.a(x18), .O(new_n70_));
  inv1   g026(.a(x23), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor2   g028(.a(x22), .b(x17), .O(new_n73_));
  nor2   g029(.a(x21), .b(x16), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(new_n76_));
  and2   g032(.a(new_n76_), .b(new_n48_), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(new_n78_));
  inv1   g034(.a(x27), .O(new_n79_));
  inv1   g035(.a(x26), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(x25), .c(x01), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n78_), .c(new_n45_), .O(z3));
  nand2  g040(.a(new_n77_), .b(new_n45_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(x28), .b(x27), .O(new_n87_));
  inv1   g043(.a(x28), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n79_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n87_), .c(new_n86_), .d(new_n82_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z4));
  nand3  g047(.a(x29), .b(x28), .c(x27), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  oai21  g049(.a(x30), .b(new_n93_), .c(new_n87_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n92_), .c(new_n82_), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(new_n78_), .O(z5));
  inv1   g052(.a(x30), .O(new_n97_));
  aoi21  g053(.a(new_n92_), .b(new_n97_), .c(new_n81_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n86_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z6));
  inv1   g056(.a(x31), .O(new_n101_));
  nand2  g057(.a(new_n82_), .b(new_n101_), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n77_), .c(new_n46_), .O(z7));
  nand2  g060(.a(new_n80_), .b(x00), .O(new_n105_));
  inv1   g061(.a(x15), .O(new_n106_));
  inv1   g062(.a(x17), .O(new_n107_));
  inv1   g063(.a(x19), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x18), .c(x14), .O(new_n109_));
  nand3  g065(.a(x19), .b(new_n70_), .c(x13), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n109_), .c(new_n107_), .O(new_n111_));
  nand4  g067(.a(x19), .b(x18), .c(new_n107_), .d(x12), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(x16), .O(new_n114_));
  inv1   g070(.a(x16), .O(new_n115_));
  nand3  g071(.a(x18), .b(x17), .c(x11), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x19), .c(new_n115_), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n114_), .c(new_n106_), .O(new_n119_));
  nand4  g075(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n120_));
  nor3   g076(.a(new_n120_), .b(new_n108_), .c(x15), .O(new_n121_));
  nand2  g077(.a(new_n89_), .b(x29), .O(new_n122_));
  aoi21  g078(.a(new_n122_), .b(new_n97_), .c(new_n101_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(z0), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  oai21  g081(.a(new_n121_), .b(new_n119_), .c(new_n125_), .O(new_n126_));
  oai21  g082(.a(new_n117_), .b(x16), .c(x15), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n120_), .O(new_n128_));
  nand4  g084(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n129_));
  aoi21  g085(.a(new_n108_), .b(x14), .c(new_n129_), .O(new_n130_));
  aoi21  g086(.a(new_n129_), .b(new_n108_), .c(new_n130_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand2  g088(.a(x18), .b(x12), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n107_), .O(new_n134_));
  inv1   g090(.a(x13), .O(new_n135_));
  nand2  g091(.a(new_n70_), .b(new_n135_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n134_), .c(new_n123_), .O(new_n137_));
  and2   g093(.a(new_n45_), .b(x32), .O(new_n138_));
  oai21  g094(.a(new_n137_), .b(new_n132_), .c(new_n138_), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n126_), .c(new_n105_), .O(z8));
  nand2  g096(.a(new_n53_), .b(new_n101_), .O(new_n141_));
  aoi21  g097(.a(new_n89_), .b(x29), .c(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n121_), .b(new_n119_), .c(new_n142_), .O(new_n143_));
  nand4  g099(.a(new_n136_), .b(new_n134_), .c(new_n122_), .d(new_n101_), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n131_), .c(new_n128_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(x33), .c(x30), .O(new_n147_));
  oai21  g103(.a(new_n53_), .b(x29), .c(x30), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n80_), .c(x00), .O(new_n149_));
  aoi21  g105(.a(new_n147_), .b(new_n143_), .c(new_n149_), .O(z9));
endmodule


