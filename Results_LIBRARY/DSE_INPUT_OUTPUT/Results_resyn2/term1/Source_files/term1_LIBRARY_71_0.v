// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x20), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x33), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  inv1   g008(.a(new_n50_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n51_), .c(new_n47_), .O(z1));
  xor2a  g011(.a(x07), .b(x04), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n57_), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n58_), .c(x01), .O(new_n61_));
  oai21  g017(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n50_), .b(x01), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(x08), .O(new_n66_));
  aoi21  g022(.a(new_n64_), .b(new_n56_), .c(new_n66_), .O(new_n67_));
  oai21  g023(.a(new_n64_), .b(new_n56_), .c(new_n67_), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n47_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  nand2  g026(.a(x03), .b(x02), .O(new_n71_));
  oai22  g027(.a(x24), .b(x19), .c(x23), .d(x18), .O(new_n72_));
  oai22  g028(.a(x22), .b(x17), .c(x21), .d(x16), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g030(.a(x15), .O(new_n75_));
  nand2  g031(.a(new_n45_), .b(new_n75_), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(x30), .O(new_n78_));
  aoi22  g034(.a(new_n78_), .b(new_n74_), .c(new_n46_), .d(new_n45_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n71_), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  nand2  g037(.a(x25), .b(x01), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(x26), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n81_), .c(new_n70_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(z3));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n70_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n70_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n88_), .c(new_n83_), .d(new_n81_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z4));
  nand2  g047(.a(new_n83_), .b(new_n71_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n88_), .b(new_n94_), .O(new_n95_));
  nand2  g051(.a(new_n87_), .b(x29), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n95_), .c(new_n93_), .d(new_n79_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(z5));
  inv1   g054(.a(new_n96_), .O(new_n99_));
  nor2   g055(.a(new_n74_), .b(new_n46_), .O(new_n100_));
  nor3   g056(.a(new_n100_), .b(new_n99_), .c(new_n77_), .O(new_n101_));
  nor2   g057(.a(new_n74_), .b(x30), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n96_), .c(new_n93_), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n101_), .c(new_n47_), .O(z6));
  oai21  g060(.a(new_n96_), .b(new_n46_), .c(x31), .O(new_n105_));
  inv1   g061(.a(x31), .O(new_n106_));
  nand3  g062(.a(new_n99_), .b(new_n106_), .c(x30), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n105_), .c(new_n83_), .d(new_n81_), .O(z7));
  inv1   g064(.a(x00), .O(new_n109_));
  nor2   g065(.a(x26), .b(new_n109_), .O(new_n110_));
  inv1   g066(.a(x17), .O(new_n111_));
  inv1   g067(.a(x19), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(x18), .c(x14), .O(new_n113_));
  inv1   g069(.a(x18), .O(new_n114_));
  nand3  g070(.a(x19), .b(new_n114_), .c(x13), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n113_), .c(new_n111_), .O(new_n116_));
  nand4  g072(.a(x19), .b(x18), .c(new_n111_), .d(x12), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x16), .O(new_n119_));
  inv1   g075(.a(x16), .O(new_n120_));
  nand3  g076(.a(x18), .b(x17), .c(x11), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x19), .c(new_n120_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nor2   g080(.a(new_n114_), .b(new_n111_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x16), .c(x10), .O(new_n126_));
  nor3   g082(.a(new_n126_), .b(new_n112_), .c(x15), .O(new_n127_));
  aoi21  g083(.a(new_n124_), .b(x15), .c(new_n127_), .O(new_n128_));
  oai21  g084(.a(x28), .b(x27), .c(x29), .O(new_n129_));
  xor2a  g085(.a(new_n129_), .b(new_n46_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n52_), .c(x31), .O(new_n131_));
  oai21  g087(.a(new_n122_), .b(x16), .c(x15), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n126_), .O(new_n133_));
  nand3  g089(.a(new_n125_), .b(x16), .c(x15), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n112_), .O(new_n135_));
  nand2  g091(.a(new_n112_), .b(x14), .O(new_n136_));
  nand4  g092(.a(new_n125_), .b(new_n136_), .c(x16), .d(x15), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(new_n133_), .O(new_n138_));
  nand2  g094(.a(x18), .b(x12), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n111_), .O(new_n140_));
  inv1   g096(.a(x13), .O(new_n141_));
  nand2  g097(.a(new_n114_), .b(new_n141_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n140_), .c(new_n130_), .d(x31), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n138_), .c(x32), .O(new_n144_));
  oai21  g100(.a(new_n131_), .b(new_n128_), .c(new_n144_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n110_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n47_), .O(z8));
  xor2a  g103(.a(new_n129_), .b(new_n106_), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(new_n130_), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n128_), .c(new_n49_), .O(new_n151_));
  inv1   g107(.a(new_n110_), .O(new_n152_));
  nand4  g108(.a(new_n149_), .b(new_n142_), .c(new_n140_), .d(x33), .O(new_n153_));
  nor2   g109(.a(new_n153_), .b(new_n138_), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n47_), .O(z9));
endmodule


