// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_;
  nor2   g000(.a(x18), .b(x15), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nand2  g002(.a(x03), .b(x02), .O(new_n47_));
  inv1   g003(.a(x02), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g007(.a(x33), .O(new_n52_));
  aoi21  g008(.a(new_n51_), .b(new_n52_), .c(new_n45_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(x32), .c(new_n53_), .O(z1));
  xnor2a g010(.a(x07), .b(x04), .O(new_n55_));
  nand2  g011(.a(new_n51_), .b(x01), .O(new_n56_));
  nand2  g012(.a(x05), .b(new_n49_), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n48_), .O(new_n58_));
  aoi21  g014(.a(new_n58_), .b(new_n57_), .c(x01), .O(new_n59_));
  oai21  g015(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  xor2a  g017(.a(new_n61_), .b(new_n55_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  inv1   g019(.a(new_n45_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n63_), .c(x08), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n62_), .O(z2));
  inv1   g022(.a(x18), .O(new_n67_));
  inv1   g023(.a(x23), .O(new_n68_));
  nor2   g024(.a(x20), .b(x15), .O(new_n69_));
  nor2   g025(.a(x21), .b(x16), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g027(.a(x24), .b(x19), .O(new_n72_));
  nor2   g028(.a(x22), .b(x17), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  aoi21  g031(.a(new_n68_), .b(new_n67_), .c(new_n75_), .O(new_n76_));
  inv1   g032(.a(x27), .O(new_n77_));
  inv1   g033(.a(x26), .O(new_n78_));
  nand4  g034(.a(new_n47_), .b(new_n78_), .c(x25), .d(x01), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n76_), .c(new_n64_), .O(z3));
  nor2   g038(.a(new_n76_), .b(new_n45_), .O(new_n83_));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n77_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n77_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n86_), .c(new_n83_), .d(new_n80_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z4));
  nand2  g045(.a(new_n85_), .b(x29), .O(new_n90_));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n90_), .c(new_n83_), .d(new_n80_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(z5));
  inv1   g050(.a(x30), .O(new_n95_));
  aoi21  g051(.a(new_n90_), .b(new_n95_), .c(new_n79_), .O(new_n96_));
  oai21  g052(.a(new_n90_), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n76_), .c(new_n64_), .O(z6));
  nand3  g054(.a(new_n68_), .b(new_n67_), .c(x15), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n74_), .c(new_n71_), .O(new_n100_));
  nor2   g056(.a(new_n79_), .b(new_n45_), .O(new_n101_));
  nand2  g057(.a(new_n90_), .b(x31), .O(new_n102_));
  xor2a  g058(.a(x31), .b(x30), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n85_), .c(x29), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(z7));
  nand2  g061(.a(new_n78_), .b(x00), .O(new_n106_));
  inv1   g062(.a(x15), .O(new_n107_));
  nand2  g063(.a(new_n67_), .b(x13), .O(new_n108_));
  inv1   g064(.a(x19), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x14), .O(new_n110_));
  oai21  g066(.a(x19), .b(x18), .c(x17), .O(new_n111_));
  aoi21  g067(.a(new_n110_), .b(new_n108_), .c(new_n111_), .O(new_n112_));
  inv1   g068(.a(x17), .O(new_n113_));
  nand4  g069(.a(x19), .b(x18), .c(new_n113_), .d(x12), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n112_), .c(x16), .O(new_n116_));
  inv1   g072(.a(x16), .O(new_n117_));
  nand3  g073(.a(x19), .b(x18), .c(x17), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n117_), .c(x11), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n116_), .c(new_n107_), .O(new_n121_));
  nand2  g077(.a(new_n107_), .b(x10), .O(new_n122_));
  nor3   g078(.a(new_n122_), .b(new_n118_), .c(new_n117_), .O(new_n123_));
  inv1   g079(.a(x32), .O(new_n124_));
  oai21  g080(.a(x28), .b(x27), .c(x29), .O(new_n125_));
  xor2a  g081(.a(new_n125_), .b(new_n95_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n124_), .c(x31), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  oai21  g084(.a(new_n123_), .b(new_n121_), .c(new_n128_), .O(new_n129_));
  nand3  g085(.a(x18), .b(x17), .c(x11), .O(new_n130_));
  nand3  g086(.a(x18), .b(x17), .c(x16), .O(new_n131_));
  aoi22  g087(.a(new_n131_), .b(new_n109_), .c(new_n130_), .d(new_n117_), .O(new_n132_));
  aoi21  g088(.a(new_n109_), .b(x14), .c(new_n131_), .O(new_n133_));
  nand2  g089(.a(x17), .b(x13), .O(new_n134_));
  oai21  g090(.a(x17), .b(x12), .c(x18), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n134_), .c(new_n133_), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n132_), .c(new_n107_), .O(new_n137_));
  nor2   g093(.a(new_n109_), .b(new_n113_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(x16), .c(x10), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n107_), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n126_), .c(x31), .O(new_n141_));
  nor2   g097(.a(new_n45_), .b(new_n124_), .O(new_n142_));
  oai21  g098(.a(new_n141_), .b(new_n137_), .c(new_n142_), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n129_), .c(new_n106_), .O(z8));
  nand2  g100(.a(new_n122_), .b(new_n108_), .O(new_n145_));
  nand2  g101(.a(new_n113_), .b(x12), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n110_), .O(new_n147_));
  nand2  g103(.a(x18), .b(x15), .O(new_n148_));
  aoi21  g104(.a(new_n109_), .b(new_n113_), .c(new_n148_), .O(new_n149_));
  aoi22  g105(.a(new_n149_), .b(new_n147_), .c(new_n145_), .d(new_n138_), .O(new_n150_));
  nand4  g106(.a(new_n119_), .b(new_n117_), .c(x15), .d(x11), .O(new_n151_));
  oai21  g107(.a(new_n150_), .b(new_n117_), .c(new_n151_), .O(new_n152_));
  nor2   g108(.a(new_n126_), .b(new_n103_), .O(new_n153_));
  aoi21  g109(.a(new_n153_), .b(new_n152_), .c(x33), .O(new_n154_));
  inv1   g110(.a(new_n106_), .O(new_n155_));
  nand2  g111(.a(new_n135_), .b(new_n134_), .O(new_n156_));
  nor2   g112(.a(new_n103_), .b(new_n52_), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(new_n140_), .c(new_n156_), .O(new_n158_));
  inv1   g114(.a(new_n126_), .O(new_n159_));
  nand2  g115(.a(new_n133_), .b(x15), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n132_), .c(new_n159_), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n158_), .c(new_n155_), .O(new_n162_));
  oai21  g118(.a(new_n162_), .b(new_n154_), .c(new_n64_), .O(z9));
endmodule


