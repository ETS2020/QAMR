// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x15), .O(new_n45_));
  nor2   g001(.a(x18), .b(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(x33), .O(new_n53_));
  aoi21  g009(.a(new_n52_), .b(new_n53_), .c(new_n46_), .O(new_n54_));
  oai21  g010(.a(new_n52_), .b(x32), .c(new_n54_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  nand2  g013(.a(new_n52_), .b(x01), .O(new_n58_));
  nand4  g014(.a(x06), .b(x05), .c(new_n50_), .d(new_n49_), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n50_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n49_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n57_), .O(new_n65_));
  nor4   g021(.a(new_n65_), .b(new_n46_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g022(.a(new_n46_), .O(new_n67_));
  inv1   g023(.a(x26), .O(new_n68_));
  nand4  g024(.a(new_n48_), .b(new_n68_), .c(x25), .d(x01), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(new_n70_));
  nor2   g026(.a(x24), .b(x19), .O(new_n71_));
  oai22  g027(.a(x22), .b(x17), .c(x21), .d(x16), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g029(.a(x20), .O(new_n74_));
  nor2   g030(.a(x23), .b(x18), .O(new_n75_));
  aoi21  g031(.a(new_n74_), .b(new_n45_), .c(new_n75_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(x27), .c(new_n67_), .O(z3));
  inv1   g035(.a(x27), .O(new_n80_));
  inv1   g036(.a(x28), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n81_), .b(new_n80_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n78_), .c(new_n67_), .O(z4));
  oai21  g042(.a(new_n75_), .b(new_n74_), .c(new_n45_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n73_), .O(new_n88_));
  inv1   g044(.a(x29), .O(new_n89_));
  nand2  g045(.a(new_n83_), .b(new_n89_), .O(new_n90_));
  aoi21  g046(.a(new_n82_), .b(x29), .c(new_n46_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n90_), .c(new_n88_), .d(new_n70_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z5));
  nand2  g049(.a(new_n82_), .b(x29), .O(new_n94_));
  xor2a  g050(.a(new_n94_), .b(x30), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n78_), .c(new_n67_), .O(z6));
  inv1   g052(.a(new_n78_), .O(new_n97_));
  xor2a  g053(.a(x31), .b(x30), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nor2   g055(.a(new_n99_), .b(new_n94_), .O(new_n100_));
  aoi21  g056(.a(new_n94_), .b(x31), .c(new_n100_), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(new_n97_), .c(new_n46_), .O(z7));
  inv1   g058(.a(x18), .O(new_n103_));
  inv1   g059(.a(x00), .O(new_n104_));
  nor2   g060(.a(x26), .b(new_n104_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x32), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n45_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n103_), .O(new_n108_));
  inv1   g064(.a(x16), .O(new_n109_));
  inv1   g065(.a(x19), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x17), .c(x14), .O(new_n111_));
  inv1   g067(.a(x17), .O(new_n112_));
  nand3  g068(.a(x19), .b(new_n112_), .c(x12), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n111_), .c(new_n109_), .O(new_n114_));
  nand4  g070(.a(x19), .b(x17), .c(new_n109_), .d(x11), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n114_), .c(x15), .O(new_n117_));
  nand3  g073(.a(x17), .b(x16), .c(x10), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(x19), .c(x18), .d(new_n45_), .O(new_n120_));
  inv1   g076(.a(x32), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x31), .O(new_n122_));
  aoi21  g078(.a(new_n120_), .b(new_n117_), .c(new_n122_), .O(new_n123_));
  nand2  g079(.a(new_n84_), .b(x29), .O(new_n124_));
  aoi21  g080(.a(new_n121_), .b(x30), .c(new_n124_), .O(new_n125_));
  oai21  g081(.a(new_n123_), .b(x30), .c(new_n125_), .O(new_n126_));
  inv1   g082(.a(x30), .O(new_n127_));
  aoi22  g083(.a(new_n84_), .b(x29), .c(new_n121_), .d(new_n127_), .O(new_n128_));
  oai21  g084(.a(new_n123_), .b(new_n127_), .c(new_n128_), .O(new_n129_));
  nand2  g085(.a(new_n110_), .b(x14), .O(new_n130_));
  nand2  g086(.a(x17), .b(x11), .O(new_n131_));
  nor2   g087(.a(new_n112_), .b(new_n109_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x15), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  oai21  g090(.a(x17), .b(x12), .c(x16), .O(new_n135_));
  aoi22  g091(.a(new_n135_), .b(new_n131_), .c(new_n134_), .d(new_n130_), .O(new_n136_));
  nand2  g092(.a(new_n118_), .b(new_n45_), .O(new_n137_));
  inv1   g093(.a(x31), .O(new_n138_));
  aoi21  g094(.a(new_n133_), .b(new_n110_), .c(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n137_), .c(new_n136_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x32), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n129_), .c(new_n126_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n105_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n108_), .O(z8));
  nand2  g100(.a(new_n120_), .b(new_n117_), .O(new_n145_));
  nand2  g101(.a(new_n120_), .b(new_n103_), .O(new_n146_));
  xor2a  g102(.a(new_n124_), .b(new_n127_), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n98_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n146_), .c(new_n145_), .O(new_n149_));
  nor2   g105(.a(new_n132_), .b(x19), .O(new_n150_));
  nor2   g106(.a(new_n150_), .b(new_n98_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n136_), .c(new_n103_), .O(new_n152_));
  nand3  g108(.a(new_n119_), .b(x19), .c(x18), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n98_), .c(new_n45_), .O(new_n154_));
  nand2  g110(.a(new_n147_), .b(new_n67_), .O(new_n155_));
  nand3  g111(.a(new_n155_), .b(new_n154_), .c(x33), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n152_), .c(new_n105_), .O(new_n157_));
  aoi21  g113(.a(new_n149_), .b(new_n53_), .c(new_n157_), .O(z9));
endmodule


