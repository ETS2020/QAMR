// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_;
  nor2   g000(.a(x18), .b(x13), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nand2  g002(.a(x33), .b(x03), .O(new_n47_));
  inv1   g003(.a(x03), .O(new_n48_));
  nand2  g004(.a(x32), .b(new_n48_), .O(new_n49_));
  nand3  g005(.a(new_n49_), .b(new_n47_), .c(x02), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  nand2  g007(.a(x32), .b(x03), .O(new_n52_));
  nand2  g008(.a(x33), .b(new_n48_), .O(new_n53_));
  nand3  g009(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nand2  g014(.a(new_n48_), .b(new_n51_), .O(new_n59_));
  nand2  g015(.a(x03), .b(x02), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g017(.a(x05), .b(new_n48_), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n51_), .O(new_n63_));
  xor2a  g019(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n58_), .c(new_n61_), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n57_), .O(new_n66_));
  nor4   g022(.a(new_n66_), .b(new_n45_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g023(.a(x27), .O(new_n68_));
  oai22  g024(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n69_));
  oai22  g025(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n70_));
  or2    g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n45_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n60_), .O(new_n75_));
  inv1   g031(.a(new_n75_), .O(new_n76_));
  nand2  g032(.a(x25), .b(x01), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(x26), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n76_), .c(new_n68_), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(z3));
  inv1   g036(.a(x28), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n68_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n81_), .b(new_n68_), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n83_), .c(new_n78_), .d(new_n76_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(z4));
  inv1   g042(.a(new_n45_), .O(new_n87_));
  nand2  g043(.a(new_n78_), .b(new_n60_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n82_), .b(x29), .O(new_n90_));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n83_), .b(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n73_), .c(new_n87_), .O(z5));
  inv1   g050(.a(x30), .O(new_n95_));
  nand2  g051(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand3  g052(.a(new_n82_), .b(x30), .c(x29), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n96_), .c(new_n89_), .d(new_n74_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z6));
  aoi21  g055(.a(x23), .b(x13), .c(x18), .O(new_n100_));
  nor2   g056(.a(new_n97_), .b(x31), .O(new_n101_));
  nand2  g057(.a(new_n97_), .b(x31), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n89_), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n101_), .c(new_n87_), .O(new_n104_));
  oai21  g060(.a(new_n100_), .b(new_n71_), .c(new_n104_), .O(z7));
  inv1   g061(.a(x15), .O(new_n106_));
  inv1   g062(.a(x17), .O(new_n107_));
  aoi21  g063(.a(x18), .b(x14), .c(x19), .O(new_n108_));
  nand3  g064(.a(x19), .b(x18), .c(x12), .O(new_n109_));
  oai21  g065(.a(new_n108_), .b(new_n107_), .c(new_n109_), .O(new_n110_));
  inv1   g066(.a(x16), .O(new_n111_));
  nand3  g067(.a(x19), .b(x18), .c(x17), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  nor2   g069(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g070(.a(x19), .b(x18), .O(new_n115_));
  nand2  g071(.a(x17), .b(x11), .O(new_n116_));
  nor3   g072(.a(new_n116_), .b(new_n115_), .c(x16), .O(new_n117_));
  aoi21  g073(.a(new_n114_), .b(new_n110_), .c(new_n117_), .O(new_n118_));
  nand4  g074(.a(new_n113_), .b(x16), .c(new_n106_), .d(x10), .O(new_n119_));
  oai21  g075(.a(new_n118_), .b(new_n106_), .c(new_n119_), .O(new_n120_));
  inv1   g076(.a(x31), .O(new_n121_));
  nor2   g077(.a(x32), .b(new_n121_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g079(.a(new_n84_), .b(x29), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  oai21  g081(.a(x32), .b(new_n95_), .c(new_n125_), .O(new_n126_));
  aoi21  g082(.a(new_n123_), .b(new_n95_), .c(new_n126_), .O(new_n127_));
  aoi21  g083(.a(new_n122_), .b(new_n120_), .c(new_n95_), .O(new_n128_));
  oai21  g084(.a(x32), .b(x30), .c(new_n124_), .O(new_n129_));
  nor2   g085(.a(x19), .b(x18), .O(new_n130_));
  nand3  g086(.a(x17), .b(x16), .c(x15), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n115_), .c(new_n130_), .O(new_n132_));
  nand3  g088(.a(x17), .b(x16), .c(x10), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n106_), .O(new_n134_));
  inv1   g090(.a(new_n131_), .O(new_n135_));
  inv1   g091(.a(x19), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x14), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(x18), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n134_), .c(new_n132_), .O(new_n139_));
  nand2  g095(.a(new_n116_), .b(new_n111_), .O(new_n140_));
  inv1   g096(.a(x12), .O(new_n141_));
  nand2  g097(.a(new_n107_), .b(new_n141_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n140_), .c(x31), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n139_), .c(x32), .O(new_n144_));
  oai21  g100(.a(new_n129_), .b(new_n128_), .c(new_n144_), .O(new_n145_));
  inv1   g101(.a(x00), .O(new_n146_));
  nor2   g102(.a(x26), .b(new_n146_), .O(new_n147_));
  oai21  g103(.a(new_n145_), .b(new_n127_), .c(new_n147_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n87_), .O(z8));
  aoi21  g105(.a(x31), .b(x30), .c(new_n124_), .O(new_n150_));
  nand2  g106(.a(new_n121_), .b(new_n95_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n124_), .c(new_n150_), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n120_), .c(x33), .O(new_n153_));
  nand4  g109(.a(new_n152_), .b(new_n142_), .c(new_n140_), .d(x33), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n139_), .c(new_n147_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n153_), .c(new_n87_), .O(z9));
endmodule


