// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x15), .O(new_n45_));
  nor2   g001(.a(x18), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  inv1   g009(.a(x32), .O(new_n54_));
  aoi21  g010(.a(x33), .b(new_n49_), .c(x02), .O(new_n55_));
  oai21  g011(.a(new_n54_), .b(new_n49_), .c(new_n55_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n46_), .O(z1));
  inv1   g013(.a(x01), .O(new_n58_));
  aoi21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  oai21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(x05), .b(new_n49_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(x06), .c(new_n51_), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n51_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x05), .c(new_n49_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n62_), .c(new_n58_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n60_), .O(new_n66_));
  xnor2a g022(.a(x07), .b(x04), .O(new_n67_));
  inv1   g023(.a(x09), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(x08), .O(new_n69_));
  aoi21  g025(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  oai21  g026(.a(new_n67_), .b(new_n66_), .c(new_n70_), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n47_), .O(z2));
  inv1   g028(.a(x27), .O(new_n73_));
  inv1   g029(.a(x26), .O(new_n74_));
  nand3  g030(.a(new_n59_), .b(new_n74_), .c(x25), .O(new_n75_));
  nor2   g031(.a(x24), .b(x19), .O(new_n76_));
  oai22  g032(.a(x22), .b(x17), .c(x21), .d(x16), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  inv1   g034(.a(x20), .O(new_n79_));
  nor2   g035(.a(x23), .b(x18), .O(new_n80_));
  aoi21  g036(.a(new_n79_), .b(new_n45_), .c(new_n80_), .O(new_n81_));
  aoi21  g037(.a(new_n81_), .b(new_n78_), .c(new_n75_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n73_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n47_), .O(z3));
  oai21  g040(.a(new_n80_), .b(new_n79_), .c(new_n45_), .O(new_n85_));
  inv1   g041(.a(new_n75_), .O(new_n86_));
  inv1   g042(.a(x28), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n73_), .O(new_n88_));
  nor2   g044(.a(new_n87_), .b(new_n73_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n88_), .c(new_n86_), .d(new_n47_), .O(new_n91_));
  aoi21  g047(.a(new_n85_), .b(new_n78_), .c(new_n91_), .O(z4));
  nand2  g048(.a(new_n89_), .b(x29), .O(new_n93_));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n90_), .b(new_n94_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n93_), .c(new_n82_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n47_), .O(z5));
  inv1   g053(.a(x30), .O(new_n98_));
  nand2  g054(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nand3  g055(.a(new_n89_), .b(x30), .c(x29), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n99_), .c(new_n82_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n47_), .O(z6));
  xor2a  g058(.a(new_n100_), .b(x31), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n82_), .c(new_n46_), .O(z7));
  oai21  g060(.a(x28), .b(x27), .c(x29), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n98_), .O(new_n106_));
  inv1   g062(.a(x19), .O(new_n107_));
  inv1   g063(.a(x31), .O(new_n108_));
  nand2  g064(.a(x17), .b(x16), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(new_n107_), .c(new_n108_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n106_), .O(new_n111_));
  nand3  g067(.a(new_n88_), .b(x30), .c(x29), .O(new_n112_));
  nand2  g068(.a(x17), .b(x11), .O(new_n113_));
  oai21  g069(.a(x17), .b(x12), .c(x16), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g071(.a(new_n107_), .b(x14), .O(new_n116_));
  nand3  g072(.a(x17), .b(x16), .c(x15), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n115_), .c(new_n112_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n111_), .c(x18), .O(new_n121_));
  nand2  g077(.a(new_n112_), .b(new_n106_), .O(new_n122_));
  nand3  g078(.a(x17), .b(x16), .c(x10), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  inv1   g080(.a(x18), .O(new_n125_));
  nor2   g081(.a(new_n108_), .b(new_n125_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n124_), .c(x19), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n122_), .c(new_n45_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n121_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x32), .O(new_n130_));
  xor2a  g086(.a(new_n105_), .b(new_n98_), .O(new_n131_));
  inv1   g087(.a(x16), .O(new_n132_));
  nand3  g088(.a(new_n107_), .b(x17), .c(x14), .O(new_n133_));
  inv1   g089(.a(x17), .O(new_n134_));
  nand3  g090(.a(x19), .b(new_n134_), .c(x12), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n133_), .c(new_n132_), .O(new_n136_));
  nand4  g092(.a(x19), .b(x17), .c(new_n132_), .d(x11), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(x15), .O(new_n139_));
  nand3  g095(.a(new_n124_), .b(x19), .c(new_n45_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n126_), .c(new_n131_), .d(new_n54_), .O(new_n142_));
  nand2  g098(.a(new_n74_), .b(x00), .O(new_n143_));
  aoi21  g099(.a(new_n142_), .b(new_n130_), .c(new_n143_), .O(z8));
  oai21  g100(.a(new_n143_), .b(new_n50_), .c(new_n45_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n125_), .O(new_n146_));
  nand4  g102(.a(new_n124_), .b(x19), .c(x18), .d(new_n45_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n139_), .O(new_n148_));
  xor2a  g104(.a(x31), .b(x30), .O(new_n149_));
  nor2   g105(.a(new_n149_), .b(new_n131_), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n148_), .c(x33), .O(new_n151_));
  inv1   g107(.a(new_n143_), .O(new_n152_));
  inv1   g108(.a(new_n150_), .O(new_n153_));
  aoi21  g109(.a(new_n114_), .b(new_n113_), .c(new_n50_), .O(new_n154_));
  nand2  g110(.a(new_n123_), .b(new_n45_), .O(new_n155_));
  nand2  g111(.a(new_n117_), .b(new_n107_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n155_), .c(new_n154_), .d(new_n119_), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n153_), .c(new_n152_), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n151_), .c(new_n146_), .O(z9));
endmodule


