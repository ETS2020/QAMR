// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_;
  inv1   g000(.a(x14), .O(new_n45_));
  inv1   g001(.a(x18), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  inv1   g005(.a(x33), .O(new_n50_));
  xnor2a g006(.a(x03), .b(x02), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  or2    g008(.a(new_n51_), .b(x32), .O(new_n53_));
  nand3  g009(.a(new_n53_), .b(new_n52_), .c(new_n48_), .O(z1));
  inv1   g010(.a(x01), .O(new_n55_));
  aoi21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  oai21  g012(.a(x03), .b(x02), .c(new_n56_), .O(new_n57_));
  inv1   g013(.a(x03), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(new_n64_));
  xor2a  g020(.a(x07), .b(x04), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand3  g023(.a(new_n48_), .b(new_n67_), .c(x08), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n66_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  nor2   g026(.a(x24), .b(x19), .O(new_n71_));
  nor2   g027(.a(x22), .b(x17), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g029(.a(x16), .O(new_n74_));
  inv1   g030(.a(x21), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x20), .b(x15), .O(new_n77_));
  nor2   g033(.a(x23), .b(x18), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(new_n80_));
  inv1   g036(.a(x26), .O(new_n81_));
  nand3  g037(.a(new_n56_), .b(new_n81_), .c(x25), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n80_), .c(new_n70_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n48_), .O(z3));
  nor2   g041(.a(new_n82_), .b(new_n47_), .O(new_n86_));
  and2   g042(.a(new_n86_), .b(new_n80_), .O(new_n87_));
  inv1   g043(.a(x28), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n70_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n88_), .b(new_n70_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n90_), .c(new_n87_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z4));
  nand2  g049(.a(new_n89_), .b(x29), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(new_n95_));
  nor2   g051(.a(new_n89_), .b(x29), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n87_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z5));
  inv1   g055(.a(x30), .O(new_n100_));
  nand2  g056(.a(new_n94_), .b(new_n100_), .O(new_n101_));
  nand2  g057(.a(new_n95_), .b(x30), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n101_), .c(new_n87_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(z6));
  xor2a  g060(.a(new_n102_), .b(x31), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n87_), .O(z7));
  nand2  g062(.a(new_n81_), .b(x00), .O(new_n107_));
  oai21  g063(.a(x28), .b(x27), .c(x29), .O(new_n108_));
  xor2a  g064(.a(new_n108_), .b(new_n100_), .O(new_n109_));
  inv1   g065(.a(x17), .O(new_n110_));
  nand2  g066(.a(x16), .b(x15), .O(new_n111_));
  aoi22  g067(.a(x16), .b(x10), .c(x15), .d(x11), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n110_), .c(new_n111_), .O(new_n113_));
  nand2  g069(.a(x31), .b(x19), .O(new_n114_));
  nand2  g070(.a(new_n110_), .b(x12), .O(new_n115_));
  nand3  g071(.a(x18), .b(x16), .c(x15), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n113_), .c(new_n109_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n45_), .O(new_n120_));
  xor2a  g076(.a(new_n108_), .b(x30), .O(new_n121_));
  inv1   g077(.a(new_n111_), .O(new_n122_));
  inv1   g078(.a(x13), .O(new_n123_));
  nor2   g079(.a(new_n110_), .b(new_n123_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n122_), .c(x31), .d(x19), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n121_), .c(new_n46_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n120_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x32), .O(new_n128_));
  nand3  g084(.a(new_n46_), .b(x17), .c(x13), .O(new_n129_));
  nor2   g085(.a(new_n129_), .b(new_n111_), .O(new_n130_));
  nand3  g086(.a(x17), .b(new_n74_), .c(x11), .O(new_n131_));
  oai21  g087(.a(new_n115_), .b(new_n74_), .c(new_n131_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x15), .O(new_n133_));
  inv1   g089(.a(x15), .O(new_n134_));
  nand2  g090(.a(x16), .b(x10), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(x17), .c(new_n134_), .O(new_n137_));
  nand2  g093(.a(x18), .b(new_n45_), .O(new_n138_));
  aoi21  g094(.a(new_n137_), .b(new_n133_), .c(new_n138_), .O(new_n139_));
  nor3   g095(.a(new_n121_), .b(new_n114_), .c(x32), .O(new_n140_));
  oai21  g096(.a(new_n139_), .b(new_n130_), .c(new_n140_), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(new_n128_), .c(new_n107_), .O(z8));
  inv1   g098(.a(new_n107_), .O(new_n143_));
  nand2  g099(.a(new_n117_), .b(new_n115_), .O(new_n144_));
  oai21  g100(.a(new_n135_), .b(new_n46_), .c(new_n134_), .O(new_n145_));
  nand2  g101(.a(new_n116_), .b(new_n110_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n145_), .c(new_n144_), .O(new_n147_));
  inv1   g103(.a(new_n147_), .O(new_n148_));
  xor2a  g104(.a(new_n108_), .b(new_n100_), .O(new_n149_));
  xnor2a g105(.a(x31), .b(x30), .O(new_n150_));
  nand2  g106(.a(x18), .b(x11), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n74_), .O(new_n152_));
  nand2  g108(.a(new_n46_), .b(new_n123_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n152_), .c(new_n150_), .d(x19), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n149_), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n148_), .c(new_n50_), .O(new_n156_));
  nand3  g112(.a(x18), .b(new_n110_), .c(x12), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(new_n129_), .c(new_n74_), .O(new_n158_));
  nand4  g114(.a(x18), .b(x17), .c(new_n74_), .d(x11), .O(new_n159_));
  inv1   g115(.a(new_n159_), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n158_), .c(x15), .O(new_n161_));
  inv1   g117(.a(new_n137_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(x18), .O(new_n163_));
  nand4  g119(.a(new_n150_), .b(new_n121_), .c(new_n50_), .d(x19), .O(new_n164_));
  aoi21  g120(.a(new_n163_), .b(new_n161_), .c(new_n164_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n156_), .c(new_n143_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n48_), .O(z9));
endmodule


