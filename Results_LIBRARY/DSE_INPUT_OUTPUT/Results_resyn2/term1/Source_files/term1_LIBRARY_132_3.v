// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  nand2  g000(.a(x30), .b(x29), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  inv1   g008(.a(x32), .O(new_n53_));
  aoi21  g009(.a(x33), .b(new_n48_), .c(x02), .O(new_n54_));
  oai21  g010(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n46_), .O(z1));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nand2  g014(.a(new_n48_), .b(new_n50_), .O(new_n59_));
  nand2  g015(.a(x03), .b(x02), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g017(.a(x05), .b(new_n48_), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n50_), .O(new_n63_));
  xor2a  g019(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n58_), .c(new_n61_), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n57_), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand3  g023(.a(new_n45_), .b(new_n67_), .c(x08), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n66_), .O(z2));
  nor2   g025(.a(x20), .b(x15), .O(new_n70_));
  nor2   g026(.a(x24), .b(x19), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x18), .O(new_n73_));
  inv1   g029(.a(x23), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x22), .b(x17), .O(new_n76_));
  nor2   g032(.a(x21), .b(x16), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n60_), .O(new_n80_));
  inv1   g036(.a(x27), .O(new_n81_));
  inv1   g037(.a(x26), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(x25), .c(x01), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n80_), .c(new_n45_), .O(z3));
  inv1   g042(.a(x28), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n81_), .O(new_n88_));
  nand2  g044(.a(x28), .b(x27), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n88_), .c(new_n84_), .d(new_n45_), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n80_), .O(z4));
  nand3  g047(.a(x29), .b(x28), .c(x27), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  oai21  g049(.a(x30), .b(new_n93_), .c(new_n89_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n92_), .c(new_n84_), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(new_n80_), .O(z5));
  inv1   g052(.a(x30), .O(new_n97_));
  nand2  g053(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n84_), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n80_), .c(new_n45_), .O(z6));
  inv1   g056(.a(new_n80_), .O(new_n101_));
  inv1   g057(.a(x31), .O(new_n102_));
  nand2  g058(.a(new_n84_), .b(new_n102_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  aoi21  g060(.a(new_n104_), .b(new_n101_), .c(new_n46_), .O(z7));
  nand2  g061(.a(new_n82_), .b(x00), .O(new_n106_));
  inv1   g062(.a(x15), .O(new_n107_));
  inv1   g063(.a(x17), .O(new_n108_));
  inv1   g064(.a(x19), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x18), .c(x14), .O(new_n110_));
  nand3  g066(.a(x19), .b(new_n73_), .c(x13), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n110_), .c(new_n108_), .O(new_n112_));
  nand4  g068(.a(x19), .b(x18), .c(new_n108_), .d(x12), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  inv1   g071(.a(x16), .O(new_n116_));
  nand3  g072(.a(x18), .b(x17), .c(x11), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x19), .c(new_n116_), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n115_), .c(new_n107_), .O(new_n120_));
  nand4  g076(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n121_));
  nor3   g077(.a(new_n121_), .b(new_n109_), .c(x15), .O(new_n122_));
  nand2  g078(.a(new_n88_), .b(x29), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(new_n97_), .c(new_n102_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(z0), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  oai21  g082(.a(new_n122_), .b(new_n120_), .c(new_n126_), .O(new_n127_));
  oai21  g083(.a(new_n118_), .b(x16), .c(x15), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n121_), .O(new_n129_));
  nand4  g085(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n130_));
  aoi21  g086(.a(new_n109_), .b(x14), .c(new_n130_), .O(new_n131_));
  aoi21  g087(.a(new_n130_), .b(new_n109_), .c(new_n131_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  nand2  g089(.a(x18), .b(x12), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n108_), .O(new_n135_));
  inv1   g091(.a(x13), .O(new_n136_));
  nand2  g092(.a(new_n73_), .b(new_n136_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(new_n124_), .O(new_n138_));
  nor2   g094(.a(new_n46_), .b(new_n53_), .O(new_n139_));
  oai21  g095(.a(new_n138_), .b(new_n133_), .c(new_n139_), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n127_), .c(new_n106_), .O(z8));
  nand2  g097(.a(new_n49_), .b(new_n102_), .O(new_n142_));
  aoi21  g098(.a(new_n88_), .b(x29), .c(new_n142_), .O(new_n143_));
  oai21  g099(.a(new_n122_), .b(new_n120_), .c(new_n143_), .O(new_n144_));
  nand4  g100(.a(new_n137_), .b(new_n135_), .c(new_n123_), .d(new_n102_), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n132_), .c(new_n129_), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(x33), .c(x30), .O(new_n148_));
  oai21  g104(.a(new_n49_), .b(x29), .c(x30), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n82_), .c(x00), .O(new_n150_));
  aoi21  g106(.a(new_n148_), .b(new_n144_), .c(new_n150_), .O(z9));
endmodule


