// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x13), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x33), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(x32), .b(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(x33), .b(new_n49_), .c(x02), .O(new_n53_));
  oai21  g009(.a(new_n52_), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  inv1   g012(.a(x01), .O(new_n57_));
  aoi21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  oai21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  inv1   g015(.a(x02), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n60_), .O(new_n61_));
  aoi21  g017(.a(x05), .b(new_n49_), .c(new_n61_), .O(new_n62_));
  nand3  g018(.a(new_n61_), .b(x05), .c(new_n49_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n62_), .c(new_n59_), .O(new_n65_));
  xor2a  g021(.a(x07), .b(x04), .O(new_n66_));
  xor2a  g022(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nor4   g023(.a(new_n67_), .b(new_n46_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g024(.a(new_n46_), .O(new_n69_));
  nor2   g025(.a(x23), .b(x18), .O(new_n70_));
  nor2   g026(.a(x22), .b(x17), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x19), .O(new_n73_));
  inv1   g029(.a(x24), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(x20), .b(x15), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(new_n79_));
  inv1   g035(.a(x26), .O(new_n80_));
  nand3  g036(.a(new_n58_), .b(new_n80_), .c(x25), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(x27), .c(new_n69_), .O(z3));
  inv1   g040(.a(x27), .O(new_n85_));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n85_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g046(.a(new_n90_), .b(new_n83_), .c(new_n69_), .O(z4));
  inv1   g047(.a(new_n83_), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  nand2  g049(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand2  g050(.a(new_n87_), .b(x29), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n94_), .c(new_n92_), .d(new_n69_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(z5));
  nand3  g053(.a(new_n87_), .b(x30), .c(x29), .O(new_n98_));
  nand2  g054(.a(x30), .b(x13), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n95_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n98_), .c(new_n92_), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(z6));
  xor2a  g058(.a(new_n98_), .b(x31), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n92_), .c(new_n46_), .O(z7));
  inv1   g060(.a(x00), .O(new_n105_));
  nor2   g061(.a(x26), .b(new_n105_), .O(new_n106_));
  inv1   g062(.a(x15), .O(new_n107_));
  inv1   g063(.a(x17), .O(new_n108_));
  nand3  g064(.a(new_n73_), .b(x18), .c(x14), .O(new_n109_));
  inv1   g065(.a(x18), .O(new_n110_));
  nand2  g066(.a(x19), .b(new_n110_), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  nand4  g068(.a(x19), .b(x18), .c(new_n108_), .d(x12), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  inv1   g071(.a(x16), .O(new_n116_));
  and2   g072(.a(x17), .b(x11), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x19), .c(new_n116_), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x18), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n115_), .c(new_n107_), .O(new_n121_));
  nand3  g077(.a(x17), .b(x16), .c(x10), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x18), .O(new_n124_));
  nand2  g080(.a(x19), .b(new_n107_), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  inv1   g082(.a(x31), .O(new_n127_));
  nor2   g083(.a(new_n127_), .b(new_n45_), .O(new_n128_));
  oai21  g084(.a(new_n126_), .b(new_n121_), .c(new_n128_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n52_), .O(new_n130_));
  nand2  g086(.a(new_n89_), .b(x29), .O(new_n131_));
  nand2  g087(.a(x32), .b(x30), .O(new_n132_));
  and2   g088(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  inv1   g090(.a(new_n131_), .O(new_n135_));
  nand2  g091(.a(new_n120_), .b(new_n115_), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(x15), .c(new_n126_), .O(new_n137_));
  inv1   g093(.a(x13), .O(new_n138_));
  and2   g094(.a(new_n109_), .b(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n120_), .c(new_n113_), .O(new_n140_));
  nand3  g096(.a(new_n52_), .b(x31), .c(new_n45_), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n140_), .b(new_n126_), .c(new_n142_), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n137_), .c(new_n132_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n135_), .O(new_n145_));
  inv1   g101(.a(x12), .O(new_n146_));
  nor2   g102(.a(x18), .b(x13), .O(new_n147_));
  oai22  g103(.a(new_n147_), .b(new_n108_), .c(new_n110_), .d(new_n146_), .O(new_n148_));
  aoi22  g104(.a(new_n148_), .b(x16), .c(new_n117_), .d(x18), .O(new_n149_));
  nand3  g105(.a(x17), .b(x16), .c(x15), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x18), .O(new_n152_));
  nand2  g108(.a(new_n73_), .b(x14), .O(new_n153_));
  nor2   g109(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  aoi21  g110(.a(new_n152_), .b(x19), .c(new_n154_), .O(new_n155_));
  nand2  g111(.a(new_n124_), .b(new_n107_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(x31), .O(new_n157_));
  or2    g113(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n149_), .c(x32), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n145_), .c(new_n134_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n106_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n69_), .O(z8));
  inv1   g118(.a(new_n111_), .O(new_n163_));
  nor2   g119(.a(x31), .b(x30), .O(new_n164_));
  nand4  g120(.a(new_n164_), .b(new_n151_), .c(new_n131_), .d(new_n163_), .O(new_n165_));
  oai21  g121(.a(new_n131_), .b(new_n129_), .c(new_n165_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(x13), .O(new_n167_));
  nand3  g123(.a(x19), .b(new_n108_), .c(x12), .O(new_n168_));
  nand3  g124(.a(new_n73_), .b(x17), .c(x14), .O(new_n169_));
  aoi21  g125(.a(new_n169_), .b(new_n168_), .c(new_n116_), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n119_), .c(x15), .O(new_n171_));
  oai21  g127(.a(new_n125_), .b(new_n122_), .c(new_n171_), .O(new_n172_));
  nand4  g128(.a(new_n131_), .b(new_n127_), .c(new_n45_), .d(x18), .O(new_n173_));
  inv1   g129(.a(new_n173_), .O(new_n174_));
  aoi21  g130(.a(new_n174_), .b(new_n172_), .c(x33), .O(new_n175_));
  aoi21  g131(.a(new_n108_), .b(new_n146_), .c(new_n116_), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n117_), .c(x15), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n122_), .O(new_n178_));
  aoi21  g134(.a(new_n150_), .b(new_n110_), .c(new_n155_), .O(new_n179_));
  aoi21  g135(.a(new_n179_), .b(new_n178_), .c(new_n46_), .O(new_n180_));
  nand2  g136(.a(new_n131_), .b(new_n127_), .O(new_n181_));
  oai21  g137(.a(new_n181_), .b(new_n147_), .c(new_n45_), .O(new_n182_));
  inv1   g138(.a(new_n99_), .O(new_n183_));
  oai21  g139(.a(new_n131_), .b(new_n127_), .c(new_n183_), .O(new_n184_));
  nand3  g140(.a(new_n184_), .b(new_n182_), .c(x33), .O(new_n185_));
  oai21  g141(.a(new_n185_), .b(new_n180_), .c(new_n106_), .O(new_n186_));
  aoi21  g142(.a(new_n175_), .b(new_n167_), .c(new_n186_), .O(z9));
endmodule


