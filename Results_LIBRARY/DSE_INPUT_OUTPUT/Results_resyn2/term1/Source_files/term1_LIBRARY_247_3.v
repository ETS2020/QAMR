// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  inv1   g000(.a(x20), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  xor2a  g005(.a(x03), .b(x02), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  aoi21  g007(.a(new_n50_), .b(new_n51_), .c(new_n47_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(x33), .c(new_n52_), .O(z1));
  inv1   g009(.a(x08), .O(new_n54_));
  xor2a  g010(.a(x07), .b(x04), .O(new_n55_));
  nand2  g011(.a(new_n50_), .b(x01), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n57_), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  xor2a  g016(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  oai21  g017(.a(new_n61_), .b(x01), .c(new_n56_), .O(new_n62_));
  xor2a  g018(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  nor4   g019(.a(new_n63_), .b(new_n47_), .c(x09), .d(new_n54_), .O(z2));
  inv1   g020(.a(x27), .O(new_n65_));
  oai22  g021(.a(x24), .b(x19), .c(x21), .d(x16), .O(new_n66_));
  oai22  g022(.a(x23), .b(x18), .c(x22), .d(x17), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  oai21  g024(.a(x20), .b(x15), .c(new_n68_), .O(new_n69_));
  and2   g025(.a(new_n69_), .b(new_n48_), .O(new_n70_));
  inv1   g026(.a(x26), .O(new_n71_));
  nand2  g027(.a(x03), .b(x02), .O(new_n72_));
  nand4  g028(.a(new_n72_), .b(new_n71_), .c(x25), .d(x01), .O(new_n73_));
  inv1   g029(.a(new_n73_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(new_n70_), .c(new_n65_), .O(new_n75_));
  inv1   g031(.a(new_n75_), .O(z3));
  inv1   g032(.a(x28), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n65_), .O(new_n78_));
  nor2   g034(.a(new_n77_), .b(new_n65_), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n78_), .c(new_n74_), .d(new_n70_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(z4));
  and2   g038(.a(new_n74_), .b(new_n69_), .O(new_n83_));
  nand2  g039(.a(new_n79_), .b(x29), .O(new_n84_));
  inv1   g040(.a(x29), .O(new_n85_));
  nand2  g041(.a(new_n80_), .b(new_n85_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n48_), .O(z5));
  inv1   g044(.a(new_n84_), .O(new_n89_));
  aoi21  g045(.a(new_n68_), .b(x15), .c(new_n46_), .O(new_n90_));
  aoi21  g046(.a(new_n89_), .b(new_n69_), .c(new_n90_), .O(new_n91_));
  nand2  g047(.a(new_n89_), .b(x30), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n74_), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n91_), .c(new_n48_), .O(z6));
  nand2  g050(.a(new_n92_), .b(x31), .O(new_n95_));
  inv1   g051(.a(x31), .O(new_n96_));
  nand3  g052(.a(new_n89_), .b(new_n96_), .c(x30), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n95_), .c(new_n83_), .d(new_n48_), .O(z7));
  inv1   g054(.a(x17), .O(new_n99_));
  inv1   g055(.a(x19), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(x18), .c(x14), .O(new_n101_));
  inv1   g057(.a(x18), .O(new_n102_));
  nand3  g058(.a(x19), .b(new_n102_), .c(x13), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n101_), .c(new_n99_), .O(new_n104_));
  nand4  g060(.a(x19), .b(x18), .c(new_n99_), .d(x12), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n104_), .c(x16), .O(new_n107_));
  inv1   g063(.a(x16), .O(new_n108_));
  nand3  g064(.a(x18), .b(x17), .c(x11), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x19), .c(new_n108_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n107_), .O(new_n112_));
  nor2   g068(.a(new_n102_), .b(new_n99_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x16), .c(x10), .O(new_n114_));
  nor3   g070(.a(new_n114_), .b(new_n100_), .c(x15), .O(new_n115_));
  aoi21  g071(.a(new_n112_), .b(x15), .c(new_n115_), .O(new_n116_));
  nand2  g072(.a(new_n78_), .b(x29), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n46_), .O(new_n118_));
  inv1   g074(.a(new_n117_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x30), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(new_n118_), .c(x31), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n116_), .c(new_n51_), .O(new_n122_));
  nand2  g078(.a(new_n71_), .b(x00), .O(new_n123_));
  nand3  g079(.a(new_n113_), .b(x16), .c(x15), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  aoi21  g081(.a(new_n109_), .b(new_n108_), .c(new_n100_), .O(new_n126_));
  nor2   g082(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g083(.a(x18), .b(x12), .O(new_n128_));
  oai21  g084(.a(x18), .b(x13), .c(x17), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g086(.a(new_n100_), .b(x14), .O(new_n131_));
  nand2  g087(.a(new_n125_), .b(new_n131_), .O(new_n132_));
  inv1   g088(.a(x15), .O(new_n133_));
  nand2  g089(.a(new_n114_), .b(new_n133_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n132_), .c(new_n130_), .O(new_n135_));
  nor2   g091(.a(new_n135_), .b(new_n127_), .O(new_n136_));
  nand4  g092(.a(new_n120_), .b(new_n118_), .c(x32), .d(x31), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n136_), .c(new_n123_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n122_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n48_), .O(z8));
  nand3  g097(.a(new_n117_), .b(new_n96_), .c(new_n46_), .O(new_n142_));
  oai21  g098(.a(new_n120_), .b(new_n96_), .c(new_n142_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n136_), .O(new_n144_));
  inv1   g100(.a(x33), .O(new_n145_));
  nor2   g101(.a(new_n47_), .b(new_n145_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  aoi21  g103(.a(new_n111_), .b(new_n107_), .c(new_n133_), .O(new_n148_));
  nand4  g104(.a(new_n119_), .b(x31), .c(x30), .d(new_n45_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n142_), .c(x33), .O(new_n150_));
  oai21  g106(.a(new_n115_), .b(new_n148_), .c(new_n150_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n147_), .c(new_n123_), .O(z9));
endmodule


