// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x00), .O(new_n46_));
  nand2  g02(.a(new_n46_), .b(x32), .O(z0));
  inv1   g03(.a(x33), .O(new_n48_));
  xnor2a g04(.a(x03), .b(x02), .O(new_n49_));
  nand2  g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  or2    g06(.a(new_n49_), .b(x32), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(new_n50_), .c(new_n46_), .O(z1));
  xor2a  g08(.a(x07), .b(x04), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  inv1   g10(.a(x01), .O(new_n55_));
  inv1   g11(.a(x02), .O(new_n56_));
  inv1   g12(.a(x03), .O(new_n57_));
  nand4  g13(.a(x06), .b(x05), .c(new_n57_), .d(new_n56_), .O(new_n58_));
  nand2  g14(.a(x05), .b(new_n57_), .O(new_n59_));
  nand2  g15(.a(x06), .b(new_n56_), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g17(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(new_n62_));
  nand2  g18(.a(new_n49_), .b(x01), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n54_), .O(new_n65_));
  nand3  g21(.a(new_n63_), .b(new_n62_), .c(new_n53_), .O(new_n66_));
  inv1   g22(.a(x09), .O(new_n67_));
  nand3  g23(.a(new_n46_), .b(new_n67_), .c(x08), .O(new_n68_));
  aoi21  g24(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(z2));
  inv1   g25(.a(x27), .O(new_n70_));
  nor2   g26(.a(x23), .b(x18), .O(new_n71_));
  nor2   g27(.a(x22), .b(x17), .O(new_n72_));
  nor2   g28(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  or2    g29(.a(x24), .b(x19), .O(new_n74_));
  nor2   g30(.a(x21), .b(x16), .O(new_n75_));
  nor2   g31(.a(x20), .b(x15), .O(new_n76_));
  nor2   g32(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(new_n74_), .c(new_n73_), .O(new_n78_));
  nand2  g34(.a(x25), .b(x01), .O(new_n79_));
  aoi21  g35(.a(x03), .b(x02), .c(new_n79_), .O(new_n80_));
  nor2   g36(.a(x26), .b(x00), .O(new_n81_));
  nand4  g37(.a(new_n81_), .b(new_n80_), .c(new_n78_), .d(new_n70_), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(z3));
  xor2a  g39(.a(x28), .b(x27), .O(new_n84_));
  nand4  g40(.a(new_n84_), .b(new_n81_), .c(new_n80_), .d(new_n78_), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(z4));
  inv1   g42(.a(x00), .O(new_n87_));
  nand3  g43(.a(x29), .b(x28), .c(x27), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  aoi21  g45(.a(x28), .b(x27), .c(x29), .O(new_n90_));
  nor2   g46(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(new_n80_), .c(new_n78_), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n87_), .c(x26), .O(z5));
  inv1   g49(.a(x30), .O(new_n94_));
  nand2  g50(.a(new_n88_), .b(new_n94_), .O(new_n95_));
  nand2  g51(.a(new_n89_), .b(x30), .O(new_n96_));
  nand4  g52(.a(new_n96_), .b(new_n95_), .c(new_n80_), .d(new_n78_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n87_), .c(x26), .O(z6));
  xor2a  g54(.a(new_n96_), .b(x31), .O(new_n99_));
  nand4  g55(.a(new_n99_), .b(new_n81_), .c(new_n80_), .d(new_n78_), .O(z7));
  inv1   g56(.a(new_n46_), .O(z8));
  zero   g57(.O(z9));
endmodule


