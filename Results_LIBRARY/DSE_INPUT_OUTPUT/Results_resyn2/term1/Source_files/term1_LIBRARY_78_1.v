// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  aoi21  g007(.a(new_n50_), .b(new_n51_), .c(new_n46_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(x33), .c(new_n52_), .O(z1));
  xnor2a g009(.a(x07), .b(x04), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  inv1   g011(.a(x02), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g014(.a(x03), .b(x02), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nand2  g017(.a(new_n45_), .b(new_n61_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n57_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n56_), .O(new_n64_));
  xnor2a g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(new_n66_));
  oai21  g022(.a(new_n66_), .b(new_n62_), .c(new_n60_), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n55_), .O(new_n68_));
  nand2  g024(.a(new_n65_), .b(new_n61_), .O(new_n69_));
  nand4  g025(.a(new_n69_), .b(new_n60_), .c(new_n54_), .d(new_n47_), .O(new_n70_));
  inv1   g026(.a(x09), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(x08), .O(new_n72_));
  aoi21  g028(.a(new_n70_), .b(new_n68_), .c(new_n72_), .O(z2));
  inv1   g029(.a(x27), .O(new_n74_));
  nor2   g030(.a(x23), .b(x18), .O(new_n75_));
  nor2   g031(.a(x22), .b(x17), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  inv1   g033(.a(x19), .O(new_n78_));
  inv1   g034(.a(x24), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nor2   g036(.a(x21), .b(x16), .O(new_n81_));
  nor2   g037(.a(x20), .b(x15), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n80_), .c(new_n77_), .O(new_n84_));
  inv1   g040(.a(x25), .O(new_n85_));
  nor2   g041(.a(x26), .b(new_n85_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n59_), .c(x01), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  and2   g044(.a(new_n88_), .b(new_n84_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n74_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z3));
  inv1   g047(.a(x28), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(new_n74_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nand2  g050(.a(new_n92_), .b(new_n74_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n94_), .c(new_n89_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n47_), .O(z4));
  nand2  g053(.a(new_n93_), .b(x29), .O(new_n98_));
  inv1   g054(.a(x29), .O(new_n99_));
  nand2  g055(.a(new_n94_), .b(new_n99_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n98_), .c(new_n89_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n47_), .O(z5));
  inv1   g058(.a(x30), .O(new_n103_));
  nand2  g059(.a(new_n98_), .b(new_n103_), .O(new_n104_));
  nand3  g060(.a(new_n93_), .b(x30), .c(x29), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n104_), .c(new_n89_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(z6));
  nand3  g063(.a(new_n86_), .b(new_n84_), .c(new_n59_), .O(new_n108_));
  inv1   g064(.a(x31), .O(new_n109_));
  xor2a  g065(.a(new_n105_), .b(new_n109_), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n108_), .c(x01), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n62_), .O(z7));
  inv1   g068(.a(x26), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x00), .O(new_n114_));
  nand3  g070(.a(x17), .b(x16), .c(x15), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x19), .c(x13), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(x28), .b(x27), .c(x29), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(x30), .c(x31), .O(new_n121_));
  aoi21  g077(.a(new_n120_), .b(x30), .c(new_n121_), .O(new_n122_));
  aoi21  g078(.a(new_n122_), .b(new_n118_), .c(x18), .O(new_n123_));
  nand2  g079(.a(new_n78_), .b(x14), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(new_n116_), .c(x18), .O(new_n125_));
  inv1   g081(.a(x15), .O(new_n126_));
  nand3  g082(.a(x17), .b(x16), .c(x10), .O(new_n127_));
  aoi22  g083(.a(new_n127_), .b(new_n126_), .c(new_n115_), .d(new_n78_), .O(new_n128_));
  and2   g084(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  nand2  g085(.a(x16), .b(x12), .O(new_n130_));
  oai21  g086(.a(x16), .b(x11), .c(x17), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n129_), .c(new_n122_), .O(new_n133_));
  and2   g089(.a(new_n133_), .b(x01), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n123_), .c(x32), .O(new_n135_));
  nor2   g091(.a(new_n117_), .b(x18), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  inv1   g093(.a(x16), .O(new_n138_));
  nand3  g094(.a(new_n78_), .b(x17), .c(x14), .O(new_n139_));
  inv1   g095(.a(x17), .O(new_n140_));
  nand3  g096(.a(x19), .b(new_n140_), .c(x12), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(new_n139_), .c(new_n138_), .O(new_n142_));
  nand4  g098(.a(x19), .b(x17), .c(new_n138_), .d(x11), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n142_), .c(x15), .O(new_n145_));
  inv1   g101(.a(new_n127_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(x19), .c(new_n126_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x18), .c(x01), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n137_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n122_), .c(new_n51_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n135_), .c(new_n114_), .O(z8));
  nand2  g108(.a(x33), .b(x30), .O(new_n153_));
  nand2  g109(.a(new_n109_), .b(new_n103_), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(x33), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n118_), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n153_), .c(x18), .O(new_n157_));
  nand3  g113(.a(new_n155_), .b(new_n148_), .c(x18), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n153_), .c(new_n61_), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n157_), .c(new_n119_), .O(new_n160_));
  inv1   g116(.a(x33), .O(new_n161_));
  nand2  g117(.a(x31), .b(x30), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n154_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n118_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n45_), .O(new_n165_));
  nand3  g121(.a(new_n163_), .b(new_n132_), .c(new_n129_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(x01), .O(new_n167_));
  aoi21  g123(.a(new_n167_), .b(new_n165_), .c(new_n161_), .O(new_n168_));
  nor2   g124(.a(new_n162_), .b(x33), .O(new_n169_));
  inv1   g125(.a(new_n169_), .O(new_n170_));
  nor3   g126(.a(new_n46_), .b(new_n161_), .c(x30), .O(new_n171_));
  aoi21  g127(.a(new_n169_), .b(new_n136_), .c(new_n171_), .O(new_n172_));
  oai21  g128(.a(new_n170_), .b(new_n149_), .c(new_n172_), .O(new_n173_));
  aoi21  g129(.a(new_n173_), .b(new_n120_), .c(new_n168_), .O(new_n174_));
  aoi21  g130(.a(new_n174_), .b(new_n160_), .c(new_n114_), .O(z9));
endmodule


