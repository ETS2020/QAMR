// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  nand2  g000(.a(x33), .b(x30), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(x03), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  aoi21  g004(.a(x33), .b(new_n47_), .c(x02), .O(new_n49_));
  oai21  g005(.a(new_n48_), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  inv1   g006(.a(x33), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  aoi21  g008(.a(x32), .b(new_n47_), .c(new_n52_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z1));
  xor2a  g011(.a(x07), .b(x04), .O(new_n56_));
  inv1   g012(.a(x01), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n47_), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(x06), .c(new_n52_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n52_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x05), .c(new_n47_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(new_n59_), .c(new_n57_), .O(new_n62_));
  nand2  g018(.a(x03), .b(x02), .O(new_n63_));
  nand2  g019(.a(new_n47_), .b(new_n52_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n63_), .c(x01), .O(new_n65_));
  and2   g021(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nor2   g022(.a(new_n66_), .b(new_n56_), .O(new_n67_));
  inv1   g023(.a(x09), .O(new_n68_));
  nand2  g024(.a(new_n66_), .b(new_n56_), .O(new_n69_));
  nand3  g025(.a(new_n69_), .b(new_n68_), .c(x08), .O(new_n70_));
  oai21  g026(.a(new_n70_), .b(new_n67_), .c(new_n45_), .O(z2));
  nand2  g027(.a(x25), .b(x01), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(x26), .O(new_n73_));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  nor2   g030(.a(x24), .b(x19), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g032(.a(x18), .O(new_n77_));
  inv1   g033(.a(x23), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g035(.a(x22), .b(x17), .O(new_n80_));
  nor2   g036(.a(x21), .b(x16), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n79_), .c(new_n76_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n63_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n73_), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(x27), .c(new_n45_), .O(z3));
  nand2  g043(.a(new_n73_), .b(new_n45_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  inv1   g045(.a(x27), .O(new_n90_));
  inv1   g046(.a(x28), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand2  g049(.a(new_n91_), .b(new_n90_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n93_), .c(new_n89_), .d(new_n85_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z4));
  nand2  g052(.a(new_n92_), .b(x29), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(new_n98_));
  nor2   g054(.a(new_n92_), .b(x29), .O(new_n99_));
  nor4   g055(.a(new_n99_), .b(new_n98_), .c(new_n88_), .d(new_n84_), .O(z5));
  oai21  g056(.a(new_n98_), .b(x33), .c(x30), .O(new_n101_));
  oai21  g057(.a(new_n98_), .b(x30), .c(new_n101_), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n86_), .O(z6));
  nand2  g059(.a(new_n86_), .b(new_n45_), .O(new_n104_));
  nand2  g060(.a(new_n51_), .b(x30), .O(new_n105_));
  nor3   g061(.a(new_n105_), .b(new_n97_), .c(x31), .O(new_n106_));
  aoi21  g062(.a(new_n101_), .b(x31), .c(new_n106_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n104_), .O(z7));
  nand4  g064(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  inv1   g066(.a(x15), .O(new_n111_));
  nand4  g067(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  oai21  g069(.a(new_n110_), .b(x19), .c(new_n113_), .O(new_n114_));
  inv1   g070(.a(x16), .O(new_n115_));
  nand3  g071(.a(x18), .b(x17), .c(x11), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  inv1   g073(.a(x19), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x14), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n110_), .O(new_n120_));
  inv1   g076(.a(x17), .O(new_n121_));
  nand2  g077(.a(x18), .b(x12), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  inv1   g079(.a(x31), .O(new_n124_));
  nor2   g080(.a(x18), .b(x13), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n123_), .c(new_n120_), .d(new_n117_), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n114_), .c(new_n45_), .O(new_n128_));
  inv1   g084(.a(x30), .O(new_n129_));
  nand2  g085(.a(new_n94_), .b(x29), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n105_), .O(new_n131_));
  aoi21  g087(.a(new_n130_), .b(new_n129_), .c(new_n131_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x32), .O(new_n134_));
  nand3  g090(.a(new_n118_), .b(x18), .c(x14), .O(new_n135_));
  nand3  g091(.a(x19), .b(new_n77_), .c(x13), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n135_), .c(new_n121_), .O(new_n137_));
  nand4  g093(.a(x19), .b(x18), .c(new_n121_), .d(x12), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n137_), .c(x16), .O(new_n140_));
  inv1   g096(.a(new_n116_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(x19), .c(new_n115_), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n140_), .c(new_n111_), .O(new_n143_));
  nor3   g099(.a(new_n112_), .b(new_n118_), .c(x15), .O(new_n144_));
  nand3  g100(.a(new_n130_), .b(new_n51_), .c(x30), .O(new_n145_));
  nand3  g101(.a(new_n94_), .b(new_n129_), .c(x29), .O(new_n146_));
  nand2  g102(.a(new_n48_), .b(x31), .O(new_n147_));
  aoi21  g103(.a(new_n146_), .b(new_n145_), .c(new_n147_), .O(new_n148_));
  oai21  g104(.a(new_n144_), .b(new_n143_), .c(new_n148_), .O(new_n149_));
  inv1   g105(.a(x26), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x00), .O(new_n151_));
  aoi21  g107(.a(new_n149_), .b(new_n134_), .c(new_n151_), .O(z8));
  oai21  g108(.a(new_n144_), .b(new_n143_), .c(new_n146_), .O(new_n153_));
  nand2  g109(.a(new_n51_), .b(x31), .O(new_n154_));
  nand2  g110(.a(x33), .b(new_n124_), .O(new_n155_));
  nor2   g111(.a(new_n155_), .b(new_n125_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n123_), .c(new_n120_), .d(new_n117_), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n114_), .c(new_n154_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n130_), .O(new_n159_));
  aoi21  g115(.a(new_n154_), .b(x30), .c(new_n151_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  aoi21  g117(.a(new_n153_), .b(new_n51_), .c(new_n161_), .O(z9));
endmodule


