// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_;
  inv1   g00(.a(x15), .O(new_n45_));
  inv1   g01(.a(x18), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x32), .O(z0));
  inv1   g05(.a(x03), .O(new_n50_));
  inv1   g06(.a(x33), .O(new_n51_));
  inv1   g07(.a(x02), .O(new_n52_));
  aoi21  g08(.a(x32), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  oai21  g09(.a(new_n51_), .b(new_n50_), .c(new_n53_), .O(new_n54_));
  inv1   g10(.a(x32), .O(new_n55_));
  aoi21  g11(.a(x33), .b(new_n50_), .c(x02), .O(new_n56_));
  oai21  g12(.a(new_n55_), .b(new_n50_), .c(new_n56_), .O(new_n57_));
  aoi21  g13(.a(new_n57_), .b(new_n54_), .c(new_n47_), .O(z1));
  inv1   g14(.a(x01), .O(new_n59_));
  aoi21  g15(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  oai21  g16(.a(x03), .b(x02), .c(new_n60_), .O(new_n61_));
  nand2  g17(.a(x05), .b(new_n50_), .O(new_n62_));
  nand3  g18(.a(new_n62_), .b(x06), .c(new_n52_), .O(new_n63_));
  nand2  g19(.a(x06), .b(new_n52_), .O(new_n64_));
  nand3  g20(.a(new_n64_), .b(x05), .c(new_n50_), .O(new_n65_));
  nand3  g21(.a(new_n65_), .b(new_n63_), .c(new_n59_), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  xnor2a g23(.a(x07), .b(x04), .O(new_n68_));
  inv1   g24(.a(x09), .O(new_n69_));
  nand3  g25(.a(new_n48_), .b(new_n69_), .c(x08), .O(new_n70_));
  aoi21  g26(.a(new_n68_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  oai21  g27(.a(new_n68_), .b(new_n67_), .c(new_n71_), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z2));
  aoi21  g29(.a(x18), .b(new_n45_), .c(x23), .O(new_n74_));
  inv1   g30(.a(x20), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n45_), .O(new_n76_));
  nor2   g32(.a(x21), .b(x16), .O(new_n77_));
  oai22  g33(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  inv1   g36(.a(x26), .O(new_n81_));
  nand3  g37(.a(new_n60_), .b(new_n81_), .c(x25), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  oai21  g39(.a(new_n80_), .b(new_n74_), .c(new_n83_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(x27), .c(new_n48_), .O(z3));
  inv1   g41(.a(x23), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n46_), .O(new_n87_));
  and2   g43(.a(new_n87_), .b(new_n76_), .O(new_n88_));
  oai21  g44(.a(new_n79_), .b(new_n47_), .c(new_n88_), .O(new_n89_));
  nor2   g45(.a(x28), .b(x27), .O(new_n90_));
  and2   g46(.a(x28), .b(x27), .O(new_n91_));
  nor2   g47(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g48(.a(new_n92_), .b(new_n89_), .c(new_n83_), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(z4));
  or2    g50(.a(new_n91_), .b(x29), .O(new_n95_));
  nand2  g51(.a(new_n91_), .b(x29), .O(new_n96_));
  nand4  g52(.a(new_n96_), .b(new_n95_), .c(new_n89_), .d(new_n83_), .O(new_n97_));
  inv1   g53(.a(new_n97_), .O(z5));
  inv1   g54(.a(x30), .O(new_n99_));
  nand2  g55(.a(new_n96_), .b(new_n99_), .O(new_n100_));
  nand3  g56(.a(new_n91_), .b(x30), .c(x29), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n84_), .c(new_n48_), .O(z6));
  nand3  g59(.a(new_n87_), .b(new_n79_), .c(new_n76_), .O(new_n104_));
  nor2   g60(.a(new_n101_), .b(x31), .O(new_n105_));
  nand2  g61(.a(new_n101_), .b(x31), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n83_), .O(new_n107_));
  nor2   g63(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n104_), .c(new_n47_), .O(z7));
  nand2  g65(.a(new_n81_), .b(x00), .O(new_n110_));
  oai21  g66(.a(x28), .b(x27), .c(x29), .O(new_n111_));
  or2    g67(.a(new_n111_), .b(new_n99_), .O(new_n112_));
  nand4  g68(.a(x31), .b(x19), .c(x17), .d(x16), .O(new_n113_));
  aoi21  g69(.a(new_n111_), .b(new_n99_), .c(new_n113_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n112_), .c(new_n47_), .O(new_n115_));
  nand2  g71(.a(x15), .b(x13), .O(new_n116_));
  oai21  g72(.a(x15), .b(x10), .c(x18), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  inv1   g74(.a(new_n118_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n115_), .c(x32), .O(new_n120_));
  nand2  g76(.a(x18), .b(x10), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n116_), .c(new_n47_), .O(new_n122_));
  nand4  g78(.a(new_n122_), .b(new_n114_), .c(new_n112_), .d(new_n55_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n120_), .c(new_n110_), .O(z8));
  inv1   g80(.a(x31), .O(new_n125_));
  aoi21  g81(.a(new_n111_), .b(new_n125_), .c(x30), .O(new_n126_));
  nand3  g82(.a(x19), .b(x17), .c(x16), .O(new_n127_));
  inv1   g83(.a(new_n127_), .O(new_n128_));
  nand2  g84(.a(x31), .b(x29), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n90_), .c(x30), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n126_), .c(new_n48_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n118_), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(x33), .O(new_n134_));
  nor2   g90(.a(new_n131_), .b(new_n126_), .O(new_n135_));
  nand3  g91(.a(new_n135_), .b(new_n122_), .c(new_n51_), .O(new_n136_));
  aoi21  g92(.a(new_n136_), .b(new_n134_), .c(new_n110_), .O(z9));
endmodule


