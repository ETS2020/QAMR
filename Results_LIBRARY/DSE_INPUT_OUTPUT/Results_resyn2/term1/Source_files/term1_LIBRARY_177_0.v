// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_;
  nor2   g000(.a(x18), .b(x15), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xnor2a g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  aoi21  g006(.a(new_n49_), .b(new_n50_), .c(new_n45_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(x33), .c(new_n51_), .O(z1));
  inv1   g008(.a(x08), .O(new_n53_));
  inv1   g009(.a(x01), .O(new_n54_));
  aoi21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  oai21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  aoi21  g015(.a(x05), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g016(.a(new_n59_), .b(x05), .c(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  oai21  g018(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  xor2a  g019(.a(x07), .b(x04), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor4   g021(.a(new_n65_), .b(new_n45_), .c(x09), .d(new_n53_), .O(z2));
  inv1   g022(.a(x27), .O(new_n67_));
  nor2   g023(.a(x21), .b(x16), .O(new_n68_));
  nor2   g024(.a(x20), .b(x15), .O(new_n69_));
  inv1   g025(.a(x19), .O(new_n70_));
  inv1   g026(.a(x24), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor2   g028(.a(x22), .b(x17), .O(new_n73_));
  nor2   g029(.a(x23), .b(x18), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nor3   g032(.a(new_n76_), .b(new_n69_), .c(new_n68_), .O(new_n77_));
  inv1   g033(.a(x26), .O(new_n78_));
  nand3  g034(.a(new_n55_), .b(new_n78_), .c(x25), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n46_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n67_), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(z3));
  inv1   g040(.a(x28), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n85_), .b(new_n67_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n87_), .c(new_n80_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n46_), .O(z4));
  nand2  g046(.a(new_n86_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n87_), .b(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n91_), .c(new_n80_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n46_), .O(z5));
  nand3  g051(.a(new_n86_), .b(x30), .c(x29), .O(new_n96_));
  inv1   g052(.a(x30), .O(new_n97_));
  nand2  g053(.a(new_n91_), .b(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n96_), .c(new_n82_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z6));
  xor2a  g056(.a(new_n96_), .b(x31), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n82_), .O(z7));
  nand3  g058(.a(new_n70_), .b(x17), .c(x14), .O(new_n103_));
  inv1   g059(.a(x17), .O(new_n104_));
  nand3  g060(.a(x19), .b(new_n104_), .c(x12), .O(new_n105_));
  nand2  g061(.a(x18), .b(x15), .O(new_n106_));
  aoi21  g062(.a(new_n105_), .b(new_n103_), .c(new_n106_), .O(new_n107_));
  inv1   g063(.a(x15), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(x10), .O(new_n109_));
  inv1   g065(.a(x18), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x13), .O(new_n111_));
  nand2  g067(.a(x19), .b(x17), .O(new_n112_));
  aoi21  g068(.a(new_n111_), .b(new_n109_), .c(new_n112_), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n107_), .c(x16), .O(new_n114_));
  inv1   g070(.a(x16), .O(new_n115_));
  nand3  g071(.a(x18), .b(x17), .c(x11), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(x19), .c(new_n115_), .d(x15), .O(new_n118_));
  nand2  g074(.a(new_n50_), .b(x31), .O(new_n119_));
  aoi21  g075(.a(new_n118_), .b(new_n114_), .c(new_n119_), .O(new_n120_));
  nand2  g076(.a(new_n88_), .b(x29), .O(new_n121_));
  aoi21  g077(.a(new_n50_), .b(x30), .c(new_n121_), .O(new_n122_));
  oai21  g078(.a(new_n120_), .b(x30), .c(new_n122_), .O(new_n123_));
  aoi22  g079(.a(new_n88_), .b(x29), .c(new_n50_), .d(new_n97_), .O(new_n124_));
  oai21  g080(.a(new_n120_), .b(new_n97_), .c(new_n124_), .O(new_n125_));
  nand3  g081(.a(x18), .b(x17), .c(x16), .O(new_n126_));
  aoi22  g082(.a(new_n126_), .b(new_n70_), .c(new_n116_), .d(new_n115_), .O(new_n127_));
  nand2  g083(.a(x16), .b(x10), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n112_), .c(new_n108_), .O(new_n129_));
  nand2  g085(.a(new_n70_), .b(x14), .O(new_n130_));
  inv1   g086(.a(new_n126_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n130_), .c(x15), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n129_), .c(new_n127_), .O(new_n133_));
  nand2  g089(.a(x18), .b(x12), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n104_), .O(new_n135_));
  inv1   g091(.a(x13), .O(new_n136_));
  nand2  g092(.a(new_n110_), .b(new_n136_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(x31), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n133_), .c(x32), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n125_), .c(new_n123_), .O(new_n140_));
  inv1   g096(.a(x00), .O(new_n141_));
  nor2   g097(.a(x26), .b(new_n141_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n46_), .O(z8));
  nand2  g100(.a(new_n118_), .b(new_n114_), .O(new_n145_));
  aoi21  g101(.a(x31), .b(x30), .c(new_n121_), .O(new_n146_));
  inv1   g102(.a(x31), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n97_), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n121_), .c(new_n146_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n145_), .c(x33), .O(new_n150_));
  nand4  g106(.a(new_n149_), .b(new_n137_), .c(new_n135_), .d(x33), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n133_), .c(new_n142_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n150_), .c(new_n46_), .O(z9));
endmodule


