// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x13), .O(new_n45_));
  nor2   g001(.a(x18), .b(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  nand2  g008(.a(x32), .b(x03), .O(new_n53_));
  nand2  g009(.a(x33), .b(new_n48_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n46_), .O(z1));
  inv1   g012(.a(x08), .O(new_n57_));
  xor2a  g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nand2  g015(.a(new_n48_), .b(new_n50_), .O(new_n60_));
  nand2  g016(.a(x03), .b(x02), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n48_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n50_), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n59_), .c(new_n62_), .O(new_n66_));
  xor2a  g022(.a(new_n66_), .b(new_n58_), .O(new_n67_));
  nor4   g023(.a(new_n67_), .b(new_n46_), .c(x09), .d(new_n57_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  oai22  g025(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n70_));
  oai22  g026(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n71_));
  or2    g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor2   g028(.a(x23), .b(x18), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n61_), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(new_n77_));
  nand2  g033(.a(x25), .b(x01), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(x26), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n77_), .c(new_n69_), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z3));
  inv1   g037(.a(x28), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n69_), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nand2  g040(.a(new_n82_), .b(new_n69_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n84_), .c(new_n79_), .d(new_n77_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(z4));
  inv1   g043(.a(new_n46_), .O(new_n88_));
  nand2  g044(.a(new_n79_), .b(new_n61_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n83_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n84_), .b(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n91_), .c(new_n90_), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n74_), .c(new_n88_), .O(z5));
  inv1   g051(.a(x30), .O(new_n96_));
  nand2  g052(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand3  g053(.a(new_n83_), .b(x30), .c(x29), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n97_), .c(new_n90_), .d(new_n75_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z6));
  aoi21  g056(.a(x23), .b(new_n45_), .c(x18), .O(new_n101_));
  nor2   g057(.a(new_n98_), .b(x31), .O(new_n102_));
  nand2  g058(.a(new_n98_), .b(x31), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n90_), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n102_), .c(new_n88_), .O(new_n105_));
  oai21  g061(.a(new_n101_), .b(new_n72_), .c(new_n105_), .O(z7));
  inv1   g062(.a(x18), .O(new_n107_));
  inv1   g063(.a(x00), .O(new_n108_));
  nor2   g064(.a(x26), .b(new_n108_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x32), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n45_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n107_), .O(new_n112_));
  aoi21  g068(.a(new_n82_), .b(new_n69_), .c(new_n92_), .O(new_n113_));
  inv1   g069(.a(x16), .O(new_n114_));
  inv1   g070(.a(x19), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x17), .c(x14), .O(new_n116_));
  inv1   g072(.a(x17), .O(new_n117_));
  nand3  g073(.a(x19), .b(new_n117_), .c(x12), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n116_), .c(new_n114_), .O(new_n119_));
  nand4  g075(.a(x19), .b(x17), .c(new_n114_), .d(x11), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n119_), .c(x15), .O(new_n122_));
  inv1   g078(.a(x15), .O(new_n123_));
  nand3  g079(.a(x17), .b(x16), .c(x10), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x19), .c(new_n123_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n122_), .O(new_n127_));
  inv1   g083(.a(x31), .O(new_n128_));
  nor2   g084(.a(x32), .b(new_n128_), .O(new_n129_));
  nor2   g085(.a(x30), .b(new_n107_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n129_), .c(new_n127_), .O(new_n131_));
  nand2  g087(.a(x32), .b(x30), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n113_), .O(new_n134_));
  inv1   g090(.a(new_n113_), .O(new_n135_));
  nor2   g091(.a(new_n96_), .b(new_n107_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n129_), .c(new_n127_), .O(new_n137_));
  nand2  g093(.a(x32), .b(new_n96_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  nand2  g096(.a(new_n115_), .b(x14), .O(new_n141_));
  nand3  g097(.a(x17), .b(x16), .c(x15), .O(new_n142_));
  nand2  g098(.a(new_n124_), .b(new_n123_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n142_), .c(x19), .O(new_n144_));
  oai21  g100(.a(new_n142_), .b(new_n141_), .c(new_n144_), .O(new_n145_));
  nand2  g101(.a(x17), .b(x11), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n114_), .O(new_n147_));
  inv1   g103(.a(x12), .O(new_n148_));
  nand2  g104(.a(new_n117_), .b(new_n148_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n147_), .c(new_n145_), .d(x31), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x32), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(new_n140_), .c(new_n134_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n109_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n112_), .O(z8));
  aoi21  g110(.a(new_n109_), .b(x33), .c(x13), .O(new_n155_));
  nand2  g111(.a(new_n130_), .b(new_n128_), .O(new_n156_));
  aoi21  g112(.a(new_n126_), .b(new_n122_), .c(new_n156_), .O(new_n157_));
  nor2   g113(.a(new_n49_), .b(x30), .O(new_n158_));
  nor2   g114(.a(new_n158_), .b(new_n113_), .O(new_n159_));
  oai21  g115(.a(new_n157_), .b(x33), .c(new_n159_), .O(new_n160_));
  nand3  g116(.a(new_n136_), .b(new_n49_), .c(x31), .O(new_n161_));
  aoi21  g117(.a(new_n126_), .b(new_n122_), .c(new_n161_), .O(new_n162_));
  oai21  g118(.a(new_n162_), .b(new_n158_), .c(new_n113_), .O(new_n163_));
  nand2  g119(.a(new_n128_), .b(x30), .O(new_n164_));
  nand2  g120(.a(x31), .b(new_n96_), .O(new_n165_));
  nand4  g121(.a(new_n165_), .b(new_n164_), .c(new_n149_), .d(new_n147_), .O(new_n166_));
  inv1   g122(.a(new_n166_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n145_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(x33), .O(new_n169_));
  nand3  g125(.a(new_n169_), .b(new_n163_), .c(new_n160_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n109_), .O(new_n171_));
  oai21  g127(.a(new_n155_), .b(x18), .c(new_n171_), .O(z9));
endmodule


