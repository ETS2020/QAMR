// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x25), .O(new_n45_));
  inv1   g001(.a(x31), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  inv1   g009(.a(x32), .O(new_n54_));
  aoi21  g010(.a(x33), .b(new_n49_), .c(x02), .O(new_n55_));
  oai21  g011(.a(new_n54_), .b(new_n49_), .c(new_n55_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n47_), .O(z1));
  inv1   g013(.a(new_n47_), .O(new_n58_));
  xor2a  g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nand2  g016(.a(x05), .b(new_n49_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(x06), .c(new_n51_), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n51_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x05), .c(new_n49_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(new_n66_));
  nand2  g022(.a(x03), .b(x02), .O(new_n67_));
  aoi21  g023(.a(new_n49_), .b(new_n51_), .c(new_n60_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n59_), .O(new_n70_));
  inv1   g026(.a(x09), .O(new_n71_));
  nand2  g027(.a(new_n69_), .b(new_n59_), .O(new_n72_));
  nand3  g028(.a(new_n72_), .b(new_n71_), .c(x08), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n58_), .O(z2));
  inv1   g030(.a(x27), .O(new_n75_));
  nor2   g031(.a(x23), .b(x18), .O(new_n76_));
  nor2   g032(.a(x22), .b(x17), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  inv1   g034(.a(x19), .O(new_n79_));
  inv1   g035(.a(x24), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nor2   g037(.a(x21), .b(x16), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n81_), .c(new_n78_), .O(new_n85_));
  nor2   g041(.a(x26), .b(new_n60_), .O(new_n86_));
  and2   g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n67_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n75_), .O(new_n90_));
  aoi21  g046(.a(new_n90_), .b(new_n46_), .c(new_n45_), .O(z3));
  nand3  g047(.a(new_n67_), .b(new_n46_), .c(x25), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  inv1   g049(.a(x28), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n75_), .O(new_n95_));
  nor2   g051(.a(new_n94_), .b(new_n75_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n95_), .c(new_n93_), .d(new_n87_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z4));
  nand2  g055(.a(new_n96_), .b(x29), .O(new_n100_));
  inv1   g056(.a(new_n100_), .O(new_n101_));
  nor2   g057(.a(new_n96_), .b(x29), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n89_), .O(new_n104_));
  aoi21  g060(.a(new_n104_), .b(new_n46_), .c(new_n45_), .O(z5));
  inv1   g061(.a(x30), .O(new_n106_));
  nand2  g062(.a(new_n100_), .b(new_n106_), .O(new_n107_));
  nand2  g063(.a(new_n101_), .b(x30), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n107_), .c(new_n89_), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(new_n46_), .c(new_n45_), .O(z6));
  inv1   g066(.a(new_n108_), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n88_), .c(new_n46_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x25), .O(z7));
  inv1   g069(.a(x15), .O(new_n114_));
  inv1   g070(.a(x17), .O(new_n115_));
  nand3  g071(.a(new_n79_), .b(x18), .c(x14), .O(new_n116_));
  inv1   g072(.a(x18), .O(new_n117_));
  nand3  g073(.a(x19), .b(new_n117_), .c(x13), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(new_n119_));
  nand4  g075(.a(x19), .b(x18), .c(new_n115_), .d(x12), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n119_), .c(x16), .O(new_n122_));
  inv1   g078(.a(x16), .O(new_n123_));
  nand3  g079(.a(x18), .b(x17), .c(x11), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x19), .c(new_n123_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n122_), .c(new_n114_), .O(new_n127_));
  nand4  g083(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n128_));
  nor3   g084(.a(new_n128_), .b(new_n79_), .c(x15), .O(new_n129_));
  oai21  g085(.a(x28), .b(x27), .c(x29), .O(new_n130_));
  xor2a  g086(.a(new_n130_), .b(x30), .O(new_n131_));
  nor2   g087(.a(new_n131_), .b(new_n46_), .O(new_n132_));
  oai21  g088(.a(new_n129_), .b(new_n127_), .c(new_n132_), .O(new_n133_));
  nand2  g089(.a(x18), .b(x12), .O(new_n134_));
  oai21  g090(.a(x18), .b(x13), .c(x17), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(x16), .c(new_n125_), .O(new_n137_));
  nand2  g093(.a(new_n79_), .b(x14), .O(new_n138_));
  nand4  g094(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand2  g097(.a(new_n128_), .b(new_n114_), .O(new_n142_));
  nand2  g098(.a(new_n139_), .b(new_n79_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n142_), .c(new_n141_), .O(new_n144_));
  nor2   g100(.a(new_n144_), .b(new_n137_), .O(new_n145_));
  nor2   g101(.a(new_n131_), .b(new_n54_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(new_n145_), .c(x25), .O(new_n147_));
  inv1   g103(.a(x00), .O(new_n148_));
  nor2   g104(.a(x26), .b(new_n148_), .O(new_n149_));
  oai21  g105(.a(new_n147_), .b(new_n46_), .c(new_n149_), .O(new_n150_));
  aoi21  g106(.a(new_n133_), .b(new_n54_), .c(new_n150_), .O(z8));
  xor2a  g107(.a(new_n130_), .b(x31), .O(new_n152_));
  and2   g108(.a(new_n152_), .b(new_n131_), .O(new_n153_));
  oai21  g109(.a(new_n129_), .b(new_n127_), .c(new_n153_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n50_), .O(new_n155_));
  inv1   g111(.a(new_n149_), .O(new_n156_));
  nand2  g112(.a(new_n153_), .b(x33), .O(new_n157_));
  inv1   g113(.a(new_n157_), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n145_), .c(new_n156_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n58_), .O(z9));
endmodule


