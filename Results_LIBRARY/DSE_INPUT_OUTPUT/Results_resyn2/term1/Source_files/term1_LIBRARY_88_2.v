// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_;
  inv1   g000(.a(x18), .O(new_n45_));
  aoi21  g001(.a(new_n45_), .b(x00), .c(x32), .O(z0));
  nand2  g002(.a(new_n45_), .b(x00), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  xor2a  g004(.a(x03), .b(x02), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g006(.a(x33), .O(new_n51_));
  inv1   g007(.a(new_n49_), .O(new_n52_));
  nand2  g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g009(.a(new_n53_), .b(new_n50_), .c(new_n47_), .O(z1));
  xor2a  g010(.a(x07), .b(x04), .O(new_n55_));
  inv1   g011(.a(x03), .O(new_n56_));
  nand2  g012(.a(x05), .b(new_n56_), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand4  g016(.a(x06), .b(x05), .c(new_n56_), .d(new_n58_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  aoi21  g018(.a(new_n49_), .b(x01), .c(new_n62_), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n55_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n63_), .b(new_n55_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n64_), .c(new_n47_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  nor2   g025(.a(x20), .b(x15), .O(new_n70_));
  nor2   g026(.a(x22), .b(x17), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x23), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n45_), .O(new_n74_));
  nor2   g030(.a(x24), .b(x19), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(new_n78_));
  inv1   g034(.a(x26), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x25), .c(x01), .O(new_n80_));
  oai21  g036(.a(new_n56_), .b(new_n58_), .c(new_n47_), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  and2   g038(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n69_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(z3));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n69_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n69_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n88_), .c(new_n83_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z4));
  nand2  g047(.a(new_n87_), .b(x29), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  nand2  g049(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n92_), .c(new_n83_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z5));
  inv1   g052(.a(x30), .O(new_n97_));
  xor2a  g053(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n83_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z6));
  nand2  g056(.a(new_n92_), .b(x31), .O(new_n101_));
  xor2a  g057(.a(x31), .b(x30), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n87_), .c(x29), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n101_), .c(new_n83_), .O(z7));
  inv1   g060(.a(x19), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x14), .O(new_n106_));
  nand3  g062(.a(x17), .b(x16), .c(x15), .O(new_n107_));
  inv1   g063(.a(x15), .O(new_n108_));
  nand3  g064(.a(x17), .b(x16), .c(x10), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(new_n107_), .c(x19), .O(new_n111_));
  oai21  g067(.a(new_n107_), .b(new_n106_), .c(new_n111_), .O(new_n112_));
  inv1   g068(.a(x31), .O(new_n113_));
  aoi21  g069(.a(new_n86_), .b(new_n69_), .c(new_n93_), .O(new_n114_));
  xor2a  g070(.a(new_n114_), .b(new_n97_), .O(new_n115_));
  nor2   g071(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g072(.a(x17), .b(x11), .O(new_n117_));
  oai21  g073(.a(x17), .b(x12), .c(x16), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n116_), .c(new_n112_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x32), .O(new_n121_));
  inv1   g077(.a(x16), .O(new_n122_));
  nand3  g078(.a(new_n105_), .b(x17), .c(x14), .O(new_n123_));
  inv1   g079(.a(x17), .O(new_n124_));
  nand3  g080(.a(x19), .b(new_n124_), .c(x12), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n123_), .c(new_n122_), .O(new_n126_));
  nand4  g082(.a(x19), .b(x17), .c(new_n122_), .d(x11), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n126_), .c(x15), .O(new_n129_));
  inv1   g085(.a(new_n109_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x19), .c(new_n108_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n116_), .c(new_n48_), .O(new_n133_));
  nand3  g089(.a(new_n79_), .b(x18), .c(x00), .O(new_n134_));
  aoi21  g090(.a(new_n133_), .b(new_n121_), .c(new_n134_), .O(z8));
  inv1   g091(.a(x00), .O(new_n136_));
  nor2   g092(.a(x31), .b(x30), .O(new_n137_));
  aoi21  g093(.a(new_n114_), .b(x30), .c(new_n137_), .O(new_n138_));
  nand3  g094(.a(new_n89_), .b(new_n113_), .c(x29), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n119_), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(new_n112_), .c(new_n51_), .O(new_n142_));
  nor2   g098(.a(new_n102_), .b(x33), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n115_), .O(new_n144_));
  aoi21  g100(.a(new_n131_), .b(new_n129_), .c(new_n144_), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n142_), .c(new_n79_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(x18), .c(new_n136_), .O(z9));
endmodule


