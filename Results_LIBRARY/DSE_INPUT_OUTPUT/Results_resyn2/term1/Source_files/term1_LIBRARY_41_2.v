// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x26), .O(new_n45_));
  aoi21  g001(.a(new_n45_), .b(x02), .c(x32), .O(z0));
  inv1   g002(.a(x33), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  nor2   g007(.a(new_n45_), .b(new_n51_), .O(new_n52_));
  inv1   g008(.a(new_n52_), .O(new_n53_));
  aoi21  g009(.a(new_n50_), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  nand2  g010(.a(new_n50_), .b(x03), .O(new_n55_));
  aoi21  g011(.a(new_n47_), .b(new_n49_), .c(x02), .O(new_n56_));
  aoi22  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .d(new_n48_), .O(z1));
  inv1   g013(.a(x06), .O(new_n58_));
  inv1   g014(.a(x05), .O(new_n59_));
  nor2   g015(.a(new_n59_), .b(x03), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  oai21  g017(.a(x06), .b(new_n59_), .c(new_n51_), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g019(.a(x01), .O(new_n64_));
  oai21  g020(.a(new_n60_), .b(new_n53_), .c(new_n64_), .O(new_n65_));
  nand2  g021(.a(new_n49_), .b(x02), .O(new_n66_));
  nor2   g022(.a(new_n49_), .b(x02), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  oai21  g024(.a(new_n66_), .b(new_n45_), .c(new_n68_), .O(new_n69_));
  oai21  g025(.a(new_n65_), .b(new_n63_), .c(new_n69_), .O(new_n70_));
  xnor2a g026(.a(x07), .b(x04), .O(new_n71_));
  aoi21  g027(.a(new_n67_), .b(new_n58_), .c(new_n71_), .O(new_n72_));
  inv1   g028(.a(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n58_), .b(new_n51_), .O(new_n74_));
  nand2  g030(.a(new_n52_), .b(new_n64_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(new_n76_));
  inv1   g032(.a(new_n67_), .O(new_n77_));
  aoi21  g033(.a(new_n45_), .b(x02), .c(new_n64_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n77_), .c(new_n66_), .O(new_n79_));
  nand3  g035(.a(new_n61_), .b(new_n51_), .c(new_n64_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n79_), .c(new_n71_), .O(new_n81_));
  inv1   g037(.a(x08), .O(new_n82_));
  nor2   g038(.a(x09), .b(new_n82_), .O(new_n83_));
  oai21  g039(.a(new_n81_), .b(new_n76_), .c(new_n83_), .O(new_n84_));
  aoi21  g040(.a(new_n72_), .b(new_n70_), .c(new_n84_), .O(z2));
  inv1   g041(.a(x27), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  nor2   g043(.a(x22), .b(x17), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g045(.a(x24), .b(x19), .O(new_n90_));
  nor2   g046(.a(x21), .b(x16), .O(new_n91_));
  nor2   g047(.a(x20), .b(x15), .O(new_n92_));
  nor3   g048(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  nand2  g049(.a(x25), .b(x01), .O(new_n94_));
  aoi21  g050(.a(new_n93_), .b(new_n89_), .c(new_n94_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n86_), .O(new_n96_));
  aoi21  g052(.a(new_n96_), .b(new_n51_), .c(x26), .O(z3));
  inv1   g053(.a(x28), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(new_n86_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  nand2  g056(.a(new_n98_), .b(new_n86_), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n100_), .c(new_n95_), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(new_n51_), .c(x26), .O(z4));
  inv1   g059(.a(x29), .O(new_n104_));
  nand2  g060(.a(new_n100_), .b(new_n104_), .O(new_n105_));
  nand2  g061(.a(new_n99_), .b(x29), .O(new_n106_));
  nor2   g062(.a(x26), .b(x02), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n106_), .c(new_n105_), .d(new_n95_), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(z5));
  nand3  g065(.a(new_n99_), .b(x30), .c(x29), .O(new_n110_));
  inv1   g066(.a(x30), .O(new_n111_));
  nand2  g067(.a(new_n106_), .b(new_n111_), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(new_n110_), .c(new_n95_), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n51_), .c(x26), .O(z6));
  xor2a  g070(.a(new_n110_), .b(x31), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n95_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n51_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n45_), .O(z7));
  inv1   g074(.a(x15), .O(new_n119_));
  inv1   g075(.a(x17), .O(new_n120_));
  inv1   g076(.a(x19), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x18), .c(x14), .O(new_n122_));
  inv1   g078(.a(x18), .O(new_n123_));
  nand3  g079(.a(x19), .b(new_n123_), .c(x13), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n122_), .c(new_n120_), .O(new_n125_));
  nand4  g081(.a(x19), .b(x18), .c(new_n120_), .d(x12), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n125_), .c(x16), .O(new_n128_));
  inv1   g084(.a(x16), .O(new_n129_));
  nand3  g085(.a(x18), .b(x17), .c(x11), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x19), .c(new_n129_), .O(new_n132_));
  aoi21  g088(.a(new_n132_), .b(new_n128_), .c(new_n119_), .O(new_n133_));
  nor2   g089(.a(new_n123_), .b(new_n120_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x16), .c(x10), .O(new_n135_));
  nor3   g091(.a(new_n135_), .b(new_n121_), .c(x15), .O(new_n136_));
  inv1   g092(.a(x31), .O(new_n137_));
  nor2   g093(.a(x32), .b(new_n137_), .O(new_n138_));
  oai21  g094(.a(new_n136_), .b(new_n133_), .c(new_n138_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n111_), .O(new_n140_));
  nand2  g096(.a(new_n101_), .b(x29), .O(new_n141_));
  aoi21  g097(.a(new_n50_), .b(x30), .c(new_n141_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand2  g099(.a(new_n139_), .b(x30), .O(new_n144_));
  inv1   g100(.a(new_n141_), .O(new_n145_));
  aoi21  g101(.a(new_n50_), .b(new_n111_), .c(new_n145_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  oai21  g103(.a(new_n131_), .b(x16), .c(x15), .O(new_n148_));
  inv1   g104(.a(x14), .O(new_n149_));
  nor2   g105(.a(x19), .b(new_n149_), .O(new_n150_));
  nand3  g106(.a(new_n134_), .b(x16), .c(x15), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n121_), .O(new_n152_));
  oai21  g108(.a(new_n151_), .b(new_n150_), .c(new_n152_), .O(new_n153_));
  aoi21  g109(.a(new_n148_), .b(new_n135_), .c(new_n153_), .O(new_n154_));
  nand2  g110(.a(x18), .b(x12), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n120_), .O(new_n156_));
  inv1   g112(.a(x13), .O(new_n157_));
  nand2  g113(.a(new_n123_), .b(new_n157_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n156_), .c(new_n154_), .d(x31), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(x32), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n147_), .c(new_n143_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(x00), .O(new_n162_));
  aoi21  g118(.a(new_n162_), .b(new_n51_), .c(x26), .O(z8));
  aoi21  g119(.a(x31), .b(x30), .c(new_n141_), .O(new_n164_));
  nand2  g120(.a(new_n137_), .b(new_n111_), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(new_n141_), .c(new_n164_), .O(new_n166_));
  oai21  g122(.a(new_n136_), .b(new_n133_), .c(new_n166_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n47_), .O(new_n168_));
  aoi21  g124(.a(new_n123_), .b(new_n157_), .c(new_n47_), .O(new_n169_));
  nand4  g125(.a(new_n169_), .b(new_n166_), .c(new_n156_), .d(new_n154_), .O(new_n170_));
  nand3  g126(.a(new_n170_), .b(new_n168_), .c(x00), .O(new_n171_));
  aoi21  g127(.a(new_n171_), .b(new_n51_), .c(x26), .O(z9));
endmodule


