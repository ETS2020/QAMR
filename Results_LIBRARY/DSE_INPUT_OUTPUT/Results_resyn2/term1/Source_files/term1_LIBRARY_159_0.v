// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  nor2   g000(.a(x30), .b(x27), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nand2  g002(.a(x03), .b(x02), .O(new_n47_));
  inv1   g003(.a(x02), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g007(.a(x33), .O(new_n52_));
  aoi21  g008(.a(new_n51_), .b(new_n52_), .c(new_n45_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(x32), .c(new_n53_), .O(z1));
  inv1   g010(.a(x08), .O(new_n55_));
  xnor2a g011(.a(x07), .b(x04), .O(new_n56_));
  nand2  g012(.a(new_n51_), .b(x01), .O(new_n57_));
  nand4  g013(.a(x06), .b(x05), .c(new_n49_), .d(new_n48_), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n49_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n48_), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(x01), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  xor2a  g019(.a(new_n63_), .b(new_n56_), .O(new_n64_));
  nor4   g020(.a(new_n64_), .b(new_n45_), .c(x09), .d(new_n55_), .O(z2));
  inv1   g021(.a(x30), .O(new_n66_));
  nor2   g022(.a(x21), .b(x16), .O(new_n67_));
  nor2   g023(.a(x24), .b(x19), .O(new_n68_));
  inv1   g024(.a(x17), .O(new_n69_));
  inv1   g025(.a(x22), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g027(.a(x20), .b(x15), .O(new_n72_));
  nor2   g028(.a(x23), .b(x18), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nor3   g031(.a(new_n75_), .b(new_n68_), .c(new_n67_), .O(new_n76_));
  inv1   g032(.a(x26), .O(new_n77_));
  nand4  g033(.a(new_n47_), .b(new_n77_), .c(x25), .d(x01), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(x27), .O(z3));
  inv1   g037(.a(new_n45_), .O(new_n82_));
  inv1   g038(.a(x27), .O(new_n83_));
  inv1   g039(.a(x28), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g041(.a(x28), .b(x27), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n85_), .c(new_n79_), .d(new_n82_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(z4));
  xnor2a g044(.a(new_n86_), .b(x29), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n79_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n82_), .O(z5));
  nand3  g047(.a(x30), .b(x29), .c(x28), .O(new_n92_));
  nand2  g048(.a(x29), .b(x28), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n66_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n92_), .c(new_n79_), .O(new_n95_));
  oai21  g051(.a(new_n80_), .b(x27), .c(new_n95_), .O(z6));
  inv1   g052(.a(x31), .O(new_n97_));
  aoi21  g053(.a(new_n97_), .b(x30), .c(x27), .O(new_n98_));
  nor2   g054(.a(new_n97_), .b(new_n83_), .O(new_n99_));
  nand2  g055(.a(new_n86_), .b(new_n97_), .O(new_n100_));
  nor2   g056(.a(x31), .b(x29), .O(new_n101_));
  nor3   g057(.a(new_n101_), .b(new_n99_), .c(new_n66_), .O(new_n102_));
  aoi22  g058(.a(new_n102_), .b(new_n100_), .c(new_n99_), .d(new_n92_), .O(new_n103_));
  oai21  g059(.a(new_n98_), .b(new_n79_), .c(new_n103_), .O(z7));
  nand2  g060(.a(new_n77_), .b(x00), .O(new_n105_));
  inv1   g061(.a(x15), .O(new_n106_));
  inv1   g062(.a(x16), .O(new_n107_));
  nand4  g063(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n108_));
  inv1   g064(.a(x18), .O(new_n109_));
  nor2   g065(.a(new_n109_), .b(new_n69_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x11), .O(new_n111_));
  aoi22  g067(.a(new_n111_), .b(new_n107_), .c(new_n108_), .d(new_n106_), .O(new_n112_));
  inv1   g068(.a(x19), .O(new_n113_));
  nand3  g069(.a(new_n110_), .b(x16), .c(x15), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  inv1   g071(.a(new_n114_), .O(new_n116_));
  nand2  g072(.a(new_n113_), .b(x14), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g074(.a(x17), .b(x13), .O(new_n119_));
  oai21  g075(.a(x17), .b(x12), .c(x18), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n119_), .c(new_n97_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n118_), .c(new_n115_), .d(new_n112_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n82_), .O(new_n123_));
  nand2  g079(.a(x30), .b(x29), .O(new_n124_));
  aoi21  g080(.a(new_n84_), .b(new_n83_), .c(new_n124_), .O(new_n125_));
  nor2   g081(.a(x30), .b(x29), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(x27), .c(new_n125_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x32), .O(new_n129_));
  nand3  g085(.a(new_n113_), .b(x18), .c(x14), .O(new_n130_));
  nand3  g086(.a(x19), .b(new_n109_), .c(x13), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n130_), .c(new_n69_), .O(new_n132_));
  nand4  g088(.a(x19), .b(x18), .c(new_n69_), .d(x12), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  nand3  g091(.a(x19), .b(x18), .c(x17), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n107_), .c(x11), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n135_), .c(new_n106_), .O(new_n139_));
  nand3  g095(.a(x16), .b(new_n106_), .c(x10), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n136_), .O(new_n141_));
  nor2   g097(.a(new_n126_), .b(new_n97_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(z0), .O(new_n143_));
  nor2   g099(.a(new_n143_), .b(new_n125_), .O(new_n144_));
  oai21  g100(.a(new_n141_), .b(new_n139_), .c(new_n144_), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n129_), .c(new_n105_), .O(z8));
  nand2  g102(.a(x33), .b(new_n84_), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n105_), .c(x30), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n83_), .O(new_n149_));
  nand2  g105(.a(new_n138_), .b(new_n135_), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(x15), .c(new_n141_), .O(new_n151_));
  nand2  g107(.a(new_n97_), .b(x30), .O(new_n152_));
  oai21  g108(.a(new_n125_), .b(new_n101_), .c(new_n152_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n151_), .c(new_n52_), .O(new_n154_));
  nand3  g110(.a(new_n118_), .b(new_n115_), .c(new_n112_), .O(new_n155_));
  nand2  g111(.a(new_n120_), .b(new_n119_), .O(new_n156_));
  oai21  g112(.a(x31), .b(x29), .c(new_n124_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n156_), .c(new_n152_), .d(x33), .O(new_n158_));
  nor2   g114(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  nor2   g115(.a(new_n159_), .b(new_n105_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n154_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n149_), .O(z9));
endmodule


