// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_;
  nor2   g000(.a(x33), .b(x31), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(new_n45_), .O(new_n47_));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  aoi21  g005(.a(x33), .b(new_n48_), .c(x02), .O(new_n50_));
  oai21  g006(.a(new_n49_), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  inv1   g007(.a(x33), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  aoi21  g009(.a(x32), .b(new_n48_), .c(new_n53_), .O(new_n54_));
  oai21  g010(.a(new_n52_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n51_), .c(new_n47_), .O(z1));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n48_), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(x06), .c(new_n53_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n53_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(x05), .c(new_n48_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nand2  g020(.a(x03), .b(x02), .O(new_n65_));
  aoi21  g021(.a(new_n48_), .b(new_n53_), .c(new_n58_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n57_), .O(new_n68_));
  inv1   g024(.a(x09), .O(new_n69_));
  nand2  g025(.a(new_n67_), .b(new_n57_), .O(new_n70_));
  nand3  g026(.a(new_n70_), .b(new_n69_), .c(x08), .O(new_n71_));
  oai21  g027(.a(new_n71_), .b(new_n68_), .c(new_n47_), .O(z2));
  inv1   g028(.a(new_n65_), .O(new_n73_));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  nor2   g030(.a(x24), .b(x19), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g032(.a(x17), .O(new_n77_));
  inv1   g033(.a(x22), .O(new_n78_));
  oai22  g034(.a(x23), .b(x18), .c(x21), .d(x16), .O(new_n79_));
  aoi21  g035(.a(new_n78_), .b(new_n77_), .c(new_n79_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n76_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n73_), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  inv1   g040(.a(x27), .O(new_n85_));
  inv1   g041(.a(x26), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(x25), .c(x01), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  oai21  g045(.a(new_n89_), .b(new_n84_), .c(new_n47_), .O(z3));
  inv1   g046(.a(x28), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n85_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand2  g049(.a(new_n91_), .b(new_n85_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n93_), .c(new_n88_), .d(new_n83_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n47_), .O(z4));
  nor4   g052(.a(new_n87_), .b(new_n82_), .c(new_n73_), .d(new_n45_), .O(new_n97_));
  nand2  g053(.a(new_n92_), .b(x29), .O(new_n98_));
  inv1   g054(.a(x29), .O(new_n99_));
  nand2  g055(.a(new_n93_), .b(new_n99_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(z5));
  inv1   g058(.a(x30), .O(new_n103_));
  nand2  g059(.a(x30), .b(x29), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  aoi21  g061(.a(new_n98_), .b(new_n103_), .c(new_n105_), .O(new_n106_));
  inv1   g062(.a(x15), .O(new_n107_));
  inv1   g063(.a(x16), .O(new_n108_));
  inv1   g064(.a(x21), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g066(.a(new_n78_), .b(new_n77_), .c(x16), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n110_), .c(new_n107_), .O(new_n112_));
  inv1   g068(.a(x31), .O(new_n113_));
  nor2   g069(.a(new_n113_), .b(x29), .O(new_n114_));
  oai21  g070(.a(new_n112_), .b(new_n74_), .c(new_n114_), .O(new_n115_));
  nor3   g071(.a(new_n92_), .b(new_n82_), .c(new_n103_), .O(new_n116_));
  aoi22  g072(.a(new_n116_), .b(new_n115_), .c(new_n106_), .d(new_n81_), .O(new_n117_));
  nand2  g073(.a(new_n88_), .b(new_n65_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n117_), .c(new_n47_), .O(z6));
  inv1   g075(.a(new_n98_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x30), .O(new_n121_));
  nor2   g077(.a(x31), .b(new_n103_), .O(new_n122_));
  aoi22  g078(.a(new_n122_), .b(new_n120_), .c(new_n121_), .d(x31), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n97_), .O(z7));
  nand2  g080(.a(new_n86_), .b(x00), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(x32), .c(new_n52_), .O(new_n127_));
  inv1   g083(.a(x19), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x14), .O(new_n129_));
  nand4  g085(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand4  g087(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n132_));
  nand3  g088(.a(x18), .b(x17), .c(x11), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand2  g090(.a(x18), .b(x12), .O(new_n135_));
  nand2  g091(.a(x17), .b(x13), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n135_), .c(new_n108_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n134_), .c(x15), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n132_), .O(new_n139_));
  and2   g095(.a(new_n130_), .b(x19), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n139_), .c(new_n131_), .O(new_n141_));
  nand2  g097(.a(new_n94_), .b(x29), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n103_), .O(new_n143_));
  aoi21  g099(.a(new_n91_), .b(new_n85_), .c(new_n104_), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n141_), .c(x32), .O(new_n147_));
  nand3  g103(.a(new_n128_), .b(x18), .c(x14), .O(new_n148_));
  inv1   g104(.a(x18), .O(new_n149_));
  nand3  g105(.a(x19), .b(new_n149_), .c(x13), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n148_), .c(new_n77_), .O(new_n151_));
  nand4  g107(.a(x19), .b(x18), .c(new_n77_), .d(x12), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n151_), .c(x16), .O(new_n154_));
  nand3  g110(.a(new_n134_), .b(x19), .c(new_n108_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nor3   g112(.a(new_n132_), .b(new_n128_), .c(x15), .O(new_n157_));
  aoi21  g113(.a(new_n156_), .b(x15), .c(new_n157_), .O(new_n158_));
  nor2   g114(.a(new_n144_), .b(new_n113_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n143_), .c(new_n49_), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n158_), .c(new_n147_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n126_), .O(new_n162_));
  oai21  g118(.a(new_n127_), .b(x31), .c(new_n162_), .O(z8));
  inv1   g119(.a(new_n142_), .O(new_n164_));
  nand2  g120(.a(new_n52_), .b(x30), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n158_), .c(x31), .O(new_n166_));
  inv1   g122(.a(new_n122_), .O(new_n167_));
  oai21  g123(.a(new_n159_), .b(new_n141_), .c(x33), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  aoi21  g125(.a(new_n166_), .b(new_n164_), .c(new_n169_), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n125_), .c(new_n47_), .O(z9));
endmodule


