// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x13), .O(new_n45_));
  oai21  g001(.a(x26), .b(new_n45_), .c(x32), .O(z0));
  xnor2a g002(.a(x03), .b(x02), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  nor2   g005(.a(x26), .b(new_n45_), .O(new_n50_));
  aoi21  g006(.a(new_n48_), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n48_), .b(x33), .c(new_n51_), .O(z1));
  inv1   g008(.a(x08), .O(new_n53_));
  inv1   g009(.a(x01), .O(new_n54_));
  aoi21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  oai21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  aoi21  g015(.a(x05), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g016(.a(new_n59_), .b(x05), .c(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  oai21  g018(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  xor2a  g019(.a(x07), .b(x04), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor4   g021(.a(new_n65_), .b(new_n50_), .c(x09), .d(new_n53_), .O(z2));
  inv1   g022(.a(x27), .O(new_n67_));
  nand2  g023(.a(new_n55_), .b(x25), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(new_n69_));
  nor2   g025(.a(x23), .b(x18), .O(new_n70_));
  nor2   g026(.a(x22), .b(x17), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x19), .O(new_n73_));
  inv1   g029(.a(x24), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(x20), .b(x15), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(new_n79_));
  and2   g035(.a(new_n79_), .b(new_n69_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n67_), .O(new_n81_));
  aoi21  g037(.a(new_n81_), .b(new_n45_), .c(x26), .O(z3));
  nor3   g038(.a(new_n68_), .b(x26), .c(x13), .O(new_n83_));
  inv1   g039(.a(x28), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  nor2   g041(.a(new_n84_), .b(new_n67_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n85_), .c(new_n83_), .d(new_n79_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z4));
  nand2  g045(.a(new_n86_), .b(x29), .O(new_n90_));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n87_), .b(new_n91_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n90_), .c(new_n83_), .d(new_n79_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(z5));
  nand3  g050(.a(new_n86_), .b(x30), .c(x29), .O(new_n95_));
  inv1   g051(.a(x30), .O(new_n96_));
  nand2  g052(.a(new_n90_), .b(new_n96_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n95_), .c(new_n80_), .O(new_n98_));
  aoi21  g054(.a(new_n98_), .b(new_n45_), .c(x26), .O(z6));
  inv1   g055(.a(x26), .O(new_n100_));
  xor2a  g056(.a(new_n95_), .b(x31), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n80_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n45_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n100_), .O(z7));
  inv1   g060(.a(x16), .O(new_n105_));
  nand3  g061(.a(new_n73_), .b(x17), .c(x14), .O(new_n106_));
  inv1   g062(.a(x17), .O(new_n107_));
  nand3  g063(.a(x19), .b(new_n107_), .c(x12), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  nand4  g065(.a(x19), .b(x17), .c(new_n105_), .d(x11), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n109_), .c(x15), .O(new_n112_));
  inv1   g068(.a(x15), .O(new_n113_));
  nand3  g069(.a(x17), .b(x16), .c(x10), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x19), .c(new_n113_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n112_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x31), .O(new_n118_));
  nand2  g074(.a(new_n96_), .b(x18), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n118_), .c(new_n49_), .O(new_n120_));
  aoi21  g076(.a(new_n84_), .b(new_n67_), .c(new_n91_), .O(new_n121_));
  nand2  g077(.a(x32), .b(new_n96_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n121_), .c(new_n120_), .O(new_n123_));
  inv1   g079(.a(new_n121_), .O(new_n124_));
  nand3  g080(.a(new_n49_), .b(x30), .c(x18), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n118_), .c(new_n122_), .O(new_n126_));
  nand2  g082(.a(new_n73_), .b(x14), .O(new_n127_));
  nand3  g083(.a(x17), .b(x16), .c(x15), .O(new_n128_));
  nand2  g084(.a(new_n114_), .b(new_n113_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n128_), .c(x19), .O(new_n130_));
  oai21  g086(.a(new_n128_), .b(new_n127_), .c(new_n130_), .O(new_n131_));
  inv1   g087(.a(x31), .O(new_n132_));
  aoi21  g088(.a(x17), .b(x11), .c(x16), .O(new_n133_));
  oai21  g089(.a(x17), .b(x12), .c(x18), .O(new_n134_));
  nor3   g090(.a(new_n134_), .b(new_n133_), .c(new_n132_), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n131_), .c(new_n49_), .O(new_n136_));
  aoi21  g092(.a(new_n126_), .b(new_n124_), .c(new_n136_), .O(new_n137_));
  nand3  g093(.a(new_n100_), .b(new_n45_), .c(x00), .O(new_n138_));
  aoi21  g094(.a(new_n137_), .b(new_n123_), .c(new_n138_), .O(z8));
  inv1   g095(.a(x33), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(x30), .O(new_n141_));
  nand4  g097(.a(new_n140_), .b(x31), .c(x30), .d(x18), .O(new_n142_));
  aoi21  g098(.a(new_n116_), .b(new_n112_), .c(new_n142_), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n141_), .c(new_n121_), .O(new_n144_));
  nand3  g100(.a(new_n132_), .b(new_n96_), .c(x18), .O(new_n145_));
  aoi21  g101(.a(new_n116_), .b(new_n112_), .c(new_n145_), .O(new_n146_));
  nor2   g102(.a(new_n141_), .b(new_n121_), .O(new_n147_));
  oai21  g103(.a(new_n146_), .b(x33), .c(new_n147_), .O(new_n148_));
  xor2a  g104(.a(x31), .b(x30), .O(new_n149_));
  nor3   g105(.a(new_n149_), .b(new_n134_), .c(new_n133_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n131_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x33), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n148_), .c(new_n144_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(x00), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n45_), .c(x26), .O(z9));
endmodule


