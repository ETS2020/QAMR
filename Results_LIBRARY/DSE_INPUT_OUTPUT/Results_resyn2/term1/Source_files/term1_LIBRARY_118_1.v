// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x15), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(x33), .O(new_n53_));
  aoi21  g009(.a(new_n52_), .b(new_n53_), .c(new_n46_), .O(new_n54_));
  oai21  g010(.a(new_n52_), .b(x32), .c(new_n54_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  nand2  g013(.a(new_n52_), .b(x01), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n50_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n49_), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(x01), .O(new_n61_));
  oai21  g017(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  xor2a  g019(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  nor4   g020(.a(new_n64_), .b(new_n46_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g021(.a(new_n46_), .O(new_n66_));
  inv1   g022(.a(x26), .O(new_n67_));
  nand4  g023(.a(new_n48_), .b(new_n67_), .c(x25), .d(x01), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(new_n69_));
  nor2   g025(.a(x24), .b(x19), .O(new_n70_));
  oai22  g026(.a(x22), .b(x17), .c(x21), .d(x16), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x23), .O(new_n73_));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  aoi21  g030(.a(new_n73_), .b(new_n45_), .c(new_n74_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(x27), .c(new_n66_), .O(z3));
  inv1   g034(.a(x27), .O(new_n79_));
  inv1   g035(.a(x28), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n80_), .b(new_n79_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(new_n77_), .c(new_n66_), .O(z4));
  oai21  g041(.a(new_n74_), .b(new_n73_), .c(new_n45_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n72_), .O(new_n87_));
  inv1   g043(.a(x29), .O(new_n88_));
  nand2  g044(.a(new_n82_), .b(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n81_), .b(x29), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n46_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n89_), .c(new_n87_), .d(new_n69_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(z5));
  xor2a  g050(.a(new_n90_), .b(x30), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n77_), .c(new_n66_), .O(z6));
  inv1   g052(.a(new_n77_), .O(new_n97_));
  inv1   g053(.a(x30), .O(new_n98_));
  inv1   g054(.a(x31), .O(new_n99_));
  nor2   g055(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nor2   g056(.a(x31), .b(x30), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nor2   g058(.a(new_n91_), .b(new_n99_), .O(new_n103_));
  aoi21  g059(.a(new_n102_), .b(new_n91_), .c(new_n103_), .O(new_n104_));
  aoi21  g060(.a(new_n104_), .b(new_n97_), .c(new_n46_), .O(z7));
  inv1   g061(.a(x15), .O(new_n106_));
  inv1   g062(.a(x32), .O(new_n107_));
  nand2  g063(.a(new_n67_), .b(x00), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n107_), .c(new_n45_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  inv1   g066(.a(new_n108_), .O(new_n111_));
  inv1   g067(.a(x17), .O(new_n112_));
  nand4  g068(.a(x19), .b(new_n45_), .c(x15), .d(x13), .O(new_n113_));
  inv1   g069(.a(x19), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x18), .c(x14), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  nand4  g072(.a(x19), .b(x18), .c(new_n112_), .d(x12), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x16), .O(new_n119_));
  inv1   g075(.a(x16), .O(new_n120_));
  nand2  g076(.a(x17), .b(x11), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(x19), .c(x18), .d(new_n120_), .O(new_n123_));
  nand2  g079(.a(new_n107_), .b(x31), .O(new_n124_));
  aoi21  g080(.a(new_n123_), .b(new_n119_), .c(new_n124_), .O(new_n125_));
  nand2  g081(.a(new_n83_), .b(x29), .O(new_n126_));
  aoi21  g082(.a(new_n107_), .b(x30), .c(new_n126_), .O(new_n127_));
  oai21  g083(.a(new_n125_), .b(x30), .c(new_n127_), .O(new_n128_));
  nand3  g084(.a(x18), .b(x17), .c(x16), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  oai21  g086(.a(new_n121_), .b(new_n45_), .c(new_n120_), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(x19), .c(new_n130_), .O(new_n132_));
  inv1   g088(.a(x13), .O(new_n133_));
  nor2   g089(.a(x17), .b(x12), .O(new_n134_));
  oai22  g090(.a(new_n134_), .b(new_n45_), .c(new_n112_), .d(new_n133_), .O(new_n135_));
  inv1   g091(.a(x14), .O(new_n136_));
  oai21  g092(.a(x19), .b(new_n136_), .c(new_n130_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(x31), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n132_), .c(x32), .O(new_n139_));
  inv1   g095(.a(new_n126_), .O(new_n140_));
  aoi21  g096(.a(new_n107_), .b(new_n98_), .c(new_n140_), .O(new_n141_));
  oai21  g097(.a(new_n125_), .b(new_n98_), .c(new_n141_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n139_), .c(new_n128_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n111_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n110_), .O(z8));
  inv1   g101(.a(new_n100_), .O(new_n146_));
  nor2   g102(.a(new_n106_), .b(new_n133_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(x19), .c(x17), .d(x16), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(x33), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n100_), .c(new_n45_), .O(new_n150_));
  nand3  g106(.a(new_n122_), .b(x19), .c(new_n120_), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  nand3  g108(.a(x19), .b(new_n112_), .c(x12), .O(new_n153_));
  nand3  g109(.a(new_n114_), .b(x17), .c(x14), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n153_), .c(new_n120_), .O(new_n155_));
  nor3   g111(.a(x33), .b(new_n45_), .c(new_n106_), .O(new_n156_));
  oai21  g112(.a(new_n155_), .b(new_n152_), .c(new_n156_), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n146_), .c(new_n150_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n140_), .O(new_n159_));
  oai22  g115(.a(new_n134_), .b(new_n114_), .c(new_n112_), .d(new_n120_), .O(new_n160_));
  aoi21  g116(.a(new_n121_), .b(new_n120_), .c(new_n102_), .O(new_n161_));
  nand3  g117(.a(new_n161_), .b(new_n160_), .c(new_n137_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(x15), .O(new_n163_));
  oai21  g119(.a(new_n148_), .b(new_n102_), .c(new_n45_), .O(new_n164_));
  aoi21  g120(.a(new_n126_), .b(new_n98_), .c(new_n46_), .O(new_n165_));
  oai21  g121(.a(new_n126_), .b(new_n98_), .c(new_n165_), .O(new_n166_));
  nand3  g122(.a(new_n166_), .b(new_n164_), .c(new_n163_), .O(new_n167_));
  nand2  g123(.a(new_n149_), .b(new_n45_), .O(new_n168_));
  nand2  g124(.a(new_n126_), .b(new_n101_), .O(new_n169_));
  aoi21  g125(.a(new_n168_), .b(new_n157_), .c(new_n169_), .O(new_n170_));
  aoi21  g126(.a(new_n167_), .b(x33), .c(new_n170_), .O(new_n171_));
  aoi21  g127(.a(new_n171_), .b(new_n159_), .c(new_n108_), .O(z9));
endmodule


