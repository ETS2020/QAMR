// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_;
  inv1   g000(.a(x17), .O(new_n45_));
  aoi21  g001(.a(x31), .b(new_n45_), .c(x32), .O(z0));
  nand2  g002(.a(x31), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  xor2a  g004(.a(x03), .b(x02), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  or2    g006(.a(new_n49_), .b(x33), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(z1));
  xor2a  g008(.a(x07), .b(x04), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nand2  g010(.a(x05), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(x02), .O(new_n56_));
  nand2  g012(.a(x06), .b(new_n56_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand4  g014(.a(x06), .b(x05), .c(new_n54_), .d(new_n56_), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  aoi21  g016(.a(new_n49_), .b(x01), .c(new_n60_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  nand2  g019(.a(new_n61_), .b(new_n53_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(new_n63_), .c(x08), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n62_), .c(new_n47_), .O(z2));
  inv1   g022(.a(x22), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n45_), .O(new_n68_));
  oai22  g024(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n69_));
  oai22  g025(.a(x24), .b(x19), .c(x23), .d(x18), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  inv1   g028(.a(x26), .O(new_n73_));
  nand2  g029(.a(x03), .b(x02), .O(new_n74_));
  nand4  g030(.a(new_n74_), .b(new_n73_), .c(x25), .d(x01), .O(new_n75_));
  inv1   g031(.a(new_n75_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(x27), .c(new_n47_), .O(z3));
  inv1   g034(.a(x27), .O(new_n79_));
  inv1   g035(.a(x28), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n80_), .b(new_n79_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n82_), .c(new_n47_), .O(new_n84_));
  or2    g040(.a(new_n84_), .b(new_n77_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(z4));
  inv1   g042(.a(x29), .O(new_n87_));
  nand2  g043(.a(new_n82_), .b(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n81_), .b(x29), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n88_), .c(new_n47_), .O(new_n90_));
  or2    g046(.a(new_n90_), .b(new_n77_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z5));
  inv1   g048(.a(x30), .O(new_n93_));
  nand2  g049(.a(new_n89_), .b(new_n93_), .O(new_n94_));
  inv1   g050(.a(new_n89_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(x30), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n77_), .c(new_n47_), .O(z6));
  oai21  g054(.a(new_n96_), .b(new_n45_), .c(x31), .O(new_n99_));
  oai21  g055(.a(new_n68_), .b(x31), .c(new_n71_), .O(new_n100_));
  inv1   g056(.a(x31), .O(new_n101_));
  nand3  g057(.a(new_n95_), .b(new_n101_), .c(x30), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n100_), .c(new_n99_), .d(new_n76_), .O(z7));
  nand3  g059(.a(x18), .b(x16), .c(x15), .O(new_n104_));
  nand3  g060(.a(x18), .b(x16), .c(x10), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  inv1   g062(.a(x15), .O(new_n107_));
  nand2  g063(.a(x18), .b(x11), .O(new_n108_));
  nand2  g064(.a(x16), .b(x13), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n106_), .c(x19), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n104_), .O(new_n112_));
  oai21  g068(.a(x28), .b(x27), .c(x29), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n93_), .O(new_n114_));
  nand3  g070(.a(new_n83_), .b(x30), .c(x29), .O(new_n115_));
  inv1   g071(.a(new_n104_), .O(new_n116_));
  inv1   g072(.a(x19), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x14), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n115_), .c(new_n114_), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n112_), .c(new_n48_), .O(new_n122_));
  inv1   g078(.a(x16), .O(new_n123_));
  nand3  g079(.a(new_n117_), .b(x18), .c(x14), .O(new_n124_));
  inv1   g080(.a(x18), .O(new_n125_));
  nand3  g081(.a(x19), .b(new_n125_), .c(x13), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(new_n127_));
  nand4  g083(.a(x19), .b(x18), .c(new_n123_), .d(x11), .O(new_n128_));
  inv1   g084(.a(new_n128_), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n127_), .c(x15), .O(new_n130_));
  nand3  g086(.a(x19), .b(x18), .c(x16), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n107_), .c(x10), .O(new_n133_));
  nand4  g089(.a(new_n115_), .b(new_n114_), .c(new_n48_), .d(x31), .O(new_n134_));
  aoi21  g090(.a(new_n133_), .b(new_n130_), .c(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n122_), .c(x17), .O(new_n136_));
  nand2  g092(.a(x32), .b(new_n101_), .O(new_n137_));
  nand2  g093(.a(new_n73_), .b(x00), .O(new_n138_));
  aoi21  g094(.a(new_n137_), .b(new_n136_), .c(new_n138_), .O(z8));
  aoi21  g095(.a(new_n126_), .b(new_n124_), .c(new_n45_), .O(new_n140_));
  nand4  g096(.a(x19), .b(x18), .c(new_n45_), .d(x12), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n140_), .c(x16), .O(new_n143_));
  nand2  g099(.a(new_n129_), .b(x17), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n143_), .c(new_n107_), .O(new_n145_));
  nor2   g101(.a(new_n133_), .b(new_n45_), .O(new_n146_));
  inv1   g102(.a(new_n114_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n101_), .O(new_n148_));
  inv1   g104(.a(new_n148_), .O(new_n149_));
  oai21  g105(.a(new_n146_), .b(new_n145_), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n133_), .b(new_n130_), .O(new_n151_));
  nor4   g107(.a(new_n113_), .b(new_n101_), .c(new_n93_), .d(new_n45_), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n151_), .c(x33), .O(new_n153_));
  inv1   g109(.a(new_n138_), .O(new_n154_));
  nand3  g110(.a(new_n111_), .b(new_n104_), .c(new_n47_), .O(new_n155_));
  nand2  g111(.a(x15), .b(x12), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n131_), .c(new_n45_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n147_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n101_), .O(new_n159_));
  nand2  g115(.a(new_n115_), .b(x31), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n119_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(x17), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(new_n159_), .c(new_n155_), .d(x33), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n154_), .O(new_n164_));
  aoi21  g120(.a(new_n153_), .b(new_n150_), .c(new_n164_), .O(z9));
endmodule


