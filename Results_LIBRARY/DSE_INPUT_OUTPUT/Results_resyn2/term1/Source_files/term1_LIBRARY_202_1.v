// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x13), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x33), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(x32), .b(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(x33), .b(new_n49_), .c(x02), .O(new_n53_));
  oai21  g009(.a(new_n52_), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(new_n49_), .b(new_n59_), .O(new_n60_));
  nand2  g016(.a(x03), .b(x02), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n49_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n59_), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n58_), .c(new_n62_), .O(new_n66_));
  xor2a  g022(.a(new_n66_), .b(new_n57_), .O(new_n67_));
  nor4   g023(.a(new_n67_), .b(new_n46_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  inv1   g025(.a(new_n46_), .O(new_n70_));
  nor2   g026(.a(x22), .b(x17), .O(new_n71_));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g029(.a(x19), .O(new_n74_));
  inv1   g030(.a(x24), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x21), .b(x16), .O(new_n77_));
  nor2   g033(.a(x20), .b(x15), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n61_), .c(new_n70_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(x25), .b(x01), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(x26), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n82_), .c(new_n69_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(z3));
  inv1   g042(.a(x28), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n69_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n87_), .b(new_n69_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n89_), .c(new_n84_), .d(new_n82_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z4));
  nand2  g048(.a(new_n84_), .b(new_n61_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  and2   g050(.a(new_n94_), .b(new_n80_), .O(new_n95_));
  nand2  g051(.a(new_n88_), .b(x29), .O(new_n96_));
  inv1   g052(.a(x29), .O(new_n97_));
  nand2  g053(.a(new_n89_), .b(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n96_), .c(new_n95_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n70_), .O(z5));
  nand2  g056(.a(new_n80_), .b(new_n70_), .O(new_n101_));
  inv1   g057(.a(x30), .O(new_n102_));
  nand2  g058(.a(new_n96_), .b(new_n102_), .O(new_n103_));
  nand3  g059(.a(new_n88_), .b(x30), .c(x29), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n103_), .c(new_n94_), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n101_), .O(z6));
  xor2a  g062(.a(new_n104_), .b(x31), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n95_), .c(new_n46_), .O(z7));
  inv1   g064(.a(x13), .O(new_n109_));
  inv1   g065(.a(x00), .O(new_n110_));
  nor2   g066(.a(x26), .b(new_n110_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x32), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n45_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n109_), .O(new_n114_));
  inv1   g070(.a(x16), .O(new_n115_));
  inv1   g071(.a(x14), .O(new_n116_));
  oai21  g072(.a(x19), .b(new_n116_), .c(x18), .O(new_n117_));
  inv1   g073(.a(x17), .O(new_n118_));
  nand2  g074(.a(x19), .b(x13), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n45_), .c(new_n118_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n117_), .O(new_n121_));
  nand2  g077(.a(x19), .b(x18), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n118_), .c(x12), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n121_), .c(new_n115_), .O(new_n125_));
  nand2  g081(.a(x17), .b(x11), .O(new_n126_));
  nor3   g082(.a(new_n126_), .b(new_n122_), .c(x16), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n125_), .c(x15), .O(new_n128_));
  inv1   g084(.a(x15), .O(new_n129_));
  nor2   g085(.a(new_n118_), .b(new_n115_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n129_), .c(x10), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n123_), .O(new_n133_));
  nand2  g089(.a(new_n52_), .b(x31), .O(new_n134_));
  aoi21  g090(.a(new_n133_), .b(new_n128_), .c(new_n134_), .O(new_n135_));
  nand2  g091(.a(new_n90_), .b(x29), .O(new_n136_));
  aoi21  g092(.a(new_n52_), .b(x30), .c(new_n136_), .O(new_n137_));
  oai21  g093(.a(new_n135_), .b(x30), .c(new_n137_), .O(new_n138_));
  aoi22  g094(.a(new_n90_), .b(x29), .c(new_n52_), .d(new_n102_), .O(new_n139_));
  oai21  g095(.a(new_n135_), .b(new_n102_), .c(new_n139_), .O(new_n140_));
  inv1   g096(.a(new_n130_), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n117_), .c(x15), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n131_), .O(new_n143_));
  aoi21  g099(.a(new_n130_), .b(x15), .c(new_n123_), .O(new_n144_));
  aoi21  g100(.a(new_n74_), .b(new_n45_), .c(new_n144_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand2  g102(.a(new_n126_), .b(new_n115_), .O(new_n147_));
  inv1   g103(.a(x12), .O(new_n148_));
  nand2  g104(.a(new_n118_), .b(new_n148_), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n147_), .c(x31), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n146_), .c(x32), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(new_n140_), .c(new_n138_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n111_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n114_), .O(z8));
  aoi21  g110(.a(new_n111_), .b(x33), .c(x18), .O(new_n155_));
  nand2  g111(.a(new_n133_), .b(new_n128_), .O(new_n156_));
  aoi21  g112(.a(x31), .b(x30), .c(new_n136_), .O(new_n157_));
  inv1   g113(.a(x31), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n102_), .O(new_n159_));
  aoi21  g115(.a(new_n159_), .b(new_n136_), .c(new_n157_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n156_), .c(x33), .O(new_n161_));
  nand4  g117(.a(new_n160_), .b(new_n149_), .c(new_n147_), .d(x33), .O(new_n162_));
  oai21  g118(.a(new_n162_), .b(new_n146_), .c(new_n111_), .O(new_n163_));
  oai22  g119(.a(new_n163_), .b(new_n161_), .c(new_n155_), .d(x13), .O(z9));
endmodule


