// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_;
  nor2   g000(.a(x22), .b(x18), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  inv1   g008(.a(x32), .O(new_n53_));
  aoi21  g009(.a(x33), .b(new_n48_), .c(x02), .O(new_n54_));
  oai21  g010(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n45_), .O(z1));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n48_), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(x06), .c(new_n50_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n50_), .O(new_n61_));
  nand3  g017(.a(new_n61_), .b(x05), .c(new_n48_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nor2   g020(.a(new_n48_), .b(new_n50_), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(new_n66_));
  aoi21  g022(.a(new_n48_), .b(new_n50_), .c(new_n58_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n57_), .O(new_n69_));
  inv1   g025(.a(x09), .O(new_n70_));
  nand2  g026(.a(new_n68_), .b(new_n57_), .O(new_n71_));
  nand3  g027(.a(new_n71_), .b(new_n70_), .c(x08), .O(new_n72_));
  oai21  g028(.a(new_n72_), .b(new_n69_), .c(new_n46_), .O(z2));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  oai22  g030(.a(x24), .b(x19), .c(x21), .d(x16), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(x22), .b(x17), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n66_), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  inv1   g038(.a(x26), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(x25), .c(x01), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n81_), .c(new_n46_), .O(z3));
  inv1   g043(.a(x28), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n82_), .O(new_n89_));
  nor2   g045(.a(new_n88_), .b(new_n82_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n89_), .c(new_n85_), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n81_), .c(new_n46_), .O(z4));
  nor3   g049(.a(new_n84_), .b(new_n65_), .c(new_n45_), .O(new_n94_));
  and2   g050(.a(new_n94_), .b(new_n80_), .O(new_n95_));
  nand2  g051(.a(new_n90_), .b(x29), .O(new_n96_));
  inv1   g052(.a(x29), .O(new_n97_));
  nand2  g053(.a(new_n91_), .b(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n96_), .c(new_n95_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z5));
  inv1   g056(.a(new_n96_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(x30), .O(new_n102_));
  inv1   g058(.a(x30), .O(new_n103_));
  nand2  g059(.a(new_n96_), .b(new_n103_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n102_), .c(new_n95_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(z6));
  nand2  g062(.a(new_n102_), .b(x31), .O(new_n107_));
  inv1   g063(.a(x31), .O(new_n108_));
  nand3  g064(.a(new_n101_), .b(new_n108_), .c(x30), .O(new_n109_));
  aoi21  g065(.a(new_n78_), .b(x18), .c(new_n77_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n76_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n109_), .c(new_n107_), .d(new_n94_), .O(z7));
  inv1   g068(.a(x15), .O(new_n113_));
  inv1   g069(.a(x17), .O(new_n114_));
  inv1   g070(.a(x18), .O(new_n115_));
  nand3  g071(.a(x19), .b(new_n115_), .c(x13), .O(new_n116_));
  inv1   g072(.a(x19), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x18), .c(x14), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n116_), .c(new_n114_), .O(new_n119_));
  nand4  g075(.a(x19), .b(x18), .c(new_n114_), .d(x12), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n119_), .c(x16), .O(new_n122_));
  inv1   g078(.a(x16), .O(new_n123_));
  nand3  g079(.a(x18), .b(x17), .c(x11), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(x19), .c(new_n123_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n122_), .c(new_n113_), .O(new_n127_));
  nor2   g083(.a(new_n114_), .b(new_n123_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x10), .O(new_n129_));
  nand3  g085(.a(x19), .b(x18), .c(new_n113_), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nor2   g087(.a(x32), .b(new_n108_), .O(new_n132_));
  oai21  g088(.a(new_n131_), .b(new_n127_), .c(new_n132_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n103_), .O(new_n134_));
  nand2  g090(.a(new_n89_), .b(x29), .O(new_n135_));
  aoi21  g091(.a(new_n53_), .b(x30), .c(new_n135_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nor2   g093(.a(x17), .b(x12), .O(new_n138_));
  nand2  g094(.a(x17), .b(x13), .O(new_n139_));
  oai21  g095(.a(new_n138_), .b(new_n115_), .c(new_n139_), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(x16), .c(new_n125_), .O(new_n141_));
  oai21  g097(.a(new_n129_), .b(new_n115_), .c(new_n113_), .O(new_n142_));
  nand2  g098(.a(new_n128_), .b(x15), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n115_), .c(new_n117_), .O(new_n144_));
  nand2  g100(.a(new_n117_), .b(x14), .O(new_n145_));
  nand4  g101(.a(new_n128_), .b(new_n145_), .c(x18), .d(x15), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n144_), .c(new_n142_), .d(x31), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n141_), .c(x32), .O(new_n148_));
  nand2  g104(.a(new_n133_), .b(x30), .O(new_n149_));
  inv1   g105(.a(new_n135_), .O(new_n150_));
  aoi21  g106(.a(new_n53_), .b(new_n103_), .c(new_n150_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n148_), .c(new_n137_), .O(new_n153_));
  nand2  g109(.a(new_n83_), .b(x00), .O(new_n154_));
  inv1   g110(.a(new_n154_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n46_), .O(z8));
  inv1   g113(.a(x22), .O(new_n158_));
  nand2  g114(.a(x33), .b(x30), .O(new_n159_));
  nand3  g115(.a(new_n128_), .b(x15), .c(x13), .O(new_n160_));
  inv1   g116(.a(new_n160_), .O(new_n161_));
  nor2   g117(.a(new_n117_), .b(x18), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n49_), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(new_n164_));
  nor2   g120(.a(x31), .b(x30), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(new_n164_), .c(new_n161_), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(new_n159_), .c(new_n158_), .O(new_n167_));
  nand4  g123(.a(x19), .b(x17), .c(new_n123_), .d(x11), .O(new_n168_));
  inv1   g124(.a(new_n168_), .O(new_n169_));
  nand2  g125(.a(new_n114_), .b(x12), .O(new_n170_));
  oai21  g126(.a(x19), .b(x17), .c(x16), .O(new_n171_));
  aoi21  g127(.a(new_n170_), .b(new_n145_), .c(new_n171_), .O(new_n172_));
  oai21  g128(.a(new_n172_), .b(new_n169_), .c(x15), .O(new_n173_));
  nand4  g129(.a(new_n128_), .b(x19), .c(new_n113_), .d(x10), .O(new_n174_));
  aoi21  g130(.a(new_n174_), .b(new_n173_), .c(x33), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n165_), .O(new_n176_));
  aoi21  g132(.a(new_n176_), .b(new_n159_), .c(new_n115_), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n167_), .c(new_n135_), .O(new_n178_));
  nor2   g134(.a(new_n49_), .b(x30), .O(new_n179_));
  nor2   g135(.a(new_n108_), .b(new_n103_), .O(new_n180_));
  inv1   g136(.a(new_n180_), .O(new_n181_));
  nor3   g137(.a(new_n181_), .b(new_n163_), .c(new_n160_), .O(new_n182_));
  oai21  g138(.a(new_n182_), .b(new_n179_), .c(x22), .O(new_n183_));
  aoi21  g139(.a(new_n180_), .b(new_n175_), .c(new_n179_), .O(new_n184_));
  oai21  g140(.a(new_n184_), .b(new_n115_), .c(new_n183_), .O(new_n185_));
  nor2   g141(.a(new_n143_), .b(new_n145_), .O(new_n186_));
  nand2  g142(.a(new_n143_), .b(x19), .O(new_n187_));
  aoi21  g143(.a(new_n129_), .b(new_n113_), .c(new_n187_), .O(new_n188_));
  aoi21  g144(.a(x17), .b(x11), .c(x16), .O(new_n189_));
  nor2   g145(.a(new_n189_), .b(new_n138_), .O(new_n190_));
  oai21  g146(.a(new_n188_), .b(new_n186_), .c(new_n190_), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(x18), .O(new_n192_));
  nor2   g148(.a(new_n180_), .b(new_n165_), .O(new_n193_));
  nand2  g149(.a(new_n161_), .b(x19), .O(new_n194_));
  aoi21  g150(.a(new_n194_), .b(new_n115_), .c(new_n193_), .O(new_n195_));
  nand2  g151(.a(new_n46_), .b(x33), .O(new_n196_));
  aoi21  g152(.a(new_n195_), .b(new_n192_), .c(new_n196_), .O(new_n197_));
  aoi21  g153(.a(new_n185_), .b(new_n150_), .c(new_n197_), .O(new_n198_));
  aoi21  g154(.a(new_n198_), .b(new_n178_), .c(new_n154_), .O(z9));
endmodule


