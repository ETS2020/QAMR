// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  inv1   g000(.a(x18), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x15), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xnor2a g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  inv1   g006(.a(new_n46_), .O(new_n51_));
  aoi21  g007(.a(new_n49_), .b(new_n50_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n49_), .b(x33), .c(new_n52_), .O(z1));
  xor2a  g009(.a(x07), .b(x04), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nand2  g011(.a(x05), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  aoi21  g014(.a(new_n58_), .b(new_n56_), .c(x01), .O(new_n59_));
  oai21  g015(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n48_), .b(x01), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x08), .O(new_n64_));
  aoi21  g020(.a(new_n62_), .b(new_n54_), .c(new_n64_), .O(new_n65_));
  oai21  g021(.a(new_n62_), .b(new_n54_), .c(new_n65_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n46_), .O(z2));
  inv1   g023(.a(x27), .O(new_n68_));
  nand2  g024(.a(x03), .b(x02), .O(new_n69_));
  inv1   g025(.a(x15), .O(new_n70_));
  inv1   g026(.a(x20), .O(new_n71_));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  oai21  g028(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(new_n73_));
  nor2   g029(.a(x21), .b(x16), .O(new_n74_));
  oai22  g030(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n73_), .c(new_n51_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n69_), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(new_n79_));
  nand2  g035(.a(x25), .b(x01), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(x26), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n79_), .c(new_n68_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(z3));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n68_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n68_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n86_), .c(new_n81_), .d(new_n79_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z4));
  nand2  g045(.a(new_n81_), .b(new_n69_), .O(new_n90_));
  aoi21  g046(.a(new_n71_), .b(new_n70_), .c(new_n72_), .O(new_n91_));
  aoi21  g047(.a(new_n91_), .b(new_n76_), .c(new_n90_), .O(new_n92_));
  nand2  g048(.a(new_n85_), .b(x29), .O(new_n93_));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n86_), .b(new_n94_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n93_), .c(new_n92_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n46_), .O(z5));
  inv1   g053(.a(new_n90_), .O(new_n98_));
  inv1   g054(.a(x30), .O(new_n99_));
  nand2  g055(.a(new_n93_), .b(new_n99_), .O(new_n100_));
  nand3  g056(.a(new_n85_), .b(x30), .c(x29), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n100_), .c(new_n98_), .d(new_n77_), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(z6));
  xor2a  g059(.a(new_n101_), .b(x31), .O(new_n104_));
  aoi21  g060(.a(new_n104_), .b(new_n92_), .c(new_n51_), .O(z7));
  inv1   g061(.a(x00), .O(new_n106_));
  nor2   g062(.a(x26), .b(new_n106_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x32), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n70_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n45_), .O(new_n110_));
  inv1   g066(.a(x16), .O(new_n111_));
  inv1   g067(.a(x19), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(x17), .c(x14), .O(new_n113_));
  inv1   g069(.a(x17), .O(new_n114_));
  nand3  g070(.a(x19), .b(new_n114_), .c(x12), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n113_), .c(new_n111_), .O(new_n116_));
  nand4  g072(.a(x19), .b(x17), .c(new_n111_), .d(x11), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x15), .O(new_n119_));
  nand3  g075(.a(x17), .b(x16), .c(x10), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(x19), .c(x18), .d(new_n70_), .O(new_n122_));
  nand2  g078(.a(new_n50_), .b(x31), .O(new_n123_));
  aoi21  g079(.a(new_n122_), .b(new_n119_), .c(new_n123_), .O(new_n124_));
  nand2  g080(.a(new_n87_), .b(x29), .O(new_n125_));
  aoi21  g081(.a(new_n50_), .b(x30), .c(new_n125_), .O(new_n126_));
  oai21  g082(.a(new_n124_), .b(x30), .c(new_n126_), .O(new_n127_));
  aoi22  g083(.a(new_n87_), .b(x29), .c(new_n50_), .d(new_n99_), .O(new_n128_));
  oai21  g084(.a(new_n124_), .b(new_n99_), .c(new_n128_), .O(new_n129_));
  inv1   g085(.a(x31), .O(new_n130_));
  nand2  g086(.a(new_n112_), .b(x14), .O(new_n131_));
  nand3  g087(.a(x17), .b(x16), .c(x15), .O(new_n132_));
  nor2   g088(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g089(.a(new_n132_), .b(x19), .O(new_n134_));
  aoi21  g090(.a(new_n120_), .b(new_n70_), .c(new_n134_), .O(new_n135_));
  nand2  g091(.a(x17), .b(x11), .O(new_n136_));
  oai21  g092(.a(x17), .b(x12), .c(x16), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  oai21  g094(.a(new_n135_), .b(new_n133_), .c(new_n138_), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n130_), .c(x32), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n129_), .c(new_n127_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n107_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n110_), .O(z8));
  aoi21  g099(.a(new_n107_), .b(x33), .c(x15), .O(new_n144_));
  nand2  g100(.a(new_n122_), .b(new_n119_), .O(new_n145_));
  aoi21  g101(.a(x31), .b(x30), .c(new_n125_), .O(new_n146_));
  nand2  g102(.a(new_n130_), .b(new_n99_), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n125_), .c(new_n146_), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n145_), .c(x33), .O(new_n149_));
  nand2  g105(.a(new_n148_), .b(x33), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n139_), .c(new_n107_), .O(new_n151_));
  oai22  g107(.a(new_n151_), .b(new_n149_), .c(new_n144_), .d(x18), .O(z9));
endmodule


