// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_;
  nor2   g000(.a(x18), .b(x07), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(x03), .O(new_n47_));
  inv1   g003(.a(x33), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  aoi21  g005(.a(x32), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n48_), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(x33), .b(new_n47_), .c(x02), .O(new_n53_));
  oai21  g009(.a(new_n52_), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g011(.a(new_n45_), .O(new_n56_));
  inv1   g012(.a(x01), .O(new_n57_));
  aoi21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  oai21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n47_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x06), .c(new_n49_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n49_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x05), .c(new_n47_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(new_n64_));
  and2   g020(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(new_n66_));
  xnor2a g022(.a(x07), .b(x04), .O(new_n67_));
  inv1   g023(.a(x09), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(x08), .O(new_n69_));
  aoi21  g025(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  oai21  g026(.a(new_n67_), .b(new_n66_), .c(new_n70_), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n56_), .O(z2));
  inv1   g028(.a(x27), .O(new_n73_));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  nor2   g030(.a(x21), .b(x16), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x22), .b(x17), .O(new_n77_));
  nor2   g033(.a(x24), .b(x19), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  oai21  g037(.a(x23), .b(x18), .c(new_n81_), .O(new_n82_));
  and2   g038(.a(new_n82_), .b(new_n56_), .O(new_n83_));
  inv1   g039(.a(x26), .O(new_n84_));
  nand3  g040(.a(new_n58_), .b(new_n84_), .c(x25), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n83_), .c(new_n73_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(z3));
  inv1   g044(.a(x28), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(new_n73_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand2  g047(.a(new_n89_), .b(new_n73_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n91_), .c(new_n86_), .d(new_n82_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n56_), .O(z4));
  nand2  g050(.a(new_n90_), .b(x29), .O(new_n95_));
  inv1   g051(.a(x29), .O(new_n96_));
  nand2  g052(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n95_), .c(new_n86_), .d(new_n82_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n56_), .O(z5));
  inv1   g055(.a(x30), .O(new_n100_));
  nand2  g056(.a(new_n95_), .b(new_n100_), .O(new_n101_));
  nand3  g057(.a(new_n90_), .b(x30), .c(x29), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n101_), .c(new_n86_), .d(new_n83_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(z6));
  aoi21  g060(.a(x23), .b(x07), .c(x18), .O(new_n105_));
  nor2   g061(.a(new_n102_), .b(x31), .O(new_n106_));
  nand2  g062(.a(new_n102_), .b(x31), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n86_), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n106_), .c(new_n56_), .O(new_n109_));
  oai21  g065(.a(new_n105_), .b(new_n80_), .c(new_n109_), .O(z7));
  inv1   g066(.a(x15), .O(new_n111_));
  inv1   g067(.a(x17), .O(new_n112_));
  inv1   g068(.a(x19), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x18), .c(x14), .O(new_n114_));
  inv1   g070(.a(x18), .O(new_n115_));
  nand3  g071(.a(x19), .b(new_n115_), .c(x13), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n114_), .c(new_n112_), .O(new_n117_));
  nand4  g073(.a(x19), .b(x18), .c(new_n112_), .d(x12), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n117_), .c(x16), .O(new_n120_));
  inv1   g076(.a(x16), .O(new_n121_));
  nand3  g077(.a(x18), .b(x17), .c(x11), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x19), .c(new_n121_), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n120_), .c(new_n111_), .O(new_n125_));
  nor2   g081(.a(new_n115_), .b(new_n112_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x16), .c(x10), .O(new_n127_));
  nor3   g083(.a(new_n127_), .b(new_n113_), .c(x15), .O(new_n128_));
  inv1   g084(.a(x31), .O(new_n129_));
  nor2   g085(.a(x32), .b(new_n129_), .O(new_n130_));
  oai21  g086(.a(new_n128_), .b(new_n125_), .c(new_n130_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n100_), .O(new_n132_));
  nand2  g088(.a(new_n92_), .b(x29), .O(new_n133_));
  aoi21  g089(.a(new_n52_), .b(x30), .c(new_n133_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g091(.a(new_n131_), .b(x30), .O(new_n136_));
  inv1   g092(.a(new_n133_), .O(new_n137_));
  aoi21  g093(.a(new_n52_), .b(new_n100_), .c(new_n137_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  oai21  g095(.a(new_n123_), .b(x16), .c(x15), .O(new_n140_));
  inv1   g096(.a(x14), .O(new_n141_));
  nor2   g097(.a(x19), .b(new_n141_), .O(new_n142_));
  nand3  g098(.a(new_n126_), .b(x16), .c(x15), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n113_), .O(new_n144_));
  oai21  g100(.a(new_n143_), .b(new_n142_), .c(new_n144_), .O(new_n145_));
  aoi21  g101(.a(new_n140_), .b(new_n127_), .c(new_n145_), .O(new_n146_));
  nand2  g102(.a(x18), .b(x12), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n112_), .O(new_n148_));
  inv1   g104(.a(x13), .O(new_n149_));
  nand2  g105(.a(new_n115_), .b(new_n149_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n148_), .c(new_n146_), .d(x31), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x32), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n139_), .c(new_n135_), .O(new_n153_));
  nand2  g109(.a(new_n84_), .b(x00), .O(new_n154_));
  inv1   g110(.a(new_n154_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n56_), .O(z8));
  aoi21  g113(.a(x31), .b(x30), .c(new_n133_), .O(new_n158_));
  nand2  g114(.a(new_n129_), .b(new_n100_), .O(new_n159_));
  aoi21  g115(.a(new_n159_), .b(new_n133_), .c(new_n158_), .O(new_n160_));
  oai21  g116(.a(new_n128_), .b(new_n125_), .c(new_n160_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n48_), .O(new_n162_));
  aoi21  g118(.a(new_n115_), .b(new_n149_), .c(new_n48_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n160_), .c(new_n148_), .d(new_n146_), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(new_n162_), .c(new_n155_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n56_), .O(z9));
endmodule


