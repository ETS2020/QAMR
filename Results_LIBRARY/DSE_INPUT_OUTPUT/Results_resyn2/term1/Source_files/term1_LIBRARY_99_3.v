// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x23), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  nand2  g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  and2   g008(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  inv1   g009(.a(new_n53_), .O(new_n54_));
  inv1   g010(.a(x33), .O(new_n55_));
  aoi21  g011(.a(new_n54_), .b(new_n55_), .c(new_n47_), .O(new_n56_));
  oai21  g012(.a(new_n54_), .b(x32), .c(new_n56_), .O(z1));
  inv1   g013(.a(new_n47_), .O(new_n58_));
  xor2a  g014(.a(x07), .b(x04), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n51_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n50_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  or2    g018(.a(new_n61_), .b(new_n60_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(x01), .O(new_n64_));
  aoi21  g020(.a(new_n53_), .b(x01), .c(new_n64_), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand2  g023(.a(new_n65_), .b(new_n59_), .O(new_n68_));
  nand3  g024(.a(new_n68_), .b(new_n67_), .c(x08), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n66_), .c(new_n58_), .O(z2));
  inv1   g026(.a(x27), .O(new_n71_));
  oai22  g027(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n72_));
  oai22  g028(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai21  g030(.a(x23), .b(x18), .c(new_n74_), .O(new_n75_));
  inv1   g031(.a(x26), .O(new_n76_));
  nand4  g032(.a(new_n49_), .b(new_n76_), .c(x25), .d(x01), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(new_n78_));
  and2   g034(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n71_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n58_), .O(z3));
  nand2  g037(.a(x28), .b(x27), .O(new_n82_));
  inv1   g038(.a(x28), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n71_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n82_), .c(new_n79_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n58_), .O(z4));
  inv1   g042(.a(new_n82_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(x29), .O(new_n88_));
  inv1   g044(.a(x29), .O(new_n89_));
  aoi21  g045(.a(new_n82_), .b(new_n89_), .c(new_n47_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n88_), .c(new_n79_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z5));
  nand2  g048(.a(new_n74_), .b(x18), .O(new_n93_));
  nand2  g049(.a(new_n82_), .b(new_n45_), .O(new_n94_));
  aoi21  g050(.a(new_n94_), .b(x29), .c(new_n46_), .O(new_n95_));
  aoi21  g051(.a(x30), .b(new_n45_), .c(new_n88_), .O(new_n96_));
  aoi22  g052(.a(new_n96_), .b(new_n75_), .c(new_n95_), .d(new_n93_), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n77_), .c(new_n58_), .O(z6));
  inv1   g054(.a(x31), .O(new_n99_));
  inv1   g055(.a(new_n88_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n99_), .c(x30), .O(new_n101_));
  nand2  g057(.a(new_n100_), .b(x30), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(x31), .c(new_n47_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n101_), .c(new_n79_), .O(z7));
  inv1   g060(.a(x32), .O(new_n105_));
  inv1   g061(.a(x17), .O(new_n106_));
  inv1   g062(.a(x19), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x18), .c(x14), .O(new_n108_));
  inv1   g064(.a(x18), .O(new_n109_));
  nand3  g065(.a(x19), .b(new_n109_), .c(x13), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(new_n111_));
  nand4  g067(.a(x19), .b(x18), .c(new_n106_), .d(x12), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(x16), .O(new_n114_));
  inv1   g070(.a(x16), .O(new_n115_));
  nand3  g071(.a(x18), .b(x17), .c(x11), .O(new_n116_));
  inv1   g072(.a(new_n116_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x19), .c(new_n115_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n114_), .O(new_n119_));
  nor2   g075(.a(new_n109_), .b(new_n106_), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x16), .c(x10), .O(new_n121_));
  nor3   g077(.a(new_n121_), .b(new_n107_), .c(x15), .O(new_n122_));
  aoi21  g078(.a(new_n119_), .b(x15), .c(new_n122_), .O(new_n123_));
  nand2  g079(.a(new_n84_), .b(x29), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n46_), .O(new_n125_));
  inv1   g081(.a(new_n124_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(x30), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n125_), .c(x31), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n123_), .c(new_n105_), .O(new_n129_));
  nand2  g085(.a(new_n76_), .b(x00), .O(new_n130_));
  nand3  g086(.a(new_n120_), .b(x16), .c(x15), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  aoi21  g088(.a(new_n116_), .b(new_n115_), .c(new_n107_), .O(new_n133_));
  nor2   g089(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g090(.a(x18), .b(x12), .O(new_n135_));
  oai21  g091(.a(x18), .b(x13), .c(x17), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g093(.a(new_n107_), .b(x14), .O(new_n138_));
  nand2  g094(.a(new_n132_), .b(new_n138_), .O(new_n139_));
  inv1   g095(.a(x15), .O(new_n140_));
  nand2  g096(.a(new_n121_), .b(new_n140_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n139_), .c(new_n137_), .O(new_n142_));
  nor2   g098(.a(new_n142_), .b(new_n134_), .O(new_n143_));
  nand4  g099(.a(new_n127_), .b(new_n125_), .c(x32), .d(x31), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n143_), .c(new_n130_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n129_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n58_), .O(z8));
  nand3  g104(.a(new_n124_), .b(new_n99_), .c(new_n46_), .O(new_n149_));
  oai21  g105(.a(new_n127_), .b(new_n99_), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n143_), .O(new_n151_));
  nor2   g107(.a(new_n47_), .b(new_n55_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  aoi21  g109(.a(new_n118_), .b(new_n114_), .c(new_n140_), .O(new_n154_));
  nand4  g110(.a(new_n126_), .b(x31), .c(x30), .d(new_n45_), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n149_), .c(x33), .O(new_n156_));
  oai21  g112(.a(new_n122_), .b(new_n154_), .c(new_n156_), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(new_n153_), .c(new_n130_), .O(z9));
endmodule


