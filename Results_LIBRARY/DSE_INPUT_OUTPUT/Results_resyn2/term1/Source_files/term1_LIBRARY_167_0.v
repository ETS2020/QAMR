// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x25), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x33), .O(new_n49_));
  xnor2a g005(.a(x03), .b(x02), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  inv1   g008(.a(new_n50_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n51_), .c(new_n47_), .O(z1));
  xor2a  g011(.a(x07), .b(x04), .O(new_n56_));
  inv1   g012(.a(x01), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  inv1   g014(.a(x03), .O(new_n59_));
  nand4  g015(.a(x06), .b(x05), .c(new_n59_), .d(new_n58_), .O(new_n60_));
  nand2  g016(.a(x05), .b(new_n59_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n58_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(new_n60_), .c(new_n57_), .O(new_n64_));
  oai21  g020(.a(new_n53_), .b(new_n57_), .c(new_n64_), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(x08), .O(new_n67_));
  aoi21  g023(.a(new_n65_), .b(new_n56_), .c(new_n67_), .O(new_n68_));
  oai21  g024(.a(new_n65_), .b(new_n56_), .c(new_n68_), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n47_), .O(z2));
  inv1   g026(.a(x27), .O(new_n71_));
  nand2  g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(x23), .b(x18), .O(new_n73_));
  nor2   g029(.a(x22), .b(x17), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g031(.a(x19), .O(new_n76_));
  inv1   g032(.a(x24), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor2   g034(.a(x21), .b(x16), .O(new_n79_));
  nor2   g035(.a(x20), .b(x15), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n78_), .c(new_n75_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n72_), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nor3   g040(.a(x26), .b(new_n45_), .c(new_n57_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n71_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z3));
  inv1   g045(.a(x28), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n71_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n90_), .b(new_n71_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n92_), .c(new_n85_), .d(new_n84_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z4));
  nand2  g051(.a(new_n91_), .b(x29), .O(new_n96_));
  inv1   g052(.a(x29), .O(new_n97_));
  nand2  g053(.a(new_n92_), .b(new_n97_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n86_), .c(new_n47_), .O(z5));
  inv1   g056(.a(x26), .O(new_n101_));
  oai21  g057(.a(new_n46_), .b(new_n45_), .c(new_n96_), .O(new_n102_));
  nand3  g058(.a(new_n91_), .b(x30), .c(x29), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n102_), .c(new_n101_), .d(x01), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n83_), .c(new_n47_), .O(z6));
  xor2a  g061(.a(new_n103_), .b(x31), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n87_), .O(z7));
  nand4  g063(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  inv1   g065(.a(x11), .O(new_n110_));
  inv1   g066(.a(x16), .O(new_n111_));
  nand2  g067(.a(x18), .b(x17), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n110_), .c(new_n111_), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(x19), .c(new_n109_), .O(new_n114_));
  nand2  g070(.a(x17), .b(x13), .O(new_n115_));
  oai21  g071(.a(x17), .b(x12), .c(x18), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand2  g073(.a(new_n76_), .b(x14), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n109_), .O(new_n119_));
  inv1   g075(.a(x15), .O(new_n120_));
  nand2  g076(.a(x16), .b(x10), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n112_), .c(new_n120_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n119_), .c(new_n117_), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n114_), .c(new_n47_), .O(new_n124_));
  oai21  g080(.a(x28), .b(x27), .c(x29), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n46_), .O(new_n126_));
  nand2  g082(.a(new_n125_), .b(new_n46_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x31), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(new_n47_), .c(new_n126_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n124_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x32), .O(new_n131_));
  inv1   g087(.a(x17), .O(new_n132_));
  nand3  g088(.a(new_n76_), .b(x18), .c(x14), .O(new_n133_));
  inv1   g089(.a(x18), .O(new_n134_));
  nand3  g090(.a(x19), .b(new_n134_), .c(x13), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n133_), .c(new_n132_), .O(new_n136_));
  nand4  g092(.a(x19), .b(x18), .c(new_n132_), .d(x12), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  nand3  g095(.a(x19), .b(x18), .c(x17), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n111_), .c(x11), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n139_), .c(new_n120_), .O(new_n143_));
  nor3   g099(.a(new_n140_), .b(new_n121_), .c(x15), .O(new_n144_));
  inv1   g100(.a(new_n126_), .O(new_n145_));
  inv1   g101(.a(x31), .O(new_n146_));
  nor2   g102(.a(x32), .b(new_n146_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n127_), .c(new_n145_), .d(new_n47_), .O(new_n148_));
  inv1   g104(.a(new_n148_), .O(new_n149_));
  oai21  g105(.a(new_n144_), .b(new_n143_), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n101_), .b(x00), .O(new_n151_));
  aoi21  g107(.a(new_n150_), .b(new_n131_), .c(new_n151_), .O(z8));
  nor2   g108(.a(x30), .b(new_n45_), .O(new_n153_));
  nand2  g109(.a(new_n125_), .b(new_n146_), .O(new_n154_));
  nand3  g110(.a(new_n93_), .b(x31), .c(x29), .O(new_n155_));
  aoi22  g111(.a(new_n155_), .b(x30), .c(new_n154_), .d(new_n153_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n124_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x33), .O(new_n158_));
  nand3  g114(.a(new_n153_), .b(new_n125_), .c(new_n146_), .O(new_n159_));
  nand2  g115(.a(new_n126_), .b(x31), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n159_), .c(x33), .O(new_n161_));
  oai21  g117(.a(new_n144_), .b(new_n143_), .c(new_n161_), .O(new_n162_));
  aoi21  g118(.a(new_n162_), .b(new_n158_), .c(new_n151_), .O(z9));
endmodule


