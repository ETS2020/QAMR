// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x30), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x07), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xor2a  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  inv1   g005(.a(new_n46_), .O(new_n50_));
  aoi21  g006(.a(new_n48_), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n48_), .b(x33), .c(new_n51_), .O(z1));
  nand2  g008(.a(new_n48_), .b(x01), .O(new_n53_));
  inv1   g009(.a(x03), .O(new_n54_));
  nand2  g010(.a(x05), .b(new_n54_), .O(new_n55_));
  inv1   g011(.a(x02), .O(new_n56_));
  nand2  g012(.a(x06), .b(new_n56_), .O(new_n57_));
  xor2a  g013(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  oai21  g014(.a(new_n58_), .b(x01), .c(new_n53_), .O(new_n59_));
  xor2a  g015(.a(x07), .b(x04), .O(new_n60_));
  xor2a  g016(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  inv1   g017(.a(x09), .O(new_n62_));
  nand3  g018(.a(new_n46_), .b(new_n62_), .c(x08), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n61_), .O(z2));
  inv1   g020(.a(x27), .O(new_n65_));
  nor2   g021(.a(x23), .b(x18), .O(new_n66_));
  nor2   g022(.a(x22), .b(x17), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  inv1   g024(.a(x19), .O(new_n69_));
  inv1   g025(.a(x24), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g027(.a(x21), .b(x16), .O(new_n72_));
  nor2   g028(.a(x20), .b(x15), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(new_n75_));
  nand2  g031(.a(x03), .b(x02), .O(new_n76_));
  and2   g032(.a(new_n76_), .b(new_n46_), .O(new_n77_));
  and2   g033(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g034(.a(x25), .b(x01), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(x26), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n78_), .c(new_n65_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(z3));
  inv1   g038(.a(x28), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n65_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n83_), .b(new_n65_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n85_), .c(new_n80_), .d(new_n78_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(z4));
  inv1   g044(.a(x29), .O(new_n89_));
  nand2  g045(.a(new_n85_), .b(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n84_), .b(x29), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n90_), .c(new_n80_), .d(new_n78_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z5));
  nand3  g049(.a(new_n80_), .b(new_n76_), .c(new_n75_), .O(new_n94_));
  nand3  g050(.a(new_n84_), .b(x30), .c(x29), .O(new_n95_));
  nor2   g051(.a(x30), .b(x07), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n91_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  aoi21  g054(.a(new_n94_), .b(new_n46_), .c(new_n98_), .O(z6));
  nand2  g055(.a(new_n94_), .b(new_n46_), .O(new_n100_));
  inv1   g056(.a(x31), .O(new_n101_));
  nor2   g057(.a(new_n50_), .b(new_n101_), .O(new_n102_));
  nor2   g058(.a(new_n95_), .b(x31), .O(new_n103_));
  aoi21  g059(.a(new_n102_), .b(new_n95_), .c(new_n103_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n100_), .O(z7));
  nand4  g061(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  inv1   g063(.a(x11), .O(new_n108_));
  inv1   g064(.a(x16), .O(new_n109_));
  nand2  g065(.a(x18), .b(x17), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n108_), .c(new_n109_), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(x19), .c(new_n107_), .O(new_n112_));
  nand2  g068(.a(x17), .b(x13), .O(new_n113_));
  oai21  g069(.a(x17), .b(x12), .c(x18), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g071(.a(new_n69_), .b(x14), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n107_), .O(new_n117_));
  inv1   g073(.a(x15), .O(new_n118_));
  nand2  g074(.a(x16), .b(x10), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n110_), .c(new_n118_), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(new_n117_), .c(new_n115_), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n112_), .c(new_n46_), .O(new_n122_));
  oai21  g078(.a(x28), .b(x27), .c(x29), .O(new_n123_));
  nor2   g079(.a(new_n123_), .b(new_n45_), .O(new_n124_));
  nand2  g080(.a(new_n123_), .b(new_n45_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x31), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n46_), .c(new_n124_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n122_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x32), .O(new_n129_));
  inv1   g085(.a(x17), .O(new_n130_));
  nand3  g086(.a(new_n69_), .b(x18), .c(x14), .O(new_n131_));
  inv1   g087(.a(x18), .O(new_n132_));
  nand3  g088(.a(x19), .b(new_n132_), .c(x13), .O(new_n133_));
  aoi21  g089(.a(new_n133_), .b(new_n131_), .c(new_n130_), .O(new_n134_));
  nand4  g090(.a(x19), .b(x18), .c(new_n130_), .d(x12), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n134_), .c(x16), .O(new_n137_));
  nand3  g093(.a(x19), .b(x18), .c(x17), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n109_), .c(x11), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n137_), .c(new_n118_), .O(new_n141_));
  nor3   g097(.a(new_n138_), .b(new_n119_), .c(x15), .O(new_n142_));
  inv1   g098(.a(new_n124_), .O(new_n143_));
  nand4  g099(.a(new_n125_), .b(new_n143_), .c(new_n102_), .d(new_n49_), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  oai21  g101(.a(new_n142_), .b(new_n141_), .c(new_n145_), .O(new_n146_));
  inv1   g102(.a(x26), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x00), .O(new_n148_));
  aoi21  g104(.a(new_n146_), .b(new_n129_), .c(new_n148_), .O(z8));
  nand2  g105(.a(new_n123_), .b(new_n101_), .O(new_n150_));
  nand3  g106(.a(new_n86_), .b(x31), .c(x29), .O(new_n151_));
  aoi22  g107(.a(new_n151_), .b(x30), .c(new_n150_), .d(new_n96_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n122_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(x33), .O(new_n154_));
  nand3  g110(.a(new_n123_), .b(new_n96_), .c(new_n101_), .O(new_n155_));
  nand2  g111(.a(new_n124_), .b(x31), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n155_), .c(x33), .O(new_n157_));
  oai21  g113(.a(new_n142_), .b(new_n141_), .c(new_n157_), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n154_), .c(new_n148_), .O(z9));
endmodule


