// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_;
  inv1   g000(.a(x31), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x26), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  nand2  g004(.a(x33), .b(x03), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(x32), .b(new_n50_), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n49_), .c(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  nand2  g009(.a(x32), .b(x03), .O(new_n54_));
  nand2  g010(.a(x33), .b(new_n50_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n52_), .c(new_n46_), .O(z1));
  inv1   g013(.a(x01), .O(new_n58_));
  aoi21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  oai21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n53_), .O(new_n61_));
  aoi21  g017(.a(x05), .b(new_n50_), .c(new_n61_), .O(new_n62_));
  nand3  g018(.a(new_n61_), .b(x05), .c(new_n50_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  xor2a  g021(.a(x07), .b(x04), .O(new_n66_));
  xor2a  g022(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  inv1   g023(.a(x09), .O(new_n68_));
  nand3  g024(.a(new_n47_), .b(new_n68_), .c(x08), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n67_), .O(z2));
  inv1   g026(.a(x27), .O(new_n71_));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  nor2   g028(.a(x22), .b(x17), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(x24), .b(x19), .O(new_n75_));
  nor2   g031(.a(x21), .b(x16), .O(new_n76_));
  nor2   g032(.a(x20), .b(x15), .O(new_n77_));
  nor3   g033(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  nand2  g034(.a(new_n59_), .b(x25), .O(new_n79_));
  aoi21  g035(.a(new_n78_), .b(new_n74_), .c(new_n79_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n71_), .O(new_n81_));
  aoi21  g037(.a(new_n81_), .b(new_n45_), .c(x26), .O(z3));
  nor2   g038(.a(x28), .b(x27), .O(new_n83_));
  nand2  g039(.a(x28), .b(x27), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nor2   g041(.a(x31), .b(x26), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nor3   g043(.a(new_n87_), .b(new_n85_), .c(new_n83_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n80_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(z4));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n84_), .b(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n85_), .b(x29), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n92_), .c(new_n80_), .O(new_n94_));
  aoi21  g050(.a(new_n94_), .b(new_n45_), .c(x26), .O(z5));
  nand3  g051(.a(new_n85_), .b(x30), .c(x29), .O(new_n96_));
  inv1   g052(.a(x30), .O(new_n97_));
  aoi21  g053(.a(new_n93_), .b(new_n97_), .c(new_n87_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n96_), .c(new_n80_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z6));
  aoi21  g056(.a(new_n96_), .b(new_n80_), .c(x31), .O(new_n101_));
  or2    g057(.a(new_n101_), .b(x26), .O(z7));
  nand3  g058(.a(new_n86_), .b(x32), .c(x00), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(z8));
  nand2  g060(.a(x16), .b(x10), .O(new_n105_));
  nand2  g061(.a(x15), .b(x11), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g063(.a(x15), .O(new_n108_));
  inv1   g064(.a(x16), .O(new_n109_));
  nor2   g065(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  nand2  g067(.a(x18), .b(x17), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n111_), .c(new_n107_), .d(x19), .O(new_n114_));
  inv1   g070(.a(x17), .O(new_n115_));
  inv1   g071(.a(x19), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x18), .c(x14), .O(new_n117_));
  inv1   g073(.a(x18), .O(new_n118_));
  nand3  g074(.a(x19), .b(new_n118_), .c(x13), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n117_), .c(new_n115_), .O(new_n120_));
  nand4  g076(.a(x19), .b(x18), .c(new_n115_), .d(x12), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n120_), .c(new_n110_), .O(new_n123_));
  oai21  g079(.a(new_n83_), .b(new_n91_), .c(new_n97_), .O(new_n124_));
  aoi21  g080(.a(new_n123_), .b(new_n114_), .c(new_n124_), .O(new_n125_));
  inv1   g081(.a(x00), .O(new_n126_));
  nand2  g082(.a(x18), .b(x12), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n115_), .O(new_n128_));
  inv1   g084(.a(x13), .O(new_n129_));
  nand2  g085(.a(new_n118_), .b(new_n129_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n128_), .c(x33), .O(new_n131_));
  nor2   g087(.a(new_n131_), .b(new_n124_), .O(new_n132_));
  nand4  g088(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n116_), .O(new_n134_));
  oai21  g090(.a(new_n112_), .b(new_n105_), .c(new_n108_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  inv1   g092(.a(x14), .O(new_n137_));
  nor2   g093(.a(x19), .b(new_n137_), .O(new_n138_));
  nand3  g094(.a(x18), .b(x17), .c(x11), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n109_), .O(new_n140_));
  oai21  g096(.a(new_n133_), .b(new_n138_), .c(new_n140_), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n136_), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n132_), .c(new_n126_), .O(new_n143_));
  oai21  g099(.a(new_n125_), .b(x33), .c(new_n143_), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n45_), .c(x26), .O(z9));
endmodule


