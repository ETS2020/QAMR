// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x12), .O(new_n45_));
  oai21  g001(.a(x26), .b(new_n45_), .c(x32), .O(z0));
  xor2a  g002(.a(x03), .b(x02), .O(new_n47_));
  inv1   g003(.a(x32), .O(new_n48_));
  nor2   g004(.a(x26), .b(new_n45_), .O(new_n49_));
  aoi21  g005(.a(new_n47_), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n47_), .b(x33), .c(new_n50_), .O(z1));
  inv1   g007(.a(x08), .O(new_n52_));
  xor2a  g008(.a(x07), .b(x04), .O(new_n53_));
  nand2  g009(.a(new_n47_), .b(x01), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nand2  g011(.a(x05), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  xor2a  g014(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(x01), .c(new_n54_), .O(new_n60_));
  xor2a  g016(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nor4   g017(.a(new_n61_), .b(new_n49_), .c(x09), .d(new_n52_), .O(z2));
  inv1   g018(.a(x27), .O(new_n63_));
  and2   g019(.a(x25), .b(x01), .O(new_n64_));
  nand2  g020(.a(x03), .b(x02), .O(new_n65_));
  nor2   g021(.a(x20), .b(x15), .O(new_n66_));
  nor2   g022(.a(x24), .b(x19), .O(new_n67_));
  nor2   g023(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  inv1   g024(.a(x18), .O(new_n69_));
  inv1   g025(.a(x23), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g027(.a(x22), .b(x17), .O(new_n72_));
  nor2   g028(.a(x21), .b(x16), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(new_n75_));
  and2   g031(.a(new_n75_), .b(new_n65_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n64_), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(new_n78_));
  nor2   g034(.a(x26), .b(x12), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n78_), .c(new_n63_), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z3));
  nand2  g037(.a(new_n79_), .b(new_n64_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n63_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n63_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n86_), .c(new_n83_), .d(new_n76_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z4));
  inv1   g045(.a(new_n76_), .O(new_n90_));
  nor2   g046(.a(new_n85_), .b(x29), .O(new_n91_));
  nand2  g047(.a(new_n85_), .b(x29), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nor4   g049(.a(new_n93_), .b(new_n91_), .c(new_n82_), .d(new_n90_), .O(z5));
  nand2  g050(.a(new_n93_), .b(x30), .O(new_n95_));
  inv1   g051(.a(x30), .O(new_n96_));
  nand2  g052(.a(new_n92_), .b(new_n96_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n95_), .c(new_n78_), .O(new_n98_));
  aoi21  g054(.a(new_n98_), .b(new_n45_), .c(x26), .O(z6));
  inv1   g055(.a(x26), .O(new_n100_));
  inv1   g056(.a(x31), .O(new_n101_));
  xor2a  g057(.a(new_n95_), .b(new_n101_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n77_), .c(new_n45_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n100_), .O(z7));
  nand2  g060(.a(new_n87_), .b(x29), .O(new_n105_));
  inv1   g061(.a(x16), .O(new_n106_));
  inv1   g062(.a(x19), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x18), .c(x14), .O(new_n108_));
  nand3  g064(.a(x19), .b(new_n69_), .c(x13), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(new_n108_), .c(new_n106_), .O(new_n110_));
  nand4  g066(.a(x19), .b(x18), .c(new_n106_), .d(x11), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n110_), .c(x15), .O(new_n113_));
  inv1   g069(.a(x15), .O(new_n114_));
  nand3  g070(.a(x18), .b(x16), .c(x10), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x19), .c(new_n114_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n113_), .O(new_n118_));
  nor2   g074(.a(x32), .b(new_n101_), .O(new_n119_));
  inv1   g075(.a(x17), .O(new_n120_));
  nor2   g076(.a(new_n96_), .b(new_n120_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n119_), .c(new_n118_), .O(new_n122_));
  nand2  g078(.a(x32), .b(new_n96_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n105_), .O(new_n125_));
  inv1   g081(.a(new_n105_), .O(new_n126_));
  nor2   g082(.a(x30), .b(new_n120_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n119_), .c(new_n118_), .O(new_n128_));
  nand2  g084(.a(x32), .b(x30), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  nand2  g087(.a(new_n107_), .b(x14), .O(new_n132_));
  nand3  g088(.a(x18), .b(x16), .c(x15), .O(new_n133_));
  nand2  g089(.a(new_n115_), .b(new_n114_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n133_), .c(x19), .O(new_n135_));
  oai21  g091(.a(new_n133_), .b(new_n132_), .c(new_n135_), .O(new_n136_));
  aoi21  g092(.a(x18), .b(x11), .c(x16), .O(new_n137_));
  oai21  g093(.a(x18), .b(x13), .c(x17), .O(new_n138_));
  nor3   g094(.a(new_n138_), .b(new_n137_), .c(new_n101_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x32), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n131_), .c(new_n125_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x00), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n45_), .c(x26), .O(z8));
  inv1   g100(.a(x33), .O(new_n145_));
  nand2  g101(.a(new_n118_), .b(new_n145_), .O(new_n146_));
  nand2  g102(.a(new_n121_), .b(x31), .O(new_n147_));
  oai22  g103(.a(new_n147_), .b(new_n146_), .c(new_n145_), .d(x30), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n126_), .O(new_n149_));
  nand2  g105(.a(new_n127_), .b(new_n101_), .O(new_n150_));
  oai22  g106(.a(new_n150_), .b(new_n146_), .c(new_n145_), .d(new_n96_), .O(new_n151_));
  xor2a  g107(.a(x31), .b(x30), .O(new_n152_));
  nor3   g108(.a(new_n152_), .b(new_n138_), .c(new_n137_), .O(new_n153_));
  aoi21  g109(.a(new_n153_), .b(new_n136_), .c(new_n145_), .O(new_n154_));
  aoi21  g110(.a(new_n151_), .b(new_n105_), .c(new_n154_), .O(new_n155_));
  nand2  g111(.a(new_n79_), .b(x00), .O(new_n156_));
  aoi21  g112(.a(new_n155_), .b(new_n149_), .c(new_n156_), .O(z9));
endmodule


