// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_;
  inv1   g000(.a(x31), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x18), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x33), .O(new_n48_));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  inv1   g007(.a(new_n49_), .O(new_n52_));
  nand2  g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g009(.a(new_n53_), .b(new_n50_), .c(new_n46_), .O(z1));
  xor2a  g010(.a(x07), .b(x04), .O(new_n55_));
  inv1   g011(.a(x03), .O(new_n56_));
  nand2  g012(.a(x05), .b(new_n56_), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n57_), .c(x01), .O(new_n60_));
  oai21  g016(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  nand2  g017(.a(new_n49_), .b(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g019(.a(x09), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(x08), .O(new_n65_));
  aoi21  g021(.a(new_n63_), .b(new_n55_), .c(new_n65_), .O(new_n66_));
  oai21  g022(.a(new_n63_), .b(new_n55_), .c(new_n66_), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n46_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  nand2  g025(.a(x03), .b(x02), .O(new_n70_));
  nor2   g026(.a(x22), .b(x17), .O(new_n71_));
  nor2   g027(.a(x23), .b(x18), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g029(.a(x19), .O(new_n74_));
  inv1   g030(.a(x24), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x21), .b(x16), .O(new_n77_));
  nor2   g033(.a(x20), .b(x15), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n70_), .c(new_n46_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(x25), .b(x01), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(x26), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n82_), .c(new_n69_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(z3));
  inv1   g042(.a(x28), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n69_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n87_), .b(new_n69_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n89_), .c(new_n84_), .d(new_n82_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z4));
  nand2  g048(.a(new_n84_), .b(new_n70_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  and2   g050(.a(new_n94_), .b(new_n80_), .O(new_n95_));
  nand2  g051(.a(new_n88_), .b(x29), .O(new_n96_));
  inv1   g052(.a(x29), .O(new_n97_));
  nand2  g053(.a(new_n89_), .b(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n96_), .c(new_n95_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n46_), .O(z5));
  nand2  g056(.a(new_n80_), .b(new_n46_), .O(new_n101_));
  inv1   g057(.a(new_n96_), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(x30), .O(new_n103_));
  nand2  g059(.a(new_n102_), .b(x30), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  nor4   g061(.a(new_n105_), .b(new_n103_), .c(new_n93_), .d(new_n101_), .O(z6));
  oai21  g062(.a(new_n105_), .b(x18), .c(new_n45_), .O(new_n107_));
  nand2  g063(.a(new_n104_), .b(x31), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n107_), .c(new_n95_), .O(z7));
  inv1   g065(.a(x18), .O(new_n110_));
  inv1   g066(.a(x00), .O(new_n111_));
  nor2   g067(.a(x26), .b(new_n111_), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n51_), .c(new_n110_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n45_), .O(new_n115_));
  inv1   g071(.a(x30), .O(new_n116_));
  inv1   g072(.a(x15), .O(new_n117_));
  inv1   g073(.a(x17), .O(new_n118_));
  nand3  g074(.a(new_n74_), .b(x18), .c(x14), .O(new_n119_));
  nand4  g075(.a(x31), .b(x19), .c(new_n110_), .d(x13), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nand4  g077(.a(x19), .b(x18), .c(new_n118_), .d(x12), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n121_), .c(x16), .O(new_n124_));
  inv1   g080(.a(x16), .O(new_n125_));
  nand3  g081(.a(x18), .b(x17), .c(x11), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(x19), .c(new_n125_), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(new_n124_), .c(new_n117_), .O(new_n129_));
  nand3  g085(.a(x17), .b(x16), .c(x10), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x19), .c(new_n117_), .O(new_n132_));
  nor2   g088(.a(new_n132_), .b(new_n110_), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n129_), .c(new_n116_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n51_), .O(new_n135_));
  nand2  g091(.a(new_n90_), .b(x29), .O(new_n136_));
  aoi21  g092(.a(x32), .b(new_n116_), .c(new_n136_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g094(.a(x32), .b(new_n116_), .O(new_n139_));
  nor2   g095(.a(x32), .b(new_n116_), .O(new_n140_));
  oai21  g096(.a(new_n133_), .b(new_n129_), .c(new_n140_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n136_), .O(new_n143_));
  inv1   g099(.a(x13), .O(new_n144_));
  nor2   g100(.a(x17), .b(x12), .O(new_n145_));
  oai22  g101(.a(new_n145_), .b(new_n110_), .c(new_n118_), .d(new_n144_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(x16), .c(new_n127_), .O(new_n147_));
  inv1   g103(.a(x14), .O(new_n148_));
  nor2   g104(.a(x19), .b(new_n148_), .O(new_n149_));
  nand3  g105(.a(x17), .b(x16), .c(x15), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x18), .O(new_n152_));
  nor2   g108(.a(new_n152_), .b(new_n149_), .O(new_n153_));
  nand2  g109(.a(new_n152_), .b(new_n74_), .O(new_n154_));
  oai21  g110(.a(new_n130_), .b(new_n110_), .c(new_n117_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  or2    g112(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n147_), .c(x32), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n143_), .c(new_n138_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n112_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n115_), .O(z8));
  nor3   g117(.a(new_n150_), .b(new_n74_), .c(new_n144_), .O(new_n162_));
  oai21  g118(.a(x31), .b(new_n116_), .c(new_n162_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n110_), .O(new_n164_));
  aoi21  g120(.a(new_n150_), .b(new_n74_), .c(new_n116_), .O(new_n165_));
  nand2  g121(.a(new_n130_), .b(new_n117_), .O(new_n166_));
  nand2  g122(.a(x17), .b(x11), .O(new_n167_));
  oai21  g123(.a(new_n145_), .b(new_n125_), .c(new_n167_), .O(new_n168_));
  nand3  g124(.a(new_n168_), .b(new_n166_), .c(new_n165_), .O(new_n169_));
  oai21  g125(.a(new_n169_), .b(new_n153_), .c(x31), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n164_), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(x33), .O(new_n172_));
  inv1   g128(.a(new_n136_), .O(new_n173_));
  nand2  g129(.a(x19), .b(new_n125_), .O(new_n174_));
  aoi21  g130(.a(new_n118_), .b(x12), .c(new_n149_), .O(new_n175_));
  oai21  g131(.a(x19), .b(x17), .c(x16), .O(new_n176_));
  oai22  g132(.a(new_n176_), .b(new_n175_), .c(new_n167_), .d(new_n174_), .O(new_n177_));
  nand2  g133(.a(new_n132_), .b(x18), .O(new_n178_));
  aoi21  g134(.a(new_n177_), .b(x15), .c(new_n178_), .O(new_n179_));
  nor3   g135(.a(x33), .b(new_n45_), .c(new_n116_), .O(new_n180_));
  oai21  g136(.a(new_n162_), .b(x18), .c(new_n180_), .O(new_n181_));
  nor2   g137(.a(x31), .b(x18), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(x33), .O(new_n183_));
  oai21  g139(.a(new_n181_), .b(new_n179_), .c(new_n183_), .O(new_n184_));
  nand2  g140(.a(x33), .b(x31), .O(new_n185_));
  nand4  g141(.a(new_n182_), .b(new_n162_), .c(new_n48_), .d(new_n116_), .O(new_n186_));
  aoi21  g142(.a(new_n186_), .b(new_n185_), .c(new_n173_), .O(new_n187_));
  aoi21  g143(.a(new_n184_), .b(new_n173_), .c(new_n187_), .O(new_n188_));
  aoi21  g144(.a(new_n188_), .b(new_n172_), .c(new_n113_), .O(z9));
endmodule


