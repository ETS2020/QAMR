// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_;
  inv1   g000(.a(x15), .O(new_n45_));
  inv1   g001(.a(x26), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  inv1   g009(.a(x32), .O(new_n54_));
  aoi21  g010(.a(x33), .b(new_n49_), .c(x02), .O(new_n55_));
  oai21  g011(.a(new_n54_), .b(new_n49_), .c(new_n55_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n47_), .O(z1));
  inv1   g013(.a(new_n47_), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  aoi21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  oai21  g016(.a(x03), .b(x02), .c(new_n60_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n51_), .O(new_n62_));
  aoi21  g018(.a(x05), .b(new_n49_), .c(new_n62_), .O(new_n63_));
  nand3  g019(.a(new_n62_), .b(x05), .c(new_n49_), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n63_), .c(new_n61_), .O(new_n66_));
  xnor2a g022(.a(x07), .b(x04), .O(new_n67_));
  inv1   g023(.a(x09), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(x08), .O(new_n69_));
  aoi21  g025(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  oai21  g026(.a(new_n67_), .b(new_n66_), .c(new_n70_), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n58_), .O(z2));
  inv1   g028(.a(x27), .O(new_n73_));
  nor2   g029(.a(x23), .b(x18), .O(new_n74_));
  nor2   g030(.a(x22), .b(x17), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x24), .b(x19), .O(new_n77_));
  nor2   g033(.a(x21), .b(x16), .O(new_n78_));
  nor2   g034(.a(x20), .b(x15), .O(new_n79_));
  nor3   g035(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  nand3  g036(.a(new_n60_), .b(new_n46_), .c(x25), .O(new_n81_));
  aoi21  g037(.a(new_n80_), .b(new_n76_), .c(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n73_), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(z3));
  inv1   g040(.a(x28), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n73_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n85_), .b(new_n73_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n87_), .c(new_n82_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(z4));
  nand2  g046(.a(new_n86_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n87_), .b(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n91_), .c(new_n82_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z5));
  inv1   g051(.a(x30), .O(new_n96_));
  xor2a  g052(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n82_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z6));
  inv1   g055(.a(x31), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n96_), .O(new_n101_));
  nor2   g057(.a(x31), .b(x30), .O(new_n102_));
  or2    g058(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  or2    g059(.a(new_n103_), .b(new_n91_), .O(new_n104_));
  nand2  g060(.a(new_n91_), .b(x31), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n104_), .c(new_n82_), .O(z7));
  inv1   g062(.a(x00), .O(new_n107_));
  nand3  g063(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  and2   g065(.a(x16), .b(x10), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n109_), .c(x15), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand2  g068(.a(new_n88_), .b(x29), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x30), .O(new_n115_));
  aoi21  g071(.a(new_n113_), .b(new_n96_), .c(new_n100_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n115_), .c(new_n112_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n46_), .O(new_n118_));
  inv1   g074(.a(x17), .O(new_n119_));
  inv1   g075(.a(x18), .O(new_n120_));
  nor2   g076(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g077(.a(x18), .b(x12), .O(new_n122_));
  oai21  g078(.a(x18), .b(x13), .c(x17), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  aoi22  g080(.a(new_n124_), .b(x16), .c(new_n121_), .d(x11), .O(new_n125_));
  nand2  g081(.a(new_n121_), .b(x16), .O(new_n126_));
  inv1   g082(.a(x19), .O(new_n127_));
  nand2  g083(.a(new_n126_), .b(new_n127_), .O(new_n128_));
  inv1   g084(.a(x14), .O(new_n129_));
  nor2   g085(.a(x19), .b(new_n129_), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n126_), .c(new_n128_), .O(new_n131_));
  nor2   g087(.a(new_n131_), .b(new_n125_), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n45_), .c(new_n118_), .O(new_n133_));
  nand3  g089(.a(new_n127_), .b(x18), .c(x14), .O(new_n134_));
  nand3  g090(.a(x19), .b(new_n120_), .c(x13), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n134_), .c(new_n119_), .O(new_n136_));
  nand4  g092(.a(x19), .b(x18), .c(new_n119_), .d(x12), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  inv1   g095(.a(x16), .O(new_n140_));
  nand3  g096(.a(new_n109_), .b(new_n140_), .c(x11), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand2  g098(.a(new_n121_), .b(new_n110_), .O(new_n143_));
  nor4   g099(.a(new_n143_), .b(x26), .c(new_n127_), .d(x15), .O(new_n144_));
  aoi21  g100(.a(new_n142_), .b(x15), .c(new_n144_), .O(new_n145_));
  nand3  g101(.a(new_n116_), .b(new_n115_), .c(new_n54_), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  aoi21  g103(.a(new_n133_), .b(x32), .c(new_n147_), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n107_), .c(new_n58_), .O(z8));
  nand2  g105(.a(x33), .b(x30), .O(new_n150_));
  nor3   g106(.a(x33), .b(new_n127_), .c(new_n120_), .O(new_n151_));
  nand3  g107(.a(new_n110_), .b(x17), .c(new_n45_), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n151_), .c(new_n102_), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n150_), .c(x26), .O(new_n155_));
  nand3  g111(.a(new_n142_), .b(new_n102_), .c(new_n50_), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n150_), .c(new_n45_), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n155_), .c(new_n113_), .O(new_n158_));
  nand2  g114(.a(x33), .b(new_n96_), .O(new_n159_));
  nand3  g115(.a(new_n153_), .b(new_n151_), .c(new_n101_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n159_), .c(x26), .O(new_n161_));
  nand3  g117(.a(new_n142_), .b(new_n101_), .c(new_n50_), .O(new_n162_));
  aoi21  g118(.a(new_n162_), .b(new_n159_), .c(new_n45_), .O(new_n163_));
  oai21  g119(.a(new_n163_), .b(new_n161_), .c(new_n114_), .O(new_n164_));
  aoi21  g120(.a(new_n132_), .b(new_n103_), .c(new_n45_), .O(new_n165_));
  aoi21  g121(.a(new_n112_), .b(new_n103_), .c(x26), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n165_), .c(x33), .O(new_n167_));
  nand3  g123(.a(new_n167_), .b(new_n164_), .c(new_n158_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(x00), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n58_), .O(z9));
endmodule


