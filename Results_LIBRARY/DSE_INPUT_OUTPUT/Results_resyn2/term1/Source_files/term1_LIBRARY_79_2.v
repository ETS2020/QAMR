// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_;
  inv1   g000(.a(x10), .O(new_n45_));
  nor2   g001(.a(x18), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  nand2  g009(.a(x32), .b(x03), .O(new_n54_));
  nand2  g010(.a(x33), .b(new_n49_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n54_), .c(new_n51_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n46_), .O(z1));
  xor2a  g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n49_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x06), .c(new_n51_), .O(new_n61_));
  nand2  g017(.a(x06), .b(new_n51_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x05), .c(new_n49_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(new_n61_), .c(new_n59_), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(new_n65_));
  nand2  g021(.a(x03), .b(x02), .O(new_n66_));
  aoi21  g022(.a(new_n49_), .b(new_n51_), .c(new_n59_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  nor2   g024(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  inv1   g025(.a(x09), .O(new_n70_));
  nand2  g026(.a(new_n68_), .b(new_n58_), .O(new_n71_));
  nand3  g027(.a(new_n71_), .b(new_n70_), .c(x08), .O(new_n72_));
  oai21  g028(.a(new_n72_), .b(new_n69_), .c(new_n47_), .O(z2));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  nor2   g030(.a(x21), .b(x16), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g032(.a(x19), .O(new_n77_));
  inv1   g033(.a(x24), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g035(.a(x23), .b(x18), .O(new_n80_));
  nor2   g036(.a(x22), .b(x17), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n79_), .c(new_n76_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n66_), .O(new_n84_));
  inv1   g040(.a(x27), .O(new_n85_));
  nand2  g041(.a(x25), .b(x01), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(x26), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n84_), .c(new_n47_), .O(z3));
  inv1   g045(.a(x28), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n85_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n90_), .b(new_n85_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n92_), .c(new_n87_), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n84_), .c(new_n47_), .O(z4));
  inv1   g051(.a(new_n84_), .O(new_n96_));
  nand3  g052(.a(new_n87_), .b(new_n96_), .c(new_n47_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(new_n98_));
  nand2  g054(.a(new_n91_), .b(x29), .O(new_n99_));
  inv1   g055(.a(x29), .O(new_n100_));
  nand2  g056(.a(new_n92_), .b(new_n100_), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n99_), .c(new_n98_), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(z5));
  inv1   g059(.a(x30), .O(new_n104_));
  xor2a  g060(.a(new_n99_), .b(new_n104_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n98_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(z6));
  inv1   g063(.a(new_n99_), .O(new_n108_));
  inv1   g064(.a(x31), .O(new_n109_));
  nor2   g065(.a(new_n109_), .b(new_n104_), .O(new_n110_));
  nor2   g066(.a(x31), .b(x30), .O(new_n111_));
  nor2   g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  oai21  g068(.a(new_n108_), .b(new_n109_), .c(new_n87_), .O(new_n113_));
  aoi21  g069(.a(new_n112_), .b(new_n108_), .c(new_n113_), .O(new_n114_));
  aoi21  g070(.a(new_n114_), .b(new_n96_), .c(new_n46_), .O(z7));
  inv1   g071(.a(x26), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x00), .O(new_n117_));
  inv1   g073(.a(x15), .O(new_n118_));
  nand3  g074(.a(x19), .b(x17), .c(x16), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  inv1   g076(.a(x12), .O(new_n121_));
  inv1   g077(.a(x17), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g079(.a(new_n77_), .b(x14), .O(new_n124_));
  nand3  g080(.a(x17), .b(x16), .c(x15), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n123_), .c(new_n120_), .O(new_n128_));
  nand2  g084(.a(new_n93_), .b(x29), .O(new_n129_));
  xor2a  g085(.a(new_n129_), .b(new_n104_), .O(new_n130_));
  oai21  g086(.a(x19), .b(x17), .c(x16), .O(new_n131_));
  nand3  g087(.a(x19), .b(x17), .c(x11), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n130_), .c(x31), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n128_), .c(x18), .O(new_n135_));
  inv1   g091(.a(x13), .O(new_n136_));
  inv1   g092(.a(x18), .O(new_n137_));
  oai21  g093(.a(new_n119_), .b(new_n136_), .c(new_n137_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(x15), .O(new_n139_));
  nand2  g095(.a(new_n130_), .b(x31), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n139_), .c(new_n45_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n135_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x32), .O(new_n143_));
  nand3  g099(.a(new_n77_), .b(x18), .c(x14), .O(new_n144_));
  nand4  g100(.a(x19), .b(new_n137_), .c(x13), .d(new_n45_), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n144_), .c(new_n122_), .O(new_n146_));
  nand4  g102(.a(x19), .b(x18), .c(new_n122_), .d(x12), .O(new_n147_));
  inv1   g103(.a(new_n147_), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n146_), .c(x16), .O(new_n149_));
  nor2   g105(.a(new_n132_), .b(x16), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x18), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n149_), .c(new_n118_), .O(new_n152_));
  inv1   g108(.a(new_n119_), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n118_), .c(x10), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n137_), .O(new_n155_));
  nor2   g111(.a(x32), .b(new_n109_), .O(new_n156_));
  and2   g112(.a(new_n156_), .b(new_n130_), .O(new_n157_));
  oai21  g113(.a(new_n155_), .b(new_n152_), .c(new_n157_), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n143_), .c(new_n117_), .O(z8));
  nand2  g115(.a(x33), .b(x30), .O(new_n160_));
  nand3  g116(.a(new_n50_), .b(x19), .c(new_n137_), .O(new_n161_));
  inv1   g117(.a(new_n161_), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(new_n126_), .c(new_n111_), .d(x13), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(new_n160_), .c(x10), .O(new_n164_));
  nand2  g120(.a(new_n122_), .b(x12), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(new_n124_), .c(new_n131_), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n150_), .c(x15), .O(new_n167_));
  aoi21  g123(.a(new_n167_), .b(new_n154_), .c(x33), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n111_), .O(new_n169_));
  aoi21  g125(.a(new_n169_), .b(new_n160_), .c(new_n137_), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n164_), .c(new_n129_), .O(new_n171_));
  inv1   g127(.a(new_n129_), .O(new_n172_));
  nor2   g128(.a(new_n50_), .b(x30), .O(new_n173_));
  nand2  g129(.a(new_n162_), .b(new_n110_), .O(new_n174_));
  nor3   g130(.a(new_n174_), .b(new_n125_), .c(new_n136_), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n173_), .c(new_n45_), .O(new_n176_));
  aoi21  g132(.a(new_n168_), .b(new_n110_), .c(new_n173_), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n137_), .c(new_n176_), .O(new_n178_));
  oai21  g134(.a(new_n139_), .b(new_n112_), .c(new_n45_), .O(new_n179_));
  oai21  g135(.a(new_n111_), .b(new_n110_), .c(new_n133_), .O(new_n180_));
  oai21  g136(.a(new_n180_), .b(new_n128_), .c(x18), .O(new_n181_));
  aoi21  g137(.a(new_n181_), .b(new_n179_), .c(new_n50_), .O(new_n182_));
  aoi21  g138(.a(new_n178_), .b(new_n172_), .c(new_n182_), .O(new_n183_));
  aoi21  g139(.a(new_n183_), .b(new_n171_), .c(new_n117_), .O(z9));
endmodule


