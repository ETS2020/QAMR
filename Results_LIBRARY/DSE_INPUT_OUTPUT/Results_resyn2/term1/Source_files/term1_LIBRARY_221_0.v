// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_;
  nor2   g000(.a(x23), .b(x18), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nand2  g002(.a(x03), .b(x02), .O(new_n47_));
  inv1   g003(.a(x02), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  and2   g006(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  inv1   g008(.a(x33), .O(new_n53_));
  aoi21  g009(.a(new_n52_), .b(new_n53_), .c(new_n45_), .O(new_n54_));
  oai21  g010(.a(new_n52_), .b(x32), .c(new_n54_), .O(z1));
  inv1   g011(.a(new_n45_), .O(new_n56_));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n49_), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n48_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  or2    g016(.a(new_n59_), .b(new_n58_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  aoi21  g018(.a(new_n51_), .b(x01), .c(new_n62_), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n63_), .b(new_n57_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n64_), .c(new_n56_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  oai22  g025(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n70_));
  oai22  g026(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x26), .O(new_n73_));
  nand4  g029(.a(new_n47_), .b(new_n73_), .c(x25), .d(x01), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n56_), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n69_), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z3));
  inv1   g035(.a(x28), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n69_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n80_), .b(new_n69_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n82_), .c(new_n75_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n56_), .O(z4));
  nand2  g041(.a(new_n81_), .b(x29), .O(new_n86_));
  inv1   g042(.a(x29), .O(new_n87_));
  nand2  g043(.a(new_n82_), .b(new_n87_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n86_), .c(new_n77_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(z5));
  xnor2a g046(.a(new_n86_), .b(x30), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n77_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z6));
  xor2a  g049(.a(x31), .b(x30), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(new_n86_), .O(new_n96_));
  aoi21  g052(.a(new_n86_), .b(x31), .c(new_n96_), .O(new_n97_));
  aoi21  g053(.a(new_n97_), .b(new_n75_), .c(new_n45_), .O(z7));
  inv1   g054(.a(x18), .O(new_n99_));
  nand3  g055(.a(x17), .b(x16), .c(x15), .O(new_n100_));
  nand2  g056(.a(x19), .b(x13), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  inv1   g058(.a(x31), .O(new_n103_));
  oai21  g059(.a(x28), .b(x27), .c(x29), .O(new_n104_));
  xor2a  g060(.a(new_n104_), .b(x30), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n102_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x23), .O(new_n108_));
  inv1   g064(.a(new_n100_), .O(new_n109_));
  inv1   g065(.a(x15), .O(new_n110_));
  inv1   g066(.a(x17), .O(new_n111_));
  nand2  g067(.a(x16), .b(x10), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(x19), .c(new_n109_), .O(new_n114_));
  inv1   g070(.a(x19), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(x14), .c(new_n100_), .O(new_n116_));
  nand2  g072(.a(x16), .b(x12), .O(new_n117_));
  oai21  g073(.a(x16), .b(x11), .c(x17), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n106_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n114_), .c(x18), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n108_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x32), .O(new_n123_));
  nand2  g079(.a(x18), .b(x17), .O(new_n124_));
  nor4   g080(.a(new_n124_), .b(new_n112_), .c(new_n115_), .d(x15), .O(new_n125_));
  nand3  g081(.a(x18), .b(x17), .c(x11), .O(new_n126_));
  nor3   g082(.a(new_n126_), .b(new_n115_), .c(x16), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand4  g084(.a(x19), .b(x18), .c(new_n111_), .d(x12), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  nand3  g086(.a(new_n115_), .b(x18), .c(x14), .O(new_n131_));
  nand3  g087(.a(x19), .b(new_n99_), .c(x13), .O(new_n132_));
  aoi21  g088(.a(new_n132_), .b(new_n131_), .c(new_n111_), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n130_), .c(x16), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n128_), .c(new_n110_), .O(new_n135_));
  inv1   g091(.a(x23), .O(new_n136_));
  nand3  g092(.a(new_n131_), .b(new_n129_), .c(new_n136_), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  nor2   g094(.a(new_n127_), .b(new_n125_), .O(new_n139_));
  inv1   g095(.a(x32), .O(new_n140_));
  nand2  g096(.a(new_n106_), .b(new_n140_), .O(new_n141_));
  aoi21  g097(.a(new_n139_), .b(new_n138_), .c(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n135_), .b(new_n125_), .c(new_n142_), .O(new_n143_));
  nand2  g099(.a(new_n73_), .b(x00), .O(new_n144_));
  aoi21  g100(.a(new_n143_), .b(new_n123_), .c(new_n144_), .O(z8));
  nand2  g101(.a(new_n134_), .b(new_n128_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(x15), .c(new_n125_), .O(new_n147_));
  nand2  g103(.a(new_n105_), .b(new_n95_), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n147_), .c(new_n53_), .O(new_n149_));
  nand2  g105(.a(x16), .b(x15), .O(new_n150_));
  oai21  g106(.a(new_n124_), .b(new_n150_), .c(new_n115_), .O(new_n151_));
  inv1   g107(.a(x16), .O(new_n152_));
  nand2  g108(.a(new_n126_), .b(new_n152_), .O(new_n153_));
  oai21  g109(.a(new_n124_), .b(new_n112_), .c(new_n110_), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(new_n153_), .c(new_n151_), .O(new_n155_));
  nand2  g111(.a(new_n116_), .b(x18), .O(new_n156_));
  nand2  g112(.a(x18), .b(x12), .O(new_n157_));
  oai21  g113(.a(x18), .b(x13), .c(x17), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nor2   g115(.a(new_n94_), .b(new_n53_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n159_), .c(new_n156_), .d(new_n105_), .O(new_n161_));
  nor2   g117(.a(new_n161_), .b(new_n155_), .O(new_n162_));
  nor2   g118(.a(new_n162_), .b(new_n144_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n149_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n56_), .O(z9));
endmodule


