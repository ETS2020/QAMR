// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x18), .O(new_n45_));
  nand2  g001(.a(x26), .b(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x33), .O(new_n48_));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  inv1   g007(.a(new_n49_), .O(new_n52_));
  nand2  g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g009(.a(new_n53_), .b(new_n50_), .c(new_n46_), .O(z1));
  inv1   g010(.a(x01), .O(new_n55_));
  aoi21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  oai21  g012(.a(x03), .b(x02), .c(new_n56_), .O(new_n57_));
  inv1   g013(.a(x03), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(new_n64_));
  xnor2a g020(.a(x07), .b(x04), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(x08), .O(new_n67_));
  aoi21  g023(.a(new_n65_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  oai21  g024(.a(new_n65_), .b(new_n64_), .c(new_n68_), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n46_), .O(z2));
  inv1   g026(.a(x23), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n45_), .O(new_n72_));
  inv1   g028(.a(x16), .O(new_n73_));
  inv1   g029(.a(x21), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g031(.a(x15), .O(new_n76_));
  inv1   g032(.a(x20), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor2   g034(.a(x24), .b(x19), .O(new_n79_));
  nor2   g035(.a(x22), .b(x17), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n78_), .c(new_n75_), .d(new_n72_), .O(new_n82_));
  nand2  g038(.a(new_n72_), .b(x26), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n82_), .c(new_n56_), .d(x25), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(x27), .c(new_n46_), .O(z3));
  inv1   g041(.a(x26), .O(new_n86_));
  nand3  g042(.a(new_n56_), .b(new_n86_), .c(x25), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  and2   g044(.a(new_n88_), .b(new_n82_), .O(new_n89_));
  inv1   g045(.a(x27), .O(new_n90_));
  inv1   g046(.a(x28), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g048(.a(new_n91_), .b(new_n90_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n92_), .c(new_n89_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z4));
  nand2  g052(.a(new_n93_), .b(x29), .O(new_n97_));
  inv1   g053(.a(x29), .O(new_n98_));
  nand2  g054(.a(new_n94_), .b(new_n98_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n84_), .c(new_n46_), .O(z5));
  nand3  g057(.a(new_n93_), .b(x30), .c(x29), .O(new_n102_));
  inv1   g058(.a(x30), .O(new_n103_));
  nand2  g059(.a(new_n97_), .b(new_n103_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n84_), .c(new_n46_), .O(z6));
  xor2a  g062(.a(new_n102_), .b(x31), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n89_), .O(z7));
  inv1   g064(.a(x17), .O(new_n109_));
  inv1   g065(.a(x19), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x18), .c(x14), .O(new_n111_));
  nand3  g067(.a(x19), .b(new_n45_), .c(x13), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n111_), .c(new_n109_), .O(new_n113_));
  nand4  g069(.a(x19), .b(x18), .c(new_n109_), .d(x12), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n113_), .c(x16), .O(new_n116_));
  nand3  g072(.a(x18), .b(x17), .c(x11), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x19), .c(new_n73_), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n116_), .c(new_n76_), .O(new_n120_));
  nor2   g076(.a(new_n45_), .b(new_n109_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x16), .c(x10), .O(new_n122_));
  nor3   g078(.a(new_n122_), .b(new_n110_), .c(x15), .O(new_n123_));
  inv1   g079(.a(x31), .O(new_n124_));
  nor2   g080(.a(x32), .b(new_n124_), .O(new_n125_));
  oai21  g081(.a(new_n123_), .b(new_n120_), .c(new_n125_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n103_), .O(new_n127_));
  nand2  g083(.a(new_n92_), .b(x29), .O(new_n128_));
  aoi21  g084(.a(new_n51_), .b(x30), .c(new_n128_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g086(.a(new_n126_), .b(x30), .O(new_n131_));
  aoi22  g087(.a(new_n92_), .b(x29), .c(new_n51_), .d(new_n103_), .O(new_n132_));
  oai21  g088(.a(new_n118_), .b(x16), .c(x15), .O(new_n133_));
  inv1   g089(.a(x14), .O(new_n134_));
  nor2   g090(.a(x19), .b(new_n134_), .O(new_n135_));
  nand3  g091(.a(new_n121_), .b(x16), .c(x15), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n110_), .O(new_n137_));
  oai21  g093(.a(new_n136_), .b(new_n135_), .c(new_n137_), .O(new_n138_));
  aoi21  g094(.a(new_n133_), .b(new_n122_), .c(new_n138_), .O(new_n139_));
  aoi21  g095(.a(x18), .b(x12), .c(x17), .O(new_n140_));
  nor2   g096(.a(x18), .b(x13), .O(new_n141_));
  nor3   g097(.a(new_n141_), .b(new_n140_), .c(new_n124_), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n139_), .c(new_n51_), .O(new_n143_));
  aoi21  g099(.a(new_n132_), .b(new_n131_), .c(new_n143_), .O(new_n144_));
  nand2  g100(.a(new_n86_), .b(x00), .O(new_n145_));
  aoi21  g101(.a(new_n144_), .b(new_n130_), .c(new_n145_), .O(z8));
  aoi21  g102(.a(x31), .b(x30), .c(new_n128_), .O(new_n147_));
  nand2  g103(.a(new_n124_), .b(new_n103_), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n128_), .c(new_n147_), .O(new_n149_));
  oai21  g105(.a(new_n123_), .b(new_n120_), .c(new_n149_), .O(new_n150_));
  inv1   g106(.a(new_n145_), .O(new_n151_));
  nor3   g107(.a(new_n141_), .b(new_n140_), .c(new_n48_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n149_), .c(new_n139_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  aoi21  g110(.a(new_n150_), .b(new_n48_), .c(new_n154_), .O(z9));
endmodule


