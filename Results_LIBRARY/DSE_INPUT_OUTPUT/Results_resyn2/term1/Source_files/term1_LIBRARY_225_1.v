// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x28), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  aoi21  g007(.a(new_n50_), .b(new_n51_), .c(new_n46_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(x33), .c(new_n52_), .O(z1));
  xor2a  g009(.a(x07), .b(x04), .O(new_n54_));
  inv1   g010(.a(x03), .O(new_n55_));
  nand2  g011(.a(x05), .b(new_n55_), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  aoi21  g014(.a(new_n58_), .b(new_n56_), .c(x01), .O(new_n59_));
  oai21  g015(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n49_), .b(x01), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g018(.a(x09), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x08), .O(new_n64_));
  aoi21  g020(.a(new_n62_), .b(new_n54_), .c(new_n64_), .O(new_n65_));
  oai21  g021(.a(new_n62_), .b(new_n54_), .c(new_n65_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n47_), .O(z2));
  inv1   g023(.a(x27), .O(new_n68_));
  nor2   g024(.a(x21), .b(x16), .O(new_n69_));
  nor2   g025(.a(x24), .b(x19), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g027(.a(x22), .b(x17), .O(new_n72_));
  nor2   g028(.a(x20), .b(x15), .O(new_n73_));
  nor2   g029(.a(x23), .b(x18), .O(new_n74_));
  nor3   g030(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(new_n75_));
  inv1   g031(.a(x26), .O(new_n76_));
  nand2  g032(.a(x03), .b(x02), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n76_), .c(x25), .d(x01), .O(new_n78_));
  aoi21  g034(.a(new_n75_), .b(new_n71_), .c(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n47_), .c(new_n68_), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z3));
  inv1   g037(.a(x28), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n68_), .O(new_n83_));
  nor2   g039(.a(new_n82_), .b(new_n68_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n83_), .c(new_n79_), .d(new_n47_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(z4));
  inv1   g043(.a(x29), .O(new_n88_));
  nand2  g044(.a(new_n85_), .b(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n84_), .b(x29), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n89_), .c(new_n79_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n47_), .O(z5));
  nand3  g048(.a(x30), .b(x29), .c(x27), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  aoi21  g050(.a(new_n90_), .b(new_n45_), .c(new_n94_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n79_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n47_), .O(z6));
  nand3  g053(.a(x31), .b(x30), .c(x29), .O(new_n98_));
  oai22  g054(.a(new_n98_), .b(new_n68_), .c(new_n94_), .d(x31), .O(new_n99_));
  aoi21  g055(.a(new_n99_), .b(new_n79_), .c(new_n46_), .O(z7));
  nand2  g056(.a(new_n76_), .b(x00), .O(new_n101_));
  nand2  g057(.a(x32), .b(new_n68_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n101_), .c(new_n45_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n82_), .O(new_n104_));
  inv1   g060(.a(new_n101_), .O(new_n105_));
  nand4  g061(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n106_));
  nand3  g062(.a(x18), .b(x17), .c(x11), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(x16), .c(x19), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand2  g066(.a(x18), .b(x12), .O(new_n111_));
  oai21  g067(.a(x18), .b(x13), .c(x17), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  inv1   g069(.a(x15), .O(new_n114_));
  inv1   g070(.a(x19), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(x14), .c(new_n106_), .O(new_n116_));
  nand4  g072(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n114_), .c(new_n116_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n113_), .c(new_n110_), .O(new_n119_));
  inv1   g075(.a(x31), .O(new_n120_));
  aoi21  g076(.a(x30), .b(x29), .c(new_n120_), .O(new_n121_));
  oai21  g077(.a(x30), .b(x29), .c(new_n121_), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n119_), .c(x32), .O(new_n123_));
  inv1   g079(.a(x17), .O(new_n124_));
  nand3  g080(.a(new_n115_), .b(x18), .c(x14), .O(new_n125_));
  inv1   g081(.a(x18), .O(new_n126_));
  nand3  g082(.a(x19), .b(new_n126_), .c(x13), .O(new_n127_));
  aoi21  g083(.a(new_n127_), .b(new_n125_), .c(new_n124_), .O(new_n128_));
  nand4  g084(.a(x19), .b(x18), .c(new_n124_), .d(x12), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n128_), .c(x16), .O(new_n131_));
  inv1   g087(.a(x16), .O(new_n132_));
  nand3  g088(.a(new_n108_), .b(x19), .c(new_n132_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nor3   g090(.a(new_n117_), .b(new_n115_), .c(x15), .O(new_n135_));
  aoi21  g091(.a(new_n134_), .b(x15), .c(new_n135_), .O(new_n136_));
  aoi21  g092(.a(new_n83_), .b(x29), .c(x30), .O(new_n137_));
  nand2  g093(.a(new_n121_), .b(new_n51_), .O(new_n138_));
  or2    g094(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n136_), .c(new_n123_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n105_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n104_), .O(z8));
  inv1   g098(.a(new_n98_), .O(new_n143_));
  aoi21  g099(.a(new_n137_), .b(new_n120_), .c(new_n143_), .O(new_n144_));
  inv1   g100(.a(x33), .O(new_n145_));
  nor2   g101(.a(new_n46_), .b(new_n145_), .O(new_n146_));
  oai21  g102(.a(new_n144_), .b(new_n119_), .c(new_n146_), .O(new_n147_));
  aoi21  g103(.a(new_n133_), .b(new_n131_), .c(new_n114_), .O(new_n148_));
  nand2  g104(.a(new_n137_), .b(new_n120_), .O(new_n149_));
  nand2  g105(.a(new_n143_), .b(x28), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n149_), .c(x33), .O(new_n151_));
  oai21  g107(.a(new_n135_), .b(new_n148_), .c(new_n151_), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n147_), .c(new_n101_), .O(z9));
endmodule


