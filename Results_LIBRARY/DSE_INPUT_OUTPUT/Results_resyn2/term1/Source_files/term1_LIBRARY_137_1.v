// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x33), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x31), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  and2   g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(new_n52_), .O(new_n53_));
  aoi21  g009(.a(new_n53_), .b(new_n45_), .c(new_n46_), .O(new_n54_));
  oai21  g010(.a(new_n53_), .b(x32), .c(new_n54_), .O(z1));
  inv1   g011(.a(new_n46_), .O(new_n56_));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  nand2  g013(.a(x05), .b(new_n50_), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n49_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  or2    g016(.a(new_n59_), .b(new_n58_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  aoi21  g018(.a(new_n52_), .b(x01), .c(new_n62_), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n63_), .b(new_n57_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n64_), .c(new_n56_), .O(z2));
  nor2   g024(.a(x23), .b(x18), .O(new_n69_));
  nor2   g025(.a(x24), .b(x19), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g027(.a(x22), .b(x17), .O(new_n72_));
  oai22  g028(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n48_), .O(new_n76_));
  inv1   g032(.a(x27), .O(new_n77_));
  nand2  g033(.a(x25), .b(x01), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(x26), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n76_), .c(new_n56_), .O(z3));
  inv1   g037(.a(x28), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n77_), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nand2  g040(.a(new_n82_), .b(new_n77_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n84_), .c(new_n79_), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n76_), .c(new_n56_), .O(z4));
  inv1   g043(.a(new_n76_), .O(new_n88_));
  nand3  g044(.a(new_n79_), .b(new_n88_), .c(new_n56_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n83_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n84_), .b(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n91_), .c(new_n90_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z5));
  inv1   g051(.a(x30), .O(new_n96_));
  inv1   g052(.a(new_n91_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n88_), .c(new_n96_), .O(new_n98_));
  inv1   g054(.a(new_n71_), .O(new_n99_));
  nor2   g055(.a(new_n97_), .b(new_n74_), .O(new_n100_));
  inv1   g056(.a(x31), .O(new_n101_));
  inv1   g057(.a(x15), .O(new_n102_));
  inv1   g058(.a(x20), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand3  g060(.a(new_n72_), .b(x16), .c(x15), .O(new_n105_));
  aoi21  g061(.a(new_n105_), .b(new_n104_), .c(new_n101_), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n83_), .c(x29), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n99_), .c(new_n100_), .O(new_n108_));
  nand2  g064(.a(new_n48_), .b(x30), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n108_), .c(new_n98_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n79_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n56_), .O(z6));
  nand2  g068(.a(new_n97_), .b(x30), .O(new_n113_));
  xor2a  g069(.a(new_n113_), .b(x31), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n90_), .O(z7));
  inv1   g071(.a(x32), .O(new_n116_));
  inv1   g072(.a(x00), .O(new_n117_));
  nor2   g073(.a(x26), .b(new_n117_), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n116_), .c(new_n45_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n101_), .O(new_n121_));
  inv1   g077(.a(x17), .O(new_n122_));
  inv1   g078(.a(x19), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x18), .c(x14), .O(new_n124_));
  inv1   g080(.a(x18), .O(new_n125_));
  nand3  g081(.a(x19), .b(new_n125_), .c(x13), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n124_), .c(new_n122_), .O(new_n127_));
  nand4  g083(.a(x19), .b(x18), .c(new_n122_), .d(x12), .O(new_n128_));
  inv1   g084(.a(new_n128_), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n127_), .c(x16), .O(new_n130_));
  inv1   g086(.a(x16), .O(new_n131_));
  nand3  g087(.a(x18), .b(x17), .c(x11), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x19), .c(new_n131_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n130_), .O(new_n135_));
  nand4  g091(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n136_));
  nor3   g092(.a(new_n136_), .b(new_n123_), .c(x15), .O(new_n137_));
  aoi21  g093(.a(new_n135_), .b(x15), .c(new_n137_), .O(new_n138_));
  nand2  g094(.a(new_n85_), .b(x29), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n96_), .O(new_n140_));
  nand3  g096(.a(new_n85_), .b(x30), .c(x29), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n140_), .c(x31), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n138_), .c(new_n116_), .O(new_n143_));
  nand2  g099(.a(x18), .b(x12), .O(new_n144_));
  oai21  g100(.a(x18), .b(x13), .c(x17), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x16), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n132_), .O(new_n148_));
  nand4  g104(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n149_));
  aoi21  g105(.a(new_n123_), .b(x14), .c(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n136_), .b(new_n102_), .O(new_n151_));
  nand2  g107(.a(new_n149_), .b(new_n123_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  nor2   g109(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n148_), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  nand2  g112(.a(new_n141_), .b(new_n140_), .O(new_n157_));
  nor2   g113(.a(new_n157_), .b(new_n116_), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n156_), .c(new_n119_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n143_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n121_), .O(z8));
  oai21  g117(.a(new_n155_), .b(new_n141_), .c(x33), .O(new_n162_));
  oai21  g118(.a(x33), .b(new_n101_), .c(new_n140_), .O(new_n163_));
  nand2  g119(.a(new_n141_), .b(x31), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n138_), .c(new_n162_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n118_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n56_), .O(z9));
endmodule


