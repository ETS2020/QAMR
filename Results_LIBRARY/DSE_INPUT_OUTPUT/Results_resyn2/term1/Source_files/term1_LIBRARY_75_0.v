// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  nor2   g000(.a(x30), .b(x28), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(x03), .O(new_n47_));
  inv1   g003(.a(x33), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  aoi21  g005(.a(x32), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n48_), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  nand2  g007(.a(x32), .b(x03), .O(new_n52_));
  nand2  g008(.a(x33), .b(new_n47_), .O(new_n53_));
  nand3  g009(.a(new_n53_), .b(new_n52_), .c(new_n49_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nand2  g014(.a(new_n47_), .b(new_n49_), .O(new_n59_));
  nand2  g015(.a(x03), .b(x02), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g017(.a(x05), .b(new_n47_), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n49_), .O(new_n63_));
  xor2a  g019(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n58_), .c(new_n61_), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n57_), .O(new_n66_));
  nor4   g022(.a(new_n66_), .b(new_n45_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g023(.a(x27), .O(new_n68_));
  inv1   g024(.a(new_n45_), .O(new_n69_));
  nor2   g025(.a(x21), .b(x16), .O(new_n70_));
  nor2   g026(.a(x24), .b(x19), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor2   g028(.a(x22), .b(x17), .O(new_n73_));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  nor2   g030(.a(x23), .b(x18), .O(new_n75_));
  nor3   g031(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(new_n76_));
  inv1   g032(.a(x26), .O(new_n77_));
  nand4  g033(.a(new_n60_), .b(new_n77_), .c(x25), .d(x01), .O(new_n78_));
  aoi21  g034(.a(new_n76_), .b(new_n72_), .c(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n69_), .c(new_n68_), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z3));
  nand2  g037(.a(x28), .b(x27), .O(new_n82_));
  inv1   g038(.a(x28), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n68_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n82_), .c(new_n79_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n69_), .O(z4));
  inv1   g042(.a(x29), .O(new_n87_));
  xor2a  g043(.a(new_n82_), .b(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n79_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n69_), .O(z5));
  nand2  g046(.a(new_n82_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x30), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(new_n87_), .O(new_n93_));
  nor2   g049(.a(x30), .b(x29), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  xnor2a g051(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n79_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(z6));
  inv1   g054(.a(x31), .O(new_n99_));
  aoi21  g055(.a(new_n99_), .b(x30), .c(x28), .O(new_n100_));
  nor2   g056(.a(new_n99_), .b(new_n83_), .O(new_n101_));
  nand3  g057(.a(x30), .b(x29), .c(x27), .O(new_n102_));
  nand2  g058(.a(new_n82_), .b(new_n99_), .O(new_n103_));
  nor2   g059(.a(x31), .b(x29), .O(new_n104_));
  nor3   g060(.a(new_n104_), .b(new_n101_), .c(new_n92_), .O(new_n105_));
  aoi22  g061(.a(new_n105_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(new_n106_));
  oai21  g062(.a(new_n100_), .b(new_n79_), .c(new_n106_), .O(z7));
  inv1   g063(.a(x15), .O(new_n108_));
  inv1   g064(.a(x16), .O(new_n109_));
  nand3  g065(.a(x18), .b(x17), .c(x11), .O(new_n110_));
  nand4  g066(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n111_));
  aoi22  g067(.a(new_n111_), .b(new_n108_), .c(new_n110_), .d(new_n109_), .O(new_n112_));
  nand4  g068(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  inv1   g070(.a(x19), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x14), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand2  g073(.a(x18), .b(x12), .O(new_n118_));
  oai21  g074(.a(x18), .b(x13), .c(x17), .O(new_n119_));
  aoi22  g075(.a(new_n119_), .b(new_n118_), .c(new_n113_), .d(new_n115_), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(new_n117_), .c(new_n112_), .O(new_n121_));
  nand2  g077(.a(new_n93_), .b(new_n84_), .O(new_n122_));
  nor2   g078(.a(new_n94_), .b(new_n99_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n121_), .c(x32), .O(new_n125_));
  inv1   g081(.a(x17), .O(new_n126_));
  nand3  g082(.a(new_n115_), .b(x18), .c(x14), .O(new_n127_));
  inv1   g083(.a(x18), .O(new_n128_));
  nand3  g084(.a(x19), .b(new_n128_), .c(x13), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(new_n127_), .c(new_n126_), .O(new_n130_));
  nand4  g086(.a(x19), .b(x18), .c(new_n126_), .d(x12), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n130_), .c(x16), .O(new_n133_));
  inv1   g089(.a(new_n110_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x19), .c(new_n109_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nor3   g092(.a(new_n111_), .b(new_n115_), .c(x15), .O(new_n137_));
  aoi21  g093(.a(new_n136_), .b(x15), .c(new_n137_), .O(new_n138_));
  inv1   g094(.a(new_n84_), .O(new_n139_));
  nor2   g095(.a(x32), .b(new_n99_), .O(new_n140_));
  oai21  g096(.a(new_n95_), .b(new_n139_), .c(new_n140_), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n138_), .c(new_n125_), .O(new_n142_));
  nand2  g098(.a(new_n77_), .b(x00), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n69_), .O(z8));
  nand2  g102(.a(new_n121_), .b(new_n69_), .O(new_n147_));
  oai21  g103(.a(new_n104_), .b(new_n83_), .c(new_n92_), .O(new_n148_));
  oai21  g104(.a(new_n122_), .b(new_n99_), .c(new_n148_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x33), .O(new_n151_));
  aoi21  g107(.a(new_n135_), .b(new_n133_), .c(new_n108_), .O(new_n152_));
  oai21  g108(.a(new_n104_), .b(new_n93_), .c(x28), .O(new_n153_));
  oai21  g109(.a(x31), .b(new_n92_), .c(new_n48_), .O(new_n154_));
  aoi21  g110(.a(new_n153_), .b(new_n102_), .c(new_n154_), .O(new_n155_));
  oai21  g111(.a(new_n137_), .b(new_n152_), .c(new_n155_), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n151_), .c(new_n143_), .O(z9));
endmodule


