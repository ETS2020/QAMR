// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_;
  inv1   g000(.a(x32), .O(new_n45_));
  nand2  g001(.a(x30), .b(x29), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(z0));
  inv1   g005(.a(x03), .O(new_n50_));
  inv1   g006(.a(x33), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  aoi21  g008(.a(x32), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(new_n50_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(x33), .b(new_n50_), .c(x02), .O(new_n55_));
  oai21  g011(.a(new_n45_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n54_), .c(new_n47_), .O(z1));
  xor2a  g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nand2  g015(.a(new_n50_), .b(new_n52_), .O(new_n60_));
  nand2  g016(.a(x03), .b(x02), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n50_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n52_), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n59_), .c(new_n62_), .O(new_n66_));
  xor2a  g022(.a(new_n66_), .b(new_n58_), .O(new_n67_));
  inv1   g023(.a(x09), .O(new_n68_));
  nand3  g024(.a(new_n46_), .b(new_n68_), .c(x08), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n67_), .O(z2));
  nor2   g026(.a(x20), .b(x15), .O(new_n71_));
  nor2   g027(.a(x24), .b(x19), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g029(.a(x18), .O(new_n74_));
  inv1   g030(.a(x23), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x22), .b(x17), .O(new_n77_));
  nor2   g033(.a(x21), .b(x16), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(new_n80_));
  inv1   g036(.a(x26), .O(new_n81_));
  nand4  g037(.a(new_n61_), .b(new_n81_), .c(x25), .d(x01), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(x27), .c(new_n46_), .O(z3));
  inv1   g041(.a(x27), .O(new_n86_));
  inv1   g042(.a(x28), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g044(.a(new_n87_), .b(new_n86_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n84_), .c(new_n46_), .O(z4));
  inv1   g048(.a(new_n84_), .O(new_n93_));
  nand2  g049(.a(new_n89_), .b(x29), .O(new_n94_));
  inv1   g050(.a(x30), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(x29), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n90_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n94_), .c(new_n93_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z5));
  aoi22  g055(.a(new_n94_), .b(new_n95_), .c(new_n84_), .d(new_n46_), .O(z6));
  inv1   g056(.a(x31), .O(new_n101_));
  nand3  g057(.a(new_n93_), .b(new_n46_), .c(new_n101_), .O(z7));
  nand2  g058(.a(new_n81_), .b(x00), .O(new_n103_));
  inv1   g059(.a(x15), .O(new_n104_));
  inv1   g060(.a(x17), .O(new_n105_));
  inv1   g061(.a(x19), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(x18), .c(x14), .O(new_n107_));
  nand3  g063(.a(x19), .b(new_n74_), .c(x13), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n107_), .c(new_n105_), .O(new_n109_));
  nand4  g065(.a(x19), .b(x18), .c(new_n105_), .d(x12), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n109_), .c(x16), .O(new_n112_));
  inv1   g068(.a(x16), .O(new_n113_));
  nand3  g069(.a(x18), .b(x17), .c(x11), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x19), .c(new_n113_), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n112_), .c(new_n104_), .O(new_n117_));
  nand4  g073(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n118_));
  nor3   g074(.a(new_n118_), .b(new_n106_), .c(x15), .O(new_n119_));
  nand2  g075(.a(new_n88_), .b(x29), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n95_), .c(new_n101_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n46_), .c(new_n45_), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  oai21  g079(.a(new_n119_), .b(new_n117_), .c(new_n123_), .O(new_n124_));
  oai21  g080(.a(new_n115_), .b(x16), .c(x15), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n118_), .O(new_n126_));
  nand4  g082(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n127_));
  aoi21  g083(.a(new_n106_), .b(x14), .c(new_n127_), .O(new_n128_));
  aoi21  g084(.a(new_n127_), .b(new_n106_), .c(new_n128_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand2  g086(.a(x18), .b(x12), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n105_), .O(new_n132_));
  inv1   g088(.a(x13), .O(new_n133_));
  nand2  g089(.a(new_n74_), .b(new_n133_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n132_), .c(new_n121_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n130_), .c(new_n48_), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n124_), .c(new_n103_), .O(z8));
  nand2  g093(.a(new_n51_), .b(new_n101_), .O(new_n138_));
  aoi21  g094(.a(new_n88_), .b(x29), .c(new_n138_), .O(new_n139_));
  oai21  g095(.a(new_n119_), .b(new_n117_), .c(new_n139_), .O(new_n140_));
  nand4  g096(.a(new_n134_), .b(new_n132_), .c(new_n120_), .d(new_n101_), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n129_), .c(new_n126_), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(x33), .c(x30), .O(new_n144_));
  oai21  g100(.a(new_n51_), .b(x29), .c(x30), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n81_), .c(x00), .O(new_n146_));
  aoi21  g102(.a(new_n144_), .b(new_n140_), .c(new_n146_), .O(z9));
endmodule


