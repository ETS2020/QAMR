// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x21), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  and2   g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(new_n52_), .O(new_n53_));
  inv1   g009(.a(x33), .O(new_n54_));
  aoi21  g010(.a(new_n53_), .b(new_n54_), .c(new_n46_), .O(new_n55_));
  oai21  g011(.a(new_n53_), .b(x32), .c(new_n55_), .O(z1));
  inv1   g012(.a(new_n46_), .O(new_n57_));
  xor2a  g013(.a(x07), .b(x04), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n50_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n49_), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand4  g017(.a(x06), .b(x05), .c(new_n50_), .d(new_n49_), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(x01), .O(new_n63_));
  aoi21  g019(.a(new_n52_), .b(x01), .c(new_n63_), .O(new_n64_));
  nor2   g020(.a(new_n64_), .b(new_n58_), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand2  g022(.a(new_n64_), .b(new_n58_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(new_n66_), .c(x08), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n65_), .c(new_n57_), .O(z2));
  inv1   g025(.a(x27), .O(new_n70_));
  oai22  g026(.a(x24), .b(x19), .c(x23), .d(x18), .O(new_n71_));
  oai22  g027(.a(x22), .b(x17), .c(x20), .d(x15), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g029(.a(x21), .b(x16), .c(new_n73_), .O(new_n74_));
  inv1   g030(.a(x26), .O(new_n75_));
  nand4  g031(.a(new_n48_), .b(new_n75_), .c(x25), .d(x01), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(new_n77_));
  and2   g033(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n57_), .c(new_n70_), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(z3));
  inv1   g036(.a(x28), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n70_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n81_), .b(new_n70_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n83_), .c(new_n78_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n57_), .O(z4));
  nand2  g042(.a(new_n82_), .b(x29), .O(new_n87_));
  inv1   g043(.a(x29), .O(new_n88_));
  nand2  g044(.a(new_n83_), .b(new_n88_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n87_), .c(new_n78_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n57_), .O(z5));
  inv1   g047(.a(x16), .O(new_n92_));
  inv1   g048(.a(x21), .O(new_n93_));
  inv1   g049(.a(new_n87_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  nand2  g051(.a(new_n94_), .b(x30), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(new_n97_));
  oai22  g053(.a(new_n94_), .b(x30), .c(new_n72_), .d(new_n71_), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n77_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n57_), .O(z6));
  oai21  g057(.a(new_n96_), .b(new_n73_), .c(x31), .O(new_n102_));
  inv1   g058(.a(x31), .O(new_n103_));
  nand2  g059(.a(new_n97_), .b(new_n103_), .O(new_n104_));
  nor2   g060(.a(new_n76_), .b(new_n46_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n104_), .c(new_n102_), .d(new_n74_), .O(z7));
  nand2  g062(.a(new_n75_), .b(x00), .O(new_n107_));
  inv1   g063(.a(x15), .O(new_n108_));
  inv1   g064(.a(x17), .O(new_n109_));
  inv1   g065(.a(x19), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x18), .c(x14), .O(new_n111_));
  inv1   g067(.a(x18), .O(new_n112_));
  nand3  g068(.a(x19), .b(new_n112_), .c(x13), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n111_), .c(new_n109_), .O(new_n114_));
  nand4  g070(.a(x19), .b(x18), .c(new_n109_), .d(x12), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n114_), .c(x16), .O(new_n117_));
  nand3  g073(.a(x19), .b(x18), .c(x17), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n92_), .c(x11), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n117_), .c(new_n108_), .O(new_n121_));
  nand3  g077(.a(x16), .b(new_n108_), .c(x10), .O(new_n122_));
  nor2   g078(.a(new_n122_), .b(new_n118_), .O(new_n123_));
  nand2  g079(.a(new_n84_), .b(x29), .O(new_n124_));
  xor2a  g080(.a(new_n124_), .b(new_n45_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(z0), .c(x31), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  oai21  g083(.a(new_n123_), .b(new_n121_), .c(new_n127_), .O(new_n128_));
  nand2  g084(.a(new_n125_), .b(x31), .O(new_n129_));
  nand4  g085(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  nand3  g087(.a(x18), .b(x17), .c(x11), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n92_), .O(new_n133_));
  aoi21  g089(.a(new_n133_), .b(x19), .c(new_n131_), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  nand2  g091(.a(x17), .b(x13), .O(new_n136_));
  oai21  g092(.a(x17), .b(x12), .c(x18), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g094(.a(new_n110_), .b(x14), .O(new_n139_));
  nand2  g095(.a(new_n131_), .b(new_n139_), .O(new_n140_));
  nand4  g096(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n108_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n140_), .c(new_n138_), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n135_), .O(new_n145_));
  inv1   g101(.a(x32), .O(new_n146_));
  nor2   g102(.a(new_n46_), .b(new_n146_), .O(new_n147_));
  oai21  g103(.a(new_n145_), .b(new_n129_), .c(new_n147_), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n128_), .c(new_n107_), .O(z8));
  oai21  g105(.a(new_n143_), .b(new_n134_), .c(new_n57_), .O(new_n150_));
  nand2  g106(.a(new_n124_), .b(new_n103_), .O(new_n151_));
  nor2   g107(.a(new_n45_), .b(new_n93_), .O(new_n152_));
  nand3  g108(.a(new_n84_), .b(x31), .c(x29), .O(new_n153_));
  aoi22  g109(.a(new_n153_), .b(new_n152_), .c(new_n151_), .d(new_n45_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n150_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x33), .O(new_n156_));
  nand2  g112(.a(new_n103_), .b(new_n45_), .O(new_n157_));
  oai21  g113(.a(new_n152_), .b(new_n124_), .c(new_n54_), .O(new_n158_));
  aoi21  g114(.a(new_n157_), .b(new_n153_), .c(new_n158_), .O(new_n159_));
  oai21  g115(.a(new_n123_), .b(new_n121_), .c(new_n159_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n156_), .c(new_n107_), .O(z9));
endmodule


