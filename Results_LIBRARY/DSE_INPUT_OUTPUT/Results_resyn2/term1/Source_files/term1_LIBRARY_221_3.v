// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x18), .O(new_n45_));
  inv1   g001(.a(x23), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  nand2  g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  and2   g008(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  inv1   g009(.a(new_n53_), .O(new_n54_));
  inv1   g010(.a(x33), .O(new_n55_));
  aoi21  g011(.a(new_n54_), .b(new_n55_), .c(new_n47_), .O(new_n56_));
  oai21  g012(.a(new_n54_), .b(x32), .c(new_n56_), .O(z1));
  inv1   g013(.a(new_n47_), .O(new_n58_));
  xor2a  g014(.a(x07), .b(x04), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n51_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n50_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  or2    g018(.a(new_n61_), .b(new_n60_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(x01), .O(new_n64_));
  aoi21  g020(.a(new_n53_), .b(x01), .c(new_n64_), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand2  g023(.a(new_n65_), .b(new_n59_), .O(new_n68_));
  nand3  g024(.a(new_n68_), .b(new_n67_), .c(x08), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n66_), .c(new_n58_), .O(z2));
  inv1   g026(.a(x27), .O(new_n71_));
  inv1   g027(.a(x17), .O(new_n72_));
  inv1   g028(.a(x22), .O(new_n73_));
  aoi22  g029(.a(new_n46_), .b(new_n45_), .c(new_n73_), .d(new_n72_), .O(new_n74_));
  nor2   g030(.a(x24), .b(x19), .O(new_n75_));
  nor2   g031(.a(x20), .b(x15), .O(new_n76_));
  nor2   g032(.a(x21), .b(x16), .O(new_n77_));
  nor3   g033(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  inv1   g034(.a(x26), .O(new_n79_));
  nand4  g035(.a(new_n49_), .b(new_n79_), .c(x25), .d(x01), .O(new_n80_));
  aoi21  g036(.a(new_n78_), .b(new_n74_), .c(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n58_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n71_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(z3));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n71_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n71_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n88_), .c(new_n81_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n58_), .O(z4));
  nand2  g047(.a(new_n87_), .b(x29), .O(new_n92_));
  inv1   g048(.a(x29), .O(new_n93_));
  nand2  g049(.a(new_n88_), .b(new_n93_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n92_), .c(new_n83_), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z5));
  nand3  g052(.a(new_n87_), .b(x30), .c(x29), .O(new_n97_));
  inv1   g053(.a(x30), .O(new_n98_));
  nand2  g054(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n97_), .c(new_n83_), .O(new_n100_));
  inv1   g056(.a(new_n100_), .O(z6));
  xor2a  g057(.a(new_n97_), .b(x31), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(new_n81_), .c(new_n47_), .O(z7));
  inv1   g059(.a(x19), .O(new_n104_));
  nand3  g060(.a(x17), .b(x16), .c(x15), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  inv1   g062(.a(x16), .O(new_n107_));
  nand2  g063(.a(x17), .b(x11), .O(new_n108_));
  nor2   g064(.a(x17), .b(x12), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n107_), .c(new_n108_), .O(new_n110_));
  nand2  g066(.a(new_n89_), .b(x29), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n98_), .O(new_n112_));
  inv1   g068(.a(x15), .O(new_n113_));
  nand3  g069(.a(x17), .b(x16), .c(x10), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n112_), .c(new_n110_), .d(new_n106_), .O(new_n116_));
  nand2  g072(.a(new_n104_), .b(x14), .O(new_n117_));
  inv1   g073(.a(new_n105_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x18), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n117_), .O(new_n121_));
  nand3  g077(.a(new_n89_), .b(x30), .c(x29), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n121_), .c(x31), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n116_), .c(new_n46_), .O(new_n124_));
  nand2  g080(.a(new_n122_), .b(new_n112_), .O(new_n125_));
  nand4  g081(.a(new_n118_), .b(x31), .c(x19), .d(x13), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(new_n125_), .c(new_n45_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x32), .O(new_n129_));
  nand3  g085(.a(x19), .b(new_n45_), .c(x13), .O(new_n130_));
  nand3  g086(.a(new_n104_), .b(x18), .c(x14), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n130_), .c(new_n72_), .O(new_n132_));
  nand2  g088(.a(new_n130_), .b(x23), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand4  g090(.a(x19), .b(x18), .c(new_n72_), .d(x12), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n46_), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n134_), .c(new_n107_), .O(new_n138_));
  nand2  g094(.a(x19), .b(x18), .O(new_n139_));
  nor3   g095(.a(new_n139_), .b(new_n108_), .c(x16), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n46_), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n138_), .c(x15), .O(new_n143_));
  nor3   g099(.a(new_n139_), .b(new_n114_), .c(x15), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n46_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  inv1   g102(.a(new_n112_), .O(new_n147_));
  nand2  g103(.a(new_n122_), .b(x31), .O(new_n148_));
  nor3   g104(.a(new_n148_), .b(new_n147_), .c(x32), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n146_), .O(new_n150_));
  nand2  g106(.a(new_n79_), .b(x00), .O(new_n151_));
  aoi21  g107(.a(new_n150_), .b(new_n129_), .c(new_n151_), .O(z8));
  inv1   g108(.a(new_n140_), .O(new_n153_));
  oai21  g109(.a(new_n136_), .b(new_n132_), .c(x16), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n153_), .c(new_n113_), .O(new_n155_));
  xor2a  g111(.a(new_n111_), .b(x31), .O(new_n156_));
  and2   g112(.a(new_n156_), .b(new_n125_), .O(new_n157_));
  oai21  g113(.a(new_n155_), .b(new_n144_), .c(new_n157_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n55_), .O(new_n159_));
  nand2  g115(.a(new_n121_), .b(x33), .O(new_n160_));
  oai21  g116(.a(new_n114_), .b(new_n45_), .c(new_n113_), .O(new_n161_));
  oai21  g117(.a(new_n120_), .b(x19), .c(new_n161_), .O(new_n162_));
  nor2   g118(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand2  g119(.a(new_n156_), .b(new_n125_), .O(new_n164_));
  inv1   g120(.a(new_n108_), .O(new_n165_));
  nand2  g121(.a(x17), .b(x13), .O(new_n166_));
  oai21  g122(.a(new_n109_), .b(new_n45_), .c(new_n166_), .O(new_n167_));
  aoi22  g123(.a(new_n167_), .b(x16), .c(new_n165_), .d(x18), .O(new_n168_));
  nor2   g124(.a(new_n168_), .b(new_n164_), .O(new_n169_));
  aoi21  g125(.a(new_n169_), .b(new_n163_), .c(new_n151_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n159_), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n58_), .O(z9));
endmodule


