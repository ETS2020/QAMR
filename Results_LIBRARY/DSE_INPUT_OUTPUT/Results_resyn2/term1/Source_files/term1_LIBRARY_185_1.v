// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x18), .O(new_n45_));
  nand2  g001(.a(x20), .b(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x33), .O(new_n48_));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g006(.a(x32), .O(new_n51_));
  inv1   g007(.a(new_n49_), .O(new_n52_));
  nand2  g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g009(.a(new_n53_), .b(new_n50_), .c(new_n46_), .O(z1));
  inv1   g010(.a(x01), .O(new_n55_));
  aoi21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  oai21  g012(.a(x03), .b(x02), .c(new_n56_), .O(new_n57_));
  inv1   g013(.a(x03), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(new_n64_));
  xnor2a g020(.a(x07), .b(x04), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(x08), .O(new_n67_));
  aoi21  g023(.a(new_n65_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  oai21  g024(.a(new_n65_), .b(new_n64_), .c(new_n68_), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n46_), .O(z2));
  inv1   g026(.a(x27), .O(new_n71_));
  inv1   g027(.a(x20), .O(new_n72_));
  oai21  g028(.a(x23), .b(x18), .c(x15), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g030(.a(x21), .b(x16), .O(new_n75_));
  oai22  g031(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi22  g033(.a(new_n77_), .b(new_n74_), .c(x20), .d(new_n45_), .O(new_n78_));
  inv1   g034(.a(x26), .O(new_n79_));
  nand3  g035(.a(new_n56_), .b(new_n79_), .c(x25), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n78_), .c(new_n71_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(z3));
  inv1   g039(.a(x15), .O(new_n84_));
  inv1   g040(.a(x23), .O(new_n85_));
  aoi22  g041(.a(new_n85_), .b(new_n45_), .c(new_n72_), .d(new_n84_), .O(new_n86_));
  aoi21  g042(.a(new_n86_), .b(new_n77_), .c(new_n80_), .O(new_n87_));
  inv1   g043(.a(x28), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n71_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n88_), .b(new_n71_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n90_), .c(new_n87_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n46_), .O(z4));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n90_), .b(new_n94_), .O(new_n95_));
  nand2  g051(.a(new_n89_), .b(x29), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n95_), .c(new_n87_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n46_), .O(z5));
  inv1   g054(.a(x30), .O(new_n99_));
  nand2  g055(.a(new_n96_), .b(new_n99_), .O(new_n100_));
  inv1   g056(.a(new_n96_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(x30), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n100_), .c(new_n81_), .d(new_n78_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(z6));
  nand2  g060(.a(new_n96_), .b(x31), .O(new_n105_));
  xor2a  g061(.a(x31), .b(x30), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n101_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n105_), .c(new_n81_), .d(new_n78_), .O(z7));
  nand3  g064(.a(x17), .b(x16), .c(x15), .O(new_n109_));
  nand2  g065(.a(x19), .b(x13), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n109_), .c(new_n45_), .O(new_n111_));
  inv1   g067(.a(x31), .O(new_n112_));
  oai21  g068(.a(x28), .b(x27), .c(x29), .O(new_n113_));
  xor2a  g069(.a(new_n113_), .b(x30), .O(new_n114_));
  nor2   g070(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n111_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n72_), .O(new_n117_));
  inv1   g073(.a(new_n109_), .O(new_n118_));
  inv1   g074(.a(x17), .O(new_n119_));
  nand2  g075(.a(x16), .b(x10), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n119_), .c(new_n84_), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(x19), .c(new_n118_), .O(new_n122_));
  inv1   g078(.a(x19), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(x14), .c(new_n109_), .O(new_n124_));
  nand2  g080(.a(x16), .b(x12), .O(new_n125_));
  oai21  g081(.a(x16), .b(x11), .c(x17), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n125_), .c(new_n124_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n115_), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n122_), .c(x18), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n117_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x32), .O(new_n131_));
  nand2  g087(.a(x18), .b(x17), .O(new_n132_));
  nor4   g088(.a(new_n132_), .b(new_n120_), .c(new_n123_), .d(x15), .O(new_n133_));
  nand3  g089(.a(x18), .b(x17), .c(x11), .O(new_n134_));
  nor3   g090(.a(new_n134_), .b(new_n123_), .c(x16), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  nand4  g092(.a(x19), .b(x18), .c(new_n119_), .d(x12), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  nand3  g094(.a(new_n123_), .b(x18), .c(x14), .O(new_n139_));
  nand3  g095(.a(x19), .b(new_n45_), .c(x13), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n139_), .c(new_n119_), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n138_), .c(x16), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n136_), .c(new_n84_), .O(new_n143_));
  nand3  g099(.a(new_n139_), .b(new_n137_), .c(x20), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  nor2   g101(.a(new_n135_), .b(new_n133_), .O(new_n146_));
  nand2  g102(.a(new_n115_), .b(new_n51_), .O(new_n147_));
  aoi21  g103(.a(new_n146_), .b(new_n145_), .c(new_n147_), .O(new_n148_));
  oai21  g104(.a(new_n143_), .b(new_n133_), .c(new_n148_), .O(new_n149_));
  nand2  g105(.a(new_n79_), .b(x00), .O(new_n150_));
  aoi21  g106(.a(new_n149_), .b(new_n131_), .c(new_n150_), .O(z8));
  nand2  g107(.a(new_n142_), .b(new_n136_), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(x15), .c(new_n133_), .O(new_n153_));
  inv1   g109(.a(new_n106_), .O(new_n154_));
  nand2  g110(.a(new_n114_), .b(new_n154_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n153_), .c(new_n48_), .O(new_n156_));
  nand2  g112(.a(x16), .b(x15), .O(new_n157_));
  oai21  g113(.a(new_n132_), .b(new_n157_), .c(new_n123_), .O(new_n158_));
  inv1   g114(.a(x16), .O(new_n159_));
  nand2  g115(.a(new_n134_), .b(new_n159_), .O(new_n160_));
  oai21  g116(.a(new_n132_), .b(new_n120_), .c(new_n84_), .O(new_n161_));
  nand3  g117(.a(new_n161_), .b(new_n160_), .c(new_n158_), .O(new_n162_));
  nand2  g118(.a(new_n124_), .b(x18), .O(new_n163_));
  nand2  g119(.a(x18), .b(x12), .O(new_n164_));
  oai21  g120(.a(x18), .b(x13), .c(x17), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  nor2   g122(.a(new_n106_), .b(new_n48_), .O(new_n167_));
  nand4  g123(.a(new_n167_), .b(new_n166_), .c(new_n163_), .d(new_n114_), .O(new_n168_));
  nor2   g124(.a(new_n168_), .b(new_n162_), .O(new_n169_));
  nor2   g125(.a(new_n169_), .b(new_n150_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n156_), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n46_), .O(z9));
endmodule


