// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x16), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  aoi21  g006(.a(x32), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  nand2  g008(.a(x32), .b(x03), .O(new_n53_));
  nand2  g009(.a(x33), .b(new_n48_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n52_), .c(new_n46_), .O(z1));
  inv1   g012(.a(new_n46_), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  aoi21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  oai21  g015(.a(x03), .b(x02), .c(new_n59_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n50_), .O(new_n61_));
  aoi21  g017(.a(x05), .b(new_n48_), .c(new_n61_), .O(new_n62_));
  nand3  g018(.a(new_n61_), .b(x05), .c(new_n48_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  xnor2a g021(.a(x07), .b(x04), .O(new_n66_));
  inv1   g022(.a(x09), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(x08), .O(new_n68_));
  aoi21  g024(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  oai21  g025(.a(new_n66_), .b(new_n65_), .c(new_n69_), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n57_), .O(z2));
  inv1   g027(.a(x27), .O(new_n72_));
  nor2   g028(.a(x23), .b(x18), .O(new_n73_));
  nor2   g029(.a(x24), .b(x19), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x20), .b(x15), .O(new_n76_));
  nor2   g032(.a(x22), .b(x17), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  inv1   g036(.a(x16), .O(new_n81_));
  inv1   g037(.a(x21), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g039(.a(new_n83_), .b(new_n80_), .c(new_n46_), .O(new_n84_));
  inv1   g040(.a(x26), .O(new_n85_));
  nand3  g041(.a(new_n59_), .b(new_n85_), .c(x25), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n84_), .c(new_n72_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z3));
  nand2  g045(.a(new_n83_), .b(new_n80_), .O(new_n90_));
  inv1   g046(.a(x28), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n72_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nor2   g049(.a(x28), .b(x27), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n93_), .c(new_n87_), .d(new_n90_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n57_), .O(z4));
  nand2  g053(.a(new_n92_), .b(x29), .O(new_n98_));
  inv1   g054(.a(x29), .O(new_n99_));
  nand2  g055(.a(new_n93_), .b(new_n99_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n98_), .c(new_n87_), .d(new_n84_), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(z5));
  nand2  g058(.a(new_n98_), .b(new_n45_), .O(new_n103_));
  nand2  g059(.a(x30), .b(x29), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  oai21  g061(.a(new_n92_), .b(new_n81_), .c(new_n105_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n103_), .c(new_n79_), .O(new_n107_));
  oai21  g063(.a(new_n98_), .b(new_n83_), .c(new_n107_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n87_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n57_), .O(z6));
  oai21  g066(.a(new_n83_), .b(x31), .c(new_n80_), .O(new_n111_));
  nand2  g067(.a(new_n105_), .b(new_n92_), .O(new_n112_));
  xor2a  g068(.a(new_n112_), .b(x31), .O(new_n113_));
  oai21  g069(.a(x31), .b(x30), .c(new_n81_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n113_), .c(new_n111_), .d(new_n87_), .O(z7));
  inv1   g071(.a(x17), .O(new_n116_));
  inv1   g072(.a(x19), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x18), .c(x14), .O(new_n118_));
  inv1   g074(.a(x18), .O(new_n119_));
  nand3  g075(.a(x19), .b(new_n119_), .c(x13), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n118_), .c(new_n116_), .O(new_n121_));
  nand4  g077(.a(x19), .b(x18), .c(new_n116_), .d(x12), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n121_), .c(x16), .O(new_n124_));
  nand3  g080(.a(x19), .b(x18), .c(x17), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n81_), .c(x11), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nor3   g084(.a(new_n117_), .b(new_n119_), .c(x15), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(x17), .c(x16), .d(x10), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  aoi21  g087(.a(new_n128_), .b(x15), .c(new_n131_), .O(new_n132_));
  nand2  g088(.a(new_n95_), .b(x29), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n45_), .O(new_n135_));
  nand3  g091(.a(new_n129_), .b(x17), .c(x10), .O(new_n136_));
  oai21  g092(.a(new_n123_), .b(new_n121_), .c(x15), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nor2   g094(.a(new_n45_), .b(new_n81_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n138_), .c(new_n133_), .O(new_n140_));
  oai21  g096(.a(new_n135_), .b(new_n132_), .c(new_n140_), .O(new_n141_));
  inv1   g097(.a(x31), .O(new_n142_));
  nor2   g098(.a(x32), .b(new_n142_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand3  g100(.a(x18), .b(x17), .c(x10), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  nand3  g102(.a(x18), .b(x17), .c(x15), .O(new_n147_));
  inv1   g103(.a(new_n147_), .O(new_n148_));
  oai21  g104(.a(x18), .b(x13), .c(x17), .O(new_n149_));
  nand2  g105(.a(x18), .b(x12), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n149_), .c(new_n117_), .O(new_n151_));
  oai22  g107(.a(new_n151_), .b(new_n148_), .c(new_n146_), .d(x15), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n57_), .O(new_n153_));
  nand2  g109(.a(x15), .b(x11), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n125_), .c(new_n81_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n134_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n45_), .O(new_n157_));
  aoi21  g113(.a(new_n117_), .b(x14), .c(new_n147_), .O(new_n158_));
  oai21  g114(.a(new_n104_), .b(new_n94_), .c(x31), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n158_), .c(new_n114_), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n157_), .c(new_n153_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(x32), .O(new_n162_));
  nand2  g118(.a(new_n85_), .b(x00), .O(new_n163_));
  aoi21  g119(.a(new_n162_), .b(new_n144_), .c(new_n163_), .O(z8));
  nand3  g120(.a(new_n133_), .b(new_n142_), .c(new_n45_), .O(new_n165_));
  or2    g121(.a(new_n165_), .b(new_n132_), .O(new_n166_));
  nor2   g122(.a(new_n133_), .b(new_n142_), .O(new_n167_));
  and2   g123(.a(new_n167_), .b(new_n139_), .O(new_n168_));
  aoi21  g124(.a(new_n168_), .b(new_n138_), .c(x33), .O(new_n169_));
  nand3  g125(.a(new_n155_), .b(new_n133_), .c(new_n142_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n45_), .O(new_n171_));
  inv1   g127(.a(new_n158_), .O(new_n172_));
  oai21  g128(.a(new_n167_), .b(new_n45_), .c(new_n172_), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(x16), .O(new_n174_));
  nand4  g130(.a(new_n174_), .b(new_n171_), .c(new_n153_), .d(x33), .O(new_n175_));
  nand3  g131(.a(new_n175_), .b(new_n85_), .c(x00), .O(new_n176_));
  aoi21  g132(.a(new_n169_), .b(new_n166_), .c(new_n176_), .O(z9));
endmodule


