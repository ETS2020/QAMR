// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_;
  nor2   g000(.a(x18), .b(x15), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(x03), .O(new_n47_));
  inv1   g003(.a(x33), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  aoi21  g005(.a(x32), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n48_), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(x33), .b(new_n47_), .c(x02), .O(new_n53_));
  oai21  g009(.a(new_n52_), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  inv1   g012(.a(x01), .O(new_n57_));
  aoi21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  oai21  g014(.a(x03), .b(x02), .c(new_n58_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n49_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n47_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n47_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n59_), .O(new_n64_));
  xor2a  g020(.a(x07), .b(x04), .O(new_n65_));
  xor2a  g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nor4   g022(.a(new_n66_), .b(new_n45_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g023(.a(new_n45_), .O(new_n68_));
  inv1   g024(.a(x27), .O(new_n69_));
  inv1   g025(.a(x18), .O(new_n70_));
  inv1   g026(.a(x23), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai22  g028(.a(x24), .b(x19), .c(x20), .d(x15), .O(new_n73_));
  oai22  g029(.a(x22), .b(x17), .c(x21), .d(x16), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  inv1   g032(.a(x26), .O(new_n77_));
  nand3  g033(.a(new_n58_), .b(new_n77_), .c(x25), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n76_), .c(new_n69_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n68_), .O(z3));
  aoi21  g037(.a(new_n75_), .b(new_n72_), .c(new_n45_), .O(new_n82_));
  inv1   g038(.a(x28), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n69_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n83_), .b(new_n69_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n85_), .c(new_n82_), .d(new_n79_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(z4));
  nand2  g044(.a(new_n84_), .b(x29), .O(new_n89_));
  inv1   g045(.a(x29), .O(new_n90_));
  nand2  g046(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n89_), .c(new_n82_), .d(new_n79_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z5));
  inv1   g049(.a(x30), .O(new_n94_));
  nand2  g050(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  inv1   g051(.a(new_n89_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(x30), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n95_), .c(new_n79_), .d(new_n76_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n68_), .O(z6));
  inv1   g055(.a(x31), .O(new_n100_));
  nand3  g056(.a(new_n96_), .b(new_n100_), .c(x30), .O(new_n101_));
  nor2   g057(.a(new_n78_), .b(new_n45_), .O(new_n102_));
  nand2  g058(.a(new_n97_), .b(x31), .O(new_n103_));
  inv1   g059(.a(x15), .O(new_n104_));
  oai21  g060(.a(new_n72_), .b(new_n104_), .c(new_n75_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n103_), .c(new_n102_), .d(new_n101_), .O(z7));
  nand2  g062(.a(new_n77_), .b(x00), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  inv1   g064(.a(x16), .O(new_n109_));
  inv1   g065(.a(x10), .O(new_n110_));
  inv1   g066(.a(x13), .O(new_n111_));
  oai22  g067(.a(x18), .b(new_n111_), .c(x15), .d(new_n110_), .O(new_n112_));
  inv1   g068(.a(x17), .O(new_n113_));
  inv1   g069(.a(x19), .O(new_n114_));
  nor2   g070(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g071(.a(new_n113_), .b(x12), .O(new_n116_));
  nand2  g072(.a(new_n114_), .b(x14), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g074(.a(x18), .b(x15), .O(new_n119_));
  aoi21  g075(.a(new_n114_), .b(new_n113_), .c(new_n119_), .O(new_n120_));
  aoi22  g076(.a(new_n120_), .b(new_n118_), .c(new_n115_), .d(new_n112_), .O(new_n121_));
  nand3  g077(.a(x19), .b(x18), .c(x17), .O(new_n122_));
  nand2  g078(.a(new_n109_), .b(x11), .O(new_n123_));
  nor2   g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x15), .O(new_n125_));
  oai21  g081(.a(new_n121_), .b(new_n109_), .c(new_n125_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n52_), .c(x31), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n94_), .O(new_n128_));
  nand2  g084(.a(new_n86_), .b(x29), .O(new_n129_));
  aoi21  g085(.a(new_n52_), .b(x30), .c(new_n129_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand4  g087(.a(new_n126_), .b(new_n52_), .c(x31), .d(x30), .O(new_n132_));
  nand2  g088(.a(x32), .b(new_n94_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n129_), .O(new_n135_));
  oai21  g091(.a(x17), .b(x12), .c(x18), .O(new_n136_));
  oai21  g092(.a(new_n113_), .b(new_n111_), .c(new_n136_), .O(new_n137_));
  nand3  g093(.a(new_n115_), .b(x16), .c(x10), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n104_), .c(new_n100_), .O(new_n139_));
  nand3  g095(.a(x18), .b(x17), .c(x11), .O(new_n140_));
  nand3  g096(.a(x18), .b(x17), .c(x16), .O(new_n141_));
  aoi22  g097(.a(new_n141_), .b(new_n114_), .c(new_n140_), .d(new_n109_), .O(new_n142_));
  inv1   g098(.a(new_n141_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n117_), .c(x15), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n142_), .c(new_n139_), .d(new_n137_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x32), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n135_), .c(new_n131_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n108_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n68_), .O(z8));
  inv1   g105(.a(new_n129_), .O(new_n150_));
  nand3  g106(.a(new_n68_), .b(x33), .c(new_n94_), .O(new_n151_));
  nand4  g107(.a(x19), .b(x18), .c(new_n113_), .d(x12), .O(new_n152_));
  aoi22  g108(.a(new_n114_), .b(x14), .c(new_n70_), .d(x13), .O(new_n153_));
  oai21  g109(.a(x19), .b(x18), .c(x17), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n153_), .c(new_n152_), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(x16), .c(new_n124_), .O(new_n156_));
  inv1   g112(.a(new_n122_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(x16), .c(new_n104_), .d(x10), .O(new_n158_));
  oai21  g114(.a(new_n156_), .b(new_n104_), .c(new_n158_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n48_), .O(new_n160_));
  nand2  g116(.a(x31), .b(x30), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n160_), .c(new_n151_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n150_), .O(new_n163_));
  nand3  g119(.a(new_n68_), .b(x33), .c(x30), .O(new_n164_));
  nand2  g120(.a(new_n100_), .b(new_n94_), .O(new_n165_));
  inv1   g121(.a(new_n165_), .O(new_n166_));
  nand3  g122(.a(new_n166_), .b(new_n159_), .c(new_n48_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n164_), .O(new_n168_));
  nand2  g124(.a(new_n138_), .b(new_n104_), .O(new_n169_));
  nand2  g125(.a(new_n165_), .b(new_n161_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(x18), .O(new_n172_));
  nand2  g128(.a(new_n143_), .b(new_n117_), .O(new_n173_));
  nand4  g129(.a(new_n170_), .b(new_n173_), .c(new_n142_), .d(new_n137_), .O(new_n174_));
  nand2  g130(.a(new_n174_), .b(x15), .O(new_n175_));
  aoi21  g131(.a(new_n175_), .b(new_n172_), .c(new_n48_), .O(new_n176_));
  aoi21  g132(.a(new_n168_), .b(new_n129_), .c(new_n176_), .O(new_n177_));
  aoi21  g133(.a(new_n177_), .b(new_n163_), .c(new_n107_), .O(z9));
endmodule


