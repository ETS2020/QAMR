// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_;
  nor2   g000(.a(x30), .b(x19), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x33), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(x32), .b(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(x33), .b(new_n49_), .c(x02), .O(new_n53_));
  oai21  g009(.a(new_n52_), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g011(.a(x01), .O(new_n56_));
  aoi21  g012(.a(x03), .b(x02), .c(new_n56_), .O(new_n57_));
  oai21  g013(.a(x03), .b(x02), .c(new_n57_), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n59_), .O(new_n60_));
  aoi21  g016(.a(x05), .b(new_n49_), .c(new_n60_), .O(new_n61_));
  nand3  g017(.a(new_n60_), .b(x05), .c(new_n49_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  oai21  g019(.a(new_n63_), .b(new_n61_), .c(new_n58_), .O(new_n64_));
  xnor2a g020(.a(x07), .b(x04), .O(new_n65_));
  inv1   g021(.a(x09), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(x08), .O(new_n67_));
  aoi21  g023(.a(new_n65_), .b(new_n64_), .c(new_n67_), .O(new_n68_));
  oai21  g024(.a(new_n65_), .b(new_n64_), .c(new_n68_), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n46_), .O(z2));
  inv1   g026(.a(x27), .O(new_n71_));
  inv1   g027(.a(x19), .O(new_n72_));
  inv1   g028(.a(x24), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai22  g030(.a(x22), .b(x17), .c(x21), .d(x16), .O(new_n75_));
  oai22  g031(.a(x23), .b(x18), .c(x20), .d(x15), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  inv1   g033(.a(x26), .O(new_n78_));
  nand3  g034(.a(new_n57_), .b(new_n78_), .c(x25), .O(new_n79_));
  aoi21  g035(.a(new_n77_), .b(new_n74_), .c(new_n79_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n46_), .c(new_n71_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(z3));
  inv1   g038(.a(x28), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n71_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(new_n83_), .b(new_n71_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n85_), .c(new_n80_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n46_), .O(z4));
  nand2  g044(.a(new_n84_), .b(x29), .O(new_n89_));
  inv1   g045(.a(x29), .O(new_n90_));
  nand2  g046(.a(new_n85_), .b(new_n90_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n89_), .c(new_n80_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n46_), .O(z5));
  inv1   g049(.a(x30), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(x19), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n84_), .c(x29), .O(new_n96_));
  nand2  g052(.a(new_n89_), .b(new_n94_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n96_), .c(new_n80_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z6));
  inv1   g055(.a(new_n77_), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n73_), .O(new_n101_));
  inv1   g057(.a(x31), .O(new_n102_));
  xor2a  g058(.a(new_n89_), .b(new_n102_), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n101_), .c(x30), .O(new_n104_));
  oai21  g060(.a(new_n102_), .b(x30), .c(new_n100_), .O(new_n105_));
  aoi21  g061(.a(new_n105_), .b(x19), .c(new_n79_), .O(new_n106_));
  aoi21  g062(.a(new_n106_), .b(new_n104_), .c(new_n45_), .O(z7));
  inv1   g063(.a(x17), .O(new_n108_));
  inv1   g064(.a(x18), .O(new_n109_));
  nand3  g065(.a(x19), .b(new_n109_), .c(x13), .O(new_n110_));
  nand3  g066(.a(new_n72_), .b(x18), .c(x14), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n110_), .c(new_n108_), .O(new_n112_));
  nand4  g068(.a(x19), .b(x18), .c(new_n108_), .d(x12), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  inv1   g071(.a(x16), .O(new_n116_));
  nand4  g072(.a(x18), .b(x17), .c(new_n116_), .d(x11), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x19), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  nor2   g076(.a(new_n108_), .b(new_n116_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x18), .c(x10), .O(new_n122_));
  nor3   g078(.a(new_n122_), .b(new_n72_), .c(x15), .O(new_n123_));
  aoi21  g079(.a(new_n120_), .b(x15), .c(new_n123_), .O(new_n124_));
  nand2  g080(.a(new_n86_), .b(x29), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x30), .O(new_n126_));
  inv1   g082(.a(new_n95_), .O(new_n127_));
  inv1   g083(.a(new_n125_), .O(new_n128_));
  inv1   g084(.a(x15), .O(new_n129_));
  inv1   g085(.a(new_n122_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g087(.a(new_n109_), .b(x13), .O(new_n132_));
  nand2  g088(.a(new_n108_), .b(x12), .O(new_n133_));
  oai21  g089(.a(x18), .b(x17), .c(x16), .O(new_n134_));
  aoi21  g090(.a(new_n133_), .b(new_n132_), .c(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n118_), .c(x15), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n131_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n128_), .c(new_n127_), .O(new_n138_));
  oai21  g094(.a(new_n126_), .b(new_n124_), .c(new_n138_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x31), .O(new_n140_));
  inv1   g096(.a(x00), .O(new_n141_));
  nor2   g097(.a(x26), .b(new_n141_), .O(new_n142_));
  inv1   g098(.a(x13), .O(new_n143_));
  nand2  g099(.a(new_n121_), .b(x15), .O(new_n144_));
  oai22  g100(.a(new_n144_), .b(new_n95_), .c(new_n94_), .d(x18), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  oai21  g102(.a(new_n128_), .b(x30), .c(x31), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n46_), .O(new_n148_));
  nand3  g104(.a(new_n121_), .b(x18), .c(x15), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  oai21  g106(.a(new_n94_), .b(x14), .c(new_n72_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n148_), .c(new_n146_), .d(x32), .O(new_n153_));
  nand2  g109(.a(x17), .b(x11), .O(new_n154_));
  nand2  g110(.a(x16), .b(x12), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n154_), .c(new_n109_), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n121_), .c(x15), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(new_n122_), .c(new_n46_), .O(new_n158_));
  aoi21  g114(.a(new_n149_), .b(new_n72_), .c(new_n128_), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n94_), .c(new_n158_), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n153_), .c(new_n142_), .O(new_n161_));
  aoi21  g117(.a(new_n140_), .b(new_n52_), .c(new_n161_), .O(z8));
  nor2   g118(.a(new_n125_), .b(new_n102_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(x30), .O(new_n164_));
  or2    g120(.a(new_n164_), .b(new_n124_), .O(new_n165_));
  aoi21  g121(.a(new_n136_), .b(new_n131_), .c(new_n95_), .O(new_n166_));
  nand2  g122(.a(new_n125_), .b(new_n102_), .O(new_n167_));
  inv1   g123(.a(new_n167_), .O(new_n168_));
  aoi21  g124(.a(new_n168_), .b(new_n166_), .c(x33), .O(new_n169_));
  nand2  g125(.a(new_n167_), .b(new_n127_), .O(new_n170_));
  nand4  g126(.a(new_n170_), .b(new_n152_), .c(new_n146_), .d(x33), .O(new_n171_));
  oai21  g127(.a(new_n150_), .b(x19), .c(new_n163_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(x30), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n158_), .O(new_n174_));
  oai21  g130(.a(new_n174_), .b(new_n171_), .c(new_n142_), .O(new_n175_));
  aoi21  g131(.a(new_n169_), .b(new_n165_), .c(new_n175_), .O(z9));
endmodule


