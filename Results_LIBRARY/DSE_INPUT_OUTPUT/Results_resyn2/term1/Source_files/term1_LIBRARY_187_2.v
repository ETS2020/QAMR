// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x31), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x12), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  inv1   g003(.a(x33), .O(new_n48_));
  xnor2a g004(.a(x03), .b(x02), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  or2    g006(.a(new_n49_), .b(x32), .O(new_n51_));
  nand3  g007(.a(new_n51_), .b(new_n50_), .c(new_n46_), .O(z1));
  inv1   g008(.a(x01), .O(new_n53_));
  aoi21  g009(.a(x03), .b(x02), .c(new_n53_), .O(new_n54_));
  oai21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  inv1   g011(.a(x03), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  nand2  g013(.a(x06), .b(new_n57_), .O(new_n58_));
  aoi21  g014(.a(x05), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g015(.a(new_n58_), .b(x05), .c(new_n56_), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  oai21  g017(.a(new_n61_), .b(new_n59_), .c(new_n55_), .O(new_n62_));
  xor2a  g018(.a(x07), .b(x04), .O(new_n63_));
  xor2a  g019(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand3  g021(.a(new_n46_), .b(new_n65_), .c(x08), .O(new_n66_));
  nor2   g022(.a(new_n66_), .b(new_n64_), .O(z2));
  nor2   g023(.a(x21), .b(x16), .O(new_n68_));
  nor2   g024(.a(x22), .b(x17), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g026(.a(x19), .O(new_n71_));
  inv1   g027(.a(x24), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g029(.a(x23), .b(x18), .O(new_n74_));
  nor2   g030(.a(x20), .b(x15), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(new_n77_));
  inv1   g033(.a(x26), .O(new_n78_));
  nand3  g034(.a(new_n54_), .b(new_n78_), .c(x25), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(x27), .c(new_n46_), .O(z3));
  inv1   g038(.a(x27), .O(new_n83_));
  inv1   g039(.a(x28), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n83_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n81_), .c(new_n46_), .O(z4));
  inv1   g045(.a(new_n81_), .O(new_n90_));
  inv1   g046(.a(x29), .O(new_n91_));
  nand2  g047(.a(new_n86_), .b(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n85_), .b(x29), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n92_), .c(new_n90_), .d(new_n46_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z5));
  inv1   g051(.a(x30), .O(new_n96_));
  nand2  g052(.a(new_n93_), .b(new_n96_), .O(new_n97_));
  nand3  g053(.a(new_n85_), .b(x30), .c(x29), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n81_), .c(new_n46_), .O(z6));
  nand2  g056(.a(new_n81_), .b(new_n46_), .O(new_n101_));
  nor2   g057(.a(x31), .b(x12), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  nor2   g059(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  aoi21  g060(.a(new_n98_), .b(x31), .c(new_n104_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n101_), .O(z7));
  nand2  g062(.a(new_n78_), .b(x00), .O(new_n107_));
  inv1   g063(.a(x12), .O(new_n108_));
  nand2  g064(.a(x31), .b(x17), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  inv1   g066(.a(x15), .O(new_n111_));
  nand3  g067(.a(x18), .b(x16), .c(x10), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g069(.a(x18), .b(x16), .c(x15), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  oai21  g071(.a(x18), .b(x13), .c(x16), .O(new_n116_));
  nand2  g072(.a(x18), .b(x11), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n116_), .c(new_n71_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n115_), .c(new_n113_), .O(new_n119_));
  oai21  g075(.a(x28), .b(x27), .c(x29), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n96_), .O(new_n121_));
  nand3  g077(.a(new_n87_), .b(x30), .c(x29), .O(new_n122_));
  nand2  g078(.a(new_n71_), .b(x14), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n115_), .c(x17), .O(new_n124_));
  inv1   g080(.a(x17), .O(new_n125_));
  nand2  g081(.a(x16), .b(x15), .O(new_n126_));
  nand2  g082(.a(x19), .b(x18), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n126_), .c(new_n125_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n124_), .c(new_n122_), .d(new_n121_), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n119_), .c(x31), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n110_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(x32), .O(new_n132_));
  nand3  g088(.a(new_n71_), .b(x18), .c(x14), .O(new_n133_));
  inv1   g089(.a(x18), .O(new_n134_));
  nand3  g090(.a(x19), .b(new_n134_), .c(x13), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n133_), .c(new_n125_), .O(new_n136_));
  nand4  g092(.a(x19), .b(x18), .c(new_n125_), .d(x12), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  inv1   g095(.a(x16), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x11), .O(new_n141_));
  nand3  g097(.a(x19), .b(x18), .c(x17), .O(new_n142_));
  or2    g098(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n139_), .c(new_n111_), .O(new_n144_));
  nand3  g100(.a(x16), .b(new_n111_), .c(x10), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g102(.a(new_n122_), .b(new_n121_), .O(new_n147_));
  nor3   g103(.a(new_n147_), .b(x32), .c(new_n45_), .O(new_n148_));
  oai21  g104(.a(new_n146_), .b(new_n144_), .c(new_n148_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n132_), .c(new_n107_), .O(z8));
  nor2   g106(.a(new_n122_), .b(new_n45_), .O(new_n151_));
  oai21  g107(.a(new_n146_), .b(new_n144_), .c(new_n151_), .O(new_n152_));
  and2   g108(.a(new_n135_), .b(new_n133_), .O(new_n153_));
  inv1   g109(.a(new_n127_), .O(new_n154_));
  oai21  g110(.a(new_n141_), .b(new_n111_), .c(new_n145_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  oai21  g112(.a(new_n153_), .b(new_n126_), .c(new_n156_), .O(new_n157_));
  nand4  g113(.a(new_n120_), .b(new_n102_), .c(new_n96_), .d(x17), .O(new_n158_));
  inv1   g114(.a(new_n158_), .O(new_n159_));
  aoi21  g115(.a(new_n159_), .b(new_n157_), .c(x33), .O(new_n160_));
  inv1   g116(.a(new_n107_), .O(new_n161_));
  and2   g117(.a(new_n119_), .b(new_n46_), .O(new_n162_));
  aoi21  g118(.a(new_n120_), .b(new_n96_), .c(x31), .O(new_n163_));
  oai21  g119(.a(new_n163_), .b(new_n125_), .c(new_n108_), .O(new_n164_));
  inv1   g120(.a(new_n128_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n122_), .c(x31), .O(new_n166_));
  aoi21  g122(.a(new_n71_), .b(x14), .c(new_n114_), .O(new_n167_));
  nand2  g123(.a(new_n109_), .b(new_n103_), .O(new_n168_));
  aoi21  g124(.a(new_n168_), .b(new_n167_), .c(new_n48_), .O(new_n169_));
  nand3  g125(.a(new_n169_), .b(new_n166_), .c(new_n164_), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n162_), .c(new_n161_), .O(new_n171_));
  aoi21  g127(.a(new_n160_), .b(new_n152_), .c(new_n171_), .O(z9));
endmodule


