// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_;
  inv1   g000(.a(x15), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  inv1   g005(.a(x03), .O(new_n50_));
  inv1   g006(.a(x33), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  aoi21  g008(.a(x32), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(new_n50_), .c(new_n53_), .O(new_n54_));
  inv1   g010(.a(x32), .O(new_n55_));
  aoi21  g011(.a(x33), .b(new_n50_), .c(x02), .O(new_n56_));
  oai21  g012(.a(new_n55_), .b(new_n50_), .c(new_n56_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n54_), .c(new_n47_), .O(z1));
  inv1   g014(.a(x08), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  aoi21  g016(.a(x03), .b(x02), .c(new_n60_), .O(new_n61_));
  oai21  g017(.a(x03), .b(x02), .c(new_n61_), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n52_), .O(new_n63_));
  aoi21  g019(.a(x05), .b(new_n50_), .c(new_n63_), .O(new_n64_));
  nand3  g020(.a(new_n63_), .b(x05), .c(new_n50_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n60_), .O(new_n66_));
  oai21  g022(.a(new_n66_), .b(new_n64_), .c(new_n62_), .O(new_n67_));
  xor2a  g023(.a(x07), .b(x04), .O(new_n68_));
  xor2a  g024(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nor4   g025(.a(new_n69_), .b(new_n47_), .c(x09), .d(new_n59_), .O(z2));
  nor2   g026(.a(x21), .b(x16), .O(new_n71_));
  nor2   g027(.a(x22), .b(x17), .O(new_n72_));
  nor2   g028(.a(x23), .b(x18), .O(new_n73_));
  nor2   g029(.a(x24), .b(x19), .O(new_n74_));
  nor4   g030(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(x30), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n45_), .O(new_n77_));
  nand2  g033(.a(new_n75_), .b(x20), .O(new_n78_));
  inv1   g034(.a(x26), .O(new_n79_));
  nand3  g035(.a(new_n61_), .b(new_n79_), .c(x25), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nor3   g038(.a(new_n82_), .b(new_n77_), .c(x27), .O(z3));
  oai21  g039(.a(x20), .b(x15), .c(new_n75_), .O(new_n84_));
  inv1   g040(.a(x27), .O(new_n85_));
  inv1   g041(.a(x28), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n85_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n88_), .c(new_n84_), .d(new_n81_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n48_), .O(z4));
  nand2  g047(.a(new_n87_), .b(x29), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nor2   g049(.a(new_n87_), .b(x29), .O(new_n94_));
  nor4   g050(.a(new_n94_), .b(new_n93_), .c(new_n82_), .d(new_n77_), .O(z5));
  nand2  g051(.a(new_n93_), .b(x30), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(new_n97_));
  nor2   g053(.a(new_n93_), .b(x30), .O(new_n98_));
  nor4   g054(.a(new_n98_), .b(new_n97_), .c(new_n82_), .d(new_n77_), .O(z6));
  inv1   g055(.a(x31), .O(new_n100_));
  nand2  g056(.a(new_n97_), .b(new_n100_), .O(new_n101_));
  nand2  g057(.a(new_n75_), .b(x15), .O(new_n102_));
  nand2  g058(.a(new_n96_), .b(x31), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  nor2   g060(.a(new_n104_), .b(new_n82_), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n47_), .O(z7));
  nand2  g062(.a(new_n55_), .b(x31), .O(new_n107_));
  inv1   g063(.a(x17), .O(new_n108_));
  inv1   g064(.a(x19), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x18), .c(x14), .O(new_n110_));
  inv1   g066(.a(x18), .O(new_n111_));
  nand3  g067(.a(x19), .b(new_n111_), .c(x13), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n110_), .c(new_n108_), .O(new_n113_));
  nand4  g069(.a(x19), .b(x18), .c(new_n108_), .d(x12), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n113_), .c(x16), .O(new_n116_));
  inv1   g072(.a(x16), .O(new_n117_));
  nand3  g073(.a(x18), .b(x17), .c(x11), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x19), .c(new_n117_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n116_), .O(new_n121_));
  nand3  g077(.a(x18), .b(x17), .c(x16), .O(new_n122_));
  nand4  g078(.a(new_n46_), .b(x19), .c(new_n45_), .d(x10), .O(new_n123_));
  nor2   g079(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  aoi21  g080(.a(new_n121_), .b(x15), .c(new_n124_), .O(new_n125_));
  nand2  g081(.a(x32), .b(x30), .O(new_n126_));
  oai21  g082(.a(new_n125_), .b(new_n107_), .c(new_n126_), .O(new_n127_));
  nand2  g083(.a(new_n89_), .b(x29), .O(new_n128_));
  inv1   g084(.a(new_n128_), .O(new_n129_));
  nor2   g085(.a(new_n46_), .b(x15), .O(new_n130_));
  inv1   g086(.a(new_n122_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(x10), .O(new_n132_));
  nand4  g088(.a(new_n55_), .b(x31), .c(x30), .d(x19), .O(new_n133_));
  oai22  g089(.a(new_n133_), .b(new_n132_), .c(new_n130_), .d(new_n55_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n128_), .O(new_n135_));
  nand2  g091(.a(x18), .b(x12), .O(new_n136_));
  oai21  g092(.a(x18), .b(x13), .c(x17), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n136_), .c(new_n117_), .O(new_n138_));
  nand2  g094(.a(new_n131_), .b(x15), .O(new_n139_));
  aoi22  g095(.a(new_n139_), .b(new_n109_), .c(new_n132_), .d(new_n45_), .O(new_n140_));
  oai21  g096(.a(new_n138_), .b(new_n119_), .c(new_n140_), .O(new_n141_));
  nand2  g097(.a(new_n109_), .b(x14), .O(new_n142_));
  nand3  g098(.a(new_n131_), .b(new_n142_), .c(x15), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x31), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n141_), .c(x32), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n135_), .O(new_n146_));
  aoi21  g102(.a(new_n129_), .b(new_n127_), .c(new_n146_), .O(new_n147_));
  nand2  g103(.a(new_n79_), .b(x00), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n147_), .c(new_n48_), .O(z8));
  nand2  g105(.a(new_n51_), .b(new_n100_), .O(new_n150_));
  nand2  g106(.a(x33), .b(x30), .O(new_n151_));
  oai21  g107(.a(new_n150_), .b(new_n125_), .c(new_n151_), .O(new_n152_));
  nand4  g108(.a(new_n51_), .b(x31), .c(x30), .d(x19), .O(new_n153_));
  oai22  g109(.a(new_n153_), .b(new_n132_), .c(new_n130_), .d(new_n51_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n129_), .O(new_n155_));
  aoi21  g111(.a(new_n131_), .b(new_n142_), .c(x31), .O(new_n156_));
  xnor2a g112(.a(x31), .b(x30), .O(new_n157_));
  oai21  g113(.a(new_n156_), .b(new_n45_), .c(new_n157_), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n141_), .c(x33), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n155_), .O(new_n160_));
  aoi21  g116(.a(new_n152_), .b(new_n128_), .c(new_n160_), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n148_), .c(new_n48_), .O(z9));
endmodule


