// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  inv1   g000(.a(x29), .O(new_n45_));
  nor2   g001(.a(x33), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  xor2a  g004(.a(x03), .b(x02), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  nor2   g007(.a(new_n46_), .b(x32), .O(new_n52_));
  nand2  g008(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  oai21  g009(.a(new_n51_), .b(new_n49_), .c(new_n53_), .O(z1));
  xor2a  g010(.a(x07), .b(x04), .O(new_n55_));
  inv1   g011(.a(x03), .O(new_n56_));
  nand2  g012(.a(x05), .b(new_n56_), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand4  g016(.a(x06), .b(x05), .c(new_n56_), .d(new_n58_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  aoi21  g018(.a(new_n49_), .b(x01), .c(new_n62_), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n55_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n63_), .b(new_n55_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n64_), .c(new_n47_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  nor2   g025(.a(x21), .b(x16), .O(new_n70_));
  nor2   g026(.a(x24), .b(x19), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x17), .O(new_n73_));
  inv1   g029(.a(x22), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x20), .b(x15), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(new_n79_));
  inv1   g035(.a(x26), .O(new_n80_));
  nand2  g036(.a(x03), .b(x02), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n80_), .c(x25), .d(x01), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n47_), .c(new_n69_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(z3));
  inv1   g043(.a(x28), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n69_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n88_), .b(new_n69_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n84_), .c(new_n47_), .O(z4));
  nand2  g049(.a(new_n89_), .b(x29), .O(new_n94_));
  oai21  g050(.a(new_n50_), .b(new_n45_), .c(new_n90_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n94_), .c(new_n85_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(z5));
  inv1   g053(.a(new_n94_), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(x30), .O(new_n99_));
  nand2  g055(.a(new_n98_), .b(x30), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n85_), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n99_), .c(new_n47_), .O(z6));
  inv1   g058(.a(x31), .O(new_n103_));
  inv1   g059(.a(new_n101_), .O(new_n104_));
  nand3  g060(.a(new_n89_), .b(x31), .c(x30), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n84_), .c(x33), .O(new_n106_));
  aoi22  g062(.a(new_n106_), .b(x29), .c(new_n104_), .d(new_n103_), .O(z7));
  inv1   g063(.a(x15), .O(new_n108_));
  inv1   g064(.a(x19), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x18), .c(x14), .O(new_n110_));
  inv1   g066(.a(x18), .O(new_n111_));
  nand3  g067(.a(x19), .b(new_n111_), .c(x13), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n110_), .c(new_n73_), .O(new_n113_));
  nand4  g069(.a(x19), .b(x18), .c(new_n73_), .d(x12), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n113_), .c(x16), .O(new_n116_));
  inv1   g072(.a(x16), .O(new_n117_));
  nand3  g073(.a(x18), .b(x17), .c(x11), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x19), .c(new_n117_), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n116_), .c(new_n108_), .O(new_n121_));
  nand4  g077(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n122_));
  nor3   g078(.a(new_n122_), .b(new_n109_), .c(x15), .O(new_n123_));
  inv1   g079(.a(x30), .O(new_n124_));
  aoi21  g080(.a(new_n88_), .b(new_n69_), .c(new_n45_), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  aoi21  g083(.a(new_n125_), .b(x30), .c(new_n103_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n127_), .c(new_n52_), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  oai21  g086(.a(new_n123_), .b(new_n121_), .c(new_n130_), .O(new_n131_));
  nand4  g087(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n132_));
  oai21  g088(.a(new_n119_), .b(x16), .c(x19), .O(new_n133_));
  aoi22  g089(.a(new_n133_), .b(new_n132_), .c(new_n122_), .d(new_n108_), .O(new_n134_));
  nand2  g090(.a(x18), .b(x12), .O(new_n135_));
  aoi21  g091(.a(new_n109_), .b(x14), .c(new_n132_), .O(new_n136_));
  oai21  g092(.a(x18), .b(x13), .c(x17), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n135_), .c(new_n136_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n134_), .c(new_n128_), .d(new_n127_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n47_), .c(x32), .O(new_n140_));
  nand2  g096(.a(new_n80_), .b(x00), .O(new_n141_));
  aoi21  g097(.a(new_n140_), .b(new_n131_), .c(new_n141_), .O(z8));
  nand4  g098(.a(new_n50_), .b(new_n103_), .c(new_n124_), .d(new_n45_), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  oai21  g100(.a(new_n123_), .b(new_n121_), .c(new_n144_), .O(new_n145_));
  inv1   g101(.a(new_n134_), .O(new_n146_));
  nand2  g102(.a(new_n103_), .b(new_n124_), .O(new_n147_));
  oai21  g103(.a(new_n126_), .b(new_n124_), .c(new_n147_), .O(new_n148_));
  nand2  g104(.a(new_n125_), .b(new_n103_), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(new_n148_), .c(new_n138_), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n146_), .c(x33), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n145_), .c(new_n141_), .O(z9));
endmodule


