// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x15), .O(new_n45_));
  nand2  g001(.a(x18), .b(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xor2a  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x32), .O(new_n49_));
  inv1   g005(.a(new_n46_), .O(new_n50_));
  aoi21  g006(.a(new_n48_), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n48_), .b(x33), .c(new_n51_), .O(z1));
  inv1   g008(.a(x09), .O(new_n53_));
  xor2a  g009(.a(x07), .b(x04), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  inv1   g011(.a(x01), .O(new_n56_));
  inv1   g012(.a(x02), .O(new_n57_));
  inv1   g013(.a(x03), .O(new_n58_));
  nand4  g014(.a(x06), .b(x05), .c(new_n58_), .d(new_n57_), .O(new_n59_));
  nand2  g015(.a(x05), .b(new_n58_), .O(new_n60_));
  nand2  g016(.a(x06), .b(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(new_n59_), .c(new_n56_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nor2   g020(.a(new_n48_), .b(new_n56_), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n55_), .O(new_n67_));
  oai21  g023(.a(new_n65_), .b(new_n64_), .c(new_n54_), .O(new_n68_));
  nand4  g024(.a(new_n68_), .b(new_n67_), .c(new_n53_), .d(x08), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n46_), .O(z2));
  inv1   g026(.a(x27), .O(new_n71_));
  nand2  g027(.a(x03), .b(x02), .O(new_n72_));
  inv1   g028(.a(x18), .O(new_n73_));
  inv1   g029(.a(x23), .O(new_n74_));
  nor2   g030(.a(x20), .b(x15), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(new_n76_));
  nor2   g032(.a(x21), .b(x16), .O(new_n77_));
  oai22  g033(.a(x24), .b(x19), .c(x22), .d(x17), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g035(.a(new_n79_), .b(new_n76_), .c(new_n50_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n72_), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(x25), .b(x01), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(x26), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n82_), .c(new_n71_), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(z3));
  nand2  g042(.a(x28), .b(x27), .O(new_n87_));
  inv1   g043(.a(x28), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n71_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n87_), .c(new_n84_), .d(new_n82_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z4));
  nand2  g047(.a(new_n84_), .b(new_n72_), .O(new_n92_));
  aoi21  g048(.a(new_n74_), .b(new_n73_), .c(new_n75_), .O(new_n93_));
  aoi21  g049(.a(new_n93_), .b(new_n79_), .c(new_n92_), .O(new_n94_));
  inv1   g050(.a(new_n87_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(x29), .O(new_n96_));
  inv1   g052(.a(x29), .O(new_n97_));
  nand2  g053(.a(new_n87_), .b(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n96_), .c(new_n94_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n46_), .O(z5));
  inv1   g056(.a(new_n92_), .O(new_n101_));
  inv1   g057(.a(x30), .O(new_n102_));
  nand2  g058(.a(new_n96_), .b(new_n102_), .O(new_n103_));
  nand3  g059(.a(new_n95_), .b(x30), .c(x29), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n103_), .c(new_n101_), .d(new_n80_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(z6));
  xor2a  g062(.a(new_n104_), .b(x31), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n94_), .c(new_n50_), .O(z7));
  inv1   g064(.a(x26), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x00), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n49_), .c(new_n73_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n45_), .O(new_n112_));
  nand3  g068(.a(new_n89_), .b(x30), .c(x29), .O(new_n113_));
  inv1   g069(.a(x31), .O(new_n114_));
  oai21  g070(.a(x28), .b(x27), .c(x29), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n102_), .c(new_n114_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  inv1   g073(.a(x17), .O(new_n118_));
  inv1   g074(.a(x19), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x18), .c(x14), .O(new_n120_));
  nand4  g076(.a(x19), .b(new_n73_), .c(x15), .d(x13), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n120_), .c(new_n118_), .O(new_n122_));
  nand4  g078(.a(x19), .b(x18), .c(new_n118_), .d(x12), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n122_), .c(x16), .O(new_n125_));
  inv1   g081(.a(x16), .O(new_n126_));
  nand3  g082(.a(x18), .b(x17), .c(x11), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x19), .c(new_n126_), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(new_n125_), .c(new_n117_), .O(new_n130_));
  nand3  g086(.a(x18), .b(x17), .c(x16), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n119_), .c(x14), .O(new_n133_));
  nand2  g089(.a(new_n127_), .b(new_n126_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n131_), .c(x19), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g092(.a(x18), .b(x12), .O(new_n137_));
  oai21  g093(.a(x18), .b(x13), .c(x17), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n137_), .c(new_n49_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n116_), .c(new_n113_), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(new_n136_), .c(new_n110_), .O(new_n142_));
  oai21  g098(.a(new_n130_), .b(x32), .c(new_n142_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n112_), .O(z8));
  inv1   g100(.a(x33), .O(new_n145_));
  oai21  g101(.a(new_n110_), .b(new_n145_), .c(new_n73_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n45_), .O(new_n147_));
  inv1   g103(.a(new_n110_), .O(new_n148_));
  nand3  g104(.a(new_n115_), .b(new_n114_), .c(new_n102_), .O(new_n149_));
  nand4  g105(.a(new_n89_), .b(x31), .c(x30), .d(x29), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n145_), .O(new_n152_));
  aoi21  g108(.a(new_n129_), .b(new_n125_), .c(new_n152_), .O(new_n153_));
  aoi22  g109(.a(new_n138_), .b(new_n137_), .c(new_n135_), .d(new_n133_), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n151_), .c(new_n145_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n153_), .c(new_n148_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n147_), .O(z9));
endmodule


