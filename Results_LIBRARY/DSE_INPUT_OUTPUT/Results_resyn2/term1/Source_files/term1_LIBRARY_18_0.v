// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_;
  nor2   g000(.a(x18), .b(x14), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  xnor2a g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  aoi21  g006(.a(new_n49_), .b(new_n50_), .c(new_n45_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(x33), .c(new_n51_), .O(z1));
  inv1   g008(.a(x08), .O(new_n53_));
  inv1   g009(.a(x01), .O(new_n54_));
  aoi21  g010(.a(x03), .b(x02), .c(new_n54_), .O(new_n55_));
  oai21  g011(.a(x03), .b(x02), .c(new_n55_), .O(new_n56_));
  inv1   g012(.a(x03), .O(new_n57_));
  inv1   g013(.a(x02), .O(new_n58_));
  nand2  g014(.a(x06), .b(new_n58_), .O(new_n59_));
  aoi21  g015(.a(x05), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand3  g016(.a(new_n59_), .b(x05), .c(new_n57_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  oai21  g018(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  xor2a  g019(.a(x07), .b(x04), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor4   g021(.a(new_n65_), .b(new_n45_), .c(x09), .d(new_n53_), .O(z2));
  inv1   g022(.a(x27), .O(new_n67_));
  nor2   g023(.a(x24), .b(x19), .O(new_n68_));
  nor2   g024(.a(x22), .b(x17), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g026(.a(x16), .O(new_n71_));
  inv1   g027(.a(x21), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  nor2   g030(.a(x23), .b(x18), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(new_n77_));
  inv1   g033(.a(x26), .O(new_n78_));
  nand3  g034(.a(new_n55_), .b(new_n78_), .c(x25), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n77_), .c(new_n67_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n46_), .O(z3));
  nor2   g038(.a(new_n79_), .b(new_n45_), .O(new_n83_));
  and2   g039(.a(new_n83_), .b(new_n77_), .O(new_n84_));
  inv1   g040(.a(x28), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n85_), .b(new_n67_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n87_), .c(new_n84_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(z4));
  nand2  g046(.a(new_n86_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n87_), .b(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n91_), .c(new_n84_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z5));
  inv1   g051(.a(x30), .O(new_n96_));
  nand2  g052(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand3  g053(.a(new_n86_), .b(x30), .c(x29), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n97_), .c(new_n84_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z6));
  xor2a  g056(.a(new_n98_), .b(x31), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n84_), .O(z7));
  nand3  g058(.a(x17), .b(x16), .c(x15), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  inv1   g060(.a(x13), .O(new_n105_));
  inv1   g061(.a(x19), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n104_), .c(x18), .O(new_n108_));
  oai21  g064(.a(x28), .b(x27), .c(x29), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n96_), .O(new_n110_));
  inv1   g066(.a(x31), .O(new_n111_));
  nor2   g067(.a(new_n109_), .b(new_n96_), .O(new_n112_));
  nor2   g068(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n108_), .c(x14), .O(new_n115_));
  inv1   g071(.a(x15), .O(new_n116_));
  inv1   g072(.a(x17), .O(new_n117_));
  nand2  g073(.a(x16), .b(x10), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(x19), .c(new_n104_), .O(new_n120_));
  nand2  g076(.a(new_n106_), .b(x14), .O(new_n121_));
  nand2  g077(.a(x16), .b(x12), .O(new_n122_));
  oai21  g078(.a(x16), .b(x11), .c(x17), .O(new_n123_));
  aoi22  g079(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n104_), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(new_n113_), .c(new_n110_), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n120_), .c(x18), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n115_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x32), .O(new_n128_));
  nand4  g084(.a(x19), .b(x18), .c(new_n117_), .d(x12), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  inv1   g086(.a(x18), .O(new_n131_));
  nand3  g087(.a(x19), .b(new_n131_), .c(x13), .O(new_n132_));
  nand2  g088(.a(new_n106_), .b(x18), .O(new_n133_));
  nand2  g089(.a(x17), .b(x14), .O(new_n134_));
  aoi21  g090(.a(new_n133_), .b(new_n132_), .c(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n130_), .c(x16), .O(new_n136_));
  nand2  g092(.a(x18), .b(x17), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(x19), .c(new_n71_), .d(x11), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n136_), .c(new_n116_), .O(new_n140_));
  nand2  g096(.a(new_n138_), .b(x19), .O(new_n141_));
  nor3   g097(.a(new_n141_), .b(new_n118_), .c(x15), .O(new_n142_));
  nand3  g098(.a(new_n113_), .b(new_n110_), .c(new_n50_), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  oai21  g100(.a(new_n142_), .b(new_n140_), .c(new_n144_), .O(new_n145_));
  nand2  g101(.a(new_n78_), .b(x00), .O(new_n146_));
  aoi21  g102(.a(new_n145_), .b(new_n128_), .c(new_n146_), .O(z8));
  inv1   g103(.a(x14), .O(new_n148_));
  nand3  g104(.a(x33), .b(new_n78_), .c(x00), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n103_), .c(x18), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  inv1   g107(.a(x33), .O(new_n152_));
  nand3  g108(.a(new_n106_), .b(x18), .c(x14), .O(new_n153_));
  aoi21  g109(.a(new_n153_), .b(new_n132_), .c(new_n117_), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n130_), .c(x16), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n139_), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(x15), .c(new_n142_), .O(new_n157_));
  nand3  g113(.a(new_n109_), .b(new_n111_), .c(new_n96_), .O(new_n158_));
  nand2  g114(.a(new_n112_), .b(x31), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  inv1   g116(.a(new_n160_), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n157_), .c(new_n152_), .O(new_n162_));
  oai21  g118(.a(new_n137_), .b(new_n118_), .c(new_n116_), .O(new_n163_));
  nand2  g119(.a(x18), .b(x12), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n117_), .O(new_n165_));
  aoi21  g121(.a(new_n131_), .b(new_n105_), .c(new_n152_), .O(new_n166_));
  nand3  g122(.a(new_n166_), .b(new_n165_), .c(new_n163_), .O(new_n167_));
  nand2  g123(.a(x16), .b(x15), .O(new_n168_));
  nor2   g124(.a(new_n141_), .b(new_n168_), .O(new_n169_));
  inv1   g125(.a(x11), .O(new_n170_));
  oai21  g126(.a(new_n137_), .b(new_n170_), .c(new_n71_), .O(new_n171_));
  oai21  g127(.a(new_n137_), .b(new_n168_), .c(new_n106_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n171_), .O(new_n173_));
  nor3   g129(.a(new_n173_), .b(new_n169_), .c(new_n167_), .O(new_n174_));
  aoi21  g130(.a(new_n174_), .b(new_n160_), .c(new_n146_), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n162_), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n151_), .O(z9));
endmodule


