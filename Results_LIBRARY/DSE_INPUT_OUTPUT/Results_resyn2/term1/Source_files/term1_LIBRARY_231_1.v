// Benchmark "FAU" written by ABC on Tue Aug 11 21:14:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  inv1   g000(.a(x31), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x29), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x33), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nand2  g005(.a(x32), .b(new_n49_), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x32), .O(new_n52_));
  aoi21  g008(.a(x33), .b(new_n49_), .c(x02), .O(new_n53_));
  oai21  g009(.a(new_n52_), .b(new_n49_), .c(new_n53_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g011(.a(x08), .O(new_n56_));
  xor2a  g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  inv1   g014(.a(x02), .O(new_n59_));
  nand2  g015(.a(new_n49_), .b(new_n59_), .O(new_n60_));
  nand2  g016(.a(x03), .b(x02), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n49_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n59_), .O(new_n64_));
  xor2a  g020(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n58_), .c(new_n62_), .O(new_n66_));
  xor2a  g022(.a(new_n66_), .b(new_n57_), .O(new_n67_));
  nor4   g023(.a(new_n67_), .b(new_n46_), .c(x09), .d(new_n56_), .O(z2));
  inv1   g024(.a(new_n46_), .O(new_n69_));
  nor2   g025(.a(x21), .b(x16), .O(new_n70_));
  nor2   g026(.a(x24), .b(x19), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g028(.a(x17), .O(new_n73_));
  inv1   g029(.a(x22), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g031(.a(x20), .b(x15), .O(new_n76_));
  nor2   g032(.a(x23), .b(x18), .O(new_n77_));
  nor2   g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(new_n79_));
  inv1   g035(.a(x26), .O(new_n80_));
  nand4  g036(.a(new_n61_), .b(new_n80_), .c(x25), .d(x01), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(x27), .c(new_n69_), .O(z3));
  inv1   g040(.a(new_n83_), .O(new_n85_));
  inv1   g041(.a(x27), .O(new_n86_));
  inv1   g042(.a(x28), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nor2   g045(.a(x28), .b(x27), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n89_), .c(new_n85_), .d(new_n69_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(z4));
  inv1   g049(.a(x29), .O(new_n94_));
  nand2  g050(.a(new_n89_), .b(new_n94_), .O(new_n95_));
  nand2  g051(.a(new_n88_), .b(x29), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n95_), .c(new_n85_), .d(new_n69_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(z5));
  inv1   g054(.a(x30), .O(new_n99_));
  nand2  g055(.a(new_n96_), .b(new_n99_), .O(new_n100_));
  nand3  g056(.a(new_n88_), .b(x30), .c(x29), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n83_), .c(new_n69_), .O(z6));
  xor2a  g059(.a(new_n101_), .b(x31), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n85_), .O(z7));
  inv1   g061(.a(x16), .O(new_n106_));
  inv1   g062(.a(x19), .O(new_n107_));
  nand2  g063(.a(x32), .b(x18), .O(new_n108_));
  inv1   g064(.a(x13), .O(new_n109_));
  nor2   g065(.a(x18), .b(new_n109_), .O(new_n110_));
  nor2   g066(.a(new_n45_), .b(new_n99_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n110_), .c(new_n90_), .d(new_n52_), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n108_), .c(new_n107_), .O(new_n113_));
  inv1   g069(.a(x18), .O(new_n114_));
  inv1   g070(.a(x14), .O(new_n115_));
  nand2  g071(.a(x32), .b(new_n115_), .O(new_n116_));
  nor2   g072(.a(x19), .b(new_n115_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n111_), .c(new_n90_), .d(new_n52_), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n116_), .c(new_n114_), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n113_), .c(x17), .O(new_n120_));
  nand3  g076(.a(new_n111_), .b(new_n90_), .c(new_n52_), .O(new_n121_));
  nand4  g077(.a(x19), .b(x18), .c(new_n73_), .d(x12), .O(new_n122_));
  or2    g078(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(new_n120_), .c(new_n106_), .O(new_n124_));
  nand3  g080(.a(x18), .b(x17), .c(x11), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x19), .c(new_n106_), .O(new_n127_));
  nor2   g083(.a(new_n127_), .b(new_n121_), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n124_), .c(x15), .O(new_n129_));
  nand2  g085(.a(x32), .b(x30), .O(new_n130_));
  inv1   g086(.a(new_n122_), .O(new_n131_));
  nand2  g087(.a(new_n114_), .b(x13), .O(new_n132_));
  nand2  g088(.a(new_n107_), .b(x14), .O(new_n133_));
  oai21  g089(.a(x19), .b(x18), .c(x17), .O(new_n134_));
  aoi21  g090(.a(new_n133_), .b(new_n132_), .c(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n131_), .c(x16), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n127_), .O(new_n137_));
  nor2   g093(.a(new_n114_), .b(new_n73_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(x16), .c(x10), .O(new_n139_));
  nor3   g095(.a(new_n139_), .b(new_n107_), .c(x15), .O(new_n140_));
  aoi21  g096(.a(new_n137_), .b(x15), .c(new_n140_), .O(new_n141_));
  nor2   g097(.a(x32), .b(new_n45_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n99_), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n141_), .c(new_n130_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n91_), .O(new_n145_));
  nand2  g101(.a(x32), .b(new_n99_), .O(new_n146_));
  nand3  g102(.a(new_n142_), .b(new_n140_), .c(x30), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  aoi22  g104(.a(x18), .b(x12), .c(x17), .d(x13), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n106_), .c(new_n125_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x15), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n139_), .O(new_n152_));
  nand3  g108(.a(new_n138_), .b(x16), .c(x15), .O(new_n153_));
  inv1   g109(.a(new_n153_), .O(new_n154_));
  aoi21  g110(.a(new_n152_), .b(x19), .c(new_n154_), .O(new_n155_));
  aoi22  g111(.a(new_n155_), .b(x32), .c(new_n148_), .d(new_n90_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n145_), .c(new_n129_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x29), .O(new_n158_));
  nand2  g114(.a(x32), .b(new_n45_), .O(new_n159_));
  nand2  g115(.a(new_n80_), .b(x00), .O(new_n160_));
  aoi21  g116(.a(new_n159_), .b(new_n158_), .c(new_n160_), .O(z8));
  aoi21  g117(.a(new_n154_), .b(new_n133_), .c(new_n155_), .O(new_n162_));
  nor2   g118(.a(new_n162_), .b(new_n46_), .O(new_n163_));
  nor3   g119(.a(new_n90_), .b(new_n45_), .c(new_n99_), .O(new_n164_));
  oai21  g120(.a(new_n91_), .b(x31), .c(x29), .O(new_n165_));
  oai22  g121(.a(new_n165_), .b(new_n164_), .c(x31), .d(new_n99_), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n163_), .c(x33), .O(new_n167_));
  inv1   g123(.a(new_n141_), .O(new_n168_));
  nand2  g124(.a(new_n164_), .b(x29), .O(new_n169_));
  nor2   g125(.a(x31), .b(x30), .O(new_n170_));
  oai21  g126(.a(new_n90_), .b(new_n94_), .c(new_n170_), .O(new_n171_));
  aoi21  g127(.a(new_n171_), .b(new_n169_), .c(x33), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n168_), .O(new_n173_));
  aoi21  g129(.a(new_n173_), .b(new_n167_), .c(new_n160_), .O(z9));
endmodule


