// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_;
  inv1   g000(.a(x06), .O(new_n45_));
  nor2   g001(.a(x18), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(x03), .O(new_n49_));
  inv1   g005(.a(x33), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  aoi21  g007(.a(x32), .b(new_n49_), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  inv1   g009(.a(x32), .O(new_n54_));
  aoi21  g010(.a(x33), .b(new_n49_), .c(x02), .O(new_n55_));
  oai21  g011(.a(new_n54_), .b(new_n49_), .c(new_n55_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n53_), .c(new_n46_), .O(z1));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n49_), .O(new_n59_));
  xnor2a g015(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g016(.a(new_n60_), .b(x06), .c(new_n51_), .O(new_n61_));
  aoi21  g017(.a(x06), .b(new_n51_), .c(new_n60_), .O(new_n62_));
  nor2   g018(.a(new_n62_), .b(x01), .O(new_n63_));
  inv1   g019(.a(x09), .O(new_n64_));
  inv1   g020(.a(new_n58_), .O(new_n65_));
  xor2a  g021(.a(x03), .b(x02), .O(new_n66_));
  inv1   g022(.a(x01), .O(new_n67_));
  aoi21  g023(.a(new_n66_), .b(new_n65_), .c(new_n67_), .O(new_n68_));
  oai21  g024(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  nand4  g025(.a(new_n69_), .b(new_n47_), .c(new_n64_), .d(x08), .O(new_n70_));
  aoi21  g026(.a(new_n63_), .b(new_n61_), .c(new_n70_), .O(z2));
  inv1   g027(.a(x27), .O(new_n72_));
  nand2  g028(.a(x03), .b(x02), .O(new_n73_));
  nor2   g029(.a(x20), .b(x15), .O(new_n74_));
  nor2   g030(.a(x21), .b(x16), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g032(.a(x22), .b(x17), .O(new_n77_));
  nor2   g033(.a(x24), .b(x19), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nor2   g036(.a(x23), .b(x18), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n46_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n73_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand2  g041(.a(x25), .b(x01), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(x26), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n85_), .c(new_n72_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(z3));
  inv1   g045(.a(x28), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n72_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nand2  g048(.a(new_n90_), .b(new_n72_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n92_), .c(new_n87_), .d(new_n85_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z4));
  nand2  g051(.a(new_n87_), .b(new_n73_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(new_n97_));
  nand2  g053(.a(new_n91_), .b(x29), .O(new_n98_));
  inv1   g054(.a(x29), .O(new_n99_));
  nand2  g055(.a(new_n92_), .b(new_n99_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n82_), .c(new_n47_), .O(z5));
  inv1   g058(.a(x30), .O(new_n103_));
  nand2  g059(.a(new_n98_), .b(new_n103_), .O(new_n104_));
  nand3  g060(.a(new_n91_), .b(x30), .c(x29), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n104_), .c(new_n97_), .d(new_n83_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(z6));
  aoi21  g063(.a(x23), .b(new_n45_), .c(x18), .O(new_n108_));
  nor2   g064(.a(new_n105_), .b(x31), .O(new_n109_));
  nand2  g065(.a(new_n105_), .b(x31), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n97_), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n109_), .c(new_n47_), .O(new_n112_));
  oai21  g068(.a(new_n108_), .b(new_n80_), .c(new_n112_), .O(z7));
  inv1   g069(.a(x18), .O(new_n114_));
  nand3  g070(.a(x17), .b(x16), .c(x15), .O(new_n115_));
  nand2  g071(.a(x19), .b(x13), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(new_n117_));
  inv1   g073(.a(x31), .O(new_n118_));
  oai21  g074(.a(x28), .b(x27), .c(x29), .O(new_n119_));
  xor2a  g075(.a(new_n119_), .b(x30), .O(new_n120_));
  nor2   g076(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n45_), .O(new_n123_));
  inv1   g079(.a(new_n115_), .O(new_n124_));
  inv1   g080(.a(x15), .O(new_n125_));
  inv1   g081(.a(x17), .O(new_n126_));
  nand2  g082(.a(x16), .b(x10), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n126_), .c(new_n125_), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(x19), .c(new_n124_), .O(new_n129_));
  inv1   g085(.a(x19), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(x14), .c(new_n115_), .O(new_n131_));
  nand2  g087(.a(x16), .b(x12), .O(new_n132_));
  oai21  g088(.a(x16), .b(x11), .c(x17), .O(new_n133_));
  aoi21  g089(.a(new_n133_), .b(new_n132_), .c(new_n131_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n121_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n129_), .c(x18), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n123_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(x32), .O(new_n138_));
  nand2  g094(.a(x18), .b(x17), .O(new_n139_));
  nor4   g095(.a(new_n139_), .b(new_n127_), .c(new_n130_), .d(x15), .O(new_n140_));
  nand3  g096(.a(x18), .b(x17), .c(x11), .O(new_n141_));
  nor3   g097(.a(new_n141_), .b(new_n130_), .c(x16), .O(new_n142_));
  inv1   g098(.a(new_n142_), .O(new_n143_));
  nand4  g099(.a(x19), .b(x18), .c(new_n126_), .d(x12), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  nand3  g101(.a(new_n130_), .b(x18), .c(x14), .O(new_n146_));
  nand3  g102(.a(x19), .b(new_n114_), .c(x13), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n146_), .c(new_n126_), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n145_), .c(x16), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n143_), .c(new_n125_), .O(new_n150_));
  nand3  g106(.a(new_n146_), .b(new_n144_), .c(x06), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  nor2   g108(.a(new_n142_), .b(new_n140_), .O(new_n153_));
  nand2  g109(.a(new_n121_), .b(new_n54_), .O(new_n154_));
  aoi21  g110(.a(new_n153_), .b(new_n152_), .c(new_n154_), .O(new_n155_));
  oai21  g111(.a(new_n150_), .b(new_n140_), .c(new_n155_), .O(new_n156_));
  inv1   g112(.a(x26), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x00), .O(new_n158_));
  aoi21  g114(.a(new_n156_), .b(new_n138_), .c(new_n158_), .O(z8));
  nand2  g115(.a(new_n149_), .b(new_n143_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(x15), .c(new_n140_), .O(new_n161_));
  xor2a  g117(.a(x31), .b(x30), .O(new_n162_));
  inv1   g118(.a(new_n162_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n120_), .O(new_n164_));
  oai21  g120(.a(new_n164_), .b(new_n161_), .c(new_n50_), .O(new_n165_));
  nand2  g121(.a(x16), .b(x15), .O(new_n166_));
  oai21  g122(.a(new_n139_), .b(new_n166_), .c(new_n130_), .O(new_n167_));
  inv1   g123(.a(x16), .O(new_n168_));
  nand2  g124(.a(new_n141_), .b(new_n168_), .O(new_n169_));
  oai21  g125(.a(new_n139_), .b(new_n127_), .c(new_n125_), .O(new_n170_));
  nand3  g126(.a(new_n170_), .b(new_n169_), .c(new_n167_), .O(new_n171_));
  nand2  g127(.a(new_n131_), .b(x18), .O(new_n172_));
  nand2  g128(.a(x18), .b(x12), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n126_), .O(new_n174_));
  oai21  g130(.a(x18), .b(x13), .c(x33), .O(new_n175_));
  nor2   g131(.a(new_n175_), .b(new_n162_), .O(new_n176_));
  nand4  g132(.a(new_n176_), .b(new_n174_), .c(new_n172_), .d(new_n120_), .O(new_n177_));
  nor2   g133(.a(new_n177_), .b(new_n171_), .O(new_n178_));
  nor2   g134(.a(new_n178_), .b(new_n158_), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n165_), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n47_), .O(z9));
endmodule


