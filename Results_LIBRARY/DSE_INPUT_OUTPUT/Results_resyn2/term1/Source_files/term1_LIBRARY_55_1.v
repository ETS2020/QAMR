// Benchmark "FAU" written by ABC on Tue Aug 11 21:13:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_;
  inv1   g000(.a(x23), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x18), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nand2  g003(.a(x03), .b(x02), .O(new_n48_));
  inv1   g004(.a(x02), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  and2   g007(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g008(.a(new_n52_), .O(new_n53_));
  inv1   g009(.a(x33), .O(new_n54_));
  aoi21  g010(.a(new_n53_), .b(new_n54_), .c(new_n46_), .O(new_n55_));
  oai21  g011(.a(new_n53_), .b(x32), .c(new_n55_), .O(z1));
  inv1   g012(.a(new_n46_), .O(new_n57_));
  xor2a  g013(.a(x07), .b(x04), .O(new_n58_));
  nand2  g014(.a(x05), .b(new_n50_), .O(new_n59_));
  nand2  g015(.a(x06), .b(new_n49_), .O(new_n60_));
  xor2a  g016(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(x01), .O(new_n62_));
  aoi21  g018(.a(new_n52_), .b(x01), .c(new_n62_), .O(new_n63_));
  nor2   g019(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  inv1   g020(.a(x09), .O(new_n65_));
  nand2  g021(.a(new_n63_), .b(new_n58_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(new_n65_), .c(x08), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n64_), .c(new_n57_), .O(z2));
  inv1   g024(.a(x27), .O(new_n69_));
  inv1   g025(.a(x18), .O(new_n70_));
  inv1   g026(.a(x19), .O(new_n71_));
  inv1   g027(.a(x24), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(new_n73_));
  nor2   g029(.a(x22), .b(x17), .O(new_n74_));
  oai22  g030(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g032(.a(x26), .O(new_n77_));
  nand4  g033(.a(new_n48_), .b(new_n77_), .c(x25), .d(x01), .O(new_n78_));
  aoi21  g034(.a(new_n76_), .b(new_n73_), .c(new_n78_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n69_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n57_), .O(z3));
  inv1   g037(.a(new_n78_), .O(new_n82_));
  nand2  g038(.a(new_n76_), .b(new_n73_), .O(new_n83_));
  xor2a  g039(.a(new_n83_), .b(new_n46_), .O(new_n84_));
  inv1   g040(.a(x28), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n69_), .O(new_n86_));
  nor2   g042(.a(new_n85_), .b(new_n69_), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n86_), .c(new_n84_), .d(new_n82_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(z4));
  nand2  g046(.a(new_n87_), .b(x29), .O(new_n91_));
  inv1   g047(.a(x29), .O(new_n92_));
  nand2  g048(.a(new_n88_), .b(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n91_), .c(new_n84_), .d(new_n82_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z5));
  inv1   g051(.a(x30), .O(new_n96_));
  xor2a  g052(.a(new_n91_), .b(new_n96_), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n79_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n57_), .O(z6));
  inv1   g055(.a(new_n91_), .O(new_n100_));
  inv1   g056(.a(x31), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n96_), .O(new_n102_));
  nor2   g058(.a(x31), .b(x30), .O(new_n103_));
  nor2   g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nor2   g060(.a(new_n100_), .b(new_n101_), .O(new_n105_));
  aoi21  g061(.a(new_n104_), .b(new_n100_), .c(new_n105_), .O(new_n106_));
  aoi22  g062(.a(new_n106_), .b(new_n79_), .c(new_n83_), .d(new_n46_), .O(z7));
  inv1   g063(.a(x32), .O(new_n108_));
  aoi21  g064(.a(new_n85_), .b(new_n69_), .c(new_n92_), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(x30), .c(x31), .O(new_n110_));
  aoi21  g066(.a(new_n109_), .b(x30), .c(new_n110_), .O(new_n111_));
  inv1   g067(.a(x15), .O(new_n112_));
  inv1   g068(.a(x17), .O(new_n113_));
  nand3  g069(.a(new_n71_), .b(x18), .c(x14), .O(new_n114_));
  nand4  g070(.a(new_n45_), .b(x19), .c(new_n70_), .d(x13), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  nand4  g072(.a(x19), .b(x18), .c(new_n113_), .d(x12), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n116_), .c(x16), .O(new_n119_));
  inv1   g075(.a(x16), .O(new_n120_));
  nand4  g076(.a(x19), .b(x17), .c(new_n120_), .d(x11), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x18), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(new_n119_), .c(new_n112_), .O(new_n124_));
  nor2   g080(.a(new_n113_), .b(new_n120_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x10), .O(new_n126_));
  nor4   g082(.a(new_n126_), .b(new_n71_), .c(new_n70_), .d(x15), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n124_), .c(new_n111_), .O(new_n128_));
  nand2  g084(.a(new_n77_), .b(x00), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  inv1   g086(.a(new_n126_), .O(new_n131_));
  nand2  g087(.a(x17), .b(x11), .O(new_n132_));
  oai21  g088(.a(x17), .b(x12), .c(x16), .O(new_n133_));
  aoi21  g089(.a(new_n133_), .b(new_n132_), .c(new_n112_), .O(new_n134_));
  nand2  g090(.a(new_n71_), .b(x14), .O(new_n135_));
  nand3  g091(.a(x17), .b(x16), .c(x15), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  nor2   g093(.a(new_n137_), .b(x19), .O(new_n138_));
  aoi21  g094(.a(new_n137_), .b(new_n135_), .c(new_n138_), .O(new_n139_));
  oai21  g095(.a(new_n134_), .b(new_n131_), .c(new_n139_), .O(new_n140_));
  nand2  g096(.a(new_n137_), .b(x13), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n71_), .c(new_n70_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n111_), .O(new_n143_));
  aoi21  g099(.a(new_n140_), .b(x18), .c(new_n143_), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n46_), .c(x32), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n130_), .O(new_n146_));
  aoi21  g102(.a(new_n128_), .b(new_n108_), .c(new_n146_), .O(z8));
  inv1   g103(.a(new_n109_), .O(new_n148_));
  nand2  g104(.a(x33), .b(x30), .O(new_n149_));
  nand3  g105(.a(new_n54_), .b(x19), .c(new_n70_), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n137_), .c(new_n103_), .d(x13), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n149_), .c(x23), .O(new_n153_));
  nand2  g109(.a(new_n113_), .b(x12), .O(new_n154_));
  oai21  g110(.a(x19), .b(x17), .c(x16), .O(new_n155_));
  aoi21  g111(.a(new_n154_), .b(new_n135_), .c(new_n155_), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n122_), .c(x15), .O(new_n157_));
  nand4  g113(.a(new_n125_), .b(x19), .c(new_n112_), .d(x10), .O(new_n158_));
  aoi21  g114(.a(new_n158_), .b(new_n157_), .c(x33), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n103_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n149_), .c(new_n70_), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n153_), .c(new_n148_), .O(new_n162_));
  nor2   g118(.a(new_n54_), .b(x30), .O(new_n163_));
  nand2  g119(.a(new_n151_), .b(new_n102_), .O(new_n164_));
  nor2   g120(.a(new_n164_), .b(new_n141_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n163_), .c(new_n45_), .O(new_n166_));
  aoi21  g122(.a(new_n159_), .b(new_n102_), .c(new_n163_), .O(new_n167_));
  oai21  g123(.a(new_n167_), .b(new_n70_), .c(new_n166_), .O(new_n168_));
  oai21  g124(.a(new_n140_), .b(new_n104_), .c(x18), .O(new_n169_));
  oai21  g125(.a(new_n103_), .b(new_n102_), .c(new_n142_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n45_), .O(new_n171_));
  aoi21  g127(.a(new_n171_), .b(new_n169_), .c(new_n54_), .O(new_n172_));
  aoi21  g128(.a(new_n168_), .b(new_n109_), .c(new_n172_), .O(new_n173_));
  aoi21  g129(.a(new_n173_), .b(new_n162_), .c(new_n129_), .O(z9));
endmodule


