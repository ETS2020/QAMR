// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n97_, new_n99_, new_n100_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n166_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n179_, new_n180_;
  inv1   g000(.a(x20), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g013(.a(x40), .b(x39), .c(new_n63_), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(z08));
  nor2   g015(.a(x15), .b(x04), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(z08), .c(new_n75_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x35), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(x28), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(x36), .c(x27), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g024(.a(x08), .O(new_n87_));
  nand2  g025(.a(new_n68_), .b(new_n87_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n82_), .O(new_n89_));
  and2   g027(.a(x29), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(x02), .c(x39), .O(new_n91_));
  aoi21  g029(.a(new_n89_), .b(x04), .c(new_n91_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x20), .c(x40), .O(z02));
  nand2  g031(.a(new_n70_), .b(x27), .O(new_n94_));
  aoi21  g032(.a(new_n69_), .b(new_n68_), .c(new_n65_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n94_), .c(new_n97_), .O(z04));
  inv1   g036(.a(new_n65_), .O(new_n99_));
  inv1   g037(.a(x28), .O(new_n100_));
  oai21  g038(.a(new_n100_), .b(new_n68_), .c(new_n69_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n99_), .O(z06));
  inv1   g040(.a(z06), .O(z05));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n99_), .O(z07));
  inv1   g052(.a(x11), .O(new_n115_));
  nand4  g053(.a(new_n99_), .b(x34), .c(x27), .d(x26), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(new_n115_), .O(z09));
  oai21  g055(.a(x39), .b(x20), .c(x40), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x07), .O(new_n119_));
  nand4  g057(.a(x40), .b(x39), .c(new_n63_), .d(x05), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  inv1   g059(.a(x36), .O(new_n122_));
  nand2  g060(.a(x35), .b(new_n100_), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(x04), .O(new_n124_));
  nand2  g062(.a(x37), .b(x06), .O(new_n125_));
  aoi21  g063(.a(x40), .b(x20), .c(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(new_n121_), .c(new_n126_), .O(new_n127_));
  nor2   g065(.a(new_n84_), .b(x04), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(z08), .c(x05), .O(new_n129_));
  oai21  g067(.a(new_n127_), .b(new_n68_), .c(new_n129_), .O(z10));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nand2  g069(.a(x27), .b(x04), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n88_), .c(new_n82_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand3  g072(.a(x39), .b(x29), .c(x08), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x40), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n134_), .c(new_n99_), .O(z11));
  inv1   g077(.a(x39), .O(new_n140_));
  nor2   g078(.a(new_n140_), .b(x04), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n75_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n63_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x40), .c(new_n71_), .O(z12));
  nand2  g082(.a(new_n70_), .b(new_n69_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(new_n145_), .c(new_n64_), .d(x20), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  nor2   g087(.a(x20), .b(x13), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n149_), .c(new_n141_), .d(x40), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  inv1   g091(.a(x13), .O(new_n154_));
  nand3  g092(.a(new_n128_), .b(z08), .c(new_n154_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n153_), .O(z13));
  nand3  g094(.a(new_n146_), .b(new_n145_), .c(x27), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n64_), .c(new_n63_), .O(new_n158_));
  nand2  g096(.a(new_n149_), .b(x27), .O(new_n159_));
  nand3  g097(.a(new_n141_), .b(x40), .c(new_n154_), .O(new_n160_));
  aoi21  g098(.a(new_n159_), .b(new_n84_), .c(new_n160_), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(new_n158_), .O(z14));
  inv1   g100(.a(x12), .O(new_n163_));
  nor2   g101(.a(new_n116_), .b(new_n163_), .O(z15));
  inv1   g102(.a(x23), .O(new_n165_));
  nand4  g103(.a(new_n99_), .b(new_n165_), .c(x22), .d(x01), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(z16));
  nand2  g105(.a(x22), .b(x01), .O(new_n168_));
  nor4   g106(.a(new_n168_), .b(new_n65_), .c(x24), .d(new_n165_), .O(z17));
  aoi21  g107(.a(new_n68_), .b(new_n87_), .c(new_n123_), .O(new_n170_));
  inv1   g108(.a(new_n131_), .O(new_n171_));
  aoi21  g109(.a(new_n132_), .b(new_n170_), .c(new_n171_), .O(new_n172_));
  aoi21  g110(.a(new_n137_), .b(new_n172_), .c(new_n65_), .O(z18));
  oai21  g111(.a(new_n136_), .b(x20), .c(x40), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n133_), .c(new_n131_), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z19));
  nand3  g114(.a(x40), .b(x39), .c(x29), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n123_), .c(new_n87_), .O(new_n178_));
  nand2  g116(.a(x35), .b(x27), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n131_), .c(new_n99_), .O(new_n180_));
  nor2   g118(.a(new_n180_), .b(new_n178_), .O(z20));
endmodule


