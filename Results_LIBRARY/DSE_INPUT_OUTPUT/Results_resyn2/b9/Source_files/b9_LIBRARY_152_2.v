// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n164_, new_n166_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x36), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  nor2   g011(.a(x36), .b(x35), .O(new_n74_));
  oai21  g012(.a(new_n74_), .b(new_n68_), .c(new_n73_), .O(new_n75_));
  inv1   g013(.a(x04), .O(new_n76_));
  nand3  g014(.a(x40), .b(x39), .c(new_n76_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n75_), .c(new_n66_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x35), .O(new_n83_));
  inv1   g021(.a(x36), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(x28), .c(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n82_), .c(new_n81_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n64_), .c(x04), .O(z01));
  inv1   g026(.a(x28), .O(new_n89_));
  nand2  g027(.a(x35), .b(new_n89_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(x04), .O(new_n92_));
  inv1   g030(.a(x02), .O(new_n93_));
  nand2  g031(.a(x40), .b(x39), .O(new_n94_));
  nand2  g032(.a(x29), .b(x08), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n93_), .c(new_n94_), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n92_), .O(new_n97_));
  xor2a  g035(.a(new_n97_), .b(new_n65_), .O(z02));
  nand2  g036(.a(new_n70_), .b(x27), .O(new_n99_));
  nand2  g037(.a(new_n69_), .b(new_n68_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n99_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g039(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n99_), .c(new_n65_), .O(z04));
  oai21  g041(.a(new_n89_), .b(new_n68_), .c(new_n69_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n64_), .O(z06));
  inv1   g043(.a(z06), .O(z05));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n66_), .O(new_n108_));
  nor2   g046(.a(x33), .b(x31), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  nand2  g050(.a(x25), .b(new_n107_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n64_), .c(x03), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n110_), .O(z07));
  inv1   g054(.a(new_n94_), .O(z08));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n64_), .O(z09));
  inv1   g057(.a(x07), .O(new_n120_));
  inv1   g058(.a(x39), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n120_), .c(x04), .O(new_n122_));
  oai21  g060(.a(x36), .b(new_n120_), .c(new_n63_), .O(new_n123_));
  inv1   g061(.a(x05), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(new_n124_), .O(new_n125_));
  nand4  g063(.a(new_n125_), .b(new_n123_), .c(new_n122_), .d(new_n85_), .O(new_n126_));
  nand3  g064(.a(new_n64_), .b(x37), .c(x06), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x27), .O(new_n129_));
  aoi21  g067(.a(new_n82_), .b(new_n81_), .c(new_n94_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(x05), .c(new_n76_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(z10));
  inv1   g070(.a(x08), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n90_), .c(new_n133_), .O(new_n135_));
  nand2  g073(.a(new_n134_), .b(x27), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nor2   g075(.a(new_n83_), .b(new_n68_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n89_), .c(new_n76_), .O(new_n139_));
  nor2   g077(.a(x30), .b(x09), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n64_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n139_), .c(new_n137_), .O(z18));
  inv1   g081(.a(z18), .O(z11));
  aoi22  g082(.a(new_n74_), .b(new_n69_), .c(x40), .d(new_n68_), .O(new_n145_));
  nand2  g083(.a(new_n77_), .b(new_n64_), .O(new_n146_));
  oai22  g084(.a(new_n146_), .b(new_n71_), .c(new_n145_), .d(x10), .O(z12));
  nand2  g085(.a(new_n70_), .b(new_n69_), .O(new_n148_));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(x20), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x28), .O(new_n152_));
  nor2   g090(.a(x13), .b(x04), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n152_), .c(z08), .O(new_n154_));
  oai21  g092(.a(new_n150_), .b(new_n65_), .c(new_n154_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x27), .O(new_n156_));
  nand2  g094(.a(new_n153_), .b(new_n130_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(z13));
  oai21  g096(.a(new_n63_), .b(x28), .c(new_n151_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n153_), .c(x39), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n150_), .O(new_n161_));
  nand2  g099(.a(new_n157_), .b(new_n64_), .O(new_n162_));
  aoi21  g100(.a(new_n161_), .b(x27), .c(new_n162_), .O(z14));
  nand4  g101(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n64_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(x23), .c(new_n64_), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x23), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n166_), .c(new_n64_), .O(z17));
  inv1   g108(.a(new_n95_), .O(new_n171_));
  inv1   g109(.a(new_n91_), .O(new_n172_));
  nand2  g110(.a(x27), .b(x04), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(new_n172_), .c(x35), .d(new_n89_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n140_), .O(new_n175_));
  aoi22  g113(.a(new_n175_), .b(new_n64_), .c(new_n171_), .d(z08), .O(z19));
  nor3   g114(.a(new_n141_), .b(new_n138_), .c(new_n135_), .O(z20));
endmodule


