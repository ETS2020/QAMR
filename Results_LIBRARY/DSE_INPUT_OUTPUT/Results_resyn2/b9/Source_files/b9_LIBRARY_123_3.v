// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n154_,
    new_n156_, new_n158_, new_n160_, new_n162_, new_n163_;
  nand2  g000(.a(x40), .b(x36), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x28), .O(new_n65_));
  inv1   g003(.a(x35), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  oai21  g005(.a(new_n66_), .b(new_n65_), .c(new_n67_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x27), .c(x10), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  and2   g009(.a(x40), .b(x39), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n73_), .b(new_n70_), .c(new_n69_), .O(new_n74_));
  aoi21  g012(.a(new_n69_), .b(x36), .c(x15), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g015(.a(x27), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n65_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nor2   g021(.a(new_n64_), .b(new_n71_), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n81_), .c(new_n84_), .O(z01));
  nor2   g023(.a(x27), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n80_), .c(x04), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nand3  g026(.a(x40), .b(x39), .c(new_n79_), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n87_), .O(z02));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n66_), .c(new_n67_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  aoi21  g033(.a(new_n93_), .b(new_n95_), .c(new_n64_), .O(z04));
  aoi21  g034(.a(new_n92_), .b(new_n67_), .c(new_n64_), .O(z05));
  nand3  g035(.a(new_n92_), .b(new_n63_), .c(new_n67_), .O(new_n98_));
  inv1   g036(.a(new_n98_), .O(z06));
  inv1   g037(.a(x14), .O(new_n100_));
  inv1   g038(.a(x15), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n101_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n100_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n63_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  inv1   g048(.a(new_n89_), .O(z08));
  nand2  g049(.a(x34), .b(x27), .O(new_n112_));
  nand2  g050(.a(x26), .b(x11), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n112_), .c(new_n63_), .O(z09));
  oai21  g052(.a(x39), .b(x36), .c(x40), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x07), .O(new_n116_));
  nand4  g054(.a(x40), .b(x39), .c(new_n79_), .d(x05), .O(new_n117_));
  aoi21  g055(.a(new_n117_), .b(new_n116_), .c(new_n80_), .O(new_n118_));
  nand2  g056(.a(x36), .b(x07), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(x40), .O(new_n120_));
  oai21  g058(.a(new_n120_), .b(new_n118_), .c(new_n71_), .O(new_n121_));
  nand3  g059(.a(new_n63_), .b(x37), .c(x06), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(x27), .O(new_n124_));
  inv1   g062(.a(new_n117_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n83_), .c(new_n71_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n124_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  nand2  g066(.a(new_n72_), .b(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n80_), .c(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  inv1   g070(.a(new_n80_), .O(new_n133_));
  nor2   g071(.a(new_n78_), .b(x04), .O(new_n134_));
  inv1   g072(.a(x09), .O(new_n135_));
  inv1   g073(.a(x30), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  aoi21  g075(.a(new_n134_), .b(new_n133_), .c(new_n137_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n132_), .c(new_n63_), .O(z18));
  inv1   g077(.a(z18), .O(z11));
  nand2  g078(.a(new_n74_), .b(new_n63_), .O(z12));
  nor2   g079(.a(x13), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n72_), .c(new_n65_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n68_), .c(x20), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n143_), .c(new_n78_), .O(new_n146_));
  oai21  g084(.a(new_n145_), .b(x40), .c(x36), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand3  g086(.a(new_n142_), .b(z08), .c(new_n83_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(z13));
  nand2  g088(.a(new_n142_), .b(new_n72_), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(new_n82_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n146_), .c(new_n63_), .O(z14));
  nand3  g091(.a(new_n63_), .b(x26), .c(x12), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n112_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(x23), .c(new_n63_), .O(z16));
  inv1   g095(.a(x23), .O(new_n158_));
  nor4   g096(.a(new_n156_), .b(new_n64_), .c(x24), .d(new_n158_), .O(z17));
  nand2  g097(.a(new_n138_), .b(new_n132_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n63_), .O(z19));
  nand2  g099(.a(x35), .b(x27), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n136_), .c(new_n135_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n130_), .c(new_n63_), .O(z20));
endmodule


