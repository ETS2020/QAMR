// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n152_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n161_;
  inv1   g000(.a(x02), .O(new_n63_));
  nor2   g001(.a(x14), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  and2   g012(.a(x40), .b(x39), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(new_n74_), .c(new_n71_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n70_), .c(new_n66_), .O(new_n77_));
  nand2  g015(.a(x40), .b(x39), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(x04), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x10), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n77_), .c(new_n65_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g021(.a(x28), .O(new_n84_));
  aoi21  g022(.a(x35), .b(new_n84_), .c(x36), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  oai21  g024(.a(new_n85_), .b(new_n66_), .c(new_n86_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n64_), .O(z01));
  nand2  g026(.a(x35), .b(new_n84_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n63_), .O(new_n93_));
  nor2   g031(.a(new_n78_), .b(new_n64_), .O(z08));
  nand3  g032(.a(z08), .b(new_n93_), .c(new_n91_), .O(z02));
  inv1   g033(.a(new_n64_), .O(new_n96_));
  nand2  g034(.a(new_n68_), .b(x27), .O(new_n97_));
  nand2  g035(.a(new_n67_), .b(new_n66_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(x21), .O(z03));
  aoi21  g037(.a(new_n67_), .b(new_n66_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n97_), .c(new_n64_), .O(z04));
  oai21  g039(.a(new_n84_), .b(new_n66_), .c(new_n67_), .O(new_n102_));
  and2   g040(.a(new_n102_), .b(new_n96_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  or2    g049(.a(x33), .b(x31), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n111_), .c(x14), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n109_), .c(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n96_), .O(z07));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n96_), .c(x11), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z09));
  nor2   g057(.a(new_n85_), .b(new_n66_), .O(new_n120_));
  nand2  g058(.a(new_n75_), .b(x05), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  and2   g060(.a(new_n78_), .b(x07), .O(new_n123_));
  aoi22  g061(.a(new_n123_), .b(new_n120_), .c(new_n122_), .d(new_n87_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n64_), .O(new_n127_));
  oai21  g065(.a(new_n124_), .b(x04), .c(new_n127_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(new_n75_), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n89_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand4  g070(.a(x35), .b(new_n84_), .c(x27), .d(new_n71_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n133_), .c(new_n96_), .O(new_n135_));
  aoi21  g073(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(z11));
  inv1   g074(.a(new_n77_), .O(new_n137_));
  nand3  g075(.a(new_n80_), .b(new_n137_), .c(new_n96_), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(z12));
  oai21  g077(.a(new_n73_), .b(new_n72_), .c(x28), .O(new_n140_));
  nor2   g078(.a(x13), .b(x04), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n75_), .O(new_n142_));
  nor2   g080(.a(x19), .b(x18), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n69_), .c(x20), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(new_n142_), .c(new_n66_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  nand2  g084(.a(new_n141_), .b(new_n75_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(new_n86_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n146_), .c(new_n64_), .O(z13));
  nor3   g088(.a(new_n148_), .b(new_n145_), .c(new_n64_), .O(z14));
  nand3  g089(.a(new_n117_), .b(new_n96_), .c(x12), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(z15));
  nand2  g091(.a(x22), .b(x01), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(x23), .c(new_n96_), .O(z16));
  inv1   g093(.a(x24), .O(new_n156_));
  inv1   g094(.a(new_n154_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(new_n96_), .c(new_n156_), .d(x23), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z17));
  inv1   g097(.a(z11), .O(z18));
  oai21  g098(.a(new_n72_), .b(new_n66_), .c(new_n134_), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n131_), .c(new_n96_), .O(z20));
  aoi21  g100(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(z19));
endmodule


