// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n154_, new_n156_, new_n157_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n165_, new_n166_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n64_), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(x10), .O(new_n70_));
  oai21  g008(.a(x36), .b(x35), .c(x27), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  nand3  g010(.a(x40), .b(x39), .c(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n71_), .b(new_n70_), .c(new_n73_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n64_), .O(new_n75_));
  oai21  g013(.a(new_n69_), .b(new_n63_), .c(new_n75_), .O(z00));
  inv1   g014(.a(x36), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n77_), .c(new_n66_), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  nor2   g020(.a(new_n63_), .b(new_n72_), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n80_), .c(new_n83_), .O(z01));
  inv1   g022(.a(x40), .O(new_n85_));
  xnor2a g023(.a(x40), .b(x39), .O(new_n86_));
  inv1   g024(.a(x08), .O(new_n87_));
  nand2  g025(.a(new_n66_), .b(new_n87_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(x35), .c(new_n78_), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  aoi21  g028(.a(new_n89_), .b(x04), .c(new_n90_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n85_), .c(new_n86_), .O(z02));
  inv1   g030(.a(x37), .O(new_n93_));
  nand3  g031(.a(x35), .b(x28), .c(x27), .O(new_n94_));
  oai21  g032(.a(new_n93_), .b(x27), .c(new_n94_), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n63_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n95_), .b(new_n97_), .c(new_n63_), .O(z04));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n63_), .O(z05));
  inv1   g038(.a(new_n99_), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n63_), .O(z06));
  inv1   g040(.a(new_n63_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n103_), .O(z07));
  inv1   g052(.a(x39), .O(new_n115_));
  nor2   g053(.a(new_n85_), .b(new_n115_), .O(z08));
  nand2  g054(.a(x34), .b(x27), .O(new_n117_));
  nand2  g055(.a(x26), .b(x11), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(new_n117_), .c(new_n103_), .O(z09));
  inv1   g057(.a(x07), .O(new_n120_));
  nand3  g058(.a(x40), .b(x39), .c(x05), .O(new_n121_));
  oai21  g059(.a(new_n86_), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  aoi21  g060(.a(new_n79_), .b(new_n77_), .c(x04), .O(new_n123_));
  nand2  g061(.a(x37), .b(x06), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n63_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(new_n122_), .c(new_n125_), .O(new_n126_));
  inv1   g064(.a(new_n121_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n82_), .c(new_n72_), .O(new_n128_));
  oai21  g066(.a(new_n126_), .b(new_n66_), .c(new_n128_), .O(z10));
  nand2  g067(.a(z08), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n79_), .c(new_n87_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand4  g070(.a(x35), .b(new_n78_), .c(x27), .d(new_n72_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n133_), .c(new_n103_), .O(new_n135_));
  aoi21  g073(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(z11));
  nor3   g074(.a(new_n74_), .b(new_n68_), .c(new_n63_), .O(z12));
  inv1   g075(.a(x13), .O(new_n138_));
  nor2   g076(.a(new_n115_), .b(x04), .O(new_n139_));
  nand2  g077(.a(x36), .b(x35), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x28), .O(new_n141_));
  nand4  g079(.a(new_n141_), .b(new_n139_), .c(x40), .d(new_n138_), .O(new_n142_));
  inv1   g080(.a(new_n67_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n143_), .c(x20), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  nand3  g085(.a(new_n139_), .b(x40), .c(new_n138_), .O(new_n148_));
  nor2   g086(.a(new_n148_), .b(new_n81_), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n147_), .c(new_n103_), .O(z13));
  aoi21  g089(.a(new_n145_), .b(new_n142_), .c(new_n66_), .O(new_n152_));
  oai21  g090(.a(new_n149_), .b(new_n152_), .c(new_n103_), .O(z14));
  nand2  g091(.a(x26), .b(x12), .O(new_n154_));
  nor3   g092(.a(new_n154_), .b(new_n117_), .c(new_n63_), .O(z15));
  inv1   g093(.a(x23), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(x22), .c(x01), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n103_), .O(z16));
  nor2   g096(.a(x24), .b(new_n156_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n103_), .c(x22), .d(x01), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z17));
  nand2  g099(.a(new_n132_), .b(new_n131_), .O(new_n162_));
  inv1   g100(.a(new_n135_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n162_), .O(z18));
  nand2  g102(.a(x35), .b(x27), .O(new_n165_));
  nand2  g103(.a(new_n134_), .b(new_n165_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n131_), .c(new_n103_), .O(z20));
  aoi21  g105(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(z19));
endmodule


