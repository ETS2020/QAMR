// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n98_, new_n99_, new_n100_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n171_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  and2   g012(.a(x40), .b(x39), .O(z08));
  nand4  g013(.a(z08), .b(new_n74_), .c(new_n64_), .d(new_n71_), .O(new_n76_));
  oai21  g014(.a(new_n70_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x36), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n78_), .c(new_n66_), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nor2   g021(.a(new_n63_), .b(new_n71_), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n81_), .c(new_n84_), .O(z01));
  nor2   g023(.a(x27), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n80_), .c(x04), .O(new_n87_));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(new_n87_), .c(z08), .O(z02));
  inv1   g029(.a(x35), .O(new_n92_));
  nand2  g030(.a(x28), .b(x27), .O(new_n93_));
  oai22  g031(.a(new_n93_), .b(new_n92_), .c(new_n67_), .d(x27), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(new_n63_), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n94_), .b(new_n96_), .c(new_n63_), .O(z04));
  inv1   g035(.a(x39), .O(new_n98_));
  inv1   g036(.a(x40), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g038(.a(new_n93_), .b(new_n100_), .c(new_n67_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n64_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n103_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n100_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  nand4  g050(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n100_), .O(z09));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  oai21  g053(.a(x40), .b(x39), .c(x07), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(z08), .c(new_n115_), .O(new_n117_));
  aoi21  g055(.a(new_n80_), .b(new_n78_), .c(x04), .O(new_n118_));
  nand2  g056(.a(x37), .b(x06), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n63_), .O(new_n120_));
  aoi21  g058(.a(new_n118_), .b(new_n117_), .c(new_n120_), .O(new_n121_));
  inv1   g059(.a(new_n115_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n83_), .c(new_n71_), .O(new_n123_));
  oai21  g061(.a(new_n121_), .b(new_n66_), .c(new_n123_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n80_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand4  g067(.a(x35), .b(new_n79_), .c(x27), .d(new_n71_), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(new_n129_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n100_), .O(z11));
  nand3  g071(.a(new_n67_), .b(new_n78_), .c(new_n92_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(x27), .c(x10), .O(new_n135_));
  nor2   g073(.a(new_n99_), .b(x04), .O(new_n136_));
  nor2   g074(.a(new_n136_), .b(new_n69_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n135_), .c(x39), .O(new_n138_));
  inv1   g076(.a(new_n69_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(x40), .c(new_n98_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n138_), .O(z12));
  inv1   g079(.a(x13), .O(new_n142_));
  nand2  g080(.a(x36), .b(x35), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x28), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(new_n136_), .c(x39), .d(new_n142_), .O(new_n145_));
  nand2  g083(.a(new_n68_), .b(new_n67_), .O(new_n146_));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(new_n146_), .c(new_n100_), .d(x20), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  nand3  g088(.a(x40), .b(new_n142_), .c(new_n71_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n83_), .c(x39), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n150_), .O(z13));
  nand3  g092(.a(new_n143_), .b(new_n82_), .c(x28), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n152_), .c(x39), .O(new_n156_));
  aoi21  g094(.a(new_n147_), .b(x20), .c(new_n63_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  oai21  g096(.a(x28), .b(new_n66_), .c(new_n82_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n152_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n139_), .c(x39), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n158_), .c(new_n140_), .O(z14));
  nand4  g100(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n100_), .O(z15));
  nand2  g102(.a(x22), .b(x01), .O(new_n165_));
  nor3   g103(.a(new_n165_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x23), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n165_), .c(new_n100_), .O(z17));
  nand4  g107(.a(new_n131_), .b(new_n130_), .c(new_n129_), .d(new_n100_), .O(z18));
  oai21  g108(.a(new_n92_), .b(new_n66_), .c(new_n131_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n127_), .c(new_n100_), .O(z20));
  nand2  g110(.a(new_n132_), .b(new_n100_), .O(z19));
endmodule


