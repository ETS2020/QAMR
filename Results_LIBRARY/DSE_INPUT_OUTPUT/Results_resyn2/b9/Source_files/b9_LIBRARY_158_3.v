// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n174_, new_n176_, new_n179_, new_n180_, new_n181_, new_n182_;
  inv1   g000(.a(x14), .O(new_n63_));
  inv1   g001(.a(x39), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g013(.a(x14), .b(x04), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .d(new_n66_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g026(.a(x04), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n83_), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand3  g031(.a(x40), .b(x39), .c(new_n63_), .O(new_n94_));
  aoi21  g032(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(new_n95_));
  oai21  g033(.a(new_n91_), .b(new_n89_), .c(new_n95_), .O(z02));
  inv1   g034(.a(new_n65_), .O(new_n97_));
  nand2  g035(.a(new_n70_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n69_), .b(new_n68_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n65_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n97_), .c(new_n69_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n63_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  or2    g049(.a(x33), .b(x31), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n111_), .c(x14), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n109_), .c(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n97_), .O(z07));
  inv1   g053(.a(new_n94_), .O(z08));
  nand2  g054(.a(x34), .b(x27), .O(new_n117_));
  nand2  g055(.a(x26), .b(x11), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(new_n117_), .c(new_n97_), .O(z09));
  inv1   g057(.a(new_n86_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n78_), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  nand2  g060(.a(new_n77_), .b(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n84_), .c(x27), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  aoi21  g064(.a(new_n78_), .b(new_n126_), .c(x04), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n97_), .O(z10));
  inv1   g068(.a(x40), .O(new_n131_));
  oai21  g069(.a(new_n93_), .b(new_n131_), .c(new_n63_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x39), .O(new_n133_));
  inv1   g071(.a(x09), .O(new_n134_));
  inv1   g072(.a(x30), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g074(.a(x27), .b(x04), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n91_), .c(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n133_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z11));
  inv1   g078(.a(x35), .O(new_n141_));
  nand3  g079(.a(new_n69_), .b(new_n81_), .c(new_n141_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  aoi21  g081(.a(x40), .b(new_n89_), .c(new_n71_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(new_n63_), .O(new_n145_));
  inv1   g083(.a(new_n71_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n64_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(z12));
  inv1   g086(.a(x13), .O(new_n149_));
  nand4  g087(.a(new_n120_), .b(new_n78_), .c(new_n76_), .d(new_n149_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  nand3  g090(.a(x20), .b(new_n152_), .c(new_n151_), .O(new_n153_));
  aoi21  g091(.a(new_n70_), .b(new_n69_), .c(new_n153_), .O(new_n154_));
  nand2  g092(.a(x36), .b(x35), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x28), .O(new_n156_));
  nand2  g094(.a(x39), .b(new_n63_), .O(new_n157_));
  nand3  g095(.a(x40), .b(new_n149_), .c(new_n89_), .O(new_n158_));
  nor2   g096(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n156_), .c(new_n154_), .O(new_n160_));
  nand2  g098(.a(new_n97_), .b(x27), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n160_), .c(new_n150_), .O(z13));
  inv1   g100(.a(new_n158_), .O(new_n163_));
  nand3  g101(.a(new_n155_), .b(new_n86_), .c(x28), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n163_), .c(x14), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n64_), .c(new_n153_), .O(new_n166_));
  nand2  g104(.a(new_n82_), .b(x27), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n86_), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n163_), .c(x14), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n64_), .c(new_n146_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n166_), .O(z14));
  nand2  g109(.a(x26), .b(x12), .O(new_n172_));
  nor3   g110(.a(new_n172_), .b(new_n117_), .c(new_n65_), .O(z15));
  nand2  g111(.a(x22), .b(x01), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(x23), .c(new_n97_), .O(z16));
  inv1   g113(.a(x23), .O(new_n176_));
  nor4   g114(.a(new_n174_), .b(new_n65_), .c(x24), .d(new_n176_), .O(z17));
  oai22  g115(.a(new_n138_), .b(new_n65_), .c(new_n94_), .d(new_n93_), .O(z18));
  inv1   g116(.a(x29), .O(new_n179_));
  oai21  g117(.a(new_n77_), .b(new_n179_), .c(new_n83_), .O(new_n180_));
  nand2  g118(.a(x35), .b(x27), .O(new_n181_));
  nand4  g119(.a(new_n181_), .b(new_n97_), .c(new_n135_), .d(new_n134_), .O(new_n182_));
  aoi21  g120(.a(new_n180_), .b(x08), .c(new_n182_), .O(z20));
  inv1   g121(.a(new_n139_), .O(z19));
endmodule


