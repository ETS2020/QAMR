// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n180_, new_n181_,
    new_n182_, new_n184_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x40), .b(x21), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  and2   g015(.a(new_n77_), .b(x27), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  nor2   g018(.a(new_n72_), .b(new_n64_), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(new_n78_), .c(new_n81_), .O(z01));
  inv1   g020(.a(x40), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x21), .O(new_n84_));
  nor2   g022(.a(new_n75_), .b(x28), .O(new_n85_));
  inv1   g023(.a(x08), .O(new_n86_));
  inv1   g024(.a(x27), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  and2   g027(.a(x29), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(x02), .c(x39), .O(new_n91_));
  aoi21  g029(.a(new_n89_), .b(x04), .c(new_n91_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n83_), .c(new_n84_), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n75_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n72_), .O(z03));
  nor2   g035(.a(new_n83_), .b(x21), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n96_), .O(z04));
  inv1   g037(.a(new_n72_), .O(new_n100_));
  nand3  g038(.a(new_n95_), .b(new_n100_), .c(new_n94_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x14), .O(new_n103_));
  inv1   g041(.a(x15), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n104_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n103_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n100_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  inv1   g051(.a(x39), .O(new_n114_));
  nor2   g052(.a(new_n83_), .b(new_n114_), .O(z08));
  inv1   g053(.a(x11), .O(new_n116_));
  nand3  g054(.a(x34), .b(x27), .c(x26), .O(new_n117_));
  nor3   g055(.a(new_n117_), .b(new_n72_), .c(new_n116_), .O(z09));
  nand2  g056(.a(z08), .b(new_n80_), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  oai21  g058(.a(new_n83_), .b(new_n114_), .c(new_n120_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n77_), .c(x27), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n119_), .O(new_n123_));
  inv1   g061(.a(x05), .O(new_n124_));
  aoi21  g062(.a(z08), .b(new_n124_), .c(x04), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n100_), .O(z10));
  nor2   g066(.a(x30), .b(x09), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  nand3  g068(.a(x40), .b(x39), .c(x29), .O(new_n131_));
  oai21  g069(.a(new_n75_), .b(x28), .c(new_n131_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x08), .O(new_n133_));
  nand3  g071(.a(new_n85_), .b(x27), .c(new_n64_), .O(new_n134_));
  nand3  g072(.a(new_n131_), .b(x27), .c(x04), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  aoi21  g074(.a(new_n134_), .b(new_n133_), .c(new_n136_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n130_), .c(new_n100_), .O(z11));
  nand2  g076(.a(new_n63_), .b(new_n94_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(x27), .c(x10), .O(new_n140_));
  nor2   g078(.a(new_n114_), .b(x04), .O(new_n141_));
  inv1   g079(.a(x28), .O(new_n142_));
  oai21  g080(.a(new_n75_), .b(new_n142_), .c(new_n94_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(new_n141_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n140_), .c(x40), .O(new_n145_));
  nand2  g083(.a(new_n143_), .b(x27), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n83_), .c(x21), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(z12));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x28), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(new_n141_), .c(x40), .d(new_n149_), .O(new_n152_));
  nor2   g090(.a(x19), .b(x18), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(new_n100_), .c(new_n143_), .d(x20), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x27), .O(new_n156_));
  nor2   g094(.a(x13), .b(x04), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(z08), .c(new_n80_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n156_), .O(z13));
  nand2  g097(.a(new_n153_), .b(x20), .O(new_n160_));
  nand3  g098(.a(new_n150_), .b(new_n79_), .c(x28), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n141_), .c(x40), .d(new_n149_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  oai21  g101(.a(x28), .b(new_n87_), .c(new_n79_), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n157_), .c(z08), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n146_), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n163_), .c(new_n72_), .O(z14));
  inv1   g105(.a(x12), .O(new_n168_));
  oai21  g106(.a(new_n117_), .b(new_n168_), .c(new_n100_), .O(z15));
  nand2  g107(.a(x22), .b(x01), .O(new_n170_));
  nor3   g108(.a(new_n170_), .b(new_n72_), .c(x23), .O(z16));
  inv1   g109(.a(x24), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x23), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n170_), .c(new_n100_), .O(z17));
  nor2   g112(.a(new_n87_), .b(new_n64_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n89_), .c(new_n129_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n100_), .O(new_n177_));
  nand2  g115(.a(z08), .b(new_n90_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n177_), .O(z18));
  inv1   g117(.a(new_n133_), .O(new_n180_));
  nand2  g118(.a(new_n131_), .b(x27), .O(new_n181_));
  nand3  g119(.a(new_n134_), .b(new_n129_), .c(new_n100_), .O(new_n182_));
  aoi21  g120(.a(new_n181_), .b(new_n180_), .c(new_n182_), .O(z19));
  oai21  g121(.a(new_n75_), .b(new_n87_), .c(new_n129_), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n180_), .c(new_n100_), .O(z20));
endmodule


