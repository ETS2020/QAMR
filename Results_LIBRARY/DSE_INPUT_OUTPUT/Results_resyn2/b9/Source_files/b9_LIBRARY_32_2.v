// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n165_, new_n167_,
    new_n169_, new_n170_, new_n172_, new_n174_, new_n175_;
  inv1   g000(.a(x37), .O(new_n63_));
  nand2  g001(.a(x35), .b(x28), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x40), .O(new_n68_));
  aoi21  g006(.a(x40), .b(x35), .c(x36), .O(new_n69_));
  oai22  g007(.a(new_n69_), .b(new_n67_), .c(new_n68_), .d(new_n66_), .O(new_n70_));
  inv1   g008(.a(x39), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(x04), .O(new_n72_));
  aoi22  g010(.a(new_n72_), .b(new_n70_), .c(new_n65_), .d(x27), .O(new_n73_));
  inv1   g011(.a(x16), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  nor2   g013(.a(x40), .b(new_n75_), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  oai21  g015(.a(new_n73_), .b(x15), .c(new_n77_), .O(z00));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n75_), .c(new_n67_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  inv1   g022(.a(x04), .O(new_n85_));
  nor2   g023(.a(new_n76_), .b(new_n85_), .O(new_n86_));
  oai21  g024(.a(new_n84_), .b(new_n81_), .c(new_n86_), .O(z01));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n80_), .c(x04), .O(new_n89_));
  nand2  g027(.a(x40), .b(x39), .O(new_n90_));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n89_), .c(new_n76_), .O(z02));
  nand2  g031(.a(new_n64_), .b(x27), .O(new_n94_));
  aoi21  g032(.a(new_n63_), .b(new_n67_), .c(new_n76_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n94_), .c(new_n97_), .O(z04));
  inv1   g036(.a(new_n76_), .O(new_n99_));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n99_), .c(new_n63_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n99_), .O(z07));
  inv1   g051(.a(new_n90_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  nor3   g054(.a(new_n116_), .b(new_n76_), .c(new_n115_), .O(z09));
  nand2  g055(.a(x40), .b(x35), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(x28), .c(new_n75_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(x39), .c(x05), .O(new_n120_));
  nand2  g058(.a(new_n71_), .b(x36), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n80_), .O(new_n122_));
  and2   g060(.a(new_n90_), .b(x07), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n120_), .c(new_n67_), .O(new_n125_));
  aoi21  g063(.a(new_n83_), .b(new_n82_), .c(new_n90_), .O(new_n126_));
  and2   g064(.a(new_n126_), .b(x05), .O(new_n127_));
  oai21  g065(.a(new_n127_), .b(new_n125_), .c(new_n85_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n76_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n128_), .O(z10));
  inv1   g070(.a(x08), .O(new_n133_));
  nand2  g071(.a(z08), .b(x29), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n80_), .c(new_n133_), .O(new_n135_));
  nand2  g073(.a(new_n134_), .b(x27), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  inv1   g075(.a(new_n80_), .O(new_n138_));
  nor2   g076(.a(new_n67_), .b(x04), .O(new_n139_));
  inv1   g077(.a(x09), .O(new_n140_));
  nand2  g078(.a(new_n82_), .b(new_n140_), .O(new_n141_));
  aoi21  g079(.a(new_n139_), .b(new_n138_), .c(new_n141_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n137_), .c(new_n99_), .O(z18));
  inv1   g081(.a(z18), .O(z11));
  nor3   g082(.a(x37), .b(x36), .c(x35), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n67_), .c(new_n66_), .O(new_n146_));
  nand2  g084(.a(new_n65_), .b(x27), .O(new_n147_));
  oai21  g085(.a(new_n90_), .b(x04), .c(new_n147_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n146_), .c(new_n99_), .O(z12));
  nor2   g087(.a(x19), .b(x18), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n65_), .c(x20), .O(new_n151_));
  nor2   g089(.a(x13), .b(x04), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x28), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  oai22  g093(.a(new_n155_), .b(new_n90_), .c(new_n151_), .d(new_n76_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nand2  g095(.a(new_n152_), .b(new_n126_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(z13));
  nand2  g097(.a(new_n153_), .b(new_n68_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x39), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n155_), .c(new_n151_), .O(new_n162_));
  nand2  g100(.a(new_n158_), .b(new_n99_), .O(new_n163_));
  aoi21  g101(.a(new_n162_), .b(x27), .c(new_n163_), .O(z14));
  inv1   g102(.a(x12), .O(new_n165_));
  oai21  g103(.a(new_n116_), .b(new_n165_), .c(new_n99_), .O(z15));
  nand2  g104(.a(x22), .b(x01), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(x23), .c(new_n99_), .O(z16));
  inv1   g106(.a(x24), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x23), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n167_), .c(new_n99_), .O(z17));
  nand2  g109(.a(new_n142_), .b(new_n137_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n99_), .O(z19));
  nand2  g111(.a(x35), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n82_), .c(new_n140_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n135_), .c(new_n99_), .O(z20));
endmodule


