// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n174_, new_n175_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  nor2   g008(.a(x40), .b(x36), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(new_n71_), .O(new_n74_));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  aoi21  g014(.a(x35), .b(new_n76_), .c(x36), .O(new_n77_));
  nor2   g015(.a(x32), .b(x30), .O(new_n78_));
  oai21  g016(.a(new_n77_), .b(new_n75_), .c(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n74_), .c(x04), .O(z01));
  inv1   g018(.a(x40), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x36), .O(new_n82_));
  inv1   g020(.a(x35), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(x28), .O(new_n84_));
  oai21  g022(.a(x27), .b(x08), .c(new_n84_), .O(new_n85_));
  and2   g023(.a(x29), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x02), .c(x39), .O(new_n87_));
  aoi21  g025(.a(new_n85_), .b(x04), .c(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n81_), .c(new_n82_), .O(z02));
  oai21  g027(.a(new_n83_), .b(new_n76_), .c(x27), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n75_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n74_), .d(x21), .O(z03));
  aoi21  g031(.a(new_n91_), .b(new_n75_), .c(x21), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n90_), .c(new_n71_), .O(z04));
  aoi21  g033(.a(x28), .b(x27), .c(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n74_), .O(z05));
  inv1   g035(.a(new_n96_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n74_), .O(z06));
  inv1   g037(.a(x14), .O(new_n100_));
  inv1   g038(.a(x15), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n101_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n100_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n74_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  nand2  g048(.a(x40), .b(x39), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n74_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  inv1   g053(.a(x39), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(x05), .O(new_n117_));
  oai21  g055(.a(new_n117_), .b(new_n77_), .c(x40), .O(new_n118_));
  inv1   g056(.a(x07), .O(new_n119_));
  aoi21  g057(.a(new_n111_), .b(new_n119_), .c(new_n75_), .O(new_n120_));
  inv1   g058(.a(x05), .O(new_n121_));
  nor3   g059(.a(new_n111_), .b(new_n78_), .c(new_n121_), .O(new_n122_));
  aoi21  g060(.a(new_n120_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n71_), .O(new_n126_));
  oai21  g064(.a(new_n123_), .b(x04), .c(new_n126_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  inv1   g066(.a(new_n84_), .O(new_n129_));
  nand2  g067(.a(z08), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n129_), .c(new_n128_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand3  g070(.a(new_n84_), .b(x27), .c(new_n63_), .O(new_n133_));
  nor3   g071(.a(new_n71_), .b(x30), .c(x09), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  aoi21  g073(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(z11));
  nand2  g074(.a(new_n66_), .b(new_n91_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(x27), .c(x10), .O(new_n138_));
  nor2   g076(.a(new_n67_), .b(new_n75_), .O(new_n139_));
  aoi21  g077(.a(x39), .b(new_n63_), .c(new_n139_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n138_), .c(x40), .O(new_n141_));
  or2    g079(.a(new_n139_), .b(new_n82_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n141_), .O(z12));
  aoi21  g081(.a(x36), .b(x35), .c(new_n76_), .O(new_n144_));
  nor2   g082(.a(x13), .b(x04), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(z08), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand3  g087(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n150_));
  nor2   g088(.a(new_n150_), .b(new_n67_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n147_), .c(x27), .O(new_n152_));
  nor2   g090(.a(new_n111_), .b(new_n78_), .O(new_n153_));
  aoi21  g091(.a(new_n145_), .b(new_n153_), .c(new_n71_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(z13));
  inv1   g093(.a(new_n78_), .O(new_n156_));
  inv1   g094(.a(new_n150_), .O(new_n157_));
  nand2  g095(.a(new_n91_), .b(new_n83_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n144_), .c(new_n75_), .O(new_n160_));
  nand2  g098(.a(new_n157_), .b(new_n139_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n146_), .c(new_n71_), .O(new_n162_));
  oai21  g100(.a(new_n160_), .b(new_n156_), .c(new_n162_), .O(z14));
  inv1   g101(.a(x12), .O(new_n164_));
  nor2   g102(.a(new_n114_), .b(new_n164_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  nor3   g104(.a(new_n166_), .b(new_n71_), .c(x23), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x23), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n166_), .c(new_n74_), .O(z17));
  nand2  g108(.a(new_n132_), .b(new_n131_), .O(new_n171_));
  inv1   g109(.a(new_n135_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n171_), .O(z18));
  nor2   g111(.a(x30), .b(x09), .O(new_n174_));
  oai21  g112(.a(new_n83_), .b(new_n75_), .c(new_n174_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n131_), .c(new_n74_), .O(z20));
  aoi21  g114(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(z19));
endmodule


