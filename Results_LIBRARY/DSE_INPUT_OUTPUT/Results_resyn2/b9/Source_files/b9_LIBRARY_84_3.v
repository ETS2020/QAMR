// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n171_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n180_, new_n181_,
    new_n182_, new_n184_, new_n185_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  and2   g005(.a(x40), .b(x39), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x10), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(new_n70_));
  aoi21  g008(.a(new_n67_), .b(x27), .c(new_n70_), .O(new_n71_));
  inv1   g009(.a(x16), .O(new_n72_));
  inv1   g010(.a(x23), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  oai21  g014(.a(new_n71_), .b(x15), .c(new_n76_), .O(z00));
  inv1   g015(.a(x36), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x27), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n75_), .O(z01));
  inv1   g023(.a(x08), .O(new_n86_));
  inv1   g024(.a(x27), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(new_n86_), .c(new_n80_), .O(new_n88_));
  nand2  g026(.a(x40), .b(x39), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor3   g028(.a(new_n90_), .b(new_n89_), .c(x23), .O(new_n91_));
  oai21  g029(.a(new_n88_), .b(new_n65_), .c(new_n91_), .O(z02));
  inv1   g030(.a(x35), .O(new_n93_));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n93_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n75_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  aoi21  g036(.a(new_n96_), .b(new_n98_), .c(new_n75_), .O(z04));
  inv1   g037(.a(new_n75_), .O(new_n100_));
  nand3  g038(.a(new_n95_), .b(new_n100_), .c(new_n94_), .O(z05));
  nand2  g039(.a(new_n95_), .b(new_n94_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n100_), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  inv1   g042(.a(x15), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n105_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n104_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n100_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nand3  g052(.a(x40), .b(x39), .c(new_n73_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n100_), .O(z09));
  inv1   g056(.a(new_n83_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n68_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(new_n89_), .b(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n81_), .c(x27), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  inv1   g062(.a(x05), .O(new_n125_));
  aoi21  g063(.a(new_n68_), .b(new_n125_), .c(x04), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n100_), .O(z10));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n86_), .c(x04), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n73_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x40), .O(new_n133_));
  nand3  g071(.a(x35), .b(new_n79_), .c(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n86_), .O(new_n135_));
  nand3  g073(.a(x40), .b(x39), .c(x29), .O(new_n136_));
  nor2   g074(.a(new_n87_), .b(new_n65_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n80_), .c(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n130_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n133_), .O(z11));
  nand2  g079(.a(new_n67_), .b(x27), .O(new_n142_));
  nand2  g080(.a(new_n69_), .b(new_n142_), .O(new_n143_));
  nand2  g081(.a(new_n100_), .b(new_n143_), .O(z12));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x20), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(new_n63_), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  inv1   g087(.a(x13), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n65_), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(new_n115_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n149_), .c(new_n147_), .O(new_n153_));
  nand2  g091(.a(new_n100_), .b(x27), .O(new_n154_));
  nor2   g092(.a(new_n151_), .b(x23), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n119_), .c(new_n68_), .O(new_n156_));
  oai21  g094(.a(new_n154_), .b(new_n153_), .c(new_n156_), .O(z13));
  nand3  g095(.a(new_n148_), .b(new_n83_), .c(x28), .O(new_n158_));
  nand3  g096(.a(x39), .b(new_n150_), .c(new_n65_), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n158_), .c(x23), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n74_), .c(new_n146_), .O(new_n162_));
  or2    g100(.a(new_n63_), .b(new_n87_), .O(new_n163_));
  nand2  g101(.a(new_n79_), .b(x27), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n83_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n160_), .c(x23), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n74_), .c(new_n163_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n162_), .O(z14));
  nand4  g106(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n100_), .O(z15));
  nand2  g108(.a(x22), .b(x01), .O(new_n171_));
  nor2   g109(.a(new_n171_), .b(x23), .O(z16));
  or2    g110(.a(new_n171_), .b(x24), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n74_), .c(new_n73_), .O(z17));
  inv1   g112(.a(new_n130_), .O(new_n175_));
  inv1   g113(.a(new_n137_), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n88_), .c(new_n175_), .O(new_n177_));
  nand3  g115(.a(z08), .b(x29), .c(x08), .O(new_n178_));
  oai21  g116(.a(new_n177_), .b(new_n75_), .c(new_n178_), .O(z18));
  oai21  g117(.a(new_n134_), .b(x04), .c(new_n86_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n138_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n130_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n100_), .O(z19));
  aoi21  g121(.a(new_n136_), .b(new_n80_), .c(new_n86_), .O(new_n184_));
  oai21  g122(.a(new_n93_), .b(new_n87_), .c(new_n130_), .O(new_n185_));
  oai21  g123(.a(new_n185_), .b(new_n184_), .c(new_n100_), .O(z20));
endmodule


