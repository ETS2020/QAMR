// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n155_, new_n157_, new_n158_, new_n160_, new_n161_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n168_, new_n169_;
  inv1   g000(.a(x08), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x16), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n67_), .c(new_n68_), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x04), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(new_n67_), .O(new_n79_));
  oai21  g017(.a(new_n73_), .b(new_n66_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n81_), .c(new_n69_), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  inv1   g023(.a(new_n85_), .O(new_n86_));
  inv1   g024(.a(x04), .O(new_n87_));
  nor2   g025(.a(new_n66_), .b(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n86_), .b(new_n84_), .c(new_n88_), .O(z01));
  aoi21  g027(.a(new_n69_), .b(new_n63_), .c(new_n83_), .O(new_n90_));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n77_), .O(new_n92_));
  oai21  g030(.a(new_n90_), .b(new_n87_), .c(new_n92_), .O(z02));
  inv1   g031(.a(x35), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n94_), .c(new_n70_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n66_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n65_), .c(new_n98_), .O(z04));
  nand3  g037(.a(new_n95_), .b(new_n65_), .c(new_n70_), .O(z05));
  nand2  g038(.a(new_n95_), .b(new_n70_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n65_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n65_), .O(z07));
  and2   g051(.a(x40), .b(x39), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n65_), .O(z09));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n85_), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  aoi21  g057(.a(x40), .b(x39), .c(x07), .O(new_n120_));
  aoi21  g058(.a(z08), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n84_), .c(new_n118_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  and2   g061(.a(new_n123_), .b(new_n65_), .O(new_n124_));
  oai21  g062(.a(new_n122_), .b(x04), .c(new_n124_), .O(z10));
  nand4  g063(.a(x35), .b(new_n82_), .c(x27), .d(new_n87_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n63_), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n83_), .O(new_n129_));
  nand3  g067(.a(new_n128_), .b(x27), .c(x04), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n127_), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n65_), .O(z11));
  nor3   g072(.a(x37), .b(x36), .c(x35), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n69_), .c(new_n74_), .O(new_n136_));
  or2    g074(.a(new_n78_), .b(new_n72_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n136_), .c(new_n65_), .O(z12));
  inv1   g076(.a(x13), .O(new_n139_));
  oai21  g077(.a(new_n81_), .b(new_n94_), .c(x28), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n78_), .c(new_n139_), .O(new_n141_));
  nand2  g079(.a(new_n71_), .b(new_n70_), .O(new_n142_));
  nor2   g080(.a(x19), .b(x18), .O(new_n143_));
  nand4  g081(.a(new_n143_), .b(new_n142_), .c(new_n65_), .d(x20), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x27), .O(new_n146_));
  nand3  g084(.a(new_n86_), .b(new_n78_), .c(new_n139_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(z13));
  nand3  g086(.a(new_n143_), .b(new_n142_), .c(x20), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n141_), .O(new_n150_));
  nand2  g088(.a(new_n147_), .b(new_n65_), .O(new_n151_));
  aoi21  g089(.a(new_n150_), .b(x27), .c(new_n151_), .O(z14));
  nand4  g090(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n65_), .O(z15));
  nand2  g092(.a(x22), .b(x01), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(x23), .c(new_n65_), .O(z16));
  inv1   g094(.a(x24), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x23), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n155_), .c(new_n65_), .O(z17));
  oai21  g097(.a(new_n69_), .b(new_n87_), .c(new_n90_), .O(new_n160_));
  nand3  g098(.a(z08), .b(x29), .c(x08), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n160_), .c(new_n132_), .d(new_n65_), .O(z18));
  inv1   g100(.a(new_n132_), .O(new_n163_));
  nand2  g101(.a(new_n130_), .b(new_n129_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x08), .O(new_n165_));
  nand3  g103(.a(new_n126_), .b(x40), .c(new_n63_), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n165_), .c(new_n163_), .O(z19));
  aoi21  g105(.a(new_n128_), .b(new_n83_), .c(new_n63_), .O(new_n168_));
  oai21  g106(.a(new_n94_), .b(new_n69_), .c(new_n132_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n168_), .c(new_n65_), .O(z20));
endmodule


