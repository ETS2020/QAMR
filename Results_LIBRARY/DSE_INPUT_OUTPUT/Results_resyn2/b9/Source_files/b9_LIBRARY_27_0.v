// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n174_,
    new_n176_, new_n177_, new_n178_, new_n180_, new_n182_, new_n183_;
  nor2   g000(.a(x36), .b(x19), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  nand4  g006(.a(x40), .b(x39), .c(x10), .d(new_n68_), .O(new_n69_));
  oai21  g007(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n64_), .c(new_n65_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  aoi21  g010(.a(x35), .b(x19), .c(x36), .O(new_n73_));
  nand2  g011(.a(x27), .b(new_n68_), .O(new_n74_));
  nor3   g012(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n64_), .O(new_n76_));
  oai21  g014(.a(new_n71_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x36), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n78_), .c(new_n66_), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nor2   g021(.a(new_n63_), .b(new_n68_), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n81_), .c(new_n84_), .O(z01));
  inv1   g023(.a(new_n63_), .O(new_n86_));
  inv1   g024(.a(new_n72_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n80_), .c(x04), .O(new_n89_));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(new_n87_), .d(new_n86_), .O(z02));
  inv1   g031(.a(x35), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  oai22  g034(.a(new_n96_), .b(new_n94_), .c(new_n95_), .d(x27), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n63_), .O(z03));
  nor2   g036(.a(new_n63_), .b(x21), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n97_), .O(z04));
  aoi21  g038(.a(new_n96_), .b(new_n95_), .c(new_n63_), .O(z05));
  nand2  g039(.a(new_n96_), .b(new_n95_), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n63_), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n64_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n104_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n86_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  nand2  g051(.a(new_n72_), .b(new_n86_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand4  g053(.a(new_n86_), .b(x34), .c(x27), .d(x26), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(new_n115_), .O(z09));
  nand2  g055(.a(new_n87_), .b(new_n68_), .O(new_n118_));
  nand3  g056(.a(x37), .b(x27), .c(x06), .O(new_n119_));
  nand2  g057(.a(new_n83_), .b(x05), .O(new_n120_));
  oai21  g058(.a(new_n120_), .b(new_n118_), .c(new_n119_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n86_), .O(new_n122_));
  inv1   g060(.a(x19), .O(new_n123_));
  oai21  g061(.a(new_n80_), .b(new_n123_), .c(new_n78_), .O(new_n124_));
  inv1   g062(.a(x07), .O(new_n125_));
  nand2  g063(.a(new_n72_), .b(new_n125_), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  aoi21  g065(.a(new_n87_), .b(new_n127_), .c(new_n74_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n126_), .c(new_n124_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n122_), .O(z10));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n80_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  inv1   g073(.a(new_n74_), .O(new_n136_));
  inv1   g074(.a(new_n80_), .O(new_n137_));
  inv1   g075(.a(x09), .O(new_n138_));
  inv1   g076(.a(x30), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  aoi21  g078(.a(new_n137_), .b(new_n136_), .c(new_n140_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n135_), .c(new_n86_), .O(z18));
  inv1   g080(.a(z18), .O(z11));
  aoi21  g081(.a(new_n70_), .b(new_n86_), .c(new_n75_), .O(z12));
  nand2  g082(.a(new_n79_), .b(x27), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n82_), .c(new_n63_), .O(new_n146_));
  nand3  g084(.a(x36), .b(x35), .c(x27), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  inv1   g086(.a(x13), .O(new_n149_));
  nand4  g087(.a(x40), .b(x39), .c(new_n149_), .d(new_n68_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  oai21  g089(.a(new_n148_), .b(new_n146_), .c(new_n151_), .O(new_n152_));
  nor2   g090(.a(new_n67_), .b(new_n66_), .O(new_n153_));
  inv1   g091(.a(x18), .O(new_n154_));
  nand3  g092(.a(x20), .b(new_n123_), .c(new_n154_), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n153_), .c(x36), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n152_), .O(z13));
  nand2  g096(.a(new_n151_), .b(new_n83_), .O(new_n159_));
  nor2   g097(.a(x37), .b(x28), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n155_), .c(new_n150_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x27), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n159_), .O(new_n163_));
  inv1   g101(.a(x32), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n139_), .c(x28), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x19), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n78_), .O(new_n167_));
  oai21  g105(.a(new_n165_), .b(new_n156_), .c(x37), .O(new_n168_));
  nand2  g106(.a(new_n165_), .b(new_n151_), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n168_), .c(new_n94_), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n167_), .c(new_n163_), .O(z14));
  inv1   g109(.a(x12), .O(new_n172_));
  nor2   g110(.a(new_n116_), .b(new_n172_), .O(z15));
  nand2  g111(.a(x22), .b(x01), .O(new_n174_));
  nor3   g112(.a(new_n174_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g113(.a(x24), .O(new_n176_));
  inv1   g114(.a(new_n174_), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n86_), .c(new_n176_), .d(x23), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z17));
  nand2  g117(.a(new_n141_), .b(new_n135_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n86_), .O(z19));
  nand2  g119(.a(x35), .b(x27), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n139_), .c(new_n138_), .O(new_n183_));
  oai21  g121(.a(new_n183_), .b(new_n133_), .c(new_n86_), .O(z20));
endmodule


