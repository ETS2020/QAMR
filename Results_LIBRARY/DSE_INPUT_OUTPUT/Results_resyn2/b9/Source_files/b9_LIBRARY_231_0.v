// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n154_,
    new_n156_, new_n158_, new_n160_, new_n161_, new_n163_, new_n165_;
  nor2   g000(.a(x22), .b(x10), .O(new_n63_));
  inv1   g001(.a(x37), .O(new_n64_));
  nand2  g002(.a(x35), .b(x28), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(x27), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(x04), .c(new_n67_), .O(new_n69_));
  inv1   g007(.a(x10), .O(new_n70_));
  inv1   g008(.a(x35), .O(new_n71_));
  inv1   g009(.a(x36), .O(new_n72_));
  nand3  g010(.a(new_n64_), .b(new_n72_), .c(new_n71_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(x27), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n70_), .c(x15), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n69_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(x16), .c(new_n63_), .O(z00));
  inv1   g015(.a(x27), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n72_), .c(new_n78_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  inv1   g022(.a(x04), .O(new_n85_));
  nor2   g023(.a(new_n63_), .b(new_n85_), .O(new_n86_));
  oai21  g024(.a(new_n84_), .b(new_n81_), .c(new_n86_), .O(z01));
  inv1   g025(.a(new_n63_), .O(new_n88_));
  inv1   g026(.a(new_n68_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n80_), .c(x04), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n91_), .c(new_n89_), .d(new_n88_), .O(z02));
  nand2  g033(.a(new_n65_), .b(x27), .O(new_n96_));
  aoi21  g034(.a(new_n64_), .b(new_n78_), .c(new_n63_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n96_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n96_), .c(new_n99_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n63_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  inv1   g042(.a(x15), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n105_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n104_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n88_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nand2  g052(.a(new_n68_), .b(new_n88_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n88_), .O(z09));
  nand2  g055(.a(new_n80_), .b(new_n72_), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  nand2  g057(.a(new_n68_), .b(x07), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n118_), .c(x27), .O(new_n122_));
  inv1   g060(.a(new_n119_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n84_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n85_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n126_), .c(new_n63_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n80_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  inv1   g071(.a(new_n80_), .O(new_n134_));
  nor2   g072(.a(new_n78_), .b(x04), .O(new_n135_));
  inv1   g073(.a(x09), .O(new_n136_));
  nand2  g074(.a(new_n82_), .b(new_n136_), .O(new_n137_));
  aoi21  g075(.a(new_n135_), .b(new_n134_), .c(new_n137_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n133_), .c(new_n63_), .O(z18));
  inv1   g077(.a(z18), .O(z11));
  inv1   g078(.a(x22), .O(new_n141_));
  oai21  g079(.a(new_n74_), .b(new_n141_), .c(new_n70_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n69_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand3  g082(.a(new_n89_), .b(new_n144_), .c(new_n85_), .O(new_n145_));
  aoi21  g083(.a(x36), .b(x35), .c(new_n79_), .O(new_n146_));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n66_), .c(x20), .O(new_n148_));
  oai21  g086(.a(new_n146_), .b(new_n145_), .c(new_n148_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  inv1   g088(.a(new_n145_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n84_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n150_), .c(new_n88_), .O(z13));
  nand2  g091(.a(new_n152_), .b(new_n88_), .O(new_n154_));
  aoi21  g092(.a(new_n149_), .b(x27), .c(new_n154_), .O(z14));
  nand4  g093(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n88_), .O(z15));
  nand2  g095(.a(x22), .b(x01), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(x23), .c(new_n88_), .O(z16));
  inv1   g097(.a(x24), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x23), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n158_), .c(new_n88_), .O(z17));
  nand3  g100(.a(new_n138_), .b(new_n133_), .c(new_n88_), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z19));
  nor2   g102(.a(new_n71_), .b(new_n78_), .O(new_n165_));
  nor4   g103(.a(new_n165_), .b(new_n137_), .c(new_n131_), .d(new_n63_), .O(z20));
endmodule


