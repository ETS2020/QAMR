// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n169_, new_n171_, new_n172_, new_n176_,
    new_n177_;
  inv1   g000(.a(x26), .O(new_n63_));
  nor2   g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  and2   g013(.a(x40), .b(x39), .O(z08));
  nand4  g014(.a(z08), .b(new_n75_), .c(new_n65_), .d(new_n72_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x35), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  oai21  g018(.a(new_n79_), .b(x28), .c(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x27), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g023(.a(x40), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n63_), .O(new_n87_));
  nor2   g025(.a(new_n79_), .b(x28), .O(new_n88_));
  inv1   g026(.a(x08), .O(new_n89_));
  nand2  g027(.a(new_n67_), .b(new_n89_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  and2   g029(.a(x29), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x02), .c(x39), .O(new_n93_));
  aoi21  g031(.a(new_n91_), .b(x04), .c(new_n93_), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n86_), .c(new_n87_), .O(z02));
  nand2  g033(.a(new_n86_), .b(x26), .O(new_n96_));
  nand2  g034(.a(new_n69_), .b(x27), .O(new_n97_));
  nand2  g035(.a(new_n68_), .b(new_n67_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(x21), .O(z03));
  aoi21  g037(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n97_), .c(new_n64_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n64_), .O(z05));
  and2   g041(.a(new_n102_), .b(new_n96_), .O(z06));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n65_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n105_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n96_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nand3  g052(.a(x34), .b(x27), .c(x11), .O(new_n115_));
  aoi21  g053(.a(new_n115_), .b(x40), .c(new_n63_), .O(z09));
  inv1   g054(.a(new_n83_), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(z08), .c(x05), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(new_n119_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(x40), .b(x39), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand4  g061(.a(new_n123_), .b(new_n120_), .c(new_n81_), .d(x27), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n118_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n72_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  nor2   g066(.a(new_n128_), .b(new_n64_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n126_), .O(z10));
  inv1   g068(.a(x28), .O(new_n131_));
  nand2  g069(.a(x35), .b(new_n131_), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n132_), .c(new_n89_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  or2    g074(.a(x30), .b(x09), .O(new_n137_));
  nor2   g075(.a(new_n67_), .b(x04), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n88_), .c(new_n137_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n96_), .O(z11));
  nand3  g079(.a(new_n68_), .b(new_n80_), .c(new_n79_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  aoi21  g081(.a(x39), .b(new_n72_), .c(new_n70_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(x40), .O(new_n145_));
  inv1   g083(.a(new_n70_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n86_), .c(new_n63_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(z12));
  aoi21  g086(.a(x36), .b(x35), .c(new_n131_), .O(new_n149_));
  nor2   g087(.a(x13), .b(x04), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(z08), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g090(.a(new_n69_), .b(new_n68_), .O(new_n153_));
  nor2   g091(.a(x19), .b(x18), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(new_n153_), .c(new_n96_), .d(x20), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n152_), .c(x27), .O(new_n157_));
  nand3  g095(.a(new_n150_), .b(new_n117_), .c(z08), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(z13));
  nand2  g097(.a(new_n154_), .b(x20), .O(new_n160_));
  nand2  g098(.a(new_n149_), .b(new_n83_), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n150_), .c(z08), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  aoi21  g101(.a(new_n131_), .b(x27), .c(new_n117_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n151_), .c(new_n146_), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n163_), .c(new_n96_), .O(z14));
  nand3  g104(.a(x34), .b(x27), .c(x12), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(x40), .c(new_n63_), .O(z15));
  nand2  g106(.a(x22), .b(x01), .O(new_n169_));
  nor3   g107(.a(new_n169_), .b(new_n64_), .c(x23), .O(z16));
  inv1   g108(.a(x24), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(x23), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n169_), .c(new_n96_), .O(z17));
  nand3  g111(.a(new_n139_), .b(new_n136_), .c(new_n96_), .O(z18));
  inv1   g112(.a(z18), .O(z19));
  nor2   g113(.a(new_n137_), .b(new_n64_), .O(new_n176_));
  oai21  g114(.a(new_n79_), .b(new_n67_), .c(new_n176_), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n134_), .O(z20));
endmodule


