// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n84_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n121_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n129_, new_n131_;
  nand2  g00(.a(x40), .b(x39), .O(z02));
  inv1   g01(.a(z02), .O(z08));
  inv1   g02(.a(x15), .O(new_n65_));
  inv1   g03(.a(x37), .O(new_n66_));
  nand2  g04(.a(x35), .b(x28), .O(new_n67_));
  nand2  g05(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g06(.a(new_n68_), .b(x27), .c(new_n65_), .O(new_n69_));
  aoi21  g07(.a(new_n69_), .b(x16), .c(z08), .O(z00));
  inv1   g08(.a(x35), .O(new_n71_));
  inv1   g09(.a(x36), .O(new_n72_));
  oai21  g10(.a(new_n71_), .b(x28), .c(new_n72_), .O(new_n73_));
  nand2  g11(.a(new_n73_), .b(x27), .O(new_n74_));
  nor2   g12(.a(x32), .b(x30), .O(new_n75_));
  nand2  g13(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g14(.a(new_n76_), .b(x04), .c(z08), .O(z01));
  inv1   g15(.a(x27), .O(new_n78_));
  aoi21  g16(.a(new_n66_), .b(new_n78_), .c(z08), .O(new_n79_));
  nand2  g17(.a(new_n67_), .b(x27), .O(new_n80_));
  nand3  g18(.a(new_n80_), .b(new_n79_), .c(x21), .O(z03));
  inv1   g19(.a(x21), .O(new_n82_));
  nand3  g20(.a(new_n80_), .b(new_n79_), .c(new_n82_), .O(z04));
  nand2  g21(.a(x28), .b(x27), .O(new_n84_));
  nand3  g22(.a(new_n84_), .b(z02), .c(new_n66_), .O(z05));
  inv1   g23(.a(z05), .O(z06));
  inv1   g24(.a(x00), .O(new_n87_));
  oai21  g25(.a(x25), .b(new_n87_), .c(x38), .O(new_n88_));
  nand2  g26(.a(x17), .b(new_n65_), .O(new_n89_));
  nor2   g27(.a(x33), .b(x31), .O(new_n90_));
  nand3  g28(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  inv1   g29(.a(x14), .O(new_n92_));
  inv1   g30(.a(x25), .O(new_n93_));
  nand3  g31(.a(x38), .b(new_n93_), .c(new_n87_), .O(new_n94_));
  nand2  g32(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g33(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand2  g34(.a(new_n96_), .b(x03), .O(new_n97_));
  nand2  g35(.a(new_n97_), .b(z02), .O(z07));
  nand4  g36(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n99_));
  nand2  g37(.a(new_n99_), .b(z02), .O(z09));
  inv1   g38(.a(x07), .O(new_n101_));
  nor2   g39(.a(new_n101_), .b(x04), .O(new_n102_));
  aoi22  g40(.a(new_n102_), .b(new_n73_), .c(x37), .d(x06), .O(new_n103_));
  oai21  g41(.a(new_n103_), .b(new_n78_), .c(z02), .O(z10));
  nor2   g42(.a(x30), .b(x09), .O(new_n105_));
  nand2  g43(.a(new_n105_), .b(z02), .O(new_n106_));
  inv1   g44(.a(x08), .O(new_n107_));
  nand2  g45(.a(new_n78_), .b(new_n107_), .O(new_n108_));
  inv1   g46(.a(x28), .O(new_n109_));
  nand2  g47(.a(x27), .b(x04), .O(new_n110_));
  nand3  g48(.a(new_n110_), .b(x35), .c(new_n109_), .O(new_n111_));
  inv1   g49(.a(new_n111_), .O(new_n112_));
  aoi21  g50(.a(new_n112_), .b(new_n108_), .c(new_n106_), .O(z11));
  nand3  g51(.a(new_n68_), .b(z02), .c(x27), .O(z12));
  nor2   g52(.a(x19), .b(x18), .O(new_n115_));
  nand3  g53(.a(new_n115_), .b(x27), .c(x20), .O(new_n116_));
  aoi21  g54(.a(new_n67_), .b(new_n66_), .c(new_n116_), .O(new_n117_));
  nand2  g55(.a(new_n117_), .b(z02), .O(new_n118_));
  inv1   g56(.a(new_n118_), .O(z13));
  nor2   g57(.a(new_n117_), .b(z08), .O(z14));
  nand4  g58(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n121_));
  nand2  g59(.a(new_n121_), .b(z02), .O(z15));
  inv1   g60(.a(x23), .O(new_n123_));
  nand4  g61(.a(z02), .b(new_n123_), .c(x22), .d(x01), .O(new_n124_));
  inv1   g62(.a(new_n124_), .O(z16));
  inv1   g63(.a(x24), .O(new_n126_));
  nand4  g64(.a(new_n126_), .b(x23), .c(x22), .d(x01), .O(new_n127_));
  nand2  g65(.a(new_n127_), .b(z02), .O(z17));
  nand2  g66(.a(new_n112_), .b(new_n108_), .O(new_n129_));
  aoi21  g67(.a(new_n129_), .b(new_n105_), .c(z08), .O(z18));
  oai21  g68(.a(x28), .b(new_n107_), .c(new_n78_), .O(new_n131_));
  aoi21  g69(.a(new_n131_), .b(x35), .c(new_n106_), .O(z20));
  aoi21  g70(.a(new_n112_), .b(new_n108_), .c(new_n106_), .O(z19));
endmodule


