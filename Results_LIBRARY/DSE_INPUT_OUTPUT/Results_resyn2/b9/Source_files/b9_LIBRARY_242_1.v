// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n158_, new_n160_, new_n161_, new_n163_,
    new_n165_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x31), .O(new_n70_));
  nand2  g008(.a(x33), .b(new_n70_), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x27), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  inv1   g019(.a(new_n71_), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n63_), .O(new_n83_));
  oai21  g021(.a(new_n81_), .b(new_n78_), .c(new_n83_), .O(z01));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n77_), .c(x04), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  aoi21  g027(.a(new_n89_), .b(new_n86_), .c(new_n82_), .O(z02));
  inv1   g028(.a(x35), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n76_), .c(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n74_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n71_), .d(x21), .O(z03));
  aoi21  g033(.a(new_n93_), .b(new_n74_), .c(x21), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n92_), .c(new_n82_), .O(z04));
  oai21  g035(.a(new_n76_), .b(new_n74_), .c(new_n93_), .O(new_n98_));
  and2   g036(.a(new_n98_), .b(new_n71_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x25), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x25), .b(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(x38), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(x15), .c(new_n70_), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n105_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n71_), .O(z07));
  nor2   g048(.a(new_n82_), .b(new_n87_), .O(z08));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n71_), .O(z09));
  nand2  g051(.a(new_n77_), .b(new_n75_), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  nand2  g053(.a(new_n87_), .b(x07), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n114_), .c(x27), .O(new_n118_));
  inv1   g056(.a(new_n115_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n81_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n63_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(new_n82_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n77_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  inv1   g067(.a(new_n77_), .O(new_n130_));
  nor2   g068(.a(new_n74_), .b(x04), .O(new_n131_));
  inv1   g069(.a(x09), .O(new_n132_));
  nand2  g070(.a(new_n79_), .b(new_n132_), .O(new_n133_));
  aoi21  g071(.a(new_n131_), .b(new_n130_), .c(new_n133_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n129_), .c(new_n82_), .O(z18));
  inv1   g073(.a(z18), .O(z11));
  nand2  g074(.a(new_n71_), .b(new_n69_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z12));
  inv1   g076(.a(x13), .O(new_n139_));
  inv1   g077(.a(new_n87_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n139_), .c(new_n63_), .O(new_n141_));
  aoi21  g079(.a(x36), .b(x35), .c(new_n76_), .O(new_n142_));
  nor2   g080(.a(x19), .b(x18), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x20), .O(new_n144_));
  oai22  g082(.a(new_n144_), .b(new_n66_), .c(new_n142_), .d(new_n141_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x27), .O(new_n146_));
  inv1   g084(.a(new_n141_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n81_), .c(new_n82_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n146_), .O(z13));
  nand3  g087(.a(new_n142_), .b(new_n80_), .c(new_n79_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n144_), .O(new_n152_));
  aoi21  g090(.a(new_n76_), .b(x27), .c(new_n81_), .O(new_n153_));
  oai22  g091(.a(new_n153_), .b(new_n141_), .c(new_n66_), .d(new_n74_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n152_), .c(new_n71_), .O(z14));
  nand4  g093(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n71_), .O(z15));
  nand2  g095(.a(x22), .b(x01), .O(new_n158_));
  nor3   g096(.a(new_n158_), .b(new_n82_), .c(x23), .O(z16));
  inv1   g097(.a(x24), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x23), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n158_), .c(new_n71_), .O(z17));
  nand3  g100(.a(new_n134_), .b(new_n129_), .c(new_n71_), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z19));
  nor2   g102(.a(new_n91_), .b(new_n74_), .O(new_n165_));
  nor4   g103(.a(new_n165_), .b(new_n133_), .c(new_n127_), .d(new_n82_), .O(z20));
endmodule


