// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n152_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n166_, new_n167_, new_n168_, new_n170_,
    new_n171_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x29), .O(new_n71_));
  nand2  g009(.a(x36), .b(new_n71_), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(new_n72_), .O(new_n75_));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(x28), .c(new_n79_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x27), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n77_), .c(new_n76_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x04), .c(new_n75_), .O(z01));
  nand2  g021(.a(x40), .b(x39), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  inv1   g027(.a(x02), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  oai21  g029(.a(new_n71_), .b(new_n91_), .c(new_n90_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(new_n72_), .d(new_n85_), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n78_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n75_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n72_), .c(new_n98_), .O(z04));
  nand2  g037(.a(new_n95_), .b(new_n94_), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n72_), .O(z06));
  inv1   g039(.a(z06), .O(z05));
  inv1   g040(.a(x14), .O(new_n103_));
  inv1   g041(.a(x15), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n104_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n103_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n72_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  nand2  g051(.a(new_n72_), .b(new_n84_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n72_), .O(z09));
  aoi21  g054(.a(new_n77_), .b(new_n76_), .c(new_n84_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  inv1   g056(.a(x07), .O(new_n119_));
  nand2  g057(.a(new_n84_), .b(new_n119_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n80_), .c(x27), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  inv1   g060(.a(x05), .O(new_n123_));
  aoi21  g061(.a(new_n85_), .b(new_n123_), .c(x04), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n72_), .O(z10));
  nor2   g065(.a(x30), .b(x09), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x27), .c(x04), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(new_n87_), .O(new_n131_));
  nand4  g069(.a(x35), .b(new_n86_), .c(x27), .d(new_n65_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n91_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n131_), .c(new_n130_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n128_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n72_), .O(z11));
  nand2  g074(.a(new_n67_), .b(x27), .O(new_n137_));
  nand3  g075(.a(new_n72_), .b(new_n68_), .c(new_n137_), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(z12));
  inv1   g077(.a(new_n63_), .O(new_n140_));
  nor2   g078(.a(x19), .b(x18), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(x20), .O(new_n142_));
  oai21  g080(.a(new_n79_), .b(new_n78_), .c(x28), .O(new_n143_));
  nor2   g081(.a(x13), .b(x04), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n143_), .c(new_n85_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  nand2  g085(.a(new_n144_), .b(new_n117_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n147_), .c(new_n72_), .O(z13));
  nand2  g087(.a(new_n148_), .b(new_n72_), .O(new_n150_));
  aoi21  g088(.a(new_n146_), .b(x27), .c(new_n150_), .O(z14));
  nand4  g089(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n72_), .O(z15));
  inv1   g091(.a(x23), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(x22), .c(x01), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n72_), .O(z16));
  nor2   g094(.a(x24), .b(new_n154_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(new_n72_), .c(x22), .d(x01), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z17));
  nor2   g097(.a(new_n88_), .b(new_n87_), .O(new_n160_));
  inv1   g098(.a(new_n128_), .O(new_n161_));
  nand2  g099(.a(x27), .b(x04), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n160_), .c(new_n161_), .O(new_n163_));
  nand3  g101(.a(new_n85_), .b(x29), .c(x08), .O(new_n164_));
  oai21  g102(.a(new_n163_), .b(new_n75_), .c(new_n164_), .O(z18));
  aoi21  g103(.a(new_n129_), .b(new_n87_), .c(new_n91_), .O(new_n166_));
  nand2  g104(.a(new_n129_), .b(x27), .O(new_n167_));
  nand3  g105(.a(new_n132_), .b(new_n128_), .c(new_n72_), .O(new_n168_));
  aoi21  g106(.a(new_n167_), .b(new_n166_), .c(new_n168_), .O(z19));
  nand2  g107(.a(x35), .b(x27), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n128_), .c(new_n72_), .O(new_n171_));
  nor2   g109(.a(new_n171_), .b(new_n166_), .O(z20));
endmodule


