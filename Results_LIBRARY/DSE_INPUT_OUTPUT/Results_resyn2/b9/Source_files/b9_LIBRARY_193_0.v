// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n98_, new_n99_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n179_, new_n181_, new_n182_;
  nor2   g000(.a(x40), .b(x05), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n64_), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  nor2   g009(.a(x36), .b(x35), .O(new_n72_));
  oai21  g010(.a(new_n72_), .b(new_n66_), .c(new_n71_), .O(new_n73_));
  and2   g011(.a(x40), .b(x39), .O(z08));
  nand4  g012(.a(z08), .b(new_n73_), .c(new_n64_), .d(new_n70_), .O(new_n75_));
  oai21  g013(.a(new_n69_), .b(new_n63_), .c(new_n75_), .O(z00));
  inv1   g014(.a(x35), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x28), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(x36), .c(x27), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g020(.a(x08), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  aoi21  g023(.a(new_n66_), .b(new_n83_), .c(new_n85_), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g027(.a(new_n86_), .b(new_n70_), .c(new_n89_), .O(z02));
  nand2  g028(.a(x35), .b(x28), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n66_), .c(new_n63_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  inv1   g035(.a(new_n63_), .O(new_n98_));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n98_), .O(z05));
  inv1   g038(.a(new_n99_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n98_), .O(z06));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n64_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n103_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n98_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n98_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  inv1   g053(.a(x36), .O(new_n116_));
  aoi21  g054(.a(new_n85_), .b(new_n116_), .c(new_n66_), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n80_), .O(new_n119_));
  inv1   g057(.a(x05), .O(new_n120_));
  aoi21  g058(.a(x40), .b(x39), .c(x07), .O(new_n121_));
  aoi21  g059(.a(z08), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n117_), .c(new_n119_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n63_), .O(new_n126_));
  oai21  g064(.a(new_n123_), .b(x04), .c(new_n126_), .O(z10));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n85_), .c(new_n83_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g069(.a(x09), .O(new_n132_));
  inv1   g070(.a(x30), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nor2   g072(.a(new_n66_), .b(x04), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n78_), .c(new_n134_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n131_), .c(new_n98_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  nand3  g076(.a(x40), .b(x39), .c(new_n70_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n72_), .c(new_n67_), .O(new_n140_));
  nor2   g078(.a(new_n87_), .b(x04), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x10), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n98_), .O(new_n143_));
  aoi21  g081(.a(new_n140_), .b(x27), .c(new_n143_), .O(z12));
  inv1   g082(.a(x13), .O(new_n145_));
  nand2  g083(.a(x36), .b(x35), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x28), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n141_), .c(new_n145_), .O(new_n148_));
  inv1   g086(.a(x18), .O(new_n149_));
  inv1   g087(.a(x19), .O(new_n150_));
  nand2  g088(.a(new_n91_), .b(new_n93_), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x20), .c(new_n150_), .d(new_n149_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n148_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  inv1   g092(.a(x32), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n133_), .O(new_n156_));
  nand3  g094(.a(z08), .b(new_n145_), .c(new_n70_), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n156_), .c(new_n63_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n154_), .O(z13));
  nand3  g098(.a(x20), .b(new_n150_), .c(new_n149_), .O(new_n161_));
  nand3  g099(.a(new_n146_), .b(new_n80_), .c(x28), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n157_), .c(new_n161_), .O(new_n164_));
  aoi21  g102(.a(new_n84_), .b(x27), .c(new_n156_), .O(new_n165_));
  oai22  g103(.a(new_n165_), .b(new_n157_), .c(new_n67_), .d(new_n66_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n164_), .c(new_n98_), .O(z14));
  inv1   g105(.a(x12), .O(new_n168_));
  nor2   g106(.a(new_n114_), .b(new_n168_), .O(z15));
  nand2  g107(.a(x22), .b(x01), .O(new_n170_));
  nor3   g108(.a(new_n170_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g109(.a(x24), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x23), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n170_), .c(new_n98_), .O(z17));
  nand2  g112(.a(x27), .b(x04), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(new_n86_), .c(new_n134_), .O(new_n176_));
  nand3  g114(.a(z08), .b(x29), .c(x08), .O(new_n177_));
  oai21  g115(.a(new_n176_), .b(new_n63_), .c(new_n177_), .O(z18));
  nand2  g116(.a(new_n136_), .b(new_n131_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n98_), .O(z19));
  nand2  g118(.a(x35), .b(x27), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n133_), .c(new_n132_), .O(new_n182_));
  oai21  g120(.a(new_n182_), .b(new_n129_), .c(new_n98_), .O(z20));
endmodule


