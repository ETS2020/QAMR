// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n154_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x14), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x08), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x27), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  inv1   g019(.a(new_n71_), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n63_), .O(new_n83_));
  oai21  g021(.a(new_n81_), .b(new_n78_), .c(new_n83_), .O(z01));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  inv1   g023(.a(new_n85_), .O(new_n86_));
  inv1   g024(.a(new_n77_), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n63_), .c(new_n86_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n71_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x04), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  oai21  g030(.a(x29), .b(new_n70_), .c(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n91_), .c(new_n89_), .O(z02));
  aoi21  g033(.a(x35), .b(x28), .c(new_n74_), .O(new_n96_));
  nor2   g034(.a(new_n96_), .b(new_n82_), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n74_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n97_), .c(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand3  g039(.a(new_n99_), .b(new_n97_), .c(new_n101_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n71_), .c(new_n98_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x15), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n106_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n70_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n70_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n71_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  nor2   g053(.a(new_n82_), .b(new_n85_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n71_), .O(z09));
  nand2  g056(.a(new_n77_), .b(new_n75_), .O(new_n119_));
  nand2  g057(.a(new_n85_), .b(x07), .O(new_n120_));
  nand3  g058(.a(x40), .b(x39), .c(x05), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n119_), .c(x27), .O(new_n123_));
  nand3  g061(.a(new_n81_), .b(new_n86_), .c(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n63_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n126_), .c(new_n82_), .O(z10));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  oai21  g067(.a(new_n74_), .b(new_n63_), .c(new_n87_), .O(new_n130_));
  nor2   g068(.a(new_n74_), .b(x04), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n87_), .c(x08), .O(new_n132_));
  aoi21  g070(.a(new_n130_), .b(new_n129_), .c(new_n132_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n133_), .c(new_n71_), .O(z11));
  nand2  g074(.a(new_n71_), .b(new_n69_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z12));
  aoi21  g076(.a(x36), .b(x35), .c(new_n76_), .O(new_n139_));
  nor2   g077(.a(x13), .b(x04), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n86_), .O(new_n141_));
  nor2   g079(.a(x19), .b(x18), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x20), .O(new_n143_));
  oai22  g081(.a(new_n143_), .b(new_n66_), .c(new_n141_), .d(new_n139_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  nand3  g083(.a(new_n140_), .b(new_n81_), .c(new_n86_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n145_), .c(new_n82_), .O(z13));
  nand3  g085(.a(new_n139_), .b(new_n80_), .c(new_n79_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n141_), .c(new_n143_), .O(new_n150_));
  aoi21  g088(.a(new_n76_), .b(x27), .c(new_n81_), .O(new_n151_));
  oai22  g089(.a(new_n151_), .b(new_n141_), .c(new_n66_), .d(new_n74_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n150_), .c(new_n71_), .O(z14));
  nand4  g091(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n71_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(x23), .c(new_n71_), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  inv1   g096(.a(new_n156_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n71_), .c(new_n158_), .d(x23), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z17));
  nand2  g099(.a(new_n129_), .b(x14), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(x08), .c(new_n135_), .O(new_n163_));
  oai21  g101(.a(new_n130_), .b(new_n90_), .c(new_n163_), .O(z18));
  inv1   g102(.a(new_n129_), .O(new_n165_));
  inv1   g103(.a(x35), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x14), .O(new_n167_));
  aoi21  g105(.a(x27), .b(x04), .c(x28), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n167_), .c(new_n165_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n132_), .c(new_n134_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n71_), .O(z19));
  inv1   g109(.a(x08), .O(new_n172_));
  oai21  g110(.a(x28), .b(new_n172_), .c(new_n74_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x35), .O(new_n174_));
  and2   g112(.a(new_n174_), .b(new_n163_), .O(z20));
endmodule


