// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n159_, new_n161_, new_n162_,
    new_n163_, new_n165_, new_n167_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x36), .O(new_n65_));
  inv1   g003(.a(x04), .O(new_n66_));
  inv1   g004(.a(x10), .O(new_n67_));
  nand2  g005(.a(new_n64_), .b(new_n67_), .O(new_n68_));
  nand4  g006(.a(new_n68_), .b(x40), .c(x39), .d(new_n66_), .O(new_n69_));
  inv1   g007(.a(x28), .O(new_n70_));
  inv1   g008(.a(x35), .O(new_n71_));
  inv1   g009(.a(x37), .O(new_n72_));
  oai21  g010(.a(new_n71_), .b(new_n70_), .c(new_n72_), .O(new_n73_));
  nand2  g011(.a(x40), .b(new_n65_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  oai22  g013(.a(new_n75_), .b(new_n64_), .c(new_n69_), .d(new_n65_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n63_), .O(new_n77_));
  inv1   g015(.a(x16), .O(new_n78_));
  nand2  g016(.a(new_n74_), .b(new_n78_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n77_), .O(z00));
  nand2  g018(.a(x35), .b(new_n70_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(new_n65_), .c(new_n64_), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  inv1   g021(.a(new_n83_), .O(new_n84_));
  inv1   g022(.a(new_n74_), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(new_n66_), .O(new_n86_));
  oai21  g024(.a(new_n84_), .b(new_n82_), .c(new_n86_), .O(z01));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n81_), .c(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nand3  g028(.a(x40), .b(x39), .c(x36), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n89_), .O(z02));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n71_), .c(new_n72_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n85_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n95_), .b(new_n97_), .c(new_n85_), .O(z04));
  nand2  g036(.a(new_n94_), .b(new_n72_), .O(new_n99_));
  and2   g037(.a(new_n99_), .b(new_n74_), .O(z05));
  nor2   g038(.a(new_n99_), .b(new_n85_), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n63_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n102_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n74_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  inv1   g049(.a(new_n91_), .O(z08));
  and2   g050(.a(x34), .b(x27), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(x26), .c(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n74_), .O(z09));
  inv1   g053(.a(x07), .O(new_n116_));
  inv1   g054(.a(x39), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x36), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(x40), .c(new_n116_), .O(new_n119_));
  nand4  g057(.a(x40), .b(x39), .c(x36), .d(x05), .O(new_n120_));
  aoi21  g058(.a(new_n83_), .b(new_n64_), .c(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n119_), .b(new_n82_), .c(new_n121_), .O(new_n122_));
  nand4  g060(.a(new_n74_), .b(x37), .c(x27), .d(x06), .O(new_n123_));
  oai21  g061(.a(new_n122_), .b(x04), .c(new_n123_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n81_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand4  g067(.a(x35), .b(new_n70_), .c(x27), .d(new_n66_), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  and2   g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n129_), .c(new_n74_), .O(z18));
  inv1   g071(.a(z18), .O(z11));
  nand2  g072(.a(new_n73_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n69_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n74_), .O(z12));
  inv1   g075(.a(x18), .O(new_n138_));
  inv1   g076(.a(x19), .O(new_n139_));
  nand3  g077(.a(x20), .b(new_n139_), .c(new_n138_), .O(new_n140_));
  inv1   g078(.a(x13), .O(new_n141_));
  nand4  g079(.a(x40), .b(x39), .c(new_n141_), .d(new_n66_), .O(new_n142_));
  oai21  g080(.a(x35), .b(new_n70_), .c(x36), .O(new_n143_));
  oai22  g081(.a(new_n143_), .b(new_n142_), .c(new_n140_), .d(new_n75_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  nand4  g083(.a(z08), .b(new_n84_), .c(new_n141_), .d(new_n66_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n145_), .O(z13));
  nand4  g085(.a(x28), .b(x20), .c(new_n139_), .d(new_n138_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n142_), .c(new_n71_), .O(new_n149_));
  nand4  g087(.a(x37), .b(x20), .c(new_n139_), .d(new_n138_), .O(new_n150_));
  oai21  g088(.a(new_n142_), .b(x28), .c(new_n150_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n149_), .c(x27), .O(new_n152_));
  inv1   g090(.a(new_n142_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n84_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n74_), .O(z14));
  nand4  g094(.a(new_n113_), .b(new_n74_), .c(x26), .d(x12), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z15));
  nand2  g096(.a(x22), .b(x01), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(x23), .c(new_n74_), .O(z16));
  inv1   g098(.a(x24), .O(new_n161_));
  inv1   g099(.a(new_n159_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(new_n74_), .c(new_n161_), .d(x23), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z17));
  nand2  g102(.a(new_n132_), .b(new_n129_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n74_), .O(z19));
  oai21  g104(.a(new_n71_), .b(new_n64_), .c(new_n131_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n127_), .c(new_n74_), .O(z20));
endmodule


