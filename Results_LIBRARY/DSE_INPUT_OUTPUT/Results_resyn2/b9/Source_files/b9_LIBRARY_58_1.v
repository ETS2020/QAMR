// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n152_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n159_, new_n161_, new_n164_,
    new_n165_, new_n166_;
  nand2  g000(.a(x40), .b(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x04), .O(new_n64_));
  aoi21  g002(.a(x35), .b(x28), .c(x37), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  and2   g005(.a(x40), .b(x39), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n66_), .c(new_n65_), .O(new_n70_));
  aoi22  g008(.a(new_n70_), .b(x27), .c(new_n64_), .d(x10), .O(new_n71_));
  inv1   g009(.a(x30), .O(new_n72_));
  nand2  g010(.a(x36), .b(new_n72_), .O(new_n73_));
  and2   g011(.a(new_n73_), .b(x16), .O(new_n74_));
  oai21  g012(.a(new_n71_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(new_n73_), .O(new_n76_));
  inv1   g014(.a(x27), .O(new_n77_));
  inv1   g015(.a(x35), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(x28), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(new_n77_), .c(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x04), .c(new_n76_), .O(z01));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n80_), .c(x04), .O(new_n85_));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand4  g026(.a(new_n88_), .b(new_n85_), .c(new_n73_), .d(new_n68_), .O(z02));
  inv1   g027(.a(x28), .O(new_n90_));
  oai21  g028(.a(new_n78_), .b(new_n90_), .c(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n77_), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n91_), .c(new_n73_), .d(x21), .O(z03));
  aoi21  g032(.a(new_n92_), .b(new_n77_), .c(x21), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n91_), .c(new_n76_), .O(z04));
  aoi21  g034(.a(x28), .b(x27), .c(x37), .O(new_n97_));
  nor2   g035(.a(new_n97_), .b(new_n76_), .O(z05));
  and2   g036(.a(new_n97_), .b(new_n73_), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n73_), .O(z07));
  nand2  g048(.a(new_n73_), .b(new_n63_), .O(z08));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n73_), .O(z09));
  inv1   g051(.a(x36), .O(new_n114_));
  oai21  g052(.a(new_n78_), .b(x28), .c(new_n114_), .O(new_n115_));
  nand4  g053(.a(new_n115_), .b(new_n73_), .c(new_n63_), .d(x07), .O(new_n116_));
  nand4  g054(.a(new_n79_), .b(new_n68_), .c(new_n114_), .d(x05), .O(new_n117_));
  aoi21  g055(.a(new_n117_), .b(new_n116_), .c(x04), .O(new_n118_));
  nand2  g056(.a(x37), .b(x06), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n76_), .O(new_n120_));
  oai21  g058(.a(new_n120_), .b(new_n118_), .c(x27), .O(new_n121_));
  inv1   g059(.a(x32), .O(new_n122_));
  oai21  g060(.a(x36), .b(new_n122_), .c(new_n72_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n64_), .c(x05), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n121_), .O(z10));
  inv1   g063(.a(x09), .O(new_n126_));
  nand2  g064(.a(new_n68_), .b(x29), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n80_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(x08), .O(new_n130_));
  nand3  g068(.a(new_n79_), .b(x27), .c(new_n67_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(new_n126_), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n114_), .c(x30), .O(z11));
  inv1   g071(.a(x10), .O(new_n134_));
  nor3   g072(.a(x37), .b(x36), .c(x35), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n77_), .c(new_n134_), .O(new_n136_));
  oai21  g074(.a(new_n65_), .b(new_n77_), .c(new_n69_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n136_), .c(new_n73_), .O(z12));
  inv1   g076(.a(new_n65_), .O(new_n139_));
  nor2   g077(.a(x19), .b(x18), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n139_), .c(x20), .O(new_n141_));
  inv1   g079(.a(x13), .O(new_n142_));
  nand3  g080(.a(new_n64_), .b(new_n90_), .c(new_n142_), .O(new_n143_));
  oai22  g081(.a(new_n143_), .b(x36), .c(new_n141_), .d(new_n76_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  nand3  g083(.a(new_n123_), .b(new_n64_), .c(new_n142_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n145_), .O(z13));
  aoi21  g085(.a(new_n143_), .b(new_n141_), .c(new_n77_), .O(new_n148_));
  nand2  g086(.a(new_n64_), .b(new_n142_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n81_), .c(new_n73_), .O(new_n150_));
  nor2   g088(.a(new_n150_), .b(new_n148_), .O(z14));
  nand4  g089(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n73_), .O(z15));
  inv1   g091(.a(x01), .O(new_n154_));
  inv1   g092(.a(x22), .O(new_n155_));
  nor4   g093(.a(new_n76_), .b(x23), .c(new_n155_), .d(new_n154_), .O(z16));
  inv1   g094(.a(x24), .O(new_n157_));
  nor2   g095(.a(new_n155_), .b(new_n154_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(new_n73_), .c(new_n157_), .d(x23), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(z17));
  nor2   g098(.a(x36), .b(x30), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n131_), .c(new_n130_), .d(new_n126_), .O(z18));
  inv1   g100(.a(z18), .O(z19));
  nand2  g101(.a(new_n128_), .b(x08), .O(new_n164_));
  aoi21  g102(.a(x35), .b(x27), .c(x09), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n114_), .c(x30), .O(z20));
endmodule


