// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n186_, new_n188_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x38), .O(new_n71_));
  nand2  g009(.a(x40), .b(new_n71_), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  inv1   g019(.a(new_n72_), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n64_), .O(new_n83_));
  oai21  g021(.a(new_n81_), .b(new_n79_), .c(new_n83_), .O(z01));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n78_), .c(x04), .O(new_n86_));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g027(.a(x39), .O(new_n90_));
  inv1   g028(.a(x40), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(new_n86_), .d(x38), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand3  g032(.a(x35), .b(x28), .c(x27), .O(new_n95_));
  oai21  g033(.a(new_n94_), .b(x27), .c(new_n95_), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n82_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  aoi21  g036(.a(new_n96_), .b(new_n98_), .c(new_n82_), .O(z04));
  aoi21  g037(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n82_), .O(z05));
  and2   g039(.a(new_n100_), .b(new_n72_), .O(z06));
  inv1   g040(.a(x14), .O(new_n103_));
  inv1   g041(.a(x15), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n104_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x00), .O(new_n108_));
  aoi21  g046(.a(x25), .b(new_n103_), .c(new_n71_), .O(new_n109_));
  oai21  g047(.a(x25), .b(new_n108_), .c(new_n109_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n72_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n107_), .O(z07));
  aoi21  g050(.a(new_n90_), .b(x38), .c(new_n91_), .O(z08));
  nand3  g051(.a(x34), .b(x27), .c(x26), .O(new_n114_));
  nand2  g052(.a(new_n72_), .b(x11), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n114_), .O(z09));
  nand2  g054(.a(new_n92_), .b(new_n81_), .O(new_n117_));
  inv1   g055(.a(x35), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(x28), .c(new_n76_), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  oai21  g058(.a(new_n91_), .b(new_n90_), .c(new_n120_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n119_), .c(x27), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n117_), .O(new_n123_));
  inv1   g061(.a(x05), .O(new_n124_));
  aoi21  g062(.a(new_n92_), .b(new_n124_), .c(x04), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n72_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(new_n72_), .b(new_n75_), .O(new_n130_));
  nand3  g068(.a(x35), .b(new_n77_), .c(new_n64_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x38), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n129_), .O(new_n134_));
  aoi21  g072(.a(x39), .b(x29), .c(new_n71_), .O(new_n135_));
  nor2   g073(.a(new_n75_), .b(new_n64_), .O(new_n136_));
  oai22  g074(.a(new_n136_), .b(new_n78_), .c(new_n135_), .d(new_n91_), .O(new_n137_));
  nor2   g075(.a(x30), .b(x09), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(new_n139_));
  aoi21  g077(.a(new_n137_), .b(new_n134_), .c(new_n139_), .O(z11));
  nand2  g078(.a(new_n63_), .b(new_n94_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(x27), .c(x10), .O(new_n142_));
  nand2  g080(.a(x35), .b(x28), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n94_), .O(new_n144_));
  aoi22  g082(.a(new_n144_), .b(x27), .c(x39), .d(new_n64_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n142_), .c(x38), .O(new_n146_));
  nand2  g084(.a(new_n144_), .b(x27), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n91_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n146_), .O(z12));
  nor2   g087(.a(x19), .b(x18), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n72_), .c(new_n144_), .d(x20), .O(new_n151_));
  nor2   g089(.a(x13), .b(x04), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x28), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(new_n152_), .c(new_n92_), .d(x38), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nand4  g095(.a(new_n152_), .b(new_n92_), .c(new_n81_), .d(x38), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(z13));
  nand2  g097(.a(new_n150_), .b(x20), .O(new_n160_));
  nand3  g098(.a(new_n153_), .b(new_n80_), .c(x28), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n152_), .c(new_n92_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  oai21  g101(.a(x28), .b(new_n75_), .c(new_n80_), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n152_), .c(new_n92_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n147_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n163_), .c(new_n72_), .O(z14));
  inv1   g105(.a(x12), .O(new_n168_));
  oai21  g106(.a(new_n114_), .b(new_n168_), .c(new_n72_), .O(z15));
  nand2  g107(.a(x22), .b(x01), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(x23), .c(new_n72_), .O(z16));
  inv1   g109(.a(x24), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x23), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n170_), .c(new_n72_), .O(z17));
  nand3  g112(.a(x40), .b(x39), .c(x29), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(new_n78_), .c(new_n129_), .O(new_n176_));
  nand2  g114(.a(new_n175_), .b(x27), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  inv1   g116(.a(new_n131_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x27), .O(new_n180_));
  nand4  g118(.a(new_n180_), .b(new_n178_), .c(new_n138_), .d(new_n72_), .O(z18));
  nand2  g119(.a(x40), .b(x04), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(x35), .c(new_n77_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(x38), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n130_), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n129_), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n137_), .c(new_n139_), .O(z19));
  oai21  g125(.a(new_n118_), .b(new_n75_), .c(new_n138_), .O(new_n188_));
  oai21  g126(.a(new_n188_), .b(new_n176_), .c(new_n72_), .O(z20));
endmodule


