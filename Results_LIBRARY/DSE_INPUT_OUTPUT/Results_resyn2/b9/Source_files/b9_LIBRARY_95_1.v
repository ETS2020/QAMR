// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n176_,
    new_n177_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand2  g002(.a(x40), .b(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand3  g005(.a(x40), .b(x10), .c(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x39), .O(new_n71_));
  nand2  g009(.a(x40), .b(new_n71_), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(new_n72_), .O(new_n75_));
  inv1   g013(.a(x35), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(x28), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(x36), .c(x27), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x04), .c(new_n75_), .O(z01));
  inv1   g019(.a(x08), .O(new_n82_));
  inv1   g020(.a(x27), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  aoi21  g023(.a(new_n83_), .b(new_n82_), .c(new_n85_), .O(new_n86_));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  inv1   g026(.a(x40), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n71_), .O(new_n90_));
  inv1   g028(.a(new_n90_), .O(new_n91_));
  aoi21  g029(.a(new_n88_), .b(new_n87_), .c(new_n91_), .O(new_n92_));
  oai21  g030(.a(new_n86_), .b(new_n64_), .c(new_n92_), .O(z02));
  nand2  g031(.a(x35), .b(x28), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n83_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n95_), .c(new_n72_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n96_), .b(new_n83_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n75_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n72_), .O(z05));
  inv1   g040(.a(new_n101_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n72_), .O(z06));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  or2    g049(.a(x33), .b(x31), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n111_), .c(x14), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n109_), .c(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n72_), .O(z07));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n72_), .c(x11), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z09));
  inv1   g057(.a(x07), .O(new_n120_));
  nand3  g058(.a(x40), .b(x39), .c(x05), .O(new_n121_));
  oai21  g059(.a(x40), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  inv1   g060(.a(x36), .O(new_n123_));
  aoi21  g061(.a(new_n85_), .b(new_n123_), .c(x04), .O(new_n124_));
  nand2  g062(.a(x37), .b(x06), .O(new_n125_));
  aoi21  g063(.a(x40), .b(new_n71_), .c(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(new_n122_), .c(new_n126_), .O(new_n127_));
  inv1   g065(.a(new_n79_), .O(new_n128_));
  inv1   g066(.a(new_n121_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n64_), .O(new_n130_));
  oai21  g068(.a(new_n127_), .b(new_n83_), .c(new_n130_), .O(z10));
  nand2  g069(.a(x27), .b(x04), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n86_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  oai21  g072(.a(new_n88_), .b(new_n89_), .c(new_n134_), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n72_), .O(z11));
  nand2  g076(.a(new_n67_), .b(x27), .O(new_n139_));
  nand2  g077(.a(new_n68_), .b(new_n139_), .O(new_n140_));
  nand2  g078(.a(new_n72_), .b(new_n140_), .O(z12));
  nand2  g079(.a(new_n94_), .b(new_n96_), .O(new_n142_));
  nor2   g080(.a(x19), .b(x18), .O(new_n143_));
  nand4  g081(.a(new_n143_), .b(new_n72_), .c(new_n142_), .d(x20), .O(new_n144_));
  oai21  g082(.a(new_n123_), .b(new_n76_), .c(x28), .O(new_n145_));
  nor2   g083(.a(x13), .b(x04), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n145_), .c(new_n90_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  nand3  g087(.a(new_n146_), .b(new_n90_), .c(new_n128_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n149_), .O(z13));
  inv1   g089(.a(new_n145_), .O(new_n152_));
  nand2  g090(.a(new_n96_), .b(new_n76_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n143_), .c(x20), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n152_), .c(new_n83_), .O(new_n155_));
  nand4  g093(.a(new_n143_), .b(new_n142_), .c(x27), .d(x20), .O(new_n156_));
  inv1   g094(.a(x13), .O(new_n157_));
  nand3  g095(.a(x40), .b(new_n157_), .c(new_n64_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n156_), .c(new_n75_), .O(new_n159_));
  oai21  g097(.a(new_n155_), .b(new_n128_), .c(new_n159_), .O(z14));
  nand3  g098(.a(new_n117_), .b(new_n72_), .c(x12), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z15));
  inv1   g100(.a(x23), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(x22), .c(x01), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n72_), .O(z16));
  nor2   g103(.a(x24), .b(new_n163_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n72_), .c(x22), .d(x01), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z17));
  aoi21  g106(.a(new_n136_), .b(new_n133_), .c(new_n75_), .O(z18));
  nand2  g107(.a(x40), .b(x29), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n85_), .c(new_n82_), .O(new_n171_));
  nand2  g109(.a(new_n170_), .b(x27), .O(new_n172_));
  nand3  g110(.a(new_n77_), .b(x27), .c(new_n64_), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n134_), .c(new_n72_), .O(new_n174_));
  aoi21  g112(.a(new_n172_), .b(new_n171_), .c(new_n174_), .O(z19));
  nand2  g113(.a(new_n134_), .b(new_n72_), .O(new_n176_));
  nor2   g114(.a(new_n76_), .b(new_n83_), .O(new_n177_));
  nor3   g115(.a(new_n177_), .b(new_n176_), .c(new_n171_), .O(z20));
  buf    g116(.a(x40), .O(z08));
endmodule


