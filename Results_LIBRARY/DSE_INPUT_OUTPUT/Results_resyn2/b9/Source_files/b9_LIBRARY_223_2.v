// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n173_,
    new_n175_, new_n176_, new_n178_, new_n180_, new_n181_;
  inv1   g000(.a(x03), .O(new_n63_));
  nor2   g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x35), .O(new_n71_));
  inv1   g009(.a(x36), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(x27), .c(x10), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(new_n74_), .c(new_n70_), .d(new_n64_), .O(z00));
  nor2   g016(.a(new_n71_), .b(x28), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(x36), .c(x27), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nor2   g023(.a(x27), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n85_), .c(x04), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n75_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n87_), .O(z02));
  nand2  g028(.a(x35), .b(x28), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n67_), .c(new_n64_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n64_), .O(z05));
  inv1   g037(.a(new_n98_), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n64_), .O(z06));
  inv1   g039(.a(x17), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(x15), .O(new_n103_));
  or2    g041(.a(x33), .b(x31), .O(new_n104_));
  oai21  g042(.a(new_n104_), .b(new_n103_), .c(x14), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  inv1   g045(.a(x14), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n108_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(x38), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n105_), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(x40), .c(new_n63_), .O(z07));
  inv1   g050(.a(new_n75_), .O(z08));
  inv1   g051(.a(new_n64_), .O(new_n114_));
  nand3  g052(.a(x34), .b(x27), .c(x26), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n114_), .c(x11), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z09));
  aoi21  g056(.a(new_n85_), .b(new_n72_), .c(new_n67_), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n81_), .O(new_n121_));
  nand2  g059(.a(new_n75_), .b(x07), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n119_), .c(new_n121_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n64_), .O(new_n127_));
  oai21  g065(.a(new_n124_), .b(x04), .c(new_n127_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n85_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  inv1   g071(.a(x09), .O(new_n134_));
  inv1   g072(.a(x30), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nor2   g074(.a(new_n67_), .b(x04), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n79_), .c(new_n136_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n133_), .c(new_n114_), .O(z18));
  inv1   g077(.a(z18), .O(z11));
  nand2  g078(.a(new_n76_), .b(new_n73_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(new_n68_), .c(new_n67_), .O(new_n142_));
  nand2  g080(.a(new_n76_), .b(x10), .O(new_n143_));
  inv1   g081(.a(new_n143_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n142_), .c(new_n114_), .O(z12));
  inv1   g083(.a(x13), .O(new_n146_));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x28), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n76_), .c(new_n146_), .O(new_n149_));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand2  g089(.a(new_n91_), .b(new_n93_), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n149_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x27), .O(new_n155_));
  inv1   g093(.a(x32), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n135_), .O(new_n157_));
  inv1   g095(.a(x04), .O(new_n158_));
  nand3  g096(.a(z08), .b(new_n146_), .c(new_n158_), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n157_), .c(new_n64_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n155_), .O(z13));
  nand3  g100(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n163_));
  nand3  g101(.a(new_n147_), .b(new_n81_), .c(x28), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n159_), .c(new_n163_), .O(new_n166_));
  nand2  g104(.a(new_n152_), .b(x27), .O(new_n167_));
  aoi21  g105(.a(new_n84_), .b(x27), .c(new_n157_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n159_), .c(new_n167_), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(new_n166_), .c(new_n64_), .O(z14));
  nand3  g108(.a(new_n116_), .b(new_n114_), .c(x12), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z15));
  nand2  g110(.a(x22), .b(x01), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(x23), .c(new_n114_), .O(z16));
  inv1   g112(.a(x24), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(x23), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n173_), .c(new_n114_), .O(z17));
  nand2  g115(.a(new_n138_), .b(new_n133_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n114_), .O(z19));
  nand2  g117(.a(x35), .b(x27), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n135_), .c(new_n134_), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n131_), .c(new_n114_), .O(z20));
endmodule


