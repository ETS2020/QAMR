// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n166_,
    new_n168_, new_n169_, new_n170_, new_n174_, new_n175_, new_n176_;
  nand2  g000(.a(x40), .b(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x04), .O(new_n64_));
  aoi21  g002(.a(x35), .b(x28), .c(x37), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  inv1   g005(.a(new_n63_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n66_), .c(new_n65_), .O(new_n70_));
  aoi22  g008(.a(new_n70_), .b(x27), .c(new_n64_), .d(x10), .O(new_n71_));
  inv1   g009(.a(x16), .O(new_n72_));
  nor2   g010(.a(x36), .b(x30), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai21  g012(.a(new_n71_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(x27), .O(new_n76_));
  nor2   g014(.a(x32), .b(x30), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(x36), .c(new_n76_), .O(new_n78_));
  oai21  g016(.a(new_n73_), .b(x04), .c(new_n78_), .O(z01));
  inv1   g017(.a(new_n73_), .O(new_n80_));
  nor2   g018(.a(x27), .b(x08), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n81_), .c(x04), .O(new_n84_));
  inv1   g022(.a(x02), .O(new_n85_));
  nand2  g023(.a(x29), .b(x08), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand4  g025(.a(new_n87_), .b(new_n84_), .c(new_n80_), .d(new_n68_), .O(z02));
  nand2  g026(.a(x35), .b(x28), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x27), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n76_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n80_), .d(x21), .O(z03));
  aoi21  g031(.a(new_n91_), .b(new_n76_), .c(x21), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n90_), .c(new_n73_), .O(z04));
  aoi21  g033(.a(x28), .b(x27), .c(x37), .O(new_n96_));
  nor2   g034(.a(new_n96_), .b(new_n73_), .O(z05));
  inv1   g035(.a(new_n96_), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n73_), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n80_), .O(z07));
  nand2  g048(.a(new_n80_), .b(new_n63_), .O(z08));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n80_), .O(z09));
  inv1   g051(.a(x36), .O(new_n114_));
  nand3  g052(.a(x35), .b(x30), .c(new_n82_), .O(new_n115_));
  nand2  g053(.a(new_n63_), .b(x07), .O(new_n116_));
  aoi21  g054(.a(new_n115_), .b(new_n114_), .c(new_n116_), .O(new_n117_));
  nand4  g055(.a(x40), .b(x39), .c(x36), .d(x05), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  oai21  g057(.a(new_n119_), .b(new_n117_), .c(new_n67_), .O(new_n120_));
  nand3  g058(.a(new_n80_), .b(x37), .c(x06), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x27), .O(new_n123_));
  inv1   g061(.a(x30), .O(new_n124_));
  nand2  g062(.a(x36), .b(x32), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n64_), .c(x05), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n123_), .O(z10));
  nand2  g066(.a(new_n68_), .b(x29), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n83_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(x08), .O(new_n132_));
  inv1   g070(.a(new_n83_), .O(new_n133_));
  nor2   g071(.a(new_n76_), .b(x04), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n133_), .c(x09), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x36), .c(x30), .O(z11));
  inv1   g075(.a(x10), .O(new_n138_));
  nor3   g076(.a(x37), .b(x36), .c(x35), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n76_), .c(new_n138_), .O(new_n140_));
  oai21  g078(.a(new_n65_), .b(new_n76_), .c(new_n69_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n80_), .O(z12));
  inv1   g080(.a(x18), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand3  g082(.a(x20), .b(new_n144_), .c(new_n143_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  nor2   g084(.a(new_n73_), .b(new_n65_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(new_n64_), .b(new_n149_), .O(new_n150_));
  oai21  g088(.a(x35), .b(new_n82_), .c(x36), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n148_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand3  g091(.a(new_n126_), .b(new_n64_), .c(new_n149_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  nand4  g093(.a(x28), .b(x20), .c(new_n144_), .d(new_n143_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n150_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x35), .O(new_n158_));
  nand2  g096(.a(new_n146_), .b(x37), .O(new_n159_));
  nand3  g097(.a(new_n64_), .b(new_n82_), .c(new_n149_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n159_), .c(new_n158_), .O(new_n161_));
  oai21  g099(.a(new_n150_), .b(new_n77_), .c(new_n80_), .O(new_n162_));
  aoi21  g100(.a(new_n161_), .b(x27), .c(new_n162_), .O(z14));
  nand4  g101(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n80_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  nor3   g104(.a(new_n166_), .b(new_n73_), .c(x23), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  inv1   g106(.a(new_n166_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(new_n80_), .c(new_n168_), .d(x23), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z17));
  nand4  g109(.a(new_n135_), .b(new_n132_), .c(x36), .d(new_n124_), .O(z18));
  inv1   g110(.a(z18), .O(z19));
  nand2  g111(.a(new_n130_), .b(x08), .O(new_n174_));
  aoi21  g112(.a(x35), .b(x27), .c(x09), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(x36), .c(x30), .O(z20));
endmodule


