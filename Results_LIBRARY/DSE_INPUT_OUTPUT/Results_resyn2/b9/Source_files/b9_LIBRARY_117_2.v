// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n160_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n169_,
    new_n171_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(z08));
  nand3  g007(.a(z08), .b(x10), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n67_), .b(x27), .c(new_n71_), .O(new_n72_));
  inv1   g010(.a(x16), .O(new_n73_));
  inv1   g011(.a(x34), .O(new_n74_));
  nor2   g012(.a(x40), .b(new_n74_), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  oai21  g014(.a(new_n72_), .b(x15), .c(new_n76_), .O(z00));
  inv1   g015(.a(new_n75_), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(x36), .c(x27), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g023(.a(x08), .O(new_n86_));
  inv1   g024(.a(x27), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(new_n86_), .c(new_n80_), .O(new_n88_));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n89_), .c(new_n68_), .O(new_n91_));
  oai21  g029(.a(new_n88_), .b(new_n65_), .c(new_n91_), .O(new_n92_));
  xor2a  g030(.a(new_n92_), .b(new_n75_), .O(z02));
  inv1   g031(.a(x35), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n79_), .c(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n87_), .c(new_n75_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n95_), .c(new_n99_), .O(z04));
  oai21  g038(.a(new_n79_), .b(new_n87_), .c(new_n96_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n78_), .O(z06));
  inv1   g040(.a(z06), .O(z05));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n78_), .O(z07));
  nand3  g052(.a(x27), .b(x26), .c(x11), .O(new_n115_));
  aoi21  g053(.a(new_n115_), .b(x40), .c(new_n74_), .O(z09));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  nand2  g055(.a(new_n68_), .b(x07), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(new_n75_), .c(new_n117_), .O(new_n119_));
  inv1   g057(.a(x36), .O(new_n120_));
  aoi21  g058(.a(new_n80_), .b(new_n120_), .c(x04), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n75_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n119_), .c(new_n123_), .O(new_n124_));
  nor2   g062(.a(new_n83_), .b(new_n66_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x05), .O(new_n126_));
  oai21  g064(.a(new_n124_), .b(new_n87_), .c(new_n126_), .O(z10));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n80_), .c(new_n86_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g069(.a(new_n81_), .b(x27), .c(new_n65_), .O(new_n132_));
  nor2   g070(.a(x30), .b(x09), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n78_), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(z11));
  nand2  g073(.a(new_n67_), .b(x27), .O(new_n136_));
  nand2  g074(.a(new_n70_), .b(new_n136_), .O(new_n137_));
  nand2  g075(.a(new_n78_), .b(new_n137_), .O(z12));
  inv1   g076(.a(x13), .O(new_n139_));
  nor2   g077(.a(new_n68_), .b(x04), .O(new_n140_));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n140_), .c(new_n139_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x20), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n63_), .c(new_n143_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  aoi21  g085(.a(new_n125_), .b(new_n139_), .c(new_n75_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(z13));
  nand2  g087(.a(new_n140_), .b(new_n139_), .O(new_n150_));
  nand2  g088(.a(new_n79_), .b(x27), .O(new_n151_));
  and2   g089(.a(new_n151_), .b(new_n83_), .O(new_n152_));
  oai22  g090(.a(new_n152_), .b(new_n150_), .c(new_n63_), .d(new_n87_), .O(new_n153_));
  nand3  g091(.a(new_n141_), .b(new_n83_), .c(x28), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n140_), .c(new_n139_), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n145_), .c(new_n75_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n153_), .O(z14));
  nand3  g095(.a(x27), .b(x26), .c(x12), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(x40), .c(new_n74_), .O(z15));
  nand2  g097(.a(x22), .b(x01), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(x23), .c(new_n78_), .O(z16));
  inv1   g099(.a(x24), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x23), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n160_), .c(new_n78_), .O(z17));
  inv1   g102(.a(new_n133_), .O(new_n165_));
  nand2  g103(.a(x27), .b(x04), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n88_), .c(new_n165_), .O(new_n167_));
  oai22  g105(.a(new_n167_), .b(new_n75_), .c(new_n90_), .d(new_n68_), .O(z18));
  nand3  g106(.a(new_n133_), .b(new_n132_), .c(new_n131_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n78_), .O(z19));
  oai21  g108(.a(new_n94_), .b(new_n87_), .c(new_n133_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n129_), .c(new_n78_), .O(z20));
endmodule


