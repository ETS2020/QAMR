// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n170_, new_n174_, new_n175_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x39), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  inv1   g014(.a(x39), .O(new_n77_));
  nor2   g015(.a(new_n63_), .b(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n65_), .O(new_n79_));
  oai22  g017(.a(new_n79_), .b(new_n76_), .c(new_n71_), .d(new_n64_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n81_), .c(new_n67_), .O(new_n84_));
  inv1   g022(.a(x30), .O(new_n85_));
  inv1   g023(.a(x32), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nor2   g025(.a(new_n64_), .b(new_n72_), .O(new_n88_));
  oai21  g026(.a(new_n87_), .b(new_n84_), .c(new_n88_), .O(z01));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  inv1   g028(.a(x35), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(x28), .O(new_n92_));
  inv1   g030(.a(x08), .O(new_n93_));
  nand2  g031(.a(new_n67_), .b(new_n93_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x04), .c(new_n90_), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(new_n77_), .c(x40), .O(z02));
  nand3  g035(.a(x35), .b(x28), .c(x27), .O(new_n98_));
  oai21  g036(.a(new_n68_), .b(x27), .c(new_n98_), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand2  g039(.a(x40), .b(new_n77_), .O(new_n102_));
  nand3  g040(.a(new_n99_), .b(new_n102_), .c(new_n101_), .O(z04));
  aoi21  g041(.a(x28), .b(x27), .c(x37), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n102_), .O(z05));
  inv1   g043(.a(new_n104_), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(new_n102_), .O(z06));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  inv1   g047(.a(x14), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n110_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(x38), .O(new_n112_));
  inv1   g050(.a(x17), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(x15), .O(new_n114_));
  or2    g052(.a(x33), .b(x31), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n114_), .c(x14), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n112_), .c(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n102_), .O(z07));
  and2   g056(.a(x34), .b(x27), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(x26), .c(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n102_), .O(z09));
  nand2  g059(.a(new_n87_), .b(x40), .O(new_n122_));
  oai21  g060(.a(new_n91_), .b(x28), .c(new_n81_), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  nand2  g062(.a(new_n63_), .b(new_n124_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n123_), .c(x27), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n122_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  aoi21  g066(.a(x40), .b(new_n128_), .c(x04), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  nor2   g070(.a(new_n132_), .b(new_n64_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(z10));
  nand2  g072(.a(x40), .b(x29), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n83_), .c(new_n93_), .O(new_n136_));
  nand2  g074(.a(new_n135_), .b(x27), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nor2   g076(.a(new_n67_), .b(x04), .O(new_n139_));
  inv1   g077(.a(x09), .O(new_n140_));
  nand2  g078(.a(new_n85_), .b(new_n140_), .O(new_n141_));
  aoi21  g079(.a(new_n139_), .b(new_n92_), .c(new_n141_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n138_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n102_), .O(z11));
  nand2  g082(.a(new_n76_), .b(x39), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(x40), .c(new_n70_), .O(z12));
  nand2  g084(.a(new_n69_), .b(new_n68_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(new_n147_), .c(new_n102_), .d(x20), .O(new_n149_));
  oai21  g087(.a(new_n81_), .b(new_n91_), .c(x28), .O(new_n150_));
  nor2   g088(.a(x13), .b(x04), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n150_), .c(new_n78_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n149_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  nand3  g092(.a(new_n151_), .b(new_n87_), .c(new_n78_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(z13));
  nand3  g094(.a(new_n148_), .b(new_n147_), .c(x20), .O(new_n157_));
  nand3  g095(.a(new_n151_), .b(new_n150_), .c(x40), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nand2  g097(.a(new_n151_), .b(new_n87_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(x39), .c(new_n63_), .O(new_n161_));
  aoi21  g099(.a(new_n159_), .b(x27), .c(new_n161_), .O(z14));
  nand4  g100(.a(new_n119_), .b(new_n102_), .c(x26), .d(x12), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z15));
  inv1   g102(.a(x01), .O(new_n165_));
  inv1   g103(.a(x22), .O(new_n166_));
  nor4   g104(.a(new_n64_), .b(x23), .c(new_n166_), .d(new_n165_), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  nor2   g106(.a(new_n166_), .b(new_n165_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(new_n102_), .c(new_n168_), .d(x23), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z17));
  nand3  g109(.a(new_n142_), .b(new_n138_), .c(new_n102_), .O(z18));
  inv1   g110(.a(z18), .O(z19));
  nand2  g111(.a(x35), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n85_), .c(new_n140_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n136_), .c(new_n102_), .O(z20));
  buf    g114(.a(x40), .O(z08));
endmodule


