// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n158_, new_n160_,
    new_n161_, new_n163_, new_n166_, new_n167_;
  nand2  g000(.a(x32), .b(x24), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  and2   g009(.a(x40), .b(x39), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(new_n71_), .c(new_n68_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n67_), .c(new_n66_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x10), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n65_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n66_), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  oai22  g023(.a(new_n85_), .b(new_n83_), .c(new_n64_), .d(x04), .O(z01));
  nand2  g024(.a(x35), .b(new_n81_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor3   g028(.a(new_n90_), .b(new_n75_), .c(new_n64_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n89_), .O(z02));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n69_), .c(new_n93_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n63_), .c(new_n97_), .O(z04));
  nand3  g036(.a(new_n94_), .b(new_n63_), .c(new_n93_), .O(z05));
  nand2  g037(.a(new_n94_), .b(new_n93_), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n63_), .O(z06));
  inv1   g039(.a(x25), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(x00), .O(new_n103_));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x25), .b(new_n104_), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(x38), .O(new_n106_));
  inv1   g044(.a(x17), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(x15), .O(new_n108_));
  or2    g046(.a(x33), .b(x31), .O(new_n109_));
  oai21  g047(.a(new_n109_), .b(new_n108_), .c(x14), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n106_), .c(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n63_), .O(z07));
  nand2  g050(.a(new_n75_), .b(new_n63_), .O(z08));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n63_), .O(z09));
  inv1   g053(.a(x30), .O(new_n116_));
  oai21  g054(.a(new_n82_), .b(new_n66_), .c(new_n116_), .O(new_n117_));
  nand2  g055(.a(new_n72_), .b(x05), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  and2   g057(.a(new_n75_), .b(x07), .O(new_n120_));
  aoi22  g058(.a(new_n120_), .b(new_n83_), .c(new_n119_), .d(new_n117_), .O(new_n121_));
  inv1   g059(.a(x24), .O(new_n122_));
  nand2  g060(.a(new_n76_), .b(x05), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(x32), .c(new_n126_), .O(new_n127_));
  oai21  g065(.a(new_n121_), .b(x04), .c(new_n127_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(new_n72_), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n87_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  inv1   g071(.a(new_n87_), .O(new_n134_));
  nor2   g072(.a(new_n66_), .b(x04), .O(new_n135_));
  inv1   g073(.a(x09), .O(new_n136_));
  nand2  g074(.a(new_n116_), .b(new_n136_), .O(new_n137_));
  aoi21  g075(.a(new_n135_), .b(new_n134_), .c(new_n137_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n133_), .c(new_n63_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z11));
  inv1   g078(.a(new_n74_), .O(new_n141_));
  nand3  g079(.a(new_n77_), .b(new_n141_), .c(new_n63_), .O(new_n142_));
  inv1   g080(.a(new_n142_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  oai21  g082(.a(new_n70_), .b(new_n69_), .c(x28), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(new_n72_), .c(new_n144_), .d(new_n68_), .O(new_n146_));
  inv1   g084(.a(new_n67_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n147_), .c(x20), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n146_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  nand3  g089(.a(new_n72_), .b(new_n144_), .c(new_n68_), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(new_n84_), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n151_), .c(new_n64_), .O(z13));
  aoi21  g093(.a(new_n149_), .b(new_n146_), .c(new_n66_), .O(new_n156_));
  oai21  g094(.a(new_n153_), .b(new_n156_), .c(new_n63_), .O(z14));
  nand4  g095(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n63_), .O(z15));
  inv1   g097(.a(x23), .O(new_n160_));
  nand4  g098(.a(new_n63_), .b(new_n160_), .c(x22), .d(x01), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z16));
  nand4  g100(.a(new_n122_), .b(x23), .c(x22), .d(x01), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z17));
  aoi21  g102(.a(new_n138_), .b(new_n133_), .c(new_n64_), .O(z18));
  nand2  g103(.a(x35), .b(x27), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n116_), .c(new_n136_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n131_), .c(new_n63_), .O(z20));
  inv1   g106(.a(new_n139_), .O(z19));
endmodule


