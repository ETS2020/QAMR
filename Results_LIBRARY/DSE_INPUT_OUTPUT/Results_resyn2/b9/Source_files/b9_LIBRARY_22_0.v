// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n169_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x39), .b(x30), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x39), .O(new_n81_));
  oai22  g019(.a(new_n81_), .b(new_n79_), .c(new_n72_), .d(x04), .O(z01));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n78_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n84_), .O(z02));
  inv1   g026(.a(x35), .O(new_n89_));
  inv1   g027(.a(x37), .O(new_n90_));
  nand2  g028(.a(x28), .b(x27), .O(new_n91_));
  oai22  g029(.a(new_n91_), .b(new_n89_), .c(new_n90_), .d(x27), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g031(.a(x21), .O(new_n94_));
  aoi21  g032(.a(new_n92_), .b(new_n94_), .c(new_n72_), .O(z04));
  aoi21  g033(.a(new_n91_), .b(new_n90_), .c(new_n72_), .O(z05));
  inv1   g034(.a(z05), .O(z06));
  inv1   g035(.a(x14), .O(new_n98_));
  inv1   g036(.a(x15), .O(new_n99_));
  nand2  g037(.a(x17), .b(new_n99_), .O(new_n100_));
  nor2   g038(.a(x33), .b(x31), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n100_), .c(new_n98_), .O(new_n102_));
  inv1   g040(.a(new_n72_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n98_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n105_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n103_), .c(x03), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(new_n102_), .O(z07));
  nand2  g047(.a(new_n85_), .b(new_n103_), .O(z08));
  inv1   g048(.a(x11), .O(new_n111_));
  nand3  g049(.a(x34), .b(x27), .c(x26), .O(new_n112_));
  nor3   g050(.a(new_n112_), .b(new_n72_), .c(new_n111_), .O(z09));
  nor2   g051(.a(new_n85_), .b(new_n80_), .O(new_n114_));
  inv1   g052(.a(x07), .O(new_n115_));
  nand2  g053(.a(new_n85_), .b(new_n115_), .O(new_n116_));
  aoi21  g054(.a(new_n116_), .b(new_n79_), .c(new_n114_), .O(new_n117_));
  oai21  g055(.a(new_n85_), .b(x05), .c(new_n65_), .O(new_n118_));
  nand3  g056(.a(x37), .b(x27), .c(x06), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n72_), .O(new_n121_));
  oai21  g059(.a(new_n118_), .b(new_n117_), .c(new_n121_), .O(z10));
  inv1   g060(.a(x09), .O(new_n123_));
  nand2  g061(.a(x40), .b(x29), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n78_), .O(new_n125_));
  nand2  g063(.a(new_n124_), .b(x27), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(x08), .O(new_n127_));
  nand4  g065(.a(x35), .b(new_n77_), .c(x27), .d(new_n65_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n123_), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(x39), .c(x30), .O(z11));
  nand2  g068(.a(new_n67_), .b(x27), .O(new_n131_));
  nand3  g069(.a(new_n103_), .b(new_n68_), .c(new_n131_), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(z12));
  aoi21  g071(.a(x36), .b(x35), .c(new_n77_), .O(new_n134_));
  nor2   g072(.a(x13), .b(x04), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x40), .c(x39), .O(new_n136_));
  inv1   g074(.a(x18), .O(new_n137_));
  inv1   g075(.a(x19), .O(new_n138_));
  nand3  g076(.a(x20), .b(new_n138_), .c(new_n137_), .O(new_n139_));
  oai22  g077(.a(new_n139_), .b(new_n63_), .c(new_n136_), .d(new_n134_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x27), .O(new_n141_));
  aoi21  g079(.a(new_n135_), .b(new_n114_), .c(new_n72_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n141_), .O(z13));
  inv1   g081(.a(x30), .O(new_n144_));
  inv1   g082(.a(x39), .O(new_n145_));
  nand3  g083(.a(new_n90_), .b(new_n89_), .c(x28), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x27), .c(x32), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n145_), .c(new_n144_), .O(new_n148_));
  inv1   g086(.a(new_n136_), .O(new_n149_));
  nand3  g087(.a(new_n139_), .b(new_n134_), .c(new_n80_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n149_), .O(new_n151_));
  inv1   g089(.a(new_n139_), .O(new_n152_));
  nor2   g090(.a(new_n63_), .b(new_n75_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n151_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n148_), .O(z14));
  inv1   g094(.a(x12), .O(new_n157_));
  oai21  g095(.a(new_n112_), .b(new_n157_), .c(new_n103_), .O(z15));
  inv1   g096(.a(x23), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(x22), .c(x01), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n103_), .O(z16));
  nor2   g099(.a(x24), .b(new_n159_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(new_n103_), .c(x22), .d(x01), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z17));
  nand3  g102(.a(x39), .b(new_n144_), .c(new_n123_), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n128_), .c(new_n127_), .O(z18));
  inv1   g105(.a(z18), .O(z19));
  oai21  g106(.a(new_n89_), .b(new_n75_), .c(new_n166_), .O(new_n169_));
  aoi21  g107(.a(new_n125_), .b(x08), .c(new_n169_), .O(z20));
endmodule


