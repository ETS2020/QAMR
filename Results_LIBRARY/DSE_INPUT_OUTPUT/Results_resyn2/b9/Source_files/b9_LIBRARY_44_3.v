// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n162_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n174_, new_n175_,
    new_n177_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x38), .O(new_n71_));
  inv1   g009(.a(x40), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  oai21  g012(.a(new_n69_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(new_n73_), .O(new_n76_));
  inv1   g014(.a(x27), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  aoi21  g016(.a(x35), .b(new_n78_), .c(x36), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  oai21  g018(.a(new_n79_), .b(new_n77_), .c(new_n80_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n76_), .c(x04), .O(z01));
  inv1   g020(.a(x08), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n78_), .O(new_n84_));
  aoi21  g022(.a(new_n77_), .b(new_n83_), .c(new_n84_), .O(new_n85_));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nand3  g024(.a(x40), .b(x39), .c(new_n71_), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g026(.a(new_n85_), .b(new_n63_), .c(new_n88_), .O(z02));
  nand2  g027(.a(x35), .b(x28), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x27), .O(new_n91_));
  nor2   g029(.a(x37), .b(x27), .O(new_n92_));
  nor2   g030(.a(new_n92_), .b(new_n73_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n91_), .c(new_n95_), .O(z04));
  aoi21  g034(.a(x28), .b(x27), .c(x37), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n76_), .O(z05));
  inv1   g036(.a(new_n97_), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n76_), .O(z06));
  inv1   g038(.a(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  aoi21  g040(.a(x25), .b(new_n102_), .c(new_n71_), .O(new_n103_));
  oai21  g041(.a(x25), .b(new_n101_), .c(new_n103_), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n76_), .O(z07));
  inv1   g048(.a(new_n87_), .O(z08));
  inv1   g049(.a(x11), .O(new_n112_));
  nand4  g050(.a(new_n76_), .b(x34), .c(x27), .d(x26), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n112_), .O(z09));
  nor2   g052(.a(new_n79_), .b(new_n77_), .O(new_n115_));
  nand3  g053(.a(x40), .b(x39), .c(x05), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  inv1   g055(.a(x07), .O(new_n118_));
  aoi21  g056(.a(x40), .b(x39), .c(new_n118_), .O(new_n119_));
  aoi22  g057(.a(new_n119_), .b(new_n115_), .c(new_n117_), .d(new_n81_), .O(new_n120_));
  nand3  g058(.a(x37), .b(x27), .c(x06), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n73_), .O(new_n123_));
  oai21  g061(.a(new_n120_), .b(x04), .c(new_n123_), .O(z10));
  nand3  g062(.a(x39), .b(x29), .c(x08), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n71_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x40), .O(new_n127_));
  nor2   g065(.a(x30), .b(x09), .O(new_n128_));
  oai21  g066(.a(new_n77_), .b(new_n63_), .c(new_n85_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n127_), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(z11));
  nor2   g069(.a(new_n67_), .b(new_n77_), .O(new_n132_));
  inv1   g070(.a(x10), .O(new_n133_));
  oai21  g071(.a(new_n66_), .b(new_n77_), .c(new_n133_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(x39), .c(new_n63_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n71_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x40), .c(new_n132_), .O(z12));
  inv1   g075(.a(x13), .O(new_n138_));
  nand3  g076(.a(x39), .b(new_n138_), .c(new_n63_), .O(new_n139_));
  inv1   g077(.a(new_n80_), .O(new_n140_));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(new_n140_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n139_), .c(new_n71_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x40), .O(new_n145_));
  inv1   g083(.a(x18), .O(new_n146_));
  inv1   g084(.a(x19), .O(new_n147_));
  nand3  g085(.a(x20), .b(new_n147_), .c(new_n146_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n132_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n145_), .O(z13));
  inv1   g089(.a(new_n139_), .O(new_n152_));
  nand3  g090(.a(new_n141_), .b(new_n80_), .c(x28), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n152_), .c(x38), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n72_), .c(new_n148_), .O(new_n155_));
  oai21  g093(.a(x28), .b(new_n77_), .c(new_n80_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n152_), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n71_), .c(new_n72_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n132_), .c(new_n155_), .O(z14));
  inv1   g097(.a(x12), .O(new_n160_));
  nor2   g098(.a(new_n113_), .b(new_n160_), .O(z15));
  nand2  g099(.a(x22), .b(x01), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(x23), .c(new_n76_), .O(z16));
  inv1   g101(.a(x23), .O(new_n164_));
  nor4   g102(.a(new_n162_), .b(new_n73_), .c(x24), .d(new_n164_), .O(z17));
  nand3  g103(.a(x40), .b(x39), .c(x29), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n84_), .c(new_n83_), .O(new_n167_));
  nand2  g105(.a(new_n166_), .b(x27), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  nand4  g107(.a(x35), .b(new_n78_), .c(x27), .d(new_n63_), .O(new_n170_));
  nand2  g108(.a(new_n128_), .b(new_n76_), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n170_), .c(new_n169_), .O(z18));
  nand2  g111(.a(new_n129_), .b(new_n128_), .O(new_n174_));
  nor3   g112(.a(new_n166_), .b(x38), .c(new_n83_), .O(new_n175_));
  aoi21  g113(.a(new_n174_), .b(new_n76_), .c(new_n175_), .O(z19));
  and2   g114(.a(x35), .b(x27), .O(new_n177_));
  nor3   g115(.a(new_n177_), .b(new_n171_), .c(new_n167_), .O(z20));
endmodule


