// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n93_, new_n94_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n165_, new_n166_, new_n167_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n175_, new_n176_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x14), .O(new_n71_));
  nor2   g009(.a(x39), .b(new_n71_), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x35), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x28), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(x36), .c(x27), .O(new_n77_));
  nor2   g015(.a(x32), .b(x30), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x04), .c(new_n72_), .O(z01));
  oai21  g018(.a(x27), .b(x08), .c(new_n76_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x04), .O(new_n82_));
  nand2  g020(.a(x40), .b(x39), .O(new_n83_));
  aoi21  g021(.a(x29), .b(x08), .c(x02), .O(new_n84_));
  nor2   g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n82_), .O(z02));
  inv1   g024(.a(x37), .O(new_n87_));
  nand2  g025(.a(x28), .b(x27), .O(new_n88_));
  oai22  g026(.a(new_n88_), .b(new_n75_), .c(new_n87_), .d(x27), .O(new_n89_));
  aoi21  g027(.a(new_n89_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g028(.a(x21), .O(new_n91_));
  aoi21  g029(.a(new_n89_), .b(new_n91_), .c(new_n72_), .O(z04));
  inv1   g030(.a(x39), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x14), .O(new_n94_));
  nand3  g032(.a(new_n88_), .b(new_n94_), .c(new_n87_), .O(z05));
  nand2  g033(.a(new_n88_), .b(new_n87_), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n94_), .O(z06));
  inv1   g035(.a(x25), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(x00), .O(new_n99_));
  nand2  g037(.a(x25), .b(new_n71_), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n99_), .c(x38), .O(new_n101_));
  inv1   g039(.a(x17), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(x15), .O(new_n103_));
  or2    g041(.a(x33), .b(x31), .O(new_n104_));
  oai21  g042(.a(new_n104_), .b(new_n103_), .c(x14), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n101_), .c(x03), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(new_n94_), .O(z07));
  inv1   g045(.a(new_n83_), .O(z08));
  nand3  g046(.a(x34), .b(x27), .c(x26), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n94_), .c(x11), .O(new_n111_));
  inv1   g049(.a(new_n111_), .O(z09));
  inv1   g050(.a(x27), .O(new_n113_));
  nand2  g051(.a(z08), .b(x05), .O(new_n114_));
  nand3  g052(.a(new_n83_), .b(new_n94_), .c(x07), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  inv1   g054(.a(x36), .O(new_n117_));
  inv1   g055(.a(x28), .O(new_n118_));
  nand2  g056(.a(x35), .b(new_n118_), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(new_n117_), .c(x04), .O(new_n120_));
  nand2  g058(.a(x37), .b(x06), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n72_), .O(new_n122_));
  aoi21  g060(.a(new_n120_), .b(new_n116_), .c(new_n122_), .O(new_n123_));
  inv1   g061(.a(new_n78_), .O(new_n124_));
  nand4  g062(.a(z08), .b(new_n124_), .c(x05), .d(new_n63_), .O(new_n125_));
  oai21  g063(.a(new_n123_), .b(new_n113_), .c(new_n125_), .O(z10));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  inv1   g066(.a(x08), .O(new_n129_));
  aoi21  g067(.a(new_n127_), .b(new_n119_), .c(new_n129_), .O(new_n130_));
  oai21  g068(.a(new_n128_), .b(new_n113_), .c(new_n130_), .O(new_n131_));
  nand3  g069(.a(new_n76_), .b(x27), .c(new_n63_), .O(new_n132_));
  nor2   g070(.a(x30), .b(x09), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(new_n131_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n94_), .O(z11));
  nand2  g073(.a(new_n68_), .b(x27), .O(new_n136_));
  nand2  g074(.a(new_n65_), .b(x10), .O(new_n137_));
  nand3  g075(.a(new_n94_), .b(new_n137_), .c(new_n136_), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(z12));
  inv1   g077(.a(x13), .O(new_n140_));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(z08), .c(new_n140_), .d(new_n63_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand3  g083(.a(x20), .b(new_n145_), .c(new_n144_), .O(new_n146_));
  inv1   g084(.a(new_n66_), .O(new_n147_));
  nand2  g085(.a(new_n94_), .b(new_n147_), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n146_), .c(new_n143_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  nand3  g088(.a(z08), .b(new_n140_), .c(new_n63_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n124_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n150_), .O(z13));
  nand3  g092(.a(new_n141_), .b(new_n78_), .c(x28), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n151_), .c(new_n146_), .O(new_n157_));
  aoi21  g095(.a(new_n118_), .b(x27), .c(new_n124_), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n151_), .c(new_n66_), .d(new_n113_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n157_), .c(new_n94_), .O(z14));
  nand3  g098(.a(new_n110_), .b(new_n94_), .c(x12), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z15));
  nand2  g100(.a(x22), .b(x01), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(x23), .c(new_n94_), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  inv1   g103(.a(new_n163_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n94_), .c(new_n165_), .d(x23), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z17));
  nand4  g106(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n94_), .O(z18));
  nor2   g107(.a(new_n113_), .b(new_n63_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n81_), .c(new_n133_), .O(new_n171_));
  nand2  g109(.a(new_n128_), .b(x08), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(new_n173_));
  aoi21  g111(.a(new_n171_), .b(new_n94_), .c(new_n173_), .O(z19));
  nand2  g112(.a(x35), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n133_), .c(new_n94_), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(new_n130_), .O(z20));
endmodule


