// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n167_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n176_, new_n177_, new_n178_, new_n179_, new_n180_, new_n181_,
    new_n184_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x26), .O(new_n71_));
  nand2  g009(.a(x40), .b(new_n71_), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  inv1   g020(.a(new_n72_), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(new_n65_), .O(new_n84_));
  oai21  g022(.a(new_n82_), .b(new_n79_), .c(new_n84_), .O(z01));
  inv1   g023(.a(x35), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(x28), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  nand2  g026(.a(new_n75_), .b(new_n88_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  and2   g028(.a(x29), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(x02), .c(x39), .O(new_n92_));
  aoi21  g030(.a(new_n90_), .b(x04), .c(new_n92_), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n71_), .c(x40), .O(z02));
  nand2  g032(.a(x35), .b(x28), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x27), .O(new_n96_));
  inv1   g034(.a(x37), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n75_), .c(new_n83_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n96_), .c(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n96_), .c(new_n100_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n72_), .O(z05));
  inv1   g041(.a(new_n102_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n72_), .O(z06));
  inv1   g043(.a(x14), .O(new_n106_));
  inv1   g044(.a(x15), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n107_), .O(new_n108_));
  nor2   g046(.a(x33), .b(x31), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n108_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  nand2  g050(.a(x25), .b(new_n106_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n72_), .c(x03), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n110_), .O(z07));
  nand3  g054(.a(x40), .b(x39), .c(x26), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z08));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z09));
  nand2  g058(.a(x40), .b(x39), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  nand2  g060(.a(new_n82_), .b(new_n122_), .O(new_n123_));
  oai21  g061(.a(new_n86_), .b(x28), .c(new_n76_), .O(new_n124_));
  inv1   g062(.a(x07), .O(new_n125_));
  nand2  g063(.a(new_n121_), .b(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n124_), .c(x27), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  inv1   g066(.a(x05), .O(new_n129_));
  aoi21  g067(.a(new_n122_), .b(new_n129_), .c(x04), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand3  g069(.a(x37), .b(x27), .c(x06), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n72_), .O(z10));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n88_), .c(x04), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x26), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x40), .O(new_n137_));
  nand3  g075(.a(x40), .b(x39), .c(x29), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(x27), .c(x04), .O(new_n139_));
  nand2  g077(.a(new_n138_), .b(new_n78_), .O(new_n140_));
  oai21  g078(.a(new_n78_), .b(new_n75_), .c(new_n88_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n139_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n134_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n137_), .O(z11));
  nand2  g082(.a(new_n67_), .b(x27), .O(new_n145_));
  nand2  g083(.a(new_n68_), .b(new_n145_), .O(new_n146_));
  nand2  g084(.a(new_n72_), .b(new_n146_), .O(z12));
  nand2  g085(.a(new_n95_), .b(new_n97_), .O(new_n148_));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand4  g087(.a(new_n149_), .b(new_n72_), .c(new_n148_), .d(x20), .O(new_n150_));
  nor2   g088(.a(x13), .b(x04), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x28), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(new_n151_), .c(new_n122_), .d(x26), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n150_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x27), .O(new_n156_));
  nand4  g094(.a(new_n151_), .b(new_n82_), .c(new_n122_), .d(x26), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(z13));
  nand2  g096(.a(new_n149_), .b(x20), .O(new_n159_));
  nand4  g097(.a(new_n152_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(new_n161_));
  nand2  g099(.a(new_n151_), .b(new_n122_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n161_), .c(new_n159_), .O(new_n163_));
  aoi21  g101(.a(new_n77_), .b(x27), .c(new_n82_), .O(new_n164_));
  oai22  g102(.a(new_n164_), .b(new_n162_), .c(new_n63_), .d(new_n75_), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n163_), .c(new_n72_), .O(z14));
  nand4  g104(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n72_), .O(z15));
  inv1   g106(.a(x01), .O(new_n169_));
  inv1   g107(.a(x22), .O(new_n170_));
  nor4   g108(.a(new_n83_), .b(x23), .c(new_n170_), .d(new_n169_), .O(z16));
  inv1   g109(.a(x24), .O(new_n172_));
  nor2   g110(.a(new_n170_), .b(new_n169_), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(new_n72_), .c(new_n172_), .d(x23), .O(new_n174_));
  inv1   g112(.a(new_n174_), .O(z17));
  aoi21  g113(.a(new_n138_), .b(new_n78_), .c(new_n88_), .O(new_n176_));
  nand2  g114(.a(new_n138_), .b(x27), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n176_), .O(new_n178_));
  nand3  g116(.a(new_n87_), .b(x27), .c(new_n65_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n134_), .c(new_n72_), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n178_), .O(z18));
  aoi21  g120(.a(new_n177_), .b(new_n176_), .c(new_n180_), .O(z19));
  oai21  g121(.a(new_n86_), .b(new_n75_), .c(new_n134_), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n176_), .c(new_n72_), .O(z20));
endmodule


