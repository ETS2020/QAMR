// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n100_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n152_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  nand2  g002(.a(x36), .b(new_n64_), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(x36), .b(x35), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  nand2  g007(.a(x40), .b(x39), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  oai21  g010(.a(new_n72_), .b(new_n68_), .c(new_n67_), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(x10), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  aoi21  g014(.a(new_n73_), .b(x27), .c(new_n76_), .O(z12));
  oai21  g015(.a(z12), .b(x15), .c(new_n66_), .O(z00));
  oai21  g016(.a(x32), .b(x30), .c(new_n74_), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  aoi21  g018(.a(x35), .b(new_n80_), .c(x36), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n64_), .c(new_n79_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x04), .O(z01));
  oai21  g021(.a(x08), .b(new_n69_), .c(new_n74_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n64_), .O(new_n85_));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g026(.a(x35), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(x28), .O(new_n90_));
  inv1   g028(.a(new_n90_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x04), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n88_), .c(new_n85_), .d(new_n71_), .O(z02));
  oai21  g031(.a(new_n89_), .b(new_n80_), .c(x27), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(x36), .c(new_n64_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n94_), .c(new_n98_), .O(z04));
  aoi21  g037(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n65_), .O(z05));
  inv1   g039(.a(new_n100_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n65_), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  inv1   g042(.a(x15), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n105_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n104_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n65_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nand2  g052(.a(new_n70_), .b(new_n65_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n65_), .O(z09));
  nor2   g055(.a(new_n81_), .b(new_n64_), .O(new_n118_));
  nand2  g056(.a(new_n71_), .b(x05), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  and2   g058(.a(new_n70_), .b(x07), .O(new_n121_));
  aoi22  g059(.a(new_n121_), .b(new_n118_), .c(new_n120_), .d(new_n82_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  oai21  g061(.a(new_n122_), .b(x04), .c(new_n123_), .O(z10));
  nor2   g062(.a(x30), .b(x09), .O(new_n125_));
  oai21  g063(.a(new_n87_), .b(new_n70_), .c(new_n125_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n65_), .O(new_n127_));
  nand3  g065(.a(new_n74_), .b(new_n64_), .c(x08), .O(new_n128_));
  oai21  g066(.a(new_n64_), .b(x04), .c(new_n128_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n90_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(z18));
  inv1   g069(.a(z18), .O(z11));
  nor2   g070(.a(x13), .b(x04), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n71_), .O(new_n134_));
  oai21  g072(.a(new_n134_), .b(new_n89_), .c(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x36), .O(new_n136_));
  nor2   g074(.a(x32), .b(x30), .O(new_n137_));
  or2    g075(.a(new_n134_), .b(new_n137_), .O(new_n138_));
  inv1   g076(.a(new_n67_), .O(new_n139_));
  nor2   g077(.a(x19), .b(x18), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n139_), .c(x20), .O(new_n141_));
  oai21  g079(.a(new_n134_), .b(x28), .c(new_n141_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x27), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n138_), .c(new_n136_), .O(z13));
  nor2   g082(.a(new_n67_), .b(new_n74_), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(new_n133_), .c(new_n71_), .d(x35), .O(new_n146_));
  nand2  g084(.a(new_n137_), .b(x28), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n133_), .c(new_n71_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n146_), .c(new_n141_), .O(new_n149_));
  nor2   g087(.a(new_n134_), .b(new_n79_), .O(new_n150_));
  aoi21  g088(.a(new_n149_), .b(x27), .c(new_n150_), .O(z14));
  nand4  g089(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(z15));
  nand2  g091(.a(x22), .b(x01), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(x23), .c(new_n65_), .O(z16));
  inv1   g093(.a(x24), .O(new_n156_));
  inv1   g094(.a(new_n154_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(new_n65_), .c(new_n156_), .d(x23), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z17));
  nand2  g097(.a(new_n90_), .b(x08), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n74_), .c(x27), .O(new_n161_));
  nand3  g099(.a(new_n90_), .b(x27), .c(new_n69_), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n161_), .c(new_n126_), .O(z19));
  nand2  g102(.a(new_n71_), .b(x29), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n91_), .O(new_n166_));
  nand2  g104(.a(x35), .b(x27), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n125_), .c(new_n65_), .O(new_n168_));
  aoi21  g106(.a(new_n166_), .b(x08), .c(new_n168_), .O(z20));
endmodule


