// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n159_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n167_, new_n168_, new_n170_,
    new_n171_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(x10), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n67_), .b(x27), .c(new_n71_), .O(new_n72_));
  inv1   g010(.a(x16), .O(new_n73_));
  inv1   g011(.a(x19), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  oai21  g015(.a(new_n72_), .b(x15), .c(new_n77_), .O(z00));
  inv1   g016(.a(x27), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  aoi21  g018(.a(x35), .b(new_n80_), .c(x36), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  oai21  g020(.a(new_n81_), .b(new_n79_), .c(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n76_), .O(z01));
  nand2  g022(.a(x35), .b(new_n80_), .O(new_n85_));
  nor2   g023(.a(x27), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n85_), .c(x04), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nand3  g026(.a(x40), .b(x39), .c(new_n74_), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n87_), .O(z02));
  inv1   g029(.a(x35), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n92_), .c(new_n93_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n76_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n95_), .b(new_n97_), .c(new_n76_), .O(z04));
  aoi21  g036(.a(new_n94_), .b(new_n93_), .c(new_n76_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(new_n76_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n101_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n106_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n105_), .O(z07));
  inv1   g050(.a(new_n89_), .O(z08));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n106_), .O(z09));
  nor2   g053(.a(new_n81_), .b(new_n79_), .O(new_n116_));
  nand2  g054(.a(new_n69_), .b(x05), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  and2   g056(.a(new_n68_), .b(x07), .O(new_n119_));
  aoi22  g057(.a(new_n119_), .b(new_n116_), .c(new_n118_), .d(new_n83_), .O(new_n120_));
  nand3  g058(.a(x37), .b(x27), .c(x06), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n76_), .O(new_n123_));
  oai21  g061(.a(new_n120_), .b(x04), .c(new_n123_), .O(z10));
  nor2   g062(.a(x30), .b(x09), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  aoi21  g064(.a(x39), .b(x29), .c(x19), .O(new_n127_));
  nand2  g065(.a(x27), .b(x04), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(x35), .c(new_n80_), .O(new_n129_));
  oai21  g067(.a(new_n127_), .b(new_n75_), .c(new_n129_), .O(new_n130_));
  inv1   g068(.a(x08), .O(new_n131_));
  oai21  g069(.a(new_n85_), .b(x04), .c(new_n74_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x27), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n106_), .c(new_n131_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n130_), .c(new_n126_), .O(z11));
  and2   g073(.a(new_n67_), .b(x27), .O(new_n136_));
  oai21  g074(.a(new_n71_), .b(new_n136_), .c(new_n106_), .O(z12));
  inv1   g075(.a(new_n82_), .O(new_n138_));
  nor2   g076(.a(x13), .b(x04), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(x39), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n74_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x40), .O(new_n142_));
  aoi21  g080(.a(x36), .b(x35), .c(new_n80_), .O(new_n143_));
  nand2  g081(.a(new_n139_), .b(new_n69_), .O(new_n144_));
  inv1   g082(.a(x18), .O(new_n145_));
  nand3  g083(.a(x20), .b(new_n74_), .c(new_n145_), .O(new_n146_));
  oai22  g084(.a(new_n146_), .b(new_n63_), .c(new_n144_), .d(new_n143_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n142_), .O(z13));
  nand2  g087(.a(x20), .b(new_n145_), .O(new_n150_));
  nand2  g088(.a(new_n143_), .b(new_n82_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n144_), .c(new_n150_), .O(new_n153_));
  aoi21  g091(.a(new_n80_), .b(x27), .c(new_n138_), .O(new_n154_));
  oai22  g092(.a(new_n154_), .b(new_n144_), .c(new_n63_), .d(new_n79_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n153_), .c(new_n74_), .O(z14));
  nand4  g094(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n106_), .O(z15));
  nand2  g096(.a(x22), .b(x01), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(x23), .c(new_n106_), .O(z16));
  inv1   g098(.a(x23), .O(new_n161_));
  nor4   g099(.a(new_n159_), .b(new_n76_), .c(x24), .d(new_n161_), .O(z17));
  oai21  g100(.a(new_n129_), .b(new_n86_), .c(new_n125_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n106_), .O(new_n164_));
  nand3  g102(.a(z08), .b(x29), .c(x08), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n164_), .O(z18));
  nand3  g104(.a(x40), .b(x39), .c(x29), .O(new_n167_));
  nor3   g105(.a(new_n167_), .b(x19), .c(new_n131_), .O(new_n168_));
  aoi21  g106(.a(new_n163_), .b(new_n106_), .c(new_n168_), .O(z19));
  aoi21  g107(.a(new_n167_), .b(new_n85_), .c(new_n131_), .O(new_n170_));
  nor2   g108(.a(new_n92_), .b(new_n79_), .O(new_n171_));
  nor4   g109(.a(new_n171_), .b(new_n170_), .c(new_n126_), .d(new_n76_), .O(z20));
endmodule


