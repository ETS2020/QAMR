// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n179_, new_n181_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x36), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g013(.a(x40), .b(x39), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(x04), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(new_n75_), .c(new_n66_), .O(new_n78_));
  oai21  g016(.a(new_n72_), .b(new_n65_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(new_n80_), .c(new_n68_), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  inv1   g023(.a(x04), .O(new_n86_));
  nor2   g024(.a(new_n65_), .b(new_n86_), .O(new_n87_));
  oai21  g025(.a(new_n85_), .b(new_n83_), .c(new_n87_), .O(z01));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n82_), .c(x04), .O(new_n90_));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n76_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n90_), .O(z02));
  nand3  g031(.a(x35), .b(x28), .c(x27), .O(new_n94_));
  oai21  g032(.a(new_n69_), .b(x27), .c(new_n94_), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  aoi21  g035(.a(new_n95_), .b(new_n97_), .c(new_n65_), .O(z04));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n65_), .O(z05));
  and2   g038(.a(new_n99_), .b(new_n64_), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n66_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n102_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n64_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  inv1   g049(.a(new_n76_), .O(z08));
  and2   g050(.a(x34), .b(x27), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(x26), .c(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n64_), .O(z09));
  inv1   g053(.a(x35), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(x28), .c(new_n80_), .O(new_n117_));
  inv1   g055(.a(x07), .O(new_n118_));
  inv1   g056(.a(x39), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(new_n118_), .c(x04), .O(new_n120_));
  oai21  g058(.a(x36), .b(new_n118_), .c(new_n63_), .O(new_n121_));
  inv1   g059(.a(x05), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(new_n122_), .O(new_n123_));
  nand4  g061(.a(new_n123_), .b(new_n121_), .c(new_n120_), .d(new_n117_), .O(new_n124_));
  nand3  g062(.a(new_n64_), .b(x37), .c(x06), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x27), .O(new_n127_));
  inv1   g065(.a(new_n77_), .O(new_n128_));
  nor2   g066(.a(new_n84_), .b(new_n128_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x05), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(z10));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  nor2   g072(.a(new_n116_), .b(x28), .O(new_n135_));
  oai21  g073(.a(new_n68_), .b(new_n86_), .c(new_n135_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n134_), .c(new_n64_), .O(new_n137_));
  inv1   g075(.a(x08), .O(new_n138_));
  nand3  g076(.a(new_n135_), .b(x27), .c(new_n86_), .O(new_n139_));
  oai21  g077(.a(x36), .b(x27), .c(new_n63_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n139_), .c(new_n138_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(new_n137_), .c(new_n133_), .O(z11));
  nor3   g080(.a(x37), .b(x36), .c(x35), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n68_), .c(new_n73_), .O(new_n144_));
  inv1   g082(.a(new_n71_), .O(new_n145_));
  nand2  g083(.a(new_n128_), .b(new_n145_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n144_), .c(new_n64_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  nand2  g086(.a(x36), .b(x35), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x28), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n77_), .c(new_n148_), .O(new_n151_));
  nand2  g089(.a(new_n70_), .b(new_n69_), .O(new_n152_));
  nor2   g090(.a(x19), .b(x18), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(new_n152_), .c(new_n64_), .d(x20), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n151_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x27), .O(new_n156_));
  nand2  g094(.a(new_n129_), .b(new_n148_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(z13));
  nand2  g096(.a(new_n77_), .b(new_n148_), .O(new_n159_));
  nand2  g097(.a(new_n153_), .b(x20), .O(new_n160_));
  nand3  g098(.a(new_n149_), .b(new_n84_), .c(x28), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n159_), .c(new_n160_), .O(new_n163_));
  aoi21  g101(.a(new_n81_), .b(x27), .c(new_n85_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n159_), .c(new_n145_), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n163_), .c(new_n64_), .O(z14));
  nand4  g104(.a(new_n113_), .b(new_n64_), .c(x26), .d(x12), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z15));
  inv1   g106(.a(x23), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(x22), .c(x01), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n64_), .O(z16));
  nor2   g109(.a(x24), .b(new_n169_), .O(new_n172_));
  nand4  g110(.a(new_n172_), .b(new_n64_), .c(x22), .d(x01), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(z17));
  aoi21  g112(.a(new_n134_), .b(new_n82_), .c(new_n138_), .O(new_n175_));
  nand2  g113(.a(new_n134_), .b(x27), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n175_), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n139_), .c(new_n132_), .d(new_n64_), .O(z18));
  nand3  g116(.a(new_n177_), .b(new_n139_), .c(new_n132_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n64_), .O(z19));
  oai21  g118(.a(new_n116_), .b(new_n68_), .c(new_n132_), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n175_), .c(new_n64_), .O(z20));
endmodule


