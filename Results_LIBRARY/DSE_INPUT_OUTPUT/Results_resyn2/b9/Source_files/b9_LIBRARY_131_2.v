// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n160_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n170_, new_n171_;
  inv1   g000(.a(x19), .O(new_n63_));
  nor2   g001(.a(x39), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  oai21  g009(.a(x36), .b(x35), .c(x27), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(x04), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(new_n73_), .c(new_n65_), .O(new_n76_));
  oai21  g014(.a(new_n70_), .b(new_n64_), .c(new_n76_), .O(z00));
  inv1   g015(.a(new_n64_), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  aoi21  g017(.a(x35), .b(new_n79_), .c(x36), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(new_n67_), .c(new_n81_), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g021(.a(x35), .O(new_n84_));
  nor2   g022(.a(new_n84_), .b(x28), .O(new_n85_));
  oai21  g023(.a(x27), .b(x08), .c(new_n85_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x04), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n74_), .O(new_n89_));
  aoi21  g027(.a(new_n89_), .b(new_n87_), .c(new_n64_), .O(z02));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n84_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  aoi21  g033(.a(new_n93_), .b(new_n95_), .c(new_n64_), .O(z04));
  nand2  g034(.a(new_n92_), .b(new_n91_), .O(new_n97_));
  inv1   g035(.a(new_n97_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n78_), .O(z05));
  nand2  g037(.a(new_n97_), .b(new_n78_), .O(z06));
  inv1   g038(.a(x25), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x25), .b(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(x38), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x15), .O(new_n107_));
  or2    g045(.a(x33), .b(x31), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(new_n107_), .c(x14), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n105_), .c(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n78_), .O(z07));
  nand2  g049(.a(new_n74_), .b(new_n78_), .O(z08));
  and2   g050(.a(x34), .b(x27), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(x26), .c(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n78_), .O(z09));
  nor2   g053(.a(new_n80_), .b(new_n67_), .O(new_n116_));
  inv1   g054(.a(new_n74_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x05), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  and2   g057(.a(new_n74_), .b(x07), .O(new_n120_));
  aoi22  g058(.a(new_n120_), .b(new_n116_), .c(new_n119_), .d(new_n82_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n64_), .O(new_n124_));
  oai21  g062(.a(new_n121_), .b(x04), .c(new_n124_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  inv1   g064(.a(new_n85_), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  inv1   g069(.a(x04), .O(new_n132_));
  nand3  g070(.a(new_n85_), .b(x27), .c(new_n132_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n131_), .c(new_n78_), .O(new_n134_));
  aoi21  g072(.a(new_n130_), .b(new_n129_), .c(new_n134_), .O(z11));
  oai21  g073(.a(x36), .b(x35), .c(new_n75_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n68_), .c(new_n67_), .O(new_n137_));
  nand2  g075(.a(new_n75_), .b(x10), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n137_), .c(new_n78_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  nand2  g079(.a(new_n75_), .b(new_n141_), .O(new_n142_));
  aoi21  g080(.a(x36), .b(x35), .c(new_n79_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  nand2  g082(.a(x39), .b(x19), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(x20), .c(new_n144_), .O(new_n146_));
  oai22  g084(.a(new_n146_), .b(new_n68_), .c(new_n143_), .d(new_n142_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  nor2   g086(.a(new_n142_), .b(new_n81_), .O(new_n149_));
  nor2   g087(.a(new_n149_), .b(new_n64_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n148_), .O(z13));
  inv1   g089(.a(new_n143_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n81_), .c(new_n142_), .O(new_n154_));
  nand4  g092(.a(new_n69_), .b(x20), .c(new_n63_), .d(new_n144_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n78_), .O(new_n156_));
  nor2   g094(.a(new_n156_), .b(new_n154_), .O(z14));
  nand4  g095(.a(new_n113_), .b(new_n78_), .c(x26), .d(x12), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z15));
  nand2  g097(.a(x22), .b(x01), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(x23), .c(new_n78_), .O(z16));
  inv1   g099(.a(x24), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x23), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n160_), .c(new_n78_), .O(z17));
  nor2   g102(.a(new_n67_), .b(new_n132_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n86_), .c(new_n131_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n78_), .O(new_n167_));
  nand3  g105(.a(new_n117_), .b(x29), .c(x08), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(z18));
  nand2  g107(.a(new_n131_), .b(new_n78_), .O(new_n170_));
  nor2   g108(.a(new_n84_), .b(new_n67_), .O(new_n171_));
  nor3   g109(.a(new_n171_), .b(new_n170_), .c(new_n129_), .O(z20));
  aoi21  g110(.a(new_n130_), .b(new_n129_), .c(new_n134_), .O(z19));
endmodule


