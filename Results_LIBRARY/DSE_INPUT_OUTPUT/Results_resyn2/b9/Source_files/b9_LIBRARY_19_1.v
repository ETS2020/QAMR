// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n149_, new_n151_, new_n153_, new_n154_, new_n155_,
    new_n158_, new_n160_, new_n161_;
  inv1   g00(.a(x38), .O(new_n63_));
  nor2   g01(.a(new_n63_), .b(x31), .O(new_n64_));
  inv1   g02(.a(x15), .O(new_n65_));
  inv1   g03(.a(x27), .O(new_n66_));
  aoi21  g04(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g05(.a(x04), .O(new_n68_));
  inv1   g06(.a(x35), .O(new_n69_));
  inv1   g07(.a(x36), .O(new_n70_));
  nand2  g08(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  and2   g09(.a(x40), .b(x39), .O(new_n72_));
  nand3  g10(.a(new_n72_), .b(new_n71_), .c(new_n68_), .O(new_n73_));
  aoi21  g11(.a(new_n73_), .b(new_n67_), .c(new_n66_), .O(new_n74_));
  nand3  g12(.a(new_n72_), .b(x10), .c(new_n68_), .O(new_n75_));
  inv1   g13(.a(new_n75_), .O(new_n76_));
  oai21  g14(.a(new_n76_), .b(new_n74_), .c(new_n65_), .O(new_n77_));
  aoi21  g15(.a(new_n77_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g16(.a(x30), .O(new_n79_));
  inv1   g17(.a(x32), .O(new_n80_));
  inv1   g18(.a(x28), .O(new_n81_));
  nand2  g19(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g20(.a(new_n82_), .b(new_n70_), .O(new_n83_));
  nand2  g21(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g22(.a(new_n84_), .b(new_n80_), .c(new_n79_), .O(new_n85_));
  aoi21  g23(.a(new_n85_), .b(x04), .c(new_n64_), .O(z01));
  nor2   g24(.a(x27), .b(x08), .O(new_n87_));
  oai21  g25(.a(new_n87_), .b(new_n82_), .c(x04), .O(new_n88_));
  nand2  g26(.a(x40), .b(x39), .O(new_n89_));
  aoi21  g27(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g28(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  aoi21  g29(.a(new_n91_), .b(new_n88_), .c(new_n64_), .O(z02));
  inv1   g30(.a(x37), .O(new_n93_));
  nand2  g31(.a(x28), .b(x27), .O(new_n94_));
  oai22  g32(.a(new_n94_), .b(new_n69_), .c(new_n93_), .d(x27), .O(new_n95_));
  aoi21  g33(.a(new_n95_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g34(.a(x21), .O(new_n97_));
  inv1   g35(.a(new_n64_), .O(new_n98_));
  nand3  g36(.a(new_n95_), .b(new_n98_), .c(new_n97_), .O(z04));
  aoi21  g37(.a(new_n94_), .b(new_n93_), .c(new_n64_), .O(z05));
  nand2  g38(.a(new_n94_), .b(new_n93_), .O(new_n101_));
  nor2   g39(.a(new_n101_), .b(new_n64_), .O(z06));
  nand2  g40(.a(x17), .b(new_n65_), .O(new_n103_));
  nor2   g41(.a(x33), .b(x31), .O(new_n104_));
  nand2  g42(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  oai21  g43(.a(x25), .b(x00), .c(x31), .O(new_n106_));
  nand2  g44(.a(new_n106_), .b(x38), .O(new_n107_));
  nand2  g45(.a(new_n107_), .b(x03), .O(new_n108_));
  aoi21  g46(.a(new_n105_), .b(x14), .c(new_n108_), .O(z07));
  nor2   g47(.a(new_n89_), .b(new_n64_), .O(z08));
  inv1   g48(.a(x11), .O(new_n111_));
  nand3  g49(.a(x34), .b(x27), .c(x26), .O(new_n112_));
  nor3   g50(.a(new_n112_), .b(new_n64_), .c(new_n111_), .O(z09));
  nand2  g51(.a(new_n80_), .b(new_n79_), .O(new_n114_));
  nand2  g52(.a(new_n114_), .b(new_n72_), .O(new_n115_));
  inv1   g53(.a(x07), .O(new_n116_));
  nand2  g54(.a(new_n89_), .b(new_n116_), .O(new_n117_));
  nand3  g55(.a(new_n117_), .b(new_n83_), .c(x27), .O(new_n118_));
  nand2  g56(.a(new_n118_), .b(new_n115_), .O(new_n119_));
  inv1   g57(.a(x05), .O(new_n120_));
  aoi21  g58(.a(new_n72_), .b(new_n120_), .c(x04), .O(new_n121_));
  nand2  g59(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand3  g60(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  aoi21  g61(.a(new_n123_), .b(new_n122_), .c(new_n64_), .O(z10));
  inv1   g62(.a(x08), .O(new_n125_));
  nand2  g63(.a(new_n72_), .b(x29), .O(new_n126_));
  aoi21  g64(.a(new_n126_), .b(new_n82_), .c(new_n125_), .O(new_n127_));
  nand2  g65(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g66(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nor2   g67(.a(new_n69_), .b(new_n66_), .O(new_n130_));
  nand3  g68(.a(new_n130_), .b(new_n81_), .c(new_n68_), .O(new_n131_));
  nor2   g69(.a(x30), .b(x09), .O(new_n132_));
  and2   g70(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand3  g71(.a(new_n133_), .b(new_n129_), .c(new_n98_), .O(new_n134_));
  inv1   g72(.a(new_n134_), .O(z11));
  oai21  g73(.a(new_n76_), .b(new_n74_), .c(new_n98_), .O(z12));
  nor2   g74(.a(x13), .b(x04), .O(new_n137_));
  nand3  g75(.a(new_n137_), .b(new_n114_), .c(new_n72_), .O(new_n138_));
  inv1   g76(.a(new_n138_), .O(new_n139_));
  inv1   g77(.a(new_n67_), .O(new_n140_));
  nor2   g78(.a(x19), .b(x18), .O(new_n141_));
  nand3  g79(.a(new_n141_), .b(new_n140_), .c(x20), .O(new_n142_));
  oai21  g80(.a(new_n70_), .b(new_n69_), .c(x28), .O(new_n143_));
  nand3  g81(.a(new_n143_), .b(new_n137_), .c(new_n72_), .O(new_n144_));
  aoi21  g82(.a(new_n144_), .b(new_n142_), .c(new_n66_), .O(new_n145_));
  nor2   g83(.a(new_n145_), .b(new_n139_), .O(new_n146_));
  nand2  g84(.a(new_n146_), .b(new_n98_), .O(z13));
  oai21  g85(.a(new_n145_), .b(new_n139_), .c(new_n98_), .O(z14));
  inv1   g86(.a(x12), .O(new_n149_));
  oai21  g87(.a(new_n112_), .b(new_n149_), .c(new_n98_), .O(z15));
  nand2  g88(.a(x22), .b(x01), .O(new_n151_));
  oai21  g89(.a(new_n151_), .b(x23), .c(new_n98_), .O(z16));
  inv1   g90(.a(x24), .O(new_n153_));
  inv1   g91(.a(new_n151_), .O(new_n154_));
  nand4  g92(.a(new_n154_), .b(new_n98_), .c(new_n153_), .d(x23), .O(new_n155_));
  inv1   g93(.a(new_n155_), .O(z17));
  aoi21  g94(.a(new_n133_), .b(new_n129_), .c(new_n64_), .O(z18));
  nand2  g95(.a(new_n133_), .b(new_n129_), .O(new_n158_));
  nand2  g96(.a(new_n158_), .b(new_n98_), .O(z19));
  nor2   g97(.a(new_n130_), .b(new_n64_), .O(new_n160_));
  nand2  g98(.a(new_n160_), .b(new_n132_), .O(new_n161_));
  nor2   g99(.a(new_n161_), .b(new_n127_), .O(z20));
endmodule


