// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n82_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n119_, new_n121_, new_n123_, new_n127_, new_n128_;
  inv1   g00(.a(x15), .O(new_n63_));
  inv1   g01(.a(x28), .O(new_n64_));
  inv1   g02(.a(x35), .O(new_n65_));
  inv1   g03(.a(x37), .O(new_n66_));
  oai21  g04(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  and2   g05(.a(new_n67_), .b(x27), .O(new_n68_));
  nand2  g06(.a(new_n68_), .b(new_n63_), .O(new_n69_));
  nand2  g07(.a(x40), .b(x39), .O(z02));
  nand3  g08(.a(z02), .b(new_n69_), .c(x16), .O(z00));
  inv1   g09(.a(z02), .O(z08));
  inv1   g10(.a(x36), .O(new_n73_));
  oai21  g11(.a(new_n65_), .b(x28), .c(new_n73_), .O(new_n74_));
  nand2  g12(.a(new_n74_), .b(x27), .O(new_n75_));
  nor2   g13(.a(x32), .b(x30), .O(new_n76_));
  nand2  g14(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g15(.a(new_n77_), .b(x04), .c(z08), .O(z01));
  nand2  g16(.a(x28), .b(x27), .O(new_n79_));
  oai22  g17(.a(new_n79_), .b(new_n65_), .c(new_n66_), .d(x27), .O(new_n80_));
  aoi21  g18(.a(new_n80_), .b(x21), .c(z08), .O(z03));
  inv1   g19(.a(x21), .O(new_n82_));
  nand3  g20(.a(new_n80_), .b(z02), .c(new_n82_), .O(z04));
  aoi21  g21(.a(new_n79_), .b(new_n66_), .c(z08), .O(z05));
  nand3  g22(.a(new_n79_), .b(z02), .c(new_n66_), .O(new_n85_));
  inv1   g23(.a(new_n85_), .O(z06));
  inv1   g24(.a(x00), .O(new_n87_));
  oai21  g25(.a(x25), .b(new_n87_), .c(x38), .O(new_n88_));
  nand2  g26(.a(x17), .b(new_n63_), .O(new_n89_));
  nor2   g27(.a(x33), .b(x31), .O(new_n90_));
  nand3  g28(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  inv1   g29(.a(x14), .O(new_n92_));
  inv1   g30(.a(x25), .O(new_n93_));
  nand3  g31(.a(x38), .b(new_n93_), .c(new_n87_), .O(new_n94_));
  nand2  g32(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g33(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand2  g34(.a(new_n96_), .b(x03), .O(new_n97_));
  nand2  g35(.a(new_n97_), .b(z02), .O(z07));
  nand4  g36(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n99_));
  nand2  g37(.a(new_n99_), .b(z02), .O(z09));
  nand2  g38(.a(z02), .b(x27), .O(new_n101_));
  nand2  g39(.a(x37), .b(x06), .O(new_n102_));
  inv1   g40(.a(x04), .O(new_n103_));
  nand3  g41(.a(new_n74_), .b(x07), .c(new_n103_), .O(new_n104_));
  aoi21  g42(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(z10));
  nor2   g43(.a(x30), .b(x09), .O(new_n106_));
  nor2   g44(.a(x27), .b(x08), .O(new_n107_));
  nand2  g45(.a(x27), .b(x04), .O(new_n108_));
  nand3  g46(.a(new_n108_), .b(x35), .c(new_n64_), .O(new_n109_));
  oai21  g47(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  nor2   g48(.a(new_n110_), .b(z08), .O(z11));
  nand3  g49(.a(z02), .b(new_n67_), .c(x27), .O(z12));
  nor2   g50(.a(x19), .b(x18), .O(new_n113_));
  nand2  g51(.a(new_n113_), .b(x20), .O(new_n114_));
  inv1   g52(.a(new_n114_), .O(new_n115_));
  nand2  g53(.a(new_n115_), .b(new_n68_), .O(new_n116_));
  nand2  g54(.a(new_n116_), .b(z02), .O(z13));
  aoi21  g55(.a(new_n115_), .b(new_n68_), .c(z08), .O(z14));
  nand3  g56(.a(x34), .b(x26), .c(x12), .O(new_n119_));
  nor2   g57(.a(new_n119_), .b(new_n101_), .O(z15));
  nand2  g58(.a(x22), .b(x01), .O(new_n121_));
  oai21  g59(.a(new_n121_), .b(x23), .c(z02), .O(z16));
  inv1   g60(.a(x23), .O(new_n123_));
  nor4   g61(.a(new_n121_), .b(z08), .c(x24), .d(new_n123_), .O(z17));
  nand2  g62(.a(new_n110_), .b(z02), .O(z19));
  inv1   g63(.a(z19), .O(z18));
  aoi21  g64(.a(new_n64_), .b(x08), .c(x27), .O(new_n127_));
  oai21  g65(.a(new_n127_), .b(new_n65_), .c(new_n106_), .O(new_n128_));
  nand2  g66(.a(new_n128_), .b(z02), .O(z20));
endmodule


