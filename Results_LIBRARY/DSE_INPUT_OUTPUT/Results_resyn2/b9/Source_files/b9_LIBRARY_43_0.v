// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n167_, new_n169_, new_n171_, new_n172_, new_n175_, new_n176_;
  nor2   g000(.a(x36), .b(x03), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(x27), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  nand2  g008(.a(x40), .b(x39), .O(new_n71_));
  inv1   g009(.a(new_n71_), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(x10), .c(new_n70_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n64_), .c(new_n65_), .O(new_n75_));
  aoi21  g013(.a(x35), .b(x03), .c(x36), .O(new_n76_));
  inv1   g014(.a(x27), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x04), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n72_), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n64_), .O(new_n81_));
  oai21  g019(.a(new_n75_), .b(new_n63_), .c(new_n81_), .O(z00));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  inv1   g022(.a(x36), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x27), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n84_), .c(new_n83_), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(x04), .c(new_n63_), .O(z01));
  nor2   g029(.a(x27), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n87_), .c(x04), .O(new_n93_));
  nor2   g031(.a(new_n71_), .b(new_n63_), .O(z08));
  inv1   g032(.a(x02), .O(new_n95_));
  nand2  g033(.a(x29), .b(x08), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(z08), .c(new_n93_), .O(z02));
  inv1   g036(.a(x35), .O(new_n99_));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  oai22  g038(.a(new_n100_), .b(new_n99_), .c(new_n66_), .d(x27), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(x21), .c(new_n63_), .O(z03));
  inv1   g040(.a(x21), .O(new_n103_));
  aoi21  g041(.a(new_n101_), .b(new_n103_), .c(new_n63_), .O(z04));
  aoi21  g042(.a(new_n100_), .b(new_n66_), .c(new_n63_), .O(z05));
  inv1   g043(.a(z05), .O(z06));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n109_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(x38), .O(new_n111_));
  inv1   g049(.a(x17), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(x15), .O(new_n113_));
  or2    g051(.a(x33), .b(x31), .O(new_n114_));
  oai21  g052(.a(new_n114_), .b(new_n113_), .c(x14), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n111_), .c(x03), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z07));
  inv1   g055(.a(x11), .O(new_n118_));
  nand3  g056(.a(x34), .b(x27), .c(x26), .O(new_n119_));
  nor3   g057(.a(new_n119_), .b(new_n63_), .c(new_n118_), .O(z09));
  inv1   g058(.a(new_n63_), .O(new_n121_));
  nand2  g059(.a(new_n84_), .b(new_n83_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n72_), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  nand2  g062(.a(new_n71_), .b(new_n124_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n88_), .c(x27), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  aoi21  g066(.a(new_n72_), .b(new_n128_), .c(x04), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(new_n121_), .O(z10));
  inv1   g070(.a(x08), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n87_), .c(new_n133_), .O(new_n135_));
  nand2  g073(.a(new_n134_), .b(x27), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand3  g075(.a(new_n78_), .b(x35), .c(new_n86_), .O(new_n138_));
  nor2   g076(.a(x30), .b(x09), .O(new_n139_));
  and2   g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n137_), .c(new_n121_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(z11));
  aoi21  g080(.a(new_n74_), .b(new_n121_), .c(new_n80_), .O(z12));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n68_), .c(x20), .O(new_n145_));
  nor2   g083(.a(x13), .b(x04), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n72_), .c(new_n86_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n145_), .c(new_n77_), .O(new_n148_));
  nand3  g086(.a(new_n146_), .b(new_n122_), .c(new_n72_), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n148_), .c(new_n121_), .O(new_n151_));
  nand2  g089(.a(new_n146_), .b(new_n72_), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(x36), .c(x35), .d(x27), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n151_), .O(z13));
  inv1   g093(.a(x18), .O(new_n156_));
  inv1   g094(.a(x19), .O(new_n157_));
  nand3  g095(.a(x20), .b(new_n157_), .c(new_n156_), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  nand2  g097(.a(new_n66_), .b(new_n99_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  aoi21  g099(.a(x36), .b(x35), .c(new_n86_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n161_), .c(new_n77_), .O(new_n163_));
  nand3  g101(.a(new_n159_), .b(new_n68_), .c(x27), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n152_), .c(new_n63_), .O(new_n165_));
  oai21  g103(.a(new_n163_), .b(new_n122_), .c(new_n165_), .O(z14));
  inv1   g104(.a(x12), .O(new_n167_));
  oai21  g105(.a(new_n119_), .b(new_n167_), .c(new_n121_), .O(z15));
  nand2  g106(.a(x22), .b(x01), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(x23), .c(new_n121_), .O(z16));
  inv1   g108(.a(x24), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(x23), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n169_), .c(new_n121_), .O(z17));
  aoi21  g111(.a(new_n140_), .b(new_n137_), .c(new_n63_), .O(z18));
  nand2  g112(.a(x35), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n139_), .c(new_n121_), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(new_n135_), .O(z20));
  inv1   g115(.a(new_n141_), .O(z19));
endmodule


