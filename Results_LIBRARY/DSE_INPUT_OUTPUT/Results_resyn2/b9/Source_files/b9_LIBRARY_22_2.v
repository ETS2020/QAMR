// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n171_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x30), .O(new_n71_));
  nor2   g009(.a(x39), .b(new_n71_), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  aoi21  g014(.a(x35), .b(new_n76_), .c(x36), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai22  g018(.a(new_n80_), .b(new_n78_), .c(new_n72_), .d(x04), .O(z01));
  inv1   g019(.a(x35), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(x28), .O(new_n83_));
  inv1   g021(.a(new_n83_), .O(new_n84_));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n84_), .c(x04), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n86_), .O(z02));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n82_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  aoi21  g033(.a(new_n93_), .b(new_n95_), .c(new_n72_), .O(z04));
  aoi21  g034(.a(new_n92_), .b(new_n91_), .c(new_n72_), .O(z05));
  inv1   g035(.a(z05), .O(z06));
  inv1   g036(.a(x14), .O(new_n99_));
  inv1   g037(.a(x15), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n100_), .O(new_n101_));
  nor2   g039(.a(x33), .b(x31), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n101_), .c(new_n99_), .O(new_n103_));
  inv1   g041(.a(new_n72_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n99_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n103_), .O(z07));
  nand2  g048(.a(new_n87_), .b(new_n104_), .O(z08));
  inv1   g049(.a(x11), .O(new_n112_));
  nand3  g050(.a(x34), .b(x27), .c(x26), .O(new_n113_));
  nor3   g051(.a(new_n113_), .b(new_n72_), .c(new_n112_), .O(z09));
  inv1   g052(.a(x32), .O(new_n115_));
  oai21  g053(.a(new_n77_), .b(new_n75_), .c(new_n115_), .O(new_n116_));
  inv1   g054(.a(new_n87_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x05), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  and2   g057(.a(new_n87_), .b(x07), .O(new_n120_));
  aoi22  g058(.a(new_n120_), .b(new_n78_), .c(new_n119_), .d(new_n116_), .O(new_n121_));
  nand3  g059(.a(x40), .b(x05), .c(new_n63_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x39), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(x30), .c(new_n125_), .O(new_n126_));
  oai21  g064(.a(new_n121_), .b(x04), .c(new_n126_), .O(z10));
  nand2  g065(.a(x40), .b(x29), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n71_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x39), .O(new_n130_));
  oai21  g068(.a(new_n75_), .b(new_n63_), .c(new_n83_), .O(new_n131_));
  nand3  g069(.a(new_n83_), .b(x27), .c(new_n63_), .O(new_n132_));
  nor2   g070(.a(x30), .b(x08), .O(new_n133_));
  aoi22  g071(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n130_), .O(new_n134_));
  oai21  g072(.a(new_n134_), .b(x09), .c(new_n104_), .O(z11));
  nand2  g073(.a(new_n104_), .b(new_n69_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(z12));
  aoi21  g075(.a(x36), .b(x35), .c(new_n76_), .O(new_n138_));
  inv1   g076(.a(x13), .O(new_n139_));
  nand4  g077(.a(x40), .b(x39), .c(new_n139_), .d(new_n63_), .O(new_n140_));
  nor2   g078(.a(x19), .b(x18), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x20), .O(new_n142_));
  oai22  g080(.a(new_n142_), .b(new_n66_), .c(new_n140_), .d(new_n138_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x27), .O(new_n144_));
  aoi21  g082(.a(x39), .b(x32), .c(x30), .O(new_n145_));
  nand3  g083(.a(x40), .b(new_n139_), .c(new_n63_), .O(new_n146_));
  and2   g084(.a(new_n146_), .b(x39), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n145_), .c(new_n144_), .O(z13));
  aoi21  g086(.a(new_n76_), .b(x27), .c(new_n80_), .O(new_n149_));
  oai22  g087(.a(new_n149_), .b(new_n140_), .c(new_n66_), .d(new_n75_), .O(new_n150_));
  nand2  g088(.a(new_n138_), .b(new_n79_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n140_), .c(new_n142_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n150_), .c(new_n104_), .O(z14));
  inv1   g092(.a(x12), .O(new_n155_));
  oai21  g093(.a(new_n113_), .b(new_n155_), .c(new_n104_), .O(z15));
  nand2  g094(.a(x22), .b(x01), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(x23), .c(new_n104_), .O(z16));
  inv1   g096(.a(x24), .O(new_n159_));
  inv1   g097(.a(new_n157_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(new_n104_), .c(new_n159_), .d(x23), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z17));
  inv1   g100(.a(x08), .O(new_n163_));
  nand2  g101(.a(new_n117_), .b(x29), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n84_), .c(new_n163_), .O(new_n165_));
  nand2  g103(.a(new_n164_), .b(x27), .O(new_n166_));
  nor2   g104(.a(x30), .b(x09), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n132_), .O(new_n168_));
  aoi21  g106(.a(new_n166_), .b(new_n165_), .c(new_n168_), .O(z19));
  inv1   g107(.a(z19), .O(z18));
  oai21  g108(.a(new_n82_), .b(new_n75_), .c(new_n167_), .O(new_n171_));
  nor2   g109(.a(new_n171_), .b(new_n165_), .O(z20));
endmodule


