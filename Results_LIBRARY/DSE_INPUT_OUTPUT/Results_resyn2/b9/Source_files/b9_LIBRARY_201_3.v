// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n156_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n173_, new_n174_, new_n175_, new_n177_;
  inv1   g000(.a(x16), .O(new_n63_));
  nand2  g001(.a(x40), .b(x18), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  oai21  g009(.a(x36), .b(x35), .c(x27), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g011(.a(x04), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nor3   g016(.a(new_n78_), .b(x18), .c(x15), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n73_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n70_), .O(z00));
  inv1   g019(.a(new_n64_), .O(new_n82_));
  inv1   g020(.a(x27), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  aoi21  g022(.a(x35), .b(new_n84_), .c(x36), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  oai21  g024(.a(new_n85_), .b(new_n83_), .c(new_n86_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n82_), .O(z01));
  inv1   g026(.a(x35), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(x28), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n90_), .O(new_n91_));
  and2   g029(.a(x29), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x02), .c(x39), .O(new_n93_));
  aoi21  g031(.a(new_n91_), .b(x04), .c(new_n93_), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(x18), .c(x40), .O(z02));
  nand2  g033(.a(new_n66_), .b(x27), .O(new_n96_));
  nand2  g034(.a(new_n65_), .b(new_n83_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n96_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n65_), .b(new_n83_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n96_), .c(new_n82_), .O(z04));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n64_), .c(new_n65_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  inv1   g042(.a(x15), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n105_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n104_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n64_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  inv1   g052(.a(x18), .O(new_n115_));
  aoi21  g053(.a(new_n75_), .b(new_n115_), .c(new_n76_), .O(z08));
  nand3  g054(.a(x34), .b(x27), .c(x26), .O(new_n117_));
  nand2  g055(.a(new_n64_), .b(x11), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  nor2   g057(.a(new_n85_), .b(new_n83_), .O(new_n120_));
  nand3  g058(.a(x40), .b(x39), .c(x05), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nor2   g061(.a(new_n77_), .b(new_n123_), .O(new_n124_));
  aoi22  g062(.a(new_n124_), .b(new_n120_), .c(new_n122_), .d(new_n87_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  and2   g064(.a(new_n126_), .b(new_n64_), .O(new_n127_));
  oai21  g065(.a(new_n125_), .b(x04), .c(new_n127_), .O(z10));
  nor2   g066(.a(x30), .b(x09), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  aoi21  g068(.a(x39), .b(x29), .c(x18), .O(new_n131_));
  oai21  g069(.a(new_n83_), .b(new_n74_), .c(new_n90_), .O(new_n132_));
  oai21  g070(.a(new_n131_), .b(new_n76_), .c(new_n132_), .O(new_n133_));
  inv1   g071(.a(x08), .O(new_n134_));
  nand3  g072(.a(x35), .b(new_n84_), .c(new_n74_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n115_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x27), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n64_), .c(new_n134_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n133_), .c(new_n130_), .O(z11));
  nor3   g077(.a(x37), .b(x36), .c(x35), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n83_), .c(new_n71_), .O(new_n141_));
  nand2  g079(.a(new_n78_), .b(new_n68_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(new_n64_), .O(z12));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n67_), .c(x20), .O(new_n145_));
  nand2  g083(.a(x36), .b(x35), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x28), .O(new_n147_));
  nor2   g085(.a(x13), .b(x04), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n147_), .c(new_n77_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n145_), .c(new_n83_), .O(new_n150_));
  inv1   g088(.a(new_n86_), .O(new_n151_));
  nand3  g089(.a(new_n148_), .b(new_n151_), .c(x39), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n115_), .c(new_n76_), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(new_n150_), .O(z14));
  inv1   g092(.a(z14), .O(z13));
  inv1   g093(.a(x12), .O(new_n156_));
  oai21  g094(.a(new_n117_), .b(new_n156_), .c(new_n64_), .O(z15));
  inv1   g095(.a(x23), .O(new_n158_));
  nand4  g096(.a(new_n64_), .b(new_n158_), .c(x22), .d(x01), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(z16));
  inv1   g098(.a(x24), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(x23), .c(x22), .d(x01), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n64_), .O(z17));
  inv1   g101(.a(new_n90_), .O(new_n164_));
  nand3  g102(.a(x40), .b(x39), .c(x29), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n164_), .c(new_n134_), .O(new_n166_));
  nand2  g104(.a(new_n165_), .b(x27), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nand3  g106(.a(new_n90_), .b(x27), .c(new_n74_), .O(new_n169_));
  nand2  g107(.a(new_n129_), .b(new_n64_), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n169_), .c(new_n168_), .O(z18));
  nor2   g110(.a(x27), .b(x08), .O(new_n173_));
  oai21  g111(.a(new_n132_), .b(new_n173_), .c(new_n129_), .O(new_n174_));
  nor3   g112(.a(new_n165_), .b(x18), .c(new_n134_), .O(new_n175_));
  aoi21  g113(.a(new_n174_), .b(new_n64_), .c(new_n175_), .O(z19));
  nor2   g114(.a(new_n89_), .b(new_n83_), .O(new_n177_));
  nor3   g115(.a(new_n177_), .b(new_n170_), .c(new_n166_), .O(z20));
endmodule


