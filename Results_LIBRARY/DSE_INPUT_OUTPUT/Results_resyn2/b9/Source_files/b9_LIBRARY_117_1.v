// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n168_, new_n170_, new_n171_, new_n173_, new_n174_,
    new_n175_, new_n177_, new_n179_, new_n180_;
  nand2  g000(.a(x40), .b(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x04), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(x36), .b(x35), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n66_), .c(new_n67_), .O(new_n69_));
  aoi22  g007(.a(new_n69_), .b(x27), .c(new_n64_), .d(x10), .O(new_n70_));
  inv1   g008(.a(x34), .O(new_n71_));
  nand2  g009(.a(x40), .b(new_n71_), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x27), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n72_), .c(x04), .O(z01));
  nor2   g019(.a(new_n75_), .b(x28), .O(new_n82_));
  oai21  g020(.a(x27), .b(x08), .c(new_n82_), .O(new_n83_));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x39), .O(new_n87_));
  aoi21  g025(.a(new_n83_), .b(x04), .c(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n71_), .c(x40), .O(z02));
  inv1   g027(.a(x28), .O(new_n90_));
  oai21  g028(.a(new_n75_), .b(new_n90_), .c(x27), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  inv1   g031(.a(new_n72_), .O(new_n94_));
  aoi21  g032(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n91_), .c(new_n97_), .O(z04));
  oai21  g036(.a(new_n90_), .b(new_n92_), .c(new_n93_), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n72_), .O(z06));
  inv1   g038(.a(z06), .O(z05));
  inv1   g039(.a(x25), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(x00), .O(new_n103_));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x25), .b(new_n104_), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(x38), .O(new_n106_));
  inv1   g044(.a(x17), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(x15), .O(new_n108_));
  or2    g046(.a(x33), .b(x31), .O(new_n109_));
  oai21  g047(.a(new_n109_), .b(new_n108_), .c(x14), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n106_), .c(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n72_), .O(z07));
  nand3  g050(.a(x40), .b(x39), .c(x34), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n72_), .O(z09));
  inv1   g054(.a(x40), .O(new_n117_));
  inv1   g055(.a(x05), .O(new_n118_));
  nand2  g056(.a(x39), .b(new_n118_), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(x34), .c(new_n117_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(new_n63_), .b(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n77_), .c(new_n65_), .O(new_n123_));
  nand3  g061(.a(new_n72_), .b(x37), .c(x06), .O(new_n124_));
  oai21  g062(.a(new_n123_), .b(new_n120_), .c(new_n124_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x27), .O(new_n126_));
  inv1   g064(.a(new_n79_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(new_n64_), .c(x34), .d(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  nand2  g068(.a(x35), .b(new_n90_), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n131_), .c(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nor2   g073(.a(new_n92_), .b(x04), .O(new_n136_));
  inv1   g074(.a(x09), .O(new_n137_));
  inv1   g075(.a(x30), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  aoi21  g077(.a(new_n136_), .b(new_n82_), .c(new_n139_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n135_), .c(new_n72_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(z11));
  nand2  g080(.a(new_n64_), .b(x10), .O(new_n143_));
  nand2  g081(.a(new_n69_), .b(x27), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g083(.a(new_n72_), .b(new_n145_), .O(z12));
  inv1   g084(.a(x13), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n64_), .c(new_n147_), .O(new_n150_));
  nor2   g088(.a(x19), .b(x18), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x20), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n67_), .c(new_n150_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  nand3  g092(.a(x39), .b(new_n147_), .c(new_n65_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n79_), .c(x34), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x40), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n154_), .O(z13));
  nand2  g096(.a(new_n64_), .b(new_n147_), .O(new_n159_));
  aoi21  g097(.a(new_n90_), .b(x27), .c(new_n127_), .O(new_n160_));
  oai22  g098(.a(new_n160_), .b(new_n159_), .c(new_n67_), .d(new_n92_), .O(new_n161_));
  nand3  g099(.a(new_n148_), .b(new_n79_), .c(x28), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n64_), .c(new_n147_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n152_), .c(new_n94_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(z14));
  nand4  g103(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n72_), .O(z15));
  nand2  g105(.a(x22), .b(x01), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(x23), .c(new_n72_), .O(z16));
  inv1   g107(.a(x24), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x23), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n168_), .c(new_n72_), .O(z17));
  aoi21  g110(.a(new_n92_), .b(new_n130_), .c(new_n131_), .O(new_n173_));
  nand2  g111(.a(x27), .b(x04), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n173_), .c(new_n139_), .O(new_n175_));
  oai22  g113(.a(new_n175_), .b(new_n94_), .c(new_n113_), .d(new_n85_), .O(z18));
  nand2  g114(.a(new_n140_), .b(new_n135_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n72_), .O(z19));
  nand2  g116(.a(x35), .b(x27), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n138_), .c(new_n137_), .O(new_n180_));
  oai21  g118(.a(new_n180_), .b(new_n133_), .c(new_n72_), .O(z20));
endmodule


