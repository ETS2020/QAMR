// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n159_,
    new_n161_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n174_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x23), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(x40), .b(x39), .c(x23), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(z08));
  nand4  g014(.a(z08), .b(new_n74_), .c(new_n65_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n70_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x35), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(x28), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(x36), .c(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g022(.a(x23), .O(new_n85_));
  oai21  g023(.a(x27), .b(x08), .c(new_n80_), .O(new_n86_));
  and2   g024(.a(x29), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(x02), .c(x39), .O(new_n88_));
  aoi21  g026(.a(new_n86_), .b(x04), .c(new_n88_), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n85_), .c(x40), .O(z02));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n79_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand2  g033(.a(x40), .b(new_n85_), .O(new_n96_));
  nand3  g034(.a(new_n93_), .b(new_n96_), .c(new_n95_), .O(z04));
  nand3  g035(.a(new_n92_), .b(new_n96_), .c(new_n91_), .O(z05));
  nand2  g036(.a(new_n92_), .b(new_n91_), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n96_), .O(z06));
  inv1   g038(.a(x25), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x25), .b(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(x38), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x15), .O(new_n107_));
  or2    g045(.a(x33), .b(x31), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(new_n107_), .c(x14), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n105_), .c(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n96_), .O(z07));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n96_), .O(z09));
  inv1   g051(.a(x07), .O(new_n114_));
  inv1   g052(.a(x39), .O(new_n115_));
  aoi21  g053(.a(new_n115_), .b(x23), .c(new_n63_), .O(new_n116_));
  nand4  g054(.a(x40), .b(x39), .c(x23), .d(x05), .O(new_n117_));
  oai21  g055(.a(new_n116_), .b(new_n114_), .c(new_n117_), .O(new_n118_));
  inv1   g056(.a(x36), .O(new_n119_));
  inv1   g057(.a(x28), .O(new_n120_));
  nand2  g058(.a(x35), .b(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n119_), .c(x04), .O(new_n122_));
  nand2  g060(.a(x37), .b(x06), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n64_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n118_), .c(new_n124_), .O(new_n125_));
  inv1   g063(.a(new_n82_), .O(new_n126_));
  inv1   g064(.a(new_n117_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n71_), .O(new_n128_));
  oai21  g066(.a(new_n125_), .b(new_n67_), .c(new_n128_), .O(z10));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  nor2   g068(.a(new_n67_), .b(new_n71_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n86_), .c(new_n130_), .O(new_n132_));
  inv1   g070(.a(x08), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  nor3   g072(.a(new_n134_), .b(new_n85_), .c(new_n133_), .O(new_n135_));
  aoi21  g073(.a(new_n132_), .b(new_n96_), .c(new_n135_), .O(z11));
  nor3   g074(.a(x37), .b(x36), .c(x35), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n67_), .c(new_n72_), .O(new_n138_));
  nand3  g076(.a(x40), .b(x39), .c(new_n71_), .O(new_n139_));
  oai21  g077(.a(new_n68_), .b(new_n67_), .c(new_n139_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n138_), .c(new_n96_), .O(z12));
  inv1   g079(.a(new_n68_), .O(new_n142_));
  nor2   g080(.a(x19), .b(x18), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n142_), .c(x20), .O(new_n144_));
  inv1   g082(.a(x13), .O(new_n145_));
  inv1   g083(.a(new_n139_), .O(new_n146_));
  oai21  g084(.a(new_n119_), .b(new_n79_), .c(x28), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n146_), .c(new_n145_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  nand2  g088(.a(x39), .b(new_n71_), .O(new_n151_));
  nand2  g089(.a(new_n126_), .b(new_n145_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n151_), .c(x23), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x40), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n150_), .O(z13));
  aoi21  g093(.a(new_n148_), .b(new_n144_), .c(new_n67_), .O(new_n156_));
  nor3   g094(.a(new_n139_), .b(new_n82_), .c(x13), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n156_), .c(new_n96_), .O(z14));
  nand4  g096(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n96_), .O(z15));
  nand4  g098(.a(new_n63_), .b(new_n85_), .c(x22), .d(x01), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z16));
  inv1   g100(.a(x24), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x23), .c(x22), .d(x01), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z17));
  aoi21  g103(.a(new_n134_), .b(new_n121_), .c(new_n133_), .O(new_n166_));
  nand2  g104(.a(new_n134_), .b(x27), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nand3  g106(.a(new_n80_), .b(x27), .c(new_n71_), .O(new_n169_));
  nand2  g107(.a(new_n130_), .b(new_n96_), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n169_), .c(new_n168_), .O(z18));
  inv1   g110(.a(z18), .O(z19));
  nor2   g111(.a(new_n79_), .b(new_n67_), .O(new_n174_));
  nor3   g112(.a(new_n174_), .b(new_n170_), .c(new_n166_), .O(z20));
endmodule


