// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n155_, new_n157_, new_n159_, new_n160_, new_n163_, new_n165_;
  nor2   g000(.a(x17), .b(x13), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x27), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nand2  g004(.a(x35), .b(x28), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  inv1   g008(.a(x35), .O(new_n71_));
  inv1   g009(.a(x36), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  and2   g011(.a(x40), .b(x39), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(new_n73_), .c(new_n70_), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n69_), .c(new_n65_), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x04), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x10), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n76_), .c(new_n64_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x16), .c(new_n63_), .O(z00));
  inv1   g020(.a(new_n63_), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  aoi21  g022(.a(x35), .b(new_n84_), .c(x36), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  oai21  g024(.a(new_n85_), .b(new_n65_), .c(new_n86_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n83_), .c(x04), .O(z01));
  nand2  g026(.a(x35), .b(new_n84_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n91_), .c(new_n74_), .d(new_n83_), .O(z02));
  nand2  g033(.a(new_n67_), .b(x27), .O(new_n96_));
  nand2  g034(.a(new_n66_), .b(new_n65_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n96_), .c(new_n83_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n66_), .b(new_n65_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n96_), .c(new_n63_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n63_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n64_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n104_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n83_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  nand2  g051(.a(new_n77_), .b(new_n83_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand4  g053(.a(new_n83_), .b(x34), .c(x27), .d(x26), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(new_n115_), .O(z09));
  aoi21  g055(.a(new_n87_), .b(x05), .c(new_n77_), .O(new_n118_));
  nand2  g056(.a(new_n89_), .b(new_n72_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(x27), .c(x07), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n77_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n70_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n63_), .O(new_n125_));
  oai21  g063(.a(new_n122_), .b(new_n118_), .c(new_n125_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  nand2  g065(.a(new_n74_), .b(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n89_), .c(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g069(.a(new_n89_), .O(new_n132_));
  nor2   g070(.a(new_n65_), .b(x04), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(new_n135_));
  aoi21  g073(.a(new_n133_), .b(new_n132_), .c(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n131_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n83_), .O(z11));
  inv1   g076(.a(new_n76_), .O(new_n139_));
  nand3  g077(.a(new_n79_), .b(new_n139_), .c(new_n83_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z12));
  inv1   g079(.a(x13), .O(new_n142_));
  oai21  g080(.a(new_n72_), .b(new_n71_), .c(x28), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n78_), .c(new_n142_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n68_), .c(x20), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n144_), .c(new_n65_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  inv1   g086(.a(new_n78_), .O(new_n149_));
  oai21  g087(.a(new_n86_), .b(new_n149_), .c(x17), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n142_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n148_), .O(z13));
  nor3   g090(.a(new_n86_), .b(new_n149_), .c(x13), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n147_), .c(new_n83_), .O(z14));
  inv1   g092(.a(x12), .O(new_n155_));
  nor2   g093(.a(new_n116_), .b(new_n155_), .O(z15));
  nand2  g094(.a(x22), .b(x01), .O(new_n157_));
  nor3   g095(.a(new_n157_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g096(.a(x24), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x23), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n157_), .c(new_n83_), .O(z17));
  aoi21  g099(.a(new_n136_), .b(new_n131_), .c(new_n63_), .O(z18));
  nand3  g100(.a(new_n136_), .b(new_n131_), .c(new_n83_), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z19));
  oai21  g102(.a(new_n71_), .b(new_n65_), .c(new_n134_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n129_), .c(new_n83_), .O(z20));
endmodule


