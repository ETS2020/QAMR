// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n179_;
  inv1   g000(.a(x26), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x35), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(new_n73_), .c(new_n68_), .O(new_n75_));
  nand3  g013(.a(x40), .b(x39), .c(x26), .O(new_n76_));
  nor3   g014(.a(new_n76_), .b(x15), .c(x04), .O(new_n77_));
  oai21  g015(.a(new_n75_), .b(x10), .c(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n72_), .b(new_n65_), .c(new_n78_), .O(z00));
  oai21  g017(.a(new_n73_), .b(x28), .c(new_n74_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n65_), .O(z01));
  nor2   g022(.a(new_n73_), .b(x28), .O(new_n85_));
  inv1   g023(.a(x08), .O(new_n86_));
  nand2  g024(.a(new_n68_), .b(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  and2   g026(.a(x29), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(x02), .c(x39), .O(new_n90_));
  aoi21  g028(.a(new_n88_), .b(x04), .c(new_n90_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n63_), .c(x40), .O(z02));
  nand2  g030(.a(new_n70_), .b(x27), .O(new_n93_));
  nand2  g031(.a(new_n69_), .b(new_n68_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n93_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g033(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n93_), .c(new_n65_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n65_), .O(z05));
  and2   g037(.a(new_n98_), .b(new_n64_), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n66_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n101_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n64_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  inv1   g048(.a(new_n76_), .O(z08));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n64_), .O(z09));
  inv1   g051(.a(x04), .O(new_n114_));
  inv1   g052(.a(new_n82_), .O(new_n115_));
  nand2  g053(.a(x40), .b(x39), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n115_), .c(x05), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(new_n119_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(new_n116_), .b(new_n121_), .O(new_n122_));
  nand4  g060(.a(new_n122_), .b(new_n120_), .c(new_n80_), .d(x27), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n118_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n114_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  and2   g064(.a(new_n126_), .b(new_n64_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(z10));
  nor2   g066(.a(x30), .b(x09), .O(new_n129_));
  nor2   g067(.a(new_n68_), .b(new_n114_), .O(new_n130_));
  oai21  g068(.a(new_n130_), .b(new_n88_), .c(new_n129_), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  nor3   g070(.a(new_n132_), .b(new_n63_), .c(new_n86_), .O(new_n133_));
  aoi21  g071(.a(new_n131_), .b(new_n64_), .c(new_n133_), .O(z11));
  nand3  g072(.a(new_n69_), .b(new_n74_), .c(new_n73_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x27), .c(x10), .O(new_n136_));
  aoi21  g074(.a(x39), .b(new_n114_), .c(new_n71_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n136_), .c(x26), .O(new_n138_));
  inv1   g076(.a(x40), .O(new_n139_));
  inv1   g077(.a(new_n71_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n138_), .O(z12));
  nand2  g080(.a(new_n70_), .b(new_n69_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(new_n143_), .c(new_n64_), .d(x20), .O(new_n145_));
  nand2  g083(.a(x36), .b(x35), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x28), .O(new_n147_));
  nor2   g085(.a(x13), .b(x04), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(new_n147_), .c(new_n117_), .d(x26), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n145_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  nand4  g089(.a(new_n148_), .b(new_n117_), .c(new_n115_), .d(x26), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n151_), .O(z13));
  nand2  g091(.a(new_n144_), .b(x20), .O(new_n154_));
  nand2  g092(.a(new_n148_), .b(new_n117_), .O(new_n155_));
  nand3  g093(.a(new_n146_), .b(new_n82_), .c(x28), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n155_), .c(new_n154_), .O(new_n158_));
  inv1   g096(.a(x28), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x27), .c(new_n115_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n155_), .c(new_n140_), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n158_), .c(new_n64_), .O(z14));
  nand4  g100(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n64_), .O(z15));
  nand2  g102(.a(x22), .b(x01), .O(new_n165_));
  nor3   g103(.a(new_n165_), .b(new_n65_), .c(x23), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x23), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n165_), .c(new_n64_), .O(z17));
  nand2  g107(.a(x35), .b(new_n159_), .O(new_n170_));
  aoi21  g108(.a(new_n132_), .b(new_n170_), .c(new_n86_), .O(new_n171_));
  nand2  g109(.a(new_n132_), .b(x27), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n171_), .O(new_n173_));
  nand3  g111(.a(new_n85_), .b(x27), .c(new_n114_), .O(new_n174_));
  nand2  g112(.a(new_n129_), .b(new_n64_), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n174_), .c(new_n173_), .O(z18));
  inv1   g115(.a(z18), .O(z19));
  nor2   g116(.a(new_n73_), .b(new_n68_), .O(new_n179_));
  nor3   g117(.a(new_n179_), .b(new_n175_), .c(new_n171_), .O(z20));
endmodule


