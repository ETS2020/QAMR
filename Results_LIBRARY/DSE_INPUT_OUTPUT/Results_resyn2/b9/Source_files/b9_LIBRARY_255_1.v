// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n172_, new_n174_, new_n175_;
  inv1   g000(.a(x33), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x32), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  and2   g009(.a(x40), .b(x39), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(new_n71_), .c(new_n68_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n67_), .c(new_n66_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x10), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n65_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g018(.a(x32), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n70_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x27), .c(x30), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(x33), .c(new_n81_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x04), .O(z01));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n83_), .c(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n75_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n89_), .c(new_n64_), .O(z02));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n69_), .c(new_n93_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand2  g035(.a(x33), .b(new_n81_), .O(new_n98_));
  nand3  g036(.a(new_n95_), .b(new_n98_), .c(new_n97_), .O(z04));
  aoi21  g037(.a(new_n94_), .b(new_n93_), .c(new_n64_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x25), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(x00), .O(new_n103_));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x25), .b(new_n104_), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(x38), .O(new_n106_));
  inv1   g044(.a(x17), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(x15), .O(new_n108_));
  inv1   g046(.a(x31), .O(new_n109_));
  nand2  g047(.a(new_n63_), .b(new_n109_), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n108_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n106_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n98_), .O(z07));
  nand2  g051(.a(new_n75_), .b(new_n98_), .O(z08));
  and2   g052(.a(x34), .b(x27), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(x26), .c(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n98_), .O(z09));
  nor2   g055(.a(new_n64_), .b(new_n93_), .O(new_n118_));
  nand3  g056(.a(new_n75_), .b(new_n98_), .c(x07), .O(new_n119_));
  nand3  g057(.a(new_n72_), .b(new_n63_), .c(x05), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  aoi21  g059(.a(new_n83_), .b(new_n70_), .c(x04), .O(new_n122_));
  aoi22  g060(.a(new_n122_), .b(new_n121_), .c(new_n118_), .d(x06), .O(new_n123_));
  inv1   g061(.a(x30), .O(new_n124_));
  oai21  g062(.a(x33), .b(new_n124_), .c(new_n81_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n76_), .c(x05), .O(new_n126_));
  oai21  g064(.a(new_n123_), .b(new_n66_), .c(new_n126_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  nand2  g066(.a(new_n72_), .b(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n83_), .c(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nor2   g070(.a(new_n69_), .b(new_n66_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n82_), .c(new_n68_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  and2   g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n132_), .c(new_n98_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  inv1   g076(.a(new_n74_), .O(new_n139_));
  nand3  g077(.a(new_n77_), .b(new_n139_), .c(new_n98_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z12));
  nor2   g079(.a(x13), .b(x04), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n72_), .O(new_n143_));
  inv1   g081(.a(new_n143_), .O(new_n144_));
  nand2  g082(.a(new_n81_), .b(new_n124_), .O(new_n145_));
  aoi21  g083(.a(x36), .b(x35), .c(new_n82_), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(new_n66_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n145_), .c(new_n144_), .O(new_n148_));
  inv1   g086(.a(x18), .O(new_n149_));
  inv1   g087(.a(x19), .O(new_n150_));
  nand3  g088(.a(x20), .b(new_n150_), .c(new_n149_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(new_n152_));
  nor2   g090(.a(new_n67_), .b(new_n66_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n148_), .c(new_n98_), .O(z13));
  nand3  g093(.a(new_n93_), .b(new_n69_), .c(x28), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x27), .c(x30), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(x33), .c(new_n81_), .O(new_n158_));
  nand4  g096(.a(new_n151_), .b(new_n146_), .c(new_n81_), .d(new_n124_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n144_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n154_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n158_), .O(z14));
  nand4  g100(.a(new_n115_), .b(new_n98_), .c(x26), .d(x12), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z15));
  nand2  g102(.a(x22), .b(x01), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(x23), .c(new_n98_), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  inv1   g105(.a(new_n165_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n98_), .c(new_n167_), .d(x23), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z17));
  aoi21  g108(.a(new_n136_), .b(new_n132_), .c(new_n64_), .O(z18));
  nand2  g109(.a(new_n136_), .b(new_n132_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n98_), .O(z19));
  nor2   g111(.a(new_n133_), .b(new_n64_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n135_), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n130_), .O(z20));
endmodule


