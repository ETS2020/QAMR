// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n175_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x28), .O(new_n64_));
  nor2   g002(.a(x36), .b(new_n64_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  nand2  g004(.a(x37), .b(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  nand4  g006(.a(x40), .b(x39), .c(x10), .d(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n67_), .c(x15), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n63_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  nand2  g010(.a(x35), .b(x28), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n68_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n64_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand4  g018(.a(new_n80_), .b(new_n77_), .c(x27), .d(new_n72_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n71_), .O(z00));
  oai21  g020(.a(x36), .b(x35), .c(x27), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n66_), .c(x04), .O(z01));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n79_), .c(x04), .O(new_n88_));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n89_), .c(new_n74_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n88_), .c(new_n65_), .O(z02));
  aoi21  g030(.a(new_n73_), .b(x27), .c(new_n65_), .O(new_n93_));
  inv1   g031(.a(x27), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n93_), .c(new_n98_), .O(z04));
  aoi21  g037(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n65_), .O(z05));
  nand2  g039(.a(x36), .b(new_n94_), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(x28), .c(x37), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n72_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n104_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n66_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  nand2  g051(.a(new_n74_), .b(new_n66_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n66_), .O(z09));
  nand3  g054(.a(x37), .b(x27), .c(x06), .O(new_n117_));
  nor2   g055(.a(new_n74_), .b(x04), .O(new_n118_));
  inv1   g056(.a(new_n84_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n118_), .c(x05), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n117_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n66_), .O(new_n122_));
  nor2   g060(.a(new_n94_), .b(x04), .O(new_n123_));
  inv1   g061(.a(x05), .O(new_n124_));
  nand2  g062(.a(new_n75_), .b(new_n124_), .O(new_n125_));
  inv1   g063(.a(x07), .O(new_n126_));
  nand2  g064(.a(new_n74_), .b(new_n126_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(new_n125_), .c(new_n123_), .d(new_n80_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n122_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  nand2  g068(.a(new_n75_), .b(x29), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n79_), .c(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n131_), .b(x27), .O(new_n133_));
  nand3  g071(.a(new_n123_), .b(x35), .c(new_n64_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n66_), .O(new_n136_));
  aoi21  g074(.a(new_n133_), .b(new_n132_), .c(new_n136_), .O(z11));
  inv1   g075(.a(x10), .O(new_n138_));
  nand2  g076(.a(new_n83_), .b(new_n138_), .O(new_n139_));
  nand2  g077(.a(x35), .b(x27), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x36), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n67_), .O(new_n143_));
  aoi21  g081(.a(new_n139_), .b(new_n118_), .c(new_n143_), .O(z12));
  nand2  g082(.a(new_n73_), .b(new_n95_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n145_), .c(x20), .O(new_n147_));
  inv1   g085(.a(x13), .O(new_n148_));
  nand3  g086(.a(new_n75_), .b(new_n148_), .c(new_n68_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n147_), .c(new_n94_), .O(new_n150_));
  nor2   g088(.a(x35), .b(new_n64_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand3  g091(.a(new_n119_), .b(new_n118_), .c(new_n148_), .O(new_n154_));
  and2   g092(.a(new_n154_), .b(new_n66_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n153_), .O(z13));
  inv1   g094(.a(new_n154_), .O(new_n157_));
  nand3  g095(.a(new_n146_), .b(x37), .c(x20), .O(new_n158_));
  nor3   g096(.a(x35), .b(x32), .c(x30), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n158_), .c(new_n78_), .O(new_n160_));
  oai22  g098(.a(new_n160_), .b(new_n64_), .c(new_n157_), .d(new_n150_), .O(z14));
  nand4  g099(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n66_), .O(z15));
  inv1   g101(.a(x23), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(x22), .c(x01), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n66_), .O(z16));
  nor2   g104(.a(x24), .b(new_n164_), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(new_n66_), .c(x22), .d(x01), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z17));
  oai21  g107(.a(new_n90_), .b(new_n74_), .c(new_n135_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n66_), .O(new_n171_));
  nor2   g109(.a(new_n87_), .b(new_n79_), .O(new_n172_));
  oai21  g110(.a(new_n94_), .b(new_n68_), .c(new_n172_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n171_), .O(z18));
  nand3  g112(.a(new_n140_), .b(new_n135_), .c(new_n66_), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n132_), .O(z20));
  aoi21  g114(.a(new_n133_), .b(new_n132_), .c(new_n136_), .O(z19));
endmodule


