// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n175_,
    new_n177_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  nor2   g008(.a(x40), .b(x23), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x27), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  nor2   g018(.a(new_n71_), .b(new_n63_), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(new_n78_), .c(new_n81_), .O(z01));
  inv1   g020(.a(x08), .O(new_n83_));
  aoi21  g021(.a(new_n74_), .b(new_n83_), .c(new_n77_), .O(new_n84_));
  inv1   g022(.a(x02), .O(new_n85_));
  nand2  g023(.a(x40), .b(x39), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(new_n85_), .c(new_n86_), .O(new_n88_));
  oai21  g026(.a(new_n84_), .b(new_n63_), .c(new_n88_), .O(z02));
  inv1   g027(.a(x35), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n90_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n71_), .O(z03));
  nor2   g032(.a(new_n71_), .b(x21), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n93_), .O(z04));
  inv1   g034(.a(x23), .O(new_n97_));
  inv1   g035(.a(x40), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand3  g037(.a(new_n92_), .b(new_n99_), .c(new_n91_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x25), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(x00), .O(new_n103_));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x25), .b(new_n104_), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(x38), .O(new_n106_));
  inv1   g044(.a(x17), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(x15), .O(new_n108_));
  or2    g046(.a(x33), .b(x31), .O(new_n109_));
  oai21  g047(.a(new_n109_), .b(new_n108_), .c(x14), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n106_), .c(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n99_), .O(z07));
  nand2  g050(.a(new_n86_), .b(new_n99_), .O(z08));
  inv1   g051(.a(x11), .O(new_n114_));
  nand3  g052(.a(x34), .b(x27), .c(x26), .O(new_n115_));
  nor3   g053(.a(new_n115_), .b(new_n71_), .c(new_n114_), .O(z09));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  nand3  g055(.a(new_n86_), .b(new_n99_), .c(x07), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  aoi21  g057(.a(new_n77_), .b(new_n75_), .c(x04), .O(new_n120_));
  nand2  g058(.a(x37), .b(x06), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n71_), .O(new_n122_));
  aoi21  g060(.a(new_n120_), .b(new_n119_), .c(new_n122_), .O(new_n123_));
  inv1   g061(.a(new_n117_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n80_), .c(new_n63_), .O(new_n125_));
  oai21  g063(.a(new_n123_), .b(new_n74_), .c(new_n125_), .O(z10));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n77_), .c(new_n83_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  inv1   g067(.a(new_n77_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(x27), .c(new_n63_), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  and2   g070(.a(new_n132_), .b(new_n99_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  aoi21  g072(.a(new_n129_), .b(new_n128_), .c(new_n134_), .O(z11));
  nand2  g073(.a(new_n66_), .b(new_n91_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x27), .c(x10), .O(new_n137_));
  nor2   g075(.a(new_n67_), .b(new_n74_), .O(new_n138_));
  aoi21  g076(.a(x39), .b(new_n63_), .c(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n137_), .c(x40), .O(new_n140_));
  inv1   g078(.a(new_n138_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n98_), .c(x23), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n140_), .O(z12));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x20), .O(new_n145_));
  inv1   g083(.a(x13), .O(new_n146_));
  nand4  g084(.a(x40), .b(x39), .c(new_n146_), .d(new_n63_), .O(new_n147_));
  aoi21  g085(.a(x36), .b(x35), .c(new_n76_), .O(new_n148_));
  oai22  g086(.a(new_n148_), .b(new_n147_), .c(new_n145_), .d(new_n67_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  inv1   g088(.a(new_n147_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n80_), .c(new_n71_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n150_), .O(z13));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n79_), .c(x28), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n147_), .c(new_n145_), .O(new_n157_));
  aoi21  g095(.a(new_n76_), .b(x27), .c(new_n80_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n147_), .c(new_n141_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n157_), .c(new_n71_), .O(z14));
  inv1   g098(.a(x12), .O(new_n161_));
  oai21  g099(.a(new_n115_), .b(new_n161_), .c(new_n99_), .O(z15));
  nand4  g100(.a(x40), .b(new_n97_), .c(x22), .d(x01), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(x23), .c(x22), .d(x01), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(z17));
  nand2  g105(.a(new_n74_), .b(new_n83_), .O(new_n168_));
  nand2  g106(.a(x27), .b(x04), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n168_), .c(new_n130_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n132_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n99_), .O(new_n172_));
  or2    g110(.a(new_n87_), .b(new_n86_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n172_), .O(z18));
  inv1   g112(.a(new_n173_), .O(new_n175_));
  aoi21  g113(.a(new_n171_), .b(new_n99_), .c(new_n175_), .O(z19));
  oai21  g114(.a(new_n90_), .b(new_n74_), .c(new_n133_), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n128_), .O(z20));
endmodule


