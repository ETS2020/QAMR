// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n151_, new_n153_, new_n155_,
    new_n156_, new_n160_, new_n161_;
  nor2   g00(.a(x14), .b(x05), .O(new_n63_));
  inv1   g01(.a(x15), .O(new_n64_));
  inv1   g02(.a(x27), .O(new_n65_));
  aoi21  g03(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  inv1   g04(.a(x04), .O(new_n67_));
  inv1   g05(.a(x35), .O(new_n68_));
  inv1   g06(.a(x36), .O(new_n69_));
  nand2  g07(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  and2   g08(.a(x40), .b(x39), .O(new_n71_));
  nand3  g09(.a(new_n71_), .b(new_n70_), .c(new_n67_), .O(new_n72_));
  aoi21  g10(.a(new_n72_), .b(new_n66_), .c(new_n65_), .O(new_n73_));
  nand2  g11(.a(x40), .b(x39), .O(new_n74_));
  nor2   g12(.a(new_n74_), .b(x04), .O(new_n75_));
  nand2  g13(.a(new_n75_), .b(x10), .O(new_n76_));
  inv1   g14(.a(new_n76_), .O(new_n77_));
  oai21  g15(.a(new_n77_), .b(new_n73_), .c(new_n64_), .O(new_n78_));
  aoi21  g16(.a(new_n78_), .b(x16), .c(new_n63_), .O(z00));
  inv1   g17(.a(x28), .O(new_n80_));
  nand2  g18(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g19(.a(new_n81_), .b(new_n69_), .O(new_n82_));
  nand2  g20(.a(new_n82_), .b(x27), .O(new_n83_));
  nor2   g21(.a(x32), .b(x30), .O(new_n84_));
  nand2  g22(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  aoi21  g23(.a(new_n85_), .b(x04), .c(new_n63_), .O(z01));
  nor2   g24(.a(x27), .b(x08), .O(new_n87_));
  oai21  g25(.a(new_n87_), .b(new_n81_), .c(x04), .O(new_n88_));
  aoi21  g26(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g27(.a(new_n89_), .b(new_n74_), .O(new_n90_));
  aoi21  g28(.a(new_n90_), .b(new_n88_), .c(new_n63_), .O(z02));
  inv1   g29(.a(x37), .O(new_n92_));
  nand2  g30(.a(x28), .b(x27), .O(new_n93_));
  oai22  g31(.a(new_n93_), .b(new_n68_), .c(new_n92_), .d(x27), .O(new_n94_));
  aoi21  g32(.a(new_n94_), .b(x21), .c(new_n63_), .O(z03));
  inv1   g33(.a(x21), .O(new_n96_));
  aoi21  g34(.a(new_n94_), .b(new_n96_), .c(new_n63_), .O(z04));
  inv1   g35(.a(new_n63_), .O(new_n98_));
  nand3  g36(.a(new_n93_), .b(new_n98_), .c(new_n92_), .O(z05));
  nand2  g37(.a(new_n93_), .b(new_n92_), .O(new_n100_));
  nand2  g38(.a(new_n100_), .b(new_n98_), .O(z06));
  inv1   g39(.a(x25), .O(new_n102_));
  nand2  g40(.a(new_n102_), .b(x00), .O(new_n103_));
  inv1   g41(.a(x14), .O(new_n104_));
  nand2  g42(.a(x25), .b(new_n104_), .O(new_n105_));
  nand3  g43(.a(new_n105_), .b(new_n103_), .c(x38), .O(new_n106_));
  inv1   g44(.a(x17), .O(new_n107_));
  nor2   g45(.a(new_n107_), .b(x15), .O(new_n108_));
  or2    g46(.a(x33), .b(x31), .O(new_n109_));
  oai21  g47(.a(new_n109_), .b(new_n108_), .c(x14), .O(new_n110_));
  nand3  g48(.a(new_n110_), .b(new_n106_), .c(x03), .O(new_n111_));
  nand2  g49(.a(new_n111_), .b(new_n98_), .O(z07));
  nor2   g50(.a(new_n74_), .b(new_n63_), .O(z08));
  nand2  g51(.a(x34), .b(x27), .O(new_n114_));
  nand2  g52(.a(x26), .b(x11), .O(new_n115_));
  oai21  g53(.a(new_n115_), .b(new_n114_), .c(new_n98_), .O(z09));
  aoi21  g54(.a(new_n74_), .b(x14), .c(x05), .O(new_n117_));
  inv1   g55(.a(x07), .O(new_n118_));
  nand2  g56(.a(new_n74_), .b(new_n118_), .O(new_n119_));
  nand3  g57(.a(new_n119_), .b(new_n82_), .c(new_n67_), .O(new_n120_));
  nand3  g58(.a(new_n98_), .b(x37), .c(x06), .O(new_n121_));
  oai21  g59(.a(new_n120_), .b(new_n117_), .c(new_n121_), .O(new_n122_));
  nand2  g60(.a(new_n122_), .b(x27), .O(new_n123_));
  inv1   g61(.a(new_n84_), .O(new_n124_));
  nand3  g62(.a(new_n124_), .b(new_n75_), .c(x05), .O(new_n125_));
  nand2  g63(.a(new_n125_), .b(new_n123_), .O(z10));
  inv1   g64(.a(x08), .O(new_n127_));
  nand2  g65(.a(new_n71_), .b(x29), .O(new_n128_));
  aoi21  g66(.a(new_n128_), .b(new_n81_), .c(new_n127_), .O(new_n129_));
  nand2  g67(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g68(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g69(.a(new_n81_), .O(new_n132_));
  nor2   g70(.a(new_n65_), .b(x04), .O(new_n133_));
  or2    g71(.a(x30), .b(x09), .O(new_n134_));
  aoi21  g72(.a(new_n133_), .b(new_n132_), .c(new_n134_), .O(new_n135_));
  nand2  g73(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand2  g74(.a(new_n136_), .b(new_n98_), .O(z11));
  oai21  g75(.a(new_n77_), .b(new_n73_), .c(new_n98_), .O(z12));
  inv1   g76(.a(new_n66_), .O(new_n139_));
  nor2   g77(.a(x19), .b(x18), .O(new_n140_));
  nand3  g78(.a(new_n140_), .b(new_n139_), .c(x20), .O(new_n141_));
  inv1   g79(.a(x13), .O(new_n142_));
  oai21  g80(.a(new_n69_), .b(new_n68_), .c(x28), .O(new_n143_));
  nand3  g81(.a(new_n143_), .b(new_n75_), .c(new_n142_), .O(new_n144_));
  aoi21  g82(.a(new_n144_), .b(new_n141_), .c(new_n65_), .O(new_n145_));
  inv1   g83(.a(new_n145_), .O(new_n146_));
  nand3  g84(.a(new_n124_), .b(new_n75_), .c(new_n142_), .O(new_n147_));
  aoi21  g85(.a(new_n147_), .b(new_n146_), .c(new_n63_), .O(z13));
  inv1   g86(.a(new_n147_), .O(new_n149_));
  nor3   g87(.a(new_n149_), .b(new_n145_), .c(new_n63_), .O(z14));
  nand2  g88(.a(x26), .b(x12), .O(new_n151_));
  nor3   g89(.a(new_n151_), .b(new_n114_), .c(new_n63_), .O(z15));
  nand2  g90(.a(x22), .b(x01), .O(new_n153_));
  nor3   g91(.a(new_n153_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g92(.a(x24), .O(new_n155_));
  nand2  g93(.a(new_n155_), .b(x23), .O(new_n156_));
  oai21  g94(.a(new_n156_), .b(new_n153_), .c(new_n98_), .O(z17));
  nand3  g95(.a(new_n135_), .b(new_n131_), .c(new_n98_), .O(z18));
  inv1   g96(.a(z18), .O(z19));
  nor2   g97(.a(new_n134_), .b(new_n63_), .O(new_n160_));
  oai21  g98(.a(new_n68_), .b(new_n65_), .c(new_n160_), .O(new_n161_));
  nor2   g99(.a(new_n161_), .b(new_n129_), .O(z20));
endmodule


