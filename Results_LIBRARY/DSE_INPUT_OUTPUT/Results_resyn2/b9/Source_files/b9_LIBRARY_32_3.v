// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n163_, new_n165_, new_n167_, new_n168_, new_n170_,
    new_n172_;
  inv1   g000(.a(x37), .O(new_n63_));
  nand2  g001(.a(x35), .b(x28), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  nand2  g004(.a(x35), .b(x27), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g006(.a(x40), .b(x39), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x04), .O(new_n70_));
  aoi22  g008(.a(new_n70_), .b(new_n68_), .c(new_n65_), .d(x27), .O(new_n71_));
  nand2  g009(.a(x40), .b(x36), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n71_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  inv1   g020(.a(x04), .O(new_n83_));
  inv1   g021(.a(new_n72_), .O(new_n84_));
  nor2   g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g023(.a(new_n82_), .b(new_n79_), .c(new_n85_), .O(z01));
  inv1   g024(.a(x35), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(x28), .O(new_n88_));
  inv1   g026(.a(x08), .O(new_n89_));
  nand2  g027(.a(new_n75_), .b(new_n89_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  and2   g029(.a(x29), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x02), .c(x39), .O(new_n93_));
  aoi21  g031(.a(new_n91_), .b(x04), .c(new_n93_), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(x36), .c(x40), .O(z02));
  nand2  g033(.a(new_n64_), .b(x27), .O(new_n96_));
  aoi21  g034(.a(new_n63_), .b(new_n75_), .c(new_n84_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n96_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n96_), .c(new_n99_), .O(z04));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n72_), .c(new_n63_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n72_), .O(z07));
  nand3  g052(.a(x40), .b(x39), .c(new_n76_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z08));
  nand3  g054(.a(x34), .b(x27), .c(x26), .O(new_n117_));
  nand2  g055(.a(new_n72_), .b(x11), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  nand4  g057(.a(new_n82_), .b(x40), .c(x39), .d(x05), .O(new_n120_));
  inv1   g058(.a(x05), .O(new_n121_));
  nand3  g059(.a(x40), .b(x39), .c(new_n121_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n69_), .b(new_n123_), .O(new_n124_));
  nand4  g062(.a(new_n124_), .b(new_n122_), .c(new_n88_), .d(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n120_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n83_), .O(new_n127_));
  inv1   g065(.a(x40), .O(new_n128_));
  nand2  g066(.a(x27), .b(new_n83_), .O(new_n129_));
  oai21  g067(.a(new_n129_), .b(new_n123_), .c(new_n128_), .O(new_n130_));
  nand3  g068(.a(x37), .b(x27), .c(x06), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  aoi21  g070(.a(new_n130_), .b(x36), .c(new_n132_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n127_), .O(z10));
  nand3  g072(.a(x40), .b(x39), .c(x29), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n78_), .c(new_n89_), .O(new_n136_));
  nand2  g074(.a(new_n135_), .b(x27), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  inv1   g076(.a(new_n129_), .O(new_n139_));
  inv1   g077(.a(x09), .O(new_n140_));
  nand2  g078(.a(new_n80_), .b(new_n140_), .O(new_n141_));
  aoi21  g079(.a(new_n139_), .b(new_n88_), .c(new_n141_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n138_), .c(new_n72_), .O(z18));
  inv1   g081(.a(z18), .O(z11));
  inv1   g082(.a(new_n71_), .O(new_n145_));
  nand2  g083(.a(new_n72_), .b(new_n145_), .O(z12));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(new_n72_), .c(new_n65_), .d(x20), .O(new_n148_));
  nor2   g086(.a(x13), .b(x04), .O(new_n149_));
  nor2   g087(.a(new_n69_), .b(x28), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n149_), .c(new_n76_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n148_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nor3   g091(.a(x36), .b(x13), .c(x04), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(new_n82_), .c(x40), .d(x39), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n153_), .O(z13));
  nand3  g094(.a(new_n147_), .b(new_n65_), .c(x20), .O(new_n157_));
  nand2  g095(.a(new_n150_), .b(new_n149_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nand3  g097(.a(new_n149_), .b(new_n82_), .c(x39), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n76_), .c(new_n128_), .O(new_n161_));
  aoi21  g099(.a(new_n159_), .b(x27), .c(new_n161_), .O(z14));
  inv1   g100(.a(x12), .O(new_n163_));
  oai21  g101(.a(new_n117_), .b(new_n163_), .c(new_n72_), .O(z15));
  nand2  g102(.a(x22), .b(x01), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(x23), .c(new_n72_), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x23), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n165_), .c(new_n72_), .O(z17));
  nand2  g107(.a(new_n142_), .b(new_n138_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n72_), .O(z19));
  nand3  g109(.a(new_n67_), .b(new_n80_), .c(new_n140_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n136_), .c(new_n72_), .O(z20));
endmodule


