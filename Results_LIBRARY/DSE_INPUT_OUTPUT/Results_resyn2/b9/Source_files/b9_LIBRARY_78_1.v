// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n170_, new_n171_;
  inv1   g000(.a(x36), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x11), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  nand4  g007(.a(x40), .b(x39), .c(x10), .d(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n68_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n65_), .c(new_n66_), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n64_), .O(z08));
  inv1   g012(.a(x35), .O(new_n75_));
  aoi21  g013(.a(new_n63_), .b(new_n75_), .c(x15), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(z08), .c(x27), .d(new_n69_), .O(new_n77_));
  oai21  g015(.a(new_n72_), .b(new_n64_), .c(new_n77_), .O(z00));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n63_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n64_), .O(z01));
  nor2   g023(.a(x27), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n81_), .c(x04), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n73_), .O(new_n89_));
  aoi21  g027(.a(new_n89_), .b(new_n87_), .c(new_n64_), .O(z02));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n75_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  inv1   g033(.a(new_n64_), .O(new_n96_));
  nand3  g034(.a(new_n93_), .b(new_n96_), .c(new_n95_), .O(z04));
  aoi21  g035(.a(new_n92_), .b(new_n91_), .c(new_n64_), .O(z05));
  nand2  g036(.a(new_n92_), .b(new_n91_), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n64_), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n65_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n101_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n96_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  nand4  g048(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n96_), .O(z09));
  nor2   g050(.a(new_n79_), .b(new_n73_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  inv1   g052(.a(x07), .O(new_n115_));
  nand2  g053(.a(new_n73_), .b(new_n115_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n82_), .c(x27), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n114_), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  inv1   g057(.a(new_n73_), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(x04), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n122_), .c(new_n96_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand2  g063(.a(new_n120_), .b(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n81_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  inv1   g066(.a(x09), .O(new_n129_));
  inv1   g067(.a(x30), .O(new_n130_));
  nand4  g068(.a(x35), .b(new_n80_), .c(x27), .d(new_n69_), .O(new_n131_));
  nand4  g069(.a(new_n131_), .b(new_n96_), .c(new_n130_), .d(new_n129_), .O(new_n132_));
  aoi21  g070(.a(new_n128_), .b(new_n127_), .c(new_n132_), .O(z11));
  inv1   g071(.a(new_n68_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x27), .O(new_n135_));
  inv1   g073(.a(x10), .O(new_n136_));
  nand2  g074(.a(new_n67_), .b(new_n136_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n120_), .c(new_n69_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n135_), .c(new_n96_), .O(new_n139_));
  nand4  g077(.a(new_n91_), .b(new_n63_), .c(new_n75_), .d(new_n136_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n139_), .O(z12));
  aoi21  g079(.a(x36), .b(x35), .c(new_n80_), .O(new_n142_));
  nor2   g080(.a(x13), .b(x04), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n120_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x20), .O(new_n146_));
  oai22  g084(.a(new_n146_), .b(new_n68_), .c(new_n144_), .d(new_n142_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  aoi21  g086(.a(new_n143_), .b(new_n113_), .c(new_n64_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(z13));
  nand2  g088(.a(new_n142_), .b(new_n79_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n144_), .c(new_n146_), .O(new_n153_));
  inv1   g091(.a(new_n79_), .O(new_n154_));
  aoi21  g092(.a(new_n80_), .b(x27), .c(new_n154_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n144_), .c(new_n135_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n153_), .c(new_n96_), .O(z14));
  nand4  g095(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n96_), .O(z15));
  inv1   g097(.a(x01), .O(new_n160_));
  inv1   g098(.a(x22), .O(new_n161_));
  nor4   g099(.a(new_n64_), .b(x23), .c(new_n161_), .d(new_n160_), .O(z16));
  inv1   g100(.a(x24), .O(new_n163_));
  nor2   g101(.a(new_n161_), .b(new_n160_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n96_), .c(new_n163_), .d(x23), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z17));
  nand2  g104(.a(new_n128_), .b(new_n127_), .O(new_n167_));
  inv1   g105(.a(new_n132_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(z18));
  nand2  g107(.a(x35), .b(x27), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(new_n96_), .c(new_n130_), .d(new_n129_), .O(new_n171_));
  nor2   g109(.a(new_n171_), .b(new_n127_), .O(z20));
  aoi21  g110(.a(new_n128_), .b(new_n127_), .c(new_n132_), .O(z19));
endmodule


