// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n98_, new_n99_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n159_, new_n161_,
    new_n162_, new_n163_, new_n166_, new_n167_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  nor2   g008(.a(x18), .b(x14), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x27), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  nor2   g018(.a(new_n71_), .b(new_n63_), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(new_n78_), .c(new_n81_), .O(z01));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n77_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  nor2   g023(.a(new_n71_), .b(new_n85_), .O(z08));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(z08), .c(new_n84_), .O(z02));
  nand2  g028(.a(x35), .b(x28), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x27), .c(new_n71_), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n74_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  inv1   g035(.a(new_n71_), .O(new_n98_));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n98_), .c(new_n93_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n102_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n98_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  inv1   g050(.a(x11), .O(new_n113_));
  nand3  g051(.a(x34), .b(x27), .c(x26), .O(new_n114_));
  nor3   g052(.a(new_n114_), .b(new_n71_), .c(new_n113_), .O(z09));
  nand2  g053(.a(new_n77_), .b(new_n75_), .O(new_n116_));
  nand2  g054(.a(new_n85_), .b(x07), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n116_), .c(x27), .O(new_n120_));
  inv1   g058(.a(new_n118_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n80_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n63_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(new_n71_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  inv1   g065(.a(new_n85_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n77_), .c(new_n127_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand4  g069(.a(x35), .b(new_n76_), .c(x27), .d(new_n63_), .O(new_n132_));
  nor3   g070(.a(new_n71_), .b(x30), .c(x09), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  aoi21  g072(.a(new_n131_), .b(new_n130_), .c(new_n134_), .O(z11));
  nand2  g073(.a(new_n98_), .b(new_n69_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(z12));
  aoi21  g075(.a(x36), .b(x35), .c(new_n76_), .O(new_n138_));
  nor2   g076(.a(new_n138_), .b(new_n74_), .O(new_n139_));
  nor2   g077(.a(new_n139_), .b(new_n80_), .O(new_n140_));
  nor2   g078(.a(x13), .b(x04), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(z08), .O(new_n142_));
  or2    g080(.a(new_n66_), .b(new_n74_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand3  g083(.a(x20), .b(new_n145_), .c(new_n144_), .O(new_n146_));
  inv1   g084(.a(new_n146_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x14), .O(new_n148_));
  oai22  g086(.a(new_n148_), .b(new_n143_), .c(new_n142_), .d(new_n140_), .O(z13));
  nand2  g087(.a(new_n141_), .b(new_n128_), .O(new_n150_));
  nand2  g088(.a(new_n138_), .b(new_n79_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n150_), .c(new_n146_), .O(new_n153_));
  aoi21  g091(.a(new_n76_), .b(x27), .c(new_n80_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n150_), .c(new_n143_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n153_), .c(new_n98_), .O(z14));
  inv1   g094(.a(x12), .O(new_n157_));
  oai21  g095(.a(new_n114_), .b(new_n157_), .c(new_n98_), .O(z15));
  nand2  g096(.a(x22), .b(x01), .O(new_n159_));
  nor3   g097(.a(new_n159_), .b(new_n71_), .c(x23), .O(z16));
  inv1   g098(.a(x24), .O(new_n161_));
  inv1   g099(.a(new_n159_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(new_n98_), .c(new_n161_), .d(x23), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z17));
  inv1   g102(.a(z11), .O(z18));
  nand2  g103(.a(x35), .b(x27), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n133_), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(new_n130_), .O(z20));
  aoi21  g106(.a(new_n131_), .b(new_n130_), .c(new_n134_), .O(z19));
endmodule


