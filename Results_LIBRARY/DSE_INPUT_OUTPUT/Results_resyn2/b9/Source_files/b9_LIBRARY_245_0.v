// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n153_, new_n155_,
    new_n157_, new_n158_, new_n159_, new_n161_, new_n162_;
  nor2   g000(.a(x36), .b(x33), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  nand4  g006(.a(x40), .b(x39), .c(x10), .d(new_n68_), .O(new_n69_));
  oai21  g007(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n64_), .c(new_n65_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  nand2  g012(.a(x35), .b(x33), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor2   g014(.a(new_n66_), .b(x04), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(new_n76_), .c(new_n73_), .d(new_n64_), .O(new_n78_));
  oai21  g016(.a(new_n71_), .b(new_n63_), .c(new_n78_), .O(z00));
  inv1   g017(.a(new_n63_), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n66_), .c(new_n83_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(x04), .O(z01));
  nand2  g023(.a(x35), .b(new_n81_), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n86_), .c(x04), .O(new_n88_));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n88_), .c(new_n73_), .d(new_n80_), .O(z02));
  inv1   g030(.a(x35), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n81_), .c(x27), .O(new_n94_));
  nor2   g032(.a(x37), .b(x27), .O(new_n95_));
  nor2   g033(.a(new_n95_), .b(new_n63_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n94_), .c(new_n98_), .O(z04));
  aoi21  g037(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n80_), .O(z05));
  inv1   g039(.a(new_n100_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n80_), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n64_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n104_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n80_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  nand2  g051(.a(new_n72_), .b(new_n80_), .O(z08));
  nand2  g052(.a(x34), .b(x27), .O(new_n115_));
  nand2  g053(.a(x26), .b(x11), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(new_n115_), .c(new_n80_), .O(z09));
  oai21  g055(.a(new_n82_), .b(new_n66_), .c(new_n72_), .O(new_n118_));
  inv1   g056(.a(x07), .O(new_n119_));
  oai21  g057(.a(new_n72_), .b(x05), .c(new_n68_), .O(new_n120_));
  aoi21  g058(.a(new_n72_), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n118_), .c(new_n84_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(new_n63_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n86_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  inv1   g067(.a(new_n86_), .O(new_n130_));
  or2    g068(.a(x30), .b(x09), .O(new_n131_));
  aoi21  g069(.a(new_n130_), .b(new_n77_), .c(new_n131_), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n129_), .c(new_n63_), .O(z18));
  inv1   g071(.a(z18), .O(z11));
  nor2   g072(.a(new_n72_), .b(x04), .O(new_n135_));
  oai21  g073(.a(x36), .b(x35), .c(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n67_), .O(new_n137_));
  nand2  g075(.a(new_n69_), .b(new_n80_), .O(new_n138_));
  aoi21  g076(.a(new_n137_), .b(x27), .c(new_n138_), .O(z12));
  inv1   g077(.a(new_n67_), .O(new_n140_));
  nor2   g078(.a(x19), .b(x18), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(x20), .O(new_n142_));
  inv1   g080(.a(x13), .O(new_n143_));
  oai21  g081(.a(new_n74_), .b(new_n93_), .c(x28), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n135_), .c(new_n143_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  nor2   g085(.a(new_n83_), .b(x13), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n135_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n147_), .c(new_n80_), .O(z13));
  nand2  g088(.a(new_n149_), .b(new_n80_), .O(new_n151_));
  aoi21  g089(.a(new_n146_), .b(x27), .c(new_n151_), .O(z14));
  nand2  g090(.a(x26), .b(x12), .O(new_n153_));
  nor3   g091(.a(new_n153_), .b(new_n115_), .c(new_n63_), .O(z15));
  nand2  g092(.a(x22), .b(x01), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(x23), .c(new_n80_), .O(z16));
  inv1   g094(.a(x24), .O(new_n157_));
  inv1   g095(.a(new_n155_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(new_n80_), .c(new_n157_), .d(x23), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(z17));
  nor2   g098(.a(new_n131_), .b(new_n63_), .O(new_n161_));
  oai21  g099(.a(new_n93_), .b(new_n66_), .c(new_n161_), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(new_n127_), .O(z20));
  inv1   g101(.a(z18), .O(z19));
endmodule


