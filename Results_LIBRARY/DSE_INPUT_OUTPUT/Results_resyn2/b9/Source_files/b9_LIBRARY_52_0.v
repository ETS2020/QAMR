// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n163_, new_n165_, new_n166_, new_n167_, new_n169_,
    new_n171_;
  nand2  g000(.a(x40), .b(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x04), .O(new_n64_));
  aoi21  g002(.a(x35), .b(x28), .c(x37), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  nand3  g005(.a(x40), .b(x39), .c(new_n67_), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(new_n69_));
  aoi22  g007(.a(new_n69_), .b(x27), .c(new_n64_), .d(x10), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x32), .b(x14), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x14), .O(new_n75_));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(new_n76_), .c(new_n79_), .O(new_n80_));
  oai22  g018(.a(new_n80_), .b(new_n75_), .c(new_n72_), .d(x04), .O(z01));
  nand2  g019(.a(x35), .b(new_n77_), .O(new_n82_));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  nor2   g022(.a(new_n72_), .b(new_n63_), .O(z08));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(z08), .c(new_n84_), .O(z02));
  inv1   g027(.a(x35), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n90_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  aoi21  g033(.a(new_n93_), .b(new_n95_), .c(new_n72_), .O(z04));
  aoi21  g034(.a(new_n92_), .b(new_n91_), .c(new_n72_), .O(z05));
  nand2  g035(.a(new_n92_), .b(new_n91_), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n72_), .O(z06));
  inv1   g037(.a(x15), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n100_), .O(new_n101_));
  nor2   g039(.a(x33), .b(x31), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n101_), .c(new_n75_), .O(new_n103_));
  inv1   g041(.a(new_n72_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n75_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n103_), .O(z07));
  inv1   g048(.a(x11), .O(new_n111_));
  nand4  g049(.a(new_n104_), .b(x34), .c(x27), .d(x26), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n111_), .O(z09));
  nor2   g051(.a(new_n78_), .b(new_n76_), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  and2   g054(.a(new_n63_), .b(x07), .O(new_n117_));
  aoi22  g055(.a(new_n117_), .b(new_n114_), .c(new_n116_), .d(new_n80_), .O(new_n118_));
  nand3  g056(.a(x37), .b(x27), .c(x06), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n72_), .O(new_n121_));
  oai21  g059(.a(new_n118_), .b(x04), .c(new_n121_), .O(z10));
  inv1   g060(.a(x08), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x29), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n82_), .c(new_n123_), .O(new_n125_));
  nand2  g063(.a(new_n124_), .b(x27), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  inv1   g065(.a(new_n82_), .O(new_n128_));
  nor2   g066(.a(new_n76_), .b(x04), .O(new_n129_));
  inv1   g067(.a(x09), .O(new_n130_));
  inv1   g068(.a(x30), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  aoi21  g070(.a(new_n129_), .b(new_n128_), .c(new_n132_), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n127_), .c(new_n72_), .O(z18));
  inv1   g072(.a(z18), .O(z11));
  nand2  g073(.a(new_n104_), .b(new_n70_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(z12));
  nor2   g075(.a(x19), .b(x18), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x20), .O(new_n139_));
  inv1   g077(.a(new_n65_), .O(new_n140_));
  nand2  g078(.a(new_n104_), .b(new_n140_), .O(new_n141_));
  inv1   g079(.a(x13), .O(new_n142_));
  nand2  g080(.a(new_n64_), .b(new_n142_), .O(new_n143_));
  nand2  g081(.a(x36), .b(x35), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x28), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x14), .O(new_n146_));
  oai22  g084(.a(new_n146_), .b(new_n143_), .c(new_n141_), .d(new_n139_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  inv1   g086(.a(x32), .O(new_n149_));
  oai21  g087(.a(new_n131_), .b(new_n75_), .c(new_n149_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n64_), .c(new_n142_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n148_), .O(z13));
  nand3  g090(.a(new_n91_), .b(new_n90_), .c(x28), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x27), .c(x30), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n75_), .c(new_n149_), .O(new_n155_));
  nand4  g093(.a(new_n144_), .b(new_n139_), .c(new_n79_), .d(x28), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(new_n157_));
  nand2  g095(.a(new_n140_), .b(x27), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n139_), .c(new_n157_), .d(new_n143_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n155_), .O(z14));
  inv1   g098(.a(x12), .O(new_n161_));
  nor2   g099(.a(new_n112_), .b(new_n161_), .O(z15));
  nand2  g100(.a(x22), .b(x01), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n72_), .c(x23), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  inv1   g103(.a(new_n163_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n104_), .c(new_n165_), .d(x23), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z17));
  nand3  g106(.a(new_n133_), .b(new_n127_), .c(new_n104_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z19));
  nor2   g108(.a(new_n90_), .b(new_n76_), .O(new_n171_));
  nor4   g109(.a(new_n171_), .b(new_n132_), .c(new_n125_), .d(new_n72_), .O(z20));
endmodule


