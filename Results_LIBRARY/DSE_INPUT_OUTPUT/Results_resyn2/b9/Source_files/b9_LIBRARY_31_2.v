// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n169_;
  inv1   g000(.a(x20), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x04), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x27), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  and2   g009(.a(x40), .b(x39), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  inv1   g012(.a(x10), .O(new_n75_));
  inv1   g013(.a(x27), .O(new_n76_));
  nor3   g014(.a(x37), .b(x36), .c(x35), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n74_), .c(new_n66_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x16), .c(new_n65_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n81_), .c(new_n76_), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x20), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n84_), .c(x04), .O(z01));
  inv1   g025(.a(x08), .O(new_n88_));
  aoi21  g026(.a(new_n76_), .b(new_n88_), .c(new_n83_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x20), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x04), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  aoi21  g030(.a(x29), .b(x08), .c(x02), .O(new_n93_));
  nor2   g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n91_), .O(z02));
  nand2  g033(.a(new_n68_), .b(x27), .O(new_n96_));
  nand2  g034(.a(new_n67_), .b(new_n76_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n96_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n67_), .b(new_n76_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n96_), .c(new_n65_), .O(z04));
  oai21  g038(.a(new_n82_), .b(new_n76_), .c(new_n67_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n64_), .O(z06));
  inv1   g040(.a(z06), .O(z05));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n66_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n104_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n64_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  nand2  g051(.a(new_n92_), .b(new_n64_), .O(z08));
  nand3  g052(.a(x34), .b(x27), .c(x26), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n64_), .c(x11), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z09));
  aoi21  g056(.a(new_n83_), .b(new_n81_), .c(x04), .O(new_n119_));
  inv1   g057(.a(x05), .O(new_n120_));
  aoi21  g058(.a(x40), .b(x39), .c(x07), .O(new_n121_));
  aoi21  g059(.a(new_n72_), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  nand2  g060(.a(x37), .b(x06), .O(new_n123_));
  aoi21  g061(.a(new_n63_), .b(x04), .c(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n119_), .c(new_n124_), .O(new_n125_));
  nor2   g063(.a(new_n92_), .b(x04), .O(new_n126_));
  inv1   g064(.a(new_n85_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(x05), .O(new_n128_));
  oai21  g066(.a(new_n125_), .b(new_n76_), .c(new_n128_), .O(z10));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  and2   g068(.a(x35), .b(x27), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n82_), .c(new_n71_), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n83_), .c(new_n88_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n132_), .c(new_n130_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n64_), .O(z11));
  nand3  g076(.a(new_n78_), .b(new_n74_), .c(new_n64_), .O(z12));
  inv1   g077(.a(x13), .O(new_n140_));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n126_), .c(new_n140_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand2  g083(.a(new_n63_), .b(new_n71_), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(new_n69_), .c(new_n145_), .d(new_n144_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  nand3  g087(.a(new_n127_), .b(new_n126_), .c(new_n140_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n149_), .c(new_n64_), .O(z13));
  inv1   g089(.a(new_n150_), .O(new_n152_));
  nand3  g090(.a(new_n69_), .b(new_n145_), .c(new_n144_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n63_), .c(new_n143_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x27), .c(new_n152_), .O(z14));
  nand3  g093(.a(new_n116_), .b(new_n64_), .c(x12), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(z15));
  inv1   g095(.a(x23), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(x22), .c(x01), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n64_), .O(z16));
  nor2   g098(.a(x24), .b(new_n158_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n64_), .c(x22), .d(x01), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(z17));
  oai21  g101(.a(new_n76_), .b(new_n71_), .c(new_n89_), .O(new_n164_));
  nand3  g102(.a(new_n72_), .b(x29), .c(x08), .O(new_n165_));
  nand2  g103(.a(new_n130_), .b(new_n64_), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n165_), .c(new_n164_), .O(z18));
  nand3  g106(.a(new_n167_), .b(new_n136_), .c(new_n132_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z19));
  nor3   g108(.a(new_n166_), .b(new_n134_), .c(new_n131_), .O(z20));
endmodule


