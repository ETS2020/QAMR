// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n97_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n171_, new_n172_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n183_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  nand2  g008(.a(x40), .b(x21), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n70_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x27), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  aoi21  g018(.a(x40), .b(x21), .c(new_n65_), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(new_n78_), .c(new_n81_), .O(z01));
  inv1   g020(.a(x35), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(x28), .O(new_n84_));
  oai21  g022(.a(x27), .b(x08), .c(new_n84_), .O(new_n85_));
  and2   g023(.a(x29), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x02), .c(x39), .O(new_n87_));
  aoi21  g025(.a(new_n85_), .b(x04), .c(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(x21), .c(x40), .O(z02));
  inv1   g027(.a(x37), .O(new_n90_));
  nand2  g028(.a(x28), .b(x27), .O(new_n91_));
  oai22  g029(.a(new_n91_), .b(new_n83_), .c(new_n90_), .d(x27), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x40), .c(x21), .O(z03));
  inv1   g031(.a(x21), .O(new_n94_));
  nand2  g032(.a(new_n92_), .b(new_n94_), .O(z04));
  nand3  g033(.a(new_n91_), .b(new_n71_), .c(new_n90_), .O(z05));
  nand2  g034(.a(new_n91_), .b(new_n90_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n71_), .O(z06));
  inv1   g036(.a(x25), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(x00), .O(new_n100_));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x25), .b(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n100_), .c(x38), .O(new_n103_));
  inv1   g041(.a(x17), .O(new_n104_));
  nor2   g042(.a(new_n104_), .b(x15), .O(new_n105_));
  or2    g043(.a(x33), .b(x31), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(new_n105_), .c(x14), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n103_), .c(x03), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n71_), .O(z07));
  inv1   g047(.a(x39), .O(new_n110_));
  inv1   g048(.a(x40), .O(new_n111_));
  aoi21  g049(.a(new_n110_), .b(new_n94_), .c(new_n111_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n71_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  nor2   g053(.a(new_n111_), .b(new_n110_), .O(new_n116_));
  nand2  g054(.a(new_n80_), .b(new_n116_), .O(new_n117_));
  nand2  g055(.a(new_n77_), .b(new_n75_), .O(new_n118_));
  inv1   g056(.a(x07), .O(new_n119_));
  oai21  g057(.a(new_n111_), .b(new_n110_), .c(new_n119_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n118_), .c(x27), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  inv1   g060(.a(x05), .O(new_n123_));
  aoi21  g061(.a(new_n116_), .b(new_n123_), .c(x04), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n71_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  nor2   g066(.a(x30), .b(x09), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(x04), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n94_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x40), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(x27), .c(x04), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(new_n77_), .O(new_n135_));
  oai21  g073(.a(new_n77_), .b(new_n74_), .c(new_n128_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n135_), .c(new_n134_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n129_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n132_), .O(z11));
  nand2  g077(.a(new_n67_), .b(x27), .O(new_n140_));
  nand2  g078(.a(new_n68_), .b(new_n140_), .O(new_n141_));
  nand2  g079(.a(new_n71_), .b(new_n141_), .O(z12));
  inv1   g080(.a(new_n63_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand3  g083(.a(x20), .b(new_n145_), .c(new_n144_), .O(new_n146_));
  inv1   g084(.a(new_n146_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand2  g086(.a(x36), .b(x35), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x28), .O(new_n150_));
  nor3   g088(.a(x21), .b(x13), .c(x04), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n150_), .c(new_n116_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n148_), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n71_), .c(x27), .O(new_n154_));
  nand3  g092(.a(new_n151_), .b(new_n80_), .c(new_n116_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(z13));
  nand3  g094(.a(new_n149_), .b(new_n79_), .c(x28), .O(new_n157_));
  inv1   g095(.a(x13), .O(new_n158_));
  nand3  g096(.a(x39), .b(new_n158_), .c(new_n65_), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n157_), .c(x21), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n111_), .c(new_n146_), .O(new_n162_));
  nand2  g100(.a(new_n143_), .b(x27), .O(new_n163_));
  nand2  g101(.a(new_n76_), .b(x27), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n79_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n160_), .c(x21), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n111_), .c(new_n163_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n162_), .O(z14));
  inv1   g106(.a(x12), .O(new_n169_));
  nor2   g107(.a(new_n114_), .b(new_n169_), .O(z15));
  inv1   g108(.a(x23), .O(new_n171_));
  nand4  g109(.a(new_n71_), .b(new_n171_), .c(x22), .d(x01), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(z16));
  inv1   g111(.a(x24), .O(new_n174_));
  nand4  g112(.a(new_n174_), .b(x23), .c(x22), .d(x01), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n71_), .O(z17));
  aoi21  g114(.a(new_n133_), .b(new_n77_), .c(new_n128_), .O(new_n177_));
  nand2  g115(.a(new_n133_), .b(x27), .O(new_n178_));
  nand3  g116(.a(new_n84_), .b(x27), .c(new_n65_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n129_), .c(new_n71_), .O(new_n180_));
  aoi21  g118(.a(new_n178_), .b(new_n177_), .c(new_n180_), .O(z19));
  inv1   g119(.a(z19), .O(z18));
  oai21  g120(.a(new_n83_), .b(new_n74_), .c(new_n129_), .O(new_n183_));
  oai21  g121(.a(new_n183_), .b(new_n177_), .c(new_n71_), .O(z20));
endmodule


