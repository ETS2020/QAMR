// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n81_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n117_, new_n118_, new_n119_, new_n122_, new_n124_,
    new_n125_, new_n127_, new_n129_, new_n132_, new_n133_, new_n134_;
  nand2  g00(.a(x40), .b(x39), .O(z02));
  inv1   g01(.a(z02), .O(new_n64_));
  inv1   g02(.a(x15), .O(new_n65_));
  inv1   g03(.a(x28), .O(new_n66_));
  inv1   g04(.a(x35), .O(new_n67_));
  inv1   g05(.a(x37), .O(new_n68_));
  oai21  g06(.a(new_n67_), .b(new_n66_), .c(new_n68_), .O(new_n69_));
  nand3  g07(.a(new_n69_), .b(x27), .c(new_n65_), .O(new_n70_));
  aoi21  g08(.a(new_n70_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g09(.a(x36), .O(new_n72_));
  oai21  g10(.a(new_n67_), .b(x28), .c(new_n72_), .O(new_n73_));
  nand2  g11(.a(new_n73_), .b(x27), .O(new_n74_));
  nor2   g12(.a(x32), .b(x30), .O(new_n75_));
  nand2  g13(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g14(.a(new_n76_), .b(z02), .c(x04), .O(z01));
  nand2  g15(.a(x28), .b(x27), .O(new_n78_));
  oai22  g16(.a(new_n78_), .b(new_n67_), .c(new_n68_), .d(x27), .O(new_n79_));
  aoi21  g17(.a(new_n79_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g18(.a(x21), .O(new_n81_));
  nand3  g19(.a(new_n79_), .b(z02), .c(new_n81_), .O(z04));
  aoi21  g20(.a(new_n78_), .b(new_n68_), .c(new_n64_), .O(z05));
  inv1   g21(.a(z05), .O(z06));
  inv1   g22(.a(x25), .O(new_n85_));
  nand2  g23(.a(new_n85_), .b(x00), .O(new_n86_));
  inv1   g24(.a(x14), .O(new_n87_));
  nand2  g25(.a(x25), .b(new_n87_), .O(new_n88_));
  nand3  g26(.a(new_n88_), .b(new_n86_), .c(x38), .O(new_n89_));
  inv1   g27(.a(x17), .O(new_n90_));
  nor2   g28(.a(new_n90_), .b(x15), .O(new_n91_));
  inv1   g29(.a(x31), .O(new_n92_));
  inv1   g30(.a(x33), .O(new_n93_));
  nand2  g31(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g32(.a(new_n94_), .b(new_n91_), .c(x14), .O(new_n95_));
  and2   g33(.a(z02), .b(x03), .O(new_n96_));
  nand3  g34(.a(new_n96_), .b(new_n95_), .c(new_n89_), .O(new_n97_));
  inv1   g35(.a(new_n97_), .O(z07));
  nand3  g36(.a(x34), .b(x27), .c(x26), .O(new_n100_));
  nand2  g37(.a(z02), .b(x11), .O(new_n101_));
  nor2   g38(.a(new_n101_), .b(new_n100_), .O(z09));
  nand2  g39(.a(z02), .b(x27), .O(new_n103_));
  nand2  g40(.a(x37), .b(x06), .O(new_n104_));
  inv1   g41(.a(x04), .O(new_n105_));
  nand3  g42(.a(new_n73_), .b(x07), .c(new_n105_), .O(new_n106_));
  aoi21  g43(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(z10));
  inv1   g44(.a(x09), .O(new_n108_));
  inv1   g45(.a(x30), .O(new_n109_));
  nand2  g46(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nor2   g47(.a(x27), .b(x08), .O(new_n111_));
  nand2  g48(.a(x27), .b(x04), .O(new_n112_));
  nand3  g49(.a(new_n112_), .b(x35), .c(new_n66_), .O(new_n113_));
  nor2   g50(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  oai21  g51(.a(new_n114_), .b(new_n110_), .c(z02), .O(z11));
  aoi21  g52(.a(new_n69_), .b(x27), .c(new_n64_), .O(z12));
  nor2   g53(.a(x19), .b(x18), .O(new_n117_));
  nand2  g54(.a(new_n117_), .b(x20), .O(new_n118_));
  inv1   g55(.a(new_n118_), .O(new_n119_));
  nand4  g56(.a(new_n119_), .b(new_n69_), .c(z02), .d(x27), .O(z14));
  inv1   g57(.a(z14), .O(z13));
  inv1   g58(.a(x12), .O(new_n122_));
  oai21  g59(.a(new_n100_), .b(new_n122_), .c(z02), .O(z15));
  inv1   g60(.a(x23), .O(new_n124_));
  nand4  g61(.a(z02), .b(new_n124_), .c(x22), .d(x01), .O(new_n125_));
  inv1   g62(.a(new_n125_), .O(z16));
  nand2  g63(.a(x22), .b(x01), .O(new_n127_));
  nor4   g64(.a(new_n127_), .b(new_n64_), .c(x24), .d(new_n124_), .O(z17));
  nand3  g65(.a(z02), .b(new_n109_), .c(new_n108_), .O(new_n129_));
  nor2   g66(.a(new_n129_), .b(new_n114_), .O(z19));
  inv1   g67(.a(z19), .O(z18));
  inv1   g68(.a(x08), .O(new_n132_));
  inv1   g69(.a(x27), .O(new_n133_));
  oai21  g70(.a(x28), .b(new_n132_), .c(new_n133_), .O(new_n134_));
  aoi21  g71(.a(new_n134_), .b(x35), .c(new_n129_), .O(z20));
  zero   g72(.O(z08));
endmodule


