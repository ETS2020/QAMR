// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n163_, new_n165_, new_n166_, new_n168_, new_n169_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x34), .O(new_n71_));
  nor2   g009(.a(x36), .b(new_n71_), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  nor2   g013(.a(x32), .b(x30), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n75_), .c(new_n76_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x04), .c(new_n72_), .O(z01));
  nand2  g018(.a(x35), .b(new_n77_), .O(new_n81_));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n81_), .c(x04), .O(new_n83_));
  nand2  g021(.a(x40), .b(x39), .O(new_n84_));
  aoi21  g022(.a(x29), .b(x08), .c(x02), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(new_n83_), .c(new_n72_), .O(z02));
  inv1   g025(.a(x35), .O(new_n88_));
  inv1   g026(.a(x37), .O(new_n89_));
  nand2  g027(.a(x28), .b(x27), .O(new_n90_));
  oai22  g028(.a(new_n90_), .b(new_n88_), .c(new_n89_), .d(x27), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g030(.a(x21), .O(new_n93_));
  inv1   g031(.a(new_n72_), .O(new_n94_));
  nand3  g032(.a(new_n91_), .b(new_n94_), .c(new_n93_), .O(z04));
  nand2  g033(.a(new_n90_), .b(new_n89_), .O(new_n96_));
  inv1   g034(.a(new_n96_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n94_), .O(z05));
  nand2  g036(.a(new_n96_), .b(new_n94_), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n94_), .O(z07));
  nand2  g048(.a(new_n94_), .b(new_n84_), .O(z08));
  nand3  g049(.a(x27), .b(x26), .c(x11), .O(new_n112_));
  aoi21  g050(.a(new_n112_), .b(x36), .c(new_n71_), .O(z09));
  oai21  g051(.a(new_n78_), .b(new_n75_), .c(new_n84_), .O(new_n114_));
  inv1   g052(.a(x05), .O(new_n115_));
  nand3  g053(.a(x40), .b(x39), .c(new_n115_), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  aoi21  g055(.a(new_n84_), .b(new_n117_), .c(x04), .O(new_n118_));
  nand4  g056(.a(new_n118_), .b(new_n116_), .c(new_n114_), .d(new_n79_), .O(new_n119_));
  nand3  g057(.a(x37), .b(x27), .c(x06), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(new_n72_), .O(z10));
  inv1   g059(.a(x08), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(x29), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n81_), .c(new_n122_), .O(new_n124_));
  nand2  g062(.a(new_n123_), .b(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g064(.a(x35), .b(new_n77_), .c(x27), .d(new_n63_), .O(new_n127_));
  nor3   g065(.a(new_n72_), .b(x30), .c(x09), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(z18));
  inv1   g067(.a(z18), .O(z11));
  nand2  g068(.a(new_n68_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n65_), .b(x10), .O(new_n132_));
  nand3  g070(.a(new_n94_), .b(new_n132_), .c(new_n131_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z12));
  nand2  g072(.a(x35), .b(x28), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n89_), .O(new_n136_));
  nor2   g074(.a(x19), .b(x18), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n136_), .c(x20), .O(new_n138_));
  nor2   g076(.a(x13), .b(x04), .O(new_n139_));
  nand4  g077(.a(new_n139_), .b(x40), .c(x39), .d(new_n77_), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n138_), .c(new_n72_), .O(new_n141_));
  nand3  g079(.a(new_n139_), .b(x40), .c(x39), .O(new_n142_));
  nand2  g080(.a(x36), .b(x35), .O(new_n143_));
  nor2   g081(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n141_), .c(x27), .O(new_n145_));
  inv1   g083(.a(x13), .O(new_n146_));
  inv1   g084(.a(new_n76_), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(new_n94_), .c(new_n65_), .d(new_n146_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n145_), .O(z13));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand3  g089(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(new_n153_));
  nand2  g091(.a(new_n89_), .b(new_n88_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  aoi21  g093(.a(x36), .b(x35), .c(new_n77_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n155_), .c(new_n75_), .O(new_n157_));
  nand3  g095(.a(new_n153_), .b(new_n136_), .c(x27), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n142_), .c(new_n72_), .O(new_n159_));
  oai21  g097(.a(new_n157_), .b(new_n147_), .c(new_n159_), .O(z14));
  nand3  g098(.a(x27), .b(x26), .c(x12), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(x36), .c(new_n71_), .O(z15));
  nand2  g100(.a(x22), .b(x01), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(x23), .c(new_n94_), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x23), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n163_), .c(new_n94_), .O(z17));
  nand2  g105(.a(x35), .b(x27), .O(new_n168_));
  nand2  g106(.a(new_n128_), .b(new_n168_), .O(new_n169_));
  nor2   g107(.a(new_n169_), .b(new_n124_), .O(z20));
  inv1   g108(.a(z18), .O(z19));
endmodule


