// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n172_, new_n173_, new_n175_,
    new_n177_, new_n179_, new_n180_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  inv1   g009(.a(x22), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  oai21  g013(.a(new_n70_), .b(x15), .c(new_n75_), .O(z00));
  inv1   g014(.a(x27), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n78_), .c(new_n77_), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nor2   g021(.a(new_n74_), .b(new_n64_), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n81_), .c(new_n84_), .O(z01));
  inv1   g023(.a(x35), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(x28), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  nand2  g026(.a(new_n77_), .b(new_n88_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  and2   g028(.a(x29), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(x02), .c(x39), .O(new_n92_));
  aoi21  g030(.a(new_n90_), .b(x04), .c(new_n92_), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(x22), .c(x40), .O(z02));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  oai22  g034(.a(new_n96_), .b(new_n86_), .c(new_n95_), .d(x27), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n74_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(new_n74_), .O(new_n100_));
  nand3  g038(.a(new_n97_), .b(new_n100_), .c(new_n99_), .O(z04));
  nand3  g039(.a(new_n96_), .b(new_n100_), .c(new_n95_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n100_), .O(z07));
  inv1   g052(.a(x39), .O(new_n115_));
  aoi21  g053(.a(new_n115_), .b(new_n72_), .c(new_n73_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand3  g055(.a(x34), .b(x27), .c(x26), .O(new_n118_));
  nor3   g056(.a(new_n118_), .b(new_n74_), .c(new_n117_), .O(z09));
  nor2   g057(.a(new_n73_), .b(new_n115_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n83_), .O(new_n121_));
  oai21  g059(.a(new_n86_), .b(x28), .c(new_n78_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  oai21  g061(.a(new_n73_), .b(new_n115_), .c(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n122_), .c(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  aoi21  g065(.a(new_n120_), .b(new_n127_), .c(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n100_), .O(z10));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n80_), .c(new_n88_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nor2   g073(.a(new_n77_), .b(x04), .O(new_n136_));
  inv1   g074(.a(x09), .O(new_n137_));
  inv1   g075(.a(x30), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  aoi21  g077(.a(new_n136_), .b(new_n87_), .c(new_n139_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n135_), .c(new_n100_), .O(z18));
  inv1   g079(.a(z18), .O(z11));
  nand2  g080(.a(new_n63_), .b(new_n95_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(x10), .O(new_n144_));
  nor2   g082(.a(new_n115_), .b(x04), .O(new_n145_));
  inv1   g083(.a(new_n66_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x27), .c(new_n145_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n144_), .c(new_n72_), .O(new_n148_));
  nand2  g086(.a(new_n146_), .b(x27), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n73_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n148_), .O(z12));
  nor2   g089(.a(x13), .b(x04), .O(new_n152_));
  nand4  g090(.a(new_n152_), .b(new_n120_), .c(new_n83_), .d(new_n72_), .O(new_n153_));
  nor2   g091(.a(x19), .b(x18), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n146_), .c(x20), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x28), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(new_n152_), .c(new_n120_), .d(new_n72_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n155_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n100_), .c(x27), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n153_), .O(z13));
  nand2  g099(.a(new_n154_), .b(x20), .O(new_n162_));
  nand3  g100(.a(new_n156_), .b(new_n82_), .c(x28), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n152_), .c(new_n120_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  oai21  g103(.a(x28), .b(new_n77_), .c(new_n82_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n152_), .c(new_n120_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n149_), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n165_), .c(new_n100_), .O(z14));
  inv1   g107(.a(x12), .O(new_n170_));
  oai21  g108(.a(new_n118_), .b(new_n170_), .c(new_n100_), .O(z15));
  inv1   g109(.a(x23), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(x01), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n73_), .c(new_n72_), .O(z16));
  nand2  g112(.a(x22), .b(x01), .O(new_n175_));
  nor4   g113(.a(new_n175_), .b(x40), .c(x24), .d(new_n172_), .O(z17));
  nand2  g114(.a(new_n140_), .b(new_n135_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n100_), .O(z19));
  nand2  g116(.a(x35), .b(x27), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n138_), .c(new_n137_), .O(new_n180_));
  oai21  g118(.a(new_n180_), .b(new_n133_), .c(new_n100_), .O(z20));
endmodule


