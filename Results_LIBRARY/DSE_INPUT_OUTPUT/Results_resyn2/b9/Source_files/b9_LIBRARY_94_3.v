// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n172_,
    new_n173_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n187_, new_n188_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  nand2  g007(.a(x40), .b(x12), .O(new_n70_));
  and2   g008(.a(new_n70_), .b(x16), .O(new_n71_));
  oai21  g009(.a(new_n69_), .b(x15), .c(new_n71_), .O(z00));
  inv1   g010(.a(x35), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  oai21  g012(.a(new_n73_), .b(x28), .c(new_n74_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(x27), .O(new_n76_));
  nor2   g014(.a(x32), .b(x30), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n70_), .c(x04), .O(z01));
  inv1   g017(.a(x28), .O(new_n80_));
  inv1   g018(.a(x08), .O(new_n81_));
  inv1   g019(.a(x27), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(x35), .c(new_n80_), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  inv1   g023(.a(new_n85_), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x02), .c(x39), .O(new_n87_));
  aoi21  g025(.a(new_n84_), .b(x04), .c(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(x12), .c(x40), .O(z02));
  inv1   g027(.a(x37), .O(new_n90_));
  inv1   g028(.a(new_n70_), .O(new_n91_));
  aoi21  g029(.a(new_n90_), .b(new_n82_), .c(new_n91_), .O(new_n92_));
  oai21  g030(.a(new_n73_), .b(new_n80_), .c(x27), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n92_), .c(new_n95_), .O(z04));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n70_), .c(new_n90_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n70_), .O(z07));
  inv1   g048(.a(x12), .O(new_n111_));
  nand3  g049(.a(x40), .b(x39), .c(new_n111_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z08));
  nand3  g051(.a(x34), .b(x27), .c(x26), .O(new_n114_));
  nand2  g052(.a(new_n70_), .b(x11), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n114_), .O(z09));
  nand3  g054(.a(new_n70_), .b(x37), .c(x06), .O(new_n117_));
  inv1   g055(.a(x40), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  nand2  g057(.a(x39), .b(new_n119_), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n111_), .c(new_n118_), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  nand2  g060(.a(x40), .b(x39), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n75_), .c(new_n63_), .O(new_n125_));
  oai21  g063(.a(new_n125_), .b(new_n121_), .c(new_n117_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x27), .O(new_n127_));
  nor2   g065(.a(new_n123_), .b(new_n77_), .O(new_n128_));
  nor2   g066(.a(x12), .b(x04), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(x05), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n127_), .O(z10));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n81_), .c(x04), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n111_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x40), .O(new_n135_));
  nand2  g073(.a(x35), .b(new_n80_), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n82_), .c(new_n81_), .O(new_n137_));
  nand3  g075(.a(x40), .b(x39), .c(x29), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand3  g077(.a(new_n138_), .b(x27), .c(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n139_), .c(new_n137_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n132_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n135_), .O(z11));
  nand2  g081(.a(new_n66_), .b(new_n90_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x27), .c(x10), .O(new_n145_));
  nor2   g083(.a(new_n67_), .b(new_n82_), .O(new_n146_));
  aoi21  g084(.a(x39), .b(new_n63_), .c(new_n146_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n145_), .c(new_n111_), .O(new_n148_));
  inv1   g086(.a(new_n146_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n118_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n148_), .O(z12));
  nand2  g089(.a(new_n70_), .b(x27), .O(new_n152_));
  nor2   g090(.a(x19), .b(x18), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x20), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n67_), .O(new_n155_));
  inv1   g093(.a(x13), .O(new_n156_));
  nand4  g094(.a(x40), .b(x39), .c(new_n156_), .d(new_n63_), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(new_n158_));
  nand2  g096(.a(x36), .b(x35), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x28), .c(x12), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n158_), .c(new_n155_), .O(new_n161_));
  nand3  g099(.a(new_n129_), .b(new_n128_), .c(new_n156_), .O(new_n162_));
  oai21  g100(.a(new_n161_), .b(new_n152_), .c(new_n162_), .O(z13));
  nand3  g101(.a(new_n159_), .b(new_n77_), .c(x28), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n157_), .c(new_n154_), .O(new_n166_));
  oai21  g104(.a(x28), .b(new_n82_), .c(new_n77_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n158_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n149_), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n166_), .c(new_n70_), .O(z14));
  aoi21  g108(.a(new_n114_), .b(new_n118_), .c(new_n111_), .O(z15));
  inv1   g109(.a(x23), .O(new_n172_));
  nand4  g110(.a(new_n70_), .b(new_n172_), .c(x22), .d(x01), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(z16));
  nand2  g112(.a(x22), .b(x01), .O(new_n175_));
  nor4   g113(.a(new_n175_), .b(new_n91_), .c(x24), .d(new_n172_), .O(z17));
  aoi21  g114(.a(new_n82_), .b(new_n81_), .c(new_n136_), .O(new_n177_));
  inv1   g115(.a(new_n132_), .O(new_n178_));
  nand2  g116(.a(x27), .b(x04), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n177_), .c(new_n178_), .O(new_n180_));
  oai22  g118(.a(new_n180_), .b(new_n91_), .c(new_n112_), .d(new_n85_), .O(z18));
  inv1   g119(.a(x39), .O(new_n182_));
  oai21  g120(.a(new_n85_), .b(new_n182_), .c(new_n111_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(x40), .O(new_n184_));
  nand2  g122(.a(new_n184_), .b(new_n180_), .O(new_n185_));
  inv1   g123(.a(new_n185_), .O(z19));
  nand2  g124(.a(x35), .b(x27), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n132_), .c(new_n70_), .O(new_n188_));
  aoi21  g126(.a(new_n139_), .b(x08), .c(new_n188_), .O(z20));
endmodule


