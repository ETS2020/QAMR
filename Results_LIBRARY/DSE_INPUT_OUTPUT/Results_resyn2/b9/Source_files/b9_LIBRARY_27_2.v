// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n171_, new_n173_, new_n174_,
    new_n176_, new_n177_, new_n178_, new_n180_, new_n182_, new_n183_;
  inv1   g000(.a(x36), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x19), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  nand4  g008(.a(x40), .b(x39), .c(x10), .d(new_n70_), .O(new_n71_));
  oai21  g009(.a(new_n69_), .b(new_n68_), .c(new_n71_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n66_), .c(new_n67_), .O(new_n73_));
  inv1   g011(.a(x35), .O(new_n74_));
  oai21  g012(.a(new_n74_), .b(x19), .c(new_n63_), .O(new_n75_));
  nand2  g013(.a(x40), .b(x39), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  nor2   g015(.a(new_n68_), .b(x04), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n75_), .c(new_n66_), .O(new_n81_));
  oai21  g019(.a(new_n73_), .b(new_n65_), .c(new_n81_), .O(z00));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(new_n63_), .c(new_n68_), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  inv1   g024(.a(new_n86_), .O(new_n87_));
  nor2   g025(.a(new_n65_), .b(new_n70_), .O(new_n88_));
  oai21  g026(.a(new_n87_), .b(new_n85_), .c(new_n88_), .O(z01));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n84_), .c(x04), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n91_), .c(new_n77_), .d(new_n64_), .O(z02));
  inv1   g033(.a(x37), .O(new_n96_));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  oai22  g035(.a(new_n97_), .b(new_n74_), .c(new_n96_), .d(x27), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n64_), .c(new_n100_), .O(z04));
  nand2  g039(.a(new_n97_), .b(new_n96_), .O(new_n102_));
  and2   g040(.a(new_n102_), .b(new_n64_), .O(z05));
  nor2   g041(.a(new_n102_), .b(new_n65_), .O(z06));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n66_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n105_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n64_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nand2  g052(.a(new_n76_), .b(new_n64_), .O(z08));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n64_), .c(x11), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z09));
  nand2  g057(.a(new_n77_), .b(new_n70_), .O(new_n120_));
  nand3  g058(.a(x37), .b(x27), .c(x06), .O(new_n121_));
  nand2  g059(.a(new_n87_), .b(x05), .O(new_n122_));
  oai21  g060(.a(new_n122_), .b(new_n120_), .c(new_n121_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n64_), .O(new_n124_));
  oai21  g062(.a(new_n84_), .b(x19), .c(new_n63_), .O(new_n125_));
  inv1   g063(.a(x07), .O(new_n126_));
  nand2  g064(.a(new_n76_), .b(new_n126_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand2  g066(.a(new_n77_), .b(new_n128_), .O(new_n129_));
  nand4  g067(.a(new_n129_), .b(new_n127_), .c(new_n125_), .d(new_n78_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n124_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n84_), .c(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  inv1   g074(.a(new_n84_), .O(new_n137_));
  inv1   g075(.a(x09), .O(new_n138_));
  inv1   g076(.a(x30), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  aoi21  g078(.a(new_n137_), .b(new_n78_), .c(new_n140_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n136_), .c(new_n64_), .O(z18));
  inv1   g080(.a(z18), .O(z11));
  aoi22  g081(.a(new_n80_), .b(new_n75_), .c(new_n72_), .d(new_n64_), .O(z12));
  nor2   g082(.a(x13), .b(x04), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n77_), .O(new_n146_));
  oai21  g084(.a(x28), .b(new_n68_), .c(new_n86_), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  aoi22  g087(.a(new_n149_), .b(x27), .c(new_n147_), .d(new_n64_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  nor2   g089(.a(new_n68_), .b(x19), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(x20), .c(new_n151_), .O(new_n153_));
  oai22  g091(.a(new_n153_), .b(new_n69_), .c(new_n150_), .d(new_n146_), .O(z13));
  nand2  g092(.a(x20), .b(new_n151_), .O(new_n155_));
  inv1   g093(.a(new_n146_), .O(new_n156_));
  inv1   g094(.a(x32), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n139_), .c(x28), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n148_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n156_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n155_), .O(new_n162_));
  inv1   g100(.a(x19), .O(new_n163_));
  aoi21  g101(.a(x37), .b(new_n163_), .c(new_n158_), .O(new_n164_));
  aoi21  g102(.a(new_n145_), .b(new_n77_), .c(x37), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n164_), .c(new_n74_), .O(new_n166_));
  oai21  g104(.a(x37), .b(x28), .c(new_n152_), .O(new_n167_));
  oai21  g105(.a(new_n87_), .b(x27), .c(new_n64_), .O(new_n168_));
  aoi21  g106(.a(new_n167_), .b(new_n146_), .c(new_n168_), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n166_), .c(new_n162_), .O(z14));
  nand3  g108(.a(new_n117_), .b(new_n64_), .c(x12), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z15));
  inv1   g110(.a(x01), .O(new_n173_));
  inv1   g111(.a(x22), .O(new_n174_));
  nor4   g112(.a(new_n65_), .b(x23), .c(new_n174_), .d(new_n173_), .O(z16));
  inv1   g113(.a(x24), .O(new_n176_));
  nor2   g114(.a(new_n174_), .b(new_n173_), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n64_), .c(new_n176_), .d(x23), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z17));
  nand2  g117(.a(new_n141_), .b(new_n136_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n64_), .O(z19));
  nand2  g119(.a(x35), .b(x27), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n139_), .c(new_n138_), .O(new_n183_));
  oai21  g121(.a(new_n183_), .b(new_n134_), .c(new_n64_), .O(z20));
endmodule


