// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n164_, new_n166_, new_n169_;
  nand2  g000(.a(x36), .b(x11), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  nand4  g007(.a(x40), .b(x39), .c(x10), .d(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n68_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n65_), .c(new_n66_), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n64_), .O(z08));
  nor2   g012(.a(new_n67_), .b(x04), .O(new_n75_));
  inv1   g013(.a(x35), .O(new_n76_));
  inv1   g014(.a(x36), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n76_), .c(x15), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n75_), .c(z08), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n64_), .c(new_n79_), .O(z00));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n77_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n81_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n64_), .O(z01));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n83_), .c(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n73_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n89_), .c(new_n64_), .O(z02));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n76_), .c(new_n93_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n63_), .c(new_n97_), .O(z04));
  aoi21  g036(.a(new_n94_), .b(new_n93_), .c(new_n64_), .O(z05));
  nand3  g037(.a(new_n94_), .b(new_n63_), .c(new_n93_), .O(new_n100_));
  inv1   g038(.a(new_n100_), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n65_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n102_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n63_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  inv1   g049(.a(x11), .O(new_n112_));
  nand3  g050(.a(x34), .b(x27), .c(x26), .O(new_n113_));
  aoi21  g051(.a(new_n113_), .b(new_n77_), .c(new_n112_), .O(z09));
  nor2   g052(.a(new_n81_), .b(new_n73_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  nand2  g055(.a(new_n73_), .b(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n84_), .c(x27), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  inv1   g058(.a(x05), .O(new_n121_));
  inv1   g059(.a(new_n73_), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(x04), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n124_), .c(new_n63_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  nand2  g065(.a(new_n122_), .b(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n83_), .c(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand3  g068(.a(new_n75_), .b(x35), .c(new_n82_), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n63_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  aoi21  g073(.a(new_n130_), .b(new_n129_), .c(new_n135_), .O(z11));
  nor2   g074(.a(x27), .b(x10), .O(new_n137_));
  nor3   g075(.a(new_n137_), .b(new_n73_), .c(x04), .O(new_n138_));
  inv1   g076(.a(new_n68_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x27), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n63_), .O(new_n141_));
  inv1   g079(.a(x10), .O(new_n142_));
  nand4  g080(.a(new_n93_), .b(new_n77_), .c(new_n76_), .d(new_n142_), .O(new_n143_));
  oai21  g081(.a(new_n141_), .b(new_n138_), .c(new_n143_), .O(z12));
  aoi21  g082(.a(x36), .b(x35), .c(new_n82_), .O(new_n145_));
  nor2   g083(.a(x13), .b(x04), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n122_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x20), .O(new_n149_));
  oai22  g087(.a(new_n149_), .b(new_n68_), .c(new_n147_), .d(new_n145_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  aoi21  g089(.a(new_n146_), .b(new_n115_), .c(new_n64_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n151_), .O(z13));
  nand2  g091(.a(new_n145_), .b(new_n81_), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n147_), .c(new_n149_), .O(new_n156_));
  inv1   g094(.a(new_n81_), .O(new_n157_));
  aoi21  g095(.a(new_n82_), .b(x27), .c(new_n157_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n147_), .c(new_n140_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n156_), .c(new_n63_), .O(z14));
  inv1   g098(.a(x12), .O(new_n161_));
  oai21  g099(.a(new_n113_), .b(new_n161_), .c(new_n63_), .O(z15));
  inv1   g100(.a(x23), .O(new_n163_));
  nand4  g101(.a(new_n63_), .b(new_n163_), .c(x22), .d(x01), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z16));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  nor4   g104(.a(new_n166_), .b(new_n64_), .c(x24), .d(new_n163_), .O(z17));
  inv1   g105(.a(z11), .O(z18));
  oai21  g106(.a(new_n76_), .b(new_n67_), .c(new_n134_), .O(new_n169_));
  nor2   g107(.a(new_n169_), .b(new_n129_), .O(z20));
  aoi21  g108(.a(new_n130_), .b(new_n129_), .c(new_n135_), .O(z19));
endmodule


