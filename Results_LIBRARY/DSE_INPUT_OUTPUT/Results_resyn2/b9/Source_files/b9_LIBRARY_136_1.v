// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n162_, new_n164_, new_n165_, new_n169_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  inv1   g009(.a(x32), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(x23), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  oai21  g012(.a(new_n70_), .b(x15), .c(new_n74_), .O(z00));
  oai21  g013(.a(new_n72_), .b(x23), .c(new_n65_), .O(new_n76_));
  inv1   g014(.a(x27), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  aoi21  g016(.a(x35), .b(new_n78_), .c(x36), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  oai21  g018(.a(new_n79_), .b(new_n77_), .c(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n76_), .O(z01));
  nand2  g020(.a(x35), .b(new_n78_), .O(new_n83_));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  nand2  g023(.a(x40), .b(x39), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n85_), .c(new_n73_), .O(z02));
  inv1   g027(.a(x23), .O(new_n90_));
  nand2  g028(.a(x32), .b(new_n90_), .O(new_n91_));
  inv1   g029(.a(x35), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n78_), .c(x27), .O(new_n93_));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n77_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n93_), .c(new_n91_), .d(x21), .O(z03));
  aoi21  g034(.a(new_n94_), .b(new_n77_), .c(x21), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n93_), .c(new_n73_), .O(z04));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n91_), .c(new_n94_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n102_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n91_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  nor2   g050(.a(new_n73_), .b(new_n86_), .O(z08));
  nand3  g051(.a(x34), .b(x27), .c(x26), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n91_), .c(x11), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z09));
  nor2   g055(.a(new_n79_), .b(new_n76_), .O(new_n118_));
  inv1   g056(.a(new_n86_), .O(new_n119_));
  nand2  g057(.a(new_n72_), .b(x05), .O(new_n120_));
  aoi21  g058(.a(x40), .b(x39), .c(x07), .O(new_n121_));
  aoi21  g059(.a(new_n120_), .b(new_n119_), .c(new_n121_), .O(new_n122_));
  nand2  g060(.a(x37), .b(x06), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n73_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n118_), .c(new_n124_), .O(new_n125_));
  inv1   g063(.a(new_n80_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n119_), .c(new_n65_), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n91_), .c(x05), .O(new_n129_));
  oai21  g067(.a(new_n125_), .b(new_n77_), .c(new_n129_), .O(z10));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n83_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nor2   g073(.a(new_n92_), .b(new_n77_), .O(new_n136_));
  nor2   g074(.a(x28), .b(x04), .O(new_n137_));
  or2    g075(.a(x30), .b(x09), .O(new_n138_));
  aoi21  g076(.a(new_n137_), .b(new_n136_), .c(new_n138_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n135_), .c(new_n91_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z11));
  nand2  g079(.a(new_n91_), .b(new_n70_), .O(new_n142_));
  inv1   g080(.a(new_n142_), .O(z12));
  aoi21  g081(.a(x36), .b(x35), .c(new_n78_), .O(new_n144_));
  nor2   g082(.a(x13), .b(x04), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n119_), .O(new_n146_));
  inv1   g084(.a(x18), .O(new_n147_));
  inv1   g085(.a(x19), .O(new_n148_));
  nand3  g086(.a(x20), .b(new_n148_), .c(new_n147_), .O(new_n149_));
  oai22  g087(.a(new_n149_), .b(new_n63_), .c(new_n146_), .d(new_n144_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  oai21  g089(.a(new_n127_), .b(x13), .c(new_n91_), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n151_), .O(z13));
  inv1   g092(.a(new_n149_), .O(new_n155_));
  aoi22  g093(.a(new_n146_), .b(new_n91_), .c(new_n144_), .d(new_n80_), .O(new_n156_));
  nand3  g094(.a(new_n145_), .b(new_n119_), .c(new_n78_), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n63_), .c(new_n77_), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n152_), .c(new_n156_), .d(new_n155_), .O(z14));
  nand3  g097(.a(new_n115_), .b(new_n91_), .c(x12), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z15));
  nand2  g099(.a(x22), .b(x01), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n72_), .c(x23), .O(z16));
  inv1   g101(.a(x24), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x23), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n162_), .c(new_n91_), .O(z17));
  aoi21  g104(.a(new_n139_), .b(new_n135_), .c(new_n73_), .O(z18));
  inv1   g105(.a(z18), .O(z19));
  oai21  g106(.a(new_n92_), .b(new_n77_), .c(new_n91_), .O(new_n169_));
  nor3   g107(.a(new_n169_), .b(new_n138_), .c(new_n133_), .O(z20));
endmodule


