// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n105_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n164_, new_n166_, new_n168_,
    new_n170_, new_n171_, new_n173_, new_n174_, new_n175_, new_n176_;
  inv1   g000(.a(x14), .O(new_n63_));
  inv1   g001(.a(x39), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g013(.a(x14), .b(x04), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .d(new_n66_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x35), .O(new_n83_));
  inv1   g021(.a(x36), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(x28), .c(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n82_), .c(new_n81_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g026(.a(x04), .O(new_n89_));
  inv1   g027(.a(x08), .O(new_n90_));
  inv1   g028(.a(x28), .O(new_n91_));
  nand2  g029(.a(x35), .b(new_n91_), .O(new_n92_));
  aoi21  g030(.a(new_n68_), .b(new_n90_), .c(new_n92_), .O(new_n93_));
  inv1   g031(.a(x02), .O(new_n94_));
  nand3  g032(.a(x40), .b(x39), .c(new_n63_), .O(new_n95_));
  nand2  g033(.a(x29), .b(x08), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n94_), .c(new_n95_), .O(new_n97_));
  oai21  g035(.a(new_n93_), .b(new_n89_), .c(new_n97_), .O(z02));
  nand2  g036(.a(new_n70_), .b(x27), .O(new_n99_));
  aoi21  g037(.a(new_n69_), .b(new_n68_), .c(new_n65_), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n99_), .c(x21), .O(z03));
  inv1   g039(.a(x21), .O(new_n102_));
  nand3  g040(.a(new_n100_), .b(new_n99_), .c(new_n102_), .O(z04));
  inv1   g041(.a(new_n65_), .O(new_n104_));
  nand2  g042(.a(x28), .b(x27), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n104_), .c(new_n69_), .O(z05));
  inv1   g044(.a(z05), .O(z06));
  nand2  g045(.a(x17), .b(new_n66_), .O(new_n108_));
  nor3   g046(.a(x39), .b(x33), .c(x31), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n108_), .c(new_n63_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  nand2  g050(.a(x25), .b(new_n63_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(x38), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x03), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n110_), .O(z07));
  inv1   g054(.a(new_n95_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n104_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  nand2  g058(.a(x37), .b(x06), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  nand2  g060(.a(new_n77_), .b(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n85_), .c(new_n89_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  nand3  g064(.a(new_n121_), .b(x40), .c(new_n126_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n63_), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(x39), .c(new_n68_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand2  g068(.a(new_n82_), .b(new_n81_), .O(new_n131_));
  nor2   g069(.a(new_n64_), .b(new_n126_), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(new_n131_), .c(new_n76_), .d(x40), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(z10));
  inv1   g072(.a(new_n96_), .O(new_n135_));
  inv1   g073(.a(x09), .O(new_n136_));
  nand2  g074(.a(new_n68_), .b(new_n90_), .O(new_n137_));
  nand2  g075(.a(x27), .b(x04), .O(new_n138_));
  nand4  g076(.a(new_n138_), .b(new_n137_), .c(x35), .d(new_n91_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n81_), .c(new_n136_), .O(new_n140_));
  aoi22  g078(.a(new_n140_), .b(new_n104_), .c(new_n135_), .d(z08), .O(z11));
  nand3  g079(.a(new_n69_), .b(new_n84_), .c(new_n83_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  nand2  g081(.a(new_n70_), .b(new_n69_), .O(new_n144_));
  aoi22  g082(.a(new_n144_), .b(x27), .c(x40), .d(new_n89_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n143_), .c(new_n63_), .O(new_n146_));
  or2    g084(.a(new_n71_), .b(x39), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(z12));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n144_), .c(x20), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x28), .O(new_n152_));
  nor2   g090(.a(x13), .b(x04), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n152_), .c(new_n78_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n150_), .c(new_n68_), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  oai21  g094(.a(x32), .b(x30), .c(x40), .O(new_n157_));
  nor3   g095(.a(new_n157_), .b(x13), .c(x04), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(x14), .c(x39), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n156_), .O(z13));
  nand4  g098(.a(new_n153_), .b(new_n131_), .c(x40), .d(x39), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n155_), .c(new_n104_), .O(z14));
  inv1   g101(.a(x12), .O(new_n164_));
  nor2   g102(.a(new_n119_), .b(new_n164_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(x23), .c(new_n104_), .O(z16));
  inv1   g105(.a(x23), .O(new_n168_));
  nor4   g106(.a(new_n166_), .b(new_n65_), .c(x24), .d(new_n168_), .O(z17));
  nand2  g107(.a(new_n140_), .b(new_n104_), .O(new_n170_));
  nand2  g108(.a(new_n135_), .b(z08), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n170_), .O(z18));
  inv1   g110(.a(x29), .O(new_n173_));
  oai21  g111(.a(new_n77_), .b(new_n173_), .c(new_n92_), .O(new_n174_));
  nand2  g112(.a(x35), .b(x27), .O(new_n175_));
  nand4  g113(.a(new_n175_), .b(new_n104_), .c(new_n81_), .d(new_n136_), .O(new_n176_));
  aoi21  g114(.a(new_n174_), .b(x08), .c(new_n176_), .O(z20));
  aoi22  g115(.a(new_n140_), .b(new_n104_), .c(new_n135_), .d(z08), .O(z19));
endmodule


