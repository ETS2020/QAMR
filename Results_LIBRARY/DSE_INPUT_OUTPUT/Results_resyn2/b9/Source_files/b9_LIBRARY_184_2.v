// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n162_, new_n165_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x34), .O(new_n71_));
  nor2   g009(.a(x36), .b(new_n71_), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  nor2   g012(.a(x32), .b(x30), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n75_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n72_), .O(z01));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n78_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(new_n84_), .c(new_n72_), .O(z02));
  inv1   g026(.a(x35), .O(new_n89_));
  inv1   g027(.a(x37), .O(new_n90_));
  nand2  g028(.a(x28), .b(x27), .O(new_n91_));
  oai22  g029(.a(new_n91_), .b(new_n89_), .c(new_n90_), .d(x27), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g031(.a(x21), .O(new_n94_));
  inv1   g032(.a(new_n72_), .O(new_n95_));
  nand3  g033(.a(new_n92_), .b(new_n95_), .c(new_n94_), .O(z04));
  aoi21  g034(.a(new_n91_), .b(new_n90_), .c(new_n72_), .O(z05));
  nand2  g035(.a(new_n91_), .b(new_n90_), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n72_), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n95_), .O(z07));
  nand2  g048(.a(new_n95_), .b(new_n85_), .O(z08));
  inv1   g049(.a(x27), .O(new_n112_));
  nand2  g050(.a(x26), .b(x11), .O(new_n113_));
  nor4   g051(.a(new_n113_), .b(new_n76_), .c(new_n71_), .d(new_n112_), .O(z09));
  inv1   g052(.a(new_n85_), .O(new_n115_));
  oai21  g053(.a(x32), .b(x30), .c(new_n115_), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  nand2  g055(.a(new_n85_), .b(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n79_), .c(x27), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  inv1   g058(.a(x05), .O(new_n121_));
  aoi21  g059(.a(new_n115_), .b(new_n121_), .c(x04), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n123_), .c(new_n95_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  nand2  g064(.a(new_n115_), .b(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n78_), .c(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand4  g067(.a(x35), .b(new_n77_), .c(x27), .d(new_n63_), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(new_n95_), .O(new_n132_));
  aoi21  g070(.a(new_n129_), .b(new_n128_), .c(new_n132_), .O(z11));
  nand2  g071(.a(new_n68_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n65_), .b(x10), .O(new_n135_));
  nand3  g073(.a(new_n95_), .b(new_n135_), .c(new_n134_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(z12));
  nand2  g075(.a(x35), .b(x28), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n90_), .O(new_n139_));
  nor2   g077(.a(x19), .b(x18), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n139_), .c(x20), .O(new_n141_));
  nor2   g079(.a(x13), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n115_), .c(new_n77_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n141_), .c(new_n112_), .O(new_n144_));
  nand2  g082(.a(new_n142_), .b(new_n115_), .O(new_n145_));
  nor2   g083(.a(new_n145_), .b(new_n75_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n144_), .c(new_n95_), .O(new_n147_));
  inv1   g085(.a(new_n145_), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(x36), .c(x35), .d(x27), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n147_), .O(z13));
  aoi21  g088(.a(x36), .b(x35), .c(new_n77_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n145_), .c(new_n141_), .O(new_n152_));
  oai21  g090(.a(new_n145_), .b(new_n75_), .c(new_n95_), .O(new_n153_));
  aoi21  g091(.a(new_n152_), .b(x27), .c(new_n153_), .O(z14));
  nand3  g092(.a(x27), .b(x26), .c(x12), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(x36), .c(new_n71_), .O(z15));
  inv1   g094(.a(x01), .O(new_n157_));
  inv1   g095(.a(x22), .O(new_n158_));
  nor4   g096(.a(new_n72_), .b(x23), .c(new_n158_), .d(new_n157_), .O(z16));
  inv1   g097(.a(x24), .O(new_n160_));
  nor2   g098(.a(new_n158_), .b(new_n157_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n95_), .c(new_n160_), .d(x23), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(z17));
  inv1   g101(.a(z11), .O(z18));
  oai21  g102(.a(new_n89_), .b(new_n112_), .c(new_n131_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n128_), .c(new_n95_), .O(z20));
  aoi21  g104(.a(new_n129_), .b(new_n128_), .c(new_n132_), .O(z19));
endmodule


