// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n96_, new_n98_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n147_, new_n150_;
  nand2  g00(.a(x22), .b(x01), .O(new_n63_));
  inv1   g01(.a(new_n63_), .O(z16));
  inv1   g02(.a(x15), .O(new_n65_));
  inv1   g03(.a(x27), .O(new_n66_));
  aoi21  g04(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g05(.a(x04), .O(new_n68_));
  inv1   g06(.a(x35), .O(new_n69_));
  inv1   g07(.a(x36), .O(new_n70_));
  nand2  g08(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  and2   g09(.a(x40), .b(x39), .O(new_n72_));
  nand3  g10(.a(new_n72_), .b(new_n71_), .c(new_n68_), .O(new_n73_));
  aoi21  g11(.a(new_n73_), .b(new_n67_), .c(new_n66_), .O(new_n74_));
  nand2  g12(.a(x40), .b(x39), .O(new_n75_));
  nor2   g13(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g14(.a(new_n76_), .b(x10), .O(new_n77_));
  inv1   g15(.a(new_n77_), .O(new_n78_));
  oai21  g16(.a(new_n78_), .b(new_n74_), .c(new_n65_), .O(new_n79_));
  aoi21  g17(.a(new_n79_), .b(x16), .c(z16), .O(z00));
  inv1   g18(.a(x28), .O(new_n81_));
  aoi21  g19(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g20(.a(x32), .b(x30), .O(new_n83_));
  oai21  g21(.a(new_n82_), .b(new_n66_), .c(new_n83_), .O(new_n84_));
  nand3  g22(.a(new_n84_), .b(new_n63_), .c(x04), .O(z01));
  nand2  g23(.a(x35), .b(new_n81_), .O(new_n86_));
  nor2   g24(.a(x27), .b(x08), .O(new_n87_));
  oai21  g25(.a(new_n87_), .b(new_n86_), .c(x04), .O(new_n88_));
  nor2   g26(.a(new_n75_), .b(z16), .O(z08));
  inv1   g27(.a(x02), .O(new_n90_));
  nand2  g28(.a(x29), .b(x08), .O(new_n91_));
  nand2  g29(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g30(.a(new_n92_), .b(z08), .c(new_n88_), .O(z02));
  inv1   g31(.a(x37), .O(new_n94_));
  nand2  g32(.a(x28), .b(x27), .O(new_n95_));
  oai22  g33(.a(new_n95_), .b(new_n69_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g34(.a(new_n96_), .b(x21), .c(z16), .O(z03));
  inv1   g35(.a(x21), .O(new_n98_));
  nand3  g36(.a(new_n96_), .b(new_n63_), .c(new_n98_), .O(z04));
  nand3  g37(.a(new_n95_), .b(new_n63_), .c(new_n94_), .O(z05));
  nand2  g38(.a(new_n95_), .b(new_n94_), .O(new_n101_));
  nand2  g39(.a(new_n101_), .b(new_n63_), .O(z06));
  inv1   g40(.a(x14), .O(new_n103_));
  nand2  g41(.a(x17), .b(new_n65_), .O(new_n104_));
  nor2   g42(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g43(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  inv1   g44(.a(x25), .O(new_n107_));
  nand2  g45(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g46(.a(x25), .b(new_n103_), .O(new_n109_));
  nand3  g47(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g48(.a(new_n110_), .b(new_n63_), .c(x03), .O(new_n111_));
  nor2   g49(.a(new_n111_), .b(new_n106_), .O(z07));
  nand4  g50(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n113_));
  nand2  g51(.a(new_n113_), .b(new_n63_), .O(z09));
  nor2   g52(.a(new_n82_), .b(new_n66_), .O(new_n115_));
  nand2  g53(.a(new_n72_), .b(x05), .O(new_n116_));
  inv1   g54(.a(new_n116_), .O(new_n117_));
  and2   g55(.a(new_n75_), .b(x07), .O(new_n118_));
  aoi22  g56(.a(new_n118_), .b(new_n115_), .c(new_n117_), .d(new_n84_), .O(new_n119_));
  nand3  g57(.a(x37), .b(x27), .c(x06), .O(new_n120_));
  and2   g58(.a(new_n120_), .b(new_n63_), .O(new_n121_));
  oai21  g59(.a(new_n119_), .b(x04), .c(new_n121_), .O(z10));
  inv1   g60(.a(x08), .O(new_n123_));
  nand2  g61(.a(new_n72_), .b(x29), .O(new_n124_));
  aoi21  g62(.a(new_n124_), .b(new_n86_), .c(new_n123_), .O(new_n125_));
  nand2  g63(.a(new_n124_), .b(x27), .O(new_n126_));
  nand4  g64(.a(x35), .b(new_n81_), .c(x27), .d(new_n68_), .O(new_n127_));
  nor2   g65(.a(x30), .b(x09), .O(new_n128_));
  nand3  g66(.a(new_n128_), .b(new_n127_), .c(new_n63_), .O(new_n129_));
  aoi21  g67(.a(new_n126_), .b(new_n125_), .c(new_n129_), .O(z11));
  inv1   g68(.a(new_n74_), .O(new_n131_));
  nand3  g69(.a(new_n77_), .b(new_n131_), .c(new_n63_), .O(new_n132_));
  inv1   g70(.a(new_n132_), .O(z12));
  inv1   g71(.a(x13), .O(new_n134_));
  oai21  g72(.a(new_n70_), .b(new_n69_), .c(x28), .O(new_n135_));
  nand4  g73(.a(new_n135_), .b(new_n72_), .c(new_n134_), .d(new_n68_), .O(new_n136_));
  inv1   g74(.a(new_n67_), .O(new_n137_));
  nor2   g75(.a(x19), .b(x18), .O(new_n138_));
  nand3  g76(.a(new_n138_), .b(new_n137_), .c(x20), .O(new_n139_));
  aoi21  g77(.a(new_n139_), .b(new_n136_), .c(new_n66_), .O(new_n140_));
  inv1   g78(.a(new_n140_), .O(new_n141_));
  nand3  g79(.a(new_n72_), .b(new_n134_), .c(new_n68_), .O(new_n142_));
  nor2   g80(.a(new_n142_), .b(new_n83_), .O(new_n143_));
  inv1   g81(.a(new_n143_), .O(new_n144_));
  nand3  g82(.a(new_n144_), .b(new_n141_), .c(new_n63_), .O(z13));
  oai21  g83(.a(new_n143_), .b(new_n140_), .c(new_n63_), .O(z14));
  nand4  g84(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n147_));
  nand2  g85(.a(new_n147_), .b(new_n63_), .O(z15));
  inv1   g86(.a(z11), .O(z18));
  oai21  g87(.a(new_n69_), .b(new_n66_), .c(new_n128_), .O(new_n150_));
  oai21  g88(.a(new_n150_), .b(new_n125_), .c(new_n63_), .O(z20));
  inv1   g89(.a(new_n63_), .O(z17));
  aoi21  g90(.a(new_n126_), .b(new_n125_), .c(new_n129_), .O(z19));
endmodule


