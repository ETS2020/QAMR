// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n153_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n169_,
    new_n171_, new_n172_;
  nor2   g000(.a(x39), .b(x14), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  oai21  g009(.a(x36), .b(x35), .c(x27), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  nand3  g011(.a(x40), .b(x39), .c(new_n73_), .O(new_n74_));
  aoi21  g012(.a(new_n72_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n64_), .O(new_n76_));
  oai21  g014(.a(new_n70_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x35), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(x28), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(x36), .c(x27), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g021(.a(x08), .O(new_n84_));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  aoi21  g024(.a(new_n66_), .b(new_n84_), .c(new_n86_), .O(new_n87_));
  nand2  g025(.a(x40), .b(x39), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g028(.a(new_n87_), .b(new_n73_), .c(new_n90_), .O(z02));
  nand2  g029(.a(new_n68_), .b(x27), .O(new_n92_));
  aoi21  g030(.a(new_n67_), .b(new_n66_), .c(new_n63_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n92_), .c(new_n95_), .O(z04));
  inv1   g034(.a(new_n63_), .O(new_n97_));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n97_), .c(new_n67_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n64_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n101_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n97_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  inv1   g048(.a(new_n88_), .O(z08));
  inv1   g049(.a(x11), .O(new_n112_));
  nand4  g050(.a(new_n97_), .b(x34), .c(x27), .d(x26), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n112_), .O(z09));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  oai21  g053(.a(x39), .b(x14), .c(x07), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(z08), .c(new_n115_), .O(new_n117_));
  inv1   g055(.a(x36), .O(new_n118_));
  aoi21  g056(.a(new_n86_), .b(new_n118_), .c(x04), .O(new_n119_));
  nand2  g057(.a(x37), .b(x06), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n63_), .O(new_n121_));
  aoi21  g059(.a(new_n119_), .b(new_n117_), .c(new_n121_), .O(new_n122_));
  inv1   g060(.a(new_n81_), .O(new_n123_));
  inv1   g061(.a(new_n115_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n123_), .c(new_n73_), .O(new_n125_));
  oai21  g063(.a(new_n122_), .b(new_n66_), .c(new_n125_), .O(z10));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n86_), .c(new_n84_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nand3  g069(.a(new_n79_), .b(x27), .c(new_n73_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n97_), .O(z11));
  nor3   g072(.a(new_n75_), .b(new_n69_), .c(new_n63_), .O(z12));
  inv1   g073(.a(x13), .O(new_n136_));
  inv1   g074(.a(x39), .O(new_n137_));
  nor2   g075(.a(new_n137_), .b(x04), .O(new_n138_));
  nand2  g076(.a(x36), .b(x35), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x28), .O(new_n140_));
  nand4  g078(.a(new_n140_), .b(new_n138_), .c(x40), .d(new_n136_), .O(new_n141_));
  nand2  g079(.a(new_n68_), .b(new_n67_), .O(new_n142_));
  nor2   g080(.a(x19), .b(x18), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n142_), .c(x20), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x27), .O(new_n146_));
  nand3  g084(.a(new_n138_), .b(x40), .c(new_n136_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(new_n81_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n146_), .c(new_n97_), .O(z13));
  aoi21  g088(.a(new_n144_), .b(new_n141_), .c(new_n66_), .O(new_n151_));
  oai21  g089(.a(new_n148_), .b(new_n151_), .c(new_n97_), .O(z14));
  inv1   g090(.a(x12), .O(new_n153_));
  nor2   g091(.a(new_n113_), .b(new_n153_), .O(z15));
  inv1   g092(.a(x23), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(x22), .c(x01), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n97_), .O(z16));
  nor2   g095(.a(x24), .b(new_n155_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(new_n97_), .c(x22), .d(x01), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(z17));
  nand2  g098(.a(new_n66_), .b(new_n84_), .O(new_n161_));
  nand2  g099(.a(x27), .b(x04), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n161_), .c(new_n79_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n131_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n97_), .O(new_n165_));
  inv1   g103(.a(new_n127_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x08), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n165_), .O(z18));
  inv1   g106(.a(new_n167_), .O(new_n169_));
  aoi21  g107(.a(new_n164_), .b(new_n97_), .c(new_n169_), .O(z19));
  nand2  g108(.a(x35), .b(x27), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n131_), .c(new_n97_), .O(new_n172_));
  nor2   g110(.a(new_n172_), .b(new_n128_), .O(z20));
endmodule


