// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_, new_n95_,
    new_n97_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n149_, new_n151_, new_n153_, new_n154_, new_n156_,
    new_n158_, new_n159_;
  inv1   g00(.a(x40), .O(new_n63_));
  nand2  g01(.a(new_n63_), .b(x39), .O(new_n64_));
  inv1   g02(.a(new_n64_), .O(new_n65_));
  inv1   g03(.a(x15), .O(new_n66_));
  inv1   g04(.a(x39), .O(new_n67_));
  inv1   g05(.a(x37), .O(new_n68_));
  nand2  g06(.a(x35), .b(x28), .O(new_n69_));
  nand2  g07(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g08(.a(new_n70_), .b(x27), .O(new_n71_));
  oai21  g09(.a(new_n67_), .b(x04), .c(new_n71_), .O(new_n72_));
  inv1   g10(.a(x10), .O(new_n73_));
  inv1   g11(.a(x27), .O(new_n74_));
  nor3   g12(.a(x37), .b(x36), .c(x35), .O(new_n75_));
  oai21  g13(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(new_n76_));
  nand3  g14(.a(new_n76_), .b(new_n72_), .c(new_n66_), .O(new_n77_));
  aoi21  g15(.a(new_n77_), .b(x16), .c(new_n65_), .O(z00));
  inv1   g16(.a(x28), .O(new_n79_));
  aoi21  g17(.a(x35), .b(new_n79_), .c(x36), .O(new_n80_));
  nor2   g18(.a(x32), .b(x30), .O(new_n81_));
  oai21  g19(.a(new_n80_), .b(new_n74_), .c(new_n81_), .O(new_n82_));
  nand3  g20(.a(new_n82_), .b(new_n64_), .c(x04), .O(z01));
  nand2  g21(.a(x35), .b(new_n79_), .O(new_n84_));
  nor2   g22(.a(x27), .b(x08), .O(new_n85_));
  oai21  g23(.a(new_n85_), .b(new_n84_), .c(x04), .O(new_n86_));
  aoi21  g24(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor3   g25(.a(new_n87_), .b(new_n63_), .c(new_n67_), .O(new_n88_));
  nand2  g26(.a(new_n88_), .b(new_n86_), .O(z02));
  nand2  g27(.a(new_n69_), .b(x27), .O(new_n90_));
  aoi21  g28(.a(new_n68_), .b(new_n74_), .c(new_n65_), .O(new_n91_));
  nand3  g29(.a(new_n91_), .b(new_n90_), .c(x21), .O(z03));
  inv1   g30(.a(x21), .O(new_n93_));
  nand3  g31(.a(new_n91_), .b(new_n90_), .c(new_n93_), .O(z04));
  aoi21  g32(.a(x28), .b(x27), .c(x37), .O(new_n95_));
  nand2  g33(.a(new_n95_), .b(new_n64_), .O(z05));
  inv1   g34(.a(new_n95_), .O(new_n97_));
  nand2  g35(.a(new_n97_), .b(new_n64_), .O(z06));
  inv1   g36(.a(x14), .O(new_n99_));
  nand2  g37(.a(x17), .b(new_n66_), .O(new_n100_));
  nor2   g38(.a(x33), .b(x31), .O(new_n101_));
  aoi21  g39(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  inv1   g40(.a(x25), .O(new_n103_));
  nand2  g41(.a(new_n103_), .b(x00), .O(new_n104_));
  nand2  g42(.a(x25), .b(new_n99_), .O(new_n105_));
  nand3  g43(.a(new_n105_), .b(new_n104_), .c(x38), .O(new_n106_));
  nand3  g44(.a(new_n106_), .b(new_n64_), .c(x03), .O(new_n107_));
  nor2   g45(.a(new_n107_), .b(new_n102_), .O(z07));
  and2   g46(.a(x34), .b(x27), .O(new_n109_));
  nand3  g47(.a(new_n109_), .b(x26), .c(x11), .O(new_n110_));
  nand2  g48(.a(new_n110_), .b(new_n64_), .O(z09));
  nor2   g49(.a(new_n80_), .b(new_n74_), .O(new_n112_));
  and2   g50(.a(x39), .b(x05), .O(new_n113_));
  inv1   g51(.a(x07), .O(new_n114_));
  nor2   g52(.a(x39), .b(new_n114_), .O(new_n115_));
  aoi22  g53(.a(new_n115_), .b(new_n112_), .c(new_n113_), .d(new_n82_), .O(new_n116_));
  nand3  g54(.a(x37), .b(x27), .c(x06), .O(new_n117_));
  and2   g55(.a(new_n117_), .b(new_n64_), .O(new_n118_));
  oai21  g56(.a(new_n116_), .b(x04), .c(new_n118_), .O(z10));
  inv1   g57(.a(x08), .O(new_n120_));
  nand2  g58(.a(x39), .b(x29), .O(new_n121_));
  aoi21  g59(.a(new_n121_), .b(new_n84_), .c(new_n120_), .O(new_n122_));
  nand2  g60(.a(new_n121_), .b(x27), .O(new_n123_));
  nand2  g61(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  inv1   g62(.a(x04), .O(new_n125_));
  nand4  g63(.a(x35), .b(new_n79_), .c(x27), .d(new_n125_), .O(new_n126_));
  nor2   g64(.a(x30), .b(x09), .O(new_n127_));
  and2   g65(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand3  g66(.a(new_n128_), .b(new_n124_), .c(new_n64_), .O(z18));
  inv1   g67(.a(z18), .O(z11));
  nand3  g68(.a(new_n76_), .b(new_n72_), .c(new_n64_), .O(z12));
  nor2   g69(.a(x13), .b(x04), .O(new_n132_));
  nand3  g70(.a(new_n132_), .b(x40), .c(x39), .O(new_n133_));
  aoi21  g71(.a(x36), .b(x35), .c(new_n79_), .O(new_n134_));
  inv1   g72(.a(x18), .O(new_n135_));
  inv1   g73(.a(x19), .O(new_n136_));
  nand3  g74(.a(x20), .b(new_n136_), .c(new_n135_), .O(new_n137_));
  nand2  g75(.a(new_n70_), .b(new_n64_), .O(new_n138_));
  oai22  g76(.a(new_n138_), .b(new_n137_), .c(new_n133_), .d(new_n134_), .O(new_n139_));
  nand2  g77(.a(new_n139_), .b(x27), .O(new_n140_));
  oai21  g78(.a(new_n133_), .b(new_n81_), .c(new_n140_), .O(z13));
  oai21  g79(.a(new_n132_), .b(new_n63_), .c(x39), .O(new_n142_));
  oai21  g80(.a(new_n137_), .b(new_n71_), .c(new_n142_), .O(new_n143_));
  inv1   g81(.a(new_n137_), .O(new_n144_));
  oai21  g82(.a(x37), .b(x35), .c(new_n144_), .O(new_n145_));
  aoi21  g83(.a(new_n145_), .b(new_n134_), .c(new_n74_), .O(new_n146_));
  nand2  g84(.a(new_n81_), .b(x40), .O(new_n147_));
  oai21  g85(.a(new_n147_), .b(new_n146_), .c(new_n143_), .O(z14));
  nand4  g86(.a(new_n109_), .b(new_n64_), .c(x26), .d(x12), .O(new_n149_));
  inv1   g87(.a(new_n149_), .O(z15));
  nand2  g88(.a(x22), .b(x01), .O(new_n151_));
  oai21  g89(.a(new_n151_), .b(x23), .c(new_n64_), .O(z16));
  inv1   g90(.a(x24), .O(new_n153_));
  nand2  g91(.a(new_n153_), .b(x23), .O(new_n154_));
  oai21  g92(.a(new_n154_), .b(new_n151_), .c(new_n64_), .O(z17));
  nand2  g93(.a(new_n128_), .b(new_n124_), .O(new_n156_));
  nand2  g94(.a(new_n156_), .b(new_n64_), .O(z19));
  nand2  g95(.a(x35), .b(x27), .O(new_n158_));
  nand2  g96(.a(new_n158_), .b(new_n127_), .O(new_n159_));
  oai21  g97(.a(new_n159_), .b(new_n122_), .c(new_n64_), .O(z20));
  buf    g98(.a(x39), .O(z08));
endmodule


