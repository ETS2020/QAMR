// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n160_,
    new_n161_, new_n163_, new_n164_, new_n167_;
  nand2  g000(.a(x40), .b(x29), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g015(.a(x29), .b(x15), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n77_), .c(new_n74_), .d(new_n71_), .O(new_n79_));
  oai21  g017(.a(new_n70_), .b(new_n64_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n67_), .c(new_n83_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n63_), .c(x04), .O(z01));
  inv1   g023(.a(x08), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n81_), .O(new_n87_));
  aoi21  g025(.a(new_n67_), .b(new_n86_), .c(new_n87_), .O(new_n88_));
  inv1   g026(.a(new_n88_), .O(new_n89_));
  nand2  g027(.a(x39), .b(x02), .O(new_n90_));
  aoi21  g028(.a(new_n89_), .b(x04), .c(new_n90_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(x29), .c(x40), .O(z02));
  nand2  g030(.a(x35), .b(x28), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n67_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n94_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g035(.a(new_n95_), .b(new_n67_), .c(x21), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n94_), .c(new_n64_), .O(z04));
  nand2  g037(.a(x28), .b(x27), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n95_), .c(new_n64_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n63_), .O(z07));
  inv1   g051(.a(x29), .O(new_n114_));
  aoi21  g052(.a(new_n75_), .b(new_n114_), .c(new_n76_), .O(z08));
  inv1   g053(.a(x11), .O(new_n116_));
  nand4  g054(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(z09));
  nor2   g056(.a(new_n82_), .b(new_n67_), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  nor2   g060(.a(new_n77_), .b(new_n122_), .O(new_n123_));
  aoi22  g061(.a(new_n123_), .b(new_n119_), .c(new_n121_), .d(new_n84_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  and2   g063(.a(new_n125_), .b(new_n63_), .O(new_n126_));
  oai21  g064(.a(new_n124_), .b(x04), .c(new_n126_), .O(z10));
  nand2  g065(.a(x27), .b(x04), .O(new_n128_));
  inv1   g066(.a(x09), .O(new_n129_));
  inv1   g067(.a(x30), .O(new_n130_));
  nand3  g068(.a(new_n63_), .b(new_n130_), .c(new_n129_), .O(new_n131_));
  aoi21  g069(.a(new_n128_), .b(new_n88_), .c(new_n131_), .O(z11));
  nor3   g070(.a(x37), .b(x36), .c(x35), .O(new_n133_));
  oai21  g071(.a(new_n133_), .b(new_n67_), .c(new_n72_), .O(new_n134_));
  inv1   g072(.a(new_n69_), .O(new_n135_));
  nand2  g073(.a(new_n77_), .b(new_n71_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n134_), .c(new_n63_), .O(z12));
  nor2   g076(.a(x19), .b(x18), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x20), .O(new_n140_));
  aoi21  g078(.a(x36), .b(x35), .c(new_n81_), .O(new_n141_));
  inv1   g079(.a(x13), .O(new_n142_));
  nand3  g080(.a(new_n77_), .b(new_n142_), .c(new_n71_), .O(new_n143_));
  oai22  g081(.a(new_n143_), .b(new_n141_), .c(new_n140_), .d(new_n68_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  nand3  g083(.a(x39), .b(new_n142_), .c(new_n71_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n83_), .c(new_n114_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x40), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n145_), .O(z13));
  inv1   g087(.a(new_n146_), .O(new_n150_));
  nand2  g088(.a(new_n141_), .b(new_n83_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n150_), .c(x29), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n76_), .c(new_n140_), .O(new_n153_));
  oai21  g091(.a(x28), .b(new_n67_), .c(new_n83_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n150_), .c(x29), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n76_), .c(new_n135_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n153_), .O(z14));
  inv1   g095(.a(x12), .O(new_n158_));
  nor2   g096(.a(new_n117_), .b(new_n158_), .O(z15));
  inv1   g097(.a(x23), .O(new_n160_));
  nand4  g098(.a(new_n63_), .b(new_n160_), .c(x22), .d(x01), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z16));
  inv1   g100(.a(x24), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(x23), .c(x22), .d(x01), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n63_), .O(z17));
  inv1   g103(.a(z11), .O(z18));
  oai21  g104(.a(x28), .b(new_n86_), .c(new_n67_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(x35), .c(new_n131_), .O(z20));
  aoi21  g106(.a(new_n128_), .b(new_n88_), .c(new_n131_), .O(z19));
endmodule


