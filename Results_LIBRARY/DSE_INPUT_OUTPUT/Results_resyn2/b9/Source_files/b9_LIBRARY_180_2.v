// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n171_, new_n173_, new_n174_,
    new_n176_, new_n178_, new_n179_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  inv1   g009(.a(x07), .O(new_n72_));
  nor2   g010(.a(x32), .b(new_n72_), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  oai21  g012(.a(new_n70_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x36), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n81_), .c(new_n72_), .O(new_n83_));
  oai22  g021(.a(new_n83_), .b(new_n80_), .c(new_n73_), .d(x04), .O(z01));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  inv1   g023(.a(new_n85_), .O(new_n86_));
  nand2  g024(.a(new_n82_), .b(x07), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n79_), .c(x04), .O(new_n89_));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(new_n87_), .d(new_n86_), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand3  g032(.a(x35), .b(x28), .c(x27), .O(new_n95_));
  oai21  g033(.a(new_n94_), .b(x27), .c(new_n95_), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n73_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  aoi21  g036(.a(new_n96_), .b(new_n98_), .c(new_n73_), .O(z04));
  aoi21  g037(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n73_), .O(z05));
  and2   g039(.a(new_n100_), .b(new_n87_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n87_), .O(z07));
  nand2  g051(.a(new_n87_), .b(new_n85_), .O(z08));
  nand3  g052(.a(x34), .b(x27), .c(x26), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n87_), .c(x11), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z09));
  aoi21  g056(.a(new_n79_), .b(new_n77_), .c(x04), .O(new_n119_));
  nand2  g057(.a(new_n72_), .b(x05), .O(new_n120_));
  aoi22  g058(.a(x40), .b(x39), .c(x32), .d(x07), .O(new_n121_));
  aoi21  g059(.a(new_n120_), .b(new_n86_), .c(new_n121_), .O(new_n122_));
  nand2  g060(.a(x37), .b(x06), .O(new_n123_));
  aoi21  g061(.a(new_n82_), .b(x07), .c(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n119_), .c(new_n124_), .O(new_n125_));
  nand2  g063(.a(x30), .b(new_n72_), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n82_), .c(new_n66_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x05), .O(new_n128_));
  oai21  g066(.a(new_n125_), .b(new_n76_), .c(new_n128_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  nand3  g068(.a(x40), .b(x39), .c(x29), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n79_), .c(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n131_), .b(x27), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  inv1   g072(.a(x35), .O(new_n135_));
  nor2   g073(.a(new_n135_), .b(x28), .O(new_n136_));
  nor2   g074(.a(new_n76_), .b(x04), .O(new_n137_));
  inv1   g075(.a(x09), .O(new_n138_));
  nand2  g076(.a(new_n81_), .b(new_n138_), .O(new_n139_));
  aoi21  g077(.a(new_n137_), .b(new_n136_), .c(new_n139_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n134_), .c(new_n87_), .O(z18));
  inv1   g079(.a(z18), .O(z11));
  nand2  g080(.a(new_n67_), .b(x27), .O(new_n143_));
  nand3  g081(.a(new_n87_), .b(new_n68_), .c(new_n143_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(z12));
  nand2  g083(.a(x35), .b(x28), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n94_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand4  g086(.a(new_n148_), .b(new_n87_), .c(new_n147_), .d(x20), .O(new_n149_));
  inv1   g087(.a(x13), .O(new_n150_));
  nor2   g088(.a(new_n85_), .b(x04), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(x07), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n151_), .c(new_n150_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n149_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x27), .O(new_n156_));
  nand2  g094(.a(new_n127_), .b(new_n150_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(z13));
  nand2  g096(.a(new_n148_), .b(x20), .O(new_n159_));
  aoi21  g097(.a(new_n151_), .b(new_n150_), .c(new_n73_), .O(new_n160_));
  nand2  g098(.a(new_n152_), .b(x28), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(new_n83_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n160_), .c(new_n159_), .O(new_n163_));
  nand4  g101(.a(x40), .b(x39), .c(new_n150_), .d(new_n65_), .O(new_n164_));
  aoi21  g102(.a(new_n78_), .b(x27), .c(new_n83_), .O(new_n165_));
  aoi21  g103(.a(new_n147_), .b(x27), .c(new_n73_), .O(new_n166_));
  oai21  g104(.a(new_n165_), .b(new_n164_), .c(new_n166_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n163_), .O(z14));
  nand3  g106(.a(new_n116_), .b(new_n87_), .c(x12), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z15));
  nand2  g108(.a(x22), .b(x01), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(x23), .c(new_n87_), .O(z16));
  inv1   g110(.a(x24), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(x23), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n171_), .c(new_n87_), .O(z17));
  nand2  g113(.a(new_n140_), .b(new_n134_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n87_), .O(z19));
  nand2  g115(.a(x35), .b(x27), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n81_), .c(new_n138_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n132_), .c(new_n87_), .O(z20));
endmodule


