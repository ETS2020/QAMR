// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n97_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n166_,
    new_n167_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n176_, new_n177_, new_n178_, new_n179_, new_n181_, new_n182_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(z08));
  nand4  g015(.a(z08), .b(new_n74_), .c(new_n64_), .d(new_n71_), .O(new_n78_));
  oai21  g016(.a(new_n70_), .b(new_n63_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x35), .O(new_n80_));
  inv1   g018(.a(x36), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(x28), .c(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g024(.a(x28), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  nand2  g026(.a(new_n66_), .b(new_n88_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(x35), .c(new_n87_), .O(new_n90_));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  aoi21  g029(.a(new_n90_), .b(x04), .c(new_n91_), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n75_), .c(z08), .d(new_n63_), .O(z02));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n80_), .c(new_n67_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n63_), .O(z03));
  nor2   g034(.a(new_n63_), .b(x21), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n95_), .O(z04));
  aoi21  g036(.a(new_n94_), .b(new_n67_), .c(new_n63_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n64_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  inv1   g042(.a(new_n63_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n105_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n104_), .O(z07));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n105_), .O(z09));
  inv1   g051(.a(new_n84_), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(z08), .c(x05), .O(new_n115_));
  inv1   g053(.a(x05), .O(new_n116_));
  nand3  g054(.a(x40), .b(x39), .c(new_n116_), .O(new_n117_));
  inv1   g055(.a(x07), .O(new_n118_));
  oai21  g056(.a(new_n76_), .b(new_n75_), .c(new_n118_), .O(new_n119_));
  nand4  g057(.a(new_n119_), .b(new_n117_), .c(new_n82_), .d(x27), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n115_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n71_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n63_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(z10));
  nor2   g064(.a(x28), .b(new_n66_), .O(new_n127_));
  nor2   g065(.a(new_n80_), .b(x04), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n127_), .c(x08), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  oai21  g068(.a(new_n80_), .b(x28), .c(new_n130_), .O(new_n131_));
  nor2   g069(.a(new_n66_), .b(new_n71_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  oai21  g073(.a(new_n134_), .b(new_n129_), .c(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n105_), .O(z11));
  nand3  g075(.a(new_n67_), .b(new_n81_), .c(new_n80_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(x27), .c(x10), .O(new_n139_));
  aoi21  g077(.a(x40), .b(new_n71_), .c(new_n69_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n139_), .c(x39), .O(new_n141_));
  inv1   g079(.a(new_n69_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(x40), .c(new_n75_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(z12));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x28), .O(new_n146_));
  nor2   g084(.a(x13), .b(x04), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n146_), .c(z08), .O(new_n148_));
  nand2  g086(.a(new_n68_), .b(new_n67_), .O(new_n149_));
  nor2   g087(.a(x19), .b(x18), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n149_), .c(new_n105_), .d(x20), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n148_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand3  g091(.a(new_n147_), .b(new_n114_), .c(z08), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  nand2  g093(.a(new_n147_), .b(z08), .O(new_n156_));
  nand2  g094(.a(new_n150_), .b(x20), .O(new_n157_));
  nand3  g095(.a(new_n145_), .b(new_n84_), .c(x28), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n156_), .c(new_n157_), .O(new_n160_));
  nor2   g098(.a(new_n127_), .b(new_n114_), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n156_), .c(new_n142_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n160_), .c(new_n105_), .O(z14));
  nand4  g101(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n105_), .O(z15));
  inv1   g103(.a(x23), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(x22), .c(x01), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n105_), .O(z16));
  nor2   g106(.a(x24), .b(new_n166_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(new_n105_), .c(x22), .d(x01), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z17));
  oai21  g109(.a(new_n132_), .b(new_n90_), .c(new_n135_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n105_), .O(new_n173_));
  nand3  g111(.a(z08), .b(x29), .c(x08), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n173_), .O(z18));
  inv1   g113(.a(new_n135_), .O(new_n176_));
  nand2  g114(.a(new_n134_), .b(new_n105_), .O(new_n177_));
  oai21  g115(.a(new_n76_), .b(x27), .c(new_n75_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n129_), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n177_), .c(new_n176_), .O(z19));
  nand2  g118(.a(x35), .b(x27), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n135_), .c(new_n105_), .O(new_n182_));
  aoi21  g120(.a(new_n131_), .b(x08), .c(new_n182_), .O(z20));
endmodule


