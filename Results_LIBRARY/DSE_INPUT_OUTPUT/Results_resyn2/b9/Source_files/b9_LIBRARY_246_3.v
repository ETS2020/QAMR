// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n154_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n163_;
  inv1   g000(.a(x31), .O(new_n63_));
  inv1   g001(.a(x33), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x35), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  and2   g013(.a(x40), .b(x39), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n75_), .c(new_n72_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n71_), .c(new_n67_), .O(new_n78_));
  nand2  g016(.a(x40), .b(x39), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(x04), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x10), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n78_), .c(new_n66_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x16), .c(new_n65_), .O(z00));
  inv1   g022(.a(x28), .O(new_n85_));
  aoi21  g023(.a(x35), .b(new_n85_), .c(x36), .O(new_n86_));
  nor2   g024(.a(x32), .b(x30), .O(new_n87_));
  oai21  g025(.a(new_n86_), .b(new_n67_), .c(new_n87_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(x04), .c(new_n65_), .O(z01));
  nand2  g027(.a(x35), .b(new_n85_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n90_), .c(x04), .O(new_n92_));
  nor2   g030(.a(new_n79_), .b(new_n65_), .O(z08));
  inv1   g031(.a(x02), .O(new_n94_));
  nand2  g032(.a(x29), .b(x08), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(z08), .c(new_n92_), .O(z02));
  inv1   g035(.a(new_n65_), .O(new_n98_));
  nand2  g036(.a(new_n69_), .b(x27), .O(new_n99_));
  nand2  g037(.a(new_n68_), .b(new_n67_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(x21), .O(z03));
  aoi21  g039(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n99_), .c(new_n65_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n98_), .c(new_n68_), .O(z05));
  inv1   g043(.a(z05), .O(z06));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n109_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(x38), .O(new_n111_));
  inv1   g049(.a(x17), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(x15), .O(new_n113_));
  nand2  g051(.a(new_n64_), .b(new_n63_), .O(new_n114_));
  oai21  g052(.a(new_n114_), .b(new_n113_), .c(x14), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n111_), .c(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n98_), .O(z07));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n98_), .O(z09));
  nor2   g057(.a(new_n86_), .b(new_n67_), .O(new_n120_));
  nand2  g058(.a(new_n76_), .b(x05), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  and2   g060(.a(new_n79_), .b(x07), .O(new_n123_));
  aoi22  g061(.a(new_n123_), .b(new_n120_), .c(new_n122_), .d(new_n88_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n65_), .O(new_n127_));
  oai21  g065(.a(new_n124_), .b(x04), .c(new_n127_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(new_n76_), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n90_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  nand4  g072(.a(x35), .b(new_n85_), .c(x27), .d(new_n72_), .O(new_n135_));
  and2   g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n98_), .O(z11));
  inv1   g076(.a(new_n78_), .O(new_n139_));
  nand3  g077(.a(new_n81_), .b(new_n139_), .c(new_n98_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z12));
  inv1   g079(.a(x13), .O(new_n142_));
  oai21  g080(.a(new_n74_), .b(new_n73_), .c(x28), .O(new_n143_));
  nand4  g081(.a(new_n143_), .b(new_n76_), .c(new_n142_), .d(new_n72_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n70_), .c(x20), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n144_), .c(new_n67_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  nand3  g086(.a(new_n76_), .b(new_n142_), .c(new_n72_), .O(new_n149_));
  nor2   g087(.a(new_n149_), .b(new_n87_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n148_), .c(new_n98_), .O(z13));
  oai21  g090(.a(new_n150_), .b(new_n147_), .c(new_n98_), .O(z14));
  nand4  g091(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n98_), .O(z15));
  inv1   g093(.a(x23), .O(new_n156_));
  nand4  g094(.a(new_n98_), .b(new_n156_), .c(x22), .d(x01), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z16));
  inv1   g096(.a(x24), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(x23), .c(x22), .d(x01), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n98_), .O(z17));
  aoi21  g099(.a(new_n136_), .b(new_n133_), .c(new_n65_), .O(z18));
  oai21  g100(.a(new_n73_), .b(new_n67_), .c(new_n134_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n131_), .c(new_n98_), .O(z20));
  nand2  g102(.a(new_n137_), .b(new_n98_), .O(z19));
endmodule


