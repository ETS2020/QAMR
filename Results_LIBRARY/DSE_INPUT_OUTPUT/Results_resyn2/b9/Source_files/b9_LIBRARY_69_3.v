// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n168_, new_n170_, new_n172_, new_n174_, new_n176_;
  nand2  g000(.a(x40), .b(x34), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  oai21  g009(.a(x36), .b(x35), .c(x27), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor3   g011(.a(x34), .b(x15), .c(x04), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(new_n73_), .c(x40), .d(x39), .O(new_n75_));
  oai21  g013(.a(new_n70_), .b(new_n64_), .c(new_n75_), .O(z00));
  inv1   g014(.a(x35), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x28), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(x36), .c(x27), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g020(.a(x08), .O(new_n83_));
  nand2  g021(.a(new_n67_), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n78_), .O(new_n85_));
  and2   g023(.a(x29), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x02), .c(x39), .O(new_n87_));
  aoi21  g025(.a(new_n85_), .b(x04), .c(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(x34), .c(x40), .O(z02));
  nand2  g027(.a(x35), .b(x28), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n67_), .c(new_n64_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n91_), .c(new_n95_), .O(z04));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n63_), .c(new_n92_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n63_), .O(z07));
  inv1   g048(.a(x34), .O(new_n111_));
  inv1   g049(.a(x39), .O(new_n112_));
  inv1   g050(.a(x40), .O(new_n113_));
  aoi21  g051(.a(new_n112_), .b(new_n111_), .c(new_n113_), .O(z08));
  nand3  g052(.a(x27), .b(x26), .c(x11), .O(new_n115_));
  aoi21  g053(.a(new_n115_), .b(new_n113_), .c(new_n111_), .O(z09));
  inv1   g054(.a(x36), .O(new_n117_));
  inv1   g055(.a(x28), .O(new_n118_));
  nand2  g056(.a(x35), .b(new_n118_), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(new_n117_), .c(new_n67_), .O(new_n120_));
  oai21  g058(.a(new_n113_), .b(new_n112_), .c(x07), .O(new_n121_));
  nand3  g059(.a(x40), .b(x39), .c(x05), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nor2   g061(.a(new_n122_), .b(new_n80_), .O(new_n124_));
  aoi21  g062(.a(new_n123_), .b(new_n120_), .c(new_n124_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  and2   g064(.a(new_n126_), .b(new_n63_), .O(new_n127_));
  oai21  g065(.a(new_n125_), .b(x04), .c(new_n127_), .O(z10));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n119_), .c(new_n83_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nor2   g070(.a(x30), .b(x09), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(new_n134_));
  nor2   g072(.a(new_n67_), .b(x04), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n78_), .c(new_n134_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n132_), .c(new_n63_), .O(z18));
  inv1   g075(.a(z18), .O(z11));
  nand3  g076(.a(new_n92_), .b(new_n117_), .c(new_n77_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(x27), .c(x10), .O(new_n140_));
  inv1   g078(.a(x04), .O(new_n141_));
  nand2  g079(.a(new_n90_), .b(new_n92_), .O(new_n142_));
  aoi22  g080(.a(new_n142_), .b(x27), .c(x39), .d(new_n141_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n140_), .c(new_n111_), .O(new_n144_));
  inv1   g082(.a(new_n69_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n113_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n144_), .O(z12));
  aoi21  g085(.a(x36), .b(x35), .c(new_n118_), .O(new_n148_));
  inv1   g086(.a(x13), .O(new_n149_));
  nand4  g087(.a(x40), .b(x39), .c(new_n149_), .d(new_n141_), .O(new_n150_));
  nor2   g088(.a(x19), .b(x18), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x20), .O(new_n152_));
  oai22  g090(.a(new_n152_), .b(new_n68_), .c(new_n150_), .d(new_n148_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  nand3  g092(.a(x39), .b(new_n149_), .c(new_n141_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n80_), .c(new_n111_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x40), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n154_), .O(z13));
  inv1   g096(.a(new_n150_), .O(new_n159_));
  oai21  g097(.a(x28), .b(new_n67_), .c(new_n80_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n145_), .O(new_n162_));
  nand2  g100(.a(x36), .b(x35), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n80_), .c(x28), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n150_), .c(new_n152_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n162_), .c(new_n63_), .O(z14));
  nand3  g105(.a(x27), .b(x26), .c(x12), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n113_), .c(new_n111_), .O(z15));
  nand2  g107(.a(x22), .b(x01), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(x23), .c(new_n63_), .O(z16));
  inv1   g109(.a(x23), .O(new_n172_));
  nor4   g110(.a(new_n170_), .b(new_n64_), .c(x24), .d(new_n172_), .O(z17));
  nand2  g111(.a(new_n136_), .b(new_n132_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n63_), .O(z19));
  oai21  g113(.a(new_n77_), .b(new_n67_), .c(new_n133_), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n130_), .c(new_n63_), .O(z20));
endmodule


