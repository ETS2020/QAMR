// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n104_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n155_, new_n157_, new_n159_, new_n160_, new_n161_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n167_, new_n169_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x39), .O(new_n65_));
  inv1   g003(.a(x40), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n67_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x35), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  and2   g013(.a(x40), .b(x39), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n75_), .c(new_n72_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n71_), .c(new_n64_), .O(new_n78_));
  nand2  g016(.a(x40), .b(x39), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(x04), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x10), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n78_), .c(new_n63_), .O(new_n83_));
  inv1   g021(.a(x16), .O(new_n84_));
  nand2  g022(.a(new_n67_), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n83_), .O(z00));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n74_), .c(new_n64_), .O(new_n89_));
  nor2   g027(.a(x32), .b(x30), .O(new_n90_));
  inv1   g028(.a(new_n90_), .O(new_n91_));
  nor2   g029(.a(x40), .b(x39), .O(new_n92_));
  nor2   g030(.a(new_n92_), .b(new_n72_), .O(new_n93_));
  oai21  g031(.a(new_n91_), .b(new_n89_), .c(new_n93_), .O(z01));
  xnor2a g032(.a(x40), .b(x39), .O(z08));
  aoi21  g033(.a(x29), .b(x08), .c(x02), .O(new_n96_));
  nor2   g034(.a(new_n73_), .b(x28), .O(new_n97_));
  oai21  g035(.a(x27), .b(x08), .c(new_n97_), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(x04), .c(new_n96_), .O(new_n99_));
  oai21  g037(.a(new_n99_), .b(new_n65_), .c(z08), .O(z02));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  oai22  g039(.a(new_n101_), .b(new_n73_), .c(new_n68_), .d(x27), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(x21), .c(new_n92_), .O(z03));
  nor2   g041(.a(new_n92_), .b(x21), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n102_), .O(z04));
  nand3  g043(.a(new_n101_), .b(new_n67_), .c(new_n68_), .O(z05));
  inv1   g044(.a(z05), .O(z06));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  inv1   g047(.a(x14), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n110_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(x38), .O(new_n112_));
  inv1   g050(.a(x17), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(x15), .O(new_n114_));
  or2    g052(.a(x33), .b(x31), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n114_), .c(x14), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n112_), .c(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n67_), .O(z07));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n67_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  inv1   g059(.a(x07), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(x05), .O(new_n123_));
  oai21  g061(.a(z08), .b(new_n122_), .c(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n88_), .b(new_n74_), .c(x04), .O(new_n125_));
  nand2  g063(.a(x37), .b(x06), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n92_), .O(new_n127_));
  aoi21  g065(.a(new_n125_), .b(new_n124_), .c(new_n127_), .O(new_n128_));
  nand3  g066(.a(new_n91_), .b(new_n80_), .c(x05), .O(new_n129_));
  oai21  g067(.a(new_n128_), .b(new_n64_), .c(new_n129_), .O(z10));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n88_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nor2   g073(.a(x30), .b(x09), .O(new_n136_));
  nand3  g074(.a(new_n97_), .b(x27), .c(new_n72_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n136_), .c(new_n135_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n67_), .O(z11));
  inv1   g077(.a(new_n70_), .O(new_n140_));
  aoi21  g078(.a(new_n77_), .b(new_n140_), .c(new_n64_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n82_), .c(new_n67_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  oai21  g081(.a(new_n74_), .b(new_n73_), .c(x28), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n80_), .c(new_n143_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x20), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n71_), .c(new_n145_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  nand3  g087(.a(new_n91_), .b(new_n80_), .c(new_n143_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n149_), .O(z13));
  oai21  g089(.a(new_n147_), .b(new_n140_), .c(new_n145_), .O(new_n152_));
  nand2  g090(.a(new_n150_), .b(new_n67_), .O(new_n153_));
  aoi21  g091(.a(new_n152_), .b(x27), .c(new_n153_), .O(z14));
  inv1   g092(.a(x12), .O(new_n155_));
  nor2   g093(.a(new_n120_), .b(new_n155_), .O(z15));
  nand2  g094(.a(x22), .b(x01), .O(new_n157_));
  nor3   g095(.a(new_n157_), .b(new_n92_), .c(x23), .O(z16));
  inv1   g096(.a(x24), .O(new_n159_));
  inv1   g097(.a(new_n157_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(new_n67_), .c(new_n159_), .d(x23), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z17));
  aoi21  g100(.a(new_n64_), .b(new_n131_), .c(new_n88_), .O(new_n163_));
  inv1   g101(.a(new_n136_), .O(new_n164_));
  nand2  g102(.a(x27), .b(x04), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n163_), .c(new_n164_), .O(new_n166_));
  nand3  g104(.a(new_n76_), .b(x29), .c(x08), .O(new_n167_));
  oai21  g105(.a(new_n166_), .b(new_n92_), .c(new_n167_), .O(z18));
  oai21  g106(.a(new_n73_), .b(new_n64_), .c(new_n136_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n133_), .c(new_n67_), .O(z20));
  nand2  g108(.a(new_n138_), .b(new_n67_), .O(z19));
endmodule


