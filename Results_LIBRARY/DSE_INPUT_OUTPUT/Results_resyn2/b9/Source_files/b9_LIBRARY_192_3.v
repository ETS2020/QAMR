// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n176_, new_n177_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n186_, new_n187_, new_n188_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  inv1   g009(.a(x22), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  oai21  g013(.a(new_n70_), .b(x15), .c(new_n75_), .O(z00));
  inv1   g014(.a(x35), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x28), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(x36), .c(x27), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n74_), .O(z01));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  inv1   g021(.a(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n78_), .O(new_n85_));
  nand2  g023(.a(x29), .b(x08), .O(new_n86_));
  inv1   g024(.a(new_n86_), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(x02), .c(x39), .O(new_n88_));
  aoi21  g026(.a(new_n85_), .b(x04), .c(new_n88_), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(x22), .c(x40), .O(z02));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n77_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n74_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  aoi21  g033(.a(new_n93_), .b(new_n95_), .c(new_n74_), .O(z04));
  inv1   g034(.a(new_n74_), .O(new_n97_));
  nand2  g035(.a(new_n92_), .b(new_n91_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n97_), .O(z06));
  inv1   g037(.a(z06), .O(z05));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n97_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  oai21  g049(.a(x39), .b(x22), .c(x40), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z08));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n97_), .O(z09));
  inv1   g053(.a(x27), .O(new_n116_));
  nand2  g054(.a(new_n112_), .b(x07), .O(new_n117_));
  nand4  g055(.a(x40), .b(x39), .c(new_n72_), .d(x05), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  inv1   g057(.a(x36), .O(new_n120_));
  inv1   g058(.a(x28), .O(new_n121_));
  nand2  g059(.a(x35), .b(new_n121_), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n120_), .c(x04), .O(new_n123_));
  nand2  g061(.a(x37), .b(x06), .O(new_n124_));
  aoi21  g062(.a(x40), .b(x22), .c(new_n124_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(new_n119_), .c(new_n125_), .O(new_n126_));
  inv1   g064(.a(new_n80_), .O(new_n127_));
  inv1   g065(.a(new_n118_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n64_), .O(new_n129_));
  oai21  g067(.a(new_n126_), .b(new_n116_), .c(new_n129_), .O(z10));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  inv1   g070(.a(x08), .O(new_n133_));
  nand3  g071(.a(new_n131_), .b(new_n133_), .c(x04), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n72_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x40), .O(new_n136_));
  nand3  g074(.a(x40), .b(x39), .c(x29), .O(new_n137_));
  nand2  g075(.a(x27), .b(x04), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(x35), .c(new_n121_), .O(new_n139_));
  nand3  g077(.a(x35), .b(new_n121_), .c(x27), .O(new_n140_));
  aoi22  g078(.a(new_n140_), .b(new_n133_), .c(new_n139_), .d(new_n137_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n132_), .c(new_n136_), .O(z11));
  nand2  g080(.a(new_n63_), .b(new_n91_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(x10), .O(new_n144_));
  inv1   g082(.a(x39), .O(new_n145_));
  nor2   g083(.a(new_n145_), .b(x04), .O(new_n146_));
  inv1   g084(.a(new_n66_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(new_n146_), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n144_), .c(new_n72_), .O(new_n149_));
  nand2  g087(.a(new_n147_), .b(x27), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n73_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(z12));
  nor2   g090(.a(x19), .b(x18), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n147_), .c(x20), .O(new_n154_));
  nor2   g092(.a(new_n73_), .b(new_n145_), .O(new_n155_));
  nand2  g093(.a(x36), .b(x35), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x28), .O(new_n157_));
  nor2   g095(.a(x13), .b(x04), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(new_n157_), .c(new_n155_), .d(new_n72_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n154_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n97_), .c(x27), .O(new_n161_));
  nand3  g099(.a(x40), .b(x39), .c(new_n72_), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  nand3  g101(.a(new_n158_), .b(new_n163_), .c(new_n127_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(z13));
  nand2  g103(.a(new_n153_), .b(x20), .O(new_n166_));
  nand2  g104(.a(new_n158_), .b(new_n155_), .O(new_n167_));
  nand3  g105(.a(new_n156_), .b(new_n80_), .c(x28), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n167_), .c(new_n166_), .O(new_n170_));
  aoi21  g108(.a(new_n121_), .b(x27), .c(new_n127_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n167_), .c(new_n150_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n170_), .c(new_n97_), .O(z14));
  nand4  g111(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n97_), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x01), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n73_), .c(new_n72_), .O(z16));
  nand2  g116(.a(x22), .b(x01), .O(new_n179_));
  nor4   g117(.a(new_n179_), .b(x40), .c(x24), .d(new_n176_), .O(z17));
  inv1   g118(.a(new_n137_), .O(new_n181_));
  oai21  g119(.a(new_n139_), .b(new_n83_), .c(new_n131_), .O(new_n182_));
  aoi21  g120(.a(new_n181_), .b(x08), .c(new_n182_), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n74_), .O(z18));
  aoi22  g122(.a(new_n182_), .b(new_n97_), .c(new_n163_), .d(new_n87_), .O(z19));
  aoi21  g123(.a(new_n137_), .b(new_n122_), .c(new_n133_), .O(new_n186_));
  nand2  g124(.a(x35), .b(x27), .O(new_n187_));
  nand3  g125(.a(new_n187_), .b(new_n131_), .c(new_n97_), .O(new_n188_));
  nor2   g126(.a(new_n188_), .b(new_n186_), .O(z20));
endmodule


