// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n156_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n175_, new_n176_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  nor2   g008(.a(x14), .b(x08), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x27), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  nor2   g018(.a(new_n71_), .b(new_n63_), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(new_n78_), .c(new_n81_), .O(z01));
  inv1   g020(.a(x08), .O(new_n83_));
  aoi21  g021(.a(new_n74_), .b(new_n83_), .c(new_n77_), .O(new_n84_));
  or2    g022(.a(new_n84_), .b(new_n63_), .O(new_n85_));
  nand2  g023(.a(x40), .b(x39), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n85_), .c(new_n71_), .O(z02));
  nand2  g027(.a(x35), .b(x28), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(x27), .c(new_n71_), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n74_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n91_), .c(new_n95_), .O(z04));
  inv1   g034(.a(new_n71_), .O(new_n97_));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n97_), .c(new_n92_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n97_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  nor2   g049(.a(new_n71_), .b(new_n86_), .O(z08));
  nand4  g050(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n97_), .O(z09));
  nand2  g052(.a(new_n77_), .b(new_n75_), .O(new_n115_));
  nand3  g053(.a(x40), .b(x39), .c(x05), .O(new_n116_));
  nand2  g054(.a(new_n86_), .b(x07), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n115_), .c(x27), .O(new_n119_));
  inv1   g057(.a(new_n116_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n80_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n63_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n71_), .O(z10));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  nand2  g064(.a(x27), .b(x04), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(x35), .c(new_n76_), .O(new_n128_));
  nor2   g066(.a(x28), .b(x04), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x35), .c(x27), .O(new_n130_));
  aoi22  g068(.a(new_n130_), .b(new_n83_), .c(new_n128_), .d(new_n126_), .O(new_n131_));
  or2    g069(.a(x30), .b(x09), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n131_), .c(new_n97_), .O(z11));
  nand2  g071(.a(new_n97_), .b(new_n69_), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(z12));
  aoi21  g073(.a(x36), .b(x35), .c(new_n76_), .O(new_n136_));
  inv1   g074(.a(x13), .O(new_n137_));
  inv1   g075(.a(new_n86_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(new_n63_), .O(new_n139_));
  nor2   g077(.a(x19), .b(x18), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x20), .O(new_n141_));
  oai22  g079(.a(new_n141_), .b(new_n66_), .c(new_n139_), .d(new_n136_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x27), .O(new_n143_));
  nor2   g081(.a(new_n139_), .b(new_n79_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n143_), .c(new_n71_), .O(z13));
  nand4  g084(.a(new_n138_), .b(new_n76_), .c(new_n137_), .d(new_n63_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n66_), .c(new_n74_), .O(new_n148_));
  nand2  g086(.a(x36), .b(x35), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n79_), .c(x28), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n138_), .c(new_n137_), .d(new_n63_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n141_), .c(new_n71_), .O(new_n152_));
  oai21  g090(.a(new_n148_), .b(new_n144_), .c(new_n152_), .O(z14));
  nand4  g091(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n97_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(x23), .c(new_n97_), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  inv1   g096(.a(new_n156_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n97_), .c(new_n158_), .d(x23), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z17));
  nand2  g099(.a(new_n127_), .b(new_n84_), .O(new_n162_));
  nand3  g100(.a(new_n138_), .b(x29), .c(x08), .O(new_n163_));
  nor2   g101(.a(new_n132_), .b(new_n71_), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n163_), .c(new_n162_), .O(z18));
  nand2  g103(.a(x35), .b(x27), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x14), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n129_), .O(new_n168_));
  inv1   g106(.a(x35), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x08), .O(new_n170_));
  aoi21  g108(.a(x27), .b(x04), .c(x28), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  aoi22  g110(.a(new_n172_), .b(new_n126_), .c(new_n168_), .d(new_n83_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n132_), .c(new_n97_), .O(z19));
  aoi21  g112(.a(new_n126_), .b(new_n77_), .c(new_n83_), .O(new_n175_));
  nand2  g113(.a(new_n164_), .b(new_n166_), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(new_n175_), .O(z20));
endmodule


