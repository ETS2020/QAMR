// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n174_, new_n175_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n64_), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  nor2   g009(.a(x36), .b(x35), .O(new_n72_));
  oai21  g010(.a(new_n72_), .b(new_n66_), .c(new_n71_), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(z08));
  nand4  g013(.a(z08), .b(new_n73_), .c(new_n64_), .d(new_n70_), .O(new_n76_));
  oai21  g014(.a(new_n69_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x36), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n78_), .c(new_n66_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g022(.a(new_n63_), .b(new_n70_), .O(new_n85_));
  oai21  g023(.a(new_n84_), .b(new_n81_), .c(new_n85_), .O(z01));
  inv1   g024(.a(x40), .O(new_n87_));
  xnor2a g025(.a(x40), .b(x39), .O(new_n88_));
  inv1   g026(.a(x35), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(x28), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  nand2  g029(.a(new_n66_), .b(new_n91_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  aoi21  g031(.a(x29), .b(x08), .c(x02), .O(new_n94_));
  aoi21  g032(.a(new_n93_), .b(x04), .c(new_n94_), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n87_), .c(new_n88_), .O(z02));
  inv1   g034(.a(x37), .O(new_n97_));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  oai22  g036(.a(new_n98_), .b(new_n89_), .c(new_n97_), .d(x27), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(x21), .c(new_n63_), .O(z03));
  nor2   g038(.a(new_n63_), .b(x21), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n99_), .O(z04));
  aoi21  g040(.a(new_n98_), .b(new_n97_), .c(new_n63_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n64_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  inv1   g046(.a(new_n63_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n105_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n109_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n108_), .O(z07));
  inv1   g053(.a(x11), .O(new_n116_));
  nand3  g054(.a(x34), .b(x27), .c(x26), .O(new_n117_));
  nor3   g055(.a(new_n117_), .b(new_n63_), .c(new_n116_), .O(z09));
  inv1   g056(.a(x07), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  oai21  g058(.a(new_n88_), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n80_), .b(new_n78_), .c(x04), .O(new_n122_));
  nand2  g060(.a(x37), .b(x06), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n63_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n121_), .c(new_n124_), .O(new_n125_));
  inv1   g063(.a(new_n120_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n84_), .c(new_n70_), .O(new_n127_));
  oai21  g065(.a(new_n125_), .b(new_n66_), .c(new_n127_), .O(z10));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n80_), .c(new_n91_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nor2   g070(.a(new_n66_), .b(x04), .O(new_n133_));
  inv1   g071(.a(x09), .O(new_n134_));
  nand2  g072(.a(new_n82_), .b(new_n134_), .O(new_n135_));
  aoi21  g073(.a(new_n133_), .b(new_n90_), .c(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n109_), .O(z11));
  nor2   g076(.a(new_n74_), .b(x04), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n72_), .c(new_n67_), .O(new_n141_));
  oai21  g079(.a(new_n140_), .b(new_n71_), .c(new_n109_), .O(new_n142_));
  aoi21  g080(.a(new_n141_), .b(x27), .c(new_n142_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x28), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n139_), .c(new_n144_), .O(new_n147_));
  nand2  g085(.a(x35), .b(x28), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n97_), .O(new_n149_));
  nor2   g087(.a(x19), .b(x18), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n149_), .c(new_n109_), .d(x20), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand3  g091(.a(new_n139_), .b(new_n84_), .c(new_n144_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  nand2  g093(.a(new_n139_), .b(new_n144_), .O(new_n156_));
  nand2  g094(.a(new_n150_), .b(x20), .O(new_n157_));
  nand4  g095(.a(new_n145_), .b(new_n83_), .c(new_n82_), .d(x28), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n156_), .c(new_n157_), .O(new_n160_));
  aoi21  g098(.a(new_n79_), .b(x27), .c(new_n84_), .O(new_n161_));
  oai22  g099(.a(new_n161_), .b(new_n156_), .c(new_n67_), .d(new_n66_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n160_), .c(new_n109_), .O(z14));
  inv1   g101(.a(x12), .O(new_n164_));
  oai21  g102(.a(new_n117_), .b(new_n164_), .c(new_n109_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  nor3   g104(.a(new_n166_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g105(.a(x23), .O(new_n168_));
  nor2   g106(.a(x24), .b(new_n168_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(new_n109_), .c(x22), .d(x01), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z17));
  nand3  g109(.a(new_n136_), .b(new_n132_), .c(new_n109_), .O(z18));
  inv1   g110(.a(z18), .O(z19));
  nor2   g111(.a(new_n135_), .b(new_n63_), .O(new_n174_));
  oai21  g112(.a(new_n89_), .b(new_n66_), .c(new_n174_), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n130_), .O(z20));
endmodule


