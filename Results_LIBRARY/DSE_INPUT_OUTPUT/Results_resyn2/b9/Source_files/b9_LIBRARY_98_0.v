// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n165_, new_n167_,
    new_n169_, new_n170_, new_n172_, new_n174_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x40), .b(x39), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g020(.a(new_n72_), .b(new_n65_), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n79_), .c(new_n83_), .O(z01));
  inv1   g022(.a(x39), .O(new_n85_));
  xnor2a g023(.a(x40), .b(x39), .O(z08));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  nand2  g026(.a(new_n75_), .b(new_n88_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(x35), .c(new_n77_), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(x04), .c(new_n87_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n85_), .c(z08), .O(z02));
  inv1   g030(.a(new_n72_), .O(new_n93_));
  inv1   g031(.a(x35), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n77_), .c(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n75_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n95_), .c(new_n93_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n96_), .b(new_n75_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n72_), .O(z04));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n93_), .c(new_n96_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  inv1   g042(.a(x15), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n105_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n104_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n93_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nand2  g052(.a(x34), .b(x27), .O(new_n115_));
  nand2  g053(.a(x26), .b(x11), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(new_n115_), .c(new_n93_), .O(z09));
  inv1   g055(.a(x07), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  oai21  g057(.a(z08), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  aoi21  g058(.a(new_n78_), .b(new_n76_), .c(x04), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n72_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n120_), .c(new_n123_), .O(new_n124_));
  inv1   g062(.a(new_n119_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n82_), .c(new_n65_), .O(new_n126_));
  oai21  g064(.a(new_n124_), .b(new_n75_), .c(new_n126_), .O(z10));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n78_), .c(new_n88_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nor2   g069(.a(new_n94_), .b(new_n75_), .O(new_n132_));
  nor2   g070(.a(x28), .b(x04), .O(new_n133_));
  inv1   g071(.a(x09), .O(new_n134_));
  nand2  g072(.a(new_n80_), .b(new_n134_), .O(new_n135_));
  aoi21  g073(.a(new_n133_), .b(new_n132_), .c(new_n135_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n131_), .c(new_n93_), .O(z18));
  inv1   g075(.a(z18), .O(z11));
  nand2  g076(.a(new_n67_), .b(x27), .O(new_n139_));
  nand3  g077(.a(new_n93_), .b(new_n68_), .c(new_n139_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z12));
  inv1   g079(.a(x18), .O(new_n142_));
  inv1   g080(.a(x19), .O(new_n143_));
  nand3  g081(.a(x20), .b(new_n143_), .c(new_n142_), .O(new_n144_));
  nor2   g082(.a(new_n144_), .b(new_n63_), .O(new_n145_));
  nand2  g083(.a(x36), .b(x35), .O(new_n146_));
  inv1   g084(.a(x13), .O(new_n147_));
  nand4  g085(.a(x40), .b(x39), .c(new_n147_), .d(new_n65_), .O(new_n148_));
  aoi21  g086(.a(new_n146_), .b(x28), .c(new_n148_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n145_), .c(x27), .O(new_n150_));
  inv1   g088(.a(x40), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(new_n85_), .O(new_n152_));
  nor2   g090(.a(x13), .b(x04), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n82_), .c(new_n152_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n150_), .c(new_n93_), .O(z13));
  nand2  g093(.a(new_n154_), .b(new_n93_), .O(new_n156_));
  inv1   g094(.a(new_n63_), .O(new_n157_));
  inv1   g095(.a(new_n144_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  inv1   g097(.a(new_n146_), .O(new_n160_));
  nand4  g098(.a(new_n153_), .b(new_n160_), .c(new_n152_), .d(new_n157_), .O(new_n161_));
  nand3  g099(.a(new_n153_), .b(new_n152_), .c(new_n77_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n161_), .c(new_n159_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(x27), .c(new_n156_), .O(z14));
  nand2  g102(.a(x26), .b(x12), .O(new_n165_));
  nor3   g103(.a(new_n165_), .b(new_n115_), .c(new_n72_), .O(z15));
  nand2  g104(.a(x22), .b(x01), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(x23), .c(new_n93_), .O(z16));
  inv1   g106(.a(x24), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x23), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n167_), .c(new_n93_), .O(z17));
  nand2  g109(.a(new_n136_), .b(new_n131_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n93_), .O(z19));
  or2    g111(.a(new_n135_), .b(new_n132_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n129_), .c(new_n93_), .O(z20));
endmodule


