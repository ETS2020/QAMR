// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n156_, new_n158_, new_n160_, new_n161_,
    new_n164_;
  inv1   g000(.a(x37), .O(new_n63_));
  nand2  g001(.a(x35), .b(x28), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nand2  g007(.a(x40), .b(x39), .O(new_n70_));
  nor2   g008(.a(new_n70_), .b(x04), .O(new_n71_));
  aoi22  g009(.a(new_n71_), .b(new_n69_), .c(new_n65_), .d(x27), .O(new_n72_));
  nand2  g010(.a(new_n63_), .b(x36), .O(new_n73_));
  and2   g011(.a(new_n73_), .b(x16), .O(new_n74_));
  oai21  g012(.a(new_n72_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(new_n73_), .O(new_n76_));
  nor2   g014(.a(x32), .b(x30), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  oai21  g016(.a(new_n68_), .b(x28), .c(new_n78_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n76_), .O(z01));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n84_), .c(x04), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n70_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n86_), .c(new_n76_), .O(z02));
  nand2  g027(.a(new_n64_), .b(x27), .O(new_n90_));
  nand2  g028(.a(new_n78_), .b(x27), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n63_), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n90_), .c(x21), .O(z03));
  nand2  g031(.a(new_n92_), .b(x21), .O(new_n94_));
  nand3  g032(.a(new_n73_), .b(new_n64_), .c(x27), .O(new_n95_));
  nand3  g033(.a(new_n63_), .b(new_n78_), .c(new_n67_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(z04));
  oai21  g035(.a(new_n91_), .b(new_n83_), .c(new_n63_), .O(z05));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n63_), .c(new_n78_), .O(new_n100_));
  inv1   g038(.a(new_n100_), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n102_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n73_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  nor2   g050(.a(new_n76_), .b(new_n70_), .O(z08));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n73_), .O(z09));
  inv1   g053(.a(x04), .O(new_n116_));
  aoi21  g054(.a(new_n63_), .b(x36), .c(new_n67_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n79_), .O(new_n118_));
  inv1   g056(.a(new_n77_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n73_), .O(new_n120_));
  inv1   g058(.a(new_n70_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x05), .O(new_n122_));
  aoi21  g060(.a(new_n120_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nand4  g061(.a(new_n117_), .b(new_n79_), .c(new_n70_), .d(x07), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  oai21  g063(.a(new_n125_), .b(new_n123_), .c(new_n116_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(new_n121_), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n84_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  inv1   g071(.a(new_n84_), .O(new_n134_));
  nor2   g072(.a(new_n67_), .b(x04), .O(new_n135_));
  or2    g073(.a(x30), .b(x09), .O(new_n136_));
  aoi21  g074(.a(new_n135_), .b(new_n134_), .c(new_n136_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n133_), .c(new_n73_), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(z11));
  nand2  g077(.a(new_n71_), .b(x10), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n67_), .c(new_n63_), .O(new_n141_));
  nand2  g079(.a(new_n71_), .b(new_n69_), .O(new_n142_));
  oai21  g080(.a(new_n99_), .b(new_n68_), .c(new_n142_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n78_), .c(new_n141_), .O(z12));
  inv1   g082(.a(x13), .O(new_n145_));
  nand3  g083(.a(new_n121_), .b(new_n145_), .c(new_n116_), .O(new_n146_));
  aoi21  g084(.a(x36), .b(x35), .c(new_n83_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n65_), .c(x20), .O(new_n149_));
  oai21  g087(.a(new_n147_), .b(new_n146_), .c(new_n149_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  inv1   g089(.a(new_n146_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n119_), .c(new_n76_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n151_), .O(z13));
  inv1   g092(.a(z13), .O(z14));
  nand4  g093(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n73_), .O(z15));
  nand2  g095(.a(x22), .b(x01), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(x23), .c(new_n73_), .O(z16));
  inv1   g097(.a(x24), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x23), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n158_), .c(new_n73_), .O(z17));
  aoi21  g100(.a(new_n137_), .b(new_n133_), .c(new_n76_), .O(z18));
  oai21  g101(.a(new_n68_), .b(new_n67_), .c(new_n73_), .O(new_n164_));
  nor3   g102(.a(new_n164_), .b(new_n136_), .c(new_n131_), .O(z20));
  inv1   g103(.a(new_n138_), .O(z19));
endmodule


