// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n99_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n171_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n180_, new_n181_,
    new_n183_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x24), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n67_), .O(new_n74_));
  nand3  g012(.a(x40), .b(x39), .c(x24), .O(new_n75_));
  nor3   g013(.a(new_n75_), .b(x15), .c(x04), .O(new_n76_));
  oai21  g014(.a(new_n74_), .b(x10), .c(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n73_), .c(new_n67_), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  inv1   g021(.a(x04), .O(new_n84_));
  nor2   g022(.a(new_n64_), .b(new_n84_), .O(new_n85_));
  oai21  g023(.a(new_n83_), .b(new_n81_), .c(new_n85_), .O(z01));
  nor2   g024(.a(new_n72_), .b(x28), .O(new_n87_));
  oai21  g025(.a(x27), .b(x08), .c(new_n87_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n75_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n89_), .O(z02));
  nand2  g030(.a(x28), .b(x27), .O(new_n93_));
  oai22  g031(.a(new_n93_), .b(new_n72_), .c(new_n68_), .d(x27), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n94_), .b(new_n96_), .c(new_n64_), .O(z04));
  inv1   g035(.a(x24), .O(new_n98_));
  nand2  g036(.a(x40), .b(new_n98_), .O(new_n99_));
  nand2  g037(.a(new_n93_), .b(new_n68_), .O(new_n100_));
  and2   g038(.a(new_n100_), .b(new_n99_), .O(z05));
  nor2   g039(.a(new_n100_), .b(new_n64_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n99_), .O(z07));
  inv1   g051(.a(x39), .O(new_n114_));
  aoi21  g052(.a(new_n114_), .b(x24), .c(new_n63_), .O(z08));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  nand2  g054(.a(new_n99_), .b(x11), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(z09));
  inv1   g056(.a(x07), .O(new_n119_));
  nand4  g057(.a(x40), .b(x39), .c(x24), .d(x05), .O(new_n120_));
  oai21  g058(.a(z08), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n80_), .b(new_n73_), .c(x04), .O(new_n122_));
  nand2  g060(.a(x37), .b(x06), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n64_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n121_), .c(new_n124_), .O(new_n125_));
  inv1   g063(.a(new_n120_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n83_), .c(new_n84_), .O(new_n127_));
  oai21  g065(.a(new_n125_), .b(new_n67_), .c(new_n127_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(x04), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x24), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x40), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  nor2   g072(.a(new_n67_), .b(new_n84_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g074(.a(new_n134_), .b(new_n80_), .O(new_n137_));
  oai21  g075(.a(new_n80_), .b(new_n67_), .c(new_n129_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(new_n136_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n130_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n133_), .O(z11));
  nand3  g079(.a(new_n68_), .b(new_n73_), .c(new_n72_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  aoi21  g081(.a(x39), .b(new_n84_), .c(new_n70_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(x24), .O(new_n145_));
  or2    g083(.a(new_n70_), .b(x40), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n145_), .O(z12));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x27), .c(new_n83_), .O(new_n150_));
  inv1   g088(.a(x13), .O(new_n151_));
  inv1   g089(.a(new_n75_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n151_), .c(new_n84_), .O(new_n153_));
  inv1   g091(.a(x18), .O(new_n154_));
  inv1   g092(.a(x19), .O(new_n155_));
  nand3  g093(.a(x20), .b(new_n155_), .c(new_n154_), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n70_), .c(new_n99_), .O(new_n158_));
  oai21  g096(.a(new_n153_), .b(new_n150_), .c(new_n158_), .O(z13));
  nand3  g097(.a(new_n148_), .b(new_n82_), .c(x28), .O(new_n160_));
  nand3  g098(.a(x39), .b(new_n151_), .c(new_n84_), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n160_), .c(new_n98_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n63_), .c(new_n156_), .O(new_n164_));
  aoi21  g102(.a(new_n79_), .b(x27), .c(new_n83_), .O(new_n165_));
  nor2   g103(.a(new_n70_), .b(new_n98_), .O(new_n166_));
  oai21  g104(.a(new_n165_), .b(new_n161_), .c(new_n166_), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n164_), .c(new_n146_), .O(z14));
  inv1   g106(.a(x12), .O(new_n169_));
  oai21  g107(.a(new_n116_), .b(new_n169_), .c(new_n99_), .O(z15));
  nand2  g108(.a(x22), .b(x01), .O(new_n171_));
  nor3   g109(.a(new_n171_), .b(new_n64_), .c(x23), .O(z16));
  nand3  g110(.a(x23), .b(x22), .c(x01), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n63_), .c(x24), .O(z17));
  nand2  g112(.a(new_n134_), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n137_), .c(x08), .O(new_n176_));
  nand3  g114(.a(new_n87_), .b(x27), .c(new_n84_), .O(new_n177_));
  and2   g115(.a(new_n130_), .b(new_n99_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n177_), .c(new_n176_), .O(z18));
  oai21  g117(.a(new_n135_), .b(new_n88_), .c(new_n130_), .O(new_n180_));
  nor3   g118(.a(new_n134_), .b(new_n98_), .c(new_n129_), .O(new_n181_));
  aoi21  g119(.a(new_n180_), .b(new_n99_), .c(new_n181_), .O(z19));
  oai21  g120(.a(new_n72_), .b(new_n67_), .c(new_n178_), .O(new_n183_));
  aoi21  g121(.a(new_n137_), .b(x08), .c(new_n183_), .O(z20));
endmodule


