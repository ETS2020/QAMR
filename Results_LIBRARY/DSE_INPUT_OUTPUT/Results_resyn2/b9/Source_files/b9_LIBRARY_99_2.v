// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n162_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n171_, new_n172_;
  inv1   g000(.a(x04), .O(new_n63_));
  inv1   g001(.a(x35), .O(new_n64_));
  inv1   g002(.a(x36), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g004(.a(new_n66_), .b(x39), .c(new_n63_), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g007(.a(x39), .b(x10), .c(new_n63_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n69_), .b(x27), .c(new_n71_), .O(new_n72_));
  inv1   g010(.a(x15), .O(new_n73_));
  nand2  g011(.a(x40), .b(new_n73_), .O(new_n74_));
  oai21  g012(.a(new_n74_), .b(new_n72_), .c(x16), .O(z00));
  inv1   g013(.a(x16), .O(new_n76_));
  nor2   g014(.a(x40), .b(new_n76_), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(x36), .c(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n77_), .O(z01));
  inv1   g022(.a(x08), .O(new_n85_));
  inv1   g023(.a(x27), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(new_n85_), .c(new_n79_), .O(new_n87_));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x40), .b(x39), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n88_), .c(new_n89_), .O(new_n91_));
  oai21  g029(.a(new_n87_), .b(new_n63_), .c(new_n91_), .O(new_n92_));
  xor2a  g030(.a(new_n92_), .b(new_n77_), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n64_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n77_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  aoi21  g036(.a(new_n96_), .b(new_n98_), .c(new_n77_), .O(z04));
  inv1   g037(.a(x40), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x16), .O(new_n101_));
  nand2  g039(.a(new_n95_), .b(new_n94_), .O(new_n102_));
  and2   g040(.a(new_n102_), .b(new_n101_), .O(z05));
  nor2   g041(.a(new_n102_), .b(new_n77_), .O(z06));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  or2    g049(.a(x33), .b(x31), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n111_), .c(x14), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n109_), .c(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n101_), .O(z07));
  inv1   g053(.a(new_n89_), .O(z08));
  and2   g054(.a(x34), .b(x27), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(x26), .c(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n101_), .O(z09));
  nand2  g057(.a(z08), .b(x05), .O(new_n120_));
  nand3  g058(.a(new_n89_), .b(new_n101_), .c(x07), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  aoi21  g060(.a(new_n79_), .b(new_n65_), .c(x04), .O(new_n123_));
  nand2  g061(.a(x37), .b(x06), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n77_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(new_n122_), .c(new_n125_), .O(new_n126_));
  nor3   g064(.a(new_n89_), .b(new_n82_), .c(x04), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x05), .O(new_n128_));
  oai21  g066(.a(new_n126_), .b(new_n86_), .c(new_n128_), .O(z10));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n79_), .c(new_n85_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand3  g070(.a(new_n80_), .b(x27), .c(new_n63_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n133_), .c(new_n101_), .O(new_n135_));
  aoi21  g073(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(z11));
  oai21  g074(.a(new_n67_), .b(new_n100_), .c(new_n68_), .O(new_n137_));
  nor2   g075(.a(new_n77_), .b(new_n86_), .O(new_n138_));
  aoi22  g076(.a(new_n138_), .b(new_n137_), .c(new_n71_), .d(x40), .O(z12));
  inv1   g077(.a(x13), .O(new_n140_));
  nor2   g078(.a(new_n89_), .b(x04), .O(new_n141_));
  nand2  g079(.a(x36), .b(x35), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x28), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n141_), .c(new_n140_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x20), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n68_), .c(new_n144_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  aoi21  g086(.a(new_n127_), .b(new_n140_), .c(new_n77_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(z13));
  inv1   g088(.a(new_n68_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n78_), .c(x27), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n82_), .O(new_n153_));
  nand2  g091(.a(new_n141_), .b(new_n140_), .O(new_n154_));
  nand3  g092(.a(new_n142_), .b(new_n82_), .c(x28), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n141_), .c(new_n140_), .O(new_n156_));
  nand2  g094(.a(new_n151_), .b(x27), .O(new_n157_));
  aoi22  g095(.a(new_n157_), .b(new_n154_), .c(new_n156_), .d(new_n146_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n153_), .c(new_n77_), .O(z14));
  nand4  g097(.a(new_n117_), .b(new_n101_), .c(x26), .d(x12), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z15));
  nand2  g099(.a(x22), .b(x01), .O(new_n162_));
  nor3   g100(.a(new_n162_), .b(new_n77_), .c(x23), .O(z16));
  inv1   g101(.a(x24), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x23), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n162_), .c(new_n101_), .O(z17));
  inv1   g104(.a(new_n134_), .O(new_n167_));
  nand2  g105(.a(x27), .b(x04), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n87_), .c(new_n167_), .O(new_n169_));
  oai22  g107(.a(new_n169_), .b(new_n77_), .c(new_n90_), .d(new_n89_), .O(z18));
  nand2  g108(.a(new_n134_), .b(new_n101_), .O(new_n171_));
  nor2   g109(.a(new_n64_), .b(new_n86_), .O(new_n172_));
  nor3   g110(.a(new_n172_), .b(new_n171_), .c(new_n131_), .O(z20));
  aoi21  g111(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(z19));
endmodule


