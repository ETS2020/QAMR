// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n170_, new_n171_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n186_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x19), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n67_), .O(new_n74_));
  nand3  g012(.a(x40), .b(x39), .c(x19), .O(new_n75_));
  nor3   g013(.a(new_n75_), .b(x15), .c(x04), .O(new_n76_));
  oai21  g014(.a(new_n74_), .b(x10), .c(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n73_), .c(new_n67_), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  inv1   g021(.a(x04), .O(new_n84_));
  nor2   g022(.a(new_n64_), .b(new_n84_), .O(new_n85_));
  oai21  g023(.a(new_n83_), .b(new_n81_), .c(new_n85_), .O(z01));
  inv1   g024(.a(x08), .O(new_n87_));
  aoi21  g025(.a(new_n67_), .b(new_n87_), .c(new_n80_), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n75_), .O(new_n90_));
  oai21  g028(.a(new_n88_), .b(new_n84_), .c(new_n90_), .O(z02));
  inv1   g029(.a(new_n64_), .O(new_n92_));
  nand2  g030(.a(new_n69_), .b(x27), .O(new_n93_));
  nand2  g031(.a(new_n68_), .b(new_n67_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(x21), .O(z03));
  aoi21  g033(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n93_), .c(new_n64_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n64_), .O(z05));
  inv1   g037(.a(new_n98_), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n64_), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n65_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n102_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n92_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  inv1   g049(.a(x39), .O(new_n112_));
  aoi21  g050(.a(new_n112_), .b(x19), .c(new_n63_), .O(z08));
  nand3  g051(.a(x34), .b(x27), .c(x26), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n92_), .c(x11), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z09));
  inv1   g055(.a(x07), .O(new_n118_));
  nand4  g056(.a(x40), .b(x39), .c(x19), .d(x05), .O(new_n119_));
  oai21  g057(.a(z08), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  aoi21  g058(.a(new_n80_), .b(new_n73_), .c(x04), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n64_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n120_), .c(new_n123_), .O(new_n124_));
  inv1   g062(.a(new_n119_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n83_), .c(new_n84_), .O(new_n126_));
  oai21  g064(.a(new_n124_), .b(new_n67_), .c(new_n126_), .O(z10));
  nor2   g065(.a(x30), .b(x09), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  nand3  g067(.a(new_n128_), .b(new_n87_), .c(x04), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x19), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x40), .O(new_n132_));
  nor2   g070(.a(new_n72_), .b(x28), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n133_), .c(x08), .O(new_n136_));
  nand2  g074(.a(new_n133_), .b(x27), .O(new_n137_));
  nand3  g075(.a(new_n134_), .b(x27), .c(x04), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(new_n139_));
  aoi21  g077(.a(new_n137_), .b(new_n136_), .c(new_n139_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n129_), .c(new_n132_), .O(z11));
  nand3  g079(.a(new_n68_), .b(new_n73_), .c(new_n72_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(x10), .O(new_n143_));
  aoi21  g081(.a(x39), .b(new_n84_), .c(new_n70_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n143_), .c(x19), .O(new_n145_));
  or2    g083(.a(new_n70_), .b(x40), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n145_), .O(z12));
  inv1   g085(.a(x18), .O(new_n148_));
  nand2  g086(.a(new_n69_), .b(new_n68_), .O(new_n149_));
  nor2   g087(.a(x40), .b(x19), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n149_), .c(x20), .d(new_n148_), .O(new_n151_));
  oai21  g089(.a(new_n73_), .b(new_n72_), .c(x28), .O(new_n152_));
  inv1   g090(.a(x13), .O(new_n153_));
  nand4  g091(.a(x40), .b(x39), .c(new_n153_), .d(new_n84_), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n152_), .c(x19), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n151_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  inv1   g096(.a(new_n75_), .O(new_n159_));
  nand4  g097(.a(new_n83_), .b(new_n159_), .c(new_n153_), .d(new_n84_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n158_), .O(z13));
  inv1   g099(.a(x19), .O(new_n162_));
  nand3  g100(.a(new_n149_), .b(x20), .c(new_n148_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n67_), .c(new_n63_), .O(new_n164_));
  nand2  g102(.a(new_n152_), .b(x27), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n82_), .c(new_n154_), .O(new_n166_));
  aoi21  g104(.a(new_n164_), .b(new_n162_), .c(new_n166_), .O(z14));
  nand3  g105(.a(new_n115_), .b(new_n92_), .c(x12), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z15));
  inv1   g107(.a(x01), .O(new_n170_));
  inv1   g108(.a(x22), .O(new_n171_));
  nor4   g109(.a(new_n64_), .b(x23), .c(new_n171_), .d(new_n170_), .O(z16));
  inv1   g110(.a(x24), .O(new_n173_));
  nor2   g111(.a(new_n171_), .b(new_n170_), .O(new_n174_));
  nand4  g112(.a(new_n174_), .b(new_n92_), .c(new_n173_), .d(x23), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z17));
  nand2  g114(.a(x27), .b(x04), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n88_), .c(new_n129_), .O(new_n178_));
  nand3  g116(.a(new_n135_), .b(x19), .c(x08), .O(new_n179_));
  oai21  g117(.a(new_n178_), .b(new_n64_), .c(new_n179_), .O(z18));
  inv1   g118(.a(new_n136_), .O(new_n181_));
  nand2  g119(.a(new_n134_), .b(x27), .O(new_n182_));
  nor2   g120(.a(new_n129_), .b(new_n64_), .O(new_n183_));
  oai21  g121(.a(new_n137_), .b(x04), .c(new_n183_), .O(new_n184_));
  aoi21  g122(.a(new_n182_), .b(new_n181_), .c(new_n184_), .O(z19));
  oai21  g123(.a(new_n72_), .b(new_n67_), .c(new_n128_), .O(new_n186_));
  oai21  g124(.a(new_n186_), .b(new_n181_), .c(new_n92_), .O(z20));
endmodule


