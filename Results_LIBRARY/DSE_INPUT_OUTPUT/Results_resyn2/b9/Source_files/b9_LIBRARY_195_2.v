// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n156_, new_n157_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n169_;
  inv1   g000(.a(x35), .O(new_n63_));
  nor2   g001(.a(x36), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  nand2  g004(.a(x37), .b(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  nand4  g006(.a(x40), .b(x39), .c(x10), .d(new_n68_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  nand2  g009(.a(x36), .b(x27), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  nand2  g012(.a(x35), .b(x28), .O(new_n75_));
  oai21  g013(.a(new_n74_), .b(x04), .c(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n73_), .c(new_n65_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n72_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x04), .c(new_n64_), .O(z01));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n82_), .c(x04), .O(new_n85_));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x29), .b(x08), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g026(.a(new_n74_), .b(new_n64_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(new_n88_), .c(new_n85_), .O(z02));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n63_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  inv1   g033(.a(new_n64_), .O(new_n96_));
  nand3  g034(.a(new_n93_), .b(new_n96_), .c(new_n95_), .O(z04));
  nand2  g035(.a(new_n92_), .b(new_n91_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n96_), .O(z06));
  inv1   g037(.a(z06), .O(z05));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n65_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n101_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n96_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  nand2  g048(.a(new_n74_), .b(new_n96_), .O(z08));
  and2   g049(.a(x34), .b(x27), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(x26), .c(x11), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n96_), .O(z09));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  aoi21  g053(.a(new_n79_), .b(new_n72_), .c(new_n115_), .O(new_n116_));
  nand3  g054(.a(new_n73_), .b(new_n74_), .c(x07), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(new_n116_), .c(new_n68_), .O(new_n119_));
  nand3  g057(.a(x37), .b(x27), .c(x06), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n119_), .c(new_n96_), .O(z10));
  nor2   g059(.a(x30), .b(x09), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  inv1   g061(.a(x08), .O(new_n124_));
  nand4  g062(.a(x35), .b(new_n83_), .c(x27), .d(new_n68_), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  nand2  g064(.a(x27), .b(x04), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(x35), .c(new_n83_), .O(new_n128_));
  aoi22  g066(.a(new_n128_), .b(new_n126_), .c(new_n125_), .d(new_n124_), .O(new_n129_));
  oai21  g067(.a(new_n129_), .b(new_n123_), .c(new_n96_), .O(z11));
  aoi22  g068(.a(new_n76_), .b(new_n73_), .c(new_n70_), .d(new_n96_), .O(z12));
  nand2  g069(.a(new_n75_), .b(new_n91_), .O(new_n132_));
  nor2   g070(.a(x19), .b(x18), .O(new_n133_));
  and2   g071(.a(x27), .b(x20), .O(new_n134_));
  nand4  g072(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n96_), .O(new_n135_));
  nand2  g073(.a(new_n83_), .b(x27), .O(new_n136_));
  nand2  g074(.a(x35), .b(x27), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n136_), .c(new_n79_), .O(new_n138_));
  inv1   g076(.a(x13), .O(new_n139_));
  nand4  g077(.a(x40), .b(x39), .c(new_n139_), .d(new_n68_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n138_), .c(new_n96_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n135_), .O(z13));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand4  g083(.a(x28), .b(x20), .c(new_n145_), .d(new_n144_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n140_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n141_), .b(new_n83_), .O(new_n149_));
  nand3  g087(.a(new_n133_), .b(x37), .c(x20), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n149_), .c(new_n148_), .O(new_n151_));
  oai21  g089(.a(new_n140_), .b(new_n79_), .c(new_n96_), .O(new_n152_));
  aoi21  g090(.a(new_n151_), .b(x27), .c(new_n152_), .O(z14));
  nand4  g091(.a(new_n112_), .b(new_n96_), .c(x26), .d(x12), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(z15));
  inv1   g093(.a(x01), .O(new_n156_));
  inv1   g094(.a(x22), .O(new_n157_));
  nor4   g095(.a(new_n64_), .b(x23), .c(new_n157_), .d(new_n156_), .O(z16));
  inv1   g096(.a(x24), .O(new_n159_));
  nor2   g097(.a(new_n157_), .b(new_n156_), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(new_n96_), .c(new_n159_), .d(x23), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z17));
  aoi21  g100(.a(new_n126_), .b(new_n84_), .c(new_n124_), .O(new_n163_));
  nand2  g101(.a(new_n126_), .b(x27), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  oai21  g103(.a(new_n136_), .b(x04), .c(x36), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x35), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n165_), .c(new_n122_), .O(z18));
  nand2  g106(.a(new_n137_), .b(new_n122_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n163_), .c(new_n96_), .O(z20));
  oai21  g108(.a(new_n129_), .b(new_n123_), .c(new_n96_), .O(z19));
endmodule


