// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n149_, new_n151_, new_n153_, new_n155_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  nand2  g008(.a(x36), .b(x29), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n70_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(new_n71_), .O(new_n74_));
  inv1   g012(.a(x30), .O(new_n75_));
  inv1   g013(.a(x32), .O(new_n76_));
  inv1   g014(.a(x35), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  oai21  g016(.a(new_n77_), .b(x28), .c(new_n78_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n76_), .c(new_n75_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n74_), .O(z01));
  nor2   g020(.a(new_n77_), .b(x28), .O(new_n83_));
  inv1   g021(.a(x08), .O(new_n84_));
  inv1   g022(.a(x27), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  and2   g024(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand2  g025(.a(x40), .b(x39), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor3   g027(.a(new_n89_), .b(new_n74_), .c(new_n88_), .O(new_n90_));
  oai21  g028(.a(new_n87_), .b(new_n65_), .c(new_n90_), .O(z02));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(x28), .b(x27), .O(new_n93_));
  oai22  g031(.a(new_n93_), .b(new_n77_), .c(new_n92_), .d(x27), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(new_n74_), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n71_), .c(new_n96_), .O(z04));
  nand2  g035(.a(new_n93_), .b(new_n92_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n71_), .O(z06));
  inv1   g037(.a(z06), .O(z05));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n71_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  nand2  g049(.a(new_n71_), .b(new_n88_), .O(z08));
  nand4  g050(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n71_), .O(z09));
  aoi21  g052(.a(new_n76_), .b(new_n75_), .c(new_n88_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  nand2  g055(.a(new_n88_), .b(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n79_), .c(x27), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  inv1   g058(.a(x05), .O(new_n121_));
  inv1   g059(.a(new_n88_), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(x04), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n124_), .c(new_n71_), .O(z10));
  inv1   g064(.a(x09), .O(new_n127_));
  nand2  g065(.a(x27), .b(x04), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n86_), .c(new_n83_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n75_), .c(new_n127_), .O(new_n130_));
  nand4  g068(.a(new_n122_), .b(new_n78_), .c(x29), .d(x08), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  aoi21  g070(.a(new_n130_), .b(new_n71_), .c(new_n132_), .O(z11));
  nand2  g071(.a(new_n67_), .b(x27), .O(new_n134_));
  nand3  g072(.a(new_n71_), .b(new_n68_), .c(new_n134_), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(z12));
  inv1   g074(.a(new_n63_), .O(new_n137_));
  nor2   g075(.a(x19), .b(x18), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(x20), .O(new_n139_));
  oai21  g077(.a(new_n78_), .b(new_n77_), .c(x28), .O(new_n140_));
  nor2   g078(.a(x13), .b(x04), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n122_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x27), .O(new_n144_));
  nand2  g082(.a(new_n141_), .b(new_n115_), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n144_), .c(new_n71_), .O(z13));
  nand2  g084(.a(new_n145_), .b(new_n71_), .O(new_n147_));
  aoi21  g085(.a(new_n143_), .b(x27), .c(new_n147_), .O(z14));
  nand4  g086(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n71_), .O(z15));
  nand2  g088(.a(x22), .b(x01), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(x23), .c(new_n71_), .O(z16));
  inv1   g090(.a(x23), .O(new_n153_));
  nor4   g091(.a(new_n151_), .b(new_n74_), .c(x24), .d(new_n153_), .O(z17));
  nand2  g092(.a(new_n130_), .b(new_n71_), .O(new_n155_));
  nand2  g093(.a(new_n131_), .b(new_n155_), .O(z18));
  inv1   g094(.a(new_n83_), .O(new_n157_));
  nand2  g095(.a(new_n122_), .b(x29), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n157_), .c(new_n84_), .O(new_n159_));
  nand2  g097(.a(new_n158_), .b(x27), .O(new_n160_));
  nand3  g098(.a(new_n83_), .b(x27), .c(new_n65_), .O(new_n161_));
  nand3  g099(.a(new_n71_), .b(new_n75_), .c(new_n127_), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n161_), .O(new_n164_));
  aoi21  g102(.a(new_n160_), .b(new_n159_), .c(new_n164_), .O(z19));
  oai21  g103(.a(new_n77_), .b(new_n85_), .c(new_n163_), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(new_n159_), .O(z20));
endmodule


