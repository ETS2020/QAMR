// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n158_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n166_, new_n167_;
  nand2  g000(.a(x40), .b(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x04), .O(new_n64_));
  aoi21  g002(.a(x35), .b(x28), .c(x37), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  nand3  g005(.a(x40), .b(x39), .c(new_n67_), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(new_n69_));
  aoi22  g007(.a(new_n69_), .b(x27), .c(new_n64_), .d(x10), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  inv1   g009(.a(x02), .O(new_n72_));
  nor2   g010(.a(x40), .b(new_n72_), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  oai21  g012(.a(new_n70_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(x36), .c(x27), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n73_), .O(z01));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n77_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(new_n72_), .c(new_n63_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n84_), .O(z02));
  inv1   g025(.a(x35), .O(new_n88_));
  inv1   g026(.a(x37), .O(new_n89_));
  nand2  g027(.a(x28), .b(x27), .O(new_n90_));
  oai22  g028(.a(new_n90_), .b(new_n88_), .c(new_n89_), .d(x27), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x21), .c(new_n73_), .O(z03));
  inv1   g030(.a(x21), .O(new_n93_));
  inv1   g031(.a(x40), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x02), .O(new_n95_));
  nand3  g033(.a(new_n91_), .b(new_n95_), .c(new_n93_), .O(z04));
  nand3  g034(.a(new_n90_), .b(new_n95_), .c(new_n89_), .O(z05));
  nand2  g035(.a(new_n90_), .b(new_n89_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n95_), .O(z06));
  inv1   g037(.a(x14), .O(new_n100_));
  inv1   g038(.a(x15), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n101_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n100_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n95_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  nand2  g048(.a(new_n95_), .b(new_n63_), .O(z08));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n95_), .O(z09));
  inv1   g051(.a(x27), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  nand3  g053(.a(new_n95_), .b(new_n63_), .c(x07), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  inv1   g055(.a(x36), .O(new_n118_));
  aoi21  g056(.a(new_n77_), .b(new_n118_), .c(x04), .O(new_n119_));
  nand2  g057(.a(x37), .b(x06), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n73_), .O(new_n121_));
  aoi21  g059(.a(new_n119_), .b(new_n117_), .c(new_n121_), .O(new_n122_));
  nor2   g060(.a(new_n80_), .b(new_n68_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(x05), .O(new_n124_));
  oai21  g062(.a(new_n122_), .b(new_n114_), .c(new_n124_), .O(z10));
  nand2  g063(.a(new_n78_), .b(new_n114_), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand3  g066(.a(new_n78_), .b(x27), .c(new_n67_), .O(new_n129_));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  and2   g068(.a(new_n130_), .b(new_n95_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  aoi21  g070(.a(new_n128_), .b(x08), .c(new_n132_), .O(z11));
  nand2  g071(.a(new_n69_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n64_), .b(x10), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g074(.a(new_n95_), .b(new_n136_), .O(z12));
  inv1   g075(.a(x13), .O(new_n138_));
  nand2  g076(.a(x36), .b(x35), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x28), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n64_), .c(new_n138_), .O(new_n141_));
  nor2   g079(.a(x19), .b(x18), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x20), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n65_), .c(new_n141_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  aoi21  g083(.a(new_n123_), .b(new_n138_), .c(new_n73_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n145_), .O(z13));
  nand2  g085(.a(new_n64_), .b(new_n138_), .O(new_n148_));
  nand2  g086(.a(new_n76_), .b(x27), .O(new_n149_));
  and2   g087(.a(new_n149_), .b(new_n80_), .O(new_n150_));
  oai22  g088(.a(new_n150_), .b(new_n148_), .c(new_n65_), .d(new_n114_), .O(new_n151_));
  nand3  g089(.a(new_n139_), .b(new_n80_), .c(x28), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n64_), .c(new_n138_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n143_), .c(new_n73_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n151_), .O(z14));
  nand4  g093(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n95_), .O(z15));
  nand2  g095(.a(x22), .b(x01), .O(new_n158_));
  nor3   g096(.a(new_n158_), .b(new_n73_), .c(x23), .O(z16));
  inv1   g097(.a(x24), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x23), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n158_), .c(new_n95_), .O(z17));
  nand2  g100(.a(new_n128_), .b(x08), .O(new_n163_));
  inv1   g101(.a(new_n132_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n163_), .O(z18));
  nand2  g103(.a(new_n127_), .b(new_n77_), .O(new_n166_));
  oai21  g104(.a(new_n88_), .b(new_n114_), .c(new_n131_), .O(new_n167_));
  aoi21  g105(.a(new_n166_), .b(x08), .c(new_n167_), .O(z20));
  aoi21  g106(.a(new_n128_), .b(x08), .c(new_n132_), .O(z19));
endmodule


