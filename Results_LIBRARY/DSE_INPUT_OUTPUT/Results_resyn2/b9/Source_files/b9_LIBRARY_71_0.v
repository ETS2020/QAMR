// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n162_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n174_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n64_), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(x10), .O(new_n70_));
  nor2   g008(.a(x36), .b(x35), .O(new_n71_));
  oai21  g009(.a(new_n71_), .b(new_n66_), .c(new_n70_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  nand3  g011(.a(x40), .b(x39), .c(new_n73_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(new_n72_), .c(new_n64_), .O(new_n76_));
  oai21  g014(.a(new_n69_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(x28), .c(new_n79_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g022(.a(x08), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  aoi21  g025(.a(new_n66_), .b(new_n85_), .c(new_n87_), .O(new_n88_));
  nand2  g026(.a(x40), .b(x39), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g029(.a(new_n88_), .b(new_n73_), .c(new_n91_), .O(z02));
  inv1   g030(.a(new_n63_), .O(new_n93_));
  oai21  g031(.a(new_n78_), .b(new_n86_), .c(x27), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n66_), .O(new_n96_));
  nand4  g034(.a(new_n96_), .b(new_n94_), .c(new_n93_), .d(x21), .O(z03));
  aoi21  g035(.a(new_n95_), .b(new_n66_), .c(x21), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(new_n94_), .c(new_n63_), .O(z04));
  aoi21  g037(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n63_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n64_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n103_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n93_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  inv1   g050(.a(new_n89_), .O(z08));
  nand2  g051(.a(x34), .b(x27), .O(new_n114_));
  nand2  g052(.a(x26), .b(x11), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n114_), .c(new_n93_), .O(z09));
  nor2   g054(.a(new_n89_), .b(new_n82_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x05), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(new_n119_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(new_n89_), .b(new_n121_), .O(new_n122_));
  nand4  g060(.a(new_n122_), .b(new_n120_), .c(new_n80_), .d(x27), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n118_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n73_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n63_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n125_), .O(z10));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n87_), .c(new_n85_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  nor2   g072(.a(new_n78_), .b(x28), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(x27), .c(new_n73_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n134_), .c(new_n133_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n93_), .O(z11));
  oai21  g076(.a(new_n74_), .b(new_n71_), .c(new_n67_), .O(new_n139_));
  oai21  g077(.a(new_n74_), .b(new_n70_), .c(new_n93_), .O(new_n140_));
  aoi21  g078(.a(new_n139_), .b(x27), .c(new_n140_), .O(z12));
  aoi21  g079(.a(x36), .b(x35), .c(new_n86_), .O(new_n142_));
  nor2   g080(.a(x13), .b(x04), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(z08), .O(new_n144_));
  inv1   g082(.a(x18), .O(new_n145_));
  inv1   g083(.a(x19), .O(new_n146_));
  nand3  g084(.a(x20), .b(new_n146_), .c(new_n145_), .O(new_n147_));
  oai22  g085(.a(new_n147_), .b(new_n67_), .c(new_n144_), .d(new_n142_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  aoi21  g087(.a(new_n143_), .b(new_n117_), .c(new_n63_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n149_), .O(z13));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n82_), .c(x28), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n144_), .c(new_n147_), .O(new_n155_));
  nand2  g093(.a(new_n86_), .b(x27), .O(new_n156_));
  and2   g094(.a(new_n156_), .b(new_n82_), .O(new_n157_));
  oai22  g095(.a(new_n157_), .b(new_n144_), .c(new_n67_), .d(new_n66_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n155_), .c(new_n93_), .O(z14));
  nand2  g097(.a(x26), .b(x12), .O(new_n160_));
  nor3   g098(.a(new_n160_), .b(new_n114_), .c(new_n63_), .O(z15));
  nand2  g099(.a(x22), .b(x01), .O(new_n162_));
  nor3   g100(.a(new_n162_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g101(.a(x24), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x23), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n162_), .c(new_n93_), .O(z17));
  nand2  g104(.a(new_n66_), .b(new_n85_), .O(new_n167_));
  nand2  g105(.a(x27), .b(x04), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n167_), .c(new_n135_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n134_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n93_), .O(new_n171_));
  nand3  g109(.a(z08), .b(x29), .c(x08), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n171_), .O(z18));
  oai21  g111(.a(new_n78_), .b(new_n66_), .c(new_n134_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n131_), .c(new_n93_), .O(z20));
  nand2  g113(.a(new_n137_), .b(new_n93_), .O(z19));
endmodule


