// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n154_,
    new_n156_, new_n158_, new_n159_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_;
  inv1   g000(.a(x19), .O(new_n63_));
  inv1   g001(.a(x39), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n66_), .c(new_n67_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x19), .O(new_n76_));
  nor2   g014(.a(x15), .b(x04), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(new_n76_), .c(new_n74_), .O(new_n78_));
  oai21  g016(.a(new_n71_), .b(new_n65_), .c(new_n78_), .O(z00));
  inv1   g017(.a(new_n65_), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n68_), .c(new_n83_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n80_), .c(x04), .O(z01));
  inv1   g023(.a(x08), .O(new_n86_));
  nand2  g024(.a(new_n68_), .b(new_n86_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(x35), .c(new_n81_), .O(new_n88_));
  inv1   g026(.a(x29), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(x02), .c(x40), .O(new_n91_));
  aoi21  g029(.a(new_n88_), .b(x04), .c(new_n91_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x19), .c(x39), .O(z02));
  inv1   g031(.a(x35), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  oai22  g034(.a(new_n96_), .b(new_n94_), .c(new_n95_), .d(x27), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi21  g037(.a(new_n97_), .b(new_n99_), .c(new_n65_), .O(z04));
  nand3  g038(.a(new_n96_), .b(new_n80_), .c(new_n95_), .O(z05));
  nand2  g039(.a(new_n96_), .b(new_n95_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n80_), .O(z06));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n80_), .O(z07));
  inv1   g052(.a(x40), .O(new_n115_));
  aoi21  g053(.a(new_n115_), .b(new_n63_), .c(new_n64_), .O(z08));
  nand2  g054(.a(x34), .b(x27), .O(new_n117_));
  nand2  g055(.a(x26), .b(x11), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(new_n117_), .c(new_n80_), .O(z09));
  nor2   g057(.a(new_n82_), .b(new_n68_), .O(new_n120_));
  nand3  g058(.a(x40), .b(x39), .c(x05), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  and2   g060(.a(new_n75_), .b(x07), .O(new_n123_));
  aoi22  g061(.a(new_n123_), .b(new_n120_), .c(new_n122_), .d(new_n84_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n65_), .O(new_n127_));
  oai21  g065(.a(new_n124_), .b(x04), .c(new_n127_), .O(z10));
  nand2  g066(.a(new_n90_), .b(x40), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n63_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x39), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  nand2  g070(.a(x27), .b(x04), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(new_n87_), .c(x35), .d(new_n81_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n132_), .c(new_n131_), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(z11));
  nor3   g074(.a(x37), .b(x36), .c(x35), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n68_), .c(new_n72_), .O(new_n138_));
  oai22  g076(.a(new_n75_), .b(x04), .c(new_n69_), .d(new_n68_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(new_n80_), .O(z12));
  inv1   g078(.a(new_n83_), .O(new_n141_));
  nor2   g079(.a(x13), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(x40), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n63_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x39), .O(new_n145_));
  inv1   g083(.a(x18), .O(new_n146_));
  nand3  g084(.a(x20), .b(new_n63_), .c(new_n146_), .O(new_n147_));
  aoi21  g085(.a(x36), .b(x35), .c(new_n81_), .O(new_n148_));
  nand3  g086(.a(new_n142_), .b(x40), .c(x39), .O(new_n149_));
  oai22  g087(.a(new_n149_), .b(new_n148_), .c(new_n147_), .d(new_n69_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n145_), .O(z13));
  inv1   g090(.a(z13), .O(z14));
  nand2  g091(.a(x26), .b(x12), .O(new_n154_));
  nor3   g092(.a(new_n154_), .b(new_n117_), .c(new_n65_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(x23), .c(new_n80_), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x23), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n156_), .c(new_n80_), .O(z17));
  nand2  g098(.a(new_n134_), .b(new_n132_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n80_), .O(new_n162_));
  nand2  g100(.a(new_n90_), .b(new_n76_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n162_), .O(z18));
  oai22  g102(.a(new_n75_), .b(new_n89_), .c(new_n94_), .d(x28), .O(new_n165_));
  nand2  g103(.a(x35), .b(x27), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n132_), .c(new_n80_), .O(new_n167_));
  aoi21  g105(.a(new_n165_), .b(x08), .c(new_n167_), .O(z20));
  inv1   g106(.a(new_n135_), .O(z19));
endmodule


