// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n175_, new_n177_,
    new_n178_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x15), .O(new_n71_));
  nand2  g009(.a(x16), .b(new_n71_), .O(new_n72_));
  inv1   g010(.a(x14), .O(new_n73_));
  inv1   g011(.a(x16), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  oai21  g013(.a(new_n72_), .b(new_n70_), .c(new_n75_), .O(z00));
  inv1   g014(.a(x27), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n78_), .c(new_n77_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  inv1   g020(.a(x32), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g022(.a(x16), .b(new_n73_), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(new_n65_), .O(new_n86_));
  oai21  g024(.a(new_n84_), .b(new_n81_), .c(new_n86_), .O(z01));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n80_), .c(x04), .O(new_n89_));
  nand2  g027(.a(x40), .b(x39), .O(new_n90_));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n89_), .c(new_n85_), .O(z02));
  nand2  g031(.a(x35), .b(x28), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x27), .c(new_n85_), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n77_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n95_), .c(new_n99_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n85_), .O(z05));
  inv1   g040(.a(new_n101_), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n85_), .O(z06));
  inv1   g042(.a(x31), .O(new_n105_));
  inv1   g043(.a(x33), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n71_), .O(new_n107_));
  nand4  g045(.a(new_n107_), .b(new_n106_), .c(new_n105_), .d(x16), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n73_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(x03), .O(new_n113_));
  aoi21  g051(.a(new_n108_), .b(x14), .c(new_n113_), .O(z07));
  inv1   g052(.a(new_n85_), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n90_), .O(z08));
  nand3  g054(.a(x34), .b(x27), .c(x26), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n115_), .c(x11), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z09));
  nor2   g058(.a(x32), .b(x30), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n90_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n90_), .b(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n81_), .c(new_n122_), .O(new_n125_));
  oai21  g063(.a(new_n90_), .b(x05), .c(new_n65_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  nor2   g066(.a(new_n128_), .b(new_n85_), .O(new_n129_));
  oai21  g067(.a(new_n126_), .b(new_n125_), .c(new_n129_), .O(z10));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n80_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  inv1   g073(.a(new_n80_), .O(new_n136_));
  nor2   g074(.a(new_n77_), .b(x04), .O(new_n137_));
  inv1   g075(.a(x09), .O(new_n138_));
  nand2  g076(.a(new_n82_), .b(new_n138_), .O(new_n139_));
  aoi21  g077(.a(new_n137_), .b(new_n136_), .c(new_n139_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n135_), .c(new_n115_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(z11));
  nand2  g080(.a(new_n67_), .b(x27), .O(new_n143_));
  nand3  g081(.a(new_n115_), .b(new_n68_), .c(new_n143_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(z12));
  inv1   g083(.a(new_n90_), .O(new_n146_));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x28), .O(new_n148_));
  nor2   g086(.a(x13), .b(x04), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n146_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  nand3  g090(.a(x20), .b(new_n152_), .c(new_n151_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n63_), .c(new_n150_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x27), .O(new_n155_));
  aoi21  g093(.a(new_n149_), .b(new_n122_), .c(new_n85_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n155_), .O(z13));
  nand2  g095(.a(new_n149_), .b(new_n146_), .O(new_n158_));
  nand3  g096(.a(new_n147_), .b(new_n121_), .c(x28), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n158_), .c(new_n153_), .O(new_n161_));
  aoi21  g099(.a(new_n79_), .b(x27), .c(new_n84_), .O(new_n162_));
  oai22  g100(.a(new_n162_), .b(new_n158_), .c(new_n63_), .d(new_n77_), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n161_), .c(new_n115_), .O(z14));
  nand3  g102(.a(new_n118_), .b(new_n115_), .c(x12), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z15));
  inv1   g104(.a(x01), .O(new_n167_));
  inv1   g105(.a(x22), .O(new_n168_));
  nor4   g106(.a(new_n85_), .b(x23), .c(new_n168_), .d(new_n167_), .O(z16));
  inv1   g107(.a(x24), .O(new_n170_));
  nor2   g108(.a(new_n168_), .b(new_n167_), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(new_n115_), .c(new_n170_), .d(x23), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(z17));
  aoi21  g111(.a(new_n140_), .b(new_n135_), .c(new_n85_), .O(z18));
  nand2  g112(.a(new_n140_), .b(new_n135_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n115_), .O(z19));
  nand2  g114(.a(x35), .b(x27), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n82_), .c(new_n138_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n133_), .c(new_n115_), .O(z20));
endmodule


