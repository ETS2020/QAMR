// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n164_, new_n166_, new_n167_, new_n169_,
    new_n170_;
  nand2  g000(.a(x40), .b(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x04), .O(new_n64_));
  aoi21  g002(.a(x35), .b(x28), .c(x37), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  nand3  g005(.a(x40), .b(x39), .c(new_n67_), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(new_n69_));
  aoi22  g007(.a(new_n69_), .b(x27), .c(new_n64_), .d(x10), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x40), .b(x36), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  nor2   g013(.a(x32), .b(x30), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n75_), .c(new_n76_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x04), .c(new_n72_), .O(z01));
  nand2  g018(.a(x35), .b(new_n77_), .O(new_n81_));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n81_), .c(x04), .O(new_n83_));
  aoi21  g021(.a(x29), .b(x08), .c(x02), .O(new_n84_));
  nor2   g022(.a(new_n84_), .b(new_n63_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n83_), .O(z02));
  inv1   g024(.a(x35), .O(new_n87_));
  inv1   g025(.a(x37), .O(new_n88_));
  nand2  g026(.a(x28), .b(x27), .O(new_n89_));
  oai22  g027(.a(new_n89_), .b(new_n87_), .c(new_n88_), .d(x27), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g029(.a(x21), .O(new_n92_));
  aoi21  g030(.a(new_n90_), .b(new_n92_), .c(new_n72_), .O(z04));
  aoi21  g031(.a(new_n89_), .b(new_n88_), .c(new_n72_), .O(z05));
  nand2  g032(.a(new_n89_), .b(new_n88_), .O(new_n95_));
  nor2   g033(.a(new_n95_), .b(new_n72_), .O(z06));
  inv1   g034(.a(x14), .O(new_n97_));
  inv1   g035(.a(x15), .O(new_n98_));
  nand2  g036(.a(x17), .b(new_n98_), .O(new_n99_));
  nor2   g037(.a(x33), .b(x31), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n99_), .c(new_n97_), .O(new_n101_));
  inv1   g039(.a(new_n72_), .O(new_n102_));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  nand2  g042(.a(x25), .b(new_n97_), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n104_), .c(x38), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n102_), .c(x03), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(new_n101_), .O(z07));
  nand2  g046(.a(new_n102_), .b(new_n63_), .O(z08));
  nand2  g047(.a(x34), .b(x27), .O(new_n110_));
  nand2  g048(.a(x26), .b(x11), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(new_n102_), .O(z09));
  inv1   g050(.a(x39), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(x05), .O(new_n114_));
  oai21  g052(.a(new_n114_), .b(new_n78_), .c(x40), .O(new_n115_));
  inv1   g053(.a(x07), .O(new_n116_));
  aoi21  g054(.a(new_n63_), .b(new_n116_), .c(new_n75_), .O(new_n117_));
  inv1   g055(.a(x05), .O(new_n118_));
  nor3   g056(.a(new_n76_), .b(new_n63_), .c(new_n118_), .O(new_n119_));
  aoi21  g057(.a(new_n117_), .b(new_n115_), .c(new_n119_), .O(new_n120_));
  nand3  g058(.a(x37), .b(x27), .c(x06), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n72_), .O(new_n123_));
  oai21  g061(.a(new_n120_), .b(x04), .c(new_n123_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n81_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand4  g067(.a(x35), .b(new_n77_), .c(x27), .d(new_n67_), .O(new_n130_));
  nor3   g068(.a(new_n72_), .b(x30), .c(x09), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(new_n129_), .O(z18));
  inv1   g070(.a(z18), .O(z11));
  inv1   g071(.a(x10), .O(new_n134_));
  nor3   g072(.a(x37), .b(x36), .c(x35), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n75_), .c(new_n134_), .O(new_n136_));
  oai21  g074(.a(new_n65_), .b(new_n75_), .c(new_n68_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n136_), .c(new_n102_), .O(z12));
  inv1   g076(.a(x13), .O(new_n139_));
  nand2  g077(.a(x36), .b(x35), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x28), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n64_), .c(new_n139_), .O(new_n142_));
  inv1   g080(.a(x18), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand3  g082(.a(x20), .b(new_n144_), .c(new_n143_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n65_), .c(new_n142_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  inv1   g085(.a(new_n76_), .O(new_n148_));
  nand2  g086(.a(new_n64_), .b(new_n139_), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n148_), .c(new_n72_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n147_), .O(z13));
  inv1   g090(.a(new_n141_), .O(new_n153_));
  inv1   g091(.a(new_n145_), .O(new_n154_));
  nand2  g092(.a(new_n88_), .b(new_n87_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n153_), .c(new_n75_), .O(new_n157_));
  nor2   g095(.a(new_n65_), .b(new_n75_), .O(new_n158_));
  nand2  g096(.a(new_n154_), .b(new_n158_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n149_), .c(new_n72_), .O(new_n160_));
  oai21  g098(.a(new_n157_), .b(new_n148_), .c(new_n160_), .O(z14));
  nand2  g099(.a(x26), .b(x12), .O(new_n162_));
  nor3   g100(.a(new_n162_), .b(new_n110_), .c(new_n72_), .O(z15));
  nand2  g101(.a(x22), .b(x01), .O(new_n164_));
  nor3   g102(.a(new_n164_), .b(new_n72_), .c(x23), .O(z16));
  inv1   g103(.a(x24), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x23), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n164_), .c(new_n102_), .O(z17));
  nor2   g106(.a(x30), .b(x09), .O(new_n169_));
  oai21  g107(.a(new_n87_), .b(new_n75_), .c(new_n169_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n127_), .c(new_n102_), .O(z20));
  inv1   g109(.a(z18), .O(z19));
endmodule


