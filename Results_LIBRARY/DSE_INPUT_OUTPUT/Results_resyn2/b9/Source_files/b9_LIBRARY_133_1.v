// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n175_,
    new_n176_, new_n177_, new_n178_, new_n179_, new_n181_, new_n182_,
    new_n183_;
  inv1   g000(.a(x39), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g014(.a(x40), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(new_n63_), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n76_), .c(new_n66_), .d(new_n73_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  inv1   g020(.a(x35), .O(new_n83_));
  inv1   g021(.a(x36), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(x28), .c(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand3  g024(.a(new_n86_), .b(new_n82_), .c(new_n81_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g026(.a(x08), .O(new_n89_));
  inv1   g027(.a(x28), .O(new_n90_));
  nand2  g028(.a(x35), .b(new_n90_), .O(new_n91_));
  aoi21  g029(.a(new_n68_), .b(new_n89_), .c(new_n91_), .O(new_n92_));
  aoi21  g030(.a(x29), .b(x08), .c(x02), .O(new_n93_));
  nor3   g031(.a(new_n93_), .b(new_n77_), .c(new_n63_), .O(new_n94_));
  oai21  g032(.a(new_n92_), .b(new_n73_), .c(new_n94_), .O(z02));
  nand2  g033(.a(new_n70_), .b(x27), .O(new_n96_));
  nand2  g034(.a(new_n69_), .b(new_n68_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n96_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n96_), .c(new_n65_), .O(z04));
  oai21  g038(.a(new_n90_), .b(new_n68_), .c(new_n69_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n64_), .O(z06));
  inv1   g040(.a(z06), .O(z05));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n64_), .O(z07));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n64_), .O(z09));
  nand2  g054(.a(new_n82_), .b(new_n81_), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x40), .O(new_n118_));
  inv1   g056(.a(x07), .O(new_n119_));
  nand2  g057(.a(new_n77_), .b(new_n119_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n85_), .c(x27), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  inv1   g060(.a(x05), .O(new_n123_));
  aoi21  g061(.a(x40), .b(new_n123_), .c(x04), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n64_), .O(z10));
  oai21  g065(.a(new_n63_), .b(x29), .c(x40), .O(new_n128_));
  nand2  g066(.a(x27), .b(x04), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(x35), .c(new_n90_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g069(.a(new_n64_), .b(new_n68_), .O(new_n132_));
  nand3  g070(.a(x35), .b(new_n90_), .c(new_n73_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x39), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n89_), .O(new_n136_));
  nor2   g074(.a(x30), .b(x09), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(new_n138_));
  aoi21  g076(.a(new_n136_), .b(new_n131_), .c(new_n138_), .O(z11));
  nor3   g077(.a(x37), .b(x36), .c(x35), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n68_), .c(new_n74_), .O(new_n141_));
  nand2  g079(.a(new_n70_), .b(new_n69_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x27), .O(new_n143_));
  oai21  g081(.a(new_n77_), .b(x04), .c(new_n143_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n141_), .c(new_n64_), .O(z12));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(new_n142_), .c(new_n64_), .d(x20), .O(new_n147_));
  oai21  g085(.a(new_n84_), .b(new_n83_), .c(x28), .O(new_n148_));
  nor2   g086(.a(x13), .b(x04), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n78_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x27), .O(new_n152_));
  nand4  g090(.a(new_n149_), .b(new_n117_), .c(x40), .d(x39), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n152_), .O(z13));
  inv1   g092(.a(new_n148_), .O(new_n155_));
  nand2  g093(.a(new_n69_), .b(new_n83_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n146_), .c(x20), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n155_), .c(new_n68_), .O(new_n158_));
  nand3  g096(.a(new_n146_), .b(new_n64_), .c(x20), .O(new_n159_));
  nand2  g097(.a(new_n149_), .b(new_n78_), .O(new_n160_));
  oai21  g098(.a(new_n159_), .b(new_n143_), .c(new_n160_), .O(new_n161_));
  oai21  g099(.a(new_n158_), .b(new_n117_), .c(new_n161_), .O(z14));
  nand4  g100(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n64_), .O(z15));
  inv1   g102(.a(x01), .O(new_n165_));
  inv1   g103(.a(x22), .O(new_n166_));
  nor4   g104(.a(new_n65_), .b(x23), .c(new_n166_), .d(new_n165_), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  nor2   g106(.a(new_n166_), .b(new_n165_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(new_n64_), .c(new_n168_), .d(x23), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z17));
  aoi21  g109(.a(new_n129_), .b(new_n92_), .c(new_n138_), .O(new_n172_));
  nand3  g110(.a(new_n78_), .b(x29), .c(x08), .O(new_n173_));
  oai21  g111(.a(new_n172_), .b(new_n65_), .c(new_n173_), .O(z18));
  nand2  g112(.a(x40), .b(x04), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(x35), .c(new_n90_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(x39), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n132_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n89_), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n131_), .c(new_n138_), .O(z19));
  inv1   g118(.a(new_n173_), .O(new_n181_));
  aoi21  g119(.a(new_n90_), .b(x08), .c(x27), .O(new_n182_));
  oai21  g120(.a(new_n182_), .b(new_n83_), .c(new_n137_), .O(new_n183_));
  aoi21  g121(.a(new_n183_), .b(new_n64_), .c(new_n181_), .O(z20));
  buf    g122(.a(x40), .O(z08));
endmodule


