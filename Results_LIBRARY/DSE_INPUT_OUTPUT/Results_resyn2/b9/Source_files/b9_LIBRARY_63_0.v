// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n175_,
    new_n176_, new_n177_, new_n179_, new_n180_, new_n181_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x14), .b(x08), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x27), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x04), .c(new_n72_), .O(z01));
  nand2  g019(.a(x40), .b(x39), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nor2   g021(.a(new_n75_), .b(x28), .O(new_n84_));
  inv1   g022(.a(x08), .O(new_n85_));
  inv1   g023(.a(x27), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  aoi21  g027(.a(new_n88_), .b(x04), .c(new_n89_), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n83_), .c(new_n72_), .O(z02));
  nand2  g029(.a(x35), .b(x28), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x27), .c(new_n72_), .O(new_n93_));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n86_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n93_), .c(new_n97_), .O(z04));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n72_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(new_n72_), .O(new_n102_));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n102_), .O(z07));
  nand2  g051(.a(new_n102_), .b(new_n82_), .O(z08));
  nand2  g052(.a(x34), .b(x27), .O(new_n115_));
  nand2  g053(.a(x26), .b(x11), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(new_n115_), .c(new_n102_), .O(z09));
  inv1   g055(.a(x30), .O(new_n118_));
  inv1   g056(.a(x32), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n83_), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  nand2  g060(.a(new_n82_), .b(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n77_), .c(x27), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  aoi21  g064(.a(new_n83_), .b(new_n126_), .c(x04), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n102_), .O(z10));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n72_), .O(new_n132_));
  nand2  g070(.a(new_n83_), .b(x29), .O(new_n133_));
  nand2  g071(.a(x27), .b(x04), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n84_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand3  g074(.a(new_n84_), .b(x27), .c(new_n65_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n105_), .c(new_n85_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n136_), .c(new_n132_), .O(z11));
  nand2  g077(.a(new_n67_), .b(x27), .O(new_n140_));
  nand2  g078(.a(new_n68_), .b(new_n140_), .O(new_n141_));
  nand2  g079(.a(new_n102_), .b(new_n141_), .O(z12));
  nand2  g080(.a(x36), .b(x35), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x28), .O(new_n144_));
  nor2   g082(.a(x13), .b(x04), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n144_), .c(new_n83_), .O(new_n146_));
  nand2  g084(.a(new_n92_), .b(new_n94_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n147_), .c(x20), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n146_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  nand3  g089(.a(new_n145_), .b(new_n120_), .c(new_n83_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n151_), .c(new_n72_), .O(z13));
  nand2  g091(.a(new_n145_), .b(new_n83_), .O(new_n154_));
  nand2  g092(.a(new_n148_), .b(x20), .O(new_n155_));
  nand3  g093(.a(new_n143_), .b(new_n79_), .c(x28), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n154_), .c(new_n155_), .O(new_n158_));
  inv1   g096(.a(x28), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x27), .c(new_n120_), .O(new_n160_));
  oai22  g098(.a(new_n160_), .b(new_n154_), .c(new_n63_), .d(new_n86_), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n158_), .c(new_n102_), .O(z14));
  nand2  g100(.a(x26), .b(x12), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n115_), .c(new_n72_), .O(z15));
  nand2  g102(.a(x22), .b(x01), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(x23), .c(new_n102_), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x23), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n165_), .c(new_n102_), .O(z17));
  inv1   g107(.a(new_n132_), .O(new_n170_));
  oai21  g108(.a(new_n86_), .b(new_n105_), .c(new_n85_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n134_), .c(new_n84_), .O(new_n172_));
  nand3  g110(.a(new_n83_), .b(x29), .c(x08), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n172_), .c(new_n170_), .O(z18));
  inv1   g112(.a(new_n131_), .O(new_n175_));
  nand3  g113(.a(new_n135_), .b(new_n133_), .c(x08), .O(new_n176_));
  nand3  g114(.a(new_n137_), .b(x14), .c(new_n85_), .O(new_n177_));
  aoi21  g115(.a(new_n177_), .b(new_n176_), .c(new_n175_), .O(z19));
  oai21  g116(.a(new_n75_), .b(x28), .c(new_n133_), .O(new_n179_));
  nand2  g117(.a(x35), .b(x27), .O(new_n180_));
  aoi21  g118(.a(new_n180_), .b(new_n131_), .c(new_n72_), .O(new_n181_));
  aoi21  g119(.a(new_n179_), .b(x08), .c(new_n181_), .O(z20));
endmodule


