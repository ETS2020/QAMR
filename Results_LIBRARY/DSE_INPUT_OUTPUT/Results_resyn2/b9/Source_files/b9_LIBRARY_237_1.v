// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n172_, new_n173_;
  inv1   g000(.a(x36), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x33), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  nand4  g007(.a(x40), .b(x39), .c(x10), .d(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n68_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n65_), .c(new_n66_), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n64_), .O(z08));
  nor2   g012(.a(new_n67_), .b(x04), .O(new_n75_));
  nor2   g013(.a(x36), .b(x35), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(x15), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(new_n75_), .c(z08), .O(new_n78_));
  oai21  g016(.a(new_n72_), .b(new_n64_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(new_n63_), .c(new_n67_), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  inv1   g021(.a(new_n83_), .O(new_n84_));
  nor2   g022(.a(new_n64_), .b(new_n69_), .O(new_n85_));
  oai21  g023(.a(new_n84_), .b(new_n82_), .c(new_n85_), .O(z01));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n81_), .c(x04), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n73_), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n88_), .c(new_n64_), .O(z02));
  inv1   g029(.a(x35), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n92_), .c(new_n93_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  inv1   g035(.a(new_n64_), .O(new_n98_));
  nand3  g036(.a(new_n95_), .b(new_n98_), .c(new_n97_), .O(z04));
  aoi21  g037(.a(new_n94_), .b(new_n93_), .c(new_n64_), .O(z05));
  nand2  g038(.a(new_n94_), .b(new_n93_), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n64_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n98_), .O(z07));
  nand3  g051(.a(x34), .b(x27), .c(x26), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n98_), .c(x11), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z09));
  inv1   g055(.a(new_n73_), .O(new_n118_));
  nand2  g056(.a(new_n84_), .b(new_n118_), .O(new_n119_));
  nand2  g057(.a(new_n81_), .b(new_n63_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(new_n73_), .b(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n120_), .c(x27), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  inv1   g062(.a(x05), .O(new_n125_));
  aoi21  g063(.a(new_n118_), .b(new_n125_), .c(x04), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n98_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  nand3  g068(.a(x40), .b(x39), .c(x29), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n81_), .c(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n131_), .b(x27), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  nand3  g073(.a(new_n75_), .b(x35), .c(new_n80_), .O(new_n136_));
  and2   g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n98_), .O(z11));
  nand2  g077(.a(new_n118_), .b(new_n69_), .O(new_n140_));
  oai21  g078(.a(new_n76_), .b(new_n140_), .c(new_n68_), .O(new_n141_));
  nand2  g079(.a(new_n70_), .b(new_n98_), .O(new_n142_));
  aoi21  g080(.a(new_n141_), .b(x27), .c(new_n142_), .O(z12));
  nand2  g081(.a(x35), .b(x28), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n93_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n145_), .c(x20), .O(new_n147_));
  nor2   g085(.a(x13), .b(x04), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n118_), .c(new_n80_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n147_), .c(new_n67_), .O(new_n150_));
  nand2  g088(.a(new_n148_), .b(new_n118_), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(new_n83_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n150_), .c(new_n98_), .O(new_n153_));
  inv1   g091(.a(new_n151_), .O(new_n154_));
  nor2   g092(.a(new_n63_), .b(new_n92_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(new_n154_), .c(x33), .d(x27), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n153_), .O(z13));
  nor2   g095(.a(new_n155_), .b(new_n80_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n151_), .c(new_n147_), .O(new_n159_));
  oai21  g097(.a(new_n151_), .b(new_n83_), .c(new_n98_), .O(new_n160_));
  aoi21  g098(.a(new_n159_), .b(x27), .c(new_n160_), .O(z14));
  nand3  g099(.a(new_n115_), .b(new_n98_), .c(x12), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(z15));
  inv1   g101(.a(x01), .O(new_n164_));
  inv1   g102(.a(x22), .O(new_n165_));
  nor4   g103(.a(new_n64_), .b(x23), .c(new_n165_), .d(new_n164_), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  nor2   g105(.a(new_n165_), .b(new_n164_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n98_), .c(new_n167_), .d(x23), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z17));
  aoi21  g108(.a(new_n137_), .b(new_n134_), .c(new_n64_), .O(z18));
  nand2  g109(.a(x35), .b(x27), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n135_), .c(new_n98_), .O(new_n173_));
  nor2   g111(.a(new_n173_), .b(new_n132_), .O(z20));
  nand2  g112(.a(new_n138_), .b(new_n98_), .O(z19));
endmodule


