// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n164_, new_n166_, new_n167_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n174_, new_n176_,
    new_n178_;
  inv1   g000(.a(x08), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g013(.a(new_n63_), .b(x04), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x15), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n81_), .c(new_n68_), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  inv1   g023(.a(new_n85_), .O(new_n86_));
  inv1   g024(.a(x04), .O(new_n87_));
  nor2   g025(.a(new_n65_), .b(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n86_), .b(new_n84_), .c(new_n88_), .O(z01));
  nand3  g027(.a(x40), .b(x39), .c(x08), .O(new_n90_));
  inv1   g028(.a(new_n90_), .O(z08));
  nand2  g029(.a(new_n83_), .b(x04), .O(new_n92_));
  inv1   g030(.a(x02), .O(new_n93_));
  inv1   g031(.a(x29), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n92_), .c(z08), .O(z02));
  inv1   g034(.a(x35), .O(new_n97_));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  oai22  g036(.a(new_n98_), .b(new_n97_), .c(new_n69_), .d(x27), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand3  g039(.a(new_n99_), .b(new_n64_), .c(new_n101_), .O(z04));
  nand3  g040(.a(new_n98_), .b(new_n64_), .c(new_n69_), .O(z05));
  nand2  g041(.a(new_n98_), .b(new_n69_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n64_), .O(z06));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  inv1   g045(.a(x14), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n108_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(x38), .O(new_n110_));
  inv1   g048(.a(x17), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(x15), .O(new_n112_));
  or2    g050(.a(x33), .b(x31), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n112_), .c(x14), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n110_), .c(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n64_), .O(z07));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n64_), .O(z09));
  nor2   g056(.a(new_n85_), .b(new_n77_), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  nand2  g058(.a(new_n77_), .b(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n84_), .c(new_n119_), .O(new_n122_));
  oai21  g060(.a(new_n77_), .b(x05), .c(new_n87_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  and2   g062(.a(new_n124_), .b(new_n64_), .O(new_n125_));
  oai21  g063(.a(new_n123_), .b(new_n122_), .c(new_n125_), .O(z10));
  nor2   g064(.a(x30), .b(x09), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n68_), .c(x40), .O(new_n128_));
  nor2   g066(.a(new_n97_), .b(x28), .O(new_n129_));
  oai21  g067(.a(new_n68_), .b(new_n87_), .c(new_n129_), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  oai21  g069(.a(new_n77_), .b(new_n94_), .c(new_n127_), .O(new_n132_));
  oai22  g070(.a(new_n132_), .b(new_n131_), .c(new_n128_), .d(x08), .O(z11));
  nor3   g071(.a(x37), .b(x36), .c(x35), .O(new_n134_));
  oai21  g072(.a(new_n134_), .b(new_n68_), .c(new_n73_), .O(new_n135_));
  inv1   g073(.a(new_n71_), .O(new_n136_));
  oai21  g074(.a(new_n77_), .b(x04), .c(new_n136_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n135_), .c(new_n64_), .O(z12));
  nor2   g076(.a(x19), .b(x18), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x20), .O(new_n140_));
  nand2  g078(.a(new_n70_), .b(new_n69_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n64_), .O(new_n142_));
  inv1   g080(.a(x13), .O(new_n143_));
  nand2  g081(.a(x36), .b(x35), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x28), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(z08), .c(new_n143_), .d(new_n87_), .O(new_n146_));
  oai21  g084(.a(new_n142_), .b(new_n140_), .c(new_n146_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  nand3  g086(.a(new_n119_), .b(new_n76_), .c(new_n143_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(z13));
  inv1   g088(.a(x40), .O(new_n151_));
  nand3  g089(.a(x39), .b(new_n143_), .c(new_n87_), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(new_n153_));
  nand3  g091(.a(new_n144_), .b(new_n85_), .c(x28), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n153_), .c(new_n63_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n151_), .c(new_n140_), .O(new_n156_));
  nand2  g094(.a(new_n82_), .b(x27), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n85_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n153_), .c(new_n63_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n151_), .c(new_n136_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n156_), .O(z14));
  nand4  g099(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n64_), .O(z15));
  nand2  g101(.a(x22), .b(x01), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(x23), .c(new_n64_), .O(z16));
  inv1   g103(.a(x24), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x23), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n164_), .c(new_n64_), .O(z17));
  nor2   g106(.a(x27), .b(x08), .O(new_n169_));
  nand2  g107(.a(x39), .b(x29), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x08), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(x40), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n127_), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(new_n174_));
  oai21  g112(.a(new_n169_), .b(new_n130_), .c(new_n174_), .O(z18));
  nand2  g113(.a(new_n169_), .b(new_n151_), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n131_), .c(new_n173_), .O(z19));
  aoi21  g115(.a(x28), .b(new_n68_), .c(new_n97_), .O(new_n178_));
  oai22  g116(.a(new_n178_), .b(new_n132_), .c(new_n128_), .d(x08), .O(z20));
endmodule


