// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n158_, new_n160_, new_n162_, new_n164_,
    new_n165_;
  inv1   g000(.a(x33), .O(new_n63_));
  inv1   g001(.a(x36), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  nand4  g008(.a(x40), .b(x39), .c(x10), .d(new_n70_), .O(new_n71_));
  oai21  g009(.a(new_n69_), .b(new_n68_), .c(new_n71_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n66_), .c(new_n67_), .O(new_n73_));
  inv1   g011(.a(new_n65_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nor2   g014(.a(new_n68_), .b(x04), .O(new_n77_));
  nor2   g015(.a(x36), .b(x35), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(x15), .O(new_n79_));
  nand4  g017(.a(new_n79_), .b(new_n77_), .c(new_n76_), .d(new_n74_), .O(new_n80_));
  oai21  g018(.a(new_n73_), .b(new_n65_), .c(new_n80_), .O(z00));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n64_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n74_), .c(x04), .O(z01));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n83_), .c(x04), .O(new_n90_));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor3   g029(.a(new_n91_), .b(new_n75_), .c(new_n65_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n90_), .O(z02));
  inv1   g031(.a(x35), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n82_), .c(x27), .O(new_n95_));
  nor2   g033(.a(x37), .b(x27), .O(new_n96_));
  nor2   g034(.a(new_n96_), .b(new_n65_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n95_), .c(new_n99_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n74_), .O(z05));
  inv1   g040(.a(new_n101_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n74_), .O(z06));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  inv1   g049(.a(x31), .O(new_n112_));
  nand2  g050(.a(new_n63_), .b(new_n112_), .O(new_n113_));
  oai22  g051(.a(new_n113_), .b(new_n111_), .c(new_n65_), .d(x14), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n109_), .c(x03), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z07));
  nand2  g054(.a(new_n75_), .b(new_n74_), .O(z08));
  nand2  g055(.a(x34), .b(x27), .O(new_n118_));
  nand2  g056(.a(x26), .b(x11), .O(new_n119_));
  oai21  g057(.a(new_n119_), .b(new_n118_), .c(new_n74_), .O(z09));
  inv1   g058(.a(x05), .O(new_n121_));
  nand2  g059(.a(new_n76_), .b(new_n121_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n75_), .b(new_n123_), .O(new_n124_));
  nand4  g062(.a(new_n124_), .b(new_n122_), .c(new_n84_), .d(new_n77_), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(new_n70_), .O(new_n126_));
  nor2   g064(.a(new_n86_), .b(new_n126_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  aoi21  g067(.a(new_n127_), .b(x05), .c(new_n129_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n125_), .c(new_n65_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n83_), .c(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  inv1   g074(.a(new_n83_), .O(new_n137_));
  inv1   g075(.a(x09), .O(new_n138_));
  inv1   g076(.a(x30), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  aoi21  g078(.a(new_n137_), .b(new_n77_), .c(new_n140_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(new_n136_), .c(new_n65_), .O(z18));
  inv1   g080(.a(z18), .O(z11));
  oai21  g081(.a(new_n78_), .b(new_n126_), .c(new_n69_), .O(new_n144_));
  nand2  g082(.a(new_n71_), .b(new_n74_), .O(new_n145_));
  aoi21  g083(.a(new_n144_), .b(x27), .c(new_n145_), .O(z12));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x20), .O(new_n148_));
  aoi21  g086(.a(x36), .b(x35), .c(new_n82_), .O(new_n149_));
  inv1   g087(.a(x13), .O(new_n150_));
  nand3  g088(.a(new_n76_), .b(new_n150_), .c(new_n70_), .O(new_n151_));
  oai22  g089(.a(new_n151_), .b(new_n149_), .c(new_n148_), .d(new_n69_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand2  g091(.a(new_n127_), .b(new_n150_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n153_), .c(new_n74_), .O(z13));
  nand2  g093(.a(new_n154_), .b(new_n74_), .O(new_n156_));
  aoi21  g094(.a(new_n152_), .b(x27), .c(new_n156_), .O(z14));
  nand2  g095(.a(x26), .b(x12), .O(new_n158_));
  nor3   g096(.a(new_n158_), .b(new_n118_), .c(new_n65_), .O(z15));
  nand2  g097(.a(x22), .b(x01), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(x23), .c(new_n74_), .O(z16));
  inv1   g099(.a(x23), .O(new_n162_));
  nor4   g100(.a(new_n160_), .b(new_n65_), .c(x24), .d(new_n162_), .O(z17));
  nand2  g101(.a(x35), .b(x27), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n74_), .c(new_n139_), .d(new_n138_), .O(new_n165_));
  nor2   g103(.a(new_n165_), .b(new_n134_), .O(z20));
  inv1   g104(.a(z18), .O(z19));
endmodule


