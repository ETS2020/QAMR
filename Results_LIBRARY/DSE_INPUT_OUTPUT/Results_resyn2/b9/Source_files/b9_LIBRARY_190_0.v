// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n160_,
    new_n162_, new_n164_, new_n165_, new_n168_, new_n170_, new_n171_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  and2   g005(.a(x40), .b(x39), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x10), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(new_n70_));
  aoi21  g008(.a(new_n67_), .b(x27), .c(new_n70_), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  nand2  g010(.a(x16), .b(new_n72_), .O(new_n73_));
  inv1   g011(.a(x16), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(x14), .O(new_n75_));
  oai21  g013(.a(new_n73_), .b(new_n71_), .c(new_n75_), .O(z00));
  nor2   g014(.a(x16), .b(x14), .O(new_n77_));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n79_), .c(new_n78_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(x04), .c(new_n77_), .O(z01));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n82_), .c(x04), .O(new_n88_));
  nand2  g026(.a(x40), .b(x39), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n88_), .c(new_n77_), .O(z02));
  nand2  g030(.a(x35), .b(x28), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x27), .c(new_n77_), .O(new_n94_));
  inv1   g032(.a(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n94_), .c(new_n99_), .O(z04));
  inv1   g038(.a(new_n77_), .O(new_n101_));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n101_), .O(z05));
  inv1   g041(.a(new_n102_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n101_), .O(z06));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  inv1   g045(.a(x14), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n108_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(x38), .O(new_n110_));
  inv1   g048(.a(x17), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(x15), .O(new_n112_));
  or2    g050(.a(x33), .b(x31), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n112_), .c(x14), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n110_), .c(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n101_), .O(z07));
  nor2   g054(.a(new_n77_), .b(new_n89_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n101_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  aoi21  g058(.a(new_n79_), .b(new_n78_), .c(new_n89_), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n89_), .b(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n83_), .c(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  aoi21  g065(.a(new_n68_), .b(new_n127_), .c(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n101_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n82_), .c(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  and2   g074(.a(x35), .b(x27), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n81_), .c(new_n65_), .O(new_n138_));
  nor2   g076(.a(x30), .b(x09), .O(new_n139_));
  and2   g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n136_), .c(new_n101_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(z11));
  nand2  g080(.a(new_n67_), .b(x27), .O(new_n143_));
  nand3  g081(.a(new_n101_), .b(new_n69_), .c(new_n143_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(z12));
  nor2   g083(.a(x13), .b(x04), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n121_), .O(new_n147_));
  nand2  g085(.a(new_n93_), .b(new_n96_), .O(new_n148_));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(x20), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x28), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n146_), .c(new_n68_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x27), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n147_), .c(new_n77_), .O(z13));
  inv1   g094(.a(new_n147_), .O(new_n157_));
  aoi21  g095(.a(new_n153_), .b(new_n150_), .c(new_n95_), .O(new_n158_));
  nor3   g096(.a(new_n158_), .b(new_n157_), .c(new_n77_), .O(z14));
  inv1   g097(.a(x12), .O(new_n160_));
  nor2   g098(.a(new_n119_), .b(new_n160_), .O(z15));
  nand2  g099(.a(x22), .b(x01), .O(new_n162_));
  nor3   g100(.a(new_n162_), .b(new_n77_), .c(x23), .O(z16));
  inv1   g101(.a(x24), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x23), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n162_), .c(new_n101_), .O(z17));
  aoi21  g104(.a(new_n140_), .b(new_n136_), .c(new_n77_), .O(z18));
  nand2  g105(.a(new_n140_), .b(new_n136_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n101_), .O(z19));
  nor2   g107(.a(new_n137_), .b(new_n77_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n139_), .O(new_n171_));
  nor2   g109(.a(new_n171_), .b(new_n134_), .O(z20));
endmodule


