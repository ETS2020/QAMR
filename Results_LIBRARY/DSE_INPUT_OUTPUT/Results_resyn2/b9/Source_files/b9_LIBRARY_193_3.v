// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n168_, new_n169_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n183_, new_n184_;
  inv1   g000(.a(x05), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n66_), .c(new_n67_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g013(.a(x40), .b(x39), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(x05), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(new_n75_), .c(new_n66_), .d(new_n72_), .O(new_n78_));
  oai21  g016(.a(new_n71_), .b(new_n65_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x28), .O(new_n80_));
  aoi21  g018(.a(x35), .b(new_n80_), .c(x36), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  oai21  g020(.a(new_n81_), .b(new_n68_), .c(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g022(.a(x08), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n80_), .O(new_n86_));
  aoi21  g024(.a(new_n68_), .b(new_n85_), .c(new_n86_), .O(new_n87_));
  inv1   g025(.a(new_n77_), .O(new_n88_));
  and2   g026(.a(x29), .b(x08), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  oai21  g029(.a(new_n87_), .b(new_n72_), .c(new_n91_), .O(z02));
  nand2  g030(.a(x35), .b(x28), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n68_), .c(new_n65_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n94_), .c(new_n98_), .O(z04));
  inv1   g037(.a(new_n65_), .O(new_n100_));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n100_), .O(z05));
  inv1   g040(.a(new_n101_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n100_), .O(z06));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n66_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n105_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n100_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nand3  g052(.a(x40), .b(x39), .c(new_n63_), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n100_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  nand2  g057(.a(x37), .b(x06), .O(new_n120_));
  nand3  g058(.a(new_n76_), .b(x07), .c(new_n72_), .O(new_n121_));
  oai21  g059(.a(new_n121_), .b(new_n81_), .c(new_n120_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x27), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n100_), .O(z10));
  nand3  g062(.a(x40), .b(x39), .c(x29), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n86_), .O(new_n126_));
  nand2  g064(.a(new_n125_), .b(x27), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(x08), .O(new_n128_));
  nor2   g066(.a(x30), .b(x09), .O(new_n129_));
  inv1   g067(.a(x35), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(x28), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x27), .c(new_n72_), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(new_n129_), .c(new_n128_), .d(new_n100_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z11));
  inv1   g072(.a(x36), .O(new_n135_));
  nand3  g073(.a(new_n95_), .b(new_n135_), .c(new_n130_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x27), .c(x10), .O(new_n137_));
  nand2  g075(.a(new_n93_), .b(new_n95_), .O(new_n138_));
  inv1   g076(.a(x39), .O(new_n139_));
  nor2   g077(.a(new_n139_), .b(x04), .O(new_n140_));
  aoi21  g078(.a(new_n138_), .b(x27), .c(new_n140_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n137_), .c(new_n63_), .O(new_n142_));
  inv1   g080(.a(new_n70_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n64_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n142_), .O(z12));
  aoi21  g083(.a(x36), .b(x35), .c(new_n80_), .O(new_n146_));
  inv1   g084(.a(x13), .O(new_n147_));
  nand4  g085(.a(x40), .b(x39), .c(new_n147_), .d(new_n72_), .O(new_n148_));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x20), .O(new_n150_));
  oai22  g088(.a(new_n150_), .b(new_n69_), .c(new_n148_), .d(new_n146_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x27), .O(new_n152_));
  nand3  g090(.a(x39), .b(new_n147_), .c(new_n72_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n82_), .c(new_n63_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x40), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(z13));
  inv1   g094(.a(new_n148_), .O(new_n157_));
  oai21  g095(.a(x28), .b(new_n68_), .c(new_n82_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n143_), .O(new_n160_));
  nand2  g098(.a(x36), .b(x35), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n82_), .c(x28), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n148_), .c(new_n150_), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n160_), .c(new_n100_), .O(z14));
  inv1   g103(.a(x12), .O(new_n166_));
  nor2   g104(.a(new_n118_), .b(new_n166_), .O(z15));
  inv1   g105(.a(x23), .O(new_n168_));
  nand4  g106(.a(new_n100_), .b(new_n168_), .c(x22), .d(x01), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z16));
  inv1   g108(.a(x24), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(x23), .c(x22), .d(x01), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n100_), .O(z17));
  nand2  g111(.a(new_n68_), .b(new_n85_), .O(new_n174_));
  nand2  g112(.a(x27), .b(x04), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n174_), .c(new_n131_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n129_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n100_), .O(new_n178_));
  nand2  g116(.a(z08), .b(new_n89_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n178_), .O(z18));
  nand3  g118(.a(new_n132_), .b(new_n129_), .c(new_n128_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n100_), .O(z19));
  aoi21  g120(.a(new_n125_), .b(new_n86_), .c(new_n85_), .O(new_n183_));
  oai21  g121(.a(new_n130_), .b(new_n68_), .c(new_n129_), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n183_), .c(new_n100_), .O(z20));
endmodule


