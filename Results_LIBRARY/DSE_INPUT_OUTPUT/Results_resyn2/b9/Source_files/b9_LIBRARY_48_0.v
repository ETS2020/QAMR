// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n168_, new_n170_, new_n171_, new_n173_, new_n174_, new_n176_;
  nor2   g000(.a(x40), .b(x00), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  and2   g007(.a(x40), .b(x39), .O(z08));
  nand2  g008(.a(z08), .b(new_n69_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x35), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  nand3  g013(.a(new_n65_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x27), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n72_), .c(new_n64_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x16), .c(new_n63_), .O(z00));
  nor2   g018(.a(new_n74_), .b(x28), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(x36), .c(x27), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g023(.a(x40), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x00), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  inv1   g026(.a(x27), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n81_), .O(new_n91_));
  and2   g029(.a(x29), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x02), .c(x39), .O(new_n93_));
  aoi21  g031(.a(new_n91_), .b(x04), .c(new_n93_), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n86_), .c(new_n87_), .O(z02));
  inv1   g033(.a(x00), .O(new_n96_));
  nand2  g034(.a(new_n86_), .b(new_n96_), .O(new_n97_));
  nand2  g035(.a(new_n66_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n65_), .b(new_n89_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n65_), .b(new_n89_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n63_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n63_), .O(z05));
  and2   g042(.a(new_n103_), .b(new_n97_), .O(z06));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  inv1   g045(.a(x14), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n108_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(x38), .O(new_n110_));
  inv1   g048(.a(x17), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(x15), .O(new_n112_));
  or2    g050(.a(x33), .b(x31), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n112_), .c(x14), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n110_), .c(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n97_), .O(z07));
  inv1   g054(.a(x11), .O(new_n117_));
  nand4  g055(.a(new_n97_), .b(x34), .c(x27), .d(x26), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n117_), .O(z09));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  oai21  g058(.a(x40), .b(x00), .c(x07), .O(new_n121_));
  oai21  g059(.a(new_n121_), .b(z08), .c(new_n120_), .O(new_n122_));
  inv1   g060(.a(x28), .O(new_n123_));
  nand2  g061(.a(x35), .b(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n75_), .c(x04), .O(new_n125_));
  nand2  g063(.a(x37), .b(x06), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n63_), .O(new_n127_));
  aoi21  g065(.a(new_n125_), .b(new_n122_), .c(new_n127_), .O(new_n128_));
  inv1   g066(.a(x30), .O(new_n129_));
  inv1   g067(.a(x32), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g069(.a(new_n120_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n69_), .O(new_n133_));
  oai21  g071(.a(new_n128_), .b(new_n89_), .c(new_n133_), .O(z10));
  nand3  g072(.a(x40), .b(x39), .c(x29), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n124_), .c(new_n88_), .O(new_n136_));
  nand2  g074(.a(new_n135_), .b(x27), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand3  g076(.a(new_n81_), .b(x27), .c(new_n69_), .O(new_n139_));
  nor2   g077(.a(x30), .b(x09), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n97_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n139_), .c(new_n138_), .O(z18));
  inv1   g081(.a(z18), .O(z11));
  aoi21  g082(.a(new_n76_), .b(x27), .c(x10), .O(new_n145_));
  aoi22  g083(.a(new_n67_), .b(x27), .c(x39), .d(new_n69_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n145_), .c(x40), .O(new_n147_));
  nand3  g085(.a(new_n68_), .b(new_n86_), .c(x00), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(z12));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x28), .O(new_n151_));
  nor2   g089(.a(x13), .b(x04), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n151_), .c(z08), .O(new_n153_));
  nor2   g091(.a(x19), .b(x18), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x20), .O(new_n155_));
  nand2  g093(.a(new_n67_), .b(new_n97_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n155_), .c(new_n153_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  nand3  g096(.a(new_n152_), .b(new_n131_), .c(z08), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z13));
  nand2  g098(.a(new_n152_), .b(z08), .O(new_n161_));
  nand3  g099(.a(new_n150_), .b(new_n83_), .c(x28), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n161_), .c(new_n155_), .O(new_n164_));
  aoi21  g102(.a(new_n123_), .b(x27), .c(new_n131_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n161_), .c(new_n68_), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n164_), .c(new_n63_), .O(z14));
  inv1   g105(.a(x12), .O(new_n168_));
  nor2   g106(.a(new_n118_), .b(new_n168_), .O(z15));
  inv1   g107(.a(x23), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(x22), .c(x01), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n97_), .O(z16));
  nor2   g110(.a(x24), .b(new_n170_), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(new_n97_), .c(x22), .d(x01), .O(new_n174_));
  inv1   g112(.a(new_n174_), .O(z17));
  nor2   g113(.a(new_n74_), .b(new_n89_), .O(new_n176_));
  nor3   g114(.a(new_n176_), .b(new_n141_), .c(new_n136_), .O(z20));
  inv1   g115(.a(z18), .O(z19));
endmodule


