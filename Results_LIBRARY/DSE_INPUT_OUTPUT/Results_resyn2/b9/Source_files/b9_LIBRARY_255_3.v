// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n153_,
    new_n155_, new_n157_, new_n160_, new_n162_;
  nand2  g000(.a(x33), .b(x32), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  and2   g009(.a(x40), .b(x39), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(new_n71_), .c(new_n68_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n67_), .c(new_n66_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x10), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n65_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n66_), .c(new_n83_), .O(new_n84_));
  nand3  g022(.a(new_n84_), .b(new_n63_), .c(x04), .O(z01));
  nand2  g023(.a(x35), .b(new_n81_), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n86_), .c(x04), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n75_), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n88_), .c(new_n64_), .O(z02));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(x28), .b(x27), .O(new_n93_));
  oai22  g031(.a(new_n93_), .b(new_n69_), .c(new_n92_), .d(x27), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n63_), .c(new_n96_), .O(z04));
  aoi21  g035(.a(new_n93_), .b(new_n92_), .c(new_n64_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n63_), .O(z07));
  nand2  g048(.a(new_n75_), .b(new_n63_), .O(z08));
  nand2  g049(.a(x34), .b(x27), .O(new_n112_));
  nand2  g050(.a(x26), .b(x11), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n112_), .c(new_n63_), .O(z09));
  nor2   g052(.a(new_n64_), .b(new_n92_), .O(new_n115_));
  inv1   g053(.a(x07), .O(new_n116_));
  inv1   g054(.a(x32), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x05), .O(new_n118_));
  oai22  g056(.a(new_n118_), .b(new_n75_), .c(z08), .d(new_n116_), .O(new_n119_));
  nor2   g057(.a(new_n82_), .b(x04), .O(new_n120_));
  aoi22  g058(.a(new_n120_), .b(new_n119_), .c(new_n115_), .d(x06), .O(new_n121_));
  nor3   g059(.a(new_n83_), .b(new_n75_), .c(x04), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n63_), .c(x05), .O(new_n123_));
  oai21  g061(.a(new_n121_), .b(new_n66_), .c(new_n123_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand2  g063(.a(new_n72_), .b(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n86_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nor2   g067(.a(new_n69_), .b(new_n66_), .O(new_n130_));
  nor2   g068(.a(x28), .b(x04), .O(new_n131_));
  inv1   g069(.a(x09), .O(new_n132_));
  inv1   g070(.a(x30), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  aoi21  g072(.a(new_n131_), .b(new_n130_), .c(new_n134_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n129_), .c(new_n63_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(z11));
  inv1   g075(.a(new_n74_), .O(new_n138_));
  nand3  g076(.a(new_n77_), .b(new_n138_), .c(new_n63_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  oai21  g079(.a(new_n70_), .b(new_n69_), .c(x28), .O(new_n142_));
  nand4  g080(.a(new_n142_), .b(new_n72_), .c(new_n141_), .d(new_n68_), .O(new_n143_));
  inv1   g081(.a(new_n67_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n144_), .c(x20), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n143_), .c(new_n66_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  nand2  g086(.a(new_n122_), .b(new_n141_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n63_), .O(z13));
  inv1   g088(.a(new_n149_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n147_), .c(new_n63_), .O(z14));
  nand3  g090(.a(new_n63_), .b(x26), .c(x12), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(new_n112_), .O(z15));
  nand2  g092(.a(x22), .b(x01), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(x23), .c(new_n63_), .O(z16));
  inv1   g094(.a(x23), .O(new_n157_));
  nor4   g095(.a(new_n155_), .b(new_n64_), .c(x24), .d(new_n157_), .O(z17));
  aoi21  g096(.a(new_n135_), .b(new_n129_), .c(new_n64_), .O(z18));
  nand2  g097(.a(new_n135_), .b(new_n129_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n63_), .O(z19));
  oai21  g099(.a(new_n69_), .b(new_n66_), .c(new_n63_), .O(new_n162_));
  nor3   g100(.a(new_n162_), .b(new_n134_), .c(new_n127_), .O(z20));
endmodule


