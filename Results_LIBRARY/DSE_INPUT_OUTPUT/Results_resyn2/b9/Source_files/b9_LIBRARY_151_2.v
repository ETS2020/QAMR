// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n172_, new_n174_,
    new_n175_, new_n177_, new_n178_, new_n179_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n187_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x35), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(x33), .O(new_n65_));
  nand2  g003(.a(x37), .b(x27), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n67_), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n66_), .c(x15), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n63_), .c(new_n65_), .O(new_n70_));
  and2   g008(.a(x40), .b(x39), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand2  g010(.a(x35), .b(x28), .O(new_n73_));
  inv1   g011(.a(x33), .O(new_n74_));
  aoi21  g012(.a(x36), .b(new_n74_), .c(x35), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n72_), .c(new_n73_), .O(new_n76_));
  inv1   g014(.a(x27), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x15), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n70_), .O(z00));
  nor2   g018(.a(x35), .b(new_n74_), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(x36), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(new_n83_), .c(new_n77_), .O(new_n86_));
  inv1   g024(.a(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n82_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(x04), .c(new_n81_), .O(z01));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(x28), .c(x04), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g032(.a(x33), .b(x04), .c(new_n64_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n94_), .c(new_n91_), .d(new_n71_), .O(z02));
  nand2  g034(.a(new_n73_), .b(x27), .O(new_n97_));
  nor2   g035(.a(x37), .b(x27), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n81_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n97_), .c(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand3  g039(.a(new_n99_), .b(new_n97_), .c(new_n101_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n65_), .O(z05));
  inv1   g042(.a(new_n103_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n65_), .O(z06));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n109_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(x38), .O(new_n111_));
  inv1   g049(.a(x17), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(x15), .O(new_n113_));
  inv1   g051(.a(x31), .O(new_n114_));
  nand2  g052(.a(new_n74_), .b(new_n114_), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n113_), .c(x14), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n111_), .c(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n65_), .O(z07));
  nand2  g056(.a(x40), .b(x39), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n81_), .O(z08));
  nand3  g058(.a(x34), .b(x27), .c(x26), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n65_), .c(x11), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(z09));
  inv1   g062(.a(x05), .O(new_n125_));
  nor3   g063(.a(new_n82_), .b(new_n119_), .c(new_n125_), .O(new_n126_));
  aoi21  g064(.a(x40), .b(x39), .c(x07), .O(new_n127_));
  aoi21  g065(.a(new_n71_), .b(new_n125_), .c(new_n127_), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n86_), .c(new_n126_), .O(new_n129_));
  inv1   g067(.a(new_n66_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(x06), .c(new_n81_), .O(new_n131_));
  oai21  g069(.a(new_n129_), .b(x04), .c(new_n131_), .O(z10));
  inv1   g070(.a(x08), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  nor2   g072(.a(new_n64_), .b(x28), .O(new_n135_));
  nand2  g073(.a(x27), .b(x04), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand3  g075(.a(new_n135_), .b(x27), .c(new_n67_), .O(new_n138_));
  aoi22  g076(.a(new_n138_), .b(new_n133_), .c(new_n137_), .d(new_n134_), .O(new_n139_));
  inv1   g077(.a(x09), .O(new_n140_));
  inv1   g078(.a(x30), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n139_), .c(new_n65_), .O(z11));
  inv1   g081(.a(x10), .O(new_n144_));
  nor3   g082(.a(x37), .b(x36), .c(x35), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n77_), .c(new_n144_), .O(new_n146_));
  aoi21  g084(.a(x35), .b(x28), .c(x37), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n77_), .c(new_n72_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n146_), .c(new_n65_), .O(z12));
  nor2   g087(.a(x13), .b(x04), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(x40), .c(x39), .O(new_n151_));
  nand2  g089(.a(x36), .b(x35), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(x28), .c(new_n151_), .O(new_n153_));
  nor2   g091(.a(x19), .b(x18), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x20), .O(new_n155_));
  nor2   g093(.a(new_n155_), .b(new_n147_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n153_), .c(x27), .O(new_n157_));
  inv1   g095(.a(new_n82_), .O(new_n158_));
  nand3  g096(.a(new_n150_), .b(new_n158_), .c(new_n71_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n157_), .c(new_n65_), .O(z13));
  inv1   g098(.a(x32), .O(new_n161_));
  nand4  g099(.a(new_n152_), .b(new_n161_), .c(new_n141_), .d(x28), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n150_), .c(new_n71_), .O(new_n163_));
  aoi22  g101(.a(new_n163_), .b(new_n155_), .c(new_n159_), .d(new_n77_), .O(new_n164_));
  nand2  g102(.a(new_n151_), .b(new_n65_), .O(new_n165_));
  nand4  g103(.a(new_n74_), .b(new_n161_), .c(new_n141_), .d(x28), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n147_), .O(new_n168_));
  oai21  g106(.a(new_n164_), .b(new_n81_), .c(new_n168_), .O(z14));
  nand3  g107(.a(new_n122_), .b(new_n65_), .c(x12), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z15));
  nand2  g109(.a(x22), .b(x01), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(x23), .c(new_n65_), .O(z16));
  inv1   g111(.a(x24), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x23), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n172_), .c(new_n65_), .O(z17));
  nand3  g114(.a(new_n71_), .b(x29), .c(x08), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n142_), .c(new_n65_), .O(new_n179_));
  oai21  g117(.a(new_n137_), .b(new_n90_), .c(new_n179_), .O(z18));
  aoi21  g118(.a(new_n134_), .b(new_n85_), .c(new_n133_), .O(new_n181_));
  nand2  g119(.a(new_n134_), .b(x27), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nor2   g121(.a(new_n142_), .b(new_n81_), .O(new_n184_));
  nand3  g122(.a(new_n184_), .b(new_n183_), .c(new_n138_), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n65_), .O(z19));
  oai21  g124(.a(new_n64_), .b(new_n77_), .c(new_n184_), .O(new_n187_));
  nor2   g125(.a(new_n187_), .b(new_n181_), .O(z20));
endmodule


