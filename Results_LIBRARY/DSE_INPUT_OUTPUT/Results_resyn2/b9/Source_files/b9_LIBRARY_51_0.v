// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n175_;
  nor2   g000(.a(x40), .b(x17), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n64_), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  nor2   g009(.a(x36), .b(x35), .O(new_n72_));
  oai21  g010(.a(new_n72_), .b(new_n66_), .c(new_n71_), .O(new_n73_));
  and2   g011(.a(x40), .b(x39), .O(new_n74_));
  nand4  g012(.a(new_n74_), .b(new_n73_), .c(new_n64_), .d(new_n70_), .O(new_n75_));
  oai21  g013(.a(new_n69_), .b(new_n63_), .c(new_n75_), .O(z00));
  inv1   g014(.a(x36), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n77_), .c(new_n66_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g021(.a(new_n63_), .b(new_n70_), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n80_), .c(new_n84_), .O(z01));
  inv1   g023(.a(x08), .O(new_n86_));
  aoi21  g024(.a(new_n66_), .b(new_n86_), .c(new_n79_), .O(new_n87_));
  nand2  g025(.a(x40), .b(x39), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g028(.a(new_n87_), .b(new_n70_), .c(new_n90_), .O(z02));
  inv1   g029(.a(x35), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n92_), .c(new_n93_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n63_), .O(z03));
  nor2   g034(.a(new_n63_), .b(x21), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n95_), .O(z04));
  nor2   g036(.a(new_n63_), .b(x37), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n94_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n64_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  inv1   g043(.a(new_n63_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n102_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n106_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n105_), .O(z07));
  nand2  g050(.a(new_n88_), .b(new_n106_), .O(z08));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n106_), .O(z09));
  nor2   g053(.a(x32), .b(x30), .O(new_n116_));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  aoi21  g057(.a(x40), .b(x39), .c(x07), .O(new_n120_));
  aoi21  g058(.a(new_n74_), .b(new_n119_), .c(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n80_), .c(new_n118_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n63_), .O(new_n125_));
  oai21  g063(.a(new_n122_), .b(x04), .c(new_n125_), .O(z10));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n79_), .c(new_n86_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nand4  g069(.a(x35), .b(new_n78_), .c(x27), .d(new_n70_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n106_), .O(z11));
  nand3  g072(.a(x40), .b(x39), .c(new_n70_), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n72_), .c(new_n67_), .O(new_n136_));
  nor2   g074(.a(new_n88_), .b(x04), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x10), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n106_), .O(new_n139_));
  aoi21  g077(.a(new_n136_), .b(x27), .c(new_n139_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  nand2  g079(.a(x36), .b(x35), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x28), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n137_), .c(new_n141_), .O(new_n144_));
  inv1   g082(.a(new_n67_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n145_), .c(x20), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n144_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  nand3  g087(.a(new_n74_), .b(new_n141_), .c(new_n70_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n83_), .c(new_n63_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n149_), .O(z13));
  nand2  g091(.a(new_n146_), .b(x20), .O(new_n154_));
  nand3  g092(.a(new_n142_), .b(new_n116_), .c(x28), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n150_), .c(new_n154_), .O(new_n157_));
  aoi21  g095(.a(new_n78_), .b(x27), .c(new_n83_), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n150_), .c(new_n67_), .d(new_n66_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n157_), .c(new_n106_), .O(z14));
  nand4  g098(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n106_), .O(z15));
  inv1   g100(.a(x23), .O(new_n163_));
  and2   g101(.a(x22), .b(x01), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n163_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n106_), .O(z16));
  nor2   g104(.a(x24), .b(new_n163_), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n164_), .c(new_n106_), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z17));
  inv1   g107(.a(new_n131_), .O(new_n170_));
  nand2  g108(.a(x27), .b(x04), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n87_), .c(new_n170_), .O(new_n172_));
  nand3  g110(.a(new_n74_), .b(x29), .c(x08), .O(new_n173_));
  oai21  g111(.a(new_n172_), .b(new_n63_), .c(new_n173_), .O(z18));
  oai21  g112(.a(new_n92_), .b(new_n66_), .c(new_n131_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n128_), .c(new_n106_), .O(z20));
  nand2  g114(.a(new_n133_), .b(new_n106_), .O(z19));
endmodule


