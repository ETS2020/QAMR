// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_, new_n96_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n148_, new_n150_, new_n152_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n162_, new_n163_, new_n164_;
  inv1   g000(.a(x16), .O(new_n63_));
  nor2   g001(.a(x40), .b(x27), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(x36), .b(x35), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  nand3  g007(.a(x40), .b(x39), .c(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(new_n71_));
  nand4  g009(.a(x40), .b(x39), .c(x10), .d(new_n69_), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  aoi21  g011(.a(new_n71_), .b(x27), .c(new_n73_), .O(z12));
  oai21  g012(.a(z12), .b(x15), .c(new_n66_), .O(z00));
  inv1   g013(.a(x27), .O(new_n76_));
  nor2   g014(.a(x32), .b(x30), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  aoi21  g016(.a(x35), .b(new_n78_), .c(x36), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n76_), .c(new_n77_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x04), .c(new_n64_), .O(z01));
  nand2  g019(.a(x35), .b(new_n78_), .O(new_n82_));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  xor2a  g026(.a(new_n88_), .b(new_n64_), .O(z02));
  nand2  g027(.a(x35), .b(x28), .O(new_n90_));
  nor2   g028(.a(x37), .b(x27), .O(new_n91_));
  aoi22  g029(.a(new_n91_), .b(x40), .c(new_n90_), .d(x27), .O(new_n92_));
  oai21  g030(.a(new_n64_), .b(x21), .c(new_n92_), .O(z03));
  nand2  g031(.a(new_n65_), .b(x21), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n92_), .O(z04));
  inv1   g033(.a(x37), .O(new_n96_));
  oai22  g034(.a(new_n64_), .b(new_n96_), .c(new_n78_), .d(new_n76_), .O(z05));
  nor2   g035(.a(new_n78_), .b(new_n76_), .O(new_n98_));
  nor3   g036(.a(new_n98_), .b(new_n64_), .c(x37), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n65_), .O(z07));
  inv1   g048(.a(new_n85_), .O(z08));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z09));
  inv1   g051(.a(x36), .O(new_n114_));
  aoi21  g052(.a(new_n82_), .b(new_n114_), .c(new_n76_), .O(new_n115_));
  aoi21  g053(.a(new_n115_), .b(x07), .c(z08), .O(new_n116_));
  inv1   g054(.a(x05), .O(new_n117_));
  aoi21  g055(.a(z08), .b(new_n117_), .c(x04), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n80_), .O(new_n119_));
  nand3  g057(.a(x37), .b(x27), .c(x06), .O(new_n120_));
  oai21  g058(.a(new_n119_), .b(new_n116_), .c(new_n120_), .O(z10));
  or2    g059(.a(x30), .b(x09), .O(new_n122_));
  nand3  g060(.a(z08), .b(x29), .c(x08), .O(new_n123_));
  inv1   g061(.a(new_n82_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n69_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n123_), .c(x27), .O(new_n126_));
  inv1   g064(.a(new_n83_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand3  g066(.a(new_n127_), .b(x39), .c(x29), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(x40), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n126_), .c(new_n122_), .O(z11));
  inv1   g069(.a(x13), .O(new_n132_));
  nor2   g070(.a(new_n85_), .b(x04), .O(new_n133_));
  nand2  g071(.a(x36), .b(x35), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x28), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n133_), .c(new_n132_), .O(new_n136_));
  inv1   g074(.a(new_n67_), .O(new_n137_));
  nor2   g075(.a(x19), .b(x18), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(x20), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x27), .O(new_n141_));
  inv1   g079(.a(new_n77_), .O(new_n142_));
  nand2  g080(.a(new_n133_), .b(new_n132_), .O(new_n143_));
  inv1   g081(.a(new_n143_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n141_), .O(z13));
  aoi22  g084(.a(new_n140_), .b(x27), .c(new_n144_), .d(new_n142_), .O(z14));
  nand4  g085(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(z15));
  nand2  g087(.a(x22), .b(x01), .O(new_n150_));
  nor3   g088(.a(new_n150_), .b(new_n64_), .c(x23), .O(z16));
  inv1   g089(.a(x23), .O(new_n152_));
  nor2   g090(.a(x24), .b(new_n152_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(new_n65_), .c(x22), .d(x01), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(z17));
  nor2   g093(.a(new_n83_), .b(new_n82_), .O(new_n156_));
  nand2  g094(.a(x27), .b(x04), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  nor2   g096(.a(new_n122_), .b(new_n64_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n158_), .c(new_n123_), .O(z18));
  inv1   g098(.a(z18), .O(z19));
  nand2  g099(.a(new_n159_), .b(new_n123_), .O(new_n162_));
  inv1   g100(.a(x08), .O(new_n163_));
  oai21  g101(.a(x28), .b(new_n163_), .c(new_n76_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(x35), .c(new_n162_), .O(z20));
endmodule


