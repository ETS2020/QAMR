// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n173_, new_n175_, new_n177_, new_n178_, new_n180_, new_n181_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n187_, new_n189_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x05), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g012(.a(x40), .b(x39), .c(x05), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(new_n74_), .c(new_n65_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n70_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x35), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  oai21  g018(.a(new_n79_), .b(x28), .c(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x27), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g023(.a(x08), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  aoi21  g026(.a(new_n67_), .b(new_n86_), .c(new_n88_), .O(new_n89_));
  and2   g027(.a(x29), .b(x08), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(x02), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n75_), .O(new_n92_));
  oai21  g030(.a(new_n89_), .b(new_n71_), .c(new_n92_), .O(z02));
  nand2  g031(.a(x35), .b(x28), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n67_), .c(new_n64_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n95_), .c(new_n99_), .O(z04));
  inv1   g038(.a(new_n64_), .O(new_n101_));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n101_), .O(z05));
  inv1   g041(.a(new_n102_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n101_), .O(z06));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n65_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n106_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n106_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n101_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  nand3  g053(.a(x40), .b(x39), .c(x05), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(z08));
  nand3  g055(.a(x34), .b(x27), .c(x26), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n101_), .c(x11), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(z09));
  inv1   g059(.a(x30), .O(new_n122_));
  inv1   g060(.a(x32), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(x40), .c(x39), .O(new_n125_));
  inv1   g063(.a(x07), .O(new_n126_));
  nand2  g064(.a(x40), .b(x39), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n81_), .c(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n71_), .O(new_n131_));
  nand3  g069(.a(x37), .b(x27), .c(x06), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n64_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n131_), .O(z10));
  nand3  g073(.a(x40), .b(x39), .c(x29), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n88_), .c(new_n86_), .O(new_n137_));
  nand2  g075(.a(new_n136_), .b(x27), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nor2   g077(.a(new_n79_), .b(x28), .O(new_n140_));
  nor2   g078(.a(x30), .b(x09), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(new_n142_));
  nor2   g080(.a(new_n67_), .b(x04), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n140_), .c(new_n142_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n139_), .c(new_n101_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(z11));
  nand3  g084(.a(new_n96_), .b(new_n80_), .c(new_n79_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(x10), .O(new_n148_));
  nand2  g086(.a(new_n94_), .b(new_n96_), .O(new_n149_));
  aoi22  g087(.a(new_n149_), .b(x27), .c(x39), .d(new_n71_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n148_), .c(x05), .O(new_n151_));
  nand2  g089(.a(new_n149_), .b(x27), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n63_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n151_), .O(z12));
  aoi21  g092(.a(x36), .b(x35), .c(new_n87_), .O(new_n155_));
  inv1   g093(.a(x13), .O(new_n156_));
  nand4  g094(.a(x40), .b(x39), .c(new_n156_), .d(new_n71_), .O(new_n157_));
  nor2   g095(.a(x19), .b(x18), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x20), .O(new_n159_));
  oai22  g097(.a(new_n159_), .b(new_n68_), .c(new_n157_), .d(new_n155_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nand3  g099(.a(x39), .b(new_n156_), .c(new_n71_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n83_), .c(x05), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x40), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(z13));
  aoi21  g103(.a(new_n87_), .b(x27), .c(new_n124_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n157_), .c(new_n152_), .O(new_n167_));
  nand2  g105(.a(x36), .b(x35), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n83_), .c(x28), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n157_), .c(new_n159_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n167_), .c(new_n101_), .O(z14));
  nand3  g110(.a(new_n119_), .b(new_n101_), .c(x12), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(z15));
  nand2  g112(.a(x22), .b(x01), .O(new_n175_));
  nor3   g113(.a(new_n175_), .b(new_n64_), .c(x23), .O(z16));
  inv1   g114(.a(x24), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(x23), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n175_), .c(new_n101_), .O(z17));
  nand2  g117(.a(new_n67_), .b(new_n86_), .O(new_n180_));
  nand2  g118(.a(x27), .b(x04), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n180_), .c(new_n140_), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n141_), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n101_), .O(new_n184_));
  nand2  g122(.a(z08), .b(new_n90_), .O(new_n185_));
  nand2  g123(.a(new_n185_), .b(new_n184_), .O(z18));
  nand2  g124(.a(new_n144_), .b(new_n139_), .O(new_n187_));
  nand2  g125(.a(new_n187_), .b(new_n101_), .O(z19));
  oai21  g126(.a(new_n79_), .b(new_n67_), .c(new_n141_), .O(new_n189_));
  oai21  g127(.a(new_n189_), .b(new_n137_), .c(new_n101_), .O(z20));
endmodule


