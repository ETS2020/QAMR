// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n142_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n154_,
    new_n156_, new_n158_, new_n159_, new_n161_, new_n162_, new_n164_,
    new_n166_;
  inv1   g000(.a(x22), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x10), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nand2  g006(.a(x40), .b(x39), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x04), .c(new_n68_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  nand3  g011(.a(new_n65_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(x27), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n71_), .c(x15), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n70_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g016(.a(x27), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(new_n73_), .c(new_n79_), .O(new_n82_));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g023(.a(x04), .O(new_n86_));
  nor2   g024(.a(new_n64_), .b(new_n86_), .O(new_n87_));
  oai21  g025(.a(new_n85_), .b(new_n82_), .c(new_n87_), .O(z01));
  inv1   g026(.a(new_n64_), .O(new_n89_));
  inv1   g027(.a(new_n69_), .O(new_n90_));
  nor2   g028(.a(x27), .b(x08), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n81_), .c(x04), .O(new_n92_));
  inv1   g030(.a(x02), .O(new_n93_));
  nand2  g031(.a(x29), .b(x08), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n92_), .c(new_n90_), .d(new_n89_), .O(z02));
  nand2  g034(.a(new_n66_), .b(x27), .O(new_n97_));
  aoi21  g035(.a(new_n65_), .b(new_n79_), .c(new_n64_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n97_), .c(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n97_), .c(new_n100_), .O(z04));
  oai21  g039(.a(new_n80_), .b(new_n79_), .c(new_n65_), .O(new_n102_));
  and2   g040(.a(new_n102_), .b(new_n89_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x14), .O(new_n105_));
  inv1   g043(.a(x15), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n106_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n105_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n89_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  nand2  g053(.a(new_n69_), .b(new_n89_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n89_), .O(z09));
  nand2  g056(.a(new_n81_), .b(new_n73_), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  nand2  g058(.a(new_n69_), .b(x07), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n119_), .c(x27), .O(new_n123_));
  inv1   g061(.a(new_n120_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n85_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n86_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n127_), .c(new_n64_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  nand3  g068(.a(x40), .b(x39), .c(x29), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n81_), .c(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n131_), .b(x27), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  inv1   g072(.a(new_n81_), .O(new_n135_));
  nor2   g073(.a(new_n79_), .b(x04), .O(new_n136_));
  inv1   g074(.a(x09), .O(new_n137_));
  nand2  g075(.a(new_n83_), .b(new_n137_), .O(new_n138_));
  aoi21  g076(.a(new_n136_), .b(new_n135_), .c(new_n138_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(new_n134_), .c(new_n64_), .O(z18));
  inv1   g078(.a(z18), .O(z11));
  oai21  g079(.a(new_n75_), .b(x22), .c(new_n71_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n70_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand3  g082(.a(new_n90_), .b(new_n144_), .c(new_n86_), .O(new_n145_));
  aoi21  g083(.a(x36), .b(x35), .c(new_n80_), .O(new_n146_));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n67_), .c(x20), .O(new_n148_));
  oai21  g086(.a(new_n146_), .b(new_n145_), .c(new_n148_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  inv1   g088(.a(new_n145_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n85_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n150_), .c(new_n89_), .O(z13));
  nand2  g091(.a(new_n152_), .b(new_n89_), .O(new_n154_));
  aoi21  g092(.a(new_n149_), .b(x27), .c(new_n154_), .O(z14));
  nand4  g093(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n89_), .O(z15));
  inv1   g095(.a(x23), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x01), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x10), .c(new_n63_), .O(z16));
  inv1   g098(.a(x24), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(x23), .c(x01), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(x10), .c(new_n63_), .O(z17));
  nand3  g101(.a(new_n139_), .b(new_n134_), .c(new_n89_), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z19));
  nor2   g103(.a(new_n72_), .b(new_n79_), .O(new_n166_));
  nor4   g104(.a(new_n166_), .b(new_n138_), .c(new_n132_), .d(new_n64_), .O(z20));
endmodule


