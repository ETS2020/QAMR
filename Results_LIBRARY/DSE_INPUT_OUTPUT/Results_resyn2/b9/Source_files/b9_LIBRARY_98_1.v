// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n173_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand2  g002(.a(x40), .b(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand3  g005(.a(x40), .b(x10), .c(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x39), .O(new_n71_));
  nand2  g009(.a(x40), .b(new_n71_), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x27), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n72_), .c(x04), .O(z01));
  aoi21  g019(.a(x29), .b(x08), .c(x02), .O(new_n82_));
  nor2   g020(.a(new_n75_), .b(x28), .O(new_n83_));
  inv1   g021(.a(x08), .O(new_n84_));
  inv1   g022(.a(x27), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n82_), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n71_), .c(x40), .O(z02));
  inv1   g027(.a(x28), .O(new_n90_));
  oai21  g028(.a(new_n75_), .b(new_n90_), .c(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n91_), .c(new_n72_), .d(x21), .O(z03));
  inv1   g032(.a(new_n72_), .O(new_n95_));
  aoi21  g033(.a(new_n92_), .b(new_n85_), .c(x21), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n91_), .c(new_n95_), .O(z04));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n72_), .c(new_n92_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n72_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  and2   g049(.a(x34), .b(x27), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(x26), .c(x11), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n72_), .O(z09));
  nand2  g052(.a(x39), .b(x05), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(x40), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  inv1   g055(.a(x40), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n117_), .c(x04), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n116_), .c(new_n77_), .O(new_n120_));
  nand3  g058(.a(new_n72_), .b(x37), .c(x06), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x27), .O(new_n123_));
  nor2   g061(.a(new_n79_), .b(new_n65_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(x39), .c(x05), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(z10));
  nand2  g064(.a(x35), .b(new_n90_), .O(new_n127_));
  nand2  g065(.a(x40), .b(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n127_), .c(new_n84_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nor2   g069(.a(new_n85_), .b(x04), .O(new_n132_));
  nor2   g070(.a(x30), .b(x09), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(new_n134_));
  aoi21  g072(.a(new_n132_), .b(new_n83_), .c(new_n134_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n131_), .c(new_n72_), .O(z18));
  inv1   g074(.a(z18), .O(z11));
  nor2   g075(.a(new_n66_), .b(new_n85_), .O(new_n138_));
  inv1   g076(.a(x10), .O(new_n139_));
  oai21  g077(.a(new_n63_), .b(new_n85_), .c(new_n139_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n64_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x39), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x40), .c(new_n138_), .O(z12));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand3  g083(.a(x20), .b(new_n145_), .c(new_n144_), .O(new_n146_));
  aoi21  g084(.a(x36), .b(x35), .c(new_n90_), .O(new_n147_));
  nor2   g085(.a(x13), .b(x04), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x40), .O(new_n149_));
  oai22  g087(.a(new_n149_), .b(new_n147_), .c(new_n146_), .d(new_n66_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  nor3   g089(.a(new_n79_), .b(x13), .c(x04), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n71_), .c(x40), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n151_), .O(z13));
  nand2  g092(.a(new_n92_), .b(new_n75_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(x20), .c(new_n145_), .d(new_n144_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n147_), .c(new_n85_), .O(new_n157_));
  nand2  g095(.a(new_n79_), .b(x39), .O(new_n158_));
  oai21  g096(.a(new_n148_), .b(new_n71_), .c(x40), .O(new_n159_));
  inv1   g097(.a(new_n146_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n138_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n159_), .O(new_n162_));
  oai21  g100(.a(new_n158_), .b(new_n157_), .c(new_n162_), .O(z14));
  nand4  g101(.a(new_n112_), .b(new_n72_), .c(x26), .d(x12), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(x23), .c(new_n72_), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x23), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n166_), .c(new_n72_), .O(z17));
  nand2  g108(.a(new_n135_), .b(new_n131_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n72_), .O(z19));
  oai21  g110(.a(new_n75_), .b(new_n85_), .c(new_n133_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n129_), .c(new_n72_), .O(z20));
  buf    g112(.a(x40), .O(z08));
endmodule


