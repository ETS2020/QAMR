// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n93_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n157_, new_n159_, new_n161_,
    new_n162_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n171_, new_n172_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(z08));
  nand3  g007(.a(z08), .b(x10), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n67_), .b(x27), .c(new_n71_), .O(new_n72_));
  inv1   g010(.a(x16), .O(new_n73_));
  inv1   g011(.a(x20), .O(new_n74_));
  nor2   g012(.a(x40), .b(new_n74_), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  oai21  g014(.a(new_n72_), .b(x15), .c(new_n76_), .O(z00));
  inv1   g015(.a(x35), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(x28), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(x36), .c(x27), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x04), .c(new_n75_), .O(z01));
  oai21  g021(.a(x27), .b(x08), .c(new_n79_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x04), .O(new_n85_));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n68_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n85_), .O(z02));
  inv1   g026(.a(x37), .O(new_n89_));
  nand2  g027(.a(x28), .b(x27), .O(new_n90_));
  oai22  g028(.a(new_n90_), .b(new_n78_), .c(new_n89_), .d(x27), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x21), .c(new_n75_), .O(z03));
  inv1   g030(.a(x21), .O(new_n93_));
  aoi21  g031(.a(new_n91_), .b(new_n93_), .c(new_n75_), .O(z04));
  aoi21  g032(.a(new_n90_), .b(new_n89_), .c(new_n75_), .O(z05));
  nand2  g033(.a(new_n90_), .b(new_n89_), .O(new_n96_));
  nor2   g034(.a(new_n96_), .b(new_n75_), .O(z06));
  inv1   g035(.a(new_n75_), .O(new_n98_));
  inv1   g036(.a(x25), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(x00), .O(new_n100_));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x25), .b(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n100_), .c(x38), .O(new_n103_));
  inv1   g041(.a(x17), .O(new_n104_));
  nor2   g042(.a(new_n104_), .b(x15), .O(new_n105_));
  or2    g043(.a(x33), .b(x31), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(new_n105_), .c(x14), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n103_), .c(x03), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n98_), .O(z07));
  and2   g047(.a(x34), .b(x27), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(x26), .c(x11), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n98_), .O(z09));
  inv1   g050(.a(x27), .O(new_n113_));
  nand3  g051(.a(x40), .b(x39), .c(x05), .O(new_n114_));
  nand2  g052(.a(new_n68_), .b(x07), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n75_), .c(new_n114_), .O(new_n116_));
  inv1   g054(.a(x36), .O(new_n117_));
  inv1   g055(.a(x28), .O(new_n118_));
  nand2  g056(.a(x35), .b(new_n118_), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(new_n117_), .c(x04), .O(new_n120_));
  nand2  g058(.a(x37), .b(x06), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n75_), .O(new_n122_));
  aoi21  g060(.a(new_n120_), .b(new_n116_), .c(new_n122_), .O(new_n123_));
  inv1   g061(.a(new_n81_), .O(new_n124_));
  inv1   g062(.a(new_n114_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n124_), .c(new_n65_), .O(new_n126_));
  oai21  g064(.a(new_n123_), .b(new_n113_), .c(new_n126_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n119_), .c(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g070(.a(new_n79_), .b(x27), .c(new_n65_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n133_), .c(new_n132_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n98_), .O(z11));
  nand2  g074(.a(new_n67_), .b(x27), .O(new_n137_));
  nand2  g075(.a(new_n70_), .b(new_n137_), .O(new_n138_));
  nand2  g076(.a(new_n98_), .b(new_n138_), .O(z12));
  inv1   g077(.a(x40), .O(new_n140_));
  inv1   g078(.a(x13), .O(new_n141_));
  nand2  g079(.a(x36), .b(x35), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x28), .O(new_n143_));
  nand4  g081(.a(new_n143_), .b(x39), .c(new_n141_), .d(new_n65_), .O(new_n144_));
  nand2  g082(.a(x35), .b(x28), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n89_), .O(new_n146_));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n146_), .c(x20), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  nand3  g088(.a(x39), .b(new_n141_), .c(new_n65_), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(new_n81_), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n150_), .c(new_n140_), .O(z13));
  aoi21  g092(.a(new_n148_), .b(new_n144_), .c(new_n113_), .O(new_n155_));
  oai21  g093(.a(new_n152_), .b(new_n155_), .c(x40), .O(z14));
  nand4  g094(.a(new_n110_), .b(new_n98_), .c(x26), .d(x12), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z15));
  nand2  g096(.a(x22), .b(x01), .O(new_n159_));
  nor3   g097(.a(new_n159_), .b(new_n75_), .c(x23), .O(z16));
  inv1   g098(.a(x24), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x23), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n159_), .c(new_n98_), .O(z17));
  nand4  g101(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(new_n98_), .O(z18));
  nor2   g102(.a(new_n113_), .b(new_n65_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n84_), .c(new_n134_), .O(new_n166_));
  inv1   g104(.a(new_n129_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x08), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  aoi21  g107(.a(new_n166_), .b(new_n98_), .c(new_n169_), .O(z19));
  nand2  g108(.a(x35), .b(x27), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n134_), .c(new_n98_), .O(new_n172_));
  nor2   g110(.a(new_n172_), .b(new_n130_), .O(z20));
endmodule


