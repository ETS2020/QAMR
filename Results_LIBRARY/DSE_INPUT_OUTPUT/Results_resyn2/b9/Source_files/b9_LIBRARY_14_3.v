// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n172_, new_n173_, new_n175_, new_n176_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  nand2  g007(.a(x36), .b(x30), .O(new_n70_));
  and2   g008(.a(new_n70_), .b(x16), .O(new_n71_));
  oai21  g009(.a(new_n69_), .b(x15), .c(new_n71_), .O(z00));
  inv1   g010(.a(x27), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  inv1   g012(.a(x28), .O(new_n75_));
  nand2  g013(.a(x35), .b(new_n75_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n74_), .c(new_n73_), .O(new_n77_));
  nor2   g015(.a(x32), .b(x30), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  inv1   g017(.a(new_n70_), .O(new_n80_));
  nor2   g018(.a(new_n80_), .b(new_n63_), .O(new_n81_));
  oai21  g019(.a(new_n79_), .b(new_n77_), .c(new_n81_), .O(z01));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n76_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(new_n84_), .c(new_n80_), .O(z02));
  inv1   g026(.a(x35), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n75_), .c(x27), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n73_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n70_), .d(x21), .O(z03));
  aoi21  g031(.a(new_n91_), .b(new_n73_), .c(x21), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n90_), .c(new_n80_), .O(z04));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n70_), .c(new_n91_), .O(z05));
  inv1   g035(.a(z05), .O(z06));
  inv1   g036(.a(x14), .O(new_n99_));
  inv1   g037(.a(x15), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n100_), .O(new_n101_));
  nor2   g039(.a(x33), .b(x31), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n101_), .c(new_n99_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n99_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n105_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n70_), .c(x03), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(new_n103_), .O(z07));
  nand2  g047(.a(new_n70_), .b(new_n85_), .O(z08));
  nand2  g048(.a(x34), .b(x27), .O(new_n111_));
  nand2  g049(.a(x26), .b(x11), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n111_), .c(new_n70_), .O(z09));
  nand3  g051(.a(x37), .b(x27), .c(x06), .O(new_n114_));
  nand2  g052(.a(x32), .b(x05), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n64_), .c(new_n114_), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n70_), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  inv1   g057(.a(x30), .O(new_n120_));
  nor2   g058(.a(x36), .b(new_n120_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  aoi21  g060(.a(new_n85_), .b(x07), .c(new_n119_), .O(new_n123_));
  oai21  g061(.a(new_n89_), .b(x28), .c(new_n74_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n70_), .c(x27), .O(new_n125_));
  oai21  g063(.a(new_n125_), .b(new_n123_), .c(new_n122_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n63_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n117_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n76_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  inv1   g070(.a(x09), .O(new_n133_));
  nand4  g071(.a(x35), .b(new_n75_), .c(x27), .d(new_n63_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  aoi21  g073(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n120_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  nand2  g076(.a(new_n68_), .b(x27), .O(new_n139_));
  nand2  g077(.a(new_n65_), .b(x10), .O(new_n140_));
  nand3  g078(.a(new_n70_), .b(new_n140_), .c(new_n139_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  nand4  g081(.a(x40), .b(x39), .c(new_n143_), .d(new_n63_), .O(new_n144_));
  inv1   g082(.a(new_n144_), .O(new_n145_));
  nand2  g083(.a(x36), .b(x35), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(x28), .c(new_n73_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n79_), .c(new_n145_), .O(new_n148_));
  nor2   g086(.a(new_n66_), .b(new_n73_), .O(new_n149_));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand3  g089(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n149_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n148_), .c(new_n80_), .O(z13));
  nand3  g093(.a(new_n146_), .b(new_n78_), .c(x28), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n144_), .c(new_n152_), .O(new_n158_));
  aoi21  g096(.a(new_n75_), .b(x27), .c(new_n79_), .O(new_n159_));
  oai22  g097(.a(new_n159_), .b(new_n144_), .c(new_n66_), .d(new_n73_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n158_), .c(new_n70_), .O(z14));
  nand3  g099(.a(new_n70_), .b(x26), .c(x12), .O(new_n162_));
  nor2   g100(.a(new_n162_), .b(new_n111_), .O(z15));
  inv1   g101(.a(x23), .O(new_n164_));
  nand4  g102(.a(new_n70_), .b(new_n164_), .c(x22), .d(x01), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(x23), .c(x22), .d(x01), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n70_), .O(z17));
  inv1   g107(.a(new_n121_), .O(new_n170_));
  oai21  g108(.a(new_n136_), .b(x30), .c(new_n170_), .O(z18));
  nand2  g109(.a(new_n132_), .b(new_n131_), .O(new_n172_));
  nand3  g110(.a(new_n134_), .b(new_n172_), .c(new_n133_), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n120_), .c(new_n121_), .O(z19));
  nand2  g112(.a(x35), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n120_), .c(new_n133_), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(new_n131_), .O(z20));
endmodule


