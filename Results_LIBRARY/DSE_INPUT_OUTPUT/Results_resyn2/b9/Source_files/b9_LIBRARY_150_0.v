// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n152_, new_n154_,
    new_n156_, new_n157_, new_n160_, new_n161_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x36), .b(x23), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  nor2   g013(.a(x32), .b(x30), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n75_), .c(new_n76_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x04), .c(new_n72_), .O(z01));
  nand2  g018(.a(x35), .b(new_n77_), .O(new_n81_));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n81_), .c(x04), .O(new_n83_));
  nand2  g021(.a(x40), .b(x39), .O(new_n84_));
  aoi21  g022(.a(x29), .b(x08), .c(x02), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(new_n83_), .c(new_n72_), .O(z02));
  inv1   g025(.a(new_n72_), .O(new_n88_));
  nand2  g026(.a(x35), .b(x28), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x27), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n75_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n88_), .d(x21), .O(z03));
  aoi21  g031(.a(new_n91_), .b(new_n75_), .c(x21), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n90_), .c(new_n72_), .O(z04));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  nor2   g034(.a(new_n72_), .b(x37), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n96_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x14), .O(new_n100_));
  inv1   g038(.a(x15), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n101_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n100_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n88_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  nand2  g048(.a(new_n88_), .b(new_n84_), .O(z08));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n88_), .O(z09));
  oai21  g051(.a(new_n78_), .b(new_n75_), .c(new_n84_), .O(new_n114_));
  inv1   g052(.a(x05), .O(new_n115_));
  nand3  g053(.a(x40), .b(x39), .c(new_n115_), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  nand2  g055(.a(new_n84_), .b(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n116_), .c(new_n65_), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n114_), .c(new_n79_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(new_n72_), .O(z10));
  inv1   g061(.a(x08), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(x29), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n81_), .c(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n125_), .b(x27), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand4  g066(.a(x35), .b(new_n77_), .c(x27), .d(new_n65_), .O(new_n129_));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  and2   g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n128_), .c(new_n88_), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(z11));
  nand2  g071(.a(new_n67_), .b(x27), .O(new_n134_));
  nand3  g072(.a(new_n88_), .b(new_n68_), .c(new_n134_), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(z12));
  inv1   g074(.a(new_n63_), .O(new_n137_));
  nor2   g075(.a(x19), .b(x18), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(x20), .O(new_n139_));
  inv1   g077(.a(x13), .O(new_n140_));
  nor2   g078(.a(new_n84_), .b(x04), .O(new_n141_));
  nand2  g079(.a(x36), .b(x35), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x28), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n141_), .c(new_n140_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n139_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x27), .O(new_n146_));
  nor2   g084(.a(new_n76_), .b(x13), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n141_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n146_), .c(new_n88_), .O(z13));
  nand2  g087(.a(new_n148_), .b(new_n88_), .O(new_n150_));
  aoi21  g088(.a(new_n145_), .b(x27), .c(new_n150_), .O(z14));
  nand4  g089(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n88_), .O(z15));
  nand2  g091(.a(x22), .b(x01), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(x36), .c(x23), .O(z16));
  inv1   g093(.a(x24), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x23), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n154_), .c(new_n88_), .O(z17));
  aoi21  g096(.a(new_n131_), .b(new_n128_), .c(new_n72_), .O(z18));
  nand2  g097(.a(x35), .b(x27), .O(new_n160_));
  nand3  g098(.a(new_n130_), .b(new_n160_), .c(new_n88_), .O(new_n161_));
  nor2   g099(.a(new_n161_), .b(new_n126_), .O(z20));
  inv1   g100(.a(new_n132_), .O(z19));
endmodule


