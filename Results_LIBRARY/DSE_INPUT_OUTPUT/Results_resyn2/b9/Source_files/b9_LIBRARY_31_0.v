// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n158_, new_n159_, new_n161_,
    new_n162_, new_n164_, new_n167_, new_n168_;
  nor2   g000(.a(x20), .b(x04), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  and2   g007(.a(x40), .b(x39), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x27), .O(new_n74_));
  nor3   g012(.a(x37), .b(x36), .c(x35), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n72_), .c(new_n64_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(x16), .c(new_n63_), .O(z00));
  nand2  g016(.a(x20), .b(new_n69_), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(new_n80_), .c(new_n74_), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x04), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n83_), .c(new_n79_), .O(z01));
  inv1   g024(.a(new_n63_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n82_), .c(x04), .O(new_n89_));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(new_n70_), .d(new_n87_), .O(z02));
  nand2  g031(.a(new_n66_), .b(x27), .O(new_n94_));
  nand2  g032(.a(new_n65_), .b(new_n74_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n94_), .c(new_n87_), .d(x21), .O(z03));
  aoi21  g034(.a(new_n65_), .b(new_n74_), .c(x21), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n94_), .c(new_n63_), .O(z04));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n63_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n64_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n102_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n87_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  nand2  g049(.a(x40), .b(x39), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n87_), .O(z08));
  inv1   g051(.a(x11), .O(new_n114_));
  nand4  g052(.a(new_n87_), .b(x34), .c(x27), .d(x26), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n114_), .O(z09));
  aoi21  g054(.a(new_n82_), .b(new_n80_), .c(new_n79_), .O(new_n117_));
  inv1   g055(.a(x05), .O(new_n118_));
  aoi21  g056(.a(x40), .b(x39), .c(x07), .O(new_n119_));
  aoi21  g057(.a(new_n70_), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  nand2  g058(.a(x37), .b(x06), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n63_), .O(new_n122_));
  aoi21  g060(.a(new_n120_), .b(new_n117_), .c(new_n122_), .O(new_n123_));
  nor2   g061(.a(new_n84_), .b(new_n112_), .O(new_n124_));
  nand4  g062(.a(new_n124_), .b(x20), .c(x05), .d(new_n69_), .O(new_n125_));
  oai21  g063(.a(new_n123_), .b(new_n74_), .c(new_n125_), .O(z10));
  nand3  g064(.a(x35), .b(new_n81_), .c(x27), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n69_), .O(new_n129_));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n82_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n130_), .c(new_n129_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n87_), .O(z11));
  nand3  g075(.a(new_n76_), .b(new_n72_), .c(new_n87_), .O(z12));
  inv1   g076(.a(x20), .O(new_n139_));
  nor3   g077(.a(new_n84_), .b(new_n112_), .c(x13), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n139_), .c(new_n69_), .O(new_n141_));
  inv1   g079(.a(x13), .O(new_n142_));
  nand2  g080(.a(x36), .b(x35), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x28), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(new_n70_), .c(new_n142_), .d(new_n69_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n67_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand2  g086(.a(new_n145_), .b(new_n139_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(x27), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n141_), .O(z13));
  aoi21  g089(.a(new_n147_), .b(new_n145_), .c(new_n74_), .O(new_n152_));
  nand3  g090(.a(new_n124_), .b(new_n142_), .c(new_n69_), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n152_), .c(x20), .O(z14));
  inv1   g093(.a(x12), .O(new_n156_));
  nor2   g094(.a(new_n115_), .b(new_n156_), .O(z15));
  inv1   g095(.a(x23), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(x22), .c(x01), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n87_), .O(z16));
  nor2   g098(.a(x24), .b(new_n158_), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n87_), .c(x22), .d(x01), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(z17));
  oai21  g101(.a(new_n128_), .b(new_n139_), .c(new_n69_), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n135_), .c(new_n130_), .O(z18));
  inv1   g103(.a(z18), .O(z19));
  nand2  g104(.a(x35), .b(x27), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n130_), .c(new_n87_), .O(new_n168_));
  nor2   g106(.a(new_n168_), .b(new_n133_), .O(z20));
endmodule


