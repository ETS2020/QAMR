// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n95_,
    new_n96_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n162_, new_n163_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n175_, new_n176_,
    new_n177_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x28), .O(new_n64_));
  nor2   g002(.a(x36), .b(new_n64_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  nand2  g004(.a(x37), .b(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  nand4  g006(.a(x40), .b(x39), .c(x10), .d(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n67_), .c(x15), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n63_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  nand2  g011(.a(x35), .b(x28), .O(new_n74_));
  oai21  g012(.a(new_n73_), .b(x04), .c(new_n74_), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n64_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n75_), .c(x27), .d(new_n72_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n71_), .O(z00));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  oai21  g019(.a(x36), .b(x35), .c(x27), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n65_), .O(z01));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n77_), .c(x04), .O(new_n86_));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n87_), .c(new_n73_), .O(new_n89_));
  aoi21  g027(.a(new_n89_), .b(new_n86_), .c(new_n65_), .O(z02));
  inv1   g028(.a(x27), .O(new_n91_));
  aoi21  g029(.a(x36), .b(new_n91_), .c(new_n64_), .O(new_n92_));
  nor2   g030(.a(new_n92_), .b(x37), .O(z06));
  inv1   g031(.a(z06), .O(z05));
  nand2  g032(.a(new_n74_), .b(x27), .O(new_n95_));
  and2   g033(.a(new_n95_), .b(x21), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(new_n65_), .c(z05), .O(z03));
  oai21  g035(.a(x28), .b(x27), .c(new_n76_), .O(new_n98_));
  inv1   g036(.a(x37), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n91_), .c(x21), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n98_), .c(new_n95_), .O(z04));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n72_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n102_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n66_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  nand2  g049(.a(new_n73_), .b(new_n66_), .O(z08));
  nand3  g050(.a(x34), .b(x27), .c(x26), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n66_), .c(x11), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z09));
  inv1   g054(.a(x07), .O(new_n117_));
  oai21  g055(.a(new_n82_), .b(new_n117_), .c(new_n73_), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  and2   g057(.a(x40), .b(x39), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(x04), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n118_), .c(new_n83_), .O(new_n122_));
  inv1   g060(.a(new_n67_), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(x06), .c(new_n65_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  nand3  g065(.a(x35), .b(new_n64_), .c(new_n91_), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(new_n129_));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  nand2  g068(.a(x27), .b(new_n68_), .O(new_n131_));
  oai21  g069(.a(new_n131_), .b(new_n77_), .c(new_n130_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n129_), .c(new_n66_), .O(z11));
  inv1   g071(.a(x10), .O(new_n134_));
  nor3   g072(.a(x37), .b(x36), .c(x35), .O(new_n135_));
  oai21  g073(.a(new_n135_), .b(new_n91_), .c(new_n134_), .O(new_n136_));
  nand2  g074(.a(new_n74_), .b(new_n99_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x27), .O(new_n138_));
  oai21  g076(.a(new_n73_), .b(x04), .c(new_n138_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n136_), .c(new_n66_), .O(z12));
  nor2   g078(.a(x19), .b(x18), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n123_), .c(x20), .O(new_n142_));
  inv1   g080(.a(new_n81_), .O(new_n143_));
  nor2   g081(.a(x13), .b(x04), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n143_), .c(new_n120_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n66_), .O(new_n147_));
  nand2  g085(.a(new_n144_), .b(new_n120_), .O(new_n148_));
  nand3  g086(.a(new_n141_), .b(x28), .c(x20), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x28), .c(new_n91_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n147_), .O(z13));
  nand3  g092(.a(new_n141_), .b(new_n137_), .c(x20), .O(new_n155_));
  nor2   g093(.a(x35), .b(new_n64_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n148_), .c(new_n155_), .O(new_n157_));
  nand2  g095(.a(new_n145_), .b(new_n66_), .O(new_n158_));
  aoi21  g096(.a(new_n157_), .b(x27), .c(new_n158_), .O(z14));
  nand3  g097(.a(new_n114_), .b(new_n66_), .c(x12), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z15));
  inv1   g099(.a(x01), .O(new_n162_));
  inv1   g100(.a(x22), .O(new_n163_));
  nor4   g101(.a(new_n65_), .b(x23), .c(new_n163_), .d(new_n162_), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  nor2   g103(.a(new_n163_), .b(new_n162_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n66_), .c(new_n165_), .d(x23), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z17));
  oai21  g106(.a(new_n88_), .b(new_n73_), .c(new_n130_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n66_), .O(new_n170_));
  nor2   g108(.a(new_n85_), .b(new_n77_), .O(new_n171_));
  oai21  g109(.a(new_n91_), .b(new_n68_), .c(new_n171_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n170_), .O(z18));
  nor3   g111(.a(new_n132_), .b(new_n129_), .c(new_n65_), .O(z19));
  aoi21  g112(.a(new_n127_), .b(new_n77_), .c(new_n126_), .O(new_n175_));
  nand2  g113(.a(x35), .b(x27), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n130_), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n175_), .c(new_n66_), .O(z20));
endmodule


