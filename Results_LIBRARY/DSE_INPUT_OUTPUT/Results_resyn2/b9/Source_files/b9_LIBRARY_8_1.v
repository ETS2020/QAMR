// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n99_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n179_, new_n180_, new_n181_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(x27), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  inv1   g005(.a(x28), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  nand3  g008(.a(x40), .b(x39), .c(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n68_), .c(new_n69_), .O(new_n72_));
  inv1   g010(.a(x37), .O(new_n73_));
  nor2   g011(.a(x36), .b(x10), .O(new_n74_));
  oai21  g012(.a(new_n74_), .b(new_n71_), .c(new_n73_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n72_), .c(x27), .O(z12));
  oai21  g014(.a(z12), .b(x15), .c(new_n67_), .O(z00));
  inv1   g015(.a(x30), .O(new_n78_));
  inv1   g016(.a(x32), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n68_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n79_), .c(new_n78_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n65_), .O(z01));
  nand2  g023(.a(x40), .b(x39), .O(new_n86_));
  inv1   g024(.a(new_n86_), .O(new_n87_));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g028(.a(new_n81_), .b(x04), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n90_), .c(new_n87_), .d(x27), .O(z02));
  oai21  g030(.a(new_n69_), .b(new_n68_), .c(x27), .O(new_n93_));
  inv1   g031(.a(x27), .O(new_n94_));
  oai21  g032(.a(x40), .b(new_n73_), .c(new_n94_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n93_), .c(new_n97_), .O(z04));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n66_), .O(z05));
  inv1   g038(.a(new_n99_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n66_), .O(z06));
  inv1   g040(.a(x14), .O(new_n103_));
  inv1   g041(.a(x15), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n104_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n103_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n66_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  inv1   g051(.a(x39), .O(new_n114_));
  aoi21  g052(.a(new_n114_), .b(x27), .c(new_n64_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n66_), .O(z09));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  aoi21  g056(.a(new_n79_), .b(new_n78_), .c(new_n118_), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n82_), .O(new_n120_));
  nand3  g058(.a(new_n86_), .b(x27), .c(x07), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n70_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n65_), .O(new_n126_));
  oai21  g064(.a(new_n123_), .b(new_n120_), .c(new_n126_), .O(z10));
  nor2   g065(.a(x27), .b(x08), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n81_), .c(new_n64_), .O(new_n129_));
  nand3  g067(.a(x39), .b(x29), .c(x08), .O(new_n130_));
  aoi21  g068(.a(new_n64_), .b(x04), .c(new_n130_), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  nand3  g070(.a(x35), .b(new_n68_), .c(new_n70_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(x27), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  aoi21  g074(.a(new_n134_), .b(new_n129_), .c(new_n136_), .O(z11));
  nand2  g075(.a(x36), .b(x35), .O(new_n138_));
  nand4  g076(.a(new_n138_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n139_));
  inv1   g077(.a(x13), .O(new_n140_));
  nand3  g078(.a(x39), .b(new_n140_), .c(new_n70_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(new_n139_), .c(new_n94_), .O(new_n143_));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand3  g083(.a(x20), .b(new_n145_), .c(new_n144_), .O(new_n146_));
  aoi21  g084(.a(x35), .b(x28), .c(x37), .O(new_n147_));
  or2    g085(.a(new_n147_), .b(new_n94_), .O(new_n148_));
  oai22  g086(.a(new_n148_), .b(new_n146_), .c(new_n143_), .d(new_n64_), .O(z13));
  inv1   g087(.a(new_n139_), .O(new_n150_));
  nand3  g088(.a(new_n87_), .b(new_n140_), .c(new_n70_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n146_), .O(new_n152_));
  nand4  g090(.a(new_n69_), .b(new_n79_), .c(new_n78_), .d(x28), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n151_), .c(new_n147_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n152_), .c(x27), .O(z14));
  nand4  g094(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z15));
  inv1   g096(.a(x01), .O(new_n159_));
  inv1   g097(.a(x22), .O(new_n160_));
  nor4   g098(.a(new_n65_), .b(x23), .c(new_n160_), .d(new_n159_), .O(z16));
  inv1   g099(.a(x24), .O(new_n162_));
  nor2   g100(.a(new_n160_), .b(new_n159_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(new_n66_), .c(new_n162_), .d(x23), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z17));
  inv1   g103(.a(x29), .O(new_n166_));
  oai22  g104(.a(new_n81_), .b(x27), .c(new_n86_), .d(new_n166_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x08), .O(new_n168_));
  inv1   g106(.a(new_n133_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x27), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(new_n168_), .c(new_n135_), .d(new_n66_), .O(z18));
  oai21  g109(.a(new_n136_), .b(x08), .c(new_n64_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n94_), .O(new_n173_));
  nand2  g111(.a(new_n133_), .b(x27), .O(new_n174_));
  nand2  g112(.a(new_n81_), .b(new_n64_), .O(new_n175_));
  oai21  g113(.a(new_n174_), .b(new_n131_), .c(new_n175_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n135_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n173_), .O(z19));
  aoi21  g116(.a(new_n130_), .b(x27), .c(new_n64_), .O(new_n179_));
  aoi21  g117(.a(new_n68_), .b(x08), .c(x27), .O(new_n180_));
  oai21  g118(.a(new_n180_), .b(new_n69_), .c(new_n135_), .O(new_n181_));
  nor2   g119(.a(new_n181_), .b(new_n179_), .O(z20));
endmodule


