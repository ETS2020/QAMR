// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n150_, new_n152_, new_n153_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n162_;
  inv1   g000(.a(x27), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n67_), .b(new_n63_), .c(new_n66_), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  nand3  g007(.a(x40), .b(x39), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(new_n72_));
  inv1   g010(.a(x16), .O(new_n73_));
  inv1   g011(.a(x39), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(x30), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  oai21  g014(.a(new_n72_), .b(x15), .c(new_n76_), .O(z00));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(x28), .c(new_n79_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n75_), .O(z01));
  inv1   g022(.a(x08), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  aoi21  g025(.a(new_n63_), .b(new_n85_), .c(new_n87_), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  inv1   g027(.a(x40), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n74_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x30), .O(new_n92_));
  nor2   g030(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  oai21  g031(.a(new_n88_), .b(new_n69_), .c(new_n93_), .O(z02));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  oai22  g034(.a(new_n96_), .b(new_n78_), .c(new_n95_), .d(x27), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n75_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi21  g037(.a(new_n97_), .b(new_n99_), .c(new_n75_), .O(z04));
  inv1   g038(.a(new_n75_), .O(new_n101_));
  nand2  g039(.a(new_n96_), .b(new_n95_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n101_), .O(z06));
  inv1   g041(.a(z06), .O(z05));
  inv1   g042(.a(x14), .O(new_n105_));
  inv1   g043(.a(x15), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n106_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n105_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n101_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  aoi21  g053(.a(new_n90_), .b(x30), .c(new_n74_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand3  g055(.a(x34), .b(x27), .c(x26), .O(new_n118_));
  nor3   g056(.a(new_n118_), .b(new_n75_), .c(new_n117_), .O(z09));
  nand2  g057(.a(new_n91_), .b(x05), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  aoi21  g059(.a(x40), .b(x39), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n80_), .c(x27), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n69_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n75_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n125_), .O(z10));
  inv1   g067(.a(x09), .O(new_n130_));
  nand2  g068(.a(new_n63_), .b(new_n85_), .O(new_n131_));
  nand2  g069(.a(x27), .b(x04), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(new_n131_), .c(x35), .d(new_n86_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n74_), .c(x30), .O(z11));
  oai21  g073(.a(new_n70_), .b(new_n67_), .c(new_n64_), .O(new_n136_));
  nor2   g074(.a(new_n90_), .b(x04), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x10), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x30), .O(new_n139_));
  aoi22  g077(.a(new_n139_), .b(x39), .c(new_n136_), .d(x27), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  inv1   g079(.a(x30), .O(new_n142_));
  aoi21  g080(.a(new_n137_), .b(new_n141_), .c(new_n142_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n65_), .c(x20), .O(new_n145_));
  oai21  g083(.a(new_n143_), .b(new_n74_), .c(new_n145_), .O(z13));
  nand3  g084(.a(new_n137_), .b(x39), .c(new_n141_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n101_), .O(z14));
  inv1   g087(.a(x12), .O(new_n150_));
  oai21  g088(.a(new_n118_), .b(new_n150_), .c(new_n101_), .O(z15));
  inv1   g089(.a(x23), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(x22), .c(x01), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n101_), .O(z16));
  nor2   g092(.a(x24), .b(new_n152_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(new_n101_), .c(x22), .d(x01), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(z17));
  inv1   g095(.a(new_n133_), .O(new_n158_));
  nand3  g096(.a(new_n74_), .b(new_n142_), .c(new_n130_), .O(new_n159_));
  nor2   g097(.a(new_n159_), .b(new_n158_), .O(z19));
  inv1   g098(.a(z19), .O(z18));
  oai21  g099(.a(x28), .b(new_n85_), .c(new_n63_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(x35), .c(new_n159_), .O(z20));
endmodule


