// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n158_, new_n160_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n168_, new_n169_;
  inv1   g000(.a(x34), .O(new_n63_));
  nand2  g001(.a(x36), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x10), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  nand2  g008(.a(x40), .b(x39), .O(new_n71_));
  oai22  g009(.a(new_n71_), .b(x04), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(new_n73_));
  inv1   g011(.a(x16), .O(new_n74_));
  inv1   g012(.a(x04), .O(new_n75_));
  nand3  g013(.a(x27), .b(new_n66_), .c(new_n75_), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n78_), .c(new_n71_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(new_n73_), .c(new_n65_), .O(z00));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  inv1   g021(.a(x28), .O(new_n84_));
  nand2  g022(.a(x35), .b(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n79_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(x27), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n83_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(x04), .c(new_n65_), .O(z01));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n85_), .c(x04), .O(new_n91_));
  nor2   g029(.a(new_n71_), .b(new_n65_), .O(z08));
  inv1   g030(.a(x02), .O(new_n93_));
  nand2  g031(.a(x29), .b(x08), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(z08), .c(new_n91_), .O(z02));
  inv1   g034(.a(x37), .O(new_n97_));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  oai22  g036(.a(new_n98_), .b(new_n78_), .c(new_n97_), .d(x27), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  aoi21  g039(.a(new_n99_), .b(new_n101_), .c(new_n65_), .O(z04));
  nand3  g040(.a(new_n98_), .b(new_n64_), .c(new_n97_), .O(z05));
  nand2  g041(.a(new_n98_), .b(new_n97_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n64_), .O(z06));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n66_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n106_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n106_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n64_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  nor2   g053(.a(new_n63_), .b(new_n68_), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(x26), .c(x11), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z09));
  inv1   g056(.a(new_n71_), .O(new_n119_));
  oai21  g057(.a(x32), .b(x30), .c(new_n119_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(new_n71_), .b(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n86_), .c(x27), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  inv1   g062(.a(x05), .O(new_n125_));
  aoi21  g063(.a(new_n119_), .b(new_n125_), .c(x04), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n64_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  nand3  g068(.a(x40), .b(x39), .c(x29), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n85_), .c(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n131_), .b(x27), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  nand4  g073(.a(x35), .b(new_n84_), .c(x27), .d(new_n75_), .O(new_n136_));
  and2   g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n134_), .c(new_n65_), .O(z18));
  inv1   g076(.a(z18), .O(z11));
  nor3   g077(.a(x37), .b(x36), .c(x35), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n68_), .c(new_n67_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n72_), .c(new_n64_), .O(z12));
  nand2  g080(.a(x35), .b(x28), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n97_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n144_), .c(x20), .O(new_n146_));
  nor2   g084(.a(x13), .b(x04), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n119_), .c(new_n84_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n146_), .c(new_n68_), .O(new_n149_));
  nand2  g087(.a(new_n147_), .b(new_n119_), .O(new_n150_));
  nor2   g088(.a(new_n150_), .b(new_n83_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n149_), .c(new_n64_), .O(new_n152_));
  inv1   g090(.a(new_n150_), .O(new_n153_));
  nand4  g091(.a(new_n153_), .b(new_n116_), .c(x36), .d(x35), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(z13));
  aoi21  g093(.a(x36), .b(x35), .c(new_n84_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n150_), .c(new_n146_), .O(new_n157_));
  oai21  g095(.a(new_n150_), .b(new_n83_), .c(new_n64_), .O(new_n158_));
  aoi21  g096(.a(new_n157_), .b(x27), .c(new_n158_), .O(z14));
  nand3  g097(.a(new_n116_), .b(x26), .c(x12), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z15));
  nand2  g099(.a(x22), .b(x01), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(x23), .c(new_n64_), .O(z16));
  inv1   g101(.a(x24), .O(new_n164_));
  inv1   g102(.a(new_n162_), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(new_n64_), .c(new_n164_), .d(x23), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(z17));
  nand2  g105(.a(x35), .b(x27), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n135_), .c(new_n64_), .O(new_n169_));
  nor2   g107(.a(new_n169_), .b(new_n132_), .O(z20));
  inv1   g108(.a(z18), .O(z19));
endmodule


