// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n182_;
  inv1   g000(.a(x19), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g014(.a(x39), .O(new_n77_));
  nor3   g015(.a(new_n64_), .b(new_n77_), .c(x19), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n76_), .c(new_n66_), .d(new_n73_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n81_), .c(new_n68_), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  inv1   g023(.a(new_n85_), .O(new_n86_));
  nor2   g024(.a(new_n65_), .b(new_n73_), .O(new_n87_));
  oai21  g025(.a(new_n86_), .b(new_n84_), .c(new_n87_), .O(z01));
  inv1   g026(.a(x35), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(x28), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n90_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x04), .O(new_n92_));
  inv1   g030(.a(x02), .O(new_n93_));
  nand2  g031(.a(x29), .b(x08), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n92_), .c(new_n78_), .O(z02));
  inv1   g034(.a(new_n65_), .O(new_n97_));
  nand2  g035(.a(new_n70_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n69_), .b(new_n68_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n65_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n65_), .O(z05));
  nand2  g042(.a(new_n103_), .b(new_n97_), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(z06));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n66_), .O(new_n108_));
  nor2   g046(.a(x33), .b(x31), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  nand2  g050(.a(x25), .b(new_n107_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n97_), .c(x03), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n110_), .O(z07));
  aoi21  g054(.a(new_n77_), .b(new_n63_), .c(new_n64_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n97_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  inv1   g058(.a(x07), .O(new_n121_));
  nand4  g059(.a(x40), .b(x39), .c(new_n63_), .d(x05), .O(new_n122_));
  oai21  g060(.a(z08), .b(new_n121_), .c(new_n122_), .O(new_n123_));
  aoi21  g061(.a(new_n83_), .b(new_n81_), .c(x04), .O(new_n124_));
  nand2  g062(.a(x37), .b(x06), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n65_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(new_n123_), .c(new_n126_), .O(new_n127_));
  nand4  g065(.a(new_n86_), .b(new_n78_), .c(x05), .d(new_n73_), .O(new_n128_));
  oai21  g066(.a(new_n127_), .b(new_n68_), .c(new_n128_), .O(z10));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(new_n130_), .b(new_n132_), .c(x04), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n63_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x40), .O(new_n135_));
  nand3  g073(.a(x40), .b(x39), .c(x29), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x08), .O(new_n138_));
  nand3  g076(.a(new_n136_), .b(x27), .c(x04), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(new_n140_));
  aoi21  g078(.a(new_n138_), .b(new_n91_), .c(new_n140_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n131_), .c(new_n135_), .O(z11));
  nand3  g080(.a(new_n69_), .b(new_n81_), .c(new_n89_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(x10), .O(new_n144_));
  nor2   g082(.a(new_n77_), .b(x04), .O(new_n145_));
  nor2   g083(.a(new_n145_), .b(new_n71_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n144_), .c(new_n63_), .O(new_n147_));
  oai21  g085(.a(new_n71_), .b(x40), .c(new_n147_), .O(z12));
  inv1   g086(.a(x18), .O(new_n149_));
  nand2  g087(.a(new_n70_), .b(new_n69_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(x20), .c(new_n149_), .O(new_n151_));
  inv1   g089(.a(x13), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(x28), .c(new_n64_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n145_), .c(new_n152_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n151_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nand4  g095(.a(new_n145_), .b(new_n86_), .c(x40), .d(new_n152_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n157_), .c(x19), .O(z13));
  aoi21  g097(.a(new_n155_), .b(new_n151_), .c(new_n68_), .O(new_n160_));
  nand3  g098(.a(new_n145_), .b(new_n86_), .c(new_n152_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n63_), .c(new_n64_), .O(new_n162_));
  nand2  g100(.a(new_n155_), .b(x19), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n160_), .c(new_n162_), .O(z14));
  inv1   g102(.a(x12), .O(new_n165_));
  nor2   g103(.a(new_n119_), .b(new_n165_), .O(z15));
  inv1   g104(.a(x23), .O(new_n167_));
  nand4  g105(.a(new_n97_), .b(new_n167_), .c(x22), .d(x01), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z16));
  nand2  g107(.a(x22), .b(x01), .O(new_n170_));
  nor4   g108(.a(new_n170_), .b(new_n65_), .c(x24), .d(new_n167_), .O(z17));
  nand3  g109(.a(new_n137_), .b(new_n63_), .c(x08), .O(new_n172_));
  aoi21  g110(.a(new_n68_), .b(new_n132_), .c(new_n83_), .O(new_n173_));
  nand2  g111(.a(x27), .b(x04), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n173_), .c(new_n131_), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(new_n172_), .c(new_n65_), .O(z18));
  aoi21  g114(.a(new_n136_), .b(new_n83_), .c(new_n132_), .O(new_n177_));
  nand2  g115(.a(new_n136_), .b(x27), .O(new_n178_));
  nand3  g116(.a(new_n90_), .b(x27), .c(new_n73_), .O(new_n179_));
  nand3  g117(.a(new_n179_), .b(new_n130_), .c(new_n97_), .O(new_n180_));
  aoi21  g118(.a(new_n178_), .b(new_n177_), .c(new_n180_), .O(z19));
  oai21  g119(.a(new_n89_), .b(new_n68_), .c(new_n130_), .O(new_n182_));
  oai21  g120(.a(new_n182_), .b(new_n177_), .c(new_n97_), .O(z20));
endmodule


