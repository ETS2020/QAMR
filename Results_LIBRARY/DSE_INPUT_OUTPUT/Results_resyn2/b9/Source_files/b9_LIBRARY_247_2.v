// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n162_, new_n163_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n174_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x06), .O(new_n71_));
  nor2   g009(.a(x40), .b(new_n71_), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(new_n72_), .O(new_n75_));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(new_n76_), .c(new_n79_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n75_), .c(x04), .O(z01));
  inv1   g019(.a(x40), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n71_), .O(new_n83_));
  inv1   g021(.a(x35), .O(new_n84_));
  nor2   g022(.a(new_n84_), .b(x28), .O(new_n85_));
  oai21  g023(.a(x27), .b(x08), .c(new_n85_), .O(new_n86_));
  and2   g024(.a(x29), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(x02), .c(x39), .O(new_n88_));
  aoi21  g026(.a(new_n86_), .b(x04), .c(new_n88_), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n82_), .c(new_n83_), .O(z02));
  oai21  g028(.a(new_n84_), .b(new_n77_), .c(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n76_), .c(new_n72_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n91_), .c(new_n95_), .O(z04));
  aoi21  g034(.a(x28), .b(x27), .c(x37), .O(new_n97_));
  nor2   g035(.a(new_n97_), .b(new_n72_), .O(z05));
  inv1   g036(.a(new_n97_), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n72_), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n75_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  and2   g049(.a(x40), .b(x39), .O(z08));
  nand3  g050(.a(x34), .b(x27), .c(x26), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n75_), .c(x11), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z09));
  nor2   g054(.a(new_n78_), .b(new_n76_), .O(new_n117_));
  nand2  g055(.a(z08), .b(x05), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  oai21  g057(.a(x40), .b(new_n71_), .c(x07), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(z08), .O(new_n121_));
  aoi22  g059(.a(new_n121_), .b(new_n117_), .c(new_n119_), .d(new_n80_), .O(new_n122_));
  nand4  g060(.a(x40), .b(x37), .c(x27), .d(x06), .O(new_n123_));
  oai21  g061(.a(new_n122_), .b(x04), .c(new_n123_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand2  g063(.a(x35), .b(new_n77_), .O(new_n126_));
  nand2  g064(.a(z08), .b(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n126_), .c(new_n125_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nand3  g069(.a(new_n85_), .b(x27), .c(new_n63_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n130_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n75_), .O(z11));
  nand2  g072(.a(new_n66_), .b(new_n92_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x27), .c(x10), .O(new_n136_));
  nor2   g074(.a(new_n67_), .b(new_n76_), .O(new_n137_));
  aoi21  g075(.a(x39), .b(new_n63_), .c(new_n137_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n136_), .c(x40), .O(new_n139_));
  inv1   g077(.a(new_n137_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n82_), .c(new_n71_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  nand3  g081(.a(z08), .b(new_n143_), .c(new_n63_), .O(new_n144_));
  aoi21  g082(.a(x36), .b(x35), .c(new_n77_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x20), .O(new_n147_));
  oai22  g085(.a(new_n147_), .b(new_n67_), .c(new_n145_), .d(new_n144_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  inv1   g087(.a(new_n79_), .O(new_n150_));
  inv1   g088(.a(new_n144_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n150_), .c(new_n72_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n149_), .O(z13));
  nand2  g091(.a(new_n145_), .b(new_n79_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n151_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n147_), .O(new_n156_));
  aoi21  g094(.a(new_n77_), .b(x27), .c(new_n150_), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n144_), .c(new_n140_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n156_), .c(new_n75_), .O(z14));
  nand3  g097(.a(new_n114_), .b(new_n75_), .c(x12), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z15));
  inv1   g099(.a(x01), .O(new_n162_));
  inv1   g100(.a(x22), .O(new_n163_));
  nor4   g101(.a(new_n72_), .b(x23), .c(new_n163_), .d(new_n162_), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  nor2   g103(.a(new_n163_), .b(new_n162_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n75_), .c(new_n165_), .d(x23), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z17));
  nor2   g106(.a(new_n76_), .b(new_n63_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n86_), .c(new_n131_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n75_), .O(new_n171_));
  nand2  g109(.a(z08), .b(new_n87_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n171_), .O(z18));
  oai21  g111(.a(new_n84_), .b(new_n76_), .c(new_n131_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n128_), .c(new_n75_), .O(z20));
  nand2  g113(.a(new_n133_), .b(new_n75_), .O(z19));
endmodule


