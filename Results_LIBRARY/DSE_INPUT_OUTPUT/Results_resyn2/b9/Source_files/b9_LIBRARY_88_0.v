// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n159_, new_n161_, new_n162_,
    new_n163_, new_n166_, new_n167_;
  inv1   g000(.a(x34), .O(new_n63_));
  inv1   g001(.a(x36), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x16), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(x27), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(x10), .c(new_n73_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n67_), .c(new_n68_), .O(new_n78_));
  aoi21  g016(.a(x35), .b(x34), .c(x36), .O(new_n79_));
  inv1   g017(.a(x27), .O(new_n80_));
  nor2   g018(.a(new_n80_), .b(x04), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n75_), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n67_), .O(new_n84_));
  oai21  g022(.a(new_n78_), .b(new_n66_), .c(new_n84_), .O(z00));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n64_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x27), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n86_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x04), .c(new_n66_), .O(z01));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n88_), .c(x04), .O(new_n94_));
  nor2   g032(.a(new_n74_), .b(new_n66_), .O(z08));
  inv1   g033(.a(x02), .O(new_n96_));
  nand2  g034(.a(x29), .b(x08), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(z08), .c(new_n94_), .O(z02));
  inv1   g037(.a(x35), .O(new_n100_));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  oai22  g039(.a(new_n101_), .b(new_n100_), .c(new_n69_), .d(x27), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(x21), .c(new_n66_), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  aoi21  g042(.a(new_n102_), .b(new_n104_), .c(new_n66_), .O(z04));
  nand3  g043(.a(new_n101_), .b(new_n65_), .c(new_n69_), .O(z05));
  nand2  g044(.a(new_n101_), .b(new_n69_), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(new_n65_), .O(z06));
  inv1   g046(.a(x14), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n67_), .O(new_n110_));
  nor2   g048(.a(x33), .b(x31), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x00), .O(new_n114_));
  nand2  g052(.a(x25), .b(new_n109_), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n114_), .c(x38), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n65_), .c(x03), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n112_), .O(z07));
  nand4  g056(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z09));
  oai21  g058(.a(x32), .b(x30), .c(new_n75_), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  nand2  g060(.a(new_n74_), .b(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n89_), .c(x27), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  aoi21  g064(.a(new_n75_), .b(new_n126_), .c(x04), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n65_), .O(z10));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n88_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nor2   g073(.a(x30), .b(x09), .O(new_n136_));
  nand3  g074(.a(new_n81_), .b(x35), .c(new_n87_), .O(new_n137_));
  and2   g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n65_), .O(z11));
  aoi21  g078(.a(new_n77_), .b(new_n65_), .c(new_n83_), .O(z12));
  nor2   g079(.a(x19), .b(x18), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n71_), .c(x20), .O(new_n143_));
  nor2   g081(.a(x13), .b(x04), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n75_), .c(new_n87_), .O(new_n145_));
  aoi21  g083(.a(new_n145_), .b(new_n143_), .c(new_n80_), .O(new_n146_));
  nand2  g084(.a(new_n144_), .b(new_n75_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(new_n86_), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n146_), .c(new_n65_), .O(new_n149_));
  inv1   g087(.a(new_n147_), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x36), .c(x35), .d(x27), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(z13));
  aoi21  g090(.a(x36), .b(x35), .c(new_n87_), .O(new_n153_));
  oai21  g091(.a(new_n153_), .b(new_n147_), .c(new_n143_), .O(new_n154_));
  oai21  g092(.a(new_n147_), .b(new_n86_), .c(new_n65_), .O(new_n155_));
  aoi21  g093(.a(new_n154_), .b(x27), .c(new_n155_), .O(z14));
  nand4  g094(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z15));
  nand2  g096(.a(x22), .b(x01), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(x23), .c(new_n65_), .O(z16));
  inv1   g098(.a(x24), .O(new_n161_));
  inv1   g099(.a(new_n159_), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(new_n65_), .c(new_n161_), .d(x23), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(z17));
  aoi21  g102(.a(new_n138_), .b(new_n135_), .c(new_n66_), .O(z18));
  nand2  g103(.a(x35), .b(x27), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n136_), .c(new_n65_), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(new_n133_), .O(z20));
  nand2  g106(.a(new_n139_), .b(new_n65_), .O(z19));
endmodule


