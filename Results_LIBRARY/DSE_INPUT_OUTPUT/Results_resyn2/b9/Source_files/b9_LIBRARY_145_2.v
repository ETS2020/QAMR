// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n175_,
    new_n176_, new_n178_, new_n179_, new_n180_, new_n182_, new_n183_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x19), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n66_), .c(new_n67_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  inv1   g011(.a(x39), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(x04), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(x40), .O(new_n76_));
  aoi21  g014(.a(new_n73_), .b(new_n72_), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n66_), .O(new_n78_));
  oai21  g016(.a(new_n71_), .b(new_n65_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(new_n80_), .c(new_n68_), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  inv1   g023(.a(x04), .O(new_n86_));
  nor2   g024(.a(new_n65_), .b(new_n86_), .O(new_n87_));
  oai21  g025(.a(new_n85_), .b(new_n83_), .c(new_n87_), .O(z01));
  inv1   g026(.a(x08), .O(new_n89_));
  aoi21  g027(.a(new_n68_), .b(new_n89_), .c(new_n82_), .O(new_n90_));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n91_), .c(new_n92_), .O(new_n94_));
  oai21  g032(.a(new_n90_), .b(new_n86_), .c(new_n94_), .O(z02));
  nand2  g033(.a(x35), .b(x28), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x27), .O(new_n97_));
  inv1   g035(.a(x37), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n68_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n97_), .c(new_n64_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n98_), .b(new_n68_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n97_), .c(new_n65_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n65_), .O(z05));
  and2   g042(.a(new_n103_), .b(new_n64_), .O(z06));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n66_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n106_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n106_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n64_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  nand2  g053(.a(new_n92_), .b(new_n64_), .O(z08));
  nand3  g054(.a(x34), .b(x27), .c(x26), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n64_), .c(x11), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(z09));
  inv1   g058(.a(x07), .O(new_n121_));
  inv1   g059(.a(new_n92_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x05), .O(new_n123_));
  oai21  g061(.a(z08), .b(new_n121_), .c(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n82_), .b(new_n80_), .c(x04), .O(new_n125_));
  nand2  g063(.a(x37), .b(x06), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n65_), .O(new_n127_));
  aoi21  g065(.a(new_n125_), .b(new_n124_), .c(new_n127_), .O(new_n128_));
  nand4  g066(.a(new_n122_), .b(new_n85_), .c(x05), .d(new_n86_), .O(new_n129_));
  oai21  g067(.a(new_n128_), .b(new_n68_), .c(new_n129_), .O(z10));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n82_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x08), .O(new_n135_));
  nand4  g073(.a(x35), .b(new_n81_), .c(x27), .d(new_n86_), .O(new_n136_));
  nor2   g074(.a(new_n68_), .b(new_n86_), .O(new_n137_));
  aoi22  g075(.a(new_n137_), .b(new_n133_), .c(new_n136_), .d(new_n135_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n132_), .c(new_n64_), .O(z11));
  nor3   g077(.a(new_n77_), .b(new_n70_), .c(new_n65_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  nand2  g079(.a(x36), .b(x35), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x28), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n75_), .c(new_n141_), .O(new_n144_));
  inv1   g082(.a(x18), .O(new_n145_));
  inv1   g083(.a(x19), .O(new_n146_));
  nand2  g084(.a(new_n96_), .b(new_n98_), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(x20), .c(new_n146_), .d(new_n145_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n144_), .c(new_n68_), .O(new_n149_));
  nand3  g087(.a(x39), .b(new_n141_), .c(new_n86_), .O(new_n150_));
  nor2   g088(.a(new_n150_), .b(new_n84_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n149_), .c(x40), .O(new_n152_));
  nand3  g090(.a(x20), .b(new_n146_), .c(new_n145_), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n70_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(z13));
  aoi21  g094(.a(new_n142_), .b(x28), .c(new_n150_), .O(new_n157_));
  nor2   g095(.a(new_n153_), .b(new_n69_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n157_), .c(x27), .O(new_n159_));
  inv1   g097(.a(new_n151_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  inv1   g099(.a(new_n70_), .O(new_n162_));
  nand3  g100(.a(new_n63_), .b(x20), .c(new_n145_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n162_), .c(new_n64_), .O(new_n164_));
  aoi21  g102(.a(new_n161_), .b(x40), .c(new_n164_), .O(z14));
  nand3  g103(.a(new_n118_), .b(new_n64_), .c(x12), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(z15));
  inv1   g105(.a(x01), .O(new_n168_));
  inv1   g106(.a(x22), .O(new_n169_));
  nor4   g107(.a(new_n65_), .b(x23), .c(new_n169_), .d(new_n168_), .O(z16));
  inv1   g108(.a(x24), .O(new_n171_));
  nor2   g109(.a(new_n169_), .b(new_n168_), .O(new_n172_));
  nand4  g110(.a(new_n172_), .b(new_n64_), .c(new_n171_), .d(x23), .O(new_n173_));
  inv1   g111(.a(new_n173_), .O(z17));
  inv1   g112(.a(new_n137_), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(new_n90_), .c(new_n132_), .O(new_n176_));
  oai22  g114(.a(new_n176_), .b(new_n65_), .c(new_n93_), .d(new_n92_), .O(z18));
  inv1   g115(.a(new_n135_), .O(new_n178_));
  nand2  g116(.a(new_n133_), .b(x27), .O(new_n179_));
  nand3  g117(.a(new_n136_), .b(new_n131_), .c(new_n64_), .O(new_n180_));
  aoi21  g118(.a(new_n179_), .b(new_n178_), .c(new_n180_), .O(z19));
  nand2  g119(.a(x35), .b(x27), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(new_n131_), .O(new_n183_));
  oai21  g121(.a(new_n183_), .b(new_n178_), .c(new_n64_), .O(z20));
endmodule


