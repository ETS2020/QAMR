// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n173_, new_n174_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x29), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n66_), .c(new_n67_), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g012(.a(new_n74_), .b(x27), .c(x10), .O(new_n75_));
  nand2  g013(.a(x40), .b(x39), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(x04), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n66_), .O(new_n78_));
  oai22  g016(.a(new_n78_), .b(new_n75_), .c(new_n71_), .d(new_n65_), .O(z00));
  nor2   g017(.a(new_n72_), .b(x28), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(x36), .c(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n86_), .c(x04), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(new_n76_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n88_), .O(z02));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(x28), .b(x27), .O(new_n93_));
  oai22  g031(.a(new_n93_), .b(new_n72_), .c(new_n92_), .d(x27), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n64_), .c(new_n96_), .O(z04));
  nand2  g035(.a(new_n93_), .b(new_n92_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n64_), .O(z06));
  inv1   g037(.a(z06), .O(z05));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n66_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n101_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n64_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  inv1   g048(.a(new_n76_), .O(z08));
  nand3  g049(.a(x34), .b(x27), .c(x26), .O(new_n112_));
  nand2  g050(.a(new_n64_), .b(x11), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n112_), .O(z09));
  aoi21  g052(.a(new_n86_), .b(new_n73_), .c(new_n68_), .O(new_n115_));
  nand3  g053(.a(x40), .b(x39), .c(x05), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(new_n82_), .O(new_n117_));
  nand2  g055(.a(new_n76_), .b(x07), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(new_n115_), .c(new_n117_), .O(new_n120_));
  nand3  g058(.a(x37), .b(x27), .c(x06), .O(new_n121_));
  and2   g059(.a(new_n121_), .b(new_n64_), .O(new_n122_));
  oai21  g060(.a(new_n120_), .b(x04), .c(new_n122_), .O(z10));
  inv1   g061(.a(x08), .O(new_n124_));
  nand2  g062(.a(x39), .b(x29), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n86_), .c(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n125_), .b(x27), .O(new_n127_));
  inv1   g065(.a(x09), .O(new_n128_));
  inv1   g066(.a(x30), .O(new_n129_));
  nor2   g067(.a(new_n68_), .b(x04), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n80_), .O(new_n131_));
  nand4  g069(.a(new_n131_), .b(new_n64_), .c(new_n129_), .d(new_n128_), .O(new_n132_));
  aoi21  g070(.a(new_n127_), .b(new_n126_), .c(new_n132_), .O(z11));
  nand2  g071(.a(new_n77_), .b(new_n74_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n69_), .c(new_n68_), .O(new_n135_));
  nand2  g073(.a(new_n77_), .b(x10), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n135_), .c(new_n64_), .O(z12));
  inv1   g076(.a(x13), .O(new_n139_));
  nand2  g077(.a(x36), .b(x35), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x28), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n77_), .c(new_n139_), .O(new_n142_));
  nand2  g080(.a(x35), .b(x28), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n92_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand4  g083(.a(new_n145_), .b(new_n144_), .c(new_n64_), .d(x20), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n142_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  inv1   g086(.a(x32), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n129_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n77_), .c(new_n139_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n148_), .O(z13));
  nand2  g090(.a(new_n77_), .b(new_n139_), .O(new_n153_));
  nand2  g091(.a(new_n145_), .b(x20), .O(new_n154_));
  nand3  g092(.a(new_n140_), .b(new_n82_), .c(x28), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n153_), .c(new_n154_), .O(new_n157_));
  aoi21  g095(.a(new_n85_), .b(x27), .c(new_n150_), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n153_), .c(new_n69_), .d(new_n68_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n157_), .c(new_n64_), .O(z14));
  inv1   g098(.a(x12), .O(new_n161_));
  oai21  g099(.a(new_n112_), .b(new_n161_), .c(new_n64_), .O(z15));
  inv1   g100(.a(x01), .O(new_n163_));
  inv1   g101(.a(x22), .O(new_n164_));
  nor4   g102(.a(new_n65_), .b(x23), .c(new_n164_), .d(new_n163_), .O(z16));
  inv1   g103(.a(x24), .O(new_n166_));
  nor2   g104(.a(new_n164_), .b(new_n163_), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(new_n64_), .c(new_n166_), .d(x23), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z17));
  nand2  g107(.a(new_n127_), .b(new_n126_), .O(new_n170_));
  inv1   g108(.a(new_n132_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n170_), .O(z18));
  nand3  g110(.a(new_n64_), .b(new_n129_), .c(new_n128_), .O(new_n173_));
  nor2   g111(.a(new_n72_), .b(new_n68_), .O(new_n174_));
  nor3   g112(.a(new_n174_), .b(new_n173_), .c(new_n126_), .O(z20));
  aoi21  g113(.a(new_n127_), .b(new_n126_), .c(new_n132_), .O(z19));
endmodule


