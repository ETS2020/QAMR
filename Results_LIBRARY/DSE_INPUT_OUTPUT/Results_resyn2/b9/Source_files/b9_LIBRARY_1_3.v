// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n97_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n170_, new_n172_;
  inv1   g000(.a(x10), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  inv1   g004(.a(new_n66_), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  nor2   g006(.a(x36), .b(x35), .O(new_n69_));
  nor3   g007(.a(new_n69_), .b(new_n68_), .c(x04), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n63_), .c(new_n67_), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x27), .O(new_n73_));
  nor2   g011(.a(new_n65_), .b(new_n73_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  oai22  g013(.a(new_n75_), .b(new_n71_), .c(new_n65_), .d(x16), .O(z00));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(new_n73_), .c(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g019(.a(new_n68_), .O(new_n82_));
  inv1   g020(.a(x35), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(x28), .O(new_n84_));
  oai21  g022(.a(x27), .b(x08), .c(new_n84_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x04), .O(new_n86_));
  and2   g024(.a(x29), .b(x08), .O(new_n87_));
  or2    g025(.a(new_n87_), .b(x02), .O(new_n88_));
  nand4  g026(.a(new_n88_), .b(new_n86_), .c(new_n82_), .d(new_n63_), .O(z02));
  inv1   g027(.a(x37), .O(new_n90_));
  nand2  g028(.a(x28), .b(x27), .O(new_n91_));
  oai22  g029(.a(new_n91_), .b(new_n83_), .c(new_n90_), .d(x27), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g031(.a(x21), .O(new_n94_));
  aoi21  g032(.a(new_n92_), .b(new_n94_), .c(new_n65_), .O(z04));
  aoi21  g033(.a(new_n91_), .b(new_n90_), .c(new_n65_), .O(z05));
  inv1   g034(.a(new_n65_), .O(new_n97_));
  nand3  g035(.a(new_n91_), .b(new_n97_), .c(new_n90_), .O(new_n98_));
  inv1   g036(.a(new_n98_), .O(z06));
  inv1   g037(.a(x14), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n72_), .O(new_n101_));
  nor2   g039(.a(x33), .b(x31), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n100_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n105_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n97_), .c(x03), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(new_n103_), .O(z07));
  nand3  g047(.a(x40), .b(x39), .c(new_n63_), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(z08));
  inv1   g049(.a(x11), .O(new_n112_));
  nand3  g050(.a(new_n74_), .b(x34), .c(x26), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n112_), .O(z09));
  nor2   g052(.a(new_n78_), .b(new_n73_), .O(new_n115_));
  nand2  g053(.a(new_n82_), .b(x05), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  and2   g055(.a(new_n68_), .b(x07), .O(new_n118_));
  aoi22  g056(.a(new_n118_), .b(new_n115_), .c(new_n117_), .d(new_n80_), .O(new_n119_));
  nand3  g057(.a(x37), .b(x27), .c(x06), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n65_), .O(new_n122_));
  oai21  g060(.a(new_n119_), .b(x04), .c(new_n122_), .O(z10));
  inv1   g061(.a(x08), .O(new_n124_));
  nand2  g062(.a(x35), .b(new_n77_), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n124_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  inv1   g067(.a(x04), .O(new_n130_));
  nand3  g068(.a(new_n84_), .b(x27), .c(new_n130_), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  nand4  g070(.a(new_n132_), .b(new_n131_), .c(new_n129_), .d(new_n97_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z11));
  oai21  g072(.a(new_n70_), .b(new_n67_), .c(new_n74_), .O(z12));
  inv1   g073(.a(x18), .O(new_n136_));
  inv1   g074(.a(x19), .O(new_n137_));
  nand3  g075(.a(x20), .b(new_n137_), .c(new_n136_), .O(new_n138_));
  nand2  g076(.a(x36), .b(x35), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(x28), .O(new_n140_));
  nor2   g078(.a(x13), .b(x04), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n82_), .O(new_n142_));
  oai21  g080(.a(new_n138_), .b(new_n66_), .c(new_n142_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x27), .O(new_n144_));
  inv1   g082(.a(new_n79_), .O(new_n145_));
  nand3  g083(.a(new_n141_), .b(new_n145_), .c(x39), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n63_), .c(new_n64_), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n144_), .O(z13));
  inv1   g087(.a(new_n138_), .O(new_n150_));
  nand3  g088(.a(new_n139_), .b(new_n79_), .c(x28), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n141_), .c(x39), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n63_), .c(new_n64_), .O(new_n153_));
  nand3  g091(.a(new_n141_), .b(new_n82_), .c(new_n77_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n66_), .c(new_n73_), .O(new_n155_));
  oai22  g093(.a(new_n155_), .b(new_n147_), .c(new_n153_), .d(new_n150_), .O(z14));
  inv1   g094(.a(x12), .O(new_n157_));
  nor2   g095(.a(new_n113_), .b(new_n157_), .O(z15));
  inv1   g096(.a(x23), .O(new_n159_));
  nand4  g097(.a(new_n97_), .b(new_n159_), .c(x22), .d(x01), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z16));
  inv1   g099(.a(x24), .O(new_n162_));
  nand4  g100(.a(new_n162_), .b(x23), .c(x22), .d(x01), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n97_), .O(z17));
  nor2   g102(.a(new_n73_), .b(new_n130_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n85_), .c(new_n132_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n97_), .O(new_n167_));
  nand2  g105(.a(z08), .b(new_n87_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(z18));
  nand3  g107(.a(new_n132_), .b(new_n131_), .c(new_n129_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n97_), .O(z19));
  oai21  g109(.a(new_n83_), .b(new_n73_), .c(new_n132_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n127_), .c(new_n97_), .O(z20));
endmodule


