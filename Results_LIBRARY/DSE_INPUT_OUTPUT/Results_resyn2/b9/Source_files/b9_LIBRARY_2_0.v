// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n154_, new_n156_, new_n158_, new_n159_, new_n160_, new_n163_,
    new_n164_;
  nor2   g000(.a(x21), .b(x17), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x27), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  inv1   g006(.a(x36), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  and2   g008(.a(x40), .b(x39), .O(new_n71_));
  nand3  g009(.a(new_n71_), .b(new_n70_), .c(new_n67_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n66_), .c(new_n65_), .O(new_n73_));
  nand3  g011(.a(new_n71_), .b(x10), .c(new_n67_), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n73_), .c(new_n64_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(x16), .c(new_n63_), .O(z00));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n69_), .c(new_n65_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g021(.a(new_n63_), .b(new_n67_), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n80_), .c(new_n84_), .O(z01));
  inv1   g023(.a(new_n63_), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n79_), .c(x04), .O(new_n88_));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n88_), .c(new_n71_), .d(new_n86_), .O(z02));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n68_), .c(new_n93_), .d(x27), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(x21), .O(z03));
  inv1   g034(.a(x17), .O(new_n97_));
  inv1   g035(.a(x21), .O(new_n98_));
  oai21  g036(.a(new_n95_), .b(new_n97_), .c(new_n98_), .O(z04));
  nand3  g037(.a(new_n94_), .b(new_n86_), .c(new_n93_), .O(z05));
  nand2  g038(.a(new_n94_), .b(new_n93_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n86_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  nor2   g045(.a(new_n97_), .b(x15), .O(new_n108_));
  or2    g046(.a(x33), .b(x31), .O(new_n109_));
  oai21  g047(.a(new_n109_), .b(new_n108_), .c(x14), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n107_), .c(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n86_), .O(z07));
  nand2  g050(.a(x40), .b(x39), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n86_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  nor3   g054(.a(new_n116_), .b(new_n63_), .c(new_n115_), .O(z09));
  nand2  g055(.a(new_n83_), .b(new_n71_), .O(new_n118_));
  nand2  g056(.a(new_n79_), .b(new_n69_), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  nand2  g058(.a(new_n113_), .b(new_n120_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n119_), .c(x27), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n118_), .O(new_n123_));
  inv1   g061(.a(x05), .O(new_n124_));
  aoi21  g062(.a(new_n71_), .b(new_n124_), .c(x04), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n126_), .c(new_n63_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(new_n71_), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n79_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  inv1   g071(.a(new_n79_), .O(new_n134_));
  nor2   g072(.a(new_n65_), .b(x04), .O(new_n135_));
  inv1   g073(.a(x09), .O(new_n136_));
  nand2  g074(.a(new_n81_), .b(new_n136_), .O(new_n137_));
  aoi21  g075(.a(new_n135_), .b(new_n134_), .c(new_n137_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n133_), .c(new_n86_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z11));
  inv1   g078(.a(new_n73_), .O(new_n141_));
  nand3  g079(.a(new_n74_), .b(new_n141_), .c(new_n86_), .O(new_n142_));
  inv1   g080(.a(new_n142_), .O(z12));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x20), .O(new_n145_));
  aoi21  g083(.a(x36), .b(x35), .c(new_n78_), .O(new_n146_));
  nor2   g084(.a(x13), .b(x04), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n71_), .O(new_n148_));
  oai22  g086(.a(new_n148_), .b(new_n146_), .c(new_n145_), .d(new_n66_), .O(new_n149_));
  nand3  g087(.a(new_n147_), .b(new_n83_), .c(new_n71_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n86_), .O(new_n151_));
  aoi21  g089(.a(new_n149_), .b(x27), .c(new_n151_), .O(z14));
  inv1   g090(.a(z14), .O(z13));
  inv1   g091(.a(x12), .O(new_n154_));
  oai21  g092(.a(new_n116_), .b(new_n154_), .c(new_n86_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  nor3   g094(.a(new_n156_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  inv1   g096(.a(new_n156_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n86_), .c(new_n158_), .d(x23), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z17));
  aoi21  g099(.a(new_n138_), .b(new_n133_), .c(new_n63_), .O(z18));
  nor2   g100(.a(new_n137_), .b(new_n63_), .O(new_n163_));
  oai21  g101(.a(new_n68_), .b(new_n65_), .c(new_n163_), .O(new_n164_));
  nor2   g102(.a(new_n164_), .b(new_n131_), .O(z20));
  inv1   g103(.a(new_n139_), .O(z19));
endmodule


