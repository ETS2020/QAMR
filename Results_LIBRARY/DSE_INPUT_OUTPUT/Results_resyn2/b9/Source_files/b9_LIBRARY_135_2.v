// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n154_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n167_;
  inv1   g000(.a(x40), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x39), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x35), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n74_), .c(new_n68_), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(x10), .c(new_n73_), .O(new_n77_));
  inv1   g015(.a(x39), .O(new_n78_));
  nor2   g016(.a(new_n63_), .b(new_n78_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  oai22  g018(.a(new_n80_), .b(new_n77_), .c(new_n72_), .d(new_n65_), .O(z00));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n75_), .c(new_n68_), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  inv1   g023(.a(new_n85_), .O(new_n86_));
  nor2   g024(.a(new_n65_), .b(new_n73_), .O(new_n87_));
  oai21  g025(.a(new_n86_), .b(new_n84_), .c(new_n87_), .O(z01));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor2   g027(.a(new_n74_), .b(x28), .O(new_n90_));
  inv1   g028(.a(x08), .O(new_n91_));
  nand2  g029(.a(new_n68_), .b(new_n91_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x04), .c(new_n89_), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n63_), .c(x39), .O(z02));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  oai22  g034(.a(new_n96_), .b(new_n74_), .c(new_n69_), .d(x27), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n64_), .c(new_n99_), .O(z04));
  nand3  g038(.a(new_n96_), .b(new_n64_), .c(new_n69_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n66_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n103_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n64_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  nand3  g050(.a(x34), .b(x27), .c(x26), .O(new_n113_));
  nand2  g051(.a(new_n64_), .b(x11), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  inv1   g053(.a(x07), .O(new_n116_));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  oai21  g055(.a(x39), .b(new_n116_), .c(new_n117_), .O(new_n118_));
  aoi21  g056(.a(new_n83_), .b(new_n75_), .c(x04), .O(new_n119_));
  nand2  g057(.a(x37), .b(x06), .O(new_n120_));
  aoi21  g058(.a(new_n63_), .b(x39), .c(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n119_), .b(new_n118_), .c(new_n121_), .O(new_n122_));
  inv1   g060(.a(new_n117_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n86_), .c(new_n73_), .O(new_n124_));
  oai21  g062(.a(new_n122_), .b(new_n68_), .c(new_n124_), .O(z10));
  nand2  g063(.a(x39), .b(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n83_), .c(new_n91_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g067(.a(new_n90_), .b(x27), .c(new_n73_), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n64_), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n130_), .c(new_n129_), .O(z18));
  inv1   g072(.a(z18), .O(z11));
  nand2  g073(.a(new_n77_), .b(x40), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x39), .c(new_n71_), .O(z12));
  nand2  g075(.a(new_n70_), .b(new_n69_), .O(new_n138_));
  nor2   g076(.a(x19), .b(x18), .O(new_n139_));
  nand4  g077(.a(new_n139_), .b(new_n138_), .c(new_n64_), .d(x20), .O(new_n140_));
  oai21  g078(.a(new_n75_), .b(new_n74_), .c(x28), .O(new_n141_));
  nor2   g079(.a(x13), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(new_n79_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  nand3  g083(.a(new_n142_), .b(new_n86_), .c(new_n79_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n145_), .O(z13));
  nand3  g085(.a(new_n139_), .b(new_n138_), .c(x20), .O(new_n148_));
  nand3  g086(.a(new_n142_), .b(new_n141_), .c(x39), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand2  g088(.a(new_n142_), .b(new_n86_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(x40), .c(new_n78_), .O(new_n152_));
  aoi21  g090(.a(new_n150_), .b(x27), .c(new_n152_), .O(z14));
  inv1   g091(.a(x12), .O(new_n154_));
  oai21  g092(.a(new_n113_), .b(new_n154_), .c(new_n64_), .O(z15));
  inv1   g093(.a(x23), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(x22), .c(x01), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n64_), .O(z16));
  nor2   g096(.a(x24), .b(new_n156_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n64_), .c(x22), .d(x01), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z17));
  nor2   g099(.a(new_n68_), .b(new_n73_), .O(new_n162_));
  oai21  g100(.a(new_n126_), .b(new_n91_), .c(new_n131_), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(new_n164_));
  oai21  g102(.a(new_n162_), .b(new_n93_), .c(new_n164_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n64_), .O(z19));
  nor2   g104(.a(new_n74_), .b(new_n68_), .O(new_n167_));
  nor3   g105(.a(new_n167_), .b(new_n132_), .c(new_n127_), .O(z20));
  buf    g106(.a(x39), .O(z08));
endmodule


