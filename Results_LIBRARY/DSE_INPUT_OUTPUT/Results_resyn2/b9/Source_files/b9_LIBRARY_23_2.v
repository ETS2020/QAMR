// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n172_, new_n173_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x33), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x27), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x28), .O(new_n74_));
  aoi21  g012(.a(x35), .b(new_n74_), .c(x36), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n63_), .c(x33), .O(new_n76_));
  nor2   g014(.a(x32), .b(x30), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  aoi22  g016(.a(new_n78_), .b(x04), .c(new_n76_), .d(x27), .O(z01));
  inv1   g017(.a(new_n71_), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n74_), .O(new_n81_));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n81_), .c(x04), .O(new_n83_));
  nand2  g021(.a(x40), .b(x39), .O(new_n84_));
  aoi21  g022(.a(x29), .b(x08), .c(x02), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(new_n83_), .c(new_n80_), .O(z02));
  nand2  g025(.a(x35), .b(x28), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x27), .O(new_n89_));
  inv1   g027(.a(x21), .O(new_n90_));
  inv1   g028(.a(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n89_), .c(new_n80_), .O(z03));
  oai21  g032(.a(new_n88_), .b(new_n70_), .c(x27), .O(new_n95_));
  aoi21  g033(.a(new_n92_), .b(new_n91_), .c(x21), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n95_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n80_), .O(z05));
  nand2  g037(.a(x33), .b(new_n74_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(x27), .c(x37), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n102_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n71_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  nor2   g050(.a(new_n80_), .b(new_n84_), .O(z08));
  nand3  g051(.a(x34), .b(x26), .c(x11), .O(new_n114_));
  aoi21  g052(.a(new_n114_), .b(x33), .c(new_n91_), .O(z09));
  nor2   g053(.a(new_n75_), .b(x04), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  nor2   g055(.a(new_n84_), .b(x05), .O(new_n118_));
  aoi21  g056(.a(new_n84_), .b(new_n117_), .c(new_n118_), .O(new_n119_));
  nand2  g057(.a(x37), .b(x06), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(x33), .O(new_n121_));
  aoi21  g059(.a(new_n119_), .b(new_n116_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n78_), .b(new_n65_), .c(x05), .O(new_n123_));
  oai21  g061(.a(new_n122_), .b(new_n91_), .c(new_n123_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n81_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  oai21  g067(.a(new_n81_), .b(x04), .c(x33), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x27), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n129_), .O(z18));
  inv1   g071(.a(z18), .O(z11));
  aoi21  g072(.a(new_n67_), .b(new_n92_), .c(new_n91_), .O(new_n135_));
  nand2  g073(.a(new_n88_), .b(new_n92_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x27), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n64_), .c(new_n80_), .O(new_n138_));
  oai21  g076(.a(new_n135_), .b(x10), .c(new_n138_), .O(z12));
  inv1   g077(.a(new_n84_), .O(new_n140_));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  nor2   g080(.a(x13), .b(x04), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n142_), .c(new_n140_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n136_), .c(x20), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n144_), .c(new_n91_), .O(new_n147_));
  nand4  g085(.a(new_n143_), .b(new_n78_), .c(new_n140_), .d(x28), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n147_), .c(x33), .O(new_n150_));
  nand4  g088(.a(new_n143_), .b(new_n78_), .c(new_n140_), .d(new_n91_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(z13));
  nand2  g090(.a(new_n145_), .b(x20), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  inv1   g092(.a(new_n142_), .O(new_n155_));
  nand2  g093(.a(new_n143_), .b(new_n140_), .O(new_n156_));
  aoi21  g094(.a(new_n155_), .b(new_n77_), .c(new_n156_), .O(new_n157_));
  nand2  g095(.a(new_n78_), .b(new_n71_), .O(new_n158_));
  nand3  g096(.a(x33), .b(new_n74_), .c(x27), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n158_), .c(new_n156_), .O(new_n160_));
  nor2   g098(.a(new_n137_), .b(new_n70_), .O(new_n161_));
  oai22  g099(.a(new_n161_), .b(new_n160_), .c(new_n157_), .d(new_n154_), .O(z14));
  nand3  g100(.a(x34), .b(x26), .c(x12), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n70_), .c(new_n91_), .O(z15));
  inv1   g102(.a(x01), .O(new_n165_));
  inv1   g103(.a(x22), .O(new_n166_));
  nor4   g104(.a(new_n80_), .b(x23), .c(new_n166_), .d(new_n165_), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  nor2   g106(.a(new_n166_), .b(new_n165_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(new_n71_), .c(new_n168_), .d(x23), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z17));
  nand2  g109(.a(x35), .b(x27), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n132_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n127_), .c(new_n71_), .O(z20));
  inv1   g112(.a(z18), .O(z19));
endmodule


