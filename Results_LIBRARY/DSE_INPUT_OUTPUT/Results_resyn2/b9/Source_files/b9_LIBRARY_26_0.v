// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n164_, new_n166_, new_n167_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n175_, new_n176_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand2  g002(.a(x39), .b(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  inv1   g005(.a(x10), .O(new_n68_));
  oai21  g006(.a(new_n65_), .b(new_n68_), .c(x40), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(x15), .c(x16), .O(z00));
  inv1   g009(.a(x27), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  inv1   g011(.a(x28), .O(new_n74_));
  nand2  g012(.a(x35), .b(new_n74_), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n73_), .c(new_n72_), .O(new_n76_));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g017(.a(x40), .b(x15), .O(new_n80_));
  nor2   g018(.a(new_n80_), .b(new_n64_), .O(new_n81_));
  oai21  g019(.a(new_n79_), .b(new_n76_), .c(new_n81_), .O(z01));
  inv1   g020(.a(x35), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(x28), .O(new_n84_));
  inv1   g022(.a(x08), .O(new_n85_));
  nand2  g023(.a(new_n72_), .b(new_n85_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x04), .O(new_n88_));
  nand2  g026(.a(x40), .b(x39), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n88_), .c(new_n80_), .O(z02));
  nand2  g030(.a(x35), .b(x28), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n72_), .c(new_n80_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n94_), .c(new_n98_), .O(z04));
  aoi21  g037(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n80_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(new_n80_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n103_), .O(z07));
  nand2  g052(.a(new_n89_), .b(new_n103_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n103_), .O(z09));
  nor2   g055(.a(x32), .b(x30), .O(new_n118_));
  nor2   g056(.a(new_n89_), .b(new_n118_), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  nand2  g058(.a(new_n89_), .b(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n76_), .c(new_n119_), .O(new_n122_));
  oai21  g060(.a(new_n89_), .b(x05), .c(new_n64_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n80_), .O(new_n126_));
  oai21  g064(.a(new_n123_), .b(new_n122_), .c(new_n126_), .O(z10));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n75_), .c(new_n85_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g069(.a(new_n84_), .b(x27), .c(new_n64_), .O(new_n132_));
  nor2   g070(.a(x30), .b(x09), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(new_n131_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n103_), .O(z11));
  aoi21  g073(.a(new_n63_), .b(new_n95_), .c(new_n72_), .O(new_n136_));
  inv1   g074(.a(x40), .O(new_n137_));
  nand2  g075(.a(new_n93_), .b(new_n95_), .O(new_n138_));
  nand3  g076(.a(new_n103_), .b(new_n138_), .c(x27), .O(new_n139_));
  oai21  g077(.a(new_n65_), .b(new_n137_), .c(new_n139_), .O(new_n140_));
  oai21  g078(.a(new_n136_), .b(x10), .c(new_n140_), .O(z12));
  inv1   g079(.a(new_n89_), .O(new_n142_));
  oai21  g080(.a(new_n73_), .b(new_n83_), .c(x28), .O(new_n143_));
  nor2   g081(.a(x13), .b(x04), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n143_), .c(new_n142_), .O(new_n145_));
  inv1   g083(.a(x18), .O(new_n146_));
  inv1   g084(.a(x19), .O(new_n147_));
  nand4  g085(.a(new_n138_), .b(x20), .c(new_n147_), .d(new_n146_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  aoi21  g088(.a(new_n144_), .b(new_n119_), .c(new_n80_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(z13));
  nand3  g090(.a(x20), .b(new_n147_), .c(new_n146_), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  inv1   g092(.a(new_n143_), .O(new_n155_));
  nand2  g093(.a(new_n144_), .b(new_n142_), .O(new_n156_));
  aoi21  g094(.a(new_n155_), .b(new_n118_), .c(new_n156_), .O(new_n157_));
  aoi21  g095(.a(new_n74_), .b(x27), .c(new_n79_), .O(new_n158_));
  nand3  g096(.a(new_n144_), .b(new_n142_), .c(new_n103_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n158_), .c(new_n139_), .O(new_n160_));
  oai21  g098(.a(new_n157_), .b(new_n154_), .c(new_n160_), .O(z14));
  nand3  g099(.a(x34), .b(x26), .c(x12), .O(new_n162_));
  nor3   g100(.a(new_n162_), .b(new_n80_), .c(new_n72_), .O(z15));
  nand2  g101(.a(x22), .b(x01), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(x23), .c(new_n103_), .O(z16));
  inv1   g103(.a(x24), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x23), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n164_), .c(new_n103_), .O(z17));
  nand4  g106(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n103_), .O(z18));
  inv1   g107(.a(new_n128_), .O(new_n170_));
  nand2  g108(.a(x27), .b(x04), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n86_), .c(new_n84_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n133_), .O(new_n173_));
  aoi22  g111(.a(new_n173_), .b(new_n103_), .c(new_n170_), .d(x08), .O(z19));
  nand2  g112(.a(x35), .b(x27), .O(new_n175_));
  nand3  g113(.a(new_n175_), .b(new_n133_), .c(new_n103_), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(new_n129_), .O(z20));
endmodule


