// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n142_, new_n143_, new_n144_, new_n145_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n160_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n168_, new_n170_, new_n171_, new_n173_, new_n175_, new_n177_;
  inv1   g000(.a(x19), .O(new_n63_));
  inv1   g001(.a(x36), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x27), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n72_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n71_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n66_), .c(new_n67_), .O(new_n77_));
  nor2   g015(.a(new_n73_), .b(new_n65_), .O(new_n78_));
  inv1   g016(.a(x27), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(x04), .O(new_n80_));
  inv1   g018(.a(x35), .O(new_n81_));
  aoi21  g019(.a(new_n64_), .b(new_n81_), .c(x15), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n80_), .c(new_n78_), .O(new_n83_));
  oai21  g021(.a(new_n77_), .b(new_n65_), .c(new_n83_), .O(z00));
  inv1   g022(.a(new_n65_), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n64_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x27), .O(new_n89_));
  nor2   g027(.a(x32), .b(x30), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g029(.a(new_n91_), .b(new_n85_), .c(x04), .O(z01));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n87_), .c(x04), .O(new_n94_));
  inv1   g032(.a(x02), .O(new_n95_));
  nand2  g033(.a(x29), .b(x08), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n94_), .c(new_n78_), .O(z02));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  oai22  g037(.a(new_n99_), .b(new_n81_), .c(new_n68_), .d(x27), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g039(.a(x21), .O(new_n102_));
  nand3  g040(.a(new_n100_), .b(new_n85_), .c(new_n102_), .O(z04));
  aoi21  g041(.a(new_n99_), .b(new_n68_), .c(new_n65_), .O(z05));
  nand3  g042(.a(new_n99_), .b(new_n85_), .c(new_n68_), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(z06));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n66_), .O(new_n108_));
  nor2   g046(.a(x33), .b(x31), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  nand2  g050(.a(x25), .b(new_n107_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n85_), .c(x03), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n110_), .O(z07));
  nand2  g054(.a(new_n73_), .b(new_n85_), .O(z08));
  inv1   g055(.a(x11), .O(new_n118_));
  nand4  g056(.a(new_n85_), .b(x34), .c(x27), .d(x26), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n118_), .O(z09));
  inv1   g058(.a(x05), .O(new_n121_));
  nand2  g059(.a(new_n74_), .b(new_n121_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n73_), .b(new_n123_), .O(new_n124_));
  nand4  g062(.a(new_n124_), .b(new_n122_), .c(new_n88_), .d(new_n80_), .O(new_n125_));
  nor2   g063(.a(new_n73_), .b(x04), .O(new_n126_));
  nor2   g064(.a(new_n90_), .b(new_n121_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  aoi21  g067(.a(new_n127_), .b(new_n126_), .c(new_n129_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n125_), .c(new_n65_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n87_), .c(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g074(.a(new_n80_), .b(x35), .c(new_n86_), .O(new_n137_));
  nor2   g075(.a(x30), .b(x09), .O(new_n138_));
  and2   g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n136_), .c(new_n85_), .O(z18));
  inv1   g078(.a(z18), .O(z11));
  inv1   g079(.a(x10), .O(new_n142_));
  nor3   g080(.a(x37), .b(x36), .c(x35), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n79_), .c(new_n142_), .O(new_n144_));
  oai21  g082(.a(new_n73_), .b(x04), .c(new_n71_), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n144_), .c(new_n85_), .O(z12));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x28), .O(new_n148_));
  nor2   g086(.a(x13), .b(x04), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n74_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  nand3  g089(.a(new_n70_), .b(x20), .c(new_n151_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n150_), .c(new_n79_), .O(new_n153_));
  nand2  g091(.a(new_n149_), .b(new_n74_), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n90_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n153_), .c(new_n63_), .O(new_n156_));
  oai21  g094(.a(x28), .b(new_n79_), .c(new_n90_), .O(new_n157_));
  nand4  g095(.a(new_n157_), .b(new_n149_), .c(new_n74_), .d(new_n64_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n156_), .O(z13));
  nand2  g097(.a(x20), .b(new_n151_), .O(new_n160_));
  nand3  g098(.a(new_n147_), .b(new_n90_), .c(x28), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n154_), .c(new_n160_), .O(new_n163_));
  nand3  g101(.a(new_n70_), .b(x27), .c(new_n63_), .O(new_n164_));
  nand3  g102(.a(new_n157_), .b(new_n149_), .c(new_n78_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n163_), .O(z14));
  inv1   g105(.a(x12), .O(new_n168_));
  nor2   g106(.a(new_n119_), .b(new_n168_), .O(z15));
  inv1   g107(.a(x23), .O(new_n170_));
  nand4  g108(.a(new_n85_), .b(new_n170_), .c(x22), .d(x01), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z16));
  nand2  g110(.a(x22), .b(x01), .O(new_n173_));
  nor4   g111(.a(new_n173_), .b(new_n65_), .c(x24), .d(new_n170_), .O(z17));
  nand2  g112(.a(new_n139_), .b(new_n136_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n85_), .O(z19));
  oai21  g114(.a(new_n81_), .b(new_n79_), .c(new_n138_), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n134_), .c(new_n85_), .O(z20));
endmodule


