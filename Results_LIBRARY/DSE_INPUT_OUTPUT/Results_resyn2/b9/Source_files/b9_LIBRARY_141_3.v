// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n195_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  inv1   g009(.a(x21), .O(new_n72_));
  inv1   g010(.a(x40), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  oai21  g013(.a(new_n70_), .b(x15), .c(new_n75_), .O(z00));
  inv1   g014(.a(x27), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n78_), .c(new_n77_), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nor2   g021(.a(new_n74_), .b(new_n64_), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n81_), .c(new_n84_), .O(z01));
  inv1   g023(.a(x35), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(x28), .O(new_n87_));
  oai21  g025(.a(x27), .b(x08), .c(new_n87_), .O(new_n88_));
  and2   g026(.a(x29), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(x02), .c(x39), .O(new_n90_));
  aoi21  g028(.a(new_n88_), .b(x04), .c(new_n90_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(x21), .c(x40), .O(z02));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n86_), .c(new_n93_), .d(x27), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(x40), .c(x21), .O(z03));
  nand2  g034(.a(new_n95_), .b(new_n72_), .O(z04));
  inv1   g035(.a(new_n74_), .O(new_n98_));
  nand3  g036(.a(new_n94_), .b(new_n98_), .c(new_n93_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n98_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  nand3  g049(.a(x40), .b(x39), .c(new_n72_), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z08));
  inv1   g051(.a(x11), .O(new_n114_));
  nand3  g052(.a(x34), .b(x27), .c(x26), .O(new_n115_));
  nor3   g053(.a(new_n115_), .b(new_n74_), .c(new_n114_), .O(z09));
  inv1   g054(.a(x39), .O(new_n117_));
  nor2   g055(.a(new_n73_), .b(new_n117_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n83_), .O(new_n119_));
  nand2  g057(.a(new_n80_), .b(new_n78_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  oai21  g059(.a(new_n73_), .b(new_n117_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n120_), .c(x27), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  inv1   g062(.a(x05), .O(new_n125_));
  aoi21  g063(.a(new_n118_), .b(new_n125_), .c(x04), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand3  g065(.a(x37), .b(x27), .c(x06), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(new_n98_), .O(z10));
  nor2   g067(.a(x30), .b(x09), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(new_n130_), .b(new_n132_), .c(x04), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n72_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x40), .O(new_n135_));
  nand3  g073(.a(x40), .b(x39), .c(x29), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x08), .O(new_n138_));
  nand3  g076(.a(new_n136_), .b(x27), .c(x04), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(new_n140_));
  aoi21  g078(.a(new_n138_), .b(new_n88_), .c(new_n140_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n131_), .c(new_n135_), .O(z11));
  nand2  g080(.a(new_n63_), .b(new_n93_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(x10), .O(new_n144_));
  nor2   g082(.a(new_n117_), .b(x04), .O(new_n145_));
  nor2   g083(.a(new_n66_), .b(new_n77_), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n144_), .c(new_n72_), .O(new_n148_));
  nor2   g086(.a(new_n146_), .b(x40), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n148_), .O(z12));
  inv1   g089(.a(new_n66_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand3  g092(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n152_), .O(new_n157_));
  nand2  g095(.a(x36), .b(x35), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x28), .O(new_n159_));
  nor2   g097(.a(x13), .b(x04), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(new_n159_), .c(new_n118_), .d(new_n72_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n157_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n98_), .c(x27), .O(new_n163_));
  nand4  g101(.a(new_n160_), .b(new_n118_), .c(new_n83_), .d(new_n72_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n163_), .O(z13));
  nand3  g103(.a(new_n158_), .b(new_n82_), .c(x28), .O(new_n166_));
  inv1   g104(.a(x13), .O(new_n167_));
  nand3  g105(.a(x39), .b(new_n167_), .c(new_n64_), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(new_n166_), .c(x21), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n73_), .c(new_n155_), .O(new_n171_));
  oai21  g109(.a(x28), .b(new_n77_), .c(new_n82_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n169_), .O(new_n173_));
  nor2   g111(.a(new_n146_), .b(x21), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n173_), .c(new_n149_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n171_), .O(z14));
  inv1   g114(.a(x12), .O(new_n177_));
  oai21  g115(.a(new_n115_), .b(new_n177_), .c(new_n98_), .O(z15));
  inv1   g116(.a(x23), .O(new_n179_));
  nand4  g117(.a(new_n98_), .b(new_n179_), .c(x22), .d(x01), .O(new_n180_));
  inv1   g118(.a(new_n180_), .O(z16));
  inv1   g119(.a(x24), .O(new_n182_));
  nand4  g120(.a(new_n182_), .b(x23), .c(x22), .d(x01), .O(new_n183_));
  nand2  g121(.a(new_n183_), .b(new_n98_), .O(z17));
  aoi21  g122(.a(new_n77_), .b(new_n132_), .c(new_n80_), .O(new_n185_));
  nand2  g123(.a(x27), .b(x04), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n185_), .c(new_n131_), .O(new_n187_));
  nand2  g125(.a(z08), .b(new_n89_), .O(new_n188_));
  oai21  g126(.a(new_n187_), .b(new_n74_), .c(new_n188_), .O(z18));
  aoi21  g127(.a(new_n136_), .b(new_n80_), .c(new_n132_), .O(new_n190_));
  nand2  g128(.a(new_n136_), .b(x27), .O(new_n191_));
  nand3  g129(.a(new_n87_), .b(x27), .c(new_n64_), .O(new_n192_));
  nand3  g130(.a(new_n192_), .b(new_n130_), .c(new_n98_), .O(new_n193_));
  aoi21  g131(.a(new_n191_), .b(new_n190_), .c(new_n193_), .O(z19));
  oai21  g132(.a(new_n86_), .b(new_n77_), .c(new_n130_), .O(new_n195_));
  oai21  g133(.a(new_n195_), .b(new_n190_), .c(new_n98_), .O(z20));
endmodule


