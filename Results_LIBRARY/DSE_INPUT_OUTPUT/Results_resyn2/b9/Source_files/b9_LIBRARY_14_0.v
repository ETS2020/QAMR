// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n168_, new_n170_, new_n171_, new_n173_, new_n176_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  inv1   g006(.a(new_n66_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x10), .O(new_n70_));
  and2   g008(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  inv1   g009(.a(x16), .O(new_n72_));
  nor2   g010(.a(x36), .b(x30), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai21  g012(.a(new_n71_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x32), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(x36), .c(x30), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x04), .O(z01));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n82_), .c(x04), .O(new_n85_));
  nand2  g023(.a(x40), .b(x39), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n85_), .c(new_n73_), .O(z02));
  inv1   g027(.a(new_n73_), .O(new_n90_));
  inv1   g028(.a(x35), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n83_), .c(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n76_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n90_), .d(x21), .O(z03));
  aoi21  g033(.a(new_n93_), .b(new_n76_), .c(x21), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n92_), .c(new_n73_), .O(z04));
  nand2  g035(.a(new_n90_), .b(new_n93_), .O(new_n98_));
  aoi21  g036(.a(x28), .b(x27), .c(new_n98_), .O(z06));
  inv1   g037(.a(z06), .O(z05));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n90_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  nand2  g049(.a(new_n90_), .b(new_n86_), .O(z08));
  nand2  g050(.a(x34), .b(x27), .O(new_n113_));
  nand2  g051(.a(x26), .b(x11), .O(new_n114_));
  oai21  g052(.a(new_n114_), .b(new_n113_), .c(new_n90_), .O(z09));
  inv1   g053(.a(x36), .O(new_n116_));
  nand3  g054(.a(x35), .b(x30), .c(new_n83_), .O(new_n117_));
  nand2  g055(.a(new_n86_), .b(x07), .O(new_n118_));
  aoi21  g056(.a(new_n117_), .b(new_n116_), .c(new_n118_), .O(new_n119_));
  nand4  g057(.a(x40), .b(x39), .c(x36), .d(x05), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(new_n121_));
  oai21  g059(.a(new_n121_), .b(new_n119_), .c(new_n65_), .O(new_n122_));
  nand3  g060(.a(new_n90_), .b(x37), .c(x06), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x27), .O(new_n125_));
  inv1   g063(.a(x30), .O(new_n126_));
  oai21  g064(.a(new_n116_), .b(new_n77_), .c(new_n126_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n69_), .c(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n125_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  nand3  g068(.a(x40), .b(x39), .c(x29), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n84_), .c(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n131_), .b(x27), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(new_n135_));
  nand4  g073(.a(x35), .b(new_n83_), .c(x27), .d(new_n65_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(new_n137_));
  inv1   g075(.a(x09), .O(new_n138_));
  nand3  g076(.a(x36), .b(new_n126_), .c(new_n138_), .O(new_n139_));
  nor3   g077(.a(new_n139_), .b(new_n137_), .c(new_n135_), .O(z11));
  nand3  g078(.a(new_n90_), .b(new_n70_), .c(new_n68_), .O(new_n141_));
  inv1   g079(.a(new_n141_), .O(z12));
  inv1   g080(.a(x18), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand3  g082(.a(x20), .b(new_n144_), .c(new_n143_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  nor2   g084(.a(new_n73_), .b(new_n63_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(new_n69_), .b(new_n149_), .O(new_n150_));
  oai21  g088(.a(x35), .b(new_n83_), .c(x36), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n148_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand3  g091(.a(new_n127_), .b(new_n69_), .c(new_n149_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  nand2  g093(.a(new_n150_), .b(new_n93_), .O(new_n156_));
  nand3  g094(.a(new_n77_), .b(new_n126_), .c(x28), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(new_n158_));
  oai21  g096(.a(new_n145_), .b(new_n93_), .c(new_n158_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n156_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n91_), .O(new_n161_));
  aoi21  g099(.a(new_n93_), .b(new_n83_), .c(new_n76_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n146_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n150_), .c(new_n79_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(z14));
  nand2  g103(.a(x26), .b(x12), .O(new_n166_));
  nor3   g104(.a(new_n166_), .b(new_n113_), .c(new_n73_), .O(z15));
  nand2  g105(.a(x22), .b(x01), .O(new_n168_));
  nor3   g106(.a(new_n168_), .b(new_n73_), .c(x23), .O(z16));
  inv1   g107(.a(x24), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x23), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n168_), .c(new_n90_), .O(z17));
  nand3  g110(.a(new_n136_), .b(new_n134_), .c(new_n138_), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(x36), .c(x30), .O(z19));
  inv1   g112(.a(z19), .O(z18));
  nor2   g113(.a(new_n91_), .b(new_n76_), .O(new_n176_));
  nor3   g114(.a(new_n139_), .b(new_n176_), .c(new_n132_), .O(z20));
endmodule


