// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n99_, new_n100_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n164_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n174_, new_n175_,
    new_n176_, new_n178_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x35), .O(new_n64_));
  nor2   g002(.a(x36), .b(new_n64_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  nand2  g004(.a(x37), .b(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  nand4  g006(.a(x40), .b(x39), .c(x10), .d(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n67_), .c(x15), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n63_), .c(new_n66_), .O(new_n71_));
  nand2  g009(.a(x35), .b(x28), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(x04), .O(new_n75_));
  nand2  g013(.a(x36), .b(x27), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(x15), .O(new_n77_));
  oai21  g015(.a(new_n75_), .b(new_n73_), .c(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n71_), .O(z00));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n76_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n65_), .O(z01));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x28), .c(x36), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x35), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x04), .O(new_n86_));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand3  g026(.a(new_n88_), .b(new_n66_), .c(new_n87_), .O(new_n89_));
  nand2  g027(.a(new_n74_), .b(new_n66_), .O(z08));
  nand3  g028(.a(z08), .b(new_n89_), .c(new_n86_), .O(z02));
  aoi21  g029(.a(new_n72_), .b(x27), .c(new_n65_), .O(new_n92_));
  inv1   g030(.a(x27), .O(new_n93_));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n92_), .c(new_n97_), .O(z04));
  inv1   g036(.a(x28), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n93_), .O(new_n100_));
  nor3   g038(.a(new_n100_), .b(new_n65_), .c(x37), .O(z06));
  inv1   g039(.a(z06), .O(z05));
  inv1   g040(.a(x14), .O(new_n103_));
  inv1   g041(.a(x15), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n104_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n103_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n66_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  inv1   g051(.a(x11), .O(new_n114_));
  nand3  g052(.a(x34), .b(x27), .c(x26), .O(new_n115_));
  nor3   g053(.a(new_n115_), .b(new_n65_), .c(new_n114_), .O(z09));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  aoi21  g055(.a(new_n80_), .b(new_n76_), .c(new_n117_), .O(new_n118_));
  nand4  g056(.a(new_n74_), .b(x36), .c(x27), .d(x07), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  oai21  g058(.a(new_n120_), .b(new_n118_), .c(new_n68_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n121_), .c(new_n66_), .O(z10));
  nand3  g061(.a(x40), .b(x39), .c(x29), .O(new_n124_));
  inv1   g062(.a(x36), .O(new_n125_));
  aoi21  g063(.a(x27), .b(x04), .c(x28), .O(new_n126_));
  oai21  g064(.a(new_n126_), .b(new_n125_), .c(x35), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  inv1   g066(.a(x08), .O(new_n129_));
  nand3  g067(.a(new_n99_), .b(x27), .c(new_n68_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x36), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x35), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(new_n135_));
  aoi21  g073(.a(new_n133_), .b(new_n128_), .c(new_n135_), .O(z11));
  aoi21  g074(.a(x36), .b(x27), .c(x10), .O(new_n137_));
  nor3   g075(.a(new_n137_), .b(new_n74_), .c(x04), .O(new_n138_));
  aoi21  g076(.a(new_n72_), .b(new_n94_), .c(new_n93_), .O(new_n139_));
  nor3   g077(.a(new_n139_), .b(new_n138_), .c(new_n65_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  nand2  g079(.a(new_n99_), .b(x27), .O(new_n142_));
  nand2  g080(.a(x35), .b(x27), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n142_), .c(new_n80_), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(new_n75_), .c(new_n66_), .d(new_n141_), .O(new_n145_));
  inv1   g083(.a(x18), .O(new_n146_));
  inv1   g084(.a(x19), .O(new_n147_));
  nand3  g085(.a(x20), .b(new_n147_), .c(new_n146_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n139_), .c(new_n66_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n145_), .O(z13));
  nand2  g089(.a(new_n80_), .b(x28), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n149_), .c(x37), .O(new_n153_));
  nand3  g091(.a(new_n152_), .b(new_n75_), .c(new_n141_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n153_), .c(new_n64_), .O(new_n155_));
  nand2  g093(.a(new_n80_), .b(new_n93_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n75_), .c(new_n141_), .O(new_n157_));
  aoi21  g095(.a(new_n94_), .b(new_n99_), .c(new_n93_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n149_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n157_), .c(new_n65_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n155_), .O(z14));
  inv1   g099(.a(x12), .O(new_n162_));
  oai21  g100(.a(new_n115_), .b(new_n162_), .c(new_n66_), .O(z15));
  nand2  g101(.a(x22), .b(x01), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(x23), .c(new_n66_), .O(z16));
  inv1   g103(.a(x24), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(x23), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n164_), .c(new_n66_), .O(z17));
  nand2  g106(.a(x35), .b(new_n99_), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(new_n124_), .c(new_n129_), .O(new_n170_));
  nand2  g108(.a(new_n124_), .b(x27), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n134_), .c(new_n132_), .O(z18));
  nand4  g111(.a(x35), .b(new_n99_), .c(x27), .d(new_n68_), .O(new_n174_));
  nand2  g112(.a(new_n126_), .b(x35), .O(new_n175_));
  aoi22  g113(.a(new_n175_), .b(new_n124_), .c(new_n174_), .d(new_n129_), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n135_), .c(new_n66_), .O(z19));
  nand2  g115(.a(new_n143_), .b(new_n134_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n170_), .c(new_n66_), .O(z20));
endmodule


