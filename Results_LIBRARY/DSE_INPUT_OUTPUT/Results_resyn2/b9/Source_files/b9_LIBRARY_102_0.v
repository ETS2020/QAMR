// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n94_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n161_,
    new_n163_, new_n164_, new_n166_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  nor2   g008(.a(x36), .b(x34), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x27), .O(new_n74_));
  nor2   g012(.a(x32), .b(x30), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  aoi21  g014(.a(x35), .b(new_n76_), .c(x36), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(new_n74_), .c(new_n75_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(x04), .c(new_n71_), .O(z01));
  nand2  g017(.a(x35), .b(new_n76_), .O(new_n80_));
  nor2   g018(.a(x27), .b(x08), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(new_n80_), .c(x04), .O(new_n82_));
  nand2  g020(.a(x40), .b(x39), .O(new_n83_));
  aoi21  g021(.a(x29), .b(x08), .c(x02), .O(new_n84_));
  nor2   g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  aoi21  g023(.a(new_n85_), .b(new_n82_), .c(new_n71_), .O(z02));
  inv1   g024(.a(x35), .O(new_n87_));
  inv1   g025(.a(x37), .O(new_n88_));
  nand2  g026(.a(x28), .b(x27), .O(new_n89_));
  oai22  g027(.a(new_n89_), .b(new_n87_), .c(new_n88_), .d(x27), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(x21), .c(new_n71_), .O(z03));
  nor2   g029(.a(new_n71_), .b(x21), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n90_), .O(z04));
  inv1   g031(.a(new_n71_), .O(new_n94_));
  nand3  g032(.a(new_n89_), .b(new_n94_), .c(new_n88_), .O(z05));
  nand2  g033(.a(new_n89_), .b(new_n88_), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n94_), .O(z06));
  inv1   g035(.a(x25), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(x00), .O(new_n99_));
  inv1   g037(.a(x14), .O(new_n100_));
  nand2  g038(.a(x25), .b(new_n100_), .O(new_n101_));
  nand3  g039(.a(new_n101_), .b(new_n99_), .c(x38), .O(new_n102_));
  inv1   g040(.a(x17), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(x15), .O(new_n104_));
  or2    g042(.a(x33), .b(x31), .O(new_n105_));
  oai21  g043(.a(new_n105_), .b(new_n104_), .c(x14), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n102_), .c(x03), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(new_n94_), .O(z07));
  nand2  g046(.a(new_n94_), .b(new_n83_), .O(z08));
  nand4  g047(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n94_), .O(z09));
  oai21  g049(.a(new_n77_), .b(new_n74_), .c(new_n83_), .O(new_n112_));
  inv1   g050(.a(x05), .O(new_n113_));
  nand3  g051(.a(x40), .b(x39), .c(new_n113_), .O(new_n114_));
  inv1   g052(.a(x07), .O(new_n115_));
  aoi21  g053(.a(new_n83_), .b(new_n115_), .c(x04), .O(new_n116_));
  nand4  g054(.a(new_n116_), .b(new_n114_), .c(new_n112_), .d(new_n78_), .O(new_n117_));
  nand3  g055(.a(x37), .b(x27), .c(x06), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n117_), .c(new_n71_), .O(z10));
  inv1   g057(.a(x08), .O(new_n120_));
  nand3  g058(.a(x40), .b(x39), .c(x29), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n80_), .c(new_n120_), .O(new_n122_));
  nand2  g060(.a(new_n121_), .b(x27), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g062(.a(x35), .b(new_n76_), .c(x27), .d(new_n63_), .O(new_n125_));
  nor3   g063(.a(new_n71_), .b(x30), .c(x09), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n124_), .O(z18));
  inv1   g065(.a(z18), .O(z11));
  nand2  g066(.a(new_n68_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n65_), .b(x10), .O(new_n130_));
  nand3  g068(.a(new_n94_), .b(new_n130_), .c(new_n129_), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(z12));
  nand2  g070(.a(x35), .b(x28), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n88_), .O(new_n134_));
  nor2   g072(.a(x19), .b(x18), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(x20), .O(new_n136_));
  nor2   g074(.a(x13), .b(x04), .O(new_n137_));
  nand4  g075(.a(new_n137_), .b(x40), .c(x39), .d(new_n76_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n136_), .c(new_n71_), .O(new_n139_));
  nand3  g077(.a(new_n137_), .b(x40), .c(x39), .O(new_n140_));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  nor2   g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n139_), .c(x27), .O(new_n143_));
  inv1   g081(.a(new_n75_), .O(new_n144_));
  nor2   g082(.a(new_n71_), .b(x13), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n144_), .c(new_n65_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n143_), .O(z13));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand3  g087(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  nand2  g089(.a(new_n88_), .b(new_n87_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  aoi21  g091(.a(x36), .b(x35), .c(new_n76_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n153_), .c(new_n74_), .O(new_n155_));
  nand3  g093(.a(new_n151_), .b(new_n134_), .c(x27), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n140_), .c(new_n71_), .O(new_n157_));
  oai21  g095(.a(new_n155_), .b(new_n144_), .c(new_n157_), .O(z14));
  nand4  g096(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n94_), .O(z15));
  nand2  g098(.a(x22), .b(x01), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(x23), .c(new_n94_), .O(z16));
  inv1   g100(.a(x24), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x23), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n161_), .c(new_n94_), .O(z17));
  oai21  g103(.a(new_n87_), .b(new_n74_), .c(new_n126_), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(new_n122_), .O(z20));
  inv1   g105(.a(z18), .O(z19));
endmodule


