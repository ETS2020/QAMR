// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n153_, new_n155_,
    new_n157_, new_n158_, new_n162_, new_n163_;
  inv1   g000(.a(x03), .O(new_n63_));
  nor2   g001(.a(x14), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  and2   g012(.a(x40), .b(x39), .O(new_n75_));
  nand3  g013(.a(new_n75_), .b(new_n74_), .c(new_n71_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n70_), .c(new_n66_), .O(new_n77_));
  nand3  g015(.a(new_n75_), .b(x10), .c(new_n71_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(new_n77_), .c(new_n65_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n73_), .c(new_n66_), .O(new_n84_));
  inv1   g022(.a(x30), .O(new_n85_));
  inv1   g023(.a(x32), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nor2   g025(.a(new_n64_), .b(new_n71_), .O(new_n88_));
  oai21  g026(.a(new_n87_), .b(new_n84_), .c(new_n88_), .O(z01));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n83_), .c(x04), .O(new_n91_));
  nand2  g029(.a(x40), .b(x39), .O(new_n92_));
  nor2   g030(.a(new_n92_), .b(new_n64_), .O(z08));
  inv1   g031(.a(x02), .O(new_n94_));
  nand2  g032(.a(x29), .b(x08), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(z08), .c(new_n91_), .O(z02));
  aoi21  g035(.a(new_n68_), .b(x27), .c(new_n64_), .O(new_n98_));
  nand2  g036(.a(new_n67_), .b(new_n66_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n98_), .c(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand3  g039(.a(new_n99_), .b(new_n98_), .c(new_n101_), .O(z04));
  inv1   g040(.a(new_n64_), .O(new_n103_));
  oai21  g041(.a(new_n82_), .b(new_n66_), .c(new_n67_), .O(new_n104_));
  and2   g042(.a(new_n104_), .b(new_n103_), .O(z05));
  inv1   g043(.a(z05), .O(z06));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x17), .b(new_n65_), .O(new_n109_));
  nor2   g047(.a(x33), .b(x31), .O(new_n110_));
  nand4  g048(.a(new_n110_), .b(new_n109_), .c(x14), .d(x03), .O(new_n111_));
  aoi21  g049(.a(new_n108_), .b(x38), .c(new_n111_), .O(z07));
  and2   g050(.a(x34), .b(x27), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(x26), .c(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n103_), .O(z09));
  nand2  g053(.a(new_n87_), .b(new_n75_), .O(new_n116_));
  nand2  g054(.a(new_n83_), .b(new_n73_), .O(new_n117_));
  inv1   g055(.a(x07), .O(new_n118_));
  nand2  g056(.a(new_n92_), .b(new_n118_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n117_), .c(x27), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n116_), .O(new_n121_));
  inv1   g059(.a(x05), .O(new_n122_));
  aoi21  g060(.a(new_n75_), .b(new_n122_), .c(x04), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(new_n64_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  nand2  g065(.a(new_n75_), .b(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n83_), .c(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g069(.a(new_n83_), .O(new_n132_));
  nor2   g070(.a(new_n66_), .b(x04), .O(new_n133_));
  inv1   g071(.a(x09), .O(new_n134_));
  nand2  g072(.a(new_n85_), .b(new_n134_), .O(new_n135_));
  aoi21  g073(.a(new_n133_), .b(new_n132_), .c(new_n135_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n131_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n103_), .O(z11));
  inv1   g076(.a(new_n77_), .O(new_n139_));
  nand3  g077(.a(new_n78_), .b(new_n139_), .c(new_n103_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z12));
  nor2   g079(.a(x13), .b(x04), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n87_), .c(new_n75_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n69_), .c(x20), .O(new_n145_));
  oai21  g083(.a(new_n73_), .b(new_n72_), .c(x28), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n142_), .c(new_n75_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(new_n145_), .c(new_n66_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n143_), .c(new_n64_), .O(z13));
  inv1   g088(.a(new_n143_), .O(new_n151_));
  nor3   g089(.a(new_n148_), .b(new_n151_), .c(new_n64_), .O(z14));
  nand4  g090(.a(new_n113_), .b(new_n103_), .c(x26), .d(x12), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(z15));
  nand2  g092(.a(x22), .b(x01), .O(new_n155_));
  nor3   g093(.a(new_n155_), .b(new_n64_), .c(x23), .O(z16));
  inv1   g094(.a(x24), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x23), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n155_), .c(new_n103_), .O(z17));
  nand3  g097(.a(new_n136_), .b(new_n131_), .c(new_n103_), .O(z18));
  inv1   g098(.a(z18), .O(z19));
  nand2  g099(.a(x35), .b(x27), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n85_), .c(new_n134_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n129_), .c(new_n103_), .O(z20));
endmodule


