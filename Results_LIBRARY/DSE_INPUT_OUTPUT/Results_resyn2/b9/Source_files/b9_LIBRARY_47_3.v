// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n170_, new_n172_, new_n173_;
  nand2  g000(.a(x36), .b(x13), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  nand4  g008(.a(x40), .b(x39), .c(x10), .d(new_n70_), .O(new_n71_));
  inv1   g009(.a(new_n71_), .O(new_n72_));
  oai21  g010(.a(new_n72_), .b(new_n69_), .c(new_n65_), .O(new_n73_));
  inv1   g011(.a(x16), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x15), .O(new_n76_));
  inv1   g014(.a(x35), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  nand2  g016(.a(x27), .b(new_n70_), .O(new_n79_));
  aoi21  g017(.a(new_n78_), .b(new_n77_), .c(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n76_), .c(new_n74_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(new_n73_), .c(new_n64_), .O(z00));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(new_n78_), .c(new_n66_), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  inv1   g024(.a(new_n86_), .O(new_n87_));
  nor2   g025(.a(new_n64_), .b(new_n70_), .O(new_n88_));
  oai21  g026(.a(new_n87_), .b(new_n85_), .c(new_n88_), .O(z01));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n84_), .c(x04), .O(new_n91_));
  nor2   g029(.a(new_n75_), .b(new_n64_), .O(z08));
  inv1   g030(.a(x02), .O(new_n93_));
  nand2  g031(.a(x29), .b(x08), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(z08), .c(new_n91_), .O(z02));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  oai22  g035(.a(new_n97_), .b(new_n77_), .c(new_n67_), .d(x27), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n63_), .c(new_n100_), .O(z04));
  nand2  g039(.a(new_n97_), .b(new_n67_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n63_), .O(z06));
  inv1   g041(.a(z06), .O(z05));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  or2    g049(.a(x33), .b(x31), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n111_), .c(x14), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n109_), .c(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n63_), .O(z07));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n63_), .O(z09));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  nand2  g056(.a(new_n75_), .b(x07), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nor2   g058(.a(new_n118_), .b(new_n86_), .O(new_n121_));
  aoi21  g059(.a(new_n120_), .b(new_n85_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  and2   g061(.a(new_n123_), .b(new_n63_), .O(new_n124_));
  oai21  g062(.a(new_n122_), .b(x04), .c(new_n124_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n84_), .c(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  inv1   g068(.a(new_n79_), .O(new_n131_));
  inv1   g069(.a(new_n84_), .O(new_n132_));
  inv1   g070(.a(x09), .O(new_n133_));
  inv1   g071(.a(x30), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  aoi21  g073(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n130_), .c(new_n63_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  inv1   g076(.a(x10), .O(new_n139_));
  nor3   g077(.a(x37), .b(x36), .c(x35), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n66_), .c(new_n139_), .O(new_n141_));
  nor2   g079(.a(new_n75_), .b(x04), .O(new_n142_));
  or2    g080(.a(new_n142_), .b(new_n69_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n141_), .c(new_n63_), .O(z12));
  inv1   g082(.a(new_n142_), .O(new_n145_));
  inv1   g083(.a(x13), .O(new_n146_));
  oai22  g084(.a(new_n78_), .b(new_n77_), .c(x28), .d(x13), .O(new_n147_));
  aoi22  g085(.a(new_n147_), .b(x27), .c(new_n87_), .d(new_n146_), .O(new_n148_));
  inv1   g086(.a(x18), .O(new_n149_));
  inv1   g087(.a(x19), .O(new_n150_));
  nand3  g088(.a(x20), .b(new_n150_), .c(new_n149_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n69_), .c(new_n64_), .O(new_n153_));
  oai21  g091(.a(new_n148_), .b(new_n145_), .c(new_n153_), .O(z13));
  nand2  g092(.a(new_n87_), .b(new_n146_), .O(new_n155_));
  nand2  g093(.a(new_n147_), .b(x27), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n155_), .O(new_n157_));
  nand2  g095(.a(new_n152_), .b(new_n69_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n63_), .O(new_n159_));
  aoi21  g097(.a(new_n157_), .b(new_n142_), .c(new_n159_), .O(z14));
  nand4  g098(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n63_), .O(z15));
  inv1   g100(.a(x23), .O(new_n163_));
  nand4  g101(.a(new_n63_), .b(new_n163_), .c(x22), .d(x01), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z16));
  inv1   g103(.a(x24), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(x23), .c(x22), .d(x01), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n63_), .O(z17));
  aoi21  g106(.a(new_n136_), .b(new_n130_), .c(new_n64_), .O(z18));
  nand2  g107(.a(new_n136_), .b(new_n130_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n63_), .O(z19));
  nand2  g109(.a(x35), .b(x27), .O(new_n172_));
  nand4  g110(.a(new_n172_), .b(new_n63_), .c(new_n134_), .d(new_n133_), .O(new_n173_));
  nor2   g111(.a(new_n173_), .b(new_n128_), .O(z20));
endmodule


