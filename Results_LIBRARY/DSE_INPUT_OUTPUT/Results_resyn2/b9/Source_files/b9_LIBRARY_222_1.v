// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n164_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n175_, new_n176_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x40), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(x01), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  aoi21  g014(.a(x35), .b(new_n76_), .c(x36), .O(new_n77_));
  nor2   g015(.a(x32), .b(x30), .O(new_n78_));
  oai21  g016(.a(new_n77_), .b(new_n75_), .c(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x04), .c(new_n72_), .O(z01));
  nand2  g018(.a(x35), .b(new_n76_), .O(new_n81_));
  nor2   g019(.a(x27), .b(x08), .O(new_n82_));
  oai21  g020(.a(new_n82_), .b(new_n81_), .c(x04), .O(new_n83_));
  inv1   g021(.a(x02), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g024(.a(x39), .O(new_n87_));
  nor2   g025(.a(new_n71_), .b(new_n87_), .O(new_n88_));
  nand4  g026(.a(new_n88_), .b(new_n86_), .c(new_n83_), .d(x01), .O(z02));
  nand2  g027(.a(x35), .b(x28), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x27), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n75_), .c(new_n72_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n91_), .c(new_n95_), .O(z04));
  inv1   g034(.a(new_n72_), .O(new_n97_));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n97_), .c(new_n92_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n97_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  aoi21  g049(.a(new_n87_), .b(x01), .c(new_n71_), .O(z08));
  and2   g050(.a(x34), .b(x27), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(x26), .c(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n97_), .O(z09));
  nor2   g053(.a(new_n77_), .b(new_n75_), .O(new_n116_));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  inv1   g056(.a(x07), .O(new_n119_));
  nor2   g057(.a(new_n88_), .b(new_n119_), .O(new_n120_));
  aoi22  g058(.a(new_n120_), .b(new_n116_), .c(new_n118_), .d(new_n79_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n72_), .O(new_n124_));
  oai21  g062(.a(new_n121_), .b(x04), .c(new_n124_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  nor2   g064(.a(x30), .b(x09), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x01), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x40), .O(new_n130_));
  nand2  g068(.a(new_n88_), .b(x29), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x27), .c(x04), .O(new_n132_));
  nand2  g070(.a(new_n131_), .b(new_n81_), .O(new_n133_));
  oai21  g071(.a(new_n81_), .b(new_n75_), .c(new_n126_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n133_), .c(new_n132_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n127_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n130_), .O(z11));
  nor2   g075(.a(new_n67_), .b(new_n75_), .O(new_n138_));
  nor2   g076(.a(new_n87_), .b(x04), .O(new_n139_));
  nor2   g077(.a(new_n66_), .b(new_n75_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(x10), .c(new_n139_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x01), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x40), .c(new_n138_), .O(z12));
  aoi21  g081(.a(x36), .b(x35), .c(new_n76_), .O(new_n144_));
  inv1   g082(.a(x13), .O(new_n145_));
  nand3  g083(.a(new_n139_), .b(x40), .c(new_n145_), .O(new_n146_));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x20), .O(new_n148_));
  oai22  g086(.a(new_n148_), .b(new_n67_), .c(new_n146_), .d(new_n144_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  nand2  g088(.a(new_n139_), .b(new_n145_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n78_), .c(x01), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x40), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n150_), .O(z13));
  oai21  g092(.a(x28), .b(new_n75_), .c(new_n78_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n65_), .c(new_n145_), .O(new_n156_));
  oai21  g094(.a(new_n67_), .b(new_n75_), .c(new_n156_), .O(new_n157_));
  nand2  g095(.a(new_n144_), .b(new_n78_), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n146_), .c(new_n148_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n157_), .c(new_n97_), .O(z14));
  nand4  g099(.a(new_n113_), .b(new_n97_), .c(x26), .d(x12), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(z15));
  nand2  g101(.a(x22), .b(x01), .O(new_n164_));
  nor2   g102(.a(new_n164_), .b(x23), .O(z16));
  inv1   g103(.a(x24), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(x23), .c(x22), .d(x01), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z17));
  aoi21  g106(.a(new_n131_), .b(new_n81_), .c(new_n126_), .O(new_n169_));
  nand2  g107(.a(new_n131_), .b(x27), .O(new_n170_));
  nand4  g108(.a(x35), .b(new_n76_), .c(x27), .d(new_n63_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n127_), .c(new_n97_), .O(new_n172_));
  aoi21  g110(.a(new_n170_), .b(new_n169_), .c(new_n172_), .O(z19));
  inv1   g111(.a(z19), .O(z18));
  nand2  g112(.a(x35), .b(x27), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n127_), .O(new_n176_));
  oai21  g114(.a(new_n176_), .b(new_n169_), .c(new_n97_), .O(z20));
endmodule


