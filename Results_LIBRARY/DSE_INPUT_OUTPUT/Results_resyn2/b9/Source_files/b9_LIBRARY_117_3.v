// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n176_, new_n178_;
  nand2  g000(.a(x40), .b(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x04), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(x36), .b(x35), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n66_), .c(new_n67_), .O(new_n69_));
  aoi22  g007(.a(new_n69_), .b(x27), .c(new_n64_), .d(x10), .O(new_n70_));
  nand2  g008(.a(x40), .b(x34), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n70_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x35), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  oai21  g013(.a(new_n74_), .b(x28), .c(new_n75_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x27), .O(new_n77_));
  nor2   g015(.a(x32), .b(x30), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n71_), .c(x04), .O(z01));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  inv1   g021(.a(x08), .O(new_n84_));
  inv1   g022(.a(x27), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  and2   g025(.a(x29), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(x02), .c(x39), .O(new_n89_));
  aoi21  g027(.a(new_n87_), .b(x04), .c(new_n89_), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(x34), .c(x40), .O(z02));
  oai21  g029(.a(new_n74_), .b(new_n81_), .c(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  aoi22  g031(.a(x40), .b(x34), .c(new_n93_), .d(new_n85_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  oai21  g035(.a(new_n81_), .b(new_n85_), .c(new_n93_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n71_), .O(z06));
  inv1   g037(.a(z06), .O(z05));
  inv1   g038(.a(x25), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x25), .b(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(x38), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x15), .O(new_n107_));
  or2    g045(.a(x33), .b(x31), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(new_n107_), .c(x14), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n105_), .c(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n71_), .O(z07));
  inv1   g049(.a(x34), .O(new_n112_));
  nand3  g050(.a(x40), .b(x39), .c(new_n112_), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z08));
  inv1   g052(.a(x40), .O(new_n115_));
  nand3  g053(.a(x27), .b(x26), .c(x11), .O(new_n116_));
  aoi21  g054(.a(new_n116_), .b(new_n115_), .c(new_n112_), .O(z09));
  inv1   g055(.a(x05), .O(new_n118_));
  nand2  g056(.a(x39), .b(new_n118_), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(new_n112_), .c(new_n115_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(new_n63_), .b(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n76_), .c(new_n65_), .O(new_n123_));
  nand3  g061(.a(new_n71_), .b(x37), .c(x06), .O(new_n124_));
  oai21  g062(.a(new_n123_), .b(new_n120_), .c(new_n124_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x27), .O(new_n126_));
  inv1   g064(.a(new_n78_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(new_n64_), .c(new_n112_), .d(x05), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(z10));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n82_), .c(new_n84_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nor2   g071(.a(new_n85_), .b(x04), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  aoi21  g074(.a(new_n134_), .b(new_n83_), .c(new_n136_), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n133_), .c(new_n71_), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(z11));
  nand2  g077(.a(new_n64_), .b(x10), .O(new_n140_));
  nand2  g078(.a(new_n69_), .b(x27), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand2  g080(.a(new_n71_), .b(new_n142_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x28), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n64_), .c(new_n144_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x20), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n67_), .c(new_n147_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  nand3  g089(.a(x39), .b(new_n144_), .c(new_n65_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n78_), .c(new_n112_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x40), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n151_), .O(z13));
  nand2  g093(.a(new_n64_), .b(new_n144_), .O(new_n156_));
  aoi21  g094(.a(new_n81_), .b(x27), .c(new_n127_), .O(new_n157_));
  oai22  g095(.a(new_n157_), .b(new_n156_), .c(new_n67_), .d(new_n85_), .O(new_n158_));
  nand3  g096(.a(new_n145_), .b(new_n78_), .c(x28), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n156_), .c(new_n149_), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n158_), .c(new_n71_), .O(z14));
  nand3  g100(.a(x27), .b(x26), .c(x12), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n115_), .c(new_n112_), .O(z15));
  nand2  g102(.a(x22), .b(x01), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(x23), .c(new_n71_), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(x23), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n165_), .c(new_n71_), .O(z17));
  nand2  g107(.a(x27), .b(x04), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n86_), .c(new_n83_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n135_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n71_), .O(new_n173_));
  nand2  g111(.a(z08), .b(new_n88_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n173_), .O(z18));
  nand2  g113(.a(new_n137_), .b(new_n133_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n71_), .O(z19));
  oai21  g115(.a(new_n74_), .b(new_n85_), .c(new_n135_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n131_), .c(new_n71_), .O(z20));
endmodule


