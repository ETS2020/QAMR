// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n176_, new_n178_, new_n180_,
    new_n181_, new_n183_, new_n184_;
  nor2   g000(.a(x36), .b(x20), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  nand4  g006(.a(x40), .b(x39), .c(x10), .d(new_n68_), .O(new_n69_));
  oai21  g007(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n64_), .c(new_n65_), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  inv1   g011(.a(x20), .O(new_n74_));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(new_n74_), .c(new_n76_), .O(new_n77_));
  nor2   g015(.a(new_n66_), .b(x04), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n77_), .c(new_n73_), .d(new_n64_), .O(new_n79_));
  oai21  g017(.a(new_n71_), .b(new_n63_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(new_n76_), .c(new_n66_), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  nor2   g023(.a(new_n63_), .b(new_n68_), .O(new_n86_));
  oai21  g024(.a(new_n85_), .b(new_n83_), .c(new_n86_), .O(z01));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n82_), .c(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n72_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n89_), .c(new_n63_), .O(z02));
  oai21  g030(.a(new_n75_), .b(new_n81_), .c(x27), .O(new_n93_));
  inv1   g031(.a(x37), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n66_), .c(new_n63_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n93_), .c(new_n97_), .O(z04));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n63_), .O(z05));
  inv1   g038(.a(new_n99_), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n63_), .O(z06));
  inv1   g040(.a(new_n63_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n103_), .O(z07));
  nand2  g052(.a(new_n72_), .b(new_n103_), .O(z08));
  nand2  g053(.a(x34), .b(x27), .O(new_n116_));
  nand2  g054(.a(x26), .b(x11), .O(new_n117_));
  oai21  g055(.a(new_n117_), .b(new_n116_), .c(new_n103_), .O(z09));
  nand3  g056(.a(x37), .b(x27), .c(x06), .O(new_n119_));
  nor2   g057(.a(new_n72_), .b(x04), .O(new_n120_));
  nand3  g058(.a(new_n85_), .b(new_n120_), .c(x05), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n103_), .O(new_n123_));
  oai21  g061(.a(new_n82_), .b(new_n74_), .c(new_n76_), .O(new_n124_));
  inv1   g062(.a(x07), .O(new_n125_));
  nand2  g063(.a(new_n72_), .b(new_n125_), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  nand2  g065(.a(new_n73_), .b(new_n127_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(new_n126_), .c(new_n124_), .d(new_n78_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n123_), .O(z10));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n82_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  inv1   g073(.a(new_n82_), .O(new_n136_));
  inv1   g074(.a(x09), .O(new_n137_));
  inv1   g075(.a(x30), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  aoi21  g077(.a(new_n136_), .b(new_n78_), .c(new_n139_), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n135_), .c(new_n63_), .O(z18));
  inv1   g079(.a(z18), .O(z11));
  oai21  g080(.a(new_n120_), .b(x28), .c(x35), .O(new_n143_));
  aoi21  g081(.a(new_n120_), .b(x36), .c(x37), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g083(.a(new_n69_), .b(new_n103_), .O(new_n146_));
  aoi21  g084(.a(new_n145_), .b(x27), .c(new_n146_), .O(z12));
  nor2   g085(.a(x13), .b(x04), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n73_), .O(new_n149_));
  nand2  g087(.a(new_n81_), .b(x27), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n84_), .O(new_n151_));
  nor2   g089(.a(new_n76_), .b(new_n75_), .O(new_n152_));
  aoi22  g090(.a(new_n152_), .b(x27), .c(new_n151_), .d(new_n103_), .O(new_n153_));
  inv1   g091(.a(x18), .O(new_n154_));
  inv1   g092(.a(x19), .O(new_n155_));
  nor2   g093(.a(new_n67_), .b(new_n66_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(x20), .c(new_n155_), .d(new_n154_), .O(new_n157_));
  oai21  g095(.a(new_n153_), .b(new_n149_), .c(new_n157_), .O(z13));
  nand2  g096(.a(new_n155_), .b(new_n154_), .O(new_n159_));
  inv1   g097(.a(new_n149_), .O(new_n160_));
  nand2  g098(.a(x36), .b(x35), .O(new_n161_));
  inv1   g099(.a(x32), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n138_), .c(x28), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n161_), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n160_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n159_), .O(new_n167_));
  oai21  g105(.a(new_n163_), .b(x20), .c(x37), .O(new_n168_));
  nand2  g106(.a(new_n163_), .b(new_n160_), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n168_), .c(new_n75_), .O(new_n170_));
  nand2  g108(.a(new_n94_), .b(new_n81_), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(x27), .c(x20), .O(new_n172_));
  oai21  g110(.a(new_n85_), .b(x27), .c(new_n103_), .O(new_n173_));
  aoi21  g111(.a(new_n172_), .b(new_n149_), .c(new_n173_), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n170_), .c(new_n167_), .O(z14));
  nand2  g113(.a(x26), .b(x12), .O(new_n176_));
  nor3   g114(.a(new_n176_), .b(new_n116_), .c(new_n63_), .O(z15));
  nand2  g115(.a(x22), .b(x01), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(x23), .c(new_n103_), .O(z16));
  inv1   g117(.a(x24), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(x23), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n178_), .c(new_n103_), .O(z17));
  nand2  g120(.a(x35), .b(x27), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n138_), .c(new_n137_), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n133_), .c(new_n103_), .O(z20));
  inv1   g123(.a(z18), .O(z19));
endmodule


