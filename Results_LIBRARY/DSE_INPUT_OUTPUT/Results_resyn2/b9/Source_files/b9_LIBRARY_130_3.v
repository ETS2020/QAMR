// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n123_, new_n124_, new_n125_, new_n126_, new_n129_,
    new_n131_, new_n133_, new_n134_, new_n136_, new_n139_, new_n140_;
  nand2  g00(.a(x40), .b(x39), .O(new_n63_));
  inv1   g01(.a(new_n63_), .O(new_n64_));
  inv1   g02(.a(x15), .O(new_n65_));
  inv1   g03(.a(x37), .O(new_n66_));
  nand2  g04(.a(x35), .b(x28), .O(new_n67_));
  nand2  g05(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g06(.a(new_n68_), .b(x27), .c(new_n65_), .O(new_n69_));
  aoi21  g07(.a(new_n69_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g08(.a(x35), .O(new_n71_));
  inv1   g09(.a(x36), .O(new_n72_));
  oai21  g10(.a(new_n71_), .b(x28), .c(new_n72_), .O(new_n73_));
  nand2  g11(.a(new_n73_), .b(x27), .O(new_n74_));
  nor2   g12(.a(x32), .b(x30), .O(new_n75_));
  nand2  g13(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g14(.a(new_n76_), .b(new_n63_), .c(x04), .O(z01));
  inv1   g15(.a(x27), .O(new_n79_));
  aoi21  g16(.a(new_n66_), .b(new_n79_), .c(new_n64_), .O(new_n80_));
  nand2  g17(.a(new_n67_), .b(x27), .O(new_n81_));
  nand3  g18(.a(new_n81_), .b(new_n80_), .c(x21), .O(z03));
  inv1   g19(.a(x21), .O(new_n83_));
  nand3  g20(.a(new_n81_), .b(new_n80_), .c(new_n83_), .O(z04));
  aoi21  g21(.a(x28), .b(x27), .c(x37), .O(new_n85_));
  nor2   g22(.a(new_n85_), .b(new_n64_), .O(z05));
  nand2  g23(.a(new_n85_), .b(new_n63_), .O(new_n87_));
  inv1   g24(.a(new_n87_), .O(z06));
  inv1   g25(.a(x25), .O(new_n89_));
  nand2  g26(.a(new_n89_), .b(x00), .O(new_n90_));
  inv1   g27(.a(x14), .O(new_n91_));
  nand2  g28(.a(x25), .b(new_n91_), .O(new_n92_));
  nand3  g29(.a(new_n92_), .b(new_n90_), .c(x38), .O(new_n93_));
  inv1   g30(.a(x17), .O(new_n94_));
  nor2   g31(.a(new_n94_), .b(x15), .O(new_n95_));
  inv1   g32(.a(x31), .O(new_n96_));
  inv1   g33(.a(x33), .O(new_n97_));
  nand2  g34(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  oai21  g35(.a(new_n98_), .b(new_n95_), .c(x14), .O(new_n99_));
  and2   g36(.a(new_n63_), .b(x03), .O(new_n100_));
  nand3  g37(.a(new_n100_), .b(new_n99_), .c(new_n93_), .O(new_n101_));
  inv1   g38(.a(new_n101_), .O(z07));
  inv1   g39(.a(x11), .O(new_n104_));
  nor2   g40(.a(new_n64_), .b(new_n79_), .O(new_n105_));
  nand3  g41(.a(new_n105_), .b(x34), .c(x26), .O(new_n106_));
  nor2   g42(.a(new_n106_), .b(new_n104_), .O(z09));
  inv1   g43(.a(x07), .O(new_n108_));
  nor2   g44(.a(new_n108_), .b(x04), .O(new_n109_));
  and2   g45(.a(x37), .b(x06), .O(new_n110_));
  aoi21  g46(.a(new_n109_), .b(new_n73_), .c(new_n110_), .O(new_n111_));
  oai21  g47(.a(new_n111_), .b(new_n79_), .c(new_n63_), .O(z10));
  inv1   g48(.a(x09), .O(new_n113_));
  inv1   g49(.a(x30), .O(new_n114_));
  nand2  g50(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nor2   g51(.a(x27), .b(x08), .O(new_n116_));
  inv1   g52(.a(x28), .O(new_n117_));
  nand2  g53(.a(x27), .b(x04), .O(new_n118_));
  nand3  g54(.a(new_n118_), .b(x35), .c(new_n117_), .O(new_n119_));
  nor2   g55(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  oai21  g56(.a(new_n120_), .b(new_n115_), .c(new_n63_), .O(z11));
  nand2  g57(.a(new_n105_), .b(new_n68_), .O(z12));
  inv1   g58(.a(x18), .O(new_n123_));
  inv1   g59(.a(x19), .O(new_n124_));
  nand3  g60(.a(x20), .b(new_n124_), .c(new_n123_), .O(new_n125_));
  inv1   g61(.a(new_n125_), .O(new_n126_));
  nand3  g62(.a(new_n126_), .b(new_n105_), .c(new_n68_), .O(z14));
  inv1   g63(.a(z14), .O(z13));
  inv1   g64(.a(x12), .O(new_n129_));
  nor2   g65(.a(new_n106_), .b(new_n129_), .O(z15));
  nand2  g66(.a(x22), .b(x01), .O(new_n131_));
  oai21  g67(.a(new_n131_), .b(x23), .c(new_n63_), .O(z16));
  inv1   g68(.a(x24), .O(new_n133_));
  nand2  g69(.a(new_n133_), .b(x23), .O(new_n134_));
  oai21  g70(.a(new_n134_), .b(new_n131_), .c(new_n63_), .O(z17));
  nand3  g71(.a(new_n63_), .b(new_n114_), .c(new_n113_), .O(new_n136_));
  nor2   g72(.a(new_n136_), .b(new_n120_), .O(z19));
  inv1   g73(.a(z19), .O(z18));
  inv1   g74(.a(x08), .O(new_n139_));
  oai21  g75(.a(x28), .b(new_n139_), .c(new_n79_), .O(new_n140_));
  aoi21  g76(.a(new_n140_), .b(x35), .c(new_n136_), .O(z20));
  one    g77(.O(z02));
  zero   g78(.O(z08));
endmodule


