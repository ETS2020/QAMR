// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n167_, new_n169_, new_n172_, new_n174_, new_n175_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  inv1   g009(.a(x31), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  oai21  g013(.a(new_n70_), .b(x15), .c(new_n75_), .O(z00));
  inv1   g014(.a(x27), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n73_), .c(new_n77_), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  inv1   g019(.a(new_n81_), .O(new_n82_));
  nor2   g020(.a(new_n74_), .b(new_n65_), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n80_), .c(new_n83_), .O(z01));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n79_), .c(x04), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  nor2   g025(.a(new_n74_), .b(new_n87_), .O(z08));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g029(.a(new_n91_), .b(z08), .c(new_n86_), .O(z02));
  inv1   g030(.a(x35), .O(new_n93_));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n93_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n74_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  inv1   g036(.a(new_n74_), .O(new_n99_));
  nand3  g037(.a(new_n96_), .b(new_n99_), .c(new_n98_), .O(z04));
  nand3  g038(.a(new_n95_), .b(new_n99_), .c(new_n94_), .O(z05));
  inv1   g039(.a(z05), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  inv1   g047(.a(x33), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n72_), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n109_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n107_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n99_), .O(z07));
  inv1   g052(.a(x11), .O(new_n115_));
  nand4  g053(.a(new_n99_), .b(x34), .c(x27), .d(x26), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(new_n115_), .O(z09));
  inv1   g055(.a(x07), .O(new_n118_));
  nand2  g056(.a(new_n87_), .b(new_n118_), .O(new_n119_));
  nor2   g057(.a(new_n87_), .b(x05), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(x04), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n119_), .c(new_n80_), .O(new_n122_));
  nor2   g060(.a(new_n81_), .b(new_n66_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(x05), .c(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n122_), .c(new_n74_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n79_), .c(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  inv1   g070(.a(new_n79_), .O(new_n133_));
  nor2   g071(.a(new_n77_), .b(x04), .O(new_n134_));
  inv1   g072(.a(x09), .O(new_n135_));
  inv1   g073(.a(x30), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  aoi21  g075(.a(new_n134_), .b(new_n133_), .c(new_n137_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n132_), .c(new_n99_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z11));
  aoi21  g078(.a(new_n64_), .b(new_n94_), .c(new_n77_), .O(new_n141_));
  inv1   g079(.a(new_n63_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x27), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n66_), .c(new_n74_), .O(new_n144_));
  oai21  g082(.a(new_n141_), .b(x10), .c(new_n144_), .O(z12));
  inv1   g083(.a(x13), .O(new_n146_));
  nor2   g084(.a(new_n87_), .b(x04), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n147_), .c(new_n146_), .O(new_n150_));
  nor2   g088(.a(x19), .b(x18), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n142_), .c(x20), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  aoi21  g092(.a(new_n123_), .b(new_n146_), .c(new_n74_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(z13));
  nand2  g094(.a(new_n147_), .b(new_n146_), .O(new_n157_));
  nand2  g095(.a(new_n151_), .b(x20), .O(new_n158_));
  nand3  g096(.a(new_n148_), .b(new_n81_), .c(x28), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n157_), .c(new_n158_), .O(new_n161_));
  aoi21  g099(.a(new_n78_), .b(x27), .c(new_n82_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n157_), .c(new_n143_), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n161_), .c(new_n99_), .O(z14));
  inv1   g102(.a(x12), .O(new_n165_));
  nor2   g103(.a(new_n116_), .b(new_n165_), .O(z15));
  nand2  g104(.a(x22), .b(x01), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(x23), .c(new_n99_), .O(z16));
  inv1   g106(.a(x23), .O(new_n169_));
  nor4   g107(.a(new_n167_), .b(new_n74_), .c(x24), .d(new_n169_), .O(z17));
  aoi21  g108(.a(new_n138_), .b(new_n132_), .c(new_n74_), .O(z18));
  nand2  g109(.a(new_n138_), .b(new_n132_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n99_), .O(z19));
  nand2  g111(.a(x35), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n136_), .c(new_n135_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n130_), .c(new_n99_), .O(z20));
endmodule


