// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n173_,
    new_n174_, new_n176_, new_n178_, new_n180_;
  inv1   g000(.a(x33), .O(new_n63_));
  inv1   g001(.a(x39), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g014(.a(x40), .b(x39), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(z08));
  nand4  g016(.a(z08), .b(new_n76_), .c(new_n66_), .d(new_n73_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n65_), .O(z01));
  inv1   g026(.a(x35), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(x28), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n90_), .O(new_n91_));
  and2   g029(.a(x29), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x02), .c(x40), .O(new_n93_));
  aoi21  g031(.a(new_n91_), .b(x04), .c(new_n93_), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(x33), .c(x39), .O(z02));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  oai22  g034(.a(new_n96_), .b(new_n89_), .c(new_n69_), .d(x27), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(new_n65_), .O(new_n100_));
  nand3  g038(.a(new_n97_), .b(new_n100_), .c(new_n99_), .O(z04));
  nand3  g039(.a(new_n96_), .b(new_n100_), .c(new_n69_), .O(z05));
  nand2  g040(.a(new_n96_), .b(new_n69_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n100_), .O(z06));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  inv1   g049(.a(x31), .O(new_n112_));
  nand2  g050(.a(new_n63_), .b(new_n112_), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n111_), .c(x14), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n109_), .c(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n100_), .O(z07));
  nand2  g054(.a(x34), .b(x27), .O(new_n117_));
  nand2  g055(.a(x26), .b(x11), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(new_n117_), .c(new_n100_), .O(z09));
  nand2  g057(.a(x40), .b(x39), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n86_), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n120_), .b(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n84_), .c(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nor2   g064(.a(new_n120_), .b(x05), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n100_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n83_), .c(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nor2   g074(.a(new_n68_), .b(x04), .O(new_n137_));
  nor2   g075(.a(x30), .b(x09), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(new_n139_));
  aoi21  g077(.a(new_n137_), .b(new_n90_), .c(new_n139_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n136_), .c(new_n100_), .O(z18));
  inv1   g079(.a(z18), .O(z11));
  nor3   g080(.a(x37), .b(x36), .c(x35), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n68_), .c(new_n74_), .O(new_n144_));
  inv1   g082(.a(new_n71_), .O(new_n145_));
  oai21  g083(.a(new_n120_), .b(x04), .c(new_n145_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n144_), .c(new_n100_), .O(z12));
  inv1   g085(.a(x13), .O(new_n148_));
  nand4  g086(.a(new_n121_), .b(new_n63_), .c(new_n148_), .d(new_n73_), .O(new_n149_));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand3  g089(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n152_));
  aoi21  g090(.a(new_n70_), .b(new_n69_), .c(new_n152_), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x28), .O(new_n155_));
  nand2  g093(.a(x39), .b(new_n63_), .O(new_n156_));
  nand3  g094(.a(x40), .b(new_n148_), .c(new_n73_), .O(new_n157_));
  nor2   g095(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n155_), .c(new_n153_), .O(new_n159_));
  nand2  g097(.a(new_n100_), .b(x27), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n159_), .c(new_n149_), .O(z13));
  inv1   g099(.a(new_n157_), .O(new_n162_));
  nand3  g100(.a(new_n154_), .b(new_n86_), .c(x28), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n162_), .c(x33), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n64_), .c(new_n152_), .O(new_n165_));
  nand2  g103(.a(new_n82_), .b(x27), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n86_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n162_), .c(x33), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n64_), .c(new_n145_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n165_), .O(z14));
  nand2  g108(.a(x26), .b(x12), .O(new_n171_));
  nor3   g109(.a(new_n171_), .b(new_n117_), .c(new_n65_), .O(z15));
  inv1   g110(.a(x23), .O(new_n173_));
  nand4  g111(.a(new_n100_), .b(new_n173_), .c(x22), .d(x01), .O(new_n174_));
  inv1   g112(.a(new_n174_), .O(z16));
  nand2  g113(.a(x22), .b(x01), .O(new_n176_));
  nor4   g114(.a(new_n176_), .b(new_n65_), .c(x24), .d(new_n173_), .O(z17));
  nand2  g115(.a(new_n140_), .b(new_n136_), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n100_), .O(z19));
  oai21  g117(.a(new_n89_), .b(new_n68_), .c(new_n138_), .O(new_n180_));
  oai21  g118(.a(new_n180_), .b(new_n134_), .c(new_n100_), .O(z20));
endmodule


