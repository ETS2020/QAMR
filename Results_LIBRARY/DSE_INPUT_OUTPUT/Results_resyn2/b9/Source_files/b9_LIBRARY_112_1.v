// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n174_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n185_, new_n186_, new_n188_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  and2   g005(.a(x40), .b(x39), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x10), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(new_n70_));
  aoi21  g008(.a(new_n67_), .b(x27), .c(new_n70_), .O(new_n71_));
  inv1   g009(.a(x20), .O(new_n72_));
  nand2  g010(.a(x40), .b(new_n72_), .O(new_n73_));
  and2   g011(.a(new_n73_), .b(x16), .O(new_n74_));
  oai21  g012(.a(new_n71_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(new_n73_), .O(new_n76_));
  inv1   g014(.a(x36), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n76_), .O(z01));
  inv1   g022(.a(x35), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(x28), .O(new_n86_));
  oai21  g024(.a(x27), .b(x08), .c(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(x04), .O(new_n88_));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand3  g028(.a(x40), .b(x39), .c(x20), .O(new_n91_));
  aoi21  g029(.a(new_n90_), .b(new_n89_), .c(new_n91_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n88_), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n85_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n76_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n73_), .c(new_n98_), .O(z04));
  nand3  g037(.a(new_n95_), .b(new_n73_), .c(new_n94_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n102_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n73_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  inv1   g050(.a(new_n91_), .O(z08));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n73_), .O(z09));
  nand2  g053(.a(x37), .b(x06), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  nand2  g055(.a(x40), .b(x39), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n80_), .c(new_n65_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n116_), .O(new_n121_));
  inv1   g059(.a(x27), .O(new_n122_));
  inv1   g060(.a(x05), .O(new_n123_));
  nand3  g061(.a(new_n116_), .b(x39), .c(new_n123_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x20), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(x40), .c(new_n122_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n121_), .O(new_n127_));
  nor2   g065(.a(new_n82_), .b(new_n118_), .O(new_n128_));
  nand4  g066(.a(new_n128_), .b(x20), .c(x05), .d(new_n65_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(z10));
  inv1   g068(.a(x08), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(x04), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x20), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x40), .O(new_n135_));
  nand2  g073(.a(new_n68_), .b(x29), .O(new_n136_));
  nor2   g074(.a(new_n122_), .b(new_n65_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n136_), .b(new_n79_), .O(new_n139_));
  oai21  g077(.a(new_n79_), .b(new_n122_), .c(new_n131_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n139_), .c(new_n138_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n132_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n135_), .O(z11));
  nand2  g081(.a(new_n67_), .b(x27), .O(new_n144_));
  nand2  g082(.a(new_n69_), .b(new_n144_), .O(new_n145_));
  nand2  g083(.a(new_n73_), .b(new_n145_), .O(z12));
  oai21  g084(.a(new_n85_), .b(new_n78_), .c(new_n94_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x28), .O(new_n151_));
  nor2   g089(.a(x13), .b(x04), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n151_), .c(new_n68_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n149_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x27), .O(new_n155_));
  nand2  g093(.a(new_n152_), .b(new_n128_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n155_), .c(new_n72_), .O(z13));
  inv1   g095(.a(x40), .O(new_n158_));
  inv1   g096(.a(new_n148_), .O(new_n159_));
  nand3  g097(.a(new_n150_), .b(new_n82_), .c(x28), .O(new_n160_));
  inv1   g098(.a(x13), .O(new_n161_));
  nand3  g099(.a(x39), .b(new_n161_), .c(new_n65_), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n160_), .c(new_n72_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n158_), .c(new_n159_), .O(new_n165_));
  nand2  g103(.a(new_n78_), .b(x27), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n82_), .O(new_n167_));
  aoi21  g105(.a(new_n167_), .b(new_n163_), .c(new_n72_), .O(new_n168_));
  nand3  g106(.a(new_n147_), .b(x27), .c(x20), .O(new_n169_));
  oai21  g107(.a(new_n168_), .b(new_n158_), .c(new_n169_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n165_), .O(z14));
  nand4  g109(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n73_), .O(z15));
  nand2  g111(.a(x22), .b(x01), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(x23), .c(new_n73_), .O(z16));
  inv1   g113(.a(x24), .O(new_n176_));
  inv1   g114(.a(new_n174_), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n73_), .c(new_n176_), .d(x23), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z17));
  nand2  g117(.a(new_n136_), .b(x27), .O(new_n180_));
  nand3  g118(.a(new_n180_), .b(new_n139_), .c(x08), .O(new_n181_));
  nand3  g119(.a(new_n86_), .b(x27), .c(new_n65_), .O(new_n182_));
  and2   g120(.a(new_n132_), .b(new_n73_), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n182_), .c(new_n181_), .O(z18));
  oai21  g122(.a(new_n137_), .b(new_n87_), .c(new_n132_), .O(new_n185_));
  nor2   g123(.a(new_n91_), .b(new_n90_), .O(new_n186_));
  aoi21  g124(.a(new_n185_), .b(new_n73_), .c(new_n186_), .O(z19));
  oai21  g125(.a(new_n85_), .b(new_n122_), .c(new_n183_), .O(new_n188_));
  aoi21  g126(.a(new_n139_), .b(x08), .c(new_n188_), .O(z20));
endmodule


