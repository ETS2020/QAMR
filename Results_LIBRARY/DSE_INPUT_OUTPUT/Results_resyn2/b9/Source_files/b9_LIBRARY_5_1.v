// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n168_, new_n170_, new_n171_, new_n174_, new_n175_, new_n176_,
    new_n177_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x34), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n67_), .O(new_n74_));
  nand3  g012(.a(x40), .b(x39), .c(x34), .O(new_n75_));
  nor3   g013(.a(new_n75_), .b(x15), .c(x04), .O(new_n76_));
  oai21  g014(.a(new_n74_), .b(x10), .c(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n73_), .c(new_n67_), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  inv1   g021(.a(x04), .O(new_n84_));
  nor2   g022(.a(new_n64_), .b(new_n84_), .O(new_n85_));
  oai21  g023(.a(new_n83_), .b(new_n81_), .c(new_n85_), .O(z01));
  inv1   g024(.a(x34), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  nand2  g026(.a(new_n67_), .b(new_n88_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(x35), .c(new_n79_), .O(new_n90_));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x39), .O(new_n94_));
  aoi21  g032(.a(new_n90_), .b(x04), .c(new_n94_), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n87_), .c(x40), .O(z02));
  inv1   g034(.a(new_n64_), .O(new_n97_));
  nand2  g035(.a(new_n69_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n68_), .b(new_n67_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n64_), .O(z04));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n97_), .c(new_n68_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  inv1   g045(.a(x14), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n108_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n107_), .c(x38), .O(new_n110_));
  inv1   g048(.a(x17), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(x15), .O(new_n112_));
  or2    g050(.a(x33), .b(x31), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n112_), .c(x14), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n110_), .c(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n97_), .O(z07));
  inv1   g054(.a(new_n75_), .O(z08));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z09));
  inv1   g057(.a(x07), .O(new_n120_));
  inv1   g058(.a(x39), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(x34), .c(new_n63_), .O(new_n122_));
  nand4  g060(.a(x40), .b(x39), .c(x34), .d(x05), .O(new_n123_));
  oai21  g061(.a(new_n122_), .b(new_n120_), .c(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n80_), .b(new_n73_), .c(x04), .O(new_n125_));
  nand2  g063(.a(x37), .b(x06), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n64_), .O(new_n127_));
  aoi21  g065(.a(new_n125_), .b(new_n124_), .c(new_n127_), .O(new_n128_));
  inv1   g066(.a(new_n123_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n83_), .c(new_n84_), .O(new_n130_));
  oai21  g068(.a(new_n128_), .b(new_n67_), .c(new_n130_), .O(z10));
  oai21  g069(.a(new_n92_), .b(new_n121_), .c(x34), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x40), .O(new_n133_));
  aoi21  g071(.a(new_n67_), .b(new_n88_), .c(new_n80_), .O(new_n134_));
  or2    g072(.a(x30), .b(x09), .O(new_n135_));
  nand2  g073(.a(x27), .b(x04), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n134_), .c(new_n135_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(z11));
  nand3  g077(.a(new_n68_), .b(new_n73_), .c(new_n72_), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(x27), .c(x10), .O(new_n141_));
  aoi21  g079(.a(x39), .b(new_n84_), .c(new_n70_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n141_), .c(x34), .O(new_n143_));
  or2    g081(.a(new_n70_), .b(x40), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n143_), .O(z12));
  nand2  g083(.a(x36), .b(x35), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x28), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(new_n83_), .O(new_n148_));
  inv1   g086(.a(x13), .O(new_n149_));
  nand3  g087(.a(z08), .b(new_n149_), .c(new_n84_), .O(new_n150_));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  nand3  g090(.a(x20), .b(new_n152_), .c(new_n151_), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n70_), .c(new_n97_), .O(new_n155_));
  oai21  g093(.a(new_n150_), .b(new_n148_), .c(new_n155_), .O(z13));
  nand3  g094(.a(new_n146_), .b(new_n82_), .c(x28), .O(new_n157_));
  nand3  g095(.a(x39), .b(new_n149_), .c(new_n84_), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n157_), .c(new_n87_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n63_), .c(new_n153_), .O(new_n161_));
  aoi21  g099(.a(new_n79_), .b(x27), .c(new_n83_), .O(new_n162_));
  nor2   g100(.a(new_n70_), .b(new_n87_), .O(new_n163_));
  oai21  g101(.a(new_n162_), .b(new_n158_), .c(new_n163_), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n161_), .c(new_n144_), .O(z14));
  nand4  g103(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n97_), .O(z15));
  nand2  g105(.a(x22), .b(x01), .O(new_n168_));
  nor3   g106(.a(new_n168_), .b(new_n64_), .c(x23), .O(z16));
  inv1   g107(.a(x24), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x23), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n168_), .c(new_n97_), .O(z17));
  oai22  g110(.a(new_n137_), .b(new_n64_), .c(new_n92_), .d(new_n75_), .O(z18));
  nand3  g111(.a(x40), .b(x39), .c(x29), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n80_), .c(new_n88_), .O(new_n175_));
  nor2   g113(.a(new_n135_), .b(new_n64_), .O(new_n176_));
  oai21  g114(.a(new_n72_), .b(new_n67_), .c(new_n176_), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n175_), .O(z20));
  inv1   g116(.a(new_n138_), .O(z19));
endmodule


