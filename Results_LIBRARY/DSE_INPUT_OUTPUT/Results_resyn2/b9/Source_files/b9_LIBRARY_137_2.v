// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n164_, new_n166_, new_n167_,
    new_n168_, new_n171_;
  inv1   g000(.a(x32), .O(new_n63_));
  nor2   g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x35), .O(new_n71_));
  inv1   g009(.a(x36), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(x27), .c(x10), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n65_), .O(new_n77_));
  oai22  g015(.a(new_n77_), .b(new_n74_), .c(new_n70_), .d(new_n64_), .O(z00));
  inv1   g016(.a(x28), .O(new_n79_));
  nand2  g017(.a(x35), .b(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(new_n72_), .c(new_n67_), .O(new_n81_));
  inv1   g019(.a(x30), .O(new_n82_));
  nand2  g020(.a(new_n63_), .b(new_n82_), .O(new_n83_));
  oai22  g021(.a(new_n83_), .b(new_n81_), .c(new_n64_), .d(x04), .O(z01));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n80_), .c(x04), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n75_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n86_), .c(new_n64_), .O(z02));
  inv1   g027(.a(x37), .O(new_n90_));
  nand2  g028(.a(x28), .b(x27), .O(new_n91_));
  oai22  g029(.a(new_n91_), .b(new_n71_), .c(new_n90_), .d(x27), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g031(.a(x21), .O(new_n94_));
  inv1   g032(.a(new_n64_), .O(new_n95_));
  nand3  g033(.a(new_n92_), .b(new_n95_), .c(new_n94_), .O(z04));
  nand2  g034(.a(new_n91_), .b(new_n90_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n95_), .O(z06));
  inv1   g036(.a(z06), .O(z05));
  inv1   g037(.a(x14), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n65_), .O(new_n101_));
  nor2   g039(.a(x33), .b(x31), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n100_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n105_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n95_), .c(x03), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(new_n103_), .O(z07));
  nand2  g047(.a(new_n75_), .b(new_n95_), .O(z08));
  and2   g048(.a(x34), .b(x27), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(x26), .c(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n95_), .O(z09));
  nand2  g051(.a(new_n75_), .b(x07), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g054(.a(x40), .b(x39), .c(x30), .d(x05), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(new_n118_));
  aoi21  g056(.a(new_n116_), .b(new_n81_), .c(new_n118_), .O(new_n119_));
  inv1   g057(.a(x04), .O(new_n120_));
  nand3  g058(.a(x39), .b(x05), .c(new_n120_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x40), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(x32), .c(new_n124_), .O(new_n125_));
  oai21  g063(.a(new_n119_), .b(x04), .c(new_n125_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n80_), .c(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  nand4  g070(.a(x35), .b(new_n79_), .c(x27), .d(new_n120_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(new_n131_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n95_), .O(z11));
  nand2  g073(.a(new_n76_), .b(new_n73_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n68_), .c(new_n67_), .O(new_n137_));
  nand2  g075(.a(new_n76_), .b(x10), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n137_), .c(new_n95_), .O(z12));
  aoi21  g078(.a(x36), .b(x35), .c(new_n79_), .O(new_n141_));
  inv1   g079(.a(x13), .O(new_n142_));
  nand4  g080(.a(x40), .b(x39), .c(new_n142_), .d(new_n120_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x20), .O(new_n145_));
  oai22  g083(.a(new_n145_), .b(new_n68_), .c(new_n143_), .d(new_n141_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  nand2  g085(.a(x40), .b(x30), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n63_), .O(new_n149_));
  nand3  g087(.a(x39), .b(new_n142_), .c(new_n120_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x40), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n147_), .O(z13));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(new_n63_), .c(new_n82_), .d(x28), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n143_), .c(new_n145_), .O(new_n157_));
  aoi21  g095(.a(new_n79_), .b(x27), .c(new_n83_), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n143_), .c(new_n68_), .d(new_n67_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n157_), .c(new_n95_), .O(z14));
  nand4  g098(.a(new_n111_), .b(new_n95_), .c(x26), .d(x12), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z15));
  inv1   g100(.a(x01), .O(new_n163_));
  inv1   g101(.a(x22), .O(new_n164_));
  nor4   g102(.a(new_n64_), .b(x23), .c(new_n164_), .d(new_n163_), .O(z16));
  inv1   g103(.a(x24), .O(new_n166_));
  nor2   g104(.a(new_n164_), .b(new_n163_), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(new_n95_), .c(new_n166_), .d(x23), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z17));
  nand4  g107(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n95_), .O(z18));
  oai21  g108(.a(new_n71_), .b(new_n67_), .c(new_n132_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n129_), .c(new_n95_), .O(z20));
  nand2  g110(.a(new_n134_), .b(new_n95_), .O(z19));
endmodule


