// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n98_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n154_, new_n156_, new_n158_, new_n159_, new_n161_, new_n163_;
  inv1   g000(.a(x16), .O(new_n63_));
  aoi21  g001(.a(x35), .b(x28), .c(x37), .O(new_n64_));
  nor2   g002(.a(x36), .b(x35), .O(new_n65_));
  inv1   g003(.a(x04), .O(new_n66_));
  nand3  g004(.a(x40), .b(x39), .c(new_n66_), .O(new_n67_));
  oai22  g005(.a(new_n67_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n68_));
  nand2  g006(.a(x40), .b(x39), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(z08));
  nand3  g008(.a(z08), .b(x10), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(new_n71_), .O(new_n72_));
  aoi21  g010(.a(new_n68_), .b(x27), .c(new_n72_), .O(new_n73_));
  nand2  g011(.a(x40), .b(new_n63_), .O(new_n74_));
  oai21  g012(.a(new_n73_), .b(x15), .c(new_n74_), .O(z00));
  nor2   g013(.a(x40), .b(x16), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  oai21  g017(.a(new_n79_), .b(x36), .c(x27), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x04), .c(new_n76_), .O(z01));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n78_), .c(x04), .O(new_n85_));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n69_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  xor2a  g026(.a(new_n88_), .b(new_n76_), .O(z02));
  nand2  g027(.a(x35), .b(x28), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x27), .O(new_n91_));
  inv1   g029(.a(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n92_), .c(new_n76_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n91_), .c(new_n96_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n76_), .O(z05));
  inv1   g037(.a(x40), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n63_), .O(new_n101_));
  and2   g039(.a(new_n98_), .b(new_n101_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n101_), .O(z07));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n101_), .O(z09));
  nand2  g053(.a(z08), .b(x05), .O(new_n116_));
  nand3  g054(.a(new_n101_), .b(new_n69_), .c(x07), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  inv1   g056(.a(x36), .O(new_n119_));
  aoi21  g057(.a(new_n78_), .b(new_n119_), .c(x04), .O(new_n120_));
  nand2  g058(.a(x37), .b(x06), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n76_), .O(new_n122_));
  aoi21  g060(.a(new_n120_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nor2   g061(.a(new_n81_), .b(new_n67_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(x05), .O(new_n125_));
  oai21  g063(.a(new_n123_), .b(new_n92_), .c(new_n125_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  nand2  g065(.a(z08), .b(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n78_), .c(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand3  g068(.a(new_n79_), .b(x27), .c(new_n66_), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  and2   g070(.a(new_n132_), .b(new_n101_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  aoi21  g072(.a(new_n130_), .b(new_n129_), .c(new_n134_), .O(z11));
  nor2   g073(.a(new_n69_), .b(x04), .O(new_n136_));
  oai21  g074(.a(x36), .b(x35), .c(new_n136_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n64_), .c(new_n92_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n72_), .c(new_n101_), .O(z12));
  inv1   g077(.a(x13), .O(new_n140_));
  inv1   g078(.a(x35), .O(new_n141_));
  oai21  g079(.a(new_n119_), .b(new_n141_), .c(x28), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n136_), .c(new_n140_), .O(new_n143_));
  nand2  g081(.a(new_n90_), .b(new_n93_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n144_), .c(x20), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n76_), .c(new_n143_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  nand2  g086(.a(new_n124_), .b(new_n140_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(z13));
  aoi21  g088(.a(new_n146_), .b(new_n143_), .c(new_n92_), .O(new_n151_));
  nand2  g089(.a(new_n149_), .b(new_n101_), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(new_n151_), .O(z14));
  nand4  g091(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n101_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  nor3   g094(.a(new_n156_), .b(new_n76_), .c(x23), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x23), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n156_), .c(new_n101_), .O(z17));
  nand2  g098(.a(new_n130_), .b(new_n129_), .O(new_n161_));
  nand3  g099(.a(new_n133_), .b(new_n131_), .c(new_n161_), .O(z18));
  oai21  g100(.a(new_n141_), .b(new_n92_), .c(new_n132_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n129_), .c(new_n101_), .O(z20));
  aoi21  g102(.a(new_n130_), .b(new_n129_), .c(new_n134_), .O(z19));
endmodule


