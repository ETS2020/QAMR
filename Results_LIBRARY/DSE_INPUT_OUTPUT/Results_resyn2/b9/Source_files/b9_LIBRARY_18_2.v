// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n168_, new_n170_, new_n171_, new_n173_, new_n174_,
    new_n175_, new_n177_, new_n179_;
  inv1   g000(.a(x18), .O(new_n63_));
  nor2   g001(.a(x36), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  nand4  g007(.a(x40), .b(x39), .c(x10), .d(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n68_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n65_), .c(new_n66_), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x18), .c(new_n76_), .O(new_n77_));
  nor2   g015(.a(new_n67_), .b(x04), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n77_), .c(new_n74_), .d(new_n65_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n64_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(new_n76_), .c(new_n67_), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  nor2   g023(.a(new_n64_), .b(new_n69_), .O(new_n86_));
  oai21  g024(.a(new_n85_), .b(new_n83_), .c(new_n86_), .O(z01));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n82_), .c(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n73_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n89_), .c(new_n64_), .O(z02));
  nand2  g030(.a(x35), .b(x28), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x27), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n67_), .c(new_n64_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n94_), .c(new_n98_), .O(z04));
  inv1   g037(.a(new_n64_), .O(new_n100_));
  oai21  g038(.a(new_n81_), .b(new_n67_), .c(new_n95_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n100_), .O(z06));
  inv1   g040(.a(z06), .O(z05));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n100_), .O(z07));
  nor2   g052(.a(new_n73_), .b(new_n64_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n100_), .O(z09));
  nor2   g055(.a(new_n73_), .b(x04), .O(new_n118_));
  nand3  g056(.a(new_n85_), .b(new_n118_), .c(x05), .O(new_n119_));
  nand3  g057(.a(x37), .b(x27), .c(x06), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n100_), .O(new_n122_));
  oai21  g060(.a(new_n82_), .b(x18), .c(new_n76_), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  nand2  g062(.a(new_n73_), .b(new_n124_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  nand2  g064(.a(new_n74_), .b(new_n126_), .O(new_n127_));
  nand4  g065(.a(new_n127_), .b(new_n125_), .c(new_n123_), .d(new_n78_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n122_), .O(z10));
  inv1   g067(.a(x08), .O(new_n130_));
  nand3  g068(.a(x40), .b(x39), .c(x29), .O(new_n131_));
  aoi21  g069(.a(new_n131_), .b(new_n82_), .c(new_n130_), .O(new_n132_));
  nand2  g070(.a(new_n131_), .b(x27), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  inv1   g072(.a(new_n82_), .O(new_n135_));
  inv1   g073(.a(x09), .O(new_n136_));
  inv1   g074(.a(x30), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  aoi21  g076(.a(new_n135_), .b(new_n78_), .c(new_n138_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n134_), .c(new_n100_), .O(z18));
  inv1   g078(.a(z18), .O(z11));
  oai21  g079(.a(new_n118_), .b(x28), .c(x35), .O(new_n142_));
  aoi21  g080(.a(new_n118_), .b(x36), .c(x37), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g082(.a(new_n70_), .b(new_n100_), .O(new_n145_));
  aoi21  g083(.a(new_n144_), .b(x27), .c(new_n145_), .O(z12));
  inv1   g084(.a(x13), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n118_), .c(new_n147_), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand2  g089(.a(new_n93_), .b(new_n95_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(x20), .c(new_n151_), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n150_), .c(new_n67_), .O(new_n154_));
  nand2  g092(.a(new_n150_), .b(x18), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand4  g094(.a(x40), .b(x39), .c(new_n147_), .d(new_n69_), .O(new_n157_));
  nor2   g095(.a(new_n157_), .b(new_n84_), .O(new_n158_));
  nand2  g096(.a(x35), .b(x27), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n157_), .c(x36), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(x18), .c(new_n158_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n156_), .O(z13));
  nand2  g100(.a(x28), .b(x18), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(x35), .c(x27), .O(new_n164_));
  aoi22  g102(.a(new_n164_), .b(new_n84_), .c(new_n157_), .d(x18), .O(new_n165_));
  oai21  g103(.a(new_n157_), .b(new_n84_), .c(new_n63_), .O(new_n166_));
  oai22  g104(.a(new_n166_), .b(new_n154_), .c(new_n165_), .d(new_n76_), .O(z14));
  nand4  g105(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n100_), .O(z15));
  inv1   g107(.a(x01), .O(new_n170_));
  inv1   g108(.a(x22), .O(new_n171_));
  nor4   g109(.a(new_n64_), .b(x23), .c(new_n171_), .d(new_n170_), .O(z16));
  inv1   g110(.a(x24), .O(new_n173_));
  nor2   g111(.a(new_n171_), .b(new_n170_), .O(new_n174_));
  nand4  g112(.a(new_n174_), .b(new_n100_), .c(new_n173_), .d(x23), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z17));
  nand2  g114(.a(new_n139_), .b(new_n134_), .O(new_n177_));
  nand2  g115(.a(new_n177_), .b(new_n100_), .O(z19));
  nand3  g116(.a(new_n159_), .b(new_n137_), .c(new_n136_), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(new_n132_), .c(new_n100_), .O(z20));
endmodule


