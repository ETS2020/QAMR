// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n98_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n156_, new_n158_, new_n159_, new_n160_, new_n163_;
  inv1   g000(.a(x27), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  aoi21  g004(.a(new_n66_), .b(new_n65_), .c(new_n63_), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(x10), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  inv1   g010(.a(x16), .O(new_n73_));
  nor2   g011(.a(x36), .b(x10), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  oai21  g013(.a(new_n72_), .b(x15), .c(new_n75_), .O(z00));
  inv1   g014(.a(new_n74_), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  aoi21  g016(.a(x35), .b(new_n78_), .c(x36), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  oai21  g018(.a(new_n79_), .b(new_n63_), .c(new_n80_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n77_), .c(x04), .O(z01));
  nand2  g020(.a(x35), .b(new_n78_), .O(new_n83_));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  nor2   g023(.a(new_n74_), .b(new_n68_), .O(z08));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(z08), .c(new_n85_), .O(z02));
  nand2  g028(.a(x35), .b(x28), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n63_), .O(new_n94_));
  nand4  g032(.a(new_n94_), .b(new_n92_), .c(new_n77_), .d(x21), .O(z03));
  aoi21  g033(.a(new_n93_), .b(new_n63_), .c(x21), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n92_), .c(new_n74_), .O(z04));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n77_), .c(new_n93_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x25), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x25), .b(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(x38), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x15), .O(new_n107_));
  or2    g045(.a(x33), .b(x31), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(new_n107_), .c(x14), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n105_), .c(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n77_), .O(z07));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n77_), .O(z09));
  oai21  g051(.a(new_n79_), .b(new_n63_), .c(new_n68_), .O(new_n114_));
  inv1   g052(.a(x05), .O(new_n115_));
  nand2  g053(.a(new_n69_), .b(new_n115_), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  aoi21  g055(.a(new_n68_), .b(new_n117_), .c(x04), .O(new_n118_));
  nand4  g056(.a(new_n118_), .b(new_n116_), .c(new_n114_), .d(new_n81_), .O(new_n119_));
  nand3  g057(.a(x37), .b(x27), .c(x06), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(new_n74_), .O(z10));
  inv1   g059(.a(x08), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(x29), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n83_), .c(new_n122_), .O(new_n124_));
  nand2  g062(.a(new_n123_), .b(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nor2   g064(.a(x30), .b(x09), .O(new_n127_));
  nor2   g065(.a(x28), .b(new_n63_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(x35), .c(new_n64_), .O(new_n129_));
  and2   g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n77_), .O(z11));
  nor3   g070(.a(new_n74_), .b(new_n71_), .c(new_n67_), .O(z12));
  nand2  g071(.a(new_n91_), .b(new_n93_), .O(new_n134_));
  nor2   g072(.a(x19), .b(x18), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(x20), .O(new_n136_));
  nor2   g074(.a(x13), .b(x04), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n69_), .c(new_n78_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n136_), .c(new_n63_), .O(new_n139_));
  nand2  g077(.a(new_n137_), .b(new_n69_), .O(new_n140_));
  nor2   g078(.a(new_n140_), .b(new_n80_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n139_), .c(new_n77_), .O(new_n142_));
  inv1   g080(.a(x36), .O(new_n143_));
  nand2  g081(.a(x35), .b(x27), .O(new_n144_));
  nor3   g082(.a(new_n144_), .b(new_n140_), .c(new_n143_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n142_), .O(z13));
  inv1   g085(.a(x30), .O(new_n148_));
  inv1   g086(.a(x32), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nor2   g088(.a(new_n128_), .b(new_n150_), .O(new_n151_));
  oai22  g089(.a(new_n151_), .b(new_n140_), .c(new_n136_), .d(new_n63_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n77_), .c(new_n145_), .O(z14));
  nand4  g091(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n77_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  nor3   g094(.a(new_n156_), .b(new_n74_), .c(x23), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  inv1   g096(.a(new_n156_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n77_), .c(new_n158_), .d(x23), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z17));
  aoi21  g099(.a(new_n130_), .b(new_n126_), .c(new_n74_), .O(z18));
  nand3  g100(.a(new_n144_), .b(new_n127_), .c(new_n77_), .O(new_n163_));
  nor2   g101(.a(new_n163_), .b(new_n124_), .O(z20));
  nand2  g102(.a(new_n131_), .b(new_n77_), .O(z19));
endmodule


