// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n160_,
    new_n162_, new_n164_, new_n165_, new_n168_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  nand2  g007(.a(x37), .b(x36), .O(new_n70_));
  and2   g008(.a(new_n70_), .b(x16), .O(new_n71_));
  oai21  g009(.a(new_n69_), .b(x15), .c(new_n71_), .O(z00));
  inv1   g010(.a(new_n70_), .O(new_n73_));
  nor2   g011(.a(x32), .b(x30), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  inv1   g013(.a(x27), .O(new_n76_));
  aoi21  g014(.a(new_n74_), .b(new_n76_), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(x28), .c(new_n79_), .O(new_n80_));
  oai22  g018(.a(new_n80_), .b(new_n75_), .c(new_n77_), .d(new_n73_), .O(z01));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  nand2  g023(.a(x40), .b(x39), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n85_), .c(new_n73_), .O(z02));
  nand2  g027(.a(new_n70_), .b(x21), .O(new_n90_));
  inv1   g028(.a(new_n90_), .O(new_n91_));
  nor2   g029(.a(new_n78_), .b(new_n82_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x27), .O(new_n93_));
  nand3  g031(.a(x37), .b(new_n79_), .c(new_n76_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n91_), .O(z03));
  inv1   g034(.a(x37), .O(new_n97_));
  nand2  g035(.a(new_n93_), .b(new_n97_), .O(new_n98_));
  inv1   g036(.a(new_n92_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n79_), .c(x27), .O(new_n100_));
  nand3  g038(.a(new_n100_), .b(new_n98_), .c(new_n90_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(z06));
  nor2   g040(.a(z06), .b(new_n73_), .O(z05));
  inv1   g041(.a(x14), .O(new_n104_));
  inv1   g042(.a(x15), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n105_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n104_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n104_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n70_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nor2   g052(.a(new_n73_), .b(new_n86_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n70_), .O(z09));
  aoi21  g055(.a(x37), .b(x36), .c(new_n76_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n80_), .O(new_n119_));
  nand2  g057(.a(new_n75_), .b(new_n70_), .O(new_n120_));
  nand3  g058(.a(x40), .b(x39), .c(x05), .O(new_n121_));
  aoi21  g059(.a(new_n120_), .b(new_n119_), .c(new_n121_), .O(new_n122_));
  nand4  g060(.a(new_n118_), .b(new_n80_), .c(new_n86_), .d(x07), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  oai21  g062(.a(new_n124_), .b(new_n122_), .c(new_n63_), .O(new_n125_));
  nand4  g063(.a(x37), .b(new_n79_), .c(x27), .d(x06), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n125_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n83_), .c(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  inv1   g070(.a(new_n83_), .O(new_n133_));
  nor2   g071(.a(new_n76_), .b(x04), .O(new_n134_));
  or2    g072(.a(x30), .b(x09), .O(new_n135_));
  aoi21  g073(.a(new_n134_), .b(new_n133_), .c(new_n135_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n132_), .c(new_n70_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  inv1   g076(.a(x10), .O(new_n139_));
  nor3   g077(.a(x37), .b(x36), .c(x35), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n76_), .c(new_n139_), .O(new_n141_));
  oai21  g079(.a(new_n66_), .b(new_n76_), .c(new_n64_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(new_n70_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand4  g082(.a(x40), .b(x39), .c(new_n144_), .d(new_n63_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n75_), .c(new_n73_), .O(new_n147_));
  oai21  g085(.a(new_n79_), .b(new_n78_), .c(x28), .O(new_n148_));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x20), .O(new_n150_));
  nor2   g088(.a(new_n150_), .b(new_n66_), .O(new_n151_));
  aoi21  g089(.a(new_n148_), .b(new_n146_), .c(new_n151_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n76_), .c(new_n147_), .O(z13));
  nand2  g091(.a(new_n147_), .b(new_n76_), .O(new_n154_));
  aoi21  g092(.a(new_n74_), .b(x28), .c(new_n145_), .O(new_n155_));
  nand2  g093(.a(new_n145_), .b(new_n97_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x36), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n150_), .c(new_n66_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n155_), .c(new_n154_), .O(z14));
  nand4  g097(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n70_), .O(z15));
  nand2  g099(.a(x22), .b(x01), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(x23), .c(new_n70_), .O(z16));
  inv1   g101(.a(x24), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x23), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n162_), .c(new_n70_), .O(z17));
  aoi21  g104(.a(new_n136_), .b(new_n132_), .c(new_n73_), .O(z18));
  nor2   g105(.a(new_n78_), .b(new_n76_), .O(new_n168_));
  nor4   g106(.a(new_n168_), .b(new_n135_), .c(new_n130_), .d(new_n73_), .O(z20));
  inv1   g107(.a(new_n137_), .O(z19));
endmodule


