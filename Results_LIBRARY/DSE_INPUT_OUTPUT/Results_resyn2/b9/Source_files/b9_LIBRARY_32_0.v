// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n156_, new_n158_, new_n160_, new_n161_, new_n163_,
    new_n165_;
  nand2  g000(.a(x40), .b(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x04), .O(new_n64_));
  aoi21  g002(.a(x35), .b(x28), .c(x37), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  nand3  g005(.a(x40), .b(x39), .c(new_n67_), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(new_n69_));
  aoi22  g007(.a(new_n69_), .b(x27), .c(new_n64_), .d(x10), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x40), .b(x36), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(new_n72_), .O(new_n75_));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(new_n76_), .c(new_n79_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n75_), .c(x04), .O(z01));
  nand2  g019(.a(x35), .b(new_n77_), .O(new_n82_));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  aoi21  g022(.a(x29), .b(x08), .c(x02), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(new_n63_), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  xor2a  g025(.a(new_n87_), .b(new_n72_), .O(z02));
  inv1   g026(.a(x35), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n77_), .c(x27), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n76_), .c(new_n72_), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n90_), .c(x21), .O(z03));
  inv1   g031(.a(x21), .O(new_n94_));
  nand3  g032(.a(new_n92_), .b(new_n90_), .c(new_n94_), .O(z04));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n75_), .c(new_n91_), .O(z05));
  inv1   g035(.a(z05), .O(z06));
  inv1   g036(.a(x25), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(x00), .O(new_n100_));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x25), .b(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n100_), .c(x38), .O(new_n103_));
  inv1   g041(.a(x17), .O(new_n104_));
  nor2   g042(.a(new_n104_), .b(x15), .O(new_n105_));
  or2    g043(.a(x33), .b(x31), .O(new_n106_));
  oai21  g044(.a(new_n106_), .b(new_n105_), .c(x14), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n103_), .c(x03), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n75_), .O(z07));
  inv1   g047(.a(new_n63_), .O(z08));
  inv1   g048(.a(x11), .O(new_n111_));
  nand3  g049(.a(x34), .b(x27), .c(x26), .O(new_n112_));
  nor3   g050(.a(new_n112_), .b(new_n72_), .c(new_n111_), .O(z09));
  inv1   g051(.a(x39), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(x05), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n78_), .c(x40), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  aoi21  g055(.a(new_n63_), .b(new_n117_), .c(new_n76_), .O(new_n118_));
  inv1   g056(.a(x05), .O(new_n119_));
  nor3   g057(.a(new_n79_), .b(new_n63_), .c(new_n119_), .O(new_n120_));
  aoi21  g058(.a(new_n118_), .b(new_n116_), .c(new_n120_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n72_), .O(new_n124_));
  oai21  g062(.a(new_n121_), .b(x04), .c(new_n124_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n82_), .c(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand4  g068(.a(x35), .b(new_n77_), .c(x27), .d(new_n67_), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  and2   g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n130_), .c(new_n75_), .O(z18));
  inv1   g072(.a(z18), .O(z11));
  inv1   g073(.a(x10), .O(new_n136_));
  nor3   g074(.a(x37), .b(x36), .c(x35), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n76_), .c(new_n136_), .O(new_n138_));
  oai21  g076(.a(new_n65_), .b(new_n76_), .c(new_n68_), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(new_n75_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  nand2  g079(.a(x36), .b(x35), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x28), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n64_), .c(new_n141_), .O(new_n144_));
  inv1   g082(.a(new_n65_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n145_), .c(x20), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n72_), .c(new_n144_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  nand2  g087(.a(new_n64_), .b(new_n141_), .O(new_n150_));
  or2    g088(.a(new_n150_), .b(new_n79_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(z13));
  aoi21  g090(.a(new_n147_), .b(new_n144_), .c(new_n76_), .O(new_n153_));
  oai21  g091(.a(new_n150_), .b(new_n79_), .c(new_n75_), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n153_), .O(z14));
  inv1   g093(.a(x12), .O(new_n156_));
  oai21  g094(.a(new_n112_), .b(new_n156_), .c(new_n75_), .O(z15));
  nand2  g095(.a(x22), .b(x01), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(x23), .c(new_n75_), .O(z16));
  inv1   g097(.a(x24), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x23), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n158_), .c(new_n75_), .O(z17));
  nand2  g100(.a(new_n133_), .b(new_n130_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n75_), .O(z19));
  oai21  g102(.a(new_n89_), .b(new_n76_), .c(new_n132_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n128_), .c(new_n75_), .O(z20));
endmodule


