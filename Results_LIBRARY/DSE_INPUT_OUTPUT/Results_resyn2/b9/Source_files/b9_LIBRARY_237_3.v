// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n172_, new_n173_;
  inv1   g000(.a(x33), .O(new_n63_));
  inv1   g001(.a(x36), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  nand4  g008(.a(x40), .b(x39), .c(x10), .d(new_n70_), .O(new_n71_));
  oai21  g009(.a(new_n69_), .b(new_n68_), .c(new_n71_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n66_), .c(new_n67_), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n65_), .O(z08));
  nor2   g013(.a(new_n68_), .b(x04), .O(new_n76_));
  nor2   g014(.a(x36), .b(x35), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x15), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(z08), .O(new_n79_));
  oai21  g017(.a(new_n73_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(new_n64_), .c(new_n68_), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  nor2   g023(.a(new_n65_), .b(new_n70_), .O(new_n86_));
  oai21  g024(.a(new_n85_), .b(new_n83_), .c(new_n86_), .O(z01));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n82_), .c(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n74_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(new_n89_), .c(new_n65_), .O(z02));
  inv1   g030(.a(x35), .O(new_n93_));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n93_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  inv1   g036(.a(new_n65_), .O(new_n99_));
  nand3  g037(.a(new_n96_), .b(new_n99_), .c(new_n98_), .O(z04));
  aoi21  g038(.a(new_n95_), .b(new_n94_), .c(new_n65_), .O(z05));
  nand3  g039(.a(new_n95_), .b(new_n99_), .c(new_n94_), .O(new_n102_));
  inv1   g040(.a(new_n102_), .O(z06));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  inv1   g048(.a(x31), .O(new_n111_));
  nand2  g049(.a(new_n63_), .b(new_n111_), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n110_), .c(x14), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n108_), .c(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n99_), .O(z07));
  inv1   g053(.a(x11), .O(new_n116_));
  nand4  g054(.a(new_n99_), .b(x34), .c(x27), .d(x26), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(z09));
  inv1   g056(.a(new_n74_), .O(new_n119_));
  nand2  g057(.a(new_n85_), .b(new_n119_), .O(new_n120_));
  nand2  g058(.a(new_n82_), .b(new_n64_), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  nand2  g060(.a(new_n74_), .b(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n121_), .c(x27), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n120_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  aoi21  g064(.a(new_n119_), .b(new_n126_), .c(x04), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n99_), .O(z10));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n82_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  inv1   g073(.a(new_n82_), .O(new_n136_));
  inv1   g074(.a(x09), .O(new_n137_));
  inv1   g075(.a(x30), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  aoi21  g077(.a(new_n136_), .b(new_n76_), .c(new_n139_), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n135_), .c(new_n65_), .O(z18));
  inv1   g079(.a(z18), .O(z11));
  nand2  g080(.a(new_n119_), .b(new_n70_), .O(new_n143_));
  oai21  g081(.a(new_n77_), .b(new_n143_), .c(new_n69_), .O(new_n144_));
  nand2  g082(.a(new_n71_), .b(new_n99_), .O(new_n145_));
  aoi21  g083(.a(new_n144_), .b(x27), .c(new_n145_), .O(z12));
  nand2  g084(.a(x35), .b(x28), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n94_), .O(new_n148_));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(x20), .O(new_n150_));
  nor2   g088(.a(x13), .b(x04), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n119_), .c(new_n81_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n150_), .c(new_n68_), .O(new_n153_));
  nand2  g091(.a(new_n151_), .b(new_n119_), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n84_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n153_), .c(new_n99_), .O(new_n156_));
  inv1   g094(.a(new_n154_), .O(new_n157_));
  nor2   g095(.a(new_n64_), .b(new_n93_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(new_n157_), .c(new_n63_), .d(x27), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n156_), .O(z13));
  nor2   g098(.a(new_n158_), .b(new_n81_), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n154_), .c(new_n150_), .O(new_n162_));
  oai21  g100(.a(new_n154_), .b(new_n84_), .c(new_n99_), .O(new_n163_));
  aoi21  g101(.a(new_n162_), .b(x27), .c(new_n163_), .O(z14));
  inv1   g102(.a(x12), .O(new_n165_));
  nor2   g103(.a(new_n117_), .b(new_n165_), .O(z15));
  inv1   g104(.a(x23), .O(new_n167_));
  nand4  g105(.a(new_n99_), .b(new_n167_), .c(x22), .d(x01), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z16));
  nand2  g107(.a(x22), .b(x01), .O(new_n170_));
  nor4   g108(.a(new_n170_), .b(new_n65_), .c(x24), .d(new_n167_), .O(z17));
  nand2  g109(.a(x35), .b(x27), .O(new_n172_));
  nand4  g110(.a(new_n172_), .b(new_n99_), .c(new_n138_), .d(new_n137_), .O(new_n173_));
  nor2   g111(.a(new_n173_), .b(new_n133_), .O(z20));
  inv1   g112(.a(z18), .O(z19));
endmodule


