// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n152_, new_n154_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_, new_n163_,
    new_n164_, new_n165_, new_n166_, new_n168_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x13), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x35), .O(new_n72_));
  inv1   g010(.a(x36), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n72_), .c(new_n67_), .O(new_n74_));
  nand3  g012(.a(x40), .b(x39), .c(x13), .O(new_n75_));
  nor3   g013(.a(new_n75_), .b(x15), .c(x04), .O(new_n76_));
  oai21  g014(.a(new_n74_), .b(x10), .c(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x30), .O(new_n79_));
  inv1   g017(.a(x32), .O(new_n80_));
  nor2   g018(.a(new_n72_), .b(x28), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(x36), .c(x27), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n80_), .c(new_n79_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g022(.a(x04), .O(new_n85_));
  inv1   g023(.a(x08), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  aoi21  g026(.a(new_n67_), .b(new_n86_), .c(new_n88_), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n75_), .O(new_n91_));
  oai21  g029(.a(new_n89_), .b(new_n85_), .c(new_n91_), .O(z02));
  inv1   g030(.a(new_n64_), .O(new_n93_));
  nand2  g031(.a(new_n69_), .b(x27), .O(new_n94_));
  nand2  g032(.a(new_n68_), .b(new_n67_), .O(new_n95_));
  nand4  g033(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(x21), .O(z03));
  aoi21  g034(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n94_), .c(new_n64_), .O(z04));
  oai21  g036(.a(new_n87_), .b(new_n67_), .c(new_n68_), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n93_), .O(z06));
  inv1   g038(.a(z06), .O(z05));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n65_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n102_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n93_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  inv1   g049(.a(x39), .O(new_n112_));
  aoi21  g050(.a(new_n112_), .b(x13), .c(new_n63_), .O(z08));
  nand4  g051(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n93_), .O(z09));
  aoi21  g053(.a(new_n88_), .b(new_n73_), .c(new_n67_), .O(new_n116_));
  nand2  g054(.a(x40), .b(x39), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(x07), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  aoi21  g058(.a(new_n80_), .b(new_n79_), .c(new_n119_), .O(new_n121_));
  aoi21  g059(.a(new_n120_), .b(new_n116_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n64_), .O(new_n125_));
  oai21  g063(.a(new_n122_), .b(x04), .c(new_n125_), .O(z10));
  nor2   g064(.a(x30), .b(x09), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  nand3  g066(.a(new_n127_), .b(new_n86_), .c(x04), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x13), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x40), .O(new_n131_));
  nand2  g069(.a(new_n67_), .b(new_n86_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n81_), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  inv1   g072(.a(new_n134_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x08), .O(new_n136_));
  nand3  g074(.a(new_n134_), .b(x27), .c(x04), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(new_n138_));
  aoi21  g076(.a(new_n136_), .b(new_n133_), .c(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n128_), .c(new_n131_), .O(z11));
  inv1   g078(.a(x10), .O(new_n141_));
  nor3   g079(.a(x37), .b(x36), .c(x35), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n67_), .c(new_n141_), .O(new_n143_));
  nor2   g081(.a(new_n117_), .b(x04), .O(new_n144_));
  or2    g082(.a(new_n144_), .b(new_n70_), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n143_), .c(new_n93_), .O(z12));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x20), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(new_n70_), .c(new_n64_), .O(z14));
  inv1   g088(.a(z14), .O(z13));
  nand4  g089(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n93_), .O(z15));
  nand2  g091(.a(x22), .b(x01), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(x23), .c(new_n93_), .O(z16));
  inv1   g093(.a(x24), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x23), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n154_), .c(new_n93_), .O(z17));
  nand2  g096(.a(x27), .b(x04), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n89_), .c(new_n128_), .O(new_n160_));
  nand3  g098(.a(new_n135_), .b(x13), .c(x08), .O(new_n161_));
  oai21  g099(.a(new_n160_), .b(new_n64_), .c(new_n161_), .O(z18));
  aoi21  g100(.a(new_n134_), .b(new_n88_), .c(new_n86_), .O(new_n163_));
  nand2  g101(.a(new_n134_), .b(x27), .O(new_n164_));
  nand3  g102(.a(new_n81_), .b(x27), .c(new_n85_), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n127_), .c(new_n93_), .O(new_n166_));
  aoi21  g104(.a(new_n164_), .b(new_n163_), .c(new_n166_), .O(z19));
  oai21  g105(.a(new_n72_), .b(new_n67_), .c(new_n127_), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(new_n163_), .c(new_n93_), .O(z20));
endmodule


