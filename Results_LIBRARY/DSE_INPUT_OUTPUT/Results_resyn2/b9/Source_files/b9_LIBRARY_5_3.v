// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n174_, new_n176_, new_n177_, new_n179_, new_n180_,
    new_n183_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x34), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  inv1   g006(.a(x37), .O(new_n69_));
  nand2  g007(.a(x35), .b(x28), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n66_), .c(new_n67_), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g014(.a(x40), .b(x39), .c(new_n63_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(z08));
  nand4  g016(.a(z08), .b(new_n76_), .c(new_n66_), .d(new_n73_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n81_), .c(new_n68_), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  inv1   g023(.a(new_n85_), .O(new_n86_));
  nor2   g024(.a(new_n65_), .b(new_n73_), .O(new_n87_));
  oai21  g025(.a(new_n86_), .b(new_n84_), .c(new_n87_), .O(z01));
  inv1   g026(.a(x08), .O(new_n89_));
  nand2  g027(.a(new_n68_), .b(new_n89_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(x35), .c(new_n82_), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x39), .O(new_n95_));
  aoi21  g033(.a(new_n91_), .b(x04), .c(new_n95_), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(x34), .c(x40), .O(z02));
  inv1   g035(.a(new_n65_), .O(new_n98_));
  nand2  g036(.a(new_n70_), .b(x27), .O(new_n99_));
  nand2  g037(.a(new_n69_), .b(new_n68_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(x21), .O(z03));
  aoi21  g039(.a(new_n69_), .b(new_n68_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n99_), .c(new_n65_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n98_), .c(new_n69_), .O(z05));
  inv1   g043(.a(z05), .O(z06));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n109_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(x38), .O(new_n111_));
  inv1   g049(.a(x17), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(x15), .O(new_n113_));
  or2    g051(.a(x33), .b(x31), .O(new_n114_));
  oai21  g052(.a(new_n114_), .b(new_n113_), .c(x14), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n111_), .c(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n98_), .O(z07));
  nand2  g055(.a(x26), .b(x11), .O(new_n118_));
  nor4   g056(.a(new_n118_), .b(x40), .c(new_n63_), .d(new_n68_), .O(z09));
  oai21  g057(.a(x39), .b(x34), .c(x40), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(x07), .O(new_n121_));
  nand4  g059(.a(x40), .b(x39), .c(new_n63_), .d(x05), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  aoi21  g061(.a(new_n83_), .b(new_n81_), .c(x04), .O(new_n124_));
  nand2  g062(.a(x37), .b(x06), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n65_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(new_n123_), .c(new_n126_), .O(new_n127_));
  inv1   g065(.a(new_n122_), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n86_), .c(new_n73_), .O(new_n129_));
  oai21  g067(.a(new_n127_), .b(new_n68_), .c(new_n129_), .O(z10));
  inv1   g068(.a(x39), .O(new_n131_));
  oai21  g069(.a(new_n93_), .b(new_n131_), .c(new_n63_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x40), .O(new_n133_));
  aoi21  g071(.a(new_n68_), .b(new_n89_), .c(new_n83_), .O(new_n134_));
  inv1   g072(.a(x09), .O(new_n135_));
  inv1   g073(.a(x30), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g075(.a(x27), .b(x04), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n134_), .c(new_n137_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n133_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z11));
  inv1   g079(.a(x35), .O(new_n142_));
  nand3  g080(.a(new_n69_), .b(new_n81_), .c(new_n142_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x27), .c(x10), .O(new_n144_));
  aoi21  g082(.a(x39), .b(new_n73_), .c(new_n71_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n144_), .c(new_n63_), .O(new_n146_));
  nor2   g084(.a(new_n71_), .b(x40), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n146_), .O(z12));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand3  g089(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n152_));
  aoi21  g090(.a(new_n70_), .b(new_n69_), .c(new_n152_), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x28), .O(new_n155_));
  inv1   g093(.a(x13), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n73_), .O(new_n157_));
  nor2   g095(.a(new_n157_), .b(new_n77_), .O(new_n158_));
  aoi21  g096(.a(new_n158_), .b(new_n155_), .c(new_n153_), .O(new_n159_));
  nand2  g097(.a(new_n98_), .b(x27), .O(new_n160_));
  nor2   g098(.a(new_n157_), .b(new_n85_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(z08), .O(new_n162_));
  oai21  g100(.a(new_n160_), .b(new_n159_), .c(new_n162_), .O(z13));
  nand3  g101(.a(new_n154_), .b(new_n85_), .c(x28), .O(new_n164_));
  nand3  g102(.a(x39), .b(new_n156_), .c(new_n73_), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(new_n166_));
  aoi21  g104(.a(new_n166_), .b(new_n164_), .c(x34), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n64_), .c(new_n152_), .O(new_n168_));
  oai21  g106(.a(x28), .b(new_n68_), .c(new_n85_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n166_), .O(new_n170_));
  nor2   g108(.a(new_n71_), .b(x34), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n170_), .c(new_n147_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n168_), .O(z14));
  nand3  g111(.a(x27), .b(x26), .c(x12), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n64_), .c(new_n63_), .O(z15));
  inv1   g113(.a(x23), .O(new_n176_));
  nand4  g114(.a(new_n98_), .b(new_n176_), .c(x22), .d(x01), .O(new_n177_));
  inv1   g115(.a(new_n177_), .O(z16));
  inv1   g116(.a(x24), .O(new_n179_));
  nand4  g117(.a(new_n179_), .b(x23), .c(x22), .d(x01), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n98_), .O(z17));
  oai22  g119(.a(new_n139_), .b(new_n65_), .c(new_n93_), .d(new_n77_), .O(z18));
  nand3  g120(.a(x40), .b(x39), .c(x29), .O(new_n183_));
  aoi21  g121(.a(new_n183_), .b(new_n83_), .c(new_n89_), .O(new_n184_));
  nand2  g122(.a(x35), .b(x27), .O(new_n185_));
  nand4  g123(.a(new_n185_), .b(new_n98_), .c(new_n136_), .d(new_n135_), .O(new_n186_));
  nor2   g124(.a(new_n186_), .b(new_n184_), .O(z20));
  inv1   g125(.a(new_n140_), .O(z19));
endmodule


