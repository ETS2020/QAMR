// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n166_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n176_,
    new_n177_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x01), .O(new_n71_));
  inv1   g009(.a(x40), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  oai21  g012(.a(new_n69_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(new_n76_), .c(new_n79_), .O(new_n80_));
  aoi21  g018(.a(new_n80_), .b(x04), .c(new_n73_), .O(z01));
  nand2  g019(.a(x35), .b(new_n77_), .O(new_n82_));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  inv1   g022(.a(x02), .O(new_n85_));
  nand2  g023(.a(x29), .b(x08), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g025(.a(x39), .O(new_n88_));
  nor2   g026(.a(new_n72_), .b(new_n88_), .O(new_n89_));
  nand4  g027(.a(new_n89_), .b(new_n87_), .c(new_n84_), .d(new_n71_), .O(z02));
  nand2  g028(.a(x35), .b(x28), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n76_), .c(new_n73_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  inv1   g035(.a(new_n73_), .O(new_n98_));
  nand2  g036(.a(x28), .b(x27), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n98_), .c(new_n93_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n102_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n98_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  aoi21  g050(.a(new_n88_), .b(new_n71_), .c(new_n72_), .O(z08));
  nand2  g051(.a(x34), .b(x27), .O(new_n114_));
  nand2  g052(.a(x26), .b(x11), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n114_), .c(new_n98_), .O(z09));
  nor2   g054(.a(new_n78_), .b(new_n76_), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  nor2   g058(.a(new_n89_), .b(new_n120_), .O(new_n121_));
  aoi22  g059(.a(new_n121_), .b(new_n117_), .c(new_n119_), .d(new_n80_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n73_), .O(new_n125_));
  oai21  g063(.a(new_n122_), .b(x04), .c(new_n125_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  nor2   g065(.a(x30), .b(x09), .O(new_n128_));
  nand3  g066(.a(new_n128_), .b(new_n127_), .c(x04), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n71_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x40), .O(new_n131_));
  nand2  g069(.a(new_n89_), .b(x29), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(x27), .c(x04), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(new_n82_), .O(new_n134_));
  oai21  g072(.a(new_n82_), .b(new_n76_), .c(new_n127_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n133_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n128_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n131_), .O(z11));
  nor2   g076(.a(new_n67_), .b(new_n76_), .O(new_n139_));
  nor2   g077(.a(new_n88_), .b(x04), .O(new_n140_));
  nor2   g078(.a(new_n66_), .b(new_n76_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(x10), .c(new_n140_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n71_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x40), .c(new_n139_), .O(z12));
  aoi21  g082(.a(x36), .b(x35), .c(new_n77_), .O(new_n145_));
  inv1   g083(.a(x13), .O(new_n146_));
  nand3  g084(.a(new_n140_), .b(x40), .c(new_n146_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x20), .O(new_n149_));
  oai22  g087(.a(new_n149_), .b(new_n67_), .c(new_n147_), .d(new_n145_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  nand2  g089(.a(new_n140_), .b(new_n146_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n79_), .c(new_n71_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x40), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n151_), .O(z13));
  oai21  g093(.a(x28), .b(new_n76_), .c(new_n79_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n65_), .c(new_n146_), .O(new_n157_));
  oai21  g095(.a(new_n67_), .b(new_n76_), .c(new_n157_), .O(new_n158_));
  nand2  g096(.a(new_n145_), .b(new_n79_), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n147_), .c(new_n149_), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n158_), .c(new_n98_), .O(z14));
  nand2  g100(.a(x26), .b(x12), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n114_), .c(new_n73_), .O(z15));
  inv1   g102(.a(x23), .O(new_n165_));
  nand4  g103(.a(new_n72_), .b(new_n165_), .c(x22), .d(x01), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(z16));
  nand2  g105(.a(x22), .b(x01), .O(new_n168_));
  nor4   g106(.a(new_n168_), .b(x40), .c(x24), .d(new_n165_), .O(z17));
  aoi21  g107(.a(new_n132_), .b(new_n82_), .c(new_n127_), .O(new_n170_));
  nand2  g108(.a(new_n132_), .b(x27), .O(new_n171_));
  nand4  g109(.a(x35), .b(new_n77_), .c(x27), .d(new_n63_), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n128_), .c(new_n98_), .O(new_n173_));
  aoi21  g111(.a(new_n171_), .b(new_n170_), .c(new_n173_), .O(z19));
  inv1   g112(.a(z19), .O(z18));
  nand2  g113(.a(x35), .b(x27), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n128_), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n170_), .c(new_n98_), .O(z20));
endmodule


