// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n171_, new_n173_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  inv1   g009(.a(x31), .O(new_n72_));
  nor2   g010(.a(x36), .b(new_n72_), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  oai21  g012(.a(new_n70_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(new_n73_), .O(new_n76_));
  inv1   g014(.a(x27), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  aoi21  g016(.a(x35), .b(new_n78_), .c(x36), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  oai21  g018(.a(new_n79_), .b(new_n77_), .c(new_n80_), .O(new_n81_));
  nand3  g019(.a(new_n81_), .b(new_n76_), .c(x04), .O(z01));
  nand2  g020(.a(x35), .b(new_n78_), .O(new_n83_));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  nand2  g023(.a(x40), .b(x39), .O(new_n86_));
  nor2   g024(.a(new_n73_), .b(new_n86_), .O(z08));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(z08), .c(new_n85_), .O(z02));
  inv1   g029(.a(x35), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n92_), .c(new_n93_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n73_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  nand3  g035(.a(new_n95_), .b(new_n76_), .c(new_n97_), .O(z04));
  nand3  g036(.a(new_n94_), .b(new_n76_), .c(new_n93_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x25), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x25), .b(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(x38), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x15), .O(new_n107_));
  inv1   g045(.a(x33), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(new_n72_), .O(new_n109_));
  oai21  g047(.a(new_n109_), .b(new_n107_), .c(x14), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n105_), .c(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n76_), .O(z07));
  nand3  g050(.a(x34), .b(x27), .c(x26), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n76_), .c(x11), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z09));
  oai21  g054(.a(new_n79_), .b(new_n77_), .c(new_n86_), .O(new_n117_));
  inv1   g055(.a(x05), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(new_n118_), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  nand2  g058(.a(new_n86_), .b(new_n120_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n119_), .c(new_n65_), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n117_), .c(new_n81_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(new_n73_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n83_), .c(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nor2   g069(.a(new_n92_), .b(new_n77_), .O(new_n132_));
  nor2   g070(.a(x28), .b(x04), .O(new_n133_));
  or2    g071(.a(x30), .b(x09), .O(new_n134_));
  aoi21  g072(.a(new_n133_), .b(new_n132_), .c(new_n134_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n131_), .c(new_n76_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(z11));
  inv1   g075(.a(x10), .O(new_n138_));
  nor3   g076(.a(x37), .b(x36), .c(x35), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n77_), .c(new_n138_), .O(new_n140_));
  oai21  g078(.a(new_n63_), .b(new_n77_), .c(new_n66_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n76_), .O(z12));
  inv1   g080(.a(x13), .O(new_n143_));
  nand4  g081(.a(x40), .b(x39), .c(new_n143_), .d(new_n65_), .O(new_n144_));
  aoi21  g082(.a(x36), .b(x35), .c(new_n78_), .O(new_n145_));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x20), .O(new_n147_));
  oai22  g085(.a(new_n147_), .b(new_n63_), .c(new_n145_), .d(new_n144_), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  inv1   g087(.a(new_n80_), .O(new_n150_));
  inv1   g088(.a(new_n144_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n150_), .c(new_n73_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n149_), .O(z13));
  nand2  g091(.a(new_n93_), .b(new_n92_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n146_), .c(x20), .O(new_n155_));
  aoi21  g093(.a(new_n155_), .b(new_n145_), .c(new_n77_), .O(new_n156_));
  nand2  g094(.a(x35), .b(x28), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n93_), .O(new_n158_));
  nand4  g096(.a(new_n146_), .b(new_n158_), .c(x27), .d(x20), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n144_), .c(new_n73_), .O(new_n160_));
  oai21  g098(.a(new_n156_), .b(new_n150_), .c(new_n160_), .O(z14));
  nand3  g099(.a(new_n114_), .b(new_n76_), .c(x12), .O(new_n162_));
  inv1   g100(.a(new_n162_), .O(z15));
  inv1   g101(.a(x23), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(x22), .c(x01), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n76_), .O(z16));
  nor2   g104(.a(x24), .b(new_n164_), .O(new_n167_));
  nand4  g105(.a(new_n167_), .b(new_n76_), .c(x22), .d(x01), .O(new_n168_));
  inv1   g106(.a(new_n168_), .O(z17));
  aoi21  g107(.a(new_n135_), .b(new_n131_), .c(new_n73_), .O(z18));
  nand2  g108(.a(new_n135_), .b(new_n131_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n76_), .O(z19));
  or2    g110(.a(new_n134_), .b(new_n132_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n129_), .c(new_n76_), .O(z20));
endmodule


