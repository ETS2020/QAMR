// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n166_, new_n168_,
    new_n169_, new_n171_, new_n173_, new_n174_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  aoi21  g004(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  inv1   g005(.a(new_n67_), .O(new_n68_));
  nand2  g006(.a(x40), .b(x39), .O(new_n69_));
  oai21  g007(.a(new_n69_), .b(x04), .c(new_n68_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  nand2  g009(.a(new_n64_), .b(new_n71_), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(new_n70_), .c(new_n63_), .O(new_n73_));
  inv1   g011(.a(x40), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(x36), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n73_), .c(x16), .O(z00));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(x28), .c(new_n79_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n76_), .c(x04), .O(z01));
  inv1   g022(.a(x28), .O(new_n85_));
  nand2  g023(.a(x35), .b(new_n85_), .O(new_n86_));
  inv1   g024(.a(new_n86_), .O(new_n87_));
  oai21  g025(.a(x27), .b(x08), .c(new_n87_), .O(new_n88_));
  and2   g026(.a(x29), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(x02), .c(x39), .O(new_n90_));
  aoi21  g028(.a(new_n88_), .b(x04), .c(new_n90_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n79_), .c(x40), .O(z02));
  nand2  g030(.a(new_n66_), .b(x27), .O(new_n93_));
  aoi21  g031(.a(new_n65_), .b(new_n64_), .c(new_n75_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n93_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n93_), .c(new_n96_), .O(z04));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n76_), .c(new_n65_), .O(z05));
  inv1   g037(.a(z05), .O(z06));
  inv1   g038(.a(x25), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x25), .b(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(x38), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x15), .O(new_n107_));
  or2    g045(.a(x33), .b(x31), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(new_n107_), .c(x14), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n105_), .c(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n76_), .O(z07));
  nand3  g049(.a(x40), .b(x39), .c(x36), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z08));
  inv1   g051(.a(x11), .O(new_n114_));
  nand3  g052(.a(x34), .b(x27), .c(x26), .O(new_n115_));
  nor3   g053(.a(new_n115_), .b(new_n75_), .c(new_n114_), .O(z09));
  inv1   g054(.a(x04), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x36), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n80_), .c(x07), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n118_), .c(new_n64_), .O(new_n121_));
  nor2   g059(.a(new_n82_), .b(new_n69_), .O(new_n122_));
  and2   g060(.a(new_n122_), .b(x05), .O(new_n123_));
  oai21  g061(.a(new_n123_), .b(new_n121_), .c(new_n117_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n75_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n124_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n86_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nor2   g071(.a(new_n64_), .b(x04), .O(new_n134_));
  inv1   g072(.a(x09), .O(new_n135_));
  inv1   g073(.a(x30), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  aoi21  g075(.a(new_n134_), .b(new_n87_), .c(new_n137_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n133_), .c(new_n76_), .O(z18));
  inv1   g077(.a(z18), .O(z11));
  nand3  g078(.a(new_n76_), .b(new_n72_), .c(new_n70_), .O(z12));
  nand2  g079(.a(new_n66_), .b(new_n65_), .O(new_n142_));
  inv1   g080(.a(x18), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand3  g082(.a(x20), .b(new_n144_), .c(new_n143_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n76_), .c(new_n142_), .O(new_n147_));
  inv1   g085(.a(new_n119_), .O(new_n148_));
  nand2  g086(.a(new_n78_), .b(x28), .O(new_n149_));
  nor2   g087(.a(x13), .b(x04), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n149_), .c(new_n148_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand3  g091(.a(new_n150_), .b(new_n122_), .c(x36), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  aoi22  g093(.a(new_n150_), .b(x39), .c(new_n146_), .d(new_n67_), .O(new_n156_));
  inv1   g094(.a(new_n82_), .O(new_n157_));
  nand4  g095(.a(x37), .b(x20), .c(new_n144_), .d(new_n143_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n78_), .c(x28), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(x27), .c(new_n157_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n156_), .c(x36), .O(new_n161_));
  oai21  g099(.a(new_n145_), .b(new_n68_), .c(new_n74_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n161_), .O(z14));
  inv1   g101(.a(x12), .O(new_n164_));
  oai21  g102(.a(new_n115_), .b(new_n164_), .c(new_n76_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(x23), .c(new_n76_), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x23), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n166_), .c(new_n76_), .O(z17));
  nand2  g108(.a(new_n138_), .b(new_n133_), .O(new_n171_));
  nand2  g109(.a(new_n171_), .b(new_n76_), .O(z19));
  nand2  g110(.a(x35), .b(x27), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n136_), .c(new_n135_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n131_), .c(new_n76_), .O(z20));
endmodule


