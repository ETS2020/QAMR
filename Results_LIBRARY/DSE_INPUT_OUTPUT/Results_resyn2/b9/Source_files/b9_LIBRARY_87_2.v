// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n152_, new_n154_, new_n155_,
    new_n157_, new_n158_, new_n159_, new_n161_, new_n163_;
  inv1   g000(.a(x04), .O(new_n63_));
  inv1   g001(.a(x35), .O(new_n64_));
  inv1   g002(.a(x36), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g004(.a(new_n66_), .b(x39), .c(new_n63_), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g007(.a(x39), .b(x10), .c(new_n63_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n69_), .b(x27), .c(new_n71_), .O(new_n72_));
  inv1   g010(.a(x15), .O(new_n73_));
  nand2  g011(.a(x40), .b(new_n73_), .O(new_n74_));
  oai21  g012(.a(new_n74_), .b(new_n72_), .c(x16), .O(z00));
  inv1   g013(.a(x16), .O(new_n76_));
  nor2   g014(.a(x40), .b(new_n76_), .O(new_n77_));
  nor2   g015(.a(new_n64_), .b(x28), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(x36), .c(x27), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n77_), .O(z01));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n84_), .c(x04), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n86_), .O(z02));
  oai21  g028(.a(new_n64_), .b(new_n83_), .c(x27), .O(new_n91_));
  nor2   g029(.a(x37), .b(x27), .O(new_n92_));
  nor2   g030(.a(new_n92_), .b(new_n77_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(new_n91_), .c(x21), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n91_), .c(new_n95_), .O(z04));
  inv1   g034(.a(x40), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(x16), .O(new_n98_));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n98_), .O(z05));
  inv1   g038(.a(new_n99_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n98_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n98_), .O(z07));
  inv1   g051(.a(new_n87_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n98_), .O(z09));
  inv1   g054(.a(x27), .O(new_n117_));
  nand2  g055(.a(z08), .b(x05), .O(new_n118_));
  nand3  g056(.a(new_n87_), .b(new_n98_), .c(x07), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  aoi21  g058(.a(new_n84_), .b(new_n65_), .c(x04), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n77_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n120_), .c(new_n123_), .O(new_n124_));
  inv1   g062(.a(new_n80_), .O(new_n125_));
  nand4  g063(.a(z08), .b(new_n125_), .c(x05), .d(new_n63_), .O(new_n126_));
  oai21  g064(.a(new_n124_), .b(new_n117_), .c(new_n126_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n84_), .c(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand4  g070(.a(x35), .b(new_n83_), .c(x27), .d(new_n63_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  and2   g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n132_), .c(new_n98_), .O(z18));
  inv1   g074(.a(z18), .O(z11));
  oai21  g075(.a(new_n67_), .b(new_n97_), .c(new_n68_), .O(new_n138_));
  nor2   g076(.a(new_n77_), .b(new_n117_), .O(new_n139_));
  aoi22  g077(.a(new_n139_), .b(new_n138_), .c(new_n71_), .d(x40), .O(z12));
  aoi21  g078(.a(x36), .b(x35), .c(new_n83_), .O(new_n141_));
  inv1   g079(.a(x13), .O(new_n142_));
  nand3  g080(.a(z08), .b(new_n142_), .c(new_n63_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x20), .O(new_n145_));
  oai22  g083(.a(new_n145_), .b(new_n68_), .c(new_n143_), .d(new_n141_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  oai21  g085(.a(new_n143_), .b(new_n80_), .c(new_n98_), .O(new_n148_));
  inv1   g086(.a(new_n148_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n147_), .O(z13));
  aoi21  g088(.a(new_n146_), .b(x27), .c(new_n148_), .O(z14));
  nand4  g089(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n98_), .O(z15));
  inv1   g091(.a(x01), .O(new_n154_));
  inv1   g092(.a(x22), .O(new_n155_));
  nor4   g093(.a(new_n77_), .b(x23), .c(new_n155_), .d(new_n154_), .O(z16));
  inv1   g094(.a(x24), .O(new_n157_));
  nor2   g095(.a(new_n155_), .b(new_n154_), .O(new_n158_));
  nand4  g096(.a(new_n158_), .b(new_n98_), .c(new_n157_), .d(x23), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(z17));
  nand2  g098(.a(new_n135_), .b(new_n132_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n98_), .O(z19));
  oai21  g100(.a(new_n64_), .b(new_n117_), .c(new_n134_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n130_), .c(new_n98_), .O(z20));
endmodule


