// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n172_, new_n174_,
    new_n175_, new_n178_;
  nand2  g000(.a(x36), .b(x20), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  nand2  g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x27), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  nand2  g009(.a(x40), .b(x39), .O(new_n72_));
  inv1   g010(.a(new_n72_), .O(new_n73_));
  nand3  g011(.a(new_n73_), .b(x10), .c(new_n71_), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(new_n65_), .c(new_n66_), .O(new_n76_));
  inv1   g014(.a(x35), .O(new_n77_));
  inv1   g015(.a(x36), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n77_), .c(new_n72_), .O(new_n79_));
  inv1   g017(.a(x27), .O(new_n80_));
  nor2   g018(.a(new_n80_), .b(x04), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n63_), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n79_), .c(new_n65_), .O(new_n84_));
  oai21  g022(.a(new_n76_), .b(new_n64_), .c(new_n84_), .O(z00));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  nand2  g025(.a(new_n87_), .b(new_n78_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x27), .O(new_n89_));
  nor2   g027(.a(x32), .b(x30), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g029(.a(new_n91_), .b(new_n63_), .c(x04), .O(z01));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n87_), .c(x04), .O(new_n94_));
  aoi21  g032(.a(x29), .b(x08), .c(x02), .O(new_n95_));
  nor2   g033(.a(new_n95_), .b(new_n72_), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n94_), .c(new_n64_), .O(z02));
  nand2  g035(.a(new_n68_), .b(x27), .O(new_n98_));
  aoi21  g036(.a(new_n67_), .b(new_n80_), .c(new_n64_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n98_), .c(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand3  g039(.a(new_n99_), .b(new_n98_), .c(new_n101_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n64_), .O(z05));
  nand2  g042(.a(new_n103_), .b(new_n63_), .O(new_n105_));
  inv1   g043(.a(new_n105_), .O(z06));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n109_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(x38), .O(new_n111_));
  inv1   g049(.a(x17), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(x15), .O(new_n113_));
  or2    g051(.a(x33), .b(x31), .O(new_n114_));
  oai21  g052(.a(new_n114_), .b(new_n113_), .c(x14), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n111_), .c(x03), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n63_), .O(z07));
  nand2  g055(.a(new_n72_), .b(new_n63_), .O(z08));
  nand2  g056(.a(x34), .b(x27), .O(new_n119_));
  nand2  g057(.a(x26), .b(x11), .O(new_n120_));
  oai21  g058(.a(new_n120_), .b(new_n119_), .c(new_n63_), .O(z09));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  nand2  g061(.a(new_n73_), .b(x05), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nor2   g063(.a(new_n90_), .b(x04), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n123_), .O(new_n127_));
  nand2  g065(.a(new_n72_), .b(x07), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n124_), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n88_), .c(new_n83_), .O(new_n130_));
  oai21  g068(.a(new_n127_), .b(new_n64_), .c(new_n130_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n87_), .c(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nor2   g074(.a(x30), .b(x09), .O(new_n137_));
  nand3  g075(.a(new_n81_), .b(x35), .c(new_n86_), .O(new_n138_));
  and2   g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n63_), .O(z11));
  inv1   g079(.a(new_n69_), .O(new_n142_));
  nand2  g080(.a(new_n79_), .b(new_n71_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g082(.a(new_n74_), .b(new_n63_), .O(new_n145_));
  aoi21  g083(.a(new_n144_), .b(x27), .c(new_n145_), .O(z12));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n69_), .c(x20), .O(new_n148_));
  nor2   g086(.a(x13), .b(x04), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n73_), .c(new_n86_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n148_), .c(x36), .O(new_n151_));
  aoi21  g089(.a(x36), .b(x35), .c(new_n86_), .O(new_n152_));
  inv1   g090(.a(x20), .O(new_n153_));
  nand3  g091(.a(new_n149_), .b(new_n73_), .c(new_n153_), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n151_), .c(x27), .O(new_n156_));
  inv1   g094(.a(new_n90_), .O(new_n157_));
  nand2  g095(.a(new_n149_), .b(new_n73_), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n157_), .c(new_n63_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n156_), .O(z13));
  nand2  g099(.a(new_n157_), .b(new_n63_), .O(new_n162_));
  nor2   g100(.a(new_n78_), .b(new_n77_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(new_n69_), .c(x27), .d(new_n153_), .O(new_n164_));
  nand3  g102(.a(new_n63_), .b(new_n86_), .c(x27), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n164_), .c(new_n162_), .O(new_n166_));
  nand3  g104(.a(new_n147_), .b(new_n78_), .c(x20), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(new_n70_), .O(new_n168_));
  aoi21  g106(.a(new_n166_), .b(new_n159_), .c(new_n168_), .O(z14));
  nand3  g107(.a(new_n63_), .b(x26), .c(x12), .O(new_n170_));
  nor2   g108(.a(new_n170_), .b(new_n119_), .O(z15));
  nand2  g109(.a(x22), .b(x01), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(x23), .c(new_n63_), .O(z16));
  inv1   g111(.a(x24), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x23), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n172_), .c(new_n63_), .O(z17));
  aoi21  g114(.a(new_n139_), .b(new_n136_), .c(new_n64_), .O(z18));
  oai21  g115(.a(new_n77_), .b(new_n80_), .c(new_n137_), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n134_), .c(new_n63_), .O(z20));
  nand2  g117(.a(new_n140_), .b(new_n63_), .O(z19));
endmodule


