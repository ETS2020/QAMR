// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n100_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n167_, new_n169_, new_n170_, new_n173_, new_n174_;
  inv1   g000(.a(x36), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x03), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x10), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nand2  g007(.a(x40), .b(x39), .O(new_n70_));
  oai22  g008(.a(new_n70_), .b(x04), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand3  g009(.a(new_n71_), .b(new_n69_), .c(new_n65_), .O(new_n72_));
  inv1   g010(.a(x16), .O(new_n73_));
  inv1   g011(.a(x04), .O(new_n74_));
  nand3  g012(.a(x27), .b(new_n65_), .c(new_n74_), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  inv1   g014(.a(x35), .O(new_n77_));
  aoi21  g015(.a(new_n63_), .b(new_n77_), .c(new_n70_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n73_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n72_), .c(new_n64_), .O(z00));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  aoi21  g020(.a(x35), .b(new_n82_), .c(x36), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n67_), .c(new_n81_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n64_), .O(z01));
  nand2  g023(.a(x35), .b(new_n82_), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n86_), .c(x04), .O(new_n88_));
  nor2   g026(.a(new_n70_), .b(new_n64_), .O(z08));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(z08), .c(new_n88_), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n77_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  aoi21  g036(.a(new_n96_), .b(new_n98_), .c(new_n64_), .O(z04));
  inv1   g037(.a(new_n64_), .O(new_n100_));
  nand2  g038(.a(new_n95_), .b(new_n94_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n100_), .O(z06));
  inv1   g040(.a(z06), .O(z05));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z07));
  inv1   g052(.a(x11), .O(new_n115_));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  nor3   g054(.a(new_n116_), .b(new_n64_), .c(new_n115_), .O(z09));
  inv1   g055(.a(new_n70_), .O(new_n118_));
  nor2   g056(.a(new_n83_), .b(new_n67_), .O(new_n119_));
  aoi21  g057(.a(new_n119_), .b(x07), .c(new_n118_), .O(new_n120_));
  inv1   g058(.a(x05), .O(new_n121_));
  aoi21  g059(.a(new_n118_), .b(new_n121_), .c(x04), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n84_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n64_), .O(new_n126_));
  oai21  g064(.a(new_n123_), .b(new_n120_), .c(new_n126_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n86_), .c(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nor2   g070(.a(new_n67_), .b(x04), .O(new_n133_));
  inv1   g071(.a(new_n86_), .O(new_n134_));
  or2    g072(.a(x30), .b(x09), .O(new_n135_));
  aoi21  g073(.a(new_n134_), .b(new_n133_), .c(new_n135_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n132_), .c(new_n100_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  nor3   g076(.a(x37), .b(x36), .c(x35), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n67_), .c(new_n66_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n71_), .c(new_n100_), .O(z12));
  nor2   g079(.a(new_n68_), .b(new_n67_), .O(new_n142_));
  nand2  g080(.a(new_n82_), .b(x27), .O(new_n143_));
  inv1   g081(.a(x13), .O(new_n144_));
  nand4  g082(.a(x40), .b(x39), .c(new_n144_), .d(new_n74_), .O(new_n145_));
  aoi21  g083(.a(new_n143_), .b(new_n81_), .c(new_n145_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n142_), .c(new_n100_), .O(new_n147_));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand3  g087(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  nor2   g089(.a(new_n151_), .b(new_n146_), .O(new_n152_));
  inv1   g090(.a(new_n145_), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(new_n153_), .c(x27), .d(x03), .O(new_n156_));
  oai21  g094(.a(new_n152_), .b(new_n147_), .c(new_n156_), .O(z13));
  inv1   g095(.a(new_n142_), .O(new_n158_));
  inv1   g096(.a(new_n146_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand3  g098(.a(new_n154_), .b(new_n81_), .c(x28), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n145_), .c(new_n150_), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n160_), .c(new_n100_), .O(z14));
  inv1   g102(.a(x12), .O(new_n165_));
  oai21  g103(.a(new_n116_), .b(new_n165_), .c(new_n100_), .O(z15));
  nand2  g104(.a(x22), .b(x01), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(x23), .c(new_n100_), .O(z16));
  inv1   g106(.a(x24), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x23), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n167_), .c(new_n100_), .O(z17));
  aoi21  g109(.a(new_n136_), .b(new_n132_), .c(new_n64_), .O(z18));
  nor2   g110(.a(new_n135_), .b(new_n64_), .O(new_n173_));
  oai21  g111(.a(new_n77_), .b(new_n67_), .c(new_n173_), .O(new_n174_));
  nor2   g112(.a(new_n174_), .b(new_n130_), .O(z20));
  inv1   g113(.a(new_n137_), .O(z19));
endmodule


