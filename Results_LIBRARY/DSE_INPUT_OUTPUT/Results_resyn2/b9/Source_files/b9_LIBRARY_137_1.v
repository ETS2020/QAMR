// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n158_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n172_, new_n174_;
  inv1   g000(.a(x40), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x32), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(new_n74_), .c(x32), .d(new_n65_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x35), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  oai21  g018(.a(new_n79_), .b(x28), .c(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x27), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g023(.a(x32), .O(new_n86_));
  nor2   g024(.a(new_n79_), .b(x28), .O(new_n87_));
  inv1   g025(.a(x08), .O(new_n88_));
  nand2  g026(.a(new_n67_), .b(new_n88_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(x39), .O(new_n94_));
  aoi21  g032(.a(new_n90_), .b(x04), .c(new_n94_), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n86_), .c(x40), .O(z02));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  oai22  g035(.a(new_n97_), .b(new_n79_), .c(new_n68_), .d(x27), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  inv1   g038(.a(new_n64_), .O(new_n101_));
  nand3  g039(.a(new_n98_), .b(new_n101_), .c(new_n100_), .O(z04));
  nand2  g040(.a(new_n97_), .b(new_n68_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n101_), .O(z06));
  inv1   g042(.a(z06), .O(z05));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n65_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n106_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n106_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n101_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  inv1   g053(.a(x39), .O(new_n116_));
  aoi21  g054(.a(new_n116_), .b(x32), .c(new_n63_), .O(z08));
  and2   g055(.a(x34), .b(x27), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(x26), .c(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n101_), .O(z09));
  inv1   g058(.a(x04), .O(new_n121_));
  inv1   g059(.a(new_n75_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x05), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  aoi21  g062(.a(x40), .b(x39), .c(new_n124_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n81_), .c(x27), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n121_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  nor2   g068(.a(new_n130_), .b(new_n64_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n128_), .O(z10));
  nor2   g070(.a(x30), .b(x09), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n88_), .c(x04), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x32), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x40), .O(new_n136_));
  nand3  g074(.a(x40), .b(x39), .c(x29), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(x27), .c(x04), .O(new_n138_));
  inv1   g076(.a(x28), .O(new_n139_));
  nand2  g077(.a(x35), .b(new_n139_), .O(new_n140_));
  nand2  g078(.a(new_n137_), .b(new_n140_), .O(new_n141_));
  oai21  g079(.a(new_n140_), .b(new_n67_), .c(new_n88_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(new_n138_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n133_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n136_), .O(z11));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n67_), .c(new_n72_), .O(new_n147_));
  or2    g085(.a(new_n76_), .b(new_n70_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n147_), .c(new_n101_), .O(z12));
  nor2   g087(.a(x13), .b(x04), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(x39), .c(new_n86_), .O(new_n151_));
  nor2   g089(.a(x19), .b(x18), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n70_), .c(x20), .O(new_n153_));
  oai21  g091(.a(new_n151_), .b(new_n63_), .c(new_n153_), .O(z13));
  nand2  g092(.a(new_n150_), .b(new_n122_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n101_), .O(z14));
  nand4  g095(.a(new_n118_), .b(new_n101_), .c(x26), .d(x12), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z15));
  inv1   g097(.a(x01), .O(new_n160_));
  inv1   g098(.a(x22), .O(new_n161_));
  nor4   g099(.a(new_n64_), .b(x23), .c(new_n161_), .d(new_n160_), .O(z16));
  inv1   g100(.a(x24), .O(new_n163_));
  nor2   g101(.a(new_n161_), .b(new_n160_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n101_), .c(new_n163_), .d(x23), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z17));
  aoi21  g104(.a(new_n137_), .b(new_n140_), .c(new_n88_), .O(new_n167_));
  nand2  g105(.a(new_n137_), .b(x27), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  nand3  g107(.a(new_n87_), .b(x27), .c(new_n121_), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(new_n169_), .c(new_n133_), .d(new_n101_), .O(z18));
  nand3  g109(.a(new_n170_), .b(new_n169_), .c(new_n133_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n101_), .O(z19));
  oai21  g111(.a(new_n79_), .b(new_n67_), .c(new_n133_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n167_), .c(new_n101_), .O(z20));
endmodule


