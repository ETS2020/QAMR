// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n155_, new_n157_, new_n162_, new_n163_;
  inv1   g000(.a(x22), .O(new_n63_));
  inv1   g001(.a(x23), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x35), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  and2   g013(.a(x40), .b(x39), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n75_), .c(new_n72_), .O(new_n77_));
  aoi21  g015(.a(new_n77_), .b(new_n71_), .c(new_n67_), .O(new_n78_));
  nand3  g016(.a(new_n76_), .b(x10), .c(new_n72_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(new_n78_), .c(new_n66_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x16), .c(new_n65_), .O(z00));
  inv1   g020(.a(x28), .O(new_n83_));
  aoi21  g021(.a(x35), .b(new_n83_), .c(x36), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  oai21  g023(.a(new_n84_), .b(new_n67_), .c(new_n85_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n65_), .O(z01));
  nand2  g025(.a(x35), .b(new_n83_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(x04), .O(new_n90_));
  nand2  g028(.a(x40), .b(x39), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n65_), .O(z08));
  inv1   g030(.a(x02), .O(new_n93_));
  nand2  g031(.a(x29), .b(x08), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g033(.a(new_n95_), .b(z08), .c(new_n90_), .O(z02));
  inv1   g034(.a(new_n65_), .O(new_n97_));
  nand2  g035(.a(new_n69_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n68_), .b(new_n67_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n97_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n65_), .O(z04));
  oai21  g040(.a(new_n83_), .b(new_n67_), .c(new_n68_), .O(new_n103_));
  and2   g041(.a(new_n103_), .b(new_n97_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n66_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n106_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n106_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n97_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  nand2  g053(.a(x34), .b(x27), .O(new_n116_));
  nand2  g054(.a(x26), .b(x11), .O(new_n117_));
  oai21  g055(.a(new_n117_), .b(new_n116_), .c(new_n97_), .O(z09));
  nor2   g056(.a(new_n84_), .b(new_n67_), .O(new_n119_));
  nand2  g057(.a(new_n76_), .b(x05), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(new_n121_));
  and2   g059(.a(new_n91_), .b(x07), .O(new_n122_));
  aoi22  g060(.a(new_n122_), .b(new_n119_), .c(new_n121_), .d(new_n86_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  inv1   g062(.a(new_n124_), .O(new_n125_));
  nor2   g063(.a(new_n125_), .b(new_n65_), .O(new_n126_));
  oai21  g064(.a(new_n123_), .b(x04), .c(new_n126_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  nand2  g066(.a(new_n76_), .b(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n88_), .c(new_n128_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  inv1   g070(.a(new_n88_), .O(new_n133_));
  nor2   g071(.a(new_n67_), .b(x04), .O(new_n134_));
  inv1   g072(.a(x09), .O(new_n135_));
  inv1   g073(.a(x30), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  aoi21  g075(.a(new_n134_), .b(new_n133_), .c(new_n137_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n132_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n97_), .O(z11));
  oai21  g078(.a(new_n80_), .b(new_n78_), .c(new_n97_), .O(z12));
  inv1   g079(.a(x13), .O(new_n142_));
  oai21  g080(.a(new_n74_), .b(new_n73_), .c(x28), .O(new_n143_));
  nand4  g081(.a(new_n143_), .b(new_n76_), .c(new_n142_), .d(new_n72_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n70_), .c(x20), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  nand3  g086(.a(new_n76_), .b(new_n142_), .c(new_n72_), .O(new_n149_));
  nor2   g087(.a(new_n149_), .b(new_n85_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n148_), .c(new_n65_), .O(z13));
  aoi21  g090(.a(new_n146_), .b(new_n144_), .c(new_n67_), .O(new_n153_));
  oai21  g091(.a(new_n150_), .b(new_n153_), .c(new_n97_), .O(z14));
  nand2  g092(.a(x26), .b(x12), .O(new_n155_));
  nor3   g093(.a(new_n155_), .b(new_n116_), .c(new_n65_), .O(z15));
  inv1   g094(.a(x01), .O(new_n157_));
  aoi21  g095(.a(new_n64_), .b(new_n157_), .c(new_n63_), .O(z16));
  nand3  g096(.a(new_n138_), .b(new_n132_), .c(new_n97_), .O(z18));
  inv1   g097(.a(z18), .O(z19));
  nand2  g098(.a(x35), .b(x27), .O(new_n162_));
  nand4  g099(.a(new_n162_), .b(new_n97_), .c(new_n136_), .d(new_n135_), .O(new_n163_));
  nor2   g100(.a(new_n163_), .b(new_n130_), .O(z20));
  zero   g101(.O(z17));
endmodule


