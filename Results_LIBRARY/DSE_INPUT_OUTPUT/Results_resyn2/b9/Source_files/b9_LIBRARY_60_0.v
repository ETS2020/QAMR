// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n168_, new_n170_, new_n171_, new_n174_;
  nor2   g000(.a(x40), .b(x36), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  oai21  g009(.a(x36), .b(x35), .c(x27), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g011(.a(x39), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(x04), .O(new_n75_));
  nand4  g013(.a(new_n75_), .b(new_n73_), .c(x40), .d(new_n64_), .O(new_n76_));
  oai21  g014(.a(new_n70_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(new_n63_), .O(new_n78_));
  inv1   g016(.a(x28), .O(new_n79_));
  aoi21  g017(.a(x35), .b(new_n79_), .c(x36), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(new_n66_), .c(new_n81_), .O(new_n82_));
  nand3  g020(.a(new_n82_), .b(new_n78_), .c(x04), .O(z01));
  inv1   g021(.a(x40), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x36), .O(new_n85_));
  inv1   g023(.a(x35), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(x28), .O(new_n87_));
  oai21  g025(.a(x27), .b(x08), .c(new_n87_), .O(new_n88_));
  and2   g026(.a(x29), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(x02), .c(x39), .O(new_n90_));
  aoi21  g028(.a(new_n88_), .b(x04), .c(new_n90_), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n84_), .c(new_n85_), .O(z02));
  nand2  g030(.a(x28), .b(x27), .O(new_n93_));
  oai22  g031(.a(new_n93_), .b(new_n86_), .c(new_n67_), .d(x27), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(new_n63_), .O(z03));
  nor2   g033(.a(new_n63_), .b(x21), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n94_), .O(z04));
  aoi21  g035(.a(new_n93_), .b(new_n67_), .c(new_n63_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n78_), .O(z07));
  nand2  g048(.a(x40), .b(x39), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n78_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n78_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  nor2   g053(.a(new_n74_), .b(x05), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(new_n80_), .c(x40), .O(new_n117_));
  inv1   g055(.a(x07), .O(new_n118_));
  aoi21  g056(.a(new_n111_), .b(new_n118_), .c(new_n66_), .O(new_n119_));
  inv1   g057(.a(x05), .O(new_n120_));
  nor3   g058(.a(new_n111_), .b(new_n81_), .c(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n119_), .b(new_n117_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  inv1   g061(.a(new_n123_), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n63_), .O(new_n125_));
  oai21  g063(.a(new_n122_), .b(x04), .c(new_n125_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  nand2  g065(.a(x35), .b(new_n79_), .O(new_n128_));
  nand3  g066(.a(x40), .b(x39), .c(x29), .O(new_n129_));
  aoi21  g067(.a(new_n129_), .b(new_n128_), .c(new_n127_), .O(new_n130_));
  nand2  g068(.a(new_n129_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nor2   g070(.a(new_n66_), .b(x04), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n87_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n132_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n78_), .O(z11));
  inv1   g075(.a(x36), .O(new_n138_));
  nand3  g076(.a(new_n67_), .b(new_n138_), .c(new_n86_), .O(new_n139_));
  aoi21  g077(.a(new_n139_), .b(x27), .c(x10), .O(new_n140_));
  nor2   g078(.a(new_n75_), .b(new_n69_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n140_), .c(x40), .O(new_n142_));
  oai21  g080(.a(new_n85_), .b(new_n69_), .c(new_n142_), .O(z12));
  inv1   g081(.a(new_n111_), .O(new_n144_));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x28), .O(new_n146_));
  nor2   g084(.a(x13), .b(x04), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n146_), .c(new_n144_), .O(new_n148_));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x20), .O(new_n150_));
  nand2  g088(.a(new_n68_), .b(new_n67_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n78_), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n150_), .c(new_n148_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  inv1   g092(.a(new_n81_), .O(new_n155_));
  nand3  g093(.a(new_n147_), .b(new_n144_), .c(new_n155_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n154_), .O(z13));
  nand2  g095(.a(new_n147_), .b(new_n144_), .O(new_n158_));
  nand3  g096(.a(new_n145_), .b(new_n81_), .c(x28), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n158_), .c(new_n150_), .O(new_n161_));
  inv1   g099(.a(new_n69_), .O(new_n162_));
  aoi21  g100(.a(new_n79_), .b(x27), .c(new_n155_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n158_), .c(new_n162_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n161_), .c(new_n63_), .O(z14));
  inv1   g103(.a(x12), .O(new_n166_));
  nor2   g104(.a(new_n114_), .b(new_n166_), .O(z15));
  nand2  g105(.a(x22), .b(x01), .O(new_n168_));
  oai21  g106(.a(new_n168_), .b(x23), .c(new_n78_), .O(z16));
  inv1   g107(.a(x24), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(x23), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n168_), .c(new_n78_), .O(z17));
  nand4  g110(.a(new_n135_), .b(new_n134_), .c(new_n132_), .d(new_n78_), .O(z18));
  oai21  g111(.a(new_n86_), .b(new_n66_), .c(new_n135_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n130_), .c(new_n78_), .O(z20));
  nand2  g113(.a(new_n136_), .b(new_n78_), .O(z19));
endmodule


