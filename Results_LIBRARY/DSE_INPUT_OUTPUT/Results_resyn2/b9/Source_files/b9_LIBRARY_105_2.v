// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n159_, new_n161_, new_n163_,
    new_n164_, new_n165_, new_n167_;
  inv1   g000(.a(x33), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x27), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand2  g003(.a(x40), .b(x39), .O(new_n66_));
  inv1   g004(.a(new_n66_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  inv1   g006(.a(x15), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(x10), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n68_), .c(x16), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n64_), .O(new_n72_));
  inv1   g010(.a(x37), .O(new_n73_));
  nand2  g011(.a(x35), .b(x28), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g013(.a(x36), .b(x35), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n68_), .O(new_n77_));
  inv1   g015(.a(x27), .O(new_n78_));
  nor2   g016(.a(new_n63_), .b(new_n78_), .O(new_n79_));
  inv1   g017(.a(new_n79_), .O(new_n80_));
  nor2   g018(.a(new_n80_), .b(x15), .O(new_n81_));
  oai21  g019(.a(new_n77_), .b(new_n75_), .c(new_n81_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n72_), .O(z00));
  inv1   g021(.a(x28), .O(new_n84_));
  aoi21  g022(.a(x35), .b(new_n84_), .c(x36), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  oai21  g024(.a(new_n85_), .b(new_n78_), .c(new_n86_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n64_), .c(x04), .O(z01));
  nand2  g026(.a(x35), .b(new_n84_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  inv1   g029(.a(new_n64_), .O(new_n92_));
  nor2   g030(.a(new_n66_), .b(new_n92_), .O(z08));
  inv1   g031(.a(x02), .O(new_n94_));
  nand2  g032(.a(x29), .b(x08), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(z08), .c(new_n91_), .O(z02));
  nand2  g035(.a(new_n74_), .b(x27), .O(new_n98_));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi21  g037(.a(new_n73_), .b(new_n78_), .c(new_n99_), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n98_), .c(new_n92_), .O(z03));
  oai21  g039(.a(new_n74_), .b(new_n63_), .c(x27), .O(new_n102_));
  aoi21  g040(.a(new_n73_), .b(new_n78_), .c(x21), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n102_), .O(z04));
  aoi21  g042(.a(x28), .b(x27), .c(x37), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(new_n92_), .O(z05));
  nand2  g044(.a(x33), .b(new_n84_), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(x27), .c(x37), .O(z06));
  inv1   g046(.a(x14), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n69_), .O(new_n110_));
  nor2   g048(.a(x33), .b(x31), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x00), .O(new_n114_));
  nand2  g052(.a(x25), .b(new_n109_), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n114_), .c(x38), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n64_), .c(x03), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n112_), .O(z07));
  nand2  g056(.a(new_n79_), .b(x34), .O(new_n119_));
  nand2  g057(.a(x26), .b(x11), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  nand3  g059(.a(new_n67_), .b(new_n64_), .c(x05), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  nand4  g061(.a(new_n66_), .b(x33), .c(x27), .d(x07), .O(new_n124_));
  nor2   g062(.a(new_n124_), .b(new_n85_), .O(new_n125_));
  aoi21  g063(.a(new_n123_), .b(new_n87_), .c(new_n125_), .O(new_n126_));
  nand3  g064(.a(new_n79_), .b(x37), .c(x06), .O(new_n127_));
  oai21  g065(.a(new_n126_), .b(x04), .c(new_n127_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n89_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  oai21  g071(.a(new_n89_), .b(x04), .c(x33), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x27), .O(new_n135_));
  nor2   g073(.a(x30), .b(x09), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n135_), .c(new_n133_), .O(z18));
  inv1   g075(.a(z18), .O(z11));
  nor2   g076(.a(x35), .b(new_n63_), .O(new_n139_));
  nor2   g077(.a(x37), .b(x36), .O(new_n140_));
  aoi21  g078(.a(new_n140_), .b(new_n139_), .c(new_n78_), .O(new_n141_));
  oai21  g079(.a(new_n75_), .b(new_n63_), .c(x27), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n68_), .O(new_n143_));
  oai21  g081(.a(new_n141_), .b(x10), .c(new_n143_), .O(z12));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n75_), .c(x20), .O(new_n146_));
  inv1   g084(.a(new_n86_), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x27), .c(new_n147_), .O(new_n150_));
  inv1   g088(.a(x13), .O(new_n151_));
  nand3  g089(.a(new_n67_), .b(new_n151_), .c(new_n65_), .O(new_n152_));
  inv1   g090(.a(new_n152_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n64_), .O(new_n154_));
  oai22  g092(.a(new_n154_), .b(new_n150_), .c(new_n146_), .d(new_n80_), .O(z13));
  nand4  g093(.a(new_n149_), .b(new_n67_), .c(new_n151_), .d(new_n65_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n146_), .c(x33), .O(new_n157_));
  aoi22  g095(.a(new_n157_), .b(x27), .c(new_n153_), .d(new_n147_), .O(z14));
  nand2  g096(.a(x26), .b(x12), .O(new_n159_));
  nor2   g097(.a(new_n159_), .b(new_n119_), .O(z15));
  nand2  g098(.a(x22), .b(x01), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(x23), .c(new_n64_), .O(z16));
  inv1   g100(.a(x24), .O(new_n163_));
  inv1   g101(.a(new_n161_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n64_), .c(new_n163_), .d(x23), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z17));
  oai21  g104(.a(new_n139_), .b(new_n78_), .c(new_n136_), .O(new_n167_));
  nor2   g105(.a(new_n167_), .b(new_n131_), .O(z20));
  inv1   g106(.a(z18), .O(z19));
endmodule


