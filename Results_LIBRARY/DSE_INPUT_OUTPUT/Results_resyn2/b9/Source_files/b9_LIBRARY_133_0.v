// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n163_, new_n165_, new_n166_, new_n167_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n174_, new_n175_;
  inv1   g000(.a(x39), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x16), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  aoi21  g007(.a(x35), .b(x28), .c(x37), .O(new_n70_));
  nor2   g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n67_), .c(new_n68_), .O(new_n72_));
  inv1   g010(.a(x35), .O(new_n73_));
  inv1   g011(.a(x36), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g013(.a(new_n75_), .b(x27), .c(x10), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x04), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n67_), .O(new_n79_));
  oai22  g017(.a(new_n79_), .b(new_n76_), .c(new_n72_), .d(new_n66_), .O(z00));
  nor2   g018(.a(new_n73_), .b(x28), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(x36), .c(x27), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n66_), .O(z01));
  inv1   g023(.a(x04), .O(new_n86_));
  inv1   g024(.a(x08), .O(new_n87_));
  inv1   g025(.a(x28), .O(new_n88_));
  nand2  g026(.a(x35), .b(new_n88_), .O(new_n89_));
  aoi21  g027(.a(new_n69_), .b(new_n87_), .c(new_n89_), .O(new_n90_));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n77_), .O(new_n92_));
  oai21  g030(.a(new_n90_), .b(new_n86_), .c(new_n92_), .O(z02));
  nand2  g031(.a(x35), .b(x28), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n69_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n95_), .c(new_n65_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n96_), .b(new_n69_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n66_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n66_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n65_), .O(z07));
  nand2  g052(.a(new_n77_), .b(new_n65_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n65_), .O(z09));
  aoi21  g055(.a(new_n89_), .b(new_n74_), .c(new_n69_), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  nor2   g057(.a(new_n119_), .b(new_n83_), .O(new_n120_));
  nand2  g058(.a(new_n77_), .b(x07), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n118_), .c(new_n120_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  and2   g062(.a(new_n124_), .b(new_n65_), .O(new_n125_));
  oai21  g063(.a(new_n123_), .b(x04), .c(new_n125_), .O(z10));
  inv1   g064(.a(new_n77_), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(x29), .O(new_n128_));
  oai21  g066(.a(new_n89_), .b(x27), .c(new_n128_), .O(new_n129_));
  nand3  g067(.a(new_n81_), .b(x27), .c(new_n86_), .O(new_n130_));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n130_), .c(new_n65_), .O(new_n132_));
  aoi21  g070(.a(new_n129_), .b(x08), .c(new_n132_), .O(z11));
  nand2  g071(.a(new_n78_), .b(new_n75_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n70_), .c(new_n69_), .O(new_n135_));
  nand2  g073(.a(new_n78_), .b(x10), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n135_), .c(new_n65_), .O(z12));
  inv1   g076(.a(x13), .O(new_n139_));
  nand2  g077(.a(x36), .b(x35), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(x28), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n78_), .c(new_n139_), .O(new_n142_));
  nand2  g080(.a(new_n94_), .b(new_n96_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand4  g082(.a(new_n144_), .b(new_n143_), .c(new_n65_), .d(x20), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  inv1   g085(.a(x30), .O(new_n148_));
  inv1   g086(.a(x32), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n78_), .c(new_n139_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n147_), .O(z13));
  nand2  g090(.a(new_n78_), .b(new_n139_), .O(new_n153_));
  nand2  g091(.a(new_n144_), .b(x20), .O(new_n154_));
  nand3  g092(.a(new_n140_), .b(new_n83_), .c(x28), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n153_), .c(new_n154_), .O(new_n157_));
  aoi21  g095(.a(new_n88_), .b(x27), .c(new_n150_), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n153_), .c(new_n70_), .d(new_n69_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n157_), .c(new_n65_), .O(z14));
  nand4  g098(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n65_), .O(z15));
  nand2  g100(.a(x22), .b(x01), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n66_), .c(x23), .O(z16));
  inv1   g102(.a(x23), .O(new_n165_));
  nor2   g103(.a(x24), .b(new_n165_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n65_), .c(x22), .d(x01), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z17));
  inv1   g106(.a(new_n131_), .O(new_n169_));
  nand2  g107(.a(x27), .b(x04), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n90_), .c(new_n169_), .O(new_n171_));
  nand3  g109(.a(new_n127_), .b(x29), .c(x08), .O(new_n172_));
  oai21  g110(.a(new_n171_), .b(new_n66_), .c(new_n172_), .O(z18));
  aoi21  g111(.a(new_n128_), .b(new_n89_), .c(new_n87_), .O(new_n174_));
  oai21  g112(.a(new_n73_), .b(new_n69_), .c(new_n131_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n174_), .c(new_n65_), .O(z20));
  aoi21  g114(.a(new_n129_), .b(x08), .c(new_n132_), .O(z19));
endmodule


