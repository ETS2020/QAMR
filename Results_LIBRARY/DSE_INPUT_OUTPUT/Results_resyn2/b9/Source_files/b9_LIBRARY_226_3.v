// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n181_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x37), .O(new_n64_));
  nand2  g002(.a(x35), .b(x28), .O(new_n65_));
  nand2  g003(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x10), .O(new_n69_));
  nand2  g007(.a(x35), .b(x27), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g009(.a(new_n71_), .b(x40), .c(x39), .d(new_n68_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n67_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n63_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x36), .O(new_n75_));
  and2   g013(.a(new_n75_), .b(x16), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n74_), .O(z00));
  inv1   g015(.a(x27), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(new_n79_), .c(new_n78_), .O(new_n82_));
  inv1   g020(.a(x30), .O(new_n83_));
  inv1   g021(.a(x32), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g023(.a(new_n75_), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n68_), .O(new_n87_));
  oai21  g025(.a(new_n85_), .b(new_n82_), .c(new_n87_), .O(z01));
  inv1   g026(.a(x08), .O(new_n89_));
  nand2  g027(.a(new_n78_), .b(new_n89_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(x35), .c(new_n80_), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x39), .O(new_n95_));
  aoi21  g033(.a(new_n91_), .b(x04), .c(new_n95_), .O(new_n96_));
  oai21  g034(.a(new_n96_), .b(x36), .c(x40), .O(z02));
  nand2  g035(.a(new_n65_), .b(x27), .O(new_n98_));
  nand2  g036(.a(new_n64_), .b(new_n78_), .O(new_n99_));
  nand4  g037(.a(new_n99_), .b(new_n98_), .c(new_n75_), .d(x21), .O(z03));
  aoi21  g038(.a(new_n64_), .b(new_n78_), .c(x21), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n98_), .c(new_n86_), .O(z04));
  aoi21  g040(.a(x28), .b(x27), .c(x37), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n75_), .O(z05));
  inv1   g042(.a(new_n103_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n75_), .O(z06));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n63_), .O(new_n108_));
  nor2   g046(.a(x33), .b(x31), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  nand2  g050(.a(x25), .b(new_n107_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n75_), .c(x03), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n110_), .O(z07));
  nand3  g054(.a(x40), .b(x39), .c(new_n79_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z08));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n75_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  nor2   g059(.a(x32), .b(x30), .O(new_n122_));
  nand3  g060(.a(x35), .b(new_n80_), .c(x27), .O(new_n123_));
  nand3  g061(.a(x40), .b(x39), .c(x05), .O(new_n124_));
  aoi21  g062(.a(new_n123_), .b(new_n122_), .c(new_n124_), .O(new_n125_));
  nand2  g063(.a(x40), .b(x39), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x07), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n125_), .c(new_n68_), .O(new_n129_));
  inv1   g067(.a(x07), .O(new_n130_));
  inv1   g068(.a(x40), .O(new_n131_));
  nand2  g069(.a(x27), .b(new_n68_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n130_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x36), .O(new_n134_));
  nand3  g072(.a(x37), .b(x27), .c(x06), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n129_), .O(z10));
  nand3  g074(.a(x40), .b(x39), .c(x29), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n81_), .c(new_n89_), .O(new_n138_));
  nand2  g076(.a(new_n137_), .b(x27), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  inv1   g078(.a(new_n81_), .O(new_n141_));
  inv1   g079(.a(new_n132_), .O(new_n142_));
  inv1   g080(.a(x09), .O(new_n143_));
  nand3  g081(.a(new_n75_), .b(new_n83_), .c(new_n143_), .O(new_n144_));
  aoi21  g082(.a(new_n142_), .b(new_n141_), .c(new_n144_), .O(new_n145_));
  and2   g083(.a(new_n145_), .b(new_n140_), .O(z11));
  nand3  g084(.a(new_n71_), .b(x39), .c(new_n68_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n79_), .O(new_n148_));
  aoi22  g086(.a(new_n148_), .b(x40), .c(new_n66_), .d(x27), .O(z12));
  inv1   g087(.a(x18), .O(new_n150_));
  inv1   g088(.a(x19), .O(new_n151_));
  nand4  g089(.a(new_n66_), .b(x20), .c(new_n151_), .d(new_n150_), .O(new_n152_));
  inv1   g090(.a(new_n126_), .O(new_n153_));
  nor2   g091(.a(x13), .b(x04), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n153_), .c(new_n80_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nand3  g095(.a(new_n154_), .b(new_n85_), .c(x39), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n79_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x40), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n157_), .O(z13));
  nand3  g099(.a(x20), .b(new_n151_), .c(new_n150_), .O(new_n162_));
  nand2  g100(.a(new_n154_), .b(new_n153_), .O(new_n163_));
  nor2   g101(.a(x37), .b(x35), .O(new_n164_));
  nand2  g102(.a(new_n122_), .b(x28), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(new_n166_));
  oai22  g104(.a(new_n166_), .b(new_n163_), .c(new_n164_), .d(new_n162_), .O(new_n167_));
  oai21  g105(.a(x37), .b(x28), .c(x27), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n163_), .O(new_n169_));
  aoi21  g107(.a(new_n122_), .b(new_n78_), .c(new_n86_), .O(new_n170_));
  nand3  g108(.a(new_n170_), .b(new_n169_), .c(new_n167_), .O(z14));
  inv1   g109(.a(x12), .O(new_n172_));
  nor2   g110(.a(new_n120_), .b(new_n172_), .O(z15));
  inv1   g111(.a(x23), .O(new_n174_));
  nand4  g112(.a(new_n75_), .b(new_n174_), .c(x22), .d(x01), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z16));
  inv1   g114(.a(x24), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(x23), .c(x22), .d(x01), .O(new_n178_));
  nand2  g116(.a(new_n178_), .b(new_n75_), .O(z17));
  nand2  g117(.a(new_n145_), .b(new_n140_), .O(z18));
  nand3  g118(.a(new_n70_), .b(new_n83_), .c(new_n143_), .O(new_n181_));
  oai21  g119(.a(new_n181_), .b(new_n138_), .c(new_n75_), .O(z20));
  and2   g120(.a(new_n145_), .b(new_n140_), .O(z19));
endmodule


