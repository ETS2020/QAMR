// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n98_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n160_,
    new_n161_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n172_, new_n173_;
  inv1   g000(.a(x35), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x27), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  nor2   g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  aoi21  g005(.a(x36), .b(x27), .c(x10), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  nand3  g007(.a(x40), .b(x39), .c(new_n69_), .O(new_n70_));
  nor2   g008(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  oai21  g009(.a(new_n71_), .b(new_n67_), .c(new_n64_), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(new_n63_), .c(x16), .O(z00));
  inv1   g011(.a(x36), .O(new_n74_));
  inv1   g012(.a(x28), .O(new_n75_));
  nand2  g013(.a(x35), .b(new_n75_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n74_), .c(new_n65_), .O(new_n77_));
  nor2   g015(.a(x32), .b(x30), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  inv1   g017(.a(x16), .O(new_n80_));
  nor2   g018(.a(new_n63_), .b(new_n80_), .O(new_n81_));
  nor2   g019(.a(new_n81_), .b(new_n69_), .O(new_n82_));
  oai21  g020(.a(new_n79_), .b(new_n77_), .c(new_n82_), .O(z01));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n76_), .c(x04), .O(new_n85_));
  nand2  g023(.a(x40), .b(x39), .O(new_n86_));
  nor2   g024(.a(new_n81_), .b(new_n86_), .O(z08));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand3  g028(.a(new_n90_), .b(z08), .c(new_n85_), .O(z02));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n63_), .c(new_n66_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n81_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  inv1   g033(.a(new_n81_), .O(new_n96_));
  nand3  g034(.a(new_n93_), .b(new_n96_), .c(new_n95_), .O(z04));
  nand2  g035(.a(new_n92_), .b(new_n66_), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n96_), .O(z06));
  inv1   g037(.a(z06), .O(z05));
  inv1   g038(.a(x25), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x25), .b(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(x38), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x15), .O(new_n107_));
  or2    g045(.a(x33), .b(x31), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(new_n107_), .c(x14), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n105_), .c(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n96_), .O(z07));
  inv1   g049(.a(x11), .O(new_n112_));
  nand4  g050(.a(new_n96_), .b(x34), .c(x27), .d(x26), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n112_), .O(z09));
  nand2  g052(.a(new_n86_), .b(x07), .O(new_n115_));
  nand3  g053(.a(x40), .b(x39), .c(x05), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nor2   g055(.a(new_n116_), .b(new_n78_), .O(new_n118_));
  aoi21  g056(.a(new_n117_), .b(new_n77_), .c(new_n118_), .O(new_n119_));
  aoi21  g057(.a(new_n67_), .b(x06), .c(new_n81_), .O(new_n120_));
  oai21  g058(.a(new_n119_), .b(x04), .c(new_n120_), .O(z10));
  inv1   g059(.a(x08), .O(new_n122_));
  nand3  g060(.a(x40), .b(x39), .c(x29), .O(new_n123_));
  nand3  g061(.a(x35), .b(new_n75_), .c(new_n65_), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n122_), .O(new_n125_));
  nor2   g063(.a(x30), .b(x09), .O(new_n126_));
  nand2  g064(.a(x27), .b(new_n69_), .O(new_n127_));
  oai21  g065(.a(new_n127_), .b(new_n76_), .c(new_n126_), .O(new_n128_));
  oai21  g066(.a(new_n128_), .b(new_n125_), .c(new_n96_), .O(z11));
  oai21  g067(.a(new_n63_), .b(new_n75_), .c(new_n66_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(x27), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n70_), .O(new_n132_));
  inv1   g070(.a(x10), .O(new_n133_));
  nor3   g071(.a(x37), .b(x36), .c(x35), .O(new_n134_));
  oai21  g072(.a(new_n134_), .b(new_n65_), .c(new_n133_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n132_), .c(new_n96_), .O(z12));
  nand2  g074(.a(x36), .b(x35), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x28), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(x27), .c(new_n79_), .O(new_n139_));
  inv1   g077(.a(x13), .O(new_n140_));
  inv1   g078(.a(new_n70_), .O(new_n141_));
  nand3  g079(.a(new_n96_), .b(new_n141_), .c(new_n140_), .O(new_n142_));
  inv1   g080(.a(x18), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand3  g082(.a(x20), .b(new_n144_), .c(new_n143_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n96_), .O(new_n147_));
  oai22  g085(.a(new_n147_), .b(new_n131_), .c(new_n142_), .d(new_n139_), .O(z13));
  nand2  g086(.a(new_n141_), .b(new_n140_), .O(new_n149_));
  nand3  g087(.a(new_n137_), .b(new_n78_), .c(x28), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n149_), .c(new_n145_), .O(new_n152_));
  nand3  g090(.a(new_n66_), .b(new_n63_), .c(x28), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x27), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n78_), .O(new_n155_));
  aoi21  g093(.a(new_n149_), .b(new_n131_), .c(new_n81_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n155_), .c(new_n152_), .O(z14));
  inv1   g095(.a(x12), .O(new_n158_));
  nor2   g096(.a(new_n113_), .b(new_n158_), .O(z15));
  inv1   g097(.a(x23), .O(new_n160_));
  nand4  g098(.a(new_n96_), .b(new_n160_), .c(x22), .d(x01), .O(new_n161_));
  inv1   g099(.a(new_n161_), .O(z16));
  nand2  g100(.a(x22), .b(x01), .O(new_n163_));
  nor4   g101(.a(new_n163_), .b(new_n81_), .c(x24), .d(new_n160_), .O(z17));
  nand3  g102(.a(new_n75_), .b(x27), .c(new_n69_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n80_), .c(new_n63_), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(new_n167_));
  inv1   g105(.a(new_n126_), .O(new_n168_));
  nor2   g106(.a(new_n168_), .b(new_n125_), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n167_), .O(z18));
  nor3   g108(.a(new_n166_), .b(new_n168_), .c(new_n125_), .O(z19));
  aoi21  g109(.a(new_n123_), .b(new_n76_), .c(new_n122_), .O(new_n172_));
  oai21  g110(.a(new_n63_), .b(new_n65_), .c(new_n126_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n172_), .c(new_n96_), .O(z20));
endmodule


