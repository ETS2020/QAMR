// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n177_, new_n178_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  nor2   g008(.a(x40), .b(x22), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(x35), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(x28), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(x36), .c(x27), .O(new_n76_));
  nor2   g014(.a(x32), .b(x30), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(x04), .c(new_n71_), .O(z01));
  inv1   g017(.a(x22), .O(new_n80_));
  inv1   g018(.a(x08), .O(new_n81_));
  inv1   g019(.a(x27), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n75_), .c(new_n63_), .O(new_n84_));
  and2   g022(.a(x29), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(x02), .c(x39), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n84_), .c(x40), .O(new_n87_));
  oai21  g025(.a(x40), .b(new_n80_), .c(new_n87_), .O(z02));
  inv1   g026(.a(x37), .O(new_n89_));
  nand2  g027(.a(x28), .b(x27), .O(new_n90_));
  oai22  g028(.a(new_n90_), .b(new_n74_), .c(new_n89_), .d(x27), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x21), .c(new_n71_), .O(z03));
  inv1   g030(.a(x21), .O(new_n93_));
  aoi21  g031(.a(new_n91_), .b(new_n93_), .c(new_n71_), .O(z04));
  aoi21  g032(.a(new_n90_), .b(new_n89_), .c(new_n71_), .O(z05));
  inv1   g033(.a(z05), .O(z06));
  inv1   g034(.a(x14), .O(new_n97_));
  inv1   g035(.a(x15), .O(new_n98_));
  nand2  g036(.a(x17), .b(new_n98_), .O(new_n99_));
  nor2   g037(.a(x33), .b(x31), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n99_), .c(new_n97_), .O(new_n101_));
  inv1   g039(.a(x40), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n80_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n97_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n105_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n103_), .c(x03), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(new_n101_), .O(z07));
  nand2  g047(.a(x40), .b(x39), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n103_), .O(z08));
  nand4  g049(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n103_), .O(z09));
  inv1   g051(.a(new_n110_), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(x05), .O(new_n115_));
  nand3  g053(.a(new_n110_), .b(new_n103_), .c(x07), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  inv1   g055(.a(x36), .O(new_n118_));
  inv1   g056(.a(x28), .O(new_n119_));
  nand2  g057(.a(x35), .b(new_n119_), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n118_), .c(x04), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n71_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n117_), .c(new_n123_), .O(new_n124_));
  inv1   g062(.a(new_n77_), .O(new_n125_));
  nand4  g063(.a(new_n114_), .b(new_n125_), .c(x05), .d(new_n63_), .O(new_n126_));
  oai21  g064(.a(new_n124_), .b(new_n82_), .c(new_n126_), .O(z10));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  aoi21  g067(.a(new_n128_), .b(new_n120_), .c(new_n81_), .O(new_n130_));
  oai21  g068(.a(new_n129_), .b(new_n82_), .c(new_n130_), .O(new_n131_));
  nor2   g069(.a(x30), .b(x09), .O(new_n132_));
  nand3  g070(.a(new_n75_), .b(x27), .c(new_n63_), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(new_n131_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n103_), .O(z11));
  nand2  g073(.a(new_n66_), .b(new_n89_), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(x27), .c(x10), .O(new_n137_));
  inv1   g075(.a(new_n67_), .O(new_n138_));
  aoi22  g076(.a(new_n138_), .b(x27), .c(x39), .d(new_n63_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n137_), .c(x40), .O(new_n140_));
  nand2  g078(.a(new_n138_), .b(x27), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n102_), .c(x22), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n140_), .O(z12));
  inv1   g081(.a(x13), .O(new_n144_));
  nand2  g082(.a(x36), .b(x35), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x28), .O(new_n146_));
  nand4  g084(.a(new_n146_), .b(new_n114_), .c(new_n144_), .d(new_n63_), .O(new_n147_));
  inv1   g085(.a(x18), .O(new_n148_));
  inv1   g086(.a(x19), .O(new_n149_));
  nand3  g087(.a(x20), .b(new_n149_), .c(new_n148_), .O(new_n150_));
  nand2  g088(.a(new_n103_), .b(new_n138_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n147_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand3  g091(.a(new_n114_), .b(new_n144_), .c(new_n63_), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n125_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n153_), .O(z13));
  nand3  g095(.a(new_n145_), .b(new_n77_), .c(x28), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n154_), .c(new_n150_), .O(new_n160_));
  aoi21  g098(.a(new_n119_), .b(x27), .c(new_n125_), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n154_), .c(new_n141_), .O(new_n162_));
  nand3  g100(.a(new_n162_), .b(new_n160_), .c(new_n103_), .O(z14));
  nand4  g101(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n103_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(x23), .c(new_n103_), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(x23), .c(x22), .d(x01), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z17));
  nand2  g108(.a(x27), .b(x04), .O(new_n171_));
  nand3  g109(.a(new_n171_), .b(new_n83_), .c(new_n75_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n132_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n103_), .O(new_n174_));
  oai21  g112(.a(new_n128_), .b(new_n81_), .c(new_n174_), .O(z18));
  aoi22  g113(.a(new_n173_), .b(new_n103_), .c(new_n129_), .d(x08), .O(z19));
  nand2  g114(.a(x35), .b(x27), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n132_), .c(new_n103_), .O(new_n178_));
  nor2   g116(.a(new_n178_), .b(new_n130_), .O(z20));
endmodule


