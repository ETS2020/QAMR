// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n167_, new_n168_;
  inv1   g000(.a(x36), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x34), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x15), .O(new_n66_));
  inv1   g004(.a(x16), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(x27), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  nand3  g012(.a(new_n74_), .b(x10), .c(new_n72_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n71_), .O(new_n76_));
  aoi21  g014(.a(new_n76_), .b(new_n66_), .c(new_n67_), .O(new_n77_));
  inv1   g015(.a(x34), .O(new_n78_));
  aoi21  g016(.a(x35), .b(new_n78_), .c(x36), .O(new_n79_));
  inv1   g017(.a(x27), .O(new_n80_));
  nor2   g018(.a(new_n80_), .b(x04), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n66_), .O(new_n84_));
  oai21  g022(.a(new_n77_), .b(new_n65_), .c(new_n84_), .O(z00));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  inv1   g024(.a(x28), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n87_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n63_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x27), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n86_), .O(new_n91_));
  aoi21  g029(.a(new_n91_), .b(x04), .c(new_n65_), .O(z01));
  nor2   g030(.a(x27), .b(x08), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n88_), .c(x04), .O(new_n94_));
  nor2   g032(.a(new_n73_), .b(new_n65_), .O(z08));
  inv1   g033(.a(x02), .O(new_n96_));
  nand2  g034(.a(x29), .b(x08), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(z08), .c(new_n94_), .O(z02));
  inv1   g037(.a(x35), .O(new_n100_));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  oai22  g039(.a(new_n101_), .b(new_n100_), .c(new_n68_), .d(x27), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g041(.a(x21), .O(new_n104_));
  aoi21  g042(.a(new_n102_), .b(new_n104_), .c(new_n65_), .O(z04));
  nand3  g043(.a(new_n101_), .b(new_n64_), .c(new_n68_), .O(z05));
  nand2  g044(.a(new_n101_), .b(new_n68_), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(new_n64_), .O(z06));
  inv1   g046(.a(x14), .O(new_n109_));
  nand2  g047(.a(x17), .b(new_n66_), .O(new_n110_));
  nor2   g048(.a(x33), .b(x31), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  inv1   g050(.a(x25), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(x00), .O(new_n114_));
  nand2  g052(.a(x25), .b(new_n109_), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(new_n114_), .c(x38), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n64_), .c(x03), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n112_), .O(z07));
  nand3  g056(.a(x36), .b(x34), .c(x27), .O(new_n119_));
  nand2  g057(.a(x26), .b(x11), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  oai21  g059(.a(x32), .b(x30), .c(new_n74_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n73_), .b(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n89_), .c(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  aoi21  g065(.a(new_n74_), .b(new_n127_), .c(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n64_), .O(z10));
  inv1   g069(.a(x08), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n88_), .c(new_n132_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nor2   g074(.a(x30), .b(x09), .O(new_n137_));
  nand3  g075(.a(new_n81_), .b(x35), .c(new_n87_), .O(new_n138_));
  and2   g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n64_), .O(z11));
  aoi21  g079(.a(new_n76_), .b(new_n64_), .c(new_n83_), .O(z12));
  nor2   g080(.a(x19), .b(x18), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n70_), .c(x20), .O(new_n144_));
  nor2   g082(.a(x13), .b(x04), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n74_), .c(new_n87_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n144_), .c(new_n80_), .O(new_n147_));
  nand2  g085(.a(new_n145_), .b(new_n74_), .O(new_n148_));
  nor2   g086(.a(new_n148_), .b(new_n86_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n147_), .c(new_n64_), .O(new_n150_));
  inv1   g088(.a(new_n148_), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(x36), .c(x35), .d(x27), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n150_), .O(z13));
  aoi21  g091(.a(x36), .b(x35), .c(new_n87_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n148_), .c(new_n144_), .O(new_n155_));
  oai21  g093(.a(new_n148_), .b(new_n86_), .c(new_n64_), .O(new_n156_));
  aoi21  g094(.a(new_n155_), .b(x27), .c(new_n156_), .O(z14));
  nand2  g095(.a(x26), .b(x12), .O(new_n158_));
  nor2   g096(.a(new_n158_), .b(new_n119_), .O(z15));
  nand2  g097(.a(x22), .b(x01), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(x23), .c(new_n64_), .O(z16));
  inv1   g099(.a(x24), .O(new_n162_));
  inv1   g100(.a(new_n160_), .O(new_n163_));
  nand4  g101(.a(new_n163_), .b(new_n64_), .c(new_n162_), .d(x23), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(z17));
  aoi21  g103(.a(new_n139_), .b(new_n136_), .c(new_n65_), .O(z18));
  nand2  g104(.a(x35), .b(x27), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n137_), .c(new_n64_), .O(new_n168_));
  nor2   g106(.a(new_n168_), .b(new_n134_), .O(z20));
  nand2  g107(.a(new_n140_), .b(new_n64_), .O(z19));
endmodule


