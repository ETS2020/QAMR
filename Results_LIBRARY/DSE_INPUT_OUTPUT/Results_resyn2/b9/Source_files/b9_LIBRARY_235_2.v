// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n171_, new_n173_, new_n174_, new_n175_, new_n176_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand2  g002(.a(x39), .b(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x40), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(x27), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x28), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(x04), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x40), .O(new_n79_));
  or2    g017(.a(x32), .b(x30), .O(new_n80_));
  aoi22  g018(.a(new_n80_), .b(x04), .c(new_n79_), .d(x27), .O(z01));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n86_), .c(new_n87_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n85_), .O(z02));
  inv1   g028(.a(new_n72_), .O(new_n91_));
  nand2  g029(.a(x35), .b(x28), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(x27), .O(new_n93_));
  inv1   g031(.a(x21), .O(new_n94_));
  inv1   g032(.a(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(new_n93_), .c(new_n91_), .O(z03));
  oai21  g036(.a(new_n92_), .b(new_n71_), .c(x27), .O(new_n99_));
  aoi21  g037(.a(new_n96_), .b(new_n95_), .c(x21), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n99_), .O(z04));
  oai21  g039(.a(new_n71_), .b(x28), .c(x27), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n96_), .O(z05));
  oai21  g041(.a(new_n82_), .b(new_n95_), .c(new_n96_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n72_), .O(z06));
  inv1   g043(.a(x14), .O(new_n106_));
  inv1   g044(.a(x15), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n107_), .O(new_n108_));
  nor2   g046(.a(x33), .b(x31), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n108_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  nand2  g050(.a(x25), .b(new_n106_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n72_), .c(x03), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n110_), .O(z07));
  nand2  g054(.a(new_n87_), .b(new_n72_), .O(z08));
  nand2  g055(.a(x34), .b(x26), .O(new_n118_));
  nor2   g056(.a(new_n71_), .b(new_n95_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x11), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n118_), .O(z09));
  inv1   g059(.a(x05), .O(new_n122_));
  nand2  g060(.a(x39), .b(new_n122_), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  inv1   g062(.a(x39), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n124_), .c(x04), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n123_), .c(new_n77_), .O(new_n127_));
  aoi21  g065(.a(x37), .b(x06), .c(new_n71_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x27), .O(new_n130_));
  inv1   g068(.a(new_n87_), .O(new_n131_));
  nand4  g069(.a(new_n131_), .b(new_n80_), .c(x05), .d(new_n64_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n130_), .O(z10));
  inv1   g071(.a(x29), .O(new_n134_));
  oai22  g072(.a(new_n87_), .b(new_n134_), .c(new_n83_), .d(x27), .O(new_n135_));
  nand4  g073(.a(new_n119_), .b(x35), .c(new_n82_), .d(new_n64_), .O(new_n136_));
  oai21  g074(.a(x30), .b(x09), .c(new_n72_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  aoi21  g076(.a(new_n135_), .b(x08), .c(new_n138_), .O(z11));
  inv1   g077(.a(new_n63_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(x39), .c(new_n64_), .O(new_n141_));
  aoi21  g079(.a(new_n66_), .b(new_n141_), .c(new_n95_), .O(new_n142_));
  nand3  g080(.a(x39), .b(x10), .c(new_n64_), .O(new_n143_));
  inv1   g081(.a(new_n143_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n142_), .c(x40), .O(z12));
  aoi21  g083(.a(x36), .b(x35), .c(new_n82_), .O(new_n146_));
  nor2   g084(.a(x13), .b(x04), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x39), .O(new_n148_));
  nor2   g086(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nor2   g087(.a(x19), .b(x18), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x20), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n66_), .c(x40), .O(new_n152_));
  oai21  g090(.a(new_n152_), .b(new_n149_), .c(x27), .O(new_n153_));
  nand3  g091(.a(new_n147_), .b(new_n131_), .c(new_n80_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  nand2  g093(.a(new_n96_), .b(new_n75_), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n150_), .c(x20), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n146_), .c(new_n95_), .O(new_n158_));
  nand2  g096(.a(new_n92_), .b(new_n96_), .O(new_n159_));
  nand4  g097(.a(new_n150_), .b(new_n159_), .c(x27), .d(x20), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n148_), .c(new_n71_), .O(new_n161_));
  oai21  g099(.a(new_n158_), .b(new_n80_), .c(new_n161_), .O(z14));
  nand3  g100(.a(x34), .b(x26), .c(x12), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(x40), .c(new_n95_), .O(z15));
  inv1   g102(.a(x23), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(x22), .c(x01), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n72_), .O(z16));
  nor2   g105(.a(x24), .b(new_n165_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n72_), .c(x22), .d(x01), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z17));
  nand2  g108(.a(new_n135_), .b(x08), .O(new_n171_));
  nand3  g109(.a(new_n137_), .b(new_n136_), .c(new_n171_), .O(z18));
  inv1   g110(.a(new_n119_), .O(new_n173_));
  nand3  g111(.a(new_n82_), .b(new_n95_), .c(x08), .O(new_n174_));
  aoi21  g112(.a(new_n174_), .b(new_n173_), .c(new_n75_), .O(new_n175_));
  oai21  g113(.a(new_n88_), .b(new_n87_), .c(new_n137_), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(new_n175_), .O(z20));
  aoi21  g115(.a(new_n135_), .b(x08), .c(new_n138_), .O(z19));
endmodule


