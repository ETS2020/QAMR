// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n160_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n174_, new_n175_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x36), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(x30), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x30), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi22  g016(.a(new_n78_), .b(new_n71_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  inv1   g017(.a(new_n72_), .O(new_n80_));
  nor2   g018(.a(new_n80_), .b(new_n65_), .O(new_n81_));
  oai21  g019(.a(new_n79_), .b(x32), .c(new_n81_), .O(z01));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n78_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(new_n84_), .c(new_n80_), .O(z02));
  nand2  g026(.a(x35), .b(x28), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x27), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n75_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n72_), .d(x21), .O(z03));
  aoi21  g031(.a(new_n91_), .b(new_n75_), .c(x21), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(new_n90_), .c(new_n80_), .O(z04));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n72_), .c(new_n91_), .O(z05));
  inv1   g035(.a(z05), .O(z06));
  inv1   g036(.a(x14), .O(new_n99_));
  inv1   g037(.a(x15), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n100_), .O(new_n101_));
  nor2   g039(.a(x33), .b(x31), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n101_), .c(new_n99_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n99_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n105_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n72_), .c(x03), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(new_n103_), .O(z07));
  nand2  g047(.a(new_n72_), .b(new_n85_), .O(z08));
  and2   g048(.a(x34), .b(x27), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(x26), .c(x11), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n72_), .O(z09));
  nor2   g051(.a(x30), .b(x28), .O(new_n114_));
  aoi21  g052(.a(new_n114_), .b(x35), .c(x36), .O(new_n115_));
  inv1   g053(.a(x07), .O(new_n116_));
  nand2  g054(.a(new_n85_), .b(new_n116_), .O(new_n117_));
  inv1   g055(.a(x05), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(new_n118_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n117_), .c(new_n65_), .O(new_n120_));
  nand3  g058(.a(new_n72_), .b(x37), .c(x06), .O(new_n121_));
  oai21  g059(.a(new_n120_), .b(new_n115_), .c(new_n121_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x27), .O(new_n123_));
  nor2   g061(.a(new_n85_), .b(x04), .O(new_n124_));
  inv1   g062(.a(x32), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n76_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n72_), .c(new_n124_), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x05), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n123_), .O(z10));
  inv1   g068(.a(x09), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n78_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n133_), .c(x08), .O(new_n135_));
  nand4  g073(.a(x35), .b(new_n77_), .c(x27), .d(new_n65_), .O(new_n136_));
  nand4  g074(.a(new_n136_), .b(new_n135_), .c(new_n76_), .d(new_n131_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  nand2  g076(.a(new_n67_), .b(x27), .O(new_n139_));
  nand3  g077(.a(new_n72_), .b(new_n68_), .c(new_n139_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z12));
  inv1   g079(.a(x13), .O(new_n142_));
  nand3  g080(.a(x36), .b(x35), .c(x28), .O(new_n143_));
  oai21  g081(.a(x30), .b(x28), .c(new_n143_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n124_), .c(new_n142_), .O(new_n145_));
  nand2  g083(.a(new_n89_), .b(new_n91_), .O(new_n146_));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(new_n72_), .c(new_n146_), .d(x20), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  nand2  g088(.a(new_n128_), .b(new_n142_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(z13));
  nor2   g090(.a(new_n127_), .b(x13), .O(new_n153_));
  nand2  g091(.a(new_n124_), .b(new_n142_), .O(new_n154_));
  aoi21  g092(.a(x36), .b(x35), .c(new_n114_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n154_), .c(new_n148_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(x27), .c(new_n153_), .O(z14));
  nand4  g095(.a(new_n111_), .b(new_n72_), .c(x26), .d(x12), .O(new_n158_));
  inv1   g096(.a(new_n158_), .O(z15));
  nand2  g097(.a(x22), .b(x01), .O(new_n160_));
  nor3   g098(.a(new_n160_), .b(new_n80_), .c(x23), .O(z16));
  inv1   g099(.a(x24), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(x23), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n160_), .c(new_n72_), .O(z17));
  inv1   g102(.a(x08), .O(new_n165_));
  aoi21  g103(.a(new_n132_), .b(new_n78_), .c(new_n165_), .O(new_n166_));
  nand2  g104(.a(new_n136_), .b(new_n131_), .O(new_n167_));
  aoi21  g105(.a(new_n134_), .b(new_n166_), .c(new_n167_), .O(new_n168_));
  nand2  g106(.a(x36), .b(x30), .O(new_n169_));
  oai21  g107(.a(new_n168_), .b(x30), .c(new_n169_), .O(z18));
  nand3  g108(.a(new_n136_), .b(new_n135_), .c(new_n131_), .O(new_n171_));
  inv1   g109(.a(new_n169_), .O(new_n172_));
  aoi21  g110(.a(new_n171_), .b(new_n76_), .c(new_n172_), .O(z19));
  nand2  g111(.a(x35), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n76_), .c(new_n131_), .O(new_n175_));
  nor2   g113(.a(new_n175_), .b(new_n166_), .O(z20));
endmodule


