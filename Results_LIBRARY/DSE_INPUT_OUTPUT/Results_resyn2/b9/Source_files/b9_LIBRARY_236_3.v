// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n85_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n118_, new_n119_, new_n122_, new_n124_,
    new_n125_, new_n127_, new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x15), .O(new_n63_));
  inv1   g01(.a(x27), .O(new_n64_));
  inv1   g02(.a(x37), .O(new_n65_));
  nand2  g03(.a(x35), .b(x28), .O(new_n66_));
  aoi21  g04(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  nand2  g05(.a(new_n67_), .b(new_n63_), .O(new_n68_));
  nand2  g06(.a(x40), .b(x39), .O(new_n69_));
  nand3  g07(.a(new_n69_), .b(new_n68_), .c(x16), .O(z00));
  inv1   g08(.a(x35), .O(new_n71_));
  inv1   g09(.a(x36), .O(new_n72_));
  oai21  g10(.a(new_n71_), .b(x28), .c(new_n72_), .O(new_n73_));
  nand2  g11(.a(new_n73_), .b(x27), .O(new_n74_));
  nor2   g12(.a(x32), .b(x30), .O(new_n75_));
  nand2  g13(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g14(.a(new_n76_), .b(new_n69_), .c(x04), .O(z01));
  nand2  g15(.a(new_n66_), .b(x27), .O(new_n79_));
  inv1   g16(.a(new_n69_), .O(new_n80_));
  aoi21  g17(.a(new_n65_), .b(new_n64_), .c(new_n80_), .O(new_n81_));
  nand3  g18(.a(new_n81_), .b(new_n79_), .c(x21), .O(z03));
  inv1   g19(.a(x21), .O(new_n83_));
  nand3  g20(.a(new_n81_), .b(new_n79_), .c(new_n83_), .O(z04));
  nand2  g21(.a(x28), .b(x27), .O(new_n85_));
  aoi21  g22(.a(new_n85_), .b(new_n65_), .c(new_n80_), .O(z05));
  inv1   g23(.a(z05), .O(z06));
  inv1   g24(.a(x25), .O(new_n88_));
  nand2  g25(.a(new_n88_), .b(x00), .O(new_n89_));
  inv1   g26(.a(x14), .O(new_n90_));
  nand2  g27(.a(x25), .b(new_n90_), .O(new_n91_));
  nand3  g28(.a(new_n91_), .b(new_n89_), .c(x38), .O(new_n92_));
  inv1   g29(.a(x17), .O(new_n93_));
  nor2   g30(.a(new_n93_), .b(x15), .O(new_n94_));
  inv1   g31(.a(x31), .O(new_n95_));
  inv1   g32(.a(x33), .O(new_n96_));
  nand2  g33(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g34(.a(new_n97_), .b(new_n94_), .c(x14), .O(new_n98_));
  and2   g35(.a(new_n69_), .b(x03), .O(new_n99_));
  nand3  g36(.a(new_n99_), .b(new_n98_), .c(new_n92_), .O(new_n100_));
  inv1   g37(.a(new_n100_), .O(z07));
  nand4  g38(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n103_));
  nand2  g39(.a(new_n103_), .b(new_n69_), .O(z09));
  inv1   g40(.a(x07), .O(new_n105_));
  nor2   g41(.a(new_n105_), .b(x04), .O(new_n106_));
  and2   g42(.a(x37), .b(x06), .O(new_n107_));
  aoi21  g43(.a(new_n106_), .b(new_n73_), .c(new_n107_), .O(new_n108_));
  oai21  g44(.a(new_n108_), .b(new_n64_), .c(new_n69_), .O(z10));
  nor2   g45(.a(x30), .b(x09), .O(new_n110_));
  nor2   g46(.a(x27), .b(x08), .O(new_n111_));
  inv1   g47(.a(x28), .O(new_n112_));
  nand2  g48(.a(x27), .b(x04), .O(new_n113_));
  nand3  g49(.a(new_n113_), .b(x35), .c(new_n112_), .O(new_n114_));
  oai21  g50(.a(new_n114_), .b(new_n111_), .c(new_n110_), .O(new_n115_));
  nand2  g51(.a(new_n115_), .b(new_n69_), .O(z11));
  nor2   g52(.a(new_n80_), .b(new_n67_), .O(z12));
  nor2   g53(.a(x19), .b(x18), .O(new_n118_));
  nand3  g54(.a(new_n118_), .b(new_n67_), .c(x20), .O(new_n119_));
  nor2   g55(.a(new_n119_), .b(new_n80_), .O(z13));
  and2   g56(.a(new_n119_), .b(new_n69_), .O(z14));
  nand4  g57(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n122_));
  nand2  g58(.a(new_n122_), .b(new_n69_), .O(z15));
  inv1   g59(.a(x23), .O(new_n124_));
  nand4  g60(.a(new_n69_), .b(new_n124_), .c(x22), .d(x01), .O(new_n125_));
  inv1   g61(.a(new_n125_), .O(z16));
  nand2  g62(.a(x22), .b(x01), .O(new_n127_));
  nor4   g63(.a(new_n127_), .b(new_n80_), .c(x24), .d(new_n124_), .O(z17));
  inv1   g64(.a(z11), .O(z18));
  inv1   g65(.a(x08), .O(new_n130_));
  oai21  g66(.a(x28), .b(new_n130_), .c(new_n64_), .O(new_n131_));
  nand2  g67(.a(new_n110_), .b(new_n69_), .O(new_n132_));
  aoi21  g68(.a(new_n131_), .b(x35), .c(new_n132_), .O(z20));
  one    g69(.O(z02));
  zero   g70(.O(z08));
  nand2  g71(.a(new_n115_), .b(new_n69_), .O(z19));
endmodule


