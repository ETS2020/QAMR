// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n173_, new_n175_;
  nor2   g000(.a(x40), .b(x29), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x37), .O(new_n65_));
  nand2  g003(.a(x35), .b(x28), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  nand2  g006(.a(x40), .b(x39), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x04), .O(new_n70_));
  inv1   g008(.a(new_n70_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  inv1   g011(.a(x35), .O(new_n74_));
  inv1   g012(.a(x36), .O(new_n75_));
  nand3  g013(.a(new_n65_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x27), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n72_), .c(new_n64_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x16), .c(new_n63_), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  inv1   g020(.a(new_n82_), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(x36), .c(x27), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g025(.a(x04), .O(new_n88_));
  inv1   g026(.a(x08), .O(new_n89_));
  inv1   g027(.a(x27), .O(new_n90_));
  aoi21  g028(.a(new_n90_), .b(new_n89_), .c(new_n82_), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n92_), .c(new_n69_), .O(new_n94_));
  oai21  g032(.a(new_n91_), .b(new_n88_), .c(new_n94_), .O(z02));
  inv1   g033(.a(x29), .O(new_n96_));
  inv1   g034(.a(x40), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g036(.a(new_n66_), .b(x27), .O(new_n99_));
  nand2  g037(.a(new_n65_), .b(new_n90_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(x21), .O(z03));
  aoi21  g039(.a(new_n65_), .b(new_n90_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n99_), .c(new_n63_), .O(z04));
  aoi21  g041(.a(x28), .b(x27), .c(x37), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n98_), .O(z05));
  inv1   g043(.a(new_n104_), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(new_n98_), .O(z06));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  inv1   g047(.a(x14), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n110_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n109_), .c(x38), .O(new_n112_));
  inv1   g050(.a(x17), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(x15), .O(new_n114_));
  or2    g052(.a(x33), .b(x31), .O(new_n115_));
  oai21  g053(.a(new_n115_), .b(new_n114_), .c(x14), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(new_n112_), .c(x03), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n98_), .O(z07));
  nand2  g056(.a(new_n69_), .b(new_n98_), .O(z08));
  nand4  g057(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n98_), .O(z09));
  inv1   g059(.a(new_n69_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(x05), .O(new_n123_));
  nand3  g061(.a(new_n69_), .b(new_n98_), .c(x07), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  aoi21  g063(.a(new_n82_), .b(new_n75_), .c(x04), .O(new_n126_));
  nand2  g064(.a(x37), .b(x06), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(new_n63_), .O(new_n128_));
  aoi21  g066(.a(new_n126_), .b(new_n125_), .c(new_n128_), .O(new_n129_));
  inv1   g067(.a(new_n85_), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(new_n122_), .c(x05), .d(new_n88_), .O(new_n131_));
  oai21  g069(.a(new_n129_), .b(new_n90_), .c(new_n131_), .O(z10));
  nand2  g070(.a(new_n122_), .b(x29), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n82_), .c(new_n89_), .O(new_n134_));
  nand2  g072(.a(new_n133_), .b(x27), .O(new_n135_));
  nand3  g073(.a(new_n83_), .b(x27), .c(new_n88_), .O(new_n136_));
  nor2   g074(.a(x30), .b(x09), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n136_), .c(new_n98_), .O(new_n138_));
  aoi21  g076(.a(new_n135_), .b(new_n134_), .c(new_n138_), .O(z11));
  aoi21  g077(.a(new_n76_), .b(x27), .c(x10), .O(new_n140_));
  aoi22  g078(.a(new_n67_), .b(x27), .c(x39), .d(new_n88_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n140_), .c(x40), .O(new_n142_));
  nand3  g080(.a(new_n68_), .b(new_n97_), .c(x29), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n142_), .O(z12));
  inv1   g082(.a(x13), .O(new_n145_));
  nand2  g083(.a(x36), .b(x35), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x28), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n70_), .c(new_n145_), .O(new_n148_));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x20), .O(new_n150_));
  nand2  g088(.a(new_n67_), .b(new_n98_), .O(new_n151_));
  oai21  g089(.a(new_n151_), .b(new_n150_), .c(new_n148_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand3  g091(.a(new_n130_), .b(new_n70_), .c(new_n145_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  nand2  g093(.a(new_n70_), .b(new_n145_), .O(new_n156_));
  nand3  g094(.a(new_n146_), .b(new_n85_), .c(x28), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n156_), .c(new_n150_), .O(new_n159_));
  aoi21  g097(.a(new_n81_), .b(x27), .c(new_n130_), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n156_), .c(new_n68_), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n159_), .c(new_n63_), .O(z14));
  nand4  g100(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n98_), .O(z15));
  nand2  g102(.a(x22), .b(x01), .O(new_n165_));
  nor3   g103(.a(new_n165_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g104(.a(x24), .O(new_n167_));
  inv1   g105(.a(new_n165_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n98_), .c(new_n167_), .d(x23), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z17));
  inv1   g108(.a(new_n137_), .O(new_n171_));
  nand2  g109(.a(x27), .b(x04), .O(new_n172_));
  aoi21  g110(.a(new_n172_), .b(new_n91_), .c(new_n171_), .O(new_n173_));
  oai22  g111(.a(new_n173_), .b(new_n63_), .c(new_n93_), .d(new_n69_), .O(z18));
  oai21  g112(.a(new_n74_), .b(new_n90_), .c(new_n137_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n134_), .c(new_n98_), .O(z20));
  aoi21  g114(.a(new_n135_), .b(new_n134_), .c(new_n138_), .O(z19));
endmodule


