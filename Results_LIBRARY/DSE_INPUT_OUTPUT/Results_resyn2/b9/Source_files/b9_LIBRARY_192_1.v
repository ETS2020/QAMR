// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n172_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n181_,
    new_n182_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x40), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(x22), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x35), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x28), .O(new_n76_));
  oai21  g014(.a(new_n76_), .b(x36), .c(x27), .O(new_n77_));
  nor2   g015(.a(x32), .b(x30), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x04), .c(new_n72_), .O(z01));
  inv1   g018(.a(x22), .O(new_n81_));
  inv1   g019(.a(x08), .O(new_n82_));
  inv1   g020(.a(x27), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n76_), .O(new_n85_));
  and2   g023(.a(x29), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x02), .c(x39), .O(new_n87_));
  aoi21  g025(.a(new_n85_), .b(x04), .c(new_n87_), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n81_), .c(x40), .O(z02));
  inv1   g027(.a(x37), .O(new_n90_));
  nand2  g028(.a(x28), .b(x27), .O(new_n91_));
  oai22  g029(.a(new_n91_), .b(new_n75_), .c(new_n90_), .d(x27), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g031(.a(x21), .O(new_n94_));
  aoi21  g032(.a(new_n92_), .b(new_n94_), .c(new_n72_), .O(z04));
  nand2  g033(.a(x40), .b(new_n81_), .O(new_n96_));
  nand2  g034(.a(new_n91_), .b(new_n90_), .O(new_n97_));
  nand2  g035(.a(new_n97_), .b(new_n96_), .O(z06));
  inv1   g036(.a(z06), .O(z05));
  inv1   g037(.a(x14), .O(new_n100_));
  inv1   g038(.a(x15), .O(new_n101_));
  nand2  g039(.a(x17), .b(new_n101_), .O(new_n102_));
  nor2   g040(.a(x33), .b(x31), .O(new_n103_));
  aoi21  g041(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(new_n104_));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n100_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n106_), .c(x38), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n96_), .c(x03), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(new_n104_), .O(z07));
  inv1   g048(.a(x39), .O(new_n111_));
  aoi21  g049(.a(new_n111_), .b(x22), .c(new_n71_), .O(z08));
  nand4  g050(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n96_), .O(z09));
  inv1   g052(.a(x07), .O(new_n115_));
  nand4  g053(.a(x40), .b(x39), .c(x22), .d(x05), .O(new_n116_));
  oai21  g054(.a(z08), .b(new_n115_), .c(new_n116_), .O(new_n117_));
  inv1   g055(.a(x36), .O(new_n118_));
  inv1   g056(.a(x28), .O(new_n119_));
  nand2  g057(.a(x35), .b(new_n119_), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n118_), .c(x04), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n72_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n117_), .c(new_n123_), .O(new_n124_));
  inv1   g062(.a(new_n78_), .O(new_n125_));
  inv1   g063(.a(new_n116_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n63_), .O(new_n127_));
  oai21  g065(.a(new_n124_), .b(new_n83_), .c(new_n127_), .O(z10));
  nor2   g066(.a(x30), .b(x09), .O(new_n129_));
  inv1   g067(.a(new_n129_), .O(new_n130_));
  nand3  g068(.a(new_n129_), .b(new_n82_), .c(x04), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(x22), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(x40), .O(new_n133_));
  nor2   g071(.a(new_n71_), .b(new_n111_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x29), .O(new_n135_));
  nand2  g073(.a(x27), .b(x04), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(x35), .c(new_n119_), .O(new_n137_));
  nand2  g075(.a(new_n76_), .b(x27), .O(new_n138_));
  aoi22  g076(.a(new_n138_), .b(new_n82_), .c(new_n137_), .d(new_n135_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n130_), .c(new_n133_), .O(z11));
  nand2  g078(.a(new_n66_), .b(new_n90_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(x27), .c(x10), .O(new_n142_));
  inv1   g080(.a(new_n67_), .O(new_n143_));
  aoi22  g081(.a(new_n143_), .b(x27), .c(x39), .d(new_n63_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n142_), .c(x22), .O(new_n145_));
  nand2  g083(.a(new_n143_), .b(x27), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n71_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(z12));
  inv1   g086(.a(x18), .O(new_n149_));
  inv1   g087(.a(x19), .O(new_n150_));
  nand3  g088(.a(x20), .b(new_n150_), .c(new_n149_), .O(new_n151_));
  nand2  g089(.a(new_n96_), .b(new_n143_), .O(new_n152_));
  nand2  g090(.a(x36), .b(x35), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(x28), .O(new_n154_));
  nor2   g092(.a(x13), .b(x04), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(new_n154_), .c(new_n134_), .d(x22), .O(new_n156_));
  oai21  g094(.a(new_n152_), .b(new_n151_), .c(new_n156_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  nand3  g096(.a(x40), .b(x39), .c(x22), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  nand3  g098(.a(new_n155_), .b(new_n160_), .c(new_n125_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n158_), .O(z13));
  nand2  g100(.a(new_n155_), .b(new_n134_), .O(new_n163_));
  nand3  g101(.a(new_n153_), .b(new_n78_), .c(x28), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n163_), .c(new_n151_), .O(new_n166_));
  aoi21  g104(.a(new_n119_), .b(x27), .c(new_n125_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n163_), .c(new_n146_), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(new_n166_), .c(new_n96_), .O(z14));
  nand4  g107(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n96_), .O(z15));
  nand2  g109(.a(x22), .b(x01), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(x23), .c(new_n96_), .O(z16));
  inv1   g111(.a(x24), .O(new_n174_));
  nand4  g112(.a(new_n174_), .b(x23), .c(x22), .d(x01), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z17));
  nand3  g114(.a(new_n134_), .b(x29), .c(x08), .O(new_n177_));
  inv1   g115(.a(new_n137_), .O(new_n178_));
  aoi21  g116(.a(new_n178_), .b(new_n84_), .c(new_n130_), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n177_), .c(new_n72_), .O(z18));
  nand2  g118(.a(new_n178_), .b(new_n84_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n129_), .O(new_n182_));
  aoi22  g120(.a(new_n182_), .b(new_n96_), .c(new_n160_), .d(new_n86_), .O(z19));
  nand2  g121(.a(new_n135_), .b(new_n120_), .O(new_n184_));
  nand2  g122(.a(x35), .b(x27), .O(new_n185_));
  nand3  g123(.a(new_n185_), .b(new_n129_), .c(new_n96_), .O(new_n186_));
  aoi21  g124(.a(new_n184_), .b(x08), .c(new_n186_), .O(z20));
endmodule


