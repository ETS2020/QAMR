// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n179_,
    new_n181_, new_n184_;
  nand2  g000(.a(x40), .b(x00), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g015(.a(x04), .b(x00), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n77_), .c(new_n74_), .d(new_n65_), .O(new_n79_));
  oai21  g017(.a(new_n71_), .b(new_n64_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g026(.a(x35), .O(new_n89_));
  nor2   g027(.a(new_n89_), .b(x28), .O(new_n90_));
  oai21  g028(.a(x27), .b(x08), .c(new_n90_), .O(new_n91_));
  and2   g029(.a(x29), .b(x08), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(x02), .c(x39), .O(new_n93_));
  aoi21  g031(.a(new_n91_), .b(x04), .c(new_n93_), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(x00), .c(x40), .O(z02));
  nand2  g033(.a(new_n69_), .b(x27), .O(new_n96_));
  nand2  g034(.a(new_n68_), .b(new_n67_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n96_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n96_), .c(new_n64_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n64_), .O(z05));
  nand2  g040(.a(new_n101_), .b(new_n63_), .O(new_n103_));
  inv1   g041(.a(new_n103_), .O(z06));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  or2    g049(.a(x33), .b(x31), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n111_), .c(x14), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n109_), .c(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n63_), .O(z07));
  inv1   g053(.a(x00), .O(new_n116_));
  nand3  g054(.a(x40), .b(x39), .c(new_n116_), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z08));
  inv1   g056(.a(x11), .O(new_n119_));
  nand4  g057(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n119_), .O(z09));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  inv1   g060(.a(x04), .O(new_n123_));
  inv1   g061(.a(x07), .O(new_n124_));
  oai21  g062(.a(new_n76_), .b(new_n75_), .c(new_n124_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n84_), .c(new_n123_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n122_), .O(new_n127_));
  inv1   g065(.a(x05), .O(new_n128_));
  nand3  g066(.a(new_n122_), .b(x39), .c(new_n128_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n116_), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(x40), .c(new_n67_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n127_), .O(new_n132_));
  inv1   g070(.a(new_n86_), .O(new_n133_));
  nand4  g071(.a(new_n133_), .b(new_n78_), .c(new_n77_), .d(x05), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n132_), .O(z10));
  inv1   g073(.a(x08), .O(new_n136_));
  nand3  g074(.a(x40), .b(x39), .c(x29), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n83_), .c(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n137_), .b(x27), .O(new_n139_));
  nand3  g077(.a(new_n90_), .b(x27), .c(new_n123_), .O(new_n140_));
  inv1   g078(.a(x09), .O(new_n141_));
  inv1   g079(.a(x30), .O(new_n142_));
  nand3  g080(.a(new_n63_), .b(new_n142_), .c(new_n141_), .O(new_n143_));
  inv1   g081(.a(new_n143_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n140_), .O(new_n145_));
  aoi21  g083(.a(new_n139_), .b(new_n138_), .c(new_n145_), .O(z11));
  nand3  g084(.a(new_n68_), .b(new_n81_), .c(new_n89_), .O(new_n147_));
  aoi21  g085(.a(new_n147_), .b(x27), .c(x10), .O(new_n148_));
  aoi21  g086(.a(x39), .b(new_n123_), .c(new_n70_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(new_n116_), .O(new_n150_));
  inv1   g088(.a(new_n70_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n76_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(new_n150_), .O(z12));
  nand2  g091(.a(new_n63_), .b(x27), .O(new_n154_));
  inv1   g092(.a(x18), .O(new_n155_));
  inv1   g093(.a(x19), .O(new_n156_));
  nand3  g094(.a(x20), .b(new_n156_), .c(new_n155_), .O(new_n157_));
  aoi21  g095(.a(new_n69_), .b(new_n68_), .c(new_n157_), .O(new_n158_));
  nand2  g096(.a(x36), .b(x35), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x28), .O(new_n160_));
  inv1   g098(.a(x13), .O(new_n161_));
  nand3  g099(.a(x39), .b(new_n161_), .c(new_n123_), .O(new_n162_));
  nand2  g100(.a(x40), .b(new_n116_), .O(new_n163_));
  nor2   g101(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n160_), .c(new_n158_), .O(new_n165_));
  nand4  g103(.a(new_n133_), .b(new_n78_), .c(new_n77_), .d(new_n161_), .O(new_n166_));
  oai21  g104(.a(new_n165_), .b(new_n154_), .c(new_n166_), .O(z13));
  inv1   g105(.a(new_n162_), .O(new_n168_));
  nand3  g106(.a(new_n159_), .b(new_n86_), .c(x28), .O(new_n169_));
  aoi21  g107(.a(new_n169_), .b(new_n168_), .c(x00), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n76_), .c(new_n157_), .O(new_n171_));
  nand2  g109(.a(new_n82_), .b(x27), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n86_), .O(new_n173_));
  aoi21  g111(.a(new_n173_), .b(new_n168_), .c(x00), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n76_), .c(new_n151_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n171_), .O(z14));
  inv1   g114(.a(x12), .O(new_n177_));
  nor2   g115(.a(new_n120_), .b(new_n177_), .O(z15));
  nand2  g116(.a(x22), .b(x01), .O(new_n179_));
  oai21  g117(.a(new_n179_), .b(x23), .c(new_n63_), .O(z16));
  inv1   g118(.a(x23), .O(new_n181_));
  nor4   g119(.a(new_n179_), .b(new_n64_), .c(x24), .d(new_n181_), .O(z17));
  inv1   g120(.a(z11), .O(z18));
  nor2   g121(.a(new_n89_), .b(new_n67_), .O(new_n184_));
  nor3   g122(.a(new_n184_), .b(new_n143_), .c(new_n138_), .O(z20));
  aoi21  g123(.a(new_n139_), .b(new_n138_), .c(new_n145_), .O(z19));
endmodule


