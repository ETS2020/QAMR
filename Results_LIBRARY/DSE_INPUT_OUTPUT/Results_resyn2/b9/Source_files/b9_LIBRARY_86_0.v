// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n161_, new_n163_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n172_, new_n174_, new_n175_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x40), .b(x24), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g020(.a(new_n72_), .b(new_n64_), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n79_), .c(new_n83_), .O(z01));
  inv1   g022(.a(x08), .O(new_n85_));
  aoi21  g023(.a(new_n75_), .b(new_n85_), .c(new_n78_), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  aoi21  g025(.a(x29), .b(x08), .c(x02), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g027(.a(new_n86_), .b(new_n64_), .c(new_n89_), .O(z02));
  inv1   g028(.a(x37), .O(new_n91_));
  nand3  g029(.a(x35), .b(x28), .c(x27), .O(new_n92_));
  oai21  g030(.a(new_n91_), .b(x27), .c(new_n92_), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n72_), .O(z03));
  nor2   g032(.a(new_n72_), .b(x21), .O(new_n95_));
  nand2  g033(.a(new_n95_), .b(new_n93_), .O(z04));
  aoi21  g034(.a(x28), .b(x27), .c(x37), .O(new_n97_));
  nor2   g035(.a(new_n97_), .b(new_n72_), .O(z05));
  inv1   g036(.a(new_n97_), .O(new_n99_));
  nor2   g037(.a(new_n99_), .b(new_n72_), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(new_n72_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n101_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n106_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n105_), .O(z07));
  inv1   g050(.a(new_n87_), .O(z08));
  inv1   g051(.a(x11), .O(new_n114_));
  nand4  g052(.a(new_n106_), .b(x34), .c(x27), .d(x26), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n114_), .O(z09));
  nand3  g054(.a(x40), .b(x39), .c(x05), .O(new_n117_));
  oai21  g055(.a(x40), .b(x24), .c(x07), .O(new_n118_));
  oai21  g056(.a(new_n118_), .b(z08), .c(new_n117_), .O(new_n119_));
  aoi21  g057(.a(new_n78_), .b(new_n76_), .c(x04), .O(new_n120_));
  nand2  g058(.a(x37), .b(x06), .O(new_n121_));
  nor2   g059(.a(new_n121_), .b(new_n72_), .O(new_n122_));
  aoi21  g060(.a(new_n120_), .b(new_n119_), .c(new_n122_), .O(new_n123_));
  inv1   g061(.a(new_n117_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n82_), .c(new_n64_), .O(new_n125_));
  oai21  g063(.a(new_n123_), .b(new_n75_), .c(new_n125_), .O(z10));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n78_), .c(new_n85_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  inv1   g068(.a(new_n78_), .O(new_n131_));
  nor2   g069(.a(new_n75_), .b(x04), .O(new_n132_));
  inv1   g070(.a(x09), .O(new_n133_));
  nand2  g071(.a(new_n80_), .b(new_n133_), .O(new_n134_));
  aoi21  g072(.a(new_n132_), .b(new_n131_), .c(new_n134_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n130_), .c(new_n106_), .O(new_n136_));
  inv1   g074(.a(new_n136_), .O(z11));
  nand2  g075(.a(new_n67_), .b(x27), .O(new_n138_));
  nand3  g076(.a(new_n106_), .b(new_n68_), .c(new_n138_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z12));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  nor2   g080(.a(x13), .b(x04), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n142_), .c(z08), .O(new_n144_));
  nand2  g082(.a(x35), .b(x28), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n91_), .O(new_n146_));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(new_n106_), .c(new_n146_), .d(x20), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(new_n144_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x27), .O(new_n150_));
  nand3  g088(.a(new_n143_), .b(z08), .c(new_n82_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(z13));
  nand2  g090(.a(new_n143_), .b(z08), .O(new_n153_));
  nand2  g091(.a(new_n147_), .b(x20), .O(new_n154_));
  nand4  g092(.a(new_n141_), .b(new_n81_), .c(new_n80_), .d(x28), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n153_), .c(new_n154_), .O(new_n157_));
  aoi21  g095(.a(new_n77_), .b(x27), .c(new_n82_), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n153_), .c(new_n66_), .d(new_n75_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n157_), .c(new_n106_), .O(z14));
  inv1   g098(.a(x12), .O(new_n161_));
  nor2   g099(.a(new_n115_), .b(new_n161_), .O(z15));
  nand2  g100(.a(x22), .b(x01), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(x23), .c(new_n106_), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  nand3  g103(.a(x40), .b(new_n165_), .c(x23), .O(new_n166_));
  nor2   g104(.a(new_n166_), .b(new_n163_), .O(z17));
  nand2  g105(.a(x27), .b(x04), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n86_), .c(new_n134_), .O(new_n169_));
  nand3  g107(.a(z08), .b(x29), .c(x08), .O(new_n170_));
  oai21  g108(.a(new_n169_), .b(new_n72_), .c(new_n170_), .O(z18));
  nand2  g109(.a(new_n135_), .b(new_n130_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n106_), .O(z19));
  nand2  g111(.a(x35), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n80_), .c(new_n133_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n128_), .c(new_n106_), .O(z20));
endmodule


