// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n171_, new_n173_, new_n174_,
    new_n175_, new_n177_, new_n178_, new_n179_, new_n180_, new_n182_,
    new_n183_, new_n184_, new_n186_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x05), .O(new_n71_));
  nor2   g009(.a(x40), .b(new_n71_), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  nor2   g019(.a(new_n72_), .b(new_n63_), .O(new_n82_));
  oai21  g020(.a(new_n81_), .b(new_n79_), .c(new_n82_), .O(z01));
  inv1   g021(.a(x40), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n71_), .O(new_n85_));
  inv1   g023(.a(x35), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(x28), .O(new_n87_));
  oai21  g025(.a(x27), .b(x08), .c(new_n87_), .O(new_n88_));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x39), .O(new_n92_));
  aoi21  g030(.a(new_n88_), .b(x04), .c(new_n92_), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(new_n84_), .c(new_n85_), .O(z02));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  oai22  g034(.a(new_n96_), .b(new_n86_), .c(new_n95_), .d(x27), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand2  g037(.a(new_n84_), .b(x05), .O(new_n100_));
  nand3  g038(.a(new_n97_), .b(new_n100_), .c(new_n99_), .O(z04));
  nand2  g039(.a(new_n96_), .b(new_n95_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n100_), .O(z06));
  inv1   g041(.a(z06), .O(z05));
  inv1   g042(.a(x14), .O(new_n105_));
  inv1   g043(.a(x15), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n106_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n105_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n100_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  nand2  g053(.a(x40), .b(x39), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n100_), .O(z08));
  and2   g055(.a(x34), .b(x27), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(x26), .c(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n100_), .O(z09));
  inv1   g058(.a(new_n116_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(x05), .O(new_n122_));
  nand3  g060(.a(new_n116_), .b(new_n100_), .c(x07), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  aoi21  g062(.a(new_n78_), .b(new_n76_), .c(x04), .O(new_n125_));
  nand2  g063(.a(x37), .b(x06), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n72_), .O(new_n127_));
  aoi21  g065(.a(new_n125_), .b(new_n124_), .c(new_n127_), .O(new_n128_));
  nand4  g066(.a(new_n121_), .b(new_n81_), .c(x05), .d(new_n63_), .O(new_n129_));
  oai21  g067(.a(new_n128_), .b(new_n75_), .c(new_n129_), .O(z10));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n78_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x08), .O(new_n135_));
  nand3  g073(.a(new_n87_), .b(x27), .c(new_n63_), .O(new_n136_));
  nand3  g074(.a(new_n133_), .b(x27), .c(x04), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(new_n138_));
  aoi21  g076(.a(new_n136_), .b(new_n135_), .c(new_n138_), .O(new_n139_));
  oai21  g077(.a(new_n139_), .b(new_n132_), .c(new_n100_), .O(z11));
  nand2  g078(.a(new_n66_), .b(new_n95_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(x27), .c(x10), .O(new_n142_));
  inv1   g080(.a(new_n67_), .O(new_n143_));
  aoi22  g081(.a(new_n143_), .b(x27), .c(x39), .d(new_n63_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n142_), .c(x40), .O(new_n145_));
  nand2  g083(.a(new_n143_), .b(x27), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n84_), .c(new_n71_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n145_), .O(z12));
  inv1   g086(.a(x13), .O(new_n149_));
  nand2  g087(.a(x36), .b(x35), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x28), .O(new_n151_));
  nand4  g089(.a(new_n151_), .b(new_n121_), .c(new_n149_), .d(new_n63_), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand3  g092(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n155_));
  nand2  g093(.a(new_n100_), .b(new_n143_), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n155_), .c(new_n152_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  nand3  g096(.a(new_n121_), .b(new_n149_), .c(new_n63_), .O(new_n159_));
  inv1   g097(.a(new_n159_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n81_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n158_), .O(z13));
  nand3  g100(.a(new_n150_), .b(new_n80_), .c(x28), .O(new_n163_));
  inv1   g101(.a(new_n163_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n159_), .c(new_n155_), .O(new_n165_));
  aoi21  g103(.a(new_n77_), .b(x27), .c(new_n81_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n159_), .c(new_n146_), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n165_), .c(new_n100_), .O(z14));
  nand4  g106(.a(new_n118_), .b(new_n100_), .c(x26), .d(x12), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z15));
  nand2  g108(.a(x22), .b(x01), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(x23), .c(new_n100_), .O(z16));
  inv1   g110(.a(x24), .O(new_n173_));
  inv1   g111(.a(new_n171_), .O(new_n174_));
  nand4  g112(.a(new_n174_), .b(new_n100_), .c(new_n173_), .d(x23), .O(new_n175_));
  inv1   g113(.a(new_n175_), .O(z17));
  inv1   g114(.a(x08), .O(new_n177_));
  aoi21  g115(.a(new_n75_), .b(new_n177_), .c(new_n78_), .O(new_n178_));
  nand2  g116(.a(x27), .b(x04), .O(new_n179_));
  aoi21  g117(.a(new_n179_), .b(new_n178_), .c(new_n132_), .O(new_n180_));
  oai22  g118(.a(new_n180_), .b(new_n72_), .c(new_n116_), .d(new_n90_), .O(z18));
  inv1   g119(.a(new_n135_), .O(new_n182_));
  nand2  g120(.a(new_n133_), .b(x27), .O(new_n183_));
  nand3  g121(.a(new_n136_), .b(new_n131_), .c(new_n100_), .O(new_n184_));
  aoi21  g122(.a(new_n183_), .b(new_n182_), .c(new_n184_), .O(z19));
  oai21  g123(.a(new_n86_), .b(new_n75_), .c(new_n131_), .O(new_n186_));
  oai21  g124(.a(new_n186_), .b(new_n182_), .c(new_n100_), .O(z20));
endmodule


