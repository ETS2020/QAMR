// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n159_, new_n161_,
    new_n162_, new_n166_, new_n167_;
  nand2  g000(.a(x40), .b(x29), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x10), .O(new_n73_));
  oai21  g011(.a(x36), .b(x35), .c(x27), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g013(.a(x29), .O(new_n76_));
  nand3  g014(.a(x40), .b(x39), .c(new_n76_), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n75_), .c(new_n65_), .d(new_n72_), .O(new_n79_));
  oai21  g017(.a(new_n71_), .b(new_n64_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n67_), .c(new_n83_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g023(.a(x08), .O(new_n86_));
  nand2  g024(.a(new_n67_), .b(new_n86_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(x35), .c(new_n81_), .O(new_n88_));
  nand2  g026(.a(x39), .b(x02), .O(new_n89_));
  aoi21  g027(.a(new_n88_), .b(x04), .c(new_n89_), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(x29), .c(x40), .O(z02));
  nand2  g029(.a(new_n69_), .b(x27), .O(new_n92_));
  nand2  g030(.a(new_n68_), .b(new_n67_), .O(new_n93_));
  nand4  g031(.a(new_n93_), .b(new_n92_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g032(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(new_n92_), .c(new_n64_), .O(z04));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n63_), .c(new_n68_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x14), .O(new_n100_));
  nand2  g038(.a(x17), .b(new_n65_), .O(new_n101_));
  nor2   g039(.a(x33), .b(x31), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n100_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n105_), .c(x38), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n63_), .c(x03), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(new_n103_), .O(z07));
  oai21  g047(.a(x39), .b(x29), .c(x40), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(z08));
  inv1   g049(.a(x11), .O(new_n112_));
  nor2   g050(.a(new_n64_), .b(new_n67_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(x34), .c(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n112_), .O(z09));
  nand2  g053(.a(new_n110_), .b(x07), .O(new_n116_));
  nand4  g054(.a(x40), .b(x39), .c(new_n76_), .d(x05), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  inv1   g056(.a(x36), .O(new_n119_));
  nand2  g057(.a(x35), .b(new_n81_), .O(new_n120_));
  aoi21  g058(.a(new_n120_), .b(new_n119_), .c(x04), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n64_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n118_), .c(new_n123_), .O(new_n124_));
  inv1   g062(.a(new_n83_), .O(new_n125_));
  inv1   g063(.a(new_n117_), .O(new_n126_));
  nand3  g064(.a(new_n126_), .b(new_n125_), .c(new_n72_), .O(new_n127_));
  oai21  g065(.a(new_n124_), .b(new_n67_), .c(new_n127_), .O(z10));
  nor2   g066(.a(x30), .b(x09), .O(new_n129_));
  nand2  g067(.a(x27), .b(x04), .O(new_n130_));
  nand4  g068(.a(new_n130_), .b(new_n87_), .c(x35), .d(new_n81_), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(new_n129_), .c(new_n63_), .O(new_n132_));
  inv1   g070(.a(new_n132_), .O(z11));
  inv1   g071(.a(x35), .O(new_n134_));
  nand3  g072(.a(new_n68_), .b(new_n119_), .c(new_n134_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x27), .c(x10), .O(new_n136_));
  aoi21  g074(.a(x39), .b(new_n72_), .c(new_n70_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n136_), .c(new_n76_), .O(new_n138_));
  or2    g076(.a(new_n70_), .b(x40), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n138_), .O(z12));
  nand2  g078(.a(new_n69_), .b(new_n68_), .O(new_n141_));
  nor2   g079(.a(x19), .b(x18), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(x20), .O(new_n143_));
  oai21  g081(.a(new_n119_), .b(new_n134_), .c(x28), .O(new_n144_));
  nor2   g082(.a(x13), .b(x04), .O(new_n145_));
  and2   g083(.a(x40), .b(x39), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n145_), .c(new_n144_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand2  g086(.a(new_n143_), .b(x29), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n113_), .O(new_n150_));
  nand3  g088(.a(new_n145_), .b(new_n125_), .c(new_n78_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n150_), .O(z13));
  aoi21  g090(.a(new_n147_), .b(new_n143_), .c(new_n67_), .O(new_n153_));
  nand2  g091(.a(new_n146_), .b(new_n145_), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n83_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n153_), .c(new_n63_), .O(z14));
  inv1   g094(.a(x12), .O(new_n157_));
  nor2   g095(.a(new_n114_), .b(new_n157_), .O(z15));
  nand2  g096(.a(x22), .b(x01), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(x23), .c(new_n63_), .O(z16));
  inv1   g098(.a(x24), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x23), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n159_), .c(new_n63_), .O(z17));
  aoi21  g101(.a(new_n131_), .b(new_n129_), .c(new_n64_), .O(z18));
  inv1   g102(.a(z18), .O(z19));
  aoi21  g103(.a(new_n81_), .b(x08), .c(x27), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n134_), .c(new_n129_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n63_), .O(z20));
endmodule


