// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n102_, new_n103_,
    new_n104_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n143_, new_n145_, new_n146_, new_n148_, new_n149_,
    new_n152_;
  nand2  g00(.a(x14), .b(x03), .O(new_n63_));
  inv1   g01(.a(new_n63_), .O(new_n64_));
  inv1   g02(.a(x15), .O(new_n65_));
  inv1   g03(.a(x27), .O(new_n66_));
  inv1   g04(.a(x37), .O(new_n67_));
  nand2  g05(.a(x35), .b(x28), .O(new_n68_));
  nand2  g06(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g07(.a(new_n69_), .O(new_n70_));
  inv1   g08(.a(x04), .O(new_n71_));
  inv1   g09(.a(x35), .O(new_n72_));
  inv1   g10(.a(x36), .O(new_n73_));
  nand2  g11(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  and2   g12(.a(x40), .b(x39), .O(new_n75_));
  nand3  g13(.a(new_n75_), .b(new_n74_), .c(new_n71_), .O(new_n76_));
  aoi21  g14(.a(new_n76_), .b(new_n70_), .c(new_n66_), .O(new_n77_));
  nand3  g15(.a(new_n75_), .b(x10), .c(new_n71_), .O(new_n78_));
  inv1   g16(.a(new_n78_), .O(new_n79_));
  oai21  g17(.a(new_n79_), .b(new_n77_), .c(new_n65_), .O(new_n80_));
  aoi21  g18(.a(new_n80_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g19(.a(x28), .O(new_n82_));
  aoi21  g20(.a(x35), .b(new_n82_), .c(x36), .O(new_n83_));
  nor2   g21(.a(x32), .b(x30), .O(new_n84_));
  oai21  g22(.a(new_n83_), .b(new_n66_), .c(new_n84_), .O(new_n85_));
  nand3  g23(.a(new_n85_), .b(new_n63_), .c(x04), .O(z01));
  nand2  g24(.a(x35), .b(new_n82_), .O(new_n87_));
  nor2   g25(.a(x27), .b(x08), .O(new_n88_));
  oai21  g26(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  nand2  g27(.a(x40), .b(x39), .O(new_n90_));
  aoi21  g28(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor3   g29(.a(new_n91_), .b(new_n90_), .c(new_n64_), .O(new_n92_));
  nand2  g30(.a(new_n92_), .b(new_n89_), .O(z02));
  aoi21  g31(.a(new_n68_), .b(x27), .c(new_n64_), .O(new_n94_));
  nand2  g32(.a(new_n67_), .b(new_n66_), .O(new_n95_));
  nand3  g33(.a(new_n95_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g34(.a(x21), .O(new_n97_));
  nand3  g35(.a(new_n95_), .b(new_n94_), .c(new_n97_), .O(z04));
  nand2  g36(.a(x28), .b(x27), .O(new_n99_));
  aoi21  g37(.a(new_n99_), .b(new_n67_), .c(new_n64_), .O(z05));
  inv1   g38(.a(z05), .O(z06));
  nor2   g39(.a(x25), .b(x00), .O(new_n102_));
  inv1   g40(.a(x14), .O(new_n103_));
  nand2  g41(.a(new_n103_), .b(x03), .O(new_n104_));
  aoi21  g42(.a(new_n102_), .b(x38), .c(new_n104_), .O(z07));
  nand2  g43(.a(new_n90_), .b(new_n63_), .O(z08));
  inv1   g44(.a(x11), .O(new_n107_));
  nand4  g45(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n108_));
  nor2   g46(.a(new_n108_), .b(new_n107_), .O(z09));
  nor2   g47(.a(new_n83_), .b(new_n66_), .O(new_n110_));
  nand2  g48(.a(new_n75_), .b(x05), .O(new_n111_));
  inv1   g49(.a(new_n111_), .O(new_n112_));
  and2   g50(.a(new_n90_), .b(x07), .O(new_n113_));
  aoi22  g51(.a(new_n113_), .b(new_n110_), .c(new_n112_), .d(new_n85_), .O(new_n114_));
  nand3  g52(.a(x37), .b(x27), .c(x06), .O(new_n115_));
  and2   g53(.a(new_n115_), .b(new_n63_), .O(new_n116_));
  oai21  g54(.a(new_n114_), .b(x04), .c(new_n116_), .O(z10));
  inv1   g55(.a(x08), .O(new_n118_));
  nand2  g56(.a(new_n75_), .b(x29), .O(new_n119_));
  aoi21  g57(.a(new_n119_), .b(new_n87_), .c(new_n118_), .O(new_n120_));
  nand2  g58(.a(new_n119_), .b(x27), .O(new_n121_));
  nand4  g59(.a(x35), .b(new_n82_), .c(x27), .d(new_n71_), .O(new_n122_));
  nor2   g60(.a(x30), .b(x09), .O(new_n123_));
  nand2  g61(.a(new_n123_), .b(new_n63_), .O(new_n124_));
  inv1   g62(.a(new_n124_), .O(new_n125_));
  nand2  g63(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  aoi21  g64(.a(new_n121_), .b(new_n120_), .c(new_n126_), .O(z11));
  inv1   g65(.a(new_n77_), .O(new_n128_));
  nand3  g66(.a(new_n78_), .b(new_n128_), .c(new_n63_), .O(new_n129_));
  inv1   g67(.a(new_n129_), .O(z12));
  oai21  g68(.a(new_n73_), .b(new_n72_), .c(x28), .O(new_n131_));
  nor2   g69(.a(x13), .b(x04), .O(new_n132_));
  nand3  g70(.a(new_n132_), .b(new_n131_), .c(new_n75_), .O(new_n133_));
  nor2   g71(.a(x19), .b(x18), .O(new_n134_));
  nand3  g72(.a(new_n134_), .b(new_n69_), .c(x20), .O(new_n135_));
  aoi21  g73(.a(new_n135_), .b(new_n133_), .c(new_n66_), .O(new_n136_));
  inv1   g74(.a(new_n136_), .O(new_n137_));
  nand2  g75(.a(new_n132_), .b(new_n75_), .O(new_n138_));
  nor2   g76(.a(new_n138_), .b(new_n84_), .O(new_n139_));
  inv1   g77(.a(new_n139_), .O(new_n140_));
  aoi21  g78(.a(new_n140_), .b(new_n137_), .c(new_n64_), .O(z13));
  nor3   g79(.a(new_n139_), .b(new_n136_), .c(new_n64_), .O(z14));
  inv1   g80(.a(x12), .O(new_n143_));
  nor2   g81(.a(new_n108_), .b(new_n143_), .O(z15));
  inv1   g82(.a(x23), .O(new_n145_));
  nand4  g83(.a(new_n63_), .b(new_n145_), .c(x22), .d(x01), .O(new_n146_));
  inv1   g84(.a(new_n146_), .O(z16));
  inv1   g85(.a(x24), .O(new_n148_));
  nand4  g86(.a(new_n148_), .b(x23), .c(x22), .d(x01), .O(new_n149_));
  nand2  g87(.a(new_n149_), .b(new_n63_), .O(z17));
  inv1   g88(.a(z11), .O(z18));
  oai21  g89(.a(new_n72_), .b(new_n66_), .c(new_n125_), .O(new_n152_));
  nor2   g90(.a(new_n152_), .b(new_n120_), .O(z20));
  aoi21  g91(.a(new_n121_), .b(new_n120_), .c(new_n126_), .O(z19));
endmodule


