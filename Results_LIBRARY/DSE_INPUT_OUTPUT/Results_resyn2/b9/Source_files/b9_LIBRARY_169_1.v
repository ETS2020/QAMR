// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n157_, new_n159_, new_n160_, new_n162_;
  nand2  g000(.a(x40), .b(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x04), .O(new_n64_));
  aoi21  g002(.a(x35), .b(x28), .c(x37), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  nand3  g005(.a(x40), .b(x39), .c(new_n67_), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(new_n69_));
  aoi22  g007(.a(new_n69_), .b(x27), .c(new_n64_), .d(x10), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  inv1   g009(.a(x31), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(x13), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  oai21  g012(.a(new_n70_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g017(.a(new_n79_), .b(x27), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x04), .c(new_n73_), .O(z01));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n78_), .c(x04), .O(new_n85_));
  aoi21  g023(.a(x29), .b(x08), .c(x02), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(new_n63_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(new_n85_), .c(new_n73_), .O(z02));
  inv1   g026(.a(x35), .O(new_n89_));
  inv1   g027(.a(x37), .O(new_n90_));
  nand2  g028(.a(x28), .b(x27), .O(new_n91_));
  oai22  g029(.a(new_n91_), .b(new_n89_), .c(new_n90_), .d(x27), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(x21), .c(new_n73_), .O(z03));
  inv1   g031(.a(x21), .O(new_n94_));
  aoi21  g032(.a(new_n92_), .b(new_n94_), .c(new_n73_), .O(z04));
  inv1   g033(.a(new_n73_), .O(new_n96_));
  nand3  g034(.a(new_n91_), .b(new_n96_), .c(new_n90_), .O(z05));
  inv1   g035(.a(z05), .O(z06));
  inv1   g036(.a(x25), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(x00), .O(new_n100_));
  inv1   g038(.a(x14), .O(new_n101_));
  nand2  g039(.a(x25), .b(new_n101_), .O(new_n102_));
  nand3  g040(.a(new_n102_), .b(new_n100_), .c(x38), .O(new_n103_));
  inv1   g041(.a(x17), .O(new_n104_));
  nor2   g042(.a(new_n104_), .b(x15), .O(new_n105_));
  oai21  g043(.a(new_n105_), .b(x33), .c(x14), .O(new_n106_));
  inv1   g044(.a(x13), .O(new_n107_));
  oai21  g045(.a(x14), .b(new_n107_), .c(x31), .O(new_n108_));
  nand4  g046(.a(new_n108_), .b(new_n106_), .c(new_n103_), .d(x03), .O(new_n109_));
  inv1   g047(.a(new_n109_), .O(z07));
  nor2   g048(.a(new_n73_), .b(new_n63_), .O(z08));
  nand3  g049(.a(x34), .b(x27), .c(x26), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n96_), .c(x11), .O(new_n114_));
  inv1   g052(.a(new_n114_), .O(z09));
  nand3  g053(.a(x40), .b(x39), .c(x05), .O(new_n116_));
  nand2  g054(.a(new_n63_), .b(x07), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n79_), .c(x27), .O(new_n119_));
  or2    g057(.a(new_n116_), .b(new_n81_), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n67_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n122_), .c(new_n73_), .O(z10));
  inv1   g062(.a(x08), .O(new_n125_));
  nand3  g063(.a(x40), .b(x39), .c(x29), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n78_), .c(new_n125_), .O(new_n127_));
  nand2  g065(.a(new_n126_), .b(x27), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  or2    g067(.a(x30), .b(x09), .O(new_n130_));
  inv1   g068(.a(x27), .O(new_n131_));
  nor2   g069(.a(x28), .b(new_n131_), .O(new_n132_));
  nor2   g070(.a(new_n89_), .b(x04), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n132_), .c(new_n130_), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n129_), .c(new_n73_), .O(z18));
  inv1   g073(.a(z18), .O(z11));
  nand2  g074(.a(new_n96_), .b(new_n70_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z12));
  oai21  g076(.a(new_n81_), .b(new_n68_), .c(new_n72_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n107_), .O(new_n140_));
  nor2   g078(.a(x19), .b(x18), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x20), .O(new_n142_));
  aoi21  g080(.a(x36), .b(x35), .c(new_n77_), .O(new_n143_));
  nand2  g081(.a(new_n64_), .b(new_n107_), .O(new_n144_));
  oai22  g082(.a(new_n144_), .b(new_n143_), .c(new_n142_), .d(new_n65_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x27), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n140_), .O(z13));
  nand2  g085(.a(new_n143_), .b(new_n81_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n64_), .c(x31), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(x13), .c(new_n142_), .O(new_n150_));
  oai21  g088(.a(x28), .b(new_n131_), .c(new_n81_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n64_), .c(x31), .O(new_n152_));
  oai22  g090(.a(new_n152_), .b(x13), .c(new_n65_), .d(new_n131_), .O(new_n153_));
  nand2  g091(.a(new_n153_), .b(new_n150_), .O(z14));
  nand3  g092(.a(new_n113_), .b(new_n96_), .c(x12), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(z15));
  nand2  g094(.a(x22), .b(x01), .O(new_n157_));
  nor3   g095(.a(new_n157_), .b(new_n73_), .c(x23), .O(z16));
  inv1   g096(.a(x24), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x23), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n157_), .c(new_n96_), .O(z17));
  nor2   g099(.a(new_n89_), .b(new_n131_), .O(new_n162_));
  nor4   g100(.a(new_n162_), .b(new_n130_), .c(new_n127_), .d(new_n73_), .O(z20));
  inv1   g101(.a(z18), .O(z19));
endmodule


