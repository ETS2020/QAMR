// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n98_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n162_, new_n163_, new_n165_, new_n166_, new_n167_,
    new_n170_, new_n171_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x02), .O(new_n71_));
  nor2   g009(.a(x18), .b(new_n71_), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x36), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  nand2  g014(.a(x35), .b(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g016(.a(new_n78_), .b(x27), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g019(.a(new_n81_), .b(x04), .c(new_n72_), .O(z01));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n77_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x29), .b(x08), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n71_), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  nor2   g025(.a(new_n72_), .b(new_n87_), .O(z08));
  nand3  g026(.a(z08), .b(new_n86_), .c(new_n84_), .O(z02));
  inv1   g027(.a(x27), .O(new_n90_));
  aoi21  g028(.a(x35), .b(x28), .c(new_n90_), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n72_), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n72_), .O(z05));
  inv1   g037(.a(new_n98_), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n72_), .O(z06));
  inv1   g039(.a(new_n72_), .O(new_n102_));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n102_), .O(z07));
  and2   g051(.a(x34), .b(x27), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(x26), .c(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n102_), .O(z09));
  nand2  g054(.a(new_n87_), .b(x07), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n78_), .c(x27), .O(new_n120_));
  inv1   g058(.a(new_n80_), .O(new_n121_));
  inv1   g059(.a(new_n118_), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n63_), .O(new_n125_));
  nand3  g063(.a(x37), .b(x27), .c(x06), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(new_n125_), .c(new_n72_), .O(z10));
  inv1   g065(.a(x08), .O(new_n128_));
  inv1   g066(.a(new_n87_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n77_), .c(new_n128_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  nand4  g072(.a(x35), .b(new_n76_), .c(x27), .d(new_n63_), .O(new_n135_));
  and2   g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n102_), .O(z11));
  nand2  g076(.a(new_n102_), .b(new_n69_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z12));
  aoi21  g078(.a(x36), .b(x35), .c(new_n76_), .O(new_n141_));
  nor2   g079(.a(new_n141_), .b(new_n90_), .O(new_n142_));
  nor2   g080(.a(new_n142_), .b(new_n121_), .O(new_n143_));
  nor2   g081(.a(x13), .b(x04), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(z08), .O(new_n145_));
  or2    g083(.a(new_n66_), .b(new_n90_), .O(new_n146_));
  inv1   g084(.a(x18), .O(new_n147_));
  inv1   g085(.a(x19), .O(new_n148_));
  nand3  g086(.a(x20), .b(new_n148_), .c(new_n147_), .O(new_n149_));
  inv1   g087(.a(new_n149_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(new_n71_), .O(new_n151_));
  oai22  g089(.a(new_n151_), .b(new_n146_), .c(new_n145_), .d(new_n143_), .O(z13));
  nand2  g090(.a(new_n144_), .b(new_n129_), .O(new_n153_));
  nand2  g091(.a(new_n141_), .b(new_n80_), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n153_), .c(new_n149_), .O(new_n156_));
  aoi21  g094(.a(new_n76_), .b(x27), .c(new_n121_), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(new_n153_), .c(new_n146_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n156_), .c(new_n102_), .O(z14));
  nand4  g097(.a(new_n114_), .b(new_n102_), .c(x26), .d(x12), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z15));
  inv1   g099(.a(x01), .O(new_n162_));
  inv1   g100(.a(x22), .O(new_n163_));
  nor4   g101(.a(new_n72_), .b(x23), .c(new_n163_), .d(new_n162_), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  nor2   g103(.a(new_n163_), .b(new_n162_), .O(new_n166_));
  nand4  g104(.a(new_n166_), .b(new_n102_), .c(new_n165_), .d(x23), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(z17));
  aoi21  g106(.a(new_n136_), .b(new_n133_), .c(new_n72_), .O(z18));
  nand2  g107(.a(x35), .b(x27), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n134_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n131_), .c(new_n102_), .O(z20));
  nand2  g110(.a(new_n137_), .b(new_n102_), .O(z19));
endmodule


