// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n165_, new_n167_,
    new_n169_, new_n170_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n184_;
  inv1   g000(.a(x15), .O(new_n63_));
  inv1   g001(.a(x28), .O(new_n64_));
  inv1   g002(.a(x35), .O(new_n65_));
  inv1   g003(.a(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(x27), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  inv1   g007(.a(x39), .O(new_n70_));
  inv1   g008(.a(x40), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n68_), .O(new_n74_));
  inv1   g012(.a(x10), .O(new_n75_));
  inv1   g013(.a(x27), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g015(.a(new_n77_), .b(new_n74_), .c(new_n63_), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  nand2  g017(.a(x40), .b(new_n79_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n78_), .c(x16), .O(z00));
  inv1   g019(.a(new_n80_), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  nor2   g021(.a(new_n65_), .b(x28), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(x36), .c(x27), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n82_), .O(z01));
  nand2  g025(.a(x35), .b(new_n64_), .O(new_n88_));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n88_), .c(x04), .O(new_n90_));
  inv1   g028(.a(x02), .O(new_n91_));
  nand3  g029(.a(x40), .b(x39), .c(x36), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n91_), .c(new_n92_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n90_), .O(z02));
  nand3  g033(.a(x35), .b(x28), .c(x27), .O(new_n96_));
  oai21  g034(.a(new_n66_), .b(x27), .c(new_n96_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n82_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  aoi21  g037(.a(new_n97_), .b(new_n99_), .c(new_n82_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n82_), .O(z05));
  and2   g040(.a(new_n101_), .b(new_n80_), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n63_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n104_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n80_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  aoi21  g051(.a(new_n70_), .b(x36), .c(new_n71_), .O(z08));
  and2   g052(.a(x34), .b(x27), .O(new_n115_));
  nand3  g053(.a(new_n115_), .b(x26), .c(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n80_), .O(z09));
  aoi21  g055(.a(new_n88_), .b(new_n79_), .c(new_n76_), .O(new_n118_));
  and2   g056(.a(new_n92_), .b(x07), .O(new_n119_));
  nand3  g057(.a(x40), .b(x39), .c(x05), .O(new_n120_));
  aoi21  g058(.a(new_n83_), .b(new_n76_), .c(new_n120_), .O(new_n121_));
  aoi21  g059(.a(new_n119_), .b(new_n118_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  and2   g061(.a(new_n123_), .b(new_n80_), .O(new_n124_));
  oai21  g062(.a(new_n122_), .b(x04), .c(new_n124_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  nand2  g064(.a(new_n80_), .b(new_n76_), .O(new_n127_));
  nand3  g065(.a(x35), .b(new_n64_), .c(new_n69_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x36), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  aoi21  g069(.a(x39), .b(x29), .c(new_n79_), .O(new_n132_));
  nor2   g070(.a(new_n76_), .b(new_n69_), .O(new_n133_));
  oai22  g071(.a(new_n133_), .b(new_n88_), .c(new_n132_), .d(new_n71_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  aoi21  g074(.a(new_n134_), .b(new_n131_), .c(new_n136_), .O(z11));
  nand3  g075(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(z12));
  nand2  g076(.a(new_n65_), .b(x28), .O(new_n139_));
  nor2   g077(.a(x13), .b(x04), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n139_), .c(new_n72_), .O(new_n141_));
  nor2   g079(.a(x19), .b(x18), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n67_), .c(x20), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  inv1   g083(.a(new_n140_), .O(new_n146_));
  oai21  g084(.a(x32), .b(x30), .c(x39), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n146_), .c(x36), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x40), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n145_), .O(z13));
  inv1   g088(.a(x18), .O(new_n151_));
  inv1   g089(.a(x19), .O(new_n152_));
  nand3  g090(.a(x20), .b(new_n152_), .c(new_n151_), .O(new_n153_));
  inv1   g091(.a(new_n153_), .O(new_n154_));
  nand2  g092(.a(new_n83_), .b(x28), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n154_), .c(x37), .O(new_n156_));
  nand3  g094(.a(new_n155_), .b(new_n140_), .c(new_n72_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n156_), .c(new_n65_), .O(new_n158_));
  nand2  g096(.a(new_n83_), .b(new_n76_), .O(new_n159_));
  nand3  g097(.a(new_n140_), .b(new_n159_), .c(new_n72_), .O(new_n160_));
  aoi21  g098(.a(new_n66_), .b(new_n64_), .c(new_n76_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n154_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n160_), .c(new_n82_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n158_), .O(z14));
  nand4  g102(.a(new_n115_), .b(new_n80_), .c(x26), .d(x12), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z15));
  nand2  g104(.a(x22), .b(x01), .O(new_n167_));
  nor3   g105(.a(new_n167_), .b(new_n82_), .c(x23), .O(z16));
  inv1   g106(.a(x24), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(x23), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n167_), .c(new_n80_), .O(z17));
  nand3  g109(.a(x40), .b(x39), .c(x29), .O(new_n172_));
  aoi21  g110(.a(new_n172_), .b(new_n88_), .c(new_n126_), .O(new_n173_));
  nand2  g111(.a(new_n172_), .b(x27), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n173_), .O(new_n175_));
  nand3  g113(.a(new_n84_), .b(x27), .c(new_n69_), .O(new_n176_));
  nand4  g114(.a(new_n176_), .b(new_n175_), .c(new_n135_), .d(new_n80_), .O(z18));
  nand2  g115(.a(x40), .b(x04), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(x35), .c(new_n64_), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(x36), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n127_), .O(new_n181_));
  nand2  g119(.a(new_n181_), .b(new_n126_), .O(new_n182_));
  aoi21  g120(.a(new_n182_), .b(new_n134_), .c(new_n136_), .O(z19));
  oai21  g121(.a(new_n65_), .b(new_n76_), .c(new_n135_), .O(new_n184_));
  oai21  g122(.a(new_n184_), .b(new_n173_), .c(new_n80_), .O(z20));
endmodule


