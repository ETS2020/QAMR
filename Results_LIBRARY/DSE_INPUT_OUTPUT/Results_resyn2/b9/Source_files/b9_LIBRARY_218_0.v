// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n98_, new_n100_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n151_, new_n153_, new_n155_,
    new_n156_, new_n160_, new_n161_;
  nor2   g00(.a(x23), .b(x22), .O(new_n63_));
  inv1   g01(.a(x15), .O(new_n64_));
  inv1   g02(.a(x27), .O(new_n65_));
  inv1   g03(.a(x37), .O(new_n66_));
  nand2  g04(.a(x35), .b(x28), .O(new_n67_));
  nand2  g05(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  inv1   g06(.a(new_n68_), .O(new_n69_));
  inv1   g07(.a(x04), .O(new_n70_));
  inv1   g08(.a(x35), .O(new_n71_));
  inv1   g09(.a(x36), .O(new_n72_));
  nand2  g10(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  and2   g11(.a(x40), .b(x39), .O(new_n74_));
  nand3  g12(.a(new_n74_), .b(new_n73_), .c(new_n70_), .O(new_n75_));
  aoi21  g13(.a(new_n75_), .b(new_n69_), .c(new_n65_), .O(new_n76_));
  nand3  g14(.a(new_n74_), .b(x10), .c(new_n70_), .O(new_n77_));
  inv1   g15(.a(new_n77_), .O(new_n78_));
  oai21  g16(.a(new_n78_), .b(new_n76_), .c(new_n64_), .O(new_n79_));
  aoi21  g17(.a(new_n79_), .b(x16), .c(new_n63_), .O(z00));
  inv1   g18(.a(x28), .O(new_n81_));
  aoi21  g19(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g20(.a(x32), .b(x30), .O(new_n83_));
  oai21  g21(.a(new_n82_), .b(new_n65_), .c(new_n83_), .O(new_n84_));
  aoi21  g22(.a(new_n84_), .b(x04), .c(new_n63_), .O(z01));
  nand2  g23(.a(x35), .b(new_n81_), .O(new_n86_));
  nor2   g24(.a(x27), .b(x08), .O(new_n87_));
  oai21  g25(.a(new_n87_), .b(new_n86_), .c(x04), .O(new_n88_));
  inv1   g26(.a(new_n63_), .O(new_n89_));
  and2   g27(.a(new_n74_), .b(new_n89_), .O(z08));
  inv1   g28(.a(x02), .O(new_n91_));
  nand2  g29(.a(x29), .b(x08), .O(new_n92_));
  nand2  g30(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g31(.a(new_n93_), .b(z08), .c(new_n88_), .O(z02));
  nand2  g32(.a(new_n67_), .b(x27), .O(new_n95_));
  nand2  g33(.a(new_n66_), .b(new_n65_), .O(new_n96_));
  nand4  g34(.a(new_n96_), .b(new_n95_), .c(new_n89_), .d(x21), .O(z03));
  aoi21  g35(.a(new_n66_), .b(new_n65_), .c(x21), .O(new_n98_));
  aoi21  g36(.a(new_n98_), .b(new_n95_), .c(new_n63_), .O(z04));
  aoi21  g37(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nor2   g38(.a(new_n100_), .b(new_n63_), .O(z05));
  inv1   g39(.a(z05), .O(z06));
  inv1   g40(.a(x14), .O(new_n103_));
  nand2  g41(.a(x17), .b(new_n64_), .O(new_n104_));
  nor2   g42(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g43(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  inv1   g44(.a(x25), .O(new_n107_));
  nand2  g45(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g46(.a(x25), .b(new_n103_), .O(new_n109_));
  nand3  g47(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g48(.a(new_n110_), .b(new_n89_), .c(x03), .O(new_n111_));
  nor2   g49(.a(new_n111_), .b(new_n106_), .O(z07));
  nand2  g50(.a(x34), .b(x27), .O(new_n113_));
  nand2  g51(.a(x26), .b(x11), .O(new_n114_));
  oai21  g52(.a(new_n114_), .b(new_n113_), .c(new_n89_), .O(z09));
  nor2   g53(.a(new_n82_), .b(new_n65_), .O(new_n116_));
  nand2  g54(.a(new_n74_), .b(x05), .O(new_n117_));
  inv1   g55(.a(new_n117_), .O(new_n118_));
  inv1   g56(.a(x07), .O(new_n119_));
  nor2   g57(.a(new_n74_), .b(new_n119_), .O(new_n120_));
  aoi22  g58(.a(new_n120_), .b(new_n116_), .c(new_n118_), .d(new_n84_), .O(new_n121_));
  nand3  g59(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  inv1   g60(.a(new_n122_), .O(new_n123_));
  nor2   g61(.a(new_n123_), .b(new_n63_), .O(new_n124_));
  oai21  g62(.a(new_n121_), .b(x04), .c(new_n124_), .O(z10));
  inv1   g63(.a(x08), .O(new_n126_));
  nand2  g64(.a(new_n74_), .b(x29), .O(new_n127_));
  aoi21  g65(.a(new_n127_), .b(new_n86_), .c(new_n126_), .O(new_n128_));
  nand2  g66(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g67(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  inv1   g68(.a(new_n86_), .O(new_n131_));
  nor2   g69(.a(new_n65_), .b(x04), .O(new_n132_));
  or2    g70(.a(x30), .b(x09), .O(new_n133_));
  aoi21  g71(.a(new_n132_), .b(new_n131_), .c(new_n133_), .O(new_n134_));
  nand2  g72(.a(new_n134_), .b(new_n130_), .O(new_n135_));
  nand2  g73(.a(new_n135_), .b(new_n89_), .O(z11));
  oai21  g74(.a(new_n78_), .b(new_n76_), .c(new_n89_), .O(z12));
  inv1   g75(.a(x13), .O(new_n138_));
  oai21  g76(.a(new_n72_), .b(new_n71_), .c(x28), .O(new_n139_));
  nand4  g77(.a(new_n139_), .b(new_n74_), .c(new_n138_), .d(new_n70_), .O(new_n140_));
  nor2   g78(.a(x19), .b(x18), .O(new_n141_));
  nand3  g79(.a(new_n141_), .b(new_n68_), .c(x20), .O(new_n142_));
  nand2  g80(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand2  g81(.a(new_n143_), .b(x27), .O(new_n144_));
  nand3  g82(.a(new_n74_), .b(new_n138_), .c(new_n70_), .O(new_n145_));
  nor2   g83(.a(new_n145_), .b(new_n83_), .O(new_n146_));
  inv1   g84(.a(new_n146_), .O(new_n147_));
  aoi21  g85(.a(new_n147_), .b(new_n144_), .c(new_n63_), .O(z13));
  aoi21  g86(.a(new_n142_), .b(new_n140_), .c(new_n65_), .O(new_n149_));
  oai21  g87(.a(new_n146_), .b(new_n149_), .c(new_n89_), .O(z14));
  nand2  g88(.a(x26), .b(x12), .O(new_n151_));
  nor3   g89(.a(new_n151_), .b(new_n113_), .c(new_n63_), .O(z15));
  inv1   g90(.a(x01), .O(new_n153_));
  aoi21  g91(.a(x22), .b(new_n153_), .c(x23), .O(z16));
  inv1   g92(.a(x24), .O(new_n155_));
  nand4  g93(.a(new_n155_), .b(x23), .c(x22), .d(x01), .O(new_n156_));
  inv1   g94(.a(new_n156_), .O(z17));
  nand3  g95(.a(new_n134_), .b(new_n130_), .c(new_n89_), .O(z18));
  inv1   g96(.a(z18), .O(z19));
  nor2   g97(.a(new_n133_), .b(new_n63_), .O(new_n160_));
  oai21  g98(.a(new_n71_), .b(new_n65_), .c(new_n160_), .O(new_n161_));
  nor2   g99(.a(new_n161_), .b(new_n128_), .O(z20));
endmodule


