// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n168_, new_n170_, new_n173_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n180_, new_n181_, new_n182_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x23), .O(new_n71_));
  inv1   g009(.a(x40), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  oai21  g012(.a(new_n69_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x36), .O(new_n77_));
  inv1   g015(.a(x28), .O(new_n78_));
  nand2  g016(.a(x35), .b(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(new_n80_));
  inv1   g018(.a(x30), .O(new_n81_));
  inv1   g019(.a(x32), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g021(.a(new_n73_), .b(new_n63_), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n80_), .c(new_n84_), .O(z01));
  nor2   g023(.a(x27), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n79_), .c(x04), .O(new_n87_));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g028(.a(x40), .b(x39), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(x23), .O(new_n92_));
  nand3  g030(.a(new_n92_), .b(new_n90_), .c(new_n87_), .O(z02));
  inv1   g031(.a(x35), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  oai22  g034(.a(new_n96_), .b(new_n94_), .c(new_n95_), .d(x27), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n73_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  inv1   g037(.a(new_n73_), .O(new_n100_));
  nand3  g038(.a(new_n97_), .b(new_n100_), .c(new_n99_), .O(z04));
  nand3  g039(.a(new_n96_), .b(new_n100_), .c(new_n95_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n100_), .O(z07));
  oai21  g052(.a(x39), .b(x23), .c(x40), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(z08));
  inv1   g054(.a(x11), .O(new_n117_));
  nand3  g055(.a(x34), .b(x27), .c(x26), .O(new_n118_));
  nor3   g056(.a(new_n118_), .b(new_n73_), .c(new_n117_), .O(z09));
  nand2  g057(.a(new_n115_), .b(x07), .O(new_n120_));
  nand4  g058(.a(x40), .b(x39), .c(new_n71_), .d(x05), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  aoi21  g060(.a(new_n79_), .b(new_n77_), .c(x04), .O(new_n123_));
  and2   g061(.a(x37), .b(x06), .O(new_n124_));
  aoi22  g062(.a(new_n124_), .b(new_n100_), .c(new_n123_), .d(new_n122_), .O(new_n125_));
  nand4  g063(.a(new_n83_), .b(new_n65_), .c(new_n71_), .d(x05), .O(new_n126_));
  oai21  g064(.a(new_n125_), .b(new_n76_), .c(new_n126_), .O(z10));
  nor2   g065(.a(x30), .b(x09), .O(new_n128_));
  inv1   g066(.a(new_n128_), .O(new_n129_));
  aoi21  g067(.a(x39), .b(x29), .c(x23), .O(new_n130_));
  nand2  g068(.a(x27), .b(x04), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x35), .c(new_n78_), .O(new_n132_));
  oai21  g070(.a(new_n130_), .b(new_n72_), .c(new_n132_), .O(new_n133_));
  inv1   g071(.a(x08), .O(new_n134_));
  oai21  g072(.a(new_n79_), .b(x04), .c(new_n71_), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x27), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n100_), .c(new_n134_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n133_), .c(new_n129_), .O(z11));
  nor2   g076(.a(new_n67_), .b(new_n76_), .O(new_n139_));
  inv1   g077(.a(x10), .O(new_n140_));
  oai21  g078(.a(new_n66_), .b(new_n76_), .c(new_n140_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(x39), .c(new_n63_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n71_), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(x40), .c(new_n139_), .O(z12));
  inv1   g082(.a(x13), .O(new_n145_));
  nand3  g083(.a(x39), .b(new_n145_), .c(new_n63_), .O(new_n146_));
  nand2  g084(.a(x36), .b(x35), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x28), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(x27), .c(new_n83_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n146_), .c(new_n71_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x40), .O(new_n151_));
  inv1   g089(.a(x18), .O(new_n152_));
  inv1   g090(.a(x19), .O(new_n153_));
  nand3  g091(.a(x20), .b(new_n153_), .c(new_n152_), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n139_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n151_), .O(z13));
  inv1   g095(.a(new_n146_), .O(new_n158_));
  nor2   g096(.a(x32), .b(x30), .O(new_n159_));
  nand3  g097(.a(new_n147_), .b(new_n159_), .c(x28), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(new_n158_), .c(x23), .O(new_n161_));
  oai21  g099(.a(new_n161_), .b(new_n72_), .c(new_n154_), .O(new_n162_));
  inv1   g100(.a(new_n139_), .O(new_n163_));
  oai21  g101(.a(x28), .b(new_n76_), .c(new_n159_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n158_), .c(x23), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n72_), .c(new_n163_), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n162_), .O(z14));
  inv1   g105(.a(x12), .O(new_n168_));
  oai21  g106(.a(new_n118_), .b(new_n168_), .c(new_n100_), .O(z15));
  nand2  g107(.a(x22), .b(x01), .O(new_n170_));
  nor2   g108(.a(new_n170_), .b(x23), .O(z16));
  nor4   g109(.a(new_n170_), .b(x40), .c(x24), .d(new_n71_), .O(z17));
  oai21  g110(.a(new_n132_), .b(new_n86_), .c(new_n128_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n100_), .O(new_n174_));
  nand3  g112(.a(new_n92_), .b(x29), .c(x08), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n174_), .O(z18));
  nand3  g114(.a(x40), .b(x39), .c(x29), .O(new_n177_));
  nor3   g115(.a(new_n177_), .b(x23), .c(new_n134_), .O(new_n178_));
  aoi21  g116(.a(new_n173_), .b(new_n100_), .c(new_n178_), .O(z19));
  aoi21  g117(.a(new_n177_), .b(new_n79_), .c(new_n134_), .O(new_n180_));
  nand2  g118(.a(x35), .b(x27), .O(new_n181_));
  nand3  g119(.a(new_n181_), .b(new_n128_), .c(new_n100_), .O(new_n182_));
  nor2   g120(.a(new_n182_), .b(new_n180_), .O(z20));
endmodule


