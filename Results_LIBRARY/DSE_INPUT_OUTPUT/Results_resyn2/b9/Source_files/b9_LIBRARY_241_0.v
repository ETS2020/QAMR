// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n103_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n170_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n178_, new_n179_, new_n182_, new_n183_;
  inv1   g000(.a(x37), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  inv1   g002(.a(x35), .O(new_n65_));
  inv1   g003(.a(x36), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g005(.a(x40), .b(x39), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  nand3  g007(.a(new_n69_), .b(new_n67_), .c(new_n64_), .O(new_n70_));
  inv1   g008(.a(x39), .O(new_n71_));
  oai21  g009(.a(new_n71_), .b(x35), .c(x28), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(new_n70_), .c(new_n63_), .O(new_n73_));
  nor2   g011(.a(new_n68_), .b(x04), .O(new_n74_));
  nand2  g012(.a(new_n74_), .b(x10), .O(new_n75_));
  inv1   g013(.a(new_n75_), .O(new_n76_));
  aoi21  g014(.a(new_n73_), .b(x27), .c(new_n76_), .O(new_n77_));
  inv1   g015(.a(x16), .O(new_n78_));
  nor2   g016(.a(x39), .b(x35), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  oai21  g018(.a(new_n77_), .b(x15), .c(new_n80_), .O(z00));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  inv1   g020(.a(x28), .O(new_n83_));
  nand2  g021(.a(x35), .b(new_n83_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n66_), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x27), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n82_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n79_), .O(z01));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n84_), .c(x04), .O(new_n90_));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n91_), .c(new_n68_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  xor2a  g032(.a(new_n94_), .b(new_n79_), .O(z02));
  oai21  g033(.a(new_n65_), .b(new_n83_), .c(x27), .O(new_n96_));
  inv1   g034(.a(x27), .O(new_n97_));
  aoi21  g035(.a(new_n63_), .b(new_n97_), .c(new_n79_), .O(new_n98_));
  nand3  g036(.a(new_n98_), .b(new_n96_), .c(x21), .O(z03));
  inv1   g037(.a(x21), .O(new_n100_));
  nand3  g038(.a(new_n98_), .b(new_n96_), .c(new_n100_), .O(z04));
  inv1   g039(.a(new_n79_), .O(new_n102_));
  nand2  g040(.a(x28), .b(x27), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n102_), .c(new_n63_), .O(z05));
  inv1   g042(.a(z05), .O(z06));
  inv1   g043(.a(x14), .O(new_n106_));
  inv1   g044(.a(x15), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n107_), .O(new_n108_));
  nor2   g046(.a(x33), .b(x31), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n108_), .c(new_n106_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  nand2  g050(.a(x25), .b(new_n106_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n102_), .c(x03), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n110_), .O(z07));
  nand2  g054(.a(new_n102_), .b(new_n68_), .O(z08));
  nand4  g055(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(new_n102_), .O(z09));
  inv1   g057(.a(x07), .O(new_n120_));
  inv1   g058(.a(x40), .O(new_n121_));
  aoi21  g059(.a(new_n121_), .b(new_n120_), .c(x04), .O(new_n122_));
  inv1   g060(.a(x05), .O(new_n123_));
  nand2  g061(.a(new_n69_), .b(new_n123_), .O(new_n124_));
  oai21  g062(.a(new_n65_), .b(new_n120_), .c(new_n71_), .O(new_n125_));
  nand4  g063(.a(new_n125_), .b(new_n124_), .c(new_n122_), .d(new_n85_), .O(new_n126_));
  nand3  g064(.a(new_n102_), .b(x37), .c(x06), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(x27), .O(new_n129_));
  inv1   g067(.a(new_n82_), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n74_), .c(x05), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(z10));
  inv1   g070(.a(x08), .O(new_n133_));
  nand3  g071(.a(x35), .b(new_n83_), .c(new_n97_), .O(new_n134_));
  nand2  g072(.a(new_n69_), .b(x29), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n134_), .c(new_n133_), .O(new_n136_));
  nand4  g074(.a(x35), .b(new_n83_), .c(x27), .d(new_n64_), .O(new_n137_));
  nor3   g075(.a(new_n79_), .b(x30), .c(x09), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nor2   g077(.a(new_n139_), .b(new_n136_), .O(z11));
  oai21  g078(.a(new_n74_), .b(x28), .c(x35), .O(new_n141_));
  nand4  g079(.a(x40), .b(x36), .c(new_n65_), .d(new_n64_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(new_n63_), .O(new_n143_));
  nand2  g081(.a(new_n102_), .b(new_n75_), .O(new_n144_));
  aoi21  g082(.a(new_n143_), .b(x27), .c(new_n144_), .O(z12));
  aoi21  g083(.a(x36), .b(x35), .c(new_n83_), .O(new_n146_));
  inv1   g084(.a(x13), .O(new_n147_));
  nand3  g085(.a(x40), .b(new_n147_), .c(new_n64_), .O(new_n148_));
  inv1   g086(.a(x18), .O(new_n149_));
  inv1   g087(.a(x19), .O(new_n150_));
  nand4  g088(.a(x37), .b(x20), .c(new_n150_), .d(new_n149_), .O(new_n151_));
  oai21  g089(.a(new_n148_), .b(new_n146_), .c(new_n151_), .O(new_n152_));
  nand3  g090(.a(x20), .b(new_n150_), .c(new_n149_), .O(new_n153_));
  oai21  g091(.a(x37), .b(x28), .c(x35), .O(new_n154_));
  nor2   g092(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  aoi21  g093(.a(new_n152_), .b(x39), .c(new_n155_), .O(new_n156_));
  nand3  g094(.a(new_n130_), .b(new_n74_), .c(new_n147_), .O(new_n157_));
  oai21  g095(.a(new_n156_), .b(new_n97_), .c(new_n157_), .O(z13));
  nor2   g096(.a(new_n154_), .b(new_n97_), .O(new_n159_));
  nand4  g097(.a(x40), .b(new_n83_), .c(new_n147_), .d(new_n64_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n63_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(x27), .O(new_n162_));
  inv1   g100(.a(new_n148_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n130_), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n162_), .c(new_n71_), .O(new_n165_));
  nand2  g103(.a(new_n146_), .b(new_n82_), .O(new_n166_));
  nand3  g104(.a(new_n166_), .b(new_n74_), .c(new_n147_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n153_), .O(new_n168_));
  oai21  g106(.a(new_n165_), .b(new_n159_), .c(new_n168_), .O(z14));
  nand4  g107(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n102_), .O(z15));
  nand2  g109(.a(x22), .b(x01), .O(new_n172_));
  nor3   g110(.a(new_n172_), .b(new_n79_), .c(x23), .O(z16));
  inv1   g111(.a(x24), .O(new_n174_));
  inv1   g112(.a(new_n172_), .O(new_n175_));
  nand4  g113(.a(new_n175_), .b(new_n102_), .c(new_n174_), .d(x23), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(z17));
  nor2   g115(.a(x30), .b(x09), .O(new_n178_));
  oai21  g116(.a(new_n178_), .b(new_n79_), .c(new_n137_), .O(new_n179_));
  nor2   g117(.a(new_n179_), .b(new_n136_), .O(z19));
  inv1   g118(.a(z19), .O(z18));
  oai21  g119(.a(x28), .b(new_n133_), .c(new_n97_), .O(new_n182_));
  oai21  g120(.a(new_n92_), .b(new_n68_), .c(new_n138_), .O(new_n183_));
  aoi21  g121(.a(new_n182_), .b(x35), .c(new_n183_), .O(z20));
endmodule


