// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n154_,
    new_n156_, new_n158_, new_n159_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_;
  nand2  g000(.a(x40), .b(x13), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x13), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g015(.a(x15), .b(x04), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n77_), .c(new_n74_), .d(new_n71_), .O(new_n79_));
  oai21  g017(.a(new_n70_), .b(new_n64_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n67_), .c(new_n83_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g023(.a(x04), .O(new_n86_));
  inv1   g024(.a(x08), .O(new_n87_));
  nand2  g025(.a(x35), .b(new_n81_), .O(new_n88_));
  aoi21  g026(.a(new_n67_), .b(new_n87_), .c(new_n88_), .O(new_n89_));
  nand2  g027(.a(new_n77_), .b(new_n71_), .O(new_n90_));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g030(.a(new_n89_), .b(new_n86_), .c(new_n92_), .O(z02));
  inv1   g031(.a(x35), .O(new_n94_));
  oai21  g032(.a(new_n94_), .b(new_n81_), .c(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n67_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n95_), .c(new_n63_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n96_), .b(new_n67_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n64_), .O(z04));
  nand2  g038(.a(x28), .b(x27), .O(new_n101_));
  aoi21  g039(.a(new_n101_), .b(new_n96_), .c(new_n64_), .O(z05));
  inv1   g040(.a(z05), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n65_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n104_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n63_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  aoi21  g051(.a(new_n75_), .b(new_n71_), .c(new_n76_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n63_), .O(z09));
  nor2   g054(.a(new_n82_), .b(new_n67_), .O(new_n117_));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  nor2   g058(.a(new_n77_), .b(new_n120_), .O(new_n121_));
  aoi22  g059(.a(new_n121_), .b(new_n117_), .c(new_n119_), .d(new_n84_), .O(new_n122_));
  nand3  g060(.a(x37), .b(x27), .c(x06), .O(new_n123_));
  and2   g061(.a(new_n123_), .b(new_n63_), .O(new_n124_));
  oai21  g062(.a(new_n122_), .b(x04), .c(new_n124_), .O(z10));
  nor2   g063(.a(x30), .b(x09), .O(new_n126_));
  inv1   g064(.a(new_n126_), .O(new_n127_));
  nand3  g065(.a(new_n126_), .b(new_n87_), .c(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n71_), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(x40), .O(new_n130_));
  nor2   g068(.a(new_n94_), .b(x28), .O(new_n131_));
  oai21  g069(.a(x27), .b(x08), .c(new_n131_), .O(new_n132_));
  nand3  g070(.a(x40), .b(x39), .c(x29), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x08), .O(new_n135_));
  nor2   g073(.a(new_n67_), .b(new_n86_), .O(new_n136_));
  aoi22  g074(.a(new_n136_), .b(new_n133_), .c(new_n135_), .d(new_n132_), .O(new_n137_));
  oai21  g075(.a(new_n137_), .b(new_n127_), .c(new_n130_), .O(z11));
  nand3  g076(.a(x40), .b(x39), .c(new_n86_), .O(new_n139_));
  oai21  g077(.a(new_n68_), .b(new_n67_), .c(new_n139_), .O(new_n140_));
  nor3   g078(.a(x37), .b(x36), .c(x35), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n67_), .c(new_n72_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n140_), .c(new_n63_), .O(z12));
  nand2  g081(.a(x39), .b(new_n86_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n83_), .c(new_n71_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x40), .O(new_n146_));
  nor2   g084(.a(x19), .b(x18), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x20), .O(new_n148_));
  aoi21  g086(.a(x36), .b(x35), .c(new_n81_), .O(new_n149_));
  oai22  g087(.a(new_n149_), .b(new_n139_), .c(new_n148_), .d(new_n68_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x27), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n146_), .O(z13));
  inv1   g090(.a(z13), .O(z14));
  nand4  g091(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n63_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(x23), .c(new_n63_), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x23), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n156_), .c(new_n63_), .O(z17));
  nand3  g098(.a(new_n134_), .b(new_n71_), .c(x08), .O(new_n161_));
  inv1   g099(.a(new_n136_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n89_), .c(new_n127_), .O(new_n163_));
  aoi21  g101(.a(new_n163_), .b(new_n161_), .c(new_n64_), .O(z18));
  aoi21  g102(.a(new_n133_), .b(new_n88_), .c(new_n87_), .O(new_n165_));
  nand2  g103(.a(new_n133_), .b(x27), .O(new_n166_));
  nand3  g104(.a(new_n131_), .b(x27), .c(new_n86_), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(new_n126_), .c(new_n63_), .O(new_n168_));
  aoi21  g106(.a(new_n166_), .b(new_n165_), .c(new_n168_), .O(z19));
  oai21  g107(.a(new_n94_), .b(new_n67_), .c(new_n126_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n165_), .c(new_n63_), .O(z20));
endmodule


