// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n159_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n168_, new_n170_;
  inv1   g000(.a(x36), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x18), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x10), .O(new_n67_));
  inv1   g005(.a(x27), .O(new_n68_));
  aoi21  g006(.a(x35), .b(x28), .c(x37), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  nand3  g008(.a(x40), .b(x39), .c(new_n70_), .O(new_n71_));
  oai22  g009(.a(new_n71_), .b(new_n67_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n65_), .c(new_n66_), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(new_n64_), .O(z08));
  nor2   g013(.a(new_n68_), .b(x04), .O(new_n76_));
  nor2   g014(.a(x36), .b(x35), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x15), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(z08), .O(new_n79_));
  oai21  g017(.a(new_n73_), .b(new_n64_), .c(new_n79_), .O(z00));
  inv1   g018(.a(new_n64_), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n63_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g025(.a(new_n87_), .b(new_n81_), .c(x04), .O(z01));
  nor2   g026(.a(x27), .b(x08), .O(new_n89_));
  oai21  g027(.a(new_n89_), .b(new_n83_), .c(x04), .O(new_n90_));
  aoi21  g028(.a(x29), .b(x08), .c(x02), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n74_), .O(new_n92_));
  aoi21  g030(.a(new_n92_), .b(new_n90_), .c(new_n64_), .O(z02));
  nand2  g031(.a(x35), .b(x28), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(new_n68_), .c(new_n64_), .O(new_n97_));
  nand3  g035(.a(new_n97_), .b(new_n95_), .c(x21), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n95_), .c(new_n99_), .O(z04));
  oai21  g038(.a(new_n82_), .b(new_n68_), .c(new_n96_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n81_), .O(z06));
  inv1   g040(.a(z06), .O(z05));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  or2    g048(.a(x33), .b(x31), .O(new_n111_));
  oai21  g049(.a(new_n111_), .b(new_n110_), .c(x14), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n108_), .c(x03), .O(new_n113_));
  nand2  g051(.a(new_n113_), .b(new_n81_), .O(z07));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n81_), .O(z09));
  or2    g054(.a(new_n74_), .b(x05), .O(new_n117_));
  inv1   g055(.a(x07), .O(new_n118_));
  nand2  g056(.a(new_n74_), .b(new_n118_), .O(new_n119_));
  nand4  g057(.a(new_n119_), .b(new_n117_), .c(new_n84_), .d(new_n76_), .O(new_n120_));
  nor2   g058(.a(new_n86_), .b(new_n71_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(x05), .c(new_n123_), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n120_), .c(new_n64_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n83_), .c(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  inv1   g068(.a(new_n83_), .O(new_n131_));
  inv1   g069(.a(x09), .O(new_n132_));
  inv1   g070(.a(x30), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  aoi21  g072(.a(new_n131_), .b(new_n76_), .c(new_n134_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n130_), .c(new_n81_), .O(z18));
  inv1   g074(.a(z18), .O(z11));
  oai21  g075(.a(new_n77_), .b(new_n71_), .c(new_n69_), .O(new_n138_));
  oai21  g076(.a(new_n71_), .b(new_n67_), .c(new_n81_), .O(new_n139_));
  aoi21  g077(.a(new_n138_), .b(x27), .c(new_n139_), .O(z12));
  aoi21  g078(.a(x36), .b(x35), .c(new_n82_), .O(new_n141_));
  inv1   g079(.a(x13), .O(new_n142_));
  nand4  g080(.a(x40), .b(x39), .c(new_n142_), .d(new_n70_), .O(new_n143_));
  nand2  g081(.a(new_n94_), .b(new_n96_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n144_), .c(x20), .O(new_n146_));
  oai21  g084(.a(new_n143_), .b(new_n141_), .c(new_n146_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  nand2  g086(.a(new_n121_), .b(new_n142_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n81_), .O(z13));
  inv1   g088(.a(new_n143_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n82_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n146_), .c(new_n68_), .O(new_n153_));
  nand2  g091(.a(x35), .b(x27), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n143_), .c(x18), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x36), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n149_), .O(new_n157_));
  nor2   g095(.a(new_n157_), .b(new_n153_), .O(z14));
  nand4  g096(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n81_), .O(z15));
  inv1   g098(.a(x01), .O(new_n161_));
  inv1   g099(.a(x22), .O(new_n162_));
  nor4   g100(.a(new_n64_), .b(x23), .c(new_n162_), .d(new_n161_), .O(z16));
  inv1   g101(.a(x24), .O(new_n164_));
  nor2   g102(.a(new_n162_), .b(new_n161_), .O(new_n165_));
  nand4  g103(.a(new_n165_), .b(new_n81_), .c(new_n164_), .d(x23), .O(new_n166_));
  inv1   g104(.a(new_n166_), .O(z17));
  nand2  g105(.a(new_n135_), .b(new_n130_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n81_), .O(z19));
  nand3  g107(.a(new_n154_), .b(new_n133_), .c(new_n132_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n128_), .c(new_n81_), .O(z20));
endmodule


