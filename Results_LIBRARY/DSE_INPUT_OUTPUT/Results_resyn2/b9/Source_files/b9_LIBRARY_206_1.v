// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n153_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n160_, new_n163_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand2  g003(.a(x40), .b(x39), .O(new_n66_));
  inv1   g004(.a(new_n66_), .O(new_n67_));
  nand2  g005(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  oai21  g006(.a(new_n68_), .b(new_n64_), .c(new_n63_), .O(new_n69_));
  inv1   g007(.a(x21), .O(new_n70_));
  nand3  g008(.a(new_n67_), .b(x10), .c(new_n65_), .O(new_n71_));
  nand2  g009(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g010(.a(new_n69_), .b(x27), .c(new_n72_), .O(new_n73_));
  oai21  g011(.a(new_n73_), .b(x15), .c(x16), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g020(.a(new_n70_), .b(x15), .O(new_n83_));
  nor2   g021(.a(new_n83_), .b(new_n65_), .O(new_n84_));
  oai21  g022(.a(new_n82_), .b(new_n79_), .c(new_n84_), .O(z01));
  inv1   g023(.a(new_n83_), .O(new_n86_));
  nor2   g024(.a(x27), .b(x08), .O(new_n87_));
  oai21  g025(.a(new_n87_), .b(new_n78_), .c(x04), .O(new_n88_));
  inv1   g026(.a(x02), .O(new_n89_));
  nand2  g027(.a(x29), .b(x08), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand4  g029(.a(new_n91_), .b(new_n88_), .c(new_n86_), .d(new_n67_), .O(z02));
  inv1   g030(.a(x15), .O(new_n93_));
  inv1   g031(.a(x35), .O(new_n94_));
  inv1   g032(.a(x37), .O(new_n95_));
  nand2  g033(.a(x28), .b(x27), .O(new_n96_));
  oai22  g034(.a(new_n96_), .b(new_n94_), .c(new_n95_), .d(x27), .O(new_n97_));
  oai21  g035(.a(new_n97_), .b(new_n93_), .c(x21), .O(z03));
  aoi21  g036(.a(new_n97_), .b(new_n70_), .c(new_n83_), .O(z04));
  aoi21  g037(.a(new_n96_), .b(new_n95_), .c(new_n83_), .O(z05));
  nand2  g038(.a(new_n96_), .b(new_n95_), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n83_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n86_), .O(z07));
  nand2  g051(.a(new_n86_), .b(new_n66_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  nor3   g054(.a(new_n116_), .b(new_n83_), .c(new_n115_), .O(z09));
  nand2  g055(.a(new_n78_), .b(new_n76_), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  nand2  g057(.a(new_n66_), .b(x07), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand3  g059(.a(new_n121_), .b(new_n118_), .c(x27), .O(new_n122_));
  inv1   g060(.a(new_n119_), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n82_), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n65_), .O(new_n126_));
  nand3  g064(.a(x37), .b(x27), .c(x06), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n126_), .c(new_n83_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(new_n67_), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n78_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand4  g071(.a(x35), .b(new_n77_), .c(x27), .d(new_n65_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n134_), .c(new_n133_), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n86_), .O(z11));
  nand2  g075(.a(new_n86_), .b(new_n71_), .O(new_n138_));
  aoi21  g076(.a(new_n69_), .b(x27), .c(new_n138_), .O(z12));
  oai21  g077(.a(new_n76_), .b(new_n94_), .c(x28), .O(new_n140_));
  nor2   g078(.a(x13), .b(x04), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(new_n67_), .O(new_n142_));
  inv1   g080(.a(new_n63_), .O(new_n143_));
  nor2   g081(.a(x19), .b(x18), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n143_), .c(x20), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x27), .O(new_n147_));
  nand3  g085(.a(new_n141_), .b(new_n82_), .c(new_n67_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n147_), .c(new_n83_), .O(z13));
  aoi21  g087(.a(new_n145_), .b(new_n142_), .c(new_n75_), .O(new_n150_));
  inv1   g088(.a(new_n148_), .O(new_n151_));
  nor3   g089(.a(new_n151_), .b(new_n150_), .c(new_n83_), .O(z14));
  inv1   g090(.a(x12), .O(new_n153_));
  oai21  g091(.a(new_n116_), .b(new_n153_), .c(new_n86_), .O(z15));
  inv1   g092(.a(x01), .O(new_n155_));
  inv1   g093(.a(x22), .O(new_n156_));
  nor4   g094(.a(new_n83_), .b(x23), .c(new_n156_), .d(new_n155_), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  nor2   g096(.a(new_n156_), .b(new_n155_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n86_), .c(new_n158_), .d(x23), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z17));
  nand4  g099(.a(new_n135_), .b(new_n134_), .c(new_n133_), .d(new_n86_), .O(z18));
  oai21  g100(.a(new_n94_), .b(new_n75_), .c(new_n135_), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(new_n131_), .c(new_n86_), .O(z20));
  nand2  g102(.a(new_n136_), .b(new_n86_), .O(z19));
endmodule


