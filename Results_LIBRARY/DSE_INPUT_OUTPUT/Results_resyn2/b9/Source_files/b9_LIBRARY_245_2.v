// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n154_,
    new_n156_, new_n158_, new_n159_, new_n160_, new_n162_, new_n163_;
  inv1   g000(.a(x33), .O(new_n63_));
  nor2   g001(.a(x36), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  inv1   g006(.a(x04), .O(new_n69_));
  nand4  g007(.a(x40), .b(x39), .c(x10), .d(new_n69_), .O(new_n70_));
  oai21  g008(.a(new_n68_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n65_), .c(new_n66_), .O(new_n72_));
  nand2  g010(.a(x40), .b(x39), .O(new_n73_));
  inv1   g011(.a(new_n73_), .O(new_n74_));
  inv1   g012(.a(x35), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  oai21  g014(.a(new_n75_), .b(x33), .c(new_n76_), .O(new_n77_));
  nor2   g015(.a(new_n67_), .b(x04), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n77_), .c(new_n74_), .d(new_n65_), .O(new_n79_));
  oai21  g017(.a(new_n72_), .b(new_n64_), .c(new_n79_), .O(z00));
  inv1   g018(.a(new_n64_), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  aoi21  g020(.a(x35), .b(new_n82_), .c(x36), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n67_), .c(new_n84_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n81_), .c(x04), .O(z01));
  nand2  g024(.a(x35), .b(new_n82_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(new_n74_), .d(new_n81_), .O(z02));
  oai21  g031(.a(new_n75_), .b(new_n82_), .c(x27), .O(new_n94_));
  nor2   g032(.a(x37), .b(x27), .O(new_n95_));
  nor2   g033(.a(new_n95_), .b(new_n64_), .O(new_n96_));
  nand3  g034(.a(new_n96_), .b(new_n94_), .c(x21), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n94_), .c(new_n98_), .O(z04));
  aoi21  g037(.a(x28), .b(x27), .c(x37), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n81_), .O(z05));
  inv1   g039(.a(new_n100_), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(new_n81_), .O(z06));
  inv1   g041(.a(x25), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(x00), .O(new_n105_));
  inv1   g043(.a(x14), .O(new_n106_));
  nand2  g044(.a(x25), .b(new_n106_), .O(new_n107_));
  nand3  g045(.a(new_n107_), .b(new_n105_), .c(x38), .O(new_n108_));
  inv1   g046(.a(x17), .O(new_n109_));
  nor2   g047(.a(new_n109_), .b(x15), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(x31), .c(x14), .O(new_n111_));
  oai21  g049(.a(new_n76_), .b(x14), .c(x33), .O(new_n112_));
  nand4  g050(.a(new_n112_), .b(new_n111_), .c(new_n108_), .d(x03), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z07));
  nand2  g052(.a(new_n73_), .b(new_n81_), .O(z08));
  and2   g053(.a(x34), .b(x27), .O(new_n116_));
  nand3  g054(.a(new_n116_), .b(x26), .c(x11), .O(new_n117_));
  nand2  g055(.a(new_n117_), .b(new_n81_), .O(z09));
  oai21  g056(.a(new_n83_), .b(new_n67_), .c(new_n73_), .O(new_n119_));
  inv1   g057(.a(x07), .O(new_n120_));
  oai21  g058(.a(new_n73_), .b(x05), .c(new_n69_), .O(new_n121_));
  aoi21  g059(.a(new_n73_), .b(new_n120_), .c(new_n121_), .O(new_n122_));
  nand3  g060(.a(new_n122_), .b(new_n119_), .c(new_n85_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n64_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n87_), .c(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  inv1   g068(.a(new_n87_), .O(new_n131_));
  or2    g069(.a(x30), .b(x09), .O(new_n132_));
  aoi21  g070(.a(new_n131_), .b(new_n78_), .c(new_n132_), .O(new_n133_));
  aoi21  g071(.a(new_n133_), .b(new_n130_), .c(new_n64_), .O(z18));
  inv1   g072(.a(z18), .O(z11));
  nor2   g073(.a(new_n73_), .b(x04), .O(new_n136_));
  oai21  g074(.a(x36), .b(x35), .c(new_n136_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n68_), .O(new_n138_));
  nand2  g076(.a(new_n70_), .b(new_n81_), .O(new_n139_));
  aoi21  g077(.a(new_n138_), .b(x27), .c(new_n139_), .O(z12));
  inv1   g078(.a(new_n68_), .O(new_n141_));
  nor2   g079(.a(x19), .b(x18), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(x20), .O(new_n143_));
  inv1   g081(.a(x13), .O(new_n144_));
  oai21  g082(.a(new_n76_), .b(new_n75_), .c(x28), .O(new_n145_));
  nand3  g083(.a(new_n145_), .b(new_n136_), .c(new_n144_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n143_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  nor2   g086(.a(new_n84_), .b(x13), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n136_), .O(new_n150_));
  nand3  g088(.a(new_n150_), .b(new_n148_), .c(new_n81_), .O(z13));
  nand2  g089(.a(new_n150_), .b(new_n81_), .O(new_n152_));
  aoi21  g090(.a(new_n147_), .b(x27), .c(new_n152_), .O(z14));
  nand4  g091(.a(new_n116_), .b(new_n81_), .c(x26), .d(x12), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(x23), .c(new_n81_), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  inv1   g096(.a(new_n156_), .O(new_n159_));
  nand4  g097(.a(new_n159_), .b(new_n81_), .c(new_n158_), .d(x23), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(z17));
  nor2   g099(.a(new_n132_), .b(new_n64_), .O(new_n162_));
  oai21  g100(.a(new_n75_), .b(new_n67_), .c(new_n162_), .O(new_n163_));
  nor2   g101(.a(new_n163_), .b(new_n128_), .O(z20));
  inv1   g102(.a(z18), .O(z19));
endmodule


