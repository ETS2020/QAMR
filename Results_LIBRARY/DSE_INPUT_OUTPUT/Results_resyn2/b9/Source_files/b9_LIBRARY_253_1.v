// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n156_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n167_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x35), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(x25), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(new_n72_), .O(new_n75_));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(new_n76_), .c(new_n79_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n75_), .c(x04), .O(z01));
  nand2  g019(.a(x35), .b(new_n77_), .O(new_n82_));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  nor2   g023(.a(new_n72_), .b(new_n85_), .O(z08));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(z08), .c(new_n84_), .O(z02));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n71_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  nand3  g033(.a(new_n93_), .b(new_n75_), .c(new_n95_), .O(z04));
  aoi21  g034(.a(new_n92_), .b(new_n91_), .c(new_n72_), .O(z05));
  nand2  g035(.a(new_n92_), .b(new_n91_), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n72_), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n75_), .O(z07));
  nor2   g048(.a(new_n72_), .b(new_n76_), .O(new_n111_));
  nand4  g049(.a(new_n111_), .b(x34), .c(x26), .d(x11), .O(new_n112_));
  inv1   g050(.a(new_n112_), .O(z09));
  nor2   g051(.a(new_n78_), .b(new_n76_), .O(new_n114_));
  nand3  g052(.a(x40), .b(x39), .c(x05), .O(new_n115_));
  inv1   g053(.a(new_n115_), .O(new_n116_));
  and2   g054(.a(new_n85_), .b(x07), .O(new_n117_));
  aoi22  g055(.a(new_n117_), .b(new_n114_), .c(new_n116_), .d(new_n80_), .O(new_n118_));
  nand3  g056(.a(x37), .b(x27), .c(x06), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  nor2   g058(.a(new_n120_), .b(new_n72_), .O(new_n121_));
  oai21  g059(.a(new_n118_), .b(x04), .c(new_n121_), .O(z10));
  nor2   g060(.a(x30), .b(x09), .O(new_n123_));
  inv1   g061(.a(x08), .O(new_n124_));
  nand3  g062(.a(new_n77_), .b(x27), .c(new_n63_), .O(new_n125_));
  oai21  g063(.a(new_n125_), .b(new_n71_), .c(new_n124_), .O(new_n126_));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n82_), .O(new_n128_));
  nand3  g066(.a(new_n127_), .b(x27), .c(x04), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n126_), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n123_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n75_), .O(z11));
  nand2  g070(.a(new_n75_), .b(new_n69_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z12));
  inv1   g072(.a(new_n66_), .O(new_n135_));
  nor2   g073(.a(x19), .b(x18), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x20), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n111_), .c(new_n135_), .O(new_n139_));
  inv1   g077(.a(new_n79_), .O(new_n140_));
  nand2  g078(.a(x36), .b(x35), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(x28), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(x27), .c(new_n140_), .O(new_n143_));
  inv1   g081(.a(x13), .O(new_n144_));
  nand3  g082(.a(new_n75_), .b(new_n65_), .c(new_n144_), .O(new_n145_));
  oai21  g083(.a(new_n145_), .b(new_n143_), .c(new_n139_), .O(z13));
  nand2  g084(.a(new_n65_), .b(new_n144_), .O(new_n147_));
  nand2  g085(.a(new_n79_), .b(new_n76_), .O(new_n148_));
  aoi22  g086(.a(new_n148_), .b(x35), .c(new_n140_), .d(x37), .O(new_n149_));
  nand2  g087(.a(new_n135_), .b(x27), .O(new_n150_));
  oai21  g088(.a(new_n149_), .b(new_n147_), .c(new_n150_), .O(new_n151_));
  oai21  g089(.a(new_n147_), .b(new_n143_), .c(new_n75_), .O(new_n152_));
  aoi21  g090(.a(new_n151_), .b(new_n138_), .c(new_n152_), .O(z14));
  nand4  g091(.a(new_n111_), .b(x34), .c(x26), .d(x12), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(z15));
  nand2  g093(.a(x22), .b(x01), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(x23), .c(new_n75_), .O(z16));
  inv1   g095(.a(x24), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x23), .O(new_n159_));
  oai21  g097(.a(new_n159_), .b(new_n156_), .c(new_n75_), .O(z17));
  aoi21  g098(.a(new_n127_), .b(new_n82_), .c(new_n124_), .O(new_n161_));
  nand2  g099(.a(new_n127_), .b(x27), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n161_), .O(new_n163_));
  nand2  g101(.a(new_n125_), .b(x25), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x35), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(new_n163_), .c(new_n123_), .O(z18));
  oai21  g104(.a(new_n71_), .b(new_n76_), .c(new_n123_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n161_), .c(new_n75_), .O(z20));
  nand2  g106(.a(new_n131_), .b(new_n75_), .O(z19));
endmodule


