// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n160_,
    new_n161_, new_n162_, new_n164_, new_n166_, new_n168_, new_n169_,
    new_n170_, new_n172_, new_n174_, new_n175_;
  inv1   g000(.a(x33), .O(new_n63_));
  inv1   g001(.a(x39), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x16), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n67_), .c(new_n68_), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x04), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(new_n67_), .O(new_n79_));
  oai21  g017(.a(new_n73_), .b(new_n66_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x35), .O(new_n81_));
  inv1   g019(.a(x36), .O(new_n82_));
  oai21  g020(.a(new_n81_), .b(x28), .c(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(x27), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g024(.a(new_n86_), .b(x04), .c(new_n66_), .O(z01));
  inv1   g025(.a(x28), .O(new_n88_));
  nand2  g026(.a(x35), .b(new_n88_), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  oai21  g028(.a(new_n90_), .b(new_n89_), .c(x04), .O(new_n91_));
  aoi21  g029(.a(x29), .b(x08), .c(x02), .O(new_n92_));
  nor2   g030(.a(new_n92_), .b(new_n77_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  xor2a  g032(.a(new_n94_), .b(new_n66_), .O(z02));
  nand3  g033(.a(x35), .b(x28), .c(x27), .O(new_n96_));
  oai21  g034(.a(new_n70_), .b(x27), .c(new_n96_), .O(new_n97_));
  aoi21  g035(.a(new_n97_), .b(x21), .c(new_n66_), .O(z03));
  inv1   g036(.a(x21), .O(new_n99_));
  nand3  g037(.a(new_n97_), .b(new_n65_), .c(new_n99_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n65_), .O(z05));
  inv1   g040(.a(new_n101_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n65_), .O(z06));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  inv1   g049(.a(x31), .O(new_n112_));
  nand2  g050(.a(new_n63_), .b(new_n112_), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n111_), .c(x14), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n109_), .c(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n65_), .O(z07));
  inv1   g054(.a(new_n77_), .O(z08));
  nand2  g055(.a(x34), .b(x27), .O(new_n118_));
  nand2  g056(.a(x26), .b(x11), .O(new_n119_));
  oai21  g057(.a(new_n119_), .b(new_n118_), .c(new_n65_), .O(z09));
  inv1   g058(.a(new_n85_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(z08), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n77_), .b(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n83_), .c(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  aoi21  g065(.a(z08), .b(new_n127_), .c(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  and2   g068(.a(new_n130_), .b(new_n65_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n129_), .O(z10));
  inv1   g070(.a(x08), .O(new_n133_));
  nand3  g071(.a(x40), .b(x39), .c(x29), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(new_n89_), .c(new_n133_), .O(new_n135_));
  nand2  g073(.a(new_n134_), .b(x27), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  inv1   g075(.a(new_n89_), .O(new_n138_));
  inv1   g076(.a(x09), .O(new_n139_));
  inv1   g077(.a(x30), .O(new_n140_));
  nand2  g078(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nor2   g079(.a(new_n69_), .b(x04), .O(new_n142_));
  aoi21  g080(.a(new_n142_), .b(new_n138_), .c(new_n141_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(new_n137_), .c(new_n65_), .O(z18));
  inv1   g082(.a(z18), .O(z11));
  nor3   g083(.a(x37), .b(x36), .c(x35), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n69_), .c(new_n74_), .O(new_n147_));
  or2    g085(.a(new_n78_), .b(new_n72_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n147_), .c(new_n65_), .O(z12));
  inv1   g087(.a(x13), .O(new_n150_));
  oai21  g088(.a(new_n82_), .b(new_n81_), .c(x28), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n78_), .c(new_n150_), .O(new_n152_));
  nand2  g090(.a(new_n71_), .b(new_n70_), .O(new_n153_));
  nor2   g091(.a(x19), .b(x18), .O(new_n154_));
  nand4  g092(.a(new_n154_), .b(new_n153_), .c(new_n65_), .d(x20), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(x27), .O(new_n157_));
  nand3  g095(.a(new_n121_), .b(new_n78_), .c(new_n150_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n157_), .O(z13));
  nand3  g097(.a(new_n154_), .b(new_n153_), .c(x20), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n152_), .O(new_n161_));
  nand2  g099(.a(new_n158_), .b(new_n65_), .O(new_n162_));
  aoi21  g100(.a(new_n161_), .b(x27), .c(new_n162_), .O(z14));
  nand3  g101(.a(new_n65_), .b(x26), .c(x12), .O(new_n164_));
  nor2   g102(.a(new_n164_), .b(new_n118_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  nor3   g104(.a(new_n166_), .b(new_n66_), .c(x23), .O(z16));
  inv1   g105(.a(x23), .O(new_n168_));
  nor2   g106(.a(x24), .b(new_n168_), .O(new_n169_));
  nand4  g107(.a(new_n169_), .b(new_n65_), .c(x22), .d(x01), .O(new_n170_));
  inv1   g108(.a(new_n170_), .O(z17));
  nand2  g109(.a(new_n143_), .b(new_n137_), .O(new_n172_));
  nand2  g110(.a(new_n172_), .b(new_n65_), .O(z19));
  nand2  g111(.a(x35), .b(x27), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n140_), .c(new_n139_), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n135_), .c(new_n65_), .O(z20));
endmodule


