// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n163_, new_n165_, new_n166_, new_n168_, new_n170_;
  nand2  g000(.a(x40), .b(x03), .O(new_n63_));
  inv1   g001(.a(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  aoi21  g005(.a(x35), .b(x28), .c(x37), .O(new_n68_));
  nor2   g006(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n65_), .c(new_n66_), .O(new_n70_));
  inv1   g008(.a(x03), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(new_n74_), .c(new_n65_), .d(new_n71_), .O(new_n77_));
  oai21  g015(.a(new_n70_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x28), .O(new_n79_));
  aoi21  g017(.a(x35), .b(new_n79_), .c(x36), .O(new_n80_));
  nor2   g018(.a(x32), .b(x30), .O(new_n81_));
  oai21  g019(.a(new_n80_), .b(new_n67_), .c(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(x04), .c(new_n64_), .O(z01));
  nand2  g021(.a(x35), .b(new_n79_), .O(new_n84_));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n84_), .c(x04), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nand3  g025(.a(x40), .b(x39), .c(new_n71_), .O(new_n88_));
  nor2   g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n86_), .O(z02));
  inv1   g028(.a(x35), .O(new_n91_));
  oai21  g029(.a(new_n91_), .b(new_n79_), .c(x27), .O(new_n92_));
  nor2   g030(.a(x37), .b(x27), .O(new_n93_));
  nor2   g031(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n64_), .O(z05));
  nand2  g037(.a(new_n98_), .b(new_n63_), .O(new_n100_));
  inv1   g038(.a(new_n100_), .O(z06));
  inv1   g039(.a(x40), .O(new_n102_));
  inv1   g040(.a(x17), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(x15), .O(new_n104_));
  or2    g042(.a(x33), .b(x31), .O(new_n105_));
  oai21  g043(.a(new_n105_), .b(new_n104_), .c(x14), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  inv1   g046(.a(x14), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n109_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n108_), .c(x38), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n106_), .O(new_n112_));
  aoi21  g050(.a(new_n112_), .b(new_n102_), .c(new_n71_), .O(z07));
  inv1   g051(.a(new_n88_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand4  g053(.a(new_n63_), .b(x34), .c(x27), .d(x26), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(new_n115_), .O(z09));
  nor2   g055(.a(new_n80_), .b(new_n67_), .O(new_n118_));
  inv1   g056(.a(new_n75_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(x05), .O(new_n120_));
  inv1   g058(.a(new_n120_), .O(new_n121_));
  and2   g059(.a(new_n75_), .b(x07), .O(new_n122_));
  aoi22  g060(.a(new_n122_), .b(new_n118_), .c(new_n121_), .d(new_n82_), .O(new_n123_));
  nand3  g061(.a(x37), .b(x27), .c(x06), .O(new_n124_));
  and2   g062(.a(new_n124_), .b(new_n63_), .O(new_n125_));
  oai21  g063(.a(new_n123_), .b(x04), .c(new_n125_), .O(z10));
  inv1   g064(.a(x08), .O(new_n127_));
  nand2  g065(.a(new_n119_), .b(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n84_), .c(new_n127_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g069(.a(x04), .O(new_n132_));
  nand4  g070(.a(x35), .b(new_n79_), .c(x27), .d(new_n132_), .O(new_n133_));
  nor2   g071(.a(x30), .b(x09), .O(new_n134_));
  and2   g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g073(.a(new_n135_), .b(new_n131_), .c(new_n63_), .O(z18));
  inv1   g074(.a(z18), .O(z11));
  nor3   g075(.a(x37), .b(x36), .c(x35), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n67_), .c(new_n72_), .O(new_n139_));
  or2    g077(.a(new_n76_), .b(new_n69_), .O(new_n140_));
  nand3  g078(.a(new_n140_), .b(new_n139_), .c(new_n63_), .O(z12));
  nor2   g079(.a(x19), .b(x18), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x20), .O(new_n143_));
  aoi21  g081(.a(x36), .b(x35), .c(new_n79_), .O(new_n144_));
  inv1   g082(.a(x13), .O(new_n145_));
  nand3  g083(.a(new_n119_), .b(new_n145_), .c(new_n132_), .O(new_n146_));
  oai22  g084(.a(new_n146_), .b(new_n144_), .c(new_n143_), .d(new_n68_), .O(new_n147_));
  nand2  g085(.a(new_n147_), .b(x27), .O(new_n148_));
  nand3  g086(.a(x39), .b(new_n145_), .c(new_n132_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n81_), .c(new_n71_), .O(new_n150_));
  nand2  g088(.a(new_n150_), .b(x40), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n148_), .O(z13));
  inv1   g090(.a(new_n149_), .O(new_n153_));
  nand2  g091(.a(new_n144_), .b(new_n81_), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n153_), .c(x03), .O(new_n155_));
  oai21  g093(.a(new_n155_), .b(new_n102_), .c(new_n143_), .O(new_n156_));
  oai21  g094(.a(x28), .b(new_n67_), .c(new_n81_), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(new_n153_), .c(x03), .O(new_n158_));
  oai22  g096(.a(new_n158_), .b(new_n102_), .c(new_n68_), .d(new_n67_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n156_), .O(z14));
  inv1   g098(.a(x12), .O(new_n161_));
  nor2   g099(.a(new_n116_), .b(new_n161_), .O(z15));
  nand2  g100(.a(x22), .b(x01), .O(new_n163_));
  oai21  g101(.a(new_n163_), .b(x23), .c(new_n63_), .O(z16));
  inv1   g102(.a(x24), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(x23), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n163_), .c(new_n63_), .O(z17));
  nand2  g105(.a(new_n135_), .b(new_n131_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n63_), .O(z19));
  oai21  g107(.a(new_n91_), .b(new_n67_), .c(new_n134_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n129_), .c(new_n63_), .O(z20));
endmodule


