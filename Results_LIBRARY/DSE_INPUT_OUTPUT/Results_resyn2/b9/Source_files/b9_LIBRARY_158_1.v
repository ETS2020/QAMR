// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n104_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n170_, new_n172_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n181_, new_n182_,
    new_n183_, new_n184_;
  inv1   g000(.a(x39), .O(new_n63_));
  nor2   g001(.a(new_n63_), .b(x14), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g012(.a(x14), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x15), .O(new_n78_));
  nand3  g016(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  oai21  g017(.a(new_n71_), .b(new_n64_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x27), .O(new_n85_));
  nor2   g023(.a(x32), .b(x30), .O(new_n86_));
  nand2  g024(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  aoi21  g025(.a(new_n87_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g026(.a(x04), .O(new_n89_));
  nor2   g027(.a(x27), .b(x08), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n83_), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  nor2   g031(.a(new_n63_), .b(new_n75_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(x40), .O(new_n95_));
  aoi21  g033(.a(new_n93_), .b(new_n92_), .c(new_n95_), .O(new_n96_));
  oai21  g034(.a(new_n91_), .b(new_n89_), .c(new_n96_), .O(z02));
  nand2  g035(.a(x39), .b(new_n75_), .O(new_n98_));
  nand2  g036(.a(new_n69_), .b(x27), .O(new_n99_));
  nand2  g037(.a(new_n68_), .b(new_n67_), .O(new_n100_));
  nand4  g038(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(x21), .O(z03));
  aoi21  g039(.a(new_n68_), .b(new_n67_), .c(x21), .O(new_n102_));
  aoi21  g040(.a(new_n102_), .b(new_n99_), .c(new_n64_), .O(z04));
  nand2  g041(.a(x28), .b(x27), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n98_), .c(new_n68_), .O(z05));
  inv1   g043(.a(z05), .O(z06));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n75_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  inv1   g048(.a(x17), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(x15), .O(new_n112_));
  or2    g050(.a(x33), .b(x31), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n112_), .c(x14), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n110_), .c(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n98_), .O(z07));
  inv1   g054(.a(new_n95_), .O(z08));
  and2   g055(.a(x34), .b(x27), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(x26), .c(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n98_), .O(z09));
  nor2   g058(.a(new_n86_), .b(new_n77_), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n77_), .b(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n84_), .c(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nor2   g064(.a(new_n77_), .b(x05), .O(new_n127_));
  nor2   g065(.a(new_n127_), .b(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(new_n129_), .c(new_n98_), .O(z10));
  inv1   g069(.a(x40), .O(new_n132_));
  oai21  g070(.a(new_n93_), .b(new_n132_), .c(x14), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x39), .O(new_n134_));
  or2    g072(.a(x30), .b(x09), .O(new_n135_));
  nand2  g073(.a(x27), .b(x04), .O(new_n136_));
  aoi21  g074(.a(new_n136_), .b(new_n91_), .c(new_n135_), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  inv1   g076(.a(new_n138_), .O(z11));
  inv1   g077(.a(x35), .O(new_n140_));
  nand3  g078(.a(new_n68_), .b(new_n81_), .c(new_n140_), .O(new_n141_));
  aoi21  g079(.a(new_n141_), .b(x27), .c(x10), .O(new_n142_));
  aoi21  g080(.a(x40), .b(new_n89_), .c(new_n70_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n142_), .c(x14), .O(new_n144_));
  inv1   g082(.a(new_n70_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n63_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n144_), .O(z12));
  nand2  g085(.a(new_n69_), .b(new_n68_), .O(new_n148_));
  inv1   g086(.a(x18), .O(new_n149_));
  inv1   g087(.a(x19), .O(new_n150_));
  nand3  g088(.a(x20), .b(new_n150_), .c(new_n149_), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n148_), .c(new_n98_), .O(new_n153_));
  nand2  g091(.a(x36), .b(x35), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(x28), .O(new_n155_));
  inv1   g093(.a(x13), .O(new_n156_));
  nand3  g094(.a(x40), .b(new_n156_), .c(new_n89_), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n155_), .c(new_n94_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n153_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x27), .O(new_n161_));
  nand3  g099(.a(new_n121_), .b(new_n76_), .c(new_n156_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n161_), .O(z13));
  nand3  g101(.a(new_n154_), .b(new_n86_), .c(x28), .O(new_n164_));
  aoi21  g102(.a(new_n164_), .b(new_n158_), .c(new_n75_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n63_), .c(new_n151_), .O(new_n166_));
  nand2  g104(.a(new_n82_), .b(x27), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n86_), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n158_), .c(new_n75_), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n63_), .c(new_n145_), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n166_), .O(z14));
  nand4  g109(.a(new_n118_), .b(new_n98_), .c(x26), .d(x12), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(z15));
  nand2  g111(.a(x22), .b(x01), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(x23), .c(new_n98_), .O(z16));
  inv1   g113(.a(x24), .O(new_n176_));
  inv1   g114(.a(new_n174_), .O(new_n177_));
  nand4  g115(.a(new_n177_), .b(new_n98_), .c(new_n176_), .d(x23), .O(new_n178_));
  inv1   g116(.a(new_n178_), .O(z17));
  oai22  g117(.a(new_n137_), .b(new_n64_), .c(new_n95_), .d(new_n93_), .O(z18));
  inv1   g118(.a(x29), .O(new_n181_));
  oai21  g119(.a(new_n77_), .b(new_n181_), .c(new_n83_), .O(new_n182_));
  nor2   g120(.a(new_n135_), .b(new_n64_), .O(new_n183_));
  oai21  g121(.a(new_n140_), .b(new_n67_), .c(new_n183_), .O(new_n184_));
  aoi21  g122(.a(new_n182_), .b(x08), .c(new_n184_), .O(z20));
  inv1   g123(.a(new_n138_), .O(z19));
endmodule


