// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n165_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n175_,
    new_n176_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  nor2   g005(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n64_), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(x04), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  oai21  g009(.a(x36), .b(x35), .c(x27), .O(new_n72_));
  nand2  g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g011(.a(x39), .O(new_n74_));
  inv1   g012(.a(x40), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand4  g014(.a(new_n76_), .b(new_n73_), .c(new_n64_), .d(new_n70_), .O(new_n77_));
  oai21  g015(.a(new_n69_), .b(new_n63_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x35), .O(new_n79_));
  nor2   g017(.a(new_n79_), .b(x28), .O(new_n80_));
  oai21  g018(.a(new_n80_), .b(x36), .c(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g022(.a(x08), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  aoi21  g025(.a(new_n66_), .b(new_n85_), .c(new_n87_), .O(new_n88_));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  nor3   g027(.a(new_n89_), .b(new_n75_), .c(new_n74_), .O(new_n90_));
  oai21  g028(.a(new_n88_), .b(new_n70_), .c(new_n90_), .O(z02));
  inv1   g029(.a(x37), .O(new_n92_));
  nand3  g030(.a(x35), .b(x28), .c(x27), .O(new_n93_));
  oai21  g031(.a(new_n92_), .b(x27), .c(new_n93_), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(new_n63_), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  aoi21  g034(.a(new_n94_), .b(new_n96_), .c(new_n63_), .O(z04));
  aoi21  g035(.a(x28), .b(x27), .c(x37), .O(new_n98_));
  nor2   g036(.a(new_n98_), .b(new_n63_), .O(z05));
  inv1   g037(.a(new_n98_), .O(new_n100_));
  nor2   g038(.a(new_n100_), .b(new_n63_), .O(z06));
  inv1   g039(.a(new_n63_), .O(new_n102_));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n102_), .O(z07));
  xnor2a g051(.a(x40), .b(x39), .O(z08));
  nand2  g052(.a(x34), .b(x27), .O(new_n115_));
  nand2  g053(.a(x26), .b(x11), .O(new_n116_));
  oai21  g054(.a(new_n116_), .b(new_n115_), .c(new_n102_), .O(z09));
  inv1   g055(.a(x07), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  oai21  g057(.a(z08), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  inv1   g058(.a(x36), .O(new_n121_));
  aoi21  g059(.a(new_n87_), .b(new_n121_), .c(x04), .O(new_n122_));
  nand2  g060(.a(x37), .b(x06), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n63_), .O(new_n124_));
  aoi21  g062(.a(new_n122_), .b(new_n120_), .c(new_n124_), .O(new_n125_));
  inv1   g063(.a(new_n82_), .O(new_n126_));
  inv1   g064(.a(new_n119_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n70_), .O(new_n128_));
  oai21  g066(.a(new_n125_), .b(new_n66_), .c(new_n128_), .O(z10));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  oai21  g068(.a(new_n87_), .b(x27), .c(new_n130_), .O(new_n131_));
  nand3  g069(.a(new_n80_), .b(x27), .c(new_n70_), .O(new_n132_));
  or2    g070(.a(x30), .b(x09), .O(new_n133_));
  nor2   g071(.a(new_n133_), .b(new_n63_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  aoi21  g073(.a(new_n131_), .b(x08), .c(new_n135_), .O(z11));
  nand3  g074(.a(new_n92_), .b(new_n121_), .c(new_n79_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(x27), .c(x10), .O(new_n138_));
  inv1   g076(.a(new_n67_), .O(new_n139_));
  nor2   g077(.a(new_n75_), .b(x04), .O(new_n140_));
  aoi21  g078(.a(new_n139_), .b(x27), .c(new_n140_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n138_), .c(x39), .O(new_n142_));
  inv1   g080(.a(new_n68_), .O(new_n143_));
  nand3  g081(.a(new_n143_), .b(x40), .c(new_n74_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n142_), .O(z12));
  nor2   g083(.a(x19), .b(x18), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x20), .O(new_n147_));
  inv1   g085(.a(new_n147_), .O(new_n148_));
  inv1   g086(.a(x13), .O(new_n149_));
  nand4  g087(.a(x40), .b(x39), .c(new_n149_), .d(new_n70_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  oai21  g089(.a(new_n121_), .b(new_n79_), .c(x28), .O(new_n152_));
  aoi22  g090(.a(new_n152_), .b(new_n151_), .c(new_n148_), .d(new_n139_), .O(new_n153_));
  aoi21  g091(.a(new_n151_), .b(new_n126_), .c(new_n63_), .O(new_n154_));
  oai21  g092(.a(new_n153_), .b(new_n66_), .c(new_n154_), .O(z13));
  oai21  g093(.a(new_n152_), .b(new_n126_), .c(new_n151_), .O(new_n156_));
  nor2   g094(.a(new_n148_), .b(new_n63_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  oai21  g096(.a(x28), .b(new_n66_), .c(new_n82_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n140_), .c(new_n149_), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n143_), .c(x39), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n158_), .c(new_n144_), .O(z14));
  nand2  g100(.a(x26), .b(x12), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n115_), .c(new_n63_), .O(z15));
  inv1   g102(.a(x23), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(x22), .c(x01), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n102_), .O(z16));
  nor2   g105(.a(x24), .b(new_n165_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n102_), .c(x22), .d(x01), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z17));
  nand2  g108(.a(x27), .b(x04), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n88_), .c(new_n133_), .O(new_n172_));
  nand3  g110(.a(new_n76_), .b(x29), .c(x08), .O(new_n173_));
  oai21  g111(.a(new_n172_), .b(new_n63_), .c(new_n173_), .O(z18));
  aoi21  g112(.a(new_n130_), .b(new_n87_), .c(new_n85_), .O(new_n175_));
  oai21  g113(.a(new_n79_), .b(new_n66_), .c(new_n134_), .O(new_n176_));
  nor2   g114(.a(new_n176_), .b(new_n175_), .O(z20));
  aoi21  g115(.a(new_n131_), .b(x08), .c(new_n135_), .O(z19));
endmodule


