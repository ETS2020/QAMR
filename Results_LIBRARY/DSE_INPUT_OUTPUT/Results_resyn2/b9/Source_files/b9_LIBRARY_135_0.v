// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n179_;
  nor2   g000(.a(x40), .b(x39), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x04), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g012(.a(x39), .O(new_n75_));
  inv1   g013(.a(x40), .O(new_n76_));
  nor2   g014(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand4  g015(.a(new_n77_), .b(new_n74_), .c(new_n64_), .d(new_n71_), .O(new_n78_));
  oai21  g016(.a(new_n70_), .b(new_n63_), .c(new_n78_), .O(z00));
  inv1   g017(.a(x36), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  nand2  g019(.a(x35), .b(new_n81_), .O(new_n82_));
  aoi21  g020(.a(new_n82_), .b(new_n80_), .c(new_n66_), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  inv1   g022(.a(new_n84_), .O(new_n85_));
  nor2   g023(.a(new_n63_), .b(new_n71_), .O(new_n86_));
  oai21  g024(.a(new_n85_), .b(new_n83_), .c(new_n86_), .O(z01));
  xnor2a g025(.a(x40), .b(x39), .O(z08));
  aoi21  g026(.a(x29), .b(x08), .c(x02), .O(new_n89_));
  inv1   g027(.a(x35), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(x28), .O(new_n91_));
  inv1   g029(.a(x08), .O(new_n92_));
  nand2  g030(.a(new_n66_), .b(new_n92_), .O(new_n93_));
  nand2  g031(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x04), .c(new_n89_), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n75_), .c(z08), .O(z02));
  nand2  g034(.a(x28), .b(x27), .O(new_n97_));
  oai22  g035(.a(new_n97_), .b(new_n90_), .c(new_n67_), .d(x27), .O(new_n98_));
  aoi21  g036(.a(new_n98_), .b(x21), .c(new_n63_), .O(z03));
  nor2   g037(.a(new_n63_), .b(x21), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(new_n98_), .O(z04));
  nand2  g039(.a(new_n76_), .b(new_n75_), .O(new_n102_));
  nand3  g040(.a(new_n97_), .b(new_n102_), .c(new_n67_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n64_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n105_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n102_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  inv1   g052(.a(x11), .O(new_n115_));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  nor3   g054(.a(new_n116_), .b(new_n63_), .c(new_n115_), .O(z09));
  inv1   g055(.a(x07), .O(new_n118_));
  nand3  g056(.a(x40), .b(x39), .c(x05), .O(new_n119_));
  oai21  g057(.a(z08), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  aoi21  g058(.a(new_n82_), .b(new_n80_), .c(x04), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n63_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n120_), .c(new_n123_), .O(new_n124_));
  inv1   g062(.a(new_n119_), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n85_), .c(new_n71_), .O(new_n126_));
  oai21  g064(.a(new_n124_), .b(new_n66_), .c(new_n126_), .O(z10));
  nand3  g065(.a(x40), .b(x39), .c(x29), .O(new_n128_));
  aoi21  g066(.a(new_n128_), .b(new_n82_), .c(new_n92_), .O(new_n129_));
  nand2  g067(.a(new_n128_), .b(x27), .O(new_n130_));
  nand2  g068(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g069(.a(new_n91_), .b(x27), .c(new_n71_), .O(new_n132_));
  nor2   g070(.a(x30), .b(x09), .O(new_n133_));
  and2   g071(.a(new_n133_), .b(new_n102_), .O(new_n134_));
  nand3  g072(.a(new_n134_), .b(new_n132_), .c(new_n131_), .O(z18));
  inv1   g073(.a(z18), .O(z11));
  nand3  g074(.a(new_n67_), .b(new_n80_), .c(new_n90_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(x27), .c(x10), .O(new_n138_));
  nor2   g076(.a(new_n76_), .b(x04), .O(new_n139_));
  nor2   g077(.a(new_n139_), .b(new_n69_), .O(new_n140_));
  oai21  g078(.a(new_n140_), .b(new_n138_), .c(x39), .O(new_n141_));
  inv1   g079(.a(new_n69_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(x40), .c(new_n75_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n141_), .O(z12));
  inv1   g082(.a(x13), .O(new_n145_));
  nand2  g083(.a(x36), .b(x35), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(x28), .O(new_n147_));
  nand4  g085(.a(new_n147_), .b(new_n139_), .c(x39), .d(new_n145_), .O(new_n148_));
  nand2  g086(.a(new_n68_), .b(new_n67_), .O(new_n149_));
  nor2   g087(.a(x19), .b(x18), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n149_), .c(new_n102_), .d(x20), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n148_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand3  g091(.a(x40), .b(new_n145_), .c(new_n71_), .O(new_n154_));
  inv1   g092(.a(new_n154_), .O(new_n155_));
  nand3  g093(.a(new_n155_), .b(new_n85_), .c(x39), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n153_), .O(z13));
  nand3  g095(.a(new_n146_), .b(new_n84_), .c(x28), .O(new_n158_));
  nand3  g096(.a(new_n158_), .b(new_n155_), .c(x39), .O(new_n159_));
  aoi21  g097(.a(new_n150_), .b(x20), .c(new_n63_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  oai21  g099(.a(x28), .b(new_n66_), .c(new_n84_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n155_), .O(new_n163_));
  nand3  g101(.a(new_n163_), .b(new_n142_), .c(x39), .O(new_n164_));
  nand3  g102(.a(new_n164_), .b(new_n161_), .c(new_n143_), .O(z14));
  inv1   g103(.a(x12), .O(new_n166_));
  oai21  g104(.a(new_n116_), .b(new_n166_), .c(new_n102_), .O(z15));
  inv1   g105(.a(x23), .O(new_n168_));
  nand3  g106(.a(new_n168_), .b(x22), .c(x01), .O(new_n169_));
  nand2  g107(.a(new_n169_), .b(new_n102_), .O(z16));
  nor2   g108(.a(x24), .b(new_n168_), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(new_n102_), .c(x22), .d(x01), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(z17));
  nor2   g111(.a(new_n66_), .b(new_n71_), .O(new_n174_));
  oai21  g112(.a(new_n174_), .b(new_n94_), .c(new_n133_), .O(new_n175_));
  nand3  g113(.a(new_n77_), .b(x29), .c(x08), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(new_n177_));
  aoi21  g115(.a(new_n175_), .b(new_n102_), .c(new_n177_), .O(z19));
  oai21  g116(.a(new_n90_), .b(new_n66_), .c(new_n134_), .O(new_n179_));
  nor2   g117(.a(new_n179_), .b(new_n129_), .O(z20));
endmodule


