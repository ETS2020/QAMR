// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n98_, new_n99_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n155_, new_n157_, new_n159_, new_n162_, new_n164_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  nor2   g004(.a(x36), .b(x35), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x32), .O(new_n71_));
  inv1   g009(.a(x36), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g011(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  oai21  g012(.a(new_n69_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  nor2   g016(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  oai22  g019(.a(new_n81_), .b(new_n79_), .c(new_n73_), .d(x04), .O(z01));
  nand2  g020(.a(x35), .b(new_n77_), .O(new_n83_));
  nor2   g021(.a(x27), .b(x08), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n83_), .c(x04), .O(new_n85_));
  nand2  g023(.a(x40), .b(x39), .O(new_n86_));
  aoi21  g024(.a(x29), .b(x08), .c(x02), .O(new_n87_));
  nor2   g025(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n85_), .c(new_n73_), .O(z02));
  inv1   g027(.a(x35), .O(new_n90_));
  inv1   g028(.a(x37), .O(new_n91_));
  nand2  g029(.a(x28), .b(x27), .O(new_n92_));
  oai22  g030(.a(new_n92_), .b(new_n90_), .c(new_n91_), .d(x27), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(x21), .c(new_n73_), .O(z03));
  inv1   g032(.a(x21), .O(new_n95_));
  aoi21  g033(.a(new_n93_), .b(new_n95_), .c(new_n73_), .O(z04));
  aoi21  g034(.a(new_n92_), .b(new_n91_), .c(new_n73_), .O(z05));
  inv1   g035(.a(new_n73_), .O(new_n98_));
  nand3  g036(.a(new_n92_), .b(new_n98_), .c(new_n91_), .O(new_n99_));
  inv1   g037(.a(new_n99_), .O(z06));
  inv1   g038(.a(x14), .O(new_n101_));
  inv1   g039(.a(x15), .O(new_n102_));
  nand2  g040(.a(x17), .b(new_n102_), .O(new_n103_));
  nor2   g041(.a(x33), .b(x31), .O(new_n104_));
  aoi21  g042(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(new_n105_));
  inv1   g043(.a(x25), .O(new_n106_));
  nand2  g044(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n101_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n107_), .c(x38), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n98_), .c(x03), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(new_n105_), .O(z07));
  nand2  g049(.a(new_n98_), .b(new_n86_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n98_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  inv1   g053(.a(x30), .O(new_n116_));
  oai21  g054(.a(new_n78_), .b(new_n76_), .c(new_n116_), .O(new_n117_));
  inv1   g055(.a(new_n86_), .O(new_n118_));
  nand2  g056(.a(new_n118_), .b(x05), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  and2   g058(.a(new_n86_), .b(x07), .O(new_n121_));
  aoi22  g059(.a(new_n121_), .b(new_n79_), .c(new_n120_), .d(new_n117_), .O(new_n122_));
  inv1   g060(.a(x05), .O(new_n123_));
  oai21  g061(.a(new_n64_), .b(new_n123_), .c(new_n72_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  aoi21  g064(.a(new_n124_), .b(x32), .c(new_n126_), .O(new_n127_));
  oai21  g065(.a(new_n122_), .b(x04), .c(new_n127_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand2  g067(.a(new_n118_), .b(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n83_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand4  g071(.a(x35), .b(new_n77_), .c(x27), .d(new_n63_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  and2   g073(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n133_), .c(new_n98_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  nand2  g076(.a(new_n98_), .b(new_n69_), .O(new_n139_));
  inv1   g077(.a(new_n139_), .O(z12));
  inv1   g078(.a(x13), .O(new_n141_));
  nand3  g079(.a(new_n118_), .b(new_n141_), .c(new_n63_), .O(new_n142_));
  inv1   g080(.a(new_n142_), .O(new_n143_));
  aoi21  g081(.a(x36), .b(x35), .c(new_n77_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n76_), .c(new_n80_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  inv1   g084(.a(new_n66_), .O(new_n147_));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n147_), .c(x20), .O(new_n149_));
  or2    g087(.a(new_n149_), .b(new_n76_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n146_), .c(new_n73_), .O(z13));
  oai21  g089(.a(new_n144_), .b(new_n142_), .c(new_n149_), .O(new_n152_));
  oai21  g090(.a(new_n142_), .b(new_n80_), .c(new_n98_), .O(new_n153_));
  aoi21  g091(.a(new_n152_), .b(x27), .c(new_n153_), .O(z14));
  inv1   g092(.a(x12), .O(new_n155_));
  nor2   g093(.a(new_n114_), .b(new_n155_), .O(z15));
  nand2  g094(.a(x22), .b(x01), .O(new_n157_));
  oai21  g095(.a(new_n157_), .b(x23), .c(new_n98_), .O(z16));
  inv1   g096(.a(x23), .O(new_n159_));
  nor4   g097(.a(new_n157_), .b(new_n73_), .c(x24), .d(new_n159_), .O(z17));
  aoi21  g098(.a(new_n136_), .b(new_n133_), .c(new_n73_), .O(z18));
  nand2  g099(.a(new_n136_), .b(new_n133_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n98_), .O(z19));
  oai21  g101(.a(new_n90_), .b(new_n76_), .c(new_n135_), .O(new_n164_));
  oai21  g102(.a(new_n164_), .b(new_n131_), .c(new_n98_), .O(z20));
endmodule


