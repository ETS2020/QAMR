// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n167_, new_n169_, new_n172_, new_n173_;
  nor2   g000(.a(x32), .b(x24), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x27), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  inv1   g004(.a(x04), .O(new_n67_));
  inv1   g005(.a(x35), .O(new_n68_));
  inv1   g006(.a(x36), .O(new_n69_));
  nand2  g007(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  and2   g008(.a(x40), .b(x39), .O(new_n71_));
  nand3  g009(.a(new_n71_), .b(new_n70_), .c(new_n67_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n66_), .c(new_n65_), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  nor2   g012(.a(new_n74_), .b(x04), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(x10), .O(new_n76_));
  inv1   g014(.a(new_n76_), .O(new_n77_));
  oai21  g015(.a(new_n77_), .b(new_n73_), .c(new_n64_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(x16), .c(new_n63_), .O(z00));
  inv1   g017(.a(x24), .O(new_n80_));
  inv1   g018(.a(x28), .O(new_n81_));
  aoi21  g019(.a(x35), .b(new_n81_), .c(x36), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n65_), .c(new_n83_), .O(new_n84_));
  oai22  g022(.a(new_n84_), .b(new_n80_), .c(new_n63_), .d(x04), .O(z01));
  inv1   g023(.a(new_n63_), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n81_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  inv1   g027(.a(x02), .O(new_n90_));
  nand2  g028(.a(x29), .b(x08), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n89_), .c(new_n71_), .d(new_n86_), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n68_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n63_), .O(z03));
  nor2   g035(.a(new_n63_), .b(x21), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(new_n96_), .O(z04));
  nand3  g037(.a(new_n95_), .b(new_n86_), .c(new_n94_), .O(z05));
  nand2  g038(.a(new_n95_), .b(new_n94_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n86_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(x00), .O(new_n104_));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x25), .b(new_n105_), .O(new_n106_));
  nand3  g044(.a(new_n106_), .b(new_n104_), .c(x38), .O(new_n107_));
  inv1   g045(.a(x17), .O(new_n108_));
  nor2   g046(.a(new_n108_), .b(x15), .O(new_n109_));
  or2    g047(.a(x33), .b(x31), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n109_), .c(x14), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n107_), .c(x03), .O(new_n112_));
  nand2  g050(.a(new_n112_), .b(new_n86_), .O(z07));
  nand2  g051(.a(new_n74_), .b(new_n86_), .O(z08));
  nand4  g052(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n86_), .O(z09));
  nor2   g054(.a(new_n82_), .b(new_n65_), .O(new_n117_));
  nand2  g055(.a(new_n71_), .b(x05), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(new_n119_));
  and2   g057(.a(new_n74_), .b(x07), .O(new_n120_));
  aoi22  g058(.a(new_n120_), .b(new_n117_), .c(new_n119_), .d(new_n84_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  inv1   g060(.a(new_n122_), .O(new_n123_));
  nor2   g061(.a(new_n123_), .b(new_n63_), .O(new_n124_));
  oai21  g062(.a(new_n121_), .b(x04), .c(new_n124_), .O(z10));
  inv1   g063(.a(x08), .O(new_n126_));
  nand2  g064(.a(new_n71_), .b(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n87_), .c(new_n126_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  inv1   g068(.a(new_n87_), .O(new_n131_));
  nor2   g069(.a(new_n65_), .b(x04), .O(new_n132_));
  inv1   g070(.a(x09), .O(new_n133_));
  inv1   g071(.a(x30), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  aoi21  g073(.a(new_n132_), .b(new_n131_), .c(new_n135_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n130_), .c(new_n86_), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(z11));
  inv1   g076(.a(new_n73_), .O(new_n139_));
  nand3  g077(.a(new_n76_), .b(new_n139_), .c(new_n86_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z12));
  nor2   g079(.a(x19), .b(x18), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x20), .O(new_n143_));
  inv1   g081(.a(new_n66_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(new_n86_), .O(new_n145_));
  inv1   g083(.a(x13), .O(new_n146_));
  nand2  g084(.a(new_n75_), .b(new_n146_), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x24), .O(new_n150_));
  oai22  g088(.a(new_n150_), .b(new_n147_), .c(new_n145_), .d(new_n143_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x27), .O(new_n152_));
  inv1   g090(.a(x32), .O(new_n153_));
  oai21  g091(.a(new_n134_), .b(new_n80_), .c(new_n153_), .O(new_n154_));
  nand3  g092(.a(new_n154_), .b(new_n75_), .c(new_n146_), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(new_n152_), .O(z13));
  nand3  g094(.a(new_n94_), .b(new_n68_), .c(x28), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x27), .c(x30), .O(new_n158_));
  oai21  g096(.a(new_n158_), .b(new_n80_), .c(new_n153_), .O(new_n159_));
  nand4  g097(.a(new_n148_), .b(new_n143_), .c(new_n83_), .d(x28), .O(new_n160_));
  inv1   g098(.a(new_n160_), .O(new_n161_));
  nand2  g099(.a(new_n144_), .b(x27), .O(new_n162_));
  oai22  g100(.a(new_n162_), .b(new_n143_), .c(new_n161_), .d(new_n147_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(new_n159_), .O(z14));
  nand4  g102(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n86_), .O(z15));
  nand2  g104(.a(x22), .b(x01), .O(new_n167_));
  nor3   g105(.a(new_n167_), .b(new_n63_), .c(x23), .O(z16));
  nand3  g106(.a(x32), .b(new_n80_), .c(x23), .O(new_n169_));
  nor2   g107(.a(new_n169_), .b(new_n167_), .O(z17));
  aoi21  g108(.a(new_n136_), .b(new_n130_), .c(new_n63_), .O(z18));
  nand2  g109(.a(x35), .b(x27), .O(new_n172_));
  nand3  g110(.a(new_n172_), .b(new_n134_), .c(new_n133_), .O(new_n173_));
  oai21  g111(.a(new_n173_), .b(new_n128_), .c(new_n86_), .O(z20));
  inv1   g112(.a(new_n137_), .O(z19));
endmodule


