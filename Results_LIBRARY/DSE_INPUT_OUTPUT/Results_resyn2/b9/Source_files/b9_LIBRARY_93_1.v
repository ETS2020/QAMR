// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n171_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n179_, new_n180_, new_n182_,
    new_n183_, new_n184_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand3  g001(.a(x40), .b(x39), .c(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x16), .O(new_n70_));
  inv1   g008(.a(x40), .O(new_n71_));
  nor2   g009(.a(new_n71_), .b(x23), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g011(.a(new_n69_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  inv1   g017(.a(x30), .O(new_n80_));
  inv1   g018(.a(x32), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g020(.a(new_n72_), .b(new_n63_), .O(new_n83_));
  oai21  g021(.a(new_n82_), .b(new_n79_), .c(new_n83_), .O(z01));
  nor2   g022(.a(x27), .b(x08), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n78_), .c(x04), .O(new_n86_));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand3  g026(.a(x40), .b(x39), .c(x23), .O(new_n89_));
  aoi21  g027(.a(new_n88_), .b(new_n87_), .c(new_n89_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n86_), .O(z02));
  inv1   g029(.a(x35), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  nand2  g031(.a(x28), .b(x27), .O(new_n94_));
  oai22  g032(.a(new_n94_), .b(new_n92_), .c(new_n93_), .d(x27), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n72_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  inv1   g035(.a(new_n72_), .O(new_n98_));
  nand3  g036(.a(new_n95_), .b(new_n98_), .c(new_n97_), .O(z04));
  nand3  g037(.a(new_n94_), .b(new_n98_), .c(new_n93_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x25), .O(new_n102_));
  nand2  g040(.a(new_n102_), .b(x00), .O(new_n103_));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x25), .b(new_n104_), .O(new_n105_));
  nand3  g043(.a(new_n105_), .b(new_n103_), .c(x38), .O(new_n106_));
  inv1   g044(.a(x17), .O(new_n107_));
  nor2   g045(.a(new_n107_), .b(x15), .O(new_n108_));
  or2    g046(.a(x33), .b(x31), .O(new_n109_));
  oai21  g047(.a(new_n109_), .b(new_n108_), .c(x14), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n106_), .c(x03), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(new_n98_), .O(z07));
  inv1   g050(.a(x39), .O(new_n113_));
  aoi21  g051(.a(new_n113_), .b(x23), .c(new_n71_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  nor3   g054(.a(new_n116_), .b(new_n72_), .c(new_n115_), .O(z09));
  inv1   g055(.a(x07), .O(new_n118_));
  nand4  g056(.a(x40), .b(x39), .c(x23), .d(x05), .O(new_n119_));
  oai21  g057(.a(z08), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  aoi21  g058(.a(new_n78_), .b(new_n76_), .c(x04), .O(new_n121_));
  nand2  g059(.a(x37), .b(x06), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n72_), .O(new_n123_));
  aoi21  g061(.a(new_n121_), .b(new_n120_), .c(new_n123_), .O(new_n124_));
  nand4  g062(.a(new_n82_), .b(new_n65_), .c(x23), .d(x05), .O(new_n125_));
  oai21  g063(.a(new_n124_), .b(new_n75_), .c(new_n125_), .O(z10));
  nor2   g064(.a(x30), .b(x09), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  inv1   g066(.a(x23), .O(new_n129_));
  and2   g067(.a(x39), .b(x29), .O(new_n130_));
  oai21  g068(.a(new_n130_), .b(new_n129_), .c(x40), .O(new_n131_));
  nand2  g069(.a(x27), .b(x04), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(x35), .c(new_n77_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  inv1   g072(.a(x08), .O(new_n135_));
  oai21  g073(.a(new_n78_), .b(x04), .c(x23), .O(new_n136_));
  nand2  g074(.a(new_n136_), .b(x27), .O(new_n137_));
  nand3  g075(.a(new_n137_), .b(new_n98_), .c(new_n135_), .O(new_n138_));
  aoi21  g076(.a(new_n138_), .b(new_n134_), .c(new_n128_), .O(z11));
  nor2   g077(.a(new_n67_), .b(new_n75_), .O(new_n140_));
  inv1   g078(.a(x10), .O(new_n141_));
  oai21  g079(.a(new_n66_), .b(new_n75_), .c(new_n141_), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(x39), .c(new_n63_), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x23), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x40), .c(new_n140_), .O(z12));
  inv1   g083(.a(x13), .O(new_n146_));
  nand3  g084(.a(x39), .b(new_n146_), .c(new_n63_), .O(new_n147_));
  nand2  g085(.a(x36), .b(x35), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x28), .O(new_n149_));
  aoi21  g087(.a(new_n149_), .b(x27), .c(new_n82_), .O(new_n150_));
  oai21  g088(.a(new_n150_), .b(new_n147_), .c(x23), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x40), .O(new_n152_));
  inv1   g090(.a(x18), .O(new_n153_));
  inv1   g091(.a(x19), .O(new_n154_));
  nand3  g092(.a(x20), .b(new_n154_), .c(new_n153_), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n140_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n152_), .O(z13));
  inv1   g096(.a(new_n147_), .O(new_n159_));
  nor2   g097(.a(x32), .b(x30), .O(new_n160_));
  nand3  g098(.a(new_n148_), .b(new_n160_), .c(x28), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(new_n159_), .c(new_n129_), .O(new_n162_));
  oai21  g100(.a(new_n162_), .b(new_n71_), .c(new_n155_), .O(new_n163_));
  inv1   g101(.a(new_n140_), .O(new_n164_));
  oai21  g102(.a(x28), .b(new_n75_), .c(new_n160_), .O(new_n165_));
  aoi21  g103(.a(new_n165_), .b(new_n159_), .c(new_n129_), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(new_n71_), .c(new_n164_), .O(new_n167_));
  nand2  g105(.a(new_n167_), .b(new_n163_), .O(z14));
  inv1   g106(.a(x12), .O(new_n169_));
  oai21  g107(.a(new_n116_), .b(new_n169_), .c(new_n98_), .O(z15));
  nand4  g108(.a(new_n71_), .b(new_n129_), .c(x22), .d(x01), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z16));
  inv1   g110(.a(x24), .O(new_n173_));
  nand4  g111(.a(new_n173_), .b(x23), .c(x22), .d(x01), .O(new_n174_));
  inv1   g112(.a(new_n174_), .O(z17));
  oai21  g113(.a(new_n133_), .b(new_n85_), .c(new_n127_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n98_), .O(new_n177_));
  oai21  g115(.a(new_n89_), .b(new_n88_), .c(new_n177_), .O(z18));
  nand2  g116(.a(new_n130_), .b(x40), .O(new_n179_));
  nor3   g117(.a(new_n179_), .b(new_n129_), .c(new_n135_), .O(new_n180_));
  aoi21  g118(.a(new_n176_), .b(new_n98_), .c(new_n180_), .O(z19));
  aoi21  g119(.a(new_n179_), .b(new_n78_), .c(new_n135_), .O(new_n182_));
  nand2  g120(.a(x35), .b(x27), .O(new_n183_));
  nand3  g121(.a(new_n183_), .b(new_n127_), .c(new_n98_), .O(new_n184_));
  nor2   g122(.a(new_n184_), .b(new_n182_), .O(z20));
endmodule


