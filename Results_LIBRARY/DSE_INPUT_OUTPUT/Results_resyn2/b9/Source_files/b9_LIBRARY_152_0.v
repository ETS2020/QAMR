// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n163_, new_n164_, new_n165_, new_n166_,
    new_n167_, new_n168_, new_n170_, new_n172_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n180_;
  inv1   g000(.a(x36), .O(new_n63_));
  inv1   g001(.a(x40), .O(new_n64_));
  nand2  g002(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  inv1   g004(.a(x15), .O(new_n67_));
  inv1   g005(.a(x16), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  inv1   g007(.a(x37), .O(new_n70_));
  nand2  g008(.a(x35), .b(x28), .O(new_n71_));
  aoi21  g009(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  aoi21  g010(.a(new_n72_), .b(new_n67_), .c(new_n68_), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  oai21  g012(.a(x36), .b(x35), .c(x27), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g014(.a(x39), .O(new_n77_));
  nor2   g015(.a(new_n77_), .b(x04), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n76_), .c(x40), .d(new_n67_), .O(new_n79_));
  oai21  g017(.a(new_n73_), .b(new_n66_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x35), .O(new_n81_));
  oai21  g019(.a(new_n81_), .b(x28), .c(new_n63_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n65_), .c(x04), .O(z01));
  nand2  g024(.a(new_n64_), .b(x36), .O(new_n87_));
  nor2   g025(.a(new_n81_), .b(x28), .O(new_n88_));
  inv1   g026(.a(x08), .O(new_n89_));
  nand2  g027(.a(new_n69_), .b(new_n89_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  inv1   g030(.a(new_n92_), .O(new_n93_));
  oai21  g031(.a(new_n93_), .b(x02), .c(x39), .O(new_n94_));
  aoi21  g032(.a(new_n91_), .b(x04), .c(new_n94_), .O(new_n95_));
  oai21  g033(.a(new_n95_), .b(new_n64_), .c(new_n87_), .O(z02));
  nand2  g034(.a(new_n71_), .b(x27), .O(new_n97_));
  nand2  g035(.a(new_n70_), .b(new_n69_), .O(new_n98_));
  nand4  g036(.a(new_n98_), .b(new_n97_), .c(new_n65_), .d(x21), .O(z03));
  aoi21  g037(.a(new_n70_), .b(new_n69_), .c(x21), .O(new_n100_));
  aoi21  g038(.a(new_n100_), .b(new_n97_), .c(new_n66_), .O(z04));
  aoi21  g039(.a(x28), .b(x27), .c(x37), .O(new_n102_));
  nor2   g040(.a(new_n102_), .b(new_n66_), .O(z05));
  inv1   g041(.a(z05), .O(z06));
  inv1   g042(.a(x14), .O(new_n105_));
  nand2  g043(.a(x17), .b(new_n67_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n105_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n65_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nor2   g052(.a(new_n64_), .b(new_n77_), .O(z08));
  nand4  g053(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n116_));
  nand2  g054(.a(new_n116_), .b(new_n65_), .O(z09));
  inv1   g055(.a(x07), .O(new_n118_));
  aoi21  g056(.a(new_n77_), .b(new_n118_), .c(x04), .O(new_n119_));
  oai21  g057(.a(new_n63_), .b(new_n118_), .c(new_n64_), .O(new_n120_));
  inv1   g058(.a(x05), .O(new_n121_));
  nand3  g059(.a(x40), .b(x39), .c(new_n121_), .O(new_n122_));
  nand4  g060(.a(new_n122_), .b(new_n120_), .c(new_n119_), .d(new_n82_), .O(new_n123_));
  nand3  g061(.a(new_n65_), .b(x37), .c(x06), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(x27), .O(new_n126_));
  inv1   g064(.a(x04), .O(new_n127_));
  inv1   g065(.a(x30), .O(new_n128_));
  inv1   g066(.a(x32), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand4  g068(.a(z08), .b(new_n130_), .c(x05), .d(new_n127_), .O(new_n131_));
  nand2  g069(.a(new_n131_), .b(new_n126_), .O(z10));
  inv1   g070(.a(x28), .O(new_n133_));
  nand2  g071(.a(x35), .b(new_n133_), .O(new_n134_));
  nand3  g072(.a(x40), .b(x39), .c(x29), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n134_), .c(new_n89_), .O(new_n136_));
  nand2  g074(.a(new_n135_), .b(x27), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand3  g076(.a(new_n88_), .b(x27), .c(new_n127_), .O(new_n139_));
  nor2   g077(.a(x30), .b(x09), .O(new_n140_));
  and2   g078(.a(new_n140_), .b(new_n65_), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n139_), .c(new_n138_), .O(z18));
  inv1   g080(.a(z18), .O(z11));
  nand3  g081(.a(new_n70_), .b(new_n63_), .c(new_n81_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x27), .c(x10), .O(new_n145_));
  nor2   g083(.a(new_n78_), .b(new_n72_), .O(new_n146_));
  oai21  g084(.a(new_n146_), .b(new_n145_), .c(x40), .O(new_n147_));
  inv1   g085(.a(new_n72_), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(new_n64_), .c(x36), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n147_), .O(z12));
  nand2  g088(.a(x36), .b(x35), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x28), .O(new_n152_));
  nor2   g090(.a(x13), .b(x04), .O(new_n153_));
  nand3  g091(.a(new_n153_), .b(new_n152_), .c(z08), .O(new_n154_));
  nand2  g092(.a(new_n71_), .b(new_n70_), .O(new_n155_));
  nor2   g093(.a(x19), .b(x18), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(new_n155_), .c(new_n65_), .d(x20), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(x27), .O(new_n159_));
  nand3  g097(.a(new_n153_), .b(z08), .c(new_n130_), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(new_n159_), .O(z13));
  nand2  g099(.a(new_n153_), .b(z08), .O(new_n162_));
  nand2  g100(.a(new_n156_), .b(x20), .O(new_n163_));
  nand3  g101(.a(new_n151_), .b(new_n84_), .c(x28), .O(new_n164_));
  inv1   g102(.a(new_n164_), .O(new_n165_));
  oai21  g103(.a(new_n165_), .b(new_n162_), .c(new_n163_), .O(new_n166_));
  aoi21  g104(.a(new_n133_), .b(x27), .c(new_n130_), .O(new_n167_));
  oai21  g105(.a(new_n167_), .b(new_n162_), .c(new_n148_), .O(new_n168_));
  aoi21  g106(.a(new_n168_), .b(new_n166_), .c(new_n66_), .O(z14));
  nand4  g107(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n170_));
  nand2  g108(.a(new_n170_), .b(new_n65_), .O(z15));
  nand2  g109(.a(x22), .b(x01), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(x23), .c(new_n65_), .O(z16));
  inv1   g111(.a(x24), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(x23), .O(new_n175_));
  oai21  g113(.a(new_n175_), .b(new_n172_), .c(new_n65_), .O(z17));
  nor2   g114(.a(new_n69_), .b(new_n127_), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n91_), .c(new_n140_), .O(new_n178_));
  aoi22  g116(.a(new_n178_), .b(new_n65_), .c(z08), .d(new_n93_), .O(z19));
  oai21  g117(.a(new_n81_), .b(new_n69_), .c(new_n141_), .O(new_n180_));
  nor2   g118(.a(new_n180_), .b(new_n136_), .O(z20));
endmodule


