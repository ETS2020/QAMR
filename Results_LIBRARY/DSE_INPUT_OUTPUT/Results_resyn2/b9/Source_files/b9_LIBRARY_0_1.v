// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n164_, new_n166_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n180_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  and2   g005(.a(x40), .b(x39), .O(new_n68_));
  nand3  g006(.a(new_n68_), .b(x10), .c(new_n65_), .O(new_n69_));
  inv1   g007(.a(new_n69_), .O(new_n70_));
  aoi21  g008(.a(new_n67_), .b(x27), .c(new_n70_), .O(new_n71_));
  inv1   g009(.a(x11), .O(new_n72_));
  nand2  g010(.a(x40), .b(new_n72_), .O(new_n73_));
  and2   g011(.a(new_n73_), .b(x16), .O(new_n74_));
  oai21  g012(.a(new_n71_), .b(x15), .c(new_n74_), .O(z00));
  inv1   g013(.a(new_n73_), .O(new_n76_));
  inv1   g014(.a(x30), .O(new_n77_));
  inv1   g015(.a(x32), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  inv1   g017(.a(x28), .O(new_n80_));
  nand2  g018(.a(x35), .b(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(x27), .O(new_n83_));
  nand3  g021(.a(new_n83_), .b(new_n78_), .c(new_n77_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n76_), .O(z01));
  inv1   g023(.a(x35), .O(new_n86_));
  nor2   g024(.a(new_n86_), .b(x28), .O(new_n87_));
  oai21  g025(.a(x27), .b(x08), .c(new_n87_), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nand3  g028(.a(x40), .b(x39), .c(x11), .O(new_n91_));
  nor2   g029(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n89_), .O(z02));
  oai21  g031(.a(new_n86_), .b(new_n80_), .c(x27), .O(new_n94_));
  inv1   g032(.a(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n94_), .c(new_n73_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n96_), .b(new_n95_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n94_), .c(new_n76_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n73_), .O(z05));
  inv1   g040(.a(new_n101_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n73_), .O(z06));
  inv1   g042(.a(x14), .O(new_n105_));
  inv1   g043(.a(x15), .O(new_n106_));
  nand2  g044(.a(x17), .b(new_n106_), .O(new_n107_));
  nor2   g045(.a(x33), .b(x31), .O(new_n108_));
  aoi21  g046(.a(new_n108_), .b(new_n107_), .c(new_n105_), .O(new_n109_));
  inv1   g047(.a(x25), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(x00), .O(new_n111_));
  nand2  g049(.a(x25), .b(new_n105_), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n111_), .c(x38), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n73_), .c(x03), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n109_), .O(z07));
  inv1   g053(.a(new_n91_), .O(z08));
  nand4  g054(.a(x34), .b(x27), .c(x26), .d(x11), .O(new_n117_));
  inv1   g055(.a(new_n117_), .O(z09));
  nand2  g056(.a(new_n78_), .b(new_n77_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n68_), .O(new_n120_));
  inv1   g058(.a(x07), .O(new_n121_));
  nand2  g059(.a(x40), .b(x39), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n82_), .c(x27), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n120_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  aoi21  g064(.a(new_n68_), .b(new_n126_), .c(x04), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n73_), .O(z10));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  nor2   g069(.a(new_n95_), .b(new_n65_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(new_n88_), .c(new_n131_), .O(new_n133_));
  inv1   g071(.a(x08), .O(new_n134_));
  inv1   g072(.a(x40), .O(new_n135_));
  nand2  g073(.a(x39), .b(x29), .O(new_n136_));
  nor4   g074(.a(new_n136_), .b(new_n135_), .c(new_n72_), .d(new_n134_), .O(new_n137_));
  aoi21  g075(.a(new_n133_), .b(new_n73_), .c(new_n137_), .O(z11));
  nand2  g076(.a(new_n67_), .b(x27), .O(new_n139_));
  nand2  g077(.a(new_n69_), .b(new_n139_), .O(new_n140_));
  nand2  g078(.a(new_n73_), .b(new_n140_), .O(z12));
  inv1   g079(.a(new_n63_), .O(new_n142_));
  inv1   g080(.a(x18), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand3  g082(.a(x20), .b(new_n144_), .c(new_n143_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(new_n73_), .c(new_n142_), .O(new_n147_));
  nor2   g085(.a(x13), .b(x04), .O(new_n148_));
  nand2  g086(.a(x36), .b(x35), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x28), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(new_n148_), .c(new_n68_), .d(x11), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand2  g090(.a(new_n152_), .b(x27), .O(new_n153_));
  nand4  g091(.a(new_n148_), .b(new_n119_), .c(new_n68_), .d(x11), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n153_), .O(z13));
  nand4  g093(.a(new_n149_), .b(new_n78_), .c(new_n77_), .d(x28), .O(new_n156_));
  nand3  g094(.a(new_n156_), .b(new_n148_), .c(x39), .O(new_n157_));
  aoi21  g095(.a(new_n157_), .b(x11), .c(new_n135_), .O(new_n158_));
  nand3  g096(.a(new_n148_), .b(new_n68_), .c(new_n80_), .O(new_n159_));
  aoi21  g097(.a(new_n159_), .b(new_n63_), .c(new_n95_), .O(new_n160_));
  nand3  g098(.a(new_n148_), .b(new_n119_), .c(x39), .O(new_n161_));
  aoi21  g099(.a(new_n161_), .b(x11), .c(new_n135_), .O(new_n162_));
  oai22  g100(.a(new_n162_), .b(new_n160_), .c(new_n158_), .d(new_n146_), .O(z14));
  nand4  g101(.a(x34), .b(x27), .c(x26), .d(x12), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(new_n73_), .O(z15));
  nand2  g103(.a(x22), .b(x01), .O(new_n166_));
  oai21  g104(.a(new_n166_), .b(x23), .c(new_n73_), .O(z16));
  inv1   g105(.a(x24), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(x23), .O(new_n169_));
  oai21  g107(.a(new_n169_), .b(new_n166_), .c(new_n73_), .O(z17));
  nand3  g108(.a(x40), .b(x39), .c(x29), .O(new_n171_));
  aoi21  g109(.a(new_n171_), .b(new_n81_), .c(new_n134_), .O(new_n172_));
  nand2  g110(.a(new_n171_), .b(x27), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n172_), .O(new_n174_));
  nand3  g112(.a(new_n87_), .b(x27), .c(new_n65_), .O(new_n175_));
  nand2  g113(.a(new_n131_), .b(new_n73_), .O(new_n176_));
  inv1   g114(.a(new_n176_), .O(new_n177_));
  nand3  g115(.a(new_n177_), .b(new_n175_), .c(new_n174_), .O(z18));
  inv1   g116(.a(z18), .O(z19));
  nor2   g117(.a(new_n86_), .b(new_n95_), .O(new_n180_));
  nor3   g118(.a(new_n180_), .b(new_n176_), .c(new_n172_), .O(z20));
endmodule


