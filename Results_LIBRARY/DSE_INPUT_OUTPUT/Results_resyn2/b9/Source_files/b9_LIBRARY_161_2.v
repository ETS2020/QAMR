// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n178_, new_n179_, new_n180_, new_n182_, new_n183_;
  inv1   g000(.a(x14), .O(new_n63_));
  nand2  g001(.a(new_n63_), .b(x04), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g005(.a(new_n67_), .O(new_n68_));
  inv1   g006(.a(x27), .O(new_n69_));
  nor2   g007(.a(new_n69_), .b(x15), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n68_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x04), .O(new_n72_));
  inv1   g010(.a(x15), .O(new_n73_));
  inv1   g011(.a(x10), .O(new_n74_));
  nor2   g012(.a(x36), .b(x35), .O(new_n75_));
  oai21  g013(.a(new_n75_), .b(new_n69_), .c(new_n74_), .O(new_n76_));
  nand2  g014(.a(x40), .b(x39), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  nand4  g016(.a(new_n78_), .b(new_n76_), .c(new_n73_), .d(new_n72_), .O(new_n79_));
  oai21  g017(.a(new_n71_), .b(new_n65_), .c(new_n79_), .O(z00));
  inv1   g018(.a(x36), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(new_n81_), .c(new_n69_), .O(new_n84_));
  nor2   g022(.a(x32), .b(x30), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x14), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(new_n84_), .c(x04), .O(z01));
  inv1   g025(.a(x08), .O(new_n88_));
  aoi21  g026(.a(new_n69_), .b(new_n88_), .c(new_n83_), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(x14), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x04), .O(new_n91_));
  inv1   g029(.a(x02), .O(new_n92_));
  nand2  g030(.a(x29), .b(x08), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n92_), .c(new_n77_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n91_), .O(z02));
  inv1   g033(.a(x35), .O(new_n96_));
  inv1   g034(.a(x37), .O(new_n97_));
  nand2  g035(.a(x28), .b(x27), .O(new_n98_));
  oai22  g036(.a(new_n98_), .b(new_n96_), .c(new_n97_), .d(x27), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  aoi21  g039(.a(new_n99_), .b(new_n101_), .c(new_n65_), .O(z04));
  nand3  g040(.a(new_n98_), .b(new_n64_), .c(new_n97_), .O(z05));
  nand2  g041(.a(new_n98_), .b(new_n97_), .O(new_n104_));
  nand2  g042(.a(new_n104_), .b(new_n64_), .O(z06));
  nand2  g043(.a(x17), .b(new_n73_), .O(new_n106_));
  nor2   g044(.a(x33), .b(x31), .O(new_n107_));
  aoi21  g045(.a(new_n107_), .b(new_n106_), .c(new_n63_), .O(new_n108_));
  inv1   g046(.a(x25), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x00), .O(new_n110_));
  nand2  g048(.a(x25), .b(new_n63_), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n110_), .c(x38), .O(new_n112_));
  nand3  g050(.a(new_n112_), .b(new_n64_), .c(x03), .O(new_n113_));
  nor2   g051(.a(new_n113_), .b(new_n108_), .O(z07));
  nor2   g052(.a(new_n77_), .b(new_n65_), .O(z08));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  nand2  g054(.a(new_n64_), .b(x11), .O(new_n117_));
  nor2   g055(.a(new_n117_), .b(new_n116_), .O(z09));
  nand2  g056(.a(x37), .b(x06), .O(new_n119_));
  aoi21  g057(.a(new_n63_), .b(x04), .c(new_n119_), .O(new_n120_));
  nand2  g058(.a(new_n77_), .b(x07), .O(new_n121_));
  nand3  g059(.a(x40), .b(x39), .c(x05), .O(new_n122_));
  nand2  g060(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  aoi21  g061(.a(new_n83_), .b(new_n81_), .c(x04), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n123_), .c(new_n120_), .O(new_n125_));
  inv1   g063(.a(new_n85_), .O(new_n126_));
  inv1   g064(.a(new_n122_), .O(new_n127_));
  nand3  g065(.a(new_n127_), .b(new_n126_), .c(new_n72_), .O(new_n128_));
  oai21  g066(.a(new_n125_), .b(new_n69_), .c(new_n128_), .O(z10));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n83_), .c(new_n88_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  inv1   g071(.a(new_n83_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  nor2   g074(.a(new_n69_), .b(x04), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n134_), .c(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n133_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n64_), .O(z11));
  nand2  g078(.a(new_n78_), .b(new_n72_), .O(new_n141_));
  oai21  g079(.a(new_n141_), .b(new_n75_), .c(new_n67_), .O(new_n142_));
  nand2  g080(.a(x10), .b(new_n72_), .O(new_n143_));
  oai21  g081(.a(new_n143_), .b(new_n77_), .c(new_n64_), .O(new_n144_));
  aoi21  g082(.a(new_n142_), .b(x27), .c(new_n144_), .O(z12));
  nor2   g083(.a(x13), .b(x04), .O(new_n146_));
  nand3  g084(.a(new_n146_), .b(x40), .c(x39), .O(new_n147_));
  aoi21  g085(.a(x36), .b(x35), .c(new_n82_), .O(new_n148_));
  nor2   g086(.a(x19), .b(x18), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(x20), .O(new_n150_));
  oai22  g088(.a(new_n150_), .b(new_n67_), .c(new_n148_), .d(new_n147_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(x27), .O(new_n152_));
  nor2   g090(.a(new_n147_), .b(new_n85_), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(new_n65_), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n152_), .O(z13));
  nand3  g093(.a(new_n146_), .b(new_n78_), .c(new_n82_), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n67_), .c(new_n69_), .O(new_n157_));
  inv1   g095(.a(x30), .O(new_n158_));
  inv1   g096(.a(x32), .O(new_n159_));
  nand2  g097(.a(x36), .b(x35), .O(new_n160_));
  nand4  g098(.a(new_n160_), .b(new_n159_), .c(new_n158_), .d(x28), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(new_n146_), .c(new_n78_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n150_), .c(new_n65_), .O(new_n163_));
  oai21  g101(.a(new_n157_), .b(new_n153_), .c(new_n163_), .O(z14));
  inv1   g102(.a(x12), .O(new_n165_));
  oai21  g103(.a(new_n116_), .b(new_n165_), .c(new_n64_), .O(z15));
  inv1   g104(.a(x23), .O(new_n167_));
  nand3  g105(.a(new_n167_), .b(x22), .c(x01), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n64_), .O(z16));
  nor2   g107(.a(x24), .b(new_n167_), .O(new_n170_));
  nand4  g108(.a(new_n170_), .b(new_n64_), .c(x22), .d(x01), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(z17));
  oai21  g110(.a(new_n93_), .b(new_n77_), .c(new_n135_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n64_), .O(new_n174_));
  oai21  g112(.a(x27), .b(new_n63_), .c(x04), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n89_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n174_), .O(z18));
  nand3  g115(.a(new_n130_), .b(new_n83_), .c(new_n64_), .O(new_n178_));
  nand3  g116(.a(new_n178_), .b(new_n132_), .c(x08), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n138_), .O(new_n180_));
  nand2  g118(.a(new_n180_), .b(new_n64_), .O(z19));
  nand2  g119(.a(x35), .b(x27), .O(new_n182_));
  nand3  g120(.a(new_n182_), .b(new_n135_), .c(new_n64_), .O(new_n183_));
  nor2   g121(.a(new_n183_), .b(new_n131_), .O(z20));
endmodule


