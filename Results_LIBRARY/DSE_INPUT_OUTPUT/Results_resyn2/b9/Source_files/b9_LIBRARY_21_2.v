// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n163_, new_n165_, new_n167_, new_n168_,
    new_n170_, new_n171_, new_n172_, new_n174_, new_n176_, new_n177_;
  inv1   g000(.a(x33), .O(new_n63_));
  nor2   g001(.a(x39), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x16), .O(new_n66_));
  inv1   g004(.a(x27), .O(new_n67_));
  inv1   g005(.a(x37), .O(new_n68_));
  nand2  g006(.a(x35), .b(x28), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  aoi21  g008(.a(new_n70_), .b(new_n65_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x10), .O(new_n72_));
  oai21  g010(.a(x36), .b(x35), .c(x27), .O(new_n73_));
  nand2  g011(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand3  g014(.a(new_n76_), .b(new_n74_), .c(new_n65_), .O(new_n77_));
  oai21  g015(.a(new_n71_), .b(new_n64_), .c(new_n77_), .O(z00));
  inv1   g016(.a(x35), .O(new_n79_));
  inv1   g017(.a(x36), .O(new_n80_));
  oai21  g018(.a(new_n79_), .b(x28), .c(new_n80_), .O(new_n81_));
  nand2  g019(.a(new_n81_), .b(x27), .O(new_n82_));
  nor2   g020(.a(x32), .b(x30), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n64_), .O(z01));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n75_), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  xor2a  g030(.a(new_n92_), .b(new_n64_), .O(z02));
  nand3  g031(.a(x35), .b(x28), .c(x27), .O(new_n94_));
  oai21  g032(.a(new_n68_), .b(x27), .c(new_n94_), .O(new_n95_));
  aoi21  g033(.a(new_n95_), .b(x21), .c(new_n64_), .O(z03));
  inv1   g034(.a(x21), .O(new_n97_));
  inv1   g035(.a(x39), .O(new_n98_));
  nand2  g036(.a(new_n98_), .b(x33), .O(new_n99_));
  nand3  g037(.a(new_n95_), .b(new_n99_), .c(new_n97_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n99_), .O(z05));
  inv1   g040(.a(new_n101_), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n99_), .O(z06));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  inv1   g049(.a(x31), .O(new_n112_));
  nand2  g050(.a(new_n63_), .b(new_n112_), .O(new_n113_));
  oai21  g051(.a(new_n113_), .b(new_n111_), .c(x14), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n109_), .c(x03), .O(new_n115_));
  nand2  g053(.a(new_n115_), .b(new_n99_), .O(z07));
  inv1   g054(.a(new_n75_), .O(z08));
  and2   g055(.a(x34), .b(x27), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(x26), .c(x11), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n99_), .O(z09));
  inv1   g058(.a(new_n83_), .O(new_n121_));
  nand2  g059(.a(new_n121_), .b(z08), .O(new_n122_));
  inv1   g060(.a(x07), .O(new_n123_));
  nand2  g061(.a(new_n75_), .b(new_n123_), .O(new_n124_));
  nand3  g062(.a(new_n124_), .b(new_n81_), .c(x27), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  inv1   g064(.a(x05), .O(new_n127_));
  aoi21  g065(.a(z08), .b(new_n127_), .c(x04), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand3  g067(.a(x37), .b(x27), .c(x06), .O(new_n130_));
  inv1   g068(.a(new_n130_), .O(new_n131_));
  nor2   g069(.a(new_n131_), .b(new_n64_), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n129_), .O(z10));
  inv1   g071(.a(x08), .O(new_n134_));
  nand3  g072(.a(x40), .b(x39), .c(x29), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(new_n87_), .c(new_n134_), .O(new_n136_));
  nand2  g074(.a(new_n135_), .b(x27), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  inv1   g076(.a(new_n87_), .O(new_n139_));
  inv1   g077(.a(x09), .O(new_n140_));
  inv1   g078(.a(x30), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nor2   g080(.a(new_n67_), .b(x04), .O(new_n143_));
  aoi21  g081(.a(new_n143_), .b(new_n139_), .c(new_n142_), .O(new_n144_));
  nand3  g082(.a(new_n144_), .b(new_n138_), .c(new_n99_), .O(z18));
  inv1   g083(.a(z18), .O(z11));
  nor3   g084(.a(x37), .b(x36), .c(x35), .O(new_n147_));
  oai21  g085(.a(new_n147_), .b(new_n67_), .c(new_n72_), .O(new_n148_));
  or2    g086(.a(new_n76_), .b(new_n70_), .O(new_n149_));
  nand3  g087(.a(new_n149_), .b(new_n148_), .c(new_n99_), .O(z12));
  inv1   g088(.a(x13), .O(new_n151_));
  oai21  g089(.a(new_n80_), .b(new_n79_), .c(x28), .O(new_n152_));
  nand3  g090(.a(new_n152_), .b(new_n76_), .c(new_n151_), .O(new_n153_));
  nand2  g091(.a(new_n69_), .b(new_n68_), .O(new_n154_));
  nor2   g092(.a(x19), .b(x18), .O(new_n155_));
  nand4  g093(.a(new_n155_), .b(new_n154_), .c(new_n99_), .d(x20), .O(new_n156_));
  nand2  g094(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(x27), .O(new_n158_));
  nand3  g096(.a(new_n121_), .b(new_n76_), .c(new_n151_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z13));
  nand3  g098(.a(new_n155_), .b(new_n154_), .c(x20), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n153_), .O(new_n162_));
  nand2  g100(.a(new_n159_), .b(new_n99_), .O(new_n163_));
  aoi21  g101(.a(new_n162_), .b(x27), .c(new_n163_), .O(z14));
  nand4  g102(.a(new_n118_), .b(new_n99_), .c(x26), .d(x12), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z15));
  inv1   g104(.a(x01), .O(new_n167_));
  inv1   g105(.a(x22), .O(new_n168_));
  nor4   g106(.a(new_n64_), .b(x23), .c(new_n168_), .d(new_n167_), .O(z16));
  inv1   g107(.a(x24), .O(new_n170_));
  nor2   g108(.a(new_n168_), .b(new_n167_), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(new_n99_), .c(new_n170_), .d(x23), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(z17));
  nand2  g111(.a(new_n144_), .b(new_n138_), .O(new_n174_));
  nand2  g112(.a(new_n174_), .b(new_n99_), .O(z19));
  nand2  g113(.a(x35), .b(x27), .O(new_n176_));
  nand3  g114(.a(new_n176_), .b(new_n141_), .c(new_n140_), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n136_), .c(new_n99_), .O(z20));
endmodule


