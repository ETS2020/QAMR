// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n168_, new_n170_;
  aoi21  g000(.a(x35), .b(x28), .c(x37), .O(new_n63_));
  nor2   g001(.a(x36), .b(x35), .O(new_n64_));
  inv1   g002(.a(x04), .O(new_n65_));
  nand3  g003(.a(x40), .b(x39), .c(new_n65_), .O(new_n66_));
  oai21  g004(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n65_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x36), .b(x31), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(new_n72_), .O(new_n75_));
  inv1   g013(.a(x27), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  aoi21  g015(.a(x35), .b(new_n77_), .c(x36), .O(new_n78_));
  nor2   g016(.a(x32), .b(x30), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(new_n76_), .c(new_n79_), .O(new_n80_));
  nand3  g018(.a(new_n80_), .b(new_n75_), .c(x04), .O(z01));
  nand2  g019(.a(x35), .b(new_n77_), .O(new_n82_));
  nor2   g020(.a(x27), .b(x08), .O(new_n83_));
  oai21  g021(.a(new_n83_), .b(new_n82_), .c(x04), .O(new_n84_));
  nand2  g022(.a(x40), .b(x39), .O(new_n85_));
  nor2   g023(.a(new_n72_), .b(new_n85_), .O(z08));
  inv1   g024(.a(x02), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  nand2  g026(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g027(.a(new_n89_), .b(z08), .c(new_n84_), .O(z02));
  inv1   g028(.a(x35), .O(new_n91_));
  inv1   g029(.a(x37), .O(new_n92_));
  nand2  g030(.a(x28), .b(x27), .O(new_n93_));
  oai22  g031(.a(new_n93_), .b(new_n91_), .c(new_n92_), .d(x27), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(new_n72_), .O(z03));
  nor2   g033(.a(new_n72_), .b(x21), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n94_), .O(z04));
  nand3  g035(.a(new_n93_), .b(new_n75_), .c(new_n92_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(x25), .O(new_n100_));
  nand2  g038(.a(new_n100_), .b(x00), .O(new_n101_));
  inv1   g039(.a(x14), .O(new_n102_));
  nand2  g040(.a(x25), .b(new_n102_), .O(new_n103_));
  nand3  g041(.a(new_n103_), .b(new_n101_), .c(x38), .O(new_n104_));
  inv1   g042(.a(x17), .O(new_n105_));
  nor2   g043(.a(new_n105_), .b(x15), .O(new_n106_));
  or2    g044(.a(x33), .b(x31), .O(new_n107_));
  oai21  g045(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n104_), .c(x03), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(new_n75_), .O(z07));
  inv1   g048(.a(x11), .O(new_n111_));
  nand4  g049(.a(new_n75_), .b(x34), .c(x27), .d(x26), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n111_), .O(z09));
  oai21  g051(.a(new_n78_), .b(new_n76_), .c(new_n85_), .O(new_n114_));
  inv1   g052(.a(x05), .O(new_n115_));
  nand3  g053(.a(x40), .b(x39), .c(new_n115_), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  nand2  g055(.a(new_n85_), .b(new_n117_), .O(new_n118_));
  nand3  g056(.a(new_n118_), .b(new_n116_), .c(new_n65_), .O(new_n119_));
  inv1   g057(.a(new_n119_), .O(new_n120_));
  nand3  g058(.a(new_n120_), .b(new_n114_), .c(new_n80_), .O(new_n121_));
  nand3  g059(.a(x37), .b(x27), .c(x06), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(new_n72_), .O(z10));
  inv1   g061(.a(x08), .O(new_n124_));
  nand3  g062(.a(x40), .b(x39), .c(x29), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n82_), .c(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n125_), .b(x27), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nor2   g066(.a(new_n91_), .b(new_n76_), .O(new_n129_));
  nor2   g067(.a(x28), .b(x04), .O(new_n130_));
  or2    g068(.a(x30), .b(x09), .O(new_n131_));
  aoi21  g069(.a(new_n130_), .b(new_n129_), .c(new_n131_), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n128_), .c(new_n75_), .O(new_n133_));
  inv1   g071(.a(new_n133_), .O(z11));
  inv1   g072(.a(x10), .O(new_n135_));
  nor3   g073(.a(x37), .b(x36), .c(x35), .O(new_n136_));
  oai21  g074(.a(new_n136_), .b(new_n76_), .c(new_n135_), .O(new_n137_));
  oai21  g075(.a(new_n63_), .b(new_n76_), .c(new_n66_), .O(new_n138_));
  nand3  g076(.a(new_n138_), .b(new_n137_), .c(new_n75_), .O(z12));
  inv1   g077(.a(x13), .O(new_n140_));
  nand4  g078(.a(x40), .b(x39), .c(new_n140_), .d(new_n65_), .O(new_n141_));
  aoi21  g079(.a(x36), .b(x35), .c(new_n77_), .O(new_n142_));
  nor2   g080(.a(x19), .b(x18), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(x20), .O(new_n144_));
  oai22  g082(.a(new_n144_), .b(new_n63_), .c(new_n142_), .d(new_n141_), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x27), .O(new_n146_));
  inv1   g084(.a(new_n79_), .O(new_n147_));
  inv1   g085(.a(new_n141_), .O(new_n148_));
  aoi21  g086(.a(new_n148_), .b(new_n147_), .c(new_n72_), .O(new_n149_));
  nand2  g087(.a(new_n149_), .b(new_n146_), .O(z13));
  nand2  g088(.a(new_n92_), .b(new_n91_), .O(new_n151_));
  nand3  g089(.a(new_n151_), .b(new_n143_), .c(x20), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n142_), .c(new_n76_), .O(new_n153_));
  nand2  g091(.a(x35), .b(x28), .O(new_n154_));
  nand2  g092(.a(new_n154_), .b(new_n92_), .O(new_n155_));
  nand4  g093(.a(new_n143_), .b(new_n155_), .c(x27), .d(x20), .O(new_n156_));
  aoi21  g094(.a(new_n156_), .b(new_n141_), .c(new_n72_), .O(new_n157_));
  oai21  g095(.a(new_n153_), .b(new_n147_), .c(new_n157_), .O(z14));
  inv1   g096(.a(x12), .O(new_n159_));
  nor2   g097(.a(new_n112_), .b(new_n159_), .O(z15));
  inv1   g098(.a(x23), .O(new_n161_));
  nand3  g099(.a(new_n161_), .b(x22), .c(x01), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n75_), .O(z16));
  nor2   g101(.a(x24), .b(new_n161_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n75_), .c(x22), .d(x01), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z17));
  aoi21  g104(.a(new_n132_), .b(new_n128_), .c(new_n72_), .O(z18));
  nand2  g105(.a(new_n132_), .b(new_n128_), .O(new_n168_));
  nand2  g106(.a(new_n168_), .b(new_n75_), .O(z19));
  or2    g107(.a(new_n131_), .b(new_n129_), .O(new_n170_));
  oai21  g108(.a(new_n170_), .b(new_n126_), .c(new_n75_), .O(z20));
endmodule


