// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n100_, new_n101_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n172_, new_n175_,
    new_n176_, new_n177_, new_n179_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x36), .O(new_n64_));
  nor2   g002(.a(new_n64_), .b(x35), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  nand2  g004(.a(x37), .b(x27), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  nand4  g006(.a(x40), .b(x39), .c(x10), .d(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n67_), .c(x15), .O(new_n70_));
  oai21  g008(.a(new_n70_), .b(new_n63_), .c(new_n66_), .O(new_n71_));
  inv1   g009(.a(x15), .O(new_n72_));
  inv1   g010(.a(x28), .O(new_n73_));
  nand2  g011(.a(x40), .b(x39), .O(new_n74_));
  inv1   g012(.a(new_n74_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n68_), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nand2  g015(.a(x35), .b(x27), .O(new_n78_));
  inv1   g016(.a(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(new_n77_), .c(new_n72_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n71_), .O(z00));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n65_), .O(new_n83_));
  aoi21  g021(.a(new_n64_), .b(x28), .c(new_n78_), .O(new_n84_));
  oai21  g022(.a(new_n84_), .b(new_n83_), .c(x04), .O(z01));
  nor2   g023(.a(x27), .b(x08), .O(new_n86_));
  oai21  g024(.a(new_n86_), .b(x28), .c(x04), .O(new_n87_));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g028(.a(x35), .O(new_n91_));
  oai21  g029(.a(x36), .b(x04), .c(new_n91_), .O(new_n92_));
  nand4  g030(.a(new_n92_), .b(new_n90_), .c(new_n87_), .d(new_n75_), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n91_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n65_), .O(z03));
  inv1   g035(.a(x21), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n66_), .c(new_n98_), .O(z04));
  nand2  g037(.a(new_n95_), .b(new_n94_), .O(new_n100_));
  inv1   g038(.a(new_n100_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n66_), .O(z05));
  nand2  g040(.a(new_n100_), .b(new_n66_), .O(z06));
  inv1   g041(.a(x14), .O(new_n104_));
  nand2  g042(.a(x17), .b(new_n72_), .O(new_n105_));
  nor2   g043(.a(x33), .b(x31), .O(new_n106_));
  aoi21  g044(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  inv1   g045(.a(x25), .O(new_n108_));
  nand2  g046(.a(new_n108_), .b(x00), .O(new_n109_));
  nand2  g047(.a(x25), .b(new_n104_), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n109_), .c(x38), .O(new_n111_));
  nand3  g049(.a(new_n111_), .b(new_n66_), .c(x03), .O(new_n112_));
  nor2   g050(.a(new_n112_), .b(new_n107_), .O(z07));
  nand2  g051(.a(new_n74_), .b(new_n66_), .O(z08));
  inv1   g052(.a(x11), .O(new_n115_));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  nor3   g054(.a(new_n116_), .b(new_n65_), .c(new_n115_), .O(z09));
  nand3  g055(.a(x40), .b(x39), .c(x05), .O(new_n118_));
  nor2   g056(.a(new_n118_), .b(new_n82_), .O(new_n119_));
  nand2  g057(.a(new_n74_), .b(x07), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  inv1   g059(.a(x27), .O(new_n122_));
  nand2  g060(.a(x35), .b(new_n73_), .O(new_n123_));
  aoi21  g061(.a(new_n123_), .b(new_n64_), .c(new_n122_), .O(new_n124_));
  aoi21  g062(.a(new_n124_), .b(new_n121_), .c(new_n119_), .O(new_n125_));
  inv1   g063(.a(new_n67_), .O(new_n126_));
  aoi21  g064(.a(new_n126_), .b(x06), .c(new_n65_), .O(new_n127_));
  oai21  g065(.a(new_n125_), .b(x04), .c(new_n127_), .O(z10));
  inv1   g066(.a(x08), .O(new_n129_));
  nand3  g067(.a(x40), .b(x39), .c(x29), .O(new_n130_));
  aoi21  g068(.a(new_n130_), .b(new_n123_), .c(new_n129_), .O(new_n131_));
  nand2  g069(.a(new_n130_), .b(x27), .O(new_n132_));
  nand2  g070(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  inv1   g071(.a(new_n123_), .O(new_n134_));
  nor2   g072(.a(x30), .b(x09), .O(new_n135_));
  inv1   g073(.a(new_n135_), .O(new_n136_));
  nor2   g074(.a(new_n122_), .b(x04), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n134_), .c(new_n136_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(new_n133_), .O(new_n139_));
  nand2  g077(.a(new_n139_), .b(new_n66_), .O(z11));
  inv1   g078(.a(x10), .O(new_n141_));
  aoi21  g079(.a(new_n78_), .b(new_n141_), .c(new_n76_), .O(new_n142_));
  nand2  g080(.a(x35), .b(x28), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n94_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(new_n66_), .O(new_n146_));
  nor2   g084(.a(new_n146_), .b(new_n142_), .O(z12));
  nor2   g085(.a(x19), .b(x18), .O(new_n148_));
  nand3  g086(.a(new_n148_), .b(x37), .c(x20), .O(new_n149_));
  nor2   g087(.a(x13), .b(x04), .O(new_n150_));
  nand4  g088(.a(new_n150_), .b(x40), .c(x39), .d(new_n73_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n149_), .c(new_n122_), .O(new_n152_));
  nand2  g090(.a(new_n150_), .b(new_n75_), .O(new_n153_));
  nor2   g091(.a(new_n153_), .b(new_n82_), .O(new_n154_));
  oai21  g092(.a(new_n154_), .b(new_n152_), .c(new_n66_), .O(new_n155_));
  nand3  g093(.a(new_n148_), .b(x28), .c(x20), .O(new_n156_));
  oai21  g094(.a(new_n153_), .b(new_n64_), .c(new_n156_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n79_), .O(new_n158_));
  nand2  g096(.a(new_n158_), .b(new_n155_), .O(z13));
  nand3  g097(.a(new_n150_), .b(new_n75_), .c(x27), .O(new_n160_));
  aoi21  g098(.a(new_n160_), .b(x35), .c(new_n64_), .O(new_n161_));
  nand3  g099(.a(new_n148_), .b(new_n144_), .c(x20), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n151_), .c(new_n122_), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n161_), .c(new_n154_), .O(z14));
  inv1   g102(.a(x12), .O(new_n165_));
  oai21  g103(.a(new_n116_), .b(new_n165_), .c(new_n66_), .O(z15));
  inv1   g104(.a(x01), .O(new_n167_));
  inv1   g105(.a(x22), .O(new_n168_));
  nor4   g106(.a(new_n65_), .b(x23), .c(new_n168_), .d(new_n167_), .O(z16));
  inv1   g107(.a(x24), .O(new_n170_));
  nor2   g108(.a(new_n168_), .b(new_n167_), .O(new_n171_));
  nand4  g109(.a(new_n171_), .b(new_n66_), .c(new_n170_), .d(x23), .O(new_n172_));
  inv1   g110(.a(new_n172_), .O(z17));
  nand3  g111(.a(new_n138_), .b(new_n133_), .c(new_n66_), .O(z18));
  inv1   g112(.a(new_n138_), .O(new_n175_));
  nand2  g113(.a(new_n134_), .b(new_n122_), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n130_), .c(new_n129_), .O(new_n177_));
  oai21  g115(.a(new_n177_), .b(new_n175_), .c(new_n66_), .O(z19));
  nand3  g116(.a(new_n135_), .b(new_n78_), .c(new_n66_), .O(new_n179_));
  nor2   g117(.a(new_n179_), .b(new_n131_), .O(z20));
endmodule


