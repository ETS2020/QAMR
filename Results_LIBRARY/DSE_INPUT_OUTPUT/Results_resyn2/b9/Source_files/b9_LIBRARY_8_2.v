// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n167_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n172_, new_n173_, new_n175_, new_n176_,
    new_n177_, new_n179_, new_n180_;
  inv1   g000(.a(x16), .O(new_n63_));
  inv1   g001(.a(x27), .O(new_n64_));
  nor2   g002(.a(x40), .b(new_n64_), .O(new_n65_));
  inv1   g003(.a(new_n65_), .O(new_n66_));
  nand2  g004(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  inv1   g005(.a(x15), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g009(.a(x39), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(x04), .O(new_n73_));
  inv1   g011(.a(x37), .O(new_n74_));
  nand2  g012(.a(x35), .b(x28), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g014(.a(new_n73_), .b(new_n71_), .c(new_n76_), .O(new_n77_));
  nand2  g015(.a(new_n73_), .b(x10), .O(new_n78_));
  oai21  g016(.a(new_n77_), .b(new_n64_), .c(new_n78_), .O(new_n79_));
  nand3  g017(.a(new_n79_), .b(x40), .c(new_n68_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(new_n67_), .O(z00));
  inv1   g019(.a(x28), .O(new_n82_));
  nand2  g020(.a(x35), .b(new_n82_), .O(new_n83_));
  nand2  g021(.a(new_n83_), .b(new_n70_), .O(new_n84_));
  nand2  g022(.a(new_n84_), .b(x04), .O(new_n85_));
  nand2  g023(.a(new_n85_), .b(x40), .O(new_n86_));
  or2    g024(.a(x32), .b(x30), .O(new_n87_));
  aoi22  g025(.a(new_n87_), .b(x04), .c(new_n86_), .d(x27), .O(z01));
  inv1   g026(.a(x04), .O(new_n89_));
  inv1   g027(.a(x08), .O(new_n90_));
  aoi21  g028(.a(new_n64_), .b(new_n90_), .c(new_n83_), .O(new_n91_));
  inv1   g029(.a(x40), .O(new_n92_));
  nor2   g030(.a(new_n92_), .b(new_n72_), .O(new_n93_));
  inv1   g031(.a(new_n93_), .O(new_n94_));
  aoi21  g032(.a(x29), .b(x08), .c(x02), .O(new_n95_));
  nor2   g033(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g034(.a(new_n91_), .b(new_n89_), .c(new_n96_), .O(z02));
  oai21  g035(.a(new_n75_), .b(new_n92_), .c(x27), .O(new_n98_));
  nand2  g036(.a(new_n74_), .b(new_n64_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n98_), .c(x21), .O(z03));
  inv1   g038(.a(x21), .O(new_n101_));
  nand3  g039(.a(new_n99_), .b(new_n98_), .c(new_n101_), .O(z04));
  oai21  g040(.a(new_n92_), .b(x28), .c(x27), .O(new_n103_));
  nand2  g041(.a(new_n103_), .b(new_n74_), .O(z05));
  oai21  g042(.a(new_n82_), .b(new_n64_), .c(new_n74_), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n66_), .O(z06));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x17), .b(new_n68_), .O(new_n108_));
  nor2   g046(.a(x33), .b(x31), .O(new_n109_));
  aoi21  g047(.a(new_n109_), .b(new_n108_), .c(new_n107_), .O(new_n110_));
  inv1   g048(.a(x25), .O(new_n111_));
  nand2  g049(.a(new_n111_), .b(x00), .O(new_n112_));
  nand2  g050(.a(x25), .b(new_n107_), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n112_), .c(x38), .O(new_n114_));
  nand3  g052(.a(new_n114_), .b(new_n66_), .c(x03), .O(new_n115_));
  nor2   g053(.a(new_n115_), .b(new_n110_), .O(z07));
  nand2  g054(.a(new_n94_), .b(new_n66_), .O(z08));
  nand3  g055(.a(x34), .b(x26), .c(x11), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(x40), .c(new_n64_), .O(z09));
  inv1   g057(.a(x07), .O(new_n120_));
  nand2  g058(.a(new_n72_), .b(new_n120_), .O(new_n121_));
  inv1   g059(.a(x05), .O(new_n122_));
  aoi21  g060(.a(x39), .b(new_n122_), .c(x04), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n121_), .c(new_n84_), .O(new_n124_));
  aoi21  g062(.a(x37), .b(x06), .c(new_n92_), .O(new_n125_));
  nand2  g063(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(x27), .O(new_n127_));
  nand2  g065(.a(new_n93_), .b(new_n87_), .O(new_n128_));
  nand2  g066(.a(x05), .b(new_n89_), .O(new_n129_));
  oai21  g067(.a(new_n129_), .b(new_n128_), .c(new_n127_), .O(z10));
  nand2  g068(.a(x27), .b(x04), .O(new_n131_));
  nand3  g069(.a(new_n93_), .b(x29), .c(x08), .O(new_n132_));
  nor2   g070(.a(x30), .b(x09), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(new_n66_), .O(new_n134_));
  aoi21  g072(.a(new_n131_), .b(new_n91_), .c(new_n134_), .O(z11));
  nand2  g073(.a(new_n79_), .b(x40), .O(z12));
  inv1   g074(.a(x13), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(new_n89_), .O(new_n138_));
  oai21  g076(.a(new_n70_), .b(new_n69_), .c(x28), .O(new_n139_));
  nand3  g077(.a(x39), .b(new_n137_), .c(new_n89_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(new_n141_));
  nand2  g079(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  inv1   g080(.a(x18), .O(new_n143_));
  inv1   g081(.a(x19), .O(new_n144_));
  nand3  g082(.a(x20), .b(new_n144_), .c(new_n143_), .O(new_n145_));
  inv1   g083(.a(new_n145_), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n76_), .O(new_n147_));
  nand3  g085(.a(new_n147_), .b(new_n142_), .c(x40), .O(new_n148_));
  nand2  g086(.a(new_n148_), .b(x27), .O(new_n149_));
  oai21  g087(.a(new_n138_), .b(new_n128_), .c(new_n149_), .O(z13));
  nand2  g088(.a(new_n74_), .b(new_n69_), .O(new_n151_));
  aoi21  g089(.a(new_n151_), .b(new_n146_), .c(new_n139_), .O(new_n152_));
  nor2   g090(.a(new_n152_), .b(new_n64_), .O(new_n153_));
  nand3  g091(.a(new_n146_), .b(new_n76_), .c(x27), .O(new_n154_));
  aoi21  g092(.a(new_n154_), .b(new_n140_), .c(new_n92_), .O(new_n155_));
  oai21  g093(.a(new_n153_), .b(new_n87_), .c(new_n155_), .O(z14));
  nand2  g094(.a(x26), .b(x12), .O(new_n157_));
  nand3  g095(.a(x40), .b(x34), .c(x27), .O(new_n158_));
  nor2   g096(.a(new_n158_), .b(new_n157_), .O(z15));
  inv1   g097(.a(x01), .O(new_n160_));
  inv1   g098(.a(x22), .O(new_n161_));
  nor4   g099(.a(new_n65_), .b(x23), .c(new_n161_), .d(new_n160_), .O(z16));
  inv1   g100(.a(x24), .O(new_n163_));
  nor2   g101(.a(new_n161_), .b(new_n160_), .O(new_n164_));
  nand4  g102(.a(new_n164_), .b(new_n66_), .c(new_n163_), .d(x23), .O(new_n165_));
  inv1   g103(.a(new_n165_), .O(z17));
  nand3  g104(.a(x40), .b(x39), .c(x29), .O(new_n167_));
  inv1   g105(.a(new_n167_), .O(new_n168_));
  aoi21  g106(.a(new_n167_), .b(new_n83_), .c(new_n90_), .O(new_n169_));
  oai21  g107(.a(new_n168_), .b(new_n64_), .c(new_n169_), .O(new_n170_));
  nand3  g108(.a(x35), .b(new_n82_), .c(new_n89_), .O(new_n171_));
  inv1   g109(.a(new_n171_), .O(new_n172_));
  oai21  g110(.a(new_n172_), .b(new_n92_), .c(x27), .O(new_n173_));
  nand3  g111(.a(new_n173_), .b(new_n170_), .c(new_n133_), .O(z18));
  inv1   g112(.a(new_n170_), .O(new_n175_));
  nand2  g113(.a(new_n172_), .b(x27), .O(new_n176_));
  aoi21  g114(.a(new_n176_), .b(new_n133_), .c(new_n65_), .O(new_n177_));
  nor2   g115(.a(new_n177_), .b(new_n175_), .O(z19));
  oai21  g116(.a(new_n92_), .b(x35), .c(x27), .O(new_n179_));
  nand2  g117(.a(new_n179_), .b(new_n133_), .O(new_n180_));
  nor2   g118(.a(new_n180_), .b(new_n169_), .O(z20));
endmodule


