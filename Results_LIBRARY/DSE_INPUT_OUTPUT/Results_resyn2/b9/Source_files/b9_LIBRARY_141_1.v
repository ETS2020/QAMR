// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n178_, new_n180_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x21), .O(new_n71_));
  nand2  g009(.a(x40), .b(new_n71_), .O(new_n72_));
  and2   g010(.a(new_n72_), .b(x16), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  inv1   g019(.a(new_n72_), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(new_n64_), .O(new_n83_));
  oai21  g021(.a(new_n81_), .b(new_n79_), .c(new_n83_), .O(z01));
  inv1   g022(.a(x35), .O(new_n85_));
  nor2   g023(.a(new_n85_), .b(x28), .O(new_n86_));
  oai21  g024(.a(x27), .b(x08), .c(new_n86_), .O(new_n87_));
  inv1   g025(.a(x02), .O(new_n88_));
  nand2  g026(.a(x29), .b(x08), .O(new_n89_));
  nand2  g027(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g028(.a(new_n90_), .b(x39), .O(new_n91_));
  aoi21  g029(.a(new_n87_), .b(x04), .c(new_n91_), .O(new_n92_));
  oai21  g030(.a(new_n92_), .b(new_n71_), .c(x40), .O(z02));
  inv1   g031(.a(x37), .O(new_n94_));
  nand2  g032(.a(x28), .b(x27), .O(new_n95_));
  oai22  g033(.a(new_n95_), .b(new_n85_), .c(new_n94_), .d(x27), .O(new_n96_));
  aoi21  g034(.a(new_n96_), .b(x21), .c(new_n82_), .O(z03));
  inv1   g035(.a(x40), .O(new_n98_));
  nand3  g036(.a(new_n96_), .b(new_n98_), .c(new_n71_), .O(z04));
  nand3  g037(.a(new_n95_), .b(new_n72_), .c(new_n94_), .O(z05));
  inv1   g038(.a(z05), .O(z06));
  inv1   g039(.a(x14), .O(new_n102_));
  inv1   g040(.a(x15), .O(new_n103_));
  nand2  g041(.a(x17), .b(new_n103_), .O(new_n104_));
  nor2   g042(.a(x33), .b(x31), .O(new_n105_));
  aoi21  g043(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  inv1   g044(.a(x25), .O(new_n107_));
  nand2  g045(.a(new_n107_), .b(x00), .O(new_n108_));
  nand2  g046(.a(x25), .b(new_n102_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n108_), .c(x38), .O(new_n110_));
  nand3  g048(.a(new_n110_), .b(new_n72_), .c(x03), .O(new_n111_));
  nor2   g049(.a(new_n111_), .b(new_n106_), .O(z07));
  nand3  g050(.a(x40), .b(x39), .c(x21), .O(new_n113_));
  inv1   g051(.a(new_n113_), .O(z08));
  nand3  g052(.a(x34), .b(x27), .c(x26), .O(new_n115_));
  nand2  g053(.a(new_n72_), .b(x11), .O(new_n116_));
  nor2   g054(.a(new_n116_), .b(new_n115_), .O(z09));
  inv1   g055(.a(x39), .O(new_n118_));
  nor2   g056(.a(new_n98_), .b(new_n118_), .O(new_n119_));
  nand2  g057(.a(new_n119_), .b(new_n81_), .O(new_n120_));
  nand2  g058(.a(new_n78_), .b(new_n76_), .O(new_n121_));
  inv1   g059(.a(x07), .O(new_n122_));
  oai21  g060(.a(new_n98_), .b(new_n118_), .c(new_n122_), .O(new_n123_));
  nand3  g061(.a(new_n123_), .b(new_n121_), .c(x27), .O(new_n124_));
  nand2  g062(.a(new_n124_), .b(new_n120_), .O(new_n125_));
  inv1   g063(.a(x05), .O(new_n126_));
  aoi21  g064(.a(new_n119_), .b(new_n126_), .c(x04), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand3  g066(.a(x37), .b(x27), .c(x06), .O(new_n129_));
  nand3  g067(.a(new_n129_), .b(new_n128_), .c(new_n72_), .O(z10));
  nor2   g068(.a(x30), .b(x09), .O(new_n131_));
  inv1   g069(.a(new_n131_), .O(new_n132_));
  inv1   g070(.a(x08), .O(new_n133_));
  nand3  g071(.a(new_n131_), .b(new_n133_), .c(x04), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(x21), .O(new_n135_));
  nand2  g073(.a(new_n135_), .b(x40), .O(new_n136_));
  nand3  g074(.a(x40), .b(x39), .c(x29), .O(new_n137_));
  inv1   g075(.a(new_n137_), .O(new_n138_));
  nand2  g076(.a(new_n138_), .b(x08), .O(new_n139_));
  nand3  g077(.a(new_n137_), .b(x27), .c(x04), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(new_n141_));
  aoi21  g079(.a(new_n139_), .b(new_n87_), .c(new_n141_), .O(new_n142_));
  oai21  g080(.a(new_n142_), .b(new_n132_), .c(new_n136_), .O(z11));
  nand2  g081(.a(new_n63_), .b(new_n94_), .O(new_n144_));
  aoi21  g082(.a(new_n144_), .b(x27), .c(x10), .O(new_n145_));
  nor2   g083(.a(new_n118_), .b(x04), .O(new_n146_));
  nor2   g084(.a(new_n66_), .b(new_n75_), .O(new_n147_));
  nor2   g085(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  oai21  g086(.a(new_n148_), .b(new_n145_), .c(x21), .O(new_n149_));
  nor2   g087(.a(new_n147_), .b(x40), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  nand2  g089(.a(new_n151_), .b(new_n149_), .O(z12));
  inv1   g090(.a(new_n66_), .O(new_n153_));
  inv1   g091(.a(x18), .O(new_n154_));
  inv1   g092(.a(x19), .O(new_n155_));
  nand3  g093(.a(x20), .b(new_n155_), .c(new_n154_), .O(new_n156_));
  inv1   g094(.a(new_n156_), .O(new_n157_));
  nand3  g095(.a(new_n157_), .b(new_n72_), .c(new_n153_), .O(new_n158_));
  nor2   g096(.a(x13), .b(x04), .O(new_n159_));
  nand2  g097(.a(x36), .b(x35), .O(new_n160_));
  nand2  g098(.a(new_n160_), .b(x28), .O(new_n161_));
  nand4  g099(.a(new_n161_), .b(new_n159_), .c(new_n119_), .d(x21), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n158_), .O(new_n163_));
  nand2  g101(.a(new_n163_), .b(x27), .O(new_n164_));
  nand4  g102(.a(new_n159_), .b(new_n119_), .c(new_n81_), .d(x21), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n164_), .O(z13));
  nand3  g104(.a(new_n160_), .b(new_n80_), .c(x28), .O(new_n167_));
  inv1   g105(.a(x13), .O(new_n168_));
  nand3  g106(.a(x39), .b(new_n168_), .c(new_n64_), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(new_n170_));
  aoi21  g108(.a(new_n170_), .b(new_n167_), .c(new_n71_), .O(new_n171_));
  oai21  g109(.a(new_n171_), .b(new_n98_), .c(new_n156_), .O(new_n172_));
  oai21  g110(.a(x28), .b(new_n75_), .c(new_n80_), .O(new_n173_));
  nand2  g111(.a(new_n173_), .b(new_n170_), .O(new_n174_));
  nor2   g112(.a(new_n147_), .b(new_n71_), .O(new_n175_));
  aoi21  g113(.a(new_n175_), .b(new_n174_), .c(new_n150_), .O(new_n176_));
  nand2  g114(.a(new_n176_), .b(new_n172_), .O(z14));
  inv1   g115(.a(x12), .O(new_n178_));
  oai21  g116(.a(new_n115_), .b(new_n178_), .c(new_n72_), .O(z15));
  nand2  g117(.a(x22), .b(x01), .O(new_n180_));
  nor3   g118(.a(new_n180_), .b(new_n82_), .c(x23), .O(z16));
  inv1   g119(.a(x24), .O(new_n182_));
  nand2  g120(.a(new_n182_), .b(x23), .O(new_n183_));
  oai21  g121(.a(new_n183_), .b(new_n180_), .c(new_n72_), .O(z17));
  aoi21  g122(.a(new_n75_), .b(new_n133_), .c(new_n78_), .O(new_n185_));
  nand2  g123(.a(x27), .b(x04), .O(new_n186_));
  aoi21  g124(.a(new_n186_), .b(new_n185_), .c(new_n132_), .O(new_n187_));
  oai22  g125(.a(new_n187_), .b(new_n82_), .c(new_n113_), .d(new_n89_), .O(z18));
  aoi21  g126(.a(new_n137_), .b(new_n78_), .c(new_n133_), .O(new_n189_));
  nand2  g127(.a(new_n137_), .b(x27), .O(new_n190_));
  nand3  g128(.a(new_n86_), .b(x27), .c(new_n64_), .O(new_n191_));
  nand3  g129(.a(new_n191_), .b(new_n131_), .c(new_n72_), .O(new_n192_));
  aoi21  g130(.a(new_n190_), .b(new_n189_), .c(new_n192_), .O(z19));
  oai21  g131(.a(new_n85_), .b(new_n75_), .c(new_n131_), .O(new_n194_));
  oai21  g132(.a(new_n194_), .b(new_n189_), .c(new_n72_), .O(z20));
endmodule


