// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n99_, new_n101_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n178_;
  nor2   g000(.a(x36), .b(x35), .O(new_n63_));
  inv1   g001(.a(x04), .O(new_n64_));
  nand3  g002(.a(x40), .b(x39), .c(new_n64_), .O(new_n65_));
  aoi21  g003(.a(x35), .b(x28), .c(x37), .O(new_n66_));
  oai21  g004(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  nand4  g005(.a(x40), .b(x39), .c(x10), .d(new_n64_), .O(new_n68_));
  inv1   g006(.a(new_n68_), .O(new_n69_));
  aoi21  g007(.a(new_n67_), .b(x27), .c(new_n69_), .O(new_n70_));
  inv1   g008(.a(x16), .O(new_n71_));
  nor2   g009(.a(x40), .b(x38), .O(new_n72_));
  nor2   g010(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g011(.a(new_n70_), .b(x15), .c(new_n73_), .O(z00));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x36), .O(new_n76_));
  inv1   g014(.a(x28), .O(new_n77_));
  nand2  g015(.a(x35), .b(new_n77_), .O(new_n78_));
  aoi21  g016(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  nor2   g017(.a(x32), .b(x30), .O(new_n80_));
  inv1   g018(.a(new_n80_), .O(new_n81_));
  nor2   g019(.a(new_n72_), .b(new_n64_), .O(new_n82_));
  oai21  g020(.a(new_n81_), .b(new_n79_), .c(new_n82_), .O(z01));
  inv1   g021(.a(x08), .O(new_n84_));
  aoi21  g022(.a(new_n75_), .b(new_n84_), .c(new_n78_), .O(new_n85_));
  inv1   g023(.a(x02), .O(new_n86_));
  nand2  g024(.a(x40), .b(x39), .O(new_n87_));
  nand2  g025(.a(x29), .b(x08), .O(new_n88_));
  aoi21  g026(.a(new_n88_), .b(new_n86_), .c(new_n87_), .O(new_n89_));
  oai21  g027(.a(new_n85_), .b(new_n64_), .c(new_n89_), .O(z02));
  nand2  g028(.a(x35), .b(x28), .O(new_n91_));
  nand2  g029(.a(new_n91_), .b(x27), .O(new_n92_));
  inv1   g030(.a(x37), .O(new_n93_));
  aoi21  g031(.a(new_n93_), .b(new_n75_), .c(new_n72_), .O(new_n94_));
  nand3  g032(.a(new_n94_), .b(new_n92_), .c(x21), .O(z03));
  inv1   g033(.a(x21), .O(new_n96_));
  nand3  g034(.a(new_n94_), .b(new_n92_), .c(new_n96_), .O(z04));
  inv1   g035(.a(new_n72_), .O(new_n98_));
  aoi21  g036(.a(x28), .b(x27), .c(x37), .O(new_n99_));
  nand2  g037(.a(new_n99_), .b(new_n98_), .O(z05));
  inv1   g038(.a(new_n99_), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(new_n98_), .O(z06));
  inv1   g040(.a(x25), .O(new_n103_));
  oai21  g041(.a(new_n103_), .b(x14), .c(x38), .O(new_n104_));
  aoi21  g042(.a(new_n103_), .b(x00), .c(new_n104_), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x15), .O(new_n107_));
  or2    g045(.a(x33), .b(x31), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(new_n107_), .c(x14), .O(new_n109_));
  nand2  g047(.a(new_n109_), .b(x03), .O(new_n110_));
  oai21  g048(.a(new_n110_), .b(new_n105_), .c(new_n98_), .O(z07));
  inv1   g049(.a(new_n87_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n98_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  nor2   g053(.a(new_n87_), .b(new_n80_), .O(new_n116_));
  inv1   g054(.a(x07), .O(new_n117_));
  nand2  g055(.a(new_n87_), .b(new_n117_), .O(new_n118_));
  aoi21  g056(.a(new_n118_), .b(new_n79_), .c(new_n116_), .O(new_n119_));
  oai21  g057(.a(new_n87_), .b(x05), .c(new_n64_), .O(new_n120_));
  nand3  g058(.a(x37), .b(x27), .c(x06), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  nor2   g060(.a(new_n122_), .b(new_n72_), .O(new_n123_));
  oai21  g061(.a(new_n120_), .b(new_n119_), .c(new_n123_), .O(z10));
  nand3  g062(.a(x40), .b(x39), .c(x29), .O(new_n125_));
  aoi21  g063(.a(new_n125_), .b(new_n78_), .c(new_n84_), .O(new_n126_));
  nand2  g064(.a(new_n125_), .b(x27), .O(new_n127_));
  nand2  g065(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  inv1   g066(.a(x35), .O(new_n129_));
  nor2   g067(.a(new_n129_), .b(x28), .O(new_n130_));
  nand3  g068(.a(new_n130_), .b(x27), .c(new_n64_), .O(new_n131_));
  nor3   g069(.a(new_n72_), .b(x30), .c(x09), .O(new_n132_));
  nand3  g070(.a(new_n132_), .b(new_n131_), .c(new_n128_), .O(z18));
  inv1   g071(.a(z18), .O(z11));
  nand2  g072(.a(new_n63_), .b(new_n93_), .O(new_n135_));
  aoi21  g073(.a(new_n135_), .b(x27), .c(x10), .O(new_n136_));
  nand2  g074(.a(new_n91_), .b(new_n93_), .O(new_n137_));
  aoi22  g075(.a(new_n137_), .b(x27), .c(x39), .d(new_n64_), .O(new_n138_));
  oai21  g076(.a(new_n138_), .b(new_n136_), .c(x40), .O(new_n139_));
  inv1   g077(.a(x40), .O(new_n140_));
  nand2  g078(.a(new_n137_), .b(x27), .O(new_n141_));
  nand3  g079(.a(new_n141_), .b(new_n140_), .c(x38), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n139_), .O(z12));
  inv1   g081(.a(x18), .O(new_n144_));
  inv1   g082(.a(x19), .O(new_n145_));
  nand3  g083(.a(x20), .b(new_n145_), .c(new_n144_), .O(new_n146_));
  inv1   g084(.a(new_n146_), .O(new_n147_));
  oai21  g085(.a(new_n76_), .b(new_n129_), .c(x28), .O(new_n148_));
  inv1   g086(.a(x13), .O(new_n149_));
  nand4  g087(.a(x40), .b(x39), .c(new_n149_), .d(new_n64_), .O(new_n150_));
  inv1   g088(.a(new_n150_), .O(new_n151_));
  aoi22  g089(.a(new_n151_), .b(new_n148_), .c(new_n147_), .d(new_n137_), .O(new_n152_));
  nor2   g090(.a(x13), .b(x04), .O(new_n153_));
  aoi21  g091(.a(new_n153_), .b(new_n116_), .c(new_n72_), .O(new_n154_));
  oai21  g092(.a(new_n152_), .b(new_n75_), .c(new_n154_), .O(z13));
  oai21  g093(.a(new_n148_), .b(new_n81_), .c(new_n151_), .O(new_n156_));
  nor2   g094(.a(new_n147_), .b(new_n72_), .O(new_n157_));
  nand2  g095(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  oai21  g096(.a(x28), .b(new_n75_), .c(new_n80_), .O(new_n159_));
  nand3  g097(.a(new_n159_), .b(new_n153_), .c(z08), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n141_), .c(new_n98_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n158_), .O(z14));
  inv1   g100(.a(x12), .O(new_n163_));
  nor2   g101(.a(new_n114_), .b(new_n163_), .O(z15));
  inv1   g102(.a(x23), .O(new_n165_));
  nand3  g103(.a(new_n165_), .b(x22), .c(x01), .O(new_n166_));
  nand2  g104(.a(new_n166_), .b(new_n98_), .O(z16));
  nor2   g105(.a(x24), .b(new_n165_), .O(new_n168_));
  nand4  g106(.a(new_n168_), .b(new_n98_), .c(x22), .d(x01), .O(new_n169_));
  inv1   g107(.a(new_n169_), .O(z17));
  inv1   g108(.a(new_n88_), .O(new_n171_));
  nor2   g109(.a(x30), .b(x09), .O(new_n172_));
  nand2  g110(.a(new_n75_), .b(new_n84_), .O(new_n173_));
  nand2  g111(.a(x27), .b(x04), .O(new_n174_));
  nand3  g112(.a(new_n174_), .b(new_n173_), .c(new_n130_), .O(new_n175_));
  nand2  g113(.a(new_n175_), .b(new_n172_), .O(new_n176_));
  aoi22  g114(.a(new_n176_), .b(new_n98_), .c(new_n171_), .d(z08), .O(z19));
  oai21  g115(.a(new_n129_), .b(new_n75_), .c(new_n132_), .O(new_n178_));
  nor2   g116(.a(new_n178_), .b(new_n126_), .O(z20));
endmodule


