// Benchmark "FAU" written by ABC on Sat Aug  8 23:39:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n151_, new_n153_, new_n155_,
    new_n156_, new_n157_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n167_, new_n169_, new_n170_;
  nor2   g000(.a(x40), .b(x02), .O(new_n63_));
  inv1   g001(.a(x15), .O(new_n64_));
  inv1   g002(.a(x16), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  inv1   g004(.a(x37), .O(new_n67_));
  nand2  g005(.a(x35), .b(x28), .O(new_n68_));
  aoi21  g006(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g007(.a(new_n69_), .b(new_n64_), .c(new_n65_), .O(new_n70_));
  inv1   g008(.a(x10), .O(new_n71_));
  oai21  g009(.a(x36), .b(x35), .c(x27), .O(new_n72_));
  inv1   g010(.a(x04), .O(new_n73_));
  nand3  g011(.a(x40), .b(x39), .c(new_n73_), .O(new_n74_));
  aoi21  g012(.a(new_n72_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nand2  g013(.a(new_n75_), .b(new_n64_), .O(new_n76_));
  oai21  g014(.a(new_n70_), .b(new_n63_), .c(new_n76_), .O(z00));
  inv1   g015(.a(x35), .O(new_n78_));
  inv1   g016(.a(x36), .O(new_n79_));
  oai21  g017(.a(new_n78_), .b(x28), .c(new_n79_), .O(new_n80_));
  nand2  g018(.a(new_n80_), .b(x27), .O(new_n81_));
  nor2   g019(.a(x32), .b(x30), .O(new_n82_));
  nand2  g020(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g021(.a(new_n83_), .b(x04), .c(new_n63_), .O(z01));
  inv1   g022(.a(x08), .O(new_n85_));
  inv1   g023(.a(x28), .O(new_n86_));
  nand2  g024(.a(x35), .b(new_n86_), .O(new_n87_));
  aoi21  g025(.a(new_n66_), .b(new_n85_), .c(new_n87_), .O(new_n88_));
  nand2  g026(.a(x40), .b(x39), .O(new_n89_));
  aoi21  g027(.a(x29), .b(x08), .c(x02), .O(new_n90_));
  nor2   g028(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g029(.a(new_n88_), .b(new_n73_), .c(new_n91_), .O(z02));
  nand2  g030(.a(x28), .b(x27), .O(new_n93_));
  oai22  g031(.a(new_n93_), .b(new_n78_), .c(new_n67_), .d(x27), .O(new_n94_));
  aoi21  g032(.a(new_n94_), .b(x21), .c(new_n63_), .O(z03));
  nor2   g033(.a(new_n63_), .b(x21), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n94_), .O(z04));
  aoi21  g035(.a(new_n93_), .b(new_n67_), .c(new_n63_), .O(z05));
  inv1   g036(.a(z05), .O(z06));
  inv1   g037(.a(new_n63_), .O(new_n100_));
  inv1   g038(.a(x25), .O(new_n101_));
  nand2  g039(.a(new_n101_), .b(x00), .O(new_n102_));
  inv1   g040(.a(x14), .O(new_n103_));
  nand2  g041(.a(x25), .b(new_n103_), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n102_), .c(x38), .O(new_n105_));
  inv1   g043(.a(x17), .O(new_n106_));
  nor2   g044(.a(new_n106_), .b(x15), .O(new_n107_));
  or2    g045(.a(x33), .b(x31), .O(new_n108_));
  oai21  g046(.a(new_n108_), .b(new_n107_), .c(x14), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n105_), .c(x03), .O(new_n110_));
  nand2  g048(.a(new_n110_), .b(new_n100_), .O(z07));
  inv1   g049(.a(new_n89_), .O(z08));
  inv1   g050(.a(x11), .O(new_n113_));
  nand4  g051(.a(new_n100_), .b(x34), .c(x27), .d(x26), .O(new_n114_));
  nor2   g052(.a(new_n114_), .b(new_n113_), .O(z09));
  inv1   g053(.a(new_n82_), .O(new_n116_));
  nand2  g054(.a(z08), .b(new_n116_), .O(new_n117_));
  inv1   g055(.a(x07), .O(new_n118_));
  nand2  g056(.a(new_n89_), .b(new_n118_), .O(new_n119_));
  nand3  g057(.a(new_n119_), .b(new_n80_), .c(x27), .O(new_n120_));
  nand2  g058(.a(new_n120_), .b(new_n117_), .O(new_n121_));
  inv1   g059(.a(x05), .O(new_n122_));
  aoi21  g060(.a(z08), .b(new_n122_), .c(x04), .O(new_n123_));
  nand2  g061(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  nand3  g063(.a(new_n125_), .b(new_n124_), .c(new_n100_), .O(z10));
  nand3  g064(.a(x40), .b(x39), .c(x29), .O(new_n127_));
  aoi21  g065(.a(new_n127_), .b(new_n87_), .c(new_n85_), .O(new_n128_));
  nand2  g066(.a(new_n127_), .b(x27), .O(new_n129_));
  nand2  g067(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nor2   g068(.a(new_n78_), .b(x28), .O(new_n131_));
  nand3  g069(.a(new_n131_), .b(x27), .c(new_n73_), .O(new_n132_));
  nor2   g070(.a(x30), .b(x09), .O(new_n133_));
  nand3  g071(.a(new_n133_), .b(new_n132_), .c(new_n130_), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n100_), .O(z11));
  nor3   g073(.a(new_n75_), .b(new_n69_), .c(new_n63_), .O(z12));
  nand2  g074(.a(x36), .b(x35), .O(new_n137_));
  nand2  g075(.a(new_n137_), .b(x28), .O(new_n138_));
  nor2   g076(.a(x13), .b(x04), .O(new_n139_));
  nand3  g077(.a(new_n139_), .b(new_n138_), .c(z08), .O(new_n140_));
  nand2  g078(.a(new_n68_), .b(new_n67_), .O(new_n141_));
  nor2   g079(.a(x19), .b(x18), .O(new_n142_));
  nand3  g080(.a(new_n142_), .b(new_n141_), .c(x20), .O(new_n143_));
  nand2  g081(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand2  g082(.a(new_n144_), .b(x27), .O(new_n145_));
  nand3  g083(.a(new_n139_), .b(z08), .c(new_n116_), .O(new_n146_));
  aoi21  g084(.a(new_n146_), .b(new_n145_), .c(new_n63_), .O(z13));
  aoi21  g085(.a(new_n143_), .b(new_n140_), .c(new_n66_), .O(new_n148_));
  inv1   g086(.a(new_n146_), .O(new_n149_));
  oai21  g087(.a(new_n149_), .b(new_n148_), .c(new_n100_), .O(z14));
  inv1   g088(.a(x12), .O(new_n151_));
  nor2   g089(.a(new_n114_), .b(new_n151_), .O(z15));
  nand2  g090(.a(x22), .b(x01), .O(new_n153_));
  nor3   g091(.a(new_n153_), .b(new_n63_), .c(x23), .O(z16));
  inv1   g092(.a(x23), .O(new_n155_));
  nor2   g093(.a(x24), .b(new_n155_), .O(new_n156_));
  nand4  g094(.a(new_n156_), .b(new_n100_), .c(x22), .d(x01), .O(new_n157_));
  inv1   g095(.a(new_n157_), .O(z17));
  nand2  g096(.a(new_n66_), .b(new_n85_), .O(new_n159_));
  nand2  g097(.a(x27), .b(x04), .O(new_n160_));
  nand3  g098(.a(new_n160_), .b(new_n159_), .c(new_n131_), .O(new_n161_));
  nand2  g099(.a(new_n161_), .b(new_n133_), .O(new_n162_));
  nand2  g100(.a(new_n162_), .b(new_n100_), .O(new_n163_));
  inv1   g101(.a(new_n127_), .O(new_n164_));
  nand2  g102(.a(new_n164_), .b(x08), .O(new_n165_));
  nand2  g103(.a(new_n165_), .b(new_n163_), .O(z18));
  inv1   g104(.a(new_n165_), .O(new_n167_));
  aoi21  g105(.a(new_n162_), .b(new_n100_), .c(new_n167_), .O(z19));
  nand2  g106(.a(x35), .b(x27), .O(new_n169_));
  nand3  g107(.a(new_n169_), .b(new_n133_), .c(new_n100_), .O(new_n170_));
  nor2   g108(.a(new_n170_), .b(new_n128_), .O(z20));
endmodule


