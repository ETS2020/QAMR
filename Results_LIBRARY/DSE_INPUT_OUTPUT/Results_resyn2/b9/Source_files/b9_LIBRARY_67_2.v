// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n151_, new_n153_, new_n155_,
    new_n156_, new_n158_, new_n159_, new_n161_, new_n162_;
  inv1   g000(.a(x09), .O(new_n63_));
  nor2   g001(.a(x14), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(x15), .O(new_n65_));
  inv1   g003(.a(x27), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  inv1   g005(.a(x04), .O(new_n68_));
  inv1   g006(.a(x35), .O(new_n69_));
  inv1   g007(.a(x36), .O(new_n70_));
  nand2  g008(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  and2   g009(.a(x40), .b(x39), .O(new_n72_));
  nand3  g010(.a(new_n72_), .b(new_n71_), .c(new_n68_), .O(new_n73_));
  aoi21  g011(.a(new_n73_), .b(new_n67_), .c(new_n66_), .O(new_n74_));
  nand2  g012(.a(x40), .b(x39), .O(new_n75_));
  nor2   g013(.a(new_n75_), .b(x04), .O(new_n76_));
  nand2  g014(.a(new_n76_), .b(x10), .O(new_n77_));
  inv1   g015(.a(new_n77_), .O(new_n78_));
  oai21  g016(.a(new_n78_), .b(new_n74_), .c(new_n65_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x16), .c(new_n64_), .O(z00));
  inv1   g018(.a(new_n64_), .O(new_n81_));
  inv1   g019(.a(x28), .O(new_n82_));
  aoi21  g020(.a(x35), .b(new_n82_), .c(x36), .O(new_n83_));
  nor2   g021(.a(x32), .b(x30), .O(new_n84_));
  oai21  g022(.a(new_n83_), .b(new_n66_), .c(new_n84_), .O(new_n85_));
  nand3  g023(.a(new_n85_), .b(new_n81_), .c(x04), .O(z01));
  nand2  g024(.a(x35), .b(new_n82_), .O(new_n87_));
  nor2   g025(.a(x27), .b(x08), .O(new_n88_));
  oai21  g026(.a(new_n88_), .b(new_n87_), .c(x04), .O(new_n89_));
  nor2   g027(.a(new_n75_), .b(new_n64_), .O(z08));
  inv1   g028(.a(x02), .O(new_n91_));
  nand2  g029(.a(x29), .b(x08), .O(new_n92_));
  nand2  g030(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g031(.a(new_n93_), .b(z08), .c(new_n89_), .O(z02));
  oai21  g032(.a(new_n69_), .b(new_n82_), .c(x27), .O(new_n95_));
  inv1   g033(.a(x37), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(new_n66_), .O(new_n97_));
  nand4  g035(.a(new_n97_), .b(new_n95_), .c(new_n81_), .d(x21), .O(z03));
  aoi21  g036(.a(new_n96_), .b(new_n66_), .c(x21), .O(new_n99_));
  aoi21  g037(.a(new_n99_), .b(new_n95_), .c(new_n64_), .O(z04));
  aoi21  g038(.a(x28), .b(x27), .c(x37), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(new_n64_), .O(z05));
  inv1   g040(.a(new_n101_), .O(new_n103_));
  nor2   g041(.a(new_n103_), .b(new_n64_), .O(z06));
  inv1   g042(.a(x25), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(x00), .O(new_n106_));
  inv1   g044(.a(x14), .O(new_n107_));
  nand2  g045(.a(x25), .b(new_n107_), .O(new_n108_));
  nand3  g046(.a(new_n108_), .b(new_n106_), .c(x38), .O(new_n109_));
  inv1   g047(.a(x17), .O(new_n110_));
  nor2   g048(.a(new_n110_), .b(x15), .O(new_n111_));
  or2    g049(.a(x33), .b(x31), .O(new_n112_));
  oai21  g050(.a(new_n112_), .b(new_n111_), .c(x14), .O(new_n113_));
  nand3  g051(.a(new_n113_), .b(new_n109_), .c(x03), .O(new_n114_));
  nand2  g052(.a(new_n114_), .b(new_n81_), .O(z07));
  nand3  g053(.a(x34), .b(x27), .c(x26), .O(new_n116_));
  inv1   g054(.a(new_n116_), .O(new_n117_));
  nand3  g055(.a(new_n117_), .b(new_n81_), .c(x11), .O(new_n118_));
  inv1   g056(.a(new_n118_), .O(z09));
  nor2   g057(.a(new_n83_), .b(new_n66_), .O(new_n120_));
  nand2  g058(.a(new_n72_), .b(x05), .O(new_n121_));
  inv1   g059(.a(new_n121_), .O(new_n122_));
  and2   g060(.a(new_n75_), .b(x07), .O(new_n123_));
  aoi22  g061(.a(new_n123_), .b(new_n120_), .c(new_n122_), .d(new_n85_), .O(new_n124_));
  nand3  g062(.a(x37), .b(x27), .c(x06), .O(new_n125_));
  inv1   g063(.a(new_n125_), .O(new_n126_));
  nor2   g064(.a(new_n126_), .b(new_n64_), .O(new_n127_));
  oai21  g065(.a(new_n124_), .b(x04), .c(new_n127_), .O(z10));
  nor2   g066(.a(new_n107_), .b(new_n63_), .O(new_n129_));
  inv1   g067(.a(x30), .O(new_n130_));
  inv1   g068(.a(x08), .O(new_n131_));
  nand3  g069(.a(x40), .b(x39), .c(x29), .O(new_n132_));
  aoi21  g070(.a(new_n132_), .b(new_n87_), .c(new_n131_), .O(new_n133_));
  nand2  g071(.a(new_n132_), .b(x27), .O(new_n134_));
  nand2  g072(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand4  g073(.a(x35), .b(new_n82_), .c(x27), .d(new_n68_), .O(new_n136_));
  nand3  g074(.a(new_n136_), .b(new_n135_), .c(new_n130_), .O(new_n137_));
  aoi21  g075(.a(new_n137_), .b(new_n63_), .c(new_n129_), .O(z11));
  inv1   g076(.a(new_n74_), .O(new_n139_));
  nand3  g077(.a(new_n77_), .b(new_n139_), .c(new_n81_), .O(new_n140_));
  inv1   g078(.a(new_n140_), .O(z12));
  nor2   g079(.a(x13), .b(x04), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(new_n72_), .O(new_n143_));
  aoi21  g081(.a(x36), .b(x35), .c(new_n82_), .O(new_n144_));
  nor2   g082(.a(x19), .b(x18), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x20), .O(new_n146_));
  oai22  g084(.a(new_n146_), .b(new_n67_), .c(new_n144_), .d(new_n143_), .O(new_n147_));
  oai21  g085(.a(new_n143_), .b(new_n84_), .c(new_n81_), .O(new_n148_));
  aoi21  g086(.a(new_n147_), .b(x27), .c(new_n148_), .O(z14));
  inv1   g087(.a(z14), .O(z13));
  nand3  g088(.a(new_n117_), .b(new_n81_), .c(x12), .O(new_n151_));
  inv1   g089(.a(new_n151_), .O(z15));
  nand2  g090(.a(x22), .b(x01), .O(new_n153_));
  nor3   g091(.a(new_n153_), .b(new_n64_), .c(x23), .O(z16));
  inv1   g092(.a(x24), .O(new_n155_));
  nand2  g093(.a(new_n155_), .b(x23), .O(new_n156_));
  oai21  g094(.a(new_n156_), .b(new_n153_), .c(new_n81_), .O(z17));
  inv1   g095(.a(new_n129_), .O(new_n158_));
  nand2  g096(.a(new_n137_), .b(new_n63_), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(new_n158_), .O(z18));
  oai21  g098(.a(new_n69_), .b(new_n66_), .c(new_n130_), .O(new_n161_));
  or2    g099(.a(new_n161_), .b(new_n133_), .O(new_n162_));
  aoi21  g100(.a(new_n162_), .b(new_n63_), .c(new_n129_), .O(z20));
  aoi21  g101(.a(new_n137_), .b(new_n63_), .c(new_n129_), .O(z19));
endmodule


