// Benchmark "FAU" written by ABC on Sat Aug  8 23:38:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20;
  wire new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n157_, new_n159_, new_n160_, new_n163_;
  inv1   g000(.a(x04), .O(new_n63_));
  nand2  g001(.a(x40), .b(new_n63_), .O(new_n64_));
  inv1   g002(.a(new_n64_), .O(new_n65_));
  nor2   g003(.a(x36), .b(x35), .O(new_n66_));
  aoi21  g004(.a(x35), .b(x28), .c(x37), .O(new_n67_));
  oai21  g005(.a(new_n64_), .b(new_n66_), .c(new_n67_), .O(new_n68_));
  aoi22  g006(.a(new_n68_), .b(x27), .c(new_n65_), .d(x10), .O(new_n69_));
  inv1   g007(.a(x39), .O(new_n70_));
  nand2  g008(.a(x40), .b(new_n70_), .O(new_n71_));
  and2   g009(.a(new_n71_), .b(x16), .O(new_n72_));
  oai21  g010(.a(new_n69_), .b(x15), .c(new_n72_), .O(z00));
  inv1   g011(.a(new_n71_), .O(new_n74_));
  inv1   g012(.a(x27), .O(new_n75_));
  inv1   g013(.a(x28), .O(new_n76_));
  aoi21  g014(.a(x35), .b(new_n76_), .c(x36), .O(new_n77_));
  nor2   g015(.a(x32), .b(x30), .O(new_n78_));
  oai21  g016(.a(new_n77_), .b(new_n75_), .c(new_n78_), .O(new_n79_));
  aoi21  g017(.a(new_n79_), .b(x04), .c(new_n74_), .O(z01));
  aoi21  g018(.a(x29), .b(x08), .c(x02), .O(new_n81_));
  inv1   g019(.a(x35), .O(new_n82_));
  nor2   g020(.a(new_n82_), .b(x28), .O(new_n83_));
  oai21  g021(.a(x27), .b(x08), .c(new_n83_), .O(new_n84_));
  aoi21  g022(.a(new_n84_), .b(x04), .c(new_n81_), .O(new_n85_));
  oai21  g023(.a(new_n85_), .b(new_n70_), .c(x40), .O(z02));
  inv1   g024(.a(x37), .O(new_n87_));
  nand2  g025(.a(x28), .b(x27), .O(new_n88_));
  oai22  g026(.a(new_n88_), .b(new_n82_), .c(new_n87_), .d(x27), .O(new_n89_));
  aoi21  g027(.a(new_n89_), .b(x21), .c(new_n74_), .O(z03));
  inv1   g028(.a(x21), .O(new_n91_));
  aoi21  g029(.a(new_n89_), .b(new_n91_), .c(new_n74_), .O(z04));
  nand3  g030(.a(new_n88_), .b(new_n71_), .c(new_n87_), .O(z05));
  nand2  g031(.a(new_n88_), .b(new_n87_), .O(new_n94_));
  nand2  g032(.a(new_n94_), .b(new_n71_), .O(z06));
  inv1   g033(.a(x25), .O(new_n96_));
  nand2  g034(.a(new_n96_), .b(x00), .O(new_n97_));
  inv1   g035(.a(x14), .O(new_n98_));
  nand2  g036(.a(x25), .b(new_n98_), .O(new_n99_));
  nand3  g037(.a(new_n99_), .b(new_n97_), .c(x38), .O(new_n100_));
  inv1   g038(.a(x17), .O(new_n101_));
  nor2   g039(.a(new_n101_), .b(x15), .O(new_n102_));
  or2    g040(.a(x33), .b(x31), .O(new_n103_));
  oai21  g041(.a(new_n103_), .b(new_n102_), .c(x14), .O(new_n104_));
  nand3  g042(.a(new_n104_), .b(new_n100_), .c(x03), .O(new_n105_));
  nand2  g043(.a(new_n105_), .b(new_n71_), .O(z07));
  and2   g044(.a(x40), .b(x39), .O(z08));
  nand3  g045(.a(x34), .b(x27), .c(x26), .O(new_n108_));
  inv1   g046(.a(new_n108_), .O(new_n109_));
  nand3  g047(.a(new_n109_), .b(new_n71_), .c(x11), .O(new_n110_));
  inv1   g048(.a(new_n110_), .O(z09));
  nor2   g049(.a(new_n77_), .b(new_n75_), .O(new_n112_));
  and2   g050(.a(x40), .b(x05), .O(new_n113_));
  inv1   g051(.a(x07), .O(new_n114_));
  nor2   g052(.a(x40), .b(new_n114_), .O(new_n115_));
  aoi22  g053(.a(new_n115_), .b(new_n112_), .c(new_n113_), .d(new_n79_), .O(new_n116_));
  nand3  g054(.a(x37), .b(x27), .c(x06), .O(new_n117_));
  and2   g055(.a(new_n117_), .b(new_n71_), .O(new_n118_));
  oai21  g056(.a(new_n116_), .b(x04), .c(new_n118_), .O(z10));
  inv1   g057(.a(x08), .O(new_n120_));
  inv1   g058(.a(new_n83_), .O(new_n121_));
  nand2  g059(.a(x40), .b(x29), .O(new_n122_));
  aoi21  g060(.a(new_n122_), .b(new_n121_), .c(new_n120_), .O(new_n123_));
  nand2  g061(.a(new_n122_), .b(x27), .O(new_n124_));
  nand3  g062(.a(new_n83_), .b(x27), .c(new_n63_), .O(new_n125_));
  nor2   g063(.a(x30), .b(x09), .O(new_n126_));
  nand2  g064(.a(new_n126_), .b(new_n71_), .O(new_n127_));
  inv1   g065(.a(new_n127_), .O(new_n128_));
  nand2  g066(.a(new_n128_), .b(new_n125_), .O(new_n129_));
  aoi21  g067(.a(new_n124_), .b(new_n123_), .c(new_n129_), .O(z11));
  nor2   g068(.a(new_n67_), .b(new_n75_), .O(new_n131_));
  nor2   g069(.a(new_n66_), .b(new_n75_), .O(new_n132_));
  oai21  g070(.a(new_n132_), .b(x10), .c(new_n63_), .O(new_n133_));
  nand2  g071(.a(new_n133_), .b(x39), .O(new_n134_));
  aoi21  g072(.a(new_n134_), .b(x40), .c(new_n131_), .O(z12));
  inv1   g073(.a(x18), .O(new_n136_));
  inv1   g074(.a(x19), .O(new_n137_));
  nand3  g075(.a(x20), .b(new_n137_), .c(new_n136_), .O(new_n138_));
  aoi21  g076(.a(x36), .b(x35), .c(new_n76_), .O(new_n139_));
  inv1   g077(.a(x13), .O(new_n140_));
  nand3  g078(.a(x40), .b(new_n140_), .c(new_n63_), .O(new_n141_));
  oai22  g079(.a(new_n141_), .b(new_n139_), .c(new_n138_), .d(new_n67_), .O(new_n142_));
  nand2  g080(.a(new_n142_), .b(x27), .O(new_n143_));
  nand2  g081(.a(new_n140_), .b(new_n63_), .O(new_n144_));
  oai21  g082(.a(new_n144_), .b(new_n78_), .c(x39), .O(new_n145_));
  nand2  g083(.a(new_n145_), .b(x40), .O(new_n146_));
  nand2  g084(.a(new_n146_), .b(new_n143_), .O(z13));
  inv1   g085(.a(new_n78_), .O(new_n148_));
  inv1   g086(.a(new_n138_), .O(new_n149_));
  oai21  g087(.a(x37), .b(x35), .c(new_n149_), .O(new_n150_));
  aoi21  g088(.a(new_n150_), .b(new_n139_), .c(new_n75_), .O(new_n151_));
  nand2  g089(.a(new_n149_), .b(new_n131_), .O(new_n152_));
  aoi21  g090(.a(new_n152_), .b(new_n141_), .c(new_n74_), .O(new_n153_));
  oai21  g091(.a(new_n151_), .b(new_n148_), .c(new_n153_), .O(z14));
  nand3  g092(.a(new_n109_), .b(new_n71_), .c(x12), .O(new_n155_));
  inv1   g093(.a(new_n155_), .O(z15));
  nand2  g094(.a(x22), .b(x01), .O(new_n157_));
  nor3   g095(.a(new_n157_), .b(new_n74_), .c(x23), .O(z16));
  inv1   g096(.a(x24), .O(new_n159_));
  nand2  g097(.a(new_n159_), .b(x23), .O(new_n160_));
  oai21  g098(.a(new_n160_), .b(new_n157_), .c(new_n71_), .O(z17));
  inv1   g099(.a(z11), .O(z18));
  nor2   g100(.a(new_n82_), .b(new_n75_), .O(new_n163_));
  nor3   g101(.a(new_n163_), .b(new_n127_), .c(new_n123_), .O(z20));
  aoi21  g102(.a(new_n124_), .b(new_n123_), .c(new_n129_), .O(z19));
endmodule


