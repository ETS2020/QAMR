// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:55 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n36_, new_n37_, new_n39_, new_n41_, new_n43_,
    new_n45_, new_n46_, new_n48_;
  nand2  g00(.a(x2), .b(x0), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(z00));
  inv1   g02(.a(x0), .O(new_n24_));
  inv1   g03(.a(x2), .O(new_n25_));
  nand3  g04(.a(x4), .b(x3), .c(x1), .O(new_n26_));
  aoi21  g05(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(z02));
  inv1   g06(.a(x1), .O(new_n28_));
  inv1   g07(.a(x3), .O(new_n29_));
  nand4  g08(.a(x4), .b(new_n29_), .c(new_n25_), .d(x0), .O(new_n30_));
  nor2   g09(.a(new_n30_), .b(new_n28_), .O(z03));
  nand3  g10(.a(x4), .b(x3), .c(new_n25_), .O(new_n32_));
  nand2  g11(.a(new_n28_), .b(x0), .O(new_n33_));
  nor2   g12(.a(new_n33_), .b(new_n32_), .O(z06));
  nor2   g13(.a(new_n30_), .b(x1), .O(z07));
  nand2  g14(.a(x4), .b(x3), .O(new_n36_));
  nand3  g15(.a(x2), .b(x1), .c(new_n24_), .O(new_n37_));
  nor2   g16(.a(new_n37_), .b(new_n36_), .O(z08));
  nand2  g17(.a(x4), .b(new_n29_), .O(new_n39_));
  nor2   g18(.a(new_n39_), .b(new_n37_), .O(z09));
  nand2  g19(.a(x1), .b(new_n24_), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n32_), .c(new_n22_), .O(z10));
  nand3  g21(.a(x4), .b(new_n29_), .c(new_n25_), .O(new_n43_));
  nor2   g22(.a(new_n41_), .b(new_n43_), .O(z11));
  nand2  g23(.a(new_n28_), .b(new_n24_), .O(new_n45_));
  nand3  g24(.a(x4), .b(x3), .c(x2), .O(new_n46_));
  nor2   g25(.a(new_n46_), .b(new_n45_), .O(z12));
  nand3  g26(.a(x4), .b(new_n29_), .c(new_n28_), .O(new_n48_));
  aoi21  g27(.a(new_n48_), .b(new_n24_), .c(new_n25_), .O(z13));
  oai21  g28(.a(new_n45_), .b(new_n32_), .c(new_n22_), .O(z14));
  nor2   g29(.a(new_n45_), .b(new_n43_), .O(z15));
  inv1   g30(.a(new_n22_), .O(z01));
  inv1   g31(.a(new_n22_), .O(z04));
  inv1   g32(.a(new_n22_), .O(z05));
endmodule


