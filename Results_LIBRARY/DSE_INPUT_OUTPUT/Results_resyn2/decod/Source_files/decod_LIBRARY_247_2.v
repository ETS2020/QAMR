// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:56 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n35_, new_n38_, new_n42_, new_n43_, new_n46_, new_n48_,
    new_n49_, new_n52_;
  nand2  g00(.a(x1), .b(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x3), .c(x2), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(z00));
  inv1   g03(.a(x2), .O(new_n26_));
  nand3  g04(.a(x4), .b(x3), .c(new_n26_), .O(new_n27_));
  nor2   g05(.a(new_n27_), .b(new_n22_), .O(z02));
  inv1   g06(.a(x3), .O(new_n29_));
  nand2  g07(.a(new_n29_), .b(x1), .O(new_n30_));
  inv1   g08(.a(new_n30_), .O(z03));
  inv1   g09(.a(x1), .O(new_n32_));
  nand2  g10(.a(new_n32_), .b(x0), .O(new_n33_));
  oai21  g11(.a(new_n33_), .b(new_n23_), .c(new_n30_), .O(z04));
  nand3  g12(.a(x4), .b(x2), .c(x0), .O(new_n35_));
  aoi21  g13(.a(new_n35_), .b(new_n32_), .c(x3), .O(z05));
  nor2   g14(.a(new_n33_), .b(new_n27_), .O(z06));
  nand3  g15(.a(x4), .b(new_n26_), .c(x0), .O(new_n38_));
  aoi21  g16(.a(new_n38_), .b(new_n32_), .c(x3), .O(z07));
  nor3   g17(.a(new_n23_), .b(new_n32_), .c(x0), .O(z08));
  inv1   g18(.a(x0), .O(new_n42_));
  nand3  g19(.a(x4), .b(new_n26_), .c(new_n42_), .O(new_n43_));
  aoi21  g20(.a(new_n43_), .b(x3), .c(new_n32_), .O(z10));
  nand2  g21(.a(new_n32_), .b(new_n42_), .O(new_n46_));
  nor2   g22(.a(new_n46_), .b(new_n23_), .O(z12));
  nand2  g23(.a(x4), .b(new_n29_), .O(new_n48_));
  nand3  g24(.a(x2), .b(new_n32_), .c(new_n42_), .O(new_n49_));
  nor2   g25(.a(new_n49_), .b(new_n48_), .O(z13));
  nor2   g26(.a(new_n46_), .b(new_n27_), .O(z14));
  nand3  g27(.a(new_n26_), .b(new_n32_), .c(new_n42_), .O(new_n52_));
  nor2   g28(.a(new_n52_), .b(new_n48_), .O(z15));
  zero   g29(.O(z01));
  zero   g30(.O(z09));
  zero   g31(.O(z11));
endmodule


