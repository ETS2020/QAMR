// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:13 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n39_, new_n41_, new_n43_, new_n44_,
    new_n47_, new_n50_;
  inv1   g00(.a(x1), .O(new_n22_));
  nand3  g01(.a(x4), .b(x3), .c(x0), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x2), .c(new_n22_), .O(z00));
  inv1   g03(.a(x3), .O(new_n25_));
  nand4  g04(.a(x4), .b(new_n25_), .c(x2), .d(x0), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n22_), .O(z01));
  inv1   g06(.a(x2), .O(new_n29_));
  nand2  g07(.a(new_n29_), .b(x1), .O(new_n30_));
  inv1   g08(.a(new_n30_), .O(z03));
  inv1   g09(.a(x0), .O(new_n32_));
  nand3  g10(.a(x4), .b(x3), .c(x2), .O(new_n33_));
  nor3   g11(.a(new_n33_), .b(x1), .c(new_n32_), .O(z04));
  nor2   g12(.a(new_n26_), .b(x1), .O(z05));
  nand2  g13(.a(x4), .b(x3), .O(new_n36_));
  nand3  g14(.a(new_n29_), .b(new_n22_), .c(x0), .O(new_n37_));
  nor2   g15(.a(new_n37_), .b(new_n36_), .O(z06));
  nand2  g16(.a(x4), .b(new_n25_), .O(new_n39_));
  nor2   g17(.a(new_n39_), .b(new_n37_), .O(z07));
  nand3  g18(.a(x4), .b(x3), .c(new_n32_), .O(new_n41_));
  aoi21  g19(.a(new_n41_), .b(x2), .c(new_n22_), .O(z08));
  nand3  g20(.a(x4), .b(new_n25_), .c(x2), .O(new_n43_));
  nand2  g21(.a(x1), .b(new_n32_), .O(new_n44_));
  nor2   g22(.a(new_n44_), .b(new_n43_), .O(z09));
  nand2  g23(.a(new_n22_), .b(new_n32_), .O(new_n47_));
  oai21  g24(.a(new_n47_), .b(new_n33_), .c(new_n30_), .O(z12));
  oai21  g25(.a(new_n47_), .b(new_n43_), .c(new_n30_), .O(z13));
  nand3  g26(.a(new_n29_), .b(new_n22_), .c(new_n32_), .O(new_n50_));
  nor2   g27(.a(new_n50_), .b(new_n36_), .O(z14));
  nor2   g28(.a(new_n50_), .b(new_n39_), .O(z15));
  zero   g29(.O(z02));
  zero   g30(.O(z11));
  inv1   g31(.a(new_n30_), .O(z10));
endmodule


