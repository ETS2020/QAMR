// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:05 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n29_, new_n30_, new_n33_, new_n36_,
    new_n37_, new_n38_, new_n43_;
  nand2  g00(.a(x3), .b(x2), .O(new_n22_));
  nand3  g01(.a(x4), .b(x1), .c(x0), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(z00));
  inv1   g03(.a(x2), .O(new_n25_));
  nor3   g04(.a(new_n23_), .b(x3), .c(new_n25_), .O(z01));
  aoi21  g05(.a(new_n23_), .b(x3), .c(x2), .O(z02));
  nor2   g06(.a(x3), .b(x2), .O(z03));
  inv1   g07(.a(x1), .O(new_n29_));
  nand3  g08(.a(x4), .b(new_n29_), .c(x0), .O(new_n30_));
  nor2   g09(.a(new_n30_), .b(new_n22_), .O(z04));
  aoi21  g10(.a(new_n30_), .b(x2), .c(x3), .O(z05));
  nand2  g11(.a(x3), .b(new_n25_), .O(new_n33_));
  nor2   g12(.a(new_n33_), .b(new_n30_), .O(z06));
  inv1   g13(.a(x0), .O(new_n36_));
  nand3  g14(.a(x4), .b(x1), .c(new_n36_), .O(new_n37_));
  xor2a  g15(.a(x3), .b(x2), .O(new_n38_));
  aoi21  g16(.a(new_n37_), .b(x2), .c(new_n38_), .O(z08));
  aoi21  g17(.a(new_n37_), .b(x2), .c(x3), .O(z09));
  nor2   g18(.a(new_n37_), .b(new_n33_), .O(z10));
  nand3  g19(.a(x4), .b(new_n29_), .c(new_n36_), .O(new_n43_));
  nor2   g20(.a(new_n43_), .b(new_n22_), .O(z12));
  aoi21  g21(.a(new_n43_), .b(x2), .c(x3), .O(z13));
  nor2   g22(.a(new_n43_), .b(new_n33_), .O(z14));
  zero   g23(.O(z07));
  zero   g24(.O(z11));
  nor2   g25(.a(x3), .b(x2), .O(z15));
endmodule


