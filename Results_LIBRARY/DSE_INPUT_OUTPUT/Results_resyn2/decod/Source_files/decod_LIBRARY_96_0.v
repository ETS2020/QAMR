// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:21 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n40_, new_n42_, new_n43_,
    new_n46_, new_n49_, new_n52_, new_n54_;
  nand2  g00(.a(x1), .b(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x3), .c(x2), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(z00));
  inv1   g03(.a(x3), .O(new_n25_));
  nand3  g04(.a(x4), .b(new_n25_), .c(x2), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n22_), .O(z01));
  inv1   g06(.a(x2), .O(new_n28_));
  nand3  g07(.a(new_n28_), .b(x1), .c(x0), .O(new_n29_));
  nand2  g08(.a(x4), .b(x3), .O(new_n30_));
  nor2   g09(.a(new_n30_), .b(new_n29_), .O(z02));
  aoi21  g10(.a(new_n29_), .b(x4), .c(x3), .O(z03));
  inv1   g11(.a(x1), .O(new_n33_));
  nand3  g12(.a(x2), .b(new_n33_), .c(x0), .O(new_n34_));
  nor2   g13(.a(new_n34_), .b(new_n30_), .O(z04));
  aoi21  g14(.a(new_n34_), .b(x4), .c(x3), .O(z05));
  nand2  g15(.a(new_n33_), .b(x0), .O(new_n37_));
  nand3  g16(.a(x4), .b(x3), .c(new_n28_), .O(new_n38_));
  nor2   g17(.a(new_n38_), .b(new_n37_), .O(z06));
  nand3  g18(.a(new_n28_), .b(new_n33_), .c(x0), .O(new_n40_));
  aoi21  g19(.a(new_n40_), .b(x4), .c(x3), .O(z07));
  inv1   g20(.a(x0), .O(new_n42_));
  nand2  g21(.a(x1), .b(new_n42_), .O(new_n43_));
  nor2   g22(.a(new_n43_), .b(new_n23_), .O(z08));
  nor2   g23(.a(new_n43_), .b(new_n26_), .O(z09));
  nand3  g24(.a(new_n28_), .b(x1), .c(new_n42_), .O(new_n46_));
  nor2   g25(.a(new_n46_), .b(new_n30_), .O(z10));
  aoi21  g26(.a(new_n46_), .b(x4), .c(x3), .O(z11));
  nand3  g27(.a(x2), .b(new_n33_), .c(new_n42_), .O(new_n49_));
  nor2   g28(.a(new_n49_), .b(new_n30_), .O(z12));
  aoi21  g29(.a(new_n49_), .b(x4), .c(x3), .O(z13));
  nand3  g30(.a(new_n28_), .b(new_n33_), .c(new_n42_), .O(new_n52_));
  nor2   g31(.a(new_n52_), .b(new_n30_), .O(z14));
  nand2  g32(.a(x4), .b(new_n25_), .O(new_n54_));
  nor2   g33(.a(new_n52_), .b(new_n54_), .O(z15));
endmodule


