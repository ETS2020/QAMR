// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:00 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n30_, new_n31_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n43_, new_n47_, new_n48_;
  inv1   g00(.a(x3), .O(new_n22_));
  nand3  g01(.a(x4), .b(x2), .c(x0), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x1), .c(new_n22_), .O(z00));
  nand2  g03(.a(x1), .b(x0), .O(new_n25_));
  nand3  g04(.a(x4), .b(new_n22_), .c(x2), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n25_), .O(z01));
  nand2  g06(.a(x4), .b(x3), .O(new_n28_));
  nor3   g07(.a(new_n28_), .b(new_n25_), .c(x2), .O(z02));
  inv1   g08(.a(x2), .O(new_n30_));
  nand3  g09(.a(x4), .b(new_n22_), .c(new_n30_), .O(new_n31_));
  nor2   g10(.a(new_n31_), .b(new_n25_), .O(z03));
  nor3   g11(.a(new_n23_), .b(x3), .c(x1), .O(z05));
  nand3  g12(.a(x4), .b(new_n30_), .c(x0), .O(new_n36_));
  aoi21  g13(.a(new_n36_), .b(new_n22_), .c(x1), .O(z07));
  inv1   g14(.a(x0), .O(new_n38_));
  nand2  g15(.a(x1), .b(new_n38_), .O(new_n39_));
  nand3  g16(.a(x4), .b(x3), .c(x2), .O(new_n40_));
  nor2   g17(.a(new_n40_), .b(new_n39_), .O(z08));
  nor2   g18(.a(new_n39_), .b(new_n26_), .O(z09));
  nand3  g19(.a(x4), .b(new_n30_), .c(new_n38_), .O(new_n43_));
  aoi21  g20(.a(new_n43_), .b(x1), .c(new_n22_), .O(z10));
  nor2   g21(.a(new_n39_), .b(new_n31_), .O(z11));
  inv1   g22(.a(x1), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n38_), .O(new_n48_));
  nor2   g24(.a(new_n48_), .b(new_n26_), .O(z13));
  aoi21  g25(.a(new_n43_), .b(new_n22_), .c(x1), .O(z15));
  zero   g26(.O(z04));
  zero   g27(.O(z06));
  zero   g28(.O(z12));
  zero   g29(.O(z14));
endmodule


