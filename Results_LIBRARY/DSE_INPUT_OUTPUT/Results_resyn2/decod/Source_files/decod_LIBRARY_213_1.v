// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:48 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n38_, new_n39_, new_n42_, new_n45_, new_n48_;
  inv1   g00(.a(x4), .O(new_n22_));
  nand3  g01(.a(x2), .b(x1), .c(x0), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x3), .c(new_n22_), .O(z00));
  inv1   g03(.a(x2), .O(new_n26_));
  nand3  g04(.a(new_n26_), .b(x1), .c(x0), .O(new_n27_));
  aoi21  g05(.a(new_n27_), .b(x3), .c(new_n22_), .O(z02));
  inv1   g06(.a(x1), .O(new_n30_));
  nand3  g07(.a(x2), .b(new_n30_), .c(x0), .O(new_n31_));
  aoi21  g08(.a(new_n31_), .b(x3), .c(new_n22_), .O(z04));
  nand2  g09(.a(new_n30_), .b(x0), .O(new_n34_));
  nand3  g10(.a(x4), .b(x3), .c(new_n26_), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(new_n34_), .O(z06));
  inv1   g12(.a(x0), .O(new_n38_));
  nand3  g13(.a(x2), .b(x1), .c(new_n38_), .O(new_n39_));
  aoi21  g14(.a(new_n39_), .b(x3), .c(new_n22_), .O(z08));
  nand3  g15(.a(new_n26_), .b(x1), .c(new_n38_), .O(new_n42_));
  aoi21  g16(.a(new_n42_), .b(x3), .c(new_n22_), .O(z10));
  nand3  g17(.a(x2), .b(new_n30_), .c(new_n38_), .O(new_n45_));
  aoi21  g18(.a(new_n45_), .b(x3), .c(new_n22_), .O(z12));
  nor2   g19(.a(new_n22_), .b(x3), .O(z13));
  nand3  g20(.a(new_n26_), .b(new_n30_), .c(new_n38_), .O(new_n48_));
  aoi21  g21(.a(new_n48_), .b(x3), .c(new_n22_), .O(z14));
  zero   g22(.O(z01));
  zero   g23(.O(z03));
  zero   g24(.O(z05));
  zero   g25(.O(z07));
  zero   g26(.O(z09));
  zero   g27(.O(z11));
  zero   g28(.O(z15));
endmodule


