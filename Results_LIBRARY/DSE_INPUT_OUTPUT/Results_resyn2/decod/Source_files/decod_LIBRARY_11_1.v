// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:02 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n28_, new_n31_, new_n32_,
    new_n35_, new_n36_, new_n38_, new_n41_, new_n43_;
  inv1   g00(.a(x4), .O(new_n22_));
  nand3  g01(.a(x2), .b(x1), .c(x0), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x3), .c(new_n22_), .O(z00));
  nand2  g03(.a(x1), .b(x0), .O(new_n26_));
  inv1   g04(.a(x2), .O(new_n27_));
  nand3  g05(.a(x4), .b(x3), .c(new_n27_), .O(new_n28_));
  nor2   g06(.a(new_n28_), .b(new_n26_), .O(z02));
  nor2   g07(.a(new_n22_), .b(x3), .O(z03));
  inv1   g08(.a(x0), .O(new_n31_));
  nand3  g09(.a(x4), .b(x3), .c(x2), .O(new_n32_));
  nor3   g10(.a(new_n32_), .b(x1), .c(new_n31_), .O(z04));
  inv1   g11(.a(x1), .O(new_n35_));
  nand3  g12(.a(new_n27_), .b(new_n35_), .c(x0), .O(new_n36_));
  aoi21  g13(.a(new_n36_), .b(x3), .c(new_n22_), .O(z06));
  nand3  g14(.a(x2), .b(x1), .c(new_n31_), .O(new_n38_));
  aoi21  g15(.a(new_n38_), .b(x3), .c(new_n22_), .O(z08));
  nand3  g16(.a(new_n27_), .b(x1), .c(new_n31_), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(x3), .c(new_n22_), .O(z10));
  nand2  g18(.a(new_n35_), .b(new_n31_), .O(new_n43_));
  nor2   g19(.a(new_n43_), .b(new_n32_), .O(z12));
  nor2   g20(.a(new_n43_), .b(new_n28_), .O(z14));
  zero   g21(.O(z01));
  zero   g22(.O(z05));
  zero   g23(.O(z09));
  zero   g24(.O(z15));
  nor2   g25(.a(new_n22_), .b(x3), .O(z07));
  nor2   g26(.a(new_n22_), .b(x3), .O(z11));
  nor2   g27(.a(new_n22_), .b(x3), .O(z13));
endmodule


