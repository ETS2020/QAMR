// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:06 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n35_, new_n40_, new_n42_, new_n43_, new_n46_,
    new_n48_, new_n50_;
  nand2  g00(.a(x1), .b(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x3), .c(x2), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(z00));
  nand3  g03(.a(x4), .b(x2), .c(x0), .O(new_n25_));
  aoi21  g04(.a(new_n25_), .b(x1), .c(x3), .O(z01));
  inv1   g05(.a(x1), .O(new_n27_));
  inv1   g06(.a(x3), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  inv1   g08(.a(x2), .O(new_n30_));
  nand3  g09(.a(x4), .b(x3), .c(new_n30_), .O(new_n31_));
  oai21  g10(.a(new_n31_), .b(new_n22_), .c(new_n29_), .O(z02));
  nand3  g11(.a(x4), .b(new_n30_), .c(x0), .O(new_n33_));
  aoi21  g12(.a(new_n33_), .b(x1), .c(x3), .O(z03));
  inv1   g13(.a(x0), .O(new_n35_));
  nor3   g14(.a(new_n23_), .b(x1), .c(new_n35_), .O(z04));
  inv1   g15(.a(new_n29_), .O(z05));
  aoi21  g16(.a(new_n33_), .b(x3), .c(x1), .O(z06));
  nand2  g17(.a(x1), .b(new_n35_), .O(new_n40_));
  nor2   g18(.a(new_n40_), .b(new_n23_), .O(z08));
  nand2  g19(.a(x4), .b(x2), .O(new_n42_));
  nand3  g20(.a(new_n28_), .b(x1), .c(new_n35_), .O(new_n43_));
  nor2   g21(.a(new_n43_), .b(new_n42_), .O(z09));
  oai21  g22(.a(new_n40_), .b(new_n31_), .c(new_n29_), .O(z10));
  nand2  g23(.a(x4), .b(new_n30_), .O(new_n46_));
  nor2   g24(.a(new_n43_), .b(new_n46_), .O(z11));
  nand3  g25(.a(x4), .b(x2), .c(new_n35_), .O(new_n48_));
  aoi21  g26(.a(new_n48_), .b(x3), .c(x1), .O(z12));
  nand3  g27(.a(x4), .b(new_n30_), .c(new_n35_), .O(new_n50_));
  aoi21  g28(.a(new_n50_), .b(x3), .c(x1), .O(z14));
  zero   g29(.O(z07));
  zero   g30(.O(z15));
  inv1   g31(.a(new_n29_), .O(z13));
endmodule


