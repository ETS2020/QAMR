// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:18 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_,
    new_n33_, new_n34_, new_n36_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n46_;
  nand2  g00(.a(x3), .b(x0), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(z00));
  inv1   g02(.a(x0), .O(new_n24_));
  inv1   g03(.a(x3), .O(new_n25_));
  nand3  g04(.a(x4), .b(x2), .c(x1), .O(new_n26_));
  aoi21  g05(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(z01));
  inv1   g06(.a(x2), .O(new_n28_));
  nand3  g07(.a(x4), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  nand2  g08(.a(x1), .b(x0), .O(new_n30_));
  nor2   g09(.a(new_n30_), .b(new_n29_), .O(z03));
  inv1   g10(.a(x1), .O(new_n33_));
  nand3  g11(.a(x4), .b(x2), .c(new_n33_), .O(new_n34_));
  aoi21  g12(.a(new_n34_), .b(new_n25_), .c(new_n24_), .O(z05));
  nand3  g13(.a(x4), .b(new_n28_), .c(new_n33_), .O(new_n36_));
  aoi21  g14(.a(new_n36_), .b(new_n25_), .c(new_n24_), .O(z07));
  nand2  g15(.a(x1), .b(new_n24_), .O(new_n38_));
  nand3  g16(.a(x4), .b(x3), .c(x2), .O(new_n39_));
  nor2   g17(.a(new_n39_), .b(new_n38_), .O(z08));
  nand3  g18(.a(x4), .b(new_n25_), .c(x2), .O(new_n41_));
  oai21  g19(.a(new_n41_), .b(new_n38_), .c(new_n22_), .O(z09));
  nand3  g20(.a(x4), .b(new_n28_), .c(x1), .O(new_n43_));
  aoi21  g21(.a(new_n43_), .b(new_n24_), .c(new_n25_), .O(z10));
  nor2   g22(.a(new_n38_), .b(new_n29_), .O(z11));
  nand2  g23(.a(new_n33_), .b(new_n24_), .O(new_n46_));
  nor2   g24(.a(new_n46_), .b(new_n39_), .O(z12));
  oai21  g25(.a(new_n46_), .b(new_n41_), .c(new_n22_), .O(z13));
  aoi21  g26(.a(new_n36_), .b(new_n24_), .c(new_n25_), .O(z14));
  oai21  g27(.a(new_n46_), .b(new_n29_), .c(new_n22_), .O(z15));
  zero   g28(.O(z04));
  inv1   g29(.a(new_n22_), .O(z02));
  inv1   g30(.a(new_n22_), .O(z06));
endmodule


