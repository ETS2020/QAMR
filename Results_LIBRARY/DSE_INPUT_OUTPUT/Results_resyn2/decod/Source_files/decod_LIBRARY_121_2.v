// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:27 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n44_,
    new_n47_, new_n50_, new_n52_;
  inv1   g00(.a(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x2), .c(x1), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x3), .c(new_n22_), .O(z00));
  inv1   g03(.a(x2), .O(new_n26_));
  nand3  g04(.a(x4), .b(new_n26_), .c(x1), .O(new_n27_));
  aoi21  g05(.a(new_n27_), .b(x3), .c(new_n22_), .O(z02));
  inv1   g06(.a(x1), .O(new_n30_));
  nand3  g07(.a(x4), .b(x2), .c(new_n30_), .O(new_n31_));
  aoi21  g08(.a(new_n31_), .b(x3), .c(new_n22_), .O(z04));
  inv1   g09(.a(x3), .O(new_n33_));
  nand2  g10(.a(new_n33_), .b(x0), .O(new_n34_));
  inv1   g11(.a(new_n34_), .O(z05));
  nand3  g12(.a(x4), .b(x3), .c(new_n26_), .O(new_n36_));
  nand2  g13(.a(new_n30_), .b(x0), .O(new_n37_));
  nor2   g14(.a(new_n37_), .b(new_n36_), .O(z06));
  nand3  g15(.a(x2), .b(x1), .c(new_n22_), .O(new_n39_));
  nand2  g16(.a(x4), .b(x3), .O(new_n40_));
  nor2   g17(.a(new_n40_), .b(new_n39_), .O(z08));
  nand2  g18(.a(x4), .b(new_n33_), .O(new_n42_));
  nor2   g19(.a(new_n42_), .b(new_n39_), .O(z09));
  nand2  g20(.a(x1), .b(new_n22_), .O(new_n44_));
  nor2   g21(.a(new_n44_), .b(new_n36_), .O(z10));
  aoi21  g22(.a(new_n27_), .b(new_n22_), .c(x3), .O(z11));
  nand3  g23(.a(x2), .b(new_n30_), .c(new_n22_), .O(new_n47_));
  nor2   g24(.a(new_n47_), .b(new_n40_), .O(z12));
  nor2   g25(.a(new_n47_), .b(new_n42_), .O(z13));
  nand2  g26(.a(new_n30_), .b(new_n22_), .O(new_n50_));
  oai21  g27(.a(new_n50_), .b(new_n36_), .c(new_n34_), .O(z14));
  nand3  g28(.a(x4), .b(new_n26_), .c(new_n30_), .O(new_n52_));
  aoi21  g29(.a(new_n52_), .b(new_n22_), .c(x3), .O(z15));
  zero   g30(.O(z01));
  zero   g31(.O(z03));
  inv1   g32(.a(new_n34_), .O(z07));
endmodule


