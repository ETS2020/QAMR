// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:04 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n36_, new_n39_, new_n40_, new_n43_, new_n44_,
    new_n46_, new_n49_, new_n51_;
  nand2  g00(.a(x4), .b(x3), .O(new_n22_));
  nand3  g01(.a(x2), .b(x1), .c(x0), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(z00));
  inv1   g03(.a(x2), .O(new_n25_));
  inv1   g04(.a(x3), .O(new_n26_));
  nand3  g05(.a(x4), .b(new_n26_), .c(x0), .O(new_n27_));
  aoi21  g06(.a(new_n27_), .b(x1), .c(new_n25_), .O(z01));
  inv1   g07(.a(x1), .O(new_n29_));
  nand2  g08(.a(x2), .b(new_n29_), .O(new_n30_));
  nand3  g09(.a(new_n25_), .b(x1), .c(x0), .O(new_n31_));
  oai21  g10(.a(new_n31_), .b(new_n22_), .c(new_n30_), .O(z02));
  nand2  g11(.a(x4), .b(new_n26_), .O(new_n33_));
  oai21  g12(.a(new_n31_), .b(new_n33_), .c(new_n30_), .O(z03));
  inv1   g13(.a(new_n30_), .O(z04));
  nand3  g14(.a(x4), .b(x3), .c(x0), .O(new_n36_));
  aoi21  g15(.a(new_n36_), .b(new_n25_), .c(x1), .O(z06));
  aoi21  g16(.a(new_n27_), .b(new_n25_), .c(x1), .O(z07));
  inv1   g17(.a(x0), .O(new_n39_));
  nand3  g18(.a(x2), .b(x1), .c(new_n39_), .O(new_n40_));
  nor2   g19(.a(new_n40_), .b(new_n22_), .O(z08));
  nor2   g20(.a(new_n40_), .b(new_n33_), .O(z09));
  nand2  g21(.a(x1), .b(new_n39_), .O(new_n43_));
  nand3  g22(.a(x4), .b(x3), .c(new_n25_), .O(new_n44_));
  nor2   g23(.a(new_n44_), .b(new_n43_), .O(z10));
  nand3  g24(.a(x4), .b(new_n26_), .c(new_n25_), .O(new_n46_));
  nor2   g25(.a(new_n46_), .b(new_n43_), .O(z11));
  nand2  g26(.a(new_n29_), .b(new_n39_), .O(new_n49_));
  nor2   g27(.a(new_n49_), .b(new_n44_), .O(z14));
  nand3  g28(.a(x4), .b(new_n26_), .c(new_n39_), .O(new_n51_));
  aoi21  g29(.a(new_n51_), .b(new_n25_), .c(x1), .O(z15));
  zero   g30(.O(z12));
  inv1   g31(.a(new_n30_), .O(z05));
  inv1   g32(.a(new_n30_), .O(z13));
endmodule


