// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:05 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n28_, new_n29_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n39_, new_n41_, new_n42_, new_n45_,
    new_n47_, new_n49_;
  nand2  g00(.a(x3), .b(x2), .O(new_n22_));
  nand3  g01(.a(x4), .b(x1), .c(x0), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(z00));
  inv1   g03(.a(x3), .O(new_n26_));
  nand2  g04(.a(new_n26_), .b(x2), .O(new_n27_));
  inv1   g05(.a(x2), .O(new_n28_));
  nand2  g06(.a(x3), .b(new_n28_), .O(new_n29_));
  oai21  g07(.a(new_n29_), .b(new_n23_), .c(new_n27_), .O(z02));
  aoi21  g08(.a(new_n23_), .b(new_n28_), .c(x3), .O(z03));
  inv1   g09(.a(x1), .O(new_n32_));
  nand2  g10(.a(new_n32_), .b(x0), .O(new_n33_));
  nand3  g11(.a(x4), .b(x3), .c(x2), .O(new_n34_));
  nor2   g12(.a(new_n34_), .b(new_n33_), .O(z04));
  inv1   g13(.a(new_n27_), .O(z05));
  nand3  g14(.a(x4), .b(x3), .c(new_n28_), .O(new_n37_));
  nor2   g15(.a(new_n37_), .b(new_n33_), .O(z06));
  nand3  g16(.a(x4), .b(new_n26_), .c(new_n28_), .O(new_n39_));
  nor2   g17(.a(new_n39_), .b(new_n33_), .O(z07));
  inv1   g18(.a(x0), .O(new_n41_));
  nand3  g19(.a(x4), .b(x1), .c(new_n41_), .O(new_n42_));
  aoi21  g20(.a(new_n42_), .b(x3), .c(new_n28_), .O(z08));
  nor2   g21(.a(new_n42_), .b(new_n29_), .O(z10));
  nand2  g22(.a(x1), .b(new_n41_), .O(new_n45_));
  nor2   g23(.a(new_n45_), .b(new_n39_), .O(z11));
  nand3  g24(.a(x4), .b(new_n32_), .c(new_n41_), .O(new_n47_));
  nor2   g25(.a(new_n47_), .b(new_n22_), .O(z12));
  nand2  g26(.a(new_n32_), .b(new_n41_), .O(new_n49_));
  nor2   g27(.a(new_n49_), .b(new_n37_), .O(z14));
  aoi21  g28(.a(new_n47_), .b(new_n28_), .c(x3), .O(z15));
  zero   g29(.O(z01));
  inv1   g30(.a(new_n27_), .O(z09));
  inv1   g31(.a(new_n27_), .O(z13));
endmodule


