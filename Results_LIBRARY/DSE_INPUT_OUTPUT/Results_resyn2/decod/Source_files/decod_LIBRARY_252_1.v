// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:57 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n32_,
    new_n33_, new_n34_, new_n38_, new_n40_, new_n41_, new_n46_, new_n50_;
  inv1   g00(.a(x2), .O(new_n22_));
  inv1   g01(.a(x3), .O(new_n23_));
  nand3  g02(.a(x4), .b(x1), .c(x0), .O(new_n24_));
  nor3   g03(.a(new_n24_), .b(new_n23_), .c(new_n22_), .O(z00));
  nand2  g04(.a(x1), .b(x0), .O(new_n26_));
  nand2  g05(.a(x3), .b(new_n22_), .O(new_n27_));
  nand3  g06(.a(x4), .b(new_n23_), .c(x2), .O(new_n28_));
  oai21  g07(.a(new_n28_), .b(new_n26_), .c(new_n27_), .O(z01));
  aoi21  g08(.a(new_n24_), .b(new_n23_), .c(x2), .O(z03));
  nand3  g09(.a(x4), .b(x3), .c(x2), .O(new_n32_));
  inv1   g10(.a(x1), .O(new_n33_));
  nand2  g11(.a(new_n33_), .b(x0), .O(new_n34_));
  nor2   g12(.a(new_n34_), .b(new_n32_), .O(z04));
  nor2   g13(.a(new_n34_), .b(new_n28_), .O(z05));
  inv1   g14(.a(new_n27_), .O(z06));
  nand3  g15(.a(x4), .b(new_n23_), .c(new_n22_), .O(new_n38_));
  nor2   g16(.a(new_n38_), .b(new_n34_), .O(z07));
  inv1   g17(.a(x0), .O(new_n40_));
  nand2  g18(.a(x1), .b(new_n40_), .O(new_n41_));
  nor2   g19(.a(new_n41_), .b(new_n32_), .O(z08));
  oai21  g20(.a(new_n41_), .b(new_n28_), .c(new_n27_), .O(z09));
  nor2   g21(.a(new_n41_), .b(new_n38_), .O(z11));
  nand2  g22(.a(new_n33_), .b(new_n40_), .O(new_n46_));
  nor2   g23(.a(new_n46_), .b(new_n32_), .O(z12));
  nor2   g24(.a(new_n46_), .b(new_n28_), .O(z13));
  nand3  g25(.a(x4), .b(new_n33_), .c(new_n40_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n23_), .c(x2), .O(z15));
  zero   g27(.O(z02));
  zero   g28(.O(z10));
  zero   g29(.O(z14));
endmodule


