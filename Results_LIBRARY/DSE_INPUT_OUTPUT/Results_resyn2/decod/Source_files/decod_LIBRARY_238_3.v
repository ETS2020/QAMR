// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:54 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n24_, new_n25_, new_n27_, new_n29_, new_n30_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n39_, new_n43_;
  inv1   g00(.a(x0), .O(new_n24_));
  inv1   g01(.a(x1), .O(new_n25_));
  nor2   g02(.a(new_n25_), .b(new_n24_), .O(z02));
  nand3  g03(.a(x4), .b(x3), .c(x2), .O(new_n27_));
  aoi21  g04(.a(new_n27_), .b(new_n25_), .c(new_n24_), .O(z04));
  inv1   g05(.a(x3), .O(new_n29_));
  nand3  g06(.a(x4), .b(new_n29_), .c(x2), .O(new_n30_));
  aoi21  g07(.a(new_n30_), .b(new_n25_), .c(new_n24_), .O(z05));
  nand2  g08(.a(new_n25_), .b(x0), .O(new_n32_));
  inv1   g09(.a(x2), .O(new_n33_));
  nand3  g10(.a(x4), .b(x3), .c(new_n33_), .O(new_n34_));
  nor2   g11(.a(new_n34_), .b(new_n32_), .O(z06));
  nand3  g12(.a(x4), .b(new_n29_), .c(new_n33_), .O(new_n36_));
  nor2   g13(.a(new_n36_), .b(new_n32_), .O(z07));
  aoi21  g14(.a(new_n27_), .b(new_n24_), .c(new_n25_), .O(z08));
  nand2  g15(.a(x1), .b(new_n24_), .O(new_n39_));
  nor2   g16(.a(new_n39_), .b(new_n30_), .O(z09));
  nor2   g17(.a(new_n39_), .b(new_n34_), .O(z10));
  nor2   g18(.a(new_n39_), .b(new_n36_), .O(z11));
  xor2a  g19(.a(x1), .b(x0), .O(new_n43_));
  aoi21  g20(.a(new_n27_), .b(new_n24_), .c(new_n43_), .O(z12));
  aoi21  g21(.a(new_n30_), .b(new_n25_), .c(new_n43_), .O(z13));
  aoi21  g22(.a(new_n34_), .b(new_n24_), .c(new_n43_), .O(z14));
  aoi21  g23(.a(new_n36_), .b(new_n24_), .c(new_n43_), .O(z15));
  zero   g24(.O(z00));
  zero   g25(.O(z01));
  nor2   g26(.a(new_n25_), .b(new_n24_), .O(z03));
endmodule


