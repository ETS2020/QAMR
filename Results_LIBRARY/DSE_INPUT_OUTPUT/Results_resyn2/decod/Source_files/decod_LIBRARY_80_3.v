// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:18 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n24_, new_n25_, new_n26_, new_n29_, new_n30_, new_n31_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n44_, new_n47_,
    new_n50_;
  nand2  g00(.a(x3), .b(x0), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(z00));
  inv1   g02(.a(x1), .O(new_n24_));
  inv1   g03(.a(x3), .O(new_n25_));
  nand4  g04(.a(x4), .b(new_n25_), .c(x2), .d(x0), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n24_), .O(z01));
  inv1   g06(.a(x0), .O(new_n29_));
  inv1   g07(.a(x2), .O(new_n30_));
  nand3  g08(.a(x4), .b(new_n30_), .c(x1), .O(new_n31_));
  aoi21  g09(.a(new_n31_), .b(new_n25_), .c(new_n29_), .O(z03));
  nor2   g10(.a(new_n26_), .b(x1), .O(z05));
  nand3  g11(.a(x4), .b(new_n25_), .c(new_n30_), .O(new_n36_));
  nand2  g12(.a(new_n24_), .b(x0), .O(new_n37_));
  nor2   g13(.a(new_n37_), .b(new_n36_), .O(z07));
  nand2  g14(.a(x1), .b(new_n29_), .O(new_n39_));
  nand3  g15(.a(x4), .b(x3), .c(x2), .O(new_n40_));
  nor2   g16(.a(new_n40_), .b(new_n39_), .O(z08));
  nand3  g17(.a(x4), .b(new_n25_), .c(x2), .O(new_n42_));
  oai21  g18(.a(new_n39_), .b(new_n42_), .c(new_n22_), .O(z09));
  nand3  g19(.a(x4), .b(x3), .c(new_n30_), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(new_n39_), .O(z10));
  oai21  g21(.a(new_n39_), .b(new_n36_), .c(new_n22_), .O(z11));
  nand2  g22(.a(new_n24_), .b(new_n29_), .O(new_n47_));
  nor2   g23(.a(new_n47_), .b(new_n40_), .O(z12));
  oai21  g24(.a(new_n47_), .b(new_n42_), .c(new_n22_), .O(z13));
  nand3  g25(.a(x4), .b(new_n30_), .c(new_n24_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n29_), .c(new_n25_), .O(z14));
  nor2   g27(.a(new_n47_), .b(new_n36_), .O(z15));
  zero   g28(.O(z02));
  zero   g29(.O(z04));
  zero   g30(.O(z06));
endmodule


