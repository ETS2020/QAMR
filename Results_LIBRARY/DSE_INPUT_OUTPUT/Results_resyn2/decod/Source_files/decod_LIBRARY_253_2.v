// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:57 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n37_, new_n42_, new_n44_, new_n47_;
  inv1   g00(.a(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x2), .c(x1), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x3), .c(new_n22_), .O(z00));
  inv1   g03(.a(x3), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x0), .O(new_n26_));
  inv1   g05(.a(new_n26_), .O(z01));
  inv1   g06(.a(x2), .O(new_n28_));
  nand3  g07(.a(x4), .b(new_n28_), .c(x1), .O(new_n29_));
  aoi21  g08(.a(new_n29_), .b(x3), .c(new_n22_), .O(z02));
  inv1   g09(.a(x1), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(x0), .O(new_n32_));
  nand3  g11(.a(x4), .b(x3), .c(x2), .O(new_n33_));
  nor2   g12(.a(new_n33_), .b(new_n32_), .O(z04));
  nand3  g13(.a(x4), .b(x3), .c(new_n28_), .O(new_n35_));
  nor2   g14(.a(new_n35_), .b(new_n32_), .O(z06));
  nand2  g15(.a(x1), .b(new_n22_), .O(new_n37_));
  oai21  g16(.a(new_n37_), .b(new_n33_), .c(new_n26_), .O(z08));
  aoi21  g17(.a(new_n23_), .b(new_n22_), .c(x3), .O(z09));
  oai21  g18(.a(new_n37_), .b(new_n35_), .c(new_n26_), .O(z10));
  aoi21  g19(.a(new_n29_), .b(new_n22_), .c(x3), .O(z11));
  nand2  g20(.a(new_n31_), .b(new_n22_), .O(new_n42_));
  oai21  g21(.a(new_n42_), .b(new_n33_), .c(new_n26_), .O(z12));
  nand3  g22(.a(x4), .b(new_n25_), .c(x2), .O(new_n44_));
  nor2   g23(.a(new_n44_), .b(new_n42_), .O(z13));
  nor2   g24(.a(new_n42_), .b(new_n35_), .O(z14));
  nand3  g25(.a(x4), .b(new_n28_), .c(new_n31_), .O(new_n47_));
  aoi21  g26(.a(new_n47_), .b(new_n22_), .c(x3), .O(z15));
  inv1   g27(.a(new_n26_), .O(z03));
  inv1   g28(.a(new_n26_), .O(z05));
  inv1   g29(.a(new_n26_), .O(z07));
endmodule


