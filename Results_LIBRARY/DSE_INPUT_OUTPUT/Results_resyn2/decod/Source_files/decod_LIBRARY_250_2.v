// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:56 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n36_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n45_, new_n47_, new_n51_;
  inv1   g00(.a(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x2), .c(x1), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x3), .c(new_n22_), .O(z00));
  inv1   g03(.a(x2), .O(new_n26_));
  nand3  g04(.a(x4), .b(x3), .c(new_n26_), .O(new_n27_));
  nand2  g05(.a(x1), .b(x0), .O(new_n28_));
  nor2   g06(.a(new_n28_), .b(new_n27_), .O(z02));
  inv1   g07(.a(x3), .O(new_n30_));
  nand2  g08(.a(new_n30_), .b(x0), .O(new_n31_));
  inv1   g09(.a(new_n31_), .O(z03));
  inv1   g10(.a(x1), .O(new_n33_));
  nand3  g11(.a(x4), .b(x2), .c(new_n33_), .O(new_n34_));
  aoi21  g12(.a(new_n34_), .b(x3), .c(new_n22_), .O(z04));
  nand3  g13(.a(x4), .b(new_n26_), .c(new_n33_), .O(new_n36_));
  aoi21  g14(.a(new_n36_), .b(x3), .c(new_n22_), .O(z06));
  nand3  g15(.a(x4), .b(x3), .c(x2), .O(new_n38_));
  nor3   g16(.a(new_n38_), .b(new_n33_), .c(x0), .O(z08));
  nand3  g17(.a(x4), .b(x1), .c(new_n22_), .O(new_n40_));
  nand2  g18(.a(new_n30_), .b(x2), .O(new_n41_));
  nor2   g19(.a(new_n41_), .b(new_n40_), .O(z09));
  nand2  g20(.a(x3), .b(new_n26_), .O(new_n43_));
  oai21  g21(.a(new_n40_), .b(new_n43_), .c(new_n31_), .O(z10));
  nand2  g22(.a(new_n30_), .b(new_n26_), .O(new_n45_));
  nor2   g23(.a(new_n45_), .b(new_n40_), .O(z11));
  nand2  g24(.a(new_n33_), .b(new_n22_), .O(new_n47_));
  nor2   g25(.a(new_n47_), .b(new_n38_), .O(z12));
  aoi21  g26(.a(new_n34_), .b(new_n22_), .c(x3), .O(z13));
  nor2   g27(.a(new_n47_), .b(new_n27_), .O(z14));
  nand3  g28(.a(x4), .b(new_n30_), .c(new_n26_), .O(new_n51_));
  nor2   g29(.a(new_n51_), .b(new_n47_), .O(z15));
  zero   g30(.O(z01));
  inv1   g31(.a(new_n31_), .O(z05));
  inv1   g32(.a(new_n31_), .O(z07));
endmodule


