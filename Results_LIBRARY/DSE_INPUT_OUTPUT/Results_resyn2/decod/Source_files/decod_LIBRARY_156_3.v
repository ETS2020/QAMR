// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:35 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n36_, new_n38_, new_n39_, new_n44_, new_n46_;
  nand2  g00(.a(x1), .b(x0), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(z00));
  inv1   g02(.a(x1), .O(new_n26_));
  nand2  g03(.a(new_n26_), .b(x0), .O(new_n27_));
  nand3  g04(.a(x4), .b(x3), .c(x2), .O(new_n28_));
  nor2   g05(.a(new_n28_), .b(new_n27_), .O(z04));
  inv1   g06(.a(x3), .O(new_n30_));
  nand3  g07(.a(x4), .b(new_n30_), .c(x2), .O(new_n31_));
  nor2   g08(.a(new_n31_), .b(new_n27_), .O(z05));
  inv1   g09(.a(x2), .O(new_n33_));
  nand3  g10(.a(x4), .b(x3), .c(new_n33_), .O(new_n34_));
  nor2   g11(.a(new_n34_), .b(new_n27_), .O(z06));
  nand3  g12(.a(x4), .b(new_n30_), .c(new_n33_), .O(new_n36_));
  nor2   g13(.a(new_n36_), .b(new_n27_), .O(z07));
  inv1   g14(.a(x0), .O(new_n38_));
  nand2  g15(.a(x1), .b(new_n38_), .O(new_n39_));
  nor2   g16(.a(new_n39_), .b(new_n28_), .O(z08));
  aoi21  g17(.a(new_n31_), .b(new_n38_), .c(new_n26_), .O(z09));
  aoi21  g18(.a(new_n34_), .b(new_n38_), .c(new_n26_), .O(z10));
  nor2   g19(.a(new_n39_), .b(new_n36_), .O(z11));
  nand2  g20(.a(new_n26_), .b(new_n38_), .O(new_n44_));
  oai21  g21(.a(new_n44_), .b(new_n28_), .c(new_n22_), .O(z12));
  xor2a  g22(.a(x1), .b(x0), .O(new_n46_));
  aoi21  g23(.a(new_n31_), .b(new_n38_), .c(new_n46_), .O(z13));
  nor2   g24(.a(new_n44_), .b(new_n34_), .O(z14));
  nor2   g25(.a(new_n44_), .b(new_n36_), .O(z15));
  zero   g26(.O(z01));
  zero   g27(.O(z02));
  inv1   g28(.a(new_n22_), .O(z03));
endmodule


