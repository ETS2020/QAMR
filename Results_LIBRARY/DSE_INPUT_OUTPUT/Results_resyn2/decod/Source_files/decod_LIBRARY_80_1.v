// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:17 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n45_, new_n48_,
    new_n51_;
  inv1   g00(.a(x3), .O(new_n22_));
  nand3  g01(.a(x4), .b(x2), .c(x1), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x0), .c(new_n22_), .O(z00));
  nand2  g03(.a(x1), .b(x0), .O(new_n25_));
  nand3  g04(.a(x4), .b(new_n22_), .c(x2), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n25_), .O(z01));
  inv1   g06(.a(x2), .O(new_n28_));
  nand3  g07(.a(x4), .b(x3), .c(new_n28_), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(new_n25_), .O(z02));
  inv1   g09(.a(x0), .O(new_n31_));
  nand2  g10(.a(x3), .b(new_n31_), .O(new_n32_));
  nand3  g11(.a(x4), .b(new_n22_), .c(new_n28_), .O(new_n33_));
  oai21  g12(.a(new_n33_), .b(new_n25_), .c(new_n32_), .O(z03));
  inv1   g13(.a(x1), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x0), .O(new_n36_));
  nand3  g15(.a(x4), .b(x3), .c(x2), .O(new_n37_));
  nor2   g16(.a(new_n37_), .b(new_n36_), .O(z04));
  nor2   g17(.a(new_n36_), .b(new_n26_), .O(z05));
  nor2   g18(.a(new_n36_), .b(new_n29_), .O(z06));
  nor2   g19(.a(new_n36_), .b(new_n33_), .O(z07));
  aoi21  g20(.a(new_n23_), .b(new_n22_), .c(x0), .O(z09));
  nand3  g21(.a(x4), .b(new_n28_), .c(x1), .O(new_n45_));
  aoi21  g22(.a(new_n45_), .b(new_n22_), .c(x0), .O(z11));
  nand3  g23(.a(x4), .b(x2), .c(new_n35_), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n22_), .c(x0), .O(z13));
  inv1   g25(.a(new_n32_), .O(z14));
  nand2  g26(.a(new_n35_), .b(new_n31_), .O(new_n51_));
  nor2   g27(.a(new_n51_), .b(new_n33_), .O(z15));
  zero   g28(.O(z08));
  zero   g29(.O(z10));
  zero   g30(.O(z12));
endmodule


