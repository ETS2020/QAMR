// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:34 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n34_, new_n37_, new_n40_, new_n41_, new_n45_, new_n47_,
    new_n48_;
  nand2  g00(.a(x1), .b(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x3), .c(x2), .O(new_n23_));
  oai22  g02(.a(new_n23_), .b(new_n22_), .c(x3), .d(x1), .O(z00));
  inv1   g03(.a(x3), .O(new_n25_));
  nand3  g04(.a(x4), .b(new_n25_), .c(x2), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n22_), .O(z01));
  inv1   g06(.a(x2), .O(new_n28_));
  nand3  g07(.a(x4), .b(x3), .c(new_n28_), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(new_n22_), .O(z02));
  nand2  g09(.a(x4), .b(new_n25_), .O(new_n31_));
  nand3  g10(.a(new_n28_), .b(x1), .c(x0), .O(new_n32_));
  nor2   g11(.a(new_n32_), .b(new_n31_), .O(z03));
  nand3  g12(.a(x4), .b(x2), .c(x0), .O(new_n34_));
  aoi21  g13(.a(new_n34_), .b(x3), .c(x1), .O(z04));
  nand3  g14(.a(x4), .b(new_n28_), .c(x0), .O(new_n37_));
  aoi21  g15(.a(new_n37_), .b(x3), .c(x1), .O(z06));
  inv1   g16(.a(x0), .O(new_n40_));
  nand2  g17(.a(x1), .b(new_n40_), .O(new_n41_));
  nor2   g18(.a(new_n41_), .b(new_n23_), .O(z08));
  nor2   g19(.a(new_n41_), .b(new_n26_), .O(z09));
  nor2   g20(.a(new_n41_), .b(new_n29_), .O(z10));
  nand3  g21(.a(x4), .b(new_n28_), .c(new_n40_), .O(new_n45_));
  aoi21  g22(.a(new_n45_), .b(x1), .c(x3), .O(z11));
  inv1   g23(.a(x1), .O(new_n47_));
  nand2  g24(.a(new_n47_), .b(new_n40_), .O(new_n48_));
  nor2   g25(.a(new_n48_), .b(new_n23_), .O(z12));
  nor2   g26(.a(new_n48_), .b(new_n29_), .O(z14));
  nor2   g27(.a(x3), .b(x1), .O(z15));
  zero   g28(.O(z05));
  zero   g29(.O(z07));
  zero   g30(.O(z13));
endmodule


