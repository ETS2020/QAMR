// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:45 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n37_, new_n38_, new_n40_, new_n42_, new_n43_,
    new_n46_, new_n48_, new_n51_;
  inv1   g00(.a(x1), .O(new_n22_));
  nand3  g01(.a(x4), .b(x2), .c(x0), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x3), .c(new_n22_), .O(z00));
  inv1   g03(.a(x2), .O(new_n26_));
  nand3  g04(.a(x4), .b(new_n26_), .c(x0), .O(new_n27_));
  aoi21  g05(.a(new_n27_), .b(x3), .c(new_n22_), .O(z02));
  inv1   g06(.a(x3), .O(new_n30_));
  nand2  g07(.a(new_n30_), .b(x1), .O(new_n31_));
  nand2  g08(.a(x4), .b(x3), .O(new_n32_));
  nand3  g09(.a(x2), .b(new_n22_), .c(x0), .O(new_n33_));
  oai21  g10(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(z04));
  nand2  g11(.a(x4), .b(new_n30_), .O(new_n35_));
  nor2   g12(.a(new_n35_), .b(new_n33_), .O(z05));
  nand2  g13(.a(new_n22_), .b(x0), .O(new_n37_));
  nand3  g14(.a(x4), .b(x3), .c(new_n26_), .O(new_n38_));
  nor2   g15(.a(new_n38_), .b(new_n37_), .O(z06));
  nand3  g16(.a(x4), .b(new_n30_), .c(new_n26_), .O(new_n40_));
  nor2   g17(.a(new_n40_), .b(new_n37_), .O(z07));
  inv1   g18(.a(x0), .O(new_n42_));
  nand3  g19(.a(x4), .b(x2), .c(new_n42_), .O(new_n43_));
  aoi21  g20(.a(new_n43_), .b(x3), .c(new_n22_), .O(z08));
  inv1   g21(.a(new_n31_), .O(z09));
  nand2  g22(.a(x1), .b(new_n42_), .O(new_n46_));
  nor2   g23(.a(new_n46_), .b(new_n38_), .O(z10));
  nand3  g24(.a(x2), .b(new_n22_), .c(new_n42_), .O(new_n48_));
  nor2   g25(.a(new_n48_), .b(new_n32_), .O(z12));
  nor2   g26(.a(new_n48_), .b(new_n35_), .O(z13));
  nand2  g27(.a(new_n22_), .b(new_n42_), .O(new_n51_));
  oai21  g28(.a(new_n51_), .b(new_n38_), .c(new_n31_), .O(z14));
  nor2   g29(.a(new_n51_), .b(new_n40_), .O(z15));
  zero   g30(.O(z01));
  zero   g31(.O(z03));
  inv1   g32(.a(new_n31_), .O(z11));
endmodule


