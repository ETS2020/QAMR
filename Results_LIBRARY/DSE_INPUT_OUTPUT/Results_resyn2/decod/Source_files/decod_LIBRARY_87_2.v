// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:19 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n28_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n38_, new_n42_, new_n43_, new_n46_, new_n48_;
  inv1   g00(.a(x1), .O(new_n22_));
  nand3  g01(.a(x4), .b(x2), .c(x0), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x3), .c(new_n22_), .O(z00));
  inv1   g03(.a(x2), .O(new_n26_));
  nand3  g04(.a(x4), .b(x3), .c(new_n26_), .O(new_n27_));
  nand2  g05(.a(x1), .b(x0), .O(new_n28_));
  nor2   g06(.a(new_n28_), .b(new_n27_), .O(z02));
  inv1   g07(.a(x3), .O(new_n31_));
  nand2  g08(.a(new_n31_), .b(x1), .O(new_n32_));
  nand2  g09(.a(new_n22_), .b(x0), .O(new_n33_));
  nand3  g10(.a(x4), .b(x3), .c(x2), .O(new_n34_));
  oai21  g11(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(z04));
  aoi21  g12(.a(new_n23_), .b(new_n22_), .c(x3), .O(z05));
  oai21  g13(.a(new_n33_), .b(new_n27_), .c(new_n32_), .O(z06));
  nand3  g14(.a(x4), .b(new_n26_), .c(x0), .O(new_n38_));
  aoi21  g15(.a(new_n38_), .b(new_n22_), .c(x3), .O(z07));
  nor3   g16(.a(new_n34_), .b(new_n22_), .c(x0), .O(z08));
  inv1   g17(.a(new_n32_), .O(z09));
  inv1   g18(.a(x0), .O(new_n42_));
  nand3  g19(.a(x4), .b(new_n26_), .c(new_n42_), .O(new_n43_));
  aoi21  g20(.a(new_n43_), .b(x3), .c(new_n22_), .O(z10));
  nand2  g21(.a(new_n22_), .b(new_n42_), .O(new_n46_));
  nor2   g22(.a(new_n46_), .b(new_n34_), .O(z12));
  nand3  g23(.a(x4), .b(x2), .c(new_n42_), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n22_), .c(x3), .O(z13));
  oai21  g25(.a(new_n46_), .b(new_n27_), .c(new_n32_), .O(z14));
  aoi21  g26(.a(new_n43_), .b(new_n22_), .c(x3), .O(z15));
  zero   g27(.O(z01));
  zero   g28(.O(z03));
  zero   g29(.O(z11));
endmodule


