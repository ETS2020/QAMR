// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:55 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n31_,
    new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n45_, new_n46_, new_n48_, new_n50_;
  nand2  g00(.a(x1), .b(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x3), .c(x2), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(z00));
  inv1   g03(.a(x3), .O(new_n25_));
  nand3  g04(.a(x4), .b(new_n25_), .c(x2), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n22_), .O(z01));
  inv1   g06(.a(x2), .O(new_n28_));
  nand3  g07(.a(x4), .b(x3), .c(new_n28_), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(new_n22_), .O(z02));
  nand3  g09(.a(x4), .b(new_n28_), .c(x0), .O(new_n31_));
  aoi21  g10(.a(new_n31_), .b(x1), .c(x3), .O(z03));
  nand3  g11(.a(x4), .b(x2), .c(x0), .O(new_n33_));
  aoi21  g12(.a(new_n33_), .b(x3), .c(x1), .O(z04));
  inv1   g13(.a(x1), .O(new_n35_));
  nand2  g14(.a(new_n25_), .b(new_n35_), .O(new_n36_));
  inv1   g15(.a(new_n36_), .O(z05));
  nand2  g16(.a(new_n28_), .b(x0), .O(new_n38_));
  nand3  g17(.a(x4), .b(x3), .c(new_n35_), .O(new_n39_));
  nor2   g18(.a(new_n39_), .b(new_n38_), .O(z06));
  inv1   g19(.a(x0), .O(new_n41_));
  nand2  g20(.a(x1), .b(new_n41_), .O(new_n42_));
  nor2   g21(.a(new_n42_), .b(new_n23_), .O(z08));
  nor2   g22(.a(new_n42_), .b(new_n26_), .O(z09));
  nand2  g23(.a(x4), .b(x3), .O(new_n45_));
  nand3  g24(.a(new_n28_), .b(x1), .c(new_n41_), .O(new_n46_));
  oai21  g25(.a(new_n46_), .b(new_n45_), .c(new_n36_), .O(z10));
  nand2  g26(.a(x4), .b(new_n25_), .O(new_n48_));
  nor2   g27(.a(new_n46_), .b(new_n48_), .O(z11));
  nand2  g28(.a(new_n35_), .b(new_n41_), .O(new_n50_));
  nor2   g29(.a(new_n50_), .b(new_n23_), .O(z12));
  nor2   g30(.a(new_n50_), .b(new_n29_), .O(z14));
  zero   g31(.O(z13));
  zero   g32(.O(z15));
  inv1   g33(.a(new_n36_), .O(z07));
endmodule


