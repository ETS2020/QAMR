// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:50 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n29_, new_n31_, new_n32_,
    new_n35_, new_n36_, new_n38_, new_n40_, new_n41_, new_n46_, new_n49_;
  nand2  g00(.a(x1), .b(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x3), .c(x2), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(z00));
  inv1   g03(.a(x2), .O(new_n26_));
  nand3  g04(.a(x4), .b(x3), .c(new_n26_), .O(new_n27_));
  nor2   g05(.a(new_n27_), .b(new_n22_), .O(z02));
  nand3  g06(.a(x4), .b(x1), .c(x0), .O(new_n29_));
  aoi21  g07(.a(new_n29_), .b(new_n26_), .c(x3), .O(z03));
  inv1   g08(.a(x1), .O(new_n31_));
  nand2  g09(.a(new_n31_), .b(x0), .O(new_n32_));
  nor2   g10(.a(new_n32_), .b(new_n23_), .O(z04));
  inv1   g11(.a(x3), .O(new_n35_));
  nand2  g12(.a(new_n35_), .b(x2), .O(new_n36_));
  oai21  g13(.a(new_n32_), .b(new_n27_), .c(new_n36_), .O(z06));
  nand3  g14(.a(x4), .b(new_n35_), .c(new_n26_), .O(new_n38_));
  nor2   g15(.a(new_n38_), .b(new_n32_), .O(z07));
  inv1   g16(.a(x0), .O(new_n40_));
  nand2  g17(.a(x1), .b(new_n40_), .O(new_n41_));
  nor2   g18(.a(new_n41_), .b(new_n23_), .O(z08));
  inv1   g19(.a(new_n36_), .O(z09));
  nor2   g20(.a(new_n41_), .b(new_n27_), .O(z10));
  nor2   g21(.a(new_n41_), .b(new_n38_), .O(z11));
  nand3  g22(.a(x4), .b(new_n31_), .c(new_n40_), .O(new_n46_));
  aoi21  g23(.a(new_n46_), .b(x3), .c(new_n26_), .O(z12));
  nand2  g24(.a(new_n31_), .b(new_n40_), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(new_n27_), .c(new_n36_), .O(z14));
  aoi21  g26(.a(new_n46_), .b(new_n26_), .c(x3), .O(z15));
  zero   g27(.O(z01));
  zero   g28(.O(z05));
  zero   g29(.O(z13));
endmodule


