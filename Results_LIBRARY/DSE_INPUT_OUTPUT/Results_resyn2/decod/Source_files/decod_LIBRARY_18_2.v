// Benchmark "FAU" written by ABC on Sat Aug  8 20:39:03 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n26_, new_n27_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n35_, new_n38_, new_n39_, new_n44_, new_n46_, new_n49_;
  inv1   g00(.a(x1), .O(new_n22_));
  nand3  g01(.a(x4), .b(x2), .c(x0), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x3), .c(new_n22_), .O(z00));
  nor2   g03(.a(x3), .b(new_n22_), .O(z01));
  inv1   g04(.a(x2), .O(new_n26_));
  nand3  g05(.a(x4), .b(new_n26_), .c(x0), .O(new_n27_));
  aoi21  g06(.a(new_n27_), .b(x3), .c(new_n22_), .O(z02));
  nand2  g07(.a(new_n22_), .b(x0), .O(new_n29_));
  nand3  g08(.a(x4), .b(x3), .c(x2), .O(new_n30_));
  nor2   g09(.a(new_n30_), .b(new_n29_), .O(z04));
  inv1   g10(.a(x3), .O(new_n32_));
  nand3  g11(.a(x4), .b(new_n32_), .c(x2), .O(new_n33_));
  nor2   g12(.a(new_n33_), .b(new_n29_), .O(z05));
  nand3  g13(.a(x4), .b(x3), .c(new_n26_), .O(new_n35_));
  nor2   g14(.a(new_n35_), .b(new_n29_), .O(z06));
  aoi21  g15(.a(new_n27_), .b(new_n22_), .c(x3), .O(z07));
  inv1   g16(.a(x0), .O(new_n38_));
  nand2  g17(.a(x1), .b(new_n38_), .O(new_n39_));
  nor2   g18(.a(new_n39_), .b(new_n30_), .O(z08));
  nor2   g19(.a(new_n39_), .b(new_n35_), .O(z10));
  nand2  g20(.a(new_n22_), .b(new_n38_), .O(new_n44_));
  nor2   g21(.a(new_n44_), .b(new_n30_), .O(z12));
  nand3  g22(.a(x4), .b(x2), .c(new_n38_), .O(new_n46_));
  aoi21  g23(.a(new_n46_), .b(new_n22_), .c(x3), .O(z13));
  nor2   g24(.a(new_n44_), .b(new_n35_), .O(z14));
  nand3  g25(.a(x4), .b(new_n26_), .c(new_n38_), .O(new_n49_));
  aoi21  g26(.a(new_n49_), .b(new_n22_), .c(x3), .O(z15));
  zero   g27(.O(z09));
  zero   g28(.O(z11));
  nor2   g29(.a(x3), .b(new_n22_), .O(z03));
endmodule


