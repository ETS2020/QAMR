// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_;
  inv1   g00(.a(x25), .O(new_n47_));
  inv1   g01(.a(x27), .O(new_n48_));
  nor2   g02(.a(new_n48_), .b(new_n47_), .O(z08));
  inv1   g03(.a(z08), .O(new_n50_));
  oai22  g04(.a(new_n50_), .b(x08), .c(x27), .d(x19), .O(z00));
  aoi22  g05(.a(z08), .b(x09), .c(new_n48_), .d(x20), .O(z01));
  oai22  g06(.a(new_n50_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g07(.a(new_n50_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g08(.a(new_n50_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g09(.a(new_n50_), .b(x13), .c(x27), .d(x24), .O(z05));
  oai21  g10(.a(new_n48_), .b(x14), .c(x25), .O(z06));
  oai22  g11(.a(new_n50_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g12(.a(new_n48_), .b(x25), .O(new_n59_));
  inv1   g13(.a(x18), .O(new_n60_));
  inv1   g14(.a(x08), .O(new_n61_));
  aoi21  g15(.a(new_n60_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g16(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g17(.a(x19), .O(new_n64_));
  nand2  g18(.a(new_n64_), .b(x17), .O(new_n65_));
  inv1   g19(.a(x16), .O(new_n66_));
  nor2   g20(.a(new_n64_), .b(x17), .O(new_n67_));
  nor2   g21(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g22(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  aoi21  g23(.a(new_n69_), .b(new_n63_), .c(new_n59_), .O(z09));
  inv1   g24(.a(x20), .O(new_n71_));
  nor2   g25(.a(x19), .b(x17), .O(new_n72_));
  nor2   g26(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor3   g27(.a(x20), .b(x19), .c(x17), .O(new_n74_));
  oai21  g28(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  inv1   g29(.a(x09), .O(new_n76_));
  aoi21  g30(.a(new_n60_), .b(new_n76_), .c(x16), .O(new_n77_));
  oai21  g31(.a(new_n60_), .b(x01), .c(new_n77_), .O(new_n78_));
  aoi21  g32(.a(new_n78_), .b(new_n75_), .c(new_n59_), .O(z10));
  inv1   g33(.a(x21), .O(new_n80_));
  nor2   g34(.a(x21), .b(x20), .O(new_n81_));
  nand2  g35(.a(new_n81_), .b(new_n72_), .O(new_n82_));
  oai21  g36(.a(new_n74_), .b(new_n80_), .c(new_n82_), .O(new_n83_));
  nand2  g37(.a(new_n83_), .b(x16), .O(new_n84_));
  inv1   g38(.a(x10), .O(new_n85_));
  aoi21  g39(.a(new_n60_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g40(.a(new_n60_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g41(.a(new_n87_), .b(new_n84_), .c(new_n59_), .O(z11));
  inv1   g42(.a(x22), .O(new_n89_));
  aoi21  g43(.a(new_n81_), .b(new_n72_), .c(new_n89_), .O(new_n90_));
  nand3  g44(.a(new_n81_), .b(new_n72_), .c(new_n89_), .O(new_n91_));
  inv1   g45(.a(new_n91_), .O(new_n92_));
  oai21  g46(.a(new_n92_), .b(new_n90_), .c(x16), .O(new_n93_));
  inv1   g47(.a(x11), .O(new_n94_));
  aoi21  g48(.a(new_n60_), .b(new_n94_), .c(x16), .O(new_n95_));
  oai21  g49(.a(new_n60_), .b(x03), .c(new_n95_), .O(new_n96_));
  aoi21  g50(.a(new_n96_), .b(new_n93_), .c(new_n59_), .O(z12));
  nand2  g51(.a(new_n91_), .b(x23), .O(new_n98_));
  nor2   g52(.a(x23), .b(x22), .O(new_n99_));
  nand3  g53(.a(new_n99_), .b(new_n81_), .c(new_n72_), .O(new_n100_));
  nand2  g54(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(x16), .O(new_n102_));
  inv1   g56(.a(x04), .O(new_n103_));
  nand2  g57(.a(x18), .b(new_n103_), .O(new_n104_));
  inv1   g58(.a(x12), .O(new_n105_));
  aoi21  g59(.a(new_n60_), .b(new_n105_), .c(x16), .O(new_n106_));
  aoi21  g60(.a(new_n106_), .b(new_n104_), .c(new_n59_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(new_n102_), .O(z13));
  nand2  g62(.a(new_n100_), .b(x24), .O(new_n109_));
  nor2   g63(.a(x22), .b(x21), .O(new_n110_));
  nor2   g64(.a(x24), .b(x23), .O(new_n111_));
  nand4  g65(.a(new_n111_), .b(new_n110_), .c(new_n72_), .d(new_n71_), .O(new_n112_));
  nand2  g66(.a(new_n112_), .b(new_n109_), .O(new_n113_));
  nand2  g67(.a(new_n113_), .b(x16), .O(new_n114_));
  inv1   g68(.a(x05), .O(new_n115_));
  nand2  g69(.a(x18), .b(new_n115_), .O(new_n116_));
  inv1   g70(.a(x13), .O(new_n117_));
  aoi21  g71(.a(new_n60_), .b(new_n117_), .c(x16), .O(new_n118_));
  aoi21  g72(.a(new_n118_), .b(new_n116_), .c(new_n59_), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(new_n114_), .O(z14));
  nor2   g74(.a(new_n112_), .b(new_n47_), .O(new_n121_));
  nand2  g75(.a(new_n112_), .b(new_n47_), .O(new_n122_));
  nand2  g76(.a(new_n122_), .b(x16), .O(new_n123_));
  inv1   g77(.a(x06), .O(new_n124_));
  nand2  g78(.a(x18), .b(new_n124_), .O(new_n125_));
  inv1   g79(.a(x14), .O(new_n126_));
  aoi21  g80(.a(new_n60_), .b(new_n126_), .c(x16), .O(new_n127_));
  aoi21  g81(.a(new_n127_), .b(new_n125_), .c(new_n59_), .O(new_n128_));
  oai21  g82(.a(new_n123_), .b(new_n121_), .c(new_n128_), .O(z15));
  inv1   g83(.a(x26), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(x16), .O(new_n131_));
  oai21  g85(.a(new_n131_), .b(new_n112_), .c(new_n48_), .O(new_n132_));
  nand2  g86(.a(new_n132_), .b(new_n47_), .O(new_n133_));
  inv1   g87(.a(x24), .O(new_n134_));
  nand3  g88(.a(new_n48_), .b(new_n47_), .c(new_n134_), .O(new_n135_));
  nor2   g89(.a(new_n130_), .b(new_n66_), .O(new_n136_));
  oai21  g90(.a(new_n135_), .b(new_n100_), .c(new_n136_), .O(new_n137_));
  inv1   g91(.a(x15), .O(new_n138_));
  aoi21  g92(.a(new_n60_), .b(new_n138_), .c(x16), .O(new_n139_));
  oai21  g93(.a(new_n60_), .b(x07), .c(new_n139_), .O(new_n140_));
  nand3  g94(.a(new_n140_), .b(new_n137_), .c(new_n133_), .O(z16));
  nand2  g95(.a(z08), .b(x17), .O(new_n142_));
  nand4  g96(.a(new_n48_), .b(new_n130_), .c(new_n47_), .d(new_n71_), .O(new_n143_));
  inv1   g97(.a(new_n143_), .O(new_n144_));
  nand4  g98(.a(new_n144_), .b(new_n111_), .c(new_n110_), .d(new_n67_), .O(new_n145_));
  aoi21  g99(.a(new_n145_), .b(new_n142_), .c(new_n66_), .O(z17));
endmodule


