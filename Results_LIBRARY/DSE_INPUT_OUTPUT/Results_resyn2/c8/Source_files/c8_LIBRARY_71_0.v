// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x07), .O(new_n47_));
  nor2   g01(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi22  g02(.a(new_n48_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g03(.a(x27), .O(new_n50_));
  inv1   g04(.a(new_n48_), .O(z08));
  oai22  g05(.a(z08), .b(x20), .c(new_n50_), .d(x09), .O(z01));
  oai22  g06(.a(z08), .b(x21), .c(new_n50_), .d(x10), .O(z02));
  aoi22  g07(.a(new_n48_), .b(x22), .c(x27), .d(x11), .O(z03));
  oai22  g08(.a(z08), .b(x23), .c(new_n50_), .d(x12), .O(z04));
  oai22  g09(.a(z08), .b(x24), .c(new_n50_), .d(x13), .O(z05));
  oai22  g10(.a(z08), .b(x25), .c(new_n50_), .d(x14), .O(z06));
  aoi22  g11(.a(new_n48_), .b(x26), .c(x27), .d(x15), .O(z07));
  nor2   g12(.a(x27), .b(x07), .O(new_n59_));
  nor2   g13(.a(x19), .b(x17), .O(new_n60_));
  and2   g14(.a(x19), .b(x17), .O(new_n61_));
  oai21  g15(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g16(.a(x18), .O(new_n63_));
  inv1   g17(.a(x08), .O(new_n64_));
  aoi21  g18(.a(new_n63_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g19(.a(new_n63_), .b(x00), .c(new_n65_), .O(new_n66_));
  aoi21  g20(.a(new_n66_), .b(new_n62_), .c(new_n59_), .O(z09));
  inv1   g21(.a(x20), .O(new_n68_));
  nor2   g22(.a(new_n60_), .b(new_n68_), .O(new_n69_));
  nor3   g23(.a(x20), .b(x19), .c(x17), .O(new_n70_));
  oai21  g24(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  inv1   g25(.a(x09), .O(new_n72_));
  aoi21  g26(.a(new_n63_), .b(new_n72_), .c(x16), .O(new_n73_));
  oai21  g27(.a(new_n63_), .b(x01), .c(new_n73_), .O(new_n74_));
  aoi21  g28(.a(new_n74_), .b(new_n71_), .c(new_n59_), .O(z10));
  inv1   g29(.a(x21), .O(new_n76_));
  nor2   g30(.a(x21), .b(x20), .O(new_n77_));
  nand2  g31(.a(new_n77_), .b(new_n60_), .O(new_n78_));
  oai21  g32(.a(new_n70_), .b(new_n76_), .c(new_n78_), .O(new_n79_));
  nand2  g33(.a(new_n79_), .b(x16), .O(new_n80_));
  inv1   g34(.a(x10), .O(new_n81_));
  aoi21  g35(.a(new_n63_), .b(new_n81_), .c(x16), .O(new_n82_));
  oai21  g36(.a(new_n63_), .b(x02), .c(new_n82_), .O(new_n83_));
  aoi21  g37(.a(new_n83_), .b(new_n80_), .c(new_n59_), .O(z11));
  inv1   g38(.a(x22), .O(new_n85_));
  aoi21  g39(.a(new_n77_), .b(new_n60_), .c(new_n85_), .O(new_n86_));
  nand3  g40(.a(new_n77_), .b(new_n60_), .c(new_n85_), .O(new_n87_));
  inv1   g41(.a(new_n87_), .O(new_n88_));
  oai21  g42(.a(new_n88_), .b(new_n86_), .c(x16), .O(new_n89_));
  inv1   g43(.a(x11), .O(new_n90_));
  aoi21  g44(.a(new_n63_), .b(new_n90_), .c(x16), .O(new_n91_));
  oai21  g45(.a(new_n63_), .b(x03), .c(new_n91_), .O(new_n92_));
  aoi21  g46(.a(new_n92_), .b(new_n89_), .c(new_n59_), .O(z12));
  inv1   g47(.a(x23), .O(new_n94_));
  xor2a  g48(.a(new_n87_), .b(new_n94_), .O(new_n95_));
  nand2  g49(.a(new_n95_), .b(x16), .O(new_n96_));
  inv1   g50(.a(x12), .O(new_n97_));
  aoi21  g51(.a(new_n63_), .b(new_n97_), .c(x16), .O(new_n98_));
  oai21  g52(.a(new_n63_), .b(x04), .c(new_n98_), .O(new_n99_));
  aoi21  g53(.a(new_n99_), .b(new_n96_), .c(new_n59_), .O(z13));
  nand4  g54(.a(new_n77_), .b(new_n60_), .c(new_n94_), .d(new_n85_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(x24), .O(new_n102_));
  nor2   g56(.a(x24), .b(x23), .O(new_n103_));
  nand4  g57(.a(new_n103_), .b(new_n70_), .c(new_n85_), .d(new_n76_), .O(new_n104_));
  nand2  g58(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(x16), .O(new_n106_));
  inv1   g60(.a(x05), .O(new_n107_));
  nand2  g61(.a(x18), .b(new_n107_), .O(new_n108_));
  inv1   g62(.a(x13), .O(new_n109_));
  aoi21  g63(.a(new_n63_), .b(new_n109_), .c(x16), .O(new_n110_));
  aoi21  g64(.a(new_n110_), .b(new_n108_), .c(new_n59_), .O(new_n111_));
  nand2  g65(.a(new_n111_), .b(new_n106_), .O(z14));
  nand2  g66(.a(new_n104_), .b(x25), .O(new_n113_));
  inv1   g67(.a(x16), .O(new_n114_));
  nor2   g68(.a(x25), .b(x22), .O(new_n115_));
  nand4  g69(.a(new_n115_), .b(new_n103_), .c(new_n77_), .d(new_n60_), .O(new_n116_));
  inv1   g70(.a(new_n116_), .O(new_n117_));
  nor2   g71(.a(new_n117_), .b(new_n114_), .O(new_n118_));
  inv1   g72(.a(new_n59_), .O(new_n119_));
  nand2  g73(.a(x18), .b(x06), .O(new_n120_));
  nand2  g74(.a(new_n63_), .b(x14), .O(new_n121_));
  nand3  g75(.a(new_n121_), .b(new_n120_), .c(new_n114_), .O(new_n122_));
  nand2  g76(.a(new_n122_), .b(new_n119_), .O(new_n123_));
  aoi21  g77(.a(new_n118_), .b(new_n113_), .c(new_n123_), .O(z15));
  inv1   g78(.a(x26), .O(new_n125_));
  nand2  g79(.a(new_n117_), .b(new_n125_), .O(new_n126_));
  aoi21  g80(.a(new_n116_), .b(x26), .c(new_n114_), .O(new_n127_));
  aoi21  g81(.a(new_n63_), .b(x15), .c(x16), .O(new_n128_));
  oai21  g82(.a(new_n63_), .b(new_n47_), .c(new_n128_), .O(new_n129_));
  nand2  g83(.a(new_n129_), .b(new_n119_), .O(new_n130_));
  aoi21  g84(.a(new_n127_), .b(new_n126_), .c(new_n130_), .O(z16));
  inv1   g85(.a(x17), .O(new_n132_));
  nand2  g86(.a(new_n115_), .b(new_n103_), .O(new_n133_));
  nand4  g87(.a(new_n77_), .b(new_n125_), .c(x19), .d(new_n132_), .O(new_n134_));
  oai22  g88(.a(new_n134_), .b(new_n133_), .c(new_n50_), .d(new_n132_), .O(new_n135_));
  nand2  g89(.a(new_n135_), .b(x16), .O(new_n136_));
  nand2  g90(.a(new_n136_), .b(new_n119_), .O(z17));
endmodule


