// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n51_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x22), .O(new_n48_));
  oai22  g002(.a(new_n48_), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g003(.a(new_n48_), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  inv1   g004(.a(new_n48_), .O(new_n51_));
  aoi22  g005(.a(new_n51_), .b(x21), .c(x27), .d(x10), .O(z02));
  aoi21  g006(.a(x27), .b(x11), .c(new_n51_), .O(z03));
  oai22  g007(.a(new_n48_), .b(x23), .c(new_n47_), .d(x12), .O(z04));
  oai22  g008(.a(new_n48_), .b(x24), .c(new_n47_), .d(x13), .O(z05));
  oai22  g009(.a(new_n48_), .b(x25), .c(new_n47_), .d(x14), .O(z06));
  aoi22  g010(.a(new_n51_), .b(x26), .c(x27), .d(x15), .O(z07));
  nor2   g011(.a(x27), .b(x22), .O(new_n58_));
  inv1   g012(.a(x17), .O(new_n59_));
  inv1   g013(.a(x19), .O(new_n60_));
  nor2   g014(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g015(.a(x19), .b(x17), .O(new_n62_));
  oai21  g016(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g017(.a(x18), .O(new_n64_));
  inv1   g018(.a(x08), .O(new_n65_));
  aoi21  g019(.a(new_n64_), .b(new_n65_), .c(x16), .O(new_n66_));
  oai21  g020(.a(new_n64_), .b(x00), .c(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n63_), .c(new_n58_), .O(z09));
  inv1   g022(.a(x20), .O(new_n69_));
  nor2   g023(.a(new_n62_), .b(new_n69_), .O(new_n70_));
  nor3   g024(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  oai21  g025(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  inv1   g026(.a(x09), .O(new_n73_));
  aoi21  g027(.a(new_n64_), .b(new_n73_), .c(x16), .O(new_n74_));
  oai21  g028(.a(new_n64_), .b(x01), .c(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n72_), .c(new_n58_), .O(z10));
  inv1   g030(.a(x21), .O(new_n77_));
  nand2  g031(.a(new_n71_), .b(new_n77_), .O(new_n78_));
  inv1   g032(.a(new_n78_), .O(new_n79_));
  nor2   g033(.a(new_n71_), .b(new_n77_), .O(new_n80_));
  oai21  g034(.a(new_n80_), .b(new_n79_), .c(x16), .O(new_n81_));
  inv1   g035(.a(x10), .O(new_n82_));
  aoi21  g036(.a(new_n64_), .b(new_n82_), .c(x16), .O(new_n83_));
  oai21  g037(.a(new_n64_), .b(x02), .c(new_n83_), .O(new_n84_));
  aoi21  g038(.a(new_n84_), .b(new_n81_), .c(new_n58_), .O(z11));
  nand2  g039(.a(new_n78_), .b(x22), .O(new_n86_));
  nor2   g040(.a(x22), .b(x21), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n71_), .c(x27), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(x16), .O(new_n90_));
  nor2   g044(.a(new_n58_), .b(x16), .O(new_n91_));
  inv1   g045(.a(x11), .O(new_n92_));
  nand2  g046(.a(new_n64_), .b(new_n92_), .O(new_n93_));
  inv1   g047(.a(x03), .O(new_n94_));
  nand2  g048(.a(x18), .b(new_n94_), .O(new_n95_));
  nand3  g049(.a(new_n95_), .b(new_n93_), .c(new_n91_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n90_), .O(z12));
  inv1   g051(.a(x22), .O(new_n98_));
  nor2   g052(.a(x23), .b(x21), .O(new_n99_));
  nand3  g053(.a(new_n99_), .b(new_n71_), .c(x16), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(x27), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  inv1   g056(.a(x04), .O(new_n103_));
  nand2  g057(.a(x18), .b(new_n103_), .O(new_n104_));
  inv1   g058(.a(x12), .O(new_n105_));
  aoi21  g059(.a(new_n64_), .b(new_n105_), .c(x16), .O(new_n106_));
  and2   g060(.a(x23), .b(x16), .O(new_n107_));
  aoi22  g061(.a(new_n107_), .b(new_n88_), .c(new_n106_), .d(new_n104_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n102_), .O(z13));
  inv1   g063(.a(x24), .O(new_n110_));
  nand3  g064(.a(new_n99_), .b(new_n62_), .c(new_n69_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(x27), .O(new_n112_));
  aoi21  g066(.a(new_n112_), .b(new_n98_), .c(new_n110_), .O(new_n113_));
  nor2   g067(.a(x24), .b(x23), .O(new_n114_));
  nand4  g068(.a(new_n114_), .b(new_n87_), .c(new_n71_), .d(x27), .O(new_n115_));
  inv1   g069(.a(new_n115_), .O(new_n116_));
  oai21  g070(.a(new_n116_), .b(new_n113_), .c(x16), .O(new_n117_));
  inv1   g071(.a(x13), .O(new_n118_));
  nand2  g072(.a(new_n64_), .b(new_n118_), .O(new_n119_));
  inv1   g073(.a(x05), .O(new_n120_));
  nand2  g074(.a(x18), .b(new_n120_), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n119_), .c(new_n91_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n117_), .O(z14));
  inv1   g077(.a(x25), .O(new_n124_));
  nand3  g078(.a(new_n69_), .b(new_n60_), .c(new_n59_), .O(new_n125_));
  inv1   g079(.a(x23), .O(new_n126_));
  nand3  g080(.a(new_n110_), .b(new_n126_), .c(new_n77_), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n125_), .c(x27), .O(new_n128_));
  aoi21  g082(.a(new_n128_), .b(new_n98_), .c(new_n124_), .O(new_n129_));
  nor2   g083(.a(new_n115_), .b(x25), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n129_), .c(x16), .O(new_n131_));
  inv1   g085(.a(x14), .O(new_n132_));
  nand2  g086(.a(new_n64_), .b(new_n132_), .O(new_n133_));
  inv1   g087(.a(x06), .O(new_n134_));
  nand2  g088(.a(x18), .b(new_n134_), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n133_), .c(new_n91_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n131_), .O(z15));
  nor2   g091(.a(x26), .b(x25), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n110_), .O(new_n139_));
  oai21  g093(.a(new_n139_), .b(new_n100_), .c(x27), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n98_), .O(new_n141_));
  inv1   g095(.a(x15), .O(new_n142_));
  aoi21  g096(.a(new_n64_), .b(new_n142_), .c(x16), .O(new_n143_));
  oai21  g097(.a(new_n64_), .b(x07), .c(new_n143_), .O(new_n144_));
  and2   g098(.a(x26), .b(x16), .O(new_n145_));
  oai21  g099(.a(new_n115_), .b(x25), .c(new_n145_), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n144_), .c(new_n141_), .O(z16));
  nor2   g101(.a(x20), .b(new_n60_), .O(new_n148_));
  nand4  g102(.a(new_n148_), .b(new_n138_), .c(new_n114_), .d(new_n87_), .O(new_n149_));
  nand2  g103(.a(x27), .b(x16), .O(new_n150_));
  aoi21  g104(.a(new_n149_), .b(new_n59_), .c(new_n150_), .O(z17));
  buf    g105(.a(x27), .O(z08));
endmodule


