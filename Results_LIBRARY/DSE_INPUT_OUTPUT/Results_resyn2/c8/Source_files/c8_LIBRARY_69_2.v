// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_;
  nor2   g00(.a(x27), .b(x14), .O(new_n47_));
  aoi22  g01(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g02(.a(x27), .O(new_n49_));
  inv1   g03(.a(new_n47_), .O(new_n50_));
  oai22  g04(.a(new_n50_), .b(x20), .c(new_n49_), .d(x09), .O(z01));
  aoi22  g05(.a(new_n47_), .b(x21), .c(x27), .d(x10), .O(z02));
  oai22  g06(.a(new_n50_), .b(x22), .c(new_n49_), .d(x11), .O(z03));
  aoi22  g07(.a(new_n47_), .b(x23), .c(x27), .d(x12), .O(z04));
  oai22  g08(.a(new_n50_), .b(x24), .c(new_n49_), .d(x13), .O(z05));
  aoi21  g09(.a(new_n49_), .b(x25), .c(x14), .O(z06));
  aoi22  g10(.a(new_n47_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g11(.a(x14), .O(new_n58_));
  nor2   g12(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g13(.a(new_n59_), .O(new_n60_));
  inv1   g14(.a(x16), .O(new_n61_));
  nand2  g15(.a(x18), .b(x00), .O(new_n62_));
  inv1   g16(.a(x18), .O(new_n63_));
  nand2  g17(.a(new_n63_), .b(x08), .O(new_n64_));
  nand3  g18(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  nand2  g19(.a(x19), .b(x17), .O(new_n66_));
  nor2   g20(.a(x19), .b(x17), .O(new_n67_));
  nor2   g21(.a(new_n67_), .b(new_n61_), .O(new_n68_));
  nand2  g22(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand2  g23(.a(new_n69_), .b(new_n65_), .O(new_n70_));
  nand2  g24(.a(new_n70_), .b(new_n60_), .O(z09));
  inv1   g25(.a(x20), .O(new_n72_));
  xor2a  g26(.a(new_n67_), .b(new_n72_), .O(new_n73_));
  inv1   g27(.a(x01), .O(new_n74_));
  nand2  g28(.a(x18), .b(new_n74_), .O(new_n75_));
  inv1   g29(.a(x09), .O(new_n76_));
  aoi21  g30(.a(new_n63_), .b(new_n76_), .c(x16), .O(new_n77_));
  aoi21  g31(.a(new_n77_), .b(new_n75_), .c(new_n59_), .O(new_n78_));
  oai21  g32(.a(new_n73_), .b(new_n61_), .c(new_n78_), .O(z10));
  nand2  g33(.a(new_n67_), .b(new_n72_), .O(new_n80_));
  nor2   g34(.a(x21), .b(x20), .O(new_n81_));
  aoi22  g35(.a(new_n81_), .b(new_n67_), .c(new_n80_), .d(x21), .O(new_n82_));
  inv1   g36(.a(x02), .O(new_n83_));
  nand2  g37(.a(x18), .b(new_n83_), .O(new_n84_));
  inv1   g38(.a(x10), .O(new_n85_));
  aoi21  g39(.a(new_n63_), .b(new_n85_), .c(x16), .O(new_n86_));
  aoi21  g40(.a(new_n86_), .b(new_n84_), .c(new_n59_), .O(new_n87_));
  oai21  g41(.a(new_n82_), .b(new_n61_), .c(new_n87_), .O(z11));
  inv1   g42(.a(x22), .O(new_n89_));
  aoi21  g43(.a(new_n81_), .b(new_n67_), .c(new_n89_), .O(new_n90_));
  nand3  g44(.a(new_n81_), .b(new_n67_), .c(new_n89_), .O(new_n91_));
  inv1   g45(.a(new_n91_), .O(new_n92_));
  oai21  g46(.a(new_n92_), .b(new_n90_), .c(x16), .O(new_n93_));
  inv1   g47(.a(x11), .O(new_n94_));
  aoi21  g48(.a(new_n63_), .b(new_n94_), .c(x16), .O(new_n95_));
  oai21  g49(.a(new_n63_), .b(x03), .c(new_n95_), .O(new_n96_));
  aoi21  g50(.a(new_n96_), .b(new_n93_), .c(new_n59_), .O(z12));
  nand2  g51(.a(new_n91_), .b(x23), .O(new_n98_));
  nor2   g52(.a(x23), .b(x22), .O(new_n99_));
  nand3  g53(.a(new_n99_), .b(new_n81_), .c(new_n67_), .O(new_n100_));
  nand2  g54(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(x16), .O(new_n102_));
  inv1   g56(.a(x04), .O(new_n103_));
  nand2  g57(.a(x18), .b(new_n103_), .O(new_n104_));
  inv1   g58(.a(x12), .O(new_n105_));
  aoi21  g59(.a(new_n63_), .b(new_n105_), .c(x16), .O(new_n106_));
  aoi21  g60(.a(new_n106_), .b(new_n104_), .c(new_n59_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(new_n102_), .O(z13));
  inv1   g62(.a(x24), .O(new_n109_));
  xor2a  g63(.a(new_n100_), .b(new_n109_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(x16), .O(new_n111_));
  inv1   g65(.a(x13), .O(new_n112_));
  aoi21  g66(.a(new_n63_), .b(new_n112_), .c(x16), .O(new_n113_));
  oai21  g67(.a(new_n63_), .b(x05), .c(new_n113_), .O(new_n114_));
  aoi21  g68(.a(new_n114_), .b(new_n111_), .c(new_n59_), .O(z14));
  nand4  g69(.a(new_n99_), .b(new_n81_), .c(new_n67_), .d(new_n109_), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(x25), .O(new_n117_));
  nor2   g71(.a(x25), .b(x24), .O(new_n118_));
  nand4  g72(.a(new_n118_), .b(new_n99_), .c(new_n81_), .d(new_n67_), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(x16), .O(new_n121_));
  oai21  g75(.a(x18), .b(x16), .c(x27), .O(new_n122_));
  inv1   g76(.a(x06), .O(new_n123_));
  nor2   g77(.a(x16), .b(new_n123_), .O(new_n124_));
  aoi22  g78(.a(new_n124_), .b(x18), .c(new_n122_), .d(x14), .O(new_n125_));
  nand2  g79(.a(new_n125_), .b(new_n121_), .O(z15));
  nand2  g80(.a(new_n119_), .b(x26), .O(new_n127_));
  nor3   g81(.a(x26), .b(x23), .c(x22), .O(new_n128_));
  nand4  g82(.a(new_n128_), .b(new_n118_), .c(new_n81_), .d(new_n67_), .O(new_n129_));
  nand2  g83(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(x16), .O(new_n131_));
  inv1   g85(.a(x07), .O(new_n132_));
  nand2  g86(.a(x18), .b(new_n132_), .O(new_n133_));
  inv1   g87(.a(x15), .O(new_n134_));
  aoi21  g88(.a(new_n63_), .b(new_n134_), .c(x16), .O(new_n135_));
  aoi21  g89(.a(new_n135_), .b(new_n133_), .c(new_n59_), .O(new_n136_));
  nand2  g90(.a(new_n136_), .b(new_n131_), .O(z16));
  inv1   g91(.a(x17), .O(new_n138_));
  inv1   g92(.a(x26), .O(new_n139_));
  nand3  g93(.a(new_n118_), .b(new_n99_), .c(new_n139_), .O(new_n140_));
  nand3  g94(.a(new_n81_), .b(x19), .c(new_n138_), .O(new_n141_));
  oai22  g95(.a(new_n141_), .b(new_n140_), .c(new_n49_), .d(new_n138_), .O(new_n142_));
  nand2  g96(.a(new_n142_), .b(x16), .O(new_n143_));
  nand2  g97(.a(new_n143_), .b(new_n60_), .O(z17));
  buf    g98(.a(x27), .O(z08));
endmodule


