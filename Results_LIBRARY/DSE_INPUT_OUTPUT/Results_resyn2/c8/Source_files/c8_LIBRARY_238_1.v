// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_;
  nor2   g000(.a(x27), .b(x19), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  nand2  g002(.a(x27), .b(x19), .O(new_n49_));
  oai21  g003(.a(new_n49_), .b(x08), .c(new_n48_), .O(z00));
  oai22  g004(.a(new_n49_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g005(.a(new_n49_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g006(.a(new_n49_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g007(.a(new_n49_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g008(.a(new_n49_), .b(x13), .c(x27), .d(x24), .O(z05));
  oai22  g009(.a(new_n49_), .b(x14), .c(x27), .d(x25), .O(z06));
  oai22  g010(.a(new_n49_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g011(.a(x18), .O(new_n58_));
  inv1   g012(.a(x08), .O(new_n59_));
  aoi21  g013(.a(new_n58_), .b(new_n59_), .c(x16), .O(new_n60_));
  oai21  g014(.a(new_n58_), .b(x00), .c(new_n60_), .O(new_n61_));
  inv1   g015(.a(x19), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  nand2  g017(.a(x19), .b(x17), .O(new_n64_));
  inv1   g018(.a(x17), .O(new_n65_));
  nand2  g019(.a(new_n62_), .b(new_n65_), .O(new_n66_));
  nand2  g020(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand2  g021(.a(new_n67_), .b(x16), .O(new_n68_));
  nand3  g022(.a(new_n68_), .b(new_n63_), .c(new_n61_), .O(z09));
  inv1   g023(.a(x27), .O(new_n70_));
  nor2   g024(.a(x20), .b(x17), .O(new_n71_));
  nand2  g025(.a(new_n71_), .b(x16), .O(new_n72_));
  nand2  g026(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand2  g027(.a(new_n73_), .b(new_n62_), .O(new_n74_));
  inv1   g028(.a(x09), .O(new_n75_));
  aoi21  g029(.a(new_n58_), .b(new_n75_), .c(x16), .O(new_n76_));
  oai21  g030(.a(new_n58_), .b(x01), .c(new_n76_), .O(new_n77_));
  nand3  g031(.a(new_n66_), .b(x20), .c(x16), .O(new_n78_));
  nand3  g032(.a(new_n78_), .b(new_n77_), .c(new_n74_), .O(z10));
  inv1   g033(.a(x16), .O(new_n80_));
  oai21  g034(.a(new_n71_), .b(x27), .c(new_n62_), .O(new_n81_));
  nor3   g035(.a(x21), .b(x20), .c(x17), .O(new_n82_));
  aoi22  g036(.a(new_n82_), .b(new_n47_), .c(new_n81_), .d(x21), .O(new_n83_));
  aoi21  g037(.a(x27), .b(new_n62_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x10), .O(new_n85_));
  nand2  g039(.a(new_n58_), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x02), .O(new_n87_));
  nand2  g041(.a(x18), .b(new_n87_), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n86_), .c(new_n84_), .O(new_n89_));
  oai21  g043(.a(new_n83_), .b(new_n80_), .c(new_n89_), .O(z11));
  inv1   g044(.a(x22), .O(new_n91_));
  nand3  g045(.a(new_n82_), .b(new_n91_), .c(new_n62_), .O(new_n92_));
  nand2  g046(.a(new_n82_), .b(new_n62_), .O(new_n93_));
  aoi21  g047(.a(new_n93_), .b(x22), .c(new_n80_), .O(new_n94_));
  nand2  g048(.a(x18), .b(x03), .O(new_n95_));
  nand2  g049(.a(new_n58_), .b(x11), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n95_), .c(new_n80_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n63_), .O(new_n98_));
  aoi21  g052(.a(new_n94_), .b(new_n92_), .c(new_n98_), .O(z12));
  inv1   g053(.a(x23), .O(new_n100_));
  inv1   g054(.a(x21), .O(new_n101_));
  nand3  g055(.a(new_n71_), .b(new_n91_), .c(new_n101_), .O(new_n102_));
  aoi21  g056(.a(new_n102_), .b(new_n70_), .c(x19), .O(new_n103_));
  nor2   g057(.a(x23), .b(x22), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n82_), .c(new_n47_), .O(new_n105_));
  oai21  g059(.a(new_n103_), .b(new_n100_), .c(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(x16), .O(new_n107_));
  inv1   g061(.a(x12), .O(new_n108_));
  nand2  g062(.a(new_n58_), .b(new_n108_), .O(new_n109_));
  inv1   g063(.a(x04), .O(new_n110_));
  nand2  g064(.a(x18), .b(new_n110_), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n109_), .c(new_n84_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n107_), .O(z13));
  inv1   g067(.a(x24), .O(new_n114_));
  nand3  g068(.a(new_n104_), .b(new_n71_), .c(new_n101_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n70_), .O(new_n116_));
  aoi21  g070(.a(new_n116_), .b(new_n62_), .c(new_n114_), .O(new_n117_));
  nand3  g071(.a(new_n70_), .b(new_n114_), .c(new_n100_), .O(new_n118_));
  nor2   g072(.a(new_n118_), .b(new_n92_), .O(new_n119_));
  oai21  g073(.a(new_n119_), .b(new_n117_), .c(x16), .O(new_n120_));
  inv1   g074(.a(x13), .O(new_n121_));
  nand2  g075(.a(new_n58_), .b(new_n121_), .O(new_n122_));
  inv1   g076(.a(x05), .O(new_n123_));
  nand2  g077(.a(x18), .b(new_n123_), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n122_), .c(new_n84_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n120_), .O(z14));
  inv1   g080(.a(x25), .O(new_n127_));
  nor2   g081(.a(new_n127_), .b(new_n80_), .O(new_n128_));
  oai21  g082(.a(new_n118_), .b(new_n92_), .c(new_n128_), .O(new_n129_));
  nand2  g083(.a(new_n104_), .b(new_n114_), .O(new_n130_));
  nand4  g084(.a(new_n71_), .b(new_n127_), .c(new_n101_), .d(x16), .O(new_n131_));
  nor2   g085(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  oai21  g086(.a(new_n132_), .b(x27), .c(new_n62_), .O(new_n133_));
  inv1   g087(.a(x14), .O(new_n134_));
  aoi21  g088(.a(new_n58_), .b(new_n134_), .c(x16), .O(new_n135_));
  oai21  g089(.a(new_n58_), .b(x06), .c(new_n135_), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n133_), .c(new_n129_), .O(z15));
  inv1   g091(.a(x26), .O(new_n138_));
  aoi21  g092(.a(new_n132_), .b(new_n138_), .c(x27), .O(new_n139_));
  nor2   g093(.a(new_n58_), .b(x07), .O(new_n140_));
  oai21  g094(.a(x18), .b(x15), .c(new_n80_), .O(new_n141_));
  nor2   g095(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nor2   g096(.a(x27), .b(x25), .O(new_n143_));
  nor3   g097(.a(x24), .b(x23), .c(x22), .O(new_n144_));
  nand4  g098(.a(new_n144_), .b(new_n82_), .c(new_n143_), .d(new_n62_), .O(new_n145_));
  nor2   g099(.a(new_n138_), .b(new_n80_), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n145_), .c(new_n142_), .O(new_n147_));
  oai21  g101(.a(new_n139_), .b(x19), .c(new_n147_), .O(z16));
  nand2  g102(.a(x27), .b(x17), .O(new_n149_));
  inv1   g103(.a(new_n102_), .O(new_n150_));
  nor2   g104(.a(x26), .b(x25), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n150_), .c(new_n114_), .d(new_n100_), .O(new_n152_));
  nand2  g106(.a(x19), .b(x16), .O(new_n153_));
  aoi21  g107(.a(new_n152_), .b(new_n149_), .c(new_n153_), .O(z17));
  buf    g108(.a(x27), .O(z08));
endmodule


