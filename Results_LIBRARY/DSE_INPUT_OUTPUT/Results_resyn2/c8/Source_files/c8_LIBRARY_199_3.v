// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x25), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  oai22  g03(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  oai22  g04(.a(new_n49_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g05(.a(new_n49_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g06(.a(new_n49_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g07(.a(new_n49_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g08(.a(new_n49_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi21  g09(.a(x27), .b(x14), .c(x25), .O(z06));
  aoi22  g10(.a(new_n48_), .b(x15), .c(new_n47_), .d(x26), .O(z07));
  inv1   g11(.a(x25), .O(new_n58_));
  nor2   g12(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  inv1   g13(.a(new_n59_), .O(new_n60_));
  inv1   g14(.a(x08), .O(new_n61_));
  inv1   g15(.a(x16), .O(new_n62_));
  oai21  g16(.a(x18), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  aoi21  g17(.a(x18), .b(x00), .c(new_n63_), .O(new_n64_));
  nor2   g18(.a(x19), .b(x17), .O(new_n65_));
  inv1   g19(.a(new_n65_), .O(new_n66_));
  nand2  g20(.a(new_n66_), .b(x16), .O(new_n67_));
  aoi21  g21(.a(x19), .b(x17), .c(new_n67_), .O(new_n68_));
  oai21  g22(.a(new_n68_), .b(new_n64_), .c(new_n60_), .O(z09));
  inv1   g23(.a(x20), .O(new_n70_));
  nor2   g24(.a(new_n65_), .b(new_n70_), .O(new_n71_));
  nor2   g25(.a(new_n66_), .b(x20), .O(new_n72_));
  oai21  g26(.a(new_n72_), .b(new_n71_), .c(x16), .O(new_n73_));
  inv1   g27(.a(x18), .O(new_n74_));
  inv1   g28(.a(x09), .O(new_n75_));
  aoi21  g29(.a(new_n74_), .b(new_n75_), .c(x16), .O(new_n76_));
  oai21  g30(.a(new_n74_), .b(x01), .c(new_n76_), .O(new_n77_));
  nand3  g31(.a(new_n77_), .b(new_n73_), .c(new_n60_), .O(z10));
  inv1   g32(.a(x21), .O(new_n79_));
  aoi21  g33(.a(new_n65_), .b(new_n70_), .c(new_n79_), .O(new_n80_));
  nor2   g34(.a(x21), .b(x20), .O(new_n81_));
  nand2  g35(.a(new_n81_), .b(new_n65_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  oai21  g37(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  inv1   g38(.a(x10), .O(new_n85_));
  aoi21  g39(.a(new_n74_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g40(.a(new_n74_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g41(.a(new_n87_), .b(new_n84_), .c(new_n59_), .O(z11));
  inv1   g42(.a(x22), .O(new_n89_));
  aoi21  g43(.a(new_n81_), .b(new_n65_), .c(new_n89_), .O(new_n90_));
  nand3  g44(.a(new_n81_), .b(new_n65_), .c(new_n89_), .O(new_n91_));
  inv1   g45(.a(new_n91_), .O(new_n92_));
  oai21  g46(.a(new_n92_), .b(new_n90_), .c(x16), .O(new_n93_));
  inv1   g47(.a(x11), .O(new_n94_));
  aoi21  g48(.a(new_n74_), .b(new_n94_), .c(x16), .O(new_n95_));
  oai21  g49(.a(new_n74_), .b(x03), .c(new_n95_), .O(new_n96_));
  aoi21  g50(.a(new_n96_), .b(new_n93_), .c(new_n59_), .O(z12));
  inv1   g51(.a(x23), .O(new_n98_));
  xor2a  g52(.a(new_n91_), .b(new_n98_), .O(new_n99_));
  nand2  g53(.a(new_n99_), .b(x16), .O(new_n100_));
  inv1   g54(.a(x12), .O(new_n101_));
  aoi21  g55(.a(new_n74_), .b(new_n101_), .c(x16), .O(new_n102_));
  oai21  g56(.a(new_n74_), .b(x04), .c(new_n102_), .O(new_n103_));
  aoi21  g57(.a(new_n103_), .b(new_n100_), .c(new_n59_), .O(z13));
  oai21  g58(.a(new_n91_), .b(x23), .c(x24), .O(new_n105_));
  nor2   g59(.a(x23), .b(x22), .O(new_n106_));
  nor2   g60(.a(x24), .b(x21), .O(new_n107_));
  nand4  g61(.a(new_n107_), .b(new_n106_), .c(new_n65_), .d(new_n70_), .O(new_n108_));
  and2   g62(.a(new_n108_), .b(x16), .O(new_n109_));
  nand2  g63(.a(x18), .b(x05), .O(new_n110_));
  nand2  g64(.a(new_n74_), .b(x13), .O(new_n111_));
  nand3  g65(.a(new_n111_), .b(new_n110_), .c(new_n62_), .O(new_n112_));
  nand2  g66(.a(new_n112_), .b(new_n60_), .O(new_n113_));
  aoi21  g67(.a(new_n109_), .b(new_n105_), .c(new_n113_), .O(z14));
  nor2   g68(.a(x27), .b(new_n58_), .O(new_n115_));
  inv1   g69(.a(x24), .O(new_n116_));
  nand3  g70(.a(new_n106_), .b(new_n58_), .c(new_n116_), .O(new_n117_));
  inv1   g71(.a(new_n117_), .O(new_n118_));
  aoi22  g72(.a(new_n118_), .b(new_n83_), .c(new_n115_), .d(new_n108_), .O(new_n119_));
  nor2   g73(.a(new_n59_), .b(x16), .O(new_n120_));
  inv1   g74(.a(x14), .O(new_n121_));
  nand2  g75(.a(new_n74_), .b(new_n121_), .O(new_n122_));
  inv1   g76(.a(x06), .O(new_n123_));
  nand2  g77(.a(x18), .b(new_n123_), .O(new_n124_));
  nand3  g78(.a(new_n124_), .b(new_n122_), .c(new_n120_), .O(new_n125_));
  oai21  g79(.a(new_n119_), .b(new_n62_), .c(new_n125_), .O(z15));
  nor3   g80(.a(x26), .b(x21), .c(x20), .O(new_n127_));
  nand4  g81(.a(new_n127_), .b(new_n106_), .c(new_n65_), .d(new_n116_), .O(new_n128_));
  nand2  g82(.a(new_n108_), .b(x26), .O(new_n129_));
  nand3  g83(.a(new_n129_), .b(new_n128_), .c(new_n58_), .O(new_n130_));
  nand2  g84(.a(new_n47_), .b(x26), .O(new_n131_));
  aoi21  g85(.a(new_n131_), .b(x25), .c(new_n62_), .O(new_n132_));
  nand2  g86(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  inv1   g87(.a(x15), .O(new_n134_));
  nand2  g88(.a(new_n74_), .b(new_n134_), .O(new_n135_));
  inv1   g89(.a(x07), .O(new_n136_));
  nand2  g90(.a(x18), .b(new_n136_), .O(new_n137_));
  nand3  g91(.a(new_n137_), .b(new_n135_), .c(new_n120_), .O(new_n138_));
  nand2  g92(.a(new_n138_), .b(new_n133_), .O(z16));
  oai21  g93(.a(x25), .b(x17), .c(x27), .O(new_n140_));
  inv1   g94(.a(x17), .O(new_n141_));
  nand4  g95(.a(new_n127_), .b(new_n118_), .c(x19), .d(new_n141_), .O(new_n142_));
  aoi21  g96(.a(new_n142_), .b(new_n140_), .c(new_n120_), .O(z17));
  buf    g97(.a(x27), .O(z08));
endmodule


