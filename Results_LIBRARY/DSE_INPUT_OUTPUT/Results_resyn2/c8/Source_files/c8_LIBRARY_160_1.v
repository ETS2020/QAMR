// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x16), .O(new_n47_));
  inv1   g01(.a(x27), .O(new_n48_));
  nor2   g02(.a(new_n48_), .b(new_n47_), .O(z08));
  inv1   g03(.a(z08), .O(new_n50_));
  oai22  g04(.a(new_n50_), .b(x08), .c(x27), .d(x19), .O(z00));
  aoi22  g05(.a(z08), .b(x09), .c(new_n48_), .d(x20), .O(z01));
  aoi22  g06(.a(z08), .b(x10), .c(new_n48_), .d(x21), .O(z02));
  aoi22  g07(.a(z08), .b(x11), .c(new_n48_), .d(x22), .O(z03));
  oai22  g08(.a(new_n50_), .b(x12), .c(x27), .d(x23), .O(z04));
  aoi22  g09(.a(z08), .b(x13), .c(new_n48_), .d(x24), .O(z05));
  oai22  g10(.a(new_n50_), .b(x14), .c(x27), .d(x25), .O(z06));
  aoi22  g11(.a(z08), .b(x15), .c(new_n48_), .d(x26), .O(z07));
  nand2  g12(.a(x18), .b(x00), .O(new_n59_));
  nor2   g13(.a(x27), .b(x16), .O(new_n60_));
  inv1   g14(.a(x18), .O(new_n61_));
  nand2  g15(.a(new_n61_), .b(x08), .O(new_n62_));
  nand3  g16(.a(new_n62_), .b(new_n60_), .c(new_n59_), .O(new_n63_));
  inv1   g17(.a(new_n63_), .O(new_n64_));
  nand2  g18(.a(x19), .b(x17), .O(new_n65_));
  nor2   g19(.a(x19), .b(x17), .O(new_n66_));
  nor2   g20(.a(new_n66_), .b(new_n47_), .O(new_n67_));
  aoi21  g21(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(z09));
  nand2  g22(.a(x18), .b(x01), .O(new_n69_));
  nand2  g23(.a(new_n61_), .b(x09), .O(new_n70_));
  nand3  g24(.a(new_n70_), .b(new_n69_), .c(new_n60_), .O(new_n71_));
  oai21  g25(.a(x19), .b(x17), .c(x20), .O(new_n72_));
  inv1   g26(.a(x20), .O(new_n73_));
  nand2  g27(.a(new_n66_), .b(new_n73_), .O(new_n74_));
  nand3  g28(.a(new_n74_), .b(new_n72_), .c(x16), .O(new_n75_));
  and2   g29(.a(new_n75_), .b(new_n71_), .O(z10));
  nor2   g30(.a(x21), .b(x20), .O(new_n77_));
  aoi22  g31(.a(new_n77_), .b(new_n66_), .c(new_n74_), .d(x21), .O(new_n78_));
  nor2   g32(.a(new_n61_), .b(x02), .O(new_n79_));
  inv1   g33(.a(x10), .O(new_n80_));
  nand2  g34(.a(new_n61_), .b(new_n80_), .O(new_n81_));
  nand2  g35(.a(new_n81_), .b(new_n60_), .O(new_n82_));
  oai22  g36(.a(new_n82_), .b(new_n79_), .c(new_n78_), .d(new_n47_), .O(z11));
  nand2  g37(.a(x18), .b(x03), .O(new_n84_));
  nand2  g38(.a(new_n61_), .b(x11), .O(new_n85_));
  nand3  g39(.a(new_n85_), .b(new_n84_), .c(new_n60_), .O(new_n86_));
  inv1   g40(.a(new_n86_), .O(new_n87_));
  nand2  g41(.a(new_n77_), .b(new_n66_), .O(new_n88_));
  nand2  g42(.a(new_n88_), .b(x22), .O(new_n89_));
  inv1   g43(.a(x22), .O(new_n90_));
  nand3  g44(.a(new_n77_), .b(new_n66_), .c(new_n90_), .O(new_n91_));
  and2   g45(.a(new_n91_), .b(x16), .O(new_n92_));
  aoi21  g46(.a(new_n92_), .b(new_n89_), .c(new_n87_), .O(z12));
  nand2  g47(.a(new_n91_), .b(x23), .O(new_n94_));
  nor2   g48(.a(x23), .b(x22), .O(new_n95_));
  nand3  g49(.a(new_n95_), .b(new_n77_), .c(new_n66_), .O(new_n96_));
  nand2  g50(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand2  g51(.a(new_n97_), .b(x16), .O(new_n98_));
  inv1   g52(.a(x04), .O(new_n99_));
  nand2  g53(.a(x18), .b(new_n99_), .O(new_n100_));
  inv1   g54(.a(x12), .O(new_n101_));
  nand2  g55(.a(new_n61_), .b(new_n101_), .O(new_n102_));
  nand3  g56(.a(new_n102_), .b(new_n100_), .c(new_n60_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(new_n98_), .O(z13));
  nand2  g58(.a(new_n96_), .b(x24), .O(new_n105_));
  nor2   g59(.a(x24), .b(x21), .O(new_n106_));
  nand4  g60(.a(new_n106_), .b(new_n95_), .c(new_n66_), .d(new_n73_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(x16), .O(new_n109_));
  nand2  g63(.a(x18), .b(x05), .O(new_n110_));
  nand2  g64(.a(new_n61_), .b(x13), .O(new_n111_));
  nand3  g65(.a(new_n111_), .b(new_n110_), .c(new_n48_), .O(new_n112_));
  nand2  g66(.a(new_n112_), .b(new_n47_), .O(new_n113_));
  nand2  g67(.a(new_n113_), .b(new_n109_), .O(z14));
  nand2  g68(.a(x18), .b(x06), .O(new_n115_));
  nand2  g69(.a(new_n61_), .b(x14), .O(new_n116_));
  nand3  g70(.a(new_n116_), .b(new_n115_), .c(new_n60_), .O(new_n117_));
  inv1   g71(.a(new_n117_), .O(new_n118_));
  nand2  g72(.a(new_n107_), .b(x25), .O(new_n119_));
  nor2   g73(.a(x25), .b(x24), .O(new_n120_));
  nand4  g74(.a(new_n120_), .b(new_n95_), .c(new_n77_), .d(new_n66_), .O(new_n121_));
  and2   g75(.a(new_n121_), .b(x16), .O(new_n122_));
  aoi21  g76(.a(new_n122_), .b(new_n119_), .c(new_n118_), .O(z15));
  inv1   g77(.a(x07), .O(new_n124_));
  nand2  g78(.a(x18), .b(new_n124_), .O(new_n125_));
  inv1   g79(.a(x15), .O(new_n126_));
  nand2  g80(.a(new_n61_), .b(new_n126_), .O(new_n127_));
  nand3  g81(.a(new_n127_), .b(new_n125_), .c(new_n60_), .O(new_n128_));
  inv1   g82(.a(x26), .O(new_n129_));
  nor2   g83(.a(new_n121_), .b(new_n129_), .O(new_n130_));
  nand2  g84(.a(new_n121_), .b(new_n129_), .O(new_n131_));
  nand2  g85(.a(new_n131_), .b(x16), .O(new_n132_));
  oai21  g86(.a(new_n132_), .b(new_n130_), .c(new_n128_), .O(z16));
  nand2  g87(.a(x27), .b(x17), .O(new_n134_));
  inv1   g88(.a(x17), .O(new_n135_));
  nand3  g89(.a(new_n129_), .b(x19), .c(new_n135_), .O(new_n136_));
  inv1   g90(.a(new_n136_), .O(new_n137_));
  nand4  g91(.a(new_n137_), .b(new_n120_), .c(new_n95_), .d(new_n77_), .O(new_n138_));
  aoi21  g92(.a(new_n138_), .b(new_n134_), .c(new_n47_), .O(z17));
endmodule


