// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n50_, new_n56_, new_n57_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_;
  nand2  g00(.a(x27), .b(x25), .O(new_n47_));
  oai22  g01(.a(new_n47_), .b(x08), .c(x27), .d(x19), .O(z00));
  inv1   g02(.a(x27), .O(new_n49_));
  inv1   g03(.a(new_n47_), .O(new_n50_));
  aoi22  g04(.a(new_n50_), .b(x09), .c(new_n49_), .d(x20), .O(z01));
  aoi22  g05(.a(new_n50_), .b(x10), .c(new_n49_), .d(x21), .O(z02));
  aoi22  g06(.a(new_n50_), .b(x11), .c(new_n49_), .d(x22), .O(z03));
  oai22  g07(.a(new_n47_), .b(x12), .c(x27), .d(x23), .O(z04));
  aoi22  g08(.a(new_n50_), .b(x13), .c(new_n49_), .d(x24), .O(z05));
  inv1   g09(.a(x25), .O(new_n56_));
  nand2  g10(.a(new_n49_), .b(new_n56_), .O(new_n57_));
  oai21  g11(.a(new_n47_), .b(x14), .c(new_n57_), .O(z06));
  aoi22  g12(.a(new_n50_), .b(x15), .c(new_n49_), .d(x26), .O(z07));
  nor2   g13(.a(new_n49_), .b(x25), .O(new_n60_));
  inv1   g14(.a(new_n60_), .O(new_n61_));
  inv1   g15(.a(x16), .O(new_n62_));
  nand2  g16(.a(x18), .b(x00), .O(new_n63_));
  inv1   g17(.a(x18), .O(new_n64_));
  nand2  g18(.a(new_n64_), .b(x08), .O(new_n65_));
  nand3  g19(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(new_n66_));
  nand2  g20(.a(x19), .b(x17), .O(new_n67_));
  nor2   g21(.a(x19), .b(x17), .O(new_n68_));
  nor2   g22(.a(new_n68_), .b(new_n62_), .O(new_n69_));
  nand2  g23(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand2  g24(.a(new_n70_), .b(new_n66_), .O(new_n71_));
  nand2  g25(.a(new_n71_), .b(new_n61_), .O(z09));
  inv1   g26(.a(x20), .O(new_n73_));
  nor2   g27(.a(new_n68_), .b(new_n73_), .O(new_n74_));
  nor3   g28(.a(x20), .b(x19), .c(x17), .O(new_n75_));
  oai21  g29(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  inv1   g30(.a(x09), .O(new_n77_));
  aoi21  g31(.a(new_n64_), .b(new_n77_), .c(x16), .O(new_n78_));
  oai21  g32(.a(new_n64_), .b(x01), .c(new_n78_), .O(new_n79_));
  aoi21  g33(.a(new_n79_), .b(new_n76_), .c(new_n60_), .O(z10));
  inv1   g34(.a(x21), .O(new_n81_));
  nor2   g35(.a(x21), .b(x20), .O(new_n82_));
  nand2  g36(.a(new_n82_), .b(new_n68_), .O(new_n83_));
  oai21  g37(.a(new_n75_), .b(new_n81_), .c(new_n83_), .O(new_n84_));
  nand2  g38(.a(new_n84_), .b(x16), .O(new_n85_));
  inv1   g39(.a(x10), .O(new_n86_));
  aoi21  g40(.a(new_n64_), .b(new_n86_), .c(x16), .O(new_n87_));
  oai21  g41(.a(new_n64_), .b(x02), .c(new_n87_), .O(new_n88_));
  aoi21  g42(.a(new_n88_), .b(new_n85_), .c(new_n60_), .O(z11));
  inv1   g43(.a(x22), .O(new_n90_));
  aoi21  g44(.a(new_n82_), .b(new_n68_), .c(new_n90_), .O(new_n91_));
  nand3  g45(.a(new_n82_), .b(new_n68_), .c(new_n90_), .O(new_n92_));
  inv1   g46(.a(new_n92_), .O(new_n93_));
  oai21  g47(.a(new_n93_), .b(new_n91_), .c(x16), .O(new_n94_));
  inv1   g48(.a(x11), .O(new_n95_));
  aoi21  g49(.a(new_n64_), .b(new_n95_), .c(x16), .O(new_n96_));
  oai21  g50(.a(new_n64_), .b(x03), .c(new_n96_), .O(new_n97_));
  aoi21  g51(.a(new_n97_), .b(new_n94_), .c(new_n60_), .O(z12));
  inv1   g52(.a(x23), .O(new_n99_));
  nand4  g53(.a(new_n82_), .b(new_n68_), .c(new_n99_), .d(new_n90_), .O(new_n100_));
  aoi21  g54(.a(new_n92_), .b(x23), .c(new_n62_), .O(new_n101_));
  nand2  g55(.a(x18), .b(x04), .O(new_n102_));
  nand2  g56(.a(new_n64_), .b(x12), .O(new_n103_));
  nand3  g57(.a(new_n103_), .b(new_n102_), .c(new_n62_), .O(new_n104_));
  nand2  g58(.a(new_n104_), .b(new_n61_), .O(new_n105_));
  aoi21  g59(.a(new_n101_), .b(new_n100_), .c(new_n105_), .O(z13));
  nand2  g60(.a(new_n100_), .b(x24), .O(new_n107_));
  nor2   g61(.a(x23), .b(x22), .O(new_n108_));
  nor2   g62(.a(x24), .b(x21), .O(new_n109_));
  nand3  g63(.a(new_n109_), .b(new_n108_), .c(new_n75_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(new_n107_), .O(new_n111_));
  nand2  g65(.a(new_n111_), .b(x16), .O(new_n112_));
  inv1   g66(.a(x05), .O(new_n113_));
  nand2  g67(.a(x18), .b(new_n113_), .O(new_n114_));
  inv1   g68(.a(x13), .O(new_n115_));
  aoi21  g69(.a(new_n64_), .b(new_n115_), .c(x16), .O(new_n116_));
  aoi21  g70(.a(new_n116_), .b(new_n114_), .c(new_n60_), .O(new_n117_));
  nand2  g71(.a(new_n117_), .b(new_n112_), .O(z14));
  nand4  g72(.a(new_n109_), .b(new_n108_), .c(new_n75_), .d(x16), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(new_n49_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(new_n56_), .O(new_n121_));
  inv1   g75(.a(x06), .O(new_n122_));
  nand2  g76(.a(x18), .b(new_n122_), .O(new_n123_));
  inv1   g77(.a(x14), .O(new_n124_));
  aoi21  g78(.a(new_n64_), .b(new_n124_), .c(x16), .O(new_n125_));
  nor2   g79(.a(new_n56_), .b(new_n62_), .O(new_n126_));
  aoi22  g80(.a(new_n126_), .b(new_n110_), .c(new_n125_), .d(new_n123_), .O(new_n127_));
  nand2  g81(.a(new_n127_), .b(new_n121_), .O(z15));
  oai21  g82(.a(new_n119_), .b(x26), .c(new_n49_), .O(new_n129_));
  nand2  g83(.a(new_n129_), .b(new_n56_), .O(new_n130_));
  nor2   g84(.a(new_n64_), .b(x07), .O(new_n131_));
  oai21  g85(.a(x18), .b(x15), .c(new_n62_), .O(new_n132_));
  nor2   g86(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nor2   g87(.a(x25), .b(x24), .O(new_n134_));
  nand4  g88(.a(new_n134_), .b(new_n93_), .c(new_n49_), .d(new_n99_), .O(new_n135_));
  inv1   g89(.a(x26), .O(new_n136_));
  nor2   g90(.a(new_n136_), .b(new_n62_), .O(new_n137_));
  aoi21  g91(.a(new_n137_), .b(new_n135_), .c(new_n133_), .O(new_n138_));
  nand2  g92(.a(new_n138_), .b(new_n130_), .O(z16));
  inv1   g93(.a(x17), .O(new_n140_));
  nand3  g94(.a(new_n136_), .b(x19), .c(new_n140_), .O(new_n141_));
  nand3  g95(.a(new_n134_), .b(new_n108_), .c(new_n82_), .O(new_n142_));
  oai22  g96(.a(new_n142_), .b(new_n141_), .c(new_n49_), .d(new_n140_), .O(new_n143_));
  nand2  g97(.a(new_n143_), .b(x16), .O(new_n144_));
  nand2  g98(.a(new_n144_), .b(new_n61_), .O(z17));
  buf    g99(.a(x27), .O(z08));
endmodule


