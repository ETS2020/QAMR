// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n51_, new_n57_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_;
  inv1   g00(.a(x14), .O(new_n47_));
  nor2   g01(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi22  g02(.a(new_n48_), .b(x19), .c(x27), .d(x08), .O(z00));
  aoi22  g03(.a(new_n48_), .b(x20), .c(x27), .d(x09), .O(z01));
  inv1   g04(.a(x27), .O(new_n51_));
  inv1   g05(.a(new_n48_), .O(z08));
  oai22  g06(.a(z08), .b(x21), .c(new_n51_), .d(x10), .O(z02));
  oai22  g07(.a(z08), .b(x22), .c(new_n51_), .d(x11), .O(z03));
  aoi22  g08(.a(new_n48_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi22  g09(.a(new_n48_), .b(x24), .c(x27), .d(x13), .O(z05));
  nand2  g10(.a(x27), .b(new_n47_), .O(new_n57_));
  oai21  g11(.a(z08), .b(x25), .c(new_n57_), .O(z06));
  oai22  g12(.a(z08), .b(x26), .c(new_n51_), .d(x15), .O(z07));
  nor2   g13(.a(x27), .b(x14), .O(new_n60_));
  inv1   g14(.a(x17), .O(new_n61_));
  inv1   g15(.a(x19), .O(new_n62_));
  nor2   g16(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nor2   g17(.a(x19), .b(x17), .O(new_n64_));
  oai21  g18(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  inv1   g19(.a(x18), .O(new_n66_));
  inv1   g20(.a(x08), .O(new_n67_));
  aoi21  g21(.a(new_n66_), .b(new_n67_), .c(x16), .O(new_n68_));
  oai21  g22(.a(new_n66_), .b(x00), .c(new_n68_), .O(new_n69_));
  aoi21  g23(.a(new_n69_), .b(new_n65_), .c(new_n60_), .O(z09));
  inv1   g24(.a(x20), .O(new_n71_));
  nor2   g25(.a(new_n64_), .b(new_n71_), .O(new_n72_));
  nor3   g26(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  oai21  g27(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  inv1   g28(.a(x01), .O(new_n75_));
  nand2  g29(.a(x18), .b(new_n75_), .O(new_n76_));
  inv1   g30(.a(x09), .O(new_n77_));
  aoi21  g31(.a(new_n66_), .b(new_n77_), .c(x16), .O(new_n78_));
  aoi21  g32(.a(new_n78_), .b(new_n76_), .c(new_n60_), .O(new_n79_));
  nand2  g33(.a(new_n79_), .b(new_n74_), .O(z10));
  inv1   g34(.a(x21), .O(new_n81_));
  nor2   g35(.a(new_n73_), .b(new_n81_), .O(new_n82_));
  nand3  g36(.a(new_n64_), .b(new_n81_), .c(new_n71_), .O(new_n83_));
  inv1   g37(.a(new_n83_), .O(new_n84_));
  oai21  g38(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  inv1   g39(.a(x10), .O(new_n86_));
  aoi21  g40(.a(new_n66_), .b(new_n86_), .c(x16), .O(new_n87_));
  oai21  g41(.a(new_n66_), .b(x02), .c(new_n87_), .O(new_n88_));
  aoi21  g42(.a(new_n88_), .b(new_n85_), .c(new_n60_), .O(z11));
  nand2  g43(.a(new_n83_), .b(x22), .O(new_n90_));
  nor2   g44(.a(x22), .b(x21), .O(new_n91_));
  nand3  g45(.a(new_n91_), .b(new_n64_), .c(new_n71_), .O(new_n92_));
  nand2  g46(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g47(.a(new_n93_), .b(x16), .O(new_n94_));
  inv1   g48(.a(x11), .O(new_n95_));
  aoi21  g49(.a(new_n66_), .b(new_n95_), .c(x16), .O(new_n96_));
  oai21  g50(.a(new_n66_), .b(x03), .c(new_n96_), .O(new_n97_));
  aoi21  g51(.a(new_n97_), .b(new_n94_), .c(new_n60_), .O(z12));
  nand2  g52(.a(new_n92_), .b(x23), .O(new_n99_));
  nor3   g53(.a(x23), .b(x22), .c(x21), .O(new_n100_));
  nand2  g54(.a(new_n100_), .b(new_n73_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(x16), .O(new_n103_));
  inv1   g57(.a(x12), .O(new_n104_));
  aoi21  g58(.a(new_n66_), .b(new_n104_), .c(x16), .O(new_n105_));
  oai21  g59(.a(new_n66_), .b(x04), .c(new_n105_), .O(new_n106_));
  aoi21  g60(.a(new_n106_), .b(new_n103_), .c(new_n60_), .O(z13));
  nand3  g61(.a(new_n71_), .b(new_n62_), .c(new_n61_), .O(new_n108_));
  inv1   g62(.a(x22), .O(new_n109_));
  inv1   g63(.a(x23), .O(new_n110_));
  nand3  g64(.a(new_n110_), .b(new_n109_), .c(new_n81_), .O(new_n111_));
  oai21  g65(.a(new_n111_), .b(new_n108_), .c(x24), .O(new_n112_));
  inv1   g66(.a(x24), .O(new_n113_));
  nand3  g67(.a(new_n100_), .b(new_n73_), .c(new_n113_), .O(new_n114_));
  nand2  g68(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g69(.a(new_n115_), .b(x16), .O(new_n116_));
  inv1   g70(.a(x13), .O(new_n117_));
  aoi21  g71(.a(new_n66_), .b(new_n117_), .c(x16), .O(new_n118_));
  oai21  g72(.a(new_n66_), .b(x05), .c(new_n118_), .O(new_n119_));
  aoi21  g73(.a(new_n119_), .b(new_n116_), .c(new_n60_), .O(z14));
  inv1   g74(.a(x16), .O(new_n121_));
  nor2   g75(.a(new_n111_), .b(new_n108_), .O(new_n122_));
  nor2   g76(.a(x25), .b(x24), .O(new_n123_));
  aoi22  g77(.a(new_n123_), .b(new_n122_), .c(new_n114_), .d(x25), .O(new_n124_));
  inv1   g78(.a(x06), .O(new_n125_));
  nand2  g79(.a(x18), .b(new_n125_), .O(new_n126_));
  aoi21  g80(.a(new_n66_), .b(new_n47_), .c(x16), .O(new_n127_));
  aoi21  g81(.a(new_n127_), .b(new_n126_), .c(new_n60_), .O(new_n128_));
  oai21  g82(.a(new_n124_), .b(new_n121_), .c(new_n128_), .O(z15));
  nand3  g83(.a(new_n123_), .b(new_n122_), .c(x26), .O(new_n130_));
  inv1   g84(.a(x26), .O(new_n131_));
  inv1   g85(.a(new_n123_), .O(new_n132_));
  oai21  g86(.a(new_n132_), .b(new_n101_), .c(new_n131_), .O(new_n133_));
  nand3  g87(.a(new_n133_), .b(new_n130_), .c(x16), .O(new_n134_));
  inv1   g88(.a(x07), .O(new_n135_));
  nand2  g89(.a(x18), .b(new_n135_), .O(new_n136_));
  inv1   g90(.a(x15), .O(new_n137_));
  aoi21  g91(.a(new_n66_), .b(new_n137_), .c(x16), .O(new_n138_));
  aoi21  g92(.a(new_n138_), .b(new_n136_), .c(new_n60_), .O(new_n139_));
  nand2  g93(.a(new_n139_), .b(new_n134_), .O(z16));
  nor2   g94(.a(x20), .b(new_n62_), .O(new_n141_));
  nand4  g95(.a(new_n141_), .b(new_n123_), .c(new_n100_), .d(new_n131_), .O(new_n142_));
  oai21  g96(.a(x17), .b(new_n47_), .c(new_n51_), .O(new_n143_));
  nand2  g97(.a(new_n143_), .b(x16), .O(new_n144_));
  aoi21  g98(.a(new_n142_), .b(new_n61_), .c(new_n144_), .O(z17));
endmodule


