// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x07), .O(z08));
  aoi22  g02(.a(z08), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g03(.a(z08), .O(new_n50_));
  oai22  g04(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  aoi22  g05(.a(z08), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  aoi22  g06(.a(z08), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  oai22  g07(.a(new_n50_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g08(.a(new_n50_), .b(x13), .c(x27), .d(x24), .O(z05));
  oai22  g09(.a(new_n50_), .b(x14), .c(x27), .d(x25), .O(z06));
  oai22  g10(.a(new_n50_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g11(.a(x07), .O(new_n58_));
  nor2   g12(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  inv1   g13(.a(x18), .O(new_n60_));
  inv1   g14(.a(x08), .O(new_n61_));
  aoi21  g15(.a(new_n60_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g16(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g17(.a(x17), .O(new_n64_));
  inv1   g18(.a(x16), .O(new_n65_));
  aoi21  g19(.a(x19), .b(new_n64_), .c(new_n65_), .O(new_n66_));
  oai21  g20(.a(x19), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  aoi21  g21(.a(new_n67_), .b(new_n63_), .c(new_n59_), .O(z09));
  inv1   g22(.a(new_n59_), .O(new_n69_));
  nor2   g23(.a(x19), .b(x17), .O(new_n70_));
  xor2a  g24(.a(new_n70_), .b(x20), .O(new_n71_));
  nand2  g25(.a(new_n71_), .b(x16), .O(new_n72_));
  inv1   g26(.a(x09), .O(new_n73_));
  aoi21  g27(.a(new_n60_), .b(new_n73_), .c(x16), .O(new_n74_));
  oai21  g28(.a(new_n60_), .b(x01), .c(new_n74_), .O(new_n75_));
  nand3  g29(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z10));
  inv1   g30(.a(x20), .O(new_n77_));
  nand2  g31(.a(new_n70_), .b(new_n77_), .O(new_n78_));
  nor2   g32(.a(x21), .b(x20), .O(new_n79_));
  aoi22  g33(.a(new_n79_), .b(new_n70_), .c(new_n78_), .d(x21), .O(new_n80_));
  inv1   g34(.a(x02), .O(new_n81_));
  nand2  g35(.a(x18), .b(new_n81_), .O(new_n82_));
  inv1   g36(.a(x10), .O(new_n83_));
  aoi21  g37(.a(new_n60_), .b(new_n83_), .c(x16), .O(new_n84_));
  aoi21  g38(.a(new_n84_), .b(new_n82_), .c(new_n59_), .O(new_n85_));
  oai21  g39(.a(new_n80_), .b(new_n65_), .c(new_n85_), .O(z11));
  nand2  g40(.a(x18), .b(x03), .O(new_n87_));
  nand2  g41(.a(new_n60_), .b(x11), .O(new_n88_));
  nand3  g42(.a(new_n88_), .b(new_n87_), .c(new_n65_), .O(new_n89_));
  inv1   g43(.a(x22), .O(new_n90_));
  nand3  g44(.a(new_n79_), .b(new_n70_), .c(new_n90_), .O(new_n91_));
  nand2  g45(.a(new_n79_), .b(new_n70_), .O(new_n92_));
  nand2  g46(.a(new_n92_), .b(x22), .O(new_n93_));
  nand3  g47(.a(new_n93_), .b(new_n91_), .c(x16), .O(new_n94_));
  nand2  g48(.a(new_n94_), .b(new_n89_), .O(new_n95_));
  nand2  g49(.a(new_n95_), .b(new_n69_), .O(z12));
  nand2  g50(.a(new_n91_), .b(x23), .O(new_n97_));
  inv1   g51(.a(x23), .O(new_n98_));
  nand4  g52(.a(new_n79_), .b(new_n70_), .c(new_n98_), .d(new_n90_), .O(new_n99_));
  nand2  g53(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nand2  g54(.a(new_n100_), .b(x16), .O(new_n101_));
  inv1   g55(.a(x12), .O(new_n102_));
  aoi21  g56(.a(new_n60_), .b(new_n102_), .c(x16), .O(new_n103_));
  oai21  g57(.a(new_n60_), .b(x04), .c(new_n103_), .O(new_n104_));
  aoi21  g58(.a(new_n104_), .b(new_n101_), .c(new_n59_), .O(z13));
  inv1   g59(.a(x24), .O(new_n106_));
  xor2a  g60(.a(new_n99_), .b(new_n106_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(x16), .O(new_n108_));
  inv1   g62(.a(x13), .O(new_n109_));
  aoi21  g63(.a(new_n60_), .b(new_n109_), .c(x16), .O(new_n110_));
  oai21  g64(.a(new_n60_), .b(x05), .c(new_n110_), .O(new_n111_));
  aoi21  g65(.a(new_n111_), .b(new_n108_), .c(new_n59_), .O(z14));
  oai21  g66(.a(new_n99_), .b(x24), .c(x25), .O(new_n113_));
  nor2   g67(.a(x24), .b(x23), .O(new_n114_));
  nor2   g68(.a(x25), .b(x22), .O(new_n115_));
  nand4  g69(.a(new_n115_), .b(new_n114_), .c(new_n79_), .d(new_n70_), .O(new_n116_));
  inv1   g70(.a(new_n116_), .O(new_n117_));
  nor2   g71(.a(new_n117_), .b(new_n65_), .O(new_n118_));
  nand2  g72(.a(x18), .b(x06), .O(new_n119_));
  nand2  g73(.a(new_n60_), .b(x14), .O(new_n120_));
  nand3  g74(.a(new_n120_), .b(new_n119_), .c(new_n65_), .O(new_n121_));
  nand2  g75(.a(new_n121_), .b(new_n69_), .O(new_n122_));
  aoi21  g76(.a(new_n118_), .b(new_n113_), .c(new_n122_), .O(z15));
  inv1   g77(.a(x26), .O(new_n124_));
  nand2  g78(.a(new_n117_), .b(new_n124_), .O(new_n125_));
  aoi21  g79(.a(new_n116_), .b(x26), .c(new_n65_), .O(new_n126_));
  aoi21  g80(.a(new_n60_), .b(x15), .c(x16), .O(new_n127_));
  oai21  g81(.a(new_n60_), .b(new_n58_), .c(new_n127_), .O(new_n128_));
  nand2  g82(.a(new_n128_), .b(new_n69_), .O(new_n129_));
  aoi21  g83(.a(new_n126_), .b(new_n125_), .c(new_n129_), .O(z16));
  nand3  g84(.a(new_n79_), .b(x19), .c(new_n64_), .O(new_n131_));
  nand3  g85(.a(new_n115_), .b(new_n114_), .c(new_n124_), .O(new_n132_));
  oai22  g86(.a(new_n132_), .b(new_n131_), .c(new_n47_), .d(new_n64_), .O(new_n133_));
  nand2  g87(.a(new_n133_), .b(new_n58_), .O(new_n134_));
  nor2   g88(.a(x27), .b(x26), .O(new_n135_));
  inv1   g89(.a(new_n131_), .O(new_n136_));
  nand4  g90(.a(new_n136_), .b(new_n115_), .c(new_n114_), .d(new_n135_), .O(new_n137_));
  aoi21  g91(.a(new_n137_), .b(new_n134_), .c(new_n65_), .O(z17));
endmodule


