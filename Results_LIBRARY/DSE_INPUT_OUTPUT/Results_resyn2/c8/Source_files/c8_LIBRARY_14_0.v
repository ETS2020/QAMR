// Benchmark "FAU" written by ABC on Sat Aug  8 23:28:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand3  g02(.a(new_n47_), .b(new_n48_), .c(x16), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x16), .O(new_n51_));
  nor2   g05(.a(x27), .b(new_n51_), .O(new_n52_));
  aoi22  g06(.a(new_n52_), .b(x20), .c(x27), .d(x09), .O(z01));
  inv1   g07(.a(new_n52_), .O(new_n54_));
  oai22  g08(.a(new_n54_), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  oai22  g09(.a(new_n54_), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  aoi22  g10(.a(new_n52_), .b(x23), .c(x27), .d(x12), .O(z04));
  oai22  g11(.a(new_n54_), .b(x24), .c(new_n47_), .d(x13), .O(z05));
  aoi22  g12(.a(new_n52_), .b(x25), .c(x27), .d(x14), .O(z06));
  aoi22  g13(.a(new_n52_), .b(x26), .c(x27), .d(x15), .O(z07));
  nand2  g14(.a(x18), .b(x00), .O(new_n61_));
  inv1   g15(.a(x18), .O(new_n62_));
  nand2  g16(.a(x27), .b(new_n51_), .O(new_n63_));
  aoi21  g17(.a(new_n62_), .b(x08), .c(new_n63_), .O(new_n64_));
  nand2  g18(.a(x19), .b(x17), .O(new_n65_));
  nor2   g19(.a(x19), .b(x17), .O(new_n66_));
  nor2   g20(.a(new_n66_), .b(new_n51_), .O(new_n67_));
  aoi22  g21(.a(new_n67_), .b(new_n65_), .c(new_n64_), .d(new_n61_), .O(z09));
  inv1   g22(.a(x20), .O(new_n69_));
  xor2a  g23(.a(new_n66_), .b(new_n69_), .O(new_n70_));
  inv1   g24(.a(x09), .O(new_n71_));
  aoi21  g25(.a(new_n62_), .b(new_n71_), .c(new_n63_), .O(new_n72_));
  oai21  g26(.a(new_n62_), .b(x01), .c(new_n72_), .O(new_n73_));
  oai21  g27(.a(new_n70_), .b(new_n51_), .c(new_n73_), .O(z10));
  inv1   g28(.a(x21), .O(new_n75_));
  aoi21  g29(.a(new_n66_), .b(new_n69_), .c(new_n75_), .O(new_n76_));
  inv1   g30(.a(x17), .O(new_n77_));
  nand4  g31(.a(new_n75_), .b(new_n69_), .c(new_n48_), .d(new_n77_), .O(new_n78_));
  inv1   g32(.a(new_n78_), .O(new_n79_));
  oai21  g33(.a(new_n79_), .b(new_n76_), .c(x16), .O(new_n80_));
  inv1   g34(.a(x10), .O(new_n81_));
  aoi21  g35(.a(new_n62_), .b(new_n81_), .c(new_n63_), .O(new_n82_));
  oai21  g36(.a(new_n62_), .b(x02), .c(new_n82_), .O(new_n83_));
  nand2  g37(.a(new_n83_), .b(new_n80_), .O(z11));
  nand2  g38(.a(x18), .b(x03), .O(new_n85_));
  aoi21  g39(.a(new_n62_), .b(x11), .c(new_n63_), .O(new_n86_));
  nand2  g40(.a(new_n78_), .b(x22), .O(new_n87_));
  inv1   g41(.a(x22), .O(new_n88_));
  nor2   g42(.a(x21), .b(x20), .O(new_n89_));
  nand3  g43(.a(new_n89_), .b(new_n66_), .c(new_n88_), .O(new_n90_));
  and2   g44(.a(new_n90_), .b(x16), .O(new_n91_));
  aoi22  g45(.a(new_n91_), .b(new_n87_), .c(new_n86_), .d(new_n85_), .O(z12));
  nand2  g46(.a(new_n90_), .b(x23), .O(new_n93_));
  nor2   g47(.a(x23), .b(x22), .O(new_n94_));
  nand3  g48(.a(new_n94_), .b(new_n89_), .c(new_n66_), .O(new_n95_));
  nand2  g49(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand2  g50(.a(new_n96_), .b(x16), .O(new_n97_));
  nand2  g51(.a(x18), .b(x04), .O(new_n98_));
  nand2  g52(.a(new_n62_), .b(x12), .O(new_n99_));
  nand3  g53(.a(new_n99_), .b(new_n98_), .c(x27), .O(new_n100_));
  nand2  g54(.a(new_n100_), .b(new_n51_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(new_n97_), .O(z13));
  nand2  g56(.a(new_n95_), .b(x24), .O(new_n103_));
  nor2   g57(.a(x24), .b(x21), .O(new_n104_));
  nand4  g58(.a(new_n104_), .b(new_n94_), .c(new_n66_), .d(new_n69_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand2  g60(.a(new_n106_), .b(x16), .O(new_n107_));
  inv1   g61(.a(x13), .O(new_n108_));
  aoi21  g62(.a(new_n62_), .b(new_n108_), .c(new_n63_), .O(new_n109_));
  oai21  g63(.a(new_n62_), .b(x05), .c(new_n109_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(new_n107_), .O(z14));
  inv1   g65(.a(x23), .O(new_n112_));
  inv1   g66(.a(x24), .O(new_n113_));
  inv1   g67(.a(x25), .O(new_n114_));
  nand4  g68(.a(new_n114_), .b(new_n113_), .c(new_n112_), .d(new_n88_), .O(new_n115_));
  inv1   g69(.a(new_n115_), .O(new_n116_));
  aoi22  g70(.a(new_n116_), .b(new_n79_), .c(new_n105_), .d(x25), .O(new_n117_));
  inv1   g71(.a(x14), .O(new_n118_));
  aoi21  g72(.a(new_n62_), .b(new_n118_), .c(new_n63_), .O(new_n119_));
  oai21  g73(.a(new_n62_), .b(x06), .c(new_n119_), .O(new_n120_));
  oai21  g74(.a(new_n117_), .b(new_n51_), .c(new_n120_), .O(z15));
  inv1   g75(.a(x26), .O(new_n122_));
  nand3  g76(.a(new_n116_), .b(new_n79_), .c(new_n122_), .O(new_n123_));
  nor2   g77(.a(x25), .b(x24), .O(new_n124_));
  nand4  g78(.a(new_n124_), .b(new_n94_), .c(new_n89_), .d(new_n66_), .O(new_n125_));
  aoi21  g79(.a(new_n125_), .b(x26), .c(new_n51_), .O(new_n126_));
  nand2  g80(.a(x18), .b(x07), .O(new_n127_));
  aoi21  g81(.a(new_n62_), .b(x15), .c(new_n63_), .O(new_n128_));
  aoi22  g82(.a(new_n128_), .b(new_n127_), .c(new_n126_), .d(new_n123_), .O(z16));
  nand2  g83(.a(x27), .b(x17), .O(new_n130_));
  nor2   g84(.a(new_n48_), .b(x17), .O(new_n131_));
  nand4  g85(.a(new_n131_), .b(new_n116_), .c(new_n89_), .d(new_n122_), .O(new_n132_));
  aoi21  g86(.a(new_n132_), .b(new_n130_), .c(new_n51_), .O(z17));
  buf    g87(.a(x27), .O(z08));
endmodule


