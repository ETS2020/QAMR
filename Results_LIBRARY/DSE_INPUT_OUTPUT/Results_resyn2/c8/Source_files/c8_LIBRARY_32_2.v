// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  inv1   g002(.a(x04), .O(new_n49_));
  nor2   g003(.a(x18), .b(new_n49_), .O(new_n50_));
  aoi21  g004(.a(new_n47_), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g005(.a(new_n47_), .b(x08), .c(new_n51_), .O(z00));
  inv1   g006(.a(x20), .O(new_n53_));
  aoi21  g007(.a(new_n47_), .b(new_n53_), .c(new_n50_), .O(new_n54_));
  oai21  g008(.a(new_n47_), .b(x09), .c(new_n54_), .O(z01));
  inv1   g009(.a(x21), .O(new_n56_));
  aoi21  g010(.a(new_n47_), .b(new_n56_), .c(new_n50_), .O(new_n57_));
  oai21  g011(.a(new_n47_), .b(x10), .c(new_n57_), .O(z02));
  inv1   g012(.a(new_n50_), .O(new_n59_));
  nand2  g013(.a(x27), .b(x11), .O(new_n60_));
  nand2  g014(.a(new_n47_), .b(x22), .O(new_n61_));
  nand3  g015(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  inv1   g016(.a(new_n62_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(new_n47_), .b(x23), .c(new_n50_), .O(new_n65_));
  oai21  g019(.a(new_n47_), .b(new_n64_), .c(new_n65_), .O(new_n66_));
  inv1   g020(.a(new_n66_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  aoi21  g022(.a(new_n47_), .b(x24), .c(new_n50_), .O(new_n69_));
  oai21  g023(.a(new_n47_), .b(new_n68_), .c(new_n69_), .O(new_n70_));
  inv1   g024(.a(new_n70_), .O(z05));
  inv1   g025(.a(x25), .O(new_n72_));
  aoi21  g026(.a(new_n47_), .b(new_n72_), .c(new_n50_), .O(new_n73_));
  oai21  g027(.a(new_n47_), .b(x14), .c(new_n73_), .O(z06));
  nand2  g028(.a(x27), .b(x15), .O(new_n75_));
  nand2  g029(.a(new_n47_), .b(x26), .O(new_n76_));
  nand3  g030(.a(new_n76_), .b(new_n75_), .c(new_n59_), .O(new_n77_));
  inv1   g031(.a(new_n77_), .O(z07));
  nor2   g032(.a(new_n50_), .b(new_n47_), .O(z08));
  nor2   g033(.a(x18), .b(x04), .O(new_n80_));
  aoi22  g034(.a(new_n80_), .b(x08), .c(x18), .d(x00), .O(new_n81_));
  inv1   g035(.a(x16), .O(new_n82_));
  nor2   g036(.a(new_n50_), .b(new_n82_), .O(new_n83_));
  inv1   g037(.a(new_n83_), .O(new_n84_));
  xor2a  g038(.a(x19), .b(x17), .O(new_n85_));
  oai22  g039(.a(new_n85_), .b(new_n84_), .c(new_n81_), .d(x16), .O(z09));
  aoi22  g040(.a(new_n80_), .b(x09), .c(x18), .d(x01), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  xor2a  g042(.a(new_n88_), .b(new_n53_), .O(new_n89_));
  oai22  g043(.a(new_n89_), .b(new_n84_), .c(new_n87_), .d(x16), .O(z10));
  aoi21  g044(.a(new_n88_), .b(new_n53_), .c(new_n56_), .O(new_n91_));
  inv1   g045(.a(x17), .O(new_n92_));
  nand4  g046(.a(new_n56_), .b(new_n53_), .c(new_n48_), .d(new_n92_), .O(new_n93_));
  inv1   g047(.a(new_n93_), .O(new_n94_));
  oai21  g048(.a(new_n94_), .b(new_n91_), .c(x16), .O(new_n95_));
  inv1   g049(.a(x02), .O(new_n96_));
  nand2  g050(.a(x18), .b(new_n96_), .O(new_n97_));
  inv1   g051(.a(x10), .O(new_n98_));
  inv1   g052(.a(x18), .O(new_n99_));
  aoi21  g053(.a(new_n99_), .b(new_n98_), .c(x16), .O(new_n100_));
  aoi21  g054(.a(new_n100_), .b(new_n97_), .c(new_n50_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n95_), .O(z11));
  nand2  g056(.a(x18), .b(x03), .O(new_n103_));
  nand2  g057(.a(new_n80_), .b(x11), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n82_), .O(new_n106_));
  xor2a  g060(.a(new_n93_), .b(x22), .O(new_n107_));
  oai21  g061(.a(new_n107_), .b(new_n84_), .c(new_n106_), .O(z12));
  inv1   g062(.a(x22), .O(new_n109_));
  nor2   g063(.a(x21), .b(x20), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n88_), .c(new_n109_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(x23), .O(new_n112_));
  nor2   g066(.a(x23), .b(x22), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n110_), .c(new_n88_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n83_), .O(new_n116_));
  oai21  g070(.a(x18), .b(new_n64_), .c(new_n49_), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n59_), .c(new_n82_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n116_), .O(z13));
  nand2  g073(.a(new_n114_), .b(x24), .O(new_n120_));
  nor2   g074(.a(x24), .b(x21), .O(new_n121_));
  nand4  g075(.a(new_n121_), .b(new_n113_), .c(new_n88_), .d(new_n53_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(x16), .O(new_n124_));
  inv1   g078(.a(x05), .O(new_n125_));
  nand2  g079(.a(x18), .b(new_n125_), .O(new_n126_));
  aoi21  g080(.a(new_n99_), .b(new_n68_), .c(x16), .O(new_n127_));
  aoi21  g081(.a(new_n127_), .b(new_n126_), .c(new_n50_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n124_), .O(z14));
  nand2  g083(.a(x18), .b(x06), .O(new_n130_));
  nand2  g084(.a(new_n80_), .b(x14), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n82_), .O(new_n133_));
  inv1   g087(.a(x23), .O(new_n134_));
  inv1   g088(.a(x24), .O(new_n135_));
  nand4  g089(.a(new_n72_), .b(new_n135_), .c(new_n134_), .d(new_n109_), .O(new_n136_));
  inv1   g090(.a(new_n136_), .O(new_n137_));
  aoi22  g091(.a(new_n137_), .b(new_n94_), .c(new_n122_), .d(x25), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n84_), .c(new_n133_), .O(z15));
  nand2  g093(.a(x18), .b(x07), .O(new_n140_));
  nand2  g094(.a(new_n80_), .b(x15), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n82_), .O(new_n143_));
  inv1   g097(.a(x26), .O(new_n144_));
  nor2   g098(.a(x25), .b(x24), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n113_), .c(new_n110_), .d(new_n88_), .O(new_n146_));
  nor2   g100(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand2  g101(.a(new_n146_), .b(new_n144_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n83_), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n147_), .c(new_n143_), .O(z16));
  nand2  g104(.a(x27), .b(x17), .O(new_n151_));
  nor2   g105(.a(new_n48_), .b(x17), .O(new_n152_));
  nand4  g106(.a(new_n137_), .b(new_n110_), .c(new_n152_), .d(new_n144_), .O(new_n153_));
  aoi21  g107(.a(new_n153_), .b(new_n151_), .c(new_n84_), .O(z17));
endmodule


