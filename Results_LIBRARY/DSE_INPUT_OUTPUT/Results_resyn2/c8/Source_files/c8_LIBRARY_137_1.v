// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_;
  inv1   g00(.a(x27), .O(new_n47_));
  nand2  g01(.a(x27), .b(x07), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(z08));
  aoi22  g03(.a(z08), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  oai22  g04(.a(new_n48_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g05(.a(new_n48_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g06(.a(z08), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  oai22  g07(.a(new_n48_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g08(.a(new_n48_), .b(x13), .c(x27), .d(x24), .O(z05));
  oai22  g09(.a(new_n48_), .b(x14), .c(x27), .d(x25), .O(z06));
  oai22  g10(.a(new_n48_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g11(.a(new_n47_), .b(x07), .O(new_n58_));
  inv1   g12(.a(new_n58_), .O(new_n59_));
  inv1   g13(.a(x08), .O(new_n60_));
  inv1   g14(.a(x16), .O(new_n61_));
  oai21  g15(.a(x18), .b(new_n60_), .c(new_n61_), .O(new_n62_));
  aoi21  g16(.a(x18), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g17(.a(x17), .O(new_n64_));
  inv1   g18(.a(x19), .O(new_n65_));
  nor2   g19(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nor2   g20(.a(x19), .b(x17), .O(new_n67_));
  nor3   g21(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  oai21  g22(.a(new_n68_), .b(new_n63_), .c(new_n59_), .O(z09));
  inv1   g23(.a(x20), .O(new_n70_));
  nor2   g24(.a(new_n67_), .b(new_n70_), .O(new_n71_));
  nor3   g25(.a(x20), .b(x19), .c(x17), .O(new_n72_));
  oai21  g26(.a(new_n72_), .b(new_n71_), .c(x16), .O(new_n73_));
  inv1   g27(.a(x18), .O(new_n74_));
  inv1   g28(.a(x09), .O(new_n75_));
  aoi21  g29(.a(new_n74_), .b(new_n75_), .c(x16), .O(new_n76_));
  oai21  g30(.a(new_n74_), .b(x01), .c(new_n76_), .O(new_n77_));
  aoi21  g31(.a(new_n77_), .b(new_n73_), .c(new_n58_), .O(z10));
  inv1   g32(.a(x21), .O(new_n79_));
  nor2   g33(.a(x21), .b(x20), .O(new_n80_));
  nand2  g34(.a(new_n80_), .b(new_n67_), .O(new_n81_));
  oai21  g35(.a(new_n72_), .b(new_n79_), .c(new_n81_), .O(new_n82_));
  nand2  g36(.a(new_n82_), .b(x16), .O(new_n83_));
  inv1   g37(.a(x10), .O(new_n84_));
  aoi21  g38(.a(new_n74_), .b(new_n84_), .c(x16), .O(new_n85_));
  oai21  g39(.a(new_n74_), .b(x02), .c(new_n85_), .O(new_n86_));
  aoi21  g40(.a(new_n86_), .b(new_n83_), .c(new_n58_), .O(z11));
  inv1   g41(.a(x22), .O(new_n88_));
  aoi21  g42(.a(new_n80_), .b(new_n67_), .c(new_n88_), .O(new_n89_));
  nand3  g43(.a(new_n80_), .b(new_n67_), .c(new_n88_), .O(new_n90_));
  inv1   g44(.a(new_n90_), .O(new_n91_));
  oai21  g45(.a(new_n91_), .b(new_n89_), .c(x16), .O(new_n92_));
  inv1   g46(.a(x11), .O(new_n93_));
  aoi21  g47(.a(new_n74_), .b(new_n93_), .c(x16), .O(new_n94_));
  oai21  g48(.a(new_n74_), .b(x03), .c(new_n94_), .O(new_n95_));
  aoi21  g49(.a(new_n95_), .b(new_n92_), .c(new_n58_), .O(z12));
  nand2  g50(.a(new_n90_), .b(x23), .O(new_n97_));
  nor2   g51(.a(x23), .b(x22), .O(new_n98_));
  nand3  g52(.a(new_n98_), .b(new_n80_), .c(new_n67_), .O(new_n99_));
  and2   g53(.a(new_n99_), .b(x16), .O(new_n100_));
  nand2  g54(.a(x18), .b(x04), .O(new_n101_));
  nand2  g55(.a(new_n74_), .b(x12), .O(new_n102_));
  nand3  g56(.a(new_n102_), .b(new_n101_), .c(new_n61_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(new_n59_), .O(new_n104_));
  aoi21  g58(.a(new_n100_), .b(new_n97_), .c(new_n104_), .O(z13));
  nand2  g59(.a(new_n99_), .b(x24), .O(new_n106_));
  nor2   g60(.a(x22), .b(x21), .O(new_n107_));
  nor2   g61(.a(x24), .b(x23), .O(new_n108_));
  nand4  g62(.a(new_n108_), .b(new_n107_), .c(new_n67_), .d(new_n70_), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(x16), .O(new_n111_));
  inv1   g65(.a(x05), .O(new_n112_));
  nand2  g66(.a(x18), .b(new_n112_), .O(new_n113_));
  inv1   g67(.a(x13), .O(new_n114_));
  aoi21  g68(.a(new_n74_), .b(new_n114_), .c(x16), .O(new_n115_));
  aoi21  g69(.a(new_n115_), .b(new_n113_), .c(new_n58_), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n111_), .O(z14));
  nand2  g71(.a(new_n109_), .b(x25), .O(new_n118_));
  nor2   g72(.a(x25), .b(x24), .O(new_n119_));
  nand4  g73(.a(new_n119_), .b(new_n98_), .c(new_n80_), .d(new_n67_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g75(.a(new_n121_), .b(x16), .O(new_n122_));
  inv1   g76(.a(x14), .O(new_n123_));
  aoi21  g77(.a(new_n74_), .b(new_n123_), .c(x16), .O(new_n124_));
  oai21  g78(.a(new_n74_), .b(x06), .c(new_n124_), .O(new_n125_));
  aoi21  g79(.a(new_n125_), .b(new_n122_), .c(new_n58_), .O(z15));
  inv1   g80(.a(x26), .O(new_n127_));
  nand3  g81(.a(new_n119_), .b(new_n98_), .c(new_n127_), .O(new_n128_));
  nor2   g82(.a(new_n128_), .b(new_n81_), .O(new_n129_));
  aoi21  g83(.a(new_n120_), .b(x26), .c(new_n129_), .O(new_n130_));
  inv1   g84(.a(x07), .O(new_n131_));
  nand2  g85(.a(x18), .b(new_n131_), .O(new_n132_));
  inv1   g86(.a(x15), .O(new_n133_));
  aoi21  g87(.a(new_n74_), .b(new_n133_), .c(x16), .O(new_n134_));
  aoi21  g88(.a(new_n134_), .b(new_n132_), .c(new_n58_), .O(new_n135_));
  oai21  g89(.a(new_n130_), .b(new_n61_), .c(new_n135_), .O(z16));
  nand3  g90(.a(new_n80_), .b(x19), .c(new_n64_), .O(new_n137_));
  oai22  g91(.a(new_n137_), .b(new_n128_), .c(new_n47_), .d(new_n64_), .O(new_n138_));
  nand2  g92(.a(new_n138_), .b(x07), .O(new_n139_));
  inv1   g93(.a(x25), .O(new_n140_));
  nand4  g94(.a(new_n47_), .b(new_n127_), .c(new_n140_), .d(new_n70_), .O(new_n141_));
  nor3   g95(.a(new_n141_), .b(new_n65_), .c(x17), .O(new_n142_));
  nand3  g96(.a(new_n142_), .b(new_n108_), .c(new_n107_), .O(new_n143_));
  aoi21  g97(.a(new_n143_), .b(new_n139_), .c(new_n61_), .O(z17));
endmodule


