// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x22), .O(new_n48_));
  aoi22  g002(.a(new_n48_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g003(.a(new_n48_), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g005(.a(new_n50_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g006(.a(x22), .O(new_n53_));
  nor2   g007(.a(x27), .b(new_n53_), .O(new_n54_));
  inv1   g008(.a(x11), .O(new_n55_));
  nor2   g009(.a(x22), .b(new_n55_), .O(new_n56_));
  aoi21  g010(.a(new_n56_), .b(x27), .c(new_n54_), .O(z03));
  aoi22  g011(.a(new_n48_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  aoi22  g012(.a(new_n48_), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  aoi22  g013(.a(new_n48_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  aoi22  g014(.a(new_n48_), .b(x15), .c(new_n47_), .d(x26), .O(z07));
  nor2   g015(.a(new_n47_), .b(new_n53_), .O(new_n62_));
  inv1   g016(.a(x18), .O(new_n63_));
  inv1   g017(.a(x08), .O(new_n64_));
  aoi21  g018(.a(new_n63_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g019(.a(new_n63_), .b(x00), .c(new_n65_), .O(new_n66_));
  inv1   g020(.a(x19), .O(new_n67_));
  nand2  g021(.a(new_n67_), .b(x17), .O(new_n68_));
  inv1   g022(.a(x16), .O(new_n69_));
  nor2   g023(.a(new_n67_), .b(x17), .O(new_n70_));
  nor2   g024(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g025(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n66_), .c(new_n62_), .O(z09));
  nor2   g027(.a(x19), .b(x17), .O(new_n74_));
  xor2a  g028(.a(new_n74_), .b(x20), .O(new_n75_));
  nand2  g029(.a(new_n75_), .b(x16), .O(new_n76_));
  inv1   g030(.a(x09), .O(new_n77_));
  aoi21  g031(.a(new_n63_), .b(new_n77_), .c(x16), .O(new_n78_));
  oai21  g032(.a(new_n63_), .b(x01), .c(new_n78_), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n76_), .c(new_n62_), .O(z10));
  nor2   g034(.a(x21), .b(x20), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  inv1   g036(.a(new_n82_), .O(new_n83_));
  inv1   g037(.a(x20), .O(new_n84_));
  inv1   g038(.a(x21), .O(new_n85_));
  aoi21  g039(.a(new_n74_), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n83_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x10), .O(new_n88_));
  aoi21  g042(.a(new_n63_), .b(new_n88_), .c(x16), .O(new_n89_));
  oai21  g043(.a(new_n63_), .b(x02), .c(new_n89_), .O(new_n90_));
  aoi21  g044(.a(new_n90_), .b(new_n87_), .c(new_n62_), .O(z11));
  xor2a  g045(.a(new_n82_), .b(x22), .O(new_n92_));
  inv1   g046(.a(x03), .O(new_n93_));
  nand2  g047(.a(x18), .b(new_n93_), .O(new_n94_));
  aoi21  g048(.a(new_n63_), .b(new_n55_), .c(x16), .O(new_n95_));
  aoi21  g049(.a(new_n95_), .b(new_n94_), .c(new_n62_), .O(new_n96_));
  oai21  g050(.a(new_n92_), .b(new_n69_), .c(new_n96_), .O(z12));
  inv1   g051(.a(x23), .O(new_n98_));
  nand3  g052(.a(new_n81_), .b(new_n74_), .c(new_n53_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  inv1   g054(.a(new_n99_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(x23), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n100_), .c(x16), .O(new_n103_));
  inv1   g057(.a(x04), .O(new_n104_));
  nand2  g058(.a(x18), .b(new_n104_), .O(new_n105_));
  inv1   g059(.a(x12), .O(new_n106_));
  aoi21  g060(.a(new_n63_), .b(new_n106_), .c(x16), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n105_), .c(new_n62_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n103_), .O(z13));
  nand3  g063(.a(new_n81_), .b(new_n74_), .c(new_n98_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x24), .O(new_n111_));
  nor2   g065(.a(x24), .b(x23), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n81_), .c(new_n74_), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n111_), .c(new_n53_), .O(new_n114_));
  nand2  g068(.a(new_n47_), .b(x24), .O(new_n115_));
  aoi21  g069(.a(new_n115_), .b(x22), .c(new_n69_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nor2   g071(.a(new_n62_), .b(x16), .O(new_n118_));
  inv1   g072(.a(x13), .O(new_n119_));
  nand2  g073(.a(new_n63_), .b(new_n119_), .O(new_n120_));
  inv1   g074(.a(x05), .O(new_n121_));
  nand2  g075(.a(x18), .b(new_n121_), .O(new_n122_));
  nand3  g076(.a(new_n122_), .b(new_n120_), .c(new_n118_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n117_), .O(z14));
  nand2  g078(.a(new_n113_), .b(x25), .O(new_n125_));
  nor2   g079(.a(x25), .b(x21), .O(new_n126_));
  nand4  g080(.a(new_n126_), .b(new_n112_), .c(new_n74_), .d(new_n84_), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(new_n125_), .c(new_n53_), .O(new_n128_));
  nand2  g082(.a(new_n47_), .b(x25), .O(new_n129_));
  aoi21  g083(.a(new_n129_), .b(x22), .c(new_n69_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  inv1   g085(.a(x14), .O(new_n132_));
  nand2  g086(.a(new_n63_), .b(new_n132_), .O(new_n133_));
  inv1   g087(.a(x06), .O(new_n134_));
  nand2  g088(.a(x18), .b(new_n134_), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n133_), .c(new_n118_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n131_), .O(z15));
  nor2   g091(.a(x26), .b(x25), .O(new_n138_));
  nand4  g092(.a(new_n138_), .b(new_n112_), .c(new_n81_), .d(new_n74_), .O(new_n139_));
  nand2  g093(.a(new_n127_), .b(x26), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n139_), .c(new_n53_), .O(new_n141_));
  nand2  g095(.a(new_n47_), .b(x26), .O(new_n142_));
  aoi21  g096(.a(new_n142_), .b(x22), .c(new_n69_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  inv1   g098(.a(x15), .O(new_n145_));
  nand2  g099(.a(new_n63_), .b(new_n145_), .O(new_n146_));
  inv1   g100(.a(x07), .O(new_n147_));
  nand2  g101(.a(x18), .b(new_n147_), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n146_), .c(new_n118_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n144_), .O(z16));
  nand2  g104(.a(x27), .b(x17), .O(new_n151_));
  nand4  g105(.a(new_n138_), .b(new_n112_), .c(new_n81_), .d(new_n70_), .O(new_n152_));
  nand2  g106(.a(new_n53_), .b(x16), .O(new_n153_));
  aoi21  g107(.a(new_n152_), .b(new_n151_), .c(new_n153_), .O(z17));
  buf    g108(.a(x27), .O(z08));
endmodule


