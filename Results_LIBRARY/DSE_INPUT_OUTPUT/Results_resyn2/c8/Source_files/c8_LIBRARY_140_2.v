// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n53_, new_n54_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(x27), .b(x11), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(z08));
  oai22  g03(.a(z08), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g04(.a(z08), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  aoi22  g05(.a(new_n48_), .b(x21), .c(x27), .d(x10), .O(z02));
  inv1   g06(.a(x11), .O(new_n53_));
  nor2   g07(.a(new_n47_), .b(new_n53_), .O(new_n54_));
  aoi21  g08(.a(new_n48_), .b(x22), .c(new_n54_), .O(z03));
  aoi22  g09(.a(new_n48_), .b(x23), .c(x27), .d(x12), .O(z04));
  oai22  g10(.a(z08), .b(x24), .c(new_n47_), .d(x13), .O(z05));
  aoi22  g11(.a(new_n48_), .b(x25), .c(x27), .d(x14), .O(z06));
  oai22  g12(.a(z08), .b(x26), .c(new_n47_), .d(x15), .O(z07));
  nor2   g13(.a(x27), .b(new_n53_), .O(new_n60_));
  nor2   g14(.a(x19), .b(x17), .O(new_n61_));
  and2   g15(.a(x19), .b(x17), .O(new_n62_));
  oai21  g16(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g17(.a(x18), .O(new_n64_));
  inv1   g18(.a(x08), .O(new_n65_));
  aoi21  g19(.a(new_n64_), .b(new_n65_), .c(x16), .O(new_n66_));
  oai21  g20(.a(new_n64_), .b(x00), .c(new_n66_), .O(new_n67_));
  aoi21  g21(.a(new_n67_), .b(new_n63_), .c(new_n60_), .O(z09));
  inv1   g22(.a(x16), .O(new_n69_));
  inv1   g23(.a(x20), .O(new_n70_));
  xor2a  g24(.a(new_n61_), .b(new_n70_), .O(new_n71_));
  inv1   g25(.a(x01), .O(new_n72_));
  nand2  g26(.a(x18), .b(new_n72_), .O(new_n73_));
  inv1   g27(.a(x09), .O(new_n74_));
  aoi21  g28(.a(new_n64_), .b(new_n74_), .c(x16), .O(new_n75_));
  aoi21  g29(.a(new_n75_), .b(new_n73_), .c(new_n60_), .O(new_n76_));
  oai21  g30(.a(new_n71_), .b(new_n69_), .c(new_n76_), .O(z10));
  nand2  g31(.a(new_n61_), .b(new_n70_), .O(new_n78_));
  nor2   g32(.a(x21), .b(x20), .O(new_n79_));
  aoi22  g33(.a(new_n79_), .b(new_n61_), .c(new_n78_), .d(x21), .O(new_n80_));
  inv1   g34(.a(x02), .O(new_n81_));
  nand2  g35(.a(x18), .b(new_n81_), .O(new_n82_));
  inv1   g36(.a(x10), .O(new_n83_));
  aoi21  g37(.a(new_n64_), .b(new_n83_), .c(x16), .O(new_n84_));
  aoi21  g38(.a(new_n84_), .b(new_n82_), .c(new_n60_), .O(new_n85_));
  oai21  g39(.a(new_n80_), .b(new_n69_), .c(new_n85_), .O(z11));
  inv1   g40(.a(x22), .O(new_n87_));
  nand3  g41(.a(new_n79_), .b(new_n61_), .c(new_n87_), .O(new_n88_));
  nand2  g42(.a(new_n79_), .b(new_n61_), .O(new_n89_));
  aoi21  g43(.a(new_n89_), .b(x22), .c(new_n69_), .O(new_n90_));
  inv1   g44(.a(new_n60_), .O(new_n91_));
  nand2  g45(.a(x18), .b(x03), .O(new_n92_));
  nand2  g46(.a(new_n64_), .b(x11), .O(new_n93_));
  nand3  g47(.a(new_n93_), .b(new_n92_), .c(new_n69_), .O(new_n94_));
  nand2  g48(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  aoi21  g49(.a(new_n90_), .b(new_n88_), .c(new_n95_), .O(z12));
  xor2a  g50(.a(new_n88_), .b(x23), .O(new_n97_));
  inv1   g51(.a(x04), .O(new_n98_));
  nand2  g52(.a(x18), .b(new_n98_), .O(new_n99_));
  inv1   g53(.a(x12), .O(new_n100_));
  aoi21  g54(.a(new_n64_), .b(new_n100_), .c(x16), .O(new_n101_));
  aoi21  g55(.a(new_n101_), .b(new_n99_), .c(new_n60_), .O(new_n102_));
  oai21  g56(.a(new_n97_), .b(new_n69_), .c(new_n102_), .O(z13));
  inv1   g57(.a(x23), .O(new_n104_));
  nand4  g58(.a(new_n79_), .b(new_n61_), .c(new_n104_), .d(new_n87_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(x24), .O(new_n106_));
  nor2   g60(.a(x24), .b(x23), .O(new_n107_));
  nor2   g61(.a(x22), .b(x21), .O(new_n108_));
  nand4  g62(.a(new_n108_), .b(new_n107_), .c(new_n61_), .d(new_n70_), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(x16), .O(new_n111_));
  inv1   g65(.a(x05), .O(new_n112_));
  nand2  g66(.a(x18), .b(new_n112_), .O(new_n113_));
  inv1   g67(.a(x13), .O(new_n114_));
  aoi21  g68(.a(new_n64_), .b(new_n114_), .c(x16), .O(new_n115_));
  aoi21  g69(.a(new_n115_), .b(new_n113_), .c(new_n60_), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n111_), .O(z14));
  nand2  g71(.a(new_n109_), .b(x25), .O(new_n118_));
  nor2   g72(.a(x25), .b(x22), .O(new_n119_));
  nand4  g73(.a(new_n119_), .b(new_n107_), .c(new_n79_), .d(new_n61_), .O(new_n120_));
  and2   g74(.a(new_n120_), .b(x16), .O(new_n121_));
  nand2  g75(.a(x18), .b(x06), .O(new_n122_));
  nand2  g76(.a(new_n64_), .b(x14), .O(new_n123_));
  nand3  g77(.a(new_n123_), .b(new_n122_), .c(new_n69_), .O(new_n124_));
  nand2  g78(.a(new_n124_), .b(new_n91_), .O(new_n125_));
  aoi21  g79(.a(new_n121_), .b(new_n118_), .c(new_n125_), .O(z15));
  inv1   g80(.a(x26), .O(new_n127_));
  nor2   g81(.a(new_n120_), .b(new_n127_), .O(new_n128_));
  nand2  g82(.a(new_n120_), .b(new_n127_), .O(new_n129_));
  nand2  g83(.a(new_n129_), .b(x16), .O(new_n130_));
  inv1   g84(.a(x07), .O(new_n131_));
  nand2  g85(.a(x18), .b(new_n131_), .O(new_n132_));
  inv1   g86(.a(x15), .O(new_n133_));
  aoi21  g87(.a(new_n64_), .b(new_n133_), .c(x16), .O(new_n134_));
  aoi21  g88(.a(new_n134_), .b(new_n132_), .c(new_n60_), .O(new_n135_));
  oai21  g89(.a(new_n130_), .b(new_n128_), .c(new_n135_), .O(z16));
  inv1   g90(.a(x17), .O(new_n137_));
  nand2  g91(.a(new_n119_), .b(new_n107_), .O(new_n138_));
  nand4  g92(.a(new_n79_), .b(new_n127_), .c(x19), .d(new_n137_), .O(new_n139_));
  oai22  g93(.a(new_n139_), .b(new_n138_), .c(new_n47_), .d(new_n137_), .O(new_n140_));
  nand2  g94(.a(new_n140_), .b(x16), .O(new_n141_));
  nand2  g95(.a(new_n141_), .b(new_n91_), .O(z17));
endmodule


