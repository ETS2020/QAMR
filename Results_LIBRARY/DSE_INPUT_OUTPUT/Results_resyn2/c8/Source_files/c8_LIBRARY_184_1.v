// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_;
  inv1   g00(.a(x14), .O(new_n47_));
  inv1   g01(.a(x27), .O(new_n48_));
  nor2   g02(.a(new_n48_), .b(new_n47_), .O(z08));
  inv1   g03(.a(z08), .O(new_n50_));
  oai22  g04(.a(new_n50_), .b(x08), .c(x27), .d(x19), .O(z00));
  aoi22  g05(.a(z08), .b(x09), .c(new_n48_), .d(x20), .O(z01));
  aoi22  g06(.a(z08), .b(x10), .c(new_n48_), .d(x21), .O(z02));
  aoi22  g07(.a(z08), .b(x11), .c(new_n48_), .d(x22), .O(z03));
  oai22  g08(.a(new_n50_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g09(.a(new_n50_), .b(x13), .c(x27), .d(x24), .O(z05));
  nor2   g10(.a(x27), .b(x25), .O(z06));
  aoi22  g11(.a(z08), .b(x15), .c(new_n48_), .d(x26), .O(z07));
  nor2   g12(.a(new_n48_), .b(x14), .O(new_n59_));
  inv1   g13(.a(new_n59_), .O(new_n60_));
  inv1   g14(.a(x08), .O(new_n61_));
  inv1   g15(.a(x16), .O(new_n62_));
  oai21  g16(.a(x18), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  aoi21  g17(.a(x18), .b(x00), .c(new_n63_), .O(new_n64_));
  inv1   g18(.a(x17), .O(new_n65_));
  inv1   g19(.a(x19), .O(new_n66_));
  nand2  g20(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g21(.a(new_n67_), .b(x16), .O(new_n68_));
  aoi21  g22(.a(x19), .b(x17), .c(new_n68_), .O(new_n69_));
  oai21  g23(.a(new_n69_), .b(new_n64_), .c(new_n60_), .O(z09));
  inv1   g24(.a(x20), .O(new_n71_));
  nor2   g25(.a(x19), .b(x17), .O(new_n72_));
  nor2   g26(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g27(.a(new_n67_), .b(x20), .O(new_n74_));
  oai21  g28(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  inv1   g29(.a(x18), .O(new_n76_));
  inv1   g30(.a(x09), .O(new_n77_));
  aoi21  g31(.a(new_n76_), .b(new_n77_), .c(x16), .O(new_n78_));
  oai21  g32(.a(new_n76_), .b(x01), .c(new_n78_), .O(new_n79_));
  aoi21  g33(.a(new_n79_), .b(new_n75_), .c(new_n59_), .O(z10));
  inv1   g34(.a(x21), .O(new_n81_));
  aoi21  g35(.a(new_n72_), .b(new_n71_), .c(new_n81_), .O(new_n82_));
  nor2   g36(.a(x21), .b(x20), .O(new_n83_));
  nand2  g37(.a(new_n83_), .b(new_n72_), .O(new_n84_));
  inv1   g38(.a(new_n84_), .O(new_n85_));
  oai21  g39(.a(new_n85_), .b(new_n82_), .c(x16), .O(new_n86_));
  inv1   g40(.a(x10), .O(new_n87_));
  aoi21  g41(.a(new_n76_), .b(new_n87_), .c(x16), .O(new_n88_));
  oai21  g42(.a(new_n76_), .b(x02), .c(new_n88_), .O(new_n89_));
  aoi21  g43(.a(new_n89_), .b(new_n86_), .c(new_n59_), .O(z11));
  xor2a  g44(.a(new_n84_), .b(x22), .O(new_n91_));
  inv1   g45(.a(x03), .O(new_n92_));
  nand2  g46(.a(x18), .b(new_n92_), .O(new_n93_));
  inv1   g47(.a(x11), .O(new_n94_));
  aoi21  g48(.a(new_n76_), .b(new_n94_), .c(x16), .O(new_n95_));
  aoi21  g49(.a(new_n95_), .b(new_n93_), .c(new_n59_), .O(new_n96_));
  oai21  g50(.a(new_n91_), .b(new_n62_), .c(new_n96_), .O(z12));
  inv1   g51(.a(x22), .O(new_n98_));
  nand3  g52(.a(new_n83_), .b(new_n72_), .c(new_n98_), .O(new_n99_));
  nand2  g53(.a(new_n99_), .b(x23), .O(new_n100_));
  nor2   g54(.a(x23), .b(x22), .O(new_n101_));
  nand3  g55(.a(new_n101_), .b(new_n83_), .c(new_n72_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g58(.a(x04), .O(new_n105_));
  nand2  g59(.a(x18), .b(new_n105_), .O(new_n106_));
  inv1   g60(.a(x12), .O(new_n107_));
  aoi21  g61(.a(new_n76_), .b(new_n107_), .c(x16), .O(new_n108_));
  aoi21  g62(.a(new_n108_), .b(new_n106_), .c(new_n59_), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(new_n104_), .O(z13));
  nand2  g64(.a(new_n102_), .b(x24), .O(new_n111_));
  nor2   g65(.a(x24), .b(x21), .O(new_n112_));
  nand4  g66(.a(new_n112_), .b(new_n101_), .c(new_n72_), .d(new_n71_), .O(new_n113_));
  nand2  g67(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g68(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g69(.a(x05), .O(new_n116_));
  nand2  g70(.a(x18), .b(new_n116_), .O(new_n117_));
  inv1   g71(.a(x13), .O(new_n118_));
  aoi21  g72(.a(new_n76_), .b(new_n118_), .c(x16), .O(new_n119_));
  aoi21  g73(.a(new_n119_), .b(new_n117_), .c(new_n59_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(new_n115_), .O(z14));
  nand2  g75(.a(new_n113_), .b(x25), .O(new_n122_));
  nor2   g76(.a(x25), .b(x24), .O(new_n123_));
  nand4  g77(.a(new_n123_), .b(new_n101_), .c(new_n83_), .d(new_n72_), .O(new_n124_));
  and2   g78(.a(new_n124_), .b(x16), .O(new_n125_));
  nand2  g79(.a(x18), .b(x06), .O(new_n126_));
  nand2  g80(.a(new_n76_), .b(x14), .O(new_n127_));
  nand3  g81(.a(new_n127_), .b(new_n126_), .c(new_n62_), .O(new_n128_));
  nand2  g82(.a(new_n128_), .b(new_n60_), .O(new_n129_));
  aoi21  g83(.a(new_n125_), .b(new_n122_), .c(new_n129_), .O(z15));
  inv1   g84(.a(x26), .O(new_n131_));
  nor2   g85(.a(new_n124_), .b(new_n131_), .O(new_n132_));
  nand2  g86(.a(new_n124_), .b(new_n131_), .O(new_n133_));
  nand2  g87(.a(new_n133_), .b(x16), .O(new_n134_));
  inv1   g88(.a(x07), .O(new_n135_));
  nand2  g89(.a(x18), .b(new_n135_), .O(new_n136_));
  inv1   g90(.a(x15), .O(new_n137_));
  aoi21  g91(.a(new_n76_), .b(new_n137_), .c(x16), .O(new_n138_));
  aoi21  g92(.a(new_n138_), .b(new_n136_), .c(new_n59_), .O(new_n139_));
  oai21  g93(.a(new_n134_), .b(new_n132_), .c(new_n139_), .O(z16));
  nand2  g94(.a(new_n123_), .b(new_n101_), .O(new_n141_));
  nand2  g95(.a(x27), .b(x17), .O(new_n142_));
  nand4  g96(.a(new_n83_), .b(new_n131_), .c(x19), .d(new_n65_), .O(new_n143_));
  oai21  g97(.a(new_n143_), .b(new_n141_), .c(new_n142_), .O(new_n144_));
  nand2  g98(.a(new_n144_), .b(x16), .O(new_n145_));
  nand2  g99(.a(new_n145_), .b(new_n60_), .O(z17));
endmodule


