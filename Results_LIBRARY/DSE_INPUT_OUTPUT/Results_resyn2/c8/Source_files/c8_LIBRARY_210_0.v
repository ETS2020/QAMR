// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x19), .O(new_n48_));
  nand3  g02(.a(new_n47_), .b(new_n48_), .c(x08), .O(new_n49_));
  oai21  g03(.a(new_n47_), .b(x08), .c(new_n49_), .O(z00));
  inv1   g04(.a(x08), .O(new_n51_));
  nor2   g05(.a(x27), .b(new_n51_), .O(new_n52_));
  inv1   g06(.a(new_n52_), .O(new_n53_));
  oai22  g07(.a(new_n53_), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  aoi22  g08(.a(new_n52_), .b(x21), .c(x27), .d(x10), .O(z02));
  aoi22  g09(.a(new_n52_), .b(x22), .c(x27), .d(x11), .O(z03));
  aoi22  g10(.a(new_n52_), .b(x23), .c(x27), .d(x12), .O(z04));
  oai22  g11(.a(new_n53_), .b(x24), .c(new_n47_), .d(x13), .O(z05));
  oai22  g12(.a(new_n53_), .b(x25), .c(new_n47_), .d(x14), .O(z06));
  oai22  g13(.a(new_n53_), .b(x26), .c(new_n47_), .d(x15), .O(z07));
  inv1   g14(.a(x18), .O(new_n61_));
  aoi21  g15(.a(new_n61_), .b(new_n51_), .c(x16), .O(new_n62_));
  oai21  g16(.a(new_n61_), .b(x00), .c(new_n62_), .O(new_n63_));
  nor2   g17(.a(x27), .b(x08), .O(new_n64_));
  inv1   g18(.a(new_n64_), .O(new_n65_));
  inv1   g19(.a(x17), .O(new_n66_));
  nor2   g20(.a(new_n48_), .b(new_n66_), .O(new_n67_));
  nor2   g21(.a(x19), .b(x17), .O(new_n68_));
  oai21  g22(.a(new_n68_), .b(new_n67_), .c(x16), .O(new_n69_));
  nand3  g23(.a(new_n69_), .b(new_n65_), .c(new_n63_), .O(z09));
  inv1   g24(.a(x16), .O(new_n71_));
  inv1   g25(.a(x20), .O(new_n72_));
  xor2a  g26(.a(new_n68_), .b(new_n72_), .O(new_n73_));
  inv1   g27(.a(x01), .O(new_n74_));
  nand2  g28(.a(x18), .b(new_n74_), .O(new_n75_));
  inv1   g29(.a(x09), .O(new_n76_));
  aoi21  g30(.a(new_n61_), .b(new_n76_), .c(x16), .O(new_n77_));
  aoi21  g31(.a(new_n77_), .b(new_n75_), .c(new_n64_), .O(new_n78_));
  oai21  g32(.a(new_n73_), .b(new_n71_), .c(new_n78_), .O(z10));
  inv1   g33(.a(x21), .O(new_n80_));
  aoi21  g34(.a(new_n68_), .b(new_n72_), .c(new_n80_), .O(new_n81_));
  nor2   g35(.a(x21), .b(x20), .O(new_n82_));
  nand2  g36(.a(new_n82_), .b(new_n68_), .O(new_n83_));
  inv1   g37(.a(new_n83_), .O(new_n84_));
  oai21  g38(.a(new_n84_), .b(new_n81_), .c(x16), .O(new_n85_));
  inv1   g39(.a(x10), .O(new_n86_));
  aoi21  g40(.a(new_n61_), .b(new_n86_), .c(x16), .O(new_n87_));
  oai21  g41(.a(new_n61_), .b(x02), .c(new_n87_), .O(new_n88_));
  aoi21  g42(.a(new_n88_), .b(new_n85_), .c(new_n64_), .O(z11));
  inv1   g43(.a(x22), .O(new_n90_));
  aoi21  g44(.a(new_n82_), .b(new_n68_), .c(new_n90_), .O(new_n91_));
  nand3  g45(.a(new_n82_), .b(new_n68_), .c(new_n90_), .O(new_n92_));
  inv1   g46(.a(new_n92_), .O(new_n93_));
  oai21  g47(.a(new_n93_), .b(new_n91_), .c(x16), .O(new_n94_));
  inv1   g48(.a(x11), .O(new_n95_));
  aoi21  g49(.a(new_n61_), .b(new_n95_), .c(x16), .O(new_n96_));
  oai21  g50(.a(new_n61_), .b(x03), .c(new_n96_), .O(new_n97_));
  aoi21  g51(.a(new_n97_), .b(new_n94_), .c(new_n64_), .O(z12));
  nand2  g52(.a(new_n92_), .b(x23), .O(new_n99_));
  nor2   g53(.a(x23), .b(x22), .O(new_n100_));
  nand3  g54(.a(new_n100_), .b(new_n82_), .c(new_n68_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(x16), .O(new_n103_));
  inv1   g57(.a(x12), .O(new_n104_));
  aoi21  g58(.a(new_n61_), .b(new_n104_), .c(x16), .O(new_n105_));
  oai21  g59(.a(new_n61_), .b(x04), .c(new_n105_), .O(new_n106_));
  aoi21  g60(.a(new_n106_), .b(new_n103_), .c(new_n64_), .O(z13));
  inv1   g61(.a(x24), .O(new_n108_));
  xor2a  g62(.a(new_n101_), .b(new_n108_), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(x16), .O(new_n110_));
  inv1   g64(.a(x13), .O(new_n111_));
  aoi21  g65(.a(new_n61_), .b(new_n111_), .c(x16), .O(new_n112_));
  oai21  g66(.a(new_n61_), .b(x05), .c(new_n112_), .O(new_n113_));
  aoi21  g67(.a(new_n113_), .b(new_n110_), .c(new_n64_), .O(z14));
  oai21  g68(.a(new_n101_), .b(x24), .c(x25), .O(new_n115_));
  nor2   g69(.a(x25), .b(x24), .O(new_n116_));
  nand4  g70(.a(new_n116_), .b(new_n100_), .c(new_n82_), .d(new_n68_), .O(new_n117_));
  nand2  g71(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g72(.a(new_n118_), .b(x16), .O(new_n119_));
  inv1   g73(.a(x06), .O(new_n120_));
  nand2  g74(.a(x18), .b(new_n120_), .O(new_n121_));
  inv1   g75(.a(x14), .O(new_n122_));
  aoi21  g76(.a(new_n61_), .b(new_n122_), .c(x16), .O(new_n123_));
  aoi21  g77(.a(new_n123_), .b(new_n121_), .c(new_n64_), .O(new_n124_));
  nand2  g78(.a(new_n124_), .b(new_n119_), .O(z15));
  nand2  g79(.a(new_n116_), .b(new_n100_), .O(new_n126_));
  inv1   g80(.a(new_n126_), .O(new_n127_));
  nand3  g81(.a(new_n127_), .b(new_n84_), .c(x26), .O(new_n128_));
  inv1   g82(.a(x26), .O(new_n129_));
  aoi21  g83(.a(new_n117_), .b(new_n129_), .c(new_n71_), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  inv1   g85(.a(x15), .O(new_n132_));
  aoi21  g86(.a(new_n61_), .b(new_n132_), .c(x16), .O(new_n133_));
  oai21  g87(.a(new_n61_), .b(x07), .c(new_n133_), .O(new_n134_));
  aoi21  g88(.a(new_n134_), .b(new_n131_), .c(new_n64_), .O(z16));
  nor2   g89(.a(x24), .b(x21), .O(new_n136_));
  nor2   g90(.a(x26), .b(x25), .O(new_n137_));
  nor2   g91(.a(x20), .b(new_n48_), .O(new_n138_));
  nand4  g92(.a(new_n138_), .b(new_n137_), .c(new_n136_), .d(new_n100_), .O(new_n139_));
  oai21  g93(.a(x17), .b(new_n51_), .c(new_n47_), .O(new_n140_));
  nand2  g94(.a(new_n140_), .b(x16), .O(new_n141_));
  aoi21  g95(.a(new_n139_), .b(new_n66_), .c(new_n141_), .O(z17));
  buf    g96(.a(x27), .O(z08));
endmodule


