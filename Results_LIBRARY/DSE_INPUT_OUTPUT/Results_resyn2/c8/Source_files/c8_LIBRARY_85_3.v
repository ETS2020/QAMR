// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n160_, new_n161_, new_n162_, new_n163_;
  aoi21  g000(.a(x27), .b(x08), .c(x19), .O(z00));
  inv1   g001(.a(x27), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x19), .O(z08));
  inv1   g003(.a(z08), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  aoi22  g005(.a(z08), .b(x10), .c(new_n48_), .d(x21), .O(z02));
  oai22  g006(.a(new_n50_), .b(x11), .c(x27), .d(x22), .O(z03));
  aoi22  g007(.a(z08), .b(x12), .c(new_n48_), .d(x23), .O(z04));
  oai22  g008(.a(new_n50_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g009(.a(z08), .b(x14), .c(new_n48_), .d(x25), .O(z06));
  aoi22  g010(.a(z08), .b(x15), .c(new_n48_), .d(x26), .O(z07));
  inv1   g011(.a(x18), .O(new_n58_));
  inv1   g012(.a(x08), .O(new_n59_));
  aoi21  g013(.a(new_n58_), .b(new_n59_), .c(x16), .O(new_n60_));
  oai21  g014(.a(new_n58_), .b(x00), .c(new_n60_), .O(new_n61_));
  nand2  g015(.a(x27), .b(x19), .O(new_n62_));
  inv1   g016(.a(x17), .O(new_n63_));
  inv1   g017(.a(x19), .O(new_n64_));
  nor2   g018(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor2   g019(.a(x19), .b(x17), .O(new_n66_));
  oai21  g020(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nand3  g021(.a(new_n67_), .b(new_n62_), .c(new_n61_), .O(z09));
  inv1   g022(.a(new_n62_), .O(new_n69_));
  inv1   g023(.a(x20), .O(new_n70_));
  nor2   g024(.a(new_n66_), .b(new_n70_), .O(new_n71_));
  nor3   g025(.a(x20), .b(x19), .c(x17), .O(new_n72_));
  oai21  g026(.a(new_n72_), .b(new_n71_), .c(x16), .O(new_n73_));
  inv1   g027(.a(x09), .O(new_n74_));
  aoi21  g028(.a(new_n58_), .b(new_n74_), .c(x16), .O(new_n75_));
  oai21  g029(.a(new_n58_), .b(x01), .c(new_n75_), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n73_), .c(new_n69_), .O(z10));
  oai21  g031(.a(x20), .b(x17), .c(x21), .O(new_n78_));
  nor2   g032(.a(x21), .b(x20), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(x16), .O(new_n82_));
  nand2  g036(.a(x21), .b(x16), .O(new_n83_));
  aoi21  g037(.a(new_n83_), .b(new_n48_), .c(new_n64_), .O(new_n84_));
  inv1   g038(.a(x02), .O(new_n85_));
  nand2  g039(.a(x18), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x10), .O(new_n87_));
  aoi21  g041(.a(new_n58_), .b(new_n87_), .c(x16), .O(new_n88_));
  aoi21  g042(.a(new_n88_), .b(new_n86_), .c(new_n84_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n82_), .O(z11));
  inv1   g044(.a(x16), .O(new_n91_));
  nor3   g045(.a(x21), .b(x20), .c(x17), .O(new_n92_));
  inv1   g046(.a(new_n92_), .O(new_n93_));
  nor2   g047(.a(x22), .b(x21), .O(new_n94_));
  aoi22  g048(.a(new_n94_), .b(new_n72_), .c(new_n93_), .d(x22), .O(new_n95_));
  nand2  g049(.a(x22), .b(x16), .O(new_n96_));
  aoi21  g050(.a(new_n96_), .b(new_n48_), .c(new_n64_), .O(new_n97_));
  inv1   g051(.a(x03), .O(new_n98_));
  nand2  g052(.a(x18), .b(new_n98_), .O(new_n99_));
  inv1   g053(.a(x11), .O(new_n100_));
  aoi21  g054(.a(new_n58_), .b(new_n100_), .c(x16), .O(new_n101_));
  aoi21  g055(.a(new_n101_), .b(new_n99_), .c(new_n97_), .O(new_n102_));
  oai21  g056(.a(new_n95_), .b(new_n91_), .c(new_n102_), .O(z12));
  nand3  g057(.a(new_n94_), .b(new_n70_), .c(new_n63_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(x23), .O(new_n105_));
  nor2   g059(.a(x23), .b(x22), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n92_), .c(new_n64_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x16), .O(new_n109_));
  nand2  g063(.a(x23), .b(x16), .O(new_n110_));
  aoi21  g064(.a(new_n110_), .b(new_n48_), .c(new_n64_), .O(new_n111_));
  inv1   g065(.a(x04), .O(new_n112_));
  nand2  g066(.a(x18), .b(new_n112_), .O(new_n113_));
  inv1   g067(.a(x12), .O(new_n114_));
  aoi21  g068(.a(new_n58_), .b(new_n114_), .c(x16), .O(new_n115_));
  aoi21  g069(.a(new_n115_), .b(new_n113_), .c(new_n111_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n109_), .O(z13));
  nand3  g071(.a(new_n106_), .b(new_n79_), .c(new_n63_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x24), .O(new_n119_));
  nor2   g073(.a(x24), .b(x23), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n94_), .c(new_n72_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand2  g077(.a(x24), .b(x16), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n48_), .c(new_n64_), .O(new_n125_));
  inv1   g079(.a(x05), .O(new_n126_));
  nand2  g080(.a(x18), .b(new_n126_), .O(new_n127_));
  inv1   g081(.a(x13), .O(new_n128_));
  aoi21  g082(.a(new_n58_), .b(new_n128_), .c(x16), .O(new_n129_));
  aoi21  g083(.a(new_n129_), .b(new_n127_), .c(new_n125_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n123_), .O(z14));
  nand4  g085(.a(new_n120_), .b(new_n94_), .c(new_n70_), .d(new_n63_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(x25), .O(new_n133_));
  nor2   g087(.a(x25), .b(x24), .O(new_n134_));
  nand4  g088(.a(new_n134_), .b(new_n106_), .c(new_n92_), .d(new_n64_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(x16), .O(new_n137_));
  nand2  g091(.a(x25), .b(x16), .O(new_n138_));
  aoi21  g092(.a(new_n138_), .b(new_n48_), .c(new_n64_), .O(new_n139_));
  inv1   g093(.a(x06), .O(new_n140_));
  nand2  g094(.a(x18), .b(new_n140_), .O(new_n141_));
  inv1   g095(.a(x14), .O(new_n142_));
  aoi21  g096(.a(new_n58_), .b(new_n142_), .c(x16), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(new_n141_), .c(new_n139_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n137_), .O(z15));
  inv1   g099(.a(new_n80_), .O(new_n146_));
  nand4  g100(.a(new_n134_), .b(new_n106_), .c(new_n79_), .d(new_n63_), .O(new_n147_));
  inv1   g101(.a(x26), .O(new_n148_));
  nand3  g102(.a(new_n134_), .b(new_n106_), .c(new_n148_), .O(new_n149_));
  inv1   g103(.a(new_n149_), .O(new_n150_));
  aoi22  g104(.a(new_n150_), .b(new_n146_), .c(new_n147_), .d(x26), .O(new_n151_));
  nand2  g105(.a(x26), .b(x16), .O(new_n152_));
  aoi21  g106(.a(new_n152_), .b(new_n48_), .c(new_n64_), .O(new_n153_));
  inv1   g107(.a(x07), .O(new_n154_));
  nand2  g108(.a(x18), .b(new_n154_), .O(new_n155_));
  inv1   g109(.a(x15), .O(new_n156_));
  aoi21  g110(.a(new_n58_), .b(new_n156_), .c(x16), .O(new_n157_));
  aoi21  g111(.a(new_n157_), .b(new_n155_), .c(new_n153_), .O(new_n158_));
  oai21  g112(.a(new_n151_), .b(new_n91_), .c(new_n158_), .O(z16));
  nand2  g113(.a(x27), .b(x17), .O(new_n160_));
  nand2  g114(.a(new_n92_), .b(x19), .O(new_n161_));
  oai21  g115(.a(new_n161_), .b(new_n149_), .c(new_n160_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x16), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n62_), .O(z17));
endmodule


