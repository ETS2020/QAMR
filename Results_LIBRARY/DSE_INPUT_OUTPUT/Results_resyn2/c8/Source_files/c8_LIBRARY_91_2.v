// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_;
  nor2   g00(.a(x27), .b(x14), .O(new_n47_));
  aoi22  g01(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g02(.a(x27), .O(new_n49_));
  inv1   g03(.a(new_n47_), .O(z08));
  oai22  g04(.a(z08), .b(x20), .c(new_n49_), .d(x09), .O(z01));
  oai22  g05(.a(z08), .b(x21), .c(new_n49_), .d(x10), .O(z02));
  oai22  g06(.a(z08), .b(x22), .c(new_n49_), .d(x11), .O(z03));
  oai22  g07(.a(z08), .b(x23), .c(new_n49_), .d(x12), .O(z04));
  aoi22  g08(.a(new_n47_), .b(x24), .c(x27), .d(x13), .O(z05));
  inv1   g09(.a(x14), .O(new_n56_));
  nor2   g10(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g11(.a(new_n57_), .O(new_n58_));
  inv1   g12(.a(x25), .O(new_n59_));
  oai21  g13(.a(x27), .b(new_n59_), .c(new_n56_), .O(new_n60_));
  nand2  g14(.a(new_n60_), .b(new_n58_), .O(z06));
  oai22  g15(.a(z08), .b(x26), .c(new_n49_), .d(x15), .O(z07));
  and2   g16(.a(x19), .b(x17), .O(new_n63_));
  nor2   g17(.a(x19), .b(x17), .O(new_n64_));
  oai21  g18(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  inv1   g19(.a(x18), .O(new_n66_));
  inv1   g20(.a(x08), .O(new_n67_));
  aoi21  g21(.a(new_n66_), .b(new_n67_), .c(x16), .O(new_n68_));
  oai21  g22(.a(new_n66_), .b(x00), .c(new_n68_), .O(new_n69_));
  aoi21  g23(.a(new_n69_), .b(new_n65_), .c(new_n57_), .O(z09));
  inv1   g24(.a(x20), .O(new_n71_));
  nor2   g25(.a(new_n64_), .b(new_n71_), .O(new_n72_));
  nor3   g26(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  oai21  g27(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  inv1   g28(.a(x09), .O(new_n75_));
  aoi21  g29(.a(new_n66_), .b(new_n75_), .c(x16), .O(new_n76_));
  oai21  g30(.a(new_n66_), .b(x01), .c(new_n76_), .O(new_n77_));
  aoi21  g31(.a(new_n77_), .b(new_n74_), .c(new_n57_), .O(z10));
  inv1   g32(.a(x21), .O(new_n79_));
  nor2   g33(.a(new_n73_), .b(new_n79_), .O(new_n80_));
  nor2   g34(.a(x21), .b(x20), .O(new_n81_));
  nand2  g35(.a(new_n81_), .b(new_n64_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  oai21  g37(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  inv1   g38(.a(x02), .O(new_n85_));
  nand2  g39(.a(x18), .b(new_n85_), .O(new_n86_));
  inv1   g40(.a(x10), .O(new_n87_));
  aoi21  g41(.a(new_n66_), .b(new_n87_), .c(x16), .O(new_n88_));
  aoi21  g42(.a(new_n88_), .b(new_n86_), .c(new_n57_), .O(new_n89_));
  nand2  g43(.a(new_n89_), .b(new_n84_), .O(z11));
  inv1   g44(.a(x16), .O(new_n91_));
  nor2   g45(.a(x22), .b(x21), .O(new_n92_));
  aoi22  g46(.a(new_n92_), .b(new_n73_), .c(new_n82_), .d(x22), .O(new_n93_));
  inv1   g47(.a(x03), .O(new_n94_));
  nand2  g48(.a(x18), .b(new_n94_), .O(new_n95_));
  inv1   g49(.a(x11), .O(new_n96_));
  aoi21  g50(.a(new_n66_), .b(new_n96_), .c(x16), .O(new_n97_));
  aoi21  g51(.a(new_n97_), .b(new_n95_), .c(new_n57_), .O(new_n98_));
  oai21  g52(.a(new_n93_), .b(new_n91_), .c(new_n98_), .O(z12));
  inv1   g53(.a(x23), .O(new_n100_));
  nand3  g54(.a(new_n92_), .b(new_n64_), .c(new_n71_), .O(new_n101_));
  xor2a  g55(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(x16), .O(new_n103_));
  inv1   g57(.a(x12), .O(new_n104_));
  aoi21  g58(.a(new_n66_), .b(new_n104_), .c(x16), .O(new_n105_));
  oai21  g59(.a(new_n66_), .b(x04), .c(new_n105_), .O(new_n106_));
  aoi21  g60(.a(new_n106_), .b(new_n103_), .c(new_n57_), .O(z13));
  nand4  g61(.a(new_n92_), .b(new_n64_), .c(new_n100_), .d(new_n71_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(x24), .O(new_n109_));
  nor2   g63(.a(x24), .b(x23), .O(new_n110_));
  nand3  g64(.a(new_n110_), .b(new_n92_), .c(new_n73_), .O(new_n111_));
  nand2  g65(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g66(.a(new_n112_), .b(x16), .O(new_n113_));
  inv1   g67(.a(x05), .O(new_n114_));
  nand2  g68(.a(x18), .b(new_n114_), .O(new_n115_));
  inv1   g69(.a(x13), .O(new_n116_));
  aoi21  g70(.a(new_n66_), .b(new_n116_), .c(x16), .O(new_n117_));
  aoi21  g71(.a(new_n117_), .b(new_n115_), .c(new_n57_), .O(new_n118_));
  nand2  g72(.a(new_n118_), .b(new_n113_), .O(z14));
  nand2  g73(.a(new_n111_), .b(x25), .O(new_n120_));
  nor2   g74(.a(x25), .b(x22), .O(new_n121_));
  nand4  g75(.a(new_n121_), .b(new_n110_), .c(new_n81_), .d(new_n64_), .O(new_n122_));
  and2   g76(.a(new_n122_), .b(x16), .O(new_n123_));
  nand2  g77(.a(x18), .b(x06), .O(new_n124_));
  nand2  g78(.a(new_n66_), .b(x14), .O(new_n125_));
  nand3  g79(.a(new_n125_), .b(new_n124_), .c(new_n91_), .O(new_n126_));
  nand2  g80(.a(new_n126_), .b(new_n58_), .O(new_n127_));
  aoi21  g81(.a(new_n123_), .b(new_n120_), .c(new_n127_), .O(z15));
  nand2  g82(.a(new_n121_), .b(new_n110_), .O(new_n129_));
  inv1   g83(.a(new_n129_), .O(new_n130_));
  nand3  g84(.a(new_n130_), .b(new_n83_), .c(x26), .O(new_n131_));
  inv1   g85(.a(x26), .O(new_n132_));
  aoi21  g86(.a(new_n122_), .b(new_n132_), .c(new_n91_), .O(new_n133_));
  nand2  g87(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  inv1   g88(.a(x15), .O(new_n135_));
  aoi21  g89(.a(new_n66_), .b(new_n135_), .c(x16), .O(new_n136_));
  oai21  g90(.a(new_n66_), .b(x07), .c(new_n136_), .O(new_n137_));
  aoi21  g91(.a(new_n137_), .b(new_n134_), .c(new_n57_), .O(z16));
  inv1   g92(.a(x17), .O(new_n139_));
  nor2   g93(.a(x26), .b(x21), .O(new_n140_));
  nand4  g94(.a(new_n140_), .b(new_n130_), .c(new_n71_), .d(x19), .O(new_n141_));
  nor2   g95(.a(x17), .b(x14), .O(new_n142_));
  oai21  g96(.a(new_n142_), .b(x27), .c(x16), .O(new_n143_));
  aoi21  g97(.a(new_n141_), .b(new_n139_), .c(new_n143_), .O(z17));
endmodule


