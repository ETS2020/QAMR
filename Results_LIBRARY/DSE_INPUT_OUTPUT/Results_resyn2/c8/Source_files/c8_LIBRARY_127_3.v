// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n154_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x21), .O(z08));
  aoi22  g002(.a(z08), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g003(.a(z08), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  inv1   g005(.a(x21), .O(new_n52_));
  nor2   g006(.a(x27), .b(new_n52_), .O(new_n53_));
  inv1   g007(.a(x10), .O(new_n54_));
  nor2   g008(.a(x21), .b(new_n54_), .O(new_n55_));
  aoi21  g009(.a(new_n55_), .b(x27), .c(new_n53_), .O(z02));
  oai22  g010(.a(new_n50_), .b(x11), .c(x27), .d(x22), .O(z03));
  aoi22  g011(.a(z08), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  oai22  g012(.a(new_n50_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g013(.a(z08), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g014(.a(new_n50_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g015(.a(new_n47_), .b(new_n52_), .O(new_n62_));
  inv1   g016(.a(x18), .O(new_n63_));
  inv1   g017(.a(x08), .O(new_n64_));
  aoi21  g018(.a(new_n63_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g019(.a(new_n63_), .b(x00), .c(new_n65_), .O(new_n66_));
  inv1   g020(.a(x19), .O(new_n67_));
  nand2  g021(.a(new_n67_), .b(x17), .O(new_n68_));
  inv1   g022(.a(x16), .O(new_n69_));
  nor2   g023(.a(new_n67_), .b(x17), .O(new_n70_));
  nor2   g024(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g025(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n66_), .c(new_n62_), .O(z09));
  inv1   g027(.a(x20), .O(new_n74_));
  nor2   g028(.a(x19), .b(x17), .O(new_n75_));
  nor2   g029(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor3   g030(.a(x20), .b(x19), .c(x17), .O(new_n77_));
  oai21  g031(.a(new_n77_), .b(new_n76_), .c(x16), .O(new_n78_));
  inv1   g032(.a(x09), .O(new_n79_));
  aoi21  g033(.a(new_n63_), .b(new_n79_), .c(x16), .O(new_n80_));
  oai21  g034(.a(new_n63_), .b(x01), .c(new_n80_), .O(new_n81_));
  aoi21  g035(.a(new_n81_), .b(new_n78_), .c(new_n62_), .O(z10));
  inv1   g036(.a(new_n53_), .O(new_n83_));
  nand3  g037(.a(new_n75_), .b(new_n52_), .c(new_n74_), .O(new_n84_));
  oai21  g038(.a(new_n77_), .b(new_n83_), .c(new_n84_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(x16), .O(new_n86_));
  nor2   g040(.a(new_n62_), .b(x16), .O(new_n87_));
  nand2  g041(.a(new_n63_), .b(new_n54_), .O(new_n88_));
  inv1   g042(.a(x02), .O(new_n89_));
  nand2  g043(.a(x18), .b(new_n89_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n86_), .O(z11));
  inv1   g046(.a(x22), .O(new_n93_));
  nand2  g047(.a(new_n84_), .b(new_n93_), .O(new_n94_));
  nand3  g048(.a(new_n77_), .b(x22), .c(new_n52_), .O(new_n95_));
  nand3  g049(.a(new_n95_), .b(new_n94_), .c(x16), .O(new_n96_));
  inv1   g050(.a(x03), .O(new_n97_));
  nand2  g051(.a(x18), .b(new_n97_), .O(new_n98_));
  inv1   g052(.a(x11), .O(new_n99_));
  aoi21  g053(.a(new_n63_), .b(new_n99_), .c(x16), .O(new_n100_));
  aoi21  g054(.a(new_n100_), .b(new_n98_), .c(new_n62_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n96_), .O(z12));
  inv1   g056(.a(x23), .O(new_n103_));
  nor2   g057(.a(x22), .b(x20), .O(new_n104_));
  aoi21  g058(.a(new_n104_), .b(new_n75_), .c(new_n103_), .O(new_n105_));
  nand2  g059(.a(new_n103_), .b(new_n93_), .O(new_n106_));
  nor2   g060(.a(new_n106_), .b(new_n84_), .O(new_n107_));
  oai21  g061(.a(new_n107_), .b(new_n105_), .c(x16), .O(new_n108_));
  nand2  g062(.a(x23), .b(x16), .O(new_n109_));
  aoi21  g063(.a(new_n109_), .b(new_n47_), .c(new_n52_), .O(new_n110_));
  inv1   g064(.a(x04), .O(new_n111_));
  nand2  g065(.a(x18), .b(new_n111_), .O(new_n112_));
  inv1   g066(.a(x12), .O(new_n113_));
  aoi21  g067(.a(new_n63_), .b(new_n113_), .c(x16), .O(new_n114_));
  aoi21  g068(.a(new_n114_), .b(new_n112_), .c(new_n110_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n108_), .O(z13));
  oai21  g070(.a(new_n106_), .b(new_n84_), .c(x24), .O(new_n117_));
  nor2   g071(.a(new_n106_), .b(x24), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n77_), .c(new_n52_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x16), .O(new_n121_));
  inv1   g075(.a(x13), .O(new_n122_));
  aoi21  g076(.a(new_n63_), .b(new_n122_), .c(x16), .O(new_n123_));
  oai21  g077(.a(new_n63_), .b(x05), .c(new_n123_), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n121_), .c(new_n62_), .O(z14));
  nand2  g079(.a(new_n118_), .b(new_n77_), .O(new_n126_));
  nor2   g080(.a(x25), .b(x24), .O(new_n127_));
  aoi22  g081(.a(new_n127_), .b(new_n107_), .c(new_n126_), .d(x25), .O(new_n128_));
  nand2  g082(.a(x25), .b(x16), .O(new_n129_));
  aoi21  g083(.a(new_n129_), .b(new_n47_), .c(new_n52_), .O(new_n130_));
  inv1   g084(.a(x06), .O(new_n131_));
  nand2  g085(.a(x18), .b(new_n131_), .O(new_n132_));
  inv1   g086(.a(x14), .O(new_n133_));
  aoi21  g087(.a(new_n63_), .b(new_n133_), .c(x16), .O(new_n134_));
  aoi21  g088(.a(new_n134_), .b(new_n132_), .c(new_n130_), .O(new_n135_));
  oai21  g089(.a(new_n128_), .b(new_n69_), .c(new_n135_), .O(z15));
  nor2   g090(.a(x23), .b(x22), .O(new_n137_));
  nand4  g091(.a(new_n127_), .b(new_n137_), .c(new_n75_), .d(new_n74_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x26), .O(new_n139_));
  nor2   g093(.a(x26), .b(x23), .O(new_n140_));
  nand4  g094(.a(new_n140_), .b(new_n127_), .c(new_n104_), .d(new_n75_), .O(new_n141_));
  aoi21  g095(.a(new_n141_), .b(new_n139_), .c(x21), .O(new_n142_));
  nand2  g096(.a(new_n53_), .b(x26), .O(new_n143_));
  inv1   g097(.a(new_n143_), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n142_), .c(x16), .O(new_n145_));
  inv1   g099(.a(x15), .O(new_n146_));
  nand2  g100(.a(new_n63_), .b(new_n146_), .O(new_n147_));
  inv1   g101(.a(x07), .O(new_n148_));
  nand2  g102(.a(x18), .b(new_n148_), .O(new_n149_));
  nand3  g103(.a(new_n149_), .b(new_n147_), .c(new_n87_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n145_), .O(z16));
  nand2  g105(.a(x27), .b(x17), .O(new_n152_));
  nand4  g106(.a(new_n140_), .b(new_n127_), .c(new_n104_), .d(new_n70_), .O(new_n153_));
  nand2  g107(.a(new_n52_), .b(x16), .O(new_n154_));
  aoi21  g108(.a(new_n153_), .b(new_n152_), .c(new_n154_), .O(z17));
endmodule


