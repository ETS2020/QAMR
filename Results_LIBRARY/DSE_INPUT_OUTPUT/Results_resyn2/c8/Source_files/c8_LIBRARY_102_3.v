// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x05), .O(z08));
  inv1   g002(.a(z08), .O(new_n49_));
  oai22  g003(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  aoi22  g004(.a(z08), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  aoi22  g005(.a(z08), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  aoi22  g006(.a(z08), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  aoi22  g007(.a(z08), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  aoi22  g008(.a(z08), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  oai22  g009(.a(new_n49_), .b(x14), .c(x27), .d(x25), .O(z06));
  oai22  g010(.a(new_n49_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g011(.a(x05), .O(new_n58_));
  nor2   g012(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(new_n59_), .O(new_n60_));
  inv1   g014(.a(x16), .O(new_n61_));
  nand2  g015(.a(x18), .b(x00), .O(new_n62_));
  inv1   g016(.a(x18), .O(new_n63_));
  nand2  g017(.a(new_n63_), .b(x08), .O(new_n64_));
  nand3  g018(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  nand2  g019(.a(x19), .b(x17), .O(new_n66_));
  nor2   g020(.a(x19), .b(x17), .O(new_n67_));
  nor2   g021(.a(new_n67_), .b(new_n61_), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand2  g023(.a(new_n69_), .b(new_n65_), .O(new_n70_));
  nand2  g024(.a(new_n70_), .b(new_n60_), .O(z09));
  xor2a  g025(.a(new_n67_), .b(x20), .O(new_n72_));
  nand2  g026(.a(new_n72_), .b(x16), .O(new_n73_));
  inv1   g027(.a(x09), .O(new_n74_));
  aoi21  g028(.a(new_n63_), .b(new_n74_), .c(x16), .O(new_n75_));
  oai21  g029(.a(new_n63_), .b(x01), .c(new_n75_), .O(new_n76_));
  nand3  g030(.a(new_n76_), .b(new_n73_), .c(new_n60_), .O(z10));
  inv1   g031(.a(x20), .O(new_n78_));
  inv1   g032(.a(x21), .O(new_n79_));
  aoi21  g033(.a(new_n67_), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nor2   g034(.a(x21), .b(x20), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n67_), .O(new_n82_));
  inv1   g036(.a(new_n82_), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x10), .O(new_n85_));
  aoi21  g039(.a(new_n63_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g040(.a(new_n63_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g041(.a(new_n87_), .b(new_n84_), .c(new_n59_), .O(z11));
  xor2a  g042(.a(new_n82_), .b(x22), .O(new_n89_));
  inv1   g043(.a(x03), .O(new_n90_));
  nand2  g044(.a(x18), .b(new_n90_), .O(new_n91_));
  inv1   g045(.a(x11), .O(new_n92_));
  aoi21  g046(.a(new_n63_), .b(new_n92_), .c(x16), .O(new_n93_));
  aoi21  g047(.a(new_n93_), .b(new_n91_), .c(new_n59_), .O(new_n94_));
  oai21  g048(.a(new_n89_), .b(new_n61_), .c(new_n94_), .O(z12));
  inv1   g049(.a(x22), .O(new_n96_));
  nand3  g050(.a(new_n81_), .b(new_n67_), .c(new_n96_), .O(new_n97_));
  nor2   g051(.a(x23), .b(x22), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n81_), .c(new_n67_), .O(new_n99_));
  inv1   g053(.a(new_n99_), .O(new_n100_));
  aoi21  g054(.a(new_n97_), .b(x23), .c(new_n100_), .O(new_n101_));
  inv1   g055(.a(x04), .O(new_n102_));
  nand2  g056(.a(x18), .b(new_n102_), .O(new_n103_));
  inv1   g057(.a(x12), .O(new_n104_));
  aoi21  g058(.a(new_n63_), .b(new_n104_), .c(x16), .O(new_n105_));
  aoi21  g059(.a(new_n105_), .b(new_n103_), .c(new_n59_), .O(new_n106_));
  oai21  g060(.a(new_n101_), .b(new_n61_), .c(new_n106_), .O(z13));
  nand2  g061(.a(new_n99_), .b(x24), .O(new_n108_));
  inv1   g062(.a(x24), .O(new_n109_));
  nand4  g063(.a(new_n98_), .b(new_n81_), .c(new_n67_), .d(new_n109_), .O(new_n110_));
  and2   g064(.a(new_n110_), .b(x16), .O(new_n111_));
  aoi21  g065(.a(new_n63_), .b(x13), .c(x16), .O(new_n112_));
  oai21  g066(.a(new_n63_), .b(new_n58_), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n60_), .O(new_n114_));
  aoi21  g068(.a(new_n111_), .b(new_n108_), .c(new_n114_), .O(z14));
  nand2  g069(.a(new_n110_), .b(x25), .O(new_n116_));
  nor2   g070(.a(x25), .b(x24), .O(new_n117_));
  nand4  g071(.a(new_n117_), .b(new_n98_), .c(new_n81_), .d(new_n67_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x16), .O(new_n120_));
  inv1   g074(.a(x06), .O(new_n121_));
  nand2  g075(.a(x18), .b(new_n121_), .O(new_n122_));
  inv1   g076(.a(x14), .O(new_n123_));
  aoi21  g077(.a(new_n63_), .b(new_n123_), .c(x16), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n122_), .c(new_n59_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n120_), .O(z15));
  nand2  g080(.a(new_n118_), .b(x26), .O(new_n127_));
  nor3   g081(.a(x26), .b(x23), .c(x22), .O(new_n128_));
  nand4  g082(.a(new_n128_), .b(new_n117_), .c(new_n81_), .d(new_n67_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(x16), .O(new_n131_));
  inv1   g085(.a(x07), .O(new_n132_));
  nand2  g086(.a(x18), .b(new_n132_), .O(new_n133_));
  inv1   g087(.a(x15), .O(new_n134_));
  aoi21  g088(.a(new_n63_), .b(new_n134_), .c(x16), .O(new_n135_));
  aoi21  g089(.a(new_n135_), .b(new_n133_), .c(new_n59_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n131_), .O(z16));
  inv1   g091(.a(x26), .O(new_n138_));
  nand3  g092(.a(new_n117_), .b(new_n98_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(x27), .b(x17), .O(new_n140_));
  inv1   g094(.a(x17), .O(new_n141_));
  nand3  g095(.a(new_n81_), .b(x19), .c(new_n141_), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n139_), .c(new_n140_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n58_), .O(new_n144_));
  nor2   g098(.a(x27), .b(x26), .O(new_n145_));
  inv1   g099(.a(new_n142_), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n117_), .c(new_n98_), .d(new_n145_), .O(new_n147_));
  aoi21  g101(.a(new_n147_), .b(new_n144_), .c(new_n61_), .O(z17));
endmodule


