// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x23), .O(z08));
  aoi22  g002(.a(z08), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g003(.a(z08), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g005(.a(new_n50_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g006(.a(z08), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  aoi21  g007(.a(x27), .b(x12), .c(x23), .O(z04));
  oai22  g008(.a(new_n50_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g009(.a(z08), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  aoi22  g010(.a(z08), .b(x15), .c(new_n47_), .d(x26), .O(z07));
  inv1   g011(.a(x23), .O(new_n58_));
  nor2   g012(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x18), .O(new_n60_));
  inv1   g014(.a(x08), .O(new_n61_));
  aoi21  g015(.a(new_n60_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g016(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g017(.a(x17), .O(new_n64_));
  inv1   g018(.a(x16), .O(new_n65_));
  aoi21  g019(.a(x19), .b(new_n64_), .c(new_n65_), .O(new_n66_));
  oai21  g020(.a(x19), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n63_), .c(new_n59_), .O(z09));
  inv1   g022(.a(new_n59_), .O(new_n69_));
  nor2   g023(.a(x19), .b(x17), .O(new_n70_));
  xor2a  g024(.a(new_n70_), .b(x20), .O(new_n71_));
  nand2  g025(.a(new_n71_), .b(x16), .O(new_n72_));
  inv1   g026(.a(x09), .O(new_n73_));
  aoi21  g027(.a(new_n60_), .b(new_n73_), .c(x16), .O(new_n74_));
  oai21  g028(.a(new_n60_), .b(x01), .c(new_n74_), .O(new_n75_));
  nand3  g029(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z10));
  inv1   g030(.a(x20), .O(new_n77_));
  nand2  g031(.a(new_n70_), .b(new_n77_), .O(new_n78_));
  nor2   g032(.a(x21), .b(x20), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n70_), .O(new_n80_));
  inv1   g034(.a(new_n80_), .O(new_n81_));
  aoi21  g035(.a(new_n78_), .b(x21), .c(new_n81_), .O(new_n82_));
  inv1   g036(.a(x02), .O(new_n83_));
  nand2  g037(.a(x18), .b(new_n83_), .O(new_n84_));
  inv1   g038(.a(x10), .O(new_n85_));
  aoi21  g039(.a(new_n60_), .b(new_n85_), .c(x16), .O(new_n86_));
  aoi21  g040(.a(new_n86_), .b(new_n84_), .c(new_n59_), .O(new_n87_));
  oai21  g041(.a(new_n82_), .b(new_n65_), .c(new_n87_), .O(z11));
  xor2a  g042(.a(new_n80_), .b(x22), .O(new_n89_));
  inv1   g043(.a(x03), .O(new_n90_));
  nand2  g044(.a(x18), .b(new_n90_), .O(new_n91_));
  inv1   g045(.a(x11), .O(new_n92_));
  aoi21  g046(.a(new_n60_), .b(new_n92_), .c(x16), .O(new_n93_));
  aoi21  g047(.a(new_n93_), .b(new_n91_), .c(new_n59_), .O(new_n94_));
  oai21  g048(.a(new_n89_), .b(new_n65_), .c(new_n94_), .O(z12));
  nor2   g049(.a(x27), .b(new_n58_), .O(new_n96_));
  oai21  g050(.a(new_n80_), .b(x22), .c(new_n96_), .O(new_n97_));
  inv1   g051(.a(x22), .O(new_n98_));
  nand4  g052(.a(new_n79_), .b(new_n70_), .c(new_n58_), .d(new_n98_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(x16), .O(new_n101_));
  nor2   g055(.a(new_n59_), .b(x16), .O(new_n102_));
  inv1   g056(.a(x12), .O(new_n103_));
  nand2  g057(.a(new_n60_), .b(new_n103_), .O(new_n104_));
  inv1   g058(.a(x04), .O(new_n105_));
  nand2  g059(.a(x18), .b(new_n105_), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n104_), .c(new_n102_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n101_), .O(z13));
  nand2  g062(.a(new_n99_), .b(x24), .O(new_n109_));
  nor2   g063(.a(x24), .b(x22), .O(new_n110_));
  nand4  g064(.a(new_n110_), .b(new_n79_), .c(new_n70_), .d(new_n58_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(x16), .O(new_n113_));
  inv1   g067(.a(x13), .O(new_n114_));
  aoi21  g068(.a(new_n60_), .b(new_n114_), .c(x16), .O(new_n115_));
  oai21  g069(.a(new_n60_), .b(x05), .c(new_n115_), .O(new_n116_));
  aoi21  g070(.a(new_n116_), .b(new_n113_), .c(new_n59_), .O(z14));
  inv1   g071(.a(x25), .O(new_n118_));
  nand2  g072(.a(new_n111_), .b(new_n118_), .O(new_n119_));
  nand3  g073(.a(new_n110_), .b(new_n81_), .c(x25), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n119_), .c(x16), .O(new_n121_));
  nand2  g075(.a(x25), .b(x16), .O(new_n122_));
  aoi21  g076(.a(new_n122_), .b(new_n47_), .c(new_n58_), .O(new_n123_));
  inv1   g077(.a(x06), .O(new_n124_));
  nand2  g078(.a(x18), .b(new_n124_), .O(new_n125_));
  inv1   g079(.a(x14), .O(new_n126_));
  aoi21  g080(.a(new_n60_), .b(new_n126_), .c(x16), .O(new_n127_));
  aoi21  g081(.a(new_n127_), .b(new_n125_), .c(new_n123_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n121_), .O(z15));
  nor2   g083(.a(x26), .b(x25), .O(new_n130_));
  nand4  g084(.a(new_n130_), .b(new_n110_), .c(new_n79_), .d(new_n70_), .O(new_n131_));
  nor2   g085(.a(x25), .b(x21), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n110_), .O(new_n133_));
  oai21  g087(.a(new_n133_), .b(new_n78_), .c(x26), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n131_), .c(new_n58_), .O(new_n135_));
  nand2  g089(.a(new_n47_), .b(x26), .O(new_n136_));
  aoi21  g090(.a(new_n136_), .b(x23), .c(new_n65_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  inv1   g092(.a(x15), .O(new_n139_));
  nand2  g093(.a(new_n60_), .b(new_n139_), .O(new_n140_));
  inv1   g094(.a(x07), .O(new_n141_));
  nand2  g095(.a(x18), .b(new_n141_), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n140_), .c(new_n102_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n138_), .O(z16));
  nor2   g098(.a(x23), .b(x17), .O(new_n145_));
  inv1   g099(.a(x24), .O(new_n146_));
  inv1   g100(.a(x19), .O(new_n147_));
  nor2   g101(.a(x22), .b(new_n147_), .O(new_n148_));
  nand4  g102(.a(new_n148_), .b(new_n130_), .c(new_n79_), .d(new_n146_), .O(new_n149_));
  oai22  g103(.a(new_n145_), .b(x27), .c(new_n59_), .d(x16), .O(new_n150_));
  aoi21  g104(.a(new_n149_), .b(new_n145_), .c(new_n150_), .O(z17));
endmodule


