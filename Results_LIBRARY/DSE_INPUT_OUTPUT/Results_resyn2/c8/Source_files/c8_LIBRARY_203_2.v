// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n50_, new_n54_, new_n55_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  nor2   g000(.a(x27), .b(x23), .O(new_n47_));
  aoi22  g001(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  aoi22  g002(.a(new_n47_), .b(x20), .c(x27), .d(x09), .O(z01));
  inv1   g003(.a(x27), .O(new_n50_));
  inv1   g004(.a(new_n47_), .O(z08));
  oai22  g005(.a(z08), .b(x21), .c(new_n50_), .d(x10), .O(z02));
  aoi22  g006(.a(new_n47_), .b(x22), .c(x27), .d(x11), .O(z03));
  inv1   g007(.a(x23), .O(new_n54_));
  nor2   g008(.a(x27), .b(new_n54_), .O(new_n55_));
  aoi21  g009(.a(x27), .b(x12), .c(new_n55_), .O(z04));
  oai22  g010(.a(z08), .b(x24), .c(new_n50_), .d(x13), .O(z05));
  oai22  g011(.a(z08), .b(x25), .c(new_n50_), .d(x14), .O(z06));
  aoi22  g012(.a(new_n47_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g013(.a(x16), .O(new_n60_));
  nand2  g014(.a(x18), .b(x00), .O(new_n61_));
  inv1   g015(.a(x18), .O(new_n62_));
  nand2  g016(.a(new_n62_), .b(x08), .O(new_n63_));
  nand3  g017(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(new_n64_));
  nand2  g018(.a(x19), .b(x17), .O(new_n65_));
  nor2   g019(.a(x19), .b(x17), .O(new_n66_));
  nor2   g020(.a(new_n66_), .b(new_n60_), .O(new_n67_));
  nand2  g021(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(new_n64_), .O(new_n69_));
  oai21  g023(.a(x27), .b(new_n54_), .c(new_n69_), .O(z09));
  xor2a  g024(.a(new_n66_), .b(x20), .O(new_n71_));
  nand2  g025(.a(new_n71_), .b(x16), .O(new_n72_));
  inv1   g026(.a(x09), .O(new_n73_));
  aoi21  g027(.a(new_n62_), .b(new_n73_), .c(x16), .O(new_n74_));
  oai21  g028(.a(new_n62_), .b(x01), .c(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n72_), .c(new_n55_), .O(z10));
  inv1   g030(.a(x20), .O(new_n77_));
  nand2  g031(.a(new_n66_), .b(new_n77_), .O(new_n78_));
  nor2   g032(.a(x21), .b(x20), .O(new_n79_));
  aoi22  g033(.a(new_n79_), .b(new_n66_), .c(new_n78_), .d(x21), .O(new_n80_));
  inv1   g034(.a(x02), .O(new_n81_));
  nand2  g035(.a(x18), .b(new_n81_), .O(new_n82_));
  inv1   g036(.a(x10), .O(new_n83_));
  aoi21  g037(.a(new_n62_), .b(new_n83_), .c(x16), .O(new_n84_));
  aoi21  g038(.a(new_n84_), .b(new_n82_), .c(new_n55_), .O(new_n85_));
  oai21  g039(.a(new_n80_), .b(new_n60_), .c(new_n85_), .O(z11));
  inv1   g040(.a(x22), .O(new_n87_));
  aoi21  g041(.a(new_n79_), .b(new_n66_), .c(new_n87_), .O(new_n88_));
  nand3  g042(.a(new_n79_), .b(new_n66_), .c(new_n87_), .O(new_n89_));
  inv1   g043(.a(new_n89_), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(new_n88_), .c(x16), .O(new_n91_));
  inv1   g045(.a(x03), .O(new_n92_));
  nand2  g046(.a(x18), .b(new_n92_), .O(new_n93_));
  inv1   g047(.a(x11), .O(new_n94_));
  aoi21  g048(.a(new_n62_), .b(new_n94_), .c(x16), .O(new_n95_));
  aoi21  g049(.a(new_n95_), .b(new_n93_), .c(new_n55_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n91_), .O(z12));
  nor2   g051(.a(x23), .b(x22), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n79_), .c(new_n66_), .O(new_n99_));
  inv1   g053(.a(new_n99_), .O(new_n100_));
  aoi21  g054(.a(new_n89_), .b(x23), .c(new_n100_), .O(new_n101_));
  inv1   g055(.a(x04), .O(new_n102_));
  nand2  g056(.a(x18), .b(new_n102_), .O(new_n103_));
  inv1   g057(.a(x12), .O(new_n104_));
  aoi21  g058(.a(new_n62_), .b(new_n104_), .c(x16), .O(new_n105_));
  aoi21  g059(.a(new_n105_), .b(new_n103_), .c(new_n55_), .O(new_n106_));
  oai21  g060(.a(new_n101_), .b(new_n60_), .c(new_n106_), .O(z13));
  nand4  g061(.a(new_n98_), .b(new_n79_), .c(new_n66_), .d(x24), .O(new_n108_));
  inv1   g062(.a(x24), .O(new_n109_));
  nand2  g063(.a(new_n99_), .b(new_n109_), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n108_), .c(x16), .O(new_n111_));
  inv1   g065(.a(x13), .O(new_n112_));
  aoi21  g066(.a(new_n62_), .b(new_n112_), .c(x16), .O(new_n113_));
  oai21  g067(.a(new_n62_), .b(x05), .c(new_n113_), .O(new_n114_));
  aoi21  g068(.a(new_n114_), .b(new_n111_), .c(new_n55_), .O(z14));
  nand4  g069(.a(new_n79_), .b(new_n66_), .c(new_n109_), .d(new_n87_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x25), .O(new_n117_));
  nor2   g071(.a(x25), .b(x24), .O(new_n118_));
  nand4  g072(.a(new_n118_), .b(new_n98_), .c(new_n79_), .d(new_n66_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x16), .O(new_n121_));
  nand2  g075(.a(x25), .b(x16), .O(new_n122_));
  aoi21  g076(.a(new_n122_), .b(x27), .c(new_n54_), .O(new_n123_));
  inv1   g077(.a(x06), .O(new_n124_));
  nand2  g078(.a(x18), .b(new_n124_), .O(new_n125_));
  inv1   g079(.a(x14), .O(new_n126_));
  aoi21  g080(.a(new_n62_), .b(new_n126_), .c(x16), .O(new_n127_));
  aoi21  g081(.a(new_n127_), .b(new_n125_), .c(new_n123_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n121_), .O(z15));
  inv1   g083(.a(x26), .O(new_n130_));
  and2   g084(.a(new_n119_), .b(new_n130_), .O(new_n131_));
  nand2  g085(.a(new_n118_), .b(x26), .O(new_n132_));
  oai21  g086(.a(new_n132_), .b(new_n89_), .c(x16), .O(new_n133_));
  nand2  g087(.a(x26), .b(x16), .O(new_n134_));
  aoi21  g088(.a(new_n134_), .b(x27), .c(new_n54_), .O(new_n135_));
  inv1   g089(.a(x07), .O(new_n136_));
  nand2  g090(.a(x18), .b(new_n136_), .O(new_n137_));
  inv1   g091(.a(x15), .O(new_n138_));
  aoi21  g092(.a(new_n62_), .b(new_n138_), .c(x16), .O(new_n139_));
  aoi21  g093(.a(new_n139_), .b(new_n137_), .c(new_n135_), .O(new_n140_));
  oai21  g094(.a(new_n133_), .b(new_n131_), .c(new_n140_), .O(z16));
  nand2  g095(.a(x27), .b(x17), .O(new_n142_));
  inv1   g096(.a(x17), .O(new_n143_));
  nand3  g097(.a(new_n130_), .b(x19), .c(new_n143_), .O(new_n144_));
  inv1   g098(.a(new_n144_), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n118_), .c(new_n98_), .d(new_n79_), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n142_), .c(new_n60_), .O(z17));
endmodule


