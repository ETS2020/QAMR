// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  nand2  g00(.a(x27), .b(x22), .O(new_n47_));
  oai22  g01(.a(new_n47_), .b(x08), .c(x27), .d(x19), .O(z00));
  inv1   g02(.a(x27), .O(new_n49_));
  inv1   g03(.a(new_n47_), .O(z08));
  aoi22  g04(.a(z08), .b(x09), .c(new_n49_), .d(x20), .O(z01));
  aoi22  g05(.a(z08), .b(x10), .c(new_n49_), .d(x21), .O(z02));
  oai21  g06(.a(new_n49_), .b(x11), .c(x22), .O(z03));
  oai22  g07(.a(new_n47_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g08(.a(new_n47_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g09(.a(z08), .b(x14), .c(new_n49_), .d(x25), .O(z06));
  aoi22  g10(.a(z08), .b(x15), .c(new_n49_), .d(x26), .O(z07));
  inv1   g11(.a(x18), .O(new_n58_));
  inv1   g12(.a(x08), .O(new_n59_));
  aoi21  g13(.a(new_n58_), .b(new_n59_), .c(x16), .O(new_n60_));
  oai21  g14(.a(new_n58_), .b(x00), .c(new_n60_), .O(new_n61_));
  and2   g15(.a(x19), .b(x17), .O(new_n62_));
  nor2   g16(.a(x19), .b(x17), .O(new_n63_));
  oai21  g17(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  nor2   g18(.a(new_n49_), .b(x22), .O(new_n65_));
  inv1   g19(.a(new_n65_), .O(new_n66_));
  nand3  g20(.a(new_n66_), .b(new_n64_), .c(new_n61_), .O(z09));
  inv1   g21(.a(x16), .O(new_n68_));
  inv1   g22(.a(x20), .O(new_n69_));
  xor2a  g23(.a(new_n63_), .b(new_n69_), .O(new_n70_));
  inv1   g24(.a(x01), .O(new_n71_));
  nand2  g25(.a(x18), .b(new_n71_), .O(new_n72_));
  inv1   g26(.a(x09), .O(new_n73_));
  aoi21  g27(.a(new_n58_), .b(new_n73_), .c(x16), .O(new_n74_));
  aoi21  g28(.a(new_n74_), .b(new_n72_), .c(new_n65_), .O(new_n75_));
  oai21  g29(.a(new_n70_), .b(new_n68_), .c(new_n75_), .O(z10));
  nand2  g30(.a(new_n63_), .b(new_n69_), .O(new_n77_));
  nor2   g31(.a(x21), .b(x20), .O(new_n78_));
  aoi22  g32(.a(new_n78_), .b(new_n63_), .c(new_n77_), .d(x21), .O(new_n79_));
  inv1   g33(.a(x02), .O(new_n80_));
  nand2  g34(.a(x18), .b(new_n80_), .O(new_n81_));
  inv1   g35(.a(x10), .O(new_n82_));
  aoi21  g36(.a(new_n58_), .b(new_n82_), .c(x16), .O(new_n83_));
  aoi21  g37(.a(new_n83_), .b(new_n81_), .c(new_n65_), .O(new_n84_));
  oai21  g38(.a(new_n79_), .b(new_n68_), .c(new_n84_), .O(z11));
  inv1   g39(.a(x22), .O(new_n86_));
  aoi21  g40(.a(new_n78_), .b(new_n63_), .c(new_n86_), .O(new_n87_));
  nand3  g41(.a(new_n78_), .b(new_n63_), .c(new_n86_), .O(new_n88_));
  inv1   g42(.a(new_n88_), .O(new_n89_));
  oai21  g43(.a(new_n89_), .b(new_n87_), .c(x16), .O(new_n90_));
  inv1   g44(.a(x03), .O(new_n91_));
  nand2  g45(.a(x18), .b(new_n91_), .O(new_n92_));
  inv1   g46(.a(x11), .O(new_n93_));
  aoi21  g47(.a(new_n58_), .b(new_n93_), .c(x16), .O(new_n94_));
  aoi21  g48(.a(new_n94_), .b(new_n92_), .c(new_n65_), .O(new_n95_));
  nand2  g49(.a(new_n95_), .b(new_n90_), .O(z12));
  inv1   g50(.a(x23), .O(new_n97_));
  xor2a  g51(.a(new_n88_), .b(new_n97_), .O(new_n98_));
  nand2  g52(.a(new_n98_), .b(x16), .O(new_n99_));
  inv1   g53(.a(x12), .O(new_n100_));
  aoi21  g54(.a(new_n58_), .b(new_n100_), .c(x16), .O(new_n101_));
  oai21  g55(.a(new_n58_), .b(x04), .c(new_n101_), .O(new_n102_));
  aoi21  g56(.a(new_n102_), .b(new_n99_), .c(new_n65_), .O(z13));
  nand4  g57(.a(new_n78_), .b(new_n63_), .c(new_n97_), .d(x16), .O(new_n104_));
  oai21  g58(.a(new_n104_), .b(x24), .c(new_n49_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(new_n86_), .O(new_n106_));
  nor2   g60(.a(x27), .b(x23), .O(new_n107_));
  nand4  g61(.a(new_n78_), .b(new_n63_), .c(new_n107_), .d(new_n86_), .O(new_n108_));
  aoi21  g62(.a(new_n108_), .b(x24), .c(new_n68_), .O(new_n109_));
  inv1   g63(.a(x13), .O(new_n110_));
  oai21  g64(.a(x18), .b(new_n110_), .c(new_n68_), .O(new_n111_));
  aoi21  g65(.a(x18), .b(x05), .c(new_n111_), .O(new_n112_));
  oai21  g66(.a(new_n112_), .b(new_n109_), .c(new_n106_), .O(z14));
  inv1   g67(.a(x25), .O(new_n114_));
  nor2   g68(.a(new_n114_), .b(new_n68_), .O(new_n115_));
  oai21  g69(.a(new_n108_), .b(x24), .c(new_n115_), .O(new_n116_));
  inv1   g70(.a(x24), .O(new_n117_));
  nand2  g71(.a(new_n114_), .b(new_n117_), .O(new_n118_));
  oai21  g72(.a(new_n118_), .b(new_n104_), .c(new_n49_), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(new_n86_), .O(new_n120_));
  inv1   g74(.a(x14), .O(new_n121_));
  aoi21  g75(.a(new_n58_), .b(new_n121_), .c(x16), .O(new_n122_));
  oai21  g76(.a(new_n58_), .b(x06), .c(new_n122_), .O(new_n123_));
  nand3  g77(.a(new_n123_), .b(new_n120_), .c(new_n116_), .O(z15));
  inv1   g78(.a(new_n108_), .O(new_n125_));
  nand3  g79(.a(new_n125_), .b(new_n114_), .c(new_n117_), .O(new_n126_));
  nand3  g80(.a(new_n126_), .b(x26), .c(x16), .O(new_n127_));
  inv1   g81(.a(x26), .O(new_n128_));
  nand3  g82(.a(new_n128_), .b(new_n114_), .c(new_n117_), .O(new_n129_));
  oai21  g83(.a(new_n129_), .b(new_n104_), .c(new_n49_), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(new_n86_), .O(new_n131_));
  inv1   g85(.a(x15), .O(new_n132_));
  aoi21  g86(.a(new_n58_), .b(new_n132_), .c(x16), .O(new_n133_));
  oai21  g87(.a(new_n58_), .b(x07), .c(new_n133_), .O(new_n134_));
  nand3  g88(.a(new_n134_), .b(new_n131_), .c(new_n127_), .O(z16));
  inv1   g89(.a(x17), .O(new_n136_));
  nor2   g90(.a(x23), .b(x22), .O(new_n137_));
  nand4  g91(.a(new_n137_), .b(new_n78_), .c(x19), .d(new_n136_), .O(new_n138_));
  oai22  g92(.a(new_n138_), .b(new_n129_), .c(new_n49_), .d(new_n136_), .O(new_n139_));
  nand2  g93(.a(new_n139_), .b(x16), .O(new_n140_));
  nand2  g94(.a(new_n140_), .b(new_n66_), .O(z17));
endmodule


