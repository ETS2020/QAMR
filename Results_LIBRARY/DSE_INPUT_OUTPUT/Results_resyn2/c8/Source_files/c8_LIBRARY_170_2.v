// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_;
  nor2   g00(.a(x27), .b(x09), .O(new_n47_));
  aoi22  g01(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g02(.a(x27), .O(new_n49_));
  aoi21  g03(.a(new_n49_), .b(x20), .c(x09), .O(z01));
  inv1   g04(.a(new_n47_), .O(z08));
  oai22  g05(.a(z08), .b(x21), .c(new_n49_), .d(x10), .O(z02));
  aoi22  g06(.a(new_n47_), .b(x22), .c(x27), .d(x11), .O(z03));
  aoi22  g07(.a(new_n47_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi22  g08(.a(new_n47_), .b(x24), .c(x27), .d(x13), .O(z05));
  aoi22  g09(.a(new_n47_), .b(x25), .c(x27), .d(x14), .O(z06));
  oai22  g10(.a(z08), .b(x26), .c(new_n49_), .d(x15), .O(z07));
  inv1   g11(.a(x09), .O(new_n58_));
  nor2   g12(.a(x27), .b(new_n58_), .O(new_n59_));
  and2   g13(.a(x19), .b(x17), .O(new_n60_));
  nor2   g14(.a(x19), .b(x17), .O(new_n61_));
  oai21  g15(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  nor2   g16(.a(x18), .b(x16), .O(new_n63_));
  inv1   g17(.a(x18), .O(new_n64_));
  nor2   g18(.a(new_n64_), .b(x16), .O(new_n65_));
  aoi22  g19(.a(new_n65_), .b(x00), .c(new_n63_), .d(x08), .O(new_n66_));
  aoi21  g20(.a(new_n66_), .b(new_n62_), .c(new_n59_), .O(z09));
  inv1   g21(.a(x20), .O(new_n68_));
  nor2   g22(.a(new_n61_), .b(new_n68_), .O(new_n69_));
  nor3   g23(.a(x20), .b(x19), .c(x17), .O(new_n70_));
  oai21  g24(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  aoi22  g25(.a(new_n65_), .b(x01), .c(new_n63_), .d(x09), .O(new_n72_));
  aoi21  g26(.a(new_n72_), .b(new_n71_), .c(new_n59_), .O(z10));
  inv1   g27(.a(x21), .O(new_n74_));
  nor2   g28(.a(x21), .b(x20), .O(new_n75_));
  nand2  g29(.a(new_n75_), .b(new_n61_), .O(new_n76_));
  oai21  g30(.a(new_n70_), .b(new_n74_), .c(new_n76_), .O(new_n77_));
  nand2  g31(.a(new_n77_), .b(x16), .O(new_n78_));
  inv1   g32(.a(x10), .O(new_n79_));
  aoi21  g33(.a(new_n64_), .b(new_n79_), .c(x16), .O(new_n80_));
  oai21  g34(.a(new_n64_), .b(x02), .c(new_n80_), .O(new_n81_));
  aoi21  g35(.a(new_n81_), .b(new_n78_), .c(new_n59_), .O(z11));
  inv1   g36(.a(new_n59_), .O(new_n83_));
  inv1   g37(.a(x16), .O(new_n84_));
  nand2  g38(.a(x18), .b(x03), .O(new_n85_));
  nand2  g39(.a(new_n64_), .b(x11), .O(new_n86_));
  nand3  g40(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(new_n87_));
  nor2   g41(.a(x22), .b(x21), .O(new_n88_));
  nand2  g42(.a(new_n88_), .b(new_n70_), .O(new_n89_));
  nand2  g43(.a(new_n76_), .b(x22), .O(new_n90_));
  nand3  g44(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  nand2  g45(.a(new_n91_), .b(new_n87_), .O(new_n92_));
  nand2  g46(.a(new_n92_), .b(new_n83_), .O(z12));
  xor2a  g47(.a(new_n89_), .b(x23), .O(new_n94_));
  inv1   g48(.a(x04), .O(new_n95_));
  nand2  g49(.a(x18), .b(new_n95_), .O(new_n96_));
  inv1   g50(.a(x12), .O(new_n97_));
  aoi21  g51(.a(new_n64_), .b(new_n97_), .c(x16), .O(new_n98_));
  aoi21  g52(.a(new_n98_), .b(new_n96_), .c(new_n59_), .O(new_n99_));
  oai21  g53(.a(new_n94_), .b(new_n84_), .c(new_n99_), .O(z13));
  inv1   g54(.a(x23), .O(new_n101_));
  nand3  g55(.a(new_n88_), .b(new_n70_), .c(new_n101_), .O(new_n102_));
  nand2  g56(.a(new_n61_), .b(new_n68_), .O(new_n103_));
  nor2   g57(.a(x24), .b(x23), .O(new_n104_));
  nand2  g58(.a(new_n104_), .b(new_n88_), .O(new_n105_));
  nor2   g59(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  aoi21  g60(.a(new_n102_), .b(x24), .c(new_n106_), .O(new_n107_));
  inv1   g61(.a(x05), .O(new_n108_));
  nand2  g62(.a(x18), .b(new_n108_), .O(new_n109_));
  inv1   g63(.a(x13), .O(new_n110_));
  aoi21  g64(.a(new_n64_), .b(new_n110_), .c(x16), .O(new_n111_));
  aoi21  g65(.a(new_n111_), .b(new_n109_), .c(new_n59_), .O(new_n112_));
  oai21  g66(.a(new_n107_), .b(new_n84_), .c(new_n112_), .O(z14));
  oai21  g67(.a(new_n105_), .b(new_n103_), .c(x25), .O(new_n114_));
  nor2   g68(.a(x25), .b(x22), .O(new_n115_));
  nand4  g69(.a(new_n115_), .b(new_n104_), .c(new_n75_), .d(new_n61_), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand2  g71(.a(new_n117_), .b(x16), .O(new_n118_));
  inv1   g72(.a(x06), .O(new_n119_));
  nand2  g73(.a(x18), .b(new_n119_), .O(new_n120_));
  inv1   g74(.a(x14), .O(new_n121_));
  aoi21  g75(.a(new_n64_), .b(new_n121_), .c(x16), .O(new_n122_));
  aoi21  g76(.a(new_n122_), .b(new_n120_), .c(new_n59_), .O(new_n123_));
  nand2  g77(.a(new_n123_), .b(new_n118_), .O(z15));
  inv1   g78(.a(new_n76_), .O(new_n125_));
  nand2  g79(.a(new_n115_), .b(new_n104_), .O(new_n126_));
  inv1   g80(.a(new_n126_), .O(new_n127_));
  nand3  g81(.a(new_n127_), .b(new_n125_), .c(x26), .O(new_n128_));
  inv1   g82(.a(x26), .O(new_n129_));
  aoi21  g83(.a(new_n116_), .b(new_n129_), .c(new_n84_), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  inv1   g85(.a(x15), .O(new_n132_));
  aoi21  g86(.a(new_n64_), .b(new_n132_), .c(x16), .O(new_n133_));
  oai21  g87(.a(new_n64_), .b(x07), .c(new_n133_), .O(new_n134_));
  aoi21  g88(.a(new_n134_), .b(new_n131_), .c(new_n59_), .O(z16));
  inv1   g89(.a(x17), .O(new_n136_));
  inv1   g90(.a(new_n105_), .O(new_n137_));
  nor2   g91(.a(x26), .b(x25), .O(new_n138_));
  nand4  g92(.a(new_n138_), .b(new_n137_), .c(new_n68_), .d(x19), .O(new_n139_));
  nor2   g93(.a(x17), .b(x09), .O(new_n140_));
  oai21  g94(.a(new_n140_), .b(x27), .c(x16), .O(new_n141_));
  aoi21  g95(.a(new_n139_), .b(new_n136_), .c(new_n141_), .O(z17));
endmodule


