// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x25), .O(new_n48_));
  nor2   g02(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g03(.a(new_n49_), .O(new_n50_));
  oai22  g04(.a(new_n50_), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g05(.a(new_n50_), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  oai22  g06(.a(new_n50_), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  oai22  g07(.a(new_n50_), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  oai22  g08(.a(new_n50_), .b(x23), .c(new_n47_), .d(x12), .O(z04));
  aoi22  g09(.a(new_n49_), .b(x24), .c(x27), .d(x13), .O(z05));
  nor2   g10(.a(new_n47_), .b(x14), .O(z06));
  aoi22  g11(.a(new_n49_), .b(x26), .c(x27), .d(x15), .O(z07));
  nor2   g12(.a(x27), .b(x25), .O(new_n59_));
  and2   g13(.a(x19), .b(x17), .O(new_n60_));
  nor2   g14(.a(x19), .b(x17), .O(new_n61_));
  oai21  g15(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g16(.a(x18), .O(new_n63_));
  inv1   g17(.a(x08), .O(new_n64_));
  aoi21  g18(.a(new_n63_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g19(.a(new_n63_), .b(x00), .c(new_n65_), .O(new_n66_));
  aoi21  g20(.a(new_n66_), .b(new_n62_), .c(new_n59_), .O(z09));
  inv1   g21(.a(x20), .O(new_n68_));
  nor2   g22(.a(new_n61_), .b(new_n68_), .O(new_n69_));
  nor3   g23(.a(x20), .b(x19), .c(x17), .O(new_n70_));
  oai21  g24(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  inv1   g25(.a(x09), .O(new_n72_));
  aoi21  g26(.a(new_n63_), .b(new_n72_), .c(x16), .O(new_n73_));
  oai21  g27(.a(new_n63_), .b(x01), .c(new_n73_), .O(new_n74_));
  aoi21  g28(.a(new_n74_), .b(new_n71_), .c(new_n59_), .O(z10));
  inv1   g29(.a(x21), .O(new_n76_));
  nor2   g30(.a(new_n70_), .b(new_n76_), .O(new_n77_));
  nand3  g31(.a(new_n61_), .b(new_n76_), .c(new_n68_), .O(new_n78_));
  inv1   g32(.a(new_n78_), .O(new_n79_));
  oai21  g33(.a(new_n79_), .b(new_n77_), .c(x16), .O(new_n80_));
  inv1   g34(.a(x10), .O(new_n81_));
  aoi21  g35(.a(new_n63_), .b(new_n81_), .c(x16), .O(new_n82_));
  oai21  g36(.a(new_n63_), .b(x02), .c(new_n82_), .O(new_n83_));
  aoi21  g37(.a(new_n83_), .b(new_n80_), .c(new_n59_), .O(z11));
  inv1   g38(.a(new_n59_), .O(new_n85_));
  inv1   g39(.a(x11), .O(new_n86_));
  inv1   g40(.a(x16), .O(new_n87_));
  oai21  g41(.a(x18), .b(new_n86_), .c(new_n87_), .O(new_n88_));
  aoi21  g42(.a(x18), .b(x03), .c(new_n88_), .O(new_n89_));
  nor2   g43(.a(x22), .b(x21), .O(new_n90_));
  nand3  g44(.a(new_n90_), .b(new_n61_), .c(new_n68_), .O(new_n91_));
  nand2  g45(.a(new_n91_), .b(x16), .O(new_n92_));
  aoi21  g46(.a(new_n78_), .b(x22), .c(new_n92_), .O(new_n93_));
  oai21  g47(.a(new_n93_), .b(new_n89_), .c(new_n85_), .O(z12));
  inv1   g48(.a(x23), .O(new_n95_));
  xor2a  g49(.a(new_n91_), .b(new_n95_), .O(new_n96_));
  nand2  g50(.a(new_n96_), .b(x16), .O(new_n97_));
  inv1   g51(.a(x12), .O(new_n98_));
  aoi21  g52(.a(new_n63_), .b(new_n98_), .c(x16), .O(new_n99_));
  oai21  g53(.a(new_n63_), .b(x04), .c(new_n99_), .O(new_n100_));
  aoi21  g54(.a(new_n100_), .b(new_n97_), .c(new_n59_), .O(z13));
  oai21  g55(.a(new_n91_), .b(x23), .c(x24), .O(new_n102_));
  nor2   g56(.a(x24), .b(x23), .O(new_n103_));
  nand4  g57(.a(new_n103_), .b(new_n90_), .c(new_n61_), .d(new_n68_), .O(new_n104_));
  inv1   g58(.a(new_n104_), .O(new_n105_));
  nor2   g59(.a(new_n105_), .b(new_n87_), .O(new_n106_));
  nand2  g60(.a(x18), .b(x05), .O(new_n107_));
  nand2  g61(.a(new_n63_), .b(x13), .O(new_n108_));
  nand3  g62(.a(new_n108_), .b(new_n107_), .c(new_n87_), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(new_n85_), .O(new_n110_));
  aoi21  g64(.a(new_n106_), .b(new_n102_), .c(new_n110_), .O(z14));
  oai21  g65(.a(new_n104_), .b(new_n87_), .c(x27), .O(new_n112_));
  nand2  g66(.a(new_n112_), .b(new_n48_), .O(new_n113_));
  nand3  g67(.a(new_n104_), .b(x25), .c(x16), .O(new_n114_));
  inv1   g68(.a(x14), .O(new_n115_));
  aoi21  g69(.a(new_n63_), .b(new_n115_), .c(x16), .O(new_n116_));
  oai21  g70(.a(new_n63_), .b(x06), .c(new_n116_), .O(new_n117_));
  nand3  g71(.a(new_n117_), .b(new_n114_), .c(new_n113_), .O(z15));
  oai21  g72(.a(new_n104_), .b(x25), .c(x26), .O(new_n119_));
  nor2   g73(.a(x26), .b(x25), .O(new_n120_));
  aoi21  g74(.a(new_n120_), .b(new_n105_), .c(new_n87_), .O(new_n121_));
  nand2  g75(.a(x18), .b(x07), .O(new_n122_));
  nand2  g76(.a(new_n63_), .b(x15), .O(new_n123_));
  nand3  g77(.a(new_n123_), .b(new_n122_), .c(new_n87_), .O(new_n124_));
  nand2  g78(.a(new_n124_), .b(new_n85_), .O(new_n125_));
  aoi21  g79(.a(new_n121_), .b(new_n119_), .c(new_n125_), .O(z16));
  nand2  g80(.a(new_n103_), .b(new_n90_), .O(new_n127_));
  nand2  g81(.a(x27), .b(x17), .O(new_n128_));
  nand3  g82(.a(new_n120_), .b(new_n68_), .c(x19), .O(new_n129_));
  oai21  g83(.a(new_n129_), .b(new_n127_), .c(new_n128_), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(x16), .O(new_n131_));
  nand2  g85(.a(new_n131_), .b(new_n85_), .O(z17));
  buf    g86(.a(x27), .O(z08));
endmodule


