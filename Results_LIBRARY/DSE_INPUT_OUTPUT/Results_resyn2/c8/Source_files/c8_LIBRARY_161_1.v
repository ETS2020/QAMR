// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n55_, new_n56_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_;
  inv1   g000(.a(x23), .O(new_n47_));
  inv1   g001(.a(x27), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(new_n49_), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x08), .c(x27), .d(x19), .O(z00));
  oai22  g005(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g006(.a(new_n50_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g007(.a(new_n49_), .b(x11), .c(new_n48_), .d(x22), .O(z03));
  nor2   g008(.a(x27), .b(x23), .O(new_n55_));
  inv1   g009(.a(new_n55_), .O(new_n56_));
  oai21  g010(.a(new_n50_), .b(x12), .c(new_n56_), .O(z04));
  aoi22  g011(.a(new_n49_), .b(x13), .c(new_n48_), .d(x24), .O(z05));
  aoi22  g012(.a(new_n49_), .b(x14), .c(new_n48_), .d(x25), .O(z06));
  oai22  g013(.a(new_n50_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g014(.a(new_n48_), .b(x23), .O(new_n61_));
  inv1   g015(.a(new_n61_), .O(new_n62_));
  inv1   g016(.a(x08), .O(new_n63_));
  inv1   g017(.a(x16), .O(new_n64_));
  oai21  g018(.a(x18), .b(new_n63_), .c(new_n64_), .O(new_n65_));
  aoi21  g019(.a(x18), .b(x00), .c(new_n65_), .O(new_n66_));
  inv1   g020(.a(x17), .O(new_n67_));
  inv1   g021(.a(x19), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g023(.a(new_n69_), .b(x16), .O(new_n70_));
  aoi21  g024(.a(x19), .b(x17), .c(new_n70_), .O(new_n71_));
  oai21  g025(.a(new_n71_), .b(new_n66_), .c(new_n62_), .O(z09));
  inv1   g026(.a(x20), .O(new_n73_));
  nor2   g027(.a(x19), .b(x17), .O(new_n74_));
  nor2   g028(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor2   g029(.a(new_n69_), .b(x20), .O(new_n76_));
  oai21  g030(.a(new_n76_), .b(new_n75_), .c(x16), .O(new_n77_));
  inv1   g031(.a(x01), .O(new_n78_));
  nand2  g032(.a(x18), .b(new_n78_), .O(new_n79_));
  inv1   g033(.a(x09), .O(new_n80_));
  inv1   g034(.a(x18), .O(new_n81_));
  aoi21  g035(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  aoi21  g036(.a(new_n82_), .b(new_n79_), .c(new_n61_), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n77_), .O(z10));
  inv1   g038(.a(x21), .O(new_n85_));
  nor2   g039(.a(new_n76_), .b(new_n85_), .O(new_n86_));
  nand3  g040(.a(new_n74_), .b(new_n85_), .c(new_n73_), .O(new_n87_));
  inv1   g041(.a(new_n87_), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n86_), .c(x16), .O(new_n89_));
  inv1   g043(.a(x10), .O(new_n90_));
  aoi21  g044(.a(new_n81_), .b(new_n90_), .c(x16), .O(new_n91_));
  oai21  g045(.a(new_n81_), .b(x02), .c(new_n91_), .O(new_n92_));
  aoi21  g046(.a(new_n92_), .b(new_n89_), .c(new_n61_), .O(z11));
  inv1   g047(.a(x11), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n94_), .c(new_n64_), .O(new_n95_));
  aoi21  g049(.a(x18), .b(x03), .c(new_n95_), .O(new_n96_));
  nor2   g050(.a(x22), .b(x21), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n74_), .c(new_n73_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(x16), .O(new_n99_));
  aoi21  g053(.a(new_n87_), .b(x22), .c(new_n99_), .O(new_n100_));
  oai21  g054(.a(new_n100_), .b(new_n96_), .c(new_n62_), .O(z12));
  nor2   g055(.a(new_n61_), .b(x16), .O(new_n102_));
  inv1   g056(.a(x12), .O(new_n103_));
  nand2  g057(.a(new_n81_), .b(new_n103_), .O(new_n104_));
  inv1   g058(.a(x04), .O(new_n105_));
  nand2  g059(.a(x18), .b(new_n105_), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n104_), .c(new_n102_), .O(new_n107_));
  aoi21  g061(.a(new_n98_), .b(new_n47_), .c(new_n64_), .O(new_n108_));
  oai21  g062(.a(new_n98_), .b(new_n55_), .c(new_n108_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n107_), .O(z13));
  inv1   g064(.a(x24), .O(new_n111_));
  nand2  g065(.a(new_n98_), .b(new_n48_), .O(new_n112_));
  aoi21  g066(.a(new_n112_), .b(new_n47_), .c(new_n111_), .O(new_n113_));
  nor3   g067(.a(x22), .b(x21), .c(x20), .O(new_n114_));
  nand4  g068(.a(new_n114_), .b(new_n74_), .c(new_n55_), .d(new_n111_), .O(new_n115_));
  inv1   g069(.a(new_n115_), .O(new_n116_));
  oai21  g070(.a(new_n116_), .b(new_n113_), .c(x16), .O(new_n117_));
  inv1   g071(.a(x13), .O(new_n118_));
  nand2  g072(.a(new_n81_), .b(new_n118_), .O(new_n119_));
  inv1   g073(.a(x05), .O(new_n120_));
  nand2  g074(.a(x18), .b(new_n120_), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n119_), .c(new_n102_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n117_), .O(z14));
  inv1   g077(.a(x14), .O(new_n124_));
  nand2  g078(.a(new_n81_), .b(new_n124_), .O(new_n125_));
  inv1   g079(.a(x06), .O(new_n126_));
  nand2  g080(.a(x18), .b(new_n126_), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(new_n125_), .c(new_n102_), .O(new_n128_));
  oai21  g082(.a(new_n98_), .b(x24), .c(new_n48_), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(x25), .c(new_n47_), .O(new_n130_));
  inv1   g084(.a(x25), .O(new_n131_));
  nand2  g085(.a(new_n115_), .b(new_n131_), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(new_n130_), .c(x16), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n128_), .O(z15));
  inv1   g088(.a(x26), .O(new_n135_));
  nor2   g089(.a(x25), .b(x24), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n97_), .c(new_n74_), .d(new_n73_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n48_), .O(new_n138_));
  aoi21  g092(.a(new_n138_), .b(new_n47_), .c(new_n135_), .O(new_n139_));
  nand3  g093(.a(new_n136_), .b(new_n55_), .c(new_n135_), .O(new_n140_));
  nor2   g094(.a(new_n140_), .b(new_n98_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n139_), .c(x16), .O(new_n142_));
  inv1   g096(.a(x15), .O(new_n143_));
  nand2  g097(.a(new_n81_), .b(new_n143_), .O(new_n144_));
  inv1   g098(.a(x07), .O(new_n145_));
  nand2  g099(.a(x18), .b(new_n145_), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n144_), .c(new_n102_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n142_), .O(z16));
  nand2  g102(.a(new_n49_), .b(x17), .O(new_n149_));
  nand3  g103(.a(new_n114_), .b(x19), .c(new_n67_), .O(new_n150_));
  or2    g104(.a(new_n150_), .b(new_n140_), .O(new_n151_));
  aoi21  g105(.a(new_n151_), .b(new_n149_), .c(new_n64_), .O(z17));
  buf    g106(.a(x27), .O(z08));
endmodule


