// Benchmark "FAU" written by ABC on Sat Aug  8 23:28:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n51_, new_n52_, new_n53_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_;
  inv1   g00(.a(x26), .O(new_n47_));
  nor2   g01(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi22  g02(.a(new_n48_), .b(x19), .c(x27), .d(x08), .O(z00));
  aoi22  g03(.a(new_n48_), .b(x20), .c(x27), .d(x09), .O(z01));
  inv1   g04(.a(x27), .O(new_n51_));
  inv1   g05(.a(x21), .O(new_n52_));
  nand2  g06(.a(new_n48_), .b(new_n52_), .O(new_n53_));
  oai21  g07(.a(new_n51_), .b(x10), .c(new_n53_), .O(z02));
  aoi22  g08(.a(new_n48_), .b(x22), .c(x27), .d(x11), .O(z03));
  aoi22  g09(.a(new_n48_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi22  g10(.a(new_n48_), .b(x24), .c(x27), .d(x13), .O(z05));
  aoi22  g11(.a(new_n48_), .b(x25), .c(x27), .d(x14), .O(z06));
  nor2   g12(.a(new_n51_), .b(x15), .O(z07));
  inv1   g13(.a(x18), .O(new_n60_));
  inv1   g14(.a(x08), .O(new_n61_));
  aoi21  g15(.a(new_n60_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g16(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g17(.a(x17), .O(new_n64_));
  inv1   g18(.a(x19), .O(new_n65_));
  nor2   g19(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nor2   g20(.a(x19), .b(x17), .O(new_n67_));
  oai21  g21(.a(new_n67_), .b(new_n66_), .c(x16), .O(new_n68_));
  nor2   g22(.a(x27), .b(x26), .O(new_n69_));
  inv1   g23(.a(new_n69_), .O(new_n70_));
  nand3  g24(.a(new_n70_), .b(new_n68_), .c(new_n63_), .O(z09));
  inv1   g25(.a(x20), .O(new_n72_));
  nor2   g26(.a(new_n67_), .b(new_n72_), .O(new_n73_));
  nor3   g27(.a(x20), .b(x19), .c(x17), .O(new_n74_));
  oai21  g28(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  inv1   g29(.a(x01), .O(new_n76_));
  nand2  g30(.a(x18), .b(new_n76_), .O(new_n77_));
  inv1   g31(.a(x09), .O(new_n78_));
  aoi21  g32(.a(new_n60_), .b(new_n78_), .c(x16), .O(new_n79_));
  aoi21  g33(.a(new_n79_), .b(new_n77_), .c(new_n69_), .O(new_n80_));
  nand2  g34(.a(new_n80_), .b(new_n75_), .O(z10));
  aoi21  g35(.a(new_n67_), .b(new_n72_), .c(new_n52_), .O(new_n82_));
  nand4  g36(.a(new_n52_), .b(new_n72_), .c(new_n65_), .d(new_n64_), .O(new_n83_));
  inv1   g37(.a(new_n83_), .O(new_n84_));
  oai21  g38(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  inv1   g39(.a(x10), .O(new_n86_));
  aoi21  g40(.a(new_n60_), .b(new_n86_), .c(x16), .O(new_n87_));
  oai21  g41(.a(new_n60_), .b(x02), .c(new_n87_), .O(new_n88_));
  aoi21  g42(.a(new_n88_), .b(new_n85_), .c(new_n69_), .O(z11));
  nand2  g43(.a(new_n83_), .b(x22), .O(new_n90_));
  inv1   g44(.a(x22), .O(new_n91_));
  nand4  g45(.a(new_n67_), .b(new_n91_), .c(new_n52_), .d(new_n72_), .O(new_n92_));
  nand2  g46(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g47(.a(new_n93_), .b(x16), .O(new_n94_));
  inv1   g48(.a(x11), .O(new_n95_));
  aoi21  g49(.a(new_n60_), .b(new_n95_), .c(x16), .O(new_n96_));
  oai21  g50(.a(new_n60_), .b(x03), .c(new_n96_), .O(new_n97_));
  aoi21  g51(.a(new_n97_), .b(new_n94_), .c(new_n69_), .O(z12));
  nand2  g52(.a(new_n92_), .b(x23), .O(new_n99_));
  nor2   g53(.a(x23), .b(x22), .O(new_n100_));
  nand4  g54(.a(new_n100_), .b(new_n67_), .c(new_n52_), .d(new_n72_), .O(new_n101_));
  and2   g55(.a(new_n101_), .b(x16), .O(new_n102_));
  inv1   g56(.a(x16), .O(new_n103_));
  nand2  g57(.a(x18), .b(x04), .O(new_n104_));
  nand2  g58(.a(new_n60_), .b(x12), .O(new_n105_));
  nand3  g59(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  nand2  g60(.a(new_n106_), .b(new_n70_), .O(new_n107_));
  aoi21  g61(.a(new_n102_), .b(new_n99_), .c(new_n107_), .O(z13));
  nor2   g62(.a(x24), .b(x21), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(new_n100_), .O(new_n110_));
  inv1   g64(.a(new_n110_), .O(new_n111_));
  aoi22  g65(.a(new_n111_), .b(new_n74_), .c(new_n101_), .d(x24), .O(new_n112_));
  inv1   g66(.a(x05), .O(new_n113_));
  nand2  g67(.a(x18), .b(new_n113_), .O(new_n114_));
  inv1   g68(.a(x13), .O(new_n115_));
  aoi21  g69(.a(new_n60_), .b(new_n115_), .c(x16), .O(new_n116_));
  aoi21  g70(.a(new_n116_), .b(new_n114_), .c(new_n69_), .O(new_n117_));
  oai21  g71(.a(new_n112_), .b(new_n103_), .c(new_n117_), .O(z14));
  nand4  g72(.a(new_n109_), .b(new_n100_), .c(new_n67_), .d(new_n72_), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(x25), .O(new_n120_));
  nor2   g74(.a(x25), .b(x24), .O(new_n121_));
  nand4  g75(.a(new_n121_), .b(new_n100_), .c(new_n74_), .d(new_n52_), .O(new_n122_));
  nand2  g76(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g77(.a(new_n123_), .b(x16), .O(new_n124_));
  inv1   g78(.a(x06), .O(new_n125_));
  nand2  g79(.a(x18), .b(new_n125_), .O(new_n126_));
  inv1   g80(.a(x14), .O(new_n127_));
  aoi21  g81(.a(new_n60_), .b(new_n127_), .c(x16), .O(new_n128_));
  aoi21  g82(.a(new_n128_), .b(new_n126_), .c(new_n69_), .O(new_n129_));
  nand2  g83(.a(new_n129_), .b(new_n124_), .O(z15));
  nand3  g84(.a(new_n122_), .b(x26), .c(x16), .O(new_n131_));
  inv1   g85(.a(x23), .O(new_n132_));
  nand3  g86(.a(new_n121_), .b(new_n132_), .c(x16), .O(new_n133_));
  oai21  g87(.a(new_n133_), .b(new_n92_), .c(x27), .O(new_n134_));
  nand2  g88(.a(new_n134_), .b(new_n47_), .O(new_n135_));
  inv1   g89(.a(x15), .O(new_n136_));
  aoi21  g90(.a(new_n60_), .b(new_n136_), .c(x16), .O(new_n137_));
  oai21  g91(.a(new_n60_), .b(x07), .c(new_n137_), .O(new_n138_));
  nand3  g92(.a(new_n138_), .b(new_n135_), .c(new_n131_), .O(z16));
  nor2   g93(.a(x26), .b(x25), .O(new_n140_));
  nand4  g94(.a(new_n140_), .b(new_n111_), .c(new_n72_), .d(x19), .O(new_n141_));
  nand2  g95(.a(x27), .b(x16), .O(new_n142_));
  aoi21  g96(.a(new_n141_), .b(new_n64_), .c(new_n142_), .O(z17));
  buf    g97(.a(x27), .O(z08));
endmodule


