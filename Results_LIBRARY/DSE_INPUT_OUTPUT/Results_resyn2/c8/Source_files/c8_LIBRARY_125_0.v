// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_;
  inv1   g000(.a(x20), .O(new_n47_));
  nor2   g001(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi22  g002(.a(new_n48_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g003(.a(x27), .O(new_n50_));
  nor2   g004(.a(new_n50_), .b(x09), .O(z01));
  aoi22  g005(.a(new_n48_), .b(x21), .c(x27), .d(x10), .O(z02));
  aoi22  g006(.a(new_n48_), .b(x22), .c(x27), .d(x11), .O(z03));
  inv1   g007(.a(new_n48_), .O(z08));
  oai22  g008(.a(z08), .b(x23), .c(new_n50_), .d(x12), .O(z04));
  aoi22  g009(.a(new_n48_), .b(x24), .c(x27), .d(x13), .O(z05));
  aoi22  g010(.a(new_n48_), .b(x25), .c(x27), .d(x14), .O(z06));
  oai22  g011(.a(z08), .b(x26), .c(new_n50_), .d(x15), .O(z07));
  inv1   g012(.a(x17), .O(new_n59_));
  inv1   g013(.a(x19), .O(new_n60_));
  nor2   g014(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g015(.a(x19), .b(x17), .O(new_n62_));
  oai21  g016(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  nor2   g017(.a(x27), .b(x20), .O(new_n64_));
  inv1   g018(.a(new_n64_), .O(new_n65_));
  inv1   g019(.a(x18), .O(new_n66_));
  inv1   g020(.a(x08), .O(new_n67_));
  aoi21  g021(.a(new_n66_), .b(new_n67_), .c(x16), .O(new_n68_));
  oai21  g022(.a(new_n66_), .b(x00), .c(new_n68_), .O(new_n69_));
  nand3  g023(.a(new_n69_), .b(new_n65_), .c(new_n63_), .O(z09));
  nor2   g024(.a(new_n62_), .b(new_n47_), .O(new_n71_));
  nand2  g025(.a(new_n60_), .b(new_n59_), .O(new_n72_));
  nor2   g026(.a(new_n72_), .b(x20), .O(new_n73_));
  oai21  g027(.a(new_n73_), .b(new_n71_), .c(x16), .O(new_n74_));
  inv1   g028(.a(x01), .O(new_n75_));
  nand2  g029(.a(x18), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x09), .O(new_n77_));
  aoi21  g031(.a(new_n66_), .b(new_n77_), .c(x16), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n64_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n74_), .O(z10));
  inv1   g034(.a(x16), .O(new_n81_));
  inv1   g035(.a(x21), .O(new_n82_));
  nand2  g036(.a(new_n62_), .b(new_n82_), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n81_), .c(x27), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n47_), .O(new_n85_));
  inv1   g039(.a(x10), .O(new_n86_));
  aoi21  g040(.a(new_n66_), .b(new_n86_), .c(x16), .O(new_n87_));
  oai21  g041(.a(new_n66_), .b(x02), .c(new_n87_), .O(new_n88_));
  inv1   g042(.a(new_n73_), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(x21), .c(x16), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n88_), .c(new_n85_), .O(z11));
  nor3   g045(.a(new_n83_), .b(x22), .c(new_n81_), .O(new_n92_));
  oai21  g046(.a(new_n92_), .b(new_n50_), .c(new_n47_), .O(new_n93_));
  nand4  g047(.a(new_n62_), .b(x27), .c(new_n82_), .d(new_n47_), .O(new_n94_));
  aoi21  g048(.a(new_n94_), .b(x22), .c(new_n81_), .O(new_n95_));
  inv1   g049(.a(x11), .O(new_n96_));
  oai21  g050(.a(x18), .b(new_n96_), .c(new_n81_), .O(new_n97_));
  aoi21  g051(.a(x18), .b(x03), .c(new_n97_), .O(new_n98_));
  oai21  g052(.a(new_n98_), .b(new_n95_), .c(new_n93_), .O(z12));
  inv1   g053(.a(x23), .O(new_n100_));
  nor2   g054(.a(x22), .b(x21), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n62_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(x27), .O(new_n103_));
  aoi21  g057(.a(new_n103_), .b(new_n47_), .c(new_n100_), .O(new_n104_));
  nor3   g058(.a(new_n94_), .b(x23), .c(x22), .O(new_n105_));
  oai21  g059(.a(new_n105_), .b(new_n104_), .c(x16), .O(new_n106_));
  nor2   g060(.a(new_n64_), .b(x16), .O(new_n107_));
  inv1   g061(.a(x12), .O(new_n108_));
  nand2  g062(.a(new_n66_), .b(new_n108_), .O(new_n109_));
  inv1   g063(.a(x04), .O(new_n110_));
  nand2  g064(.a(x18), .b(new_n110_), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n106_), .O(z13));
  inv1   g067(.a(x24), .O(new_n114_));
  nand3  g068(.a(new_n101_), .b(new_n62_), .c(new_n100_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(x27), .O(new_n116_));
  aoi21  g070(.a(new_n116_), .b(new_n47_), .c(new_n114_), .O(new_n117_));
  nor2   g071(.a(x23), .b(x22), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n114_), .O(new_n119_));
  nor2   g073(.a(new_n119_), .b(new_n94_), .O(new_n120_));
  oai21  g074(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  inv1   g075(.a(x13), .O(new_n122_));
  nand2  g076(.a(new_n66_), .b(new_n122_), .O(new_n123_));
  inv1   g077(.a(x05), .O(new_n124_));
  nand2  g078(.a(x18), .b(new_n124_), .O(new_n125_));
  nand3  g079(.a(new_n125_), .b(new_n123_), .c(new_n107_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n121_), .O(z14));
  inv1   g081(.a(x14), .O(new_n128_));
  nand2  g082(.a(new_n66_), .b(new_n128_), .O(new_n129_));
  inv1   g083(.a(x06), .O(new_n130_));
  nand2  g084(.a(x18), .b(new_n130_), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n129_), .c(new_n107_), .O(new_n132_));
  oai21  g086(.a(new_n119_), .b(new_n83_), .c(x27), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(x25), .c(new_n47_), .O(new_n134_));
  inv1   g088(.a(x25), .O(new_n135_));
  oai21  g089(.a(new_n119_), .b(new_n94_), .c(new_n135_), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n134_), .c(x16), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n132_), .O(z15));
  inv1   g092(.a(x26), .O(new_n139_));
  nor2   g093(.a(x25), .b(x24), .O(new_n140_));
  nand4  g094(.a(new_n140_), .b(new_n118_), .c(new_n62_), .d(new_n82_), .O(new_n141_));
  aoi21  g095(.a(new_n141_), .b(x27), .c(x20), .O(new_n142_));
  nand4  g096(.a(new_n139_), .b(new_n135_), .c(new_n114_), .d(new_n100_), .O(new_n143_));
  inv1   g097(.a(new_n143_), .O(new_n144_));
  nand2  g098(.a(new_n101_), .b(x27), .O(new_n145_));
  inv1   g099(.a(new_n145_), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n144_), .c(new_n73_), .O(new_n147_));
  oai21  g101(.a(new_n142_), .b(new_n139_), .c(new_n147_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(x16), .O(new_n149_));
  inv1   g103(.a(x15), .O(new_n150_));
  nand2  g104(.a(new_n66_), .b(new_n150_), .O(new_n151_));
  inv1   g105(.a(x07), .O(new_n152_));
  nand2  g106(.a(x18), .b(new_n152_), .O(new_n153_));
  nand3  g107(.a(new_n153_), .b(new_n151_), .c(new_n107_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n149_), .O(z16));
  nand4  g109(.a(new_n144_), .b(new_n101_), .c(new_n47_), .d(x19), .O(new_n156_));
  nand2  g110(.a(x27), .b(x16), .O(new_n157_));
  aoi21  g111(.a(new_n156_), .b(new_n59_), .c(new_n157_), .O(z17));
endmodule


