// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n52_, new_n53_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_;
  nand2  g00(.a(x27), .b(x15), .O(new_n47_));
  oai22  g01(.a(new_n47_), .b(x08), .c(x27), .d(x19), .O(z00));
  oai22  g02(.a(new_n47_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g03(.a(new_n47_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g04(.a(new_n47_), .b(x11), .c(x27), .d(x22), .O(z03));
  inv1   g05(.a(x27), .O(new_n52_));
  inv1   g06(.a(new_n47_), .O(new_n53_));
  aoi22  g07(.a(new_n53_), .b(x12), .c(new_n52_), .d(x23), .O(z04));
  aoi22  g08(.a(new_n53_), .b(x13), .c(new_n52_), .d(x24), .O(z05));
  oai22  g09(.a(new_n47_), .b(x14), .c(x27), .d(x25), .O(z06));
  nor2   g10(.a(new_n52_), .b(x15), .O(new_n57_));
  inv1   g11(.a(new_n57_), .O(new_n58_));
  oai21  g12(.a(x27), .b(x26), .c(new_n58_), .O(z07));
  inv1   g13(.a(x18), .O(new_n60_));
  inv1   g14(.a(x08), .O(new_n61_));
  aoi21  g15(.a(new_n60_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g16(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g17(.a(x19), .O(new_n64_));
  nand2  g18(.a(new_n64_), .b(x17), .O(new_n65_));
  inv1   g19(.a(x16), .O(new_n66_));
  nor2   g20(.a(new_n64_), .b(x17), .O(new_n67_));
  nor2   g21(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g22(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  aoi21  g23(.a(new_n69_), .b(new_n63_), .c(new_n57_), .O(z09));
  nor2   g24(.a(x19), .b(x17), .O(new_n71_));
  xor2a  g25(.a(new_n71_), .b(x20), .O(new_n72_));
  nand2  g26(.a(new_n72_), .b(x16), .O(new_n73_));
  inv1   g27(.a(x09), .O(new_n74_));
  aoi21  g28(.a(new_n60_), .b(new_n74_), .c(x16), .O(new_n75_));
  oai21  g29(.a(new_n60_), .b(x01), .c(new_n75_), .O(new_n76_));
  aoi21  g30(.a(new_n76_), .b(new_n73_), .c(new_n57_), .O(z10));
  inv1   g31(.a(x20), .O(new_n78_));
  inv1   g32(.a(x21), .O(new_n79_));
  aoi21  g33(.a(new_n71_), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nor2   g34(.a(x21), .b(x20), .O(new_n81_));
  nand2  g35(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  oai21  g37(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  inv1   g38(.a(x10), .O(new_n85_));
  aoi21  g39(.a(new_n60_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g40(.a(new_n60_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g41(.a(new_n87_), .b(new_n84_), .c(new_n57_), .O(z11));
  xor2a  g42(.a(new_n82_), .b(x22), .O(new_n89_));
  inv1   g43(.a(x03), .O(new_n90_));
  nand2  g44(.a(x18), .b(new_n90_), .O(new_n91_));
  inv1   g45(.a(x11), .O(new_n92_));
  aoi21  g46(.a(new_n60_), .b(new_n92_), .c(x16), .O(new_n93_));
  aoi21  g47(.a(new_n93_), .b(new_n91_), .c(new_n57_), .O(new_n94_));
  oai21  g48(.a(new_n89_), .b(new_n66_), .c(new_n94_), .O(z12));
  oai21  g49(.a(new_n82_), .b(x22), .c(x23), .O(new_n96_));
  nor2   g50(.a(x23), .b(x22), .O(new_n97_));
  nand3  g51(.a(new_n97_), .b(new_n81_), .c(new_n71_), .O(new_n98_));
  nand2  g52(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand2  g53(.a(new_n99_), .b(x16), .O(new_n100_));
  inv1   g54(.a(x04), .O(new_n101_));
  nand2  g55(.a(x18), .b(new_n101_), .O(new_n102_));
  inv1   g56(.a(x12), .O(new_n103_));
  aoi21  g57(.a(new_n60_), .b(new_n103_), .c(x16), .O(new_n104_));
  aoi21  g58(.a(new_n104_), .b(new_n102_), .c(new_n57_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(new_n100_), .O(z13));
  inv1   g60(.a(x24), .O(new_n107_));
  xor2a  g61(.a(new_n98_), .b(new_n107_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(x16), .O(new_n109_));
  inv1   g63(.a(x13), .O(new_n110_));
  aoi21  g64(.a(new_n60_), .b(new_n110_), .c(x16), .O(new_n111_));
  oai21  g65(.a(new_n60_), .b(x05), .c(new_n111_), .O(new_n112_));
  aoi21  g66(.a(new_n112_), .b(new_n109_), .c(new_n57_), .O(z14));
  nand4  g67(.a(new_n97_), .b(new_n81_), .c(new_n71_), .d(new_n107_), .O(new_n114_));
  nor2   g68(.a(x25), .b(x24), .O(new_n115_));
  nand2  g69(.a(new_n115_), .b(new_n97_), .O(new_n116_));
  inv1   g70(.a(new_n116_), .O(new_n117_));
  aoi22  g71(.a(new_n117_), .b(new_n83_), .c(new_n114_), .d(x25), .O(new_n118_));
  inv1   g72(.a(x06), .O(new_n119_));
  nand2  g73(.a(x18), .b(new_n119_), .O(new_n120_));
  inv1   g74(.a(x14), .O(new_n121_));
  aoi21  g75(.a(new_n60_), .b(new_n121_), .c(x16), .O(new_n122_));
  aoi21  g76(.a(new_n122_), .b(new_n120_), .c(new_n57_), .O(new_n123_));
  oai21  g77(.a(new_n118_), .b(new_n66_), .c(new_n123_), .O(z15));
  aoi21  g78(.a(new_n52_), .b(x18), .c(x15), .O(new_n125_));
  oai21  g79(.a(new_n60_), .b(x07), .c(new_n66_), .O(new_n126_));
  nand4  g80(.a(new_n115_), .b(new_n97_), .c(new_n81_), .d(new_n71_), .O(new_n127_));
  xor2a  g81(.a(new_n127_), .b(x26), .O(new_n128_));
  nand2  g82(.a(new_n58_), .b(x16), .O(new_n129_));
  oai22  g83(.a(new_n129_), .b(new_n128_), .c(new_n126_), .d(new_n125_), .O(z16));
  nand2  g84(.a(x27), .b(x17), .O(new_n131_));
  inv1   g85(.a(x26), .O(new_n132_));
  nand4  g86(.a(new_n117_), .b(new_n81_), .c(new_n67_), .d(new_n132_), .O(new_n133_));
  aoi21  g87(.a(new_n133_), .b(new_n131_), .c(new_n129_), .O(z17));
  buf    g88(.a(x27), .O(z08));
endmodule


