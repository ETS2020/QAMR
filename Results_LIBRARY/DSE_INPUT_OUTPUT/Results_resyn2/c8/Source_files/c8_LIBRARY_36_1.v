// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_, new_n147_,
    new_n148_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(x27), .b(x23), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(z08));
  aoi22  g003(.a(z08), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  aoi22  g004(.a(z08), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  oai22  g005(.a(new_n48_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g006(.a(new_n48_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai21  g007(.a(new_n47_), .b(x12), .c(x23), .O(z04));
  aoi22  g008(.a(z08), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  oai22  g009(.a(new_n48_), .b(x14), .c(x27), .d(x25), .O(z06));
  aoi22  g010(.a(z08), .b(x15), .c(new_n47_), .d(x26), .O(z07));
  inv1   g011(.a(x16), .O(new_n58_));
  nand2  g012(.a(x18), .b(x00), .O(new_n59_));
  inv1   g013(.a(x18), .O(new_n60_));
  nand2  g014(.a(new_n60_), .b(x08), .O(new_n61_));
  nand3  g015(.a(new_n61_), .b(new_n59_), .c(new_n58_), .O(new_n62_));
  nand2  g016(.a(x19), .b(x17), .O(new_n63_));
  nor2   g017(.a(x19), .b(x17), .O(new_n64_));
  nor2   g018(.a(new_n64_), .b(new_n58_), .O(new_n65_));
  nand2  g019(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g020(.a(new_n66_), .b(new_n62_), .O(new_n67_));
  oai21  g021(.a(new_n47_), .b(x23), .c(new_n67_), .O(z09));
  inv1   g022(.a(x20), .O(new_n69_));
  nor2   g023(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  nor3   g024(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  oai21  g025(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  nor2   g026(.a(new_n47_), .b(x23), .O(new_n73_));
  inv1   g027(.a(x01), .O(new_n74_));
  nand2  g028(.a(x18), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x09), .O(new_n76_));
  aoi21  g030(.a(new_n60_), .b(new_n76_), .c(x16), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n73_), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n72_), .O(z10));
  inv1   g033(.a(x21), .O(new_n80_));
  aoi21  g034(.a(new_n64_), .b(new_n69_), .c(new_n80_), .O(new_n81_));
  nand3  g035(.a(new_n64_), .b(new_n80_), .c(new_n69_), .O(new_n82_));
  inv1   g036(.a(new_n82_), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n81_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x10), .O(new_n85_));
  aoi21  g039(.a(new_n60_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g040(.a(new_n60_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g041(.a(new_n87_), .b(new_n84_), .c(new_n73_), .O(z11));
  nor2   g042(.a(x22), .b(x21), .O(new_n89_));
  aoi22  g043(.a(new_n89_), .b(new_n71_), .c(new_n82_), .d(x22), .O(new_n90_));
  inv1   g044(.a(x03), .O(new_n91_));
  nand2  g045(.a(x18), .b(new_n91_), .O(new_n92_));
  inv1   g046(.a(x11), .O(new_n93_));
  aoi21  g047(.a(new_n60_), .b(new_n93_), .c(x16), .O(new_n94_));
  aoi21  g048(.a(new_n94_), .b(new_n92_), .c(new_n73_), .O(new_n95_));
  oai21  g049(.a(new_n90_), .b(new_n58_), .c(new_n95_), .O(z12));
  nand3  g050(.a(new_n89_), .b(new_n64_), .c(new_n69_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(x23), .O(new_n98_));
  nor3   g052(.a(x27), .b(x23), .c(x22), .O(new_n99_));
  nand3  g053(.a(new_n99_), .b(new_n71_), .c(new_n80_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(x16), .O(new_n102_));
  nor2   g056(.a(new_n73_), .b(x16), .O(new_n103_));
  inv1   g057(.a(x12), .O(new_n104_));
  nand2  g058(.a(new_n60_), .b(new_n104_), .O(new_n105_));
  inv1   g059(.a(x04), .O(new_n106_));
  nand2  g060(.a(x18), .b(new_n106_), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n102_), .O(z13));
  inv1   g063(.a(x23), .O(new_n110_));
  inv1   g064(.a(x24), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(x16), .O(new_n112_));
  oai21  g066(.a(new_n112_), .b(new_n97_), .c(new_n47_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  inv1   g068(.a(x13), .O(new_n115_));
  aoi21  g069(.a(new_n60_), .b(new_n115_), .c(x16), .O(new_n116_));
  oai21  g070(.a(new_n60_), .b(x05), .c(new_n116_), .O(new_n117_));
  nand3  g071(.a(new_n100_), .b(x24), .c(x16), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n117_), .c(new_n114_), .O(z14));
  inv1   g073(.a(x25), .O(new_n120_));
  nor2   g074(.a(new_n120_), .b(new_n58_), .O(new_n121_));
  oai21  g075(.a(new_n100_), .b(x24), .c(new_n121_), .O(new_n122_));
  nor2   g076(.a(x25), .b(x24), .O(new_n123_));
  nand4  g077(.a(new_n123_), .b(new_n89_), .c(new_n64_), .d(new_n69_), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n58_), .c(new_n47_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n110_), .O(new_n126_));
  inv1   g080(.a(x14), .O(new_n127_));
  aoi21  g081(.a(new_n60_), .b(new_n127_), .c(x16), .O(new_n128_));
  oai21  g082(.a(new_n60_), .b(x06), .c(new_n128_), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n126_), .c(new_n122_), .O(z15));
  inv1   g084(.a(x15), .O(new_n131_));
  nand2  g085(.a(new_n60_), .b(new_n131_), .O(new_n132_));
  inv1   g086(.a(x07), .O(new_n133_));
  nand2  g087(.a(x18), .b(new_n133_), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n132_), .c(new_n103_), .O(new_n135_));
  inv1   g089(.a(x22), .O(new_n136_));
  nand4  g090(.a(new_n120_), .b(new_n111_), .c(new_n136_), .d(new_n80_), .O(new_n137_));
  inv1   g091(.a(new_n137_), .O(new_n138_));
  nand4  g092(.a(new_n138_), .b(new_n71_), .c(x26), .d(new_n110_), .O(new_n139_));
  inv1   g093(.a(x26), .O(new_n140_));
  oai21  g094(.a(new_n124_), .b(x23), .c(new_n140_), .O(new_n141_));
  nor2   g095(.a(new_n73_), .b(new_n58_), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n141_), .c(new_n139_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n135_), .O(z16));
  oai21  g098(.a(new_n110_), .b(x17), .c(x27), .O(new_n145_));
  inv1   g099(.a(x17), .O(new_n146_));
  nor3   g100(.a(x26), .b(x23), .c(x20), .O(new_n147_));
  nand4  g101(.a(new_n147_), .b(new_n138_), .c(x19), .d(new_n146_), .O(new_n148_));
  aoi21  g102(.a(new_n148_), .b(new_n145_), .c(new_n103_), .O(z17));
endmodule


