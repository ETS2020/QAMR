// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x11), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  oai22  g03(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  oai22  g04(.a(new_n49_), .b(x09), .c(x27), .d(x20), .O(z01));
  aoi22  g05(.a(new_n48_), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  nand2  g06(.a(new_n47_), .b(x22), .O(z03));
  aoi22  g07(.a(new_n48_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  oai22  g08(.a(new_n49_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g09(.a(new_n48_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g10(.a(new_n49_), .b(x15), .c(x27), .d(x26), .O(z07));
  nand2  g11(.a(x27), .b(x11), .O(new_n58_));
  inv1   g12(.a(new_n58_), .O(new_n59_));
  nor2   g13(.a(x19), .b(x17), .O(new_n60_));
  and2   g14(.a(x19), .b(x17), .O(new_n61_));
  oai21  g15(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g16(.a(x18), .O(new_n63_));
  inv1   g17(.a(x08), .O(new_n64_));
  aoi21  g18(.a(new_n63_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g19(.a(new_n63_), .b(x00), .c(new_n65_), .O(new_n66_));
  aoi21  g20(.a(new_n66_), .b(new_n62_), .c(new_n59_), .O(z09));
  xor2a  g21(.a(new_n60_), .b(x20), .O(new_n68_));
  nand2  g22(.a(new_n68_), .b(x16), .O(new_n69_));
  inv1   g23(.a(x09), .O(new_n70_));
  aoi21  g24(.a(new_n63_), .b(new_n70_), .c(x16), .O(new_n71_));
  oai21  g25(.a(new_n63_), .b(x01), .c(new_n71_), .O(new_n72_));
  nand3  g26(.a(new_n72_), .b(new_n69_), .c(new_n58_), .O(z10));
  inv1   g27(.a(x16), .O(new_n74_));
  inv1   g28(.a(x20), .O(new_n75_));
  nand2  g29(.a(new_n60_), .b(new_n75_), .O(new_n76_));
  nor2   g30(.a(x21), .b(x20), .O(new_n77_));
  aoi22  g31(.a(new_n77_), .b(new_n60_), .c(new_n76_), .d(x21), .O(new_n78_));
  inv1   g32(.a(x02), .O(new_n79_));
  nand2  g33(.a(x18), .b(new_n79_), .O(new_n80_));
  inv1   g34(.a(x10), .O(new_n81_));
  aoi21  g35(.a(new_n63_), .b(new_n81_), .c(x16), .O(new_n82_));
  aoi21  g36(.a(new_n82_), .b(new_n80_), .c(new_n59_), .O(new_n83_));
  oai21  g37(.a(new_n78_), .b(new_n74_), .c(new_n83_), .O(z11));
  inv1   g38(.a(x22), .O(new_n85_));
  nand3  g39(.a(new_n77_), .b(new_n60_), .c(new_n85_), .O(new_n86_));
  nand2  g40(.a(new_n77_), .b(new_n60_), .O(new_n87_));
  aoi21  g41(.a(new_n87_), .b(x22), .c(new_n74_), .O(new_n88_));
  nand2  g42(.a(x18), .b(x03), .O(new_n89_));
  nand2  g43(.a(new_n63_), .b(x11), .O(new_n90_));
  nand3  g44(.a(new_n90_), .b(new_n89_), .c(new_n74_), .O(new_n91_));
  nand2  g45(.a(new_n91_), .b(new_n58_), .O(new_n92_));
  aoi21  g46(.a(new_n88_), .b(new_n86_), .c(new_n92_), .O(z12));
  xor2a  g47(.a(new_n86_), .b(x23), .O(new_n94_));
  inv1   g48(.a(x04), .O(new_n95_));
  nand2  g49(.a(x18), .b(new_n95_), .O(new_n96_));
  inv1   g50(.a(x12), .O(new_n97_));
  aoi21  g51(.a(new_n63_), .b(new_n97_), .c(x16), .O(new_n98_));
  aoi21  g52(.a(new_n98_), .b(new_n96_), .c(new_n59_), .O(new_n99_));
  oai21  g53(.a(new_n94_), .b(new_n74_), .c(new_n99_), .O(z13));
  inv1   g54(.a(x23), .O(new_n101_));
  nand4  g55(.a(new_n77_), .b(new_n60_), .c(new_n101_), .d(new_n85_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(x24), .O(new_n103_));
  nor2   g57(.a(x24), .b(x23), .O(new_n104_));
  nor2   g58(.a(x22), .b(x21), .O(new_n105_));
  nand4  g59(.a(new_n105_), .b(new_n104_), .c(new_n60_), .d(new_n75_), .O(new_n106_));
  nand2  g60(.a(new_n106_), .b(new_n103_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(x16), .O(new_n108_));
  inv1   g62(.a(x05), .O(new_n109_));
  nand2  g63(.a(x18), .b(new_n109_), .O(new_n110_));
  inv1   g64(.a(x13), .O(new_n111_));
  aoi21  g65(.a(new_n63_), .b(new_n111_), .c(x16), .O(new_n112_));
  aoi21  g66(.a(new_n112_), .b(new_n110_), .c(new_n59_), .O(new_n113_));
  nand2  g67(.a(new_n113_), .b(new_n108_), .O(z14));
  nand2  g68(.a(new_n106_), .b(x25), .O(new_n115_));
  nor2   g69(.a(x25), .b(x22), .O(new_n116_));
  nand4  g70(.a(new_n116_), .b(new_n104_), .c(new_n77_), .d(new_n60_), .O(new_n117_));
  and2   g71(.a(new_n117_), .b(x16), .O(new_n118_));
  nand2  g72(.a(x18), .b(x06), .O(new_n119_));
  nand2  g73(.a(new_n63_), .b(x14), .O(new_n120_));
  nand3  g74(.a(new_n120_), .b(new_n119_), .c(new_n74_), .O(new_n121_));
  nand2  g75(.a(new_n121_), .b(new_n58_), .O(new_n122_));
  aoi21  g76(.a(new_n118_), .b(new_n115_), .c(new_n122_), .O(z15));
  inv1   g77(.a(x26), .O(new_n124_));
  nor2   g78(.a(new_n117_), .b(new_n124_), .O(new_n125_));
  nand2  g79(.a(new_n117_), .b(new_n124_), .O(new_n126_));
  nand2  g80(.a(new_n126_), .b(x16), .O(new_n127_));
  inv1   g81(.a(x07), .O(new_n128_));
  nand2  g82(.a(x18), .b(new_n128_), .O(new_n129_));
  inv1   g83(.a(x15), .O(new_n130_));
  aoi21  g84(.a(new_n63_), .b(new_n130_), .c(x16), .O(new_n131_));
  aoi21  g85(.a(new_n131_), .b(new_n129_), .c(new_n59_), .O(new_n132_));
  oai21  g86(.a(new_n127_), .b(new_n125_), .c(new_n132_), .O(z16));
  inv1   g87(.a(x17), .O(new_n134_));
  nand2  g88(.a(new_n116_), .b(new_n104_), .O(new_n135_));
  nand4  g89(.a(new_n77_), .b(new_n124_), .c(x19), .d(new_n134_), .O(new_n136_));
  oai22  g90(.a(new_n136_), .b(new_n135_), .c(new_n47_), .d(new_n134_), .O(new_n137_));
  nand2  g91(.a(new_n137_), .b(x16), .O(new_n138_));
  nand2  g92(.a(new_n138_), .b(new_n58_), .O(z17));
  buf    g93(.a(x27), .O(z08));
endmodule


