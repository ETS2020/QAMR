// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x14), .O(new_n47_));
  nor2   g01(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi22  g02(.a(new_n48_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g03(.a(x27), .O(new_n50_));
  inv1   g04(.a(new_n48_), .O(z08));
  oai22  g05(.a(z08), .b(x20), .c(new_n50_), .d(x09), .O(z01));
  oai22  g06(.a(z08), .b(x21), .c(new_n50_), .d(x10), .O(z02));
  oai22  g07(.a(z08), .b(x22), .c(new_n50_), .d(x11), .O(z03));
  oai22  g08(.a(z08), .b(x23), .c(new_n50_), .d(x12), .O(z04));
  aoi22  g09(.a(new_n48_), .b(x24), .c(x27), .d(x13), .O(z05));
  oai21  g10(.a(x27), .b(x25), .c(x14), .O(z06));
  oai22  g11(.a(z08), .b(x26), .c(new_n50_), .d(x15), .O(z07));
  nor2   g12(.a(x27), .b(x14), .O(new_n59_));
  and2   g13(.a(x19), .b(x17), .O(new_n60_));
  nor2   g14(.a(x19), .b(x17), .O(new_n61_));
  oai21  g15(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g16(.a(x18), .O(new_n63_));
  inv1   g17(.a(x08), .O(new_n64_));
  aoi21  g18(.a(new_n63_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g19(.a(new_n63_), .b(x00), .c(new_n65_), .O(new_n66_));
  aoi21  g20(.a(new_n66_), .b(new_n62_), .c(new_n59_), .O(z09));
  inv1   g21(.a(x20), .O(new_n68_));
  nor2   g22(.a(new_n61_), .b(new_n68_), .O(new_n69_));
  nor3   g23(.a(x20), .b(x19), .c(x17), .O(new_n70_));
  oai21  g24(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  inv1   g25(.a(x09), .O(new_n72_));
  aoi21  g26(.a(new_n63_), .b(new_n72_), .c(x16), .O(new_n73_));
  oai21  g27(.a(new_n63_), .b(x01), .c(new_n73_), .O(new_n74_));
  aoi21  g28(.a(new_n74_), .b(new_n71_), .c(new_n59_), .O(z10));
  inv1   g29(.a(x21), .O(new_n76_));
  nor2   g30(.a(new_n70_), .b(new_n76_), .O(new_n77_));
  nor2   g31(.a(x21), .b(x20), .O(new_n78_));
  nand2  g32(.a(new_n78_), .b(new_n61_), .O(new_n79_));
  inv1   g33(.a(new_n79_), .O(new_n80_));
  oai21  g34(.a(new_n80_), .b(new_n77_), .c(x16), .O(new_n81_));
  inv1   g35(.a(x02), .O(new_n82_));
  nand2  g36(.a(x18), .b(new_n82_), .O(new_n83_));
  inv1   g37(.a(x10), .O(new_n84_));
  aoi21  g38(.a(new_n63_), .b(new_n84_), .c(x16), .O(new_n85_));
  aoi21  g39(.a(new_n85_), .b(new_n83_), .c(new_n59_), .O(new_n86_));
  nand2  g40(.a(new_n86_), .b(new_n81_), .O(z11));
  inv1   g41(.a(x16), .O(new_n88_));
  nor2   g42(.a(x22), .b(x21), .O(new_n89_));
  aoi22  g43(.a(new_n89_), .b(new_n70_), .c(new_n79_), .d(x22), .O(new_n90_));
  inv1   g44(.a(x03), .O(new_n91_));
  nand2  g45(.a(x18), .b(new_n91_), .O(new_n92_));
  inv1   g46(.a(x11), .O(new_n93_));
  aoi21  g47(.a(new_n63_), .b(new_n93_), .c(x16), .O(new_n94_));
  aoi21  g48(.a(new_n94_), .b(new_n92_), .c(new_n59_), .O(new_n95_));
  oai21  g49(.a(new_n90_), .b(new_n88_), .c(new_n95_), .O(z12));
  inv1   g50(.a(x23), .O(new_n97_));
  nand3  g51(.a(new_n89_), .b(new_n61_), .c(new_n68_), .O(new_n98_));
  xor2a  g52(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g53(.a(new_n99_), .b(x16), .O(new_n100_));
  inv1   g54(.a(x12), .O(new_n101_));
  aoi21  g55(.a(new_n63_), .b(new_n101_), .c(x16), .O(new_n102_));
  oai21  g56(.a(new_n63_), .b(x04), .c(new_n102_), .O(new_n103_));
  aoi21  g57(.a(new_n103_), .b(new_n100_), .c(new_n59_), .O(z13));
  nand4  g58(.a(new_n89_), .b(new_n61_), .c(new_n97_), .d(new_n68_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(x24), .O(new_n106_));
  nor2   g60(.a(x24), .b(x23), .O(new_n107_));
  nand3  g61(.a(new_n107_), .b(new_n89_), .c(new_n70_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(x16), .O(new_n110_));
  inv1   g64(.a(x05), .O(new_n111_));
  nand2  g65(.a(x18), .b(new_n111_), .O(new_n112_));
  inv1   g66(.a(x13), .O(new_n113_));
  aoi21  g67(.a(new_n63_), .b(new_n113_), .c(x16), .O(new_n114_));
  aoi21  g68(.a(new_n114_), .b(new_n112_), .c(new_n59_), .O(new_n115_));
  nand2  g69(.a(new_n115_), .b(new_n110_), .O(z14));
  nand2  g70(.a(new_n108_), .b(x25), .O(new_n117_));
  nor2   g71(.a(x25), .b(x22), .O(new_n118_));
  nand4  g72(.a(new_n118_), .b(new_n107_), .c(new_n78_), .d(new_n61_), .O(new_n119_));
  and2   g73(.a(new_n119_), .b(x16), .O(new_n120_));
  nand2  g74(.a(x18), .b(x06), .O(new_n121_));
  nand2  g75(.a(new_n63_), .b(x14), .O(new_n122_));
  nand3  g76(.a(new_n122_), .b(new_n121_), .c(new_n88_), .O(new_n123_));
  oai21  g77(.a(x27), .b(x14), .c(new_n123_), .O(new_n124_));
  aoi21  g78(.a(new_n120_), .b(new_n117_), .c(new_n124_), .O(z15));
  nand2  g79(.a(new_n118_), .b(new_n107_), .O(new_n126_));
  inv1   g80(.a(new_n126_), .O(new_n127_));
  nand3  g81(.a(new_n127_), .b(new_n80_), .c(x26), .O(new_n128_));
  inv1   g82(.a(x26), .O(new_n129_));
  aoi21  g83(.a(new_n119_), .b(new_n129_), .c(new_n88_), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  inv1   g85(.a(x15), .O(new_n132_));
  aoi21  g86(.a(new_n63_), .b(new_n132_), .c(x16), .O(new_n133_));
  oai21  g87(.a(new_n63_), .b(x07), .c(new_n133_), .O(new_n134_));
  aoi21  g88(.a(new_n134_), .b(new_n131_), .c(new_n59_), .O(z16));
  inv1   g89(.a(x17), .O(new_n136_));
  nor2   g90(.a(x26), .b(x21), .O(new_n137_));
  nand4  g91(.a(new_n137_), .b(new_n127_), .c(new_n68_), .d(x19), .O(new_n138_));
  oai21  g92(.a(x17), .b(new_n47_), .c(new_n50_), .O(new_n139_));
  nand2  g93(.a(new_n139_), .b(x16), .O(new_n140_));
  aoi21  g94(.a(new_n138_), .b(new_n136_), .c(new_n140_), .O(z17));
endmodule


