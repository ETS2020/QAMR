// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n56_, new_n57_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x14), .O(new_n48_));
  aoi22  g002(.a(new_n48_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g003(.a(new_n48_), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g005(.a(new_n50_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g006(.a(new_n50_), .b(x11), .c(x27), .d(x22), .O(z03));
  aoi22  g007(.a(new_n48_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  oai22  g008(.a(new_n50_), .b(x13), .c(x27), .d(x24), .O(z05));
  nand2  g009(.a(x27), .b(x14), .O(new_n56_));
  inv1   g010(.a(new_n56_), .O(new_n57_));
  aoi21  g011(.a(new_n47_), .b(x25), .c(new_n57_), .O(z06));
  oai22  g012(.a(new_n50_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g013(.a(x16), .O(new_n60_));
  nand2  g014(.a(x18), .b(x00), .O(new_n61_));
  inv1   g015(.a(x18), .O(new_n62_));
  nand2  g016(.a(new_n62_), .b(x08), .O(new_n63_));
  nand3  g017(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(new_n64_));
  inv1   g018(.a(x17), .O(new_n65_));
  inv1   g019(.a(x19), .O(new_n66_));
  nor2   g020(.a(x19), .b(x17), .O(new_n67_));
  nor2   g021(.a(new_n67_), .b(new_n60_), .O(new_n68_));
  oai21  g022(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  nand2  g023(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  nand2  g024(.a(new_n70_), .b(new_n56_), .O(z09));
  xor2a  g025(.a(new_n67_), .b(x20), .O(new_n72_));
  nand2  g026(.a(new_n72_), .b(x16), .O(new_n73_));
  inv1   g027(.a(x09), .O(new_n74_));
  aoi21  g028(.a(new_n62_), .b(new_n74_), .c(x16), .O(new_n75_));
  oai21  g029(.a(new_n62_), .b(x01), .c(new_n75_), .O(new_n76_));
  nand3  g030(.a(new_n76_), .b(new_n73_), .c(new_n56_), .O(z10));
  inv1   g031(.a(x20), .O(new_n78_));
  inv1   g032(.a(x21), .O(new_n79_));
  aoi21  g033(.a(new_n67_), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nand4  g034(.a(new_n79_), .b(new_n78_), .c(new_n66_), .d(new_n65_), .O(new_n81_));
  inv1   g035(.a(new_n81_), .O(new_n82_));
  oai21  g036(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  inv1   g037(.a(x02), .O(new_n84_));
  nand2  g038(.a(x18), .b(new_n84_), .O(new_n85_));
  inv1   g039(.a(x10), .O(new_n86_));
  aoi21  g040(.a(new_n62_), .b(new_n86_), .c(x16), .O(new_n87_));
  aoi21  g041(.a(new_n87_), .b(new_n85_), .c(new_n57_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n83_), .O(z11));
  xor2a  g043(.a(new_n81_), .b(x22), .O(new_n90_));
  inv1   g044(.a(x03), .O(new_n91_));
  nand2  g045(.a(x18), .b(new_n91_), .O(new_n92_));
  inv1   g046(.a(x11), .O(new_n93_));
  aoi21  g047(.a(new_n62_), .b(new_n93_), .c(x16), .O(new_n94_));
  aoi21  g048(.a(new_n94_), .b(new_n92_), .c(new_n57_), .O(new_n95_));
  oai21  g049(.a(new_n90_), .b(new_n60_), .c(new_n95_), .O(z12));
  inv1   g050(.a(x22), .O(new_n97_));
  nor2   g051(.a(x21), .b(x20), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n67_), .c(new_n97_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(x23), .O(new_n100_));
  nor2   g054(.a(x23), .b(x22), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n98_), .c(new_n67_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g058(.a(x04), .O(new_n105_));
  nand2  g059(.a(x18), .b(new_n105_), .O(new_n106_));
  inv1   g060(.a(x12), .O(new_n107_));
  aoi21  g061(.a(new_n62_), .b(new_n107_), .c(x16), .O(new_n108_));
  aoi21  g062(.a(new_n108_), .b(new_n106_), .c(new_n57_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n104_), .O(z13));
  xor2a  g064(.a(new_n102_), .b(x24), .O(new_n111_));
  inv1   g065(.a(x05), .O(new_n112_));
  nand2  g066(.a(x18), .b(new_n112_), .O(new_n113_));
  inv1   g067(.a(x13), .O(new_n114_));
  aoi21  g068(.a(new_n62_), .b(new_n114_), .c(x16), .O(new_n115_));
  aoi21  g069(.a(new_n115_), .b(new_n113_), .c(new_n57_), .O(new_n116_));
  oai21  g070(.a(new_n111_), .b(new_n60_), .c(new_n116_), .O(z14));
  inv1   g071(.a(x24), .O(new_n118_));
  nand4  g072(.a(new_n101_), .b(new_n98_), .c(new_n67_), .d(new_n118_), .O(new_n119_));
  inv1   g073(.a(x23), .O(new_n120_));
  inv1   g074(.a(x25), .O(new_n121_));
  nand4  g075(.a(new_n121_), .b(new_n118_), .c(new_n120_), .d(new_n97_), .O(new_n122_));
  inv1   g076(.a(new_n122_), .O(new_n123_));
  aoi22  g077(.a(new_n123_), .b(new_n82_), .c(new_n119_), .d(x25), .O(new_n124_));
  oai21  g078(.a(x18), .b(x16), .c(new_n47_), .O(new_n125_));
  nand3  g079(.a(x18), .b(new_n60_), .c(x06), .O(new_n126_));
  inv1   g080(.a(new_n126_), .O(new_n127_));
  aoi21  g081(.a(new_n125_), .b(x14), .c(new_n127_), .O(new_n128_));
  oai21  g082(.a(new_n124_), .b(new_n60_), .c(new_n128_), .O(z15));
  inv1   g083(.a(x26), .O(new_n130_));
  nor2   g084(.a(x25), .b(x24), .O(new_n131_));
  nand4  g085(.a(new_n131_), .b(new_n101_), .c(new_n98_), .d(new_n67_), .O(new_n132_));
  nor2   g086(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g087(.a(new_n132_), .b(new_n130_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(x16), .O(new_n135_));
  inv1   g089(.a(x07), .O(new_n136_));
  nand2  g090(.a(x18), .b(new_n136_), .O(new_n137_));
  inv1   g091(.a(x15), .O(new_n138_));
  aoi21  g092(.a(new_n62_), .b(new_n138_), .c(x16), .O(new_n139_));
  aoi21  g093(.a(new_n139_), .b(new_n137_), .c(new_n57_), .O(new_n140_));
  oai21  g094(.a(new_n135_), .b(new_n133_), .c(new_n140_), .O(z16));
  nor2   g095(.a(new_n66_), .b(x17), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n123_), .c(new_n98_), .d(new_n130_), .O(new_n143_));
  nand2  g097(.a(x27), .b(x17), .O(new_n144_));
  nand2  g098(.a(new_n56_), .b(x16), .O(new_n145_));
  aoi21  g099(.a(new_n144_), .b(new_n143_), .c(new_n145_), .O(z17));
  buf    g100(.a(x27), .O(z08));
endmodule


