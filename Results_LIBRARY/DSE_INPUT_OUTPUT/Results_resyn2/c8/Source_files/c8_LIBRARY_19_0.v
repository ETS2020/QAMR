// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n57_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x14), .O(new_n48_));
  nor2   g02(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g03(.a(new_n49_), .O(new_n50_));
  oai22  g04(.a(new_n50_), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  aoi22  g05(.a(new_n49_), .b(x20), .c(x27), .d(x09), .O(z01));
  aoi22  g06(.a(new_n49_), .b(x21), .c(x27), .d(x10), .O(z02));
  oai22  g07(.a(new_n50_), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  aoi22  g08(.a(new_n49_), .b(x23), .c(x27), .d(x12), .O(z04));
  oai22  g09(.a(new_n50_), .b(x24), .c(new_n47_), .d(x13), .O(z05));
  nand2  g10(.a(x27), .b(new_n48_), .O(new_n57_));
  oai21  g11(.a(new_n50_), .b(x25), .c(new_n57_), .O(z06));
  oai22  g12(.a(new_n50_), .b(x26), .c(new_n47_), .d(x15), .O(z07));
  nor2   g13(.a(x27), .b(x14), .O(new_n60_));
  inv1   g14(.a(x17), .O(new_n61_));
  inv1   g15(.a(x19), .O(new_n62_));
  nor2   g16(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nor2   g17(.a(x19), .b(x17), .O(new_n64_));
  oai21  g18(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  inv1   g19(.a(x18), .O(new_n66_));
  inv1   g20(.a(x08), .O(new_n67_));
  aoi21  g21(.a(new_n66_), .b(new_n67_), .c(x16), .O(new_n68_));
  oai21  g22(.a(new_n66_), .b(x00), .c(new_n68_), .O(new_n69_));
  aoi21  g23(.a(new_n69_), .b(new_n65_), .c(new_n60_), .O(z09));
  inv1   g24(.a(x20), .O(new_n71_));
  nor2   g25(.a(new_n64_), .b(new_n71_), .O(new_n72_));
  nor3   g26(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  oai21  g27(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  inv1   g28(.a(x09), .O(new_n75_));
  aoi21  g29(.a(new_n66_), .b(new_n75_), .c(x16), .O(new_n76_));
  oai21  g30(.a(new_n66_), .b(x01), .c(new_n76_), .O(new_n77_));
  aoi21  g31(.a(new_n77_), .b(new_n74_), .c(new_n60_), .O(z10));
  inv1   g32(.a(x21), .O(new_n79_));
  nor2   g33(.a(new_n73_), .b(new_n79_), .O(new_n80_));
  nand3  g34(.a(new_n64_), .b(new_n79_), .c(new_n71_), .O(new_n81_));
  inv1   g35(.a(new_n81_), .O(new_n82_));
  oai21  g36(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  inv1   g37(.a(x02), .O(new_n84_));
  nand2  g38(.a(x18), .b(new_n84_), .O(new_n85_));
  inv1   g39(.a(x10), .O(new_n86_));
  aoi21  g40(.a(new_n66_), .b(new_n86_), .c(x16), .O(new_n87_));
  aoi21  g41(.a(new_n87_), .b(new_n85_), .c(new_n60_), .O(new_n88_));
  nand2  g42(.a(new_n88_), .b(new_n83_), .O(z11));
  inv1   g43(.a(new_n60_), .O(new_n90_));
  inv1   g44(.a(x11), .O(new_n91_));
  inv1   g45(.a(x16), .O(new_n92_));
  oai21  g46(.a(x18), .b(new_n91_), .c(new_n92_), .O(new_n93_));
  aoi21  g47(.a(x18), .b(x03), .c(new_n93_), .O(new_n94_));
  nor2   g48(.a(x22), .b(x21), .O(new_n95_));
  nand3  g49(.a(new_n95_), .b(new_n64_), .c(new_n71_), .O(new_n96_));
  nand2  g50(.a(new_n96_), .b(x16), .O(new_n97_));
  aoi21  g51(.a(new_n81_), .b(x22), .c(new_n97_), .O(new_n98_));
  oai21  g52(.a(new_n98_), .b(new_n94_), .c(new_n90_), .O(z12));
  nand2  g53(.a(new_n96_), .b(x23), .O(new_n100_));
  nor2   g54(.a(x23), .b(x22), .O(new_n101_));
  nand3  g55(.a(new_n101_), .b(new_n73_), .c(new_n79_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g58(.a(x12), .O(new_n105_));
  aoi21  g59(.a(new_n66_), .b(new_n105_), .c(x16), .O(new_n106_));
  oai21  g60(.a(new_n66_), .b(x04), .c(new_n106_), .O(new_n107_));
  aoi21  g61(.a(new_n107_), .b(new_n104_), .c(new_n60_), .O(z13));
  nor3   g62(.a(x24), .b(x23), .c(x22), .O(new_n109_));
  aoi22  g63(.a(new_n109_), .b(new_n82_), .c(new_n102_), .d(x24), .O(new_n110_));
  inv1   g64(.a(x05), .O(new_n111_));
  nand2  g65(.a(x18), .b(new_n111_), .O(new_n112_));
  inv1   g66(.a(x13), .O(new_n113_));
  aoi21  g67(.a(new_n66_), .b(new_n113_), .c(x16), .O(new_n114_));
  aoi21  g68(.a(new_n114_), .b(new_n112_), .c(new_n60_), .O(new_n115_));
  oai21  g69(.a(new_n110_), .b(new_n92_), .c(new_n115_), .O(z14));
  nand3  g70(.a(new_n109_), .b(new_n73_), .c(new_n79_), .O(new_n117_));
  inv1   g71(.a(x24), .O(new_n118_));
  inv1   g72(.a(x25), .O(new_n119_));
  nand3  g73(.a(new_n101_), .b(new_n119_), .c(new_n118_), .O(new_n120_));
  nor2   g74(.a(new_n120_), .b(new_n81_), .O(new_n121_));
  aoi21  g75(.a(new_n117_), .b(x25), .c(new_n121_), .O(new_n122_));
  inv1   g76(.a(x06), .O(new_n123_));
  nand2  g77(.a(x18), .b(new_n123_), .O(new_n124_));
  aoi21  g78(.a(new_n66_), .b(new_n48_), .c(x16), .O(new_n125_));
  aoi21  g79(.a(new_n125_), .b(new_n124_), .c(new_n60_), .O(new_n126_));
  oai21  g80(.a(new_n122_), .b(new_n92_), .c(new_n126_), .O(z15));
  oai21  g81(.a(new_n120_), .b(new_n81_), .c(x26), .O(new_n128_));
  nor2   g82(.a(x26), .b(x25), .O(new_n129_));
  nand4  g83(.a(new_n129_), .b(new_n109_), .c(new_n73_), .d(new_n79_), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g85(.a(new_n131_), .b(x16), .O(new_n132_));
  inv1   g86(.a(x07), .O(new_n133_));
  nand2  g87(.a(x18), .b(new_n133_), .O(new_n134_));
  inv1   g88(.a(x15), .O(new_n135_));
  aoi21  g89(.a(new_n66_), .b(new_n135_), .c(x16), .O(new_n136_));
  aoi21  g90(.a(new_n136_), .b(new_n134_), .c(new_n60_), .O(new_n137_));
  nand2  g91(.a(new_n137_), .b(new_n132_), .O(z16));
  nor2   g92(.a(x20), .b(new_n62_), .O(new_n139_));
  nand4  g93(.a(new_n139_), .b(new_n129_), .c(new_n109_), .d(new_n79_), .O(new_n140_));
  oai21  g94(.a(x17), .b(new_n48_), .c(new_n47_), .O(new_n141_));
  nand2  g95(.a(new_n141_), .b(x16), .O(new_n142_));
  aoi21  g96(.a(new_n140_), .b(new_n61_), .c(new_n142_), .O(z17));
  buf    g97(.a(x27), .O(z08));
endmodule


