// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n51_, new_n52_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x16), .O(new_n47_));
  nor2   g01(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi22  g02(.a(new_n48_), .b(x19), .c(x27), .d(x08), .O(z00));
  aoi22  g03(.a(new_n48_), .b(x20), .c(x27), .d(x09), .O(z01));
  inv1   g04(.a(x27), .O(new_n51_));
  inv1   g05(.a(new_n48_), .O(new_n52_));
  oai22  g06(.a(new_n52_), .b(x21), .c(new_n51_), .d(x10), .O(z02));
  oai22  g07(.a(new_n52_), .b(x22), .c(new_n51_), .d(x11), .O(z03));
  aoi22  g08(.a(new_n48_), .b(x23), .c(x27), .d(x12), .O(z04));
  oai22  g09(.a(new_n52_), .b(x24), .c(new_n51_), .d(x13), .O(z05));
  oai22  g10(.a(new_n52_), .b(x25), .c(new_n51_), .d(x14), .O(z06));
  oai22  g11(.a(new_n52_), .b(x26), .c(new_n51_), .d(x15), .O(z07));
  inv1   g12(.a(x18), .O(new_n59_));
  nor2   g13(.a(new_n59_), .b(x00), .O(new_n60_));
  nand2  g14(.a(x27), .b(new_n47_), .O(new_n61_));
  inv1   g15(.a(new_n61_), .O(new_n62_));
  oai21  g16(.a(x18), .b(x08), .c(new_n62_), .O(new_n63_));
  inv1   g17(.a(x17), .O(new_n64_));
  aoi21  g18(.a(x19), .b(new_n64_), .c(new_n47_), .O(new_n65_));
  oai21  g19(.a(x19), .b(new_n64_), .c(new_n65_), .O(new_n66_));
  oai21  g20(.a(new_n63_), .b(new_n60_), .c(new_n66_), .O(z09));
  inv1   g21(.a(x20), .O(new_n68_));
  nor2   g22(.a(x19), .b(x17), .O(new_n69_));
  xor2a  g23(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  inv1   g24(.a(x09), .O(new_n71_));
  nand2  g25(.a(x18), .b(x01), .O(new_n72_));
  oai21  g26(.a(x18), .b(new_n71_), .c(new_n72_), .O(new_n73_));
  oai21  g27(.a(new_n73_), .b(new_n51_), .c(new_n47_), .O(new_n74_));
  oai21  g28(.a(new_n70_), .b(new_n47_), .c(new_n74_), .O(z10));
  nand2  g29(.a(x18), .b(x02), .O(new_n76_));
  aoi21  g30(.a(new_n59_), .b(x10), .c(new_n61_), .O(new_n77_));
  nand2  g31(.a(new_n69_), .b(new_n68_), .O(new_n78_));
  nand2  g32(.a(new_n78_), .b(x21), .O(new_n79_));
  nor2   g33(.a(x21), .b(x20), .O(new_n80_));
  aoi21  g34(.a(new_n80_), .b(new_n69_), .c(new_n47_), .O(new_n81_));
  aoi22  g35(.a(new_n81_), .b(new_n79_), .c(new_n77_), .d(new_n76_), .O(z11));
  nand2  g36(.a(new_n80_), .b(new_n69_), .O(new_n83_));
  xor2a  g37(.a(new_n83_), .b(x22), .O(new_n84_));
  nor2   g38(.a(new_n59_), .b(x03), .O(new_n85_));
  oai21  g39(.a(x18), .b(x11), .c(new_n62_), .O(new_n86_));
  oai22  g40(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n47_), .O(z12));
  inv1   g41(.a(x22), .O(new_n88_));
  nand3  g42(.a(new_n80_), .b(new_n69_), .c(new_n88_), .O(new_n89_));
  nand2  g43(.a(new_n89_), .b(x23), .O(new_n90_));
  nor2   g44(.a(x23), .b(x22), .O(new_n91_));
  nand3  g45(.a(new_n91_), .b(new_n80_), .c(new_n69_), .O(new_n92_));
  nand2  g46(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g47(.a(new_n93_), .b(x16), .O(new_n94_));
  inv1   g48(.a(x12), .O(new_n95_));
  nand2  g49(.a(x18), .b(x04), .O(new_n96_));
  oai21  g50(.a(x18), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  oai21  g51(.a(new_n97_), .b(new_n51_), .c(new_n47_), .O(new_n98_));
  nand2  g52(.a(new_n98_), .b(new_n94_), .O(z13));
  nand2  g53(.a(new_n92_), .b(x24), .O(new_n100_));
  nor2   g54(.a(x24), .b(x21), .O(new_n101_));
  nand4  g55(.a(new_n101_), .b(new_n91_), .c(new_n69_), .d(new_n68_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g58(.a(x13), .O(new_n105_));
  nand2  g59(.a(x18), .b(x05), .O(new_n106_));
  oai21  g60(.a(x18), .b(new_n105_), .c(new_n106_), .O(new_n107_));
  oai21  g61(.a(new_n107_), .b(new_n51_), .c(new_n47_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(new_n104_), .O(z14));
  nand2  g63(.a(x18), .b(x06), .O(new_n110_));
  nand2  g64(.a(new_n59_), .b(x14), .O(new_n111_));
  nand3  g65(.a(new_n111_), .b(new_n110_), .c(new_n62_), .O(new_n112_));
  inv1   g66(.a(new_n112_), .O(new_n113_));
  nand2  g67(.a(new_n102_), .b(x25), .O(new_n114_));
  nor2   g68(.a(x25), .b(x24), .O(new_n115_));
  nand4  g69(.a(new_n115_), .b(new_n91_), .c(new_n80_), .d(new_n69_), .O(new_n116_));
  and2   g70(.a(new_n116_), .b(x16), .O(new_n117_));
  aoi21  g71(.a(new_n117_), .b(new_n114_), .c(new_n113_), .O(z15));
  inv1   g72(.a(x26), .O(new_n119_));
  inv1   g73(.a(new_n116_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  aoi21  g75(.a(new_n116_), .b(x26), .c(new_n47_), .O(new_n122_));
  nand2  g76(.a(x18), .b(x07), .O(new_n123_));
  nand2  g77(.a(new_n59_), .b(x15), .O(new_n124_));
  nand3  g78(.a(new_n124_), .b(new_n123_), .c(new_n62_), .O(new_n125_));
  inv1   g79(.a(new_n125_), .O(new_n126_));
  aoi21  g80(.a(new_n122_), .b(new_n121_), .c(new_n126_), .O(z16));
  nand2  g81(.a(new_n51_), .b(new_n47_), .O(new_n128_));
  nand2  g82(.a(new_n115_), .b(new_n91_), .O(new_n129_));
  nand2  g83(.a(x27), .b(x17), .O(new_n130_));
  nand4  g84(.a(new_n80_), .b(new_n119_), .c(x19), .d(new_n64_), .O(new_n131_));
  oai21  g85(.a(new_n131_), .b(new_n129_), .c(new_n130_), .O(new_n132_));
  nand2  g86(.a(new_n132_), .b(x16), .O(new_n133_));
  nand2  g87(.a(new_n133_), .b(new_n128_), .O(z17));
  buf    g88(.a(x27), .O(z08));
endmodule


