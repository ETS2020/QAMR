// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(x27), .b(x01), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi22  g003(.a(new_n49_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  aoi22  g004(.a(new_n49_), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  oai22  g005(.a(new_n48_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g006(.a(new_n49_), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  aoi22  g007(.a(new_n49_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  aoi22  g008(.a(new_n49_), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  oai22  g009(.a(new_n48_), .b(x14), .c(x27), .d(x25), .O(z06));
  oai22  g010(.a(new_n48_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g011(.a(new_n47_), .b(x01), .O(new_n58_));
  inv1   g012(.a(x18), .O(new_n59_));
  inv1   g013(.a(x08), .O(new_n60_));
  aoi21  g014(.a(new_n59_), .b(new_n60_), .c(x16), .O(new_n61_));
  oai21  g015(.a(new_n59_), .b(x00), .c(new_n61_), .O(new_n62_));
  inv1   g016(.a(x17), .O(new_n63_));
  inv1   g017(.a(x16), .O(new_n64_));
  aoi21  g018(.a(x19), .b(new_n63_), .c(new_n64_), .O(new_n65_));
  oai21  g019(.a(x19), .b(new_n63_), .c(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n62_), .c(new_n58_), .O(z09));
  inv1   g021(.a(new_n58_), .O(new_n68_));
  inv1   g022(.a(x19), .O(new_n69_));
  nand2  g023(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nand2  g024(.a(new_n70_), .b(x20), .O(new_n71_));
  inv1   g025(.a(x20), .O(new_n72_));
  nor2   g026(.a(x19), .b(x17), .O(new_n73_));
  nand2  g027(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g028(.a(new_n74_), .b(new_n71_), .c(x16), .O(new_n75_));
  nand2  g029(.a(x18), .b(x01), .O(new_n76_));
  nand2  g030(.a(new_n59_), .b(x09), .O(new_n77_));
  nand3  g031(.a(new_n77_), .b(new_n76_), .c(new_n64_), .O(new_n78_));
  nand3  g032(.a(new_n78_), .b(new_n75_), .c(new_n68_), .O(new_n79_));
  inv1   g033(.a(new_n79_), .O(z10));
  nor2   g034(.a(x21), .b(x20), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n73_), .O(new_n82_));
  inv1   g036(.a(new_n82_), .O(new_n83_));
  aoi21  g037(.a(new_n74_), .b(x21), .c(new_n83_), .O(new_n84_));
  inv1   g038(.a(x02), .O(new_n85_));
  nand2  g039(.a(x18), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x10), .O(new_n87_));
  aoi21  g041(.a(new_n59_), .b(new_n87_), .c(x16), .O(new_n88_));
  aoi21  g042(.a(new_n88_), .b(new_n86_), .c(new_n58_), .O(new_n89_));
  oai21  g043(.a(new_n84_), .b(new_n64_), .c(new_n89_), .O(z11));
  inv1   g044(.a(x22), .O(new_n91_));
  aoi21  g045(.a(new_n81_), .b(new_n73_), .c(new_n91_), .O(new_n92_));
  nand3  g046(.a(new_n81_), .b(new_n73_), .c(new_n91_), .O(new_n93_));
  inv1   g047(.a(new_n93_), .O(new_n94_));
  oai21  g048(.a(new_n94_), .b(new_n92_), .c(x16), .O(new_n95_));
  inv1   g049(.a(x11), .O(new_n96_));
  aoi21  g050(.a(new_n59_), .b(new_n96_), .c(x16), .O(new_n97_));
  oai21  g051(.a(new_n59_), .b(x03), .c(new_n97_), .O(new_n98_));
  aoi21  g052(.a(new_n98_), .b(new_n95_), .c(new_n58_), .O(z12));
  inv1   g053(.a(x23), .O(new_n100_));
  xor2a  g054(.a(new_n93_), .b(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(x16), .O(new_n102_));
  inv1   g056(.a(x12), .O(new_n103_));
  aoi21  g057(.a(new_n59_), .b(new_n103_), .c(x16), .O(new_n104_));
  oai21  g058(.a(new_n59_), .b(x04), .c(new_n104_), .O(new_n105_));
  aoi21  g059(.a(new_n105_), .b(new_n102_), .c(new_n58_), .O(z13));
  inv1   g060(.a(new_n74_), .O(new_n107_));
  nand4  g061(.a(new_n81_), .b(new_n73_), .c(new_n100_), .d(new_n91_), .O(new_n108_));
  nor2   g062(.a(x24), .b(x23), .O(new_n109_));
  nor2   g063(.a(x22), .b(x21), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  inv1   g065(.a(new_n111_), .O(new_n112_));
  aoi22  g066(.a(new_n112_), .b(new_n107_), .c(new_n108_), .d(x24), .O(new_n113_));
  inv1   g067(.a(x05), .O(new_n114_));
  nand2  g068(.a(x18), .b(new_n114_), .O(new_n115_));
  inv1   g069(.a(x13), .O(new_n116_));
  aoi21  g070(.a(new_n59_), .b(new_n116_), .c(x16), .O(new_n117_));
  aoi21  g071(.a(new_n117_), .b(new_n115_), .c(new_n58_), .O(new_n118_));
  oai21  g072(.a(new_n113_), .b(new_n64_), .c(new_n118_), .O(z14));
  oai21  g073(.a(new_n111_), .b(new_n74_), .c(x25), .O(new_n120_));
  nor2   g074(.a(x25), .b(x22), .O(new_n121_));
  nand4  g075(.a(new_n121_), .b(new_n109_), .c(new_n81_), .d(new_n73_), .O(new_n122_));
  and2   g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand2  g077(.a(x18), .b(x06), .O(new_n124_));
  nand2  g078(.a(new_n59_), .b(x14), .O(new_n125_));
  nand3  g079(.a(new_n125_), .b(new_n124_), .c(new_n64_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n68_), .O(new_n127_));
  aoi21  g081(.a(new_n123_), .b(new_n120_), .c(new_n127_), .O(z15));
  nand2  g082(.a(new_n121_), .b(new_n109_), .O(new_n129_));
  inv1   g083(.a(new_n129_), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n83_), .c(x26), .O(new_n131_));
  inv1   g085(.a(x26), .O(new_n132_));
  aoi21  g086(.a(new_n122_), .b(new_n132_), .c(new_n64_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  inv1   g088(.a(x15), .O(new_n135_));
  aoi21  g089(.a(new_n59_), .b(new_n135_), .c(x16), .O(new_n136_));
  oai21  g090(.a(new_n59_), .b(x07), .c(new_n136_), .O(new_n137_));
  aoi21  g091(.a(new_n137_), .b(new_n134_), .c(new_n58_), .O(z16));
  nand2  g092(.a(x27), .b(x17), .O(new_n139_));
  nand3  g093(.a(new_n132_), .b(x19), .c(new_n63_), .O(new_n140_));
  nand3  g094(.a(new_n121_), .b(new_n109_), .c(new_n81_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n140_), .c(new_n139_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x01), .O(new_n143_));
  inv1   g097(.a(new_n140_), .O(new_n144_));
  nor3   g098(.a(x27), .b(x25), .c(x20), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n144_), .c(new_n112_), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n143_), .c(new_n64_), .O(z17));
  buf    g101(.a(x27), .O(z08));
endmodule


