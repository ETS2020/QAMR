// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x26), .O(new_n48_));
  nor2   g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(new_n49_), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g005(.a(new_n50_), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  oai22  g006(.a(new_n50_), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  aoi22  g007(.a(new_n49_), .b(x22), .c(x27), .d(x11), .O(z03));
  oai22  g008(.a(new_n50_), .b(x23), .c(new_n47_), .d(x12), .O(z04));
  aoi22  g009(.a(new_n49_), .b(x24), .c(x27), .d(x13), .O(z05));
  aoi22  g010(.a(new_n49_), .b(x25), .c(x27), .d(x14), .O(z06));
  aoi21  g011(.a(x27), .b(x15), .c(new_n49_), .O(z07));
  inv1   g012(.a(x18), .O(new_n59_));
  inv1   g013(.a(x08), .O(new_n60_));
  aoi21  g014(.a(new_n59_), .b(new_n60_), .c(x16), .O(new_n61_));
  oai21  g015(.a(new_n59_), .b(x00), .c(new_n61_), .O(new_n62_));
  inv1   g016(.a(x17), .O(new_n63_));
  inv1   g017(.a(x19), .O(new_n64_));
  nor2   g018(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor2   g019(.a(x19), .b(x17), .O(new_n66_));
  oai21  g020(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nor2   g021(.a(x27), .b(x26), .O(new_n68_));
  inv1   g022(.a(new_n68_), .O(new_n69_));
  nand3  g023(.a(new_n69_), .b(new_n67_), .c(new_n62_), .O(z09));
  oai21  g024(.a(x19), .b(x17), .c(x20), .O(new_n71_));
  inv1   g025(.a(x20), .O(new_n72_));
  nand3  g026(.a(new_n72_), .b(new_n64_), .c(new_n63_), .O(new_n73_));
  nand2  g027(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g028(.a(new_n74_), .b(x16), .O(new_n75_));
  inv1   g029(.a(x01), .O(new_n76_));
  nand2  g030(.a(x18), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x09), .O(new_n78_));
  aoi21  g032(.a(new_n59_), .b(new_n78_), .c(x16), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n77_), .c(new_n68_), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n75_), .O(z10));
  inv1   g035(.a(x21), .O(new_n82_));
  aoi21  g036(.a(new_n66_), .b(new_n72_), .c(new_n82_), .O(new_n83_));
  nor2   g037(.a(x21), .b(x20), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n66_), .O(new_n85_));
  inv1   g039(.a(new_n85_), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n83_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x10), .O(new_n88_));
  aoi21  g042(.a(new_n59_), .b(new_n88_), .c(x16), .O(new_n89_));
  oai21  g043(.a(new_n59_), .b(x02), .c(new_n89_), .O(new_n90_));
  aoi21  g044(.a(new_n90_), .b(new_n87_), .c(new_n68_), .O(z11));
  inv1   g045(.a(x16), .O(new_n92_));
  xor2a  g046(.a(new_n85_), .b(x22), .O(new_n93_));
  inv1   g047(.a(x03), .O(new_n94_));
  nand2  g048(.a(x18), .b(new_n94_), .O(new_n95_));
  inv1   g049(.a(x11), .O(new_n96_));
  aoi21  g050(.a(new_n59_), .b(new_n96_), .c(x16), .O(new_n97_));
  aoi21  g051(.a(new_n97_), .b(new_n95_), .c(new_n68_), .O(new_n98_));
  oai21  g052(.a(new_n93_), .b(new_n92_), .c(new_n98_), .O(z12));
  oai21  g053(.a(new_n85_), .b(x22), .c(x23), .O(new_n100_));
  nor2   g054(.a(x23), .b(x22), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n84_), .c(new_n66_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g058(.a(x04), .O(new_n105_));
  nand2  g059(.a(x18), .b(new_n105_), .O(new_n106_));
  inv1   g060(.a(x12), .O(new_n107_));
  aoi21  g061(.a(new_n59_), .b(new_n107_), .c(x16), .O(new_n108_));
  aoi21  g062(.a(new_n108_), .b(new_n106_), .c(new_n68_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n104_), .O(z13));
  nand2  g064(.a(new_n102_), .b(x24), .O(new_n111_));
  nor2   g065(.a(x24), .b(x21), .O(new_n112_));
  nand4  g066(.a(new_n112_), .b(new_n101_), .c(new_n66_), .d(new_n72_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g069(.a(x13), .O(new_n116_));
  aoi21  g070(.a(new_n59_), .b(new_n116_), .c(x16), .O(new_n117_));
  oai21  g071(.a(new_n59_), .b(x05), .c(new_n117_), .O(new_n118_));
  aoi21  g072(.a(new_n118_), .b(new_n115_), .c(new_n68_), .O(z14));
  nor2   g073(.a(x25), .b(x24), .O(new_n120_));
  nand4  g074(.a(new_n120_), .b(new_n101_), .c(new_n84_), .d(new_n66_), .O(new_n121_));
  inv1   g075(.a(new_n121_), .O(new_n122_));
  aoi21  g076(.a(new_n113_), .b(x25), .c(new_n122_), .O(new_n123_));
  inv1   g077(.a(x06), .O(new_n124_));
  nand2  g078(.a(x18), .b(new_n124_), .O(new_n125_));
  inv1   g079(.a(x14), .O(new_n126_));
  aoi21  g080(.a(new_n59_), .b(new_n126_), .c(x16), .O(new_n127_));
  aoi21  g081(.a(new_n127_), .b(new_n125_), .c(new_n68_), .O(new_n128_));
  oai21  g082(.a(new_n123_), .b(new_n92_), .c(new_n128_), .O(z15));
  inv1   g083(.a(x22), .O(new_n130_));
  inv1   g084(.a(x23), .O(new_n131_));
  inv1   g085(.a(x24), .O(new_n132_));
  nand4  g086(.a(new_n132_), .b(new_n131_), .c(new_n130_), .d(new_n82_), .O(new_n133_));
  nor2   g087(.a(new_n133_), .b(new_n73_), .O(new_n134_));
  nor2   g088(.a(x26), .b(x25), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(x27), .O(new_n136_));
  inv1   g090(.a(new_n136_), .O(new_n137_));
  aoi22  g091(.a(new_n137_), .b(new_n134_), .c(new_n121_), .d(x26), .O(new_n138_));
  inv1   g092(.a(x07), .O(new_n139_));
  nand2  g093(.a(x18), .b(new_n139_), .O(new_n140_));
  inv1   g094(.a(x15), .O(new_n141_));
  nand2  g095(.a(new_n59_), .b(new_n141_), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n140_), .c(new_n69_), .d(new_n92_), .O(new_n143_));
  oai21  g097(.a(new_n138_), .b(new_n92_), .c(new_n143_), .O(z16));
  inv1   g098(.a(new_n133_), .O(new_n145_));
  nand4  g099(.a(new_n135_), .b(new_n145_), .c(new_n72_), .d(x19), .O(new_n146_));
  nand2  g100(.a(x27), .b(x16), .O(new_n147_));
  aoi21  g101(.a(new_n146_), .b(new_n63_), .c(new_n147_), .O(z17));
  buf    g102(.a(x27), .O(z08));
endmodule


