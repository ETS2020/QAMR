// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n50_, new_n54_, new_n55_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_;
  nor2   g000(.a(x27), .b(x23), .O(new_n47_));
  aoi22  g001(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g002(.a(x27), .O(new_n49_));
  inv1   g003(.a(new_n47_), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x20), .c(new_n49_), .d(x09), .O(z01));
  oai22  g005(.a(new_n50_), .b(x21), .c(new_n49_), .d(x10), .O(z02));
  aoi22  g006(.a(new_n47_), .b(x22), .c(x27), .d(x11), .O(z03));
  inv1   g007(.a(x23), .O(new_n54_));
  nor2   g008(.a(x27), .b(new_n54_), .O(new_n55_));
  aoi21  g009(.a(x27), .b(x12), .c(new_n55_), .O(z04));
  oai22  g010(.a(new_n50_), .b(x24), .c(new_n49_), .d(x13), .O(z05));
  aoi22  g011(.a(new_n47_), .b(x25), .c(x27), .d(x14), .O(z06));
  aoi22  g012(.a(new_n47_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g013(.a(x18), .O(new_n60_));
  inv1   g014(.a(x08), .O(new_n61_));
  aoi21  g015(.a(new_n60_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g016(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g017(.a(x19), .O(new_n64_));
  nand2  g018(.a(new_n64_), .b(x17), .O(new_n65_));
  inv1   g019(.a(x16), .O(new_n66_));
  nor2   g020(.a(new_n64_), .b(x17), .O(new_n67_));
  nor2   g021(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n63_), .c(new_n55_), .O(z09));
  inv1   g024(.a(x20), .O(new_n71_));
  nor2   g025(.a(x19), .b(x17), .O(new_n72_));
  xor2a  g026(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g027(.a(x01), .O(new_n74_));
  nand2  g028(.a(x18), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x09), .O(new_n76_));
  aoi21  g030(.a(new_n60_), .b(new_n76_), .c(x16), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n55_), .O(new_n78_));
  oai21  g032(.a(new_n73_), .b(new_n66_), .c(new_n78_), .O(z10));
  nand2  g033(.a(new_n72_), .b(new_n71_), .O(new_n80_));
  nor2   g034(.a(x21), .b(x20), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n72_), .O(new_n82_));
  inv1   g036(.a(new_n82_), .O(new_n83_));
  aoi21  g037(.a(new_n80_), .b(x21), .c(new_n83_), .O(new_n84_));
  inv1   g038(.a(x02), .O(new_n85_));
  nand2  g039(.a(x18), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x10), .O(new_n87_));
  aoi21  g041(.a(new_n60_), .b(new_n87_), .c(x16), .O(new_n88_));
  aoi21  g042(.a(new_n88_), .b(new_n86_), .c(new_n55_), .O(new_n89_));
  oai21  g043(.a(new_n84_), .b(new_n66_), .c(new_n89_), .O(z11));
  inv1   g044(.a(x22), .O(new_n91_));
  nor2   g045(.a(new_n83_), .b(new_n91_), .O(new_n92_));
  nand3  g046(.a(new_n81_), .b(new_n72_), .c(new_n91_), .O(new_n93_));
  inv1   g047(.a(new_n93_), .O(new_n94_));
  oai21  g048(.a(new_n94_), .b(new_n92_), .c(x16), .O(new_n95_));
  inv1   g049(.a(x03), .O(new_n96_));
  nand2  g050(.a(x18), .b(new_n96_), .O(new_n97_));
  inv1   g051(.a(x11), .O(new_n98_));
  aoi21  g052(.a(new_n60_), .b(new_n98_), .c(x16), .O(new_n99_));
  aoi21  g053(.a(new_n99_), .b(new_n97_), .c(new_n55_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n95_), .O(z12));
  nand3  g055(.a(new_n93_), .b(x27), .c(x23), .O(new_n102_));
  nand4  g056(.a(new_n81_), .b(new_n72_), .c(new_n54_), .d(new_n91_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(x16), .O(new_n105_));
  nor2   g059(.a(new_n55_), .b(x16), .O(new_n106_));
  inv1   g060(.a(x12), .O(new_n107_));
  nand2  g061(.a(new_n60_), .b(new_n107_), .O(new_n108_));
  inv1   g062(.a(x04), .O(new_n109_));
  nand2  g063(.a(x18), .b(new_n109_), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n105_), .O(z13));
  nand2  g066(.a(new_n103_), .b(x24), .O(new_n113_));
  nor2   g067(.a(x24), .b(x22), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n83_), .c(new_n54_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x16), .O(new_n117_));
  inv1   g071(.a(x13), .O(new_n118_));
  aoi21  g072(.a(new_n60_), .b(new_n118_), .c(x16), .O(new_n119_));
  oai21  g073(.a(new_n60_), .b(x05), .c(new_n119_), .O(new_n120_));
  aoi21  g074(.a(new_n120_), .b(new_n117_), .c(new_n55_), .O(z14));
  inv1   g075(.a(x25), .O(new_n122_));
  nand2  g076(.a(new_n115_), .b(new_n122_), .O(new_n123_));
  nand3  g077(.a(new_n114_), .b(new_n83_), .c(x25), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n123_), .c(x16), .O(new_n125_));
  nand2  g079(.a(x25), .b(x16), .O(new_n126_));
  aoi21  g080(.a(new_n126_), .b(x27), .c(new_n54_), .O(new_n127_));
  inv1   g081(.a(x06), .O(new_n128_));
  nand2  g082(.a(x18), .b(new_n128_), .O(new_n129_));
  inv1   g083(.a(x14), .O(new_n130_));
  aoi21  g084(.a(new_n60_), .b(new_n130_), .c(x16), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(new_n129_), .c(new_n127_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n125_), .O(z15));
  nor2   g087(.a(x25), .b(x21), .O(new_n134_));
  nand4  g088(.a(new_n134_), .b(new_n114_), .c(new_n72_), .d(new_n71_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(x26), .O(new_n136_));
  nor2   g090(.a(x26), .b(x25), .O(new_n137_));
  nand4  g091(.a(new_n137_), .b(new_n114_), .c(new_n81_), .d(new_n72_), .O(new_n138_));
  aoi21  g092(.a(new_n138_), .b(new_n136_), .c(x23), .O(new_n139_));
  nand3  g093(.a(x27), .b(x26), .c(x23), .O(new_n140_));
  inv1   g094(.a(new_n140_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n139_), .c(x16), .O(new_n142_));
  inv1   g096(.a(x15), .O(new_n143_));
  nand2  g097(.a(new_n60_), .b(new_n143_), .O(new_n144_));
  inv1   g098(.a(x07), .O(new_n145_));
  nand2  g099(.a(x18), .b(new_n145_), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n144_), .c(new_n106_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n142_), .O(z16));
  nor3   g102(.a(x24), .b(x23), .c(x22), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n137_), .c(new_n81_), .d(new_n67_), .O(new_n150_));
  aoi21  g104(.a(x27), .b(x17), .c(new_n55_), .O(new_n151_));
  aoi21  g105(.a(new_n151_), .b(new_n150_), .c(new_n106_), .O(z17));
  buf    g106(.a(x27), .O(z08));
endmodule


