// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(x27), .b(x07), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(z08));
  oai22  g03(.a(z08), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  aoi22  g04(.a(new_n48_), .b(x20), .c(x27), .d(x09), .O(z01));
  oai22  g05(.a(z08), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  oai22  g06(.a(z08), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  aoi22  g07(.a(new_n48_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi22  g08(.a(new_n48_), .b(x24), .c(x27), .d(x13), .O(z05));
  aoi22  g09(.a(new_n48_), .b(x25), .c(x27), .d(x14), .O(z06));
  aoi22  g10(.a(new_n48_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g11(.a(x18), .O(new_n58_));
  inv1   g12(.a(x08), .O(new_n59_));
  aoi21  g13(.a(new_n58_), .b(new_n59_), .c(x16), .O(new_n60_));
  oai21  g14(.a(new_n58_), .b(x00), .c(new_n60_), .O(new_n61_));
  inv1   g15(.a(x17), .O(new_n62_));
  inv1   g16(.a(x19), .O(new_n63_));
  nor2   g17(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nor2   g18(.a(x19), .b(x17), .O(new_n65_));
  oai21  g19(.a(new_n65_), .b(new_n64_), .c(x16), .O(new_n66_));
  inv1   g20(.a(x07), .O(new_n67_));
  nor2   g21(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g22(.a(new_n68_), .O(new_n69_));
  nand3  g23(.a(new_n69_), .b(new_n66_), .c(new_n61_), .O(z09));
  inv1   g24(.a(x20), .O(new_n71_));
  nor2   g25(.a(new_n65_), .b(new_n71_), .O(new_n72_));
  nor3   g26(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  oai21  g27(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  inv1   g28(.a(x01), .O(new_n75_));
  nand2  g29(.a(x18), .b(new_n75_), .O(new_n76_));
  inv1   g30(.a(x09), .O(new_n77_));
  aoi21  g31(.a(new_n58_), .b(new_n77_), .c(x16), .O(new_n78_));
  aoi21  g32(.a(new_n78_), .b(new_n76_), .c(new_n68_), .O(new_n79_));
  nand2  g33(.a(new_n79_), .b(new_n74_), .O(z10));
  inv1   g34(.a(x21), .O(new_n81_));
  nor2   g35(.a(new_n73_), .b(new_n81_), .O(new_n82_));
  nand4  g36(.a(new_n81_), .b(new_n71_), .c(new_n63_), .d(new_n62_), .O(new_n83_));
  inv1   g37(.a(new_n83_), .O(new_n84_));
  oai21  g38(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  inv1   g39(.a(x02), .O(new_n86_));
  nand2  g40(.a(x18), .b(new_n86_), .O(new_n87_));
  inv1   g41(.a(x10), .O(new_n88_));
  aoi21  g42(.a(new_n58_), .b(new_n88_), .c(x16), .O(new_n89_));
  aoi21  g43(.a(new_n89_), .b(new_n87_), .c(new_n68_), .O(new_n90_));
  nand2  g44(.a(new_n90_), .b(new_n85_), .O(z11));
  inv1   g45(.a(x22), .O(new_n92_));
  xor2a  g46(.a(new_n83_), .b(new_n92_), .O(new_n93_));
  nand2  g47(.a(new_n93_), .b(x16), .O(new_n94_));
  inv1   g48(.a(x11), .O(new_n95_));
  aoi21  g49(.a(new_n58_), .b(new_n95_), .c(x16), .O(new_n96_));
  oai21  g50(.a(new_n58_), .b(x03), .c(new_n96_), .O(new_n97_));
  aoi21  g51(.a(new_n97_), .b(new_n94_), .c(new_n68_), .O(z12));
  inv1   g52(.a(x16), .O(new_n99_));
  nand2  g53(.a(new_n84_), .b(new_n92_), .O(new_n100_));
  inv1   g54(.a(x23), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(new_n92_), .O(new_n102_));
  nor2   g56(.a(new_n102_), .b(new_n83_), .O(new_n103_));
  aoi21  g57(.a(new_n100_), .b(x23), .c(new_n103_), .O(new_n104_));
  inv1   g58(.a(x04), .O(new_n105_));
  nand2  g59(.a(x18), .b(new_n105_), .O(new_n106_));
  inv1   g60(.a(x12), .O(new_n107_));
  aoi21  g61(.a(new_n58_), .b(new_n107_), .c(x16), .O(new_n108_));
  aoi21  g62(.a(new_n108_), .b(new_n106_), .c(new_n68_), .O(new_n109_));
  oai21  g63(.a(new_n104_), .b(new_n99_), .c(new_n109_), .O(z13));
  nand3  g64(.a(new_n84_), .b(new_n101_), .c(new_n92_), .O(new_n111_));
  inv1   g65(.a(x24), .O(new_n112_));
  nand3  g66(.a(new_n112_), .b(new_n101_), .c(new_n92_), .O(new_n113_));
  nor2   g67(.a(new_n113_), .b(new_n83_), .O(new_n114_));
  aoi21  g68(.a(new_n111_), .b(x24), .c(new_n114_), .O(new_n115_));
  inv1   g69(.a(x05), .O(new_n116_));
  nand2  g70(.a(x18), .b(new_n116_), .O(new_n117_));
  inv1   g71(.a(x13), .O(new_n118_));
  aoi21  g72(.a(new_n58_), .b(new_n118_), .c(x16), .O(new_n119_));
  aoi21  g73(.a(new_n119_), .b(new_n117_), .c(new_n68_), .O(new_n120_));
  oai21  g74(.a(new_n115_), .b(new_n99_), .c(new_n120_), .O(z14));
  oai21  g75(.a(new_n113_), .b(new_n83_), .c(x25), .O(new_n122_));
  inv1   g76(.a(x25), .O(new_n123_));
  nor3   g77(.a(x24), .b(x23), .c(x22), .O(new_n124_));
  nand4  g78(.a(new_n124_), .b(new_n73_), .c(new_n123_), .d(new_n81_), .O(new_n125_));
  nand2  g79(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand2  g80(.a(new_n126_), .b(x16), .O(new_n127_));
  inv1   g81(.a(x14), .O(new_n128_));
  aoi21  g82(.a(new_n58_), .b(new_n128_), .c(x16), .O(new_n129_));
  oai21  g83(.a(new_n58_), .b(x06), .c(new_n129_), .O(new_n130_));
  aoi21  g84(.a(new_n130_), .b(new_n127_), .c(new_n68_), .O(z15));
  nand2  g85(.a(new_n125_), .b(x26), .O(new_n132_));
  nor2   g86(.a(x26), .b(x25), .O(new_n133_));
  aoi21  g87(.a(new_n133_), .b(new_n114_), .c(new_n99_), .O(new_n134_));
  aoi21  g88(.a(new_n58_), .b(x15), .c(x16), .O(new_n135_));
  oai21  g89(.a(new_n58_), .b(new_n67_), .c(new_n135_), .O(new_n136_));
  nand2  g90(.a(new_n136_), .b(new_n69_), .O(new_n137_));
  aoi21  g91(.a(new_n134_), .b(new_n132_), .c(new_n137_), .O(z16));
  nor2   g92(.a(x20), .b(new_n63_), .O(new_n139_));
  nand4  g93(.a(new_n139_), .b(new_n133_), .c(new_n124_), .d(new_n81_), .O(new_n140_));
  nor2   g94(.a(x17), .b(x07), .O(new_n141_));
  oai21  g95(.a(new_n141_), .b(x27), .c(x16), .O(new_n142_));
  aoi21  g96(.a(new_n140_), .b(new_n62_), .c(new_n142_), .O(z17));
endmodule


