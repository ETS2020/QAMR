// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x20), .O(z08));
  inv1   g002(.a(z08), .O(new_n49_));
  oai22  g003(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  inv1   g004(.a(x20), .O(new_n51_));
  nor2   g005(.a(x27), .b(new_n51_), .O(new_n52_));
  inv1   g006(.a(x09), .O(new_n53_));
  nor2   g007(.a(x20), .b(new_n53_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(x27), .c(new_n52_), .O(z01));
  aoi22  g009(.a(z08), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  aoi22  g010(.a(z08), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  oai22  g011(.a(new_n49_), .b(x12), .c(x27), .d(x23), .O(z04));
  aoi22  g012(.a(z08), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  aoi22  g013(.a(z08), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  aoi22  g014(.a(z08), .b(x15), .c(new_n47_), .d(x26), .O(z07));
  nor2   g015(.a(new_n47_), .b(new_n51_), .O(new_n62_));
  inv1   g016(.a(x18), .O(new_n63_));
  inv1   g017(.a(x08), .O(new_n64_));
  aoi21  g018(.a(new_n63_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g019(.a(new_n63_), .b(x00), .c(new_n65_), .O(new_n66_));
  inv1   g020(.a(x17), .O(new_n67_));
  inv1   g021(.a(x16), .O(new_n68_));
  inv1   g022(.a(x19), .O(new_n69_));
  nor2   g023(.a(new_n69_), .b(x17), .O(new_n70_));
  nor2   g024(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  oai21  g025(.a(x19), .b(new_n67_), .c(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n66_), .c(new_n62_), .O(z09));
  nand2  g027(.a(new_n69_), .b(new_n67_), .O(new_n74_));
  nor2   g028(.a(x19), .b(x17), .O(new_n75_));
  nand2  g029(.a(new_n75_), .b(new_n51_), .O(new_n76_));
  inv1   g030(.a(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n74_), .b(new_n52_), .c(new_n77_), .O(new_n78_));
  nor2   g032(.a(new_n62_), .b(x16), .O(new_n79_));
  nand2  g033(.a(new_n63_), .b(new_n53_), .O(new_n80_));
  inv1   g034(.a(x01), .O(new_n81_));
  nand2  g035(.a(x18), .b(new_n81_), .O(new_n82_));
  nand3  g036(.a(new_n82_), .b(new_n80_), .c(new_n79_), .O(new_n83_));
  oai21  g037(.a(new_n78_), .b(new_n68_), .c(new_n83_), .O(z10));
  oai21  g038(.a(new_n47_), .b(new_n51_), .c(x16), .O(new_n85_));
  xor2a  g039(.a(new_n76_), .b(x21), .O(new_n86_));
  inv1   g040(.a(x10), .O(new_n87_));
  nand2  g041(.a(new_n63_), .b(new_n87_), .O(new_n88_));
  inv1   g042(.a(x02), .O(new_n89_));
  nand2  g043(.a(x18), .b(new_n89_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n88_), .c(new_n79_), .O(new_n91_));
  oai21  g045(.a(new_n86_), .b(new_n85_), .c(new_n91_), .O(z11));
  inv1   g046(.a(x11), .O(new_n93_));
  nand2  g047(.a(new_n63_), .b(new_n93_), .O(new_n94_));
  inv1   g048(.a(x03), .O(new_n95_));
  nand2  g049(.a(x18), .b(new_n95_), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n94_), .c(new_n79_), .O(new_n97_));
  inv1   g051(.a(x22), .O(new_n98_));
  inv1   g052(.a(x21), .O(new_n99_));
  nand3  g053(.a(new_n75_), .b(new_n99_), .c(new_n51_), .O(new_n100_));
  aoi21  g054(.a(new_n100_), .b(new_n98_), .c(new_n85_), .O(new_n101_));
  oai21  g055(.a(new_n100_), .b(new_n98_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n97_), .O(z12));
  inv1   g057(.a(x12), .O(new_n104_));
  nand2  g058(.a(new_n63_), .b(new_n104_), .O(new_n105_));
  inv1   g059(.a(x04), .O(new_n106_));
  nand2  g060(.a(x18), .b(new_n106_), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n105_), .c(new_n79_), .O(new_n108_));
  inv1   g062(.a(x23), .O(new_n109_));
  nor2   g063(.a(x22), .b(x21), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n75_), .c(new_n51_), .O(new_n111_));
  aoi21  g065(.a(new_n111_), .b(new_n109_), .c(new_n85_), .O(new_n112_));
  oai21  g066(.a(new_n111_), .b(new_n109_), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n108_), .O(z13));
  nand3  g068(.a(new_n110_), .b(new_n75_), .c(new_n109_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(x24), .O(new_n116_));
  nor2   g070(.a(x24), .b(x23), .O(new_n117_));
  nand4  g071(.a(new_n117_), .b(new_n110_), .c(new_n75_), .d(new_n51_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x16), .O(new_n120_));
  nand2  g074(.a(x24), .b(x16), .O(new_n121_));
  aoi21  g075(.a(new_n121_), .b(new_n47_), .c(new_n51_), .O(new_n122_));
  inv1   g076(.a(x05), .O(new_n123_));
  nand2  g077(.a(x18), .b(new_n123_), .O(new_n124_));
  inv1   g078(.a(x13), .O(new_n125_));
  aoi21  g079(.a(new_n63_), .b(new_n125_), .c(x16), .O(new_n126_));
  aoi21  g080(.a(new_n126_), .b(new_n124_), .c(new_n122_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n120_), .O(z14));
  nor3   g082(.a(x25), .b(x22), .c(x21), .O(new_n129_));
  nand4  g083(.a(new_n129_), .b(new_n117_), .c(new_n75_), .d(new_n51_), .O(new_n130_));
  nand3  g084(.a(new_n117_), .b(new_n110_), .c(new_n75_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(x25), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x16), .O(new_n134_));
  nand2  g088(.a(x25), .b(x16), .O(new_n135_));
  aoi21  g089(.a(new_n135_), .b(new_n47_), .c(new_n51_), .O(new_n136_));
  inv1   g090(.a(x06), .O(new_n137_));
  nand2  g091(.a(x18), .b(new_n137_), .O(new_n138_));
  inv1   g092(.a(x14), .O(new_n139_));
  aoi21  g093(.a(new_n63_), .b(new_n139_), .c(x16), .O(new_n140_));
  aoi21  g094(.a(new_n140_), .b(new_n138_), .c(new_n136_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n134_), .O(z15));
  inv1   g096(.a(x25), .O(new_n143_));
  nand4  g097(.a(new_n117_), .b(new_n110_), .c(new_n75_), .d(new_n143_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x26), .O(new_n145_));
  nor2   g099(.a(x26), .b(x25), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n117_), .c(new_n110_), .d(new_n75_), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n145_), .c(new_n51_), .O(new_n148_));
  nand2  g102(.a(new_n47_), .b(x26), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(x20), .c(new_n68_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  inv1   g105(.a(x15), .O(new_n152_));
  nand2  g106(.a(new_n63_), .b(new_n152_), .O(new_n153_));
  inv1   g107(.a(x07), .O(new_n154_));
  nand2  g108(.a(x18), .b(new_n154_), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n153_), .c(new_n79_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n151_), .O(z16));
  nand2  g111(.a(x27), .b(x17), .O(new_n158_));
  nand4  g112(.a(new_n146_), .b(new_n117_), .c(new_n110_), .d(new_n70_), .O(new_n159_));
  nand2  g113(.a(new_n51_), .b(x16), .O(new_n160_));
  aoi21  g114(.a(new_n159_), .b(new_n158_), .c(new_n160_), .O(z17));
endmodule


