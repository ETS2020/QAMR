// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n57_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x14), .O(new_n48_));
  nor2   g02(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g03(.a(new_n49_), .O(z08));
  oai22  g04(.a(z08), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g05(.a(z08), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  oai22  g06(.a(z08), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  oai22  g07(.a(z08), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  oai22  g08(.a(z08), .b(x23), .c(new_n47_), .d(x12), .O(z04));
  aoi22  g09(.a(new_n49_), .b(x24), .c(x27), .d(x13), .O(z05));
  nand2  g10(.a(x27), .b(new_n48_), .O(new_n57_));
  oai21  g11(.a(z08), .b(x25), .c(new_n57_), .O(z06));
  aoi22  g12(.a(new_n49_), .b(x26), .c(x27), .d(x15), .O(z07));
  nor2   g13(.a(x27), .b(x14), .O(new_n60_));
  and2   g14(.a(x19), .b(x17), .O(new_n61_));
  nor2   g15(.a(x19), .b(x17), .O(new_n62_));
  oai21  g16(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g17(.a(x18), .O(new_n64_));
  inv1   g18(.a(x08), .O(new_n65_));
  aoi21  g19(.a(new_n64_), .b(new_n65_), .c(x16), .O(new_n66_));
  oai21  g20(.a(new_n64_), .b(x00), .c(new_n66_), .O(new_n67_));
  aoi21  g21(.a(new_n67_), .b(new_n63_), .c(new_n60_), .O(z09));
  xor2a  g22(.a(new_n62_), .b(x20), .O(new_n69_));
  nand2  g23(.a(new_n69_), .b(x16), .O(new_n70_));
  inv1   g24(.a(x09), .O(new_n71_));
  aoi21  g25(.a(new_n64_), .b(new_n71_), .c(x16), .O(new_n72_));
  oai21  g26(.a(new_n64_), .b(x01), .c(new_n72_), .O(new_n73_));
  aoi21  g27(.a(new_n73_), .b(new_n70_), .c(new_n60_), .O(z10));
  inv1   g28(.a(x20), .O(new_n75_));
  inv1   g29(.a(x21), .O(new_n76_));
  aoi21  g30(.a(new_n62_), .b(new_n75_), .c(new_n76_), .O(new_n77_));
  nor2   g31(.a(x21), .b(x20), .O(new_n78_));
  nand2  g32(.a(new_n78_), .b(new_n62_), .O(new_n79_));
  inv1   g33(.a(new_n79_), .O(new_n80_));
  oai21  g34(.a(new_n80_), .b(new_n77_), .c(x16), .O(new_n81_));
  inv1   g35(.a(x10), .O(new_n82_));
  aoi21  g36(.a(new_n64_), .b(new_n82_), .c(x16), .O(new_n83_));
  oai21  g37(.a(new_n64_), .b(x02), .c(new_n83_), .O(new_n84_));
  aoi21  g38(.a(new_n84_), .b(new_n81_), .c(new_n60_), .O(z11));
  inv1   g39(.a(x22), .O(new_n86_));
  aoi21  g40(.a(new_n78_), .b(new_n62_), .c(new_n86_), .O(new_n87_));
  nand3  g41(.a(new_n78_), .b(new_n62_), .c(new_n86_), .O(new_n88_));
  inv1   g42(.a(new_n88_), .O(new_n89_));
  oai21  g43(.a(new_n89_), .b(new_n87_), .c(x16), .O(new_n90_));
  inv1   g44(.a(x11), .O(new_n91_));
  aoi21  g45(.a(new_n64_), .b(new_n91_), .c(x16), .O(new_n92_));
  oai21  g46(.a(new_n64_), .b(x03), .c(new_n92_), .O(new_n93_));
  aoi21  g47(.a(new_n93_), .b(new_n90_), .c(new_n60_), .O(z12));
  nand2  g48(.a(new_n88_), .b(x23), .O(new_n95_));
  nor2   g49(.a(x23), .b(x22), .O(new_n96_));
  nand3  g50(.a(new_n96_), .b(new_n78_), .c(new_n62_), .O(new_n97_));
  nand2  g51(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g52(.a(new_n98_), .b(x16), .O(new_n99_));
  inv1   g53(.a(x12), .O(new_n100_));
  aoi21  g54(.a(new_n64_), .b(new_n100_), .c(x16), .O(new_n101_));
  oai21  g55(.a(new_n64_), .b(x04), .c(new_n101_), .O(new_n102_));
  aoi21  g56(.a(new_n102_), .b(new_n99_), .c(new_n60_), .O(z13));
  inv1   g57(.a(x24), .O(new_n104_));
  xor2a  g58(.a(new_n97_), .b(new_n104_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(x16), .O(new_n106_));
  inv1   g60(.a(x13), .O(new_n107_));
  aoi21  g61(.a(new_n64_), .b(new_n107_), .c(x16), .O(new_n108_));
  oai21  g62(.a(new_n64_), .b(x05), .c(new_n108_), .O(new_n109_));
  aoi21  g63(.a(new_n109_), .b(new_n106_), .c(new_n60_), .O(z14));
  oai21  g64(.a(new_n97_), .b(x24), .c(x25), .O(new_n111_));
  nor2   g65(.a(x25), .b(x24), .O(new_n112_));
  nand4  g66(.a(new_n112_), .b(new_n96_), .c(new_n78_), .d(new_n62_), .O(new_n113_));
  and2   g67(.a(new_n113_), .b(x16), .O(new_n114_));
  inv1   g68(.a(x16), .O(new_n115_));
  nand2  g69(.a(x18), .b(x06), .O(new_n116_));
  nand2  g70(.a(new_n64_), .b(x14), .O(new_n117_));
  nand3  g71(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(new_n118_));
  oai21  g72(.a(x27), .b(x14), .c(new_n118_), .O(new_n119_));
  aoi21  g73(.a(new_n114_), .b(new_n111_), .c(new_n119_), .O(z15));
  nand2  g74(.a(new_n112_), .b(new_n96_), .O(new_n121_));
  inv1   g75(.a(new_n121_), .O(new_n122_));
  nand3  g76(.a(new_n122_), .b(new_n80_), .c(x26), .O(new_n123_));
  inv1   g77(.a(x26), .O(new_n124_));
  aoi21  g78(.a(new_n113_), .b(new_n124_), .c(new_n115_), .O(new_n125_));
  nand2  g79(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  inv1   g80(.a(x15), .O(new_n127_));
  aoi21  g81(.a(new_n64_), .b(new_n127_), .c(x16), .O(new_n128_));
  oai21  g82(.a(new_n64_), .b(x07), .c(new_n128_), .O(new_n129_));
  aoi21  g83(.a(new_n129_), .b(new_n126_), .c(new_n60_), .O(z16));
  inv1   g84(.a(x17), .O(new_n131_));
  inv1   g85(.a(x25), .O(new_n132_));
  nand4  g86(.a(new_n124_), .b(new_n132_), .c(new_n104_), .d(new_n76_), .O(new_n133_));
  inv1   g87(.a(new_n133_), .O(new_n134_));
  nand4  g88(.a(new_n134_), .b(new_n96_), .c(new_n75_), .d(x19), .O(new_n135_));
  oai21  g89(.a(x17), .b(new_n48_), .c(new_n47_), .O(new_n136_));
  nand2  g90(.a(new_n136_), .b(x16), .O(new_n137_));
  aoi21  g91(.a(new_n135_), .b(new_n131_), .c(new_n137_), .O(z17));
endmodule


