// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x22), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi22  g003(.a(new_n49_), .b(x19), .c(x27), .d(x08), .O(z00));
  aoi22  g004(.a(new_n49_), .b(x20), .c(x27), .d(x09), .O(z01));
  oai22  g005(.a(new_n48_), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  aoi21  g006(.a(x27), .b(x11), .c(new_n49_), .O(z03));
  oai22  g007(.a(new_n48_), .b(x23), .c(new_n47_), .d(x12), .O(z04));
  oai22  g008(.a(new_n48_), .b(x24), .c(new_n47_), .d(x13), .O(z05));
  oai22  g009(.a(new_n48_), .b(x25), .c(new_n47_), .d(x14), .O(z06));
  aoi22  g010(.a(new_n49_), .b(x26), .c(x27), .d(x15), .O(z07));
  nor2   g011(.a(x27), .b(x22), .O(new_n58_));
  inv1   g012(.a(x17), .O(new_n59_));
  inv1   g013(.a(x19), .O(new_n60_));
  nor2   g014(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g015(.a(x19), .b(x17), .O(new_n62_));
  oai21  g016(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g017(.a(x18), .O(new_n64_));
  inv1   g018(.a(x08), .O(new_n65_));
  aoi21  g019(.a(new_n64_), .b(new_n65_), .c(x16), .O(new_n66_));
  oai21  g020(.a(new_n64_), .b(x00), .c(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n63_), .c(new_n58_), .O(z09));
  inv1   g022(.a(x20), .O(new_n69_));
  nor2   g023(.a(new_n62_), .b(new_n69_), .O(new_n70_));
  nand3  g024(.a(new_n69_), .b(new_n60_), .c(new_n59_), .O(new_n71_));
  inv1   g025(.a(new_n71_), .O(new_n72_));
  oai21  g026(.a(new_n72_), .b(new_n70_), .c(x16), .O(new_n73_));
  inv1   g027(.a(x01), .O(new_n74_));
  nand2  g028(.a(x18), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x09), .O(new_n76_));
  aoi21  g030(.a(new_n64_), .b(new_n76_), .c(x16), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n58_), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n73_), .O(z10));
  inv1   g033(.a(x21), .O(new_n80_));
  nor2   g034(.a(new_n72_), .b(new_n80_), .O(new_n81_));
  nand3  g035(.a(new_n62_), .b(new_n80_), .c(new_n69_), .O(new_n82_));
  inv1   g036(.a(new_n82_), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n81_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x02), .O(new_n85_));
  nand2  g039(.a(x18), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x10), .O(new_n87_));
  aoi21  g041(.a(new_n64_), .b(new_n87_), .c(x16), .O(new_n88_));
  aoi21  g042(.a(new_n88_), .b(new_n86_), .c(new_n58_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n84_), .O(z11));
  inv1   g044(.a(x16), .O(new_n91_));
  inv1   g045(.a(x22), .O(new_n92_));
  nand2  g046(.a(x27), .b(new_n92_), .O(new_n93_));
  nor2   g047(.a(new_n93_), .b(new_n82_), .O(new_n94_));
  aoi21  g048(.a(new_n82_), .b(x22), .c(new_n94_), .O(new_n95_));
  nor2   g049(.a(new_n58_), .b(x16), .O(new_n96_));
  inv1   g050(.a(x11), .O(new_n97_));
  nand2  g051(.a(new_n64_), .b(new_n97_), .O(new_n98_));
  inv1   g052(.a(x03), .O(new_n99_));
  nand2  g053(.a(x18), .b(new_n99_), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n98_), .c(new_n96_), .O(new_n101_));
  oai21  g055(.a(new_n95_), .b(new_n91_), .c(new_n101_), .O(z12));
  inv1   g056(.a(x23), .O(new_n103_));
  nand2  g057(.a(new_n82_), .b(x27), .O(new_n104_));
  aoi21  g058(.a(new_n104_), .b(new_n92_), .c(new_n103_), .O(new_n105_));
  nor3   g059(.a(new_n93_), .b(new_n82_), .c(x23), .O(new_n106_));
  oai21  g060(.a(new_n106_), .b(new_n105_), .c(x16), .O(new_n107_));
  inv1   g061(.a(x12), .O(new_n108_));
  nand2  g062(.a(new_n64_), .b(new_n108_), .O(new_n109_));
  inv1   g063(.a(x04), .O(new_n110_));
  nand2  g064(.a(x18), .b(new_n110_), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n109_), .c(new_n96_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n107_), .O(z13));
  nand2  g067(.a(new_n94_), .b(new_n103_), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(x24), .c(x16), .O(new_n115_));
  inv1   g069(.a(x24), .O(new_n116_));
  nand3  g070(.a(new_n116_), .b(new_n103_), .c(new_n80_), .O(new_n117_));
  nor3   g071(.a(new_n117_), .b(new_n71_), .c(new_n91_), .O(new_n118_));
  oai21  g072(.a(new_n118_), .b(new_n47_), .c(new_n92_), .O(new_n119_));
  inv1   g073(.a(x13), .O(new_n120_));
  aoi21  g074(.a(new_n64_), .b(new_n120_), .c(x16), .O(new_n121_));
  oai21  g075(.a(new_n64_), .b(x05), .c(new_n121_), .O(new_n122_));
  nand3  g076(.a(new_n122_), .b(new_n119_), .c(new_n115_), .O(z14));
  inv1   g077(.a(x25), .O(new_n124_));
  oai21  g078(.a(new_n117_), .b(new_n71_), .c(x27), .O(new_n125_));
  aoi21  g079(.a(new_n125_), .b(new_n92_), .c(new_n124_), .O(new_n126_));
  nor2   g080(.a(x24), .b(x23), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nor3   g082(.a(new_n128_), .b(new_n93_), .c(new_n82_), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n126_), .c(x16), .O(new_n130_));
  inv1   g084(.a(x14), .O(new_n131_));
  nand2  g085(.a(new_n64_), .b(new_n131_), .O(new_n132_));
  inv1   g086(.a(x06), .O(new_n133_));
  nand2  g087(.a(x18), .b(new_n133_), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n132_), .c(new_n96_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n130_), .O(z15));
  nor2   g090(.a(new_n117_), .b(new_n71_), .O(new_n137_));
  nor2   g091(.a(x26), .b(x25), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(new_n137_), .c(x16), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(x27), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n92_), .O(new_n141_));
  nand3  g095(.a(new_n127_), .b(new_n94_), .c(new_n124_), .O(new_n142_));
  nor2   g096(.a(new_n64_), .b(x07), .O(new_n143_));
  oai21  g097(.a(x18), .b(x15), .c(new_n91_), .O(new_n144_));
  nor2   g098(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  and2   g099(.a(x26), .b(x16), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n142_), .c(new_n145_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n141_), .O(z16));
  nand3  g102(.a(new_n138_), .b(new_n69_), .c(x19), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n117_), .c(x27), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n92_), .O(new_n151_));
  nand2  g105(.a(x27), .b(x17), .O(new_n152_));
  aoi21  g106(.a(new_n152_), .b(new_n151_), .c(new_n96_), .O(z17));
  buf    g107(.a(x27), .O(z08));
endmodule


