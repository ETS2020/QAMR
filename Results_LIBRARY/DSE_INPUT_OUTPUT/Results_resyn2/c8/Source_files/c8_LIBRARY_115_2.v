// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_;
  nand2  g000(.a(x27), .b(x08), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(x16), .O(new_n49_));
  inv1   g003(.a(x27), .O(new_n50_));
  nand2  g004(.a(new_n50_), .b(x19), .O(new_n51_));
  nand3  g005(.a(new_n51_), .b(new_n49_), .c(new_n47_), .O(new_n52_));
  inv1   g006(.a(new_n52_), .O(z00));
  nand2  g007(.a(x27), .b(x09), .O(new_n54_));
  nand2  g008(.a(new_n50_), .b(x20), .O(new_n55_));
  nand3  g009(.a(new_n55_), .b(new_n54_), .c(new_n49_), .O(new_n56_));
  inv1   g010(.a(new_n56_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n50_), .b(new_n60_), .O(new_n61_));
  nand3  g015(.a(new_n61_), .b(new_n59_), .c(new_n49_), .O(z02));
  nand2  g016(.a(x27), .b(x11), .O(new_n63_));
  nand2  g017(.a(new_n50_), .b(x22), .O(new_n64_));
  nand3  g018(.a(new_n64_), .b(new_n63_), .c(new_n49_), .O(new_n65_));
  inv1   g019(.a(new_n65_), .O(z03));
  nand2  g020(.a(x27), .b(x12), .O(new_n67_));
  nand2  g021(.a(new_n50_), .b(x23), .O(new_n68_));
  nand3  g022(.a(new_n68_), .b(new_n67_), .c(new_n49_), .O(new_n69_));
  inv1   g023(.a(new_n69_), .O(z04));
  nand2  g024(.a(x27), .b(x13), .O(new_n71_));
  nand2  g025(.a(new_n50_), .b(x24), .O(new_n72_));
  nand3  g026(.a(new_n72_), .b(new_n71_), .c(new_n49_), .O(new_n73_));
  inv1   g027(.a(new_n73_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x25), .O(new_n77_));
  nand2  g031(.a(new_n50_), .b(new_n77_), .O(new_n78_));
  nand3  g032(.a(new_n78_), .b(new_n76_), .c(new_n49_), .O(z06));
  nand2  g033(.a(x27), .b(x15), .O(new_n80_));
  nand2  g034(.a(new_n50_), .b(x26), .O(new_n81_));
  nand3  g035(.a(new_n81_), .b(new_n80_), .c(new_n49_), .O(new_n82_));
  inv1   g036(.a(new_n82_), .O(z07));
  aoi21  g037(.a(new_n48_), .b(x16), .c(new_n50_), .O(z08));
  inv1   g038(.a(x08), .O(new_n85_));
  inv1   g039(.a(x16), .O(new_n86_));
  nand2  g040(.a(new_n48_), .b(new_n86_), .O(new_n87_));
  inv1   g041(.a(x19), .O(new_n88_));
  nor2   g042(.a(new_n88_), .b(x17), .O(new_n89_));
  inv1   g043(.a(x17), .O(new_n90_));
  nor2   g044(.a(x19), .b(new_n90_), .O(new_n91_));
  oai21  g045(.a(new_n91_), .b(new_n89_), .c(x16), .O(new_n92_));
  inv1   g046(.a(x00), .O(new_n93_));
  aoi21  g047(.a(new_n86_), .b(new_n93_), .c(new_n48_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  oai21  g049(.a(new_n87_), .b(new_n85_), .c(new_n95_), .O(z09));
  inv1   g050(.a(x20), .O(new_n97_));
  nor2   g051(.a(x19), .b(x17), .O(new_n98_));
  xor2a  g052(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g053(.a(new_n48_), .b(new_n86_), .O(new_n100_));
  inv1   g054(.a(new_n100_), .O(new_n101_));
  nor2   g055(.a(new_n48_), .b(x01), .O(new_n102_));
  oai21  g056(.a(x18), .b(x09), .c(new_n86_), .O(new_n103_));
  oai22  g057(.a(new_n103_), .b(new_n102_), .c(new_n101_), .d(new_n99_), .O(z10));
  aoi21  g058(.a(new_n98_), .b(new_n97_), .c(new_n60_), .O(new_n105_));
  nand4  g059(.a(new_n60_), .b(new_n97_), .c(new_n88_), .d(new_n90_), .O(new_n106_));
  inv1   g060(.a(new_n106_), .O(new_n107_));
  oai21  g061(.a(new_n107_), .b(new_n105_), .c(new_n100_), .O(new_n108_));
  aoi21  g062(.a(new_n48_), .b(new_n58_), .c(x16), .O(new_n109_));
  oai21  g063(.a(new_n48_), .b(x02), .c(new_n109_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n108_), .O(z11));
  xor2a  g065(.a(new_n106_), .b(x22), .O(new_n112_));
  nor2   g066(.a(new_n48_), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(x11), .c(new_n86_), .O(new_n114_));
  oai22  g068(.a(new_n114_), .b(new_n113_), .c(new_n112_), .d(new_n101_), .O(z12));
  inv1   g069(.a(x22), .O(new_n116_));
  nor2   g070(.a(x21), .b(x20), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n98_), .c(new_n116_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x23), .O(new_n119_));
  nor2   g073(.a(x23), .b(x22), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n117_), .c(new_n98_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n100_), .O(new_n123_));
  inv1   g077(.a(x12), .O(new_n124_));
  aoi21  g078(.a(new_n48_), .b(new_n124_), .c(x16), .O(new_n125_));
  oai21  g079(.a(new_n48_), .b(x04), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n123_), .O(z13));
  nand2  g081(.a(new_n121_), .b(x24), .O(new_n128_));
  nor2   g082(.a(x24), .b(x21), .O(new_n129_));
  nand4  g083(.a(new_n129_), .b(new_n120_), .c(new_n98_), .d(new_n97_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(x16), .O(new_n132_));
  inv1   g086(.a(x05), .O(new_n133_));
  oai21  g087(.a(x16), .b(new_n133_), .c(x18), .O(new_n134_));
  oai21  g088(.a(new_n87_), .b(x13), .c(new_n134_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n132_), .O(z14));
  inv1   g090(.a(x23), .O(new_n137_));
  inv1   g091(.a(x24), .O(new_n138_));
  nand4  g092(.a(new_n77_), .b(new_n138_), .c(new_n137_), .d(new_n116_), .O(new_n139_));
  inv1   g093(.a(new_n139_), .O(new_n140_));
  aoi22  g094(.a(new_n140_), .b(new_n107_), .c(new_n130_), .d(x25), .O(new_n141_));
  aoi21  g095(.a(new_n48_), .b(new_n75_), .c(x16), .O(new_n142_));
  oai21  g096(.a(new_n48_), .b(x06), .c(new_n142_), .O(new_n143_));
  oai21  g097(.a(new_n141_), .b(new_n101_), .c(new_n143_), .O(z15));
  inv1   g098(.a(x15), .O(new_n145_));
  aoi21  g099(.a(new_n48_), .b(new_n145_), .c(x16), .O(new_n146_));
  oai21  g100(.a(new_n48_), .b(x07), .c(new_n146_), .O(new_n147_));
  inv1   g101(.a(x26), .O(new_n148_));
  nor2   g102(.a(x25), .b(x24), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n120_), .c(new_n117_), .d(new_n98_), .O(new_n150_));
  nor2   g104(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand2  g105(.a(new_n150_), .b(new_n148_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n100_), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(new_n151_), .c(new_n147_), .O(z16));
  nand4  g108(.a(new_n140_), .b(new_n117_), .c(new_n89_), .d(new_n148_), .O(new_n155_));
  aoi21  g109(.a(x27), .b(x17), .c(new_n48_), .O(new_n156_));
  aoi21  g110(.a(new_n156_), .b(new_n155_), .c(new_n86_), .O(z17));
endmodule


