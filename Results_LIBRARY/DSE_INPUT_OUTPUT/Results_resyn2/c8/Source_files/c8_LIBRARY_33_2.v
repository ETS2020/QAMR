// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_;
  nor2   g000(.a(x27), .b(x17), .O(new_n47_));
  aoi22  g001(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g002(.a(x27), .O(new_n49_));
  inv1   g003(.a(new_n47_), .O(z08));
  oai22  g004(.a(z08), .b(x20), .c(new_n49_), .d(x09), .O(z01));
  aoi22  g005(.a(new_n47_), .b(x21), .c(x27), .d(x10), .O(z02));
  aoi22  g006(.a(new_n47_), .b(x22), .c(x27), .d(x11), .O(z03));
  oai22  g007(.a(z08), .b(x23), .c(new_n49_), .d(x12), .O(z04));
  aoi22  g008(.a(new_n47_), .b(x24), .c(x27), .d(x13), .O(z05));
  aoi22  g009(.a(new_n47_), .b(x25), .c(x27), .d(x14), .O(z06));
  oai22  g010(.a(z08), .b(x26), .c(new_n49_), .d(x15), .O(z07));
  inv1   g011(.a(x18), .O(new_n58_));
  inv1   g012(.a(x08), .O(new_n59_));
  aoi21  g013(.a(new_n58_), .b(new_n59_), .c(x16), .O(new_n60_));
  oai21  g014(.a(new_n58_), .b(x00), .c(new_n60_), .O(new_n61_));
  inv1   g015(.a(x17), .O(new_n62_));
  nor2   g016(.a(x27), .b(new_n62_), .O(new_n63_));
  inv1   g017(.a(new_n63_), .O(new_n64_));
  inv1   g018(.a(x19), .O(new_n65_));
  nor2   g019(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nor2   g020(.a(x19), .b(x17), .O(new_n67_));
  oai21  g021(.a(new_n67_), .b(new_n66_), .c(x16), .O(new_n68_));
  nand3  g022(.a(new_n68_), .b(new_n64_), .c(new_n61_), .O(z09));
  inv1   g023(.a(x16), .O(new_n70_));
  aoi21  g024(.a(x20), .b(new_n65_), .c(new_n70_), .O(new_n71_));
  oai21  g025(.a(new_n67_), .b(x20), .c(new_n71_), .O(new_n72_));
  nand2  g026(.a(x20), .b(x16), .O(new_n73_));
  nand2  g027(.a(new_n73_), .b(x27), .O(new_n74_));
  nand2  g028(.a(new_n74_), .b(x17), .O(new_n75_));
  inv1   g029(.a(x09), .O(new_n76_));
  aoi21  g030(.a(new_n58_), .b(new_n76_), .c(x16), .O(new_n77_));
  oai21  g031(.a(new_n58_), .b(x01), .c(new_n77_), .O(new_n78_));
  nand3  g032(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(z10));
  nor3   g033(.a(x20), .b(x19), .c(x17), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(x21), .O(new_n81_));
  nor2   g035(.a(new_n63_), .b(new_n70_), .O(new_n82_));
  inv1   g036(.a(x21), .O(new_n83_));
  nor2   g037(.a(x20), .b(x19), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n62_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand3  g040(.a(new_n86_), .b(new_n82_), .c(new_n81_), .O(new_n87_));
  nor2   g041(.a(new_n63_), .b(x16), .O(new_n88_));
  inv1   g042(.a(x10), .O(new_n89_));
  nand2  g043(.a(new_n58_), .b(new_n89_), .O(new_n90_));
  inv1   g044(.a(x02), .O(new_n91_));
  nand2  g045(.a(x18), .b(new_n91_), .O(new_n92_));
  nand3  g046(.a(new_n92_), .b(new_n90_), .c(new_n88_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n87_), .O(z11));
  inv1   g048(.a(x22), .O(new_n95_));
  aoi21  g049(.a(new_n84_), .b(new_n83_), .c(new_n95_), .O(new_n96_));
  nor2   g050(.a(x22), .b(x21), .O(new_n97_));
  inv1   g051(.a(new_n97_), .O(new_n98_));
  nor2   g052(.a(new_n98_), .b(new_n85_), .O(new_n99_));
  oai21  g053(.a(new_n99_), .b(new_n96_), .c(x16), .O(new_n100_));
  nand2  g054(.a(x22), .b(x16), .O(new_n101_));
  aoi21  g055(.a(new_n101_), .b(x27), .c(new_n62_), .O(new_n102_));
  inv1   g056(.a(x03), .O(new_n103_));
  nand2  g057(.a(x18), .b(new_n103_), .O(new_n104_));
  inv1   g058(.a(x11), .O(new_n105_));
  aoi21  g059(.a(new_n58_), .b(new_n105_), .c(x16), .O(new_n106_));
  aoi21  g060(.a(new_n106_), .b(new_n104_), .c(new_n102_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n100_), .O(z12));
  inv1   g062(.a(x12), .O(new_n109_));
  nand2  g063(.a(new_n58_), .b(new_n109_), .O(new_n110_));
  inv1   g064(.a(x04), .O(new_n111_));
  nand2  g065(.a(x18), .b(new_n111_), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n110_), .c(new_n88_), .O(new_n113_));
  nor2   g067(.a(new_n99_), .b(x23), .O(new_n114_));
  nand3  g068(.a(new_n97_), .b(new_n80_), .c(x23), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n82_), .O(new_n116_));
  oai21  g070(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(z13));
  nor2   g071(.a(x24), .b(x23), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n97_), .c(new_n80_), .O(new_n119_));
  inv1   g073(.a(x23), .O(new_n120_));
  nand3  g074(.a(new_n97_), .b(new_n84_), .c(new_n120_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(x24), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n119_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(x16), .O(new_n124_));
  nand2  g078(.a(x24), .b(x16), .O(new_n125_));
  aoi21  g079(.a(new_n125_), .b(x27), .c(new_n62_), .O(new_n126_));
  inv1   g080(.a(x05), .O(new_n127_));
  nand2  g081(.a(x18), .b(new_n127_), .O(new_n128_));
  inv1   g082(.a(x13), .O(new_n129_));
  aoi21  g083(.a(new_n58_), .b(new_n129_), .c(x16), .O(new_n130_));
  aoi21  g084(.a(new_n130_), .b(new_n128_), .c(new_n126_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n124_), .O(z14));
  inv1   g086(.a(x14), .O(new_n133_));
  nand2  g087(.a(new_n58_), .b(new_n133_), .O(new_n134_));
  inv1   g088(.a(x06), .O(new_n135_));
  nand2  g089(.a(x18), .b(new_n135_), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n134_), .c(new_n88_), .O(new_n137_));
  nand4  g091(.a(new_n118_), .b(new_n97_), .c(new_n80_), .d(x25), .O(new_n138_));
  inv1   g092(.a(x25), .O(new_n139_));
  nand2  g093(.a(new_n119_), .b(new_n139_), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n138_), .c(new_n82_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n137_), .O(z15));
  nand4  g096(.a(new_n118_), .b(new_n97_), .c(new_n84_), .d(new_n139_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(x26), .O(new_n144_));
  nor2   g098(.a(x26), .b(x25), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n118_), .c(new_n97_), .d(new_n80_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(x16), .O(new_n148_));
  nand2  g102(.a(x26), .b(x16), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(x27), .c(new_n62_), .O(new_n150_));
  inv1   g104(.a(x07), .O(new_n151_));
  nand2  g105(.a(x18), .b(new_n151_), .O(new_n152_));
  inv1   g106(.a(x15), .O(new_n153_));
  aoi21  g107(.a(new_n58_), .b(new_n153_), .c(x16), .O(new_n154_));
  aoi21  g108(.a(new_n154_), .b(new_n152_), .c(new_n150_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n148_), .O(z16));
  inv1   g110(.a(x24), .O(new_n157_));
  nor2   g111(.a(new_n98_), .b(x23), .O(new_n158_));
  nor2   g112(.a(x20), .b(new_n65_), .O(new_n159_));
  nand4  g113(.a(new_n159_), .b(new_n145_), .c(new_n158_), .d(new_n157_), .O(new_n160_));
  aoi21  g114(.a(new_n160_), .b(new_n62_), .c(new_n88_), .O(z17));
endmodule


