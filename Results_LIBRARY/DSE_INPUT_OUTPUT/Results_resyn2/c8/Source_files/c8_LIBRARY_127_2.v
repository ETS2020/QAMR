// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_;
  nor2   g000(.a(x27), .b(x21), .O(new_n47_));
  aoi22  g001(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g002(.a(x27), .O(new_n49_));
  inv1   g003(.a(new_n47_), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x20), .c(new_n49_), .d(x09), .O(z01));
  nand2  g005(.a(x27), .b(x10), .O(z02));
  oai22  g006(.a(new_n50_), .b(x22), .c(new_n49_), .d(x11), .O(z03));
  aoi22  g007(.a(new_n47_), .b(x23), .c(x27), .d(x12), .O(z04));
  oai22  g008(.a(new_n50_), .b(x24), .c(new_n49_), .d(x13), .O(z05));
  aoi22  g009(.a(new_n47_), .b(x25), .c(x27), .d(x14), .O(z06));
  oai22  g010(.a(new_n50_), .b(x26), .c(new_n49_), .d(x15), .O(z07));
  inv1   g011(.a(x21), .O(new_n58_));
  nor2   g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x18), .O(new_n60_));
  inv1   g014(.a(x08), .O(new_n61_));
  aoi21  g015(.a(new_n60_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g016(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g017(.a(x19), .O(new_n64_));
  nand2  g018(.a(new_n64_), .b(x17), .O(new_n65_));
  inv1   g019(.a(x16), .O(new_n66_));
  nor2   g020(.a(new_n64_), .b(x17), .O(new_n67_));
  nor2   g021(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n63_), .c(new_n59_), .O(z09));
  inv1   g024(.a(x20), .O(new_n71_));
  nor2   g025(.a(x19), .b(x17), .O(new_n72_));
  nor2   g026(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor3   g027(.a(x20), .b(x19), .c(x17), .O(new_n74_));
  oai21  g028(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  inv1   g029(.a(x09), .O(new_n76_));
  aoi21  g030(.a(new_n60_), .b(new_n76_), .c(x16), .O(new_n77_));
  oai21  g031(.a(new_n60_), .b(x01), .c(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n75_), .c(new_n59_), .O(z10));
  inv1   g033(.a(new_n74_), .O(new_n80_));
  nor2   g034(.a(new_n49_), .b(new_n58_), .O(new_n81_));
  nor2   g035(.a(x21), .b(x20), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n72_), .O(new_n83_));
  inv1   g037(.a(new_n83_), .O(new_n84_));
  aoi21  g038(.a(new_n81_), .b(new_n80_), .c(new_n84_), .O(new_n85_));
  inv1   g039(.a(new_n59_), .O(new_n86_));
  inv1   g040(.a(x10), .O(new_n87_));
  nand2  g041(.a(new_n60_), .b(new_n87_), .O(new_n88_));
  inv1   g042(.a(x02), .O(new_n89_));
  nand2  g043(.a(x18), .b(new_n89_), .O(new_n90_));
  nand4  g044(.a(new_n90_), .b(new_n88_), .c(new_n86_), .d(new_n66_), .O(new_n91_));
  oai21  g045(.a(new_n85_), .b(new_n66_), .c(new_n91_), .O(z11));
  inv1   g046(.a(x22), .O(new_n93_));
  nor2   g047(.a(new_n84_), .b(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n74_), .b(new_n93_), .O(new_n95_));
  nor2   g049(.a(new_n95_), .b(x21), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n94_), .c(x16), .O(new_n97_));
  inv1   g051(.a(x03), .O(new_n98_));
  nand2  g052(.a(x18), .b(new_n98_), .O(new_n99_));
  inv1   g053(.a(x11), .O(new_n100_));
  aoi21  g054(.a(new_n60_), .b(new_n100_), .c(x16), .O(new_n101_));
  aoi21  g055(.a(new_n101_), .b(new_n99_), .c(new_n59_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n97_), .O(z12));
  nand2  g057(.a(new_n95_), .b(x23), .O(new_n104_));
  nor2   g058(.a(x23), .b(x22), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n84_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(x16), .O(new_n108_));
  nand2  g062(.a(x23), .b(x16), .O(new_n109_));
  aoi21  g063(.a(new_n109_), .b(x27), .c(new_n58_), .O(new_n110_));
  inv1   g064(.a(x04), .O(new_n111_));
  nand2  g065(.a(x18), .b(new_n111_), .O(new_n112_));
  inv1   g066(.a(x12), .O(new_n113_));
  aoi21  g067(.a(new_n60_), .b(new_n113_), .c(x16), .O(new_n114_));
  aoi21  g068(.a(new_n114_), .b(new_n112_), .c(new_n110_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n108_), .O(z13));
  nand3  g070(.a(new_n105_), .b(new_n84_), .c(x24), .O(new_n117_));
  inv1   g071(.a(x24), .O(new_n118_));
  nand2  g072(.a(new_n106_), .b(new_n118_), .O(new_n119_));
  nand3  g073(.a(new_n119_), .b(new_n117_), .c(x16), .O(new_n120_));
  inv1   g074(.a(x13), .O(new_n121_));
  aoi21  g075(.a(new_n60_), .b(new_n121_), .c(x16), .O(new_n122_));
  oai21  g076(.a(new_n60_), .b(x05), .c(new_n122_), .O(new_n123_));
  aoi21  g077(.a(new_n123_), .b(new_n120_), .c(new_n59_), .O(z14));
  inv1   g078(.a(x25), .O(new_n125_));
  nor3   g079(.a(x24), .b(x23), .c(x22), .O(new_n126_));
  aoi21  g080(.a(new_n126_), .b(new_n74_), .c(new_n125_), .O(new_n127_));
  nor2   g081(.a(x25), .b(x24), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n105_), .O(new_n129_));
  nor2   g083(.a(new_n129_), .b(new_n83_), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n127_), .c(x16), .O(new_n131_));
  nand2  g085(.a(x25), .b(x16), .O(new_n132_));
  aoi21  g086(.a(new_n132_), .b(x27), .c(new_n58_), .O(new_n133_));
  inv1   g087(.a(x06), .O(new_n134_));
  nand2  g088(.a(x18), .b(new_n134_), .O(new_n135_));
  inv1   g089(.a(x14), .O(new_n136_));
  aoi21  g090(.a(new_n60_), .b(new_n136_), .c(x16), .O(new_n137_));
  aoi21  g091(.a(new_n137_), .b(new_n135_), .c(new_n133_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n131_), .O(z15));
  nand4  g093(.a(new_n128_), .b(new_n105_), .c(new_n72_), .d(new_n71_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x26), .O(new_n141_));
  nor2   g095(.a(x26), .b(x23), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n128_), .c(new_n74_), .d(new_n93_), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(new_n141_), .c(x21), .O(new_n144_));
  nand2  g098(.a(new_n81_), .b(x26), .O(new_n145_));
  inv1   g099(.a(new_n145_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  inv1   g101(.a(x15), .O(new_n148_));
  nand2  g102(.a(new_n60_), .b(new_n148_), .O(new_n149_));
  inv1   g103(.a(x07), .O(new_n150_));
  nand2  g104(.a(x18), .b(new_n150_), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n149_), .c(new_n86_), .d(new_n66_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n147_), .O(z16));
  nand2  g107(.a(x27), .b(x17), .O(new_n154_));
  nor2   g108(.a(x26), .b(x25), .O(new_n155_));
  nand4  g109(.a(new_n155_), .b(new_n126_), .c(new_n82_), .d(new_n67_), .O(new_n156_));
  aoi21  g110(.a(new_n156_), .b(new_n154_), .c(new_n66_), .O(z17));
  buf    g111(.a(x27), .O(z08));
endmodule


