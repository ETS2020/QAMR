// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x18), .O(new_n48_));
  aoi22  g002(.a(new_n48_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  nand2  g003(.a(x27), .b(x09), .O(new_n50_));
  inv1   g004(.a(x08), .O(new_n51_));
  nor2   g005(.a(x18), .b(new_n51_), .O(new_n52_));
  inv1   g006(.a(new_n52_), .O(new_n53_));
  nand2  g007(.a(new_n47_), .b(x20), .O(new_n54_));
  nand3  g008(.a(new_n54_), .b(new_n53_), .c(new_n50_), .O(new_n55_));
  inv1   g009(.a(new_n55_), .O(z01));
  inv1   g010(.a(x10), .O(new_n57_));
  aoi21  g011(.a(new_n47_), .b(x21), .c(new_n52_), .O(new_n58_));
  oai21  g012(.a(new_n47_), .b(new_n57_), .c(new_n58_), .O(new_n59_));
  inv1   g013(.a(new_n59_), .O(z02));
  nand2  g014(.a(x27), .b(x11), .O(new_n61_));
  nand2  g015(.a(new_n47_), .b(x22), .O(new_n62_));
  nand3  g016(.a(new_n62_), .b(new_n61_), .c(new_n53_), .O(new_n63_));
  inv1   g017(.a(new_n63_), .O(z03));
  inv1   g018(.a(x23), .O(new_n65_));
  aoi21  g019(.a(new_n47_), .b(new_n65_), .c(new_n52_), .O(new_n66_));
  oai21  g020(.a(new_n47_), .b(x12), .c(new_n66_), .O(z04));
  inv1   g021(.a(x24), .O(new_n68_));
  aoi21  g022(.a(new_n47_), .b(new_n68_), .c(new_n52_), .O(new_n69_));
  oai21  g023(.a(new_n47_), .b(x13), .c(new_n69_), .O(z05));
  nand2  g024(.a(x27), .b(x14), .O(new_n71_));
  nand2  g025(.a(new_n47_), .b(x25), .O(new_n72_));
  nand3  g026(.a(new_n72_), .b(new_n71_), .c(new_n53_), .O(new_n73_));
  inv1   g027(.a(new_n73_), .O(z06));
  inv1   g028(.a(x26), .O(new_n75_));
  aoi21  g029(.a(new_n47_), .b(new_n75_), .c(new_n52_), .O(new_n76_));
  oai21  g030(.a(new_n47_), .b(x15), .c(new_n76_), .O(z07));
  nand2  g031(.a(new_n53_), .b(new_n47_), .O(z08));
  inv1   g032(.a(x17), .O(new_n79_));
  inv1   g033(.a(x19), .O(new_n80_));
  nor2   g034(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nor2   g035(.a(x19), .b(x17), .O(new_n82_));
  nor2   g036(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g037(.a(x16), .O(new_n84_));
  nor2   g038(.a(new_n52_), .b(new_n84_), .O(new_n85_));
  inv1   g039(.a(new_n85_), .O(new_n86_));
  nand3  g040(.a(x18), .b(new_n84_), .c(x00), .O(new_n87_));
  oai21  g041(.a(new_n86_), .b(new_n83_), .c(new_n87_), .O(z09));
  nor2   g042(.a(x18), .b(x08), .O(new_n89_));
  aoi22  g043(.a(new_n89_), .b(x09), .c(x18), .d(x01), .O(new_n90_));
  inv1   g044(.a(x20), .O(new_n91_));
  xor2a  g045(.a(new_n82_), .b(new_n91_), .O(new_n92_));
  oai22  g046(.a(new_n92_), .b(new_n86_), .c(new_n90_), .d(x16), .O(z10));
  inv1   g047(.a(x21), .O(new_n94_));
  aoi21  g048(.a(new_n82_), .b(new_n91_), .c(new_n94_), .O(new_n95_));
  nand4  g049(.a(new_n94_), .b(new_n91_), .c(new_n80_), .d(new_n79_), .O(new_n96_));
  inv1   g050(.a(new_n96_), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n95_), .c(x16), .O(new_n98_));
  inv1   g052(.a(x02), .O(new_n99_));
  nand2  g053(.a(x18), .b(new_n99_), .O(new_n100_));
  inv1   g054(.a(x18), .O(new_n101_));
  aoi21  g055(.a(new_n101_), .b(new_n57_), .c(x16), .O(new_n102_));
  aoi21  g056(.a(new_n102_), .b(new_n100_), .c(new_n52_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n98_), .O(z11));
  nand2  g058(.a(x18), .b(x03), .O(new_n105_));
  nand2  g059(.a(new_n89_), .b(x11), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n84_), .O(new_n108_));
  xor2a  g062(.a(new_n96_), .b(x22), .O(new_n109_));
  oai21  g063(.a(new_n109_), .b(new_n86_), .c(new_n108_), .O(z12));
  inv1   g064(.a(x22), .O(new_n111_));
  nor2   g065(.a(x21), .b(x20), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n82_), .c(new_n111_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(x23), .O(new_n114_));
  nor2   g068(.a(x23), .b(x22), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n112_), .c(new_n82_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x16), .O(new_n118_));
  inv1   g072(.a(x04), .O(new_n119_));
  nand2  g073(.a(x18), .b(new_n119_), .O(new_n120_));
  inv1   g074(.a(x12), .O(new_n121_));
  aoi21  g075(.a(new_n101_), .b(new_n121_), .c(x16), .O(new_n122_));
  aoi21  g076(.a(new_n122_), .b(new_n120_), .c(new_n52_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n118_), .O(z13));
  nand2  g078(.a(x18), .b(x05), .O(new_n125_));
  nand2  g079(.a(new_n89_), .b(x13), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n84_), .O(new_n128_));
  nand2  g082(.a(new_n116_), .b(x24), .O(new_n129_));
  nor2   g083(.a(x24), .b(x21), .O(new_n130_));
  nand4  g084(.a(new_n130_), .b(new_n115_), .c(new_n82_), .d(new_n91_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n85_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n128_), .O(z14));
  nand2  g088(.a(x18), .b(x06), .O(new_n135_));
  nand2  g089(.a(new_n89_), .b(x14), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n84_), .O(new_n138_));
  inv1   g092(.a(x25), .O(new_n139_));
  nand4  g093(.a(new_n139_), .b(new_n68_), .c(new_n65_), .d(new_n111_), .O(new_n140_));
  inv1   g094(.a(new_n140_), .O(new_n141_));
  aoi22  g095(.a(new_n141_), .b(new_n97_), .c(new_n131_), .d(x25), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n86_), .c(new_n138_), .O(z15));
  nor2   g097(.a(x25), .b(x24), .O(new_n144_));
  nand4  g098(.a(new_n144_), .b(new_n115_), .c(new_n112_), .d(new_n82_), .O(new_n145_));
  nor2   g099(.a(new_n145_), .b(new_n75_), .O(new_n146_));
  nand2  g100(.a(new_n145_), .b(new_n75_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(x16), .O(new_n148_));
  inv1   g102(.a(x07), .O(new_n149_));
  nand2  g103(.a(x18), .b(new_n149_), .O(new_n150_));
  inv1   g104(.a(x15), .O(new_n151_));
  aoi21  g105(.a(new_n101_), .b(new_n151_), .c(x16), .O(new_n152_));
  aoi21  g106(.a(new_n152_), .b(new_n150_), .c(new_n52_), .O(new_n153_));
  oai21  g107(.a(new_n148_), .b(new_n146_), .c(new_n153_), .O(z16));
  nand4  g108(.a(new_n141_), .b(new_n112_), .c(new_n75_), .d(x19), .O(new_n155_));
  nand2  g109(.a(new_n47_), .b(x17), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n85_), .O(new_n157_));
  aoi21  g111(.a(new_n155_), .b(new_n79_), .c(new_n157_), .O(z17));
endmodule


