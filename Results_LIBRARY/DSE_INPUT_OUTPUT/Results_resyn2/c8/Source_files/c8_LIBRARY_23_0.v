// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n55_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x12), .O(new_n47_));
  nor2   g001(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi22  g002(.a(new_n48_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g003(.a(x27), .O(new_n50_));
  inv1   g004(.a(new_n48_), .O(new_n51_));
  oai22  g005(.a(new_n51_), .b(x20), .c(new_n50_), .d(x09), .O(z01));
  aoi22  g006(.a(new_n48_), .b(x21), .c(x27), .d(x10), .O(z02));
  aoi22  g007(.a(new_n48_), .b(x22), .c(x27), .d(x11), .O(z03));
  nand2  g008(.a(x27), .b(new_n47_), .O(new_n55_));
  oai21  g009(.a(new_n51_), .b(x23), .c(new_n55_), .O(z04));
  oai22  g010(.a(new_n51_), .b(x24), .c(new_n50_), .d(x13), .O(z05));
  oai22  g011(.a(new_n51_), .b(x25), .c(new_n50_), .d(x14), .O(z06));
  aoi22  g012(.a(new_n48_), .b(x26), .c(x27), .d(x15), .O(z07));
  nor2   g013(.a(x27), .b(x12), .O(new_n60_));
  inv1   g014(.a(new_n60_), .O(new_n61_));
  inv1   g015(.a(x08), .O(new_n62_));
  inv1   g016(.a(x16), .O(new_n63_));
  oai21  g017(.a(x18), .b(new_n62_), .c(new_n63_), .O(new_n64_));
  aoi21  g018(.a(x18), .b(x00), .c(new_n64_), .O(new_n65_));
  inv1   g019(.a(x17), .O(new_n66_));
  inv1   g020(.a(x19), .O(new_n67_));
  nor2   g021(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nor2   g022(.a(x19), .b(x17), .O(new_n69_));
  nor3   g023(.a(new_n69_), .b(new_n68_), .c(new_n63_), .O(new_n70_));
  oai21  g024(.a(new_n70_), .b(new_n65_), .c(new_n61_), .O(z09));
  inv1   g025(.a(x20), .O(new_n72_));
  nor2   g026(.a(new_n69_), .b(new_n72_), .O(new_n73_));
  nor3   g027(.a(x20), .b(x19), .c(x17), .O(new_n74_));
  oai21  g028(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  inv1   g029(.a(x01), .O(new_n76_));
  nand2  g030(.a(x18), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x09), .O(new_n78_));
  inv1   g032(.a(x18), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n78_), .c(x16), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n77_), .c(new_n60_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n75_), .O(z10));
  nand2  g036(.a(x18), .b(x02), .O(new_n83_));
  nand2  g037(.a(new_n79_), .b(x10), .O(new_n84_));
  nand3  g038(.a(new_n84_), .b(new_n83_), .c(new_n63_), .O(new_n85_));
  inv1   g039(.a(x21), .O(new_n86_));
  nor2   g040(.a(new_n74_), .b(new_n86_), .O(new_n87_));
  nand4  g041(.a(new_n86_), .b(new_n72_), .c(new_n67_), .d(new_n66_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(x16), .O(new_n89_));
  oai21  g043(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n61_), .O(z11));
  inv1   g045(.a(x22), .O(new_n92_));
  nor2   g046(.a(x21), .b(x20), .O(new_n93_));
  aoi21  g047(.a(new_n93_), .b(new_n69_), .c(new_n92_), .O(new_n94_));
  nand3  g048(.a(new_n93_), .b(new_n69_), .c(new_n92_), .O(new_n95_));
  inv1   g049(.a(new_n95_), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n94_), .c(x16), .O(new_n97_));
  inv1   g051(.a(x11), .O(new_n98_));
  aoi21  g052(.a(new_n79_), .b(new_n98_), .c(x16), .O(new_n99_));
  oai21  g053(.a(new_n79_), .b(x03), .c(new_n99_), .O(new_n100_));
  aoi21  g054(.a(new_n100_), .b(new_n97_), .c(new_n60_), .O(z12));
  nand2  g055(.a(new_n95_), .b(x23), .O(new_n102_));
  nor2   g056(.a(x23), .b(x22), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n93_), .c(new_n69_), .O(new_n104_));
  and2   g058(.a(new_n104_), .b(x16), .O(new_n105_));
  nand2  g059(.a(x18), .b(x04), .O(new_n106_));
  nand2  g060(.a(new_n79_), .b(x12), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n106_), .c(new_n63_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n61_), .O(new_n109_));
  aoi21  g063(.a(new_n105_), .b(new_n102_), .c(new_n109_), .O(z13));
  nand2  g064(.a(new_n104_), .b(x24), .O(new_n111_));
  nor2   g065(.a(x24), .b(x21), .O(new_n112_));
  nand4  g066(.a(new_n112_), .b(new_n103_), .c(new_n69_), .d(new_n72_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g069(.a(x05), .O(new_n116_));
  nand2  g070(.a(x18), .b(new_n116_), .O(new_n117_));
  inv1   g071(.a(x13), .O(new_n118_));
  aoi21  g072(.a(new_n79_), .b(new_n118_), .c(x16), .O(new_n119_));
  aoi21  g073(.a(new_n119_), .b(new_n117_), .c(new_n60_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n115_), .O(z14));
  inv1   g075(.a(new_n88_), .O(new_n122_));
  inv1   g076(.a(x23), .O(new_n123_));
  inv1   g077(.a(x24), .O(new_n124_));
  inv1   g078(.a(x25), .O(new_n125_));
  nand4  g079(.a(new_n125_), .b(new_n124_), .c(new_n123_), .d(new_n92_), .O(new_n126_));
  inv1   g080(.a(new_n126_), .O(new_n127_));
  aoi22  g081(.a(new_n127_), .b(new_n122_), .c(new_n113_), .d(x25), .O(new_n128_));
  inv1   g082(.a(x06), .O(new_n129_));
  nand2  g083(.a(x18), .b(new_n129_), .O(new_n130_));
  inv1   g084(.a(x14), .O(new_n131_));
  aoi21  g085(.a(new_n79_), .b(new_n131_), .c(x16), .O(new_n132_));
  aoi21  g086(.a(new_n132_), .b(new_n130_), .c(new_n60_), .O(new_n133_));
  oai21  g087(.a(new_n128_), .b(new_n63_), .c(new_n133_), .O(z15));
  inv1   g088(.a(x26), .O(new_n135_));
  nor2   g089(.a(x25), .b(x24), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n103_), .c(new_n93_), .d(new_n69_), .O(new_n137_));
  nor2   g091(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g092(.a(new_n137_), .b(new_n135_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(x16), .O(new_n140_));
  inv1   g094(.a(x07), .O(new_n141_));
  nand2  g095(.a(x18), .b(new_n141_), .O(new_n142_));
  inv1   g096(.a(x15), .O(new_n143_));
  aoi21  g097(.a(new_n79_), .b(new_n143_), .c(x16), .O(new_n144_));
  aoi21  g098(.a(new_n144_), .b(new_n142_), .c(new_n60_), .O(new_n145_));
  oai21  g099(.a(new_n140_), .b(new_n138_), .c(new_n145_), .O(z16));
  nand2  g100(.a(x27), .b(x17), .O(new_n147_));
  nand4  g101(.a(new_n93_), .b(new_n135_), .c(x19), .d(new_n66_), .O(new_n148_));
  oai21  g102(.a(new_n148_), .b(new_n126_), .c(new_n147_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(x16), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n61_), .O(z17));
  buf    g105(.a(x27), .O(z08));
endmodule


