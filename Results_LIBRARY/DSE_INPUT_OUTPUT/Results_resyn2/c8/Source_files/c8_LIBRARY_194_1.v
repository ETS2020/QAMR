// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_;
  nand2  g000(.a(x27), .b(x08), .O(new_n47_));
  inv1   g001(.a(x17), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x03), .O(new_n49_));
  inv1   g003(.a(new_n49_), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(x19), .O(new_n52_));
  nand3  g006(.a(new_n52_), .b(new_n50_), .c(new_n47_), .O(new_n53_));
  inv1   g007(.a(new_n53_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(new_n51_), .b(x20), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(new_n51_), .b(new_n55_), .c(new_n56_), .O(new_n57_));
  inv1   g011(.a(new_n57_), .O(z01));
  inv1   g012(.a(x10), .O(new_n59_));
  aoi21  g013(.a(new_n51_), .b(x21), .c(new_n49_), .O(new_n60_));
  oai21  g014(.a(new_n51_), .b(new_n59_), .c(new_n60_), .O(new_n61_));
  inv1   g015(.a(new_n61_), .O(z02));
  nand2  g016(.a(x27), .b(x11), .O(new_n63_));
  nand2  g017(.a(new_n51_), .b(x22), .O(new_n64_));
  nand3  g018(.a(new_n64_), .b(new_n63_), .c(new_n50_), .O(new_n65_));
  inv1   g019(.a(new_n65_), .O(z03));
  inv1   g020(.a(x23), .O(new_n67_));
  aoi21  g021(.a(new_n51_), .b(new_n67_), .c(new_n49_), .O(new_n68_));
  oai21  g022(.a(new_n51_), .b(x12), .c(new_n68_), .O(z04));
  inv1   g023(.a(x24), .O(new_n70_));
  aoi21  g024(.a(new_n51_), .b(new_n70_), .c(new_n49_), .O(new_n71_));
  oai21  g025(.a(new_n51_), .b(x13), .c(new_n71_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  aoi21  g027(.a(new_n51_), .b(x25), .c(new_n49_), .O(new_n74_));
  oai21  g028(.a(new_n51_), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  inv1   g029(.a(new_n75_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  aoi21  g031(.a(new_n51_), .b(x26), .c(new_n49_), .O(new_n78_));
  oai21  g032(.a(new_n51_), .b(new_n77_), .c(new_n78_), .O(new_n79_));
  inv1   g033(.a(new_n79_), .O(z07));
  nor2   g034(.a(new_n49_), .b(new_n51_), .O(z08));
  inv1   g035(.a(x16), .O(new_n82_));
  nand2  g036(.a(x18), .b(x00), .O(new_n83_));
  inv1   g037(.a(x18), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(x08), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(new_n83_), .c(new_n82_), .O(new_n86_));
  inv1   g040(.a(x19), .O(new_n87_));
  nor2   g041(.a(new_n87_), .b(x17), .O(new_n88_));
  nor2   g042(.a(x19), .b(new_n48_), .O(new_n89_));
  oai21  g043(.a(new_n89_), .b(new_n88_), .c(x16), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n86_), .c(new_n50_), .O(new_n91_));
  inv1   g045(.a(new_n91_), .O(z09));
  inv1   g046(.a(x20), .O(new_n93_));
  nor2   g047(.a(x20), .b(x19), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n48_), .O(new_n95_));
  oai21  g049(.a(new_n93_), .b(new_n87_), .c(new_n95_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(x16), .O(new_n97_));
  oai21  g051(.a(new_n93_), .b(new_n82_), .c(x03), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(x17), .O(new_n99_));
  aoi21  g053(.a(new_n84_), .b(new_n55_), .c(x16), .O(new_n100_));
  oai21  g054(.a(new_n84_), .b(x01), .c(new_n100_), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n99_), .c(new_n97_), .O(z10));
  inv1   g056(.a(x21), .O(new_n103_));
  nand3  g057(.a(new_n94_), .b(new_n103_), .c(new_n48_), .O(new_n104_));
  oai21  g058(.a(new_n94_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(x16), .O(new_n106_));
  nand2  g060(.a(x21), .b(x16), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(x03), .c(new_n48_), .O(new_n108_));
  inv1   g062(.a(x02), .O(new_n109_));
  nand2  g063(.a(x18), .b(new_n109_), .O(new_n110_));
  aoi21  g064(.a(new_n84_), .b(new_n59_), .c(x16), .O(new_n111_));
  aoi21  g065(.a(new_n111_), .b(new_n110_), .c(new_n108_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n106_), .O(z11));
  nand2  g067(.a(new_n104_), .b(x22), .O(new_n114_));
  inv1   g068(.a(x22), .O(new_n115_));
  nand4  g069(.a(new_n94_), .b(new_n115_), .c(new_n103_), .d(new_n48_), .O(new_n116_));
  nand3  g070(.a(new_n116_), .b(new_n114_), .c(x16), .O(new_n117_));
  nand2  g071(.a(x18), .b(x03), .O(new_n118_));
  aoi21  g072(.a(new_n84_), .b(x11), .c(x16), .O(new_n119_));
  aoi21  g073(.a(new_n119_), .b(new_n118_), .c(new_n49_), .O(new_n120_));
  and2   g074(.a(new_n120_), .b(new_n117_), .O(z12));
  nor2   g075(.a(x21), .b(x20), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n87_), .O(new_n123_));
  nand2  g077(.a(new_n115_), .b(new_n48_), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n123_), .c(new_n67_), .O(new_n125_));
  nor3   g079(.a(x21), .b(x20), .c(x19), .O(new_n126_));
  nand4  g080(.a(new_n126_), .b(x23), .c(new_n115_), .d(new_n48_), .O(new_n127_));
  nand4  g081(.a(new_n127_), .b(new_n125_), .c(new_n50_), .d(x16), .O(new_n128_));
  inv1   g082(.a(x12), .O(new_n129_));
  nand2  g083(.a(new_n84_), .b(new_n129_), .O(new_n130_));
  inv1   g084(.a(x04), .O(new_n131_));
  nand2  g085(.a(x18), .b(new_n131_), .O(new_n132_));
  nand4  g086(.a(new_n132_), .b(new_n130_), .c(new_n50_), .d(new_n82_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n128_), .O(z13));
  nor2   g088(.a(x23), .b(x22), .O(new_n135_));
  aoi21  g089(.a(new_n135_), .b(new_n126_), .c(new_n70_), .O(new_n136_));
  nor3   g090(.a(new_n116_), .b(x24), .c(x23), .O(new_n137_));
  oai21  g091(.a(new_n137_), .b(new_n136_), .c(x16), .O(new_n138_));
  nand2  g092(.a(x24), .b(x16), .O(new_n139_));
  aoi21  g093(.a(new_n139_), .b(x03), .c(new_n48_), .O(new_n140_));
  inv1   g094(.a(x05), .O(new_n141_));
  nand2  g095(.a(x18), .b(new_n141_), .O(new_n142_));
  inv1   g096(.a(x13), .O(new_n143_));
  aoi21  g097(.a(new_n84_), .b(new_n143_), .c(x16), .O(new_n144_));
  aoi21  g098(.a(new_n144_), .b(new_n142_), .c(new_n140_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n138_), .O(z14));
  nand2  g100(.a(new_n135_), .b(new_n70_), .O(new_n147_));
  nor2   g101(.a(new_n147_), .b(new_n123_), .O(new_n148_));
  aoi21  g102(.a(new_n148_), .b(x25), .c(new_n82_), .O(new_n149_));
  oai21  g103(.a(new_n137_), .b(x25), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(x25), .b(x16), .O(new_n151_));
  aoi21  g105(.a(new_n151_), .b(x03), .c(new_n48_), .O(new_n152_));
  inv1   g106(.a(x06), .O(new_n153_));
  nand2  g107(.a(x18), .b(new_n153_), .O(new_n154_));
  aoi21  g108(.a(new_n84_), .b(new_n73_), .c(x16), .O(new_n155_));
  aoi21  g109(.a(new_n155_), .b(new_n154_), .c(new_n152_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n150_), .O(z15));
  nor2   g111(.a(x25), .b(x24), .O(new_n158_));
  nand4  g112(.a(new_n158_), .b(new_n135_), .c(new_n122_), .d(new_n87_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(x26), .O(new_n160_));
  nor3   g114(.a(x24), .b(x23), .c(x22), .O(new_n161_));
  nor2   g115(.a(x26), .b(x25), .O(new_n162_));
  nand3  g116(.a(new_n162_), .b(new_n161_), .c(new_n126_), .O(new_n163_));
  aoi21  g117(.a(new_n163_), .b(new_n160_), .c(x17), .O(new_n164_));
  nand3  g118(.a(x26), .b(x17), .c(x03), .O(new_n165_));
  inv1   g119(.a(new_n165_), .O(new_n166_));
  oai21  g120(.a(new_n166_), .b(new_n164_), .c(x16), .O(new_n167_));
  nand2  g121(.a(new_n84_), .b(new_n77_), .O(new_n168_));
  inv1   g122(.a(x07), .O(new_n169_));
  nand2  g123(.a(x18), .b(new_n169_), .O(new_n170_));
  nand4  g124(.a(new_n170_), .b(new_n168_), .c(new_n50_), .d(new_n82_), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(new_n167_), .O(z16));
  nand3  g126(.a(x27), .b(x17), .c(x03), .O(new_n173_));
  nand4  g127(.a(new_n162_), .b(new_n161_), .c(new_n122_), .d(new_n88_), .O(new_n174_));
  aoi21  g128(.a(new_n174_), .b(new_n173_), .c(new_n82_), .O(z17));
endmodule


