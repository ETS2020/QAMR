// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n50_, new_n52_, new_n53_, new_n56_, new_n57_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_;
  nor2   g000(.a(x27), .b(x21), .O(new_n47_));
  aoi22  g001(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  nor2   g002(.a(x21), .b(x20), .O(new_n49_));
  nor2   g003(.a(new_n49_), .b(x27), .O(new_n50_));
  aoi21  g004(.a(x27), .b(x09), .c(new_n50_), .O(z01));
  inv1   g005(.a(x21), .O(new_n52_));
  nor2   g006(.a(x27), .b(new_n52_), .O(new_n53_));
  aoi21  g007(.a(x27), .b(x10), .c(new_n53_), .O(z02));
  aoi22  g008(.a(new_n47_), .b(x22), .c(x27), .d(x11), .O(z03));
  inv1   g009(.a(x27), .O(new_n56_));
  inv1   g010(.a(new_n47_), .O(new_n57_));
  oai22  g011(.a(new_n57_), .b(x23), .c(new_n56_), .d(x12), .O(z04));
  oai22  g012(.a(new_n57_), .b(x24), .c(new_n56_), .d(x13), .O(z05));
  aoi22  g013(.a(new_n47_), .b(x25), .c(x27), .d(x14), .O(z06));
  oai22  g014(.a(new_n57_), .b(x26), .c(new_n56_), .d(x15), .O(z07));
  inv1   g015(.a(x18), .O(new_n62_));
  inv1   g016(.a(x08), .O(new_n63_));
  aoi21  g017(.a(new_n62_), .b(new_n63_), .c(x16), .O(new_n64_));
  oai21  g018(.a(new_n62_), .b(x00), .c(new_n64_), .O(new_n65_));
  inv1   g019(.a(x17), .O(new_n66_));
  inv1   g020(.a(x16), .O(new_n67_));
  inv1   g021(.a(x19), .O(new_n68_));
  nor2   g022(.a(new_n68_), .b(x17), .O(new_n69_));
  nor2   g023(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  oai21  g024(.a(x19), .b(new_n66_), .c(new_n70_), .O(new_n71_));
  aoi21  g025(.a(new_n71_), .b(new_n65_), .c(new_n53_), .O(z09));
  inv1   g026(.a(x20), .O(new_n73_));
  nor2   g027(.a(x19), .b(x17), .O(new_n74_));
  nor2   g028(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nor3   g029(.a(x20), .b(x19), .c(x17), .O(new_n76_));
  oai21  g030(.a(new_n76_), .b(new_n75_), .c(x16), .O(new_n77_));
  inv1   g031(.a(x09), .O(new_n78_));
  aoi21  g032(.a(new_n62_), .b(new_n78_), .c(x16), .O(new_n79_));
  oai21  g033(.a(new_n62_), .b(x01), .c(new_n79_), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n77_), .c(new_n53_), .O(z10));
  nand3  g035(.a(new_n73_), .b(new_n68_), .c(new_n66_), .O(new_n82_));
  nor2   g036(.a(new_n56_), .b(new_n52_), .O(new_n83_));
  nand2  g037(.a(new_n74_), .b(new_n49_), .O(new_n84_));
  inv1   g038(.a(new_n84_), .O(new_n85_));
  aoi21  g039(.a(new_n83_), .b(new_n82_), .c(new_n85_), .O(new_n86_));
  nor2   g040(.a(new_n53_), .b(x16), .O(new_n87_));
  inv1   g041(.a(x10), .O(new_n88_));
  nand2  g042(.a(new_n62_), .b(new_n88_), .O(new_n89_));
  inv1   g043(.a(x02), .O(new_n90_));
  nand2  g044(.a(x18), .b(new_n90_), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n89_), .c(new_n87_), .O(new_n92_));
  oai21  g046(.a(new_n86_), .b(new_n67_), .c(new_n92_), .O(z11));
  inv1   g047(.a(x11), .O(new_n94_));
  nand2  g048(.a(new_n62_), .b(new_n94_), .O(new_n95_));
  inv1   g049(.a(x03), .O(new_n96_));
  nand2  g050(.a(x18), .b(new_n96_), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n95_), .c(new_n87_), .O(new_n98_));
  nand2  g052(.a(new_n85_), .b(x22), .O(new_n99_));
  nor2   g053(.a(new_n53_), .b(new_n67_), .O(new_n100_));
  inv1   g054(.a(x22), .O(new_n101_));
  nand2  g055(.a(new_n84_), .b(new_n101_), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n100_), .c(new_n99_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n98_), .O(z12));
  inv1   g058(.a(x12), .O(new_n105_));
  nand2  g059(.a(new_n62_), .b(new_n105_), .O(new_n106_));
  inv1   g060(.a(x04), .O(new_n107_));
  nand2  g061(.a(x18), .b(new_n107_), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n106_), .c(new_n87_), .O(new_n109_));
  nand3  g063(.a(new_n85_), .b(x23), .c(new_n101_), .O(new_n110_));
  inv1   g064(.a(x23), .O(new_n111_));
  oai21  g065(.a(new_n84_), .b(x22), .c(new_n111_), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n110_), .c(new_n100_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n109_), .O(z13));
  nor2   g068(.a(x23), .b(x22), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n76_), .O(new_n116_));
  nor3   g070(.a(x24), .b(x23), .c(x22), .O(new_n117_));
  aoi22  g071(.a(new_n117_), .b(new_n85_), .c(new_n116_), .d(x24), .O(new_n118_));
  nand2  g072(.a(x24), .b(x16), .O(new_n119_));
  aoi21  g073(.a(new_n119_), .b(x27), .c(new_n52_), .O(new_n120_));
  inv1   g074(.a(x05), .O(new_n121_));
  nand2  g075(.a(x18), .b(new_n121_), .O(new_n122_));
  inv1   g076(.a(x13), .O(new_n123_));
  aoi21  g077(.a(new_n62_), .b(new_n123_), .c(x16), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n122_), .c(new_n120_), .O(new_n125_));
  oai21  g079(.a(new_n118_), .b(new_n67_), .c(new_n125_), .O(z14));
  inv1   g080(.a(x24), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(new_n111_), .c(new_n101_), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n82_), .c(x25), .O(new_n129_));
  nor2   g083(.a(x25), .b(x24), .O(new_n130_));
  nand4  g084(.a(new_n130_), .b(new_n115_), .c(new_n74_), .d(new_n73_), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(new_n129_), .c(x21), .O(new_n132_));
  nand2  g086(.a(new_n83_), .b(x25), .O(new_n133_));
  inv1   g087(.a(new_n133_), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  inv1   g089(.a(x14), .O(new_n136_));
  nand2  g090(.a(new_n62_), .b(new_n136_), .O(new_n137_));
  inv1   g091(.a(x06), .O(new_n138_));
  nand2  g092(.a(x18), .b(new_n138_), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n137_), .c(new_n87_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n135_), .O(z15));
  nand2  g095(.a(new_n131_), .b(x26), .O(new_n142_));
  nor2   g096(.a(x26), .b(x25), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n117_), .c(new_n76_), .O(new_n144_));
  aoi21  g098(.a(new_n144_), .b(new_n142_), .c(x21), .O(new_n145_));
  nand2  g099(.a(new_n83_), .b(x26), .O(new_n146_));
  inv1   g100(.a(new_n146_), .O(new_n147_));
  oai21  g101(.a(new_n147_), .b(new_n145_), .c(x16), .O(new_n148_));
  inv1   g102(.a(x15), .O(new_n149_));
  nand2  g103(.a(new_n62_), .b(new_n149_), .O(new_n150_));
  inv1   g104(.a(x07), .O(new_n151_));
  nand2  g105(.a(x18), .b(new_n151_), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n150_), .c(new_n87_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n148_), .O(z16));
  nand2  g108(.a(x27), .b(x17), .O(new_n155_));
  nand4  g109(.a(new_n143_), .b(new_n117_), .c(new_n69_), .d(new_n49_), .O(new_n156_));
  aoi21  g110(.a(new_n156_), .b(new_n155_), .c(new_n67_), .O(z17));
  buf    g111(.a(x27), .O(z08));
endmodule


