// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x16), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  oai22  g03(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  aoi22  g04(.a(new_n48_), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  aoi22  g05(.a(new_n48_), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  oai22  g06(.a(new_n49_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g07(.a(new_n49_), .b(x12), .c(x27), .d(x23), .O(z04));
  aoi22  g08(.a(new_n48_), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  aoi22  g09(.a(new_n48_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g10(.a(new_n49_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g11(.a(x18), .O(new_n58_));
  nor2   g12(.a(new_n58_), .b(x00), .O(new_n59_));
  inv1   g13(.a(x16), .O(new_n60_));
  oai21  g14(.a(x18), .b(x08), .c(new_n60_), .O(new_n61_));
  inv1   g15(.a(x19), .O(new_n62_));
  nand2  g16(.a(new_n62_), .b(x17), .O(new_n63_));
  nand2  g17(.a(new_n47_), .b(x16), .O(new_n64_));
  nor2   g18(.a(new_n62_), .b(x17), .O(new_n65_));
  nor2   g19(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g20(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  oai21  g21(.a(new_n61_), .b(new_n59_), .c(new_n67_), .O(z09));
  inv1   g22(.a(x20), .O(new_n69_));
  nor2   g23(.a(x19), .b(x17), .O(new_n70_));
  aoi21  g24(.a(new_n70_), .b(new_n69_), .c(x27), .O(new_n71_));
  oai21  g25(.a(new_n70_), .b(new_n69_), .c(new_n71_), .O(new_n72_));
  nand2  g26(.a(new_n72_), .b(x16), .O(new_n73_));
  nor2   g27(.a(new_n58_), .b(x01), .O(new_n74_));
  oai21  g28(.a(x18), .b(x09), .c(new_n60_), .O(new_n75_));
  oai21  g29(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(z10));
  inv1   g30(.a(x21), .O(new_n77_));
  aoi21  g31(.a(new_n70_), .b(new_n69_), .c(new_n77_), .O(new_n78_));
  nor2   g32(.a(x21), .b(x20), .O(new_n79_));
  nand2  g33(.a(new_n79_), .b(new_n70_), .O(new_n80_));
  nand2  g34(.a(new_n80_), .b(new_n47_), .O(new_n81_));
  oai21  g35(.a(new_n81_), .b(new_n78_), .c(x16), .O(new_n82_));
  nor2   g36(.a(new_n58_), .b(x02), .O(new_n83_));
  oai21  g37(.a(x18), .b(x10), .c(new_n60_), .O(new_n84_));
  oai21  g38(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(z11));
  xor2a  g39(.a(new_n80_), .b(x22), .O(new_n86_));
  nor2   g40(.a(new_n58_), .b(x03), .O(new_n87_));
  oai21  g41(.a(x18), .b(x11), .c(new_n60_), .O(new_n88_));
  oai22  g42(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n64_), .O(z12));
  inv1   g43(.a(x22), .O(new_n90_));
  inv1   g44(.a(x23), .O(new_n91_));
  nand4  g45(.a(new_n79_), .b(new_n70_), .c(new_n91_), .d(new_n90_), .O(new_n92_));
  oai21  g46(.a(new_n80_), .b(x22), .c(x23), .O(new_n93_));
  nand3  g47(.a(new_n93_), .b(new_n92_), .c(new_n47_), .O(new_n94_));
  nand2  g48(.a(new_n94_), .b(x16), .O(new_n95_));
  inv1   g49(.a(x12), .O(new_n96_));
  aoi21  g50(.a(new_n58_), .b(new_n96_), .c(x16), .O(new_n97_));
  oai21  g51(.a(new_n58_), .b(x04), .c(new_n97_), .O(new_n98_));
  nand2  g52(.a(new_n98_), .b(new_n95_), .O(z13));
  nor2   g53(.a(x24), .b(x23), .O(new_n100_));
  nand4  g54(.a(new_n100_), .b(new_n79_), .c(new_n70_), .d(new_n90_), .O(new_n101_));
  inv1   g55(.a(new_n101_), .O(new_n102_));
  aoi21  g56(.a(new_n92_), .b(x24), .c(new_n102_), .O(new_n103_));
  nor2   g57(.a(new_n58_), .b(x05), .O(new_n104_));
  oai21  g58(.a(x18), .b(x13), .c(new_n60_), .O(new_n105_));
  oai22  g59(.a(new_n105_), .b(new_n104_), .c(new_n103_), .d(new_n64_), .O(z14));
  inv1   g60(.a(new_n80_), .O(new_n107_));
  nor2   g61(.a(x25), .b(x22), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(new_n100_), .O(new_n109_));
  inv1   g63(.a(new_n109_), .O(new_n110_));
  aoi22  g64(.a(new_n110_), .b(new_n107_), .c(new_n101_), .d(x25), .O(new_n111_));
  nor2   g65(.a(new_n58_), .b(x06), .O(new_n112_));
  oai21  g66(.a(x18), .b(x14), .c(new_n60_), .O(new_n113_));
  oai22  g67(.a(new_n113_), .b(new_n112_), .c(new_n111_), .d(new_n64_), .O(z15));
  inv1   g68(.a(x26), .O(new_n115_));
  nand3  g69(.a(new_n110_), .b(new_n107_), .c(new_n115_), .O(new_n116_));
  nand4  g70(.a(new_n108_), .b(new_n100_), .c(new_n79_), .d(new_n70_), .O(new_n117_));
  aoi21  g71(.a(new_n117_), .b(x26), .c(x27), .O(new_n118_));
  nand2  g72(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(x16), .O(new_n120_));
  inv1   g74(.a(x15), .O(new_n121_));
  aoi21  g75(.a(new_n58_), .b(new_n121_), .c(x16), .O(new_n122_));
  oai21  g76(.a(new_n58_), .b(x07), .c(new_n122_), .O(new_n123_));
  nand2  g77(.a(new_n123_), .b(new_n120_), .O(z16));
  nand4  g78(.a(new_n110_), .b(new_n79_), .c(new_n65_), .d(new_n115_), .O(new_n125_));
  aoi21  g79(.a(new_n125_), .b(new_n47_), .c(new_n60_), .O(z17));
  buf    g80(.a(x27), .O(z08));
endmodule


