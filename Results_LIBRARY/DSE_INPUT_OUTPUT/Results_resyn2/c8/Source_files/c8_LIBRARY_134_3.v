// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x07), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  oai22  g03(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  aoi22  g04(.a(new_n48_), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  oai22  g05(.a(new_n49_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g06(.a(new_n49_), .b(x11), .c(x27), .d(x22), .O(z03));
  aoi22  g07(.a(new_n48_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  aoi22  g08(.a(new_n48_), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  aoi22  g09(.a(new_n48_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  inv1   g10(.a(x26), .O(new_n57_));
  inv1   g11(.a(x07), .O(new_n58_));
  nor2   g12(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  aoi21  g13(.a(new_n47_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  oai21  g14(.a(new_n47_), .b(x15), .c(new_n60_), .O(z07));
  inv1   g15(.a(new_n59_), .O(new_n62_));
  inv1   g16(.a(x16), .O(new_n63_));
  nand2  g17(.a(x18), .b(x00), .O(new_n64_));
  inv1   g18(.a(x18), .O(new_n65_));
  nand2  g19(.a(new_n65_), .b(x08), .O(new_n66_));
  nand3  g20(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand2  g21(.a(x19), .b(x17), .O(new_n68_));
  nor2   g22(.a(x19), .b(x17), .O(new_n69_));
  nor2   g23(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nand2  g24(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand2  g25(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand2  g26(.a(new_n72_), .b(new_n62_), .O(z09));
  xor2a  g27(.a(new_n69_), .b(x20), .O(new_n74_));
  nand2  g28(.a(new_n74_), .b(x16), .O(new_n75_));
  inv1   g29(.a(x09), .O(new_n76_));
  aoi21  g30(.a(new_n65_), .b(new_n76_), .c(x16), .O(new_n77_));
  oai21  g31(.a(new_n65_), .b(x01), .c(new_n77_), .O(new_n78_));
  nand3  g32(.a(new_n78_), .b(new_n75_), .c(new_n62_), .O(z10));
  inv1   g33(.a(x20), .O(new_n80_));
  nand2  g34(.a(new_n69_), .b(new_n80_), .O(new_n81_));
  nor2   g35(.a(x21), .b(x20), .O(new_n82_));
  aoi22  g36(.a(new_n82_), .b(new_n69_), .c(new_n81_), .d(x21), .O(new_n83_));
  inv1   g37(.a(x02), .O(new_n84_));
  nand2  g38(.a(x18), .b(new_n84_), .O(new_n85_));
  inv1   g39(.a(x10), .O(new_n86_));
  aoi21  g40(.a(new_n65_), .b(new_n86_), .c(x16), .O(new_n87_));
  aoi21  g41(.a(new_n87_), .b(new_n85_), .c(new_n59_), .O(new_n88_));
  oai21  g42(.a(new_n83_), .b(new_n63_), .c(new_n88_), .O(z11));
  inv1   g43(.a(x22), .O(new_n90_));
  aoi21  g44(.a(new_n82_), .b(new_n69_), .c(new_n90_), .O(new_n91_));
  nand3  g45(.a(new_n82_), .b(new_n69_), .c(new_n90_), .O(new_n92_));
  inv1   g46(.a(new_n92_), .O(new_n93_));
  oai21  g47(.a(new_n93_), .b(new_n91_), .c(x16), .O(new_n94_));
  inv1   g48(.a(x11), .O(new_n95_));
  aoi21  g49(.a(new_n65_), .b(new_n95_), .c(x16), .O(new_n96_));
  oai21  g50(.a(new_n65_), .b(x03), .c(new_n96_), .O(new_n97_));
  aoi21  g51(.a(new_n97_), .b(new_n94_), .c(new_n59_), .O(z12));
  xor2a  g52(.a(new_n92_), .b(x23), .O(new_n99_));
  inv1   g53(.a(x04), .O(new_n100_));
  nand2  g54(.a(x18), .b(new_n100_), .O(new_n101_));
  inv1   g55(.a(x12), .O(new_n102_));
  aoi21  g56(.a(new_n65_), .b(new_n102_), .c(x16), .O(new_n103_));
  aoi21  g57(.a(new_n103_), .b(new_n101_), .c(new_n59_), .O(new_n104_));
  oai21  g58(.a(new_n99_), .b(new_n63_), .c(new_n104_), .O(z13));
  inv1   g59(.a(x23), .O(new_n106_));
  nand4  g60(.a(new_n82_), .b(new_n69_), .c(new_n106_), .d(new_n90_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(x24), .O(new_n108_));
  nor2   g62(.a(x24), .b(x23), .O(new_n109_));
  nand4  g63(.a(new_n109_), .b(new_n82_), .c(new_n69_), .d(new_n90_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand2  g65(.a(new_n111_), .b(x16), .O(new_n112_));
  inv1   g66(.a(x05), .O(new_n113_));
  nand2  g67(.a(x18), .b(new_n113_), .O(new_n114_));
  inv1   g68(.a(x13), .O(new_n115_));
  aoi21  g69(.a(new_n65_), .b(new_n115_), .c(x16), .O(new_n116_));
  aoi21  g70(.a(new_n116_), .b(new_n114_), .c(new_n59_), .O(new_n117_));
  nand2  g71(.a(new_n117_), .b(new_n112_), .O(z14));
  nand2  g72(.a(new_n110_), .b(x25), .O(new_n119_));
  nor2   g73(.a(x25), .b(x22), .O(new_n120_));
  nand4  g74(.a(new_n120_), .b(new_n109_), .c(new_n82_), .d(new_n69_), .O(new_n121_));
  inv1   g75(.a(new_n121_), .O(new_n122_));
  nor2   g76(.a(new_n122_), .b(new_n63_), .O(new_n123_));
  nand2  g77(.a(x18), .b(x06), .O(new_n124_));
  nand2  g78(.a(new_n65_), .b(x14), .O(new_n125_));
  nand3  g79(.a(new_n125_), .b(new_n124_), .c(new_n63_), .O(new_n126_));
  nand2  g80(.a(new_n126_), .b(new_n62_), .O(new_n127_));
  aoi21  g81(.a(new_n123_), .b(new_n119_), .c(new_n127_), .O(z15));
  nand2  g82(.a(new_n122_), .b(new_n57_), .O(new_n129_));
  aoi21  g83(.a(new_n121_), .b(x26), .c(new_n63_), .O(new_n130_));
  aoi21  g84(.a(new_n65_), .b(x15), .c(x16), .O(new_n131_));
  oai21  g85(.a(new_n65_), .b(new_n58_), .c(new_n131_), .O(new_n132_));
  nand2  g86(.a(new_n132_), .b(new_n62_), .O(new_n133_));
  aoi21  g87(.a(new_n130_), .b(new_n129_), .c(new_n133_), .O(z16));
  nand2  g88(.a(x27), .b(x17), .O(new_n135_));
  nand2  g89(.a(new_n120_), .b(new_n109_), .O(new_n136_));
  inv1   g90(.a(new_n136_), .O(new_n137_));
  inv1   g91(.a(x19), .O(new_n138_));
  nor2   g92(.a(new_n138_), .b(x17), .O(new_n139_));
  nand4  g93(.a(new_n139_), .b(new_n137_), .c(new_n82_), .d(new_n57_), .O(new_n140_));
  nand2  g94(.a(new_n62_), .b(x16), .O(new_n141_));
  aoi21  g95(.a(new_n140_), .b(new_n135_), .c(new_n141_), .O(z17));
  buf    g96(.a(x27), .O(z08));
endmodule


