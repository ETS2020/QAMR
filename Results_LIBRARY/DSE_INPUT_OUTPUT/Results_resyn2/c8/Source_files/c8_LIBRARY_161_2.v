// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n54_, new_n55_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(x27), .b(x23), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(z08));
  oai22  g003(.a(z08), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g004(.a(z08), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  oai22  g005(.a(z08), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  aoi22  g006(.a(new_n48_), .b(x22), .c(x27), .d(x11), .O(z03));
  inv1   g007(.a(x23), .O(new_n54_));
  nor2   g008(.a(x27), .b(new_n54_), .O(new_n55_));
  aoi21  g009(.a(x27), .b(x12), .c(new_n55_), .O(z04));
  aoi22  g010(.a(new_n48_), .b(x24), .c(x27), .d(x13), .O(z05));
  aoi22  g011(.a(new_n48_), .b(x25), .c(x27), .d(x14), .O(z06));
  oai22  g012(.a(z08), .b(x26), .c(new_n47_), .d(x15), .O(z07));
  inv1   g013(.a(new_n55_), .O(new_n60_));
  and2   g014(.a(x19), .b(x17), .O(new_n61_));
  nor2   g015(.a(x19), .b(x17), .O(new_n62_));
  oai21  g016(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g017(.a(x18), .O(new_n64_));
  inv1   g018(.a(x08), .O(new_n65_));
  aoi21  g019(.a(new_n64_), .b(new_n65_), .c(x16), .O(new_n66_));
  oai21  g020(.a(new_n64_), .b(x00), .c(new_n66_), .O(new_n67_));
  nand3  g021(.a(new_n67_), .b(new_n63_), .c(new_n60_), .O(z09));
  inv1   g022(.a(x20), .O(new_n69_));
  nor2   g023(.a(new_n62_), .b(new_n69_), .O(new_n70_));
  nor3   g024(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  oai21  g025(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  inv1   g026(.a(x01), .O(new_n73_));
  nand2  g027(.a(x18), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x09), .O(new_n75_));
  aoi21  g029(.a(new_n64_), .b(new_n75_), .c(x16), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n55_), .O(new_n77_));
  nand2  g031(.a(new_n77_), .b(new_n72_), .O(z10));
  inv1   g032(.a(x21), .O(new_n79_));
  nor2   g033(.a(x21), .b(x20), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n62_), .O(new_n81_));
  oai21  g035(.a(new_n71_), .b(new_n79_), .c(new_n81_), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(x16), .O(new_n83_));
  inv1   g037(.a(x10), .O(new_n84_));
  aoi21  g038(.a(new_n64_), .b(new_n84_), .c(x16), .O(new_n85_));
  oai21  g039(.a(new_n64_), .b(x02), .c(new_n85_), .O(new_n86_));
  aoi21  g040(.a(new_n86_), .b(new_n83_), .c(new_n55_), .O(z11));
  inv1   g041(.a(x16), .O(new_n88_));
  nand2  g042(.a(x18), .b(x03), .O(new_n89_));
  nand2  g043(.a(new_n64_), .b(x11), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  inv1   g045(.a(x22), .O(new_n92_));
  nand3  g046(.a(new_n80_), .b(new_n62_), .c(new_n92_), .O(new_n93_));
  nand2  g047(.a(new_n81_), .b(x22), .O(new_n94_));
  nand3  g048(.a(new_n94_), .b(new_n93_), .c(x16), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n60_), .O(z12));
  nor2   g051(.a(new_n55_), .b(x16), .O(new_n98_));
  inv1   g052(.a(x12), .O(new_n99_));
  nand2  g053(.a(new_n64_), .b(new_n99_), .O(new_n100_));
  inv1   g054(.a(x04), .O(new_n101_));
  nand2  g055(.a(x18), .b(new_n101_), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n100_), .c(new_n98_), .O(new_n103_));
  inv1   g057(.a(new_n93_), .O(new_n104_));
  nor2   g058(.a(new_n47_), .b(new_n54_), .O(new_n105_));
  nor2   g059(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  oai21  g060(.a(new_n93_), .b(new_n54_), .c(x16), .O(new_n107_));
  oai21  g061(.a(new_n107_), .b(new_n106_), .c(new_n103_), .O(z13));
  nand2  g062(.a(new_n93_), .b(x24), .O(new_n109_));
  nor2   g063(.a(x24), .b(x22), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n80_), .c(new_n62_), .O(new_n111_));
  aoi21  g065(.a(new_n111_), .b(new_n109_), .c(x23), .O(new_n112_));
  nand2  g066(.a(new_n105_), .b(x24), .O(new_n113_));
  inv1   g067(.a(new_n113_), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n112_), .c(x16), .O(new_n115_));
  inv1   g069(.a(x13), .O(new_n116_));
  nand2  g070(.a(new_n64_), .b(new_n116_), .O(new_n117_));
  inv1   g071(.a(x05), .O(new_n118_));
  nand2  g072(.a(x18), .b(new_n118_), .O(new_n119_));
  nand3  g073(.a(new_n119_), .b(new_n117_), .c(new_n98_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n115_), .O(z14));
  nand2  g075(.a(new_n111_), .b(x25), .O(new_n122_));
  nor2   g076(.a(x25), .b(x21), .O(new_n123_));
  nand4  g077(.a(new_n123_), .b(new_n110_), .c(new_n62_), .d(new_n69_), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n122_), .c(x23), .O(new_n125_));
  nand2  g079(.a(new_n105_), .b(x25), .O(new_n126_));
  inv1   g080(.a(new_n126_), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n125_), .c(x16), .O(new_n128_));
  inv1   g082(.a(x14), .O(new_n129_));
  nand2  g083(.a(new_n64_), .b(new_n129_), .O(new_n130_));
  inv1   g084(.a(x06), .O(new_n131_));
  nand2  g085(.a(x18), .b(new_n131_), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(new_n130_), .c(new_n98_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n128_), .O(z15));
  nand2  g088(.a(new_n124_), .b(x26), .O(new_n135_));
  nor2   g089(.a(x26), .b(x25), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n110_), .c(new_n80_), .d(new_n62_), .O(new_n137_));
  aoi21  g091(.a(new_n137_), .b(new_n135_), .c(x23), .O(new_n138_));
  nand2  g092(.a(new_n105_), .b(x26), .O(new_n139_));
  inv1   g093(.a(new_n139_), .O(new_n140_));
  oai21  g094(.a(new_n140_), .b(new_n138_), .c(x16), .O(new_n141_));
  inv1   g095(.a(x15), .O(new_n142_));
  nand2  g096(.a(new_n64_), .b(new_n142_), .O(new_n143_));
  inv1   g097(.a(x07), .O(new_n144_));
  nand2  g098(.a(x18), .b(new_n144_), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n143_), .c(new_n98_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n141_), .O(z16));
  nand2  g101(.a(x27), .b(x17), .O(new_n148_));
  inv1   g102(.a(x17), .O(new_n149_));
  nand3  g103(.a(new_n54_), .b(x19), .c(new_n149_), .O(new_n150_));
  inv1   g104(.a(new_n150_), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n136_), .c(new_n110_), .d(new_n80_), .O(new_n152_));
  aoi21  g106(.a(new_n152_), .b(new_n148_), .c(new_n88_), .O(z17));
endmodule


