// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  nand2  g000(.a(x27), .b(x13), .O(new_n47_));
  oai22  g001(.a(new_n47_), .b(x08), .c(x27), .d(x19), .O(z00));
  inv1   g002(.a(x27), .O(new_n49_));
  inv1   g003(.a(new_n47_), .O(z08));
  aoi22  g004(.a(z08), .b(x09), .c(new_n49_), .d(x20), .O(z01));
  aoi22  g005(.a(z08), .b(x10), .c(new_n49_), .d(x21), .O(z02));
  oai22  g006(.a(new_n47_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g007(.a(new_n47_), .b(x12), .c(x27), .d(x23), .O(z04));
  aoi21  g008(.a(new_n49_), .b(x24), .c(z08), .O(z05));
  oai22  g009(.a(new_n47_), .b(x14), .c(x27), .d(x25), .O(z06));
  aoi22  g010(.a(z08), .b(x15), .c(new_n49_), .d(x26), .O(z07));
  nor2   g011(.a(new_n49_), .b(x13), .O(new_n58_));
  inv1   g012(.a(new_n58_), .O(new_n59_));
  inv1   g013(.a(x08), .O(new_n60_));
  inv1   g014(.a(x16), .O(new_n61_));
  oai21  g015(.a(x18), .b(new_n60_), .c(new_n61_), .O(new_n62_));
  aoi21  g016(.a(x18), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g017(.a(x17), .O(new_n64_));
  inv1   g018(.a(x19), .O(new_n65_));
  nor2   g019(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nor2   g020(.a(x19), .b(x17), .O(new_n67_));
  nor3   g021(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(new_n68_));
  oai21  g022(.a(new_n68_), .b(new_n63_), .c(new_n59_), .O(z09));
  xor2a  g023(.a(new_n67_), .b(x20), .O(new_n70_));
  nand2  g024(.a(new_n70_), .b(x16), .O(new_n71_));
  inv1   g025(.a(x18), .O(new_n72_));
  inv1   g026(.a(x09), .O(new_n73_));
  aoi21  g027(.a(new_n72_), .b(new_n73_), .c(x16), .O(new_n74_));
  oai21  g028(.a(new_n72_), .b(x01), .c(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n71_), .c(new_n58_), .O(z10));
  inv1   g030(.a(x20), .O(new_n77_));
  inv1   g031(.a(x21), .O(new_n78_));
  aoi21  g032(.a(new_n67_), .b(new_n77_), .c(new_n78_), .O(new_n79_));
  nor2   g033(.a(x21), .b(x20), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n67_), .O(new_n81_));
  inv1   g035(.a(new_n81_), .O(new_n82_));
  oai21  g036(.a(new_n82_), .b(new_n79_), .c(x16), .O(new_n83_));
  inv1   g037(.a(x10), .O(new_n84_));
  aoi21  g038(.a(new_n72_), .b(new_n84_), .c(x16), .O(new_n85_));
  oai21  g039(.a(new_n72_), .b(x02), .c(new_n85_), .O(new_n86_));
  aoi21  g040(.a(new_n86_), .b(new_n83_), .c(new_n58_), .O(z11));
  inv1   g041(.a(new_n67_), .O(new_n88_));
  inv1   g042(.a(x22), .O(new_n89_));
  nand2  g043(.a(new_n80_), .b(new_n89_), .O(new_n90_));
  nor2   g044(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  aoi21  g045(.a(new_n81_), .b(x22), .c(new_n91_), .O(new_n92_));
  inv1   g046(.a(x03), .O(new_n93_));
  nand2  g047(.a(x18), .b(new_n93_), .O(new_n94_));
  inv1   g048(.a(x11), .O(new_n95_));
  aoi21  g049(.a(new_n72_), .b(new_n95_), .c(x16), .O(new_n96_));
  aoi21  g050(.a(new_n96_), .b(new_n94_), .c(new_n58_), .O(new_n97_));
  oai21  g051(.a(new_n92_), .b(new_n61_), .c(new_n97_), .O(z12));
  inv1   g052(.a(x23), .O(new_n99_));
  nor2   g053(.a(x23), .b(x22), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n80_), .c(new_n67_), .O(new_n101_));
  oai21  g055(.a(new_n91_), .b(new_n99_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(x16), .O(new_n103_));
  inv1   g057(.a(x04), .O(new_n104_));
  nand2  g058(.a(x18), .b(new_n104_), .O(new_n105_));
  inv1   g059(.a(x12), .O(new_n106_));
  aoi21  g060(.a(new_n72_), .b(new_n106_), .c(x16), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n105_), .c(new_n58_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n103_), .O(z13));
  nand2  g063(.a(new_n101_), .b(x24), .O(new_n110_));
  nor2   g064(.a(x24), .b(x23), .O(new_n111_));
  nand4  g065(.a(new_n111_), .b(new_n80_), .c(new_n67_), .d(new_n89_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(x16), .O(new_n114_));
  inv1   g068(.a(x05), .O(new_n115_));
  nand2  g069(.a(x18), .b(new_n115_), .O(new_n116_));
  inv1   g070(.a(x13), .O(new_n117_));
  aoi21  g071(.a(new_n72_), .b(new_n117_), .c(x16), .O(new_n118_));
  aoi21  g072(.a(new_n118_), .b(new_n116_), .c(new_n58_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n114_), .O(z14));
  nand2  g074(.a(new_n112_), .b(x25), .O(new_n121_));
  nor2   g075(.a(x25), .b(x24), .O(new_n122_));
  nand4  g076(.a(new_n122_), .b(new_n100_), .c(new_n80_), .d(new_n67_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(x16), .O(new_n125_));
  inv1   g079(.a(x14), .O(new_n126_));
  aoi21  g080(.a(new_n72_), .b(new_n126_), .c(x16), .O(new_n127_));
  oai21  g081(.a(new_n72_), .b(x06), .c(new_n127_), .O(new_n128_));
  aoi21  g082(.a(new_n128_), .b(new_n125_), .c(new_n58_), .O(z15));
  inv1   g083(.a(x26), .O(new_n130_));
  nand3  g084(.a(new_n122_), .b(new_n100_), .c(new_n130_), .O(new_n131_));
  nor2   g085(.a(new_n131_), .b(new_n81_), .O(new_n132_));
  aoi21  g086(.a(new_n123_), .b(x26), .c(new_n132_), .O(new_n133_));
  inv1   g087(.a(x07), .O(new_n134_));
  nand2  g088(.a(x18), .b(new_n134_), .O(new_n135_));
  inv1   g089(.a(x15), .O(new_n136_));
  aoi21  g090(.a(new_n72_), .b(new_n136_), .c(x16), .O(new_n137_));
  aoi21  g091(.a(new_n137_), .b(new_n135_), .c(new_n58_), .O(new_n138_));
  oai21  g092(.a(new_n133_), .b(new_n61_), .c(new_n138_), .O(z16));
  nand3  g093(.a(new_n80_), .b(x19), .c(new_n64_), .O(new_n140_));
  oai22  g094(.a(new_n140_), .b(new_n131_), .c(new_n49_), .d(new_n64_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x13), .O(new_n142_));
  inv1   g096(.a(new_n90_), .O(new_n143_));
  nor2   g097(.a(new_n65_), .b(x17), .O(new_n144_));
  nor3   g098(.a(x27), .b(x26), .c(x25), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n144_), .c(new_n111_), .d(new_n143_), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n142_), .c(new_n61_), .O(z17));
endmodule


