// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_;
  nand2  g00(.a(x27), .b(x14), .O(new_n47_));
  oai22  g01(.a(new_n47_), .b(x08), .c(x27), .d(x19), .O(z00));
  inv1   g02(.a(x27), .O(new_n49_));
  inv1   g03(.a(new_n47_), .O(new_n50_));
  aoi22  g04(.a(new_n50_), .b(x09), .c(new_n49_), .d(x20), .O(z01));
  oai22  g05(.a(new_n47_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g06(.a(new_n47_), .b(x11), .c(x27), .d(x22), .O(z03));
  aoi22  g07(.a(new_n50_), .b(x12), .c(new_n49_), .d(x23), .O(z04));
  aoi22  g08(.a(new_n50_), .b(x13), .c(new_n49_), .d(x24), .O(z05));
  nor2   g09(.a(x27), .b(x25), .O(z06));
  oai22  g10(.a(new_n47_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g11(.a(new_n49_), .b(x14), .O(new_n58_));
  inv1   g12(.a(x18), .O(new_n59_));
  inv1   g13(.a(x08), .O(new_n60_));
  aoi21  g14(.a(new_n59_), .b(new_n60_), .c(x16), .O(new_n61_));
  oai21  g15(.a(new_n59_), .b(x00), .c(new_n61_), .O(new_n62_));
  inv1   g16(.a(x19), .O(new_n63_));
  nand2  g17(.a(new_n63_), .b(x17), .O(new_n64_));
  inv1   g18(.a(x16), .O(new_n65_));
  nor2   g19(.a(new_n63_), .b(x17), .O(new_n66_));
  nor2   g20(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g21(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  aoi21  g22(.a(new_n68_), .b(new_n62_), .c(new_n58_), .O(z09));
  inv1   g23(.a(x20), .O(new_n70_));
  nor2   g24(.a(x19), .b(x17), .O(new_n71_));
  xor2a  g25(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g26(.a(x01), .O(new_n73_));
  nand2  g27(.a(x18), .b(new_n73_), .O(new_n74_));
  inv1   g28(.a(x09), .O(new_n75_));
  aoi21  g29(.a(new_n59_), .b(new_n75_), .c(x16), .O(new_n76_));
  aoi21  g30(.a(new_n76_), .b(new_n74_), .c(new_n58_), .O(new_n77_));
  oai21  g31(.a(new_n72_), .b(new_n65_), .c(new_n77_), .O(z10));
  nand2  g32(.a(new_n71_), .b(new_n70_), .O(new_n79_));
  nor2   g33(.a(x21), .b(x20), .O(new_n80_));
  aoi22  g34(.a(new_n80_), .b(new_n71_), .c(new_n79_), .d(x21), .O(new_n81_));
  inv1   g35(.a(x02), .O(new_n82_));
  nand2  g36(.a(x18), .b(new_n82_), .O(new_n83_));
  inv1   g37(.a(x10), .O(new_n84_));
  aoi21  g38(.a(new_n59_), .b(new_n84_), .c(x16), .O(new_n85_));
  aoi21  g39(.a(new_n85_), .b(new_n83_), .c(new_n58_), .O(new_n86_));
  oai21  g40(.a(new_n81_), .b(new_n65_), .c(new_n86_), .O(z11));
  inv1   g41(.a(x22), .O(new_n88_));
  aoi21  g42(.a(new_n80_), .b(new_n71_), .c(new_n88_), .O(new_n89_));
  nand3  g43(.a(new_n80_), .b(new_n71_), .c(new_n88_), .O(new_n90_));
  inv1   g44(.a(new_n90_), .O(new_n91_));
  oai21  g45(.a(new_n91_), .b(new_n89_), .c(x16), .O(new_n92_));
  inv1   g46(.a(x11), .O(new_n93_));
  aoi21  g47(.a(new_n59_), .b(new_n93_), .c(x16), .O(new_n94_));
  oai21  g48(.a(new_n59_), .b(x03), .c(new_n94_), .O(new_n95_));
  aoi21  g49(.a(new_n95_), .b(new_n92_), .c(new_n58_), .O(z12));
  nand2  g50(.a(new_n90_), .b(x23), .O(new_n97_));
  nor2   g51(.a(x23), .b(x22), .O(new_n98_));
  nand3  g52(.a(new_n98_), .b(new_n80_), .c(new_n71_), .O(new_n99_));
  nand2  g53(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nand2  g54(.a(new_n100_), .b(x16), .O(new_n101_));
  inv1   g55(.a(x12), .O(new_n102_));
  aoi21  g56(.a(new_n59_), .b(new_n102_), .c(x16), .O(new_n103_));
  oai21  g57(.a(new_n59_), .b(x04), .c(new_n103_), .O(new_n104_));
  aoi21  g58(.a(new_n104_), .b(new_n101_), .c(new_n58_), .O(z13));
  xor2a  g59(.a(new_n99_), .b(x24), .O(new_n106_));
  inv1   g60(.a(x05), .O(new_n107_));
  nand2  g61(.a(x18), .b(new_n107_), .O(new_n108_));
  inv1   g62(.a(x13), .O(new_n109_));
  aoi21  g63(.a(new_n59_), .b(new_n109_), .c(x16), .O(new_n110_));
  aoi21  g64(.a(new_n110_), .b(new_n108_), .c(new_n58_), .O(new_n111_));
  oai21  g65(.a(new_n106_), .b(new_n65_), .c(new_n111_), .O(z14));
  inv1   g66(.a(x24), .O(new_n113_));
  nand4  g67(.a(new_n98_), .b(new_n80_), .c(new_n71_), .d(new_n113_), .O(new_n114_));
  nand2  g68(.a(new_n80_), .b(new_n71_), .O(new_n115_));
  nor2   g69(.a(x25), .b(x24), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n98_), .O(new_n117_));
  nor2   g71(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  aoi21  g72(.a(new_n114_), .b(x25), .c(new_n118_), .O(new_n119_));
  inv1   g73(.a(x06), .O(new_n120_));
  nand2  g74(.a(x18), .b(new_n120_), .O(new_n121_));
  inv1   g75(.a(x14), .O(new_n122_));
  aoi21  g76(.a(new_n59_), .b(new_n122_), .c(x16), .O(new_n123_));
  aoi21  g77(.a(new_n123_), .b(new_n121_), .c(new_n58_), .O(new_n124_));
  oai21  g78(.a(new_n119_), .b(new_n65_), .c(new_n124_), .O(z15));
  inv1   g79(.a(x26), .O(new_n126_));
  nor3   g80(.a(new_n117_), .b(new_n115_), .c(new_n126_), .O(new_n127_));
  oai21  g81(.a(new_n118_), .b(x26), .c(x16), .O(new_n128_));
  inv1   g82(.a(x07), .O(new_n129_));
  nand2  g83(.a(x18), .b(new_n129_), .O(new_n130_));
  inv1   g84(.a(x15), .O(new_n131_));
  aoi21  g85(.a(new_n59_), .b(new_n131_), .c(x16), .O(new_n132_));
  aoi21  g86(.a(new_n132_), .b(new_n130_), .c(new_n58_), .O(new_n133_));
  oai21  g87(.a(new_n128_), .b(new_n127_), .c(new_n133_), .O(z16));
  nand2  g88(.a(x27), .b(x17), .O(new_n135_));
  inv1   g89(.a(new_n117_), .O(new_n136_));
  nand4  g90(.a(new_n136_), .b(new_n80_), .c(new_n66_), .d(new_n126_), .O(new_n137_));
  oai21  g91(.a(new_n49_), .b(x14), .c(x16), .O(new_n138_));
  aoi21  g92(.a(new_n137_), .b(new_n135_), .c(new_n138_), .O(z17));
  buf    g93(.a(x27), .O(z08));
endmodule


