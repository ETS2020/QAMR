// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(x27), .b(x13), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  oai22  g03(.a(new_n49_), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  aoi22  g04(.a(new_n48_), .b(x20), .c(x27), .d(x09), .O(z01));
  aoi22  g05(.a(new_n48_), .b(x21), .c(x27), .d(x10), .O(z02));
  oai22  g06(.a(new_n49_), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  oai22  g07(.a(new_n49_), .b(x23), .c(new_n47_), .d(x12), .O(z04));
  inv1   g08(.a(x13), .O(new_n55_));
  nor2   g09(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g10(.a(new_n56_), .O(new_n57_));
  inv1   g11(.a(x24), .O(new_n58_));
  oai21  g12(.a(x27), .b(new_n58_), .c(new_n55_), .O(new_n59_));
  nand2  g13(.a(new_n59_), .b(new_n57_), .O(z05));
  oai22  g14(.a(new_n49_), .b(x25), .c(new_n47_), .d(x14), .O(z06));
  aoi22  g15(.a(new_n48_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g16(.a(x18), .O(new_n63_));
  inv1   g17(.a(x08), .O(new_n64_));
  aoi21  g18(.a(new_n63_), .b(new_n64_), .c(x16), .O(new_n65_));
  oai21  g19(.a(new_n63_), .b(x00), .c(new_n65_), .O(new_n66_));
  inv1   g20(.a(x17), .O(new_n67_));
  inv1   g21(.a(x19), .O(new_n68_));
  nor2   g22(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nor2   g23(.a(x19), .b(x17), .O(new_n70_));
  oai21  g24(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  nand3  g25(.a(new_n71_), .b(new_n66_), .c(new_n57_), .O(z09));
  inv1   g26(.a(x20), .O(new_n73_));
  nor2   g27(.a(new_n70_), .b(new_n73_), .O(new_n74_));
  nor3   g28(.a(x20), .b(x19), .c(x17), .O(new_n75_));
  oai21  g29(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  inv1   g30(.a(x09), .O(new_n77_));
  aoi21  g31(.a(new_n63_), .b(new_n77_), .c(x16), .O(new_n78_));
  oai21  g32(.a(new_n63_), .b(x01), .c(new_n78_), .O(new_n79_));
  aoi21  g33(.a(new_n79_), .b(new_n76_), .c(new_n56_), .O(z10));
  inv1   g34(.a(x21), .O(new_n81_));
  aoi21  g35(.a(new_n70_), .b(new_n73_), .c(new_n81_), .O(new_n82_));
  nand3  g36(.a(new_n70_), .b(new_n81_), .c(new_n73_), .O(new_n83_));
  inv1   g37(.a(new_n83_), .O(new_n84_));
  oai21  g38(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  inv1   g39(.a(x10), .O(new_n86_));
  aoi21  g40(.a(new_n63_), .b(new_n86_), .c(x16), .O(new_n87_));
  oai21  g41(.a(new_n63_), .b(x02), .c(new_n87_), .O(new_n88_));
  aoi21  g42(.a(new_n88_), .b(new_n85_), .c(new_n56_), .O(z11));
  inv1   g43(.a(x11), .O(new_n90_));
  inv1   g44(.a(x16), .O(new_n91_));
  oai21  g45(.a(x18), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  aoi21  g46(.a(x18), .b(x03), .c(new_n92_), .O(new_n93_));
  nor2   g47(.a(x22), .b(x21), .O(new_n94_));
  nand3  g48(.a(new_n94_), .b(new_n70_), .c(new_n73_), .O(new_n95_));
  nand2  g49(.a(new_n95_), .b(x16), .O(new_n96_));
  aoi21  g50(.a(new_n83_), .b(x22), .c(new_n96_), .O(new_n97_));
  oai21  g51(.a(new_n97_), .b(new_n93_), .c(new_n57_), .O(z12));
  xor2a  g52(.a(new_n95_), .b(x23), .O(new_n99_));
  inv1   g53(.a(x04), .O(new_n100_));
  nand2  g54(.a(x18), .b(new_n100_), .O(new_n101_));
  inv1   g55(.a(x12), .O(new_n102_));
  aoi21  g56(.a(new_n63_), .b(new_n102_), .c(x16), .O(new_n103_));
  aoi21  g57(.a(new_n103_), .b(new_n101_), .c(new_n56_), .O(new_n104_));
  oai21  g58(.a(new_n99_), .b(new_n91_), .c(new_n104_), .O(z13));
  inv1   g59(.a(x23), .O(new_n106_));
  nand4  g60(.a(new_n94_), .b(new_n70_), .c(new_n106_), .d(new_n73_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(x24), .O(new_n108_));
  nor2   g62(.a(x24), .b(x23), .O(new_n109_));
  nand3  g63(.a(new_n109_), .b(new_n94_), .c(new_n75_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand2  g65(.a(new_n111_), .b(x16), .O(new_n112_));
  oai21  g66(.a(x18), .b(x16), .c(x27), .O(new_n113_));
  inv1   g67(.a(x05), .O(new_n114_));
  nor2   g68(.a(x16), .b(new_n114_), .O(new_n115_));
  aoi22  g69(.a(new_n115_), .b(x18), .c(new_n113_), .d(x13), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n112_), .O(z14));
  nand2  g71(.a(new_n110_), .b(x25), .O(new_n118_));
  inv1   g72(.a(x22), .O(new_n119_));
  inv1   g73(.a(x25), .O(new_n120_));
  nand3  g74(.a(new_n109_), .b(new_n120_), .c(new_n119_), .O(new_n121_));
  inv1   g75(.a(new_n121_), .O(new_n122_));
  aoi21  g76(.a(new_n122_), .b(new_n84_), .c(new_n91_), .O(new_n123_));
  nand2  g77(.a(x18), .b(x06), .O(new_n124_));
  nand2  g78(.a(new_n63_), .b(x14), .O(new_n125_));
  nand3  g79(.a(new_n125_), .b(new_n124_), .c(new_n91_), .O(new_n126_));
  nand2  g80(.a(new_n126_), .b(new_n57_), .O(new_n127_));
  aoi21  g81(.a(new_n123_), .b(new_n118_), .c(new_n127_), .O(z15));
  nand3  g82(.a(new_n122_), .b(new_n84_), .c(x26), .O(new_n129_));
  inv1   g83(.a(x26), .O(new_n130_));
  oai21  g84(.a(new_n121_), .b(new_n83_), .c(new_n130_), .O(new_n131_));
  nand3  g85(.a(new_n131_), .b(new_n129_), .c(x16), .O(new_n132_));
  inv1   g86(.a(x07), .O(new_n133_));
  nand2  g87(.a(x18), .b(new_n133_), .O(new_n134_));
  inv1   g88(.a(x15), .O(new_n135_));
  aoi21  g89(.a(new_n63_), .b(new_n135_), .c(x16), .O(new_n136_));
  aoi21  g90(.a(new_n136_), .b(new_n134_), .c(new_n56_), .O(new_n137_));
  nand2  g91(.a(new_n137_), .b(new_n132_), .O(z16));
  nor2   g92(.a(x20), .b(new_n68_), .O(new_n139_));
  nor2   g93(.a(x26), .b(x25), .O(new_n140_));
  nand4  g94(.a(new_n140_), .b(new_n139_), .c(new_n109_), .d(new_n94_), .O(new_n141_));
  nor2   g95(.a(x17), .b(x13), .O(new_n142_));
  oai21  g96(.a(new_n142_), .b(x27), .c(x16), .O(new_n143_));
  aoi21  g97(.a(new_n141_), .b(new_n67_), .c(new_n143_), .O(z17));
  buf    g98(.a(x27), .O(z08));
endmodule


