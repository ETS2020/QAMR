// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n50_, new_n51_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_;
  nor2   g000(.a(x27), .b(x22), .O(new_n47_));
  aoi22  g001(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  aoi22  g002(.a(new_n47_), .b(x20), .c(x27), .d(x09), .O(z01));
  inv1   g003(.a(x27), .O(new_n50_));
  inv1   g004(.a(new_n47_), .O(new_n51_));
  oai22  g005(.a(new_n51_), .b(x21), .c(new_n50_), .d(x10), .O(z02));
  nand2  g006(.a(x27), .b(x11), .O(z03));
  oai22  g007(.a(new_n51_), .b(x23), .c(new_n50_), .d(x12), .O(z04));
  oai22  g008(.a(new_n51_), .b(x24), .c(new_n50_), .d(x13), .O(z05));
  oai22  g009(.a(new_n51_), .b(x25), .c(new_n50_), .d(x14), .O(z06));
  aoi22  g010(.a(new_n47_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g011(.a(x22), .O(new_n58_));
  nor2   g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x18), .O(new_n60_));
  inv1   g014(.a(x08), .O(new_n61_));
  aoi21  g015(.a(new_n60_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g016(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g017(.a(x19), .O(new_n64_));
  nand2  g018(.a(new_n64_), .b(x17), .O(new_n65_));
  inv1   g019(.a(x16), .O(new_n66_));
  nor2   g020(.a(new_n64_), .b(x17), .O(new_n67_));
  nor2   g021(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n63_), .c(new_n59_), .O(z09));
  inv1   g024(.a(x20), .O(new_n71_));
  nor2   g025(.a(x19), .b(x17), .O(new_n72_));
  xor2a  g026(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g027(.a(x01), .O(new_n74_));
  nand2  g028(.a(x18), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x09), .O(new_n76_));
  aoi21  g030(.a(new_n60_), .b(new_n76_), .c(x16), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n59_), .O(new_n78_));
  oai21  g032(.a(new_n73_), .b(new_n66_), .c(new_n78_), .O(z10));
  nand2  g033(.a(new_n50_), .b(x22), .O(new_n80_));
  nand2  g034(.a(x18), .b(x02), .O(new_n81_));
  nand2  g035(.a(new_n60_), .b(x10), .O(new_n82_));
  nand3  g036(.a(new_n82_), .b(new_n81_), .c(new_n66_), .O(new_n83_));
  inv1   g037(.a(x21), .O(new_n84_));
  aoi21  g038(.a(new_n72_), .b(new_n71_), .c(new_n84_), .O(new_n85_));
  nor2   g039(.a(x21), .b(x20), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n72_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(x16), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n85_), .c(new_n83_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n80_), .O(z11));
  nand2  g044(.a(new_n80_), .b(x16), .O(new_n91_));
  xor2a  g045(.a(new_n87_), .b(x22), .O(new_n92_));
  nor2   g046(.a(new_n59_), .b(x16), .O(new_n93_));
  inv1   g047(.a(x11), .O(new_n94_));
  nand2  g048(.a(new_n60_), .b(new_n94_), .O(new_n95_));
  inv1   g049(.a(x03), .O(new_n96_));
  nand2  g050(.a(x18), .b(new_n96_), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n95_), .c(new_n93_), .O(new_n98_));
  oai21  g052(.a(new_n92_), .b(new_n91_), .c(new_n98_), .O(z12));
  inv1   g053(.a(x12), .O(new_n100_));
  nand2  g054(.a(new_n60_), .b(new_n100_), .O(new_n101_));
  inv1   g055(.a(x04), .O(new_n102_));
  nand2  g056(.a(x18), .b(new_n102_), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n101_), .c(new_n93_), .O(new_n104_));
  nand3  g058(.a(new_n86_), .b(new_n72_), .c(new_n58_), .O(new_n105_));
  inv1   g059(.a(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(x23), .O(new_n107_));
  inv1   g061(.a(x23), .O(new_n108_));
  aoi21  g062(.a(new_n105_), .b(new_n108_), .c(new_n91_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n104_), .O(z13));
  nand3  g065(.a(new_n86_), .b(new_n72_), .c(new_n108_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(x24), .O(new_n113_));
  nor2   g067(.a(x24), .b(x23), .O(new_n114_));
  nand4  g068(.a(new_n114_), .b(new_n86_), .c(new_n72_), .d(new_n58_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x16), .O(new_n117_));
  nand2  g071(.a(x24), .b(x16), .O(new_n118_));
  aoi21  g072(.a(new_n118_), .b(x27), .c(new_n58_), .O(new_n119_));
  inv1   g073(.a(x05), .O(new_n120_));
  nand2  g074(.a(x18), .b(new_n120_), .O(new_n121_));
  inv1   g075(.a(x13), .O(new_n122_));
  aoi21  g076(.a(new_n60_), .b(new_n122_), .c(x16), .O(new_n123_));
  aoi21  g077(.a(new_n123_), .b(new_n121_), .c(new_n119_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n117_), .O(z14));
  inv1   g079(.a(x14), .O(new_n126_));
  nand2  g080(.a(new_n60_), .b(new_n126_), .O(new_n127_));
  inv1   g081(.a(x06), .O(new_n128_));
  nand2  g082(.a(x18), .b(new_n128_), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n127_), .c(new_n93_), .O(new_n130_));
  inv1   g084(.a(x25), .O(new_n131_));
  nor2   g085(.a(new_n115_), .b(new_n131_), .O(new_n132_));
  nand2  g086(.a(new_n115_), .b(new_n131_), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n80_), .c(x16), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n132_), .c(new_n130_), .O(z15));
  nor2   g089(.a(x25), .b(x24), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n86_), .c(new_n72_), .d(new_n108_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(x26), .O(new_n138_));
  nor3   g092(.a(x26), .b(x23), .c(x22), .O(new_n139_));
  nand4  g093(.a(new_n139_), .b(new_n136_), .c(new_n86_), .d(new_n72_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x16), .O(new_n142_));
  nand2  g096(.a(x26), .b(x16), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(x27), .c(new_n58_), .O(new_n144_));
  inv1   g098(.a(x07), .O(new_n145_));
  nand2  g099(.a(x18), .b(new_n145_), .O(new_n146_));
  inv1   g100(.a(x15), .O(new_n147_));
  aoi21  g101(.a(new_n60_), .b(new_n147_), .c(x16), .O(new_n148_));
  aoi21  g102(.a(new_n148_), .b(new_n146_), .c(new_n144_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n142_), .O(z16));
  nand4  g104(.a(new_n139_), .b(new_n136_), .c(new_n86_), .d(new_n67_), .O(new_n151_));
  aoi21  g105(.a(x27), .b(x17), .c(new_n59_), .O(new_n152_));
  aoi21  g106(.a(new_n152_), .b(new_n151_), .c(new_n93_), .O(z17));
  buf    g107(.a(x27), .O(z08));
endmodule


