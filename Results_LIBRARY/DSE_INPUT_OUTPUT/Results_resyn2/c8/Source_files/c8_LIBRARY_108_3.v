// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n144_, new_n145_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x13), .O(z08));
  inv1   g02(.a(z08), .O(new_n49_));
  oai22  g03(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  aoi22  g04(.a(z08), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  aoi22  g05(.a(z08), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  oai22  g06(.a(new_n49_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g07(.a(new_n49_), .b(x12), .c(x27), .d(x23), .O(z04));
  nand2  g08(.a(new_n47_), .b(x24), .O(z05));
  oai22  g09(.a(new_n49_), .b(x14), .c(x27), .d(x25), .O(z06));
  aoi22  g10(.a(z08), .b(x15), .c(new_n47_), .d(x26), .O(z07));
  inv1   g11(.a(x13), .O(new_n58_));
  nor2   g12(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  inv1   g13(.a(new_n59_), .O(new_n60_));
  inv1   g14(.a(x08), .O(new_n61_));
  inv1   g15(.a(x16), .O(new_n62_));
  oai21  g16(.a(x18), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  aoi21  g17(.a(x18), .b(x00), .c(new_n63_), .O(new_n64_));
  inv1   g18(.a(x17), .O(new_n65_));
  inv1   g19(.a(x19), .O(new_n66_));
  nor2   g20(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nor2   g21(.a(x19), .b(x17), .O(new_n68_));
  nor3   g22(.a(new_n68_), .b(new_n67_), .c(new_n62_), .O(new_n69_));
  oai21  g23(.a(new_n69_), .b(new_n64_), .c(new_n60_), .O(z09));
  xor2a  g24(.a(new_n68_), .b(x20), .O(new_n71_));
  nand2  g25(.a(new_n71_), .b(x16), .O(new_n72_));
  inv1   g26(.a(x18), .O(new_n73_));
  inv1   g27(.a(x09), .O(new_n74_));
  aoi21  g28(.a(new_n73_), .b(new_n74_), .c(x16), .O(new_n75_));
  oai21  g29(.a(new_n73_), .b(x01), .c(new_n75_), .O(new_n76_));
  aoi21  g30(.a(new_n76_), .b(new_n72_), .c(new_n59_), .O(z10));
  inv1   g31(.a(x20), .O(new_n78_));
  inv1   g32(.a(x21), .O(new_n79_));
  aoi21  g33(.a(new_n68_), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nor2   g34(.a(x21), .b(x20), .O(new_n81_));
  nand2  g35(.a(new_n81_), .b(new_n68_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  oai21  g37(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  inv1   g38(.a(x10), .O(new_n85_));
  aoi21  g39(.a(new_n73_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g40(.a(new_n73_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g41(.a(new_n87_), .b(new_n84_), .c(new_n59_), .O(z11));
  inv1   g42(.a(new_n68_), .O(new_n89_));
  inv1   g43(.a(x22), .O(new_n90_));
  nand2  g44(.a(new_n81_), .b(new_n90_), .O(new_n91_));
  nor2   g45(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  aoi21  g46(.a(new_n82_), .b(x22), .c(new_n92_), .O(new_n93_));
  inv1   g47(.a(x03), .O(new_n94_));
  nand2  g48(.a(x18), .b(new_n94_), .O(new_n95_));
  inv1   g49(.a(x11), .O(new_n96_));
  aoi21  g50(.a(new_n73_), .b(new_n96_), .c(x16), .O(new_n97_));
  aoi21  g51(.a(new_n97_), .b(new_n95_), .c(new_n59_), .O(new_n98_));
  oai21  g52(.a(new_n93_), .b(new_n62_), .c(new_n98_), .O(z12));
  inv1   g53(.a(x23), .O(new_n100_));
  nor2   g54(.a(x23), .b(x22), .O(new_n101_));
  nand3  g55(.a(new_n101_), .b(new_n81_), .c(new_n68_), .O(new_n102_));
  oai21  g56(.a(new_n92_), .b(new_n100_), .c(new_n102_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g58(.a(x04), .O(new_n105_));
  nand2  g59(.a(x18), .b(new_n105_), .O(new_n106_));
  inv1   g60(.a(x12), .O(new_n107_));
  aoi21  g61(.a(new_n73_), .b(new_n107_), .c(x16), .O(new_n108_));
  aoi21  g62(.a(new_n108_), .b(new_n106_), .c(new_n59_), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(new_n104_), .O(z13));
  nand2  g64(.a(new_n102_), .b(x24), .O(new_n111_));
  nor2   g65(.a(x24), .b(x23), .O(new_n112_));
  nand4  g66(.a(new_n112_), .b(new_n81_), .c(new_n68_), .d(new_n90_), .O(new_n113_));
  nand2  g67(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g68(.a(new_n114_), .b(x16), .O(new_n115_));
  nor2   g69(.a(x18), .b(x16), .O(new_n116_));
  oai21  g70(.a(new_n116_), .b(x27), .c(x13), .O(new_n117_));
  nand3  g71(.a(x18), .b(new_n62_), .c(x05), .O(new_n118_));
  nand3  g72(.a(new_n118_), .b(new_n117_), .c(new_n115_), .O(z14));
  nand2  g73(.a(new_n113_), .b(x25), .O(new_n120_));
  nor2   g74(.a(x25), .b(x24), .O(new_n121_));
  nand4  g75(.a(new_n121_), .b(new_n101_), .c(new_n81_), .d(new_n68_), .O(new_n122_));
  nand2  g76(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g77(.a(new_n123_), .b(x16), .O(new_n124_));
  inv1   g78(.a(x14), .O(new_n125_));
  aoi21  g79(.a(new_n73_), .b(new_n125_), .c(x16), .O(new_n126_));
  oai21  g80(.a(new_n73_), .b(x06), .c(new_n126_), .O(new_n127_));
  aoi21  g81(.a(new_n127_), .b(new_n124_), .c(new_n59_), .O(z15));
  inv1   g82(.a(x26), .O(new_n129_));
  nand3  g83(.a(new_n121_), .b(new_n101_), .c(new_n129_), .O(new_n130_));
  nor2   g84(.a(new_n130_), .b(new_n82_), .O(new_n131_));
  aoi21  g85(.a(new_n122_), .b(x26), .c(new_n131_), .O(new_n132_));
  inv1   g86(.a(x07), .O(new_n133_));
  nand2  g87(.a(x18), .b(new_n133_), .O(new_n134_));
  inv1   g88(.a(x15), .O(new_n135_));
  aoi21  g89(.a(new_n73_), .b(new_n135_), .c(x16), .O(new_n136_));
  aoi21  g90(.a(new_n136_), .b(new_n134_), .c(new_n59_), .O(new_n137_));
  oai21  g91(.a(new_n132_), .b(new_n62_), .c(new_n137_), .O(z16));
  nand3  g92(.a(new_n81_), .b(x19), .c(new_n65_), .O(new_n139_));
  oai22  g93(.a(new_n139_), .b(new_n130_), .c(new_n47_), .d(new_n65_), .O(new_n140_));
  nand2  g94(.a(new_n140_), .b(new_n58_), .O(new_n141_));
  inv1   g95(.a(new_n91_), .O(new_n142_));
  nor2   g96(.a(new_n66_), .b(x17), .O(new_n143_));
  nor3   g97(.a(x27), .b(x26), .c(x25), .O(new_n144_));
  nand4  g98(.a(new_n144_), .b(new_n143_), .c(new_n112_), .d(new_n142_), .O(new_n145_));
  aoi21  g99(.a(new_n145_), .b(new_n141_), .c(new_n62_), .O(z17));
endmodule


