// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x25), .O(new_n48_));
  aoi22  g02(.a(new_n48_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g03(.a(new_n48_), .O(new_n50_));
  oai22  g04(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g05(.a(new_n50_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g06(.a(new_n48_), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  aoi22  g07(.a(new_n48_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  aoi22  g08(.a(new_n48_), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  aoi21  g09(.a(x27), .b(x14), .c(x25), .O(z06));
  oai22  g10(.a(new_n50_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g11(.a(x25), .O(new_n58_));
  nor2   g12(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  inv1   g13(.a(new_n59_), .O(new_n60_));
  inv1   g14(.a(x16), .O(new_n61_));
  nand2  g15(.a(x18), .b(x00), .O(new_n62_));
  inv1   g16(.a(x18), .O(new_n63_));
  nand2  g17(.a(new_n63_), .b(x08), .O(new_n64_));
  nand3  g18(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  inv1   g19(.a(x17), .O(new_n66_));
  inv1   g20(.a(x19), .O(new_n67_));
  nor2   g21(.a(x19), .b(x17), .O(new_n68_));
  nor2   g22(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  oai21  g23(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  nand2  g24(.a(new_n70_), .b(new_n65_), .O(new_n71_));
  nand2  g25(.a(new_n71_), .b(new_n60_), .O(z09));
  xor2a  g26(.a(new_n68_), .b(x20), .O(new_n73_));
  nand2  g27(.a(new_n73_), .b(x16), .O(new_n74_));
  inv1   g28(.a(x09), .O(new_n75_));
  aoi21  g29(.a(new_n63_), .b(new_n75_), .c(x16), .O(new_n76_));
  oai21  g30(.a(new_n63_), .b(x01), .c(new_n76_), .O(new_n77_));
  nand3  g31(.a(new_n77_), .b(new_n74_), .c(new_n60_), .O(z10));
  inv1   g32(.a(x20), .O(new_n79_));
  nand2  g33(.a(new_n68_), .b(new_n79_), .O(new_n80_));
  nor2   g34(.a(x21), .b(x20), .O(new_n81_));
  aoi22  g35(.a(new_n81_), .b(new_n68_), .c(new_n80_), .d(x21), .O(new_n82_));
  inv1   g36(.a(x02), .O(new_n83_));
  nand2  g37(.a(x18), .b(new_n83_), .O(new_n84_));
  inv1   g38(.a(x10), .O(new_n85_));
  aoi21  g39(.a(new_n63_), .b(new_n85_), .c(x16), .O(new_n86_));
  aoi21  g40(.a(new_n86_), .b(new_n84_), .c(new_n59_), .O(new_n87_));
  oai21  g41(.a(new_n82_), .b(new_n61_), .c(new_n87_), .O(z11));
  nand2  g42(.a(new_n81_), .b(new_n68_), .O(new_n89_));
  xor2a  g43(.a(new_n89_), .b(x22), .O(new_n90_));
  inv1   g44(.a(x03), .O(new_n91_));
  nand2  g45(.a(x18), .b(new_n91_), .O(new_n92_));
  inv1   g46(.a(x11), .O(new_n93_));
  aoi21  g47(.a(new_n63_), .b(new_n93_), .c(x16), .O(new_n94_));
  aoi21  g48(.a(new_n94_), .b(new_n92_), .c(new_n59_), .O(new_n95_));
  oai21  g49(.a(new_n90_), .b(new_n61_), .c(new_n95_), .O(z12));
  nand2  g50(.a(x18), .b(x04), .O(new_n97_));
  nand2  g51(.a(new_n63_), .b(x12), .O(new_n98_));
  nand3  g52(.a(new_n98_), .b(new_n97_), .c(new_n61_), .O(new_n99_));
  oai21  g53(.a(new_n89_), .b(x22), .c(x23), .O(new_n100_));
  nor2   g54(.a(x23), .b(x22), .O(new_n101_));
  nand3  g55(.a(new_n101_), .b(new_n81_), .c(new_n68_), .O(new_n102_));
  nand3  g56(.a(new_n102_), .b(new_n100_), .c(x16), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(new_n99_), .O(new_n104_));
  nand2  g58(.a(new_n104_), .b(new_n60_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x24), .O(new_n106_));
  inv1   g60(.a(x24), .O(new_n107_));
  nand4  g61(.a(new_n101_), .b(new_n81_), .c(new_n68_), .d(new_n107_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(x16), .O(new_n110_));
  inv1   g64(.a(x13), .O(new_n111_));
  aoi21  g65(.a(new_n63_), .b(new_n111_), .c(x16), .O(new_n112_));
  oai21  g66(.a(new_n63_), .b(x05), .c(new_n112_), .O(new_n113_));
  aoi21  g67(.a(new_n113_), .b(new_n110_), .c(new_n59_), .O(z14));
  nor2   g68(.a(x27), .b(new_n58_), .O(new_n115_));
  and2   g69(.a(new_n115_), .b(new_n108_), .O(new_n116_));
  nor2   g70(.a(new_n108_), .b(x25), .O(new_n117_));
  oai21  g71(.a(new_n117_), .b(new_n116_), .c(x16), .O(new_n118_));
  inv1   g72(.a(x06), .O(new_n119_));
  nand2  g73(.a(x18), .b(new_n119_), .O(new_n120_));
  inv1   g74(.a(x14), .O(new_n121_));
  nand2  g75(.a(new_n63_), .b(new_n121_), .O(new_n122_));
  nand4  g76(.a(new_n122_), .b(new_n120_), .c(new_n60_), .d(new_n61_), .O(new_n123_));
  nand2  g77(.a(new_n123_), .b(new_n118_), .O(z15));
  nor2   g78(.a(new_n117_), .b(x26), .O(new_n125_));
  inv1   g79(.a(x26), .O(new_n126_));
  oai21  g80(.a(new_n108_), .b(new_n126_), .c(x16), .O(new_n127_));
  nand2  g81(.a(x26), .b(x16), .O(new_n128_));
  aoi21  g82(.a(new_n128_), .b(new_n47_), .c(new_n58_), .O(new_n129_));
  inv1   g83(.a(x07), .O(new_n130_));
  nand2  g84(.a(x18), .b(new_n130_), .O(new_n131_));
  inv1   g85(.a(x15), .O(new_n132_));
  aoi21  g86(.a(new_n63_), .b(new_n132_), .c(x16), .O(new_n133_));
  aoi21  g87(.a(new_n133_), .b(new_n131_), .c(new_n129_), .O(new_n134_));
  oai21  g88(.a(new_n127_), .b(new_n125_), .c(new_n134_), .O(z16));
  nor2   g89(.a(x24), .b(new_n67_), .O(new_n136_));
  nand4  g90(.a(new_n136_), .b(new_n101_), .c(new_n81_), .d(new_n126_), .O(new_n137_));
  nand2  g91(.a(new_n47_), .b(x17), .O(new_n138_));
  nand3  g92(.a(new_n138_), .b(new_n58_), .c(x16), .O(new_n139_));
  aoi21  g93(.a(new_n137_), .b(new_n66_), .c(new_n139_), .O(z17));
  buf    g94(.a(x27), .O(z08));
endmodule


