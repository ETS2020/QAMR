// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x23), .O(new_n48_));
  oai22  g002(.a(new_n48_), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  inv1   g003(.a(new_n48_), .O(new_n50_));
  aoi22  g004(.a(new_n50_), .b(x20), .c(x27), .d(x09), .O(z01));
  aoi22  g005(.a(new_n50_), .b(x21), .c(x27), .d(x10), .O(z02));
  aoi22  g006(.a(new_n50_), .b(x22), .c(x27), .d(x11), .O(z03));
  nor2   g007(.a(new_n47_), .b(x12), .O(z04));
  oai22  g008(.a(new_n48_), .b(x24), .c(new_n47_), .d(x13), .O(z05));
  oai22  g009(.a(new_n48_), .b(x25), .c(new_n47_), .d(x14), .O(z06));
  aoi22  g010(.a(new_n50_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g011(.a(x17), .O(new_n58_));
  inv1   g012(.a(x19), .O(new_n59_));
  nor2   g013(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor2   g014(.a(x19), .b(x17), .O(new_n61_));
  oai21  g015(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  nor2   g016(.a(x27), .b(x23), .O(new_n63_));
  inv1   g017(.a(new_n63_), .O(new_n64_));
  inv1   g018(.a(x18), .O(new_n65_));
  inv1   g019(.a(x08), .O(new_n66_));
  aoi21  g020(.a(new_n65_), .b(new_n66_), .c(x16), .O(new_n67_));
  oai21  g021(.a(new_n65_), .b(x00), .c(new_n67_), .O(new_n68_));
  nand3  g022(.a(new_n68_), .b(new_n64_), .c(new_n62_), .O(z09));
  inv1   g023(.a(x20), .O(new_n70_));
  nor2   g024(.a(new_n61_), .b(new_n70_), .O(new_n71_));
  nor3   g025(.a(x20), .b(x19), .c(x17), .O(new_n72_));
  oai21  g026(.a(new_n72_), .b(new_n71_), .c(x16), .O(new_n73_));
  inv1   g027(.a(x01), .O(new_n74_));
  nand2  g028(.a(x18), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x09), .O(new_n76_));
  aoi21  g030(.a(new_n65_), .b(new_n76_), .c(x16), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n63_), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n73_), .O(z10));
  inv1   g033(.a(x21), .O(new_n80_));
  nor2   g034(.a(new_n72_), .b(new_n80_), .O(new_n81_));
  nand2  g035(.a(new_n72_), .b(new_n80_), .O(new_n82_));
  inv1   g036(.a(new_n82_), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n81_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x10), .O(new_n85_));
  aoi21  g039(.a(new_n65_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g040(.a(new_n65_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g041(.a(new_n87_), .b(new_n84_), .c(new_n63_), .O(z11));
  nor2   g042(.a(x22), .b(x21), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n61_), .c(new_n70_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(x16), .O(new_n91_));
  aoi21  g045(.a(new_n82_), .b(x22), .c(new_n91_), .O(new_n92_));
  inv1   g046(.a(x11), .O(new_n93_));
  inv1   g047(.a(x16), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  aoi21  g049(.a(x18), .b(x03), .c(new_n95_), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n92_), .c(new_n64_), .O(z12));
  inv1   g051(.a(x23), .O(new_n98_));
  oai21  g052(.a(new_n90_), .b(new_n94_), .c(x27), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g054(.a(new_n90_), .b(x23), .c(x16), .O(new_n101_));
  inv1   g055(.a(x12), .O(new_n102_));
  aoi21  g056(.a(new_n65_), .b(new_n102_), .c(x16), .O(new_n103_));
  oai21  g057(.a(new_n65_), .b(x04), .c(new_n103_), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n101_), .c(new_n100_), .O(z13));
  inv1   g059(.a(x24), .O(new_n106_));
  nand2  g060(.a(new_n90_), .b(x27), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n98_), .c(new_n106_), .O(new_n108_));
  nor2   g062(.a(x24), .b(x23), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(x27), .O(new_n110_));
  nor2   g064(.a(new_n110_), .b(new_n90_), .O(new_n111_));
  oai21  g065(.a(new_n111_), .b(new_n108_), .c(x16), .O(new_n112_));
  nor2   g066(.a(new_n63_), .b(x16), .O(new_n113_));
  inv1   g067(.a(x13), .O(new_n114_));
  nand2  g068(.a(new_n65_), .b(new_n114_), .O(new_n115_));
  inv1   g069(.a(x05), .O(new_n116_));
  nand2  g070(.a(x18), .b(new_n116_), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n112_), .O(z14));
  inv1   g073(.a(x25), .O(new_n120_));
  nand3  g074(.a(new_n70_), .b(new_n59_), .c(new_n58_), .O(new_n121_));
  inv1   g075(.a(x22), .O(new_n122_));
  nand3  g076(.a(new_n106_), .b(new_n122_), .c(new_n80_), .O(new_n123_));
  oai21  g077(.a(new_n123_), .b(new_n121_), .c(x27), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n98_), .c(new_n120_), .O(new_n125_));
  nor3   g079(.a(new_n110_), .b(new_n90_), .c(x25), .O(new_n126_));
  oai21  g080(.a(new_n126_), .b(new_n125_), .c(x16), .O(new_n127_));
  inv1   g081(.a(x14), .O(new_n128_));
  nand2  g082(.a(new_n65_), .b(new_n128_), .O(new_n129_));
  inv1   g083(.a(x06), .O(new_n130_));
  nand2  g084(.a(x18), .b(new_n130_), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n129_), .c(new_n113_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n127_), .O(z15));
  nand2  g087(.a(new_n111_), .b(new_n120_), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(x26), .c(x16), .O(new_n135_));
  nand3  g089(.a(new_n89_), .b(new_n72_), .c(new_n106_), .O(new_n136_));
  nor2   g090(.a(x26), .b(x25), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(x16), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n136_), .c(x27), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n98_), .O(new_n140_));
  inv1   g094(.a(x15), .O(new_n141_));
  aoi21  g095(.a(new_n65_), .b(new_n141_), .c(x16), .O(new_n142_));
  oai21  g096(.a(new_n65_), .b(x07), .c(new_n142_), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n140_), .c(new_n135_), .O(z16));
  oai21  g098(.a(new_n47_), .b(x17), .c(new_n48_), .O(new_n145_));
  nor2   g099(.a(x20), .b(new_n59_), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n137_), .c(new_n109_), .d(new_n89_), .O(new_n147_));
  aoi21  g101(.a(new_n147_), .b(new_n145_), .c(new_n113_), .O(z17));
  buf    g102(.a(x27), .O(z08));
endmodule


