// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n51_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x17), .O(new_n48_));
  aoi22  g002(.a(new_n48_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  aoi22  g003(.a(new_n48_), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  inv1   g004(.a(new_n48_), .O(new_n51_));
  oai22  g005(.a(new_n51_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g006(.a(new_n51_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g007(.a(new_n51_), .b(x12), .c(x27), .d(x23), .O(z04));
  aoi22  g008(.a(new_n48_), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  oai22  g009(.a(new_n51_), .b(x14), .c(x27), .d(x25), .O(z06));
  oai22  g010(.a(new_n51_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g011(.a(x18), .O(new_n58_));
  inv1   g012(.a(x08), .O(new_n59_));
  aoi21  g013(.a(new_n58_), .b(new_n59_), .c(x16), .O(new_n60_));
  oai21  g014(.a(new_n58_), .b(x00), .c(new_n60_), .O(new_n61_));
  nand2  g015(.a(x27), .b(x17), .O(new_n62_));
  and2   g016(.a(x19), .b(x17), .O(new_n63_));
  nor2   g017(.a(x19), .b(x17), .O(new_n64_));
  oai21  g018(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  nand3  g019(.a(new_n65_), .b(new_n62_), .c(new_n61_), .O(z09));
  inv1   g020(.a(new_n62_), .O(new_n67_));
  nor3   g021(.a(x20), .b(x19), .c(x17), .O(new_n68_));
  inv1   g022(.a(x20), .O(new_n69_));
  nor2   g023(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  oai21  g024(.a(new_n70_), .b(new_n68_), .c(x16), .O(new_n71_));
  inv1   g025(.a(x09), .O(new_n72_));
  aoi21  g026(.a(new_n58_), .b(new_n72_), .c(x16), .O(new_n73_));
  oai21  g027(.a(new_n58_), .b(x01), .c(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n71_), .c(new_n67_), .O(z10));
  inv1   g029(.a(x21), .O(new_n76_));
  nor2   g030(.a(x20), .b(x19), .O(new_n77_));
  nand2  g031(.a(new_n68_), .b(new_n76_), .O(new_n78_));
  oai21  g032(.a(new_n77_), .b(new_n76_), .c(new_n78_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(x16), .O(new_n80_));
  inv1   g034(.a(x17), .O(new_n81_));
  nand2  g035(.a(x21), .b(x16), .O(new_n82_));
  aoi21  g036(.a(new_n82_), .b(new_n47_), .c(new_n81_), .O(new_n83_));
  inv1   g037(.a(x02), .O(new_n84_));
  nand2  g038(.a(x18), .b(new_n84_), .O(new_n85_));
  inv1   g039(.a(x10), .O(new_n86_));
  aoi21  g040(.a(new_n58_), .b(new_n86_), .c(x16), .O(new_n87_));
  aoi21  g041(.a(new_n87_), .b(new_n85_), .c(new_n83_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n80_), .O(z11));
  inv1   g043(.a(x16), .O(new_n90_));
  nor2   g044(.a(x22), .b(x21), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n77_), .O(new_n92_));
  inv1   g046(.a(new_n92_), .O(new_n93_));
  aoi22  g047(.a(new_n93_), .b(new_n81_), .c(new_n78_), .d(x22), .O(new_n94_));
  inv1   g048(.a(x03), .O(new_n95_));
  nand2  g049(.a(x18), .b(new_n95_), .O(new_n96_));
  inv1   g050(.a(x11), .O(new_n97_));
  aoi21  g051(.a(new_n58_), .b(new_n97_), .c(x16), .O(new_n98_));
  aoi21  g052(.a(new_n98_), .b(new_n96_), .c(new_n67_), .O(new_n99_));
  oai21  g053(.a(new_n94_), .b(new_n90_), .c(new_n99_), .O(z12));
  nor3   g054(.a(x23), .b(x22), .c(x21), .O(new_n101_));
  aoi22  g055(.a(new_n101_), .b(new_n68_), .c(new_n92_), .d(x23), .O(new_n102_));
  nand2  g056(.a(x23), .b(x16), .O(new_n103_));
  aoi21  g057(.a(new_n103_), .b(new_n47_), .c(new_n81_), .O(new_n104_));
  inv1   g058(.a(x04), .O(new_n105_));
  nand2  g059(.a(x18), .b(new_n105_), .O(new_n106_));
  inv1   g060(.a(x12), .O(new_n107_));
  aoi21  g061(.a(new_n58_), .b(new_n107_), .c(x16), .O(new_n108_));
  aoi21  g062(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  oai21  g063(.a(new_n102_), .b(new_n90_), .c(new_n109_), .O(z13));
  inv1   g064(.a(x24), .O(new_n111_));
  nand2  g065(.a(new_n101_), .b(new_n68_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g067(.a(new_n101_), .b(new_n77_), .c(x24), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n113_), .c(x16), .O(new_n115_));
  nand2  g069(.a(x24), .b(x16), .O(new_n116_));
  aoi21  g070(.a(new_n116_), .b(new_n47_), .c(new_n81_), .O(new_n117_));
  inv1   g071(.a(x05), .O(new_n118_));
  nand2  g072(.a(x18), .b(new_n118_), .O(new_n119_));
  inv1   g073(.a(x13), .O(new_n120_));
  aoi21  g074(.a(new_n58_), .b(new_n120_), .c(x16), .O(new_n121_));
  aoi21  g075(.a(new_n121_), .b(new_n119_), .c(new_n117_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n115_), .O(z14));
  inv1   g077(.a(x25), .O(new_n124_));
  nor2   g078(.a(x24), .b(x23), .O(new_n125_));
  aoi21  g079(.a(new_n125_), .b(new_n93_), .c(new_n124_), .O(new_n126_));
  nor2   g080(.a(x25), .b(x24), .O(new_n127_));
  inv1   g081(.a(new_n127_), .O(new_n128_));
  nor2   g082(.a(new_n128_), .b(new_n112_), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n126_), .c(x16), .O(new_n130_));
  nand2  g084(.a(x25), .b(x16), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(new_n47_), .c(new_n81_), .O(new_n132_));
  inv1   g086(.a(x06), .O(new_n133_));
  nand2  g087(.a(x18), .b(new_n133_), .O(new_n134_));
  inv1   g088(.a(x14), .O(new_n135_));
  aoi21  g089(.a(new_n58_), .b(new_n135_), .c(x16), .O(new_n136_));
  aoi21  g090(.a(new_n136_), .b(new_n134_), .c(new_n132_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n130_), .O(z15));
  inv1   g092(.a(x23), .O(new_n139_));
  nand4  g093(.a(new_n127_), .b(new_n91_), .c(new_n77_), .d(new_n139_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x26), .O(new_n141_));
  nor2   g095(.a(x26), .b(x25), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n125_), .c(new_n91_), .d(new_n77_), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(new_n141_), .c(x17), .O(new_n144_));
  nand2  g098(.a(x26), .b(x17), .O(new_n145_));
  nor2   g099(.a(new_n145_), .b(x27), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  inv1   g101(.a(x07), .O(new_n148_));
  nand2  g102(.a(x18), .b(new_n148_), .O(new_n149_));
  inv1   g103(.a(x15), .O(new_n150_));
  nand2  g104(.a(new_n58_), .b(new_n150_), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n149_), .c(new_n62_), .d(new_n90_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n147_), .O(z16));
  nand3  g107(.a(new_n111_), .b(new_n69_), .c(x19), .O(new_n154_));
  inv1   g108(.a(new_n154_), .O(new_n155_));
  nor2   g109(.a(x17), .b(new_n90_), .O(new_n156_));
  nand4  g110(.a(new_n156_), .b(new_n155_), .c(new_n142_), .d(new_n101_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n62_), .O(z17));
  buf    g112(.a(x27), .O(z08));
endmodule


