// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x24), .O(new_n48_));
  nor2   g002(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  aoi22  g003(.a(new_n49_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g004(.a(new_n49_), .O(new_n51_));
  oai22  g005(.a(new_n51_), .b(x09), .c(x27), .d(x20), .O(z01));
  aoi22  g006(.a(new_n49_), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  aoi22  g007(.a(new_n49_), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  oai22  g008(.a(new_n51_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai21  g009(.a(new_n47_), .b(x13), .c(x24), .O(z05));
  aoi22  g010(.a(new_n49_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g011(.a(new_n51_), .b(x15), .c(x27), .d(x26), .O(z07));
  and2   g012(.a(x19), .b(x17), .O(new_n59_));
  nor2   g013(.a(x19), .b(x17), .O(new_n60_));
  oai21  g014(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n61_));
  nor2   g015(.a(new_n47_), .b(x24), .O(new_n62_));
  inv1   g016(.a(new_n62_), .O(new_n63_));
  inv1   g017(.a(x18), .O(new_n64_));
  inv1   g018(.a(x08), .O(new_n65_));
  aoi21  g019(.a(new_n64_), .b(new_n65_), .c(x16), .O(new_n66_));
  oai21  g020(.a(new_n64_), .b(x00), .c(new_n66_), .O(new_n67_));
  nand3  g021(.a(new_n67_), .b(new_n63_), .c(new_n61_), .O(z09));
  inv1   g022(.a(x20), .O(new_n69_));
  nor2   g023(.a(new_n60_), .b(new_n69_), .O(new_n70_));
  nor3   g024(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  oai21  g025(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  inv1   g026(.a(x01), .O(new_n73_));
  nand2  g027(.a(x18), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x09), .O(new_n75_));
  aoi21  g029(.a(new_n64_), .b(new_n75_), .c(x16), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n62_), .O(new_n77_));
  nand2  g031(.a(new_n77_), .b(new_n72_), .O(z10));
  inv1   g032(.a(x16), .O(new_n79_));
  nand2  g033(.a(x18), .b(x02), .O(new_n80_));
  nand2  g034(.a(new_n64_), .b(x10), .O(new_n81_));
  nand3  g035(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  inv1   g036(.a(x21), .O(new_n83_));
  nor2   g037(.a(new_n71_), .b(new_n83_), .O(new_n84_));
  nor2   g038(.a(x21), .b(x20), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n60_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(x16), .O(new_n87_));
  oai21  g041(.a(new_n87_), .b(new_n84_), .c(new_n82_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n63_), .O(z11));
  xor2a  g043(.a(new_n86_), .b(x22), .O(new_n90_));
  inv1   g044(.a(x03), .O(new_n91_));
  nand2  g045(.a(x18), .b(new_n91_), .O(new_n92_));
  inv1   g046(.a(x11), .O(new_n93_));
  aoi21  g047(.a(new_n64_), .b(new_n93_), .c(x16), .O(new_n94_));
  aoi21  g048(.a(new_n94_), .b(new_n92_), .c(new_n62_), .O(new_n95_));
  oai21  g049(.a(new_n90_), .b(new_n79_), .c(new_n95_), .O(z12));
  inv1   g050(.a(x22), .O(new_n97_));
  nand3  g051(.a(new_n85_), .b(new_n60_), .c(new_n97_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(x23), .O(new_n99_));
  nor2   g053(.a(x23), .b(x22), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n85_), .c(new_n60_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(x16), .O(new_n103_));
  inv1   g057(.a(x04), .O(new_n104_));
  nand2  g058(.a(x18), .b(new_n104_), .O(new_n105_));
  inv1   g059(.a(x12), .O(new_n106_));
  aoi21  g060(.a(new_n64_), .b(new_n106_), .c(x16), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n105_), .c(new_n62_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n103_), .O(z13));
  oai21  g063(.a(new_n101_), .b(new_n79_), .c(new_n47_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n48_), .O(new_n111_));
  inv1   g065(.a(x05), .O(new_n112_));
  nand2  g066(.a(x18), .b(new_n112_), .O(new_n113_));
  inv1   g067(.a(x13), .O(new_n114_));
  aoi21  g068(.a(new_n64_), .b(new_n114_), .c(x16), .O(new_n115_));
  nor2   g069(.a(new_n48_), .b(new_n79_), .O(new_n116_));
  aoi22  g070(.a(new_n116_), .b(new_n101_), .c(new_n115_), .d(new_n113_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n111_), .O(z14));
  inv1   g072(.a(x14), .O(new_n119_));
  nand2  g073(.a(new_n64_), .b(new_n119_), .O(new_n120_));
  inv1   g074(.a(x06), .O(new_n121_));
  nand2  g075(.a(x18), .b(new_n121_), .O(new_n122_));
  nand4  g076(.a(new_n122_), .b(new_n120_), .c(new_n63_), .d(new_n79_), .O(new_n123_));
  inv1   g077(.a(x25), .O(new_n124_));
  nand4  g078(.a(new_n100_), .b(new_n85_), .c(new_n60_), .d(new_n48_), .O(new_n125_));
  nor2   g079(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nor2   g080(.a(new_n62_), .b(new_n79_), .O(new_n127_));
  nand2  g081(.a(new_n125_), .b(new_n124_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n126_), .c(new_n123_), .O(z15));
  inv1   g084(.a(x15), .O(new_n131_));
  nand2  g085(.a(new_n64_), .b(new_n131_), .O(new_n132_));
  inv1   g086(.a(x07), .O(new_n133_));
  nand2  g087(.a(x18), .b(new_n133_), .O(new_n134_));
  nand4  g088(.a(new_n134_), .b(new_n132_), .c(new_n63_), .d(new_n79_), .O(new_n135_));
  inv1   g089(.a(x26), .O(new_n136_));
  nand2  g090(.a(new_n100_), .b(new_n48_), .O(new_n137_));
  nor2   g091(.a(x25), .b(x21), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(new_n60_), .c(new_n69_), .O(new_n139_));
  oai21  g093(.a(new_n139_), .b(new_n137_), .c(new_n136_), .O(new_n140_));
  nor3   g094(.a(x24), .b(x23), .c(x22), .O(new_n141_));
  nand4  g095(.a(new_n138_), .b(new_n141_), .c(new_n71_), .d(x26), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n140_), .c(new_n127_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n135_), .O(z16));
  inv1   g098(.a(x17), .O(new_n145_));
  nand4  g099(.a(new_n85_), .b(new_n136_), .c(new_n124_), .d(x19), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n137_), .c(new_n145_), .O(new_n147_));
  aoi21  g101(.a(new_n47_), .b(x17), .c(new_n79_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n63_), .O(z17));
  buf    g104(.a(x27), .O(z08));
endmodule


