// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n57_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x14), .O(new_n48_));
  nor2   g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(new_n49_), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  aoi22  g005(.a(new_n49_), .b(x20), .c(x27), .d(x09), .O(z01));
  aoi22  g006(.a(new_n49_), .b(x21), .c(x27), .d(x10), .O(z02));
  aoi22  g007(.a(new_n49_), .b(x22), .c(x27), .d(x11), .O(z03));
  oai22  g008(.a(new_n50_), .b(x23), .c(new_n47_), .d(x12), .O(z04));
  oai22  g009(.a(new_n50_), .b(x24), .c(new_n47_), .d(x13), .O(z05));
  nand2  g010(.a(x27), .b(new_n48_), .O(new_n57_));
  oai21  g011(.a(new_n50_), .b(x25), .c(new_n57_), .O(z06));
  aoi22  g012(.a(new_n49_), .b(x26), .c(x27), .d(x15), .O(z07));
  nor2   g013(.a(x27), .b(x14), .O(new_n60_));
  inv1   g014(.a(new_n60_), .O(new_n61_));
  inv1   g015(.a(x08), .O(new_n62_));
  inv1   g016(.a(x16), .O(new_n63_));
  oai21  g017(.a(x18), .b(new_n62_), .c(new_n63_), .O(new_n64_));
  aoi21  g018(.a(x18), .b(x00), .c(new_n64_), .O(new_n65_));
  inv1   g019(.a(x17), .O(new_n66_));
  inv1   g020(.a(x19), .O(new_n67_));
  nand2  g021(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(x16), .O(new_n69_));
  aoi21  g023(.a(x19), .b(x17), .c(new_n69_), .O(new_n70_));
  oai21  g024(.a(new_n70_), .b(new_n65_), .c(new_n61_), .O(z09));
  inv1   g025(.a(x20), .O(new_n72_));
  nor2   g026(.a(x19), .b(x17), .O(new_n73_));
  nor2   g027(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nor2   g028(.a(new_n68_), .b(x20), .O(new_n75_));
  oai21  g029(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  inv1   g030(.a(x18), .O(new_n77_));
  inv1   g031(.a(x09), .O(new_n78_));
  aoi21  g032(.a(new_n77_), .b(new_n78_), .c(x16), .O(new_n79_));
  oai21  g033(.a(new_n77_), .b(x01), .c(new_n79_), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n76_), .c(new_n60_), .O(z10));
  inv1   g035(.a(x21), .O(new_n82_));
  aoi21  g036(.a(new_n73_), .b(new_n72_), .c(new_n82_), .O(new_n83_));
  nor2   g037(.a(x21), .b(x20), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n73_), .O(new_n85_));
  inv1   g039(.a(new_n85_), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n83_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x10), .O(new_n88_));
  aoi21  g042(.a(new_n77_), .b(new_n88_), .c(x16), .O(new_n89_));
  oai21  g043(.a(new_n77_), .b(x02), .c(new_n89_), .O(new_n90_));
  aoi21  g044(.a(new_n90_), .b(new_n87_), .c(new_n60_), .O(z11));
  xor2a  g045(.a(new_n85_), .b(x22), .O(new_n92_));
  inv1   g046(.a(x03), .O(new_n93_));
  nand2  g047(.a(x18), .b(new_n93_), .O(new_n94_));
  inv1   g048(.a(x11), .O(new_n95_));
  aoi21  g049(.a(new_n77_), .b(new_n95_), .c(x16), .O(new_n96_));
  aoi21  g050(.a(new_n96_), .b(new_n94_), .c(new_n60_), .O(new_n97_));
  oai21  g051(.a(new_n92_), .b(new_n63_), .c(new_n97_), .O(z12));
  inv1   g052(.a(x22), .O(new_n99_));
  nand3  g053(.a(new_n84_), .b(new_n73_), .c(new_n99_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(x23), .O(new_n101_));
  nor2   g055(.a(x23), .b(x22), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n84_), .c(new_n73_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(x16), .O(new_n105_));
  inv1   g059(.a(x04), .O(new_n106_));
  nand2  g060(.a(x18), .b(new_n106_), .O(new_n107_));
  inv1   g061(.a(x12), .O(new_n108_));
  aoi21  g062(.a(new_n77_), .b(new_n108_), .c(x16), .O(new_n109_));
  aoi21  g063(.a(new_n109_), .b(new_n107_), .c(new_n60_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n105_), .O(z13));
  nand2  g065(.a(new_n103_), .b(x24), .O(new_n112_));
  nor2   g066(.a(x24), .b(x21), .O(new_n113_));
  nand4  g067(.a(new_n113_), .b(new_n102_), .c(new_n73_), .d(new_n72_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(x16), .O(new_n116_));
  inv1   g070(.a(x05), .O(new_n117_));
  nand2  g071(.a(x18), .b(new_n117_), .O(new_n118_));
  inv1   g072(.a(x13), .O(new_n119_));
  aoi21  g073(.a(new_n77_), .b(new_n119_), .c(x16), .O(new_n120_));
  aoi21  g074(.a(new_n120_), .b(new_n118_), .c(new_n60_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n116_), .O(z14));
  nand2  g076(.a(new_n114_), .b(x25), .O(new_n123_));
  nor2   g077(.a(x25), .b(x24), .O(new_n124_));
  nand4  g078(.a(new_n124_), .b(new_n102_), .c(new_n84_), .d(new_n73_), .O(new_n125_));
  and2   g079(.a(new_n125_), .b(x16), .O(new_n126_));
  nand2  g080(.a(x18), .b(x06), .O(new_n127_));
  nand2  g081(.a(new_n77_), .b(x14), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n127_), .c(new_n63_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n61_), .O(new_n130_));
  aoi21  g084(.a(new_n126_), .b(new_n123_), .c(new_n130_), .O(z15));
  inv1   g085(.a(x26), .O(new_n132_));
  nor2   g086(.a(new_n125_), .b(new_n132_), .O(new_n133_));
  nand2  g087(.a(new_n125_), .b(new_n132_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(x16), .O(new_n135_));
  inv1   g089(.a(x07), .O(new_n136_));
  nand2  g090(.a(x18), .b(new_n136_), .O(new_n137_));
  inv1   g091(.a(x15), .O(new_n138_));
  aoi21  g092(.a(new_n77_), .b(new_n138_), .c(x16), .O(new_n139_));
  aoi21  g093(.a(new_n139_), .b(new_n137_), .c(new_n60_), .O(new_n140_));
  oai21  g094(.a(new_n135_), .b(new_n133_), .c(new_n140_), .O(z16));
  nand2  g095(.a(new_n124_), .b(new_n102_), .O(new_n142_));
  nand2  g096(.a(x27), .b(x17), .O(new_n143_));
  nand4  g097(.a(new_n84_), .b(new_n132_), .c(x19), .d(new_n66_), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n142_), .c(new_n143_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(x16), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n61_), .O(z17));
  buf    g101(.a(x27), .O(z08));
endmodule


