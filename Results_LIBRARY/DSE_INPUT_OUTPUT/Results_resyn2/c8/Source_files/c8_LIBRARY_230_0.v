// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x21), .O(z08));
  inv1   g002(.a(z08), .O(new_n49_));
  aoi22  g003(.a(new_n49_), .b(x19), .c(x27), .d(x08), .O(z00));
  aoi22  g004(.a(new_n49_), .b(x20), .c(x27), .d(x09), .O(z01));
  nor2   g005(.a(new_n47_), .b(x10), .O(z02));
  aoi22  g006(.a(new_n49_), .b(x22), .c(x27), .d(x11), .O(z03));
  aoi22  g007(.a(new_n49_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi22  g008(.a(new_n49_), .b(x24), .c(x27), .d(x13), .O(z05));
  oai22  g009(.a(z08), .b(x25), .c(new_n47_), .d(x14), .O(z06));
  oai22  g010(.a(z08), .b(x26), .c(new_n47_), .d(x15), .O(z07));
  nor2   g011(.a(x27), .b(x21), .O(new_n58_));
  inv1   g012(.a(x17), .O(new_n59_));
  inv1   g013(.a(x19), .O(new_n60_));
  nor2   g014(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g015(.a(x19), .b(x17), .O(new_n62_));
  oai21  g016(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g017(.a(x18), .O(new_n64_));
  inv1   g018(.a(x08), .O(new_n65_));
  aoi21  g019(.a(new_n64_), .b(new_n65_), .c(x16), .O(new_n66_));
  oai21  g020(.a(new_n64_), .b(x00), .c(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n63_), .c(new_n58_), .O(z09));
  inv1   g022(.a(x20), .O(new_n69_));
  nor2   g023(.a(new_n62_), .b(new_n69_), .O(new_n70_));
  nor3   g024(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  oai21  g025(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  inv1   g026(.a(x09), .O(new_n73_));
  aoi21  g027(.a(new_n64_), .b(new_n73_), .c(x16), .O(new_n74_));
  oai21  g028(.a(new_n64_), .b(x01), .c(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n72_), .c(new_n58_), .O(z10));
  inv1   g030(.a(x21), .O(new_n77_));
  nand2  g031(.a(new_n71_), .b(x16), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(x27), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  inv1   g034(.a(x10), .O(new_n81_));
  aoi21  g035(.a(new_n64_), .b(new_n81_), .c(x16), .O(new_n82_));
  oai21  g036(.a(new_n64_), .b(x02), .c(new_n82_), .O(new_n83_));
  nand3  g037(.a(new_n69_), .b(new_n60_), .c(new_n59_), .O(new_n84_));
  nand3  g038(.a(new_n84_), .b(x21), .c(x16), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(new_n83_), .c(new_n80_), .O(z11));
  inv1   g040(.a(x16), .O(new_n87_));
  nor3   g041(.a(new_n84_), .b(x22), .c(new_n87_), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n47_), .c(new_n77_), .O(new_n89_));
  inv1   g043(.a(x03), .O(new_n90_));
  nand2  g044(.a(x18), .b(new_n90_), .O(new_n91_));
  inv1   g045(.a(x11), .O(new_n92_));
  aoi21  g046(.a(new_n64_), .b(new_n92_), .c(x16), .O(new_n93_));
  nand2  g047(.a(new_n71_), .b(new_n77_), .O(new_n94_));
  inv1   g048(.a(x22), .O(new_n95_));
  nor2   g049(.a(new_n95_), .b(new_n87_), .O(new_n96_));
  aoi22  g050(.a(new_n96_), .b(new_n94_), .c(new_n93_), .d(new_n91_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n89_), .O(z12));
  inv1   g052(.a(x23), .O(new_n99_));
  oai21  g053(.a(new_n84_), .b(x22), .c(x27), .O(new_n100_));
  aoi21  g054(.a(new_n100_), .b(new_n77_), .c(new_n99_), .O(new_n101_));
  nand2  g055(.a(new_n99_), .b(new_n95_), .O(new_n102_));
  nand3  g056(.a(new_n71_), .b(x27), .c(new_n77_), .O(new_n103_));
  nor2   g057(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  oai21  g058(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  nor2   g059(.a(new_n58_), .b(x16), .O(new_n106_));
  inv1   g060(.a(x12), .O(new_n107_));
  nand2  g061(.a(new_n64_), .b(new_n107_), .O(new_n108_));
  inv1   g062(.a(x04), .O(new_n109_));
  nand2  g063(.a(x18), .b(new_n109_), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n105_), .O(z13));
  inv1   g066(.a(x24), .O(new_n113_));
  oai21  g067(.a(new_n102_), .b(new_n84_), .c(x27), .O(new_n114_));
  aoi21  g068(.a(new_n114_), .b(new_n77_), .c(new_n113_), .O(new_n115_));
  nor3   g069(.a(x24), .b(x23), .c(x22), .O(new_n116_));
  nand4  g070(.a(new_n116_), .b(new_n71_), .c(x27), .d(new_n77_), .O(new_n117_));
  inv1   g071(.a(new_n117_), .O(new_n118_));
  oai21  g072(.a(new_n118_), .b(new_n115_), .c(x16), .O(new_n119_));
  inv1   g073(.a(x13), .O(new_n120_));
  nand2  g074(.a(new_n64_), .b(new_n120_), .O(new_n121_));
  inv1   g075(.a(x05), .O(new_n122_));
  nand2  g076(.a(x18), .b(new_n122_), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n121_), .c(new_n106_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n119_), .O(z14));
  inv1   g079(.a(x25), .O(new_n126_));
  nand3  g080(.a(new_n113_), .b(new_n99_), .c(new_n95_), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n84_), .c(x27), .O(new_n128_));
  aoi21  g082(.a(new_n128_), .b(new_n77_), .c(new_n126_), .O(new_n129_));
  nor2   g083(.a(new_n117_), .b(x25), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n129_), .c(x16), .O(new_n131_));
  inv1   g085(.a(x14), .O(new_n132_));
  nand2  g086(.a(new_n64_), .b(new_n132_), .O(new_n133_));
  inv1   g087(.a(x06), .O(new_n134_));
  nand2  g088(.a(x18), .b(new_n134_), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n133_), .c(new_n106_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n131_), .O(z15));
  inv1   g091(.a(x26), .O(new_n138_));
  nor2   g092(.a(new_n138_), .b(new_n87_), .O(new_n139_));
  oai21  g093(.a(new_n117_), .b(x25), .c(new_n139_), .O(new_n140_));
  nand3  g094(.a(new_n116_), .b(new_n138_), .c(new_n126_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n78_), .c(x27), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n77_), .O(new_n143_));
  inv1   g097(.a(x15), .O(new_n144_));
  aoi21  g098(.a(new_n64_), .b(new_n144_), .c(x16), .O(new_n145_));
  oai21  g099(.a(new_n64_), .b(x07), .c(new_n145_), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n143_), .c(new_n140_), .O(z16));
  nand4  g101(.a(new_n138_), .b(new_n126_), .c(new_n69_), .d(x19), .O(new_n148_));
  oai21  g102(.a(new_n148_), .b(new_n127_), .c(x27), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n77_), .O(new_n150_));
  nand2  g104(.a(x27), .b(x17), .O(new_n151_));
  aoi21  g105(.a(new_n151_), .b(new_n150_), .c(new_n106_), .O(z17));
endmodule


