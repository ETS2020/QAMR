// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(x27), .b(x23), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi22  g003(.a(new_n49_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  aoi22  g004(.a(new_n49_), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  aoi22  g005(.a(new_n49_), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  oai22  g006(.a(new_n48_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai21  g007(.a(new_n47_), .b(x12), .c(x23), .O(z04));
  oai22  g008(.a(new_n48_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g009(.a(new_n49_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g010(.a(new_n48_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g011(.a(new_n47_), .b(x23), .O(new_n58_));
  inv1   g012(.a(x18), .O(new_n59_));
  inv1   g013(.a(x08), .O(new_n60_));
  aoi21  g014(.a(new_n59_), .b(new_n60_), .c(x16), .O(new_n61_));
  oai21  g015(.a(new_n59_), .b(x00), .c(new_n61_), .O(new_n62_));
  inv1   g016(.a(x17), .O(new_n63_));
  inv1   g017(.a(x16), .O(new_n64_));
  inv1   g018(.a(x19), .O(new_n65_));
  nor2   g019(.a(new_n65_), .b(x17), .O(new_n66_));
  nor2   g020(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  oai21  g021(.a(x19), .b(new_n63_), .c(new_n67_), .O(new_n68_));
  aoi21  g022(.a(new_n68_), .b(new_n62_), .c(new_n58_), .O(z09));
  inv1   g023(.a(x20), .O(new_n70_));
  nor2   g024(.a(x19), .b(x17), .O(new_n71_));
  nor2   g025(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor3   g026(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  oai21  g027(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  inv1   g028(.a(x01), .O(new_n75_));
  nand2  g029(.a(x18), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x09), .O(new_n77_));
  aoi21  g031(.a(new_n59_), .b(new_n77_), .c(x16), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n58_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n74_), .O(z10));
  inv1   g034(.a(x21), .O(new_n81_));
  nor2   g035(.a(new_n73_), .b(new_n81_), .O(new_n82_));
  nand4  g036(.a(new_n81_), .b(new_n70_), .c(new_n65_), .d(new_n63_), .O(new_n83_));
  inv1   g037(.a(new_n83_), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  inv1   g039(.a(x10), .O(new_n86_));
  aoi21  g040(.a(new_n59_), .b(new_n86_), .c(x16), .O(new_n87_));
  oai21  g041(.a(new_n59_), .b(x02), .c(new_n87_), .O(new_n88_));
  aoi21  g042(.a(new_n88_), .b(new_n85_), .c(new_n58_), .O(z11));
  inv1   g043(.a(x22), .O(new_n90_));
  nor2   g044(.a(x22), .b(x21), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n73_), .O(new_n92_));
  oai21  g046(.a(new_n84_), .b(new_n90_), .c(new_n92_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(x16), .O(new_n94_));
  inv1   g048(.a(x11), .O(new_n95_));
  aoi21  g049(.a(new_n59_), .b(new_n95_), .c(x16), .O(new_n96_));
  oai21  g050(.a(new_n59_), .b(x03), .c(new_n96_), .O(new_n97_));
  aoi21  g051(.a(new_n97_), .b(new_n94_), .c(new_n58_), .O(z12));
  inv1   g052(.a(x23), .O(new_n99_));
  oai21  g053(.a(new_n92_), .b(new_n64_), .c(new_n47_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  inv1   g055(.a(x04), .O(new_n102_));
  nand2  g056(.a(x18), .b(new_n102_), .O(new_n103_));
  inv1   g057(.a(x12), .O(new_n104_));
  aoi21  g058(.a(new_n59_), .b(new_n104_), .c(x16), .O(new_n105_));
  nor2   g059(.a(new_n99_), .b(new_n64_), .O(new_n106_));
  aoi22  g060(.a(new_n106_), .b(new_n92_), .c(new_n105_), .d(new_n103_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n101_), .O(z13));
  inv1   g062(.a(new_n58_), .O(new_n109_));
  nand4  g063(.a(new_n91_), .b(new_n71_), .c(new_n99_), .d(new_n70_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x24), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(x16), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n109_), .O(new_n113_));
  inv1   g067(.a(x24), .O(new_n114_));
  nand4  g068(.a(new_n47_), .b(new_n114_), .c(new_n99_), .d(new_n90_), .O(new_n115_));
  nor2   g069(.a(new_n115_), .b(new_n83_), .O(new_n116_));
  inv1   g070(.a(new_n116_), .O(new_n117_));
  inv1   g071(.a(x05), .O(new_n118_));
  nand2  g072(.a(x18), .b(new_n118_), .O(new_n119_));
  nor2   g073(.a(x18), .b(x13), .O(new_n120_));
  nor2   g074(.a(new_n120_), .b(new_n58_), .O(new_n121_));
  aoi21  g075(.a(new_n121_), .b(new_n119_), .c(x16), .O(new_n122_));
  aoi21  g076(.a(new_n117_), .b(new_n113_), .c(new_n122_), .O(z14));
  nor2   g077(.a(x25), .b(x24), .O(new_n124_));
  nand4  g078(.a(new_n124_), .b(new_n91_), .c(new_n73_), .d(x16), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n47_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n99_), .O(new_n127_));
  nor2   g081(.a(new_n59_), .b(x06), .O(new_n128_));
  oai21  g082(.a(x18), .b(x14), .c(new_n64_), .O(new_n129_));
  nor2   g083(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  inv1   g084(.a(x25), .O(new_n131_));
  nor2   g085(.a(new_n131_), .b(new_n64_), .O(new_n132_));
  aoi21  g086(.a(new_n132_), .b(new_n117_), .c(new_n130_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n127_), .O(z15));
  oai21  g088(.a(new_n125_), .b(x26), .c(new_n47_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n99_), .O(new_n136_));
  inv1   g090(.a(x26), .O(new_n137_));
  aoi21  g091(.a(new_n116_), .b(new_n131_), .c(new_n137_), .O(new_n138_));
  nand2  g092(.a(x18), .b(x07), .O(new_n139_));
  nand2  g093(.a(new_n59_), .b(x15), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n139_), .c(new_n64_), .O(new_n141_));
  oai21  g095(.a(new_n138_), .b(new_n64_), .c(new_n141_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n136_), .O(z16));
  nand2  g097(.a(new_n49_), .b(x17), .O(new_n144_));
  nand4  g098(.a(new_n47_), .b(new_n137_), .c(new_n99_), .d(new_n70_), .O(new_n145_));
  inv1   g099(.a(new_n145_), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n124_), .c(new_n91_), .d(new_n66_), .O(new_n147_));
  aoi21  g101(.a(new_n147_), .b(new_n144_), .c(new_n64_), .O(z17));
  buf    g102(.a(x27), .O(z08));
endmodule


