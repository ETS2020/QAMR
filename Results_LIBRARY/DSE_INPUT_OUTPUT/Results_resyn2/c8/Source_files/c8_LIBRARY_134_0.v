// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x07), .O(new_n48_));
  nor2   g02(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g03(.a(new_n49_), .O(z08));
  oai22  g04(.a(z08), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  aoi22  g05(.a(new_n49_), .b(x20), .c(x27), .d(x09), .O(z01));
  oai22  g06(.a(z08), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  oai22  g07(.a(z08), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  aoi22  g08(.a(new_n49_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi22  g09(.a(new_n49_), .b(x24), .c(x27), .d(x13), .O(z05));
  aoi22  g10(.a(new_n49_), .b(x25), .c(x27), .d(x14), .O(z06));
  aoi22  g11(.a(new_n49_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g12(.a(x18), .O(new_n59_));
  inv1   g13(.a(x08), .O(new_n60_));
  aoi21  g14(.a(new_n59_), .b(new_n60_), .c(x16), .O(new_n61_));
  oai21  g15(.a(new_n59_), .b(x00), .c(new_n61_), .O(new_n62_));
  and2   g16(.a(x19), .b(x17), .O(new_n63_));
  nor2   g17(.a(x19), .b(x17), .O(new_n64_));
  oai21  g18(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  nor2   g19(.a(x27), .b(x07), .O(new_n66_));
  inv1   g20(.a(new_n66_), .O(new_n67_));
  nand3  g21(.a(new_n67_), .b(new_n65_), .c(new_n62_), .O(z09));
  inv1   g22(.a(x20), .O(new_n69_));
  nor2   g23(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  nor3   g24(.a(x20), .b(x19), .c(x17), .O(new_n71_));
  oai21  g25(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  inv1   g26(.a(x01), .O(new_n73_));
  nand2  g27(.a(x18), .b(new_n73_), .O(new_n74_));
  inv1   g28(.a(x09), .O(new_n75_));
  aoi21  g29(.a(new_n59_), .b(new_n75_), .c(x16), .O(new_n76_));
  aoi21  g30(.a(new_n76_), .b(new_n74_), .c(new_n66_), .O(new_n77_));
  nand2  g31(.a(new_n77_), .b(new_n72_), .O(z10));
  inv1   g32(.a(x21), .O(new_n79_));
  nor2   g33(.a(new_n71_), .b(new_n79_), .O(new_n80_));
  nand3  g34(.a(new_n64_), .b(new_n79_), .c(new_n69_), .O(new_n81_));
  inv1   g35(.a(new_n81_), .O(new_n82_));
  oai21  g36(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  inv1   g37(.a(x02), .O(new_n84_));
  nand2  g38(.a(x18), .b(new_n84_), .O(new_n85_));
  inv1   g39(.a(x10), .O(new_n86_));
  aoi21  g40(.a(new_n59_), .b(new_n86_), .c(x16), .O(new_n87_));
  aoi21  g41(.a(new_n87_), .b(new_n85_), .c(new_n66_), .O(new_n88_));
  nand2  g42(.a(new_n88_), .b(new_n83_), .O(z11));
  inv1   g43(.a(x22), .O(new_n90_));
  xor2a  g44(.a(new_n81_), .b(new_n90_), .O(new_n91_));
  nand2  g45(.a(new_n91_), .b(x16), .O(new_n92_));
  inv1   g46(.a(x11), .O(new_n93_));
  aoi21  g47(.a(new_n59_), .b(new_n93_), .c(x16), .O(new_n94_));
  oai21  g48(.a(new_n59_), .b(x03), .c(new_n94_), .O(new_n95_));
  aoi21  g49(.a(new_n95_), .b(new_n92_), .c(new_n66_), .O(z12));
  inv1   g50(.a(x16), .O(new_n97_));
  nand4  g51(.a(new_n64_), .b(new_n90_), .c(new_n79_), .d(new_n69_), .O(new_n98_));
  nor2   g52(.a(x23), .b(x22), .O(new_n99_));
  aoi22  g53(.a(new_n99_), .b(new_n82_), .c(new_n98_), .d(x23), .O(new_n100_));
  inv1   g54(.a(x04), .O(new_n101_));
  nand2  g55(.a(x18), .b(new_n101_), .O(new_n102_));
  inv1   g56(.a(x12), .O(new_n103_));
  aoi21  g57(.a(new_n59_), .b(new_n103_), .c(x16), .O(new_n104_));
  aoi21  g58(.a(new_n104_), .b(new_n102_), .c(new_n66_), .O(new_n105_));
  oai21  g59(.a(new_n100_), .b(new_n97_), .c(new_n105_), .O(z13));
  nand4  g60(.a(new_n99_), .b(new_n64_), .c(new_n79_), .d(new_n69_), .O(new_n107_));
  inv1   g61(.a(x24), .O(new_n108_));
  nand3  g62(.a(new_n99_), .b(new_n108_), .c(new_n79_), .O(new_n109_));
  inv1   g63(.a(new_n109_), .O(new_n110_));
  aoi22  g64(.a(new_n110_), .b(new_n71_), .c(new_n107_), .d(x24), .O(new_n111_));
  inv1   g65(.a(x05), .O(new_n112_));
  nand2  g66(.a(x18), .b(new_n112_), .O(new_n113_));
  inv1   g67(.a(x13), .O(new_n114_));
  aoi21  g68(.a(new_n59_), .b(new_n114_), .c(x16), .O(new_n115_));
  aoi21  g69(.a(new_n115_), .b(new_n113_), .c(new_n66_), .O(new_n116_));
  oai21  g70(.a(new_n111_), .b(new_n97_), .c(new_n116_), .O(z14));
  nor2   g71(.a(x25), .b(x24), .O(new_n118_));
  nand4  g72(.a(new_n118_), .b(new_n99_), .c(new_n71_), .d(new_n79_), .O(new_n119_));
  nand4  g73(.a(new_n99_), .b(new_n71_), .c(new_n108_), .d(new_n79_), .O(new_n120_));
  aoi21  g74(.a(new_n120_), .b(x25), .c(new_n97_), .O(new_n121_));
  nand2  g75(.a(x18), .b(x06), .O(new_n122_));
  nand2  g76(.a(new_n59_), .b(x14), .O(new_n123_));
  nand3  g77(.a(new_n123_), .b(new_n122_), .c(new_n97_), .O(new_n124_));
  nand2  g78(.a(new_n124_), .b(new_n67_), .O(new_n125_));
  aoi21  g79(.a(new_n121_), .b(new_n119_), .c(new_n125_), .O(z15));
  inv1   g80(.a(x26), .O(new_n127_));
  nand4  g81(.a(new_n118_), .b(new_n99_), .c(new_n82_), .d(new_n127_), .O(new_n128_));
  aoi21  g82(.a(new_n119_), .b(x26), .c(new_n97_), .O(new_n129_));
  aoi21  g83(.a(new_n59_), .b(x15), .c(x16), .O(new_n130_));
  oai21  g84(.a(new_n59_), .b(new_n48_), .c(new_n130_), .O(new_n131_));
  nand2  g85(.a(new_n131_), .b(new_n67_), .O(new_n132_));
  aoi21  g86(.a(new_n129_), .b(new_n128_), .c(new_n132_), .O(z16));
  inv1   g87(.a(x17), .O(new_n134_));
  nor2   g88(.a(x26), .b(x25), .O(new_n135_));
  nand4  g89(.a(new_n135_), .b(new_n110_), .c(new_n69_), .d(x19), .O(new_n136_));
  oai21  g90(.a(x17), .b(new_n48_), .c(new_n47_), .O(new_n137_));
  nand2  g91(.a(new_n137_), .b(x16), .O(new_n138_));
  aoi21  g92(.a(new_n136_), .b(new_n134_), .c(new_n138_), .O(z17));
endmodule


