// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x07), .O(z08));
  aoi22  g02(.a(z08), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g03(.a(z08), .O(new_n50_));
  oai22  g04(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g05(.a(new_n50_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g06(.a(z08), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  oai22  g07(.a(new_n50_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g08(.a(new_n50_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g09(.a(z08), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g10(.a(new_n50_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g11(.a(x07), .O(new_n58_));
  nor2   g12(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  inv1   g13(.a(new_n59_), .O(new_n60_));
  inv1   g14(.a(x08), .O(new_n61_));
  inv1   g15(.a(x16), .O(new_n62_));
  oai21  g16(.a(x18), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  aoi21  g17(.a(x18), .b(x00), .c(new_n63_), .O(new_n64_));
  inv1   g18(.a(x17), .O(new_n65_));
  inv1   g19(.a(x19), .O(new_n66_));
  nand2  g20(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g21(.a(new_n67_), .b(x16), .O(new_n68_));
  aoi21  g22(.a(x19), .b(x17), .c(new_n68_), .O(new_n69_));
  oai21  g23(.a(new_n69_), .b(new_n64_), .c(new_n60_), .O(z09));
  inv1   g24(.a(x20), .O(new_n71_));
  nor2   g25(.a(x19), .b(x17), .O(new_n72_));
  nor2   g26(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nor2   g27(.a(new_n67_), .b(x20), .O(new_n74_));
  oai21  g28(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  inv1   g29(.a(x18), .O(new_n76_));
  inv1   g30(.a(x09), .O(new_n77_));
  aoi21  g31(.a(new_n76_), .b(new_n77_), .c(x16), .O(new_n78_));
  oai21  g32(.a(new_n76_), .b(x01), .c(new_n78_), .O(new_n79_));
  nand3  g33(.a(new_n79_), .b(new_n75_), .c(new_n60_), .O(z10));
  nand2  g34(.a(x18), .b(x02), .O(new_n81_));
  nand2  g35(.a(new_n76_), .b(x10), .O(new_n82_));
  nand3  g36(.a(new_n82_), .b(new_n81_), .c(new_n62_), .O(new_n83_));
  inv1   g37(.a(x21), .O(new_n84_));
  nor2   g38(.a(new_n74_), .b(new_n84_), .O(new_n85_));
  nor2   g39(.a(x21), .b(x20), .O(new_n86_));
  nand2  g40(.a(new_n86_), .b(new_n72_), .O(new_n87_));
  nand2  g41(.a(new_n87_), .b(x16), .O(new_n88_));
  oai21  g42(.a(new_n88_), .b(new_n85_), .c(new_n83_), .O(new_n89_));
  nand2  g43(.a(new_n89_), .b(new_n60_), .O(z11));
  inv1   g44(.a(x22), .O(new_n91_));
  aoi21  g45(.a(new_n86_), .b(new_n72_), .c(new_n91_), .O(new_n92_));
  nand3  g46(.a(new_n86_), .b(new_n72_), .c(new_n91_), .O(new_n93_));
  inv1   g47(.a(new_n93_), .O(new_n94_));
  oai21  g48(.a(new_n94_), .b(new_n92_), .c(x16), .O(new_n95_));
  inv1   g49(.a(x11), .O(new_n96_));
  aoi21  g50(.a(new_n76_), .b(new_n96_), .c(x16), .O(new_n97_));
  oai21  g51(.a(new_n76_), .b(x03), .c(new_n97_), .O(new_n98_));
  aoi21  g52(.a(new_n98_), .b(new_n95_), .c(new_n59_), .O(z12));
  inv1   g53(.a(x12), .O(new_n100_));
  oai21  g54(.a(x18), .b(new_n100_), .c(new_n62_), .O(new_n101_));
  aoi21  g55(.a(x18), .b(x04), .c(new_n101_), .O(new_n102_));
  nor2   g56(.a(x23), .b(x22), .O(new_n103_));
  nand3  g57(.a(new_n103_), .b(new_n86_), .c(new_n72_), .O(new_n104_));
  nand2  g58(.a(new_n104_), .b(x16), .O(new_n105_));
  aoi21  g59(.a(new_n93_), .b(x23), .c(new_n105_), .O(new_n106_));
  oai21  g60(.a(new_n106_), .b(new_n102_), .c(new_n60_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x24), .O(new_n108_));
  nor2   g62(.a(x22), .b(x21), .O(new_n109_));
  nor2   g63(.a(x24), .b(x23), .O(new_n110_));
  nand4  g64(.a(new_n110_), .b(new_n109_), .c(new_n72_), .d(new_n71_), .O(new_n111_));
  nand2  g65(.a(new_n111_), .b(new_n108_), .O(new_n112_));
  nand2  g66(.a(new_n112_), .b(x16), .O(new_n113_));
  inv1   g67(.a(x13), .O(new_n114_));
  aoi21  g68(.a(new_n76_), .b(new_n114_), .c(x16), .O(new_n115_));
  oai21  g69(.a(new_n76_), .b(x05), .c(new_n115_), .O(new_n116_));
  aoi21  g70(.a(new_n116_), .b(new_n113_), .c(new_n59_), .O(z14));
  nand2  g71(.a(new_n111_), .b(x25), .O(new_n118_));
  nor2   g72(.a(x25), .b(x24), .O(new_n119_));
  nand4  g73(.a(new_n119_), .b(new_n103_), .c(new_n86_), .d(new_n72_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g75(.a(new_n121_), .b(x16), .O(new_n122_));
  inv1   g76(.a(x14), .O(new_n123_));
  aoi21  g77(.a(new_n76_), .b(new_n123_), .c(x16), .O(new_n124_));
  oai21  g78(.a(new_n76_), .b(x06), .c(new_n124_), .O(new_n125_));
  aoi21  g79(.a(new_n125_), .b(new_n122_), .c(new_n59_), .O(z15));
  inv1   g80(.a(x26), .O(new_n127_));
  nand3  g81(.a(new_n119_), .b(new_n103_), .c(new_n127_), .O(new_n128_));
  nor2   g82(.a(new_n128_), .b(new_n87_), .O(new_n129_));
  aoi21  g83(.a(new_n120_), .b(x26), .c(new_n129_), .O(new_n130_));
  oai21  g84(.a(new_n76_), .b(x16), .c(new_n47_), .O(new_n131_));
  nand3  g85(.a(new_n76_), .b(new_n62_), .c(x15), .O(new_n132_));
  inv1   g86(.a(new_n132_), .O(new_n133_));
  aoi21  g87(.a(new_n131_), .b(x07), .c(new_n133_), .O(new_n134_));
  oai21  g88(.a(new_n130_), .b(new_n62_), .c(new_n134_), .O(z16));
  nand3  g89(.a(new_n86_), .b(x19), .c(new_n65_), .O(new_n136_));
  oai22  g90(.a(new_n136_), .b(new_n128_), .c(new_n47_), .d(new_n65_), .O(new_n137_));
  nand2  g91(.a(new_n137_), .b(new_n58_), .O(new_n138_));
  inv1   g92(.a(x25), .O(new_n139_));
  nand4  g93(.a(new_n47_), .b(new_n127_), .c(new_n139_), .d(new_n71_), .O(new_n140_));
  nor3   g94(.a(new_n140_), .b(new_n66_), .c(x17), .O(new_n141_));
  nand3  g95(.a(new_n141_), .b(new_n110_), .c(new_n109_), .O(new_n142_));
  aoi21  g96(.a(new_n142_), .b(new_n138_), .c(new_n62_), .O(z17));
endmodule


