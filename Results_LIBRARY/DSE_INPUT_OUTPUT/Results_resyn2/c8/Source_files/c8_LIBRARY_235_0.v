// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n51_, new_n52_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x16), .O(new_n47_));
  nor2   g01(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi22  g02(.a(new_n48_), .b(x19), .c(x27), .d(x08), .O(z00));
  aoi22  g03(.a(new_n48_), .b(x20), .c(x27), .d(x09), .O(z01));
  inv1   g04(.a(x27), .O(new_n51_));
  inv1   g05(.a(new_n48_), .O(new_n52_));
  oai22  g06(.a(new_n52_), .b(x21), .c(new_n51_), .d(x10), .O(z02));
  aoi22  g07(.a(new_n48_), .b(x22), .c(x27), .d(x11), .O(z03));
  oai22  g08(.a(new_n52_), .b(x23), .c(new_n51_), .d(x12), .O(z04));
  oai22  g09(.a(new_n52_), .b(x24), .c(new_n51_), .d(x13), .O(z05));
  oai22  g10(.a(new_n52_), .b(x25), .c(new_n51_), .d(x14), .O(z06));
  aoi22  g11(.a(new_n48_), .b(x26), .c(x27), .d(x15), .O(z07));
  nand2  g12(.a(x18), .b(x00), .O(new_n59_));
  inv1   g13(.a(x18), .O(new_n60_));
  nand2  g14(.a(x27), .b(new_n47_), .O(new_n61_));
  aoi21  g15(.a(new_n60_), .b(x08), .c(new_n61_), .O(new_n62_));
  nand2  g16(.a(x19), .b(x17), .O(new_n63_));
  nor2   g17(.a(x19), .b(x17), .O(new_n64_));
  nor2   g18(.a(new_n64_), .b(new_n47_), .O(new_n65_));
  aoi22  g19(.a(new_n65_), .b(new_n63_), .c(new_n62_), .d(new_n59_), .O(z09));
  inv1   g20(.a(x20), .O(new_n67_));
  xor2a  g21(.a(new_n64_), .b(new_n67_), .O(new_n68_));
  inv1   g22(.a(x09), .O(new_n69_));
  nand2  g23(.a(x18), .b(x01), .O(new_n70_));
  oai21  g24(.a(x18), .b(new_n69_), .c(new_n70_), .O(new_n71_));
  oai21  g25(.a(new_n71_), .b(new_n51_), .c(new_n47_), .O(new_n72_));
  oai21  g26(.a(new_n68_), .b(new_n47_), .c(new_n72_), .O(z10));
  nand2  g27(.a(x18), .b(x02), .O(new_n74_));
  aoi21  g28(.a(new_n60_), .b(x10), .c(new_n61_), .O(new_n75_));
  nand2  g29(.a(new_n64_), .b(new_n67_), .O(new_n76_));
  nand2  g30(.a(new_n76_), .b(x21), .O(new_n77_));
  nor2   g31(.a(x21), .b(x20), .O(new_n78_));
  aoi21  g32(.a(new_n78_), .b(new_n64_), .c(new_n47_), .O(new_n79_));
  aoi22  g33(.a(new_n79_), .b(new_n77_), .c(new_n75_), .d(new_n74_), .O(z11));
  nand2  g34(.a(new_n78_), .b(new_n64_), .O(new_n81_));
  xor2a  g35(.a(new_n81_), .b(x22), .O(new_n82_));
  inv1   g36(.a(x11), .O(new_n83_));
  nand2  g37(.a(x18), .b(x03), .O(new_n84_));
  oai21  g38(.a(x18), .b(new_n83_), .c(new_n84_), .O(new_n85_));
  oai21  g39(.a(new_n85_), .b(new_n51_), .c(new_n47_), .O(new_n86_));
  oai21  g40(.a(new_n82_), .b(new_n47_), .c(new_n86_), .O(z12));
  inv1   g41(.a(x22), .O(new_n88_));
  nand3  g42(.a(new_n78_), .b(new_n64_), .c(new_n88_), .O(new_n89_));
  nand2  g43(.a(new_n89_), .b(x23), .O(new_n90_));
  nor2   g44(.a(x23), .b(x22), .O(new_n91_));
  nand3  g45(.a(new_n91_), .b(new_n78_), .c(new_n64_), .O(new_n92_));
  nand2  g46(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g47(.a(new_n93_), .b(x16), .O(new_n94_));
  inv1   g48(.a(x12), .O(new_n95_));
  nand2  g49(.a(x18), .b(x04), .O(new_n96_));
  oai21  g50(.a(x18), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  oai21  g51(.a(new_n97_), .b(new_n51_), .c(new_n47_), .O(new_n98_));
  nand2  g52(.a(new_n98_), .b(new_n94_), .O(z13));
  nand2  g53(.a(new_n92_), .b(x24), .O(new_n100_));
  nor2   g54(.a(x24), .b(x21), .O(new_n101_));
  nand4  g55(.a(new_n101_), .b(new_n91_), .c(new_n64_), .d(new_n67_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g58(.a(x05), .O(new_n105_));
  aoi21  g59(.a(x18), .b(new_n105_), .c(new_n61_), .O(new_n106_));
  oai21  g60(.a(x18), .b(x13), .c(new_n106_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(new_n104_), .O(z14));
  nor2   g62(.a(new_n51_), .b(x16), .O(new_n109_));
  nand2  g63(.a(x18), .b(x06), .O(new_n110_));
  nand2  g64(.a(new_n60_), .b(x14), .O(new_n111_));
  nand3  g65(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  inv1   g66(.a(new_n112_), .O(new_n113_));
  nand2  g67(.a(new_n102_), .b(x25), .O(new_n114_));
  nor2   g68(.a(x25), .b(x24), .O(new_n115_));
  nand4  g69(.a(new_n115_), .b(new_n91_), .c(new_n78_), .d(new_n64_), .O(new_n116_));
  and2   g70(.a(new_n116_), .b(x16), .O(new_n117_));
  aoi21  g71(.a(new_n117_), .b(new_n114_), .c(new_n113_), .O(z15));
  nand2  g72(.a(x18), .b(x07), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(new_n109_), .O(new_n120_));
  aoi21  g74(.a(new_n60_), .b(x15), .c(new_n120_), .O(new_n121_));
  inv1   g75(.a(x26), .O(new_n122_));
  inv1   g76(.a(new_n116_), .O(new_n123_));
  nand2  g77(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  aoi21  g78(.a(new_n116_), .b(x26), .c(new_n47_), .O(new_n125_));
  aoi21  g79(.a(new_n125_), .b(new_n124_), .c(new_n121_), .O(z16));
  nand2  g80(.a(new_n51_), .b(new_n47_), .O(new_n127_));
  nand2  g81(.a(new_n115_), .b(new_n91_), .O(new_n128_));
  nand2  g82(.a(x27), .b(x17), .O(new_n129_));
  inv1   g83(.a(x17), .O(new_n130_));
  nand4  g84(.a(new_n78_), .b(new_n122_), .c(x19), .d(new_n130_), .O(new_n131_));
  oai21  g85(.a(new_n131_), .b(new_n128_), .c(new_n129_), .O(new_n132_));
  nand2  g86(.a(new_n132_), .b(x16), .O(new_n133_));
  nand2  g87(.a(new_n133_), .b(new_n127_), .O(z17));
  buf    g88(.a(x27), .O(z08));
endmodule


