// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x19), .O(new_n47_));
  nor2   g001(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi21  g002(.a(x27), .b(x08), .c(new_n48_), .O(z00));
  inv1   g003(.a(x27), .O(new_n50_));
  nand2  g004(.a(new_n50_), .b(new_n47_), .O(z08));
  oai22  g005(.a(z08), .b(x20), .c(new_n50_), .d(x09), .O(z01));
  oai22  g006(.a(z08), .b(x21), .c(new_n50_), .d(x10), .O(z02));
  oai22  g007(.a(z08), .b(x22), .c(new_n50_), .d(x11), .O(z03));
  oai22  g008(.a(z08), .b(x23), .c(new_n50_), .d(x12), .O(z04));
  oai22  g009(.a(z08), .b(x24), .c(new_n50_), .d(x13), .O(z05));
  oai22  g010(.a(z08), .b(x25), .c(new_n50_), .d(x14), .O(z06));
  oai22  g011(.a(z08), .b(x26), .c(new_n50_), .d(x15), .O(z07));
  nand2  g012(.a(new_n50_), .b(x19), .O(new_n59_));
  inv1   g013(.a(x18), .O(new_n60_));
  inv1   g014(.a(x08), .O(new_n61_));
  aoi21  g015(.a(new_n60_), .b(new_n61_), .c(x16), .O(new_n62_));
  oai21  g016(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  inv1   g017(.a(x17), .O(new_n64_));
  nor2   g018(.a(new_n47_), .b(new_n64_), .O(new_n65_));
  nor2   g019(.a(x19), .b(x17), .O(new_n66_));
  oai21  g020(.a(new_n66_), .b(new_n65_), .c(x16), .O(new_n67_));
  nand3  g021(.a(new_n67_), .b(new_n63_), .c(new_n59_), .O(z09));
  inv1   g022(.a(x09), .O(new_n69_));
  aoi21  g023(.a(new_n60_), .b(new_n69_), .c(x16), .O(new_n70_));
  oai21  g024(.a(new_n60_), .b(x01), .c(new_n70_), .O(new_n71_));
  nand2  g025(.a(x20), .b(x16), .O(new_n72_));
  nand2  g026(.a(new_n72_), .b(x27), .O(new_n73_));
  nand2  g027(.a(new_n73_), .b(x19), .O(new_n74_));
  inv1   g028(.a(x16), .O(new_n75_));
  aoi21  g029(.a(x20), .b(new_n64_), .c(new_n75_), .O(new_n76_));
  oai21  g030(.a(new_n66_), .b(x20), .c(new_n76_), .O(new_n77_));
  nand3  g031(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(z10));
  oai21  g032(.a(x20), .b(x17), .c(x21), .O(new_n79_));
  nor2   g033(.a(x21), .b(x20), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n64_), .O(new_n81_));
  aoi21  g035(.a(new_n81_), .b(new_n79_), .c(x19), .O(new_n82_));
  nand3  g036(.a(x27), .b(x21), .c(x19), .O(new_n83_));
  inv1   g037(.a(new_n83_), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  nor2   g039(.a(new_n48_), .b(x16), .O(new_n86_));
  inv1   g040(.a(x10), .O(new_n87_));
  nand2  g041(.a(new_n60_), .b(new_n87_), .O(new_n88_));
  inv1   g042(.a(x02), .O(new_n89_));
  nand2  g043(.a(x18), .b(new_n89_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n88_), .c(new_n86_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n85_), .O(z11));
  inv1   g046(.a(x22), .O(new_n93_));
  nor3   g047(.a(x21), .b(x20), .c(x17), .O(new_n94_));
  nor2   g048(.a(x22), .b(x17), .O(new_n95_));
  aoi21  g049(.a(new_n95_), .b(new_n80_), .c(x19), .O(new_n96_));
  oai21  g050(.a(new_n94_), .b(new_n93_), .c(new_n96_), .O(new_n97_));
  nand2  g051(.a(new_n59_), .b(x16), .O(new_n98_));
  aoi21  g052(.a(new_n93_), .b(x19), .c(new_n98_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  inv1   g054(.a(x11), .O(new_n101_));
  nand2  g055(.a(new_n60_), .b(new_n101_), .O(new_n102_));
  inv1   g056(.a(x03), .O(new_n103_));
  nand2  g057(.a(x18), .b(new_n103_), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n102_), .c(new_n86_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n100_), .O(z12));
  inv1   g060(.a(x23), .O(new_n107_));
  nand3  g061(.a(new_n95_), .b(new_n80_), .c(new_n47_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  inv1   g063(.a(new_n108_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x23), .O(new_n111_));
  nand4  g065(.a(new_n111_), .b(new_n109_), .c(new_n59_), .d(x16), .O(new_n112_));
  inv1   g066(.a(x12), .O(new_n113_));
  nand2  g067(.a(new_n60_), .b(new_n113_), .O(new_n114_));
  inv1   g068(.a(x04), .O(new_n115_));
  nand2  g069(.a(x18), .b(new_n115_), .O(new_n116_));
  nand3  g070(.a(new_n116_), .b(new_n114_), .c(new_n86_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n112_), .O(z13));
  inv1   g072(.a(x13), .O(new_n119_));
  nand2  g073(.a(new_n60_), .b(new_n119_), .O(new_n120_));
  inv1   g074(.a(x05), .O(new_n121_));
  nand2  g075(.a(x18), .b(new_n121_), .O(new_n122_));
  nand3  g076(.a(new_n122_), .b(new_n120_), .c(new_n86_), .O(new_n123_));
  inv1   g077(.a(x24), .O(new_n124_));
  nand4  g078(.a(new_n95_), .b(new_n80_), .c(new_n107_), .d(new_n47_), .O(new_n125_));
  nor2   g079(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  inv1   g080(.a(new_n125_), .O(new_n127_));
  nor2   g081(.a(new_n48_), .b(new_n124_), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n127_), .c(x16), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n126_), .c(new_n123_), .O(z14));
  nor3   g084(.a(x25), .b(x24), .c(x22), .O(new_n131_));
  nand4  g085(.a(new_n131_), .b(new_n94_), .c(new_n107_), .d(new_n47_), .O(new_n132_));
  nand4  g086(.a(new_n95_), .b(new_n80_), .c(new_n124_), .d(new_n107_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x25), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(x16), .O(new_n136_));
  nand2  g090(.a(x25), .b(x16), .O(new_n137_));
  aoi21  g091(.a(new_n137_), .b(x27), .c(new_n47_), .O(new_n138_));
  inv1   g092(.a(x06), .O(new_n139_));
  nand2  g093(.a(x18), .b(new_n139_), .O(new_n140_));
  inv1   g094(.a(x14), .O(new_n141_));
  aoi21  g095(.a(new_n60_), .b(new_n141_), .c(x16), .O(new_n142_));
  aoi21  g096(.a(new_n142_), .b(new_n140_), .c(new_n138_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n136_), .O(z15));
  inv1   g098(.a(x26), .O(new_n145_));
  nand2  g099(.a(new_n132_), .b(new_n145_), .O(new_n146_));
  nand4  g100(.a(new_n131_), .b(new_n94_), .c(x26), .d(new_n107_), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n146_), .c(x16), .O(new_n148_));
  nand2  g102(.a(x26), .b(x16), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(x27), .c(new_n47_), .O(new_n150_));
  inv1   g104(.a(x07), .O(new_n151_));
  nand2  g105(.a(x18), .b(new_n151_), .O(new_n152_));
  inv1   g106(.a(x15), .O(new_n153_));
  aoi21  g107(.a(new_n60_), .b(new_n153_), .c(x16), .O(new_n154_));
  aoi21  g108(.a(new_n154_), .b(new_n152_), .c(new_n150_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n148_), .O(z16));
  nor2   g110(.a(x23), .b(new_n47_), .O(new_n157_));
  nand4  g111(.a(new_n157_), .b(new_n131_), .c(new_n80_), .d(new_n145_), .O(new_n158_));
  nand2  g112(.a(x27), .b(x16), .O(new_n159_));
  aoi21  g113(.a(new_n158_), .b(new_n64_), .c(new_n159_), .O(z17));
endmodule


