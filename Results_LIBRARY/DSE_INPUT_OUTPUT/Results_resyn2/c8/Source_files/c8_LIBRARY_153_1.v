// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_;
  inv1   g00(.a(x27), .O(new_n47_));
  nand2  g01(.a(x27), .b(x16), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  aoi22  g03(.a(new_n49_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  aoi22  g04(.a(new_n49_), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  oai22  g05(.a(new_n48_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g06(.a(new_n49_), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  aoi22  g07(.a(new_n49_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  oai22  g08(.a(new_n48_), .b(x13), .c(x27), .d(x24), .O(z05));
  oai22  g09(.a(new_n48_), .b(x14), .c(x27), .d(x25), .O(z06));
  oai22  g10(.a(new_n48_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g11(.a(x17), .O(new_n58_));
  inv1   g12(.a(x19), .O(new_n59_));
  nor2   g13(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor2   g14(.a(x19), .b(x17), .O(new_n61_));
  oai21  g15(.a(new_n61_), .b(new_n60_), .c(x16), .O(new_n62_));
  inv1   g16(.a(x18), .O(new_n63_));
  nor2   g17(.a(new_n63_), .b(x00), .O(new_n64_));
  nor2   g18(.a(x27), .b(x16), .O(new_n65_));
  oai21  g19(.a(x18), .b(x08), .c(new_n65_), .O(new_n66_));
  oai21  g20(.a(new_n66_), .b(new_n64_), .c(new_n62_), .O(z09));
  inv1   g21(.a(x16), .O(new_n68_));
  inv1   g22(.a(x20), .O(new_n69_));
  xor2a  g23(.a(new_n61_), .b(new_n69_), .O(new_n70_));
  nor2   g24(.a(new_n63_), .b(x01), .O(new_n71_));
  oai21  g25(.a(x18), .b(x09), .c(new_n65_), .O(new_n72_));
  oai22  g26(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n68_), .O(z10));
  inv1   g27(.a(x21), .O(new_n74_));
  aoi21  g28(.a(new_n61_), .b(new_n69_), .c(new_n74_), .O(new_n75_));
  nand4  g29(.a(new_n74_), .b(new_n69_), .c(new_n59_), .d(new_n58_), .O(new_n76_));
  inv1   g30(.a(new_n76_), .O(new_n77_));
  oai21  g31(.a(new_n77_), .b(new_n75_), .c(x16), .O(new_n78_));
  nor2   g32(.a(new_n63_), .b(x02), .O(new_n79_));
  oai21  g33(.a(x18), .b(x10), .c(new_n65_), .O(new_n80_));
  oai21  g34(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(z11));
  nand2  g35(.a(x18), .b(x03), .O(new_n82_));
  nand2  g36(.a(new_n63_), .b(x11), .O(new_n83_));
  nand3  g37(.a(new_n83_), .b(new_n82_), .c(new_n65_), .O(new_n84_));
  inv1   g38(.a(new_n84_), .O(new_n85_));
  nand2  g39(.a(new_n76_), .b(x22), .O(new_n86_));
  inv1   g40(.a(x22), .O(new_n87_));
  nor2   g41(.a(x21), .b(x20), .O(new_n88_));
  nand3  g42(.a(new_n88_), .b(new_n61_), .c(new_n87_), .O(new_n89_));
  and2   g43(.a(new_n89_), .b(x16), .O(new_n90_));
  aoi21  g44(.a(new_n90_), .b(new_n86_), .c(new_n85_), .O(z12));
  nand2  g45(.a(new_n89_), .b(x23), .O(new_n92_));
  nor2   g46(.a(x23), .b(x22), .O(new_n93_));
  nand3  g47(.a(new_n93_), .b(new_n88_), .c(new_n61_), .O(new_n94_));
  nand2  g48(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g49(.a(new_n95_), .b(x16), .O(new_n96_));
  inv1   g50(.a(x04), .O(new_n97_));
  nand2  g51(.a(x18), .b(new_n97_), .O(new_n98_));
  inv1   g52(.a(x12), .O(new_n99_));
  nand2  g53(.a(new_n63_), .b(new_n99_), .O(new_n100_));
  nand3  g54(.a(new_n100_), .b(new_n98_), .c(new_n65_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(new_n96_), .O(z13));
  nand2  g56(.a(new_n94_), .b(x24), .O(new_n103_));
  nor2   g57(.a(x24), .b(x21), .O(new_n104_));
  nand4  g58(.a(new_n104_), .b(new_n93_), .c(new_n61_), .d(new_n69_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand2  g60(.a(new_n106_), .b(x16), .O(new_n107_));
  inv1   g61(.a(x05), .O(new_n108_));
  nand2  g62(.a(x18), .b(new_n108_), .O(new_n109_));
  inv1   g63(.a(x13), .O(new_n110_));
  nand2  g64(.a(new_n63_), .b(new_n110_), .O(new_n111_));
  nand3  g65(.a(new_n111_), .b(new_n109_), .c(new_n65_), .O(new_n112_));
  nand2  g66(.a(new_n112_), .b(new_n107_), .O(z14));
  inv1   g67(.a(x23), .O(new_n114_));
  inv1   g68(.a(x24), .O(new_n115_));
  inv1   g69(.a(x25), .O(new_n116_));
  nand4  g70(.a(new_n116_), .b(new_n115_), .c(new_n114_), .d(new_n87_), .O(new_n117_));
  inv1   g71(.a(new_n117_), .O(new_n118_));
  aoi22  g72(.a(new_n118_), .b(new_n77_), .c(new_n105_), .d(x25), .O(new_n119_));
  inv1   g73(.a(x06), .O(new_n120_));
  nand2  g74(.a(x18), .b(new_n120_), .O(new_n121_));
  inv1   g75(.a(x14), .O(new_n122_));
  nand2  g76(.a(new_n63_), .b(new_n122_), .O(new_n123_));
  nand3  g77(.a(new_n123_), .b(new_n121_), .c(new_n65_), .O(new_n124_));
  oai21  g78(.a(new_n119_), .b(new_n68_), .c(new_n124_), .O(z15));
  inv1   g79(.a(x07), .O(new_n126_));
  nand2  g80(.a(x18), .b(new_n126_), .O(new_n127_));
  inv1   g81(.a(x15), .O(new_n128_));
  nand2  g82(.a(new_n63_), .b(new_n128_), .O(new_n129_));
  nand3  g83(.a(new_n129_), .b(new_n127_), .c(new_n65_), .O(new_n130_));
  inv1   g84(.a(x26), .O(new_n131_));
  nor2   g85(.a(x25), .b(x24), .O(new_n132_));
  nand4  g86(.a(new_n132_), .b(new_n93_), .c(new_n88_), .d(new_n61_), .O(new_n133_));
  nor2   g87(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g88(.a(new_n133_), .b(new_n131_), .O(new_n135_));
  nand2  g89(.a(new_n135_), .b(x16), .O(new_n136_));
  oai21  g90(.a(new_n136_), .b(new_n134_), .c(new_n130_), .O(z16));
  nor2   g91(.a(x17), .b(new_n68_), .O(new_n138_));
  nand4  g92(.a(new_n138_), .b(new_n88_), .c(new_n131_), .d(x19), .O(new_n139_));
  oai22  g93(.a(new_n139_), .b(new_n117_), .c(new_n138_), .d(new_n47_), .O(z17));
  buf    g94(.a(x27), .O(z08));
endmodule


