// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n50_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n145_, new_n146_;
  nand2  g000(.a(x27), .b(x22), .O(new_n47_));
  oai22  g001(.a(new_n47_), .b(x08), .c(x27), .d(x19), .O(z00));
  oai22  g002(.a(new_n47_), .b(x09), .c(x27), .d(x20), .O(z01));
  inv1   g003(.a(x27), .O(new_n50_));
  inv1   g004(.a(new_n47_), .O(z08));
  aoi22  g005(.a(z08), .b(x10), .c(new_n50_), .d(x21), .O(z02));
  oai21  g006(.a(new_n50_), .b(x11), .c(x22), .O(z03));
  aoi22  g007(.a(z08), .b(x12), .c(new_n50_), .d(x23), .O(z04));
  aoi22  g008(.a(z08), .b(x13), .c(new_n50_), .d(x24), .O(z05));
  oai22  g009(.a(new_n47_), .b(x14), .c(x27), .d(x25), .O(z06));
  oai22  g010(.a(new_n47_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g011(.a(new_n50_), .b(x22), .O(new_n58_));
  inv1   g012(.a(x18), .O(new_n59_));
  inv1   g013(.a(x08), .O(new_n60_));
  aoi21  g014(.a(new_n59_), .b(new_n60_), .c(x16), .O(new_n61_));
  oai21  g015(.a(new_n59_), .b(x00), .c(new_n61_), .O(new_n62_));
  inv1   g016(.a(x19), .O(new_n63_));
  nand2  g017(.a(new_n63_), .b(x17), .O(new_n64_));
  inv1   g018(.a(x16), .O(new_n65_));
  nor2   g019(.a(new_n63_), .b(x17), .O(new_n66_));
  nor2   g020(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g021(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  aoi21  g022(.a(new_n68_), .b(new_n62_), .c(new_n58_), .O(z09));
  inv1   g023(.a(x20), .O(new_n70_));
  nor2   g024(.a(x19), .b(x17), .O(new_n71_));
  xor2a  g025(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g026(.a(x01), .O(new_n73_));
  nand2  g027(.a(x18), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x09), .O(new_n75_));
  aoi21  g029(.a(new_n59_), .b(new_n75_), .c(x16), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n58_), .O(new_n77_));
  oai21  g031(.a(new_n72_), .b(new_n65_), .c(new_n77_), .O(z10));
  inv1   g032(.a(x21), .O(new_n79_));
  aoi21  g033(.a(new_n71_), .b(new_n70_), .c(new_n79_), .O(new_n80_));
  nor2   g034(.a(x21), .b(x20), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  inv1   g036(.a(new_n82_), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x10), .O(new_n85_));
  aoi21  g039(.a(new_n59_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g040(.a(new_n59_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g041(.a(new_n87_), .b(new_n84_), .c(new_n58_), .O(z11));
  xor2a  g042(.a(new_n82_), .b(x22), .O(new_n89_));
  inv1   g043(.a(x03), .O(new_n90_));
  nand2  g044(.a(x18), .b(new_n90_), .O(new_n91_));
  inv1   g045(.a(x11), .O(new_n92_));
  aoi21  g046(.a(new_n59_), .b(new_n92_), .c(x16), .O(new_n93_));
  aoi21  g047(.a(new_n93_), .b(new_n91_), .c(new_n58_), .O(new_n94_));
  oai21  g048(.a(new_n89_), .b(new_n65_), .c(new_n94_), .O(z12));
  inv1   g049(.a(x23), .O(new_n96_));
  inv1   g050(.a(x22), .O(new_n97_));
  nand3  g051(.a(new_n81_), .b(new_n71_), .c(new_n97_), .O(new_n98_));
  aoi21  g052(.a(new_n98_), .b(new_n96_), .c(new_n65_), .O(new_n99_));
  oai21  g053(.a(new_n98_), .b(new_n96_), .c(new_n99_), .O(new_n100_));
  inv1   g054(.a(x12), .O(new_n101_));
  aoi21  g055(.a(new_n59_), .b(new_n101_), .c(x16), .O(new_n102_));
  oai21  g056(.a(new_n59_), .b(x04), .c(new_n102_), .O(new_n103_));
  aoi21  g057(.a(new_n103_), .b(new_n100_), .c(new_n58_), .O(z13));
  inv1   g058(.a(x24), .O(new_n105_));
  nand3  g059(.a(new_n81_), .b(new_n71_), .c(new_n96_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n50_), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n97_), .c(new_n105_), .O(new_n108_));
  nor2   g062(.a(x23), .b(x22), .O(new_n109_));
  nor2   g063(.a(x27), .b(x24), .O(new_n110_));
  nand4  g064(.a(new_n110_), .b(new_n109_), .c(new_n81_), .d(new_n71_), .O(new_n111_));
  inv1   g065(.a(new_n111_), .O(new_n112_));
  oai21  g066(.a(new_n112_), .b(new_n108_), .c(x16), .O(new_n113_));
  nor2   g067(.a(new_n58_), .b(x16), .O(new_n114_));
  inv1   g068(.a(x13), .O(new_n115_));
  nand2  g069(.a(new_n59_), .b(new_n115_), .O(new_n116_));
  inv1   g070(.a(x05), .O(new_n117_));
  nand2  g071(.a(x18), .b(new_n117_), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n116_), .c(new_n114_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n113_), .O(z14));
  inv1   g074(.a(x25), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n105_), .O(new_n122_));
  nand4  g076(.a(new_n81_), .b(new_n71_), .c(new_n96_), .d(x16), .O(new_n123_));
  oai21  g077(.a(new_n123_), .b(new_n122_), .c(new_n50_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n97_), .O(new_n125_));
  inv1   g079(.a(x06), .O(new_n126_));
  nand2  g080(.a(x18), .b(new_n126_), .O(new_n127_));
  inv1   g081(.a(x14), .O(new_n128_));
  aoi21  g082(.a(new_n59_), .b(new_n128_), .c(x16), .O(new_n129_));
  nor2   g083(.a(new_n121_), .b(new_n65_), .O(new_n130_));
  aoi22  g084(.a(new_n130_), .b(new_n111_), .c(new_n129_), .d(new_n127_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n125_), .O(z15));
  inv1   g086(.a(new_n123_), .O(new_n133_));
  nor2   g087(.a(new_n122_), .b(x26), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n50_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n97_), .O(new_n137_));
  oai21  g091(.a(new_n111_), .b(x25), .c(x26), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x16), .O(new_n139_));
  nand2  g093(.a(x18), .b(x07), .O(new_n140_));
  nand2  g094(.a(new_n59_), .b(x15), .O(new_n141_));
  nand3  g095(.a(new_n141_), .b(new_n140_), .c(new_n65_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n139_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n137_), .O(z16));
  oai21  g098(.a(new_n97_), .b(x17), .c(x27), .O(new_n145_));
  nand4  g099(.a(new_n134_), .b(new_n109_), .c(new_n81_), .d(new_n66_), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n145_), .c(new_n114_), .O(z17));
endmodule


