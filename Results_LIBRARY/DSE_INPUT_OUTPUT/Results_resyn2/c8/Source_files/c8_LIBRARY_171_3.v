// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n51_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x03), .O(new_n48_));
  aoi22  g02(.a(new_n48_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  aoi22  g03(.a(new_n48_), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  inv1   g04(.a(new_n48_), .O(new_n51_));
  oai22  g05(.a(new_n51_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g06(.a(new_n48_), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  oai22  g07(.a(new_n51_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g08(.a(new_n51_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g09(.a(new_n48_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g10(.a(new_n51_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g11(.a(x03), .O(new_n58_));
  nor2   g12(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  nor2   g13(.a(x19), .b(x17), .O(new_n60_));
  inv1   g14(.a(x17), .O(new_n61_));
  inv1   g15(.a(x19), .O(new_n62_));
  nor2   g16(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  oai21  g17(.a(new_n63_), .b(new_n60_), .c(x16), .O(new_n64_));
  inv1   g18(.a(x18), .O(new_n65_));
  inv1   g19(.a(x08), .O(new_n66_));
  aoi21  g20(.a(new_n65_), .b(new_n66_), .c(x16), .O(new_n67_));
  oai21  g21(.a(new_n65_), .b(x00), .c(new_n67_), .O(new_n68_));
  aoi21  g22(.a(new_n68_), .b(new_n64_), .c(new_n59_), .O(z09));
  xor2a  g23(.a(new_n60_), .b(x20), .O(new_n70_));
  nand2  g24(.a(new_n70_), .b(x16), .O(new_n71_));
  inv1   g25(.a(x09), .O(new_n72_));
  aoi21  g26(.a(new_n65_), .b(new_n72_), .c(x16), .O(new_n73_));
  oai21  g27(.a(new_n65_), .b(x01), .c(new_n73_), .O(new_n74_));
  aoi21  g28(.a(new_n74_), .b(new_n71_), .c(new_n59_), .O(z10));
  inv1   g29(.a(x16), .O(new_n76_));
  inv1   g30(.a(x20), .O(new_n77_));
  nand2  g31(.a(new_n60_), .b(new_n77_), .O(new_n78_));
  nor2   g32(.a(x21), .b(x20), .O(new_n79_));
  nand2  g33(.a(new_n79_), .b(new_n60_), .O(new_n80_));
  inv1   g34(.a(new_n80_), .O(new_n81_));
  aoi21  g35(.a(new_n78_), .b(x21), .c(new_n81_), .O(new_n82_));
  inv1   g36(.a(x02), .O(new_n83_));
  nand2  g37(.a(x18), .b(new_n83_), .O(new_n84_));
  inv1   g38(.a(x10), .O(new_n85_));
  aoi21  g39(.a(new_n65_), .b(new_n85_), .c(x16), .O(new_n86_));
  aoi21  g40(.a(new_n86_), .b(new_n84_), .c(new_n59_), .O(new_n87_));
  oai21  g41(.a(new_n82_), .b(new_n76_), .c(new_n87_), .O(z11));
  nor3   g42(.a(x22), .b(x21), .c(x20), .O(new_n89_));
  nand2  g43(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  aoi21  g44(.a(new_n80_), .b(x22), .c(new_n76_), .O(new_n91_));
  inv1   g45(.a(new_n59_), .O(new_n92_));
  aoi21  g46(.a(new_n65_), .b(x11), .c(x16), .O(new_n93_));
  oai21  g47(.a(new_n65_), .b(new_n58_), .c(new_n93_), .O(new_n94_));
  nand2  g48(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  aoi21  g49(.a(new_n91_), .b(new_n90_), .c(new_n95_), .O(z12));
  nor3   g50(.a(x23), .b(x19), .c(x17), .O(new_n97_));
  aoi22  g51(.a(new_n97_), .b(new_n89_), .c(new_n90_), .d(x23), .O(new_n98_));
  inv1   g52(.a(x04), .O(new_n99_));
  nand2  g53(.a(x18), .b(new_n99_), .O(new_n100_));
  inv1   g54(.a(x12), .O(new_n101_));
  aoi21  g55(.a(new_n65_), .b(new_n101_), .c(x16), .O(new_n102_));
  aoi21  g56(.a(new_n102_), .b(new_n100_), .c(new_n59_), .O(new_n103_));
  oai21  g57(.a(new_n98_), .b(new_n76_), .c(new_n103_), .O(z13));
  inv1   g58(.a(x24), .O(new_n105_));
  aoi21  g59(.a(new_n97_), .b(new_n89_), .c(new_n105_), .O(new_n106_));
  inv1   g60(.a(x22), .O(new_n107_));
  nor2   g61(.a(x24), .b(x23), .O(new_n108_));
  nand4  g62(.a(new_n108_), .b(new_n79_), .c(new_n60_), .d(new_n107_), .O(new_n109_));
  inv1   g63(.a(new_n109_), .O(new_n110_));
  oai21  g64(.a(new_n110_), .b(new_n106_), .c(x16), .O(new_n111_));
  inv1   g65(.a(x13), .O(new_n112_));
  aoi21  g66(.a(new_n65_), .b(new_n112_), .c(x16), .O(new_n113_));
  oai21  g67(.a(new_n65_), .b(x05), .c(new_n113_), .O(new_n114_));
  aoi21  g68(.a(new_n114_), .b(new_n111_), .c(new_n59_), .O(z14));
  nand2  g69(.a(new_n109_), .b(x25), .O(new_n116_));
  nor2   g70(.a(x25), .b(x22), .O(new_n117_));
  nand4  g71(.a(new_n117_), .b(new_n108_), .c(new_n79_), .d(new_n60_), .O(new_n118_));
  and2   g72(.a(new_n118_), .b(x16), .O(new_n119_));
  nand2  g73(.a(x18), .b(x06), .O(new_n120_));
  nand2  g74(.a(new_n65_), .b(x14), .O(new_n121_));
  nand3  g75(.a(new_n121_), .b(new_n120_), .c(new_n76_), .O(new_n122_));
  nand2  g76(.a(new_n122_), .b(new_n92_), .O(new_n123_));
  aoi21  g77(.a(new_n119_), .b(new_n116_), .c(new_n123_), .O(z15));
  nand2  g78(.a(new_n117_), .b(new_n108_), .O(new_n125_));
  inv1   g79(.a(new_n125_), .O(new_n126_));
  nand3  g80(.a(new_n126_), .b(new_n81_), .c(x26), .O(new_n127_));
  inv1   g81(.a(x26), .O(new_n128_));
  aoi21  g82(.a(new_n118_), .b(new_n128_), .c(new_n76_), .O(new_n129_));
  nand2  g83(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  inv1   g84(.a(x15), .O(new_n131_));
  aoi21  g85(.a(new_n65_), .b(new_n131_), .c(x16), .O(new_n132_));
  oai21  g86(.a(new_n65_), .b(x07), .c(new_n132_), .O(new_n133_));
  aoi21  g87(.a(new_n133_), .b(new_n130_), .c(new_n59_), .O(z16));
  nand3  g88(.a(new_n79_), .b(new_n128_), .c(x19), .O(new_n135_));
  oai21  g89(.a(new_n135_), .b(new_n125_), .c(new_n61_), .O(new_n136_));
  nand2  g90(.a(new_n136_), .b(new_n48_), .O(new_n137_));
  nor2   g91(.a(x26), .b(new_n62_), .O(new_n138_));
  nor3   g92(.a(x27), .b(x25), .c(x17), .O(new_n139_));
  nand4  g93(.a(new_n139_), .b(new_n138_), .c(new_n108_), .d(new_n89_), .O(new_n140_));
  aoi21  g94(.a(new_n140_), .b(new_n137_), .c(new_n76_), .O(z17));
  buf    g95(.a(x27), .O(z08));
endmodule


