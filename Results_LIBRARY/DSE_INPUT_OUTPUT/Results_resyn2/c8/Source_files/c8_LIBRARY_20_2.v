// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  inv1   g002(.a(x18), .O(new_n49_));
  nor2   g003(.a(x26), .b(new_n49_), .O(new_n50_));
  aoi21  g004(.a(new_n47_), .b(new_n48_), .c(new_n50_), .O(new_n51_));
  oai21  g005(.a(new_n47_), .b(x08), .c(new_n51_), .O(z00));
  inv1   g006(.a(x09), .O(new_n53_));
  aoi21  g007(.a(new_n47_), .b(x20), .c(new_n50_), .O(new_n54_));
  oai21  g008(.a(new_n47_), .b(new_n53_), .c(new_n54_), .O(new_n55_));
  inv1   g009(.a(new_n55_), .O(z01));
  inv1   g010(.a(x10), .O(new_n57_));
  aoi21  g011(.a(new_n47_), .b(x21), .c(new_n50_), .O(new_n58_));
  oai21  g012(.a(new_n47_), .b(new_n57_), .c(new_n58_), .O(new_n59_));
  inv1   g013(.a(new_n59_), .O(z02));
  inv1   g014(.a(x22), .O(new_n61_));
  aoi21  g015(.a(new_n47_), .b(new_n61_), .c(new_n50_), .O(new_n62_));
  oai21  g016(.a(new_n47_), .b(x11), .c(new_n62_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(new_n47_), .b(x23), .c(new_n50_), .O(new_n65_));
  oai21  g019(.a(new_n47_), .b(new_n64_), .c(new_n65_), .O(new_n66_));
  inv1   g020(.a(new_n66_), .O(z04));
  inv1   g021(.a(x24), .O(new_n68_));
  aoi21  g022(.a(new_n47_), .b(new_n68_), .c(new_n50_), .O(new_n69_));
  oai21  g023(.a(new_n47_), .b(x13), .c(new_n69_), .O(z05));
  inv1   g024(.a(x25), .O(new_n71_));
  aoi21  g025(.a(new_n47_), .b(new_n71_), .c(new_n50_), .O(new_n72_));
  oai21  g026(.a(new_n47_), .b(x14), .c(new_n72_), .O(z06));
  inv1   g027(.a(x26), .O(new_n74_));
  oai21  g028(.a(new_n47_), .b(x18), .c(new_n74_), .O(new_n75_));
  oai21  g029(.a(new_n47_), .b(x15), .c(new_n75_), .O(z07));
  inv1   g030(.a(new_n50_), .O(new_n77_));
  nand2  g031(.a(new_n77_), .b(new_n47_), .O(z08));
  inv1   g032(.a(x17), .O(new_n79_));
  inv1   g033(.a(x16), .O(new_n80_));
  nor2   g034(.a(x19), .b(x17), .O(new_n81_));
  nor2   g035(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai21  g036(.a(new_n48_), .b(new_n79_), .c(new_n82_), .O(new_n83_));
  nand2  g037(.a(x18), .b(x00), .O(new_n84_));
  nand2  g038(.a(new_n49_), .b(x08), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(new_n84_), .c(new_n80_), .O(new_n86_));
  nand3  g040(.a(new_n86_), .b(new_n83_), .c(new_n77_), .O(new_n87_));
  inv1   g041(.a(new_n87_), .O(z09));
  inv1   g042(.a(x20), .O(new_n89_));
  nor2   g043(.a(new_n81_), .b(new_n89_), .O(new_n90_));
  nor3   g044(.a(x20), .b(x19), .c(x17), .O(new_n91_));
  oai21  g045(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  inv1   g046(.a(x01), .O(new_n93_));
  nand2  g047(.a(x18), .b(new_n93_), .O(new_n94_));
  aoi21  g048(.a(new_n49_), .b(new_n53_), .c(x16), .O(new_n95_));
  aoi21  g049(.a(new_n95_), .b(new_n94_), .c(new_n50_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n92_), .O(z10));
  nand2  g051(.a(new_n81_), .b(new_n89_), .O(new_n98_));
  xor2a  g052(.a(new_n98_), .b(x21), .O(new_n99_));
  inv1   g053(.a(x02), .O(new_n100_));
  nand2  g054(.a(x18), .b(new_n100_), .O(new_n101_));
  aoi21  g055(.a(new_n49_), .b(new_n57_), .c(x16), .O(new_n102_));
  aoi21  g056(.a(new_n102_), .b(new_n101_), .c(new_n50_), .O(new_n103_));
  oai21  g057(.a(new_n99_), .b(new_n80_), .c(new_n103_), .O(z11));
  inv1   g058(.a(x21), .O(new_n105_));
  nand4  g059(.a(new_n105_), .b(new_n89_), .c(new_n48_), .d(new_n79_), .O(new_n106_));
  nor2   g060(.a(x22), .b(x21), .O(new_n107_));
  aoi22  g061(.a(new_n107_), .b(new_n91_), .c(new_n106_), .d(x22), .O(new_n108_));
  inv1   g062(.a(x03), .O(new_n109_));
  nand2  g063(.a(x18), .b(new_n109_), .O(new_n110_));
  inv1   g064(.a(x11), .O(new_n111_));
  aoi21  g065(.a(new_n49_), .b(new_n111_), .c(x16), .O(new_n112_));
  aoi21  g066(.a(new_n112_), .b(new_n110_), .c(new_n50_), .O(new_n113_));
  oai21  g067(.a(new_n108_), .b(new_n80_), .c(new_n113_), .O(z12));
  nand3  g068(.a(new_n107_), .b(new_n81_), .c(new_n89_), .O(new_n115_));
  nor3   g069(.a(x23), .b(x22), .c(x21), .O(new_n116_));
  aoi22  g070(.a(new_n116_), .b(new_n91_), .c(new_n115_), .d(x23), .O(new_n117_));
  inv1   g071(.a(x04), .O(new_n118_));
  nand2  g072(.a(x18), .b(new_n118_), .O(new_n119_));
  aoi21  g073(.a(new_n49_), .b(new_n64_), .c(x16), .O(new_n120_));
  aoi21  g074(.a(new_n120_), .b(new_n119_), .c(new_n50_), .O(new_n121_));
  oai21  g075(.a(new_n117_), .b(new_n80_), .c(new_n121_), .O(z13));
  aoi21  g076(.a(new_n116_), .b(new_n91_), .c(new_n68_), .O(new_n123_));
  nor2   g077(.a(x24), .b(x23), .O(new_n124_));
  nand4  g078(.a(new_n124_), .b(new_n107_), .c(new_n81_), .d(new_n89_), .O(new_n125_));
  inv1   g079(.a(new_n125_), .O(new_n126_));
  oai21  g080(.a(new_n126_), .b(new_n123_), .c(x16), .O(new_n127_));
  inv1   g081(.a(x05), .O(new_n128_));
  nand2  g082(.a(x18), .b(new_n128_), .O(new_n129_));
  inv1   g083(.a(x13), .O(new_n130_));
  aoi21  g084(.a(new_n49_), .b(new_n130_), .c(x16), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(new_n129_), .c(new_n50_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n127_), .O(z14));
  inv1   g087(.a(new_n106_), .O(new_n134_));
  inv1   g088(.a(x23), .O(new_n135_));
  nand4  g089(.a(new_n71_), .b(new_n68_), .c(new_n135_), .d(new_n61_), .O(new_n136_));
  inv1   g090(.a(new_n136_), .O(new_n137_));
  aoi22  g091(.a(new_n137_), .b(new_n134_), .c(new_n125_), .d(x25), .O(new_n138_));
  inv1   g092(.a(x06), .O(new_n139_));
  nand2  g093(.a(x18), .b(new_n139_), .O(new_n140_));
  inv1   g094(.a(x14), .O(new_n141_));
  aoi21  g095(.a(new_n49_), .b(new_n141_), .c(x16), .O(new_n142_));
  aoi21  g096(.a(new_n142_), .b(new_n140_), .c(new_n50_), .O(new_n143_));
  oai21  g097(.a(new_n138_), .b(new_n80_), .c(new_n143_), .O(z15));
  nand2  g098(.a(new_n107_), .b(new_n135_), .O(new_n145_));
  nor2   g099(.a(x25), .b(x24), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n81_), .c(new_n89_), .d(x16), .O(new_n147_));
  oai21  g101(.a(new_n147_), .b(new_n145_), .c(new_n49_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n74_), .O(new_n149_));
  nand2  g103(.a(new_n137_), .b(new_n134_), .O(new_n150_));
  nor2   g104(.a(new_n74_), .b(new_n80_), .O(new_n151_));
  nor2   g105(.a(new_n49_), .b(x07), .O(new_n152_));
  oai21  g106(.a(x18), .b(x15), .c(new_n80_), .O(new_n153_));
  nor2   g107(.a(new_n153_), .b(new_n152_), .O(new_n154_));
  aoi21  g108(.a(new_n151_), .b(new_n150_), .c(new_n154_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n149_), .O(z16));
  nand4  g110(.a(new_n74_), .b(new_n105_), .c(new_n89_), .d(x19), .O(new_n157_));
  inv1   g111(.a(new_n157_), .O(new_n158_));
  nand4  g112(.a(new_n158_), .b(new_n137_), .c(new_n49_), .d(new_n79_), .O(new_n159_));
  nand3  g113(.a(new_n77_), .b(x27), .c(x17), .O(new_n160_));
  aoi21  g114(.a(new_n160_), .b(new_n159_), .c(new_n80_), .O(z17));
endmodule


