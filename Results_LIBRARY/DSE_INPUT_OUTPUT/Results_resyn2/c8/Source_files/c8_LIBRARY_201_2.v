// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(x27), .b(x25), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  oai22  g03(.a(new_n49_), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g04(.a(new_n49_), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  aoi22  g05(.a(new_n48_), .b(x21), .c(x27), .d(x10), .O(z02));
  oai22  g06(.a(new_n49_), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  aoi22  g07(.a(new_n48_), .b(x23), .c(x27), .d(x12), .O(z04));
  nor2   g08(.a(x25), .b(x24), .O(new_n55_));
  nor2   g09(.a(new_n55_), .b(x27), .O(new_n56_));
  aoi21  g10(.a(x27), .b(x13), .c(new_n56_), .O(z05));
  inv1   g11(.a(x25), .O(new_n58_));
  nor2   g12(.a(x27), .b(new_n58_), .O(new_n59_));
  aoi21  g13(.a(x27), .b(x14), .c(new_n59_), .O(z06));
  oai22  g14(.a(new_n49_), .b(x26), .c(new_n47_), .d(x15), .O(z07));
  inv1   g15(.a(new_n59_), .O(new_n62_));
  inv1   g16(.a(x16), .O(new_n63_));
  nand2  g17(.a(x18), .b(x00), .O(new_n64_));
  inv1   g18(.a(x18), .O(new_n65_));
  nand2  g19(.a(new_n65_), .b(x08), .O(new_n66_));
  nand3  g20(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  nand2  g21(.a(x19), .b(x17), .O(new_n68_));
  nor2   g22(.a(x19), .b(x17), .O(new_n69_));
  nor2   g23(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nand2  g24(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand2  g25(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand2  g26(.a(new_n72_), .b(new_n62_), .O(z09));
  inv1   g27(.a(x20), .O(new_n74_));
  xor2a  g28(.a(new_n69_), .b(new_n74_), .O(new_n75_));
  inv1   g29(.a(x01), .O(new_n76_));
  nand2  g30(.a(x18), .b(new_n76_), .O(new_n77_));
  inv1   g31(.a(x09), .O(new_n78_));
  aoi21  g32(.a(new_n65_), .b(new_n78_), .c(x16), .O(new_n79_));
  aoi21  g33(.a(new_n79_), .b(new_n77_), .c(new_n59_), .O(new_n80_));
  oai21  g34(.a(new_n75_), .b(new_n63_), .c(new_n80_), .O(z10));
  nand2  g35(.a(new_n69_), .b(new_n74_), .O(new_n82_));
  nor2   g36(.a(x21), .b(x20), .O(new_n83_));
  aoi22  g37(.a(new_n83_), .b(new_n69_), .c(new_n82_), .d(x21), .O(new_n84_));
  inv1   g38(.a(x02), .O(new_n85_));
  nand2  g39(.a(x18), .b(new_n85_), .O(new_n86_));
  inv1   g40(.a(x10), .O(new_n87_));
  aoi21  g41(.a(new_n65_), .b(new_n87_), .c(x16), .O(new_n88_));
  aoi21  g42(.a(new_n88_), .b(new_n86_), .c(new_n59_), .O(new_n89_));
  oai21  g43(.a(new_n84_), .b(new_n63_), .c(new_n89_), .O(z11));
  inv1   g44(.a(x22), .O(new_n91_));
  aoi21  g45(.a(new_n83_), .b(new_n69_), .c(new_n91_), .O(new_n92_));
  nand3  g46(.a(new_n83_), .b(new_n69_), .c(new_n91_), .O(new_n93_));
  inv1   g47(.a(new_n93_), .O(new_n94_));
  oai21  g48(.a(new_n94_), .b(new_n92_), .c(x16), .O(new_n95_));
  inv1   g49(.a(x11), .O(new_n96_));
  aoi21  g50(.a(new_n65_), .b(new_n96_), .c(x16), .O(new_n97_));
  oai21  g51(.a(new_n65_), .b(x03), .c(new_n97_), .O(new_n98_));
  aoi21  g52(.a(new_n98_), .b(new_n95_), .c(new_n59_), .O(z12));
  nand2  g53(.a(new_n93_), .b(x23), .O(new_n100_));
  nor2   g54(.a(x23), .b(x22), .O(new_n101_));
  nand3  g55(.a(new_n101_), .b(new_n83_), .c(new_n69_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(x16), .O(new_n104_));
  inv1   g58(.a(x12), .O(new_n105_));
  aoi21  g59(.a(new_n65_), .b(new_n105_), .c(x16), .O(new_n106_));
  oai21  g60(.a(new_n65_), .b(x04), .c(new_n106_), .O(new_n107_));
  aoi21  g61(.a(new_n107_), .b(new_n104_), .c(new_n59_), .O(z13));
  nand2  g62(.a(new_n102_), .b(x24), .O(new_n109_));
  inv1   g63(.a(x24), .O(new_n110_));
  nand4  g64(.a(new_n101_), .b(new_n83_), .c(new_n69_), .d(new_n110_), .O(new_n111_));
  nand2  g65(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g66(.a(new_n112_), .b(x16), .O(new_n113_));
  inv1   g67(.a(x13), .O(new_n114_));
  aoi21  g68(.a(new_n65_), .b(new_n114_), .c(x16), .O(new_n115_));
  oai21  g69(.a(new_n65_), .b(x05), .c(new_n115_), .O(new_n116_));
  aoi21  g70(.a(new_n116_), .b(new_n113_), .c(new_n59_), .O(z14));
  nand3  g71(.a(new_n111_), .b(x27), .c(x25), .O(new_n118_));
  nand4  g72(.a(new_n101_), .b(new_n83_), .c(new_n69_), .d(new_n55_), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(x16), .O(new_n121_));
  inv1   g75(.a(x06), .O(new_n122_));
  nand2  g76(.a(x18), .b(new_n122_), .O(new_n123_));
  inv1   g77(.a(x14), .O(new_n124_));
  nand2  g78(.a(new_n65_), .b(new_n124_), .O(new_n125_));
  nand4  g79(.a(new_n125_), .b(new_n123_), .c(new_n62_), .d(new_n63_), .O(new_n126_));
  nand2  g80(.a(new_n126_), .b(new_n121_), .O(z15));
  inv1   g81(.a(x26), .O(new_n128_));
  nor2   g82(.a(new_n111_), .b(new_n128_), .O(new_n129_));
  nand2  g83(.a(new_n119_), .b(new_n128_), .O(new_n130_));
  nand2  g84(.a(new_n130_), .b(x16), .O(new_n131_));
  nand2  g85(.a(x26), .b(x16), .O(new_n132_));
  aoi21  g86(.a(new_n132_), .b(x27), .c(new_n58_), .O(new_n133_));
  inv1   g87(.a(x07), .O(new_n134_));
  nand2  g88(.a(x18), .b(new_n134_), .O(new_n135_));
  inv1   g89(.a(x15), .O(new_n136_));
  aoi21  g90(.a(new_n65_), .b(new_n136_), .c(x16), .O(new_n137_));
  aoi21  g91(.a(new_n137_), .b(new_n135_), .c(new_n133_), .O(new_n138_));
  oai21  g92(.a(new_n131_), .b(new_n129_), .c(new_n138_), .O(z16));
  nand2  g93(.a(x27), .b(x17), .O(new_n140_));
  inv1   g94(.a(x17), .O(new_n141_));
  nand3  g95(.a(new_n128_), .b(x19), .c(new_n141_), .O(new_n142_));
  inv1   g96(.a(new_n142_), .O(new_n143_));
  nand4  g97(.a(new_n143_), .b(new_n101_), .c(new_n83_), .d(new_n55_), .O(new_n144_));
  aoi21  g98(.a(new_n144_), .b(new_n140_), .c(new_n63_), .O(z17));
  buf    g99(.a(x27), .O(z08));
endmodule


