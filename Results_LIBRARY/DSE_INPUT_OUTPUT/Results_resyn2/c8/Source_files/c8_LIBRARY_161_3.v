// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x23), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  oai22  g003(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  oai22  g004(.a(new_n49_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g005(.a(new_n49_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g006(.a(new_n48_), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  aoi21  g007(.a(x27), .b(x12), .c(x23), .O(z04));
  aoi22  g008(.a(new_n48_), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  aoi22  g009(.a(new_n48_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g010(.a(new_n49_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g011(.a(x23), .O(new_n58_));
  nor2   g012(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(new_n59_), .O(new_n60_));
  inv1   g014(.a(x08), .O(new_n61_));
  inv1   g015(.a(x16), .O(new_n62_));
  oai21  g016(.a(x18), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  aoi21  g017(.a(x18), .b(x00), .c(new_n63_), .O(new_n64_));
  nor2   g018(.a(x19), .b(x17), .O(new_n65_));
  inv1   g019(.a(new_n65_), .O(new_n66_));
  nand2  g020(.a(new_n66_), .b(x16), .O(new_n67_));
  aoi21  g021(.a(x19), .b(x17), .c(new_n67_), .O(new_n68_));
  oai21  g022(.a(new_n68_), .b(new_n64_), .c(new_n60_), .O(z09));
  inv1   g023(.a(x20), .O(new_n70_));
  nor2   g024(.a(new_n65_), .b(new_n70_), .O(new_n71_));
  nor2   g025(.a(new_n66_), .b(x20), .O(new_n72_));
  oai21  g026(.a(new_n72_), .b(new_n71_), .c(x16), .O(new_n73_));
  inv1   g027(.a(x18), .O(new_n74_));
  inv1   g028(.a(x09), .O(new_n75_));
  aoi21  g029(.a(new_n74_), .b(new_n75_), .c(x16), .O(new_n76_));
  oai21  g030(.a(new_n74_), .b(x01), .c(new_n76_), .O(new_n77_));
  nand3  g031(.a(new_n77_), .b(new_n73_), .c(new_n60_), .O(z10));
  inv1   g032(.a(x21), .O(new_n79_));
  nor2   g033(.a(x21), .b(x20), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n65_), .O(new_n81_));
  oai21  g035(.a(new_n72_), .b(new_n79_), .c(new_n81_), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(x16), .O(new_n83_));
  inv1   g037(.a(x10), .O(new_n84_));
  aoi21  g038(.a(new_n74_), .b(new_n84_), .c(x16), .O(new_n85_));
  oai21  g039(.a(new_n74_), .b(x02), .c(new_n85_), .O(new_n86_));
  aoi21  g040(.a(new_n86_), .b(new_n83_), .c(new_n59_), .O(z11));
  nand2  g041(.a(x18), .b(x03), .O(new_n88_));
  nand2  g042(.a(new_n74_), .b(x11), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n88_), .c(new_n62_), .O(new_n90_));
  inv1   g044(.a(x22), .O(new_n91_));
  nand3  g045(.a(new_n80_), .b(new_n65_), .c(new_n91_), .O(new_n92_));
  nand2  g046(.a(new_n81_), .b(x22), .O(new_n93_));
  nand3  g047(.a(new_n93_), .b(new_n92_), .c(x16), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n90_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n60_), .O(z12));
  nor2   g050(.a(new_n59_), .b(x16), .O(new_n97_));
  inv1   g051(.a(x12), .O(new_n98_));
  nand2  g052(.a(new_n74_), .b(new_n98_), .O(new_n99_));
  inv1   g053(.a(x04), .O(new_n100_));
  nand2  g054(.a(x18), .b(new_n100_), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n99_), .c(new_n97_), .O(new_n102_));
  oai21  g056(.a(x27), .b(new_n58_), .c(new_n92_), .O(new_n103_));
  inv1   g057(.a(new_n92_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(x23), .O(new_n105_));
  nand3  g059(.a(new_n105_), .b(new_n103_), .c(x16), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n102_), .O(z13));
  nand2  g061(.a(new_n92_), .b(x24), .O(new_n108_));
  nor2   g062(.a(x24), .b(x22), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n80_), .c(new_n65_), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n108_), .c(new_n58_), .O(new_n111_));
  nand2  g065(.a(new_n47_), .b(x24), .O(new_n112_));
  aoi21  g066(.a(new_n112_), .b(x23), .c(new_n62_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  inv1   g068(.a(x13), .O(new_n115_));
  nand2  g069(.a(new_n74_), .b(new_n115_), .O(new_n116_));
  inv1   g070(.a(x05), .O(new_n117_));
  nand2  g071(.a(x18), .b(new_n117_), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n116_), .c(new_n97_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n114_), .O(z14));
  nand2  g074(.a(new_n110_), .b(x25), .O(new_n121_));
  nor2   g075(.a(x25), .b(x21), .O(new_n122_));
  nand4  g076(.a(new_n122_), .b(new_n109_), .c(new_n65_), .d(new_n70_), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n121_), .c(new_n58_), .O(new_n124_));
  nand2  g078(.a(new_n47_), .b(x25), .O(new_n125_));
  aoi21  g079(.a(new_n125_), .b(x23), .c(new_n62_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  inv1   g081(.a(x14), .O(new_n128_));
  nand2  g082(.a(new_n74_), .b(new_n128_), .O(new_n129_));
  inv1   g083(.a(x06), .O(new_n130_));
  nand2  g084(.a(x18), .b(new_n130_), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n129_), .c(new_n97_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n127_), .O(z15));
  nand2  g087(.a(new_n123_), .b(x26), .O(new_n134_));
  nor2   g088(.a(x26), .b(x25), .O(new_n135_));
  nand4  g089(.a(new_n135_), .b(new_n109_), .c(new_n80_), .d(new_n65_), .O(new_n136_));
  aoi21  g090(.a(new_n136_), .b(new_n134_), .c(x23), .O(new_n137_));
  nand3  g091(.a(new_n47_), .b(x26), .c(x23), .O(new_n138_));
  inv1   g092(.a(new_n138_), .O(new_n139_));
  oai21  g093(.a(new_n139_), .b(new_n137_), .c(x16), .O(new_n140_));
  inv1   g094(.a(x15), .O(new_n141_));
  nand2  g095(.a(new_n74_), .b(new_n141_), .O(new_n142_));
  inv1   g096(.a(x07), .O(new_n143_));
  nand2  g097(.a(x18), .b(new_n143_), .O(new_n144_));
  nand3  g098(.a(new_n144_), .b(new_n142_), .c(new_n97_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n140_), .O(z16));
  nand2  g100(.a(x27), .b(x17), .O(new_n147_));
  inv1   g101(.a(x19), .O(new_n148_));
  nor2   g102(.a(new_n148_), .b(x17), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n135_), .c(new_n109_), .d(new_n80_), .O(new_n150_));
  nand2  g104(.a(new_n58_), .b(x16), .O(new_n151_));
  aoi21  g105(.a(new_n150_), .b(new_n147_), .c(new_n151_), .O(z17));
  buf    g106(.a(x27), .O(z08));
endmodule


