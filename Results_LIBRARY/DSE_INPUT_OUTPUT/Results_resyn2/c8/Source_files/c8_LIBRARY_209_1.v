// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_;
  inv1   g00(.a(x27), .O(new_n47_));
  nand2  g01(.a(x27), .b(x14), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  aoi22  g03(.a(new_n49_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  oai22  g04(.a(new_n48_), .b(x09), .c(x27), .d(x20), .O(z01));
  aoi22  g05(.a(new_n49_), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  oai22  g06(.a(new_n48_), .b(x11), .c(x27), .d(x22), .O(z03));
  aoi22  g07(.a(new_n49_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  oai22  g08(.a(new_n48_), .b(x13), .c(x27), .d(x24), .O(z05));
  nor2   g09(.a(x27), .b(x25), .O(z06));
  inv1   g10(.a(x26), .O(new_n57_));
  nor2   g11(.a(new_n47_), .b(x14), .O(new_n58_));
  aoi21  g12(.a(new_n47_), .b(new_n57_), .c(new_n58_), .O(new_n59_));
  oai21  g13(.a(new_n47_), .b(x15), .c(new_n59_), .O(z07));
  inv1   g14(.a(x18), .O(new_n61_));
  inv1   g15(.a(x08), .O(new_n62_));
  aoi21  g16(.a(new_n61_), .b(new_n62_), .c(x16), .O(new_n63_));
  oai21  g17(.a(new_n61_), .b(x00), .c(new_n63_), .O(new_n64_));
  inv1   g18(.a(x19), .O(new_n65_));
  nand2  g19(.a(new_n65_), .b(x17), .O(new_n66_));
  inv1   g20(.a(x16), .O(new_n67_));
  nor2   g21(.a(new_n65_), .b(x17), .O(new_n68_));
  nor2   g22(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g23(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  aoi21  g24(.a(new_n70_), .b(new_n64_), .c(new_n58_), .O(z09));
  inv1   g25(.a(x20), .O(new_n72_));
  nor2   g26(.a(x19), .b(x17), .O(new_n73_));
  xor2a  g27(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g28(.a(x01), .O(new_n75_));
  nand2  g29(.a(x18), .b(new_n75_), .O(new_n76_));
  inv1   g30(.a(x09), .O(new_n77_));
  aoi21  g31(.a(new_n61_), .b(new_n77_), .c(x16), .O(new_n78_));
  aoi21  g32(.a(new_n78_), .b(new_n76_), .c(new_n58_), .O(new_n79_));
  oai21  g33(.a(new_n74_), .b(new_n67_), .c(new_n79_), .O(z10));
  nand2  g34(.a(new_n73_), .b(new_n72_), .O(new_n81_));
  nor2   g35(.a(x21), .b(x20), .O(new_n82_));
  nand2  g36(.a(new_n82_), .b(new_n73_), .O(new_n83_));
  inv1   g37(.a(new_n83_), .O(new_n84_));
  aoi21  g38(.a(new_n81_), .b(x21), .c(new_n84_), .O(new_n85_));
  inv1   g39(.a(x02), .O(new_n86_));
  nand2  g40(.a(x18), .b(new_n86_), .O(new_n87_));
  inv1   g41(.a(x10), .O(new_n88_));
  aoi21  g42(.a(new_n61_), .b(new_n88_), .c(x16), .O(new_n89_));
  aoi21  g43(.a(new_n89_), .b(new_n87_), .c(new_n58_), .O(new_n90_));
  oai21  g44(.a(new_n85_), .b(new_n67_), .c(new_n90_), .O(z11));
  xor2a  g45(.a(new_n83_), .b(x22), .O(new_n92_));
  inv1   g46(.a(x03), .O(new_n93_));
  nand2  g47(.a(x18), .b(new_n93_), .O(new_n94_));
  inv1   g48(.a(x11), .O(new_n95_));
  aoi21  g49(.a(new_n61_), .b(new_n95_), .c(x16), .O(new_n96_));
  aoi21  g50(.a(new_n96_), .b(new_n94_), .c(new_n58_), .O(new_n97_));
  oai21  g51(.a(new_n92_), .b(new_n67_), .c(new_n97_), .O(z12));
  oai21  g52(.a(new_n83_), .b(x22), .c(x23), .O(new_n99_));
  nor2   g53(.a(x23), .b(x22), .O(new_n100_));
  nand3  g54(.a(new_n100_), .b(new_n82_), .c(new_n73_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(x16), .O(new_n103_));
  inv1   g57(.a(x04), .O(new_n104_));
  nand2  g58(.a(x18), .b(new_n104_), .O(new_n105_));
  inv1   g59(.a(x12), .O(new_n106_));
  aoi21  g60(.a(new_n61_), .b(new_n106_), .c(x16), .O(new_n107_));
  aoi21  g61(.a(new_n107_), .b(new_n105_), .c(new_n58_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(new_n103_), .O(z13));
  nand2  g63(.a(new_n101_), .b(x24), .O(new_n110_));
  inv1   g64(.a(x22), .O(new_n111_));
  nor2   g65(.a(x24), .b(x23), .O(new_n112_));
  nand4  g66(.a(new_n112_), .b(new_n82_), .c(new_n73_), .d(new_n111_), .O(new_n113_));
  nand2  g67(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  nand2  g68(.a(new_n114_), .b(x16), .O(new_n115_));
  inv1   g69(.a(x13), .O(new_n116_));
  aoi21  g70(.a(new_n61_), .b(new_n116_), .c(x16), .O(new_n117_));
  oai21  g71(.a(new_n61_), .b(x05), .c(new_n117_), .O(new_n118_));
  aoi21  g72(.a(new_n118_), .b(new_n115_), .c(new_n58_), .O(z14));
  nand2  g73(.a(new_n113_), .b(x25), .O(new_n120_));
  nor2   g74(.a(x25), .b(x24), .O(new_n121_));
  nand4  g75(.a(new_n121_), .b(new_n100_), .c(new_n82_), .d(new_n73_), .O(new_n122_));
  and2   g76(.a(new_n122_), .b(x16), .O(new_n123_));
  inv1   g77(.a(new_n58_), .O(new_n124_));
  nand2  g78(.a(x18), .b(x06), .O(new_n125_));
  nand2  g79(.a(new_n61_), .b(x14), .O(new_n126_));
  nand3  g80(.a(new_n126_), .b(new_n125_), .c(new_n67_), .O(new_n127_));
  nand2  g81(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  aoi21  g82(.a(new_n123_), .b(new_n120_), .c(new_n128_), .O(z15));
  nand2  g83(.a(new_n121_), .b(new_n100_), .O(new_n130_));
  inv1   g84(.a(new_n130_), .O(new_n131_));
  nand3  g85(.a(new_n131_), .b(new_n84_), .c(x26), .O(new_n132_));
  aoi21  g86(.a(new_n122_), .b(new_n57_), .c(new_n67_), .O(new_n133_));
  nand2  g87(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  inv1   g88(.a(x15), .O(new_n135_));
  aoi21  g89(.a(new_n61_), .b(new_n135_), .c(x16), .O(new_n136_));
  oai21  g90(.a(new_n61_), .b(x07), .c(new_n136_), .O(new_n137_));
  aoi21  g91(.a(new_n137_), .b(new_n134_), .c(new_n58_), .O(z16));
  nand2  g92(.a(x27), .b(x17), .O(new_n139_));
  nand4  g93(.a(new_n131_), .b(new_n82_), .c(new_n68_), .d(new_n57_), .O(new_n140_));
  nand2  g94(.a(new_n124_), .b(x16), .O(new_n141_));
  aoi21  g95(.a(new_n140_), .b(new_n139_), .c(new_n141_), .O(z17));
  buf    g96(.a(x27), .O(z08));
endmodule


