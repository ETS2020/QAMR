// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_;
  nor2   g00(.a(x27), .b(x13), .O(new_n47_));
  aoi22  g01(.a(new_n47_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g02(.a(x27), .O(new_n49_));
  inv1   g03(.a(new_n47_), .O(z08));
  oai22  g04(.a(z08), .b(x20), .c(new_n49_), .d(x09), .O(z01));
  oai22  g05(.a(z08), .b(x21), .c(new_n49_), .d(x10), .O(z02));
  aoi22  g06(.a(new_n47_), .b(x22), .c(x27), .d(x11), .O(z03));
  aoi22  g07(.a(new_n47_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi21  g08(.a(new_n49_), .b(x24), .c(x13), .O(z05));
  aoi22  g09(.a(new_n47_), .b(x25), .c(x27), .d(x14), .O(z06));
  oai22  g10(.a(z08), .b(x26), .c(new_n49_), .d(x15), .O(z07));
  inv1   g11(.a(x18), .O(new_n58_));
  inv1   g12(.a(x08), .O(new_n59_));
  aoi21  g13(.a(new_n58_), .b(new_n59_), .c(x16), .O(new_n60_));
  oai21  g14(.a(new_n58_), .b(x00), .c(new_n60_), .O(new_n61_));
  and2   g15(.a(x19), .b(x17), .O(new_n62_));
  nor2   g16(.a(x19), .b(x17), .O(new_n63_));
  oai21  g17(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  nand2  g18(.a(new_n49_), .b(x13), .O(new_n65_));
  nand3  g19(.a(new_n65_), .b(new_n64_), .c(new_n61_), .O(z09));
  inv1   g20(.a(x16), .O(new_n67_));
  inv1   g21(.a(x20), .O(new_n68_));
  xor2a  g22(.a(new_n63_), .b(new_n68_), .O(new_n69_));
  inv1   g23(.a(new_n65_), .O(new_n70_));
  inv1   g24(.a(x01), .O(new_n71_));
  nand2  g25(.a(x18), .b(new_n71_), .O(new_n72_));
  inv1   g26(.a(x09), .O(new_n73_));
  aoi21  g27(.a(new_n58_), .b(new_n73_), .c(x16), .O(new_n74_));
  aoi21  g28(.a(new_n74_), .b(new_n72_), .c(new_n70_), .O(new_n75_));
  oai21  g29(.a(new_n69_), .b(new_n67_), .c(new_n75_), .O(z10));
  inv1   g30(.a(x21), .O(new_n77_));
  aoi21  g31(.a(new_n63_), .b(new_n68_), .c(new_n77_), .O(new_n78_));
  nor2   g32(.a(x21), .b(x20), .O(new_n79_));
  nand2  g33(.a(new_n79_), .b(new_n63_), .O(new_n80_));
  inv1   g34(.a(new_n80_), .O(new_n81_));
  oai21  g35(.a(new_n81_), .b(new_n78_), .c(x16), .O(new_n82_));
  inv1   g36(.a(x10), .O(new_n83_));
  aoi21  g37(.a(new_n58_), .b(new_n83_), .c(x16), .O(new_n84_));
  oai21  g38(.a(new_n58_), .b(x02), .c(new_n84_), .O(new_n85_));
  aoi21  g39(.a(new_n85_), .b(new_n82_), .c(new_n70_), .O(z11));
  inv1   g40(.a(x22), .O(new_n87_));
  aoi21  g41(.a(new_n79_), .b(new_n63_), .c(new_n87_), .O(new_n88_));
  nor2   g42(.a(x22), .b(x21), .O(new_n89_));
  nand3  g43(.a(new_n89_), .b(new_n63_), .c(new_n68_), .O(new_n90_));
  inv1   g44(.a(new_n90_), .O(new_n91_));
  oai21  g45(.a(new_n91_), .b(new_n88_), .c(x16), .O(new_n92_));
  inv1   g46(.a(x11), .O(new_n93_));
  aoi21  g47(.a(new_n58_), .b(new_n93_), .c(x16), .O(new_n94_));
  oai21  g48(.a(new_n58_), .b(x03), .c(new_n94_), .O(new_n95_));
  aoi21  g49(.a(new_n95_), .b(new_n92_), .c(new_n70_), .O(z12));
  inv1   g50(.a(x23), .O(new_n97_));
  xor2a  g51(.a(new_n90_), .b(new_n97_), .O(new_n98_));
  nand2  g52(.a(new_n98_), .b(x16), .O(new_n99_));
  inv1   g53(.a(x12), .O(new_n100_));
  aoi21  g54(.a(new_n58_), .b(new_n100_), .c(x16), .O(new_n101_));
  oai21  g55(.a(new_n58_), .b(x04), .c(new_n101_), .O(new_n102_));
  aoi21  g56(.a(new_n102_), .b(new_n99_), .c(new_n70_), .O(z13));
  oai21  g57(.a(new_n90_), .b(x23), .c(x24), .O(new_n104_));
  nor2   g58(.a(x24), .b(x23), .O(new_n105_));
  nand4  g59(.a(new_n105_), .b(new_n89_), .c(new_n63_), .d(new_n68_), .O(new_n106_));
  and2   g60(.a(new_n106_), .b(x16), .O(new_n107_));
  nand2  g61(.a(x18), .b(x05), .O(new_n108_));
  nand2  g62(.a(new_n58_), .b(x13), .O(new_n109_));
  nand3  g63(.a(new_n109_), .b(new_n108_), .c(new_n67_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(new_n65_), .O(new_n111_));
  aoi21  g65(.a(new_n107_), .b(new_n104_), .c(new_n111_), .O(z14));
  nand2  g66(.a(new_n106_), .b(x25), .O(new_n113_));
  nor2   g67(.a(x25), .b(x22), .O(new_n114_));
  nand4  g68(.a(new_n114_), .b(new_n105_), .c(new_n79_), .d(new_n63_), .O(new_n115_));
  and2   g69(.a(new_n115_), .b(x16), .O(new_n116_));
  nand2  g70(.a(x18), .b(x06), .O(new_n117_));
  nand2  g71(.a(new_n58_), .b(x14), .O(new_n118_));
  nand3  g72(.a(new_n118_), .b(new_n117_), .c(new_n67_), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(new_n65_), .O(new_n120_));
  aoi21  g74(.a(new_n116_), .b(new_n113_), .c(new_n120_), .O(z15));
  nand2  g75(.a(new_n114_), .b(new_n105_), .O(new_n122_));
  inv1   g76(.a(new_n122_), .O(new_n123_));
  nand3  g77(.a(new_n123_), .b(new_n81_), .c(x26), .O(new_n124_));
  inv1   g78(.a(x26), .O(new_n125_));
  aoi21  g79(.a(new_n115_), .b(new_n125_), .c(new_n67_), .O(new_n126_));
  nand2  g80(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  inv1   g81(.a(x15), .O(new_n128_));
  aoi21  g82(.a(new_n58_), .b(new_n128_), .c(x16), .O(new_n129_));
  oai21  g83(.a(new_n58_), .b(x07), .c(new_n129_), .O(new_n130_));
  aoi21  g84(.a(new_n130_), .b(new_n127_), .c(new_n70_), .O(z16));
  inv1   g85(.a(x17), .O(new_n132_));
  nand4  g86(.a(new_n79_), .b(new_n125_), .c(x19), .d(new_n132_), .O(new_n133_));
  oai22  g87(.a(new_n133_), .b(new_n122_), .c(new_n49_), .d(new_n132_), .O(new_n134_));
  nand2  g88(.a(new_n134_), .b(x16), .O(new_n135_));
  nand2  g89(.a(new_n135_), .b(new_n65_), .O(z17));
endmodule


