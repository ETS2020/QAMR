// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x15), .O(new_n48_));
  nor2   g02(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g03(.a(new_n49_), .O(z08));
  oai22  g04(.a(z08), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g05(.a(z08), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  oai22  g06(.a(z08), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  oai22  g07(.a(z08), .b(x22), .c(new_n47_), .d(x11), .O(z03));
  aoi22  g08(.a(new_n49_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi22  g09(.a(new_n49_), .b(x24), .c(x27), .d(x13), .O(z05));
  oai22  g10(.a(z08), .b(x25), .c(new_n47_), .d(x14), .O(z06));
  oai21  g11(.a(x27), .b(x26), .c(x15), .O(z07));
  nor2   g12(.a(x27), .b(x15), .O(new_n59_));
  inv1   g13(.a(x17), .O(new_n60_));
  inv1   g14(.a(x19), .O(new_n61_));
  nor2   g15(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nor2   g16(.a(x19), .b(x17), .O(new_n63_));
  oai21  g17(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  inv1   g18(.a(x18), .O(new_n65_));
  inv1   g19(.a(x08), .O(new_n66_));
  aoi21  g20(.a(new_n65_), .b(new_n66_), .c(x16), .O(new_n67_));
  oai21  g21(.a(new_n65_), .b(x00), .c(new_n67_), .O(new_n68_));
  aoi21  g22(.a(new_n68_), .b(new_n64_), .c(new_n59_), .O(z09));
  xor2a  g23(.a(new_n63_), .b(x20), .O(new_n70_));
  nand2  g24(.a(new_n70_), .b(x16), .O(new_n71_));
  inv1   g25(.a(x09), .O(new_n72_));
  aoi21  g26(.a(new_n65_), .b(new_n72_), .c(x16), .O(new_n73_));
  oai21  g27(.a(new_n65_), .b(x01), .c(new_n73_), .O(new_n74_));
  aoi21  g28(.a(new_n74_), .b(new_n71_), .c(new_n59_), .O(z10));
  inv1   g29(.a(x20), .O(new_n76_));
  inv1   g30(.a(x21), .O(new_n77_));
  aoi21  g31(.a(new_n63_), .b(new_n76_), .c(new_n77_), .O(new_n78_));
  nor2   g32(.a(x21), .b(x20), .O(new_n79_));
  nand2  g33(.a(new_n79_), .b(new_n63_), .O(new_n80_));
  inv1   g34(.a(new_n80_), .O(new_n81_));
  oai21  g35(.a(new_n81_), .b(new_n78_), .c(x16), .O(new_n82_));
  inv1   g36(.a(x10), .O(new_n83_));
  aoi21  g37(.a(new_n65_), .b(new_n83_), .c(x16), .O(new_n84_));
  oai21  g38(.a(new_n65_), .b(x02), .c(new_n84_), .O(new_n85_));
  aoi21  g39(.a(new_n85_), .b(new_n82_), .c(new_n59_), .O(z11));
  inv1   g40(.a(new_n59_), .O(new_n87_));
  inv1   g41(.a(x16), .O(new_n88_));
  nand2  g42(.a(x18), .b(x03), .O(new_n89_));
  nand2  g43(.a(new_n65_), .b(x11), .O(new_n90_));
  nand3  g44(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  inv1   g45(.a(x22), .O(new_n92_));
  nand3  g46(.a(new_n79_), .b(new_n63_), .c(new_n92_), .O(new_n93_));
  nand2  g47(.a(new_n80_), .b(x22), .O(new_n94_));
  nand3  g48(.a(new_n94_), .b(new_n93_), .c(x16), .O(new_n95_));
  nand2  g49(.a(new_n95_), .b(new_n91_), .O(new_n96_));
  nand2  g50(.a(new_n96_), .b(new_n87_), .O(z12));
  nand2  g51(.a(new_n93_), .b(x23), .O(new_n98_));
  nor2   g52(.a(x23), .b(x22), .O(new_n99_));
  nand3  g53(.a(new_n99_), .b(new_n79_), .c(new_n63_), .O(new_n100_));
  nand2  g54(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(x16), .O(new_n102_));
  inv1   g56(.a(x04), .O(new_n103_));
  nand2  g57(.a(x18), .b(new_n103_), .O(new_n104_));
  inv1   g58(.a(x12), .O(new_n105_));
  aoi21  g59(.a(new_n65_), .b(new_n105_), .c(x16), .O(new_n106_));
  aoi21  g60(.a(new_n106_), .b(new_n104_), .c(new_n59_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(new_n102_), .O(z13));
  inv1   g62(.a(x24), .O(new_n109_));
  xor2a  g63(.a(new_n100_), .b(new_n109_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(x16), .O(new_n111_));
  inv1   g65(.a(x13), .O(new_n112_));
  aoi21  g66(.a(new_n65_), .b(new_n112_), .c(x16), .O(new_n113_));
  oai21  g67(.a(new_n65_), .b(x05), .c(new_n113_), .O(new_n114_));
  aoi21  g68(.a(new_n114_), .b(new_n111_), .c(new_n59_), .O(z14));
  nand4  g69(.a(new_n99_), .b(new_n79_), .c(new_n63_), .d(new_n109_), .O(new_n116_));
  nor2   g70(.a(x25), .b(x24), .O(new_n117_));
  nand4  g71(.a(new_n117_), .b(new_n99_), .c(new_n79_), .d(new_n63_), .O(new_n118_));
  inv1   g72(.a(new_n118_), .O(new_n119_));
  aoi21  g73(.a(new_n116_), .b(x25), .c(new_n119_), .O(new_n120_));
  inv1   g74(.a(x06), .O(new_n121_));
  nand2  g75(.a(x18), .b(new_n121_), .O(new_n122_));
  inv1   g76(.a(x14), .O(new_n123_));
  aoi21  g77(.a(new_n65_), .b(new_n123_), .c(x16), .O(new_n124_));
  aoi21  g78(.a(new_n124_), .b(new_n122_), .c(new_n59_), .O(new_n125_));
  oai21  g79(.a(new_n120_), .b(new_n88_), .c(new_n125_), .O(z15));
  inv1   g80(.a(x26), .O(new_n127_));
  nand2  g81(.a(new_n119_), .b(new_n127_), .O(new_n128_));
  aoi21  g82(.a(new_n118_), .b(x26), .c(new_n88_), .O(new_n129_));
  nand2  g83(.a(x18), .b(x07), .O(new_n130_));
  nand2  g84(.a(new_n65_), .b(x15), .O(new_n131_));
  nand3  g85(.a(new_n131_), .b(new_n130_), .c(new_n88_), .O(new_n132_));
  nand2  g86(.a(new_n132_), .b(new_n87_), .O(new_n133_));
  aoi21  g87(.a(new_n129_), .b(new_n128_), .c(new_n133_), .O(z16));
  nor2   g88(.a(x20), .b(new_n61_), .O(new_n135_));
  nor2   g89(.a(x26), .b(x21), .O(new_n136_));
  nand4  g90(.a(new_n136_), .b(new_n135_), .c(new_n117_), .d(new_n99_), .O(new_n137_));
  oai21  g91(.a(x17), .b(new_n48_), .c(new_n47_), .O(new_n138_));
  nand2  g92(.a(new_n138_), .b(x16), .O(new_n139_));
  aoi21  g93(.a(new_n137_), .b(new_n60_), .c(new_n139_), .O(z17));
endmodule


