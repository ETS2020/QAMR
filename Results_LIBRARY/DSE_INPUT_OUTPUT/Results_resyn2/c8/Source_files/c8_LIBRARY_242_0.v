// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n48_, new_n49_, new_n50_, new_n51_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_;
  oai21  g00(.a(x27), .b(x19), .c(x08), .O(z00));
  inv1   g01(.a(x27), .O(new_n48_));
  inv1   g02(.a(x08), .O(new_n49_));
  nor2   g03(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g04(.a(new_n50_), .O(new_n51_));
  oai22  g05(.a(new_n51_), .b(x20), .c(new_n48_), .d(x09), .O(z01));
  oai22  g06(.a(new_n51_), .b(x21), .c(new_n48_), .d(x10), .O(z02));
  aoi22  g07(.a(new_n50_), .b(x22), .c(x27), .d(x11), .O(z03));
  aoi22  g08(.a(new_n50_), .b(x23), .c(x27), .d(x12), .O(z04));
  oai22  g09(.a(new_n51_), .b(x24), .c(new_n48_), .d(x13), .O(z05));
  aoi22  g10(.a(new_n50_), .b(x25), .c(x27), .d(x14), .O(z06));
  aoi22  g11(.a(new_n50_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g12(.a(x16), .O(new_n59_));
  nand2  g13(.a(x18), .b(x00), .O(new_n60_));
  inv1   g14(.a(x18), .O(new_n61_));
  nand2  g15(.a(new_n61_), .b(x08), .O(new_n62_));
  nand3  g16(.a(new_n62_), .b(new_n60_), .c(new_n59_), .O(new_n63_));
  nor2   g17(.a(x27), .b(x08), .O(new_n64_));
  inv1   g18(.a(new_n64_), .O(new_n65_));
  xor2a  g19(.a(x19), .b(x17), .O(new_n66_));
  nand2  g20(.a(new_n66_), .b(x16), .O(new_n67_));
  nand3  g21(.a(new_n67_), .b(new_n65_), .c(new_n63_), .O(new_n68_));
  inv1   g22(.a(new_n68_), .O(z09));
  inv1   g23(.a(x20), .O(new_n70_));
  nor2   g24(.a(x19), .b(x17), .O(new_n71_));
  xor2a  g25(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g26(.a(x01), .O(new_n73_));
  nand2  g27(.a(x18), .b(new_n73_), .O(new_n74_));
  inv1   g28(.a(x09), .O(new_n75_));
  aoi21  g29(.a(new_n61_), .b(new_n75_), .c(x16), .O(new_n76_));
  aoi21  g30(.a(new_n76_), .b(new_n74_), .c(new_n64_), .O(new_n77_));
  oai21  g31(.a(new_n72_), .b(new_n59_), .c(new_n77_), .O(z10));
  inv1   g32(.a(x21), .O(new_n79_));
  aoi21  g33(.a(new_n71_), .b(new_n70_), .c(new_n79_), .O(new_n80_));
  nor2   g34(.a(x21), .b(x20), .O(new_n81_));
  nand2  g35(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  oai21  g37(.a(new_n83_), .b(new_n80_), .c(x16), .O(new_n84_));
  inv1   g38(.a(x10), .O(new_n85_));
  aoi21  g39(.a(new_n61_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g40(.a(new_n61_), .b(x02), .c(new_n86_), .O(new_n87_));
  aoi21  g41(.a(new_n87_), .b(new_n84_), .c(new_n64_), .O(z11));
  inv1   g42(.a(x22), .O(new_n89_));
  aoi21  g43(.a(new_n81_), .b(new_n71_), .c(new_n89_), .O(new_n90_));
  nand3  g44(.a(new_n81_), .b(new_n71_), .c(new_n89_), .O(new_n91_));
  inv1   g45(.a(new_n91_), .O(new_n92_));
  oai21  g46(.a(new_n92_), .b(new_n90_), .c(x16), .O(new_n93_));
  inv1   g47(.a(x11), .O(new_n94_));
  aoi21  g48(.a(new_n61_), .b(new_n94_), .c(x16), .O(new_n95_));
  oai21  g49(.a(new_n61_), .b(x03), .c(new_n95_), .O(new_n96_));
  aoi21  g50(.a(new_n96_), .b(new_n93_), .c(new_n64_), .O(z12));
  inv1   g51(.a(x23), .O(new_n98_));
  xor2a  g52(.a(new_n91_), .b(new_n98_), .O(new_n99_));
  nand2  g53(.a(new_n99_), .b(x16), .O(new_n100_));
  inv1   g54(.a(x12), .O(new_n101_));
  aoi21  g55(.a(new_n61_), .b(new_n101_), .c(x16), .O(new_n102_));
  oai21  g56(.a(new_n61_), .b(x04), .c(new_n102_), .O(new_n103_));
  aoi21  g57(.a(new_n103_), .b(new_n100_), .c(new_n64_), .O(z13));
  nand4  g58(.a(new_n81_), .b(new_n71_), .c(new_n98_), .d(new_n89_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(x24), .O(new_n106_));
  nor2   g60(.a(x24), .b(x23), .O(new_n107_));
  nor2   g61(.a(x22), .b(x21), .O(new_n108_));
  nand4  g62(.a(new_n108_), .b(new_n107_), .c(new_n71_), .d(new_n70_), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(x16), .O(new_n111_));
  inv1   g65(.a(x05), .O(new_n112_));
  nand2  g66(.a(x18), .b(new_n112_), .O(new_n113_));
  inv1   g67(.a(x13), .O(new_n114_));
  aoi21  g68(.a(new_n61_), .b(new_n114_), .c(x16), .O(new_n115_));
  aoi21  g69(.a(new_n115_), .b(new_n113_), .c(new_n64_), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n111_), .O(z14));
  nand2  g71(.a(new_n109_), .b(x25), .O(new_n118_));
  nor2   g72(.a(x25), .b(x22), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(new_n107_), .O(new_n120_));
  nor2   g74(.a(new_n120_), .b(new_n82_), .O(new_n121_));
  nor2   g75(.a(new_n121_), .b(new_n59_), .O(new_n122_));
  nand2  g76(.a(x18), .b(x06), .O(new_n123_));
  nand2  g77(.a(new_n61_), .b(x14), .O(new_n124_));
  nand3  g78(.a(new_n124_), .b(new_n123_), .c(new_n59_), .O(new_n125_));
  nand2  g79(.a(new_n125_), .b(new_n65_), .O(new_n126_));
  aoi21  g80(.a(new_n122_), .b(new_n118_), .c(new_n126_), .O(z15));
  inv1   g81(.a(x26), .O(new_n128_));
  nor3   g82(.a(new_n120_), .b(new_n82_), .c(new_n128_), .O(new_n129_));
  oai21  g83(.a(new_n121_), .b(x26), .c(x16), .O(new_n130_));
  inv1   g84(.a(x07), .O(new_n131_));
  nand2  g85(.a(x18), .b(new_n131_), .O(new_n132_));
  inv1   g86(.a(x15), .O(new_n133_));
  aoi21  g87(.a(new_n61_), .b(new_n133_), .c(x16), .O(new_n134_));
  aoi21  g88(.a(new_n134_), .b(new_n132_), .c(new_n64_), .O(new_n135_));
  oai21  g89(.a(new_n130_), .b(new_n129_), .c(new_n135_), .O(z16));
  inv1   g90(.a(x17), .O(new_n137_));
  nand4  g91(.a(new_n81_), .b(new_n128_), .c(x19), .d(new_n137_), .O(new_n138_));
  oai22  g92(.a(new_n138_), .b(new_n120_), .c(new_n48_), .d(new_n137_), .O(new_n139_));
  nand2  g93(.a(new_n139_), .b(x16), .O(new_n140_));
  nand2  g94(.a(new_n140_), .b(new_n65_), .O(z17));
  buf    g95(.a(x27), .O(z08));
endmodule


