// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x22), .O(new_n48_));
  aoi22  g002(.a(new_n48_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g003(.a(new_n48_), .O(new_n50_));
  oai22  g004(.a(new_n50_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g005(.a(new_n50_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g006(.a(x22), .O(new_n53_));
  nor2   g007(.a(x27), .b(new_n53_), .O(new_n54_));
  inv1   g008(.a(x11), .O(new_n55_));
  nor2   g009(.a(x22), .b(new_n55_), .O(new_n56_));
  aoi21  g010(.a(new_n56_), .b(x27), .c(new_n54_), .O(z03));
  aoi22  g011(.a(new_n48_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  oai22  g012(.a(new_n50_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g013(.a(new_n48_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g014(.a(new_n50_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g015(.a(new_n47_), .b(new_n53_), .O(new_n62_));
  nor2   g016(.a(x19), .b(x17), .O(new_n63_));
  and2   g017(.a(x19), .b(x17), .O(new_n64_));
  oai21  g018(.a(new_n64_), .b(new_n63_), .c(x16), .O(new_n65_));
  inv1   g019(.a(x18), .O(new_n66_));
  inv1   g020(.a(x08), .O(new_n67_));
  aoi21  g021(.a(new_n66_), .b(new_n67_), .c(x16), .O(new_n68_));
  oai21  g022(.a(new_n66_), .b(x00), .c(new_n68_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n65_), .c(new_n62_), .O(z09));
  inv1   g024(.a(x20), .O(new_n71_));
  nor2   g025(.a(new_n63_), .b(new_n71_), .O(new_n72_));
  nor3   g026(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  oai21  g027(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  inv1   g028(.a(x09), .O(new_n75_));
  aoi21  g029(.a(new_n66_), .b(new_n75_), .c(x16), .O(new_n76_));
  oai21  g030(.a(new_n66_), .b(x01), .c(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n74_), .c(new_n62_), .O(z10));
  inv1   g032(.a(x21), .O(new_n79_));
  nor2   g033(.a(new_n73_), .b(new_n79_), .O(new_n80_));
  nand2  g034(.a(new_n73_), .b(new_n79_), .O(new_n81_));
  inv1   g035(.a(new_n81_), .O(new_n82_));
  oai21  g036(.a(new_n82_), .b(new_n80_), .c(x16), .O(new_n83_));
  inv1   g037(.a(x10), .O(new_n84_));
  aoi21  g038(.a(new_n66_), .b(new_n84_), .c(x16), .O(new_n85_));
  oai21  g039(.a(new_n66_), .b(x02), .c(new_n85_), .O(new_n86_));
  aoi21  g040(.a(new_n86_), .b(new_n83_), .c(new_n62_), .O(z11));
  nor2   g041(.a(new_n81_), .b(new_n53_), .O(new_n88_));
  oai21  g042(.a(new_n82_), .b(x22), .c(x16), .O(new_n89_));
  inv1   g043(.a(x03), .O(new_n90_));
  nand2  g044(.a(x18), .b(new_n90_), .O(new_n91_));
  aoi21  g045(.a(new_n66_), .b(new_n55_), .c(x16), .O(new_n92_));
  aoi21  g046(.a(new_n92_), .b(new_n91_), .c(new_n62_), .O(new_n93_));
  oai21  g047(.a(new_n89_), .b(new_n88_), .c(new_n93_), .O(z12));
  nand2  g048(.a(new_n81_), .b(x23), .O(new_n95_));
  nor2   g049(.a(x23), .b(x21), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n73_), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n95_), .c(new_n53_), .O(new_n98_));
  inv1   g052(.a(x16), .O(new_n99_));
  nand2  g053(.a(new_n47_), .b(x23), .O(new_n100_));
  aoi21  g054(.a(new_n100_), .b(x22), .c(new_n99_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  nor2   g056(.a(new_n62_), .b(x16), .O(new_n103_));
  inv1   g057(.a(x12), .O(new_n104_));
  nand2  g058(.a(new_n66_), .b(new_n104_), .O(new_n105_));
  inv1   g059(.a(x04), .O(new_n106_));
  nand2  g060(.a(x18), .b(new_n106_), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n102_), .O(z13));
  inv1   g063(.a(x24), .O(new_n110_));
  oai21  g064(.a(new_n97_), .b(x22), .c(new_n110_), .O(new_n111_));
  nand4  g065(.a(new_n96_), .b(new_n73_), .c(x24), .d(new_n53_), .O(new_n112_));
  nor2   g066(.a(new_n62_), .b(new_n99_), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  inv1   g068(.a(x13), .O(new_n115_));
  nand2  g069(.a(new_n66_), .b(new_n115_), .O(new_n116_));
  inv1   g070(.a(x05), .O(new_n117_));
  nand2  g071(.a(x18), .b(new_n117_), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n116_), .c(new_n103_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n114_), .O(z14));
  inv1   g074(.a(x25), .O(new_n121_));
  nor3   g075(.a(x24), .b(x23), .c(x21), .O(new_n122_));
  aoi21  g076(.a(new_n122_), .b(new_n73_), .c(new_n121_), .O(new_n123_));
  nor2   g077(.a(x24), .b(x23), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n121_), .c(new_n53_), .O(new_n125_));
  nor2   g079(.a(new_n125_), .b(new_n81_), .O(new_n126_));
  oai21  g080(.a(new_n126_), .b(new_n123_), .c(x16), .O(new_n127_));
  nand2  g081(.a(x25), .b(x16), .O(new_n128_));
  aoi21  g082(.a(new_n128_), .b(new_n47_), .c(new_n53_), .O(new_n129_));
  inv1   g083(.a(x06), .O(new_n130_));
  nand2  g084(.a(x18), .b(new_n130_), .O(new_n131_));
  inv1   g085(.a(x14), .O(new_n132_));
  aoi21  g086(.a(new_n66_), .b(new_n132_), .c(x16), .O(new_n133_));
  aoi21  g087(.a(new_n133_), .b(new_n131_), .c(new_n129_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n127_), .O(z15));
  nor2   g089(.a(x25), .b(x21), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n124_), .c(new_n63_), .d(new_n71_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(x26), .O(new_n138_));
  nor2   g092(.a(x26), .b(x25), .O(new_n139_));
  nand4  g093(.a(new_n139_), .b(new_n124_), .c(new_n73_), .d(new_n79_), .O(new_n140_));
  aoi21  g094(.a(new_n140_), .b(new_n138_), .c(x22), .O(new_n141_));
  nand2  g095(.a(new_n54_), .b(x26), .O(new_n142_));
  inv1   g096(.a(new_n142_), .O(new_n143_));
  oai21  g097(.a(new_n143_), .b(new_n141_), .c(x16), .O(new_n144_));
  inv1   g098(.a(x15), .O(new_n145_));
  nand2  g099(.a(new_n66_), .b(new_n145_), .O(new_n146_));
  inv1   g100(.a(x07), .O(new_n147_));
  nand2  g101(.a(x18), .b(new_n147_), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n146_), .c(new_n103_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n144_), .O(z16));
  oai21  g104(.a(x22), .b(x17), .c(x27), .O(new_n151_));
  nand3  g105(.a(new_n79_), .b(new_n71_), .c(x19), .O(new_n152_));
  nor3   g106(.a(new_n152_), .b(x22), .c(x17), .O(new_n153_));
  nand3  g107(.a(new_n153_), .b(new_n139_), .c(new_n124_), .O(new_n154_));
  aoi21  g108(.a(new_n154_), .b(new_n151_), .c(new_n103_), .O(z17));
  buf    g109(.a(x27), .O(z08));
endmodule


