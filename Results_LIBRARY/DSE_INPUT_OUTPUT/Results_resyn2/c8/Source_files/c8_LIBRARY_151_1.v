// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x27), .O(new_n47_));
  nand2  g01(.a(x27), .b(x16), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  aoi22  g03(.a(new_n49_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  oai22  g04(.a(new_n48_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g05(.a(new_n48_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g06(.a(new_n48_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g07(.a(new_n48_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g08(.a(new_n48_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g09(.a(new_n49_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  aoi22  g10(.a(new_n49_), .b(x15), .c(new_n47_), .d(x26), .O(z07));
  inv1   g11(.a(x18), .O(new_n58_));
  nor2   g12(.a(new_n58_), .b(x00), .O(new_n59_));
  nor2   g13(.a(x27), .b(x16), .O(new_n60_));
  inv1   g14(.a(x08), .O(new_n61_));
  nand2  g15(.a(new_n58_), .b(new_n61_), .O(new_n62_));
  nand2  g16(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  inv1   g17(.a(x17), .O(new_n64_));
  nor2   g18(.a(x19), .b(new_n64_), .O(new_n65_));
  inv1   g19(.a(x19), .O(new_n66_));
  oai21  g20(.a(new_n66_), .b(x17), .c(x16), .O(new_n67_));
  oai22  g21(.a(new_n67_), .b(new_n65_), .c(new_n63_), .d(new_n59_), .O(z09));
  inv1   g22(.a(x16), .O(new_n69_));
  inv1   g23(.a(x20), .O(new_n70_));
  nor2   g24(.a(x19), .b(x17), .O(new_n71_));
  xor2a  g25(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor2   g26(.a(new_n58_), .b(x01), .O(new_n73_));
  oai21  g27(.a(x18), .b(x09), .c(new_n60_), .O(new_n74_));
  oai22  g28(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n69_), .O(z10));
  inv1   g29(.a(x21), .O(new_n76_));
  aoi21  g30(.a(new_n71_), .b(new_n70_), .c(new_n76_), .O(new_n77_));
  nand4  g31(.a(new_n76_), .b(new_n70_), .c(new_n66_), .d(new_n64_), .O(new_n78_));
  inv1   g32(.a(new_n78_), .O(new_n79_));
  oai21  g33(.a(new_n79_), .b(new_n77_), .c(x16), .O(new_n80_));
  nor2   g34(.a(new_n58_), .b(x02), .O(new_n81_));
  oai21  g35(.a(x18), .b(x10), .c(new_n60_), .O(new_n82_));
  oai21  g36(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(z11));
  nand2  g37(.a(x18), .b(x03), .O(new_n84_));
  nand2  g38(.a(new_n58_), .b(x11), .O(new_n85_));
  nand3  g39(.a(new_n85_), .b(new_n84_), .c(new_n60_), .O(new_n86_));
  inv1   g40(.a(new_n86_), .O(new_n87_));
  nand2  g41(.a(new_n78_), .b(x22), .O(new_n88_));
  inv1   g42(.a(x22), .O(new_n89_));
  nor2   g43(.a(x21), .b(x20), .O(new_n90_));
  nand3  g44(.a(new_n90_), .b(new_n71_), .c(new_n89_), .O(new_n91_));
  and2   g45(.a(new_n91_), .b(x16), .O(new_n92_));
  aoi21  g46(.a(new_n92_), .b(new_n88_), .c(new_n87_), .O(z12));
  nand2  g47(.a(new_n91_), .b(x23), .O(new_n94_));
  nor2   g48(.a(x23), .b(x22), .O(new_n95_));
  nand3  g49(.a(new_n95_), .b(new_n90_), .c(new_n71_), .O(new_n96_));
  nand2  g50(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand2  g51(.a(new_n97_), .b(x16), .O(new_n98_));
  inv1   g52(.a(x04), .O(new_n99_));
  nand2  g53(.a(x18), .b(new_n99_), .O(new_n100_));
  inv1   g54(.a(x12), .O(new_n101_));
  nand2  g55(.a(new_n58_), .b(new_n101_), .O(new_n102_));
  nand3  g56(.a(new_n102_), .b(new_n100_), .c(new_n60_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(new_n98_), .O(z13));
  nand2  g58(.a(new_n96_), .b(x24), .O(new_n105_));
  nor2   g59(.a(x24), .b(x21), .O(new_n106_));
  nand4  g60(.a(new_n106_), .b(new_n95_), .c(new_n71_), .d(new_n70_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(x16), .O(new_n109_));
  inv1   g63(.a(x05), .O(new_n110_));
  nand2  g64(.a(x18), .b(new_n110_), .O(new_n111_));
  inv1   g65(.a(x13), .O(new_n112_));
  nand2  g66(.a(new_n58_), .b(new_n112_), .O(new_n113_));
  nand3  g67(.a(new_n113_), .b(new_n111_), .c(new_n60_), .O(new_n114_));
  nand2  g68(.a(new_n114_), .b(new_n109_), .O(z14));
  inv1   g69(.a(x23), .O(new_n116_));
  inv1   g70(.a(x24), .O(new_n117_));
  inv1   g71(.a(x25), .O(new_n118_));
  nand4  g72(.a(new_n118_), .b(new_n117_), .c(new_n116_), .d(new_n89_), .O(new_n119_));
  inv1   g73(.a(new_n119_), .O(new_n120_));
  aoi22  g74(.a(new_n120_), .b(new_n79_), .c(new_n107_), .d(x25), .O(new_n121_));
  inv1   g75(.a(x06), .O(new_n122_));
  nand2  g76(.a(x18), .b(new_n122_), .O(new_n123_));
  inv1   g77(.a(x14), .O(new_n124_));
  nand2  g78(.a(new_n58_), .b(new_n124_), .O(new_n125_));
  nand3  g79(.a(new_n125_), .b(new_n123_), .c(new_n60_), .O(new_n126_));
  oai21  g80(.a(new_n121_), .b(new_n69_), .c(new_n126_), .O(z15));
  inv1   g81(.a(x26), .O(new_n128_));
  nand3  g82(.a(new_n120_), .b(new_n79_), .c(new_n128_), .O(new_n129_));
  nor2   g83(.a(x25), .b(x24), .O(new_n130_));
  nand4  g84(.a(new_n130_), .b(new_n95_), .c(new_n90_), .d(new_n71_), .O(new_n131_));
  aoi21  g85(.a(new_n131_), .b(x26), .c(new_n69_), .O(new_n132_));
  nand2  g86(.a(x18), .b(x07), .O(new_n133_));
  nand2  g87(.a(new_n58_), .b(x15), .O(new_n134_));
  nand3  g88(.a(new_n134_), .b(new_n133_), .c(new_n60_), .O(new_n135_));
  inv1   g89(.a(new_n135_), .O(new_n136_));
  aoi21  g90(.a(new_n132_), .b(new_n129_), .c(new_n136_), .O(z16));
  nand2  g91(.a(x27), .b(x17), .O(new_n138_));
  nor2   g92(.a(new_n66_), .b(x17), .O(new_n139_));
  nand4  g93(.a(new_n120_), .b(new_n90_), .c(new_n139_), .d(new_n128_), .O(new_n140_));
  aoi21  g94(.a(new_n140_), .b(new_n138_), .c(new_n69_), .O(z17));
  buf    g95(.a(x27), .O(z08));
endmodule


