// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_;
  inv1   g00(.a(x27), .O(new_n47_));
  nand2  g01(.a(x27), .b(x07), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(z08));
  aoi22  g03(.a(z08), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  oai22  g04(.a(new_n48_), .b(x09), .c(x27), .d(x20), .O(z01));
  aoi22  g05(.a(z08), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  aoi22  g06(.a(z08), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  oai22  g07(.a(new_n48_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g08(.a(new_n48_), .b(x13), .c(x27), .d(x24), .O(z05));
  oai22  g09(.a(new_n48_), .b(x14), .c(x27), .d(x25), .O(z06));
  oai22  g10(.a(new_n48_), .b(x15), .c(x27), .d(x26), .O(z07));
  nor2   g11(.a(new_n47_), .b(x07), .O(new_n58_));
  inv1   g12(.a(x18), .O(new_n59_));
  inv1   g13(.a(x08), .O(new_n60_));
  aoi21  g14(.a(new_n59_), .b(new_n60_), .c(x16), .O(new_n61_));
  oai21  g15(.a(new_n59_), .b(x00), .c(new_n61_), .O(new_n62_));
  inv1   g16(.a(x17), .O(new_n63_));
  inv1   g17(.a(x16), .O(new_n64_));
  inv1   g18(.a(x19), .O(new_n65_));
  nor2   g19(.a(new_n65_), .b(x17), .O(new_n66_));
  nor2   g20(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  oai21  g21(.a(x19), .b(new_n63_), .c(new_n67_), .O(new_n68_));
  aoi21  g22(.a(new_n68_), .b(new_n62_), .c(new_n58_), .O(z09));
  inv1   g23(.a(x20), .O(new_n70_));
  nor2   g24(.a(x19), .b(x17), .O(new_n71_));
  nor2   g25(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor3   g26(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  oai21  g27(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  inv1   g28(.a(x01), .O(new_n75_));
  nand2  g29(.a(x18), .b(new_n75_), .O(new_n76_));
  inv1   g30(.a(x09), .O(new_n77_));
  aoi21  g31(.a(new_n59_), .b(new_n77_), .c(x16), .O(new_n78_));
  aoi21  g32(.a(new_n78_), .b(new_n76_), .c(new_n58_), .O(new_n79_));
  nand2  g33(.a(new_n79_), .b(new_n74_), .O(z10));
  inv1   g34(.a(new_n58_), .O(new_n81_));
  nand2  g35(.a(x18), .b(x02), .O(new_n82_));
  nand2  g36(.a(new_n59_), .b(x10), .O(new_n83_));
  nand3  g37(.a(new_n83_), .b(new_n82_), .c(new_n64_), .O(new_n84_));
  inv1   g38(.a(x21), .O(new_n85_));
  nor2   g39(.a(new_n73_), .b(new_n85_), .O(new_n86_));
  nor2   g40(.a(x21), .b(x20), .O(new_n87_));
  nand2  g41(.a(new_n87_), .b(new_n71_), .O(new_n88_));
  nand2  g42(.a(new_n88_), .b(x16), .O(new_n89_));
  oai21  g43(.a(new_n89_), .b(new_n86_), .c(new_n84_), .O(new_n90_));
  nand2  g44(.a(new_n90_), .b(new_n81_), .O(z11));
  xor2a  g45(.a(new_n88_), .b(x22), .O(new_n92_));
  inv1   g46(.a(x03), .O(new_n93_));
  nand2  g47(.a(x18), .b(new_n93_), .O(new_n94_));
  inv1   g48(.a(x11), .O(new_n95_));
  aoi21  g49(.a(new_n59_), .b(new_n95_), .c(x16), .O(new_n96_));
  aoi21  g50(.a(new_n96_), .b(new_n94_), .c(new_n58_), .O(new_n97_));
  oai21  g51(.a(new_n92_), .b(new_n64_), .c(new_n97_), .O(z12));
  oai21  g52(.a(new_n88_), .b(x22), .c(x23), .O(new_n99_));
  nor2   g53(.a(x23), .b(x22), .O(new_n100_));
  nand3  g54(.a(new_n100_), .b(new_n87_), .c(new_n71_), .O(new_n101_));
  and2   g55(.a(new_n101_), .b(x16), .O(new_n102_));
  nand2  g56(.a(x18), .b(x04), .O(new_n103_));
  nand2  g57(.a(new_n59_), .b(x12), .O(new_n104_));
  nand3  g58(.a(new_n104_), .b(new_n103_), .c(new_n64_), .O(new_n105_));
  nand2  g59(.a(new_n105_), .b(new_n81_), .O(new_n106_));
  aoi21  g60(.a(new_n102_), .b(new_n99_), .c(new_n106_), .O(z13));
  nand2  g61(.a(new_n101_), .b(x24), .O(new_n108_));
  nor2   g62(.a(x22), .b(x21), .O(new_n109_));
  nor2   g63(.a(x24), .b(x23), .O(new_n110_));
  nand4  g64(.a(new_n110_), .b(new_n109_), .c(new_n71_), .d(new_n70_), .O(new_n111_));
  nand2  g65(.a(new_n111_), .b(new_n108_), .O(new_n112_));
  nand2  g66(.a(new_n112_), .b(x16), .O(new_n113_));
  inv1   g67(.a(x13), .O(new_n114_));
  aoi21  g68(.a(new_n59_), .b(new_n114_), .c(x16), .O(new_n115_));
  oai21  g69(.a(new_n59_), .b(x05), .c(new_n115_), .O(new_n116_));
  aoi21  g70(.a(new_n116_), .b(new_n113_), .c(new_n58_), .O(z14));
  nand2  g71(.a(new_n111_), .b(x25), .O(new_n118_));
  nor2   g72(.a(x25), .b(x24), .O(new_n119_));
  nand4  g73(.a(new_n119_), .b(new_n100_), .c(new_n87_), .d(new_n71_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g75(.a(new_n121_), .b(x16), .O(new_n122_));
  inv1   g76(.a(x14), .O(new_n123_));
  aoi21  g77(.a(new_n59_), .b(new_n123_), .c(x16), .O(new_n124_));
  oai21  g78(.a(new_n59_), .b(x06), .c(new_n124_), .O(new_n125_));
  aoi21  g79(.a(new_n125_), .b(new_n122_), .c(new_n58_), .O(z15));
  inv1   g80(.a(x26), .O(new_n127_));
  nand3  g81(.a(new_n119_), .b(new_n100_), .c(new_n127_), .O(new_n128_));
  nor2   g82(.a(new_n128_), .b(new_n88_), .O(new_n129_));
  aoi21  g83(.a(new_n120_), .b(x26), .c(new_n129_), .O(new_n130_));
  nand2  g84(.a(new_n81_), .b(x16), .O(new_n131_));
  nor2   g85(.a(x27), .b(x18), .O(new_n132_));
  inv1   g86(.a(x15), .O(new_n133_));
  aoi21  g87(.a(new_n59_), .b(new_n133_), .c(x16), .O(new_n134_));
  oai21  g88(.a(new_n132_), .b(x07), .c(new_n134_), .O(new_n135_));
  oai21  g89(.a(new_n131_), .b(new_n130_), .c(new_n135_), .O(z16));
  nand3  g90(.a(new_n87_), .b(x19), .c(new_n63_), .O(new_n137_));
  oai22  g91(.a(new_n137_), .b(new_n128_), .c(new_n47_), .d(new_n63_), .O(new_n138_));
  nand2  g92(.a(new_n138_), .b(x07), .O(new_n139_));
  inv1   g93(.a(x25), .O(new_n140_));
  nand4  g94(.a(new_n47_), .b(new_n127_), .c(new_n140_), .d(new_n70_), .O(new_n141_));
  inv1   g95(.a(new_n141_), .O(new_n142_));
  nand4  g96(.a(new_n142_), .b(new_n110_), .c(new_n109_), .d(new_n66_), .O(new_n143_));
  aoi21  g97(.a(new_n143_), .b(new_n139_), .c(new_n64_), .O(z17));
endmodule


