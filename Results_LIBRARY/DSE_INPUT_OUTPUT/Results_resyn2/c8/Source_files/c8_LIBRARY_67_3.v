// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x17), .O(z08));
  inv1   g002(.a(z08), .O(new_n49_));
  oai22  g003(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  oai22  g004(.a(new_n49_), .b(x09), .c(x27), .d(x20), .O(z01));
  aoi22  g005(.a(z08), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  oai22  g006(.a(new_n49_), .b(x11), .c(x27), .d(x22), .O(z03));
  oai22  g007(.a(new_n49_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g008(.a(new_n49_), .b(x13), .c(x27), .d(x24), .O(z05));
  oai22  g009(.a(new_n49_), .b(x14), .c(x27), .d(x25), .O(z06));
  aoi22  g010(.a(z08), .b(x15), .c(new_n47_), .d(x26), .O(z07));
  inv1   g011(.a(x16), .O(new_n58_));
  aoi21  g012(.a(x27), .b(x17), .c(new_n58_), .O(new_n59_));
  inv1   g013(.a(new_n59_), .O(new_n60_));
  xor2a  g014(.a(x19), .b(x17), .O(new_n61_));
  aoi21  g015(.a(x27), .b(x17), .c(x16), .O(new_n62_));
  inv1   g016(.a(x08), .O(new_n63_));
  inv1   g017(.a(x18), .O(new_n64_));
  nand2  g018(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g019(.a(x00), .O(new_n66_));
  nand2  g020(.a(x18), .b(new_n66_), .O(new_n67_));
  nand3  g021(.a(new_n67_), .b(new_n65_), .c(new_n62_), .O(new_n68_));
  oai21  g022(.a(new_n61_), .b(new_n60_), .c(new_n68_), .O(z09));
  inv1   g023(.a(x20), .O(new_n70_));
  nor2   g024(.a(x19), .b(x17), .O(new_n71_));
  xor2a  g025(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g026(.a(x09), .O(new_n73_));
  nand2  g027(.a(new_n64_), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x01), .O(new_n75_));
  nand2  g029(.a(x18), .b(new_n75_), .O(new_n76_));
  nand3  g030(.a(new_n76_), .b(new_n74_), .c(new_n62_), .O(new_n77_));
  oai21  g031(.a(new_n72_), .b(new_n60_), .c(new_n77_), .O(z10));
  inv1   g032(.a(x21), .O(new_n79_));
  aoi21  g033(.a(new_n71_), .b(new_n70_), .c(new_n79_), .O(new_n80_));
  inv1   g034(.a(x17), .O(new_n81_));
  nor3   g035(.a(x21), .b(x20), .c(x19), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g037(.a(new_n83_), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n80_), .c(new_n59_), .O(new_n85_));
  inv1   g039(.a(x10), .O(new_n86_));
  nand2  g040(.a(new_n64_), .b(new_n86_), .O(new_n87_));
  inv1   g041(.a(x02), .O(new_n88_));
  nand2  g042(.a(x18), .b(new_n88_), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n87_), .c(new_n62_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n85_), .O(z11));
  aoi21  g045(.a(new_n82_), .b(x22), .c(new_n58_), .O(new_n92_));
  oai21  g046(.a(new_n84_), .b(x22), .c(new_n92_), .O(new_n93_));
  nand2  g047(.a(x22), .b(x16), .O(new_n94_));
  aoi21  g048(.a(new_n94_), .b(new_n47_), .c(new_n81_), .O(new_n95_));
  inv1   g049(.a(x03), .O(new_n96_));
  nand2  g050(.a(x18), .b(new_n96_), .O(new_n97_));
  inv1   g051(.a(x11), .O(new_n98_));
  aoi21  g052(.a(new_n64_), .b(new_n98_), .c(x16), .O(new_n99_));
  aoi21  g053(.a(new_n99_), .b(new_n97_), .c(new_n95_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n93_), .O(z12));
  inv1   g055(.a(x23), .O(new_n102_));
  nor2   g056(.a(x20), .b(x19), .O(new_n103_));
  nor2   g057(.a(x22), .b(x21), .O(new_n104_));
  aoi21  g058(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  nand4  g059(.a(new_n104_), .b(new_n103_), .c(new_n102_), .d(new_n81_), .O(new_n106_));
  inv1   g060(.a(new_n106_), .O(new_n107_));
  oai21  g061(.a(new_n107_), .b(new_n105_), .c(x16), .O(new_n108_));
  nand2  g062(.a(x23), .b(x16), .O(new_n109_));
  aoi21  g063(.a(new_n109_), .b(new_n47_), .c(new_n81_), .O(new_n110_));
  inv1   g064(.a(x04), .O(new_n111_));
  nand2  g065(.a(x18), .b(new_n111_), .O(new_n112_));
  inv1   g066(.a(x12), .O(new_n113_));
  aoi21  g067(.a(new_n64_), .b(new_n113_), .c(x16), .O(new_n114_));
  aoi21  g068(.a(new_n114_), .b(new_n112_), .c(new_n110_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n108_), .O(z13));
  inv1   g070(.a(x13), .O(new_n117_));
  nand2  g071(.a(new_n64_), .b(new_n117_), .O(new_n118_));
  inv1   g072(.a(x05), .O(new_n119_));
  nand2  g073(.a(x18), .b(new_n119_), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n118_), .c(new_n62_), .O(new_n121_));
  inv1   g075(.a(x24), .O(new_n122_));
  nor2   g076(.a(new_n106_), .b(new_n122_), .O(new_n123_));
  oai21  g077(.a(new_n107_), .b(x24), .c(new_n59_), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n123_), .c(new_n121_), .O(z14));
  nand4  g079(.a(new_n104_), .b(new_n103_), .c(new_n122_), .d(new_n102_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(x25), .O(new_n127_));
  nor2   g081(.a(x24), .b(x23), .O(new_n128_));
  nor2   g082(.a(x25), .b(x22), .O(new_n129_));
  nand4  g083(.a(new_n129_), .b(new_n128_), .c(new_n82_), .d(new_n81_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(x16), .O(new_n132_));
  nand2  g086(.a(x25), .b(x16), .O(new_n133_));
  aoi21  g087(.a(new_n133_), .b(new_n47_), .c(new_n81_), .O(new_n134_));
  inv1   g088(.a(x06), .O(new_n135_));
  nand2  g089(.a(x18), .b(new_n135_), .O(new_n136_));
  inv1   g090(.a(x14), .O(new_n137_));
  aoi21  g091(.a(new_n64_), .b(new_n137_), .c(x16), .O(new_n138_));
  aoi21  g092(.a(new_n138_), .b(new_n136_), .c(new_n134_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n132_), .O(z15));
  nand4  g094(.a(new_n129_), .b(new_n128_), .c(new_n103_), .d(new_n79_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x26), .O(new_n142_));
  nor2   g096(.a(x26), .b(x25), .O(new_n143_));
  nand4  g097(.a(new_n143_), .b(new_n128_), .c(new_n104_), .d(new_n103_), .O(new_n144_));
  nand3  g098(.a(new_n144_), .b(new_n142_), .c(new_n81_), .O(new_n145_));
  nand2  g099(.a(new_n47_), .b(x26), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(x17), .c(new_n58_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  inv1   g102(.a(x15), .O(new_n149_));
  nand2  g103(.a(new_n64_), .b(new_n149_), .O(new_n150_));
  inv1   g104(.a(x07), .O(new_n151_));
  nand2  g105(.a(x18), .b(new_n151_), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n150_), .c(new_n62_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n148_), .O(z16));
  nand2  g108(.a(new_n143_), .b(new_n128_), .O(new_n155_));
  nand4  g109(.a(new_n70_), .b(x19), .c(new_n81_), .d(x16), .O(new_n156_));
  nor4   g110(.a(new_n156_), .b(new_n155_), .c(x22), .d(x21), .O(z17));
endmodule


