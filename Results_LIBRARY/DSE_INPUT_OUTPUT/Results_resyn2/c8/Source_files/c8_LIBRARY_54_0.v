// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n52_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x24), .O(z08));
  oai22  g002(.a(z08), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g003(.a(z08), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  oai22  g004(.a(z08), .b(x21), .c(new_n47_), .d(x10), .O(z02));
  inv1   g005(.a(z08), .O(new_n52_));
  aoi22  g006(.a(new_n52_), .b(x22), .c(x27), .d(x11), .O(z03));
  aoi22  g007(.a(new_n52_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi21  g008(.a(x27), .b(x13), .c(new_n52_), .O(z05));
  oai22  g009(.a(z08), .b(x25), .c(new_n47_), .d(x14), .O(z06));
  oai22  g010(.a(z08), .b(x26), .c(new_n47_), .d(x15), .O(z07));
  nor2   g011(.a(x27), .b(x24), .O(new_n58_));
  inv1   g012(.a(x17), .O(new_n59_));
  inv1   g013(.a(x19), .O(new_n60_));
  nor2   g014(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g015(.a(x19), .b(x17), .O(new_n62_));
  oai21  g016(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  inv1   g017(.a(x18), .O(new_n64_));
  inv1   g018(.a(x08), .O(new_n65_));
  aoi21  g019(.a(new_n64_), .b(new_n65_), .c(x16), .O(new_n66_));
  oai21  g020(.a(new_n64_), .b(x00), .c(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n63_), .c(new_n58_), .O(z09));
  inv1   g022(.a(x20), .O(new_n69_));
  nor2   g023(.a(new_n62_), .b(new_n69_), .O(new_n70_));
  nand3  g024(.a(new_n69_), .b(new_n60_), .c(new_n59_), .O(new_n71_));
  inv1   g025(.a(new_n71_), .O(new_n72_));
  oai21  g026(.a(new_n72_), .b(new_n70_), .c(x16), .O(new_n73_));
  inv1   g027(.a(x01), .O(new_n74_));
  nand2  g028(.a(x18), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x09), .O(new_n76_));
  aoi21  g030(.a(new_n64_), .b(new_n76_), .c(x16), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n58_), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n73_), .O(z10));
  inv1   g033(.a(x16), .O(new_n80_));
  xor2a  g034(.a(new_n71_), .b(x21), .O(new_n81_));
  inv1   g035(.a(x02), .O(new_n82_));
  nand2  g036(.a(x18), .b(new_n82_), .O(new_n83_));
  inv1   g037(.a(x10), .O(new_n84_));
  aoi21  g038(.a(new_n64_), .b(new_n84_), .c(x16), .O(new_n85_));
  aoi21  g039(.a(new_n85_), .b(new_n83_), .c(new_n58_), .O(new_n86_));
  oai21  g040(.a(new_n81_), .b(new_n80_), .c(new_n86_), .O(z11));
  oai21  g041(.a(new_n71_), .b(x21), .c(x22), .O(new_n88_));
  inv1   g042(.a(x21), .O(new_n89_));
  inv1   g043(.a(x22), .O(new_n90_));
  nand4  g044(.a(new_n62_), .b(new_n90_), .c(new_n89_), .d(new_n69_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(x16), .O(new_n93_));
  inv1   g047(.a(x11), .O(new_n94_));
  aoi21  g048(.a(new_n64_), .b(new_n94_), .c(x16), .O(new_n95_));
  oai21  g049(.a(new_n64_), .b(x03), .c(new_n95_), .O(new_n96_));
  aoi21  g050(.a(new_n96_), .b(new_n93_), .c(new_n58_), .O(z12));
  nand2  g051(.a(new_n91_), .b(x23), .O(new_n98_));
  inv1   g052(.a(x23), .O(new_n99_));
  nor2   g053(.a(x22), .b(x21), .O(new_n100_));
  nand4  g054(.a(new_n100_), .b(new_n62_), .c(new_n99_), .d(new_n69_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(x16), .O(new_n103_));
  inv1   g057(.a(x12), .O(new_n104_));
  aoi21  g058(.a(new_n64_), .b(new_n104_), .c(x16), .O(new_n105_));
  oai21  g059(.a(new_n64_), .b(x04), .c(new_n105_), .O(new_n106_));
  aoi21  g060(.a(new_n106_), .b(new_n103_), .c(new_n58_), .O(z13));
  and2   g061(.a(new_n101_), .b(x24), .O(new_n108_));
  nor2   g062(.a(x24), .b(x23), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(x27), .O(new_n110_));
  nor2   g064(.a(new_n110_), .b(new_n91_), .O(new_n111_));
  oai21  g065(.a(new_n111_), .b(new_n108_), .c(x16), .O(new_n112_));
  nor2   g066(.a(new_n58_), .b(x16), .O(new_n113_));
  inv1   g067(.a(x13), .O(new_n114_));
  nand2  g068(.a(new_n64_), .b(new_n114_), .O(new_n115_));
  inv1   g069(.a(x05), .O(new_n116_));
  nand2  g070(.a(x18), .b(new_n116_), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n112_), .O(z14));
  inv1   g073(.a(x24), .O(new_n120_));
  inv1   g074(.a(x25), .O(new_n121_));
  nand3  g075(.a(new_n99_), .b(new_n90_), .c(new_n89_), .O(new_n122_));
  oai21  g076(.a(new_n122_), .b(new_n71_), .c(x27), .O(new_n123_));
  aoi21  g077(.a(new_n123_), .b(new_n120_), .c(new_n121_), .O(new_n124_));
  nor3   g078(.a(new_n110_), .b(new_n91_), .c(x25), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n124_), .c(x16), .O(new_n126_));
  inv1   g080(.a(x14), .O(new_n127_));
  nand2  g081(.a(new_n64_), .b(new_n127_), .O(new_n128_));
  inv1   g082(.a(x06), .O(new_n129_));
  nand2  g083(.a(x18), .b(new_n129_), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n128_), .c(new_n113_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n126_), .O(z15));
  nand2  g086(.a(new_n111_), .b(new_n121_), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(x26), .c(x16), .O(new_n134_));
  inv1   g088(.a(x26), .O(new_n135_));
  nand3  g089(.a(new_n100_), .b(new_n135_), .c(new_n121_), .O(new_n136_));
  nand3  g090(.a(new_n72_), .b(new_n99_), .c(x16), .O(new_n137_));
  oai21  g091(.a(new_n137_), .b(new_n136_), .c(x27), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n120_), .O(new_n139_));
  inv1   g093(.a(x15), .O(new_n140_));
  aoi21  g094(.a(new_n64_), .b(new_n140_), .c(x16), .O(new_n141_));
  oai21  g095(.a(new_n64_), .b(x07), .c(new_n141_), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n139_), .c(new_n134_), .O(z16));
  oai21  g097(.a(new_n47_), .b(x17), .c(z08), .O(new_n144_));
  inv1   g098(.a(new_n136_), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n109_), .c(new_n69_), .d(x19), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n144_), .c(new_n113_), .O(z17));
endmodule


