// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_;
  inv1   g00(.a(x27), .O(new_n47_));
  nand2  g01(.a(x27), .b(x13), .O(new_n48_));
  inv1   g02(.a(new_n48_), .O(new_n49_));
  aoi22  g03(.a(new_n49_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  oai22  g04(.a(new_n48_), .b(x09), .c(x27), .d(x20), .O(z01));
  aoi22  g05(.a(new_n49_), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  aoi22  g06(.a(new_n49_), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  oai22  g07(.a(new_n48_), .b(x12), .c(x27), .d(x23), .O(z04));
  aoi21  g08(.a(new_n47_), .b(x24), .c(new_n49_), .O(z05));
  oai22  g09(.a(new_n48_), .b(x14), .c(x27), .d(x25), .O(z06));
  inv1   g10(.a(x26), .O(new_n57_));
  nor2   g11(.a(new_n47_), .b(x13), .O(new_n58_));
  aoi21  g12(.a(new_n47_), .b(new_n57_), .c(new_n58_), .O(new_n59_));
  oai21  g13(.a(new_n47_), .b(x15), .c(new_n59_), .O(z07));
  inv1   g14(.a(x18), .O(new_n61_));
  inv1   g15(.a(x08), .O(new_n62_));
  aoi21  g16(.a(new_n61_), .b(new_n62_), .c(x16), .O(new_n63_));
  oai21  g17(.a(new_n61_), .b(x00), .c(new_n63_), .O(new_n64_));
  inv1   g18(.a(x19), .O(new_n65_));
  nand2  g19(.a(new_n65_), .b(x17), .O(new_n66_));
  inv1   g20(.a(x16), .O(new_n67_));
  nor2   g21(.a(new_n65_), .b(x17), .O(new_n68_));
  nor2   g22(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g23(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  aoi21  g24(.a(new_n70_), .b(new_n64_), .c(new_n58_), .O(z09));
  nor2   g25(.a(x19), .b(x17), .O(new_n72_));
  xor2a  g26(.a(new_n72_), .b(x20), .O(new_n73_));
  nand2  g27(.a(new_n73_), .b(x16), .O(new_n74_));
  inv1   g28(.a(x09), .O(new_n75_));
  aoi21  g29(.a(new_n61_), .b(new_n75_), .c(x16), .O(new_n76_));
  oai21  g30(.a(new_n61_), .b(x01), .c(new_n76_), .O(new_n77_));
  aoi21  g31(.a(new_n77_), .b(new_n74_), .c(new_n58_), .O(z10));
  inv1   g32(.a(x20), .O(new_n79_));
  nand2  g33(.a(new_n72_), .b(new_n79_), .O(new_n80_));
  nor2   g34(.a(x21), .b(x20), .O(new_n81_));
  nand2  g35(.a(new_n81_), .b(new_n72_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  aoi21  g37(.a(new_n80_), .b(x21), .c(new_n83_), .O(new_n84_));
  inv1   g38(.a(x02), .O(new_n85_));
  nand2  g39(.a(x18), .b(new_n85_), .O(new_n86_));
  inv1   g40(.a(x10), .O(new_n87_));
  aoi21  g41(.a(new_n61_), .b(new_n87_), .c(x16), .O(new_n88_));
  aoi21  g42(.a(new_n88_), .b(new_n86_), .c(new_n58_), .O(new_n89_));
  oai21  g43(.a(new_n84_), .b(new_n67_), .c(new_n89_), .O(z11));
  xor2a  g44(.a(new_n82_), .b(x22), .O(new_n91_));
  inv1   g45(.a(x03), .O(new_n92_));
  nand2  g46(.a(x18), .b(new_n92_), .O(new_n93_));
  inv1   g47(.a(x11), .O(new_n94_));
  aoi21  g48(.a(new_n61_), .b(new_n94_), .c(x16), .O(new_n95_));
  aoi21  g49(.a(new_n95_), .b(new_n93_), .c(new_n58_), .O(new_n96_));
  oai21  g50(.a(new_n91_), .b(new_n67_), .c(new_n96_), .O(z12));
  inv1   g51(.a(x23), .O(new_n98_));
  inv1   g52(.a(x22), .O(new_n99_));
  nand3  g53(.a(new_n81_), .b(new_n72_), .c(new_n99_), .O(new_n100_));
  xor2a  g54(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g55(.a(new_n101_), .b(x16), .O(new_n102_));
  inv1   g56(.a(x12), .O(new_n103_));
  aoi21  g57(.a(new_n61_), .b(new_n103_), .c(x16), .O(new_n104_));
  oai21  g58(.a(new_n61_), .b(x04), .c(new_n104_), .O(new_n105_));
  aoi21  g59(.a(new_n105_), .b(new_n102_), .c(new_n58_), .O(z13));
  oai21  g60(.a(new_n100_), .b(x23), .c(x24), .O(new_n107_));
  nor2   g61(.a(x24), .b(x23), .O(new_n108_));
  nand4  g62(.a(new_n108_), .b(new_n81_), .c(new_n72_), .d(new_n99_), .O(new_n109_));
  and2   g63(.a(new_n109_), .b(x16), .O(new_n110_));
  inv1   g64(.a(new_n58_), .O(new_n111_));
  nand2  g65(.a(x18), .b(x05), .O(new_n112_));
  nand2  g66(.a(new_n61_), .b(x13), .O(new_n113_));
  nand3  g67(.a(new_n113_), .b(new_n112_), .c(new_n67_), .O(new_n114_));
  nand2  g68(.a(new_n114_), .b(new_n111_), .O(new_n115_));
  aoi21  g69(.a(new_n110_), .b(new_n107_), .c(new_n115_), .O(z14));
  nor2   g70(.a(x25), .b(x22), .O(new_n117_));
  nand2  g71(.a(new_n117_), .b(new_n108_), .O(new_n118_));
  inv1   g72(.a(new_n118_), .O(new_n119_));
  aoi22  g73(.a(new_n119_), .b(new_n83_), .c(new_n109_), .d(x25), .O(new_n120_));
  inv1   g74(.a(x06), .O(new_n121_));
  nand2  g75(.a(x18), .b(new_n121_), .O(new_n122_));
  inv1   g76(.a(x14), .O(new_n123_));
  aoi21  g77(.a(new_n61_), .b(new_n123_), .c(x16), .O(new_n124_));
  aoi21  g78(.a(new_n124_), .b(new_n122_), .c(new_n58_), .O(new_n125_));
  oai21  g79(.a(new_n120_), .b(new_n67_), .c(new_n125_), .O(z15));
  nand3  g80(.a(new_n119_), .b(new_n83_), .c(x26), .O(new_n127_));
  nand4  g81(.a(new_n117_), .b(new_n108_), .c(new_n81_), .d(new_n72_), .O(new_n128_));
  aoi21  g82(.a(new_n128_), .b(new_n57_), .c(new_n67_), .O(new_n129_));
  nand2  g83(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  inv1   g84(.a(x15), .O(new_n131_));
  aoi21  g85(.a(new_n61_), .b(new_n131_), .c(x16), .O(new_n132_));
  oai21  g86(.a(new_n61_), .b(x07), .c(new_n132_), .O(new_n133_));
  aoi21  g87(.a(new_n133_), .b(new_n130_), .c(new_n58_), .O(z16));
  nand2  g88(.a(x27), .b(x17), .O(new_n135_));
  nand4  g89(.a(new_n119_), .b(new_n81_), .c(new_n68_), .d(new_n57_), .O(new_n136_));
  nand2  g90(.a(new_n111_), .b(x16), .O(new_n137_));
  aoi21  g91(.a(new_n136_), .b(new_n135_), .c(new_n137_), .O(z17));
  buf    g92(.a(x27), .O(z08));
endmodule


