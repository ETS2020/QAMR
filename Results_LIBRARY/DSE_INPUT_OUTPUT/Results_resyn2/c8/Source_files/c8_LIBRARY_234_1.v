// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n50_, new_n51_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n141_, new_n142_, new_n143_, new_n144_;
  nand2  g00(.a(x27), .b(x18), .O(new_n47_));
  oai22  g01(.a(new_n47_), .b(x08), .c(x27), .d(x19), .O(z00));
  oai22  g02(.a(new_n47_), .b(x09), .c(x27), .d(x20), .O(z01));
  inv1   g03(.a(x27), .O(new_n50_));
  inv1   g04(.a(new_n47_), .O(new_n51_));
  aoi22  g05(.a(new_n51_), .b(x10), .c(new_n50_), .d(x21), .O(z02));
  aoi22  g06(.a(new_n51_), .b(x11), .c(new_n50_), .d(x22), .O(z03));
  aoi22  g07(.a(new_n51_), .b(x12), .c(new_n50_), .d(x23), .O(z04));
  aoi22  g08(.a(new_n51_), .b(x13), .c(new_n50_), .d(x24), .O(z05));
  oai22  g09(.a(new_n47_), .b(x14), .c(x27), .d(x25), .O(z06));
  aoi22  g10(.a(new_n51_), .b(x15), .c(new_n50_), .d(x26), .O(z07));
  nor2   g11(.a(x27), .b(x18), .O(new_n58_));
  aoi22  g12(.a(new_n58_), .b(x08), .c(x18), .d(x00), .O(new_n59_));
  nor2   g13(.a(new_n50_), .b(x18), .O(new_n60_));
  inv1   g14(.a(new_n60_), .O(new_n61_));
  nand2  g15(.a(new_n61_), .b(x16), .O(new_n62_));
  xor2a  g16(.a(x19), .b(x17), .O(new_n63_));
  oai22  g17(.a(new_n63_), .b(new_n62_), .c(new_n59_), .d(x16), .O(z09));
  inv1   g18(.a(x16), .O(new_n65_));
  nand2  g19(.a(x18), .b(x01), .O(new_n66_));
  nand2  g20(.a(new_n58_), .b(x09), .O(new_n67_));
  nand2  g21(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g22(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  inv1   g23(.a(x20), .O(new_n70_));
  nor2   g24(.a(x19), .b(x17), .O(new_n71_));
  xor2a  g25(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  oai21  g26(.a(new_n72_), .b(new_n62_), .c(new_n69_), .O(z10));
  nand2  g27(.a(x18), .b(x02), .O(new_n74_));
  nand2  g28(.a(new_n58_), .b(x10), .O(new_n75_));
  nand2  g29(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g30(.a(new_n76_), .b(new_n65_), .O(new_n77_));
  inv1   g31(.a(x21), .O(new_n78_));
  aoi21  g32(.a(new_n71_), .b(new_n70_), .c(new_n78_), .O(new_n79_));
  inv1   g33(.a(x17), .O(new_n80_));
  inv1   g34(.a(x19), .O(new_n81_));
  nand4  g35(.a(new_n78_), .b(new_n70_), .c(new_n81_), .d(new_n80_), .O(new_n82_));
  inv1   g36(.a(new_n82_), .O(new_n83_));
  nor2   g37(.a(new_n83_), .b(new_n79_), .O(new_n84_));
  oai21  g38(.a(new_n84_), .b(new_n62_), .c(new_n77_), .O(z11));
  xor2a  g39(.a(new_n82_), .b(x22), .O(new_n86_));
  inv1   g40(.a(x03), .O(new_n87_));
  nand2  g41(.a(x18), .b(new_n87_), .O(new_n88_));
  inv1   g42(.a(x11), .O(new_n89_));
  inv1   g43(.a(x18), .O(new_n90_));
  aoi21  g44(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  aoi21  g45(.a(new_n91_), .b(new_n88_), .c(new_n60_), .O(new_n92_));
  oai21  g46(.a(new_n86_), .b(new_n65_), .c(new_n92_), .O(z12));
  inv1   g47(.a(x22), .O(new_n94_));
  nor2   g48(.a(x21), .b(x20), .O(new_n95_));
  nand3  g49(.a(new_n95_), .b(new_n71_), .c(new_n94_), .O(new_n96_));
  nand2  g50(.a(new_n96_), .b(x23), .O(new_n97_));
  nor2   g51(.a(x23), .b(x22), .O(new_n98_));
  nand3  g52(.a(new_n98_), .b(new_n95_), .c(new_n71_), .O(new_n99_));
  nand2  g53(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  nand2  g54(.a(new_n100_), .b(x16), .O(new_n101_));
  inv1   g55(.a(x04), .O(new_n102_));
  nand2  g56(.a(x18), .b(new_n102_), .O(new_n103_));
  inv1   g57(.a(x12), .O(new_n104_));
  aoi21  g58(.a(new_n90_), .b(new_n104_), .c(x16), .O(new_n105_));
  aoi21  g59(.a(new_n105_), .b(new_n103_), .c(new_n60_), .O(new_n106_));
  nand2  g60(.a(new_n106_), .b(new_n101_), .O(z13));
  nand2  g61(.a(x18), .b(x05), .O(new_n108_));
  nand2  g62(.a(new_n58_), .b(x13), .O(new_n109_));
  nand2  g63(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g64(.a(new_n110_), .b(new_n65_), .O(new_n111_));
  nand2  g65(.a(new_n99_), .b(x24), .O(new_n112_));
  nor2   g66(.a(x24), .b(x21), .O(new_n113_));
  nand4  g67(.a(new_n113_), .b(new_n98_), .c(new_n71_), .d(new_n70_), .O(new_n114_));
  nand2  g68(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand3  g69(.a(new_n115_), .b(new_n61_), .c(x16), .O(new_n116_));
  nand2  g70(.a(new_n116_), .b(new_n111_), .O(z14));
  nand2  g71(.a(x18), .b(x06), .O(new_n118_));
  nand2  g72(.a(new_n58_), .b(x14), .O(new_n119_));
  nand2  g73(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(new_n65_), .O(new_n121_));
  inv1   g75(.a(x23), .O(new_n122_));
  inv1   g76(.a(x24), .O(new_n123_));
  inv1   g77(.a(x25), .O(new_n124_));
  nand4  g78(.a(new_n124_), .b(new_n123_), .c(new_n122_), .d(new_n94_), .O(new_n125_));
  inv1   g79(.a(new_n125_), .O(new_n126_));
  aoi22  g80(.a(new_n126_), .b(new_n83_), .c(new_n114_), .d(x25), .O(new_n127_));
  oai21  g81(.a(new_n127_), .b(new_n62_), .c(new_n121_), .O(z15));
  inv1   g82(.a(x26), .O(new_n129_));
  nor2   g83(.a(x25), .b(x24), .O(new_n130_));
  nand4  g84(.a(new_n130_), .b(new_n98_), .c(new_n95_), .d(new_n71_), .O(new_n131_));
  nor2   g85(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand2  g86(.a(new_n131_), .b(new_n129_), .O(new_n133_));
  nand2  g87(.a(new_n133_), .b(x16), .O(new_n134_));
  inv1   g88(.a(x07), .O(new_n135_));
  nand2  g89(.a(x18), .b(new_n135_), .O(new_n136_));
  inv1   g90(.a(x15), .O(new_n137_));
  aoi21  g91(.a(new_n90_), .b(new_n137_), .c(x16), .O(new_n138_));
  aoi21  g92(.a(new_n138_), .b(new_n136_), .c(new_n60_), .O(new_n139_));
  oai21  g93(.a(new_n134_), .b(new_n132_), .c(new_n139_), .O(z16));
  nand2  g94(.a(x27), .b(x17), .O(new_n141_));
  nand4  g95(.a(new_n95_), .b(new_n129_), .c(x19), .d(new_n80_), .O(new_n142_));
  oai21  g96(.a(new_n142_), .b(new_n125_), .c(new_n141_), .O(new_n143_));
  nand2  g97(.a(new_n143_), .b(x16), .O(new_n144_));
  nand2  g98(.a(new_n144_), .b(new_n61_), .O(z17));
  buf    g99(.a(x27), .O(z08));
endmodule


