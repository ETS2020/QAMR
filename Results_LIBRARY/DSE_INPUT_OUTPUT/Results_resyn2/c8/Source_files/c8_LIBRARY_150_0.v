// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x20), .O(new_n47_));
  nor2   g001(.a(x27), .b(new_n47_), .O(new_n48_));
  aoi22  g002(.a(new_n48_), .b(x19), .c(x27), .d(x08), .O(z00));
  inv1   g003(.a(x27), .O(new_n50_));
  nor2   g004(.a(new_n50_), .b(x09), .O(z01));
  inv1   g005(.a(new_n48_), .O(z08));
  oai22  g006(.a(z08), .b(x21), .c(new_n50_), .d(x10), .O(z02));
  oai22  g007(.a(z08), .b(x22), .c(new_n50_), .d(x11), .O(z03));
  aoi22  g008(.a(new_n48_), .b(x23), .c(x27), .d(x12), .O(z04));
  aoi22  g009(.a(new_n48_), .b(x24), .c(x27), .d(x13), .O(z05));
  oai22  g010(.a(z08), .b(x25), .c(new_n50_), .d(x14), .O(z06));
  aoi22  g011(.a(new_n48_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g012(.a(x17), .O(new_n59_));
  inv1   g013(.a(x19), .O(new_n60_));
  nor2   g014(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nor2   g015(.a(x19), .b(x17), .O(new_n62_));
  oai21  g016(.a(new_n62_), .b(new_n61_), .c(x16), .O(new_n63_));
  nor2   g017(.a(x27), .b(x20), .O(new_n64_));
  inv1   g018(.a(new_n64_), .O(new_n65_));
  inv1   g019(.a(x18), .O(new_n66_));
  inv1   g020(.a(x08), .O(new_n67_));
  aoi21  g021(.a(new_n66_), .b(new_n67_), .c(x16), .O(new_n68_));
  oai21  g022(.a(new_n66_), .b(x00), .c(new_n68_), .O(new_n69_));
  nand3  g023(.a(new_n69_), .b(new_n65_), .c(new_n63_), .O(z09));
  nand3  g024(.a(new_n62_), .b(x27), .c(new_n47_), .O(new_n71_));
  oai21  g025(.a(new_n62_), .b(new_n47_), .c(new_n71_), .O(new_n72_));
  nand2  g026(.a(new_n72_), .b(x16), .O(new_n73_));
  nor2   g027(.a(new_n64_), .b(x16), .O(new_n74_));
  inv1   g028(.a(x09), .O(new_n75_));
  nand2  g029(.a(new_n66_), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x01), .O(new_n77_));
  nand2  g031(.a(x18), .b(new_n77_), .O(new_n78_));
  nand3  g032(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n73_), .O(z10));
  nor3   g034(.a(x21), .b(x19), .c(x17), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(x16), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(x27), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n47_), .O(new_n84_));
  inv1   g038(.a(x10), .O(new_n85_));
  aoi21  g039(.a(new_n66_), .b(new_n85_), .c(x16), .O(new_n86_));
  oai21  g040(.a(new_n66_), .b(x02), .c(new_n86_), .O(new_n87_));
  nand3  g041(.a(new_n71_), .b(x21), .c(x16), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n87_), .c(new_n84_), .O(z11));
  oai21  g043(.a(new_n82_), .b(x22), .c(x27), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n47_), .O(new_n91_));
  inv1   g045(.a(x03), .O(new_n92_));
  nand2  g046(.a(x18), .b(new_n92_), .O(new_n93_));
  inv1   g047(.a(x11), .O(new_n94_));
  aoi21  g048(.a(new_n66_), .b(new_n94_), .c(x16), .O(new_n95_));
  nand2  g049(.a(new_n81_), .b(new_n47_), .O(new_n96_));
  and2   g050(.a(x22), .b(x16), .O(new_n97_));
  aoi22  g051(.a(new_n97_), .b(new_n96_), .c(new_n95_), .d(new_n93_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n91_), .O(z12));
  inv1   g053(.a(x12), .O(new_n100_));
  nand2  g054(.a(new_n66_), .b(new_n100_), .O(new_n101_));
  inv1   g055(.a(x04), .O(new_n102_));
  nand2  g056(.a(x18), .b(new_n102_), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n101_), .c(new_n74_), .O(new_n104_));
  inv1   g058(.a(x23), .O(new_n105_));
  nor3   g059(.a(new_n96_), .b(new_n105_), .c(x22), .O(new_n106_));
  oai21  g060(.a(new_n96_), .b(x22), .c(new_n105_), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n65_), .c(x16), .O(new_n108_));
  oai21  g062(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(z13));
  inv1   g063(.a(x24), .O(new_n110_));
  nor2   g064(.a(x22), .b(x21), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n62_), .c(new_n105_), .O(new_n112_));
  aoi21  g066(.a(new_n112_), .b(x27), .c(x20), .O(new_n113_));
  nand2  g067(.a(new_n111_), .b(new_n62_), .O(new_n114_));
  inv1   g068(.a(new_n114_), .O(new_n115_));
  nand4  g069(.a(x27), .b(new_n110_), .c(new_n105_), .d(new_n47_), .O(new_n116_));
  inv1   g070(.a(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  oai21  g072(.a(new_n113_), .b(new_n110_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x16), .O(new_n120_));
  inv1   g074(.a(x13), .O(new_n121_));
  nand2  g075(.a(new_n66_), .b(new_n121_), .O(new_n122_));
  inv1   g076(.a(x05), .O(new_n123_));
  nand2  g077(.a(x18), .b(new_n123_), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n122_), .c(new_n74_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n120_), .O(z14));
  nor2   g080(.a(x24), .b(x23), .O(new_n127_));
  nor2   g081(.a(x25), .b(x22), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n82_), .c(x27), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n47_), .O(new_n131_));
  inv1   g085(.a(x14), .O(new_n132_));
  aoi21  g086(.a(new_n66_), .b(new_n132_), .c(x16), .O(new_n133_));
  oai21  g087(.a(new_n66_), .b(x06), .c(new_n133_), .O(new_n134_));
  nand3  g088(.a(new_n118_), .b(x25), .c(x16), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n134_), .c(new_n131_), .O(z15));
  inv1   g090(.a(x26), .O(new_n137_));
  inv1   g091(.a(x21), .O(new_n138_));
  nand4  g092(.a(new_n128_), .b(new_n127_), .c(new_n62_), .d(new_n138_), .O(new_n139_));
  aoi21  g093(.a(new_n139_), .b(x27), .c(x20), .O(new_n140_));
  nor2   g094(.a(x26), .b(x25), .O(new_n141_));
  nand3  g095(.a(new_n141_), .b(new_n117_), .c(new_n115_), .O(new_n142_));
  oai21  g096(.a(new_n140_), .b(new_n137_), .c(new_n142_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(x16), .O(new_n144_));
  inv1   g098(.a(x15), .O(new_n145_));
  nand2  g099(.a(new_n66_), .b(new_n145_), .O(new_n146_));
  inv1   g100(.a(x07), .O(new_n147_));
  nand2  g101(.a(x18), .b(new_n147_), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n146_), .c(new_n74_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n144_), .O(z16));
  nor2   g104(.a(x20), .b(new_n60_), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n141_), .c(new_n127_), .d(new_n111_), .O(new_n152_));
  nand2  g106(.a(x27), .b(x16), .O(new_n153_));
  aoi21  g107(.a(new_n152_), .b(new_n59_), .c(new_n153_), .O(z17));
endmodule


