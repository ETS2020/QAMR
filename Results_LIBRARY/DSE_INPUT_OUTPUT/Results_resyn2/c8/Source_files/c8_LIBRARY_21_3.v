// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n49_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x27), .O(new_n47_));
  nor2   g01(.a(new_n47_), .b(x14), .O(z08));
  inv1   g02(.a(z08), .O(new_n49_));
  oai22  g03(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  aoi22  g04(.a(z08), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  oai22  g05(.a(new_n49_), .b(x10), .c(x27), .d(x21), .O(z02));
  aoi22  g06(.a(z08), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  aoi22  g07(.a(z08), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  aoi22  g08(.a(z08), .b(x13), .c(new_n47_), .d(x24), .O(z05));
  nand2  g09(.a(new_n47_), .b(x25), .O(z06));
  oai22  g10(.a(new_n49_), .b(x15), .c(x27), .d(x26), .O(z07));
  inv1   g11(.a(x18), .O(new_n58_));
  inv1   g12(.a(x08), .O(new_n59_));
  aoi21  g13(.a(new_n58_), .b(new_n59_), .c(x16), .O(new_n60_));
  oai21  g14(.a(new_n58_), .b(x00), .c(new_n60_), .O(new_n61_));
  and2   g15(.a(x19), .b(x17), .O(new_n62_));
  nor2   g16(.a(x19), .b(x17), .O(new_n63_));
  oai21  g17(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  nand2  g18(.a(x27), .b(x14), .O(new_n65_));
  nand3  g19(.a(new_n65_), .b(new_n64_), .c(new_n61_), .O(z09));
  xor2a  g20(.a(new_n63_), .b(x20), .O(new_n67_));
  nand2  g21(.a(new_n67_), .b(x16), .O(new_n68_));
  inv1   g22(.a(x09), .O(new_n69_));
  aoi21  g23(.a(new_n58_), .b(new_n69_), .c(x16), .O(new_n70_));
  oai21  g24(.a(new_n58_), .b(x01), .c(new_n70_), .O(new_n71_));
  nand3  g25(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z10));
  inv1   g26(.a(new_n65_), .O(new_n73_));
  inv1   g27(.a(x20), .O(new_n74_));
  inv1   g28(.a(x21), .O(new_n75_));
  aoi21  g29(.a(new_n63_), .b(new_n74_), .c(new_n75_), .O(new_n76_));
  nor2   g30(.a(x21), .b(x20), .O(new_n77_));
  nand2  g31(.a(new_n77_), .b(new_n63_), .O(new_n78_));
  inv1   g32(.a(new_n78_), .O(new_n79_));
  oai21  g33(.a(new_n79_), .b(new_n76_), .c(x16), .O(new_n80_));
  inv1   g34(.a(x10), .O(new_n81_));
  aoi21  g35(.a(new_n58_), .b(new_n81_), .c(x16), .O(new_n82_));
  oai21  g36(.a(new_n58_), .b(x02), .c(new_n82_), .O(new_n83_));
  aoi21  g37(.a(new_n83_), .b(new_n80_), .c(new_n73_), .O(z11));
  inv1   g38(.a(x16), .O(new_n85_));
  xor2a  g39(.a(new_n78_), .b(x22), .O(new_n86_));
  inv1   g40(.a(x03), .O(new_n87_));
  nand2  g41(.a(x18), .b(new_n87_), .O(new_n88_));
  inv1   g42(.a(x11), .O(new_n89_));
  aoi21  g43(.a(new_n58_), .b(new_n89_), .c(x16), .O(new_n90_));
  aoi21  g44(.a(new_n90_), .b(new_n88_), .c(new_n73_), .O(new_n91_));
  oai21  g45(.a(new_n86_), .b(new_n85_), .c(new_n91_), .O(z12));
  inv1   g46(.a(x22), .O(new_n93_));
  nand3  g47(.a(new_n77_), .b(new_n63_), .c(new_n93_), .O(new_n94_));
  xor2a  g48(.a(new_n94_), .b(x23), .O(new_n95_));
  inv1   g49(.a(x04), .O(new_n96_));
  nand2  g50(.a(x18), .b(new_n96_), .O(new_n97_));
  inv1   g51(.a(x12), .O(new_n98_));
  aoi21  g52(.a(new_n58_), .b(new_n98_), .c(x16), .O(new_n99_));
  aoi21  g53(.a(new_n99_), .b(new_n97_), .c(new_n73_), .O(new_n100_));
  oai21  g54(.a(new_n95_), .b(new_n85_), .c(new_n100_), .O(z13));
  inv1   g55(.a(x23), .O(new_n102_));
  nand4  g56(.a(new_n77_), .b(new_n63_), .c(new_n102_), .d(new_n93_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(x24), .O(new_n104_));
  nor2   g58(.a(x24), .b(x23), .O(new_n105_));
  nand4  g59(.a(new_n105_), .b(new_n77_), .c(new_n63_), .d(new_n93_), .O(new_n106_));
  nand2  g60(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(x16), .O(new_n108_));
  inv1   g62(.a(x05), .O(new_n109_));
  nand2  g63(.a(x18), .b(new_n109_), .O(new_n110_));
  inv1   g64(.a(x13), .O(new_n111_));
  aoi21  g65(.a(new_n58_), .b(new_n111_), .c(x16), .O(new_n112_));
  aoi21  g66(.a(new_n112_), .b(new_n110_), .c(new_n73_), .O(new_n113_));
  nand2  g67(.a(new_n113_), .b(new_n108_), .O(z14));
  nor2   g68(.a(x25), .b(x22), .O(new_n115_));
  nand2  g69(.a(new_n115_), .b(new_n105_), .O(new_n116_));
  inv1   g70(.a(new_n116_), .O(new_n117_));
  aoi22  g71(.a(new_n117_), .b(new_n79_), .c(new_n106_), .d(x25), .O(new_n118_));
  oai21  g72(.a(x18), .b(x16), .c(new_n47_), .O(new_n119_));
  nand3  g73(.a(x18), .b(new_n85_), .c(x06), .O(new_n120_));
  inv1   g74(.a(new_n120_), .O(new_n121_));
  aoi21  g75(.a(new_n119_), .b(x14), .c(new_n121_), .O(new_n122_));
  oai21  g76(.a(new_n118_), .b(new_n85_), .c(new_n122_), .O(z15));
  nand3  g77(.a(new_n117_), .b(new_n79_), .c(x26), .O(new_n124_));
  inv1   g78(.a(x26), .O(new_n125_));
  nand4  g79(.a(new_n115_), .b(new_n105_), .c(new_n77_), .d(new_n63_), .O(new_n126_));
  aoi21  g80(.a(new_n126_), .b(new_n125_), .c(new_n85_), .O(new_n127_));
  nand2  g81(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  inv1   g82(.a(x15), .O(new_n129_));
  aoi21  g83(.a(new_n58_), .b(new_n129_), .c(x16), .O(new_n130_));
  oai21  g84(.a(new_n58_), .b(x07), .c(new_n130_), .O(new_n131_));
  aoi21  g85(.a(new_n131_), .b(new_n128_), .c(new_n73_), .O(z16));
  inv1   g86(.a(x17), .O(new_n133_));
  nand4  g87(.a(new_n77_), .b(new_n125_), .c(x19), .d(new_n133_), .O(new_n134_));
  oai22  g88(.a(new_n134_), .b(new_n116_), .c(new_n47_), .d(new_n133_), .O(new_n135_));
  nand2  g89(.a(new_n135_), .b(x16), .O(new_n136_));
  nand2  g90(.a(new_n136_), .b(new_n65_), .O(z17));
endmodule


