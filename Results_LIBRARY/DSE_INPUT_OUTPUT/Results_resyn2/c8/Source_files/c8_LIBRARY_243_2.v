// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(x27), .b(x23), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  oai22  g003(.a(new_n49_), .b(x19), .c(new_n47_), .d(x08), .O(z00));
  oai22  g004(.a(new_n49_), .b(x20), .c(new_n47_), .d(x09), .O(z01));
  aoi22  g005(.a(new_n48_), .b(x21), .c(x27), .d(x10), .O(z02));
  nor2   g006(.a(x23), .b(x22), .O(new_n53_));
  nor2   g007(.a(new_n53_), .b(x27), .O(new_n54_));
  aoi21  g008(.a(x27), .b(x11), .c(new_n54_), .O(z03));
  inv1   g009(.a(x23), .O(new_n56_));
  nor2   g010(.a(x27), .b(new_n56_), .O(new_n57_));
  aoi21  g011(.a(x27), .b(x12), .c(new_n57_), .O(z04));
  aoi22  g012(.a(new_n48_), .b(x24), .c(x27), .d(x13), .O(z05));
  oai22  g013(.a(new_n49_), .b(x25), .c(new_n47_), .d(x14), .O(z06));
  aoi22  g014(.a(new_n48_), .b(x26), .c(x27), .d(x15), .O(z07));
  inv1   g015(.a(new_n57_), .O(new_n62_));
  inv1   g016(.a(x08), .O(new_n63_));
  inv1   g017(.a(x16), .O(new_n64_));
  oai21  g018(.a(x18), .b(new_n63_), .c(new_n64_), .O(new_n65_));
  aoi21  g019(.a(x18), .b(x00), .c(new_n65_), .O(new_n66_));
  nor2   g020(.a(x19), .b(x17), .O(new_n67_));
  inv1   g021(.a(new_n67_), .O(new_n68_));
  nand2  g022(.a(new_n68_), .b(x16), .O(new_n69_));
  aoi21  g023(.a(x19), .b(x17), .c(new_n69_), .O(new_n70_));
  oai21  g024(.a(new_n70_), .b(new_n66_), .c(new_n62_), .O(z09));
  inv1   g025(.a(x20), .O(new_n72_));
  nor2   g026(.a(new_n67_), .b(new_n72_), .O(new_n73_));
  nor2   g027(.a(new_n68_), .b(x20), .O(new_n74_));
  oai21  g028(.a(new_n74_), .b(new_n73_), .c(x16), .O(new_n75_));
  inv1   g029(.a(x01), .O(new_n76_));
  nand2  g030(.a(x18), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x09), .O(new_n78_));
  inv1   g032(.a(x18), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n78_), .c(x16), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n77_), .c(new_n57_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n75_), .O(z10));
  inv1   g036(.a(new_n74_), .O(new_n83_));
  nor2   g037(.a(x21), .b(x20), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n67_), .O(new_n85_));
  inv1   g039(.a(new_n85_), .O(new_n86_));
  aoi21  g040(.a(new_n83_), .b(x21), .c(new_n86_), .O(new_n87_));
  inv1   g041(.a(x02), .O(new_n88_));
  nand2  g042(.a(x18), .b(new_n88_), .O(new_n89_));
  inv1   g043(.a(x10), .O(new_n90_));
  aoi21  g044(.a(new_n79_), .b(new_n90_), .c(x16), .O(new_n91_));
  aoi21  g045(.a(new_n91_), .b(new_n89_), .c(new_n57_), .O(new_n92_));
  oai21  g046(.a(new_n87_), .b(new_n64_), .c(new_n92_), .O(z11));
  inv1   g047(.a(x22), .O(new_n94_));
  nor2   g048(.a(new_n86_), .b(new_n94_), .O(new_n95_));
  nand3  g049(.a(new_n84_), .b(new_n67_), .c(new_n94_), .O(new_n96_));
  inv1   g050(.a(new_n96_), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n95_), .c(x16), .O(new_n98_));
  inv1   g052(.a(x11), .O(new_n99_));
  aoi21  g053(.a(new_n79_), .b(new_n99_), .c(x16), .O(new_n100_));
  oai21  g054(.a(new_n79_), .b(x03), .c(new_n100_), .O(new_n101_));
  aoi21  g055(.a(new_n101_), .b(new_n98_), .c(new_n57_), .O(z12));
  xor2a  g056(.a(new_n96_), .b(x23), .O(new_n103_));
  inv1   g057(.a(x04), .O(new_n104_));
  nand2  g058(.a(x18), .b(new_n104_), .O(new_n105_));
  inv1   g059(.a(x12), .O(new_n106_));
  aoi21  g060(.a(new_n79_), .b(new_n106_), .c(x16), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n105_), .c(new_n57_), .O(new_n108_));
  oai21  g062(.a(new_n103_), .b(new_n64_), .c(new_n108_), .O(z13));
  oai21  g063(.a(new_n96_), .b(x23), .c(x24), .O(new_n110_));
  nor2   g064(.a(x24), .b(x22), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n86_), .c(new_n56_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(x16), .O(new_n114_));
  inv1   g068(.a(x13), .O(new_n115_));
  aoi21  g069(.a(new_n79_), .b(new_n115_), .c(x16), .O(new_n116_));
  oai21  g070(.a(new_n79_), .b(x05), .c(new_n116_), .O(new_n117_));
  aoi21  g071(.a(new_n117_), .b(new_n114_), .c(new_n57_), .O(z14));
  inv1   g072(.a(x25), .O(new_n119_));
  nand2  g073(.a(new_n112_), .b(new_n119_), .O(new_n120_));
  nand3  g074(.a(new_n111_), .b(new_n86_), .c(x25), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n120_), .c(x16), .O(new_n122_));
  nand2  g076(.a(x25), .b(x16), .O(new_n123_));
  aoi21  g077(.a(new_n123_), .b(x27), .c(new_n56_), .O(new_n124_));
  inv1   g078(.a(x06), .O(new_n125_));
  nand2  g079(.a(x18), .b(new_n125_), .O(new_n126_));
  inv1   g080(.a(x14), .O(new_n127_));
  aoi21  g081(.a(new_n79_), .b(new_n127_), .c(x16), .O(new_n128_));
  aoi21  g082(.a(new_n128_), .b(new_n126_), .c(new_n124_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n122_), .O(z15));
  nor2   g084(.a(x25), .b(x21), .O(new_n131_));
  nand4  g085(.a(new_n131_), .b(new_n111_), .c(new_n67_), .d(new_n72_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(x26), .O(new_n133_));
  nor2   g087(.a(x26), .b(x25), .O(new_n134_));
  nand4  g088(.a(new_n134_), .b(new_n111_), .c(new_n84_), .d(new_n67_), .O(new_n135_));
  aoi21  g089(.a(new_n135_), .b(new_n133_), .c(x23), .O(new_n136_));
  nand3  g090(.a(x27), .b(x26), .c(x23), .O(new_n137_));
  inv1   g091(.a(new_n137_), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  inv1   g093(.a(x07), .O(new_n140_));
  nand2  g094(.a(x18), .b(new_n140_), .O(new_n141_));
  inv1   g095(.a(x15), .O(new_n142_));
  nand2  g096(.a(new_n79_), .b(new_n142_), .O(new_n143_));
  nand4  g097(.a(new_n143_), .b(new_n141_), .c(new_n62_), .d(new_n64_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n139_), .O(z16));
  nand2  g099(.a(x27), .b(x17), .O(new_n146_));
  inv1   g100(.a(x17), .O(new_n147_));
  inv1   g101(.a(x24), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(x19), .c(new_n147_), .O(new_n149_));
  inv1   g103(.a(new_n149_), .O(new_n150_));
  nand4  g104(.a(new_n150_), .b(new_n134_), .c(new_n84_), .d(new_n53_), .O(new_n151_));
  aoi21  g105(.a(new_n151_), .b(new_n146_), .c(new_n64_), .O(z17));
  buf    g106(.a(x27), .O(z08));
endmodule


