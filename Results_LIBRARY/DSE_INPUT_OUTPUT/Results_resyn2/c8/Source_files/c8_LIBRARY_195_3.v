// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n58_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_;
  inv1   g000(.a(x27), .O(new_n47_));
  nor2   g001(.a(new_n47_), .b(x20), .O(new_n48_));
  aoi22  g002(.a(new_n48_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g003(.a(x20), .O(new_n50_));
  nor2   g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x09), .O(new_n52_));
  nor2   g006(.a(x20), .b(new_n52_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(x27), .c(new_n51_), .O(z01));
  aoi22  g008(.a(new_n48_), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  aoi22  g009(.a(new_n48_), .b(x11), .c(new_n47_), .d(x22), .O(z03));
  aoi22  g010(.a(new_n48_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  inv1   g011(.a(new_n48_), .O(new_n58_));
  oai22  g012(.a(new_n58_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g013(.a(new_n48_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g014(.a(new_n58_), .b(x15), .c(x27), .d(x26), .O(z07));
  and2   g015(.a(x19), .b(x17), .O(new_n62_));
  nor2   g016(.a(x19), .b(x17), .O(new_n63_));
  oai21  g017(.a(new_n63_), .b(new_n62_), .c(x16), .O(new_n64_));
  nor2   g018(.a(new_n47_), .b(new_n50_), .O(new_n65_));
  inv1   g019(.a(new_n65_), .O(new_n66_));
  inv1   g020(.a(x18), .O(new_n67_));
  inv1   g021(.a(x08), .O(new_n68_));
  aoi21  g022(.a(new_n67_), .b(new_n68_), .c(x16), .O(new_n69_));
  oai21  g023(.a(new_n67_), .b(x00), .c(new_n69_), .O(new_n70_));
  nand3  g024(.a(new_n70_), .b(new_n66_), .c(new_n64_), .O(z09));
  xor2a  g025(.a(new_n63_), .b(x20), .O(new_n72_));
  nand2  g026(.a(new_n72_), .b(x16), .O(new_n73_));
  aoi21  g027(.a(new_n67_), .b(new_n52_), .c(x16), .O(new_n74_));
  oai21  g028(.a(new_n67_), .b(x01), .c(new_n74_), .O(new_n75_));
  nand3  g029(.a(new_n75_), .b(new_n73_), .c(new_n66_), .O(z10));
  inv1   g030(.a(x21), .O(new_n77_));
  aoi21  g031(.a(new_n63_), .b(new_n50_), .c(new_n77_), .O(new_n78_));
  nand3  g032(.a(new_n63_), .b(new_n77_), .c(new_n50_), .O(new_n79_));
  inv1   g033(.a(new_n79_), .O(new_n80_));
  oai21  g034(.a(new_n80_), .b(new_n78_), .c(x16), .O(new_n81_));
  inv1   g035(.a(x10), .O(new_n82_));
  aoi21  g036(.a(new_n67_), .b(new_n82_), .c(x16), .O(new_n83_));
  oai21  g037(.a(new_n67_), .b(x02), .c(new_n83_), .O(new_n84_));
  aoi21  g038(.a(new_n84_), .b(new_n81_), .c(new_n65_), .O(z11));
  inv1   g039(.a(x22), .O(new_n86_));
  aoi21  g040(.a(new_n63_), .b(new_n77_), .c(new_n86_), .O(new_n87_));
  nor2   g041(.a(x22), .b(x21), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n63_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n50_), .O(new_n90_));
  inv1   g044(.a(x16), .O(new_n91_));
  nand2  g045(.a(new_n47_), .b(x22), .O(new_n92_));
  aoi21  g046(.a(new_n92_), .b(x20), .c(new_n91_), .O(new_n93_));
  oai21  g047(.a(new_n90_), .b(new_n87_), .c(new_n93_), .O(new_n94_));
  nor2   g048(.a(new_n65_), .b(x16), .O(new_n95_));
  inv1   g049(.a(x11), .O(new_n96_));
  nand2  g050(.a(new_n67_), .b(new_n96_), .O(new_n97_));
  inv1   g051(.a(x03), .O(new_n98_));
  nand2  g052(.a(x18), .b(new_n98_), .O(new_n99_));
  nand3  g053(.a(new_n99_), .b(new_n97_), .c(new_n95_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n94_), .O(z12));
  nand2  g055(.a(new_n89_), .b(x23), .O(new_n102_));
  nor2   g056(.a(x23), .b(x22), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n80_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(x16), .O(new_n106_));
  nand2  g060(.a(x23), .b(x16), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n47_), .c(new_n50_), .O(new_n108_));
  inv1   g062(.a(x04), .O(new_n109_));
  nand2  g063(.a(x18), .b(new_n109_), .O(new_n110_));
  inv1   g064(.a(x12), .O(new_n111_));
  aoi21  g065(.a(new_n67_), .b(new_n111_), .c(x16), .O(new_n112_));
  aoi21  g066(.a(new_n112_), .b(new_n110_), .c(new_n108_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n106_), .O(z13));
  nor2   g068(.a(x24), .b(x23), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n88_), .c(new_n63_), .O(new_n116_));
  oai21  g070(.a(new_n89_), .b(x23), .c(x24), .O(new_n117_));
  oai21  g071(.a(new_n116_), .b(x20), .c(new_n117_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x16), .O(new_n119_));
  nand2  g073(.a(x24), .b(x16), .O(new_n120_));
  aoi21  g074(.a(new_n120_), .b(new_n47_), .c(new_n50_), .O(new_n121_));
  inv1   g075(.a(x05), .O(new_n122_));
  nand2  g076(.a(x18), .b(new_n122_), .O(new_n123_));
  inv1   g077(.a(x13), .O(new_n124_));
  aoi21  g078(.a(new_n67_), .b(new_n124_), .c(x16), .O(new_n125_));
  aoi21  g079(.a(new_n125_), .b(new_n123_), .c(new_n121_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n119_), .O(z14));
  nand2  g081(.a(new_n116_), .b(x25), .O(new_n128_));
  nor2   g082(.a(x25), .b(x24), .O(new_n129_));
  nand4  g083(.a(new_n129_), .b(new_n103_), .c(new_n63_), .d(new_n77_), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n128_), .c(new_n50_), .O(new_n131_));
  nand2  g085(.a(new_n47_), .b(x25), .O(new_n132_));
  aoi21  g086(.a(new_n132_), .b(x20), .c(new_n91_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  inv1   g088(.a(x14), .O(new_n135_));
  nand2  g089(.a(new_n67_), .b(new_n135_), .O(new_n136_));
  inv1   g090(.a(x06), .O(new_n137_));
  nand2  g091(.a(x18), .b(new_n137_), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(new_n136_), .c(new_n95_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n134_), .O(z15));
  nand2  g094(.a(new_n130_), .b(x26), .O(new_n141_));
  nor2   g095(.a(x26), .b(x25), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n115_), .c(new_n88_), .d(new_n63_), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(new_n141_), .c(x20), .O(new_n144_));
  nand2  g098(.a(new_n51_), .b(x26), .O(new_n145_));
  inv1   g099(.a(new_n145_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  inv1   g101(.a(x15), .O(new_n148_));
  nand2  g102(.a(new_n67_), .b(new_n148_), .O(new_n149_));
  inv1   g103(.a(x07), .O(new_n150_));
  nand2  g104(.a(x18), .b(new_n150_), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n149_), .c(new_n95_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n147_), .O(z16));
  inv1   g107(.a(x19), .O(new_n154_));
  nor2   g108(.a(new_n154_), .b(x17), .O(new_n155_));
  nor3   g109(.a(x26), .b(x21), .c(x20), .O(new_n156_));
  nand4  g110(.a(new_n156_), .b(new_n155_), .c(new_n129_), .d(new_n103_), .O(new_n157_));
  oai21  g111(.a(x20), .b(x17), .c(x27), .O(new_n158_));
  aoi21  g112(.a(new_n158_), .b(new_n157_), .c(new_n95_), .O(z17));
  buf    g113(.a(x27), .O(z08));
endmodule


