// Benchmark "FAU" written by ABC on Sat Aug  8 23:30:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_;
  inv1   g000(.a(x27), .O(new_n47_));
  nand2  g001(.a(x27), .b(x22), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(z08));
  aoi22  g003(.a(z08), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  aoi22  g004(.a(z08), .b(x09), .c(new_n47_), .d(x20), .O(z01));
  oai22  g005(.a(new_n48_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai21  g006(.a(new_n47_), .b(x11), .c(x22), .O(z03));
  oai22  g007(.a(new_n48_), .b(x12), .c(x27), .d(x23), .O(z04));
  oai22  g008(.a(new_n48_), .b(x13), .c(x27), .d(x24), .O(z05));
  oai22  g009(.a(new_n48_), .b(x14), .c(x27), .d(x25), .O(z06));
  aoi22  g010(.a(z08), .b(x15), .c(new_n47_), .d(x26), .O(z07));
  nor2   g011(.a(new_n47_), .b(x22), .O(new_n58_));
  inv1   g012(.a(x18), .O(new_n59_));
  inv1   g013(.a(x08), .O(new_n60_));
  aoi21  g014(.a(new_n59_), .b(new_n60_), .c(x16), .O(new_n61_));
  oai21  g015(.a(new_n59_), .b(x00), .c(new_n61_), .O(new_n62_));
  inv1   g016(.a(x19), .O(new_n63_));
  nand2  g017(.a(new_n63_), .b(x17), .O(new_n64_));
  inv1   g018(.a(x16), .O(new_n65_));
  nor2   g019(.a(new_n63_), .b(x17), .O(new_n66_));
  nor2   g020(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g021(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  aoi21  g022(.a(new_n68_), .b(new_n62_), .c(new_n58_), .O(z09));
  inv1   g023(.a(x20), .O(new_n70_));
  nor2   g024(.a(x19), .b(x17), .O(new_n71_));
  nor2   g025(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor3   g026(.a(x20), .b(x19), .c(x17), .O(new_n73_));
  oai21  g027(.a(new_n73_), .b(new_n72_), .c(x16), .O(new_n74_));
  inv1   g028(.a(x01), .O(new_n75_));
  nand2  g029(.a(x18), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x09), .O(new_n77_));
  aoi21  g031(.a(new_n59_), .b(new_n77_), .c(x16), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n58_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n74_), .O(z10));
  inv1   g034(.a(new_n58_), .O(new_n81_));
  nand2  g035(.a(x18), .b(x02), .O(new_n82_));
  nand2  g036(.a(new_n59_), .b(x10), .O(new_n83_));
  nand3  g037(.a(new_n83_), .b(new_n82_), .c(new_n65_), .O(new_n84_));
  inv1   g038(.a(x21), .O(new_n85_));
  nor2   g039(.a(new_n73_), .b(new_n85_), .O(new_n86_));
  nor2   g040(.a(x21), .b(x20), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n71_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(x16), .O(new_n89_));
  oai21  g043(.a(new_n89_), .b(new_n86_), .c(new_n84_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n81_), .O(z11));
  nor3   g045(.a(x27), .b(x22), .c(x21), .O(new_n92_));
  aoi22  g046(.a(new_n92_), .b(new_n73_), .c(new_n88_), .d(x22), .O(new_n93_));
  nor2   g047(.a(new_n58_), .b(x16), .O(new_n94_));
  inv1   g048(.a(x11), .O(new_n95_));
  nand2  g049(.a(new_n59_), .b(new_n95_), .O(new_n96_));
  inv1   g050(.a(x03), .O(new_n97_));
  nand2  g051(.a(x18), .b(new_n97_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n96_), .c(new_n94_), .O(new_n99_));
  oai21  g053(.a(new_n93_), .b(new_n65_), .c(new_n99_), .O(z12));
  inv1   g054(.a(x23), .O(new_n101_));
  aoi21  g055(.a(new_n88_), .b(new_n47_), .c(x22), .O(new_n102_));
  nand3  g056(.a(new_n92_), .b(new_n73_), .c(new_n101_), .O(new_n103_));
  oai21  g057(.a(new_n102_), .b(new_n101_), .c(new_n103_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(x16), .O(new_n105_));
  inv1   g059(.a(x12), .O(new_n106_));
  nand2  g060(.a(new_n59_), .b(new_n106_), .O(new_n107_));
  inv1   g061(.a(x04), .O(new_n108_));
  nand2  g062(.a(x18), .b(new_n108_), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n107_), .c(new_n94_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n105_), .O(z13));
  inv1   g065(.a(x22), .O(new_n112_));
  nor2   g066(.a(x24), .b(x23), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n87_), .c(new_n71_), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n65_), .c(new_n47_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n112_), .O(new_n116_));
  inv1   g070(.a(x13), .O(new_n117_));
  aoi21  g071(.a(new_n59_), .b(new_n117_), .c(x16), .O(new_n118_));
  oai21  g072(.a(new_n59_), .b(x05), .c(new_n118_), .O(new_n119_));
  nand3  g073(.a(new_n103_), .b(x24), .c(x16), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n119_), .c(new_n116_), .O(z14));
  inv1   g075(.a(x25), .O(new_n122_));
  nand2  g076(.a(new_n114_), .b(new_n47_), .O(new_n123_));
  aoi21  g077(.a(new_n123_), .b(new_n112_), .c(new_n122_), .O(new_n124_));
  nor2   g078(.a(x23), .b(x22), .O(new_n125_));
  nor2   g079(.a(x25), .b(x24), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n125_), .c(new_n47_), .O(new_n127_));
  nor2   g081(.a(new_n127_), .b(new_n88_), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n124_), .c(x16), .O(new_n129_));
  inv1   g083(.a(x14), .O(new_n130_));
  nand2  g084(.a(new_n59_), .b(new_n130_), .O(new_n131_));
  inv1   g085(.a(x06), .O(new_n132_));
  nand2  g086(.a(x18), .b(new_n132_), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n131_), .c(new_n94_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n129_), .O(z15));
  nand2  g089(.a(new_n73_), .b(new_n101_), .O(new_n136_));
  nor3   g090(.a(x26), .b(x25), .c(x24), .O(new_n137_));
  nand3  g091(.a(new_n137_), .b(new_n85_), .c(x16), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n136_), .c(new_n47_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n112_), .O(new_n140_));
  inv1   g094(.a(new_n128_), .O(new_n141_));
  nor2   g095(.a(new_n59_), .b(x07), .O(new_n142_));
  oai21  g096(.a(x18), .b(x15), .c(new_n65_), .O(new_n143_));
  nor2   g097(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  and2   g098(.a(x26), .b(x16), .O(new_n145_));
  aoi21  g099(.a(new_n145_), .b(new_n141_), .c(new_n144_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n140_), .O(z16));
  oai21  g101(.a(new_n112_), .b(x17), .c(x27), .O(new_n148_));
  nand4  g102(.a(new_n137_), .b(new_n125_), .c(new_n87_), .d(new_n66_), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(new_n148_), .c(new_n94_), .O(z17));
endmodule


