// Benchmark "FAU" written by ABC on Sat Aug  8 23:29:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_;
  inv1   g00(.a(x27), .O(new_n47_));
  inv1   g01(.a(x16), .O(new_n48_));
  nor2   g02(.a(new_n47_), .b(new_n48_), .O(new_n49_));
  aoi22  g03(.a(new_n49_), .b(x08), .c(new_n47_), .d(x19), .O(z00));
  inv1   g04(.a(new_n49_), .O(new_n51_));
  oai22  g05(.a(new_n51_), .b(x09), .c(x27), .d(x20), .O(z01));
  aoi22  g06(.a(new_n49_), .b(x10), .c(new_n47_), .d(x21), .O(z02));
  oai22  g07(.a(new_n51_), .b(x11), .c(x27), .d(x22), .O(z03));
  aoi22  g08(.a(new_n49_), .b(x12), .c(new_n47_), .d(x23), .O(z04));
  oai22  g09(.a(new_n51_), .b(x13), .c(x27), .d(x24), .O(z05));
  aoi22  g10(.a(new_n49_), .b(x14), .c(new_n47_), .d(x25), .O(z06));
  oai22  g11(.a(new_n51_), .b(x15), .c(x27), .d(x26), .O(z07));
  nand2  g12(.a(x18), .b(x00), .O(new_n59_));
  nor2   g13(.a(x27), .b(x16), .O(new_n60_));
  inv1   g14(.a(x18), .O(new_n61_));
  nand2  g15(.a(new_n61_), .b(x08), .O(new_n62_));
  nand3  g16(.a(new_n62_), .b(new_n60_), .c(new_n59_), .O(new_n63_));
  inv1   g17(.a(new_n63_), .O(new_n64_));
  nand2  g18(.a(x19), .b(x17), .O(new_n65_));
  nor2   g19(.a(x19), .b(x17), .O(new_n66_));
  nor2   g20(.a(new_n66_), .b(new_n48_), .O(new_n67_));
  aoi21  g21(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(z09));
  nand2  g22(.a(x18), .b(x01), .O(new_n69_));
  nand2  g23(.a(new_n61_), .b(x09), .O(new_n70_));
  nand3  g24(.a(new_n70_), .b(new_n69_), .c(new_n60_), .O(new_n71_));
  oai21  g25(.a(x19), .b(x17), .c(x20), .O(new_n72_));
  inv1   g26(.a(x20), .O(new_n73_));
  nand2  g27(.a(new_n66_), .b(new_n73_), .O(new_n74_));
  nand3  g28(.a(new_n74_), .b(new_n72_), .c(x16), .O(new_n75_));
  and2   g29(.a(new_n75_), .b(new_n71_), .O(z10));
  nor2   g30(.a(x21), .b(x20), .O(new_n77_));
  aoi22  g31(.a(new_n77_), .b(new_n66_), .c(new_n74_), .d(x21), .O(new_n78_));
  nor2   g32(.a(new_n61_), .b(x02), .O(new_n79_));
  oai21  g33(.a(x18), .b(x10), .c(new_n60_), .O(new_n80_));
  oai22  g34(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n48_), .O(z11));
  nand2  g35(.a(new_n77_), .b(new_n66_), .O(new_n82_));
  xor2a  g36(.a(new_n82_), .b(x22), .O(new_n83_));
  nor2   g37(.a(new_n61_), .b(x03), .O(new_n84_));
  oai21  g38(.a(x18), .b(x11), .c(new_n60_), .O(new_n85_));
  oai22  g39(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n48_), .O(z12));
  inv1   g40(.a(x22), .O(new_n87_));
  nand3  g41(.a(new_n77_), .b(new_n66_), .c(new_n87_), .O(new_n88_));
  nand2  g42(.a(new_n88_), .b(x23), .O(new_n89_));
  nor2   g43(.a(x23), .b(x22), .O(new_n90_));
  nand3  g44(.a(new_n90_), .b(new_n77_), .c(new_n66_), .O(new_n91_));
  nand2  g45(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g46(.a(new_n92_), .b(x16), .O(new_n93_));
  inv1   g47(.a(x04), .O(new_n94_));
  nand2  g48(.a(x18), .b(new_n94_), .O(new_n95_));
  inv1   g49(.a(x12), .O(new_n96_));
  nand2  g50(.a(new_n61_), .b(new_n96_), .O(new_n97_));
  nand3  g51(.a(new_n97_), .b(new_n95_), .c(new_n60_), .O(new_n98_));
  nand2  g52(.a(new_n98_), .b(new_n93_), .O(z13));
  nand2  g53(.a(new_n91_), .b(x24), .O(new_n100_));
  nor2   g54(.a(x24), .b(x21), .O(new_n101_));
  nand4  g55(.a(new_n101_), .b(new_n90_), .c(new_n66_), .d(new_n73_), .O(new_n102_));
  nand2  g56(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g57(.a(new_n103_), .b(x16), .O(new_n104_));
  nand2  g58(.a(x18), .b(x05), .O(new_n105_));
  nand2  g59(.a(new_n61_), .b(x13), .O(new_n106_));
  nand3  g60(.a(new_n106_), .b(new_n105_), .c(new_n47_), .O(new_n107_));
  nand2  g61(.a(new_n107_), .b(new_n48_), .O(new_n108_));
  nand2  g62(.a(new_n108_), .b(new_n104_), .O(z14));
  nand2  g63(.a(x18), .b(x06), .O(new_n110_));
  nand2  g64(.a(new_n61_), .b(x14), .O(new_n111_));
  nand3  g65(.a(new_n111_), .b(new_n110_), .c(new_n60_), .O(new_n112_));
  inv1   g66(.a(new_n112_), .O(new_n113_));
  nand2  g67(.a(new_n102_), .b(x25), .O(new_n114_));
  nor2   g68(.a(x25), .b(x24), .O(new_n115_));
  nand4  g69(.a(new_n115_), .b(new_n90_), .c(new_n77_), .d(new_n66_), .O(new_n116_));
  and2   g70(.a(new_n116_), .b(x16), .O(new_n117_));
  aoi21  g71(.a(new_n117_), .b(new_n114_), .c(new_n113_), .O(z15));
  inv1   g72(.a(x26), .O(new_n119_));
  inv1   g73(.a(new_n116_), .O(new_n120_));
  nand2  g74(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  aoi21  g75(.a(new_n116_), .b(x26), .c(new_n48_), .O(new_n122_));
  nand2  g76(.a(x18), .b(x07), .O(new_n123_));
  nand2  g77(.a(new_n61_), .b(x15), .O(new_n124_));
  nand3  g78(.a(new_n124_), .b(new_n123_), .c(new_n60_), .O(new_n125_));
  inv1   g79(.a(new_n125_), .O(new_n126_));
  aoi21  g80(.a(new_n122_), .b(new_n121_), .c(new_n126_), .O(z16));
  nand2  g81(.a(new_n115_), .b(new_n90_), .O(new_n128_));
  nor2   g82(.a(x17), .b(new_n48_), .O(new_n129_));
  nand4  g83(.a(new_n129_), .b(new_n77_), .c(new_n119_), .d(x19), .O(new_n130_));
  oai22  g84(.a(new_n130_), .b(new_n128_), .c(new_n129_), .d(new_n47_), .O(z17));
  buf    g85(.a(x27), .O(z08));
endmodule


