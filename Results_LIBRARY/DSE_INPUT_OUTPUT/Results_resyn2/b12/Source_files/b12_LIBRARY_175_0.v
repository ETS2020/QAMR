// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand3  g03(.a(x07), .b(new_n27_), .c(x01), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  nor2   g07(.a(new_n27_), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n25_), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  nand4  g11(.a(x07), .b(x03), .c(new_n27_), .d(x01), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  oai21  g14(.a(x04), .b(x03), .c(x02), .O(new_n39_));
  oai21  g15(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  inv1   g17(.a(x07), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n27_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n41_), .O(z1));
  inv1   g20(.a(x03), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n31_), .c(x02), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(new_n27_), .c(new_n42_), .O(new_n48_));
  oai21  g24(.a(new_n46_), .b(x00), .c(new_n48_), .O(z2));
  nand2  g25(.a(x12), .b(new_n35_), .O(new_n50_));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n50_), .c(new_n43_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(z3));
  nand2  g29(.a(new_n51_), .b(new_n43_), .O(new_n54_));
  nand2  g30(.a(x08), .b(x00), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(new_n56_));
  inv1   g32(.a(x09), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x07), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(z4));
  nor2   g35(.a(x10), .b(x07), .O(new_n60_));
  inv1   g36(.a(x13), .O(new_n61_));
  nor2   g37(.a(new_n61_), .b(x12), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(new_n60_), .c(x02), .d(new_n35_), .O(z5));
  nor2   g39(.a(new_n31_), .b(x00), .O(new_n64_));
  nand3  g40(.a(new_n60_), .b(new_n55_), .c(x02), .O(new_n65_));
  aoi21  g41(.a(new_n64_), .b(x14), .c(new_n65_), .O(z6));
  nand3  g42(.a(new_n60_), .b(x08), .c(x02), .O(z7));
  nand4  g43(.a(new_n60_), .b(new_n57_), .c(x02), .d(x00), .O(z8));
endmodule


