// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n79_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  nand2  g02(.a(x13), .b(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(x04), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(new_n27_), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x04), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(new_n30_), .c(x01), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g10(.a(x13), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x02), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n37_), .c(x02), .O(new_n41_));
  inv1   g17(.a(x02), .O(new_n42_));
  nor2   g18(.a(x13), .b(new_n42_), .O(new_n43_));
  aoi21  g19(.a(new_n26_), .b(x03), .c(new_n43_), .O(new_n44_));
  aoi22  g20(.a(new_n44_), .b(new_n41_), .c(new_n36_), .d(x00), .O(z1));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  nand2  g23(.a(new_n36_), .b(x00), .O(new_n48_));
  aoi21  g24(.a(new_n47_), .b(new_n46_), .c(new_n48_), .O(new_n49_));
  nor2   g25(.a(x10), .b(x07), .O(new_n50_));
  nor2   g26(.a(new_n50_), .b(x03), .O(new_n51_));
  inv1   g27(.a(x10), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n51_), .c(new_n25_), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n42_), .c(new_n49_), .O(z2));
  and2   g32(.a(x11), .b(x07), .O(new_n57_));
  inv1   g33(.a(x12), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x00), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n57_), .c(new_n36_), .O(z3));
  nand4  g36(.a(x09), .b(x08), .c(new_n46_), .d(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n57_), .c(new_n36_), .O(z4));
  inv1   g39(.a(x00), .O(new_n64_));
  nand4  g40(.a(new_n50_), .b(x13), .c(new_n58_), .d(new_n64_), .O(z5));
  nand3  g41(.a(x14), .b(x01), .c(new_n64_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n30_), .O(new_n67_));
  nand2  g43(.a(x09), .b(new_n38_), .O(new_n68_));
  inv1   g44(.a(x09), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x03), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n68_), .c(new_n25_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n42_), .O(new_n72_));
  oai21  g48(.a(new_n47_), .b(new_n64_), .c(new_n50_), .O(new_n73_));
  aoi21  g49(.a(new_n72_), .b(new_n67_), .c(new_n73_), .O(z6));
  oai21  g50(.a(new_n35_), .b(new_n47_), .c(x02), .O(new_n75_));
  oai21  g51(.a(new_n38_), .b(x01), .c(new_n47_), .O(new_n76_));
  nand4  g52(.a(x09), .b(x03), .c(new_n42_), .d(new_n25_), .O(new_n77_));
  nand4  g53(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n50_), .O(z7));
  oai22  g54(.a(new_n77_), .b(x12), .c(x09), .d(new_n64_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n50_), .c(new_n43_), .O(z8));
endmodule


