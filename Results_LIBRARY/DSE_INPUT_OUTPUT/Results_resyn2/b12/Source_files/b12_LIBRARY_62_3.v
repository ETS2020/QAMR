// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n71_;
  nand2  g00(.a(x05), .b(x01), .O(new_n25_));
  nor2   g01(.a(x03), .b(x01), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x04), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x07), .O(new_n28_));
  nand2  g04(.a(x03), .b(x01), .O(new_n29_));
  aoi21  g05(.a(new_n28_), .b(x02), .c(new_n29_), .O(new_n30_));
  aoi21  g06(.a(new_n27_), .b(x02), .c(new_n30_), .O(new_n31_));
  nand2  g07(.a(x07), .b(x02), .O(new_n32_));
  oai21  g08(.a(new_n31_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x02), .O(new_n34_));
  nand3  g10(.a(x03), .b(new_n34_), .c(x01), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  inv1   g13(.a(x04), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand4  g15(.a(new_n39_), .b(new_n36_), .c(new_n28_), .d(x02), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n35_), .c(x00), .O(z1));
  inv1   g17(.a(x09), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g20(.a(x03), .b(x00), .c(x07), .O(new_n45_));
  aoi21  g21(.a(x10), .b(x03), .c(x01), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  inv1   g23(.a(x00), .O(new_n48_));
  inv1   g24(.a(x08), .O(new_n49_));
  nor3   g25(.a(new_n49_), .b(x07), .c(new_n48_), .O(new_n50_));
  aoi21  g26(.a(new_n47_), .b(new_n34_), .c(new_n50_), .O(z2));
  and2   g27(.a(x11), .b(x07), .O(new_n52_));
  inv1   g28(.a(x12), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(x00), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n52_), .c(new_n32_), .O(z3));
  aoi22  g31(.a(new_n52_), .b(new_n34_), .c(new_n50_), .d(x09), .O(z4));
  nand4  g32(.a(new_n43_), .b(x13), .c(new_n53_), .d(new_n48_), .O(z5));
  inv1   g33(.a(x01), .O(new_n58_));
  nand2  g34(.a(new_n42_), .b(x03), .O(new_n59_));
  nand2  g35(.a(x09), .b(new_n37_), .O(new_n60_));
  nand3  g36(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n28_), .O(new_n62_));
  nand4  g38(.a(x14), .b(x02), .c(x01), .d(new_n48_), .O(new_n63_));
  aoi21  g39(.a(x08), .b(x00), .c(x10), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n63_), .c(x07), .O(new_n65_));
  aoi21  g41(.a(new_n62_), .b(new_n34_), .c(new_n65_), .O(z6));
  nand4  g42(.a(x09), .b(x03), .c(new_n34_), .d(new_n58_), .O(new_n67_));
  nand3  g43(.a(x03), .b(new_n34_), .c(new_n58_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n49_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n67_), .c(new_n43_), .O(z7));
  oai22  g46(.a(new_n67_), .b(x12), .c(x09), .d(new_n48_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n43_), .O(z8));
endmodule


