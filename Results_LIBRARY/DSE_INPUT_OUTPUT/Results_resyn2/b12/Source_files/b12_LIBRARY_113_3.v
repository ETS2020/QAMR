// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  inv1   g03(.a(x14), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(new_n27_), .c(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand4  g09(.a(new_n33_), .b(new_n28_), .c(x02), .d(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  nand2  g11(.a(x14), .b(x02), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(x00), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  inv1   g15(.a(x03), .O(new_n40_));
  nand2  g16(.a(new_n27_), .b(new_n40_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n39_), .c(x02), .O(new_n42_));
  inv1   g18(.a(x01), .O(new_n43_));
  nor2   g19(.a(x02), .b(new_n43_), .O(new_n44_));
  inv1   g20(.a(new_n36_), .O(new_n45_));
  aoi21  g21(.a(new_n44_), .b(x03), .c(new_n45_), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n42_), .c(new_n38_), .O(z1));
  inv1   g23(.a(x07), .O(new_n48_));
  inv1   g24(.a(x08), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n48_), .c(new_n37_), .O(new_n50_));
  inv1   g26(.a(x09), .O(new_n51_));
  nor2   g27(.a(x10), .b(x07), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  oai21  g29(.a(x10), .b(x07), .c(x03), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n53_), .c(new_n43_), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n25_), .c(new_n50_), .O(z2));
  inv1   g32(.a(x12), .O(new_n57_));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  oai21  g34(.a(new_n57_), .b(x00), .c(new_n58_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n36_), .O(z3));
  nand2  g36(.a(x08), .b(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  nor2   g38(.a(new_n51_), .b(x07), .O(new_n63_));
  nand2  g39(.a(new_n58_), .b(new_n36_), .O(new_n64_));
  aoi21  g40(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(z4));
  nor2   g41(.a(x07), .b(x00), .O(new_n66_));
  nor2   g42(.a(x12), .b(x10), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(x13), .O(new_n68_));
  and2   g44(.a(new_n68_), .b(new_n36_), .O(z5));
  xnor2a g45(.a(x09), .b(x03), .O(new_n70_));
  nor2   g46(.a(x02), .b(x01), .O(new_n71_));
  nand3  g47(.a(new_n61_), .b(new_n52_), .c(new_n36_), .O(new_n72_));
  aoi21  g48(.a(new_n71_), .b(new_n70_), .c(new_n72_), .O(z6));
  nand3  g49(.a(new_n71_), .b(x09), .c(x03), .O(new_n74_));
  oai21  g50(.a(new_n40_), .b(x01), .c(new_n49_), .O(new_n75_));
  oai21  g51(.a(x14), .b(new_n49_), .c(x02), .O(new_n76_));
  nand4  g52(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n52_), .O(z7));
  oai22  g53(.a(new_n74_), .b(x12), .c(new_n37_), .d(x09), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n52_), .O(z8));
endmodule


