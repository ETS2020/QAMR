// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nor2   g03(.a(x02), .b(x01), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  aoi21  g06(.a(x04), .b(x02), .c(new_n30_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  nand2  g08(.a(x12), .b(x02), .O(new_n33_));
  oai21  g09(.a(new_n32_), .b(x00), .c(new_n33_), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  nand4  g11(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n36_));
  nand2  g12(.a(x04), .b(x02), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n30_), .O(new_n38_));
  nand4  g14(.a(new_n38_), .b(new_n36_), .c(new_n29_), .d(new_n35_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n33_), .O(z1));
  inv1   g16(.a(x07), .O(new_n41_));
  inv1   g17(.a(x08), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n41_), .c(new_n35_), .O(new_n43_));
  inv1   g19(.a(x12), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x02), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n28_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n45_), .c(new_n43_), .O(z2));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  oai21  g28(.a(new_n44_), .b(x00), .c(new_n52_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n33_), .O(z3));
  nor2   g30(.a(new_n42_), .b(new_n35_), .O(new_n55_));
  nor2   g31(.a(new_n46_), .b(x07), .O(new_n56_));
  nand2  g32(.a(new_n52_), .b(new_n33_), .O(new_n57_));
  aoi21  g33(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(z4));
  nor2   g34(.a(x12), .b(x10), .O(new_n59_));
  nand4  g35(.a(new_n59_), .b(x13), .c(new_n41_), .d(new_n35_), .O(new_n60_));
  and2   g36(.a(new_n60_), .b(new_n33_), .O(z5));
  inv1   g37(.a(x02), .O(new_n62_));
  inv1   g38(.a(x01), .O(new_n63_));
  nand2  g39(.a(x09), .b(new_n30_), .O(new_n64_));
  nand2  g40(.a(new_n46_), .b(x03), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n62_), .O(new_n67_));
  nand3  g43(.a(x14), .b(x01), .c(new_n35_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n44_), .c(x02), .O(new_n69_));
  oai21  g45(.a(new_n42_), .b(new_n35_), .c(new_n47_), .O(new_n70_));
  aoi21  g46(.a(new_n69_), .b(new_n67_), .c(new_n70_), .O(z6));
  oai21  g47(.a(new_n30_), .b(x01), .c(new_n42_), .O(new_n72_));
  oai21  g48(.a(x12), .b(new_n42_), .c(x02), .O(new_n73_));
  nand3  g49(.a(new_n28_), .b(x09), .c(x03), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n47_), .O(z7));
  inv1   g51(.a(new_n33_), .O(new_n76_));
  nand2  g52(.a(new_n46_), .b(x00), .O(new_n77_));
  oai21  g53(.a(new_n74_), .b(x12), .c(new_n77_), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n47_), .c(new_n76_), .O(z8));
endmodule


