// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n77_;
  inv1   g00(.a(x03), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x00), .O(new_n27_));
  inv1   g03(.a(new_n27_), .O(new_n28_));
  nand2  g04(.a(x05), .b(x02), .O(new_n29_));
  oai21  g05(.a(new_n25_), .b(x02), .c(new_n29_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x01), .O(new_n31_));
  inv1   g07(.a(x04), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(new_n25_), .c(x02), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n31_), .c(new_n28_), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x02), .O(new_n36_));
  inv1   g12(.a(x02), .O(new_n37_));
  nand3  g13(.a(x03), .b(new_n37_), .c(x01), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n36_), .c(new_n26_), .O(new_n39_));
  and2   g15(.a(new_n39_), .b(new_n27_), .O(z1));
  inv1   g16(.a(x01), .O(new_n41_));
  inv1   g17(.a(x09), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g20(.a(x10), .b(x07), .c(x03), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n44_), .c(new_n41_), .O(new_n46_));
  oai21  g22(.a(x08), .b(x07), .c(x00), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n26_), .O(new_n48_));
  aoi21  g24(.a(new_n46_), .b(new_n37_), .c(new_n48_), .O(z2));
  inv1   g25(.a(x00), .O(new_n50_));
  nand2  g26(.a(x12), .b(new_n50_), .O(new_n51_));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n51_), .c(new_n26_), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(z3));
  nand2  g30(.a(new_n52_), .b(new_n26_), .O(new_n55_));
  inv1   g31(.a(x08), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  nor2   g33(.a(new_n42_), .b(x07), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n57_), .c(new_n55_), .O(z4));
  nor2   g35(.a(x12), .b(x10), .O(new_n60_));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(new_n60_), .c(new_n26_), .d(x13), .O(z5));
  nand3  g38(.a(x14), .b(x01), .c(new_n50_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(x02), .O(new_n64_));
  nand2  g40(.a(x09), .b(new_n25_), .O(new_n65_));
  oai21  g41(.a(x09), .b(new_n25_), .c(new_n41_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n37_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(new_n68_));
  oai21  g44(.a(new_n56_), .b(new_n50_), .c(new_n43_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n26_), .O(z6));
  nand4  g48(.a(x09), .b(x03), .c(new_n37_), .d(new_n41_), .O(new_n73_));
  oai21  g49(.a(new_n25_), .b(x01), .c(new_n56_), .O(new_n74_));
  oai21  g50(.a(new_n56_), .b(new_n25_), .c(x02), .O(new_n75_));
  nand4  g51(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n43_), .O(z7));
  oai22  g52(.a(new_n73_), .b(x12), .c(new_n27_), .d(x09), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n43_), .O(z8));
endmodule


