// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  nor2   g04(.a(x08), .b(x02), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n28_), .c(x03), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n27_), .c(x00), .O(z0));
  inv1   g09(.a(new_n29_), .O(new_n34_));
  inv1   g10(.a(x00), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(new_n37_));
  oai21  g13(.a(x04), .b(x03), .c(x02), .O(new_n38_));
  inv1   g14(.a(x02), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n39_), .c(x01), .O(new_n40_));
  oai21  g16(.a(new_n38_), .b(new_n37_), .c(new_n40_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n35_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n34_), .O(z1));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n44_), .c(new_n35_), .O(new_n46_));
  oai21  g22(.a(x10), .b(x07), .c(x03), .O(new_n47_));
  inv1   g23(.a(x09), .O(new_n48_));
  nor2   g24(.a(x10), .b(x07), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nor2   g26(.a(x01), .b(x00), .O(new_n51_));
  nand4  g27(.a(new_n51_), .b(new_n50_), .c(new_n47_), .d(x08), .O(new_n52_));
  oai21  g28(.a(new_n46_), .b(new_n39_), .c(new_n52_), .O(z2));
  nand2  g29(.a(x12), .b(new_n35_), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n34_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  nand2  g33(.a(new_n55_), .b(new_n34_), .O(new_n58_));
  nand2  g34(.a(x08), .b(x00), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(new_n60_));
  nor2   g36(.a(new_n48_), .b(x07), .O(new_n61_));
  aoi21  g37(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(z4));
  nor2   g38(.a(x12), .b(x10), .O(new_n63_));
  nor2   g39(.a(x07), .b(x00), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(new_n63_), .c(new_n34_), .d(x13), .O(z5));
  xor2a  g41(.a(x09), .b(x03), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(x01), .c(x08), .O(new_n67_));
  nand4  g43(.a(x14), .b(x02), .c(x01), .d(new_n35_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n59_), .c(new_n49_), .O(new_n69_));
  aoi21  g45(.a(new_n67_), .b(new_n39_), .c(new_n69_), .O(z6));
  and2   g46(.a(x09), .b(x03), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n30_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n49_), .c(x08), .O(z7));
  inv1   g49(.a(x12), .O(new_n74_));
  nand4  g50(.a(new_n71_), .b(new_n30_), .c(new_n74_), .d(x08), .O(new_n75_));
  nand3  g51(.a(new_n34_), .b(new_n48_), .c(x00), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n49_), .O(z8));
endmodule


