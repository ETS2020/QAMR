// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x07), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  nand3  g03(.a(x03), .b(x01), .c(new_n27_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  nand2  g06(.a(x05), .b(x01), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(new_n27_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n30_), .O(z0));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  inv1   g13(.a(x04), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n36_), .c(x02), .O(new_n40_));
  nand4  g16(.a(new_n26_), .b(x03), .c(new_n25_), .d(x01), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n40_), .c(x00), .O(z1));
  oai21  g18(.a(new_n25_), .b(new_n27_), .c(x07), .O(new_n43_));
  oai21  g19(.a(x08), .b(x07), .c(x00), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  aoi21  g24(.a(x10), .b(x03), .c(x01), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n48_), .c(x02), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n45_), .c(new_n43_), .O(z2));
  and2   g27(.a(x11), .b(x07), .O(new_n52_));
  inv1   g28(.a(x12), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(x00), .O(new_n54_));
  nand2  g30(.a(x07), .b(new_n25_), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(new_n52_), .c(new_n55_), .O(z3));
  nor2   g32(.a(x07), .b(new_n27_), .O(new_n57_));
  inv1   g33(.a(x08), .O(new_n58_));
  nor2   g34(.a(new_n46_), .b(new_n58_), .O(new_n59_));
  aoi22  g35(.a(new_n59_), .b(new_n57_), .c(new_n52_), .d(x02), .O(z4));
  nor2   g36(.a(x10), .b(x07), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(x13), .c(new_n53_), .d(new_n27_), .O(z5));
  nand2  g38(.a(new_n46_), .b(x03), .O(new_n63_));
  nor2   g39(.a(x02), .b(x01), .O(new_n64_));
  nand2  g40(.a(x09), .b(new_n37_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  inv1   g42(.a(new_n66_), .O(new_n67_));
  inv1   g43(.a(x01), .O(new_n68_));
  oai21  g44(.a(x02), .b(new_n68_), .c(x07), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n27_), .O(new_n70_));
  nand4  g46(.a(new_n70_), .b(new_n69_), .c(new_n44_), .d(new_n47_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n67_), .c(new_n55_), .O(z6));
  nand2  g48(.a(new_n64_), .b(x03), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n58_), .O(new_n74_));
  nand3  g50(.a(new_n64_), .b(x09), .c(x03), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n74_), .c(new_n61_), .O(z7));
  oai21  g52(.a(new_n73_), .b(x12), .c(x09), .O(new_n77_));
  nand2  g53(.a(new_n46_), .b(new_n27_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n77_), .c(new_n61_), .O(z8));
endmodule


