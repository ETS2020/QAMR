// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x02), .b(new_n25_), .O(new_n26_));
  nand2  g02(.a(x05), .b(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  oai21  g04(.a(x03), .b(x01), .c(new_n28_), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n27_), .c(new_n26_), .O(z0));
  nand3  g06(.a(x06), .b(x05), .c(x01), .O(new_n31_));
  inv1   g07(.a(x03), .O(new_n32_));
  aoi21  g08(.a(new_n28_), .b(new_n32_), .c(new_n26_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(z1));
  nand2  g11(.a(x07), .b(new_n32_), .O(new_n36_));
  inv1   g12(.a(x07), .O(new_n37_));
  inv1   g13(.a(x10), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(x09), .c(new_n37_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n36_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n25_), .O(new_n41_));
  nor2   g17(.a(x08), .b(x07), .O(new_n42_));
  inv1   g18(.a(x02), .O(new_n43_));
  oai21  g19(.a(new_n38_), .b(x03), .c(new_n43_), .O(new_n44_));
  oai21  g20(.a(new_n42_), .b(new_n25_), .c(new_n44_), .O(new_n45_));
  inv1   g21(.a(x01), .O(new_n46_));
  nor2   g22(.a(x02), .b(new_n46_), .O(new_n47_));
  inv1   g23(.a(x09), .O(new_n48_));
  nor2   g24(.a(x10), .b(new_n48_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n42_), .c(new_n47_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n45_), .c(new_n41_), .O(z2));
  nand2  g27(.a(new_n43_), .b(x01), .O(new_n52_));
  inv1   g28(.a(x12), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  oai21  g30(.a(new_n53_), .b(x00), .c(new_n54_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n52_), .O(z3));
  nor2   g32(.a(new_n48_), .b(x07), .O(new_n57_));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  nand2  g35(.a(new_n54_), .b(new_n52_), .O(new_n60_));
  aoi21  g36(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z4));
  nor2   g37(.a(x12), .b(x10), .O(new_n62_));
  nor2   g38(.a(x07), .b(x00), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(new_n52_), .d(x13), .O(z5));
  nand2  g40(.a(x14), .b(new_n25_), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(x02), .c(new_n46_), .O(new_n66_));
  xnor2a g42(.a(x09), .b(x03), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n43_), .O(new_n68_));
  nand3  g44(.a(new_n58_), .b(new_n38_), .c(new_n37_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nor2   g47(.a(new_n71_), .b(new_n66_), .O(z6));
  nor2   g48(.a(x10), .b(x07), .O(new_n73_));
  oai21  g49(.a(new_n48_), .b(new_n32_), .c(new_n46_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n43_), .O(new_n75_));
  inv1   g51(.a(x08), .O(new_n76_));
  nand2  g52(.a(x03), .b(new_n43_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n75_), .c(new_n73_), .O(z7));
  nand2  g55(.a(new_n48_), .b(x00), .O(new_n80_));
  nor2   g56(.a(new_n80_), .b(new_n47_), .O(new_n81_));
  nand3  g57(.a(new_n53_), .b(x09), .c(new_n46_), .O(new_n82_));
  nor2   g58(.a(new_n82_), .b(new_n77_), .O(new_n83_));
  oai21  g59(.a(new_n83_), .b(new_n81_), .c(new_n73_), .O(z8));
endmodule


