// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(x03), .b(x01), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x00), .c(x12), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nor2   g04(.a(new_n25_), .b(x00), .O(new_n29_));
  nand2  g05(.a(x05), .b(x01), .O(new_n30_));
  nor2   g06(.a(x03), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n28_), .O(z0));
  inv1   g10(.a(x12), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(x02), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(x03), .c(x01), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n38_), .c(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n37_), .c(x00), .O(z1));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(x03), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n47_), .c(new_n44_), .O(new_n52_));
  inv1   g28(.a(x00), .O(new_n53_));
  nor2   g29(.a(x08), .b(x07), .O(new_n54_));
  nor2   g30(.a(x12), .b(x02), .O(new_n55_));
  nor3   g31(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g32(.a(new_n52_), .b(new_n36_), .c(new_n56_), .O(z2));
  inv1   g33(.a(new_n55_), .O(new_n58_));
  inv1   g34(.a(x11), .O(new_n59_));
  nor2   g35(.a(new_n59_), .b(new_n48_), .O(new_n60_));
  aoi22  g36(.a(new_n60_), .b(new_n58_), .c(x12), .d(new_n53_), .O(z3));
  nand2  g37(.a(x08), .b(x00), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  nor2   g39(.a(new_n45_), .b(x07), .O(new_n64_));
  oai21  g40(.a(new_n59_), .b(new_n48_), .c(new_n58_), .O(new_n65_));
  aoi21  g41(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(z4));
  nand4  g42(.a(new_n46_), .b(new_n29_), .c(x13), .d(new_n35_), .O(z5));
  nand2  g43(.a(new_n45_), .b(x03), .O(new_n68_));
  nand2  g44(.a(x09), .b(new_n39_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n44_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x12), .O(new_n71_));
  nand4  g47(.a(x14), .b(x02), .c(x01), .d(new_n53_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n62_), .c(new_n46_), .O(new_n73_));
  aoi21  g49(.a(new_n71_), .b(new_n25_), .c(new_n73_), .O(z6));
  nand3  g50(.a(x09), .b(x03), .c(new_n44_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(x12), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n25_), .O(new_n77_));
  inv1   g53(.a(x08), .O(new_n78_));
  nand3  g54(.a(x03), .b(new_n25_), .c(new_n44_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n78_), .c(new_n50_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n77_), .O(z7));
  inv1   g57(.a(new_n47_), .O(new_n82_));
  aoi21  g58(.a(new_n82_), .b(x00), .c(new_n55_), .O(z8));
endmodule


