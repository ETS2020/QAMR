// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x09), .O(new_n26_));
  nand2  g02(.a(x10), .b(new_n26_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x02), .O(new_n31_));
  nor2   g07(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n29_), .c(new_n32_), .O(new_n33_));
  nand2  g09(.a(x04), .b(x02), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  nor2   g11(.a(x02), .b(x01), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n33_), .c(new_n28_), .O(z0));
  nand2  g15(.a(new_n29_), .b(new_n35_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n27_), .c(new_n25_), .O(new_n41_));
  oai21  g17(.a(new_n35_), .b(new_n30_), .c(new_n31_), .O(new_n42_));
  nand4  g18(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(new_n41_), .O(z1));
  oai21  g21(.a(x10), .b(x07), .c(x03), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n30_), .c(x02), .O(new_n47_));
  nor2   g23(.a(x07), .b(x02), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(x10), .c(new_n26_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nor2   g27(.a(new_n51_), .b(new_n47_), .O(z2));
  nand2  g28(.a(x12), .b(new_n25_), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n53_), .c(new_n27_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(z3));
  aoi21  g32(.a(x10), .b(new_n26_), .c(new_n54_), .O(new_n57_));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  nor2   g35(.a(new_n26_), .b(x07), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(z4));
  inv1   g37(.a(x07), .O(new_n62_));
  nor2   g38(.a(x12), .b(x10), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n62_), .d(new_n25_), .O(z5));
  nand2  g40(.a(new_n26_), .b(x03), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n35_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n36_), .O(new_n67_));
  nand3  g43(.a(new_n32_), .b(x14), .c(new_n25_), .O(new_n68_));
  inv1   g44(.a(x10), .O(new_n69_));
  nand3  g45(.a(new_n58_), .b(new_n69_), .c(new_n62_), .O(new_n70_));
  inv1   g46(.a(new_n70_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n68_), .c(new_n67_), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(z6));
  aoi21  g49(.a(new_n36_), .b(x03), .c(x10), .O(new_n74_));
  inv1   g50(.a(x08), .O(new_n75_));
  nand3  g51(.a(x03), .b(new_n31_), .c(new_n30_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n75_), .c(x07), .O(new_n77_));
  oai22  g53(.a(new_n77_), .b(x10), .c(new_n74_), .d(new_n26_), .O(z7));
  nor2   g54(.a(x09), .b(x00), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(x07), .c(new_n69_), .O(new_n80_));
  nand3  g56(.a(new_n63_), .b(new_n36_), .c(x03), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(x09), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n80_), .O(z8));
endmodule


