// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  inv1   g07(.a(x09), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x01), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(z0));
  nand4  g10(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n35_));
  inv1   g11(.a(x01), .O(new_n36_));
  inv1   g12(.a(x02), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  oai21  g14(.a(new_n38_), .b(new_n36_), .c(new_n37_), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n38_), .c(x00), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n33_), .O(z1));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x09), .O(new_n45_));
  inv1   g21(.a(new_n44_), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n38_), .c(x02), .O(new_n47_));
  oai21  g23(.a(new_n32_), .b(new_n37_), .c(x01), .O(new_n48_));
  oai21  g24(.a(x08), .b(x07), .c(x00), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g26(.a(new_n47_), .b(new_n45_), .c(new_n50_), .O(z2));
  nand2  g27(.a(x12), .b(new_n25_), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n33_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(z3));
  nand2  g31(.a(new_n53_), .b(new_n33_), .O(new_n56_));
  inv1   g32(.a(x08), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(new_n25_), .O(new_n58_));
  nor2   g34(.a(new_n32_), .b(x07), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n58_), .c(new_n56_), .O(z4));
  nor2   g36(.a(x12), .b(x10), .O(new_n61_));
  nor2   g37(.a(x07), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(new_n61_), .c(new_n33_), .d(x13), .O(z5));
  nand3  g39(.a(x14), .b(x02), .c(new_n25_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(x01), .O(new_n65_));
  nand2  g41(.a(new_n32_), .b(x03), .O(new_n66_));
  oai21  g42(.a(new_n32_), .b(x03), .c(new_n37_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n36_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(new_n69_));
  oai21  g45(.a(new_n57_), .b(new_n25_), .c(new_n44_), .O(new_n70_));
  inv1   g46(.a(new_n70_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n33_), .O(z6));
  nand3  g49(.a(x03), .b(new_n37_), .c(new_n36_), .O(new_n74_));
  oai21  g50(.a(x08), .b(new_n36_), .c(new_n74_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(x09), .O(new_n76_));
  nand2  g52(.a(new_n46_), .b(new_n33_), .O(new_n77_));
  nand2  g53(.a(x03), .b(new_n37_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n57_), .c(new_n36_), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(z7));
  oai21  g56(.a(new_n74_), .b(x12), .c(x09), .O(new_n81_));
  nand3  g57(.a(new_n32_), .b(new_n36_), .c(new_n25_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n81_), .c(new_n77_), .O(z8));
endmodule


