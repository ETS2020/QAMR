// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_;
  inv1   g00(.a(x01), .O(new_n25_));
  nand2  g01(.a(x09), .b(x02), .O(new_n26_));
  oai22  g02(.a(new_n26_), .b(x04), .c(x02), .d(new_n25_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x03), .O(new_n28_));
  inv1   g04(.a(new_n26_), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n29_), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n28_), .c(x00), .O(z0));
  inv1   g09(.a(x02), .O(new_n34_));
  nand3  g10(.a(x03), .b(new_n34_), .c(x01), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  or2    g12(.a(x04), .b(x03), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n36_), .c(new_n29_), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n35_), .c(x00), .O(z1));
  inv1   g15(.a(x09), .O(new_n40_));
  nor2   g16(.a(x10), .b(x07), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  oai21  g18(.a(x10), .b(x07), .c(x03), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(new_n42_), .c(new_n25_), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g23(.a(x00), .O(new_n48_));
  aoi21  g24(.a(new_n40_), .b(x02), .c(new_n48_), .O(new_n49_));
  aoi22  g25(.a(new_n49_), .b(new_n47_), .c(new_n44_), .d(new_n34_), .O(z2));
  nor2   g26(.a(x09), .b(new_n34_), .O(new_n51_));
  inv1   g27(.a(new_n51_), .O(new_n52_));
  inv1   g28(.a(x12), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(x00), .O(new_n54_));
  and2   g30(.a(x11), .b(x07), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n54_), .c(new_n52_), .O(z3));
  nor2   g32(.a(new_n46_), .b(new_n48_), .O(new_n57_));
  nor2   g33(.a(new_n40_), .b(x07), .O(new_n58_));
  aoi22  g34(.a(new_n58_), .b(new_n57_), .c(new_n55_), .d(new_n52_), .O(z4));
  nor2   g35(.a(x12), .b(x10), .O(new_n60_));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(new_n60_), .c(new_n52_), .d(x13), .O(z5));
  xor2a  g38(.a(x09), .b(x03), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(x01), .c(new_n34_), .O(new_n64_));
  nand3  g40(.a(x14), .b(x01), .c(new_n48_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n29_), .O(new_n66_));
  oai21  g42(.a(new_n46_), .b(new_n48_), .c(new_n41_), .O(new_n67_));
  aoi21  g43(.a(new_n66_), .b(new_n64_), .c(new_n67_), .O(z6));
  nand3  g44(.a(x03), .b(new_n34_), .c(new_n25_), .O(new_n69_));
  oai21  g45(.a(new_n51_), .b(new_n46_), .c(new_n69_), .O(new_n70_));
  inv1   g46(.a(new_n69_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x09), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n41_), .O(z7));
  oai21  g49(.a(new_n69_), .b(x12), .c(x09), .O(new_n74_));
  nand2  g50(.a(new_n40_), .b(new_n48_), .O(new_n75_));
  and2   g51(.a(new_n75_), .b(new_n41_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n74_), .c(new_n51_), .O(z8));
endmodule


