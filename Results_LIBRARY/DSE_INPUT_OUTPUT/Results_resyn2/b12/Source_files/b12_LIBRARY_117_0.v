// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  inv1   g02(.a(x05), .O(new_n27_));
  oai22  g03(.a(new_n27_), .b(new_n25_), .c(x04), .d(new_n26_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x03), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nor2   g06(.a(new_n26_), .b(new_n25_), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n30_), .c(new_n31_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  nand2  g09(.a(new_n27_), .b(new_n26_), .O(new_n34_));
  nor2   g10(.a(x02), .b(x01), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  nand3  g12(.a(x06), .b(x02), .c(x01), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n36_), .c(new_n27_), .O(new_n38_));
  inv1   g14(.a(x00), .O(new_n39_));
  aoi21  g15(.a(x05), .b(new_n26_), .c(new_n30_), .O(new_n40_));
  oai21  g16(.a(new_n40_), .b(x03), .c(new_n39_), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n38_), .c(new_n34_), .O(z1));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x08), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  nand4  g25(.a(new_n49_), .b(new_n48_), .c(x05), .d(new_n25_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n26_), .c(new_n45_), .O(z2));
  nand2  g27(.a(x12), .b(new_n39_), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n34_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(z3));
  nand2  g31(.a(new_n53_), .b(new_n34_), .O(new_n56_));
  nand2  g32(.a(x08), .b(x00), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  nor2   g34(.a(new_n46_), .b(x07), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n58_), .c(new_n56_), .O(z4));
  nor2   g36(.a(x12), .b(x10), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(x13), .c(new_n43_), .d(new_n39_), .O(new_n62_));
  and2   g38(.a(new_n62_), .b(new_n34_), .O(z5));
  nand2  g39(.a(new_n46_), .b(x03), .O(new_n64_));
  inv1   g40(.a(x03), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n65_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n64_), .c(new_n35_), .O(new_n67_));
  nand3  g43(.a(new_n31_), .b(x14), .c(new_n39_), .O(new_n68_));
  nand2  g44(.a(new_n57_), .b(new_n47_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n34_), .O(z6));
  nand2  g48(.a(x03), .b(new_n25_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n48_), .c(x05), .O(new_n74_));
  nand2  g50(.a(new_n47_), .b(x08), .O(new_n75_));
  aoi21  g51(.a(new_n35_), .b(x03), .c(new_n75_), .O(new_n76_));
  aoi21  g52(.a(new_n74_), .b(new_n26_), .c(new_n76_), .O(z7));
  nand3  g53(.a(new_n61_), .b(x09), .c(new_n43_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n73_), .c(x05), .O(new_n79_));
  nand3  g55(.a(new_n47_), .b(new_n46_), .c(x00), .O(new_n80_));
  inv1   g56(.a(new_n80_), .O(new_n81_));
  aoi21  g57(.a(new_n79_), .b(new_n26_), .c(new_n81_), .O(z8));
endmodule


