// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  nor2   g02(.a(x09), .b(new_n26_), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x09), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(new_n29_), .c(x02), .O(new_n33_));
  inv1   g09(.a(x04), .O(new_n34_));
  inv1   g10(.a(x03), .O(new_n35_));
  aoi21  g11(.a(new_n26_), .b(new_n30_), .c(new_n35_), .O(new_n36_));
  oai21  g12(.a(new_n34_), .b(new_n26_), .c(new_n36_), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n33_), .c(new_n28_), .O(z0));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  nand2  g15(.a(new_n34_), .b(new_n35_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n39_), .c(x02), .O(new_n41_));
  nor2   g17(.a(new_n35_), .b(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(x01), .c(new_n27_), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n41_), .c(new_n28_), .O(z1));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g23(.a(x10), .b(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n29_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n30_), .O(new_n51_));
  aoi22  g27(.a(new_n51_), .b(new_n26_), .c(new_n47_), .d(new_n28_), .O(z2));
  inv1   g28(.a(new_n27_), .O(new_n53_));
  nand2  g29(.a(x12), .b(new_n25_), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  nand2  g33(.a(new_n55_), .b(new_n53_), .O(new_n58_));
  nand2  g34(.a(x08), .b(x00), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(new_n60_));
  nor2   g36(.a(new_n29_), .b(x07), .O(new_n61_));
  aoi21  g37(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(z4));
  nor2   g38(.a(x12), .b(x10), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n45_), .d(new_n25_), .O(new_n64_));
  and2   g40(.a(new_n64_), .b(new_n53_), .O(z5));
  nand3  g41(.a(x14), .b(x01), .c(new_n25_), .O(new_n66_));
  aoi21  g42(.a(new_n66_), .b(x09), .c(new_n26_), .O(new_n67_));
  oai21  g43(.a(new_n35_), .b(x02), .c(x09), .O(new_n68_));
  aoi21  g44(.a(new_n29_), .b(x03), .c(x01), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g46(.a(new_n59_), .b(new_n48_), .O(new_n71_));
  inv1   g47(.a(new_n71_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nor2   g49(.a(new_n73_), .b(new_n67_), .O(z6));
  nand3  g50(.a(x03), .b(new_n26_), .c(new_n30_), .O(new_n75_));
  oai21  g51(.a(x08), .b(new_n26_), .c(new_n75_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x09), .O(new_n77_));
  inv1   g53(.a(new_n48_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n53_), .O(new_n79_));
  nand2  g55(.a(x03), .b(new_n30_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n46_), .c(new_n26_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n79_), .c(new_n77_), .O(z7));
  oai21  g58(.a(new_n75_), .b(x12), .c(x09), .O(new_n83_));
  nand3  g59(.a(new_n29_), .b(new_n26_), .c(new_n25_), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(new_n83_), .c(new_n79_), .O(z8));
endmodule


