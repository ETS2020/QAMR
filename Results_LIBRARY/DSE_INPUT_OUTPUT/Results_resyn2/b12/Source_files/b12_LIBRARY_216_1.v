// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(new_n27_), .c(x05), .O(new_n29_));
  inv1   g05(.a(x00), .O(new_n30_));
  inv1   g06(.a(x02), .O(new_n31_));
  oai21  g07(.a(new_n26_), .b(new_n25_), .c(new_n31_), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nand2  g09(.a(x05), .b(new_n25_), .O(new_n34_));
  oai21  g10(.a(new_n33_), .b(new_n29_), .c(new_n34_), .O(z0));
  nand4  g11(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n36_));
  oai21  g12(.a(x04), .b(x03), .c(new_n36_), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n33_), .c(new_n34_), .O(z1));
  inv1   g14(.a(x07), .O(new_n39_));
  inv1   g15(.a(x10), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(x09), .c(new_n39_), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(new_n42_));
  inv1   g18(.a(x05), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(x03), .c(new_n43_), .O(new_n45_));
  oai21  g21(.a(new_n45_), .b(new_n42_), .c(new_n25_), .O(new_n46_));
  nor2   g22(.a(x08), .b(x07), .O(new_n47_));
  nand2  g23(.a(new_n34_), .b(x00), .O(new_n48_));
  nor2   g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  aoi21  g25(.a(new_n46_), .b(new_n31_), .c(new_n49_), .O(z2));
  and2   g26(.a(x11), .b(x07), .O(new_n51_));
  inv1   g27(.a(x12), .O(new_n52_));
  nor2   g28(.a(new_n52_), .b(x00), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n51_), .c(new_n34_), .O(z3));
  nand4  g30(.a(x09), .b(x08), .c(new_n39_), .d(x00), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n51_), .c(new_n34_), .O(z4));
  nor2   g33(.a(x07), .b(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(x13), .c(new_n52_), .d(new_n40_), .O(new_n59_));
  and2   g35(.a(new_n59_), .b(new_n34_), .O(z5));
  inv1   g36(.a(x09), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(x03), .O(new_n62_));
  nand2  g38(.a(x09), .b(new_n26_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n31_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n43_), .O(new_n65_));
  nand2  g41(.a(x08), .b(x00), .O(new_n66_));
  nand4  g42(.a(x14), .b(x02), .c(x01), .d(new_n30_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n44_), .O(new_n68_));
  aoi21  g44(.a(new_n65_), .b(new_n25_), .c(new_n68_), .O(z6));
  nand2  g45(.a(x03), .b(new_n31_), .O(new_n70_));
  nand2  g46(.a(new_n44_), .b(new_n61_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n70_), .c(new_n43_), .O(new_n72_));
  inv1   g48(.a(new_n70_), .O(new_n73_));
  nand2  g49(.a(new_n44_), .b(x08), .O(new_n74_));
  aoi21  g50(.a(new_n73_), .b(new_n25_), .c(new_n74_), .O(new_n75_));
  aoi21  g51(.a(new_n72_), .b(new_n25_), .c(new_n75_), .O(z7));
  nand4  g52(.a(new_n52_), .b(x09), .c(new_n43_), .d(new_n25_), .O(new_n77_));
  oai22  g53(.a(new_n77_), .b(new_n70_), .c(new_n48_), .d(x09), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n44_), .O(z8));
endmodule


