// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand4  g06(.a(new_n30_), .b(new_n27_), .c(x01), .d(new_n25_), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(z0));
  aoi21  g08(.a(x03), .b(x01), .c(x02), .O(new_n33_));
  nand3  g09(.a(x06), .b(x05), .c(x02), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n29_), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n34_), .c(new_n25_), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(x01), .c(new_n33_), .O(z1));
  inv1   g14(.a(x01), .O(new_n39_));
  inv1   g15(.a(x07), .O(new_n40_));
  inv1   g16(.a(x09), .O(new_n41_));
  inv1   g17(.a(x10), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  oai21  g19(.a(x10), .b(x07), .c(x03), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(new_n43_), .c(new_n39_), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n40_), .O(new_n47_));
  aoi21  g23(.a(x02), .b(new_n39_), .c(new_n25_), .O(new_n48_));
  aoi22  g24(.a(new_n48_), .b(new_n47_), .c(new_n45_), .d(new_n28_), .O(z2));
  nand2  g25(.a(x02), .b(new_n39_), .O(new_n50_));
  and2   g26(.a(x11), .b(x07), .O(new_n51_));
  inv1   g27(.a(x12), .O(new_n52_));
  nor2   g28(.a(new_n52_), .b(x00), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(z3));
  nand4  g30(.a(x09), .b(x08), .c(new_n40_), .d(x00), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n51_), .c(new_n50_), .O(z4));
  nor2   g33(.a(x07), .b(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(x13), .c(new_n52_), .d(new_n42_), .O(new_n59_));
  and2   g35(.a(new_n59_), .b(new_n50_), .O(z5));
  nand2  g36(.a(x09), .b(x03), .O(new_n61_));
  nand2  g37(.a(new_n41_), .b(new_n29_), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n61_), .c(x01), .O(new_n63_));
  nor2   g39(.a(x10), .b(x07), .O(new_n64_));
  nand2  g40(.a(x08), .b(x00), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  oai21  g43(.a(new_n67_), .b(new_n63_), .c(new_n50_), .O(z6));
  oai21  g44(.a(x08), .b(x03), .c(new_n61_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n33_), .O(new_n70_));
  oai21  g46(.a(x08), .b(new_n39_), .c(new_n64_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n50_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n70_), .O(z7));
  nand3  g49(.a(new_n52_), .b(new_n42_), .c(new_n40_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n61_), .c(new_n28_), .O(new_n75_));
  nand3  g51(.a(new_n64_), .b(new_n41_), .c(x00), .O(new_n76_));
  inv1   g52(.a(new_n76_), .O(new_n77_));
  aoi21  g53(.a(new_n75_), .b(new_n39_), .c(new_n77_), .O(z8));
endmodule


