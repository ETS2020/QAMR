// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_;
  inv1   g00(.a(x02), .O(new_n25_));
  nor2   g01(.a(new_n25_), .b(x00), .O(new_n26_));
  inv1   g02(.a(new_n26_), .O(new_n27_));
  nand2  g03(.a(x05), .b(x01), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  oai21  g05(.a(x03), .b(x01), .c(new_n29_), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(new_n27_), .O(z0));
  nand3  g07(.a(x06), .b(x05), .c(x01), .O(new_n32_));
  inv1   g08(.a(x03), .O(new_n33_));
  nand2  g09(.a(new_n29_), .b(new_n33_), .O(new_n34_));
  nand3  g10(.a(new_n34_), .b(new_n32_), .c(new_n26_), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(z1));
  oai21  g12(.a(x08), .b(x07), .c(x00), .O(new_n37_));
  inv1   g13(.a(x07), .O(new_n38_));
  inv1   g14(.a(x10), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(x09), .c(new_n38_), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(new_n41_));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(x03), .c(new_n25_), .O(new_n43_));
  oai21  g19(.a(new_n43_), .b(new_n41_), .c(new_n37_), .O(new_n44_));
  nand2  g20(.a(new_n25_), .b(x01), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(z2));
  inv1   g22(.a(x00), .O(new_n47_));
  nand2  g23(.a(x12), .b(new_n47_), .O(new_n48_));
  nand2  g24(.a(x11), .b(x07), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n45_), .O(new_n50_));
  inv1   g26(.a(new_n50_), .O(z3));
  nand2  g27(.a(new_n49_), .b(new_n45_), .O(new_n52_));
  nand2  g28(.a(x08), .b(x00), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(new_n54_));
  inv1   g30(.a(x09), .O(new_n55_));
  nor2   g31(.a(new_n55_), .b(x07), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n54_), .c(new_n52_), .O(z4));
  inv1   g33(.a(x01), .O(new_n58_));
  nor2   g34(.a(x02), .b(new_n58_), .O(new_n59_));
  nor2   g35(.a(x07), .b(x00), .O(new_n60_));
  inv1   g36(.a(x12), .O(new_n61_));
  nand3  g37(.a(x13), .b(new_n61_), .c(new_n39_), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n60_), .c(new_n59_), .O(z5));
  nand2  g40(.a(new_n55_), .b(x03), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n33_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n25_), .O(new_n67_));
  nand3  g43(.a(new_n26_), .b(x14), .c(x01), .O(new_n68_));
  nand2  g44(.a(new_n53_), .b(new_n42_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n45_), .O(z6));
  nand3  g48(.a(new_n42_), .b(new_n55_), .c(x03), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n58_), .O(new_n74_));
  nand2  g50(.a(x03), .b(new_n25_), .O(new_n75_));
  nand2  g51(.a(new_n42_), .b(x08), .O(new_n76_));
  inv1   g52(.a(new_n76_), .O(new_n77_));
  aoi22  g53(.a(new_n77_), .b(new_n75_), .c(new_n74_), .d(new_n25_), .O(z7));
  nand2  g54(.a(new_n55_), .b(x00), .O(new_n79_));
  nor2   g55(.a(new_n79_), .b(new_n59_), .O(new_n80_));
  nand3  g56(.a(new_n61_), .b(x09), .c(new_n58_), .O(new_n81_));
  nor2   g57(.a(new_n81_), .b(new_n75_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n80_), .c(new_n42_), .O(z8));
endmodule


