// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  nor2   g02(.a(x10), .b(new_n26_), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  nor2   g05(.a(x02), .b(new_n29_), .O(new_n30_));
  nor2   g06(.a(x04), .b(new_n26_), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(new_n30_), .c(x03), .O(new_n32_));
  inv1   g08(.a(x10), .O(new_n33_));
  inv1   g09(.a(x05), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(x04), .c(new_n29_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n33_), .c(x02), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n32_), .c(new_n28_), .O(z0));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n38_), .c(x02), .O(new_n42_));
  aoi21  g18(.a(new_n30_), .b(x03), .c(new_n27_), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n42_), .c(new_n28_), .O(z1));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g23(.a(x09), .O(new_n48_));
  nor2   g24(.a(x10), .b(x07), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  oai21  g26(.a(x10), .b(x07), .c(x03), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n50_), .c(new_n29_), .O(new_n52_));
  aoi22  g28(.a(new_n52_), .b(new_n26_), .c(new_n47_), .d(new_n28_), .O(z2));
  nand2  g29(.a(new_n33_), .b(x02), .O(new_n54_));
  nand2  g30(.a(x12), .b(new_n25_), .O(new_n55_));
  nand2  g31(.a(x11), .b(x07), .O(new_n56_));
  nand3  g32(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(z3));
  nand2  g34(.a(new_n56_), .b(new_n54_), .O(new_n59_));
  nand2  g35(.a(x08), .b(x00), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  nor2   g37(.a(new_n48_), .b(x07), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(z4));
  inv1   g39(.a(x12), .O(new_n64_));
  nor2   g40(.a(x02), .b(x00), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n49_), .c(x13), .d(new_n64_), .O(z5));
  nand2  g42(.a(new_n48_), .b(x03), .O(new_n67_));
  nand2  g43(.a(x09), .b(new_n39_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n67_), .c(new_n29_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n60_), .c(new_n45_), .O(new_n70_));
  aoi21  g46(.a(new_n70_), .b(new_n26_), .c(x10), .O(z6));
  oai21  g47(.a(new_n39_), .b(x01), .c(x08), .O(new_n72_));
  nand3  g48(.a(new_n48_), .b(x03), .c(new_n29_), .O(new_n73_));
  aoi21  g49(.a(new_n73_), .b(new_n72_), .c(x07), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(x02), .c(new_n33_), .O(z7));
  nand3  g51(.a(new_n64_), .b(x03), .c(new_n29_), .O(new_n76_));
  oai21  g52(.a(x09), .b(x00), .c(new_n45_), .O(new_n77_));
  aoi21  g53(.a(new_n76_), .b(x09), .c(new_n77_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(x02), .c(new_n33_), .O(z8));
endmodule


