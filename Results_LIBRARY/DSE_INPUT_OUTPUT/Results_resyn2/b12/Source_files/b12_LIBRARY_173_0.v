// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nor2   g04(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  inv1   g06(.a(new_n30_), .O(new_n31_));
  inv1   g07(.a(x03), .O(new_n32_));
  aoi21  g08(.a(x04), .b(x02), .c(new_n32_), .O(new_n33_));
  aoi22  g09(.a(new_n33_), .b(new_n31_), .c(new_n29_), .d(new_n26_), .O(new_n34_));
  inv1   g10(.a(x12), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n27_), .O(new_n36_));
  oai21  g12(.a(new_n34_), .b(x00), .c(new_n36_), .O(z0));
  inv1   g13(.a(x00), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n32_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n36_), .c(new_n38_), .O(new_n41_));
  oai21  g17(.a(new_n32_), .b(new_n27_), .c(new_n28_), .O(new_n42_));
  nand3  g18(.a(new_n29_), .b(x06), .c(x05), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(new_n41_), .O(z1));
  nor2   g21(.a(x08), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n36_), .b(x00), .O(new_n47_));
  nor2   g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x09), .O(new_n51_));
  inv1   g27(.a(x10), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(x12), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n27_), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n28_), .c(new_n48_), .O(z2));
  inv1   g33(.a(x11), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi22  g35(.a(new_n59_), .b(new_n36_), .c(x12), .d(new_n38_), .O(z3));
  nand2  g36(.a(x08), .b(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  nor2   g38(.a(new_n51_), .b(x07), .O(new_n63_));
  oai21  g39(.a(new_n58_), .b(new_n50_), .c(new_n36_), .O(new_n64_));
  aoi21  g40(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(z4));
  nor2   g41(.a(x10), .b(x07), .O(new_n66_));
  nor2   g42(.a(new_n27_), .b(x00), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n66_), .c(x13), .d(new_n35_), .O(z5));
  nand2  g44(.a(new_n51_), .b(x03), .O(new_n69_));
  nand2  g45(.a(x09), .b(new_n32_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n30_), .O(new_n71_));
  nand3  g47(.a(new_n29_), .b(x14), .c(new_n38_), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n71_), .c(new_n61_), .d(new_n66_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n36_), .O(z6));
  nand2  g50(.a(new_n66_), .b(new_n28_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(new_n69_), .c(x12), .O(new_n76_));
  nand2  g52(.a(new_n66_), .b(x08), .O(new_n77_));
  aoi21  g53(.a(new_n30_), .b(x03), .c(new_n77_), .O(new_n78_));
  aoi21  g54(.a(new_n76_), .b(new_n27_), .c(new_n78_), .O(z7));
  or2    g55(.a(new_n53_), .b(new_n47_), .O(z8));
endmodule


