// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n72_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  oai21  g03(.a(new_n26_), .b(new_n25_), .c(new_n27_), .O(new_n28_));
  nand2  g04(.a(x05), .b(x01), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(x04), .c(x02), .O(new_n30_));
  nand2  g06(.a(x03), .b(new_n26_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x00), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(new_n33_));
  inv1   g09(.a(new_n33_), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n27_), .O(new_n38_));
  nand4  g14(.a(new_n38_), .b(new_n36_), .c(x02), .d(new_n35_), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(z1));
  aoi21  g16(.a(x03), .b(new_n26_), .c(new_n35_), .O(new_n41_));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x08), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n45_), .c(new_n42_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n25_), .O(new_n48_));
  nor2   g24(.a(x03), .b(x02), .O(new_n49_));
  aoi22  g25(.a(new_n49_), .b(new_n48_), .c(new_n44_), .d(new_n41_), .O(z2));
  inv1   g26(.a(x12), .O(new_n51_));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  oai21  g28(.a(new_n51_), .b(x00), .c(new_n52_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n31_), .O(z3));
  nand2  g30(.a(x08), .b(x00), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(new_n56_));
  nor2   g32(.a(new_n45_), .b(x07), .O(new_n57_));
  nand2  g33(.a(new_n52_), .b(new_n31_), .O(new_n58_));
  aoi21  g34(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(z4));
  inv1   g35(.a(new_n31_), .O(new_n60_));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nand3  g37(.a(x13), .b(new_n51_), .c(new_n46_), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(z5));
  nand2  g40(.a(new_n45_), .b(new_n25_), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n27_), .c(x02), .O(new_n66_));
  nand4  g42(.a(x14), .b(x02), .c(x01), .d(new_n35_), .O(new_n67_));
  nor2   g43(.a(x10), .b(x07), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n67_), .c(new_n55_), .O(new_n69_));
  nor2   g45(.a(new_n69_), .b(new_n66_), .O(z6));
  nand3  g46(.a(new_n68_), .b(new_n31_), .c(x08), .O(z7));
  inv1   g47(.a(new_n47_), .O(new_n72_));
  aoi21  g48(.a(new_n72_), .b(x00), .c(new_n60_), .O(z8));
endmodule


