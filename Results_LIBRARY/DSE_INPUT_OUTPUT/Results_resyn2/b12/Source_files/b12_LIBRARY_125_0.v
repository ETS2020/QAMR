// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x03), .O(new_n25_));
  nand2  g01(.a(x05), .b(x02), .O(new_n26_));
  oai21  g02(.a(new_n25_), .b(x02), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(x03), .b(x02), .O(new_n28_));
  nor2   g04(.a(new_n28_), .b(x04), .O(new_n29_));
  aoi21  g05(.a(new_n27_), .b(x01), .c(new_n29_), .O(new_n30_));
  nor2   g06(.a(x05), .b(x04), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(new_n32_));
  oai21  g08(.a(new_n30_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x01), .O(new_n34_));
  inv1   g10(.a(x02), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nor2   g12(.a(new_n31_), .b(x00), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g14(.a(x04), .b(x02), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n25_), .O(new_n40_));
  nand4  g16(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nor2   g18(.a(new_n42_), .b(new_n38_), .O(z1));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  inv1   g24(.a(new_n48_), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(new_n45_), .c(new_n34_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n32_), .O(new_n52_));
  aoi21  g28(.a(new_n50_), .b(new_n35_), .c(new_n52_), .O(z2));
  inv1   g29(.a(x12), .O(new_n54_));
  and2   g30(.a(x11), .b(x07), .O(new_n55_));
  nor2   g31(.a(new_n55_), .b(new_n31_), .O(new_n56_));
  oai21  g32(.a(new_n54_), .b(x00), .c(new_n56_), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(z3));
  nand4  g34(.a(x09), .b(x08), .c(new_n46_), .d(x00), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n55_), .c(new_n32_), .O(z4));
  nand4  g37(.a(new_n44_), .b(new_n37_), .c(x13), .d(new_n54_), .O(z5));
  xor2a  g38(.a(x09), .b(x03), .O(new_n63_));
  nor2   g39(.a(new_n63_), .b(new_n36_), .O(new_n64_));
  nand2  g40(.a(x08), .b(x00), .O(new_n65_));
  inv1   g41(.a(x00), .O(new_n66_));
  nand4  g42(.a(x14), .b(x02), .c(x01), .d(new_n66_), .O(new_n67_));
  nand2  g43(.a(new_n47_), .b(new_n46_), .O(new_n68_));
  nor2   g44(.a(new_n68_), .b(new_n31_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n67_), .c(new_n65_), .O(new_n70_));
  nor2   g46(.a(new_n70_), .b(new_n64_), .O(z6));
  nand3  g47(.a(x03), .b(new_n35_), .c(new_n34_), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x09), .O(new_n74_));
  inv1   g50(.a(x08), .O(new_n75_));
  aoi21  g51(.a(new_n72_), .b(new_n75_), .c(new_n68_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n74_), .c(new_n31_), .O(z7));
  oai21  g53(.a(new_n72_), .b(x12), .c(x09), .O(new_n78_));
  inv1   g54(.a(x09), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n66_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n78_), .c(new_n69_), .O(z8));
endmodule


