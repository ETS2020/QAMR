// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x10), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  nand3  g03(.a(x03), .b(x01), .c(new_n27_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  nand2  g06(.a(x05), .b(x01), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(new_n27_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n30_), .O(z0));
  inv1   g11(.a(x06), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  inv1   g13(.a(x04), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n37_), .c(new_n25_), .O(new_n39_));
  oai21  g15(.a(new_n31_), .b(new_n36_), .c(new_n39_), .O(new_n40_));
  nand4  g16(.a(new_n26_), .b(x03), .c(new_n25_), .d(x01), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n40_), .c(x00), .O(z1));
  oai21  g18(.a(x08), .b(x07), .c(x00), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x02), .O(new_n44_));
  inv1   g20(.a(x01), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  nand2  g22(.a(x09), .b(new_n46_), .O(new_n47_));
  nand3  g23(.a(x07), .b(new_n37_), .c(new_n27_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g25(.a(x08), .b(x00), .O(new_n50_));
  nand4  g26(.a(new_n50_), .b(new_n49_), .c(new_n26_), .d(new_n45_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n44_), .O(z2));
  inv1   g28(.a(x12), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  oai21  g30(.a(new_n53_), .b(x00), .c(new_n54_), .O(new_n55_));
  nor2   g31(.a(new_n26_), .b(x02), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n55_), .O(z3));
  oai21  g34(.a(new_n50_), .b(new_n47_), .c(new_n54_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n57_), .O(z4));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nand3  g37(.a(x13), .b(new_n53_), .c(new_n26_), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n61_), .c(new_n56_), .O(z5));
  nand3  g40(.a(x14), .b(x01), .c(new_n27_), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n26_), .c(new_n25_), .O(new_n66_));
  nand2  g42(.a(new_n25_), .b(new_n45_), .O(new_n67_));
  xor2a  g43(.a(x09), .b(x03), .O(new_n68_));
  aoi21  g44(.a(x08), .b(x00), .c(x07), .O(new_n69_));
  oai21  g45(.a(new_n68_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  aoi21  g46(.a(new_n70_), .b(new_n26_), .c(new_n66_), .O(z6));
  nand3  g47(.a(x03), .b(new_n25_), .c(new_n45_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n26_), .c(x08), .O(new_n73_));
  inv1   g49(.a(x09), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(x03), .c(new_n25_), .d(new_n45_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n46_), .c(new_n56_), .O(z7));
  nand3  g53(.a(new_n53_), .b(x03), .c(new_n45_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n47_), .c(new_n26_), .O(new_n79_));
  nand4  g55(.a(new_n26_), .b(new_n74_), .c(new_n46_), .d(x00), .O(new_n80_));
  inv1   g56(.a(new_n80_), .O(new_n81_));
  aoi21  g57(.a(new_n79_), .b(new_n25_), .c(new_n81_), .O(z8));
endmodule


