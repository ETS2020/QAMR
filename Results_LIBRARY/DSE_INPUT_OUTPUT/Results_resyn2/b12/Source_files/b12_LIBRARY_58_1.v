// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(x05), .c(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x04), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(x01), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  oai21  g14(.a(x04), .b(x03), .c(x02), .O(new_n39_));
  oai22  g15(.a(new_n39_), .b(new_n38_), .c(new_n28_), .d(new_n36_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  nand2  g17(.a(x05), .b(new_n27_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(z1));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n44_), .c(new_n35_), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(x09), .c(new_n44_), .O(new_n48_));
  inv1   g24(.a(new_n48_), .O(new_n49_));
  nor2   g25(.a(x10), .b(x07), .O(new_n50_));
  nor2   g26(.a(new_n50_), .b(x03), .O(new_n51_));
  nor2   g27(.a(x05), .b(x01), .O(new_n52_));
  oai21  g28(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n27_), .c(new_n46_), .O(z2));
  inv1   g30(.a(x12), .O(new_n55_));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  aoi21  g32(.a(x05), .b(new_n27_), .c(new_n56_), .O(new_n57_));
  oai21  g33(.a(new_n55_), .b(x00), .c(new_n57_), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(z3));
  nand4  g35(.a(x09), .b(x08), .c(new_n44_), .d(x00), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(new_n56_), .c(new_n42_), .O(z4));
  nor2   g38(.a(x07), .b(x00), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n55_), .d(new_n47_), .O(new_n64_));
  and2   g40(.a(new_n64_), .b(new_n42_), .O(z5));
  inv1   g41(.a(x01), .O(new_n66_));
  inv1   g42(.a(x09), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(x03), .O(new_n68_));
  nand2  g44(.a(x09), .b(new_n36_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n66_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n31_), .O(new_n71_));
  nand2  g47(.a(x08), .b(x00), .O(new_n72_));
  nand4  g48(.a(x14), .b(x02), .c(x01), .d(new_n35_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n72_), .c(new_n50_), .O(new_n74_));
  aoi21  g50(.a(new_n71_), .b(new_n27_), .c(new_n74_), .O(z6));
  nor2   g51(.a(new_n36_), .b(x01), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n50_), .c(new_n67_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n31_), .O(new_n78_));
  nand2  g54(.a(new_n50_), .b(x08), .O(new_n79_));
  aoi21  g55(.a(new_n76_), .b(new_n27_), .c(new_n79_), .O(new_n80_));
  aoi21  g56(.a(new_n78_), .b(new_n27_), .c(new_n80_), .O(z7));
  nand3  g57(.a(new_n55_), .b(x03), .c(new_n66_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n48_), .c(new_n31_), .O(new_n83_));
  nor2   g59(.a(x09), .b(new_n35_), .O(new_n84_));
  aoi22  g60(.a(new_n84_), .b(new_n50_), .c(new_n83_), .d(new_n27_), .O(z8));
endmodule


