// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x10), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x07), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x04), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(x02), .c(x01), .O(new_n30_));
  nand2  g06(.a(x04), .b(x02), .O(new_n31_));
  inv1   g07(.a(x01), .O(new_n32_));
  inv1   g08(.a(x02), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g10(.a(new_n34_), .b(new_n31_), .c(x03), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n30_), .c(new_n27_), .O(z0));
  inv1   g12(.a(x07), .O(new_n37_));
  aoi21  g13(.a(x10), .b(new_n37_), .c(x00), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n38_), .O(new_n42_));
  oai21  g18(.a(new_n39_), .b(new_n32_), .c(new_n33_), .O(new_n43_));
  nand4  g19(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nor2   g21(.a(new_n45_), .b(new_n42_), .O(z1));
  inv1   g22(.a(x08), .O(new_n47_));
  oai21  g23(.a(x10), .b(new_n47_), .c(new_n37_), .O(new_n48_));
  nand2  g24(.a(x07), .b(x03), .O(new_n49_));
  inv1   g25(.a(x09), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n49_), .c(new_n32_), .O(new_n52_));
  aoi21  g28(.a(x10), .b(new_n37_), .c(x02), .O(new_n53_));
  aoi22  g29(.a(new_n53_), .b(new_n52_), .c(new_n48_), .d(x00), .O(z2));
  and2   g30(.a(x11), .b(x07), .O(new_n55_));
  aoi21  g31(.a(new_n38_), .b(x12), .c(new_n55_), .O(z3));
  nor2   g32(.a(x10), .b(new_n47_), .O(new_n57_));
  nand3  g33(.a(x09), .b(new_n37_), .c(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n57_), .c(new_n55_), .O(z4));
  inv1   g36(.a(x12), .O(new_n61_));
  nand3  g37(.a(x13), .b(new_n61_), .c(new_n25_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n26_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n37_), .O(z5));
  nor2   g40(.a(x02), .b(x01), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n39_), .O(new_n66_));
  nand2  g42(.a(new_n50_), .b(x03), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n69_));
  nand2  g45(.a(x08), .b(x00), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n26_), .c(new_n37_), .O(new_n71_));
  inv1   g47(.a(new_n71_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n69_), .c(new_n68_), .O(new_n73_));
  inv1   g49(.a(new_n73_), .O(z6));
  nand3  g50(.a(new_n65_), .b(x09), .c(x03), .O(new_n75_));
  oai21  g51(.a(new_n34_), .b(new_n39_), .c(new_n47_), .O(new_n76_));
  nand4  g52(.a(new_n76_), .b(new_n75_), .c(new_n26_), .d(new_n37_), .O(z7));
  nand4  g53(.a(new_n65_), .b(new_n61_), .c(x09), .d(x03), .O(new_n78_));
  aoi21  g54(.a(new_n50_), .b(x00), .c(x10), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n37_), .O(z8));
endmodule


