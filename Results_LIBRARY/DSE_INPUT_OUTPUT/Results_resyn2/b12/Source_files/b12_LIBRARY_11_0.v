// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nor2   g03(.a(x02), .b(x01), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  aoi21  g06(.a(x04), .b(x02), .c(new_n30_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  nor2   g08(.a(x14), .b(x04), .O(new_n33_));
  inv1   g09(.a(new_n33_), .O(new_n34_));
  oai21  g10(.a(new_n32_), .b(x00), .c(new_n34_), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand3  g12(.a(new_n34_), .b(new_n29_), .c(new_n36_), .O(new_n37_));
  nand2  g13(.a(x06), .b(x05), .O(new_n38_));
  inv1   g14(.a(x02), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  oai21  g16(.a(new_n40_), .b(new_n39_), .c(new_n30_), .O(new_n41_));
  oai21  g17(.a(new_n38_), .b(new_n26_), .c(new_n41_), .O(new_n42_));
  nor2   g18(.a(new_n42_), .b(new_n37_), .O(z1));
  oai21  g19(.a(x08), .b(x07), .c(x00), .O(new_n44_));
  inv1   g20(.a(x01), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n39_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n44_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n34_), .O(z2));
  inv1   g30(.a(x12), .O(new_n55_));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n33_), .O(new_n57_));
  oai21  g33(.a(new_n55_), .b(x00), .c(new_n57_), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(z3));
  nand4  g35(.a(x09), .b(x08), .c(new_n46_), .d(x00), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(new_n56_), .c(new_n34_), .O(z4));
  nand3  g38(.a(x13), .b(new_n55_), .c(new_n48_), .O(new_n63_));
  nor3   g39(.a(new_n63_), .b(x07), .c(x00), .O(new_n64_));
  nor2   g40(.a(new_n64_), .b(new_n33_), .O(z5));
  nor2   g41(.a(x10), .b(x07), .O(new_n66_));
  nand2  g42(.a(x08), .b(x00), .O(new_n67_));
  xnor2a g43(.a(x09), .b(x03), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n28_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n36_), .O(new_n71_));
  inv1   g47(.a(new_n71_), .O(new_n72_));
  aoi21  g48(.a(new_n70_), .b(new_n34_), .c(new_n72_), .O(z6));
  inv1   g49(.a(x08), .O(new_n74_));
  oai21  g50(.a(new_n29_), .b(new_n30_), .c(new_n74_), .O(new_n75_));
  nand3  g51(.a(new_n28_), .b(x09), .c(x03), .O(new_n76_));
  nand4  g52(.a(new_n76_), .b(new_n75_), .c(new_n66_), .d(new_n34_), .O(z7));
  oai22  g53(.a(new_n76_), .b(x12), .c(x09), .d(new_n36_), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n66_), .c(new_n33_), .O(z8));
endmodule


