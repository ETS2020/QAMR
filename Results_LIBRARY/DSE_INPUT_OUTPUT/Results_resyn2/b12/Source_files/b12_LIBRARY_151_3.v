// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(x04), .b(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nor2   g06(.a(new_n27_), .b(new_n25_), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n30_), .c(new_n31_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x03), .O(new_n34_));
  nand2  g10(.a(new_n30_), .b(new_n34_), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n35_), .c(x00), .O(new_n37_));
  inv1   g13(.a(x00), .O(new_n38_));
  nand3  g14(.a(new_n26_), .b(x03), .c(new_n38_), .O(new_n39_));
  oai21  g15(.a(new_n37_), .b(new_n27_), .c(new_n39_), .O(z1));
  inv1   g16(.a(x10), .O(new_n41_));
  oai21  g17(.a(x08), .b(x02), .c(x00), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n41_), .c(x09), .O(new_n43_));
  inv1   g19(.a(x08), .O(new_n44_));
  nand4  g20(.a(x10), .b(new_n44_), .c(new_n34_), .d(new_n27_), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n43_), .c(x07), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  nor3   g23(.a(new_n47_), .b(x03), .c(x00), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n46_), .c(new_n25_), .O(new_n49_));
  nand2  g25(.a(x02), .b(new_n38_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(z2));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  nand2  g28(.a(x12), .b(new_n38_), .O(new_n53_));
  nand2  g29(.a(x02), .b(x00), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(z3));
  inv1   g32(.a(x07), .O(new_n57_));
  nand3  g33(.a(x09), .b(x08), .c(new_n57_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n27_), .O(new_n59_));
  aoi22  g35(.a(new_n59_), .b(x00), .c(x11), .d(x07), .O(z4));
  nor2   g36(.a(x12), .b(x10), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(x13), .c(new_n57_), .d(new_n38_), .O(new_n62_));
  and2   g38(.a(new_n62_), .b(new_n54_), .O(z5));
  inv1   g39(.a(new_n47_), .O(new_n64_));
  nand2  g40(.a(x14), .b(x01), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(x02), .c(new_n38_), .O(new_n66_));
  nor2   g42(.a(x09), .b(new_n34_), .O(new_n67_));
  inv1   g43(.a(x09), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(x03), .c(new_n25_), .O(new_n69_));
  aoi21  g45(.a(x08), .b(x00), .c(x02), .O(new_n70_));
  oai21  g46(.a(new_n69_), .b(new_n67_), .c(new_n70_), .O(new_n71_));
  aoi21  g47(.a(new_n71_), .b(new_n66_), .c(new_n64_), .O(z6));
  nor2   g48(.a(x08), .b(new_n27_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n64_), .c(new_n54_), .O(new_n74_));
  nand2  g50(.a(x03), .b(new_n25_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(x08), .O(new_n76_));
  nand3  g52(.a(new_n68_), .b(x03), .c(new_n25_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n76_), .c(new_n27_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n74_), .O(z7));
  oai21  g55(.a(new_n75_), .b(x12), .c(x09), .O(new_n80_));
  nand2  g56(.a(new_n68_), .b(new_n38_), .O(new_n81_));
  nand4  g57(.a(new_n81_), .b(new_n80_), .c(new_n47_), .d(new_n27_), .O(z8));
endmodule


