// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand3  g01(.a(x06), .b(new_n25_), .c(x01), .O(new_n26_));
  oai21  g02(.a(x04), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x03), .O(new_n28_));
  inv1   g04(.a(x05), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(x01), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(x00), .O(z0));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x03), .O(new_n34_));
  nor2   g10(.a(x04), .b(x03), .O(new_n35_));
  inv1   g11(.a(x06), .O(new_n36_));
  nand2  g12(.a(x05), .b(x01), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(x02), .c(new_n36_), .O(new_n38_));
  oai22  g14(.a(new_n38_), .b(new_n35_), .c(new_n26_), .d(new_n34_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n33_), .O(new_n40_));
  nand2  g16(.a(new_n36_), .b(new_n25_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n40_), .O(z1));
  oai21  g18(.a(x08), .b(x07), .c(x00), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x09), .O(new_n45_));
  oai21  g21(.a(x10), .b(x07), .c(new_n34_), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n45_), .c(x01), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(x02), .c(new_n43_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n41_), .O(z2));
  nand2  g25(.a(x12), .b(new_n33_), .O(new_n50_));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n50_), .c(new_n41_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(z3));
  nand2  g29(.a(x08), .b(x00), .O(new_n54_));
  inv1   g30(.a(x07), .O(new_n55_));
  nand2  g31(.a(x09), .b(new_n55_), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n54_), .c(new_n51_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n41_), .O(z4));
  nor2   g34(.a(x12), .b(x10), .O(new_n59_));
  nor2   g35(.a(x07), .b(x00), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(new_n59_), .c(new_n41_), .d(x13), .O(z5));
  aoi21  g37(.a(x09), .b(new_n34_), .c(x01), .O(new_n62_));
  oai21  g38(.a(x09), .b(new_n34_), .c(new_n62_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(x06), .O(new_n64_));
  nand4  g40(.a(x14), .b(x02), .c(x01), .d(new_n33_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n54_), .c(new_n44_), .O(new_n66_));
  aoi21  g42(.a(new_n64_), .b(new_n25_), .c(new_n66_), .O(z6));
  inv1   g43(.a(x09), .O(new_n68_));
  nor2   g44(.a(new_n34_), .b(x01), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n44_), .c(new_n68_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x06), .O(new_n71_));
  nand2  g47(.a(new_n44_), .b(x08), .O(new_n72_));
  aoi21  g48(.a(new_n69_), .b(new_n25_), .c(new_n72_), .O(new_n73_));
  aoi21  g49(.a(new_n71_), .b(new_n25_), .c(new_n73_), .O(z7));
  inv1   g50(.a(x12), .O(new_n75_));
  nand2  g51(.a(new_n69_), .b(new_n75_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n45_), .c(x06), .O(new_n77_));
  nand3  g53(.a(new_n44_), .b(new_n68_), .c(x00), .O(new_n78_));
  inv1   g54(.a(new_n78_), .O(new_n79_));
  aoi21  g55(.a(new_n77_), .b(new_n25_), .c(new_n79_), .O(z8));
endmodule


