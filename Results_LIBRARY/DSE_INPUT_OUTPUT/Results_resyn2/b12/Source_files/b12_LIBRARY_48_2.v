// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x06), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x00), .O(new_n27_));
  inv1   g03(.a(new_n27_), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  nor2   g05(.a(x02), .b(new_n29_), .O(new_n30_));
  inv1   g06(.a(x02), .O(new_n31_));
  nor2   g07(.a(x04), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(new_n30_), .c(x03), .O(new_n33_));
  inv1   g09(.a(x05), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(x04), .c(new_n29_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n25_), .c(x02), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n33_), .c(new_n28_), .O(z0));
  nand2  g13(.a(x05), .b(x01), .O(new_n38_));
  or2    g14(.a(x04), .b(x03), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nor2   g16(.a(x06), .b(new_n31_), .O(new_n41_));
  aoi21  g17(.a(new_n30_), .b(x03), .c(new_n41_), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n40_), .c(new_n28_), .O(z1));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nor2   g23(.a(x10), .b(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g25(.a(x10), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n44_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x03), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n49_), .c(new_n29_), .O(new_n53_));
  aoi22  g29(.a(new_n53_), .b(new_n31_), .c(new_n46_), .d(new_n28_), .O(z2));
  inv1   g30(.a(x00), .O(new_n55_));
  nand2  g31(.a(x12), .b(new_n55_), .O(new_n56_));
  and2   g32(.a(x11), .b(x07), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(new_n41_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(z3));
  nand4  g36(.a(x09), .b(x08), .c(new_n44_), .d(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n57_), .c(new_n26_), .O(z4));
  nor2   g39(.a(x12), .b(x10), .O(new_n64_));
  nor2   g40(.a(x07), .b(x00), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n64_), .c(new_n26_), .d(x13), .O(z5));
  xor2a  g42(.a(x09), .b(x03), .O(new_n67_));
  nor3   g43(.a(new_n67_), .b(x02), .c(x01), .O(new_n68_));
  nand2  g44(.a(x08), .b(x00), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n55_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n48_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n68_), .c(new_n26_), .O(z6));
  aoi21  g48(.a(x03), .b(new_n29_), .c(x08), .O(new_n73_));
  nand3  g49(.a(x09), .b(x03), .c(new_n29_), .O(new_n74_));
  inv1   g50(.a(new_n74_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(new_n73_), .c(new_n31_), .O(new_n76_));
  aoi21  g52(.a(new_n45_), .b(x02), .c(new_n51_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n76_), .c(new_n41_), .O(z7));
  inv1   g54(.a(x12), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n31_), .O(new_n80_));
  oai22  g56(.a(new_n80_), .b(new_n74_), .c(new_n27_), .d(x09), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n48_), .O(z8));
endmodule


