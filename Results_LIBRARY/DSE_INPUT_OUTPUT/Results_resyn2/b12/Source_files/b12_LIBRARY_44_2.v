// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x01), .O(new_n25_));
  nand2  g01(.a(x06), .b(x02), .O(new_n26_));
  oai22  g02(.a(new_n26_), .b(x04), .c(x02), .d(new_n25_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x03), .O(new_n28_));
  inv1   g04(.a(new_n26_), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n29_), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n28_), .c(x00), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x02), .O(new_n35_));
  nand3  g11(.a(x03), .b(new_n35_), .c(x01), .O(new_n36_));
  nor2   g12(.a(new_n30_), .b(new_n25_), .O(new_n37_));
  oai21  g13(.a(x04), .b(x03), .c(x02), .O(new_n38_));
  oai21  g14(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n34_), .O(new_n40_));
  nor2   g16(.a(x06), .b(new_n35_), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n40_), .O(z1));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  inv1   g24(.a(new_n48_), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(new_n45_), .c(new_n25_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n42_), .O(new_n52_));
  aoi21  g28(.a(new_n50_), .b(new_n35_), .c(new_n52_), .O(z2));
  inv1   g29(.a(x12), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(x00), .c(new_n55_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n42_), .O(z3));
  inv1   g33(.a(x09), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x07), .O(new_n59_));
  inv1   g35(.a(x08), .O(new_n60_));
  nor2   g36(.a(new_n60_), .b(new_n34_), .O(new_n61_));
  nand2  g37(.a(new_n55_), .b(new_n42_), .O(new_n62_));
  aoi21  g38(.a(new_n61_), .b(new_n59_), .c(new_n62_), .O(z4));
  nand3  g39(.a(x13), .b(new_n54_), .c(new_n47_), .O(new_n64_));
  nor3   g40(.a(new_n64_), .b(x07), .c(x00), .O(new_n65_));
  nor2   g41(.a(new_n65_), .b(new_n41_), .O(z5));
  nand3  g42(.a(x14), .b(x01), .c(new_n34_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n29_), .O(new_n68_));
  nor2   g44(.a(new_n58_), .b(x03), .O(new_n69_));
  inv1   g45(.a(x03), .O(new_n70_));
  oai21  g46(.a(x09), .b(new_n70_), .c(new_n25_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n69_), .c(new_n35_), .O(new_n72_));
  oai21  g48(.a(new_n60_), .b(new_n34_), .c(new_n44_), .O(new_n73_));
  aoi21  g49(.a(new_n72_), .b(new_n68_), .c(new_n73_), .O(z6));
  aoi21  g50(.a(x03), .b(new_n25_), .c(x08), .O(new_n75_));
  nand3  g51(.a(x09), .b(x03), .c(new_n25_), .O(new_n76_));
  inv1   g52(.a(new_n76_), .O(new_n77_));
  oai21  g53(.a(new_n77_), .b(new_n75_), .c(new_n35_), .O(new_n78_));
  inv1   g54(.a(new_n44_), .O(new_n79_));
  aoi21  g55(.a(new_n60_), .b(x02), .c(new_n79_), .O(new_n80_));
  aoi21  g56(.a(new_n80_), .b(new_n78_), .c(new_n41_), .O(z7));
  nand2  g57(.a(new_n58_), .b(x00), .O(new_n82_));
  nand2  g58(.a(new_n54_), .b(new_n35_), .O(new_n83_));
  oai22  g59(.a(new_n83_), .b(new_n76_), .c(new_n82_), .d(new_n41_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n44_), .O(z8));
endmodule


