// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_;
  nand2  g00(.a(x05), .b(x01), .O(new_n25_));
  nor2   g01(.a(x03), .b(x01), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x04), .c(new_n25_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x02), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand4  g05(.a(x04), .b(x03), .c(new_n29_), .d(x01), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(x00), .O(z0));
  nand3  g07(.a(x06), .b(x05), .c(x01), .O(new_n32_));
  inv1   g08(.a(x03), .O(new_n33_));
  inv1   g09(.a(x04), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(new_n32_), .c(x02), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n30_), .c(x00), .O(z1));
  inv1   g13(.a(x00), .O(new_n38_));
  inv1   g14(.a(x07), .O(new_n39_));
  inv1   g15(.a(x08), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(new_n41_));
  inv1   g17(.a(x01), .O(new_n42_));
  oai21  g18(.a(x10), .b(x07), .c(x03), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  nor2   g20(.a(x10), .b(x07), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand4  g22(.a(new_n46_), .b(new_n43_), .c(x04), .d(new_n42_), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n29_), .c(new_n41_), .O(z2));
  nand2  g24(.a(new_n34_), .b(new_n29_), .O(new_n49_));
  inv1   g25(.a(x12), .O(new_n50_));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  oai21  g27(.a(new_n50_), .b(x00), .c(new_n51_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n49_), .O(z3));
  nand2  g29(.a(x08), .b(x00), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  nor2   g31(.a(new_n44_), .b(x07), .O(new_n56_));
  nand2  g32(.a(new_n51_), .b(new_n49_), .O(new_n57_));
  aoi21  g33(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(z4));
  nor2   g34(.a(x12), .b(x10), .O(new_n59_));
  nor2   g35(.a(x07), .b(x00), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(new_n59_), .c(new_n49_), .d(x13), .O(z5));
  nand2  g37(.a(new_n44_), .b(new_n33_), .O(new_n62_));
  nand2  g38(.a(x09), .b(x03), .O(new_n63_));
  nand2  g39(.a(new_n29_), .b(new_n42_), .O(new_n64_));
  aoi21  g40(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n38_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n54_), .c(new_n45_), .O(new_n67_));
  oai21  g43(.a(new_n67_), .b(new_n65_), .c(new_n49_), .O(z6));
  oai21  g44(.a(new_n63_), .b(x01), .c(x04), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n29_), .O(new_n70_));
  nand3  g46(.a(x03), .b(new_n29_), .c(new_n42_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n40_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n45_), .O(z7));
  nand3  g49(.a(new_n49_), .b(new_n44_), .c(x00), .O(new_n74_));
  nand3  g50(.a(new_n50_), .b(x09), .c(x04), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(new_n71_), .c(new_n74_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n45_), .O(z8));
endmodule


